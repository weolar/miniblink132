...
{
    ImageInfo info = { gfx::Size(1920, 1080), viz::SinglePlaneFormat::kRGBA_8888, {} };
...
{
    ImageInfo info = { gfx::Size(1024, 768), viz::SinglePlaneFormat::kRGBA_8888, {} };
    const auto max_pool_size = 1;
    auto pool = SharedImagePool<TestClientImage>::Create(info, test_sii_, max_pool_size);
...
TEST_F(SharedImagePoolTest, MaxPoolSizeEnforcement)
{
    ...
    auto pool = SharedImagePool<ClientImage>::Create(info, test_sii_, max_pool_size);
...
{
    ImageInfo info = { gfx::Size(800, 600), viz::SinglePlaneFormat::kRGBA_8888, {} };
    const size_t max_pool_size = 1;
    auto pool = SharedImagePool<ClientImage>::Create(info, test_sii_, max_pool_size);
...
{
    ImageInfo info = { gfx::Size(1280, 720), viz::SinglePlaneFormat::kRGBA_8888, {} };
...
{
    ImageInfo pool1_info = { gfx::Size(1920, 1080), viz::SinglePlaneFormat::kRGBA_8888, {} };
    ImageInfo pool2_info = { gfx::Size(800, 600), viz::SinglePlaneFormat::kRGBA_8888, {} };
...
{
    // Initial ImageInfo.
    ImageInfo initial_info = { gfx::Size(1920, 1080), viz::SinglePlaneFormat::kRGBA_8888, {} };

    // Create the pool with an initial configuration.
    auto pool = SharedImagePool<ClientImage>::Create(initial_info, test_sii_.get(), /*max_pool_size=*/2);

    // Create a different ImageInfo.
    ImageInfo new_info = { gfx::Size(1280, 720), viz::SinglePlaneFormat::kBGRA_8888, {} };
...
TEST_F(SharedImagePoolTest, SetReleaseSyncToken)
{
    ...
    SyncToken release_sync_token(gpu::CommandBufferNamespace::GPU_IO, gpu::CommandBufferId::FromUnsafeValue(1), 12345);
...
TEST_F(SharedImagePoolTest, CreatesMappableSharedImageWhenBufferUsageIsSet)
{
    ...
    ImageInfo info = { gfx::Size(100, 100), viz::SinglePlaneFormat::kRGBA_8888, {}, gfx::BufferUsage::GPU_READ };
    auto pool = SharedImagePool<ClientImage>::Create(info, test_sii_);

    // Expect CreateSharedImage to be called with buffer_usage specified.
    EXPECT_CALL(*test_sii_, DoCreateSharedImage(gfx::Size(100, 100), viz::SinglePlaneFormat::kRGBA_8888, gpu::kNullSurfaceHandle, gfx::BufferUsage::GPU_READ));

    scoped_refptr<ClientImage> image = pool->GetImage();
    EXPECT_NE(image, nullptr);
    EXPECT_EQ(image->GetSharedImage()->buffer_usage(), gfx::BufferUsage::GPU_READ);
}
...
TEST_F(SharedImagePoolTest, DoesNotReuseSharedImageWithDifferentBufferUsage)
{
    ...
    EXPECT_EQ(image1->GetSharedImage()->buffer_usage(), gfx::BufferUsage::GPU_READ);
    ...
    EXPECT_CALL(*test_sii_,
        DoCreateSharedImage(gfx::Size(100, 100), viz::SinglePlaneFormat::kRGBA_8888, gpu::kNullSurfaceHandle, gfx::BufferUsage::GPU_READ_CPU_READ_WRITE))
        .Times(1);
    ...
    EXPECT_EQ(image2->GetSharedImage()->buffer_usage(), gfx::BufferUsage::GPU_READ_CPU_READ_WRITE);
