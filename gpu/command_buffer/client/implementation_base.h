...
class GLES2_IMPL_EXPORT ImplementationBase : public base::trace_event::MemoryDumpProvider, public ContextSupport, public GpuControlClient {
public:
    // The maximum result size from simple GL get commands.
    static const uint32_t kMaxSizeOfSimpleResult = 16 * sizeof(uint32_t); // NOLINT.
    ...
    ImplementationBase(CommandBufferHelper* helper, TransferBufferInterface* transfer_buffer, GpuControl* gpu_control);
...
protected:
    ...
    template <typename T> ScopedResultPtr<T> GetResultAs()
    {
    ...
    bool GetVerifiedSyncTokenForIPC(const SyncToken& sync_token, SyncToken* verified_sync_token);
...
private:
    ...
    virtual void SetGLError(GLenum error, const char* function_name, const char* msg) = 0;
