...
template <typename> class ScopedResultPtr;
...
public:
    ...
    virtual bool Initialize(
        unsigned int buffer_size, unsigned int result_size, unsigned int min_buffer_size, unsigned int max_buffer_size, unsigned int alignment)
        = 0;
...
protected:
    template <typename> friend class ScopedResultPtr;
...
public:
    ...
    bool Initialize(unsigned int default_buffer_size, unsigned int result_size, unsigned int min_buffer_size, unsigned int max_buffer_size,
...
public:
    ScopedTransferBufferPtr(unsigned int size, CommandBufferHelper* helper, TransferBufferInterface* transfer_buffer)
        : buffer_(nullptr)
        , size_(0)
        , helper_(helper)
...
template <typename T> class ScopedTransferBufferArray : public ScopedTransferBufferPtr {
public:
    ScopedTransferBufferArray(unsigned int num_elements, CommandBufferHelper* helper, TransferBufferInterface* transfer_buffer)
        : ScopedTransferBufferPtr(num_elements * sizeof(T), helper, transfer_buffer)
    {
    }
...
template <typename T> class ScopedResultPtr {
public:
    ...
    ScopedResultPtr(ScopedResultPtr<T>&& other)
    {
        *this = std::move(other);
    }
