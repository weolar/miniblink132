...
public:
    ...
    //   the pointer to the allocated memory block, or NULL if out of
    //   memory.
    template <typename T> T* AllocTyped(uint32_t count)
    {
    ...
    {
        return pointer ? static_cast<FencedAllocator::Offset>(static_cast<char*>(pointer) - static_cast<char*>(base_)) : FencedAllocator::kInvalidOffset;
    }
    ...
    {
        return allocator_.GetBlockStatusForTest(GetOffset(pointer), token_if_pending);
    }
