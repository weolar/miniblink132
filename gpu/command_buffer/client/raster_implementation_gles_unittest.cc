...
public:
    ...
    MOCK_METHOD2(VerifySyncTokensCHROMIUM, void(GLbyte** sync_tokens, GLsizei count));
    ...
    MOCK_METHOD3(GetQueryObjectuivEXT, void(GLuint id, GLenum pname, GLuint* params));
    MOCK_METHOD3(GetQueryObjectui64vEXT, void(GLuint id, GLenum pname, GLuint64* params));
    ...
    // Mailboxes.
    MOCK_METHOD1(CreateAndTexStorage2DSharedImageCHROMIUM, GLuint(const GLbyte* mailbox));
    MOCK_METHOD2(BeginSharedImageAccessDirectCHROMIUM, void(GLuint texture, GLenum mode));
    ...
    MOCK_METHOD2(TraceBeginCHROMIUM, void(const char* category_name, const char* trace_name));
...
public:
    ...
    void SignalSyncToken(const SyncToken& sync_token, base::OnceClosure callback) override
    {
    }
...
protected:
    ...
    void SetUp() override
    {
        ...
        ri_ = std::make_unique<RasterImplementationGLES>(gl_.get(), &support_, gpu::Capabilities());
    }
...
TEST_F(RasterImplementationGLESTest, VerifySyncTokensCHROMIUM)
{
    ...
    EXPECT_CALL(*gl_, VerifySyncTokensCHROMIUM(sync_tokens, kNumSyncTokens)).Times(1);
...
TEST_F(RasterImplementationGLESTest, GetGraphicsResetStatusKHR)
{
    ...
    EXPECT_CALL(*gl_, GetGraphicsResetStatusKHR()).WillOnce(Return(kGraphicsResetStatus));
...
TEST_F(RasterImplementationGLESTest, GetQueryObjectuivEXT)
{
    ...
    EXPECT_CALL(*gl_, GetQueryObjectuivEXT(kQueryId, kQueryParam, &result)).Times(1);
...
TEST_F(RasterImplementationGLESTest, GetQueryObjectui64vEXT)
{
    ...
    EXPECT_CALL(*gl_, GetQueryObjectui64vEXT(kQueryId, kQueryParam, &result)).Times(1);
...
TEST_F(RasterImplementationGLESTest, CreateAndConsumeForGpuRaster)
{
    ...
    EXPECT_CALL(*gl_, CreateAndTexStorage2DSharedImageCHROMIUM(mailbox.name)).WillOnce(Return(kTextureId));
...
{
    const GLuint kTextureId = 23;
    EXPECT_CALL(*gl_, BeginSharedImageAccessDirectCHROMIUM(kTextureId, GL_SHARED_IMAGE_ACCESS_MODE_READWRITE_CHROMIUM)).Times(1);
    ri_->BeginSharedImageAccessDirectCHROMIUM(kTextureId, GL_SHARED_IMAGE_ACCESS_MODE_READWRITE_CHROMIUM);
}
...
{
    EXPECT_CALL(*gl_, TraceBeginCHROMIUM(StrEq("BeginGpuRaster"), StrEq("GpuRasterization"))).Times(1);
    ri_->BeginGpuRaster();
}
