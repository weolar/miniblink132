...
protected:
    ...
    {
        ASSERT_TRUE(transfer_buffer_->Initialize(kTransferBufferSize, kStartingOffset, kTransferBufferSize, kTransferBufferSize, kAlignment));
    }
...
{
    command_buffer_ = std::make_unique<StrictMock<MockClientCommandBufferMockFlush>>();

    helper_ = std::make_unique<CommandBufferHelper>(command_buffer());
    ASSERT_EQ(helper_->Initialize(kCommandBufferSizeBytes), gpu::ContextResult::kSuccess);
...
TEST_F(TransferBufferTest, Basic)
{
    ...
    EXPECT_EQ(kTransferBufferSize - kStartingOffset, transfer_buffer_->GetCurrentMaxAllocationWithoutRealloc());
...
TEST_F(TransferBufferTest, Free)
{
    ...
    // Free buffer.
    EXPECT_CALL(*command_buffer(), DestroyTransferBuffer(_)).Times(1).RetiresOnSaturation();
    EXPECT_CALL(*command_buffer(), OrderingBarrier(_)).Times(1).RetiresOnSaturation();
...
TEST_F(TransferBufferTest, TooLargeAllocation)
{
    ...
    ptr = transfer_buffer_->AllocUpTo(kTransferBufferSize + 1, &size_allocated);
...
public:
    ...
    MOCK_METHOD4(CreateTransferBuffer, scoped_refptr<Buffer>(uint32_t size, int32_t* id, uint32_t alignment, TransferBufferAllocationOption option));

    scoped_refptr<gpu::Buffer> RealCreateTransferBuffer(uint32_t size, int32_t* id, uint32_t alignment, TransferBufferAllocationOption option)
    {
        return MockClientCommandBufferMockFlush::CreateTransferBuffer(size, id, alignment, option);
    }
};
...
void TransferBufferExpandContractTest::SetUp()
{
    ...
    EXPECT_CALL(*command_buffer(), CreateTransferBuffer(kCommandBufferSizeBytes, _, _, _))
        .WillOnce(Invoke(command_buffer(), &MockClientCommandBufferCanFail::RealCreateTransferBuffer))
        .RetiresOnSaturation();
    ...
    EXPECT_CALL(*command_buffer(), CreateTransferBuffer(kStartTransferBufferSize, _, _, _))
        .WillOnce(Invoke(command_buffer(), &MockClientCommandBufferCanFail::RealCreateTransferBuffer))
        .RetiresOnSaturation();

    transfer_buffer_ = std::make_unique<TransferBuffer>(helper_.get());
    ASSERT_TRUE(transfer_buffer_->Initialize(kStartTransferBufferSize, kStartingOffset, kMinTransferBufferSize, kMaxTransferBufferSize, kAlignment));
}
...
TEST_F(TransferBufferExpandContractTest, ExpandWithSmallAllocations)
{
    ...
    EXPECT_EQ(kStartTransferBufferSize - kStartingOffset, transfer_buffer_->GetCurrentMaxAllocationWithoutRealloc());

    // Fill the free space.
    uint32_t size_allocated = 0;
    void* ptr = transfer_buffer_->AllocUpTo(transfer_buffer_->GetFreeSize(), &size_allocated);
    ...
    ptr = transfer_buffer_->AllocUpTo(transfer_buffer_->GetFreeSize(), &size_allocated);
    transfer_buffer_->FreePendingToken(ptr, token);
    ...
    EXPECT_EQ(1u, size_allocated);
    transfer_buffer_->FreePendingToken(ptr, token);
    EXPECT_EQ(kMaxTransferBufferSize - kStartingOffset, transfer_buffer_->GetCurrentMaxAllocationWithoutRealloc());
}
...
TEST_F(TransferBufferExpandContractTest, ExpandWithLargeAllocations)
{
    ...
    uint32_t size_allocated = 0;
    ExpectCreateTransferBuffer(kStartTransferBufferSize * 2);
    void* ptr = transfer_buffer_->AllocUpTo(transfer_buffer_->GetFreeSize() + 1, &size_allocated);
    transfer_buffer_->FreePendingToken(ptr, token);
    ...
    // Try to expand again, no expansion should occur because we are at max.
    EXPECT_CALL(*command_buffer(), Flush(_)).Times(1).RetiresOnSaturation();
    size_requested = transfer_buffer_->GetCurrentMaxAllocationWithoutRealloc() + 1;
    ptr = transfer_buffer_->AllocUpTo(size_requested, &size_allocated);
...
TEST_F(TransferBufferExpandContractTest, ShrinkRingBuffer)
{
    ...
    for (uint32_t allocated = kMaxTransferBufferSize - kStartingOffset;
         allocated < (kStartTransferBufferSize + kStartingOffset) * (TransferBuffer::kShrinkThreshold);) {
...
TEST_F(TransferBufferExpandContractTest, Contract)
{
    ...
    EXPECT_CALL(*command_buffer(), CreateTransferBuffer(kStartTransferBufferSize, _, _, _))
        .WillOnce(DoAll(SetArgPointee<1>(-1), Return(scoped_refptr<gpu::Buffer>())))
        .RetiresOnSaturation();
    EXPECT_CALL(*command_buffer(), CreateTransferBuffer(kMinTransferBufferSize, _, _, _))
        .WillOnce(Invoke(command_buffer(), &MockClientCommandBufferCanFail::RealCreateTransferBuffer))
        .RetiresOnSaturation();
...
TEST_F(TransferBufferTest, MultipleAllocsAndFrees)
{
    ...
    EXPECT_EQ(transfer_buffer_->GetSize(), original_free_size);
    EXPECT_EQ(transfer_buffer_->GetFreeSize(), original_free_size - kArbitrarySize);
    EXPECT_EQ(transfer_buffer_->GetFragmentedFreeSize(), original_free_size - kArbitrarySize);

    void* ptr2 = transfer_buffer_->Alloc(kArbitrarySize);
    EXPECT_NE(ptr2, nullptr);
    EXPECT_EQ(transfer_buffer_->GetSize(), original_free_size);
    EXPECT_EQ(transfer_buffer_->GetFreeSize(), original_free_size - kArbitrarySize * 2);
    EXPECT_EQ(transfer_buffer_->GetFragmentedFreeSize(), original_free_size - kArbitrarySize * 2);

    void* ptr3 = transfer_buffer_->Alloc(kArbitrarySize);
    EXPECT_NE(ptr3, nullptr);
    EXPECT_EQ(transfer_buffer_->GetSize(), original_free_size);
    EXPECT_EQ(transfer_buffer_->GetFreeSize(), original_free_size - kArbitrarySize * 3);
    EXPECT_EQ(transfer_buffer_->GetFragmentedFreeSize(), original_free_size - kArbitrarySize * 3);
    ...
    EXPECT_EQ(transfer_buffer_->GetSize(), original_free_size);
    EXPECT_EQ(transfer_buffer_->GetFreeSize(), original_free_size - kArbitrarySize * 4);
    EXPECT_EQ(transfer_buffer_->GetFragmentedFreeSize(), original_free_size - kArbitrarySize * 3);
...
TEST_F(TransferBufferTest, ResizeDuringScopedResultPtr)
{
    ...
    ASSERT_DEATH(transfer_buffer_->AllocUpTo(transfer_buffer_->GetFreeSize() + 1, &size_allocated), "outstanding_result_pointer_");
}
...
TEST_F(TransferBufferTest, TwoScopedResultPtrs)
{
    ...
    ScopedResultPtr<int> ptr(transfer_buffer_.get());
    ASSERT_DEATH(ScopedResultPtr<int>(transfer_buffer_.get()), "outstanding_result_pointer_");
}
