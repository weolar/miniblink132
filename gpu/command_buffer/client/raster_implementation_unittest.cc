...
protected:
    ...
    static const uint32_t kCommandBufferSizeBytes = kNumCommandEntries * sizeof(CommandBufferEntry);
    ...
    public:
        ...
        bool Initialize(bool bind_generates_resource_client, bool lose_context_when_out_of_memory, bool transfer_buffer_initialize_fail, bool sync_query)
        {
            SharedMemoryLimits limits = SharedMemoryLimitsForTesting();
            command_buffer_ = std::make_unique<StrictMock<MockClientCommandBuffer>>();

            transfer_buffer_ = base::WrapUnique(new MockTransferBuffer(command_buffer_.get(), kTransferBufferSize, RasterImplementation::kStartingOffset,
...

            {
                ...
                gl_ = std::make_unique<RasterImplementation>(helper_.get(), transfer_buffer_.get(), bind_generates_resource_client,
                    lose_context_when_out_of_memory, gpu_control_.get(), nullptr /* image_decode_accelerator */);
            }
    ...
    {
        bool success = true;
        if (!test_context_.Initialize(init_options.bind_generates_resource_client, init_options.lose_context_when_out_of_memory,
                init_options.transfer_buffer_initialize_fail, init_options.sync_query)) {
            success = false;
        }
    ...
    int GetNextToken()
    {
        return test_context_.GetNextToken();
    }
...
TEST_F(RasterImplementationTest, GetBucketContents)
{
    ...
    EXPECT_CALL(*command_buffer(), OnFlush())
        .WillOnce(DoAll(SetMemory(result1.ptr, kTestSize), SetMemoryFromArray(mem1.ptr, buf.data(), MaxTransferBufferSize())))
        .WillOnce(SetMemoryFromArray(mem2.ptr, buf.data() + MaxTransferBufferSize(), kTestSize - MaxTransferBufferSize()))
        .RetiresOnSaturation();
...
TEST_F(RasterImplementationTest, BeginEndQueryEXT)
{
    ...
    expected_begin_cmds.begin_query.Init(GL_COMMANDS_COMPLETED_CHROMIUM, id1, query->shm_id(), query->shm_offset());
    ...
    expected_end_cmds.end_query.Init(GL_COMMANDS_COMPLETED_CHROMIUM, query->submit_count());
    ...
    available = 1u;
    gl_->GetQueryObjectuivEXT(id1, GL_QUERY_RESULT_AVAILABLE_NO_FLUSH_CHROMIUM_EXT, &available);
    EXPECT_EQ(0u, available);
}
...
TEST_F(RasterImplementationTest, SetActiveURLCHROMIUM)
{
    ...
    const uint32_t kPaddedStringSize = transfer_buffer_->RoundToAlignment(url.size());
