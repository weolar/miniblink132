...
void CommandBufferHelper::CalcImmediateEntries(int waiting_count)
{
    ...
    if (flush_automatically_) {
        int32_t limit = total_entry_count_ / ((curr_get == last_flush_put_) ? kAutoFlushSmall : kAutoFlushBig);

        int32_t pending = (put_ + total_entry_count_ - last_ordering_barrier_put_) % total_entry_count_;
...
void CommandBufferHelper::SetGetBuffer(int32_t id, scoped_refptr<Buffer> buffer)
{
...
void CommandBufferHelper::UpdateCachedState(const CommandBuffer::State& state)
{
    ...
    // in it, i.e. get == 0.
    service_on_old_buffer_ = (state.set_get_buffer_count != set_get_buffer_count_);
...
bool CommandBufferHelper::WaitForGetOffsetInRange(int32_t start, int32_t end)
{
    ...
    CommandBuffer::State last_state = command_buffer_->WaitForGetOffsetInRange(set_get_buffer_count_, start, end);
...
void CommandBufferHelper::PeriodicFlushCheck()
{
    ...
    if (current_time - last_flush_time_ > base::Microseconds(kPeriodicFlushDelayInMicroseconds)) {
        Flush();
    }
}
#endif
...
void CommandBufferHelper::WaitForToken(int32_t token)
{
    ...
    if (HasTokenPassed(token))
        return;
    FlushLazy();
    CommandBuffer::State last_state = command_buffer_->WaitForTokenInRange(token, token_);
    UpdateCachedState(last_state);
}
...
void CommandBufferHelper::WaitForAvailableEntries(int32_t count)
{
    ...
    if (immediate_entry_count_ < count) {
        ...
        if (immediate_entry_count_ < count) {
            ...
            if (!WaitForGetOffsetInRange((put_ + count + 1) % total_entry_count_, put_))
                return;
            CalcImmediateEntries(count);
            if (immediate_entry_count_ < count) {
...
bool CommandBufferHelper::OnMemoryDump(const base::trace_event::MemoryDumpArgs& args, base::trace_event::ProcessMemoryDump* pmd)
{
    ...
    MemoryAllocatorDump* dump = pmd->CreateAllocatorDump(base::StringPrintf("gpu/command_buffer_memory/buffer_0x%x", ring_buffer_id_));
    dump->AddScalar(MemoryAllocatorDump::kNameSize, MemoryAllocatorDump::kUnitsBytes, ring_buffer_size_);

    if (args.level_of_detail != MemoryDumpLevelOfDetail::kBackground) {
        dump->AddScalar("free_size", MemoryAllocatorDump::kUnitsBytes, GetTotalFreeEntriesNoWaiting() * sizeof(CommandBufferEntry));
        base::UnguessableToken shared_memory_guid = ring_buffer_->backing()->GetGUID();
