...
void FakeCommandBufferServiceBase::SetGetBufferHelper(int transfer_buffer_id, int32_t token)
{
...
scoped_refptr<gpu::Buffer> FakeCommandBufferServiceBase::CreateTransferBufferHelper(uint32_t size, int32_t* id)
{
    *id = GetNextFreeTransferBufferId();
    if (*id >= 0) {
        int32_t ndx = *id - kTransferBufferBaseId;
        base::UnsafeSharedMemoryRegion shared_memory_region = base::UnsafeSharedMemoryRegion::Create(size);
        base::WritableSharedMemoryMapping shared_memory_mapping = shared_memory_region.Map();
        transfer_buffer_buffers_[ndx] = MakeBufferFromSharedMemory(std::move(shared_memory_region), std::move(shared_memory_mapping));
    }
...
scoped_refptr<Buffer> FakeCommandBufferServiceBase::GetTransferBuffer(int32_t id)
{
    if ((id < kTransferBufferBaseId) || (id >= kTransferBufferBaseId + kMaxTransferBuffers))
        return nullptr;
...
void FakeCommandBufferServiceBase::SetContextLostReason(error::ContextLostReason reason)
{
...
CommandBuffer::State MockClientCommandBuffer::WaitForTokenInRange(int32_t start, int32_t end)
{
    return GetState();
}

CommandBuffer::State MockClientCommandBuffer::WaitForGetOffsetInRange(uint32_t set_get_buffer_count, int32_t start, int32_t end)
{
...
scoped_refptr<gpu::Buffer> MockClientCommandBuffer::CreateTransferBuffer(uint32_t size, int32_t* id, uint32_t alignment, TransferBufferAllocationOption option)
{
...
{
    ON_CALL(*this, DestroyTransferBuffer(_)).WillByDefault(Invoke(this, &FakeCommandBufferServiceBase::DestroyTransferBufferHelper));
}

void MockClientCommandBuffer::ForceLostContext(error::ContextLostReason reason)
{
...
void MockClientCommandBufferMockFlush::DelegateToFake()
{
    ...
    ON_CALL(*this, Flush(_)).WillByDefault(Invoke(this, &MockClientCommandBufferMockFlush::DoFlush));
}
...
void FakeDecoderClient::CacheBlob(gpu::GpuDiskCacheType, const std::string&, const std::string&)
{
}
