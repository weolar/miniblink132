...
static_assert(gpu::kInvalidResource == 0, "GL expects kInvalidResource to be 0");
...
public:
    ...
    void MakeIds(GLES2Implementation* /* gl_impl */, GLuint id_offset, GLsizei n, GLuint* ids) override
    {
    ...
    // Overridden from IdHandlerInterface.
    bool FreeIds(GLES2Implementation* gl_impl, GLsizei n, const GLuint* ids, DeleteFn delete_fn) override
    {
        base::AutoLock auto_lock(lock_);
    ...
    // Overridden from IdHandlerInterface.
    bool MarkAsUsedForBind(GLES2Implementation* gl_impl, GLenum target, GLuint id, BindFn bind_fn) override
    {
        base::AutoLock auto_lock(lock_);
    ...
    bool MarkAsUsedForBind(GLES2Implementation* gl_impl, GLenum target, GLuint index, GLuint id, BindIndexedFn bind_fn) override
    {
        base::AutoLock auto_lock(lock_);
        bool result = id ? id_allocator_.MarkAsUsed(id) : true;
    ...
    bool MarkAsUsedForBind(
        GLES2Implementation* gl_impl, GLenum target, GLuint index, GLuint id, GLintptr offset, GLsizeiptr size, BindIndexedRangeFn bind_fn) override
    {
        base::AutoLock auto_lock(lock_);
        bool result = id ? id_allocator_.MarkAsUsed(id) : true;
...
private:
    ...
    void CollectPendingFreeIds(GLES2Implementation* gl_impl) EXCLUSIVE_LOCKS_REQUIRED(lock_)
    {
...
public:
    ...
    bool MarkAsUsedForBind(GLES2Implementation* /* gl_impl */, GLenum /* target */, GLuint /* index */, GLuint /* id */, GLintptr /* offset */,
        GLsizeiptr /* size */, BindIndexedRangeFn /* bind_fn */) override
    {
        // This is only used for Shaders and Programs which have no bind.
        return false;
    }
...
public:
    ...
    void MakeIdRange(GLES2Implementation* /*gl_impl*/, GLsizei n, GLuint* first_id) override
    {
        base::AutoLock auto_lock(lock_);
        *first_id = id_allocator_.AllocateIDRange(n);
    }

    void FreeIdRange(GLES2Implementation* gl_impl, const GLuint first_id, GLsizei range, DeleteRangeFn delete_fn) override
    {
        base::AutoLock auto_lock(lock_);
        DCHECK(range > 0);
