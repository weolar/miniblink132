...
protected:
    ...
    struct ExpectedBuffers {
        ...
        ExpectedBuffers(uint32_t count)
            : a(count)
            , b(count)
            , c(count)
            , d(count)
    ...
    template <uint32_t BufferSize> void CheckTransferArraysAndExecute(uint32_t count)
    {
        ...
        EXPECT_TRUE(::internal::TransferArraysAndExecute(
            ...
            [&](std::array<uint32_t, 4>& byte_offsets, uint32_t copy_offset, uint32_t copy_count) {
                // Check that each sub-copy is correct
                const uint8_t* buffer = reinterpret_cast<uint8_t*>(transfer_buffer.address());
                EXPECT_EQ(memcmp(&buffer[byte_offsets[0]], &expected.a[copy_offset], copy_count * sizeof(char)), 0);
                EXPECT_EQ(memcmp(&buffer[byte_offsets[1]], &expected.b[copy_offset], copy_count * sizeof(short)), 0);
                EXPECT_EQ(memcmp(&buffer[byte_offsets[2]], &expected.c[copy_offset], copy_count * sizeof(float)), 0);
                EXPECT_EQ(memcmp(&buffer[byte_offsets[3]], &expected.d[copy_offset], copy_count * sizeof(size_t)), 0);
            },
            expected.a.data(), expected.b.data(), expected.c.data(), expected.d.data()));
    }
};
...
TEST_F(TransferBufferCmdCopyHelpersTest, CheckedSizeOfTypes)
{
    ...
    EXPECT_EQ((SizeOfPackedTypes<uint32_t, float, int>()), sizeof(float) + sizeof(int));
...
{
    EXPECT_EQ((ComputeCombinedCopySize<char, int, float>(4)), 4 * sizeof(char) + 4 * sizeof(int) + 4 * sizeof(float));

    EXPECT_EQ((ComputeCombinedCopySize<float, int, char>(3)), 3 * sizeof(float) + 3 * sizeof(int) + 3 * sizeof(char));
...
{
    EXPECT_EQ((ComputeCombinedCopySize<char, int, float>(3)), 4 * sizeof(char) + 3 * sizeof(int) + 3 * sizeof(float));

    EXPECT_EQ((ComputeCombinedCopySize<char, int, float>(5)), 8 * sizeof(char) + 5 * sizeof(int) + 5 * sizeof(float));
}
...
TEST_F(TransferBufferCmdCopyHelpersTest, ComputeCombinedCopySizeOverflow)
{
    ...
    EXPECT_EQ((ComputeCombinedCopySize<float>(UINT32_MAX / sizeof(float) + 1)), UINT32_MAX);
...
{
    EXPECT_EQ((ComputeMaxCopyCount<char, float>(SizeOfPackedTypes<uint32_t, char, float>())), 0u);
    EXPECT_EQ((ComputeMaxCopyCount<short, float>(SizeOfPackedTypes<uint32_t, short, float>())), 0u);
    EXPECT_EQ((ComputeMaxCopyCount<char, size_t>(SizeOfPackedTypes<uint32_t, char, size_t>())), 0u);
    EXPECT_EQ((ComputeMaxCopyCount<short, size_t>(SizeOfPackedTypes<uint32_t, short, size_t>())), 0u);
}
