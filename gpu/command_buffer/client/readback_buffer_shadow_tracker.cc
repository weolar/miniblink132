...
ReadbackBufferShadowTracker::Buffer::Buffer(GLuint buffer_id, MappedMemoryManager* mapped_memory, GLES2CmdHelper* helper)
...
uint32_t ReadbackBufferShadowTracker::Buffer::Alloc(int32_t* shm_id, uint32_t* shm_offset, bool* already_allocated)
{
    *already_allocated = readback_shm_address_ != nullptr;
    if (!readback_shm_address_) {
        readback_shm_address_ = mapped_memory_->Alloc(size_, &shm_id_, &shm_offset_);
    }
    *shm_id = shm_id_;
...
{
    if (readback_shm_address_) {
        mapped_memory_->FreePendingToken(readback_shm_address_, helper_->InsertToken());
    }
    readback_shm_address_ = nullptr;
}

void* ReadbackBufferShadowTracker::Buffer::MapReadbackShm(uint32_t offset, uint32_t map_size)
{
    DCHECK(!is_mapped_);
...
ReadbackBufferShadowTracker::ReadbackBufferShadowTracker(MappedMemoryManager* mapped_memory, GLES2CmdHelper* helper)
    : mapped_memory_(mapped_memory)
    , helper_(helper)
{
}
...
ReadbackBufferShadowTracker::Buffer* ReadbackBufferShadowTracker::GetOrCreateBuffer(GLuint id, GLuint size)
{
...
ReadbackBufferShadowTracker::BufferList ReadbackBufferShadowTracker::TakeUnfencedBufferList()
{
    BufferList buffers;
