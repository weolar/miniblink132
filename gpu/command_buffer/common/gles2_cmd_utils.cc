...
bool GLES2Util::ComputeImageRowSizeHelper(
    int width, uint32_t bytes_per_group, int alignment, uint32_t* rt_unpadded_row_size, uint32_t* rt_padded_row_size, uint32_t* rt_padding)
{
    DCHECK(alignment == 1 || alignment == 2 || alignment == 4 || alignment == 8);
    uint32_t unpadded_row_size;
    if (!base::CheckMul(width, bytes_per_group).AssignIfValid(&unpadded_row_size)) {
        return false;
    }
    ...
    if (residual > 0) {
        ...
        if (!base::CheckAdd(unpadded_row_size, padding).AssignIfValid(&padded_row_size)) {
            return false;
        }
    }
...
bool GLES2Util::ComputeImagePaddedRowSize(int width, int format, int type, int alignment, uint32_t* padded_row_size)
{
    uint32_t bytes_per_group = ComputeImageGroupSize(format, type);
    return ComputeImageRowSizeHelper(width, bytes_per_group, alignment, nullptr, padded_row_size, nullptr);
}

// Returns the amount of data glTexImage*D or glTexSubImage*D will access.
bool GLES2Util::ComputeImageDataSizes(
    int width, int height, int depth, int format, int type, int alignment, uint32_t* size, uint32_t* opt_unpadded_row_size, uint32_t* opt_padded_row_size)
{
    PixelStoreParams params;
    params.alignment = alignment;
    return ComputeImageDataSizesES3(width, height, depth, format, type, params, size, opt_unpadded_row_size, opt_padded_row_size, nullptr, nullptr);
}

bool GLES2Util::ComputeImageDataSizesES3(int width, int height, int depth, int format, int type, const PixelStoreParams& params, uint32_t* size,
    uint32_t* opt_unpadded_row_size, uint32_t* opt_padded_row_size, uint32_t* opt_skip_size, uint32_t* opt_padding)
{
    ...
    if (!ComputeImageRowSizeHelper(width, bytes_per_group, params.alignment, &unpadded_row_size, &padded_row_size, opt_padding)) {
        return false;
    }
    if (params.row_length > 0 && !ComputeImageRowSizeHelper(params.row_length, bytes_per_group, params.alignment, nullptr, &padded_row_size, opt_padding)) {
    ...
    if (depth > 0) {
        if (!base::CheckAdd(base::CheckMul(image_height, depth - 1), height).AssignIfValid(&num_of_rows)) {
            return false;
        }
    } else {
        num_of_rows = 0;
    ...
    if (num_of_rows > 0) {
        if (!base::CheckAdd(base::CheckMul(num_of_rows - 1, padded_row_size), unpadded_row_size).AssignIfValid(size)) {
            return false;
        }
    } else {
        *size = 0;
...
uint32_t GLES2Util::GetGLReadPixelsImplementationFormat(uint32_t internal_format, uint32_t texture_type, bool supports_bgra)
{
...
uint32_t GLES2Util::GetGLReadPixelsImplementationType(uint32_t internal_format, uint32_t texture_type)
{
    switch (internal_format) {
    case GL_R16UI:
    case GL_RG16UI:
...
void GLES2Util::GetColorFormatComponentSizes(uint32_t internal_format, uint32_t type, int* r, int* g, int* b, int* a)
{
    DCHECK(r && g && b && a);
    *r = 0;
    *g = 0;
...
uint32_t GLES2Util::GetChannelsNeededForAttachmentType(int type, uint32_t max_color_attachments)
{
    switch (type) {
...
std::string GLES2Util::GetStringError(uint32_t value)
{
    ...
    return GLES2Util::GetQualifiedEnumString(string_table, std::size(string_table), value);
}
...
std::string GLES2Util::GetQualifiedEnumString(const EnumToString* table, size_t count, uint32_t value)
{
...
// static
void GLES2Util::MapUint64ToTwoUint32(uint64_t v64, uint32_t* v32_0, uint32_t* v32_1)
{
    DCHECK(v32_0 && v32_1);
