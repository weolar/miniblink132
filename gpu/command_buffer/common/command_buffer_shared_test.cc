...
void WriteToState(int32_t* buffer, CommandBufferSharedState* shared_state)
{
    ...
    for (int i = 0; i < kSize; i++) {
        ...
        state.error = static_cast<gpu::error::Error>((i + 3) % (gpu::error::kErrorLast + 1));
...
TEST_F(CommandBufferSharedTest, TestConsistency)
{
    ...
    consumer.Start();
    consumer.task_runner()->PostTask(FROM_HERE, base::BindOnce(&WriteToState, buffer.get(), shared_state_.get()));
    ...
    while (true) {
        ...
        if (state.get_offset >= 1) {
            ...
            EXPECT_EQ(state.generation, static_cast<unsigned int>(state.get_offset) + 1);
            EXPECT_EQ(state.error, (state.get_offset + 2) % (gpu::error::kErrorLast + 1));
