...
RETURN_TYPE GetImmediateDataAs(const volatile COMMAND_TYPE& pod, uint32_t size, uint32_t immediate_data_size)
{
    return (size <= immediate_data_size) ? static_cast<RETURN_TYPE>(const_cast<volatile void*>(AddressAfterStruct(pod))) : nullptr;
}
...
public:
    ...
    static bool ComputeImagePaddedRowSize(int width, int format, int type, int alignment, uint32_t* padded_row_size);
    ...
    static bool ComputeImageDataSizesES3(int width, int height, int depth, int format, int type, const PixelStoreParams& params, uint32_t* size,
    ...
    static uint32_t GetGLReadPixelsImplementationFormat(uint32_t internal_format, uint32_t texture_type, bool supports_bgra);

    static uint32_t GetGLReadPixelsImplementationType(uint32_t internal_format, uint32_t texture_type);
    ...
    static uint32_t GetChannelsNeededForAttachmentType(int type, uint32_t max_color_attachments);
    ...
    constexpr static uint32_t CalcFramebufferPixelLocalClearValueufvANGLEDataCount(int plane)
    {
        return 4;
    }
    constexpr static uint32_t CalcFramebufferPixelLocalClearValueuivANGLEDataCount(int plane)
    {
        return 4;
    }
    constexpr static uint32_t CalcFramebufferPixelLocalClearValueuuivANGLEDataCount(int plane)
    {
        return 4;
    }

    static void MapUint64ToTwoUint32(uint64_t v64, uint32_t* v32_0, uint32_t* v32_1);
    ...
    static void GetColorFormatComponentSizes(uint32_t internal_format, uint32_t type, int* r, int* g, int* b, int* a);

    // Computes the data size for certain gl commands like glUniform.
    template <typename VALUE_TYPE, unsigned int ELEMENTS_PER_UNIT> static bool ComputeDataSize(uint32_t count, uint32_t* dst)
    {
...
private:
    static std::string GetQualifiedEnumString(const EnumToString* table, size_t count, uint32_t value);
...
public:
    ...
    bool IsArrayName() const
    {
        return element_index_ >= 0;
    }
