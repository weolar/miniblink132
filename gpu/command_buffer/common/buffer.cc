...
const base::UnsafeSharedMemoryRegion& BufferBacking::shared_memory_region() const
{
    static const base::NoDestructor<base::UnsafeSharedMemoryRegion> kInvalidRegion;
...
MemoryBufferBacking::MemoryBufferBacking(uint32_t size, uint32_t alignment)
    : memory_(new char[size + alignment])
    , size_(size)
...
{
    return alignment_ > 0 ? base::bits::AlignUp(memory_.get(), alignment_) : memory_.get();
}
...
const base::UnsafeSharedMemoryRegion& SharedMemoryBufferBacking::shared_memory_region() const
{
...
void* Buffer::GetDataAddressAndSize(uint32_t data_offset, uint32_t* data_size) const
{
...
base::trace_event::MemoryAllocatorDumpGuid GetBufferGUIDForTracing(uint64_t tracing_process_id, int32_t buffer_id)
{
    return base::trace_event::MemoryAllocatorDumpGuid(base::StringPrintf("gpu-buffer-x-process/%" PRIx64 "/%d", tracing_process_id, buffer_id));
}
