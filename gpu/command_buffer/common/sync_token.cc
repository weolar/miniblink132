...
SyncPointClientId::SyncPointClientId(CommandBufferNamespace in_namespace_id, CommandBufferId in_command_buffer_id)
    : namespace_id(in_namespace_id)
    , command_buffer_id(in_command_buffer_id)
{
}
...
SyncToken::SyncToken(CommandBufferNamespace namespace_id, CommandBufferId command_buffer_id, uint64_t release_count)
    : verified_flush_(false)
...
std::string SyncToken::ToDebugString() const
{
    ...
    stream << static_cast<int>(namespace_id()) << ":" << channel_or_high << ":" << route_or_low << ":" << release_count();
    return stream.str();
}
...
std::vector<SyncToken> ReduceSyncTokens(base::span<const SyncToken> tokens)
{
    ...
    for (const SyncToken& next_token : tokens) {
        auto itr = base::ranges::find_if(reduced, [&next_token](const SyncToken& token) {
            return next_token.namespace_id() == token.namespace_id() && next_token.command_buffer_id() == token.command_buffer_id();
        });
        if (itr == reduced.end()) {
