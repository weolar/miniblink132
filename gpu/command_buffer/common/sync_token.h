...
struct GPU_EXPORT SyncPointClientId {
    ...
    SyncPointClientId(CommandBufferNamespace in_namespace_id, CommandBufferId in_command_buffer_id);

    bool operator<(const SyncPointClientId& other) const
    {
        return std::tie(namespace_id, command_buffer_id) < std::tie(other.namespace_id, other.command_buffer_id);
    }
...
struct GPU_EXPORT SyncToken {
    ...
    SyncToken(CommandBufferNamespace namespace_id, CommandBufferId command_buffer_id, uint64_t release_count);

    SyncToken(const SyncToken& other);
    SyncToken& operator=(const SyncToken& other);

    void Set(CommandBufferNamespace namespace_id, CommandBufferId command_buffer_id, uint64_t release_count)
    {
    ...
    int8_t* GetData()
    {
        return reinterpret_cast<int8_t*>(this);
    }
    ...
    friend std::strong_ordering operator<=>(const SyncToken&, const SyncToken&) = default;
...
static_assert(sizeof(SyncToken) <= GL_SYNC_TOKEN_SIZE_CHROMIUM, "SyncToken size must not exceed GL_SYNC_TOKEN_SIZE_CHROMIUM");
...
GPU_EXPORT std::vector<SyncToken> ReduceSyncTokens(base::span<const SyncToken> tokens);
