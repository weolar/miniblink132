...
enum ShaderVariableBaseType { SHADER_VARIABLE_INT = 0x01, SHADER_VARIABLE_UINT = 0x02, SHADER_VARIABLE_FLOAT = 0x03, SHADER_VARIABLE_UNDEFINED_TYPE = 0x00 };
...
public:
    ...
    void RequestCompile(scoped_refptr<ShaderTranslatorInterface> translator, TranslatedShaderSourceType type);
    ...
    bool CanCompile()
    {
        return shader_state_ == kShaderStateCompileRequested;
    }
    bool HasCompiled()
    {
        return shader_state_ == kShaderStateCompiled;
    }
    void DoCompile();
    ...
    const sh::InterfaceBlock* GetInterfaceBlockInfo(const std::string& name) const;
    const sh::OutputVariable* GetOutputVariableInfo(const std::string& name) const;
    ...
    // If the original_name is not found, return nullptr.
    const std::string* GetVaryingMappedName(const std::string& original_name) const;

    // If the original_name is not found, return nullptr.
    const std::string* GetInterfaceBlockMappedName(const std::string& original_name) const;

    // If the original_name is not found, return nullptr.
    const std::string* GetOutputVariableMappedName(const std::string& original_name) const;
    ...
    void set_output_variable_list(const OutputVariableList& output_variable_list)
    {
        // copied because cache might be cleared
