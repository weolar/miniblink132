...
ValidatingAbstractTextureImpl::ValidatingAbstractTextureImpl(
    scoped_refptr<TextureRef> texture_ref, DecoderContext* decoder_context, DestructionCB destruction_cb)
...
void ValidatingAbstractTextureImpl::SetParameteri(GLenum pname, GLint param)
{
    ...
    gl::ScopedTextureBinder binder(texture_ref_->texture()->target(), service_id());
    GetTextureManager()->SetParameteri(__func__, GetErrorState(), texture_ref_.get(), pname, param);
}

void ValidatingAbstractTextureImpl::BindImage(gl::GLImage* image, bool client_managed)
{
    if (!texture_ref_)
        return;
    ...
    if (decoder_managed_image_) {
        ...
        gl::GLImage* current_image = texture_ref_->texture()->GetLevelImage(target, 0, &image_state);
    ...
    Texture::ImageState state = image && client_managed ? Texture::ImageState::BOUND : Texture::ImageState::UNBOUND;
    GetTextureManager()->SetLevelImage(texture_ref_.get(), target, level, image, state);
    GetTextureManager()->SetLevelCleared(texture_ref_.get(), target, level, image);
}

void ValidatingAbstractTextureImpl::BindStreamTextureImage(gl::GLImage* image, GLuint service_id)
{
    DCHECK(image);
    ...
    GetTextureManager()->SetLevelStreamTextureImage(texture_ref_.get(), target, level, image, Texture::ImageState::UNBOUND, service_id);
    SetCleared();
}
...
void ValidatingAbstractTextureImpl::SetCleared()
{
    ...
    const GLint level = 0;
    GetTextureManager()->SetLevelCleared(texture_ref_.get(), texture_ref_->texture()->target(), level, true);
}
