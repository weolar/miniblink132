...
protected:
    ...
    void DoGetErrorWithContextLost(GLenum reset_status)
    {
        ...
        EXPECT_CALL(*gl_, GetError()).WillOnce(Return(GL_CONTEXT_LOST_KHR)).RetiresOnSaturation();
        EXPECT_CALL(*gl_, GetGraphicsResetStatusARB()).WillOnce(Return(reset_status));
        cmds::GetError cmd;
...
TEST_P(GLES2DecoderLostContextTest, LostFromMakeCurrent)
{
    ...
    EXPECT_CALL(*context_, MakeCurrentImpl(surface_.get())).WillOnce(Return(false));
...
TEST_P(GLES2DecoderLostContextTest, TextureDestroyAfterLostFromMakeCurrent)
{
    ...
    DoTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 5, 6, 0, GL_RGBA, GL_UNSIGNED_BYTE, shared_memory_id_, kSharedMemoryOffset);
    DoBindFramebuffer(GL_FRAMEBUFFER, client_framebuffer_id_, kServiceFramebufferId);
    DoFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, kClientTextureId, kServiceTextureId, 0, GL_NO_ERROR);

    // The texture should never be deleted at the GL level.
    EXPECT_CALL(*gl_, DeleteTextures(1, Pointee(kServiceTextureId))).Times(0).RetiresOnSaturation();
...
TEST_P(GLES2DecoderLostContextTest, LostFromResetAfterMakeCurrent)
{
    ...
    EXPECT_CALL(*gl_, GetGraphicsResetStatusARB()).WillOnce(Return(GL_GUILTY_CONTEXT_RESET_KHR));
    // Expect the group to be lost.
    EXPECT_CALL(*mock_decoder_, MarkContextLost(error::kUnknown)).Times(1);
    decoder_->MakeCurrent();
    EXPECT_TRUE(decoder_->WasContextLost());
...
INSTANTIATE_TEST_SUITE_P(Service, GLES2DecoderLostContextTest, ::testing::Bool());
