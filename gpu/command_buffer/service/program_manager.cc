...
bool GetUniformNameSansElement(const std::string& name, int* element_index, std::string* new_name)
{
...
bool IsBuiltInFragmentVarying(const std::string& name)
{
    ...
    const char* kBuiltInVaryings[] = { "gl_FragCoord", "gl_FrontFacing", "gl_PointCoord" };
...
Program::UniformInfo::UniformInfo(
    const std::string& client_name, int client_location_base, GLenum _type, bool _is_array, const std::vector<GLint>& service_locations)
    ...
    , element_locations(service_locations)
{
    ...
    case GL_BOOL:
        accepts_api_type = UniformApiType::kUniform1i | UniformApiType::kUniform1ui | UniformApiType::kUniform1f;
        break;
    case GL_BOOL_VEC2:
        accepts_api_type = UniformApiType::kUniform2i | UniformApiType::kUniform2ui | UniformApiType::kUniform2f;
        break;
    case GL_BOOL_VEC3:
        accepts_api_type = UniformApiType::kUniform3i | UniformApiType::kUniform3ui | UniformApiType::kUniform3f;
        break;
    case GL_BOOL_VEC4:
        accepts_api_type = UniformApiType::kUniform4i | UniformApiType::kUniform4ui | UniformApiType::kUniform4f;
        break;
...
{
    return name.length() >= 3 && name[0] == 'g' && name[1] == 'l' && name[2] == '_';
}
...
void Program::UpdateFragmentOutputBaseTypes()
{
    fragment_output_type_mask_ = 0u;
    fragment_output_written_mask_ = 0u;
    Shader* fragment_shader = shaders_from_last_successful_link_[ShaderTypeToIndex(GL_FRAGMENT_SHADER)].get();
    DCHECK(fragment_shader);
    for (auto const& output : fragment_shader->output_variable_list()) {
        int location = output.location;
        DCHECK(location == -1 || (location >= 0 && location < static_cast<int>(manager_->max_draw_buffers())));
        if (location == -1)
            location = 0;
        ...
        int count = static_cast<int>(output.isArray() ? output.getOutermostArraySize() : 1);
        ...
        for (int ii = location; ii < location + count; ++ii) {
            ...
            fragment_output_type_mask_ |= InputOutputTypeToBaseType(false, output.type) << shift_bits;
        }
    }
}
...
void Program::UpdateVertexInputBaseTypes()
{
    ...
    for (size_t ii = 0; ii < attrib_infos_.size(); ++ii) {
        ...
        DCHECK_LE(input.location + input.location_count, manager_->max_vertex_attribs());
        for (size_t location = input.location; location < input.location + input.location_count; ++location) {
            int shift_bits = (location % 16) * 2;
            vertex_input_active_mask_[location / 16] |= 0x3 << shift_bits;
            vertex_input_base_type_mask_[location / 16] |= InputOutputTypeToBaseType(true, input.type) << shift_bits;
        }
    }
}
...
void Program::UpdateUniformBlockSizeInfo()
{
    ...
    for (GLint ii = 0; ii < num_uniform_blocks; ++ii) {
        GLint binding = 0;
        glGetActiveUniformBlockiv(service_id_, ii, GL_UNIFORM_BLOCK_BINDING, &binding);
        uniform_block_size_info_[ii].binding = static_cast<GLuint>(binding);

        GLint size = 0;
        glGetActiveUniformBlockiv(service_id_, ii, GL_UNIFORM_BLOCK_DATA_SIZE, &size);
...
void Program::UpdateTransformFeedbackInfo()
{
    ...
    if (effective_transform_feedback_buffer_mode_ == GL_INTERLEAVED_ATTRIBS) {
        ...
        transform_feedback_data_size_per_vertex_.resize(effective_transform_feedback_varyings_.size());
    }

    base::CheckedNumeric<GLsizeiptr> total = 0;
    for (size_t ii = 0; ii < effective_transform_feedback_varyings_.size(); ++ii) {
        const std::string& client_name = effective_transform_feedback_varyings_[ii];
        const std::string* service_name = vertex_shader->GetVaryingMappedName(client_name);
        DCHECK(service_name);
    ...
    if (effective_transform_feedback_buffer_mode_ == GL_INTERLEAVED_ATTRIBS) {
        transform_feedback_data_size_per_vertex_[0] = total.ValueOrDefault(std::numeric_limits<GLsizeiptr>::max());
    }
}
...
void Program::UpdateDrawIDUniformLocation()
{
    ...
    draw_id_uniform_location_ = -1;
    GLint array_index;
    GetUniformInfoByFakeLocation(fake_location, &draw_id_uniform_location_, &array_index);
}
...
void Program::UpdateBaseVertexUniformLocation()
{
    ...
    base_vertex_uniform_location_ = -1;
    GLint array_index;
    GetUniformInfoByFakeLocation(fake_location, &base_vertex_uniform_location_, &array_index);
}
...
void Program::UpdateBaseInstanceUniformLocation()
{
    ...
    base_instance_uniform_location_ = -1;
    GLint array_index;
    GetUniformInfoByFakeLocation(fake_location, &base_instance_uniform_location_, &array_index);
}
...
std::string Program::ProcessLogInfo(const std::string& log)
{
    ...
    while (RE2::Consume(&input, "(.*?)(webgl_[0123456789abcdefABCDEF]+)", &prior_log, &hashed_name)) {
        output += prior_log;

        const std::string* original_name = GetOriginalNameFromHashedName(hashed_name);
        if (original_name)
...
void Program::Update()
{
    ...
    for (GLint ii = 0; ii < num_attribs; ++ii) {
        ...
        GLsizei size = 0;
        GLenum type = 0;
        glGetActiveAttrib(service_id_, ii, max_len, &length, &size, &type, name_buffer.data());
        DCHECK(max_len == 0 || length < max_len);
        DCHECK(length == 0 || name_buffer[length] == '\0');
        std::string original_name;
        GetVertexAttribData(std::string(name_buffer.data(), length), &original_name, &type);
        ...
        if (!location_count.AssignIfValid(&safe_location_count))
            return;
        GLint location;
        if (base::StartsWith(base::as_string_view(name_buffer), "gl_", base::CompareCase::SENSITIVE)) {
        ...
        attrib_infos_.push_back(VertexAttrib(1, type, original_name, location, safe_location_count));
        max_attrib_name_length_ = std::max(max_attrib_name_length_, static_cast<GLsizei>(original_name.size()));
    }
    ...
    if (manager_->gpu_preferences_->enable_gpu_service_logging_gpu) {
        ...
        for (size_t ii = 0; ii < attrib_infos_.size(); ++ii) {
            const VertexAttrib& info = attrib_infos_[ii];
            DVLOG(1) << ii << ": loc = " << info.location << ", size = " << info.size << ", type = " << GLES2Util::GetStringEnum(info.type)
                     << ", name = " << info.name;
        }
    }
    ...
    if (manager_->gpu_preferences_->enable_gpu_service_logging_gpu) {
        ...
        for (const UniformInfo& info : uniform_infos_) {
            DVLOG(1) << ii++ << ": loc = " << info.element_locations[0] << ", size = " << info.size << ", type = " << GLES2Util::GetStringEnum(info.type)
                     << ", name = " << info.name;
        }
    }
...
bool Program::UpdateUniforms()
{
    ...
    glGetProgramiv(service_id_, GL_ACTIVE_UNIFORM_MAX_LENGTH, &name_buffer_length);
    DCHECK(name_buffer_length > 0);
    base::HeapArray<char> name_buffer = base::HeapArray<char>::Uninit(name_buffer_length);
    ...
    for (GLint uniform_index = 0; uniform_index < num_uniforms; ++uniform_index) {
        ...
        GLsizei size = 0;
        GLenum type = GL_NONE;
        glGetActiveUniform(service_id_, uniform_index, name_buffer_length, &name_length, &size, &type, name_buffer.data());
        ...
        if (!ProgramManager::HasBuiltInPrefix(service_name)) {
            service_location = glGetUniformLocation(service_id_, service_name.c_str());
        }
        ...
        for (size_t i = 0; i < kMaxAttachedShaders && client_name.empty(); ++i) {
            ...
            if (uniform && uniform->findInfoByMappedName(service_name, &info, &client_name)) {
                DCHECK(!client_name.empty());
                is_array = info->isArray();
                type = info->type;
                size = std::max(1u, info->getOutermostArraySize());
            } else {
                const InterfaceBlockMap& interface_block_map = shader->interface_block_map();
                for (const auto& key_value : interface_block_map) {
                    ...
                    if (block.instanceName.empty()) {
                        for (const auto& value : block.fields) {
                            if (value.findInfoByMappedName(service_name, &info, &client_name)) {
                                find = true;
                                break;
                            }
                        }
                        ...
                        if (block.mappedName == top_variable_name) {
                            ...
                            for (const auto& field : block.fields) {
                                if (field.findInfoByMappedName(service_name.substr(pos + 1), &info, &client_name)) {
                                    find = true;
        ...
        LocationMap::const_iterator it = bind_uniform_location_map_.find(client_base_name);
        if (it != bind_uniform_location_map_.end()) {
            client_location_base = it->second;
        } else {
            while (unused_client_location_cursor < uniform_locations_.size() && !uniform_locations_[unused_client_location_cursor].IsUnused())
        ...
        if (size > 1) {
            for (GLsizei ii = 1; ii < size; ++ii) {
                std::string element_name(service_base_name + "[" + base::NumberToString(ii) + "]");
                service_locations[ii] = glGetUniformLocation(service_id_, element_name.c_str());
            }
        }

        UniformInfo& info = uniform_infos_[uniform_index];
        info = UniformInfo(client_name, client_location_base, type, is_array, service_locations);
        if (info.IsSampler()) {
        ...
        max_uniform_name_length_ = std::max(max_uniform_name_length_, static_cast<GLsizei>(info.name.size()));
    }
    return true;
}
...
void Program::UpdateProgramOutputs()
{
    ...
    for (auto const& output_var : fragment_shader->output_variable_list()) {
        ...
        if (!output_var.isArray()) {
            GLint color_name = glGetFragDataLocation(service_id_, service_name.c_str());
            if (color_name < 0)
                continue;
            GLint index = 0;
            ...
            if (index < 0)
                continue;
            program_output_infos_.push_back(ProgramOutputInfo(color_name, index, client_name));
        } else {
            for (size_t ii = 0; ii < output_var.getOutermostArraySize(); ++ii) {
                ...
                GLint color_name = glGetFragDataLocation(service_id_, service_element_name.c_str());
                if (color_name < 0)
                    continue;
                GLint index = 0;
                if (feature_info().feature_flags().ext_blend_func_extended)
                ...
                if (index < 0)
                    continue;
                program_output_infos_.push_back(ProgramOutputInfo(color_name, index, client_name + array_spec));
            }
        }
    }
}
...
bool Program::ExecuteTransformFeedbackVaryingsCall()
{
    if (!transform_feedback_varyings_.empty()) {
        ...
        for (StringVector::const_iterator it = transform_feedback_varyings_.begin(); it != transform_feedback_varyings_.end(); ++it) {
        ...
        glTransformFeedbackVaryings(service_id_, mapped_names.size(), &mapped_names.front(), transform_feedback_buffer_mode_);
    }
...
void Program::ExecuteProgramOutputBindCalls()
{
    ...
    // This is called before program linking, so refer to attached_shaders_.
    Shader* fragment_shader = attached_shaders_[ShaderTypeToIndex(GL_FRAGMENT_SHADER)].get();
        ...
        for (auto const& output_var : fragment_shader->output_variable_list()) {
            ...
            for (size_t jj = 0; jj < count; ++jj) {
                ...
                if (binding.second == 0) {
                    ...
                    glBindFragDataLocation(service_id_, binding.first, mapped_name.c_str());
                } else {
                    DCHECK(feature_info().feature_flags().ext_blend_func_extended);
                    glBindFragDataLocationIndexed(service_id_, binding.first, binding.second, mapped_name.c_str());
                }
            }
        }
    }
...
bool Program::Link(ShaderManager* manager, DecoderClient* client)
{
    ClearLinkStatus();
    ...
    // This is called before program linking, so refer to attached_shaders_.
    if (cache && !attached_shaders_[0]->last_compiled_source().empty() && !attached_shaders_[1]->last_compiled_source().empty()) {
        ProgramCache::LinkedProgramStatus status = cache->GetLinkedProgramStatus(attached_shaders_[0]->last_compiled_signature(),
            attached_shaders_[1]->last_compiled_signature(), &bind_attrib_location_map_, transform_feedback_varyings_, transform_feedback_buffer_mode_);

        bool cache_hit = status == ProgramCache::LINK_SUCCEEDED;
        UMA_HISTOGRAM_BOOLEAN("GPU.ProgramCache.CacheHit", cache_hit);

        if (cache_hit) {
            ProgramCache::ProgramLoadResult success = cache->LoadLinkedProgram(service_id(), attached_shaders_[0].get(), attached_shaders_[1].get(),
                &bind_attrib_location_map_, transform_feedback_varyings_, transform_feedback_buffer_mode_, client);
    ...
    if (link) {
        ...
        if (DetectInterfaceBlocksMismatch(&conflicting_name)) {
            std::string info_log = "Interface blocks with the same name but different"
                                   " fields/layout: "
                + conflicting_name;
            set_log_info(ProcessLogInfo(info_log).c_str());
            return false;
        ...
        if (cache && feature_info().gl_version_info().IsAtLeastGLES(3, 0)) {
            glProgramParameteri(service_id(), PROGRAM_BINARY_RETRIEVABLE_HINT, GL_TRUE);
        }
    ...
    if (success == GL_TRUE) {
        ...
        if (link) {
            ...
            if (cache) {
                cache->SaveLinkedProgram(service_id(), attached_shaders_[0].get(), attached_shaders_[1].get(), &bind_attrib_location_map_,
                    effective_transform_feedback_varyings_, effective_transform_feedback_buffer_mode_, client);
            }
        }
    } else {
        UpdateLogInfo();
            ...
            if (info.name.compare(0, open_pos, parsed_name.base_name()) == 0) {
                ...
                if (index < info.size) {
                    ...
                    if (info.element_locations[index] == -1)
                        return -1;
                    return ProgramManager::MakeFakeLocation(info.fake_location_base, index);
                }
            }
        }
    }
...
const Program::UniformInfo* Program::GetUniformInfoByFakeLocation(GLint fake_location, GLint* real_location, GLint* array_index) const
{
    DCHECK(real_location);
    DCHECK(array_index);
    if (fake_location < 0)
        return nullptr;
    size_t location_index = GetUniformLocationIndexFromFakeLocation(fake_location);
    ...
    const UniformInfo* info = uniform_locations_[location_index].shader_variable();
...
bool Program::IsInactiveUniformLocationByFakeLocation(GLint fake_location) const
{
    if (fake_location < 0)
        return true;
    size_t location_index = GetUniformLocationIndexFromFakeLocation(fake_location);
...
const std::string* Program::GetAttribMappedName(const std::string& original_name) const
{
...
const std::string* Program::GetUniformMappedName(const std::string& original_name) const
{
...
const std::string* Program::GetOriginalNameFromHashedName(const std::string& hashed_name) const
{
...
const sh::Varying* Program::GetVaryingInfo(const std::string& hashed_name) const
{
...
const sh::InterfaceBlock* Program::GetInterfaceBlockInfo(const std::string& hashed_name) const
{
    for (auto shader : shaders_from_last_successful_link_) {
        if (shader) {
            const sh::InterfaceBlock* info = shader->GetInterfaceBlockInfo(hashed_name);
            if (info)
                return info;
        }
    }
...
bool Program::SetUniformLocationBinding(const std::string& name, GLint location)
{
...
void Program::SetProgramOutputLocationBinding(const std::string& name, GLuint color_name)
{
    SetProgramOutputLocationIndexedBinding(name, color_name, 0);
}

void Program::SetProgramOutputLocationIndexedBinding(const std::string& name, GLuint color_name, GLuint index)
{
    bind_program_output_location_index_map_[name] = std::make_pair(color_name, index);
    bind_program_output_location_index_map_[name + "[0]"] = std::make_pair(color_name, index);
}

void Program::GetVertexAttribData(const std::string& name, std::string* original_name, GLenum* type) const
{
    DCHECK(original_name);
    DCHECK(type);
    Shader* shader = shaders_from_last_successful_link_[ShaderTypeToIndex(GL_VERTEX_SHADER)].get();
    if (shader) {
...
bool Program::SetSamplers(GLint num_texture_units, GLint fake_location, GLsizei count, const GLint* value)
{
    ...
    if (info->IsSampler() && count > 0) {
        ...
        std::copy(value, value + count, info->texture_units.begin() + element_index);
        return true;
    }
    return true;
}
...
{
    return attached_shaders_[ShaderTypeToIndex(shader->shader_type())].get() == shader;
}
...
bool Program::DetectInterfaceBlocksMismatch(std::string* conflicting_name) const
{
...
{
    DCHECK(attached_shaders_[0].get() && attached_shaders_[0]->shader_type() == GL_VERTEX_SHADER && attached_shaders_[1].get()
    ...
    for (const auto& key_value : *fragment_varyings) {
        ...
        if (!hit->second.isSameVaryingAtLinkTime(key_value.second, shader_version)) {
            *conflicting_name = name;
            return true;
        }
    }
...
bool Program::DetectProgramOutputLocationBindingConflicts() const
{
    ...
    Shader* shader = attached_shaders_[ShaderTypeToIndex(GL_FRAGMENT_SHADER)].get();
...
bool Program::DetectBuiltInInvariantConflicts() const
{
    ...
    bool gl_position_invariant = IsBuiltInInvariant(vertex_varyings, "gl_Position");
    bool gl_point_size_invariant = IsBuiltInInvariant(vertex_varyings, "gl_PointSize");

    bool gl_frag_coord_invariant = IsBuiltInInvariant(fragment_varyings, "gl_FragCoord");
    bool gl_point_coord_invariant = IsBuiltInInvariant(fragment_varyings, "gl_PointCoord");
...
bool Program::CheckVaryingsPacking() const
{
    ...
    for (const auto& key_value : *fragment_varyings) {
        ...
        if (!IsBuiltInFragmentVarying(key_value.first)) {
            ...
            if (vertex_iter == vertex_varyings->end() || !vertex_iter->second.staticUse) {
                continue;
            }
        }
    ...
    return sh::CheckVariablesWithinPackingLimits(static_cast<int>(manager_->max_varying_vectors()), variables);
}

void Program::GetProgramInfo(ProgramManager* manager, CommonDecoder::Bucket* bucket) const
{
    ...
    uint32_t size = sizeof(ProgramInfoHeader) + input_size + location_size + total_string_size;

    bucket->SetSize(size);
    ProgramInfoHeader* header = bucket->GetDataAs<ProgramInfoHeader*>(0, size);
    ProgramInput* inputs = bucket->GetDataAs<ProgramInput*>(sizeof(ProgramInfoHeader), input_size);
    int32_t* locations = bucket->GetDataAs<int32_t*>(sizeof(ProgramInfoHeader) + input_size, location_size);
    char* strings = bucket->GetDataAs<char*>(sizeof(ProgramInfoHeader) + input_size + location_size, total_string_size);
    DCHECK(header);
    DCHECK(inputs);
    DCHECK(locations);
    DCHECK(strings);
    ...
    for (const UniformInfo& info : uniform_infos_) {
        ...
        for (size_t jj = 0; jj < info.element_locations.size(); ++jj) {
            if (info.element_locations[jj] == -1)
                ...
                *locations++ = ProgramManager::MakeFakeLocation(info.fake_location_base, jj);
        }
        memcpy(strings, info.name.c_str(), info.name.size());
        strings += info.name.size();
        ++inputs;
...
bool Program::GetUniformBlocks(CommonDecoder::Bucket* bucket) const
{
    ...
    glGetProgramiv(program, GL_ACTIVE_UNIFORM_BLOCK_MAX_NAME_LENGTH, &max_name_length);
    ...
    for (uint32_t ii = 0; ii < num_uniform_blocks; ++ii) {
        ...
        param = 0;
        glGetActiveUniformBlockiv(program, ii, GL_UNIFORM_BLOCK_NAME_LENGTH, &param);
        DCHECK_GE(max_name_length, param);
        memset(&buffer[0], 0, param);
        length = 0;
        glGetActiveUniformBlockName(program, ii, static_cast<GLsizei>(param), &length, &buffer[0]);
        ...
        param = 0;
        glGetActiveUniformBlockiv(program, ii, GL_UNIFORM_BLOCK_ACTIVE_UNIFORMS, &param);
        ...
        param = 0;
        glGetActiveUniformBlockiv(program, ii, GL_UNIFORM_BLOCK_REFERENCED_BY_VERTEX_SHADER, &param);
        blocks[ii].referenced_by_vertex_shader = static_cast<uint32_t>(param);

        param = 0;
        glGetActiveUniformBlockiv(program, ii, GL_UNIFORM_BLOCK_REFERENCED_BY_FRAGMENT_SHADER, &param);
    ...
    UniformBlockInfo* entries = bucket->GetDataAs<UniformBlockInfo*>(header_size, entry_size);
    ...
    for (uint32_t ii = 0; ii < num_uniform_blocks; ++ii) {
        ...
        glGetActiveUniformBlockiv(program, ii, GL_UNIFORM_BLOCK_ACTIVE_UNIFORM_INDICES, &params[0]);
...
bool Program::GetTransformFeedbackVaryings(CommonDecoder::Bucket* bucket) const
{
    // The data is packed into the bucket in the following order
    //   1) header
    ...
    std::vector<TransformFeedbackVaryingInfo> varyings(num_transform_feedback_varyings);
    ...
    for (uint32_t ii = 0; ii < num_transform_feedback_varyings; ++ii) {
        ...
        GLenum var_type = 0;
        glGetTransformFeedbackVarying(program, ii, max_name_length, &var_name_length, &var_size, &var_type, &buffer[0]);
...
bool Program::GetUniformsES3(CommonDecoder::Bucket* bucket) const
{
    ...
    for (size_t pname_index = 0; pname_index < kNumPnames; ++pname_index) {
        ...
        glGetActiveUniformsiv(program, count, &indices[0], kPname[pname_index], &params[0]);
        for (GLsizei ii = 0; ii < count; ++ii) {
...
const Program::ProgramOutputInfo* Program::GetProgramOutputInfo(const std::string& name) const
{
...
void Program::TransformFeedbackVaryings(GLsizei count, const char* const* varyings, GLenum buffer_mode)
{
...
ProgramManager::ProgramManager(ProgramCache* program_cache, uint32_t max_varying_vectors, uint32_t max_draw_buffers, uint32_t max_dual_source_draw_buffers,
    uint32_t max_vertex_attribs, const GpuPreferences& gpu_preferences, FeatureInfo* feature_info, gl::ProgressReporter* progress_reporter)
    : program_count_(0)
    , have_context_(true)
...
{
    std::pair<ProgramMap::iterator, bool> result = programs_.insert(std::make_pair(client_id, scoped_refptr<Program>(new Program(this, service_id))));
...
{
    if (program_cache_) {
        return program_cache_->HasSuccessfullyCompiledShader(shader->last_compiled_signature());
    }
    return false;
}

void ProgramManager::RemoveProgramInfoIfUnused(ShaderManager* shader_manager, Program* program)
{
    DCHECK(shader_manager);
    DCHECK(program);
    ...
    if (program->IsDeleted() && !program->InUse()) {
        ...
        for (ProgramMap::iterator it = programs_.begin(); it != programs_.end(); ++it) {
...
void ProgramManager::MarkAsDeleted(ShaderManager* shader_manager, Program* program)
{
    DCHECK(shader_manager);
    DCHECK(program);
    DCHECK(IsOwned(program));
...
void ProgramManager::UnuseProgram(ShaderManager* shader_manager, Program* program)
{
    DCHECK(shader_manager);
    DCHECK(program);
    DCHECK(IsOwned(program));
