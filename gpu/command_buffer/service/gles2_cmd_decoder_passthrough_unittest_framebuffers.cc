...
TEST_F(GLES2DecoderPassthroughTest, ReadPixelsOutOfRange)
{
    ...
    DoFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, kClientTextureId, 0);
    ...
    for (auto test : tests) {
        ...
        cmds::ReadPixels cmd;
        cmd.Init(test.x, test.y, test.w, test.h, kFormat, GL_UNSIGNED_BYTE, pixels_shm_id, pixels_shm_offset, result_shm_id, result_shm_offset, false);
        result->success = 0;
        EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
        ...
        for (GLint dx = 0; dx < test.w; ++dx) {
            ...
            for (GLint dy = 0; dy < test.h; ++dy) {
                ...
                for (GLint component = 0; component < 4; ++component) {
                    ...
                    EXPECT_EQ(expect_written, value != 0) << x << " " << y << " " << value;
                }
            }
        }
    }
...
TEST_F(GLES2DecoderPassthroughTest, ReadPixelsAsync)
{
    ...
    read_pixels_cmd.Init(0, 0, kWidth, kHeight, GL_RGBA, GL_UNSIGNED_BYTE, pixels_shm_id, pixels_shm_offset, result_shm_id, result_shm_offset, true);
    result->success = 0;
...
TEST_F(GLES3DecoderPassthroughTest, ReadPixelsAsyncSkippedIfPBOBound)
{
    ...
    buffer_data_cmd.Init(GL_PIXEL_PACK_BUFFER, read_pixels_result_size, 0, 0, GL_STREAM_DRAW);
    ...
    cmds::ReadPixels read_pixels_cmd;
    read_pixels_cmd.Init(0, 0, kWidth, kHeight, GL_RGBA, GL_UNSIGNED_BYTE, 0, 0, result_shm_id, result_shm_offset, true);
    result->success = 0;
    EXPECT_EQ(error::kNoError, ExecuteCmd(read_pixels_cmd));
    EXPECT_EQ(GL_NO_ERROR, GetGLError());
    EXPECT_FALSE(GetDecoder()->HasMoreIdleWork());
...
{
    size_t shm_size = 0;
    auto* result = GetSharedMemoryAsWithSize<cmds::ReadPixels::Result*>(&shm_size);
    const GLsizei kWidth = 4;
    const GLsizei kHeight = 4;
    uint32_t result_shm_id = shared_memory_id_;
    uint32_t result_shm_offset = kSharedMemoryOffset;
    ...
    read_pixels_cmd.Init(1, 2, 6, 7, GL_RGB, GL_UNSIGNED_SHORT, pixels_shm_id, pixels_shm_offset, result_shm_id, result_shm_offset, false);
    ...
    for (size_t i = 0; i < pixels_memory_size; i++) {
        ...
        char expected_value = i < read_pixels_result_size ? kReadPixelsValue : kDummyValue;
...
TEST_F(GLES2DecoderPassthroughTest, ReadPixelsAsyncError)
{
    ...
    cmds::ReadPixels cmd;
    cmd.Init(0, 0, kWidth, kHeight, GL_RGBA, GL_UNSIGNED_SHORT, pixels_shm_id, pixels_shm_offset, result_shm_id, result_shm_offset, true);
    result->success = 0;
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
...
TEST_F(GLES2DecoderPassthroughTest, RenderbufferStorageMultisampleEXTNotSupported)
{
...
TEST_F(GLES2DecoderPassthroughTest, GetFramebufferAttachmentParameterivWithRenderbuffer)
{
    DoBindFramebuffer(GL_FRAMEBUFFER, kClientFramebufferId);
    DoBindRenderbuffer(GL_RENDERBUFFER, kClientRenderbufferId);
    DoFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_RENDERBUFFER, kClientRenderbufferId);
...
TEST_F(GLES2DecoderPassthroughTest, GetFramebufferAttachmentParameterivWithTexture)
{
    DoBindFramebuffer(GL_FRAMEBUFFER, kClientFramebufferId);
    DoBindTexture(GL_TEXTURE_2D, kClientTextureId);
    DoFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, kClientTextureId, 0);
