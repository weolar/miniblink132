...
class GPU_GLES2_EXPORT ServiceFontManager : public base::RefCountedThreadSafe<ServiceFontManager> {
public:
    ...
    bool Deserialize(const volatile uint8_t* memory, uint32_t memory_size, std::vector<SkDiscardableHandleId>* locked_handles);
    bool Unlock(const std::vector<SkDiscardableHandleId>& handles);
    SkStrikeClient* strike_client()
    {
        return strike_client_.get();
    }
...
private:
    ...
    bool AddHandle(SkDiscardableHandleId handle_id, ServiceDiscardableHandle handle) EXCLUSIVE_LOCKS_REQUIRED(lock_);
    ...
    public:
        ...
        bool IsLocked() const
        {
            return handle_.IsLockedForTesting();
        }
    ...
    base::flat_map<SkDiscardableHandleId, Handle> discardable_handle_map_ GUARDED_BY(lock_);
