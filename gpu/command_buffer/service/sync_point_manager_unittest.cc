...
class SyncPointManagerTest : public base::test::WithFeatureOverride, public testing::Test {
public:
...
struct SyncPointStream {
    ...
    SyncPointStream(SyncPointManager* sync_point_manager, CommandBufferNamespace namespace_id, CommandBufferId command_buffer_id)
        : order_data(sync_point_manager->CreateSyncPointOrderData())
        , client_state(sync_point_manager->CreateSyncPointClientState(namespace_id, command_buffer_id, order_data->sequence_id()))
    {
    }
...
{
    scoped_refptr<SyncPointOrderData> order_data = sync_point_manager_->CreateSyncPointOrderData();
...
TEST_P(SyncPointManagerTest, MultipleClientsPerOrderData)
{
    ...
    SyncPointStream stream1(sync_point_manager_.get(), kNamespaceId, kCmdBufferId1);
    SyncPointStream stream2(sync_point_manager_.get(), kNamespaceId, kCmdBufferId2);
...
TEST_P(SyncPointManagerTest, BasicFenceSyncWaitRelease)
{
    ...
    SyncPointStream release_stream(sync_point_manager_.get(), kNamespaceId, kReleaseCmdBufferId);
    SyncPointStream wait_stream(sync_point_manager_.get(), kNamespaceId, kWaitCmdBufferId);
    ...
    int test_num = 10;
    bool valid_wait = wait_stream.client_state->Wait(sync_token, base::BindOnce(&SyncPointManagerTest::SetIntegerFunction, &test_num, 123));
...
TEST_P(SyncPointManagerTest, WaitWithOutOfOrderSyncTokenRelease)
{
    ...
    wait_stream.AllocateOrderNum();
    wait_stream.BeginProcessing();
    bool valid_wait = wait_stream.client_state->Wait(sync_token_1, base::BindOnce(&SyncPointManagerTest::SetIntegerFunction, &test_num_1, 123));
    EXPECT_TRUE(valid_wait);
    ...
    wait_stream.AllocateOrderNum();
    wait_stream.BeginProcessing();
    valid_wait = wait_stream.client_state->Wait(sync_token_2, base::BindOnce(&SyncPointManagerTest::SetIntegerFunction, &test_num_2, 123));
    EXPECT_TRUE(valid_wait);
    ...
    wait_stream.AllocateOrderNum();
    wait_stream.BeginProcessing();
    valid_wait = wait_stream.client_state->Wait(sync_token_3, base::BindOnce(&SyncPointManagerTest::SetIntegerFunction, &test_num_3, 123));
    EXPECT_TRUE(valid_wait);
...
TEST_P(SyncPointManagerTest, WaitOnSameSequenceFails)
{
    ...
    int test_num = 10;
    bool valid_wait = sync_point_manager_->Wait(sync_token, stream.order_data->sequence_id(), stream.order_data->unprocessed_order_num(),
        base::BindOnce(&SyncPointManagerTest::SetIntegerFunction, &test_num, 123));
    EXPECT_FALSE(valid_wait);
    EXPECT_EQ(10, test_num);
    EXPECT_FALSE(sync_point_manager_->IsSyncTokenReleased(sync_token));
}
...
TEST_P(SyncPointManagerOrderValidationTest, HandleInvalidWaitOrderNumber)
{
    ...
    int test_num = 10;
    bool valid_wait = sync_point_manager_->Wait(SyncToken(kNamespaceId, kCmdBufferId1, 1), stream2.order_data->sequence_id(), 3,
        base::BindOnce(&SyncPointManagerTest::SetIntegerFunction, &test_num, 123));
    EXPECT_FALSE(valid_wait);
    EXPECT_EQ(10, test_num);
}

TEST_P(SyncPointManagerOrderValidationTest, RetireInvalidWaitAfterOrderNumberPasses)
{
    CommandBufferNamespace kNamespaceId = gpu::CommandBufferNamespace::GPU_IO;
    CommandBufferId kCmdBufferId1 = CommandBufferId::FromUnsafeValue(0x123);
    CommandBufferId kCmdBufferId2 = CommandBufferId::FromUnsafeValue(0x234);
...
TEST_P(SyncPointManagerOrderValidationTest, HandleInvalidCyclicWaits)
{
    ...
    int test_num1 = 10;
    bool valid_wait = sync_point_manager_->Wait(SyncToken(kNamespaceId, kCmdBufferId1, 1), stream2.order_data->sequence_id(), 2,
        base::BindOnce(&SyncPointManagerTest::SetIntegerFunction, &test_num1, 123));
    EXPECT_TRUE(valid_wait);
    EXPECT_EQ(10, test_num1);

    // Stream 1 waits on stream 2 with order num 3.
    int test_num2 = 10;
    valid_wait = sync_point_manager_->Wait(SyncToken(kNamespaceId, kCmdBufferId2, 1), stream1.order_data->sequence_id(), 3,
        base::BindOnce(&SyncPointManagerTest::SetIntegerFunction, &test_num2, 123));
    EXPECT_TRUE(valid_wait);
...
INSTANTIATE_TEST_SUITE_P(All, SyncPointManagerTest, testing::Values(false, true));

// Only test the case of IsSyncPointGraphValidationEnabled() being false.
INSTANTIATE_TEST_SUITE_P(All, SyncPointManagerOrderValidationTest, testing::Values(false));
