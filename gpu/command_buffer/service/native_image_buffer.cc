...
scoped_refptr<NativeImageBufferEGL> NativeImageBufferEGL::Create(GLuint texture_id)
{
    ...
    DCHECK(display->ext->b_EGL_KHR_image_base && display->ext->b_EGL_KHR_gl_texture_2D_image && gl::g_current_gl_driver->ext.b_GL_OES_EGL_image);
    ...
    EGLImageKHR egl_image = eglCreateImageKHR(egl_display, egl_context, egl_target, egl_buffer, egl_attrib_list);

    if (egl_image == EGL_NO_IMAGE_KHR) {
        LOG(ERROR) << "eglCreateImageKHR for cross-thread sharing failed: 0x" << std::hex << eglGetError();
        return nullptr;
    }
...
NativeImageBufferEGL::NativeImageBufferEGL(EGLDisplay display, EGLImageKHR image)
...
void NativeImageBufferEGL::RemoveClient(gl::GLImage* client)
{
    ...
    if (write_client_ == client)
        write_client_ = nullptr;
    for (std::list<ClientInfo>::iterator it = client_infos_.begin(); it != client_infos_.end(); it++) {
        if (it->client == client) {
