...
{
    constexpr skgpu::graphite::RenderPassProperties kProps = { skgpu::graphite::DepthStencilFlags::kDepth, kBGRA_8888_SkColorType,
        /* requiresMSAA= */ false };
    ...
    Precompile(context, paintOptions, skgpu::graphite::DrawTypeFlags::kBitmapText_Mask, { &kProps, 1 });
}

void InitiatePrecompilation(skgpu::graphite::Context* context)
{
    constexpr base::TaskTraits precompile_traits = { base::TaskPriority::BEST_EFFORT, base::TaskShutdownBehavior::SKIP_ON_SHUTDOWN };
    ...
    base::ThreadPool::PostDelayedTask(FROM_HERE, precompile_traits, base::BindOnce(&PerformPrecompilation, context), precompile_wait);
}
...
std::unique_ptr<skgpu::graphite::Recorder> MakeGraphiteRecorder(
    skgpu::graphite::Context* context, size_t max_resource_cache_bytes, size_t max_image_provider_cache_bytes)
{
    skgpu::graphite::RecorderOptions options;
    options.fGpuBudgetInBytes = max_resource_cache_bytes;
    options.fImageProvider = sk_make_sp<gpu::GraphiteImageProvider>(max_image_provider_cache_bytes);
...
GLsizeiptr APIENTRY GLBlobCacheGetCallback(const void* key, GLsizeiptr key_size, void* value, GLsizeiptr value_size, const void* user_param)
{
    DCHECK(user_param != nullptr);
    raster::GrShaderCache* cache = static_cast<raster::GrShaderCache*>(const_cast<void*>(user_param));
...
void APIENTRY GLBlobCacheSetCallback(const void* key, GLsizeiptr key_size, const void* value, GLsizeiptr value_size, const void* user_param)
{
    DCHECK(user_param != nullptr);
    raster::GrShaderCache* cache = static_cast<raster::GrShaderCache*>(const_cast<void*>(user_param));
...
void BindGLContextToShaderCache(scoped_refptr<gles2::FeatureInfo> feature_info, raster::GrShaderCache* cache)
{
    if (!cache || !feature_info->feature_flags().angle_blob_cache) {
        return;
    }

    glBlobCacheCallbacksANGLE(GLBlobCacheSetCallback, GLBlobCacheGetCallback, cache);
}

void UnbindGLContextFromShaderCache(scoped_refptr<gles2::FeatureInfo> feature_info)
{
...
void SharedContextState::compileError(const char* shader, const char* errors, bool shaderWasCached)
{
    if (!context_lost()) {
        LOG(ERROR) << "Skia shader compilation error (was cached = " << shaderWasCached << ")"
                   << "\n"
                   << "------------------------\n"
                   << shader << "\nErrors:\n"
                   << errors;
        ...
        if (shaderWasCached && use_shader_cache_shm_count_ != nullptr) {
            ...
            // the cache.
            GpuProcessShmCount::ScopedIncrement increment(use_shader_cache_shm_count_.get());
...
SharedContextState::MemoryTrackerObserver::MemoryTrackerObserver(base::WeakPtr<gpu::MemoryTracker::Observer> peak_memory_monitor)
...
void SharedContextState::MemoryTrackerObserver::OnMemoryAllocatedChange(
    CommandBufferId id, uint64_t old_size, uint64_t new_size, GpuPeakMemoryAllocationSource source)
{
    ...
    if (peak_memory_monitor_) {
        peak_memory_monitor_->OnMemoryAllocatedChange(id, old_size, new_size, source);
    }
}
...
SharedContextState::MemoryTracker::MemoryTracker(Observer* observer)
    : command_buffer_id_(gpu::CommandBufferId::FromUnsafeValue(g_next_command_buffer_id.GetNext() + 1))
    , client_tracing_id_(base::trace_event::MemoryDumpManager::GetInstance()->GetTracingProcessId())
    , observer_(observer)
{
}
...
void SharedContextState::MemoryTracker::TrackMemoryAllocatedChange(int64_t delta)
{
    DCHECK(delta >= 0 || size_ >= static_cast<uint64_t>(-delta));
    uint64_t old_size = size_;
    size_ += delta;
    DCHECK(observer_);
    observer_->OnMemoryAllocatedChange(command_buffer_id_, old_size, size_, gpu::GpuPeakMemoryAllocationSource::SKIA);
}
...
SharedContextState::SharedContextState(scoped_refptr<gl::GLShareGroup> share_group, scoped_refptr<gl::GLSurface> surface, scoped_refptr<gl::GLContext> context,
    bool use_virtualized_gl_contexts, ContextLostCallback context_lost_callback, GrContextType gr_context_type,
    viz::VulkanContextProvider* vulkan_context_provider, viz::MetalContextProvider* metal_context_provider, DawnContextProvider* dawn_context_provider,
    base::WeakPtr<gpu::MemoryTracker::Observer> peak_memory_monitor, bool created_on_compositor_gpu_thread)
    ...
    , sk_surface_cache_(MaxNumSkSurface())
{
...
#if BUILDFLAG(ENABLE_VULKAN) && (BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_FUCHSIA) || BUILDFLAG(IS_WIN))
    ...
    if (base::SingleThreadTaskRunner::HasCurrentDefault()) {
        base::trace_event::MemoryDumpManager::GetInstance()->RegisterDumpProvider(
            this, "SharedContextState", base::SingleThreadTaskRunner::GetCurrentDefault());
    }
    // Initialize the scratch buffer to some small initial size.
    scratch_deserialization_buffer_.resize(kInitialScratchDeserializationBufferSize);
}
...
SharedContextState::~SharedContextState()
{
    ...
    DCHECK_EQ(skia_resource_cache_size_, memory_tracker_observer_.GetMemoryUsage());
...
bool SharedContextState::IsUsingGL() const
{
    ...
    return gr_context_type_ == GrContextType::kGL || gr_context_type_ == GrContextType::kNone;
}

bool SharedContextState::IsGraphiteDawn() const
{
    return gr_context_type() == GrContextType::kGraphiteDawn && dawn_context_provider();
}

bool SharedContextState::IsGraphiteMetal() const
{
    return gr_context_type() == GrContextType::kGraphiteMetal && metal_context_provider();
}

bool SharedContextState::IsGraphiteDawnMetal() const
{
#if BUILDFLAG(SKIA_USE_DAWN)
    return IsGraphiteDawn() && dawn_context_provider()->backend_type() == wgpu::BackendType::Metal;
#else
    return false;
#endif
}
...
bool SharedContextState::IsGraphiteDawnD3D() const
{
    ...
    return IsGraphiteDawn()
        && (dawn_context_provider()->backend_type() == wgpu::BackendType::D3D11 || dawn_context_provider()->backend_type() == wgpu::BackendType::D3D12);
#else
    return false;
#endif
}
...
{
#if BUILDFLAG(SKIA_USE_DAWN)
    return IsGraphiteDawn() && dawn_context_provider()->backend_type() == wgpu::BackendType::Vulkan;
#else
    return false;
#endif
}
...
{
#if BUILDFLAG(SKIA_USE_DAWN)
    return IsGraphiteDawn() && dawn_context_provider()->is_vulkan_swiftshader_adapter();
#else
    return false;
#endif
}

bool SharedContextState::InitializeSkia(const GpuPreferences& gpu_preferences, const GpuDriverBugWorkarounds& workarounds, gpu::raster::GrShaderCache* cache,
    GpuProcessShmCount* use_shader_cache_shm_count, gl::ProgressReporter* progress_reporter)
{
    ...
    if (gr_context_type_ == GrContextType::kGraphiteDawn || gr_context_type_ == GrContextType::kGraphiteMetal) {
        return InitializeGraphite(gpu_preferences, workarounds);
    }

    return InitializeGanesh(gpu_preferences, workarounds, cache, use_shader_cache_shm_count, progress_reporter);
}

bool SharedContextState::InitializeGanesh(const GpuPreferences& gpu_preferences, const GpuDriverBugWorkarounds& workarounds, gpu::raster::GrShaderCache* cache,
    GpuProcessShmCount* use_shader_cache_shm_count, gl::ProgressReporter* progress_reporter)
{
    ...
    DetermineGrCacheLimitsFromAvailableMemory(&max_resource_cache_bytes, &glyph_cache_max_texture_bytes);
    ...
    if (gr_context_type_ == GrContextType::kGL) {
        ...
        sk_sp<GrGLInterface> gr_gl_interface(gl::init::CreateGrGLInterface(*context_->GetVersionInfo(), progress_reporter));
        if (!gr_gl_interface) {
        ...
        if (use_shader_cache_shm_count && cache) {
            ...
            gr_gl_interface->fFunctions.fProgramBinary = [use_shader_cache_shm_count](GrGLuint program, GrGLenum binaryFormat, void* binary, GrGLsizei length) {
                GpuProcessShmCount::ScopedIncrement increment(use_shader_cache_shm_count);
        ...
        options.fDriverBugWorkarounds = GrDriverBugWorkarounds(workarounds.ToIntSet());
        options.fAvoidStencilBuffers = workarounds.avoid_stencil_buffers;
        if (workarounds.disable_program_disk_cache) {
            options.fShaderCacheStrategy = GrContextOptions::ShaderCacheStrategy::kBackendSource;
        }
        ...
        if (gl::GetGLImplementation() == gl::kGLImplementationStubGL) {
            ...
            owned_gr_context_ = GrDirectContexts::MakeGL(std::move(gr_gl_interface), options);
        }
    ...
    transfer_cache_
        = std::make_unique<ServiceTransferCache>(gpu_preferences, base::BindRepeating(&SharedContextState::ScheduleSkiaCleanup, base::Unretained(this)));
...
bool SharedContextState::InitializeGraphite(const GpuPreferences& gpu_preferences, const GpuDriverBugWorkarounds& workarounds)
{
    const skgpu::graphite::ContextOptions context_options = GetDefaultGraphiteContextOptions(workarounds);
...
#if BUILDFLAG(SKIA_USE_METAL)
        if (metal_context_provider_ && metal_context_provider_->InitializeGraphiteContext(context_options)) {
    ...
    if (features::IsSkiaGraphitePrecompilationEnabled(base::CommandLine::ForCurrentProcess())) {
    ...
    DetermineGraphiteImageProviderCacheLimits(&max_gpu_main_image_provider_cache_bytes, &max_viz_compositor_image_provider_cache_bytes);

    gpu_main_graphite_recorder_ = MakeGraphiteRecorder(graphite_context_, context_options.fGpuBudgetInBytes, max_gpu_main_image_provider_cache_bytes);
    gpu_main_graphite_cache_controller_
        = base::MakeRefCounted<raster::GraphiteCacheController>(gpu_main_graphite_recorder_.get(), graphite_context_.get(), dawn_context_provider_);

    viz_compositor_graphite_recorder_
        = MakeGraphiteRecorder(graphite_context_, context_options.fGpuBudgetInBytes, max_viz_compositor_image_provider_cache_bytes);
...
bool SharedContextState::InitializeGL(const GpuPreferences& gpu_preferences, scoped_refptr<gles2::FeatureInfo> feature_info)
{
    ...
    bool use_passthrough_cmd_decoder = gpu_preferences.use_passthrough_cmd_decoder && gles2::PassthroughCommandDecoderSupported();
    // Virtualized contexts don't work with passthrough command decoder.
    // See https://crbug.com/914976
    DCHECK(!use_passthrough_cmd_decoder || !use_virtualized_gl_contexts_);

    feature_info_ = std::move(feature_info);
    feature_info_->Initialize(feature_info_->context_type(), use_passthrough_cmd_decoder, gles2::DisallowedFeatures());
    ...
    if (max_vertex_attribs < kGLES2RequiredMinimumVertexAttribs) {
        LOG(ERROR) << "SharedContextState::InitializeGL failure max_vertex_attribs : " << max_vertex_attribs
                   << " is less that minimum required : " << kGLES2RequiredMinimumVertexAttribs;
    ...
    if (max_texture_units < kGLES2RequiredMinimumTextureUnits) {
        LOG(ERROR) << "SharedContextState::InitializeGL failure max_texture_units : " << max_texture_units
                   << " is less that minimum required : " << kGLES2RequiredMinimumTextureUnits;
        feature_info_ = nullptr;
        return false;
    }

    context_state_ = std::make_unique<gles2::ContextState>(feature_info_.get(), false /* track_texture_and_sampler_units */);
    ...
    if (driver_status != GL_NO_ERROR) {
        ...
        LOG(ERROR) << "SharedContextState::InitializeGL failure driver error : " << driver_status;
        feature_info_ = nullptr;
    ...
    if (use_virtualized_gl_contexts_) {
        auto virtual_context = base::MakeRefCounted<GLContextVirtual>(share_group_.get(), real_context_.get(), weak_ptr_factory_.GetWeakPtr());
    ...
    bool gl_supports_memory_object = gl::g_current_gl_driver->ext.b_GL_EXT_memory_object_fd || gl::g_current_gl_driver->ext.b_GL_EXT_memory_object_win32
        || gl::g_current_gl_driver->ext.b_GL_ANGLE_memory_object_fuchsia;
    bool gl_supports_semaphore = gl::g_current_gl_driver->ext.b_GL_EXT_semaphore_fd || gl::g_current_gl_driver->ext.b_GL_EXT_semaphore_win32
    ...
    gpu::VulkanImplementationName vulkan_implementation = gpu::VulkanImplementationName::kNone;
#if BUILDFLAG(ENABLE_VULKAN)
    if (vk_context_provider_) {
        vulkan_implementation = vk_context_provider_->GetVulkanImplementation()->use_swiftshader() ? gpu::VulkanImplementationName::kSwiftshader
                                                                                                   : gpu::VulkanImplementationName::kNative;
        auto* device_queue = vk_context_provider_->GetDeviceQueue();
#if BUILDFLAG(IS_WIN)
        vk_supports_external_memory = gfx::HasExtension(device_queue->enabled_extensions(), VK_KHR_EXTERNAL_MEMORY_WIN32_EXTENSION_NAME);
#elif BUILDFLAG(IS_FUCHSIA)
        vk_supports_external_memory = gfx::HasExtension(device_queue->enabled_extensions(), VK_FUCHSIA_EXTERNAL_MEMORY_EXTENSION_NAME);
#else
        vk_supports_external_memory = gfx::HasExtension(device_queue->enabled_extensions(), VK_KHR_EXTERNAL_MEMORY_FD_EXTENSION_NAME);
#endif
        vk_supports_external_semaphore = IsVkOpaqueExternalSemaphoreSupported(device_queue);
    }
#endif // BUILDFLAG(ENABLE_VULKAN)

    const bool is_native_vulkan_and_gl = vulkan_implementation == gpu::VulkanImplementationName::kNative && !gl::g_current_gl_version->is_swiftshader
    ...
    const bool is_swiftshader_vulkan_and_gl
        = vulkan_implementation == gpu::VulkanImplementationName::kSwiftshader && gl::g_current_gl_version->is_angle_swiftshader;

    support_vulkan_external_object_ = (is_native_vulkan_and_gl || is_swiftshader_vulkan_and_gl) && gl_supports_memory_object && gl_supports_semaphore
        && vk_supports_external_memory && vk_supports_external_semaphore;

    support_gl_external_object_flags_ = gl::g_current_gl_driver->ext.b_GL_ANGLE_memory_object_flags;
...
void SharedContextState::FlushAndSubmit(bool sync_to_cpu)
{
    if (graphite_context()) {
        ...
        graphite_context()->submit(sync_to_cpu ? skgpu::graphite::SyncToCpu::kYes : skgpu::graphite::SyncToCpu::kNo);
    } else if (gr_context()) {
        gr_context()->flushAndSubmit(sync_to_cpu ? GrSyncCpu::kYes : GrSyncCpu::kNo);
    }
}

void SharedContextState::FlushWriteAccess(SkiaImageRepresentation::ScopedWriteAccess* access)
{
    ...
    if (flush_count < 100) {
        ...
        base::UmaHistogramCustomMicrosecondsTimes(
            "GPU.RasterDecoder.TimeToFlush", base::TimeTicks::Now() - start, base::Microseconds(1), base::Seconds(1), 100);
    }
}

void SharedContextState::SubmitIfNecessary(std::vector<GrBackendSemaphore> signal_semaphores, bool need_graphite_submit)
{
    ...
    // happening per frame.
    const bool need_submit = sync_cpu || !signal_semaphores.empty() || is_drdc_enabled_;
...
bool SharedContextState::MakeCurrent(gl::GLSurface* surface, bool needs_gl)
{
    ...
    if (using_gl) {
        gl::GLSurface* dont_care_surface = last_current_surface_ ? last_current_surface_.get() : context_->default_surface();
...
void SharedContextState::MarkContextLost(error::ContextLostReason reason)
{
    if (!context_lost()) {
        ...
        // resources.
        std::move(context_lost_callback_).Run(!device_needs_reset_, context_lost_reason_.value());
...
bool SharedContextState::OnMemoryDump(const base::trace_event::MemoryDumpArgs& args, base::trace_event::ProcessMemoryDump* pmd)
{
    ...
    if (gr_context()) {
        ...
        if (background) {
            DumpBackgroundGraphiteMemoryStatistics(graphite_context(), gpu_main_graphite_recorder(), pmd);
        } else {
...
void SharedContextState::PurgeMemory(base::MemoryPressureListener::MemoryPressureLevel memory_pressure_level)
{
    ...
    case base::MemoryPressureListener::MEMORY_PRESSURE_LEVEL_MODERATE:
        ...
        if (gr_context_) {
            gr_context_->purgeUnlockedResources(GrPurgeResourceOptions::kScratchResourcesOnly);
...
void SharedContextState::UpdateSkiaOwnedMemorySize()
{
    ...
    if (!gr_context_ && !graphite_context_) {
        memory_tracker_observer_.OnMemoryAllocatedChange(CommandBufferId(), skia_resource_cache_size_, 0u);
    ...
    if (gr_context_) {
        ...
        const auto* image_provider = static_cast<const gpu::GraphiteImageProvider*>(gpu_main_graphite_recorder_->clientImageProvider());
        new_size = graphite_context_->currentBudgetedBytes() + gpu_main_graphite_recorder_->currentBudgetedBytes() + image_provider->CurrentSizeInBytes();
    }
    ...
    // prevent confusion.
    memory_tracker_observer_.OnMemoryAllocatedChange(CommandBufferId(), skia_resource_cache_size_, static_cast<uint64_t>(new_size));
...
void SharedContextState::StoreVkPipelineCacheIfNeeded()
{
    ...
    if (gr_context_ && gr_shader_cache_ && GrContextIsVulkan() && !created_on_compositor_gpu_thread_) {
        gpu::raster::GrShaderCache::ScopedCacheUse use(gr_shader_cache_, kDisplayCompositorClientId);
        gr_shader_cache_->StoreVkPipelineCacheIfNeeded(gr_context_);
    }
}

void SharedContextState::UseShaderCache(std::optional<gpu::raster::GrShaderCache::ScopedCacheUse>& cache_use, int32_t client_id) const
{
    if (gr_shader_cache_) {
...
void SharedContextState::RestoreAllTextureUnitAndSamplerBindings(const gles2::ContextState* prev_state) const
{
    PessimisticallyResetGrContext();
}

void SharedContextState::RestoreActiveTextureUnitBinding(unsigned int target) const
{
    PessimisticallyResetGrContext();
}
...
std::optional<error::ContextLostReason> SharedContextState::GetResetStatus(bool needs_gl)
{
    DCHECK(!context_lost());
...
#if BUILDFLAG(SKIA_USE_DAWN)
    if (gr_context_type_ == GrContextType::kGraphiteDawn && dawn_context_provider_) {
    ...
    if (!disable_check_reset_status_throttling_for_test_ && now < last_gl_check_graphics_reset_status_ + kMinCheckDelay) {
        return std::nullopt;
    }
...
#if BUILDFLAG(ENABLE_VULKAN)
        max_texture_size = vk_context_provider()->GetDeviceQueue()->vk_physical_device_properties().limits.maxImageDimension2D;
#else
        NOTREACHED();
#endif
    } else {
...
#if BUILDFLAG(IS_WIN)
Microsoft::WRL::ComPtr<ID3D11Device> SharedContextState::GetD3D11Device() const
{
