...
public:
    ...
    static ExternalSemaphore CreateFromHandle(viz::VulkanContextProvider* context_provider, SemaphoreHandle handle);

    ExternalSemaphore();
    ExternalSemaphore(ExternalSemaphore&& other);
    ExternalSemaphore(base::PassKey<ExternalSemaphore>, viz::VulkanContextProvider* context_provider, VkSemaphore semaphore, SemaphoreHandle handle);
    ~ExternalSemaphore();
    ...
    bool is_valid() const
    {
        return context_provider_ && handle_.is_valid();
    }
