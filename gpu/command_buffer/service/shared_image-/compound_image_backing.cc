...
bool IsValidSharedMemoryBufferFormat(const gfx::Size& size, gfx::BufferFormat buffer_format, gfx::BufferPlane plane)
{
...
public:
    WrappedSkiaCompoundImageRepresentation(
        SharedImageManager* manager, SharedImageBacking* backing, MemoryTypeTracker* tracker, std::unique_ptr<SkiaImageRepresentation> wrapped)
    ...
    sk_sp<SkSurface> BeginWriteAccess(int final_msaa_count, const SkSurfaceProps& surface_props, std::vector<GrBackendSemaphore>* begin_semaphores,
        std::vector<GrBackendSemaphore>* end_semaphores, std::unique_ptr<GrBackendSurfaceMutableState>* end_state) final
    {
        compound_backing()->NotifyBeginAccess(SharedImageAccessStream::kSkia, AccessMode::kWrite);
        return wrapped_->BeginWriteAccess(final_msaa_count, surface_props, begin_semaphores, end_semaphores, end_state);
    }
    sk_sp<SkPromiseImageTexture> BeginWriteAccess(std::vector<GrBackendSemaphore>* begin_semaphores, std::vector<GrBackendSemaphore>* end_semaphores,
...
public:
    WrappedOverlayCompoundImageRepresentation(SharedImageManager* manager, SharedImageBacking* backing, MemoryTypeTracker* tracker,
        SharedImageAccessStream access_stream, std::unique_ptr<OverlayImageRepresentation> wrapped)
    ...
    gl::GLImage* GetGLImage() final
    {
        return wrapped_->GetGLImage();
    }
...
// static
std::unique_ptr<SharedImageBacking> CompoundImageBacking::CreateSharedMemory(SharedImageBackingFactory* gpu_backing_factory, bool allow_shm_overlays,
    const Mailbox& mailbox, gfx::GpuMemoryBufferHandle handle, gfx::BufferFormat buffer_format, gfx::BufferPlane plane, SurfaceHandle surface_handle,
    const gfx::Size& size, const gfx::ColorSpace& color_space, GrSurfaceOrigin surface_origin, SkAlphaType alpha_type, uint32_t usage)
{
    ...
    const viz::ResourceFormat plane_format = viz::GetResourceFormat(GetPlaneBufferFormat(plane, buffer_format));

    const size_t plane_index = plane == gfx::BufferPlane::UV ? 1 : 0;
    handle.offset += gfx::BufferOffsetForBufferFormat(size, buffer_format, plane_index);
    ...
    auto shm_backing = std::make_unique<SharedMemoryImageBacking>(
        mailbox, si_format, plane_size, color_space, surface_origin, alpha_type, SHARED_IMAGE_USAGE_CPU_WRITE, std::move(shm_wrapper));
    shm_backing->SetNotRefCounted();

    return std::make_unique<CompoundImageBacking>(mailbox, si_format, plane_size, color_space, surface_origin, alpha_type, usage, surface_handle,
        allow_shm_overlays, std::move(shm_backing), gpu_backing_factory->GetWeakPtr());
}
...
CompoundImageBacking::CompoundImageBacking(const Mailbox& mailbox, viz::SharedImageFormat format, const gfx::Size& size, const gfx::ColorSpace& color_space,
    GrSurfaceOrigin surface_origin, SkAlphaType alpha_type, uint32_t usage, SurfaceHandle surface_handle, bool allow_shm_overlays,
    std::unique_ptr<SharedMemoryImageBacking> shm_backing, base::WeakPtr<SharedImageBackingFactory> gpu_backing_factory)
    : SharedImageBacking(mailbox, format, size, color_space, surface_origin, alpha_type, usage, shm_backing->estimated_size(),
...
void CompoundImageBacking::NotifyBeginAccess(SharedImageAccessStream stream, RepresentationAccessMode mode)
{
    ...
    if (!gpu_has_latest_content_) {
        ...
        auto& wrapper = static_cast<SharedMemoryImageBacking*>(shm_backing_.get())->shared_memory_wrapper();
        DCHECK(wrapper.IsValid());

        SkPixmap pixmap(shm_backing_->AsSkImageInfo(), wrapper.GetMemory(), wrapper.GetStride());
    ...
    SharedImageManager* manager, MemoryTypeTracker* tracker, WGPUDevice device, WGPUBackendType backend_type)
{
    ...
    auto real_rep = gpu_backing_->ProduceDawn(manager, tracker, device, backend_type);
    if (!real_rep)
        return nullptr;
    ...
    SharedImageManager* manager, MemoryTypeTracker* tracker, scoped_refptr<SharedContextState> context_state)
{
    ...
    auto real_rep = gpu_backing_->ProduceSkia(manager, tracker, std::move(context_state));
    if (!real_rep)
        return nullptr;

    return std::make_unique<WrappedSkiaCompoundImageRepresentation>(manager, this, tracker, std::move(real_rep));
}
...
std::unique_ptr<OverlayImageRepresentation> CompoundImageBacking::ProduceOverlay(SharedImageManager* manager, MemoryTypeTracker* tracker)
{
    if (allow_shm_overlays_) {
        ...
        return std::make_unique<WrappedOverlayCompoundImageRepresentation>(manager, this, tracker, SharedImageAccessStream::kMemory, std::move(real_rep));
    }
    ...
    return std::make_unique<WrappedOverlayCompoundImageRepresentation>(manager, this, tracker, SharedImageAccessStream::kOverlay, std::move(real_rep));
}
    ...
    const std::string& dump_name, base::trace_event::MemoryAllocatorDumpGuid client_guid, base::trace_event::ProcessMemoryDump* pmd, uint64_t client_tracing_id)
{
    ...
    std::string shm_dump_name = base::StringPrintf("%s/shared_memory", dump_name.c_str());
    shm_backing_->OnMemoryDump(shm_dump_name, shm_client_guid, pmd, client_tracing_id);
    ...
    if (gpu_backing_) {
        ...
        gpu_backing_->OnMemoryDump(gpu_dump_name, gpu_client_guid, pmd, client_tracing_id);
    }
}
...
void CompoundImageBacking::LazyAllocateGpuBacking()
{
    ...
    gpu_backing_ = gpu_backing_factory_->CreateSharedImage(mailbox(), format(), surface_handle_, size(), color_space(), surface_origin(), alpha_type(),
