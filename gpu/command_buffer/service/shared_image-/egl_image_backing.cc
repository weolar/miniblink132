...
public:
    ...
    GLRepresentationShared(EGLImageBacking* backing, scoped_refptr<TextureHolder> texture_holder)
        : backing_(backing)
    ...
    {
        if (mode == GL_SHARED_IMAGE_ACCESS_MODE_READ_CHROMIUM || mode == GL_SHARED_IMAGE_ACCESS_MODE_OVERLAY_CHROMIUM) {
...
public:
    GLTextureEGLImageRepresentation(
        SharedImageManager* manager, EGLImageBacking* backing, MemoryTypeTracker* tracker, scoped_refptr<TextureHolder> texture_holder)
...
EGLImageBacking::EGLImageBacking(const Mailbox& mailbox, viz::SharedImageFormat format, const gfx::Size& size, const gfx::ColorSpace& color_space,
    GrSurfaceOrigin surface_origin, SkAlphaType alpha_type, uint32_t usage, size_t estimated_size, const GLCommonImageBackingFactory::FormatInfo& format_info,
    const GpuDriverBugWorkarounds& workarounds, bool use_passthrough, base::span<const uint8_t> pixel_data)
    : ClearTrackingSharedImageBacking(mailbox, format, size, color_space, surface_origin, alpha_type, usage, estimated_size, true /*is_thread_safe*/)
...
template <class T> std::unique_ptr<T> EGLImageBacking::ProduceGLTextureInternal(SharedImageManager* manager, MemoryTypeTracker* tracker)
{
    ...
    // doesn't need lock.
    if (created_on_context_ == gl::g_current_gl_context && source_texture_holder_) {
        return std::make_unique<T>(manager, this, tracker, source_texture_holder_);
    }

    auto texture_holder = GenEGLImageSibling(/*pixel_data=*/base::span<const uint8_t>());
    if (!texture_holder)
        return nullptr;
...
scoped_refptr<EGLImageBacking::TextureHolder> EGLImageBacking::GenEGLImageSibling(base::span<const uint8_t> pixel_data)
{
    ...
    scoped_refptr<gles2::NativeImageBuffer> buffer;
    {
        ...
        if (!egl_image_buffer_) {
            ...
            if (format_info_.supports_storage) {
                api->glTexStorage2DEXTFn(target, 1, format_info_.storage_internal_format, size().width(), size().height());

                if (!pixel_data.empty()) {
                    ScopedUnpackState scoped_unpack_state(
                        /*uploading_data=*/true);
                    api->glTexSubImage2DFn(
                        target, 0, 0, 0, size().width(), size().height(), format_info_.adjusted_format, format_info_.gl_type, pixel_data.data());
                }
            } else if (format_info_.is_compressed) {
                ScopedUnpackState scoped_unpack_state(!pixel_data.empty());
                api->glCompressedTexImage2DFn(
                    target, 0, format_info_.image_internal_format, size().width(), size().height(), 0, pixel_data.size(), pixel_data.data());
            } else {
                ScopedUnpackState scoped_unpack_state(!pixel_data.empty());

                api->glTexImage2DFn(target, 0, format_info_.image_internal_format, size().width(), size().height(), 0, format_info_.adjusted_format,
    ...
    if (use_passthrough_) {
        auto texture_passthrough
            = base::MakeRefCounted<gpu::gles2::TexturePassthrough>(service_id, GL_TEXTURE_2D, format_info_.gl_format, size().width(), size().height(),
                /*depth=*/1, /*border=*/0, format_info_.gl_format, format_info_.gl_type);
        return base::MakeRefCounted<TextureHolder>(std::move(texture_passthrough));
    }

    auto* texture = gles2::CreateGLES2TextureWithLightRef(service_id, GL_TEXTURE_2D);
    ...
    texture->SetLevelInfo(
        GL_TEXTURE_2D, 0, format_info_.gl_format, size().width(), size().height(), 1, 0, format_info_.gl_format, format_info_.gl_type, cleared_rect);

    texture->SetImmutable(true /*immutable*/, false /*immutable_storage*/);
    return base::MakeRefCounted<TextureHolder>(std::move(texture));
}

void EGLImageBacking::SetEndReadFence(scoped_refptr<gl::SharedGLFenceEGL> shared_egl_fence)
{
    AutoLock auto_lock(this);
