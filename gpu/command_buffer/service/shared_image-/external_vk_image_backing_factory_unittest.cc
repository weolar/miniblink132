...
protected:
    ...
    void SetUp() override
    {
        ...
        DCHECK(vulkan_context_provider_) << "Failed to create Vulkan context provider";
        ...
        context_state_ = base::MakeRefCounted<SharedContextState>(std::move(share_group), gl_surface_, gl_context_, false /* use_virtualized_gl_contexts */,
            base::DoNothing(), GrContextType::kVulkan, vulkan_context_provider_.get());
        ...
        shared_image_factory_ = std::make_unique<ExternalVkImageBackingFactory>(context_state_);
        ...
        auto adapter_it = base::ranges::find(adapters, wgpu::BackendType::Vulkan, [](dawn::native::Adapter adapter) {
        ...
        dawn_device_ = wgpu::Device::Acquire(adapter_it->CreateDevice(&device_descriptor));
...
TEST_F(ExternalVkImageBackingFactoryTest, DawnWrite_SkiaVulkanRead)
{
    ...
    const uint32_t usage = SHARED_IMAGE_USAGE_DISPLAY_READ | SHARED_IMAGE_USAGE_WEBGPU;
...

    {
        ...
        auto dawn_representation = shared_image_representation_factory_->ProduceDawn(mailbox, dawn_device_.Get(), WGPUBackendType_Vulkan);
...

    {
        ...
        context_state_->gr_context()->wait(begin_semaphores.size(), begin_semaphores.data(),
        ...
        if (auto end_state = skia_scoped_access->TakeEndState()) {
            context_state_->gr_context()->setBackendTextureState(backend_texture, *end_state);
        }
...
TEST_F(ExternalVkImageBackingFactoryTest, SkiaVulkanWrite_DawnRead)
{
...

    {
        ...
        auto skia_scoped_access = skia_representation->BeginScopedWriteAccess(
            /*final_msaa_count=*/1, SkSurfaceProps(0 /* flags */, kUnknown_SkPixelGeometry), &begin_semaphores, &end_semaphores,
...

    {
        ...
        // contents.
        auto dawn_scoped_access = dawn_representation->BeginScopedAccess(WGPUTextureUsage_CopySrc, SharedImageRepresentation::AllowUnclearedAccess::kNo);
        ASSERT_TRUE(dawn_scoped_access);
