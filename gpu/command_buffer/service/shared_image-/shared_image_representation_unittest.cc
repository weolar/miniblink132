...
public:
    void SetUp() override
    {
        ...
        auto backing = std::make_unique<TestImageBacking>(mailbox_, format, size, color_space, surface_origin, alpha_type, usage, 0 /* estimated_size */);
...
TEST_F(SharedImageRepresentationTest, GLTextureClearing)
{
    ...
    // Clearing underlying GL texture should clear the SharedImage.
    {
        ...
        representation->GetTexture()->SetLevelCleared(GL_TEXTURE_2D, 0, true /* cleared */);
    }
...
{
    auto representation = manager_.ProduceDawn(mailbox_, tracker_.get(), nullptr /* device */, WGPUBackendType_Null);
    EXPECT_FALSE(representation->IsCleared());

    // We should not be able to begin access with |allow_uncleared| == false.
    {
        auto scoped_access = representation->BeginScopedAccess(WGPUTextureUsage_None, SharedImageRepresentation::AllowUnclearedAccess::kNo);
        EXPECT_FALSE(scoped_access);
    }
    EXPECT_FALSE(representation->IsCleared());
    ...
    {
        auto scoped_access = representation->BeginScopedAccess(WGPUTextureUsage_None, SharedImageRepresentation::AllowUnclearedAccess::kYes);
        EXPECT_TRUE(scoped_access);
    }
    EXPECT_FALSE(representation->IsCleared());
...
TEST_F(SharedImageRepresentationTest, OverlayClearing)
{
    ...
    {
        auto scoped_access = representation->BeginScopedReadAccess(false /* needs_gl_image */);
        EXPECT_FALSE(scoped_access);
    }
    EXPECT_FALSE(representation->IsCleared());
