...
public:
    void SetUp() override
    {
        ...
        context_state_
            = base::MakeRefCounted<SharedContextState>(std::move(share_group), surface_, context_, false /* use_virtualized_gl_contexts */, base::DoNothing());
        ...
        backing_factory_ = std::make_unique<AHardwareBufferImageBackingFactory>(context_state_->feature_info());
...
public:
    GlLegacySharedImage(AHardwareBufferImageBackingFactory* backing_factory, bool is_thread_safe, SharedImageManager* shared_image_manager,
...
TEST_F(AHardwareBufferImageBackingFactoryTest, Basic)
{
    ...
    GlLegacySharedImage gl_legacy_shared_image { backing_factory_.get(), false /* is_thread_safe */, &shared_image_manager_, memory_type_tracker_.get(),
...
TEST_F(AHardwareBufferImageBackingFactoryTest, GLSkiaGL)
{
    ...
    auto dst_pixels = ReadPixels(mailbox, size, context_state_.get(), shared_image_representation_factory_.get());
...
TEST_F(AHardwareBufferImageBackingFactoryTest, DISABLED_OnlyOneWriter)
{
    ...
    GlLegacySharedImage gl_legacy_shared_image { backing_factory_.get(), true /* is_thread_safe */, &shared_image_manager_, memory_type_tracker_.get(),
        shared_image_representation_factory_.get() };
...
GlLegacySharedImage::GlLegacySharedImage(AHardwareBufferImageBackingFactory* backing_factory, bool is_thread_safe, SharedImageManager* shared_image_manager,
    MemoryTypeTracker* memory_type_tracker, SharedImageRepresentationFactory* shared_image_representation_factory)
    : size_(256, 256)
{
...
TEST_F(AHardwareBufferImageBackingFactoryTest, Overlay)
{
    ...
    auto scoped_read_access = overlay_representation->BeginScopedReadAccess(true /* needs_gl_image */);
    EXPECT_TRUE(scoped_read_access);
