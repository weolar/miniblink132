...
public:
    ...
    cc::PaintOpBuffer* BeginWriteAccess(scoped_refptr<SharedContextState> context_state, int final_msaa_count, const SkSurfaceProps& surface_props,
        const absl::optional<SkColor4f>& clear_color, bool visible) override
    {
    ...
    cc::PaintOpBuffer* BeginReadAccess(absl::optional<SkColor4f>& clear_color) override
    {
...
class RawDrawImageBacking::SkiaRawDrawImageRepresentation : public SkiaImageRepresentation {
public:
    SkiaRawDrawImageRepresentation(SharedImageManager* manager, SharedImageBacking* backing, MemoryTypeTracker* tracker)
...
size_t RawDrawImageBacking::EstimatedSizeForMemTracking() const
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    AutoLock auto_lock(this);
    return backend_texture_.isValid() ? viz::ResourceSizes::UncheckedSizeInBytes<size_t>(size(), format()) : 0u;
}
    ...
    SharedImageManager* manager, MemoryTypeTracker* tracker, scoped_refptr<SharedContextState> context_state)
{
    ...
    return std::make_unique<SkiaRawDrawImageRepresentation>(manager, this, tracker);
}
...
bool RawDrawImageBacking::CreateBackendTextureAndFlushPaintOps(bool flush)
{
    ...
    backend_texture_
        = context_state_->gr_context()->createBackendTexture(size().width(), size().height(), sk_color, mipmap, GrRenderable::kYes, GrProtected::kNo, label);
    ...
    auto surface = SkSurface::MakeFromBackendTexture(
        context_state_->gr_context(), backend_texture_, surface_origin(), final_msaa_count_, sk_color, color_space().ToSkColorSpace(), &surface_props_);
    if (!surface) {
        DLOG(ERROR) << "SkSurface::MakeFromBackendTexture() failed! SkColorType:" << sk_color;
        DestroyBackendTexture();
        return false;
    }
...
cc::PaintOpBuffer* RawDrawImageBacking::BeginRasterReadAccess(absl::optional<SkColor4f>& clear_color)
{
