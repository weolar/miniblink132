...
public:
    ...
    const scoped_refptr<gles2::TexturePassthrough>& GetTexturePassthrough() const
    {
    ...
    bool need_synchronization() const
    {
        ...
        if ((usage() & SHARED_IMAGE_USAGE_RASTER) && (usage() & SHARED_IMAGE_USAGE_SCANOUT)) {
            return true;
        }
...
protected:
    ...
    std::unique_ptr<DawnImageRepresentation> ProduceDawn(
        SharedImageManager* manager, MemoryTypeTracker* tracker, WGPUDevice dawnDevice, WGPUBackendType backend_type) override;
...
private:
    ...
    bool WritePixelsWithCallback(size_t data_size, size_t stride, WriteBufferCallback callback);
    using ReadBufferCallback = base::OnceCallback<void(const void* buffer)>;
    bool ReadPixelsWithCallback(size_t data_size, size_t stride, ReadBufferCallback callback);
