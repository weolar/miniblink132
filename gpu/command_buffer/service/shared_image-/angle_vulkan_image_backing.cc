...
class AngleVulkanImageBacking::SkiaAngleVulkanImageRepresentation : public SkiaImageRepresentation {
public:
    SkiaAngleVulkanImageRepresentation(SharedImageManager* manager, AngleVulkanImageBacking* backing, MemoryTypeTracker* tracker)
    ...
    {
        auto promise_texture = BeginWriteAccess(begin_semaphores, end_semaphores, end_state);
        if (!promise_texture)
            return nullptr;

        auto surface = backing_impl()->context_state_->GetCachedSkSurface(backing_impl()->promise_texture_.get());
        ...
        if (!surface || surface_props != surface->props() || final_msaa_count != backing_impl()->surface_msaa_count_) {
            SkColorType sk_color_type = viz::ResourceFormatToClosestSkColorType(true /* gpu_compositing */, format());
            surface = SkSurface::MakeFromBackendTexture(backing_impl()->gr_context(), backing_impl()->backend_texture_, surface_origin(), final_msaa_count,
                sk_color_type, backing_impl()->color_space().ToSkColorSpace(), &surface_props);
            if (!surface) {
                backing_impl()->context_state_->EraseCachedSkSurface(backing_impl()->promise_texture_.get());
                return nullptr;
            }
            backing_impl()->surface_msaa_count_ = final_msaa_count;
            backing_impl()->context_state_->CacheSkSurface(backing_impl()->promise_texture_.get(), surface);
        }
    ...
    void EndWriteAccess(sk_sp<SkSurface> surface) override
    {
        if (surface) {
            ...
            surface = nullptr;
            DCHECK(backing_impl()->context_state_->CachedSkSurfaceIsUnique(backing_impl()->promise_texture_.get()));
        }
        backing_impl()->EndAccessSkia();
    }
...
AngleVulkanImageBacking::AngleVulkanImageBacking(const raw_ptr<SharedContextState>& context_state, const Mailbox& mailbox, viz::SharedImageFormat format,
    ...
    : ClearTrackingSharedImageBacking(mailbox, format, size, color_space, surface_origin, alpha_type, usage,
        viz::ResourceSizes::UncheckedSizeInBytes<size_t>(size, format), false /* is_thread_safe */)
...
AngleVulkanImageBacking::~AngleVulkanImageBacking()
{
    ...
    if (vulkan_image_) {
        ...
        fence_helper->EnqueueVulkanObjectCleanupForSubmittedWork(std::move(vulkan_image_));
    }
}
...
bool AngleVulkanImageBacking::Initialize(const base::span<const uint8_t>& data)
{
    ...
    auto vulkan_image = VulkanImage::Create(device_queue, size(), vk_format, vk_usage, vk_flags, VK_IMAGE_TILING_OPTIMAL);
    ...
    SharedImageManager* manager, MemoryTypeTracker* tracker, scoped_refptr<SharedContextState> context_state)
{
    ...
    return std::make_unique<SkiaAngleVulkanImageRepresentation>(manager, this, tracker);
}

bool AngleVulkanImageBacking::GLTextureImageRepresentationBeginAccess(bool readonly)
{
    if (!readonly) {
        // For GL write access.
...
void AngleVulkanImageBacking::GLTextureImageRepresentationEndAccess(bool readonly)
{
    if (readonly) {
        // For GL read access.
        if (gl_reads_in_process_ == 0) {
...
void AngleVulkanImageBacking::GLTextureImageRepresentationRelease(bool have_context)
{
}
...
bool AngleVulkanImageBacking::InitializePassthroughTexture()
{
    ...
    if (!egl_image->Initialize(vulkan_image_->image(), &vulkan_image_->create_info(), viz::GLInternalFormat(format()))) {
        return false;
    }

    scoped_refptr<gles2::TexturePassthrough> passthrough_texture;
    GLTextureImageBackingHelper::MakeTextureAndSetParameters(GL_TEXTURE_2D, /*service_id=*/0,
...
void AngleVulkanImageBacking::WritePixels(const base::span<const uint8_t>& pixel_data, size_t stride)
{
