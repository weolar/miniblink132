...
gfx::Size PlaneSize(DXGI_FORMAT dxgi_format, const gfx::Size& size, size_t plane)
{
    DCHECK_LT(plane, NumPlanes(dxgi_format));
    switch (dxgi_format) {
    case DXGI_FORMAT_NV12:
...
scoped_refptr<gles2::TexturePassthrough> CreateGLTexture(viz::SharedImageFormat format, const gfx::Size& size, const gfx::ColorSpace& color_space,
    Microsoft::WRL::ComPtr<ID3D11Texture2D> d3d11_texture, GLenum texture_target = GL_TEXTURE_2D, unsigned array_slice = 0u, unsigned plane_index = 0u,
    Microsoft::WRL::ComPtr<IDXGISwapChain1> swap_chain = nullptr)
{
    ...
    auto image = base::MakeRefCounted<gl::GLImageD3D>(size, internal_format, data_type, color_space, d3d11_texture, array_slice, plane_index, swap_chain);
...
void CopyPlane(
    const uint8_t* source_memory, size_t source_stride, uint8_t* dest_memory, size_t dest_stride, viz::SharedImageFormat format, const gfx::Size& size)
{
    int row_bytes = size.width() * viz::BitsPerPixel(format) / 8;
    libyuv::CopyPlane(source_memory, source_stride, dest_memory, dest_stride, row_bytes, size.height());
}

} // namespace

// static
std::unique_ptr<D3DImageBacking> D3DImageBacking::CreateFromSwapChainBuffer(const Mailbox& mailbox, viz::ResourceFormat format, const gfx::Size& size,
    const gfx::ColorSpace& color_space, GrSurfaceOrigin surface_origin, SkAlphaType alpha_type, uint32_t usage,
    Microsoft::WRL::ComPtr<ID3D11Texture2D> d3d11_texture, Microsoft::WRL::ComPtr<IDXGISwapChain1> swap_chain, bool is_back_buffer)
{
    auto si_format = viz::SharedImageFormat::SinglePlane(format);
    auto gl_texture = CreateGLTexture(si_format, size, color_space, d3d11_texture, GL_TEXTURE_2D,
    ...
    return base::WrapUnique(
        new D3DImageBacking(mailbox, si_format, size, color_space, surface_origin, alpha_type, usage, std::move(d3d11_texture), std::move(gl_texture),
...
std::unique_ptr<D3DImageBacking> D3DImageBacking::Create(const Mailbox& mailbox, viz::SharedImageFormat format, const gfx::Size& size,
    const gfx::ColorSpace& color_space, GrSurfaceOrigin surface_origin, SkAlphaType alpha_type, uint32_t usage,
    Microsoft::WRL::ComPtr<ID3D11Texture2D> d3d11_texture, scoped_refptr<DXGISharedHandleState> dxgi_shared_handle_state, GLenum texture_target,
    size_t array_slice, size_t plane_index)
{
    ...
    DCHECK(!has_webgpu_usage || (dxgi_shared_handle_state && dxgi_shared_handle_state->has_keyed_mutex()));
    ...
    if (!has_webgpu_usage) {
        ...
        gl_texture = CreateGLTexture(format, size, color_space, d3d11_texture, texture_target, array_slice, plane_index);
        if (!gl_texture) {
            LOG(ERROR) << "Failed to create GL texture";
            return nullptr;
        }
    }
    auto backing = base::WrapUnique(new D3DImageBacking(mailbox, format, size, color_space, surface_origin, alpha_type, usage, std::move(d3d11_texture),
        std::move(gl_texture), std::move(dxgi_shared_handle_state), texture_target, array_slice, plane_index));
    return backing;
}

std::unique_ptr<D3DImageBacking> D3DImageBacking::CreateFromGLTexture(const Mailbox& mailbox, viz::ResourceFormat format, const gfx::Size& size,
    const gfx::ColorSpace& color_space, GrSurfaceOrigin surface_origin, SkAlphaType alpha_type, uint32_t usage,
    Microsoft::WRL::ComPtr<ID3D11Texture2D> d3d11_texture, scoped_refptr<gles2::TexturePassthrough> gl_texture)
{
    return base::WrapUnique(new D3DImageBacking(mailbox, viz::SharedImageFormat::SinglePlane(format), size, color_space, surface_origin, alpha_type, usage,
        std::move(d3d11_texture), std::move(gl_texture)));
}

// static
std::vector<std::unique_ptr<SharedImageBacking>> D3DImageBacking::CreateFromVideoTexture(base::span<const Mailbox> mailboxes, DXGI_FORMAT dxgi_format,
    const gfx::Size& size, uint32_t usage, Microsoft::WRL::ComPtr<ID3D11Texture2D> d3d11_texture, unsigned array_slice,
    scoped_refptr<DXGISharedHandleState> dxgi_shared_handle_state)
{
    ...
    const bool has_keyed_mutex = dxgi_shared_handle_state && dxgi_shared_handle_state->has_keyed_mutex();
    DCHECK(!has_webgpu_usage || has_keyed_mutex);

    std::vector<std::unique_ptr<SharedImageBacking>> shared_images(NumPlanes(dxgi_format));
    for (size_t plane_index = 0; plane_index < shared_images.size(); plane_index++) {
        ...
        if (!has_webgpu_usage) {
            // Creating the GL texture doesn't require exclusive access to the
            // underlying D3D11 texture.
            gl_texture = CreateGLTexture(plane_format, plane_size, kInvalidColorSpace, d3d11_texture, kTextureTarget, array_slice, plane_index);
            if (!gl_texture) {
                LOG(ERROR) << "Failed to create GL texture";
                return {};
            }
        ...
        shared_images[plane_index] = base::WrapUnique(new D3DImageBacking(mailbox, plane_format, plane_size, kInvalidColorSpace, kTopLeft_GrSurfaceOrigin,
            kPremul_SkAlphaType, usage, d3d11_texture, std::move(gl_texture), dxgi_shared_handle_state, kTextureTarget, array_slice, plane_index));
...
D3DImageBacking::D3DImageBacking(const Mailbox& mailbox, viz::SharedImageFormat format, const gfx::Size& size, const gfx::ColorSpace& color_space,
    GrSurfaceOrigin surface_origin, SkAlphaType alpha_type, uint32_t usage, Microsoft::WRL::ComPtr<ID3D11Texture2D> d3d11_texture,
    scoped_refptr<gles2::TexturePassthrough> gl_texture, scoped_refptr<DXGISharedHandleState> dxgi_shared_handle_state, GLenum texture_target,
    size_t array_slice, size_t plane_index, Microsoft::WRL::ComPtr<IDXGISwapChain1> swap_chain, bool is_back_buffer)
    : ClearTrackingSharedImageBacking(mailbox, format, size, color_space, surface_origin, alpha_type, usage,
        gl_texture ? gl_texture->estimated_size() : gfx::BufferSizeForBufferFormat(size, viz::BufferFormat(format)), false /* is_thread_safe */)
...
ID3D11Texture2D* D3DImageBacking::GetOrCreateStagingTexture()
{
    if (!staging_texture_) {
        ...
        HRESULT hr = d3d11_device->CreateTexture2D(&staging_desc, nullptr, &staging_texture_);
        if (FAILED(hr)) {
...
bool D3DImageBacking::UploadFromMemory(const SkPixmap& pixmap)
{
    ...
    if (texture_desc.CPUAccessFlags & D3D11_CPU_ACCESS_WRITE) {
        ...
        hr = device_context->Map(d3d11_texture_.Get(), 0, D3D11_MAP_WRITE, 0, nullptr);
        if (FAILED(hr)) {
            LOG(ERROR) << "Failed to map texture for write. hr = " << std::hex << hr;
            return false;
        }
        device3->WriteToSubresource(d3d11_texture_.Get(), 0, nullptr, source_memory, source_stride, 0);
        ...
        CopyPlane(source_memory, source_stride, dest_memory, dest_stride, format(), size());
        device_context->Unmap(staging_texture, 0);
        device_context->CopySubresourceRegion(d3d11_texture_.Get(), 0, 0, 0, 0, staging_texture, 0, nullptr);
    }
    return true;
}
...
bool D3DImageBacking::ReadbackToMemory(SkPixmap& pixmap)
{
    ...
    if (texture_desc.CPUAccessFlags & D3D11_CPU_ACCESS_READ) {
        ...
        hr = device_context->Map(d3d11_texture_.Get(), 0, D3D11_MAP_READ, 0, nullptr);
        if (FAILED(hr)) {
            LOG(ERROR) << "Failed to map texture for read. hr=" << std::hex << hr;
            return false;
        }
        device3->ReadFromSubresource(dest_memory, dest_stride, 0, d3d11_texture_.Get(), 0, nullptr);
        device_context->Unmap(d3d11_texture_.Get(), 0);
    } else {
        ID3D11Texture2D* staging_texture = GetOrCreateStagingTexture();
        if (!staging_texture)
            return false;
        device_context->CopySubresourceRegion(staging_texture, 0, 0, 0, 0, d3d11_texture_.Get(), 0, nullptr);
        D3D11_MAPPED_SUBRESOURCE mapped_resource = {};
...
WGPUTextureUsageFlags D3DImageBacking::GetAllowedDawnUsages(const WGPUTextureFormat wgpu_format) const
{
    ...
    const WGPUTextureUsageFlags kBasicUsage
        = WGPUTextureUsage_CopySrc | WGPUTextureUsage_CopyDst | WGPUTextureUsage_TextureBinding | WGPUTextureUsage_RenderAttachment;
    switch (wgpu_format) {
    ...
    SharedImageManager* manager, MemoryTypeTracker* tracker, WGPUDevice device, WGPUBackendType backend_type)
{
    ...
    if (backend_type == WGPUBackendType_OpenGLES) {
        return std::make_unique<DawnEGLImageRepresentation>(ProduceGLTexturePassthrough(manager, tracker), manager, this, tracker, device);
    }
#endif
    ...
    if (usage == WGPUTextureUsage_None) {
        LOG(ERROR) << "WGPUTextureUsage is unknown for viz format: " << viz_si_format.ToString();
        return nullptr;
    }
    ...
    texture_descriptor.size = { static_cast<uint32_t>(size().width()), static_cast<uint32_t>(size().height()), 1 };
    ...
    internalDesc.internalUsage = WGPUTextureUsage_CopySrc | WGPUTextureUsage_RenderAttachment | WGPUTextureUsage_TextureBinding;
    texture_descriptor.nextInChain = reinterpret_cast<WGPUChainedStruct*>(&internalDesc);

    // Evict invalid external images e.g. due to their device being destroyed.
    base::EraseIf(dawn_external_images_, [](const auto& kv) { return !kv.second->IsValid(); });
    ...
    if (it == dawn_external_images_.end()) {
        ...
        std::unique_ptr<dawn::native::d3d12::ExternalImageDXGI> external_image = dawn::native::d3d12::ExternalImageDXGI::Create(device, &externalImageDesc);
        if (!external_image) {
    ...
    return std::make_unique<DawnD3DImageRepresentation>(manager, this, tracker, device, external_image_ptr);
#else
    return nullptr;
#endif // BUILDFLAG(USE_DAWN)
}
...
{
    SharedImageBacking::OnMemoryDump(dump_name, client_guid, pmd, client_tracing_id);

    // Add a |service_guid| which expresses shared ownership between the
    // various GPU dumps.
    base::trace_event::MemoryAllocatorDumpGuid service_guid = gl::GetGLTextureServiceGUIDForTracing(gl_texture_->service_id());
...
{
    return gl_texture_ ? gl_texture_->GetLevelImage(gl_texture_->target(), 0u) : nullptr;
}
...
std::unique_ptr<GLTexturePassthroughImageRepresentation> D3DImageBacking::ProduceGLTexturePassthrough(SharedImageManager* manager, MemoryTypeTracker* tracker)
{
    ...
    if (!gl_texture) {
        gl_texture = CreateGLTexture(format(), size(), color_space(), d3d11_texture_, texture_target_, array_slice_, plane_index_, swap_chain_);
        if (!gl_texture) {
            LOG(ERROR) << "Failed to create GL texture";
            return nullptr;
        }
    }
    return std::make_unique<GLTexturePassthroughD3DImageRepresentation>(manager, this, tracker, std::move(gl_texture));
}
...
{
    return SkiaGLImageRepresentation::Create(ProduceGLTexturePassthrough(manager, tracker), std::move(context_state), manager, this, tracker);
}
...
std::unique_ptr<OverlayImageRepresentation> D3DImageBacking::ProduceOverlay(SharedImageManager* manager, MemoryTypeTracker* tracker)
{
    ...
    if (!gl_image) {
        ...
        const auto data_type = viz::GLDataType(format());
        gl_image
            = base::MakeRefCounted<gl::GLImageD3D>(size(), internal_format, data_type, color_space(), d3d11_texture_, array_slice_, plane_index_, swap_chain_);
    }
    return std::make_unique<OverlayD3DImageRepresentation>(manager, this, tracker, std::move(gl_image));
}
