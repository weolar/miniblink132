...
// static
void ExternalVkImageGLRepresentationShared::AcquireTexture(ExternalSemaphore* semaphore, GLuint texture_id, VkImageLayout src_layout)
{
    GLuint gl_semaphore = semaphore->GetGLSemaphore();
    if (gl_semaphore) {
        GLenum gl_layout = VkImageLayoutToGLImageLayout(src_layout);
        auto* api = gl::g_current_gl_context;
        api->glWaitSemaphoreEXTFn(gl_semaphore, 0, nullptr, 1, &texture_id, &gl_layout);
    }
}

// static
ExternalSemaphore ExternalVkImageGLRepresentationShared::ReleaseTexture(ExternalSemaphorePool* pool, GLuint texture_id, VkImageLayout dst_layout)
{
    ...
    auto* api = gl::g_current_gl_context;
    api->glSignalSemaphoreEXTFn(gl_semaphore, 0, nullptr, 1, &texture_id, &gl_layout);
...
ExternalVkImageGLRepresentationShared::ExternalVkImageGLRepresentationShared(SharedImageBacking* backing, GLuint texture_service_id)
...
bool ExternalVkImageGLRepresentationShared::BeginAccess(GLenum mode)
{
    ...
    if (!backing_impl()->BeginAccess(readonly, &begin_access_semaphores_, true /* is_gl */))
        return false;
...
void ExternalVkImageGLRepresentationShared::EndAccess()
{
    ...
    if (backing_impl()->need_synchronization() && backing_impl()->gl_reads_in_progress() <= 1) {
        ...
        DCHECK_NE(info.fImageLayout, VK_IMAGE_LAYOUT_UNDEFINED);
        DCHECK_NE(info.fImageLayout, VK_IMAGE_LAYOUT_PREINITIALIZED);
        external_semaphore = ReleaseTexture(backing_impl()->external_semaphore_pool(), texture_service_id_, info.fImageLayout);
        if (!external_semaphore) {
            backing_impl()->context_state()->MarkContextLost();
            return;
        }
    }
    backing_impl()->EndAccess(readonly, std::move(external_semaphore), true /* is_gl */);
...
ExternalVkImageGLRepresentation::ExternalVkImageGLRepresentation(
    SharedImageManager* manager, SharedImageBacking* backing, MemoryTypeTracker* tracker, gles2::Texture* texture, GLuint texture_service_id)
...
ExternalVkImageGLPassthroughRepresentation::ExternalVkImageGLPassthroughRepresentation(
    SharedImageManager* manager, SharedImageBacking* backing, MemoryTypeTracker* tracker, GLuint texture_service_id)
