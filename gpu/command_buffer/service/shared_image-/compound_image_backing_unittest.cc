...
public:
        ...
        bool is_thread_safe) override
    {
        ...
        return std::make_unique<TestImageBacking>(mailbox, format, size, color_space, surface_origin, alpha_type, usage, 0);
    }
    ...
    std::unique_ptr<SharedImageBacking> CreateSharedImage(const Mailbox& mailbox, int client_id, gfx::GpuMemoryBufferHandle handle, gfx::BufferFormat format,
        ...
        SkAlphaType alpha_type, uint32_t usage) override
    {
        return nullptr;
    }
    bool IsSupported(uint32_t usage, viz::SharedImageFormat format, const gfx::Size& size, bool thread_safe, gfx::GpuMemoryBufferType gmb_type,
...
public:
    ...
    {
        return !backing->gpu_backing_factory_ && !backing->gpu_backing_factory_.WasInvalidated();
    }
    ...
    std::unique_ptr<SharedImageBacking> CreateCompoundBacking(bool allow_shm_overlays = false)
    {
        ...
        return CompoundImageBacking::CreateSharedMemory(&test_factory_, allow_shm_overlays, Mailbox::GenerateForSharedImage(), std::move(handle), buffer_format,
            gfx::BufferPlane::DEFAULT, kNullSurfaceHandle, size, gfx::ColorSpace(), kBottomLeft_GrSurfaceOrigin, kOpaque_SkAlphaType,
