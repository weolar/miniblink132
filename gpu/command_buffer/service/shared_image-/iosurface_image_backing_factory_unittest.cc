...
public:
    void SetUp() override
    {
        ...
        preferences.texture_target_exception_list.push_back(gfx::BufferUsageAndFormat(gfx::BufferUsage::SCANOUT, gfx::BufferFormat::RGBA_8888));
        ...
        backing_factory_ = std::make_unique<GLImageBackingFactory>(preferences, workarounds, context_state_->feature_info(), &image_factory_,
...
TEST_F(IOSurfaceImageBackingFactoryTest, Basic)
{
    ...
    scoped_read_access = skia_representation->BeginScopedReadAccess(nullptr, nullptr);
    auto* promise_texture = scoped_read_access->promise_image_texture();
...
TEST_F(IOSurfaceImageBackingFactoryTest, Dawn_SkiaGL)
{
    ...
    auto adapter_it = base::ranges::find(adapters, wgpu::BackendType::Metal, [](dawn::native::Adapter adapter) {
    ...
    auto dawn_representation = shared_image_representation_factory_->ProduceDawn(mailbox, device.Get(), WGPUBackendType_Metal);
...
TEST_F(IOSurfaceImageBackingFactoryTest, GL_Dawn_Skia_UnclearTexture)
{
    ...
    const uint32_t usage = SHARED_IMAGE_USAGE_GLES2 | SHARED_IMAGE_USAGE_SCANOUT | SHARED_IMAGE_USAGE_WEBGPU;
    auto backing = backing_factory_->CreateSharedImage(
        mailbox, format, surface_handle, size, color_space, surface_origin, alpha_type, usage, false /* is_thread_safe */);
    EXPECT_TRUE(backing);
