...
sk_sp<SkSurface> ExternalVkImageSkiaImageRepresentation::BeginWriteAccess(int final_msaa_count, const SkSurfaceProps& surface_props,
    ...
    std::unique_ptr<GrBackendSurfaceMutableState>* end_state)
{
    ...
    auto promise_texture = BeginAccess(false /* readonly */, begin_semaphores, end_semaphores);
    if (!promise_texture) {
    ...
    if (!surface || surface_props != surface->props() || final_msaa_count != surface_msaa_count_) {
        SkColorType sk_color_type = viz::ResourceFormatToClosestSkColorType(true /* gpu_compositing */, format());
        surface = SkSurface::MakeFromBackendTexture(gr_context, promise_texture->backendTexture(), surface_origin(), final_msaa_count, sk_color_type,
    ...
    if (backing_impl()->need_synchronization()) {
        ...
        *end_state = std::make_unique<GrBackendSurfaceMutableState>(VK_IMAGE_LAYOUT_UNDEFINED, VK_QUEUE_FAMILY_EXTERNAL);
    }

    return surface;
}

sk_sp<SkPromiseImageTexture> ExternalVkImageSkiaImageRepresentation::BeginWriteAccess(std::vector<GrBackendSemaphore>* begin_semaphores,
...
void ExternalVkImageSkiaImageRepresentation::EndWriteAccess(sk_sp<SkSurface> surface)
{
...
sk_sp<SkPromiseImageTexture> ExternalVkImageSkiaImageRepresentation::BeginReadAccess(std::vector<GrBackendSemaphore>* begin_semaphores,
    std::vector<GrBackendSemaphore>* end_semaphores, std::unique_ptr<GrBackendSurfaceMutableState>* end_state)
{
    if (access_mode_ != kNone) {
    ...
    auto promise_texture = BeginAccess(true /* readonly */, begin_semaphores, end_semaphores);
    if (!promise_texture) {
        LOG(ERROR) << "BeginAccess failed";
        return nullptr;
    }
...
sk_sp<SkPromiseImageTexture> ExternalVkImageSkiaImageRepresentation::BeginAccess(
    bool readonly, std::vector<GrBackendSemaphore>* begin_semaphores, std::vector<GrBackendSemaphore>* end_semaphores)
{
    ...
    if (!backing_impl()->BeginAccess(readonly, &begin_access_semaphores_, false /* is_gl */)) {
        return nullptr;
    }
    ...
    if (backing_impl()->need_synchronization()) {
        ...
        end_access_semaphore_ = backing_impl()->external_semaphore_pool()->GetOrCreateSemaphore();
...
void ExternalVkImageSkiaImageRepresentation::EndAccess(bool readonly)
{
    ...
    DCHECK(result);
    DCHECK(!backing_impl()->need_synchronization() || info.fCurrentQueueFamily == VK_QUEUE_FAMILY_EXTERNAL);
#endif

    backing_impl()->EndAccess(readonly, std::move(end_access_semaphore_), false /* is_gl */);
    ...
    backing_impl()->ReturnPendingSemaphoresWithFenceHelper(std::move(begin_access_semaphores_));
