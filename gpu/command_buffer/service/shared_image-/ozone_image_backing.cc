...
class OzoneImageBacking::VaapiOzoneImageRepresentation : public VaapiImageRepresentation {
public:
    VaapiOzoneImageRepresentation(SharedImageManager* manager, SharedImageBacking* backing, MemoryTypeTracker* tracker, VaapiDependencies* vaapi_dependency)
...
private:
    ...
    {
        ozone_backing()->has_pending_va_writes_ = true;
    }
...
private:
    bool BeginReadAccess(gfx::GpuFenceHandle& acquire_fence) override
    {
        ...
        bool need_end_fence;
        ozone_backing->BeginAccess(/*readonly=*/true, AccessStream::kOverlay, &fences, need_end_fence);
    ...
    gl::GLImage* GetGLImage() override
    {
        if (!gl_image_) {
            ...
            auto pixmap = static_cast<OzoneImageBacking*>(backing())->GetNativePixmap();
            gl_image_ = base::MakeRefCounted<gl::GLImageNativePixmap>(pixmap->GetBufferSize(), buffer_format);
    ...
    SharedImageManager* manager, MemoryTypeTracker* tracker, WGPUDevice device, WGPUBackendType backend_type)
{
    ...
    return std::make_unique<DawnOzoneImageRepresentation>(manager, this, tracker, device, webgpu_format, pixmap_, dawn_procs_);
#else // !BUILDFLAG(USE_DAWN)
    return nullptr;
#endif
}

std::unique_ptr<GLTextureImageRepresentation> OzoneImageBacking::ProduceGLTexture(SharedImageManager* manager, MemoryTypeTracker* tracker)
{
    ...
    return GLTextureOzoneImageRepresentation::Create(manager, this, tracker, pixmap_, plane_, need_cache ? &cached_texture_holder_ : nullptr);
}
...
std::unique_ptr<GLTexturePassthroughImageRepresentation> OzoneImageBacking::ProduceGLTexturePassthrough(SharedImageManager* manager, MemoryTypeTracker* tracker)
{
    ...
    const bool need_cache = workarounds_.cache_texture_in_ozone_backing;
    return GLTexturePassthroughOzoneImageRepresentation::Create(manager, this, tracker, pixmap_, plane_, need_cache ? &cached_texture_holder_ : nullptr);
}
    ...
    SharedImageManager* manager, MemoryTypeTracker* tracker, scoped_refptr<SharedContextState> context_state)
{
    ...
    if (context_state->GrContextIsVulkan()) {
        ...
        auto vulkan_image
            = vulkan_implementation->CreateImageFromGpuMemoryHandle(device_queue, std::move(gmb_handle), size(), ToVkFormat(format()), gfx::ColorSpace());

        if (!vulkan_image)
            return nullptr;

        return std::make_unique<SkiaVkOzoneImageRepresentation>(manager, this, std::move(context_state), std::move(vulkan_image), tracker);
    }
...
OzoneImageBacking::OzoneImageBacking(const Mailbox& mailbox, viz::SharedImageFormat format, gfx::BufferPlane plane, const gfx::Size& size,
    const gfx::ColorSpace& color_space, GrSurfaceOrigin surface_origin, SkAlphaType alpha_type, uint32_t usage, scoped_refptr<SharedContextState> context_state,
    scoped_refptr<gfx::NativePixmap> pixmap, scoped_refptr<base::RefCountedData<DawnProcTable>> dawn_procs, const GpuDriverBugWorkarounds& workarounds,
    bool use_passthrough)
    : ClearTrackingSharedImageBacking(mailbox, format, size, color_space, surface_origin, alpha_type, usage, GetPixmapSizeInBytes(*pixmap), false)
    , plane_(plane)
...
{
    bool used_by_skia = (usage & SHARED_IMAGE_USAGE_RASTER) || (usage & SHARED_IMAGE_USAGE_DISPLAY_READ);
    bool used_by_gl = (usage & SHARED_IMAGE_USAGE_GLES2) || (used_by_skia && context_state_->gr_context_type() == GrContextType::kGL);
...
std::unique_ptr<VaapiImageRepresentation> OzoneImageBacking::ProduceVASurface(
    SharedImageManager* manager, MemoryTypeTracker* tracker, VaapiDependenciesFactory* dep_factory)
{
    DCHECK(pixmap_);
    if (!vaapi_deps_)
    ...
    return std::make_unique<OzoneImageBacking::VaapiOzoneImageRepresentation>(manager, this, tracker, vaapi_deps_.get());
}
...
bool OzoneImageBacking::UploadFromMemory(const SkPixmap& pixmap)
{
    ...
    auto representation = ProduceSkia(nullptr, context_state_->memory_type_tracker(), context_state_);
    ...
    bool written = context_state_->gr_context()->updateBackendTexture(dest_scoped_access->promise_image_texture()->backendTexture(), &pixmap,
    ...
    if (written && !representation->IsCleared()) {
        representation->SetClearedRect(gfx::Rect(pixmap.info().width(), pixmap.info().height()));
    }
    return written;
}
...
bool OzoneImageBacking::BeginAccess(bool readonly, AccessStream access_stream, std::vector<gfx::GpuFenceHandle>* fences, bool& need_end_fence)
{
    ...
    if (readonly) {
        ...
        // write access ends.
        need_end_fence = (write_streams_count_ > 1) || !(usage() & SHARED_IMAGE_USAGE_RASTER_DELEGATED_COMPOSITING);
    }
