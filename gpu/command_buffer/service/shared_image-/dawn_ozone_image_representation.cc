...
DawnOzoneImageRepresentation::DawnOzoneImageRepresentation(SharedImageManager* manager, SharedImageBacking* backing, MemoryTypeTracker* tracker,
    WGPUDevice device, WGPUTextureFormat format, scoped_refptr<gfx::NativePixmap> pixmap, scoped_refptr<base::RefCountedData<DawnProcTable>> dawn_procs)
...
WGPUTexture DawnOzoneImageRepresentation::BeginAccess(WGPUTextureUsage usage)
{
    ...
    if (!ozone_backing()->BeginAccess(
            /*readonly=*/false, OzoneImageBacking::AccessStream::kWebGPU, &fences, need_end_fence)) {
        return nullptr;
    }
    ...
    if (pixmap_->GetNumberOfPlanes() == 1) {
        internalDesc.internalUsage |= WGPUTextureUsage_RenderAttachment | WGPUTextureUsage_TextureBinding;
    }
    ...
    if (!texture_) {
        ozone_backing()->EndAccess(/*readonly=*/false, OzoneImageBacking::AccessStream::kWebGPU, gfx::GpuFenceHandle());
        close(fd);
    }
...
void DawnOzoneImageRepresentation::EndAccess()
{
    ...
    if (!dawn::native::vulkan::ExportVulkanImage(texture_, VK_IMAGE_LAYOUT_UNDEFINED, &export_info)) {
        ...
        ozone_backing()->EndAccess(/*readonly=*/false, OzoneImageBacking::AccessStream::kWebGPU, std::move(fence));
    }
