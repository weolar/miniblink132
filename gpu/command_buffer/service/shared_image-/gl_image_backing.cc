...
using InitializeGLTextureParams = GLTextureImageBackingHelper::InitializeGLTextureParams;
...
GLTextureGLCommonRepresentation::GLTextureGLCommonRepresentation(
    SharedImageManager* manager, SharedImageBacking* backing, GLTextureImageRepresentationClient* client, MemoryTypeTracker* tracker, gles2::Texture* texture)
...
void GLTextureGLCommonRepresentation::EndAccess()
{
    ...
    mode_ = 0;
    if (client_)
        return client_->GLTextureImageRepresentationEndAccess(current_mode != GL_SHARED_IMAGE_ACCESS_MODE_READWRITE_CHROMIUM);
}

///////////////////////////////////////////////////////////////////////////////
// GLTexturePassthroughGLCommonRepresentation

GLTexturePassthroughGLCommonRepresentation::GLTexturePassthroughGLCommonRepresentation(SharedImageManager* manager, SharedImageBacking* backing,
    GLTextureImageRepresentationClient* client, MemoryTypeTracker* tracker, scoped_refptr<gles2::TexturePassthrough> texture_passthrough)
...
GLTexturePassthroughGLCommonRepresentation::~GLTexturePassthroughGLCommonRepresentation()
{
...
const scoped_refptr<gles2::TexturePassthrough>& GLTexturePassthroughGLCommonRepresentation::GetTexturePassthrough(int plane_index)
{
    DCHECK_EQ(plane_index, 0);
...
SkiaGLCommonRepresentation::SkiaGLCommonRepresentation(SharedImageManager* manager, SharedImageBacking* backing, GLTextureImageRepresentationClient* client,
    scoped_refptr<SharedContextState> context_state, sk_sp<SkPromiseImageTexture> promise_texture, MemoryTypeTracker* tracker)
...
sk_sp<SkSurface> SkiaGLCommonRepresentation::BeginWriteAccess(int final_msaa_count, const SkSurfaceProps& surface_props,
    ...
    std::unique_ptr<GrBackendSurfaceMutableState>* end_state)
{
    ...
    auto surface = SkSurface::MakeFromBackendTexture(context_state_->gr_context(), promise_texture_->backendTexture(), surface_origin(), final_msaa_count,
        sk_color_type, backing()->color_space().GetAsFullRangeRGB().ToSkColorSpace(), &surface_props);
    write_surface_ = surface.get();
    return surface;
}

sk_sp<SkPromiseImageTexture> SkiaGLCommonRepresentation::BeginWriteAccess(std::vector<GrBackendSemaphore>* begin_semaphores,
...
sk_sp<SkPromiseImageTexture> SkiaGLCommonRepresentation::BeginReadAccess(std::vector<GrBackendSemaphore>* begin_semaphores,
    std::vector<GrBackendSemaphore>* end_semaphores, std::unique_ptr<GrBackendSurfaceMutableState>* end_state)
{
    CheckContext();
    if (client_) {
...
OverlayGLImageRepresentation::OverlayGLImageRepresentation(
    SharedImageManager* manager, SharedImageBacking* backing, MemoryTypeTracker* tracker, scoped_refptr<gl::GLImage> gl_image)
...
bool OverlayGLImageRepresentation::BeginReadAccess(gfx::GpuFenceHandle& acquire_fence)
{
...
void OverlayGLImageRepresentation::EndReadAccess(gfx::GpuFenceHandle release_fence)
{
    auto* gl_backing = static_cast<GLImageBacking*>(backing());
...
// static
std::unique_ptr<GLImageBacking> GLImageBacking::CreateFromGLTexture(scoped_refptr<gl::GLImage> image, const Mailbox& mailbox, viz::ResourceFormat format,
    const gfx::Size& size, const gfx::ColorSpace& color_space, GrSurfaceOrigin surface_origin, SkAlphaType alpha_type, uint32_t usage, GLenum texture_target,
    scoped_refptr<gles2::TexturePassthrough> wrapped_gl_texture)
{
    ...
    auto shared_image
        = std::make_unique<GLImageBacking>(std::move(image), mailbox, si_format, size, color_space, surface_origin, alpha_type, usage, params, true);
...
GLImageBacking::GLImageBacking(scoped_refptr<gl::GLImage> image, const Mailbox& mailbox, viz::SharedImageFormat format, const gfx::Size& size,
    const gfx::ColorSpace& color_space, GrSurfaceOrigin surface_origin, SkAlphaType alpha_type, uint32_t usage, const InitializeGLTextureParams& params,
    bool is_passthrough)
    : SharedImageBacking(mailbox, format, size, color_space, surface_origin, alpha_type, usage, viz::ResourceSizes::UncheckedSizeInBytes<size_t>(size, format),
...
void GLImageBacking::RetainGLTexture()
{
    ...
    GLTextureImageBackingHelper::MakeTextureAndSetParameters(gl_params_.target, 0 /* service_id */, gl_params_.framebuffer_attachment_angle,
        is_passthrough_ ? &passthrough_texture_ : nullptr, is_passthrough_ ? nullptr : &texture_);

    // Set the GLImage to be initially unbound from the GL texture.
    image_bind_or_copy_needed_ = true;
    if (is_passthrough_) {
        passthrough_texture_->SetEstimatedSize(viz::ResourceSizes::UncheckedSizeInBytes<size_t>(size(), format()));
        ...
        texture_->SetLevelInfo(
            gl_params_.target, 0, gl_params_.internal_format, size().width(), size().height(), 1, 0, gl_params_.format, gl_params_.type, cleared_rect_);
        texture_->SetLevelImage(gl_params_.target, 0, image_.get(), gles2::Texture::UNBOUND);
...
void GLImageBacking::ReleaseGLTexture(bool have_context)
{
    ...
    if (cached_promise_texture_) {
        if (cached_promise_texture_->backendTexture().backend() == GrBackendApi::kOpenGL) {
...
std::unique_ptr<GLTextureImageRepresentation> GLImageBacking::ProduceGLTexture(SharedImageManager* manager, MemoryTypeTracker* tracker)
{
    ...
    RetainGLTexture();
    DCHECK(texture_);
    return std::make_unique<GLTextureGLCommonRepresentation>(manager, this, this, tracker, texture_);
}
std::unique_ptr<GLTexturePassthroughImageRepresentation> GLImageBacking::ProduceGLTexturePassthrough(SharedImageManager* manager, MemoryTypeTracker* tracker)
{
    // The corresponding release will be done when the returned representation is
    // destroyed, in GLTextureImageRepresentationRelease.
    RetainGLTexture();
    DCHECK(passthrough_texture_);
    return std::make_unique<GLTexturePassthroughGLCommonRepresentation>(manager, this, this, tracker, passthrough_texture_);
}

std::unique_ptr<OverlayImageRepresentation> GLImageBacking::ProduceOverlay(SharedImageManager* manager, MemoryTypeTracker* tracker)
{
#if BUILDFLAG(IS_MAC) || defined(USE_OZONE) || BUILDFLAG(IS_WIN)
    return std::make_unique<OverlayGLImageRepresentation>(manager, this, tracker, image_);
...
{
#if BUILDFLAG(IS_MAC)
    auto result = IOSurfaceImageBackingFactory::ProduceDawn(manager, this, tracker, device, image_);
    if (result)
        return result;
    ...
    return GLTextureImageBackingHelper::ProduceDawnCommon(factory(), manager, tracker, device, backend_type, this, IsPassthrough());
}
...
#if BUILDFLAG(IS_MAC)
            cached_promise_texture_ = IOSurfaceImageBackingFactory::ProduceSkiaPromiseTextureMetal(this, context_state, image_);
            DCHECK(cached_promise_texture_);
#endif
        } else {
            GrBackendTexture backend_texture;
            GetGrBackendTexture(context_state->feature_info(), GetGLTarget(), size(), GetGLServiceId(), format().resource_format(),
                context_state->gr_context()->threadSafeProxy(), &backend_texture);
            cached_promise_texture_ = SkPromiseImageTexture::Make(backend_texture);
        }
    }
    return std::make_unique<SkiaGLCommonRepresentation>(manager, this, gl_client, std::move(context_state), cached_promise_texture_, tracker);
}

MemoryGLImageRepresentation::MemoryGLImageRepresentation(
    SharedImageManager* manager, SharedImageBacking* backing, MemoryTypeTracker* tracker, scoped_refptr<gl::GLImageMemory> image_memory)
...
{
    return SkPixmap(backing()->AsSkImageInfo(), image_memory_->memory(), image_memory_->stride());
}

std::unique_ptr<MemoryImageRepresentation> GLImageBacking::ProduceMemory(SharedImageManager* manager, MemoryTypeTracker* tracker)
{
    gl::GLImageMemory* image_memory = gl::GLImageMemory::FromGLImage(image_.get());
    if (!image_memory)
        return nullptr;

    return std::make_unique<MemoryGLImageRepresentation>(manager, this, tracker, base::WrapRefCounted(image_memory));
}
...
void GLImageBacking::Update(std::unique_ptr<gfx::GpuFence> in_fence)
{
    if (in_fence) {
        ...
        // the fence.
        std::unique_ptr<gl::GLFence> egl_fence = gl::GLFence::CreateFromGpuFence(*in_fence.get());
...
void GLImageBacking::GLTextureImageRepresentationEndAccess(bool readonly)
{
    ...
    bool needs_sync_for_swangle = (gl::GetANGLEImplementation() == gl::ANGLEImplementation::kSwiftShader && (num_ongoing_read_accesses_ == 0));
    ...
    bool needs_sync_for_metal = (gl::GetANGLEImplementation() == gl::ANGLEImplementation::kMetal && !readonly);

    bool needs_synchronization = IsPassthrough() && (needs_sync_for_swangle || needs_sync_for_metal);
    if (needs_synchronization && image_->ShouldBindOrCopy() == gl::GLImage::BIND) {
        const GLenum target = GetGLTarget();
        gl::ScopedTextureBinder binder(target, passthrough_texture_->service_id());
        if (!passthrough_texture_->is_bind_pending()) {
            image_->ReleaseTexImage(target);
    ...
    if (!readonly && usage() & SHARED_IMAGE_USAGE_SCANOUT && gl::GLFence::IsGpuFenceSupported()) {
...
bool GLImageBacking::BindOrCopyImageIfNeeded()
{
    ...
    DCHECK(new_state == gles2::Texture::BOUND || new_state == gles2::Texture::COPIED);
    if (IsPassthrough()) {
...
void GLImageBacking::InitializePixels(GLenum format, GLenum type, const uint8_t* data)
{
    ...
    ScopedUnpackState scoped_unpack_state(
        /*uploading_data=*/true);
    api->glTexSubImage2DFn(target, 0, 0, 0, size().width(), size().height(), format, type, data);
