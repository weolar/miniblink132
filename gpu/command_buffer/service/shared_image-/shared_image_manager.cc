    ...
    : display_context_on_another_thread_(display_context_on_another_thread)
{
    ...
    if (d3d11_device) {
        dxgi_shared_handle_manager_ = base::MakeRefCounted<DXGISharedHandleManager>(std::move(d3d11_device));
    }
#endif
    ...
    if (base::ThreadTaskRunnerHandle::IsSet()) {
        ...
        base::trace_event::MemoryDumpManager::GetInstance()->RegisterDumpProvider(this, "SharedImageManager", base::ThreadTaskRunnerHandle::Get());
    }
}
...
std::unique_ptr<SharedImageRepresentationFactoryRef> SharedImageManager::Register(std::unique_ptr<SharedImageBacking> backing, MemoryTypeTracker* tracker)
{
    ...
    auto factory_ref = std::make_unique<SharedImageRepresentationFactoryRef>(this, backing.get(), tracker);
...
std::unique_ptr<DawnImageRepresentation> SharedImageManager::ProduceDawn(
    const Mailbox& mailbox, MemoryTypeTracker* tracker, WGPUDevice device, WGPUBackendType backend_type)
{
    CALLED_ON_VALID_THREAD();
    ...
    auto representation = (*found)->ProduceDawn(this, tracker, device, backend_type);
    if (!representation) {
        LOG(ERROR) << "SharedImageManager::ProduceDawn: Trying to produce a "
...
std::unique_ptr<VaapiImageRepresentation> SharedImageManager::ProduceVASurface(
    const Mailbox& mailbox, MemoryTypeTracker* tracker, VaapiDependenciesFactory* dep_factory)
{
    CALLED_ON_VALID_THREAD();
