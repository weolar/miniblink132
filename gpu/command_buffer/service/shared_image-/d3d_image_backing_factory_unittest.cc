...
void FillYUV(uint8_t* data, const gfx::Size& size, uint8_t y_fill_value, uint8_t u_fill_value, uint8_t v_fill_value)
{
...
void CheckYUV(const uint8_t* data, size_t stride, const gfx::Size& size, uint8_t y_fill_value, uint8_t u_fill_value, uint8_t v_fill_value)
{
...
public:
    void SetUp() override
    {
        ...
        shared_image_factory_
            = std::make_unique<D3DImageBackingFactory>(gl::QueryD3D11DeviceObjectFromANGLE(), shared_image_manager_.dxgi_shared_handle_manager());
    }
...
TEST_F(D3DImageBackingFactoryTestSwapChain, CreateAndPresentSwapChain)
{
    ...
    auto surface_origin = kTopLeft_GrSurfaceOrigin;
    auto alpha_type = kPremul_SkAlphaType;
    uint32_t usage = gpu::SHARED_IMAGE_USAGE_GLES2 | gpu::SHARED_IMAGE_USAGE_GLES2_FRAMEBUFFER_HINT | gpu::SHARED_IMAGE_USAGE_DISPLAY_READ
    ...
    auto back_texture = shared_image_representation_factory_->ProduceGLTexturePassthrough(back_buffer_mailbox)->GetTexturePassthrough();
    ...
    auto* back_image = gl::GLImageD3D::FromGLImage(back_texture->GetLevelImage(GL_TEXTURE_2D, 0));

    auto front_texture = shared_image_representation_factory_->ProduceGLTexturePassthrough(front_buffer_mailbox)->GetTexturePassthrough();
    ...
    auto* front_image = gl::GLImageD3D::FromGLImage(front_texture->GetLevelImage(GL_TEXTURE_2D, 0));

    ASSERT_TRUE(back_image);
    EXPECT_EQ(back_image->ShouldBindOrCopy(), gl::GLImage::BIND);

    Microsoft::WRL::ComPtr<ID3D11Texture2D> d3d11_texture;
    EXPECT_EQ(S_OK, back_image->swap_chain()->GetBuffer(0 /* buffer_index */, IID_PPV_ARGS(&d3d11_texture)));
    ...
    EXPECT_EQ(S_OK, front_image->swap_chain()->GetBuffer(1 /* buffer_index */, IID_PPV_ARGS(&d3d11_texture)));
    EXPECT_TRUE(d3d11_texture);
    ...
    api->glRenderbufferStorageMultisampleFn(GL_RENDERBUFFER, 4 /* sample_count */, GL_RGBA8_OES, 1, 1);
...
protected:
    ...
    void CheckSkiaPixels(const Mailbox& mailbox, const gfx::Size& size, const std::vector<uint8_t> expected_color) const
    {
        ...
        auto sk_image = SkImage::MakeFromTexture(gr_context(), backend_texture, kTopLeft_GrSurfaceOrigin, kRGBA_8888_SkColorType, kOpaque_SkAlphaType, nullptr);
    ...
    std::vector<std::unique_ptr<SharedImageRepresentationFactoryRef>> CreateVideoImages(
...
TEST_F(D3DImageBackingFactoryTest, GL_SkiaGL)
{
    ...
    const uint32_t usage = SHARED_IMAGE_USAGE_GLES2 | SHARED_IMAGE_USAGE_DISPLAY_READ;
    const gpu::SurfaceHandle surface_handle = gpu::kNullSurfaceHandle;
    auto backing = shared_image_factory_->CreateSharedImage(
        mailbox, format, surface_handle, size, color_space, kTopLeft_GrSurfaceOrigin, kPremul_SkAlphaType, usage, false /* is_thread_safe */);
...
TEST_F(D3DImageBackingFactoryTest, Dawn_SkiaGL)
{
    ...
    auto adapter_it = base::ranges::find(adapters, wgpu::BackendType::D3D12, [](dawn::native::Adapter adapter) {
    ...
    wgpu::Device device = wgpu::Device::Acquire(adapter_it->CreateDevice(&device_descriptor));
    ...
    const auto color_space = gfx::ColorSpace::CreateSRGB();
    const gpu::SurfaceHandle surface_handle = gpu::kNullSurfaceHandle;
    const uint32_t usage = SHARED_IMAGE_USAGE_WEBGPU | SHARED_IMAGE_USAGE_DISPLAY_READ;
    auto backing = shared_image_factory_->CreateSharedImage(
        mailbox, format, surface_handle, size, color_space, kTopLeft_GrSurfaceOrigin, kPremul_SkAlphaType, usage, false /* is_thread_safe */);
    ASSERT_NE(backing, nullptr);
    ...
    // Clear the shared image to green using Dawn.
    {
        ...
        auto dawn_representation = shared_image_representation_factory_->ProduceDawn(mailbox, device.Get(), WGPUBackendType_D3D12);
        ASSERT_TRUE(dawn_representation);

        auto scoped_access = dawn_representation->BeginScopedAccess(WGPUTextureUsage_RenderAttachment, SharedImageRepresentation::AllowUnclearedAccess::kYes);
...
TEST_F(D3DImageBackingFactoryTest, GL_Dawn_Skia_UnclearTexture)
{
    ...
    const gfx::Size size(1, 1);
    const auto color_space = gfx::ColorSpace::CreateSRGB();
    const uint32_t usage = SHARED_IMAGE_USAGE_GLES2 | SHARED_IMAGE_USAGE_DISPLAY_READ | SHARED_IMAGE_USAGE_WEBGPU;
    const gpu::SurfaceHandle surface_handle = gpu::kNullSurfaceHandle;
    auto backing = shared_image_factory_->CreateSharedImage(
        mailbox, format, surface_handle, size, color_space, kTopLeft_GrSurfaceOrigin, kPremul_SkAlphaType, usage, false /* is_thread_safe */);
    ASSERT_NE(backing, nullptr);
    ...
    dawnProcSetProcs(&procs);
    {
        ...
        auto dawn_scoped_access
            = dawn_representation->BeginScopedAccess(WGPUTextureUsage_RenderAttachment, SharedImageRepresentation::AllowUnclearedAccess::kYes);
...
void D3DImageBackingFactoryTest::RunCreateSharedImageFromHandleTest(DXGI_FORMAT dxgi_format)
{
    ...
    const auto format = viz::SharedImageFormat::SinglePlane(viz::GetResourceFormat(buffer_format));
    const gfx::Size size(1, 1);
    ...
    EXPECT_TRUE(
        shared_image_factory_->IsSupported(usage, format, size, /*thread_safe=*/false, gfx::DXGI_SHARED_HANDLE, GrContextType::kGL, /*pixel_data=*/ {}));
    ...
    auto backing = shared_image_factory_->CreateSharedImage(
        mailbox, 0, std::move(gpu_memory_buffer_handle), buffer_format, plane, surface_handle, size, color_space, surface_origin, alpha_type, usage);
    ASSERT_NE(backing, nullptr);
    ...
    auto dup_backing = shared_image_factory_->CreateSharedImage(
        dup_mailbox, 0, std::move(dup_handle), buffer_format, plane, surface_handle, size, color_space, surface_origin, alpha_type, usage);
...
std::vector<std::unique_ptr<SharedImageRepresentationFactoryRef>> D3DImageBackingFactoryTest::CreateVideoImages(
    const gfx::Size& size, uint8_t y_fill_value, uint8_t u_fill_value, uint8_t v_fill_value, bool use_shared_handle, bool use_factory)
{
    ...
    uint32_t usage = gpu::SHARED_IMAGE_USAGE_VIDEO_DECODE | gpu::SHARED_IMAGE_USAGE_GLES2 | gpu::SHARED_IMAGE_USAGE_RASTER
    ...
    const gpu::Mailbox mailboxes[kNumPlanes] = { gpu::Mailbox::GenerateForSharedImage(), gpu::Mailbox::GenerateForSharedImage() };
    const gfx::BufferPlane planes[kNumPlanes] = { gfx::BufferPlane::Y, gfx::BufferPlane::UV };

    std::vector<std::unique_ptr<SharedImageBacking>> shared_image_backings;
    if (use_factory) {
        HANDLE dup_handle = nullptr;
        if (!::DuplicateHandle(::GetCurrentProcess(), shared_handle.get(), ::GetCurrentProcess(), &dup_handle, 0, false, DUPLICATE_SAME_ACCESS)) {
            return {};
        }
        ...
        for (size_t plane = 0; plane < kNumPlanes; plane++) {
            auto backing = shared_image_factory_->CreateSharedImage(mailboxes[plane], 0, std::move(gmb_handles[plane]), gfx::BufferFormat::YUV_420_BIPLANAR,
                planes[plane], kNullSurfaceHandle, size, gfx::ColorSpace(), kTopLeft_GrSurfaceOrigin, kPremul_SkAlphaType, usage);
            if (!backing)
                return {};
        ...
        shared_image_backings = D3DImageBacking::CreateFromVideoTexture(mailboxes, DXGI_FORMAT_NV12, size, usage, d3d11_texture,
            /*array_slice=*/0, std::move(dxgi_shared_handle_state));
    }
    EXPECT_EQ(shared_image_backings.size(), kNumPlanes);

    const gfx::Size plane_sizes[kNumPlanes] = { size, gfx::Size(size.width() / 2, size.height() / 2) };
    const viz::ResourceFormat plane_formats[kNumPlanes] = { viz::RED_8, viz::RG_88 };

    std::vector<std::unique_ptr<SharedImageRepresentationFactoryRef>> shared_image_refs;
    for (size_t i = 0; i < std::min(shared_image_backings.size(), kNumPlanes); i++) {
        auto& backing = shared_image_backings[i];

        EXPECT_EQ(backing->mailbox(), mailboxes[i]);
        EXPECT_EQ(backing->size(), plane_sizes[i]);
        EXPECT_EQ(backing->format(), viz::SharedImageFormat::SinglePlane(plane_formats[i]));
        ...
        shared_image_refs.push_back(shared_image_manager_.Register(std::move(backing), memory_type_tracker_.get()));
    }
...
void D3DImageBackingFactoryTest::RunVideoTest(bool use_shared_handle, bool use_factory)
{
    ...
    auto shared_image_refs = CreateVideoImages(size, kYFillValue, kUFillValue, kVFillValue, use_shared_handle, use_factory);
    ASSERT_EQ(shared_image_refs.size(), 2u);

    // Setup GL shaders, framebuffers, uniforms, etc.
    static const char* kVideoFragmentShaderSrc = "#extension GL_OES_EGL_image_external : require\n"
    ...
    // samplers, and draw.
    {
        auto y_texture = shared_image_representation_factory_->ProduceGLTexturePassthrough(shared_image_refs[0]->mailbox());
        ASSERT_NE(y_texture, nullptr);

        auto uv_texture = shared_image_representation_factory_->ProduceGLTexturePassthrough(shared_image_refs[1]->mailbox());
        ASSERT_NE(uv_texture, nullptr);

        auto y_texture_access = y_texture->BeginScopedAccess(GL_SHARED_IMAGE_ACCESS_MODE_READ_CHROMIUM, SharedImageRepresentation::AllowUnclearedAccess::kNo);
        ASSERT_NE(y_texture_access, nullptr);

        auto uv_texture_access = uv_texture->BeginScopedAccess(GL_SHARED_IMAGE_ACCESS_MODE_READ_CHROMIUM, SharedImageRepresentation::AllowUnclearedAccess::kNo);
        ASSERT_NE(uv_texture_access, nullptr);

        api->glActiveTextureFn(GL_TEXTURE0);
        api->glBindTextureFn(GL_TEXTURE_EXTERNAL_OES, y_texture->GetTexturePassthrough()->service_id());
        ASSERT_EQ(api->glGetErrorFn(), static_cast<GLenum>(GL_NO_ERROR));

        api->glActiveTextureFn(GL_TEXTURE1);
        api->glBindTextureFn(GL_TEXTURE_EXTERNAL_OES, uv_texture->GetTexturePassthrough()->service_id());
        ASSERT_EQ(api->glGetErrorFn(), static_cast<GLenum>(GL_NO_ERROR));
...
void D3DImageBackingFactoryTest::RunOverlayTest(bool use_shared_handle, bool use_factory)
{
    ...
    auto overlay_representation = shared_image_representation_factory_->ProduceOverlay(shared_image_refs[0]->mailbox());

    auto scoped_read_access = overlay_representation->BeginScopedReadAccess(/*needs_gl_image=*/true);
    ASSERT_TRUE(scoped_read_access);

    auto* gl_image_d3d = gl::GLImageD3D::FromGLImage(scoped_read_access->gl_image());
    ...
    device_context->CopyResource(staging_texture.Get(), gl_image_d3d->texture().Get());
    D3D11_MAPPED_SUBRESOURCE mapped_resource = {};
    hr = device_context->Map(staging_texture.Get(), 0, D3D11_MAP_READ, 0, &mapped_resource);
    ASSERT_EQ(hr, S_OK);

    CheckYUV(static_cast<const uint8_t*>(mapped_resource.pData), mapped_resource.RowPitch, size, kYFillValue, kUFillValue, kVFillValue);
...
TEST_F(D3DImageBackingFactoryTest, CreateFromSharedMemory)
{
    ...
    base::UnsafeSharedMemoryRegion shm_region = base::UnsafeSharedMemoryRegion::Create(kDataSize);
    {
    ...
    const gpu::Mailbox mailboxes[kNumPlanes] = { gpu::Mailbox::GenerateForSharedImage(), gpu::Mailbox::GenerateForSharedImage() };
    const gfx::BufferPlane planes[kNumPlanes] = { gfx::BufferPlane::Y, gfx::BufferPlane::UV };
    constexpr uint32_t usage = gpu::SHARED_IMAGE_USAGE_VIDEO_DECODE | gpu::SHARED_IMAGE_USAGE_GLES2 | gpu::SHARED_IMAGE_USAGE_RASTER
        | gpu::SHARED_IMAGE_USAGE_DISPLAY_READ | gpu::SHARED_IMAGE_USAGE_SCANOUT;
    std::vector<std::unique_ptr<SharedImageBacking>> shared_image_backings;
    for (size_t i = 0; i < kNumPlanes; i++) {
        ...
        // memory support.
        auto backing = CompoundImageBacking::CreateSharedMemory(shared_image_factory_.get(), /*allow_shm_overlays=*/true, mailboxes[i],
            std::move(shm_gmb_handle), gfx::BufferFormat::YUV_420_BIPLANAR, planes[i], kNullSurfaceHandle, size, gfx::ColorSpace(), kTopLeft_GrSurfaceOrigin,
    ...
    auto y_access = y_texture->BeginScopedAccess(GL_SHARED_IMAGE_ACCESS_MODE_READWRITE_CHROMIUM, SharedImageRepresentation::AllowUnclearedAccess::kNo);
    ...
    auto uv_access = uv_texture->BeginScopedAccess(GL_SHARED_IMAGE_ACCESS_MODE_READWRITE_CHROMIUM, SharedImageRepresentation::AllowUnclearedAccess::kNo);
    ...
    {
        base::WritableSharedMemoryMapping shm_mapping = shm_region.Map();
        CheckYUV(shm_mapping.GetMemoryAs<uint8_t>(), size.width(), size, kYClearValue, kUClearValue, kVClearValue);
    }
    ...
    // offsets. Accessing via the Y plane overlay allows reading both planes.
    {
        ...
        auto* gl_image_memory = gl::GLImageMemory::FromGLImage(scoped_read_access->gl_image());
        ASSERT_TRUE(gl_image_memory);

        CheckYUV(gl_image_memory->memory(), gl_image_memory->stride(), size, kYClearValue, kUClearValue, kVClearValue);
    }
}
