...
{
    if (usage & (WGPUTextureUsage_CopyDst | WGPUTextureUsage_RenderAttachment | WGPUTextureUsage_StorageBinding | WGPUTextureUsage_Present)) {
...
DawnEGLImageRepresentation::DawnEGLImageRepresentation(std::unique_ptr<GLTextureImageRepresentationBase> gl_representation, SharedImageManager* manager,
    SharedImageBacking* backing, MemoryTypeTracker* tracker, WGPUDevice device)
...
WGPUTexture DawnEGLImageRepresentation::BeginAccess(WGPUTextureUsage usage)
{
    ...
    texture_descriptor.usage = WGPUTextureUsage_CopySrc | WGPUTextureUsage_CopyDst | WGPUTextureUsage_RenderAttachment;
    ...
    if (texture->GetType() == gpu::TextureBase::Type::kPassthrough) {
        image = static_cast<gles2::TexturePassthrough*>(texture)->GetLevelImage(texture->target(), 0u);
    } else {
        image = static_cast<gles2::Texture*>(texture)->GetLevelImage(texture->target(), 0u);
    }
    externalImageDesc.image = image->GetEGLImage();
    DCHECK(externalImageDesc.image);
    externalImageDesc.isInitialized = true;
    texture_ = dawn::native::opengl::WrapExternalEGLImage(device_, &externalImageDesc);
    return texture_;
}
