...
{
    return gl::g_current_gl_version->is_es3_capable || gl::g_current_gl_driver->ext.b_GL_EXT_unpack_subimage;
}
...
GLTextureImageBacking::GLTextureImageBacking(const Mailbox& mailbox, viz::SharedImageFormat format, const gfx::Size& size, const gfx::ColorSpace& color_space,
    GrSurfaceOrigin surface_origin, SkAlphaType alpha_type, uint32_t usage, bool is_passthrough)
    ...
    const std::string& dump_name, base::trace_event::MemoryAllocatorDumpGuid client_guid, base::trace_event::ProcessMemoryDump* pmd, uint64_t client_tracing_id)
{
    ...
    if (!IsPassthrough()) {
        const auto service_guid = gl::GetGLTextureServiceGUIDForTracing(texture_->service_id());
...
bool GLTextureImageBacking::UploadFromMemory(const SkPixmap& pixmap)
{
    ...
    size_t expected_stride = gfx::RowSizeForBufferFormat(size().width(), viz::BufferFormat(format()), /*plane=*/0);
    ...
    if (resource_format == viz::BGRX_8888 || resource_format == viz::RGBX_8888) {
        ...
        // pixel.
        repacked_data = RepackPixelDataAsRgb(size(), pixmap, resource_format == viz::BGRX_8888);
        ...
        if (SupportsUnpackSubimage()) {
            ...
            gl_unpack_row_length = base::checked_cast<int>(pixmap_stride) / BytesPerPixel(format());
        } else {
            // If GL_UNPACK_ROW_LENGTH isn't supported then repack pixels with the
            // expected stride.
            repacked_data = RepackPixelDataWithStride(size(), pixmap, expected_stride);
        }
    }

    gl::ScopedTextureBinder scoped_texture_binder(gl_target, texture_id);
    ScopedUnpackState scoped_unpack_state(/*uploading_data=*/true, gl_unpack_row_length);

    const void* pixels = !repacked_data.empty() ? repacked_data.data() : pixmap.addr();
    gl::GLApi* api = gl::g_current_gl_context;
    api->glTexSubImage2DFn(gl_target, /*level=*/0, 0, 0, size().width(), size().height(), gl_format, gl_type, pixels);
...
{
    DCHECK(texture_);
    return std::make_unique<GLTextureGLCommonRepresentation>(manager, this, nullptr, tracker, texture_);
}
    ...
    SharedImageManager* manager, MemoryTypeTracker* tracker)
{
    ...
    return std::make_unique<GLTexturePassthroughGLCommonRepresentation>(manager, this, nullptr, tracker, passthrough_texture_);
}
    ...
    SharedImageManager* manager, MemoryTypeTracker* tracker, WGPUDevice device, WGPUBackendType backend_type)
{
    ...
    if (backend_type == WGPUBackendType_OpenGLES) {
        ...
        return std::make_unique<DawnEGLImageRepresentation>(std::move(texture), manager, this, tracker, device);
    }
#endif
    ...
    SharedImageManager* manager, MemoryTypeTracker* tracker, scoped_refptr<SharedContextState> context_state)
{
    ...
    return std::make_unique<SkiaGLCommonRepresentation>(manager, this, nullptr, std::move(context_state), cached_promise_texture_, tracker);
}

void GLTextureImageBacking::InitializeGLTexture(GLuint service_id, const InitializeGLTextureParams& params)
{
    GLTextureImageBackingHelper::MakeTextureAndSetParameters(params.target, service_id, params.framebuffer_attachment_angle,
        IsPassthrough() ? &passthrough_texture_ : nullptr, IsPassthrough() ? nullptr : &texture_);
    ...
    if (IsPassthrough()) {
        ...
        texture_->SetLevelInfo(params.target, 0, params.internal_format, size().width(), size().height(), 1, 0, params.format, params.type,
...
void GLTextureImageBacking::CreateEGLImage()
{
    ...
    auto image_np = base::MakeRefCounted<gl::GLImageNativePixmap>(size(), viz::BufferFormat(format()));
    ...
    if (passthrough_texture_) {
        ...
        texture_->SetLevelImage(texture_->target(), 0, image_egl_.get(), gles2::Texture::ImageState::BOUND);
    }
#endif
}

void GLTextureImageBacking::SetCompatibilitySwizzle(const gles2::Texture::CompatibilitySwizzle* swizzle)
{
    if (!IsPassthrough())
