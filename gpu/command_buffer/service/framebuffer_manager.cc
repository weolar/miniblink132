...
class RenderbufferAttachment : public Framebuffer::Attachment {
public:
    explicit RenderbufferAttachment(Renderbuffer* renderbuffer)
    ...
    {
        return TextureManager::ExtractTypeFromStorageFormat(renderbuffer_->internal_format());
    }

    GLsizei samples() const override
    {
        return renderbuffer_->samples();
    }

    GLuint object_name() const override
    {
    ...
    void SetCleared(RenderbufferManager* renderbuffer_manager, TextureManager* /* texture_manager */, bool cleared) override
    {
    ...
    bool IsTextureAttachment() const override
    {
        return false;
    }
    bool IsRenderbufferAttachment() const override
    {
        return true;
    }
    ...
    bool IsSameAttachment(const Attachment* attachment) const override
    {
        if (attachment->IsRenderbufferAttachment()) {
            const RenderbufferAttachment* other = reinterpret_cast<const RenderbufferAttachment*>(attachment);
    ...
    bool CanRenderTo(const FeatureInfo*) const override
    {
        return true;
    }

    void DetachFromFramebuffer(Framebuffer* framebuffer, GLenum attachment) const override
    {
    ...
    bool ValidForAttachmentType(GLenum attachment_type, uint32_t max_color_attachments) override
    {
        uint32_t need = GLES2Util::GetChannelsNeededForAttachmentType(attachment_type, max_color_attachments);
        DCHECK_NE(0u, need);
    ...
    void AddToSignature(TextureManager* texture_manager, std::string* signature) const override
    {
        DCHECK(signature);
        renderbuffer_->AddToSignature(signature);
    }

    bool FormsFeedbackLoop(TextureRef* /* texture */, GLint /* level */, GLint /* layer */) const override
    {
        return false;
    }
...
class TextureAttachment : public Framebuffer::Attachment {
public:
    TextureAttachment(TextureRef* texture_ref, GLenum target, GLint level, GLsizei samples, GLint layer)
    ...
    GLsizei width() const override
    {
        ...
        texture_ref_->texture()->GetLevelSize(target_, level_, &temp_width, &temp_height, nullptr);
        return temp_width;
    }
    ...
    GLenum internal_format() const override
    {
        ...
        texture_ref_->texture()->GetLevelType(target_, level_, &temp_type, &temp_internal_format);
    ...
    void SetCleared(RenderbufferManager* /* renderbuffer_manager */, TextureManager* texture_manager, bool cleared) override
    {
        ...
        texture_manager->SetLevelCleared(texture_ref_.get(), target_, level_, cleared);
    }
    ...
    bool IsSameAttachment(const Attachment* attachment) const override
    {
        if (attachment->IsTextureAttachment()) {
            const TextureAttachment* other = reinterpret_cast<const TextureAttachment*>(attachment);
            return IsTexture(other->texture()) && layer_ == other->layer() && target_ == other->target() && level_ == other->level();
        }
        return false;
    }
    ...
    bool IsLayerValid() const override
    {
        ...
        return (texture->GetLevelSize(target_, level_, &width, &height, &depth) && layer_ < depth);
    }
    ...
    bool ValidForAttachmentType(GLenum attachment_type, uint32_t max_color_attachments) override
    {
        ...
        if (!texture_ref_->texture()->GetLevelType(target_, level_, &type, &internal_format)) {
            return false;
        }
        uint32_t need = GLES2Util::GetChannelsNeededForAttachmentType(attachment_type, max_color_attachments);
        DCHECK_NE(0u, need);
    ...
    bool FormsFeedbackLoop(TextureRef* texture, GLint level, GLint layer) const override
    {
        return texture == texture_ref_.get() && level == level_ && layer == layer_;
    }
...
FramebufferManager::FramebufferManager(
    uint32_t max_draw_buffers, uint32_t max_color_attachments, FramebufferCompletenessCache* framebuffer_combo_complete_cache)
...
void FramebufferManager::StartTracking(Framebuffer* /* framebuffer */)
{
    ++framebuffer_count_;
}

void FramebufferManager::StopTracking(Framebuffer* /* framebuffer */)
{
    --framebuffer_count_;
}

void FramebufferManager::CreateFramebuffer(GLuint client_id, GLuint service_id)
{
    std::pair<FramebufferMap::iterator, bool> result
        = framebuffers_.insert(std::make_pair(client_id, scoped_refptr<Framebuffer>(new Framebuffer(this, service_id))));
...
bool Framebuffer::HasUnclearedAttachment(GLenum attachment_type) const
{
...
bool Framebuffer::HasDepthStencilFormatAttachment() const
{
    ...
    if (depth_attachment && stencil_attachment) {
        ...
        stencil_format = TextureManager::ExtractFormatFromStorageFormat(stencil_format);
        return depth_format == GL_DEPTH_STENCIL && stencil_format == GL_DEPTH_STENCIL;
    }
    return false;
}
...
{
    for (AttachmentMap::const_iterator it = attachments_.begin(); it != attachments_.end(); ++it) {
...
void Framebuffer::ClearUnclearedIntRenderbufferAttachments(RenderbufferManager* renderbuffer_manager)
{
    for (AttachmentMap::const_iterator it = attachments_.begin(); it != attachments_.end(); ++it) {
        if (!it->second->IsRenderbufferAttachment() || it->second->cleared())
            continue;
        ...
        if (GLES2Util::IsIntegerFormat(internal_format)) {
            ...
            DCHECK_GT(GL_COLOR_ATTACHMENT0 + manager_->max_draw_buffers_, attaching_point);
...
bool Framebuffer::PrepareDrawBuffersForClearingUninitializedAttachments() const
{
    base::HeapArray<GLenum> buffers = base::HeapArray<GLenum>::Uninit(manager_->max_draw_buffers_);
    for (uint32_t i = 0; i < manager_->max_draw_buffers_; ++i)
        buffers[i] = GL_NONE;
    for (auto const& it : attachments_) {
        if (it.first >= GL_COLOR_ATTACHMENT0 && it.first < GL_COLOR_ATTACHMENT0 + manager_->max_draw_buffers_ && !it.second->cleared()) {
...
bool Framebuffer::ValidateAndAdjustDrawBuffers(uint32_t fragment_output_type_mask, uint32_t fragment_output_written_mask)
{
    uint32_t mask = draw_buffer_bound_mask_ & fragment_output_written_mask;
    if (mask != draw_buffer_bound_mask_ || (mask & fragment_output_type_mask) != (mask & draw_buffer_type_mask_))
        return false;
...
void Framebuffer::ClearUnclearedIntOr3DTexturesOrPartiallyClearedTextures(GLES2Decoder* decoder, TextureManager* texture_manager)
{
    for (AttachmentMap::const_iterator it = attachments_.begin(); it != attachments_.end(); ++it) {
        if (!it->second->IsTextureAttachment() || it->second->cleared())
            continue;
        TextureAttachment* attachment = reinterpret_cast<TextureAttachment*>(it->second.get());
        if (attachment->IsPartiallyCleared() || attachment->Is3D() || GLES2Util::IsIntegerFormat(attachment->internal_format())) {
            texture_manager->ClearTextureLevel(decoder, attachment->texture(), attachment->target(), attachment->level());
        }
    }
}
...
void Framebuffer::MarkAttachmentAsCleared(RenderbufferManager* renderbuffer_manager, TextureManager* texture_manager, GLenum attachment, bool cleared)
{
    AttachmentMap::iterator it = attachments_.find(attachment);
    if (it != attachments_.end()) {
        Attachment* a = it->second.get();
        if (a->cleared() != cleared) {
            a->SetCleared(renderbuffer_manager, texture_manager, cleared);
        }
    }
}

void Framebuffer::MarkAttachmentsAsCleared(RenderbufferManager* renderbuffer_manager, TextureManager* texture_manager, bool cleared)
{
    for (AttachmentMap::iterator it = attachments_.begin(); it != attachments_.end(); ++it) {
...
{
    const Attachment* attachment = GetReadBufferAttachment();
    return attachment ? attachment->IsTextureAttachment() && attachment->samples() > 0 : false;
}
...
GLenum Framebuffer::IsPossiblyComplete(const FeatureInfo* feature_info) const
{
    ...
    for (AttachmentMap::const_iterator it = attachments_.begin(); it != attachments_.end(); ++it) {
        ...
        Attachment* attachment = it->second.get();
        if (!attachment->ValidForAttachmentType(attachment_type, manager_->max_color_attachments_)) {
        ...
        if (it->first >= GL_COLOR_ATTACHMENT0 && it->first < GL_COLOR_ATTACHMENT0 + manager_->max_color_attachments_) {
            ...
            if (feature_info->context_type() == CONTEXT_TYPE_WEBGL1) {
                if (colorbufferSizeValid) {
                    if (colorbufferSize != GLES2Util::GetGLTypeSizeForTextures(attachment->texture_type())) {
                        return GL_FRAMEBUFFER_UNSUPPORTED;
                    }
                } else {
                    colorbufferSize = GLES2Util::GetGLTypeSizeForTextures(attachment->texture_type());
            ...
            for (GLenum i = it->first + 1; i < GL_COLOR_ATTACHMENT0 + manager_->max_color_attachments_; i++) {
    ...
    if (depth_attachment && stencil_attachment) {
        ...
        DCHECK_EQ(depth_attachment->internal_format(), stencil_attachment->internal_format());
    }
    if (feature_info->context_type() == CONTEXT_TYPE_WEBGL1) {
        ...
        if (depth_attachment) {
            ...
            have_channels |= GLES2Util::GetChannelsForFormat(depth_attachment->internal_format());
        }
        if (stencil_attachment) {
            need_channels |= GLES2Util::kStencil;
            have_channels |= GLES2Util::GetChannelsForFormat(stencil_attachment->internal_format());
        }
...
GLenum Framebuffer::GetStatus(TextureManager* texture_manager, GLenum target) const
{
    ...
    for (AttachmentMap::const_iterator it = attachments_.begin(); it != attachments_.end(); ++it) {
        Attachment* attachment = it->second.get();
        signature.append(reinterpret_cast<const char*>(&it->first), sizeof(it->first));
...
{
    GLsizei index = static_cast<GLsizei>(draw_buffer - GL_DRAW_BUFFER0_ARB);
...
bool Framebuffer::HasAlphaMRT() const
{
    for (uint32_t i = 0; i < manager_->max_draw_buffers_; ++i) {
        if (draw_buffers_[i] != GL_NONE) {
            ...
            if (!attachment)
                continue;
            if ((GLES2Util::GetChannelsForFormat(attachment->internal_format()) & GLES2Util::kAlpha) != 0)
                return true;
        }
    }
    return false;
...
void Framebuffer::UnbindRenderbuffer(GLenum target, Renderbuffer* renderbuffer)
{
    bool done;
    do {
        done = true;
...
void Framebuffer::UnbindTexture(GLenum target, TextureRef* texture_ref)
{
    bool done;
    do {
        done = true;
...
Framebuffer* FramebufferManager::GetFramebuffer(GLuint client_id)
{
...
{
    if (attachment >= GL_COLOR_ATTACHMENT0 && attachment < GL_COLOR_ATTACHMENT0 + manager_->max_color_attachments_) {
        last_color_attachment_id_ = std::max(last_color_attachment_id_, static_cast<GLsizei>(attachment - GL_COLOR_ATTACHMENT0));
    }
}
...
{
    if (attachment >= GL_COLOR_ATTACHMENT0 && attachment < GL_COLOR_ATTACHMENT0 + manager_->max_color_attachments_
        && static_cast<GLsizei>(attachment - GL_COLOR_ATTACHMENT0) == last_color_attachment_id_) {
        for (last_color_attachment_id_--; last_color_attachment_id_ >= 0; last_color_attachment_id_--) {
            if (base::Contains(attachments_, GL_COLOR_ATTACHMENT0 + last_color_attachment_id_)) {
                break;
            }
        }
    }
...
void Framebuffer::AttachRenderbuffer(GLenum attachment, Renderbuffer* renderbuffer)
{
    ...
    if (renderbuffer) {
        attachments_[attachment] = scoped_refptr<Attachment>(new RenderbufferAttachment(renderbuffer));
...
void Framebuffer::AttachTexture(GLenum attachment, TextureRef* texture_ref, GLenum target, GLint level, GLsizei samples)
{
    DCHECK_NE(static_cast<GLenum>(GL_DEPTH_STENCIL_ATTACHMENT), attachment);
    const Attachment* a = GetAttachment(attachment);
    if (a)
        a->DetachFromFramebuffer(this, attachment);
    if (texture_ref) {
        attachments_[attachment] = scoped_refptr<Attachment>(new TextureAttachment(texture_ref, target, level, samples, 0));
...
void Framebuffer::AttachTextureLayer(GLenum attachment, TextureRef* texture_ref, GLenum target, GLint level, GLint layer)
{
    DCHECK_NE(static_cast<GLenum>(GL_DEPTH_STENCIL_ATTACHMENT), attachment);
    const Attachment* a = GetAttachment(attachment);
    if (a)
        a->DetachFromFramebuffer(this, attachment);
    if (texture_ref) {
        attachments_[attachment] = scoped_refptr<Attachment>(new TextureAttachment(texture_ref, target, level, 0, layer));
        texture_ref->texture()->AttachToFramebuffer();
        OnInsertUpdateLastColorAttachmentId(attachment);
    } else {
        attachments_.erase(attachment);
...
const Framebuffer::Attachment* Framebuffer::GetAttachment(GLenum attachment) const
{
...
bool FramebufferManager::GetClientId(GLuint service_id, GLuint* client_id) const
{
    // This doesn't need to be fast. It's only used during slow queries.
    for (FramebufferMap::const_iterator it = framebuffers_.begin(); it != framebuffers_.end(); ++it) {
...
void FramebufferManager::MarkAttachmentsAsCleared(Framebuffer* framebuffer, RenderbufferManager* renderbuffer_manager, TextureManager* texture_manager)
{
    DCHECK(framebuffer);
    framebuffer->MarkAttachmentsAsCleared(renderbuffer_manager, texture_manager, true);
    MarkAsComplete(framebuffer);
}

void FramebufferManager::MarkAsComplete(Framebuffer* framebuffer)
{
    DCHECK(framebuffer);
