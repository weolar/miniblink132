...
class GPU_EXPORT CommandBufferDirect : public CommandBuffer, public CommandBufferServiceClient, public DecoderClient {
public:
    CommandBufferDirect();
    ~CommandBufferDirect() override;

    void set_handler(AsyncAPIInterface* handler)
    {
        handler_ = handler;
    }
    CommandBufferService* service()
    {
        return &service_;
    }
