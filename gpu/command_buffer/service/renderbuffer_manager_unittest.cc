...
protected:
    void SetUpBase(MemoryTracker* memory_tracker, bool depth24_supported)
    {
        ...
        TestHelper::SetupFeatureInfoInitExpectationsWithGLVersion(
            gl_.get(), depth24_supported ? "GL_OES_depth24" : "", "", "OpenGL ES 3.0", feature_info_->context_type());
        feature_info_->InitializeForTesting();
        manager_ = std::make_unique<RenderbufferManager>(memory_tracker, kMaxSize, kMaxSamples, feature_info_.get());
    }
...
class RenderbufferManagerMemoryTrackerTest : public RenderbufferManagerTestBase {
protected:
    void SetUp() override
    {
        bool depth24_supported = false;
...
#define EXPECT_MEMORY_ALLOCATION_CHANGE(old_size, new_size)                                                                                                    \
    EXPECT_CALL(mock_memory_tracker_, TrackMemoryAllocatedChange(static_cast<uint64_t>(new_size) - static_cast<uint64_t>(old_size)))                           \
        .Times(1)                                                                                                                                              \
        .RetiresOnSaturation()
...
TEST_F(RenderbufferManagerTest, Basic)
{
    ...
    scoped_refptr<Renderbuffer> renderbuffer1 = manager_->GetRenderbuffer(kClient1Id);
    ...
    EXPECT_CALL(*gl_, DeleteRenderbuffersEXT(1, ::testing::Pointee(kService1Id))).Times(1).RetiresOnSaturation();
...
TEST_F(RenderbufferManagerTest, Destroy)
{
    ...
    manager_->CreateRenderbuffer(kClient1Id, kService1Id);
    // Check renderbuffer got created.
    Renderbuffer* renderbuffer1 = manager_->GetRenderbuffer(kClient1Id);
...
TEST_F(RenderbufferManagerTest, Renderbuffer)
{
    ...
    manager_->SetInfoAndInvalidate(renderbuffer1, kSamples, kFormat, kWidth, kHeight);
    ...
    EXPECT_EQ(kWidth * kHeight * kSamples * kNumOfBytesPerPixel, renderbuffer1->EstimatedSize());
...
TEST_F(RenderbufferManagerMemoryTrackerTest, Basic)
{
    ...
    manager_->ComputeEstimatedRenderbufferSize(kWidth, kHeight1, kSamples, kFormat, &expected_size_1);
    manager_->ComputeEstimatedRenderbufferSize(kWidth, kHeight2, kSamples, kFormat, &expected_size_2);
    EXPECT_MEMORY_ALLOCATION_CHANGE(0, expected_size_1);
    manager_->SetInfoAndInvalidate(renderbuffer1, kSamples, kFormat, kWidth, kHeight1);
    EXPECT_MEMORY_ALLOCATION_CHANGE(expected_size_1, 0);
    EXPECT_MEMORY_ALLOCATION_CHANGE(0, expected_size_2);
    manager_->SetInfoAndInvalidate(renderbuffer1, kSamples, kFormat, kWidth, kHeight2);
...
TEST_F(RenderbufferManagerTest, UseDeletedRenderbufferInfo)
{
    ...
    const GLsizei kWidth = 128;
    const GLsizei kHeight = 64;
    manager_->SetInfoAndInvalidate(renderbuffer1.get(), kSamples, kFormat, kWidth, kHeight);
...
TEST_F(RenderbufferManagerTest, AddToSignature)
{
    ...
    manager_->SetInfoAndInvalidate(renderbuffer1.get(), kSamples + 1, kFormat, kWidth, kHeight);
    renderbuffer1->AddToSignature(&signature2);
    EXPECT_FALSE(InSet(&string_set, signature2));

    manager_->SetInfoAndInvalidate(renderbuffer1.get(), kSamples, kFormat + 1, kWidth, kHeight);
    signature2.clear();
    renderbuffer1->AddToSignature(&signature2);
    EXPECT_FALSE(InSet(&string_set, signature2));

    manager_->SetInfoAndInvalidate(renderbuffer1.get(), kSamples, kFormat, kWidth + 1, kHeight);
    signature2.clear();
    renderbuffer1->AddToSignature(&signature2);
    EXPECT_FALSE(InSet(&string_set, signature2));

    manager_->SetInfoAndInvalidate(renderbuffer1.get(), kSamples, kFormat, kWidth, kHeight + 1);
    signature2.clear();
    renderbuffer1->AddToSignature(&signature2);
    EXPECT_FALSE(InSet(&string_set, signature2));
...
{
    GLenum impl_format = manager_->InternalRenderbufferFormatToImplFormat(GL_DEPTH_COMPONENT16);
