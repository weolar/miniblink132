...
using Param = std::tuple<MultiDrawManager::IndexStorageType, MultiDrawManager::DrawFunction>;
...
protected:
    bool DoMultiDraw(uint32_t count, GLenum mode = GL_TRIANGLES, GLenum type = GL_UNSIGNED_INT)
    {
        ...
        case MultiDrawManager::DrawFunction::DrawArrays:
            return multi_draw_manager_->MultiDrawArrays(mode, data.data(), data.data(), count);

        case MultiDrawManager::DrawFunction::DrawArraysInstanced:
            return multi_draw_manager_->MultiDrawArraysInstanced(mode, data.data(), data.data(), data.data(), count);

        case MultiDrawManager::DrawFunction::DrawArraysInstancedBaseInstance:
            return multi_draw_manager_->MultiDrawArraysInstancedBaseInstance(mode, data.data(), data.data(), data.data(), baseinstances.data(), count);

        case MultiDrawManager::DrawFunction::DrawElements:
            return multi_draw_manager_->MultiDrawElements(mode, data.data(), type, data.data(), count);

        case MultiDrawManager::DrawFunction::DrawElementsInstanced:
            return multi_draw_manager_->MultiDrawElementsInstanced(mode, data.data(), type, data.data(), data.data(), count);

        case MultiDrawManager::DrawFunction::DrawElementsInstancedBaseVertexBaseInstance:
            return multi_draw_manager_->MultiDrawElementsInstancedBaseVertexBaseInstance(
                mode, data.data(), type, data.data(), data.data(), basevertices.data(), baseinstances.data(), count);
        }
    }

    void CheckResultSize(uint32_t count, const MultiDrawManager::ResultData& result)
    {
...
{
    MultiDrawManager::DrawFunction draw_function = std::get<1>(GetParam());
    if (draw_function != MultiDrawManager::DrawFunction::DrawElements && draw_function != MultiDrawManager::DrawFunction::DrawElementsInstanced) {
        return;
    }
...
INSTANTIATE_TEST_SUITE_P(All, MultiDrawManagerTest,
    testing::Combine(testing::Values(MultiDrawManager::IndexStorageType::Offset, MultiDrawManager::IndexStorageType::Pointer),
        testing::Values(MultiDrawManager::DrawFunction::DrawArrays, MultiDrawManager::DrawFunction::DrawArraysInstanced,
            MultiDrawManager::DrawFunction::DrawArraysInstancedBaseInstance, MultiDrawManager::DrawFunction::DrawElements,
            MultiDrawManager::DrawFunction::DrawElementsInstanced, MultiDrawManager::DrawFunction::DrawElementsInstancedBaseVertexBaseInstance)));
