...
GLenum Sampler::SetParameteri(const FeatureInfo* feature_info, GLenum pname, GLint param)
{
    DCHECK(feature_info);
...
GLenum Sampler::SetParameterf(const FeatureInfo* feature_info, GLenum pname, GLfloat param)
{
    switch (pname) {
    case GL_TEXTURE_MIN_FILTER:
    case GL_TEXTURE_MAG_FILTER:
...
Sampler* SamplerManager::CreateSampler(GLuint client_id, GLuint service_id)
{
    ...
    auto result = samplers_.insert(std::make_pair(client_id, scoped_refptr<Sampler>(new Sampler(this, client_id, service_id))));
...
void SamplerManager::SetParameteri(const char* function_name, ErrorState* error_state, Sampler* sampler, GLenum pname, GLint param)
{
    DCHECK(error_state);
    DCHECK(sampler);
...
void SamplerManager::SetParameterf(const char* function_name, ErrorState* error_state, Sampler* sampler, GLenum pname, GLfloat param)
{
    DCHECK(error_state);
    DCHECK(sampler);
