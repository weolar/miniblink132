...
void Shader::RequestCompile(scoped_refptr<ShaderTranslatorInterface> translator, TranslatedShaderSourceType type)
{
    shader_state_ = kShaderStateCompileRequested;
    translator_ = translator;
    if (translator_) {
        options_affecting_compilation_ = translator_->GetStringForOptionsThatWouldAffectCompilation();
    }
    source_type_ = type;
...
void Shader::DoCompile()
{
    ...
    if (translator) {
        bool success = translator->Translate(last_compiled_source_, &log_info_, &translated_source_, &shader_version_, &attrib_map_, &uniform_map_,
            &varying_map_, &interface_block_map_, &output_variable_list_);
        if (!success) {
            return;
        }
    ...
    if (status == GL_TRUE) {
        ...
        LOG_IF(ERROR, translator) << "Shader translator allowed/produced an invalid shader "
...
void Shader::RefreshTranslatedShaderSource()
{
    if (source_type_ == kANGLE) {
        GLint max_len = 0;
        glGetShaderiv(service_id_, GL_TRANSLATED_SHADER_SOURCE_LENGTH_ANGLE, &max_len);
        translated_source_.resize(max_len);
        if (max_len) {
            GLint len = 0;
            glGetTranslatedShaderSourceANGLE(service_id_, translated_source_.size(), &len, &translated_source_.at(0));
            DCHECK(max_len == 0 || len < max_len);
...
const std::string* Shader::GetAttribMappedName(const std::string& original_name) const
{
...
const std::string* Shader::GetUniformMappedName(const std::string& original_name) const
{
...
const std::string* Shader::GetVaryingMappedName(const std::string& original_name) const
{
    for (VaryingMap::const_iterator it = varying_map_.begin(); it != varying_map_.end(); ++it) {
...
const std::string* Shader::GetInterfaceBlockMappedName(const std::string& original_name) const
{
...
const std::string* Shader::GetOutputVariableMappedName(const std::string& original_name) const
{
...
const std::string* Shader::GetOriginalNameFromHashedName(const std::string& hashed_name) const
{
...
const sh::InterfaceBlock* Shader::GetInterfaceBlockInfo(const std::string& name) const
{
    InterfaceBlockMap::const_iterator it = interface_block_map_.find(GetTopVariableName(name));
    return it != interface_block_map_.end() ? &it->second : nullptr;
}

const sh::OutputVariable* Shader::GetOutputVariableInfo(const std::string& name) const
{
...
Shader* ShaderManager::CreateShader(GLuint client_id, GLuint service_id, GLenum shader_type)
{
    std::pair<ShaderMap::iterator, bool> result = shaders_.insert(std::make_pair(client_id, scoped_refptr<Shader>(new Shader(service_id, shader_type))));
...
bool ShaderManager::GetClientId(GLuint service_id, GLuint* client_id) const
{
    ...
    for (ShaderMap::const_iterator it = shaders_.begin(); it != shaders_.end(); ++it) {
