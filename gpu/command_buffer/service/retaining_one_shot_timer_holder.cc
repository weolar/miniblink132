...
RetainingOneShotTimerHolder::RetainingOneShotTimerHolder(
    base::TimeDelta max_delay, base::TimeDelta min_delay, scoped_refptr<base::SingleThreadTaskRunner> task_runner, base::RepeatingClosure user_task)
    : min_delay_(min_delay)
    , task_runner_(std::move(task_runner))
    , user_task_(std::move(user_task))
    , timer_(std::make_unique<base::RetainingOneShotTimer>(
          FROM_HERE, max_delay, base::BindRepeating(&RetainingOneShotTimerHolder::OnTimerFired, base::RetainedRef(this))))
{
}
...
void RetainingOneShotTimerHolder::ResetTimerIfNecessary()
{
    if (!task_runner_->BelongsToCurrentThread()) {
        task_runner_->PostTask(FROM_HERE, base::BindOnce(&RetainingOneShotTimerHolder::ResetTimerIfNecessary, base::RetainedRef(this)));
        return;
    }

    if (timer_ && (!timer_->IsRunning() || timer_->desired_run_time() - base::TimeTicks::Now() < min_delay_)) {
        timer_->Reset();
    }
}
...
void RetainingOneShotTimerHolder::DestroyTimer()
{
    ...
    task_runner_->PostTask(FROM_HERE, base::BindOnce(&RetainingOneShotTimerHolder::DestroyTimerOnTaskRunner, base::RetainedRef(this)));
}
