...
public:
    ...
    public:
        ...
        template <typename T> T GetDataAs(size_t offset, size_t size) const
        {
        ...
        bool GetAsStrings(GLsizei* _count, std::vector<char*>* _string, std::vector<GLint>* _length);
    ...
    explicit CommonDecoder(DecoderClient* client, CommandBufferServiceBase* command_buffer_service);
    ...
    void* GetAddressAndCheckSize(unsigned int shm_id, unsigned int offset, unsigned int size);

    // Typed version of GetAddressAndCheckSize.
    template <typename T> T GetSharedMemoryAs(unsigned int shm_id, unsigned int offset, unsigned int size)
    {
        return static_cast<T>(GetAddressAndCheckSize(shm_id, offset, size));
    }

    void* GetAddressAndSize(unsigned int shm_id, unsigned int offset, unsigned int minimum_size, unsigned int* size);

    template <typename T> T GetSharedMemoryAndSizeAs(unsigned int shm_id, unsigned int offset, unsigned int minimum_size, unsigned int* size)
    {
        return static_cast<T>(GetAddressAndSize(shm_id, offset, minimum_size, size));
    }
...
protected:
    ...
    error::Error DoCommonCommand(unsigned int command, unsigned int arg_count, const volatile void* cmd_data);
    ...
    bool ReadColorSpace(uint32_t shm_id, uint32_t shm_offset, uint32_t color_space_size, gfx::ColorSpace* color_space);

private:
    // Generate a member function prototype for each command in an automated and
    // typesafe way.
#define COMMON_COMMAND_BUFFER_CMD_OP(name) error::Error Handle##name(uint32_t immediate_data_size, const volatile void* data);
...
#undef COMMON_COMMAND_BUFFER_CMD_OP
    ...
    using CmdHandler = Error (CommonDecoder::*)(uint32_t immediate_data_size, const volatile void* data);
