...
struct RenderbufferSignature {
    ...
    RenderbufferSignature(GLenum internal_format, GLsizei samples, GLsizei width, GLsizei height)
    {
...
RenderbufferManager::RenderbufferManager(MemoryTracker* memory_tracker, GLint max_renderbuffer_size, GLint max_samples, FeatureInfo* feature_info)
    ...
    , have_context_(true)
{
    ...
    if (memory_tracker_) {
        base::trace_event::MemoryDumpManager::GetInstance()->RegisterDumpProvider(
            this, "gpu::RenderbufferManager", base::SingleThreadTaskRunner::GetCurrentDefault());
    }
}
...
{
    uint32_t size = 0;
    manager_->ComputeEstimatedRenderbufferSize(width_, height_, samples_, internal_format_, &size);
    return size;
}
...
void Renderbuffer::SetInfoAndInvalidate(GLsizei samples, GLenum internalformat, GLsizei width, GLsizei height)
{
    samples_ = samples;
...
{
    DCHECK(signature);
    RenderbufferSignature signature_data(internal_format_, samples_, width_, height_);
...
Renderbuffer::Renderbuffer(RenderbufferManager* manager, GLuint client_id, GLuint service_id)
    : manager_(manager)
...
bool Renderbuffer::RegenerateAndBindBackingObjectIfNeeded(const GpuDriverBugWorkarounds& workarounds)
{
    bool multisample_workaround = workarounds.multisample_renderbuffer_resize_emulation;
    ...
    for (auto& point : framebuffer_attachment_points_) {
        ...
        glFramebufferRenderbufferEXT(GL_DRAW_FRAMEBUFFER, point.second, GL_RENDERBUFFER, service_id_);
    }
...
void Renderbuffer::AddFramebufferAttachmentPoint(Framebuffer* framebuffer, GLenum attachment)
{
    DCHECK_NE(static_cast<GLenum>(GL_DEPTH_STENCIL_ATTACHMENT), attachment);
    framebuffer_attachment_points_.insert(std::make_pair(framebuffer, attachment));
}

void Renderbuffer::RemoveFramebufferAttachmentPoint(Framebuffer* framebuffer, GLenum attachment)
{
    DCHECK_NE(static_cast<GLenum>(GL_DEPTH_STENCIL_ATTACHMENT), attachment);
...
void RenderbufferManager::SetInfoAndInvalidate(Renderbuffer* renderbuffer, GLsizei samples, GLenum internalformat, GLsizei width, GLsizei height)
{
    DCHECK(renderbuffer);
    if (!renderbuffer->cleared()) {
        --num_uncleared_renderbuffers_;
...
void RenderbufferManager::SetCleared(Renderbuffer* renderbuffer, bool cleared)
{
    DCHECK(renderbuffer);
    if (!renderbuffer->cleared()) {
        --num_uncleared_renderbuffers_;
...
void RenderbufferManager::CreateRenderbuffer(GLuint client_id, GLuint service_id)
{
    scoped_refptr<Renderbuffer> renderbuffer(new Renderbuffer(this, client_id, service_id));
...
Renderbuffer* RenderbufferManager::GetRenderbuffer(GLuint client_id)
{
...
bool RenderbufferManager::ComputeEstimatedRenderbufferSize(int width, int height, int samples, int internal_format, uint32_t* size) const
{
    DCHECK(size);
...
GLenum RenderbufferManager::InternalRenderbufferFormatToImplFormat(GLenum impl_format) const
{
...
bool RenderbufferManager::OnMemoryDump(const base::trace_event::MemoryDumpArgs& args, base::trace_event::ProcessMemoryDump* pmd)
{
    using base::trace_event::MemoryAllocatorDump;
    using base::trace_event::MemoryDumpLevelOfDetail;
    const uint64_t context_group_tracing_id = memory_tracker_->ContextGroupTracingId();

    if (args.level_of_detail == MemoryDumpLevelOfDetail::kBackground) {
        std::string dump_name = base::StringPrintf("gpu/gl/renderbuffers/context_group_0x%" PRIX64, context_group_tracing_id);
    ...
    for (const auto& renderbuffer_entry : renderbuffers_) {
        ...
        std::string dump_name
            = base::StringPrintf("gpu/gl/renderbuffers/context_group_0x%" PRIX64 "/renderbuffer_0x%" PRIX32, context_group_tracing_id, client_renderbuffer_id);
        MemoryAllocatorDump* dump = pmd->CreateAllocatorDump(dump_name);
        dump->AddScalar(MemoryAllocatorDump::kNameSize, MemoryAllocatorDump::kUnitsBytes, static_cast<uint64_t>(renderbuffer->EstimatedSize()));

        auto guid = gl::GetGLRenderbufferGUIDForTracing(context_group_tracing_id, client_renderbuffer_id);
