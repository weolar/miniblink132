...
bool Validators::BackbufferAttachmentValidator::IsValid(const GLenum value) const
{
    switch (value) {
    case GL_COLOR_EXT:
    case GL_DEPTH_EXT:
...
bool Validators::IndexedBufferTargetValidator::IsValid(const GLenum value) const
{
    switch (value) {
    case GL_TRANSFORM_FEEDBACK_BUFFER:
    case GL_UNIFORM_BUFFER:
...
bool Validators::InternalFormatParameterValidator::IsValid(const GLenum value) const
{
    switch (value) {
...
bool Validators::PixelStoreAlignmentValidator::IsValid(const GLint value) const
{
    switch (value) {
    case 1:
    case 2:
...
bool Validators::QueryObjectParameterValidator::IsValid(const GLenum value) const
{
    switch (value) {
...
bool Validators::SharedImageAccessModeValidator::IsValid(const GLenum value) const
{
    switch (value) {
...
bool Validators::SwapBuffersFlagsValidator::IsValid(const GLbitfield value) const
{
    switch (value) {
    case 0:
...
bool Validators::TextureCompareFuncValidator::IsValid(const GLenum value) const
{
    switch (value) {
    case GL_LEQUAL:
    case GL_GEQUAL:
...
bool Validators::TextureMagFilterModeValidator::IsValid(const GLenum value) const
{
    switch (value) {
    case GL_NEAREST:
    case GL_LINEAR:
...
bool Validators::TextureMinFilterModeValidator::IsValid(const GLenum value) const
{
    switch (value) {
    case GL_NEAREST:
    case GL_LINEAR:
...
static const GLenum valid_texture_sized_color_renderable_internal_format_table[] = {
    GL_R8,
    GL_R8UI,
    GL_R8I,
    GL_R16UI,
...
static const GLenum valid_texture_sized_texture_filterable_internal_format_table[] = {
    GL_R8,
    GL_R8_SNORM,
    GL_R16F,
    GL_RG8,
...
bool Validators::TextureSrgbDecodeExtValidator::IsValid(const GLenum value) const
{
    switch (value) {
    case GL_DECODE_EXT:
...
static const GLenum valid_texture_stencil_renderable_internal_format_table_es3[] = {
    GL_STENCIL_INDEX8,
    GL_DEPTH24_STENCIL8,
    GL_DEPTH32F_STENCIL8,
};
...
bool Validators::TransformFeedbackPrimitiveModeValidator::IsValid(const GLenum value) const
{
    switch (value) {
    case GL_POINTS:
    case GL_LINES:
...
bool Validators::UniformBlockParameterValidator::IsValid(const GLenum value) const
{
    switch (value) {
...
bool Validators::WindowRectanglesModeValidator::IsValid(const GLenum value) const
{
    switch (value) {
...
Validators::Validators()
    ...
    , attachment_query(valid_attachment_query_table, std::size(valid_attachment_query_table))
    ...
    , dst_blend_factor(valid_dst_blend_factor_table, std::size(valid_dst_blend_factor_table))
    , equation(valid_equation_table, std::size(valid_equation_table))
    , framebuffer_attachment_parameter(valid_framebuffer_attachment_parameter_table, std::size(valid_framebuffer_attachment_parameter_table))
    , framebuffer_parameter()
    , framebuffer_target(valid_framebuffer_target_table, std::size(valid_framebuffer_target_table))
    , g_l_state(valid_g_l_state_table, std::size(valid_g_l_state_table))
    , get_tex_param_target(valid_get_tex_param_target_table, std::size(valid_get_tex_param_target_table))
    , hint_target(valid_hint_target_table, std::size(valid_hint_target_table))
    , image_internal_format(valid_image_internal_format_table, std::size(valid_image_internal_format_table))
    , index_type(valid_index_type_table, std::size(valid_index_type_table))
    , indexed_g_l_state(valid_indexed_g_l_state_table, std::size(valid_indexed_g_l_state_table))
    ...
    , program_parameter(valid_program_parameter_table, std::size(valid_program_parameter_table))
    , read_buffer(valid_read_buffer_table, std::size(valid_read_buffer_table))
    , read_pixel_format(valid_read_pixel_format_table, std::size(valid_read_pixel_format_table))
    , read_pixel_type(valid_read_pixel_type_table, std::size(valid_read_pixel_type_table))
    , render_buffer_format(valid_render_buffer_format_table, std::size(valid_render_buffer_format_table))
    , render_buffer_parameter(valid_render_buffer_parameter_table, std::size(valid_render_buffer_parameter_table))
    , render_buffer_target(valid_render_buffer_target_table, std::size(valid_render_buffer_target_table))
    , sampler_parameter(valid_sampler_parameter_table, std::size(valid_sampler_parameter_table))
    , shader_binary_format()
    , shader_parameter(valid_shader_parameter_table, std::size(valid_shader_parameter_table))
    , src_blend_factor(valid_src_blend_factor_table, std::size(valid_src_blend_factor_table))
    , sync_flush_flags(valid_sync_flush_flags_table, std::size(valid_sync_flush_flags_table))
    , texture_bind_target(valid_texture_bind_target_table, std::size(valid_texture_bind_target_table))
    , texture_compare_mode(valid_texture_compare_mode_table, std::size(valid_texture_compare_mode_table))
    , texture_depth_renderable_internal_format()
    , texture_fbo_target(valid_texture_fbo_target_table, std::size(valid_texture_fbo_target_table))
    , texture_format(valid_texture_format_table, std::size(valid_texture_format_table))
    , texture_internal_format(valid_texture_internal_format_table, std::size(valid_texture_internal_format_table))
    , texture_internal_format_storage(valid_texture_internal_format_storage_table, std::size(valid_texture_internal_format_storage_table))
    , texture_parameter(valid_texture_parameter_table, std::size(valid_texture_parameter_table))
    , texture_sized_color_renderable_internal_format(
          valid_texture_sized_color_renderable_internal_format_table, std::size(valid_texture_sized_color_renderable_internal_format_table))
    , texture_sized_texture_filterable_internal_format(
          valid_texture_sized_texture_filterable_internal_format_table, std::size(valid_texture_sized_texture_filterable_internal_format_table))
    , texture_stencil_renderable_internal_format()
    , texture_target(valid_texture_target_table, std::size(valid_texture_target_table))
    , texture_unsized_internal_format(valid_texture_unsized_internal_format_table, std::size(valid_texture_unsized_internal_format_table))
    , transform_feedback_bind_target(valid_transform_feedback_bind_target_table, std::size(valid_transform_feedback_bind_target_table))
    , vertex_attrib_type(valid_vertex_attrib_type_table, std::size(valid_vertex_attrib_type_table))
    , vertex_attribute(valid_vertex_attribute_table, std::size(valid_vertex_attribute_table))
    , vertex_pointer(valid_vertex_pointer_table, std::size(valid_vertex_pointer_table))
{
}

void Validators::UpdateValuesES3()
{
    attachment.AddValues(valid_attachment_table_es3, std::size(valid_attachment_table_es3));
    attachment_query.AddValues(valid_attachment_query_table_es3, std::size(valid_attachment_query_table_es3));
    buffer_parameter.SetIsES3(true);
    buffer_target.SetIsES3(true);
    buffer_usage.SetIsES3(true);
    capability.AddValues(valid_capability_table_es3, std::size(valid_capability_table_es3));
    dst_blend_factor.AddValues(valid_dst_blend_factor_table_es3, std::size(valid_dst_blend_factor_table_es3));
    equation.AddValues(valid_equation_table_es3, std::size(valid_equation_table_es3));
    framebuffer_attachment_parameter.AddValues(valid_framebuffer_attachment_parameter_table_es3, std::size(valid_framebuffer_attachment_parameter_table_es3));
    framebuffer_target.AddValues(valid_framebuffer_target_table_es3, std::size(valid_framebuffer_target_table_es3));
    g_l_state.AddValues(valid_g_l_state_table_es3, std::size(valid_g_l_state_table_es3));
    get_tex_param_target.AddValues(valid_get_tex_param_target_table_es3, std::size(valid_get_tex_param_target_table_es3));
    hint_target.AddValues(valid_hint_target_table_es3, std::size(valid_hint_target_table_es3));
    index_type.AddValues(valid_index_type_table_es3, std::size(valid_index_type_table_es3));
    pixel_store.AddValues(valid_pixel_store_table_es3, std::size(valid_pixel_store_table_es3));
    pixel_type.AddValues(valid_pixel_type_table_es3, std::size(valid_pixel_type_table_es3));
    program_parameter.AddValues(valid_program_parameter_table_es3, std::size(valid_program_parameter_table_es3));
    read_pixel_format.AddValues(valid_read_pixel_format_table_es3, std::size(valid_read_pixel_format_table_es3));
    read_pixel_type.AddValues(valid_read_pixel_type_table_es3, std::size(valid_read_pixel_type_table_es3));
    render_buffer_format.AddValues(valid_render_buffer_format_table_es3, std::size(valid_render_buffer_format_table_es3));
    render_buffer_parameter.AddValues(valid_render_buffer_parameter_table_es3, std::size(valid_render_buffer_parameter_table_es3));
    texture_bind_target.AddValues(valid_texture_bind_target_table_es3, std::size(valid_texture_bind_target_table_es3));
    texture_depth_renderable_internal_format.AddValues(
        valid_texture_depth_renderable_internal_format_table_es3, std::size(valid_texture_depth_renderable_internal_format_table_es3));
    texture_format.AddValues(valid_texture_format_table_es3, std::size(valid_texture_format_table_es3));
    texture_internal_format.AddValues(valid_texture_internal_format_table_es3, std::size(valid_texture_internal_format_table_es3));
    texture_internal_format_storage.RemoveValues(
        deprecated_texture_internal_format_storage_table_es3, std::size(deprecated_texture_internal_format_storage_table_es3));
    texture_internal_format_storage.AddValues(valid_texture_internal_format_storage_table_es3, std::size(valid_texture_internal_format_storage_table_es3));
    texture_parameter.AddValues(valid_texture_parameter_table_es3, std::size(valid_texture_parameter_table_es3));
    texture_stencil_renderable_internal_format.AddValues(
        valid_texture_stencil_renderable_internal_format_table_es3, std::size(valid_texture_stencil_renderable_internal_format_table_es3));
    vertex_attrib_type.AddValues(valid_vertex_attrib_type_table_es3, std::size(valid_vertex_attrib_type_table_es3));
    vertex_attribute.AddValues(valid_vertex_attribute_table_es3, std::size(valid_vertex_attribute_table_es3));
}
...
void Validators::UpdateETCCompressedTextureFormats()
{
    ...
    compressed_texture_format.AddValue(GL_COMPRESSED_RGB8_PUNCHTHROUGH_ALPHA1_ETC2);
    compressed_texture_format.AddValue(GL_COMPRESSED_SRGB8_PUNCHTHROUGH_ALPHA1_ETC2);
    ...
    texture_internal_format_storage.AddValue(GL_COMPRESSED_RGB8_PUNCHTHROUGH_ALPHA1_ETC2);
    texture_internal_format_storage.AddValue(GL_COMPRESSED_SRGB8_PUNCHTHROUGH_ALPHA1_ETC2);
