...
void GLES2DecoderManualInitTest::EnableDisableTest(GLenum cap, bool enable, bool expect_set)
{
    if (expect_set) {
...
TEST_P(GLES3DecoderTest, Basic)
{
    ...
    EXPECT_TRUE(feature_info()->validators()->texture_bind_target.IsValid(GL_TEXTURE_3D));
}
...
TEST_P(GLES2DecoderTest, GetIntegervCached)
{
    ...
    for (size_t ii = 0; ii < sizeof(tests) / sizeof(tests[0]); ++ii) {
        ...
        EXPECT_EQ(decoder_->GetGLES2Util()->GLGetNumValuesReturned(test.pname), result->GetNumResults());
        EXPECT_EQ(GL_NO_ERROR, GetGLError());
...
{
    SetupIndexBuffer();
    auto* result = static_cast<cmds::GetMaxValueInBufferCHROMIUM::Result*>(shared_memory_address_);
    *result = 0;

    cmds::GetMaxValueInBufferCHROMIUM cmd;
    cmd.Init(client_element_buffer_id_, kValidIndexRangeCount, GL_UNSIGNED_SHORT, kValidIndexRangeStart * 2, shared_memory_id_, kSharedMemoryOffset);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(7u, *result);
    EXPECT_EQ(GL_NO_ERROR, GetGLError());
    cmd.Init(client_element_buffer_id_, kValidIndexRangeCount + 1, GL_UNSIGNED_SHORT, kValidIndexRangeStart * 2, shared_memory_id_, kSharedMemoryOffset);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(100u, *result);
    EXPECT_EQ(GL_NO_ERROR, GetGLError());

    cmd.Init(kInvalidClientId, kValidIndexRangeCount, GL_UNSIGNED_SHORT, kValidIndexRangeStart * 2, shared_memory_id_, kSharedMemoryOffset);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(GL_INVALID_VALUE, GetGLError());
    cmd.Init(client_element_buffer_id_, kOutOfRangeIndexRangeEnd, GL_UNSIGNED_SHORT, kValidIndexRangeStart * 2, shared_memory_id_, kSharedMemoryOffset);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(GL_INVALID_OPERATION, GetGLError());
    cmd.Init(client_element_buffer_id_, kValidIndexRangeCount + 1, GL_UNSIGNED_SHORT, kOutOfRangeIndexRangeEnd * 2, shared_memory_id_, kSharedMemoryOffset);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(GL_INVALID_OPERATION, GetGLError());
    cmd.Init(client_element_buffer_id_, kValidIndexRangeCount + 1, GL_UNSIGNED_SHORT, kValidIndexRangeStart * 2, shared_memory_id_, kSharedMemoryOffset);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    cmd.Init(client_buffer_id_, kValidIndexRangeCount + 1, GL_UNSIGNED_SHORT, kValidIndexRangeStart * 2, shared_memory_id_, kSharedMemoryOffset);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(GL_INVALID_OPERATION, GetGLError());
    cmd.Init(client_element_buffer_id_, kValidIndexRangeCount + 1, GL_UNSIGNED_SHORT, kValidIndexRangeStart * 2, kInvalidSharedMemoryId, kSharedMemoryOffset);
    EXPECT_NE(error::kNoError, ExecuteCmd(cmd));
    cmd.Init(client_element_buffer_id_, kValidIndexRangeCount + 1, GL_UNSIGNED_SHORT, kValidIndexRangeStart * 2, shared_memory_id_, kInvalidSharedMemoryOffset);
    EXPECT_NE(error::kNoError, ExecuteCmd(cmd));
}
...
TEST_P(GLES3DecoderTest, GetInternalformativValidArgsSamples)
{
    ...
    auto* result = static_cast<cmds::GetInternalformativ::Result*>(shared_memory_address_);
    EXPECT_CALL(*gl_, GetInternalformativ(GL_RENDERBUFFER, GL_RGBA8, GL_NUM_SAMPLE_COUNTS, 1, _))
        .WillOnce(SetArgPointee<4>(kNumSampleCounts))
        .RetiresOnSaturation();
    EXPECT_CALL(*gl_, GetInternalformativ(GL_RENDERBUFFER, GL_RGBA8, GL_SAMPLES, kNumSampleCounts, _)).Times(1).RetiresOnSaturation();
    result->size = 0;
    cmds::GetInternalformativ cmd;
    cmd.Init(GL_RENDERBUFFER, GL_RGBA8, GL_SAMPLES, shared_memory_id_, shared_memory_offset_);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
...
TEST_P(GLES3DecoderTest, GetInternalformativValidArgsNumSampleCounts)
{
    ...
    result->size = 0;
    cmds::GetInternalformativ cmd;
    cmd.Init(GL_RENDERBUFFER, GL_RGBA8, GL_NUM_SAMPLE_COUNTS, shared_memory_id_, shared_memory_offset_);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
...
{
    auto* result = static_cast<cmds::ClientWaitSync::Result*>(shared_memory_address_);
    cmds::ClientWaitSync cmd;
    cmd.Init(client_sync_id_, GL_SYNC_FLUSH_COMMANDS_BIT, 0, shared_memory_id_, shared_memory_offset_);
    EXPECT_CALL(*gl_, ClientWaitSync(reinterpret_cast<GLsync>(kServiceSyncId), GL_SYNC_FLUSH_COMMANDS_BIT, 0))
...
TEST_P(GLES3DecoderTest, ClientWaitSyncNonZeroTimeoutValid)
{
    ...
    cmd.Init(client_sync_id_, GL_SYNC_FLUSH_COMMANDS_BIT, kTimeout, shared_memory_id_, shared_memory_offset_);
    EXPECT_CALL(*gl_, ClientWaitSync(reinterpret_cast<GLsync>(kServiceSyncId), GL_SYNC_FLUSH_COMMANDS_BIT, kTimeout))
        .WillOnce(Return(GL_CONDITION_SATISFIED))
        .RetiresOnSaturation();
    *result = GL_WAIT_FAILED;
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
...
TEST_P(GLES3DecoderTest, ClientWaitSyncInvalidSyncFails)
{
    auto* result = static_cast<cmds::ClientWaitSync::Result*>(shared_memory_address_);
    cmds::ClientWaitSync cmd;
    cmd.Init(kInvalidClientId, GL_SYNC_FLUSH_COMMANDS_BIT, 0, shared_memory_id_, shared_memory_offset_);
    *result = GL_WAIT_FAILED;
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
...
TEST_P(GLES3DecoderTest, ClientWaitSyncBadSharedMemoryFails)
{
    ...
    cmds::ClientWaitSync cmd;
    *result = GL_WAIT_FAILED;
    cmd.Init(client_sync_id_, GL_SYNC_FLUSH_COMMANDS_BIT, 0, kInvalidSharedMemoryId, shared_memory_offset_);
    EXPECT_NE(error::kNoError, ExecuteCmd(cmd));

    *result = GL_WAIT_FAILED;
    cmd.Init(client_sync_id_, GL_SYNC_FLUSH_COMMANDS_BIT, 0, shared_memory_id_, kInvalidSharedMemoryOffset);
    EXPECT_NE(error::kNoError, ExecuteCmd(cmd));
}
...
TEST_P(GLES3DecoderTest, WaitSyncValidArgs)
{
    ...
    EXPECT_CALL(*gl_, WaitSync(reinterpret_cast<GLsync>(kServiceSyncId), 0, kTimeout)).Times(1).RetiresOnSaturation();
...
TEST_P(GLES2DecoderManualInitTest, InitFailsIfLostContext)
{
    ...
    EXPECT_EQ(ContextResult::kTransientFailure, MaybeInitDecoderWithWorkarounds(init, GpuDriverBugWorkarounds()));
}
...
{
    cmds::GenQueriesEXTImmediate* cmd = GetImmediateAs<cmds::GenQueriesEXTImmediate>();
...
TEST_P(GLES2DecoderTest, GenQueriesEXTImmediateInvalidArgs)
{
    ...
    EXPECT_EQ(error::kInvalidArguments, ExecuteImmediateCmd(*cmd, sizeof(&client_query_id_)));
}
...
TEST_P(GLES2DecoderManualInitTest, BeginEndQueryEXT)
{
    ...
    // Test id = 0 fails.
    begin_cmd.Init(GL_ANY_SAMPLES_PASSED_EXT, 0, shared_memory_id_, kSharedMemoryOffset);
    ...
    EXPECT_CALL(*gl_, GenQueries(1, _)).WillOnce(SetArgPointee<1>(kNewServiceId)).RetiresOnSaturation();
    EXPECT_CALL(*gl_, BeginQuery(GL_ANY_SAMPLES_PASSED_EXT, kNewServiceId)).Times(1).RetiresOnSaturation();
    ...
    begin_cmd.Init(GL_ANY_SAMPLES_PASSED_EXT, kInvalidClientId, shared_memory_id_, kSharedMemoryOffset);
    EXPECT_EQ(error::kNoError, ExecuteCmd(begin_cmd));
    EXPECT_EQ(GL_INVALID_OPERATION, GetGLError());

    begin_cmd.Init(GL_ANY_SAMPLES_PASSED_EXT, kNewClientId, shared_memory_id_, kSharedMemoryOffset);
    EXPECT_EQ(error::kNoError, ExecuteCmd(begin_cmd));
    EXPECT_EQ(GL_NO_ERROR, GetGLError());
    ...
    // Test end succeeds.
    EXPECT_CALL(*gl_, EndQuery(GL_ANY_SAMPLES_PASSED_EXT)).Times(1).RetiresOnSaturation();
    end_cmd.Init(GL_ANY_SAMPLES_PASSED_EXT, 1);
    EXPECT_EQ(error::kNoError, ExecuteCmd(end_cmd));
    EXPECT_EQ(GL_NO_ERROR, GetGLError());
    ...
    begin_cmd.Init(GL_ANY_SAMPLES_PASSED_CONSERVATIVE_EXT, kNewClientId, shared_memory_id_, kSharedMemoryOffset);
    EXPECT_EQ(error::kNoError, ExecuteCmd(begin_cmd));
    EXPECT_EQ(GL_INVALID_OPERATION, GetGLError());

    // Begin should fail if using a different sync
    begin_cmd.Init(GL_ANY_SAMPLES_PASSED_CONSERVATIVE_EXT, kNewClientId, shared_memory_id_, kSharedMemoryOffset + sizeof(QuerySync));
    EXPECT_EQ(error::kNoError, ExecuteCmd(begin_cmd));
    EXPECT_EQ(GL_INVALID_OPERATION, GetGLError());

    // QueryCounter should fail if using a different target
    cmds::QueryCounterEXT query_counter_cmd;
    query_counter_cmd.Init(kNewClientId, GL_TIMESTAMP, shared_memory_id_, kSharedMemoryOffset, 1);
    EXPECT_EQ(error::kNoError, ExecuteCmd(query_counter_cmd));
    EXPECT_EQ(GL_INVALID_OPERATION, GetGLError());

    // QueryCounter should fail if using a different sync
    query_counter_cmd.Init(kNewClientId, GL_TIMESTAMP, shared_memory_id_, kSharedMemoryOffset + sizeof(QuerySync), 1);
    EXPECT_EQ(error::kNoError, ExecuteCmd(query_counter_cmd));
    EXPECT_EQ(GL_INVALID_OPERATION, GetGLError());
...
static void ExecuteGenerateQueryCmd(GLES2DecoderTestBase* test, ::gl::MockGLInterface* gl, GLenum target, GLuint client_id, GLuint service_id)
{
    test->GenHelper<cmds::GenQueriesEXTImmediate>(client_id);
    if (GL_ANY_SAMPLES_PASSED_EXT == target) {
        EXPECT_CALL(*gl, GenQueries(1, _)).WillOnce(SetArgPointee<1>(service_id)).RetiresOnSaturation();
    }
}

static error::Error ExecuteBeginQueryCmd(GLES2DecoderTestBase* test, ::gl::MockGLInterface* gl, ::gl::GPUTimingFake* timing_queries, GLenum target,
    GLuint client_id, GLuint service_id, int32_t shm_id, uint32_t shm_offset)
{
    if (GL_ANY_SAMPLES_PASSED_EXT == target) {
        EXPECT_CALL(*gl, BeginQuery(target, service_id)).Times(1).RetiresOnSaturation();
...
static error::Error ExecuteEndQueryCmd(GLES2DecoderTestBase* test, ::gl::MockGLInterface* gl, GLenum target, uint32_t submit_count)
{
    if (GL_ANY_SAMPLES_PASSED_EXT == target) {
        EXPECT_CALL(*gl, EndQuery(target)).Times(1).RetiresOnSaturation();
        ...
        EXPECT_CALL(*gl, FenceSync(GL_SYNC_GPU_COMMANDS_COMPLETE, 0)).WillOnce(Return(kGlSync)).RetiresOnSaturation();
#if DCHECK_IS_ON()
        EXPECT_CALL(*gl, IsSync(kGlSync)).WillRepeatedly(Return(GL_TRUE));
#endif
    }
...
static error::Error ExecuteQueryCounterCmd(GLES2DecoderTestBase* test, ::gl::MockGLInterface* gl, ::gl::GPUTimingFake* timing_queries, GLenum target,
    GLuint client_id, GLuint service_id, int32_t shm_id, uint32_t shm_offset, uint32_t submit_count)
{
    ...
    cmds::QueryCounterEXT query_counter_cmd;
    query_counter_cmd.Init(client_id, target, shm_id, shm_offset, submit_count);
    return test->ExecuteCmd(query_counter_cmd);
}

static void ProcessQuery(GLES2DecoderTestBase* test, ::gl::MockGLInterface* gl, GLenum target, GLuint service_id)
{
    if (GL_ANY_SAMPLES_PASSED_EXT == target) {
        EXPECT_CALL(*gl, GetQueryObjectuiv(service_id, GL_QUERY_RESULT_AVAILABLE_EXT, _)).WillOnce(SetArgPointee<2>(1)).RetiresOnSaturation();
        EXPECT_CALL(*gl, GetQueryObjectuiv(service_id, GL_QUERY_RESULT_EXT, _)).WillOnce(SetArgPointee<2>(1)).RetiresOnSaturation();
    } else if (GL_COMMANDS_COMPLETED_CHROMIUM == target) {
        EXPECT_CALL(*gl, ClientWaitSync(kGlSync, _, _)).WillOnce(Return(GL_ALREADY_SIGNALED)).RetiresOnSaturation();
...
static void CheckBeginEndQueryBadMemoryFails(GLES2DecoderTestBase* test, GLuint client_id, const QueryType& query_type, int32_t shm_id, uint32_t shm_offset)
{
...
TEST_P(GLES2DecoderManualInitTest, BeginEndQueryEXTBadMemoryIdFails)
{
    for (size_t i = 0; i < std::size(kQueryTypes); ++i) {
        CheckBeginEndQueryBadMemoryFails(this, kNewClientId, kQueryTypes[i], kInvalidSharedMemoryId, kSharedMemoryOffset);
    }
}
...
TEST_P(GLES2DecoderManualInitTest, BeginEndQueryEXTBadMemoryOffsetFails)
{
    for (size_t i = 0; i < std::size(kQueryTypes); ++i) {
        // Out-of-bounds.
        CheckBeginEndQueryBadMemoryFails(this, kNewClientId, kQueryTypes[i], shared_memory_id_, kInvalidSharedMemoryOffset);
        // Overflow.
        CheckBeginEndQueryBadMemoryFails(this, kNewClientId, kQueryTypes[i], shared_memory_id_, 0xfffffffcu);
    }
}
...
TEST_P(GLES2DecoderManualInitTest, QueryReuseTest)
{
    for (size_t i = 0; i < std::size(kQueryTypes); ++i) {
        ...
        ExecuteGenerateQueryCmd(this, gl, query_type.type, kNewClientId, kNewServiceId);

        // Query once.
        if (query_type.is_counter) {
            EXPECT_EQ(error::kNoError,
                ExecuteQueryCounterCmd(this, gl, &gpu_timing_queries, query_type.type, kNewClientId, kNewServiceId, shared_memory_id_, kSharedMemoryOffset, 1));
        } else {
            EXPECT_EQ(error::kNoError,
                ExecuteBeginQueryCmd(this, gl, &gpu_timing_queries, query_type.type, kNewClientId, kNewServiceId, shared_memory_id_, kSharedMemoryOffset));
            EXPECT_EQ(error::kNoError, ExecuteEndQueryCmd(this, gl, query_type.type, 1));
        }
        ...
        if (query_type.is_counter) {
            EXPECT_EQ(error::kNoError,
                ExecuteQueryCounterCmd(this, gl, &gpu_timing_queries, query_type.type, kNewClientId, kNewServiceId, shared_memory_id_, kSharedMemoryOffset, 2));
        } else {
            EXPECT_EQ(error::kNoError,
                ExecuteBeginQueryCmd(this, gl, &gpu_timing_queries, query_type.type, kNewClientId, kNewServiceId, shared_memory_id_, kSharedMemoryOffset));
            EXPECT_EQ(error::kNoError, ExecuteEndQueryCmd(this, gl, query_type.type, 2));
        }
...
TEST_P(GLES2DecoderTest, BeginEndQueryEXTCommandsIssuedCHROMIUM)
{
    ...
    // Test valid parameters work.
    begin_cmd.Init(GL_COMMANDS_ISSUED_CHROMIUM, kNewClientId, shared_memory_id_, kSharedMemoryOffset);
    EXPECT_EQ(error::kNoError, ExecuteCmd(begin_cmd));
    EXPECT_EQ(GL_NO_ERROR, GetGLError());
...
TEST_P(GLES2DecoderTest, QueryCounterEXTCommandsIssuedTimestampCHROMIUM)
{
    ...
    cmds::QueryCounterEXT query_counter_cmd;
    query_counter_cmd.Init(kNewClientId, GL_COMMANDS_ISSUED_TIMESTAMP_CHROMIUM, shared_memory_id_, kSharedMemoryOffset, 1);
    EXPECT_EQ(error::kNoError, ExecuteCmd(query_counter_cmd));
    EXPECT_EQ(GL_NO_ERROR, GetGLError());
...
TEST_P(GLES2DecoderTest, BeginEndQueryEXTGetErrorQueryCHROMIUM)
{
    ...
    // Test valid parameters work.
    begin_cmd.Init(GL_GET_ERROR_QUERY_CHROMIUM, kNewClientId, shared_memory_id_, kSharedMemoryOffset);
    EXPECT_EQ(error::kNoError, ExecuteCmd(begin_cmd));
    EXPECT_EQ(GL_NO_ERROR, GetGLError());
    ...
    EXPECT_CALL(*gl_, GetError()).WillOnce(Return(GL_INVALID_VALUE)).RetiresOnSaturation();
    ...
    EXPECT_FALSE(query->IsPending());
    EXPECT_FALSE(query->IsActive());
    EXPECT_EQ(static_cast<GLenum>(GL_INVALID_VALUE), static_cast<GLenum>(sync->result));
}
...
TEST_P(GLES2DecoderManualInitTest, BeginEndQueryEXTCommandsCompletedCHROMIUM)
{
    ...
    cmds::BeginQueryEXT begin_cmd;
    begin_cmd.Init(GL_COMMANDS_COMPLETED_CHROMIUM, kNewClientId, shared_memory_id_, kSharedMemoryOffset);
    EXPECT_EQ(error::kNoError, ExecuteCmd(begin_cmd));
    EXPECT_EQ(GL_NO_ERROR, GetGLError());
    ...
    EXPECT_CALL(*gl_, FenceSync(GL_SYNC_GPU_COMMANDS_COMPLETE, 0)).WillOnce(Return(kGlSync)).RetiresOnSaturation();
#if DCHECK_IS_ON()
    EXPECT_CALL(*gl_, IsSync(kGlSync)).WillOnce(Return(GL_TRUE)).RetiresOnSaturation();
#endif
    ...
    EXPECT_CALL(*gl_, IsSync(kGlSync)).WillOnce(Return(GL_TRUE)).RetiresOnSaturation();
#endif
    EXPECT_CALL(*gl_, ClientWaitSync(kGlSync, _, _)).WillOnce(Return(GL_TIMEOUT_EXPIRED)).RetiresOnSaturation();
    query_manager->ProcessPendingQueries(false);
    ...
    EXPECT_CALL(*gl_, IsSync(kGlSync)).WillOnce(Return(GL_TRUE)).RetiresOnSaturation();
#endif
    EXPECT_CALL(*gl_, ClientWaitSync(kGlSync, _, _)).WillOnce(Return(GL_ALREADY_SIGNALED)).RetiresOnSaturation();
    query_manager->ProcessPendingQueries(false);
...
TEST_P(GLES2DecoderManualInitTest, BeginInvalidTargetQueryFails)
{
    ...
    begin_cmd.Init(GL_ANY_SAMPLES_PASSED, kNewClientId, shared_memory_id_, kSharedMemoryOffset);
    EXPECT_EQ(error::kNoError, ExecuteCmd(begin_cmd));
    EXPECT_EQ(GL_INVALID_OPERATION, GetGLError());

    begin_cmd.Init(GL_ANY_SAMPLES_PASSED_CONSERVATIVE, kNewClientId, shared_memory_id_, kSharedMemoryOffset);
    EXPECT_EQ(error::kNoError, ExecuteCmd(begin_cmd));
    EXPECT_EQ(GL_INVALID_OPERATION, GetGLError());

    begin_cmd.Init(GL_TIME_ELAPSED, kNewClientId, shared_memory_id_, kSharedMemoryOffset);
    EXPECT_EQ(error::kNoError, ExecuteCmd(begin_cmd));
    EXPECT_EQ(GL_INVALID_OPERATION, GetGLError());

    begin_cmd.Init(0xdeadbeef, kNewClientId, shared_memory_id_, kSharedMemoryOffset);
    EXPECT_EQ(error::kNoError, ExecuteCmd(begin_cmd));
...
TEST_P(GLES2DecoderManualInitTest, QueryCounterEXTTimeStamp)
{
    ...
    EXPECT_CALL(*gl_, GenQueries(1, _)).WillOnce(SetArgPointee<1>(kNewServiceId)).RetiresOnSaturation();
    EXPECT_CALL(*gl_, GetQueryiv(GL_TIMESTAMP, GL_QUERY_COUNTER_BITS, _)).WillOnce(SetArgPointee<2>(64)).RetiresOnSaturation();
    EXPECT_CALL(*gl_, QueryCounter(kNewServiceId, GL_TIMESTAMP)).Times(1).RetiresOnSaturation();
...
TEST_P(GLES2DecoderManualInitTest, InvalidTargetQueryCounterFails)
{
    ...
    query_counter_cmd.Init(kNewClientId, 0xdeadbeef, shared_memory_id_, kSharedMemoryOffset, 1);
    EXPECT_EQ(error::kNoError, ExecuteCmd(query_counter_cmd));
    EXPECT_EQ(GL_INVALID_ENUM, GetGLError());
}
...
TEST_P(GLES2DecoderTest, IsEnabledReturnsCachedValue)
{
    ...
    for (size_t ii = 0; ii < std::size(kStates); ++ii) {
        ...
        auto* result = static_cast<cmds::IsEnabled::Result*>(shared_memory_address_);
...
TEST_P(GLES2DecoderManualInitTest, MemoryTrackerTexImage2D)
{
    ...
    InitDecoder(init);
    DoBindTexture(GL_TEXTURE_2D, client_texture_id_, kServiceTextureId);
    DoTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 8, 4, 0, GL_RGBA, GL_UNSIGNED_BYTE, shared_memory_id_, kSharedMemoryOffset);
    EXPECT_EQ(128u, memory_tracker_ptr->GetSize());
    DoTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 4, 4, 0, GL_RGBA, GL_UNSIGNED_BYTE, shared_memory_id_, kSharedMemoryOffset);
...
TEST_P(GLES2DecoderManualInitTest, MemoryTrackerTexStorage2DEXT)
{
    ...
    InitDecoder(init);
    DoBindTexture(GL_TEXTURE_2D, client_texture_id_, kServiceTextureId);
    EXPECT_CALL(*gl_, TexStorage2DEXT(GL_TEXTURE_2D, 1, GL_RGBA8, 8, 4)).Times(1).RetiresOnSaturation();
...
TEST_P(GLES2DecoderManualInitTest, MemoryTrackerRenderbufferStorage)
{
    ...
    EXPECT_CALL(*gl_, GetError()).WillOnce(Return(GL_NO_ERROR)).WillOnce(Return(GL_NO_ERROR)).RetiresOnSaturation();
    EXPECT_CALL(*gl_, RenderbufferStorageEXT(GL_RENDERBUFFER, kFormat, kWidth, kHeight)).Times(1).RetiresOnSaturation();
    cmds::RenderbufferStorage cmd;
    cmd.Init(GL_RENDERBUFFER, kFormat, kWidth, kHeight);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(GL_NO_ERROR, GetGLError());
    EXPECT_EQ(kWidth * kHeight * kNumOfBytesPerPixel, memory_tracker_ptr->GetSize());
}
...
TEST_P(GLES2DecoderManualInitTest, MemoryTrackerBufferData)
{
    ...
    DoBindBuffer(GL_ARRAY_BUFFER, client_buffer_id_, kServiceBufferId);
    EXPECT_CALL(*gl_, GetError()).WillOnce(Return(GL_NO_ERROR)).WillOnce(Return(GL_NO_ERROR)).RetiresOnSaturation();
    EXPECT_CALL(*gl_, BufferData(GL_ARRAY_BUFFER, 128, _, GL_STREAM_DRAW)).Times(1).RetiresOnSaturation();
    cmds::BufferData cmd;
...
TEST_P(GLES2DecoderManualInitTest, ImmutableCopyTexImage2D)
{
    ...
    EXPECT_CALL(*gl_, GetError()).Times(AtLeast(1));

    EXPECT_CALL(*gl_, CopyTexImage2D(kTarget, kLevel, kInternalFormat, 0, 0, kWidth, kHeight, kBorder)).Times(1);

    EXPECT_CALL(*gl_, TexStorage2DEXT(kTarget, kLevels, kSizedInternalFormat, kWidth, kHeight)).Times(1);
...
{
    EXPECT_CALL(*mock_decoder_, MarkContextLost(error::kInnocent)).Times(1);
...
{
    EXPECT_CALL(*mock_decoder_, MarkContextLost(_)).Times(0);
    cmds::LoseContextCHROMIUM cmd;
...
{
    EXPECT_CALL(*gl_, BindTransformFeedback(GL_TRANSFORM_FEEDBACK, kServiceTransformFeedbackId));
    ...
    EXPECT_CALL(*gl_, BindBuffer(GL_TRANSFORM_FEEDBACK_BUFFER, 0)).Times(1).RetiresOnSaturation();
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(GL_NO_ERROR, GetGLError());
}
...
{
    cmds::DeleteTransformFeedbacksImmediate& cmd = *GetImmediateAs<cmds::DeleteTransformFeedbacksImmediate>();
...
TEST_P(GLES3DecoderTest, GetIntegeri_vValidArgs)
{
    ...
    auto* result = static_cast<cmds::GetIntegeri_v::Result*>(shared_memory_address_);
    result->size = 0;
    cmds::GetIntegeri_v cmd;
    cmd.Init(GL_TRANSFORM_FEEDBACK_BUFFER_BINDING, 2, shared_memory_id_, shared_memory_offset_);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(decoder_->GetGLES2Util()->GLGetNumValuesReturned(GL_TRANSFORM_FEEDBACK_BUFFER_BINDING), result->GetNumResults());
    EXPECT_EQ(GL_NO_ERROR, GetGLError());
}
...
TEST_P(GLES3DecoderTest, GetInteger64i_vValidArgs)
{
    ...
    auto* result = static_cast<cmds::GetInteger64i_v::Result*>(shared_memory_address_);
    result->size = 0;
    cmds::GetInteger64i_v cmd;
    cmd.Init(GL_UNIFORM_BUFFER_SIZE, 2, shared_memory_id_, shared_memory_offset_);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(decoder_->GetGLES2Util()->GLGetNumValuesReturned(GL_UNIFORM_BUFFER_SIZE), result->GetNumResults());
    EXPECT_EQ(GL_NO_ERROR, GetGLError());
}
...
#if DCHECK_IS_ON()
    EXPECT_CALL(*gl_, GetIntegerv(GL_SAMPLER_BINDING, _)).WillOnce(testing::SetArgumentPointee<1>(kServiceID)).RetiresOnSaturation();
#endif
...
TEST_P(GLES3DecoderTest, GetTransformFeedbackBinding)
{
    ...
    auto* result = static_cast<cmds::GetIntegerv::Result*>(shared_memory_address_);
    cmds::GetIntegerv cmd;
    cmd.Init(GL_TRANSFORM_FEEDBACK_BINDING, shared_memory_id_, shared_memory_offset_);
    result->size = 0;
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(1, result->GetNumResults());
    EXPECT_EQ(kClientID, static_cast<GLuint>(result->GetData()[0]));
...
TEST_P(GLES2DecoderDoCommandsTest, DoCommandsOneOfZero)
{
    ...
    EXPECT_EQ(error::kNoError, decoder_->DoCommands(1, &cmds_, entries_per_cmd_ * 0, &num_processed));
    EXPECT_EQ(GL_NO_ERROR, GetGLError());
...
TEST_P(GLES2DecoderDoCommandsTest, DoCommandsOneOfOne)
{
    ...
    EXPECT_EQ(error::kNoError, decoder_->DoCommands(1, &cmds_, entries_per_cmd_ * 1, &num_processed));
    EXPECT_EQ(GL_NO_ERROR, GetGLError());
...
TEST_P(GLES2DecoderDoCommandsTest, DoCommandsThreeOfThree)
{
    ...
    EXPECT_EQ(error::kNoError, decoder_->DoCommands(3, &cmds_, entries_per_cmd_ * 3, &num_processed));
    EXPECT_EQ(GL_NO_ERROR, GetGLError());
...
TEST_P(GLES2DecoderDoCommandsTest, DoCommandsTwoOfThree)
{
    ...
    EXPECT_EQ(error::kNoError, decoder_->DoCommands(2, &cmds_, entries_per_cmd_ * 3, &num_processed));
    EXPECT_EQ(GL_NO_ERROR, GetGLError());
...
TEST_P(GLES2DecoderDoCommandsTest, DoCommandsZeroCmdSize)
{
    ...
    int num_processed = -1;
    SetExpectationsForNCommands(1);
    EXPECT_EQ(error::kInvalidSize, decoder_->DoCommands(2, &cmds_, entries_per_cmd_ * 2, &num_processed));
    EXPECT_EQ(GL_NO_ERROR, GetGLError());
    EXPECT_EQ(entries_per_cmd_, num_processed);
}
...
TEST_P(GLES2DecoderDoCommandsTest, DoCommandsOutOfBounds)
{
    int num_processed = -1;
    SetExpectationsForNCommands(1);
    EXPECT_EQ(error::kOutOfBounds, decoder_->DoCommands(2, &cmds_, entries_per_cmd_ * 2 - 1, &num_processed));
    EXPECT_EQ(GL_NO_ERROR, GetGLError());
    EXPECT_EQ(entries_per_cmd_, num_processed);
}
...
TEST_P(GLES2DecoderDoCommandsTest, DoCommandsBadArgSize)
{
    ...
    int num_processed = -1;
    SetExpectationsForNCommands(1);
    EXPECT_EQ(error::kInvalidArguments, decoder_->DoCommands(2, &cmds_, entries_per_cmd_ * 2 + 1, &num_processed));
    EXPECT_EQ(GL_NO_ERROR, GetGLError());
    ...
    EXPECT_EQ(entries_per_cmd_ + static_cast<int>(cmds_[1].header.size), num_processed);
}
...
public:
    ...
    void SetUp() override
    {
        ...
        EXPECT_CALL(*gl_, DeleteSync(sync_service_id_)).Times(1).RetiresOnSaturation();
        EXPECT_CALL(*gl_, DeleteSync(sync_service_id2_)).Times(1).RetiresOnSaturation();
    }
...
{
    EXPECT_CALL(*gl_, ClientWaitSync(sync_service_id_, 0, 0)).Times(1).WillOnce(Return(GL_ALREADY_SIGNALED)).RetiresOnSaturation();
...
{
    EXPECT_CALL(*gl_, ClientWaitSync(sync_service_id_, 0, 0)).Times(1).WillOnce(Return(GL_TIMEOUT_EXPIRED)).RetiresOnSaturation();
...
INSTANTIATE_TEST_SUITE_P(Service, GLES2DecoderWithShaderTest, ::testing::Bool());

INSTANTIATE_TEST_SUITE_P(Service, GLES2DecoderManualInitTest, ::testing::Bool());

INSTANTIATE_TEST_SUITE_P(Service, GLES2DecoderRGBBackbufferTest, ::testing::Bool());

INSTANTIATE_TEST_SUITE_P(Service, GLES2DecoderDoCommandsTest, ::testing::Bool());

INSTANTIATE_TEST_SUITE_P(Service, GLES2DecoderDescheduleUntilFinishedTest, ::testing::Bool());
...
INSTANTIATE_TEST_SUITE_P(Service, GLES3DecoderWithShaderTest, ::testing::Bool());

INSTANTIATE_TEST_SUITE_P(Service, GLES3DecoderManualInitTest, ::testing::Bool());
