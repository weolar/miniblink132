...
TEST_P(GLES2DecoderWithShaderTest, GetProgramInfoCHROMIUMInvalidArgs)
{
    ...
    ProgramInfoHeader* info = bucket->GetDataAs<ProgramInfoHeader*>(0, sizeof(ProgramInfoHeader));
    ASSERT_TRUE(info != 0);
...
TEST_P(GLES3DecoderWithShaderTest, GetUniformBlocksCHROMIUMValidArgs)
{
    ...
    cmd.Init(client_program_id_, kBucketId);
    EXPECT_CALL(*gl_, GetProgramiv(kServiceProgramId, GL_LINK_STATUS, _)).WillOnce(SetArgPointee<2>(GL_TRUE)).RetiresOnSaturation();
    EXPECT_CALL(*gl_, GetProgramiv(kServiceProgramId, GL_ACTIVE_UNIFORM_BLOCKS, _)).WillOnce(SetArgPointee<2>(0)).RetiresOnSaturation();
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    CommonDecoder::Bucket* bucket = decoder_->GetBucket(kBucketId);
...
TEST_P(GLES3DecoderWithShaderTest, GetUniformsES3CHROMIUMValidArgs)
{
    ...
    cmd.Init(client_program_id_, kBucketId);
    EXPECT_CALL(*gl_, GetProgramiv(kServiceProgramId, GL_LINK_STATUS, _)).WillOnce(SetArgPointee<2>(GL_TRUE)).RetiresOnSaturation();
    EXPECT_CALL(*gl_, GetProgramiv(kServiceProgramId, GL_ACTIVE_UNIFORMS, _)).WillOnce(SetArgPointee<2>(0)).RetiresOnSaturation();
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    CommonDecoder::Bucket* bucket = decoder_->GetBucket(kBucketId);
...
TEST_P(GLES3DecoderWithShaderTest, GetTransformFeedbackVaryingsCHROMIUMValidArgs)
{
    const uint32_t kBucketId = 123;
    ...
    EXPECT_CALL(*(gl_.get()), GetProgramiv(kServiceProgramId, GL_TRANSFORM_FEEDBACK_BUFFER_MODE, _))
        ...
        .RetiresOnSaturation();
    EXPECT_CALL(*gl_, GetProgramiv(kServiceProgramId, GL_LINK_STATUS, _)).WillOnce(SetArgPointee<2>(GL_TRUE)).RetiresOnSaturation();
    EXPECT_CALL(*gl_, GetProgramiv(kServiceProgramId, GL_TRANSFORM_FEEDBACK_VARYINGS, _)).WillOnce(SetArgPointee<2>(0)).RetiresOnSaturation();
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    CommonDecoder::Bucket* bucket = decoder_->GetBucket(kBucketId);
...
TEST_P(GLES3DecoderWithShaderTest, GetTransformFeedbackVaryingsCHROMIUMInvalidArgs)
{
    const uint32_t kBucketId = 123;
    CommonDecoder::Bucket* bucket = decoder_->GetBucket(kBucketId);
    EXPECT_TRUE(bucket == nullptr);
...
{
    auto* result = static_cast<cmds::GetUniformiv::Result*>(shared_memory_address_);
    result->size = 0;
    cmds::GetUniformiv cmd;
    cmd.Init(client_program_id_, kUniform2FakeLocation, shared_memory_id_, kSharedMemoryOffset);
    EXPECT_CALL(*gl_, GetUniformiv(kServiceProgramId, kUniform2RealLocation, _)).Times(1);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(GLES2Util::GetElementCountForUniformType(kUniform2Type), static_cast<uint32_t>(result->GetNumResults()));
}
...
TEST_P(GLES2DecoderWithShaderTest, GetUniformivArrayElementSucceeds)
{
    ...
    result->size = 0;
    cmds::GetUniformiv cmd;
    cmd.Init(client_program_id_, kUniform2ElementFakeLocation, shared_memory_id_, kSharedMemoryOffset);
    EXPECT_CALL(*gl_, GetUniformiv(kServiceProgramId, kUniform2ElementRealLocation, _)).Times(1);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(GLES2Util::GetElementCountForUniformType(kUniform2Type), static_cast<uint32_t>(result->GetNumResults()));
}
...
TEST_P(GLES2DecoderWithShaderTest, GetUniformivBadProgramFails)
{
    ...
    cmd.Init(kInvalidClientId, kUniform2FakeLocation, shared_memory_id_, kSharedMemoryOffset);
...
// this case.
    ...
    cmd.Init(client_shader_id_, kUniform2FakeLocation, shared_memory_id_, kSharedMemoryOffset);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(0U, result->size);
    EXPECT_EQ(GL_INVALID_OPERATION, GetGLError());
#endif // GLES2_TEST_SHADER_VS_PROGRAM_IDS
    // Unlinked program
    EXPECT_CALL(*gl_, CreateProgram()).Times(1).WillOnce(Return(kNewServiceId)).RetiresOnSaturation();
    cmds::CreateProgram cmd2;
    cmd2.Init(kNewClientId);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd2));
    result->size = kInitialResult;
    cmd.Init(kNewClientId, kUniform2FakeLocation, shared_memory_id_, kSharedMemoryOffset);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(0U, result->size);
    EXPECT_EQ(GL_INVALID_OPERATION, GetGLError());
}
...
TEST_P(GLES2DecoderWithShaderTest, GetUniformivBadLocationFails)
{
    ...
    cmds::GetUniformiv cmd;
    // invalid location
    cmd.Init(client_program_id_, kInvalidUniformLocation, shared_memory_id_, kSharedMemoryOffset);
    EXPECT_CALL(*gl_, GetUniformiv(_, _, _)).Times(0);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(0U, result->size);
    EXPECT_EQ(GL_INVALID_OPERATION, GetGLError());
...
{
    cmds::GetUniformiv cmd;
    cmd.Init(client_program_id_, kUniform2FakeLocation, kInvalidSharedMemoryId, kSharedMemoryOffset);
    EXPECT_CALL(*gl_, GetUniformiv(_, _, _)).Times(0);
    EXPECT_NE(error::kNoError, ExecuteCmd(cmd));
    cmd.Init(client_program_id_, kUniform2FakeLocation, shared_memory_id_, kInvalidSharedMemoryOffset);
    EXPECT_NE(error::kNoError, ExecuteCmd(cmd));
}

TEST_P(GLES3DecoderWithShaderTest, GetUniformuivSucceeds)
{
    auto* result = static_cast<cmds::GetUniformuiv::Result*>(shared_memory_address_);
    result->size = 0;
    cmds::GetUniformuiv cmd;
    cmd.Init(client_program_id_, kUniform2FakeLocation, shared_memory_id_, kSharedMemoryOffset);
    EXPECT_CALL(*gl_, GetUniformuiv(kServiceProgramId, kUniform2RealLocation, _)).Times(1);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(GLES2Util::GetElementCountForUniformType(kUniform2Type), static_cast<uint32_t>(result->GetNumResults()));
}
...
TEST_P(GLES3DecoderWithShaderTest, GetUniformuivArrayElementSucceeds)
{
    ...
    cmds::GetUniformuiv cmd;
    cmd.Init(client_program_id_, kUniform2ElementFakeLocation, shared_memory_id_, kSharedMemoryOffset);
    EXPECT_CALL(*gl_, GetUniformuiv(kServiceProgramId, kUniform2ElementRealLocation, _)).Times(1);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(GLES2Util::GetElementCountForUniformType(kUniform2Type), static_cast<uint32_t>(result->GetNumResults()));
}
...
{
    auto* result = static_cast<cmds::GetUniformfv::Result*>(shared_memory_address_);
    result->size = 0;
    cmds::GetUniformfv cmd;
    cmd.Init(client_program_id_, kUniform2FakeLocation, shared_memory_id_, kSharedMemoryOffset);
    EXPECT_CALL(*gl_, GetUniformfv(kServiceProgramId, kUniform2RealLocation, _)).Times(1);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(GLES2Util::GetElementCountForUniformType(kUniform2Type), static_cast<uint32_t>(result->GetNumResults()));
}
...
TEST_P(GLES2DecoderWithShaderTest, GetUniformfvArrayElementSucceeds)
{
    ...
    cmds::GetUniformfv cmd;
    cmd.Init(client_program_id_, kUniform2ElementFakeLocation, shared_memory_id_, kSharedMemoryOffset);
    EXPECT_CALL(*gl_, GetUniformfv(kServiceProgramId, kUniform2ElementRealLocation, _)).Times(1);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(GLES2Util::GetElementCountForUniformType(kUniform2Type), static_cast<uint32_t>(result->GetNumResults()));
}
...
TEST_P(GLES2DecoderWithShaderTest, GetAttachedShadersSucceeds)
{
    ...
    result->size = 0;
    EXPECT_CALL(*gl_, GetAttachedShaders(kServiceProgramId, 1, _, _)).WillOnce(DoAll(SetArgPointee<2>(1), SetArgPointee<3>(kServiceShaderId)));
    cmd.Init(client_program_id_, shared_memory_id_, shared_memory_offset_, Result::ComputeSize(1).ValueOrDie());
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
...
TEST_P(GLES2DecoderWithShaderTest, GetAttachedShadersBadProgramFails)
{
    ...
    result->size = 0;
    EXPECT_CALL(*gl_, GetAttachedShaders(_, _, _, _)).Times(0);
    cmd.Init(kInvalidClientId, shared_memory_id_, shared_memory_offset_, Result::ComputeSize(1).ValueOrDie());
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(0U, result->size);
    EXPECT_EQ(GL_INVALID_VALUE, GetGLError());
}
...
TEST_P(GLES2DecoderWithShaderTest, GetAttachedShadersBadSharedMemoryFails)
{
    cmds::GetAttachedShaders cmd;
    using Result = cmds::GetAttachedShaders::Result;
    cmd.Init(client_program_id_, kInvalidSharedMemoryId, shared_memory_offset_, Result::ComputeSize(1).ValueOrDie());
    EXPECT_CALL(*gl_, GetAttachedShaders(_, _, _, _)).Times(0);
    EXPECT_NE(error::kNoError, ExecuteCmd(cmd));
    cmd.Init(client_program_id_, shared_memory_id_, kInvalidSharedMemoryOffset, Result::ComputeSize(1).ValueOrDie());
    EXPECT_NE(error::kNoError, ExecuteCmd(cmd));
}
...
TEST_P(GLES2DecoderManualInitTest, GetShaderPrecisionFormatSucceeds)
{
    ...
    auto* result = static_cast<cmds::GetShaderPrecisionFormat::Result*>(shared_memory_address_);
    result->success = 0;
    const GLint range[2] = { 62, 62 };
    const GLint precision = 16;
    EXPECT_CALL(*gl_, GetShaderPrecisionFormat(_, _, _, _))
        .WillOnce(DoAll(SetArrayArgument<2>(range, range + 2), SetArgPointee<3>(precision)))
        .RetiresOnSaturation();
    cmd.Init(GL_VERTEX_SHADER, GL_HIGH_FLOAT, shared_memory_id_, shared_memory_offset_);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
...
TEST_P(GLES2DecoderWithShaderTest, GetShaderPrecisionFormatBadArgsFails)
{
    ...
    result->success = 0;
    cmds::GetShaderPrecisionFormat cmd;
    cmd.Init(GL_TEXTURE_2D, GL_HIGH_FLOAT, shared_memory_id_, shared_memory_offset_);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(GL_INVALID_ENUM, GetGLError());
    result->success = 0;
    cmd.Init(GL_VERTEX_SHADER, GL_TEXTURE_2D, shared_memory_id_, shared_memory_offset_);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(GL_INVALID_ENUM, GetGLError());
}

TEST_P(GLES2DecoderWithShaderTest, GetShaderPrecisionFormatBadSharedMemoryFails)
{
    cmds::GetShaderPrecisionFormat cmd;
    cmd.Init(GL_VERTEX_SHADER, GL_HIGH_FLOAT, kInvalidSharedMemoryId, shared_memory_offset_);
    EXPECT_NE(error::kNoError, ExecuteCmd(cmd));
    cmd.Init(GL_VERTEX_SHADER, GL_TEXTURE_2D, shared_memory_id_, kInvalidSharedMemoryOffset);
    EXPECT_NE(error::kNoError, ExecuteCmd(cmd));
}
...
TEST_P(GLES2DecoderWithShaderTest, GetActiveUniformSucceeds)
{
    ...
    auto* result = static_cast<cmds::GetActiveUniform::Result*>(shared_memory_address_);
    result->success = 0;
    cmd.Init(client_program_id_, kUniformIndex, kBucketId, shared_memory_id_, shared_memory_offset_);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_NE(0, result->success);
    ...
    CommonDecoder::Bucket* bucket = decoder_->GetBucket(kBucketId);
    ASSERT_TRUE(bucket != nullptr);
    EXPECT_EQ(0, memcmp(bucket->GetData(0, bucket->size()), kUniform2Name, bucket->size()));
}
...
TEST_P(GLES2DecoderWithShaderTest, GetActiveUniformBadProgramFails)
{
    ...
    auto* result = static_cast<cmds::GetActiveUniform::Result*>(shared_memory_address_);
    result->success = 0;
    cmd.Init(kInvalidClientId, kUniformIndex, kBucketId, shared_memory_id_, shared_memory_offset_);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(0, result->success);
    EXPECT_EQ(GL_INVALID_VALUE, GetGLError());
#if GLES2_TEST_SHADER_VS_PROGRAM_IDS
    result->success = 0;
    cmd.Init(client_shader_id_, kUniformIndex, kBucketId, shared_memory_id_, shared_memory_offset_);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(0, result->success);
    EXPECT_EQ(GL_INVALID_OPERATION, GetGLError());
#endif // GLES2_TEST_SHADER_VS_PROGRAM_IDS
...
TEST_P(GLES2DecoderWithShaderTest, GetActiveUniformBadIndexFails)
{
    ...
    auto* result = static_cast<cmds::GetActiveUniform::Result*>(shared_memory_address_);
    result->success = 0;
    cmd.Init(client_program_id_, kBadUniformIndex, kBucketId, shared_memory_id_, shared_memory_offset_);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(0, result->success);
    EXPECT_EQ(GL_INVALID_VALUE, GetGLError());
}
...
TEST_P(GLES2DecoderWithShaderTest, GetActiveUniformBadSharedMemoryFails)
{
    ...
    const uint32_t kBucketId = 123;
    cmds::GetActiveUniform cmd;
    cmd.Init(client_program_id_, kUniformIndex, kBucketId, kInvalidSharedMemoryId, shared_memory_offset_);
    EXPECT_NE(error::kNoError, ExecuteCmd(cmd));
    cmd.Init(client_program_id_, kUniformIndex, kBucketId, shared_memory_id_, kInvalidSharedMemoryOffset);
    EXPECT_NE(error::kNoError, ExecuteCmd(cmd));
}
...
TEST_P(GLES3DecoderWithShaderTest, GetActiveUniformBlockNameSucceeds)
{
    ...
    auto* result = static_cast<cmds::GetActiveUniformBlockName::Result*>(shared_memory_address_);
    *result = 0;
    cmd.Init(client_program_id_, 0, kBucketId, shared_memory_id_, shared_memory_offset_);
    EXPECT_CALL(*gl_, GetProgramiv(kServiceProgramId, GL_LINK_STATUS, _)).WillOnce(SetArgPointee<2>(GL_TRUE)).RetiresOnSaturation();
    const char kName[] = "HolyCow";
    const GLsizei kMaxLength = strlen(kName) + 1;
    EXPECT_CALL(*gl_, GetProgramiv(kServiceProgramId, GL_ACTIVE_UNIFORM_BLOCK_MAX_NAME_LENGTH, _)).WillOnce(SetArgPointee<2>(kMaxLength)).RetiresOnSaturation();
    EXPECT_CALL(*gl_, GetActiveUniformBlockName(kServiceProgramId, 0, _, _, _))
        .WillOnce(DoAll(SetArgPointee<3>(strlen(kName)), SetArrayArgument<4>(kName, kName + strlen(kName) + 1)))
        .RetiresOnSaturation();
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_NE(0, *result);
    EXPECT_EQ(GL_NO_ERROR, GetGLError());
    CommonDecoder::Bucket* bucket = decoder_->GetBucket(kBucketId);
    ASSERT_TRUE(bucket != nullptr);
    EXPECT_EQ(0, memcmp(bucket->GetData(0, bucket->size()), kName, bucket->size()));
}
...
TEST_P(GLES3DecoderWithShaderTest, GetActiveUniformBlockNameUnlinkedProgram)
{
    ...
    *result = 0;
    cmd.Init(client_program_id_, 0, kBucketId, shared_memory_id_, shared_memory_offset_);
    EXPECT_CALL(*gl_, GetProgramiv(kServiceProgramId, GL_LINK_STATUS, _)).WillOnce(SetArgPointee<2>(GL_FALSE)).RetiresOnSaturation();
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(0, *result);
    EXPECT_EQ(GL_INVALID_OPERATION, GetGLError());
}

TEST_P(GLES3DecoderWithShaderTest, GetActiveUniformBlockNameResultNotInitFails)
{
    const uint32_t kBucketId = 123;
    cmds::GetActiveUniformBlockName cmd;
    auto* result = static_cast<cmds::GetActiveUniformBlockName::Result*>(shared_memory_address_);
...
TEST_P(GLES3DecoderWithShaderTest, GetActiveUniformBlockNameBadProgramFails)
{
    ...
    auto* result = static_cast<cmds::GetActiveUniformBlockName::Result*>(shared_memory_address_);
    *result = 0;
    cmd.Init(kInvalidClientId, 0, kBucketId, shared_memory_id_, shared_memory_offset_);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(0, *result);
    EXPECT_EQ(GL_INVALID_VALUE, GetGLError());
}

TEST_P(GLES3DecoderWithShaderTest, GetActiveUniformBlockNameBadSharedMemoryFails)
{
    const uint32_t kBucketId = 123;
    cmds::GetActiveUniformBlockName cmd;
    cmd.Init(client_program_id_, 0, kBucketId, kInvalidSharedMemoryId, shared_memory_offset_);
    EXPECT_NE(error::kNoError, ExecuteCmd(cmd));
    cmd.Init(client_program_id_, 0, kBucketId, shared_memory_id_, kInvalidSharedMemoryOffset);
    EXPECT_NE(error::kNoError, ExecuteCmd(cmd));
}
...
TEST_P(GLES3DecoderWithShaderTest, GetActiveUniformBlockivSucceeds)
{
    ...
    auto* result = static_cast<cmds::GetActiveUniformBlockiv::Result*>(shared_memory_address_);
    GLenum kPname[] {
    ...
    for (size_t ii = 0; ii < std::size(kPname); ++ii) {
        ...
        cmd.Init(client_program_id_, 0, kPname[ii], shared_memory_id_, shared_memory_offset_);
        EXPECT_CALL(*gl_, GetProgramiv(kServiceProgramId, GL_LINK_STATUS, _)).WillOnce(SetArgPointee<2>(GL_TRUE)).RetiresOnSaturation();
        if (kPname[ii] == GL_UNIFORM_BLOCK_ACTIVE_UNIFORM_INDICES) {
            ...
            EXPECT_CALL(*gl_, GetActiveUniformBlockiv(kServiceProgramId, 0, GL_UNIFORM_BLOCK_ACTIVE_UNIFORMS, _))
                .WillOnce(SetArgPointee<3>(1))
                .RetiresOnSaturation();
        }
        EXPECT_CALL(*gl_, GetActiveUniformBlockiv(kServiceProgramId, 0, kPname[ii], _)).WillOnce(SetArgPointee<3>(1976)).RetiresOnSaturation();
        EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
        EXPECT_EQ(1, result->GetNumResults());
        EXPECT_EQ(GL_NO_ERROR, GetGLError());
...
TEST_P(GLES3DecoderWithShaderTest, GetActiveUniformBlockivSucceedsZeroUniforms)
{
    cmds::GetActiveUniformBlockiv cmd;
    auto* result = static_cast<cmds::GetActiveUniformBlockiv::Result*>(shared_memory_address_);
    result->SetNumResults(0);
    cmd.Init(client_program_id_, 0, GL_UNIFORM_BLOCK_ACTIVE_UNIFORM_INDICES, shared_memory_id_, shared_memory_offset_);
    EXPECT_CALL(*gl_, GetError()).WillOnce(Return(GL_NO_ERROR)).RetiresOnSaturation();
    EXPECT_CALL(*gl_, GetProgramiv(kServiceProgramId, GL_LINK_STATUS, _)).WillOnce(SetArgPointee<2>(GL_TRUE)).RetiresOnSaturation();
    EXPECT_CALL(*gl_, GetActiveUniformBlockiv(kServiceProgramId, 0, GL_UNIFORM_BLOCK_ACTIVE_UNIFORMS, _)).WillOnce(SetArgPointee<3>(0)).RetiresOnSaturation();
    EXPECT_CALL(*gl_, GetActiveUniformBlockiv(kServiceProgramId, 0, GL_UNIFORM_BLOCK_ACTIVE_UNIFORM_INDICES, _)).Times(1).RetiresOnSaturation();
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
...
TEST_P(GLES3DecoderWithShaderTest, GetActiveUniformBlockivUnlinkedProgram)
{
    ...
    auto* result = static_cast<cmds::GetActiveUniformBlockiv::Result*>(shared_memory_address_);
    result->SetNumResults(0);
    cmd.Init(client_program_id_, 0, GL_UNIFORM_BLOCK_BINDING, shared_memory_id_, shared_memory_offset_);
    EXPECT_CALL(*gl_, GetProgramiv(kServiceProgramId, GL_LINK_STATUS, _)).WillOnce(SetArgPointee<2>(GL_FALSE)).RetiresOnSaturation();
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(0, result->GetNumResults());
    EXPECT_EQ(GL_INVALID_OPERATION, GetGLError());
...
TEST_P(GLES3DecoderWithShaderTest, GetActiveUniformBlockivResultNotInitFails)
{
    cmds::GetActiveUniformBlockiv cmd;
    auto* result = static_cast<cmds::GetActiveUniformBlockiv::Result*>(shared_memory_address_);
...
TEST_P(GLES3DecoderWithShaderTest, GetActiveUniformBlockivBadProgramFails)
{
    ...
    auto* result = static_cast<cmds::GetActiveUniformBlockiv::Result*>(shared_memory_address_);
    result->SetNumResults(0);
    cmd.Init(kInvalidClientId, 0, GL_UNIFORM_BLOCK_BINDING, shared_memory_id_, shared_memory_offset_);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(0, result->GetNumResults());
    EXPECT_EQ(GL_INVALID_VALUE, GetGLError());
}
...
TEST_P(GLES3DecoderWithShaderTest, GetActiveUniformBlockivBadSharedMemoryFails)
{
    ...
    EXPECT_CALL(*gl_, GetProgramiv(kServiceProgramId, GL_LINK_STATUS, _))
        ...
        .WillOnce(SetArgPointee<2>(GL_TRUE))
        .RetiresOnSaturation();
    cmd.Init(client_program_id_, 0, GL_UNIFORM_BLOCK_BINDING, kInvalidSharedMemoryId, shared_memory_offset_);
    EXPECT_NE(error::kNoError, ExecuteCmd(cmd));
    cmd.Init(client_program_id_, 0, GL_UNIFORM_BLOCK_BINDING, shared_memory_id_, kInvalidSharedMemoryOffset);
    EXPECT_NE(error::kNoError, ExecuteCmd(cmd));
}
...
TEST_P(GLES2DecoderWithShaderTest, GetActiveAttribSucceeds)
{
    ...
    auto* result = static_cast<cmds::GetActiveAttrib::Result*>(shared_memory_address_);
    result->success = 0;
    cmd.Init(client_program_id_, kAttribIndex, kBucketId, shared_memory_id_, shared_memory_offset_);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_NE(0, result->success);
    ...
    CommonDecoder::Bucket* bucket = decoder_->GetBucket(kBucketId);
    ASSERT_TRUE(bucket != nullptr);
    EXPECT_EQ(0, memcmp(bucket->GetData(0, bucket->size()), kAttrib2Name, bucket->size()));
}
...
TEST_P(GLES2DecoderWithShaderTest, GetActiveAttribBadProgramFails)
{
    ...
    auto* result = static_cast<cmds::GetActiveAttrib::Result*>(shared_memory_address_);
    result->success = 0;
    cmd.Init(kInvalidClientId, kAttribIndex, kBucketId, shared_memory_id_, shared_memory_offset_);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(0, result->success);
    EXPECT_EQ(GL_INVALID_VALUE, GetGLError());
#if GLES2_TEST_SHADER_VS_PROGRAM_IDS
    result->success = 0;
    cmd.Init(client_shader_id_, kAttribIndex, kBucketId, shared_memory_id_, shared_memory_offset_);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(0, result->success);
    EXPECT_EQ(GL_INVALID_OPERATION, GetGLError());
#endif // GLES2_TEST_SHADER_VS_PROGRAM_IDS
...
TEST_P(GLES2DecoderWithShaderTest, GetActiveAttribBadIndexFails)
{
    ...
    auto* result = static_cast<cmds::GetActiveAttrib::Result*>(shared_memory_address_);
    result->success = 0;
    cmd.Init(client_program_id_, kBadAttribIndex, kBucketId, shared_memory_id_, shared_memory_offset_);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(0, result->success);
    EXPECT_EQ(GL_INVALID_VALUE, GetGLError());
}
...
TEST_P(GLES2DecoderWithShaderTest, GetActiveAttribBadSharedMemoryFails)
{
    ...
    const uint32_t kBucketId = 123;
    cmds::GetActiveAttrib cmd;
    cmd.Init(client_program_id_, kAttribIndex, kBucketId, kInvalidSharedMemoryId, shared_memory_offset_);
    EXPECT_NE(error::kNoError, ExecuteCmd(cmd));
    cmd.Init(client_program_id_, kAttribIndex, kBucketId, shared_memory_id_, kInvalidSharedMemoryOffset);
    EXPECT_NE(error::kNoError, ExecuteCmd(cmd));
}
...
TEST_P(GLES3DecoderWithShaderTest, GetUniformIndicesSucceeds)
{
    ...
    auto* result = static_cast<cmds::GetUniformIndices::Result*>(shared_memory_address_);
    cmds::GetUniformIndices cmd;
    cmd.Init(client_program_id_, kBucketId, shared_memory_id_, kSharedMemoryOffset);
    EXPECT_CALL(*gl_, GetUniformIndices(kServiceProgramId, kCount, _, _)).WillOnce(SetArrayArgument<3>(kIndices, kIndices + kCount)).RetiresOnSaturation();
    EXPECT_CALL(*gl_, GetProgramiv(kServiceProgramId, GL_LINK_STATUS, _)).WillOnce(SetArgPointee<2>(GL_TRUE)).RetiresOnSaturation();
...
TEST_P(GLES3DecoderWithShaderTest, GetUniformIndicesBadParamsFails)
{
    ...
    EXPECT_CALL(*gl_, GetUniformIndices(kServiceProgramId, kCount, _, _)).WillOnce(SetArrayArgument<3>(kIndices, kIndices + kCount)).RetiresOnSaturation();
    EXPECT_CALL(*gl_, GetProgramiv(kServiceProgramId, GL_LINK_STATUS, _)).WillOnce(SetArgPointee<2>(GL_TRUE)).RetiresOnSaturation();
    EXPECT_CALL(*gl_, GetError()).WillOnce(Return(GL_NO_ERROR)).WillOnce(Return(GL_INVALID_VALUE)).RetiresOnSaturation();
    result->size = 0;
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(0, result->GetNumResults());
    EXPECT_EQ(GL_INVALID_VALUE, GetGLError());
...
TEST_P(GLES3DecoderWithShaderTest, GetUniformIndicesBadSharedMemoryFails)
{
    ...
    auto* result = static_cast<cmds::GetUniformIndices::Result*>(shared_memory_address_);
    cmds::GetUniformIndices cmd;
    cmd.Init(client_program_id_, kBucketId, kInvalidSharedMemoryId, kSharedMemoryOffset);
    result->size = 0;
    EXPECT_NE(error::kNoError, ExecuteCmd(cmd));
    cmd.Init(client_program_id_, kBucketId, shared_memory_id_, kInvalidSharedMemoryOffset);
    result->size = 0;
    EXPECT_NE(error::kNoError, ExecuteCmd(cmd));
}
...
TEST_P(GLES3DecoderWithShaderTest, GetActiveUniformsivSucceeds)
{
    ...
    auto* result = static_cast<cmds::GetActiveUniformsiv::Result*>(shared_memory_address_);
    cmds::GetActiveUniformsiv cmd;
    cmd.Init(client_program_id_, kBucketId, GL_UNIFORM_TYPE, shared_memory_id_, kSharedMemoryOffset);
    EXPECT_CALL(*gl_, GetActiveUniformsiv(kServiceProgramId, kCount, _, GL_UNIFORM_TYPE, _))
...
TEST_P(GLES3DecoderWithShaderTest, GetActiveUniformsivBadProgramFails)
{
    ...
    cmds::GetActiveUniformsiv cmd;
    // None-existant program
    cmd.Init(kInvalidClientId, kBucketId, GL_UNIFORM_TYPE, shared_memory_id_, kSharedMemoryOffset);
    result->size = 0;
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(0, result->GetNumResults());
    EXPECT_EQ(GL_INVALID_VALUE, GetGLError());
...
TEST_P(GLES3DecoderWithShaderTest, GetActiveUniformsivBadPnameFails)
{
    ...
    cmd.Init(client_program_id_, kBucketId, GL_UNIFORM_BLOCK_NAME_LENGTH, shared_memory_id_, kSharedMemoryOffset);
    result->size = 0;
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(0, result->GetNumResults());
    EXPECT_EQ(GL_INVALID_ENUM, GetGLError());
    // Invalid pname
    cmd.Init(client_program_id_, kBucketId, 1, shared_memory_id_, kSharedMemoryOffset);
    result->size = 0;
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(0, result->GetNumResults());
    EXPECT_EQ(GL_INVALID_ENUM, GetGLError());
...
TEST_P(GLES3DecoderWithShaderTest, GetActiveUniformsivBadSharedMemoryFails)
{
    ...
    cmds::GetActiveUniformsiv cmd;
    result->size = 0;
    cmd.Init(client_program_id_, kBucketId, GL_UNIFORM_TYPE, kInvalidSharedMemoryId, kSharedMemoryOffset);
    EXPECT_NE(error::kNoError, ExecuteCmd(cmd));
    result->size = 0;
    cmd.Init(client_program_id_, kBucketId, GL_UNIFORM_TYPE, shared_memory_id_, kInvalidSharedMemoryOffset);
    EXPECT_NE(error::kNoError, ExecuteCmd(cmd));
}
...
TEST_P(GLES2DecoderWithShaderTest, GetShaderInfoLogValidArgs)
{
    ...
    EXPECT_CALL(*gl_, GetShaderiv(kServiceShaderId, GL_COMPILE_STATUS, _)).WillOnce(SetArgPointee<2>(GL_FALSE)).RetiresOnSaturation();
    EXPECT_CALL(*gl_, GetShaderiv(kServiceShaderId, GL_INFO_LOG_LENGTH, _)).WillOnce(SetArgPointee<2>(strlen(kInfo) + 1)).RetiresOnSaturation();
    EXPECT_CALL(*gl_, GetShaderInfoLog(kServiceShaderId, strlen(kInfo) + 1, _, _))
        .WillOnce(DoAll(SetArgPointee<2>(strlen(kInfo)), SetArrayArgument<3>(kInfo, kInfo + strlen(kInfo) + 1)));
    ...
    EXPECT_EQ(0, memcmp(bucket->GetData(0, bucket->size()), kInfo, bucket->size()));
    EXPECT_EQ(GL_NO_ERROR, GetGLError());
}
...
TEST_P(GLES3DecoderWithShaderTest, GetTransformFeedbackVaryingSucceeds)
{
    ...
    auto* result = static_cast<cmds::GetTransformFeedbackVarying::Result*>(shared_memory_address_);
    result->success = 0;
    EXPECT_CALL(*gl_, GetProgramiv(kServiceProgramId, GL_LINK_STATUS, _)).WillOnce(SetArgPointee<2>(GL_TRUE)).RetiresOnSaturation();
    EXPECT_CALL(*gl_, GetProgramiv(kServiceProgramId, GL_TRANSFORM_FEEDBACK_VARYINGS, _)).WillOnce(SetArgPointee<2>(kNumVaryings)).RetiresOnSaturation();
    EXPECT_CALL(*gl_, GetProgramiv(kServiceProgramId, GL_TRANSFORM_FEEDBACK_VARYING_MAX_LENGTH, _))
        .WillOnce(SetArgPointee<2>(kBufferSize))
        .RetiresOnSaturation();
    EXPECT_CALL(*gl_, GetTransformFeedbackVarying(kServiceProgramId, kIndex, _, _, _, _, _))
        .WillOnce(DoAll(SetArgPointee<3>(kBufferSize - 1), SetArgPointee<4>(kSize), SetArgPointee<5>(kType), SetArrayArgument<6>(kName, kName + kBufferSize)))
        .RetiresOnSaturation();
    cmd.Init(client_program_id_, kIndex, kBucketId, shared_memory_id_, shared_memory_offset_);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_NE(0, result->success);
...
TEST_P(GLES3DecoderWithShaderTest, GetTransformFeedbackVaryingBadProgramFails)
{
    ...
    auto* result = static_cast<cmds::GetTransformFeedbackVarying::Result*>(shared_memory_address_);
    result->success = 0;
    cmd.Init(kInvalidClientId, kIndex, kBucketId, shared_memory_id_, shared_memory_offset_);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(0, result->success);
    EXPECT_EQ(GL_INVALID_VALUE, GetGLError());
}
...
TEST_P(GLES3DecoderWithShaderTest, GetTransformFeedbackVaryingBadSharedMemoryFails)
{
    const GLuint kIndex = 1;
    const uint32_t kBucketId = 123;
    cmds::GetTransformFeedbackVarying cmd;
    auto* result = static_cast<cmds::GetTransformFeedbackVarying::Result*>(shared_memory_address_);
    result->success = 0;
    cmd.Init(client_program_id_, kIndex, kBucketId, kInvalidSharedMemoryId, shared_memory_offset_);
    EXPECT_NE(error::kNoError, ExecuteCmd(cmd));
    cmd.Init(client_program_id_, kIndex, kBucketId, shared_memory_id_, kInvalidSharedMemoryOffset);
    EXPECT_NE(error::kNoError, ExecuteCmd(cmd));
}
...
TEST_P(GLES2DecoderTest, CompileShaderValidArgs)
{
    ...
    EXPECT_CALL(*gl_, ShaderSource(kServiceShaderId, 1, _, _));
    EXPECT_CALL(*gl_, CompileShader(kServiceShaderId));
    EXPECT_CALL(*gl_, GetShaderiv(kServiceShaderId, GL_COMPILE_STATUS, _)).WillOnce(SetArgPointee<2>(GL_TRUE)).RetiresOnSaturation();
    EXPECT_CALL(*gl_, GetError()).WillOnce(Return(GL_NO_ERROR)).WillOnce(Return(GL_NO_ERROR)).RetiresOnSaturation();

    auto* result = static_cast<cmds::GetShaderiv::Result*>(shared_memory_address_);
    result->size = 0;
    cmds::GetShaderiv status_cmd;
    status_cmd.Init(client_shader_id_, GL_COMPILE_STATUS, shared_memory_id_, kSharedMemoryOffset);
...
TEST_P(GLES2DecoderTest, ShaderSourceBucketAndGetShaderSourceValidArgs)
{
    ...
    EXPECT_EQ(0, memcmp(bucket->GetData(0, bucket->size()), kSource0, bucket->size()));
}
...
TEST_P(GLES2DecoderWithShaderTest, Uniform1ivImmediateValidArgs)
{
    ...
    EXPECT_CALL(*gl_, Uniform1iv(kUniform1RealLocation, 1, PointsToArray(temp, 1)));
...
TEST_P(GLES2DecoderWithShaderTest, Uniform1ivArray)
{
    ...
    EXPECT_CALL(*gl_, Uniform1iv(kUniform8RealLocation, 2, PointsToArray(temp, 2)));
...
TEST_P(GLES3DecoderWithShaderTest, Uniform1uivImmediateValidArgs)
{
    ...
    EXPECT_CALL(*gl_, Uniform1uiv(kUniform4RealLocation, 1, PointsToArray(temp, 1)));
...
TEST_P(GLES3DecoderWithShaderTest, Uniform2uivImmediateValidArgs)
{
    ...
    EXPECT_CALL(*gl_, Uniform2uiv(kUniform5RealLocation, 1, PointsToArray(temp, 2)));
...
TEST_P(GLES3DecoderWithShaderTest, Uniform3uivImmediateValidArgs)
{
    ...
    EXPECT_CALL(*gl_, Uniform3uiv(kUniform6RealLocation, 1, PointsToArray(temp, 3)));
...
TEST_P(GLES3DecoderWithShaderTest, Uniform4uivImmediateValidArgs)
{
    ...
    EXPECT_CALL(*gl_, Uniform4uiv(kUniform7RealLocation, 1, PointsToArray(temp, 4)));
...
TEST_P(GLES3DecoderWithShaderTest, GetUniformBlockIndex)
{
    ...
    *result = GL_INVALID_INDEX;
    ...
    cmd.Init(client_program_id_, kBucketId, shared_memory_id_, kSharedMemoryOffset);
    EXPECT_CALL(*gl_, GetUniformBlockIndex(kServiceProgramId, StrEq(kName))).WillOnce(Return(kIndex)).RetiresOnSaturation();
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
...
TEST_P(GLES2DecoderWithShaderTest, BindUniformLocationCHROMIUMBucket)
{
    ...
    SetBucketAsCString(kBucketId, kName);
    GLint kMaxLocation = (kMaxFragmentUniformVectors + kMaxVertexUniformVectors) * 4 - 1;
...
TEST_P(GLES2DecoderWithShaderTest, DetachDeletedShader)
{
    ...
    {
        EXPECT_CALL(*gl_, DetachShader(kServiceProgramId, kServiceFragmentShaderId)).Times(1).RetiresOnSaturation();
