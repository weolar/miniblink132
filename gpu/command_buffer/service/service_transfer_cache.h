...
class GPU_GLES2_EXPORT ServiceTransferCache : public base::trace_event::MemoryDumpProvider {
public:
    struct GPU_GLES2_EXPORT EntryKey {
        EntryKey(int decoder_id, cc::TransferCacheEntryType entry_type, uint32_t entry_id);
        int decoder_id;
        cc::TransferCacheEntryType entry_type;
        uint32_t entry_id;
    };

    ServiceTransferCache(const GpuPreferences& preferences, base::RepeatingClosure flush_callback);
    ...
    void CreateLocalEntry(const EntryKey& key, std::unique_ptr<cc::ServiceTransferCacheEntry> entry);
    ...
    // false otherwise.
    bool CreateLockedHardwareDecodedImageEntry(int decoder_id, uint32_t entry_id, ServiceDiscardableHandle handle, GrDirectContext* context,
    ...
    size_t cache_size_for_testing() const
    {
        return total_size_;
    }
    size_t entries_count_for_testing() const
    {
...
private:
    ...
    struct CacheEntryInternal {
        CacheEntryInternal(std::optional<ServiceDiscardableHandle> handle, std::unique_ptr<cc::ServiceTransferCacheEntry> entry);
    ...
    int RemoveOldEntriesUntil(base::FunctionRef<bool(EntryCache::reverse_iterator)> should_stop);

    template <typename Iterator> Iterator ForceDeleteEntry(Iterator it);
