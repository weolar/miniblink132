...
public:
    GLES2ExternalFramebuffer(bool passthrough, const FeatureInfo& feature_info, SharedImageRepresentationFactory* shared_image_representation_factory);
    ...
    bool AttachSharedImage(const Mailbox& mailbox, int samples_count, bool preserve, bool need_depth, bool need_stencil);
...
private:
    ...
    bool UpdateAttachment(GLenum attachment, const gfx::Size& size, int samples, GLenum format);
    std::unique_ptr<Attachment> CreateAttachment(GLenum attachment, const gfx::Size& size, int samples, GLenum format);
    ...
    const raw_ptr<SharedImageRepresentationFactory> shared_image_representation_factory_;

    std::unique_ptr<GLTextureImageRepresentationBase> shared_image_representation_;
    std::unique_ptr<GLTextureImageRepresentationBase::ScopedAccess> scoped_access_;
};
