...
class GPU_EXPORT SyncPointOrderData : public base::RefCountedThreadSafe<SyncPointOrderData> {
public:
...
private:
    ...
    struct OrderFence {
        ...
        OrderFence(uint32_t order, uint64_t release, scoped_refptr<SyncPointClientState> state, uint64_t callback_id);
        OrderFence(const OrderFence& other);
        ~OrderFence();

        bool operator>(const OrderFence& rhs) const
        {
            return std::tie(order_num, fence_release) > std::tie(rhs.order_num, rhs.fence_release);
        }
    };
    typedef std::priority_queue<OrderFence, std::vector<OrderFence>, std::greater<>> OrderFenceQueue;

    SyncPointOrderData(SyncPointManager* sync_point_manager, SequenceId seqeunce_id);
    ...
    uint64_t ValidateReleaseOrderNumber(scoped_refptr<SyncPointClientState> client_state, uint32_t wait_order_num, uint64_t fence_release)
...
class GPU_EXPORT SyncPointClientState : public base::RefCountedThreadSafe<SyncPointClientState> {
public:
    ...
    CommandBufferId command_buffer_id() const
    {
        return command_buffer_id_;
    }
    SequenceId sequence_id() const
    {
        return order_data_->sequence_id();
    }
    ...
    bool Wait(const SyncToken& sync_token, base::OnceClosure callback) LOCKS_EXCLUDED(fence_sync_lock_);
    ...
    bool WaitNonThreadSafe(const SyncToken& sync_token, scoped_refptr<base::SingleThreadTaskRunner> task_runner, base::OnceClosure callback)
...
private:
    ...
    struct ReleaseCallback {
        ...
        ReleaseCallback(uint64_t release, base::OnceClosure callback, uint64_t callback_id);
    ...
    typedef std::priority_queue<ReleaseCallback, std::vector<ReleaseCallback>, std::greater<>> ReleaseCallbackQueue;

    SyncPointClientState(SyncPointManager* sync_point_manager, scoped_refptr<SyncPointOrderData> order_data, CommandBufferNamespace namespace_id,
        CommandBufferId command_buffer_id);

    ~SyncPointClientState();

    std::vector<base::OnceClosure> DestroyAndReturnCallbacks() LOCKS_EXCLUDED(fence_sync_lock_);
    ...
    // be called.
    bool WaitForRelease(uint64_t release, uint32_t wait_order_num, base::OnceClosure callback) LOCKS_EXCLUDED(fence_sync_lock_);

    // Does not release the fence sync, but releases callbacks waiting on that
    // fence sync.
    void EnsureWaitReleased(uint64_t release, uint64_t callback_id) LOCKS_EXCLUDED(fence_sync_lock_);

    void EnsureFenceSyncReleased(uint64_t release, ReleaseCause cause) LOCKS_EXCLUDED(fence_sync_lock_);
...
public:
    ...
    SequenceId GetSyncTokenReleaseSequenceId(const SyncToken& sync_token) LOCKS_EXCLUDED(lock_);
    ...
    bool Wait(const SyncToken& sync_token, SequenceId sequence_id, uint32_t wait_order_num, base::OnceClosure callback) LOCKS_EXCLUDED(lock_);

    // Like Wait but runs the callback on the given task runner's thread.
    // TODO(elgarawany): Rename this method to instead make it explicit that the
    // callback is going to run on |task_runner|.
    bool WaitNonThreadSafe(const SyncToken& sync_token, SequenceId sequence_id, uint32_t wait_order_num,
        scoped_refptr<base::SingleThreadTaskRunner> task_runner, base::OnceClosure callback) LOCKS_EXCLUDED(lock_);
    ...
    // client_state_map_.
    void RemoveSyncPointOrderData(scoped_refptr<SyncPointOrderData> order_data) LOCKS_EXCLUDED(lock_);

    // Grabs any remaining callbacks in |client_state|'s release queue, destroys
    // |client_state|, then runs those remaining callbacks.
    void DestroySyncPointClientState(scoped_refptr<SyncPointClientState> client_state) LOCKS_EXCLUDED(lock_, client_state->fence_sync_lock_);

    // Ensures release count reaches `release`.
    void EnsureFenceSyncReleased(const SyncToken& release, ReleaseCause cause) LOCKS_EXCLUDED(lock_);
    ...
    {
        return graph_validation_enabled_;
    }
...
private:
    using ClientStateMap = base::flat_map<CommandBufferId, scoped_refptr<SyncPointClientState>>;

    using OrderDataMap = base::flat_map<SequenceId, scoped_refptr<SyncPointOrderData>>;

    scoped_refptr<SyncPointOrderData> GetSyncPointOrderData(SequenceId sequence_id) EXCLUSIVE_LOCKS_REQUIRED(lock_);

    scoped_refptr<SyncPointClientState> GetSyncPointClientState(CommandBufferNamespace namespace_id, CommandBufferId command_buffer_id)
        EXCLUSIVE_LOCKS_REQUIRED(lock_);

    // Internal version of GetSyncTokenReleaseSequenceId that requires lock to be
    // acquired.
    SequenceId GetSyncTokenReleaseSequenceIdInternal(const SyncToken& sync_token) EXCLUSIVE_LOCKS_REQUIRED(lock_);
    ...
    ClientStateMap client_state_maps_[NUM_COMMAND_BUFFER_NAMESPACES] GUARDED_BY(lock_);
