...
public:
    explicit DawnPlatform(std::unique_ptr<DawnCachingInterface> dawn_caching_interface, const char* uma_prefix, bool record_cache_count_uma);
    ...
    uint64_t AddTraceEvent(char phase, const unsigned char* category_group_enabled, const char* name, uint64_t id, double timestamp, int num_args,
        const char** arg_names, const unsigned char* arg_types, const uint64_t* arg_values, unsigned char flags) override;

    void HistogramCustomCounts(const char* name, int sample, int min, int max, int bucketCount) override;

    void HistogramCustomCountsHPC(const char* name, int sample, int min, int max, int bucketCount) override;
...
private:
    void HistogramCacheCountHelper(std::string name, int sample, int min, int max, int bucketCount);
