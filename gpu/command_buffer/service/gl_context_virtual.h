...
public:
    GLContextVirtual(gl::GLShareGroup* share_group, gl::GLContext* shared_context, base::WeakPtr<GLContextVirtualDelegate> delegate);

    GLContextVirtual(const GLContextVirtual&) = delete;
    GLContextVirtual& operator=(const GLContextVirtual&) = delete;

    // Implement GLContext.
    bool InitializeImpl(gl::GLSurface* compatible_surface, const gl::GLContextAttribs& attribs) override;
    ...
    void AddMetalSharedEventsForBackpressure(std::vector<std::unique_ptr<BackpressureMetalSharedEvent>> events) override;
