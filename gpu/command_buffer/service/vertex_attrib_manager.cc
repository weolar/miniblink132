...
void VertexAttrib::SetInfo(
    Buffer* buffer, GLint size, GLenum type, GLboolean normalized, GLsizei gl_stride, GLsizei real_stride, GLsizei offset, GLboolean integer)
{
...
bool VertexAttrib::CanAccess(GLuint index) const
{
    ...
    GLuint num_elements = usable_size / real_stride_ + ((usable_size % real_stride_) >= (GLES2Util::GetGroupSizeForBufferType(size_, type_)) ? 1 : 0);
...
VertexAttribManager::VertexAttribManager(VertexArrayManager* manager, GLuint service_id, uint32_t num_vertex_attribs, bool do_buffer_refcounting)
    : num_fixed_attribs_(0)
    , element_array_buffer_(nullptr)
    , manager_(manager)
    , deleted_(false)
        ...
        if (vertex_attribs_[vv].buffer_ == buffer) {
            ...
            if (manager_ && manager_->have_context_) {
                ...
                glVertexAttribPointer(
                    vv, vertex_attribs_[vv].size_, vertex_attribs_[vv].type_, vertex_attribs_[vv].normalized_, vertex_attribs_[vv].gl_stride_, 0);
            }
        }
    }
    if (new_buffer) {
        glDeleteBuffersARB(1, &new_buffer);
        glBindBuffer(GL_ARRAY_BUFFER, bound_array_buffer ? bound_array_buffer->service_id() : 0u);
    }
}
...
bool VertexAttribManager::ValidateBindings(const char* function_name, GLES2Decoder* decoder, FeatureInfo* feature_info, BufferManager* buffer_manager,
    Program* current_program, GLuint max_vertex_accessed, bool instanced, GLsizei primcount, GLint basevertex, GLuint baseinstance)
{
    DCHECK(primcount);
    ...
    bool use_client_side_arrays_for_stream_buffers = feature_info->workarounds().use_client_side_arrays_for_stream_buffers;
    ...
    // assigned.
    for (VertexAttribList::iterator it = enabled_vertex_attribs_.begin(); it != enabled_vertex_attribs_.end(); ++it) {
        VertexAttrib* attrib = *it;
        Buffer* buffer = attrib->buffer();
        if (!buffer_manager->RequestBufferAccess(error_state, buffer, function_name, "attached to enabled attrib %u", attrib->index())) {
            return false;
        }
        const Program::VertexAttrib* attrib_info = current_program->GetAttribInfoByLocation(attrib->index());
        ...
        if (attrib_info) {
            ...
            GLuint count = attrib->MaxVertexAccessed(primcount, max_vertex_accessed, basevertex, baseinstance);
            ...
            if (!attrib->CanAccess(count)) {
                ERRORSTATE_SET_GL_ERROR(error_state, GL_INVALID_OPERATION, function_name,
                    (std::string("attempt to access out of range vertices in attribute ") + base::NumberToString(attrib->index())).c_str());
                return false;
            }
            if (use_client_side_arrays_for_stream_buffers) {
                if (buffer->IsClientSideArray()) {
                    ...
                    DCHECK(ptr);
                    glVertexAttribPointer(attrib->index(), attrib->size(), attrib->type(), attrib->normalized(), attrib->gl_stride(), ptr);
    ...
    if (feature_info->IsWebGL1OrES2Context() && !divisor0 && (instanced || have_enabled_active_attribs)) {
        ERRORSTATE_SET_GL_ERROR(error_state, GL_INVALID_OPERATION, function_name, "attempt to draw with all attributes having non-zero divisors");
        return false;
    }
