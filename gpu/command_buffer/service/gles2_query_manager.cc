...
public:
    AbstractIntegerQuery(QueryManager* manager, GLenum target, scoped_refptr<gpu::Buffer> buffer, QuerySync* sync);
    void Begin() override;
...
AbstractIntegerQuery::AbstractIntegerQuery(QueryManager* manager, GLenum target, scoped_refptr<gpu::Buffer> buffer, QuerySync* sync)
...
{
    GLuint available = 0;
    glGetQueryObjectuiv(service_ids_.back(), GL_QUERY_RESULT_AVAILABLE_EXT, &available);
    return !!available;
}

class BooleanQuery : public AbstractIntegerQuery {
public:
    BooleanQuery(QueryManager* manager, GLenum target, scoped_refptr<gpu::Buffer> buffer, QuerySync* sync);
...
BooleanQuery::BooleanQuery(QueryManager* manager, GLenum target, scoped_refptr<gpu::Buffer> buffer, QuerySync* sync)
...
public:
    SummedIntegerQuery(QueryManager* manager, GLenum target, scoped_refptr<gpu::Buffer> buffer, QuerySync* sync);
    void Process(bool did_finish) override;
...
SummedIntegerQuery::SummedIntegerQuery(QueryManager* manager, GLenum target, scoped_refptr<gpu::Buffer> buffer, QuerySync* sync)
    : AbstractIntegerQuery(manager, target, std::move(buffer), sync)
{
}
...
class AsyncReadPixelsCompletedQuery final : public GLES2QueryManager::GLES2Query {
public:
    AsyncReadPixelsCompletedQuery(GLES2QueryManager* manager, GLenum target, scoped_refptr<gpu::Buffer> buffer, QuerySync* sync);
...
AsyncReadPixelsCompletedQuery::AsyncReadPixelsCompletedQuery(GLES2QueryManager* manager, GLenum target, scoped_refptr<gpu::Buffer> buffer, QuerySync* sync)
...
void AsyncReadPixelsCompletedQuery::End(base::subtle::Atomic32 submit_count)
{
    ...
    gles2_query_manager()->decoder()->WaitForReadPixels(base::BindOnce(&AsyncReadPixelsCompletedQuery::Complete, weak_ptr_factory_.GetWeakPtr()));
}

void AsyncReadPixelsCompletedQuery::QueryCounter(base::subtle::Atomic32 submit_count)
{
    NOTREACHED();
}
...
public:
    GetErrorQuery(GLES2QueryManager* manager, GLenum target, scoped_refptr<gpu::Buffer> buffer, QuerySync* sync);
...
GetErrorQuery::GetErrorQuery(GLES2QueryManager* manager, GLenum target, scoped_refptr<gpu::Buffer> buffer, QuerySync* sync)
    : GLES2Query(manager, target, std::move(buffer), sync)
{
}
...
{
    MarkAsPending(submit_count);
    MarkAsCompleted(gles2_query_manager()->decoder()->GetErrorState()->GetGLError());
}
...
public:
    TimeElapsedQuery(GLES2QueryManager* manager, GLenum target, scoped_refptr<gpu::Buffer> buffer, QuerySync* sync);
...
TimeElapsedQuery::TimeElapsedQuery(GLES2QueryManager* manager, GLenum target, scoped_refptr<gpu::Buffer> buffer, QuerySync* sync)
    : GLES2Query(manager, target, std::move(buffer), sync)
...
public:
    TimeStampQuery(GLES2QueryManager* manager, GLenum target, scoped_refptr<gpu::Buffer> buffer, QuerySync* sync);
...
TimeStampQuery::TimeStampQuery(GLES2QueryManager* manager, GLenum target, scoped_refptr<gpu::Buffer> buffer, QuerySync* sync)
    : GLES2Query(manager, target, std::move(buffer), sync)
...
GLES2QueryManager::GLES2Query::GLES2Query(GLES2QueryManager* manager, GLenum target, scoped_refptr<gpu::Buffer> buffer, QuerySync* sync)
...
GLES2QueryManager::GLES2QueryManager(GLES2Decoder* decoder, FeatureInfo* feature_info)
    : decoder_(decoder)
...
QueryManager::Query* GLES2QueryManager::CreateQuery(GLenum target, GLuint client_id, scoped_refptr<gpu::Buffer> buffer, QuerySync* sync)
{
    scoped_refptr<Query> query;
    switch (target) {
    case GL_ASYNC_PIXEL_PACK_COMPLETED_CHROMIUM:
        query = new AsyncReadPixelsCompletedQuery(this, target, std::move(buffer), sync);
        break;
...
error::Error GLES2QueryManager::SetDisjointSync(int32_t shm_id, uint32_t shm_offset)
{
    if (disjoint_notify_shm_id_ != -1 || shm_id == -1)
        return error::kInvalidArguments;

    DisjointValueSync* sync = decoder_->GetSharedMemoryAs<DisjointValueSync*>(shm_id, shm_offset, sizeof(*sync));
    if (!sync)
...
std::unique_ptr<gl::GPUTimer> GLES2QueryManager::CreateGPUTimer(bool elapsed_time)
{
...
void GLES2QueryManager::UpdateDisjointValue()
{
    if (disjoint_notify_shm_id_ != -1) {
        if (gpu_timing_client_->CheckAndResetTimerErrors()) {
            ...
            DisjointValueSync* sync = decoder_->GetSharedMemoryAs<DisjointValueSync*>(disjoint_notify_shm_id_, disjoint_notify_shm_offset_, sizeof(*sync));
            if (!sync) {
