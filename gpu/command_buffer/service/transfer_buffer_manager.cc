    ...
    , memory_tracker_(memory_tracker)
{
    ...
    if (memory_tracker_) {
        base::trace_event::MemoryDumpManager::GetInstance()->RegisterDumpProvider(
            this, "gpu::TransferBufferManager", base::SingleThreadTaskRunner::GetCurrentDefault());
    }
}
...
bool TransferBufferManager::RegisterTransferBuffer(int32_t id, scoped_refptr<Buffer> buffer)
{
    if (id <= 0) {
...
bool TransferBufferManager::OnMemoryDump(const base::trace_event::MemoryDumpArgs& args, base::trace_event::ProcessMemoryDump* pmd)
{
    using base::trace_event::MemoryAllocatorDump;
    using base::trace_event::MemoryDumpLevelOfDetail;

    if (args.level_of_detail == MemoryDumpLevelOfDetail::kBackground) {
        std::string dump_name = base::StringPrintf("gpu/transfer_memory/client_%d", memory_tracker_->ClientId());
        MemoryAllocatorDump* dump = pmd->CreateAllocatorDump(dump_name);
        dump->AddScalar(MemoryAllocatorDump::kNameSize, MemoryAllocatorDump::kUnitsBytes, shared_memory_bytes_allocated_);
    ...
    for (const auto& buffer_entry : registered_buffers_) {
        ...
        std::string dump_name = base::StringPrintf("gpu/transfer_memory/client_%d/buffer_%d", memory_tracker_->ClientId(), buffer_id);
        MemoryAllocatorDump* dump = pmd->CreateAllocatorDump(dump_name);
        dump->AddScalar(MemoryAllocatorDump::kNameSize, MemoryAllocatorDump::kUnitsBytes, buffer->size());
