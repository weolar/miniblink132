...
template <typename ClientType, typename ServiceType> class ClientServiceMap {
public:
    ClientServiceMap()
    ...
    void Clear()
    {
        ...
        client_to_service_array_.resize(kInitialFlatArraySize, invalid_service_id());
        client_to_service_map_.clear();
    }

    ALWAYS_INLINE bool GetServiceID(ClientType client_id, ServiceType* service_id) const
    {
        DCHECK(service_id);
        if (client_id >= kMaxFlatArraySize) {
        ...
        if (client_id < client_to_service_array_.size() && client_to_service_array_[client_id] != invalid_service_id()) {
    ...
    ALWAYS_INLINE bool HasClientID(ClientType client_id) const
    {
        ...
        return client_id == 0 || (client_id < client_to_service_array_.size() && client_to_service_array_[client_id] != invalid_service_id());
    }
    ...
    {
        DCHECK(service_id != invalid_service_id());
        for (size_t client_id_idx = 0; client_id_idx < client_to_service_array_.size(); client_id_idx++) {
    ...
    const ServiceType& invalid_service_id() const
    {
        return invalid_service_id_;
    }
...
private:
    bool GetServiceIDFromMap(ClientType client_id, ServiceType* service_id) const
    {
