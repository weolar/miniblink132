...
public:
    CommandsIssuedQuery(QueryManager* manager, GLenum target, scoped_refptr<gpu::Buffer> buffer, QuerySync* sync);
...
private:
    ...
    CommandProcessingState command_processing_state_ = CommandProcessingState::kNotStarted;
    base::TimeDelta elapsed_time_;
    base::TimeTicks begin_command_processing_time_;
};

CommandsIssuedQuery::CommandsIssuedQuery(QueryManager* manager, GLenum target, scoped_refptr<gpu::Buffer> buffer, QuerySync* sync)
...
{
    DCHECK_NE(command_processing_state_, CommandProcessingState::kProcessingCommands);
...
{
    DCHECK_NE(command_processing_state_, CommandProcessingState::kNotProcessingCommands);
...
public:
    CommandsIssuedTimestampQuery(QueryManager* manager, GLenum target, scoped_refptr<gpu::Buffer> buffer, QuerySync* sync);
...
CommandsIssuedTimestampQuery::CommandsIssuedTimestampQuery(QueryManager* manager, GLenum target, scoped_refptr<gpu::Buffer> buffer, QuerySync* sync)
    : Query(manager, target, std::move(buffer), sync)
{
}
...
void CommandsIssuedTimestampQuery::QueryCounter(base::subtle::Atomic32 submit_count)
{
...
public:
    CommandsCompletedQuery(QueryManager* manager, GLenum target, scoped_refptr<gpu::Buffer> buffer, QuerySync* sync);
...
CommandsCompletedQuery::CommandsCompletedQuery(QueryManager* manager, GLenum target, scoped_refptr<gpu::Buffer> buffer, QuerySync* sync)
    : Query(manager, target, std::move(buffer), sync)
{
}
...
QueryManager::Query* QueryManager::CreateQuery(GLenum target, GLuint client_id, scoped_refptr<gpu::Buffer> buffer, QuerySync* sync)
{
    ...
    case GL_COMMANDS_ISSUED_TIMESTAMP_CHROMIUM:
        query = new CommandsIssuedTimestampQuery(this, target, std::move(buffer), sync);
        break;
...
QueryManager::Query::Query(QueryManager* manager, GLenum target, scoped_refptr<gpu::Buffer> buffer, QuerySync* sync)
    : manager_(manager)
    , target_(target)
...
void QueryManager::AddPendingQuery(Query* query, base::subtle::Atomic32 submit_count)
{
    DCHECK(query);
...
void QueryManager::RemovePendingQuery(Query* query)
{
    ...
    if (query->IsPending()) {
        ...
        for (QueryQueue::iterator it = pending_queries_.begin(); it != pending_queries_.end(); ++it) {
            if (it->get() == query) {
...
void QueryManager::QueryCounter(Query* query, base::subtle::Atomic32 submit_count)
{
    DCHECK(query);
    RemovePendingQuery(query);
