...
GpuTaskSchedulerHelper::GpuTaskSchedulerHelper(std::unique_ptr<SingleTaskSequence> task_sequence)
...
GpuTaskSchedulerHelper::GpuTaskSchedulerHelper(gpu::CommandBufferTaskExecutor* command_buffer_task_executor)
...
void GpuTaskSchedulerHelper::Initialize(gpu::CommandBufferHelper* command_buffer_helper)
{
...
void GpuTaskSchedulerHelper::ScheduleGpuTask(
    base::OnceClosure callback, std::vector<gpu::SyncToken> sync_token_fences, const SyncToken& release, SingleTaskSequence::ReportingCallback report_callback)
{
    ...
    task_sequence_->ScheduleTask(std::move(callback), std::move(sync_token_fences), release, std::move(report_callback));
}

void GpuTaskSchedulerHelper::ScheduleOrRetainGpuTask(base::OnceClosure task, std::vector<SyncToken> sync_token_fences)
{
    DCHECK(!using_command_buffer_);
    DCHECK(!command_buffer_helper_);
    task_sequence_->ScheduleOrRetainTask(std::move(task), sync_token_fences, SyncToken());
}
...
ScopedSyncPointClientState GpuTaskSchedulerHelper::CreateSyncPointClientState(CommandBufferNamespace namespace_id, CommandBufferId command_buffer_id)
{
    return task_sequence_->CreateSyncPointClientState(namespace_id, command_buffer_id);
}
