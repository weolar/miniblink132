...
protected:
    void SetUp() override
    {
        ...
        ASSERT_TRUE(vertex_translator_->Init(GL_VERTEX_SHADER, SH_GLES2_SPEC, &resources, SH_ESSL_OUTPUT, {}, false));
        ASSERT_TRUE(fragment_translator_->Init(GL_FRAGMENT_SHADER, SH_GLES2_SPEC, &resources, SH_ESSL_OUTPUT, {}, false));
...
protected:
    void SetUp() override
    {
        ...
        ASSERT_TRUE(vertex_translator_->Init(GL_VERTEX_SHADER, SH_GLES3_SPEC, &resources, SH_ESSL_OUTPUT, {}, false));
        ASSERT_TRUE(fragment_translator_->Init(GL_FRAGMENT_SHADER, SH_GLES3_SPEC, &resources, SH_ESSL_OUTPUT, {}, false));
#endif //  BUILDFLAG(IS_WIN) && defined(ARCH_CPU_ARM64)
    }
    void TearDown() override
    {
...
TEST_F(ShaderTranslatorTest, ValidVertexShader)
{
    ...
    EXPECT_TRUE(vertex_translator_->Translate(
        shader, &info_log, &translated_source, &shader_version, &attrib_map, &uniform_map, &varying_map, &interface_block_map, &output_variable_list));
...
TEST_F(ShaderTranslatorTest, InvalidVertexShader)
{
    ...
    EXPECT_FALSE(vertex_translator_->Translate(
        bad_shader, &info_log, &translated_source, &shader_version, &attrib_map, &uniform_map, &varying_map, &interface_block_map, &output_variable_list));
    ...
    info_log.clear();
    EXPECT_TRUE(vertex_translator_->Translate(
        good_shader, &info_log, &translated_source, &shader_version, &attrib_map, &uniform_map, &varying_map, &interface_block_map, &output_variable_list));
    EXPECT_TRUE(info_log.empty());
...
TEST_F(ShaderTranslatorTest, GetUniforms)
{
    ...
    EXPECT_TRUE(iter->second.findInfoByMappedName("_ubar[0]._ufoo._ucolor[0]", &info, &original_name));
    ...
    // Second uniform.
    EXPECT_TRUE(iter->second.findInfoByMappedName("_ubar[1]._ufoo._ucolor[0]", &info, &original_name));
    EXPECT_EQ(static_cast<GLenum>(GL_FLOAT_VEC4), info->type);
    EXPECT_EQ(1u, info->getOutermostArraySize());
    EXPECT_STREQ("color", info->name.c_str());
...
{
    const char* shader = "#version 300 es\n"
...
TEST_F(ES3ShaderTranslatorTest, GetInterfaceBlocks)
{
    ...
    for (iter = interface_block_map.begin(); iter != interface_block_map.end(); ++iter) {
...
TEST_F(ShaderTranslatorTest, OptionsString)
{
    ...
    ASSERT_TRUE(translator_1->Init(GL_VERTEX_SHADER, SH_GLES2_SPEC, &resources, SH_GLSL_150_CORE_OUTPUT, {}, false));
    ASSERT_TRUE(translator_2->Init(GL_FRAGMENT_SHADER, SH_GLES2_SPEC, &resources, SH_GLSL_150_CORE_OUTPUT, with_init_output_variables, false));
    resources.EXT_draw_buffers = 1;
    ASSERT_TRUE(translator_3->Init(GL_VERTEX_SHADER, SH_GLES2_SPEC, &resources, SH_GLSL_150_CORE_OUTPUT, {}, false));

    std::string options_1(translator_1->GetStringForOptionsThatWouldAffectCompilation()->data);
    std::string options_2(translator_1->GetStringForOptionsThatWouldAffectCompilation()->data);
    std::string options_3(translator_2->GetStringForOptionsThatWouldAffectCompilation()->data);
    std::string options_4(translator_3->GetStringForOptionsThatWouldAffectCompilation()->data);
...
class ShaderTranslatorOutputVersionTest : public testing::TestWithParam<testing::tuple<const char*, const char*>> {
public:
#if BUILDFLAG(IS_WIN) && defined(ARCH_CPU_ARM64)
    void SetUp() override
    {
...
TEST_F(ShaderTranslatorOutputVersionTest, DISABLED_CompatibilityOutput)
{
    ...
    ASSERT_TRUE(vertex_translator->Init(GL_VERTEX_SHADER, SH_GLES2_SPEC, &resources, shader_output_language, compile_options, false));
    scoped_refptr<ShaderTranslator> fragment_translator = new ShaderTranslator();
    ASSERT_TRUE(fragment_translator->Init(GL_FRAGMENT_SHADER, SH_GLES2_SPEC, &resources, shader_output_language, compile_options, false));
    ...
    int shader_version;
    {
        ...
        EXPECT_TRUE(vertex_translator->Translate(kShader, nullptr, &translated_source, &shader_version, nullptr, nullptr, nullptr, nullptr, nullptr));
    ...
    {
        const char* kShader = "#pragma STDGL invariant(all)\n"
                              "precision mediump float;\n"
        ...
        EXPECT_TRUE(fragment_translator->Translate(kShader, nullptr, &translated_source, &shader_version, nullptr, nullptr, nullptr, nullptr, nullptr));
...
TEST_P(ShaderTranslatorOutputVersionTest, HasCorrectOutputGLSLVersion)
{
    ...
    ASSERT_TRUE(translator->Init(GL_VERTEX_SHADER, SH_GLES2_SPEC, &resources, SH_ESSL_OUTPUT, compile_options, false));

    std::string translated_source;
    int shader_version;
    EXPECT_TRUE(translator->Translate(kShader, nullptr, &translated_source, &shader_version, nullptr, nullptr, nullptr, nullptr, nullptr));

    std::string expected_version_directive = testing::get<1>(GetParam());
    if (expected_version_directive.empty()) {
        EXPECT_TRUE(!base::Contains(translated_source, "#version")) << "Translation was:\n" << translated_source;
    } else {
        EXPECT_TRUE(base::Contains(translated_source, expected_version_directive)) << "Translation was:\n" << translated_source;
    }
}
...
namespace {
testing::tuple<const char*, const char*> make_gl_glsl_tuple(const char* gl_version, const char* glsl_version_directive)
{
...
INSTANTIATE_TEST_SUITE_P(OpenGLESContexts, ShaderTranslatorOutputVersionTest,
    testing::Values(make_gl_glsl_tuple("opengl es 2.0", ""), make_gl_glsl_tuple("opengl es 3.0", ""), make_gl_glsl_tuple("opengl es 3.1", ""),
