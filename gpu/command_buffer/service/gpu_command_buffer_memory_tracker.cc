...
GpuCommandBufferMemoryTracker::GpuCommandBufferMemoryTracker(
    CommandBufferId command_buffer_id, uint64_t client_tracing_id, scoped_refptr<base::SingleThreadTaskRunner> task_runner, Observer* observer)
...
void GpuCommandBufferMemoryTracker::TrackMemoryAllocatedChange(int64_t delta)
{
    ...
    size_ += delta;
    if (observer_)
        observer_->OnMemoryAllocatedChange(command_buffer_id_, old_size, size_, GpuPeakMemoryAllocationSource::COMMAND_BUFFER);
}
