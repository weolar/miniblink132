...
public:
    explicit ScopedPixelUnpackBufferOverride(bool has_pixel_buffers, GLuint binding_override)
        : orig_binding_(-1)
    {
...
bool IsWebGLDrawBuffersSupported(bool webglCompatibilityContext, GLenum depth_texture_internal_format, GLenum depth_stencil_texture_internal_format)
{
    ...
    if (depth_stencil_texture_internal_format != GL_NONE) {
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, depth_stencil_texture_internal_format, 1, 1, 0, GL_DEPTH_STENCIL, GL_UNSIGNED_INT_24_8, nullptr);
    }
    ...
    if (depth_texture_internal_format != GL_NONE) {
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, depth_texture_internal_format, 1, 1, 0, GL_DEPTH_COMPONENT, GL_UNSIGNED_INT, nullptr);
    }
    ...
    for (GLint i = 0; i < max_allowed_buffers; ++i) {
        ...
        glFramebufferTexture2DEXT(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + i, GL_TEXTURE_2D, color, 0);
        if (glCheckFramebufferStatusEXT(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE) {
            result = false;
            break;
        }
        if (depth_texture != 0) {
            glFramebufferTexture2DEXT(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_TEXTURE_2D, depth_texture, 0);
            if (glCheckFramebufferStatusEXT(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE) {
                result = false;
                break;
            }
        ...
        if (depth_stencil_texture != 0) {
            ...
            if (webglCompatibilityContext) {
                glFramebufferTexture2DEXT(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, GL_TEXTURE_2D, depth_stencil_texture, 0);
            } else {
                glFramebufferTexture2DEXT(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_TEXTURE_2D, depth_stencil_texture, 0);
                glFramebufferTexture2DEXT(GL_FRAMEBUFFER, GL_STENCIL_ATTACHMENT, GL_TEXTURE_2D, depth_stencil_texture, 0);
            }
            if (glCheckFramebufferStatusEXT(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE) {
                result = false;
                break;
            ...
            if (webglCompatibilityContext) {
                ...
                glFramebufferTexture2DEXT(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_TEXTURE_2D, 0, 0);
                glFramebufferTexture2DEXT(GL_FRAMEBUFFER, GL_STENCIL_ATTACHMENT, GL_TEXTURE_2D, 0, 0);
            }
        }
    }
...
{
    InitializeBasicState(base::CommandLine::InitializedForCurrentProcess() ? base::CommandLine::ForCurrentProcess() : nullptr);
}

FeatureInfo::FeatureInfo(const GpuDriverBugWorkarounds& gpu_driver_bug_workarounds, const GpuFeatureInfo& gpu_feature_info)
    : workarounds_(gpu_driver_bug_workarounds)
{
    ...
    feature_flags_.chromium_image_ycbcr_420v
        = base::Contains(gpu_feature_info.supported_buffer_formats_for_allocation_and_texturing, gfx::BufferFormat::YUV_420_BIPLANAR);
#elif BUILDFLAG(IS_APPLE)
    feature_flags_.chromium_image_ycbcr_420v = true;
#endif

#if BUILDFLAG(IS_CHROMEOS) || BUILDFLAG(IS_FUCHSIA)
    feature_flags_.chromium_image_ycbcr_p010 = base::Contains(gpu_feature_info.supported_buffer_formats_for_allocation_and_texturing, gfx::BufferFormat::P010);
#elif BUILDFLAG(IS_APPLE)
...
void FeatureInfo::InitializeBasicState(const base::CommandLine* command_line)
{
    ...
    feature_flags_.is_swiftshader_for_webgl = (useGL == gl::kGLImplementationANGLEName) && (useANGLE == gl::kANGLEImplementationSwiftShaderForWebGLName);
...
void FeatureInfo::Initialize(ContextType context_type, bool is_passthrough_cmd_decoder, const DisallowedFeatures& disallowed_features, bool force_reinitialize)
{
    if (initialized_) {
...
void FeatureInfo::InitializeForTesting(const DisallowedFeatures& disallowed_features)
{
    initialized_ = false;
    Initialize(CONTEXT_TYPE_OPENGLES2, false /* is_passthrough_cmd_decoder */, disallowed_features);
}
...
{
    initialized_ = false;
    Initialize(context_type, false /* is_passthrough_cmd_decoder */, DisallowedFeatures());
}
...
bool IsGL_REDSupportedOnFBOs()
{
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RED_EXT, 8, 8, 0, GL_RED_EXT, GL_UNSIGNED_BYTE, nullptr);
    ...
    bool result = glCheckFramebufferStatusEXT(GL_FRAMEBUFFER) == GL_FRAMEBUFFER_COMPLETE;
...
void FeatureInfo::EnableEXTColorBufferFloat()
{
    ...
    validators_.texture_sized_color_renderable_internal_format.AddValue(GL_RGBA16F);
    ...
    validators_.texture_sized_color_renderable_internal_format.AddValue(GL_RGBA32F);
    validators_.texture_sized_color_renderable_internal_format.AddValue(GL_R11F_G11F_B10F);
...
void FeatureInfo::EnableEXTColorBufferHalfFloat()
{
    ...
    validators_.texture_sized_color_renderable_internal_format.AddValue(GL_R16F);
    validators_.texture_sized_color_renderable_internal_format.AddValue(GL_RG16F);
    validators_.texture_sized_color_renderable_internal_format.AddValue(GL_RGB16F);
    validators_.texture_sized_color_renderable_internal_format.AddValue(GL_RGBA16F);
...
void FeatureInfo::EnableCHROMIUMColorBufferFloatRGB()
{
    ...
    validators_.texture_sized_color_renderable_internal_format.AddValue(GL_RGB32F);
...
void FeatureInfo::EnableOESTextureFloatLinear()
{
    ...
    validators_.texture_sized_texture_filterable_internal_format.AddValue(GL_R32F);
    validators_.texture_sized_texture_filterable_internal_format.AddValue(GL_RG32F);
    validators_.texture_sized_texture_filterable_internal_format.AddValue(GL_RGB32F);
    validators_.texture_sized_texture_filterable_internal_format.AddValue(GL_RGBA32F);
}
...
void FeatureInfo::EnableANGLEInstancedArrayIfPossible(const gfx::ExtensionSet& extensions)
{
    if (!feature_flags_.angle_instanced_arrays) {
        if (gfx::HasExtension(extensions, "GL_ANGLE_instanced_arrays") || gl_version_info_->is_es3) {
...
void FeatureInfo::EnableWEBGLMultiDrawIfPossible(const gfx::ExtensionSet& extensions)
{
    if (!feature_flags_.webgl_multi_draw) {
        if (!is_passthrough_cmd_decoder_ || gfx::HasExtension(extensions, "GL_ANGLE_multi_draw")) {
            if (gfx::HasExtension(extensions, "GL_ANGLE_instanced_arrays") || feature_flags_.angle_instanced_arrays || gl_version_info_->is_es3) {
...
void FeatureInfo::InitializeFeatures()
{
    ...
    gl_version_info_ = std::make_unique<gl::GLVersionInfo>(version_str, renderer_str, extensions);
    ...
    bool has_pixel_buffers = gl_version_info_->is_es3 || gfx::HasExtension(extensions, "GL_NV_pixel_buffer_object");
    ...
    // SPIR-V).
    if (!is_passthrough_cmd_decoder_ || feature_flags_.angle_translated_shader_source) {
    ...
    bool have_s3tc = gfx::HasExtension(extensions, "GL_EXT_texture_compression_s3tc");
    bool have_dxt3 = have_s3tc || gfx::HasExtension(extensions, "GL_ANGLE_texture_compression_dxt3");
    bool have_dxt5 = have_s3tc || gfx::HasExtension(extensions, "GL_ANGLE_texture_compression_dxt5");

    if (gfx::HasExtension(extensions, "GL_EXT_texture_compression_dxt1") || gfx::HasExtension(extensions, "GL_ANGLE_texture_compression_dxt1") || have_s3tc) {
        enable_dxt1 = true;
    }
    if (have_dxt3) {
        enable_dxt3 = true;
    ...
    if (enable_dxt1) {
        ...
        validators_.compressed_texture_format.AddValue(GL_COMPRESSED_RGB_S3TC_DXT1_EXT);
        validators_.compressed_texture_format.AddValue(GL_COMPRESSED_RGBA_S3TC_DXT1_EXT);

        validators_.texture_internal_format_storage.AddValue(GL_COMPRESSED_RGB_S3TC_DXT1_EXT);
        validators_.texture_internal_format_storage.AddValue(GL_COMPRESSED_RGBA_S3TC_DXT1_EXT);
    }
    ...
    if (enable_dxt3) {
        ...
        validators_.compressed_texture_format.AddValue(GL_COMPRESSED_RGBA_S3TC_DXT3_EXT);
        validators_.texture_internal_format_storage.AddValue(GL_COMPRESSED_RGBA_S3TC_DXT3_EXT);
    }
    ...
    if (enable_dxt5) {
        ...
        validators_.compressed_texture_format.AddValue(GL_COMPRESSED_RGBA_S3TC_DXT5_EXT);
        validators_.texture_internal_format_storage.AddValue(GL_COMPRESSED_RGBA_S3TC_DXT5_EXT);
    }

    bool have_bptc = gfx::HasExtension(extensions, "GL_EXT_texture_compression_bptc");
    if (have_bptc) {
        feature_flags_.ext_texture_compression_bptc = true;
        AddExtensionString("GL_EXT_texture_compression_bptc");
        validators_.compressed_texture_format.AddValue(GL_COMPRESSED_RGBA_BPTC_UNORM_EXT);
        validators_.compressed_texture_format.AddValue(GL_COMPRESSED_SRGB_ALPHA_BPTC_UNORM_EXT);
        validators_.compressed_texture_format.AddValue(GL_COMPRESSED_RGB_BPTC_SIGNED_FLOAT_EXT);
        validators_.compressed_texture_format.AddValue(GL_COMPRESSED_RGB_BPTC_UNSIGNED_FLOAT_EXT);
        validators_.texture_internal_format_storage.AddValue(GL_COMPRESSED_RGBA_BPTC_UNORM_EXT);
        validators_.texture_internal_format_storage.AddValue(GL_COMPRESSED_SRGB_ALPHA_BPTC_UNORM_EXT);
        validators_.texture_internal_format_storage.AddValue(GL_COMPRESSED_RGB_BPTC_SIGNED_FLOAT_EXT);
        validators_.texture_internal_format_storage.AddValue(GL_COMPRESSED_RGB_BPTC_UNSIGNED_FLOAT_EXT);
    }

    bool have_rgtc = gfx::HasExtension(extensions, "GL_EXT_texture_compression_rgtc");
    if (have_rgtc) {
        ...
        validators_.compressed_texture_format.AddValue(GL_COMPRESSED_SIGNED_RED_RGTC1_EXT);
        validators_.compressed_texture_format.AddValue(GL_COMPRESSED_RED_GREEN_RGTC2_EXT);
        validators_.compressed_texture_format.AddValue(GL_COMPRESSED_SIGNED_RED_GREEN_RGTC2_EXT);
        validators_.texture_internal_format_storage.AddValue(GL_COMPRESSED_RED_RGTC1_EXT);
        validators_.texture_internal_format_storage.AddValue(GL_COMPRESSED_SIGNED_RED_RGTC1_EXT);
        validators_.texture_internal_format_storage.AddValue(GL_COMPRESSED_RED_GREEN_RGTC2_EXT);
        validators_.texture_internal_format_storage.AddValue(GL_COMPRESSED_SIGNED_RED_GREEN_RGTC2_EXT);
    }
    ...
    if (have_astc) {
        ...
        bool have_astc_hdr = gfx::HasExtension(extensions, "GL_KHR_texture_compression_astc_hdr");
        if (have_astc_hdr) {
    ...
    bool have_atc = gfx::HasExtension(extensions, "GL_AMD_compressed_ATC_texture") || gfx::HasExtension(extensions, "GL_ATI_texture_compression_atitc");
    if (have_atc) {
        ...
        validators_.compressed_texture_format.AddValue(GL_ATC_RGBA_INTERPOLATED_ALPHA_AMD);

        validators_.texture_internal_format_storage.AddValue(GL_ATC_RGB_AMD);
        validators_.texture_internal_format_storage.AddValue(GL_ATC_RGBA_INTERPOLATED_ALPHA_AMD);
    }
    ...
    if (!workarounds_.disable_depth_texture
        && (gfx::HasExtension(extensions, "GL_OES_depth_texture") || gfx::HasExtension(extensions, "GL_ANGLE_depth_texture"))) {
        ...
        feature_flags_.angle_depth_texture = gfx::HasExtension(extensions, "GL_ANGLE_depth_texture");
    }
    ...
    if (enable_depth_texture) {
        ...
        validators_.texture_depth_renderable_internal_format.AddValue(GL_DEPTH_COMPONENT);
    }

    GLenum depth_stencil_texture_format = GL_NONE;
    if (gfx::HasExtension(extensions, "GL_EXT_packed_depth_stencil") || gfx::HasExtension(extensions, "GL_OES_packed_depth_stencil")
        ...
        if (enable_depth_texture) {
            ...
            validators_.texture_depth_renderable_internal_format.AddValue(GL_DEPTH_STENCIL);
            validators_.texture_stencil_renderable_internal_format.AddValue(GL_DEPTH_STENCIL);
        }
    ...
    if (gl_version_info_->is_es3 || gfx::HasExtension(extensions, "GL_OES_vertex_array_object")) {
    ...
    if (gl_version_info_->is_es3 || gfx::HasExtension(extensions, "GL_OES_element_index_uint")) {
    ...
    if (is_passthrough_cmd_decoder_ && gfx::HasExtension(extensions, "GL_OES_draw_buffers_indexed")) {
        if (!disallowed_features_.oes_draw_buffers_indexed) {
            EnableOESDrawBuffersIndexed();
        }
    }

    if (gl_version_info_->is_es3 || gfx::HasExtension(extensions, "GL_OES_fbo_render_mipmap") || gfx::HasExtension(extensions, "GL_EXT_framebuffer_object")) {
    ...
    // GL_EXT_sRGB.
    if ((gl_version_info_->is_es3 || gfx::HasExtension(extensions, "GL_OES_rgb8_rgba8")) && gfx::HasExtension(extensions, "GL_EXT_sRGB")
        ...
        validators_.framebuffer_attachment_parameter.AddValue(GL_FRAMEBUFFER_ATTACHMENT_COLOR_ENCODING_EXT);
    ...
    if (gfx::HasExtension(extensions, "GL_EXT_texture_sRGB_decode") && !IsWebGLContext()) {
    ...
    bool have_s3tc_srgb = gfx::HasExtension(extensions, "GL_NV_sRGB_formats") || gfx::HasExtension(extensions, "GL_EXT_texture_compression_s3tc_srgb");

    if (have_s3tc_srgb) {
        AddExtensionString("GL_EXT_texture_compression_s3tc_srgb");

        validators_.compressed_texture_format.AddValue(GL_COMPRESSED_SRGB_S3TC_DXT1_EXT);
        validators_.compressed_texture_format.AddValue(GL_COMPRESSED_SRGB_ALPHA_S3TC_DXT1_EXT);
        validators_.compressed_texture_format.AddValue(GL_COMPRESSED_SRGB_ALPHA_S3TC_DXT3_EXT);
        validators_.compressed_texture_format.AddValue(GL_COMPRESSED_SRGB_ALPHA_S3TC_DXT5_EXT);

        validators_.texture_internal_format_storage.AddValue(GL_COMPRESSED_SRGB_S3TC_DXT1_EXT);
        validators_.texture_internal_format_storage.AddValue(GL_COMPRESSED_SRGB_ALPHA_S3TC_DXT1_EXT);
        validators_.texture_internal_format_storage.AddValue(GL_COMPRESSED_SRGB_ALPHA_S3TC_DXT3_EXT);
        validators_.texture_internal_format_storage.AddValue(GL_COMPRESSED_SRGB_ALPHA_S3TC_DXT5_EXT);
    }
    ...
    bool has_apple_bgra = gfx::HasExtension(extensions, "GL_APPLE_texture_format_BGRA8888");
    bool has_ext_bgra = gfx::HasExtension(extensions, "GL_EXT_texture_format_BGRA8888");
    bool enable_texture_format_bgra8888 = has_ext_bgra || has_apple_bgra;

    bool has_ext_texture_storage = gfx::HasExtension(extensions, "GL_EXT_texture_storage");
    bool has_texture_storage = !workarounds_.disable_texture_storage && (has_ext_texture_storage || gl_version_info_->is_es3);

    bool enable_texture_storage = has_texture_storage;

    bool texture_storage_incompatible_with_bgra = gl_version_info_->is_es3 && !has_ext_texture_storage && !has_apple_bgra;
    if (texture_storage_incompatible_with_bgra && enable_texture_format_bgra8888 && enable_texture_storage) {
    ...
    if (enable_texture_format_bgra8888) {
        ...
        validators_.texture_sized_color_renderable_internal_format.AddValue(GL_BGRA8_EXT);
        validators_.texture_sized_texture_filterable_internal_format.AddValue(GL_BGRA8_EXT);
    ...
    if (gl_version_info_->is_angle_swiftshader) {
        feature_flags_.gpu_memory_buffer_formats.Remove(gfx::BufferFormat::RGBX_8888);
        feature_flags_.gpu_memory_buffer_formats.Remove(gfx::BufferFormat::BGRX_8888);
    }
#endif
    ...
    // extension.
    bool enable_read_format_bgra = gfx::HasExtension(extensions, "GL_EXT_read_format_bgra");
    ...
    if (!disallowed_features_.npot_support && (gl_version_info_->is_es3 || gfx::HasExtension(extensions, "GL_OES_texture_npot"))) {
    ...
    if (!workarounds_.disable_chromium_framebuffer_multisample) {
        bool ext_has_multisample = (gfx::HasExtension(extensions, "GL_EXT_framebuffer_multisample") && gfx::HasExtension(extensions, "GL_EXT_framebuffer_blit"))
            || gl_version_info_->is_es3;
    ...
    } else if (gfx::HasExtension(extensions, "GL_IMG_multisampled_render_to_texture")) {
        feature_flags_.multisampled_render_to_texture = true;
    ...
    if (feature_flags_.multisampled_render_to_texture) {
        validators_.render_buffer_parameter.AddValue(GL_RENDERBUFFER_SAMPLES_EXT);
        validators_.g_l_state.AddValue(GL_MAX_SAMPLES_EXT);
        validators_.framebuffer_attachment_parameter.AddValue(GL_FRAMEBUFFER_ATTACHMENT_TEXTURE_SAMPLES_EXT);
    ...
    if (gfx::HasExtension(extensions, "GL_OES_depth24") || gl_version_info_->is_es3) {
        AddExtensionString("GL_OES_depth24");
        feature_flags_.oes_depth24 = true;
        validators_.render_buffer_format.AddValue(GL_DEPTH_COMPONENT24);
    }

    if (gl_version_info_->is_es3 || gfx::HasExtension(extensions, "GL_OES_standard_derivatives")) {
    ...
    if (feature_flags_.oes_egl_image_external || feature_flags_.nv_egl_stream_consumer_external) {
    ...
    if (gfx::HasExtension(extensions, "GL_ANGLE_compressed_texture_etc") || (gl_version_info_->is_es3 && !gl_version_info_->is_angle)) {
    ...
    if (gfx::HasExtension(extensions, "GL_AMD_compressed_ATC_texture")) {
        AddExtensionString("GL_AMD_compressed_ATC_texture");
        validators_.compressed_texture_format.AddValue(GL_ATC_RGB_AMD);
        validators_.compressed_texture_format.AddValue(GL_ATC_RGBA_EXPLICIT_ALPHA_AMD);
        validators_.compressed_texture_format.AddValue(GL_ATC_RGBA_INTERPOLATED_ALPHA_AMD);

        validators_.texture_internal_format_storage.AddValue(GL_ATC_RGB_AMD);
        validators_.texture_internal_format_storage.AddValue(GL_ATC_RGBA_EXPLICIT_ALPHA_AMD);
        validators_.texture_internal_format_storage.AddValue(GL_ATC_RGBA_INTERPOLATED_ALPHA_AMD);
    }

    if (gfx::HasExtension(extensions, "GL_IMG_texture_compression_pvrtc")) {
        AddExtensionString("GL_IMG_texture_compression_pvrtc");
        validators_.compressed_texture_format.AddValue(GL_COMPRESSED_RGB_PVRTC_4BPPV1_IMG);
        validators_.compressed_texture_format.AddValue(GL_COMPRESSED_RGB_PVRTC_2BPPV1_IMG);
        validators_.compressed_texture_format.AddValue(GL_COMPRESSED_RGBA_PVRTC_4BPPV1_IMG);
        validators_.compressed_texture_format.AddValue(GL_COMPRESSED_RGBA_PVRTC_2BPPV1_IMG);

        validators_.texture_internal_format_storage.AddValue(GL_COMPRESSED_RGB_PVRTC_4BPPV1_IMG);
        validators_.texture_internal_format_storage.AddValue(GL_COMPRESSED_RGB_PVRTC_2BPPV1_IMG);
        validators_.texture_internal_format_storage.AddValue(GL_COMPRESSED_RGBA_PVRTC_4BPPV1_IMG);
        validators_.texture_internal_format_storage.AddValue(GL_COMPRESSED_RGBA_PVRTC_2BPPV1_IMG);
    }
    ...
    if (feature_flags_.chromium_image_ycbcr_420v) {
        ...
        feature_flags_.gpu_memory_buffer_formats.Put(gfx::BufferFormat::YUV_420_BIPLANAR);
    }
    ...
    feature_flags_.chromium_image_ab30 = gl_version_info_->IsAtLeastGLES(3, 0) || gfx::HasExtension(extensions, "GL_EXT_texture_type_2_10_10_10_REV");
#endif
    if (feature_flags_.chromium_image_ar30 || feature_flags_.chromium_image_ab30) {
    ...
    if (feature_flags_.chromium_image_ar30) {
        feature_flags_.gpu_memory_buffer_formats.Put(gfx::BufferFormat::BGRA_1010102);
    }
    if (feature_flags_.chromium_image_ab30) {
        feature_flags_.gpu_memory_buffer_formats.Put(gfx::BufferFormat::RGBA_1010102);
    }
    ...
    if (base::mac::MacOSMajorVersion() >= 11) {
        feature_flags_.gpu_memory_buffer_formats.Put(gfx::BufferFormat::YUVA_420_TRIPLANAR);
    }
#endif // BUILDFLAG(IS_MAC)
    ...
    bool have_ext_occlusion_query_boolean = gfx::HasExtension(extensions, "GL_EXT_occlusion_query_boolean");
    ...
    bool have_es2_draw_buffers_vendor_agnostic = gfx::HasExtension(extensions, "GL_EXT_draw_buffers");
    bool can_emulate_es2_draw_buffers_on_es3_nv = gl_version_info_->is_es3 && gfx::HasExtension(extensions, "GL_NV_draw_buffers");
    bool is_webgl_compatibility_context = gfx::HasExtension(extensions, "GL_ANGLE_webgl_compatibility");
    bool have_es2_draw_buffers = (have_es2_draw_buffers_vendor_agnostic || can_emulate_es2_draw_buffers_on_es3_nv)
        && (context_type_ == CONTEXT_TYPE_OPENGLES2
            || (context_type_ == CONTEXT_TYPE_WEBGL1
                && IsWebGLDrawBuffersSupported(is_webgl_compatibility_context, depth_texture_format, depth_stencil_texture_format)));
    if (have_es2_draw_buffers) {
        ...
        feature_flags_.nv_draw_buffers = can_emulate_es2_draw_buffers_on_es3_nv && !have_es2_draw_buffers_vendor_agnostic;
    }

    if (IsWebGL2OrES3OrHigherContext() || have_es2_draw_buffers) {
        GLint max_color_attachments = 0;
        glGetIntegerv(GL_MAX_COLOR_ATTACHMENTS_EXT, &max_color_attachments);
        for (GLenum i = GL_COLOR_ATTACHMENT1_EXT; i < static_cast<GLenum>(GL_COLOR_ATTACHMENT0 + max_color_attachments); ++i) {
            validators_.attachment.AddValue(i);
            validators_.attachment_query.AddValue(i);
        }
        static_assert(GL_COLOR_ATTACHMENT0_EXT == GL_COLOR_ATTACHMENT0, "GL_COLOR_ATTACHMENT0_EXT should equal GL_COLOR_ATTACHMENT0");
        ...
        for (GLenum i = GL_DRAW_BUFFER0_ARB; i < static_cast<GLenum>(GL_DRAW_BUFFER0_ARB + max_draw_buffers); ++i) {
            validators_.g_l_state.AddValue(i);
        }
    }

    if (gl_version_info_->is_es3 || gfx::HasExtension(extensions, "GL_EXT_blend_minmax")) {
        AddExtensionString("GL_EXT_blend_minmax");
        validators_.equation.AddValue(GL_MIN_EXT);
        validators_.equation.AddValue(GL_MAX_EXT);
        static_assert(GL_MIN_EXT == GL_MIN && GL_MAX_EXT == GL_MAX, "min & max variations must match");
    }
    ...
    feature_flags_.map_buffer_range = gl_version_info_->is_es3 || gfx::HasExtension(extensions, "GL_EXT_map_buffer_range");
    ...
    if ((gl_version_info_->is_es3 || gfx::HasExtension(extensions, "GL_EXT_discard_framebuffer")) && !workarounds_.disable_discard_framebuffer) {
    ...
    if (!workarounds_.disable_blend_equation_advanced) {
        bool blend_equation_advanced_coherent = gfx::HasExtension(extensions, "GL_NV_blend_equation_advanced_coherent")
            || gfx::HasExtension(extensions, "GL_KHR_blend_equation_advanced_coherent");

        if (blend_equation_advanced_coherent || gfx::HasExtension(extensions, "GL_NV_blend_equation_advanced")
            || gfx::HasExtension(extensions, "GL_KHR_blend_equation_advanced")) {
            const GLenum equations[]
                = { GL_MULTIPLY_KHR, GL_SCREEN_KHR, GL_OVERLAY_KHR, GL_DARKEN_KHR, GL_LIGHTEN_KHR, GL_COLORDODGE_KHR, GL_COLORBURN_KHR, GL_HARDLIGHT_KHR,
                      GL_SOFTLIGHT_KHR, GL_DIFFERENCE_KHR, GL_EXCLUSION_KHR, GL_HSL_HUE_KHR, GL_HSL_SATURATION_KHR, GL_HSL_COLOR_KHR, GL_HSL_LUMINOSITY_KHR };
    ...
    if ((gl_version_info_->is_es3 || gfx::HasExtension(extensions, "GL_EXT_texture_rg")) && IsGL_REDSupportedOnFBOs()) {
    ...
    const bool is_texture_norm16_supported_for_webgl2_or_es3 = IsWebGL2OrES3OrHigherContext() && gfx::HasExtension(extensions, "GL_EXT_texture_norm16");
    const bool is_texture_norm16_supported_for_angle_es2
        = is_passthrough_cmd_decoder_ && context_type_ == CONTEXT_TYPE_OPENGLES2 && gfx::HasExtension(extensions, "GL_EXT_texture_norm16");
    if (is_texture_norm16_supported_for_webgl2_or_es3 || is_texture_norm16_supported_for_angle_es2) {
        ...
        validators_.texture_sized_color_renderable_internal_format.AddValue(GL_R16_EXT);
        validators_.texture_sized_color_renderable_internal_format.AddValue(GL_RG16_EXT);
        validators_.texture_sized_color_renderable_internal_format.AddValue(GL_RGBA16_EXT);
    ...
    if (!disable_shader_translator_ && gl_version_info_->IsAtLeastGLES(3, 0) && gfx::HasExtension(extensions, "GL_EXT_blend_func_extended")) {
    ...
    feature_flags_.angle_robust_client_memory = gfx::HasExtension(extensions, "GL_ANGLE_robust_client_memory");

    feature_flags_.khr_debug = gl_version_info_->IsAtLeastGLES(3, 2) || gfx::HasExtension(extensions, "GL_KHR_debug");
    ...
    feature_flags_.chromium_bind_generates_resource = gfx::HasExtension(extensions, "GL_CHROMIUM_bind_generates_resource");
    feature_flags_.angle_webgl_compatibility = is_webgl_compatibility_context;
    feature_flags_.chromium_copy_texture = gfx::HasExtension(extensions, "GL_CHROMIUM_copy_texture");
    feature_flags_.chromium_copy_compressed_texture = gfx::HasExtension(extensions, "GL_CHROMIUM_copy_compressed_texture");
    feature_flags_.angle_client_arrays = gfx::HasExtension(extensions, "GL_ANGLE_client_arrays");
    feature_flags_.angle_request_extension = gfx::HasExtension(extensions, "GL_ANGLE_request_extension");
    feature_flags_.ext_pixel_buffer_object = gfx::HasExtension(extensions, "GL_NV_pixel_buffer_object");
    feature_flags_.ext_unpack_subimage = gfx::HasExtension(extensions, "GL_EXT_unpack_subimage");
    feature_flags_.oes_rgb8_rgba8 = gfx::HasExtension(extensions, "GL_OES_rgb8_rgba8");
    feature_flags_.angle_robust_resource_initialization = gfx::HasExtension(extensions, "GL_ANGLE_robust_resource_initialization");
    ...
    feature_flags_.separate_stencil_ref_mask_writemask = !(gl_version_info_->is_d3d) && !IsWebGLContext();
    ...
    if (is_passthrough_cmd_decoder_ && gfx::HasExtension(extensions, "GL_KHR_parallel_shader_compile")) {
...
#if BUILDFLAG(IS_MAC)
    if (is_passthrough_cmd_decoder_ && gfx::HasExtension(extensions, "GL_ANGLE_base_vertex_base_instance")) {
#else
    if ((!is_passthrough_cmd_decoder_ && ((gl_version_info_->IsAtLeastGLES(3, 2) && gfx::HasExtension(extensions, "GL_EXT_base_instance"))))
        ...
        if (feature_flags_.webgl_multi_draw) {
            feature_flags_.webgl_multi_draw_instanced_base_vertex_base_instance = true;
            AddExtensionString("GL_WEBGL_multi_draw_instanced_base_vertex_base_instance");
        }
    }
    ...
    if (gfx::HasExtension(extensions, "GL_AMD_framebuffer_multisample_advanced")) {
    ...
    if (gfx::HasExtension(extensions, "GL_ANGLE_shader_pixel_local_storage_coherent")) {
    ...
    if (is_passthrough_cmd_decoder_ && gfx::HasExtension(extensions, "GL_ANGLE_stencil_texturing")) {
        AddExtensionString("GL_ANGLE_stencil_texturing");
    }

    if (is_passthrough_cmd_decoder_ && gfx::HasExtension(extensions, "GL_EXT_blend_func_extended")) {
        feature_flags_.ext_blend_func_extended = true;
        AddExtensionString("GL_EXT_blend_func_extended");
    }

    if (is_passthrough_cmd_decoder_ && gfx::HasExtension(extensions, "GL_EXT_clip_control")) {
        feature_flags_.ext_clip_control = true;
        AddExtensionString("GL_EXT_clip_control");
    }

    if (is_passthrough_cmd_decoder_ && gfx::HasExtension(extensions, "GL_EXT_conservative_depth")) {
        AddExtensionString("GL_EXT_conservative_depth");
    }

    if (is_passthrough_cmd_decoder_ && gfx::HasExtension(extensions, "GL_EXT_depth_clamp")) {
        AddExtensionString("GL_EXT_depth_clamp");
    }

    if (is_passthrough_cmd_decoder_ && gfx::HasExtension(extensions, "GL_EXT_polygon_offset_clamp")) {
        feature_flags_.ext_polygon_offset_clamp = true;
        AddExtensionString("GL_EXT_polygon_offset_clamp");
    }

    if (is_passthrough_cmd_decoder_ && gfx::HasExtension(extensions, "GL_EXT_render_snorm")) {
        AddExtensionString("GL_EXT_render_snorm");
    }

    if (is_passthrough_cmd_decoder_ && gfx::HasExtension(extensions, "GL_EXT_texture_mirror_clamp_to_edge")) {
        AddExtensionString("GL_EXT_texture_mirror_clamp_to_edge");
    }

    if (is_passthrough_cmd_decoder_ && gfx::HasExtension(extensions, "GL_NV_shader_noperspective_interpolation")) {
        AddExtensionString("GL_NV_shader_noperspective_interpolation");
    }

    if (is_passthrough_cmd_decoder_ && gfx::HasExtension(extensions, "GL_OES_sample_variables")) {
        AddExtensionString("GL_OES_sample_variables");
    }

    if (is_passthrough_cmd_decoder_ && gfx::HasExtension(extensions, "GL_OES_shader_multisample_interpolation")) {
        AddExtensionString("GL_OES_shader_multisample_interpolation");
    }

    if (is_passthrough_cmd_decoder_ && gfx::HasExtension(extensions, "GL_QCOM_render_shared_exponent")) {
        AddExtensionString("GL_QCOM_render_shared_exponent");
    }

    if (is_passthrough_cmd_decoder_ && gfx::HasExtension(extensions, "GL_ANGLE_blob_cache")) {
...
void FeatureInfo::InitializeFloatAndHalfFloatFeatures(const gfx::ExtensionSet& extensions)
{
    ...
    if (gl_version_info_->IsAtLeastGLES(3, 2) || gfx::HasExtension(extensions, "GL_EXT_float_blend")) {
    ...
    if (may_enable_chromium_color_buffer_float && !had_native_chromium_color_buffer_float_ext) {
        if (workarounds_.force_enable_color_buffer_float || workarounds_.force_enable_color_buffer_float_except_rgb32f) {
            if (enable_es3)
                enable_ext_color_buffer_float = true;
            feature_flags_.chromium_color_buffer_float_rgba = true;
            if (!disallowed_features_.chromium_color_buffer_float_rgba)
            ...
            glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB32F, width, width, 0, GL_RGB, GL_FLOAT, nullptr);
            ...
            if (status_rgba == GL_FRAMEBUFFER_COMPLETE && enable_es3) {
                ...
                for (size_t i = 0; i < std::size(kFormats); ++i) {
                    glTexImage2D(GL_TEXTURE_2D, 0, kInternalFormats[i], width, width, 0, kFormats[i], GL_FLOAT, nullptr);
                    bool supported = glCheckFramebufferStatusEXT(GL_FRAMEBUFFER) == GL_FRAMEBUFFER_COMPLETE;
            ...
            if (IsWebGL1OrES2Context() && !enable_ext_color_buffer_half_float && gl_version_info_->IsAtLeastGLES(3, 0)) {
                ...
                glTexImage2D(GL_TEXTURE_2D, 0, internal_format, width, width, 0, format, data_type, nullptr);
                bool supported = glCheckFramebufferStatusEXT(GL_FRAMEBUFFER) == GL_FRAMEBUFFER_COMPLETE;
    ...
    if (enable_texture_float) {
        ...
        validators_.texture_internal_format_storage.AddValue(GL_LUMINANCE_ALPHA32F_EXT);
    }
    ...
    if (enable_texture_half_float) {
        ...
        validators_.texture_internal_format_storage.AddValue(GL_LUMINANCE_ALPHA16F_EXT);
    }
}
...
void FeatureInfo::EnableES3Validators()
{
    ...
    if (max_color_attachments < kTotalColorAttachmentEnums) {
        validators_.attachment.RemoveValues(kColorAttachments + max_color_attachments, kTotalColorAttachmentEnums - max_color_attachments);
        validators_.attachment_query.RemoveValues(kColorAttachments + max_color_attachments, kTotalColorAttachmentEnums - max_color_attachments);
        validators_.read_buffer.RemoveValues(kColorAttachments + max_color_attachments, kTotalColorAttachmentEnums - max_color_attachments);
    }
    ...
    if (max_draw_buffers < kTotalDrawBufferEnums) {
        validators_.g_l_state.RemoveValues(kDrawBuffers + max_draw_buffers, kTotalDrawBufferEnums - max_draw_buffers);
    }
