...
public:
    struct GPU_EXPORT Task {
        ...
        // execution.
        Task(SequenceId sequence_id, TaskCallback task_callback, std::vector<SyncToken> sync_token_fences, const SyncToken& release,
            ReportingCallback report_callback = ReportingCallback());

        Task(SequenceId sequence_id, base::OnceClosure task_closure, std::vector<SyncToken> sync_token_fences, const SyncToken& release,
            ReportingCallback report_callback = ReportingCallback());

        Task(SequenceId sequence_id, base::OnceClosure task_closure, std::vector<SyncToken> sync_token_fences,
            ReportingCallback report_callback = ReportingCallback());
    ...
    public:
        ScopedSetSequencePriority(Scheduler* scheduler, SequenceId sequence_id, SchedulingPriority priority);
    ...
    SequenceId CreateSequence(SchedulingPriority priority, scoped_refptr<base::SingleThreadTaskRunner> task_runner) LOCKS_EXCLUDED(lock());
    ...
    SequenceId CreateSequence(SchedulingPriority priority, scoped_refptr<base::SingleThreadTaskRunner> task_runner, CommandBufferNamespace namespace_id,
    ...
    [[nodiscard]] ScopedSyncPointClientState CreateSyncPointClientState(
        SequenceId sequence_id, CommandBufferNamespace namespace_id, CommandBufferId command_buffer_id) LOCKS_EXCLUDED(lock());
    ...
    SchedulingPriority GetSequenceDefaultPriority(SequenceId sequence_id) LOCKS_EXCLUDED(lock());

    // Changes a sequence's priority. Used in WaitForGetOffset/TokenInRange to
    // temporarily increase a sequence's priority.
    void SetSequencePriority(SequenceId sequence_id, SchedulingPriority priority) LOCKS_EXCLUDED(lock());
    ...
    void ContinueTask(SequenceId sequence_id, TaskCallback task_callback) LOCKS_EXCLUDED(lock());
    void ContinueTask(SequenceId sequence_id, base::OnceClosure task_closure) LOCKS_EXCLUDED(lock());
    ...
    base::SingleThreadTaskRunner* GetTaskRunnerForTesting(SequenceId sequence_id) LOCKS_EXCLUDED(lock());
...
private:
    struct SchedulingState {
        ...
        static bool RunsBefore(const SchedulingState& lhs, const SchedulingState& rhs)
        {
            return std::tie(lhs.priority, lhs.order_num) < std::tie(rhs.priority, rhs.order_num);
        }
    ...
    public:
        Sequence(Scheduler* scheduler, scoped_refptr<base::SingleThreadTaskRunner> task_runner, SchedulingPriority priority,
        ...
        bool enabled() const EXCLUSIVE_LOCKS_REQUIRED(lock())
        {
            return enabled_;
        }
        ...
        bool ShouldYieldTo(const Sequence* other) const EXCLUSIVE_LOCKS_REQUIRED(lock());
        ...
        uint32_t AddTask(base::OnceClosure task_closure, std::vector<SyncToken> wait_fences, const SyncToken& release,
            TaskGraph::ReportingCallback report_callback) override EXCLUSIVE_LOCKS_REQUIRED(lock());

        // Returns the next order number and closure. Sets running state to RUNNING.
        uint32_t BeginTask(base::OnceClosure* task_closure) override EXCLUSIVE_LOCKS_REQUIRED(lock());
        ...
        void ContinueTask(base::OnceClosure task_closure) override EXCLUSIVE_LOCKS_REQUIRED(lock());

        SchedulingPriority current_priority() const EXCLUSIVE_LOCKS_REQUIRED(lock())
        {
    ...
    Sequence* GetSequence(SequenceId sequence_id) EXCLUSIVE_LOCKS_REQUIRED(lock());

    void ScheduleTaskHelper(Scheduler::Task task) EXCLUSIVE_LOCKS_REQUIRED(lock());

    void TryScheduleSequence(Sequence* sequence) EXCLUSIVE_LOCKS_REQUIRED(lock());

    // Returns a sorted list of runnable sequences.
    const std::vector<SchedulingState>& GetSortedRunnableSequences(base::SingleThreadTaskRunner* task_runner) EXCLUSIVE_LOCKS_REQUIRED(lock());
    ...
    // rescheduled.
    bool HasAnyUnblockedTasksOnRunner(const base::SingleThreadTaskRunner* task_runner) const EXCLUSIVE_LOCKS_REQUIRED(lock());
    ...
    Sequence* FindNextTaskFromRoot(Sequence* root_sequence) EXCLUSIVE_LOCKS_REQUIRED(lock());
    ...
    base::flat_map<SequenceId, Sequence*> scheduler_sequence_map_ GUARDED_BY(lock());
    ...
    base::flat_map<base::SingleThreadTaskRunner*, PerThreadState> per_thread_state_map_ GUARDED_BY(lock());
