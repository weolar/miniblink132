...
template <typename T> void RunFunctor(T functor)
{
    functor();
}

template <typename T> base::OnceClosure GetClosure(T functor)
{
    return base::BindOnce(&RunFunctor<T>, functor);
}

class SchedulerTest : public base::test::WithFeatureOverride, public testing::Test {
public:
    SchedulerTest()
...
protected:
    ...
    void RunAllPendingTasks()
    {
        ...
        SequenceId sequence_id = scheduler()->CreateSequence(SchedulingPriority::kLow, task_runner());
        scheduler()->ScheduleTask(Scheduler::Task(sequence_id, run_loop.QuitClosure(), std::vector<SyncToken>()));
        run_loop.Run();
...
{
    SequenceId sequence_id = scheduler()->CreateSequence(SchedulingPriority::kNormal, task_runner());

    int count = 0;
    int ran1 = 0;
    scheduler()->ScheduleTask(Scheduler::Task(sequence_id, GetClosure([&] { ran1 = ++count; }), std::vector<SyncToken>()));

    int ran2 = 0;
    scheduler()->ScheduleTask(Scheduler::Task(sequence_id, GetClosure([&] { ran2 = ++count; }), std::vector<SyncToken>()));
...
TEST_P(SchedulerTest, ScheduledTasksRunAfterReporting)
{
    ...
    bool ran = false;
    bool reported = false;
    scheduler()->ScheduleTask(Scheduler::Task(sequence_id, GetClosure([&] {
...
TEST_P(SchedulerTest, ContinuedTasksRunFirst)
{
    ...
    int continued1 = 0;
    scheduler()->ScheduleTask(Scheduler::Task(sequence_id, GetClosure([&] {
        scheduler()->ContinueTask(sequence_id, GetClosure([&] { continued1 = ++count; }));
        ran1 = ++count;
    }),
        std::vector<SyncToken>()));
...
protected:
    ...
    {
        CommandBufferId command_buffer_id = CommandBufferId::FromUnsafeValue(sequence_key);
        SequenceId sequence_id = scheduler()->CreateSequence(priority, task_runner(), kNamespaceId, command_buffer_id);

        sequence_info_.emplace(sequence_key, SequenceInfo(sequence_id, command_buffer_id));
    }
    ...
    void CreateExternalSequence(int sequence_key)
    {
        ...
        auto release_state = sync_point_manager()->CreateSyncPointClientState(kNamespaceId, command_buffer_id, order_data->sequence_id());

        sequence_info_.emplace(sequence_key, SequenceInfo(std::move(order_data), command_buffer_id, release_state));
    }
    ...
    void CreateSyncToken(int sequence_key, int release_sync)
    {
        ...
        sync_tokens_.emplace(release_sync, SyncToken(kNamespaceId, info_it->second.command_buffer_id, release));
    }
    ...
    TaskCallback GetTaskCallback(int sequence_key, int release_sync)
    {
        ...
        return base::BindLambdaForTesting([this, task_id](FenceSyncReleaseDelegate* release_delegate) {
            if (release_delegate) {
    ...
    void ScheduleTask(int sequence_key, const std::vector<int>& wait_syncs, int release_sync)
    {
        ...
        scheduler()->ScheduleTask(Scheduler::Task(info_it->second.sequence_id, std::move(task_callback), waits, release));
    }

    const std::vector<int>& tasks_executed()
    {
...
private:
    ...
    struct SequenceInfo {
        ...
        SequenceInfo(scoped_refptr<SyncPointOrderData> order_data, CommandBufferId command_buffer_id, scoped_refptr<SyncPointClientState> release_state)
...
*/
    ...
    const int expected_task_order[] = { 2, 1, 0, 3, 5, 7, 8, 6, 4, 9, 10, 11, 14, 13, 12 };
    EXPECT_THAT(tasks_executed(), testing::ElementsAreArray(expected_task_order));
}
...
TEST_P(SchedulerTest, ShouldNotYieldWhenNoTasksToRun)
{
    ...
    SequenceId sequence_id1 = scheduler()->CreateSequence(SchedulingPriority::kNormal, task_runner(), namespace_id, command_buffer_id);

    SyncToken sync_token(namespace_id, command_buffer_id, 1);

    scheduler()->ScheduleTask(
        Scheduler::Task(sequence_id1, GetClosure([&] { EXPECT_FALSE(scheduler()->ShouldYield(sequence_id1)); }), std::vector<SyncToken>(), sync_token));
    ...
    SequenceId sequence_id2 = scheduler()->CreateSequence(SchedulingPriority::kNormal, task_runner());
    scheduler()->ScheduleTask(Scheduler::Task(sequence_id2, GetClosure([] {}), { sync_token }));
    RunAllPendingTasks();
...
TEST_P(SchedulerTest, ReleaseSequenceShouldYield)
{
    CommandBufferNamespace namespace_id = CommandBufferNamespace::GPU_IO;
    CommandBufferId command_buffer_id = CommandBufferId::FromUnsafeValue(1);
    SequenceId sequence_id1 = scheduler()->CreateSequence(SchedulingPriority::kLow, task_runner(), namespace_id, command_buffer_id);

    SyncToken sync_token(namespace_id, command_buffer_id, 1);
    int count = 0;
    int ran1 = 0;
    scheduler()->ScheduleTask(Scheduler::Task(sequence_id1, base::BindLambdaForTesting([&](FenceSyncReleaseDelegate* release_delegate) {
    ...
    int ran2 = 0;
    SequenceId sequence_id2 = scheduler()->CreateSequence(SchedulingPriority::kHigh, task_runner());
    scheduler()->ScheduleTask(Scheduler::Task(sequence_id2, GetClosure([&] { ran2 = ++count; }), { sync_token }));
...
TEST_P(SchedulerTest, ShouldYieldIsValidWhenSequenceReleaseIsPending)
{
    ...
    SequenceId sequence_id1 = scheduler()->CreateSequence(SchedulingPriority::kHigh, task_runner(), namespace_id, command_buffer_id1);

    CommandBufferId command_buffer_id2 = CommandBufferId::FromUnsafeValue(2);
    SequenceId sequence_id2 = scheduler()->CreateSequence(SchedulingPriority::kNormal, task_runner(), namespace_id, command_buffer_id2);
    ...
    scheduler()->ScheduleTask(Scheduler::Task(sequence_id1, GetClosure([&] { EXPECT_FALSE(scheduler()->ShouldYield(sequence_id1)); }), {}, sync_token1));

    // Job 2.1 depends on Job 1.1.
    scheduler()->ScheduleTask(Scheduler::Task(sequence_id2, GetClosure([&] {}), { sync_token1 }, sync_token2));

    // Job 1.2 depends on Job 2.1.
    scheduler()->ScheduleTask(Scheduler::Task(sequence_id1, GetClosure([&] {}), { sync_token2 }));
...
{
    SequenceId sequence_id1 = scheduler()->CreateSequence(SchedulingPriority::kHigh, task_runner());
    CommandBufferNamespace namespace_id = CommandBufferNamespace::GPU_IO;
    CommandBufferId command_buffer_id1 = CommandBufferId::FromUnsafeValue(1);
    scoped_refptr<SyncPointClientState> release_state1 = sync_point_manager()->CreateSyncPointClientState(namespace_id, command_buffer_id1, sequence_id1);

    SequenceId sequence_id2 = scheduler()->CreateSequence(SchedulingPriority::kNormal, task_runner());
    CommandBufferId command_buffer_id2 = CommandBufferId::FromUnsafeValue(2);
    auto scoped_release_state2 = scheduler()->CreateSyncPointClientState(sequence_id2, namespace_id, command_buffer_id2);
    ...
    scheduler()->ScheduleTask(Scheduler::Task(sequence_id2, GetClosure([&] { ran2 = ++count; }), std::vector<SyncToken>()));
    ...
    scheduler()->ScheduleTask(Scheduler::Task(sequence_id1, GetClosure([&] {
        ran1 = ++count;
        release_state1->Wait(sync_token, base::BindOnce(&Scheduler::EnableSequence, base::Unretained(scheduler()), sequence_id1));
...
{
    SequenceId sequence_id1 = scheduler()->CreateSequence(SchedulingPriority::kNormal, task_runner());
    SequenceId sequence_id2 = scheduler()->CreateSequence(SchedulingPriority::kLow, task_runner());
    SequenceId sequence_id3 = scheduler()->CreateSequence(SchedulingPriority::kHigh, task_runner());

    int count = 0;
    int ran1 = 0, ran2 = 0, ran3 = 0;

    scheduler()->ScheduleTask(Scheduler::Task(sequence_id1, GetClosure([&] { ran1 = ++count; }), std::vector<SyncToken>()));

    scheduler()->ScheduleTask(Scheduler::Task(sequence_id2, GetClosure([&] { ran2 = ++count; }), std::vector<SyncToken>()));

    scheduler()->ScheduleTask(Scheduler::Task(sequence_id3, GetClosure([&] { ran3 = ++count; }), std::vector<SyncToken>()));
    ...
    scheduler()->SetSequencePriority(sequence_id2, scheduler()->GetSequenceDefaultPriority(sequence_id2));
...
{
    SequenceId seq_id1 = scheduler()->CreateSequence(SchedulingPriority::kLow, task_runner());
    SequenceId seq_id2 = scheduler()->CreateSequence(SchedulingPriority::kNormal, task_runner());
    SequenceId seq_id3 = scheduler()->CreateSequence(SchedulingPriority::kHigh, task_runner());
...
TEST_P(SchedulerGraphValidationTest, ValidationWaitWithoutRelease)
{
    ...
    task_environment_.FastForwardBy(TaskGraph::kMaxValidationDelay - TaskGraph::kMinValidationDelay + base::Seconds(1));
    ...
    // of task 1.
    task_environment_.FastForwardBy(TaskGraph::kMaxValidationDelay + base::Seconds(1));
    RunAllPendingTasks();
...
INSTANTIATE_TEST_SUITE_P(All, SchedulerTaskRunOrderTest, testing::Values(false, true));

// Only test the case of IsSyncPointGraphValidationEnabled() being true.
INSTANTIATE_TEST_SUITE_P(All, SchedulerGraphValidationTest, testing::Values(true));
