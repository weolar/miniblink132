...
GrCacheController::GrCacheController(SharedContextState* context_state)
    ...
    , task_runner_(base::SingleThreadTaskRunner::HasCurrentDefault() ? base::SingleThreadTaskRunner::GetCurrentDefault() : nullptr)
{
}

GrCacheController::GrCacheController(SharedContextState* context_state, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
    : context_state_(context_state)
...
void GrCacheController::ScheduleGrContextCleanup()
{
    ...
    purge_gr_cache_cb_.Reset(base::BindOnce(&GrCacheController::PurgeGrCache, base::Unretained(this), current_idle_id_));
    task_runner_->PostDelayedTask(FROM_HERE, purge_gr_cache_cb_.callback(), base::Seconds(kIdleCleanupDelaySeconds));
}
...
void GrCacheController::PerformDeferredCleanup()
{
    ...
    context_state_->gr_context()->performDeferredCleanup(std::chrono::seconds(kOldResourceDelaySeconds));
}
