...
IndexedBufferBindingHost::IndexedBufferBinding::IndexedBufferBinding(const IndexedBufferBindingHost::IndexedBufferBinding& other)
    : type(other.type)
...
IndexedBufferBindingHost::IndexedBufferBinding::~IndexedBufferBinding() = default;

bool IndexedBufferBindingHost::IndexedBufferBinding::operator==(const IndexedBufferBindingHost::IndexedBufferBinding& other) const
{
    if (type == IndexedBufferBindingType::kBindBufferNone && other.type == IndexedBufferBindingType::kBindBufferNone) {
        // This should be the most common case so an early out.
        return true;
    }
    return (type == other.type && buffer.get() == other.buffer.get() && offset == other.offset && size == other.size
        && effective_full_buffer_size == other.effective_full_buffer_size);
}

void IndexedBufferBindingHost::IndexedBufferBinding::SetBindBufferBase(Buffer* _buffer)
{
    if (!_buffer) {
        Reset();
        return;
...
void IndexedBufferBindingHost::IndexedBufferBinding::SetBindBufferRange(Buffer* _buffer, GLintptr _offset, GLsizeiptr _size)
{
    if (!_buffer) {
        Reset();
        return;
...
IndexedBufferBindingHost::IndexedBufferBindingHost(uint32_t max_bindings, GLenum target, bool needs_emulation, bool round_down_uniform_bind_buffer_range_size)
    : is_bound_(false)
    , needs_emulation_(needs_emulation)
    , round_down_uniform_bind_buffer_range_size_(round_down_uniform_bind_buffer_range_size)
...
void IndexedBufferBindingHost::DoBindBufferRange(GLuint index, Buffer* buffer, GLintptr offset, GLsizeiptr size)
{
    DCHECK_LT(index, buffer_bindings_.size());
    GLuint service_id = buffer ? buffer->service_id() : 0;
    if (buffer && needs_emulation_) {
        DoAdjustedBindBufferRange(target_, index, service_id, offset, size, buffer->size(), round_down_uniform_bind_buffer_range_size_);
    } else {
...
// static
void IndexedBufferBindingHost::DoAdjustedBindBufferRange(GLenum target, GLuint index, GLuint service_id, GLintptr offset, GLsizeiptr size,
    GLsizeiptr full_buffer_size, bool round_down_uniform_bind_buffer_range_size)
{
    ...
    if (needs_emulation_) {
        ...
        for (size_t ii = 0; ii < buffer_bindings_.size(); ++ii) {
            if (buffer_bindings_[ii].buffer.get() != buffer)
                continue;
            if (buffer_bindings_[ii].type == IndexedBufferBindingType::kBindBufferRange && buffer_bindings_[ii].effective_full_buffer_size != buffer->size()) {
                DoAdjustedBindBufferRange(target_, ii, buffer->service_id(), buffer_bindings_[ii].offset, buffer_bindings_[ii].size, buffer->size(),
...
void IndexedBufferBindingHost::RemoveBoundBuffer(GLenum target, Buffer* buffer, Buffer* target_generic_bound_buffer, bool have_context)
{
    DCHECK(buffer);
...
void IndexedBufferBindingHost::SetIsBound(bool is_bound)
{
    if (is_bound && needs_emulation_) {
        ...
        for (size_t ii = 0; ii < buffer_bindings_.size(); ++ii) {
            ...
            if (buffer && buffer_bindings_[ii].type == IndexedBufferBindingType::kBindBufferRange
                && buffer_bindings_[ii].effective_full_buffer_size != buffer->size()) {
                DoAdjustedBindBufferRange(target_, ii, buffer->service_id(), buffer_bindings_[ii].offset, buffer_bindings_[ii].size, buffer->size(),
                    round_down_uniform_bind_buffer_range_size_);
                buffer_bindings_[ii].effective_full_buffer_size = buffer->size();
...
GLsizeiptr IndexedBufferBindingHost::GetEffectiveBufferSize(GLuint index) const
{
    DCHECK_LT(index, buffer_bindings_.size());
...
void IndexedBufferBindingHost::RestoreBindings(IndexedBufferBindingHost* prev)
{
...
void IndexedBufferBindingHost::UpdateMaxNonNullBindingIndex(size_t changed_index)
{
    size_t plus_one = changed_index + 1;
    DCHECK_LT(changed_index, buffer_bindings_.size());
    if (buffer_bindings_[changed_index].buffer.get()) {
        max_non_null_binding_index_plus_one_ = std::max(max_non_null_binding_index_plus_one_, plus_one);
    } else {
...
bool IndexedBufferBindingHost::UsesBuffer(size_t used_binding_count, const Buffer* buffer) const
{
    DCHECK(buffer);
