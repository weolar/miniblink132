...
void CreateSharedContext(const GpuPreferences& preferences, const GpuDriverBugWorkarounds& workarounds, scoped_refptr<gl::GLSurface>& surface,
    scoped_refptr<gl::GLContext>& context, scoped_refptr<SharedContextState>& context_state, scoped_refptr<gles2::FeatureInfo>& feature_info)
{
    ...
    scoped_refptr<gl::GLShareGroup> share_group = base::MakeRefCounted<gl::GLShareGroup>();
...
using TestParamsTuple = testing::tuple<viz::SharedImageFormat, int, bool, bool, bool>;
class GLES2ExternalFrameBufferTest : public testing::TestWithParam<TestParamsTuple> {
public:
    explicit GLES2ExternalFrameBufferTest()
        : shared_image_manager_(std::make_unique<SharedImageManager>(/*is_thread_safe=*/false))
    {
    }
    ~GLES2ExternalFrameBufferTest() override
    {
        // |context_state_| must be destroyed on its own context.
        bool have_context = context_state_->MakeCurrent(surface_.get(), true /* needs_gl */);
    ...
    void SetUp() override
    {
        ...
        CreateSharedContext(preferences, workarounds, surface_, context_, context_state_, feature_info);

        backing_factory_ = std::make_unique<SharedImageFactory>(preferences, workarounds, GpuFeatureInfo(), context_state_.get(), shared_image_manager_.get(),
            context_state_->memory_tracker(),
        ...
        gles2_external_framebuffer_
            = std::make_unique<gles2::GLES2ExternalFramebuffer>(use_passthrough(), *feature_info, shared_image_representation_factory_.get());
...
protected:
    std::unique_ptr<GLTextureImageRepresentationBase> ProduceGLTextureBase(const Mailbox& mailbox)
    {
        if (use_passthrough())
            return shared_image_representation_factory_->ProduceGLTexturePassthrough(mailbox);
        else
    ...
    Mailbox CreateSharedImage(const viz::SharedImageFormat& format)
    {
        ...
        backing_factory_->CreateSharedImage(mailbox, format, gfx::Size(64, 64), gfx::ColorSpace::CreateSRGB(), kTopLeft_GrSurfaceOrigin, kPremul_SkAlphaType,
            SurfaceHandle(), SHARED_IMAGE_USAGE_GLES2_READ | SHARED_IMAGE_USAGE_GLES2_WRITE, "TestLabel");
        return mailbox;
    }

    void ReadColors(gl::GLApi* const api, const Mailbox& mailbox, std::array<SkColor, 4>& quadrants)
    {
        auto rep = ProduceGLTextureBase(mailbox);
        auto access = rep->BeginScopedAccess(GLTextureImageRepresentationBase::kReadAccessMode, GLTextureImageRepresentationBase::AllowUnclearedAccess::kYes);
        EXPECT_TRUE(rep->IsCleared());

        GLuint fbo;
        api->glGenFramebuffersEXTFn(1, &fbo);
        api->glBindFramebufferEXTFn(GL_FRAMEBUFFER, fbo);
        api->glFramebufferTexture2DEXTFn(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, rep->GetTextureBase()->service_id(), 0);
        ...
        for (auto& color : quadrants) {
            color = SkColorSetARGB(SkColorGetA(color), SkColorGetB(color), SkColorGetG(color), SkColorGetR(color));
        }
...
std::string TestParamToString(const testing::TestParamInfo<TestParamsTuple>& param_info)
{
...
INSTANTIATE_TEST_SUITE_P(, GLES2ExternalFrameBufferTest,
    ::testing::Combine(::testing::Values(viz::SinglePlaneFormat::kRGBA_8888, viz::SinglePlaneFormat::kRGBX_8888), ::testing::Values(0, 8), ::testing::Bool(),
...
TEST_P(GLES2ExternalFrameBufferTest, Test)
{
...

    {
        ...
        EXPECT_TRUE(gles2_external_framebuffer_->AttachSharedImage(mailbox1, params.samples, params.preserve, params.need_depth, params.need_stencil));
    ...
    api->glBindFramebufferEXTFn(GL_FRAMEBUFFER, gles2_external_framebuffer_->GetFramebufferId());
    ...
    const bool draw_direct = !params.preserve && (std::min(max_sample_count_, params.samples) == 0);
    ...
    // succeed.
    EXPECT_TRUE(gles2_external_framebuffer_->AttachSharedImage(Mailbox(), 0, false, false, false));

    const SkColor clear_color = params.format.HasAlpha() ? SK_ColorTRANSPARENT : SK_ColorBLACK;
    {
    ...
    EXPECT_TRUE(gles2_external_framebuffer_->AttachSharedImage(mailbox2, params.samples, params.preserve, params.need_depth, params.need_stencil));
    api->glBindFramebufferEXTFn(GL_FRAMEBUFFER, gles2_external_framebuffer_->GetFramebufferId());
    api->glViewportFn(0, 0, 64, 64);
