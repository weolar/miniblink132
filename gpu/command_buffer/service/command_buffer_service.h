...
public:
    ...
    CommandBufferService(CommandBufferServiceClient* client, MemoryTracker* memory_tracker);
    ...
    // id.
    scoped_refptr<Buffer> CreateTransferBuffer(uint32_t size, int32_t* id, uint32_t alignment = 0);

    // Creates an in-process transfer buffer and register it with a given id.
    scoped_refptr<Buffer> CreateTransferBufferWithId(uint32_t size, int32_t id, uint32_t alignment = 0);
