...
{
    return unit.bound_texture_2d_array.get() ? unit.bound_texture_2d_array->service_id() : 0;
}
...
{
    return unit.bound_texture_cube_map.get() ? unit.bound_texture_cube_map->service_id() : 0;
}

GLuint GetOesServiceId(const TextureUnit& unit)
{
    return unit.bound_texture_external_oes.get() ? unit.bound_texture_external_oes->service_id() : 0;
}

GLuint GetArbServiceId(const TextureUnit& unit)
{
    return unit.bound_texture_rectangle_arb.get() ? unit.bound_texture_rectangle_arb->service_id() : 0;
}
...
bool TargetIsSupported(const FeatureInfo* feature_info, GLuint target)
{
    ...
    case GL_TEXTURE_EXTERNAL_OES:
        return feature_info->feature_flags().oes_egl_image_external || feature_info->feature_flags().nv_egl_stream_consumer_external;
    default:
        NOTREACHED();
    }
}
...
template <> void Vec4::GetValues<GLfloat>(GLfloat* values) const
{
    DCHECK(values);
    switch (type_) {
    case SHADER_VARIABLE_FLOAT:
...
template <> void Vec4::GetValues<GLint>(GLint* values) const
{
    DCHECK(values);
    switch (type_) {
    case SHADER_VARIABLE_FLOAT:
...
template <> void Vec4::GetValues<GLuint>(GLuint* values) const
{
    DCHECK(values);
    switch (type_) {
    case SHADER_VARIABLE_FLOAT:
...
template <> void Vec4::SetValues<GLfloat>(const GLfloat* values)
{
    DCHECK(values);
    for (size_t ii = 0; ii < 4; ++ii)
...
template <> void Vec4::SetValues<GLint>(const GLint* values)
{
    DCHECK(values);
    for (size_t ii = 0; ii < 4; ++ii)
...
template <> void Vec4::SetValues<GLuint>(const GLuint* values)
{
    DCHECK(values);
    for (size_t ii = 0; ii < 4; ++ii)
...
ContextState::ContextState(FeatureInfo* feature_info, bool track_texture_and_sampler_units)
...
void ContextState::RestoreTextureUnitBindings(GLuint unit, const ContextState* prev_state) const
{
    DCHECK(unit < texture_units.size() || (unit == 0 && !track_texture_and_sampler_units));
    ...
    bool bind_texture_oes = feature_info_->feature_flags().oes_egl_image_external || feature_info_->feature_flags().nv_egl_stream_consumer_external;
    ...
    if (prev_state) {
        if (prev_state->track_texture_and_sampler_units) {
            ...
            bind_texture_2d_array = bind_texture_2d_array && service_id_2d_array != Get2dArrayServiceId(prev_unit);
            bind_texture_3d = bind_texture_3d && service_id_3d != Get3dServiceId(prev_unit);
    ...
    if (!bind_texture_2d && !bind_texture_2d_array && !bind_texture_3d && !bind_texture_cube && !bind_texture_oes && !bind_texture_arb) {
        return;
    }
...
void ContextState::RestoreSamplerBinding(GLuint unit, const ContextState* prev_state) const
{
    if (!feature_info_->IsES3Capable())
        return;
...
void ContextState::RestoreUnpackState() const
{
    ...
    if (bound_pixel_unpack_buffer.get()) {
        api()->glBindBufferFn(GL_PIXEL_UNPACK_BUFFER, GetBufferId(bound_pixel_unpack_buffer.get()));
...
void ContextState::RestoreBufferBindings() const
{
    if (vertex_attrib_manager.get()) {
        ...
        api()->glBindBufferFn(GL_ELEMENT_ARRAY_BUFFER, GetBufferId(element_array_buffer));
    }
    api()->glBindBufferFn(GL_ARRAY_BUFFER, GetBufferId(bound_array_buffer.get()));
    if (feature_info_->IsES3Capable()) {
        api()->glBindBufferFn(GL_COPY_READ_BUFFER, GetBufferId(bound_copy_read_buffer.get()));
        api()->glBindBufferFn(GL_COPY_WRITE_BUFFER, GetBufferId(bound_copy_write_buffer.get()));
        api()->glBindBufferFn(GL_PIXEL_PACK_BUFFER, GetBufferId(bound_pixel_pack_buffer.get()));
        UpdatePackParameters();
        api()->glBindBufferFn(GL_PIXEL_UNPACK_BUFFER, GetBufferId(bound_pixel_unpack_buffer.get()));
        UpdateUnpackParameters();
        api()->glBindBufferFn(GL_TRANSFORM_FEEDBACK_BUFFER, GetBufferId(bound_transform_feedback_buffer.get()));
        api()->glBindBufferFn(GL_UNIFORM_BUFFER, GetBufferId(bound_uniform_buffer.get()));
    }
}
...
void ContextState::RestoreProgramSettings(const ContextState* prev_state, bool restore_transform_feedback_bindings) const
{
    bool flag = (restore_transform_feedback_bindings && feature_info_->IsES3Capable());
    if (flag && prev_state) {
        if (prev_state->bound_transform_feedback.get() && prev_state->bound_transform_feedback->active() && !prev_state->bound_transform_feedback->paused()) {
            api()->glPauseTransformFeedbackFn();
        }
    }
    api()->glUseProgramFn(current_program.get() ? current_program->service_id() : 0);
    if (flag) {
        if (bound_transform_feedback.get()) {
            bound_transform_feedback->DoBindTransformFeedback(GL_TRANSFORM_FEEDBACK, bound_transform_feedback.get(), bound_transform_feedback_buffer.get());
        } else {
...
void ContextState::RestoreIndexedUniformBufferBindings(const ContextState* prev_state)
{
    if (!feature_info_->IsES3Capable())
        return;
    indexed_uniform_buffer_bindings->RestoreBindings(prev_state ? prev_state->indexed_uniform_buffer_bindings.get() : nullptr);
}
...
void ContextState::RestoreAllTextureUnitAndSamplerBindings(const ContextState* prev_state) const
{
    if (!track_texture_and_sampler_units) {
        if (prev_state) {
            if (!prev_state->track_texture_and_sampler_units) {
                texture_units_in_ground_state = prev_state->texture_units_in_ground_state;
                return;
            }
...
{
    DCHECK(active_texture_unit < texture_units.size() || (active_texture_unit == 0 && !track_texture_and_sampler_units));
    GLuint service_id = 0;
    if (track_texture_and_sampler_units) {
...
{
    for (size_t attrib = 0; attrib < vertex_attrib_manager->num_attribs(); ++attrib) {
...
void ContextState::RestoreVertexAttribArrays(const scoped_refptr<VertexAttribManager> attrib_manager) const
{
    ...
    for (size_t attrib_index = 0; attrib_index < attrib_manager->num_attribs(); ++attrib_index) {
...
void ContextState::RestoreVertexAttribs(const ContextState* prev_state) const
{
    ...
    if (feature_info_->feature_flags().native_vertex_array_object) {
        ...
        if (prev_state && prev_state->feature_info_->feature_flags().native_vertex_array_object
...
{
    if (pname == GL_PRIMITIVE_RESTART_FIXED_INDEX && feature_info_->feature_flags().emulate_primitive_restart_fixed_index) {
...
void ContextState::SetWindowRectangles(GLenum mode, size_t count, const volatile GLint* box)
{
...
void ContextState::UpdateWindowRectangles() const
{
    ...
    if (current_draw_framebuffer_client_id == 0) {
        ...
        DCHECK_LE(static_cast<size_t>(num_window_rectangles), GetMaxWindowRectangles());
        const GLint* data = num_window_rectangles ? window_rectangles_.data() : nullptr;
        api()->glWindowRectanglesEXTFn(window_rectangles_mode, num_window_rectangles, data);
    }
}

void ContextState::UpdateWindowRectanglesForBoundDrawFramebufferClientID(GLuint client_id)
{
...
void ContextState::RemoveBoundBuffer(Buffer* buffer)
{
    ...
    if (bound_transform_feedback.get()) {
        bound_transform_feedback->RemoveBoundBuffer(GL_TRANSFORM_FEEDBACK_BUFFER, buffer, bound_transform_feedback_buffer.get(), !context_lost_);
    }
    ...
    if (indexed_uniform_buffer_bindings) {
        indexed_uniform_buffer_bindings->RemoveBoundBuffer(GL_UNIFORM_BUFFER, buffer, bound_uniform_buffer.get(), !context_lost_);
    }
}
