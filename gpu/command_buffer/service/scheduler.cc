...
Scheduler::Task::Task(
    SequenceId sequence_id, TaskCallback task_callback, std::vector<SyncToken> sync_token_fences, const SyncToken& release, ReportingCallback report_callback)
...
Scheduler::Task::Task(SequenceId sequence_id, base::OnceClosure task_closure, std::vector<SyncToken> sync_token_fences, const SyncToken& release,
...
Scheduler::Task::Task(SequenceId sequence_id, base::OnceClosure task_closure, std::vector<SyncToken> sync_token_fences, ReportingCallback report_callback)
    : Task(sequence_id, std::move(task_closure), std::move(sync_token_fences),
...
Scheduler::ScopedSetSequencePriority::ScopedSetSequencePriority(Scheduler* scheduler, SequenceId sequence_id, SchedulingPriority priority)
...
{
    scheduler_->SetSequencePriority(sequence_id_, scheduler_->GetSequenceDefaultPriority(sequence_id_));
}

Scheduler::SchedulingState::SchedulingState() = default;
Scheduler::SchedulingState::SchedulingState(const SchedulingState& other) = default;
Scheduler::SchedulingState::~SchedulingState() = default;

void Scheduler::SchedulingState::WriteIntoTrace(perfetto::TracedValue context) const
{
...
bool Scheduler::SchedulingState::operator==(const Scheduler::SchedulingState& rhs) const
{
    return std::tie(sequence_id, priority, order_num) == std::tie(rhs.sequence_id, rhs.priority, rhs.order_num);
}

Scheduler::PerThreadState::PerThreadState() = default;
Scheduler::PerThreadState::PerThreadState(PerThreadState&& other) = default;
Scheduler::PerThreadState::~PerThreadState() = default;
Scheduler::PerThreadState& Scheduler::PerThreadState::operator=(PerThreadState&& other) = default;

Scheduler::Sequence::Sequence(Scheduler* scheduler, scoped_refptr<base::SingleThreadTaskRunner> task_runner, SchedulingPriority priority,
    CommandBufferNamespace namespace_id, CommandBufferId command_buffer_id)
    : TaskGraph::Sequence(
        &scheduler->task_graph_, base::BindRepeating(&Sequence::OnFrontTaskUnblocked, base::Unretained(this)), task_runner, namespace_id, command_buffer_id)
    , scheduler_(scheduler)
...
Scheduler::Sequence::~Sequence()
{
    for (const auto& wait_fence : wait_fences_) {
        Sequence* release_sequence = scheduler_->GetSequence(wait_fence.release_sequence_id);
        if (release_sequence) {
...
bool Scheduler::Sequence::ShouldYieldTo(const Sequence* other) const
{
    ...
    return SchedulingState::RunsBefore(other->scheduling_state_, scheduling_state_);
}
...
void Scheduler::Sequence::SetEnabled(bool enabled)
{
    ...
    enabled_ = enabled;
    if (enabled) {
        TRACE_EVENT_NESTABLE_ASYNC_BEGIN1("gpu", "SequenceEnabled", TRACE_ID_LOCAL(this), "sequence_id", sequence_id_.GetUnsafeValue());
    } else {
        TRACE_EVENT_NESTABLE_ASYNC_END1("gpu", "SequenceEnabled", TRACE_ID_LOCAL(this), "sequence_id", sequence_id_.GetUnsafeValue());
    }
...
uint32_t Scheduler::Sequence::AddTask(
    base::OnceClosure task_closure, std::vector<SyncToken> wait_fences, const SyncToken& release, TaskGraph::ReportingCallback report_callback)
{
    uint32_t order_num = TaskGraph::Sequence::AddTask(std::move(task_closure), std::move(wait_fences), release, std::move(report_callback));

    TRACE_EVENT_WITH_FLOW0("gpu,toplevel.flow", "Scheduler::ScheduleTask", GetTaskFlowId(sequence_id_.value(), order_num), TRACE_EVENT_FLAG_FLOW_OUT);
    return order_num;
}
...
SequenceId Scheduler::CreateSequence(SchedulingPriority priority, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
{
    return CreateSequence(priority, std::move(task_runner), CommandBufferNamespace::INVALID,
        /*command_buffer_id=*/ {});
}

SequenceId Scheduler::CreateSequence(SchedulingPriority priority, scoped_refptr<base::SingleThreadTaskRunner> task_runner, CommandBufferNamespace namespace_id,
    CommandBufferId command_buffer_id)
{
    auto sequence = std::make_unique<Sequence>(this, std::move(task_runner), priority, namespace_id, command_buffer_id);
...
ScopedSyncPointClientState Scheduler::CreateSyncPointClientState(SequenceId sequence_id, CommandBufferNamespace namespace_id, CommandBufferId command_buffer_id)
{
    return task_graph_.CreateSyncPointClientState(sequence_id, namespace_id, command_buffer_id);
}
...
SchedulingPriority Scheduler::GetSequenceDefaultPriority(SequenceId sequence_id)
{
    base::AutoLock auto_lock(lock());
    Sequence* sequence = GetSequence(sequence_id);
    if (sequence) {
...
void Scheduler::SetSequencePriority(SequenceId sequence_id, SchedulingPriority priority)
{
    base::AutoLock auto_lock(lock());
    Sequence* sequence = GetSequence(sequence_id);
    if (sequence) {
...
void Scheduler::ScheduleTaskHelper(Task task)
{
    ...
    if (task.task_callback) {
        sequence->AddTask(std::move(task.task_callback), std::move(task.sync_token_fences), task.release, std::move(task.report_callback));
    } else {
        sequence->AddTask(std::move(task.task_closure), std::move(task.sync_token_fences), task.release, std::move(task.report_callback));
    }

    TryScheduleSequence(sequence);
}

void Scheduler::ContinueTask(SequenceId sequence_id, TaskCallback task_callback)
{
    base::AutoLock auto_lock(lock());
    Sequence* sequence = GetSequence(sequence_id);
    DCHECK(sequence);
...
void Scheduler::ContinueTask(SequenceId sequence_id, base::OnceClosure task_closure)
{
    base::AutoLock auto_lock(lock());
    Sequence* sequence = GetSequence(sequence_id);
    DCHECK(sequence);
...
base::SingleThreadTaskRunner* Scheduler::GetTaskRunnerForTesting(SequenceId sequence_id)
{
    base::AutoLock auto_lock(lock());
...
void Scheduler::TryScheduleSequence(Sequence* sequence)
{
    ...
    DVLOG(10) << "Trying to schedule or wake up sequence " << sequence->sequence_id().value() << ". running: " << sequence->running() << ".";
    ...
    if (sequence->running()) {
        ...
        if (!thread_state.running && HasAnyUnblockedTasksOnRunner(task_runner)) {
            TRACE_EVENT_NESTABLE_ASYNC_BEGIN0("gpu", "Scheduler::Running", TRACE_ID_LOCAL(this));
            ...
            task_runner->PostTask(FROM_HERE, base::BindOnce(&Scheduler::RunNextTask, base::Unretained(this)));
        }
    }
}

const std::vector<Scheduler::SchedulingState>& Scheduler::GetSortedRunnableSequences(base::SingleThreadTaskRunner* task_runner)
{
    auto& thread_state = per_thread_state_map_[task_runner];
    std::vector<SchedulingState>& sorted_sequences = thread_state.sorted_sequences;
    ...
    std::stable_sort(sorted_sequences.begin(), sorted_sequences.end(), &SchedulingState::RunsBefore);
    return sorted_sequences;
}

bool Scheduler::HasAnyUnblockedTasksOnRunner(const base::SingleThreadTaskRunner* task_runner) const
{
    ...
    for (const auto& [_, sequence] : scheduler_sequence_map_) {
        CHECK(sequence);
        if (sequence->task_runner() == task_runner && sequence->enabled() && sequence->IsFrontTaskUnblocked()) {
            return true;
        }
    }
...
Scheduler::Sequence* Scheduler::FindNextTaskFromRoot(Sequence* root_sequence)
{
    ...
    VLOG_IF(10, !root_sequence->enabled()) << "Sequence " << root_sequence->sequence_id() << " is not enabled!";
    DVLOG_IF(10, !root_sequence->HasTasks()) << "Sequence " << root_sequence->sequence_id() << " does not have any tasks!";
    ...
    DVLOG(10) << "Sequence " << root_sequence->sequence_id() << " (order_num: " << first_task_order_num << ") has " << root_sequence->wait_fences_.size()
              << " waits.";

    for (auto fence_iter = root_sequence->wait_fences_.begin();
         fence_iter != root_sequence->wait_fences_.end() && fence_iter->order_num <= first_task_order_num; ++fence_iter) {
        // Recurse into the dependent sequence. If a subtask was found, then
        // we're done.
        DVLOG(10) << "Recursing into dependency in sequence " << fence_iter->release_sequence_id << " (order_num: " << fence_iter->order_num << ").";
        ...
        if (release_sequence && release_sequence->HasTasksAndEnabled() && release_sequence->tasks_.front().order_num >= fence_iter->order_num) {
            continue;
        }
        if (Sequence* result = FindNextTaskFromRoot(release_sequence); result != nullptr) {
            return result;
        }
    }
    // It's possible that none of root_sequence's dependencies can be run
    // because they are tied to another thread.
    const bool are_dependencies_done = root_sequence->wait_fences_.empty() || root_sequence->wait_fences_.begin()->order_num > first_task_order_num;
    // Return |root_sequence| only if its dependencies are done, and if it can
    // run on the current thread.
    DVLOG_IF(10, root_sequence->task_runner() != base::SingleThreadTaskRunner::GetCurrentDefault().get())
        << "Will not run sequence because it does not belong to this thread.";
    if (are_dependencies_done && root_sequence->task_runner() == base::SingleThreadTaskRunner::GetCurrentDefault().get()) {
        return root_sequence;
    } else {
        DVLOG_IF(10, !are_dependencies_done) << "Sequence " << root_sequence->sequence_id() << "'s dependencies are not yet done.";
        return nullptr;
    }
}
...
Scheduler::Sequence* Scheduler::FindNextTask()
{
    ...
    for (const SchedulingState& state : sorted_sequences) {
        ...
        DVLOG(10) << "FindNextTask: Calling FindNextTaskFromRoot on sequence " << root_sequence->sequence_id().value();
        if (Sequence* sequence = FindNextTaskFromRoot(root_sequence); sequence != nullptr) {
            return sequence;
        }
    }
...
void Scheduler::RunNextTask()
{
...

    {
      ...
      */
            ...
            TRACE_EVENT_NESTABLE_ASYNC_END0("gpu", "Scheduler::Running", TRACE_ID_LOCAL(this));
        ...
        DCHECK(sequence->task_runner() == task_runner) << "FindNextTaskFromRoot returned sequence that does not belong to "
                                                          "this thread.";
...
void Scheduler::ExecuteSequence(const SequenceId sequence_id)
{
    ...
    if (log_histograms) {
        UMA_HISTOGRAM_CUSTOM_MICROSECONDS_TIMES("GPU.SchedulerDfs.ThreadSuspendedTime", base::TimeTicks::Now() - thread_state->run_next_task_scheduled,
    ...
    if (log_histograms) {
        UMA_HISTOGRAM_CUSTOM_MICROSECONDS_TIMES(
            "GPU.SchedulerDfs.TaskDependencyTime", sequence->FrontTaskWaitingDependencyDelta(), base::Microseconds(10), base::Seconds(30), 100);

        UMA_HISTOGRAM_CUSTOM_MICROSECONDS_TIMES(
            "GPU.SchedulerDfs.TaskSchedulingDelayTime", sequence->FrontTaskSchedulingDelay(), base::Microseconds(10), base::Seconds(30), 100);
    }
    ...
    TRACE_EVENT_WITH_FLOW0("gpu,toplevel.flow", "Scheduler::RunNextTask", GetTaskFlowId(sequence_id.value(), order_num), TRACE_EVENT_FLAG_FLOW_IN);
    ...
    base::TimeDelta blocked_time;
    {
        ...
        if (order_data->IsProcessingOrderNumber()) {
            if (release.HasData()) {
                task_graph_.sync_point_manager()->EnsureFenceSyncReleased(release, ReleaseCause::kTaskCompletionRelease);
            }
