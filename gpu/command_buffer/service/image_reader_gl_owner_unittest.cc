...
protected:
    void SetUp() override
    {
        ...
        auto context_state = base::MakeRefCounted<SharedContextState>(
            share_group_, surface, context_, false /* use_virtualized_gl_contexts */, base::DoNothing(), GrContextType::kGL);
        ...
        image_reader_ = new ImageReaderGLOwner(std::move(texture), SecureMode(), std::move(context_state),
            features::NeedThreadSafeAndroidMedia() ? base::MakeRefCounted<gpu::RefCountedLockForTest>() : nullptr, TextureOwnerCodecType::kMediaCodec);
    }
...
TEST_F(ImageReaderGLOwnerTest, DestructionWorksWithWrongContext)
{
    ...
    scoped_refptr<gl::GLSurface> new_surface(new gl::PbufferGLSurfaceEGL(gl::GLSurfaceEGL::GetGLDisplayEGL(), gfx::Size(320, 240)));
    new_surface->Initialize();

    scoped_refptr<gl::GLShareGroup> new_share_group(new gl::GLShareGroup());
    scoped_refptr<gl::GLContext> new_context(new gl::GLContextEGL(new_share_group.get()));
...
TEST_F(ImageReaderGLOwnerTest, MaxImageExpectation)
{
    ...
    EXPECT_EQ(static_cast<ImageReaderGLOwner*>(image_reader_.get())->max_images_for_testing(), features::LimitAImageReaderMaxSizeToOne() ? 1 : 2);
}

class ImageReaderGLOwnerSecureSurfaceControlTest : public ImageReaderGLOwnerTest {
public:
...
TEST_F(ImageReaderGLOwnerSecureSurfaceControlTest, CreatesSecureAImageReader)
{
    ...
    ASSERT_TRUE(image_reader_);
    auto* a_image_reader = static_cast<ImageReaderGLOwner*>(image_reader_.get())->image_reader_for_testing();
...
TEST_F(ImageReaderGLOwnerSecureSurfaceControlTest, MaxImageExpectation)
{
    if (!IsImageReaderSupported())
        return;
    EXPECT_EQ(static_cast<ImageReaderGLOwner*>(image_reader_.get())->max_images_for_testing(), 3);
}

class ImageReaderGLOwnerInsecureSurfaceControlTest : public ImageReaderGLOwnerTest {
public:
    TextureOwner::Mode SecureMode() final
    {
