...
void DumpMemoryForImageTransferCacheEntry(
    base::trace_event::ProcessMemoryDump* pmd, const std::string& dump_name, const cc::ServiceImageTransferCacheEntry* entry)
{
    using base::trace_event::MemoryAllocatorDump;
    DCHECK(entry->image());

    MemoryAllocatorDump* dump = pmd->CreateAllocatorDump(dump_name);
    dump->AddScalar(MemoryAllocatorDump::kNameSize, MemoryAllocatorDump::kUnitsBytes, entry->CachedSize());

    GrBackendTexture image_backend_texture;
    if (SkImages::GetBackendTextureFromImage(entry->image(), &image_backend_texture, false /* flushPendingGrContextIO */)) {
...
void DumpMemoryForYUVImageTransferCacheEntry(
    base::trace_event::ProcessMemoryDump* pmd, const std::string& dump_base_name, const cc::ServiceImageTransferCacheEntry* entry)
{
    using base::trace_event::MemoryAllocatorDump;
    DCHECK(entry->image());
    DCHECK(entry->is_yuv());
    ...
    if (plane_sizes.empty()) {
        ...
        MemoryAllocatorDump* dump = pmd->CreateAllocatorDump(dump_base_name + base::StringPrintf("/dma_buf"));
        dump->AddScalar(MemoryAllocatorDump::kNameSize, MemoryAllocatorDump::kUnitsBytes, entry->CachedSize());
    ...
    for (size_t i = 0u; i < entry->num_planes(); ++i) {
        MemoryAllocatorDump* dump = pmd->CreateAllocatorDump(dump_base_name + base::StringPrintf("/plane_%0u", base::checked_cast<uint32_t>(i)));
        ...
        // undesirable.
        GrBackendTexture image_backend_texture;
        if (SkImages::GetBackendTextureFromImage(entry->GetPlaneImage(i), &image_backend_texture, false /* flushPendingGrContextIO */)) {
            GrGLTextureInfo info;
            if (GrBackendTextures::GetGLTextureInfo(image_backend_texture, &info)) {
                auto guid = gl::GetGLTextureRasterGUIDForTracing(info.fID);
                pmd->CreateSharedGlobalAllocatorDump(guid);
...
ServiceTransferCache::CacheEntryInternal::CacheEntryInternal(
    std::optional<ServiceDiscardableHandle> handle, std::unique_ptr<cc::ServiceTransferCacheEntry> entry)
    : handle(handle)
...
ServiceTransferCache::CacheEntryInternal::~CacheEntryInternal()
{
    if (entry) {
        ...
        UMA_HISTOGRAM_LONG_TIMES("GPU.TransferCache.TimeSinceLastUseOnDelete", base::TimeTicks::Now() - last_use);
    }
}

ServiceTransferCache::CacheEntryInternal::CacheEntryInternal(CacheEntryInternal&& other) = default;

ServiceTransferCache::CacheEntryInternal& ServiceTransferCache::CacheEntryInternal::operator=(CacheEntryInternal&& other) = default;

ServiceTransferCache::ServiceTransferCache(const GpuPreferences& preferences, base::RepeatingClosure flush_callback)
    : flush_callback_(std::move(flush_callback))
    , entries_(EntryCache::NO_AUTO_EVICT)
    , cache_size_limit_(preferences.force_gpu_mem_discardable_limit_bytes ? preferences.force_gpu_mem_discardable_limit_bytes : DiscardableCacheSizeLimit())
    , max_cache_entries_(kMaxCacheEntries)
{
    ...
    if (base::SingleThreadTaskRunner::HasCurrentDefault()) {
        base::trace_event::MemoryDumpManager::GetInstance()->RegisterDumpProvider(
            this, "gpu::ServiceTransferCache", base::SingleThreadTaskRunner::GetCurrentDefault());
    }
}
...
bool ServiceTransferCache::CreateLockedEntry(
    const EntryKey& key, ServiceDiscardableHandle handle, GrDirectContext* context, skgpu::graphite::Recorder* graphite_recorder, base::span<uint8_t> data)
{
    ...
    std::unique_ptr<cc::ServiceTransferCacheEntry> entry = cc::ServiceTransferCacheEntry::Create(key.entry_type);
    if (!entry) {
        return false;
    }
...
void ServiceTransferCache::CreateLocalEntry(const EntryKey& key, std::unique_ptr<cc::ServiceTransferCacheEntry> entry)
{
    if (!entry)
        return;
...
template <typename Iterator> Iterator ServiceTransferCache::ForceDeleteEntry(Iterator it)
{
    if (it->second.handle)
...
cc::ServiceTransferCacheEntry* ServiceTransferCache::GetEntry(const EntryKey& key)
{
    ...
    UMA_HISTOGRAM_LONG_TIMES("GPU.TransferCache.TimeSinceLastUse", last_use_delta);
    UMA_HISTOGRAM_LONG_TIMES("GPU.TransferCache.MaxHistoricalTimeSinceLastUse", entry->second.max_last_use_delta);
    return entry->second.entry.get();
}

void ServiceTransferCache::EnforceLimits()
{
    RemoveOldEntriesUntil([&](EntryCache::reverse_iterator it) { return total_size_ <= cache_size_limit_ && entries_.size() <= max_cache_entries_; });
}
...
void ServiceTransferCache::MaybePostPruneOldEntries()
{
    ...
    prune_old_entries_timer_.Start(FROM_HERE, kOldEntryPruneInterval, this, &ServiceTransferCache::PruneOldEntries);
}
...
void ServiceTransferCache::PruneOldEntries()
{
    ...
    int removed_count = RemoveOldEntriesUntil([&](EntryCache::reverse_iterator it) { return now - it->second.last_use < kOldEntryCutoffTimeDelta; });
...
int ServiceTransferCache::RemoveOldEntriesUntil(base::FunctionRef<bool(EntryCache::reverse_iterator)> should_stop)
{
    int removed_count = 0;
...
void ServiceTransferCache::PurgeMemory(base::MemoryPressureListener::MemoryPressureLevel memory_pressure_level)
{
    base::AutoReset<size_t> reset_limit(&cache_size_limit_, DiscardableCacheSizeLimitForPressure(cache_size_limit_, memory_pressure_level));
    EnforceLimits();
}
...
bool ServiceTransferCache::CreateLockedHardwareDecodedImageEntry(int decoder_id, uint32_t entry_id, ServiceDiscardableHandle handle, GrDirectContext* context,
    std::vector<sk_sp<SkImage>> plane_images, SkYUVAInfo::PlaneConfig plane_config, SkYUVAInfo::Subsampling subsampling, SkYUVColorSpace yuv_color_space,
    size_t buffer_byte_size, bool needs_mips)
{
    ...
    if (!entry->BuildFromHardwareDecodedImage(context, std::move(plane_images), plane_config, subsampling, yuv_color_space, buffer_byte_size, needs_mips)) {
        return false;
    }
...
bool ServiceTransferCache::OnMemoryDump(const base::trace_event::MemoryDumpArgs& args, base::trace_event::ProcessMemoryDump* pmd)
{
    using base::trace_event::MemoryAllocatorDump;
    using base::trace_event::MemoryDumpLevelOfDetail;

    if (args.level_of_detail == MemoryDumpLevelOfDetail::kBackground) {
        std::string dump_name = base::StringPrintf("gpu/transfer_cache/cache_0x%" PRIXPTR, reinterpret_cast<uintptr_t>(this));
        MemoryAllocatorDump* dump = pmd->CreateAllocatorDump(dump_name);
        dump->AddScalar(MemoryAllocatorDump::kNameSize, MemoryAllocatorDump::kUnitsBytes, total_image_size_);

        if (total_image_count_ > 0) {
            MemoryAllocatorDump* dump_avg_size = pmd->CreateAllocatorDump(dump_name + "/avg_image_size");
            const size_t avg_image_size = total_image_size_ / (total_image_count_ * 1.0);
            dump_avg_size->AddScalar("average_size", MemoryAllocatorDump::kUnitsBytes, avg_image_size);
        }
    ...
    for (auto it = entries_.begin(); it != entries_.end(); it++) {
        ...
        if (entry_type == cc::TransferCacheEntryType::kImage) {
            image_entry = static_cast<const cc::ServiceImageTransferCacheEntry*>(entry);
        }

        if (image_entry && image_entry->fits_on_gpu()) {
            std::string dump_base_name = base::StringPrintf(
                "gpu/transfer_cache/cache_0x%" PRIXPTR "/gpu/entry_0x%" PRIXPTR, reinterpret_cast<uintptr_t>(this), reinterpret_cast<uintptr_t>(entry));
            if (image_entry->is_yuv()) {
                DumpMemoryForYUVImageTransferCacheEntry(pmd, dump_base_name, image_entry);
            } else {
                DumpMemoryForImageTransferCacheEntry(pmd, dump_base_name, image_entry);
            }
        } else {
            std::string dump_name = base::StringPrintf(
                "gpu/transfer_cache/cache_0x%" PRIXPTR "/cpu/entry_0x%" PRIXPTR, reinterpret_cast<uintptr_t>(this), reinterpret_cast<uintptr_t>(entry));
            MemoryAllocatorDump* dump = pmd->CreateAllocatorDump(dump_name);
            dump->AddScalar(MemoryAllocatorDump::kNameSize, MemoryAllocatorDump::kUnitsBytes, entry->CachedSize());
        }
    }
...
ServiceTransferCache::EntryKey::EntryKey(int decoder_id, cc::TransferCacheEntryType entry_type, uint32_t entry_id)
