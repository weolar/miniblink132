...
protected:
    void SetUpBase(MemoryTracker* memory_tracker, FeatureInfo* feature_info, const char* extensions)
    {
    ...
    void DoBufferData(Buffer* buffer, GLenum target, GLsizeiptr size, GLenum usage, const GLvoid* data, GLenum error)
    {
        TestHelper::DoBufferData(gl_.get(), error_state_.get(), manager_.get(), buffer, target, size, usage, data, error);
    }

    bool DoBufferSubData(Buffer* buffer, GLenum target, GLintptr offset, GLsizeiptr size, const GLvoid* data)
    {
        if (!buffer->CheckRange(offset, size)) {
            return false;
        }
        if (!buffer->IsClientSideArray()) {
            EXPECT_CALL(*gl_, BufferSubData(target, offset, size, _)).Times(1).RetiresOnSaturation();
        }
    ...
    void RunGetMaxValueForRangeUint8Test(bool enable_primitive_restart)
    {
        ...
        DoBufferData(buffer, kTarget, sizeof(data), GL_STATIC_DRAW, nullptr, GL_NO_ERROR);
        EXPECT_TRUE(DoBufferSubData(buffer, kTarget, 0, sizeof(data), data));
        GLuint max_value;
        // Check entire range succeeds.
        EXPECT_TRUE(buffer->GetMaxValueForRange(0, 10, GL_UNSIGNED_BYTE, enable_primitive_restart, &max_value));
        ...
        EXPECT_TRUE(buffer->GetMaxValueForRange(4, 3, GL_UNSIGNED_BYTE, enable_primitive_restart, &max_value));
        if (enable_primitive_restart) {
        ...
        EXPECT_FALSE(buffer->GetMaxValueForRange(0, 11, GL_UNSIGNED_BYTE, enable_primitive_restart, &max_value));
        EXPECT_FALSE(buffer->GetMaxValueForRange(10, 1, GL_UNSIGNED_BYTE, enable_primitive_restart, &max_value));
    }
    ...
    void RunGetMaxValueForRangeUint16Test(bool enable_primitive_restart)
    {
        ...
        GLuint max_value;
        // Check entire range succeeds.
        EXPECT_TRUE(buffer->GetMaxValueForRange(0, 10, GL_UNSIGNED_SHORT, enable_primitive_restart, &max_value));
        if (enable_primitive_restart) {
            EXPECT_EQ(10u, max_value);
        } else {
        ...
        EXPECT_FALSE(buffer->GetMaxValueForRange(1, 10, GL_UNSIGNED_SHORT, enable_primitive_restart, &max_value));
        // Check sub range succeeds.
        EXPECT_TRUE(buffer->GetMaxValueForRange(8, 3, GL_UNSIGNED_SHORT, enable_primitive_restart, &max_value));
        if (enable_primitive_restart) {
            EXPECT_EQ(6u, max_value);
        } else {
            EXPECT_EQ(0xFFFFu, max_value);
        ...
        EXPECT_EQ(120u, max_value);
        // Check out of range fails.
        EXPECT_FALSE(buffer->GetMaxValueForRange(0, 11, GL_UNSIGNED_SHORT, enable_primitive_restart, &max_value));
        EXPECT_FALSE(buffer->GetMaxValueForRange(20, 1, GL_UNSIGNED_SHORT, enable_primitive_restart, &max_value));
    }
    ...
    void RunGetMaxValueForRangeUint32Test(bool enable_primitive_restart)
    {
        ...
        GLuint max_value;
        // Check entire range succeeds.
        EXPECT_TRUE(buffer->GetMaxValueForRange(0, 10, GL_UNSIGNED_INT, enable_primitive_restart, &max_value));
        if (enable_primitive_restart) {
            EXPECT_EQ(10u, max_value);
        } else {
        ...
        EXPECT_FALSE(buffer->GetMaxValueForRange(1, 10, GL_UNSIGNED_INT, enable_primitive_restart, &max_value));
        EXPECT_FALSE(buffer->GetMaxValueForRange(2, 10, GL_UNSIGNED_INT, enable_primitive_restart, &max_value));
        EXPECT_FALSE(buffer->GetMaxValueForRange(3, 10, GL_UNSIGNED_INT, enable_primitive_restart, &max_value));
        // Check sub range succeeds.
        EXPECT_TRUE(buffer->GetMaxValueForRange(16, 3, GL_UNSIGNED_INT, enable_primitive_restart, &max_value));
        if (enable_primitive_restart) {
            EXPECT_EQ(6u, max_value);
        } else {
            EXPECT_EQ(0xFFFFFFFFu, max_value);
        ...
        EXPECT_EQ(120u, max_value);
        // Check out of range fails.
        EXPECT_FALSE(buffer->GetMaxValueForRange(0, 11, GL_UNSIGNED_INT, enable_primitive_restart, &max_value));
        EXPECT_FALSE(buffer->GetMaxValueForRange(40, 1, GL_UNSIGNED_INT, enable_primitive_restart, &max_value));
    }
...
protected:
    void SetUp() override
    {
        ...
        feature_info_ = new FeatureInfo(gpu_driver_bug_workarounds, gpu_feature_info);
...
TEST_F(BufferManagerTest, Basic)
{
    ...
    DoBufferData(buffer1, kTarget, kBuffer1Size, GL_DYNAMIC_DRAW, nullptr, GL_NO_ERROR);
    ...
    EXPECT_CALL(*gl_, DeleteBuffersARB(1, ::testing::Pointee(kServiceBuffer1Id))).Times(1).RetiresOnSaturation();
...
TEST_F(BufferManagerMemoryTrackerTest, Basic)
{
    ...
    DoBufferData(buffer1, kTarget, kBuffer1Size1, GL_DYNAMIC_DRAW, nullptr, GL_NO_ERROR);
    EXPECT_MEMORY_ALLOCATION_CHANGE(kBuffer1Size1, 0);
    EXPECT_MEMORY_ALLOCATION_CHANGE(0, kBuffer1Size2);
    DoBufferData(buffer1, kTarget, kBuffer1Size2, GL_DYNAMIC_DRAW, nullptr, GL_NO_ERROR);
...
TEST_F(BufferManagerTest, Destroy)
{
    ...
    ASSERT_TRUE(buffer1 != nullptr);
    EXPECT_CALL(*gl_, DeleteBuffersARB(1, ::testing::Pointee(kService1Id))).Times(1).RetiresOnSaturation();
    manager_->Destroy();
...
TEST_F(BufferManagerTest, GetRange)
{
    ...
    ASSERT_TRUE(buffer != nullptr);
    manager_->SetTarget(buffer, kTarget);
    DoBufferData(buffer, kTarget, kDataSize, GL_STATIC_DRAW, nullptr, GL_NO_ERROR);
...
TEST_F(BufferManagerTest, UseDeletedBuffer)
{
    ...
    DoBufferData(buffer.get(), kTarget, kDataSize, GL_STATIC_DRAW, nullptr, GL_NO_ERROR);
    // Check that it gets deleted when the last reference is released.
    EXPECT_CALL(*gl_, DeleteBuffersARB(1, ::testing::Pointee(kServiceBufferId))).Times(1).RetiresOnSaturation();
    buffer = nullptr;
}
...
TEST_F(BufferManagerClientSideArraysTest, StreamBuffersAreShadowed)
{
    ...
    ASSERT_TRUE(buffer != nullptr);
    manager_->SetTarget(buffer, kTarget);
    DoBufferData(buffer, kTarget, sizeof(data), GL_STREAM_DRAW, data, GL_NO_ERROR);
    EXPECT_TRUE(buffer->IsClientSideArray());
    EXPECT_EQ(0, memcmp(data, buffer->GetRange(0, sizeof(data)), sizeof(data)));
    DoBufferData(buffer, kTarget, sizeof(data), GL_DYNAMIC_DRAW, data, GL_NO_ERROR);
...
TEST_F(BufferManagerTest, MaxValueCacheClearedCorrectly)
{
    ...
    DoBufferData(buffer, kTarget, sizeof(data1), GL_STATIC_DRAW, data1, GL_NO_ERROR);
    EXPECT_TRUE(buffer->GetMaxValueForRange(0, 10, GL_UNSIGNED_INT, false, &max_value));
    EXPECT_EQ(10u, max_value);
    ...
    DoBufferData(buffer, kTarget, sizeof(data2), GL_STATIC_DRAW, data2, GL_NO_ERROR);
    EXPECT_TRUE(buffer->GetMaxValueForRange(0, 10, GL_UNSIGNED_INT, false, &max_value));
    EXPECT_EQ(20u, max_value);
    // Check that any cached values are invalidated if the buffer is reloaded
    // with entirely different content.
    ASSERT_NE(sizeof(data3), sizeof(data1));
    DoBufferData(buffer, kTarget, sizeof(data3), GL_STATIC_DRAW, data3, GL_NO_ERROR);
    EXPECT_TRUE(buffer->GetMaxValueForRange(0, 3, GL_UNSIGNED_INT, false, &max_value));
...
TEST_F(BufferManagerTest, BindBufferConflicts)
{
...

    {
        ...
        const GLenum kTargets[] = { GL_ARRAY_BUFFER, GL_COPY_READ_BUFFER, GL_COPY_WRITE_BUFFER, GL_PIXEL_PACK_BUFFER, GL_PIXEL_UNPACK_BUFFER,
