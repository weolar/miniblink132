...
base::flat_map<gfx::BufferFormat, std::vector<uint64_t>> GetDawnModifierMap(wgpu::Adapter adapter)
{
    ...
    for (int i = 0; i <= static_cast<int>(gfx::BufferFormat::LAST); i++) {
        ...
        for (size_t j = 0; j < drmCapabilities.propertiesCount; j++) {
            if (!base::Contains(kModifierBlocklist, drmCapabilities.properties[j].modifier)) {
...
void PopulateDawnDrmFormatsAndModifiers(wgpu::Adapter adapter, base::flat_map<uint32_t, std::vector<uint64_t>>& fourcc_modifier_map)
{
...
std::vector<uint64_t> DrmModifiersFilterDawn::Filter(gfx::BufferFormat format, const std::vector<uint64_t>& modifiers)
{
