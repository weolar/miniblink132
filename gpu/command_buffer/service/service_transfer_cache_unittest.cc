...
std::unique_ptr<cc::ServiceTransferCacheEntry> CreateEntry(size_t size)
{
    ...
    entry->Deserialize(/*gr_context=*/nullptr, /*graphite_recorder=*/nullptr, data);
    return entry;
}
...
TEST(ServiceTransferCacheTest, EnforcesOnPurgeMemory)
{
    ...
    cache.CreateLocalEntry(ServiceTransferCache::EntryKey(kDecoderId, kEntryType, ++entry_id), CreateEntry(entry_size));
    EXPECT_EQ(cache.cache_size_for_testing(), entry_size);
    cache.PurgeMemory(base::MemoryPressureListener::MEMORY_PRESSURE_LEVEL_CRITICAL);
    ...
    cache.PurgeMemory(base::MemoryPressureListener::MEMORY_PRESSURE_LEVEL_MODERATE);
...
TEST(ServiceTransferCache, MultipleDecoderUse)
{
    ...
    cache.CreateLocalEntry(ServiceTransferCache::EntryKey(decoder1, kEntryType, entry_id), std::move(decoder_1_entry));

    // Decoder 2 entry.
    int decoder2 = 2;
    auto decoder_2_entry = CreateEntry(entry_size);
    auto* decoder_2_entry_ptr = decoder_2_entry.get();
    cache.CreateLocalEntry(ServiceTransferCache::EntryKey(decoder2, kEntryType, entry_id), std::move(decoder_2_entry));

    EXPECT_EQ(decoder_1_entry_ptr, cache.GetEntry(ServiceTransferCache::EntryKey(decoder1, kEntryType, entry_id)));
    EXPECT_EQ(decoder_2_entry_ptr, cache.GetEntry(ServiceTransferCache::EntryKey(decoder2, kEntryType, entry_id)));
}
...
TEST(ServiceTransferCache, DeleteEntriesForDecoder)
{
    ...
    cache.CreateLocalEntry(ServiceTransferCache::EntryKey(1, kEntryType, 1), CreateEntry(entry_size));
    cache.CreateLocalEntry(ServiceTransferCache::EntryKey(1, kEntryType, 2), CreateEntry(entry_size));

    // Add 2 entries for decoder 2.
    cache.CreateLocalEntry(ServiceTransferCache::EntryKey(2, kEntryType, 1), CreateEntry(entry_size));
    cache.CreateLocalEntry(ServiceTransferCache::EntryKey(2, kEntryType, 2), CreateEntry(entry_size));
    ...
    EXPECT_NE(cache.GetEntry(ServiceTransferCache::EntryKey(2, kEntryType, 1)), nullptr);
    EXPECT_NE(cache.GetEntry(ServiceTransferCache::EntryKey(2, kEntryType, 2)), nullptr);
}
...
TEST(ServiceTransferCacheTest, PurgeEntryOnTimer)
{
    ...
    ServiceTransferCache cache { GpuPreferences(), base::BindLambdaForTesting([&]() { flush_called = true; }) };
