...
class GPU_GLES2_EXPORT SharedContextState : public base::trace_event::MemoryDumpProvider,
                                            public gpu::GLContextVirtualDelegate,
                                            public base::RefCounted<SharedContextState>,
                                            public GrContextOptions::ShaderErrorHandler {
public:
    using ContextLostCallback = base::OnceCallback<void(bool, error::ContextLostReason)>;
    ...
    SharedContextState(scoped_refptr<gl::GLShareGroup> share_group, scoped_refptr<gl::GLSurface> surface, scoped_refptr<gl::GLContext> context,
        bool use_virtualized_gl_contexts, ContextLostCallback context_lost_callback, GrContextType gr_context_type,
        viz::VulkanContextProvider* vulkan_context_provider = nullptr, viz::MetalContextProvider* metal_context_provider = nullptr,
        DawnContextProvider* dawn_context_provider = nullptr, base::WeakPtr<gpu::MemoryTracker::Observer> peak_memory_monitor = nullptr,
    ...
    bool InitializeSkia(const GpuPreferences& gpu_preferences, const GpuDriverBugWorkarounds& workarounds, gpu::raster::GrShaderCache* cache = nullptr,
        GpuProcessShmCount* use_shader_cache_shm_count = nullptr, gl::ProgressReporter* progress_reporter = nullptr);
    ...
    void SubmitIfNecessary(std::vector<GrBackendSemaphore> signal_semaphores, bool need_graphite_submit);
    ...
    void PurgeMemory(base::MemoryPressureListener::MemoryPressureLevel memory_pressure_level);
    ...
    void UseShaderCache(std::optional<gpu::raster::GrShaderCache::ScopedCacheUse>& cache_use, int32_t client_id) const;

    GLFormatCaps GetGLFormatCaps()
    {
        return GLFormatCaps(feature_info());
    }
    ...
    gl::GLContext* real_context() const
    {
        return real_context_.get();
    }
    ...
    gl::ProgressReporter* progress_reporter() const
    {
        return progress_reporter_;
    }
    ...
    gles2::ContextState* context_state() const
    {
    ...
    bool need_context_state_reset() const
    {
        return need_context_state_reset_;
    }
...
private:
    ...
    class GPU_GLES2_EXPORT MemoryTrackerObserver : public gpu::MemoryTracker::Observer {
    public:
        explicit MemoryTrackerObserver(base::WeakPtr<gpu::MemoryTracker::Observer> peak_memory_monitor);
    ...
    bool InitializeGanesh(const GpuPreferences& gpu_preferences, const GpuDriverBugWorkarounds& workarounds, gpu::raster::GrShaderCache* cache,
        GpuProcessShmCount* use_shader_cache_shm_count = nullptr, gl::ProgressReporter* progress_reporter = nullptr);

    bool InitializeGraphite(const GpuPreferences& gpu_preferences, const GpuDriverBugWorkarounds& workarounds);
    ...
    raw_ptr<skgpu::graphite::Context, DanglingUntriaged> graphite_context_ = nullptr;
    ...
    raw_ptr<gpu::raster::GrShaderCache, DanglingUntriaged> gr_shader_cache_ = nullptr;
    raw_ptr<GpuProcessShmCount, DanglingUntriaged> use_shader_cache_shm_count_ = nullptr;
    ...
    scoped_refptr<raster::GraphiteCacheController> gpu_main_graphite_cache_controller_;
