...
// format on Dawn.
GPU_GLES2_EXPORT void PopulateDawnDrmFormatsAndModifiers(wgpu::Adapter adapter, base::flat_map<uint32_t, std::vector<uint64_t>>& fourcc_modifier_map);
...
public:
    ...
    std::vector<uint64_t> Filter(gfx::BufferFormat format, const std::vector<uint64_t>& modifiers) override;
