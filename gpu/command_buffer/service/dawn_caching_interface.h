...
class GPU_GLES2_EXPORT DawnCachingBackend : public base::RefCounted<DawnCachingBackend> {
public:
...
private:
    ...
    friend bool operator<(const std::unique_ptr<Entry>& lhs, const std::unique_ptr<Entry>& rhs);
    friend bool operator<(const std::unique_ptr<Entry>& lhs, const std::string& rhs);
    friend bool operator<(const std::string& lhs, const std::unique_ptr<Entry>& rhs);
...
class GPU_GLES2_EXPORT DawnCachingInterface : public dawn::platform::CachingInterface {
public:
    using CacheBlobCallback = base::RepeatingCallback<void(gpu::GpuDiskCacheType type, const std::string& key, const std::string& blob)>;

    ~DawnCachingInterface() override;

    size_t LoadData(const void* key, size_t key_size, void* value_out, size_t value_size) override;

    void StoreData(const void* key, size_t key_size, const void* value, size_t value_size) override;

private:
    friend class DawnCachingInterfaceFactory;

    // Simplified accessor to the backend.
    detail::DawnCachingBackend* backend()
    {
        return backend_.get();
    }
    ...
    // the factory.
    explicit DawnCachingInterface(scoped_refptr<detail::DawnCachingBackend> backend, CacheBlobCallback callback = {});
...
public:
    ...
    using BackendFactory = base::RepeatingCallback<scoped_refptr<detail::DawnCachingBackend>()>;
    ...
    std::unique_ptr<DawnCachingInterface> CreateInstance(const gpu::GpuDiskCacheHandle& handle, DawnCachingInterface::CacheBlobCallback callback = {});
...
private:
    ...
    static scoped_refptr<detail::DawnCachingBackend> CreateDefaultInMemoryBackend();

    // Factory to create backends.
    BackendFactory backend_factory_;

    // Map that holds existing backends.
    base::flat_map<gpu::GpuDiskCacheHandle, scoped_refptr<detail::DawnCachingBackend>> backends_;
};
