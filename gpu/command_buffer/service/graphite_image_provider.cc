...
GraphiteImageProvider::ImageHolder::ImageHolder(sk_sp<SkImage> image)
    ...
    , last_use_time(base::TimeTicks::Now())
{
}
GraphiteImageProvider::ImageHolder::ImageHolder(ImageHolder&&) = default;
GraphiteImageProvider::ImageHolder& GraphiteImageProvider::ImageHolder::operator=(ImageHolder&&) = default;
...
sk_sp<SkImage> GraphiteImageProvider::findOrCreate(skgpu::graphite::Recorder* recorder, const SkImage* image, SkImage::RequiredProperties required_props)
{
    ...
    UMA_HISTOGRAM_BOOLEAN("Gpu.Graphite.GraphiteImageProviderAccessHitInCache", hit_in_cache);
    ...
    sk_sp<SkImage> new_image = SkImages::TextureFromImage(recorder, image, required_props);
    if (!new_image) {
        return nullptr;
    }
    ...
    CHECK(current_cache_bytes_ <= preferred_max_cache_bytes_ || current_cache_bytes_ == new_image_bytes);

    return new_image;
}

void GraphiteImageProvider::PurgeCacheIfNecessaryToAllowForNewImage(size_t new_bytes)
{
...
void GraphiteImageProvider::PurgeImagesNotUsedSince(base::TimeDelta last_use_delta)
{
    base::TimeTicks now = base::TimeTicks::Now();
    while (!cache_.empty() && cache_.rbegin()->second.last_use_time + last_use_delta < now) {
        current_cache_bytes_ -= cache_.rbegin()->second.image->textureSize();
        cache_.Erase(cache_.rbegin());
    }
}
