...
ProgramCache::ScopedCacheUse::ScopedCacheUse(ProgramCache* cache, CacheProgramCallback callback)
    : cache_(cache)
{
...
bool ProgramCache::HasSuccessfullyCompiledShader(const std::string& shader_signature) const
{
    Hash sha;
...
ProgramCache::LinkedProgramStatus ProgramCache::GetLinkedProgramStatus(const std::string& shader_signature_a, const std::string& shader_signature_b,
    const std::map<std::string, GLint>* bind_attrib_location_map, const std::vector<std::string>& transform_feedback_varyings,
...
void ProgramCache::LinkedProgramCacheSuccess(const std::string& shader_signature_a, const std::string& shader_signature_b,
...
void ProgramCache::Evict(const Hash& program_hash, const Hash& shader_0_hash, const Hash& shader_1_hash)
{
...
void ProgramCache::ComputeProgramHash(HashView hashed_shader_0, HashView hashed_shader_1, const std::map<std::string, GLint>* bind_attrib_location_map,
    const std::vector<std::string>& transform_feedback_varyings, GLenum transform_feedback_buffer_mode, Hash& result) const
{
    ...
    const size_t total_size = shader0_size + shader1_size + angle_commit_size + map_size + var_size + sizeof(transform_feedback_buffer_mode);
    ...
    CHECK(buffer.Write(base::as_byte_span(
        ...
        UNSAFE_BUFFERS(base::span(angle::GetANGLECommitHash(), angle_commit_size)))));

    if (map_size != 0) {
        // copy our map
        for (auto it = bind_attrib_location_map->begin(); it != bind_attrib_location_map->end(); ++it) {
...
void ProgramCache::HandleMemoryPressure(base::MemoryPressureListener::MemoryPressureLevel memory_pressure_level)
{
