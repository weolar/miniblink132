...
protected:
    ...
    void SetupMockEGL(const char* extensions)
    {
        ...
        ON_CALL(*egl_, QueryString(_, EGL_EXTENSIONS)).WillByDefault(Return(extensions));
...
TEST_F(GpuFenceManagerTest, Basic)
{
    ...
    EXPECT_CALL(*egl_, CreateSyncKHR(_, EGL_SYNC_NATIVE_FENCE_ANDROID, nullptr)).Times(1).WillOnce(Return(kDummySync)).RetiresOnSaturation();
    EXPECT_CALL(*egl_, GetSyncAttribKHR(_, kDummySync, EGL_SYNC_STATUS_KHR, _)).WillRepeatedly(DoAll(SetArgPointee<3>(EGL_UNSIGNALED_KHR), Return(EGL_TRUE)));
    ...
    EXPECT_CALL(*egl_, WaitSyncKHR(_, kDummySync, _)).Times(1).WillOnce(Return(EGL_TRUE)).RetiresOnSaturation();
    EXPECT_TRUE(manager_->GpuFenceServerWait(kClient1Id));

    // Removing the fence marks it invalid.
    EXPECT_CALL(*egl_, DestroySyncKHR(_, kDummySync)).Times(1).RetiresOnSaturation();
...
TEST_F(GpuFenceManagerTest, SmartHandleImplmentation)
{
    ...
    EXPECT_CALL(*egl_, DupNativeFenceFDANDROID(_, kDummySync)).Times(1).WillOnce(Return(kFenceFD)).RetiresOnSaturation();
...
TEST_F(GpuFenceManagerTest, Duplication)
{
    ...
    EXPECT_CALL(*egl_, CreateSyncKHR(_, EGL_SYNC_NATIVE_FENCE_ANDROID, _)).Times(1).WillOnce(Return(kDummySync)).RetiresOnSaturation();
    EXPECT_CALL(*egl_, GetSyncAttribKHR(_, kDummySync, EGL_SYNC_STATUS_KHR, _)).WillRepeatedly(DoAll(SetArgPointee<3>(EGL_UNSIGNALED_KHR), Return(EGL_TRUE)));
    EXPECT_CALL(*gl_, Flush()).Times(1).RetiresOnSaturation();
    EXPECT_TRUE(manager_->CreateGpuFenceFromHandle(kClient1Id, std::move(handle)));
    EXPECT_TRUE(manager_->IsValidGpuFence(kClient1Id));
