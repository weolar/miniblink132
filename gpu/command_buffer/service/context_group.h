...
public:
    ContextGroup(const GpuPreferences& gpu_preferences, bool supports_passthrough_command_decoders, std::unique_ptr<MemoryTracker> memory_tracker,
    ...
    gpu::ContextResult Initialize(DecoderContext* decoder, ContextType context_type, const DisallowedFeatures& disallowed_features);
    ...
    uint32_t max_texture_units() const
    {
        return max_texture_units_;
    }

    uint32_t max_texture_image_units() const
    {
    ...
    {
        return min_program_texel_offset_;
    }

    int32_t max_program_texel_offset() const
    {
        return max_program_texel_offset_;
    }
    ...
    SharedImageRepresentationFactory* shared_image_representation_factory() const
    {
    ...
    {
        std::unordered_map<GLuint, GLsync>::const_iterator iter = syncs_id_map_.find(client_id);
