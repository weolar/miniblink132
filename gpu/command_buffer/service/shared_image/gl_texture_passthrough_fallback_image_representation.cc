...
namespace gpu {
namespace {
GLFormatDesc GetGLFormatDesc(viz::SharedImageFormat format, int plane_index, const GLFormatCaps& gl_format_caps)
{
...
scoped_refptr<gles2::TexturePassthrough> CreateGLTexture(const GLFormatDesc& format_desc, const gfx::Size& size, gl::ProgressReporter* progress_reporter)
{
    ...
    api->glTexImage2DFn(target, 0, format_desc.image_internal_format, size.width(), size.height(), 0, format_desc.data_format, format_desc.data_type, nullptr);

    return base::MakeRefCounted<gles2::TexturePassthrough>(service_id, target);
}
} // namespace

GLTexturePassthroughFallbackImageRepresentation::GLTexturePassthroughFallbackImageRepresentation(SharedImageManager* manager, SharedImageBacking* backing,
    MemoryTypeTracker* tracker, gl::ProgressReporter* progress_reporter, const GLFormatCaps& gl_format_caps)
    : GLTexturePassthroughImageRepresentation(manager, backing, tracker)
{
    for (int plane = 0; plane < format().NumberOfPlanes(); plane++) {
        ...
        const SkImageInfo plane_info = SkImageInfo::Make(plane_size.width(), plane_size.height(), plane_ct, kPremul_SkAlphaType);
        static constexpr size_t kDefaultGLAlignment = 4;
        plane_bitmaps_.emplace_back().allocPixels(plane_info, base::bits::AlignUp(plane_info.minRowBytes(), kDefaultGLAlignment));
        plane_pixmaps_.push_back(plane_bitmaps_.back().pixmap());

        const GLFormatDesc format_desc = GetGLFormatDesc(format(), plane, gl_format_caps);
        plane_textures_.emplace_back(viz::SkColorTypeToSinglePlaneSharedImageFormat(plane_ct), plane_size, /*is_passthrough=*/true, progress_reporter)
            .InitializeWithTexture(format_desc, CreateGLTexture(format_desc, plane_size, progress_reporter));
    }
}

GLTexturePassthroughFallbackImageRepresentation::~GLTexturePassthroughFallbackImageRepresentation() = default;

const scoped_refptr<gles2::TexturePassthrough>& GLTexturePassthroughFallbackImageRepresentation::GetTexturePassthrough(int plane_index)
{
