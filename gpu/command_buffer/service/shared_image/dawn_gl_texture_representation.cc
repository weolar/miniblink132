...
DawnGLTextureRepresentation::DawnGLTextureRepresentation(std::unique_ptr<GLTextureImageRepresentationBase> gl_representation, SharedImageManager* manager,
    SharedImageBacking* backing, MemoryTypeTracker* tracker, wgpu::Device device, std::vector<wgpu::TextureFormat> view_formats)
...
wgpu::Texture DawnGLTextureRepresentation::BeginAccess(wgpu::TextureUsage usage, wgpu::TextureUsage internal_usage)
{
    ...
    externalImageDesc.texture = gl_representation_->GetTextureBase()->service_id();
    externalImageDesc.isInitialized = IsCleared();
    texture_ = wgpu::Texture::Acquire(dawn::native::opengl::WrapExternalGLTexture(device_.Get(), &externalImageDesc));
    return texture_;
}
