...
constexpr SharedImageUsageSet kUsage
    = SHARED_IMAGE_USAGE_DISPLAY_READ | SHARED_IMAGE_USAGE_RASTER_READ | SHARED_IMAGE_USAGE_RASTER_WRITE | SHARED_IMAGE_USAGE_CPU_UPLOAD;

class WrappedSkImageBackingFactoryTest : public SharedImageTestBase, public testing::WithParamInterface<std::tuple<viz::SharedImageFormat, GrContextType>> {
public:
    WrappedSkImageBackingFactoryTest() = default;
    ~WrappedSkImageBackingFactoryTest() override = default;

    viz::SharedImageFormat GetFormat()
    {
        return std::get<0>(GetParam());
    }
    GrContextType GetGrContextType()
    {
    ...
    void SetUp() override
    {
        ...
        if (gr_context_type == GrContextType::kGraphiteDawn && !IsGraphiteDawnSupported()) {
            GTEST_SKIP() << "Graphite/Dawn not supported";
        }
        ASSERT_NO_FATAL_FAILURE(InitializeContext(gr_context_type));

        auto format = GetFormat();
        if (gr_context_type == GrContextType::kGL && format == viz::SinglePlaneFormat::kBGRA_8888
            && !context_state_->feature_info()->feature_flags().ext_texture_format_bgra8888) {
            // We don't support GL context with Dawn for now.
            GTEST_SKIP();
        }

        // We don't use WrappedSkImageBacking with ALPHA8 if it's GL context.
        if (format == viz::SinglePlaneFormat::kALPHA_8 && gr_context_type == GrContextType::kGL) {
            GTEST_SKIP();
        }
        ...
        if (gr_context_type == GrContextType::kGraphiteDawn) {
            ...
            if (format == viz::SinglePlaneFormat::kRGBA_4444 || format == viz::SinglePlaneFormat::kRGB_565) {
                GTEST_SKIP();
            }
        }

        backing_factory_ = std::make_unique<WrappedSkImageBackingFactory>(context_state_);
    }
};
...
TEST_P(WrappedSkImageBackingFactoryTest, Basic)
{
    ...
    bool supported = backing_factory_->CanCreateSharedImage(kUsage, format, size, /*thread_safe=*/false, gfx::EMPTY_BUFFER, gr_context_type(), {});
    ASSERT_TRUE(supported);

    auto backing
        = backing_factory_->CreateSharedImage(mailbox, format, gpu::kNullSurfaceHandle, size, kColorSpace, kSurfaceOrigin, kAlphaType, kUsage, "TestLabel",
...
std::string TestParamToString(const testing::TestParamInfo<std::tuple<viz::SharedImageFormat, GrContextType>>& param_info)
{
    std::string format = std::get<0>(param_info.param).ToTestParamString();
    std::string context_type = (std::get<1>(param_info.param) == GrContextType::kGL) ? "GL" : "GraphiteDawn";
...
const auto kFormats
    = ::testing::Values(viz::SinglePlaneFormat::kALPHA_8, viz::SinglePlaneFormat::kR_8, viz::SinglePlaneFormat::kRG_88, viz::SinglePlaneFormat::kRGBA_4444,
        viz::SinglePlaneFormat::kRGB_565, viz::SinglePlaneFormat::kRGBA_8888, viz::SinglePlaneFormat::kBGRA_8888, viz::SinglePlaneFormat::kRGBX_8888,
        viz::SinglePlaneFormat::kRGBA_1010102, viz::MultiPlaneFormat::kNV12, viz::MultiPlaneFormat::kYV12, viz::MultiPlaneFormat::kI420);

INSTANTIATE_TEST_SUITE_P(
    , WrappedSkImageBackingFactoryTest, testing::Combine(kFormats, testing::Values(GrContextType::kGL, GrContextType::kGraphiteDawn)), TestParamToString);
