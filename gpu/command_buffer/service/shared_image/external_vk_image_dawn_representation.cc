...
ExternalVkImageDawnImageRepresentation::ExternalVkImageDawnImageRepresentation(SharedImageManager* manager, SharedImageBacking* backing,
    MemoryTypeTracker* tracker, wgpu::Device device, wgpu::TextureFormat wgpu_format, std::vector<wgpu::TextureFormat> view_formats, base::ScopedFD memory_fd)
...
wgpu::Texture ExternalVkImageDawnImageRepresentation::BeginAccess(wgpu::TextureUsage usage, wgpu::TextureUsage internal_usage)
{
    ...
    descriptor.cTextureDescriptor = reinterpret_cast<WGPUTextureDescriptor*>(&texture_descriptor);
    ...
    DCHECK(image_info.fCurrentQueueFamily == VK_QUEUE_FAMILY_IGNORED || image_info.fCurrentQueueFamily == VK_QUEUE_FAMILY_EXTERNAL);
    ...
    for (auto& external_semaphore : begin_access_semaphores_) {
        descriptor.waitFDs.push_back(external_semaphore.handle().TakeHandle().release());
    }

    texture_ = wgpu::Texture::Acquire(dawn::native::vulkan::WrapVulkanImage(device_.Get(), &descriptor));
    if (!texture_) {
        ...
        backing_impl()->ReleaseSemaphoresWithFenceHelper(std::move(begin_access_semaphores_));
...
void ExternalVkImageDawnImageRepresentation::EndAccess()
{
    ...
    if (!dawn::native::vulkan::ExportVulkanImage(texture_.Get(), VK_IMAGE_LAYOUT_UNDEFINED, &export_info)) {
        ...
        backend_texture.setMutableState(skgpu::MutableTextureStates::MakeVulkan(export_info.releasedNewLayout, VK_QUEUE_FAMILY_EXTERNAL));
        ...
        SemaphoreHandle handle(VK_EXTERNAL_SEMAPHORE_HANDLE_TYPE_OPAQUE_FD_BIT_KHR, base::ScopedFD(export_info.semaphoreHandles[0]));

        auto semaphore = ExternalSemaphore::CreateFromHandle(backing_impl()->context_provider(), std::move(handle));
