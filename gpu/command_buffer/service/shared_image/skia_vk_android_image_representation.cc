...
SkiaVkAndroidImageRepresentation::SkiaVkAndroidImageRepresentation(
    SharedImageManager* manager, AndroidImageBacking* backing, scoped_refptr<SharedContextState> context_state, MemoryTypeTracker* tracker)
...
std::vector<sk_sp<SkSurface>> SkiaVkAndroidImageRepresentation::BeginWriteAccess(int final_msaa_count, const SkSurfaceProps& surface_props,
    ...
    std::unique_ptr<skgpu::MutableTextureState>* end_state)
{
    ...
    if (!BeginAccess(/*readonly=*/false, begin_semaphores, end_semaphores, base::ScopedFD())) {
        return {};
    }
    ...
    if (!surface_ || final_msaa_count != surface_msaa_count_ || surface_props != surface_->props()) {
        SkColorType sk_color_type = viz::ToClosestSkColorType(
            /*gpu_compositing=*/true, format());
        surface_ = SkSurfaces::WrapBackendTexture(
            gr_context, promise_texture_->backendTexture(), surface_origin(), final_msaa_count, sk_color_type, color_space().ToSkColorSpace(), &surface_props);
        if (!surface_) {
...
std::vector<sk_sp<GrPromiseImageTexture>> SkiaVkAndroidImageRepresentation::BeginWriteAccess(
...
std::vector<sk_sp<GrPromiseImageTexture>> SkiaVkAndroidImageRepresentation::BeginReadAccess(
    std::vector<GrBackendSemaphore>* begin_semaphores, std::vector<GrBackendSemaphore>* end_semaphores, std::unique_ptr<skgpu::MutableTextureState>* end_state)
{
    DCHECK_EQ(mode_, RepresentationAccessMode::kNone);
    DCHECK(!surface_);
    ...
    if (!BeginAccess(/*readonly=*/true, begin_semaphores, end_semaphores, std::move(init_read_fence_))) {
        return {};
    }
...
gpu::VulkanImplementation* SkiaVkAndroidImageRepresentation::vk_implementation()
{
...
{
    return context_state_->vk_context_provider()->GetDeviceQueue()->GetVulkanPhysicalDevice();
}

VkQueue SkiaVkAndroidImageRepresentation::vk_queue()
{
    return context_state_->vk_context_provider()->GetDeviceQueue()->GetVulkanQueue();
}

bool SkiaVkAndroidImageRepresentation::BeginAccess(
    bool readonly, std::vector<GrBackendSemaphore>* begin_semaphores, std::vector<GrBackendSemaphore>* end_semaphores, base::ScopedFD init_read_fence)
{
    ...
    if (sync_fd.is_valid()) {
        begin_access_semaphore_
            = vk_implementation()->ImportSemaphoreHandle(vk_device(), SemaphoreHandle(VK_EXTERNAL_SEMAPHORE_HANDLE_TYPE_SYNC_FD_BIT, std::move(sync_fd)));
    ...
    if (begin_access_semaphore_ != VK_NULL_HANDLE) {
        begin_semaphores->emplace_back(GrBackendSemaphores::MakeVk(begin_access_semaphore_));
    }
    if (end_semaphores) {
...
std::unique_ptr<skgpu::MutableTextureState> SkiaVkAndroidImageRepresentation::GetEndAccessState()
{
    ...
    if (!kSingleDeviceUsage.HasAll(android_backing()->usage()) || android_backing()->is_thread_safe()) {
        return std::make_unique<skgpu::MutableTextureState>(
            skgpu::MutableTextureStates::MakeVulkan(VK_IMAGE_LAYOUT_UNDEFINED, vulkan_image_->queue_family_index()));
    }
    return nullptr;
}
