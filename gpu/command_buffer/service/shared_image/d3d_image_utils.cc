...
bool ClearD3D11TextureToColor(const Microsoft::WRL::ComPtr<ID3D11Texture2D>& d3d11_texture, const SkColor4f& color)
{
    Microsoft::WRL::ComPtr<ID3D11Device> d3d11_device;
    d3d11_texture->GetDevice(&d3d11_device);

    Microsoft::WRL::ComPtr<ID3D11RenderTargetView> render_target;
    HRESULT hr = d3d11_device->CreateRenderTargetView(d3d11_texture.Get(), nullptr, &render_target);
    if (FAILED(hr)) {
...
wgpu::Texture CreateDawnSharedTexture(const wgpu::SharedTextureMemory& shared_texture_memory, wgpu::TextureUsage usage, wgpu::TextureUsage internal_usage,
    base::span<wgpu::TextureFormat> view_formats)
{
    ...
    wgpu_texture_desc.viewFormatCount = static_cast<uint32_t>(view_formats.size());
...
wgpu::SharedTextureMemory CreateDawnSharedTextureMemory(const wgpu::Device& device, bool use_keyed_mutex, HANDLE handle)
{
...
wgpu::SharedTextureMemory CreateDawnSharedTextureMemory(const wgpu::Device& device, Microsoft::WRL::ComPtr<ID3D11Texture2D> texture)
{
    wgpu::SharedTextureMemory shared_texture_memory;
...
wgpu::Buffer CreateDawnSharedBuffer(const wgpu::SharedBufferMemory& shared_buffer_memory, wgpu::BufferUsage usage)
{
...
wgpu::SharedBufferMemory CreateDawnSharedBufferMemory(const wgpu::Device& device, Microsoft::WRL::ComPtr<ID3D12Resource> resource)
{
...
wgpu::SharedFence CreateDawnSharedFence(const wgpu::Device& device, scoped_refptr<gfx::D3DSharedFence> fence)
{
