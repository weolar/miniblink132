...
public:
        ...
        std::string debug_label, bool is_thread_safe) override
    {
        ...
        return std::make_unique<TestImageBacking>(mailbox, format, size, color_space, surface_origin, alpha_type, usage, kTestBackingSize);
    }
...
public:
    ...
    std::unique_ptr<SharedImageBacking> CreateCompoundBacking(SharedImageUsageSet usage)
    {
        constexpr gfx::Size size(100, 100);
        constexpr gfx::BufferFormat buffer_format = gfx::BufferFormat::RGBA_8888;
        constexpr gfx::BufferUsage buffer_usage = gfx::BufferUsage::SCANOUT_CPU_READ_WRITE;

        gfx::GpuMemoryBufferHandle handle
            = GpuMemoryBufferImplSharedMemory::CreateGpuMemoryBuffer(static_cast<gfx::GpuMemoryBufferId>(1), size, buffer_format, buffer_usage);

        return CompoundImageBacking::CreateSharedMemory(&test_factory_, Mailbox::Generate(), std::move(handle), viz::SinglePlaneFormat::kRGBA_8888, size,
            gfx::ColorSpace(), kTopLeft_GrSurfaceOrigin, kOpaque_SkAlphaType, usage, "TestLabel");
    }

    std::unique_ptr<SharedImageBacking> CreateMultiplanarCompoundBacking()
    {
        constexpr gfx::Size size(100, 100);
        constexpr gfx::BufferFormat buffer_format = gfx::BufferFormat::YUV_420_BIPLANAR;
        constexpr gfx::BufferUsage buffer_usage = gfx::BufferUsage::SCANOUT_CPU_READ_WRITE;

        gfx::GpuMemoryBufferHandle handle
            = GpuMemoryBufferImplSharedMemory::CreateGpuMemoryBuffer(static_cast<gfx::GpuMemoryBufferId>(1), size, buffer_format, buffer_usage);

        return CompoundImageBacking::CreateSharedMemory(&test_factory_, Mailbox::Generate(), std::move(handle), viz::MultiPlaneFormat::kNV12, size,
            gfx::ColorSpace(), kTopLeft_GrSurfaceOrigin, kOpaque_SkAlphaType,
            SharedImageUsageSet({ SHARED_IMAGE_USAGE_DISPLAY_READ, SHARED_IMAGE_USAGE_SCANOUT }), "TestLabel");
    }
...
{
    auto backing = CreateCompoundBacking({ SHARED_IMAGE_USAGE_DISPLAY_READ, SHARED_IMAGE_USAGE_SCANOUT });
    ...
    auto overlay_rep = manager_.ProduceOverlay(compound_backing->mailbox(), &tracker_);
    ...
    EXPECT_FALSE(HasGpuBacking(compound_backing));
    {
        ...
        std::optional<gl::DCLayerOverlayImage> overlay_image = access->GetDCLayerOverlayImage();
...
TEST_F(CompoundImageBackingTest, UploadOnAccess)
{
    auto backing = CreateCompoundBacking(
        { SHARED_IMAGE_USAGE_SCANOUT, SHARED_IMAGE_USAGE_GLES2_READ, SHARED_IMAGE_USAGE_DISPLAY_READ, SHARED_IMAGE_USAGE_DISPLAY_WRITE });
    auto* compound_backing = static_cast<CompoundImageBacking*>(backing.get());
    ...
    auto gl_passthrough_rep = manager_.ProduceGLTexturePassthrough(compound_backing->mailbox(), &tracker_);
    compound_backing->Update(nullptr);
    gl_passthrough_rep->BeginScopedAccess(0, SharedImageRepresentation::AllowUnclearedAccess::kNo);
    ASSERT_TRUE(HasGpuBacking(compound_backing));
    auto* gpu_backing = GetGpuBacking(compound_backing);
    EXPECT_TRUE(gpu_backing->GetUploadFromMemoryCalledAndReset());

    // Test that GLTexture access causes upload.
    auto gl_rep = manager_.ProduceGLTexture(compound_backing->mailbox(), &tracker_);
    compound_backing->Update(nullptr);
    gl_rep->BeginScopedAccess(0, SharedImageRepresentation::AllowUnclearedAccess::kNo);
    EXPECT_TRUE(gpu_backing->GetUploadFromMemoryCalledAndReset());

    // Test that both read and write access by Skia triggers upload.
    std::vector<GrBackendSemaphore> begin_semaphores;
    std::vector<GrBackendSemaphore> end_semaphores;
    auto skia_rep = manager_.ProduceSkia(compound_backing->mailbox(), &tracker_, nullptr);
    ...
    skia_rep->BeginScopedWriteAccess(&begin_semaphores, &end_semaphores, SharedImageRepresentation::AllowUnclearedAccess::kNo);
    EXPECT_TRUE(gpu_backing->GetUploadFromMemoryCalledAndReset());
