...
public:
    void SetUp() override
    {
        ...
        SharedImageUsageSet usage = { SHARED_IMAGE_USAGE_GLES2_READ, SHARED_IMAGE_USAGE_GLES2_WRITE, SHARED_IMAGE_USAGE_RASTER_READ,
            SHARED_IMAGE_USAGE_RASTER_WRITE, SHARED_IMAGE_USAGE_WEBGPU_WRITE, SHARED_IMAGE_USAGE_SCANOUT };
        auto backing = std::make_unique<TestImageBacking>(mailbox_, format, size, color_space, surface_origin, alpha_type, usage,
            /*estimated_size=*/0);
...
TEST_F(SharedImageRepresentationTest, GLTextureClearing)
{
    ...
    {
        auto scoped_access
            = representation->BeginScopedAccess(GL_SHARED_IMAGE_ACCESS_MODE_READWRITE_CHROMIUM, SharedImageRepresentation::AllowUnclearedAccess::kNo);
    ...
    {
        auto scoped_access
            = representation->BeginScopedAccess(GL_SHARED_IMAGE_ACCESS_MODE_READWRITE_CHROMIUM, SharedImageRepresentation::AllowUnclearedAccess::kYes);
    ...
    representation->SetClearedRect(gfx::Rect());
    {
        auto scoped_access
            = representation->BeginScopedAccess(GL_SHARED_IMAGE_ACCESS_MODE_READWRITE_CHROMIUM, SharedImageRepresentation::AllowUnclearedAccess::kYes);
        ASSERT_TRUE(scoped_access);
        EXPECT_FALSE(representation->GetTexture()->IsLevelCleared(GL_TEXTURE_2D, 0));
    }
    EXPECT_FALSE(representation->IsCleared());
}
...
{
    auto representation = manager_.ProduceGLTexturePassthrough(mailbox_, tracker_.get());
    EXPECT_FALSE(representation->IsCleared());
...
TEST_F(SharedImageRepresentationTest, SkiaClearing)
{
    ...
    {
        auto scoped_access = representation->BeginScopedReadAccess(nullptr, nullptr);
        EXPECT_FALSE(scoped_access);
    }
    EXPECT_FALSE(representation->IsCleared());
    ...
    // false.
    {
        auto scoped_access = representation->BeginScopedWriteAccess(nullptr, nullptr, SharedImageRepresentation::AllowUnclearedAccess::kNo);
        EXPECT_FALSE(scoped_access);
    }
    EXPECT_FALSE(representation->IsCleared());
    ...
    {
        auto scoped_access = representation->BeginScopedWriteAccess(nullptr, nullptr, SharedImageRepresentation::AllowUnclearedAccess::kYes);
        EXPECT_TRUE(scoped_access);
    }
    EXPECT_FALSE(representation->IsCleared());
...
TEST_F(SharedImageRepresentationTest, DawnClearing)
{
    ...
    auto representation = manager_.ProduceDawn(mailbox_, tracker_.get(), /*device=*/nullptr, wgpu::BackendType::Null, {},
        /*context_state=*/nullptr);
    EXPECT_FALSE(representation->IsCleared());

    // We should not be able to begin access with |allow_uncleared| == false.
    {
        auto scoped_access = representation->BeginScopedAccess(wgpu::TextureUsage::None, SharedImageRepresentation::AllowUnclearedAccess::kNo);
        EXPECT_FALSE(scoped_access);
    }
    EXPECT_FALSE(representation->IsCleared());
    ...
    {
        auto scoped_access = representation->BeginScopedAccess(wgpu::TextureUsage::None, SharedImageRepresentation::AllowUnclearedAccess::kYes);
        EXPECT_TRUE(scoped_access);
    }
    EXPECT_FALSE(representation->IsCleared());
