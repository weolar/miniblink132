...
// `usage`.
void EnforceSharedImageUsage(const SharedImageBacking& backing, SharedImageUsageSet usage)
{
    if (!(backing.usage() & usage)) {
        SCOPED_CRASH_KEY_STRING32("SharedImageUsage", "debug_label", backing.debug_label());
        SCOPED_CRASH_KEY_STRING32("SharedImageUsage", "name", backing.GetName());
        SCOPED_CRASH_KEY_NUMBER("SharedImageUsage", "required_usage", static_cast<int>(usage));
        SCOPED_CRASH_KEY_NUMBER("ShareDImageUsage", "actual_usage", backing.usage());
...
bool operator<(const std::unique_ptr<SharedImageBacking>& lhs, const std::unique_ptr<SharedImageBacking>& rhs)
{
    return lhs->mailbox() < rhs->mailbox();
}

bool operator<(const Mailbox& lhs, const std::unique_ptr<SharedImageBacking>& rhs)
{
    return lhs < rhs->mailbox();
}

bool operator<(const std::unique_ptr<SharedImageBacking>& lhs, const Mailbox& rhs)
{
...
public:
    explicit AutoLock(SharedImageManager* manager) EXCLUSIVE_LOCK_FUNCTION(manager->lock_)
        : auto_lock_(manager->is_thread_safe() ? &manager->lock_.value() : nullptr)
    {
    }
...
SharedImageManager::SharedImageManager(bool thread_safe, bool display_context_on_another_thread)
    : display_context_on_another_thread_(display_context_on_another_thread)
#if BUILDFLAG(IS_WIN)
    , dxgi_shared_handle_manager_(base::MakeRefCounted<DXGISharedHandleManager>())
#endif
{
    ...
    if (base::SingleThreadTaskRunner::HasCurrentDefault()) {
        ...
        base::trace_event::MemoryDumpManager::GetInstance()->RegisterDumpProvider(
            this, "SharedImageManager", base::SingleThreadTaskRunner::GetCurrentDefault());
    }
}
...
std::unique_ptr<SharedImageRepresentationFactoryRef> SharedImageManager::Register(std::unique_ptr<SharedImageBacking> backing, MemoryTypeTracker* tracker)
{
    CALLED_ON_VALID_THREAD();
    ...
    auto factory_ref = std::make_unique<SharedImageRepresentationFactoryRef>(this, backing.get(), tracker, /*is_primary=*/true);
    images_.emplace(std::move(backing));

    return factory_ref;
}

std::unique_ptr<SharedImageRepresentationFactoryRef> SharedImageManager::AddSecondaryReference(const Mailbox& mailbox, MemoryTypeTracker* tracker)
{
    CALLED_ON_VALID_THREAD();
    ...
    return std::make_unique<SharedImageRepresentationFactoryRef>(this, found->get(), tracker, /*is_primary=*/false);
}

std::unique_ptr<GLTextureImageRepresentation> SharedImageManager::ProduceGLTexture(const Mailbox& mailbox, MemoryTypeTracker* tracker)
{
    CALLED_ON_VALID_THREAD();
...
std::unique_ptr<GLTexturePassthroughImageRepresentation> SharedImageManager::ProduceGLTexturePassthrough(const Mailbox& mailbox, MemoryTypeTracker* tracker)
{
    CALLED_ON_VALID_THREAD();
...
std::unique_ptr<SkiaImageRepresentation> SharedImageManager::ProduceSkia(
    const Mailbox& mailbox, MemoryTypeTracker* tracker, scoped_refptr<SharedContextState> context_state)
{
    CALLED_ON_VALID_THREAD();
...
std::unique_ptr<DawnImageRepresentation> SharedImageManager::ProduceDawn(const Mailbox& mailbox, MemoryTypeTracker* tracker, const wgpu::Device& device,
    wgpu::BackendType backend_type, std::vector<wgpu::TextureFormat> view_formats, scoped_refptr<SharedContextState> context_state)
{
    ...
    EnforceSharedImageUsage(**found, { SHARED_IMAGE_USAGE_WEBGPU_READ, SHARED_IMAGE_USAGE_WEBGPU_WRITE });
    auto representation = (*found)->ProduceDawn(this, tracker, device, backend_type, std::move(view_formats), context_state);
    if (!representation) {
        LOG(ERROR) << "SharedImageManager::ProduceDawn: Trying to produce a "
...
std::unique_ptr<DawnBufferRepresentation> SharedImageManager::ProduceDawnBuffer(
    const Mailbox& mailbox, MemoryTypeTracker* tracker, const wgpu::Device& device, wgpu::BackendType backend_type)
{
    CALLED_ON_VALID_THREAD();
    ...
    auto representation = (*found)->ProduceDawnBuffer(this, tracker, device, backend_type);
    if (!representation) {
        LOG(ERROR) << "SharedImageManager::ProduceDawnBuffer: Trying to produce a "
...
std::unique_ptr<OverlayImageRepresentation> SharedImageManager::ProduceOverlay(const gpu::Mailbox& mailbox, gpu::MemoryTypeTracker* tracker)
{
    CALLED_ON_VALID_THREAD();
...
std::unique_ptr<MemoryImageRepresentation> SharedImageManager::ProduceMemory(const Mailbox& mailbox, MemoryTypeTracker* tracker)
{
    CALLED_ON_VALID_THREAD();
...
std::unique_ptr<RasterImageRepresentation> SharedImageManager::ProduceRaster(const Mailbox& mailbox, MemoryTypeTracker* tracker)
{
    CALLED_ON_VALID_THREAD();
...
std::unique_ptr<VideoImageRepresentation> SharedImageManager::ProduceVideo(VideoDevice device, const Mailbox& mailbox, MemoryTypeTracker* tracker)
{
    CALLED_ON_VALID_THREAD();
    ...
    auto found = images_.find(mailbox);
    if (found == images_.end()) {
        LOG(ERROR) << "SharedImageManager::ProduceVideoDecode: Trying to Produce a D3D"
                      "representation from a non-existent mailbox.";
        return nullptr;
    }
...
std::unique_ptr<VulkanImageRepresentation> SharedImageManager::ProduceVulkan(const Mailbox& mailbox, MemoryTypeTracker* tracker,
    gpu::VulkanDeviceQueue* vulkan_device_queue, gpu::VulkanImplementation& vulkan_impl, bool needs_detiling)
{
    ...
    auto found = images_.find(mailbox);
    if (found == images_.end()) {
        LOG(ERROR) << "SharedImageManager::ProduceVulkanImage: Trying to produce vulkan"
                      "representation from a non-existent mailbox.";
        return nullptr;
    }

    return (*found)->ProduceVulkan(this, tracker, vulkan_device_queue, vulkan_impl, needs_detiling);
}
#endif

#if BUILDFLAG(IS_ANDROID)
std::unique_ptr<LegacyOverlayImageRepresentation> SharedImageManager::ProduceLegacyOverlay(const Mailbox& mailbox, MemoryTypeTracker* tracker)
{
    CALLED_ON_VALID_THREAD();
    ...
    auto found = images_.find(mailbox);
    if (found == images_.end()) {
        LOG(ERROR) << "SharedImageManager::ProduceLegacyOverlay: Trying to Produce a "
...
#if BUILDFLAG(IS_WIN)
void SharedImageManager::UpdateExternalFence(const Mailbox& mailbox, scoped_refptr<gfx::D3DSharedFence> external_fence)
{
    CALLED_ON_VALID_THREAD();
    AutoLock autolock(this);
    auto found = images_.find(mailbox);
...
std::optional<SharedImageUsageSet> SharedImageManager::GetUsageForMailbox(const Mailbox& mailbox)
{
    AutoLock autolock(this);
...
void SharedImageManager::OnRepresentationDestroyed(const Mailbox& mailbox, SharedImageRepresentation* representation)
{
    CALLED_ON_VALID_THREAD();
...
bool SharedImageManager::OnMemoryDump(const base::trace_event::MemoryDumpArgs& args, base::trace_event::ProcessMemoryDump* pmd)
{
    CALLED_ON_VALID_THREAD();
    AutoLock autolock(this);
    ...
    if (args.level_of_detail == base::trace_event::MemoryDumpLevelOfDetail::kBackground) {
        ...
        base::trace_event::MemoryAllocatorDump* dump = pmd->CreateAllocatorDump(base_dump_name);
        ...
        dump->AddScalar("purgeable_size", base::trace_event::MemoryAllocatorDump::kUnitsBytes, total_purgeable_size);
        dump->AddScalar("non_exo_size", base::trace_event::MemoryAllocatorDump::kUnitsBytes, total_non_exo_size);
    ...
    for (auto& backing : images_) {
        ...
        std::string dump_name
            = base::StringPrintf("%s/client_0x%" PRIX32 "/mailbox_%s", base_dump_name, memory_tracker->ClientId(), backing->mailbox().ToDebugString().c_str());
        ...
        // edge to `client_guid`.
        backing->OnMemoryDump(dump_name, client_guid, pmd, memory_tracker->ClientTracingId());
    }

    return true;
}

scoped_refptr<gfx::NativePixmap> SharedImageManager::GetNativePixmap(const gpu::Mailbox& mailbox)
{
    AutoLock autolock(this);
    auto found = images_.find(mailbox);
    if (found == images_.end())
...
bool SharedImageManager::SupportsScanoutImages()
{
    ...
    if (base::FeatureList::IsEnabled(features::kSharedImageSupportScanoutOnOzoneOnlyIfOverlaysSupported)) {
        return supports_overlays_on_ozone_;
    } else {
        return ui::OzonePlatform::GetInstance()->GetPlatformRuntimeProperties().supports_native_pixmaps;
    }
