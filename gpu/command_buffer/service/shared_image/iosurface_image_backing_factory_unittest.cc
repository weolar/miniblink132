...
public:
    void SetUp() override
    {
        ...
        backing_factory_ = std::make_unique<IOSurfaceImageBackingFactory>(context_state_->gr_context_type(), context_state_->GetMaxTextureSize(),
...
protected:
    void CheckSkiaPixels(const Mailbox& mailbox, const gfx::Size& size, const std::vector<uint8_t> expected_color)
    {
...
TEST_F(IOSurfaceImageBackingFactoryTest, GL_SkiaGL)
{
    ...
    SharedImageUsageSet usage = { SHARED_IMAGE_USAGE_GLES2_WRITE, SHARED_IMAGE_USAGE_DISPLAY_READ, SHARED_IMAGE_USAGE_SCANOUT };
    auto backing = backing_factory_->CreateSharedImage(
        mailbox, format, surface_handle, size, color_space, surface_origin, alpha_type, usage, "TestLabel", /*is_thread_safe=*/false);
    EXPECT_TRUE(backing);
    backing->SetCleared();
...
class IOSurfaceImageBackingFactoryDawnTest : public IOSurfaceImageBackingFactoryTest, public testing::WithParamInterface<wgpu::BackendType> {
protected:
    void SetUp() override
    {
        ...
        std::vector<dawn::native::Adapter> adapters = instance_.EnumerateAdapters(&adapter_options);
    ...
    wgpu::Device CreateDevice()
    {
        ...
        if (adapter_.HasFeature(wgpu::FeatureName::MultiPlanarFormatExtendedUsages)) {
    ...
    std::unique_ptr<SharedImageRepresentationFactoryRef> CreateSharedImage(const gfx::Size& size, SharedImageUsageSet usage)
    {
        // Create a backing using mailbox.
        auto mailbox = Mailbox::Generate();
        auto format = viz::SinglePlaneFormat::kRGBA_8888;
    ...
    void ClearSharedImageWithDawn(wgpu::Device device, const gpu::Mailbox& mailbox, wgpu::Color color)
    {
        // Create a DawnImageRepresentation.
        auto dawn_representation = shared_image_representation_factory_.ProduceDawn(mailbox, device, backend_type(), {}, context_state_);
    ...
    auto CopySharedImageWithDawn(wgpu::Device device, const gpu::Mailbox& src, const gpu::Mailbox& dst)
    {
        auto src_rep = shared_image_representation_factory_.ProduceDawn(src, device, backend_type(), {}, context_state_);
        auto src_scoped_access = src_rep->BeginScopedAccess(wgpu::TextureUsage::CopySrc, SharedImageRepresentation::AllowUnclearedAccess::kYes);
        wgpu::Texture src_texture(src_scoped_access->texture());

        auto dst_rep = shared_image_representation_factory_.ProduceDawn(dst, device, backend_type(), {}, context_state_);

        auto dst_scoped_access = dst_rep->BeginScopedAccess(wgpu::TextureUsage::CopyDst, SharedImageRepresentation::AllowUnclearedAccess::kYes);
...
TEST_P(IOSurfaceImageBackingFactoryDawnTest, Dawn_MultipleRepresentationsFromSingleDevice)
{
    ...
    SharedImageUsageSet usage = { SHARED_IMAGE_USAGE_WEBGPU_READ, SHARED_IMAGE_USAGE_DISPLAY_READ, SHARED_IMAGE_USAGE_SCANOUT };
    auto factory_ref = CreateSharedImage(size, usage);

    auto rep_0 = shared_image_representation_factory_.ProduceDawn(factory_ref->mailbox(), device, backend_type(), {}, context_state_);
    auto scoped_access_0 = rep_0->BeginScopedAccess(wgpu::TextureUsage::CopySrc, SharedImageRepresentation::AllowUnclearedAccess::kYes);

    auto rep_1 = shared_image_representation_factory_.ProduceDawn(factory_ref->mailbox(), device, backend_type(), {}, context_state_);
    auto scoped_access_1 = rep_1->BeginScopedAccess(wgpu::TextureUsage::CopySrc, SharedImageRepresentation::AllowUnclearedAccess::kYes);
...
TEST_P(IOSurfaceImageBackingFactoryDawnTest, Dawn_FailureToBeginAccess)
{
    ...
    SharedImageUsageSet usage
        = { SHARED_IMAGE_USAGE_WEBGPU_READ, SHARED_IMAGE_USAGE_WEBGPU_WRITE, SHARED_IMAGE_USAGE_DISPLAY_READ, SHARED_IMAGE_USAGE_SCANOUT };
    auto factory_ref = CreateSharedImage(size, usage);

    auto rep = shared_image_representation_factory_.ProduceDawn(factory_ref->mailbox(), device, backend_type(), {}, context_state_);

    device.Destroy();

    auto scoped_access = rep->BeginScopedAccess(wgpu::TextureUsage::CopySrc, SharedImageRepresentation::AllowUnclearedAccess::kYes);
    ...
    if (backend_type() == wgpu::BackendType::Metal) {
        ...
        EXPECT_TRUE(dawn::native::CheckIsErrorForTesting(scoped_access->texture().Get()));
    }
}
...
TEST_P(IOSurfaceImageBackingFactoryDawnTest, Dawn_SkiaGL)
{
    ...
    gfx::Size size(1, 1);
    SharedImageUsageSet usage = { SHARED_IMAGE_USAGE_WEBGPU_WRITE, SHARED_IMAGE_USAGE_DISPLAY_READ, SHARED_IMAGE_USAGE_SCANOUT };
    auto factory_ref = CreateSharedImage(size, usage);
...
TEST_P(IOSurfaceImageBackingFactoryDawnTest, Dawn_WriteReadReadOnThreeDevices)
{
    ...
    // Clear the shared image to green using Dawn.
    ClearSharedImageWithDawn(device_0, factory_ref_0->mailbox(), { 0, 255, 0, 255 });

    auto [rep_1, access_1] = CopySharedImageWithDawn(device_1, factory_ref_0->mailbox(), factory_ref_1->mailbox());
    auto [rep_2, access_2] = CopySharedImageWithDawn(device_2, factory_ref_0->mailbox(), factory_ref_2->mailbox());
...
TEST_P(IOSurfaceImageBackingFactoryDawnTest, GL_Dawn_Skia_UnclearTexture)
{
    gfx::Size size(1, 1);
    SharedImageUsageSet usage
        = { SHARED_IMAGE_USAGE_GLES2_WRITE, SHARED_IMAGE_USAGE_SCANOUT, SHARED_IMAGE_USAGE_WEBGPU_WRITE, SHARED_IMAGE_USAGE_DISPLAY_READ };
    auto factory_ref = CreateSharedImage(size, usage);

    {
        // Create a GLTextureImageRepresentation.
        auto gl_representation = shared_image_representation_factory_.ProduceGLTexturePassthrough(factory_ref->mailbox());
        GLenum expected_target =
#if BUILDFLAG(IS_MAC)
            GetTextureTargetForIOSurfaces();
#else
    ...
    {
        auto dawn_representation = shared_image_representation_factory_.ProduceDawn(factory_ref->mailbox(), device, backend_type(), {}, context_state_);
...
{
    gfx::Size size(1, 1);
    SharedImageUsageSet usage = { SHARED_IMAGE_USAGE_SCANOUT, SHARED_IMAGE_USAGE_WEBGPU_WRITE, SHARED_IMAGE_USAGE_DISPLAY_READ };
    auto factory_ref = CreateSharedImage(size, usage);
    ...
    auto skia_representation = shared_image_representation_factory_.ProduceSkia(factory_ref->mailbox(), context_state_);
    ASSERT_NE(skia_representation, nullptr);
...
TEST_P(IOSurfaceImageBackingFactoryDawnTest, Dawn_SamplingVideoTexture)
{
    ...
    if (backend_type() == wgpu::BackendType::Vulkan) {
        // Vulkan doesn't support IOSurface image backing, so we need
        // MultiPlanarFormatExtendedUsages to copy to/from multiplanar texture.
        ASSERT_TRUE(device.HasFeature(wgpu::FeatureName::MultiPlanarFormatExtendedUsages));
    }
    ...
    GrSurfaceOrigin surface_origin = kTopLeft_GrSurfaceOrigin;
    SkAlphaType alpha_type = kPremul_SkAlphaType;
    const SharedImageUsageSet usage = { SHARED_IMAGE_USAGE_SCANOUT, SHARED_IMAGE_USAGE_WEBGPU_READ };
    ...
    for (int plane_index = 0; plane_index < 2; ++plane_index) {
        ...
        pixmaps[plane_index] = SkPixmap(info, plane_datas[plane_index].data(), info.minRowBytes());
    }
    ...
    auto dawn_image = shared_image_representation_factory_.ProduceDawn(mailbox, device, backend_type(), {}, context_state_);
    ASSERT_NE(dawn_image, nullptr);

    RunDawnVideoSamplingTest(instance_.Get(), device, dawn_image, kYFillValue, kUFillValue, kVFillValue);
}
...
TEST_F(IOSurfaceImageBackingFactoryTest, SkiaAccessFirstFails)
{
    ...
    GrSurfaceOrigin surface_origin = kTopLeft_GrSurfaceOrigin;
    SkAlphaType alpha_type = kPremul_SkAlphaType;
    SharedImageUsageSet usage = { SHARED_IMAGE_USAGE_SCANOUT, SHARED_IMAGE_USAGE_DISPLAY_READ };
    const gpu::SurfaceHandle surface_handle = gpu::kNullSurfaceHandle;
    auto backing = backing_factory_->CreateSharedImage(
        mailbox, format, surface_handle, size, color_space, surface_origin, alpha_type, usage, "TestLabel", /*is_thread_safe=*/false);
    ASSERT_NE(backing, nullptr);
...
struct ContextTypeParams {
    ContextTypeParams(GrContextType context_type, wgpu::BackendType dawn_backend_type = wgpu::BackendType::Null)
        : context_type(context_type)
        , dawn_backend_type(dawn_backend_type)
    {
    }
...
class IOSurfaceImageBackingFactoryParameterizedTestBase : public SharedImageTestBase,
                                                          public testing::WithParamInterface<std::tuple<viz::SharedImageFormat, ContextTypeParams>> {
public:
    ...
    void SetUp() override
    {
        ...
        if (gr_context_type == GrContextType::kGraphiteDawn && format == viz::SinglePlaneFormat::kBGRA_1010102) {
            GTEST_SKIP();
        }
        ...
        supports_etc1_ = feature_info->validators()->compressed_texture_format.IsValid(GL_ETC1_RGB8_OES);
        ...
        supports_ycbcr_p010_ = feature_info->feature_flags().chromium_image_ycbcr_p010;
    ...
    viz::SharedImageFormat get_format()
    {
...
class IOSurfaceImageBackingFactoryScanoutTest : public IOSurfaceImageBackingFactoryParameterizedTestBase {
public:
    bool can_create_scanout_shared_image(viz::SharedImageFormat format, bool has_pixel_data = false) const
    {
...
TEST_P(IOSurfaceImageBackingFactoryScanoutTest, Basic)
{
    ...
    GrSurfaceOrigin surface_origin = kTopLeft_GrSurfaceOrigin;
    SkAlphaType alpha_type = kPremul_SkAlphaType;
    SharedImageUsageSet usage { SHARED_IMAGE_USAGE_SCANOUT, SHARED_IMAGE_USAGE_RASTER_READ, SHARED_IMAGE_USAGE_RASTER_WRITE };
    ...
    std::unique_ptr<SharedImageRepresentationFactoryRef> shared_image = shared_image_manager_.Register(std::move(backing), &memory_type_tracker_);
    ...
    if (get_gr_context_type() == GrContextType::kGL) {
        ...
        auto dawn_representation = shared_image_representation_factory_.ProduceDawn(mailbox, device, context_provider->backend_type(), {}, context_state_);
        ASSERT_TRUE(dawn_representation);
        EXPECT_EQ(usage, dawn_representation->usage());
        EXPECT_EQ(color_space, dawn_representation->color_space());

        auto dawn_scoped_access
            = dawn_representation->BeginScopedAccess(wgpu::TextureUsage::TextureBinding, SharedImageRepresentation::AllowUnclearedAccess::kYes);
        ASSERT_TRUE(dawn_scoped_access);
    ...
    if (format == viz::SinglePlaneFormat::kBGRA_1010102 || format == viz::MultiPlaneFormat::kP010) {
    ...
    // Vulkan-Swiftshader.
    if (get_gr_context_type() == GrContextType::kGraphiteDawn && GetDawnBackendType() == wgpu::BackendType::Vulkan && format.is_multi_plane()) {
        return;
    }

    // Finally, validate a SkiaImageRepresentation.
    auto skia_representation = shared_image_representation_factory_.ProduceSkia(mailbox, context_state_.get());
    EXPECT_TRUE(skia_representation);
    std::vector<GrBackendSemaphore> begin_semaphores;
    std::vector<GrBackendSemaphore> end_semaphores;
    std::unique_ptr<SkiaImageRepresentation::ScopedWriteAccess> scoped_write_access;
    scoped_write_access
        = skia_representation->BeginScopedWriteAccess(&begin_semaphores, &end_semaphores, SharedImageRepresentation::AllowUnclearedAccess::kYes);
    for (auto i = 0; i < format.NumberOfPlanes(); i++) {
    ...
    scoped_read_access = skia_representation->BeginScopedReadAccess(&begin_semaphores, &end_semaphores);
    if (get_gr_context_type() == GrContextType::kGL) {
...
TEST_P(IOSurfaceImageBackingFactoryScanoutTest, InitialData)
{
    ...
    auto backing = backing_factory_->CreateSharedImage(
        mailbox, format, size, color_space, surface_origin, alpha_type, usage, "TestLabel", /*is_thread_safe=*/false, initial_data);
    ::testing::Mock::VerifyAndClearExpectations(&progress_reporter_);
    if (!should_succeed) {
        EXPECT_FALSE(backing);
        return;
    ...
    if (get_gr_context_type() == GrContextType::kGL) {
        ...
        auto dawn_scoped_access
            = dawn_representation->BeginScopedAccess(wgpu::TextureUsage::TextureBinding, SharedImageRepresentation::AllowUnclearedAccess::kNo);
        ASSERT_TRUE(dawn_scoped_access);
...
{
    const bool should_succeed = can_create_scanout_shared_image(get_format(), /*has_pixel_data=*/true);
    if (should_succeed) {
        EXPECT_CALL(progress_reporter_, ReportProgress).Times(AtLeast(1));
    }
    auto mailbox = Mailbox::Generate();
...
TEST_P(IOSurfaceImageBackingFactoryScanoutTest, InitialDataWrongSize)
{
    ...
    auto backing = backing_factory_->CreateSharedImage(
        mailbox, format, size, color_space, surface_origin, alpha_type, usage, "TestLabel", /*is_thread_safe=*/false, initial_data_small);
    EXPECT_FALSE(backing);
    backing = backing_factory_->CreateSharedImage(
        mailbox, format, size, color_space, surface_origin, alpha_type, usage, "TestLabel", /*is_thread_safe=*/false, initial_data_large);
    EXPECT_FALSE(backing);
}
...
TEST_P(IOSurfaceImageBackingFactoryScanoutTest, InvalidFormatForCreationWithPixelData)
{
    auto mailbox = Mailbox::Generate();
...
TEST_P(IOSurfaceImageBackingFactoryScanoutTest, TexImageTexStorageEquivalence)
{
    ...
    scoped_refptr<gles2::FeatureInfo> feature_info = new gles2::FeatureInfo(GpuDriverBugWorkarounds(), GpuFeatureInfo());
    feature_info->Initialize(ContextType::CONTEXT_TYPE_OPENGLES2,
        /*is_passthrough_cmd_decoder=*/true, gles2::DisallowedFeatures());
    ...
    for (auto format : viz::SinglePlaneFormat::kAll) {
        ...
        int storage_gl_format = gles2::TextureManager::ExtractFormatFromStorageFormat(storage_format);
        EXPECT_EQ(image_gl_format, storage_gl_format);

        int image_gl_type = format_desc.data_type;
        int storage_gl_type = gles2::TextureManager::ExtractTypeFromStorageFormat(storage_format);

        // Ignore the HALF_FLOAT / HALF_FLOAT_OES discrepancy for now.
        // TODO(ericrk): Figure out if we need additional action to support
        // HALF_FLOAT.
        if (!(image_gl_type == GL_HALF_FLOAT_OES && storage_gl_type == GL_HALF_FLOAT)) {
        ...
        bool supports_tex_image = validators->texture_internal_format.IsValid(image_internal_format) && validators->texture_format.IsValid(image_gl_format)
            && validators->pixel_type.IsValid(image_gl_type);
        bool supports_tex_storage = validators->texture_internal_format_storage.IsValid(storage_format);
...
// SharedImageFormat parameterized tests.
class IOSurfaceImageBackingFactoryGMBTest : public IOSurfaceImageBackingFactoryParameterizedTestBase {
public:
    ...
    std::unique_ptr<SharedImageRepresentationFactoryRef> CreateSharedImage(
        gfx::Size size, viz::SharedImageFormat format, SharedImageUsageSet usage, gfx::ColorSpace color_space)
    {
        ...
        handle.id = kBufferId;
        handle.io_surface = gfx::CreateIOSurface(size, buffer_format, /*should_clear=*/true, override_rgba_to_bgra);
        DCHECK(handle.io_surface);

        auto backing
            = backing_factory_->CreateSharedImage(mailbox, format, size, color_space, surface_origin, alpha_type, usage, "TestLabel", std::move(handle));
        ...
        return shared_image_manager_.Register(std::move(backing), &memory_type_tracker_);
    }
};
...
TEST_P(IOSurfaceImageBackingFactoryGMBTest, Basic)
{
    auto format = get_format();
    gfx::Size size(256, 256);
    SharedImageUsageSet usage = { SHARED_IMAGE_USAGE_SCANOUT, SHARED_IMAGE_USAGE_DISPLAY_READ, SHARED_IMAGE_USAGE_DISPLAY_WRITE };
    if (get_gr_context_type() == GrContextType::kGL) {
        usage.PutAll({ SHARED_IMAGE_USAGE_GLES2_READ });
    } else if constexpr (BUILDFLAG(SKIA_USE_DAWN)) {
        usage.PutAll({ SHARED_IMAGE_USAGE_WEBGPU_READ });
    ...
    if (get_gr_context_type() == GrContextType::kGL) {
        ...
        for (int plane = 0; plane < format.NumberOfPlanes(); plane++) {
            EXPECT_TRUE(gl_representation->GetTexturePassthrough(plane)->service_id());
        }
        EXPECT_EQ(size, gl_representation->size());
        EXPECT_EQ(format, gl_representation->format());
        EXPECT_EQ(color_space, gl_representation->color_space());
...
TEST_P(IOSurfaceImageBackingFactoryGMBTest, Dawn_MultipleRepresentationsWithSameUsageFromGraphiteDevice)
{
    if ((get_gr_context_type() != GrContextType::kGraphiteDawn) || GetDawnBackendType() != wgpu::BackendType::Metal) {
        GTEST_SKIP();
    }

    auto format = get_format();
    gfx::Size size(256, 256);
    SharedImageUsageSet usage = { SHARED_IMAGE_USAGE_SCANOUT, SHARED_IMAGE_USAGE_WEBGPU_READ };
    auto color_space = gfx::ColorSpace::CreateSRGB();
    ...
    auto dawn_representation_0 = shared_image_representation_factory_.ProduceDawn(mailbox, device, context_provider->backend_type(), {}, context_state_);
    auto dawn_scoped_access_0
        = dawn_representation_0->BeginScopedAccess(wgpu::TextureUsage::TextureBinding, SharedImageRepresentation::AllowUnclearedAccess::kNo);

    auto dawn_representation_1 = shared_image_representation_factory_.ProduceDawn(mailbox, device, context_provider->backend_type(), {}, context_state_);
    auto dawn_scoped_access_1
        = dawn_representation_1->BeginScopedAccess(wgpu::TextureUsage::TextureBinding, SharedImageRepresentation::AllowUnclearedAccess::kNo);
...
TEST_P(IOSurfaceImageBackingFactoryGMBTest, Dawn_MultipleRepresentationsWithDifferentUsagesFromGraphiteDevice)
{
    if ((get_gr_context_type() != GrContextType::kGraphiteDawn) || GetDawnBackendType() != wgpu::BackendType::Metal) {
        GTEST_SKIP();
    }
    ...
    auto dawn_representation_1 = shared_image_representation_factory_.ProduceDawn(mailbox, device, context_provider->backend_type(), {}, context_state_);
    auto dawn_scoped_access_1 = dawn_representation_1->BeginScopedAccess(wgpu::TextureUsage::CopySrc, SharedImageRepresentation::AllowUnclearedAccess::kNo);
...
TEST_P(IOSurfaceImageBackingFactoryGMBTest, Dawn_SequentialAccessesOnSingleRepresentationFromGraphiteDevice)
{
    if ((get_gr_context_type() != GrContextType::kGraphiteDawn) || GetDawnBackendType() != wgpu::BackendType::Metal) {
        GTEST_SKIP();
    }
    ...
    auto dawn_scoped_access_2 = dawn_representation->BeginScopedAccess(wgpu::TextureUsage::CopySrc, SharedImageRepresentation::AllowUnclearedAccess::kNo);
...
TEST_P(IOSurfaceImageBackingFactoryGMBTest, Dawn_SequentialAccessesOnDifferentRepresentationsFromGraphiteDevice)
{
    if ((get_gr_context_type() != GrContextType::kGraphiteDawn) || GetDawnBackendType() != wgpu::BackendType::Metal) {
        GTEST_SKIP();
    }
    ...
    auto dawn_representation_2 = shared_image_representation_factory_.ProduceDawn(mailbox, device, context_provider->backend_type(), {}, context_state_);
    auto dawn_scoped_access_2 = dawn_representation_2->BeginScopedAccess(wgpu::TextureUsage::CopySrc, SharedImageRepresentation::AllowUnclearedAccess::kNo);
    wgpu::Texture texture_2(dawn_scoped_access_2->texture());
    EXPECT_NE(texture_0.Get(), texture_2.Get());
}
...
TEST_P(IOSurfaceImageBackingFactoryGMBTest, Dawn_SecondAccessFromGraphiteDeviceStaysOpenWhenFirstDestroyed)
{
    if ((get_gr_context_type() != GrContextType::kGraphiteDawn) || GetDawnBackendType() != wgpu::BackendType::Metal) {
        GTEST_SKIP();
    }
    ...
    auto dawn_representation_0 = shared_image_representation_factory_.ProduceDawn(mailbox, device, context_provider->backend_type(), {}, context_state_);
    auto dawn_scoped_access_0 = dawn_representation_0->BeginScopedAccess(wgpu::TextureUsage::CopySrc, SharedImageRepresentation::AllowUnclearedAccess::kNo);
    ...
    auto dst = CreateSharedImage(size, format, usage | SharedImageUsageSet({ SHARED_IMAGE_USAGE_WEBGPU_WRITE }), color_space);
    auto dst_rep = shared_image_representation_factory_.ProduceDawn(dst->mailbox(), device, context_provider->backend_type(), {}, context_state_);
    auto dst_scoped_access = dst_rep->BeginScopedAccess(wgpu::TextureUsage::CopyDst, SharedImageRepresentation::AllowUnclearedAccess::kYes);
    wgpu::Texture dst_texture(dst_scoped_access->texture());
...
const auto kScanoutFormats = ::testing::Values(viz::SinglePlaneFormat::kRGBA_8888, viz::SinglePlaneFormat::kBGRA_8888, viz::SinglePlaneFormat::kBGRA_1010102,
    viz::SinglePlaneFormat::kRGBA_1010102, viz::MultiPlaneFormat::kNV12, viz::MultiPlaneFormat::kP010);

const auto kGMBFormats = ::testing::Values(viz::SinglePlaneFormat::kRGBA_8888, viz::SinglePlaneFormat::kBGRA_8888, viz::SinglePlaneFormat::kBGRA_1010102,
    viz::MultiPlaneFormat::kNV12, viz::MultiPlaneFormat::kP010);

std::string TestBackendTypeParamToString(const testing::TestParamInfo<wgpu::BackendType>& param_info)
{
    // We only support metal & vulkan on mac. And for vulkan, the only driver
    // supported is swiftshader.
    std::string adapter_type = (param_info.param == wgpu::BackendType::Metal) ? "Metal" : "Vulkan_SwiftShader";
    return adapter_type;
}

std::string TestContextTypeFormatParamsToString(const testing::TestParamInfo<std::tuple<viz::SharedImageFormat, ContextTypeParams>>& param_info)
{
    ...
    if ((std::get<1>(param_info.param).context_type == GrContextType::kGL)) {
        ...
        if (std::get<1>(param_info.param).dawn_backend_type == wgpu::BackendType::Metal) {
            context_type += "_Metal";
        } else {
...
INSTANTIATE_TEST_SUITE_P(
    , IOSurfaceImageBackingFactoryDawnTest, testing::Values(wgpu::BackendType::Metal, wgpu::BackendType::Vulkan), TestBackendTypeParamToString);

INSTANTIATE_TEST_SUITE_P(, IOSurfaceImageBackingFactoryScanoutTest,
    testing::Combine(kScanoutFormats,
        testing::Values(ContextTypeParams(GrContextType::kGL), ContextTypeParams(GrContextType::kGraphiteDawn, wgpu::BackendType::Metal),
            ContextTypeParams(GrContextType::kGraphiteDawn, wgpu::BackendType::Vulkan))),
    TestContextTypeFormatParamsToString);
INSTANTIATE_TEST_SUITE_P(, IOSurfaceImageBackingFactoryGMBTest,
