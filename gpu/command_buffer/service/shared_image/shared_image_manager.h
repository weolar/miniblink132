...
class GPU_GLES2_EXPORT SharedImageManager : public base::trace_event::MemoryDumpProvider {
public:
    ...
    explicit SharedImageManager(bool thread_safe = false, bool display_context_on_another_thread = false);
    ...
    std::unique_ptr<SharedImageRepresentationFactoryRef> Register(std::unique_ptr<SharedImageBacking> backing, MemoryTypeTracker* ref);

    std::unique_ptr<SharedImageRepresentationFactoryRef> AddSecondaryReference(const Mailbox& mailbox, MemoryTypeTracker* tracker);
    ...
    // destroyed.
    std::unique_ptr<GLTextureImageRepresentation> ProduceGLTexture(const Mailbox& mailbox, MemoryTypeTracker* ref);
    std::unique_ptr<GLTexturePassthroughImageRepresentation> ProduceGLTexturePassthrough(const Mailbox& mailbox, MemoryTypeTracker* ref);
    std::unique_ptr<SkiaImageRepresentation> ProduceSkia(const Mailbox& mailbox, MemoryTypeTracker* ref, scoped_refptr<SharedContextState> context_state);
    ...
    std::unique_ptr<DawnImageRepresentation> ProduceDawn(const Mailbox& mailbox, MemoryTypeTracker* ref, const wgpu::Device& device,
    ...
    std::unique_ptr<DawnBufferRepresentation> ProduceDawnBuffer(
        const Mailbox& mailbox, MemoryTypeTracker* ref, const wgpu::Device& device, wgpu::BackendType backend_type);
    std::unique_ptr<OverlayImageRepresentation> ProduceOverlay(const Mailbox& mailbox, MemoryTypeTracker* ref);
    std::unique_ptr<MemoryImageRepresentation> ProduceMemory(const Mailbox& mailbox, MemoryTypeTracker* ref);
    std::unique_ptr<RasterImageRepresentation> ProduceRaster(const Mailbox& mailbox, MemoryTypeTracker* ref);
    std::unique_ptr<VideoImageRepresentation> ProduceVideo(VideoDevice device, const Mailbox& mailbox, MemoryTypeTracker* ref);

#if BUILDFLAG(ENABLE_VULKAN)
    std::unique_ptr<VulkanImageRepresentation> ProduceVulkan(const Mailbox& mailbox, MemoryTypeTracker* ref, gpu::VulkanDeviceQueue* vulkan_device_queue,
        gpu::VulkanImplementation& vulkan_impl, bool needs_detiling);
#endif

#if BUILDFLAG(IS_ANDROID)
    std::unique_ptr<LegacyOverlayImageRepresentation> ProduceLegacyOverlay(const Mailbox& mailbox, MemoryTypeTracker* ref);
#endif

#if BUILDFLAG(IS_WIN)
    void UpdateExternalFence(const Mailbox& mailbox, scoped_refptr<gfx::D3DSharedFence> external_fence);
#endif
    ...
    void OnRepresentationDestroyed(const Mailbox& mailbox, SharedImageRepresentation* representation);
...
#if BUILDFLAG(IS_WIN)
    const scoped_refptr<DXGISharedHandleManager>& dxgi_shared_handle_manager() const
    {
