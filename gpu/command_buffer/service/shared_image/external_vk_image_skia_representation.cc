...
std::vector<sk_sp<SkSurface>> ExternalVkImageSkiaImageRepresentation::BeginWriteAccess(int final_msaa_count, const SkSurfaceProps& surface_props,
    ...
    std::unique_ptr<skgpu::MutableTextureState>* end_state)
{
    ...
    auto promise_textures = BeginAccess(/*readonly=*/false, begin_semaphores, end_semaphores);
    ...
    for (size_t plane = 0; plane < promise_textures.size(); ++plane) {
        ...
        if (!surface || surface_props != surface->props() || final_msaa_count != surface_msaa_count_) {
            ...
            surface = SkSurfaces::WrapBackendTexture(gr_context, promise_texture->backendTexture(), surface_origin(), final_msaa_count, sk_color_type,
    ...
    if (backing_impl()->need_synchronization()) {
        ...
        *end_state = std::make_unique<skgpu::MutableTextureState>(skgpu::MutableTextureStates::MakeVulkan(VK_IMAGE_LAYOUT_UNDEFINED, VK_QUEUE_FAMILY_EXTERNAL));
    }
...
std::vector<sk_sp<GrPromiseImageTexture>> ExternalVkImageSkiaImageRepresentation::BeginWriteAccess(
...
std::vector<sk_sp<GrPromiseImageTexture>> ExternalVkImageSkiaImageRepresentation::BeginReadAccess(
    std::vector<GrBackendSemaphore>* begin_semaphores, std::vector<GrBackendSemaphore>* end_semaphores, std::unique_ptr<skgpu::MutableTextureState>* end_state)
{
    DCHECK_EQ(access_mode_, AccessMode::kNone);

    auto promise_textures = BeginAccess(/*readonly=*/true, begin_semaphores, end_semaphores);
    if (promise_textures.empty()) {
...
std::vector<sk_sp<GrPromiseImageTexture>> ExternalVkImageSkiaImageRepresentation::BeginAccess(
    bool readonly, std::vector<GrBackendSemaphore>* begin_semaphores, std::vector<GrBackendSemaphore>* end_semaphores)
{
    ...
    if (backing_impl()->need_synchronization()) {
        ...
        if (!end_access_semaphore_)
            return {};
        end_semaphores->emplace_back(GrBackendSemaphores::MakeVk(end_access_semaphore_.GetVkSemaphore()));
    }
...
#if DCHECK_IS_ON()
    GrVkImageInfo info;
    auto result = GrBackendTextures::GetVkImageInfo(backing_impl()->backend_texture(), &info);
    DCHECK(result);
