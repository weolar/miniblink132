...
AndroidVideoImageBacking::AndroidVideoImageBacking(const Mailbox& mailbox, const gfx::Size& size, const gfx::ColorSpace color_space,
    GrSurfaceOrigin surface_origin, SkAlphaType alpha_type, std::string debug_label, bool is_thread_safe)
    : AndroidImageBacking(mailbox, viz::SinglePlaneFormat::kRGBA_8888, size, color_space, surface_origin, alpha_type,
        ...
        { SHARED_IMAGE_USAGE_DISPLAY_READ, SHARED_IMAGE_USAGE_GLES2_READ, SHARED_IMAGE_USAGE_SCANOUT }, std::move(debug_label),
        viz::SinglePlaneFormat::kRGBA_8888.EstimatedSizeInBytes(size), is_thread_safe, base::ScopedFD())
{
}
...
// Static.
std::unique_ptr<AndroidVideoImageBacking> AndroidVideoImageBacking::Create(const Mailbox& mailbox, const gfx::Size& size, const gfx::ColorSpace color_space,
    ...
    scoped_refptr<SharedContextState> context_state, scoped_refptr<RefCountedLock> drdc_lock)
{
    if (base::android::EnableAndroidImageReader()) {
        return std::make_unique<VideoImageReaderImageBacking>(mailbox, size, color_space, surface_origin, alpha_type, std::move(debug_label),
            std::move(stream_texture_sii), std::move(context_state), std::move(drdc_lock));
    } else {
        DCHECK(!drdc_lock);
        return std::make_unique<VideoSurfaceTextureImageBacking>(
            mailbox, size, color_space, surface_origin, alpha_type, std::move(debug_label), std::move(stream_texture_sii), std::move(context_state));
    }
}

// Static.
std::optional<VulkanYCbCrInfo> AndroidVideoImageBacking::GetYcbcrInfo(
    TextureOwner* texture_owner, viz::VulkanContextProvider* vulkan_context_provider, DawnContextProvider* dawn_context_provider)
{
    ...
    if (vulkan_context_provider) {
        ...
        VulkanImplementation* vk_implementation = vulkan_context_provider->GetVulkanImplementation();
        VkDevice vk_device = vulkan_context_provider->GetDeviceQueue()->GetVulkanDevice();

        VulkanYCbCrInfo ycbcr_info;
        if (!vk_implementation->GetSamplerYcbcrConversionInfo(vk_device, scoped_hardware_buffer->TakeBuffer(), &ycbcr_info)) {
    ...
    if (!device.GetAHardwareBufferProperties(scoped_hardware_buffer->buffer(), &ahb_properties)) {
        LOG(ERROR) << "Failed to get the ycbcr info.";
        return std::nullopt;
    }
    ...
    if (ycbcr_info.vkChromaFilter == wgpu::FilterMode::Linear) {
        format_features |= VK_FORMAT_FEATURE_SAMPLED_IMAGE_YCBCR_CONVERSION_LINEAR_FILTER_BIT;
    }

    return VulkanYCbCrInfo(vk_format, ycbcr_info.externalFormat, ycbcr_info.vkYCbCrModel, ycbcr_info.vkYCbCrRange, ycbcr_info.vkXChromaOffset,
        ycbcr_info.vkYChromaOffset, format_features);
#else
    return std::nullopt;
#endif
}

std::unique_ptr<AbstractTextureAndroid> AndroidVideoImageBacking::GenAbstractTexture(const bool passthrough)
{
    if (passthrough) {
