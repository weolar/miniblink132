...
public:
    explicit ScopedDedicatedMemoryObject(gl::GLApi* api)
        : api_(api)
    {
        api_->glCreateMemoryObjectsEXTFn(1, &id_);
        int dedicated = GL_TRUE;
        api_->glMemoryObjectParameterivEXTFn(id_, GL_DEDICATED_MEMORY_OBJECT_EXT, &dedicated);
    }
    ~ScopedDedicatedMemoryObject()
    {
        api_->glDeleteMemoryObjectsEXTFn(1, &id_);
    }
...
bool UseSeparateGLTexture(SharedContextState* context_state, viz::SharedImageFormat format)
{
...
void WaitSemaphoresOnGrContext(GrDirectContext* gr_context, std::vector<ExternalSemaphore>* semaphores)
{
    ...
    for (auto& semaphore : *semaphores) {
        backend_semaphores.emplace_back(GrBackendSemaphores::MakeVk(semaphore.GetVkSemaphore()));
    }
...
// static
std::unique_ptr<ExternalVkImageBacking> ExternalVkImageBacking::Create(scoped_refptr<SharedContextState> context_state, VulkanCommandPool* command_pool,
    ...
    SkAlphaType alpha_type, SharedImageUsageSet usage, std::string debug_label, const base::flat_map<VkFormat, VkImageUsageFlags>& image_usage_cache,
    base::span<const uint8_t> pixel_data)
{
    ...
    usages_needing_color_attachment
        = SHARED_IMAGE_USAGE_GLES2_WRITE | SHARED_IMAGE_USAGE_RASTER_WRITE | SHARED_IMAGE_USAGE_DISPLAY_WRITE | SHARED_IMAGE_USAGE_WEBGPU_WRITE;

    VkImageUsageFlags vk_usage = VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT;
    ...
    bool request_all_flags = is_external && (HasGLES2ReadOrWriteUsage(usage)) && !UseMinimalUsageFlags(context_state.get());
    ...
    for (int plane = 0; plane < format.NumberOfPlanes(); ++plane) {
        ...
        if (is_external) {
            image = VulkanImage::CreateWithExternalMemory(device_queue, plane_size, vk_format, vk_plane_usage, vk_create, VK_IMAGE_TILING_OPTIMAL);
        } else {
            image = VulkanImage::Create(device_queue, plane_size, vk_format, vk_plane_usage, vk_create, VK_IMAGE_TILING_OPTIMAL);
        }
        if (!image) {
            return nullptr;
        }
    ...
    bool use_separate_gl_texture = UseSeparateGLTexture(context_state.get(), format);
    auto backing = std::make_unique<ExternalVkImageBacking>(base::PassKey<ExternalVkImageBacking>(), mailbox, format, size, color_space, surface_origin,
        alpha_type, usage, std::move(debug_label), estimated_size, std::move(context_state), std::move(textures), command_pool, use_separate_gl_texture);
...
// static
std::unique_ptr<ExternalVkImageBacking> ExternalVkImageBacking::CreateFromGMB(scoped_refptr<SharedContextState> context_state, VulkanCommandPool* command_pool,
    const Mailbox& mailbox, gfx::GpuMemoryBufferHandle handle, viz::SharedImageFormat format, const gfx::Size& size, const gfx::ColorSpace& color_space,
    GrSurfaceOrigin surface_origin, SkAlphaType alpha_type, SharedImageUsageSet usage, std::string debug_label, std::optional<gfx::BufferUsage> buffer_usage)
{
    if (!gpu::IsImageSizeValidForGpuMemoryBufferFormat(size, ToBufferFormat(format))) {
        DLOG(ERROR) << "Invalid image size for format.";
        return nullptr;
    }

    auto* vulkan_implementation = context_state->vk_context_provider()->GetVulkanImplementation();
    auto* device_queue = context_state->vk_context_provider()->GetDeviceQueue();
    DCHECK(vulkan_implementation->CanImportGpuMemoryBuffer(device_queue, handle.type));

    // Note that `format` is implicitly assumed to not be using per-plane
    // sampling as we create only one TextureHolderVk below.
    VkFormat vk_format = format.PrefersExternalSampler() ? ToVkFormatExternalSampler(format) : ToVkFormatSinglePlanar(format);
    auto image = vulkan_implementation->CreateImageFromGpuMemoryHandle(device_queue, handle.Clone(), size, vk_format, color_space);
    if (!image) {
...
std::unique_ptr<ExternalVkImageBacking> ExternalVkImageBacking::CreateWithPixmap(scoped_refptr<SharedContextState> context_state,
    VulkanCommandPool* command_pool, const Mailbox& mailbox, viz::SharedImageFormat format, SurfaceHandle surface_handle, const gfx::Size& size,
    ...
    gfx::BufferUsage buffer_usage)
{
    ...
    scoped_refptr<gfx::NativePixmap> pixmap
        = ui::OzonePlatform::GetInstance()->GetSurfaceFactoryOzone()->CreateNativePixmap(surface_handle, device_queue, size, buffer_format, buffer_usage);
    if (!pixmap) {
    ...
    return CreateFromGMB(std::move(context_state), command_pool, mailbox, std::move(handle), format, size, color_space, surface_origin, alpha_type, usage,
...
ExternalVkImageBacking::ExternalVkImageBacking(base::PassKey<ExternalVkImageBacking>, const Mailbox& mailbox, viz::SharedImageFormat format,
    ...
    std::string debug_label, size_t estimated_size_bytes, scoped_refptr<SharedContextState> context_state, std::vector<TextureHolderVk> vk_textures,
    VulkanCommandPool* command_pool, bool use_separate_gl_texture, gfx::GpuMemoryBufferHandle handle, std::optional<gfx::BufferUsage> buffer_usage)
    : ClearTrackingSharedImageBacking(mailbox, format, size, color_space, surface_origin, alpha_type, usage, std::move(debug_label), estimated_size_bytes,
        /*is_thread_safe=*/false, std::move(buffer_usage))
    ...
    , use_separate_gl_texture_(use_separate_gl_texture)
{
    ...
    if (!handle.is_null()) {
        ...
        pixmap_ = ui::OzonePlatform::GetInstance()->GetSurfaceFactoryOzone()->CreateNativePixmapFromHandle(
            kNullSurfaceHandle, size, ToBufferFormat(format), std::move(handle.native_pixmap_handle));
    }
#endif // BUILDFLAG(IS_OZONE)
}
...
ExternalVkImageBacking::~ExternalVkImageBacking()
{
    ...
    for (auto& vk_texture : vk_textures_) {
        fence_helper()->EnqueueVulkanObjectCleanupForSubmittedWork(std::move(vk_texture.vulkan_image));
    }
    vk_textures_.clear();
...
std::vector<sk_sp<GrPromiseImageTexture>> ExternalVkImageBacking::GetPromiseTextures()
{
...
bool ExternalVkImageBacking::BeginAccess(bool readonly, std::vector<ExternalSemaphore>* external_semaphores, bool is_gl)
{
    DLOG_IF(ERROR, gl_reads_in_progress_ != 0 && !is_gl) << "Backing is being accessed by both GL and Vulkan.";
    ...
    if (gl_reads_in_progress_ && need_synchronization()) {
        ...
        auto release_semaphore = ExternalVkImageGLRepresentationShared::ReleaseTexture(external_semaphore_pool(), texture_ids, GetVkImageLayoutsForGL());
    ...
    if (need_synchronization() && external_semaphores->empty()) {
        ...
        for (auto& vk_texture : vk_textures_) {
            gr_context->setBackendTextureState(
                vk_texture.backend_texture, skgpu::MutableTextureStates::MakeVulkan(VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, VK_QUEUE_FAMILY_EXTERNAL));
        }

        ExternalSemaphore external_semaphore = external_semaphore_pool()->GetOrCreateSemaphore();
        GrBackendSemaphore semaphore = GrBackendSemaphores::MakeVk(external_semaphore.GetVkSemaphore());
...
void ExternalVkImageBacking::EndAccess(bool readonly, ExternalSemaphore external_semaphore, bool is_gl)
{
    if (is_gl && readonly) {
        DCHECK(gl_reads_in_progress_);
    ...
    if (gl_reads_in_progress_ && need_synchronization()) {
        ...
        for (auto& semaphore : external_semaphores) {
            ExternalVkImageGLRepresentationShared::AcquireTexture(&semaphore, texture_ids, GetVkImageLayoutsForGL());
        }
...
bool ExternalVkImageBacking::UploadFromMemory(const std::vector<SkPixmap>& pixmaps)
{
...
void ExternalVkImageBacking::AddSemaphoresToPendingListOrRelease(std::vector<ExternalSemaphore> semaphores)
{
    ...
    for (auto& semaphore : semaphores)
        DCHECK(semaphore);
#endif
    while (pending_semaphores_.size() < kMaxPendingSemaphores && !semaphores.empty()) {
...
void ExternalVkImageBacking::ReleaseSemaphoresWithFenceHelper(std::vector<ExternalSemaphore> semaphores)
{
    if (semaphores.empty()) {
        return;
    }
    ...
    fence_helper()->EnqueueCleanupTaskForSubmittedWork(base::BindOnce(
        [](scoped_refptr<SharedContextState> shared_context_state, std::vector<ExternalSemaphore>, VulkanDeviceQueue* device_queue, bool device_lost) {
...
void ExternalVkImageBacking::ReturnPendingSemaphoresWithFenceHelper(std::vector<ExternalSemaphore> semaphores)
{
    std::move(semaphores.begin(), semaphores.end(), std::back_inserter(pending_semaphores_));
    external_semaphore_pool()->ReturnSemaphoresWithFenceHelper(std::move(pending_semaphores_));
    pending_semaphores_.clear();
}

std::unique_ptr<DawnImageRepresentation> ExternalVkImageBacking::ProduceDawn(SharedImageManager* manager, MemoryTypeTracker* tracker,
    const wgpu::Device& wgpuDevice, wgpu::BackendType backend_type, std::vector<wgpu::TextureFormat> view_formats,
    scoped_refptr<SharedContextState> context_state)
{
    ...
    if (backend_type == wgpu::BackendType::OpenGLES) {
        ...
        return std::make_unique<DawnGLTextureRepresentation>(std::move(image), manager, this, tracker, wgpuDevice, std::move(view_formats));
    }
#endif
    ...
    return std::make_unique<ExternalVkImageDawnImageRepresentation>(
        manager, this, tracker, wgpuDevice, wgpu_format, std::move(view_formats), std::move(memory_fd));
...
bool ExternalVkImageBacking::ProduceGLTextureInternal(bool is_passthrough)
{
    ...
    if (format().PrefersExternalSampler()) {
        LOG(ERROR) << "Importing textures with external sampling into GL is not possible";
        return false;
    }
...
bool ExternalVkImageBacking::CreateGLTexture(bool is_passthrough, size_t plane_index)
{
    ...
    auto& gl_texture = gl_textures_.emplace_back(plane_format, plane_size, is_passthrough, nullptr);
    ...
    if (!use_separate_gl_texture()) {
        ...
        api->glImportMemoryFdEXTFn(memory_object->id(), image_info.fAlloc.fSize, GL_HANDLE_TYPE_OPAQUE_FD_EXT, memory_fd.release());
        ...
        memory_object.emplace(api);
        api->glImportMemoryWin32HandleEXTFn(memory_object->id(), image_info.fAlloc.fSize, GL_HANDLE_TYPE_OPAQUE_WIN32_EXT, memory_handle.Take());
        ...
        memory_object.emplace(api);
        api->glImportMemoryZirconHandleANGLEFn(memory_object->id(), image_info.fAlloc.fSize, GL_HANDLE_TYPE_ZIRCON_VMO_ANGLE, vmo.release());
#else
#error Unsupported OS
#endif
    }

    GLFormatDesc format_desc = context_state()->GetGLFormatCaps().ToGLFormatDesc(format(), plane_index);

    GLuint texture_service_id = 0;
    api->glGenTexturesFn(1, &texture_service_id);
    gl::ScopedTextureBinder scoped_texture_binder(GL_TEXTURE_2D, texture_service_id);
    ...
    if (use_separate_gl_texture()) {
        ...
        if (IsTexStorage2DAvailable()) {
            api->glTexStorage2DEXTFn(GL_TEXTURE_2D, 1, format_desc.storage_internal_format, plane_size.width(), plane_size.height());
        } else {
            api->glTexImage2DFn(GL_TEXTURE_2D, 0, format_desc.image_internal_format, plane_size.width(), plane_size.height(), 0, format_desc.data_format,
        ...
        if (UseMinimalUsageFlags(context_state())) {
            api->glTexStorageMemFlags2DANGLEFn(GL_TEXTURE_2D, 1, format_desc.storage_internal_format, plane_size.width(), plane_size.height(),
                memory_object->id(), 0, vulkan_image->flags(), vulkan_image->usage(), nullptr);
        } else {
            api->glTexStorageMem2DEXTFn(GL_TEXTURE_2D, 1, format_desc.storage_internal_format, plane_size.width(), plane_size.height(), memory_object->id(), 0);
        }
    }

    if (is_passthrough) {
        auto texture = base::MakeRefCounted<gpu::gles2::TexturePassthrough>(texture_service_id, GL_TEXTURE_2D);
        gl_texture.InitializeWithTexture(format_desc, std::move(texture));
    } else {
        auto* texture = gles2::CreateGLES2TextureWithLightRef(texture_service_id, GL_TEXTURE_2D);
        ...
        texture->SetLevelInfo(GL_TEXTURE_2D, 0, format_desc.storage_internal_format, plane_size.width(), plane_size.height(), 1, 0, format_desc.data_format,
...
std::unique_ptr<GLTextureImageRepresentation> ExternalVkImageBacking::ProduceGLTexture(SharedImageManager* manager, MemoryTypeTracker* tracker)
{
    if (gl_textures_.empty()) {
    ...
    return std::make_unique<ExternalVkImageGLRepresentation>(manager, this, tracker, std::move(textures));
}

std::unique_ptr<GLTexturePassthroughImageRepresentation> ExternalVkImageBacking::ProduceGLTexturePassthrough(
    SharedImageManager* manager, MemoryTypeTracker* tracker)
{
    ...
    return std::make_unique<ExternalVkImageGLPassthroughRepresentation>(manager, this, tracker, std::move(textures));
}

std::unique_ptr<SkiaGaneshImageRepresentation> ExternalVkImageBacking::ProduceSkiaGanesh(
    SharedImageManager* manager, MemoryTypeTracker* tracker, scoped_refptr<SharedContextState> context_state)
{
    if (context_state->GrContextIsVulkan()) {
        ...
        return std::make_unique<ExternalVkImageSkiaImageRepresentation>(context_state->gr_context(), manager, this, tracker);
    }
...
std::unique_ptr<OverlayImageRepresentation> ExternalVkImageBacking::ProduceOverlay(SharedImageManager* manager, MemoryTypeTracker* tracker)
{
    return std::make_unique<ExternalVkImageOverlayImageRepresentation>(manager, this, tracker);
}
...
std::pair<std::vector<ExternalVkImageBacking::MapPlaneData>, size_t> ExternalVkImageBacking::GetMapPlaneData() const
{
...
void ExternalVkImageBacking::CopyPixelsFromGLTextureToVkImage()
{
    ...
    VmaAllocator allocator = context_state()->vk_context_provider()->GetDeviceQueue()->vma_allocator();
    VkBuffer stage_buffer = VK_NULL_HANDLE;
    VmaAllocation stage_allocation = VK_NULL_HANDLE;
    VkResult result = vma::CreateBuffer(
        allocator, &buffer_create_info, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, 0, &stage_buffer, &stage_allocation);
    if (result != VK_SUCCESS) {
        DLOG(ERROR) << "vkCreateBuffer() failed." << result;
        return;
    }

    absl::Cleanup destroy_buffer = [&]() { vma::DestroyBuffer(allocator, stage_buffer, stage_allocation); };
    ...
    CHECK(command_buffer);
    {
        ...
        for (size_t plane = 0; plane < vk_textures_.size(); ++plane) {
            ...
            if (image_info.fImageLayout != VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL) {
                command_buffer->TransitionImageLayout(image_info.fImage, image_info.fImageLayout, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
                GrBackendTextures::SetVkImageLayout(&vk_textures_[plane].backend_texture, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
            }

            auto& sk_image_info = plane_data[plane].image_info;
            command_buffer->CopyBufferToImage(stage_buffer, image_info.fImage, sk_image_info.width(), sk_image_info.height(), sk_image_info.width(),
                sk_image_info.height(), plane_data[plane].offset);
        }
    }
    ...
    if (!need_synchronization()) {
        ...
        fence_helper()->EnqueueVulkanObjectCleanupForSubmittedWork(std::move(command_buffer));
        fence_helper()->EnqueueBufferCleanupForSubmittedWork(stage_buffer, stage_allocation);
        return;
    }
    ...
    command_buffer->Submit(begin_access_semaphores.size(), begin_access_semaphores.data(), 1, &vk_end_access_semaphore);
    ...
    CHECK(command_buffer);
    {
        ...
        for (size_t plane = 0; plane < vk_textures_.size(); ++plane) {
            ...
            if (image_info.fImageLayout != VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL) {
                command_buffer->TransitionImageLayout(image_info.fImage, image_info.fImageLayout, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL);
                GrBackendTextures::SetVkImageLayout(&vk_textures_[plane].backend_texture, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL);
            }

            auto& sk_image_info = plane_data[plane].image_info;
            command_buffer->CopyImageToBuffer(stage_buffer, image_info.fImage, sk_image_info.width(), sk_image_info.height(), sk_image_info.width(),
                sk_image_info.height(), plane_data[plane].offset);
        }
    }
...
bool ExternalVkImageBacking::UploadToVkImage(const std::vector<SkPixmap>& pixmaps)
{
    ...
    bool success = true;
    for (size_t plane = 0; plane < vk_textures_.size(); ++plane) {
        if (!gr_context->updateBackendTexture(vk_textures_[plane].backend_texture, &pixmaps[plane],
    ...
    auto end_access_semaphore = external_semaphore_pool()->GetOrCreateSemaphore();
    VkSemaphore vk_end_access_semaphore = end_access_semaphore.GetVkSemaphore();
    GrBackendSemaphore end_access_backend_semaphore = GrBackendSemaphores::MakeVk(vk_end_access_semaphore);
    GrFlushInfo flush_info = {
        .fNumSemaphores = 1,
...
bool ExternalVkImageBacking::UploadToGLTexture(const std::vector<SkPixmap>& pixmaps)
{
    DCHECK(use_separate_gl_texture());
...
bool ExternalVkImageBacking::BeginAccessInternal(bool readonly, std::vector<ExternalSemaphore>* external_semaphores)
{
    ...
    if (reads_in_progress_ && !readonly) {
        DLOG(ERROR) << "Unable to begin write access because a read access is in progress";
        return false;
    }

    if (readonly) {
        DLOG_IF(ERROR, reads_in_progress_) << "Concurrent reading may cause problem.";
        ++reads_in_progress_;
...
void ExternalVkImageBacking::EndAccessInternal(bool readonly, ExternalSemaphore external_semaphore)
{
    if (readonly) {
