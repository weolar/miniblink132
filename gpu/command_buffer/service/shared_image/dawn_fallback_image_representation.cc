...
DawnFallbackImageRepresentation::DawnFallbackImageRepresentation(SharedImageManager* manager, SharedImageBacking* backing, MemoryTypeTracker* tracker,
    wgpu::Device device, wgpu::TextureFormat wgpu_format, std::vector<wgpu::TextureFormat> view_formats)
...
bool DawnFallbackImageRepresentation::ComputeStagingBufferParams(int plane_index, uint32_t* bytes_per_row, size_t* bytes_per_plane) const
{
    ...
    std::optional<size_t> min_bytes_per_row(format.MaybeEstimatedPlaneSizeInBytes(plane_index, gfx::Size(size().width(), 1)));
    ...
    // copies.
    base::CheckedNumeric<uint32_t> aligned_bytes_per_row = base::bits::AlignUp(*min_bytes_per_row, size_t { 256 });
...
bool DawnFallbackImageRepresentation::AllocateStagingBuffers(wgpu::BufferUsage usage, bool map_at_creation, std::vector<StagingBuffer>* buffers)
{
    ...
    for (int plane_index = 0; plane_index < format().NumberOfPlanes(); ++plane_index) {
        ...
        if (!ComputeStagingBufferParams(plane_index, &bytes_per_row, &bytes_per_plane)) {
            return false;
        }
...
SkPixmap DawnFallbackImageRepresentation::MappedStagingBufferToPixmap(const StagingBuffer& staging_buffer, int plane_index, bool writable)
{
    const void* pixels_pointer
        = writable ? staging_buffer.buffer.GetMappedRange(0, wgpu::kWholeMapSize) : staging_buffer.buffer.GetConstMappedRange(0, wgpu::kWholeMapSize);
...
bool DawnFallbackImageRepresentation::ReadbackFromBacking()
{
    ...
    CHECK_EQ(static_cast<size_t>(format.NumberOfPlanes()), staging_buffers.size());

    std::vector<SkPixmap> staging_pixmaps;
    for (int plane_index = 0; plane_index < format.NumberOfPlanes(); ++plane_index) {
        staging_pixmaps.push_back(MappedStagingBufferToPixmap(staging_buffers[plane_index], plane_index, /*writable=*/true));
    }
    ...
    for (int plane_index = 0; plane_index < format.NumberOfPlanes(); ++plane_index) {
        ...
        bool is_yuv_plane = (wgpu_format_ == wgpu::TextureFormat::R8BG8Biplanar420Unorm || wgpu_format_ == wgpu::TextureFormat::R10X6BG10X6Biplanar420Unorm
        ...
        wgpu::Extent3D extent = { static_cast<uint32_t>(plane_size.width()), static_cast<uint32_t>(plane_size.height()), 1 };
...
bool DawnFallbackImageRepresentation::UploadToBacking()
{
    ...
    // Allocate staging buffers. One staging buffer per plane.
    std::vector<StagingBuffer> staging_buffers;
    if (!AllocateStagingBuffers(wgpu::BufferUsage::CopyDst | wgpu::BufferUsage::MapRead,
    ...
    std::vector<SkPixmap> staging_pixmaps;
    for (int plane_index = 0; plane_index < static_cast<int>(staging_buffers.size()); ++plane_index) {
        const auto& staging_buffer_entry = staging_buffers[plane_index];
        ...
        wgpu::FutureWaitInfo wait_info = { staging_buffer_entry.buffer.MapAsync(
            ...
            [](wgpu::MapAsyncStatus status, wgpu::StringView, bool* success) { *success = status == wgpu::MapAsyncStatus::Success; }, &success) };

        if (device_.GetAdapter().GetInstance().WaitAny(1, &wait_info, UINT64_MAX) != wgpu::WaitStatus::Success) {
            return false;
        }
        ...
        staging_pixmaps.push_back(MappedStagingBufferToPixmap(staging_buffers[plane_index], plane_index, /*writable=*/false));
    }

    return backing()->UploadFromMemory(staging_pixmaps);
}

wgpu::Texture DawnFallbackImageRepresentation::BeginAccess(wgpu::TextureUsage wgpu_texture_usage, wgpu::TextureUsage internal_usage)
{
    const std::string debug_label = "DawnFallbackSharedImageRep(" + CreateLabelForSharedImageUsage(usage()) + ")";
    ...
    internalDesc.internalUsage = internal_usage | wgpu::TextureUsage::CopySrc | wgpu::TextureUsage::CopyDst;
...
void DawnFallbackImageRepresentation::EndAccess()
{
    ...
    // initialized.
    if (dawn::native::IsTextureSubresourceInitialized(texture_.Get(), /*baseMipLevel=*/0, /*levelCount=*/1,
            /*baseArrayLayer=*/0,
            /*layerCount=*/1)
        && UploadToBacking()) {
        SetCleared();
