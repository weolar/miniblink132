...
std::vector<uint8_t> RepackPixelDataAsRgb(const gfx::Size& size, const SkPixmap& src_pixmap, bool src_is_bgrx)
{
    ...
    size_t dst_stride = base::bits::AlignUp<size_t>(size.width() * kDstBytesPerPixel, 4);
...
std::vector<uint8_t> RepackPixelDataWithStride(const gfx::Size& size, const SkPixmap& src_pixmap, size_t dst_stride)
{
    const uint8_t* src_data = static_cast<const uint8_t*>(src_pixmap.addr());
    size_t src_stride = src_pixmap.rowBytes();
    ...
    std::vector<uint8_t> dst_data(dst_stride * size.height());
    CopyImagePlane(src_data, src_stride, dst_data.data(), dst_stride, src_pixmap.info().minRowBytes(), size.height());

    return dst_data;
}

void UnpackPixelDataWithStride(const gfx::Size& size, const std::vector<uint8_t>& src_data, size_t src_stride, const SkPixmap& dst_pixmap)
{
    ...
    CopyImagePlane(src_data.data(), src_stride, dst_data, dst_stride, dst_pixmap.info().minRowBytes(), size.height());
}
