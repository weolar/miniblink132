...
public:
    explicit TextureHolder(gles2::Texture* texture)
        : texture_(texture)
    {
    }
    explicit TextureHolder(scoped_refptr<gles2::TexturePassthrough> texture_passthrough)
...
public:
    ...
    GLRepresentationShared(EGLImageBacking* backing, std::vector<scoped_refptr<TextureHolder>> texture_holders)
        : backing_(backing)
...
class EGLImageBacking::GLTextureEGLImageRepresentation : public GLTextureImageRepresentation {
public:
    GLTextureEGLImageRepresentation(
        SharedImageManager* manager, EGLImageBacking* backing, MemoryTypeTracker* tracker, std::vector<scoped_refptr<TextureHolder>> texture_holders)
        : GLTextureImageRepresentation(manager, backing, tracker)
        , shared_(backing, std::move(texture_holders))
    {
    }

    GLTextureEGLImageRepresentation(const GLTextureEGLImageRepresentation&) = delete;
    GLTextureEGLImageRepresentation& operator=(const GLTextureEGLImageRepresentation&) = delete;

    ~GLTextureEGLImageRepresentation() override = default;

    bool BeginAccess(GLenum mode) override
    {
        return shared_.BeginAccess(mode);
    }
...
class EGLImageBacking::GLTexturePassthroughEGLImageRepresentation : public GLTexturePassthroughImageRepresentation {
public:
    ...
    GLTexturePassthroughEGLImageRepresentation(const GLTexturePassthroughEGLImageRepresentation&) = delete;
    GLTexturePassthroughEGLImageRepresentation& operator=(const GLTexturePassthroughEGLImageRepresentation&) = delete;
...
EGLImageBacking::EGLImageBacking(const Mailbox& mailbox, viz::SharedImageFormat format, const gfx::Size& size, const gfx::ColorSpace& color_space,
    GrSurfaceOrigin surface_origin, SkAlphaType alpha_type, SharedImageUsageSet usage, std::string debug_label, size_t estimated_size,
    const std::vector<GLCommonImageBackingFactory::FormatInfo>& format_info, const GpuDriverBugWorkarounds& workarounds, bool use_passthrough,
    base::span<const uint8_t> pixel_data)
    : ClearTrackingSharedImageBacking(
        mailbox, format, size, color_space, surface_origin, alpha_type, usage, std::move(debug_label), estimated_size, true /*is_thread_safe*/)
...
template <class T> std::unique_ptr<T> EGLImageBacking::ProduceGLTextureInternal(SharedImageManager* manager, MemoryTypeTracker* tracker)
{
    ...
    // doesn't need lock.
    if (created_on_context_ == gl::g_current_gl_context && !source_texture_holders_.empty()) {
        return std::make_unique<T>(manager, this, tracker, source_texture_holders_);
    }

    auto texture_holders = GenEGLImageSiblings(/*pixel_data=*/base::span<const uint8_t>());
    if (texture_holders.empty()) {
        return nullptr;
    }
    return std::make_unique<T>(manager, this, tracker, std::move(texture_holders));
}

std::unique_ptr<GLTextureImageRepresentation> EGLImageBacking::ProduceGLTexture(SharedImageManager* manager, MemoryTypeTracker* tracker)
{
    return ProduceGLTextureInternal<GLTextureEGLImageRepresentation>(manager, tracker);
}

std::unique_ptr<GLTexturePassthroughImageRepresentation> EGLImageBacking::ProduceGLTexturePassthrough(SharedImageManager* manager, MemoryTypeTracker* tracker)
{
    return ProduceGLTextureInternal<GLTexturePassthroughEGLImageRepresentation>(manager, tracker);
}

std::unique_ptr<SkiaGaneshImageRepresentation> EGLImageBacking::ProduceSkiaGanesh(
...
std::unique_ptr<DawnImageRepresentation> EGLImageBacking::ProduceDawn(SharedImageManager* manager, MemoryTypeTracker* tracker, const wgpu::Device& device,
    wgpu::BackendType backend_type, std::vector<wgpu::TextureFormat> view_formats, scoped_refptr<SharedContextState> context_state)
{
    ...
    if (backend_type == wgpu::BackendType::OpenGLES) {
        ...
        return std::make_unique<DawnEGLImageRepresentation>(
            std::move(gl_representation), egl_image, manager, this, tracker, device.Get(), std::move(view_formats));
    }
...
void EGLImageBacking::EndRead(const GLRepresentationShared* reader)
{
    ...
    AutoLock auto_lock(this);
    read_fences_[gl::g_current_gl_context] = base::MakeRefCounted<gl::SharedGLFenceEGL>();
}

gl::ScopedEGLImage EGLImageBacking::GenEGLImageSibling(base::span<const uint8_t> pixel_data, std::vector<GLuint>& service_ids, int plane)
{
    ...
    if (format_info.supports_storage && IsTexStorage2DAvailable()) {
        api->glTexStorage2DEXTFn(target, 1, format_info.adjusted_storage_internal_format, plane_size.width(), plane_size.height());
        ...
        if (!pixel_data.empty()) {
            ...
            api->glTexSubImage2DFn(
                target, 0, 0, 0, plane_size.width(), plane_size.height(), format_info.adjusted_format, format_info.gl_type, pixel_data.data());
        }
    } else if (format_info.is_compressed) {
        ScopedUnpackState scoped_unpack_state(!pixel_data.empty());
        api->glCompressedTexImage2DFn(
            target, 0, format_info.image_internal_format, plane_size.width(), plane_size.height(), 0, pixel_data.size(), pixel_data.data());
    } else {
        ScopedUnpackState scoped_unpack_state(!pixel_data.empty());
        api->glTexImage2DFn(target, 0, format_info.image_internal_format, plane_size.width(), plane_size.height(), 0, format_info.adjusted_format,
            format_info.gl_type, pixel_data.data());
    }

    // Use service id of the texture as a source to create the EGLImage.
    const EGLint egl_attrib_list[] = { EGL_GL_TEXTURE_LEVEL_KHR, 0, EGL_IMAGE_PRESERVED_KHR, EGL_TRUE, EGL_NONE };
    return gl::MakeScopedEGLImage(eglGetCurrentContext(), EGL_GL_TEXTURE_2D_KHR, reinterpret_cast<EGLClientBuffer>(service_ids[plane]), egl_attrib_list);
}

std::vector<scoped_refptr<EGLImageBacking::TextureHolder>> EGLImageBacking::GenEGLImageSiblings(base::span<const uint8_t> pixel_data)
{
    GLenum target = GL_TEXTURE_2D;
    gl::GLApi* api = gl::g_current_gl_context;
    ...
    bool create_egl_images = false;
    {
        ...
        if (create_egl_images) {
            for (int plane = 0; plane < num_planes; plane++) {
                gl::ScopedEGLImage egl_image = GenEGLImageSibling(pixel_data, service_ids, plane);
    ...
    for (int plane = 0; plane < num_planes; plane++) {
        if (use_passthrough_) {
            auto texture_passthrough = base::MakeRefCounted<gpu::gles2::TexturePassthrough>(service_ids[plane], GL_TEXTURE_2D);
            texture_holders.push_back(base::MakeRefCounted<TextureHolder>(std::move(texture_passthrough)));
        } else {
            auto* texture = gles2::CreateGLES2TextureWithLightRef(service_ids[plane], GL_TEXTURE_2D);
            gfx::Size plane_size = format().GetPlaneSize(plane, size());
            ...
            // Set the level info.
            texture->SetLevelInfo(GL_TEXTURE_2D, 0, format_info_[plane].gl_format, plane_size.width(), plane_size.height(), 1, 0, format_info_[plane].gl_format,
                format_info_[plane].gl_type, cleared_rect);

            texture->SetImmutable(/*immutable=*/true, /*immutable_storage=*/false);
            texture_holders.push_back(base::MakeRefCounted<TextureHolder>(std::move(texture)));
        }
    }
