...
class AHardwareBufferImageBackingFactoryTest : public SharedImageTestBase, public testing::WithParamInterface<GrContextType> {
public:
    GrContextType GrContextType() const
    {
        return GetParam();
    }
    ...
    void SetUp() override
    {
        ...
        backing_factory_
            = std::make_unique<AHardwareBufferImageBackingFactory>(context_state_->feature_info(), gpu_preferences_, context_state_->vk_context_provider());
    }
};

class GlLegacySharedImage {
public:
    GlLegacySharedImage(SharedImageBackingFactory* backing_factory, bool is_thread_safe, bool concurrent_read_write, SharedImageManager* shared_image_manager,
        MemoryTypeTracker* memory_type_tracker, SharedImageRepresentationFactory* shared_image_representation_factory);
    ~GlLegacySharedImage();
...
{
    GlLegacySharedImage gl_legacy_shared_image { backing_factory_.get(), /*is_thread_safe=*/false,
        /*concurrent_read_write=*/false, &shared_image_manager_, &memory_type_tracker_, &shared_image_representation_factory_ };

    // Validate a SkiaImageRepresentation.
    auto skia_representation = shared_image_representation_factory_.ProduceSkia(gl_legacy_shared_image.mailbox(), context_state_.get());
    ...
    if (IsGraphiteDawn()) {
        ...
        EXPECT_EQ(gl_legacy_shared_image.size(), gfx::SkISizeToSize(graphite_texture.dimensions()));
    } else {
...
TEST_F(AHardwareBufferImageBackingFactoryTest, GLWriteSkiaRead)
{
    ...
    gpu::SharedImageUsageSet usage = SHARED_IMAGE_USAGE_GLES2_WRITE | SHARED_IMAGE_USAGE_DISPLAY_READ;
...
TEST_P(AHardwareBufferImageBackingFactoryTest, ProduceDawnOpenGLES)
{
    ...
    GrSurfaceOrigin surface_origin = kTopLeft_GrSurfaceOrigin;
    SkAlphaType alpha_type = kPremul_SkAlphaType;
    gpu::SharedImageUsageSet usage = SHARED_IMAGE_USAGE_WEBGPU_WRITE | SHARED_IMAGE_USAGE_DISPLAY_READ | SHARED_IMAGE_USAGE_SCANOUT;
    gpu::SurfaceHandle surface_handle = gpu::kNullSurfaceHandle;
    auto backing = backing_factory_->CreateSharedImage(
        mailbox, format, surface_handle, size, color_space, surface_origin, alpha_type, usage, "TestLabel", /*is_thread_safe=*/false);
    EXPECT_TRUE(backing);
...
TEST_P(AHardwareBufferImageBackingFactoryTest, InitialData)
{
    ...
    auto image_info = SkImageInfo::Make(gfx::SizeToSkISize(size), viz::ToClosestSkColorType(true, format), alpha_type);
    SkBitmap expected_bitmap;
    expected_bitmap.allocPixels(image_info);

    base::span<uint8_t> pixel_span(static_cast<uint8_t*>(expected_bitmap.pixmap().writable_addr()), expected_bitmap.computeByteSize());
    ...
    auto backing = backing_factory_->CreateSharedImage(
        mailbox, format, size, color_space, surface_origin, alpha_type, usage, "TestLabel", /*is_thread_safe=*/false, pixel_span);
    EXPECT_TRUE(backing);
...
{
    GlLegacySharedImage gl_legacy_shared_image { backing_factory_.get(), /*is_thread_safe=*/true,
        /*concurrent_read_write=*/false, &shared_image_manager_, &memory_type_tracker_, &shared_image_representation_factory_ };
    ...
    auto skia_representation2 = shared_image_representation_factory_.ProduceSkia(gl_legacy_shared_image.mailbox(), context_state_.get());
    std::vector<GrBackendSemaphore> begin_semaphores2;
    std::vector<GrBackendSemaphore> end_semaphores2;
    std::unique_ptr<SkiaImageRepresentation::ScopedWriteAccess> scoped_write_access2;
    scoped_write_access2
        = skia_representation2->BeginScopedWriteAccess(&begin_semaphores2, &end_semaphores2, SharedImageRepresentation::AllowUnclearedAccess::kYes);
...
TEST_P(AHardwareBufferImageBackingFactoryTest, CanHaveMultipleReaders)
{
    ...
    std::unique_ptr<SkiaImageRepresentation::ScopedReadAccess> scoped_read_access2;
    scoped_read_access2 = skia_representation2->BeginScopedReadAccess(&begin_semaphores, &end_semaphores);
...
TEST_P(AHardwareBufferImageBackingFactoryTest, CannotReadWhileWriting)
{
    ...
    std::vector<GrBackendSemaphore> end_semaphores2;
    std::unique_ptr<SkiaImageRepresentation::ScopedReadAccess> scoped_read_access;
    scoped_read_access = skia_representation2->BeginScopedReadAccess(&begin_semaphores2, &end_semaphores2);
...
{
    GlLegacySharedImage gl_legacy_shared_image { backing_factory_.get(), /*is_thread_safe=*/true,
        /*concurrent_read_write=*/true, &shared_image_manager_, &memory_type_tracker_, &shared_image_representation_factory_ };
    ...
    std::unique_ptr<SkiaImageRepresentation::ScopedWriteAccess> scoped_write_access;
    scoped_write_access
        = skia_representation2->BeginScopedWriteAccess(&begin_semaphores, &end_semaphores, SharedImageRepresentation::AllowUnclearedAccess::kYes);
    EXPECT_TRUE(scoped_write_access);
    EXPECT_EQ(0u, begin_semaphores.size());
    EXPECT_EQ(0u, end_semaphores.size());
}

GlLegacySharedImage::GlLegacySharedImage(SharedImageBackingFactory* backing_factory, bool is_thread_safe, bool concurrent_read_write,
    ...
    : size_(256, 256)
{
    ...
    SharedImageUsageSet usage = { SHARED_IMAGE_USAGE_GLES2_READ, SHARED_IMAGE_USAGE_RASTER_WRITE, SHARED_IMAGE_USAGE_SCANOUT };
    if (!is_thread_safe) {
...
TEST_P(AHardwareBufferImageBackingFactoryTest, Overlay)
{
    ...
    auto overlay_representation = shared_image_representation_factory_.ProduceOverlay(gl_legacy_shared_image.mailbox());
...
INSTANTIATE_TEST_SUITE_P(
    , AHardwareBufferImageBackingFactoryTest, testing::Values(GrContextType::kGL, GrContextType::kGraphiteDawn), testing::PrintToStringParamName());
