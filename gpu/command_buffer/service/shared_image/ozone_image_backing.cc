...
{
    return gfx::BufferSizeForBufferFormat(pixmap.GetBufferSize(), pixmap.GetBufferFormat());
}
...
class OzoneImageBacking::OverlayOzoneImageRepresentation : public OverlayImageRepresentation {
public:
    OverlayOzoneImageRepresentation(SharedImageManager* manager, SharedImageBacking* backing, MemoryTypeTracker* tracker)
...
private:
    bool BeginReadAccess(gfx::GpuFenceHandle& acquire_fence) override
    {
        ...
        bool need_end_fence;
        if (!ozone_backing->BeginAccess(/*readonly=*/true, AccessStream::kOverlay, &fences, need_end_fence)) {
            return false;
        }
    ...
    {
        auto* ozone_backing = static_cast<OzoneImageBacking*>(backing());
        ozone_backing->EndAccess(/*readonly=*/true, AccessStream::kOverlay, std::move(release_fence));
    }
};
...
gfx::GpuMemoryBufferHandle OzoneImageBacking::GetSinglePlaneGpuMemoryBufferHandle(uint32_t index)
{
...
std::unique_ptr<DawnImageRepresentation> OzoneImageBacking::ProduceDawn(SharedImageManager* manager, MemoryTypeTracker* tracker, const wgpu::Device& device,
    wgpu::BackendType backend_type, std::vector<wgpu::TextureFormat> view_formats, scoped_refptr<SharedContextState> context_state)
{
    ...
    return std::make_unique<DawnOzoneImageRepresentation>(manager, this, tracker, device, webgpu_format, std::move(view_formats), pixmap_);
#else // !BUILDFLAG(USE_DAWN)
    return nullptr;
#endif
}

std::unique_ptr<SkiaGraphiteImageRepresentation> OzoneImageBacking::ProduceSkiaGraphite(
...
scoped_refptr<OzoneImageGLTexturesHolder> OzoneImageBacking::RetainGLTexturePerContextCache()
{
    ...
    if (!current_context->default_surface()) {
        return OzoneImageGLTexturesHolder::CreateAndInitTexturesHolder(this, pixmap_);
    }
    ...
    if (found != per_context_cached_textures_holders_.end()) {
        ...
        if (!format().PrefersExternalSampler()) {
            DCHECK_EQ(static_cast<int>(holder->GetNumberOfTextures()), format().NumberOfPlanes());
        }
        return holder;
    }
    ...
    const auto context_cache_pair = base::ranges::find_if(per_context_cached_textures_holders_.begin(), per_context_cached_textures_holders_.end(),
        [current_context](const auto& holder_per_context) { return holder_per_context.first->CanShareTexturesWithContext(current_context); });
    ...
    if (context_cache_pair != per_context_cached_textures_holders_.end()) {
        ...
        new_holder = OzoneImageGLTexturesHolder::CreateAndInitTexturesHolder(this, pixmap_);
    }
    ...
    auto result = per_context_cached_textures_holders_.insert(std::make_pair(current_context, std::move(new_holder)));
...
std::unique_ptr<GLTexturePassthroughImageRepresentation> OzoneImageBacking::ProduceGLTexturePassthrough(SharedImageManager* manager, MemoryTypeTracker* tracker)
{
    ...
    const bool should_mark_context_lost_textures_holder_ = (texture_holder->GetCacheCount() == 0);
    return std::make_unique<GLTexturePassthroughOzoneImageRepresentation>(
        manager, this, tracker, std::move(texture_holder), should_mark_context_lost_textures_holder_);
}

std::unique_ptr<SkiaGaneshImageRepresentation> OzoneImageBacking::ProduceSkiaGanesh(
    SharedImageManager* manager, MemoryTypeTracker* tracker, scoped_refptr<SharedContextState> context_state)
{
    if (context_state->GrContextIsGL()) {
        ...
        auto skia_representation = SkiaGLImageRepresentation::Create(std::move(gl_representation), std::move(context_state), manager, this, tracker);
    ...
    if (context_state->GrContextIsVulkan()) {
        ...
        if (format().is_single_plane() || format().PrefersExternalSampler()) {
            ...
            // entire GMB.
            gfx::GpuMemoryBufferHandle gmb_handle = GetGpuMemoryBufferHandle();
            auto vk_format = format().is_single_plane() ? ToVkFormat(format(), /*plane_index=*/0) : ToVkFormatExternalSampler(format());
            auto vulkan_image
                = vulkan_implementation->CreateImageFromGpuMemoryHandle(device_queue, std::move(gmb_handle), size(), vk_format, gfx::ColorSpace());
            if (!vulkan_image) {
                return nullptr;
            }
            ...
            for (int i = 0; i < format().NumberOfPlanes(); i++) {
                gfx::GpuMemoryBufferHandle gmb_handle = GetSinglePlaneGpuMemoryBufferHandle(i);
                gfx::Size plane_size = format().GetPlaneSize(i, size());
                VkFormat vk_format = ToVkFormat(format(), i);
                auto vulkan_image
                    = vulkan_implementation->CreateImageFromGpuMemoryHandle(device_queue, std::move(gmb_handle), plane_size, vk_format, gfx::ColorSpace());
                if (!vulkan_image) {
                    return nullptr;
                }
                vulkan_images.push_back(std::move(vulkan_image));
        ...
        return std::make_unique<SkiaVkOzoneImageRepresentation>(manager, this, std::move(context_state), std::move(vulkan_images), tracker);
#else
...
std::unique_ptr<OverlayImageRepresentation> OzoneImageBacking::ProduceOverlay(SharedImageManager* manager, MemoryTypeTracker* tracker)
{
    return std::make_unique<OverlayOzoneImageRepresentation>(manager, this, tracker);
}

OzoneImageBacking::OzoneImageBacking(const Mailbox& mailbox, viz::SharedImageFormat format, const gfx::Size& size, const gfx::ColorSpace& color_space,
    GrSurfaceOrigin surface_origin, SkAlphaType alpha_type, SharedImageUsageSet usage, std::string debug_label, scoped_refptr<SharedContextState> context_state,
    scoped_refptr<gfx::NativePixmap> pixmap, const GpuDriverBugWorkarounds& workarounds, std::optional<gfx::BufferUsage> buffer_usage)
    : ClearTrackingSharedImageBacking(mailbox, format, size, color_space, surface_origin, alpha_type, usage, std::move(debug_label),
        GetPixmapSizeInBytes(*pixmap), false, std::move(buffer_usage))
    , pixmap_(std::move(pixmap))
    , context_state_(std::move(context_state))
    , workarounds_(workarounds)
    , imported_from_exo_(IsExoTexture(this->debug_label()))
{
    bool used_by_skia = usage.HasAny(SHARED_IMAGE_USAGE_RASTER_READ | SHARED_IMAGE_USAGE_RASTER_WRITE | SHARED_IMAGE_USAGE_DISPLAY_READ);
    bool used_by_gl = (HasGLES2ReadOrWriteUsage(usage)) || (used_by_skia && context_state_->gr_context_type() == GrContextType::kGL);
    bool used_by_vulkan = used_by_skia && context_state_->gr_context_type() == GrContextType::kVulkan;
    bool used_by_webgpu = usage.HasAny(SHARED_IMAGE_USAGE_WEBGPU_READ | SHARED_IMAGE_USAGE_WEBGPU_WRITE);
    ...
    if (write_streams_count_ == 1) {
        ...
        last_write_stream_ = used_by_gl ? AccessStream::kGL : (used_by_vulkan ? AccessStream::kVulkan : AccessStream::kWebGPU);
    }
}
...
#if BUILDFLAG(ENABLE_VULKAN)
std::unique_ptr<VulkanImageRepresentation> OzoneImageBacking::ProduceVulkan(SharedImageManager* manager, MemoryTypeTracker* tracker,
    gpu::VulkanDeviceQueue* vulkan_device_queue, gpu::VulkanImplementation& vulkan_impl, bool needs_detiling)
{
    ...
    if (needs_detiling && image_format == viz::MultiPlaneFormat::kP010) {
        ...
        image_size = gfx::Size(image_size.width(), image_size.height() * kMT2TBppNumerator / kMT2TBppDenominator);
        base::CheckedNumeric<uint32_t> stride = gmb_handle.native_pixmap_handle.planes[0].stride;
        ...
        gmb_handle.native_pixmap_handle.planes[1].stride = gmb_handle.native_pixmap_handle.planes[0].stride;
    ...
    auto vulkan_image = vulkan_impl.CreateImageFromGpuMemoryHandle(vulkan_device_queue, std::move(gmb_handle), image_size,
        image_format.PrefersExternalSampler() ? ToVkFormatExternalSampler(image_format) : ToVkFormatSinglePlanar(image_format), color_space());

    if (!vulkan_image) {
        return nullptr;
    }

    return std::make_unique<VulkanOzoneImageRepresentation>(manager, this, tracker, std::move(vulkan_image), vulkan_device_queue, vulkan_impl);
}
#endif
...
bool OzoneImageBacking::UploadFromMemory(const std::vector<SkPixmap>& pixmaps)
{
    ...
    auto representation = ProduceSkiaGanesh(nullptr, context_state_->memory_type_tracker(), context_state_);
    ...
    auto dest_scoped_access = representation->BeginScopedWriteAccess(&begin_semaphores, &end_semaphores, SharedImageRepresentation::AllowUnclearedAccess::kYes,
        /*use_sk_surface=*/false);
    if (!dest_scoped_access) {
        return false;
    }
    if (!begin_semaphores.empty()) {
        bool result = context_state_->gr_context()->wait(begin_semaphores.size(), begin_semaphores.data(),
    ...
    for (int plane = 0; plane < format().NumberOfPlanes(); ++plane) {
        GrBackendTexture backend_texture = dest_scoped_access->promise_image_texture(plane)->backendTexture();
        if (!context_state_->gr_context()->updateBackendTexture(backend_texture, &pixmaps[plane],
...
#if BUILDFLAG(USE_DAWN)
bool OzoneImageBacking::UploadFromMemoryGraphite(const std::vector<SkPixmap>& pixmaps)
{
    DCHECK(context_state_->IsGraphiteDawn());
    auto representation = ProduceSkiaGraphite(nullptr, context_state_->memory_type_tracker(), context_state_);
    DCHECK_EQ(pixmaps.size(), representation->NumPlanesExpected());
    ...
    bool written = true;
    for (int plane = 0; plane < format().NumberOfPlanes(); ++plane) {
        skgpu::graphite::BackendTexture backend_texture = dest_scoped_access->graphite_texture(plane);
        if (!context_state_->gpu_main_graphite_recorder()->updateBackendTexture(backend_texture, &pixmaps[plane],
                /*numLevels=*/1)) {
            written = false;
        }
    }
...
void OzoneImageBacking::FlushAndSubmitIfNecessary(std::vector<GrBackendSemaphore> signal_semaphores, SharedContextState* const shared_context_state)
{
    bool sync_cpu = gpu::ShouldVulkanSyncCpuForSkiaSubmit(shared_context_state->vk_context_provider());
    ...
    if (!signal_semaphores.empty()) {
        ...
        gpu::AddVulkanCleanupTaskForSkiaFlush(shared_context_state->vk_context_provider(), &flush_info);
    }
...
bool OzoneImageBacking::BeginAccess(bool readonly, AccessStream access_stream, std::vector<gfx::GpuFenceHandle>* fences, bool& need_end_fence)
{
    ...
    if (!write_fence_.is_null() && last_write_stream_ != access_stream) {
        DCHECK(external_write_fence_.is_null()); // `external_write_fence_` should be null.
    ...
    if (readonly) {
        ...
        // the same stream.
        need_end_fence = (write_streams_count_ > 1) || (last_write_stream_ != access_stream);
    } else {
        // Log if it's a single write stream and write comes from a different stream
        // than expected (GL, Vulkan or WebGPU).
        LOG_IF(DFATAL, write_streams_count_ == 1 && last_write_stream_ != access_stream)
            << "Unexpected write stream: " << static_cast<int>(access_stream) << ", " << static_cast<int>(last_write_stream_) << ", " << write_streams_count_;
        ...
        // raster delegation.
        const bool skip_fence_in_delegation = usage().Has(SHARED_IMAGE_USAGE_RASTER_DELEGATED_COMPOSITING) && context_state_->GrContextIsGL();
...
void OzoneImageBacking::EndAccess(bool readonly, AccessStream access_stream, gfx::GpuFenceHandle fence)
{
    // Track reads and writes if not being used for concurrent read/writes.
    if (!usage().Has(SHARED_IMAGE_USAGE_CONCURRENT_READ_WRITE)) {
        if (readonly) {
...
void OzoneImageBacking::OnGLContextLostOrDestroy(gl::GLContext* context, bool mark_context_lost)
{
    auto it = per_context_cached_textures_holders_.find(context);
    CHECK(it != per_context_cached_textures_holders_.end(), base::NotFatalUntil::M130);
...
void OzoneImageBacking::DestroyTexturesOnContext(OzoneImageGLTexturesHolder* holder, gl::GLContext* context)
{
