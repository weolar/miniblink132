...
void InsertRecordingAndSubmit(gpu::SharedContextState* context, bool sync_cpu = false)
{
    ...
    context->graphite_context()->submit(sync_cpu ? skgpu::graphite::SyncToCpu::kYes : skgpu::graphite::SyncToCpu::kNo);
}
...
// static
SkBitmap SharedImageTestBase::MakeRedBitmap(SkColorType color_type, const gfx::Size& size, size_t added_stride)
{
    SkImageInfo info = SkImageInfo::Make(size.width(), size.height(), color_type, kOpaque_SkAlphaType);
    const size_t stride = info.minRowBytes() + added_stride * info.bytesPerPixel();
...
// static
std::vector<SkBitmap> SharedImageTestBase::AllocateRedBitmaps(viz::SharedImageFormat format, const gfx::Size& size, size_t added_stride)
{
...
// static
std::vector<SkPixmap> SharedImageTestBase::GetSkPixmaps(const std::vector<SkBitmap>& bitmaps)
{
...
SharedImageTestBase::SharedImageTestBase()
{
    gpu_preferences_.use_passthrough_cmd_decoder
        = gles2::UsePassthroughCommandDecoder(base::CommandLine::ForCurrentProcess()) && gles2::PassthroughCommandDecoderSupported();
}
...
bool SharedImageTestBase::IsGraphiteDawnSupported()
{
    ...
    return base::android::BuildInfo::GetInstance()->sdk_int() >= base::android::SDK_VERSION_Q;
#else
    return false;
#endif
}
...
void SharedImageTestBase::InitializeContext(GrContextType context_type)
{
    ...
    if (context_type == GrContextType::kGraphiteDawn) {
        ...
        dawn_context_provider_ = DawnContextProvider::CreateWithBackend(
            GetDawnBackendType(), DawnForceFallbackAdapter(), gpu_preferences_, DawnContextProvider::DefaultValidateAdapterFn);
        ...
        vulkan_context_provider_ = viz::VulkanInProcessContextProvider::Create(vulkan_implementation_.get());
    ...
    gl_surface_ = gl::init::CreateOffscreenGLSurface(gl::GetDefaultDisplayEGL(), gfx::Size());
    ASSERT_TRUE(gl_surface_);
    gl_context_ = gl::init::CreateGLContext(nullptr, gl_surface_.get(), gl::GLContextAttribs());
    ASSERT_TRUE(gl_context_);
    bool make_current_result = gl_context_->MakeCurrent(gl_surface_.get());
    ASSERT_TRUE(make_current_result);

    context_state_ = base::MakeRefCounted<SharedContextState>(base::MakeRefCounted<gl::GLShareGroup>(), gl_surface_, gl_context_,
    ...
    bool initialize_gl = context_state_->InitializeGL(gpu_preferences_, base::MakeRefCounted<gles2::FeatureInfo>(gpu_workarounds_, GpuFeatureInfo()));
    ASSERT_TRUE(initialize_gl);

    bool initialize_skia = context_state_->InitializeSkia(gpu_preferences_, gpu_workarounds_);
    ASSERT_TRUE(initialize_skia);
}

void SharedImageTestBase::VerifyPixelsWithReadback(const Mailbox& mailbox, const std::vector<SkBitmap>& expected_bitmaps)
{
    if (gr_context()) {
...
void SharedImageTestBase::VerifyPixelsWithReadbackGanesh(const Mailbox& mailbox, const std::vector<SkBitmap>& expected_bitmaps)
{
    ...
    int num_planes = format.NumberOfPlanes();
    for (int plane = 0; plane < num_planes; ++plane) {
        SkColorType plane_color_type = viz::ToClosestSkColorType(true, format, plane);
        gfx::Size plane_size = format.GetPlaneSize(plane, size);
        SkImageInfo dst_info = SkImageInfo::Make(plane_size.width(), plane_size.height(), plane_color_type, expected_bitmaps[plane].alphaType());
        SkBitmap dst_bitmap;
        ...
        auto sk_image = SkImages::BorrowTextureFrom(gr_context(), promise_texture->backendTexture(), skia_representation->surface_origin(), plane_color_type,
            skia_representation->alpha_type(), nullptr);
        ASSERT_TRUE(sk_image) << "plane_index=" << plane;

        ASSERT_TRUE(sk_image->readPixels(dst_info, dst_bitmap.getPixels(), dst_bitmap.rowBytes(), /*srcX=*/0,
            /*srcY=*/0))
            << "plane_index=" << plane;

        EXPECT_TRUE(cc::MatchesBitmap(dst_bitmap, expected_bitmaps[plane], cc::ExactPixelComparator())) << "plane_index=" << plane;
    }
    scoped_read_access->ApplyBackendSurfaceEndState();
}

void SharedImageTestBase::VerifyPixelsWithReadbackGraphite(const Mailbox& mailbox, const std::vector<SkBitmap>& expected_bitmaps)
{
    // Create Skia representation to readback from.
    auto skia_representation = shared_image_representation_factory_.ProduceSkia(mailbox, context_state_);
    ASSERT_TRUE(skia_representation);
    ...
    for (int plane = 0; plane < num_planes; ++plane) {
        ...
        auto sk_image = SkImages::WrapTexture(
            context_state_->gpu_main_graphite_recorder(), graphite_texture, plane_color_type, skia_representation->alpha_type(), nullptr);
        ASSERT_TRUE(sk_image) << "plane_index=" << plane;
        ...
        context_state_->graphite_context()->asyncRescaleAndReadPixels(
            sk_image.get(), dst_info, src_rect, SkImage::RescaleGamma::kSrc, SkImage::RescaleMode::kRepeatedLinear, &OnReadPixelsDone, &context);
        ...
        if (context.async_result) {
            CopyImagePlane(static_cast<const uint8_t*>(context.async_result->data(0)), context.async_result->rowBytes(0),
                static_cast<uint8_t*>(dst_bitmap.getPixels()), dst_bitmap.rowBytes(), dst_info.width() * dst_info.bytesPerPixel(), dst_info.height());
            EXPECT_TRUE(cc::MatchesBitmap(dst_bitmap, expected_bitmaps[plane], cc::ExactPixelComparator())) << "plane_index=" << plane;
        }
    }
}
