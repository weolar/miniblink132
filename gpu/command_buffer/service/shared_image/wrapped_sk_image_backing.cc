...
class WrappedSkImageBacking::SkiaImageRepresentationImpl : public SkiaGaneshImageRepresentation {
public:
    ...
    {
        write_surfaces_ = wrapped_sk_image()->GetSkSurfaces(final_msaa_count, surface_props, context_state_);
...
WrappedSkImageBacking::WrappedSkImageBacking(base::PassKey<WrappedSkImageBackingFactory>, const Mailbox& mailbox, viz::SharedImageFormat format,
...
{
    auto destroy_resources = [](scoped_refptr<SharedContextState> context_state, std::vector<TextureHolder> textures) {
    ...
    if (task_runner_ && !task_runner_->BelongsToCurrentThread()) {
        task_runner_->PostTask(FROM_HERE, base::BindOnce(destroy_resources, std::move(context_state_), std::move(textures_)));
    } else {
...
bool WrappedSkImageBacking::Initialize(const std::string& debug_label)
{
    ...
    auto mipmap = usage().Has(SHARED_IMAGE_USAGE_MIPMAP) ? skgpu::Mipmapped::kYes : skgpu::Mipmapped::kNo;
    ...
    for (int plane = 0; plane < num_planes; ++plane) {
        ...
        SkColor4f fallback_color = format().is_single_plane() ? SkColors::kBlue : SkColors::kWhite;
        ...
        texture.backend_texture = context_state_->gr_context()->createBackendTexture(plane_size.width(), plane_size.height(), GetSkColorType(plane),
            fallback_color, mipmap, is_renderable, is_protected, nullptr, nullptr, GetLabel(debug_label));
...
#else
        texture.backend_texture = context_state_->gr_context()->createBackendTexture(
            plane_size.width(), plane_size.height(), GetSkColorType(plane), mipmap, is_renderable, is_protected, GetLabel(debug_label));
#endif

        if (!texture.backend_texture.isValid()) {
            DLOG(ERROR) << "createBackendTexture() failed with SkColorType:" << GetSkColorType(plane);
            return false;
        }

        texture.promise_texture = GrPromiseImageTexture::Make(texture.backend_texture);
    }

    return true;
}

bool WrappedSkImageBacking::InitializeWithData(const std::string& debug_label, base::span<const uint8_t> pixels)
{
...

    {
        ...
        if (format().IsCompressed()) {
            textures_[0].backend_texture = context_state_->gr_context()->createCompressedBackendTexture(
                size().width(), size().height(), SkTextureCompressionType::kETC1_RGB8, pixels.data(), pixels.size(), skgpu::Mipmapped::kNo, GrProtected::kNo);
        } else {
            ...
            textures_[0].backend_texture
                = context_state_->gr_context()->createBackendTexture(pixmap, GrRenderable::kYes, GrProtected::kNo, nullptr, nullptr, GetLabel(debug_label));
        }
    }
    ...
    textures_[0].promise_texture = GrPromiseImageTexture::Make(textures_[0].backend_texture);
...
bool WrappedSkImageBacking::UploadFromMemory(const std::vector<SkPixmap>& pixmaps)
{
    ...
    for (size_t i = 0; i < textures_.size(); ++i) {
        updated = updated
            && context_state_->gr_context()->updateBackendTexture(textures_[i].backend_texture, &pixmaps[i],
...
std::vector<sk_sp<GrPromiseImageTexture>> WrappedSkImageBacking::GetPromiseTextures()
{
...
{
    return viz::ToClosestSkColorType(/*gpu_compositing=*/true, format(), plane_index);
}

std::vector<sk_sp<SkSurface>> WrappedSkImageBacking::GetSkSurfaces(
    int final_msaa_count, const SkSurfaceProps& surface_props, scoped_refptr<SharedContextState> context_state)
{
    if (context_state->context_lost()) {
        return {};
    }
    ...
    if (context_state == context_state_) {
        for (int plane = 0; plane < format().NumberOfPlanes(); ++plane) {
            ...
            DCHECK(texture.promise_texture);
            auto surface = context_state_->GetCachedSkSurface(texture.promise_texture.get());
            if (!surface || final_msaa_count != surface_msaa_count_ || surface_props != surface->props()) {
                surface = SkSurfaces::WrapBackendTexture(context_state_->gr_context(), texture.backend_texture, surface_origin(), final_msaa_count,
                    GetSkColorType(plane), color_space().ToSkColorSpace(), &surface_props);
                if (!surface) {
        ...
        for (int plane = 0; plane < format().NumberOfPlanes(); ++plane) {
            auto surface = SkSurfaces::WrapBackendTexture(context_state->gr_context(), textures_[plane].backend_texture, surface_origin(), final_msaa_count,
                GetSkColorType(plane), color_space().ToSkColorSpace(), &surface_props);
            if (!surface) {
                LOG(ERROR) << "MakeFromBackendTexture() failed.";
                return {};
...
std::unique_ptr<SkiaGaneshImageRepresentation> WrappedSkImageBacking::ProduceSkiaGanesh(
...
WrappedSkImageBacking::TextureHolder::TextureHolder(TextureHolder&& other) = default;
WrappedSkImageBacking::TextureHolder& WrappedSkImageBacking::TextureHolder::operator=(TextureHolder&& other) = default;
WrappedSkImageBacking::TextureHolder::~TextureHolder() = default;
