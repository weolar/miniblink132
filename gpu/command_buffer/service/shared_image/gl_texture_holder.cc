...
{
    return gl::g_current_gl_version->IsAtLeastGLES(3, 0) || gl::g_current_gl_driver->ext.b_GL_EXT_unpack_subimage;
}
...
viz::SharedImageFormat GLTextureHolder::GetPlaneFormat(viz::SharedImageFormat format, int plane_index)
{
    ...
    case viz::SharedImageFormat::ChannelFormat::k8:
        return num_channels == 2 ? viz::SinglePlaneFormat::kRG_88 : viz::SinglePlaneFormat::kR_8;
    case viz::SharedImageFormat::ChannelFormat::k10:
    case viz::SharedImageFormat::ChannelFormat::k16:
        return num_channels == 2 ? viz::SinglePlaneFormat::kRG_1616 : viz::SinglePlaneFormat::kR_16;
...
GLTextureHolder::GLTextureHolder(viz::SharedImageFormat format, const gfx::Size& size, bool is_passthrough, gl::ProgressReporter* progress_reporter)
    : format_(format)
    , size_(size)
...
{
    return is_passthrough_ ? passthrough_texture_->service_id() : texture_->service_id();
}

void GLTextureHolder::Initialize(const GLCommonImageBackingFactory::FormatInfo& format_info, bool framebuffer_attachment_angle,
    base::span<const uint8_t> pixel_data, const std::string& debug_label)
{
    ...
    MakeTextureAndSetParameters(
        format_desc_.target, framebuffer_attachment_angle, is_passthrough_ ? &passthrough_texture_ : nullptr, is_passthrough_ ? nullptr : &texture_);
    ...
    if (is_passthrough_) {
        ...
        texture_->SetLevelInfo(format_desc_.target, 0, format_desc_.data_format, size_.width(), size_.height(), /*depth=*/1, 0, format_desc_.data_format,
            format_desc_.data_type,
            /*cleared_rect=*/gfx::Rect());
        texture_->SetImmutable(true, format_info.supports_storage);
    }

    gl::GLApi* api = gl::g_current_gl_context;
    gl::ScopedRestoreTexture scoped_restore(api, format_desc_.target, GetServiceId());
    ...
    if (format_info.supports_storage) {
        {
            ...
            if (gl::g_current_gl_driver->ext.b_GL_ANGLE_renderability_validation) {
                api->glTexParameteriFn(format_desc_.target, GL_RENDERABILITY_VALIDATION_ANGLE, GL_FALSE);
            }
#endif
            gl::ScopedProgressReporter scoped_progress_reporter(progress_reporter_);
            api->glTexStorage2DEXTFn(format_desc_.target, /*levels=*/1, format_info.adjusted_storage_internal_format, size_.width(), size_.height());
        }
        ...
        if (!pixel_data.empty()) {
            ...
            api->glTexSubImage2DFn(format_desc_.target, /*level=*/0, /*xoffset=*/0,
                /*yoffset=*/0, size_.width(), size_.height(), format_info.adjusted_format, format_desc_.data_type, pixel_data.data());
        }
    } else if (format_info.is_compressed) {
        ScopedUnpackState scoped_unpack_state(!pixel_data.empty());
        gl::ScopedProgressReporter scoped_progress_reporter(progress_reporter_);
        api->glCompressedTexImage2DFn(
            format_desc_.target, 0, format_desc_.image_internal_format, size_.width(), size_.height(), /*border=*/0, pixel_data.size(), pixel_data.data());
    } else {
        ScopedUnpackState scoped_unpack_state(!pixel_data.empty());
        gl::ScopedProgressReporter scoped_progress_reporter(progress_reporter_);
        api->glTexImage2DFn(format_desc_.target, /*level=*/0, format_desc_.image_internal_format, size_.width(), size_.height(), /*border=*/0,
...
void GLTextureHolder::InitializeWithTexture(const GLFormatDesc& format_desc, scoped_refptr<gles2::TexturePassthrough> texture)
{
    DCHECK(!texture_ && !passthrough_texture_);
...
void GLTextureHolder::InitializeWithTexture(const GLFormatDesc& format_desc, gles2::Texture* texture)
{
    DCHECK(!texture_ && !passthrough_texture_);
    DCHECK(!is_passthrough_);
...
bool GLTextureHolder::UploadFromMemory(const SkPixmap& pixmap)
{
    ...
    const int gl_unpack_alignment = ComputeBestAlignment(src_bytes_per_pixel, src_stride);
    DCHECK_EQ(src_stride % gl_unpack_alignment, 0u);

    std::vector<uint8_t> repacked_data;
    if (format_ == viz::SinglePlaneFormat::kBGRX_8888 || format_ == viz::SinglePlaneFormat::kRGBX_8888) {
        ...
        // pixel.
        repacked_data = RepackPixelDataAsRgb(size_, pixmap, format_ == viz::SinglePlaneFormat::kBGRX_8888);
        src_stride = base::bits::AlignUp<size_t>(size_.width() * 3, gl_unpack_alignment);
    ...
    bool result = gles2::GLES2Util::ComputeImageDataSizes(
        size_.width(), size_.height(), /*depth=*/1, gl_format, gl_type, gl_unpack_alignment, &expected_total_bytes, nullptr, &expected_stride);
    DCHECK(result);
    ...
    const void* pixels = !repacked_data.empty() ? repacked_data.data() : pixmap.addr();
    gl::GLApi* api = gl::g_current_gl_context;
    {
        gl::ScopedProgressReporter scoped_progress_reporter(progress_reporter_);
        api->glTexSubImage2DFn(gl_target, /*level=*/0, 0, 0, size_.width(), size_.height(), gl_format, gl_type, pixels);
    }
...
bool GLTextureHolder::ReadbackToMemory(const SkPixmap& pixmap)
{
    ...
    if (format_ == viz::SinglePlaneFormat::kBGRX_8888 || format_ == viz::SinglePlaneFormat::kRGBX_8888) {
        ...
        // temporary buffer.
        gl_format = format_ == viz::SinglePlaneFormat::kBGRX_8888 ? GL_BGRA_EXT : GL_RGBA;
    }
    ...
    // GLES2 compatibility.
    api->glFramebufferTexture2DEXTFn(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, texture_id, /*level=*/0);
    DCHECK_EQ(api->glCheckFramebufferStatusEXTFn(GL_FRAMEBUFFER), static_cast<GLenum>(GL_FRAMEBUFFER_COMPLETE));
    ...
    if (gl_format != GL_RGBA || gl_type != GL_UNSIGNED_BYTE) {
        ...
        api->glGetIntegervFn(GL_IMPLEMENTATION_COLOR_READ_FORMAT, &preferred_format);
        GLint preferred_type = 0;
        api->glGetIntegervFn(GL_IMPLEMENTATION_COLOR_READ_TYPE, &preferred_type);

        if (gl_format != static_cast<GLenum>(preferred_format) || gl_type != static_cast<GLenum>(preferred_type)) {
            if (format_ == viz::SinglePlaneFormat::kBGRA_8888 || format_ == viz::SinglePlaneFormat::kBGRX_8888) {
                ...
                DLOG(ERROR) << format_.ToString() << " is not supported by glReadPixels()";
                return false;
            }
        }
    }
    ...
    const int gl_pack_alignment = ComputeBestAlignment(dst_bytes_per_pixel, dst_stride);
    ...
    uint32_t expected_stride = 0;
    bool result = gles2::GLES2Util::ComputeImageDataSizes(
        size_.width(), size_.height(), /*depth=*/1, gl_format, gl_type, gl_pack_alignment, &expected_total_bytes, nullptr, &expected_stride);
    DCHECK(result);
    ...
    void* pixels = !unpack_buffer.empty() ? unpack_buffer.data() : pixmap.writable_addr();
    {
        gl::ScopedProgressReporter scoped_progress_reporter(progress_reporter_);
        api->glReadPixelsFn(0, 0, size_.width(), size_.height(), gl_format, gl_type, pixels);
    }
...
sk_sp<GrPromiseImageTexture> GLTextureHolder::GetPromiseImage(SharedContextState* context_state)
{
    GrBackendTexture backend_texture;
    GetGrBackendTexture(context_state->feature_info(), format_desc_.target, size_, GetServiceId(), format_desc_.storage_internal_format,
