...
DawnOzoneImageRepresentation::DawnOzoneImageRepresentation(SharedImageManager* manager, SharedImageBacking* backing, MemoryTypeTracker* tracker,
    wgpu::Device device, wgpu::TextureFormat format, std::vector<wgpu::TextureFormat> view_formats, scoped_refptr<gfx::NativePixmap> pixmap)
...
wgpu::Texture DawnOzoneImageRepresentation::BeginAccess(wgpu::TextureUsage usage, wgpu::TextureUsage internal_usage)
{
    ...
    DCHECK(pixmap_->SupportsZeroCopyWebGPUImport() || pixmap_->GetNumberOfPlanes() == 1) << "Disjoint Multi-plane importing is not supported.";

    std::vector<gfx::GpuFenceHandle> fences;
    bool need_end_fence;
    is_readonly_ = (usage & kWriteUsage) == 0 && (internal_usage & kWriteUsage) == 0;
    ...
    if (!ozone_backing()->BeginAccess(
            /*readonly=*/is_readonly_, OzoneImageBacking::AccessStream::kWebGPU, &fences, need_end_fence)) {
        return nullptr;
    }
    ...
    texture_descriptor.size = { static_cast<uint32_t>(pixmap_size.width()), static_cast<uint32_t>(pixmap_size.height()), 1 };
    ...
    if (!texture_) {
        ozone_backing()->EndAccess(is_readonly_, OzoneImageBacking::AccessStream::kWebGPU, gfx::GpuFenceHandle());
        close(fd);
    }
...
void DawnOzoneImageRepresentation::EndAccess()
{
    ...
    if (!dawn::native::vulkan::ExportVulkanImage(texture_.Get(), VK_IMAGE_LAYOUT_UNDEFINED, &export_info)) {
        ...
        ozone_backing()->EndAccess(is_readonly_, OzoneImageBacking::AccessStream::kWebGPU, std::move(fence));
    }
    texture_.Destroy();
    texture_ = nullptr;
}
