...
public:
    static std::unique_ptr<ExternalVkImageBacking> Create(scoped_refptr<SharedContextState> context_state, VulkanCommandPool* command_pool,
    ...
    static std::unique_ptr<ExternalVkImageBacking> CreateFromGMB(scoped_refptr<SharedContextState> context_state, VulkanCommandPool* command_pool,
    ...
    static std::unique_ptr<ExternalVkImageBacking> CreateWithPixmap(scoped_refptr<SharedContextState> context_state, VulkanCommandPool* command_pool,
        const Mailbox& mailbox, viz::SharedImageFormat format, SurfaceHandle surface_handle, const gfx::Size& size, const gfx::ColorSpace& color_space,
        GrSurfaceOrigin surface_origin, SkAlphaType alpha_type, SharedImageUsageSet usage, std::string debug_label, gfx::BufferUsage buffer_usage);

    ExternalVkImageBacking(base::PassKey<ExternalVkImageBacking>, const Mailbox& mailbox, viz::SharedImageFormat format, const gfx::Size& size,
        ...
        size_t estimated_size_bytes, scoped_refptr<SharedContextState> context_state, std::vector<TextureHolderVk> vk_textures, VulkanCommandPool* command_pool,
        bool use_separate_gl_texture, gfx::GpuMemoryBufferHandle handle = gfx::GpuMemoryBufferHandle(),
        std::optional<gfx::BufferUsage> buffer_usage = std::nullopt);
    ...
    SharedContextState* context_state() const
    {
    ...
    {
        return vk_textures_[0].vulkan_image.get();
    }
    ...
    {
        return use_separate_gl_texture_;
    }
    bool need_synchronization() const
    {
        if (usage().HasAny(SHARED_IMAGE_USAGE_WEBGPU_READ | SHARED_IMAGE_USAGE_WEBGPU_WRITE)) {
            return true;
        }
        ...
        if (usage().HasAny(SHARED_IMAGE_USAGE_RASTER_READ | SHARED_IMAGE_USAGE_RASTER_WRITE) && usage().Has(SHARED_IMAGE_USAGE_SCANOUT)) {
            return true;
        }

        return false;
    }
    uint32_t reads_in_progress() const
    {
        return reads_in_progress_;
    }
    uint32_t gl_reads_in_progress() const
    {
        return gl_reads_in_progress_;
    }
    ...
    bool BeginAccess(bool readonly, std::vector<ExternalSemaphore>* external_semaphores, bool is_gl);
    ...
    // access.
    void EndAccess(bool readonly, ExternalSemaphore external_semaphore, bool is_gl);
    ...
    void AddSemaphoresToPendingListOrRelease(std::vector<ExternalSemaphore> semaphores);
    // Release semaphores immediately.
    void ReleaseSemaphoresWithFenceHelper(std::vector<ExternalSemaphore> semaphores);
    // Return |pending_semaphores_| and passed in |semaphores| to
    // ExternalSemaphorePool for reusing.
    void ReturnPendingSemaphoresWithFenceHelper(std::vector<ExternalSemaphore> semaphores);

protected:
    void UpdateContent(uint32_t content_flags);
    bool BeginAccessInternal(bool readonly, std::vector<ExternalSemaphore>* external_semaphores);
    void EndAccessInternal(bool readonly, ExternalSemaphore external_semaphore);

    // SharedImageBacking implementation.
    std::unique_ptr<DawnImageRepresentation> ProduceDawn(SharedImageManager* manager, MemoryTypeTracker* tracker, const wgpu::Device& dawnDevice,
