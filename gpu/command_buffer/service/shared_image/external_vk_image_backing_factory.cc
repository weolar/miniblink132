...
BASE_FEATURE(kRestrictExternalVkImageBackingScanoutSupportToFuchsia, "RestrictExternalVkImageBackingScanoutSupportToFuchsia", base::FEATURE_ENABLED_BY_DEFAULT);
...
bool RestrictScanoutSupportToFuchsia()
{
    ...
    return base::FeatureList::IsEnabled(kRestrictExternalVkImageBackingScanoutSupportToFuchsia);
}
...
VkImageUsageFlags GetMaximalImageUsageFlags(VkFormatFeatureFlags feature_flags)
{
    ...
    if (feature_flags & VK_FORMAT_FEATURE_COLOR_ATTACHMENT_BIT)
        usage_flags |= VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_INPUT_ATTACHMENT_BIT;
    if (feature_flags & VK_FORMAT_FEATURE_DEPTH_STENCIL_ATTACHMENT_BIT)
        usage_flags |= VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT | VK_IMAGE_USAGE_INPUT_ATTACHMENT_BIT;
...
base::flat_map<VkFormat, VkImageUsageFlags> CreateImageUsageCache(VkPhysicalDevice vk_physical_device)
{
    base::flat_map<VkFormat, VkImageUsageFlags> image_usage_cache;

    auto add_to_cache_if_supported = [&image_usage_cache, &vk_physical_device](viz::SharedImageFormat format) {
        ...
        vkGetPhysicalDeviceFormatProperties(vk_physical_device, vk_format, &format_props);
        image_usage_cache[vk_format] = GetMaximalImageUsageFlags(format_props.optimalTilingFeatures);
    };
...
bool IsFormatSupported(viz::SharedImageFormat format, gfx::GpuMemoryBufferType gmb_type, gpu::SharedImageUsageSet usage)
{
    ...
    if (format == viz::SinglePlaneFormat::kALPHA_8 || format == viz::SinglePlaneFormat::kLUMINANCE_8) {
        return false;
    }
...
SharedImageUsageSet SupportedUsage()
{
    SharedImageUsageSet supported_usage =
        ...
        SHARED_IMAGE_USAGE_WEBGPU_READ | SHARED_IMAGE_USAGE_WEBGPU_WRITE | SHARED_IMAGE_USAGE_WEBGPU_SWAP_CHAIN_TEXTURE
        | SHARED_IMAGE_USAGE_WEBGPU_STORAGE_TEXTURE |
#endif
        SHARED_IMAGE_USAGE_GLES2_READ | SHARED_IMAGE_USAGE_GLES2_WRITE | SHARED_IMAGE_USAGE_GLES2_FOR_RASTER_ONLY | SHARED_IMAGE_USAGE_DISPLAY_WRITE
        | SHARED_IMAGE_USAGE_DISPLAY_READ | SHARED_IMAGE_USAGE_RASTER_READ | SHARED_IMAGE_USAGE_RASTER_WRITE | SHARED_IMAGE_USAGE_RASTER_OVER_GLES2_ONLY
        | SHARED_IMAGE_USAGE_OOP_RASTERIZATION | SHARED_IMAGE_USAGE_VIDEO_DECODE | SHARED_IMAGE_USAGE_HIGH_PERFORMANCE_GPU | SHARED_IMAGE_USAGE_CPU_UPLOAD
...
ExternalVkImageBackingFactory::ExternalVkImageBackingFactory(scoped_refptr<SharedContextState> context_state)
    : SharedImageBackingFactory(SupportedUsage())
    , context_state_(std::move(context_state))
    , command_pool_(context_state_->vk_context_provider()->GetDeviceQueue()->CreateCommandPool())
    , image_usage_cache_(CreateImageUsageCache(context_state_->vk_context_provider()->GetDeviceQueue()->GetVulkanPhysicalDevice()))
{
}
...
{
    if (command_pool_) {
        context_state_->vk_context_provider()->GetDeviceQueue()->GetFenceHelper()->EnqueueVulkanObjectCleanupForSubmittedWork(std::move(command_pool_));
    }
}

std::unique_ptr<SharedImageBacking> ExternalVkImageBackingFactory::CreateSharedImage(const Mailbox& mailbox, viz::SharedImageFormat format,
    ...
    SharedImageUsageSet usage, std::string debug_label, bool is_thread_safe)
{
    ...
    return ExternalVkImageBacking::Create(context_state_, command_pool_.get(), mailbox, format, size, color_space, surface_origin, alpha_type,
        SharedImageUsageSet(usage), std::move(debug_label), image_usage_cache_, base::span<const uint8_t>());
}
    ...
    std::string debug_label, bool is_thread_safe, base::span<const uint8_t> pixel_data)
{
    CHECK(!is_thread_safe);
    return ExternalVkImageBacking::Create(context_state_, command_pool_.get(), mailbox, format, size, color_space, surface_origin, alpha_type,
        SharedImageUsageSet(usage), std::move(debug_label), image_usage_cache_, pixel_data);
}
    ...
    std::string debug_label, gfx::GpuMemoryBufferHandle handle)
{
    ...
    return ExternalVkImageBacking::CreateFromGMB(
        context_state_, command_pool_.get(), mailbox, std::move(handle), format, size, color_space, surface_origin, alpha_type, usage, std::move(debug_label));
}
    ...
    SharedImageUsageSet usage, std::string debug_label, bool is_thread_safe, gfx::BufferUsage buffer_usage)
{
    ...
    return ExternalVkImageBacking::CreateWithPixmap(context_state_, command_pool_.get(), mailbox, format, surface_handle, size, color_space, surface_origin,
...
bool ExternalVkImageBackingFactory::CanImportGpuMemoryBuffer(gfx::GpuMemoryBufferType memory_buffer_type)
{
...
bool ExternalVkImageBackingFactory::IsSupported(SharedImageUsageSet usage, viz::SharedImageFormat format, const gfx::Size& size, bool thread_safe,
