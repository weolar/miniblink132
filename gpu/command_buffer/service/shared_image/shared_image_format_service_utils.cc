...
VkFormat ToVkFormatSinglePlanarInternal(viz::SharedImageFormat format)
{
    ...
    } else if (format == viz::SinglePlaneFormat::kLUMINANCE_F16 || format == viz::SinglePlaneFormat::kR_F16) {
...
GLenum GLDataFormat(viz::SharedImageFormat format, int plane_index)
{
    ...
    if (format.is_single_plane()) {
        if (format == viz::SinglePlaneFormat::kRGBA_8888 || format == viz::SinglePlaneFormat::kRGBA_4444 || format == viz::SinglePlaneFormat::kRGBA_F16
            || format == viz::SinglePlaneFormat::kRGBA_1010102 || format == viz::SinglePlaneFormat::kBGRA_1010102) {
            return GL_RGBA;
        } else if (format == viz::SinglePlaneFormat::kBGRA_8888) {
            return GL_BGRA_EXT;
        } else if (format == viz::SinglePlaneFormat::kALPHA_8) {
            return GL_ALPHA;
        } else if (format == viz::SinglePlaneFormat::kLUMINANCE_8 || format == viz::SinglePlaneFormat::kLUMINANCE_F16) {
            return GL_LUMINANCE;
        } else if (format == viz::SinglePlaneFormat::kRGB_565 || format == viz::SinglePlaneFormat::kBGR_565 || format == viz::SinglePlaneFormat::kETC1
            || format == viz::SinglePlaneFormat::kRGBX_8888 || format == viz::SinglePlaneFormat::kBGRX_8888) {
            return GL_RGB;
        } else if (format == viz::SinglePlaneFormat::kR_8 || format == viz::SinglePlaneFormat::kR_16 || format == viz::SinglePlaneFormat::kR_F16) {
            return GL_RED_EXT;
        } else if (format == viz::SinglePlaneFormat::kRG_88 || format == viz::SinglePlaneFormat::kRG_1616) {
            return GL_RG_EXT;
        }
...
{
    if (format.is_single_plane()) {
        if (format == viz::SinglePlaneFormat::kRGBA_8888 || format == viz::SinglePlaneFormat::kBGRA_8888 || format == viz::SinglePlaneFormat::kALPHA_8
            || format == viz::SinglePlaneFormat::kLUMINANCE_8 || format == viz::SinglePlaneFormat::kETC1 || format == viz::SinglePlaneFormat::kR_8
            || format == viz::SinglePlaneFormat::kRG_88 || format == viz::SinglePlaneFormat::kRGBX_8888 || format == viz::SinglePlaneFormat::kBGRX_8888) {
            return GL_UNSIGNED_BYTE;
        } else if (format == viz::SinglePlaneFormat::kRGBA_4444) {
            return GL_UNSIGNED_SHORT_4_4_4_4;
        } else if (format == viz::SinglePlaneFormat::kBGR_565 || format == viz::SinglePlaneFormat::kRGB_565) {
            return GL_UNSIGNED_SHORT_5_6_5;
        } else if (format == viz::SinglePlaneFormat::kLUMINANCE_F16 || format == viz::SinglePlaneFormat::kR_F16
            || format == viz::SinglePlaneFormat::kRGBA_F16) {
            return GL_HALF_FLOAT_OES;
        } else if (format == viz::SinglePlaneFormat::kR_16 || format == viz::SinglePlaneFormat::kRG_1616) {
            return GL_UNSIGNED_SHORT;
        } else if (format == viz::SinglePlaneFormat::kRGBA_1010102 || format == viz::SinglePlaneFormat::kBGRA_1010102) {
...
public:
    ...
    static GLenum TextureStorageFormat(viz::SharedImageFormat format, int plane_index, bool use_angle_rgbx_format)
    {
        DCHECK(format.IsValidPlaneIndex(plane_index));
        if (format.is_single_plane()) {
            return viz::SharedImageFormatRestrictedSinglePlaneUtils::ToGLTextureStorageFormat(format, use_angle_rgbx_format);
        }
...
public:
    ...
    {
        return viz::SharedImageFormatToBufferFormatRestrictedUtils::ToBufferFormat(format);
    }
};

gfx::BufferFormat ToBufferFormat(viz::SharedImageFormat format)
{
    return SharedImageFormatToBufferFormatRestrictedUtilsAccessor::ToBufferFormat(format);
}
...
GLFormatCaps::GLFormatCaps(const gles2::FeatureInfo* feature_info)
    : angle_rgbx_internal_format_(feature_info->feature_flags().angle_rgbx_internal_format)
    ...
    , disable_r8_shared_images_(feature_info->workarounds().r8_egl_images_broken)
    , enable_texture_half_float_linear_(feature_info->feature_flags().enable_texture_half_float_linear)
    , is_atleast_gles3_(feature_info->gl_version_info().IsAtLeastGLES(3, 0))
{
}

GLFormatDesc GLFormatCaps::ToGLFormatDescExternalSampler(viz::SharedImageFormat format) const
{
    CHECK(format.PrefersExternalSampler());
    ...
    switch (format.channel_format()) {
    case ChannelFormat::k8:
        gl_format.storage_internal_format = format.HasAlpha() ? GL_RGBA8_OES : GL_RGB8_OES;
        break;
    case ChannelFormat::k10:
...
GLFormatDesc GLFormatCaps::ToGLFormatDesc(viz::SharedImageFormat format, int plane_index) const
{
    GLFormatDesc gl_format;
    ...
    gl_format.storage_internal_format = SharedImageFormatRestrictedUtilsAccessor::TextureStorageFormat(format, plane_index, angle_rgbx_internal_format_);
    if (format.is_multi_plane()) {
        gl_format.data_format = GetFallbackFormatIfNotSupported(gl_format.data_format);
        gl_format.image_internal_format = GetFallbackFormatIfNotSupported(gl_format.image_internal_format);
        gl_format.storage_internal_format = GetFallbackFormatIfNotSupported(gl_format.storage_internal_format);
    }
    gl_format.target = GL_TEXTURE_2D;
    return gl_format;
}

GLFormatDesc GLFormatCaps::ToGLFormatDescOverrideHalfFloatType(viz::SharedImageFormat format, int plane_index) const
{
    ...
    // interchangeably.
    if (format_desc.data_type == GL_HALF_FLOAT_OES && !oes_texture_float_available_) {
        format_desc.data_type = GL_HALF_FLOAT;
    }
    // ES3 requires using sized internal format for GL_HALF_FLOAT.
    if (format_desc.image_internal_format == GL_RGBA && format_desc.data_format == GL_RGBA && format_desc.data_type == GL_HALF_FLOAT) {
...
GLenum GLFormatCaps::GetFallbackFormatIfNotSupported(GLenum gl_format) const
{
    ...
    if (gl_format == GL_RED_EXT && (disable_r8_shared_images_ || !ext_texture_rg_)) {
        return GL_ALPHA;
    }
    // Fallback to GL_ALPHA8 for sized R8 format.
    if (gl_format == GL_R8_EXT && (disable_r8_shared_images_ || !ext_texture_rg_)) {
    ...
    if ((gl_format == GL_R16_EXT || gl_format == GL_RG16_EXT) && !ext_texture_norm16_) {
        return GL_ZERO;
    }
    // Fallback to GL_LUMINANCE16F for R16F format based on extensions and ES3
    // support.
    if (gl_format == GL_R16F_EXT && (!is_atleast_gles3_ || !enable_texture_half_float_linear_)) {
...
VkFormat ToVkFormatExternalSampler(viz::SharedImageFormat format)
{
    ...
    switch (format.channel_format()) {
    case ChannelFormat::k8:
        return format.plane_config() == PlaneConfig::kY_UV ? VK_FORMAT_G8_B8R8_2PLANE_420_UNORM : VK_FORMAT_G8_B8_R8_3PLANE_420_UNORM;
    case ChannelFormat::k10:
        return format.plane_config() == PlaneConfig::kY_UV ? VK_FORMAT_G10X6_B10X6R10X6_2PLANE_420_UNORM_3PACK16
                                                           : VK_FORMAT_G10X6_B10X6_R10X6_3PLANE_420_UNORM_3PACK16;
    case ChannelFormat::k16:
        return format.plane_config() == PlaneConfig::kY_UV ? VK_FORMAT_G16_B16R16_2PLANE_420_UNORM : VK_FORMAT_G16_B16_R16_3PLANE_420_UNORM;
    case ChannelFormat::k16F:
        return VK_FORMAT_UNDEFINED;
    }
}
...
VkFormat ToVkFormatSinglePlanar(viz::SharedImageFormat format)
{
    ...
    DCHECK_NE(result, VK_FORMAT_UNDEFINED) << "Unsupported format " << format.ToString();
    return result;
}
...
{
    if (format == viz::SinglePlaneFormat::kRGBA_8888 || format == viz::SinglePlaneFormat::kRGBX_8888) {
        return wgpu::TextureFormat::RGBA8Unorm;
    } else if (format == viz::SinglePlaneFormat::kBGRA_8888 || format == viz::SinglePlaneFormat::kBGRX_8888) {
        return wgpu::TextureFormat::BGRA8Unorm;
    } else if (format == viz::SinglePlaneFormat::kR_8 || format == viz::SinglePlaneFormat::kALPHA_8 || format == viz::SinglePlaneFormat::kLUMINANCE_8) {
    ...
    static crash_reporter::CrashKeyString<256> crash_key("SIFServiceUtils ToDawnFormat error");
    crash_reporter::ScopedCrashKeyString crash_key_scope(&crash_key, format.ToString());
    NOTREACHED() << "Unsupported format: " << format.ToString();
}

wgpu::TextureFormat ToDawnTextureViewFormat(viz::SharedImageFormat format, int plane_index)
{
    ...
    if (format.is_multi_plane()) {
        ...
        case viz::SharedImageFormat::ChannelFormat::k8:
            return num_channels == 1 ? wgpu::TextureFormat::R8Unorm : wgpu::TextureFormat::RG8Unorm;
        case viz::SharedImageFormat::ChannelFormat::k10:
        case viz::SharedImageFormat::ChannelFormat::k16:
            return num_channels == 1 ? wgpu::TextureFormat::R16Unorm : wgpu::TextureFormat::RG16Unorm;
...
wgpu::TextureUsage SupportedDawnTextureUsage(
    viz::SharedImageFormat format, bool is_yuv_plane, bool is_dcomp_surface, bool supports_multiplanar_rendering, bool supports_multiplanar_copy)
{
    ...
    if (is_dcomp_surface) {
        ...
        return usage | wgpu::TextureUsage::RenderAttachment | wgpu::TextureUsage::CopySrc | wgpu::TextureUsage::CopyDst;
    }
...
skgpu::graphite::TextureInfo GraphiteBackendTextureInfo(GrContextType gr_context_type, viz::SharedImageFormat format, bool readonly, int plane_index,
    bool is_yuv_plane, bool mipmapped, bool scanout_dcomp_surface, bool supports_multiplanar_rendering, bool supports_multiplanar_copy)
{
    if (gr_context_type == GrContextType::kGraphiteMetal) {
#if BUILDFLAG(SKIA_USE_METAL)
        return GraphiteMetalTextureInfo(format, plane_index, is_yuv_plane, mipmapped);
#else
        NOTREACHED();
#endif
    } else {
...
#if BUILDFLAG(SKIA_USE_DAWN)
        return skgpu::graphite::TextureInfos::MakeDawn(DawnBackendTextureInfo(format, readonly, is_yuv_plane, plane_index,
            /*array_slice=*/0, mipmapped, scanout_dcomp_surface, supports_multiplanar_rendering, supports_multiplanar_copy));
#else
        NOTREACHED();
#endif
    }
...
skgpu::graphite::TextureInfo GraphitePromiseTextureInfo(
    GrContextType gr_context_type, viz::SharedImageFormat format, std::optional<VulkanYCbCrInfo> ycbcr_info, int plane_index, bool mipmapped)
{
    if (gr_context_type == GrContextType::kGraphiteMetal) {
#if BUILDFLAG(SKIA_USE_METAL)
        ...
        dawn_texture_info.fFormat = format.is_multi_plane() ? wgpu::TextureFormat::Undefined : wgpu_view_format;
        ...
        dawn_texture_info.fMipmapped = mipmapped ? skgpu::Mipmapped::kYes : skgpu::Mipmapped::kNo;

#if BUILDFLAG(ENABLE_VULKAN)
        if (ycbcr_info) {
            // Populate the YCbCr info of the DawnTextureInfo from the Chromium info.
            dawn_texture_info.fYcbcrVkDescriptor = ToDawnYCbCrVkDescriptor(ycbcr_info.value());
        }
#endif
...
wgpu::YCbCrVkDescriptor ToDawnYCbCrVkDescriptor(const VulkanYCbCrInfo& ycbcr_info)
{
    ...
    ycbcr_desc.vkChromaFilter = ycbcr_info.format_features & VK_FORMAT_FEATURE_SAMPLED_IMAGE_YCBCR_CONVERSION_LINEAR_FILTER_BIT ? wgpu::FilterMode::Linear
...
#if BUILDFLAG(SKIA_USE_DAWN)
skgpu::graphite::DawnTextureInfo DawnBackendTextureInfo(viz::SharedImageFormat format, bool readonly, bool is_yuv_plane, int plane_index, int array_slice,
    bool mipmapped, bool scanout_dcomp_surface, bool supports_multiplanar_rendering, bool supports_multiplanar_copy)
{
    skgpu::graphite::DawnTextureInfo dawn_texture_info;
    wgpu::TextureFormat wgpu_view_format = ToDawnTextureViewFormat(format, plane_index);
    if (wgpu_view_format == wgpu::TextureFormat::Undefined) {
        return dawn_texture_info;
    }
    dawn_texture_info.fSampleCount = 1;
    dawn_texture_info.fFormat = is_yuv_plane ? ToDawnFormat(format) : wgpu_view_format;
    dawn_texture_info.fViewFormat = wgpu_view_format;
    dawn_texture_info.fAspect = ToDawnTextureAspect(is_yuv_plane, plane_index);
    dawn_texture_info.fUsage
        = SupportedDawnTextureUsage(format, is_yuv_plane, scanout_dcomp_surface, supports_multiplanar_rendering, supports_multiplanar_copy);
    if (readonly) {
        constexpr wgpu::TextureUsage kReadOnlyTextureUsage = wgpu::TextureUsage::CopySrc | wgpu::TextureUsage::TextureBinding;
...
skgpu::graphite::TextureInfo FallbackGraphiteBackendTextureInfo(const skgpu::graphite::TextureInfo& texture_info)
{
#if BUILDFLAG(SKIA_USE_DAWN)
    skgpu::graphite::DawnTextureInfo info;
    if (skgpu::graphite::TextureInfos::GetDawnTextureInfo(texture_info, &info) && info.fFormat == wgpu::TextureFormat::Undefined) {
