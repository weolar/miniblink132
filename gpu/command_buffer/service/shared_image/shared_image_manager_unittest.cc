...
std::unique_ptr<TestImageBacking> CreateImageBacking(size_t size_in_bytes)
{
    ...
    return std::make_unique<TestImageBacking>(mailbox, format, size, color_space, surface_origin, alpha_type, usage, size_in_bytes);
}
...
TEST(SharedImageManagerTest, BasicRefCounting)
{
    ...
    auto backing = std::make_unique<TestImageBacking>(mailbox, format, size, color_space, surface_origin, alpha_type, usage, kSizeBytes);
...
TEST(SharedImageManagerTest, MemoryDumps)
{
    ...
    auto factory_ref1 = manager.Register(CreateImageBacking(kSizeBytes1), tracker.get());
    auto factory_ref2 = manager.Register(CreateImageBacking(kSizeBytes2), tracker.get());

    base::trace_event::MemoryDumpArgs args = { base::trace_event::MemoryDumpLevelOfDetail::kBackground };
    ...
    for (const auto& entry : dump->entries()) {
        if (entry.name == "size") {
            ...
            EXPECT_EQ(entry.units, base::trace_event::MemoryAllocatorDump::kUnitsBytes);
...
public:
    ...
    ~SequenceValidatingMemoryTracker() override
    {
        EXPECT_EQ(size_, 0u);
    }
...
TEST(SharedImageManagerTest, TransferRefCrossThread)
{
    ...
    auto memory_type_tracker1 = std::make_unique<MemoryTypeTracker>(&memory_tracker1, memory_tracker1.task_runner());
    auto memory_type_tracker2 = std::make_unique<MemoryTypeTracker>(&memory_tracker2, memory_tracker2.task_runner());
    ...
    auto factory_ref = manager.Register(std::move(backing), memory_type_tracker1.get());
    ...
    // Take an additional ref/representation with a new tracker. Memory should
    // stay accounted to the original tracker.
    auto gl_representation = manager.ProduceGLTexture(mailbox, memory_type_tracker2.get());
    EXPECT_EQ(kSizeBytes, memory_type_tracker1->GetMemRepresented());
