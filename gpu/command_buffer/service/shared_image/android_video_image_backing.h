...
public:
    static std::unique_ptr<AndroidVideoImageBacking> Create(const Mailbox& mailbox, const gfx::Size& size, const gfx::ColorSpace color_space,
...
protected:
    AndroidVideoImageBacking(const Mailbox& mailbox, const gfx::Size& size, const gfx::ColorSpace color_space, GrSurfaceOrigin surface_origin,
        SkAlphaType alpha_type, std::string debug_label, bool is_thread_safe);

    std::unique_ptr<AbstractTextureAndroid> GenAbstractTexture(const bool passthrough);
};
