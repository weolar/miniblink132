...
public:
    ...
    void MaybeCacheSharedTextureMemory(const wgpu::Device& device, const wgpu::SharedTextureMemory& memory);

    // Returns the cached Texture for this device and texture_usage.
    wgpu::Texture GetCachedWGPUTexture(const wgpu::Device& device, wgpu::TextureUsage texture_usage);
    // Cache the `texture` for given device if it is not already cached.
    void MaybeCacheWGPUTexture(const wgpu::Device& device, const wgpu::Texture& texture);
    // Remove the `texture` for given device from cache.
    void RemoveWGPUTextureFromCache(const wgpu::Device& device, const wgpu::Texture& texture);
    // Destroys the `texture` if it is not cached for the given device.
    void DestroyWGPUTextureIfNotCached(const wgpu::Device& device, const wgpu::Texture& texture);
