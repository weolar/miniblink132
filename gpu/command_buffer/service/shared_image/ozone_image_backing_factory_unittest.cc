...
public:
    ...
    void SetUp() override
    {
        ...
        backing_factory_ = std::make_unique<OzoneImageBackingFactory>(context_state_.get(), gpu_workarounds_);
...
protected:
    ...
    {
        bool result = context_state_->MakeCurrent(gl_surface_.get(), /*needs_gl=*/true);
        DCHECK(result);

        // Check the required extensions to support egl images.
        auto* egl_display = gl::GetDefaultDisplayEGL();
        if (egl_display && egl_display->ext->b_EGL_KHR_image_base && egl_display->ext->b_EGL_KHR_gl_texture_2D_image
...
TEST_F(OzoneImageBackingFactoryTest, UsesCacheForTextureHolders)
{
    ...
    EXPECT_TRUE(context_state_->MakeCurrent(context_state_->surface(), true /* needs_gl*/));

    const Mailbox mailbox = Mailbox::Generate();
    auto backing = backing_factory_->CreateSharedImage(mailbox, viz::SinglePlaneFormat::kRGBA_8888, gpu::kNullSurfaceHandle, { 100, 100 },
        gfx::ColorSpace::CreateSRGB(), kTopLeft_GrSurfaceOrigin, kPremul_SkAlphaType, SHARED_IMAGE_USAGE_GLES2_READ, "TestLabel", false);
    EXPECT_TRUE(backing);

    auto* backing_ptr = static_cast<OzoneImageBacking*>(backing.get());

    auto shared_image = shared_image_manager_.Register(std::move(backing), &memory_type_tracker_);
    ...
    auto& cached_textures_holdes = *backing_ptr->per_context_cached_textures_holders_.begin();
    EXPECT_EQ(context_state_->context(), cached_textures_holdes.first);

    auto gl_representation2 = shared_image_representation_factory_->ProduceGLTexturePassthrough(mailbox);
    ...
    // repeat).
    EXPECT_EQ(gl_representation->GetTexturePassthrough()->service_id(), gl_representation2->GetTexturePassthrough()->service_id());

    auto gl_context = gl::init::CreateGLContext(nullptr, gl_surface_.get(), gl::GLContextAttribs());
    ASSERT_TRUE(gl_context);
    bool make_current_result = gl_context->MakeCurrent(gl_surface_.get());
    ASSERT_TRUE(make_current_result);

    auto gl_representation3 = shared_image_representation_factory_->ProduceGLTexturePassthrough(mailbox);
    ...
    auto gl_representation4 = shared_image_representation_factory_->ProduceGLTexturePassthrough(mailbox);
    ...
    EXPECT_EQ(gl_representation3->GetTexturePassthrough()->service_id(), gl_representation4->GetTexturePassthrough()->service_id());
...
TEST_F(OzoneImageBackingFactoryTest, UsesCacheForTextureHolders2)
{
    ...
    scoped_refptr<gl::GLSurface> fake_onscreen_gl_surface(new FakeOnScreenSurface(gl::GetDefaultDisplayEGL(), { 100, 100 }));
    ASSERT_FALSE(fake_onscreen_gl_surface->IsOffscreen());

    scoped_refptr<gl::GLContext> gl_context = gl::init::CreateGLContext(nullptr, fake_onscreen_gl_surface.get(), gl::GLContextAttribs());
    ASSERT_TRUE(gl_context);
    ASSERT_FALSE(gl_context->default_surface());
    bool make_current_result = gl_context->MakeCurrent(fake_onscreen_gl_surface.get());
    ASSERT_TRUE(make_current_result);
...
TEST_F(OzoneImageBackingFactoryTest, MarksContextLostOnContextLost)
{
    ...
    // reference.
    auto it = backing_ptr->per_context_cached_textures_holders_.find(gl_context.get());
    ...
    const GLuint service_id = textures_holder_ref->texture(/*plane_index=*/0)->service_id();
...
TEST_F(OzoneImageBackingFactoryTest, MarksContextLostOnContextLost2)
{
...

    {
        ...
        auto* ozone_reprensentation = static_cast<GLTexturePassthroughOzoneImageRepresentation*>(gl_representation.get());
...
TEST_F(OzoneImageBackingFactoryTest, FindsCompatibleContextAndReusesTexture)
{
    ...
    auto gl_context = gl::init::CreateGLContext(nullptr, gl_surface_.get(), attribs);
    ASSERT_TRUE(gl_context);
    bool make_current_result = gl_context->MakeCurrent(gl_surface_.get());
    ASSERT_TRUE(make_current_result);
    ...
    EXPECT_NE(gl_representation->GetTexturePassthrough(), gl_representation2->GetTexturePassthrough());

    const std::vector<std::pair<bool, gl::AngleContextVirtualizationGroup>> kTestVariations = {
    ...
    for (const auto& variation : kTestVariations) {
        ...
        auto new_context = gl::init::CreateGLContext(nullptr, gl_surface_.get(), attributes);
        ...
        if (gl_context->CanShareTexturesWithContext(new_context.get())) {
            EXPECT_EQ(representation->GetTexturePassthrough(), gl_representation2->GetTexturePassthrough());
        } else {
            EXPECT_NE(representation->GetTexturePassthrough(), gl_representation2->GetTexturePassthrough());
        }
        // And they are always different from the first context, of course.
        EXPECT_NE(gl_representation->GetTexturePassthrough(), representation->GetTexturePassthrough());
    }
}
...
TEST_F(OzoneImageBackingFactoryTest, CorrectlyDestroysAndMarksContextLost)
{
    ...
    auto new_context = gl::init::CreateGLContext(nullptr, gl_surface_.get(), attribs);
    ASSERT_TRUE(new_context);
    make_current_result = new_context->MakeCurrent(gl_surface_.get());
    ASSERT_TRUE(make_current_result);
    ...
    auto gl_representation2 = shared_image_representation_factory_->ProduceGLTexturePassthrough(mailbox);
    EXPECT_TRUE(gl_representation2);
    EXPECT_EQ(gl_representation->GetTexturePassthrough(), gl_representation2->GetTexturePassthrough());

    EXPECT_EQ(2u, backing_ptr->per_context_cached_textures_holders_.size());

    auto holder_ref1 = backing_ptr->per_context_cached_textures_holders_.begin()->second;
    auto holder_ref2 = backing_ptr->per_context_cached_textures_holders_.rbegin()->second;
