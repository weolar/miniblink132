...
// static
std::unique_ptr<DXGISwapChainImageBacking> DXGISwapChainImageBacking::Create(Microsoft::WRL::ComPtr<ID3D11Device> d3d11_device, const Mailbox& mailbox,
    viz::SharedImageFormat format, DXGI_FORMAT internal_format, const gfx::Size& size, const gfx::ColorSpace& color_space, GrSurfaceOrigin surface_origin,
    SkAlphaType alpha_type, gpu::SharedImageUsageSet usage, std::string debug_label)
{
    if (!d3d11_device) {
        return nullptr;
    }
    ...
    desc.AlphaMode = SkAlphaTypeIsOpaque(alpha_type) ? DXGI_ALPHA_MODE_IGNORE : DXGI_ALPHA_MODE_PREMULTIPLIED;
    desc.Flags = 0;
    ...
    HRESULT hr = dxgi_factory->CreateSwapChainForComposition(d3d11_device.Get(), &desc, nullptr, &dxgi_swap_chain);
    ...
    if (FAILED(hr)) {
        DLOG(ERROR) << "CreateSwapChainForComposition failed: " << logging::SystemErrorCodeToString(hr);
    ...
    gl::LabelSwapChainAndBuffers(dxgi_swap_chain.Get(), kDXGISwapChainImageBackingLabel);

    Microsoft::WRL::ComPtr<IDXGISwapChain3> swap_chain_3;
    if (SUCCEEDED(dxgi_swap_chain.As(&swap_chain_3))) {
        hr = swap_chain_3->SetColorSpace1(gfx::ColorSpaceWin::GetDXGIColorSpace(color_space));
        DCHECK_EQ(hr, S_OK) << ", SetColorSpace1 failed: " << logging::SystemErrorCodeToString(hr);
        if (gl::DXGIWaitableSwapChainEnabled()) {
            hr = swap_chain_3->SetMaximumFrameLatency(gl::GetDXGIWaitableSwapChainMaxQueuedFrames());
            DCHECK_EQ(hr, S_OK) << ", SetMaximumFrameLatency failed: " << logging::SystemErrorCodeToString(hr);
        }
    }
    ...
    int buffers_need_alpha_initialization_count = !format.HasAlpha() ? desc.BufferCount : 0;

    return base::WrapUnique(new DXGISwapChainImageBacking(mailbox, format, size, color_space, surface_origin, alpha_type, usage, std::move(debug_label),
        std::move(d3d11_device), std::move(dxgi_swap_chain), buffers_need_alpha_initialization_count));
}

DXGISwapChainImageBacking::DXGISwapChainImageBacking(const Mailbox& mailbox, viz::SharedImageFormat format, const gfx::Size& size,
    ...
    Microsoft::WRL::ComPtr<ID3D11Device> d3d11_device, Microsoft::WRL::ComPtr<IDXGISwapChain1> dxgi_swap_chain, int buffers_need_alpha_initialization_count)
    ...
    , buffers_need_alpha_initialization_count_(buffers_need_alpha_initialization_count)
{
...
void DXGISwapChainImageBacking::Update(std::unique_ptr<gfx::GpuFence> in_fence)
{
    DCHECK(!in_fence);
}

bool DXGISwapChainImageBacking::DidBeginWriteAccess(const gfx::Rect& swap_rect)
{
    ...
    if (initialize_color.has_value()) {
        ...
        if (!D3DImageBackingFactory::ClearBackBufferToColor(dxgi_swap_chain_.Get(), initialize_color.value())) {
...
bool DXGISwapChainImageBacking::Present(bool should_synchronize_present_with_vblank)
{
    ...
    const bool use_swap_chain_tearing = gl::DirectCompositionSwapChainTearingEnabled();
    const bool force_present_interval_0 = base::FeatureList::IsEnabled(features::kDXGISwapChainPresentInterval0);
    UINT interval = first_swap_ || !should_synchronize_present_with_vblank || use_swap_chain_tearing || force_present_interval_0 ? 0 : 1;
    UINT flags = use_swap_chain_tearing ? DXGI_PRESENT_ALLOW_TEARING : 0;

    TRACE_EVENT2("gpu", "IDXGISwapChain1::Present1", "has_alpha", !SkAlphaTypeIsOpaque(alpha_type()), "dirty_rect", pending_swap_rect_->ToString());
...
std::unique_ptr<OverlayImageRepresentation> DXGISwapChainImageBacking::ProduceOverlay(SharedImageManager* manager, MemoryTypeTracker* tracker)
{
    return std::make_unique<DXGISwapChainOverlayImageRepresentation>(manager, this, tracker);
}

std::unique_ptr<SkiaGaneshImageRepresentation> DXGISwapChainImageBacking::ProduceSkiaGanesh(
    SharedImageManager* manager, MemoryTypeTracker* tracker, scoped_refptr<SharedContextState> context_state)
{
    ...
    if (!gl_texture_holder_) {
        ...
        HRESULT hr = dxgi_swap_chain_->GetBuffer(0, IID_PPV_ARGS(&backbuffer_texture));
        if (FAILED(hr)) {
            DLOG(ERROR) << "GetBuffer(0) failed: " << logging::SystemErrorCodeToString(hr);
            return nullptr;
        }

        auto gl_format_desc = context_state->GetGLFormatCaps().ToGLFormatDesc(format(), /*plane_index=*/0);
        gl_texture_holder_ = D3DImageBacking::CreateGLTexture(
            gl_format_desc, size(), color_space(), backbuffer_texture, GL_TEXTURE_2D, /*array_slice=*/0, /*plane_index=*/0, dxgi_swap_chain_);
        if (!gl_texture_holder_) {
    ...
    return SkiaGLImageRepresentationDXGISwapChain::Create(
        std::make_unique<GLTexturePassthroughDXGISwapChainBufferRepresentation>(manager, this, tracker, gl_texture_holder_), std::move(context_state), manager,
        this, tracker);
}

std::unique_ptr<SkiaGraphiteImageRepresentation> DXGISwapChainImageBacking::ProduceSkiaGraphite(
    SharedImageManager* manager, MemoryTypeTracker* tracker, scoped_refptr<SharedContextState> context_state)
{
#if BUILDFLAG(SKIA_USE_DAWN)
    ...
    if (!shared_texture_memory_) {
        ...
        shared_texture_memory_ = CreateDawnSharedTextureMemory(device, backbuffer_texture);
        if (!shared_texture_memory_) {
    ...
    auto dawn_representation = std::make_unique<DawnRepresentationDXGISwapChain>(manager, this, tracker, device, wgpu::BackendType::D3D11);
...
wgpu::Texture DXGISwapChainImageBacking::BeginAccessDawn(
    const wgpu::Device& device, wgpu::TextureUsage usage, wgpu::TextureUsage internal_usage, const gfx::Rect& update_rect)
{
...
void DXGISwapChainImageBacking::EndAccessDawn(const wgpu::Device& device, wgpu::Texture texture)
{
