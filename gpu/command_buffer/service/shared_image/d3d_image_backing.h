...
class GPU_GLES2_EXPORT D3DImageBacking final : public ClearTrackingSharedImageBacking {
public:
    ...
    static std::unique_ptr<D3DImageBacking> Create(const Mailbox& mailbox, viz::SharedImageFormat format, const gfx::Size& size,
        ...
        bool use_update_subresource1 = false, bool is_thread_safe = false);

    // Creation method meant for buffer resources originating as ID3D12Resources.
    static std::unique_ptr<D3DImageBacking> CreateFromD3D12Resource(const Mailbox& mailbox, const gfx::Size& size, gpu::SharedImageUsageSet usage,
        std::string debug_label, Microsoft::WRL::ComPtr<ID3D12Resource> d3d12_resource);

    static std::unique_ptr<D3DImageBacking> CreateFromSwapChainBuffer(const Mailbox& mailbox, viz::SharedImageFormat format, const gfx::Size& size,
    ...
    void ReadbackToMemoryAsync(const std::vector<SkPixmap>& pixmaps, base::OnceCallback<void(bool)> callback) override;
    ...
    void UpdateExternalFence(scoped_refptr<gfx::D3DSharedFence> external_fence) override;

    bool BeginAccessD3D11(Microsoft::WRL::ComPtr<ID3D11Device> d3d11_device, bool write_access);
    ...
    scoped_refptr<gfx::D3DSharedFence> GetDCompTextureAvailabilityFenceForCurrentFrame() const;

    wgpu::Texture BeginAccessDawn(const wgpu::Device& device, wgpu::BackendType backend_type, wgpu::TextureUsage usage, wgpu::TextureUsage internal_usage,
    ...
    std::unique_ptr<DawnBufferRepresentation> ProduceDawnBuffer(
        SharedImageManager* manager, MemoryTypeTracker* tracker, const wgpu::Device& device, wgpu::BackendType backend_type) override;
    wgpu::Buffer BeginAccessDawnBuffer(const wgpu::Device& device, wgpu::BackendType backend_type, wgpu::BufferUsage usage);
    ...
    {
        return dxgi_shared_handle_state_ && dxgi_shared_handle_state_->has_keyed_mutex();
    }

    // Holds a gles2::TexturePassthrough and corresponding egl image.
    class GLTextureHolder : public base::RefCounted<GLTextureHolder> {
    public:
        GLTextureHolder(base::PassKey<D3DImageBacking>, scoped_refptr<gles2::TexturePassthrough> texture_passthrough, gl::ScopedEGLImage egl_image);
        ...
        void* egl_image() const
        {
            return egl_image_.get();
        }
        void set_needs_rebind(bool needs_rebind)
        {
            needs_rebind_ = needs_rebind;
        }
    ...
    static scoped_refptr<GLTextureHolder> CreateGLTexture(const GLFormatDesc& gl_format_desc, const gfx::Size& size, const gfx::ColorSpace& color_space,
    ...
    scoped_refptr<DXGISharedHandleState> dxgi_shared_handle_state_for_testing() const
    {
...
protected:
    ...
    std::unique_ptr<VideoImageRepresentation> ProduceVideo(SharedImageManager* manager, MemoryTypeTracker* tracker, VideoDevice device) override;

private:
    using D3DSharedFenceSet = base::flat_set<scoped_refptr<gfx::D3DSharedFence>>;
    D3DImageBacking(const Mailbox& mailbox, viz::SharedImageFormat format, const gfx::Size& size, const gfx::ColorSpace& color_space,
        GrSurfaceOrigin surface_origin, SkAlphaType alpha_type, gpu::SharedImageUsageSet usage, std::string debug_label,
        Microsoft::WRL::ComPtr<ID3D11Texture2D> d3d11_texture, Microsoft::WRL::ComPtr<IDCompositionTexture> dcomp_texture,
        scoped_refptr<DXGISharedHandleState> dxgi_shared_handle_state, const GLFormatCaps& gl_format_caps, GLenum texture_target = GL_TEXTURE_2D,
        size_t array_slice = 0u, Microsoft::WRL::ComPtr<IDXGISwapChain1> swap_chain = nullptr, bool is_back_buffer = false,
        bool use_update_subresource1 = false, bool is_thread_safe = false);

    D3DImageBacking(const Mailbox& mailbox, const gfx::Size& size, gpu::SharedImageUsageSet usage, std::string debug_label,
    ...
    bool ReadbackFromStagingTexture(const std::vector<SkPixmap>& pixmaps) EXCLUSIVE_LOCKS_REQUIRED(lock_);

    void OnCopyToStagingTextureDone(const std::vector<SkPixmap>& pixmaps, base::OnceCallback<void(bool)> readback_cb);

    // Common state tracking for both D3D11 and Dawn access.
    bool ValidateBeginAccess(bool write_access) const EXCLUSIVE_LOCKS_REQUIRED(lock_);
    void BeginAccessCommon(bool write_access) EXCLUSIVE_LOCKS_REQUIRED(lock_);
    void EndAccessCommon(const D3DSharedFenceSet& fences) EXCLUSIVE_LOCKS_REQUIRED(lock_);
    ...
    std::vector<scoped_refptr<gfx::D3DSharedFence>> GetPendingWaitFences(
        const Microsoft::WRL::ComPtr<ID3D11Device>& wait_d3d11_device, const wgpu::Device& wait_dawn_device, bool write_access) EXCLUSIVE_LOCKS_REQUIRED(lock_);
    ...
    wgpu::SharedTextureMemory GetSharedTextureMemory(const wgpu::Device& device) EXCLUSIVE_LOCKS_REQUIRED(lock_);

    // Returns the number of ongoing accesses that were already present on this
    // texture prior to beginning this access.
    int TrackBeginAccessToWGPUTexture(wgpu::Texture texture) EXCLUSIVE_LOCKS_REQUIRED(lock_);

    // Returns the number of ongoing accesses that will still be present on this
    // texture after ending this access.
    int TrackEndAccessToWGPUTexture(wgpu::Texture texture) EXCLUSIVE_LOCKS_REQUIRED(lock_);
    ...
    std::array<base::WeakPtr<GLTextureHolder>, 3> gl_texture_holders_ GUARDED_BY(lock_);
    ...
    base::flat_map<Microsoft::WRL::ComPtr<ID3D11Device>, scoped_refptr<gfx::D3DSharedFence>> d3d11_signaled_fence_map_ GUARDED_BY(lock_);
    ...
    base::flat_map<WGPUTexture, int> wgpu_texture_ongoing_accesses_ GUARDED_BY(lock_);
    ...
    base::flat_map<WGPUDevice, D3DSharedFenceSet> dawn_signaled_fences_map_ GUARDED_BY(lock_);

    std::optional<base::WaitableEventWatcher> pending_copy_event_watcher_ GUARDED_BY(lock_);
