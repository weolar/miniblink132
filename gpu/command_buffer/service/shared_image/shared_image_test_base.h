...
protected:
    ...
    static SkBitmap MakeRedBitmap(SkColorType color_type, const gfx::Size& size, size_t added_stride = 0);

    // Allocate a bitmap for each plane by calling MakeRedBitmap().
    static std::vector<SkBitmap> AllocateRedBitmaps(viz::SharedImageFormat format, const gfx::Size& size, size_t added_stride = 0);

    // Returns SkPixmap from each SkBitmap.
    static std::vector<SkPixmap> GetSkPixmaps(const std::vector<SkBitmap>& bitmaps);
    ...
    void VerifyPixelsWithReadback(const Mailbox& mailbox, const std::vector<SkBitmap>& expect_bitmaps);
    ...
    void VerifyPixelsWithReadbackGanesh(const Mailbox& mailbox, const std::vector<SkBitmap>& expect_bitmaps);
    ...
    void VerifyPixelsWithReadbackGraphite(const Mailbox& mailbox, const std::vector<SkBitmap>& expect_bitmaps);
    ...
    SharedImageRepresentationFactory shared_image_representation_factory_ { &shared_image_manager_, nullptr };
