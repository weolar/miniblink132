...
// usage.
constexpr SharedImageUsageSet kSupportedUsage = SHARED_IMAGE_USAGE_DISPLAY_READ | SHARED_IMAGE_USAGE_DISPLAY_WRITE | SHARED_IMAGE_USAGE_RASTER_READ
    | SHARED_IMAGE_USAGE_RASTER_WRITE | SHARED_IMAGE_USAGE_OOP_RASTERIZATION | SHARED_IMAGE_USAGE_CPU_UPLOAD | SHARED_IMAGE_USAGE_MIPMAP;
...
SharedImageUsageSet GetSupportedUsage(const SharedContextState* context_state)
{
    ...
    constexpr SharedImageUsageSet kGraphiteDawnFallbackUsage = SHARED_IMAGE_USAGE_GLES2_READ | SHARED_IMAGE_USAGE_GLES2_WRITE
        | SHARED_IMAGE_USAGE_GLES2_FOR_RASTER_ONLY |
        // NOTE: In this case, it is also possible to support raster-over-GLES2.
        SHARED_IMAGE_USAGE_RASTER_OVER_GLES2_ONLY | SHARED_IMAGE_USAGE_WEBGPU_READ | SHARED_IMAGE_USAGE_WEBGPU_WRITE
...
bool GraphiteSupportsCompressedTextures(const SharedContextState* context_state)
{
#if BUILDFLAG(SKIA_USE_DAWN)
    // TODO(b/281151641): Query graphite instead of dawn to see if compressed
    // textures are supported.
    if (context_state->IsGraphiteDawn()) {
        return context_state->dawn_context_provider()->SupportsFeature(wgpu::FeatureName::TextureCompressionETC2);
    }
#endif
    return false;
}
...
WrappedSkImageBackingFactory::WrappedSkImageBackingFactory(scoped_refptr<SharedContextState> context_state)
    ...
    , graphite_supports_compressed_textures_(GraphiteSupportsCompressedTextures(context_state_.get()))
{
}
...
{
    if (use_graphite_) {
        auto backing = std::make_unique<WrappedGraphiteTextureBacking>(base::PassKey<WrappedSkImageBackingFactory>(), mailbox, format, size, color_space,
            surface_origin, alpha_type, usage, std::move(debug_label), context_state_, is_thread_safe);
    ...
    auto backing = std::make_unique<WrappedSkImageBacking>(base::PassKey<WrappedSkImageBackingFactory>(), mailbox, format, size, color_space, surface_origin,
        alpha_type, usage, debug_label, context_state_, is_thread_safe);
...
std::unique_ptr<SharedImageBacking> WrappedSkImageBackingFactory::CreateSharedImage(const Mailbox& mailbox, viz::SharedImageFormat format,
    ...
    std::string debug_label, bool is_thread_safe, base::span<const uint8_t> data)
{
    if (use_graphite_) {
        auto backing = std::make_unique<WrappedGraphiteTextureBacking>(base::PassKey<WrappedSkImageBackingFactory>(), mailbox, format, size, color_space,
            surface_origin, alpha_type, usage, std::move(debug_label), context_state_, is_thread_safe);
...
bool WrappedSkImageBackingFactory::IsSupported(SharedImageUsageSet usage, viz::SharedImageFormat format, const gfx::Size& size, bool thread_safe,
    gfx::GpuMemoryBufferType gmb_type, GrContextType gr_context_type, base::span<const uint8_t> pixel_data)
{
    ...
    // semaphores.
    if (thread_safe) {
        bool is_vulkan = gr_context_type == GrContextType::kVulkan || context_state_->IsGraphiteDawnVulkan();
    ...
    } else if (format == viz::SinglePlaneFormat::kBGRX_8888 || format == viz::SinglePlaneFormat::kBGR_565) {
    ...
    if (format.IsCompressed()) {
        ...
        auto backend_format = context_state_->gr_context()->compressedBackendFormat(SkTextureCompressionType::kETC1_RGB8);
    ...
    if (context_state_->gr_context()) {
        ...
        for (int plane = 0; plane < format.NumberOfPlanes(); ++plane) {
            SkColorType color_type = viz::ToClosestSkColorType(/*gpu_compositing=*/true, format, plane);
            auto backend_format = context_state_->gr_context()->defaultBackendFormat(color_type, GrRenderable::kYes);
