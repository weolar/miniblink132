...
public:
    AndroidImageBacking(const Mailbox& mailbox, viz::SharedImageFormat format, const gfx::Size& size, const gfx::ColorSpace& color_space,
        ...
        bool is_thread_safe, base::ScopedFD initial_upload_fd);
    ...
    virtual bool BeginRead(const SharedImageRepresentation* reader, base::ScopedFD* fd_to_wait_on);
    virtual void EndRead(const SharedImageRepresentation* reader, base::ScopedFD end_read_fd);
...
protected:
    ...
    base::flat_set<raw_ptr<const SharedImageRepresentation, CtnExperimental>> active_readers_ GUARDED_BY(lock_);
