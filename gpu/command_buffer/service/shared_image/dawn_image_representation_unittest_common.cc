...
void CopyTexelToBuffer(const wgpu::CommandEncoder& encoder, const wgpu::Texture& texture, uint32_t x, uint32_t y, const wgpu::Buffer& buffer)
{
    wgpu::ImageCopyBuffer buffer_copy;
    buffer_copy.buffer = buffer;
    buffer_copy.layout = wgpu::TextureDataLayout { .bytesPerRow = kBufferSizeMinAlignment };
...
wgpu::RenderPipeline CreateRenderPipeline(
    const wgpu::Device& device, wgpu::ShaderModule vs_module, wgpu::ShaderModule fs_module, wgpu::TextureFormat render_pass_color_format)
{
...
wgpu::Buffer CreateBuffer(const wgpu::Device& device, uint32_t size, wgpu::BufferUsage usage)
{
...
wgpu::Texture CreateTexture(const wgpu::Device& device, uint32_t width, uint32_t height, wgpu::TextureUsage usage, wgpu::TextureFormat format)
{
...
wgpu::TextureView CreateTextureView(const wgpu::Texture& texture, wgpu::TextureAspect aspect)
{
...
void RunDawnVideoSamplingTest(wgpu::Instance instance, wgpu::Device device, const std::unique_ptr<DawnImageRepresentation>& shared_image,
    uint8_t expected_y_value, uint8_t expected_u_value, uint8_t expected_v_value)
{
...
@fragment fn main(@location(0) tex_coord : vec2f) -> @location(0) vec4f {
  ...
  return vec4f(y, uv.r, uv.g, 1.0);
    ...
    auto texture_access = shared_image->BeginScopedAccess(wgpu::TextureUsage::TextureBinding, SharedImageRepresentation::AllowUnclearedAccess::kNo);
    ...
    bind_group_entries[1].textureView = CreateTextureView(texture_access->texture(), wgpu::TextureAspect::Plane0Only);
    bind_group_entries[2].binding = 2;
    bind_group_entries[2].textureView = CreateTextureView(texture_access->texture(), wgpu::TextureAspect::Plane1Only);
    ...
    auto render_target_texture = CreateTexture(
        device, size.width(), size.height(), wgpu::TextureUsage::RenderAttachment | wgpu::TextureUsage::CopySrc, wgpu::TextureFormat::RGBA8Unorm);
    ...
    auto readback_buffer = CreateBuffer(device, kBufferSizeMinAlignment, wgpu::BufferUsage::CopyDst | wgpu::BufferUsage::MapRead);
    ...
    wgpu::FutureWaitInfo wait_info { readback_buffer.MapAsync(wgpu::MapMode::Read, 0, wgpu::kWholeMapSize, wgpu::CallbackMode::WaitAnyOnly,
    ...
    memcpy(pixel_color, readback_buffer.GetConstMappedRange(), sizeof(pixel_color));
