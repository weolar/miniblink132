...
bool InitializeDCompSurface(IDCompositionSurface* surface, const gfx::Size& surface_size)
{
    HRESULT hr = S_OK;
...
class DCompSurfaceImageBacking::D3DTextureGLSurfaceEGL : public gl::GLSurfaceEGL {
public:
    ...
    bool Initialize(gl::GLSurfaceFormat format) override
    {
        if (display_->GetDisplay() == EGL_NO_DISPLAY) {
            LOG(ERROR) << "Trying to create D3DTextureGLSurfaceEGL with invalid display.";
            return false;
        }
    ...
    {
        NOTREACHED() << "Attempted to call SwapBuffers on a D3DTextureGLSurfaceEGL.";
    }
    ...
    // surface.
    bool BindTextureToSurface(ID3D11Texture2D* texture, const gfx::Vector2d& draw_offset)
    {
        DCHECK(!surface_);
        DCHECK(texture);
        ...
        surface_ = eglCreatePbufferFromClientBuffer(display_->GetDisplay(), EGL_D3D_TEXTURE_ANGLE, buffer, GetConfig(), pbuffer_attribs.data());
        if (!surface_) {
            LOG(ERROR) << "eglCreatePbufferFromClientBuffer failed with error " << ui::GetLastEGLErrorString();
            return false;
        }
...
// static
std::unique_ptr<DCompSurfaceImageBacking> DCompSurfaceImageBacking::Create(const Mailbox& mailbox, viz::SharedImageFormat format, DXGI_FORMAT internal_format,
    ...
    std::string debug_label)
{
    ...
    DCHECK(internal_format == DXGI_FORMAT_B8G8R8A8_UNORM || internal_format == DXGI_FORMAT_R8G8B8A8_UNORM || internal_format == DXGI_FORMAT_R16G16B16A16_FLOAT)
        << "Incompatible DXGI_FORMAT = " << internal_format;

    TRACE_EVENT2("gpu", "DCompSurfaceImageBacking::Create", "width", size.width(), "height", size.height());
    ...
    HRESULT hr = gl::GetDirectCompositionDevice()->CreateSurface(
        size.width(), size.height(), internal_format, SkAlphaTypeIsOpaque(alpha_type) ? DXGI_ALPHA_MODE_IGNORE : DXGI_ALPHA_MODE_PREMULTIPLIED, &dcomp_surface);
    base::UmaHistogramSparse("GPU.DirectComposition.DcompDeviceCreateSurface", hr);
    if (FAILED(hr)) {
        DLOG(ERROR) << "CreateSurface failed: " << logging::SystemErrorCodeToString(hr);
    ...
    return base::WrapUnique(
        new DCompSurfaceImageBacking(mailbox, format, size, color_space, surface_origin, alpha_type, usage, std::move(debug_label), std::move(dcomp_surface)));
}

DCompSurfaceImageBacking::DCompSurfaceImageBacking(const Mailbox& mailbox, viz::SharedImageFormat format, const gfx::Size& size,
    ...
    , gl_surface_(scoped_refptr(new D3DTextureGLSurfaceEGL(gl::GLSurfaceEGL::GetGLDisplayEGL(), size)))
    , dcomp_surface_(std::move(dcomp_surface))
{
    const bool has_scanout = usage.Has(SHARED_IMAGE_USAGE_SCANOUT);
    const bool write_only = !usage.Has(SHARED_IMAGE_USAGE_DISPLAY_READ) && usage.Has(SHARED_IMAGE_USAGE_DISPLAY_WRITE);
    DCHECK(has_scanout);
    DCHECK(write_only);
...
std::unique_ptr<OverlayImageRepresentation> DCompSurfaceImageBacking::ProduceOverlay(SharedImageManager* manager, MemoryTypeTracker* tracker)
{
    return std::make_unique<DCompSurfaceOverlayImageRepresentation>(manager, this, tracker);
}

std::unique_ptr<SkiaGaneshImageRepresentation> DCompSurfaceImageBacking::ProduceSkiaGanesh(
    SharedImageManager* manager, MemoryTypeTracker* tracker, scoped_refptr<SharedContextState> context_state)
{
    ...
    return std::make_unique<DCompSurfaceSkiaGaneshImageRepresentation>(std::move(context_state), manager, this, tracker);
}

std::unique_ptr<SkiaGraphiteImageRepresentation> DCompSurfaceImageBacking::ProduceSkiaGraphite(
    SharedImageManager* manager, MemoryTypeTracker* tracker, scoped_refptr<SharedContextState> context_state)
{
#if BUILDFLAG(SKIA_USE_DAWN)
    ...
    auto dawn_representation = std::make_unique<DCompSurfaceDawnImageRepresentation>(manager, this, tracker, device, wgpu::BackendType::D3D11);
...
Microsoft::WRL::ComPtr<ID3D11Texture2D> DCompSurfaceImageBacking::BeginDraw(const gfx::Rect& update_rect, gfx::Point& update_offset_out)
{
    ...
    Microsoft::WRL::ComPtr<ID3D11Texture2D> draw_texture;
    POINT update_offset = {};
    HRESULT hr = dcomp_surface_->BeginDraw(&rect, IID_PPV_ARGS(&draw_texture), &update_offset);
    if (FAILED(hr)) {
        DCHECK_NE(hr, DCOMPOSITION_ERROR_SURFACE_BEING_RENDERED) << "Concurrent writes to multiple DCompSurfaceImageBacking "
                                                                    "not allowed.";
...
sk_sp<SkSurface> DCompSurfaceImageBacking::BeginDrawGanesh(
    SharedContextState* context_state, int final_msaa_count, const SkSurfaceProps& surface_props, const gfx::Rect& update_rect)
{
    ...
    auto render_target = GrBackendRenderTargets::MakeGL(size().width(), size().height(), final_msaa_count, 0, framebuffer_info);
    auto surface = SkSurfaces::WrapBackendRenderTarget(
        context_state->gr_context(), render_target, surface_origin(), color_type, color_space().ToSkColorSpace(), &surface_props);
    DCHECK(surface);
...
wgpu::Texture DCompSurfaceImageBacking::BeginDrawDawn(
    const wgpu::Device& device, const wgpu::TextureUsage usage, const wgpu::TextureUsage internal_usage, const gfx::Rect& update_rect)
{
    ...
    if (!dcomp_surface_draw_texture_copy_) {
        ...
        HRESULT hr = d3d11_device->CreateTexture2D(&d3d11_texture_desc, nullptr, &dcomp_surface_draw_texture_copy_);

        if (FAILED(hr)) {
            LOG(ERROR) << "CreateTexture2D failed: " << logging::SystemErrorCodeToString(hr);
            return nullptr;
        }
    ...
    shared_texture_memory_ = CreateDawnSharedTextureMemory(device, dcomp_surface_draw_texture_copy_);
    ...
    wgpu::Texture texture = CreateDawnSharedTexture(shared_texture_memory_, usage, internal_usage,
        /*view_formats=*/ {});
    if (!texture || shared_texture_memory_.BeginAccess(texture, &desc) != wgpu::Status::Success) {
...
void DCompSurfaceImageBacking::EndDrawDawn(const wgpu::Device& device, wgpu::Texture texture)
{
    ...
    D3D11_BOX src_box = {
        static_cast<UINT>(update_rect_.x()), static_cast<UINT>(update_rect_.y()),
        0u, // front
        static_cast<UINT>(update_rect_.right()), static_cast<UINT>(update_rect_.bottom()),
        1u, // back
    };
    d3d11_context->CopySubresourceRegion(dcomp_surface_draw_texture_.Get(), /*DstSubresource=*/0, dcomp_update_offset_.x(), dcomp_update_offset_.y(),
        /*DstZ=*/0, dcomp_surface_draw_texture_copy_.Get(), /*SrcSubresource=*/0, &src_box);
