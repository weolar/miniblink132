...
gfx::BufferFormat GetBufferFormatForPlane(viz::SharedImageFormat format, int plane_index)
{
    ...
    case viz::SharedImageFormat::ChannelFormat::k8:
        return num_channels == 2 ? gfx::BufferFormat::RG_88 : gfx::BufferFormat::R_8;
    case viz::SharedImageFormat::ChannelFormat::k10:
    case viz::SharedImageFormat::ChannelFormat::k16:
    case viz::SharedImageFormat::ChannelFormat::k16F:
        return num_channels == 2 ? gfx::BufferFormat::RG_1616 : gfx::BufferFormat::R_16;
    }
    NOTREACHED();
}

gfx::BufferPlane GetBufferPlane(viz::SharedImageFormat format, int plane_index)
{
    DCHECK(format.IsValidPlaneIndex(plane_index));
...
// nullptr.
std::unique_ptr<ui::NativePixmapGLBinding> GetBinding(scoped_refptr<gfx::NativePixmap> pixmap, gfx::BufferFormat buffer_format, gfx::BufferPlane buffer_plane,
    const gfx::Size& size, const gfx::ColorSpace& color_space, GLuint& gl_texture_service_id, GLenum& target)
{
    ...
    std::unique_ptr<ui::NativePixmapGLBinding> np_gl_binding
        = gl_ozone->ImportNativePixmap(pixmap, buffer_format, buffer_plane, size, color_space, target, gl_texture_service_id);
    if (!np_gl_binding) {
...
// static
scoped_refptr<OzoneImageGLTexturesHolder> OzoneImageGLTexturesHolder::CreateAndInitTexturesHolder(
    SharedImageBacking* backing, scoped_refptr<gfx::NativePixmap> pixmap)
{
    scoped_refptr<OzoneImageGLTexturesHolder> holder = base::WrapRefCounted(new OzoneImageGLTexturesHolder());
...
bool OzoneImageGLTexturesHolder::Initialize(SharedImageBacking* backing, scoped_refptr<gfx::NativePixmap> pixmap)
{
    ...
    if (format.is_single_plane() || format.PrefersExternalSampler()) {
        ...
        return CreateAndStoreTexture(backing, std::move(pixmap), buffer_format, gfx::BufferPlane::DEFAULT, size);
    } else {
        ...
        for (int plane_index = 0; plane_index < format.NumberOfPlanes(); plane_index++) {
            ...
            if (!CreateAndStoreTexture(backing, pixmap, buffer_format, buffer_plane, size)) {
                return false;
            }
        }
    }
...
bool OzoneImageGLTexturesHolder::CreateAndStoreTexture(
    SharedImageBacking* backing, scoped_refptr<gfx::NativePixmap> pixmap, gfx::BufferFormat buffer_format, gfx::BufferPlane buffer_plane, const gfx::Size& size)
{
    GLenum target;
    GLuint gl_texture_service_id;
    std::unique_ptr<ui::NativePixmapGLBinding> np_gl_binding
        = GetBinding(pixmap, buffer_format, buffer_plane, size, backing->color_space(), gl_texture_service_id, target);
    if (!np_gl_binding) {
        return false;
    }

    textures_.push_back(base::MakeRefCounted<gpu::gles2::TexturePassthrough>(gl_texture_service_id, target));
