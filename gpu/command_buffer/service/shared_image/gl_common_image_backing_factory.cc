...
BASE_FEATURE(kAllowEs3F16CoreTypeForGlSi, "AllowEs3F16CoreTypeForGlSi", base::FEATURE_ENABLED_BY_DEFAULT);

std::optional<viz::SharedImageFormat> GetFallbackFormatIfNotSupported(viz::SharedImageFormat plane_format, const GLFormatCaps& caps)
{
    if (plane_format == viz::SinglePlaneFormat::kR_8 && (!caps.ext_texture_rg() || caps.disable_r8_shared_images())) {
        // Fallback to ALPHA_8 for R_8 format.
        return viz::SinglePlaneFormat::kALPHA_8;
    }
    if (plane_format == viz::SinglePlaneFormat::kRG_88 && !caps.ext_texture_rg()) {
        // No fallback for RG_88 format.
        return std::nullopt;
    }
    if ((plane_format == viz::SinglePlaneFormat::kR_16 || plane_format == viz::SinglePlaneFormat::kRG_1616) && !caps.ext_texture_norm16()) {
        // No fallback for R_16, RG_1616 format.
        return std::nullopt;
    }
    if (plane_format == viz::SinglePlaneFormat::kR_F16 && (!caps.is_atleast_gles3() || !caps.enable_texture_half_float_linear())) {
...
std::vector<GLCommonImageBackingFactory::FormatInfo> GetMultiPlaneFormatInfo(
    const std::map<viz::SharedImageFormat, GLCommonImageBackingFactory::FormatInfo>& supported_formats, const GLFormatCaps& gl_format_caps,
    viz::SharedImageFormat format)
{
    std::vector<viz::SharedImageFormat> plane_formats;
    for (int plane = 0; plane < format.NumberOfPlanes(); plane++) {
        viz::SharedImageFormat plane_format = GLTextureHolder::GetPlaneFormat(format, plane);
        auto fallback_format = GetFallbackFormatIfNotSupported(plane_format, gl_format_caps);
        if (!fallback_format) {
...
GLCommonImageBackingFactory::GLCommonImageBackingFactory(SharedImageUsageSet supported_usages, const GpuPreferences& gpu_preferences,
    const GpuDriverBugWorkarounds& workarounds, const gles2::FeatureInfo* feature_info, gl::ProgressReporter* progress_reporter)
    : SharedImageBackingFactory(supported_usages)
    , use_passthrough_(gpu_preferences.use_passthrough_cmd_decoder && gles2::PassthroughCommandDecoderSupported())
    ...
    , progress_reporter_(progress_reporter)
{
    ...
    bool enable_texture_storage = feature_info->feature_flags().ext_texture_storage;
    ...
    for (auto format : viz::SinglePlaneFormat::kAll) {
        ...
        const GLFormatDesc format_desc = gl_format_caps_.ToGLFormatDescOverrideHalfFloatType(format,
            /*plane_index=*/0);
        ...
        const bool at_least_es3 = gl::g_current_gl_version->IsAtLeastGLES(3, 0) && base::FeatureList::IsEnabled(kAllowEs3F16CoreTypeForGlSi);
        const bool supports_data_type = (gl_type == GL_HALF_FLOAT && at_least_es3) || validators->pixel_type.IsValid(gl_type);
        const bool supports_internal_format
            = (image_internal_format == GL_RGBA16F && at_least_es3) || validators->texture_internal_format.IsValid(image_internal_format);

        const bool uncompressed_format_valid = supports_internal_format && validators->texture_format.IsValid(gl_format);
        const bool compressed_format_valid = validators->compressed_texture_format.IsValid(image_internal_format);

        if (!(uncompressed_format_valid || compressed_format_valid) || !supports_data_type) {
            continue;
        }
        ...
        info.swizzle = gles2::TextureManager::GetCompatibilitySwizzle(feature_info, gl_format);
        info.image_internal_format = gles2::TextureManager::AdjustTexInternalFormat(feature_info, image_internal_format, gl_type);
        info.storage_internal_format = format_desc.storage_internal_format;
        info.adjusted_format = gles2::TextureManager::AdjustTexFormat(feature_info, gl_format);

        if (enable_texture_storage && !info.is_compressed && validators->texture_internal_format_storage.IsValid(info.storage_internal_format)) {
            ...
            info.adjusted_storage_internal_format = gles2::TextureManager::AdjustTexStorageFormat(feature_info, info.storage_internal_format);
        }
    }
}
...
std::vector<GLCommonImageBackingFactory::FormatInfo> GLCommonImageBackingFactory::GetFormatInfo(viz::SharedImageFormat format) const
{
...
bool GLCommonImageBackingFactory::CanCreateTexture(viz::SharedImageFormat format, const gfx::Size& size, base::span<const uint8_t> pixel_data, GLenum target)
{
    ...
    if (size.width() < 1 || size.height() < 1 || size.width() > max_texture_size_ || size.height() > max_texture_size_) {
        DVLOG(2) << "CreateSharedImage: invalid size: " << size.ToString() << ", max_texture_size_=" << max_texture_size_;
        return false;
    }
    ...
    if (!pixel_data.empty()) {
        ...
        if (format_info.is_compressed) {
            ...
            if (!gles2::ValidateCompressedTexDimensions(
                    target, /*level=*/0, size.width(), size.height(), /*depth=*/1, format_info.image_internal_format, &error_message)) {
            ...
            if (bytes_required < 0 || pixel_data.size() != static_cast<size_t>(bytes_required)) {
            ...
            if (!gles2::GLES2Util::ComputeImageDataSizes(size.width(), size.height(), /*depth=*/1, format_info.gl_format, format_info.gl_type, /*alignment=*/4,
                    &bytes_required, &unpadded_row_size, &padded_row_size)) {
...
GLCommonImageBackingFactory::FormatInfo::FormatInfo(const FormatInfo& other) = default;
GLCommonImageBackingFactory::FormatInfo::~FormatInfo() = default;
