...
AndroidImageBacking::AndroidImageBacking(const Mailbox& mailbox, viz::SharedImageFormat format, const gfx::Size& size, const gfx::ColorSpace& color_space,
    GrSurfaceOrigin surface_origin, SkAlphaType alpha_type, gpu::SharedImageUsageSet usage, std::string debug_label, size_t estimated_size, bool is_thread_safe,
    base::ScopedFD initial_upload_fd)
    : ClearTrackingSharedImageBacking(
        mailbox, format, size, color_space, surface_origin, alpha_type, usage, std::move(debug_label), estimated_size, is_thread_safe)
...
bool AndroidImageBacking::BeginWrite(base::ScopedFD* fd_to_wait_on)
{
    ...
    if (is_writing_) {
        LOG(ERROR) << "BeginWrite should only be called when there are no other writers";
        return false;
    }
    if (!allow_concurrent_read_write() && (!active_readers_.empty() || is_overlay_accessing_)) {
        LOG(ERROR) << "BeginWrite should only be called when there are no other readers";
        return false;
    }
    ...
    if (allow_concurrent_read_write()) {
        ...
        (*fd_to_wait_on) = gl::MergeFDs(std::move(read_sync_fd_), std::move(write_sync_fd_));
    }
...
bool AndroidImageBacking::BeginRead(const SharedImageRepresentation* reader, base::ScopedFD* fd_to_wait_on)
{
    AutoLock auto_lock(this);
...
void AndroidImageBacking::EndRead(const SharedImageRepresentation* reader, base::ScopedFD end_read_fd)
{
    AutoLock auto_lock(this);
    ...
    if (!allow_concurrent_read_write()) {
        read_sync_fd_ = gl::MergeFDs(std::move(read_sync_fd_), std::move(end_read_fd));
    }
}
