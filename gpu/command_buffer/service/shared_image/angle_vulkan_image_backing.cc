...
gl::ScopedEGLImage CreateEGLImage(VkImage image, const VkImageCreateInfo* create_info, unsigned int internal_format)
{
    ...
    return gl::MakeScopedEGLImage(EGL_NO_CONTEXT, EGL_VULKAN_IMAGE_ANGLE, reinterpret_cast<EGLClientBuffer>(&image), attribs);
}

} // namespace

// GLTexturePassthroughImageRepresentation implementation.
class AngleVulkanImageBacking::GLTexturePassthroughAngleVulkanImageRepresentation : public GLTexturePassthroughImageRepresentation {
public:
    GLTexturePassthroughAngleVulkanImageRepresentation(SharedImageManager* manager, AngleVulkanImageBacking* backing, MemoryTypeTracker* tracker,
...
class AngleVulkanImageBacking::SkiaAngleVulkanImageRepresentation : public SkiaGaneshImageRepresentation {
public:
    SkiaAngleVulkanImageRepresentation(GrDirectContext* gr_context, SharedImageManager* manager, AngleVulkanImageBacking* backing, MemoryTypeTracker* tracker)
    ...
    {
        auto promise_textures = BeginWriteAccess(begin_semaphores, end_semaphores, end_state);
        ...
        for (size_t plane = 0; plane < promise_textures.size(); ++plane) {
            ...
            if (!surface || surface_props != surface->props() || final_msaa_count != backing_impl()->surface_msaa_count_) {
                SkColorType sk_color_type = viz::ToClosestSkColorType(
                    /*gpu_compositing=*/true, format(), plane);
                surface = SkSurfaces::WrapBackendTexture(backing_impl()->gr_context(), promise_texture->backendTexture(), surface_origin(), final_msaa_count,
                    sk_color_type, backing_impl()->color_space().ToSkColorSpace(), &surface_props);
                if (!surface) {
...
AngleVulkanImageBacking::AngleVulkanImageBacking(scoped_refptr<SharedContextState> context_state, const Mailbox& mailbox, viz::SharedImageFormat format,
...
bool AngleVulkanImageBacking::Initialize(const base::span<const uint8_t>& data)
{
    ...
    constexpr gpu::SharedImageUsageSet usages_needing_color_attachment
        = SHARED_IMAGE_USAGE_GLES2_WRITE | SHARED_IMAGE_USAGE_RASTER_WRITE | SHARED_IMAGE_USAGE_DISPLAY_WRITE;
    ...
    for (int plane = 0; plane < num_planes; ++plane) {
        ...
        auto vulkan_image = VulkanImage::Create(device_queue, plane_size, vk_format, vk_usage, vk_create_flags, VK_IMAGE_TILING_OPTIMAL);
...
bool AngleVulkanImageBacking::InitializeWihGMB(gfx::GpuMemoryBufferHandle handle)
{
    DCHECK(format().is_single_plane() || format().PrefersExternalSampler());

    auto* vulkan_implementation = context_state_->vk_context_provider()->GetVulkanImplementation();
    auto* device_queue = context_state_->vk_context_provider()->GetDeviceQueue();
    ...
    auto vulkan_image = vulkan_implementation->CreateImageFromGpuMemoryHandle(device_queue, std::move(handle), size(), vk_format, color_space());
...
bool AngleVulkanImageBacking::UploadFromMemory(const std::vector<SkPixmap>& pixmaps)
{
    ...
    for (size_t i = 0; i < vk_textures_.size(); ++i) {
        ...
        bool result = gr_context()->updateBackendTexture(vk_textures_[i].backend_texture, pixmaps[i], surface_origin());
...
std::unique_ptr<GLTexturePassthroughImageRepresentation> AngleVulkanImageBacking::ProduceGLTexturePassthrough(
    SharedImageManager* manager, MemoryTypeTracker* tracker)
{
    ...
    return std::make_unique<GLTexturePassthroughAngleVulkanImageRepresentation>(manager, this, tracker, std::move(textures));
}

std::unique_ptr<SkiaGaneshImageRepresentation> AngleVulkanImageBacking::ProduceSkiaGanesh(
    SharedImageManager* manager, MemoryTypeTracker* tracker, scoped_refptr<SharedContextState> context_state)
{
    if (context_state->GrContextIsVulkan()) {
        ...
        return std::make_unique<SkiaAngleVulkanImageRepresentation>(context_state->gr_context(), manager, this, tracker);
    }
...
std::vector<sk_sp<GrPromiseImageTexture>> AngleVulkanImageBacking::GetPromiseTextures()
{
...
void AngleVulkanImageBacking::AcquireTextureANGLE()
{
    ...
    api->glAcquireTexturesANGLEFn(gl_textures_.size(), gl_texture_ids_.data(), gl_layouts_.data());
}
...
void AngleVulkanImageBacking::ReleaseTextureANGLE()
{
    ...
    api->glReleaseTexturesANGLEFn(gl_textures_.size(), gl_texture_ids_.data(), gl_layouts_.data());
...
void AngleVulkanImageBacking::PrepareBackendTexture()
{
    ...
    for (size_t i = 0; i < vk_textures_.size(); ++i) {
        ...
        GrBackendTextures::SetVkImageLayout(&vk_textures_[i].backend_texture, vk_layout);
    }
}
...
bool AngleVulkanImageBacking::InitializePassthroughTexture()
{
    ...
    for (int plane = 0; plane < num_planes; ++plane) {
        ...
        auto format_desc = context_state_->GetGLFormatCaps().ToGLFormatDesc(format(), plane);
        auto egl_image = CreateEGLImage(vulkan_image->image(), &vulkan_image->create_info(), format_desc.image_internal_format);
        ...
        GLuint texture_id = MakeTextureAndSetParameters(GL_TEXTURE_2D,
        ...
        if (gl::g_current_gl_driver->ext.b_GL_KHR_debug) {
            const std::string label = "SharedImage_AngleVulkan" + CreateLabelForSharedImageUsage(usage());
...
AngleVulkanImageBacking::TextureHolderGL::TextureHolderGL(TextureHolderGL&& other) = default;
AngleVulkanImageBacking::TextureHolderGL& AngleVulkanImageBacking::TextureHolderGL::operator=(TextureHolderGL&& other) = default;
AngleVulkanImageBacking::TextureHolderGL::~TextureHolderGL() = default;
