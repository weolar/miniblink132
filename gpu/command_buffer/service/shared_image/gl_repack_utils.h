...
GPU_GLES2_EXPORT std::vector<uint8_t> RepackPixelDataAsRgb(const gfx::Size& size, const SkPixmap& src_pixmap, bool src_is_bgrx);

// Repacks `src_pixmap` so it has `dst_stride`. `dst_stride` must be smaller
// than the stride of `src_pixmap`.
GPU_GLES2_EXPORT std::vector<uint8_t> RepackPixelDataWithStride(const gfx::Size& size, const SkPixmap& src_pixmap, size_t dst_stride);

// Unpacks `src_data` into `dst_pixmap`. The stride of `dst_pixmap` must be
// larger than `src_stride`.
GPU_GLES2_EXPORT void UnpackPixelDataWithStride(const gfx::Size& size, const std::vector<uint8_t>& src_data, size_t src_stride, const SkPixmap& dst_pixmap);
