...
public:
    ...
    void SetUp() override
    {
        ...
        decoder_.reset(WebGPUDecoder::Create(decoder_client_.get(), command_buffer_service_.get(), nullptr, nullptr, &outputter_, {}, nullptr,
            DawnCacheOptions(), &mock_isolation_key_provider_));
...
TEST_F(WebGPUDecoderTest, IsolationKeyFromDocument)
{
    ...
    EXPECT_CALL(mock_isolation_key_provider_, GetIsolationKey(wgpu_context_token, _)).Times(1);

    cmds::SetWebGPUExecutionContextToken cmd;
    cmd.Init(base::to_underlying(wgpu_context_token.variant_index()), high >> 32, high, low >> 32, low);
    ExecuteCmd(cmd);
}
