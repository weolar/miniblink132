...
#if BUILDFLAG(IS_MAC)
namespace {
class AppleGpuMemoryDumpProvider : public base::trace_event::MemoryDumpProvider {
public:
...
{
    base::trace_event::MemoryDumpManager::GetInstance()->RegisterDumpProvider(this, "CommandBuffer", nullptr);
}

bool AppleGpuMemoryDumpProvider::OnMemoryDump(const base::trace_event::MemoryDumpArgs& args, base::trace_event::ProcessMemoryDump* pmd)
{
    ...
    while (true) {
        ...
        base::MachVMRegionResult result = base::GetBasicInfo(task, &size, &address, &basic_info);
        ...
        kern_return_t ret = mach_vm_region(task, &address, &size, VM_REGION_EXTENDED_INFO, reinterpret_cast<vm_region_info_t>(&info), &count, &object_name);
        ...
        if (info.user_tag != VM_MEMORY_IOSURFACE && info.user_tag != VM_MEMORY_IOACCELERATOR) {
            continue;
        }

        int purgeable_state = 0;
        ret = mach_vm_purgable_control(task, address, VM_PURGABLE_GET_STATE, &purgeable_state);
        ...
        case VM_MEMORY_IOSURFACE:
            ...
            surface_swapped_out_size += info.pages_swapped_out * base::GetPageSize();
            surface_dirty_size += info.pages_dirtied * base::GetPageSize();
            if (purgeable_state == VM_PURGABLE_VOLATILE || purgeable_state == VM_PURGABLE_EMPTY) {
        ...
        case VM_MEMORY_IOACCELERATOR:
            ...
            accelerator_swapped_out_size += info.pages_swapped_out * base::GetPageSize();
    ...
    dump->AddScalar("resident_swapped", "bytes", surface_resident_size + surface_swapped_out_size);
    ...
    dump->AddScalar("resident_swapped", "bytes", accelerator_resident_size + accelerator_swapped_out_size);
...
{
    static int slice_size = (base::FeatureList::IsEnabled(features::kIncreasedCmdBufferParseSlice) ? CommandBufferService::kParseCommandsSliceLarge
                                                                                                   : CommandBufferService::kParseCommandsSliceSmall);
    return slice_size;
}

CommandBufferService::CommandBufferService(CommandBufferServiceClient* client, MemoryTracker* memory_tracker)
    : client_(client)
    , transfer_buffer_manager_(std::make_unique<TransferBufferManager>(memory_tracker))
{
    DCHECK(client_);
    state_.token = 0;
#if BUILDFLAG(IS_MAC)
...
void CommandBufferService::Flush(int32_t put_offset, AsyncAPIInterface* handler)
{
    DCHECK(handler);
    ...
    TRACE_EVENT1("gpu", "CommandBufferService:PutChanged", "handler", std::string(handler->GetLogPrefix()));
    ...
    while (put_offset_ != state_.get_offset) {
        ...
        error::Error error = handler->DoCommands(GetCommandBufferSliceSize(), buffer_ + state_.get_offset, num_entries, &entries_processed);
        ...
        if (client_->OnCommandBatchProcessed() == CommandBufferServiceClient::kPauseExecution) {
            paused_ = true;
            TRACE_COUNTER_ID1("gpu", "CommandBufferService::Paused", this, paused_);
            break;
        }
...
{
    DCHECK((put_offset_ == state_.get_offset) || (state_.error != error::kNoError));
    put_offset_ = 0;
    state_.get_offset = 0;
    ...
    if (transfer_buffer) {
        ...
        DCHECK_EQ(0u, (reinterpret_cast<intptr_t>(memory)) % alignof(CommandBufferEntry));
...
void CommandBufferService::SetSharedStateBuffer(std::unique_ptr<BufferBacking> shared_state_buffer)
{
    shared_state_buffer_ = std::move(shared_state_buffer);
    DCHECK(shared_state_buffer_->GetSize() >= sizeof(*shared_state_));

    shared_state_ = static_cast<CommandBufferSharedState*>(shared_state_buffer_->GetMemory());
...
{
    DLOG_IF(ERROR, release_count < state_.release_count) << "Non-monotonic SetReleaseCount";
    state_.release_count = release_count;
    UpdateState();
}

scoped_refptr<Buffer> CommandBufferService::CreateTransferBuffer(uint32_t size, int32_t* id, uint32_t alignment)
{
    *id = GetNextBufferId();
...
bool CommandBufferService::RegisterTransferBuffer(int32_t id, scoped_refptr<Buffer> buffer)
{
    return transfer_buffer_manager_->RegisterTransferBuffer(id, std::move(buffer));
}

scoped_refptr<Buffer> CommandBufferService::CreateTransferBufferWithId(uint32_t size, int32_t id, uint32_t alignment)
{
...
void CommandBufferService::SetContextLostReason(error::ContextLostReason reason)
{
...
{
    return client_->OnCommandBatchProcessed() == CommandBufferServiceClient::kPauseExecution;
}

void CommandBufferService::SetScheduled(bool scheduled)
{
    TRACE_EVENT2("gpu", "CommandBufferService:SetScheduled", "this", static_cast<void*>(this), "scheduled", scheduled);
