...
SchedulerSequence::SchedulerSequence(Scheduler* scheduler, scoped_refptr<base::SingleThreadTaskRunner> task_runner, bool target_thread_is_always_available)
    : SingleTaskSequence()
    , scheduler_(scheduler)
    , sequence_id_(scheduler->CreateSequence(SchedulingPriority::kHigh, std::move(task_runner)))
...
void SchedulerSequence::ScheduleTask(
    base::OnceClosure task, std::vector<SyncToken> sync_token_fences, const SyncToken& release, ReportingCallback report_callback)
{
...
#if DCHECK_IS_ON()
        DCHECK(!schedule_task_disallowed) << "If your CL is failing this DCHECK, then that means you are "
    ...
    ScheduleOrRetainTask(std::move(task), std::move(sync_token_fences), release, std::move(report_callback));
}

void SchedulerSequence::ScheduleOrRetainTask(
    base::OnceClosure task, std::vector<gpu::SyncToken> sync_token_fences, const SyncToken& release, ReportingCallback report_callback)
{
    scheduler_->ScheduleTask(Scheduler::Task(sequence_id_, std::move(task), std::move(sync_token_fences), release, std::move(report_callback)));
}
...
ScopedSyncPointClientState SchedulerSequence::CreateSyncPointClientState(CommandBufferNamespace namespace_id, CommandBufferId command_buffer_id)
{
    return scheduler_->CreateSyncPointClientState(sequence_id_, namespace_id, command_buffer_id);
}
