...
public:
    GLContextVirtualTest()
        : decoder_(new MockGLES2Decoder(&client_, &command_buffer_service_, &outputter_))
    {
    }
...
{
    EXPECT_CALL(*gl_, GetError()).Times(AnyNumber()).WillRepeatedly(Return(GL_NO_ERROR));
    EXPECT_CALL(*gl_, GetString(GL_VERSION)).Times(AnyNumber()).WillRepeatedly(Return(reinterpret_cast<unsigned const char*>("2.0")));
    EXPECT_CALL(*gl_, GetString(GL_EXTENSIONS)).Times(AnyNumber()).WillRepeatedly(Return(reinterpret_cast<unsigned const char*>("")));
    {
        ...
        auto context = base::MakeRefCounted<GLContextVirtual>(share_group, base_context.get(), decoder_->AsWeakPtr());
...
TEST_F(GLContextVirtualTest, CheckStickyGraphicsResetStatus)
{
    ...
    // guilty context.
    EXPECT_CALL(*gl_, GetGraphicsResetStatusARB()).WillOnce(Return(GL_GUILTY_CONTEXT_RESET_ARB));
    EXPECT_EQ(unsigned { GL_UNKNOWN_CONTEXT_RESET_ARB }, context->CheckStickyGraphicsResetStatus());
    // The underlying real context still knows, though.
    EXPECT_EQ(unsigned { GL_GUILTY_CONTEXT_RESET_ARB }, base_context->CheckStickyGraphicsResetStatus());
}
