...
ExternalSemaphorePool::ExternalSemaphorePool(SharedContextState* shared_context_state)
...
ExternalSemaphore ExternalSemaphorePool::GetOrCreateSemaphore()
{
    ...
    return ExternalSemaphore::Create(shared_context_state_->vk_context_provider());
}
...
void ExternalSemaphorePool::ReturnSemaphores(std::vector<ExternalSemaphore> semaphores)
{
...
void ExternalSemaphorePool::ReturnSemaphoresWithFenceHelper(std::vector<ExternalSemaphore> semaphores)
{
#if DCHECK_IS_ON()
    ...
    auto* fence_helper = shared_context_state_->vk_context_provider()->GetDeviceQueue()->GetFenceHelper();
    fence_helper->EnqueueCleanupTaskForSubmittedWork(base::BindOnce(
        [](base::WeakPtr<ExternalSemaphorePool> pool, std::vector<ExternalSemaphore> semaphores, VulkanDeviceQueue* device_queue, bool device_lost) {
            if (pool)
