...
private:
    bool AlignMemory(uint32_t size, size_t alignment)
    {
        ...
        if (!checked_padded_size.AssignIfValid(&padded_size) || padded_size > memory_size_) {
            return false;
        }
...
class ServiceFontManager::SkiaDiscardableManager : public SkStrikeClient::DiscardableHandleManager {
public:
    explicit SkiaDiscardableManager(scoped_refptr<ServiceFontManager> font_manager)
    ...
    {
        UMA_HISTOGRAM_ENUMERATION("GPU.OopRaster.GlyphCacheMiss", type, SkStrikeClient::CacheMissType::kLast + 1);
...
#if DCHECK_IS_ON()
        crash_reporter::ScopedCrashKeyString auto_clear(&crash_key, base::StringPrintf(kFormatString, type, fontSize));
        NOTREACHED();
#else
        if (dump_count_ < kMaxDumps && base::RandInt(1, 100) == 1 && !font_manager_->disable_oopr_debug_crash_dump()) {
            crash_reporter::ScopedCrashKeyString auto_clear(&crash_key, base::StringPrintf(kFormatString, type, fontSize));
    ...
    void notifyReadFailure(const DiscardableHandleManager::ReadFailureData& data) override
    {
        if (dump_count_ >= kMaxDumps)
            return;

        std::string str = base::StringPrintf("ms: %zd, br: %zd, ts: %" PRIu64 ", sc: %" PRIu64 ", gic: %" PRIu64 ", gpc: %" PRIu64, data.memorySize,
            data.bytesRead, data.typefaceSize, data.strikeCount, data.glyphImagesCount, data.glyphPathsCount);
...
ServiceFontManager::ServiceFontManager(Client* client, bool disable_oopr_debug_crash_dump)
    : client_(client)
    , client_thread_id_(base::PlatformThread::CurrentId())
    , strike_client_(std::make_unique<SkStrikeClient>(sk_make_sp<SkiaDiscardableManager>(this)))
...
{
    DCHECK(destroyed_);
    DLOG_IF(ERROR, !discardable_handle_map_.empty()) << "discardable_handle_map_ is not empty.";
}
...
bool ServiceFontManager::Deserialize(const volatile uint8_t* memory, uint32_t memory_size, std::vector<SkDiscardableHandleId>* locked_handles)
{
    ...
    for (uint32_t i = 0; i < new_handles_created; ++i) {
        ...
        if (!DiscardableHandleBase::ValidateParameters(buffer.get(), handle.byte_offset)) {
            return false;
        }

        if (!AddHandle(handle.handle_id, ServiceDiscardableHandle(std::move(buffer), handle.byte_offset, handle.shm_id))) {
            return false;
        }
    }
...
bool ServiceFontManager::AddHandle(SkDiscardableHandleId handle_id, ServiceDiscardableHandle handle)
{
    bool inserted;
    std::tie(std::ignore, inserted) = discardable_handle_map_.try_emplace(handle_id, std::move(handle));
    return inserted;
}

bool ServiceFontManager::Unlock(const std::vector<SkDiscardableHandleId>& handles)
{
    base::AutoLock hold(lock_);
...
bool ServiceFontManager::DeleteHandle(SkDiscardableHandleId handle_id)
{
    ...
    const bool report_progress = base::PlatformThread::CurrentId() == client_thread_id_ && !destroyed_;

    auto it = discardable_handle_map_.find(handle_id);
    if (it == discardable_handle_map_.end()) {
        LOG(ERROR) << "Tried to delete invalid SkDiscardableHandleId: " << handle_id;
        if (report_progress) {
            DCHECK(client_);
