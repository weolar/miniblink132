...
std::unique_ptr<ui::ScopedMakeCurrent> MakeCurrentIfNeeded(gpu::TextureOwner* texture_owner)
{
    ...
    auto scoped_current = std::make_unique<ui::ScopedMakeCurrent>(context, texture_owner->GetSurface());
...
SurfaceTextureGLOwner::SurfaceTextureGLOwner(std::unique_ptr<AbstractTextureAndroid> texture, scoped_refptr<SharedContextState> context_state)
    : TextureOwner(true /*binds_texture_on_update */, std::move(texture), std::move(context_state))
...
void SurfaceTextureGLOwner::SetFrameAvailableCallback(const base::RepeatingClosure& frame_available_cb)
{
...
void SurfaceTextureGLOwner::RunWhenBufferIsAvailable(base::OnceClosure callback)
{
...
void SurfaceTextureGLOwner::UpdateTexImage()
{
    ...
    if (surface_texture_) {
        ...
        gl::ScopedRestoreTexture scoped_restore_texture(gl::g_current_gl_context, GL_TEXTURE_EXTERNAL_OES);
...
std::unique_ptr<base::android::ScopedHardwareBufferFenceSync> SurfaceTextureGLOwner::GetAHardwareBuffer()
{
    NOTREACHED() << "Don't use AHardwareBuffers with SurfaceTextureGLOwner";
}

bool SurfaceTextureGLOwner::GetCodedSizeAndVisibleRect(gfx::Size rotated_visible_size, gfx::Size* coded_size, gfx::Rect* visible_rect)
{
    DCHECK(coded_size);
    DCHECK(visible_rect);
    ...
    if (!result || !coded_rect.Contains(*visible_rect) || !kMaxRect.Contains(coded_rect)) {
        ...
        *visible_rect = gfx::Rect(rotated_visible_size);
        ...
        LOG(ERROR) << "Wrong matrix decomposition: coded: " << coded_size_for_debug.ToString() << "visible: " << visible_rect_for_debug.ToString()
                   << "matrix: " << mtx[0] << ", " << mtx[1] << ", " << mtx[4] << ", " << mtx[5] << ", " << mtx[12] << ", " << mtx[13];
...
// static
bool SurfaceTextureGLOwner::DecomposeTransform(float mtx[16], gfx::Size rotated_visible_size, gfx::Size* coded_size, gfx::Rect* visible_rect)
{
    DCHECK(coded_size);
    DCHECK(visible_rect);
    ...
    *visible_rect = gfx::Rect();
    ...
    if (mtx[0]) {
        ...
        LOG_IF(DFATAL, mtx[1] || mtx[4] || !mtx[5]) << "Invalid matrix: " << mtx[0] << ", " << mtx[1] << ", " << mtx[4] << ", " << mtx[5];
    ...
    *coded_size = visible_rect->size();
    ...
    for (float shrink_amount : possible_shrinks_amounts) {
        if (sx < 1.0f) {
            coded_size->set_width(std::round((visible_rect->width() - 2.0f * shrink_amount) / sx));
            visible_rect->set_x(std::round(tx * coded_size->width() - shrink_amount));
        }
        if (sy < 1.0f) {
            coded_size->set_height(std::round((visible_rect->height() - 2.0f * shrink_amount) / sy));
...
bool SurfaceTextureGLOwner::OnMemoryDump(const base::trace_event::MemoryDumpArgs& args, base::trace_event::ProcessMemoryDump* pmd)
{
    ...
    if (last_coded_size_for_memory_dumps_) {
        total_size = format.EstimatedSizeInBytes(last_coded_size_for_memory_dumps_.value());
    }
    ...
    if (args.level_of_detail != base::trace_event::MemoryDumpLevelOfDetail::kBackground) {
        dump->AddString("dimensions", "", last_coded_size_for_memory_dumps_.value_or(gfx::Size()).ToString());
    }
    return true;
}
