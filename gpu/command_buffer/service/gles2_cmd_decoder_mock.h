...
public:
    MockGLES2Decoder(DecoderClient* client, CommandBufferServiceBase* command_buffer_service, Outputter* outputter);
    ...
    MOCK_METHOD5(Initialize,
        gpu::ContextResult(const scoped_refptr<gl::GLSurface>& surface, const scoped_refptr<gl::GLContext>& context, bool offscreen,
    ...
    MOCK_METHOD5(SetDefaultFramebufferSharedImage, void(const Mailbox& mailbox, int samples, bool preserve, bool needs_depth, bool needs_stencil));
    ...
    MOCK_METHOD1(GetTranslator, scoped_refptr<ShaderTranslatorInterface>(unsigned int type));
    ...
    MOCK_CONST_METHOD1(RestoreActiveTextureUnitBinding, void(unsigned int target));
    ...
    MOCK_METHOD0(GetTransformFeedbackManager, gpu::gles2::TransformFeedbackManager*());
    ...
    MOCK_METHOD2(GetServiceTextureId, bool(uint32_t client_texture_id, uint32_t* service_texture_id));
    MOCK_METHOD9(
        ClearLevel, bool(Texture* texture, unsigned target, int level, unsigned format, unsigned type, int x_offset, int y_offset, int width, int height));
    MOCK_METHOD6(ClearCompressedTextureLevel, bool(Texture* texture, unsigned target, int level, unsigned format, int width, int height));
    MOCK_METHOD7(ClearCompressedTextureLevel3D, bool(Texture* texture, unsigned target, int level, unsigned format, int width, int height, int depth));
    MOCK_METHOD1(IsCompressedTextureFormat, bool(unsigned format));
    MOCK_METHOD8(ClearLevel3D, bool(Texture* texture, unsigned target, int level, unsigned format, unsigned type, int width, int height, int depth));
    ...
    MOCK_METHOD1(SetCopyTextureResourceManagerForTest, void(CopyTextureCHROMIUMResourceManager* copy_texture_resource_manager));
    MOCK_METHOD1(SetCopyTexImageBlitterForTest, void(CopyTexImageResourceManager* copy_texture_resource_manager));
