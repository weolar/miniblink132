...
// static
std::unique_ptr<DawnInstance> DawnInstance::Create(dawn::platform::Platform* platform, const GpuPreferences& gpu_preferences, SafetyLevel safety,
    WGPULoggingCallback logging_callback, void* logging_callback_userdata)
{
    ...
    if (safety == SafetyLevel::kSafeExperimental) {
        require_instance_enabled_toggles.push_back("expose_wgsl_experimental_features");
    } else if (safety == SafetyLevel::kUnsafe) {
        require_instance_enabled_toggles.push_back("allow_unsafe_apis");
    }

    for (const std::string& toggles : gpu_preferences.enabled_dawn_features_list) {
        require_instance_enabled_toggles.push_back(toggles.c_str());
    }
    for (const std::string& toggles : gpu_preferences.disabled_dawn_features_list) {
    ...
    dawn_toggle_desc.disabledToggleCount = require_instance_disabled_toggles.size();
    ...
    auto instance = std::make_unique<DawnInstance>(reinterpret_cast<const WGPUInstanceDescriptor*>(&instance_desc));
