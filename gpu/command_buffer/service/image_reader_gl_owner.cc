...
BASE_FEATURE(kAlwaysUsePrivateFormatForImageReader, "AlwaysUsePrivateFormatForImageReader", base::FEATURE_ENABLED_BY_DEFAULT);
...
std::optional<gfx::Size> GetImageSize(AImage* image)
{
    ...
    if (AImage_getWidth(image, &width) != AMEDIA_OK || AImage_getHeight(image, &height) != AMEDIA_OK || width <= 0 || height <= 0) {
        return std::nullopt;
    }
...
class ImageReaderGLOwner::ScopedHardwareBufferImpl : public base::android::ScopedHardwareBufferFenceSync {
public:
    ScopedHardwareBufferImpl(
        scoped_refptr<ImageReaderGLOwner> texture_owner, AImage* image, base::android::ScopedHardwareBufferHandle handle, base::ScopedFD fence_fd)
        : base::android::ScopedHardwareBufferFenceSync(std::move(handle), std::move(fence_fd), base::ScopedFD())
...
ImageReaderGLOwner::ImageReaderGLOwner(std::unique_ptr<AbstractTextureAndroid> texture, Mode mode, scoped_refptr<SharedContextState> context_state,
    scoped_refptr<RefCountedLock> drdc_lock, TextureOwnerCodecType type_for_metrics)
    : TextureOwner(false /* binds_texture_on_image_update */, std::move(texture), std::move(context_state))
    ...
    , type_for_metrics_(type_for_metrics)
{
    ...
    AIMAGE_FORMATS format = mode == Mode::kAImageReaderSecureSurfaceControl ? AIMAGE_FORMAT_PRIVATE : AIMAGE_FORMAT_YUV_420_888;
    ...
    media_status_t return_code = AImageReader_newWithUsage(width, height, format, usage, max_images_, &reader);
    if (return_code != AMEDIA_OK) {
        LOG(ERROR) << " Image reader creation failed on device model : " << base::android::BuildInfo::GetInstance()->model()
    ...
    if (AImageReader_setImageListener(image_reader_, listener_.get()) != AMEDIA_OK) {
...
void ImageReaderGLOwner::SetFrameAvailableCallback(const base::RepeatingClosure& frame_available_cb)
{
...
gl::ScopedJavaSurface ImageReaderGLOwner::CreateJavaSurface() const
{
    ...
    auto j_surface = base::android::ScopedJavaLocalRef<jobject>::Adopt(env, ANativeWindow_toSurface(env, window));
    DCHECK(j_surface);
...
void ImageReaderGLOwner::UpdateTexImage()
{
    ...
    if (max_images_ - image_refs_.size() < 2) {
        ...
        return_code = AImageReader_acquireNextImageAsync(image_reader_, &image, &acquire_fence_fd);
    } else {
        return_code = AImageReader_acquireLatestImageAsync(image_reader_, &image, &acquire_fence_fd);
    }
    base::UmaHistogramSparse("Media.AImageReaderGLOwner.AcquireImageResult", return_code);

    UMA_HISTOGRAM_ENUMERATION("Media.AImageReaderGLOwner.CodecType", type_for_metrics_);
    ...
    case AMEDIA_IMGREADER_MAX_IMAGES_ACQUIRED:
        LOG(ERROR) << "number of concurrently acquired images has reached the limit";
        return;
    ...
    UMA_HISTOGRAM_BOOLEAN("Media.AImageReaderGLOwner.HasFence", scoped_acquire_fence_fd.is_valid());

    // Make the newly acquired image as current image.
    current_image_ref_.emplace(this, image, std::move(scoped_acquire_fence_fd));
}

std::unique_ptr<base::android::ScopedHardwareBufferFenceSync> ImageReaderGLOwner::GetAHardwareBuffer()
{
    base::AutoLock auto_lock(lock_);
    ...
    return std::make_unique<ScopedHardwareBufferImpl>(
        this, current_image_ref_->image(), base::android::ScopedHardwareBufferHandle::Create(buffer), current_image_ref_->GetReadyFence());
}
...
gfx::Rect ImageReaderGLOwner::GetCropRectLocked()
{
    ...
    media_status_t return_code = AImage_getCropRect(current_image_ref_->image(), &crop_rect);
    if (return_code != AMEDIA_OK) {
        DLOG(ERROR) << "Error querying crop rectangle from the image : " << return_code;
        return gfx::Rect();
    }
    DCHECK_GE(crop_rect.right, crop_rect.left);
    DCHECK_GE(crop_rect.bottom, crop_rect.top);
    return gfx::Rect(crop_rect.left, crop_rect.top, crop_rect.right - crop_rect.left, crop_rect.bottom - crop_rect.top);
}
...
void ImageReaderGLOwner::ReleaseRefOnImage(AImage* image, base::ScopedFD fence_fd)
{
    base::AutoLock auto_lock(lock_);
    ReleaseRefOnImageLocked(image, std::move(fence_fd));
}

void ImageReaderGLOwner::ReleaseRefOnImageLocked(AImage* image, base::ScopedFD fence_fd)
{
    lock_.AssertAcquired();
    ...
    image_ref.count--;
    image_ref.release_fence_fd = gl::MergeFDs(std::move(image_ref.release_fence_fd), std::move(fence_fd));
...
{
    ImageReaderGLOwner* image_reader_ptr = reinterpret_cast<ImageReaderGLOwner*>(context);
...
bool ImageReaderGLOwner::GetCodedSizeAndVisibleRect(gfx::Size rotated_visible_size, gfx::Size* coded_size, gfx::Rect* visible_rect)
{
    base::AutoLock auto_lock(lock_);
    DCHECK(visible_rect);
    DCHECK(coded_size);
...
bool ImageReaderGLOwner::OnMemoryDump(const base::trace_event::MemoryDumpArgs& args, base::trace_event::ProcessMemoryDump* pmd)
{
    if (args.level_of_detail == base::trace_event::MemoryDumpLevelOfDetail::kBackground) {
        auto dump_name = base::StringPrintf("gpu/media_texture_owner_%d/", tracing_id());

        base::trace_event::MemoryAllocatorDump* dump = pmd->CreateAllocatorDump(dump_name);
        dump->AddScalar(base::trace_event::MemoryAllocatorDump::kNameSize, base::trace_event::MemoryAllocatorDump::kUnitsBytes, total_estimated_size_in_bytes_);
    ...
    for (const auto& image : image_refs_) {
        std::string dump_name = base::StringPrintf("gpu/media_texture_owner_%d/image_%d", tracing_id(), i++);
        ...
        base::trace_event::MemoryAllocatorDump* dump = pmd->CreateAllocatorDump(dump_name);
        dump->AddScalar(
            base::trace_event::MemoryAllocatorDump::kNameSize, base::trace_event::MemoryAllocatorDump::kUnitsBytes, image.second.estimated_size_in_bytes);
...
ImageReaderGLOwner::ImageRef& ImageReaderGLOwner::ImageRef::operator=(ImageRef&& other) = default;

ImageReaderGLOwner::ScopedCurrentImageRef::ScopedCurrentImageRef(ImageReaderGLOwner* texture_owner, AImage* image, base::ScopedFD ready_fence)
...
base::ScopedFD ImageReaderGLOwner::ScopedCurrentImageRef::GetReadyFence() const
{
