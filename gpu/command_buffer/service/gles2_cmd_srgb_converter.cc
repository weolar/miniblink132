...
SRGBConverter::SRGBConverter(const gles2::FeatureInfo* feature_info)
...
void SRGBConverter::InitializeSRGBConverterProgram()
{
    ...
    const char* kShaderPreamble = "#ifdef GL_ES\n"
    ...
    if (feature_info_->gl_version_info().is_es) {
        if (feature_info_->gl_version_info().is_es3) {
            ...
            vs_source += "#define ATTRIBUTE in\n"
                         "#define VARYING out\n";
        } else {
            vs_source += "#define ATTRIBUTE attribute\n"
    ...
    vs_source += "VARYING TexCoordPrecision vec2 v_texcoord;\n"
                 "\n"
                 "void main()\n"
                 "{\n"
    ...
    if (feature_info_->gl_version_info().is_es) {
        if (feature_info_->gl_version_info().is_es3) {
            fs_source += "#define VARYING in\n"
                         "out vec4 frag_color;\n"
                         "#define FRAGCOLOR frag_color\n"
                         "#define TextureLookup texture\n";
        } else {
            fs_source += "#define VARYING varying\n"
    ...
    fs_source += "uniform mediump sampler2D u_source_texture;\n"
    ...
    GLuint texture_uniform = glGetUniformLocation(srgb_converter_program_, "u_source_texture");
    glUseProgram(srgb_converter_program_);
    glUniform1i(texture_uniform, 0);
}

void SRGBConverter::InitializeSRGBConverter(const gles2::GLES2Decoder* decoder)
{
    ...
    glGenTextures(srgb_converter_textures_.size(), srgb_converter_textures_.data());
...
{
    if (srgb_converter_initialized_) {
        glDeleteTextures(srgb_converter_textures_.size(), srgb_converter_textures_.data());
...
void SRGBConverter::Blit(const gles2::GLES2Decoder* decoder, GLint srcX0, GLint srcY0, GLint srcX1, GLint srcY1, GLint dstX0, GLint dstY0, GLint dstX1,
    GLint dstY1, GLbitfield mask, GLenum filter, const gfx::Size& framebuffer_size, GLuint src_framebuffer, GLenum src_framebuffer_internal_format,
    GLenum src_framebuffer_format, GLenum src_framebuffer_type, GLuint dst_framebuffer, bool decode, bool encode, bool enable_scissor_test)
{
    ...
    if (decode) {
        ...
        xoffset = c.x() - x;
        yoffset = c.y() - y;
        glCopyTexImage2D(GL_TEXTURE_2D, 0, src_framebuffer_internal_format, c.x(), c.y(), c.width(), c.height(), 0);
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA32F, c.width(), c.height(), 0, GL_RGBA, GL_FLOAT, nullptr);
        glBindFramebufferEXT(GL_FRAMEBUFFER, srgb_decoder_fbo_);
        glFramebufferTexture2DEXT(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, srgb_converter_textures_[1], 0);
    ...
    if (encode) {
        ...
        glBindBuffer(GL_PIXEL_UNPACK_BUFFER, 0);
        glTexImage2D(GL_TEXTURE_2D, 0, decode ? GL_RGBA32F : src_framebuffer_internal_format, width_draw, height_draw, 0,
            decode ? GL_RGBA : src_framebuffer_format, decode ? GL_FLOAT : src_framebuffer_type, nullptr);

        glBindFramebufferEXT(GL_DRAW_FRAMEBUFFER, srgb_encoder_fbo_);
        glFramebufferTexture2DEXT(GL_DRAW_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, srgb_converter_textures_[0], 0);
    } else {
    ...
    glBlitFramebuffer(decode ? (srcX0 < srcX1 ? 0 - xoffset : width_read - xoffset) : srcX0,
        decode ? (srcY0 < srcY1 ? 0 - yoffset : height_read - yoffset) : srcY0, decode ? (srcX0 < srcX1 ? width_read - xoffset : 0 - xoffset) : srcX1,
        decode ? (srcY0 < srcY1 ? height_read - yoffset : 0 - yoffset) : srcY1, encode ? (dstX0 < dstX1 ? 0 : width_draw) : dstX0,
        encode ? (dstY0 < dstY1 ? 0 : height_draw) : dstY0, encode ? (dstX0 < dstX1 ? width_draw : 0) : dstX1,
        encode ? (dstY0 < dstY1 ? height_draw : 0) : dstY1, mask, filter);
...
void SRGBConverter::GenerateMipmap(gles2::GLES2Decoder* decoder, Texture* tex, GLenum target)
{
    ...
    if (tex->IsImmutable()) {
        ...
        mipmap_levels = TextureManager::ComputeMipMapCount(target, width, height, depth);
    }
    ...
    glBindTexture(GL_TEXTURE_2D, srgb_converter_textures_[1]);
    if (feature_info_->ext_color_buffer_float_available() && feature_info_->oes_texture_float_linear_available()) {
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA32F, width, height, 0, GL_RGBA, GL_FLOAT, nullptr);
    } else {
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, nullptr);
    }
    glBindFramebufferEXT(GL_DRAW_FRAMEBUFFER, srgb_decoder_fbo_);
    glFramebufferTexture2DEXT(GL_DRAW_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, srgb_converter_textures_[1], 0);
    ...
    if (!tex->IsImmutable()) {
        ...
        for (base::CheckedNumeric<GLint> i = level; i.IsValid() && i.ValueOrDie() <= max_mipmap_available_level; ++i) {
            glTexImage2D(GL_TEXTURE_2D, i.ValueOrDie(), internal_format, level_width, level_height, 0, format, type, nullptr);
    ...
    for (base::CheckedNumeric<GLint> i = level; i.IsValid() && i.ValueOrDie() <= max_mipmap_available_level; ++i) {
        ...
        glFramebufferTexture2DEXT(GL_DRAW_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, tex->service_id(), i.ValueOrDie());

        DCHECK_EQ(static_cast<GLenum>(GL_FRAMEBUFFER_COMPLETE), glCheckFramebufferStatusEXT(GL_DRAW_FRAMEBUFFER));
