...
void SetupUpdateES3UnpackParametersExpectations(::gl::MockGLInterface* gl, GLint row_length, GLint image_height)
{
    EXPECT_CALL(*gl, PixelStorei(GL_UNPACK_ROW_LENGTH, row_length)).Times(1).RetiresOnSaturation();
    EXPECT_CALL(*gl, PixelStorei(GL_UNPACK_IMAGE_HEIGHT, image_height)).Times(1).RetiresOnSaturation();
}
...
protected:
    ...
    void InitializeContextState(ContextState* state, uint32_t non_default_unit, uint32_t active_unit);
...
INSTANTIATE_TEST_SUITE_P(Service, GLES2DecoderRestoreStateTest, ::testing::Bool());

void GLES2DecoderRestoreStateTest::AddExpectationsForActiveTexture(GLenum unit)
{
    EXPECT_CALL(*gl_, ActiveTexture(unit)).Times(1).RetiresOnSaturation();
}

void GLES2DecoderRestoreStateTest::AddExpectationsForBindTexture(GLenum target, GLuint id)
{
    EXPECT_CALL(*gl_, BindTexture(target, id)).Times(1).RetiresOnSaturation();
}

void GLES2DecoderRestoreStateTest::InitializeContextState(ContextState* state, uint32_t non_default_unit, uint32_t active_unit)
{
    state->texture_units.resize(group().max_texture_units());
    for (uint32_t tt = 0; tt < state->texture_units.size(); ++tt) {
        TextureRef* ref_cube_map = group().texture_manager()->GetDefaultTextureInfo(GL_TEXTURE_CUBE_MAP);
        state->texture_units[tt].bound_texture_cube_map = ref_cube_map;
        TextureRef* ref_2d = (tt == non_default_unit) ? group().texture_manager()->GetTexture(client_texture_id_)
...
scoped_refptr<FeatureInfo> GLES2DecoderRestoreStateTest::SetupForES3Test()
{
    ...
    TestHelper::SetupFeatureInfoInitExpectationsWithGLVersion(gl_.get(), "", "", "OpenGL ES 3.0", CONTEXT_TYPE_OPENGLES3);
    feature_info->InitializeForTesting(CONTEXT_TYPE_OPENGLES3);
    return feature_info;
}

void GLES2DecoderRestoreStateTest::AddExpectationsForBindSampler(GLuint unit, GLuint id)
{
...
TEST_P(GLES2DecoderRestoreStateTest, NullPreviousStateBGR)
{
    ...
    AddExpectationsForBindTexture(GL_TEXTURE_CUBE_MAP, TestHelper::kServiceDefaultTextureCubemapId);
    ...
    for (uint32_t i = 1; i < group().max_texture_units(); ++i) {
        ...
        AddExpectationsForBindTexture(GL_TEXTURE_2D, TestHelper::kServiceDefaultTexture2dId);
        AddExpectationsForBindTexture(GL_TEXTURE_CUBE_MAP, TestHelper::kServiceDefaultTextureCubemapId);
    }
...
TEST_P(GLES2DecoderManualInitTest, ContextStateCapabilityCaching)
{
    ...
    TestInfo test[] = { { GL_BLEND, false, true }, { GL_CULL_FACE, false, true }, { GL_DEPTH_TEST, false, false }, { GL_DITHER, true, true },
        { GL_POLYGON_OFFSET_FILL, false, true }, { GL_SAMPLE_ALPHA_TO_COVERAGE, false, true }, { GL_SAMPLE_COVERAGE, false, true },
        { GL_SCISSOR_TEST, false, true }, { GL_STENCIL_TEST, false, false }, { 0, false, false } };
...
TEST_P(GLES3DecoderTest, ES3PixelStoreiWithPixelUnpackBuffer)
{
    ...
    EXPECT_CALL(*gl_, PixelStorei(GL_UNPACK_ROW_LENGTH, 16)).Times(1).RetiresOnSaturation();
