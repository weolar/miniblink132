...
{
    EXPECT_CALL(*gl_, BindBufferBase(GL_TRANSFORM_FEEDBACK_BUFFER, 2, kServiceBufferId));
...
{
    EXPECT_CALL(*gl_, BindBufferBase(GL_TRANSFORM_FEEDBACK_BUFFER, 2, kNewServiceId));
...
TEST_P(GLES3DecoderTest, BindBufferRangeValidArgs)
{
    ...
    EXPECT_CALL(*gl_, BindBufferRange(kTarget, 2, kServiceBufferId, kRangeOffset, kRangeSize));
...
TEST_P(GLES3DecoderTest, BindBufferRangeValidArgsWithLessData)
{
    ...
    DoBindBuffer(kTarget, client_buffer_id_, kServiceBufferId);
    DoBufferData(kTarget, kBufferSize);
    EXPECT_CALL(*gl_, BindBufferRange(kTarget, 2, kServiceBufferId, kRangeOffset, kRangeSize - 4));
    SpecializedSetup<cmds::BindBufferRange, 0>(true);
    cmds::BindBufferRange cmd;
    cmd.Init(kTarget, 2, client_buffer_id_, kRangeOffset, kRangeSize);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
...
TEST_P(GLES3DecoderTest, MapBufferRangeUnmapBufferReadSucceeds)
{
    ...
    { // MapBufferRange
        EXPECT_CALL(*gl_, MapBufferRange(kTarget, kOffset, kSize, kAccess)).WillOnce(Return(&data[0])).RetiresOnSaturation();

        auto* result = GetSharedMemoryAs<cmds::MapBufferRange::Result*>();

        cmds::MapBufferRange cmd;
        cmd.Init(kTarget, kOffset, kSize, kAccess, data_shm_id, data_shm_offset, result_shm_id, result_shm_offset);
        *result = 0;
        EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    ...
    { // UnmapBuffer
        EXPECT_CALL(*gl_, UnmapBuffer(kTarget)).WillOnce(Return(GL_TRUE)).RetiresOnSaturation();
...
TEST_P(GLES3DecoderTest, MapBufferRangeUnmapBufferWriteSucceeds)
{
    ...
    const int8_t* shadow_data = reinterpret_cast<const int8_t*>(buffer->GetRange(0, kTotalSize));
    ...
    { // MapBufferRange succeeds
        EXPECT_CALL(*gl_, MapBufferRange(kTarget, kOffset, kSize, kMappedAccess)).WillOnce(Return(&gpu_data[kOffset])).RetiresOnSaturation();
...
TEST_P(GLES3DecoderTest, FlushMappedBufferRangeSucceeds)
{
    ...
    { // MapBufferRange succeeds
        EXPECT_CALL(*gl_, MapBufferRange(kTarget, kMappedOffset, kMappedSize, kMappedAccess)).WillOnce(Return(&gpu_data[kMappedOffset])).RetiresOnSaturation();

        cmds::MapBufferRange cmd;
        cmd.Init(kTarget, kMappedOffset, kMappedSize, kAccess, data_shm_id, data_shm_offset, result_shm_id, result_shm_offset);
        *result = 0;
        EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
        EXPECT_EQ(1u, *result);
        // Verify the buffer range from GPU is copied to client mem.
    ...
    { // FlushMappedBufferRange succeeds
        EXPECT_CALL(*gl_, FlushMappedBufferRange(kTarget, kFlushRangeOffset, kFlushRangeSize)).Times(1).RetiresOnSaturation();
        ...
        // the flushed range.
        for (GLsizeiptr ii = 0; ii < kTotalSize; ++ii) {
            if (ii >= kMappedOffset + kFlushRangeOffset && ii < kMappedOffset + kFlushRangeOffset + kFlushRangeSize) {
                EXPECT_EQ(kValue0, gpu_data[ii]);
                EXPECT_EQ(kValue0, shadow_data[ii]);
            } else {
                EXPECT_EQ(static_cast<int8_t>(ii % 128), gpu_data[ii]);
...
TEST_P(GLES3DecoderTest, MapBufferRangeWriteInvalidateBufferSucceeds)
{
    ...
    EXPECT_CALL(*gl_, MapBufferRange(kTarget, kOffset, kSize, kFilteredAccess)).WillOnce(Return(&data[0])).RetiresOnSaturation();
...
TEST_P(GLES3DecoderTest, MapBufferRangeBadSharedMemoryFails)
{
    ...
    *result = 0;
    ...
    cmds::MapBufferRange cmd;
    cmd.Init(kTarget, kOffset, kSize, kAccess, kInvalidSharedMemoryId, data_shm_offset, result_shm_id, result_shm_offset);
    EXPECT_NE(error::kNoError, ExecuteCmd(cmd));
    cmd.Init(kTarget, kOffset, kSize, kAccess, data_shm_id, data_shm_offset, kInvalidSharedMemoryId, result_shm_offset);
    EXPECT_NE(error::kNoError, ExecuteCmd(cmd));
    cmd.Init(kTarget, kOffset, kSize, kAccess, data_shm_id, kInvalidSharedMemoryOffset, result_shm_id, result_shm_offset);
    EXPECT_NE(error::kNoError, ExecuteCmd(cmd));
    cmd.Init(kTarget, kOffset, kSize, kAccess, data_shm_id, data_shm_offset, result_shm_id, kInvalidSharedMemoryOffset);
    EXPECT_NE(error::kNoError, ExecuteCmd(cmd));
}
...
TEST_P(GLES3DecoderTest, CopyBufferSubDataValidArgs)
{
    ...
    Buffer* buffer = GetBuffer(client_buffer_id_);
    EXPECT_TRUE(buffer);
    const char* shadow_data = reinterpret_cast<const char*>(buffer->GetRange(0, kSize));
    EXPECT_TRUE(shadow_data);
    // Verify the shadow data is initialized.
    ...
    EXPECT_CALL(*gl_, CopyBufferSubData(kTarget, kTarget, kReadOffset, kWriteOffset, kCopySize));
