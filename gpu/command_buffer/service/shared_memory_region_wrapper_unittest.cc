...
gfx::GpuMemoryBufferHandle CreateGpuMemoryBuffer(const gfx::Size& size, gfx::BufferFormat format)
{
    return GpuMemoryBufferImplSharedMemory::CreateGpuMemoryBuffer(gfx::GpuMemoryBufferId { 1 }, size, format, gfx::BufferUsage::SCANOUT_CPU_READ_WRITE);
}
...
TEST(SharedMemoryRegionWrapperTest, BufferTooSmallWrongSize)
{
    ...
    SharedMemoryRegionWrapper wrapper;
    EXPECT_FALSE(wrapper.Initialize(std::move(handle), gfx::Size(200, 200), buffer_format));
}
