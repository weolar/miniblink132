...
void ContextState::InitCapabilities(const ContextState* prev_state) const
{
    if (prev_state) {
        ...
        if (prev_state->enable_flags.cached_cull_face != enable_flags.cached_cull_face) {
            EnableDisable(GL_CULL_FACE, enable_flags.cached_cull_face);
        }
        if (prev_state->enable_flags.cached_depth_test != enable_flags.cached_depth_test) {
        ...
        if (prev_state->enable_flags.cached_polygon_offset_fill != enable_flags.cached_polygon_offset_fill) {
            EnableDisable(GL_POLYGON_OFFSET_FILL, enable_flags.cached_polygon_offset_fill);
        }
        if (prev_state->enable_flags.cached_sample_alpha_to_coverage != enable_flags.cached_sample_alpha_to_coverage) {
            EnableDisable(GL_SAMPLE_ALPHA_TO_COVERAGE, enable_flags.cached_sample_alpha_to_coverage);
        }
        if (prev_state->enable_flags.cached_sample_coverage != enable_flags.cached_sample_coverage) {
            EnableDisable(GL_SAMPLE_COVERAGE, enable_flags.cached_sample_coverage);
        }
        if (prev_state->enable_flags.cached_scissor_test != enable_flags.cached_scissor_test) {
            EnableDisable(GL_SCISSOR_TEST, enable_flags.cached_scissor_test);
        }
        if (prev_state->enable_flags.cached_stencil_test != enable_flags.cached_stencil_test) {
        ...
        if (feature_info_->feature_flags().ext_multisample_compatibility) {
            if (prev_state->enable_flags.cached_multisample_ext != enable_flags.cached_multisample_ext) {
                EnableDisable(GL_MULTISAMPLE_EXT, enable_flags.cached_multisample_ext);
            }
        }
        if (feature_info_->feature_flags().ext_multisample_compatibility) {
            if (prev_state->enable_flags.cached_sample_alpha_to_one_ext != enable_flags.cached_sample_alpha_to_one_ext) {
                EnableDisable(GL_SAMPLE_ALPHA_TO_ONE_EXT, enable_flags.cached_sample_alpha_to_one_ext);
            }
        }
        if (feature_info_->IsES3Capable()) {
            if (prev_state->enable_flags.cached_rasterizer_discard != enable_flags.cached_rasterizer_discard) {
                EnableDisable(GL_RASTERIZER_DISCARD, enable_flags.cached_rasterizer_discard);
            }
            if (prev_state->enable_flags.cached_primitive_restart_fixed_index != enable_flags.cached_primitive_restart_fixed_index) {
                EnableDisable(GL_PRIMITIVE_RESTART_FIXED_INDEX, enable_flags.cached_primitive_restart_fixed_index);
            }
        }
    } else {
        EnableDisable(GL_BLEND, enable_flags.cached_blend);
...
{
    if (prev_state) {
        if ((blend_color_red != prev_state->blend_color_red) || (blend_color_green != prev_state->blend_color_green)
            || (blend_color_blue != prev_state->blend_color_blue) || (blend_color_alpha != prev_state->blend_color_alpha))
            api()->glBlendColorFn(blend_color_red, blend_color_green, blend_color_blue, blend_color_alpha);
        if ((blend_equation_rgb != prev_state->blend_equation_rgb) || (blend_equation_alpha != prev_state->blend_equation_alpha))
            api()->glBlendEquationSeparateFn(blend_equation_rgb, blend_equation_alpha);
        if ((blend_source_rgb != prev_state->blend_source_rgb) || (blend_dest_rgb != prev_state->blend_dest_rgb)
            || (blend_source_alpha != prev_state->blend_source_alpha) || (blend_dest_alpha != prev_state->blend_dest_alpha))
            api()->glBlendFuncSeparateFn(blend_source_rgb, blend_dest_rgb, blend_source_alpha, blend_dest_alpha);
        if ((color_clear_red != prev_state->color_clear_red) || (color_clear_green != prev_state->color_clear_green)
            || (color_clear_blue != prev_state->color_clear_blue) || (color_clear_alpha != prev_state->color_clear_alpha))
            api()->glClearColorFn(color_clear_red, color_clear_green, color_clear_blue, color_clear_alpha);
        ...
        if ((cached_color_mask_red != prev_state->cached_color_mask_red) || (cached_color_mask_green != prev_state->cached_color_mask_green)
            || (cached_color_mask_blue != prev_state->cached_color_mask_blue) || (cached_color_mask_alpha != prev_state->cached_color_mask_alpha))
            api()->glColorMaskFn(cached_color_mask_red, cached_color_mask_green, cached_color_mask_blue, cached_color_mask_alpha);
        ...
        if (feature_info_->feature_flags().oes_standard_derivatives) {
            if (prev_state->hint_fragment_shader_derivative != hint_fragment_shader_derivative) {
                api()->glHintFn(GL_FRAGMENT_SHADER_DERIVATIVE_HINT_OES, hint_fragment_shader_derivative);
            }
        }
        ...
        if ((polygon_offset_factor != prev_state->polygon_offset_factor) || (polygon_offset_units != prev_state->polygon_offset_units))
            api()->glPolygonOffsetFn(polygon_offset_factor, polygon_offset_units);
        if ((sample_coverage_value != prev_state->sample_coverage_value) || (sample_coverage_invert != prev_state->sample_coverage_invert))
            api()->glSampleCoverageFn(sample_coverage_value, sample_coverage_invert);
        if ((scissor_x != prev_state->scissor_x) || (scissor_y != prev_state->scissor_y) || (scissor_width != prev_state->scissor_width)
            || (scissor_height != prev_state->scissor_height))
            api()->glScissorFn(scissor_x, scissor_y, scissor_width, scissor_height);
        if ((stencil_front_func != prev_state->stencil_front_func) || (stencil_front_ref != prev_state->stencil_front_ref)
            || (stencil_front_mask != prev_state->stencil_front_mask))
            api()->glStencilFuncSeparateFn(GL_FRONT, stencil_front_func, stencil_front_ref, stencil_front_mask);
        if ((stencil_back_func != prev_state->stencil_back_func) || (stencil_back_ref != prev_state->stencil_back_ref)
            || (stencil_back_mask != prev_state->stencil_back_mask))
            api()->glStencilFuncSeparateFn(GL_BACK, stencil_back_func, stencil_back_ref, stencil_back_mask);
        if ((cached_stencil_front_writemask != prev_state->cached_stencil_front_writemask))
            api()->glStencilMaskSeparateFn(GL_FRONT, cached_stencil_front_writemask);
        if ((cached_stencil_back_writemask != prev_state->cached_stencil_back_writemask))
            api()->glStencilMaskSeparateFn(GL_BACK, cached_stencil_back_writemask);
        if ((stencil_front_fail_op != prev_state->stencil_front_fail_op) || (stencil_front_z_fail_op != prev_state->stencil_front_z_fail_op)
            || (stencil_front_z_pass_op != prev_state->stencil_front_z_pass_op))
            api()->glStencilOpSeparateFn(GL_FRONT, stencil_front_fail_op, stencil_front_z_fail_op, stencil_front_z_pass_op);
        if ((stencil_back_fail_op != prev_state->stencil_back_fail_op) || (stencil_back_z_fail_op != prev_state->stencil_back_z_fail_op)
            || (stencil_back_z_pass_op != prev_state->stencil_back_z_pass_op))
            api()->glStencilOpSeparateFn(GL_BACK, stencil_back_fail_op, stencil_back_z_fail_op, stencil_back_z_pass_op);
        if ((viewport_x != prev_state->viewport_x) || (viewport_y != prev_state->viewport_y) || (viewport_width != prev_state->viewport_width)
            || (viewport_height != prev_state->viewport_height))
            api()->glViewportFn(viewport_x, viewport_y, viewport_width, viewport_height);
    } else {
        api()->glBlendColorFn(blend_color_red, blend_color_green, blend_color_blue, blend_color_alpha);
        api()->glBlendEquationSeparateFn(blend_equation_rgb, blend_equation_alpha);
        api()->glBlendFuncSeparateFn(blend_source_rgb, blend_dest_rgb, blend_source_alpha, blend_dest_alpha);
...
bool ContextState::GetStateAsGLint(GLenum pname, GLint* params, GLsizei* num_written) const
{
    switch (pname) {
    case GL_BLEND_COLOR:
        *num_written = 4;
    ...
    case GL_PRIMITIVE_RESTART_FIXED_INDEX:
        *num_written = 1;
        if (params) {
            params[0] = static_cast<GLint>(enable_flags.primitive_restart_fixed_index);
        }
        return true;
    case GL_MULTISAMPLE_EXT:
        *num_written = 1;
...
bool ContextState::GetStateAsGLfloat(GLenum pname, GLfloat* params, GLsizei* num_written) const
{
    switch (pname) {
    case GL_BLEND_COLOR:
        *num_written = 4;
    ...
    case GL_PRIMITIVE_RESTART_FIXED_INDEX:
        *num_written = 1;
        if (params) {
            params[0] = static_cast<GLfloat>(enable_flags.primitive_restart_fixed_index);
        }
        return true;
    case GL_MULTISAMPLE_EXT:
        *num_written = 1;
