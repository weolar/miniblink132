...
TEST_F(ShaderManagerTest, Basic)
{
    ...
    Shader* info0 = manager_.CreateShader(kClient1Id, kService1Id, kShader1Type);
    ...
    EXPECT_CALL(*gl_, DeleteShader(kService1Id)).Times(1).RetiresOnSaturation();
...
TEST_F(ShaderManagerTest, Destroy)
{
    ...
    const GLenum kShader1Type = GL_VERTEX_SHADER;
    // Check we can create shader.
    Shader* shader1 = manager_.CreateShader(kClient1Id, kService1Id, kShader1Type);
    // Check shader got created.
...
TEST_F(ShaderManagerTest, DeleteBug)
{
    ...
    // Check we can create shader.
    scoped_refptr<Shader> shader1(manager_.CreateShader(kClient1Id, kService1Id, kShaderType));
    scoped_refptr<Shader> shader2(manager_.CreateShader(kClient2Id, kService2Id, kShaderType));
    ASSERT_TRUE(shader1.get());
    ASSERT_TRUE(shader2.get());
    manager_.UseShader(shader1.get());
    manager_.Delete(shader1.get());

    EXPECT_CALL(*gl_, DeleteShader(kService2Id)).Times(1).RetiresOnSaturation();
...
TEST_F(ShaderManagerTest, DoCompile)
{
    ...
    attrib_map[kAttrib2Name] = TestHelper::ConstructAttribute(kAttrib2Type, kAttrib2Size, kAttrib2Precision, kAttribStaticUse, kAttrib2Name);
    ...
    uniform_map[kUniform2Name] = TestHelper::ConstructUniform(kUniform2Type, kUniform2Size, kUniform2Precision, kUniform2StaticUse, kUniform2Name);
    VaryingMap varying_map;
    varying_map[kVarying1Name] = TestHelper::ConstructVarying(kVarying1Type, kVarying1Size, kVarying1Precision, kVarying1StaticUse, kVarying1Name);
    ...
    interface_block1_fields.push_back(TestHelper::ConstructInterfaceBlockField(
        kInterfaceBlock1Field1Type, kInterfaceBlock1Field1Size, kInterfaceBlock1Field1Precision, kInterfaceBlock1Field1StaticUse, kInterfaceBlock1Field1Name));
    interface_block_map[kInterfaceBlock1Name] = TestHelper::ConstructInterfaceBlock(kInterfaceBlock1Size, kInterfaceBlock1Layout, kInterfaceBlock1RowMajor,
        kInterfaceBlock1StaticUse, kInterfaceBlock1Name, kInterfaceBlock1InstanceName, interface_block1_fields);

    TestHelper::SetShaderStates(gl_.get(), shader1, true, &kLog, &kTranslatedSource, nullptr, &attrib_map, &uniform_map, &varying_map, &interface_block_map,
    ...
    for (AttributeMap::const_iterator it = attrib_map.begin(); it != attrib_map.end(); ++it) {
        ...
        EXPECT_STREQ(it->second.name.c_str(), shader1->GetOriginalNameFromHashedName(it->first)->c_str());
    }
    // Check uniform infos got copied.
    EXPECT_EQ(uniform_map.size(), shader1->uniform_map().size());
    for (UniformMap::const_iterator it = uniform_map.begin(); it != uniform_map.end(); ++it) {
    ...
    for (VaryingMap::const_iterator it = varying_map.begin(); it != varying_map.end(); ++it) {
    ...
    for (const auto& it : interface_block_map) {
        const sh::InterfaceBlock* block_info = shader1->GetInterfaceBlockInfo(it.first);
        ...
        EXPECT_STREQ(it.second.name.c_str(), shader1->GetOriginalNameFromHashedName(it.first)->c_str());
        EXPECT_STREQ(it.second.instanceName.c_str(), block_info->instanceName.c_str());
    }
    // Check interface block field infos got copied.
    const sh::InterfaceBlock* interface_block1_info = shader1->GetInterfaceBlockInfo(kInterfaceBlock1Name);
    EXPECT_EQ(interface_block1_fields.size(), interface_block1_info->fields.size());
    ...
    EXPECT_EQ(output_variable_list.size(), shader1->output_variable_list().size());
    for (auto it = output_variable_list.begin(); it != output_variable_list.end(); ++it) {
        const sh::OutputVariable* variable_info = shader1->GetOutputVariableInfo(it->mappedName);
        ASSERT_TRUE(variable_info != nullptr);
        EXPECT_EQ(it->type, variable_info->type);
        EXPECT_EQ(it->getOutermostArraySize(), variable_info->getOutermostArraySize());
        ...
        EXPECT_STREQ(it->name.c_str(), shader1->GetOriginalNameFromHashedName(it->mappedName)->c_str());
    }

    // Compile failure case.
    TestHelper::SetShaderStates(
        gl_.get(), shader1, false, &kLog, &kTranslatedSource, nullptr, &attrib_map, &uniform_map, &varying_map, nullptr, &output_variable_list, nullptr);
    EXPECT_FALSE(shader1->valid());
