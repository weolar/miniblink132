...
public:
    explicit AsyncAPIMock(bool default_do_commands, CommandBufferDirect* command_buffer, CommandBufferServiceBase* command_buffer_service);
    ~AsyncAPIMock() override;

    error::Error FakeDoCommands(unsigned int num_commands, const volatile void* buffer, int num_entries, int* entries_processed);
    ...
    public:
        IsArgs(unsigned int arg_count, const volatile void* args)
            ...
            , args_(static_cast<volatile CommandBufferEntry*>(const_cast<volatile void*>(args)))
        {
        }

        bool operator()(const volatile void* _args) const
        {
            const volatile CommandBufferEntry* args = static_cast<const volatile CommandBufferEntry*>(_args) + 1;
            for (unsigned int i = 0; i < arg_count_; ++i) {
                if (args[i].value_uint32 != args_[i].value_uint32)
                    return false;
            }
            return true;
        }
    ...
    private:
        ...
        raw_ptr<volatile CommandBufferEntry, DanglingUntriaged | AllowPtrArithmetic> args_;
    };
    ...
    MOCK_METHOD3(DoCommand, error::Error(unsigned int command, unsigned int arg_count, const volatile void* cmd_data));

    MOCK_METHOD4(DoCommands, error::Error(unsigned int num_commands, const volatile void* buffer, int num_entries, int* entries_processed));
    ...
    void SetToken(unsigned int command, unsigned int arg_count, const volatile void* _args);
...
public:
    ...
    MOCK_METHOD(void, CacheBlob, (gpu::GpuDiskCacheType type, const std::string& key, const std::string& shader));
...
public:
    ...
    MOCK_METHOD(void, GetIsolationKey, (const blink::WebGPUExecutionContextToken& token, GetIsolationKeyCallback cb), (override));
};
...
public:
    ...
    MOCK_METHOD6(Init,
        bool(sh::GLenum shader_type, ShShaderSpec shader_spec, const ShBuiltInResources* resources, ShShaderOutput shader_output_language,
            const ShCompileOptions& driver_bug_workarounds, bool gl_shader_interm_output));
    MOCK_CONST_METHOD9(Translate,
        bool(const std::string& shader_source, std::string* info_log, std::string* translated_source, int* shader_version, AttributeMap* attrib_map,
            UniformMap* uniform_map, VaryingMap* varying_map, InterfaceBlockMap* interface_block_map, OutputVariableList* output_variable_list));
    MOCK_CONST_METHOD0(GetStringForOptionsThatWouldAffectCompilation, OptionsAffectingCompilationString*());
...
public:
    ...
    MOCK_METHOD7(LoadLinkedProgram,
        ProgramLoadResult(GLuint program, Shader* shader_a, Shader* shader_b, const LocationMap* bind_attrib_location_map,
    ...
    MOCK_METHOD7(SaveLinkedProgram,
        void(GLuint program, const Shader* shader_a, const Shader* shader_b, const LocationMap* bind_attrib_location_map,
            const std::vector<std::string>& transform_feedback_varyings, GLenum transform_feedback_buffer_mode, DecoderClient* client));
