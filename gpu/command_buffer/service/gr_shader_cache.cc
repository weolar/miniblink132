...
GrShaderCache::GrShaderCache(size_t max_cache_size_bytes, Client* client)
    ...
    , client_(client)
    , enable_vk_pipeline_cache_(base::FeatureList::IsEnabled(features::kEnableVkPipelineCache))
{
    if (base::SingleThreadTaskRunner::HasCurrentDefault()) {
        base::trace_event::MemoryDumpManager::GetInstance()->RegisterDumpProvider(this, "GrShaderCache", base::SingleThreadTaskRunner::GetCurrentDefault());
    }
}
...
sk_sp<SkData> GrShaderCache::load(const SkData& key)
{
    ...
    UMA_HISTOGRAM_BOOLEAN("Gpu.GrShaderCacheLoadHitInCache", (it != store_.end()));
...
void GrShaderCache::PopulateCache(const std::string& key, const std::string& data)
{
...
GrShaderCache::Store::iterator GrShaderCache::AddToCache(CacheKey key, CacheData data)
{
...
template <typename Iterator> void GrShaderCache::EraseFromCache(Iterator it)
{
    lock_.AssertAcquired();
...
void GrShaderCache::PurgeMemory(base::MemoryPressureListener::MemoryPressureLevel memory_pressure_level)
{
    base::AutoLock auto_lock(lock_);
...
bool GrShaderCache::OnMemoryDump(const base::trace_event::MemoryDumpArgs& args, base::trace_event::ProcessMemoryDump* pmd)
{
    base::AutoLock auto_lock(lock_);
    using base::trace_event::MemoryAllocatorDump;
    std::string dump_name = base::StringPrintf("gpu/gr_shader_cache/cache_0x%" PRIXPTR, reinterpret_cast<uintptr_t>(this));
    MemoryAllocatorDump* dump = pmd->CreateAllocatorDump(dump_name);
    dump->AddScalar(MemoryAllocatorDump::kNameSize, MemoryAllocatorDump::kUnitsBytes, curr_size_bytes_);
...
GrShaderCache::ScopedCacheUse::ScopedCacheUse(GrShaderCache* cache, int32_t client_id)
    : cache_(cache)
{
...
GrShaderCache::CacheKey& GrShaderCache::CacheKey::operator=(const CacheKey& other) = default;
GrShaderCache::CacheKey& GrShaderCache::CacheKey::operator=(CacheKey&& other) = default;
...
GrShaderCache::CacheData& GrShaderCache::CacheData::operator=(CacheData&& other) = default;
