...
AbstractTextureImpl::AbstractTextureImpl(
    GLenum target, GLenum internal_format, GLsizei width, GLsizei height, GLsizei depth, GLint border, GLenum format, GLenum type)
{
    ...
    texture_->SetLevelInfo(target, 0, internal_format, width, height, depth, border, format, type, cleared_rect);
...
void AbstractTextureImpl::BindStreamTextureImage(gl::GLImage* image, GLuint service_id)
{
    const GLint level = 0;
    const GLuint target = texture_->target();
    texture_->SetLevelStreamTextureImage(target, level, image, Texture::ImageState::UNBOUND, service_id);
    ...
    GLenum target, GLenum internal_format, GLsizei width, GLsizei height, GLsizei depth, GLint border, GLenum format, GLenum type)
{
    ...
    texture_ = new TexturePassthrough(service_id, target, internal_format, width, height, depth, border, format, type);
}
...
void AbstractTextureImplPassthrough::BindStreamTextureImage(gl::GLImage* image, GLuint service_id)
{
    const GLint level = 0;
    const GLuint target = texture_->target();
...
void AbstractTextureImplPassthrough::BindImage(gl::GLImage* image, bool client_managed)
{
    NOTIMPLEMENTED();
}
