...
public:
    ...
    explicit GpuTaskSchedulerHelper(std::unique_ptr<SingleTaskSequence> task_sequence);
    // This constructor is used for command buffer GLOutputSurface.
    explicit GpuTaskSchedulerHelper(CommandBufferTaskExecutor* command_buffer_task_executor);
    ...
    void ScheduleGpuTask(
        base::OnceClosure task, std::vector<SyncToken> sync_token_fences, const SyncToken& release, ReportingCallback report_callback = ReportingCallback());

    // This is only called with SkiaOutputSurface, no need to flush command buffer
    // here.
    void ScheduleOrRetainGpuTask(base::OnceClosure task, std::vector<SyncToken> sync_token_fences);
