...
protected:
    ...
    base::RepeatingClosure CreateTimerCallback()
    {
        return base::BindLambdaForTesting([this]() {
            CHECK(task_environment_.GetMainThreadTaskRunner()->BelongsToCurrentThread());
            timer_fired_count_++;
        });
    }
    ...
    void PostToThreadPoolAndWait(base::OnceClosure task)
    {
        ...
        base::ThreadPool::PostTaskAndReply(FROM_HERE, std::move(task), run_loop.QuitClosure());
        run_loop.Run();
    }
...
{
    scoped_refptr<RetainingOneShotTimerHolder> timer(new RetainingOneShotTimerHolder(
        /*max_delay=*/base::Milliseconds(100),
        /*min_delay=*/base::Milliseconds(50), task_environment_.GetMainThreadTaskRunner(), CreateTimerCallback()));
...
TEST_F(RetainingOneShotTimerHolderTest, MultiThreadUsage)
{
    ...
    PostToThreadPoolAndWait(base::BindLambdaForTesting([timer]() { timer->ResetTimerIfNecessary(); }));
    ...
    PostToThreadPoolAndWait(base::BindLambdaForTesting([timer]() { timer->DestroyTimer(); }));
