...
TEST_F(GLES3DecoderPassthroughTest, BindBufferRangeValidArgs)
{
    ...
    bind_buffer_range_cmd.Init(kTarget, 2, kClientBufferId, kRangeOffset, kRangeSize);
...
TEST_F(GLES3DecoderPassthroughTest, MapBufferRangeUnmapBufferWriteSucceeds)
{
    ...
    { // MapBufferRange succeeds
        ...
        *result = 0;
        ...
        auto mapped_buffer_info_iter = passthrough_resources->mapped_buffer_map.find(kClientBufferId);
        EXPECT_NE(mapped_buffer_info_iter, passthrough_resources->mapped_buffer_map.end());
    ...
    { // Re-map the buffer to verify the data
        ...
        cmds::MapBufferRange cmd;
        cmd.Init(kTarget, 0, kTotalSize, kReadAccess, data_shm_id, data_shm_offset, result_shm_id, result_shm_offset);
        *result = 0;
        EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
        EXPECT_EQ(1u, *result);
...
TEST_F(GLES3DecoderPassthroughTest, MapBufferRangeWriteInvalidateRangeSucceeds)
{
    const GLenum kTarget = GL_ARRAY_BUFFER;
    const GLintptr kOffset = 10;
    const GLsizeiptr kSize = 64;
...
TEST_F(GLES3DecoderPassthroughTest, MapBufferRangeWriteInvalidateBufferSucceeds)
{
...
TEST_F(GLES3DecoderPassthroughTest, MapBufferRangeWithError)
{
    ...
    *result = 0;
    ...
    PassthroughResources* passthrough_resources = GetPassthroughResources();
    auto mapped_buffer_info_iter = passthrough_resources->mapped_buffer_map.find(kClientBufferId);
    EXPECT_EQ(mapped_buffer_info_iter, passthrough_resources->mapped_buffer_map.end());
}
...
TEST_F(GLES3DecoderPassthroughTest, BufferDataDestroysDataStore)
{
    ...
    { // Buffer is no longer tracked as mapped
        PassthroughResources* passthrough_resources = GetPassthroughResources();
        EXPECT_EQ(passthrough_resources->mapped_buffer_map.find(kClientBufferId), passthrough_resources->mapped_buffer_map.end());
    }
