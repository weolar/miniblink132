...
public:
    ...
    FeatureInfo(const GpuDriverBugWorkarounds& gpu_driver_bug_workarounds, const GpuFeatureInfo& gpu_feature_info);

    FeatureInfo(const FeatureInfo&) = delete;
    FeatureInfo& operator=(const FeatureInfo&) = delete;

    // Initializes the feature information. Needs a current GL context.
    void Initialize(ContextType context_type, bool is_passthrough_cmd_decoder, const DisallowedFeatures& disallowed_features, bool force_reinitialize = false);
    ...
    const gfx::ExtensionSet& extensions() const
    {
        return extensions_;
    }
    ...
    bool disable_shader_translator() const
    {
        return disable_shader_translator_;
    }
