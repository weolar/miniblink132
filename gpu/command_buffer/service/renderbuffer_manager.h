...
public:
    Renderbuffer(RenderbufferManager* manager, GLuint client_id, GLuint service_id);
    ...
    bool RegenerateAndBindBackingObjectIfNeeded(const GpuDriverBugWorkarounds& workarounds);

    void AddFramebufferAttachmentPoint(Framebuffer* framebuffer, GLenum attachment);
    void RemoveFramebufferAttachmentPoint(Framebuffer* framebuffer, GLenum attachment);
...
private:
    ...
    void SetInfoAndInvalidate(GLsizei samples, GLenum internalformat, GLsizei width, GLsizei height);
    ...
    base::flat_set<std::pair<Framebuffer*, GLenum>> framebuffer_attachment_points_;
};

// This class keeps track of the renderbuffers and whether or not they have
// been cleared.
class GPU_GLES2_EXPORT RenderbufferManager : public base::trace_event::MemoryDumpProvider {
public:
    RenderbufferManager(MemoryTracker* memory_tracker, GLint max_renderbuffer_size, GLint max_samples, FeatureInfo* feature_info);
    ...
    void SetInfoAndInvalidate(Renderbuffer* renderbuffer, GLsizei samples, GLenum internalformat, GLsizei width, GLsizei height);
    ...
    bool ComputeEstimatedRenderbufferSize(int width, int height, int samples, int internal_format, uint32_t* size) const;
...
private:
    ...
    typedef std::unordered_map<GLuint, scoped_refptr<Renderbuffer>> RenderbufferMap;
