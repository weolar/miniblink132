...
GLuint ImportSemaphoreHandleToGLSemaphore(SemaphoreHandle handle)
{
    ...
    if (handle.vk_handle_type() != VK_EXTERNAL_SEMAPHORE_HANDLE_TYPE_OPAQUE_FD_BIT) {
    ...
    api->glImportSemaphoreFdEXTFn(gl_semaphore, GL_HANDLE_TYPE_OPAQUE_FD_EXT, fd.release());

    return gl_semaphore;
#elif BUILDFLAG(IS_WIN)
    if (handle.vk_handle_type() != VK_EXTERNAL_SEMAPHORE_HANDLE_TYPE_OPAQUE_WIN32_BIT) {
        DLOG(ERROR) << "Importing semaphore handle of unexpected type:" << handle.vk_handle_type();
        return 0;
    }
    ...
    GLuint gl_semaphore;
    api->glGenSemaphoresEXTFn(1, &gl_semaphore);
    api->glImportSemaphoreWin32HandleEXTFn(gl_semaphore, GL_HANDLE_TYPE_OPAQUE_WIN32_EXT, win32_handle.Take());

    return gl_semaphore;
#elif BUILDFLAG(IS_FUCHSIA)
    if (handle.vk_handle_type() != VK_EXTERNAL_SEMAPHORE_HANDLE_TYPE_ZIRCON_EVENT_BIT_FUCHSIA) {
        DLOG(ERROR) << "Importing semaphore handle of unexpected type:" << handle.vk_handle_type();
        return 0;
    }
    ...
    GLuint gl_semaphore;
    api->glGenSemaphoresEXTFn(1, &gl_semaphore);
    api->glImportSemaphoreZirconHandleANGLEFn(gl_semaphore, GL_HANDLE_TYPE_ZIRCON_EVENT_ANGLE, event.release());
    return gl_semaphore;
#else
...
// static
ExternalSemaphore ExternalSemaphore::Create(viz::VulkanContextProvider* context_provider)
{
    ...
    return ExternalSemaphore(base::PassKey<ExternalSemaphore>(), context_provider, semaphore, std::move(handle));
}

// static
ExternalSemaphore ExternalSemaphore::CreateFromHandle(viz::VulkanContextProvider* context_provider, SemaphoreHandle handle)
{
    if (!handle.is_valid())
        return {};
    ...
    VkSemaphore semaphore = implementation->ImportSemaphoreHandle(device, handle.Duplicate());
    if (semaphore == VK_NULL_HANDLE)
        return {};
...
ExternalSemaphore::ExternalSemaphore(
    base::PassKey<ExternalSemaphore>, viz::VulkanContextProvider* context_provider, VkSemaphore semaphore, SemaphoreHandle handle)
...
VkSemaphore ExternalSemaphore::GetVkSemaphore()
{
    ...
    if (semaphore_ == VK_NULL_HANDLE) {
        ...
        VkDevice device = context_provider_->GetDeviceQueue()->GetVulkanDevice();
        semaphore_ = implementation->ImportSemaphoreHandle(device, handle_.Duplicate());
    }
    return semaphore_;
}
