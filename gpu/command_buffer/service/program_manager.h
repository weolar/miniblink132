...
public:
    ...
    struct ProgramOutputInfo {
        ProgramOutputInfo(GLuint _color_name, GLuint _index, const std::string& _name)
    ...
    struct UniformInfo {
        ...
        UniformInfo(const std::string& client_name, GLint client_location_base, GLenum _type, bool _is_array, const std::vector<GLint>& service_locations);
        ~UniformInfo();
    ...
    struct VertexAttrib {
        VertexAttrib(GLsizei size, GLenum type, const std::string& name, GLint location, size_t location_count)
            : size(size)
            , type(type)
            , location(location)
    ...
    template <typename T> class ShaderVariableLocationEntry {
    public:
        ...
        bool IsUnused() const
        {
            return !shader_variable_ && !inactive_;
        }
        ...
        bool IsActive() const
        {
            return shader_variable_ != nullptr;
        }
        void SetInactive()
        {
            shader_variable_ = nullptr;
    ...
    typedef std::vector<ShaderVariableLocationEntry<UniformInfo>> UniformLocationVector;
    ...
    const std::string* GetAttribMappedName(const std::string& original_name) const;

    // If the original name is not found, return nullptr.
    const std::string* GetUniformMappedName(const std::string& original_name) const;
    ...
    const std::string* GetOriginalNameFromHashedName(const std::string& hashed_name) const;

    // If the hashed name is not found, return nullptr.
    const sh::Varying* GetVaryingInfo(const std::string& hashed_name) const;

    // If the hashed name is not found, return nullptr.
    const sh::InterfaceBlock* GetInterfaceBlockInfo(const std::string& hashed_name) const;
    ...
    const UniformInfo* GetUniformInfoByFakeLocation(GLint fake_location, GLint* real_location, GLint* array_index) const;
    ...
    const ProgramOutputInfo* GetProgramOutputInfo(const std::string& original_name) const;

    // Gets all the program info.
    void GetProgramInfo(ProgramManager* manager, CommonDecoder::Bucket* bucket) const;
    ...
    bool SetSamplers(GLint num_texture_units, GLint fake_location, GLsizei count, const GLint* value);
    ...
    bool Link(ShaderManager* manager, DecoderClient* client);
    ...
    void SetProgramOutputLocationIndexedBinding(const std::string& name, GLuint colorName, GLuint index);
    ...
    void TransformFeedbackVaryings(GLsizei count, const char* const* varyings, GLenum buffer_mode);
    ...
    const std::vector<std::string>& effective_transform_feedback_varyings() const
    {
    ...
    GLint draw_id_uniform_location() const
    {
        return draw_id_uniform_location_;
    }
...
private:
    ...
    void GetVertexAttribData(const std::string& name, std::string* original_name, GLenum* type) const;

    void DetachShaders(ShaderManager* manager);

    static inline size_t GetUniformLocationIndexFromFakeLocation(GLint fake_location)
    {
        return static_cast<size_t>(fake_location & 0xFFFF);
    }

    static inline size_t GetArrayElementIndexFromFakeLocation(GLint fake_location)
    {
...
public:
    ProgramManager(ProgramCache* program_cache, uint32_t max_varying_vectors, uint32_t max_draw_buffers, uint32_t max_dual_source_draw_buffers,
    ...
    uint32_t max_varying_vectors() const
    {
        return max_varying_vectors_;
    }

    uint32_t max_draw_buffers() const
    {
    ...
    uint32_t max_vertex_attribs() const
    {
...
private:
    ...
    void RemoveProgramInfoIfUnused(ShaderManager* shader_manager, Program* program);
