...
SchedulerTaskRunner::SchedulerTaskRunner(Scheduler& scheduler, SequenceId sequence_id)
...
bool SchedulerTaskRunner::PostDelayedTask(const base::Location& from_here, base::OnceClosure task, base::TimeDelta delay)
{
    return PostNonNestableDelayedTask(from_here, std::move(task), delay);
}

bool SchedulerTaskRunner::PostNonNestableDelayedTask(const base::Location& from_here, base::OnceClosure task, base::TimeDelta delay)
{
    base::AutoLock lock(lock_);
    if (!is_running_)
        return false;
    ...
    CHECK(delay.is_zero());
    scheduler_->ScheduleTask(Scheduler::Task(sequence_id_, base::BindOnce(&SchedulerTaskRunner::RunTask, this, std::move(task)), std::vector<SyncToken>()));
    return true;
}

bool SchedulerTaskRunner::RunsTasksInCurrentSequence() const
{
    return current_task_runner && current_task_runner->sequence_id_ == sequence_id_;
}
...
void SchedulerTaskRunner::RunTask(base::OnceClosure task)
{
    ...
    const base::AutoReset<const SchedulerTaskRunner*> resetter(&current_task_runner, this, nullptr);
    std::move(task).Run();
}
