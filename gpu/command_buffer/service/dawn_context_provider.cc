...
BASE_FEATURE(kForceDawnInitializeFailure, "ForceDawnInitializeFailure", base::FEATURE_DISABLED_BY_DEFAULT);
...
// threads.
template <uint32_t KeySize> void SetCrashKeyThreadSafe(crash_reporter::CrashKeyString<KeySize>& crash_key, std::string_view message)
{
...
std::vector<const char*> GetDisabledToggles(const GpuPreferences& gpu_preferences)
{
...
std::vector<const char*> GetEnabledToggles(wgpu::BackendType backend_type, bool force_fallback_adapter, const GpuPreferences& gpu_preferences)
{
    ...
    if (backend_type == wgpu::BackendType::Vulkan && force_fallback_adapter && command_line->HasSwitch(switches::kDisableGLDrawingForTests)) {
...
std::vector<wgpu::FeatureName> GetRequiredFeatures(wgpu::BackendType backend_type, wgpu::Adapter adapter)
{
    ...
    for (auto feature : kOptionalFeatures) {
        ...
        if (feature == wgpu::FeatureName::TransientAttachments && adapter.HasFeature(wgpu::FeatureName::MSAARenderToSingleSampled)) {
...
public:
    ...
    const unsigned char* GetTraceCategoryEnabledFlag(dawn::platform::TraceCategory category) override
    {
        return TRACE_EVENT_API_GET_CATEGORY_GROUP_ENABLED(TRACE_DISABLED_BY_DEFAULT("gpu.graphite.dawn"));
    }
};
...
bool IsD3D11DebugLayerEnabled(const Microsoft::WRL::ComPtr<ID3D11Device>& d3d11_device)
{
...
{
    const auto switch_value = base::CommandLine::ForCurrentProcess()->GetSwitchValueASCII(switches::kSkiaGraphiteBackend);
    ...
    } else if (switch_value == switches::kSkiaGraphiteBackendDawnSwiftshader || switch_value == switches::kSkiaGraphiteBackendDawnVulkan) {
...
#if BUILDFLAG(IS_WIN)
    return base::FeatureList::IsEnabled(features::kSkiaGraphiteDawnUseD3D12) ? wgpu::BackendType::D3D12 : wgpu::BackendType::D3D11;
...
{
    return base::CommandLine::ForCurrentProcess()->GetSwitchValueASCII(switches::kSkiaGraphiteBackend) == switches::kSkiaGraphiteBackendDawnSwiftshader
        || gl::GetANGLEImplementation() == gl::ANGLEImplementation::kSwiftShader;
}

// static
bool DawnContextProvider::DefaultValidateAdapterFn(wgpu::BackendType, wgpu::Adapter)
{
    return true;
}
...
class DawnSharedContext : public base::RefCountedThreadSafe<DawnSharedContext>, public base::trace_event::MemoryDumpProvider {
public:
    DawnSharedContext() = default;

    bool Initialize(wgpu::BackendType backend_type, bool force_fallback_adapter, const GpuPreferences& gpu_preferences,
        DawnContextProvider::ValidateAdapterFn validate_adapter_fn, const GpuDriverBugWorkarounds& gpu_driver_workarounds);
    void SetCachingInterface(std::unique_ptr<webgpu::DawnCachingInterface> caching_interface);
...
private:
    ...
    static size_t LoadCachedData(const void* key, size_t key_size, void* value, size_t value_size, void* userdata)
    {
        if (auto& caching_interface = static_cast<DawnSharedContext*>(userdata)->caching_interface_) {
    ...
    // Provided to wgpu::Device as caching callback.
    static void StoreCachedData(const void* key, size_t key_size, const void* value, size_t value_size, void* userdata)
    {
        if (auto& caching_interface = static_cast<DawnSharedContext*>(userdata)->caching_interface_) {
    ...
    static void LogInfo(WGPULoggingType type, WGPUStringView message, void*)
    {
    ...
    std::optional<error::ContextLostReason> context_lost_reason_ GUARDED_BY(context_lost_lock_);
};
...
bool DawnSharedContext::Initialize(wgpu::BackendType backend_type, bool force_fallback_adapter, const GpuPreferences& gpu_preferences,
    DawnContextProvider::ValidateAdapterFn validate_adapter_fn, const GpuDriverBugWorkarounds& gpu_driver_workarounds)
{
    ...
    instance_ = webgpu::DawnInstance::Create(&platform_, gpu_preferences, webgpu::SafetyLevel::kUnsafe, &DawnSharedContext::LogInfo, nullptr);

    std::vector<const char*> enabled_toggles = GetEnabledToggles(backend_type, force_fallback_adapter, gpu_preferences);
    std::vector<const char*> disabled_toggles = GetDisabledToggles(gpu_preferences);
    ...
    if ((adapter_options.backendType == wgpu::BackendType::D3D11 || adapter_options.backendType == wgpu::BackendType::D3D12)
    ...
    dawn::native::d3d11::RequestAdapterOptionsD3D11Device adapter_options_d3d11_device;
    if (adapter_options.backendType == wgpu::BackendType::D3D11) {
        Microsoft::WRL::ComPtr<ID3D11Device> d3d11_device = gl::QueryD3D11DeviceObjectFromANGLE();
        CHECK(d3d11_device) << "Query d3d11 device from ANGLE failed.";

        static crash_reporter::CrashKeyString<16> feature_level_key("d3d11-feature-level");
        std::string feature_level = D3DFeatureLevelToString(d3d11_device->GetFeatureLevel());
        ...
        CHECK(SUCCEEDED(hr)) << "Query ID3D11Multithread interface failed: 0x" << std::hex << hr;
    ...
    descriptor.SetUncapturedErrorCallback(
        [](const wgpu::Device&, wgpu::ErrorType type, wgpu::StringView message, DawnSharedContext* state) {
    ...
    descriptor.SetDeviceLostCallback(
        ...
        [](const wgpu::Device&, wgpu::DeviceLostReason reason, wgpu::StringView message, DawnSharedContext* state) {
    ...
    std::vector<wgpu::FeatureName> features = GetRequiredFeatures(backend_type, adapter_);
    ...
    constexpr bool enable_backend_validation = DCHECK_IS_ON() && BUILDFLAG(IS_WIN);

    std::vector<dawn::native::BackendValidationLevel> backend_validation_levels = { dawn::native::BackendValidationLevel::Disabled };
    if (features::kSkiaGraphiteDawnBackendValidation.Get() || enable_backend_validation) {
        backend_validation_levels.push_back(dawn::native::BackendValidationLevel::Partial);
        backend_validation_levels.push_back(dawn::native::BackendValidationLevel::Full);
    }
    ...
    for (auto it = backend_validation_levels.rbegin(); it != backend_validation_levels.rend(); ++it) {
        auto level = *it;
    ...
    is_vulkan_swiftshader_adapter_ = backend_type == wgpu::BackendType::Vulkan && force_fallback_adapter;

#if BUILDFLAG(IS_WIN)
    if (auto d3d11_device = GetD3D11Device()) {
        static auto* crash_key = base::debug::AllocateCrashKeyString("d3d11-debug-layer", base::debug::CrashKeySize::Size32);
    ...
    if (base::SingleThreadTaskRunner::HasCurrentDefault()) {
        base::trace_event::MemoryDumpManager::GetInstance()->RegisterDumpProvider(this, "DawnSharedContext", base::SingleThreadTaskRunner::GetCurrentDefault());
...
void DawnSharedContext::SetCachingInterface(std::unique_ptr<webgpu::DawnCachingInterface> caching_interface)
{
    CHECK(!caching_interface_);
    caching_interface_ = std::move(caching_interface);
}

std::optional<error::ContextLostReason> DawnSharedContext::GetResetStatus() const
{
    base::AutoLock auto_lock(context_lost_lock_);
    return context_lost_reason_;
}

void DawnSharedContext::OnError(wgpu::ErrorType error_type, wgpu::StringView message)
{
    LOG(ERROR) << message;
...
#if BUILDFLAG(IS_WIN)
    if (auto d3d11_device = GetD3D11Device()) {
        static crash_reporter::CrashKeyString<64> reason_message_key("d3d11-device-removed-reason");
...
public:
    ...
    {
        pmd_->GetOrCreateAllocatorDump(base::JoinString({ kDawnMemoryDumpPrefix, name }, "/"))->AddScalar(key, units, value);
    }

    void AddString(const char* name, const char* key, const std::string& value) override
    {
        pmd_->GetOrCreateAllocatorDump(base::JoinString({ kDawnMemoryDumpPrefix, name }, "/"))->AddString(key, "", value);
    }
...
bool DawnSharedContext::OnMemoryDump(const base::trace_event::MemoryDumpArgs& args, base::trace_event::ProcessMemoryDump* pmd)
{
    if (args.level_of_detail == base::trace_event::MemoryDumpLevelOfDetail::kBackground) {
        ...
        pmd->GetOrCreateAllocatorDump(kDawnMemoryDumpPrefix)
            ->AddScalar(MemoryAllocatorDump::kNameSize, MemoryAllocatorDump::kUnitsBytes, dawn::native::ComputeEstimatedMemoryUsage(device_.Get()));
    } else {
...
std::unique_ptr<DawnContextProvider> DawnContextProvider::Create(
    const GpuPreferences& gpu_preferences, ValidateAdapterFn validate_adapter_fn, const GpuDriverBugWorkarounds& gpu_driver_workarounds)
{
    return DawnContextProvider::CreateWithBackend(
        GetDefaultBackendType(), DefaultForceFallbackAdapter(), gpu_preferences, validate_adapter_fn, gpu_driver_workarounds);
}

std::unique_ptr<DawnContextProvider> DawnContextProvider::CreateWithBackend(wgpu::BackendType backend_type, bool force_fallback_adapter,
    const GpuPreferences& gpu_preferences, ValidateAdapterFn validate_adapter_fn, const GpuDriverBugWorkarounds& gpu_driver_workarounds)
{
    auto dawn_shared_context = base::MakeRefCounted<DawnSharedContext>();
    if (!dawn_shared_context->Initialize(backend_type, force_fallback_adapter, gpu_preferences, validate_adapter_fn, gpu_driver_workarounds)) {
        return nullptr;
    }

    return base::WrapUnique(new DawnContextProvider(std::move(dawn_shared_context)));
}

std::unique_ptr<DawnContextProvider> DawnContextProvider::CreateWithSharedDevice(const DawnContextProvider* existing)
{
    CHECK(existing);
    CHECK(existing->dawn_shared_context_);
    return base::WrapUnique(new DawnContextProvider(existing->dawn_shared_context_));
}

DawnContextProvider::DawnContextProvider(scoped_refptr<DawnSharedContext> dawn_shared_context)
...
bool DawnContextProvider::InitializeGraphiteContext(const skgpu::graphite::ContextOptions& context_options)
{
    ...
    if (auto device = GetDevice()) {
        ...
        graphite_context_ = skgpu::graphite::ContextFactory::MakeDawn(backend_context, context_options);
    }

    return !!graphite_context_;
}

void DawnContextProvider::SetCachingInterface(std::unique_ptr<webgpu::DawnCachingInterface> caching_interface)
{
...
#if BUILDFLAG(IS_WIN)
Microsoft::WRL::ComPtr<ID3D11Device> DawnContextProvider::GetD3D11Device() const
{
...
std::optional<error::ContextLostReason> DawnContextProvider::GetResetStatus() const
{
