...
TEST_F(GLES2DecoderPassthroughTest, TestInitDiscardableTexture)
{
    ...
    EXPECT_EQ(0u, passthrough_discardable_texture_manager()->NumCacheEntriesForTesting());
    DoInitializeDiscardableTextureCHROMIUM(kClientTextureId);
    EXPECT_EQ(1u, passthrough_discardable_texture_manager()->NumCacheEntriesForTesting());
}
...
TEST_F(GLES2DecoderPassthroughTest, TestInitDiscardableTextureWithInvalidArguments)
{
    EXPECT_EQ(0u, passthrough_discardable_texture_manager()->NumCacheEntriesForTesting());
...
TEST_F(GLES2DecoderPassthroughTest, TestUnlockDiscardableTexture)
{
    ...
    EXPECT_EQ(0u, passthrough_discardable_texture_manager()->NumCacheEntriesForTesting());
    DoInitializeDiscardableTextureCHROMIUM(kClientTextureId);
    EXPECT_TRUE(passthrough_discardable_texture_manager()->IsEntryLockedForTesting(kClientTextureId, context_group));
    DoUnlockDiscardableTextureCHROMIUM(kClientTextureId);
    EXPECT_FALSE(passthrough_discardable_texture_manager()->IsEntryLockedForTesting(kClientTextureId, context_group));
}
...
TEST_F(GLES2DecoderPassthroughTest, TestDiscardableTextureUnusableWhileUnlocked)
{
    DoBindTexture(GL_TEXTURE_2D, kClientTextureId);
    DoInitializeDiscardableTextureCHROMIUM(kClientTextureId);
...
TEST_F(GLES2DecoderPassthroughTest, DiscardableTextureLimits)
{
    ...
    constexpr size_t texture_size = texture_dimension_size * texture_dimension_size * 4;
    ...
    PassthroughDiscardableManager* discardable_manager = passthrough_discardable_texture_manager();
    ...
    for (size_t i = 0; i < texture_count; i++) {
        ...
        DoTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, texture_dimension_size, texture_dimension_size, 0, GL_RGBA, GL_UNSIGNED_BYTE, 0, 0);
        ...
        // been added
        EXPECT_EQ((i + 1) * texture_size, discardable_manager->TotalSizeForTesting());
    }
    ...
    for (size_t i = 0; i < texture_count; i++) {
        ...
        EXPECT_FALSE(discardable_manager->IsEntryTrackedForTesting(unlock_order[i], group()));
    ...
    DoTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, texture_dimension_size * 3, texture_dimension_size, 0, GL_RGBA, GL_UNSIGNED_BYTE, 0, 0);
...
TEST_F(GLES2DecoderPassthroughTest, DiscardableTextureSizeChanged)
{
    ...
    constexpr size_t small_texture_size = small_texture_dimension * small_texture_dimension * 4;

    constexpr size_t large_texture_dimension = 128;
    constexpr size_t large_texture_size = large_texture_dimension * large_texture_dimension * 4;

    // Initialize the texture to the small size
    DoGenTexture(kClientTextureId);
    DoBindTexture(GL_TEXTURE_2D, kClientTextureId);
    DoTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, small_texture_dimension, small_texture_dimension, 0, GL_RGBA, GL_UNSIGNED_BYTE, 0, 0);
    DoInitializeDiscardableTextureCHROMIUM(kClientTextureId);
    ...
    // new size
    DoTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, large_texture_dimension, large_texture_dimension, 0, GL_RGBA, GL_UNSIGNED_BYTE, 0, 0);
...
TEST_F(GLES2DecoderPassthroughTest, DiscardableTextureOwnershipOnUnlock)
{
    ...
    EXPECT_TRUE(discardable_manager->UnlockTexture(kClientTextureId, group(), &texture_to_unbind));
    ...
    EXPECT_NE(nullptr, discardable_manager->UnlockedTextureForTesting(kClientTextureId, group()));
}
...
TEST_F(GLES2DecoderPassthroughTest, DiscardableTextureMemoryPressure)
{
    ...
    discardable_manager->HandleMemoryPressure(base::MemoryPressureListener::MEMORY_PRESSURE_LEVEL_CRITICAL);
    EXPECT_EQ(cache_size_limit, discardable_manager->TotalSizeForTesting());
    EXPECT_EQ(texture_count, discardable_manager->NumCacheEntriesForTesting());
    ...
    discardable_manager->HandleMemoryPressure(base::MemoryPressureListener::MEMORY_PRESSURE_LEVEL_CRITICAL);
    EXPECT_FALSE(discardable_manager->IsEntryTrackedForTesting(client_textures[2], group()));
    EXPECT_EQ(cache_size_limit - texture_size, discardable_manager->TotalSizeForTesting());
    EXPECT_EQ(texture_count - 1, discardable_manager->NumCacheEntriesForTesting());
    ...
    discardable_manager->HandleMemoryPressure(base::MemoryPressureListener::MEMORY_PRESSURE_LEVEL_MODERATE);
    EXPECT_FALSE(discardable_manager->IsEntryTrackedForTesting(client_textures[0], group()));
    EXPECT_FALSE(discardable_manager->IsEntryTrackedForTesting(client_textures[1], group()));
    ...
    // textures.
    discardable_manager->HandleMemoryPressure(base::MemoryPressureListener::MEMORY_PRESSURE_LEVEL_CRITICAL);
    EXPECT_FALSE(discardable_manager->IsEntryTrackedForTesting(client_textures[3], group()));
    EXPECT_EQ(0u, discardable_manager->TotalSizeForTesting());
    EXPECT_EQ(0u, discardable_manager->NumCacheEntriesForTesting());
}

TEST_F(GLES2DecoderPassthroughTest, DiscardableTextureBindGeneratedTextureLock)
{
    PassthroughDiscardableManager* discardable_manager = passthrough_discardable_texture_manager();
    ...
    GLuint service_id = 0;
    EXPECT_TRUE(resources->texture_id_map.GetServiceID(kClientTextureId, &service_id));
    ...
    EXPECT_TRUE(resources->texture_id_map.GetServiceID(kClientTextureId, &generated_service_id));
    ...
    EXPECT_TRUE(resources->texture_id_map.GetServiceID(kClientTextureId, &final_service_id));
...
TEST_F(GLES2DecoderPassthroughTest, DiscardableTextureBindGeneratedTextureSizeChange)
{
    PassthroughDiscardableManager* discardable_manager = passthrough_discardable_texture_manager();
...
TEST_F(GLES2DecoderPassthroughTest, DiscardableTextureBindGeneratedTextureSizeChangeCubeMap)
{
    PassthroughDiscardableManager* discardable_manager = passthrough_discardable_texture_manager();
    ...
    for (size_t face_idx = 0; face_idx < 6; face_idx++) {
        DoTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + face_idx, 0, GL_RGBA, texture_dimension, texture_dimension, 0, GL_RGBA, GL_UNSIGNED_BYTE, 0, 0);
        EXPECT_EQ(texture_size * (face_idx + 1), discardable_manager->TotalSizeForTesting());
    }
}
