...
protected:
    virtual void SetupProgramManager()
    {
        manager_ = std::make_unique<ProgramManager>(
            nullptr, kMaxVaryingVectors, kMaxDrawBuffers, kMaxDualSourceDrawBuffers, kMaxVertexAttribs, gpu_preferences_, feature_info_.get(), nullptr);
    }
    void SetUpBase(const char* gl_version, const char* gl_extensions, FeatureInfo* feature_info = nullptr)
    {
        GpuServiceTest::SetUpWithGLVersion(gl_version, gl_extensions);
        if (!feature_info)
            feature_info = new FeatureInfo();
        TestHelper::SetupFeatureInfoInitExpectationsWithGLVersion(gl_.get(), gl_extensions, "", gl_version, feature_info->context_type());
    ...
    void CacheBlob(gpu::GpuDiskCacheType type, const std::string& key, const std::string& blob) override
    {
    }
...
TEST_F(ProgramManagerTest, Destroy)
{
    ...
    EXPECT_CALL(*gl_, DeleteProgram(kService1Id)).Times(1).RetiresOnSaturation();
...
TEST_F(ProgramManagerTest, DeleteBug)
{
    ...
    // Check we can create program.
    scoped_refptr<Program> program1(manager_->CreateProgram(kClient1Id, kService1Id));
    scoped_refptr<Program> program2(manager_->CreateProgram(kClient2Id, kService2Id));
    // Check program got created.
    ...
    EXPECT_CALL(*gl_, DeleteProgram(kService2Id)).Times(1).RetiresOnSaturation();
...
protected:
    ...
    {
        SetupShaderExpectations(kAttribs, kNumAttribs, kUniforms, kNumUniforms, nullptr, 0, kServiceProgramId);

        Shader* vertex_shader = shader_manager_.CreateShader(kVertexShaderClientId, kVertexShaderServiceId, GL_VERTEX_SHADER);
        Shader* fragment_shader = shader_manager_.CreateShader(kFragmentShaderClientId, kFragmentShaderServiceId, GL_FRAGMENT_SHADER);
        ...
        Program* program = manager_->CreateProgram(kClientProgramId, kServiceProgramId);
    ...
    void SetupShaderExpectations(AttribInfo* attribs, size_t num_attribs, UniformInfo* uniforms, size_t num_uniforms, ProgramOutputInfo* program_outputs,
        size_t num_program_outputs, GLuint service_id)
    {
        TestHelper::SetupShaderExpectationsWithVaryings(
            gl_.get(), feature_info_.get(), attribs, num_attribs, uniforms, num_uniforms, nullptr, 0, program_outputs, num_program_outputs, service_id);
    }

    // Return true if link status matches expected_link_status
    bool LinkAsExpected(Program* program, bool expected_link_status, ProgramOutputInfo* program_outputs = nullptr, size_t num_program_outputs = 0)
    {
        GLuint service_id = program->service_id();
        if (expected_link_status) {
            SetupShaderExpectations(kAttribs, kNumAttribs, kUniforms, kNumUniforms, program_outputs, num_program_outputs, service_id);
        }
        program->Link(nullptr, this);
        GLint link_status;
    ...
    Program* SetupProgramForVariables(const VarInfo* vertex_variables, size_t vertex_variable_size, const VarInfo* fragment_variables,
        size_t fragment_variable_size, const int* const shader_version = nullptr)
    {
        // Set up shader
        ...
        for (size_t ii = 0; ii < vertex_variable_size; ++ii) {
            ...
            case kVarAttribute:
                vertex_attrib_map[vertex_variables[ii].name] = TestHelper::ConstructAttribute(vertex_variables[ii].type, vertex_variables[ii].size,
                    vertex_variables[ii].precision, vertex_variables[ii].static_use, vertex_variables[ii].name);
                break;
            case kVarUniform:
                vertex_uniform_map[vertex_variables[ii].name] = TestHelper::ConstructUniform(vertex_variables[ii].type, vertex_variables[ii].size,
                    vertex_variables[ii].precision, vertex_variables[ii].static_use, vertex_variables[ii].name);
                break;
            case kVarVarying:
                vertex_varying_map[vertex_variables[ii].name] = TestHelper::ConstructVarying(vertex_variables[ii].type, vertex_variables[ii].size,
                    vertex_variables[ii].precision, vertex_variables[ii].static_use, vertex_variables[ii].name);
                break;
            case kVarOutput:
                vertex_output_variable_list.push_back(TestHelper::ConstructOutputVariable(vertex_variables[ii].type, vertex_variables[ii].size,
                    vertex_variables[ii].precision, vertex_variables[ii].static_use, vertex_variables[ii].name));
                break;
            default:
                NOTREACHED();
        ...
        for (size_t ii = 0; ii < fragment_variable_size; ++ii) {
            ...
            case kVarAttribute:
                frag_attrib_map[fragment_variables[ii].name] = TestHelper::ConstructAttribute(fragment_variables[ii].type, fragment_variables[ii].size,
                    fragment_variables[ii].precision, fragment_variables[ii].static_use, fragment_variables[ii].name);
                break;
            case kVarUniform:
                frag_uniform_map[fragment_variables[ii].name] = TestHelper::ConstructUniform(fragment_variables[ii].type, fragment_variables[ii].size,
                    fragment_variables[ii].precision, fragment_variables[ii].static_use, fragment_variables[ii].name);
                break;
            case kVarVarying:
                frag_varying_map[fragment_variables[ii].name] = TestHelper::ConstructVarying(fragment_variables[ii].type, fragment_variables[ii].size,
                    fragment_variables[ii].precision, fragment_variables[ii].static_use, fragment_variables[ii].name);
                break;
            case kVarOutput:
                frag_output_variable_list.push_back(TestHelper::ConstructOutputVariable(fragment_variables[ii].type, fragment_variables[ii].size,
                    fragment_variables[ii].precision, fragment_variables[ii].static_use, fragment_variables[ii].name));
                break;
            default:
                NOTREACHED();
        ...
        Shader* vshader = shader_manager_.CreateShader(kVertexShaderClientId, kVertexShaderServiceId, GL_VERTEX_SHADER);
        Shader* fshader = shader_manager_.CreateShader(kFragmentShaderClientId, kFragmentShaderServiceId, GL_FRAGMENT_SHADER);
        ...
        TestHelper::SetShaderStates(gl_.get(), fshader, true, nullptr, nullptr, shader_version, &frag_attrib_map, &frag_uniform_map, &frag_varying_map, nullptr,
...
ProgramManagerWithShaderTest::AttribInfo ProgramManagerWithShaderTest::kAttribs[] = {
    {
        kAttrib1Name,
        kAttrib1Size,
        kAttrib1Type,
...
const size_t ProgramManagerWithShaderTest::kNumAttribs = std::size(ProgramManagerWithShaderTest::kAttribs);

ProgramManagerWithShaderTest::UniformInfo ProgramManagerWithShaderTest::kUniforms[] = {
    {
        kUniform1Name,
        kUniform1Size,
        kUniform1Type,
...
const size_t ProgramManagerWithShaderTest::kNumUniforms = std::size(ProgramManagerWithShaderTest::kUniforms);
...
const char* ProgramManagerWithShaderTest::kUniform2NameWithArrayIndex = "uniform2[0]";
const char* ProgramManagerWithShaderTest::kUniform3Name = "uniform3";
const char* ProgramManagerWithShaderTest::kUniform3NameWithArrayIndex = "uniform3[0]";
...
TEST_F(ProgramManagerWithShaderTest, GetAttribInfos)
{
    ...
    const Program::AttribInfoVector& infos = program->GetAttribInfos();
...
TEST_F(ProgramManagerWithShaderTest, GetAttribInfo)
{
    ...
    const Program* program = SetupDefaultProgram();
    ASSERT_TRUE(program != nullptr);
    const Program::VertexAttrib* info = program->GetAttribInfo(kValidIndex);
...
TEST_F(ProgramManagerWithShaderTest, GetAttribInfoByLocation)
{
    ...
    const Program::VertexAttrib* info = program->GetAttribInfoByLocation(kAttrib2Location);
...
TEST_F(ProgramManagerWithShaderTest, GetUniformInfo)
{
    ...
    const Program* program = SetupDefaultProgram();
    ASSERT_TRUE(program != nullptr);
    const Program::UniformInfo* info = program->GetUniformInfo(0);
    ASSERT_TRUE(info != nullptr);
...
TEST_F(ProgramManagerWithShaderTest, GetUniformFakeLocation)
{
    ...
    Program::UniformInfo* uniform = const_cast<Program::UniformInfo*>(program->GetUniformInfo(2));
    ASSERT_TRUE(uniform != nullptr && kUniform3Size == 2);
    EXPECT_EQ(kUniform3Size, uniform->size);
    uniform->element_locations[1] = -1;
    EXPECT_EQ(kUniform1FakeLocation, program->GetUniformFakeLocation(kUniform1Name));
    EXPECT_EQ(kUniform2FakeLocation, program->GetUniformFakeLocation(kUniform2Name));
    EXPECT_EQ(kUniform3FakeLocation, program->GetUniformFakeLocation(kUniform3Name));
    // Check we can get uniform2 as "uniform2" even though the name is
    // "uniform2[0]"
    EXPECT_EQ(kUniform2FakeLocation, program->GetUniformFakeLocation("uniform2"));
    // Check we can get uniform3 as "uniform3[0]" even though we simulated GL
    // returning "uniform3"
    EXPECT_EQ(kUniform3FakeLocation, program->GetUniformFakeLocation(kUniform3NameWithArrayIndex));
    // Check that we can get the locations of the array elements > 1
    EXPECT_EQ(ProgramManager::MakeFakeLocation(kUniform2FakeLocation, 1), program->GetUniformFakeLocation("uniform2[1]"));
    EXPECT_EQ(ProgramManager::MakeFakeLocation(kUniform2FakeLocation, 2), program->GetUniformFakeLocation("uniform2[2]"));
...
TEST_F(ProgramManagerWithShaderTest, GetUniformInfoByFakeLocation)
{
    ...
    GLint array_index = -1;
    ASSERT_TRUE(program != nullptr);
    info = program->GetUniformInfoByFakeLocation(kUniform2FakeLocation, &real_location, &array_index);
    ...
    real_location = -1;
    array_index = -1;
    info = program->GetUniformInfoByFakeLocation(kInvalidLocation, &real_location, &array_index);
    EXPECT_TRUE(info == nullptr);
    EXPECT_EQ(-1, real_location);
    EXPECT_EQ(-1, array_index);
    GLint loc = program->GetUniformFakeLocation("uniform2[2]");
    info = program->GetUniformInfoByFakeLocation(loc, &real_location, &array_index);
    ASSERT_TRUE(info != nullptr);
...
TEST_F(ProgramManagerWithShaderTest, GLDriverReturnsWrongTypeInfo)
{
    ...
    attrib_map[kAttrib1Name] = TestHelper::ConstructAttribute(kAttrib1Type, kAttrib1Size, kAttrib1Precision, kAttribStaticUse, kAttrib1Name);
    attrib_map[kAttrib2Name] = TestHelper::ConstructAttribute(kAttrib2GoodType, kAttrib2Size, kAttrib2Precision, kAttribStaticUse, kAttrib2Name);
    attrib_map[kAttrib3Name] = TestHelper::ConstructAttribute(kAttrib3Type, kAttrib3Size, kAttrib3Precision, kAttribStaticUse, kAttrib3Name);
    uniform_map[kUniform1Name] = TestHelper::ConstructUniform(kUniform1Type, kUniform1Size, kUniform1Precision, kUniform1StaticUse, kUniform1Name);
    uniform_map[kUniform2Name] = TestHelper::ConstructUniform(kUniform2GoodType, kUniform2Size, kUniform2Precision, kUniform2StaticUse, kUniform2Name);
    uniform_map[kUniform3Name] = TestHelper::ConstructUniform(kUniform3Type, kUniform3Size, kUniform3Precision, kUniform3StaticUse, kUniform3Name);
    output_variable_list.push_back(TestHelper::ConstructOutputVariable(
        kOutputVariable1Type, kOutputVariable1Size, kOutputVariable1Precision, kOutputVariable1StaticUse, kOutputVariable1Name));

    Shader* vshader = shader_manager_.CreateShader(kVertexShaderClientId, kVertexShaderServiceId, GL_VERTEX_SHADER);
    ASSERT_TRUE(vshader != nullptr);
    TestHelper::SetShaderStates(
        gl_.get(), vshader, true, nullptr, nullptr, nullptr, &attrib_map, &uniform_map, &varying_map, nullptr, &output_variable_list, nullptr);
    Shader* fshader = shader_manager_.CreateShader(kFragmentShaderClientId, kFragmentShaderServiceId, GL_FRAGMENT_SHADER);
    ASSERT_TRUE(fshader != nullptr);
    TestHelper::SetShaderStates(
        gl_.get(), fshader, true, nullptr, nullptr, nullptr, &attrib_map, &uniform_map, &varying_map, nullptr, &output_variable_list, nullptr);
    ...
    SetupShaderExpectations(kAttribs, kNumAttribs, kUniforms, kNumUniforms, kProgramOutputs, kNumProgramOutputs, kServiceProgramId);
    Program* program = manager_->CreateProgram(kClientProgramId, kServiceProgramId);
    ASSERT_TRUE(program != nullptr);
    EXPECT_TRUE(program->AttachShader(&shader_manager_, vshader));
    EXPECT_TRUE(program->AttachShader(&shader_manager_, fshader));
    ...
    for (unsigned index = 0; index < kNumAttribs; ++index) {
        ...
        EXPECT_TRUE(it->second.findInfoByMappedName(attrib_info->name, &info, &original_name));
        EXPECT_EQ(info->type, attrib_info->type);
        EXPECT_EQ(static_cast<GLint>(info->getOutermostArraySize()), attrib_info->size);
    ...
    for (unsigned index = 0; index < kNumUniforms; ++index) {
        ...
        const sh::ShaderVariable* info;
        std::string original_name;
        EXPECT_TRUE(it->second.findInfoByMappedName(uniform_info->name, &info, &original_name));
        EXPECT_EQ(info->type, uniform_info->type);
        EXPECT_EQ(static_cast<GLint>(info->getOutermostArraySize()), uniform_info->size);
...
TEST_F(ProgramManagerWithShaderTest, ProgramInfoUseCount)
{
    ...
    EXPECT_CALL(*gl_, DeleteProgram(kServiceProgramId)).Times(1).RetiresOnSaturation();
    manager_->UnuseProgram(&shader_manager_, program);
...
TEST_F(ProgramManagerWithShaderTest, ProgramInfoGetProgramInfo)
{
    ...
    ProgramInfoHeader* header = bucket.GetDataAs<ProgramInfoHeader*>(0, sizeof(ProgramInfoHeader));
    ...
    const ProgramInput* inputs = bucket.GetDataAs<const ProgramInput*>(sizeof(*header), sizeof(ProgramInput) * (header->num_attribs + header->num_uniforms));
    ...
    for (uint32_t ii = 0; ii < header->num_attribs; ++ii) {
        ...
        const int32_t* location = bucket.GetDataAs<const int32_t*>(input->location_offset, sizeof(int32_t));
        ASSERT_TRUE(location != nullptr);
        EXPECT_EQ(expected.location, *location);
        const char* name_buf = bucket.GetDataAs<const char*>(input->name_offset, input->name_length);
    ...
    for (uint32_t ii = 0; ii < header->num_uniforms; ++ii) {
        ...
        EXPECT_EQ(expected.size, input->size);
        EXPECT_EQ(expected.type, input->type);
        const int32_t* locations = bucket.GetDataAs<const int32_t*>(input->location_offset, sizeof(int32_t) * input->size);
        ASSERT_TRUE(locations != nullptr);
        for (int32_t jj = 0; jj < input->size; ++jj) {
            EXPECT_EQ(ProgramManager::MakeFakeLocation(expected.fake_location, jj), locations[jj]);
        }
        const char* name_buf = bucket.GetDataAs<const char*>(input->name_offset, input->name_length);
        ASSERT_TRUE(name_buf != nullptr);
        std::string name(name_buf, input->name_length);
    ...
    EXPECT_EQ(header->num_attribs + header->num_uniforms, static_cast<uint32_t>(input - inputs));
}
...
TEST_F(ProgramManagerWithShaderTest, ProgramInfoGetUniformBlocksNone)
{
    ...
    EXPECT_CALL(*(gl_.get()), GetProgramiv(kServiceProgramId, GL_LINK_STATUS, _)).WillOnce(SetArgPointee<2>(GL_FALSE)).RetiresOnSaturation();
    EXPECT_TRUE(program->GetUniformBlocks(&bucket));
    EXPECT_EQ(sizeof(UniformBlocksHeader), bucket.size());
    UniformBlocksHeader* header = bucket.GetDataAs<UniformBlocksHeader*>(0, sizeof(UniformBlocksHeader));
    EXPECT_TRUE(header != nullptr);
    EXPECT_EQ(0u, header->num_uniform_blocks);
    // Zero uniform blocks.
    EXPECT_CALL(*(gl_.get()), GetProgramiv(kServiceProgramId, GL_LINK_STATUS, _)).WillOnce(SetArgPointee<2>(GL_TRUE)).RetiresOnSaturation();
    EXPECT_CALL(*(gl_.get()), GetProgramiv(kServiceProgramId, GL_ACTIVE_UNIFORM_BLOCKS, _)).WillOnce(SetArgPointee<2>(0)).RetiresOnSaturation();
    EXPECT_TRUE(program->GetUniformBlocks(&bucket));
    EXPECT_EQ(sizeof(UniformBlocksHeader), bucket.size());
    header = bucket.GetDataAs<UniformBlocksHeader*>(0, sizeof(UniformBlocksHeader));
    EXPECT_TRUE(header != nullptr);
    EXPECT_EQ(0u, header->num_uniform_blocks);
}
...
TEST_F(ProgramManagerWithShaderTest, ProgramInfoGetUniformBlocksValid)
{
    ...
    EXPECT_CALL(*(gl_.get()), GetProgramiv(kServiceProgramId, GL_LINK_STATUS, _)).WillOnce(SetArgPointee<2>(GL_TRUE)).RetiresOnSaturation();
    EXPECT_CALL(*(gl_.get()), GetProgramiv(kServiceProgramId, GL_ACTIVE_UNIFORM_BLOCKS, _))
        .WillOnce(SetArgPointee<2>(data.header.num_uniform_blocks))
        .RetiresOnSaturation();
    EXPECT_CALL(*(gl_.get()), GetProgramiv(kServiceProgramId, GL_ACTIVE_UNIFORM_BLOCK_MAX_NAME_LENGTH, _))
        .WillOnce(SetArgPointee<2>(1 + std::max(strlen(kName[0]), strlen(kName[1]))))
        .RetiresOnSaturation();
    for (uint32_t ii = 0; ii < data.header.num_uniform_blocks; ++ii) {
        EXPECT_CALL(*(gl_.get()), GetActiveUniformBlockiv(kServiceProgramId, ii, GL_UNIFORM_BLOCK_BINDING, _))
            .WillOnce(SetArgPointee<3>(data.entry[ii].binding))
            .RetiresOnSaturation();
        EXPECT_CALL(*(gl_.get()), GetActiveUniformBlockiv(kServiceProgramId, ii, GL_UNIFORM_BLOCK_DATA_SIZE, _))
            .WillOnce(SetArgPointee<3>(data.entry[ii].data_size))
            .RetiresOnSaturation();
        EXPECT_CALL(*(gl_.get()), GetActiveUniformBlockiv(kServiceProgramId, ii, GL_UNIFORM_BLOCK_NAME_LENGTH, _))
            .WillOnce(SetArgPointee<3>(data.entry[ii].name_length))
            .RetiresOnSaturation();
        EXPECT_CALL(*(gl_.get()), GetActiveUniformBlockName(kServiceProgramId, ii, data.entry[ii].name_length, _, _))
            .WillOnce(DoAll(SetArgPointee<3>(strlen(kName[ii])), SetArrayArgument<4>(kName[ii], kName[ii] + data.entry[ii].name_length)))
            .RetiresOnSaturation();
        EXPECT_CALL(*(gl_.get()), GetActiveUniformBlockiv(kServiceProgramId, ii, GL_UNIFORM_BLOCK_ACTIVE_UNIFORMS, _))
            .WillOnce(SetArgPointee<3>(data.entry[ii].active_uniforms))
            .RetiresOnSaturation();
        EXPECT_CALL(*(gl_.get()), GetActiveUniformBlockiv(kServiceProgramId, ii, GL_UNIFORM_BLOCK_REFERENCED_BY_VERTEX_SHADER, _))
            .WillOnce(SetArgPointee<3>(data.entry[ii].referenced_by_vertex_shader))
            .RetiresOnSaturation();
        EXPECT_CALL(*(gl_.get()), GetActiveUniformBlockiv(kServiceProgramId, ii, GL_UNIFORM_BLOCK_REFERENCED_BY_FRAGMENT_SHADER, _))
            .WillOnce(SetArgPointee<3>(data.entry[ii].referenced_by_fragment_shader))
            .RetiresOnSaturation();
    }
    for (uint32_t ii = 0; ii < data.header.num_uniform_blocks; ++ii) {
        EXPECT_CALL(*(gl_.get()), GetActiveUniformBlockiv(kServiceProgramId, ii, GL_UNIFORM_BLOCK_ACTIVE_UNIFORM_INDICES, _))
            .WillOnce(SetArrayArgument<3>(kIndices[ii], kIndices[ii] + data.entry[ii].active_uniforms))
            .RetiresOnSaturation();
    }
...
TEST_F(ProgramManagerWithShaderTest, ProgramInfoGetTransformFeedbackVaryingsNone)
{
    CommonDecoder::Bucket bucket;
    const Program* program = SetupDefaultProgram();
    ASSERT_TRUE(program != nullptr);
    // The program's previous link failed.
    EXPECT_CALL(*(gl_.get()), GetProgramiv(kServiceProgramId, GL_TRANSFORM_FEEDBACK_BUFFER_MODE, _))
    ...
    TransformFeedbackVaryingsHeader* header = bucket.GetDataAs<TransformFeedbackVaryingsHeader*>(0, sizeof(TransformFeedbackVaryingsHeader));
    EXPECT_TRUE(header != nullptr);
    EXPECT_EQ(0u, header->num_transform_feedback_varyings);
    EXPECT_EQ(static_cast<uint32_t>(GL_INTERLEAVED_ATTRIBS), header->transform_feedback_buffer_mode);
    // Zero transform feedback blocks.
    EXPECT_CALL(*(gl_.get()), GetProgramiv(kServiceProgramId, GL_TRANSFORM_FEEDBACK_BUFFER_MODE, _))
        .WillOnce(SetArgPointee<2>(GL_SEPARATE_ATTRIBS))
        .RetiresOnSaturation();
    EXPECT_CALL(*(gl_.get()), GetProgramiv(kServiceProgramId, GL_LINK_STATUS, _)).WillOnce(SetArgPointee<2>(GL_TRUE)).RetiresOnSaturation();
    EXPECT_CALL(*(gl_.get()), GetProgramiv(kServiceProgramId, GL_TRANSFORM_FEEDBACK_VARYINGS, _)).WillOnce(SetArgPointee<2>(0)).RetiresOnSaturation();
    EXPECT_TRUE(program->GetTransformFeedbackVaryings(&bucket));
    EXPECT_EQ(sizeof(TransformFeedbackVaryingsHeader), bucket.size());
    header = bucket.GetDataAs<TransformFeedbackVaryingsHeader*>(0, sizeof(TransformFeedbackVaryingsHeader));
    EXPECT_TRUE(header != nullptr);
    EXPECT_EQ(static_cast<uint32_t>(GL_SEPARATE_ATTRIBS), header->transform_feedback_buffer_mode);
    EXPECT_EQ(0u, header->num_transform_feedback_varyings);
}

TEST_F(ProgramManagerWithShaderTest, ProgramInfoGetTransformFeedbackVaryingsValid)
{
    CommonDecoder::Bucket bucket;
    const Program* program = SetupDefaultProgram();
    ASSERT_TRUE(program != nullptr);
    ...
    EXPECT_CALL(*(gl_.get()), GetProgramiv(kServiceProgramId, GL_TRANSFORM_FEEDBACK_BUFFER_MODE, _))
        ...
        .RetiresOnSaturation();
    EXPECT_CALL(*(gl_.get()), GetProgramiv(kServiceProgramId, GL_LINK_STATUS, _)).WillOnce(SetArgPointee<2>(GL_TRUE)).RetiresOnSaturation();
    EXPECT_CALL(*(gl_.get()), GetProgramiv(kServiceProgramId, GL_TRANSFORM_FEEDBACK_VARYINGS, _))
    ...
    EXPECT_CALL(*(gl_.get()), GetProgramiv(kServiceProgramId, GL_TRANSFORM_FEEDBACK_VARYING_MAX_LENGTH, _))
        .WillOnce(SetArgPointee<2>(max_length))
        .RetiresOnSaturation();
    for (uint32_t ii = 0; ii < data.header.num_transform_feedback_varyings; ++ii) {
        EXPECT_CALL(*(gl_.get()), GetTransformFeedbackVarying(kServiceProgramId, ii, max_length, _, _, _, _))
            .WillOnce(DoAll(SetArgPointee<3>(data.entry[ii].name_length - 1), SetArgPointee<4>(data.entry[ii].size), SetArgPointee<5>(data.entry[ii].type),
                SetArrayArgument<6>(kName[ii], kName[ii] + data.entry[ii].name_length)))
            .RetiresOnSaturation();
    }
...
TEST_F(ProgramManagerWithShaderTest, ProgramInfoGetUniformsES3None)
{
    ...
    UniformsES3Header* header = bucket.GetDataAs<UniformsES3Header*>(0, sizeof(UniformsES3Header));
    EXPECT_TRUE(header != nullptr);
    EXPECT_EQ(0u, header->num_uniforms);
    // Zero uniform blocks.
    EXPECT_CALL(*(gl_.get()), GetProgramiv(kServiceProgramId, GL_LINK_STATUS, _)).WillOnce(SetArgPointee<2>(GL_TRUE)).RetiresOnSaturation();
    EXPECT_CALL(*(gl_.get()), GetProgramiv(kServiceProgramId, GL_ACTIVE_UNIFORMS, _)).WillOnce(SetArgPointee<2>(0)).RetiresOnSaturation();
    EXPECT_TRUE(program->GetUniformsES3(&bucket));
    EXPECT_EQ(sizeof(UniformsES3Header), bucket.size());
    header = bucket.GetDataAs<UniformsES3Header*>(0, sizeof(UniformsES3Header));
    EXPECT_TRUE(header != nullptr);
    EXPECT_EQ(0u, header->num_uniforms);
}
...
TEST_F(ProgramManagerWithShaderTest, ProgramInfoGetUniformsES3Valid)
{
    ...
    EXPECT_CALL(*(gl_.get()), GetProgramiv(kServiceProgramId, GL_LINK_STATUS, _)).WillOnce(SetArgPointee<2>(GL_TRUE)).RetiresOnSaturation();
    EXPECT_CALL(*(gl_.get()), GetProgramiv(kServiceProgramId, GL_ACTIVE_UNIFORMS, _))
    ...
    for (size_t ii = 0; ii < kNumIterations; ++ii) {
        EXPECT_CALL(*(gl_.get()), GetActiveUniformsiv(kServiceProgramId, data.header.num_uniforms, _, kPname[ii], _))
            .WillOnce(SetArrayArgument<4>(kParams[ii], kParams[ii] + data.header.num_uniforms))
            .RetiresOnSaturation();
    }
...
TEST_F(ProgramManagerWithShaderTest, UnusedUniformArrayElements)
{
    ...
    for (size_t ii = 0; ii < std::size(kUniforms); ++ii) {
        Program::UniformInfo* uniform = const_cast<Program::UniformInfo*>(program->GetUniformInfo(ii));
        ASSERT_TRUE(uniform != nullptr);
        EXPECT_EQ(static_cast<size_t>(kUniforms[ii].size), uniform->element_locations.size());
    ...
    for (uint32_t ii = 0; ii < header->num_uniforms; ++ii) {
        ...
        const int32_t* locations = bucket.GetDataAs<const int32_t*>(input->location_offset, sizeof(int32_t) * input->size);
        ASSERT_TRUE(locations != nullptr);
        EXPECT_EQ(ProgramManager::MakeFakeLocation(expected.fake_location, 0), locations[0]);
...
TEST_F(ProgramManagerWithShaderTest, BindAttribLocationConflicts)
{
    ...
    for (uint32_t ii = 0; ii < kNumAttribs; ++ii) {
        attrib_map[kAttribs[ii].name]
            = TestHelper::ConstructAttribute(kAttribs[ii].type, kAttribs[ii].size, GL_MEDIUM_FLOAT, kAttribStaticUse, kAttribs[ii].name);
    }
    const char kAttribMatName[] = "matAttrib";
    attrib_map[kAttribMatName] = TestHelper::ConstructAttribute(GL_FLOAT_MAT2, 1, GL_MEDIUM_FLOAT, kAttribStaticUse, kAttribMatName);
    // Check we can create shader.
    Shader* vshader = shader_manager_.CreateShader(kVertexShaderClientId, kVertexShaderServiceId, GL_VERTEX_SHADER);
    Shader* fshader = shader_manager_.CreateShader(kFragmentShaderClientId, kFragmentShaderServiceId, GL_FRAGMENT_SHADER);
    // Check shader got created.
    ...
    // Set Status
    TestHelper::SetShaderStates(gl_.get(), vshader, true, nullptr, nullptr, nullptr, &attrib_map, nullptr, nullptr, nullptr, nullptr, nullptr);
    // Check attrib infos got copied.
    for (AttributeMap::const_iterator it = attrib_map.begin(); it != attrib_map.end(); ++it) {
        const sh::Attribute* variable_info = vshader->GetAttribInfo(it->first);
        ASSERT_TRUE(variable_info != nullptr);
        EXPECT_EQ(it->second.type, variable_info->type);
        EXPECT_EQ(it->second.getOutermostArraySize(), variable_info->getOutermostArraySize());
    ...
    TestHelper::SetShaderStates(gl_.get(), fshader, true, nullptr, nullptr, nullptr, &attrib_map, nullptr, nullptr, nullptr, nullptr, nullptr);
    // Set up program
    Program* program = manager_->CreateProgram(kClientProgramId, kServiceProgramId);
    ASSERT_TRUE(program != nullptr);
    EXPECT_TRUE(program->AttachShader(&shader_manager_, vshader));
    ...
    EXPECT_FALSE(program->DetectAttribLocationBindingConflicts());
    EXPECT_CALL(*(gl_.get()), BindAttribLocation(_, 0, _)).Times(1).RetiresOnSaturation();
    EXPECT_TRUE(LinkAsExpected(program, true));
    ...
    EXPECT_FALSE(program->DetectAttribLocationBindingConflicts());
    EXPECT_CALL(*(gl_.get()), BindAttribLocation(_, _, _)).Times(2).RetiresOnSaturation();
    EXPECT_TRUE(LinkAsExpected(program, true));
    ...
    EXPECT_CALL(*(gl_.get()), BindAttribLocation(_, _, _)).Times(3).RetiresOnSaturation();
    EXPECT_FALSE(program->DetectAttribLocationBindingConflicts());
    EXPECT_TRUE(LinkAsExpected(program, true));
...
TEST_F(ProgramManagerWithShaderTest, UniformsPrecisionMismatch)
{
    // Set up shader
    UniformMap vertex_uniform_map;
    vertex_uniform_map["a"] = TestHelper::ConstructUniform(GL_FLOAT, 3, GL_MEDIUM_FLOAT, true, "a");
    UniformMap frag_uniform_map;
    frag_uniform_map["a"] = TestHelper::ConstructUniform(GL_FLOAT, 3, GL_LOW_FLOAT, true, "a");
    ...
    ASSERT_TRUE(vshader != nullptr && fshader != nullptr);
    // Set Status
    TestHelper::SetShaderStates(gl_.get(), vshader, true, nullptr, nullptr, nullptr, nullptr, &vertex_uniform_map, nullptr, nullptr, nullptr, nullptr);
    TestHelper::SetShaderStates(gl_.get(), fshader, true, nullptr, nullptr, nullptr, nullptr, &frag_uniform_map, nullptr, nullptr, nullptr, nullptr);
    // Set up program
    Program* program = manager_->CreateProgram(kClientProgramId, kServiceProgramId);
    ASSERT_TRUE(program != nullptr);
    EXPECT_TRUE(program->AttachShader(&shader_manager_, vshader));
...
{
    const VarInfo kVertexVarying = { GL_FLOAT_VEC3, 1, GL_MEDIUM_FLOAT, true, "a", kVarVarying };
    const VarInfo kFragmentVarying = { GL_FLOAT_VEC4, 1, GL_MEDIUM_FLOAT, true, "a", kVarVarying };
    Program* program = SetupProgramForVariables(&kVertexVarying, 1, &kFragmentVarying, 1);
...
{
    const VarInfo kVertexVarying = { GL_FLOAT, 2, GL_MEDIUM_FLOAT, true, "a", kVarVarying };
    const VarInfo kFragmentVarying = { GL_FLOAT, 3, GL_MEDIUM_FLOAT, true, "a", kVarVarying };
    Program* program = SetupProgramForVariables(&kVertexVarying, 1, &kFragmentVarying, 1);
...
{
    const VarInfo kVertexVarying = { GL_FLOAT, 2, GL_HIGH_FLOAT, true, "a", kVarVarying };
    const VarInfo kFragmentVarying = { GL_FLOAT, 2, GL_MEDIUM_FLOAT, true, "a", kVarVarying };
    Program* program = SetupProgramForVariables(&kVertexVarying, 1, &kFragmentVarying, 1);
...
{
    const VarInfo kFragmentVarying = { GL_FLOAT, 3, GL_MEDIUM_FLOAT, false, "a", kVarVarying };
    Program* program = SetupProgramForVariables(nullptr, 0, &kFragmentVarying, 1);
...
{
    const VarInfo kVertexAttribute = { GL_FLOAT_VEC4, 1, GL_MEDIUM_FLOAT, true, "a", kVarAttribute };
    const VarInfo kFragmentUniform = { GL_FLOAT_VEC4, 1, GL_MEDIUM_FLOAT, true, "a", kVarUniform };
    Program* program = SetupProgramForVariables(&kVertexAttribute, 1, &kFragmentUniform, 1);
...
TEST_F(ProgramManagerWithShaderTest, FragmentOutputTypes)
{
    // Set up program
    Shader* vshader = shader_manager_.CreateShader(kVertexShaderClientId, kVertexShaderServiceId, GL_VERTEX_SHADER);
    TestHelper::SetShaderStates(gl_.get(), vshader, true, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr);
    Shader* fshader = shader_manager_.CreateShader(kFragmentShaderClientId, kFragmentShaderServiceId, GL_FRAGMENT_SHADER);
    ...
    { // No outputs.
        TestHelper::SetShaderStates(gl_.get(), fshader, true, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr);
        EXPECT_TRUE(LinkAsExpected(program, true));
    ...
    { // gl_FragColor
        ...
        sh::OutputVariable var = TestHelper::ConstructOutputVariable(GL_FLOAT_VEC4, 0, GL_MEDIUM_FLOAT, true, "gl_FragColor");
        var.location = -1;
        fragment_outputs.push_back(var);
        TestHelper::SetShaderStates(gl_.get(), fshader, true, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, &fragment_outputs, nullptr);
        EXPECT_TRUE(LinkAsExpected(program, true));
    ...
    { // gl_FragData
        OutputVariableList fragment_outputs;
        sh::OutputVariable var = TestHelper::ConstructOutputVariable(GL_FLOAT_VEC4, 8, GL_MEDIUM_FLOAT, true, "gl_FragData");
        var.location = -1;
        fragment_outputs.push_back(var);
        TestHelper::SetShaderStates(gl_.get(), fshader, true, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, &fragment_outputs, nullptr);
        EXPECT_TRUE(LinkAsExpected(program, true));
    ...
    { // gl_FragColor, gl_FragDepth
        ...
        var.location = -1;
        fragment_outputs.push_back(var);
        var = TestHelper::ConstructOutputVariable(GL_FLOAT, 0, GL_MEDIUM_FLOAT, true, "gl_FragDepth");
        var.location = -1;
        fragment_outputs.push_back(var);
        TestHelper::SetShaderStates(gl_.get(), fshader, true, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, &fragment_outputs, nullptr);
        EXPECT_TRUE(LinkAsExpected(program, true));
    ...
    { // Single user defined output.
        ...
        const size_t kNumProgramOutputs = std::size(kProgramOutputs);
        OutputVariableList fragment_outputs;
        sh::OutputVariable var = TestHelper::ConstructOutputVariable(GL_UNSIGNED_INT_VEC4, 0, GL_MEDIUM_INT, true, "myOutput");
        var.location = -1;
        fragment_outputs.push_back(var);
        TestHelper::SetShaderStates(gl_.get(), fshader, true, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, &fragment_outputs, nullptr);
        EXPECT_TRUE(LinkAsExpected(program, true, kProgramOutputs, kNumProgramOutputs));
    ...
    { // Single user defined output - no static use.
        ...
        const size_t kNumProgramOutputs = std::size(kProgramOutputs);
        OutputVariableList fragment_outputs;
        sh::OutputVariable var = TestHelper::ConstructOutputVariable(GL_UNSIGNED_INT_VEC4, 0, GL_MEDIUM_INT, false, "myOutput");
        var.location = -1;
        fragment_outputs.push_back(var);
        TestHelper::SetShaderStates(gl_.get(), fshader, true, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, &fragment_outputs, nullptr);
        EXPECT_TRUE(LinkAsExpected(program, true, kProgramOutputs, kNumProgramOutputs));
    ...
    { // Multiple user defined outputs.
        ...
        OutputVariableList fragment_outputs;
        sh::OutputVariable var = TestHelper::ConstructOutputVariable(GL_INT_VEC4, 0, GL_MEDIUM_INT, true, "myOutput");
        var.location = 0;
        fragment_outputs.push_back(var);
        var = TestHelper::ConstructOutputVariable(GL_FLOAT_VEC4, 0, GL_MEDIUM_FLOAT, true, "myOutput1");
        var.location = 2;
        fragment_outputs.push_back(var);
        TestHelper::SetShaderStates(gl_.get(), fshader, true, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, &fragment_outputs, nullptr);
        EXPECT_TRUE(LinkAsExpected(program, true, program_outputs, num_program_outputs));
...
TEST_F(ProgramManagerWithShaderTest, TooManyVaryings)
{
    const VarInfo kVertexVaryings[]
        = { { GL_FLOAT_VEC4, 4, GL_MEDIUM_FLOAT, true, "a", kVarVarying }, { GL_FLOAT_VEC4, 5, GL_MEDIUM_FLOAT, true, "b", kVarVarying } };
    const VarInfo kFragmentVaryings[]
        = { { GL_FLOAT_VEC4, 4, GL_MEDIUM_FLOAT, true, "a", kVarVarying }, { GL_FLOAT_VEC4, 5, GL_MEDIUM_FLOAT, true, "b", kVarVarying } };
    Program* program = SetupProgramForVariables(kVertexVaryings, 2, kFragmentVaryings, 2);
...
TEST_F(ProgramManagerWithShaderTest, TooManyInactiveVaryings)
{
    const VarInfo kVertexVaryings[]
        = { { GL_FLOAT_VEC4, 4, GL_MEDIUM_FLOAT, true, "a", kVarVarying }, { GL_FLOAT_VEC4, 5, GL_MEDIUM_FLOAT, true, "b", kVarVarying } };
    const VarInfo kFragmentVaryings[]
        = { { GL_FLOAT_VEC4, 4, GL_MEDIUM_FLOAT, false, "a", kVarVarying }, { GL_FLOAT_VEC4, 5, GL_MEDIUM_FLOAT, true, "b", kVarVarying } };
    Program* program = SetupProgramForVariables(kVertexVaryings, 2, kFragmentVaryings, 2);
...
TEST_F(ProgramManagerWithShaderTest, BindUniformLocation)
{
    ...
    EXPECT_TRUE(program->AttachShader(&shader_manager_, vshader));
    EXPECT_TRUE(program->AttachShader(&shader_manager_, fshader));
    EXPECT_TRUE(program->SetUniformLocationBinding(kUniform1Name, kUniform1DesiredLocation));
    EXPECT_TRUE(program->SetUniformLocationBinding(kUniform3Name, kUniform3DesiredLocation));
    ...
    EXPECT_EQ(kUniform1DesiredLocation, program->GetUniformFakeLocation(kUniform1Name));
    EXPECT_EQ(kUniform3DesiredLocation, program->GetUniformFakeLocation(kUniform3Name));
    EXPECT_EQ(kUniform3DesiredLocation, program->GetUniformFakeLocation(kUniform3NameWithArrayIndex));
}
...
TEST_F(ProgramManagerWithShaderTest, ZeroSizeUniformMarkedInvalid)
{
    ...
    SetupShaderExpectations(kAttribs, kNumAttribs, kInvalidUniforms, kNumInvalidUniforms, nullptr, 0, kServiceProgramId);
...
protected:
    ...
    {
        manager_ = std::make_unique<ProgramManager>(
            cache_.get(), kMaxVaryingVectors, kMaxDrawBuffers, kMaxDualSourceDrawBuffers, kMaxVertexAttribs, gpu_preferences_, feature_info_.get(), nullptr);
    }
    ...
    void SetUp() override
    {
        ...
        vertex_shader_ = shader_manager_.CreateShader(kVertexShaderClientId, kVertexShaderServiceId, GL_VERTEX_SHADER);
        fragment_shader_ = shader_manager_.CreateShader(kFragmentShaderClientId, kFragmentShaderServiceId, GL_FRAGMENT_SHADER);
    ...
    {
        TestHelper::SetShaderStates(gl_.get(), vertex_shader_, true, compilation_options_string);
        TestHelper::SetShaderStates(gl_.get(), fragment_shader_, true, compilation_options_string);
    }

    void SetProgramCached()
    {
        cache_->LinkedProgramCacheSuccess(vertex_shader_->source(), fragment_shader_->source(), &program_->bind_attrib_location_map(),
            program_->effective_transform_feedback_varyings(), program_->effective_transform_feedback_buffer_mode());
    }

    void SetExpectationsForProgramCached()
    {
        SetExpectationsForProgramCached(program_, vertex_shader_, fragment_shader_);
    }

    void SetExpectationsForProgramCached(Program* program, Shader* vertex_shader, Shader* fragment_shader)
    {
        EXPECT_CALL(*cache_.get(),
            SaveLinkedProgram(program->service_id(), vertex_shader, fragment_shader, &program->bind_attrib_location_map(),
                program_->effective_transform_feedback_varyings(), program_->effective_transform_feedback_buffer_mode(), _))
            .Times(1);
    }

    void SetExpectationsForNotCachingProgram()
    {
        SetExpectationsForNotCachingProgram(program_, vertex_shader_, fragment_shader_);
    }

    void SetExpectationsForNotCachingProgram(Program* program, Shader* vertex_shader, Shader* fragment_shader)
    {
        EXPECT_CALL(*cache_.get(),
            SaveLinkedProgram(program->service_id(), vertex_shader, fragment_shader, &program->bind_attrib_location_map(),
                program_->effective_transform_feedback_varyings(), program_->effective_transform_feedback_buffer_mode(), _))
    ...
    {
        SetExpectationsForProgramLoad(kServiceProgramId, program_, vertex_shader_, fragment_shader_, result);
    }

    void SetExpectationsForProgramLoad(
        GLuint service_program_id, Program* program, Shader* vertex_shader, Shader* fragment_shader, ProgramCache::ProgramLoadResult result)
    {
        EXPECT_CALL(*cache_.get(),
            LoadLinkedProgram(service_program_id, vertex_shader, fragment_shader, &program->bind_attrib_location_map(),
                program_->effective_transform_feedback_varyings(), program_->effective_transform_feedback_buffer_mode(), _))
    ...
    {
        TestHelper::SetupProgramSuccessExpectations(gl_.get(), feature_info_.get(), nullptr, 0, nullptr, 0, nullptr, 0, nullptr, 0, service_program_id);
    }

    void SetExpectationsForProgramNotLoaded()
    {
        EXPECT_CALL(*cache_.get(),
            LoadLinkedProgram(program_->service_id(), vertex_shader_.get(), fragment_shader_.get(), &program_->bind_attrib_location_map(),
                program_->effective_transform_feedback_varyings(), program_->effective_transform_feedback_buffer_mode(), _))
            .Times(Exactly(0));
    }
    ...
    {
        TestHelper::SetupShaderExpectations(gl_.get(), feature_info_.get(), nullptr, 0, nullptr, 0, service_program_id);
        if (gl::g_current_gl_driver->ext.b_GL_OES_get_program_binary) {
            EXPECT_CALL(*gl_.get(), ProgramParameteri(service_program_id, PROGRAM_BINARY_RETRIEVABLE_HINT, GL_TRUE)).Times(1);
        }
    }

    void SetExpectationsForSuccessCompile(const Shader* shader)
    {
        const GLuint shader_id = shader->service_id();
        const char* src = shader->source().c_str();
        EXPECT_CALL(*gl_.get(), ShaderSource(shader_id, 1, Pointee(src), nullptr)).Times(1);
        EXPECT_CALL(*gl_.get(), CompileShader(shader_id)).Times(1);
        EXPECT_CALL(*gl_.get(), GetShaderiv(shader_id, GL_COMPILE_STATUS, _)).WillOnce(SetArgPointee<2>(GL_TRUE));
    }
    ...
    void SetExpectationsForNoCompile(const Shader* shader)
    {
        const GLuint shader_id = shader->service_id();
        const char* src = shader->source().c_str();
        EXPECT_CALL(*gl_.get(), ShaderSource(shader_id, 1, Pointee(src), nullptr)).Times(0);
        EXPECT_CALL(*gl_.get(), CompileShader(shader_id)).Times(0);
        EXPECT_CALL(*gl_.get(), GetShaderiv(shader_id, GL_COMPILE_STATUS, _)).Times(0);
    }
    ...
    void SetExpectationsForErrorCompile(const Shader* shader)
    {
        ...
        EXPECT_CALL(*gl_.get(), ShaderSource(shader_id, 1, Pointee(src), nullptr)).Times(1);
        EXPECT_CALL(*gl_.get(), CompileShader(shader_id)).Times(1);
        EXPECT_CALL(*gl_.get(), GetShaderiv(shader_id, GL_COMPILE_STATUS, _)).WillOnce(SetArgPointee<2>(GL_FALSE));
        EXPECT_CALL(*gl_.get(), GetShaderiv(shader_id, GL_INFO_LOG_LENGTH, _)).WillOnce(SetArgPointee<2>(0));
        EXPECT_CALL(*gl_.get(), GetShaderInfoLog(shader_id, 0, _, _)).Times(1);
    }
...
namespace {
testing::tuple<const char*, const char*> make_gl_ext_tuple(const char* gl_version, const char* gl_extensions)
{
    return testing::make_tuple(gl_version, gl_extensions);
}
}

class ProgramManagerDualSourceBlendingTest : public ProgramManagerWithShaderTest, public testing::WithParamInterface<testing::tuple<const char*, const char*>> {
public:
...
protected:
    ...
    void SetUp() override
    {
        SetUpWithFeatureInfo(nullptr);
    }
};

class ProgramManagerDualSourceBlendingES2Test : public ProgramManagerDualSourceBlendingTest { };
...
TEST_P(ProgramManagerDualSourceBlendingES2Test, UseSecondaryFragCoord)
{
    ...
    const VarInfo kFragmentVaryings[] = {
        { GL_FLOAT_VEC4, 0, GL_MEDIUM_FLOAT, true, "gl_SecondaryFragColorEXT", kVarOutput },
        { GL_FLOAT_VEC4, 0, GL_MEDIUM_FLOAT, true, "gl_FragColor", kVarOutput },
    };

    int shader_version = 100;
    Program* program = SetupProgramForVariables(nullptr, 0, kFragmentVaryings, std::size(kFragmentVaryings), &shader_version);
    EXPECT_TRUE(LinkAsExpected(program, true));
}
...
{
    const VarInfo kFragmentVaryings[] = {
        { GL_FLOAT_VEC4, kMaxDualSourceDrawBuffers, GL_MEDIUM_FLOAT, true, "gl_SecondaryFragDataEXT", kVarOutput },
        { GL_FLOAT_VEC4, kMaxDrawBuffers, GL_MEDIUM_FLOAT, true, "gl_FragData", kVarOutput },
    };
...
INSTANTIATE_TEST_SUITE_P(SupportedContexts, ProgramManagerDualSourceBlendingES2Test,
    testing::Values(make_gl_ext_tuple("OpenGL ES 3.1", "GL_EXT_draw_buffers GL_EXT_blend_func_extended")));
