...
public:
    ProgramBinaryEmulator(GLsizei length, GLenum format, const char* binary)
        : length_(length)
        , format_(format)
        , binary_(binary)
    {
    ...
    void GetProgramBinary(GLuint program, GLsizei buffer_size, GLsizei* length, GLenum* format, GLvoid* binary)
    {
        if (length) {
            *length = length_;
        }
    ...
    void ProgramBinary(GLuint program, GLenum format, const GLvoid* binary, GLsizei length)
    {
...
public:
    ...
    MemoryProgramCacheTest()
        : cache_(new MemoryProgramCache(kCacheSizeBytes, kDisableGpuDiskCache, kDisableCachingForTransformFeedback, &use_shader_cache_shm_count_))
    ...
    int32_t shader_cache_count()
    {
        return shader_cache_count_;
    }
    const std::string& shader_cache_shader()
    {
        return shader_cache_shader_;
    }

protected:
    void SetUp() override
    {
        GpuServiceTest::SetUpWithGLVersion("OpenGL ES 3.0", "GL_OES_get_program_binary");
        ...
        vertex_attrib_map["a"] = TestHelper::ConstructAttribute(GL_FLOAT_VEC2, 34, GL_LOW_FLOAT, false, "a");
        vertex_uniform_map["a"] = TestHelper::ConstructUniform(GL_FLOAT, 10, GL_MEDIUM_FLOAT, true, "a");
        vertex_uniform_map["b"] = TestHelper::ConstructUniform(GL_FLOAT_VEC3, 3114, GL_HIGH_FLOAT, true, "b");
        vertex_varying_map["c"] = TestHelper::ConstructVarying(GL_FLOAT_VEC4, 2, GL_HIGH_FLOAT, true, "c");
        vertex_output_variable_list.push_back(TestHelper::ConstructOutputVariable(GL_FLOAT, 0, GL_HIGH_FLOAT, true, "d"));
        fragment_attrib_map["jjjbb"] = TestHelper::ConstructAttribute(GL_FLOAT_MAT4, 1114, GL_MEDIUM_FLOAT, false, "jjjbb");
        fragment_uniform_map["k"] = TestHelper::ConstructUniform(GL_FLOAT_MAT2, 34413, GL_MEDIUM_FLOAT, true, "k");
        fragment_varying_map["c"] = TestHelper::ConstructVarying(GL_FLOAT_VEC4, 2, GL_HIGH_FLOAT, true, "c");
        fragment_output_variable_list.push_back(TestHelper::ConstructOutputVariable(GL_FLOAT, 0, GL_HIGH_FLOAT, true, "d"));

        vertex_shader_->set_source("bbbalsldkdkdkd");
        fragment_shader_->set_source("bbbal   sldkdkdkas 134 ad");

        TestHelper::SetShaderStates(gl_.get(), vertex_shader_, true, nullptr, nullptr, nullptr, &vertex_attrib_map, &vertex_uniform_map, &vertex_varying_map,
            nullptr, &vertex_output_variable_list, nullptr);
        TestHelper::SetShaderStates(gl_.get(), fragment_shader_, true, nullptr, nullptr, nullptr, &fragment_attrib_map, &fragment_uniform_map,
            &fragment_varying_map, nullptr, &fragment_output_variable_list, nullptr);
    }

    void SetExpectationsForSaveLinkedProgram(const GLint program_id, ProgramBinaryEmulator* emulator) const
    {
        EXPECT_CALL(*gl_.get(), GetProgramiv(program_id, GL_PROGRAM_BINARY_LENGTH_OES, _)).WillOnce(SetArgPointee<2>(emulator->length()));
        EXPECT_CALL(*gl_.get(), GetProgramBinary(program_id, emulator->length(), _, _, _)).WillOnce(Invoke(emulator, &ProgramBinaryEmulator::GetProgramBinary));
    }

    void SetExpectationsForLoadLinkedProgram(const GLint program_id, ProgramBinaryEmulator* emulator) const
    {
        EXPECT_CALL(*gl_.get(), ProgramBinary(program_id, emulator->format(), _, emulator->length()))
            .WillOnce(Invoke(emulator, &ProgramBinaryEmulator::ProgramBinary));
        EXPECT_CALL(*gl_.get(), GetProgramiv(program_id, GL_LINK_STATUS, _)).WillOnce(SetArgPointee<2>(GL_TRUE));
    }

    void SetExpectationsForLoadLinkedProgramFailure(const GLint program_id, ProgramBinaryEmulator* emulator) const
    {
        EXPECT_CALL(*gl_.get(), ProgramBinary(program_id, emulator->format(), _, emulator->length()))
            .WillOnce(Invoke(emulator, &ProgramBinaryEmulator::ProgramBinary));
        EXPECT_CALL(*gl_.get(), GetProgramiv(program_id, GL_LINK_STATUS, _)).WillOnce(SetArgPointee<2>(GL_FALSE));
    }
...
TEST_F(MemoryProgramCacheTest, CacheSave)
{
    ...
    cache_->SaveLinkedProgram(kProgramId, vertex_shader_, fragment_shader_, nullptr, varyings_, GL_NONE, this);

    EXPECT_EQ(ProgramCache::LINK_SUCCEEDED,
        cache_->GetLinkedProgramStatus(vertex_shader_->last_compiled_signature(), fragment_shader_->last_compiled_signature(), nullptr, varyings_, GL_NONE));
...
TEST_F(MemoryProgramCacheTest, CacheLoadMatchesSave)
{
    ...
    OutputVariableList vertex_output_variable_list = vertex_shader_->output_variable_list();
    ...
    OutputVariableList fragment_output_variable_list = fragment_shader_->output_variable_list();
    ...
    EXPECT_EQ(ProgramCache::PROGRAM_LOAD_SUCCESS, cache_->LoadLinkedProgram(kProgramId, vertex_shader_, fragment_shader_, nullptr, varyings_, GL_NONE, this));
    ...
    EXPECT_EQ(vertex_output_variable_list, vertex_shader_->output_variable_list());
    ...
    EXPECT_EQ(fragment_output_variable_list, fragment_shader_->output_variable_list());
#endif
}
...
TEST_F(MemoryProgramCacheTest, LoadFailOnLinkFalse)
{
    ...
    EXPECT_EQ(ProgramCache::PROGRAM_LOAD_FAILURE, cache_->LoadLinkedProgram(kProgramId, vertex_shader_, fragment_shader_, nullptr, varyings_, GL_NONE, this));
}
...
TEST_F(MemoryProgramCacheTest, LoadFailOnDifferentMap)
{
    ...
    cache_->SaveLinkedProgram(kProgramId, vertex_shader_, fragment_shader_, &binding_map, varyings_, GL_NONE, this);

    binding_map["different!"] = 59;
    EXPECT_EQ(
        ProgramCache::PROGRAM_LOAD_FAILURE, cache_->LoadLinkedProgram(kProgramId, vertex_shader_, fragment_shader_, &binding_map, varyings_, GL_NONE, this));
    EXPECT_EQ(ProgramCache::PROGRAM_LOAD_FAILURE, cache_->LoadLinkedProgram(kProgramId, vertex_shader_, fragment_shader_, nullptr, varyings_, GL_NONE, this));
}
...
TEST_F(MemoryProgramCacheTest, LoadFailOnDifferentTransformFeedbackVaryings)
{
    ...
    cache_->SaveLinkedProgram(kProgramId, vertex_shader_, fragment_shader_, nullptr, varyings_, GL_INTERLEAVED_ATTRIBS, this);

    EXPECT_EQ(ProgramCache::PROGRAM_LOAD_FAILURE,
        cache_->LoadLinkedProgram(kProgramId, vertex_shader_, fragment_shader_, nullptr, varyings_, GL_SEPARATE_ATTRIBS, this));

    varyings_.push_back("different!");
    EXPECT_EQ(ProgramCache::PROGRAM_LOAD_FAILURE,
        cache_->LoadLinkedProgram(kProgramId, vertex_shader_, fragment_shader_, nullptr, varyings_, GL_INTERLEAVED_ATTRIBS, this));
}
...
TEST_F(MemoryProgramCacheTest, LoadFailIfTransformFeedbackCachingDisabled)
{
    ...
    cache_.reset(new MemoryProgramCache(kCacheSizeBytes, kDisableGpuDiskCache, true, &use_shader_cache_shm_count_));
    varyings_.push_back("test");
    cache_->SaveLinkedProgram(kProgramId, vertex_shader_, fragment_shader_, nullptr, varyings_, GL_INTERLEAVED_ATTRIBS, this);
    EXPECT_EQ(ProgramCache::PROGRAM_LOAD_FAILURE,
        cache_->LoadLinkedProgram(kProgramId, vertex_shader_, fragment_shader_, nullptr, varyings_, GL_INTERLEAVED_ATTRIBS, this));
...
TEST_F(MemoryProgramCacheTest, MemoryProgramCacheEviction)
{
    ...
    ProgramBinaryEmulator emulator2(kEvictingBinaryLength, kFormat, bigTestBinary.data());

    SetExpectationsForSaveLinkedProgram(kEvictingProgramId, &emulator2);
    cache_->SaveLinkedProgram(kEvictingProgramId, vertex_shader_, fragment_shader_, nullptr, varyings_, GL_NONE, this);

    EXPECT_EQ(ProgramCache::LINK_SUCCEEDED,
        cache_->GetLinkedProgramStatus(vertex_shader_->last_compiled_signature(), fragment_shader_->last_compiled_signature(), nullptr, varyings_, GL_NONE));
    EXPECT_EQ(ProgramCache::LINK_UNKNOWN, cache_->GetLinkedProgramStatus(vertex_shader_->last_compiled_signature(), old_sig, nullptr, varyings_, GL_NONE));
}
...
TEST_F(MemoryProgramCacheTest, MemoryProgramCacheTrim)
{
    ...
    cache_->SaveLinkedProgram(kSecondProgramId, vertex_shader_, fragment_shader_, nullptr, varyings_, GL_NONE, this);

    // Both programs should be present.
    EXPECT_EQ(ProgramCache::LINK_SUCCEEDED,
        cache_->GetLinkedProgramStatus(vertex_shader_->last_compiled_signature(), fragment_shader_->last_compiled_signature(), nullptr, varyings_, GL_NONE));
    EXPECT_EQ(ProgramCache::LINK_SUCCEEDED, cache_->GetLinkedProgramStatus(vertex_shader_->last_compiled_signature(), first_sig, nullptr, varyings_, GL_NONE));

    // Trim cache to 20 bytes - this should evict the first program.
    cache_->Trim(20);
    EXPECT_EQ(ProgramCache::LINK_SUCCEEDED,
        cache_->GetLinkedProgramStatus(vertex_shader_->last_compiled_signature(), fragment_shader_->last_compiled_signature(), nullptr, varyings_, GL_NONE));
    EXPECT_EQ(ProgramCache::LINK_UNKNOWN, cache_->GetLinkedProgramStatus(vertex_shader_->last_compiled_signature(), first_sig, nullptr, varyings_, GL_NONE));
