...
TEST_P(GLES2DecoderWithShaderTest, BindAndDeleteFramebuffer)
{
    ...
    DoDeleteFramebuffer(client_framebuffer_id_, kServiceFramebufferId, true, GL_FRAMEBUFFER, 0, true, GL_FRAMEBUFFER, 0);
    EXPECT_CALL(*gl_, DrawArrays(GL_TRIANGLES, 0, kNumVertices)).Times(1).RetiresOnSaturation();
    cmds::DrawArrays cmd;
...
TEST_P(GLES2DecoderTest, FramebufferTexture2DWithNoBoundTarget)
{
    ...
    cmd.Init(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, client_texture_id_, 0);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(GL_INVALID_OPERATION, GetGLError());
}
...
TEST_P(GLES3DecoderTest, FramebufferTexture2DWithNoBoundTarget)
{
    EXPECT_CALL(*gl_, FramebufferTexture2DEXT(_, _, _, _, _)).Times(0);
    cmds::FramebufferTexture2D cmd;
    cmd.Init(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, client_texture_id_, 1);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(GL_INVALID_OPERATION, GetGLError());
}
...
{
    EXPECT_CALL(*gl_, FramebufferTexture2DEXT(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, kServiceTextureId, 0));
...
{
    EXPECT_CALL(*gl_, FramebufferTexture2DEXT(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, kServiceTextureId, 1));
    DoBindTexture(GL_TEXTURE_2D, client_texture_id_, kServiceTextureId);
    DoBindFramebuffer(GL_FRAMEBUFFER, client_framebuffer_id_, kServiceFramebufferId);
    EXPECT_CALL(*gl_, GetError()).WillOnce(Return(GL_NO_ERROR)).WillOnce(Return(GL_NO_ERROR)).RetiresOnSaturation();
    cmds::FramebufferTexture2D cmd;
...
TEST_P(GLES3DecoderTest, FramebufferTexture2DDepthStencil)
{
    DoBindFramebuffer(GL_FRAMEBUFFER, client_framebuffer_id_, kServiceFramebufferId);
    DoBindTexture(GL_TEXTURE_2D, client_texture_id_, kServiceTextureId);
    EXPECT_CALL(*gl_, GetError()).WillOnce(Return(GL_NO_ERROR)).WillOnce(Return(GL_NO_ERROR)).WillOnce(Return(GL_NO_ERROR)).RetiresOnSaturation();
    EXPECT_CALL(*gl_, FramebufferTexture2DEXT(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_TEXTURE_2D, kServiceTextureId, 4)).Times(1).RetiresOnSaturation();
    EXPECT_CALL(*gl_, FramebufferTexture2DEXT(GL_FRAMEBUFFER, GL_STENCIL_ATTACHMENT, GL_TEXTURE_2D, kServiceTextureId, 4)).Times(1).RetiresOnSaturation();
    cmds::FramebufferTexture2D cmd;
    cmd.Init(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, GL_TEXTURE_2D, client_texture_id_, 4);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(GL_NO_ERROR, GetGLError());
...
TEST_P(GLES2DecoderTest, FramebufferTexture2DInvalidArgs0_0)
{
    ...
    DoBindFramebuffer(GL_FRAMEBUFFER, client_framebuffer_id_, kServiceFramebufferId);
    cmds::FramebufferTexture2D cmd;
    cmd.Init(GL_RENDERBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, client_texture_id_, 0);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
...
TEST_P(GLES3DecoderTest, FramebufferTexture2DInvalidArgs0_0)
{
    ...
    DoBindFramebuffer(GL_FRAMEBUFFER, client_framebuffer_id_, kServiceFramebufferId);
    cmds::FramebufferTexture2D cmd;
    cmd.Init(GL_RENDERBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, client_texture_id_, 1);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(GL_INVALID_ENUM, GetGLError());
}
...
TEST_P(GLES2DecoderTest, FramebufferTexture2DInvalidArgs2_0)
{
    ...
    DoBindFramebuffer(GL_FRAMEBUFFER, client_framebuffer_id_, kServiceFramebufferId);
    cmds::FramebufferTexture2D cmd;
    cmd.Init(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_PROXY_TEXTURE_CUBE_MAP, client_texture_id_, 0);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(GL_INVALID_ENUM, GetGLError());
}
...
TEST_P(GLES3DecoderTest, FramebufferTexture2DInvalidArgs2_0)
{
    ...
    DoBindFramebuffer(GL_FRAMEBUFFER, client_framebuffer_id_, kServiceFramebufferId);
    cmds::FramebufferTexture2D cmd;
    cmd.Init(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_PROXY_TEXTURE_CUBE_MAP, client_texture_id_, 1);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(GL_INVALID_ENUM, GetGLError());
}
...
{
    EXPECT_CALL(*gl_, GetFramebufferAttachmentParameterivEXT(_, _, _, _)).Times(0);
    DoBindTexture(GL_TEXTURE_2D, client_texture_id_, kServiceTextureId);
    DoBindFramebuffer(GL_FRAMEBUFFER, client_framebuffer_id_, kServiceFramebufferId);
    cmds::FramebufferTexture2D cmd;
    cmd.Init(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_CUBE_MAP_NEGATIVE_Y, client_texture_id_, 0);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(GL_INVALID_OPERATION, GetGLError());
}
...
TEST_P(GLES2DecoderTest, GetFramebufferAttachmentParameterivWithNoBoundTarget)
{
    ...
    cmd.Init(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_FRAMEBUFFER_ATTACHMENT_OBJECT_TYPE, shared_memory_id_, shared_memory_offset_);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(GL_INVALID_OPERATION, GetGLError());
}
...
{
    DoBindFramebuffer(GL_FRAMEBUFFER, client_framebuffer_id_, kServiceFramebufferId);
    DoBindRenderbuffer(GL_RENDERBUFFER, client_renderbuffer_id_, kServiceRenderbufferId);
    EXPECT_CALL(*gl_, GetError()).WillRepeatedly(Return(GL_NO_ERROR));
    EXPECT_CALL(*gl_, FramebufferRenderbufferEXT(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_RENDERBUFFER, kServiceRenderbufferId)).Times(1).RetiresOnSaturation();
    auto* result = static_cast<cmds::GetFramebufferAttachmentParameteriv::Result*>(shared_memory_address_);
    result->size = 0;
    const GLint* result_value = result->GetData();
    cmds::FramebufferRenderbuffer fbrb_cmd;
    cmds::GetFramebufferAttachmentParameteriv cmd;
    fbrb_cmd.Init(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_RENDERBUFFER, client_renderbuffer_id_);
    cmd.Init(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_FRAMEBUFFER_ATTACHMENT_OBJECT_NAME, shared_memory_id_, shared_memory_offset_);
...
TEST_P(GLES2DecoderTest, GetFramebufferAttachmentParameterivWithTexture)
{
    ...
    DoBindFramebuffer(GL_FRAMEBUFFER, client_framebuffer_id_, kServiceFramebufferId);
    EXPECT_CALL(*gl_, GetError()).WillRepeatedly(Return(GL_NO_ERROR));
    EXPECT_CALL(*gl_, FramebufferTexture2DEXT(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, kServiceTextureId, 0)).Times(1).RetiresOnSaturation();
    auto* result = static_cast<cmds::GetFramebufferAttachmentParameteriv::Result*>(shared_memory_address_);
    result->SetNumResults(0);
    const GLint* result_value = result->GetData();
    cmds::FramebufferTexture2D fbtex_cmd;
    cmds::GetFramebufferAttachmentParameteriv cmd;
    fbtex_cmd.Init(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, client_texture_id_, 0);
    cmd.Init(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_FRAMEBUFFER_ATTACHMENT_OBJECT_NAME, shared_memory_id_, shared_memory_offset_);
...
TEST_P(GLES2DecoderWithShaderTest, GetRenderbufferParameterivRebindRenderbuffer)
{
    SetupTexture();
    DoBindRenderbuffer(GL_RENDERBUFFER, client_renderbuffer_id_, kServiceRenderbufferId);
    ...
    cmd.Init(GL_RENDERBUFFER, GL_RENDERBUFFER_RED_SIZE, shared_memory_id_, shared_memory_offset_);

    RestoreRenderbufferBindings();
    EnsureRenderbufferBound(true);

    EXPECT_CALL(*gl_, GetError()).WillOnce(Return(GL_NO_ERROR)).WillOnce(Return(GL_NO_ERROR)).RetiresOnSaturation();
    EXPECT_CALL(*gl_, GetRenderbufferParameterivEXT(GL_RENDERBUFFER, GL_RENDERBUFFER_RED_SIZE, _));
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(GL_NO_ERROR, GetGLError());
}
...
TEST_P(GLES2DecoderTest, GetRenderbufferParameterivWithNoBoundTarget)
{
    ...
    cmds::GetRenderbufferParameteriv cmd;
    cmd.Init(GL_RENDERBUFFER, GL_RENDERBUFFER_WIDTH, shared_memory_id_, shared_memory_offset_);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(GL_INVALID_OPERATION, GetGLError());
}
...
public:
    ...
    ReadPixelsEmulator(GLsizei width, GLsizei height, GLint bytes_per_pixel, const void* src_pixels, const void* expected_pixels, GLint pack_alignment)
        : width_(width)
        , height_(height)
    ...
    bool CompareRowSegment(GLint x, GLint y, GLsizei width, const void* data) const
    {
        DCHECK(x + width <= width_ || width == 0);
        return memcmp(data, GetPixelAddress(expected_pixels_, x, y), width * bytes_per_pixel_) == 0;
    }

    // Helper to compute address of pixel in pack aligned data.
    const void* ComputePackAlignmentAddress(GLint x, GLint y, GLsizei width, const void* address) const
    {
...
void GLES2DecoderTest::CheckReadPixelsOutOfRange(GLint in_read_x, GLint in_read_y, GLsizei in_read_width, GLsizei in_read_height, bool init)
{
    ...
    static const uint8_t kSrcPixels[kWidth * kHeight * kBytesPerPixel]
        = { 12, 13, 14, 255, 18, 19, 18, 255, 19, 12, 13, 255, 14, 18, 19, 255, 18, 19, 13, 255, 29, 28, 23, 255, 22, 21, 22, 255, 21, 29, 28, 255, 23, 22, 21,
              255, 22, 21, 28, 255, 31, 34, 39, 255, 37, 32, 37, 255, 32, 31, 34, 255, 39, 37, 32, 255, 37, 32, 34, 255 };
    ...
    if (init) {
        DoBindTexture(GL_TEXTURE_2D, client_texture_id_, kServiceTextureId);
        DoTexImage2D(GL_TEXTURE_2D, 0, kFormat, kWidth, kHeight, 0, kFormat, GL_UNSIGNED_BYTE, shared_memory_id_, kSharedMemoryOffset);
        DoBindFramebuffer(GL_FRAMEBUFFER, client_framebuffer_id_, kServiceFramebufferId);
        ...
        EXPECT_CALL(*gl_, CheckFramebufferStatusEXT(GL_FRAMEBUFFER)).WillOnce(Return(GL_FRAMEBUFFER_COMPLETE)).RetiresOnSaturation();
    }

    ReadPixelsEmulator emu(kWidth, kHeight, kBytesPerPixel, kSrcPixels, kSrcPixels, kPackAlignment);
    ...
    if (read_width > 0 && read_height > 0) {
        for (GLint yy = read_y; yy < read_end_y; ++yy) {
            EXPECT_CALL(*gl_, ReadPixels(read_x, yy, read_width, 1, kFormat, GL_UNSIGNED_BYTE, _))
                .WillOnce(Invoke(&emu, &ReadPixelsEmulator::ReadPixels))
                .RetiresOnSaturation();
        }
    }
    cmds::ReadPixels cmd;
    cmd.Init(in_read_x, in_read_y, in_read_width, in_read_height, kFormat, GL_UNSIGNED_BYTE, pixels_shm_id, pixels_shm_offset, result_shm_id, result_shm_offset,
        false);
    result->success = 0;
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    ...
    for (GLint yy = 0; yy < in_read_height; ++yy) {
        const int8_t* row = static_cast<const int8_t*>(emu.ComputePackAlignmentAddress(0, yy, in_read_width, dest));
        ...
        if (y < 0 || y >= kHeight) {
            ...
            EXPECT_EQ(0, memcmp(zero.data(), row + unpadded_row_size - num_right_bytes, num_right_bytes));

            // check middle.
            GLint x = std::max(in_read_x, 0);
            GLint num_middle_pixels = std::max(in_read_width - num_left_pixels - num_right_pixels, 0);
            EXPECT_TRUE(emu.CompareRowSegment(x, y, num_middle_pixels, row + num_left_bytes));
        }
        ...
        if (yy != in_read_height - 1) {
            ...
            if (num_padding_bytes) {
                EXPECT_EQ(0, memcmp(pack.data(), row + unpadded_row_size, num_padding_bytes));
            }
        }
    }
}
...
TEST_P(GLES2DecoderTest, ReadPixels)
{
    ...
    void* dest = &result[1];
    EXPECT_CALL(*gl_, GetError()).WillOnce(Return(GL_NO_ERROR)).WillOnce(Return(GL_NO_ERROR)).RetiresOnSaturation();
    EXPECT_CALL(*gl_, ReadPixels(0, 0, kWidth, kHeight, GL_RGBA, GL_UNSIGNED_BYTE, _)).WillOnce(Invoke(&emu, &ReadPixelsEmulator::ReadPixels));
    cmds::ReadPixels cmd;
    cmd.Init(0, 0, kWidth, kHeight, GL_RGBA, GL_UNSIGNED_BYTE, pixels_shm_id, pixels_shm_offset, result_shm_id, result_shm_offset, false);
    result->success = 1;
    EXPECT_EQ(error::kInvalidArguments, ExecuteCmd(cmd));
    result->success = 0;
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    for (GLint yy = 0; yy < kHeight; ++yy) {
        EXPECT_TRUE(emu.CompareRowSegment(0, yy, kWidth, emu.ComputePackAlignmentAddress(0, yy, kWidth, dest)));
    }
}
...
TEST_P(GLES3DecoderTest, ReadPixels2PixelPackBuffer)
{
    ...
    EXPECT_CALL(*gl_, ReadPixels(0, 0, kWidth, kHeight, GL_RGBA, GL_UNSIGNED_BYTE, _));
    cmds::ReadPixels cmd;
    cmd.Init(0, 0, kWidth, kHeight, GL_RGBA, GL_UNSIGNED_BYTE, 0, 0, 0, 0, false);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(GL_NO_ERROR, GetGLError());
...
TEST_P(GLES3DecoderTest, ReadPixelsPixelPackBufferMapped)
{
    ...
    EXPECT_CALL(*gl_, MapBufferRange(GL_PIXEL_PACK_BUFFER, 0, size, GL_MAP_READ_BIT)).WillOnce(Return(mapped_data.data())).RetiresOnSaturation();
    cmds::MapBufferRange map_buffer_range;
    map_buffer_range.Init(GL_PIXEL_PACK_BUFFER, 0, size, GL_MAP_READ_BIT, data_shm_id, data_shm_offset, result_shm_id, result_shm_offset);
...
TEST_P(GLES2DecoderManualInitTest, ReadPixels2RowLengthWorkaround)
{
    ...
    for (GLint ii = 0; ii < kHeight; ++ii) {
        if (ii + 1 == kHeight) {
            EXPECT_CALL(*gl_, PixelStorei(GL_PACK_ROW_LENGTH, kWidth)).Times(1).RetiresOnSaturation();
        }
        void* offset = reinterpret_cast<void*>(ii * kRowLength * kBytesPerPixel);
        EXPECT_CALL(*gl_, ReadPixels(0, ii, kWidth, 1, kFormat, kType, offset)).Times(1).RetiresOnSaturation();
        if (ii + 1 == kHeight) {
            EXPECT_CALL(*gl_, PixelStorei(GL_PACK_ROW_LENGTH, kRowLength)).Times(1).RetiresOnSaturation();
        }
    }
...
TEST_P(GLES2DecoderManualInitTest, ReadPixels2AlignmentWorkaround)
{
    ...
    EXPECT_CALL(*gl_, ReadPixels(0, 0, kWidth, kHeight - 1, kFormat, kType, offset)).Times(1).RetiresOnSaturation();
    EXPECT_CALL(*gl_, PixelStorei(GL_PACK_ALIGNMENT, 1)).Times(1).RetiresOnSaturation();
    offset += (kWidth * kBytesPerPixel + kPadding) * (kHeight - 1);
    EXPECT_CALL(*gl_, ReadPixels(0, kHeight - 1, kWidth, 1, kFormat, kType, offset)).Times(1).RetiresOnSaturation();
    EXPECT_CALL(*gl_, PixelStorei(GL_PACK_ALIGNMENT, kAlignment)).Times(1).RetiresOnSaturation();
...
TEST_P(GLES2DecoderManualInitTest, ReadPixels2RowLengthAndAlignmentWorkarounds)
{
    gpu::GpuDriverBugWorkarounds workarounds;
    workarounds.pack_parameters_workaround_with_pack_buffer = true;
    InitState init;
...
TEST_P(GLES2DecoderTest, ReadPixelsOutOfRange)
{
    ...
    for (size_t tt = 0; tt < std::size(tests); ++tt) {
        CheckReadPixelsOutOfRange(tests[tt][0], tests[tt][1], tests[tt][2], tests[tt][3], tt == 0);
    }
}
...
TEST_P(GLES2DecoderTest, ReadPixelsInvalidArgs)
{
    ...
    EXPECT_CALL(*gl_, ReadPixels(_, _, _, _, _, _, _)).Times(0);
    cmds::ReadPixels cmd;
    cmd.Init(0, 0, -1, 1, GL_RGB, GL_UNSIGNED_BYTE, pixels_shm_id, pixels_shm_offset, result_shm_id, result_shm_offset, false);
    result->success = 0;
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(GL_INVALID_VALUE, GetGLError());
    cmd.Init(0, 0, 1, -1, GL_RGB, GL_UNSIGNED_BYTE, pixels_shm_id, pixels_shm_offset, result_shm_id, result_shm_offset, false);
    ...
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(GL_INVALID_VALUE, GetGLError());
    cmd.Init(0, 0, 1, 1, GL_RGB, GL_INT, pixels_shm_id, pixels_shm_offset, result_shm_id, result_shm_offset, false);
    result->success = 0;
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(GL_INVALID_ENUM, GetGLError());
    cmd.Init(0, 0, 1, 1, GL_RGB, GL_UNSIGNED_BYTE, kInvalidSharedMemoryId, pixels_shm_offset, result_shm_id, result_shm_offset, false);
    result->success = 0;
    EXPECT_NE(error::kNoError, ExecuteCmd(cmd));
    cmd.Init(0, 0, 1, 1, GL_RGB, GL_UNSIGNED_BYTE, pixels_shm_id, kInvalidSharedMemoryOffset, result_shm_id, result_shm_offset, false);
    EXPECT_NE(error::kNoError, ExecuteCmd(cmd));
    cmd.Init(0, 0, 1, 1, GL_RGB, GL_UNSIGNED_BYTE, pixels_shm_id, pixels_shm_offset, kInvalidSharedMemoryId, result_shm_offset, false);
    result->success = 0;
    EXPECT_NE(error::kNoError, ExecuteCmd(cmd));
    cmd.Init(0, 0, 1, 1, GL_RGB, GL_UNSIGNED_BYTE, pixels_shm_id, pixels_shm_offset, result_shm_id, kInvalidSharedMemoryOffset, false);
    result->success = 0;
    EXPECT_NE(error::kNoError, ExecuteCmd(cmd));
}
...
TEST_P(GLES2DecoderManualInitTest, ReadPixelsAsyncError)
{
    ...
    EXPECT_CALL(*gl_, ReadPixels(0, 0, kWidth, kHeight, GL_RGBA, GL_UNSIGNED_BYTE, _)).Times(1);
    ...
    EXPECT_CALL(*gl_, BufferData(GL_PIXEL_PACK_BUFFER_ARB, _, nullptr, GL_STREAM_READ)).Times(1);

    cmds::ReadPixels cmd;
    cmd.Init(0, 0, kWidth, kHeight, GL_RGBA, GL_UNSIGNED_BYTE, pixels_shm_id, pixels_shm_offset, result_shm_id, result_shm_offset, true);
    result->success = 0;
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
}
...
public:
    ...
    void SetupReadPixelsAsyncExpectation(GLsizei width, GLsizei height)
    {
        ...
        EXPECT_CALL(*gl_, GetError()).WillRepeatedly(Return(GL_NO_ERROR)).RetiresOnSaturation();

        EXPECT_CALL(*gl_, ReadPixels(0, 0, width, height, GL_RGBA, GL_UNSIGNED_BYTE, _)).Times(1);
        EXPECT_CALL(*gl_, GenBuffersARB(1, _)).WillOnce(SetArgPointee<1>(kServiceBufferId)).RetiresOnSaturation();
        EXPECT_CALL(*gl_, BindBuffer(GL_PIXEL_PACK_BUFFER_ARB, kServiceBufferId)).Times(1);
        EXPECT_CALL(*gl_, BindBuffer(GL_PIXEL_PACK_BUFFER_ARB, 0)).Times(1);
        EXPECT_CALL(*gl_, BufferData(GL_PIXEL_PACK_BUFFER_ARB, kBufferSize, nullptr, GL_STREAM_READ)).Times(1);
    ...
    void FinishReadPixelsAndCheckResult(GLsizei width, GLsizei height, char* pixels)
    {
        ...
        GLsync sync = reinterpret_cast<GLsync>(kServiceSyncId);
        EXPECT_CALL(*gl_, ClientWaitSync(sync, 0, 0)).WillOnce(Return(GL_CONDITION_SATISFIED));
        EXPECT_CALL(*gl_, BindBuffer(GL_PIXEL_PACK_BUFFER_ARB, kServiceBufferId)).Times(1);
        EXPECT_CALL(*gl_, MapBufferRange(GL_PIXEL_PACK_BUFFER_ARB, 0, kBufferSize, GL_MAP_READ_BIT)).WillOnce(Return(buffer.get())).RetiresOnSaturation();
...
TEST_P(GLES2ReadPixelsAsyncTest, ReadPixelsAsync)
{
    ...
    GLsync sync = reinterpret_cast<GLsync>(kServiceSyncId);
    EXPECT_CALL(*gl_, ClientWaitSync(sync, 0, 0)).WillOnce(Return(GL_TIMEOUT_EXPIRED));
    decoder_->PerformIdleWork();
...
TEST_P(GLES2ReadPixelsAsyncTest, ReadPixelsAsyncModifyCommand)
{
    ...
    cmd.Init(1, 2, 6, 7, GL_RGB, GL_UNSIGNED_SHORT, pixels_shm_id, pixels_shm_offset, result_shm_id, result_shm_offset, false);
...
TEST_P(GLES2DecoderTest, FramebufferRenderbufferClearColor)
{
    ...
    cmd.Init(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_RENDERBUFFER, client_renderbuffer_id_);
    InSequence sequence;
    EXPECT_CALL(*gl_, ClearColor(0.1f, 0.2f, 0.3f, 0.4f)).Times(1).RetiresOnSaturation();
...
TEST_P(GLES2DecoderTest, FramebufferRenderbufferClearDepth)
{
    ...
    cmd.Init(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, client_renderbuffer_id_);
    InSequence sequence;
    EXPECT_CALL(*gl_, ClearDepth(0.5f)).Times(1).RetiresOnSaturation();
    EXPECT_CALL(*gl_, GetError()).WillOnce(Return(GL_NO_ERROR)).RetiresOnSaturation();
    EXPECT_CALL(*gl_, FramebufferRenderbufferEXT(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, kServiceRenderbufferId)).Times(1).RetiresOnSaturation();
    EXPECT_CALL(*gl_, GetError()).WillOnce(Return(GL_NO_ERROR)).RetiresOnSaturation();
...
TEST_P(GLES2DecoderTest, FramebufferRenderbufferClearStencil)
{
    ...
    cmd.Init(GL_FRAMEBUFFER, GL_STENCIL_ATTACHMENT, GL_RENDERBUFFER, client_renderbuffer_id_);
    InSequence sequence;
    EXPECT_CALL(*gl_, ClearStencil(123)).Times(1).RetiresOnSaturation();
    EXPECT_CALL(*gl_, GetError()).WillOnce(Return(GL_NO_ERROR)).RetiresOnSaturation();
    EXPECT_CALL(*gl_, FramebufferRenderbufferEXT(GL_FRAMEBUFFER, GL_STENCIL_ATTACHMENT, GL_RENDERBUFFER, kServiceRenderbufferId))
        .Times(1)
        .RetiresOnSaturation();
    EXPECT_CALL(*gl_, GetError()).WillOnce(Return(GL_NO_ERROR)).RetiresOnSaturation();
...
TEST_P(GLES3DecoderTest, FramebufferRenderbufferClearDepthStencil)
{
    ...
    depth_cmd.Init(0.5f);
    stencil_cmd.Init(123);
    cmd.Init(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, GL_RENDERBUFFER, client_renderbuffer_id_);
    InSequence sequence;
    EXPECT_CALL(*gl_, ClearDepth(0.5f)).Times(1).RetiresOnSaturation();
    EXPECT_CALL(*gl_, ClearStencil(123)).Times(1).RetiresOnSaturation();
    EXPECT_CALL(*gl_, GetError()).WillOnce(Return(GL_NO_ERROR)).RetiresOnSaturation();
...
TEST_P(GLES2DecoderManualInitTest, ActualAlphaMatchesRequestedAlpha)
{
    ...
    EXPECT_EQ(decoder_->GetGLES2Util()->GLGetNumValuesReturned(GL_ALPHA_BITS), result->GetNumResults());
    EXPECT_EQ(GL_NO_ERROR, GetGLError());
...
TEST_P(GLES2DecoderManualInitTest, PackedDepthStencilReportsCorrectValues)
{
    ...
    EXPECT_CALL(*gl_, GetIntegerv(GL_STENCIL_BITS, _)).WillOnce(SetArgPointee<1>(8)).RetiresOnSaturation();
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd2));
    EXPECT_EQ(decoder_->GetGLES2Util()->GLGetNumValuesReturned(GL_STENCIL_BITS), result->GetNumResults());
    EXPECT_EQ(GL_NO_ERROR, GetGLError());
    EXPECT_EQ(8, result->GetData()[0]);
    result->size = 0;
    cmd2.Init(GL_DEPTH_BITS, shared_memory_id_, shared_memory_offset_);
    EXPECT_CALL(*gl_, GetIntegerv(GL_DEPTH_BITS, _)).WillOnce(SetArgPointee<1>(24)).RetiresOnSaturation();
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd2));
    EXPECT_EQ(decoder_->GetGLES2Util()->GLGetNumValuesReturned(GL_DEPTH_BITS), result->GetNumResults());
    EXPECT_EQ(GL_NO_ERROR, GetGLError());
...
TEST_P(GLES2DecoderManualInitTest, PackedDepthStencilRenderbufferDepth)
{
    ...
    EXPECT_CALL(*gl_, RenderbufferStorageEXT(GL_RENDERBUFFER, GL_DEPTH24_STENCIL8, 100, 50)).Times(1).RetiresOnSaturation();
    cmds::RenderbufferStorage cmd;
    cmd.Init(GL_RENDERBUFFER, GL_DEPTH24_STENCIL8, 100, 50);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_CALL(*gl_, FramebufferRenderbufferEXT(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, kServiceRenderbufferId)).Times(1).RetiresOnSaturation();
    cmds::FramebufferRenderbuffer fbrb_cmd;
    fbrb_cmd.Init(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, client_renderbuffer_id_);
    EXPECT_EQ(error::kNoError, ExecuteCmd(fbrb_cmd));
...
TEST_P(GLES2DecoderManualInitTest, PackedDepthStencilRenderbufferStencil)
{
    ...
    EXPECT_CALL(*gl_, FramebufferRenderbufferEXT(GL_FRAMEBUFFER, GL_STENCIL_ATTACHMENT, GL_RENDERBUFFER, kServiceRenderbufferId))
        ...
        .RetiresOnSaturation();
    cmds::FramebufferRenderbuffer fbrb_cmd;
    fbrb_cmd.Init(GL_FRAMEBUFFER, GL_STENCIL_ATTACHMENT, GL_RENDERBUFFER, client_renderbuffer_id_);
    EXPECT_EQ(error::kNoError, ExecuteCmd(fbrb_cmd));
...
TEST_P(GLES2DecoderTest, FramebufferTexture2DGLError)
{
    ...
    DoBindTexture(GL_TEXTURE_2D, client_texture_id_, kServiceTextureId);
    DoTexImage2D(GL_TEXTURE_2D, 0, kFormat, kWidth, kHeight, 0, kFormat, GL_UNSIGNED_BYTE, 0, 0);
    DoBindFramebuffer(GL_FRAMEBUFFER, client_framebuffer_id_, kServiceFramebufferId);
    EXPECT_CALL(*gl_, GetError()).WillOnce(Return(GL_NO_ERROR)).WillOnce(Return(GL_OUT_OF_MEMORY)).RetiresOnSaturation();
    EXPECT_CALL(*gl_, FramebufferTexture2DEXT(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, kServiceTextureId, 0)).Times(1).RetiresOnSaturation();
    cmds::FramebufferTexture2D fbtex_cmd;
...
TEST_P(GLES2DecoderTest, RenderbufferStorageGLError)
{
    ...
    EnsureRenderbufferBound(false);
    EXPECT_CALL(*gl_, GetError()).WillOnce(Return(GL_NO_ERROR)).WillOnce(Return(GL_OUT_OF_MEMORY)).RetiresOnSaturation();
    EXPECT_CALL(*gl_, RenderbufferStorageEXT(GL_RENDERBUFFER, GL_RGBA4, 100, 50)).Times(1).RetiresOnSaturation();
    cmds::RenderbufferStorage cmd;
...
{
    DoBindRenderbuffer(GL_RENDERBUFFER, client_renderbuffer_id_, kServiceRenderbufferId);
    EXPECT_CALL(*gl_, RenderbufferStorageEXT(_, _, _, _)).Times(0).RetiresOnSaturation();
    cmds::RenderbufferStorage cmd;
...
TEST_P(GLES3DecoderTest, ClearBufferivImmediateValidArgs)
{
    ...
    DoFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_RENDERBUFFER, client_renderbuffer_id_, kServiceRenderbufferId, GL_NO_ERROR);

    // TODO(zmo): Set up expectations for the path where the attachment isn't
    // marked as cleared.
    Framebuffer* framebuffer = GetFramebuffer(client_framebuffer_id_);
    framebuffer->MarkAttachmentAsCleared(group().renderbuffer_manager(), nullptr, GL_COLOR_ATTACHMENT0, true);

    auto& cmd = *GetImmediateAs<cmds::ClearBufferivImmediate>();
    GLint temp[4] = { 0 };
    cmd.Init(GL_COLOR, 0, &temp[0]);
    EXPECT_CALL(*gl_, CheckFramebufferStatusEXT(GL_DRAW_FRAMEBUFFER)).WillOnce(Return(GL_FRAMEBUFFER_COMPLETE)).RetiresOnSaturation();
    SetupExpectationsForApplyingDirtyState(false, false, false, 0x1111, false, false, 0, 0, false);
...
TEST_P(GLES3DecoderTest, ClearBufferfvImmediateValidArgs)
{
    DoBindFramebuffer(GL_FRAMEBUFFER, client_framebuffer_id_, kServiceFramebufferId);
    DoBindRenderbuffer(GL_RENDERBUFFER, client_renderbuffer_id_, kServiceRenderbufferId);
    DoRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT32F, 1, 1, GL_NO_ERROR);
    DoFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, client_renderbuffer_id_, kServiceRenderbufferId, GL_NO_ERROR);

    // TODO(zmo): Set up expectations for the path where the attachment isn't
    // marked as cleared.
    Framebuffer* framebuffer = GetFramebuffer(client_framebuffer_id_);
    framebuffer->MarkAttachmentAsCleared(group().renderbuffer_manager(), nullptr, GL_DEPTH_ATTACHMENT, true);
    ...
    EXPECT_CALL(*gl_, CheckFramebufferStatusEXT(GL_DRAW_FRAMEBUFFER)).WillOnce(Return(GL_FRAMEBUFFER_COMPLETE)).RetiresOnSaturation();
    SetupExpectationsForApplyingDirtyState(true, true, false, 0x1110, true, true, 0, 0, false);
...
TEST_P(GLES3DecoderTest, ClearBufferfiValidArgs)
{
    DoBindFramebuffer(GL_FRAMEBUFFER, client_framebuffer_id_, kServiceFramebufferId);
    DoBindRenderbuffer(GL_RENDERBUFFER, client_renderbuffer_id_, kServiceRenderbufferId);
    DoRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH24_STENCIL8, 1, 1, GL_NO_ERROR);
    DoFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, GL_RENDERBUFFER, client_renderbuffer_id_, kServiceRenderbufferId, GL_NO_ERROR);

    // TODO(zmo): Set up expectations for the path where the attachment isn't
    // marked as cleared.
    Framebuffer* framebuffer = GetFramebuffer(client_framebuffer_id_);
    framebuffer->MarkAttachmentAsCleared(group().renderbuffer_manager(), nullptr, GL_DEPTH_ATTACHMENT, true);
    framebuffer->MarkAttachmentAsCleared(group().renderbuffer_manager(), nullptr, GL_STENCIL_ATTACHMENT, true);
    ...
    EXPECT_CALL(*gl_, CheckFramebufferStatusEXT(GL_DRAW_FRAMEBUFFER)).WillOnce(Return(GL_FRAMEBUFFER_COMPLETE)).RetiresOnSaturation();
    SetupExpectationsForApplyingDirtyState(true, true, true, 0x1110, true, true, GLES2Decoder::kDefaultStencilMask, GLES2Decoder::kDefaultStencilMask, true);
    EXPECT_CALL(*gl_, ClearBufferfi(GL_DEPTH_STENCIL, 0, 1.0f, 0));
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(GL_NO_ERROR, GetGLError());
}

TEST_P(GLES2DecoderManualInitTest, RenderbufferStorageMultisampleCHROMIUMGLError)
{
    InitState init;
    init.gl_version = "OpenGL ES 3.0";
    init.bind_generates_resource = true;
    ...
    EnsureRenderbufferBound(false);
    EXPECT_CALL(*gl_, GetError()).WillOnce(Return(GL_NO_ERROR)).WillOnce(Return(GL_OUT_OF_MEMORY)).RetiresOnSaturation();
    SetupExpectationsForInternalFormatSampleCountsHelper(GL_RENDERBUFFER, GL_RGBA4, 1, TestHelper::kMaxSamples);
    EXPECT_CALL(*gl_, RenderbufferStorageMultisample(GL_RENDERBUFFER, 1, GL_RGBA4, 100, 50)).Times(1).RetiresOnSaturation();
...
TEST_P(GLES2DecoderManualInitTest, RenderbufferStorageMultisampleCHROMIUMBadArgs)
{
    InitState init;
    init.gl_version = "OpenGL ES 3.0";
    init.bind_generates_resource = true;
    InitDecoder(init);
    DoBindRenderbuffer(GL_RENDERBUFFER, client_renderbuffer_id_, kServiceRenderbufferId);
    EXPECT_CALL(*gl_, RenderbufferStorageMultisample(_, _, _, _, _)).Times(0).RetiresOnSaturation();
    cmds::RenderbufferStorageMultisampleCHROMIUM cmd;
    cmd.Init(GL_RENDERBUFFER, TestHelper::kMaxSamples + 1, GL_RGBA4, TestHelper::kMaxRenderbufferSize, 1);
    SetupExpectationsForInternalFormatSampleCountsHelper(GL_RENDERBUFFER, GL_RGBA4, 1, TestHelper::kMaxSamples);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(GL_INVALID_OPERATION, GetGLError());

    cmd.Init(GL_RENDERBUFFER, TestHelper::kMaxSamples, GL_RGBA4, TestHelper::kMaxRenderbufferSize + 1, 1);
    SetupExpectationsForInternalFormatSampleCountsHelper(GL_RENDERBUFFER, GL_RGBA4, 1, TestHelper::kMaxSamples);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(GL_INVALID_VALUE, GetGLError());

    cmd.Init(GL_RENDERBUFFER, TestHelper::kMaxSamples, GL_RGBA4, 1, TestHelper::kMaxRenderbufferSize + 1);
    SetupExpectationsForInternalFormatSampleCountsHelper(GL_RENDERBUFFER, GL_RGBA4, 1, TestHelper::kMaxSamples);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(GL_INVALID_VALUE, GetGLError());
}
...
TEST_P(GLES2DecoderManualInitTest, RenderbufferStorageMultisampleCHROMIUM)
{
    ...
    DoBindRenderbuffer(GL_RENDERBUFFER, client_renderbuffer_id_, kServiceRenderbufferId);
    InSequence sequence;
    DoRenderbufferStorageMultisampleCHROMIUM(GL_RENDERBUFFER, TestHelper::kMaxSamples, GL_RGBA4, TestHelper::kMaxRenderbufferSize, 1, false);
}

TEST_P(GLES2DecoderManualInitTest, RenderbufferStorageMultisampleCHROMIUMRebindRenderbuffer)
{
    InitState init;
    init.gl_version = "OpenGL ES 3.0";
    InitDecoder(init);
    ...
    RestoreRenderbufferBindings();
    InSequence sequence;
    DoRenderbufferStorageMultisampleCHROMIUM(GL_RENDERBUFFER, TestHelper::kMaxSamples, GL_RGBA4, TestHelper::kMaxRenderbufferSize, 1, true);
}

TEST_P(GLES2DecoderManualInitTest, RenderbufferStorageMultisampleEXTNotSupported)
{
    InitState init;
    init.gl_version = "OpenGL ES 3.0";
    init.bind_generates_resource = true;
    ...
    cmd.Init(GL_RENDERBUFFER, TestHelper::kMaxSamples, GL_RGBA4, TestHelper::kMaxRenderbufferSize, 1);
...
public:
    ...
    void TestRenderbufferStorageMultisampleEXT(const char* extension, bool rb_rebind)
    {
        DoBindRenderbuffer(GL_RENDERBUFFER, client_renderbuffer_id_, kServiceRenderbufferId);
        InSequence sequence;
        ...
        EXPECT_CALL(*gl_, RenderbufferStorageMultisampleEXT(GL_RENDERBUFFER, TestHelper::kMaxSamples, GL_RGBA4, TestHelper::kMaxRenderbufferSize, 1))
            .Times(1)
            .RetiresOnSaturation();
        EXPECT_CALL(*gl_, GetError()).WillOnce(Return(GL_NO_ERROR)).RetiresOnSaturation();
        cmds::RenderbufferStorageMultisampleEXT cmd;
...
INSTANTIATE_TEST_SUITE_P(Service, GLES2DecoderMultisampledRenderToTextureTest, ::testing::Bool());

TEST_P(GLES2DecoderMultisampledRenderToTextureTest, NotCompatibleWithRenderbufferStorageMultisampleCHROMIUM_EXT)
{
    Init("GL_EXT_multisampled_render_to_texture");
    TestNotCompatibleWithRenderbufferStorageMultisampleCHROMIUM();
}

TEST_P(GLES2DecoderMultisampledRenderToTextureTest, NotCompatibleWithRenderbufferStorageMultisampleCHROMIUM_IMG)
{
    Init("GL_IMG_multisampled_render_to_texture");
    TestNotCompatibleWithRenderbufferStorageMultisampleCHROMIUM();
}

TEST_P(GLES2DecoderMultisampledRenderToTextureTest, RenderbufferStorageMultisampleEXT_EXT)
{
    Init("GL_EXT_multisampled_render_to_texture");
    TestRenderbufferStorageMultisampleEXT("GL_EXT_multisampled_render_to_texture", false);
}

TEST_P(GLES2DecoderMultisampledRenderToTextureTest, RenderbufferStorageMultisampleEXT_IMG)
{
    Init("GL_IMG_multisampled_render_to_texture");
    TestRenderbufferStorageMultisampleEXT("GL_IMG_multisampled_render_to_texture", false);
}

TEST_P(GLES2DecoderMultisampledRenderToTextureTest, RenderbufferStorageMultisampleEXT_EXT_RebindRenderbuffer)
{
    Init("GL_EXT_multisampled_render_to_texture");
    TestRenderbufferStorageMultisampleEXT("GL_EXT_multisampled_render_to_texture", true);
}

TEST_P(GLES2DecoderMultisampledRenderToTextureTest, RenderbufferStorageMultisampleEXT_IMG_RebindRenderbuffer)
{
    Init("GL_IMG_multisampled_render_to_texture");
    TestRenderbufferStorageMultisampleEXT("GL_IMG_multisampled_render_to_texture", true);
}
...
TEST_P(GLES2DecoderTest, ReadPixelsGLError)
{
    ...
    uint32_t pixels_shm_offset = kSharedMemoryOffset + sizeof(*result);
    EXPECT_CALL(*gl_, GetError()).WillOnce(Return(GL_NO_ERROR)).WillOnce(Return(GL_OUT_OF_MEMORY)).RetiresOnSaturation();
    EXPECT_CALL(*gl_, ReadPixels(x, y, width, height, kFormat, GL_UNSIGNED_BYTE, _)).Times(1).RetiresOnSaturation();
    cmds::ReadPixels cmd;
    cmd.Init(x, y, width, height, kFormat, GL_UNSIGNED_BYTE, pixels_shm_id, pixels_shm_offset, result_shm_id, result_shm_offset, false);
    result->success = 0;
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(GL_OUT_OF_MEMORY, GetGLError());
}
...
TEST_P(GLES2DecoderWithShaderTest, UnClearedAttachmentsGetClearedOnClear)
{
    ...
    EXPECT_CALL(*gl_, GenTextures(_, _)).WillOnce(SetArgPointee<1>(kFBOServiceTextureId)).RetiresOnSaturation();
    GenHelper<cmds::GenTexturesImmediate>(kFBOClientTextureId);

    // Setup "render to" texture.
    DoBindTexture(GL_TEXTURE_2D, kFBOClientTextureId, kFBOServiceTextureId);
    DoTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 1, 1, 0, GL_RGBA, GL_UNSIGNED_BYTE, 0, 0);
    DoBindFramebuffer(GL_FRAMEBUFFER, client_framebuffer_id_, kServiceFramebufferId);
    DoFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, kFBOClientTextureId, kFBOServiceTextureId, 0, GL_NO_ERROR);
...
TEST_P(GLES2DecoderWithShaderTest, UnClearedAttachmentsGetClearedOnReadPixels)
{
    ...
    EXPECT_CALL(*gl_, GetError()).WillOnce(Return(GL_NO_ERROR)).WillOnce(Return(GL_NO_ERROR)).RetiresOnSaturation();
    EXPECT_CALL(*gl_, ReadPixels(0, 0, 1, 1, GL_RGBA, GL_UNSIGNED_BYTE, _)).Times(1).RetiresOnSaturation();
    auto* result = GetSharedMemoryAs<cmds::ReadPixels::Result*>();
    uint32_t result_shm_id = shared_memory_id_;
    uint32_t result_shm_offset = kSharedMemoryOffset;
    uint32_t pixels_shm_id = shared_memory_id_;
    uint32_t pixels_shm_offset = kSharedMemoryOffset + sizeof(*result);
    cmds::ReadPixels cmd;
    cmd.Init(0, 0, 1, 1, GL_RGBA, GL_UNSIGNED_BYTE, pixels_shm_id, pixels_shm_offset, result_shm_id, result_shm_offset, false);
    result->success = 0;
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(GL_NO_ERROR, GetGLError());
}
...
TEST_P(GLES3DecoderTest, CopyTexImage2DValidInternalFormat)
{
    ...
    DoBindTexture(GL_TEXTURE_2D, kFBOClientTextureId, kFBOServiceTextureId);
    DoTexImage2D(GL_TEXTURE_2D, level, internal_format, width, height, 0, format, type, shared_memory_id_, kSharedMemoryOffset);
    DoBindFramebuffer(GL_READ_FRAMEBUFFER, client_framebuffer_id_, kServiceFramebufferId);
    DoFramebufferTexture2D(GL_READ_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, kFBOClientTextureId, kFBOServiceTextureId, 0, GL_NO_ERROR);
    EXPECT_CALL(*gl_, CheckFramebufferStatusEXT(GL_READ_FRAMEBUFFER)).WillOnce(Return(GL_FRAMEBUFFER_COMPLETE)).RetiresOnSaturation();
...
TEST_P(GLES3DecoderManualInitTest, CopyTexImage2DValidInternalFormat_FloatEXT)
{
    ...
    DoBindTexture(GL_TEXTURE_2D, kFBOClientTextureId, kFBOServiceTextureId);
    DoTexImage2D(GL_TEXTURE_2D, level, GL_RGBA16F, width, height, 0, format, type, shared_memory_id_, kSharedMemoryOffset);
    DoBindFramebuffer(GL_READ_FRAMEBUFFER, client_framebuffer_id_, kServiceFramebufferId);
    DoFramebufferTexture2D(GL_READ_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, kFBOClientTextureId, kFBOServiceTextureId, 0, GL_NO_ERROR);
    EXPECT_CALL(*gl_, CopyTexImage2D(target, level, internal_format, 0, 0, width, height, border)).Times(1).RetiresOnSaturation();
    EXPECT_CALL(*gl_, GetError()).WillOnce(Return(GL_NO_ERROR)).WillOnce(Return(GL_NO_ERROR)).RetiresOnSaturation();
...
TEST_P(GLES3DecoderManualInitTest, CopyTexImage2DInvalidInternalFormat_FloatEXT)
{
    InitState init;
    init.extensions = "GL_EXT_color_buffer_float";
    init.gl_version = "OpenGL ES 3.0";
    ...
    DoBindTexture(GL_TEXTURE_2D, kFBOClientTextureId, kFBOServiceTextureId);
    DoTexImage2D(GL_TEXTURE_2D, level, GL_RGBA8, width, height, 0, format, type, shared_memory_id_, kSharedMemoryOffset);
    DoBindFramebuffer(GL_READ_FRAMEBUFFER, client_framebuffer_id_, kServiceFramebufferId);
    DoFramebufferTexture2D(GL_READ_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, kFBOClientTextureId, kFBOServiceTextureId, 0, GL_NO_ERROR);
    EXPECT_CALL(*gl_, CopyTexImage2D(target, level, internal_format, 0, 0, width, height, border)).Times(0);
    EXPECT_CALL(*gl_, CheckFramebufferStatusEXT(GL_READ_FRAMEBUFFER)).WillOnce(Return(GL_FRAMEBUFFER_COMPLETE)).RetiresOnSaturation();
    DoBindTexture(GL_TEXTURE_2D, client_texture_id_, kServiceTextureId);
    cmds::CopyTexImage2D cmd;
    cmd.Init(target, level, internal_format, 0, 0, width, height);
...
TEST_P(GLES3DecoderTest, CopyTexImage2DInvalidInternalFormat)
{
    ...
    DoBindTexture(GL_TEXTURE_2D, kFBOClientTextureId, kFBOServiceTextureId);
    DoTexImage2D(GL_TEXTURE_2D, level, GL_RG8, width, height, 0, format, type, shared_memory_id_, kSharedMemoryOffset);
    DoBindFramebuffer(GL_READ_FRAMEBUFFER, client_framebuffer_id_, kServiceFramebufferId);
    DoFramebufferTexture2D(GL_READ_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, kFBOClientTextureId, kFBOServiceTextureId, 0, GL_NO_ERROR);
    EXPECT_CALL(*gl_, CopyTexImage2D(target, level, internal_format, 0, 0, width, height, border)).Times(0);
    DoBindTexture(GL_TEXTURE_2D, client_texture_id_, kServiceTextureId);
...
TEST_P(GLES3DecoderTest, CopyTexImage2DInvalidInternalFormat_Integer)
{
    ...
    DoBindTexture(GL_TEXTURE_2D, kFBOClientTextureId, kFBOServiceTextureId);
    DoTexImage2D(GL_TEXTURE_2D, level, GL_RG8UI, width, height, 0, format, type, shared_memory_id_, kSharedMemoryOffset);
    DoBindFramebuffer(GL_READ_FRAMEBUFFER, client_framebuffer_id_, kServiceFramebufferId);
    DoFramebufferTexture2D(GL_READ_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, kFBOClientTextureId, kFBOServiceTextureId, 0, GL_NO_ERROR);
    EXPECT_CALL(*gl_, CopyTexImage2D(target, level, internal_format, 0, 0, width, height, border)).Times(0);
    EXPECT_CALL(*gl_, CheckFramebufferStatusEXT(GL_READ_FRAMEBUFFER)).WillOnce(Return(GL_FRAMEBUFFER_COMPLETE)).RetiresOnSaturation();
...
TEST_P(GLES3DecoderTest, CopyTexImage2DInvalidInternalFormat_sRGB)
{
    ...
    DoBindTexture(GL_TEXTURE_2D, kFBOClientTextureId, kFBOServiceTextureId);
    DoTexImage2D(GL_TEXTURE_2D, level, GL_RGB, width, height, 0, format, type, shared_memory_id_, kSharedMemoryOffset);
    DoBindFramebuffer(GL_READ_FRAMEBUFFER, client_framebuffer_id_, kServiceFramebufferId);
    DoFramebufferTexture2D(GL_READ_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, kFBOClientTextureId, kFBOServiceTextureId, 0, GL_NO_ERROR);
    EXPECT_CALL(*gl_, CopyTexImage2D(target, level, internal_format, 0, 0, width, height, border)).Times(0);
    EXPECT_CALL(*gl_, CheckFramebufferStatusEXT(GL_READ_FRAMEBUFFER)).WillOnce(Return(GL_FRAMEBUFFER_COMPLETE)).RetiresOnSaturation();
...
TEST_P(GLES2DecoderManualInitTest, UnClearedAttachmentsGetClearedOnReadPixelsAndDrawBufferGetsRestored)
{
    ...
    SetupExpectationsForFramebufferClearingMulti(kServiceFramebufferId, // read framebuffer service id
...
TEST_P(GLES2DecoderWithShaderTest, CopyTexImageWithInCompleteFBOFails)
{
    ...
    EXPECT_CALL(*gl_, CopyTexImage2D(_, _, _, _, _, _, _, _)).Times(0).RetiresOnSaturation();
    cmds::CopyTexImage2D cmd;
    cmd.Init(target, level, internal_format, 0, 0, width, height);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
...
void GLES2DecoderWithShaderTest::CheckRenderbufferChangesMarkFBOAsNotComplete(bool bound_fbo)
{
    ...
    Framebuffer* framebuffer = framebuffer_manager->GetFramebuffer(client_framebuffer_id_);
    ...
    // Cleanup
    DoDeleteFramebuffer(client_framebuffer_id_, kServiceFramebufferId, bound_fbo, GL_FRAMEBUFFER, 0, bound_fbo, GL_FRAMEBUFFER, 0);
}

TEST_P(GLES2DecoderWithShaderTest, RenderbufferChangesMarkFBOAsNotCompleteBoundFBO)
{
    CheckRenderbufferChangesMarkFBOAsNotComplete(true);
}

TEST_P(GLES2DecoderWithShaderTest, RenderbufferChangesMarkFBOAsNotCompleteUnboundFBO)
{
    CheckRenderbufferChangesMarkFBOAsNotComplete(false);
}

void GLES2DecoderWithShaderTest::CheckTextureChangesMarkFBOAsNotComplete(bool bound_fbo)
{
    FramebufferManager* framebuffer_manager = GetFramebufferManager();
    const GLuint kFBOClientTextureId = 4100;
    const GLuint kFBOServiceTextureId = 4101;
    ...
    DoBindRenderbuffer(GL_RENDERBUFFER, client_renderbuffer_id_, kServiceRenderbufferId);
    DoBindFramebuffer(GL_FRAMEBUFFER, client_framebuffer_id_, kServiceFramebufferId);
    DoRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT16, 1, 1, GL_NO_ERROR);
    DoFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, client_renderbuffer_id_, kServiceRenderbufferId, GL_NO_ERROR);
    ...
    DoTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, 1, 1, 0, GL_RGB, GL_UNSIGNED_BYTE, 0, 0);
    EXPECT_FALSE(framebuffer_manager->IsComplete(framebuffer));
    framebuffer_manager->MarkAsComplete(framebuffer);
    EXPECT_TRUE(framebuffer_manager->IsComplete(framebuffer));
    ...
    EXPECT_CALL(*gl_, GetError()).WillOnce(Return(GL_NO_ERROR)).RetiresOnSaturation();
    EXPECT_CALL(*gl_, CopyTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, 0, 0, 1, 1, 0)).Times(1).RetiresOnSaturation();
    EXPECT_CALL(*gl_, GetError()).WillOnce(Return(GL_NO_ERROR)).RetiresOnSaturation();
    cmds::CopyTexImage2D cmd;
...
TEST_P(GLES2DecoderWithShaderTest, TextureChangesMarkFBOAsNotCompleteUnboundFBO)
{
...
TEST_P(GLES2DecoderTest, CanChangeSurface)
{
    ...
    EXPECT_CALL(*other_surface.get(), GetBackingFramebufferObject()).WillOnce(Return(7));
...
TEST_P(GLES3DecoderTest, DrawBuffersEXTMainFramebuffer)
{
    auto& cmd = *GetImmediateAs<cmds::DrawBuffersEXTImmediate>();
    {
        ...
        EXPECT_CALL(*gl_, DrawBuffersARB(count, Pointee(GL_BACK))).Times(1).RetiresOnSaturation();
        EXPECT_EQ(error::kNoError, ExecuteImmediateCmd(cmd, sizeof(bufs)));
        EXPECT_EQ(GL_NO_ERROR, GetGLError());
    }
    {
...
TEST_P(GLES2DecoderManualInitTest, InvalidateFramebufferBinding)
{
    ...
    EXPECT_TRUE(gl::MockGLInterface::GetGLProcAddress("glInvalidateFramebuffer")
        != reinterpret_cast<gl::GLFunctionPointerType>(gl::g_current_gl_driver->fn.glDiscardFramebufferEXTFn));
    EXPECT_TRUE(gl::MockGLInterface::GetGLProcAddress("glInvalidateFramebuffer") != gl::MockGLInterface::GetGLProcAddress("glDiscardFramebufferEXT"));
}
...
TEST_P(GLES2DecoderTest, ClearBackbufferBitsOnFlipSwap)
{
    ...
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(GL_NO_ERROR, GetGLError());
    EXPECT_EQ(static_cast<uint32_t>(GL_COLOR_BUFFER_BIT), GetAndClearBackbufferClearBitsForTest());
...
TEST_P(GLES2DecoderManualInitTest, DiscardFramebufferEXT)
{
    ...
    // EXPECT_EQ can't be used to compare function pointers
    EXPECT_TRUE(gl::MockGLInterface::GetGLProcAddress("glDiscardFramebufferEXT")
        == reinterpret_cast<gl::GLFunctionPointerType>(gl::g_current_gl_driver->fn.glDiscardFramebufferEXTFn));
    ...
    EXPECT_CALL(*gl_, DiscardFramebufferEXT(target, count, _)).Times(1).RetiresOnSaturation();
...
TEST_P(GLES2DecoderManualInitTest, ClearBackbufferBitsOnDiscardFramebufferEXT)
{
    ...
    EXPECT_EQ(error::kNoError, ExecuteImmediateCmd(cmd, sizeof(attachments)));
    EXPECT_EQ(GL_NO_ERROR, GetGLError());
    EXPECT_EQ(static_cast<uint32_t>(GL_DEPTH_BUFFER_BIT), GetAndClearBackbufferClearBitsForTest());
    ...
    EXPECT_EQ(error::kNoError, ExecuteImmediateCmd(cmd, sizeof(attachments)));
    EXPECT_EQ(GL_NO_ERROR, GetGLError());
    EXPECT_EQ(static_cast<uint32_t>(GL_STENCIL_BUFFER_BIT), GetAndClearBackbufferClearBitsForTest());

    const GLsizei count0 = 3;
    const GLenum attachments0[] = { GL_COLOR_EXT, GL_DEPTH_EXT, GL_STENCIL_EXT };
    EXPECT_CALL(*gl_, DiscardFramebufferEXT(target, count0, _)).Times(1).RetiresOnSaturation();
    cmd.Init(target, count0, attachments0);
    EXPECT_EQ(error::kNoError, ExecuteImmediateCmd(cmd, sizeof(attachments0)));
    EXPECT_EQ(GL_NO_ERROR, GetGLError());
    EXPECT_EQ(static_cast<uint32_t>(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT | GL_STENCIL_BUFFER_BIT), GetAndClearBackbufferClearBitsForTest());
}
...
TEST_P(GLES2DecoderTest, DiscardFramebufferEXTUnsupported)
{
    ...
    EXPECT_EQ(error::kUnknownCommand, ExecuteImmediateCmd(cmd, sizeof(attachments)));
}
...
TEST_P(GLES3DecoderTest, DiscardFramebufferEXTUseCorrectTarget)
{
    ...
    SetupTexture();
    DoBindFramebuffer(GL_READ_FRAMEBUFFER, client_framebuffer_id_, kServiceFramebufferId);
    DoFramebufferTexture2D(GL_READ_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, client_texture_id_, kServiceTextureId, 0, GL_NO_ERROR);

    EXPECT_CALL(*gl_, GenFramebuffersEXT(_, _)).WillOnce(SetArgPointee<1>(kServiceFramebufferId + 1)).RetiresOnSaturation();
    DoBindFramebuffer(GL_DRAW_FRAMEBUFFER, client_framebuffer_id_ + 1, kServiceFramebufferId + 1);
    EXPECT_CALL(*gl_, GenTextures(_, _)).WillOnce(SetArgPointee<1>(kServiceTextureId + 1)).RetiresOnSaturation();
    ...
    DoFramebufferTexture2D(GL_DRAW_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, client_texture_id_ + 1, kServiceTextureId + 1, 0, GL_NO_ERROR);

    FramebufferManager* framebuffer_manager = GetFramebufferManager();
    Framebuffer* framebuffer = framebuffer_manager->GetFramebuffer(client_framebuffer_id_);
    EXPECT_TRUE(framebuffer->IsCleared());
    Framebuffer* other_framebuffer = framebuffer_manager->GetFramebuffer(client_framebuffer_id_ + 1);
    EXPECT_TRUE(other_framebuffer->IsCleared());

    EXPECT_CALL(*gl_, InvalidateFramebuffer(target, count, _)).Times(1).RetiresOnSaturation();
    auto& cmd = *GetImmediateAs<cmds::DiscardFramebufferEXTImmediate>();
    cmd.Init(target, count, attachments);
...
TEST_P(GLES2DecoderManualInitTest, DiscardedAttachmentsEXTMarksFramebufferIncomplete)
{
    InitState init;
    init.extensions = "GL_EXT_discard_framebuffer";
    init.gl_version = "OpenGL ES 2.0";
    ...
    EXPECT_CALL(*gl_, DiscardFramebufferEXT(target, count, _)).Times(1).RetiresOnSaturation();
    EXPECT_EQ(error::kNoError, ExecuteImmediateCmd(discard_cmd, sizeof(attachments)));
    EXPECT_EQ(GL_NO_ERROR, GetGLError());
    EXPECT_FALSE(framebuffer->IsCleared());
    EXPECT_FALSE(framebuffer_manager->IsComplete(framebuffer));
}
...
TEST_P(GLES3DecoderTest, ImplementationReadColorFormatAndType)
{
    ...
    result->size = 0;
    EXPECT_CALL(*gl_, GetError()).WillOnce(Return(GL_NO_ERROR)).WillOnce(Return(GL_NO_ERROR)).RetiresOnSaturation();
    EXPECT_CALL(*gl_, GetIntegerv(GL_IMPLEMENTATION_COLOR_READ_FORMAT, _)).WillOnce(SetArgPointee<1>(GL_RGBA)).RetiresOnSaturation();
    cmd.Init(GL_IMPLEMENTATION_COLOR_READ_FORMAT, shared_memory_id_, shared_memory_offset_);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    ...
    result->size = 0;
    EXPECT_CALL(*gl_, GetError()).WillOnce(Return(GL_NO_ERROR)).WillOnce(Return(GL_NO_ERROR)).RetiresOnSaturation();
    EXPECT_CALL(*gl_, GetIntegerv(GL_IMPLEMENTATION_COLOR_READ_TYPE, _)).WillOnce(SetArgPointee<1>(GL_UNSIGNED_BYTE)).RetiresOnSaturation();
    cmd.Init(GL_IMPLEMENTATION_COLOR_READ_TYPE, shared_memory_id_, shared_memory_offset_);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(1, result->GetNumResults());
    EXPECT_EQ(GL_NO_ERROR, GetGLError());
}
...
TEST_P(GLES3DecoderTest, FramebufferTextureLayerValidArgs)
{
    DoBindFramebuffer(GL_FRAMEBUFFER, client_framebuffer_id_, kServiceFramebufferId);
    DoBindTexture(GL_TEXTURE_3D, client_texture_id_, kServiceTextureId);
    EXPECT_CALL(*gl_, FramebufferTextureLayer(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, kServiceTextureId, 4, 5)).Times(1).RetiresOnSaturation();
    cmds::FramebufferTextureLayer cmd;
    cmd.Init(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, client_texture_id_, 4, 5);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(GL_NO_ERROR, GetGLError());
...
TEST_P(GLES3DecoderTest, FramebufferTextureLayerDepthStencil)
{
    ...
    EXPECT_CALL(*gl_, FramebufferTextureLayer(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, kServiceTextureId, 4, 5)).Times(1).RetiresOnSaturation();
    cmds::FramebufferTextureLayer cmd;
    cmd.Init(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, client_texture_id_, 4, 5);
    EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
    EXPECT_EQ(GL_NO_ERROR, GetGLError());
    Framebuffer* framebuffer = GetFramebuffer(client_framebuffer_id_);
    ASSERT_TRUE(framebuffer);
...
TEST_P(GLES3DecoderTest, InvalidateFramebufferDepthStencilAttachment)
{
    ...
    EXPECT_CALL(*gl_, InvalidateFramebuffer(target, 0, _)).Times(1).RetiresOnSaturation();
    ...
    EXPECT_CALL(*gl_, InvalidateFramebuffer(target, 2, _)).Times(1).RetiresOnSaturation();
    cmd.Init(target, count, attachments);
...
TEST_P(GLES3DecoderTest, BlitFramebufferDisabledReadBuffer)
{
    ...
    DoBindFramebuffer(GL_DRAW_FRAMEBUFFER, client_framebuffer_id_, kServiceFramebufferId);
    DoBindRenderbuffer(GL_RENDERBUFFER, client_renderbuffer_id_, kServiceRenderbufferId);
    DoRenderbufferStorage(GL_RENDERBUFFER, GL_RGBA8, 1, 1, GL_NO_ERROR);
    DoFramebufferRenderbuffer(GL_DRAW_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_RENDERBUFFER, client_renderbuffer_id_, kServiceRenderbufferId, GL_NO_ERROR);

    EXPECT_CALL(*gl_, GenFramebuffersEXT(1, _)).WillOnce(SetArgPointee<1>(kNewServiceId)).RetiresOnSaturation();
    GLuint read_fbo = client_framebuffer_id_ + 1;
    DoBindFramebuffer(GL_READ_FRAMEBUFFER, read_fbo, kNewServiceId);
    DoFramebufferRenderbuffer(GL_READ_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_RENDERBUFFER, client_renderbuffer_id_, kServiceRenderbufferId, GL_NO_ERROR);
    {
...
TEST_P(GLES3DecoderTest, BlitFramebufferMissingDepthOrStencil)
{
    ...
    EXPECT_CALL(*gl_, GenRenderbuffersEXT(1, _)).WillOnce(SetArgPointee<1>(color_renderbuffer_service)).RetiresOnSaturation();
    DoBindRenderbuffer(GL_RENDERBUFFER, color_renderbuffer, color_renderbuffer_service);
    DoRenderbufferStorage(GL_RENDERBUFFER, GL_RGBA8, 1, 1, GL_NO_ERROR);
    DoBindFramebuffer(GL_DRAW_FRAMEBUFFER, client_framebuffer_id_, kServiceFramebufferId);
    DoFramebufferRenderbuffer(GL_DRAW_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, client_renderbuffer_id_, kServiceRenderbufferId, GL_NO_ERROR);
    DoFramebufferRenderbuffer(GL_DRAW_FRAMEBUFFER, GL_STENCIL_ATTACHMENT, GL_RENDERBUFFER, client_renderbuffer_id_, kServiceRenderbufferId, GL_NO_ERROR);
    DoFramebufferRenderbuffer(GL_DRAW_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_RENDERBUFFER, color_renderbuffer, color_renderbuffer_service, GL_NO_ERROR);

    EXPECT_CALL(*gl_, GenFramebuffersEXT(1, _)).WillOnce(SetArgPointee<1>(kNewServiceId)).RetiresOnSaturation();
    GLuint color_fbo = client_framebuffer_id_ + 1;
    DoBindFramebuffer(GL_READ_FRAMEBUFFER, color_fbo, kNewServiceId);
    DoFramebufferRenderbuffer(GL_READ_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_RENDERBUFFER, color_renderbuffer, color_renderbuffer_service, GL_NO_ERROR);

    {
        SetupExpectationsForFramebufferClearing(GL_DRAW_FRAMEBUFFER, // target
            GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT | GL_STENCIL_BUFFER_BIT, // clear bits
            0, 0, 0, 0, // color
            0, // stencil
            1.0f, // depth
            false, // scissor test
...
TEST_P(GLES2DecoderManualInitTest, MESAFramebufferFlipYExtensionEnabled)
{
    ...
    EXPECT_TRUE(feature_info()->validators()->framebuffer_parameter.IsValid(GL_FRAMEBUFFER_FLIP_Y_MESA));

    EXPECT_CALL(*gl_, FramebufferParameteri(_, _, _)).Times(1).RetiresOnSaturation();
...
TEST_P(GLES2DecoderManualInitTest, MESAFramebufferFlipYExtensionDisabled)
{
    ...
    EXPECT_FALSE(feature_info()->validators()->framebuffer_parameter.IsValid(GL_FRAMEBUFFER_FLIP_Y_MESA));

    EXPECT_CALL(*gl_, FramebufferParameteri(_, _, _)).Times(0).RetiresOnSaturation();
