...
GraphiteCacheController::GraphiteCacheController(
    skgpu::graphite::Recorder* recorder, skgpu::graphite::Context* context, DawnContextProvider* dawn_context_provider)
    : recorder_(recorder)
    , context_(context)
    , dawn_context_provider_(dawn_context_provider)
{
    ...
    timer_ = std::make_unique<base::RetainingOneShotTimer>(
        FROM_HERE, kCleanUpAllResourcesDelay, base::BindRepeating(&GraphiteCacheController::CleanUpAllResources, weak_ptr_factory_.GetWeakPtr()));
...
void GraphiteCacheController::ScheduleCleanup()
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    if (context_) {
        context_->performDeferredCleanup(std::chrono::seconds(kResourceNotUsedSinceDelay.InSeconds()));
    }
    auto* image_provider = static_cast<GraphiteImageProvider*>(recorder_->clientImageProvider());
    image_provider->PurgeImagesNotUsedSince(kResourceNotUsedSinceDelay);
    recorder_->performDeferredCleanup(std::chrono::seconds(kResourceNotUsedSinceDelay.InSeconds()));
