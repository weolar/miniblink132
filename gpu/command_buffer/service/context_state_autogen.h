...
inline void SetDeviceCapabilityState(GLenum cap, bool enable)
{
    ...
    case GL_FRAMEBUFFER_SRGB_EXT:
        if (enable_flags.cached_framebuffer_srgb_ext == enable && !ignore_cached_state)
            return;
    ...
    case GL_SAMPLE_ALPHA_TO_COVERAGE:
        if (enable_flags.cached_sample_alpha_to_coverage == enable && !ignore_cached_state)
            return;
    ...
    case GL_RASTERIZER_DISCARD:
        if (enable_flags.cached_rasterizer_discard == enable && !ignore_cached_state)
            return;
        enable_flags.cached_rasterizer_discard = enable;
        break;
    case GL_PRIMITIVE_RESTART_FIXED_INDEX:
        if (enable_flags.cached_primitive_restart_fixed_index == enable && !ignore_cached_state)
            return;
    ...
    case GL_SAMPLE_ALPHA_TO_ONE_EXT:
        if (enable_flags.cached_sample_alpha_to_one_ext == enable && !ignore_cached_state)
            return;
