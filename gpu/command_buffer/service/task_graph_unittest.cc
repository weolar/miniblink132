...
protected:
    ...
    {
        scoped_feature_list_.InitAndEnableFeature(features::kSyncPointGraphValidation);
    ...
    void CreateSequence(int sequence_key, bool manual_validation = false)
    {
        ...
        SequenceId sequence_id = task_graph_->CreateSequence(base::DoNothing(),
            manual_validation ? scoped_refptr<base::SingleThreadTaskRunner>() : base::SingleThreadTaskRunner::GetCurrentDefault(), kNamespaceId,
            command_buffer_id);
    ...
    void AddTask(int sequence_key, const std::vector<int>& wait_syncs, int release_sync)
    {
        auto info_it = sequence_info_.find(sequence_key);
        ASSERT_TRUE(info_it != sequence_info_.end());
    ...
    void RunAllPendingTasks()
    {
        ...
        do {
            ...
            for (auto& info : sequence_info_) {
                TaskGraph::Sequence* sequence = task_graph_->GetSequence(info.second.sequence_id);
...
TEST_F(TaskGraphTest, ValidationCircularWaits4)
{
    ...
    // task 0.
    task_environment_.FastForwardBy(TaskGraph::kMaxValidationDelay - base::Seconds(2));
...
TEST_F(TaskGraphTest, ValidationPartiallyValidated)
{
    ...
    task_environment_.FastForwardBy(TaskGraph::kMaxValidationDelay - base::Seconds(3));
