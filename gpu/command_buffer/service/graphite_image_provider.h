...
public:
    ...
    sk_sp<SkImage> findOrCreate(skgpu::graphite::Recorder* recorder, const SkImage* image, SkImage::RequiredProperties required_props) override;
...
private:
    ...
    class ImageKey {
    public:
        ImageKey(const SkImage* image, SkImage::RequiredProperties required_properties)
        ...
        {
            return std::tie(values_, required_properties_) == std::tie(other.values_, other.required_properties_);
        }
        bool operator<(const ImageKey& other) const
        {
            return std::tie(values_, required_properties_) < std::tie(other.values_, other.required_properties_);
