...
AsyncAPIMock::AsyncAPIMock(bool default_do_commands, CommandBufferDirect* command_buffer, CommandBufferServiceBase* command_buffer_service)
    : command_buffer_(command_buffer)
    , command_buffer_service_(command_buffer_service)
{
    testing::DefaultValue<error::Error>::Set(error::kNoError);

    if (default_do_commands) {
        ON_CALL(*this, DoCommands(_, _, _, _)).WillByDefault(Invoke(this, &AsyncAPIMock::FakeDoCommands));
    }
    if (command_buffer_) {
...
error::Error AsyncAPIMock::FakeDoCommands(unsigned int num_commands, const volatile void* buffer, int num_entries, int* entries_processed)
{
...
void AsyncAPIMock::SetToken(unsigned int command, unsigned int arg_count, const volatile void* _args)
{
    DCHECK(command_buffer_service_);
    DCHECK_EQ(1u, command);
    DCHECK_EQ(1u, arg_count);
    const volatile cmd::SetToken* args = static_cast<const volatile cmd::SetToken*>(_args);
