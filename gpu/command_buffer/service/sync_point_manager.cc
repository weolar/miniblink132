...
void RunOnThread(scoped_refptr<base::SingleThreadTaskRunner> task_runner, base::OnceClosure callback)
{
...
SyncPointOrderData::OrderFence::OrderFence(uint32_t order, uint64_t release, scoped_refptr<SyncPointClientState> state, uint64_t callback_id)
    : order_num(order)
...
SyncPointOrderData::SyncPointOrderData(SyncPointManager* sync_point_manager, SequenceId sequence_id)
...
void SyncPointOrderData::FinishProcessingOrderNumber(uint32_t order_num)
{
    ...
    for (OrderFence& order_fence : ensure_releases) {
        order_fence.client_state->EnsureWaitReleased(order_fence.fence_release, order_fence.callback_id);
    }
}

uint64_t SyncPointOrderData::ValidateReleaseOrderNumber(scoped_refptr<SyncPointClientState> client_state, uint32_t wait_order_num, uint64_t fence_release)
{
    ...
    uint32_t expected_order_num = std::min(unprocessed_order_nums_.back(), wait_order_num);
    uint64_t callback_id = ++current_callback_id_;
    order_fence_queue_.emplace(expected_order_num, fence_release, std::move(client_state), callback_id);
    return callback_id;
}

SyncPointClientState::ReleaseCallback::ReleaseCallback(uint64_t release, base::OnceClosure callback, uint64_t callback_id)
...
SyncPointClientState::ReleaseCallback::ReleaseCallback(ReleaseCallback&& other) = default;

SyncPointClientState::ReleaseCallback::~ReleaseCallback() = default;

SyncPointClientState::SyncPointClientState(
    SyncPointManager* sync_point_manager, scoped_refptr<SyncPointOrderData> order_data, CommandBufferNamespace namespace_id, CommandBufferId command_buffer_id)
    : sync_point_manager_(sync_point_manager)
...
std::vector<base::OnceClosure> SyncPointClientState::DestroyAndReturnCallbacks()
{
    ...
    while (!release_callback_queue_.empty()) {
        callbacks.push_back(std::move(const_cast<ReleaseCallback&>(release_callback_queue_.top()).callback_closure));
...
bool SyncPointClientState::Wait(const SyncToken& sync_token, base::OnceClosure callback)
{
    DCHECK(!destroyed_.IsSet());
    ...
    return sync_point_manager_->Wait(sync_token, order_data_->sequence_id(), order_data_->current_order_num(), std::move(callback));
}

bool SyncPointClientState::WaitNonThreadSafe(const SyncToken& sync_token, scoped_refptr<base::SingleThreadTaskRunner> task_runner, base::OnceClosure callback)
{
    return Wait(sync_token, base::BindOnce(&RunOnThread, task_runner, std::move(callback)));
}
...
bool SyncPointClientState::WaitForRelease(uint64_t release, uint32_t wait_order_num, base::OnceClosure callback)
{
    ...
    uint64_t callback_id = order_data_->ValidateReleaseOrderNumber(this, wait_order_num, release);
    if (callback_id) {
    ...
    DLOG(ERROR) << "Client waiting on non-existent sync token or sequence destroyed";
    return false;
}
...
void SyncPointClientState::ReleaseFenceSync(uint64_t release)
{
    ...
    // FinishProcessingOrderNumber(), or else we may deadlock.
    DCHECK(order_data_->IsProcessingOrderNumber());
    DCHECK(!destroyed_.IsSet()) << "Attempting to release fence on destroyed client state.";

    EnsureFenceSyncReleased(release, ReleaseCause::kExplicitClientRelease);
}

void SyncPointClientState::EnsureFenceSyncReleased(uint64_t release, ReleaseCause cause)
{
    ...
    std::vector<base::OnceClosure> callback_list;
    {
        ...
        if ((cause == ReleaseCause::kExplicitClientRelease && release <= client_fence_sync_release_)
            || (cause == ReleaseCause::kTaskCompletionRelease && release < client_fence_sync_release_)) {
            static constexpr char error_message[] = "Client attempted to release a fence sync that has been released.";
        ...
        if (cause == ReleaseCause::kExplicitClientRelease || cause == ReleaseCause::kTaskCompletionRelease) {
            client_fence_sync_release_ = release;
        }

        if (release <= fence_sync_release_) {
            DCHECK(release_callback_queue_.empty() || release_callback_queue_.top().release_count > release);
            return;
        }
        fence_sync_release_ = release;

        while (!release_callback_queue_.empty() && release_callback_queue_.top().release_count <= release) {
            ReleaseCallback& release_callback = const_cast<ReleaseCallback&>(release_callback_queue_.top());
...
void SyncPointClientState::EnsureWaitReleased(uint64_t release, uint64_t callback_id)
{
...

    {
        ...
        while (!release_callback_queue_.empty() && release_callback_queue_.top().release_count <= release) {
            ReleaseCallback& top_item = const_cast<ReleaseCallback&>(release_callback_queue_.top());
            if (top_item.release_count == release && top_item.callback_id == callback_id) {
...
scoped_refptr<SyncPointOrderData> SyncPointManager::CreateSyncPointOrderData()
{
    ...
    scoped_refptr<SyncPointOrderData> order_data = new SyncPointOrderData(this, sequence_id);
    DCHECK(!order_data_map_.count(sequence_id));
    order_data_map_.insert(std::make_pair(sequence_id, order_data));
    return order_data;
}

void SyncPointManager::RemoveSyncPointOrderData(scoped_refptr<SyncPointOrderData> order_data)
{
    base::AutoLock auto_lock(lock_);
...
scoped_refptr<SyncPointClientState> SyncPointManager::CreateSyncPointClientState(
    CommandBufferNamespace namespace_id, CommandBufferId command_buffer_id, SequenceId sequence_id)
{
    base::AutoLock auto_lock(lock_);

    scoped_refptr<SyncPointOrderData> order_data = GetSyncPointOrderData(sequence_id);
    scoped_refptr<SyncPointClientState> client_state = new SyncPointClientState(this, order_data, namespace_id, command_buffer_id);

    DCHECK_GE(namespace_id, 0);
    DCHECK_LT(static_cast<size_t>(namespace_id), std::size(client_state_maps_));
    const auto [_, inserted] = client_state_maps_[namespace_id].insert(std::make_pair(command_buffer_id, client_state));
    CHECK(inserted);

    return client_state;
}

void SyncPointManager::DestroySyncPointClientState(scoped_refptr<SyncPointClientState> client_state)
{
    std::vector<base::OnceClosure> callbacks;
...
void SyncPointManager::EnsureFenceSyncReleased(const SyncToken& release, ReleaseCause cause)
{
    scoped_refptr<SyncPointClientState> client_state;
    {
        base::AutoLock lock(lock_);
        client_state = GetSyncPointClientState(release.namespace_id(), release.command_buffer_id());
    }
    if (client_state) {
...
{
    base::AutoLock auto_lock(lock_);
    scoped_refptr<SyncPointClientState> release_state = GetSyncPointClientState(sync_token.namespace_id(), sync_token.command_buffer_id());
...
SequenceId SyncPointManager::GetSyncTokenReleaseSequenceId(const SyncToken& sync_token)
{
    base::AutoLock auto_lock(lock_);
    return GetSyncTokenReleaseSequenceIdInternal(sync_token);
}

SequenceId SyncPointManager::GetSyncTokenReleaseSequenceIdInternal(const SyncToken& sync_token)
{
    scoped_refptr<SyncPointClientState> client_state = GetSyncPointClientState(sync_token.namespace_id(), sync_token.command_buffer_id());
    if (client_state) {
...
uint32_t SyncPointManager::GetProcessedOrderNum() const
{
    ...
    for (const auto& kv : order_data_map_) {
        processed_order_num = std::max(processed_order_num, kv.second->processed_order_num());
    }
...
uint32_t SyncPointManager::GetUnprocessedOrderNum() const
{
    ...
    for (const auto& kv : order_data_map_) {
        unprocessed_order_num = std::max(unprocessed_order_num, kv.second->unprocessed_order_num());
    }
    return unprocessed_order_num;
}

bool SyncPointManager::Wait(const SyncToken& sync_token, SequenceId sequence_id, uint32_t wait_order_num, base::OnceClosure callback)
{
    base::AutoLock auto_lock(lock_);
    ...
    scoped_refptr<SyncPointClientState> release_state = GetSyncPointClientState(sync_token.namespace_id(), sync_token.command_buffer_id());
    if (release_state && release_state->WaitForRelease(sync_token.release_count(), wait_order_num, std::move(callback))) {
        return true;
    }
...
bool SyncPointManager::WaitNonThreadSafe(const SyncToken& sync_token, SequenceId sequence_id, uint32_t wait_order_num,
    scoped_refptr<base::SingleThreadTaskRunner> task_runner, base::OnceClosure callback)
{
    return Wait(sync_token, sequence_id, wait_order_num, base::BindOnce(&RunOnThread, task_runner, std::move(callback)));
}
...
scoped_refptr<SyncPointClientState> SyncPointManager::GetSyncPointClientState(CommandBufferNamespace namespace_id, CommandBufferId command_buffer_id)
{
    if (namespace_id >= 0) {
        DCHECK_LT(static_cast<size_t>(namespace_id), std::size(client_state_maps_));
...
scoped_refptr<SyncPointOrderData> SyncPointManager::GetSyncPointOrderData(SequenceId sequence_id)
{
