...
struct PassthroughResources {
    ...
    static_assert(sizeof(uintptr_t) == sizeof(GLsync), "GLsync not the same size as uintptr_t");
    ...
    ClientServiceMap<GLuint, scoped_refptr<TexturePassthrough>> texture_object_map;

    class SharedImageData {
    public:
        SharedImageData();
        explicit SharedImageData(const GLES2DecoderPassthroughImpl*, std::unique_ptr<GLTexturePassthroughImageRepresentation>);
        ...
        bool is_being_accessed() const
        {
            return access_mode_.has_value();
        }
    ...
    private:
        ...
        std::unique_ptr<GLTexturePassthroughImageRepresentation::ScopedAccess> scoped_access_;
...
class GPU_GLES2_EXPORT GLES2DecoderPassthroughImpl : public GLES2Decoder, public ui::GpuSwitchingObserver {
public:
    GLES2DecoderPassthroughImpl(DecoderClient* client, CommandBufferServiceBase* command_buffer_service, Outputter* outputter, ContextGroup* group);
    ~GLES2DecoderPassthroughImpl() override;

    Error DoCommands(unsigned int num_commands, const volatile void* buffer, int num_entries, int* entries_processed) override;

    template <bool DebugImpl> Error DoCommandsImpl(unsigned int num_commands, const volatile void* buffer, int num_entries, int* entries_processed);
    ...
    GLsizeiptr BlobCacheGet(const void* key, GLsizeiptr key_size, void* value, GLsizeiptr value_size);
    void BlobCacheSet(const void* key, GLsizeiptr key_size, const void* value, GLsizeiptr value_size);
...
private:
    ...
    void InitializeFeatureInfo(ContextType context_type, const DisallowedFeatures& disallowed_features, bool force_reinitialize);

    template <typename T, typename GLGetFunction>
    error::Error GetNumericHelper(GLenum pname, GLsizei bufsize, GLsizei* length, T* params, GLGetFunction get_call)
    {
    ...
    template <typename T> error::Error PatchGetNumericResults(GLenum pname, GLsizei length, T* params);
    error::Error PatchGetFramebufferAttachmentParameter(GLenum target, GLenum attachment, GLenum pname, GLsizei length, GLint* params);

    template <typename T> error::Error PatchGetBufferResults(GLenum target, GLenum pname, GLsizei bufsize, GLsizei* length, T* params);

    error::Error PatchGetFramebufferPixelLocalStorageParameterivANGLE(GLint plane, GLenum pname, GLsizei length, GLint* params);
    ...
    void UpdateTextureBinding(GLenum target, GLuint client_id, TexturePassthrough* texture);
    void RebindTexture(TexturePassthrough* texture);

    void UpdateTextureSizeFromTexturePassthrough(TexturePassthrough* texture, GLuint client_id);
    ...
    void CheckSwapBuffersAsyncResult(const char* function_name, uint64_t swap_id, gfx::SwapCompletionResult result);
    error::Error CheckSwapBuffersResult(gfx::SwapResult result, const char* function_name);
...
#undef GLES2_CMD_OP
    ...
    using CmdHandler = Error (GLES2DecoderPassthroughImpl::*)(uint32_t immediate_data_size, const volatile void* data);
    ...
    static constexpr size_t kNumTextureTypes = static_cast<size_t>(TextureTarget::kCount);
    std::array<std::array<BoundTexture, kMaxTextureUnits>, kNumTextureTypes> bound_textures_;
    ...
    struct EmulatedDefaultFramebuffer {
        ...
        EmulatedDefaultFramebuffer& operator=(const EmulatedDefaultFramebuffer&) = delete;
    ...
    public:
        static std::unique_ptr<LazySharedContextState> Create(GLES2DecoderPassthroughImpl* impl);
    ...
    // get rescheduled.
    base::circular_deque<std::unique_ptr<gl::GLFence>> deschedule_until_finished_fences_;
