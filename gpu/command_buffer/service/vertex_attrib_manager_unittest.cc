...
TEST_F(VertexAttribManagerTest, Basic)
{
    ...
    const VertexAttribManager::VertexAttribList& enabled_attribs = manager_->GetEnabledVertexAttribs();
...
{
    EXPECT_FALSE(manager_->HaveFixedAttribs());
    manager_->SetAttribInfo(1, nullptr, 4, GL_FIXED, GL_FALSE, 0, 16, 0, GL_FALSE);
    EXPECT_TRUE(manager_->HaveFixedAttribs());
    manager_->SetAttribInfo(3, nullptr, 4, GL_FIXED, GL_FALSE, 0, 16, 0, GL_FALSE);
    EXPECT_TRUE(manager_->HaveFixedAttribs());
    manager_->SetAttribInfo(1, nullptr, 4, GL_FLOAT, GL_FALSE, 0, 16, 0, GL_FALSE);
    EXPECT_TRUE(manager_->HaveFixedAttribs());
    manager_->SetAttribInfo(3, nullptr, 4, GL_FLOAT, GL_FALSE, 0, 16, 0, GL_FALSE);
    EXPECT_FALSE(manager_->HaveFixedAttribs());
}
...
TEST_F(VertexAttribManagerTest, CanAccess)
{
    ...
    TestHelper::DoBufferData(gl_.get(), &error_state, &buffer_manager, buffer, kTarget, 15, GL_STATIC_DRAW, nullptr, GL_NO_ERROR);

    EXPECT_FALSE(attrib->CanAccess(0));
    TestHelper::DoBufferData(gl_.get(), &error_state, &buffer_manager, buffer, kTarget, 16, GL_STATIC_DRAW, nullptr, GL_NO_ERROR);
    EXPECT_TRUE(attrib->CanAccess(0));
    ...
    TestHelper::DoBufferData(gl_.get(), &error_state, &buffer_manager, buffer, kTarget, 32, GL_STATIC_DRAW, nullptr, GL_NO_ERROR);
    EXPECT_TRUE(attrib->CanAccess(0));
    EXPECT_FALSE(attrib->CanAccess(1));
    manager_->SetAttribInfo(1, buffer, 4, GL_FLOAT, GL_FALSE, 0, 16, 0, GL_FALSE);
...
TEST_F(VertexAttribManagerTest, Unbind)
{
    ...
    // Attach to 2 buffers.
    manager_->SetAttribInfo(1, buffer1, 3, GL_SHORT, GL_TRUE, 32, 32, 4, GL_FALSE);
    manager_->SetAttribInfo(3, buffer1, 3, GL_SHORT, GL_TRUE, 32, 32, 4, GL_FALSE);
