...
//   callback returns.
using TaskCallback = base::OnceCallback<void(FenceSyncReleaseDelegate* release_delegate)>;
...
public:
    ...
    ScopedSyncPointClientState(TaskGraph* task_graph, SequenceId sequence_id, CommandBufferNamespace namespace_id, CommandBufferId command_buffer_id);
...
public:
    ...
    SequenceId CreateSequence(base::RepeatingClosure front_task_unblocked_callback, scoped_refptr<base::SingleThreadTaskRunner> validation_runner)
        LOCKS_EXCLUDED(lock_);

    SequenceId CreateSequence(base::RepeatingClosure front_task_unblocked_callback, scoped_refptr<base::SingleThreadTaskRunner> validation_runner,
        CommandBufferNamespace namespace_id, CommandBufferId command_buffer_id) LOCKS_EXCLUDED(lock_);
    ...
    [[nodiscard]] ScopedSyncPointClientState CreateSyncPointClientState(
        SequenceId sequence_id, CommandBufferNamespace namespace_id, CommandBufferId command_buffer_id) LOCKS_EXCLUDED(lock_);
    ...
    base::Lock& lock() const LOCK_RETURNED(lock_)
    {
        return lock_;
    }
    ...
    void ValidateSequenceTaskFenceDeps(Sequence* root_sequence) LOCKS_EXCLUDED(lock_);
    ...
    public:
        ...
        Sequence(TaskGraph* task_graph, base::RepeatingClosure front_task_unblocked_callback, scoped_refptr<base::SingleThreadTaskRunner> validation_runner,
            CommandBufferNamespace namespace_id = CommandBufferNamespace::INVALID, CommandBufferId command_buffer_id = {});
        ...
        [[nodiscard]] ScopedSyncPointClientState CreateSyncPointClientState(CommandBufferNamespace namespace_id, CommandBufferId command_buffer_id)
        ...
        uint32_t AddTask(TaskCallback task_callback, std::vector<SyncToken> wait_fences, const SyncToken& release, ReportingCallback report_callback)
            EXCLUSIVE_LOCKS_REQUIRED(&TaskGraph::lock_);

        virtual uint32_t AddTask(base::OnceClosure task_closure, std::vector<SyncToken> wait_fences, const SyncToken& release,
            ReportingCallback report_callback) EXCLUSIVE_LOCKS_REQUIRED(&TaskGraph::lock_);

        virtual uint32_t BeginTask(base::OnceClosure* task_closure) EXCLUSIVE_LOCKS_REQUIRED(&TaskGraph::lock_);
        ...
        void ContinueTask(TaskCallback task_callback) EXCLUSIVE_LOCKS_REQUIRED(&TaskGraph::lock_);
        virtual void ContinueTask(base::OnceClosure task_closure) EXCLUSIVE_LOCKS_REQUIRED(&TaskGraph::lock_);

        // Sets the first dependency added time on the last task if it wasn't
        // already set, no-op otherwise.
        void SetLastTaskFirstDependencyTimeIfNeeded() EXCLUSIVE_LOCKS_REQUIRED(&TaskGraph::lock_);

        // The time delta it took for the front task's dependencies to be completed.
        base::TimeDelta FrontTaskWaitingDependencyDelta() EXCLUSIVE_LOCKS_REQUIRED(&TaskGraph::lock_);
        ...
        base::TimeDelta FrontTaskSchedulingDelay() EXCLUSIVE_LOCKS_REQUIRED(&TaskGraph::lock_);

        // Removes a waiting sync token fence.
        void RemoveWaitFence(const SyncToken& sync_token, uint32_t order_num, SequenceId release_sequence_id) EXCLUSIVE_LOCKS_REQUIRED(&TaskGraph::lock_);

        const SyncToken& current_task_release() const EXCLUSIVE_LOCKS_REQUIRED(&TaskGraph::lock_)
        {
        ...
        bool IsFrontTaskUnblocked() const EXCLUSIVE_LOCKS_REQUIRED(&TaskGraph::lock_);
    ...
    protected:
        ...
        struct WaitFence {
            ...
            WaitFence(const SyncToken& sync_token, uint32_t order_num, SequenceId release_sequence_id);
            ...
            {
                return std::tie(order_num, release_sequence_id, sync_token) == std::tie(other.order_num, release_sequence_id, other.sync_token);
            }
        ...
        struct Task {
            Task(Task&& other);
            Task(base::OnceClosure task_closure, uint32_t order_num, const SyncToken& release, ReportingCallback report_callback);
            ~Task();
        ...
        scoped_refptr<SyncPointClientState> TakeSyncPointClientState(CommandBufferNamespace namespace_id, CommandBufferId command_buffer_id)
            EXCLUSIVE_LOCKS_REQUIRED(&TaskGraph::lock_);

        void UpdateValidationTimer() EXCLUSIVE_LOCKS_REQUIRED(&TaskGraph::lock_);

        // Returns the range of wait fences for `task`.
        std::pair<WaitFenceConstIter, WaitFenceConstIter> GetTaskWaitFences(const Task& task) const EXCLUSIVE_LOCKS_REQUIRED(&TaskGraph::lock_);

        const Task* FindReleaseTask(const SyncToken& sync_token) const EXCLUSIVE_LOCKS_REQUIRED(&TaskGraph::lock_);
        ...
        const base::RepeatingClosure front_task_unblocked_callback_ GUARDED_BY(&TaskGraph::lock_);

        std::vector<scoped_refptr<SyncPointClientState>> sync_point_states_ GUARDED_BY(&TaskGraph::lock_);
...
private:
    ...
    void SyncTokenFenceReleased(const SyncToken& sync_token, uint32_t order_num, SequenceId release_sequence_id, SequenceId waiting_sequence_id)
        LOCKS_EXCLUDED(lock_);

    // Accessed by ScopedSyncPointClientState.
    void DestroySyncPointClientState(SequenceId sequence_id, CommandBufferNamespace namespace_id, CommandBufferId command_buffer_id) LOCKS_EXCLUDED(lock_);
    ...
    void ValidateTaskFenceDeps(Sequence* sequence, Sequence::TaskIter task_iter, ReleaseMap* pending_releases, ReleaseMap* force_releases,
        ValidateStateMap* validate_states) EXCLUSIVE_LOCKS_REQUIRED(lock_);
    ...
    // tasks.
    ValidateState& GetSequenceValidateState(ValidateStateMap* validate_states, ReleaseMap* pending_releases, Sequence* sequence)
    ...
    base::flat_map<SequenceId, std::unique_ptr<Sequence>> sequence_map_ GUARDED_BY(lock_);
};
