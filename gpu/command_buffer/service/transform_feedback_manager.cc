...
TransformFeedback::TransformFeedback(TransformFeedbackManager* manager, GLuint client_id, GLuint service_id)
    : IndexedBufferBindingHost(manager->max_transform_feedback_separate_attribs(), GL_TRANSFORM_FEEDBACK_BUFFER, manager->needs_emulation(), false)
    , manager_(manager)
...
void TransformFeedback::DoBindTransformFeedback(GLenum target, TransformFeedback* last_bound_transform_feedback, Buffer* bound_transform_feedback_buffer)
{
    ...
    if (bound_transform_feedback_buffer && !bound_transform_feedback_buffer->IsDeleted()) {
        glBindBuffer(GL_TRANSFORM_FEEDBACK_BUFFER, bound_transform_feedback_buffer->service_id());
    } else {
...
{
    DCHECK(!active_);
    DCHECK(primitive_mode == GL_POINTS || primitive_mode == GL_LINES || primitive_mode == GL_TRIANGLES);
...
bool TransformFeedback::GetVerticesNeededForDraw(GLenum mode, GLsizei count, GLsizei primcount, GLsizei pending_vertices_drawn, GLsizei* vertices_out) const
{
    ...
    // instances.
    base::CheckedNumeric<GLsizei> checked_vertices = vertices_drawn_ + pending_vertices_drawn;
    base::CheckedNumeric<GLsizei> checked_count = count;
    base::CheckedNumeric<GLsizei> checked_primcount = primcount;
    switch (mode) {
    case GL_TRIANGLES:
        checked_vertices += checked_primcount * (checked_count - checked_count % 3);
        break;
    case GL_LINES:
        checked_vertices += checked_primcount * (checked_count - checked_count % 2);
        break;
    default:
        NOTREACHED();
    case GL_POINTS:
...
TransformFeedbackManager::TransformFeedbackManager(GLuint max_transform_feedback_separate_attribs, bool needs_emulation)
    : max_transform_feedback_separate_attribs_(max_transform_feedback_separate_attribs)
...
TransformFeedback* TransformFeedbackManager::CreateTransformFeedback(GLuint client_id, GLuint service_id)
{
    scoped_refptr<TransformFeedback> transform_feedback(new TransformFeedback(this, client_id, service_id));
    auto result = transform_feedbacks_.insert(std::make_pair(client_id, transform_feedback));
    DCHECK(result.second);
    return result.first->second.get();
}

TransformFeedback* TransformFeedbackManager::GetTransformFeedback(GLuint client_id)
{
    if (client_id == 0) {
        return nullptr;
    }
