...
TEST_F(VertexArrayManagerTest, Basic)
{
    ...
    // Check we can create
    manager_->CreateVertexAttribManager(kClient1Id, kService1Id, kNumVertexAttribs, true, false);
    ...
    EXPECT_CALL(*gl_, DeleteVertexArraysOES(1, ::testing::Pointee(kService1Id))).Times(1).RetiresOnSaturation();
...
TEST_F(VertexArrayManagerTest, Destroy)
{
    ...
    // Check we can create
    manager.CreateVertexAttribManager(kClient1Id, kService1Id, kNumVertexAttribs, true, false);
    // Check creation success
