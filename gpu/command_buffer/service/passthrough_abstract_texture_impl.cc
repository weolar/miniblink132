...
PassthroughAbstractTextureImpl::PassthroughAbstractTextureImpl(scoped_refptr<TexturePassthrough> texture_passthrough, GLES2DecoderPassthroughImpl* decoder)
...
void PassthroughAbstractTextureImpl::BindImage(gl::GLImage* image, bool client_managed)
{
    if (!texture_passthrough_)
        return;
    ...
    if (decoder_managed_image_) {
        gl::GLImage* current_image = texture_passthrough_->GetLevelImage(target, level);
...
void PassthroughAbstractTextureImpl::BindStreamTextureImage(gl::GLImage* image, GLuint service_id)
{
    DCHECK(image);
...
gl::GLImage* PassthroughAbstractTextureImpl::GetImageForTesting() const
{
    ...
    const GLint level = 0;
    return texture_passthrough_->GetLevelImage(texture_passthrough_->target(), level);
}
...
scoped_refptr<TexturePassthrough> PassthroughAbstractTextureImpl::OnDecoderWillDestroy()
{
