...
bool Snapshot::SaveScreenshot(const gfx::Size& size)
{
    ...
    glReadPixels(0, 0, screenshot_size_.width(), screenshot_size_.height(), GL_RGBA, GL_UNSIGNED_BYTE, &screenshot_pixels_[0]);
    ...
    for (int y = 0; y < screenshot_size_.height() / 2; y++) {
        for (int x = 0; x < bytes_per_row; x++) {
            std::swap(screenshot_pixels_[y * bytes_per_row + x], screenshot_pixels_[(screenshot_size_.height() - y - 1) * bytes_per_row + x]);
        }
    }
    return true;
}
...
void Snapshot::AppendAsTraceFormat(std::string* out) const
{
    *out += "{";
    if (screenshot_pixels_.size()) {
        std::optional<std::vector<uint8_t>> png_data = gfx::PNGCodec::Encode(screenshot_pixels_.data(), gfx::PNGCodec::FORMAT_RGBA, screenshot_size_,
            /*row_byte_width=*/screenshot_size_.width() * kBytesPerPixel,
            /*discard_transparency=*/false, std::vector<gfx::PNGCodec::Comment>());

        DCHECK(png_data);
        std::string base64_output = base::Base64Encode(png_data.value_or(std::vector<uint8_t>()));
...
GPUStateTracer::GPUStateTracer(const ContextState* state)
    : state_(state)
{
    TRACE_EVENT_OBJECT_CREATED_WITH_ID(TRACE_DISABLED_BY_DEFAULT("gpu.debug"), "gpu::State", state_);
}

GPUStateTracer::~GPUStateTracer()
{
    TRACE_EVENT_OBJECT_DELETED_WITH_ID(TRACE_DISABLED_BY_DEFAULT("gpu.debug"), "gpu::State", state_);
}

void GPUStateTracer::TakeSnapshotWithCurrentFramebuffer(const gfx::Size& size)
{
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("gpu.debug"), "GPUStateTracer::TakeSnapshotWithCurrentFramebuffer");
    ...
    TRACE_EVENT_OBJECT_SNAPSHOT_WITH_ID(TRACE_DISABLED_BY_DEFAULT("gpu.debug"), "gpu::State", state_, std::move(snapshot));
}
