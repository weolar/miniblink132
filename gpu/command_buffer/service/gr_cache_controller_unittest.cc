...
public:
    void SetUp() override
    {
        ...
        context_state_ = base::MakeRefCounted<SharedContextState>(
            std::move(share_group), std::move(surface), std::move(context), false /* use_virtualized_gl_contexts */, base::DoNothing(), GrContextType::kGL);
        ...
        controller_ = base::WrapUnique(new GrCacheController(context_state_.get(), task_runner_));
    }

    void CreateControllerWithoutTaskRunner()
    {
        controller_ = base::WrapUnique(new GrCacheController(context_state_.get(), nullptr));
    }
...
TEST_F(GrCacheControllerTest, PurgeGrCache)
{
    EXPECT_EQ(gr_context()->getResourceCachePurgeableBytes(), 0u);
    {
        ...
        sk_sp<SkImage> uploaded = SkImages::TextureFromImage(gr_context(), SkImages::RasterFromBitmap(bm));
