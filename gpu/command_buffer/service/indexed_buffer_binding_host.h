...
class GPU_GLES2_EXPORT IndexedBufferBindingHost : public base::RefCounted<IndexedBufferBindingHost> {
public:
    ...
    IndexedBufferBindingHost(uint32_t max_bindings, GLenum target, bool needs_emulation, bool round_down_uniform_bind_buffer_range_size);
    ...
    void DoBindBufferRange(GLuint index, Buffer* buffer, GLintptr offset, GLsizeiptr size);

    // This is called on the active host when glBufferData is called and buffer
    // size might change.
    void OnBufferData(Buffer* buffer);

    void RemoveBoundBuffer(GLenum target, Buffer* buffer, Buffer* target_generic_bound_buffer, bool have_context);
...
private:
    enum class IndexedBufferBindingType { kBindBufferBase, kBindBufferRange, kBindBufferNone };
    ...
    struct IndexedBufferBinding {
        ...
        void SetBindBufferRange(Buffer* _buffer, GLintptr _offset, GLsizeiptr _size);
        void Reset();
    };

    // This is called when |needs_emulation_| is true, where the range
    // (offset + size) can't go beyond the buffer's size.
    static void DoAdjustedBindBufferRange(GLenum target, GLuint index, GLuint service_id, GLintptr offset, GLsizeiptr size, GLsizeiptr full_buffer_size,
