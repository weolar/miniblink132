...
class GPU_GLES2_EXPORT ImageReaderGLOwner : public TextureOwner, public RefCountedLockHelperDrDc {
public:
    ...
    void SetFrameAvailableCallback(const base::RepeatingClosure& frame_available_cb) override;
    ...
    bool GetCodedSizeAndVisibleRect(gfx::Size rotated_visible_size, gfx::Size* coded_size, gfx::Rect* visible_rect) override;

    void RunWhenBufferIsAvailable(base::OnceClosure callback) override;

    const AImageReader* image_reader_for_testing() const NO_THREAD_SAFETY_ANALYSIS
    {
        return image_reader_;
    }
    int32_t max_images_for_testing() const
    {
        return max_images_;
    }
...
private:
    ...
    public:
        ScopedCurrentImageRef(ImageReaderGLOwner* texture_owner, AImage* image, base::ScopedFD ready_fence);
    ...
    ImageReaderGLOwner(std::unique_ptr<AbstractTextureAndroid> texture, Mode secure_mode, scoped_refptr<SharedContextState> context_state,
