...
{
    base::UnsafeSharedMemoryRegion shm_region = base::UnsafeSharedMemoryRegion::Create(kBufferSize);
    base::WritableSharedMemoryMapping shm_mapping = shm_region.Map();
    auto shm_guid = shm_region.GetGUID();
    auto backing = std::make_unique<SharedMemoryBufferBacking>(std::move(shm_region), std::move(shm_mapping));
    SharedMemoryBufferBacking* backing_raw_ptr = backing.get();

    EXPECT_TRUE(transfer_buffer_manager_->RegisterTransferBuffer(1, base::MakeRefCounted<Buffer>(std::move(backing))));
    scoped_refptr<Buffer> registered = transfer_buffer_manager_->GetTransferBuffer(1);
...
{
    EXPECT_TRUE(transfer_buffer_manager_->RegisterTransferBuffer(1, MakeMemoryBuffer(42)));
...
{
    EXPECT_TRUE(transfer_buffer_manager_->RegisterTransferBuffer(1, MakeMemoryBuffer(42)));
    EXPECT_FALSE(transfer_buffer_manager_->RegisterTransferBuffer(1, MakeMemoryBuffer(42)));
    EXPECT_FALSE(transfer_buffer_manager_->RegisterTransferBuffer(1, MakeMemoryBuffer(42)));
}
...
{
    EXPECT_FALSE(transfer_buffer_manager_->RegisterTransferBuffer(0, MakeMemoryBuffer(42)));
}
