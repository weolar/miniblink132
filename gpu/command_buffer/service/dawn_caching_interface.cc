...
DawnCachingInterface::DawnCachingInterface(scoped_refptr<detail::DawnCachingBackend> backend, CacheBlobCallback callback)
...
size_t DawnCachingInterface::LoadData(const void* key, size_t key_size, void* value_out, size_t value_size)
{
    if (backend() == nullptr) {
        return 0u;
    }
...
void DawnCachingInterface::StoreData(const void* key, size_t key_size, const void* value, size_t value_size)
{
    ...
    if (cache_blob_callback_) {
        ...
        cache_blob_callback_.Run(gpu::GpuDiskCacheType::kDawnWebGPU, key_str, value_str);
    }
}
...
DawnCachingInterfaceFactory::DawnCachingInterfaceFactory()
    : DawnCachingInterfaceFactory(base::BindRepeating(&DawnCachingInterfaceFactory::CreateDefaultInMemoryBackend))
{
}

DawnCachingInterfaceFactory::~DawnCachingInterfaceFactory() = default;

std::unique_ptr<DawnCachingInterface> DawnCachingInterfaceFactory::CreateInstance(
    const gpu::GpuDiskCacheHandle& handle, DawnCachingInterface::CacheBlobCallback callback)
{
    DCHECK(gpu::GetHandleType(handle) == gpu::GpuDiskCacheType::kDawnWebGPU || gpu::GetHandleType(handle) == gpu::GpuDiskCacheType::kDawnGraphite);

    if (const auto it = backends_.find(handle); it != backends_.end()) {
        return base::WrapUnique(new DawnCachingInterface(it->second, std::move(callback)));
    }
    ...
    return base::WrapUnique(new DawnCachingInterface(std::move(backend), std::move(callback)));
}
...
void DawnCachingInterfaceFactory::ReleaseHandle(const gpu::GpuDiskCacheHandle& handle)
{
    DCHECK(gpu::GetHandleType(handle) == gpu::GpuDiskCacheType::kDawnWebGPU || gpu::GetHandleType(handle) == gpu::GpuDiskCacheType::kDawnGraphite);
...
scoped_refptr<detail::DawnCachingBackend> DawnCachingInterfaceFactory::CreateDefaultInMemoryBackend()
{
    return base::MakeRefCounted<detail::DawnCachingBackend>(GetDefaultGpuDiskCacheSize());
}

namespace detail {

DawnCachingBackend::Entry::Entry(const std::string& key, const void* value, size_t value_size)
    : key_(key)
...
size_t DawnCachingBackend::Entry::ReadData(void* value_out, size_t value_size) const
{
...
bool operator<(const std::unique_ptr<DawnCachingBackend::Entry>& lhs, const std::unique_ptr<DawnCachingBackend::Entry>& rhs)
{
    return lhs->Key() < rhs->Key();
}

bool operator<(const std::unique_ptr<DawnCachingBackend::Entry>& lhs, const std::string& rhs)
{
    return lhs->Key() < rhs;
}

bool operator<(const std::string& lhs, const std::unique_ptr<DawnCachingBackend::Entry>& rhs)
{
    return lhs < rhs->Key();
}
...
size_t DawnCachingBackend::LoadData(const std::string& key, void* value_out, size_t value_size)
{
...
void DawnCachingBackend::StoreData(const std::string& key, const void* value, size_t value_size)
{
