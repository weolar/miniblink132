...
public:
    IndexedBufferBindingHostTest()
        : uniform_host_(new IndexedBufferBindingHost(kMaxBindings, GL_UNIFORM_BUFFER, true, false))
        , tf_host_(new IndexedBufferBindingHost(kMaxBindings, GL_TRANSFORM_FEEDBACK_BUFFER, true, false))
...
protected:
    ...
    {
        buffer_manager_->SetInfo(buffer_.get(), target, size, GL_STATIC_DRAW, false);
    }
...
TEST_F(IndexedBufferBindingHostTest, DoBindBufferRangeUninitializedBuffer)
{
    ...
    EXPECT_CALL(*gl_, BindBufferBase(kTarget, kIndex, kBufferServiceId)).Times(1).RetiresOnSaturation();
...
TEST_F(IndexedBufferBindingHostTest, DoBindBufferRangeBufferWithoutEnoughSize)
{
    ...
    EXPECT_CALL(*gl_, BindBufferRange(kTarget, kIndex, kBufferServiceId, kOffset, clamped_size)).Times(1).RetiresOnSaturation();
    ...
    EXPECT_CALL(*gl_, BindBufferRange(kTarget, kIndex, kBufferServiceId, kOffset, kSize)).Times(1).RetiresOnSaturation();
...
TEST_F(IndexedBufferBindingHostTest, RestoreBindings)
{
    ...
    scoped_refptr<IndexedBufferBindingHost> other = new IndexedBufferBindingHost(kMaxBindings, kTarget, true, false);
    EXPECT_CALL(*gl_, BindBufferRange(kTarget, kOtherIndex, kBufferServiceId, kOffset, clamped_size)).Times(1).RetiresOnSaturation();
    other->DoBindBufferRange(kOtherIndex, buffer_.get(), kOffset, kSize);

    {
        // Switching from |other| to |uniform_host_|.
        EXPECT_CALL(*gl_, BindBufferBase(kTarget, kIndex, kBufferServiceId)).Times(1).RetiresOnSaturation();
        EXPECT_CALL(*gl_, BindBufferBase(kTarget, kOtherIndex, 0)).Times(1).RetiresOnSaturation();
        uniform_host_->RestoreBindings(other.get());
    }

    {
        // Switching from |uniform_host_| to |other|.
        EXPECT_CALL(*gl_, BindBufferBase(kTarget, kIndex, 0)).Times(1).RetiresOnSaturation();
        EXPECT_CALL(*gl_, BindBufferRange(kTarget, kOtherIndex, kBufferServiceId, kOffset, clamped_size)).Times(1).RetiresOnSaturation();
