...
protected:
    ...
    base::win::ScopedHandle CreateSharedHandle(const Microsoft::WRL::ComPtr<ID3D11Texture2D>& d3d11_texture)
    {
...
TEST_F(DXGISharedHandleManagerTest, LookupByToken)
{
    ...
    scoped_refptr<DXGISharedHandleState> orig_state
        = dxgi_shared_handle_manager_->GetOrCreateSharedHandleState(orig_token, std::move(orig_handle), d3d11_device_);
    ASSERT_NE(orig_state, nullptr);

    EXPECT_EQ(dxgi_shared_handle_manager_->GetSharedHandleMapSizeForTesting(), 1u);

    for (int i = 0; i < kNumHandles - 1; i++) {
        HANDLE handle;
        ::DuplicateHandle(::GetCurrentProcess(), orig_state->GetSharedHandle(), ::GetCurrentProcess(), &handle,
            /*dwDesiredAccess=*/0,
        ...
        scoped_refptr<DXGISharedHandleState> state
            = dxgi_shared_handle_manager_->GetOrCreateSharedHandleState(orig_token, std::move(new_handle), d3d11_device_);
    ...
    EXPECT_EQ(dxgi_shared_handle_manager_->GetSharedHandleMapSizeForTesting(), 0u);
}
