...
PassthroughProgramCache::PassthroughProgramCache(size_t max_cache_size_bytes, bool disable_gpu_shader_disk_cache, ValueAddedHook* value_added_hook)
    ...
    , value_added_hook_(value_added_hook)
{
    ...
    if (egl_display != EGL_NO_DISPLAY && !g_blob_cache_funcs_set && BlobCacheExtensionAvailable(gl_display)) {
...
ProgramCache::ProgramLoadResult PassthroughProgramCache::LoadLinkedProgram(GLuint program, Shader* shader_a, Shader* shader_b,
...
void PassthroughProgramCache::SaveLinkedProgram(GLuint program, const Shader* shader_a, const Shader* shader_b, const LocationMap* bind_attrib_location_map,
    const std::vector<std::string>& transform_feedback_varyings, GLenum transform_feedback_buffer_mode, DecoderClient* client)
{
    NOTREACHED();
}

void PassthroughProgramCache::LoadProgram(const std::string& key, const std::string& program)
{
    base::AutoLock auto_lock(lock_);
    if (!CacheEnabled()) {
        return;
...
void PassthroughProgramCache::Set(Key&& key, Value&& value, CacheProgramCallback callback)
{
    {
        base::AutoLock auto_lock(lock_);
    ...
    if (callback) {
        ...
        std::string_view key_string(reinterpret_cast<const char*>(key.data()), key.size());
        std::string_view value_string(reinterpret_cast<const char*>(value.data()), value.size());
...
size_t PassthroughProgramCache::Get(const Key& key, void* out_value, size_t value_size)
{
    ...
    UMA_HISTOGRAM_BOOLEAN("Gpu.PassthroughProgramCacheLoadHitInCache", found != store_.end());
...
EGLsizeiANDROID PassthroughProgramCache::BlobCacheGetImpl(const void* key, EGLsizeiANDROID key_size, void* value, EGLsizeiANDROID value_size)
{
    if (key_size < 0) {
        return 0;
    }
...
void PassthroughProgramCache::BlobCacheSetImpl(const void* key, EGLsizeiANDROID key_size, const void* value, EGLsizeiANDROID value_size)
{
...
void PassthroughProgramCache::BlobCacheSet(const void* key, EGLsizeiANDROID key_size, const void* value, EGLsizeiANDROID value_size)
{
    if (!g_program_cache)
        return;
...
EGLsizeiANDROID PassthroughProgramCache::BlobCacheGet(const void* key, EGLsizeiANDROID key_size, void* value, EGLsizeiANDROID value_size)
{
    if (!g_program_cache)
        return 0;
...
PassthroughProgramCache::ProgramCacheValue::ProgramCacheValue(PassthroughProgramCache::Value&& program_blob, PassthroughProgramCache* program_cache)
    : program_blob_(std::move(program_blob))
...
PassthroughProgramCache::ProgramCacheValue::ProgramCacheValue(ProgramCacheValue&& other)
    : program_blob_(std::move(other.program_blob_))
    , program_cache_(std::exchange(other.program_cache_, nullptr))
{
}

PassthroughProgramCache::ProgramCacheValue& PassthroughProgramCache::ProgramCacheValue::operator=(ProgramCacheValue&& other)
{
    program_blob_ = std::move(other.program_blob_);
    program_cache_ = std::exchange(other.program_cache_, nullptr);
    return *this;
