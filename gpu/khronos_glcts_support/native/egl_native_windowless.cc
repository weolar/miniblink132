...
public:
    Surface(tcu::egl::Display& display, EGLConfig config, const EGLint* attribList, int width, int height)
        : tcu::egl::WindowSurface(display, config, (EGLNativeWindowType) nullptr, attribList)
        , width_(width)
        , height_(height)
    {
    }
...
public:
    Window(tcu::egl::Display& display, EGLConfig config, const EGLint* attribList, int width, int height)
    ...
    tcu::egl::Display& getEglDisplay() override
    {
        return eglDisplay_;
    }

    tcu::egl::WindowSurface& getEglSurface() override
    {
        return surface_;
    }
...
public:
    ...
    tcu::NativeWindow* createWindow(
        tcu::NativeDisplay& dpy, EGLConfig config, const EGLint* attribList, int width, int height, qpVisibility visibility) override
    {
        tcu::egl::Display& eglDisplay = dpy.getEglDisplay();
        EGLDisplay display = eglDisplay.getEGLDisplay();
        CommandBufferGLESSetNextCreateWindowSurfaceCreatesPBuffer(display, width, height);
