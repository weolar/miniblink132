...
EGLAPI EGLBoolean EGLAPIENTRY eglChooseConfig(EGLDisplay dpy, const EGLint* attrib_list, EGLConfig* configs, EGLint config_size, EGLint* num_config)
{
    ...
    if (!display)
        return ts->ReturnError(EGL_BAD_DISPLAY, EGL_FALSE);
    return display->ChooseConfig(ts, attrib_list, configs, config_size, num_config);
}
...
EGLAPI EGLBoolean EGLAPIENTRY eglGetConfigAttrib(EGLDisplay dpy, EGLConfig cfg, EGLint attribute, EGLint* value)
{
    ThreadState* ts = ThreadState::Get();
    Display* display = ts->GetDisplay(dpy);
    if (!display)
...
EGLAPI EGLSurface EGLAPIENTRY eglCreateWindowSurface(EGLDisplay dpy, EGLConfig cfg, EGLNativeWindowType win, const EGLint* attrib_list)
{
    ThreadState* ts = ThreadState::Get();
    Display* display = ts->GetDisplay(dpy);
    if (!display)
...
EGLAPI EGLSurface EGLAPIENTRY eglCreatePbufferSurface(EGLDisplay dpy, EGLConfig cfg, const EGLint* attrib_list)
{
    ThreadState* ts = ThreadState::Get();
    Display* display = ts->GetDisplay(dpy);
    if (!display)
...
EGLAPI EGLBoolean EGLAPIENTRY eglDestroySurface(EGLDisplay dpy, EGLSurface sfe)
{
    ThreadState* ts = ThreadState::Get();
    Display* display = ts->GetDisplay(dpy);
    if (!display)
...
EGLAPI EGLContext EGLAPIENTRY eglCreateContext(EGLDisplay dpy, EGLConfig cfg, EGLContext share_ctx, const EGLint* attrib_list)
{
    ThreadState* ts = ThreadState::Get();
    Display* display = ts->GetDisplay(dpy);
    if (!display)
...
EGLAPI EGLBoolean EGLAPIENTRY eglMakeCurrent(EGLDisplay dpy, EGLSurface draw, EGLSurface read, EGLContext ctx)
{
    ...
    if (draw == EGL_NO_SURFACE && read == EGL_NO_SURFACE && ctx == EGL_NO_CONTEXT) {
        Display* display = dpy == EGL_NO_DISPLAY ? ts->GetDefaultDisplay() : ts->GetDisplay(dpy);
        if (!display)
            return ts->ReturnError(EGL_BAD_DISPLAY, EGL_FALSE);
...
{
    return reinterpret_cast<__eglMustCastToProperFunctionPointerType>(gles2::GetGLFunctionPointer(procname));
}
} // extern "C"
