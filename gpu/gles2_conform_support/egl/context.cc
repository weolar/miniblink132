...
// static
void Context::SetPlatformGpuFeatureInfo(const gpu::GpuFeatureInfo& gpu_feature_info)
{
...
Context::Context(Display* display, const Config* config)
    ...
    , is_destroyed_(false)
    , gpu_driver_bug_workarounds_(platform_gpu_feature_info_.enabled_gpu_driver_bug_workarounds)
...
bool Context::MakeCurrent(Context* current_context, Surface* current_surface, Context* new_context, Surface* new_surface)
{
...
void Context::GetGpuFence(uint32_t gpu_fence_id, base::OnceCallback<void(std::unique_ptr<gfx::GpuFence>)> callback)
{
    NOTREACHED();
}
...
void Context::SignalSyncToken(const gpu::SyncToken& sync_token, base::OnceClosure callback)
{
    NOTREACHED();
}
...
bool Context::CreateService(gl::GLSurface* gl_surface)
{
    ...
    scoped_refptr<gpu::gles2::FeatureInfo> feature_info(new gpu::gles2::FeatureInfo(gpu_driver_bug_workarounds_, gpu_feature_info));
    scoped_refptr<gpu::gles2::ContextGroup> group(new gpu::gles2::ContextGroup(gpu_preferences, true, &mailbox_manager_, nullptr /* memory_tracker */,
        &translator_cache_, &completeness_cache_, feature_info, true, nullptr /* image_factory */, nullptr /* progress_reporter */, gpu_feature_info,
        &discardable_manager_, &passthrough_discardable_manager_, &shared_image_manager_));

    auto command_buffer = std::make_unique<gpu::CommandBufferDirect>();

    std::unique_ptr<gpu::gles2::GLES2Decoder> decoder(
        gpu::gles2::GLES2Decoder::Create(command_buffer.get(), command_buffer->service(), &outputter_, group.get()));
    ...
    scoped_refptr<gl::GLContext> gl_context(gl::init::CreateGLContext(nullptr, gl_surface, context_attribs));
    if (!gl_context)
        return false;
    ...
    auto result = decoder->Initialize(gl_surface, gl_context.get(), gl_surface->IsOffscreen(), gpu::gles2::DisallowedFeatures(), helper);
    if (result != gpu::ContextResult::kSuccess)
        return false;

    auto gles2_cmd_helper = std::make_unique<gpu::gles2::GLES2CmdHelper>(command_buffer.get());
    ...
    auto transfer_buffer = std::make_unique<gpu::TransferBuffer>(gles2_cmd_helper.get());
    ...
    auto context = std::make_unique<gpu::gles2::GLES2Implementation>(
        gles2_cmd_helper_.get(), nullptr, transfer_buffer_.get(), kBindGeneratesResources, kLoseContextWhenOutOfMemory, kSupportClientSideArrays, this);
