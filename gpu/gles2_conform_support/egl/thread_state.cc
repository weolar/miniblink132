...
egl::ThreadState* ThreadState::Get()
{
...
#if BUILDFLAG(IS_WIN)
            argv = base::SplitString(base::UTF8ToWide(env_string), base::kWhitespaceWide, base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
            argv.insert(argv.begin(), base::UTF8ToWide("dummy"));
#else
            argv = base::SplitString(env_string, base::kWhitespaceASCII, base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
...
#endif
            gl::GLDisplay* display = gl::init::InitializeGLNoExtensionsOneOff(/*init_bindings=*/true,
                /*system_device_id=*/0);
            gpu::GpuFeatureInfo gpu_feature_info;
            if (!command_line->HasSwitch(switches::kDisableGpuDriverBugWorkarounds)) {
                gpu::GPUInfo gpu_info;
                gpu::CollectGraphicsInfoForTesting(&gpu_info);
                gpu_feature_info = gpu::ComputeGpuFeatureInfo(gpu_info, gpu::GpuPreferences(), command_line, nullptr);
                Context::SetPlatformGpuFeatureInfo(gpu_feature_info);
            }

            gl::init::SetDisabledExtensionsPlatform(gpu_feature_info.disabled_extensions);
    ...
    egl::ThreadState* thread_state = static_cast<egl::ThreadState*>(gpu::ThreadLocalGetValue(g_egl_thread_state_key));
    if (!thread_state) {
...
void ThreadState::ReleaseThread()
{
    ...
    if (g_egl_active_thread_count > 0) {
        ...
        if (thread_state->current_context()) {
            Context::MakeCurrent(thread_state->current_context(), thread_state->current_surface(), nullptr, nullptr);
        }
        delete thread_state;
...
ThreadState::AutoCurrentContextRestore::AutoCurrentContextRestore(ThreadState* thread_state)
...
ThreadState::AutoCurrentContextRestore::~AutoCurrentContextRestore()
{
    if (Context* current_context = thread_state_->current_context()) {
        current_context->ApplyCurrentContext(thread_state_->current_surface()->gl_surface());
    } else {
        Context::ApplyContextReleased();
    }
}

void ThreadState::AutoCurrentContextRestore::SetCurrent(Surface* surface, Context* context)
{
