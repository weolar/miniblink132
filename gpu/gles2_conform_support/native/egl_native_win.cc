...
GTFbool GTFNativeCreateWindow(EGLNativeDisplayType nativeDisplay, EGLDisplay eglDisplay, EGLConfig eglConfig, const char* title, int width, int height,
    EGLNativeWindowType* pNativeWindow)
{
    ...
    HWND hwnd = CreateWindow(wnd_class.lpszClassName, wnd_title.c_str(), wnd_style, 0, 0, wnd_rect.right - wnd_rect.left, wnd_rect.bottom - wnd_rect.top,
...
void GTFNativeDestroyWindow(EGLNativeDisplayType nativeDisplay, EGLNativeWindowType nativeWindow)
{
    DestroyWindow(nativeWindow);
    UnregisterClass(kWindowClassName, GetModuleHandle(nullptr));
}

EGLImageKHR GTFCreateEGLImage(int width, int height, GLenum format, GLenum type)
{
