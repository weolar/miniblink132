...
bool RunGLES2ConformTest(const char* path)
{
    ...
    base::FilePath test_expectations_path = src_path.Append(FILE_PATH_LITERAL("gpu"))
    ...
    if ((config_os & gpu::GPUTestConfig::kOsWin) != 0) {
        std::string angle_renderer = current_cmd_line->HasSwitch("use-angle") ? current_cmd_line->GetSwitchValueASCII("use-angle") : "d3d11";
    ...
    int32_t expectation = test_expectations.GetTestExpectation(test_name, bot_config);
    ...
    base::FilePath program(test_path.Append(FILE_PATH_LITERAL("gles2_conform_test_windowless")));
