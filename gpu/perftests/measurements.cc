...
Measurement::Measurement(const std::string& metric_basename, const base::TimeDelta wall_time, const base::TimeDelta cpu_time, const base::TimeDelta gpu_time)
...
{
    auto reporter = std::make_unique<perf_test::PerfResultReporter>(metric_basename, story);
...
{
    return Measurement(metric_basename, wall_time / a, cpu_time / a, gpu_time / a);
}
...
Measurement MeasurementTimers::GetAsMeasurement(const std::string& metric_basename)
{
    ...
    return Measurement(metric_basename, wall_time_, cpu_time_, base::Microseconds(gpu_time));
}
