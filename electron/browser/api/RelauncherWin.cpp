...
HANDLE GetParentProcessHandle(base::ProcessHandle handle)
{
    ...
    LONG status = NtQueryInformationProcess(handle, ProcessBasicInformation, &pbi, sizeof(PROCESS_BASIC_INFORMATION), NULL);
    if (!NT_SUCCESS(status)) {
        LOG(ERROR) << "NtQueryInformationProcess failed";
        return NULL;
    }

    return ::OpenProcess(PROCESS_ALL_ACCESS, TRUE, pbi.InheritedFromUniqueProcessId);
}
...
void RelauncherSynchronizeWithParent()
{
    ...
    base::win::ScopedHandle parent_process(GetParentProcessHandle(process.Handle()));

    // Notify the parent process that it can quit now.
    StringType name = internal::GetWaitEventName(process.Pid());
    base::win::ScopedHandle wait_event(::CreateEventW(NULL, TRUE, FALSE, name.c_str()));
...
int LaunchProgram(const StringVector& relauncher_args, const StringVector& argv)
{
    base::LaunchOptions options;
    base::Process process = base::LaunchProcess(ArgvToCommandLineString(argv), options);
