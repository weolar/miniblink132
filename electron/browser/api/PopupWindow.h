...
public:
    ...
    static mbWebView onCreateViewCallbackStatic(
        mbWebView webView, void* param, wkeNavigationType navigationType, const wkeString url, const wkeWindowFeatures* windowFeatures)
    {
    ...
    void onPaintUpdatedInCompositeThread(const HDC hdc, int x, int y, int cx, int cy)
    {
        ...
        if (!m_memoryBMP || m_clientRect.top != rectDest.top || m_clientRect.bottom != rectDest.bottom || m_clientRect.right != rectDest.right
            || m_clientRect.left != rectDest.left) {
    ...
    static LRESULT CALLBACK windowProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
    {
        ...
        case WM_NCDESTROY:
            ...
            ::RevokeDragDrop(hWnd);
            ThreadCall::callBlinkThreadSync([wkeWebview, self] { wkeDestroyWebView(wkeWebview); });
        ...
        case WM_KEYDOWN: {
            ...
            ThreadCall::callBlinkThreadAsync([wkeWebview, virtualKeyCode, flags] { wkeFireKeyDownEvent(wkeWebview, virtualKeyCode, flags, false); });
        ...
        case WM_KEYUP: {
            ...
            ThreadCall::callBlinkThreadAsync([wkeWebview, virtualKeyCode, flags] { wkeFireKeyUpEvent(wkeWebview, virtualKeyCode, flags, false); });
...
private:
    ...
    static mbWebView onCreateViewInBlinkThread(
        mbWebView webView, wkeNavigationType navigationType, const wkeString url, const wkeWindowFeatures* windowFeatures)
    {
    ...
    void newWindowInUiThreadWhenBlinkCall(const WebContents::CreateWindowParam* createWindowParam)
    {
        ...
        m_hWnd = ::CreateWindowEx(createWindowParam->styleEx, // window ex-style
...
private:
    enum WindowState { WindowUninited, WindowInited, WindowDestroying, WindowDestroyed };
    WindowState m_state;
