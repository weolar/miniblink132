...
private:
    ...
    v8::Local<v8::Object> getBoundsApi()
    {
        ...
        v8::Local<v8::Integer> x = v8::Integer::New(isolate(), clientRect.left + pt.x);
        v8::Local<v8::Integer> y = v8::Integer::New(isolate(), clientRect.top + pt.y);
...
//         OutputDebugStringA(temp.c_str());
    ...
    static void MB_CALL_TYPE onConsoleCallback(
    ...
    static void MB_CALL_TYPE onViewLoadInfo(mbWebView webView, void* param, mbViewLoadType type, mbViewLoadCallbackInfo* info)
    {
        ...
        if (MB_DID_NAVIGATE == type)
            mbRunJs(self->m_webContents->getMbView(), mbWebFrameGetMainFrame(webView), ";", false, nullptr, nullptr,
            ...
            FROM_HERE, [id, self, state, isDestroyApiBeCalled, type, resourceType, httpResponseCode, url, newUrl, method, referrer] {
                if ((!IdLiveDetect::get()->isLive(id) || WindowDestroying == state || WindowDestroyed == state || isDestroyApiBeCalled)) {
                    ...
                    self->m_webContents->mate::EventEmitter<WebContents>::emit(
                        "did-get-redirect-request", *url, *newUrl, resourceType == MB_RESOURCE_TYPE_MAIN_FRAME, httpResponseCode, *method, *referrer);
                }
