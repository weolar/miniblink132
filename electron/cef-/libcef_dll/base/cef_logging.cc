...
static void POSSIBLY_UNUSED wrap_posix_strerror_r(char* (*strerror_r_ptr)(int, char*, size_t), int err, char* buf, size_t len)
{
    // GNU version.
...
static void POSSIBLY_UNUSED wrap_posix_strerror_r(int (*strerror_r_ptr)(int, char*, size_t), int err, char* buf, size_t len)
{
    int old_errno = errno;
...
std::string SystemErrorCodeToString(SystemErrorCode error_code)
{
    ...
    DWORD len = FormatMessageA(flags, NULL, error_code, 0, msgbuf, arraysize(msgbuf), NULL);
    std::stringstream ss;
    if (len) {
        std::string s(msgbuf);
        ...
        ss << "Error (0x" << std::hex << GetLastError() << ") while retrieving error. (0x" << error_code << ")";
    }
    return ss.str();
}
#elif defined(OS_POSIX)
