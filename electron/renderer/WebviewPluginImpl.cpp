...
void WebviewPluginImpl::staticOnWillReleaseScriptContextCallback(mbWebView webView, WebviewPluginImpl* self, mbWebFrameHandle frame, void* context, int worldId)
{
    //     v8::Local<v8::Context>& contextV8 = *(v8::Local<v8::Context>*)context;
...
void WebviewPluginImpl::onLoadingFinishCallback(mbWebView webView, WebviewPluginImpl* self, const utf8* url, mbLoadingResult result, const utf8* failedReason)
{
    //     int id = self->m_id;
...
void WebviewPluginImpl::onDocumentReady(mbWebView webView, WebviewPluginImpl* self)
{
    //     base::ListValue listParams;
    //     std::string jsonRet;
...
WebviewPluginImpl::WebviewPluginImpl(mbWebView parentWebview)
{
    ...
    //
    //     mbOnLoadingFinish(m_webview, (wkeLoadingFinishCallback)onLoadingFinishCallback, this);
...
void WebviewPluginImpl::staticOnPaintUpdated(mbWebView webView, WebviewPluginImpl* self, const HDC hdc, int x, int y, int cx, int cy)
{
    //     int id = self->m_id;
