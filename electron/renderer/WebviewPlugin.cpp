...
static void npVariantToListValue(WebviewPluginImpl* impl, const NPVariant& arg, base::Value::List* listParams);

static void npVariantTypeObjectToDirValue(WebviewPluginImpl* impl, NPObject* objectValue, base::Value::Dict* dirParams)
{
    ...
    for (uint32_t i = 0; i < count; ++i) {
        ...
        case NPVariantType_Bool:
            dirParams->Set(method, result.value.boolValue);
            break;
        case NPVariantType_Int32:
            dirParams->Set(method, result.value.intValue);
            break;
        case NPVariantType_Double:
            dirParams->Set(method, result.value.doubleValue);
            break;
        case NPVariantType_String:
            dirParams->Set(method, std::string(result.value.stringValue.UTF8Characters, result.value.stringValue.UTF8Length));
            break;

        case NPVariantType_Object:
            base::Value::Dict dirParamsObj;
            npVariantTypeObjectToDirValue(impl, result.value.objectValue, &dirParamsObj);
            dirParams->Set(method, std::move(dirParamsObj));
            break;
        }
...
void npVariantToListValue(WebviewPluginImpl* impl, const NPVariant& arg, base::Value::List* listParams)
{
    switch (arg.type) {
    case NPVariantType_Bool:
        listParams->Append(arg.value.boolValue);
        break;
    case NPVariantType_Int32:
        listParams->Append(arg.value.intValue);
        break;
    case NPVariantType_Double:
        listParams->Append(arg.value.doubleValue);
        break;
    case NPVariantType_String:
        listParams->Append(std::string(arg.value.stringValue.UTF8Characters, arg.value.stringValue.UTF8Length));
        break;

    case NPVariantType_Object:
        base::Value::Dict dirParams;
        npVariantTypeObjectToDirValue(impl, arg.value.objectValue, &dirParams);
        listParams->Append(std::move(dirParams));
        break;
    }
}
...
bool pluginHasMethod(NPObject* obj, NPIdentifier methodName)
{
    ...
    const char* allowList[] = { "native_getURL",
        //"native_loadURL",
...
static bool doExecuteJavaScript(WebviewPluginImpl* impl, const NPVariant* args, uint32_t argCount, NPVariant* result)
{
    ...
    //     mbWebView webview = impl->getWebview();
    //     mbWebFrameHandle frame = mbWebFrameGetMainFrame(webview);
    //
    //     std::string* scriptString = new std::string("return function(args){ return JSON.stringify({\"ret\": args}); }");
...
bool pluginInvoke(NPObject* obj, NPIdentifier methodName, const NPVariant* args, uint32_t argCount, NPVariant* result)
{
    ...
    if ("native_getTitle" == method) {
        const utf8* titile = mbGetTitle(impl->getWebview());
        toResultString(titile, result);
        return true;
    }
    if ("native_isLoading" == method) {
        BOOL b = mbIsLoading(impl->getWebview());
    ...
    if ("native_stop" == method) {
        mbStopLoading(impl->getWebview());
        INT32_TO_NPVARIANT(0, *result);
        return true;
    }
    if ("native_reload" == method || "native_reloadIgnoringCache" == method) {
        mbReload(impl->getWebview());
        INT32_TO_NPVARIANT(0, *result);
        return true;
    }
    if ("native_canGoBack" == method) {
        bool b = mbCanGoBackOrForward(impl->getWebview(), TRUE);
        BOOLEAN_TO_NPVARIANT(b, *result);
        return true;
    }
    if ("native_canGoForward" == method) {
        BOOL b = mbCanGoBackOrForward(impl->getWebview(), FALSE);
        BOOLEAN_TO_NPVARIANT(b, *result);
        return true;
    }
    ...
    if ("native_goBack" == method) {
        mbGoBack(impl->getWebview());
        BOOLEAN_TO_NPVARIANT(true, *result);
        return true;
    }
    if ("native_goForward" == method) {
        mbGoForward(impl->getWebview());
        BOOLEAN_TO_NPVARIANT(true, *result);
        return true;
    }
    ...
    if ("native_setUserAgent" == method) {
        if (argCount >= 1 && NPVariantType_String == args[0].type) {
            ...
            mbSetUserAgent(impl->getWebview(), ua.c_str());
        }
        BOOLEAN_TO_NPVARIANT(false, *result);
        return true;
    }
    ...
    if ("native_undo" == method) {
        mbEditorUndo(impl->getWebview());
        BOOLEAN_TO_NPVARIANT(true, *result);
        return true;
    }
    if ("native_redo" == method) {
        mbEditorRedo(impl->getWebview());
        BOOLEAN_TO_NPVARIANT(true, *result);
        return true;
    }
    if ("native_cut" == method) {
        mbEditorCut(impl->getWebview());
        BOOLEAN_TO_NPVARIANT(true, *result);
        return true;
    }
    if ("native_copy" == method) {
        mbEditorCopy(impl->getWebview());
        BOOLEAN_TO_NPVARIANT(true, *result);
        return true;
    }
    ...
    if ("native_delete" == method) {
        mbEditorDelete(impl->getWebview());
        BOOLEAN_TO_NPVARIANT(true, *result);
        return true;
    }
    if ("selectAll" == method) {
        mbEditorSelectAll(impl->getWebview());
        BOOLEAN_TO_NPVARIANT(true, *result);
        return true;
    }
    if ("native_unselect" == method) {
        mbEditorUnSelect(impl->getWebview());
        BOOLEAN_TO_NPVARIANT(true, *result);
        return true;
    }
    ...
    if ("native_setZoomFactor" == method) {
        if (argCount >= 1 && NPVariantType_Double == args[0].type) {
            mbSetZoomFactor(impl->getWebview(), (float)args[0].value.doubleValue);
        }
    }
    if ("native_insertCSS" == method) {
        if (argCount >= 1) {
            if (NPVariantType_String == args[0].type) {
                ...
                mbInsertCSSByFrame(impl->getWebview(), mbWebFrameGetMainFrame(impl->getWebview()), cssText.c_str());
            }
        }
        BOOLEAN_TO_NPVARIANT(false, *result);
        return true;
