...
public:
    ...
    void onStartDragging(wkeWebView webView, void* param, wkeWebFrameHandle frame, const wkeWebDragData* wkeDragData, wkeWebDragOperationsMask mask,
        const void* image, const wkePoint* dragImageOffset)
    {
        ...
        *clientPoint = *screenPoint;
        ...
        ThreadCall::callBlinkThreadAsync([data, id, webview, screenPoint, clientPoint, grfKeyState] {
            if (IdLiveDetect::get()->isLive(id)) {
                ...
                wkeWebDragOperation op = wkeDragTargetDragEnter(
                    webview, data, clientPoint, screenPoint, keyStateToDragOperation(grfKeyState), keyStateToDragOperation(grfKeyState));
            }
        ...
        if (m_dragData) {
            ...
            *clientPoint = *screenPoint;
            ...
            ThreadCall::callBlinkThreadAsync([id, webview, screenPoint, clientPoint, grfKeyState] {
                if (IdLiveDetect::get()->isLive(id)) {
                    wkeWebDragOperation op
                        = wkeDragTargetDragOver(webview, clientPoint, screenPoint, wkeWebDragOperationEvery, keyStateToDragOperation(grfKeyState));
                }
                delete screenPoint;
                delete clientPoint;
            });
