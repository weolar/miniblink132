...
template <typename T> struct CallbackParamTraits {
    typedef T LocalType;
};
template <typename T> struct CallbackParamTraits<const T&> {
    typedef T LocalType;
};
template <typename T> struct CallbackParamTraits<const T*> {
    typedef T* LocalType;
...
private:
    static void FirstWeakCallback(const v8::WeakCallbackInfo<CallbackHolderBase>& data);
    static void SecondWeakCallback(const v8::WeakCallbackInfo<CallbackHolderBase>& data);
...
template <typename Sig> class CallbackHolder : public CallbackHolderBase {
public:
    CallbackHolder(v8::Isolate* isolate, const base::RepeatingCallback<Sig>& callback, int flags)
...
template <typename ClassType, typename Type> class CallbackHolderGetSet : public CallbackHolderBase {
public:
    CallbackHolderGetSet(
        v8::Isolate* isolate, const base::RepeatingCallback<Type(ClassType)>& get_callback, const base::RepeatingCallback<void(ClassType, Type)>& set_callback)
        : CallbackHolderBase(isolate)
        , get_callback_(get_callback)
        , set_callback_(set_callback)
    {
    }
    base::RepeatingCallback<Type(ClassType)> get_callback_;
    base::RepeatingCallback<void(ClassType, Type)> set_callback_;
...
template <typename T> bool GetNextArgument(Arguments* args, int create_flags, bool is_first, T* result)
{
    bool b = false;
...
inline bool GetNextArgument(Arguments* args, int create_flags, bool is_first, Arguments* result)
{
    *result = *args;
    return true;
}
...
inline bool GetNextArgument(Arguments* args, int create_flags, bool is_first, v8::Isolate** result)
{
    *result = args->isolate();
    return true;
}
...
// at position |index|.
template <size_t index, typename ArgType> struct ArgumentHolder {
...
template <> struct ArgumentHolder<1, const v8::FunctionCallbackInfo<v8::Value>&> {
...
template <typename IndicesType, typename... ArgTypes> class Invoker { };

template <size_t... indices, typename... ArgTypes> class Invoker<IndicesHolder<indices...>, ArgTypes...> : public ArgumentHolder<indices, ArgTypes>... {
public:
    ...
    template <typename ReturnType> void DispatchToCallback(base::RepeatingCallback<ReturnType(ArgTypes...)> callback)
    {
...
private:
    ...
    template <typename... T> static bool And(bool arg1, T... args)
    {
...
template <typename Sig> struct Dispatcher { };

template <typename ReturnType, typename... ArgTypes> struct Dispatcher<ReturnType(ArgTypes...)> {
    static void DispatchToCallback(const v8::FunctionCallbackInfo<v8::Value>& info)
    {
        Arguments args(info);
...
template <typename ClassType, typename Type> struct DispatcherAccessor {
...
// JavaScript arguments are automatically converted via gin_helper::Converter, as is
...
v8::Local<v8::FunctionTemplate> CreateFunctionTemplate(v8::Isolate* isolate, const base::RepeatingCallback<Sig> callback, int callback_flags = 0)
{
    typedef internal::CallbackHolder<Sig> HolderT;
    HolderT* holder = new HolderT(isolate, callback, callback_flags);

    return v8::FunctionTemplate::New(
        isolate, &internal::Dispatcher<Sig>::DispatchToCallback, ConvertToV8<v8::Local<v8::External>>(isolate, holder->GetHandle(isolate)));
}
...
void SetMemberGetSetAccessor(v8::Isolate* isolate, v8::Local<v8::ObjectTemplate> obj_template, v8::Local<v8::String> name,
    const base::RepeatingCallback<Type(ClassType)> get_callback, const base::RepeatingCallback<void(ClassType, Type)> set_callback)
{
...
template <typename Sig>
void CreateFunctionHandler(v8::Isolate* isolate, v8::Local<v8::ObjectTemplate> tmpl, const base::RepeatingCallback<Sig> callback, int callback_flags = 0)
{
    typedef internal::CallbackHolder<Sig> HolderT;
    HolderT* holder = new HolderT(isolate, callback, callback_flags);
    tmpl->SetCallAsFunctionHandler(&internal::Dispatcher<Sig>::DispatchToCallback, ConvertToV8<v8::Local<v8::External>>(isolate, holder->GetHandle(isolate)));
}
