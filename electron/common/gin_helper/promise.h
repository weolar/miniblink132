...
//#include "gin/microtasks_scope.h"
...
public:
    ...
    // Promise<T> type.
    static void RejectPromise(PromiseBase&& promise, std::string errmsg)
    {
        //         if (gin_helper::Locker::IsBrowserProcess() && !content::BrowserThread::CurrentlyOn(content::BrowserThread::UI)) {
        //             base::PostTask(
    ...
    v8::Maybe<bool> RejectWithErrorMessage(std::string message);
...
template <typename RT> class Promise : public PromiseBase {
public:
    ...
    static v8::Local<v8::Promise> ResolvedPromise(v8::Isolate* isolate, RT result)
    {
    ...
    v8::Maybe<bool> Resolve(const RT& value)
    {
        ...
        return GetInner()->Resolve(GetContext(), gin_helper::ConvertToV8(isolate(), value));
    }

    template <typename... ResolveType> v8::MaybeLocal<v8::Promise> Then(std::function<void(ResolveType...)>&& cb)
    {
        static_assert(sizeof...(ResolveType) <= 1, "A promise's 'Then' callback should only receive at most one parameter");
        static_assert(std::is_same<RT, std::tuple_element_t<0, std::tuple<ResolveType...>>>(),
        ...
        v8::Local<v8::Value> value = gin_helper::ConvertToV8(isolate(), std::move(cb));
...
template <typename T> struct Converter<gin_helper::Promise<T>> {
    static v8::Local<v8::Value> ToV8(v8::Isolate* isolate, const gin_helper::Promise<T>& val)
    {
...
template <> struct Converter<v8::Local<v8::Promise>> {
    static v8::Local<v8::Value> ToV8(v8::Isolate* isolate, const v8::Local<v8::Promise>& val)
    {
