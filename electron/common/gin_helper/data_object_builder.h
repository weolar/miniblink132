...
#ifndef GIN_HELPER_DATA_OBJECT_BUILDER_H_
#define GIN_HELPER_DATA_OBJECT_BUILDER_H_
...
public:
    ...
    template <typename T> DataObjectBuilder& Set(base::StringPiece key, T&& value)
    {
        DCHECK(!object_.IsEmpty());
        v8::Local<v8::String> v8_key = StringToSymbol(isolate_, key);
        v8::Local<v8::Value> v8_value = ConvertToV8(isolate_, std::forward<T>(value));
        CHECK(object_->CreateDataProperty(context_, v8_key, v8_value).ToChecked());
        return *this;
    }

    template <typename T> DataObjectBuilder& Set(uint32_t index, T&& value)
    {
        DCHECK(!object_.IsEmpty());
        v8::Local<v8::Value> v8_value = ConvertToV8(isolate_, std::forward<T>(value));
...
#endif // GIN_HELPER_DATA_OBJECT_BUILDER_H_
