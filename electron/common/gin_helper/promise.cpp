...
PromiseBase::PromiseBase(v8::Isolate* isolate)
    : PromiseBase(isolate, v8::Promise::Resolver::New(isolate->GetCurrentContext()).ToLocalChecked())
{
}

PromiseBase::PromiseBase(v8::Isolate* isolate, v8::Local<v8::Promise::Resolver> handle)
    : isolate_(isolate)
...
v8::Maybe<bool> PromiseBase::RejectWithErrorMessage(std::string message)
{
    //gin_helper::Locker locker(isolate());
    v8::HandleScope handle_scope(isolate());
    //gin_helper::MicrotasksScope microtasks_scope(isolate());
    ...
    v8::Local<v8::Value> error = v8::Exception::Error(gin_helper::StringToV8(isolate(), message));
...
void Promise<void>::ResolvePromise(Promise<void> promise)
{
    ...
    //             base::BindOnce([](Promise<void> promise) { promise.Resolve(); }, std::move(promise)));
    //     } else {
    promise.Resolve();
    //     }
}
