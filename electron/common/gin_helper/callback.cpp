...
void CallTranslater(v8::Local<v8::External> external, v8::Local<v8::Object> state, gin_helper::Arguments* args)
{
    // Whether the callback should only be called once.
    v8::Isolate* isolate = args->isolate();
    auto context = isolate->GetCurrentContext();
    bool one_time = state->Has(context, gin_helper::StringToSymbol(isolate, "oneTime")).ToChecked();

    // Check if the callback has already been called.
    if (one_time) {
        auto called_symbol = gin_helper::StringToSymbol(isolate, "called");
        if (state->Has(context, called_symbol).ToChecked()) {
            args->ThrowTypeError("One-time callback was called more than once");
            return;
        } else {
            state->Set(context, called_symbol, v8::Boolean::New(isolate, true)).ToChecked();
        }
    }
...
struct DeleteOnUIThread {
    template <typename T> static void Destruct(const T* x)
    {
        ...
        //             content::BrowserThread::DeleteSoon(content::BrowserThread::UI, FROM_HERE, x);
        //         } else {
        //             delete x;
        //         }

        if (!content::ThreadCall::isUiThread()) {
            content::ThreadCall::callUiThreadAsync(FROM_HERE, [x] { delete x; });
        } else
            delete x;
    }
};
...
template <typename T> class RefCountedGlobal : public base::RefCountedThreadSafe<RefCountedGlobal<T>, DeleteOnUIThread> {
public:
...
v8::Local<v8::Value> CreateFunctionFromTranslater(v8::Isolate* isolate, const Translater& translater, bool one_time)
{
    // The FunctionTemplate is cached.
    if (g_call_translater.IsEmpty())
        g_call_translater.Reset(isolate, CreateFunctionTemplate(isolate, base::BindRepeating(&CallTranslater)));

    v8::Local<v8::FunctionTemplate> call_translater = v8::Local<v8::FunctionTemplate>::New(isolate, g_call_translater);
    auto* holder = new TranslaterHolder(isolate);
    holder->translater = translater;
    gin_helper::Dictionary state = gin_helper::Dictionary::CreateEmpty(isolate);
    if (one_time)
        state.Set("oneTime", true);
    auto context = isolate->GetCurrentContext();

    return BindFunctionWith(
        isolate, context, call_translater->GetFunction(context).ToLocalChecked(), holder->handle.Get(isolate), gin_helper::ConvertToV8(isolate, state));
}

// func.bind(func, arg1).
// NB(zcbenz): Using C++11 version crashes VS.
v8::Local<v8::Value> BindFunctionWith(
    v8::Isolate* isolate, v8::Local<v8::Context> context, v8::Local<v8::Function> func, v8::Local<v8::Value> arg1, v8::Local<v8::Value> arg2)
{
    v8::MaybeLocal<v8::Value> bind = func->Get(context, gin_helper::StringToV8(isolate, "bind"));
    CHECK(!bind.IsEmpty());
    v8::Local<v8::Function> bind_func = bind.ToLocalChecked().As<v8::Function>();
    v8::Local<v8::Value> converted[] = { func, arg1, arg2 };
    return bind_func->Call(context, func, std::size(converted), converted).ToLocalChecked();
}
