...
template <typename T> class RefCountedGlobal;
...
template <typename Sig> struct V8FunctionInvoker { };

template <typename... ArgTypes> struct V8FunctionInvoker<v8::Local<v8::Value>(ArgTypes...)> {
    static v8::Local<v8::Value> Go(v8::Isolate* isolate, const SafeV8Function& function, ArgTypes... raw)
    {
...
template <typename... ArgTypes> struct V8FunctionInvoker<void(ArgTypes...)> {
    static void Go(v8::Isolate* isolate, const SafeV8Function& function, ArgTypes... raw)
    {
        gin_helper::Locker locker(isolate);
        ...
        std::vector<v8::Local<v8::Value>> args { gin_helper::ConvertToV8(isolate, std::forward<ArgTypes>(raw))... };
        holder->Call(context, holder, args.size(), args.empty() ? nullptr : &args.front()).IsEmpty();
    }
};

template <typename ReturnType, typename... ArgTypes> struct V8FunctionInvoker<ReturnType(ArgTypes...)> {
    static ReturnType Go(v8::Isolate* isolate, const SafeV8Function& function, ArgTypes... raw)
    {
        gin_helper::Locker locker(isolate);
        v8::HandleScope handle_scope(isolate);
...
using Translater = base::RepeatingCallback<void(gin_helper::Arguments* args)>;
...
template <typename Sig> struct NativeFunctionInvoker { };
...
template <typename Sig> v8::Local<v8::Value> CallbackToV8Leaked(v8::Isolate* isolate, const base::RepeatingCallback<Sig>& val)
{
    Translater translater = base::BindRepeating(&NativeFunctionInvoker<Sig>::Go, val);
