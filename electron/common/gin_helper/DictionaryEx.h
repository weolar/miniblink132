...
// DictionaryEx is useful when writing bindings for a function that either
// receives an arbitrary JavaScript object as an argument or returns an
// arbitrary JavaScript object as a result. For example, DictionaryEx is useful
// when you might use the |dictionary| type in WebIDL:
//
//   http://heycam.github.io/webidl/#idl-dictionaries
//
// WARNING: You cannot retain a DictionaryEx object in the heap. The underlying
//          storage for DictionaryEx is tied to the closest enclosing
//          v8::HandleScope. Generally speaking, you should store a DictionaryEx
//          on the stack.
//
class GIN_EXPORT DictionaryEx {
public:
    explicit DictionaryEx(v8::Isolate* isolate);
    DictionaryEx(v8::Isolate* isolate, v8::Local<v8::Object> object);
    ~DictionaryEx();

    static DictionaryEx CreateEmpty(v8::Isolate* isolate);
    ...
    {
        v8::Maybe<bool> result = GetHandle()->Has(isolate()->GetCurrentContext(), gin::StringToV8(isolate(), key));
        return !result.IsNothing() && result.FromJust();
    }
...
template <> struct GIN_EXPORT Converter<DictionaryEx> {
    static v8::Local<v8::Value> ToV8(v8::Isolate* isolate, DictionaryEx val);
    static bool FromV8(v8::Isolate* isolate, v8::Local<v8::Value> val, DictionaryEx* out);
};
