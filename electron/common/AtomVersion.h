...
#define ATOM_VERSION_STRING                                                                                                                                    \
    ...
    "." ATOM_STRINGIFY(ATOM_MINOR_VERSION) "." ATOM_STRINGIFY(ATOM_PATCH_VERSION) ATOM_TAG
#else
#define ATOM_VERSION_STRING                                                                                                                                    \
    ATOM_STRINGIFY(ATOM_MAJOR_VERSION)                                                                                                                         \
    "." ATOM_STRINGIFY(ATOM_MINOR_VERSION) "." ATOM_STRINGIFY(ATOM_PATCH_VERSION) ATOM_TAG "-pre"
#endif

#define ATOM_VERSION "v" ATOM_VERSION_STRING

#define ATOM_VERSION_AT_LEAST(major, minor, patch)                                                                                                             \
    (((major) < ATOM_MAJOR_VERSION) || ((major) == ATOM_MAJOR_VERSION && (minor) < ATOM_MINOR_VERSION)                                                         \
        || ((major) == ATOM_MAJOR_VERSION && (minor) == ATOM_MINOR_VERSION && (patch) <= ATOM_PATCH_VERSION))

#define kElectronUtilProcChannelId "ele-utilproc-channel-id"
#define kElectronUtilProcSetupPath "ele-utilproc-setup-path"
