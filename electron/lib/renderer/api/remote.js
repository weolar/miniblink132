...
const isPromise = require('electron').isPromise;
...
const metaToPlainObject = function (meta) {
  ...
  ref1 = meta.members
  for (i = 0, len = ref1.length; i < len; i++) {
    let name = ref1[i].name;
    let value = ref1[i].value;
    obj[name] = value
  }
  return obj
}
...
exports.getGlobal = function (name) {
  var ret = metaToValue(ipcRenderer.sendSync('ELECTRON_BROWSER_GLOBAL', name));
  return ret;
}
