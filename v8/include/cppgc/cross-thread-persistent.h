...
public:
    ...
    explicit CrossThreadPersistentBase(const void* raw)
        : PersistentBase(raw)
    {
    }
    ...
    {
        return reinterpret_cast<std::atomic<PersistentNode*>*>(&node_)->load(std::memory_order_acquire);
    }
...
#define V8_IS_ASAN 1
        ...
        reinterpret_cast<std::atomic<PersistentNode*>*>(&node_)->store(value, std::memory_order_release);
#endif // !V8_IS_ASAN
...
class BasicCrossThreadPersistent final : public CrossThreadPersistentBase, public LocationPolicy, private WeaknessPolicy, private CheckingPolicy {
public:
    ...
    ~BasicCrossThreadPersistent()
    {
        ...
        if (GetNodeSafe()) {
            ...
            if (IsValid(old_value)) {
                CrossThreadPersistentRegion& region = this->GetPersistentRegion(old_value);
    ...
    private:
        ...
        template <typename U, typename OtherWeaknessPolicy, typename OtherLocationPolicy, typename OtherCheckingPolicy> friend class BasicCrossThreadPersistent;
    };
   ...
   */
    ...
    {
        using OtherBasicCrossThreadPersistent = BasicCrossThreadPersistent<U, OtherWeaknessPolicy, OtherLocationPolicy, OtherCheckingPolicy>;
        PersistentRegionLock guard;
        return OtherBasicCrossThreadPersistent(typename OtherBasicCrossThreadPersistent::UnsafeCtorTag(), static_cast<U*>(Get()));
    }
    ...
    {
        return BasicCrossThreadPersistent<U, internal::StrongCrossThreadPersistentPolicy>(*this);
    }
...
private:
    ...
    void ClearFromGC() const
    {
        if (IsValid(GetValueFromGC())) {
            WeaknessPolicy::GetPersistentRegion(GetValueFromGC()).FreeNode(GetNodeFromGC());
...
struct IsWeak<BasicCrossThreadPersistent<T, internal::WeakCrossThreadPersistentPolicy, LocationPolicy, CheckingPolicy>> : std::true_type { };
 ...
 */
template <typename T> using CrossThreadPersistent = internal::BasicCrossThreadPersistent<T, internal::StrongCrossThreadPersistentPolicy>;
 ...
 *   moved concurrently on the thread owning the object.
 */
template <typename T> using WeakCrossThreadPersistent = internal::BasicCrossThreadPersistent<T, internal::WeakCrossThreadPersistentPolicy>;
