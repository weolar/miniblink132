...
struct SerializeContextDataCallback {
    using CallbackFunction = StartupData (*)(Local<Context> holder, int index, void* data);
...
struct SerializeAPIWrapperCallback {
    ...
    explicit SerializeAPIWrapperCallback(CallbackFunction function = nullptr, void* data = nullptr)
        : callback(function)
        , data(data)
    {
    }
...
struct DeserializeContextDataCallback {
    using CallbackFunction = void (*)(Local<Context> holder, int index, StartupData payload, void* data);
...
struct DeserializeAPIWrapperCallback {
    using CallbackFunction = void (*)(Local<Object> holder, StartupData payload, void* data);
    explicit DeserializeAPIWrapperCallback(CallbackFunction function = nullptr, void* data = nullptr)
        : callback(function)
        , data(data)
    {
    }
...
public:
   ...
   */
    ...
    explicit SnapshotCreator(
        Isolate* isolate, const intptr_t* external_references = nullptr, const StartupData* existing_blob = nullptr, bool owns_isolate = true);
   ...
   */
    V8_DEPRECATE_SOON("Use the version that passes CreateParams instead.")
    explicit SnapshotCreator(const intptr_t* external_references = nullptr, const StartupData* existing_blob = nullptr);
   ...
   */
    void SetDefaultContext(Local<Context> context, SerializeInternalFieldsCallback internal_fields_serializer = SerializeInternalFieldsCallback(),
   ...
   * in SetDefaultContext() but only applies to the context being added.
   */
    size_t AddContext(Local<Context> context, SerializeInternalFieldsCallback internal_fields_serializer = SerializeInternalFieldsCallback(),
        SerializeContextDataCallback context_data_serializer = SerializeContextDataCallback(),
        SerializeAPIWrapperCallback api_wrapper_serializer = SerializeAPIWrapperCallback());
   ...
   */
    template <class T> V8_INLINE size_t AddData(Local<Context> context, Local<T> object);
   ...
   * \returns the index for retrieval.
   */
    template <class T> V8_INLINE size_t AddData(Local<T> object);
...
template <class T> size_t SnapshotCreator::AddData(Local<Context> context, Local<T> object)
{
