...
public:
   ...
   */
    static Local<Context> New(Isolate* isolate, ExtensionConfiguration* extensions = nullptr,
        MaybeLocal<ObjectTemplate> global_template = MaybeLocal<ObjectTemplate>(), MaybeLocal<Value> global_object = MaybeLocal<Value>(),
        DeserializeInternalFieldsCallback internal_fields_deserializer = DeserializeInternalFieldsCallback(), MicrotaskQueue* microtask_queue = nullptr,
        DeserializeContextDataCallback context_data_deserializer = DeserializeContextDataCallback(),
        DeserializeAPIWrapperCallback api_wrapper_deserializer = DeserializeAPIWrapperCallback());
   ...
   */
    static MaybeLocal<Context> FromSnapshot(Isolate* isolate, size_t context_snapshot_index,
        DeserializeInternalFieldsCallback internal_fields_deserializer = DeserializeInternalFieldsCallback(), ExtensionConfiguration* extensions = nullptr,
        MaybeLocal<Value> global_object = MaybeLocal<Value>(), MicrotaskQueue* microtask_queue = nullptr,
        DeserializeContextDataCallback context_data_deserializer = DeserializeContextDataCallback(),
        DeserializeAPIWrapperCallback api_wrapper_deserializer = DeserializeAPIWrapperCallback());
   ...
   */
    template <class T> V8_INLINE MaybeLocal<T> GetDataFromSnapshotOnce(size_t index);
...
Local<Value> Context::GetEmbedderData(int index)
{
    ...
    auto isolate = reinterpret_cast<v8::Isolate*>(internal::IsolateFromNeverReadOnlySpaceObject(ctx));
...
void* Context::GetAlignedPointerFromEmbedderData(Isolate* isolate, int index)
{
    ...
    return reinterpret_cast<void*>(I::ReadExternalPointerField<internal::kEmbedderDataSlotPayloadTag>(isolate, embedder_data, value_offset));
#else
...
{
    if (auto repr = GetDataFromSnapshotOnce(index); repr != internal::ValueHelper::kEmpty) {
