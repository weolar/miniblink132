 ...
 * isolate.
 */
template <class T> class Eternal : public api_internal::IndirectHandleBase {
public:
    V8_INLINE Eternal() = default;

    template <class S> V8_INLINE Eternal(Isolate* isolate, Local<S> handle)
    {
        Set(isolate, handle);
    }
    ...
    template <class S> void Set(Isolate* isolate, Local<S> handle)
    {
        ...
        slot() = api_internal::Eternalize(isolate, *handle.template UnsafeAs<Value>());
    }
};
 ...
 *
 */
template <class T> class PersistentBase : public api_internal::IndirectHandleBase {
public:
    /**
   ...
   * and create a new one with the contents of other if other is non empty
   */
    template <class S> V8_INLINE void Reset(Isolate* isolate, const PersistentBase<S>& other);
   ...
   */
    template <typename P> V8_INLINE void SetWeak(P* parameter, typename WeakCallbackInfo<P>::Callback callback, WeakCallbackType type);
...
public:
    ...
    template <class S, class M> V8_INLINE static void Copy(const Persistent<S, M>& source, NonCopyablePersistent* dest)
    {
        static_assert(sizeof(S) < 0, "NonCopyablePersistentTraits::Copy is not instantiable");
    }
};
...
public:
   ...
   */
    ...
    V8_INLINE Persistent(Isolate* isolate, Local<S> that)
        : PersistentBase<T>(PersistentBase<T>::New(isolate, that.template value<S>()))
    {
        static_assert(std::is_base_of<T, S>::value, "type check");
    }
   ...
   */
    ...
    template <class S, class M2> V8_INLINE Persistent& operator=(const Persistent<S, M2>& that)
    {
        Copy(that);
        return *this;
    }
   ...
   */
    ...
    template <class S, class M2> V8_INLINE static Persistent<T, M>& Cast(const Persistent<S, M2>& that)
    {
...
#endif
        return reinterpret_cast<Persistent<T, M>&>(const_cast<Persistent<S, M2>&>(that));
    }

    // TODO(dcarney): this is pretty useless, fix or remove
    template <class S, class M2> V8_INLINE Persistent<S, M2>& As() const
    {
...
private:
    ...
    template <class S, class M2> V8_INLINE void Copy(const Persistent<S, M2>& that);
};
...
public:
   ...
   * Move via assignment.
   */
    template <class S> V8_INLINE Global& operator=(Global<S>&& rhs);
...
template <class T> internal::Address* PersistentBase<T>::New(Isolate* isolate, T* that)
{
    if (internal::ValueHelper::IsEmpty(that))
        return nullptr;
    return api_internal::GlobalizeReference(reinterpret_cast<internal::Isolate*>(isolate), internal::ValueHelper::ValueAsAddress(that));
}
 ...
 * and create a new one with the contents of other if other is non empty
 */
template <class T> template <class S> void PersistentBase<T>::Reset(Isolate* isolate, const Local<S>& other)
{
    static_assert(std::is_base_of<T, S>::value, "type check");
    Reset();
    if (other.IsEmpty())
 ...
 * and create a new one with the contents of other if other is non empty
 */
template <class T> template <class S> void PersistentBase<T>::Reset(Isolate* isolate, const PersistentBase<S>& other)
{
    static_assert(std::is_base_of<T, S>::value, "type check");
    Reset();
    if (other.IsEmpty())
...
template <class T>
template <typename P>
V8_INLINE void PersistentBase<T>::SetWeak(P* parameter, typename WeakCallbackInfo<P>::Callback callback, WeakCallbackType type)
{
...
#endif
    api_internal::MakeWeak(this->slot(), parameter, reinterpret_cast<Callback>(callback), type);
#if (__GNUC__ >= 8) && !defined(__clang__)
