...
public:
   ...
   */
    virtual uint64_t AddTraceEvent(char phase, const uint8_t* category_enabled_flag, const char* name, const char* scope, uint64_t id, uint64_t bind_id,
        int32_t num_args, const char** arg_names, const uint8_t* arg_types, const uint64_t* arg_values,
        std::unique_ptr<ConvertableToTraceFormat>* arg_convertables, unsigned int flags)
    {
        return 0;
    }
    virtual uint64_t AddTraceEventWithTimestamp(char phase, const uint8_t* category_enabled_flag, const char* name, const char* scope, uint64_t id,
        uint64_t bind_id, int32_t num_args, const char** arg_names, const uint8_t* arg_types, const uint64_t* arg_values,
        std::unique_ptr<ConvertableToTraceFormat>* arg_convertables, unsigned int flags, int64_t timestamp)
    {
        return 0;
    }
...
// entry object.
inline PlatformSharedMemoryHandle SharedMemoryHandleFromMachMemoryEntry(unsigned int port)
{
    return static_cast<PlatformSharedMemoryHandle>(port);
}
inline unsigned int MachMemoryEntryFromSharedMemoryHandle(PlatformSharedMemoryHandle handle)
{
...
// object.
inline PlatformSharedMemoryHandle SharedMemoryHandleFromFileMapping(void* handle)
{
    return reinterpret_cast<PlatformSharedMemoryHandle>(handle);
}
inline void* FileMappingFromSharedMemoryHandle(PlatformSharedMemoryHandle handle)
{
...
inline int FileDescriptorFromSharedMemoryHandle(PlatformSharedMemoryHandle handle)
{
...
public:
   ...
   * failure.
   */
    virtual V8_WARN_UNUSED_RESULT Address AllocateSharedPages(
        Address hint, size_t size, PagePermissions permissions, PlatformSharedMemoryHandle handle, uint64_t offset)
        = 0;
...
public:
   ...
   */
    ...
    {
        PostTaskOnWorkerThreadImpl(TaskPriority::kUserVisible, std::move(task), location);
    }
   ...
   */
    void CallBlockingTaskOnWorkerThread(std::unique_ptr<Task> task, const SourceLocation& location = SourceLocation::Current())
    {
        ...
        // priority pool.
        PostTaskOnWorkerThreadImpl(TaskPriority::kUserBlocking, std::move(task), location);
    }
   ...
   */
    void CallLowPriorityTaskOnWorkerThread(std::unique_ptr<Task> task, const SourceLocation& location = SourceLocation::Current())
    {
        ...
        // priority pool.
        PostTaskOnWorkerThreadImpl(TaskPriority::kBestEffort, std::move(task), location);
    }
   ...
   */
    ...
    {
        PostDelayedTaskOnWorkerThreadImpl(TaskPriority::kUserVisible, std::move(task), delay_in_seconds, location);
    }
...
protected:
   ...
   */
    virtual void PostDelayedTaskOnWorkerThreadImpl(TaskPriority priority, std::unique_ptr<Task> task, double delay_in_seconds, const SourceLocation& location)
        = 0;
};
