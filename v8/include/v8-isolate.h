...
public:
   ...
   */
    ...
    public:
        explicit SuppressMicrotaskExecutionScope(Isolate* isolate, MicrotaskQueue* microtask_queue = nullptr);
   ...
   */
    template <int N> Local<Value> ThrowError(const char (&message)[N])
    {
   ...
   */
    V8_DEPRECATE_SOON("Set the heap on Isolate creation using CreateParams instead.")
   ...
   * |kTerminatedExecution|.
   */
    using AtomicsWaitCallback = void (*)(AtomicsWaitEvent event, Local<SharedArrayBuffer> array_buffer, size_t offset_in_bytes, int64_t value,
        double timeout_in_ms, AtomicsWaitWakeHandle* stop_handle, void* data);
   ...
   * enabled.
   */
    V8_DEPRECATED("Will be removed, use ScriptCompiler::CompileOptions for enabling the "
