...
template <class T> class LocalBase;
template <class T> class Local;
template <class T> class LocalVector;
template <class F> class MaybeLocal;
...
template <class T> class BasicTracedReference;
template <class F> class TracedReference;
...
class Isolate;
class Object;
template <class F1, class F2, class F3> class PersistentValueMapBase;
class Primitive;
class Private;
template <class F> class PropertyCallbackInfo;
template <class F> class ReturnValue;
class String;
template <class F> class Traced;
...
template <typename T> class LocalUnchecked;
...
private:
    ...
    template <typename T> friend class LocalBase;
...
template <typename T> class LocalBase : public api_internal::DirectHandleBase {
protected:
    template <class F> friend class Local;

    V8_INLINE LocalBase() = default;

    V8_INLINE explicit LocalBase(internal::Address ptr)
    ...
    {
        return LocalBase<T>::New(isolate, internal::ValueHelper::ValueAsAddress(that));
    }
    ...
    V8_INLINE static LocalBase<T> FromRepr(internal::ValueHelper::InternalRepresentationType repr)
    {
...
template <typename T> class LocalBase : public api_internal::IndirectHandleBase {
protected:
    template <class F> friend class Local;
    ...
    template <typename S>
    V8_INLINE LocalBase(const LocalBase<S>& other)
        : IndirectHandleBase(other)
    {
    }

    V8_INLINE static LocalBase<T> New(Isolate* isolate, internal::Address value)
    {
        return LocalBase(HandleScope::CreateHandle(reinterpret_cast<internal::Isolate*>(isolate), value));
    }
...
public:
   ...
   */
    ...
    template <class S> V8_INLINE bool operator==(const Local<S>& that) const
    {
        return internal::HandleHelper::EqualHandles(*this, that);
    }

    template <class S> V8_INLINE bool operator==(const PersistentBase<S>& that) const
    {
        return internal::HandleHelper::EqualHandles(*this, that);
    }
    ...
    template <class S> V8_INLINE bool operator!=(const Persistent<S>& that) const
    {
        return !operator==(that);
    }
   ...
   * of the target type.
   */
    template <class S> V8_INLINE Local<S> As() const
    {
   ...
   */
    ...
    V8_INLINE static Local<T> New(Isolate* isolate, const BasicTracedReference<T>& that)
    {
        return New(isolate, that.template value<T, true>());
    }
...
private:
    ...
    template <class F1, class F2, class F3> friend class PersistentValueMapBase;
    template <class F> friend class ReturnValue;
    template <class F> friend class Traced;
    ...
    V8_INLINE explicit Local(const LocalBase<T>& other)
        : LocalBase<T>(other)
    {
    }

    V8_INLINE static Local<T> FromRepr(internal::ValueHelper::InternalRepresentationType repr)
    {
    ...
    template <class S> V8_INLINE Local<S> UnsafeAs() const
    {
...
template <typename T> class V8_TRIVIAL_ABI LocalUnchecked : public Local<T> {
public:
    LocalUnchecked()
        : Local<T>(Local<T>::do_not_check)
    {
    }
    ...
    // need to suppress it.
    LocalUnchecked(const LocalUnchecked& other) noexcept // NOLINT(runtime/explicit)
...
template <typename T> class StrongRootAllocator<LocalUnchecked<T>> : public StrongRootAllocatorBase {
public:
...
public:
   ...
   */
    template <class S> V8_WARN_UNUSED_RESULT V8_INLINE bool ToLocal(Local<S>* out) const
    {
        *out = local_;
        return !IsEmpty();
    }
   ...
   * MaybeLocal<> is empty.
   */
    template <class S> V8_INLINE Local<S> FromMaybe(Local<S> default_value) const
    {
   ...
   * type.
   */
    template <class S> V8_INLINE static MaybeLocal<T> Cast(MaybeLocal<S> that)
    {
#ifdef V8_ENABLE_CHECKS
        // If we're going to perform the type check then we have to check
        // that the handle isn't empty before doing the checked cast.
   ...
   * of the target type.
   */
    template <class S> V8_INLINE MaybeLocal<S> As() const
    {
...
private:
    ...
    template <typename S> friend class MaybeLocal;
};
...
class V8_EXPORT V8_NODISCARD EscapableHandleScope : public EscapableHandleScopeBase {
public:
    ...
    template <class T> V8_INLINE MaybeLocal<T> EscapeMaybe(MaybeLocal<T> value)
    {
