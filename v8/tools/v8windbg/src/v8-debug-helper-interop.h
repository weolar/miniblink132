...
constexpr char16_t kTaggedObjectU[] = u"v8::internal::Tagged<v8::internal::Object>";
...
struct StructField {
    StructField(std::u16string field_name, std::u16string type_name, uint64_t address, uint8_t num_bits, uint8_t shift_bits);
    ~StructField();
...
struct Property {
    Property(std::u16string property_name, std::u16string type_name, uint64_t address, size_t item_size);
    ~Property();
...
V8HeapObject GetHeapObject(WRL::ComPtr<IDebugHostContext> sp_context, uint64_t address, uint64_t referring_pointer, const char* type_name, bool is_compressed);

// Expand a compressed pointer from 32 bits to the format that
// GetObjectProperties expects for compressed pointers.
inline uint64_t ExpandCompressedPointer(uint32_t ptr)
{
    return ptr;
}
...
std::vector<Property> GetStackFrame(WRL::ComPtr<IDebugHostContext> sp_context, uint64_t frame_pointer);
