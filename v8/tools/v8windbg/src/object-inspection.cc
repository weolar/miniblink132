...
V8CachedObject::V8CachedObject(Location location, std::string uncompressed_type_name, WRL::ComPtr<IDebugHostContext> context, bool is_compressed)
...
HRESULT V8CachedObject::Create(IModelObject* p_v8_object_instance, IV8CachedObject** result)
{
    Location location;
    ...
    *result = WRL::Make<V8CachedObject>(location, static_cast<const char*>(type_name), context, is_compressed).Detach();
    return S_OK;
}

V8CachedObject::V8CachedObject(V8HeapObject heap_object)
    : heap_object_(std::move(heap_object))
...
IFACEMETHODIMP V8CachedObject::GetCachedV8HeapObject(V8HeapObject** pp_heap_object) noexcept
{
    if (!heap_object_initialized_) {
        ...
        HRESULT hr = sp_debug_host_memory->ReadBytes(
            context_.Get(), location_, reinterpret_cast<void*>(&tagged_ptr), is_compressed_ ? i::kTaggedSize : sizeof(void*), &bytes_read);
        ...
        if (hr != S_OK) {
            ...
            heap_object_ = ::GetHeapObject(context_, tagged_ptr, location_.GetOffset(), uncompressed_type_name_.c_str(), is_compressed_);
        }
    }
...
bool TryUnwrapTaggedMemberType(const std::u16string& type, std::u16string* result)
{
    std::u16string prefix = u"v8::internal::TaggedMember<";
    if (type.substr(0, prefix.length()) == prefix && type.back() == u'>') {
        if (result) {
            *result = type.substr(prefix.length(), type.length() - prefix.length() - 1);
        }
        return true;
    }
    return false;
...
V8ObjectKeyEnumerator::V8ObjectKeyEnumerator(WRL::ComPtr<IV8CachedObject>& v8_cached_object)
...
IFACEMETHODIMP V8ObjectKeyEnumerator::GetNext(BSTR* key, IModelObject** value, IKeyStore** metadata) noexcept
{
...
IFACEMETHODIMP V8LocalDataModel::InitializeObject(
    IModelObject* model_object, IDebugHostTypeSignature* matching_type_signature, IDebugHostSymbolEnumerator* wildcard_matches) noexcept
{
    return S_OK;
}
...
IFACEMETHODIMP V8ObjectDataModel::ToDisplayString(IModelObject* context_object, IKeyStore* metadata, BSTR* display_string) noexcept
{
    ...
    *display_string = ::SysAllocString(reinterpret_cast<const wchar_t*>(p_v8_heap_object->friendly_name.data()));
    return S_OK;
}
...
HRESULT CreateSyntheticObjectWithParentAndDataContext(IDebugHostContext* ctx, IModelObject* parent_model, IUnknown* data_context, IModelObject** result)
{
    WRL::ComPtr<IModelObject> value;
    RETURN_IF_FAIL(sp_data_model_manager->CreateSyntheticObject(ctx, &value));
    RETURN_IF_FAIL(value->AddParentModel(parent_model, nullptr, true /*override*/));
...
// anywhere in memory).
HRESULT CreateSyntheticObjectForV8Object(IDebugHostContext* ctx, V8CachedObject* cached_object, IModelObject** result)
{
    ...
    return CreateSyntheticObjectWithParentAndDataContext(ctx, Extension::Current()->GetObjectDataModel(), cached_object, result);
}

// Creates an IModelObject to represent a field that is not a struct or array.
HRESULT GetModelForBasicField(const uint64_t address, const std::u16string& type_name, WRL::ComPtr<IDebugHostContext>& sp_ctx, IModelObject** result)
{
    ...
    if (!TryUnwrapTaggedMemberType(type_name, &unwrapped_type_name)) {
        WRL::ComPtr<IDebugHostType> type = Extension::Current()->GetTypeFromV8Module(sp_ctx, type_name.c_str());
        if (type == nullptr)
            return E_FAIL;
        return sp_data_model_manager->CreateTypedObject(sp_ctx.Get(), Location { address }, type.Get(), result);
    }
    ...
    WRL::ComPtr<V8CachedObject> cached_object
        = WRL::Make<V8CachedObject>(Location(address), ConvertFromU16String(unwrapped_type_name), sp_ctx, COMPRESS_POINTERS_BOOL);
    return CreateSyntheticObjectForV8Object(sp_ctx.Get(), cached_object.Get(), result);
}

// Creates an IModelObject representing the value of a bitfield.
HRESULT GetModelForBitField(uint64_t address, const uint8_t num_bits, uint8_t shift_bits, const std::u16string& type_name,
    WRL::ComPtr<IDebugHostContext>& sp_ctx, IModelObject** result)
{
    ...
    if (bytes_to_read > sizeof(value)) {
        std::stringstream message;
        message << "Fatal v8windbg error: found bitfield struct of " << bytes_to_read << "bytes, which exceeds the supported size of " << sizeof(value);
        return CreateString(ConvertToU16String(message.str()), result);
    }

    uint64_t bytes_read;
    HRESULT hr = sp_debug_host_memory->ReadBytes(sp_ctx.Get(), address, reinterpret_cast<void*>(&value), bytes_to_read, &bytes_read);
...
HRESULT GetModelForStruct(const uint64_t address, const std::vector<StructField>& fields, WRL::ComPtr<IDebugHostContext>& sp_ctx, IModelObject** result)
{
    WRL::ComPtr<IModelObject> sp_value;
    RETURN_IF_FAIL(sp_data_model_manager->CreateSyntheticObject(sp_ctx.Get(), &sp_value));
    ...
    for (const StructField& field : fields) {
        ...
        if (field.num_bits == 0) {
            if (FAILED(GetModelForBasicField(address + field.offset, field.type_name, sp_ctx, &field_model))) {
                continue;
            }
        } else {
            if (FAILED(GetModelForBitField(address + field.offset, field.num_bits, field.shift_bits, field.type_name, sp_ctx, &field_model))) {
                continue;
            }
        }
        RETURN_IF_FAIL(sp_value->SetKey(reinterpret_cast<const wchar_t*>(field.name.c_str()), field_model.Get(), nullptr));
    }
...
HRESULT GetModelForNativeArray(
    const uint64_t address, const std::u16string& type_name, size_t count, WRL::ComPtr<IDebugHostContext>& sp_ctx, IModelObject** result)
{
    WRL::ComPtr<IDebugHostType> type = Extension::Current()->GetTypeFromV8Module(sp_ctx, type_name.c_str());
    if (type == nullptr)
        return E_FAIL;
    ...
    ArrayDimension dimensions[] = { { /*start=*/0, /*length=*/count, /*stride=*/object_size } };
    WRL::ComPtr<IDebugHostType> array_type;
    RETURN_IF_FAIL(type->CreateArrayOf(/*dimensions=*/1, dimensions, &array_type));

    return sp_data_model_manager->CreateTypedObject(sp_ctx.Get(), Location { address }, array_type.Get(), result);
}

// Creates an IModelObject that represents an array of structs or compressed
// tagged values.
HRESULT GetModelForCustomArray(const Property& prop, WRL::ComPtr<IDebugHostContext>& sp_ctx, IModelObject** result)
{
    ...
    WRL::ComPtr<IndexedFieldData> context_data = WRL::Make<IndexedFieldData>(prop);

    return CreateSyntheticObjectWithParentAndDataContext(sp_ctx.Get(), Extension::Current()->GetIndexedFieldDataModel(), context_data.Get(), result);
}
...
HRESULT GetModelForCustomArrayElement(IModelObject* context_object, size_t index, IModelObject** object)
{
    // Open a few layers of wrapper objects to get to the Property object that
    // describes the array.
    WRL::ComPtr<IUnknown> data_model_context;
    RETURN_IF_FAIL(context_object->GetContextForDataModel(Extension::Current()->GetIndexedFieldDataModel(), &data_model_context));
...
IFACEMETHODIMP IndexedFieldParent::GetDimensionality(IModelObject* context_object, ULONG64* dimensionality) noexcept
{
    *dimensionality = 1;
    return S_OK;
}

IFACEMETHODIMP IndexedFieldParent::GetAt(
    IModelObject* context_object, ULONG64 indexer_count, IModelObject** indexers, IModelObject** object, IKeyStore** metadata) noexcept
{
    if (indexer_count != 1)
        return E_INVALIDARG;
    if (metadata != nullptr)
...
IFACEMETHODIMP IndexedFieldParent::SetAt(IModelObject* context_object, ULONG64 indexer_count, IModelObject** indexers, IModelObject* value) noexcept
{
    return E_NOTIMPL;
}

IFACEMETHODIMP IndexedFieldParent::GetDefaultIndexDimensionality(IModelObject* context_object, ULONG64* dimensionality) noexcept
{
    *dimensionality = 1;
    return S_OK;
}

IFACEMETHODIMP IndexedFieldParent::GetIterator(IModelObject* context_object, IModelIterator** iterator) noexcept
{
...
IFACEMETHODIMP IndexedFieldIterator::GetNext(IModelObject** object, ULONG64 dimensions, IModelObject** indexers, IKeyStore** metadata) noexcept
{
    if (dimensions > 1)
        return E_INVALIDARG;

    WRL::ComPtr<IModelObject> sp_index, sp_value;
    RETURN_IF_FAIL(GetModelForCustomArrayElement(context_object_.Get(), next_, &sp_value));
...
IFACEMETHODIMP V8ObjectDataModel::GetKey(IModelObject* context_object, PCWSTR key, IModelObject** key_value, IKeyStore** metadata, bool* has_key) noexcept
{
    if (metadata != nullptr)
        *metadata = nullptr;
...
IFACEMETHODIMP V8ObjectDataModel::SetKey(IModelObject* context_object, PCWSTR key, IModelObject* key_value, IKeyStore* metadata) noexcept
{
    return E_NOTIMPL;
}

IFACEMETHODIMP V8ObjectDataModel::EnumerateKeys(IModelObject* context_object, IKeyEnumerator** pp_enumerator) noexcept
{
    WRL::ComPtr<IV8CachedObject> sp_v8_cached_object;
    RETURN_IF_FAIL(GetCachedObject(context_object, &sp_v8_cached_object));
...
IFACEMETHODIMP V8LocalValueProperty::GetValue(PCWSTR pwsz_key, IModelObject* p_v8_local_instance, IModelObject** pp_value) noexcept
{
    ...
    ULONG64 obj_address;
    RETURN_IF_FAIL(sp_debug_host_memory->ReadPointers(sp_ctx.Get(), loc, 1, &obj_address));
    ...
    if (obj_address == 0) {
        ...
        std::string narrow_tagged_name = std::string("v8::internal::Tagged<") + static_cast<const char*>(generic_name) + ">";
        std::u16string tagged_type_name = ConvertToU16String(narrow_tagged_name);
        WRL::ComPtr<IDebugHostType> tagged_type = Extension::Current()->GetTypeFromV8Module(sp_ctx, tagged_type_name.c_str());
        ...
        // Create the result.
        RETURN_IF_FAIL(sp_data_model_manager->CreateTypedObject(sp_ctx.Get(), obj_address, tagged_type.Get(), pp_value));
    }

    return S_OK;
}

IFACEMETHODIMP V8LocalValueProperty::SetValue(PCWSTR /*pwsz_key*/, IModelObject* /*p_process_instance*/, IModelObject* /*p_value*/) noexcept
{
    return E_NOTIMPL;
}

IFACEMETHODIMP V8InternalCompilerNodeIdProperty::GetValue(PCWSTR pwsz_key, IModelObject* p_v8_compiler_node_instance, IModelObject** pp_value) noexcept
{
    WRL::ComPtr<IModelObject> sp_bit_field;
    RETURN_IF_FAIL(p_v8_compiler_node_instance->GetRawValue(SymbolKind::SymbolField, L"bit_field_", RawSearchNone, &sp_bit_field));

    uint64_t bit_field_value;
    RETURN_IF_FAIL(UnboxULong64(sp_bit_field.Get(), &bit_field_value, true /*convert*/));
    ...
    RETURN_IF_FAIL(Extension::Current()->GetV8Module(sp_host_context)->FindTypeByName(L"v8::internal::compiler::Node::IdField", &sp_id_field_type));
...
IFACEMETHODIMP V8InternalCompilerNodeIdProperty::SetValue(PCWSTR /*pwsz_key*/, IModelObject* /*p_process_instance*/, IModelObject* /*p_value*/) noexcept
{
    return E_NOTIMPL;
}

IFACEMETHODIMP V8InternalCompilerBitsetNameProperty::GetValue(PCWSTR pwsz_key, IModelObject* p_v8_compiler_type_instance, IModelObject** pp_value) noexcept
{
    WRL::ComPtr<IModelObject> sp_payload;
    RETURN_IF_FAIL(p_v8_compiler_type_instance->GetRawValue(SymbolKind::SymbolField, L"payload_", RawSearchNone, &sp_payload));

    uint64_t payload_value;
    RETURN_IF_FAIL(UnboxULong64(sp_payload.Get(), &payload_value, true /*convert*/));
...
IFACEMETHODIMP V8InternalCompilerBitsetNameProperty::SetValue(PCWSTR /*pwsz_key*/, IModelObject* /*p_process_instance*/, IModelObject* /*p_value*/) noexcept
{
    return E_NOTIMPL;
}

constexpr wchar_t usage[] = LR"(Invalid arguments.
...
IFACEMETHODIMP InspectV8ObjectMethod::Call(IModelObject* p_context_object, ULONG64 arg_count, _In_reads_(arg_count) IModelObject** pp_arguments,
    IModelObject** pp_result, IKeyStore** pp_metadata) noexcept
{
    // Read the arguments.
    ULONG64 tagged_value;
    _bstr_t type_name;
    if (arg_count < 1 || FAILED(UnboxULong64(pp_arguments[0], &tagged_value, /*convert=*/true))
    ...
    WRL::ComPtr<V8CachedObject> cached_object = WRL::Make<V8CachedObject>(::GetHeapObject(sp_ctx, tagged_value, 0, static_cast<const char*>(type_name),
        /*is_compressed=*/false));
    return CreateSyntheticObjectForV8Object(sp_ctx.Get(), cached_object.Get(), pp_result);
}

// Creates an IModelObject representing the data in the given property.
HRESULT GetModelForProperty(const Property& prop, WRL::ComPtr<IDebugHostContext>& sp_ctx, IModelObject** result)
{
    switch (prop.type) {
    case PropertyType::kPointer:
        return GetModelForBasicField(prop.addr_value, prop.type_name, sp_ctx, result);
    ...
    case PropertyType::kStructArray:
        ...
        if (prop.type == PropertyType::kArray && !TryUnwrapTaggedMemberType(prop.type_name, nullptr)) {
            // An array of things that are not structs or compressed tagged values
            // is most cleanly represented by a native array.
            return GetModelForNativeArray(prop.addr_value, prop.type_name, prop.length, sp_ctx, result);
        }
