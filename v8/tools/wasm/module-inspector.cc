...
public:
    ...
    void RecordCodeSize(size_t chunk)
    {
        total_code_size_ += chunk;
    }
    ...
    void WriteTo(std::ostream& out)
    {
        ...
        std::sort(sorted.begin(), sorted.end(), [](const Entry& a, const Entry& b) { return a.count > b.count; });
        ...
        longest_mnemo = std::max(longest_mnemo, static_cast<int>(strlen("Instruction"))) + kSpacing;
        ...
        auto PrintLine = [&](const char* name, uint32_t count, uint32_t total_size) {
            out << std::setw(longest_mnemo) << std::left << name;
            out << std::setw(count_digits) << std::right << count;
            out << std::setw(kSpacing) << " ";
            out << std::setw(8) << total_size;
            out << std::setw(kSpacing) << " ";
            out << std::fixed << std::setprecision(2) << std::setw(8) << static_cast<double>(total_size) / count;
            out << std::setw(kSpacing) << " ";
            out << std::fixed << std::setprecision(1) << std::setw(8) << 100.0 * total_size / total_code_size_ << "%\n";
        };
        for (const Entry& e : sorted) {
        ...
        for (const auto& imm : immediates) {
            ...
            std::sort(counts.begin(), counts.end(), [](const std::pair<int, uint32_t>& a, const std::pair<int, uint32_t>& b) { return a.second > b.second; });
            constexpr int kImmLen = 9; // Length of "Immediate".
            int count_len = std::max(GetNumDigits(counts[0].second), static_cast<int>(strlen("count")));
...
public:
    ExtendedFunctionDis(Zone* zone, const WasmModule* module, uint32_t func_index, bool shared, WasmDetectedFeatures* detected, const FunctionSig* sig,
        const uint8_t* start, const uint8_t* end, uint32_t offset, const ModuleWireBytes wire_bytes, NamesProvider* names)
        : FunctionBodyDisassembler(zone, module, func_index, shared, detected, sig, start, end, offset, wire_bytes, names)
    {
    }
    ...
    void HexDump(MultiLineStringBuilder& out, FunctionHeader include_header)
    {
        ...
        if (include_header == kPrintHeader) {
            ...
            PrintSignatureOneLine(out, sig_, func_index_, names_, true, NamesProvider::kIndexAsComment);
        ...
        while (entries-- > 0) {
            ...
            auto [type, type_length] = value_type_reader::read_value_type<ValidationTag>(this, pc_ + count_length, WasmEnabledFeatures::All());
            PrintHexBytes(out, count_length + type_length, pc_, 4);
            out << " // " << count << (count != 1 ? " locals" : " local") << " of type ";
        ...
        while (pc_ < end_ && ok()) {
            ...
            if (opcode == kExprBlock || opcode == kExprIf || opcode == kExprLoop || opcode == kExprTry) {
                label_stack_.emplace_back(out.line_number(), out.length(), label_occurrence_index_++);
            }
            pc_ += length;
            out.NextLine(pc_offset());
        }
    ...
    void PrintHexBytes(StringBuilder& out, uint32_t num_bytes, const uint8_t* start, uint32_t fill_to_minimum = 0)
    {
...
public:
    HexDumpModuleDis(
        MultiLineStringBuilder& out, const WasmModule* module, NamesProvider* names, const ModuleWireBytes wire_bytes, AccountingAllocator* allocator)
        : out_(out)
        , module_(module)
        , names_(names)
    ...
    void PrintModule()
    {
        ...
        if (!names_) {
            ...
            names_provider.reset(new NamesProvider(fake_module.get(), wire_bytes_.module_bytes()));
        ...
        if (total_bytes_ != wire_bytes_.length()) {
            std::cerr << "WARNING: OUTPUT INCOMPLETE. Disassembled " << total_bytes_ << " out of " << wire_bytes_.length() << " bytes.\n";
        }
    ...
    void FunctionName(uint32_t func_index) override
    {
        ...
        names_->PrintFunctionName(description_, func_index, NamesProvider::kDevTools);
    }

    void NextLineIfFull() override
    {
        if (queue_ || line_bytes_ >= kPadBytes)
            NextLine();
    }
    void NextLineIfNonEmpty() override
    {
        if (queue_ || line_bytes_ > 0)
            NextLine();
    }
    ...
    void InitializerExpression(const uint8_t* start, const uint8_t* end, ValueType expected_type) override
    {
        ...
        if (!module)
            module = decoder_->shared_module().get();
        ExtendedFunctionDis d(&zone_, module, 0, false, &detected, &sig, start, end, offset, wire_bytes_, names_);
    ...
    void FunctionBody(const WasmFunction* func, const uint8_t* start) override
    {
        ...
        ExtendedFunctionDis d(&zone_, module, func->func_index, shared, &detected, func->sig, start, end, offset, wire_bytes_, names_);
    ...
    void NameSection(const uint8_t* start, const uint8_t* end, uint32_t offset) override
    {
        ...
        while (decoder.ok() && decoder.more()) {
            ...
            NextLine();
            if (!decoder.checkAvailable(payload_length))
                break;
            switch (name_type) {
            case kModuleCode:
                consume_string(&decoder, unibrow::Utf8Variant::kLossyUtf8, "module name", this);
                break;
            case kFunctionCode:
            case kTypeCode:
            case kTableCode:
...
private:
    ...
    void PrintHexBytes(StringBuilder& out, uint32_t num_bytes, const uint8_t* start)
    {
    ...
    uint32_t pc_offset()
    {
        return static_cast<uint32_t>(total_bytes_);
    }
...
public:
    ...
    void WriteTo(std::ostream& out)
    {
        ...
        for (size_t i = 0; i < buckets_.size(); ++i) {
            ...
            out << "  (" << std::fixed << std::setw(4) << std::setprecision(1) << percent << "%)\n";
        }
        out << "Total function count: " << fct_count << '\n';
        out << "Average size per function: " << total_bytes_ / fct_count << " bytes\n";
    }
...
public:
    ...
    explicit FormatConverter(const char* input, const char* output, bool print_offsets)
        : output_(output)
        , out_(output_.get())
        , print_offsets_(print_offsets)
    {
        if (!output_.ok())
            return;
        if (!LoadFile(input))
        ...
        ModuleResult result = DecodeWasmModuleForDisassembler(raw_bytes(), offsets_provider_.get());
        if (result.failed()) {
            WasmError error = result.error();
            std::cerr << "Decoding error: " << error.message() << " at offset " << error.offset() << "\n";
            return;
        }
        status_ = kModuleReady;
        module_ = result.value();
        names_provider_ = std::make_unique<NamesProvider>(module_.get(), raw_bytes());
    }
    ...
    void ListFunctions()
    {
        ...
        double small_function_percentage = module_->num_small_functions * 100.0 / module_->num_declared_functions;
        out_ << "There are " << num_functions << " functions (" << m->num_imported_functions << " imported, " << m->num_declared_functions
             << " locally defined; " << small_function_percentage << "% of them \"small\"); the following have names:\n";
    ...
    static bool sig_uses_vector_comparison(std::pair<uint32_t, uint32_t> left, std::pair<uint32_t, uint32_t> right)
    {
        return left.second > right.second;
    }

    void SortAndPrintSigUses(std::map<uint32_t, uint32_t> uses, const WasmModule* module, const char* kind)
    {
        std::vector<std::pair<uint32_t, uint32_t>> sig_uses_vector { uses.begin(), uses.end() };
        std::sort(sig_uses_vector.begin(), sig_uses_vector.end(), sig_uses_vector_comparison);

        out_ << sig_uses_vector.size() << " different signatures get used by " << kind << std::endl;
    ...
    void SectionStats()
    {
        ...
        for (WasmSectionIterator it(&decoder, ITracer::NoTrace); it.more(); it.advance(true)) {
            const char* name = SectionName(it.section_code());
            size_t name_len = strlen(name);
            out_ << SectionName(it.section_code()) << " section: ";
            for (; name_len < kMinNameLength; name_len++)
                out_ << " ";

            uint32_t length = it.section_length();
            out_ << std::setw(name_len > kMinNameLength ? 0 : digits) << length << " bytes / ";

            out_ << std::fixed << std::setprecision(1) << std::setw(4) << 100.0 * length / module_size;
            out_ << "% of total\n";
        }
    }
    ...
    void Strip()
    {
        ...
        for (WasmSectionIterator it(&decoder, ITracer::NoTrace); it.more(); it.advance(true)) {
            if (it.section_code() == kNameSectionCode)
                continue;
            out_.write(reinterpret_cast<const char*>(it.section_start()), it.section_length());
        }
    }
    ...
    void InstructionStats()
    {
        ...
        for (uint32_t i = module()->num_imported_functions; i < module()->functions.size(); i++) {
            const WasmFunction* func = &module()->functions[i];
            bool shared = module()->type(func->sig_index).is_shared;
            WasmDetectedFeatures detected;
            base::Vector<const uint8_t> code = wire_bytes_.GetFunctionBytes(func);
            ExtendedFunctionDis d(&zone, module(), i, shared, &detected, func->sig, code.begin(), code.end(), func->code.offset(), wire_bytes_, names());
    ...
    void DisassembleFunction(uint32_t func_index, OutputMode mode)
    {
        ...
        ExtendedFunctionDis d(&zone, module(), func_index, shared, &detected, func->sig, code.begin(), code.end(), func->code.offset(), wire_bytes_, names());
        ...
        ModuleDisassembler md(sb, module(), names(), wire_bytes_, &allocator_, std::move(offsets_provider_));
        for (uint32_t type_index : d.used_types()) {
            md.PrintTypeDefinition(type_index, { 0, 1 }, NamesProvider::kIndexAsComment);
        }
    ...
    void HexdumpForModule()
    {
        ...
        DCHECK_IMPLIES(status_ == kIoInitialized, module() == nullptr && names() == nullptr);
        MultiLineStringBuilder sb;
...
private:
    ...
    {
        if (path == "-")
            return LoadFileFromStream(std::cin);
    ...
    bool LoadFileFromStream(std::istream& input)
    {
        ...
        if (c0 == 0 && c1 == 'a' && c2 == 's' && c3 == 'm') {
            // Wasm binary module.
            raw_bytes_ = std::vector<uint8_t>(std::istreambuf_iterator<char>(input), {});
            return true;
        }
        if (TryParseLiteral(input, raw_bytes_))
            return true;
    ...
    bool TryParseLiteral(std::istream& input, std::vector<uint8_t>& output_bytes)
    {
        int c = input.get();
...
DumpingModuleDecoder::DumpingModuleDecoder(ModuleWireBytes wire_bytes, HexDumpModuleDis* module_dis)
    : ModuleDecoderImpl(WasmEnabledFeatures::All(), wire_bytes.module_bytes(), kWasmOrigin, &unused_detected_features_, module_dis)
{
}
...
bool ParseInt(char* s, int* out)
{
    ...
    if (errno != 0 || *end != '\0' || l > std::numeric_limits<int>::max() || l < std::numeric_limits<int>::min()) {
        return false;
    }
...
int ParseOptions(int argc, char** argv, Options* options)
{
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--help") == 0 || strcmp(argv[i], "-h") == 0 || strcmp(argv[i], "help") == 0) {
            ...
            options->action = Action::kSingleWat;
            if (!ParseInt(argv[i] + 13, &options->func_index))
                return PrintHelp(argv);
            ...
            if (!ParseInt(argv[i] + 17, &options->func_index))
                return PrintHelp(argv);
...
int main(int argc, char** argv)
{
    ...
    if (fc.status() == FormatConverter::kNotReady)
        return 1;
    // Allow hex dumping invalid modules.
    if (fc.status() != FormatConverter::kModuleReady && options.action != Action::kFullHexdump) {
