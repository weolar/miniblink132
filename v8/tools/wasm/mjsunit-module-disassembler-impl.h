...
class MaybeLebScope {
public:
    MaybeLebScope(StringBuilder& out, uint32_t index)
        : out(out)
        , index(index)
    {
        if (index > 0x7F) {
...
public:
        ...
        , wire_bytes_(wire_bytes)
    {
        ...
        for (uint32_t i = 0; i < module->functions.size(); i++) {
            WireBytesRef name = module_->lazily_generated_names.LookupFunctionName(wire_bytes, i);
        ...
        for (const WasmImport& imp : module_->import_table) {
            if (imp.kind != kExternalFunction)
                continue;
            if (function_variable_names_[imp.index].is_set())
                continue;
        ...
        for (const WasmExport& ex : module_->export_table) {
            if (ex.kind != kExternalFunction)
                continue;
            if (function_variable_names_[ex.index].is_set())
                continue;
    ...
    {
        WireBytesRef ref = module_->lazily_generated_names.LookupFunctionName(wire_bytes_, function_index);
        return ref.is_set();
    }

    bool FunctionNameEquals(uint32_t function_index, WireBytesRef ref)
    {
        WireBytesRef name_ref = module_->lazily_generated_names.LookupFunctionName(wire_bytes_, function_index);
        if (name_ref.length() != ref.length())
            return false;
        if (name_ref.offset() == ref.offset())
            return true;
    ...
    void PrintStructType(StringBuilder& out, ModuleTypeIndex index, OutputContext mode)
    {
        DCHECK(module_->has_struct(index));
        PrintMaybeLEB(out, "$struct", index, mode);
    }

    void PrintArrayType(StringBuilder& out, ModuleTypeIndex index, OutputContext mode)
    {
        DCHECK(module_->has_array(index));
        PrintMaybeLEB(out, "$array", index, mode);
    }

    void PrintSigType(StringBuilder& out, ModuleTypeIndex index, OutputContext mode)
    {
        DCHECK(module_->has_signature(index));
        PrintMaybeLEB(out, "$sig", index, mode);
    }

    void PrintTypeIndex(StringBuilder& out, ModuleTypeIndex index, OutputContext mode)
    {
        if (module_->has_struct(index)) {
    ...
    {
        WireBytesRef ref = module_->lazily_generated_names.LookupFunctionName(wire_bytes_, index);
    ...
    void PrintFunctionReference(StringBuilder& out, uint32_t index)
    {
        ...
        if (index < module_->functions.size() && !module_->functions[index].imported) {
            out << ".index";
        }
    }
...
#undef CASE
            ...
            case HeapType::kTop:
                UNREACHABLE();
            default:
                out << (mode == kEmitObjects ? "wasmRefNullType(" : "kWasmRefNull, ");
                break;
            }
            break;
        case kRef:
...
private:
    bool IsSuitableFunctionVariableName(WireBytesRef ref)
    {
        ...
        if (name.length() >= 8) {
            if (memcmp(name.begin(), "$segment", 8) == 0)
                return false;
        }
        if (name.length() >= 7) {
            if (memcmp(name.begin(), "$global", 7) == 0)
                return false;
            if (memcmp(name.begin(), "$struct", 7) == 0)
                return false;
        }
        if (name.length() >= 6) {
            if (memcmp(name.begin(), "$array", 6) == 0)
                return false;
            if (memcmp(name.begin(), "$table", 6) == 0)
                return false;
        }
        if (name.length() >= 5) {
            if (memcmp(name.begin(), "$data", 5) == 0)
                return false;
            if (memcmp(name.begin(), "$func", 5) == 0)
                return false;
            if (memcmp(name.begin(), "kExpr", 5) == 0)
                return false;
            if (memcmp(name.begin(), "kSig_", 5) == 0)
                return false;
            if (memcmp(name.begin(), "kWasm", 5) == 0)
                return false;
        }
        if (name.length() >= 4) {
            if (memcmp(name.begin(), "$mem", 4) == 0)
                return false;
            if (memcmp(name.begin(), "$sig", 4) == 0)
                return false;
            if (memcmp(name.begin(), "$tag", 4) == 0)
                return false;
        }
        return true;
    }

    void PrintMaybeLEB(StringBuilder& out, const char* prefix, ModuleTypeIndex index, OutputContext mode)
    {
...
template <typename ValidationTag> class MjsunitImmediatesPrinter;
class MjsunitFunctionDis : public WasmDecoder<Decoder::FullValidationTag> {
public:
    using ValidationTag = Decoder::FullValidationTag;

    MjsunitFunctionDis(Zone* zone, const WasmModule* module, uint32_t func_index, bool shared, WasmDetectedFeatures* detected, const FunctionSig* sig,
        const uint8_t* start, const uint8_t* end, uint32_t offset, MjsunitNamesProvider* mjsunit_names, Indentation indentation)
        : WasmDecoder<ValidationTag>(zone, module, WasmEnabledFeatures::All(), detected, sig, shared, start, end, offset)
...
private:
    template <typename ValidationTag> friend class MjsunitImmediatesPrinter;
...
void MjsunitFunctionDis::WriteMjsunit(MultiLineStringBuilder& out)
{
    ...
    if (num_locals_ > num_params) {
        for (uint32_t pos = num_params, count = 1; pos < num_locals_; pos += count, count = 1) {
...
void PrintF32Const(StringBuilder& out, ImmF32Immediate& imm)
{
    ...
    char buffer[100];
    const char* str = DoubleToCString(imm.value, base::VectorOf(buffer, sizeof(buffer)));
...
void MjsunitFunctionDis::DecodeGlobalInitializer(StringBuilder& out)
{
    ...
    while (pc_ < end_ && ok()) {
        ...
        if (WasmOpcodes::IsPrefixOpcode(opcode)) {
            ...
            if (opcode != kExprI32Const && opcode != kExprI64Const && opcode != kExprF32Const && opcode != kExprF64Const) {
                out << RawOpcodeName(opcode) << ",";
            }
        }
        current_opcode_ = opcode;
...
public:
    ...
    {
        if (owner_->current_opcode_ == kExprBrOnCast || owner_->current_opcode_ == kExprBrOnCastFail) {
        ...
        out_ << " ";
        names()->PrintValueType(out_, ValueType::RefMaybeNull(imm.type, is_nullable ? kNullable : kNonNullable), kEmitWireBytes);
        out_ << ",";
    }
    ...
    void BrOnCastFlags(BrOnCastImmediate& flags)
    {
        ...
        out_ << " /* " << (flags.flags.src_is_null ? "" : "non-") << "nullable -> " << (flags.flags.res_is_null ? "" : "non-") << "nullable */,";
    }

    void BranchDepth(BranchDepthImmediate& imm)
    {
        WriteUnsignedLEB(imm.depth);
    }
    ...
    void MemoryAccess(MemoryAccessImmediate& imm)
    {
        ...
        if (imm.mem_index < owner_->module_->memories.size() && owner_->module_->memories[imm.mem_index].is_memory64()) {
...
uint32_t MjsunitFunctionDis::PrintMjsunitImmediatesAndGetLength(StringBuilder& out)
{
...
public:
    MjsunitModuleDis(MultiLineStringBuilder& out, const WasmModule* module, NamesProvider* names, const ModuleWireBytes wire_bytes,
    ...
    void PrintModule()
    {
        ...
        auto MarkAsNeededHere = [&needed_at](ValueType vt, uint32_t here) {
            if (!vt.is_object_reference())
                return;
            HeapType ht = vt.heap_type();
            if (!ht.is_index())
                return;
            if (ht.ref_index().index < here)
                return;
            if (needed_at[ht.ref_index().index] < here)
                return;
        ...
        for (uint32_t i = 0; i < module_->types.size(); i++) {
            ...
            uint32_t end_index = recgroup.end_type_index != OffsetsProvider::RecGroup::kInvalid ? recgroup.end_type_index : i + 1;
        ...
        for (const WasmImport& imported : module_->import_table) {
            ...
            case kExternalFunction:
                ...
                names()->PrintTypeIndex(out_, module_->functions[imported.index].sig_index, kEmitObjects);
                break;
            ...
            case kExternalTag: {
                ...
                out_ << "', '" << V(imported.field_name) << "', ";
                names()->PrintTypeIndex(out_, module_->tags[imported.index].sig_index, kEmitObjects);
                break;
            }
            }
            out_ << ");";
        ...
        if constexpr (kMaintainExportOrder) {
            for (const WasmExport& ex : module_->export_table) {
                if (ex.kind != kExternalFunction || module_->functions[ex.index].imported) {
        ...
        for (const WasmFunction& func : module_->functions) {
            ...
            if (func.exported && !export_functions_late) {
                for (const WasmExport& ex : module_->export_table) {
                    if (ex.kind != kExternalFunction || ex.index != index)
                        continue;
        ...
        for (uint32_t i = 0; i < module_->data_segments.size(); i++) {
            ...
            base::Vector<const uint8_t> data = wire_bytes_.module_bytes().SubVector(segment.source.offset(), segment.source.end_offset());
            out_ << "let ";
        ...
        for (uint32_t i = 0; i < module_->elem_segments.size(); i++) {
            ...
            for (uint32_t j = 0; j < segment.element_count; j++) {
                if (j > 0)
                    out_ << ", ";
                ConstantExpression expr = decoder.consume_element_segment_entry(const_cast<WasmModule*>(module_), segment);
        ...
        for (const WasmFunction& func : module_->functions) {
            ...
            base::Vector<const uint8_t> func_code = wire_bytes_.GetFunctionBytes(&func);

            // Locals and body.
            bool shared = module_->type(func.sig_index).is_shared;
            WasmDetectedFeatures detected;
            MjsunitFunctionDis d(&zone_, module_, index, shared, &detected, func.sig, func_code.begin(), func_code.end(), func.code.offset(), &mjsunit_names_,
                Indentation { 2, 2 });
            d.WriteMjsunit(out_);
            if (d.failed())
                has_error_ = true;
        ...
        for (const WasmExport& ex : module_->export_table) {
            switch (ex.kind) {
            case kExternalFunction:
                if (!export_functions_late && !module_->functions[ex.index].imported) {
            ...
            case kExternalMemory:
                ...
                break;
            case kExternalGlobal:
                if (!kMaintainExportOrder && ex.index >= module_->num_imported_globals) {
                    continue;
                }
                ...
                break;
            case kExternalTable:
                if (!kMaintainExportOrder && ex.index >= module_->num_imported_tables) {
                    continue;
                }
                out_ << "builder.addExportOfKind('";
                PrintName(ex.name);
...
private:
    ...
    {
        return { reinterpret_cast<const char*>(wire_bytes_.start()) + ref.offset(), ref.length() };
    }
    ...
    void DecodeAndAppendInitExpr(ConstantExpression init, ValueType expected)
    {
        ...
        case ConstantExpression::kWireBytesRef: {
            ...
            WasmDetectedFeatures detected;
            MjsunitFunctionDis d(&zone_, module_, 0, false, &detected, &sig, start, end, ref.offset(), &mjsunit_names_, Indentation { 0, 0 });
