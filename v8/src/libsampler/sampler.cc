...
zx_status_t zx_thread_read_state(zx_handle_t h, uint32_t k, void* b, size_t l)
{
    ...
    return zx_thread_read_state(h, k, b, static_cast<uint32_t>(l), &dummy_out_len);
}
...
public:
    PlatformData()
        : vm_tid_(base::OS::GetCurrentThreadId())
        , vm_tself_(pthread_self())
    {
    }
    int vm_tid() const
    {
...
public:
    ...
    PlatformData()
    {
        ...
        BOOL result = DuplicateHandle(current_process, GetCurrentThread(), current_process, &profiled_thread_,
...
public:
    PlatformData()
    {
        zx_handle_duplicate(zx_thread_self(), ZX_RIGHT_SAME_RIGHTS, &profiled_thread_);
    }
    ~PlatformData()
    {
...
public:
    ...
    static v8::base::RecursiveMutex* mutex()
    {
        return mutex_.Pointer();
    }
...
base::LazyRecursiveMutex SignalHandler::mutex_ = LAZY_RECURSIVE_MUTEX_INITIALIZER;
...
#if !(V8_OS_OPENBSD || V8_OS_ZOS || (V8_OS_LINUX && (V8_HOST_ARCH_S390X || V8_HOST_ARCH_PPC64)))
...
Sampler::Sampler(Isolate* isolate)
    : isolate_(isolate)
    , data_(std::make_unique<PlatformData>())
{
}
...
void Sampler::DoSample()
{
    ...
    zx_status_t suspended = zx_object_wait_one(profiled_thread, ZX_THREAD_SUSPENDED | ZX_THREAD_TERMINATED, zx_deadline_after(ZX_MSEC(100)), &signals);
    ...
    if (zx_thread_read_state(profiled_thread, ZX_THREAD_STATE_GENERAL_REGS, &thread_state, sizeof(thread_state)) == ZX_OK) {
        v8::RegisterState state;
#if V8_HOST_ARCH_X64
