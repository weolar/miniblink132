...
template <> struct MaxDecimalDigitsIn<1> {
...
public:
    ...
    void AddSubstring(const char* s, int n)
    {
        ...
        while (s < s_end) {
            int s_chunk_size = std::min(chunk_size_ - chunk_pos_, static_cast<int>(s_end - s));
...
private:
    template <typename T> void AddNumberImpl(T n, const char* format)
    {
        ...
        if (chunk_size_ - chunk_pos_ >= kMaxNumberSize) {
            int result = SNPrintF(chunk_.SubVector(chunk_pos_, chunk_size_), format, n);
            DCHECK_NE(result, -1);
            chunk_pos_ += result;
            MaybeWriteChunk();
        } else {
    ...
    void WriteChunk()
    {
        if (aborted_)
            return;
        if (stream_->WriteAsciiChunk(chunk_.begin(), chunk_pos_) == v8::OutputStream::kAbort)
            aborted_ = true;
        chunk_pos_ = 0;
    }
