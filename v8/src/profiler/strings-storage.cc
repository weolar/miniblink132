...
StringsStorage::~StringsStorage()
{
    for (base::HashMap::Entry* p = names_.Start(); p != nullptr; p = names_.Next(p)) {
...
const char* StringsStorage::GetCopy(const char* src)
{
    ...
    entry->value = reinterpret_cast<void*>(reinterpret_cast<size_t>(entry->value) + 1);
...
const char* StringsStorage::GetSymbol(Tagged<Symbol> sym)
{
    ...
    uint32_t length = std::min(v8_flags.heap_snapshot_string_limit.value(), description->length());
...
const char* StringsStorage::GetName(Tagged<Name> name)
{
    if (IsString(name)) {
        ...
        uint32_t length = std::min(v8_flags.heap_snapshot_string_limit.value(), str->length());
...
const char* StringsStorage::GetConsName(const char* prefix, Tagged<Name> name)
{
    if (IsString(name)) {
        ...
        uint32_t cons_length = actual_length + static_cast<uint32_t>(strlen(prefix)) + 1;
