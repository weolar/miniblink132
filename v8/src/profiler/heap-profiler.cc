...
{
    if (snapshots_.empty() && !sampling_heap_profiler_ && !allocation_tracker_ && !is_taking_snapshot_) {
...
{
    snapshots_.erase(std::find_if(snapshots_.begin(), snapshots_.end(), [&](const std::unique_ptr<HeapSnapshot>& entry) { return entry.get() == snapshot; }));
}
...
std::vector<v8::Local<v8::Value>> HeapProfiler::GetDetachedJSWrapperObjects()
{
    ...
    for (Tagged<HeapObject> obj = iterator.Next(); !obj.is_null(); obj = iterator.Next()) {
        if (HeapLayout::InCodeSpace(obj))
            continue;
        if (!IsJSApiWrapperObject(obj))
            continue;
        // Ensure object is wrappable, otherwise GetDetachedness() can crash
        JSApiWrapper wrapper = JSApiWrapper(Cast<JSObject>(obj));
        if (!wrapper.GetCppHeapWrappable(isolate(), kAnyCppHeapPointer))
            continue;

        v8::Local<v8::Value> data(Utils::ToLocal(handle(Cast<JSObject>(obj), isolate())));
        v8::EmbedderGraph::Node::Detachedness detachedness = GetDetachedness(data, 0);
...
void HeapProfiler::AddBuildEmbedderGraphCallback(v8::HeapProfiler::BuildEmbedderGraphCallback callback, void* data)
{
    build_embedder_graph_callbacks_.push_back({ callback, data });
}

void HeapProfiler::RemoveBuildEmbedderGraphCallback(v8::HeapProfiler::BuildEmbedderGraphCallback callback, void* data)
{
    auto it = std::find(build_embedder_graph_callbacks_.begin(), build_embedder_graph_callbacks_.end(), std::make_pair(callback, data));
    if (it != build_embedder_graph_callbacks_.end())
        build_embedder_graph_callbacks_.erase(it);
}

void HeapProfiler::BuildEmbedderGraph(Isolate* isolate, v8::EmbedderGraph* graph)
{
...
void HeapProfiler::SetGetDetachednessCallback(v8::HeapProfiler::GetDetachednessCallback callback, void* data)
{
    get_detachedness_callback_ = { callback, data };
}

v8::EmbedderGraph::Node::Detachedness HeapProfiler::GetDetachedness(const v8::Local<v8::Value> v8_value, uint16_t class_id)
{
    DCHECK(HasGetDetachednessCallback());
    return get_detachedness_callback_.first(reinterpret_cast<v8::Isolate*>(heap()->isolate()), v8_value, class_id, get_detachedness_callback_.second);
}
...
HeapSnapshot* HeapProfiler::TakeSnapshot(const v8::HeapProfiler::HeapSnapshotOptions options)
{
    is_taking_snapshot_ = true;
    HeapSnapshot* result = new HeapSnapshot(this, options.snapshot_mode, options.numerics_mode);
    ...
    heap()->stack().SetMarkerIfNeededAndCallback([this, &options, &result]() {
        ...
        HeapSnapshotGenerator generator(result, options.control, options.global_object_name_resolver, heap(), options.stack_state);
...
void HeapProfiler::WriteSnapshotToDiskAfterGC(HeapSnapshotMode snapshot_mode)
{
    ...
    heap()->stack().SetMarkerIfNeededAndCallback([this, snapshot_mode]() {
        ...
        std::unique_ptr<HeapSnapshot> result(new HeapSnapshot(this, snapshot_mode, options.numerics_mode));
        HeapSnapshotGenerator generator(result.get(), options.control, options.global_object_name_resolver, heap(), options.stack_state);
        if (!generator.GenerateSnapshotAfterGC())
            return;
...
void HeapProfiler::TakeSnapshotToFile(const v8::HeapProfiler::HeapSnapshotOptions options, std::string filename)
{
...
bool HeapProfiler::StartSamplingHeapProfiler(uint64_t sample_interval, int stack_depth, v8::HeapProfiler::SamplingFlags flags)
{
    if (sampling_heap_profiler_)
        return false;
    sampling_heap_profiler_.reset(new SamplingHeapProfiler(heap(), names_.get(), sample_interval, stack_depth, flags));
    return true;
}
...
SnapshotObjectId HeapProfiler::PushHeapObjectsStats(OutputStream* stream, int64_t* timestamp_us)
{
...
void HeapProfilerNativeMoveListener::ObjectMoveEvent(Address from, Address to, int size)
{
    profiler_->ObjectMoveEvent(from, to, size, /*is_native_object=*/true);
}

void HeapProfiler::ObjectMoveEvent(Address from, Address to, int size, bool is_native_object)
{
...
{
    CombinedHeapObjectIterator iterator(heap(), HeapObjectIterator::kFilterUnreachable);
...
void HeapProfiler::QueryObjects(DirectHandle<Context> context, v8::QueryObjectPredicate* predicate, std::vector<v8::Global<v8::Object>>* objects)
{
    // We need a stack marker here to allow deterministic passes over the stack.
    ...
    heap()->stack().SetMarkerIfNeededAndCallback([this, predicate, objects]() {
        {
            ...
            CombinedHeapObjectIterator heap_iterator(heap(), HeapObjectIterator::kFilterUnreachable);
            for (Tagged<HeapObject> heap_obj = heap_iterator.Next(); !heap_obj.is_null(); heap_obj = heap_iterator.Next()) {
                if (IsFeedbackVector(heap_obj)) {
                    Cast<FeedbackVector>(heap_obj)->ClearSlots(isolate());
                } else if (IsJSTypedArray(heap_obj) && Cast<JSTypedArray>(heap_obj)->is_on_heap()) {
                    ...
                    on_heap_typed_arrays.push_back(handle(Cast<JSTypedArray>(heap_obj), isolate()));
                }
            }
        ...
        for (Tagged<HeapObject> heap_obj = heap_iterator.Next(); !heap_obj.is_null(); heap_obj = heap_iterator.Next()) {
            if (!IsJSObject(heap_obj, cage_base) || IsJSExternalObject(heap_obj, cage_base))
                continue;
            v8::Local<v8::Object> v8_obj(Utils::ToLocal(handle(Cast<JSObject>(heap_obj), isolate())));
