...
#else
    V8::GetCurrentPlatform()->GetTracingController()->RemoveTraceStateObserver(this);
#endif
}

#if defined(V8_USE_PERFETTO)
void TracingCpuProfilerImpl::OnStart(const perfetto::DataSourceBase::StartArgs&)
{
#else
void TracingCpuProfilerImpl::OnTraceEnabled()
{
    ...
    isolate_->RequestInterrupt([](v8::Isolate*, void* data) { reinterpret_cast<TracingCpuProfilerImpl*>(data)->StartProfiling(); }, this);
}
...
void TracingCpuProfilerImpl::OnTraceDisabled()
{
    ...
    if (!profiling_enabled_)
        return;
    profiling_enabled_ = false;
    isolate_->RequestInterrupt([](v8::Isolate*, void* data) { reinterpret_cast<TracingCpuProfilerImpl*>(data)->StopProfiling(); }, this);
    ...
    V8::GetCurrentPlatform()->GetForegroundTaskRunner(reinterpret_cast<v8::Isolate*>(isolate_))->PostTask(std::make_unique<RunInterruptsTask>(isolate_));
}
...
void TracingCpuProfilerImpl::StartProfiling()
{
    ...
    profiler_->set_sampling_interval(base::TimeDelta::FromMicroseconds(sampling_interval_us));
