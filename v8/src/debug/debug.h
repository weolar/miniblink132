...
public:
    ...
    {
        return BreakLocation(-1, NOT_DEBUG_BREAK);
    }
    ...
    {
        return type_ == DEBUG_BREAK_AT_ENTRY;
    }
...
public:
    ...
    inline int statement_position() const
    {
...
public:
    ...
    void OnDebugBreak(Handle<FixedArray> break_points_hit, StepAction stepAction, debug::BreakReasons break_reasons = {});
    debug::DebugDelegate::ActionAfterInstrumentation OnInstrumentationBreak();

    std::optional<Tagged<Object>> OnThrow(Handle<Object> exception) V8_WARN_UNUSED_RESULT;
    ...
    void SetInstrumentationBreakpointForWasmScript(Handle<Script> script, int* id);
    ...
    void InstallCoverageInfo(DirectHandle<SharedFunctionInfo> shared, Handle<CoverageInfo> coverage_info);
    ...
    bool FindSharedFunctionInfosIntersectingRange(
        Handle<Script> script, int start_position, int end_position, std::vector<Handle<SharedFunctionInfo>>* candidates);

    MaybeHandle<SharedFunctionInfo> GetTopLevelWithRecompile(Handle<Script> script, bool* did_compile = nullptr);

    static Handle<Object> GetSourceBreakLocations(Isolate* isolate, DirectHandle<SharedFunctionInfo> shared);
    ...
    bool PerformSideEffectCheckForAccessor(DirectHandle<AccessorInfo> accessor_info, Handle<Object> receiver, AccessorComponent component);
    bool PerformSideEffectCheckForCallback(Handle<FunctionTemplateInfo> function);
    bool PerformSideEffectCheckForInterceptor(Handle<InterceptorInfo> interceptor_info);
...
private:
    ...
    void OnException(Handle<Object> exception, MaybeHandle<JSPromise> promise, v8::debug::ExceptionType exception_type);
    ...
    MaybeHandle<FixedArray> CheckBreakPointsForLocations(Handle<DebugInfo> debug_info, std::vector<BreakLocation>& break_locations, bool* has_break_points);
...
public:
    explicit DisableTemporaryObjectTracking(Debug* debug)
        : debug_(debug)
        , previous_tracking_disabled_(debug->GetTemporaryObjectTrackingDisabled())
    {
...
public:
    ...
    ~SuppressDebug()
    {
        debug_->is_suppressed_ = old_state_;
    }
