...
FrameInspector::FrameInspector(CommonFrame* frame, int inlined_frame_index, Isolate* isolate)
    : frame_(frame)
    ...
    , isolate_(isolate)
{
...
#if V8_ENABLE_WEBASSEMBLY
    JavaScriptFrame* js_frame = frame->is_javascript() ? javascript_frame() : nullptr;
    ...
    if (is_optimized_) {
        ...
        deoptimized_frame_.reset(Deoptimizer::DebuggerInspectableFrame(js_frame, inlined_frame_index, isolate));
    }
}
...
{
    return is_optimized_ ? deoptimized_frame_->GetExpression(index) : handle(frame_->GetExpression(index), isolate_);
}

Handle<Object> FrameInspector::GetContext()
{
    return deoptimized_frame_ ? deoptimized_frame_->GetContext() : handle(frame_->context(), isolate_);
}
...
Handle<String> FrameInspector::GetFunctionName()
{
    ...
    if (IsWasm()) {
        ...
        if (IsWasmInterpreter()) {
            ...
            return GetWasmFunctionDebugName(isolate_, instance_data, wasm_frame->function_index(inlined_frame_index_));
        }
#endif // V8_ENABLE_DRUMBRAKE
        auto wasm_frame = WasmFrame::cast(frame_);
        auto instance_data = handle(wasm_frame->trusted_instance_data(), isolate_);
        return GetWasmFunctionDebugName(isolate_, instance_data, wasm_frame->function_index());
    }
#endif // V8_ENABLE_WEBASSEMBLY
...
{
    return frame_->is_javascript();
}

bool FrameInspector::ParameterIsShadowedByContextLocal(DirectHandle<ScopeInfo> info, Handle<String> parameter_name)
{
    return info->ContextSlotIndex(parameter_name) != -1;
}

RedirectActiveFunctions::RedirectActiveFunctions(Isolate* isolate, Tagged<SharedFunctionInfo> shared, Mode mode)
    : shared_(shared)
    , mode_(mode)
{
...
void RedirectActiveFunctions::VisitThread(Isolate* isolate, ThreadLocalTop* top)
{
    for (JavaScriptStackFrameIterator it(isolate, top); !it.done(); it.Advance()) {
        ...
        Tagged<BytecodeArray> bytecode
            = mode_ == Mode::kUseDebugBytecode ? shared_->GetDebugInfo(isolate)->DebugBytecodeArray(isolate) : shared_->GetBytecodeArray(isolate);
