...
void Target::InitQueryPropertyMap()
{
    ...
    query_properties_["Supported"] = "PacketSize=1000;vContSupported-;qXfer:libraries:read+;wasm+;";

    query_properties_["Attached"] = "1";

    // There is only one register, named 'pc', in this architecture
    query_properties_["RegisterInfo0"] = "name:pc;alt-name:pc;bitsize:64;offset:0;encoding:uint;format:hex;set:"
                                         "General Purpose Registers;gcc:16;dwarf:16;generic:pc;";
    query_properties_["RegisterInfo1"] = "E45";

    // ProcessInfo for wasm32
    query_properties_["ProcessInfo"] = "pid:1;ppid:1;uid:1;gid:1;euid:1;egid:1;name:6c6c6462;triple:" + Mem2Hex("wasm32-unknown-unknown-wasm") + ";ptrsize:4;";
...
void Target::OnProgramBreak(Isolate* isolate, const std::vector<wasm_addr_t>& call_frames)
{
    OnSuspended(isolate, kSigTrace, call_frames);
}
void Target::OnException(Isolate* isolate, const std::vector<wasm_addr_t>& call_frames)
{
    OnSuspended(isolate, kSigSegv, call_frames);
}
void Target::OnSuspended(Isolate* isolate, int signal, const std::vector<wasm_addr_t>& call_frames)
{
...
Target::ProcessPacketResult Target::ProcessPacket(Packet* pkt_in, Packet* pkt_out)
{
    ...
    case 'm': {
        ...
        if (wasm_addr.ModuleId() > 0) {
            uint32_t read = gdb_server_->GetWasmModuleBytes(wasm_addr, buff, length);
            if (read > 0) {
    ...
    case 'Z': {
        ...
        if (!pkt_in->GetNumberSep(&breakpoint_type, 0) || breakpoint_type != 0 || !pkt_in->GetNumberSep(&breakpoint_address, 0)
        ...
        if (!gdb_server_->AddBreakpoint(wasm_breakpoint_addr.ModuleId(), wasm_breakpoint_addr.Offset())) {
            err = ErrorCode::Failed;
            break;
        }
    ...
    case 'z': {
        ...
        wasm_addr_t wasm_breakpoint_addr(breakpoint_address);
        if (!gdb_server_->RemoveBreakpoint(wasm_breakpoint_addr.ModuleId(), wasm_breakpoint_addr.Offset())) {
            err = ErrorCode::Failed;
            break;
        }
...
Target::ErrorCode Target::ProcessQueryPacket(const Packet* pkt_in, Packet* pkt_out)
{
    ...
    if (!strncmp(str, tmp.data(), tmp.length())) {
        std::vector<GdbServer::WasmModuleInfo> modules = gdb_server_->GetLoadedModules(true);
        std::string result("l<library-list>");
        for (const auto& module : modules) {
            wasm_addr_t address(module.module_id, 0);
            char address_string[32];
            snprintf(address_string, sizeof(address_string), "%" PRIu64, static_cast<uint64_t>(address));
            result += "<library name=\"";
    ...
    if (toks[0] == "WasmCallStack") {
        ...
        pkt_out->AddBlock(buffer.data(), static_cast<uint32_t>(sizeof(uint64_t) * buffer.size()));
        return ErrorCode::None;
    }
    ...
    if (toks[0] == "WasmGlobal") {
        if (toks.size() == 3) {
            uint32_t frame_index = static_cast<uint32_t>(strtol(toks[1].data(), nullptr, 10));
            uint32_t index = static_cast<uint32_t>(strtol(toks[2].data(), nullptr, 10));
            uint8_t buff[16];
            uint32_t size = 0;
    ...
    if (toks[0] == "WasmMem") {
        if (toks.size() == 4) {
            ...
            uint8_t buff[Transport::kBufSize];
            uint32_t read = gdb_server_->GetWasmMemory(module_id, address, buff, length);
            if (read > 0) {
                pkt_out->AddBlock(buff, read);
                return ErrorCode::None;
            } else {
    ...
    if (toks[0] == "WasmData") {
        if (toks.size() == 4) {
            ...
            uint8_t buff[Transport::kBufSize];
            uint32_t read = gdb_server_->GetWasmData(module_id, address, buff, length);
            if (read > 0) {
                pkt_out->AddBlock(buff, read);
                return ErrorCode::None;
            } else {
...
void Target::SetStopReply(Packet* pkt_out) const
{
    ...
    char buff[64];
    snprintf(buff, sizeof(buff), "thread-pcs:%" PRIx64 ";", static_cast<uint64_t>(GetCurrentPc()));
    ...
    // that.
    if (gdb_server_->HasModuleListChanged())
        pkt_out->AddString("library:;");
}

void Target::SetStatus(Status status, int8_t signal, std::vector<wasm_addr_t> call_frames, Isolate* isolate)
{
    v8::base::MutexGuard guard(&mutex_);

    DCHECK((status == Status::Suspended && signal != 0 && call_frames.size() > 0 && isolate != nullptr)
