...
SocketBinding SocketBinding::Bind(uint16_t tcp_port)
{
    ...
    if (setsockopt(socket_handle, SOL_SOCKET, SO_EXCLUSIVEADDRUSE, reinterpret_cast<char*>(&exclusive_address), sizeof(exclusive_address))) {
    ...
    // to bind it.
    int reuse_address = 1;
    if (setsockopt(socket_handle, SOL_SOCKET, SO_REUSEADDR, reinterpret_cast<char*>(&reuse_address), sizeof(reuse_address))) {
        TRACE_GDB_REMOTE("Failed to set SO_REUSEADDR option.\n");
    }
#endif

    if (bind(socket_handle, reinterpret_cast<struct sockaddr*>(&sockaddr), addrlen)) {
...
{
    int nodelay = 1;
    if (::setsockopt(socket, IPPROTO_TCP, TCP_NODELAY, reinterpret_cast<char*>(&nodelay), sizeof(nodelay))) {
...
SocketTransport::SocketTransport(SocketHandle s)
    : Transport(s)
{
    socket_event_ = WSA_INVALID_EVENT;
    faulted_thread_event_ = ::CreateEvent(NULL, TRUE, FALSE, NULL);
    if (faulted_thread_event_ == NULL) {
        TRACE_GDB_REMOTE("SocketTransport::SocketTransport: Failed to create event object for "
                         "faulted thread\n");
    }
}
...
SocketTransport::~SocketTransport()
{
    if (!CloseHandle(faulted_thread_event_)) {
        TRACE_GDB_REMOTE("SocketTransport::~SocketTransport: Failed to close "
                         "event\n");
    }
...
bool SocketTransport::AcceptConnection()
{
    ...
    if (handle_accept_ != InvalidSocket) {
        ...
        if (socket_event_ == WSA_INVALID_EVENT) {
            TRACE_GDB_REMOTE("SocketTransport::AcceptConnection: Failed to create socket event\n");
        }
        ...
        if (::WSAEventSelect(handle_accept_, socket_event_, FD_CLOSE | FD_READ) == SOCKET_ERROR) {
            TRACE_GDB_REMOTE("SocketTransport::AcceptConnection: Failed to bind event to "
                             "socket\n");
        }
        return true;
    }
...
{
    while (true) {
        ssize_t result = ::recv(handle_accept_, buf_.get() + size_, kBufSize - size_, 0);
        if (result > 0) {
            size_ += result;
            return true;
        }
        ...
        if (SocketGetLastError() == WSAEWOULDBLOCK) {
            if (::WaitForSingleObject(socket_event_, INFINITE) == WAIT_FAILED) {
                TRACE_GDB_REMOTE("SocketTransport::ReadSomeData: Failed to wait on socket event\n");
            }
            if (!::ResetEvent(socket_event_)) {
                TRACE_GDB_REMOTE("SocketTransport::ReadSomeData: Failed to reset socket event\n");
            }
            continue;
        }
...
void SocketTransport::WaitForDebugStubEvent()
{
    ...
    int result = WaitForMultipleObjects(count, handles, FALSE, wait ? INFINITE : 0);
    if (result == WAIT_OBJECT_0 + 1) {
        if (!ResetEvent(socket_event_)) {
            TRACE_GDB_REMOTE("SocketTransport::WaitForDebugStubEvent: Failed to reset socket "
                             "event\n");
        }
        return;
    } else if (result == WAIT_OBJECT_0) {
        if (!ResetEvent(faulted_thread_event_)) {
            TRACE_GDB_REMOTE("SocketTransport::WaitForDebugStubEvent: Failed to reset event\n");
        }
        return;
    } else if (result == WAIT_TIMEOUT) {
        return;
    }
    TRACE_GDB_REMOTE("SocketTransport::WaitForDebugStubEvent: Wait for events failed\n");
}
...
#endif
    if (ret < 0) {
        TRACE_GDB_REMOTE("SocketTransport::SocketTransport: Failed to allocate pipe for faulted "
                         "thread\n");
    }
...
void SocketTransport::WaitForDebugStubEvent()
{
    ...
    if (ret < 0) {
        TRACE_GDB_REMOTE("SocketTransport::WaitForDebugStubEvent: Failed to wait for "
                         "debug stub event\n");
    }
    ...
    if (ret > 0) {
        if (FD_ISSET(faulted_thread_fd_read_, &fds)) {
            ...
            if (read(faulted_thread_fd_read_, &buf, sizeof(buf)) < 0) {
                TRACE_GDB_REMOTE("SocketTransport::WaitForDebugStubEvent: Failed to read from "
...
bool SocketTransport::SignalThreadEvent()
{
    ...
    if (write(faulted_thread_fd_write_, &buf, sizeof(buf)) != sizeof(buf)) {
        TRACE_GDB_REMOTE("SocketTransport:SignalThreadEvent: Can't send debug stub "
                         "event\n");
        return false;
    }
    return true;
