...
WasmModuleDebug::WasmModuleDebug(v8::Isolate* isolate, Local<debug::WasmScript> wasm_script)
{
...
Handle<WasmInstanceObject> WasmModuleDebug::GetFirstWasmInstance()
{
    ...
    Handle<WeakArrayList> weak_instance_list(script->wasm_weak_instance_list(), GetIsolate());
    if (weak_instance_list->length() > 0) {
        Tagged<MaybeObject> maybe_instance = weak_instance_list->Get(0);
        if (maybe_instance.IsWeak()) {
            Handle<WasmInstanceObject> instance(Cast<WasmInstanceObject>(maybe_instance.GetHeapObjectAssumeWeak()), GetIsolate());
            return instance;
        }
    }
...
// static
std::vector<wasm_addr_t> WasmModuleDebug::GetCallStack(uint32_t debug_context_id, Isolate* isolate)
{
    std::vector<wasm_addr_t> call_stack;
    for (StackFrameIterator frame_it(isolate); !frame_it.done(); frame_it.Advance()) {
        ...
        case StackFrame::WASM: {
            ...
            for (size_t i = frames.size(); i-- != 0;) {
                ...
                if (summary.IsJavaScript()) {
                    FrameSummary::JavaScriptFrameSummary const& javascript = summary.AsJavaScript();
                    ...
                    offset = GetWasmFunctionOffset(wasm.wasm_instance()->module(), wasm.function_index()) + wasm.code_offset();
                    script = wasm.script();

                    bool zeroth_frame = call_stack.empty();
                    if (!zeroth_frame) {
                        const NativeModule* native_module = wasm.wasm_instance()->module_object().native_module();
                        offset = ReturnPc(native_module, offset);
                    }
                }

                if (offset > 0) {
                    call_stack.push_back({ debug_context_id << 16 | script->id(), uint32_t(offset) });
                }
            }
            break;
        }
    ...
    if (call_stack.empty())
        call_stack.push_back({ 1, 0 });
    return call_stack;
}

// static
std::vector<FrameSummary> WasmModuleDebug::FindWasmFrame(DebuggableStackFrameIterator* frame_it, uint32_t* frame_index)
{
    while (!frame_it->done()) {
        StackFrame* const frame = frame_it->frame();
        switch (frame->type()) {
...
// static
Handle<WasmInstanceObject> WasmModuleDebug::GetWasmInstance(Isolate* isolate, uint32_t frame_index)
{
    ...
    const FrameSummary::WasmFrameSummary& summary = frames[reversed_index].AsWasm();
    return summary.wasm_instance();
}

// static
bool WasmModuleDebug::GetWasmGlobal(Isolate* isolate, uint32_t frame_index, uint32_t index, uint8_t* buffer, uint32_t buffer_size, uint32_t* size)
{
    ...
    if (!instance.is_null()) {
        ...
        if (index < module->globals.size()) {
            wasm::WasmValue wasm_value = WasmInstanceObject::GetGlobalValue(instance, module->globals[index]);
...
// static
bool WasmModuleDebug::GetWasmLocal(Isolate* isolate, uint32_t frame_index, uint32_t index, uint8_t* buffer, uint32_t buffer_size, uint32_t* size)
{
    HandleScope handles(isolate);
    ...
    if (summary.IsWasm()) {
        ...
        if (!instance.is_null()) {
            ...
            if (static_cast<uint32_t>(debug_info->GetNumLocals(frame_it.frame()->pc())) > index) {
                wasm::WasmValue wasm_value = debug_info->GetLocalValue(index, frame_it.frame()->pc(), frame_it.frame()->fp(), frame_it.frame()->callee_fp());
                return GetWasmValue(wasm_value, buffer, buffer_size, size);
            }
        }
    }
...
// static
bool WasmModuleDebug::GetWasmStackValue(Isolate* isolate, uint32_t frame_index, uint32_t index, uint8_t* buffer, uint32_t buffer_size, uint32_t* size)
{
    HandleScope handles(isolate);
    ...
    if (summary.IsWasm()) {
        ...
        if (!instance.is_null()) {
            ...
            wasm::NativeModule* native_module = module_object->native_module();
            DebugInfo* debug_info = native_module->GetDebugInfo();
            if (static_cast<uint32_t>(debug_info->GetStackDepth(frame_it.frame()->pc())) > index) {
                WasmValue wasm_value = debug_info->GetStackValue(index, frame_it.frame()->pc(), frame_it.frame()->fp(), frame_it.frame()->callee_fp());
                return GetWasmValue(wasm_value, buffer, buffer_size, size);
            }
        }
    }
...
uint32_t WasmModuleDebug::GetWasmMemory(Isolate* isolate, uint32_t offset, uint8_t* buffer, uint32_t size)
{
    HandleScope handles(isolate);
...
uint32_t WasmModuleDebug::GetWasmData(Isolate* isolate, uint32_t offset, uint8_t* buffer, uint32_t size)
{
    HandleScope handles(isolate);
...
uint32_t WasmModuleDebug::GetWasmModuleBytes(wasm_addr_t wasm_addr, uint8_t* buffer, uint32_t size)
{
    uint32_t bytes_read = 0;
    // Any instance will work.
    Handle<WasmInstanceObject> instance = GetFirstWasmInstance();
    if (!instance.is_null()) {
        Handle<WasmModuleObject> module_object(instance->module_object(), GetIsolate());
        wasm::NativeModule* native_module = module_object->native_module();
...
bool WasmModuleDebug::AddBreakpoint(uint32_t offset, int* breakpoint_id)
{
    ...
    return GetIsolate()->debug()->SetBreakPointForScript(script, condition, &breakpoint_address, breakpoint_id);
}
...
void WasmModuleDebug::PrepareStep()
{
    ...
    debug::PrepareStep(reinterpret_cast<v8::Isolate*>(isolate), debug::StepAction::StepInto);
}

template <typename T> bool StoreValue(const T& value, uint8_t* buffer, uint32_t buffer_size, uint32_t* size)
{
    *size = sizeof(value);
    if (*size > buffer_size)
        return false;
...
// static
bool WasmModuleDebug::GetWasmValue(const wasm::WasmValue& wasm_value, uint8_t* buffer, uint32_t buffer_size, uint32_t* size)
{
