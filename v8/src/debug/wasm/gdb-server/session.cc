...
void Session::WaitForDebugStubEvent()
{
    io_->WaitForDebugStubEvent();
}

bool Session::SignalThreadEvent()
{
    return io_->SignalThreadEvent();
}

bool Session::IsDataAvailable() const
{
...
bool Session::SendPacket(Packet* pkt, bool expect_ack)
{
    ...
    do {
        ...
        TRACE_GDB_REMOTE("TX %s\n", data.size() < 160 ? data.c_str() : (data.substr(0, 160) + "...").c_str());
...
bool Session::GetPacket(Packet* pkt)
{
    while (true) {
        ...
        char chars[2];
        if (!GetChar(&chars[0]) || !GetChar(&chars[1]) || !HexToUInt8(chars, &trailing_checksum)) {
            return false;
        }
