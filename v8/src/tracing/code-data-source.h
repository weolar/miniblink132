...
class CodeDataSource : public perfetto::DataSource<CodeDataSource, CodeDataSourceTraits> {
public:
...
public:
    ...
    uint64_t InternJsFunction(Isolate& isolate, Handle<SharedFunctionInfo> info, uint64_t v8_js_script_iid, int line_num, int column_num);
    uint64_t InternWasmScript(Isolate& isolate, int script_id, const std::string& url);
...
private:
    ...
    struct Function {
        ...
        {
            return v8_js_script_iid == other.v8_js_script_iid && is_toplevel == other.is_toplevel && start_position == other.start_position;
        }
        ...
        struct Hash {
            ...
            {
                return base::Hasher::Combine(f.v8_js_script_iid, f.is_toplevel, f.start_position);
            }
        };
    };
