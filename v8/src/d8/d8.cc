...
private:
    ...
    void* AllocateVM(size_t length)
    {
        ...
        return i::AllocatePages(page_allocator, nullptr, allocated, page_size, PageAllocator::kReadWrite);
    }
...
protected:
    ...
    void* AllocateUninitialized(size_t length) override
    {
        ...
        i::Address in_sandbox_page_reservation
            = vas->AllocatePages(VirtualAddressSpace::kNoHint, rounded_length, vas->allocation_granularity(), PagePermissions::kNoAccess);
        void* virtual_alloc = in_sandbox_page_reservation != 0 ? reinterpret_cast<void*>(in_sandbox_page_reservation) : reinterpret_cast<void*>(-1);
#else
        void* virtual_alloc = mmap(nullptr, rounded_length, prot, flags | MAP_NORESERVE, -1, 0);
#endif
        ...
        for (i::Address to_map = virtual_base; to_map < virtual_end; to_map += kChunkSize) {
            ...
            void* result = mremap(real_alloc, 0, kChunkSize, MREMAP_MAYMOVE | MREMAP_FIXED, reinterpret_cast<void*>(to_map));
...
template <int N> void ThrowError(Isolate* isolate, const char (&message)[N])
{
...
static MaybeLocal<Value> TryGetValue(v8::Isolate* isolate, Local<Context> context, Local<v8::Object> object, const char* property)
{
...
static Local<Value> GetValue(v8::Isolate* isolate, Local<Context> context, Local<v8::Object> object, const char* property)
{
    return TryGetValue(isolate, context, object, property).ToLocalChecked();
}

std::shared_ptr<Worker> GetWorkerFromInternalField(Isolate* isolate, Local<Object> object)
{
    ...
    i::DirectHandle<i::Object> handle = Utils::OpenDirectHandle(*object->GetInternalField(0));
    if (IsSmi(*handle)) {
...
public:
    static void FillTraceConfig(v8::Isolate* isolate, platform::tracing::TraceConfig* trace_config, const char* json_str)
    {
        ...
        Local<String> source = String::NewFromUtf8(isolate, json_str).ToLocalChecked();
        ...
        Local<Value> maybe_trace_config_object = GetValue(isolate, context, trace_config_object, kTraceConfigParam);
        if (maybe_trace_config_object->IsObject()) {
            trace_config_object = maybe_trace_config_object.As<Object>();
        }

        UpdateIncludedCategoriesList(isolate, context, trace_config_object, trace_config);
    }

private:
    static int UpdateIncludedCategoriesList(
        v8::Isolate* isolate, Local<Context> context, Local<v8::Object> object, platform::tracing::TraceConfig* trace_config)
    {
        ...
        if (value->IsArray()) {
            ...
            for (int i = 0, length = v8_array->Length(); i < length; ++i) {
                Local<Value> v = v8_array->Get(context, i).ToLocalChecked()->ToString(context).ToLocalChecked();
...
class ExternalOwningOneByteStringResource : public String::ExternalOneByteStringResource {
public:
...
std::map<Isolate*, std::pair<Global<Function>, Global<Context>>> Shell::profiler_end_callback_;
...
std::map<std::string, std::unique_ptr<ScriptCompiler::CachedData>> Shell::cached_code_map_;
...
ScriptCompiler::CachedData* Shell::LookupCodeCache(Isolate* isolate, Local<Value> source)
{
    i::ParkedMutexGuard lock_guard(reinterpret_cast<i::Isolate*>(isolate)->main_thread_local_isolate(), cached_code_mutex_.Pointer());
    ...
    if (entry != cached_code_map_.end() && entry->second) {
        ...
        ScriptCompiler::CachedData* cached_data = new ScriptCompiler::CachedData(cache, length, ScriptCompiler::CachedData::BufferOwned);
        return cached_data;
    }
    return nullptr;
}

void Shell::StoreInCodeCache(Isolate* isolate, Local<Value> source, const ScriptCompiler::CachedData* cache_data)
{
    i::ParkedMutexGuard lock_guard(reinterpret_cast<i::Isolate*>(isolate)->main_thread_local_isolate(), cached_code_mutex_.Pointer());
    CHECK(source->IsString());
    ...
    cached_code_map_[*key]
        = std::unique_ptr<ScriptCompiler::CachedData>(new ScriptCompiler::CachedData(cache, length, ScriptCompiler::CachedData::BufferOwned));
}
...
namespace {
template <class T>
MaybeLocal<T> CompileStreamed(Local<Context> context, ScriptCompiler::StreamedSource* v8_source, Local<String> full_source_string, const ScriptOrigin& origin)
{
}

template <>
MaybeLocal<Script> CompileStreamed(
    Local<Context> context, ScriptCompiler::StreamedSource* v8_source, Local<String> full_source_string, const ScriptOrigin& origin)
{
    return ScriptCompiler::Compile(context, v8_source, full_source_string, origin);
}

template <>
MaybeLocal<Module> CompileStreamed(
    Local<Context> context, ScriptCompiler::StreamedSource* v8_source, Local<String> full_source_string, const ScriptOrigin& origin)
{
    return ScriptCompiler::CompileModule(context, v8_source, full_source_string, origin);
}

template <class T> MaybeLocal<T> Compile(Local<Context> context, ScriptCompiler::Source* source, ScriptCompiler::CompileOptions options)
{
}
template <> MaybeLocal<Script> Compile(Local<Context> context, ScriptCompiler::Source* source, ScriptCompiler::CompileOptions options)
{
    return ScriptCompiler::Compile(context, source, options);
}

template <> MaybeLocal<Module> Compile(Local<Context> context, ScriptCompiler::Source* source, ScriptCompiler::CompileOptions options)
{
...
template <class T> MaybeLocal<T> Shell::CompileString(Isolate* isolate, Local<Context> context, Local<String> source, const ScriptOrigin& origin)
{
    if (options.streaming_compile) {
        v8::ScriptCompiler::StreamedSource streamed_source(std::make_unique<DummySourceStream>(isolate, source), v8::ScriptCompiler::StreamedSource::TWO_BYTE);
        std::unique_ptr<v8::ScriptCompiler::ScriptStreamingTask> streaming_task(
            v8::ScriptCompiler::StartStreaming(isolate, &streamed_source, std::is_same<T, Module>::value ? v8::ScriptType::kModule : v8::ScriptType::kClassic));
    ...
    MaybeLocal<T> result = Compile<T>(context, &script_source, cached_code ? ScriptCompiler::kConsumeCodeCache : ScriptCompiler::kNoCompileOptions);
    if (cached_code)
...
public:
    ...
    explicit ModuleEmbedderData(Isolate* isolate)
        ...
        , json_module_to_parsed_json_map(10, module_to_specifier_map.hash_function())
    {
    }
    ...
    Local<Module> GetModule(std::pair<std::string, ModuleType> module_specifier_and_type)
    {
    ...
    Local<Object> GetModuleSource(std::pair<std::string, ModuleType> module_specifier_and_type)
    {
    ...
    {
        auto json_value_it = json_module_to_parsed_json_map.find(Global<Module>(isolate_, module));
    ...
    static ModuleType ModuleTypeFromImportSpecifierAndAttributes(
        Local<Context> context, const std::string& specifier, Local<FixedArray> import_attributes, bool hasPositions)
    {
        Isolate* isolate = context->GetIsolate();
        const int kV8AssertionEntrySize = hasPositions ? 3 : 2;
        for (int i = 0; i < import_attributes->Length(); i += kV8AssertionEntrySize) {
            Local<String> v8_assertion_key = import_attributes->Get(context, i).As<v8::String>();
            std::string assertion_key = ToSTLString(isolate, v8_assertion_key);

            if (assertion_key == "type") {
                Local<String> v8_assertion_value = import_attributes->Get(context, i + 1).As<String>();
    ...
    std::map<std::pair<std::string, ModuleType>, Global<Object>> module_source_map;
    ...
    std::unordered_map<Global<Module>, Global<Value>, ModuleGlobalHash> json_module_to_parsed_json_map;
...
std::shared_ptr<ModuleEmbedderData> InitializeModuleEmbedderData(Local<Context> context)
{
    ...
    i::Handle<i::Managed<ModuleEmbedderData>> module_data_managed
        = i::Managed<ModuleEmbedderData>::From(i_isolate, kModuleEmbedderDataEstimate, std::make_shared<ModuleEmbedderData>(context->GetIsolate()));
...
std::shared_ptr<ModuleEmbedderData> GetModuleDataFromContext(Local<Context> context)
{
    v8::Local<v8::Value> module_data = context->GetEmbedderData(kModuleEmbedderDataIndex);
    i::DirectHandle<i::Managed<ModuleEmbedderData>> module_data_managed
        = i::Cast<i::Managed<ModuleEmbedderData>>(Utils::OpenDirectHandle<Value, i::Object>(module_data));
    return module_data_managed->get();
}

ScriptOrigin CreateScriptOrigin(Isolate* isolate, Local<String> resource_name, v8::ScriptType type)
{
    Local<PrimitiveArray> options = PrimitiveArray::New(isolate, kHostDefinedOptionsLength);
    options->Set(isolate, 0, v8::Uint32::New(isolate, kHostDefinedOptionsMagicConstant));
    options->Set(isolate, 1, resource_name);
    return ScriptOrigin(resource_name, 0, 0, false, -1, Local<Value>(), false, false, type == v8::ScriptType::kModule, options);
}

bool IsValidHostDefinedOptions(Local<Context> context, Local<Data> options, Local<Value> resource_name)
{
    if (!options->IsFixedArray())
        return false;
    Local<FixedArray> array = options.As<FixedArray>();
    if (array->Length() != kHostDefinedOptionsLength)
        return false;
    uint32_t magic = 0;
    if (!array->Get(context, 0).As<Value>()->Uint32Value(context).To(&magic)) {
        return false;
    }
    if (magic != kHostDefinedOptionsMagicConstant)
        return false;
...
public:
    ...
    void Run() override
    {
        ...
        Maybe<bool> ret = success_ == WasmAsyncSuccess::kSuccess ? resolver->Resolve(context, result) : resolver->Reject(context, result);
    ...
    v8::Isolate* isolate, v8::Local<v8::Context> context, v8::Local<v8::Promise::Resolver> resolver, v8::Local<v8::Value> result, WasmAsyncSuccess success)
{
    ...
    // the result promise.
    g_platform->GetForegroundTaskRunner(isolate)->PostTask(std::make_unique<D8WasmAsyncResolvePromiseTask>(isolate, context, resolver, result, success));
}

} // namespace

// Executes a string within the current v8 context.
bool Shell::ExecuteString(Isolate* isolate, Local<String> source, Local<String> name, ReportExceptions report_exceptions, Global<Value>* out_result)
{
    ...
    if (i::v8_flags.parse_only) {
        ...
        i::UnoptimizedCompileFlags flags = i::UnoptimizedCompileFlags::ForToplevelCompile(
            i_isolate, true, i::construct_language_mode(i::v8_flags.use_strict), i::REPLMode::kNo, ScriptType::kClassic, i::v8_flags.lazy);
        ...
        i::Handle<i::Script> script = parse_info.CreateScript(i_isolate, str, i::kNullMaybeHandle, ScriptOriginOptions());
        if (!i::parsing::ParseProgram(&parse_info, script, i_isolate, i::parsing::ReportStatisticsMode::kYes)) {
            parse_info.pending_error_handler()->PrepareErrors(i_isolate, parse_info.ast_value_factory());
    ...
    std::shared_ptr<ModuleEmbedderData> module_data = GetModuleDataFromContext(realm);
    ...
    Local<Script> script;
    if (!CompileString<Script>(isolate, context, source, origin).ToLocal(&script)) {
        return false;
    }
    ...
    if (options.compile_only)
        return true;
    if (options.compile_options & ScriptCompiler::kConsumeCodeCache) {
        i::DirectHandle<i::Script> i_script(i::Cast<i::Script>(Utils::OpenDirectHandle(*script)->shared()->script()), i_isolate);
        // TODO(cbruni, chromium:1244145): remove once context-allocated.
        i_script->set_host_defined_options(i::Cast<i::FixedArray>(*Utils::OpenDirectHandle(*(origin.GetHostDefinedOptions()))));
    }
...
std::string NormalizePath(const std::string& path, const std::string& dir_name)
{
    ...
    if (segments.size() > 1) {
        std::copy(segments.begin(), segments.end() - 1, std::ostream_iterator<std::string>(os, "/"));
        os << *segments.rbegin();
    } else {
        os << "/";
        if (!segments.empty())
...
std::string NormalizeModuleSpecifier(const std::string& specifier, const std::string& dir_name)
{
...
MaybeLocal<Module> ResolveModuleCallback(Local<Context> context, Local<String> specifier, Local<FixedArray> import_attributes, Local<Module> referrer)
{
    Isolate* isolate = context->GetIsolate();
    ...
    std::string absolute_path = NormalizeModuleSpecifier(stl_specifier, DirName(referrer_specifier));
    ModuleType module_type = ModuleEmbedderData::ModuleTypeFromImportSpecifierAndAttributes(context, stl_specifier, import_attributes, true);
    return module_data->GetModule(std::make_pair(absolute_path, module_type));
}

MaybeLocal<Object> ResolveModuleSourceCallback(Local<Context> context, Local<String> specifier, Local<FixedArray> import_attributes, Local<Module> referrer)
{
    Isolate* isolate = context->GetIsolate();
    std::shared_ptr<ModuleEmbedderData> module_data = GetModuleDataFromContext(context);
    std::string referrer_specifier = module_data->GetModuleSpecifier(referrer);
    ...
    return module_data->GetModuleSource(std::make_pair(absolute_path, module_type));
}

} // anonymous namespace

MaybeLocal<Object> Shell::FetchModuleSource(Local<Module> referrer, Local<Context> context, const std::string& module_specifier, ModuleType module_type)
{
    Isolate* isolate = context->GetIsolate();
    ...
    if (!file) {
        ...
        ThrowError(isolate, v8::String::NewFromUtf8(isolate, msg.c_str()).ToLocalChecked());
    ...
    case ModuleType::kWebAssembly: {
        if (!v8::WasmModuleObject::Compile(isolate, MemorySpan<const uint8_t>(static_cast<uint8_t*>(file->memory()), file->size())).ToLocal(&module_source)) {
            return MaybeLocal<Object>();
        }
        break;
    }
    default:
        ...
        // is a ReferenceError.
        ThrowException(isolate, v8::Exception::ReferenceError(String::NewFromUtf8Literal(isolate, "Module source can not be imported for type")));
        return MaybeLocal<Object>();
    }

    CHECK(module_data->module_source_map.insert(std::make_pair(std::make_pair(module_specifier, module_type), Global<Object>(isolate, module_source))).second);
    return module_source;
}

// file_name must be either an absolute path to the filesystem or a data URL.
MaybeLocal<Module> Shell::FetchModuleTree(Local<Module> referrer, Local<Context> context, const std::string& module_specifier, ModuleType module_type)
{
    Isolate* isolate = context->GetIsolate();
    const bool is_data_url = module_specifier.starts_with(kDataURLPrefix);
    MaybeLocal<String> source_text;
    if (is_data_url) {
        source_text = String::NewFromUtf8(isolate, module_specifier.c_str() + strlen(kDataURLPrefix));
    } else {
        DCHECK(IsAbsolutePath(module_specifier));
    ...
    if (module_type == ModuleType::kJavaScript) {
        ...
        if (!CompileString<Module>(isolate, context, source_text.ToLocalChecked(), origin).ToLocal(&module)) {
            return MaybeLocal<Module>();
        }
    } else if (module_type == ModuleType::kJSON) {
        Local<Value> parsed_json;
        if (!v8::JSON::Parse(context, source_text.ToLocalChecked()).ToLocal(&parsed_json)) {
            return MaybeLocal<Module>();
        }

        auto export_names = v8::to_array<Local<String>>({ String::NewFromUtf8(isolate, "default").ToLocalChecked() });

        module = v8::Module::CreateSyntheticModule(
            isolate, String::NewFromUtf8(isolate, module_specifier.c_str()).ToLocalChecked(), export_names, Shell::JSONModuleEvaluationSteps);

        CHECK(module_data->json_module_to_parsed_json_map.insert(std::make_pair(Global<Module>(isolate, module), Global<Value>(isolate, parsed_json))).second);
    } else {
        UNREACHABLE();
    }

    CHECK(module_data->module_map.insert(std::make_pair(std::make_pair(module_specifier, module_type), Global<Module>(isolate, module))).second);
    CHECK(module_data->module_to_specifier_map.insert(std::make_pair(Global<Module>(isolate, module), module_specifier)).second);
    ...
    for (int i = 0, length = module_requests->Length(); i < length; ++i) {
        Local<ModuleRequest> module_request = module_requests->Get(context, i).As<ModuleRequest>();
        std::string specifier = ToSTLString(isolate, module_request->GetSpecifier());
        std::string normalized_specifier = NormalizeModuleSpecifier(specifier, dir_name);
        Local<FixedArray> import_attributes = module_request->GetImportAttributes();
        ModuleType request_module_type = ModuleEmbedderData::ModuleTypeFromImportSpecifierAndAttributes(context, normalized_specifier, import_attributes, true);
        ...
        if (module_request->GetPhase() == ModuleImportPhase::kSource) {
            if (module_data->module_source_map.count(std::make_pair(normalized_specifier, request_module_type))) {
                continue;
            }

            if (FetchModuleSource(module, context, normalized_specifier, request_module_type).IsEmpty()) {
                return MaybeLocal<Module>();
            }
        } else {
            if (module_data->module_map.count(std::make_pair(normalized_specifier, request_module_type))) {
                continue;
            }

            if (FetchModuleTree(module, context, normalized_specifier, request_module_type).IsEmpty()) {
                return MaybeLocal<Module>();
            }
        }
    }
...
MaybeLocal<Value> Shell::JSONModuleEvaluationSteps(Local<Context> context, Local<Module> module)
{
    Isolate* isolate = context->GetIsolate();
    ...
    TryCatch try_catch(isolate);
    Maybe<bool> result = module->SetSyntheticModuleExport(isolate, String::NewFromUtf8Literal(isolate, "default", NewStringType::kInternalized), json_value);
...
struct DynamicImportData {
    DynamicImportData(Isolate* isolate_, Local<Context> context_, Local<Value> referrer_, Local<String> specifier_, ModuleImportPhase phase_,
        Local<FixedArray> import_attributes_, Local<Promise::Resolver> resolver_)
        : isolate(isolate_)
        , phase(phase_)
    {
...
void Shell::ModuleResolutionSuccessCallback(const FunctionCallbackInfo<Value>& info)
{
    ...
    Local<Promise::Resolver> resolver(module_resolution_data->Get(context, ModuleResolutionDataIndex::kResolver).ToLocalChecked().As<Promise::Resolver>());
    Local<Value> namespace_or_source(module_resolution_data->Get(context, ModuleResolutionDataIndex::kNamespaceOrSource).ToLocalChecked());
...
{
    return HostImportModuleWithPhaseDynamically(context, host_defined_options, resource_name, specifier, ModuleImportPhase::kEvaluation, import_attributes);
}

MaybeLocal<Promise> Shell::HostImportModuleWithPhaseDynamically(Local<Context> context, Local<Data> host_defined_options, Local<Value> resource_name,
    Local<String> specifier, ModuleImportPhase phase, Local<FixedArray> import_attributes)
{
    Isolate* isolate = context->GetIsolate();
    ...
    if (!resource_name->IsNull() && !IsValidHostDefinedOptions(context, host_defined_options, resource_name)) {
        resolver->Reject(context, v8::Exception::TypeError(String::NewFromUtf8Literal(isolate, "Invalid host defined options"))).ToChecked();
    } else {
        DynamicImportData* data = new DynamicImportData(isolate, context, resource_name, specifier, phase, import_attributes, resolver);
...
void Shell::HostInitializeImportMetaObject(Local<Context> context, Local<Module> module, Local<Object> meta)
{
    Isolate* isolate = context->GetIsolate();
    HandleScope handle_scope(isolate);
    ...
    Local<String> url_key = String::NewFromUtf8Literal(isolate, "url", NewStringType::kInternalized);
    Local<String> url = String::NewFromUtf8(isolate, specifier.c_str()).ToLocalChecked();
    meta->CreateDataProperty(context, url_key, url).ToChecked();
}

MaybeLocal<Context> Shell::HostCreateShadowRealmContext(Local<Context> initiator_context)
{
    Local<Context> context = v8::Context::New(initiator_context->GetIsolate());
    std::shared_ptr<ModuleEmbedderData> shadow_realm_data = InitializeModuleEmbedderData(context);
    std::shared_ptr<ModuleEmbedderData> initiator_data = GetModuleDataFromContext(initiator_context);
...
{
    DynamicImportData* import_data_ = static_cast<DynamicImportData*>(import_data);
...

    {
        ...
        Local<FixedArray> import_attributes = import_data_->import_attributes.Get(isolate);
        ...
        ModuleType module_type = ModuleEmbedderData::ModuleTypeFromImportSpecifierAndAttributes(realm, specifier, import_attributes, false);
        ...
        std::string source_url = referrer->IsNull() ? module_data->origin : ToSTLString(isolate, referrer.As<String>());
        ...
        case ModuleImportPhase::kSource: {
            Local<Object> module_source;
            auto module_it = module_data->module_source_map.find(std::make_pair(absolute_path, module_type));
            if (module_it != module_data->module_source_map.end()) {
                module_source = module_it->second.Get(isolate);
            } else if (!FetchModuleSource(Local<Module>(), realm, absolute_path, module_type).ToLocal(&module_source)) {
                CHECK(try_catch.HasCaught());
                if (isolate->IsExecutionTerminating()) {
            ...
            Local<Promise::Resolver> module_resolver = Promise::Resolver::New(realm).ToLocalChecked();
        ...
        case v8::ModuleImportPhase::kEvaluation: {
            ...
            auto module_it = module_data->module_map.find(std::make_pair(absolute_path, module_type));
            if (module_it != module_data->module_map.end()) {
                root_module = module_it->second.Get(isolate);
            } else if (!FetchModuleTree(Local<Module>(), realm, absolute_path, module_type).ToLocal(&root_module)) {
                CHECK(try_catch.HasCaught());
                if (isolate->IsExecutionTerminating()) {
                    Shell::ReportException(isolate, try_catch);
                } else {
            ...
            if (root_module->InstantiateModule(realm, ResolveModuleCallback, ResolveModuleSourceCallback).FromMaybe(false)) {
                MaybeLocal<Value> maybe_result = root_module->Evaluate(realm);
                CHECK(!maybe_result.IsEmpty());
                global_result_promise.Reset(isolate, maybe_result.ToLocalChecked().As<Promise>());
                global_namespace_or_source.Reset(isolate, root_module->GetModuleNamespace());
            }
            break;
        }
        default: {
...

    {
        ...
        i::EmbedderStackStateScope scope(heap, i::EmbedderStackStateOrigin::kExplicitInvocation, StackState::kMayContainHeapPointers);
    ...
    module_resolution_data->Set(realm, ModuleResolutionDataIndex::kResolver, resolver).ToChecked();
    module_resolution_data->Set(realm, ModuleResolutionDataIndex::kNamespaceOrSource, namespace_or_source).ToChecked();
    Local<Function> callback_success;
    CHECK(Function::New(realm, ModuleResolutionSuccessCallback, module_resolution_data).ToLocal(&callback_success));
    Local<Function> callback_failure;
    CHECK(Function::New(realm, ModuleResolutionFailureCallback, module_resolution_data).ToLocal(&callback_failure));
...
bool Shell::ExecuteModule(Isolate* isolate, const char* file_name)
{
...

    {
        ...
        std::string absolute_path = NormalizeModuleSpecifier(file_name, GetWorkingDirectory());

        std::shared_ptr<ModuleEmbedderData> module_data = GetModuleDataFromContext(realm);
        Local<Module> root_module;
        auto module_it = module_data->module_map.find(std::make_pair(absolute_path, ModuleType::kJavaScript));
        if (module_it != module_data->module_map.end()) {
            root_module = module_it->second.Get(isolate);
        } else if (!FetchModuleTree(Local<Module>(), realm, absolute_path, ModuleType::kJavaScript).ToLocal(&root_module)) {
            CHECK(try_catch.HasCaught());
    ...
    while (isolate->HasPendingBackgroundTasks()
        || (i::ValueHelper::HandleAsValue(global_result_promise)->State() == Promise::kPending
            && reinterpret_cast<i::Isolate*>(isolate)->default_microtask_queue()->size() > 0)) {
...

    {
        ...
        auto [stalled_modules, stalled_messages] = root_module->GetStalledTopLevelAwaitMessages(isolate);
...
bool Shell::LoadJSON(Isolate* isolate, const char* file_name)
{
    ...
    Local<Context> realm = isolate_data->realms_[isolate_data->realm_current_].Get(isolate);
    Context::Scope context_scope(realm);
    TryCatch try_catch(isolate);
    ...
    while (std::getline(stream, line, '\n')) {
        for (int r = 0; r < DeserializationRunCount(); ++r) {
            Local<String> source = String::NewFromUtf8(isolate, line.c_str()).ToLocalChecked();
...
void PerIsolateData::RemoveUnhandledPromise(Local<Promise> promise)
{
    ...
    for (auto it = unhandled_promises_.begin(); it != unhandled_promises_.end(); ++it) {
...
void PerIsolateData::AddUnhandledPromise(Local<Promise> promise, Local<Message> message, Local<Value> exception)
{
    if (ignore_unhandled_promises_)
        return;
    DCHECK_EQ(promise->GetIsolate(), isolate_);
    unhandled_promises_.emplace_back(
        v8::Global<v8::Promise>(isolate_, promise), v8::Global<v8::Message>(isolate_, message), v8::Global<v8::Value>(isolate_, exception));
}
...
void PerIsolateData::SubscribeWorkerOnMessage(const std::shared_ptr<Worker>& worker, Local<Context> context, Local<Function> callback)
{
    ...
    worker_message_callbacks_.emplace(worker, std::make_pair(Global<Context>(isolate_, context), Global<Function>(isolate_, callback)));
}

std::pair<Local<Context>, Local<Function>> PerIsolateData::GetWorkerOnMessage(const std::shared_ptr<Worker>& worker) const
{
...
PerIsolateData::RealmScope::RealmScope(Isolate* isolate, const Global<Context>& context)
...
PerIsolateData::ExplicitRealmScope::ExplicitRealmScope(PerIsolateData* data, int index)
    : data_(data)
    , index_(index)
{
...
int PerIsolateData::RealmIndexOrThrow(const v8::FunctionCallbackInfo<v8::Value>& info, int arg_offset)
{
    if (info.Length() < arg_offset || !info[arg_offset]->IsNumber()) {
        ThrowError(info.GetIsolate(), "Invalid argument");
        return -1;
    }
    int index = info[arg_offset]->Int32Value(info.GetIsolate()->GetCurrentContext()).FromMaybe(-1);
...
uint64_t Shell::GetTracingTimestampFromPerformanceTimestamp(double performance_timestamp)
{
    ...
    base::TimeDelta delta = base::TimeDelta::FromMillisecondsD(performance_timestamp);
...
void Shell::PerformanceMark(const v8::FunctionCallbackInfo<v8::Value>& info)
{
    ...
    performance_entry->DefineOwnProperty(context, String::NewFromUtf8Literal(isolate, "entryType"), String::NewFromUtf8Literal(isolate, "mark"), ReadOnly)
        .Check();
    performance_entry->DefineOwnProperty(context, String::NewFromUtf8Literal(isolate, "name"), name, ReadOnly).Check();
    performance_entry->DefineOwnProperty(context, String::NewFromUtf8Literal(isolate, "startTime"), Number::New(isolate, timestamp), ReadOnly).Check();
    performance_entry->DefineOwnProperty(context, String::NewFromUtf8Literal(isolate, "duration"), Integer::New(isolate, 0), ReadOnly).Check();
...
void Shell::PerformanceMeasure(const v8::FunctionCallbackInfo<v8::Value>& info)
{
    DCHECK(i::ValidateCallbackInfo(info));
    Isolate* isolate = info.GetIsolate();
    Local<Context> context = isolate->GetCurrentContext();
    ...
    if (info.Length() >= 2) {
        ...
        if (!start_mark->IsObject()) {
            ThrowError(info.GetIsolate(), "Invalid 'startMark' argument: Not an Object");
            return;
        }
        Local<Value> start_time_field;
        if (!start_mark.As<Object>()->Get(context, String::NewFromUtf8Literal(isolate, "startTime")).ToLocal(&start_time_field)) {
            return;
        }
        if (!start_time_field->IsNumber()) {
            ThrowError(info.GetIsolate(), "Invalid 'startMark' argument: No numeric 'startTime' field");
            return;
        }
    ...
    if (options.trace_enabled) {
        size_t hash = base::hash_combine(name->GetIdentityHash(), start_timestamp, end_timestamp);

        String::Utf8Value utf8(isolate, name);
        TRACE_EVENT_COPY_NESTABLE_ASYNC_BEGIN_WITH_TIMESTAMP1(
            "v8", *utf8, static_cast<uint64_t>(hash), GetTracingTimestampFromPerformanceTimestamp(start_timestamp), "startTime", start_timestamp);
        TRACE_EVENT_COPY_NESTABLE_ASYNC_END_WITH_TIMESTAMP0(
            "v8", *utf8, static_cast<uint64_t>(hash), GetTracingTimestampFromPerformanceTimestamp(end_timestamp));
    }

    Local<Object> performance_entry = Object::New(isolate);
    performance_entry->DefineOwnProperty(context, String::NewFromUtf8Literal(isolate, "entryType"), String::NewFromUtf8Literal(isolate, "measure"), ReadOnly)
        .Check();
    performance_entry->DefineOwnProperty(context, String::NewFromUtf8Literal(isolate, "name"), name, ReadOnly).Check();
    performance_entry->DefineOwnProperty(context, String::NewFromUtf8Literal(isolate, "startTime"), Number::New(isolate, start_timestamp), ReadOnly).Check();
    performance_entry
        ->DefineOwnProperty(context, String::NewFromUtf8Literal(isolate, "duration"), Number::New(isolate, end_timestamp - start_timestamp), ReadOnly)
        .Check();
...
void Shell::PerformanceMeasureMemory(const v8::FunctionCallbackInfo<v8::Value>& info)
{
    DCHECK(i::ValidateCallbackInfo(info));
    ...
    if (info.Length() >= 1 && info[0]->IsObject()) {
        ...
        Local<Value> value = TryGetValue(isolate, context, object, "detailed").FromMaybe(Local<Value>());
        if (value.IsEmpty()) {
    ...
    Local<v8::Promise::Resolver> promise_resolver = v8::Promise::Resolver::New(context).ToLocalChecked();
    info.GetIsolate()->MeasureMemory(v8::MeasureMemoryDelegate::Default(isolate, context, promise_resolver, mode), v8::MeasureMemoryExecution::kEager);
...
void Shell::RealmOwner(const v8::FunctionCallbackInfo<v8::Value>& info)
{
    ...
    Local<Object> object = info[0]->ToObject(isolate->GetCurrentContext()).ToLocalChecked();
    i::Handle<i::JSReceiver> i_object = Utils::OpenHandle(*object);
    if (IsJSGlobalProxy(*i_object) && i::Cast<i::JSGlobalProxy>(i_object)->IsDetached()) {
        return;
    }
...
void Shell::RealmGlobal(const v8::FunctionCallbackInfo<v8::Value>& info)
{
    ...
    // global objects.
    Local<Object> global = Local<Context>::New(info.GetIsolate(), data->realms_[index])->Global();
    ...
    if (IsJSGlobalObject(*i_global)) {
        ...
        i::Handle<i::JSObject> i_global_proxy = handle(i::Cast<i::JSGlobalObject>(i_global)->global_proxy(), i_isolate);
        global = Utils::ToLocal(i_global_proxy);
    }
    info.GetReturnValue().Set(global);
}

MaybeLocal<Context> Shell::CreateRealm(const v8::FunctionCallbackInfo<v8::Value>& info, int index, v8::MaybeLocal<Value> global_object)
{
    DCHECK(i::ValidateCallbackInfo(info));
...
void Shell::DisposeRealm(const v8::FunctionCallbackInfo<v8::Value>& info, int index)
{
    DCHECK(i::ValidateCallbackInfo(info));
    Isolate* isolate = info.GetIsolate();
    PerIsolateData* data = PerIsolateData::Get(isolate);
...
void Shell::RealmCreateAllowCrossRealmAccess(const v8::FunctionCallbackInfo<v8::Value>& info)
{
    DCHECK(i::ValidateCallbackInfo(info));
    Local<Context> context;
    if (CreateRealm(info, -1, v8::MaybeLocal<Value>()).ToLocal(&context)) {
        context->SetSecurityToken(info.GetIsolate()->GetEnteredOrMicrotaskContext()->GetSecurityToken());
    }
}
...
void Shell::RealmNavigate(const v8::FunctionCallbackInfo<v8::Value>& info)
{
    ...
    if (!global_object.IsEmpty()) {
        ...
        if (!IsJSGlobalProxy(*Utils::OpenDirectHandle(*global_object.ToLocalChecked()))) {
...
void Shell::RealmEval(const v8::FunctionCallbackInfo<v8::Value>& info)
{
    ...
    ScriptOrigin origin = CreateScriptOrigin(isolate, String::NewFromUtf8Literal(isolate, "(d8)"), ScriptType::kClassic);

    if (isolate->IsExecutionTerminating())
        return;
    ScriptCompiler::Source script_source(source, origin);
    Local<UnboundScript> script;
    if (!ScriptCompiler::CompileUnboundScript(isolate, &script_source).ToLocal(&script)) {
        return;
    }
    Local<Value> result;
    {
        PerIsolateData::ExplicitRealmScope realm_scope(data, index);
        if (!script->BindToCurrentContext()->Run(realm_scope.context()).ToLocal(&result)) {
            return;
        }
    }
...
void Shell::RealmSharedGet(Local<Name> property, const PropertyCallbackInfo<Value>& info)
{
    DCHECK(i::ValidateCallbackInfo(info));
    Isolate* isolate = info.GetIsolate();
    PerIsolateData* data = PerIsolateData::Get(isolate);
...
void Shell::RealmSharedSet(Local<Name> property, Local<Value> value, const PropertyCallbackInfo<void>& info)
{
    DCHECK(i::ValidateCallbackInfo(info));
    Isolate* isolate = info.GetIsolate();
    PerIsolateData* data = PerIsolateData::Get(isolate);
...
void Shell::LogGetAndStop(const v8::FunctionCallbackInfo<v8::Value>& info)
{
    ...
    Local<String> result = String::NewFromUtf8(isolate, raw_log.c_str(), NewStringType::kNormal, static_cast<int>(raw_log.size())).ToLocalChecked();

    info.GetReturnValue().Set(result);
}

void Shell::TestVerifySourcePositions(const v8::FunctionCallbackInfo<v8::Value>& info)
{
    Isolate* isolate = info.GetIsolate();
    ...
    if (!IsHeapObject(*arg_handle) || !IsJSFunctionOrBoundFunctionOrWrappedFunction(*i::Cast<i::HeapObject>(arg_handle))) {
        ThrowError(isolate, "Expected function as single argument.");
        return;
    }
    ...
    auto callable = i::Cast<i::JSFunctionOrBoundFunctionOrWrappedFunction>(arg_handle);
    while (IsJSBoundFunction(*callable)) {
        ...
        callable = handle(i::Cast<i::JSFunctionOrBoundFunctionOrWrappedFunction>(bound_target), i_isolate);
    }
    ...
    i::Handle<i::BytecodeArray> bytecodes = handle(function->shared()->GetBytecodeArray(i_isolate), i_isolate);
    ...
    if (has_baseline) {
        bytecode_offsets = handle(i::Cast<i::TrustedByteArray>(function->shared()->GetCode(i_isolate)->bytecode_offset_table()), i_isolate);
        offset_iterator = std::make_unique<i::baseline::BytecodeOffsetIterator>(bytecode_offsets, bytecodes);
        // A freshly initiated BytecodeOffsetIterator points to the prologue.
        DCHECK_EQ(offset_iterator->current_pc_start_offset(), 0);
        DCHECK_EQ(offset_iterator->current_bytecode_offset(), i::kFunctionEntryBytecodeOffset);
        offset_iterator->Advance();
    }
    while (!bytecode_iterator.done()) {
        if (has_baseline) {
            if (offset_iterator->current_bytecode_offset() != bytecode_iterator.current_offset()) {
            ...
            for (i::Address pc = offset_iterator->current_pc_start_offset() + 1; pc <= offset_iterator->current_pc_end_offset(); ++pc) {
                i::baseline::BytecodeOffsetIterator pc_lookup(bytecode_offsets, bytecodes);
                pc_lookup.AdvanceToPCOffset(pc);
                if (pc_lookup.current_bytecode_offset() != bytecode_iterator.current_offset()) {
                    ThrowError(isolate, "Baseline bytecode offset mismatch for PC lookup.");
                    return;
                }
            }
        }
...
void Shell::InstallConditionalFeatures(const v8::FunctionCallbackInfo<v8::Value>& info)
{
    Isolate* isolate = info.GetIsolate();
...
void Shell::AsyncHooksCreateHook(const v8::FunctionCallbackInfo<v8::Value>& info)
{
    Local<Object> wrap = PerIsolateData::Get(info.GetIsolate())->GetAsyncHooks()->CreateHook(info);
    info.GetReturnValue().Set(wrap);
}

// async_hooks.executionAsyncId() returns the asyncId of the current execution
// context.
void Shell::AsyncHooksExecutionAsyncId(const v8::FunctionCallbackInfo<v8::Value>& info)
{
    Isolate* isolate = info.GetIsolate();
    HandleScope handle_scope(isolate);
    info.GetReturnValue().Set(v8::Number::New(isolate, PerIsolateData::Get(isolate)->GetAsyncHooks()->GetExecutionAsyncId()));
}

void Shell::AsyncHooksTriggerAsyncId(const v8::FunctionCallbackInfo<v8::Value>& info)
{
    Isolate* isolate = info.GetIsolate();
    HandleScope handle_scope(isolate);
    info.GetReturnValue().Set(v8::Number::New(isolate, PerIsolateData::Get(isolate)->GetAsyncHooks()->GetTriggerAsyncId()));
}
...
void Shell::SetPromiseHooks(const v8::FunctionCallbackInfo<v8::Value>& info)
{
    ...
    context->SetPromiseHooks(info[0]->IsFunction() ? info[0].As<Function>() : Local<Function>(),
        info[1]->IsFunction() ? info[1].As<Function>() : Local<Function>(), info[2]->IsFunction() ? info[2].As<Function>() : Local<Function>(),
...
void Shell::SerializerSerialize(const v8::FunctionCallbackInfo<v8::Value>& info)
{
    DCHECK(i::ValidateCallbackInfo(info));
    Isolate* isolate = info.GetIsolate();
    HandleScope handle_scope(isolate);
    ...
    for (int i = 0; i < info.Length(); i++) {
        bool ok;
        if (!serializer.WriteValue(context, info[i]).To(&ok))
            return;
    }
...
void Shell::SerializerDeserialize(const v8::FunctionCallbackInfo<v8::Value>& info)
{
    DCHECK(i::ValidateCallbackInfo(info));
    Isolate* isolate = info.GetIsolate();
    HandleScope handle_scope(isolate);
    ...
    std::shared_ptr<BackingStore> backing_store = info[0].As<ArrayBuffer>()->GetBackingStore();
    ValueDeserializer deserializer(isolate, static_cast<const uint8_t*>(backing_store->Data()), backing_store->ByteLength());
    bool ok;
    if (!deserializer.ReadHeader(context).To(&ok))
        return;
    Local<Value> result;
...
void Shell::ProfilerSetOnProfileEndListener(const v8::FunctionCallbackInfo<v8::Value>& info)
{
    DCHECK(i::ValidateCallbackInfo(info));
    Isolate* isolate = info.GetIsolate();
    HandleScope handle_scope(isolate);
    ...
    profiler_end_callback_[isolate] = std::make_pair(Global<Function>(isolate, info[0].As<Function>()), Global<Context>(isolate, isolate->GetCurrentContext()));
}
...
void Shell::ResetOnProfileEndListener(Isolate* isolate)
{
    ...
    i::Isolate* i_isolate = reinterpret_cast<i::Isolate*>(isolate);
    D8Console* console = reinterpret_cast<D8Console*>(i_isolate->console_delegate());
    if (console) {
        console->DisposeProfiler();
    }
}

void Shell::ProfilerTriggerSample(const v8::FunctionCallbackInfo<v8::Value>& info)
{
    Isolate* isolate = info.GetIsolate();
    i::Isolate* i_isolate = reinterpret_cast<i::Isolate*>(isolate);
    D8Console* console = reinterpret_cast<D8Console*>(i_isolate->console_delegate());
...
void Shell::TriggerOnProfileEndListener(Isolate* isolate, std::string profile)
{
    ...
    Local<Context> context;
    Local<Value> argv[1] = { String::NewFromUtf8(isolate, profile.c_str()).ToLocalChecked() };
    {
        base::MutexGuard lock_guard(&profiler_end_callback_lock_);
...
void WriteToFile(FILE* file, const v8::FunctionCallbackInfo<v8::Value>& info, int first_arg_index = 0)
{
    for (int i = first_arg_index; i < info.Length(); i++) {
        ...
        if (!arg->ToString(info.GetIsolate()->GetCurrentContext()).ToLocal(&str_obj)) {
            try_catch.ReThrow();
            return;
        }
...
void Shell::WriteFile(const v8::FunctionCallbackInfo<v8::Value>& info)
{
    ...
    FILE* file;
    if (info.Length() == 2 && (info[1]->IsArrayBuffer() || info[1]->IsArrayBufferView())) {
        ...
        if (info[1]->IsArrayBuffer()) {
            ...
            Local<v8::ArrayBufferView> buffer_view = Local<v8::ArrayBufferView>::Cast(info[1]);
            length = buffer_view->ByteLength();
            data = static_cast<uint8_t*>(buffer_view->Buffer()->Data()) + buffer_view->ByteOffset();
        }
...
void Shell::ReadFile(const v8::FunctionCallbackInfo<v8::Value>& info)
{
    ...
    Local<String> source;
    if (!ReadFile(info.GetIsolate(), *file_name).ToLocal(&source))
        return;
...
Local<String> Shell::ReadFromStdin(Isolate* isolate)
{
    ...
    while (true) {
        ...
        if (length == 0) {
            ...
            accumulator = String::Concat(isolate, accumulator, String::NewFromUtf8(isolate, buffer, NewStringType::kNormal, length).ToLocalChecked());
        } else if (length > 1 && buffer[length - 2] == '\\') {
            buffer[length - 2] = '\n';
            accumulator = String::Concat(isolate, accumulator, String::NewFromUtf8(isolate, buffer, NewStringType::kNormal, length - 1).ToLocalChecked());
        } else {
            return String::Concat(isolate, accumulator, String::NewFromUtf8(isolate, buffer, NewStringType::kNormal, length - 1).ToLocalChecked());
        }
    }
}
...
void Shell::ExecuteFile(const v8::FunctionCallbackInfo<v8::Value>& info)
{
    ...
    for (int i = 0; i < info.Length(); i++) {
        ...
        if (*file_name == nullptr) {
            ...
            ThrowError(isolate, String::NewFromUtf8(isolate, oss.str().c_str()).ToLocalChecked());
            return;
        }
        Local<String> source;
        if (!ReadFile(isolate, *file_name).ToLocal(&source))
            return;
        if (!ExecuteString(info.GetIsolate(), source, String::NewFromUtf8(isolate, *file_name).ToLocalChecked(),
...
public:
    ...
    void Run() override
    {
        ...
        Context::Scope context_scope(context);
        MaybeLocal<Value> result = callback->Call(context, Undefined(isolate_), 0, nullptr);
        USE(result);
    }
...
void Shell::SetTimeout(const v8::FunctionCallbackInfo<v8::Value>& info)
{
    ...
    Local<Context> context = isolate->GetCurrentContext();
    g_platform->GetForegroundTaskRunner(isolate)->PostTask(std::make_unique<SetTimeoutTask>(isolate, context, callback));
}

#ifdef V8_ENABLE_CONTINUATION_PRESERVED_EMBEDDER_DATA
void Shell::GetContinuationPreservedEmbedderData(const v8::FunctionCallbackInfo<v8::Value>& info)
{
    info.GetReturnValue().Set(info.GetIsolate()->GetContinuationPreservedEmbedderData());
}
#endif // V8_ENABLE_CONTINUATION_PRESERVED_EMBEDDER_DATA

void Shell::GetExtrasBindingObject(const v8::FunctionCallbackInfo<v8::Value>& info)
{
    Local<Context> context = info.GetIsolate()->GetCurrentContext();
    info.GetReturnValue().Set(context->GetExtrasBindingObject());
}

void Shell::ReadCodeTypeAndArguments(const v8::FunctionCallbackInfo<v8::Value>& info, int index, CodeType* code_type, Local<Value>* arguments)
{
    Isolate* isolate = info.GetIsolate();
    if (info.Length() > index && info[index]->IsObject()) {
        ...
        Local<String> worker_type_string = value->ToString(context).ToLocalChecked();
        ...
        if (arguments != nullptr) {
            bool got_arguments = TryGetValue(isolate, context, object, "arguments").ToLocal(arguments);
            USE(got_arguments);
        }
    } else {
        *code_type = CodeType::kNone;
...
bool Shell::FunctionAndArgumentsToString(Local<Function> function, Local<Value> arguments, Local<String>* source, Isolate* isolate)
{
    Local<Context> context = isolate->GetCurrentContext();
    MaybeLocal<String> maybe_function_string = function->FunctionProtoToString(context);
...
MaybeLocal<String> Shell::ReadSource(const v8::FunctionCallbackInfo<v8::Value>& info, int index, CodeType default_type)
{
    ...
    case CodeType::kFunction:
        ...
        if (!FunctionAndArgumentsToString(info[index].As<Function>(), arguments, &source, isolate)) {
            return MaybeLocal<String>();
        }
        break;
...
void Shell::WorkerNew(const v8::FunctionCallbackInfo<v8::Value>& info)
{
...

    {
        ...
        i::ParkedMutexGuard lock_guard(reinterpret_cast<i::Isolate*>(isolate)->main_thread_local_isolate(), workers_mutex_.Pointer());
        ...
        i::Handle<i::Object> managed = i::Managed<Worker>::From(i_isolate, kWorkerSizeEstimate, worker);
        info.This()->SetInternalField(0, Utils::ToLocal(managed));
        base::Thread::Priority priority = options.apply_priority ? base::Thread::Priority::kUserBlocking : base::Thread::Priority::kDefault;
...
void Shell::WorkerPostMessage(const v8::FunctionCallbackInfo<v8::Value>& info)
{
    ...
    std::shared_ptr<Worker> worker = GetWorkerFromInternalField(isolate, info.This());
    if (!worker.get()) {
        return;
    }

    Local<Value> message = info[0];
    Local<Value> transfer = info.Length() >= 2 ? info[1] : Undefined(isolate).As<Value>();
...
public:
    ...
    void Run() override
    {
        ...
        if (Shell::DeserializeValue(isolate_, std::move(data_)).ToLocal(&value)) {
            ...
            event->CreateDataProperty(context, String::NewFromUtf8Literal(isolate_, "data", NewStringType::kInternalized), value).ToChecked();
...
public:
    ...
    void Run() override
    {
        ...
        // executes, which will
        auto callback_pair = PerIsolateData::Get(isolate_)->GetWorkerOnMessage(worker_);
        ...
        if (callback_pair.second.IsEmpty())
            return;
        ...
        while ((result = worker_->TryGetMessage())) {
            ...
            g_platform->GetForegroundTaskRunner(isolate_)->PostTask(
                std::make_unique<OnMessageFromWorkerTask>(isolate_, callback_pair.first, callback_pair.second, std::move(result)));
        }
    }
...
void Shell::WorkerOnMessageGetter(const v8::FunctionCallbackInfo<v8::Value>& info)
{
    DCHECK(i::ValidateCallbackInfo(info));
    Isolate* isolate = info.GetIsolate();
    HandleScope handle_scope(isolate);
    ...
    Local<Function> callback = PerIsolateData::Get(isolate)->GetWorkerOnMessage(worker).second;
...
void Shell::WorkerOnMessageSetter(const v8::FunctionCallbackInfo<v8::Value>& info)
{
    DCHECK(i::ValidateCallbackInfo(info));
    Isolate* isolate = info.GetIsolate();
    HandleScope handle_scope(isolate);
    ...
    PerIsolateData::Get(isolate)->SubscribeWorkerOnMessage(worker, isolate->GetCurrentContext(), Local<Function>::Cast(callback));
}
...
void Shell::WorkerTerminateAndWait(const v8::FunctionCallbackInfo<v8::Value>& info)
{
    DCHECK(i::ValidateCallbackInfo(info));
    Isolate* isolate = info.GetIsolate();
    HandleScope handle_scope(isolate);
    ...
    reinterpret_cast<i::Isolate*>(isolate)->main_thread_local_isolate()->ExecuteMainThreadWhileParked(
        [worker](const i::ParkedScope& parked) { worker->TerminateAndWaitForThread(parked); });
}
...
void Shell::QuitOnce(v8::FunctionCallbackInfo<v8::Value>* info)
{
    ...
    if (i_isolate->is_shared_space_isolate()) {
        i_isolate->main_thread_local_isolate()->ExecuteMainThreadWhileParked([](const i::ParkedScope& parked) { WaitForRunningWorkers(parked); });
    }
...
{
    DCHECK(i::ValidateCallbackInfo(info));
    base::CallOnce(&quit_once_, &QuitOnce, const_cast<v8::FunctionCallbackInfo<v8::Value>*>(&info));
}

void Shell::Version(const v8::FunctionCallbackInfo<v8::Value>& info)
{
    DCHECK(i::ValidateCallbackInfo(info));
    info.GetReturnValue().Set(String::NewFromUtf8(info.GetIsolate(), V8::GetVersion()).ToLocalChecked());
}

void Shell::ReportException(Isolate* isolate, Local<v8::Message> message, Local<v8::Value> exception_obj)
{
    HandleScope handle_scope(isolate);
    Local<Context> context = isolate->GetCurrentContext();
    ...
    if (v8::TryCatch::StackTrace(context, exception_obj).ToLocal(&stack_trace_string) && stack_trace_string->IsString()) {
...
void Shell::ReportException(v8::Isolate* isolate, const v8::TryCatch& try_catch)
{
    if (isolate->IsExecutionTerminating()) {
...
{
    counters_file_ = base::OS::MemoryMappedFile::create(name, sizeof(CounterCollection), &local_counters_);
...
Local<String> Shell::Stringify(Isolate* isolate, Local<Value> value)
{
    ...
    if (stringify_function_.IsEmpty()) {
        Local<String> source = String::NewFromUtf8(isolate, stringify_source_).ToLocalChecked();
        ...
        stringify_function_.Reset(isolate, script->Run(context).ToLocalChecked().As<Function>());
    }
...
{
    return FunctionTemplate::New(isolate, nullptr, Local<Value>(), Local<Signature>(), 0, ConstructorBehavior::kAllow, SideEffectType::kHasSideEffect, nullptr,
        static_cast<uint16_t>(instance_type));
}

Local<FunctionTemplate> Shell::CreateEventTargetTemplate(Isolate* isolate)
{
    Local<FunctionTemplate> event_target = NewDOMFunctionTemplate(isolate, JSApiInstanceType::kEventTarget);
    return event_target;
}

Local<FunctionTemplate> Shell::CreateNodeTemplates(Isolate* isolate, Local<FunctionTemplate> event_target)
{
    Local<FunctionTemplate> node = NewDOMFunctionTemplate(isolate, JSApiInstanceType::kNode);
    ...
    Local<FunctionTemplate> nodeType = FunctionTemplate::New(isolate, NodeTypeCallback, Local<Value>(), signature, 0, ConstructorBehavior::kThrow,
        SideEffectType::kHasSideEffect, nullptr, static_cast<uint16_t>(JSApiInstanceType::kGenericApiObject),
        static_cast<uint16_t>(JSApiInstanceType::kElement), static_cast<uint16_t>(JSApiInstanceType::kHTMLDivElement));
    nodeType->SetAcceptAnyReceiver(false);
    proto_template->SetAccessorProperty(String::NewFromUtf8Literal(isolate, "nodeType"), nodeType);

    Local<FunctionTemplate> element = NewDOMFunctionTemplate(isolate, JSApiInstanceType::kElement);
    element->Inherit(node);

    Local<FunctionTemplate> html_element = NewDOMFunctionTemplate(isolate, JSApiInstanceType::kHTMLElement);
    html_element->Inherit(element);

    Local<FunctionTemplate> div_element = NewDOMFunctionTemplate(isolate, JSApiInstanceType::kHTMLDivElement);
...
Local<ObjectTemplate> Shell::CreateGlobalTemplate(Isolate* isolate)
{
    ...
    global_template->Set(Symbol::GetToStringTag(isolate), String::NewFromUtf8Literal(isolate, "global"));
    global_template->Set(isolate, "version", FunctionTemplate::New(isolate, Version));

    global_template->Set(isolate, "print", FunctionTemplate::New(isolate, Print));
    global_template->Set(isolate, "printErr", FunctionTemplate::New(isolate, PrintErr));
    global_template->Set(isolate, "write", FunctionTemplate::New(isolate, WriteStdout));
    if (!i::v8_flags.fuzzing) {
        global_template->Set(isolate, "writeFile", FunctionTemplate::New(isolate, WriteFile));
    }
    global_template->Set(isolate, "read", FunctionTemplate::New(isolate, ReadFile));
    global_template->Set(isolate, "readbuffer", FunctionTemplate::New(isolate, ReadBuffer));
    global_template->Set(isolate, "readline", FunctionTemplate::New(isolate, ReadLine));
    global_template->Set(isolate, "load", FunctionTemplate::New(isolate, ExecuteFile));
    global_template->Set(isolate, "setTimeout", FunctionTemplate::New(isolate, SetTimeout));
    ...
    global_template->Set(isolate, "testRunner", Shell::CreateTestRunnerTemplate(isolate));
    global_template->Set(isolate, "Realm", Shell::CreateRealmTemplate(isolate));
    global_template->Set(isolate, "performance", Shell::CreatePerformanceTemplate(isolate));
    ...
    if (i::v8_flags.expose_async_hooks) {
        global_template->Set(isolate, "async_hooks", Shell::CreateAsyncHookTemplate(isolate));
    }
...
Local<ObjectTemplate> Shell::CreateOSTemplate(Isolate* isolate)
{
    ...
    os_template->Set(isolate, "name", v8::String::NewFromUtf8Literal(isolate, V8_TARGET_OS_STRING), PropertyAttribute::ReadOnly);
    os_template->Set(isolate, "d8Path", v8::String::NewFromUtf8(isolate, options.d8_path).ToLocalChecked(), PropertyAttribute::ReadOnly);
    return os_template;
}
...
Local<FunctionTemplate> Shell::CreateWorkerTemplate(Isolate* isolate)
{
    ...
    worker_fun_template->SetClassName(String::NewFromUtf8Literal(isolate, "Worker"));
    worker_fun_template->ReadOnlyPrototype();
    worker_fun_template->PrototypeTemplate()->Set(isolate, "terminate", FunctionTemplate::New(isolate, WorkerTerminate, Local<Value>(), worker_signature));
    worker_fun_template->PrototypeTemplate()->Set(
        isolate, "terminateAndWait", FunctionTemplate::New(isolate, WorkerTerminateAndWait, Local<Value>(), worker_signature));
    worker_fun_template->PrototypeTemplate()->Set(isolate, "postMessage", FunctionTemplate::New(isolate, WorkerPostMessage, Local<Value>(), worker_signature));
    worker_fun_template->PrototypeTemplate()->Set(isolate, "getMessage", FunctionTemplate::New(isolate, WorkerGetMessage, Local<Value>(), worker_signature));
    worker_fun_template->PrototypeTemplate()->SetAccessorProperty(String::NewFromUtf8(isolate, "onmessage", NewStringType::kInternalized).ToLocalChecked(),
        FunctionTemplate::New(isolate, WorkerOnMessageGetter, Local<Value>(), worker_signature),
        FunctionTemplate::New(isolate, WorkerOnMessageSetter, Local<Value>(), worker_signature));
...
Local<ObjectTemplate> Shell::CreateAsyncHookTemplate(Isolate* isolate)
{
    ...
    async_hooks_templ->Set(isolate, "createHook", FunctionTemplate::New(isolate, AsyncHooksCreateHook));
    async_hooks_templ->Set(isolate, "executionAsyncId", FunctionTemplate::New(isolate, AsyncHooksExecutionAsyncId));
    async_hooks_templ->Set(isolate, "triggerAsyncId", FunctionTemplate::New(isolate, AsyncHooksTriggerAsyncId));
...
Local<ObjectTemplate> Shell::CreatePerformanceTemplate(Isolate* isolate)
{
    ...
    performance_template->Set(isolate, "now", FunctionTemplate::New(isolate, PerformanceNow));
    performance_template->Set(isolate, "mark", FunctionTemplate::New(isolate, PerformanceMark));
    performance_template->Set(isolate, "measure", FunctionTemplate::New(isolate, PerformanceMeasure));
    performance_template->Set(isolate, "measureMemory", FunctionTemplate::New(isolate, PerformanceMeasureMemory));
...
Local<ObjectTemplate> Shell::CreateRealmTemplate(Isolate* isolate)
{
    ...
    realm_template->Set(isolate, "current", FunctionTemplate::New(isolate, RealmCurrent));
    realm_template->Set(isolate, "owner", FunctionTemplate::New(isolate, RealmOwner));
    realm_template->Set(isolate, "global", FunctionTemplate::New(isolate, RealmGlobal));
    realm_template->Set(isolate, "create", FunctionTemplate::New(isolate, RealmCreate));
    realm_template->Set(isolate, "createAllowCrossRealmAccess", FunctionTemplate::New(isolate, RealmCreateAllowCrossRealmAccess));
    realm_template->Set(isolate, "navigate", FunctionTemplate::New(isolate, RealmNavigate));
    realm_template->Set(isolate, "detachGlobal", FunctionTemplate::New(isolate, RealmDetachGlobal));
    realm_template->Set(isolate, "dispose", FunctionTemplate::New(isolate, RealmDispose));
    realm_template->Set(isolate, "switch", FunctionTemplate::New(isolate, RealmSwitch));
    realm_template->Set(isolate, "eval", FunctionTemplate::New(isolate, RealmEval));
    realm_template->SetNativeDataProperty(String::NewFromUtf8Literal(isolate, "shared"), RealmSharedGet, RealmSharedSet);
...
Local<ObjectTemplate> Shell::CreateD8Template(Isolate* isolate)
{
    Local<ObjectTemplate> d8_template = ObjectTemplate::New(isolate);
    {
        ...
        file_template->Set(isolate, "execute", FunctionTemplate::New(isolate, Shell::ExecuteFile));
        d8_template->Set(isolate, "file", file_template);
    }
    {
        Local<ObjectTemplate> log_template = ObjectTemplate::New(isolate);
        log_template->Set(isolate, "getAndStop", FunctionTemplate::New(isolate, LogGetAndStop));

        d8_template->Set(isolate, "log", log_template);
    }
    {
        Local<ObjectTemplate> dom_template = ObjectTemplate::New(isolate);
        Local<FunctionTemplate> event_target = Shell::CreateEventTargetTemplate(isolate);
        dom_template->Set(isolate, "EventTarget", event_target);
        dom_template->Set(isolate, "Div", Shell::CreateNodeTemplates(isolate, event_target));
    ...
    }
    {
        ...
        if (!i::v8_flags.correctness_fuzzer_suppressions) {
            test_template->Set(isolate, "verifySourcePositions", FunctionTemplate::New(isolate, TestVerifySourcePositions));
        }
        ...
        if (options.expose_fast_api && i::v8_flags.turbo_fast_api_calls && !i::v8_flags.correctness_fuzzer_suppressions) {
            test_template->Set(isolate, "FastCAPI", Shell::CreateTestFastCApiTemplate(isolate));
            test_template->Set(isolate, "LeafInterfaceType", Shell::CreateLeafInterfaceTypeTemplate(isolate));
        }
        // Allows testing code paths that are triggered when Origin Trials are
        // added in the browser.
        test_template->Set(isolate, "installConditionalFeatures", FunctionTemplate::New(isolate, Shell::InstallConditionalFeatures));

        // Enable JavaScript Promise Integration at runtime, to simulate
        // Origin Trial behavior.
        test_template->Set(isolate, "enableJSPI", FunctionTemplate::New(isolate, Shell::EnableJSPI));

        d8_template->Set(isolate, "test", test_template);
    }
    {
        Local<ObjectTemplate> promise_template = ObjectTemplate::New(isolate);
        promise_template->Set(isolate, "setHooks", FunctionTemplate::New(isolate, SetPromiseHooks, Local<Value>(), Local<Signature>(), 4));
        d8_template->Set(isolate, "promise", promise_template);
    }
    {
        Local<ObjectTemplate> debugger_template = ObjectTemplate::New(isolate);
        debugger_template->Set(isolate, "enable", FunctionTemplate::New(isolate, EnableDebugger, Local<Value>(), Local<Signature>(), 0));
        debugger_template->Set(isolate, "disable", FunctionTemplate::New(isolate, DisableDebugger, Local<Value>(), Local<Signature>(), 0));
        d8_template->Set(isolate, "debugger", debugger_template);
    }
    {
        Local<ObjectTemplate> serializer_template = ObjectTemplate::New(isolate);
        serializer_template->Set(isolate, "serialize", FunctionTemplate::New(isolate, SerializerSerialize));
        serializer_template->Set(isolate, "deserialize", FunctionTemplate::New(isolate, SerializerDeserialize, Local<Value>(), Local<Signature>(), 1));
        d8_template->Set(isolate, "serializer", serializer_template);
    }
    {
        Local<ObjectTemplate> profiler_template = ObjectTemplate::New(isolate);
        profiler_template->Set(isolate, "setOnProfileEndListener", FunctionTemplate::New(isolate, ProfilerSetOnProfileEndListener));
        profiler_template->Set(isolate, "triggerSample", FunctionTemplate::New(isolate, ProfilerTriggerSample));
        d8_template->Set(isolate, "profiler", profiler_template);
    }
#ifdef V8_ENABLE_CONTINUATION_PRESERVED_EMBEDDER_DATA
    d8_template->Set(isolate, "getContinuationPreservedEmbedderDataViaAPIForTesting", FunctionTemplate::New(isolate, GetContinuationPreservedEmbedderData));
#endif // V8_ENABLE_CONTINUATION_PRESERVED_EMBEDDER_DATA
    d8_template->Set(isolate, "terminate", FunctionTemplate::New(isolate, Terminate));
    d8_template->Set(isolate, "getExtrasBindingObject", FunctionTemplate::New(isolate, GetExtrasBindingObject));
    if (!options.omit_quit) {
...
{
    if (options.ignore_unhandled_promises)
        return;
    ...
    i::Isolate* i_isolate = reinterpret_cast<i::Isolate*>(isolate);
    bool capture_exceptions = i_isolate->get_capture_stack_trace_for_uncaught_exceptions();
...
void Shell::Initialize(Isolate* isolate, D8Console* console, bool isOnMainThread)
{
    isolate->SetPromiseRejectCallback(PromiseRejectCallback);
    isolate->SetWasmAsyncResolvePromiseCallback(D8WasmAsyncResolvePromiseCallback);
    if (isOnMainThread) {
        // Set up counters
    ...
    isolate->SetHostImportModuleWithPhaseDynamicallyCallback(Shell::HostImportModuleWithPhaseDynamically);
    isolate->SetHostInitializeImportMetaObjectCallback(Shell::HostInitializeImportMetaObject);
    isolate->SetHostCreateShadowRealmContextCallback(Shell::HostCreateShadowRealmContext);

    debug::SetConsoleDelegate(isolate, console);
}

Local<String> Shell::WasmLoadSourceMapCallback(Isolate* isolate, const char* path)
{
...
MaybeLocal<Context> Shell::CreateEvaluationContext(Isolate* isolate)
{
    ...
    i::ParkedMutexGuard lock_guard(reinterpret_cast<i::Isolate*>(isolate)->main_thread_local_isolate(), context_mutex_.Pointer());
    ...
    if (i::v8_flags.perf_prof_annotate_wasm || i::v8_flags.vtune_prof_annotate_wasm) {
    ...
    if (options.include_arguments) {
        ...
        for (int i = 0; i < size; i++) {
            Local<String> arg = v8::String::NewFromUtf8(isolate, args[i]).ToLocalChecked();
            Local<Number> index = v8::Number::New(isolate, i);
            array->Set(context, index, arg).FromJust();
        }
        Local<String> name = String::NewFromUtf8Literal(isolate, "arguments", NewStringType::kInternalized);
        context->Global()->Set(context, name, array).FromJust();
    }
    {
        // setup console global.
        Local<String> name = String::NewFromUtf8Literal(isolate, "console", NewStringType::kInternalized);
        Local<Value> console = context->GetExtrasBindingObject()->Get(context, name).ToLocalChecked();
...
void Shell::WriteIgnitionDispatchCountersFile(v8::Isolate* isolate)
{
    ...
    i::Handle<i::JSObject> dispatch_counters = reinterpret_cast<i::Isolate*>(isolate)->interpreter()->GetDispatchCountersObject();
    std::ofstream dispatch_counters_stream(i::v8_flags.trace_ignition_dispatches_output_file);
    dispatch_counters_stream << *String::Utf8Value(isolate, JSON::Stringify(context, Utils::ToLocal(dispatch_counters)).ToLocalChecked());
}
...
void WriteLcovDataForRange(std::vector<uint32_t>* lines, int start_line, int end_line, uint32_t count)
{
...
void WriteLcovDataForNamedRange(std::ostream& sink, std::vector<uint32_t>* lines, const std::string& name, int start_line, int end_line, uint32_t count)
{
    ...
    for (size_t i = 0; i < coverage.ScriptCount(); i++) {
        ...
        for (size_t j = 0; j < script_data.FunctionCount(); j++) {
            ...
            // Write function stats.
            {
                ...
                WriteLcovDataForNamedRange(sink, &lines, name_stream.str(), start_line, end_line, count);
            }
...
void Shell::OnExit(v8::Isolate* isolate, bool dispose)
{
    ...
    if (options.dump_counters || options.dump_counters_nvp) {
        base::SharedMutexGuard<base::kShared> mutex_guard(&counter_mutex_);
        std::vector<std::pair<std::string, Counter*>> counters(counter_map_->begin(), counter_map_->end());
        ...
        if (options.dump_counters_nvp) {
            ...
            for (const auto& pair : counters) {
                ...
                Counter* counter = pair.second;
                if (counter->is_histogram()) {
                    std::cout << "| c:" << std::setw(kNameBoxSize - 4) << std::left << key << " | " << std::setw(kValueBoxSize - 2) << std::right
                              << counter->count() << " |\n";
                    std::cout << "| t:" << std::setw(kNameBoxSize - 4) << std::left << key << " | " << std::setw(kValueBoxSize - 2) << std::right
                              << counter->sample_total() << " |\n";
                } else {
                    std::cout << "| " << std::setw(kNameBoxSize - 2) << std::left << key << " | " << std::setw(kValueBoxSize - 2) << std::right
                              << counter->count() << " |\n";
                }
            }
            std::cout << "+" << std::string(kNameBoxSize, '-') << "+" << std::string(kValueBoxSize, '-') << "+\n";
    ...
    if (options.dump_system_memory_stats) {
        ...
        std::cout << "System peak memory usage (kb): " << peak_memory_usage << std::endl;
...
MaybeLocal<PrimitiveArray> Shell::ReadLines(Isolate* isolate, const char* name)
{
    int length;
    ...
    for (int i = 0; i < size; ++i) {
        MaybeLocal<String> maybe_str = v8::String::NewFromUtf8(isolate, lines[i].c_str(), NewStringType::kNormal, static_cast<int>(lines[i].length()));
        Local<String> str;
...
{
    DCHECK(i::ValidateCallbackInfo(info));
    static_assert(sizeof(char) == sizeof(uint8_t), "char and uint8_t should both have 1 byte");
    Isolate* isolate = info.GetIsolate();
...
std::unique_ptr<base::OS::MemoryMappedFile> Shell::ReadFileData(Isolate* isolate, const char* name, bool should_throw)
{
    std::unique_ptr<base::OS::MemoryMappedFile> file(base::OS::MemoryMappedFile::open(name, base::OS::MemoryMappedFile::FileMode::kReadOnly));
    if (!file) {
        if (should_throw) {
            std::ostringstream oss;
            oss << "Error loading file: " << name;
            ThrowError(isolate, v8::String::NewFromUtf8(isolate, oss.str().substr(0, String::kMaxLength).c_str()).ToLocalChecked());
        }
        return nullptr;
    }
    return file;
...
MaybeLocal<String> Shell::ReadFile(Isolate* isolate, const char* name, bool should_throw)
{
...
void Shell::RunShell(Isolate* isolate)
{
    ...
    while (true) {
        ...
        bool success;
        {
            ...
            success = ExecuteString(isolate, input, name, kReportExceptions, &global_result);
            CHECK_EQ(success, !global_result.IsEmpty());
        }
        if (!FinishExecuting(isolate, context))
            success = false;
        if (success) {
            HandleScope scope(isolate);
            Context::Scope context_scope(context.Get(isolate));
...
private:
    ...
    void Send(const v8_inspector::StringView& string)
    {
        ...
        Local<String> message = (string.is8Bit()
                ? v8::String::NewFromOneByte(isolate_, reinterpret_cast<const uint8_t*>(string.characters8()), v8::NewStringType::kNormal, length)
                : v8::String::NewFromTwoByte(isolate_, reinterpret_cast<const uint16_t*>(string.characters16()), v8::NewStringType::kNormal, length))
                                    .ToLocalChecked();
...
public:
    InspectorClient(Isolate* isolate, const Global<Context>& global_context, bool connect)
    {
        ...
        session_ = inspector_->connect(
            1, channel_.get(), v8_inspector::StringView(), v8_inspector::V8Inspector::kFullyTrusted, v8_inspector::V8Inspector::kNotWaitingForDebugger);
    ...
    void runMessageLoopOnPause(int contextGroupId) override
    {
        ...
        i::EmbedderStackStateScope stack_scanning_scope(heap, i::EmbedderStackStateOrigin::kExplicitInvocation, v8::StackState::kMayContainHeapPointers);
...
bool SourceGroup::Execute(Isolate* isolate)
{
    ...
    if (fuzzilli_reprl) {
        HandleScope handle_scope(isolate);
        Local<String> file_name = String::NewFromUtf8(isolate, "fuzzcode.js", NewStringType::kNormal).ToLocalChecked();
        ...
        Local<String> source = String::NewFromUtf8(isolate, buffer, NewStringType::kNormal).ToLocalChecked();
        delete[] buffer;
        Shell::set_script_executed();
        if (!Shell::ExecuteString(isolate, source, file_name, Shell::kReportExceptions)) {
            return false;
        }
    }
#endif // V8_FUZZILLI
    for (int i = begin_offset_; i < end_offset_; ++i) {
        ...
        if (strcmp(arg, "-e") == 0 && i + 1 < end_offset_) {
            ...
            Local<String> source = String::NewFromUtf8(isolate, argv_[i + 1]).ToLocalChecked();
            Shell::set_script_executed();
            if (!Shell::ExecuteString(isolate, source, file_name, Shell::kReportExceptions)) {
                success = false;
                break;
        ...
        HandleScope handle_scope(isolate);
        Local<String> file_name = String::NewFromUtf8(isolate, arg).ToLocalChecked();
        Local<String> source;
...
void SourceGroup::ExecuteInThread()
{
...

    {
        ...
        for (int i = 0; i < Shell::options.stress_runs; ++i) {
            next_semaphore_.ParkedWait(reinterpret_cast<i::Isolate*>(isolate)->main_thread_local_isolate());
            {
                ...
                InspectorClient inspector_client(isolate, global_context, Shell::options.enable_inspector);
                {
...
bool SerializationDataQueue::Dequeue(std::unique_ptr<SerializationData>* out_data)
{
    out_data->reset();
    base::MutexGuard lock_guard(&mutex_);
    if (data_.empty())
...
bool Worker::StartWorkerThread(Isolate* requester, std::shared_ptr<Worker> worker, base::Thread::Priority priority)
{
    auto expected = State::kReady;
    CHECK(worker->state_.compare_exchange_strong(expected, State::kPrepareRunning));
    auto thread = new WorkerThread(worker, priority);
    worker->thread_ = thread;
    if (!thread->Start())
        return false;
    // Wait until the worker is ready to receive messages.
    worker->started_semaphore_.ParkedWait(reinterpret_cast<i::Isolate*>(requester)->main_thread_local_isolate());
...
public:
    ...
    {
        worker_->ProcessMessage(std::move(data_));
    }
...
void Worker::PostMessage(std::unique_ptr<SerializationData> data)
{
    ...
    if (!is_running())
        return;
    std::unique_ptr<v8::Task> task(new ProcessMessageTask(task_manager_, shared_from_this(), std::move(data)));
...
public:
    ...
    void RunInternal() override
    {
        ...
        CHECK(worker_->state_.compare_exchange_strong(expected, Worker::State::kTerminated));
    }
...
std::unique_ptr<SerializationData> Worker::GetMessage(Isolate* requester)
{
    ...
    while (!out_queue_.Dequeue(&result)) {
        ...
        if (!is_running())
            break;
        out_semaphore_.ParkedWait(reinterpret_cast<i::Isolate*>(requester)->main_thread_local_isolate());
    }
    return result;
}
...
void Worker::Terminate()
{
    ...
    if (!state_.compare_exchange_strong(expected, State::kTerminating))
        return;
    std::unique_ptr<v8::Task> task(new TerminateTask(task_manager_, shared_from_this()));
...
void Worker::ProcessMessage(std::unique_ptr<SerializationData> data)
{
    ...
    // Get the message handler.
    MaybeLocal<Value> maybe_onmessage = global->Get(context, String::NewFromUtf8Literal(isolate_, "onmessage", NewStringType::kInternalized));
...
void Worker::ProcessMessages()
{
    ...
    while (is_running() && v8::platform::PumpMessageLoop(g_default_platform, isolate_, platform::MessageLoopBehavior::kWaitForWork)) {
        if (try_catch.HasCaught())
            return;
        if (is_running()) {
...
void Worker::ExecuteInThread()
{
    ...
    task_manager_ = reinterpret_cast<i::Isolate*>(isolate_)->cancelable_task_manager();
        ...
        if (!context_.IsEmpty()) {
            {
                ...
                PerIsolateData::RealmScope realm_scope(isolate_, context_);
                {
                    ...
                    Local<FunctionTemplate> postmessage_fun_template = FunctionTemplate::New(isolate_, Worker::PostMessageOut, this_value);
                    Local<Function> postmessage_fun;
                    if (postmessage_fun_template->GetFunction(context).ToLocal(&postmessage_fun)) {
                        global->Set(context, v8::String::NewFromUtf8Literal(isolate_, "postMessage", NewStringType::kInternalized), postmessage_fun).FromJust();
                    }

                    Local<FunctionTemplate> close_fun_template = FunctionTemplate::New(isolate_, Worker::Close, this_value);
                    Local<Function> close_fun;
                    if (close_fun_template->GetFunction(context).ToLocal(&close_fun)) {
                        global->Set(context, v8::String::NewFromUtf8Literal(isolate_, "close", NewStringType::kInternalized), close_fun).FromJust();
                    }

                    Local<FunctionTemplate> importScripts_fun_template = FunctionTemplate::New(isolate_, Worker::ImportScripts, this_value);
                    Local<Function> importScripts_fun;
                    if (importScripts_fun_template->GetFunction(context).ToLocal(&importScripts_fun)) {
                        global->Set(context, v8::String::NewFromUtf8Literal(isolate_, "importScripts", NewStringType::kInternalized), importScripts_fun)
                            .FromJust();
                    }

                    // First run the script
                    Local<String> file_name = String::NewFromUtf8Literal(isolate_, "unnamed");
                    Local<String> source = String::NewFromUtf8(isolate_, script_).ToLocalChecked();
                    success = Shell::ExecuteString(isolate_, source, file_name, Shell::kReportExceptions);
                }
                if (!Shell::FinishExecuting(isolate_, context_))
                    success = false;
                if (success) {
                    bool handler_present;
                    {
                        ...
                        MaybeLocal<Value> maybe_onmessage
                            = global->Get(context, String::NewFromUtf8Literal(isolate_, "onmessage", NewStringType::kInternalized));
                        Local<Value> onmessage;
                        handler_present = maybe_onmessage.ToLocal(&onmessage) && onmessage->IsFunction();
                    }
                    if (handler_present) {
    ...
    g_platform->GetForegroundTaskRunner(parent_isolate_)->PostTask(std::make_unique<CleanUpWorkerTask>(parent_isolate_, this->shared_from_this()));
}
...
void Worker::PostMessageOut(const v8::FunctionCallbackInfo<v8::Value>& info)
{
    ...
    if (data) {
        ...
        g_platform->GetForegroundTaskRunner(worker->parent_isolate_)
            ->PostTask(std::make_unique<CheckMessageFromWorkerTask>(worker->parent_isolate_, worker->shared_from_this()));
    }
}
...
char* ConvertUtf16StringToUtf8(const wchar_t* str)
{
    ...
    int len = WideCharToMultiByte(CP_UTF8, 0, str, -1, nullptr, 0, nullptr, FALSE);
    DCHECK_LT(0, len);
...
template <size_t N> bool FlagWithArgMatches(const char (&flag)[N], char** flag_value, int argc, char* argv[], int* i)
{
...
bool Shell::SetOptions(int argc, char* argv[])
{
    ...
    for (int i = 0; i < argc; i++) {
        ...
        } else if (FlagMatches("--notest", &argv[i]) || FlagMatches("--no-test", &argv[i])) {
        ...
        } else if (FlagWithArgMatches("--icu-data-file", &flag_value, argc, argv, &i)) {
            options.icu_data_file = flag_value;
        } else if (FlagWithArgMatches("--icu-locale", &flag_value, argc, argv, &i)) {
            options.icu_locale = flag_value;
#ifdef V8_USE_EXTERNAL_STARTUP_DATA
        } else if (FlagWithArgMatches("--snapshot_blob", &flag_value, argc, argv, &i)) {
            options.snapshot_blob = flag_value;
#endif // V8_USE_EXTERNAL_STARTUP_DATA
        } else if (FlagMatches("--cache", &argv[i]) || FlagWithArgMatches("--cache", &flag_value, argc, argv, &i)) {
        ...
        } else if ((FlagMatches("--no-streaming-compile", &argv[i])) || (FlagMatches("--nostreaming-compile", &argv[i]))) {
            options.streaming_compile = false;
        } else if (FlagMatches("--enable-tracing", &argv[i])) {
            options.trace_enabled = true;
        } else if (FlagWithArgMatches("--trace-path", &flag_value, argc, argv, &i)) {
            options.trace_path = flag_value;
        } else if (FlagWithArgMatches("--trace-config", &flag_value, argc, argv, &i)) {
        ...
        } else if (FlagWithArgMatches("--perf-ctl-fd", &flag_value, argc, argv, &i)) {
            options.perf_ctl_fd = atoi(flag_value);
        } else if (FlagWithArgMatches("--perf-ack-fd", &flag_value, argc, argv, &i)) {
...
#ifdef V8_OS_POSIX
        } else if (FlagWithArgMatches("--read-from-tcp-port", &flag_value, argc, argv, &i)) {
        ...
        } else if (FlagWithArgMatches("--thread-pool-size", &flag_value, argc, argv, &i)) {
        ...
        } else if (FlagWithArgMatches("--repeat-compile", &flag_value, argc, argv, &i)) {
            options.repeat_compile = atoi(flag_value);
        } else if (FlagWithArgMatches("--max-serializer-memory", &flag_value, argc, argv, &i)) {
    ...
    i::FlagList::SetFlagsFromCommandLine(&argc, argv, true, HelpOptions(HelpOptions::kExit, usage));
    i::FlagList::ResolveContradictionsWhenFuzzing();

    options.mock_arraybuffer_allocator = i::v8_flags.mock_arraybuffer_allocator;
    options.mock_arraybuffer_allocator_limit = i::v8_flags.mock_arraybuffer_allocator_limit;
#ifdef V8_OS_LINUX
    options.multi_mapped_mock_allocator = i::v8_flags.multi_mapped_mock_allocator;
#endif // V8_OS_LINUX

    if (i::v8_flags.stress_snapshot && options.expose_fast_api && check_d8_flag_contradictions) {
...
int Shell::RunMain(v8::Isolate* isolate, bool last_run)
{
    ...
    i_isolate->main_thread_local_heap()->ExecuteMainThreadWhileParked([last_run](const i::ParkedScope& parked) {
        for (int i = 1; i < options.num_isolates; ++i) {
            if (last_run) {
    ...
    if (success && last_run && i::v8_flags.stress_snapshot) {
        ...
        i_isolate->heap()->CollectAllAvailableGarbage(i::GarbageCollectionReason::kSnapshotCreator);
        i::Snapshot::ClearReconstructableDataForSerialization(i_isolate, kClearRecompilableData);
...
bool Shell::RunMainIsolate(v8::Isolate* isolate, bool keep_context_alive)
{
    ...
    PerIsolateData::RealmScope realm_scope(isolate, global_context);
    InspectorClient inspector_client(isolate, global_context, options.enable_inspector);
    bool success = true;
    {
    ...
    if (!FinishExecuting(isolate, global_context))
        success = false;
...
namespace {
bool ProcessMessages(Isolate* isolate, const std::function<platform::MessageLoopBehavior()>& behavior)
{
    i::Isolate* i_isolate = reinterpret_cast<i::Isolate*>(isolate);
    i::SaveAndSwitchContext saved_context(i_isolate, i::Context());
    ...
    while (true) {
        bool ran_a_task;
        ran_a_task = v8::platform::PumpMessageLoop(g_default_platform, isolate, behavior());
        if (isolate->IsExecutionTerminating())
            return true;
        if (try_catch.HasCaught())
            return false;
        ...
        if (i::v8_flags.verify_predictable) {
            ...
            while (v8::platform::PumpMessageLoop(
                g_default_platform, kProcessGlobalPredictablePlatformWorkerTaskQueue, platform::MessageLoopBehavior::kDoNotWait)) {
                ran_a_task = true;
                if (try_catch.HasCaught())
                    return false;
                if (isolate->IsExecutionTerminating())
    ...
    if (g_default_platform->IdleTasksEnabled(isolate)) {
        v8::platform::RunIdleTasks(g_default_platform, isolate, 50.0 / base::Time::kMillisecondsPerSecond);
        if (try_catch.HasCaught())
            return false;
        if (isolate->IsExecutionTerminating())
            return true;
...
bool Shell::CompleteMessageLoop(Isolate* isolate)
{
    auto get_waiting_behaviour = [isolate]() {
        if (options.wait_for_background_tasks && isolate->HasPendingBackgroundTasks()) {
    ...
    if (i::v8_flags.verify_predictable) {
        bool ran_tasks = ProcessMessages(isolate, [] { return platform::MessageLoopBehavior::kDoNotWait; });
        if (get_waiting_behaviour() == platform::MessageLoopBehavior::kWaitForWork) {
            FATAL("There is outstanding work after executing all tasks in predictable "
...
{
    return ProcessMessages(isolate, []() { return platform::MessageLoopBehavior::kDoNotWait; });
}
...
bool Shell::HandleUnhandledPromiseRejections(Isolate* isolate)
{
    ...
    Shell::unhandled_promise_rejections_.store(Shell::unhandled_promise_rejections_.load() + count);
    return count == 0;
}
...
public:
    ...
    {
        to->insert(to->end(), std::make_move_iterator(backing_stores_.begin()), std::make_move_iterator(backing_stores_.end()));
...
protected:
    ...
    Maybe<uint32_t> GetSharedArrayBufferId(Isolate* isolate, Local<SharedArrayBuffer> shared_array_buffer) override
    {
        ...
        data_->sab_backing_stores_.push_back(shared_array_buffer->GetBackingStore());
        return Just<uint32_t>(static_cast<uint32_t>(index));
    }
            ...
            for (uint32_t i = 0; i < length; ++i) {
                ...
                if (transfer_array->Get(context, i).ToLocal(&element)) {
                    if (!element->IsArrayBuffer()) {
                        isolate_->ThrowError("Transfer array elements must be an ArrayBuffer");
                        return Nothing<bool>();
                    }

                    Local<ArrayBuffer> array_buffer = element.As<ArrayBuffer>();

                    if (std::find(array_buffers_.begin(), array_buffers_.end(), array_buffer) != array_buffers_.end()) {
                        isolate_->ThrowError("ArrayBuffer occurs in the transfer array more than once");
                        return Nothing<bool>();
                    }

                    serializer_.TransferArrayBuffer(static_cast<uint32_t>(array_buffers_.size()), array_buffer);
    ...
    Maybe<bool> FinalizeTransfer()
    {
        for (const auto& global_array_buffer : array_buffers_) {
            ...
            if (!array_buffer->IsDetachable()) {
                isolate_->ThrowError("ArrayBuffer is not detachable and could not be transferred");
                return Nothing<bool>();
            }
...
public:
    ...
    MaybeLocal<Value> ReadValue(Local<Context> context)
    {
        ...
        for (const auto& backing_store : data_->backing_stores()) {
            Local<ArrayBuffer> array_buffer = ArrayBuffer::New(isolate_, std::move(backing_store));
    ...
    MaybeLocal<SharedArrayBuffer> GetSharedArrayBufferFromId(Isolate* isolate, uint32_t clone_id) override
    {
        ...
        if (clone_id < data_->sab_backing_stores().size()) {
            return SharedArrayBuffer::New(isolate_, std::move(data_->sab_backing_stores().at(clone_id)));
        }
    ...
    {
        DCHECK_NOT_NULL(data_);
        if (transfer_id >= data_->compiled_wasm_modules().size())
            return {};
        return WasmModuleObject::FromCompiledModule(isolate_, data_->compiled_wasm_modules().at(transfer_id));
    }
...
std::unique_ptr<SerializationData> Shell::SerializeValue(Isolate* isolate, Local<Value> value, Local<Value> transfer)
{
    bool ok;
    Local<Context> context = isolate->GetCurrentContext();
...
MaybeLocal<Value> Shell::DeserializeValue(Isolate* isolate, std::unique_ptr<SerializationData> data)
{
    Local<Context> context = isolate->GetCurrentContext();
...
int Shell::Main(int argc, char* argv[])
{
    ...
    if (options.apply_priority) {
        struct task_category_policy category = { .role = TASK_FOREGROUND_APPLICATION };
        task_policy_set(mach_task_self(), TASK_CATEGORY_POLICY, (task_policy_t)&category, TASK_CATEGORY_POLICY_COUNT);
    ...
    v8::platform::InProcessStackDumping in_process_stack_dumping
        = options.disable_in_process_stack_traces ? v8::platform::InProcessStackDumping::kDisabled : v8::platform::InProcessStackDumping::kEnabled;
    ...
    if (options.trace_enabled && !i::v8_flags.verify_predictable) {
        ...
        if (!options.enable_etw_stack_walking) {
            const char* trace_path = options.trace_path ? options.trace_path : "v8_trace.json";
        ...
        if (options.enable_system_instrumentation) {
            trace_buffer = platform::tracing::TraceBuffer::CreateTraceBufferRingBuffer(
                platform::tracing::TraceBuffer::kRingBufferChunks, platform::tracing::TraceWriter::CreateSystemInstrumentationTraceWriter());
        }
#endif // V8_ENABLE_SYSTEM_INSTRUMENTATION
        if (!trace_buffer) {
            trace_buffer = platform::tracing::TraceBuffer::CreateTraceBufferRingBuffer(
                platform::tracing::TraceBuffer::kRingBufferChunks, platform::tracing::TraceWriter::CreateJSONTraceWriter(trace_file));
        }
    ...
    if (i::v8_flags.single_threaded) {
        g_platform = v8::platform::NewSingleThreadedDefaultPlatform(v8::platform::IdleTaskSupport::kEnabled, in_process_stack_dumping, std::move(tracing));
    } else {
        g_platform = v8::platform::NewDefaultPlatform(options.thread_pool_size, v8::platform::IdleTaskSupport::kEnabled, in_process_stack_dumping,
            std::move(tracing), options.apply_priority ? v8::platform::PriorityMode::kApply : v8::platform::PriorityMode::kDontApply);
    }
    ...
    if (options.stress_delay_tasks) {
        ...
        if (!random_seed)
            random_seed = i::v8_flags.random_seed;
    ...
    MockArrayBufferAllocatiorWithLimit mock_arraybuffer_allocator_with_limit(
        memory_limit >= options.mock_arraybuffer_allocator_limit ? memory_limit : std::numeric_limits<size_t>::max());
#ifdef V8_OS_LINUX
    ...
    create_params.constraints.ConfigureDefaults(base::SysInfo::AmountOfPhysicalMemory(), base::SysInfo::AmountOfVirtualMemory());
    ...
    if (options.fuzzilli_enable_builtins_coverage) {
        cov_init_builtins_edges(static_cast<uint32_t>(i::BasicBlockProfiler::Get()->GetCoverageBitmap(reinterpret_cast<i::Isolate*>(isolate)).size()));
    }
    char helo[] = "HELO";
...

    {
        ...
        do {
            ...
            if (options.cpu_profiler) {
                ...
                cpu_profiler->StartProfiling(String::Empty(isolate), CpuProfilingOptions {});
            }

            if (i::v8_flags.stress_runs > 0) {
                options.stress_runs = i::v8_flags.stress_runs;
                for (int i = 0; i < options.stress_runs && result == 0; i++) {
                    printf("============ Run %d/%d ============\n", i + 1, options.stress_runs.get());
                ...
                reinterpret_cast<i::Isolate*>(isolate)->main_thread_local_isolate()->ExecuteMainThreadWhileParked([&result]() {
                ...
                options.compile_options.Overwrite(v8::ScriptCompiler::kConsumeCodeCache);
                ...
                options.compile_options.Overwrite(v8::ScriptCompiler::kNoCompileOptions);
            } else {
                bool last_run = true;
                result = RunMain(isolate, last_run);
            }
            ...
            if (options.cpu_profiler) {
                ...
                if (options.cpu_profiler_print) {
                    const internal::ProfileNode* root = reinterpret_cast<const internal::ProfileNode*>(profile->GetTopDownRoot());
                    root->Print(0);
                }
                profile->Delete();
            ...
            if (fuzzilli_reprl) {
                ...
                if (options.fuzzilli_enable_builtins_coverage) {
                    bitmap = i::BasicBlockProfiler::Get()->GetCoverageBitmap(reinterpret_cast<i::Isolate*>(isolate));
                ...
                if (options.fuzzilli_coverage_statistics) {
                    ...
                    covlog << iteration_counter << "\t" << tot << "\t" << sanitizer_cov_count_discovered_edges() << "\t" << bitmap.size() << std::endl;
                    iteration_counter++;
                }
                ...
                if (options.fuzzilli_enable_builtins_coverage) {
                    i::BasicBlockProfiler::Get()->ResetCounts(reinterpret_cast<i::Isolate*>(isolate));
                }
            }
#endif // V8_FUZZILLI
