...
static bool WaitOnFD(int fd, int read_timeout, int total_timeout, const struct timeval& start_time)
{
    ...
    int number_of_fds_ready = select(fd + 1, &readfds, &writefds, &exceptfds, read_timeout != -1 ? &timeout : nullptr);
...
public:
    ...
    bool Init(Isolate* isolate, Local<Value> arg0, Local<Array> command_args)
    {
        ...
        for (unsigned j = 0; j < command_args->Length(); i++, j++) {
            Local<Value> arg(command_args->Get(isolate->GetCurrentContext(), Integer::New(isolate, j)).ToLocalChecked());
            String::Utf8Value utf8_arg(isolate, arg);
            if (*utf8_arg == nullptr) {
                exec_args_[i] = nullptr; // Consistent state for destructor.
                isolate->ThrowError("os.system(): String conversion of argument failed.");
                return false;
            }
...
static Local<Value> GetStdout(Isolate* isolate, int child_fd, const struct timeval& start_time, int read_timeout, int total_timeout)
{
    ...
    int bytes_read;
    do {
        bytes_read = static_cast<int>(read(child_fd, buffer + fullness, kStdoutReadBufferSize - fullness));
        if (bytes_read == -1) {
            if (errno == EAGAIN) {
                if (!WaitOnFD(child_fd, read_timeout, total_timeout, start_time) || (TimeIsOut(start_time, total_timeout))) {
        ...
        if (bytes_read + fullness > 0) {
            ...
            Local<String> addition = String::NewFromUtf8(isolate, buffer, NewStringType::kNormal, length).ToLocalChecked();
...
static bool WaitForChild(Isolate* isolate, int pid, ZombieProtector& child_waiter, const struct timeval& start_time, int read_timeout, int total_timeout)
{
    ...
    if (child_info.si_code == CLD_KILLED) {
        char message[999];
        snprintf(message, sizeof(message), "Child killed by signal %d", child_info.si_status);
        isolate->ThrowError(message);
        return false;
    }
    if (child_info.si_code == CLD_EXITED && child_info.si_status != 0) {
        char message[999];
        snprintf(message, sizeof(message), "Child exited with status %d", child_info.si_status);
        isolate->ThrowError(message);
        return false;
    }
    ...
    if (WIFSIGNALED(child_status)) {
        char message[999];
        snprintf(message, sizeof(message), "Child killed by signal %d", WTERMSIG(child_status));
        isolate->ThrowError(message);
        return false;
    }
    if (WEXITSTATUS(child_status) != 0) {
        ...
        snprintf(message, sizeof(message), "Child exited with status %d", exit_status);
        isolate->ThrowError(message);
        return false;
    }
...
void Shell::System(const v8::FunctionCallbackInfo<v8::Value>& info)
{
    ...
    int read_timeout = -1;
    int total_timeout = -1;
    if (!GetTimeouts(info, &read_timeout, &total_timeout))
        return;
    ...
    Local<Value> accumulator = GetStdout(isolate, stdout_fds[kReadFD], start_time, read_timeout, total_timeout);
    ...
    if (!WaitForChild(isolate, pid, child_waiter, start_time, read_timeout, total_timeout)) {
        return;
    }
...
void Shell::ChangeDirectory(const v8::FunctionCallbackInfo<v8::Value>& info)
{
    ...
    if (*directory == nullptr) {
        info.GetIsolate()->ThrowError("os.chdir(): String conversion of argument failed.");
        return;
    }
...
void Shell::SetUMask(const v8::FunctionCallbackInfo<v8::Value>& info)
{
    ...
    if (info[0]->IsNumber()) {
        int previous = umask(info[0]->Int32Value(info.GetIsolate()->GetCurrentContext()).FromJust());
...
void Shell::MakeDirectory(const v8::FunctionCallbackInfo<v8::Value>& info)
{
    ...
    if (info.Length() == 2) {
        if (info[1]->IsNumber()) {
            mask = info[1]->Int32Value(info.GetIsolate()->GetCurrentContext()).FromJust();
        } else {
    ...
    String::Utf8Value directory(info.GetIsolate(), info[0]);
    if (*directory == nullptr) {
        info.GetIsolate()->ThrowError("os.mkdirp(): String conversion of argument failed.");
        return;
    }
...
void Shell::RemoveDirectory(const v8::FunctionCallbackInfo<v8::Value>& info)
{
    ...
    String::Utf8Value directory(info.GetIsolate(), info[0]);
    if (*directory == nullptr) {
        info.GetIsolate()->ThrowError("os.rmdir(): String conversion of argument failed.");
        return;
    }
    rmdir(*directory);
}
...
void Shell::SetEnvironment(const v8::FunctionCallbackInfo<v8::Value>& info)
{
    ...
    if (*var == nullptr) {
        info.GetIsolate()->ThrowError("os.setenv(): String conversion of variable name failed.");
        return;
    }
    if (*value == nullptr) {
        info.GetIsolate()->ThrowError("os.setenv(): String conversion of variable contents failed.");
        return;
    }
    setenv(*var, *value, 1);
}
...
void Shell::AddOSMethods(Isolate* isolate, Local<ObjectTemplate> os_templ)
{
    ...
    os_templ->Set(isolate, "chdir", FunctionTemplate::New(isolate, ChangeDirectory));
    os_templ->Set(isolate, "setenv", FunctionTemplate::New(isolate, SetEnvironment));
    os_templ->Set(isolate, "unsetenv", FunctionTemplate::New(isolate, UnsetEnvironment));
    os_templ->Set(isolate, "umask", FunctionTemplate::New(isolate, SetUMask));
    os_templ->Set(isolate, "mkdirp", FunctionTemplate::New(isolate, MakeDirectory));
    os_templ->Set(isolate, "rmdir", FunctionTemplate::New(isolate, RemoveDirectory));
}
