...
public:
    ...
    void PostTaskOnWorkerThreadImpl(TaskPriority priority, std::unique_ptr<Task> task, const SourceLocation& location) override
    {
        ...
        platform_->GetForegroundTaskRunner(kProcessGlobalPredictablePlatformWorkerTaskQueue, priority)->PostTask(std::move(task));
    }
    ...
    std::unique_ptr<JobHandle> CreateJobImpl(TaskPriority priority, std::unique_ptr<JobTask> job_task, const SourceLocation& location) override
    {
        ...
        return platform::NewDefaultJobHandle(this, priority, std::move(job_task), NumberOfWorkerThreads());
    }
    ...
    double MonotonicallyIncreasingTime() override
    {
        ...
        // increments here.
        uint64_t synthetic_time = synthetic_time_.fetch_add(1, std::memory_order_relaxed);
...
public:
    ...
    std::shared_ptr<TaskRunner> GetForegroundTaskRunner(v8::Isolate* isolate, TaskPriority priority) override
    {
        std::shared_ptr<TaskRunner> runner = platform_->GetForegroundTaskRunner(isolate, priority);
    ...
    void PostDelayedTaskOnWorkerThreadImpl(TaskPriority priority, std::unique_ptr<Task> task, double delay_in_seconds, const SourceLocation& location) override
    {
        platform_->CallDelayedOnWorkerThread(MakeDelayedTask(std::move(task)), delay_in_seconds, location);
    }
    ...
    std::unique_ptr<JobHandle> CreateJobImpl(TaskPriority priority, std::unique_ptr<JobTask> job_task, const SourceLocation& location) override
    {
        return platform_->CreateJob(priority, MakeDelayedJob(std::move(job_task)), location);
    }
...
private:
    ...
    public:
        ...
        {
            return task_runner_->IdleTasksEnabled();
        }
    ...
    private:
        ...
        {
            task_runner_->PostTask(platform_->MakeDelayedTask(std::move(task)), location);
        }

        void PostNonNestableTaskImpl(std::unique_ptr<Task> task, const SourceLocation& location) final
        {
            task_runner_->PostNonNestableTask(platform_->MakeDelayedTask(std::move(task)), location);
        }

        void PostDelayedTaskImpl(std::unique_ptr<Task> task, double delay_in_seconds, const SourceLocation& location) final
        {
            task_runner_->PostDelayedTask(platform_->MakeDelayedTask(std::move(task)), delay_in_seconds, location);
        }

        void PostIdleTaskImpl(std::unique_ptr<IdleTask> task, const SourceLocation& location) final
        {
            task_runner_->PostIdleTask(platform_->MakeDelayedIdleTask(std::move(task)), location);
        }
    ...
    {
        return std::make_unique<DelayedJob>(std::move(task), GetRandomDelayInMilliseconds());
    }
};
