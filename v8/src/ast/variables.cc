...
{
    return scope()->is_repl_mode_scope() && (mode() == VariableMode::kLet || mode() == VariableMode::kConst);
}
...
void Variable::RewriteLocationForRepl()
{
    ...
    if (mode() == VariableMode::kLet || mode() == VariableMode::kConst) {
        ...
        bit_field_ = LocationField::update(bit_field_, VariableLocation::REPL_GLOBAL);
    }
}

void Variable::AssignHoleCheckBitmapIndex(ZoneVector<Variable*>& list, uint8_t next_index)
{
    ...
    hole_check_analysis_bit_field_ = HoleCheckBitmapIndexField::update(hole_check_analysis_bit_field_, next_index);
    list.push_back(this);
}
