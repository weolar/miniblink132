...
SourceRangeAstVisitor::SourceRangeAstVisitor(uintptr_t stack_limit, Expression* root, SourceRangeMap* source_range_map)
...
void SourceRangeAstVisitor::VisitTryFinallyStatement(TryFinallyStatement* stmt)
{
...
void SourceRangeAstVisitor::MaybeRemoveContinuationRange(Statement* last_statement)
{
    ...
    if (last_statement->IsExpressionStatement() && last_statement->AsExpressionStatement()->expression()->IsThrow()) {
        ...
        // ExpressionStatement.
        last_range = source_range_map_->Find(last_statement->AsExpressionStatement()->expression());
    } else {
...
void SourceRangeAstVisitor::MaybeRemoveLastContinuationRange(ZonePtrList<Statement>* statements)
{
...
Statement* FindLastNonSyntheticStatement(ZonePtrList<Statement>* statements)
{
    for (int i = statements->length() - 1; i >= 0; --i) {
        ...
        if (stmt->IsReturnStatement() && stmt->AsReturnStatement()->is_synthetic_async_return()) {
            continue;
        }
        return stmt;
    }
...
void SourceRangeAstVisitor::MaybeRemoveContinuationRangeOfAsyncReturn(TryCatchStatement* try_catch_stmt)
{
