...
namespace base {
template <> struct PointerWithPayloadTraits<v8::internal::Scope> {
...
using UnresolvedList = base::ThreadedList<VariableProxy, VariableProxy::UnresolvedNext>;
...
public:
    ...
    Variable* Declare(Zone* zone, Scope* scope, const AstRawString* name, VariableMode mode, VariableKind kind, InitializationFlag initialization_flag,
...
public:
    ...
    static Scope* DeserializeScopeChain(IsolateT* isolate, Zone* zone, Tagged<ScopeInfo> scope_info, DeclarationScope* script_scope,
        AstValueFactory* ast_value_factory, DeserializationMode deserialization_mode, ParseInfo* info = nullptr);

    template <typename IsolateT> EXPORT_TEMPLATE_DECLARE(V8_EXPORT_PRIVATE) static void SetScriptScopeInfo(IsolateT* isolate, DeclarationScope* script_scope);
    ...
    Variable* DeclareVariable(Declaration* declaration, const AstRawString* name, int pos, VariableMode mode, VariableKind kind, InitializationFlag init,
    ...
    void RecordInnerScopeEvalCall()
    {
        ...
        for (Scope* scope = outer_scope(); scope != nullptr; scope = scope->outer_scope()) {
    ...
    {
        return scope_type_ == CLASS_SCOPE;
    }
    bool is_home_object_scope() const
    {
        return is_class_scope() || (is_block_scope() && is_block_scope_for_object_literal_);
    }
    ...
    //   });
    // }
    template <typename FunctionType> V8_INLINE void ForEach(FunctionType callback);
    enum Iteration {
    ...
    bool HasContextExtensionSlot() const
    {
        ...
        case REPL_MODE_SCOPE:
            return true;
        default:
            DCHECK_IMPLIES(sloppy_eval_can_extend_vars_, scope_type_ == FUNCTION_SCOPE || scope_type_ == EVAL_SCOPE || scope_type_ == BLOCK_SCOPE);
    ...
    {
        return scope_type_ == REPL_MODE_SCOPE;
    }
...
private:
    Variable* Declare(Zone* zone, const AstRawString* name, VariableMode mode, VariableKind kind, InitializationFlag initialization_flag,
        MaybeAssignedFlag maybe_assigned_flag, bool* was_added)
    {
        // Static variables can only be declared using ClassScope methods.
        Variable* result = variables_.Declare(zone, this, name, mode, kind, initialization_flag, maybe_assigned_flag, IsStaticFlag::kNotStatic, was_added);
        if (mode == VariableMode::kUsing)
            has_using_declaration_ = true;
        if (mode == VariableMode::kAwaitUsing)
            has_await_using_declaration_ = true;
        if (*was_added)
            locals_.Add(result);
        return result;
    }
    ...
    void AllocateScopeInfosRecursively(
        IsolateT* isolate, MaybeHandle<ScopeInfo> outer_scope, std::unordered_map<int, IndirectHandle<ScopeInfo>>& scope_infos_to_reuse);
...
public:
    ...
    bool is_being_lazily_parsed() const
    {
    ...
    void MakeParametersNonSimple()
    {
        ...
        for (ZoneHashMap::Entry* p = variables_.Start(); p != nullptr; p = variables_.Next(p)) {
    ...
    CreateArgumentsType GetArgumentsType() const
    {
        ...
        return is_sloppy(language_mode()) && has_simple_parameters() ? CreateArgumentsType::kMappedArguments : CreateArgumentsType::kUnmappedArguments;
    }
    ...
    Variable* this_function_var() const
    {
        ...
        DCHECK(this_function == nullptr
            || (is_function_scope() && (IsClassConstructor(function_kind()) || IsConciseMethod(function_kind()) || IsAccessorFunction(function_kind()))));
    ...
    template <typename IsolateT> V8_EXPORT_PRIVATE static void AllocateScopeInfos(ParseInfo* info, DirectHandle<Script> script, IsolateT* isolate);
    ...
    {
        has_this_reference_ = true;
    }
    bool has_this_reference() const
    {
        return has_this_reference_;
    }
    void UsesThis()
    {
...
private:
    ...
    V8_INLINE Variable* GetRareVariable(RareVariable id) const
    {
        if (rare_data_ == nullptr)
            return nullptr;
        return *reinterpret_cast<Variable**>(reinterpret_cast<uint8_t*>(rare_data_) + static_cast<ptrdiff_t>(id));
    }

    // Set `var` to null if it's non-null and Predicate (Variable*) -> bool
    // returns true.
    template <typename Predicate> V8_INLINE void NullifyRareVariableIf(RareVariable id, Predicate predicate)
    {
        if (V8_LIKELY(rare_data_ == nullptr))
            return;
        Variable** var = reinterpret_cast<Variable**>(reinterpret_cast<uint8_t*>(rare_data_) + static_cast<ptrdiff_t>(id));
...
Scope::Snapshot::Snapshot(Scope* scope)
    ...
    , sloppy_eval_can_extend_vars_(declaration_scope_->sloppy_eval_can_extend_vars_)
{
...
public:
    ...
    // Deserialization.
    template <typename IsolateT> ClassScope(IsolateT* isolate, Zone* zone, AstValueFactory* ast_value_factory, Handle<ScopeInfo> scope_info);
    ...
    struct HeritageParsingScope {
        ...
        {
            class_scope_->SetIsParsingHeritage(false);
        }
    ...
    {
        return should_save_class_variable_index_ || has_explicit_static_private_methods_access_ || (has_static_private_methods_ && inner_scope_calls_eval_);
    }

    // Only maintained when the scope is parsed.
    bool is_anonymous_class() const
    {
        return is_anonymous_class_;
    }
...
private:
    ...
    V8_INLINE RareData* EnsureRareData()
    {
        if (GetRareData() == nullptr) {
            rare_data_and_is_parsing_heritage_.SetPointer(zone()->New<RareData>(zone()));
        }
        return GetRareData();
    }
