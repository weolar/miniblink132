...
{
    if (negative_)
        return std::string("-") + std::to_string(absolute_value_);
...
inline IntegerLiteral operator<<(const IntegerLiteral& x, const IntegerLiteral& y)
{
    ...
    return IntegerLiteral(x.is_negative(), x.absolute_value() << y.absolute_value());
}
...
inline IntegerLiteral operator+(const IntegerLiteral& x, const IntegerLiteral& y)
{
    ...
    return IntegerLiteral(!x.is_negative(), y.absolute_value() - x.absolute_value());
}
