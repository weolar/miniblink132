...
public:
    enum Kind { kEmpty, kI32Const, kRefNull, kRefFunc, kWireBytesRef, kLastKind = kWireBytesRef };

    constexpr ConstantExpression() = default;

    static constexpr ConstantExpression I32Const(int32_t value)
    {
        return ConstantExpression(ValueField::encode(value) | KindField::encode(kI32Const));
    }
    static constexpr ConstantExpression RefFunc(uint32_t index)
    {
        return ConstantExpression(ValueField::encode(index) | KindField::encode(kRefFunc));
    }
    static constexpr ConstantExpression RefNull(HeapType::Representation repr)
    {
        return ConstantExpression(ValueField::encode(repr) | KindField::encode(kRefNull));
    }
    static constexpr ConstantExpression WireBytes(uint32_t offset, uint32_t length)
    {
        return ConstantExpression(OffsetField::encode(offset) | LengthField::encode(length) | KindField::encode(kWireBytesRef));
    }
    ...
    constexpr HeapType::Representation repr() const
    {
        ...
        return static_cast<HeapType::Representation>(ValueField::decode(bit_field_));
    }
