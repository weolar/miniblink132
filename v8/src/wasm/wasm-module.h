...
{
    const uintptr_t platform_max_pages = memory->is_memory64() ? kV8MaxWasmMemory64Pages : kV8MaxWasmMemory32Pages;
    memory->min_memory_size = std::min(platform_max_pages, uintptr_t { memory->initial_pages }) * kWasmPageSize;
    memory->max_memory_size = std::min(platform_max_pages, uintptr_t { memory->maximum_pages }) * kWasmPageSize;
...
struct WasmStringRefLiteral {
    explicit WasmStringRefLiteral(const WireBytesRef& source)
        : source(source)
    {
    }
...
struct WasmDataSegment {
    explicit WasmDataSegment(bool is_active, bool is_shared, uint32_t memory_index, ConstantExpression dest_addr, WireBytesRef source)
        : active(is_active)
        , shared(is_shared)
...
struct WasmElemSegment {
    ...
    WasmElemSegment(bool shared, ValueType type, uint32_t table_index, ConstantExpression offset, ElementType element_type, uint32_t element_count,
...
#define SELECT_WASM_COUNTER(counters, origin, prefix, suffix)                                                                                                  \
    ((origin) == kWasmOrigin ? (counters)->prefix##_wasm_##suffix() : (counters)->prefix##_asm_##suffix())
...
template <class Value> class AdaptiveMap {
public:
...
public:
    ...
    CallSiteFeedback(int function_index, int call_count)
        : index_or_count_(function_index)
        , frequency_or_ool_(call_count)
    {
    }
...
private:
    ...
    {
        return index_or_count_ >= 0;
    }
    bool is_polymorphic() const
    {
        return index_or_count_ <= -2;
    }
    bool is_invalid() const
    {
        return index_or_count_ == -1;
    }
...
struct V8_EXPORT_PRIVATE WasmModule {
    ...
    void AddTypeForTesting(TypeDefinition type)
    {
        ...
        if (type.supertype.valid()) {
            ...
            types.back().subtyping_depth = this->type(type.supertype).subtyping_depth + 1;
        }
    ...
    {
        return index.index < types.size() && types[index.index].kind == TypeDefinition::kFunction;
    }
    ...
    {
        return index.index < types.size() && types[index.index].kind == TypeDefinition::kStruct;
    }
    ...
    {
        return index.index < types.size() && types[index.index].kind == TypeDefinition::kArray;
    }
    ...
    CanonicalTypeIndex MaxCanonicalTypeIndex() const
    {
        ...
        return *std::max_element(isorecursive_canonical_type_ids.begin(), isorecursive_canonical_type_ids.end());
    }
    ...
    bool function_was_validated(int func_index) const
    {
        ...
        uint8_t byte = validated_functions[pos >> 3].load(std::memory_order_relaxed);
    ...
    void set_function_validated(int func_index) const
    {
        ...
        while ((old_byte & new_bit) == 0 && !atomic_byte->compare_exchange_weak(old_byte, old_byte | new_bit, std::memory_order_relaxed)) {
...
#if V8_ENABLE_DRUMBRAKE
    void SetWasmInterpreter(std::shared_ptr<WasmInterpreterRuntime> interpreter) const
    {
...
V8_EXPORT_PRIVATE int GetSubtypingDepth(const WasmModule* module, ModuleTypeIndex type_index);
...
struct V8_EXPORT_PRIVATE ModuleWireBytes {
    ...
    {
        return module_bytes_.SubVector(function->code.offset(), function->code.end_offset());
    }

    base::Vector<const uint8_t> module_bytes() const
    {
        return module_bytes_;
    }
...
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, const WasmFunctionName& name);

V8_EXPORT_PRIVATE bool IsWasmCodegenAllowed(Isolate* isolate, Handle<NativeContext> context);
V8_EXPORT_PRIVATE DirectHandle<String> ErrorStringForCodegen(Isolate* isolate, DirectHandle<Context> context);

template <typename T> Handle<JSObject> GetTypeForFunction(Isolate* isolate, const Signature<T>* sig, bool for_exception = false);
...
Handle<JSArray> GetImports(Isolate* isolate, DirectHandle<WasmModuleObject> module);
Handle<JSArray> GetExports(Isolate* isolate, DirectHandle<WasmModuleObject> module);
Handle<JSArray> GetCustomSections(Isolate* isolate, DirectHandle<WasmModuleObject> module, DirectHandle<String> name, ErrorThrower* thrower);

// Get the source position from a given function index and byte offset,
// for either asm.js or pure Wasm modules.
int GetSourcePosition(const WasmModule*, uint32_t func_index, uint32_t byte_offset, bool is_at_number_conversion);
...
template <int kMaxLen = 50> class TruncatedUserString {
...
V8_EXPORT_PRIVATE size_t GetWireBytesHash(base::Vector<const uint8_t> wire_bytes);
