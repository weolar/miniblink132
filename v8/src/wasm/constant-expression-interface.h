...
public:
    ...
    using FullDecoder = WasmFullDecoder<ValidationTag, ConstantExpressionInterface, decoding_mode>;

    ConstantExpressionInterface(const WasmModule* module, Isolate* isolate, Handle<WasmTrustedInstanceData> trusted_instance_data,
...
#define DECLARE_INTERFACE_FUNCTION(name, ...) void name(FullDecoder* decoder, ##__VA_ARGS__);
...
#undef DECLARE_INTERFACE_FUNCTION
    ...
    {
        return error_ != MessageTemplate::kNone;
    }
...
private:
    ...
    {
        return isolate_ != nullptr && !has_error();
    }
    Handle<WasmTrustedInstanceData> GetTrustedInstanceDataForTypeIndex(ModuleTypeIndex index);
