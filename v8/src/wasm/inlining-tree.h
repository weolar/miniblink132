...
public:
    ...
    {
        InliningTree* tree = zone->New<InliningTree>(zone->New<Data>(zone, module, function_index), function_index,
            0, // Call count.
    ...
    static int NoLiftoffBudget(const WasmModule* module, uint32_t func_index)
    {
        ...
        int high_growth = static_cast<int>(v8_flags.wasm_inlining_factor) + kTurboshaftAdjustment;
        ...
        return std::max(static_cast<int>(v8_flags.wasm_inlining_min_budget), static_cast<int>(max_growth_factor * wirebytes));
    }
    ...
    int64_t score() const
    {
        ...
        return int64_t { call_count_ } * count_factor - int64_t { wire_byte_size_ } * size_factor;
    }
...
private:
    ...
    struct Data {
            ...
            , topmost_caller_index(topmost_caller_index)
        {
            ...
            double high_cap = v8_flags.wasm_inlining_budget * kTurboshaftCorrectionFactor;
    ...
    InliningTree(
        Data* shared, uint32_t function_index, int call_count, int wire_byte_size, uint32_t caller_index, int feedback_slot, int the_case, uint32_t depth)
        : data_(shared)
...
{
    is_inlined_ = true;
    auto feedback = data_->module->type_feedback.feedback_for_function.find(function_index_);
    if (feedback != data_->module->type_feedback.feedback_for_function.end()
        && feedback->second.feedback_vector.size() == feedback->second.call_targets.size()) {
        std::vector<CallSiteFeedback>& type_feedback = feedback->second.feedback_vector;
        feedback_found_ = true;
        function_calls_ = data_->zone->AllocateVector<CasesPerCallSite>(type_feedback.size());
        has_non_inlineable_targets_ = data_->zone->AllocateVector<bool>(type_feedback.size());
        for (size_t i = 0; i < type_feedback.size(); i++) {
            function_calls_[i] = data_->zone->AllocateVector<InliningTree*>(type_feedback[i].num_cases());
            has_non_inlineable_targets_[i] = type_feedback[i].has_non_inlineable_targets();
            for (int the_case = 0; the_case < type_feedback[i].num_cases(); the_case++) {
                ...
                function_calls_[i][the_case] = data_->zone->New<InliningTree>(data_, callee_index, type_feedback[i].call_count(the_case),
                    data_->module->functions[callee_index].code.length(), function_index_, static_cast<int>(i), the_case, depth_ + 1);
            }
        }
    }
}
...
struct TreeNodeOrdering {
    bool operator()(InliningTree* t1, InliningTree* t2)
    {
        ...
        return std::make_pair(t1->score(), t2->function_index()) < std::make_pair(t2->score(), t1->function_index());
    }
};
...
void InliningTree::FullyExpand()
{
    ...
    size_t initial_wire_byte_size = data_->module->functions[function_index_].code.length();
    size_t inlined_wire_byte_count = 0;
    std::priority_queue<InliningTree*, std::vector<InliningTree*>, TreeNodeOrdering> queue;
    queue.push(this);
    int inlined_count = 0;
    base::SharedMutexGuard<base::kShared> mutex_guard(&data_->module->type_feedback.mutex);
    while (!queue.empty() && inlined_count < kMaxInlinedCount) {
        InliningTree* top = queue.top();
        if (v8_flags.trace_wasm_inlining) {
            if (top != this) {
                PrintF("[function %d: in function %d, considering call #%d, case #%d, to "
                       "function %d (count=%d, size=%d, score=%lld)... ",
                    data_->topmost_caller_index, top->caller_index_, top->feedback_slot_, static_cast<int>(top->case_), static_cast<int>(top->function_index_),
                    top->call_count_, top->wire_byte_size_, static_cast<long long>(top->score()));
            } else {
                PrintF("[function %d: expanding topmost caller... ", data_->topmost_caller_index);
            }
        }
        queue.pop();
        ...
        if (top != this && top->wire_byte_size_ >= 12 && !v8_flags.wasm_inlining_ignore_call_counts) {
        ...
        if (!top->SmallEnoughToInline(initial_wire_byte_size, inlined_wire_byte_count)) {
            if (v8_flags.trace_wasm_inlining && top != this) {
...
bool InliningTree::SmallEnoughToInline(size_t initial_wire_byte_size, size_t inlined_wire_byte_count)
{
    ...
    // inlining.
    size_t budget_small_function = std::max<size_t>(v8_flags.wasm_inlining_min_budget, data_->max_growth_factor * initial_wire_byte_size);
    ...
    size_t budget_large_function = std::max<size_t>(data_->budget_cap, initial_wire_byte_size * 1.1);
    size_t total_size = initial_wire_byte_size + inlined_wire_byte_count + static_cast<size_t>(wire_byte_size_);
    if (v8_flags.trace_wasm_inlining) {
        PrintF("budget=min(%zu, %zu), size %zu->%zu ", budget_small_function, budget_large_function, (initial_wire_byte_size + inlined_wire_byte_count),
            total_size);
    }
    return total_size < std::min<size_t>(budget_small_function, budget_large_function);
}
