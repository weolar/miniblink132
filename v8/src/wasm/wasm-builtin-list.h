...
{
    std::array<uint8_t, static_cast<int>(Builtin::kFirstBytecodeHandler)> result {};
    uint8_t next_index = 0;
#define DEF_INIT_LOOKUP(NAME) result[static_cast<int>(Builtin::k##NAME)] = next_index++;
...
#define BUILTIN_COUNTER(NAME) +1
    static constexpr int kBuiltinCount = 0 WASM_BUILTIN_LIST(BUILTIN_COUNTER, BUILTIN_COUNTER);
#undef BUILTIN_COUNTER

    static constexpr auto kFarJumpTableIndexToBuiltin = base::make_array<static_cast<int>(kBuiltinCount)>([](size_t index) {
        size_t next_index = 0;
...
#undef DEF_INIT_LOOKUP
    ...
    static constexpr auto kBuiltinToFarJumpTableIndex = detail::InitBuiltinToFarJumpTableIndex();
};
