...
public:
    ...
    explicit ZoneBuffer(Zone* zone, size_t initial = kInitialSize)
        : zone_(zone)
        , buffer_(zone->AllocateArray<uint8_t, Buffer>(initial))
    {
        pos_ = buffer_;
    ...
    {
        write_u32(base::bit_cast<uint32_t>(val));
    }

    void write_f64(double val)
    {
        write_u64(base::bit_cast<uint64_t>(val));
    }
...
public:
    ...
    private:
        ...
        {
            return kind == WasmInitExpr::kI32Const || kind == WasmInitExpr::kI64Const || kind == WasmInitExpr::kGlobalGet
    ...
    void EndRecursiveTypeGroup()
    {
        ...
        recursive_groups_.emplace(current_recursive_group_start_, static_cast<uint32_t>(types_.size()) - current_recursive_group_start_);
    ...
    {
        return IsSignature(index.index);
    }
    ...
    {
        return IsStructType(index.index);
    }
    ...
    {
        return IsArrayType(index.index);
    }
    ...
    int NumDataSegments()
    {
        return static_cast<int>(data_segments_.size());
    }

    bool IsMemory64(uint32_t index)
    {
        return memories_[index].is_memory64();
    }

    bool IsTable64(uint32_t index)
    {
        return tables_[index].is_table64();
    }
