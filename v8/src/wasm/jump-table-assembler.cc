...
void JumpTableAssembler::GenerateLazyCompileTable(
    AccountingAllocator* allocator, Address base, uint32_t num_slots, uint32_t num_imported_functions, Address wasm_compile_lazy_target)
{
    uint32_t lazy_compile_table_size = num_slots * kLazyCompileTableSlotSize;
    WritableJitAllocation jit_allocation = ThreadIsolation::LookupJitAllocation(
        base, RoundUp<kCodeAlignment>(lazy_compile_table_size), ThreadIsolation::JitAllocationType::kWasmLazyCompileTable);
    ...
    for (uint32_t slot_index = 0; slot_index < num_slots; ++slot_index) {
        ...
        jtasm.EmitLazyCompileJumpSlot(slot_index + num_imported_functions, wasm_compile_lazy_target);
    }
    DCHECK_EQ(lazy_compile_table_size, jtasm.pc_offset());
    FlushInstructionCache(base, lazy_compile_table_size);
}

void JumpTableAssembler::InitializeJumpsToLazyCompileTable(AccountingAllocator* allocator, Address base, uint32_t num_slots, Address lazy_compile_table_start)
{
    uint32_t jump_table_size = SizeForNumberOfSlots(num_slots);
    WritableJitAllocation jit_allocation
        = ThreadIsolation::LookupJitAllocation(base, RoundUp<kCodeAlignment>(jump_table_size), ThreadIsolation::JitAllocationType::kWasmJumpTable);
    JumpTableAssembler jtasm(allocator, base, jump_table_size + 256);

    for (uint32_t slot_index = 0; slot_index < num_slots; ++slot_index) {
        // Make sure we write at the correct offset.
        int slot_offset = static_cast<int>(JumpTableAssembler::JumpSlotIndexToOffset(slot_index));

        jtasm.SkipUntil(slot_offset);

        Address target = lazy_compile_table_start + JumpTableAssembler::LazyCompileSlotIndexToOffset(slot_index);
...
template <typename T> void JumpTableAssembler::emit(T value)
{
    base::WriteUnalignedValue<T>(reinterpret_cast<Address>(pc_), value);
    pc_ += sizeof(T);
}

template <typename T> void JumpTableAssembler::emit(T value, RelaxedStoreTag) DISABLE_UBSAN
{
    ...
    DCHECK_EQ(write_start >> kSystemPointerSizeLog2, write_end >> kSystemPointerSizeLog2);
#endif
#endif
    reinterpret_cast<std::atomic<T>*>(pc_)->store(value, std::memory_order_relaxed);
    pc_ += sizeof(T);
}
...
#if V8_TARGET_ARCH_X64
void JumpTableAssembler::EmitLazyCompileJumpSlot(uint32_t func_index, Address lazy_compile_target)
{
    ...
    intptr_t displacement = static_cast<intptr_t>(reinterpret_cast<uint8_t*>(lazy_compile_target) - (pc_ + kLazyCompileTableSlotSize));
...
bool JumpTableAssembler::EmitJumpSlot(Address target)
{
    ...
    intptr_t displacement = static_cast<intptr_t>(reinterpret_cast<uint8_t*>(target) - (pc_ + kIntraSegmentJmpInstrSize));
...
bool JumpTableAssembler::EmitJumpSlot(Address target)
{
    intptr_t displacement = static_cast<intptr_t>(reinterpret_cast<uint8_t*>(target) - (pc_ + kJumpTableSlotSize));
...
void JumpTableAssembler::EmitLazyCompileJumpSlot(uint32_t func_index, Address lazy_compile_target)
{
    ...
    int64_t target_offset = MacroAssembler::CalculateTargetOffset(lazy_compile_target, RelocInfo::NO_INFO, pc_ + 3 * kInstrSize);
    ...
    emit<uint32_t>(inst[3] | Assembler::ImmUncondBranch(base::checked_cast<int32_t>(target_offset)));
}
...
bool JumpTableAssembler::EmitJumpSlot(Address target)
{
    ...
    int64_t target_offset = MacroAssembler::CalculateTargetOffset(target, RelocInfo::NO_INFO, pc_ + kCodeEntryMarkerSize);
    ...
    uint32_t branch_inst = 0x14000000 | Assembler::ImmUncondBranch(base::checked_cast<int32_t>(target_offset));
...
void JumpTableAssembler::PatchFarJumpSlot(Address slot, Address target)
{
    ...
    reinterpret_cast<std::atomic<Address>*>(slot + kTargetOffset)->store(target, std::memory_order_relaxed);
...
void JumpTableAssembler::PatchFarJumpSlot(Address slot, Address target)
{
    ...
    reinterpret_cast<std::atomic<Address>*>(target_addr)->store(target, std::memory_order_relaxed);
}
...
void JumpTableAssembler::EmitLazyCompileJumpSlot(uint32_t func_index, Address lazy_compile_target)
{
    ...
    for (int i = 0; i < nop_bytes; i += kInstrSize)
        nop();
}
...
{
    uint8_t* start = pc_;
    mov(ip, Operand(reinterpret_cast<Address>(start + kFarJumpTableSlotSize - 8))); // 5 instr
