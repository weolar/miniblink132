...
PRINTF_FORMAT(3, 0)
void VPrintFToString(std::string* str, size_t str_offset, const char* format, va_list args)
{
    ...
    for (;; len = base::bits::RoundUpToPowerOfTwo64(len + 1)) {
        ...
        int written = base::VSNPrintF(base::Vector<char>(&str->front() + str_offset, static_cast<int>(len - str_offset)), format, args_copy);
        va_end(args_copy);
        if (written < 0)
...
Handle<Object> ErrorThrower::Reify()
{
    ...
    DirectHandle<String> message = isolate_->factory()->NewStringFromUtf8(base::VectorOf(error_msg_)).ToHandleChecked();
    Reset();
...
{
    if (!error() || isolate_->has_exception())
        return;
