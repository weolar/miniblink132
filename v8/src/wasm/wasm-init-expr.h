...
public:
    ...
    {
        DCHECK(op == kI32Add || op == kI32Sub || op == kI32Mul || op == kI64Add || op == kI64Sub || op == kI64Mul);
    ...
    {
        return RefNullConst(static_cast<HeapType::Representation>(type_index.index));
    }
    ...
    bool operator==(const WasmInitExpr& other) const
    {
        ...
        case kI64Sub:
        case kI64Mul:
            return operands_[0] == other.operands_[0] && operands_[1] == other.operands_[1];
        case kS128Const:
        ...
        case kArrayNewDefault:
            if (immediate().index != other.immediate().index)
                return false;
...
private:
    ...
    explicit WasmInitExpr(Operator kind)
        : kind_(kind)
        , operands_(nullptr)
    {
    }
