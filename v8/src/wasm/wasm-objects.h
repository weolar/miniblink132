...
template <typename CppType> class Managed;
template <typename CppType> class TrustedManaged;
...
public:
    FunctionTargetAndImplicitArg(Isolate* isolate, Handle<WasmTrustedInstanceData> target_instance_data, int target_func_index);
    // The "implicit_arg" will be a WasmTrustedInstanceData or a WasmImportData.
    Handle<TrustedObject> implicit_arg()
    {
        return implicit_arg_;
    }
...
public:
    inline ImportedFunctionEntry(Isolate*, DirectHandle<WasmInstanceObject>, int index);
    ...
    void SetGenericWasmToJs(Isolate*, DirectHandle<JSReceiver> callable, wasm::Suspend suspend, const wasm::CanonicalSig* sig);
    V8_EXPORT_PRIVATE void SetCompiledWasmToJs(
        Isolate*, DirectHandle<JSReceiver> callable, wasm::WasmCode* wasm_to_js_wrapper, wasm::Suspend suspend, const wasm::CanonicalSig* sig);

    // Initialize this entry as a Wasm to Wasm call.
    void SetWasmToWasm(Tagged<WasmTrustedInstanceData> target_instance_object, WasmCodePointer call_target
#if V8_ENABLE_DRUMBRAKE
        ,
...
class WasmModuleObject : public TorqueGeneratedWasmModuleObject<WasmModuleObject, JSObject> {
public:
    inline wasm::NativeModule* native_module() const;
    inline const std::shared_ptr<wasm::NativeModule>& shared_native_module() const;
    ...
    V8_EXPORT_PRIVATE static Handle<WasmModuleObject> New(Isolate* isolate, std::shared_ptr<wasm::NativeModule> native_module, DirectHandle<Script> script);

    // Check whether this module was generated from asm.js source.
    inline bool is_asm_js();

    // Get the module name, if set. Returns an empty handle otherwise.
    static MaybeHandle<String> GetModuleNameOrNull(Isolate*, DirectHandle<WasmModuleObject>);

    // Get the function name of the function identified by the given index.
    // Returns a null handle if the function is unnamed or the name is not a valid
    // UTF-8 string.
    static MaybeHandle<String> GetFunctionNameOrNull(Isolate*, DirectHandle<WasmModuleObject>, uint32_t func_index);
    ...
    static Handle<String> ExtractUtf8StringFromModuleBytes(Isolate*, DirectHandle<WasmModuleObject>, wasm::WireBytesRef, InternalizeString);
    static Handle<String> ExtractUtf8StringFromModuleBytes(Isolate*, base::Vector<const uint8_t> wire_byte, wasm::WireBytesRef, InternalizeString);
...
class WasmTableObject : public TorqueGeneratedWasmTableObject<WasmTableObject, JSObject> {
public:
    ...
    V8_EXPORT_PRIVATE static int Grow(Isolate* isolate, DirectHandle<WasmTableObject> table, uint32_t count, DirectHandle<Object> init_value);

    V8_EXPORT_PRIVATE static Handle<WasmTableObject> New(Isolate* isolate, Handle<WasmTrustedInstanceData> trusted_data, wasm::ValueType type, uint32_t initial,
    ...
    V8_EXPORT_PRIVATE static void AddUse(Isolate* isolate, DirectHandle<WasmTableObject> table, Handle<WasmInstanceObject> instance_object, int table_index);
    ...
    static MaybeHandle<Object> JSToWasmElement(Isolate* isolate, DirectHandle<WasmTableObject> table, Handle<Object> entry, const char** error_message);

    // This function will not handle JS objects; i.e., {entry} needs to be in wasm
    // representation.
    V8_EXPORT_PRIVATE static void Set(Isolate* isolate, DirectHandle<WasmTableObject> table, uint32_t index, DirectHandle<Object> entry);

    V8_EXPORT_PRIVATE static Handle<Object> Get(Isolate* isolate, DirectHandle<WasmTableObject> table, uint32_t index);

    V8_EXPORT_PRIVATE static void Fill(Isolate* isolate, DirectHandle<WasmTableObject> table, uint32_t start, DirectHandle<Object> entry, uint32_t count);

    // TODO(wasm): Unify these three methods into one.
    static void UpdateDispatchTables(Isolate* isolate, DirectHandle<WasmTableObject> table, int entry_index, const wasm::WasmFunction* func,
    ...
    static void UpdateDispatchTables(Isolate* isolate, DirectHandle<WasmTableObject> table, int entry_index, DirectHandle<WasmJSFunction> function);
    static void UpdateDispatchTables(Isolate* isolate, DirectHandle<WasmTableObject> table, int entry_index, DirectHandle<WasmCapiFunction> capi_function);
...
class WasmMemoryObject : public TorqueGeneratedWasmMemoryObject<WasmMemoryObject, JSObject> {
public:
    class BodyDescriptor;
    ...
    V8_EXPORT_PRIVATE static void UseInInstance(Isolate* isolate, DirectHandle<WasmMemoryObject> memory,
        DirectHandle<WasmTrustedInstanceData> trusted_instance_data, Handle<WasmTrustedInstanceData> shared_trusted_instance_data,
        int memory_index_in_instance);
    inline bool has_maximum_pages();

    inline bool is_memory64() const;

    V8_EXPORT_PRIVATE static Handle<WasmMemoryObject> New(Isolate* isolate, Handle<JSArrayBuffer> buffer, int maximum, wasm::AddressType address_type);

    V8_EXPORT_PRIVATE static MaybeHandle<WasmMemoryObject> New(Isolate* isolate, int initial, int maximum, SharedFlag shared, wasm::AddressType address_type);
    ...
    V8_EXPORT_PRIVATE static int32_t Grow(Isolate*, Handle<WasmMemoryObject>, uint32_t pages);
...
class WasmGlobalObject : public TorqueGeneratedWasmGlobalObject<WasmGlobalObject, JSObject> {
public:
    class BodyDescriptor;
    ...
    V8_EXPORT_PRIVATE static MaybeHandle<WasmGlobalObject> New(Isolate* isolate, Handle<WasmTrustedInstanceData> instance_object,
...
public:
    ...
    DECL_PROTECTED_POINTER_ACCESSORS(dispatch_table_for_imports, WasmDispatchTable)
    ...
    DECL_PROTECTED_POINTER_ACCESSORS(managed_native_module, TrustedManaged<wasm::NativeModule>)
    ...
    DECL_PROTECTED_POINTER_ACCESSORS(memory_bases_and_sizes, TrustedFixedAddressArray)
...
#define WASM_TAGGED_INSTANCE_DATA_FIELDS(V)                                                                                                                    \
    ...
    IF_WASM_DRUMBRAKE(V, kImportedFunctionIndicesOffset, "imported_function_indices")                                                                          \
...
#endif // V8_ENABLE_DRUMBRAKE
    static constexpr size_t kTaggedFieldsCount = 16 + kWasmInterpreterAdditionalFields;

    static constexpr std::array<uint16_t, kTaggedFieldsCount> kTaggedFieldOffsets = { WASM_TAGGED_INSTANCE_DATA_FIELDS(WASM_INSTANCE_FIELD_OFFSET) };
    static constexpr std::array<const char*, kTaggedFieldsCount> kTaggedFieldNames = { WASM_TAGGED_INSTANCE_DATA_FIELDS(WASM_INSTANCE_FIELD_NAME) };
    static constexpr std::array<uint16_t, 6> kProtectedFieldOffsets = { WASM_PROTECTED_INSTANCE_DATA_FIELDS(WASM_INSTANCE_FIELD_OFFSET) };
    static constexpr std::array<const char*, 6> kProtectedFieldNames = { WASM_PROTECTED_INSTANCE_DATA_FIELDS(WASM_INSTANCE_FIELD_NAME) };
...
#undef WASM_PROTECTED_INSTANCE_DATA_FIELDS
    ...
    static_assert(kTaggedFieldOffsets.size() == kTaggedFieldNames.size(), "every tagged field offset needs a name");
    static_assert(kProtectedFieldOffsets.size() == kProtectedFieldNames.size(), "every protected field offset needs a name");
    ...
    static Handle<Tuple2> GetOrCreateInterpreterObject(Handle<WasmInstanceObject>);
    static Handle<Tuple2> GetInterpreterObject(Handle<WasmInstanceObject>);
#endif // V8_ENABLE_DRUMBRAKE

    static Handle<WasmTrustedInstanceData> New(Isolate*, DirectHandle<WasmModuleObject>, bool shared);
    ...
    static bool CopyTableEntries(Isolate* isolate, DirectHandle<WasmTrustedInstanceData> trusted_instance_data, uint32_t table_dst_index,
        uint32_t table_src_index, uint32_t dst, uint32_t src, uint32_t count) V8_WARN_UNUSED_RESULT;
    ...
    static std::optional<MessageTemplate> InitTableEntries(Isolate* isolate, Handle<WasmTrustedInstanceData> trusted_instance_data,
        Handle<WasmTrustedInstanceData> shared_trusted_instance_data, uint32_t table_index, uint32_t segment_index, uint32_t dst, uint32_t src,
    ...
    static Handle<WasmFuncRef> GetOrCreateFuncRef(Isolate* isolate, DirectHandle<WasmTrustedInstanceData> trusted_instance_data, int function_index);
    ...
    static void ImportWasmJSFunctionIntoTable(Isolate* isolate, DirectHandle<WasmTrustedInstanceData> trusted_instance_data, int table_index, int entry_index,
...
// user-set properties.
class WasmInstanceObject : public TorqueGeneratedWasmInstanceObject<WasmInstanceObject, JSObject> {
public:
    DECL_TRUSTED_POINTER_ACCESSORS(trusted_data, WasmTrustedInstanceData)
...
class WasmTagObject : public TorqueGeneratedWasmTagObject<WasmTagObject, JSObject> {
public:
    class BodyDescriptor;
    ...
    static Handle<WasmTagObject> New(Isolate* isolate, const wasm::FunctionSig* sig, wasm::CanonicalTypeIndex type_index, DirectHandle<HeapObject> tag,
...
private:
    ...
    void Add(WasmCodePointer call_target, wasm::WasmCode* wrapper_if_known, IsAWrapper contextual_knowledge);
...
public:
    ...
    static constexpr size_t kProtectedOffheapDataOffset = kCapacityOffset + kUInt32Size;
    static constexpr size_t kEntriesOffset = kProtectedOffheapDataOffset + kTaggedSize;
    ...
    static constexpr size_t kEntryPaddingBytes = kEntryPaddingOffset % kTaggedSize;
    ...
    DECL_PROTECTED_POINTER_ACCESSORS(protected_offheap_data, TrustedManaged<WasmDispatchTableData>)
    ...
    // Smi::zero().
    void V8_EXPORT_PRIVATE Set(int index, Tagged<Object> implicit_arg, WasmCodePointer call_target, wasm::CanonicalTypeIndex sig_id,
#if V8_ENABLE_DRUMBRAKE
    ...
    void V8_EXPORT_PRIVATE SetForImport(
    ...
    static V8_EXPORT_PRIVATE V8_WARN_UNUSED_RESULT Handle<WasmDispatchTable> New(Isolate* isolate, int length);
    static V8_WARN_UNUSED_RESULT Handle<WasmDispatchTable> Grow(Isolate*, Handle<WasmDispatchTable>, int new_length);
...
public:
    static Handle<WasmExceptionPackage> New(Isolate* isolate, DirectHandle<WasmExceptionTag> exception_tag, int encoded_size);

    static Handle<WasmExceptionPackage> New(Isolate* isolate, DirectHandle<WasmExceptionTag> exception_tag, DirectHandle<FixedArray> values);
    ...
    static Handle<Object> GetExceptionTag(Isolate* isolate, Handle<WasmExceptionPackage> exception_package);
    static Handle<Object> GetExceptionValues(Isolate* isolate, Handle<WasmExceptionPackage> exception_package);
    ...
    static constexpr int kSize = kHeaderSize + (kTaggedSize * kInObjectFieldCount);
...
void V8_EXPORT_PRIVATE EncodeI32ExceptionValue(DirectHandle<FixedArray> encoded_values, uint32_t* encoded_index, uint32_t value);

void V8_EXPORT_PRIVATE EncodeI64ExceptionValue(DirectHandle<FixedArray> encoded_values, uint32_t* encoded_index, uint64_t value);

void V8_EXPORT_PRIVATE DecodeI32ExceptionValue(DirectHandle<FixedArray> encoded_values, uint32_t* encoded_index, uint32_t* value);

void V8_EXPORT_PRIVATE DecodeI64ExceptionValue(DirectHandle<FixedArray> encoded_values, uint32_t* encoded_index, uint64_t* value);
...
public:
    ...
    V8_EXPORT_PRIVATE static Handle<WasmExportedFunction> New(Isolate* isolate, DirectHandle<WasmTrustedInstanceData> instance_data,
...
public:
    ...
    static Handle<WasmJSFunction> New(Isolate* isolate, const wasm::FunctionSig* sig, Handle<JSReceiver> callable, wasm::Suspend suspend);
...
public:
    ...
    static Handle<WasmCapiFunction> New(Isolate* isolate, Address call_target, DirectHandle<Foreign> embedder_data, wasm::CanonicalTypeIndex sig_index,
...
class WasmFunctionData : public TorqueGeneratedWasmFunctionData<WasmFunctionData, ExposedTrustedObject> {
public:
    ...
    using BodyDescriptor = StackedBodyDescriptor<FixedExposedTrustedObjectBodyDescriptor<WasmFunctionData, kWasmFunctionDataIndirectPointerTag>,
        WithStrongCodePointer<kWrapperCodeOffset>, WithProtectedPointer<kProtectedInternalOffset>>;
...
class WasmExportedFunctionData : public TorqueGeneratedWasmExportedFunctionData<WasmExportedFunctionData, WasmFunctionData> {
public:
    ...
    using BodyDescriptor = StackedBodyDescriptor<SubclassBodyDescriptor<WasmFunctionData::BodyDescriptor, FixedBodyDescriptorFor<WasmExportedFunctionData>>,
        WithProtectedPointer<kProtectedInstanceDataOffset>, WithStrongCodePointer<kCWrapperCodeOffset>>;

    TQ_OBJECT_CONSTRUCTORS(WasmExportedFunctionData)
};

class WasmImportData : public TorqueGeneratedWasmImportData<WasmImportData, TrustedObject> {
public:
    // Dispatched behavior.
    ...
    static void SetImportIndexAsCallOrigin(DirectHandle<WasmImportData> import_data, int entry_index);
    ...
    static void SetIndexInTableAsCallOrigin(DirectHandle<WasmImportData> import_data, int entry_index);
    ...
    static void SetFuncRefAsCallOrigin(DirectHandle<WasmImportData> import_data, DirectHandle<WasmFuncRef> func_ref);

    using BodyDescriptor = StackedBodyDescriptor<FixedBodyDescriptorFor<WasmImportData>, WithProtectedPointer<kProtectedInstanceDataOffset>>;

    TQ_OBJECT_CONSTRUCTORS(WasmImportData)
};

class WasmInternalFunction : public TorqueGeneratedWasmInternalFunction<WasmInternalFunction, ExposedTrustedObject> {
public:
    ...
    V8_EXPORT_PRIVATE static Handle<JSFunction> GetOrCreateExternal(DirectHandle<WasmInternalFunction> internal);
    ...
    using BodyDescriptor = StackedBodyDescriptor<FixedExposedTrustedObjectBodyDescriptor<WasmInternalFunction, kWasmInternalFunctionIndirectPointerTag>,
...
public:
    ...
    using BodyDescriptor
        = StackedBodyDescriptor<FixedBodyDescriptorFor<WasmFuncRef>, WithStrongTrustedPointer<kTrustedInternalOffset, kWasmInternalFunctionIndirectPointerTag>>;
...
class WasmJSFunctionData : public TorqueGeneratedWasmJSFunctionData<WasmJSFunctionData, WasmFunctionData> {
public:
    ...
    DECL_PROTECTED_POINTER_ACCESSORS(protected_offheap_data, TrustedManaged<OffheapData>)
    ...
    using BodyDescriptor = StackedBodyDescriptor<SubclassBodyDescriptor<WasmFunctionData::BodyDescriptor, FixedBodyDescriptorFor<WasmJSFunctionData>>,
...
class WasmCapiFunctionData : public TorqueGeneratedWasmCapiFunctionData<WasmCapiFunctionData, WasmFunctionData> {
public:
    // Prefer to use this convenience wrapper of the Torque-generated
    // {canonical_sig_index()}.
    inline wasm::CanonicalTypeIndex sig_index() const;
    ...
    using BodyDescriptor = SubclassBodyDescriptor<WasmFunctionData::BodyDescriptor, FixedBodyDescriptorFor<WasmCapiFunctionData>>;

    TQ_OBJECT_CONSTRUCTORS(WasmCapiFunctionData)
};

class WasmResumeData : public TorqueGeneratedWasmResumeData<WasmResumeData, HeapObject> {
public:
    using BodyDescriptor = FlexibleBodyDescriptor<WasmResumeData::kStartOfStrongFieldsOffset>;
...
public:
    ...
    V8_EXPORT_PRIVATE static bool SetBreakPoint(DirectHandle<Script>, int* position, DirectHandle<BreakPoint> break_point);
    ...
    // module.
    V8_EXPORT_PRIVATE static void SetInstrumentationBreakpoint(DirectHandle<Script>, DirectHandle<BreakPoint> break_point);
    ...
    // the module.
    V8_EXPORT_PRIVATE static bool SetBreakPointOnFirstBreakableForFunction(DirectHandle<Script>, int function_index, DirectHandle<BreakPoint> break_point);
    ...
    V8_EXPORT_PRIVATE static bool SetBreakPointForFunction(
        DirectHandle<Script>, int function_index, int breakable_offset, DirectHandle<BreakPoint> break_point);
    ...
    V8_EXPORT_PRIVATE static bool ClearBreakPoint(DirectHandle<Script>, int position, DirectHandle<BreakPoint> break_point);

    // Remove a previously set breakpoint by id. If this breakpoint is not found,
    // returns false.
    V8_EXPORT_PRIVATE static bool ClearBreakPointById(DirectHandle<Script>, int breakpoint_id);
    ...
    V8_EXPORT_PRIVATE static bool GetPossibleBreakpoints(
        wasm::NativeModule* native_module, const debug::Location& start, const debug::Location& end, std::vector<debug::BreakLocation>* locations);

    // Return an empty handle if no breakpoint is hit at that location, or a
    // FixedArray with all hit breakpoint objects.
    static MaybeHandle<FixedArray> CheckBreakPoints(Isolate*, DirectHandle<Script>, int position, StackFrameId stack_frame_id);
...
class WasmExceptionTag : public TorqueGeneratedWasmExceptionTag<WasmExceptionTag, Struct> {
public:
    V8_EXPORT_PRIVATE static Handle<WasmExceptionTag> New(Isolate* isolate, int index);
...
class WasmTypeInfo : public TorqueGeneratedWasmTypeInfo<WasmTypeInfo, HeapObject> {
public:
...
protected:
    ...
    // offset.
    static inline Handle<Object> ReadValueAt(Isolate* isolate, DirectHandle<HeapObject> obj, wasm::ValueType type, uint32_t offset);

private:
    template <typename ElementType> static ElementType FromNumber(Tagged<Object> value);
...
public:
    ...
    static inline void EncodeInstanceSizeInMap(int instance_size, Tagged<Map> map);
    ...
    static inline void SetField(Isolate* isolate, Handle<WasmStruct> obj, uint32_t field_index, Handle<Object> value);
...
int WasmStruct::Size(const wasm::StructType* type)
{
    ...
    return std::max(kHeaderSize + static_cast<int>(type->total_fields_size()), Heap::kMinObjectSizeInTaggedWords * kTaggedSize);
}
...
public:
    ...
    static inline Handle<Object> GetElement(Isolate* isolate, DirectHandle<WasmArray> array, uint32_t index);
...
class WasmContinuationObject : public TorqueGeneratedWasmContinuationObject<WasmContinuationObject, HeapObject> {
public:
    static Handle<WasmContinuationObject> New(
        Isolate* isolate, wasm::StackMemory* stack, wasm::JumpBuffer::StackState state, AllocationType allocation_type = AllocationType::kYoung);
    static Handle<WasmContinuationObject> New(Isolate* isolate, wasm::StackMemory* stack, wasm::JumpBuffer::StackState state, DirectHandle<HeapObject> parent,
    ...
    using BodyDescriptor = StackedBodyDescriptor<FixedBodyDescriptorFor<WasmContinuationObject>, WithExternalPointer<kStackOffset, kWasmStackMemoryTag>,
...
class WasmSuspenderObject : public TorqueGeneratedWasmSuspenderObject<WasmSuspenderObject, HeapObject> {
public:
...
class WasmSuspendingObject : public TorqueGeneratedWasmSuspendingObject<WasmSuspendingObject, JSObject> {
public:
    V8_EXPORT_PRIVATE static Handle<WasmSuspendingObject> New(Isolate* isolate, DirectHandle<JSReceiver> callable);
...
public:
    ...
    static_assert(kSize >= WasmStruct::kHeaderSize + (wasm::kMaxStructFieldIndexForImplicitNullCheck + 1) * kSimd128Size);

    Address payload()
    {
        return ptr() + kHeaderSize - kHeapObjectTag;
    }
