...
inline MemOperand GetMemOp(
    LiftoffAssembler* assm, Register addr, Register offset, uintptr_t offset_imm, Register scratch, bool i64_offset = false, unsigned shift_amount = 0)
{
...
inline void StoreToMemory(LiftoffAssembler* assm, MemOperand dst, const LiftoffAssembler::VarState& src, Register scratch1, Register scratch2)
{
    if (src.is_reg()) {
        switch (src.kind()) {
        case kI16:
...
void LiftoffAssembler::PrepareTailCall(int num_callee_stack_params, int stack_param_delta)
{
    ...
    for (int i = slot_count - 1; i >= 0; --i) {
        ...
        StoreU64(scratch, MemOperand(fp, (i - stack_param_delta) * kSystemPointerSize), r0);
    }
...
{
    int frame_size = GetTotalFrameSize() - (V8_EMBEDDED_CONSTANT_POOL_BOOL ? 3 : 2) * kSystemPointerSize;
    // The frame setup builtin also pushes the feedback vector.
    if (feedback_vector_slot) {
        frame_size -= kSystemPointerSize;
    }

    Assembler patching_assembler(AssemblerOptions {}, ExternalAssemblerBuffer(buffer_start_ + offset, kInstrSize + kGap));
...
void LiftoffAssembler::LoadCodeEntrypointViaCodePointer(Register dst, Register src_addr, int32_t offset_imm)
{
    UseScratchRegisterScope temps(this);
    Register scratch = temps.Acquire();
    MemOperand src_op = liftoff::GetMemOp(this, src_addr, no_reg, offset_imm, scratch);
    ...
    uint32_t* protected_store_pc, SkipWriteBarrier skip_write_barrier)
{
    ...
    // not that it is zero!
    CheckPageFlag(dst_addr, ip, MemoryChunk::kPointersFromHereAreInterestingMask, to_condition(kZero), &exit);
    JumpIfSmi(src, &exit);
    CheckPageFlag(src, ip, MemoryChunk::kPointersToHereAreInterestingMask, eq, &exit);
    mov(ip, Operand(offset_imm));
    add(ip, ip, dst_addr);
    if (offset_reg != no_reg) {
        add(ip, ip, offset_reg);
    }
    CallRecordWriteStubSaveRegisters(dst_addr, ip, SaveFPRegsMode::kSave, StubCallMode::kCallWasmRuntimeStub);
    bind(&exit);
}
...
#define ATOMIC_OP(instr)                                                                                                                                       \
    {                                                                                                                                                          \
        ...
        case StoreType::kI32Store8:                                                                                                                            \
        case StoreType::kI64Store8: {                                                                                                                          \
            auto op_func = [&](Register dst, Register lhs, Register rhs) { instr(dst, lhs, rhs); };                                                            \
    ...
    LiftoffRegister result, StoreType type, bool i64_offset)
{
    ...
    case StoreType::kI32Store8:
    case StoreType::kI64Store8: {
        MacroAssembler::AtomicCompareExchange<uint8_t>(dst, expected.gp(), new_value.gp(), result.gp(), r0);
        break;
    }
    case StoreType::kI32Store16:
    case StoreType::kI64Store16: {
        if (is_be) {
            ...
            Register scratch = GetRegisterThatIsNotOneOf(new_value.gp(), expected.gp(), result.gp());
            push(scratch);
            ByteReverseU16(new_value.gp(), new_value.gp(), scratch);
            ByteReverseU16(expected.gp(), expected.gp(), scratch);
            pop(scratch);
            MacroAssembler::AtomicCompareExchange<uint16_t>(dst, expected.gp(), new_value.gp(), result.gp(), r0);
    ...
    case StoreType::kI64Store32: {
        if (is_be) {
            ...
            pop(scratch);
            MacroAssembler::AtomicCompareExchange<uint32_t>(dst, expected.gp(), new_value.gp(), result.gp(), r0);
    ...
    case StoreType::kI64Store: {
        if (is_be) {
            ...
            MacroAssembler::AtomicCompareExchange<uint64_t>(dst, expected.gp(), new_value.gp(), result.gp(), r0);
            ByteReverseU64(result.gp(), result.gp());
            Pop(new_value.gp(), expected.gp());
        } else {
            MacroAssembler::AtomicCompareExchange<uint64_t>(dst, expected.gp(), new_value.gp(), result.gp(), r0);
...
#define UNOP_LIST(V)                                                                                                                                           \
    ...
    V(f32_sqrt, fsqrt, DoubleRegister, DoubleRegister, , , ROUND_F64_TO_F32, , void)                                                                           \
    V(f32_floor, frim, DoubleRegister, DoubleRegister, , , ROUND_F64_TO_F32, true, bool)                                                                       \
    V(f32_ceil, frip, DoubleRegister, DoubleRegister, , , ROUND_F64_TO_F32, true, bool)                                                                        \
    V(f32_trunc, friz, DoubleRegister, DoubleRegister, , , ROUND_F64_TO_F32, true, bool)                                                                       \
    ...
    V(i64_clz, CountLeadingZerosU64, LiftoffRegister, LiftoffRegister, LFR_TO_REG, LFR_TO_REG, USE, , void)                                                    \
    V(i64_ctz, CountTrailingZerosU64, LiftoffRegister, LiftoffRegister, LFR_TO_REG, LFR_TO_REG, USE, , void)                                                   \
    ...
    V(i64_signextend_i8, extsb, LiftoffRegister, LiftoffRegister, LFR_TO_REG, LFR_TO_REG, USE, , void)                                                         \
    V(i64_signextend_i16, extsh, LiftoffRegister, LiftoffRegister, LFR_TO_REG, LFR_TO_REG, USE, , void)                                                        \
    V(i64_signextend_i32, extsw, LiftoffRegister, LiftoffRegister, LFR_TO_REG, LFR_TO_REG, USE, , void)                                                        \
    V(i32_popcnt, Popcnt32, Register, Register, , , USE, true, bool)                                                                                           \
    V(i64_popcnt, Popcnt64, LiftoffRegister, LiftoffRegister, LFR_TO_REG, LFR_TO_REG, USE, true, bool)
...
#define BINOP_LIST(V)                                                                                                                                          \
    V(f32_copysign, CopySignF64, DoubleRegister, DoubleRegister, DoubleRegister, , , , USE, , void)                                                            \
    V(f64_copysign, CopySignF64, DoubleRegister, DoubleRegister, DoubleRegister, , , , USE, , void)                                                            \
    V(f32_min, MinF64, DoubleRegister, DoubleRegister, DoubleRegister, , , , USE, , void)                                                                      \
    V(f32_max, MaxF64, DoubleRegister, DoubleRegister, DoubleRegister, , , , USE, , void)                                                                      \
    V(f64_min, MinF64, DoubleRegister, DoubleRegister, DoubleRegister, , , , USE, , void)                                                                      \
    V(f64_max, MaxF64, DoubleRegister, DoubleRegister, DoubleRegister, , , , USE, , void)                                                                      \
    V(i64_sub, SubS64, LiftoffRegister, LiftoffRegister, LiftoffRegister, LFR_TO_REG, LFR_TO_REG, LFR_TO_REG, USE, , void)                                     \
    V(i64_add, AddS64, LiftoffRegister, LiftoffRegister, LiftoffRegister, LFR_TO_REG, LFR_TO_REG, LFR_TO_REG, USE, , void)                                     \
    V(i64_addi, AddS64, LiftoffRegister, LiftoffRegister, int64_t, LFR_TO_REG, LFR_TO_REG, Operand, USE, , void)                                               \
    ...
    V(i64_mul, MulS64, LiftoffRegister, LiftoffRegister, LiftoffRegister, LFR_TO_REG, LFR_TO_REG, LFR_TO_REG, USE, , void)                                     \
    ...
    V(i64_and, AndU64, LiftoffRegister, LiftoffRegister, LiftoffRegister, LFR_TO_REG, LFR_TO_REG, LFR_TO_REG, USE, , void)                                     \
    V(i64_or, OrU64, LiftoffRegister, LiftoffRegister, LiftoffRegister, LFR_TO_REG, LFR_TO_REG, LFR_TO_REG, USE, , void)                                       \
    V(i64_xor, XorU64, LiftoffRegister, LiftoffRegister, LiftoffRegister, LFR_TO_REG, LFR_TO_REG, LFR_TO_REG, USE, , void)                                     \
    V(i64_andi, AndU64, LiftoffRegister, LiftoffRegister, int32_t, LFR_TO_REG, LFR_TO_REG, Operand, USE, , void)                                               \
    V(i64_ori, OrU64, LiftoffRegister, LiftoffRegister, int32_t, LFR_TO_REG, LFR_TO_REG, Operand, USE, , void)                                                 \
    V(i64_xori, XorU64, LiftoffRegister, LiftoffRegister, int32_t, LFR_TO_REG, LFR_TO_REG, Operand, USE, , void)                                               \
    V(i32_shli, ShiftLeftU32, Register, Register, int32_t, , , INT32_AND_WITH_1F, USE, , void)                                                                 \
    V(i32_sari, ShiftRightS32, Register, Register, int32_t, , , INT32_AND_WITH_1F, USE, , void)                                                                \
    V(i32_shri, ShiftRightU32, Register, Register, int32_t, , , INT32_AND_WITH_1F, USE, , void)                                                                \
    V(i32_shl, ShiftLeftU32, Register, Register, Register, , , REGISTER_AND_WITH_1F, USE, , void)                                                              \
    V(i32_sar, ShiftRightS32, Register, Register, Register, , , REGISTER_AND_WITH_1F, USE, , void)                                                             \
    V(i32_shr, ShiftRightU32, Register, Register, Register, , , REGISTER_AND_WITH_1F, USE, , void)                                                             \
    V(i64_shl, ShiftLeftU64, LiftoffRegister, LiftoffRegister, Register, LFR_TO_REG, LFR_TO_REG, REGISTER_AND_WITH_3F, USE, , void)                            \
    V(i64_sar, ShiftRightS64, LiftoffRegister, LiftoffRegister, Register, LFR_TO_REG, LFR_TO_REG, REGISTER_AND_WITH_3F, USE, , void)                           \
    V(i64_shr, ShiftRightU64, LiftoffRegister, LiftoffRegister, Register, LFR_TO_REG, LFR_TO_REG, REGISTER_AND_WITH_3F, USE, , void)                           \
    V(i64_shli, ShiftLeftU64, LiftoffRegister, LiftoffRegister, int32_t, LFR_TO_REG, LFR_TO_REG, INT32_AND_WITH_3F, USE, , void)                               \
    V(i64_sari, ShiftRightS64, LiftoffRegister, LiftoffRegister, int32_t, LFR_TO_REG, LFR_TO_REG, INT32_AND_WITH_3F, USE, , void)                              \
    V(i64_shri, ShiftRightU64, LiftoffRegister, LiftoffRegister, int32_t, LFR_TO_REG, LFR_TO_REG, INT32_AND_WITH_3F, USE, , void)                              \
    V(f64_add, AddF64, DoubleRegister, DoubleRegister, DoubleRegister, , , , USE, , void)                                                                      \
    V(f64_sub, SubF64, DoubleRegister, DoubleRegister, DoubleRegister, , , , USE, , void)                                                                      \
    V(f64_mul, MulF64, DoubleRegister, DoubleRegister, DoubleRegister, , , , USE, , void)                                                                      \
    V(f64_div, DivF64, DoubleRegister, DoubleRegister, DoubleRegister, , , , USE, , void)                                                                      \
    V(f32_add, AddF32, DoubleRegister, DoubleRegister, DoubleRegister, , , , USE, , void)                                                                      \
    V(f32_sub, SubF32, DoubleRegister, DoubleRegister, DoubleRegister, , , , USE, , void)                                                                      \
    V(f32_mul, MulF32, DoubleRegister, DoubleRegister, DoubleRegister, , , , USE, , void)                                                                      \
    V(f32_div, DivF32, DoubleRegister, DoubleRegister, DoubleRegister, , , , USE, , void)

#define EMIT_BINOP_FUNCTION(name, instr, dtype, stype1, stype2, dcast, scast1, scast2, rcast, ret, return_type)                                                \
    return_type LiftoffAssembler::emit_##name(dtype dst, stype1 lhs, stype2 rhs)                                                                               \
    {                                                                                                                                                          \
        auto _dst = dcast(dst);                                                                                                                                \
...
bool LiftoffAssembler::emit_type_conversion(WasmOpcode opcode, LiftoffRegister dst, LiftoffRegister src, Label* trap)
{
    ...
    case kExprI32UConvertF32: {
        mtfsb0(VXCVI); // clear FPSCR:VXCVI bit
        ConvertDoubleToUnsignedInt64(src.fp(), r0, kScratchDoubleReg, kRoundToZero);
...
#define EMIT_SIMD_BINOP(name, op)                                                                                                                              \
    void LiftoffAssembler::emit_##name(LiftoffRegister dst, LiftoffRegister lhs, LiftoffRegister rhs)                                                          \
    {                                                                                                                                                          \
...
#define EMIT_SIMD_BINOP_WITH_SCRATCH(name, op)                                                                                                                 \
    void LiftoffAssembler::emit_##name(LiftoffRegister dst, LiftoffRegister lhs, LiftoffRegister rhs)                                                          \
    {                                                                                                                                                          \
        op(dst.fp().toSimd(), lhs.fp().toSimd(), rhs.fp().toSimd(), kScratchSimd128Reg);                                                                       \
    }
...
#define EMIT_SIMD_SHIFT_RI(name, op)                                                                                                                           \
    void LiftoffAssembler::emit_##name(LiftoffRegister dst, LiftoffRegister lhs, int32_t rhs)                                                                  \
    {                                                                                                                                                          \
        op(dst.fp().toSimd(), lhs.fp().toSimd(), Operand(rhs), r0, kScratchSimd128Reg);                                                                        \
    }
...
#define EMIT_SIMD_UNOP(name, op, return_val, return_type)                                                                                                      \
    return_type LiftoffAssembler::emit_##name(LiftoffRegister dst, LiftoffRegister src)                                                                        \
    {                                                                                                                                                          \
...
#define EMIT_SIMD_QFM(name, op)                                                                                                                                \
    void LiftoffAssembler::emit_##name(LiftoffRegister dst, LiftoffRegister src1, LiftoffRegister src2, LiftoffRegister src3)                                  \
    {                                                                                                                                                          \
        op(dst.fp().toSimd(), src1.fp().toSimd(), src2.fp().toSimd(), src3.fp().toSimd(), kScratchSimd128Reg);                                                 \
    }
...
#define EMIT_SIMD_EXT_ADD_PAIRWISE(name, op)                                                                                                                   \
    void LiftoffAssembler::emit_##name(LiftoffRegister dst, LiftoffRegister src)                                                                               \
    {                                                                                                                                                          \
        op(dst.fp().toSimd(), src.fp().toSimd(), kScratchSimd128Reg, kScratchSimd128Reg2);                                                                     \
    }
...
#define VISIT_F16_UNOP(name)                                                                                                                                   \
    bool LiftoffAssembler::emit_##name(LiftoffRegister dst, LiftoffRegister src)                                                                               \
    {                                                                                                                                                          \
        return false;                                                                                                                                          \
    }
...
#define VISIT_F16_BINOP(name)                                                                                                                                  \
    bool LiftoffAssembler::emit_##name(LiftoffRegister dst, LiftoffRegister lhs, LiftoffRegister rhs)                                                          \
    {                                                                                                                                                          \
        return false;                                                                                                                                          \
    }
...
{
    F64x2ReplaceLane(dst.fp().toSimd(), src1.fp().toSimd(), src2.fp(), imm_lane_idx, r0, kScratchSimd128Reg);
}
...
{
    F32x4ReplaceLane(dst.fp().toSimd(), src1.fp().toSimd(), src2.fp(), imm_lane_idx, r0, kScratchDoubleReg, kScratchSimd128Reg);
}
...
{
    I64x2ReplaceLane(dst.fp().toSimd(), src1.fp().toSimd(), src2.gp(), imm_lane_idx, kScratchSimd128Reg);
}
...
{
    I32x4ReplaceLane(dst.fp().toSimd(), src1.fp().toSimd(), src2.gp(), imm_lane_idx, kScratchSimd128Reg);
}
...
{
    I16x8ReplaceLane(dst.fp().toSimd(), src1.fp().toSimd(), src2.gp(), imm_lane_idx, kScratchSimd128Reg);
}
...
{
    I8x16ReplaceLane(dst.fp().toSimd(), src1.fp().toSimd(), src2.gp(), imm_lane_idx, kScratchSimd128Reg);
}
...
void LiftoffAssembler::emit_i64x2_mul(LiftoffRegister dst, LiftoffRegister lhs, LiftoffRegister rhs)
{
    ...
    push(scratch);
    I64x2Mul(dst.fp().toSimd(), lhs.fp().toSimd(), rhs.fp().toSimd(), ip, r0, scratch, kScratchSimd128Reg);
    pop(scratch);
}

void LiftoffAssembler::emit_f64x2_min(LiftoffRegister dst, LiftoffRegister lhs, LiftoffRegister rhs)
{
    F64x2Min(dst.fp().toSimd(), lhs.fp().toSimd(), rhs.fp().toSimd(), kScratchSimd128Reg, kScratchSimd128Reg2);
}

void LiftoffAssembler::emit_f64x2_max(LiftoffRegister dst, LiftoffRegister lhs, LiftoffRegister rhs)
{
    F64x2Max(dst.fp().toSimd(), lhs.fp().toSimd(), rhs.fp().toSimd(), kScratchSimd128Reg, kScratchSimd128Reg2);
}
...
{
    F64x2ConvertLowI32x4U(dst.fp().toSimd(), src.fp().toSimd(), r0, kScratchSimd128Reg);
}
...
{
    I64x2UConvertI32x4Low(dst.fp().toSimd(), src.fp().toSimd(), r0, kScratchSimd128Reg);
}

void LiftoffAssembler::emit_i64x2_uconvert_i32x4_high(LiftoffRegister dst, LiftoffRegister src)
{
    I64x2UConvertI32x4High(dst.fp().toSimd(), src.fp().toSimd(), r0, kScratchSimd128Reg);
}
...
{
    I32x4DotI8x16AddS(dst.fp().toSimd(), lhs.fp().toSimd(), rhs.fp().toSimd(), acc.fp().toSimd());
}
...
#endif
    I8x16Shuffle(dst.fp().toSimd(), lhs.fp().toSimd(), rhs.fp().toSimd(), vals[1], vals[0], r0, ip, kScratchSimd128Reg);
}
...
void LiftoffAssembler::PushRegisters(LiftoffRegList regs)
{
    ...
    MultiPushF64AndV128(fp_regs, Simd128RegList::FromBits(fp_regs.bits()), ip, r0);
}
...
void LiftoffAssembler::CallC(const std::initializer_list<VarState> args, ExternalReference ext_ref)
{
    ...
    for (const VarState& arg : args) {
        if (reg_args < int { arraysize(kCArgRegs) }) {
            ...
            if (arg.kind() == kI32 || arg.kind() == kF32)
                bias = -stack_bias;
            int offset = (kStackFrameExtraParamSlot + stack_args) * kSystemPointerSize;
...
void LiftoffAssembler::emit_s128_set_if_nan(Register dst, LiftoffRegister src, Register tmp_gp, LiftoffRegister tmp_s128, ValueKind lane_kind)
{
    Label done;
    if (lane_kind == kF32) {
        xvcmpeqsp(tmp_s128.fp().toSimd(), src.fp().toSimd(), src.fp().toSimd(), SetRC);
    } else {
        DCHECK_EQ(lane_kind, kF64);
        xvcmpeqdp(tmp_s128.fp().toSimd(), src.fp().toSimd(), src.fp().toSimd(), SetRC);
    }
...
void LiftoffStackSlots::Construct(int param_slots)
{
    ...
    for (auto& slot : slots_) {
        ...
        case LiftoffAssembler::VarState::kStack: {
            ...
            case kF32: {
                asm_->AllocateStackSpace(stack_decrement - kSystemPointerSize);
                asm_->LoadF32(kScratchDoubleReg, liftoff::GetStackSlot(slot.src_offset_ + stack_bias), r0);
            ...
            case kF64: {
                ...
                asm_->LoadF64(kScratchDoubleReg, liftoff::GetStackSlot(slot.src_offset_), r0);
            ...
            case kS128: {
                ...
                asm_->LoadSimd128(kScratchSimd128Reg, liftoff::GetStackSlot(slot.src_offset_), r0);
