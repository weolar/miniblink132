...
inline MemOperand GetMemOp(LiftoffAssembler* assm, Register addr, Register offset, uintptr_t offset_imm, bool i64_offset = false, unsigned shift_amount = 0)
{
    if (offset != no_reg) {
        if (!i64_offset) {
...
inline Register CalculateActualAddress(LiftoffAssembler* lasm, UseScratchRegisterScope& temps, Register addr_reg, Register offset_reg, uintptr_t offset_imm)
{
    ...
    if (offset_reg == no_reg) {
        ...
        if (offset_imm != 0)
            __ AddWord(result, result, Operand(offset_imm));
    }
    return result;
}
    ...
    StoreType type, Binop op)
{
    ...
    if (result_reg == value.gp() || result_reg == dst_addr || result_reg == offset_reg) {
