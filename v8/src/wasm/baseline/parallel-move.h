...
public:
    ...
    V8_INLINE void Execute()
    {
        ...
        // registers.
        if (!move_dst_regs_.is_empty())
            ExecuteMoves();
        DCHECK(move_dst_regs_.is_empty());
        if (!load_dst_regs_.is_empty())
            ExecuteLoads();
    ...
    void LoadI64HalfIntoRegister(LiftoffRegister dst, const VarState& src, RegPairHalf half)
    {
        ...
        case VarState::kRegister: {
            ...
            if (dst != src_half)
                MoveRegister(dst, src_half, kI32);
            break;
        }
    ...
    void MoveRegister(LiftoffRegister dst, LiftoffRegister src, ValueKind kind)
    {
        ...
        if (src.is_gp_pair()) {
            DCHECK_EQ(kI64, kind);
            if (dst.low() != src.low())
                MoveRegister(dst.low(), src.low(), kI32);
            if (dst.high() != src.high())
                MoveRegister(dst.high(), src.high(), kI32);
            return;
        }
        if (src.is_fp_pair()) {
        ...
        if (move_dst_regs_.has(dst)) {
            ...
            DCHECK_EQ(is_object_reference(register_move(dst)->kind), is_object_reference(kind));
            DCHECK_EQ(dst.is_fp(), register_move(dst)->kind == kF32 || register_move(dst)->kind == kF64 || register_move(dst)->kind == kS128);
    ...
    void LoadStackSlot(LiftoffRegister dst, int stack_offset, ValueKind kind)
    {
        ...
        if (dst.is_gp_pair()) {
            DCHECK_EQ(kI64, kind);
            *register_load(dst.low()) = RegisterLoad::HalfStack(stack_offset, kLowWord);
            *register_load(dst.high()) = RegisterLoad::HalfStack(stack_offset, kHighWord);
...
private:
    using MovesStorage = std::aligned_storage<kAfterMaxLiftoffRegCode * sizeof(RegisterMove), alignof(RegisterMove)>::type;
    using LoadsStorage = std::aligned_storage<kAfterMaxLiftoffRegCode * sizeof(RegisterLoad), alignof(RegisterLoad)>::type;
