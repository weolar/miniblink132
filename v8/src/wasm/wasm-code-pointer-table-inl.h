...
void WasmCodePointerTable::SetEntrypointWithWriteScope(uint32_t index, Address value, WriteScope& write_scope)
{
    at(index).MakeCodePointerEntry(value);
}

void WasmCodePointerTable::SetEntrypointWithRwxWriteScope(uint32_t index, Address value, RwxMemoryWriteScope& write_scope)
{
    at(index).MakeCodePointerEntry(value);
}
...
bool WasmCodePointerTable::TryAllocateFromFreelist(uint32_t* index)
{
    while (true) {
        ...
        if (!freelist_head_.compare_exchange_strong(current_freelist_head, kRetryMarker)) {
            continue;
        }

        uint32_t next_freelist_entry = at(current_freelist_head.next()).GetNextFreelistEntryIndex();
        FreelistHead new_freelist_head(next_freelist_entry, current_freelist_head.length() - 1);
...
uint32_t WasmCodePointerTable::AllocateEntryFromFreelistNonAtomic(FreelistHead* freelist_head)
{
...
WasmCodePointerTable::FreelistHead WasmCodePointerTable::LinkFreelist(FreelistHead freelist_to_link, uint32_t last_element)
{
    ...
    do {
        ...
        new_head = FreelistHead(freelist_to_link.next(), freelist_to_link.length() + current_head.length());
    ...
    } while (!freelist_head_.compare_exchange_strong(current_head, new_head, std::memory_order_release));
