...
{
    int digits = 1;
    for (uint32_t compare = 10; value >= compare; compare *= 10)
        digits++;
    return digits;
}
...
public:
    ...
    {
        return pending_bytecode_offset_;
    }
    ...
    void PatchLabel(LabelInfo& label, const char* label_source)
    {
        ...
        if (length() == 0) {
            ...
            if (label_source >= unfinished_start && label_source < unfinished_start + unfinished_length) {
