...
public:
    ...
    {
        return code_desc.buffer != nullptr;
    }
    bool failed() const
    {
        return !succeeded();
...
public:
    ...
    {
        DCHECK_IMPLIES(for_debugging != ForDebugging::kNotForDebugging, tier_ == ExecutionTier::kLiftoff);
    }
...
private:
    WasmCompilationResult ExecuteFunctionCompilation(CompilationEnv*, const WireBytesStorage*, Counters*, WasmDetectedFeatures* detected);
...
public:
    ...
    JSToWasmWrapperCompilationUnit& operator=(JSToWasmWrapperCompilationUnit&&) V8_NOEXCEPT = default;
...
inline bool CanUseGenericJsToWasmWrapper(const WasmModule* module, const CanonicalSig* sig)
{
#if (V8_TARGET_ARCH_X64 || V8_TARGET_ARCH_ARM64 || V8_TARGET_ARCH_IA32 || V8_TARGET_ARCH_ARM || V8_TARGET_ARCH_S390X || V8_TARGET_ARCH_PPC64                   \
    || V8_TARGET_ARCH_LOONG64)
    // We don't use the generic wrapper for asm.js, because it creates invalid
    // stack traces.
    return !is_asmjs_module(module) && v8_flags.wasm_generic_wrapper && IsJSCompatibleSignature(sig);
#else
    return false;
#endif
}
