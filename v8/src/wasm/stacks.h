...
public:
    ...
    void* jslimit() const
    {
        return (active_segment_ ? active_segment_->limit_ : limit_) + kJSLimitOffsetKB * KB;
    }
    Address base() const
    {
        return active_segment_ ? active_segment_->base() : reinterpret_cast<Address>(limit_ + size_);
    }
    JumpBuffer* jmpbuf()
    {
        return &jmpbuf_;
    ...
    bool Contains(Address addr)
    {
        ...
        for (auto segment = first_segment_; segment; segment = segment->next_segment_) {
            if (reinterpret_cast<Address>(segment->limit_) <= addr && addr < segment->base()) {
                return true;
            }
    ...
    bool IsActive()
    {
        return jmpbuf_.state == JumpBuffer::Active;
    }
    ...
    public:
        Address base() const
        {
            return reinterpret_cast<Address>(limit_ + size_);
        }
