...
public:
    ...
    static void GenerateLazyCompileTable(
    ...
    static void InitializeJumpsToLazyCompileTable(AccountingAllocator* allocator, Address base, uint32_t num_slots, Address lazy_compile_table_start);

    static void GenerateFarJumpTable(AccountingAllocator* allocator, Address base, Address* stub_targets, int num_runtime_slots, int num_function_slots)
    {
        uint32_t table_size = SizeForNumberOfFarJumpSlots(num_runtime_slots, num_function_slots);
        ...
        int offset = 0;
        for (int index = 0; index < num_runtime_slots + num_function_slots; ++index) {
            DCHECK_EQ(offset, FarJumpSlotIndexToOffset(index));
            // Functions slots initially jump to themselves. They are patched before
            // being used.
            Address target = index < num_runtime_slots ? stub_targets[index] : base + offset;
    ...
    static void PatchJumpTableSlot(AccountingAllocator* allocator, Address jump_table_slot, Address far_jump_table_slot, Address target)
    {
...
private:
    ...
    JumpTableAssembler(AccountingAllocator* allocator, Address slot_addr, int size = 256)
        : MacroAssembler(allocator, JumpTableAssemblerOptions(), CodeObjectRequired::kNo, ExternalAssemblerBuffer(reinterpret_cast<uint8_t*>(slot_addr), size))
    {
    }
...
#error Unknown architecture.
    ...
    template <typename V> void emit(V value);

    template <typename V> void emit(V value, RelaxedStoreTag);
};
