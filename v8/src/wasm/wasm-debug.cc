...
Address FindNewPC(WasmFrame* frame, WasmCode* wasm_code, int byte_offset, ReturnLocation return_location)
{
    ...
    if (return_location == kAfterBreakpoint) {
        ...
        return wasm_code->instruction_start() + it.code_offset() + call_instruction_size;
    }
...
{
    os << "Debug side table (" << num_locals_ << " locals, " << entries_.size() << " entries):\n";
...
{
    os << std::setw(6) << std::hex << pc_offset_ << std::dec << " stack height " << stack_height_ << " [";
...
public:
    ...
    {
        FrameInspectionScope scope(this, pc, isolate);
        return GetValue(scope.debug_side_table, scope.debug_side_table_entry, local, fp, debug_break_fp, isolate);
    }
    ...
    WasmValue GetStackValue(int index, Address pc, Address fp, Address debug_break_fp, Isolate* isolate)
    {
        ...
        if (num_locals + index >= value_count)
            return {};
        return GetValue(scope.debug_side_table, scope.debug_side_table_entry, num_locals + index, fp, debug_break_fp, isolate);
    }
    ...
    {
        const auto& function = native_module_->module()->functions[frame->function_index()];
    ...
    int DeadBreakpoint(int func_index, base::Vector<const int> breakpoints, Isolate* isolate)
    {
        ...
        if (static_cast<int>(wasm_frame->function_index()) != func_index)
            return 0;
    ...
    WasmCode* RecompileLiftoffWithBreakpoints(int func_index, base::Vector<const int> offsets, int dead_breakpoint)
    {
        ...
        ForDebugging for_debugging = offsets.size() == 1 && offsets[0] == 0 ? kForStepping : kWithBreakpoints;

        // Check the cache first.
        for (auto begin = cached_debugging_code_.begin(), it = begin, end = cached_debugging_code_.end(); it != end; ++it) {
            if (it->func_index == func_index && it->breakpoint_offsets.as_vector() == offsets && it->dead_breakpoint == dead_breakpoint) {
        ...
        FunctionBody body { function->sig, function->code.offset(), wire_bytes.begin() + function->code.offset(),
            wire_bytes.begin() + function->code.end_offset(), is_shared };
        ...
        if (V8_UNLIKELY(!env.module->function_was_validated(func_index))) {
            ...
            DecodeResult validation_result = ValidateFunctionBody(&validation_zone, env.enabled_features, env.module, &unused_detected_features, body);
            ...
            CHECK_WITH_MSG(validation_result.ok(), validation_result.error().message().c_str());
            env.module->set_function_validated(func_index);
        }
        WasmCompilationResult result = ExecuteLiftoffCompilation(&env, body,
            LiftoffOptions {}
                .set_func_index(func_index)
                .set_for_debugging(for_debugging)
                .set_breakpoints(offsets)
                .set_dead_breakpoint(dead_breakpoint)
                .set_debug_sidetable(generate_debug_sidetable ? &debug_sidetable : nullptr));
        // Liftoff compilation failure is a FATAL error. We rely on complete Liftoff
        // support for debugging.
        if (!result.succeeded())
            FATAL("Liftoff compilation failed");
        DCHECK_EQ(generate_debug_sidetable, debug_sidetable != nullptr);

        WasmCode* new_code = native_module_->PublishCode(native_module_->AddCompiledCode(result));
        ...
        cached_debugging_code_.insert(
            cached_debugging_code_.begin(), CachedDebuggingCode { func_index, base::OwnedVector<int>::Of(offsets), dead_breakpoint, new_code });
    ...
    void SetBreakpoint(int func_index, int offset, Isolate* isolate)
    {
        ...
        std::vector<int>& breakpoints = isolate_data.breakpoints_per_function[func_index];
        auto insertion_point = std::lower_bound(breakpoints.begin(), breakpoints.end(), offset);
        ...
        insertion_point = std::lower_bound(all_breakpoints.begin(), all_breakpoints.end(), offset);
        bool breakpoint_exists = insertion_point != all_breakpoints.end() && *insertion_point == offset;
        ...
        if (breakpoint_exists) {
            ...
            int dead_breakpoint = DeadBreakpoint(func_index, base::VectorOf(all_breakpoints), isolate);
            new_code = RecompileLiftoffWithBreakpoints(func_index, base::VectorOf(all_breakpoints), dead_breakpoint);
        }
    ...
    std::vector<int> FindAllBreakpoints(int func_index)
    {
        ...
        for (auto& data : per_isolate_data_) {
            ...
            if (it == data.second.breakpoints_per_function.end())
                continue;
    ...
    {
        mutex_.AssertHeld(); // Mutex is held externally.
        WasmCode* new_code = RecompileLiftoffWithBreakpoints(func_index, breakpoints, dead_breakpoint);
    ...
    void FloodWithBreakpoints(WasmFrame* frame, ReturnLocation return_location)
    {
        ...
        base::MutexGuard guard(&mutex_);
        WasmCode* new_code = RecompileLiftoffWithBreakpoints(frame->function_index(), base::ArrayVector(kFloodingBreakpoints), 0);
    ...
    bool IsFrameBlackboxed(WasmFrame* frame)
    {
        ...
        WireBytesRef func_code = native_module->module()->functions[func_index].code;
        Isolate* isolate = frame->isolate();
        DirectHandle<Script> script(Cast<Script>(frame->script()), isolate);
        return isolate->debug()->IsFunctionBlackboxed(script, func_code.offset(), func_code.end_offset());
    }
    ...
    bool PrepareStep(WasmFrame* frame)
    {
        ...
        if (!code->is_liftoff())
            return false; // Cannot step in TurboFan code.
        if (IsAtReturn(frame))
            return false; // Will return after this step.
    ...
    void PrepareStepOutTo(WasmFrame* frame)
    {
        ...
        wasm::WasmCode* code = frame->wasm_code();
        if (!code->is_liftoff())
            return; // Cannot step out to TurboFan code.
    ...
    void ClearStepping(WasmFrame* frame)
    {
        ...
        if (code->for_debugging() != kForStepping)
            return;
        ...
        WasmCode* new_code = RecompileLiftoffWithBreakpoints(func_index, base::VectorOf(breakpoints), dead_breakpoint);
    ...
    void ClearStepping(Isolate* isolate)
    {
        ...
        if (it != per_isolate_data_.end())
            it->second.stepping_frame = NO_ID;
    }
    ...
    bool IsStepping(WasmFrame* frame)
    {
        ...
        base::MutexGuard guard(&mutex_);
        auto it = per_isolate_data_.find(isolate);
        return it != per_isolate_data_.end() && it->second.stepping_frame == frame->id();
    }
    ...
    void RemoveBreakpoint(int func_index, int position, Isolate* isolate)
    {
        ...
        DCHECK_LT(0, offset);
        auto insertion_point = std::lower_bound(breakpoints.begin(), breakpoints.end(), offset);
        if (insertion_point == breakpoints.end())
            return;
        if (*insertion_point != offset)
            return;
        ...
        if (std::binary_search(remaining.begin(), remaining.end(), offset))
            return;
        int dead_breakpoint = DeadBreakpoint(func_index, base::VectorOf(remaining), isolate);
        UpdateBreakpoints(func_index, base::VectorOf(remaining), isolate, isolate_data.stepping_frame, dead_breakpoint);
    }
    ...
    void RemoveIsolate(Isolate* isolate)
    {
        ...
        if (per_isolate_data_it == per_isolate_data_.end())
            return;
        std::unordered_map<int, std::vector<int>> removed_per_function = std::move(per_isolate_data_it->second.breakpoints_per_function);
        ...
        for (auto& entry : removed_per_function) {
            ...
            if (HasRemovedBreakpoints(removed, remaining)) {
                RecompileLiftoffWithBreakpoints(func_index, base::VectorOf(remaining), 0);
            }
        }
    }
...
private:
    struct FrameInspectionScope {
        FrameInspectionScope(DebugInfoImpl* debug_info, Address pc, Isolate* isolate)
            ...
            , debug_side_table(code->is_inspectable() ? debug_info->GetDebugSideTable(code) : nullptr)
            , debug_side_table_entry(debug_side_table ? debug_side_table->GetEntry(pc_offset) : nullptr)
        {
    ...
    const DebugSideTable* GetDebugSideTable(WasmCode* code)
    {
        DCHECK(code->is_inspectable());
        {
            ...
            base::MutexGuard guard(&debug_side_tables_mutex_);
            auto it = debug_side_tables_.find(code);
            if (it != debug_side_tables_.end())
                return it->second.get();
        }

        // Otherwise create the debug side table now.
        std::unique_ptr<DebugSideTable> debug_side_table = GenerateLiftoffDebugSideTable(code);
    ...
    WasmValue GetValue(const DebugSideTable* debug_side_table, const DebugSideTable::Entry* debug_side_table_entry, int index, Address stack_frame_base,
        Address debug_break_fp, Isolate* isolate) const
    {
        const DebugSideTable::Entry::Value* value = debug_side_table->FindValue(debug_side_table_entry, index);
        if (value->is_constant()) {
            DCHECK(value->type == kWasmI32 || value->type == kWasmI64);
            return value->type == kWasmI32 ? WasmValue(value->i32_const) : WasmValue(int64_t { value->i32_const });
        }

        if (value->is_register()) {
            auto reg = LiftoffRegister::from_liftoff_code(value->reg_code);
            auto gp_addr = [debug_break_fp](Register reg) { return debug_break_fp + WasmDebugBreakFrameConstants::GetPushedGpRegisterOffset(reg.code()); };
            if (reg.is_gp_pair()) {
                DCHECK_EQ(kWasmI64, value->type);
                uint32_t low_word = ReadUnalignedValue<uint32_t>(gp_addr(reg.low_gp()));
                uint32_t high_word = ReadUnalignedValue<uint32_t>(gp_addr(reg.high_gp()));
            ...
            if (reg.is_gp()) {
                if (value->type == kWasmI32) {
                    ...
                    Handle<Object> obj(Tagged<Object>(ReadUnalignedValue<Address>(gp_addr(reg.gp()))), isolate);
...
#endif
            Address spilled_addr = debug_break_fp + WasmDebugBreakFrameConstants::GetPushedFpRegisterOffset(code);
        ...
        case kRefNull:
        case kRtt: {
            Handle<Object> obj(Tagged<Object>(ReadUnalignedValue<Address>(stack_address)), isolate);
            return WasmValue(obj, value->type, value->module);
        }
        case kI8:
        case kI16:
    ...
    void UpdateReturnAddresses(Isolate* isolate, WasmCode* new_code, StackFrameId stepping_frame)
    {
        ...
        for (DebuggableStackFrameIterator it(isolate); !it.done(); it.Advance(), return_location = kAfterWasmCall) {
            // We still need the flooded function for stepping.
            if (it.frame()->id() == stepping_frame)
                continue;
#if !V8_ENABLE_DRUMBRAKE
            if (!it.is_wasm())
                continue;
#else // !V8_ENABLE_DRUMBRAKE
            // TODO(paolosev@microsoft.com) - Implement for Wasm interpreter.
            if (!it.is_wasm() || it.is_wasm_interpreter_entry())
                continue;
#endif // !V8_ENABLE_DRUMBRAKE
            WasmFrame* frame = WasmFrame::cast(it.frame());
            if (frame->native_module() != new_code->native_module())
                continue;
            if (frame->function_index() != new_code->index())
                continue;
            if (!frame->wasm_code()->is_liftoff())
                continue;
            UpdateReturnAddress(frame, new_code, return_location);
        }
    }
...
#else
        PointerAuthentication::ReplacePC(frame->pc_address(), new_pc, kSystemPointerSize);
#endif
...
WasmValue DebugInfo::GetLocalValue(int local, Address pc, Address fp, Address debug_break_fp, Isolate* isolate)
{
...
WasmValue DebugInfo::GetStackValue(int index, Address pc, Address fp, Address debug_break_fp, Isolate* isolate)
{
    return impl_->GetStackValue(index, pc, fp, debug_break_fp, isolate);
}

const wasm::WasmFunction& DebugInfo::GetFunctionAtAddress(Address pc, Isolate* isolate)
{
    return impl_->GetFunctionAtAddress(pc, isolate);
}

void DebugInfo::SetBreakpoint(int func_index, int offset, Isolate* current_isolate)
{
...
void DebugInfo::RemoveBreakpoint(int func_index, int offset, Isolate* current_isolate)
{
...
DebugSideTable* DebugInfo::GetDebugSideTableIfExists(const WasmCode* code) const
{
...
int FindNextBreakablePosition(wasm::NativeModule* native_module, int func_index, int offset_in_func)
{
    ...
    const wasm::WasmFunction& func = native_module->module()->functions[func_index];
    wasm::BytecodeIterator iterator(module_start + func.code.offset(), module_start + func.code.end_offset(), &locals, &zone);
    DCHECK_LT(0, locals.encoded_size);
    if (offset_in_func < 0)
        return 0;
    for (; iterator.has_next(); iterator.next()) {
        if (iterator.pc_offset() < static_cast<uint32_t>(offset_in_func))
            continue;
        if (!wasm::WasmOpcodes::IsBreakable(iterator.current()))
            continue;
...
void SetBreakOnEntryFlag(Tagged<Script> script, bool enabled)
{
    ...
    for (int i = 0; i < weak_instance_list->length(); ++i) {
        if (weak_instance_list->Get(i).IsCleared())
            continue;
        i::Tagged<i::WasmInstanceObject> instance = i::Cast<i::WasmInstanceObject>(weak_instance_list->Get(i).GetHeapObject());
...
// static
bool WasmScript::SetBreakPoint(DirectHandle<Script> script, int* position, DirectHandle<BreakPoint> break_point)
{
    ...
    int breakable_offset = FindNextBreakablePosition(script->wasm_native_module(), func_index, offset_in_func);
    if (breakable_offset == 0)
        return false;
    *position = func.code.offset() + breakable_offset;

    return WasmScript::SetBreakPointForFunction(script, func_index, breakable_offset, break_point);
}

// static
void WasmScript::SetInstrumentationBreakpoint(DirectHandle<Script> script, DirectHandle<BreakPoint> break_point)
{
...
// static
bool WasmScript::SetBreakPointOnFirstBreakableForFunction(DirectHandle<Script> script, int func_index, DirectHandle<BreakPoint> break_point)
{
    if (func_index < 0)
        return false;
...
// static
bool WasmScript::SetBreakPointForFunction(DirectHandle<Script> script, int func_index, int offset, DirectHandle<BreakPoint> break_point)
{
...
int GetBreakpointPos(Isolate* isolate, Tagged<Object> break_point_info_or_undef)
{
    if (IsUndefined(break_point_info_or_undef, isolate))
        return kMaxInt;
    return Cast<BreakPointInfo>(break_point_info_or_undef)->source_position();
}

int FindBreakpointInfoInsertPos(Isolate* isolate, DirectHandle<FixedArray> breakpoint_infos, int position)
{
...
// static
bool WasmScript::ClearBreakPoint(DirectHandle<Script> script, int position, DirectHandle<BreakPoint> break_point)
{
    if (!script->has_wasm_breakpoint_infos())
        return false;

    Isolate* isolate = script->GetIsolate();
    DirectHandle<FixedArray> breakpoint_infos(script->wasm_breakpoint_infos(), isolate);
    ...
    DirectHandle<BreakPointInfo> info(Cast<BreakPointInfo>(breakpoint_infos->get(pos)), isolate);
    ...
    if (info->GetBreakPointCount(isolate) == 0) {
        ...
        for (int i = pos; i < breakpoint_infos->length() - 1; i++) {
            ...
            if (IsUndefined(entry, isolate))
                break;
        }
        // Make sure last array element is empty as a result.
        breakpoint_infos->set(breakpoint_infos->length() - 1, ReadOnlyRoots { isolate }.undefined_value(), SKIP_WRITE_BARRIER);
    }
    ...
    if (break_point->id() == v8::internal::Debug::kInstrumentationId) {
        ...
        const wasm::WasmModule* module = native_module->module();
        int func_index = GetContainingWasmFunction(module, position);
        native_module->GetDebugInfo()->RemoveBreakpoint(func_index, position, isolate);
    }
...
// static
bool WasmScript::ClearBreakPointById(DirectHandle<Script> script, int breakpoint_id)
{
    if (!script->has_wasm_breakpoint_infos()) {
        return false;
    }
    ...
    for (int i = 0, e = breakpoint_infos->length(); i < e; ++i) {
        ...
        if (BreakPointInfo::GetBreakPointById(isolate, breakpoint_info, breakpoint_id).ToHandle(&breakpoint)) {
            DCHECK(breakpoint->id() == breakpoint_id);
            return WasmScript::ClearBreakPoint(script, breakpoint_info->source_position(), breakpoint);
        }
    }
    return false;
}
...
{
    script->set_wasm_breakpoint_infos(ReadOnlyRoots(script->GetIsolate()).empty_fixed_array());
    SetBreakOnEntryFlag(script, false);
}

// static
void WasmScript::AddBreakpointToInfo(DirectHandle<Script> script, int position, DirectHandle<BreakPoint> break_point)
{
    Isolate* isolate = script->GetIsolate();
    ...
    if (insert_pos < breakpoint_infos->length() && GetBreakpointPos(isolate, breakpoint_infos->get(insert_pos)) == position) {
        DirectHandle<BreakPointInfo> old_info(Cast<BreakPointInfo>(breakpoint_infos->get(insert_pos)), isolate);
        BreakPointInfo::SetBreakPoint(isolate, old_info, break_point);
        return;
    }

    // Enlarge break positions array if necessary.
    bool need_realloc = !IsUndefined(breakpoint_infos->get(breakpoint_infos->length() - 1), isolate);
    DirectHandle<FixedArray> new_breakpoint_infos = breakpoint_infos;
    if (need_realloc) {
        new_breakpoint_infos = isolate->factory()->NewFixedArray(2 * breakpoint_infos->length(), AllocationType::kOld);
    ...
    DirectHandle<BreakPointInfo> breakpoint_info = isolate->factory()->NewBreakPointInfo(position);
...
bool WasmScript::GetPossibleBreakpoints(
    wasm::NativeModule* native_module, const v8::debug::Location& start, const v8::debug::Location& end, std::vector<v8::debug::BreakLocation>* locations)
{
    DisallowGarbageCollection no_gc;
    ...
    if (start.GetLineNumber() != 0 || start.GetColumnNumber() < 0
        || (!end.IsEmpty() && (end.GetLineNumber() != 0 || end.GetColumnNumber() < 0 || end.GetColumnNumber() < start.GetColumnNumber())))
        return false;
    ...
    int start_func_index = GetNearestWasmFunction(module, start.GetColumnNumber());
    if (start_func_index < 0)
        return false;
    ...
    if (start_func_index == end_func_index && start_offset > functions[end_func_index].code.end_offset())
        return false;
    Zone zone { wasm::GetWasmEngine()->allocator(), ZONE_NAME };
    const uint8_t* module_start = native_module->wire_bytes().begin();

    for (int func_idx = start_func_index; func_idx <= end_func_index; ++func_idx) {
...
bool CheckBreakPoint(Isolate* isolate, DirectHandle<BreakPoint> break_point, StackFrameId frame_id)
{
    if (break_point->condition()->length() == 0)
        return true;
    ...
    if (!DebugEvaluate::Local(isolate, frame_id, inlined_jsframe_index, condition, throw_on_side_effect).ToHandle(&result)) {
...
// static
MaybeHandle<FixedArray> WasmScript::CheckBreakPoints(Isolate* isolate, DirectHandle<Script> script, int position, StackFrameId frame_id)
{
    if (!script->has_wasm_breakpoint_infos())
        return {};
    ...
    DirectHandle<Object> maybe_breakpoint_info(breakpoint_infos->get(insert_pos), isolate);
    if (IsUndefined(*maybe_breakpoint_info, isolate))
        return {};
    ...
    Handle<FixedArray> break_points_hit = isolate->factory()->NewFixedArray(array->length());
    int break_points_hit_count = 0;
    for (int i = 0; i < array->length(); ++i) {
        DirectHandle<BreakPoint> break_point(Cast<BreakPoint>(array->get(i)), isolate);
