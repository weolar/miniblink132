...
inline Address WasmInterpreterRuntime::EffectiveAddress(uint64_t index) const
{
    ...
    DCHECK_GE(std::numeric_limits<uintptr_t>::max(), trusted_data->memory0_size());
...
inline bool WasmInterpreterRuntime::BoundsCheckMemRange(uint64_t index, uint64_t* size, Address* out_address) const
{
    Handle<WasmTrustedInstanceData> trusted_data = wasm_trusted_instance_data();
    DCHECK_GE(std::numeric_limits<uintptr_t>::max(), trusted_data->memory0_size());
    if (!base::ClampToBounds<uint64_t>(index, size, trusted_data->memory0_size())) {
        return false;
    }
...
inline Handle<Object> WasmInterpreterRuntime::GetGlobalRef(uint32_t index) const
{
    ...
    std::tie(global_buffer, global_index) = wasm_trusted_instance_data()->GetGlobalBufferAndIndex(global);
    return Handle<Object>(global_buffer->get(global_index), isolate_);
}

inline void WasmInterpreterRuntime::SetGlobalRef(uint32_t index, Handle<Object> ref) const
{
    // This function assumes that it is executed in a HandleScope.
    const wasm::WasmGlobal& global = module_->globals[index];
    DCHECK(global.type.is_reference());
...
{
    wasm_trusted_instance_data()->element_segments()->set(index, *isolate_->factory()->empty_fixed_array());
}

inline WasmBytecode* WasmInterpreterRuntime::GetFunctionBytecode(uint32_t func_index)
{
    return codemap_->GetFunctionBytecode(func_index);
}

inline bool WasmInterpreterRuntime::IsNullTypecheck(const WasmRef obj, const ValueType obj_type) const
{
    return IsNull(isolate_, obj, obj_type);
}

// static
inline Tagged<Object> WasmInterpreterRuntime::GetNullValue(const ValueType obj_type) const
{
...
// static
inline bool WasmInterpreterRuntime::IsNull(Isolate* isolate, const WasmRef obj, const ValueType obj_type)
{
    if (obj_type == kWasmExternRef || obj_type == kWasmNullExternRef) {
...
inline Handle<Object> WasmInterpreterRuntime::GetFunctionRef(uint32_t index) const
{
    // This function assumes that it is executed in a HandleScope.
    return WasmTrustedInstanceData::GetOrCreateFuncRef(isolate_, wasm_trusted_instance_data(), index);
}

inline const ArrayType* WasmInterpreterRuntime::GetArrayType(uint32_t array_index) const
{
    return module_->array_type(ModuleTypeIndex { array_index });
}

inline WasmRef WasmInterpreterRuntime::GetWasmArrayRefElement(Tagged<WasmArray> array, uint32_t index) const
{
    return WasmArray::GetElement(isolate_, handle(array, isolate_), index);
}

inline Handle<WasmTrustedInstanceData> WasmInterpreterRuntime::wasm_trusted_instance_data() const
{
    return handle(instance_object_->trusted_data(isolate_), isolate_);
}

inline WasmInterpreterThread::State InterpreterHandle::ContinueExecution(WasmInterpreterThread* thread, bool called_from_js)
{
