...
public:
    struct CacheKey {
        ...
        {
            return kind == rhs.kind && type_index == rhs.type_index && expected_arity == rhs.expected_arity && suspend == rhs.suspend;
        }
    ...
    public:
        ...
        {
            return base::hash_combine(static_cast<uint8_t>(key.kind), key.type_index.index, key.expected_arity);
        }
    };
    ...
    public:
        ...
        WasmCode* AddWrapper(const CacheKey& key, WasmCompilationResult result, WasmCode::Kind kind);
    ...
    {
        if (call_target == kInvalidWasmCodePointer)
            return nullptr;
    ...
    WasmCode* CompileWasmImportCallWrapper(Isolate* isolate, ImportCallKind kind, const CanonicalSig* sig, CanonicalTypeIndex sig_index, bool source_positions,
