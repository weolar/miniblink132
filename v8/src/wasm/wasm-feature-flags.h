...
#define FOREACH_WASM_EXPERIMENTAL_FEATURE_FLAG(V) /*     (force 80 columns) */                                                                                 \
    ...
    V(skip_null_checks, "skip null checks for call.ref and array and struct operations (unsafe)", false)                                                       \
...
#define CHECK_WASM_FEATURE_OFF_BY_DEFAULT(name, desc, enabled) static_assert(enabled == false);
#define CHECK_WASM_FEATURE_ON_BY_DEFAULT(name, desc, enabled) static_assert(enabled == true);
