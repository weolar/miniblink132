...
class V8_EXPORT_PRIVATE WasmCodePointerTable : public SegmentedTable<WasmCodePointerTableEntry, kCodePointerTableReservationSize> {
    using Base = SegmentedTable<WasmCodePointerTableEntry, kCodePointerTableReservationSize>;
...
public:
    ...
    inline void SetEntrypointWithWriteScope(uint32_t index, Address value, WriteScope& write_scope);
    inline void SetEntrypointWithRwxWriteScope(uint32_t index, Address value, RwxMemoryWriteScope& write_scope);
...
private:
    ...
    {
        return freelist.length() == kRetryMarker.length() && freelist.next() == kRetryMarker.next();
    }
