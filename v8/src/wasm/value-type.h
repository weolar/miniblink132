...
public:
    ...
    {
        return representation_ == kBottom;
    }
    constexpr bool is_top() const
    {
        return representation_ == kTop;
    }

    constexpr bool is_string_view() const
    {
        return representation_ == kStringViewWtf8 || representation_ == kStringViewWtf16 || representation_ == kStringViewIter;
    }
...
protected:
    explicit constexpr HeapTypeBase(Representation repr)
...
private:
    ...
    {
        return representation_ <= kLastSentinel;
    }

    static constexpr Representation kFirstSentinel = static_cast<Representation>(kV8MaxWasmTypes);
    static constexpr Representation kLastSentinel = static_cast<Representation>(kBottom - 1);
...
#define ELEM_SIZE_LOG2(kind, log2Size, ...) log2Size == -1 ? -1 : (1 << std::max(0, log2Size)),
...
#define KIND_NAME(kind, log2Size, code, machineType, shortName, kindName, ...) kindName,
...
#define MACH_TYPE(kind, log2Size, code, machineType, ...) MachineType::machineType(),
...
public:
    ...
    {
        return wasm::is_numeric(kind());
    }

    constexpr bool is_reference() const
    {
        return wasm::is_reference(kind());
    }
    ...
    {
        return (kind() == kRef || kind() == kRefNull) && heap_representation() == htype;
    }

    constexpr bool is_rtt() const
    {
        return wasm::is_rtt(kind());
    }
    ...
    {
        return wasm::is_defaultable(kind());
    }
    ...
    constexpr bool is_uninhabited() const
    {
        return is_bottom()
            || (is_non_nullable()
                && (is_reference_to(HeapType::kNone) || is_reference_to(HeapType::kNoExn) || is_reference_to(HeapType::kNoExtern)
                    || is_reference_to(HeapType::kNoFunc) || is_reference_to(HeapType::kNoneShared) || is_reference_to(HeapType::kNoExnShared)
                    || is_reference_to(HeapType::kNoExternShared) || is_reference_to(HeapType::kNoFuncShared)));
    }

    constexpr bool is_packed() const
    {
        return wasm::is_packed(kind());
    }

    /***************************** Field Accessors ******************************/
    constexpr ValueKind kind() const
    {
        return KindField::decode(bit_field_);
    }
    constexpr HeapType::Representation heap_representation() const
    {
        DCHECK(is_object_reference());
        return static_cast<HeapType::Representation>(HeapTypeField::decode(bit_field_));
    }
    ...
    constexpr bool use_wasm_null() const
    {
        ...
        if (repr == HeapType::kExtern)
            return false;
        if (repr == HeapType::kExternString)
            return false;
        if (repr == HeapType::kNoExtern)
            return false;
        return true;
    }
...
#undef NUMERIC_TYPE_CASE
    ...
    {
        return kind() == kRef || (kind() == kRefNull && !heap_type().is_abstract_non_shared());
    }
    ...
    std::string name() const
    {
        ...
        case kRef:
            ...
            break;
        case kRefNull:
            if (heap_type().is_abstract_non_shared() && !heap_type().is_string_view()) {
...
protected:
    ...
    static constexpr ValueTypeBase Ref(HeapType::Representation heap_type)
    {
        ...
        return ValueTypeBase(KindField::encode(kRef) | HeapTypeField::encode(heap_type));
    }
    ...
    {
        DCHECK(HeapTypeBase(heap_type).is_valid());
        return ValueTypeBase(KindField::encode(kRefNull) | HeapTypeField::encode(heap_type));
    }
    static constexpr ValueTypeBase RefNull(HeapTypeBase heap_type)
    {
        return RefNull(heap_type.representation());
    }
    static constexpr ValueTypeBase RefMaybeNull(HeapType::Representation heap_type, Nullability nullability)
    {
        DCHECK(HeapTypeBase(heap_type).is_valid());
        return ValueTypeBase(KindField::encode(nullability == kNullable ? kRefNull : kRef) | HeapTypeField::encode(heap_type));
    }
    static constexpr ValueTypeBase RefMaybeNull(HeapTypeBase heap_type, Nullability nullability)
    {
    ...
    static_assert(kV8MaxWasmTypes < (1u << kHeapTypeBits), "Type indices fit in kHeapTypeBits");
    // This is implemented defensively against field order changes.
    static_assert(kLastUsedBit == std::max(KindField::kLastUsedBit, HeapTypeField::kLastUsedBit), "kLastUsedBit is consistent");
...
public:
    ...
    static constexpr ValueType RefMaybeNull(HeapType::Representation heap_type, Nullability nullability)
    {
        return ValueType { ValueTypeBase::RefMaybeNull(heap_type, nullability) };
    }

    static constexpr ValueType RefMaybeNull(ModuleTypeIndex type, Nullability nullability)
    {
        return ValueType::RefMaybeNull(HeapType(type), nullability);
    }

    static constexpr ValueType RefMaybeNull(HeapType heap_type, Nullability nullability)
    {
        return ValueType { ValueTypeBase::RefMaybeNull(heap_type, nullability) };
    }
    ...
    static constexpr ValueType Rtt(ModuleTypeIndex type)
    {
        ...
        return ValueType { ValueTypeBase { KindField::encode(kRtt) | HeapTypeField::encode(type.index) } };
    }
...
public:
    ...
    static constexpr CanonicalValueType RefNull(HeapType::Representation heap_type)
    {
        return CanonicalValueType { ValueTypeBase::RefNull(heap_type) };
    }

    static constexpr CanonicalValueType FromIndex(ValueKind kind, CanonicalTypeIndex index)
    {
        DCHECK(kind == kRefNull || kind == kRef);
        CHECK_LT(index.index, kV8MaxWasmTypes);
        return CanonicalValueType { ValueTypeBase(KindField::encode(kind) | HeapTypeField::encode(index.index)) };
    }
...
inline constexpr intptr_t ValueTypeBase::kBitFieldOffset = offsetof(ValueTypeBase, bit_field_);

static_assert(sizeof(ValueTypeBase) <= kUInt32Size, "ValueType is small and can be passed by value");
static_assert(ValueTypeBase::kLastUsedBit < kSmiValueSize, "ValueType has space to be encoded in a Smi");
...
constexpr ValueType kWasmRefNullExternString = ValueType::RefNull(HeapType::kExternString);
constexpr ValueType kWasmRefExternString = ValueType::Ref(HeapType::kExternString);
constexpr ValueType kWasmStringViewWtf8 = ValueType::Ref(HeapType::kStringViewWtf8);
constexpr ValueType kWasmStringViewWtf16 = ValueType::Ref(HeapType::kStringViewWtf16);
constexpr ValueType kWasmStringViewIter = ValueType::Ref(HeapType::kStringViewIter);
constexpr ValueType kWasmNullRef = ValueType::RefNull(HeapType::kNone);
constexpr ValueType kWasmNullExternRef = ValueType::RefNull(HeapType::kNoExtern);
...
constexpr CanonicalValueType kCanonicalI32 = CanonicalValueType::Primitive(kI32);
constexpr CanonicalValueType kCanonicalI64 = CanonicalValueType::Primitive(kI64);
constexpr CanonicalValueType kCanonicalF32 = CanonicalValueType::Primitive(kF32);
constexpr CanonicalValueType kCanonicalF64 = CanonicalValueType::Primitive(kF64);
constexpr CanonicalValueType kCanonicalS128 = CanonicalValueType::Primitive(kS128);
constexpr CanonicalValueType kCanonicalExternRef = CanonicalValueType::RefNull(HeapType::kExtern);
constexpr CanonicalValueType kCanonicalAnyRef = CanonicalValueType::RefNull(HeapType::kAny);
...
V8_EXPORT_PRIVATE bool EquivalentNumericSig(const CanonicalSig* a, const FunctionSig* b);
...
#define LOAD_SIZE(_, __, memtype) static_cast<uint8_t>(ElementSizeInBytes(MachineType::memtype().representation())),
...
#undef LOAD_SIZE
    ...
    // MSVC wants a static_cast here.
#define LOAD_SIZE(_, __, memtype) static_cast<uint8_t>(ElementSizeLog2Of(MachineType::memtype().representation())),
        FOREACH_LOAD_TYPE(LOAD_SIZE)
#undef LOAD_SIZE
    };
...
private:
    ...
    // MSVC wants a static_cast here.
#define STORE_SIZE(_, __, memrep) static_cast<uint8_t>(ElementSizeLog2Of(MachineRepresentation::k##memrep)),
...
V8_EXPORT_PRIVATE const wasm::FunctionSig* GetI32Sig(Zone* zone, const wasm::FunctionSig* sig);
