...
constexpr size_t kV8MaxWasmMemory32Pages = kSystemPointerSize == 4 ? 32'767 // = 2 GiB - 64Kib
                                                                   : 65'536; // = 4 GiB
constexpr size_t kV8MaxWasmMemory64Pages = kSystemPointerSize == 4 ? 32'767 // = 2 GiB - 64Kib
                                                                   : 262'144; // = 16 GiB
...
static_assert(kV8MaxWasmTableInitEntries <= kV8MaxWasmTableSize, "JS-API should not exceed v8's limit");

// 64-bit platforms support the full spec'ed memory limits.
static_assert(kSystemPointerSize == 4 || (kV8MaxWasmMemory32Pages == kSpecMaxMemory32Pages && kV8MaxWasmMemory64Pages == kSpecMaxMemory64Pages));

constexpr uint64_t kWasmMaxHeapOffset = static_cast<uint64_t>(std::numeric_limits<uint32_t>::max()) // maximum base value
...
// imported functions.
constexpr size_t kV8MaxWasmTotalFunctions = kV8MaxWasmDefinedFunctions + kV8MaxWasmImports;
