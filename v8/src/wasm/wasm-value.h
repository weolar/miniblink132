...
#define DEFINE_TYPE_SPECIFIC_METHODS(name, localtype, ctype)                                                                                                   \
    explicit WasmValue(ctype v)                                                                                                                                \
        ...
        , bit_pattern_ {}                                                                                                                                      \
    {                                                                                                                                                          \
        static_assert(sizeof(ctype) <= sizeof(bit_pattern_), "size too big for WasmValue");                                                                    \
        base::WriteUnalignedValue<ctype>(reinterpret_cast<Address>(bit_pattern_), v);                                                                          \
    }                                                                                                                                                          \
    ...
    {                                                                                                                                                          \
        return base::ReadUnalignedValue<ctype>(reinterpret_cast<Address>(bit_pattern_));                                                                       \
    }
...
#undef DEFINE_TYPE_SPECIFIC_METHODS
    ...
    WasmValue(Handle<Object> ref, ValueType type, const WasmModule* module)
        ...
        , module_(module)
    {
        static_assert(sizeof(Handle<Object>) <= sizeof(bit_pattern_), "bit_pattern_ must be large enough to fit a Handle");
        DCHECK(type.is_reference());
        base::WriteUnalignedValue<Handle<Object>>(reinterpret_cast<Address>(bit_pattern_), ref);
    }

    Handle<Object> to_ref() const
    {
        DCHECK(type_.is_reference());
        return base::ReadUnalignedValue<Handle<Object>>(reinterpret_cast<Address>(bit_pattern_));
    }
    ...
    {
        return type_ == other.type_ && !memcmp(bit_pattern_, other.bit_pattern_, type_.is_reference() ? sizeof(Handle<Object>) : type_.value_kind_size());
    }
    ...
    template <typename T> inline T to_unchecked() const;

    static WasmValue ForUintPtr(uintptr_t value)
    {
        using type = std::conditional<kSystemPointerSize == 8, uint64_t, uint32_t>::type;
    ...
    bool zero_byte_representation()
    {
        ...
        return static_cast<uint32_t>(std::count(bit_pattern_, bit_pattern_ + byte_count, 0)) == byte_count;
    }
...
#define DECLARE_CAST(name, localtype, ctype, ...)                                                                                                              \
    template <> inline ctype WasmValue::to_unchecked() const                                                                                                   \
    {                                                                                                                                                          \
        return to_##name##_unchecked();                                                                                                                        \
    }                                                                                                                                                          \
    template <> inline ctype WasmValue::to() const                                                                                                             \
    {                                                                                                                                                          \
        return to_##name();                                                                                                                                    \
    }
