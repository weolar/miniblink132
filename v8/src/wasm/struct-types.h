...
public:
    StructTypeBase(uint32_t field_count, uint32_t* field_offsets, const ValueTypeBase* reps, const bool* mutabilities)
    ...
    template <class Subclass, class ValueTypeSubclass> class BuilderImpl {
    public:
        enum ComputeOffsets : bool { kComputeOffsets = true, kUseProvidedOffsets = false };
        ...
        BuilderImpl(Zone* zone, uint32_t field_count)
            ...
            , buffer_(zone->AllocateArray<ValueTypeSubclass>(static_cast<int>(field_count)))
            , mutabilities_(zone->AllocateArray<bool>(static_cast<int>(field_count)))
        {
        }
        ...
        Subclass* Build(ComputeOffsets compute_offsets = kComputeOffsets)
        {
            ...
            Subclass* result = zone_->New<Subclass>(field_count_, field_offsets_, buffer_, mutabilities_);
    ...
    static const size_t kMaxFieldOffset = (kV8MaxWasmStructFields - 1) * kMaxValueTypeSize;
...
public:
    ...
    StructType(uint32_t field_count, uint32_t* field_offsets, const ValueType* reps, const bool* mutabilities)
    ...
    bool operator==(const StructType& other) const
    {
        ...
        return std::equal(fields().begin(), fields().end(), other.fields().begin())
            && std::equal(mutabilities().begin(), mutabilities().end(), other.mutabilities().begin());
    }
    bool operator!=(const StructType& other) const
    {
        return !(*this == other);
...
public:
    using Builder = StructTypeBase::BuilderImpl<CanonicalStructType, CanonicalValueType>;

    CanonicalStructType(uint32_t field_count, uint32_t* field_offsets, const CanonicalValueType* reps, const bool* mutabilities)
        : StructTypeBase(field_count, field_offsets, reps, mutabilities)
    {
    }
    ...
    {
        const CanonicalValueType* cast_reps = static_cast<const CanonicalValueType*>(reps_);
        return { cast_reps, cast_reps + field_count_ };
    }
};
...
inline size_t hash_value(const StructTypeBase& type)
{
    ...
    return base::Hasher {}.AddRange(type.fields()).AddRange(type.mutabilities()).hash();
}

class ArrayTypeBase : public ZoneObject {
public:
    constexpr explicit ArrayTypeBase(bool mutability)
