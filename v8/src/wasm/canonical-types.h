...
public:
    ...
    V8_EXPORT_PRIVATE void AddRecursiveGroup(WasmModule* module, uint32_t size, uint32_t start_index);
    ...
    V8_EXPORT_PRIVATE void AddRecursiveSingletonGroup(WasmModule* module, uint32_t start_index);
    ...
    V8_EXPORT_PRIVATE bool IsCanonicalSubtype(
        ModuleTypeIndex sub_index, ModuleTypeIndex super_index, const WasmModule* sub_module, const WasmModule* super_module);
    ...
    V8_EXPORT_PRIVATE static void PrepareForCanonicalTypeId(Isolate* isolate, CanonicalTypeIndex id);
    ...
    V8_EXPORT_PRIVATE static void ClearWasmCanonicalTypesForTesting(Isolate* isolate);
...
private:
    struct CanonicalType {
        ...
        constexpr CanonicalType(const CanonicalStructType* type, CanonicalTypeIndex supertype, bool is_final, bool is_shared)
            : struct_type(type)
            , supertype(supertype)
            , kind(kStruct)
            , is_final(is_final)
    ...
    struct CanonicalHashing {
        ...
        void Add(CanonicalType type)
        {
            ...
            uint32_t supertype_index = type.supertype.index - is_relative * recgroup.start.index;
            static_assert(kMaxCanonicalTypes <= kMaxUInt32 >> 2);
            uint32_t metadata = (supertype_index << 2) | (is_relative << 1) | (type.is_final ? 1 : 0);
            hasher.Add(metadata);
            switch (type.kind) {
        ...
        void Add(CanonicalValueType value_type)
        {
            if (value_type.has_index() && recgroup.Contains(value_type.ref_index())) {
                ...
                hasher.Add((value_type.ref_index().index - recgroup.start.index) + kMaxCanonicalTypes);
            } else {
        ...
        void Add(const CanonicalSig& sig)
        {
            ...
            for (CanonicalValueType type : sig.all())
                Add(type);
        }
    ...
    struct CanonicalEquality {
        ...
        bool EqualTypeIndex(CanonicalTypeIndex index1, CanonicalTypeIndex index2) const
        {
            ...
            if (relative_index != recgroup2.Contains(index2))
                return false;
            if (relative_index) {
        ...
        {
            if (!EqualTypeIndex(type1.supertype, type2.supertype))
                return false;
            if (type1.is_final != type2.is_final)
                return false;
            if (type1.is_shared != type2.is_shared)
                return false;
            switch (type1.kind) {
            case CanonicalType::kFunction:
                return type2.kind == CanonicalType::kFunction && EqualSig(*type1.function_sig, *type2.function_sig);
            case CanonicalType::kStruct:
                return type2.kind == CanonicalType::kStruct && EqualStructType(*type1.struct_type, *type2.struct_type);
            case CanonicalType::kArray:
                return type2.kind == CanonicalType::kArray && EqualArrayType(*type1.array_type, *type2.array_type);
            }
        }

        bool EqualTypes(base::Vector<const CanonicalType> types1, base::Vector<const CanonicalType> types2) const
        {
            return std::equal(types1.begin(), types1.end(), types2.begin(), types2.end(), std::bind_front(&CanonicalEquality::EqualType, this));
        }
        ...
        bool EqualValueType(CanonicalValueType type1, CanonicalValueType type2) const
        {
            ...
            if (is_ref && type1.heap_representation() != type2.heap_representation()) {
                return false;
            }
            return true;
        }
        ...
        bool EqualSig(const CanonicalSig& sig1, const CanonicalSig& sig2) const
        {
            if (sig1.parameter_count() != sig2.parameter_count())
                return false;
            return std::equal(
                sig1.all().begin(), sig1.all().end(), sig2.all().begin(), sig2.all().end(), std::bind_front(&CanonicalEquality::EqualValueType, this));
        }
        ...
        bool EqualStructType(const CanonicalStructType& type1, const CanonicalStructType& type2) const
        {
            return
                ...
                std::equal(type1.fields().begin(), type1.fields().end(), type2.fields().begin(), type2.fields().end(),
                    std::bind_front(&CanonicalEquality::EqualValueType, this))
                &&
                // Compare mutabilities, skipping the check for the size.
                std::equal(type1.mutabilities().begin(), type1.mutabilities().end(), type2.mutabilities().begin());
        }

        bool EqualArrayType(const CanonicalArrayType& type1, const CanonicalArrayType& type2) const
        {
            return type1.mutability() == type2.mutability() && EqualValueType(type1.element_type(), type2.element_type());
        }
    };
    ...
    struct CanonicalGroup {
        ...
        {
            CanonicalTypeIndex end { start.index + static_cast<uint32_t>(types.size() - 1) };
            CanonicalTypeIndex other_end { other.start.index + static_cast<uint32_t>(other.types.size() - 1) };
    ...
    std::unordered_set<CanonicalGroup, base::hash<CanonicalGroup>> canonical_groups_;
    // Set of all known canonical recgroups of size 1.
    std::unordered_set<CanonicalSingletonGroup, base::hash<CanonicalSingletonGroup>> canonical_singleton_groups_;
    // Maps canonical indices back to the function signature.
    std::unordered_map<CanonicalTypeIndex, const CanonicalSig*, base::hash<CanonicalTypeIndex>> canonical_function_sigs_;
