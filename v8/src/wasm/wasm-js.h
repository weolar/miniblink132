...
V8_EXPORT_PRIVATE std::unique_ptr<WasmStreaming> StartStreamingForTesting(Isolate*, std::shared_ptr<wasm::CompilationResultResolver>);
...
#define DECL_WASM_JS_EXTERNAL_REFERENCE(Name) V8_EXPORT_PRIVATE void Name(const v8::FunctionCallbackInfo<v8::Value>& info);
...
public:
    ...
    V8_EXPORT_PRIVATE static void InstallConditionalFeatures(Isolate* isolate, Handle<NativeContext> context);

private:
    V8_EXPORT_PRIVATE static void InstallModule(Isolate* isolate, Handle<JSObject> webassembly);

    V8_EXPORT_PRIVATE static bool InstallTypeReflection(Isolate* isolate, DirectHandle<NativeContext> context, Handle<JSObject> webassembly);

    V8_EXPORT_PRIVATE static bool InstallJSPromiseIntegration(Isolate* isolate, DirectHandle<NativeContext> context, Handle<JSObject> webassembly);
};
