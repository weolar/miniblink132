    ...
    AllocationAlignment alignment)
{
    ...
    __ MacroAssembler::Branch(
        __ MakeDeferredCode([](MaglevAssembler* masm, RegisterSnapshot register_snapshot, Register object, AllocationType alloc_type, T size_in_bytes,
                                ZoneLabelRef done) { AllocateSlow(masm, register_snapshot, object, AllocateBuiltin(alloc_type), size_in_bytes, done); },
...
void MaglevAssembler::OSRPrologue(Graph* graph)
{
    ...
    if (v8_flags.maglev_assert_stack_size && v8_flags.debug_code) {
        ...
        Register scratch = temps.AcquireScratch();
        int32_t expected_osr_stack_size = source_frame_size * kSystemPointerSize + StandardFrameConstants::kFixedFrameSizeFromFp;
        AddWord(scratch, sp, Operand(expected_osr_stack_size));
        MacroAssembler::Assert(eq, AbortReason::kOsrUnexpectedStackSize, scratch, Operand(fp));
    }
    ...
    if (source_frame_size < target_frame_size) {
        ...
        if (size_so_far < target_frame_size) {
            Sub64(sp, sp, Operand((target_frame_size - size_so_far) * kSystemPointerSize));
        }
    }
}
...
void MaglevAssembler::Prologue(Graph* graph)
{
    ...
    if (v8_flags.turbofan) {
        ...
        DCHECK(!AreAliased(flags, feedback_vector,
            ...
            // a0
            kJSFunctionRegister, kContextRegister, kJavaScriptCallNewTargetRegister));
        ...
        Condition needs_processing = LoadFeedbackVectorFlagsAndCheckIfNeedsProcessing(flags, feedback_vector, flag_reg, CodeKind::MAGLEV);
        TailCallBuiltin(Builtin::kMaglevOptimizeCodeOrTailCallOptimizedCodeSlot, needs_processing, flag_reg, Operand(zero_reg));
    }
...
void MaglevAssembler::LoadSingleCharacterString(Register result, Register char_code, Register scratch)
{
    ...
    if (v8_flags.debug_code) {
        MacroAssembler::Assert(less_equal, AbortReason::kUnexpectedValue, char_code, Operand(String::kMaxOneByteCharCode));
    }
    ...
    MacroAssembler::Branch( // FIXME: reimplement with JumpToDeferredIf
        MakeDeferredCode(
            [](MaglevAssembler* masm, RegisterSnapshot register_snapshot, ZoneLabelRef done, Register result, Register char_code, Register scratch) {
                ...
                Register string = need_restore_result ? temps.AcquireScratch() : result;
                ...
                __ Sh(scratch, FieldMemOperand(string, OFFSET_OF_DATA_START(SeqTwoByteString)));
...
void MaglevAssembler::IsObjectType(Register object, Register scratch1, Register scratch2, InstanceType type)
{
    ...
    CompareObjectTypeAndJump(object, scratch1, scratch2, type, Condition::kEqual, &ConditionMet, Label::kNear);
...
void MaglevAssembler::StringCharCodeOrCodePointAt(BuiltinStringPrototypeCharCodeOrCodePointAt::Mode mode, RegisterSnapshot& register_snapshot, Register result,
    Register string, Register index, Register instance_type, [[maybe_unused]] Register scratch2, Label* result_fits_one_byte)
{
    ZoneLabelRef done(this);
    Label seq_string;
    Label cons_string;
    ...
    if (v8_flags.debug_code) {
        ...
        Check(kUnsignedLessThan, AbortReason::kUnexpectedValue, index, Operand(scratch));
    }
...

    {
        ...
        MacroAssembler::Branch(&seq_string, kEqual, representation, Operand(kSeqStringTag), Label::kNear);
        MacroAssembler::Branch(&cons_string, kEqual, representation, Operand(kConsStringTag), Label::kNear);
        MacroAssembler::Branch(&sliced_string, kEqual, representation, Operand(kSlicedStringTag), Label::kNear);
        MacroAssembler::Branch(deferred_runtime_call, kNotEqual, representation, Operand(kThinStringTag));
    ...
    bind(&seq_string);
    {
        ...
        MacroAssembler::Branch(&two_byte_string, equal, instance_type, Operand(kTwoByteStringTag), Label::kNear);
        // The result of one-byte string will be the same for both modes
        // (CharCodeAt/CodePointAt), since it cannot be the first half of a
        // surrogate pair.
        AddWord(result, string, Operand(index));
        Lbu(result, MemOperand(result, OFFSET_OF_DATA_START(SeqOneByteString) - kHeapObjectTag));
        ...
        Lhu(result, MemOperand(result, OFFSET_OF_DATA_START(SeqTwoByteString) - kHeapObjectTag));

        if (mode == BuiltinStringPrototypeCharCodeOrCodePointAt::kCodePointAt) {
            Register first_code_point = scratch;
            And(first_code_point, result, Operand(0xfc00));
            MacroAssembler::Branch(*done, kNotEqual, first_code_point, Operand(0xd800), Label::kNear);

            Register length = scratch;
            Lw(length, FieldMemOperand(string, offsetof(String, length_)));
            Add32(index, index, Operand(1));
            MacroAssembler::Branch(*done, kGreaterThanEqual, index, Operand(length), Label::kNear);

            Register second_code_point = scratch;
            Sll32(second_code_point, index, Operand(1));
            AddWord(second_code_point, string, second_code_point);
            Lhu(second_code_point, MemOperand(second_code_point, OFFSET_OF_DATA_START(SeqTwoByteString) - kHeapObjectTag));

            // {index} is not needed at this point.
            Register scratch2 = index;
            And(scratch2, second_code_point, Operand(0xfc00));
            MacroAssembler::Branch(*done, kNotEqual, scratch2, Operand(0xdc00), Label::kNear);
...
void MaglevAssembler::TryTruncateDoubleToInt32(Register dst, DoubleRegister src, Label* fail)
{
    ...
    MacroAssembler::Branch(fail, eq, rcmp, Operand(zero_reg)); // if we don't know branch distance
    ...
    MacroAssembler::Move(rcmp, src); // FIXME: should we enable this in MaglevAssembler as well?
