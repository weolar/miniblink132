...
public:
    ...
    using MaybeSnapshot = compiler::turboshaft::SnapshotTable<ValueNode*>::MaybeSnapshot;
    ...
    bool has_phi() const
    {
        return has_state() && state_->has_phi();
    }
    ...
    {
        return type_ == kEdgeSplit;
    }

    bool is_loop() const
    {
        return has_state() && state()->is_loop();
    }
    ...
    template <typename Func> void ForEachPredecessor(Func&& functor) const
    {
    ...
    template <typename Func> void ForEachSuccessor(Func&& functor) const
    {
    ...
    {
        return type_ == kMerge && state_ != nullptr;
    }
    ...
    {
        snapshot_.Set(snapshot);
    }

    ZonePtrList<ValueNode>& reload_hints()
    {
        return reload_hints_;
    }
    ...
    BasicBlock* RealJumpTarget()
    {
        ...
        while (true) {
            if (!current->nodes_.is_empty() || current->is_loop() || current->is_exception_handler_block() || current->HasPhisOrRegisterMerges()) {
                break;
            }
...
private:
    bool HasPhisOrRegisterMerges() const
    {
        ...
        state()->register_state().ForEachGeneralRegister([&](Register reg, RegisterState& state) {
            ValueNode* node;
            RegisterMerge* merge;
        ...
        state()->register_state().ForEachDoubleRegister([&](DoubleRegister reg, RegisterState& state) {
            ValueNode* node;
            RegisterMerge* merge;
            if (LoadMergeState(state, &node, &merge)) {
                has_register_merge = true;
