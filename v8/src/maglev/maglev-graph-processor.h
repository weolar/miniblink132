...
//
template <typename NodeProcessor, bool visit_identity_nodes = false> class GraphProcessor;
...
public:
    explicit ProcessingState(BlockConstIterator block_it, NodeIterator* node_it = nullptr)
        : block_it_(block_it)
        , node_it_(node_it)
    {
    }
...
template <typename NodeProcessor, bool visit_identity_nodes> class GraphProcessor {
public:
    ...
    void ProcessGraph(Graph* graph)
    {
        ...
        auto process_constants = [&](auto& map) {
            for (auto it = map.begin(); it != map.end();) {
                ProcessResult result = node_processor_.Process(it->second, GetCurrentState());
                switch (result) {
        ...
        for (block_it_ = graph->begin(); block_it_ != graph->end(); ++block_it_) {
            ...
            BlockProcessResult preprocess_result = node_processor_.PreProcessBasicBlock(block);
            ...
            for (node_it_ = block->nodes().begin(); node_it_ != block->nodes().end();) {
                ...
                case ProcessResult::kRemove:
                    ...
                    break;
                case ProcessResult::kHoist: {
                    DCHECK(block->predecessor_count() == 1 || (block->predecessor_count() == 2 && block->is_loop()));
            ...
            {
                ProcessResult control_result = ProcessNodeBase(block->control_node(), GetCurrentState());
...
// them iteratively.
template <typename... Processors> class NodeMultiProcessor;

template <> class NodeMultiProcessor<> {
public:
    void PreProcessGraph(Graph* graph)
    {
    }
...
template <typename Processor, typename... Processors> class NodeMultiProcessor<Processor, Processors...> : NodeMultiProcessor<Processors...> {
...
public:
    ...
    template <typename Node> ProcessResult Process(Node* node, const ProcessingState& state)
    {
...
template <typename... Processors> using GraphMultiProcessor = GraphProcessor<NodeMultiProcessor<Processors...>>;
