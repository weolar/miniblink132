...
void MaglevAssembler::OSRPrologue(Graph* graph)
{
    ...
    if (v8_flags.maglev_assert_stack_size && v8_flags.debug_code) {
        lgr(scratch, sp);
        lay(scratch, MemOperand(scratch, source_frame_size * kSystemPointerSize + StandardFrameConstants::kFixedFrameSizeFromFp));
        CmpU64(scratch, fp);
    ...
    if (source_frame_size < target_frame_size) {
        ...
        if (size_so_far < target_frame_size) {
            lay(sp, MemOperand(sp, -(target_frame_size - size_so_far) * kSystemPointerSize));
        }
    }
}
...
void MaglevAssembler::Prologue(Graph* graph)
{
    ...
    if (v8_flags.turbofan) {
        ...
        TailCallBuiltin(Builtin::kMaglevOptimizeCodeOrTailCallOptimizedCodeSlot,
            LoadFeedbackVectorFlagsAndCheckIfNeedsProcessing(flags, feedback_vector, CodeKind::MAGLEV));
    }
    ...
    if (graph->untagged_stack_slots() > 0) {
        ...
        lay(sp, MemOperand(sp, -graph->untagged_stack_slots() * kSystemPointerSize));
    }
}
    ...
    RegisterSnapshot register_snapshot, Label* char_code_fits_one_byte, Register result, Register char_code, Register scratch, CharCodeMaskMode mask_mode)
{
    ...
    JumpToDeferredIf(
        ...
        [](MaglevAssembler* masm, RegisterSnapshot register_snapshot, ZoneLabelRef done, Register result, Register char_code, Register scratch) {
            ...
            __ StoreU16(char_code, FieldMemOperand(result, OFFSET_OF_DATA_START(SeqTwoByteString)));
            __ b(*done);
        },
...
void MaglevAssembler::StringCharCodeOrCodePointAt(BuiltinStringPrototypeCharCodeOrCodePointAt::Mode mode, RegisterSnapshot& register_snapshot, Register result,
    Register string, Register index, Register instance_type, Register scratch2, Label* result_fits_one_byte)
{
    ZoneLabelRef done(this);
    Label seq_string;
    Label cons_string;
    ...
    // Is a thin string.
    {
        LoadTaggedField(string, FieldMemOperand(string, offsetof(ThinString, actual_)));
        b(&loop);
    }
    ...
    bind(&cons_string);
    {
        ...
        LoadU64(second_string, FieldMemOperand(string, offsetof(ConsString, second_)));
        CompareRoot(second_string, RootIndex::kempty_string);
        bne(deferred_runtime_call);
        LoadTaggedField(string, FieldMemOperand(string, offsetof(ConsString, first_)));
    ...
    bind(&seq_string);
    {
        ...
        LoadU8(result, FieldMemOperand(string, index, OFFSET_OF_DATA_START(SeqOneByteString)));
        ...
        AddU64(scratch, Operand(OFFSET_OF_DATA_START(SeqTwoByteString) - kHeapObjectTag));
        ...
        if (mode == BuiltinStringPrototypeCharCodeOrCodePointAt::kCharCodeAt) {
            ...
            AddU32(index, Operand(OFFSET_OF_DATA_START(SeqTwoByteString) - kHeapObjectTag));
