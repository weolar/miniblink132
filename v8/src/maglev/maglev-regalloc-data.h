...
template <typename T> struct AllocatableRegisters;

template <> struct AllocatableRegisters<Register> {
    static constexpr RegList kRegisters = MaglevAssembler::GetAllocatableRegisters();
};

template <> struct AllocatableRegisters<DoubleRegister> {
    static constexpr DoubleRegList kRegisters = MaglevAssembler::GetAllocatableDoubleRegisters();
};
...
struct RegisterStateFlags {
    ...
    {
        return (is_initialized ? 1 << kIsInitializedShift : 0) | (is_merge ? 1 << kIsMergeShift : 0);
    }
...
{
    return left.is_initialized == right.is_initialized && left.is_merge == right.is_merge;
}
...
inline bool LoadMergeState(RegisterState state, ValueNode** node, RegisterMerge** merge)
{
    DCHECK(state.GetPayload().is_initialized);
