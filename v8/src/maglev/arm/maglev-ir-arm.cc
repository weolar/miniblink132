...
void BuiltinStringFromCharCode::GenerateCode(MaglevAssembler* masm, const ProcessingState& state)
{
    ...
    if (Int32Constant* constant = code_input().node()->TryCast<Int32Constant>()) {
        ...
        if (0 <= char_code && char_code < String::kMaxOneByteCharCode) {
            ...
            __ strh(scratch, FieldMemOperand(result_string, OFFSET_OF_DATA_START(SeqTwoByteString)));
        }
    } else {
...
{
    if (offset() != 0) {
        __ add(ToRegister(result()), ToRegister(allocation_block()), Operand(offset()));
    }
}
...
namespace {
void Uint32Mod(MaglevAssembler* masm, Register out, Register left, Register right)
{
    MaglevAssembler::TemporaryRegisterScope temps(masm);
...
void Float64Modulus::GenerateCode(MaglevAssembler* masm, const ProcessingState& state)
{
    ...
    __ MovToFloatParameters(ToDoubleRegister(left_input()), ToDoubleRegister(right_input()));
...
void LoadTypedArrayLength::GenerateCode(MaglevAssembler* masm, const ProcessingState& state)
{
    ...
    if (element_size > 1) {
        ...
        __ lsr(result_register, result_register, Operand(base::bits::CountTrailingZeros(element_size)));
    }
}
