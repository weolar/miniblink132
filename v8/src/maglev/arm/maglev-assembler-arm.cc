...
void MaglevAssembler::OSRPrologue(Graph* graph)
{
    ...
    if (v8_flags.maglev_assert_stack_size && v8_flags.debug_code) {
        add(scratch, sp, Operand(source_frame_size * kSystemPointerSize + StandardFrameConstants::kFixedFrameSizeFromFp));
        cmp(scratch, fp);
    ...
    if (source_frame_size < target_frame_size) {
        ...
        if (size_so_far < target_frame_size) {
            sub(sp, sp, Operand((target_frame_size - size_so_far) * kSystemPointerSize));
        }
    }
}
...
void MaglevAssembler::Prologue(Graph* graph)
{
    ...
    if (v8_flags.turbofan) {
        ...
        // code.
        JumpToDeferredIf(needs_processing, [](MaglevAssembler* masm) { __ TailCallBuiltin(Builtin::kMaglevOptimizeCodeOrTailCallOptimizedCodeSlot); });
    }
    ...
    RegisterSnapshot register_snapshot, Label* char_code_fits_one_byte, Register result, Register char_code, Register scratch, CharCodeMaskMode mask_mode)
{
    ...
    JumpToDeferredIf(
        ...
        [](MaglevAssembler* masm, RegisterSnapshot register_snapshot, ZoneLabelRef done, Register result, Register char_code, Register scratch) {
            ...
            __ strh(char_code, FieldMemOperand(result, OFFSET_OF_DATA_START(SeqTwoByteString)));
            __ b(*done);
        },
    ...
    Register string, Register index, Register instance_type, Register scratch2, Label* result_fits_one_byte)
{
    ...
    bind(&seq_string);
    {
        ...
        // surrogate pair.
        add(index, index, Operand(OFFSET_OF_DATA_START(SeqOneByteString) - kHeapObjectTag));
        ...
        add(scratch, scratch, Operand(OFFSET_OF_DATA_START(SeqTwoByteString) - kHeapObjectTag));
        ...
        if (mode == BuiltinStringPrototypeCharCodeOrCodePointAt::kCharCodeAt) {
            ...
            add(index, index, Operand(OFFSET_OF_DATA_START(SeqTwoByteString) - kHeapObjectTag));
