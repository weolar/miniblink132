...
class MaglevPhiRepresentationSelector {
    template <class Value> using SnapshotTable = compiler::turboshaft::SnapshotTable<Value>;
...
public:
    ...
    template <class NodeT> ProcessResult Process(NodeT* node, const ProcessingState& state)
    {
...
private:
    ...
    template <class NodeT> ProcessResult UpdateNodeInputs(NodeT* n, const ProcessingState* state)
    {
        NodeBase* node = static_cast<NodeBase*>(n);

        ProcessResult result = ProcessResult::kContinue;
        if (IsUntagging(n->opcode())) {
            if (node->input(0).node()->Is<Phi>() && node->input(0).node()->value_representation() != ValueRepresentation::kTagged) {
                ...
                UpdateUntaggingOfPhi(node->input(0).node()->Cast<Phi>(), n->template Cast<ValueNode>());
            }
        } else {
    ...
    template <class NodeT> ProcessResult UpdateNonUntaggingNodeInputs(NodeT* n, const ProcessingState* state)
    {
        NodeBase* node = static_cast<NodeBase*>(n);
    ...
    ValueNode* EnsurePhiTagged(
        Phi* phi, BasicBlock* block, NewNodePosition pos, const ProcessingState* state, std::optional<int> predecessor_index = std::nullopt);
    ...
    template <typename DeoptInfoT> void BypassIdentities(DeoptInfoT* deopt_info);
