...
void BuiltinStringFromCharCode::GenerateCode(MaglevAssembler* masm, const ProcessingState& state)
{
    ...
    if (Int32Constant* constant = code_input().node()->TryCast<Int32Constant>()) {
        ...
        if (0 <= char_code && char_code < String::kMaxOneByteCharCode) {
            ...
            __ Strh(scratch.W(), FieldMemOperand(result_string, OFFSET_OF_DATA_START(SeqTwoByteString)));
        }
    } else {
...
void Int32MultiplyWithOverflow::GenerateCode(MaglevAssembler* masm, const ProcessingState& state)
{
    ...
    __ CompareAndBranch(res, Immediate(0), ne, &end);
    {
        ...
        __ Tbnz(temp, temp.SizeInBits() - 1, __ GetDeoptLabel(this, DeoptimizeReason::kOverflow));
    }
    __ Bind(&end);
    if (out_alias_input) {
        __ Move(out, res.W());
...
void Int32DivideWithOverflow::GenerateCode(MaglevAssembler* masm, const ProcessingState& state)
{
    ...
    __ CompareAndBranch(temp, Immediate(0), ne, __ GetDeoptLabel(this, DeoptimizeReason::kNotInt32));
...
void LoadTypedArrayLength::GenerateCode(MaglevAssembler* masm, const ProcessingState& state)
{
    ...
    if (element_size > 1) {
        ...
        __ Lsr(result_register, result_register, base::bits::CountTrailingZeros(element_size));
    }
}
...
void GenerateReduceInterruptBudget(MaglevAssembler* masm, Node* node, ReduceInterruptBudgetType type, int amount)
{
    ...
    __ Ldr(feedback_cell, MemOperand(fp, StandardFrameConstants::kFunctionOffset));
    __ LoadTaggedField(feedback_cell, FieldMemOperand(feedback_cell, JSFunction::kFeedbackCellOffset));
    __ Ldr(budget, FieldMemOperand(feedback_cell, FeedbackCell::kInterruptBudgetOffset));
    __ Subs(budget, budget, Immediate(amount));
    __ Str(budget, FieldMemOperand(feedback_cell, FeedbackCell::kInterruptBudgetOffset));
    ZoneLabelRef done(masm);
...
void Return::GenerateCode(MaglevAssembler* masm, const ProcessingState& state)
{
    ...
    __ CompareAndBranch(params_size, actual_params_size, ge, &corrected_args_count);
