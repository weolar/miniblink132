...
using BlockConstReverseIterator = ZoneVector<BasicBlock*>::const_reverse_iterator;
...
public:
    ...
    {
        return tagged_stack_slots_;
    }
    uint32_t untagged_stack_slots() const
    {
        return untagged_stack_slots_;
    }
    uint32_t max_call_stack_args() const
    {
        return max_call_stack_args_;
    }
    uint32_t max_deopted_stack_size() const
    {
        return max_deopted_stack_size_;
    }
    ...
    {
        return total_peeled_bytecode_size_;
    }
    ...
    {
        return has_recursive_calls_;
    }
    void set_has_recursive_calls(bool value)
    {
        has_recursive_calls_ = value;
    }
    ...
    {
        has_resumable_generator_ = true;
    }
    bool has_resumable_generator() const
    {
        return has_resumable_generator_;
    }
    ...
    compiler::OptionalScopeInfoRef TryGetScopeInfo(ValueNode* context, compiler::JSHeapBroker* broker)
    {
        ...
        if (auto context_const = context->TryCast<Constant>()) {
            ...
            compiler::OptionalScopeInfoRef cur = TryGetScopeInfoForContextLoad(load->input(0).node(), load->offset(), broker);
            if (cur.has_value())
                res = cur;
        } else if (auto load = context->TryCast<LoadTaggedFieldForScriptContextSlot>()) {
            compiler::OptionalScopeInfoRef cur = TryGetScopeInfoForContextLoad(load->input(0).node(), load->offset(), broker);
            if (cur.has_value())
                res = cur;
            ...
            DCHECK(context->Is<Phi>() || context->Is<GeneratorRestoreRegister>() || context->Is<RegisterInput>() || context->Is<CallRuntime>());
        }
