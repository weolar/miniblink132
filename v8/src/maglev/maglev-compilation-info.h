...
public:
    static std::unique_ptr<MaglevCompilationInfo> NewForTurboshaft(
        Isolate* isolate, compiler::JSHeapBroker* broker, IndirectHandle<JSFunction> function, BytecodeOffset osr_offset, bool specialize_to_function_context)
    {
        // Doesn't use make_unique due to the private ctor.
        return std::unique_ptr<MaglevCompilationInfo>(new MaglevCompilationInfo(isolate, function, osr_offset, broker, specialize_to_function_context,
    ...
    {
        // Doesn't use make_unique due to the private ctor.
        return std::unique_ptr<MaglevCompilationInfo>(new MaglevCompilationInfo(isolate, function, osr_offset));
    }
    ...
    {
        return osr_offset_ != BytecodeOffset::None();
    }
    ...
    bool for_turboshaft_frontend() const
    {
        return for_turboshaft_frontend_;
    }

    bool has_graph_labeller() const
    {
        return !!graph_labeller_;
    }
    ...
    MaglevCodeGenerator* code_generator() const
    {
        return code_generator_.get();
    }
#endif
...
#undef V
    ...
    {
        return collect_source_positions_;
    }
...
private:
    MaglevCompilationInfo(Isolate* isolate, IndirectHandle<JSFunction> function, BytecodeOffset osr_offset,
        std::optional<compiler::JSHeapBroker*> broker = std::nullopt, std::optional<bool> specialize_to_function_context = std::nullopt,
