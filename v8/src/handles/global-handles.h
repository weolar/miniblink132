...
public:
    ...
    template <class NodeType> class NodeBlock;
    ...
    template <typename T> inline IndirectHandle<T> Create(Tagged<T> value);
    ...
    {
        return last_gc_custom_callbacks_;
    }
...
private:
    ...
    class Node;
    template <class BlockType> class NodeIterator;
    template <class NodeType> class NodeSpace;
...
public:
    class Iterator {
    public:
        explicit Iterator(std::vector<Address, StrongRootAllocator<Address>>::iterator it)
            : it_(it)
        {
        }
        Iterator& operator++()
        ...
        Tagged<T> raw()
        {
            return Cast<T>(Tagged<Object>(*it_));
        }
    ...
    {
        locations_.reserve(size);
    }
    void Push(Tagged<T> val)
    {
        locations_.push_back(val.ptr());
    }
    ...
    Iterator begin()
    {
        return Iterator(locations_.begin());
    }
    Iterator end()
    {
        return Iterator(locations_.end());
    }
