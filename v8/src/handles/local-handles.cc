...
Address* LocalHandleScope::GetMainThreadHandle(LocalHeap* local_heap, Address value)
{
...
void LocalHandleScope::CloseMainThreadScope(LocalHeap* local_heap, Address* prev_next, Address* prev_limit)
{
    Isolate* isolate = local_heap->heap()->isolate();
...
void LocalHandles::Iterate(RootVisitor* visitor)
{
    for (int i = 0; i < static_cast<int>(blocks_.size()) - 1; i++) {
        ...
        visitor->VisitRootPointers(Root::kHandleScope, nullptr, FullObjectSlot(block), FullObjectSlot(&block[kHandleBlockSize]));
    }

    if (!blocks_.empty()) {
        Address* block = blocks_.back();
        visitor->VisitRootPointers(Root::kHandleScope, nullptr, FullObjectSlot(block), FullObjectSlot(scope_.next));
    }
}
...
bool LocalHandles::Contains(Address* location)
{
    ...
    for (auto it = blocks_.begin(); it != blocks_.end(); ++it) {
        ...
        // block_size_ handles.
        Address* upper_bound = lower_bound != blocks_.back() ? lower_bound + kHandleBlockSize : scope_.next;
