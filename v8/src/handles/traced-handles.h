...
public:
    ...
    bool is_weak() const
    {
        return IsWeak::decode(flags_);
    }
    void set_weak(bool v)
    {
        flags_ = IsWeak::update(flags_, v);
    }

    bool is_droppable() const
    {
        return IsDroppable::decode(flags_);
    }
    void set_droppable(bool v)
    {
        flags_ = IsDroppable::update(flags_, v);
    }
    ...
    {
        flags_ = IsInUse::update(flags_, v);
    }
    ...
    {
        return flags_ == 0 && !markbit();
    }

    void clear_markbit()
    {
        is_marked_.store(false, std::memory_order_relaxed);
    }

    bool has_old_host() const
    {
        return HasOldHost::decode(flags_);
    }
    void set_has_old_host(bool v)
    {
        flags_ = HasOldHost::update(flags_, v);
    }

    template <AccessMode access_mode = AccessMode::NON_ATOMIC> void set_raw_object(Address value)
    {
        if constexpr (access_mode == AccessMode::NON_ATOMIC) {
            object_ = value;
        } else {
            reinterpret_cast<std::atomic<Address>*>(&object_)->store(value, std::memory_order_relaxed);
        }
    }
...
class TracedNodeBlock final {
    class NodeIteratorImpl final : public base::iterator<std::forward_iterator_tag, TracedNode> {
    public:
        ...
        NodeIteratorImpl(const NodeIteratorImpl& other) V8_NOEXCEPT : block_(other.block_), current_index_(other.current_index_)
        {
        }

        TracedNode* operator*()
        {
            return block_->at(current_index_);
        }
    ...
    template <typename ConcreteTraits> struct BaseListTraits {
...
public:
    ...
    using OverallList = v8::base::DoublyThreadedList<TracedNodeBlock*, OverallListTraits>;
    using UsableList = v8::base::DoublyThreadedList<TracedNodeBlock*, UsableListTraits>;
    using YoungList = v8::base::DoublyThreadedList<TracedNodeBlock*, YoungListTraits>;
...
#endif // !V8_HOST_ARCH_64_BIT
    static constexpr size_t kMaxCapacity = std::numeric_limits<TracedNode::IndexType>::max() - 1;
    ...
    {
        return in_young_list_;
    }

    void SetInYoungList(bool in_young_list)
    {
        in_young_list_ = in_young_list;
    }
...
public:
    ...
    void ProcessYoungObjects(RootVisitor* v, WeakSlotCallbackWithHeap should_reset_handle);
    ...
    {
        return sizeof(TracedNode) * used_nodes_;
    }
