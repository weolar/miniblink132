...
bool TracedHandles::NeedsTrackingInYoungNodes(Tagged<Object> object, TracedNode* node) const
{
...
bool TracedHandles::NeedsToBeRemembered(Tagged<Object> object, TracedNode* node, Address* slot, TracedReferenceStoreMode store_mode) const
{
...
FullObjectSlot TracedNode::Publish(Tagged<Object> object, bool needs_young_bit_update, bool needs_black_allocation, bool has_old_host, bool is_droppable_value)
{
    DCHECK(IsMetadataCleared());

    flags_ = needs_young_bit_update << IsInYoungList::kShift | has_old_host << HasOldHost::kShift | is_droppable_value << IsDroppable::kShift
        | 1 << IsInUse::kShift;
...
FullObjectSlot TracedHandles::Create(Address value, Address* slot, TracedReferenceStoreMode store_mode, TracedReferenceHandling reference_handling)
{
    ...
    const bool needs_black_allocation = is_marking_ && store_mode != TracedReferenceStoreMode::kInitializingStore;
    const bool is_droppable = reference_handling == TracedReferenceHandling::kDroppable;
    auto result_slot = node->Publish(object, needs_young_bit_update, needs_black_allocation, has_old_host, is_droppable);
