...
bool HandleBase::IsDereferenceAllowed() const
{
    ...
    if (isolate->IsBuiltinTableHandleLocation(location_))
        return true;
    ...
    if (!local_heap->is_main_thread()) {
        ...
        return local_heap->ContainsPersistentHandle(location_) || local_heap->ContainsLocalHandle(location_);
    }
...
Address* HandleScope::Extend(Isolate* isolate)
{
    ...
    if (!Utils::ApiCheck(current->level != current->sealed_level, "v8::HandleScope::CreateHandle()", "Cannot create a handle without a HandleScope")) {
        return nullptr;
    }
    HandleScopeImplementer* impl = isolate->handle_scope_implementer();
...
{
#if 0//ndef _DEBUG // some nodejs addon(e.t node-pty) will crash on this func
