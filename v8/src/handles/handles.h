...
template <typename T> class DirectHandleUnchecked;
#endif
...
protected:
    ...
    template <typename T> friend IndirectHandle<T> indirect_handle(DirectHandle<T> handle);
    template <typename T> friend IndirectHandle<T> indirect_handle(DirectHandle<T> handle, Isolate* isolate);
    template <typename T> friend IndirectHandle<T> indirect_handle(DirectHandle<T> handle, LocalIsolate* isolate);
    template <typename T> friend IndirectHandle<T> indirect_handle(DirectHandle<T> handle, LocalHeap* local_heap);
#endif // V8_ENABLE_DIRECT_HANDLE
...
template <typename T> class Handle final : public HandleBase {
public:
    ...
    V8_INLINE explicit Handle(Address* location)
        : HandleBase(location)
    {
    ...
    V8_INLINE Tagged<T> operator->() const
    {
        ...
        static_assert(is_subtype_v<T, HeapObject>, "This handle does not reference a heap object. Use `(*handle).foo`.");
        return **this;
    }
...
private:
    ...
    template <typename> friend class MaybeHandle;
    // Casts are allowed to access location_.
    template <typename To, typename From> friend inline Handle<To> UncheckedCast(Handle<From> value);
};

template <typename T> std::ostream& operator<<(std::ostream& os, Handle<T> handle);
...
public:
    ...
    // handles ship.
    template <typename T, template <typename> typename HandleType, typename = std::enable_if_t<std::is_convertible_v<HandleType<T>, DirectHandle<T>>>>
...
template <typename V, class AllocationPolicy> class IdentityMap;
...
struct HandleScopeData final {
    static constexpr uint32_t kSizeInBytes = 2 * kSystemPointerSize + 2 * kInt32Size;
...
template <typename T> struct is_direct_handle : public std::false_type { };
template <typename T> static constexpr bool is_direct_handle_v = is_direct_handle<T>::value;
...
public:
    ...
    V8_INLINE bool is_null() const
    {
        return obj_ == kTaggedNullAddress;
    }
    ...
    V8_INLINE static int NumberOfHandles()
    {
    ...
    public:
        ResetNumberOfHandlesScope()
            : saved_number_of_handles_(number_of_handles_)
        {
...
protected:
    ...
    V8_INLINE DirectHandleBase(const DirectHandleBase& other) V8_NOEXCEPT : obj_(other.obj_)
    {
        Register();
    }
    DirectHandleBase& operator=(const DirectHandleBase&) V8_NOEXCEPT = default;
    V8_INLINE ~DirectHandleBase() V8_NOEXCEPT
    {
        Unregister();
    }
#endif

    V8_INLINE explicit DirectHandleBase(Address object)
        : obj_(object)
    {
        Register();
    }
...
// https://docs.google.com/document/d/1uRGYQM76vk1fc_aDqDH3pm2qhaJtnK2oyzeVng4cS6I/
template <typename T> class DirectHandle : public DirectHandleBase {
public:
    // Handles denote strong references.
    static_assert(!is_maybe_weak_v<T>);

    V8_INLINE DirectHandle()
        : DirectHandle(kTaggedNullAddress)
    {
    }

    V8_INLINE explicit DirectHandle(Address object)
        : DirectHandleBase(object)
    {
    }
    ...
    V8_INLINE DirectHandle(DirectHandle<S> handle)
        : DirectHandle(handle.obj_)
    {
    }

    template <typename S, typename = std::enable_if_t<is_subtype_v<S, T>>>
    V8_INLINE DirectHandle(Handle<S> handle)
        : DirectHandle(handle.location() != nullptr ? *handle.location() : kTaggedNullAddress)
    {
    }
    ...
    V8_INLINE Tagged<T> operator->() const
    {
        if constexpr (is_subtype_v<T, HeapObject>) {
            ...
            // If you got an error here and want to access the Tagged<T>, use
            // operator* -- e.g. for `Tagged<Smi>::value()`, use `(*handle).value()`.
            static_assert(false, "This handle does not reference a heap object. Use `(*handle).foo`.");
        }
    }
    ...
    V8_INLINE static const DirectHandle<T> null()
    {
...
private:
    ...
    // obj_.
    template <typename> friend class DirectHandle;
    // MaybeDirectHandle is allowed to access obj_.
    template <typename> friend class MaybeDirectHandle;
    friend class DirectHandleUnchecked<T>;
    // Casts are allowed to access obj_.
    template <typename To, typename From> friend inline DirectHandle<To> UncheckedCast(DirectHandle<From> value);
    ...
    explicit DirectHandle(const DirectHandle<T>& other, no_checking_tag do_not_check)
        : DirectHandleBase(other.obj_, do_not_check)
    {
    }
};

template <typename T> IndirectHandle<T> indirect_handle(DirectHandle<T> handle)
{
    if (handle.is_null())
...
template <typename T> IndirectHandle<T> indirect_handle(DirectHandle<T> handle, Isolate* isolate)
{
    if (handle.is_null())
        return IndirectHandle<T>();
    return IndirectHandle<T>(HandleBase::indirect_handle(handle.address(), isolate));
}

template <typename T> IndirectHandle<T> indirect_handle(DirectHandle<T> handle, LocalIsolate* isolate)
{
    if (handle.is_null())
        return IndirectHandle<T>();
    return IndirectHandle<T>(HandleBase::indirect_handle(handle.address(), isolate));
...
template <typename T> IndirectHandle<T> indirect_handle(DirectHandle<T> handle, LocalHeap* local_heap)
{
    if (handle.is_null())
        return IndirectHandle<T>();
    return IndirectHandle<T>(HandleBase::indirect_handle(handle.address(), local_heap));
}
...
template <typename T> class V8_TRIVIAL_ABI DirectHandleUnchecked final : public DirectHandle<T> {
public:
    DirectHandleUnchecked()
        : DirectHandle<T>(DirectHandle<T>::do_not_check)
    {
    }
    ...
    // need to suppress it.
    DirectHandleUnchecked(const DirectHandleUnchecked& other) V8_NOEXCEPT : DirectHandle<T>(other, DirectHandle<T>::do_not_check)
    {
    }
    DirectHandleUnchecked& operator=(const DirectHandleUnchecked&) V8_NOEXCEPT = default;
#endif

    // Implicit conversion from DirectHandle.
    DirectHandleUnchecked(const DirectHandle<T>& other) V8_NOEXCEPT // NOLINT(runtime/explicit)
...
template <typename T> class StrongRootAllocator<DirectHandleUnchecked<T>> : public StrongRootAllocatorBase {
public:
...
private:
    ...
    template <typename IsolateT> static allocator_type make_allocator(IsolateT* isolate) noexcept
    {
...
public:
    ...
    using iterator = internal::WrappedIterator<typename vector_type::iterator, DirectHandle<T>>;
    using const_iterator = internal::WrappedIterator<typename vector_type::const_iterator, const DirectHandle<T>>;
    ...
    template <typename IsolateT>
    DirectHandleVector(IsolateT* isolate, std::initializer_list<DirectHandle<T>> init)
        : backing_(make_allocator(isolate))
    {
        if (init.size() == 0)
            return;
    ...
    DirectHandleVector<T>& operator=(std::initializer_list<DirectHandle<T>> init)
    {
        backing_.clear();
        backing_.reserve(init.size());
        backing_.insert(backing_.end(), init.begin(), init.end());
...
private:
    ...
    template <typename To, typename From> friend inline DirectHandle<To> UncheckedCast(DirectHandle<From> value);
    template <typename U> friend inline IndirectHandle<U> indirect_handle(DirectHandle<U>);
    template <typename U> friend inline IndirectHandle<U> indirect_handle(DirectHandle<U>, Isolate*);
    template <typename U> friend inline IndirectHandle<U> indirect_handle(DirectHandle<U>, LocalIsolate*);
    template <typename U> friend inline IndirectHandle<U> indirect_handle(DirectHandle<U>, LocalHeap*);

    IndirectHandle<T> handle_;
};

template <typename T> V8_INLINE IndirectHandle<T> indirect_handle(DirectHandle<T> handle)
{
    return handle.handle_;
}

template <typename T> V8_INLINE IndirectHandle<T> indirect_handle(DirectHandle<T> handle, Isolate* isolate)
{
    return handle.handle_;
}

template <typename T> V8_INLINE IndirectHandle<T> indirect_handle(DirectHandle<T> handle, LocalIsolate* isolate)
{
    return handle.handle_;
}

template <typename T> V8_INLINE IndirectHandle<T> indirect_handle(DirectHandle<T> handle, LocalHeap* local_heap)
{
    return handle.handle_;
}

template <typename T> class DirectHandleVector : public std::vector<DirectHandle<T>> {
public:
    template <typename IsolateT>
    explicit DirectHandleVector(IsolateT* isolate)
...
template <typename T> std::ostream& operator<<(std::ostream& os, DirectHandle<T> handle);

template <typename T> struct is_direct_handle<DirectHandle<T>> : public std::true_type { };
