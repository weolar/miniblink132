...
{
    SLOW_DCHECK((this->location_ == nullptr || this->IsDereferenceAllowed()) && (that.location_ == nullptr || that.IsDereferenceAllowed()));
...
template <typename T> Handle<T> Handle<T>::New(Tagged<T> object, Isolate* isolate)
{
...
template <typename T> V8_INLINE Handle<T> handle(Tagged<T> object, Isolate* isolate)
{
    return Handle<T>(object, isolate);
}

template <typename T> V8_INLINE Handle<T> handle(Tagged<T> object, LocalIsolate* isolate)
{
    return Handle<T>(object, isolate);
}

template <typename T> V8_INLINE Handle<T> handle(Tagged<T> object, LocalHeap* local_heap)
{
    return Handle<T>(object, local_heap);
}

template <typename T> V8_INLINE Handle<T> handle(T object, Isolate* isolate)
{
    static_assert(kTaggedCanConvertToRawObjects);
    return handle(Tagged<T>(object), isolate);
}

template <typename T> V8_INLINE Handle<T> handle(T object, LocalIsolate* isolate)
{
    static_assert(kTaggedCanConvertToRawObjects);
    return handle(Tagged<T>(object), isolate);
}

template <typename T> V8_INLINE Handle<T> handle(T object, LocalHeap* local_heap)
{
    static_assert(kTaggedCanConvertToRawObjects);
    return handle(Tagged<T>(object), local_heap);
}

template <typename T> inline std::ostream& operator<<(std::ostream& os, Handle<T> handle)
{
...
template <typename T> inline std::ostream& operator<<(std::ostream& os, DirectHandle<T> handle)
{
    return os << Brief(*handle);
}

template <typename T> V8_INLINE DirectHandle<T> direct_handle(Tagged<T> object, Isolate* isolate)
{
    return DirectHandle<T>(object, isolate);
}

template <typename T> V8_INLINE DirectHandle<T> direct_handle(Tagged<T> object, LocalIsolate* isolate)
{
    return DirectHandle<T>(object, isolate);
}

template <typename T> V8_INLINE DirectHandle<T> direct_handle(Tagged<T> object, LocalHeap* local_heap)
{
    return DirectHandle<T>(object, local_heap);
}

template <typename T> V8_INLINE DirectHandle<T> direct_handle(T object, Isolate* isolate)
{
    static_assert(kTaggedCanConvertToRawObjects);
    return direct_handle(Tagged<T>(object), isolate);
}

template <typename T> V8_INLINE DirectHandle<T> direct_handle(T object, LocalIsolate* isolate)
{
    static_assert(kTaggedCanConvertToRawObjects);
    return direct_handle(Tagged<T>(object), isolate);
}

template <typename T> V8_INLINE DirectHandle<T> direct_handle(T object, LocalHeap* local_heap)
{
    static_assert(kTaggedCanConvertToRawObjects);
...
HandleScope::HandleScope(HandleScope&& other) V8_NOEXCEPT : isolate_(other.isolate_), prev_next_(other.prev_next_), prev_limit_(other.prev_limit_)
{
...
template <typename T, template <typename> typename HandleType, typename> HandleType<T> HandleScope::CloseAndEscape(HandleType<T> handle_value)
{
...
Address* HandleScope::CreateHandle(Isolate* isolate, Address value)
{
    ...
    DCHECK_LT(reinterpret_cast<Address>(result), reinterpret_cast<Address>(data->limit));
...
{
    SLOW_DCHECK((this->location_ == nullptr || this->IsDereferenceAllowed()) && (that.address() == kTaggedNullAddress || that.IsDereferenceAllowed()));
...
{
    SLOW_DCHECK((this->address() == kTaggedNullAddress || this->IsDereferenceAllowed()) && (that.location_ == nullptr || that.IsDereferenceAllowed()));
...
{
    SLOW_DCHECK(
        (this->address() == kTaggedNullAddress || this->IsDereferenceAllowed()) && (that.address() == kTaggedNullAddress || that.IsDereferenceAllowed()));
    if (this->address() == kTaggedNullAddress && that.address() == kTaggedNullAddress)
        return true;
    if (this->address() == kTaggedNullAddress || that.address() == kTaggedNullAddress)
        return false;
