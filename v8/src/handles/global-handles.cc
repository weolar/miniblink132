...
template <class _NodeType> class GlobalHandles::NodeBlock final {
public:
    ...
    NodeBlock(GlobalHandles* global_handles, GlobalHandles::NodeSpace<NodeType>* space, NodeBlock* next) V8_NOEXCEPT : next_(next),
...
template <class NodeType> const GlobalHandles::NodeBlock<NodeType>* GlobalHandles::NodeBlock<NodeType>::From(const NodeType* node)
{
...
template <class NodeType> GlobalHandles::NodeBlock<NodeType>* GlobalHandles::NodeBlock<NodeType>::From(NodeType* node)
{
...
template <class NodeType> bool GlobalHandles::NodeBlock<NodeType>::IncreaseUsage()
{
    DCHECK_LT(used_nodes_, kBlockSize);
    return used_nodes_++ == 0;
}

template <class NodeType> void GlobalHandles::NodeBlock<NodeType>::ListAdd(BlockType** top)
{
...
template <class NodeType> bool GlobalHandles::NodeBlock<NodeType>::DecreaseUsage()
{
    DCHECK_GT(used_nodes_, 0);
    return --used_nodes_ == 0;
}

template <class NodeType> void GlobalHandles::NodeBlock<NodeType>::ListRemove(BlockType** top)
{
...
template <class BlockType> class GlobalHandles::NodeIterator final {
public:
    ...
    NodeIterator(NodeIterator&& other) V8_NOEXCEPT : block_(other.block_), index_(other.index_)
    {
    }
...
template <class NodeType> class GlobalHandles::NodeSpace final {
public:
    ...
    explicit NodeSpace(GlobalHandles* global_handles) V8_NOEXCEPT : global_handles_(global_handles)
    {
    }
    ~NodeSpace();
    ...
    iterator begin()
    {
        return iterator(first_used_block_);
    }
    iterator end()
    {
    ...
    {
        return blocks_ * sizeof(NodeType) * kBlockSize;
    }
    size_t handles_count() const
    {
        return handles_count_;
    }
...
template <class NodeType> GlobalHandles::NodeSpace<NodeType>::~NodeSpace()
{
...
template <class NodeType> NodeType* GlobalHandles::NodeSpace<NodeType>::Allocate()
{
...
template <class NodeType> void GlobalHandles::NodeSpace<NodeType>::PutNodesOnFreeList(BlockType* block)
{
...
template <class NodeType> void GlobalHandles::NodeSpace<NodeType>::Release(NodeType* node)
{
    BlockType* block = BlockType::From(node);
    block->space()->Free(node);
}

template <class NodeType> void GlobalHandles::NodeSpace<NodeType>::Free(NodeType* node)
{
    CHECK(node->IsInUse());
...
template <class Child> class NodeBase {
public:
    ...
    IndirectHandle<Object> Publish(Tagged<Object> object)
    {
        ...
        AsChild()->MarkAsUsed();
        reinterpret_cast<std::atomic<Address>*>(&object_)->store(object.ptr(), std::memory_order_release);
        DCHECK(AsChild()->IsInUse());
        return handle();
    }
    ...
    {
        return IndirectHandle<Object>(&object_);
    }
...
void ExtractInternalFields(Tagged<JSObject> jsobject, void** embedder_fields, int len)
{
...
public:
    ...
    {
        return state() == NORMAL ? reinterpret_cast<char*>(data_.parameter) : nullptr;
    }
    ...
    {
        DCHECK(weakness_type() == WeaknessType::kCallback || weakness_type() == WeaknessType::kCallbackWithTwoEmbedderFields);
        DCHECK_NOT_NULL(weak_callback_);

        void* embedder_fields[v8::kEmbedderFieldsInWeakCallback] = { nullptr, nullptr };
        if (weakness_type() == WeaknessType::kCallbackWithTwoEmbedderFields && IsJSObject(object())) {
            ExtractInternalFields(Cast<JSObject>(object()), embedder_fields, v8::kEmbedderFieldsInWeakCallback);
        }

        // Zap with something dangerous.
        location().store(Tagged<Object>(0xCA11));

        pending_phantom_callbacks->push_back(std::make_pair(this, PendingPhantomCallback(weak_callback_, parameter(), embedder_fields)));
        DCHECK(IsInUse());
...
{
    return regular_nodes_->TotalSize();
}
...
template <typename NodeType> bool NeedsTrackingInYoungNodes(Tagged<Object> value, NodeType* node)
{
...
void GlobalHandles::MakeWeak(Address* location, void* parameter, GenericCallback phantom_callback, v8::WeakCallbackType type)
{
...
void GlobalHandles::AnnotateStrongRetainer(Address* location, const char* label)
{
...
V8_INLINE bool GlobalHandles::ResetWeakNodeIfDead(Node* node, WeakSlotCallbackWithHeap should_reset_handle)
{
...
DISABLE_CFI_PERF
void GlobalHandles::IterateWeakRootsForPhantomHandles(WeakSlotCallbackWithHeap should_reset_handle)
{
...
void GlobalHandles::ProcessWeakYoungObjects(RootVisitor* v, WeakSlotCallbackWithHeap should_reset_handle)
{
    for (Node* node : young_nodes_) {
...
void GlobalHandles::InvokeSecondPassPhantomCallbacks()
{
    ...
    if (scope.CheckReenter()) {
        ...
        isolate()->heap()->CallGCPrologueCallbacks(GCType::kGCTypeProcessWeakCallbacks, kNoGCCallbackFlags, GCTracer::Scope::HEAP_EXTERNAL_PROLOGUE);
        {
            TRACE_GC(isolate_->heap()->tracer(), GCTracer::Scope::HEAP_EXTERNAL_SECOND_PASS_CALLBACKS);
        ...
        isolate()->heap()->CallGCEpilogueCallbacks(GCType::kGCTypeProcessWeakCallbacks, kNoGCCallbackFlags, GCTracer::Scope::HEAP_EXTERNAL_EPILOGUE);
    }
}

namespace {

template <typename T> void UpdateListOfYoungNodesImpl(Isolate* isolate, std::vector<T*>* node_list)
{
    size_t last = 0;
...
template <typename T> void ClearListOfYoungNodesImpl(Isolate* isolate, std::vector<T*>* node_list)
{
    for (T* node : *node_list) {
        DCHECK(node->is_in_young_list());
        node->set_in_young_list(false);
        DCHECK_IMPLIES(node->IsInUse() && node->state() != T::NEAR_DEATH, !HeapLayout::InYoungGeneration(node->object()));
    }
    isolate->heap()->IncrementNodesDiedInNewSpace(static_cast<int>(node_list->size()));
    node_list->clear();
    node_list->shrink_to_fit();
}
...
size_t GlobalHandles::InvokeFirstPassWeakCallbacks()
{
    ...
    if (pending_phantom_callbacks_.empty())
        return 0;

    TRACE_GC(isolate()->heap()->tracer(), GCTracer::Scope::HEAP_EXTERNAL_WEAK_GLOBAL_HANDLES);
...
void GlobalHandles::PendingPhantomCallback::Invoke(Isolate* isolate, InvocationType type)
{
    Data::Callback* callback_addr = nullptr;
    if (type == kFirstPass) {
        callback_addr = &callback_;
    }
    Data data(reinterpret_cast<v8::Isolate*>(isolate), parameter_, embedder_fields_, callback_addr);
    Data::Callback callback = callback_;
    callback_ = nullptr;
    callback(data);
}

void GlobalHandles::PostGarbageCollectionProcessing(v8::GCCallbackFlags gc_callback_flags)
{
    ...
    const bool synchronous_second_pass = v8_flags.optimize_for_size || v8_flags.predictable || isolate_->heap()->IsTearingDown()
        || (gc_callback_flags & (kGCCallbackFlagForced | kGCCallbackFlagCollectAllAvailableGarbage | kGCCallbackFlagSynchronousPhantomCallbackProcessing)) != 0;
    ...
    if (!second_pass_callbacks_task_posted_) {
        ...
        V8::GetCurrentPlatform()->GetForegroundTaskRunner(reinterpret_cast<v8::Isolate*>(isolate()))->PostTask(MakeCancelableTask(isolate(), [this] {
...
DISABLE_CFI_PERF
void GlobalHandles::ApplyPersistentHandleVisitor(v8::PersistentHandleVisitor* visitor, GlobalHandles::Node* node)
{
    v8::Value* value = ToApi<v8::Value>(node->handle());
    visitor->VisitPersistentHandle(reinterpret_cast<v8::Persistent<v8::Value>*>(&value), node->wrapper_class_id());
}

void GlobalHandles::IterateAllRootsForTesting(v8::PersistentHandleVisitor* visitor)
{
    for (Node* node : *regular_nodes_) {
        if (node->IsWeakOrStrongRetainer()) {
...
void GlobalHandles::Print()
{
    ...
    for (Node* node : *regular_nodes_) {
        PrintF("  handle %p to %p%s\n", node->location().ToVoidPtr(), reinterpret_cast<void*>(node->object().ptr()), node->IsWeak() ? " (weak)" : "");
    }
}
...
void EternalHandles::IterateAllRoots(RootVisitor* visitor)
{
    ...
    for (Address* block : blocks_) {
        DCHECK_GT(limit, 0);
        visitor->VisitRootPointers(Root::kEternalHandles, nullptr, FullObjectSlot(block), FullObjectSlot(block + std::min({ limit, kSize })));
        limit -= kSize;
    }
}
...
void EternalHandles::IterateYoungRoots(RootVisitor* visitor)
{
    for (int index : young_node_indices_) {
        visitor->VisitRootPointer(Root::kEternalHandles, nullptr, FullObjectSlot(GetLocation(index)));
    }
}
...
void EternalHandles::Create(Isolate* isolate, Tagged<Object> object, int* index)
{
    DCHECK_EQ(kInvalidIndex, *index);
    if (object == Tagged<Object>())
        return;
