...
template <typename T> class MaybeHandle final {
public:
    ...
    template <typename S> V8_WARN_UNUSED_RESULT V8_INLINE bool ToHandle(Handle<S>* out) const
    {
    ...
    template <typename S> V8_WARN_UNUSED_RESULT V8_INLINE bool ToHandle(DirectHandle<S>* out) const;
...
protected:
    V8_INLINE explicit MaybeHandle(Address* location)
        : location_(location)
    {
    }
    ...
    template <typename To, typename From> friend inline MaybeHandle<To> UncheckedCast(MaybeHandle<From> value);
};

template <typename T> std::ostream& operator<<(std::ostream& os, MaybeHandle<T> handle);
...
private:
    inline MaybeObjectHandle(Tagged<Object> object, HeapObjectReferenceType reference_type, Isolate* isolate);
    inline MaybeObjectHandle(Handle<Object> object, HeapObjectReferenceType reference_type);
...
template <typename T> class MaybeDirectHandle final {
public:
    ...
    V8_INLINE MaybeDirectHandle(MaybeIndirectHandle<S> maybe_handle)
        : location_(maybe_handle.location_ == nullptr ? kTaggedNullAddress : *maybe_handle.location_)
    {
    }
    ...
    V8_INLINE void Assert() const
    {
        DCHECK_NE(location_, kTaggedNullAddress);
    }
    V8_INLINE void Check() const
    {
        CHECK_NE(location_, kTaggedNullAddress);
    }
    ...
    bool is_null() const
    {
        return location_ == kTaggedNullAddress;
    }
...
protected:
    ...
    template <typename> friend class MaybeHandle;
    // Casts are allowed to access location_.
    template <typename To, typename From> friend inline MaybeDirectHandle<To> UncheckedCast(MaybeDirectHandle<From> value);
};
...
private:
    ...
    template <typename To, typename From> friend inline MaybeDirectHandle<To> UncheckedCast(MaybeDirectHandle<From> value);
    template <typename U> friend inline MaybeIndirectHandle<U> indirect_handle(MaybeDirectHandle<U>, Isolate*);
    template <typename U> friend inline MaybeIndirectHandle<U> indirect_handle(MaybeDirectHandle<U>, LocalIsolate*);
...
public:
    ...
    inline MaybeObjectDirectHandle(Tagged<MaybeObject> object, LocalHeap* local_heap);
    ...
    static inline MaybeObjectDirectHandle Weak(Tagged<Object> object, Isolate* isolate);
...
private:
    inline MaybeObjectDirectHandle(Tagged<Object> object, HeapObjectReferenceType reference_type, Isolate* isolate);
    inline MaybeObjectDirectHandle(DirectHandle<Object> object, HeapObjectReferenceType reference_type);
