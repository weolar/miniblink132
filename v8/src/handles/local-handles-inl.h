...
// static
V8_INLINE Address* LocalHandleScope::GetHandle(LocalHeap* local_heap, Address value)
{
...
template <typename T, template <typename> typename HandleType, typename> HandleType<T> LocalHandleScope::CloseAndEscape(HandleType<T> handle_value)
{
...
void LocalHandleScope::CloseScope(LocalHeap* local_heap, Address* prev_next, Address* prev_limit)
{
    LocalHandles* handles = local_heap->handles();
    ...
    MSAN_ALLOCATED_UNINITIALIZED_MEMORY(
        handles->scope_.next, static_cast<size_t>(reinterpret_cast<Address>(old_limit) - reinterpret_cast<Address>(handles->scope_.next)));
}
