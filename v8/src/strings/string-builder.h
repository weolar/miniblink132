...
public:
    ...
    static inline void AddSubjectSlice(FixedArrayBuilder* builder, int from, int to);
...
public:
    ...
    template <typename SrcChar> void AppendSubstring(const SrcChar* src, size_t from, size_t to);
    ...
    template <typename DestChar> class NoExtend {
    public:
    ...
    template <typename DestChar> class NoExtendBuilder : public NoExtend<DestChar> {
    public:
        inline NoExtendBuilder(IncrementalStringBuilder* builder, int required_length, const DisallowGarbageCollection& no_gc);
