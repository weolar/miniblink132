...
template <class T, int size = 256> class Predicate {
public:
...
private:
    ...
    public:
        ...
        inline CacheEntry(uchar code_point, bool value)
            : bit_field_(CodePointField::encode(CodePointField::kMask & code_point) | ValueField::encode(value))
        {
            DCHECK_IMPLIES((CodePointField::kMask & code_point) != code_point, code_point == static_cast<uchar>(-1));
        }
...
template <class T, int size = 256> class Mapping {
public:
...
public:
    ...
    static inline bool HasUnpairedSurrogate(const uint16_t* code_units, size_t length);
...
public:
    ...
    static inline uchar ValueOf(const uint8_t* str, size_t length, size_t* cursor);

    using Utf8IncrementalBuffer = uint32_t;
    static inline uchar ValueOfIncremental(const uint8_t** cursor, State* state, Utf8IncrementalBuffer* buffer);
...
public:
    ...
    static void ScanForSurrogates(v8::base::Vector<const uint8_t> wtf8, std::vector<size_t>* surrogate_offsets);
};
