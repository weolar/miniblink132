...
void FixedArrayBuilder::EnsureCapacity(Isolate* isolate, int elements)
{
    ...
    if (length < required_length) {
        if (length == 0) {
            ...
            array_ = isolate->factory()->NewFixedArrayWithHoles(std::max(kInitialCapacityForLazy, elements));
            return;
        }
        ...
        DirectHandle<FixedArray> extended_array = isolate->factory()->NewFixedArrayWithHoles(new_length);
...
ReplacementStringBuilder::ReplacementStringBuilder(Heap* heap, DirectHandle<String> subject, int estimated_part_count)
    : heap_(heap)
...
MaybeDirectHandle<String> ReplacementStringBuilder::ToString()
{
    ...
    if (is_one_byte_) {
        ...
        ASSIGN_RETURN_ON_EXCEPTION(isolate, seq, isolate->factory()->NewRawOneByteString(character_count_));

        DisallowGarbageCollection no_gc;
        uint8_t* char_buffer = seq->GetChars(no_gc);
        StringBuilderConcatHelper(*subject_, char_buffer, *array_builder_.array(), array_builder_.length());
        joined_string = Cast<String>(seq);
    } else {
        // Two-byte.
        DirectHandle<SeqTwoByteString> seq;
        ASSIGN_RETURN_ON_EXCEPTION(isolate, seq, isolate->factory()->NewRawTwoByteString(character_count_));
    ...
    , current_index_(0)
{
    ...
    accumulator_ = DirectHandle<String>::New(ReadOnlyRoots(isolate).empty_string(), isolate);
...
void IncrementalStringBuilder::Accumulate(DirectHandle<String> new_part)
{
    ...
    if (accumulator()->length() + new_part->length() > String::kMaxLength) {
        ...
        new_accumulator = factory()->NewConsString(indirect_handle(accumulator(), isolate_), indirect_handle(new_part, isolate_)).ToHandleChecked();
    }
...
MaybeDirectHandle<String> IncrementalStringBuilder::Finish()
{
    ...
    if (isolate()->serializer_enabled()) {
        return factory()->InternalizeString(indirect_handle(accumulator(), isolate_));
    }
    return accumulator();
}
...
void IncrementalStringBuilder::AppendStringByCopy(DirectHandle<String> string)
{
...

    {
        DisallowGarbageCollection no_gc;
        if (encoding_ == String::ONE_BYTE_ENCODING) {
            String::WriteToFlat(*string, Cast<SeqOneByteString>(current_part())->GetChars(no_gc) + current_index_, 0, string->length());
        } else {
            String::WriteToFlat(*string, Cast<SeqTwoByteString>(current_part())->GetChars(no_gc) + current_index_, 0, string->length());
        }
    }
