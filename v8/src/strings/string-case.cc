...
template <bool is_lower> uint32_t FastAsciiConvert(char* dst, const char* src, uint32_t length, bool* changed_out)
{
    ...
    DCHECK(CheckFastAsciiConvert(saved_dst, saved_src, length, changed, is_lower));

    *changed_out = changed;
    return length;
}

template uint32_t FastAsciiConvert<false>(char* dst, const char* src, uint32_t length, bool* changed_out);
template uint32_t FastAsciiConvert<true>(char* dst, const char* src, uint32_t length, bool* changed_out);
