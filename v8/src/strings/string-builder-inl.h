...
using StringBuilderSubstringLength = base::BitField<int, 0, kStringBuilderConcatHelperLengthBits>;
using StringBuilderSubstringPosition = base::BitField<int, kStringBuilderConcatHelperLengthBits, kStringBuilderConcatHelperPositionBits>;

template <typename sinkchar> void StringBuilderConcatHelper(Tagged<String> special, sinkchar* sink, Tagged<FixedArray> fixed_array, int array_length);
...
// static
inline void ReplacementStringBuilder::AddSubjectSlice(FixedArrayBuilder* builder, int from, int to)
{
    DCHECK_GE(from, 0);
    int length = to - from;
    DCHECK_GT(length, 0);
...
template <typename SrcChar, typename DestChar> void IncrementalStringBuilder::Append(SrcChar c)
{
    DCHECK_EQ(encoding_ == String::ONE_BYTE_ENCODING, sizeof(DestChar) == 1);
    if (sizeof(DestChar) == 1) {
        DCHECK_EQ(String::ONE_BYTE_ENCODING, encoding_);
        Cast<SeqOneByteString>(*current_part_)->SeqOneByteStringSet(current_index_++, c);
    } else {
        DCHECK_EQ(String::TWO_BYTE_ENCODING, encoding_);
        Cast<SeqTwoByteString>(*current_part_)->SeqTwoByteStringSet(current_index_++, c);
    }
...
template <int N> V8_INLINE void IncrementalStringBuilder::AppendCStringLiteral(const char (&literal)[N])
{
    ...
    if (encoding_ == String::ONE_BYTE_ENCODING && CurrentPartCanFit(N)) {
        ...
        Cast<SeqOneByteString>(*current_part_)->SeqOneByteStringSetChars(current_index_, chars, length);
...
template <typename SrcChar> V8_INLINE void IncrementalStringBuilder::AppendCString(const SrcChar* s)
{
    if (encoding_ == String::ONE_BYTE_ENCODING) {
        while (*s != '\0')
...
V8_INLINE void IncrementalStringBuilder::AppendInt(int i)
{
    ...
    const char* str = IntToCString(i, base::Vector<char>(buffer, kIntToCStringBufferSize));
    AppendCString(str);
}

V8_INLINE int IncrementalStringBuilder::EscapedLengthIfCurrentPartFits(int length)
{
...
inline IncrementalStringBuilder::NoExtend<DestChar>::NoExtend(Tagged<String> string, int offset, const DisallowGarbageCollection& no_gc)
{
    DCHECK(IsSeqOneByteString(string) || IsSeqTwoByteString(string));
    if (sizeof(DestChar) == 1) {
        start_ = reinterpret_cast<DestChar*>(Cast<SeqOneByteString>(string)->GetChars(no_gc) + offset);
    } else {
...
template <typename DestChar> inline IncrementalStringBuilder::NoExtend<DestChar>::~NoExtend()
{
    ...
    if (sizeof(DestChar) == 1) {
        ...
        end = reinterpret_cast<DestChar*>(one_byte_string->GetChars(no_gc_) + one_byte_string->length());
    } else {
        auto two_byte_string = Cast<SeqTwoByteString>(string_);
        end = reinterpret_cast<DestChar*>(two_byte_string->GetChars(no_gc_) + two_byte_string->length());
    }
...
inline IncrementalStringBuilder::NoExtendBuilder<DestChar>::NoExtendBuilder(
    IncrementalStringBuilder* builder, int required_length, const DisallowGarbageCollection& no_gc)
    : NoExtend<DestChar>(*(builder->current_part()), builder->current_index_, no_gc)
    , builder_(builder)
{
...
V8_INLINE void IncrementalStringBuilder::ShrinkCurrentPart()
{
    ...
    set_current_part(SeqString::Truncate(isolate_, indirect_handle(Cast<SeqString>(current_part()), isolate_), current_index_));
}
