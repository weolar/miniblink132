...
template <class T, int s> bool Predicate<T, s>::CalculateValue(uchar code_point)
{
...
template <class T, int s> int Mapping<T, s>::get(uchar c, uchar n, uchar* result)
{
...
template <class T, int s> int Mapping<T, s>::CalculateValue(uchar c, uchar n, uchar* result)
{
...
bool Utf16::HasUnpairedSurrogate(const uint16_t* code_units, size_t length)
{
    for (size_t i = 0; i < length; ++i) {
        ...
        if (IsLeadSurrogate(code_unit)) {
            ...
            if (i == length - 1)
                return true;
            if (!IsTrailSurrogate(code_units[i + 1]))
                return true;
...
uchar Utf8::ValueOfIncremental(const uint8_t** cursor, State* state, Utf8IncrementalBuffer* buffer)
{
...
unsigned Utf8::Encode(char* str, uchar c, int previous, bool replace_invalid)
{
    ...
    if (c <= kMaxOneByteChar) {
        ...
        if (Utf16::IsSurrogatePair(previous, c)) {
            ...
            return Encode(str - kUnmatchedSize, Utf16::CombineSurrogatePair(previous, c), Utf16::kNoPreviousCharacter, replace_invalid) - kUnmatchedSize;
