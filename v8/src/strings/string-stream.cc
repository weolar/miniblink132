...
void StringStream::Add(base::Vector<const char> format, base::Vector<FmtElm> elms)
{
...
{
    return isolate->factory()->NewStringFromUtf8(base::Vector<const char>(buffer_, length_)).ToHandleChecked();
}
...
void StringStream::PrintUsingMap(Tagged<JSObject> js_object)
{
    ...
    Tagged<DescriptorArray> descs = map->instance_descriptors(js_object->GetIsolate());
...
void StringStream::PrintFixedArray(Tagged<FixedArray> array, unsigned int limit)
{
...
void StringStream::PrintMentionedObjectCache(Isolate* isolate)
{
    ...
    for (size_t i = 0; i < debug_object_cache->size(); i++) {
        ...
        Add(" #%d# %p: ", static_cast<int>(i), reinterpret_cast<void*>(printee.ptr()));
        ...
        if (IsJSObject(printee)) {
            ...
            if (IsJSArray(printee)) {
                ...
                if (array->HasObjectElements()) {
                    ...
                    unsigned int length = static_cast<uint32_t>(Object::NumberValue(Cast<JSArray>(array)->length()));
                    if (length < limit)
                        limit = length;
...
void StringStream::PrintFunction(Tagged<JSFunction> fun, Tagged<Object> receiver)
{
    PrintPrototype(fun, receiver);
}

void StringStream::PrintPrototype(Tagged<JSFunction> fun, Tagged<Object> receiver)
{
    Tagged<Object> name = fun->shared()->Name();
    bool print_name = false;
    Isolate* isolate = fun->GetIsolate();
    if (IsNullOrUndefined(receiver, isolate) || IsTheHole(receiver, isolate) || IsJSProxy(receiver) || IsWasmObject(receiver)) {
        print_name = true;
    } else if (!isolate->context().is_null()) {
        if (!IsJSObject(receiver)) {
            receiver = Object::GetPrototypeChainRootMap(receiver, isolate)->prototype();
        }

        for (PrototypeIterator iter(isolate, Cast<JSObject>(receiver), kStartAtReceiver); !iter.IsAtEnd(); iter.Advance()) {
            if (!IsJSObject(iter.GetCurrent()))
                break;
            Tagged<Object> key = iter.GetCurrent<JSObject>()->SlowReverseLookup(fun);
            if (!IsUndefined(key, isolate)) {
                if (!IsString(name) || !IsString(key) || !Cast<String>(name)->Equals(Cast<String>(key))) {
                    print_name = true;
                }
