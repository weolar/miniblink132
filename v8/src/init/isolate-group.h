...
public:
    ...
    int IncrementIsolateCount()
    {
        return ++isolate_count_;
    }
    int DecrementIsolateCount()
    {
        return --isolate_count_;
    }
    ...
    static IsolateGroup* current()
    {
        return GetProcessWideIsolateGroup();
    }
#endif // !V8_COMPRESS_POINTERS_IN_MULTIPLE_CAGES

    MemorySpan<Address> external_ref_table()
    {
        return external_ref_table_;
    }
...
private:
    ...
    int IsolateCount() const
    {
        return isolate_count_.load();
    }
