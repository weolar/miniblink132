...
bool SetupIsolateDelegate::SetupHeap(Isolate* isolate, bool create_heap_objects)
{
    ...
    CHECK_WITH_MSG(!create_heap_objects, "Heap setup supported only in mksnapshot");
    return true;
}
...
void SetupIsolateDelegate::SetupBuiltins(Isolate* isolate, bool compile_builtins)
{
    ...
    CHECK_WITH_MSG(!compile_builtins, "Builtin compilation supported only in mksnapshot");
}
