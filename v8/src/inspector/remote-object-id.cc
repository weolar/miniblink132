...
String16 serializeId(uint64_t isolateId, int injectedScriptId, int id)
{
    return String16::concat(
        String16::fromInteger64(static_cast<int64_t>(isolateId)), ".", String16::fromInteger(injectedScriptId), ".", String16::fromInteger(id));
}
...
bool RemoteObjectIdBase::parseId(const String16& objectId)
{
    ...
    if (firstDotPos == String16::kNotFound)
        return false;
    bool ok = false;
    ...
    if (secondDotPos == String16::kNotFound)
        return false;
    int injectedScriptId = objectId.substring(firstDotPos, secondDotPos - firstDotPos).toInteger(&ok);
    if (!ok)
        return false;
    secondDotPos++;
...
Response RemoteObjectId::parse(const String16& objectId, std::unique_ptr<RemoteObjectId>* result)
{
...
String16 RemoteObjectId::serialize(uint64_t isolateId, int injectedScriptId, int id)
{
    return serializeId(isolateId, injectedScriptId, id);
}

Response RemoteCallFrameId::parse(const String16& objectId, std::unique_ptr<RemoteCallFrameId>* result)
{
...
String16 RemoteCallFrameId::serialize(uint64_t isolateId, int injectedScriptId, int frameOrdinal)
{
