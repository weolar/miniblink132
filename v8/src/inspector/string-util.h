...
public:
    ...
    {
        //return Binary(std::make_shared<std::vector<uint8_t>>(span.begin(), span.end()));
        std::shared_ptr<std::vector<uint8_t>> bytes = std::make_shared<std::vector<uint8_t>>();
        bytes->resize(span.size());
        memcpy(bytes->data(), span.data(), bytes->size());
        return Binary(bytes);
    }
...
template <size_t N> StringView toStringView(const char* str[N])
{
...
template <> struct ProtocolTypeTraits<v8_inspector::String16> {
...
template <> struct ProtocolTypeTraits<v8_inspector::protocol::Binary> {
...
namespace detail {
template <> struct MaybeTypedef<v8_inspector::String16> {
    typedef ValueMaybe<v8_inspector::String16> type;
};

template <> struct MaybeTypedef<v8_inspector::protocol::Binary> {
