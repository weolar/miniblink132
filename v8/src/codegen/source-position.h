...
public:
    explicit SourcePosition(int script_offset = kNoSourcePosition, int inlining_id = kNotInlined)
        : value_(0)
    {
        SetIsExternal(false);
    ...
    bool IsKnown() const
    {
        return raw() != SourcePosition::Unknown().raw();
    }
    ...
    {
        return IsExternalField::decode(value_);
    }
    ...
    std::vector<SourcePositionInfo> InliningStack(Isolate* isolate, OptimizedCompilationInfo* cinfo) const;
...
struct SourcePositionInfo {
    SourcePositionInfo(Isolate* isolate, SourcePosition pos, Handle<SharedFunctionInfo> f);
