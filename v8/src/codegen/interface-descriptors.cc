...
void CallInterfaceDescriptorData::InitializeRegisters(Flags flags, CodeEntrypointTag tag, int return_count, int parameter_count, StackArgumentOrder stack_order,
...
void CallInterfaceDescriptorData::InitializeTypes(const MachineType* machine_types, int machine_types_length)
{
...
// static
CallInterfaceDescriptorData CallDescriptors::call_descriptor_data_[NUMBER_OF_DESCRIPTORS];

void CallDescriptors::InitializeOncePerProcess()
{
#define INTERFACE_DESCRIPTOR(name, ...) name##Descriptor().Initialize(&call_descriptor_data_[CallDescriptors::name]);
...
void WriteBarrierDescriptor::Verify(CallInterfaceDescriptorData* data)
{
    ...
    // reduce code size:
    DCHECK(!AreAliased(ObjectRegister(), kContextRegister, kInterpreterAccumulatorRegister));
    DCHECK(!AreAliased(SlotAddressRegister(), kContextRegister, kInterpreterAccumulatorRegister));
    DCHECK(!AreAliased(ValueRegister(), kContextRegister, kInterpreterAccumulatorRegister));
...
// static
void IndirectPointerWriteBarrierDescriptor::Verify(CallInterfaceDescriptorData* data)
{
