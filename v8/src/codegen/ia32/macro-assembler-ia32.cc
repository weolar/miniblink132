...
void MacroAssembler::LoadRoot(Register destination, RootIndex index)
{
    ...
    ExternalReference isolate_root = ExternalReference::isolate_root(isolate());
    lea(destination, Operand(isolate_root.address(), RelocInfo::EXTERNAL_REFERENCE));
    mov(destination, Operand(destination, RootRegisterOffsetForRootIndex(index)));
}

void MacroAssembler::CompareRoot(Register with, Register scratch, RootIndex index)
{
    ASM_CODE_COMMENT(this);
    if (root_array_available()) {
...
void MacroAssembler::CompareRange(Register value, unsigned lower_limit, unsigned higher_limit, Register scratch)
{
    ASM_CODE_COMMENT(this);
...
void MacroAssembler::JumpIfIsInRange(
    Register value, unsigned lower_limit, unsigned higher_limit, Register scratch, Label* on_in_range, Label::Distance near_jump)
{
...
Operand MacroAssembler::ExternalReferenceAsOperand(ExternalReference reference, Register scratch)
{
    if (root_array_available()) {
        ...
        if (options().enable_root_relative_access) {
            intptr_t delta = RootRegisterOffsetForExternalReference(isolate(), reference);
...
void MacroAssembler::LoadFromConstantsTable(Register destination, int constant_index)
{
    ...
    mov(destination, FieldOperand(destination, FixedArray::OffsetOfElementAt(constant_index)));
}
...
void MacroAssembler::LoadAddress(Register destination, ExternalReference source)
{
    if (root_array_available()) {
        if (source.IsIsolateFieldId()) {
            lea(destination, Operand(kRootRegister, source.offset_from_root_register()));
            return;
        }
        if (options().isolate_independent_code) {
...
void MacroAssembler::RecordWriteField(Register object, int offset, Register value, Register slot_address, SaveFPRegsMode save_fp, SmiCheck smi_check)
{
    ASM_CODE_COMMENT(this);
...
void MacroAssembler::RecordWrite(Register object, Register slot_address, Register value, SaveFPRegsMode fp_mode, SmiCheck smi_check)
{
    ASM_CODE_COMMENT(this);
...
void MacroAssembler::LoadFeedbackVector(Register dst, Register closure, Register scratch, Label* fbv_undef, Label::Distance distance)
{
    Label done;
...
void MacroAssembler::CmpInstanceTypeRange(Register map, Register instance_type_out, Register scratch, InstanceType lower_limit, InstanceType higher_limit)
{
    ASM_CODE_COMMENT(this);
    DCHECK_LT(lower_limit, higher_limit);
...
{
    test(FieldOperand(code, Code::kFlagsOffset), Immediate(1 << Code::kMarkedForDeoptimizationBit));
}
...
void TailCallOptimizedCodeSlot(MacroAssembler* masm, Register optimized_code_entry)
{
    ...
    __ mov(optimized_code_entry, FieldOperand(optimized_code_entry, CodeWrapper::kCodeOffset));
    ...
    __ ReplaceClosureCodeWithOptimizedCode(optimized_code_entry, closure, edx, ecx);
...
void MacroAssembler::ReplaceClosureCodeWithOptimizedCode(Register optimized_code, Register closure, Register value, Register slot_address)
{
    ASM_CODE_COMMENT(this);
    ...
    RecordWriteField(closure, JSFunction::kCodeOffset, value, slot_address, SaveFPRegsMode::kIgnore, SmiCheck::kOmit);
}
...
void MacroAssembler::LoadFeedbackVectorFlagsAndJumpIfNeedsProcessing(
    Register flags, XMMRegister saved_feedback_vector, CodeKind current_code_kind, Label* flags_need_processing)
{
    ASM_CODE_COMMENT(this);
...
void MacroAssembler::OptimizeCodeOrTailCallOptimizedCodeSlot(Register flags, XMMRegister saved_feedback_vector)
{
    ASM_CODE_COMMENT(this);
    ...
    mov(optimized_code_entry, FieldOperand(feedback_vector, FeedbackVector::kMaybeOptimizedCodeOffset));
...
void MacroAssembler::AssertConstructor(Register object)
{
    if (v8_flags.debug_code) {
        ...
        test_b(FieldOperand(object, Map::kBitFieldOffset), Immediate(Map::Bits1::IsConstructorBit::kMask));
        Pop(object);
...
void MacroAssembler::AssertFunction(Register object, Register scratch)
{
    if (v8_flags.debug_code) {
        ...
        Push(object);
        LoadMap(object, object);
        CmpInstanceTypeRange(object, scratch, scratch, FIRST_JS_FUNCTION_TYPE, LAST_JS_FUNCTION_TYPE);
        Pop(object);
...
void MacroAssembler::AssertCallableFunction(Register object, Register scratch)
{
    if (v8_flags.debug_code) {
        ...
        Push(object);
        LoadMap(object, object);
        CmpInstanceTypeRange(object, scratch, scratch, FIRST_CALLABLE_JS_FUNCTION_TYPE, LAST_CALLABLE_JS_FUNCTION_TYPE);
        Pop(object);
...
void MacroAssembler::AssertGeneratorObject(Register object)
{
...

    {
        ...
        CmpInstanceTypeRange(map, map, map, FIRST_JS_GENERATOR_OBJECT_TYPE, LAST_JS_GENERATOR_OBJECT_TYPE);
        Pop(object);
    }
...
void MacroAssembler::LeaveFrame(StackFrame::Type type)
{
    ...
    if (v8_flags.debug_code && !StackFrame::IsJavaScript(type)) {
        cmp(Operand(ebp, CommonFrameConstants::kContextOrFrameTypeOffset), Immediate(StackFrame::TypeToMarker(type)));
...
void MacroAssembler::StackOverflowCheck(Register num_args, Register scratch, Label* stack_overflow, bool include_receiver)
{
    ASM_CODE_COMMENT(this);
    ...
    // limit" is checked.
    ExternalReference real_stack_limit = ExternalReference::address_of_real_jslimit(isolate());
...
void MacroAssembler::InvokePrologue(Register expected_parameter_count, Register actual_parameter_count, Label* done, InvokeType type)
{
    ...
    // receiver and the return address.
    {
        ...
        mov(src, esp);
        lea(scratch, Operand(expected_parameter_count, times_system_pointer_size, 0));
    ...
    LoadRoot(scratch, RootIndex::kUndefinedValue);
    {
        ...
        mov(Operand(edx, expected_parameter_count, times_system_pointer_size, 0), scratch);
...
void MacroAssembler::InvokeFunction(Register fun, Register new_target, Register actual_parameter_count, InvokeType type)
{
    ASM_CODE_COMMENT(this);
    // You can't call a function without a valid frame.
...
void MacroAssembler::LoadNativeContextSlot(Register destination, int index)
{
    ...
    mov(destination, FieldOperand(destination, Map::kConstructorOrBackPointerOrNativeContextOffset));
...
void MacroAssembler::Push(Immediate value)
{
    if (root_array_available()) {
        if (value.is_external_reference()) {
            ...
            if (options().isolate_independent_code) {
                push(kRootRegister);
                add(Operand(esp, 0), Immediate(RootRegisterOffsetForExternalReference(isolate(), reference)));
                return;
            }
        }
...
namespace {
bool ShouldUsePushPopForMove(bool root_array_available, bool isolate_independent_code, const Immediate& src)
{
    if (root_array_available) {
        if (src.is_external_reference() && src.external_reference().IsIsolateFieldId()) {
            return true;
        }
...
void MacroAssembler::Move(Operand dst, const Immediate& src)
{
    ...
    // stack.
    if (ShouldUsePushPopForMove(root_array_available(), options().isolate_independent_code, src)) {
        Push(src);
        pop(dst);
...
void MacroAssembler::EmitIncrementCounter(StatsCounter* counter, int value, Register scratch)
{
    DCHECK_GT(value, 0);
...
void MacroAssembler::EmitDecrementCounter(StatsCounter* counter, int value, Register scratch)
{
    DCHECK_GT(value, 0);
    if (v8_flags.native_code_counters && counter->Enabled()) {
        ASM_CODE_COMMENT(this);
...
int MacroAssembler::CallCFunction(ExternalReference function, int num_arguments, SetIsolateDataSlots set_isolate_data_slots, Label* return_location)
{
    ...
    return CallCFunction(eax, num_arguments, set_isolate_data_slots, return_location);
}
...
int MacroAssembler::CallCFunction(Register function, int num_arguments, SetIsolateDataSlots set_isolate_data_slots, Label* return_location)
{
    ...
    if (set_isolate_data_slots == SetIsolateDataSlots::kYes) {
        ...
        mov(ExternalReferenceAsOperand(IsolateFieldId::kFastCCallCallerPC), pc_scratch);
    ...
    if (set_isolate_data_slots == SetIsolateDataSlots::kYes) {
        ...
        mov(ExternalReferenceAsOperand(IsolateFieldId::kFastCCallCallerFP), Immediate(0));
    }
...
void MacroAssembler::LoadEntryFromBuiltinIndex(Register builtin_index, Register target)
{
    ...
    mov(target, Operand(kRootRegister, builtin_index, times_half_system_pointer_size, IsolateData::builtin_entry_table_offset()));
}
    ...
    int slots_to_drop_on_return, MemOperand* argc_operand, MemOperand return_value_operand)
{
    ...
    DCHECK(!AreAliased(return_value, scratch, prev_next_address_reg, prev_limit_reg));
...

    {
        ...
        __ mov(scratch, __ ExternalReferenceAsOperand(ER::exception_address(isolate), no_reg));
    ...
    if (argc_operand == nullptr) {
        ...
        __ lea(esp, Operand(esp, argc_reg, times_system_pointer_size, slots_to_drop_on_return * kSystemPointerSize));
        __ jmp(scratch);
    }
