...
Register GetRegisterThatIsNotOneOf(
    Register reg1, Register reg2 = no_reg, Register reg3 = no_reg, Register reg4 = no_reg, Register reg5 = no_reg, Register reg6 = no_reg);
...
public:
    ...
    void Debug(uint32_t parameters)
    {
        break_(parameters, false);
    }
#endif
...
#undef COND_ARGS
    ...
    inline void NegateBool(Register rd, Register rs)
    {
        Xor(rd, rs, 1);
    }
...
#define COND_ARGS Condition cond = al, Register rs = zero_reg, const Operand &rt = Operand(zero_reg)
...
private:
    template <typename... Rs> void push_helper(Register r, Rs... rs)
    {
        StoreWord(r, MemOperand(sp, sizeof...(rs) * kSystemPointerSize));
        push_helper(rs...);
    }

    template <> void push_helper(Register r)
    {
...
public:
    ...
    // address).
    template <typename... Rs> void Push(Register r, Rs... rs)
    {
...
private:
    template <typename... Rs> void pop_helper(Register r, Rs... rs)
    {
        pop_helper(rs...);
        LoadWord(r, MemOperand(sp, sizeof...(rs) * kSystemPointerSize));
    }

    template <> void pop_helper(Register r)
    {
...
public:
    ...
    // address).
    template <typename... Rs> void Pop(Register r, Rs... rs)
    {
        pop_helper(r, rs...);
...
#undef DEFINE_INSTRUCTION3
    ...
    // function).
    int CallCFunction(ExternalReference function, int num_arguments, SetIsolateDataSlots set_isolate_data_slots = SetIsolateDataSlots::kYes,
        Label* return_location = nullptr);
    int CallCFunction(
        Register function, int num_arguments, SetIsolateDataSlots set_isolate_data_slots = SetIsolateDataSlots::kYes, Label* return_location = nullptr);
...
#undef COND_ARGS
...
#elif V8_TARGET_ARCH_RISCV32
    void AddPair(Register dst_low, Register dst_high, Register left_low, Register left_high, Register right_low, Register right_high, Register scratch1,
        Register scratch2);

    void SubPair(Register dst_low, Register dst_high, Register left_low, Register left_high, Register right_low, Register right_high, Register scratch1,
        Register scratch2);
    ...
    void MulPair(Register dst_low, Register dst_high, Register left_low, Register left_high, Register right_low, Register right_high, Register scratch1,
        Register scratch2);
    ...
    void ShlPair(Register dst_low, Register dst_high, Register src_low, Register src_high, int32_t shift, Register scratch1, Register scratch2);
    ...
    void ShrPair(Register dst_low, Register dst_high, Register src_low, Register src_high, int32_t shift, Register scratch1, Register scratch2);
    ...
    void SarPair(Register dst_low, Register dst_high, Register src_low, Register src_high, int32_t shift, Register scratch1, Register scratch2);
#endif
    ...
    template <int NBYTES> void ReverseBytesHelper(Register rd, Register rs, Register tmp1, Register tmp2);
    ...
    template <int NBYTES, bool IS_SIGNED> void UnalignedLoadHelper(Register rd, const MemOperand& rs);
    template <int NBYTES> void UnalignedStoreHelper(Register rd, const MemOperand& rs, Register scratch_other = no_reg);

    template <int NBYTES> void UnalignedFLoadHelper(FPURegister frd, const MemOperand& rs, Register scratch);
    template <int NBYTES> void UnalignedFStoreHelper(FPURegister frd, const MemOperand& rs, Register scratch);
#if V8_TARGET_ARCH_RISCV32
    void UnalignedDoubleHelper(FPURegister frd, const MemOperand& rs, Register scratch_base);
    void UnalignedDStoreHelper(FPURegister frd, const MemOperand& rs, Register scratch);
#endif

    template <typename Reg_T, typename Func> void AlignedLoadHelper(Reg_T target, const MemOperand& rs, Func generator);
    template <typename Reg_T, typename Func> void AlignedStoreHelper(Reg_T value, const MemOperand& rs, Func generator);

    template <int NBYTES, bool LOAD_SIGNED> void LoadNBytes(Register rd, const MemOperand& rs, Register scratch);
    template <int NBYTES, bool LOAD_SIGNED> void LoadNBytesOverwritingBaseReg(const MemOperand& rs, Register scratch0, Register scratch1);
    // load/store macros
    ...
    inline void Load32U(Register rd, const MemOperand& rs, Trapper&& trapper = [](int) {})
    {
        Lwu(rd, rs, std::forward<Trapper>(trapper));
    }
    inline void LoadWord(Register rd, const MemOperand& rs, Trapper&& trapper = [](int) {})
    {
        Ld(rd, rs, std::forward<Trapper>(trapper));
    }
    inline void StoreWord(Register rd, const MemOperand& rs, Trapper&& trapper = [](int) {})
    {
    ...
    template <typename F> void FloatMinMaxHelper(FPURegister dst, FPURegister src1, FPURegister src2, MaxMinKind kind);
    ...
    {
        return has_single_zero_reg_set_;
    }
    ...
    inline void Move(Register dst, Tagged<Smi> smi)
    {
        li(dst, Operand(smi));
    }
    ...
    inline void Move(Register dst, FPURegister src)
    {
        fmv_x_d(dst, src);
    }

    inline void Move(FPURegister dst, Register src)
    {
        fmv_d_x(dst, src);
    }
#elif V8_TARGET_ARCH_RISCV32
    ...
    void AddOverflow64(Register dst, Register left, const Operand& right, Register overflow);
    // SubOverflow64 sets overflow register to a negative value if
    // overflow occured, otherwise it is zero or positive
    void SubOverflow64(Register dst, Register left, const Operand& right, Register overflow);
    // MIPS-style 32-bit unsigned mulh
    void Mulhu32(Register dst, Register left, const Operand& right, Register left_zero, Register right_zero);
#elif V8_TARGET_ARCH_RISCV32
    ...
    // MIPS-style 32-bit unsigned mulh
    void Mulhu(Register dst, Register left, const Operand& right, Register left_zero, Register right_zero);
#endif
    // MulOverflow32 sets overflow register to zero if no overflow occured
    void MulOverflow32(Register dst, Register left, const Operand& right, Register overflow);
    ...
    template <typename Func> void GenerateSwitchTable(Register index, size_t case_count, Func GetLabelFunction);
    ...
    void LoadTaggedField(const Register& destination, const MemOperand& field_operand, Trapper&& trapper = [](int) {});
    ...
    void StoreTaggedField(const Register& value, const MemOperand& dst_field_operand, Trapper&& trapper = [](int) {});
    ...
    void DecompressTagged(const Register& destination, const MemOperand& field_operand, Trapper&& trapper = [](int) {});
    ...
    void DecompressProtected(const Register& destination, const MemOperand& field_operand, Trapper&& trapper = [](int) {});
    ...
    void LoadSandboxedPointerField(Register destination, const MemOperand& field_operand);
    void StoreSandboxedPointerField(Register value, const MemOperand& dst_field_operand);
    ...
    // Only available when the sandbox is enabled, but always visible to avoid
    // having to place the #ifdefs into the caller.
    void StoreIndirectPointerField(Register value, MemOperand dst_field_operand, Trapper&& trapper = [](int) {});
    ...
    // simplify builtins.
    inline void LoadTaggedField(
        const Register& destination, const MemOperand& field_operand, Trapper&& trapper = [](int) {})
    {
        Lw(destination, field_operand, std::forward<Trapper>(trapper));
    }

    inline void LoadTaggedSignedField(const Register& destination, const MemOperand& field_operand)
    {
    ...
    void StoreTaggedField(
        const Register& value, const MemOperand& dst_field_operand, Trapper&& trapper = [](int) {})
    {
    ...
    void LoadLane(
        int sz, VRegister dst, uint8_t laneidx, MemOperand src, Trapper&& trapper = [](int) {});
    void StoreLane(
        int sz, VRegister src, uint8_t laneidx, MemOperand dst, Trapper&& trapper = [](int) {});
    ...
    void RecordWriteField(Register object, int offset, Register value, RAStatus ra_status, SaveFPRegsMode save_fp, SmiCheck smi_check = SmiCheck::kInline,
        SlotDescriptor slot = SlotDescriptor::ForDirectPointerSlot());

    // For a given |object| notify the garbage collector that the slot |address|
    // has been written.  |value| is the object being stored. The value and
    // address registers are clobbered by the operation.
    void RecordWrite(Register object, Operand offset, Register value, RAStatus ra_status, SaveFPRegsMode save_fp, SmiCheck smi_check = SmiCheck::kInline,
        SlotDescriptor slot = SlotDescriptor::ForDirectPointerSlot());
    ...
    // other registers.
    void CompareObjectTypeAndJump(
        Register heap_object, Register map, Register type_reg, InstanceType type, Condition cond, Label* target, Label::Distance distance);
    ...
    Condition LoadFeedbackVectorFlagsAndCheckIfNeedsProcessing(Register flags, Register feedback_vector, Register result, CodeKind current_code_kind);
...
private:
    ...
    int CallCFunctionHelper(Register function, int num_reg_arguments, int num_double_arguments,
        SetIsolateDataSlots set_isolate_data_slots = SetIsolateDataSlots::kYes, Label* return_location = nullptr);
...
#endif
    template <typename F> void RoundHelper(VRegister dst, VRegister src, Register scratch, VRegister v_scratch, FPURoundingMode frm, bool keep_nan_same = true);
    ...
    void InvokePrologue(Register expected_parameter_count, Register actual_parameter_count, Label* done, InvokeType type);
...
{
    return MemOperand(fp, (ExitFrameConstants::kFixedSlotCountAboveFp + index) * kSystemPointerSize);
}
