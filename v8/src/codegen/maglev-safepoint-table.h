...
public:
    ...
    MaglevSafepointEntry(int pc, int deopt_index, uint32_t num_tagged_slots, uint32_t num_untagged_slots, uint8_t num_extra_spill_slots,
    ...
    {
        return this->SafepointEntryBase::operator==(other) && num_tagged_slots_ == other.num_tagged_slots_ && num_untagged_slots_ == other.num_untagged_slots_
            && num_extra_spill_slots_ == other.num_extra_spill_slots_ && tagged_register_indexes_ == other.tagged_register_indexes_;
    }

    uint32_t num_tagged_slots() const
    {
        return num_tagged_slots_;
    }
    uint32_t num_untagged_slots() const
    {
        return num_untagged_slots_;
    }
    uint8_t num_extra_spill_slots() const
    {
        return num_extra_spill_slots_;
    }
...
public:
    ...
    explicit MaglevSafepointTable(Isolate* isolate, Address pc, Tagged<Code> code);
    ...
    int byte_size() const
    {
        return kHeaderSize + length_ * entry_size();
    }
    ...
    MaglevSafepointEntry GetEntry(int index) const
    {
        ...
        Address entry_ptr = safepoint_table_address_ + kHeaderSize + index * entry_size();
        ...
        if (has_deopt_data()) {
            ...
            DCHECK(deopt_index >= 0 || deopt_index == MaglevSafepointEntry::kNoDeoptIndex);
            DCHECK(trampoline_pc >= 0 || trampoline_pc == MaglevSafepointEntry::kNoTrampolinePC);
        }
        uint8_t num_extra_spill_slots = read_byte(&entry_ptr);
        int tagged_register_indexes = read_bytes(&entry_ptr, register_indexes_size());

        return MaglevSafepointEntry(pc, deopt_index, num_tagged_slots_, num_untagged_slots_, num_extra_spill_slots, tagged_register_indexes, trampoline_pc);
    }
...
private:
    ...
    static constexpr int kNumTaggedSlotsOffset = kEntryConfigurationOffset + kUInt32Size;
    static constexpr int kNumUntaggedSlotsOffset = kNumTaggedSlotsOffset + kUInt32Size;
    ...
    int entry_size() const
    {
        ...
        return pc_size() + deopt_data_size + num_pushed_registers_size + register_indexes_size();
    }
    ...
    int pc_size() const
    {
        return PcSizeField::decode(entry_configuration_);
    }
...
public:
    explicit MaglevSafepointTableBuilder(Zone* zone, uint32_t num_tagged_slots, uint32_t num_untagged_slots)
        : num_tagged_slots_(num_tagged_slots)
        , num_untagged_slots_(num_untagged_slots)
        , entries_(zone)
    {
    ...
    public:
        ...
        {
            DCHECK_LT(reg_code, kBitsPerByte * sizeof(EntryBuilder::tagged_register_indexes));
    ...
    private:
        friend class MaglevSafepointTableBuilder;
        explicit Safepoint(EntryBuilder* entry)
            : entry_(entry)
        {
        }
