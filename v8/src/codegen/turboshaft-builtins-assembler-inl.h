...
#define DEFINE_TURBOSHAFT_ALIASES()                                                                                                                            \
    template <typename T> using V = compiler::turboshaft::V<T>;                                                                                                \
    template <typename T> using ConstOrV = compiler::turboshaft::ConstOrV<T>;                                                                                  \
    template <typename T> using OptionalV = compiler::turboshaft::OptionalV<T>;                                                                                \
    template <typename... Ts> using Label = compiler::turboshaft::Label<Ts...>;                                                                                \
    template <typename... Ts> using LoopLabel = compiler::turboshaft::LoopLabel<Ts...>;                                                                        \
...
template <typename Assembler> class BuiltinArgumentsTS {
public:
    DEFINE_TURBOSHAFT_ALIASES()
        ...
        : assembler_(assembler)
    {
        ...
        const intptr_t offset = (StandardFrameConstants::kFixedSlotCountAboveFp + 1) * kSystemPointerSize;
    ...
    V<Object> AtIndex(ConstOrV<WordPtr> index) const
    {
        ...
        return V<Object>::Cast(__ LoadOffHeap(AtIndexPtr(index), MemoryRepresentation::AnyTagged()));
    }
    ...
    public:
        ...
        {
            return assembler.WordPtrAdd(current_iterator, ElementsKindToByteSize(SYSTEM_POINTER_ELEMENTS));
        }

        template <typename A> value_type Dereference(A& assembler, iterator_type current_iterator) const
        {
            return V<Object>::Cast(assembler.LoadOffHeap(current_iterator, MemoryRepresentation::AnyTagged()));
        }
...
private:
    ...
    {
        V<WordPtr> offset = assembler_->ElementOffsetFromIndex(index, SYSTEM_POINTER_ELEMENTS, 0);
...
template <typename Next> class FeedbackCollectorReducer : public Next {
public:
    ...
    void CombineFeedback(int additional_feedback)
    {
        ...
        feedback_ = __ SmiBitwiseOr(feedback_, __ SmiConstant(Smi::FromInt(additional_feedback)));
    }
    ...
    {
        return __ SmiEqual(feedback_, __ SmiConstant(Smi::FromInt(checked_feedback)));
    }

    V<FeedbackVectorOrUndefined> LoadFeedbackVector()
    {
        return V<FeedbackVectorOrUndefined>::Cast(__ LoadRegister(interpreter::Register::feedback_vector()));
    }

    V<WordPtr> LoadFeedbackVectorLength(V<FeedbackVector> feedback_vector)
    {
        V<Word32> length = __ LoadField(feedback_vector, AccessBuilderTS::ForFeedbackVectorLength());
    ...
    V<MaybeObject> LoadFeedbackVectorSlot(V<FeedbackVector> feedback_vector, V<WordPtr> slot, int additional_offset = 0)
    {
        ...
        int32_t header_size = FeedbackVector::kRawFeedbackSlotsOffset + additional_offset;
        V<WordPtr> offset = __ ElementOffsetFromIndex(slot, HOLEY_ELEMENTS, header_size);
        TSA_SLOW_DCHECK(this, IsOffsetInBounds(offset, LoadFeedbackVectorLength(feedback_vector), FeedbackVector::kHeaderSize));
        return V<MaybeObject>::Cast(__ Load(feedback_vector, offset, compiler::turboshaft::LoadOp::Kind::TaggedBase(), MemoryRepresentation::AnyTagged()));
    }

    void StoreFeedbackVectorSlot(
        V<FeedbackVector> feedback_vector, V<WordPtr> slot, V<Object> value, WriteBarrierMode barrier_mode = UPDATE_WRITE_BARRIER, int additional_offset = 0)
    {
        __ CodeComment("StoreFeedbackVectorSlot");
        DCHECK(IsAligned(additional_offset, kTaggedSize));
        int header_size = FeedbackVector::kRawFeedbackSlotsOffset + additional_offset;
        V<WordPtr> offset = __ ElementOffsetFromIndex(slot, HOLEY_ELEMENTS, header_size);
        TSA_DCHECK(this, IsOffsetInBounds(offset, LoadFeedbackVectorLength(feedback_vector), FeedbackVector::kHeaderSize));
        switch (barrier_mode) {
        case SKIP_WRITE_BARRIER: {
            __ Store(feedback_vector, offset, value, compiler::turboshaft::StoreOp::Kind::TaggedBase(), MemoryRepresentation::AnyTagged(),
    ...
    void UpdateFeedback()
    {
        ...
        V<FeedbackVector> feedback_vector = V<FeedbackVector>::Cast(maybe_feedback_vector_);

        V<MaybeObject> feedback_element = LoadFeedbackVectorSlot(feedback_vector, slot_id_);
        ...
        {
            StoreFeedbackVectorSlot(feedback_vector, slot_id_, combined_feedback, SKIP_WRITE_BARRIER);
            // TODO(nicohartmann):
    ...
    {
        return __ BitcastWord32ToSmi(__ Word32BitwiseOr(__ BitcastSmiToWord32(a), __ BitcastSmiToWord32(b)));
    }
    ...
    V<Word32> IsOffsetInBounds(V<WordPtr> offset, V<WordPtr> length, int header_size, ElementsKind kind = HOLEY_ELEMENTS)
    {
        ...
        V<WordPtr> last_offset = __ ElementOffsetFromIndex(length, kind, correction);
...
template <typename Next> class NoFeedbackCollectorReducer : public Next {
public:
...
template <typename Next> class BuiltinsReducer : public Next {
public:
    ...
    void EmitBuiltinProlog(Builtin builtin_id)
    {
        ...
        const compiler::CallDescriptor* desc = __ data()->builtin_call_descriptor();
        for (int i = 0; i < static_cast<int>(desc->ParameterCount()); ++i) {
            __ Parameter(i, RegisterRepresentation::FromMachineType(desc->GetParameterType(i)));
        }
    ...
    void EmitEpilog(Block* catch_block)
    {
        ...
        if (catch_block) {
            ...
            if (__ Bind(catch_block)) {
                ...
                __ UpdateFeedback();
                __ template CallRuntime<compiler::turboshaft::RuntimeCallDescriptor::ReThrow>(__ data()->isolate(), __ NoContextConstant(), { exception });
                __ Unreachable();
            }
        }
    }
    ...
    V<Context> JSContextParameter()
    {
        return __ template Parameter<Context>(
            compiler::Linkage::GetJSCallContextParamIndex(static_cast<int>(__ data()->builtin_call_descriptor()->JSParameterCount())));
    }

    void PerformStackCheck(V<Context> context)
    {
        __ JSStackCheck(context, OptionalV<compiler::turboshaft::FrameState>::Nullopt(), compiler::turboshaft::JSStackCheckOp::Kind::kBuiltinEntry);
    }
    ...
    V<Word32> TruncateTaggedToWord32(V<Context> context, V<Object> value)
    {
        ...
        TaggedToWord32OrBigIntImpl<Object::Conversion::kToNumber>(context, value, IsKnownTaggedPointer::kNo, is_number);
    ...
    {
        return __ WordPtrBitwiseAnd(__ WordPtrAdd(size, kObjectAlignmentMask), ~kObjectAlignmentMask);
    }
    ...
    V<WordPtr> ElementOffsetFromIndex(ConstOrV<WordPtr> index, ElementsKind kind, intptr_t base_size)
    {
        ...
        if (std::optional<intptr_t> constant_index = TryToIntPtrConstant(index)) {
            return __ WordPtrConstant(base_size + (1 << element_size_shift) * (*constant_index));
        }
        if (element_size_shift == 0) {
            return __ WordPtrAdd(base_size, index);
        } else {
            DCHECK_LT(0, element_size_shift);
            return __ WordPtrAdd(base_size, __ WordPtrShiftLeft(index, element_size_shift));
        }
    }
    ...
    void TaggedToWord32OrBigIntImpl(V<Context> context, V<Object> value, IsKnownTaggedPointer is_known_tagged_pointer, Label<Word32>& if_number,
        Label<BigInt>* if_bigint = nullptr, Label<BigInt>* if_bigint64 = nullptr)
    {
        DCHECK_EQ(Conversion == Object::Conversion::kToNumeric, if_bigint != nullptr);
        ...
        WHILE(1)
        {
            ...
            IF(__ IsHeapNumberMap(map))
            {
                ...
                V<Float64> value_float64 = __ LoadHeapNumberValue(V<HeapNumber>::Cast(value_heap_object));
            ...
            IF(InstanceTypeEqual(instance_type, ODDBALL_TYPE))
            {
                ...
                V<Float64> oddball_value = __ LoadField(V<Oddball>::Cast(value_heap_object), AccessBuilderTS::ForHeapNumberOrOddballOrHoleValue());
            ...
            using Builtin = std::conditional_t<Conversion == Object::Conversion::kToNumeric, BuiltinCallDescriptor::NonNumberToNumeric,
                BuiltinCallDescriptor::NonNumberToNumber>;
            converted_value = __ template CallBuiltin<Builtin>(isolate(), context, { V<JSAnyNotNumber>::Cast(value_heap_object) });

            GOTO_IF(__ IsSmi(converted_value), if_number, __ UntagSmi(V<Smi>::Cast(converted_value)));
...
private:
    ...
    Isolate* isolate()
    {
        return __ data()->isolate();
    }
};

template <template <typename> typename Reducer, template <typename> typename FeedbackReducer>
class TurboshaftBuiltinsAssembler : public compiler::turboshaft::TSAssembler<Reducer, BuiltinsReducer, FeedbackReducer,
                                        compiler::turboshaft::MachineLoweringReducer, compiler::turboshaft::VariableReducer> {
public:
    using Base = compiler::turboshaft::TSAssembler<Reducer, BuiltinsReducer, FeedbackReducer, compiler::turboshaft::MachineLoweringReducer,
