...
#if !V8_HOST_ARCH_PPC64 || !V8_TARGET_ARCH_PPC64 || V8_TARGET_LITTLE_ENDIAN || (defined(_CALL_ELF) && _CALL_ELF == 2)
...
#define PPC_PREFIX_OPCODE_TYPE_10_LIST(V) V(pload_store_mls, PLOAD_STORE_MLS, 0x6000000)
...
#define DECLARE_INSTRUCTION(name, opcode_name, opcode_value) opcode_name = opcode_value,
    PPC_OPCODE_LIST(DECLARE_INSTRUCTION)
#undef DECLARE_INSTRUCTION
        EXTP
    = 0x4000000, // Extended code set prefixed
...
#define DECLARE_STATIC_ACCESSOR(Name) DECLARE_STATIC_TYPED_ACCESSOR(int, Name)
    ...
    inline int RAValue() const
    {
        return Bits(20, 16);
    }
    DECLARE_STATIC_ACCESSOR(RAValue)
    inline int RBValue() const
    {
        return Bits(15, 11);
    }
    DECLARE_STATIC_ACCESSOR(RBValue)
    inline int RCValue() const
    {
        return Bits(10, 6);
    }
