...
// static
void CodeDesc::Initialize(CodeDesc* desc, Assembler* assembler, int safepoint_table_offset, int handler_table_offset, int constant_pool_offset,
    int code_comments_offset, int builtin_jump_table_info_offset, int reloc_info_offset)
{
    ...
    desc->builtin_jump_table_info_size = desc->instr_size - builtin_jump_table_info_offset;

    desc->code_comments_offset = code_comments_offset;
    desc->code_comments_size = desc->builtin_jump_table_info_offset - code_comments_offset;
...
void CodeDesc::Verify(const CodeDesc* desc)
{
    ...
    DCHECK_EQ(desc->safepoint_table_size + desc->safepoint_table_offset, desc->handler_table_offset);
    DCHECK_GE(desc->handler_table_size, 0);
    DCHECK_EQ(desc->handler_table_size + desc->handler_table_offset, desc->constant_pool_offset);
    DCHECK_GE(desc->constant_pool_size, 0);
    DCHECK_EQ(desc->constant_pool_size + desc->constant_pool_offset, desc->code_comments_offset);
    DCHECK_GE(desc->code_comments_size, 0);
    DCHECK_EQ(desc->code_comments_size + desc->code_comments_offset, desc->builtin_jump_table_info_offset);
    DCHECK_GE(desc->builtin_jump_table_info_size, 0);
    DCHECK_EQ(desc->builtin_jump_table_info_size + desc->builtin_jump_table_info_offset, desc->instr_size);
