...
public:
    ...
    {
        return bit_field_ == other.bit_field_ && machine_type_ == other.machine_type_;
    }
    ...
    static bool IsSameLocation(const LinkageLocation& a, const LinkageLocation& b)
    {
        ...
        return (a.bit_field_ == b.bit_field_)
            && (IsSubtype(a.machine_type_.representation(), b.machine_type_.representation())
                || IsSubtype(b.machine_type_.representation(), a.machine_type_.representation()));
    }
    ...
    static LinkageLocation ForSavedCallerReturnAddress()
    {
        return ForCalleeFrameSlot(
            (StandardFrameConstants::kCallerPCOffset - StandardFrameConstants::kCallerPCOffset) / kSystemPointerSize, MachineType::Pointer());
    }

    static LinkageLocation ForSavedCallerFramePtr()
    {
        return ForCalleeFrameSlot(
            (StandardFrameConstants::kCallerPCOffset - StandardFrameConstants::kCallerFPOffset) / kSystemPointerSize, MachineType::Pointer());
    }
    ...
    static LinkageLocation ForSavedCallerConstantPool()
    {
        ...
        return ForCalleeFrameSlot(
            (StandardFrameConstants::kCallerPCOffset - StandardFrameConstants::kConstantPoolOffset) / kSystemPointerSize, MachineType::AnyTagged());
    }

    static LinkageLocation ForSavedCallerFunction()
    {
        return ForCalleeFrameSlot(
            (StandardFrameConstants::kCallerPCOffset - StandardFrameConstants::kFunctionOffset) / kSystemPointerSize, MachineType::AnyTagged());
    }
    ...
    static LinkageLocation ConvertToTailCallerLocation(LinkageLocation caller_location, int stack_param_delta)
    {
        if (!caller_location.IsRegister()) {
            return LinkageLocation(STACK_SLOT, caller_location.GetLocation() + stack_param_delta, caller_location.GetType());
        }
