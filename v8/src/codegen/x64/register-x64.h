...
public:
    explicit TaggedRegister(Register reg)
        : reg_(reg)
    {
    }
    Register reg()
...
static_assert(static_cast<int>(kDoubleAfterLast) == static_cast<int>(kYMMAfterLast),
...
static_assert(sizeof(XMMRegister) <= sizeof(int), "XMMRegister can efficiently be passed by value");
...
private:
    ...
    explicit constexpr YMMRegister(int code)
        : XMMRegister(code)
    {
    }
};
...
static_assert(sizeof(YMMRegister) <= sizeof(int), "YMMRegister can efficiently be passed by value");
...
#define DECLARE_REGISTER(R) constexpr DoubleRegister R = DoubleRegister::from_code(kDoubleCode_##R);
DOUBLE_REGISTERS(DECLARE_REGISTER)
#undef DECLARE_REGISTER
constexpr DoubleRegister no_dreg = DoubleRegister::no_reg();

#define DECLARE_REGISTER(R) constexpr YMMRegister R = YMMRegister::from_code(kYMMCode_##R);
