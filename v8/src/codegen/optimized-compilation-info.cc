...
OptimizedCompilationInfo::OptimizedCompilationInfo(
    Zone* zone, Isolate* isolate, IndirectHandle<SharedFunctionInfo> shared, IndirectHandle<JSFunction> closure, CodeKind code_kind, BytecodeOffset osr_offset)
    ...
    , optimization_id_(isolate->NextOptimizationId())
{
    ...
    shared_info_ = shared;
    closure_ = closure;
    canonical_handles_ = std::make_unique<CanonicalHandlesMap>(isolate->heap(), ZoneAllocationPolicy(zone));
...
OptimizedCompilationInfo::OptimizedCompilationInfo(base::Vector<const char> debug_name, Zone* zone, CodeKind code_kind, Builtin builtin)
...
{
    DCHECK_IMPLIES(builtin_ != Builtin::kNoBuiltinId, (code_kind_ == CodeKind::BUILTIN || code_kind_ == CodeKind::BYTECODE_HANDLER));
    SetTracingFlags(PassesFilter(debug_name, base::CStrVector(v8_flags.trace_turbo_filter)));
    ConfigureFlags();
...
{
    if (v8_flags.turbo_inline_js_wasm_calls)
        set_inline_js_wasm_calls();
    ...
    case CodeKind::BYTECODE_HANDLER:
        ...
        if (v8_flags.turbo_splitting)
            set_splitting();
        if (v8_flags.enable_allocation_folding)
            set_allocation_folding();
        break;
...
void OptimizedCompilationInfo::ReopenAndCanonicalizeHandlesInNewScope(Isolate* isolate)
{
...
std::unique_ptr<char[]> OptimizedCompilationInfo::GetDebugName() const
{
    ...
    if (name_vec.empty())
        name_vec = base::ArrayVector("unknown");
...
#if V8_ENABLE_WEBASSEMBLY
void OptimizedCompilationInfo::SetWasmCompilationResult(std::unique_ptr<wasm::WasmCompilationResult> wasm_compilation_result)
{
    wasm_compilation_result_ = std::move(wasm_compilation_result);
}

std::unique_ptr<wasm::WasmCompilationResult> OptimizedCompilationInfo::ReleaseWasmCompilationResult()
{
...
int OptimizedCompilationInfo::AddInlinedFunction(
    IndirectHandle<SharedFunctionInfo> inlined_function, IndirectHandle<BytecodeArray> inlined_bytecode, SourcePosition pos)
{
    int id = static_cast<int>(inlined_functions_.size());
    inlined_functions_.push_back(InlinedFunctionHolder(inlined_function, inlined_bytecode, pos));
    return id;
}
...
void OptimizedCompilationInfo::SetTracingFlags(bool passes_filter)
{
    ...
    if (v8_flags.trace_turbo)
        set_trace_turbo_json();
    if (v8_flags.trace_turbo_graph)
        set_trace_turbo_graph();
    if (v8_flags.trace_turbo_scheduled)
        set_trace_turbo_scheduled();
    if (v8_flags.trace_turbo_alloc)
        set_trace_turbo_allocation();
    if (v8_flags.trace_heap_broker)
        set_trace_heap_broker();
    if (v8_flags.turboshaft_trace_reduction)
        set_turboshaft_trace_reduction();
}

OptimizedCompilationInfo::InlinedFunctionHolder::InlinedFunctionHolder(
    IndirectHandle<SharedFunctionInfo> inlined_shared_info, IndirectHandle<BytecodeArray> inlined_bytecode, SourcePosition pos)
