...
public:
    ...
    void AtomicCmpExchangeHelper(Register addr, Register output, Register old_value, Register new_value, int start, int end, int shift_amount, int offset,
    ...
    template <class T> void CmpP(Register src1, T src2)
    {
        CmpS64(src1, src2);
    }
    ...
    template <class T> void LoadF64(DoubleRegister result, T value, Register scratch)
    {
    ...
    template <class T> void LoadF32(DoubleRegister result, T value, Register scratch)
    {
    ...
    int CallCFunction(ExternalReference function, int num_arguments, SetIsolateDataSlots set_isolate_data_slots = SetIsolateDataSlots::kYes,
        bool has_function_descriptor = ABI_USES_FUNCTION_DESCRIPTORS, Label* return_label = nullptr);
    int CallCFunction(Register function, int num_arguments, SetIsolateDataSlots set_isolate_data_slots = SetIsolateDataSlots::kYes,
        bool has_function_descriptor = ABI_USES_FUNCTION_DESCRIPTORS, Label* return_label = nullptr);
    int CallCFunction(ExternalReference function, int num_reg_arguments, int num_double_arguments,
        SetIsolateDataSlots set_isolate_data_slots = SetIsolateDataSlots::kYes, bool has_function_descriptor = ABI_USES_FUNCTION_DESCRIPTORS,
    ...
    inline void ExtractBitRange(Register dst, Register src, int rangeStart, int rangeEnd)
    {
        ...
        if (CpuFeatures::IsSupported(GENERAL_INSTR_EXT)) {
            ...
            RotateInsertSelectBits(dst, src, Operand(startBit), Operand(endBit), Operand(shiftAmount), true);
        } else {
    ...
    inline void ExtractBitMask(Register dst, Register src, uintptr_t mask, RCBit rc = LeaveRC)
    {
...
#define PROTOTYPE_SIMD_EXT_MUL(name) void name(Simd128Register dst, Simd128Register src1, Simd128Register src2, Simd128Register scratch);
...
#define PROTOTYPE_SIMD_ALL_TRUE(name) void name(Register dst, Simd128Register src, Register scratch1, Simd128Register scratch2);
...
#define PROTOTYPE_SIMD_QFM(name) void name(Simd128Register dst, Simd128Register src1, Simd128Register src2, Simd128Register src3);
...
#define PROTOTYPE_SIMD_ADD_SUB_SAT(name)                                                                                                                       \
    void name(Simd128Register dst, Simd128Register src1, Simd128Register src2, Simd128Register scratch1, Simd128Register scratch2);
...
#undef SIMD_EXT_ADD_PAIRWISE_LIST
    ...
    template <bool use_unsigned_cmp = false> void CompareObjectType(Register heap_object, Register map, Register type_reg, InstanceType type)
    {
    ...
    // sets the flags and leaves the object type in the type_reg register.
    template <bool use_unsigned_cmp = false> void CompareInstanceType(Register map, Register type_reg, InstanceType type)
    {
    ...
    template <class T> void CompareTagged(Register src1, T src2)
    {
...
#define SmiWordOffset(offset) offset
    ...
    void RecordWrite(
        Register object, Register slot_address, Register value, LinkRegisterStatus lr_status, SaveFPRegsMode save_fp, SmiCheck smi_check = SmiCheck::kInline);
