...
#ifdef V8_DEBUG
#define UNIMPLEMENTED_S390() v8::internal::PrintF("%s, \tline %d: \tfunction %s not implemented. \n", __FILE__, __LINE__, __func__)
#else
...
constexpr int kHasFunctionDescriptorBitMask = 1 << kHasFunctionDescriptorBitShift;
#endif
...
#define S390_RSY_B_OPCODE_LIST(V)                                                                                                                              \
    V(clmh, CLMH, 0xEB20) /* type = RSY_B COMPARE LOGICAL CHAR. UNDER MASK (high)  */                                                                          \
    V(clmy, CLMY, 0xEB21) /* type = RSY_B COMPARE LOGICAL CHAR. UNDER MASK (low)  */                                                                           \
    V(clt, CLT, 0xEB23) /* type = RSY_B COMPARE LOGICAL AND TRAP (32)  */                                                                                      \
    V(clgt, CLGT, 0xEB2B) /* type = RSY_B COMPARE LOGICAL AND TRAP (64)  */                                                                                    \
    V(stcmh, STCMH, 0xEB2C) /* type = RSY_B STORE CHARACTERS UNDER MASK (high)  */                                                                             \
...
#define S390_RXE_OPCODE_LIST(V)                                                                                                                                \
    ...
    V(lxdb, LXDB, 0xED05) /* type = RXE   LOAD LENGTHENED (long to extended BFP)  */                                                                           \
    V(lxeb, LXEB, 0xED06) /* type = RXE   LOAD LENGTHENED (short to extended BFP)  */                                                                          \
    ...
    V(lxd, LXD, 0xED25) /* type = RXE   LOAD LENGTHENED (long to extended HFP)  */                                                                             \
    V(lxe, LXE, 0xED26) /* type = RXE   LOAD LENGTHENED (short to extended HFP)  */                                                                            \
...
#define S390_RXF_OPCODE_LIST(V)                                                                                                                                \
    ...
    V(mayl, MAYL, 0xED38) /* type = RXF   MULTIPLY AND ADD UNNRM. (long to ext. low HFP)  */                                                                   \
    V(myl, MYL, 0xED39) /* type = RXF   MULTIPLY UNNORM. (long to ext. low HFP)  */                                                                            \
    V(may, MAY, 0xED3A) /* type = RXF   MULTIPLY & ADD UNNORMALIZED (long to ext. HFP)  */                                                                     \
    V(my, MY, 0xED3B) /* type = RXF   MULTIPLY UNNORMALIZED (long to ext. HFP)  */                                                                             \
    V(mayh, MAYH, 0xED3C) /* type = RXF   MULTIPLY AND ADD UNNRM. (long to ext. high HFP) */                                                                   \
    V(myh, MYH, 0xED3D) /* type = RXF   MULTIPLY UNNORM. (long to ext. high HFP)  */                                                                           \
    ...
    V(slxt, SLXT, 0xED48) /* type = RXF   SHIFT SIGNIFICAND LEFT (extended DFP)  */                                                                            \
    V(srxt, SRXT, 0xED49) /* type = RXF   SHIFT SIGNIFICAND RIGHT (extended DFP)  */

#define S390_IE_OPCODE_LIST(V) V(niai, NIAI, 0xB2FA) /* type = IE    NEXT INSTRUCTION ACCESS INTENT  */
...
#define S390_RRF_B_OPCODE_LIST(V)                                                                                                                              \
    ...
    V(iedtr, IEDTR, 0xB3F6) /* type = RRF_B INSERT BIASED EXPONENT (64 to long DFP)  */                                                                        \
    V(rrdtr, RRDTR, 0xB3F7) /* type = RRF_B REROUND (long DFP)  */                                                                                             \
    V(qaxtr, QAXTR, 0xB3FD) /* type = RRF_B QUANTIZE (extended DFP)  */                                                                                        \
    V(iextr, IEXTR, 0xB3FE) /* type = RRF_B INSERT BIASED EXPONENT (64 to extended DFP)  */                                                                    \
    ...
    V(crdte, CRDTE, 0xB98F) /* type = RRF_B COMPARE AND REPLACE DAT TABLE ENTRY  */                                                                            \
...
#define S390_RRF_C_OPCODE_LIST(V)                                                                                                                              \
    ...
    V(trtre, TRTRE, 0xB9BD) /* type = RRF_C TRANSLATE AND TEST REVERSE EXTENDED  */                                                                            \
...
#define S390_MII_OPCODE_LIST(V) V(bprp, BPRP, 0xC5) /* type = MII   BRANCH PREDICTION RELATIVE PRELOAD  */

#define S390_RRF_D_OPCODE_LIST(V)                                                                                                                              \
    V(ldetr, LDETR, 0xB3D4) /* type = RRF_D LOAD LENGTHENED (short to long DFP)  */                                                                            \
    V(lxdtr, LXDTR, 0xB3DC) /* type = RRF_D LOAD LENGTHENED (long to extended DFP)  */                                                                         \
    V(csdtr, CSDTR, 0xB3E3) /* type = RRF_D CONVERT TO SIGNED PACKED (long DFP to 64)  */                                                                      \
    V(csxtr, CSXTR, 0xB3EB) /* type = RRF_D CONVERT TO SIGNED PACKED (extended DFP to 128)  */

#define S390_RRF_E_OPCODE_LIST(V)                                                                                                                              \
    V(ledbra, LEDBRA, 0xB344) /* type = RRF_E LOAD ROUNDED (long to short BFP)  */                                                                             \
    V(ldxbra, LDXBRA, 0xB345) /* type = RRF_E LOAD ROUNDED (extended to long BFP)  */                                                                          \
    V(lexbra, LEXBRA, 0xB346) /* type = RRF_E LOAD ROUNDED (extended to short BFP)  */                                                                         \
    V(fixbra, FIXBRA, 0xB347) /* type = RRF_E LOAD FP INTEGER (extended BFP)  */                                                                               \
    V(tbedr, TBEDR, 0xB350) /* type = RRF_E CONVERT HFP TO BFP (long to short)  */                                                                             \
    ...
    V(celfbr, CELFBR, 0xB390) /* type = RRF_E CONVERT FROM LOGICAL (32 to short BFP)  */                                                                       \
    V(cdlfbr, CDLFBR, 0xB391) /* type = RRF_E CONVERT FROM LOGICAL (32 to long BFP)  */                                                                        \
    V(cxlfbr, CXLFBR, 0xB392) /* type = RRF_E CONVERT FROM LOGICAL (32 to extended BFP)  */                                                                    \
    V(cefbra, CEFBRA, 0xB394) /* type = RRF_E CONVERT FROM FIXED (32 to short BFP)  */                                                                         \
    V(cdfbra, CDFBRA, 0xB395) /* type = RRF_E CONVERT FROM FIXED (32 to long BFP)  */                                                                          \
    V(cxfbra, CXFBRA, 0xB396) /* type = RRF_E CONVERT FROM FIXED (32 to extended BFP)  */                                                                      \
    V(cfebra, CFEBRA, 0xB398) /* type = RRF_E CONVERT TO FIXED (short BFP to 32)  */                                                                           \
    V(cfdbra, CFDBRA, 0xB399) /* type = RRF_E CONVERT TO FIXED (long BFP to 32)  */                                                                            \
    V(cfxbra, CFXBRA, 0xB39A) /* type = RRF_E CONVERT TO FIXED (extended BFP to 32)  */                                                                        \
    V(clfebr, CLFEBR, 0xB39C) /* type = RRF_E CONVERT TO LOGICAL (short BFP to 32)  */                                                                         \
    V(clfdbr, CLFDBR, 0xB39D) /* type = RRF_E CONVERT TO LOGICAL (long BFP to 32)  */                                                                          \
    V(clfxbr, CLFXBR, 0xB39E) /* type = RRF_E CONVERT TO LOGICAL (extended BFP to 32)  */                                                                      \
    V(celgbr, CELGBR, 0xB3A0) /* type = RRF_E CONVERT FROM LOGICAL (64 to short BFP)  */                                                                       \
    V(cdlgbr, CDLGBR, 0xB3A1) /* type = RRF_E CONVERT FROM LOGICAL (64 to long BFP)  */                                                                        \
    V(cxlgbr, CXLGBR, 0xB3A2) /* type = RRF_E CONVERT FROM LOGICAL (64 to extended BFP)  */                                                                    \
    V(cegbra, CEGBRA, 0xB3A4) /* type = RRF_E CONVERT FROM FIXED (64 to short BFP)  */                                                                         \
    V(cdgbra, CDGBRA, 0xB3A5) /* type = RRF_E CONVERT FROM FIXED (64 to long BFP)  */                                                                          \
    V(cxgbra, CXGBRA, 0xB3A6) /* type = RRF_E CONVERT FROM FIXED (64 to extended BFP)  */                                                                      \
    V(cgebra, CGEBRA, 0xB3A8) /* type = RRF_E CONVERT TO FIXED (short BFP to 64)  */                                                                           \
    V(cgdbra, CGDBRA, 0xB3A9) /* type = RRF_E CONVERT TO FIXED (long BFP to 64)  */                                                                            \
    V(cgxbra, CGXBRA, 0xB3AA) /* type = RRF_E CONVERT TO FIXED (extended BFP to 64)  */                                                                        \
    V(clgebr, CLGEBR, 0xB3AC) /* type = RRF_E CONVERT TO LOGICAL (short BFP to 64)  */                                                                         \
    V(clgdbr, CLGDBR, 0xB3AD) /* type = RRF_E CONVERT TO LOGICAL (long BFP to 64)  */                                                                          \
    V(clgxbr, CLGXBR, 0xB3AE) /* type = RRF_E CONVERT TO LOGICAL (extended BFP to 64)  */                                                                      \
    ...
    V(cfxr, CFXR, 0xB3BA) /* type = RRF_E CONVERT TO FIXED (extended HFP to 32)  */                                                                            \
    ...
    V(cgxr, CGXR, 0xB3CA) /* type = RRF_E CONVERT TO FIXED (extended HFP to 64)  */                                                                            \
    V(ledtr, LEDTR, 0xB3D5) /* type = RRF_E LOAD ROUNDED (long to short DFP)  */                                                                               \
    V(fidtr, FIDTR, 0xB3D7) /* type = RRF_E LOAD FP INTEGER (long DFP)  */                                                                                     \
    V(ldxtr, LDXTR, 0xB3DD) /* type = RRF_E LOAD ROUNDED (extended to long DFP)  */                                                                            \
    V(fixtr, FIXTR, 0xB3DF) /* type = RRF_E LOAD FP INTEGER (extended DFP)  */                                                                                 \
    V(cgdtra, CGDTRA, 0xB3E1) /* type = RRF_E CONVERT TO FIXED (long DFP to 64)  */                                                                            \
    V(cgxtra, CGXTRA, 0xB3E9) /* type = RRF_E CONVERT TO FIXED (extended DFP to 64)  */                                                                        \
    V(cdgtra, CDGTRA, 0xB3F1) /* type = RRF_E CONVERT FROM FIXED (64 to long DFP)  */                                                                          \
    V(cxgtra, CXGTRA, 0xB3F9) /* type = RRF_E CONVERT FROM FIXED (64 to extended DFP)  */                                                                      \
    V(cfdtr, CFDTR, 0xB941) /* type = RRF_E CONVERT TO FIXED (long DFP to 32) */                                                                               \
    V(clgdtr, CLGDTR, 0xB942) /* type = RRF_E CONVERT TO LOGICAL (long DFP to 64)  */                                                                          \
    V(clfdtr, CLFDTR, 0xB943) /* type = RRF_E CONVERT TO LOGICAL (long DFP to 32)  */                                                                          \
    V(cfxtr, CFXTR, 0xB949) /* type = RRF_E CONVERT TO FIXED (extended DFP to 32)  */                                                                          \
    V(clgxtr, CLGXTR, 0xB94A) /* type = RRF_E CONVERT TO LOGICAL (extended DFP to 64)  */                                                                      \
    V(clfxtr, CLFXTR, 0xB94B) /* type = RRF_E CONVERT TO LOGICAL (extended DFP to 32)  */                                                                      \
    V(cdlgtr, CDLGTR, 0xB952) /* type = RRF_E CONVERT FROM LOGICAL (64 to long DFP)  */                                                                        \
    V(cdlftr, CDLFTR, 0xB953) /* type = RRF_E CONVERT FROM LOGICAL (32 to long DFP)  */                                                                        \
    V(cxlgtr, CXLGTR, 0xB95A) /* type = RRF_E CONVERT FROM LOGICAL (64 to extended DFP)  */                                                                    \
    V(cxlftr, CXLFTR, 0xB95B) /* type = RRF_E CONVERT FROM LOGICAL (32 to extended DFP)  */
...
#define S390_VRR_A_OPCODE_LIST(V)                                                                                                                              \
    ...
    V(vclgd, VCLGD, 0xE7C0) /* type = VRR_A VECTOR FP CONVERT TO LOGICAL 64-BIT  */                                                                            \
    V(vcdlg, VCDLG, 0xE7C1) /* type = VRR_A VECTOR FP CONVERT FROM LOGICAL 64-BIT  */                                                                          \
...
#define S390_VRR_C_OPCODE_LIST(V)                                                                                                                              \
    ...
    V(verllv, VERLLV, 0xE773) /* type = VRR_C VECTOR ELEMENT ROTATE LEFT LOGICAL  */                                                                           \
    V(vsl, VSL, 0xE774) /* type = VRR_C VECTOR SHIFT LEFT  */                                                                                                  \
    V(vslb, VSLB, 0xE775) /* type = VRR_C VECTOR SHIFT LEFT BY BYTE  */                                                                                        \
    V(vesrlv, VESRLV, 0xE778) /* type = VRR_C VECTOR ELEMENT SHIFT RIGHT LOGICAL  */                                                                           \
    V(vesrav, VESRAV, 0xE77A) /* type = VRR_C VECTOR ELEMENT SHIFT RIGHT ARITHMETIC  */                                                                        \
    V(vsrl, VSRL, 0xE77C) /* type = VRR_C VECTOR SHIFT RIGHT LOGICAL  */                                                                                       \
    V(vsrlb, VSRLB, 0xE77D) /* type = VRR_C VECTOR SHIFT RIGHT LOGICAL BY BYTE  */                                                                             \
    V(vsra, VSRA, 0xE77E) /* type = VRR_C VECTOR SHIFT RIGHT ARITHMETIC  */                                                                                    \
    V(vsrab, VSRAB, 0xE77F) /* type = VRR_C VECTOR SHIFT RIGHT ARITHMETIC BY BYTE  */                                                                          \
    ...
    V(vscbi, VSCBI, 0xE7F5) /* type = VRR_C VECTOR SUBTRACT COMPUTE BORROW INDICATION  */                                                                      \
...
#define S390_VRI_A_OPCODE_LIST(V)                                                                                                                              \
    ...
    V(vleih, VLEIH, 0xE741) /* type = VRI_A VECTOR LOAD ELEMENT IMMEDIATE (16)  */                                                                             \
    V(vleig, VLEIG, 0xE742) /* type = VRI_A VECTOR LOAD ELEMENT IMMEDIATE (64)  */                                                                             \
    V(vleif, VLEIF, 0xE743) /* type = VRI_A VECTOR LOAD ELEMENT IMMEDIATE (32)  */                                                                             \
...
#define S390_VRR_D_OPCODE_LIST(V)                                                                                                                              \
    ...
    V(vmalh, VMALH, 0xE7A9) /* type = VRR_D VECTOR MULTIPLY AND ADD LOGICAL HIGH  */                                                                           \
    V(vmal, VMAL, 0xE7AA) /* type = VRR_D VECTOR MULTIPLY AND ADD LOW  */                                                                                      \
    V(vmah, VMAH, 0xE7AB) /* type = VRR_D VECTOR MULTIPLY AND ADD HIGH  */                                                                                     \
    V(vmale, VMALE, 0xE7AC) /* type = VRR_D VECTOR MULTIPLY AND ADD LOGICAL EVEN  */                                                                           \
    V(vmalo, VMALO, 0xE7AD) /* type = VRR_D VECTOR MULTIPLY AND ADD LOGICAL ODD  */                                                                            \
    V(vmae, VMAE, 0xE7AE) /* type = VRR_D VECTOR MULTIPLY AND ADD EVEN  */                                                                                     \
    V(vmao, VMAO, 0xE7AF) /* type = VRR_D VECTOR MULTIPLY AND ADD ODD  */                                                                                      \
    V(vaccc, VACCC, 0xE7B9) /* type = VRR_D VECTOR ADD WITH CARRY COMPUTE CARRY  */                                                                            \
    V(vac, VAC, 0xE7BB) /* type = VRR_D VECTOR ADD WITH CARRY  */                                                                                              \
    V(vgfma, VGFMA, 0xE7BC) /* type = VRR_D VECTOR GALOIS FIELD MULTIPLY SUM AND ACCUMULATE */                                                                 \
    V(vsbcbi, VSBCBI, 0xE7BD) /* type = VRR_D VECTOR SUBTRACT WITH BORROW     */                                                                               \
    /* COMPUTE BORROW INDICATION  */                                                                                                                           \
    V(vsbi, VSBI, 0xE7BF) /* type = VRR_D VECTOR SUBTRACT WITH BORROW INDICATION  */

#define S390_VRI_B_OPCODE_LIST(V) V(vgm, VGM, 0xE746) /* type = VRI_B VECTOR GENERATE MASK  */
...
#define S390_VRR_E_OPCODE_LIST(V)                                                                                                                              \
    ...
    V(vfnms, VFNMS, 0xE79E) /* type = VRR_E VECTOR FP NEGATIVE MULTIPLY AND SUBTRACT  */                                                                       \
    V(vfma, VFMA, 0xE78F) /* type = VRR_E VECTOR FP MULTIPLY AND ADD  */

#define S390_VRI_C_OPCODE_LIST(V) V(vrep, VREP, 0xE74D) /* type = VRI_C VECTOR REPLICATE  */

#define S390_VRI_D_OPCODE_LIST(V)                                                                                                                              \
    V(verim, VERIM, 0xE772) /* type = VRI_D VECTOR ELEMENT ROTATE AND INSERT UNDER MASK  */                                                                    \
    V(vsldb, VSLDB, 0xE777) /* type = VRI_D VECTOR SHIFT LEFT DOUBLE BY BYTE  */

#define S390_VRR_F_OPCODE_LIST(V) V(vlvgp, VLVGP, 0xE762) /* type = VRR_F VECTOR LOAD VR FROM GRS DISJOINT  */

#define S390_RIS_OPCODE_LIST(V)                                                                                                                                \
    V(cgib, CGIB, 0xECFC) /* type = RIS   COMPARE IMMEDIATE AND BRANCH (64<-8)  */                                                                             \
    V(clgib, CLGIB, 0xECFD) /* type = RIS   COMPARE LOGICAL IMMEDIATE AND BRANCH (64<-8)  */                                                                   \
    V(cib, CIB, 0xECFE) /* type = RIS   COMPARE IMMEDIATE AND BRANCH (32<-8)  */                                                                               \
    V(clib, CLIB, 0xECFF) /* type = RIS   COMPARE LOGICAL IMMEDIATE AND BRANCH (32<-8)  */

#define S390_VRI_E_OPCODE_LIST(V) V(vftci, VFTCI, 0xE74A) /* type = VRI_E VECTOR FP TEST DATA CLASS IMMEDIATE  */

#define S390_RSL_A_OPCODE_LIST(V) V(tp, TP, 0xEBC0) /* type = RSL_A TEST DECIMAL  */

#define S390_RSL_B_OPCODE_LIST(V)                                                                                                                              \
    V(cpdt, CPDT, 0xEDAC) /* type = RSL_B CONVERT TO PACKED (from long DFP)  */                                                                                \
    V(cpxt, CPXT, 0xEDAD) /* type = RSL_B CONVERT TO PACKED (from extended DFP)  */                                                                            \
    V(cdpt, CDPT, 0xEDAE) /* type = RSL_B CONVERT FROM PACKED (to long DFP)  */                                                                                \
    V(cxpt, CXPT, 0xEDAF) /* type = RSL_B CONVERT FROM PACKED (to extended DFP)  */                                                                            \
...
#define S390_SIL_OPCODE_LIST(V)                                                                                                                                \
    ...
    V(chhsi, CHHSI, 0xE554) /* type = SIL   COMPARE HALFWORD IMMEDIATE (16<-16)  */                                                                            \
    V(clhhsi, CLHHSI, 0xE555) /* type = SIL   COMPARE LOGICAL IMMEDIATE (16<-16)  */                                                                           \
    V(cghsi, CGHSI, 0xE558) /* type = SIL   COMPARE HALFWORD IMMEDIATE (64<-16)  */                                                                            \
    V(clghsi, CLGHSI, 0xE559) /* type = SIL   COMPARE LOGICAL IMMEDIATE (64<-16)  */                                                                           \
    V(chsi, CHSI, 0xE55C) /* type = SIL   COMPARE HALFWORD IMMEDIATE (32<-16) */                                                                               \
    V(clfhsi, CLFHSI, 0xE55D) /* type = SIL   COMPARE LOGICAL IMMEDIATE (32<-16)  */                                                                           \
    V(tbegin, TBEGIN, 0xE560) /* type = SIL   TRANSACTION BEGIN (nonconstrained)  */                                                                           \
    V(tbeginc, TBEGINC, 0xE561) /* type = SIL   TRANSACTION BEGIN (constrained)  */

#define S390_VRS_A_OPCODE_LIST(V)                                                                                                                              \
    V(vesl, VESL, 0xE730) /* type = VRS_A VECTOR ELEMENT SHIFT LEFT  */                                                                                        \
    V(verll, VERLL, 0xE733) /* type = VRS_A VECTOR ELEMENT ROTATE LEFT LOGICAL  */                                                                             \
    V(vlm, VLM, 0xE736) /* type = VRS_A VECTOR LOAD MULTIPLE  */                                                                                               \
    V(vesrl, VESRL, 0xE738) /* type = VRS_A VECTOR ELEMENT SHIFT RIGHT LOGICAL  */                                                                             \
    V(vesra, VESRA, 0xE73A) /* type = VRS_A VECTOR ELEMENT SHIFT RIGHT ARITHMETIC  */                                                                          \
...
#define S390_RIL_A_OPCODE_LIST(V)                                                                                                                              \
    ...
    V(slgfi, SLGFI, 0xC24) /* type = RIL_A SUBTRACT LOGICAL IMMEDIATE (64<-32)  */                                                                             \
    ...
    V(alsih, ALSIH, 0xCCA) /* type = RIL_A ADD LOGICAL WITH SIGNED IMMEDIATE HIGH (32)  */                                                                     \
    V(alsihn, ALSIHN, 0xCCB) /* type = RIL_A ADD LOGICAL WITH SIGNED IMMEDIATE HIGH (32)  */                                                                   \
...
#define S390_RIL_B_OPCODE_LIST(V)                                                                                                                              \
    ...
    V(llhrl, LLHRL, 0xC42) /* type = RIL_B LOAD LOGICAL HALFWORD RELATIVE LONG (32<-16)  */                                                                    \
    V(lghrl, LGHRL, 0xC44) /* type = RIL_B LOAD HALFWORD RELATIVE LONG (64<-16)  */                                                                            \
    V(lhrl, LHRL, 0xC45) /* type = RIL_B LOAD HALFWORD RELATIVE LONG (32<-16) */                                                                               \
    V(llghrl, LLGHRL, 0xC46) /* type = RIL_B LOAD LOGICAL HALFWORD RELATIVE LONG (64<-16)  */                                                                  \
    ...
    V(llgfrl, LLGFRL, 0xC4E) /* type = RIL_B LOAD LOGICAL RELATIVE LONG (64<-32)  */                                                                           \
    V(strl, STRL, 0xC4F) /* type = RIL_B STORE RELATIVE LONG (32)  */                                                                                          \
    V(exrl, EXRL, 0xC60) /* type = RIL_B EXECUTE RELATIVE LONG  */                                                                                             \
    V(cghrl, CGHRL, 0xC64) /* type = RIL_B COMPARE HALFWORD RELATIVE LONG (64<-16)  */                                                                         \
    V(chrl, CHRL, 0xC65) /* type = RIL_B COMPARE HALFWORD RELATIVE LONG (32<-16)  */                                                                           \
    V(clghrl, CLGHRL, 0xC66) /* type = RIL_B COMPARE LOGICAL RELATIVE LONG (64<-16)  */                                                                        \
    V(clhrl, CLHRL, 0xC67) /* type = RIL_B COMPARE LOGICAL RELATIVE LONG (32<-16)  */                                                                          \
    ...
    V(clgfrl, CLGFRL, 0xC6E) /* type = RIL_B COMPARE LOGICAL RELATIVE LONG (64<-32)  */                                                                        \
...
#define S390_VRS_C_OPCODE_LIST(V) V(vlgv, VLGV, 0xE721) /* type = VRS_C VECTOR LOAD GR FROM VR ELEMENT  */
...
#define S390_RSI_OPCODE_LIST(V)                                                                                                                                \
    ...
    V(brxle, BRXLE, 0x85) /* type = RSI   BRANCH RELATIVE ON INDEX LOW OR EQ. (32)  */
...
#define S390_RI_C_OPCODE_LIST(V) V(brc, BRC, 0xA74) /* type = RI_C BRANCH RELATIVE ON CONDITION  */

#define S390_SMI_OPCODE_LIST(V) V(bpp, BPP, 0xC7) /* type = SMI   BRANCH PREDICTION PRELOAD  */
...
#define S390_RXY_A_OPCODE_LIST(V)                                                                                                                              \
    ...
    V(llgt, LLGT, 0xE317) /* type = RXY_A LOAD LOGICAL THIRTY ONE BITS (64<-31)  */                                                                            \
    ...
    V(llzrgf, LLZRGF, 0xE33A) /* type = RXY_A LOAD LOGICAL AND ZERO RIGHTMOST BYTE (64<-32)  */                                                                \
    ...
    V(llgtat, LLGTAT, 0xE39C) /* type = RXY_A LOAD LOGICAL THIRTY ONE BITS AND TRAP (64<-31)  */                                                               \
...
#define S390_RXY_B_OPCODE_LIST(V) V(pfd, PFD, 0xE336) /* type = RXY_B PREFETCH DATA  */
...
#define S390_SIY_OPCODE_LIST(V)                                                                                                                                \
    ...
    V(alsi, ALSI, 0xEB6E) /* type = SIY   ADD LOGICAL WITH SIGNED IMMEDIATE (32<-8)  */                                                                        \
    V(agsi, AGSI, 0xEB7A) /* type = SIY   ADD IMMEDIATE (64<-8)  */                                                                                            \
    V(algsi, ALGSI, 0xEB7E) /* type = SIY   ADD LOGICAL WITH SIGNED IMMEDIATE (64<-8)  */
...
#define S390_SS_C_OPCODE_LIST(V) V(srp, SRP, 0xF0) /* type = SS_C  SHIFT AND ROUND DECIMAL  */
...
#define S390_I_OPCODE_LIST(V) V(svc, SVC, 0x0A) /* type = I     SUPERVISOR CALL  */
...
#define S390_RX_B_OPCODE_LIST(V) V(bc, BC, 0x47) /* type = RX_B  BRANCH ON CONDITION  */

#define S390_RIE_A_OPCODE_LIST(V)                                                                                                                              \
    V(cgit, CGIT, 0xEC70) /* type = RIE_A COMPARE IMMEDIATE AND TRAP (64<-16) */                                                                               \
    V(clgit, CLGIT, 0xEC71) /* type = RIE_A COMPARE LOGICAL IMMEDIATE AND TRAP (64<-16)  */                                                                    \
    V(cit, CIT, 0xEC72) /* type = RIE_A COMPARE IMMEDIATE AND TRAP (32<-16)  */                                                                                \
    V(clfit, CLFIT, 0xEC73) /* type = RIE_A COMPARE LOGICAL IMMEDIATE AND TRAP (32<-16)  */
...
#define S390_RRD_OPCODE_LIST(V)                                                                                                                                \
    ...
    V(maylr, MAYLR, 0xB338) /* type = RRD   MULTIPLY AND ADD UNNRM. (long to ext. low HFP)  */                                                                 \
    V(mylr, MYLR, 0xB339) /* type = RRD   MULTIPLY UNNORM. (long to ext. low HFP)  */                                                                          \
    V(mayr, MAYR, 0xB33A) /* type = RRD   MULTIPLY & ADD UNNORMALIZED (long to ext. HFP)  */                                                                   \
    V(myr, MYR, 0xB33B) /* type = RRD   MULTIPLY UNNORMALIZED (long to ext. HFP)  */                                                                           \
    V(mayhr, MAYHR, 0xB33C) /* type = RRD   MULTIPLY AND ADD UNNRM. (long to ext. high HFP) */                                                                 \
    V(myhr, MYHR, 0xB33D) /* type = RRD   MULTIPLY UNNORM. (long to ext. high HFP)  */                                                                         \
...
#define S390_RIE_B_OPCODE_LIST(V)                                                                                                                              \
    ...
    V(clgrj, CLGRJ, 0xEC65) /* type = RIE_B COMPARE LOGICAL AND BRANCH RELATIVE (64)  */                                                                       \
    V(crj, CRJ, 0xEC76) /* type = RIE_B COMPARE AND BRANCH RELATIVE (32)  */                                                                                   \
    V(clrj, CLRJ, 0xEC77) /* type = RIE_B COMPARE LOGICAL AND BRANCH RELATIVE (32)  */
...
#define S390_RRE_OPCODE_LIST(V)                                                                                                                                \
    ...
    V(ldebr, LDEBR, 0xB304) /* type = RRE   LOAD LENGTHENED (short to long BFP)  */                                                                            \
    V(lxdbr, LXDBR, 0xB305) /* type = RRE   LOAD LENGTHENED (long to extended BFP)  */                                                                         \
    V(lxebr, LXEBR, 0xB306) /* type = RRE   LOAD LENGTHENED (short to extended BFP)  */                                                                        \
    ...
    V(lxdr, LXDR, 0xB325) /* type = RRE   LOAD LENGTHENED (long to extended HFP)  */                                                                           \
    V(lxer, LXER, 0xB326) /* type = RRE   LOAD LENGTHENED (short to extended HFP)  */                                                                          \
    ...
    V(thder, THDER, 0xB358) /* type = RRE   CONVERT BFP TO HFP (short to long)  */                                                                             \
    ...
    V(lexr, LEXR, 0xB366) /* type = RRE   LOAD ROUNDED (extended to short HFP)  */                                                                             \
    ...
    V(cefr, CEFR, 0xB3B4) /* type = RRE   CONVERT FROM FIXED (32 to short HFP)  */                                                                             \
    V(cdfr, CDFR, 0xB3B5) /* type = RRE   CONVERT FROM FIXED (32 to long HFP) */                                                                               \
    V(cxfr, CXFR, 0xB3B6) /* type = RRE   CONVERT FROM FIXED (32 to extended HFP)  */                                                                          \
    V(ldgr, LDGR, 0xB3C1) /* type = RRE   LOAD FPR FROM GR (64 to long)  */                                                                                    \
    V(cegr, CEGR, 0xB3C4) /* type = RRE   CONVERT FROM FIXED (64 to short HFP)  */                                                                             \
    V(cdgr, CDGR, 0xB3C5) /* type = RRE   CONVERT FROM FIXED (64 to long HFP) */                                                                               \
    V(cxgr, CXGR, 0xB3C6) /* type = RRE   CONVERT FROM FIXED (64 to extended HFP)  */                                                                          \
    ...
    V(eedtr, EEDTR, 0xB3E5) /* type = RRE   EXTRACT BIASED EXPONENT (long DFP to 64)  */                                                                       \
    V(esdtr, ESDTR, 0xB3E7) /* type = RRE   EXTRACT SIGNIFICANCE (long DFP to 64)  */                                                                          \
    V(kxtr, KXTR, 0xB3E8) /* type = RRE   COMPARE AND SIGNAL (extended DFP)  */                                                                                \
    V(cuxtr, CUXTR, 0xB3EA) /* type = RRE   CONVERT TO UNSIGNED PACKED (extended DFP       */                                                                  \
    /* CUXTR to 128)  */                                                                                                                                       \
    V(cxtr, CXTR, 0xB3EC) /* type = RRE   COMPARE (extended DFP)  */                                                                                           \
    V(eextr, EEXTR, 0xB3ED) /* type = RRE   EXTRACT BIASED EXPONENT (extended DFP to 64)  */                                                                   \
    V(esxtr, ESXTR, 0xB3EF) /* type = RRE   EXTRACT SIGNIFICANCE (extended DFP to 64)  */                                                                      \
    V(cdutr, CDUTR, 0xB3F2) /* type = RRE   CONVERT FROM UNSIGNED PACKED (64 to long DFP)  */                                                                  \
    V(cdstr, CDSTR, 0xB3F3) /* type = RRE   CONVERT FROM SIGNED PACKED (64 to long DFP)  */                                                                    \
    V(cedtr, CEDTR, 0xB3F4) /* type = RRE   COMPARE BIASED EXPONENT (long DFP)  */                                                                             \
    V(cxutr, CXUTR, 0xB3FA) /* type = RRE   CONVERT FROM UNSIGNED PACKED (128 to ext. DFP)  */                                                                 \
    V(cxstr, CXSTR, 0xB3FB) /* type = RRE   CONVERT FROM SIGNED PACKED (128 to*/                                                                               \
    /* extended DFP)  */                                                                                                                                       \
    V(cextr, CEXTR, 0xB3FC) /* type = RRE   COMPARE BIASED EXPONENT (extended DFP)  */                                                                         \
    ...
    V(llgtr, LLGTR, 0xB917) /* type = RRE   LOAD LOGICAL THIRTY ONE BITS (64<-31)  */                                                                          \
    ...
    V(pckmo, PCKMO, 0xB928) /* type = RRE   PERFORM CRYPTOGRAPHIC KEY MGMT. OPERATIONS  */                                                                     \
    ...
    V(ppno, PPNO, 0xB93C) /* type = RRE   PERFORM PSEUDORANDOM NUMBER OPERATION  */                                                                            \
    ...
    V(cdftr, CDFTR, 0xB951) /* type = RRE   CONVERT FROM FIXED (32 to long DFP)  */                                                                            \
    V(cxftr, CXFTR, 0xB959) /* type = RRE   CONVERT FROM FIXED (32 to extended DFP)  */                                                                        \
    ...
    V(esair, ESAIR, 0xB99B) /* type = RRE   EXTRACT SECONDARY ASN AND INSTANCE  */                                                                             \
...
#define S390_RIE_C_OPCODE_LIST(V)                                                                                                                              \
    V(cgij, CGIJ, 0xEC7C) /* type = RIE_C COMPARE IMMEDIATE AND BRANCH RELATIVE (64<-8)  */                                                                    \
    V(clgij, CLGIJ, 0xEC7D) /* type = RIE_C COMPARE LOGICAL IMMEDIATE AND BRANCH RELATIVE  */                                                                  \
    /* (64<-8)  */                                                                                                                                             \
    V(cij, CIJ, 0xEC7E) /* type = RIE_C COMPARE IMMEDIATE AND BRANCH RELATIVE (32<-8)  */                                                                      \
...
#define S390_RIE_D_OPCODE_LIST(V)                                                                                                                              \
    ...
    V(alhsik, ALHSIK, 0xECDA) /* type = RIE_D ADD LOGICAL WITH SIGNED IMMEDIATE (32<-16)  */                                                                   \
    V(alghsik, ALGHSIK, 0xECDB) /* type = RIE_D ADD LOGICAL WITH SIGNED IMMEDIATE (64<-16)  */
...
#define S390_RIE_E_OPCODE_LIST(V)                                                                                                                              \
    V(brxhg, BRXHG, 0xEC44) /* type = RIE_E BRANCH RELATIVE ON INDEX HIGH (64)  */                                                                             \
    V(brxlg, BRXLG, 0xEC45) /* type = RIE_E BRANCH RELATIVE ON INDEX LOW OR EQ. (64)  */
...
#define S390_RIE_F_OPCODE_LIST(V)                                                                                                                              \
    V(risblg, RISBLG, 0xEC51) /* type = RIE_F ROTATE THEN INSERT SELECTED BITS LOW (64)  */                                                                    \
    V(rnsbg, RNSBG, 0xEC54) /* type = RIE_F ROTATE THEN AND SELECTED BITS (64)  */                                                                             \
    V(risbg, RISBG, 0xEC55) /* type = RIE_F ROTATE THEN INSERT SELECTED BITS (64)  */                                                                          \
    V(rosbg, ROSBG, 0xEC56) /* type = RIE_F ROTATE THEN OR SELECTED BITS (64) */                                                                               \
    V(rxsbg, RXSBG, 0xEC57) /* type = RIE_F ROTATE THEN EXCLUSIVE OR SELECT. BITS (64)  */                                                                     \
    V(risbgn, RISBGN, 0xEC59) /* type = RIE_F ROTATE THEN INSERT SELECTED BITS (64)  */                                                                        \
    V(risbhg, RISBHG, 0xEC5D) /* type = RIE_F ROTATE THEN INSERT SELECTED BITS HIGH (64)  */
...
#define S390_VRX_OPCODE_LIST(V)                                                                                                                                \
    ...
    V(vllez, VLLEZ, 0xE704) /* type = VRX   VECTOR LOAD LOGICAL ELEMENT AND ZERO  */                                                                           \
    ...
    V(vlbrrep, VLBRREP, 0xE605) /* type = VRX VECTOR LOAD BYTE REVERSED ELEMENT AND REPLICATE */                                                               \
    V(vlebrh, VLEBRH, 0xE601) /* type = VRX VECTOR LOAD BYTE REVERSED ELEMENT (16) */                                                                          \
    V(vlebrf, VLEBRF, 0xE603) /* type = VRX VECTOR LOAD BYTE REVERSED ELEMENT (32) */                                                                          \
    V(vlebrg, VLEBRG, 0xE602) /* type = VRX VECTOR LOAD BYTE REVERSED ELEMENT (64) */                                                                          \
    ...
    V(vstebrh, VSTEBRH, 0xE609) /* type = VRX VECTOR STORE BYTE REVERSED ELEMENT (16) */                                                                       \
    V(vstebrf, VSTEBRF, 0xE60B) /* type = VRX VECTOR STORE BYTE REVERSED ELEMENT (32) */                                                                       \
    V(vstebrg, VSTEBRG, 0xE60A) /* type = VRX VECTOR STORE BYTE REVERSED ELEMENT (64) */

#define S390_RIE_G_OPCODE_LIST(V)                                                                                                                              \
    V(lochi, LOCHI, 0xEC42) /* type = RIE_G LOAD HALFWORD IMMEDIATE ON CONDITION (32<-16)  */                                                                  \
    V(locghi, LOCGHI, 0xEC46) /* type = RIE_G LOAD HALFWORD IMMEDIATE ON CONDITION (64<-16)  */                                                                \
...
enum Opcode {
#define DECLARE_OPCODES(name, opcode_name, opcode_value) opcode_name = opcode_value,
...
enum CheckForInexactConversion { kCheckForInexactConversion, kDontCheckForInexactConversion };
...
public:
    ...
    template <typename T> inline T InstructionBits() const
    {
        return Instruction::InstructionBits<T>(reinterpret_cast<const uint8_t*>(this));
    }
    ...
    template <typename T> inline void SetInstructionBits(T value, WritableJitAllocation* jit_allocation = nullptr) const
    {
        Instruction::SetInstructionBits<T>(reinterpret_cast<const uint8_t*>(this), value, jit_allocation);
    }
    ...
    template <typename T, typename U> inline U Bits(int hi, int lo) const
    {
    ...
    {
        return Instruction::InstructionLength(reinterpret_cast<const uint8_t*>(this));
    }
    ...
    template <typename T> static inline T InstructionBits(const uint8_t* instr)
    {
    ...
    template <typename T> static inline void SetInstructionBits(uint8_t* instr, T value, WritableJitAllocation* jit_allocation = nullptr)
    {
#if V8_TARGET_LITTLE_ENDIAN
        ...
        if (sizeof(T) == 2) {
            ...
            value = (static_cast<uint64_t>(orig_value & 0xFF) << 40) | (static_cast<uint64_t>((orig_value >> 8) & 0xFF) << 32)
                | (static_cast<uint64_t>((orig_value >> 16) & 0xFF) << 24) | (static_cast<uint64_t>((orig_value >> 24) & 0xFF) << 16)
                | (static_cast<uint64_t>((orig_value >> 32) & 0xFF) << 8) | (static_cast<uint64_t>((orig_value >> 40) & 0xFF));
        }
#endif
        if (sizeof(T) <= 4) {
            if (jit_allocation) {
                jit_allocation->WriteUnalignedValue(reinterpret_cast<Address>(instr), value);
            } else {
            ...
            uint64_t orig_value = static_cast<uint64_t>(value);
            if (jit_allocation) {
                jit_allocation->WriteUnalignedValue(reinterpret_cast<Address>(instr), static_cast<uint32_t>(value));
                jit_allocation->WriteUnalignedValue(reinterpret_cast<Address>(instr + 4), static_cast<uint16_t>((orig_value >> 32) & 0xFFFF));
            } else {
...
#else
            if (jit_allocation) {
                jit_allocation->WriteUnalignedValue(reinterpret_cast<Address>(instr), static_cast<uint32_t>(value >> 16));
                jit_allocation->WriteUnalignedValue(reinterpret_cast<Address>(instr + 4), static_cast<uint16_t>(value & 0xFFFF));
            } else {
