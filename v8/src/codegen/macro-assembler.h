...
public:
    explicit FrameScope(MacroAssembler* masm, StackFrame::Type type, const SourceLocation& loc = SourceLocation())
        :
...
public:
    FrameAndConstantPoolScope(MacroAssembler* masm, StackFrame::Type type)
        ...
        , type_(type)
        , old_has_frame_(masm->has_frame())
        , old_constant_pool_available_(V8_EMBEDDED_CONSTANT_POOL_BOOL && masm->is_constant_pool_available())
    {
        masm->set_has_frame(true);
...
public:
    explicit ConstantPoolUnavailableScope(Assembler* assembler)
        ...
        , old_constant_pool_available_(V8_EMBEDDED_CONSTANT_POOL_BOOL && assembler->is_constant_pool_available())
    {
        if (V8_EMBEDDED_CONSTANT_POOL_BOOL) {
...
public:
    ...
    ~NoRootArrayScope()
    {
        masm_->set_root_array_available(old_value_);
    }
