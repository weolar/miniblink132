...
CodeCommentsIterator::CodeCommentsIterator(Address code_comments_start, uint32_t code_comments_size)
    ...
    , current_entry_(code_comments_start + kOffsetToFirstCommentEntry)
{
    ...
    DCHECK_IMPLIES(code_comments_size, code_comments_size == base::ReadUnalignedValue<uint32_t>(code_comments_start_));
}

uint32_t CodeCommentsIterator::size() const
{
    return code_comments_size_;
}
