...
class Immediate {
public:
    template <typename T> inline explicit Immediate(Handle<T> handle, RelocInfo::Mode mode = RelocInfo::FULL_EMBEDDED_OBJECT);

    // This is allowed to be an implicit constructor because Immediate is
    // a wrapper class that doesn't normally perform any type conversion.
    template <typename T> inline Immediate(T value); // NOLINT(runtime/explicit)

    template <typename T> inline Immediate(T value, RelocInfo::Mode rmode);
...
public:
    ...
    template <typename T> inline Operand(T t); // NOLINT(runtime/explicit)

    // Implicit constructor for int types.
    template <typename T> inline Operand(T t, RelocInfo::Mode rmode);
...
public:
    ...
    inline explicit MemOperand(Register base, Register regoffset, Shift shift = LSL, unsigned shift_amount = 0);
    inline explicit MemOperand(Register base, Register regoffset, Extend extend, unsigned shift_amount = 0);
...
public:
    explicit AssemblerZone(const MaybeAssemblerZone& zone)
        ...
        : maybe_local_zone_(std::holds_alternative<Zone*>(zone) ? std::nullopt : std::make_optional<Zone>(std::get<AccountingAllocator*>(zone), ZONE_NAME))
        , zone_(std::holds_alternative<Zone*>(zone) ? std::get<Zone*>(zone) : &maybe_local_zone_.value())
    {
    }
...
public:
    ...
    inline void set_embedded_object_index_referenced_from(Address p, EmbeddedObjectIndex index);
    ...
    enum NopMarkerTypes { DEBUG_BREAK_NOP, INTERRUPT_CODE_NOP, ADR_FAR_NOP, FIRST_NOP_MARKER = DEBUG_BREAK_NOP, LAST_NOP_MARKER = ADR_FAR_NOP };
    ...
    {
        bool offset_is_size_multiple = (static_cast<int64_t>(static_cast<uint64_t>(offset >> size_log2) << size_log2) == offset);
...
protected:
    ...
    inline void DataProcPlainRegister(const Register& rd, const Register& rn, const Register& rm, Instr op);
    inline void CmpPlainRegister(const Register& rn, const Register& rm);
    inline void DataProcImmediate(const Register& rd, const Register& rn, int immediate, Instr op);
    ...
    static inline LoadStorePairOp LoadPairOpFor(const CPURegister& rt, const CPURegister& rt2);
    static inline LoadStoreOp StoreOpFor(const CPURegister& rt);
    static inline LoadStorePairOp StorePairOpFor(const CPURegister& rt, const CPURegister& rt2);
...
public:
    ...
    size_t GetConstantPoolEntriesSizeForTesting() const
    {
        ...
        return constpool_.Entry32Count() * kInt32Size + constpool_.Entry64Count() * kInt64Size;
    }
...
public:
    ...
    PatchingAssembler(Zone* zone, const AssemblerOptions& options, uint8_t* start, unsigned count)
        : Assembler(zone, options, ExternalAssemblerBuffer(start, count * kInstrSize + kGap))
