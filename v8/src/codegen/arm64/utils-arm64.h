...
template <typename T> T ReverseBytes(T value, int block_bytes_log2)
{
    ...
    static const uint8_t permute_table[3][8] = { { 6, 7, 4, 5, 2, 3, 0, 1 }, { 4, 5, 6, 7, 0, 1, 2, 3 }, { 0, 1, 2, 3, 4, 5, 6, 7 } };
...
template <typename T> inline bool IsQuietNaN(T num)
{
...
{
    DCHECK(std::isnan(num));
    return base::bit_cast<float>(base::bit_cast<uint32_t>(num) | static_cast<uint32_t>(kSQuietNanMask));
}
