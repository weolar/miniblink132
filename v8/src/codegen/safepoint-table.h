...
public:
    ...
    {
        return this->SafepointEntryBase::operator==(other) && tagged_register_indexes_ == other.tagged_register_indexes_
...
public:
    ...
    explicit SafepointTable(Isolate* isolate, Address pc, Tagged<InstructionStream> code);
    ...
    SafepointEntry GetEntry(int index) const
    {
        ...
        if (has_deopt_data()) {
            ...
            DCHECK(trampoline_pc >= 0 || trampoline_pc == SafepointEntry::kNoTrampolinePC);
        }
        ...
        uint8_t* tagged_slots_start = reinterpret_cast<uint8_t*>(safepoint_table_address_ + kHeaderSize + length_ * entry_size());
        base::Vector<uint8_t> tagged_slots(tagged_slots_start + index * tagged_slots_bytes(), tagged_slots_bytes());

        return SafepointEntry(pc, deopt_index, tagged_register_indexes, tagged_slots, trampoline_pc);
    }
...
private:
    struct EntryBuilder {
        ...
        EntryBuilder(Zone* zone, int pc)
            : pc(pc)
            , stack_indexes(zone->New<GrowableBitVector>())
        {
        }
    };
...
public:
    ...
    public:
        ...
        {
            DCHECK_LT(reg_code, kBitsPerByte * sizeof(EntryBuilder::register_indexes));
...
#endif // DEBUG
        if (index < min_stack_index_)
            min_stack_index_ = index;
    }

    int min_stack_index() const
    {
        return min_stack_index_ == std::numeric_limits<int>::max() ? 0 : min_stack_index_;
    }
