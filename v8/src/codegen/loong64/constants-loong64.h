...
const Instr rtCallRedirInstr = static_cast<uint32_t>(BREAK) | call_rt_redirected;
...
public:
    enum Type { kOp6Type, kOp7Type, kOp8Type, kOp10Type, kOp12Type, kOp14Type, kOp17Type, kOp22Type, kUnsupported = -1 };
...
public:
    ...
    inline int Offs21Value() const
    {
        ...
        int high = this->Bits(kOffs21HighShift + kOffs21HighBits - 1, kOffs21HighShift);
    ...
    {
        int low = this->Bits(kOffsLowShift + kOffsLowBits - 1, kOffsLowShift);
        int high = this->Bits(kOffs26HighShift + kOffs26HighBits - 1, kOffs26HighShift);
        return ((high << kOffsLowBits) + low);
    }
    ...
    /*TODO*/
    inline int32_t Imm12Value() const
    {
        abort();
    }

    inline int32_t Imm14Value() const
    {
        abort();
    }
