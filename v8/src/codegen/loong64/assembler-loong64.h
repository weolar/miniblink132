...
public:
    // Immediate.
    V8_INLINE explicit Operand(int64_t immediate, RelocInfo::Mode rmode = RelocInfo::NO_INFO)
        : rm_(no_reg)
        , rmode_(rmode)
    {
...
public:
    ...
    enum OffsetSize : int { kOffset26 = 26, kOffset21 = 21, kOffset20 = 20, kOffset16 = 16 };
    ...
    {
        set_target_compressed_value_at(pc, target, jit_allocation, icache_flush_mode);
    }

    inline Handle<Code> code_target_object_handle_at(Address pc, Address constant_pool);
    ...
    inline Handle<HeapObject> embedded_object_handle_at(Address pc, Address constant_pool);
    ...
    {
        b(shifted_branch_offset26(L));
    }
    void bl(int32_t offset);
    inline void bl(Label* L)
    {
        bl(shifted_branch_offset26(L));
    }
...
private:
    ...
    {
        prev_instr_compact_branch_ = true;
    }
...
public:
    ...
    {
        available_->clear(list);
    }
    void ExcludeFp(const DoubleRegList& list)
    {
        availablefp_->clear(list);
    }
