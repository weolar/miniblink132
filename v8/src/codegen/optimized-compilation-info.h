...
#undef DEF_SETTER
    ...
    // For testing.
    OptimizedCompilationInfo(Zone* zone, Isolate* isolate, IndirectHandle<SharedFunctionInfo> shared, IndirectHandle<JSFunction> closure, CodeKind code_kind)
        : OptimizedCompilationInfo(zone, isolate, shared, closure, code_kind, BytecodeOffset::None())
    {
    }
    ...
    {
        return code_kind() == CodeKind::WASM_TO_JS_FUNCTION || code_kind() == CodeKind::WASM_TO_CAPI_FUNCTION || code_kind() == CodeKind::JS_TO_WASM_FUNCTION
            || (code_kind() == CodeKind::BUILTIN
                && (builtin() == Builtin::kJSToWasmWrapper || builtin() == Builtin::kJSToWasmHandleReturns || builtin() == Builtin::kWasmToJsWrapperCSA
    ...
    template <typename T> IndirectHandle<T> CanonicalHandle(Tagged<T> object, Isolate* isolate)
    {
        DCHECK_NOT_NULL(canonical_handles_);
    ...
    struct InlinedFunctionHolder {
        ...
        IndirectHandle<BytecodeArray> bytecode_array; // Explicit to prevent flushing.
