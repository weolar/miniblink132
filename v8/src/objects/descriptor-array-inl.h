...
RELAXED_INT16_ACCESSORS(DescriptorArray, number_of_all_descriptors, kNumberOfAllDescriptorsOffset)
RELAXED_INT16_ACCESSORS(DescriptorArray, number_of_descriptors, kNumberOfDescriptorsOffset)
...
InternalIndex DescriptorArray::Search(Tagged<Name> name, int valid_descriptors, bool concurrent_search)
{
...
InternalIndex DescriptorArray::BinarySearch(Tagged<Name> name, int valid_descriptors)
{
    ...
    // search hash.
    int number = *std::ranges::lower_bound(std::views::iota(0, end), hash, std::less<>(), [&](int i) {
...
InternalIndex DescriptorArray::LinearSearch(Tagged<Name> name, int valid_descriptors)
{
    DCHECK_LE(valid_descriptors, number_of_descriptors());
    for (int i = 0; i < valid_descriptors; ++i) {
        InternalIndex index(i);
        if (GetKey(index) == name)
            return index;
    }
    return InternalIndex::NotFound();
}

InternalIndex DescriptorArray::Search(Tagged<Name> name, Tagged<Map> map, bool concurrent_search)
{
    DCHECK(IsUniqueName(name));
...
{
    for (int desc_index = field_index; desc_index < valid_descriptors; ++desc_index) {
...
InternalIndex DescriptorArray::SearchWithCache(Isolate* isolate, Tagged<Name> name, Tagged<Map> map)
{
    DCHECK(IsUniqueName(name));
    int number_of_own_descriptors = map->NumberOfOwnDescriptors();
    if (number_of_own_descriptors == 0)
...
{
    static_assert(kEndOfStrongFieldsOffset == kStartOfWeakFieldsOffset, "Weak and strong fields are continuous.");
    static_assert(kEndOfWeakFieldsOffset == kHeaderSize, "Weak fields extend up to the end of the header.");
...
Tagged<Name> DescriptorArray::GetKey(PtrComprCageBase cage_base, InternalIndex descriptor_number) const
{
    DCHECK_LT(descriptor_number.as_int(), number_of_descriptors());
    int entry_offset = OffsetOfDescriptorAt(descriptor_number.as_int());
    return Cast<Name>(EntryKeyField::Relaxed_Load(cage_base, *this, entry_offset));
}

void DescriptorArray::SetKey(InternalIndex descriptor_number, Tagged<Name> key)
{
    DCHECK_LT(descriptor_number.as_int(), number_of_descriptors());
    int entry_offset = OffsetOfDescriptorAt(descriptor_number.as_int());
...
Tagged<Name> DescriptorArray::GetSortedKey(PtrComprCageBase cage_base, int descriptor_number)
{
...
Tagged<Object> DescriptorArray::GetStrongValue(InternalIndex descriptor_number)
{
    PtrComprCageBase cage_base = GetPtrComprCageBase(*this);
    return Cast<Object>(GetStrongValue(cage_base, descriptor_number));
}

Tagged<Object> DescriptorArray::GetStrongValue(PtrComprCageBase cage_base, InternalIndex descriptor_number)
{
    return Cast<Object>(GetValue(cage_base, descriptor_number));
}

void DescriptorArray::SetValue(InternalIndex descriptor_number, Tagged<MaybeObject> value)
{
    DCHECK_LT(descriptor_number.as_int(), number_of_descriptors());
    int entry_offset = OffsetOfDescriptorAt(descriptor_number.as_int());
...
Tagged<MaybeObject> DescriptorArray::GetValue(PtrComprCageBase cage_base, InternalIndex descriptor_number)
{
    DCHECK_LT(descriptor_number.as_int(), number_of_descriptors());
    int entry_offset = OffsetOfDescriptorAt(descriptor_number.as_int());
...
void DescriptorArray::SetDetails(InternalIndex descriptor_number, PropertyDetails details)
{
    DCHECK_LT(descriptor_number.as_int(), number_of_descriptors());
    int entry_offset = OffsetOfDescriptorAt(descriptor_number.as_int());
...
Tagged<FieldType> DescriptorArray::GetFieldType(InternalIndex descriptor_number)
{
    PtrComprCageBase cage_base = GetPtrComprCageBase(*this);
    return GetFieldType(cage_base, descriptor_number);
}

Tagged<FieldType> DescriptorArray::GetFieldType(PtrComprCageBase cage_base, InternalIndex descriptor_number)
{
    DCHECK_EQ(GetDetails(descriptor_number).location(), PropertyLocation::kField);
...
void DescriptorArray::Set(InternalIndex descriptor_number, Tagged<Name> key, Tagged<MaybeObject> value, PropertyDetails details)
{
...
void DescriptorArray::Append(Descriptor* desc)
{
    ...
    if (V8_LIKELY(collision_hash != desc_hash))
        return;
...
// static
bool DescriptorArrayMarkingState::TryUpdateIndicesToMark(unsigned gc_epoch, Tagged<DescriptorArray> array, DescriptorIndex index_to_mark)
{
    ...
    while (true) {
        ...
        if (current_epoch != epoch_from_state) {
            ...
            // by 1.
            DCHECK_IMPLIES(raw_gc_state != 0, Epoch::decode(epoch_from_state + 1) == current_epoch);
            ...
            new_raw_gc_state = NewState(current_epoch, already_marked, index_to_mark - already_marked);
        }
...
// static
std::pair<DescriptorArrayMarkingState::DescriptorIndex, DescriptorArrayMarkingState::DescriptorIndex> DescriptorArrayMarkingState::AcquireDescriptorRangeToMark(
    unsigned gc_epoch, Tagged<DescriptorArray> array)
{
    ...
    while (true) {
        ...
        if (current_epoch != Epoch::decode(raw_gc_state) || (marked + delta) == 0) {
            ...
            const int16_t number_of_descriptors = array->number_of_descriptors() ? array->number_of_descriptors() : array->number_of_all_descriptors();
            DCHECK_GT(number_of_descriptors, 0);
            if (SwapState(array, raw_gc_state, NewState(current_epoch, number_of_descriptors, 0))) {
        ...
        if (SwapState(array, raw_gc_state, NewState(current_epoch, marked + delta, 0))) {
