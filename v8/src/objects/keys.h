...
public:
    ...
    static MaybeHandle<FixedArray> GetKeys(Isolate* isolate, Handle<JSReceiver> object, KeyCollectionMode mode, PropertyFilter filter,
        GetKeysConversion keys_conversion = GetKeysConversion::kKeepNumbers, bool is_for_in = false, bool skip_indices = false);

    Handle<FixedArray> GetKeys(GetKeysConversion convert = GetKeysConversion::kKeepNumbers);
    Maybe<bool> CollectKeys(DirectHandle<JSReceiver> receiver, Handle<JSReceiver> object);
    ...
    V8_WARN_UNUSED_RESULT ExceptionStatus AddKey(Handle<Object> key, AddKeyConversion convert = DO_NOT_CONVERT);
...
private:
    ...
    Maybe<bool> CollectOwnElementIndices(DirectHandle<JSReceiver> receiver, Handle<JSObject> object);
    Maybe<bool> CollectOwnPropertyNames(DirectHandle<JSReceiver> receiver, Handle<JSObject> object);
    Maybe<bool> CollectOwnKeys(DirectHandle<JSReceiver> receiver, Handle<JSObject> object);
    Maybe<bool> CollectOwnJSProxyKeys(DirectHandle<JSReceiver> receiver, DirectHandle<JSProxy> proxy);
    Maybe<bool> CollectOwnJSProxyTargetKeys(DirectHandle<JSProxy> proxy, Handle<JSReceiver> target);
    ...
    Maybe<bool> AddKeysFromJSProxy(DirectHandle<JSProxy> proxy, Handle<FixedArray> keys);
    V8_WARN_UNUSED_RESULT ExceptionStatus AddKeys(DirectHandle<FixedArray> array, AddKeyConversion convert);
    V8_WARN_UNUSED_RESULT ExceptionStatus AddKeys(Handle<JSObject> array_like, AddKeyConversion convert);
    ...
    void set_receiver(Handle<JSReceiver> object)
    {
        receiver_ = object;
    }
...
public:
    FastKeyAccumulator(
        Isolate* isolate, Handle<JSReceiver> receiver, KeyCollectionMode mode, PropertyFilter filter, bool is_for_in = false, bool skip_indices = false)
        : isolate_(isolate)
        , receiver_(receiver)
        , mode_(mode)
        , filter_(filter)
    ...
    static Handle<FixedArray> InitializeFastPropertyEnumCache(
        Isolate* isolate, DirectHandle<Map> map, int enum_length, AllocationType allocation = AllocationType::kOld);
