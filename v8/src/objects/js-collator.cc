...
Maybe<CaseFirst> GetCaseFirst(Isolate* isolate, Handle<JSReceiver> options, const char* method_name)
{
    return GetStringOption<CaseFirst>(isolate, options, "caseFirst", method_name, { "upper", "lower", "false" },
        { CaseFirst::kUpper, CaseFirst::kLower, CaseFirst::kFalse }, CaseFirst::kUndefined);
}

// TODO(gsathya): Consider internalizing the value strings.
void CreateDataPropertyForOptions(Isolate* isolate, Handle<JSObject> options, Handle<String> key, const char* value)
{
    ...
    Maybe<bool> maybe = JSReceiver::CreateDataProperty(isolate, options, key, value_str, Just(kDontThrow));
    DCHECK(maybe.FromJust());
    USE(maybe);
}

void CreateDataPropertyForOptions(Isolate* isolate, Handle<JSObject> options, Handle<String> key, bool value)
{
    Handle<Object> value_obj = isolate->factory()->ToBoolean(value);

    // This is a brand new JSObject that shouldn't already have the same
    // key so this shouldn't fail.
    Maybe<bool> maybe = JSReceiver::CreateDataProperty(isolate, options, key, value_obj, Just(kDontThrow));
    DCHECK(maybe.FromJust());
    USE(maybe);
}
...
// static
Handle<JSObject> JSCollator::ResolvedOptions(Isolate* isolate, DirectHandle<JSCollator> collator)
{
    ...
    status = U_ZERO_ERROR;
    bool ignore_punctuation = icu_collator->getAttribute(UCOL_ALTERNATE_HANDLING, status) == UCOL_SHIFTED;
    DCHECK(U_SUCCESS(status));
    ...
    if (collator->locale()->length() != 0) {
        ...
        Maybe<bool> maybe = JSReceiver::CreateDataProperty(isolate, options, isolate->factory()->locale_string(), locale_from_collator, Just(kDontThrow));
        DCHECK(maybe.FromJust());
        USE(maybe);
    } else {
        // Just return from the collator for most of the cases that we can recover
        // from the collator.
        CreateDataPropertyForOptions(isolate, options, isolate->factory()->locale_string(), locale.c_str());
    }

    CreateDataPropertyForOptions(isolate, options, isolate->factory()->usage_string(), usage);
    CreateDataPropertyForOptions(isolate, options, isolate->factory()->sensitivity_string(), sensitivity);
    CreateDataPropertyForOptions(isolate, options, isolate->factory()->ignorePunctuation_string(), ignore_punctuation);
    CreateDataPropertyForOptions(isolate, options, isolate->factory()->collation_string(), collation);
    CreateDataPropertyForOptions(isolate, options, isolate->factory()->numeric_string(), numeric);
    CreateDataPropertyForOptions(isolate, options, isolate->factory()->caseFirst_string(), case_first);
    return options;
}
...
CaseFirst ToCaseFirst(const char* str)
{
    if (strcmp(str, "upper") == 0)
        return CaseFirst::kUpper;
    if (strcmp(str, "lower") == 0)
        return CaseFirst::kLower;
    if (strcmp(str, "false") == 0)
        return CaseFirst::kFalse;
...
void SetCaseFirstOption(icu::Collator* icu_collator, CaseFirst case_first)
{
    DCHECK_NOT_NULL(icu_collator);
    UErrorCode status = U_ZERO_ERROR;
    icu_collator->setAttribute(UCOL_CASE_FIRST, ToUColAttributeValue(case_first), status);
    DCHECK(U_SUCCESS(status));
}
...
// static
MaybeHandle<JSCollator> JSCollator::New(Isolate* isolate, DirectHandle<Map> map, Handle<Object> locales, Handle<Object> options_obj, const char* service)
{
    ...
    // "search" », "sort").
    Maybe<Usage> maybe_usage = GetStringOption<Usage>(isolate, options, "usage", service, { "sort", "search" }, { Usage::SORT, Usage::SEARCH }, Usage::SORT);
    ...
    Maybe<bool> maybe_collation = GetStringOption(isolate, options, "collation", empty_values, service, &collation_str);
    ...
    if (maybe_collation.FromJust() && collation_str != nullptr) {
        ...
        if (!JSLocale::Is38AlphaNumList(collation_str.get())) {
            THROW_NEW_ERROR_RETURN_VALUE(isolate,
                NewRangeError(
                    MessageTemplate::kInvalid, isolate->factory()->collation_string(), isolate->factory()->NewStringFromAsciiChecked(collation_str.get())),
    ...
    bool numeric;
    Maybe<bool> found_numeric = GetBoolOption(isolate, options, "numeric", service, &numeric);
    ...
    Maybe<Intl::ResolvedLocale> maybe_resolve_locale
        = Intl::ResolveLocale(isolate, JSCollator::GetAvailableLocales(), requested_locales, matcher, relevant_extension_keys);
    ...
    if (collation_str != nullptr) {
        ...
        if (co_extension_it != r.extensions.end() && co_extension_it->second != collation_str.get()) {
    ...
    if (usage == Usage::SEARCH) {
        ...
        if (collation_str != nullptr && Intl::IsValidCollation(icu_locale, collation_str.get())) {
    ...
    if (U_FAILURE(status) || icu_collator == nullptr) {
        ...
        icu_collator.reset(icu::Collator::createInstance(no_extension_locale, status));
    ...
    if (case_first != CaseFirst::kUndefined) {
        ...
        if (kf_extension_it != r.extensions.end()) {
            SetCaseFirstOption(icu_collator.get(), ToCaseFirst(kf_extension_it->second.c_str()));
        }
    }
    ...
    Maybe<Sensitivity> maybe_sensitivity = GetStringOption<Sensitivity>(isolate, options, "sensitivity", service, { "base", "accent", "case", "variant" },
        { Sensitivity::kBase, Sensitivity::kAccent, Sensitivity::kCase, Sensitivity::kVariant }, Sensitivity::kUndefined);
    ...
    Maybe<bool> found_ignore_punctuation = GetBoolOption(isolate, options, "ignorePunctuation", service, &ignore_punctuation);
    ...
    if (found_ignore_punctuation.FromJust()) {
        status = U_ZERO_ERROR;
        icu_collator->setAttribute(UCOL_ALTERNATE_HANDLING, ignore_punctuation ? UCOL_SHIFTED : UCOL_NON_IGNORABLE, status);
        DCHECK(U_SUCCESS(status));
    }

    DirectHandle<Managed<icu::Collator>> managed_collator = Managed<icu::Collator>::From(isolate, 0, std::move(icu_collator));

    // We only need to do so if it is different from the collator would return.
    DirectHandle<String> locale_str = isolate->factory()->NewStringFromAsciiChecked((collator_locale != icu_locale) ? r.locale.c_str() : "");
    // Now all properties are ready, so we can allocate the result object.
    Handle<JSCollator> collator = Cast<JSCollator>(isolate->factory()->NewFastOrSlowJSObjectFromMap(map));
...
public:
    CollatorAvailableLocales()
    {
        ...
        const icu::Locale* icu_available_locales = icu::Collator::getAvailableLocales(num_locales);
        std::vector<std::string> locales;
        locales.reserve(num_locales);
        for (int32_t i = 0; i < num_locales; ++i) {
            locales.push_back(Intl::ToLanguageTag(icu_available_locales[i]).FromJust());
        }
...
{
    static base::LazyInstance<CollatorAvailableLocales>::type available_locales = LAZY_INSTANCE_INITIALIZER;
