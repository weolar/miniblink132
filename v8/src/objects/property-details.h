...
static const int kPropertyAttributesCombinationsCount = 1 << kPropertyAttributesBitsCount;
...
// ALL_ATTRIBUTES_MASK.
static_assert((ALL_ATTRIBUTES_MASK == (READ_ONLY | DONT_ENUM | DONT_DELETE)) == (kPropertyAttributesBitsCount == 3));
...
public:
    ...
    {
        return Representation(kSmi);
    }
    static constexpr Representation Double()
    {
        return Representation(kDouble);
    }
    ...
    bool CanBeInPlaceChangedTo(const Representation& other) const
    {
        if (Equals(other))
            return true;
        if (IsWasmValue() || other.IsWasmValue())
            return false;
    ...
    {
        if (IsWasmValue())
            return Representation::WasmValue();
...
private:
    explicit constexpr Representation(Kind k)
        : kind_(k)
    {
    }
...
public:
    ...
    constexpr PropertyDetails(PropertyKind kind, PropertyAttributes attributes, PropertyCellType cell_type, int dictionary_index = 0)
        : value_(KindField::encode(kind) | LocationField::encode(PropertyLocation::kField) | AttributesField::encode(attributes) |
            ...
            ConstnessField::encode(PropertyConstness::kMutable) | DictionaryStorageField::encode(dictionary_index) | PropertyCellTypeField::encode(cell_type))
    {
    }
    ...
    constexpr PropertyDetails(PropertyKind kind, PropertyAttributes attributes, PropertyConstness constness, int dictionary_index = 0)
        : value_(KindField::encode(kind) | LocationField::encode(PropertyLocation::kField) | AttributesField::encode(attributes)
            | ConstnessField::encode(constness) | DictionaryStorageField::encode(dictionary_index) | PropertyCellTypeField::encode(PropertyCellType::kNoCell))
    {
    }

    // Property details for fast mode properties.
    constexpr PropertyDetails(PropertyKind kind, PropertyAttributes attributes, PropertyLocation location, PropertyConstness constness,
        Representation representation, int field_index = 0)
        : value_(KindField::encode(kind) | AttributesField::encode(attributes) | LocationField::encode(location) | ConstnessField::encode(constness)
            | RepresentationField::encode(EncodeRepresentation(representation)) | FieldIndexField::encode(field_index))
    {
    }
    ...
    using DescriptorPointer = RepresentationField::Next<uint32_t, kDescriptorIndexBitCount>;
    using FieldIndexField = DescriptorPointer::Next<uint32_t, kDescriptorIndexBitCount>;
    ...
    static constexpr PropertyConstness kConstIfDictConstnessTracking
        = V8_DICT_PROPERTY_CONST_TRACKING_BOOL ? PropertyConstness::kConst : PropertyConstness::kMutable;
...
private:
    ...
    {
        value_ = RepresentationField::update(value, EncodeRepresentation(representation));
    }
    ...
    explicit PropertyDetails(uint32_t value)
        : value_ { value }
    {
    }
...
inline PropertyConstness GeneralizeConstness(PropertyConstness a, PropertyConstness b)
{
...
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, PropertyCellType type);
} // namespace internal
} // namespace v8
