...
INT32_ACCESSORS(FeedbackMetadata, create_closure_slot_count, kCreateClosureSlotCountOffset)
...
// static
constexpr uint32_t FeedbackVector::FlagMaskForNeedsProcessingCheckFrom(CodeKind code_kind)
{
    ...
    uint32_t flag_mask = FeedbackVector::kFlagsTieringStateIsAnyRequested | FeedbackVector::kFlagsLogNextExecution;
...
{
    return SizeFor(slot_count(kAcquireLoad), create_closure_slot_count(kAcquireLoad));
}
...
{
    return shared_function_info(cage_base)->feedback_metadata(cage_base, kAcquireLoad);
}

RELAXED_INT32_ACCESSORS(FeedbackVector, invocation_count, kInvocationCountOffset)
...
RELAXED_UINT8_ACCESSORS(FeedbackVector, invocation_count_before_stable, kInvocationCountBeforeStableOffset)
...
void FeedbackVector::reset_osr_urgency()
{
    set_osr_urgency(0);
}
...
void FeedbackVector::reset_osr_state()
{
    set_osr_state(0);
}
...
void FeedbackVector::set_maybe_has_optimized_osr_code(bool value, CodeKind code_kind)
{
    if (code_kind == CodeKind::MAGLEV) {
...
{
    return invocation_count_before_stable(kRelaxedLoad) == kInvocationCountBeforeStableDeoptSentinel;
}

void FeedbackVector::set_was_once_deoptimized()
{
    set_invocation_count_before_stable(kInvocationCountBeforeStableDeoptSentinel, kRelaxedStore);
}
...
Tagged<Code> FeedbackVector::optimized_code(IsolateForSandbox isolate) const
{
    ...
    DCHECK_IMPLIES(!code.is_null(), maybe_has_maglev_code() || maybe_has_turbofan_code());
    DCHECK_IMPLIES(!code.is_null() && code->is_maglevved(), maybe_has_maglev_code());
    DCHECK_IMPLIES(!code.is_null() && code->is_turbofanned(), maybe_has_turbofan_code());
    return code;
}
...
bool FeedbackVector::has_optimized_code() const
{
    ...
    DCHECK_IMPLIES(!is_cleared, maybe_has_maglev_code() || maybe_has_turbofan_code());
    return !is_cleared;
}
...
std::optional<Tagged<Code>> FeedbackVector::GetOptimizedOsrCode(Isolate* isolate, FeedbackSlot slot)
{
    Tagged<MaybeObject> maybe_code = Get(isolate, slot);
    if (maybe_code.IsCleared())
        return {};

    Tagged<Code> code = Cast<CodeWrapper>(maybe_code.GetHeapObject())->code(isolate);
...
FeedbackSlot FeedbackVector::ToSlot(intptr_t index)
{
    ...
    DCHECK_LE(static_cast<uintptr_t>(index), static_cast<uintptr_t>(std::numeric_limits<int>::max()));
...
Tagged<MaybeObject> FeedbackVector::Get(PtrComprCageBase cage_base, FeedbackSlot slot) const
{
    Tagged<MaybeObject> value = raw_feedback_slots(cage_base, GetIndex(slot), kRelaxedLoad);
    DCHECK(!IsOfLegacyType(value));
    return value;
}

Handle<FeedbackCell> FeedbackVector::GetClosureFeedbackCell(Isolate* isolate, int index) const
{
    DCHECK_GE(index, 0);
...
Tagged<MaybeObject> FeedbackVector::SynchronizedGet(FeedbackSlot slot) const
{
    ...
    Tagged<MaybeObject> value = TaggedField<MaybeObject>::Acquire_Load(*this, offset);
    DCHECK(!IsOfLegacyType(value));
    return value;
}

void FeedbackVector::SynchronizedSet(FeedbackSlot slot, Tagged<MaybeObject> value, WriteBarrierMode mode)
{
    DCHECK(!IsOfLegacyType(value));
    const int i = slot.ToInt();
    DCHECK_LT(static_cast<unsigned>(i), static_cast<unsigned>(this->length()));
...
void FeedbackVector::Set(FeedbackSlot slot, Tagged<MaybeObject> value, WriteBarrierMode mode)
{
    DCHECK(!IsOfLegacyType(value));
...
template <CompareOperationFeedback::Type Feedback> bool Is(int type_feedback)
{
...
CompareOperationHint CompareOperationHintFromFeedback(int type_feedback)
{
    ...
    } else if (Is<CompareOperationFeedback::kReceiverOrNullOrUndefined>(type_feedback)) {
...
template <typename T> Handle<T> NexusConfig::NewHandle(Tagged<T> object) const
{
...
Tagged<MaybeObject> NexusConfig::GetFeedback(Tagged<FeedbackVector> vector, FeedbackSlot slot) const
{
    return vector->SynchronizedGet(slot);
}

void NexusConfig::SetFeedback(Tagged<FeedbackVector> vector, FeedbackSlot slot, Tagged<MaybeObject> feedback, WriteBarrierMode mode) const
{
    DCHECK(can_write());
...
{
    return value.IsCleared() ? MaybeObjectHandle() : MaybeObjectHandle(config()->NewHandle(value));
}
...
std::pair<Tagged<MaybeObject>, Tagged<MaybeObject>> FeedbackNexus::GetFeedbackPair() const
{
    if (config()->mode() == NexusConfig::BackgroundThread && feedback_cache_.has_value()) {
        return std::make_pair(FromHandle(feedback_cache_->first), FromHandle(feedback_cache_->second));
    }
    auto pair = FeedbackMetadata::GetSlotSize(kind()) == 2 ? config()->GetFeedbackPair(vector(), slot())
                                                           : std::make_pair(config()->GetFeedback(vector(), slot()), Tagged<MaybeObject>());
    if (config()->mode() == NexusConfig::BackgroundThread && !feedback_cache_.has_value()) {
        feedback_cache_ = std::make_pair(ToHandle(pair.first), ToHandle(pair.second));
    }
    return pair;
}

template <typename FeedbackType> void FeedbackNexus::SetFeedback(Tagged<FeedbackType> feedback, WriteBarrierMode mode)
{
    config()->SetFeedback(vector(), slot(), feedback, mode);
}

template <typename FeedbackType, typename FeedbackExtraType>
void FeedbackNexus::SetFeedback(Tagged<FeedbackType> feedback, WriteBarrierMode mode, Tagged<FeedbackExtraType> feedback_extra, WriteBarrierMode mode_extra)
{
    config()->SetFeedbackPair(vector(), slot(), feedback, mode, feedback_extra, mode_extra);
}

template <typename F> void FeedbackNexus::IterateMapsWithUnclearedHandler(F function) const
{
