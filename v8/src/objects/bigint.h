...
public:
    ...
    static const uint32_t kMaxLength = kMaxLengthBits / (kSystemPointerSize * kBitsPerByte);
...
public:
    ...
    static MaybeHandle<BigInt> BitwiseNot(Isolate* isolate, DirectHandle<BigInt> x);
    static MaybeHandle<BigInt> Exponentiate(Isolate* isolate, Handle<BigInt> base, DirectHandle<BigInt> exponent);
    static MaybeHandle<BigInt> Multiply(Isolate* isolate, Handle<BigInt> x, Handle<BigInt> y);
    static MaybeHandle<BigInt> Divide(Isolate* isolate, Handle<BigInt> x, DirectHandle<BigInt> y);
    static MaybeHandle<BigInt> Remainder(Isolate* isolate, Handle<BigInt> x, DirectHandle<BigInt> y);
    static MaybeHandle<BigInt> Add(Isolate* isolate, Handle<BigInt> x, Handle<BigInt> y);
    static MaybeHandle<BigInt> Subtract(Isolate* isolate, Handle<BigInt> x, Handle<BigInt> y);
    ...
    static Maybe<bool> EqualToString(Isolate* isolate, DirectHandle<BigInt> x, Handle<String> y);
    static bool EqualToNumber(DirectHandle<BigInt> x, Handle<Object> y);
    static Maybe<ComparisonResult> CompareToString(Isolate* isolate, DirectHandle<BigInt> x, Handle<String> y);
    ...
    V8_EXPORT_PRIVATE static ComparisonResult CompareToDouble(DirectHandle<BigInt> x, double y);

    static Handle<BigInt> AsIntN(Isolate* isolate, uint64_t n, Handle<BigInt> x);
    static MaybeHandle<BigInt> AsUintN(Isolate* isolate, uint64_t n, Handle<BigInt> x);

    V8_EXPORT_PRIVATE static Handle<BigInt> FromInt64(Isolate* isolate, int64_t n);
    V8_EXPORT_PRIVATE static Handle<BigInt> FromUint64(Isolate* isolate, uint64_t n);
    static MaybeHandle<BigInt> FromWords64(Isolate* isolate, int sign_bit, uint32_t words64_count, const uint64_t* words);
    ...
    static Handle<String> NoSideEffectsToString(Isolate* isolate, DirectHandle<BigInt> bigint);
    ...
    V8_EXPORT_PRIVATE static MaybeHandle<BigInt> FromNumber(Isolate* isolate, Handle<Object> number);

    // ECMAScript's ToBigInt (throws for Number input)
    V8_EXPORT_PRIVATE static MaybeHandle<BigInt> FromObject(Isolate* isolate, Handle<Object> obj);

    class BodyDescriptor;

private:
    template <typename IsolateT> friend class StringToBigIntHelper;
    ...
    V8_WARN_UNUSED_RESULT static MaybeHandle<BigInt> FromSerializedDigits(Isolate* isolate, uint32_t bitfield, base::Vector<const uint8_t> digits_storage);
} V8_OBJECT_END;
