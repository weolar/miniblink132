...
Tagged<TrustedByteArray> AbstractCode::SourcePositionTable(Isolate* isolate, Tagged<SharedFunctionInfo> sfi)
{
...
bool AbstractCode::contains(Isolate* isolate, Address inner_pointer)
{
    ...
    if (InstanceTypeChecker::IsCode(map_object)) {
        ...
        DCHECK(InstanceTypeChecker::IsBytecodeArray(map_object));
        return (address() <= inner_pointer) && (inner_pointer <= address() + Size(cage_base));
    }
}
