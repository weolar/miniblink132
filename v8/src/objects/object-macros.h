...
#if V8_CC_GNU
#define V8_OBJECT_PUSH _Pragma("pack(push)") _Pragma("pack(4)") _Pragma("GCC diagnostic push") _Pragma("GCC diagnostic error \"-Wpadded\"")
#define V8_OBJECT_POP _Pragma("pack(pop)") _Pragma("GCC diagnostic pop")
#elif V8_CC_MSVC
#define V8_OBJECT_PUSH __pragma(pack(push)) __pragma(pack(4)) __pragma(warning(push)) __pragma(warning(default : 4820))
...
#define OBJECT_CONSTRUCTORS_IMPL(Type, Super)                                                                                                                  \
    inline void Type::CheckTypeOnCast()                                                                                                                        \
    {                                                                                                                                                          \
...
#define DECL_RELAXED_INT32_ACCESSORS(name) DECL_RELAXED_PRIMITIVE_ACCESSORS(name, int32_t)

#define DECL_RELAXED_UINT32_ACCESSORS(name) DECL_RELAXED_PRIMITIVE_ACCESSORS(name, uint32_t)

#define DECL_RELAXED_UINT16_ACCESSORS(name) DECL_RELAXED_PRIMITIVE_ACCESSORS(name, uint16_t)

#define DECL_RELAXED_UINT8_ACCESSORS(name) DECL_RELAXED_PRIMITIVE_ACCESSORS(name, uint8_t)
...
#define TQ_FIELD_TYPE(name, tq_type) static constexpr const char* k##name##TqFieldType = tq_type;

#define DECL_FIELD_OFFSET_TQ(name, value, tq_type)                                                                                                             \
    static const int k##name##Offset = value;                                                                                                                  \
    TQ_FIELD_TYPE(name, tq_type)

#define DECL_SETTER(name, ...) inline void set_##name(__VA_ARGS__ value, WriteBarrierMode mode = UPDATE_WRITE_BARRIER);
...
#define DECL_ACCESSORS_STORE_TAG(name, type, tag_type) inline void set_##name(UNPAREN(type) value, tag_type, WriteBarrierMode mode = UPDATE_WRITE_BARRIER);

#define DECL_RELAXED_GETTER(name, ...) DECL_ACCESSORS_LOAD_TAG(name, (__VA_ARGS__), RelaxedLoadTag)

#define DECL_RELAXED_SETTER(name, ...) DECL_ACCESSORS_STORE_TAG(name, (__VA_ARGS__), RelaxedStoreTag)

#define DECL_RELAXED_ACCESSORS(name, ...)                                                                                                                      \
    DECL_RELAXED_GETTER(name, __VA_ARGS__)                                                                                                                     \
    DECL_RELAXED_SETTER(name, __VA_ARGS__)

#define DECL_ACQUIRE_GETTER(name, ...) DECL_ACCESSORS_LOAD_TAG(name, (__VA_ARGS__), AcquireLoadTag)

#define DECL_RELEASE_SETTER(name, ...) DECL_ACCESSORS_STORE_TAG(name, (__VA_ARGS__), ReleaseStoreTag)
...
#define INT_ACCESSORS(holder, name, offset) DEF_PRIMITIVE_ACCESSORS(holder, name, offset, int)

#define INT32_ACCESSORS(holder, name, offset) DEF_PRIMITIVE_ACCESSORS(holder, name, offset, int32_t)

#define UINT16_ACCESSORS(holder, name, offset) DEF_PRIMITIVE_ACCESSORS(holder, name, offset, uint16_t)

#define UINT8_ACCESSORS(holder, name, offset) DEF_PRIMITIVE_ACCESSORS(holder, name, offset, uint8_t)
...
#define ACCESSORS_CHECKED(holder, name, type, offset, condition) ACCESSORS_CHECKED2(holder, name, type, offset, condition, condition)

#define ACCESSORS(holder, name, type, offset) ACCESSORS_CHECKED(holder, name, type, offset, true)
...
#define ACCESSORS_RELAXED_CHECKED(holder, name, type, offset, condition) ACCESSORS_RELAXED_CHECKED2(holder, name, type, offset, condition, condition)

#define ACCESSORS_RELAXED(holder, name, type, offset) ACCESSORS_RELAXED_CHECKED(holder, name, type, offset, true)
...
#define RELAXED_ACCESSORS_CHECKED2(holder, name, type, offset, get_condition, set_condition)                                                                   \
    ...
    {                                                                                                                                                          \
        UNPAREN(type)                                                                                                                                          \
        value = TaggedField<UNPAREN(type), offset>::Relaxed_Load(cage_base, *this);                                                                            \
        DCHECK(get_condition);                                                                                                                                 \
        return value;                                                                                                                                          \
    }                                                                                                                                                          \
    void holder::set_##name(UNPAREN(type) value, RelaxedStoreTag, WriteBarrierMode mode)                                                                       \
    {                                                                                                                                                          \
        DCHECK(set_condition);                                                                                                                                 \
...
#define RELAXED_ACCESSORS_CHECKED(holder, name, type, offset, condition) RELAXED_ACCESSORS_CHECKED2(holder, name, type, offset, condition, condition)

#define RELAXED_ACCESSORS(holder, name, type, offset) RELAXED_ACCESSORS_CHECKED(holder, name, type, offset, true)

#define RELEASE_ACQUIRE_GETTER_CHECKED(holder, name, type, offset, get_condition)                                                                              \
    DEF_ACQUIRE_GETTER(holder, name, UNPAREN(type))                                                                                                            \
    {                                                                                                                                                          \
        UNPAREN(type)                                                                                                                                          \
        value = TaggedField<UNPAREN(type), offset>::Acquire_Load(cage_base, *this);                                                                            \
        DCHECK(get_condition);                                                                                                                                 \
        return value;                                                                                                                                          \
    }

#define RELEASE_ACQUIRE_SETTER_CHECKED(holder, name, type, offset, set_condition)                                                                              \
    void holder::set_##name(UNPAREN(type) value, ReleaseStoreTag, WriteBarrierMode mode)                                                                       \
    {                                                                                                                                                          \
        DCHECK(set_condition);                                                                                                                                 \
...
#define RELEASE_ACQUIRE_ACCESSORS(holder, name, type, offset) RELEASE_ACQUIRE_ACCESSORS_CHECKED(holder, name, type, offset, true)
...
#define SMI_ACCESSORS(holder, name, offset) SMI_ACCESSORS_CHECKED(holder, name, offset, true)
...
#define RELAXED_BOOL_ACCESSORS(holder, field, name, offset)                                                                                                    \
    ...
    {                                                                                                                                                          \
        set_##field(BooleanBit::set(field(kRelaxedLoad), offset, value), kRelaxedStore);                                                                       \
    }
...
#define DECL_EXTERNAL_POINTER_ACCESSORS_MAYBE_READ_ONLY_HOST(name, type)                                                                                       \
    ...
    inline void init_##name(i::IsolateForSandbox isolate, const type initial_value);                                                                           \
...
#define EXTERNAL_POINTER_ACCESSORS_MAYBE_READ_ONLY_HOST(holder, name, type, offset, tag)                                                                       \
    ...
    {                                                                                                                                                          \
        ...
        struct C2440 { };                                                                                                                                      \
        Address result = HeapObject::ReadExternalPointerField<tag>(offset, isolate);                                                                           \
        return reinterpret_cast<type>(reinterpret_cast<C2440*>(result));                                                                                       \
    }                                                                                                                                                          \
    void holder::init_##name(i::IsolateForSandbox isolate, const type initial_value)                                                                           \
    {                                                                                                                                                          \
        /* This is a workaround for MSVC error C2440 not allowing  */                                                                                          \
        /* reinterpret casts to the same type. */                                                                                                              \
        struct C2440 { };                                                                                                                                      \
        Address the_value = reinterpret_cast<Address>(reinterpret_cast<const C2440*>(initial_value));                                                          \
    ...
    {                                                                                                                                                          \
        ...
        /* reinterpret casts to the same type. */                                                                                                              \
        struct C2440 { };                                                                                                                                      \
        Address the_value = reinterpret_cast<Address>(reinterpret_cast<const C2440*>(value));                                                                  \
...
#define DECL_TRUSTED_POINTER_SETTERS(name, type)                                                                                                               \
    ...
    /* If desired, we could create separate {Read|Write}TrustedPointer */                                                                                      \
    /* routines for relaxed- and release-acquire semantics in the future. */                                                                                   \
    inline void set_##name(Tagged<type> value, WriteBarrierMode mode = UPDATE_WRITE_BARRIER);                                                                  \
    inline void set_##name(Tagged<type> value, ReleaseStoreTag, WriteBarrierMode mode = UPDATE_WRITE_BARRIER);                                                 \
...
#define TRUSTED_POINTER_ACCESSORS(holder, name, type, offset, tag)                                                                                             \
    ...
    void holder::set_##name(Tagged<type> value, ReleaseStoreTag, WriteBarrierMode mode)                                                                        \
    {                                                                                                                                                          \
        WriteTrustedPointerField<tag>(offset, value);                                                                                                          \
        CONDITIONAL_TRUSTED_POINTER_WRITE_BARRIER(*this, offset, tag, value, mode);                                                                            \
    }                                                                                                                                                          \
...
#define DECL_CODE_POINTER_ACCESSORS(name) DECL_TRUSTED_POINTER_ACCESSORS(name, Code)
#define CODE_POINTER_ACCESSORS(holder, name, offset) TRUSTED_POINTER_ACCESSORS(holder, name, Code, offset, kCodeIndirectPointerTag)
...
#define PROTECTED_POINTER_ACCESSORS(holder, name, type, offset)                                                                                                \
    ...
    void holder::clear_##name()                                                                                                                                \
    {                                                                                                                                                          \
        return ClearProtectedPointerField(offset);                                                                                                             \
    }
...
#define RELEASE_ACQUIRE_PROTECTED_POINTER_ACCESSORS(holder, name, type, offset)                                                                                \
    ...
    void holder::set_##name(Tagged<type> value, ReleaseStoreTag tag, WriteBarrierMode mode)                                                                    \
    {                                                                                                                                                          \
...
#define BIT_FIELD_ACCESSORS(holder, field, name, BitField) BIT_FIELD_ACCESSORS2(holder, field, field, name, BitField)
...
#define SEQ_CST_READ_FIELD(p, offset) TaggedField<Object>::SeqCst_Load(p, offset)

#define ACQUIRE_READ_FIELD(p, offset) TaggedField<Object>::Acquire_Load(p, offset)

#define RELAXED_READ_FIELD(p, offset) TaggedField<Object>::Relaxed_Load(p, offset)

#define RELAXED_READ_WEAK_FIELD(p, offset) TaggedField<MaybeObject>::Relaxed_Load(p, offset)

#define WRITE_FIELD(p, offset, value) TaggedField<Object>::store(p, offset, value)

#define SEQ_CST_WRITE_FIELD(p, offset, value) TaggedField<Object>::SeqCst_Store(p, offset, value)

#define RELEASE_WRITE_FIELD(p, offset, value) TaggedField<Object>::Release_Store(p, offset, value)

#define RELAXED_WRITE_FIELD(p, offset, value) TaggedField<Object>::Relaxed_Store(p, offset, value)

#define RELAXED_WRITE_WEAK_FIELD(p, offset, value) TaggedField<MaybeObject>::Relaxed_Store(p, offset, value)

#define SEQ_CST_SWAP_FIELD(p, offset, value) TaggedField<Object>::SeqCst_Swap(p, offset, value)

#define SEQ_CST_COMPARE_AND_SWAP_FIELD(p, offset, expected, value) TaggedField<Object>::SeqCst_CompareAndSwap(p, offset, expected, value)
...
#define WRITE_BARRIER(object, offset, value)                                                                                                                   \
    do {                                                                                                                                                       \
        ...
        WriteBarrier::ForValue(object, Tagged(object)->RawMaybeWeakField(offset), value, UPDATE_WRITE_BARRIER);                                                \
    } while (false)
#endif
...
#define EXTERNAL_POINTER_WRITE_BARRIER(object, offset, tag)                                                                                                    \
    do {                                                                                                                                                       \
        DCHECK(HeapLayout::IsOwnedByAnyHeap(object));                                                                                                          \
        WriteBarrier::ForExternalPointer(object, Tagged(object)->RawExternalPointerField(offset, tag), UPDATE_WRITE_BARRIER);                                  \
    } while (false)
#endif
...
#define INDIRECT_POINTER_WRITE_BARRIER(object, offset, tag, value)                                                                                             \
    do {                                                                                                                                                       \
        DCHECK(HeapLayout::IsOwnedByAnyHeap(object));                                                                                                          \
        WriteBarrier::ForIndirectPointer(object, Tagged(object)->RawIndirectPointerField(offset, tag), value, UPDATE_WRITE_BARRIER);                           \
    } while (false)
#endif
...
#define CONDITIONAL_WRITE_BARRIER(object, offset, value, mode) WRITE_BARRIER(object, offset, value)
#else
#define CONDITIONAL_WRITE_BARRIER(object, offset, value, mode)                                                                                                 \
    do {                                                                                                                                                       \
        DCHECK(HeapLayout::IsOwnedByAnyHeap(object));                                                                                                          \
        /* For write barriers, it doesn't matter if the slot is strong or weak, */                                                                             \
        /* so use the most generic slot (a maybe weak one). */                                                                                                 \
        WriteBarrier::ForValue(object, (object)->RawMaybeWeakField(offset), value, mode);                                                                      \
    } while (false)
#endif
...
#define CONDITIONAL_EXTERNAL_POINTER_WRITE_BARRIER(object, offset, tag, mode)                                                                                  \
    do {                                                                                                                                                       \
        DCHECK(HeapLayout::IsOwnedByAnyHeap(object));                                                                                                          \
        WriteBarrier::ForExternalPointer(object, Tagged(object)->RawExternalPointerField(offset, tag), mode);                                                  \
    } while (false)
#endif
#ifdef V8_DISABLE_WRITE_BARRIERS
...
#define CONDITIONAL_INDIRECT_POINTER_WRITE_BARRIER(object, offset, tag, value, mode)                                                                           \
    do {                                                                                                                                                       \
        DCHECK(HeapLayout::IsOwnedByAnyHeap(object));                                                                                                          \
        WriteBarrier::ForIndirectPointer(object, (object).RawIndirectPointerField(offset, tag), value, mode);                                                  \
    } while (false)
#endif

#ifdef V8_ENABLE_SANDBOX
#define CONDITIONAL_TRUSTED_POINTER_WRITE_BARRIER(object, offset, tag, value, mode) CONDITIONAL_INDIRECT_POINTER_WRITE_BARRIER(object, offset, tag, value, mode)
#else
#define CONDITIONAL_TRUSTED_POINTER_WRITE_BARRIER(object, offset, tag, value, mode) CONDITIONAL_WRITE_BARRIER(*this, offset, value, mode);
#endif // V8_ENABLE_SANDBOX
#define CONDITIONAL_CODE_POINTER_WRITE_BARRIER(object, offset, value, mode)                                                                                    \
    CONDITIONAL_TRUSTED_POINTER_WRITE_BARRIER(object, offset, kCodeIndirectPointerTag, value, mode)

#define CONDITIONAL_PROTECTED_POINTER_WRITE_BARRIER(object, offset, value, mode)                                                                               \
    do {                                                                                                                                                       \
        DCHECK(HeapLayout::IsOwnedByAnyHeap(object));                                                                                                          \
        WriteBarrier::ForProtectedPointer(object, (object).RawProtectedPointerField(offset), value, mode);                                                     \
    } while (false)
...
#define ACQUIRE_READ_INT8_FIELD(p, offset) static_cast<int8_t>(base::Acquire_Load(reinterpret_cast<const base::Atomic8*>(FIELD_ADDR(p, offset))))

#define ACQUIRE_READ_INT32_FIELD(p, offset) static_cast<int32_t>(base::Acquire_Load(reinterpret_cast<const base::Atomic32*>(FIELD_ADDR(p, offset))))

#define RELAXED_WRITE_INT8_FIELD(p, offset, value)                                                                                                             \
    base::Relaxed_Store(reinterpret_cast<base::Atomic8*>(FIELD_ADDR(p, offset)), static_cast<base::Atomic8>(value));
#define RELAXED_READ_INT8_FIELD(p, offset) static_cast<int8_t>(base::Relaxed_Load(reinterpret_cast<const base::Atomic8*>(FIELD_ADDR(p, offset))))

#define RELAXED_WRITE_UINT8_FIELD(p, offset, value)                                                                                                            \
    base::Relaxed_Store(reinterpret_cast<base::Atomic8*>(FIELD_ADDR(p, offset)), static_cast<base::Atomic8>(value));
#define RELAXED_READ_UINT8_FIELD(p, offset) static_cast<uint8_t>(base::Relaxed_Load(reinterpret_cast<const base::Atomic8*>(FIELD_ADDR(p, offset))))

#define RELAXED_READ_UINT16_FIELD(p, offset) static_cast<uint16_t>(base::Relaxed_Load(reinterpret_cast<const base::Atomic16*>(FIELD_ADDR(p, offset))))

#define RELAXED_WRITE_UINT16_FIELD(p, offset, value)                                                                                                           \
    base::Relaxed_Store(reinterpret_cast<base::Atomic16*>(FIELD_ADDR(p, offset)), static_cast<base::Atomic16>(value));

#define RELAXED_READ_INT16_FIELD(p, offset) static_cast<int16_t>(base::Relaxed_Load(reinterpret_cast<const base::Atomic16*>(FIELD_ADDR(p, offset))))

#define RELAXED_WRITE_INT16_FIELD(p, offset, value)                                                                                                            \
    base::Relaxed_Store(reinterpret_cast<base::Atomic16*>(FIELD_ADDR(p, offset)), static_cast<base::Atomic16>(value));

#define RELAXED_READ_UINT32_FIELD(p, offset) static_cast<uint32_t>(base::Relaxed_Load(reinterpret_cast<const base::Atomic32*>(FIELD_ADDR(p, offset))))

#define ACQUIRE_READ_UINT32_FIELD(p, offset) static_cast<uint32_t>(base::Acquire_Load(reinterpret_cast<const base::Atomic32*>(FIELD_ADDR(p, offset))))

#define RELAXED_WRITE_UINT32_FIELD(p, offset, value)                                                                                                           \
    base::Relaxed_Store(reinterpret_cast<base::Atomic32*>(FIELD_ADDR(p, offset)), static_cast<base::Atomic32>(value));

#define RELEASE_WRITE_INT8_FIELD(p, offset, value)                                                                                                             \
    base::Release_Store(reinterpret_cast<base::Atomic8*>(FIELD_ADDR(p, offset)), static_cast<base::Atomic8>(value));

#define RELEASE_WRITE_UINT32_FIELD(p, offset, value)                                                                                                           \
    base::Release_Store(reinterpret_cast<base::Atomic32*>(FIELD_ADDR(p, offset)), static_cast<base::Atomic32>(value));

#define RELAXED_READ_INT32_FIELD(p, offset) static_cast<int32_t>(base::Relaxed_Load(reinterpret_cast<const base::Atomic32*>(FIELD_ADDR(p, offset))))

#if defined(V8_HOST_ARCH_64_BIT)
#define RELAXED_READ_INT64_FIELD(p, offset) static_cast<int64_t>(base::Relaxed_Load(reinterpret_cast<const base::Atomic64*>(FIELD_ADDR(p, offset))))
#endif
...
static_assert(sizeof(int) == sizeof(int32_t), "sizeof int must match sizeof int32_t");

#define RELAXED_READ_INT_FIELD(p, offset) RELAXED_READ_INT32_FIELD(p, offset)

#define RELAXED_WRITE_INT_FIELD(p, offset, value) RELAXED_WRITE_INT32_FIELD(p, offset, value)

static_assert(sizeof(unsigned) == sizeof(uint32_t), "sizeof unsigned must match sizeof uint32_t");

#define RELAXED_READ_UINT_FIELD(p, offset) RELAXED_READ_UINT32_FIELD(p, offset)

#define RELAXED_WRITE_UINT_FIELD(p, offset, value) RELAXED_WRITE_UINT32_FIELD(p, offset, value)

#define RELAXED_READ_BYTE_FIELD(p, offset) static_cast<uint8_t>(base::Relaxed_Load(reinterpret_cast<const base::Atomic8*>(FIELD_ADDR(p, offset))))

#define ACQUIRE_READ_BYTE_FIELD(p, offset) static_cast<uint8_t>(base::Acquire_Load(reinterpret_cast<const base::Atomic8*>(FIELD_ADDR(p, offset))))
...
#define EXPORT_DECL_VERIFIER(Name) V8_EXPORT_PRIVATE void Name##Verify(Isolate* isolate);
#define DECL_STATIC_VERIFIER(Name) static void Name##Verify(Tagged<Name> obj, Isolate* isolate);
#define EXPORT_DECL_STATIC_VERIFIER(Name) V8_EXPORT_PRIVATE static void Name##Verify(Tagged<Name> obj, Isolate* isolate);
#else
...
#define TQ_CPP_OBJECT_DEFINITION_ASSERTS(_class, parent) template class TorqueGenerated##_class##Asserts<_class, parent>;
