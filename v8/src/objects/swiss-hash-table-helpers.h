...
// (unsuccesful) probe.
template <size_t GroupSize> class ProbeSequence {
public:
...
public:
    ...
    {
        return (sizeof(T) * CHAR_BIT - base::bits::CountLeadingZeros(mask_) - 1) >> Shift;
    }
...
static_assert(kEmpty & kDeleted & kSentinel & kNotFullMask, "Special markers need to have the MSB to make checking for them efficient");
...
struct GroupSse2Polyfill {
    ...
    explicit GroupSse2Polyfill(const ctrl_t* pos)
    {
...
struct GroupPortableImpl {
    ...
    explicit GroupPortableImpl(const ctrl_t* pos)
        : ctrl(base::ReadLittleEndianValue<uint64_t>(reinterpret_cast<uintptr_t>(const_cast<ctrl_t*>(pos))))
    {
    }
