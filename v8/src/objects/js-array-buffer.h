...
class JSArrayBuffer : public TorqueGeneratedJSArrayBuffer<JSArrayBuffer, JSAPIObjectWithEmbedderSlots> {
public:
...
// restriction for the length.
    ...
    V8_EXPORT_PRIVATE V8_WARN_UNUSED_RESULT static Maybe<bool> Detach(
        DirectHandle<JSArrayBuffer> buffer, bool force_for_wasm_memory = false, Handle<Object> key = Handle<Object>());
    ...
    static constexpr int kSizeWithEmbedderFields = kHeaderSize + v8::ArrayBuffer::kEmbedderFieldCount * kEmbedderDataSlotSize;
    static constexpr bool kContainsEmbedderFields = v8::ArrayBuffer::kEmbedderFieldCount > 0;
...
public:
    ...
    struct AccountingState {
        ...
        {
            DCHECK_LE(value >> kAccountingLengthBitOffset, std::numeric_limits<size_t>::max());
            return static_cast<size_t>(value >> kAccountingLengthBitOffset);
        }
        Age age() const
        {
            return static_cast<Age>(value & kAgeMask);
        }
    ...
    BackingStore* backing_store_raw()
    {
    ...
    size_t accounting_length() const
    {
        return AccountingState { accounting_state_.load(std::memory_order_relaxed) }.accounting_length();
    }
    void set_accounting_state(size_t accounting_length, Age age)
    {
        accounting_state_.store(
            (static_cast<uint64_t>(accounting_length) << kAccountingLengthBitOffset) | static_cast<uint8_t>(age), std::memory_order_relaxed);
    }
    ...
    {
        return { accounting_state_.fetch_add(delta << kAccountingLengthBitOffset, std::memory_order_relaxed) };
    }
    ...
    Age age() const
    {
        return AccountingState { accounting_state_.load(std::memory_order_relaxed) }.age();
    }
...
class JSArrayBufferView : public TorqueGeneratedJSArrayBufferView<JSArrayBufferView, JSAPIObjectWithEmbedderSlots> {
public:
    class BodyDescriptor;
...
public:
    ...
    static inline void ForFixedTypedArray(ExternalArrayType array_type, size_t* element_size, ElementsKind* element_kind);
    ...
    static inline Address ExternalPointerCompensationForOnHeapArray(PtrComprCageBase cage_base);
    ...
    inline void RemoveExternalPointerCompensationForSerialization(Isolate* isolate);
    // Adds external pointer compensation to the external pointer value.
    inline void AddExternalPointerCompensationForDeserialization(Isolate* isolate);

    static inline MaybeHandle<JSTypedArray> Validate(Isolate* isolate, Handle<Object> receiver, const char* method_name);
    ...
    static constexpr int kSizeWithEmbedderFields = kHeaderSize + v8::ArrayBufferView::kEmbedderFieldCount * kEmbedderDataSlotSize;
    static constexpr bool kContainsEmbedderFields = v8::ArrayBufferView::kEmbedderFieldCount > 0;
...
class JSDataViewOrRabGsabDataView : public TorqueGeneratedJSDataViewOrRabGsabDataView<JSDataViewOrRabGsabDataView, JSArrayBufferView> {
public:
...
class JSDataView : public TorqueGeneratedJSDataView<JSDataView, JSDataViewOrRabGsabDataView> {
public:
    // Dispatched behavior.
...
class JSRabGsabDataView : public TorqueGeneratedJSRabGsabDataView<JSRabGsabDataView, JSDataViewOrRabGsabDataView> {
public:
    // Dispatched behavior.
