...
ACCESSORS(AllocationSite, transition_info_or_boilerplate, Tagged<Object>, kTransitionInfoOrBoilerplateOffset)
RELEASE_ACQUIRE_ACCESSORS(AllocationSite, transition_info_or_boilerplate, Tagged<Object>, kTransitionInfoOrBoilerplateOffset)
...
INT32_ACCESSORS(AllocationSite, pretenure_create_count, kPretenureCreateCountOffset)
ACCESSORS(AllocationSite, dependent_code, Tagged<DependentCode>, kDependentCodeOffset)
ACCESSORS_CHECKED(AllocationSite, weak_next, Tagged<Object>, kWeakNextOffset, HasWeakNext())
ACCESSORS(AllocationMemento, allocation_site, Tagged<Object>, kAllocationSiteOffset)
...
void AllocationSite::set_boilerplate(Tagged<JSObject> value, ReleaseStoreTag tag, WriteBarrierMode mode)
{
...
{
    DCHECK(!PointsToLiteral());
    set_transition_info_or_boilerplate(Smi::FromInt(value), kReleaseStore, SKIP_WRITE_BARRIER);
}
...
void AllocationSite::Initialize()
{
    ...
    set_dependent_code(DependentCode::empty_dependent_code(GetReadOnlyRoots()), SKIP_WRITE_BARRIER);
}
...
void AllocationSite::set_pretenure_decision(PretenureDecision decision)
{
    ...
    set_pretenure_data(PretenureDecisionBits::update(value, decision), kRelaxedStore);
}
...
{
    int32_t value = pretenure_data(kRelaxedLoad);
    set_pretenure_data(DeoptDependentCodeBit::update(value, deopt), kRelaxedStore);
}
...
inline void AllocationSite::set_memento_found_count(int count)
{
    ...
    set_pretenure_data(MementoFoundCountBits::update(value, count), kRelaxedStore);
}
...
{
    return IsAllocationSite(allocation_site()) && !Cast<AllocationSite>(allocation_site())->IsZombie();
}
...
template <AllocationSiteUpdateMode update_or_check> bool AllocationSite::DigestTransitionFeedback(DirectHandle<AllocationSite> site, ElementsKind to_kind)
{
    ...
    if (site->PointsToLiteral() && IsJSArray(site->boilerplate())) {
        ...
        if (IsMoreGeneralElementsKindTransition(kind, to_kind)) {
            ...
            if (length <= kMaximumArrayBytesToPretransition) {
                ...
                DependentCode::DeoptimizeDependencyGroups(isolate, *site, DependentCode::kAllocationSiteTransitionChangedGroup);
                result = true;
            }
        }
    } else {
