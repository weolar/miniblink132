...
public:
    ...
    static constexpr int kInitialBlockSizeHighestBit = kBitsPerInt - base::bits::CountLeadingZeros32(kInitialBlockSize) - 1;
    ...
    template <typename T> EXPORT_TEMPLATE_DECLARE(V8_EXPORT_PRIVATE) int AddExternalResourceAndHash(Tagged<String> string, T* resource, uint32_t raw_hash);
    ...
    template <typename T> EXPORT_TEMPLATE_DECLARE(V8_EXPORT_PRIVATE) bool TryUpdateExternalResource(int index, T* resource);
    ...
    template <typename Func> V8_INLINE void IterateElements(Func&& callback);
