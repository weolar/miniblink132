...
LookupIterator::LookupIterator(Isolate* isolate, Handle<JSAny> receiver, Handle<Name> name, Configuration configuration)
    : LookupIterator(isolate, receiver, name, kInvalidIndex, receiver, configuration)
{
}

LookupIterator::LookupIterator(Isolate* isolate, Handle<JSAny> receiver, Handle<Name> name, Handle<JSAny> lookup_start_object, Configuration configuration)
    : LookupIterator(isolate, receiver, name, kInvalidIndex, Cast<JSAny>(lookup_start_object), configuration)
{
}

LookupIterator::LookupIterator(Isolate* isolate, Handle<JSAny> receiver, size_t index, Configuration configuration)
    : LookupIterator(isolate, receiver, Handle<Name>(), index, receiver, configuration)
{
    DCHECK_NE(index, kInvalidIndex);
}

LookupIterator::LookupIterator(Isolate* isolate, Handle<JSAny> receiver, size_t index, Handle<JSAny> lookup_start_object, Configuration configuration)
    : LookupIterator(isolate, receiver, Handle<Name>(), index, Cast<JSAny>(lookup_start_object), configuration)
{
    DCHECK_NE(index, kInvalidIndex);
}

LookupIterator::LookupIterator(Isolate* isolate, Handle<JSAny> receiver, const PropertyKey& key, Configuration configuration)
    : LookupIterator(isolate, receiver, key.name(), key.index(), receiver, configuration)
{
}

LookupIterator::LookupIterator(Isolate* isolate, Handle<JSAny> receiver, const PropertyKey& key, Handle<JSAny> lookup_start_object, Configuration configuration)
    : LookupIterator(isolate, receiver, key.name(), key.index(), Cast<JSAny>(lookup_start_object), configuration)
{
}
    ...
    , index_(index)
{
    if (IsElement()) {
        ...
        if (index_ > JSObject::kMaxElementIndex && !IsJSTypedArray(*lookup_start_object, isolate_)
...
LookupIterator::LookupIterator(Isolate* isolate, Configuration configuration, Handle<JSAny> receiver, Handle<Symbol> name)
    ...
    , index_(kInvalidIndex)
{
    ...
    DCHECK(*name_ == *isolate->factory()->error_stack_symbol() || *name_ == *isolate->factory()->error_message_symbol());
...
PropertyKey::PropertyKey(Isolate* isolate, double index)
{
    ...
    if (index <= JSObject::kMaxElementIndex) {
        static_assert(JSObject::kMaxElementIndex <= std::numeric_limits<size_t>::max());
        index_ = static_cast<size_t>(index);
    } else {
        index_ = LookupIterator::kInvalidIndex;
        name_ = isolate->factory()->InternalizeString(isolate->factory()->HeapNumberToString(isolate->factory()->NewHeapNumber(index), index));
    }
#else
    index_ = static_cast<size_t>(index);
#endif
...
PropertyKey::PropertyKey(Isolate* isolate, Handle<Object> valid_key)
{
    ...
    if (Object::ToIntegerIndex(*valid_key, &index_))
        return;
...
{
    return index_ <= JSObject::kMaxElementIndex || (index_ != kInvalidIndex && object->map()->has_any_typed_array_or_wasm_array_elements());
}
...
template <class T> Handle<T> LookupIterator::GetHolder() const
{
    DCHECK(IsFound());
...
bool LookupIterator::ExtendingNonExtensible(Handle<JSReceiver> receiver)
{
    ...
    return !receiver->map(isolate_)->is_extensible() && (IsElement() || (!name_->IsPrivate() || IsAlwaysSharedSpaceJSObject(*receiver)));
}

bool LookupIterator::IsCacheableTransition()
{
    DCHECK_EQ(TRANSITION, state_);
    return IsPropertyCell(*transition_, isolate_) || (transition_map()->is_dictionary_map() && !GetStoreTarget<JSReceiver>()->HasFastProperties(isolate_))
        || IsMap(transition_map()->GetBackPointer(isolate_), isolate_);
}

// static
void LookupIterator::UpdateProtector(Isolate* isolate, Handle<JSAny> receiver, DirectHandle<Name> name)
{
...
// static
LookupIterator::Configuration LookupIterator::ComputeConfiguration(Isolate* isolate, Configuration configuration, Handle<Name> name)
{
    return (!name.is_null() && name->IsPrivate()) ? OWN_SKIP_INTERCEPTOR : configuration;
}

// static
MaybeHandle<JSReceiver> LookupIterator::GetRoot(Isolate* isolate, Handle<JSAny> lookup_start_object, size_t index, Configuration configuration)
{
    if (IsJSReceiver(*lookup_start_object, isolate)) {
        return Cast<JSReceiver>(lookup_start_object);
    }
    return GetRootForNonJSReceiver(isolate, Cast<JSPrimitive>(lookup_start_object), index, configuration);
}

template <class T> Handle<T> LookupIterator::GetStoreTarget() const
{
    DCHECK(IsJSReceiver(*receiver_, isolate_));
    if (IsJSGlobalProxy(*receiver_, isolate_)) {
        Tagged<HeapObject> prototype = Cast<JSGlobalProxy>(*receiver_)->map(isolate_)->prototype(isolate_);
...
template <bool is_element> Tagged<InterceptorInfo> LookupIterator::GetInterceptor(Tagged<JSObject> holder) const
{
...
inline Handle<InterceptorInfo> LookupIterator::GetInterceptor() const
{
    ...
    Tagged<InterceptorInfo> result = IsElement(holder) ? GetInterceptor<true>(holder) : GetInterceptor<false>(holder);
