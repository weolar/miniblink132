...
public:
    ...
    static inline Tagged<Object> RawToTagged(PtrComprCageBase cage_base, Tagged_t raw);
...
// Its address() is the address of the slot.
// The slot's contents can be read and written using operator* and store().
class CompressedMaybeObjectSlot : public SlotBase<CompressedMaybeObjectSlot, Tagged_t> {
public:
    using TCompressionScheme = V8HeapCompressionScheme;
    ...
    template <typename T>
    explicit CompressedMaybeObjectSlot(SlotBase<T, TData, kSlotDataAlignment> slot)
        : SlotBase(slot.address())
    {
    }
...
class CompressedHeapObjectSlot : public SlotBase<CompressedHeapObjectSlot, Tagged_t> {
public:
    using TCompressionScheme = V8HeapCompressionScheme;
...
template <typename CompressionScheme> class OffHeapCompressedObjectSlot : public SlotBase<OffHeapCompressedObjectSlot<CompressionScheme>, Tagged_t> {
public:
    using TSlotBase = SlotBase<OffHeapCompressedObjectSlot<CompressionScheme>, Tagged_t>;
    ...
    OffHeapCompressedObjectSlot()
        : TSlotBase(kNullAddress)
    {
    }
    explicit OffHeapCompressedObjectSlot(Address ptr)
        : TSlotBase(ptr)
    {
    }
    ...
    inline void Relaxed_Store(Tagged<Object> value) const;
    inline void Release_Store(Tagged<Object> value) const;
    inline void Release_CompareAndSwap(Tagged<Object> old, Tagged<Object> target) const;
};
