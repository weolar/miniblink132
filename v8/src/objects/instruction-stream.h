...
public:
    ...
    static constexpr bool kBodyIsContiguous = kOnHeapBodyIsContiguous && kOffHeapBodyIsContiguous;
    ...
    inline bool TryGetCodeUnchecked(Tagged<Code>* code_out, AcquireLoadTag tag) const;
    ...
    {
        return RoundUp<kCodeAlignment>(kHeaderSize + body_size) - kHeaderSize - body_size;
    }
    ...
    static inline Tagged<InstructionStream> FromEntryAddress(Address location_of_address);
...
#undef ISTREAM_FIELDS
    ...
    static constexpr int kCodeAlignmentMinusCodeHeader = kCodeAlignment - kHeaderSize;
...
private:
    ...
    void RelocateFromDescWriteBarriers(
        Heap* heap, const CodeDesc& desc, Address constant_pool, WriteBarrierPromise& promise, const DisallowGarbageCollection& no_gc);
