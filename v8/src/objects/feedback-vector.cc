...
static bool IsPropertyNameFeedback(Tagged<MaybeObject> feedback)
{
    ...
    if (!IsSymbol(heap_object))
        return false;
    Tagged<Symbol> symbol = Cast<Symbol>(heap_object);
    ReadOnlyRoots roots = symbol->GetReadOnlyRoots();
    return symbol != roots.uninitialized_symbol() && symbol != roots.mega_dom_symbol() && symbol != roots.megamorphic_symbol();
}
...
void FeedbackMetadata::SetCreateClosureParameterCount(int index, uint16_t parameter_count)
{
    DCHECK_LT(index, create_closure_slot_count());
    int offset = kHeaderSize + word_count() * kInt32Size + index * kUInt16Size;
...
// static
template <typename IsolateT> Handle<FeedbackMetadata> FeedbackMetadata::New(IsolateT* isolate, const FeedbackVectorSpec* spec)
{
    ...
    Handle<FeedbackMetadata> metadata = factory->NewFeedbackMetadata(slot_count, create_closure_slot_count);
...
template Handle<FeedbackMetadata> FeedbackMetadata::New(Isolate* isolate, const FeedbackVectorSpec* spec);
template Handle<FeedbackMetadata> FeedbackMetadata::New(LocalIsolate* isolate, const FeedbackVectorSpec* spec);

bool FeedbackMetadata::SpecDiffersFrom(const FeedbackVectorSpec* other_spec) const
{
...
FeedbackSlotKind FeedbackVector::GetKind(FeedbackSlot slot, AcquireLoadTag tag) const
{
    DCHECK(!is_empty());
...
// static
Handle<ClosureFeedbackCellArray> ClosureFeedbackCellArray::New(Isolate* isolate, DirectHandle<SharedFunctionInfo> shared, AllocationType allocation)
{
    ...
    for (int i = 0; i < length; i++) {
        ...
        uint16_t parameter_count = shared->feedback_metadata()->GetCreateClosureParameterCount(i);
...
// static
Handle<FeedbackVector> FeedbackVector::New(Isolate* isolate, DirectHandle<SharedFunctionInfo> shared,
    DirectHandle<ClosureFeedbackCellArray> closure_feedback_cell_array, DirectHandle<FeedbackCell> parent_feedback_cell, IsCompiledScope* is_compiled_scope)
{
    DCHECK(is_compiled_scope->is_compiled());
    Factory* factory = isolate->factory();

    DirectHandle<FeedbackMetadata> feedback_metadata(shared->feedback_metadata(), isolate);
    const int slot_count = feedback_metadata->slot_count();

    Handle<FeedbackVector> vector = factory->NewFeedbackVector(shared, closure_feedback_cell_array, parent_feedback_cell);
    ...
    DCHECK_EQ(ReadOnlyRoots(isolate).uninitialized_symbol(), *uninitialized_sentinel);
    for (int i = 0; i < slot_count;) {
        FeedbackSlot slot(i);
...
// static
Handle<FeedbackVector> FeedbackVector::NewForTesting(Isolate* isolate, const FeedbackVectorSpec* spec)
{
    DirectHandle<FeedbackMetadata> metadata = FeedbackMetadata::New(isolate, spec);
    DirectHandle<SharedFunctionInfo> shared
        = isolate->factory()->NewSharedFunctionInfoForBuiltin(isolate->factory()->empty_string(), Builtin::kIllegal, 0, kDontAdapt);
    // Set the raw feedback metadata to circumvent checks that we are not
    // overwriting existing metadata.
    shared->set_raw_outer_scope_info_or_feedback_metadata(*metadata);
    DirectHandle<ClosureFeedbackCellArray> closure_feedback_cell_array = ClosureFeedbackCellArray::New(isolate, shared);
    DirectHandle<FeedbackCell> parent_cell = isolate->factory()->NewNoClosuresCell();

    IsCompiledScope is_compiled_scope(shared->is_compiled_scope(isolate));
    return FeedbackVector::New(isolate, shared, closure_feedback_cell_array, parent_cell, &is_compiled_scope);
}

// static
Handle<FeedbackVector> FeedbackVector::NewWithOneBinarySlotForTesting(Zone* zone, Isolate* isolate)
{
...
// static
Handle<FeedbackVector> FeedbackVector::NewWithOneCompareSlotForTesting(Zone* zone, Isolate* isolate)
{
    FeedbackVectorSpec one_slot(zone);
...
// static
void FeedbackVector::AddToVectorsForProfilingTools(Isolate* isolate, DirectHandle<FeedbackVector> vector)
{
    DCHECK(!isolate->is_best_effort_code_coverage());
    if (!vector->shared_function_info()->IsSubjectToDebugging())
        return;
    Handle<ArrayList> list = Cast<ArrayList>(isolate->factory()->feedback_vectors_for_profiling_tools());
...
void FeedbackVector::SetOptimizedCode(IsolateForSandbox isolate, Tagged<Code> code)
{
    ...
    } else if (!CodeKindCanTierUp(optimized_code(isolate)->kind()) || optimized_code(isolate)->kind() > code->kind()) {
        if (!v8_flags.stress_concurrent_inlining_attach_code && !optimized_code(isolate)->marked_for_deoptimization()) {
            return;
        }
...
void FeedbackVector::EvictOptimizedCodeMarkedForDeoptimization(Isolate* isolate, Tagged<SharedFunctionInfo> shared, const char* reason)
{
...
void FeedbackVector::SetOptimizedOsrCode(Isolate* isolate, FeedbackSlot slot, Tagged<Code> code)
{
    DCHECK(CodeKindIsOptimizedJSFunction(code->kind()));
...
{
    set_flags(TieringStateBits::encode(TieringState::kNone) | LogNextExecutionBit::encode(false) |
#ifndef V8_ENABLE_LEAPTIERING
        MaybeHasMaglevCodeBit::encode(false) | MaybeHasTurbofanCodeBit::encode(false) |
#endif // !V8_ENABLE_LEAPTIERING
        OsrTieringInProgressBit::encode(false) | MaybeHasMaglevOsrCodeBit::encode(false) | MaybeHasTurbofanOsrCodeBit::encode(false));
}
...
bool FeedbackVector::ClearSlots(Isolate* isolate, ClearBehavior behavior)
{
    if (!shared_function_info()->HasFeedbackMetadata())
        return false;
    Tagged<MaybeObject> uninitialized_sentinel = FeedbackVector::RawUninitializedSentinel(isolate);
...
// static
void FeedbackVector::TraceFeedbackChange(Isolate* isolate, Tagged<FeedbackVector> vector, FeedbackSlot slot, const char* reason)
{
    ...
    if (slot.IsInvalid()) {
        ...
        os << "[Feedback slot " << slot.ToInt() << "/" << slot_count << " (" << FeedbackMetadata::Kind2String(kind) << ")"
           << " in ";
    }
...
void NexusConfig::SetFeedbackPair(Tagged<FeedbackVector> vector, FeedbackSlot start_slot, Tagged<MaybeObject> feedback, WriteBarrierMode mode,
    Tagged<MaybeObject> feedback_extra, WriteBarrierMode mode_extra) const
{
    CHECK(can_write());
    CHECK_GT(vector->length(), start_slot.WithOffset(1).ToInt());
    base::SharedMutexGuard<base::kExclusive> shared_mutex_guard(isolate()->feedback_vector_access());
    vector->Set(start_slot, feedback, mode);
    vector->Set(start_slot.WithOffset(1), feedback_extra, mode_extra);
}

std::pair<Tagged<MaybeObject>, Tagged<MaybeObject>> NexusConfig::GetFeedbackPair(Tagged<FeedbackVector> vector, FeedbackSlot slot) const
{
    base::SharedMutexGuardIf<base::kShared> scope(isolate()->feedback_vector_access(), mode() == BackgroundThread);
...
FeedbackNexus::FeedbackNexus(Isolate* isolate, Handle<FeedbackVector> vector, FeedbackSlot slot)
...
FeedbackNexus::FeedbackNexus(Isolate* isolate, Tagged<FeedbackVector> vector, FeedbackSlot slot)
    : vector_(vector)
    , slot_(slot)
    , config_(NexusConfig::FromMainThread(isolate))
{
...
FeedbackNexus::FeedbackNexus(Handle<FeedbackVector> vector, FeedbackSlot slot, const NexusConfig& config)
    : vector_handle_(vector)
    , slot_(slot)
...
Handle<WeakFixedArray> FeedbackNexus::CreateArrayOfSize(int length)
{
    ...
    Handle<WeakFixedArray> array = config()->isolate()->factory()->NewWeakFixedArray(length);
    return array;
}
...
void FeedbackNexus::ConfigureUninitialized()
{
    ...
    case FeedbackSlotKind::kLoadGlobalNotInsideTypeof:
    case FeedbackSlotKind::kLoadGlobalInsideTypeof:
        SetFeedback(ClearedValue(isolate), SKIP_WRITE_BARRIER, UninitializedSentinel(), SKIP_WRITE_BARRIER);
        break;
    case FeedbackSlotKind::kCloneObject:
    case FeedbackSlotKind::kCall:
        SetFeedback(UninitializedSentinel(), SKIP_WRITE_BARRIER, Smi::zero(), SKIP_WRITE_BARRIER);
        break;
    case FeedbackSlotKind::kInstanceOf:
    ...
    case FeedbackSlotKind::kHasKeyed:
    case FeedbackSlotKind::kDefineKeyedOwnPropertyInLiteral:
        SetFeedback(UninitializedSentinel(), SKIP_WRITE_BARRIER, UninitializedSentinel(), SKIP_WRITE_BARRIER);
        break;
    case FeedbackSlotKind::kJumpLoop:
...
bool FeedbackNexus::ConfigureMegamorphic(IcCheckType property_type)
{
    DisallowGarbageCollection no_gc;
    Tagged<MaybeObject> sentinel = MegamorphicSentinel();
    Tagged<MaybeObject> maybe_extra = Smi::FromInt(static_cast<int>(property_type));
...
InlineCacheState FeedbackNexus::ic_state() const
{
    ...
    case FeedbackSlotKind::kHasKeyed: {
        ...
        if (feedback.GetHeapObjectIfStrong(&heap_object)) {
            ...
            if (IsName(heap_object)) {
                DCHECK(IsKeyedLoadICKind(kind()) || IsKeyedStoreICKind(kind()) || IsKeyedHasICKind(kind()) || IsDefineKeyedOwnICKind(kind()));
                Tagged<Object> extra_object = extra.GetHeapObjectAssumeStrong();
                Tagged<WeakFixedArray> extra_array = Cast<WeakFixedArray>(extra_object);
                return extra_array->length() > 2 ? InlineCacheState::POLYMORPHIC : InlineCacheState::MONOMORPHIC;
            }
        }
        // TODO(1393773): Remove once the issue is solved.
        Address vector_ptr = vector().ptr();
        config_.isolate()->PushParamsAndDie(reinterpret_cast<void*>(feedback.ptr()), reinterpret_cast<void*>(extra.ptr()), reinterpret_cast<void*>(vector_ptr),
            reinterpret_cast<void*>(static_cast<intptr_t>(slot_.ToInt())), reinterpret_cast<void*>(static_cast<intptr_t>(kind())),
            // Include part of the feedback vector containing the slot.
            reinterpret_cast<void*>(vector_ptr + FeedbackVector::OffsetOfElementAt(slot_.ToInt())));
        UNREACHABLE();
    }
    case FeedbackSlotKind::kCall: {
        Tagged<HeapObject> heap_object;
        if (feedback == MegamorphicSentinel()) {
            ...
            return InlineCacheState::MONOMORPHIC;
        } else if (feedback.GetHeapObjectIfStrong(&heap_object) && IsAllocationSite(heap_object)) {
            return InlineCacheState::MONOMORPHIC;
        }
    ...
    case FeedbackSlotKind::kTypeOf: {
        if (feedback == Smi::zero()) {
            ...
            return InlineCacheState::MEGAMORPHIC;
        } else if (base::bits::CountPopulation(static_cast<uint32_t>(feedback.ToSmi().value())) == 1) {
            return InlineCacheState::MONOMORPHIC;
        } else {
            return InlineCacheState::POLYMORPHIC;
        }
...
bool FeedbackNexus::ConfigureLexicalVarMode(int script_context_index, int context_slot_index, bool immutable)
{
    DCHECK(IsGlobalICKind(kind()));
    ...
    SetFeedback(Smi::From31BitPattern(config), SKIP_WRITE_BARRIER, UninitializedSentinel(), SKIP_WRITE_BARRIER);
    return true;
}
...
void FeedbackNexus::ConfigureHandlerMode(const MaybeObjectHandle& handler)
{
    ...
    SetFeedback(ClearedValue(config()->isolate()), UPDATE_WRITE_BARRIER, *handler, UPDATE_WRITE_BARRIER);
}

void FeedbackNexus::ConfigureCloneObject(Handle<Map> source_map, const MaybeObjectHandle& handler_handle)
{
    ...
    case InlineCacheState::MONOMORPHIC:
        if (feedback.is_null() || feedback.is_identical_to(source_map) || Cast<Map>(*feedback)->is_deprecated()) {
            SetFeedback(MakeWeak(*source_map), UPDATE_WRITE_BARRIER, GetHandler());
        } else {
            // Transition to POLYMORPHIC.
            DirectHandle<WeakFixedArray> array = CreateArrayOfSize(2 * kCloneObjectPolymorphicEntrySize);
            DisallowGarbageCollection no_gc;
    ...
    case InlineCacheState::POLYMORPHIC: {
        const int kMaxElements = v8_flags.max_valid_polymorphic_map_count * kCloneObjectPolymorphicEntrySize;
        ...
        for (; i < array->length(); i += kCloneObjectPolymorphicEntrySize) {
            ...
            if (feedback_map.IsCleared())
                break;
            Handle<Map> cached_map(Cast<Map>(feedback_map.GetHeapObject()), isolate);
        ...
        if (i >= array->length()) {
            ...
            DirectHandle<WeakFixedArray> new_array = CreateArrayOfSize(array->length() + kCloneObjectPolymorphicEntrySize);
...
void FeedbackNexus::SetSpeculationMode(SpeculationMode mode)
{
    ...
    SetFeedback(feedback, UPDATE_WRITE_BARRIER, Smi::FromInt(count), SKIP_WRITE_BARRIER);
}
...
void FeedbackNexus::ConfigureMonomorphic(Handle<Name> name, DirectHandle<Map> receiver_map, const MaybeObjectHandle& handler)
{
...
void FeedbackNexus::ConfigurePolymorphic(Handle<Name> name, std::vector<MapAndHandler> const& maps_and_handlers)
{
    ...
    if (name.is_null()) {
        SetFeedback(*array, UPDATE_WRITE_BARRIER, UninitializedSentinel(), SKIP_WRITE_BARRIER);
    } else {
        SetFeedback(*name, UPDATE_WRITE_BARRIER, *array);
    }
}
...
int FeedbackNexus::ExtractMapsAndHandlers(std::vector<MapAndHandler>* maps_and_handlers, TryUpdateHandler map_handler) const
{
...
MaybeObjectHandle FeedbackNexus::FindHandlerForMap(DirectHandle<Map> map) const
{
...
{
    if (IsKeyedStoreICKind(kind()) || IsKeyedLoadICKind(kind()) || IsKeyedHasICKind(kind()) || IsDefineKeyedOwnICKind(kind())) {
        Tagged<MaybeObject> feedback = GetFeedback();
...
KeyedAccessLoadMode FeedbackNexus::GetKeyedAccessLoadMode() const
{
    ...
    for (MapAndHandler map_and_handler : maps_and_handlers) {
        mode = GeneralizeKeyedAccessLoadMode(mode, LoadHandler::GetKeyedAccessLoadMode(*map_and_handler.second));
    }
    return mode;
}
...
{
    DCHECK(
        IsKeyedStoreICKind(kind()) || IsStoreInArrayLiteralICKind(kind()) || IsDefineKeyedOwnPropertyInLiteralKind(kind()) || IsDefineKeyedOwnICKind(kind()));
    ...
    for (const MapAndHandler& map_and_handler : maps_and_handlers) {
        ...
        if (IsStoreHandler(*maybe_code_handler.object())) {
            ...
            if (IsSmi(data_handler->smi_handler())) {
                ...
                mode = StoreHandler::GetKeyedAccessStoreMode(data_handler->smi_handler());
                if (!StoreModeIsInBounds(mode))
                    return mode;
                continue;
            } else {
        ...
        if (Builtins::IsBuiltinId(builtin_handler)) {
            if (!BuiltinHasKeyedAccessStoreMode(builtin_handler))
                continue;
...
IcCheckType FeedbackNexus::GetKeyType() const
{
    DCHECK(IsKeyedStoreICKind(kind()) || IsKeyedLoadICKind(kind()) || IsStoreInArrayLiteralICKind(kind()) || IsKeyedHasICKind(kind())
        || IsDefineKeyedOwnPropertyInLiteralKind(kind()) || IsDefineKeyedOwnICKind(kind()));
    auto pair = GetFeedbackPair();
    Tagged<MaybeObject> feedback = pair.first;
    if (feedback == MegamorphicSentinel()) {
        return static_cast<IcCheckType>(Smi::ToInt(Cast<Smi>(pair.second)));
    }
    Tagged<MaybeObject> maybe_name = IsDefineKeyedOwnPropertyInLiteralKind(kind()) || IsDefineKeyedOwnICKind(kind()) ? pair.second : feedback;
    return IsPropertyNameFeedback(maybe_name) ? IcCheckType::kProperty : IcCheckType::kElement;
}
...
FeedbackIterator::FeedbackIterator(const FeedbackNexus* nexus)
    ...
    , state_(kOther)
{
    DCHECK(IsLoadICKind(nexus->kind()) || IsSetNamedICKind(nexus->kind()) || IsKeyedLoadICKind(nexus->kind()) || IsKeyedStoreICKind(nexus->kind())
        || IsDefineNamedOwnICKind(nexus->kind()) || IsDefineKeyedOwnPropertyInLiteralKind(nexus->kind()) || IsStoreInArrayLiteralICKind(nexus->kind())
    ...
    if ((feedback.GetHeapObjectIfStrong(&heap_object) && IsWeakFixedArray(heap_object)) || is_named_feedback) {
        index_ = 0;
        state_ = kPolymorphic;
        heap_object = feedback.GetHeapObjectAssumeStrong();
        if (is_named_feedback) {
            polymorphic_feedback_ = nexus->config()->NewHandle(Cast<WeakFixedArray>(pair.second.GetHeapObjectAssumeStrong()));
        } else {
            polymorphic_feedback_ = nexus->config()->NewHandle(Cast<WeakFixedArray>(heap_object));
        }
        AdvancePolymorphic();
...
void FeedbackIterator::AdvancePolymorphic()
{
    ...
    while (index_ < length) {
        if (polymorphic_feedback_->get(index_).GetHeapObjectIfWeak(&heap_object)) {
            Tagged<MaybeObject> handler = polymorphic_feedback_->get(index_ + kHandlerOffset);
            map_ = Cast<Map>(heap_object);
            handler_ = handler;
            index_ += kEntrySize;
            return;
