...
class JSDurationFormat : public TorqueGeneratedJSDurationFormat<JSDurationFormat, JSObject> {
public:
    // Creates duration format object with properties derived from input
    // locales and options.
    V8_WARN_UNUSED_RESULT static MaybeHandle<JSDurationFormat> New(Isolate* isolate, DirectHandle<Map> map, Handle<Object> locales, Handle<Object> options);

    V8_WARN_UNUSED_RESULT static Handle<JSObject> ResolvedOptions(Isolate* isolate, DirectHandle<JSDurationFormat> format_holder);

    V8_WARN_UNUSED_RESULT static MaybeHandle<String> Format(Isolate* isolate, Handle<JSDurationFormat> df, Handle<Object> duration);

    V8_WARN_UNUSED_RESULT static MaybeHandle<JSArray> FormatToParts(Isolate* isolate, Handle<JSDurationFormat> df, Handle<Object> duration);
...
#define DECLARE_INLINE_DISPLAY_SETTER_GETTER(f) DECLARE_INLINE_SETTER_GETTER(Display, f##_display)

#define DECLARE_INLINE_FIELD_STYLE_SETTER_GETTER(f) DECLARE_INLINE_SETTER_GETTER(FieldStyle, f##_style)
