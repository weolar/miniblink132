...
ACCESSORS(Map, instance_descriptors, Tagged<DescriptorArray>, kInstanceDescriptorsOffset)
RELAXED_ACCESSORS(Map, instance_descriptors, Tagged<DescriptorArray>, kInstanceDescriptorsOffset)
RELEASE_ACQUIRE_ACCESSORS(Map, instance_descriptors, Tagged<DescriptorArray>, kInstanceDescriptorsOffset)
...
// layout descriptor.
ACCESSORS(Map, raw_transitions, (Tagged<UnionOf<Smi, MaybeWeak<Map>, TransitionArray>>), kTransitionsOrPrototypeInfoOffset)
RELEASE_ACQUIRE_ACCESSORS(Map, raw_transitions, (Tagged<UnionOf<Smi, MaybeWeak<Map>, TransitionArray>>), kTransitionsOrPrototypeInfoOffset)

ACCESSORS_CHECKED2(Map, prototype, Tagged<JSPrototype>, kPrototypeOffset, true,
    IsNull(value) || IsJSProxy(value) || IsWasmObject(value)
        || (IsJSObject(value) && (HeapLayout::InWritableSharedSpace(value) || value->map()->is_prototype_map())))

DEF_GETTER(Map, prototype_info, Tagged<UnionOf<Smi, PrototypeInfo>>)
{
    Tagged<UnionOf<Smi, PrototypeInfo>> value = TaggedField<UnionOf<Smi, PrototypeInfo>, kTransitionsOrPrototypeInfoOffset>::load(cage_base, *this);
    DCHECK(this->is_prototype_map());
    return value;
}
RELEASE_ACQUIRE_ACCESSORS(Map, prototype_info, (Tagged<UnionOf<Smi, PrototypeInfo>>), kTransitionsOrPrototypeInfoOffset)

void Map::init_prototype_and_constructor_or_back_pointer(ReadOnlyRoots roots)
{
    Tagged<HeapObject> null = roots.null_value();
    TaggedField<HeapObject, kConstructorOrBackPointerOrNativeContextOffset>::store(*this, null);
...
BIT_FIELD_ACCESSORS(Map, relaxed_bit_field, has_non_instance_prototype, Map::Bits1::HasNonInstancePrototypeBit)
BIT_FIELD_ACCESSORS(Map, relaxed_bit_field, has_prototype_slot, Map::Bits1::HasPrototypeSlotBit)
...
BIT_FIELD_ACCESSORS2(Map, relaxed_bit_field, bit_field, is_callable, Map::Bits1::IsCallableBit)
BIT_FIELD_ACCESSORS2(Map, relaxed_bit_field, bit_field, has_named_interceptor, Map::Bits1::HasNamedInterceptorBit)
BIT_FIELD_ACCESSORS2(Map, relaxed_bit_field, bit_field, has_indexed_interceptor, Map::Bits1::HasIndexedInterceptorBit)
BIT_FIELD_ACCESSORS2(Map, relaxed_bit_field, bit_field, is_undetectable, Map::Bits1::IsUndetectableBit)
BIT_FIELD_ACCESSORS2(Map, relaxed_bit_field, bit_field, is_access_check_needed, Map::Bits1::IsAccessCheckNeededBit)
BIT_FIELD_ACCESSORS2(Map, relaxed_bit_field, bit_field, is_constructor, Map::Bits1::IsConstructorBit)

// |bit_field2| fields.
BIT_FIELD_ACCESSORS(Map, bit_field2, new_target_is_base, Map::Bits2::NewTargetIsBaseBit)
BIT_FIELD_ACCESSORS(Map, bit_field2, is_immutable_proto, Map::Bits2::IsImmutablePrototypeBit)

// |bit_field3| fields.
BIT_FIELD_ACCESSORS(Map, relaxed_bit_field3, owns_descriptors, Map::Bits3::OwnsDescriptorsBit)
BIT_FIELD_ACCESSORS(Map, release_acquire_bit_field3, is_deprecated, Map::Bits3::IsDeprecatedBit)
BIT_FIELD_ACCESSORS(Map, relaxed_bit_field3, is_in_retained_map_list, Map::Bits3::IsInRetainedMapListBit)
BIT_FIELD_ACCESSORS(Map, release_acquire_bit_field3, is_prototype_map, Map::Bits3::IsPrototypeMapBit)
BIT_FIELD_ACCESSORS(Map, relaxed_bit_field3, is_migration_target, Map::Bits3::IsMigrationTargetBit)
BIT_FIELD_ACCESSORS2(Map, relaxed_bit_field3, bit_field3, is_extensible, Map::Bits3::IsExtensibleBit)
BIT_FIELD_ACCESSORS(Map, bit_field3, may_have_interesting_properties, Map::Bits3::MayHaveInterestingPropertiesBit)
BIT_FIELD_ACCESSORS(Map, relaxed_bit_field3, construction_counter, Map::Bits3::ConstructionCounterBits)
...
// static
bool Map::IsMostGeneralFieldType(Representation representation, Tagged<FieldType> field_type)
{
...
{
    return instance_type == JS_ARRAY_TYPE || instance_type == JS_PRIMITIVE_WRAPPER_TYPE || instance_type == JS_ARGUMENTS_OBJECT_TYPE;
}
...
bool Map::IsDetached(Isolate* isolate) const
{
    if (is_prototype_map())
        return true;
    return instance_type() == JS_OBJECT_TYPE && NumberOfOwnDescriptors() > 0 && IsUndefined(GetBackPointer(), isolate);
}

// static
void Map::GeneralizeIfCanHaveTransitionableFastElementsKind(
    Isolate* isolate, InstanceType instance_type, Representation* representation, Handle<FieldType>* field_type)
{
...
Handle<Map> Map::Normalize(Isolate* isolate, Handle<Map> fast_map, PropertyNormalizationMode mode, const char* reason)
{
    ...
    return Normalize(isolate, fast_map, fast_map->elements_kind(), {}, mode, kUseCache, reason);
}
...
bool Map::TooManyFastProperties(StoreOrigin store_origin) const
{
    if (UnusedPropertyFields() != 0)
        return false;
    if (store_origin != StoreOrigin::kMaybeKeyed)
        return false;
    if (is_prototype_map())
        return false;
    int limit = std::max({ v8_flags.fast_properties_soft_limit.value(), GetInObjectProperties() });
    int external = NumberOfFields(ConcurrencyMode::kSynchronous) - GetInObjectProperties();
...
{
    return Bits3::NumberOfOwnDescriptorsBits::decode(release_acquire_bit_field3());
}

void Map::SetNumberOfOwnDescriptors(int number)
{
    DCHECK_LE(number, instance_descriptors()->number_of_descriptors());
    CHECK_LE(static_cast<unsigned>(number), static_cast<unsigned>(kMaxNumberOfDescriptors));
    set_release_acquire_bit_field3(Bits3::NumberOfOwnDescriptorsBits::update(bit_field3(), number));
}
...
void Map::SetEnumLength(int length)
{
    if (length != kInvalidEnumCacheSentinel) {
        ...
        CHECK_LE(static_cast<unsigned>(length), static_cast<unsigned>(kMaxNumberOfDescriptors));
    }
...
Tagged<FixedArrayBase> Map::GetInitialElements() const
{
    ...
    if (has_fast_elements() || has_fast_string_wrapper_elements() || has_any_nonextensible_elements()) {
...
{
    return static_cast<VisitorId>(RELAXED_READ_BYTE_FIELD(*this, kVisitorIdOffset));
}
...
{
    RELAXED_WRITE_BYTE_FIELD(*this, kInstanceSizeInWordsOffset, static_cast<uint8_t>(value));
}
...
int Map::inobject_properties_start_or_constructor_function_index() const
{
    ...
    return RELAXED_READ_BYTE_FIELD(*this, kInobjectPropertiesStartOrConstructorFunctionIndexOffset);
}

void Map::set_inobject_properties_start_or_constructor_function_index(int value)
{
    CHECK_LE(static_cast<unsigned>(value), kMaxUInt8);
    RELAXED_WRITE_BYTE_FIELD(*this, kInobjectPropertiesStartOrConstructorFunctionIndexOffset, static_cast<uint8_t>(value));
}
...
bool Map::HasOutOfObjectProperties() const
{
    ...
    DCHECK_EQ(ret, GetInObjectProperties() < NumberOfFields(ConcurrencyMode::kSynchronous));
    return ret;
}
...
Handle<Map> Map::AddMissingTransitionsForTesting(Isolate* isolate, Handle<Map> split_map, DirectHandle<DescriptorArray> descriptors)
{
...
InstanceType Map::instance_type() const
{
    ...
    return static_cast<InstanceType>(RELAXED_READ_UINT16_FIELD(*this, kInstanceTypeOffset));
}
...
void Map::set_used_or_unused_instance_size_in_words(int value)
{
    ...
    RELAXED_WRITE_BYTE_FIELD(*this, kUsedOrUnusedInstanceSizeInWordsOffset, static_cast<uint8_t>(value));
}
...
void Map::SetInObjectUnusedPropertyFields(int value)
{
    ...
    set_used_or_unused_instance_size_in_words(GetInObjectPropertyOffset(used_inobject_properties) / kTaggedSize);
...
{
    set_used_or_unused_instance_size_in_words(map->used_or_unused_instance_size_in_words());
...
void Map::AccountAddedOutOfObjectPropertyField(int unused_in_property_array)
{
    ...
    CHECK_LT(static_cast<unsigned>(unused_in_property_array), JSObject::kFieldsAdded);
...
void Map::set_elements_kind(ElementsKind elements_kind)
{
    ...
    set_bit_field2(Map::Bits2::ElementsKindBits::update(bit_field2(), elements_kind));
}
...
{
    uint32_t new_bit_field3 = Bits3::IsDictionaryMapBit::update(bit_field3(), value);
...
{
    set_release_acquire_bit_field3(Bits3::IsUnstableBit::update(bit_field3(), true));
}
...
bool Map::CanBeDeprecated() const
{
    for (InternalIndex i : IterateOwnDescriptors()) {
        ...
        if (details.representation().MightCauseMapDeprecation())
            return true;
...
void Map::NotifyLeafMapLayoutChange(Isolate* isolate)
{
    if (is_stable()) {
        mark_unstable();
        DependentCode::DeoptimizeDependencyGroups(isolate, *this, DependentCode::kPrototypeCheckGroup);
    }
}
...
bool Map::CanTransition() const
{
    ...
    DCHECK_IMPLIES(InstanceTypeChecker::IsAlwaysSharedSpaceJSObject(type), HeapLayout::InAnySharedSpace(*this));
    return InstanceTypeChecker::IsJSObject(type) && !InstanceTypeChecker::IsAlwaysSharedSpaceJSObject(type);
}
...
void Map::UpdateDescriptors(Isolate* isolate, Tagged<DescriptorArray> descriptors, int number_of_own_descriptors)
{
    SetInstanceDescriptors(isolate, descriptors, number_of_own_descriptors);
}

void Map::InitializeDescriptors(Isolate* isolate, Tagged<DescriptorArray> descriptors)
{
    SetInstanceDescriptors(isolate, descriptors, descriptors->number_of_descriptors());
}
...
void Map::clear_padding()
{
    ...
    memset(reinterpret_cast<void*>(address() + kOptionalPaddingOffset), 0, FIELD_SIZE(kOptionalPaddingOffset));
}
...
void Map::AppendDescriptor(Isolate* isolate, Descriptor* desc)
{
    ...
    DCHECK(descriptors->number_of_descriptors() == number_of_own_descriptors);
    {
        ...
        WriteBarrier::ForDescriptorArray(descriptors, number_of_own_descriptors + 1);
#endif
    }
...
// static
bool Map::ConcurrentIsHeapObjectWithMap(PtrComprCageBase cage_base, Tagged<Object> object, Tagged<Map> meta_map)
{
...
bool Map::TryGetBackPointer(PtrComprCageBase cage_base, Tagged<Map>* back_pointer) const
{
...
void Map::SetBackPointer(Tagged<HeapObject> value, WriteBarrierMode mode)
{
    ...
    CHECK_EQ(Cast<Map>(value)->GetConstructorRaw(), constructor_or_back_pointer());
...
// static
Tagged<Map> Map::ElementsTransitionMap(Isolate* isolate, ConcurrencyMode cmode)
{
    return TransitionsAccessor(isolate, *this, IsConcurrent(cmode)).SearchSpecial(ReadOnlyRoots(isolate).elements_transition_symbol());
}

ACCESSORS(Map, dependent_code, Tagged<DependentCode>, kDependentCodeOffset)
RELAXED_ACCESSORS(Map, prototype_validity_cell, (Tagged<UnionOf<Smi, Cell>>), kPrototypeValidityCellOffset)
ACCESSORS_CHECKED2(Map, constructor_or_back_pointer, Tagged<Object>, kConstructorOrBackPointerOrNativeContextOffset, !IsContextMap(*this),
    IsNull(value) || !IsContextMap(*this))
RELAXED_ACCESSORS_CHECKED2(Map, constructor_or_back_pointer, Tagged<Object>, kConstructorOrBackPointerOrNativeContextOffset, !IsContextMap(*this),
    IsNull(value) || !IsContextMap(*this))
ACCESSORS_CHECKED(Map, native_context, Tagged<NativeContext>, kConstructorOrBackPointerOrNativeContextOffset, IsContextMap(*this) || IsMapMap(*this))
ACCESSORS_CHECKED(Map, native_context_or_null, Tagged<Object>, kConstructorOrBackPointerOrNativeContextOffset,
...
{
    Tagged<Object> value = TaggedField<Tagged<Object>, kConstructorOrBackPointerOrNativeContextOffset>::load(cage_base, *this);
...
#if V8_ENABLE_WEBASSEMBLY
ACCESSORS_CHECKED(Map, wasm_type_info, Tagged<WasmTypeInfo>, kConstructorOrBackPointerOrNativeContextOffset,
    IsWasmStructMap(*this) || IsWasmArrayMap(*this) || IsWasmFuncRefMap(*this))
#endif // V8_ENABLE_WEBASSEMBLY
...
bool Map::BelongsToSameNativeContextAs(Tagged<Map> other_map) const
{
    ...
    if (this_meta_map == GetReadOnlyRoots().meta_map())
        return false;
...
DEF_GETTER(Map, GetConstructorRaw, Tagged<Object>)
{
    ...
    // so just compare object's map against current map's meta map.
    Tagged<Map> meta_map = map(cage_base);
    while (ConcurrentIsHeapObjectWithMap(cage_base, maybe_constructor, meta_map)) {
        DCHECK(IsMap(maybe_constructor));
        // Sanity check - only contextful maps can transition.
        DCHECK(IsNativeContext(meta_map->native_context_or_null()));
        maybe_constructor = Cast<Map>(maybe_constructor)->constructor_or_back_pointer(cage_base);
    }
    // If it was a map that'd mean that there are maps from different native
    // contexts in the transition tree.
    DCHECK(!IsMap(maybe_constructor));
...
DEF_GETTER(Map, GetNonInstancePrototype, Tagged<Object>)
{
    ...
    Tagged<Tuple2> non_instance_prototype_constructor_tuple = Cast<Tuple2>(raw_constructor);
...
Tagged<Object> Map::TryGetConstructor(PtrComprCageBase cage_base, int max_steps)
{
    Tagged<Object> maybe_constructor = constructor_or_back_pointer(cage_base);
    // Follow any back pointers.
...
DEF_GETTER(Map, GetFunctionTemplateInfo, Tagged<FunctionTemplateInfo>)
{
    ...
    if (IsJSFunction(constructor, cage_base)) {
        Tagged<SharedFunctionInfo> sfi = Cast<JSFunction>(constructor)->shared(cage_base);
...
{
    return CopyInitialMap(isolate, map, map->instance_size(), map->GetInObjectProperties(), map->UnusedPropertyFields());
}
...
void Map::InobjectSlackTrackingStep(Isolate* isolate)
{
    ...
    if (!this->IsInobjectSlackTrackingInProgress())
        return;
...
int NormalizedMapCache::GetIndex(Isolate* isolate, Tagged<Map> map, Tagged<HeapObject> prototype)
{
    DisallowGarbageCollection no_gc;
...
{
    if (!IsWeakFixedArray(obj, cage_base))
        return false;
