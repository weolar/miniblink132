...
V8_EXPORT_PRIVATE void ManagedObjectFinalizer(const v8::WeakCallbackInfo<void>& data);
...
template <class CppType> class Managed : public Foreign {
public:
    Managed()
        : Foreign()
    {
    ...
    {
        return GetDestructor()->estimated_size_;
    }
    ...
    static Handle<Managed<CppType>> From(
        Isolate* isolate, size_t estimated_size, std::shared_ptr<CppType> shared_ptr, AllocationType allocation_type = AllocationType::kYoung);
...
private:
    ...
    {
        return reinterpret_cast<std::shared_ptr<CppType>*>(GetDestructor()->shared_ptr_ptr_);
    }
};
...
template <class CppType> class TrustedManaged : public TrustedForeign {
public:
    TrustedManaged()
        : TrustedForeign()
    {
    }
    explicit TrustedManaged(Address ptr)
        : TrustedForeign(ptr)
    {
    }
    ...
    static Handle<TrustedManaged<CppType>> From(Isolate* isolate, size_t estimated_size, std::shared_ptr<CppType> shared_ptr);
...
private:
    ...
    std::shared_ptr<CppType>* GetSharedPtrPtr()
    {
        ...
        return reinterpret_cast<std::shared_ptr<CppType>*>(destructor->shared_ptr_ptr_);
    }
};
