...
#define INSTANCE_TYPE_CHECKERS_RANGE(V)                                                                                                                        \
    ...
    V(CallableJSFunction, FIRST_CALLABLE_JS_FUNCTION_TYPE, LAST_CALLABLE_JS_FUNCTION_TYPE)
...
constexpr Tagged_t kNonJsReceiverMapLimit = StaticReadOnlyRootsPointerTable[static_cast<size_t>(RootIndex::kFirstJSReceiverMapRoot)] & ~0xFFF;
...
constexpr Tagged_t kStringMapUpperBound = StaticReadOnlyRoot::kThinOneByteStringMap;

#define ASSERT_IS_LAST_STRING_MAP(instance_type, size, name, Name) static_assert(StaticReadOnlyRoot::k##Name##Map <= kStringMapUpperBound);
...
static_assert(kNonJsReceiverMapLimit != 0 && is_uint12(kNonJsReceiverMapLimit >> 12) && ((kNonJsReceiverMapLimit & 0xFFF) == 0));
