...
FieldIndex FieldIndex::ForSmiLoadHandler(Tagged<Map> map, int32_t handler)
{
    ...
    return FieldIndex(is_inobject, LoadHandler::FieldIndexBits::decode(handler) * kTaggedSize, LoadHandler::IsDoubleBits::decode(handler) ? kDouble : kTagged,
        inobject_properties, first_inobject_offset);
}

FieldIndex FieldIndex::ForPropertyIndex(Tagged<Map> map, int property_index, Representation representation)
{
    ...
    return FieldIndex(is_inobject, offset, encoding, inobject_properties, first_inobject_offset);
}
...
FieldIndex FieldIndex::ForDescriptor(Tagged<Map> map, InternalIndex descriptor_index)
{
    PtrComprCageBase cage_base = GetPtrComprCageBase(map);
    return ForDescriptor(cage_base, map, descriptor_index);
}

FieldIndex FieldIndex::ForDescriptor(PtrComprCageBase cage_base, Tagged<Map> map, InternalIndex descriptor_index)
{
    PropertyDetails details = map->instance_descriptors(cage_base, kRelaxedLoad)->GetDetails(descriptor_index);
