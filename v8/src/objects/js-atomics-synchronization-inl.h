...
std::atomic<JSSynchronizationPrimitive::StateT>* JSSynchronizationPrimitive::AtomicStatePtr()
{
...
void JSSynchronizationPrimitive::SetNullWaiterQueueHead()
{
    ...
    base::AsAtomic32::Relaxed_Store(waiter_queue_head_handle_location(), kNullExternalPointerHandle);
#else
...
#if V8_COMPRESS_POINTERS
ExternalPointerHandle* JSSynchronizationPrimitive::waiter_queue_head_handle_location() const
{
    Address location = field_address(kWaiterQueueHeadOffset);
    return reinterpret_cast<ExternalPointerHandle*>(location);
}
#else
WaiterQueueNode** JSSynchronizationPrimitive::waiter_queue_head_location() const
{
    Address location = field_address(kWaiterQueueHeadOffset);
...
WaiterQueueNode* JSSynchronizationPrimitive::DestructivelyGetWaiterQueueHead(Isolate* requester)
{
...
#if V8_COMPRESS_POINTERS
    ExternalPointerHandle handle = base::AsAtomic32::Relaxed_Load(waiter_queue_head_handle_location());
    ...
    WaiterQueueNode* waiter_head
        = reinterpret_cast<WaiterQueueNode*>(requester->shared_external_pointer_table().Exchange(handle, kNullAddress, kWaiterQueueNodeTag));
...
JSSynchronizationPrimitive::StateT JSSynchronizationPrimitive::SetWaiterQueueHead(Isolate* requester, WaiterQueueNode* waiter_head, StateT new_state)
{
    if (V8_UNLIKELY(DEBUG_BOOL)) {
        StateT state = AtomicStatePtr()->load(std::memory_order_relaxed);
        DCHECK(IsWaiterQueueLockedField::decode(state));
    ...
    if (waiter_head) {
        ...
        if (handle == kNullExternalPointerHandle) {
            handle = table.AllocateAndInitializeEntry(requester->shared_external_pointer_space(), reinterpret_cast<Address>(waiter_head), kWaiterQueueNodeTag);
            // Use a Release_Store to ensure that the store of the pointer into the
            // table is not reordered after the store of the handle. Otherwise, other
            // threads may access an uninitialized table entry and crash.
            base::AsAtomic32::Release_Store(waiter_queue_head_handle_location(), handle);
            EXTERNAL_POINTER_WRITE_BARRIER(*this, kWaiterQueueHeadOffset, kWaiterQueueNodeTag);
            return new_state;
        }
        if (DEBUG_BOOL) {
            Address old = requester->shared_external_pointer_table().Exchange(handle, reinterpret_cast<Address>(waiter_head), kWaiterQueueNodeTag);
            DCHECK_EQ(kNullAddress, old);
            USE(old);
        } else {
            requester->shared_external_pointer_table().Set(handle, reinterpret_cast<Address>(waiter_head), kWaiterQueueNodeTag);
        }
    } else {
    ...
    base::AsAtomicPointer::Relaxed_Store(waiter_queue_head_location(), waiter_head);
#endif // V8_COMPRESS_POINTERS
    return new_state;
}
...
JSAtomicsMutex::LockGuardBase::LockGuardBase(Isolate* isolate, Handle<JSAtomicsMutex> mutex, bool locked)
    : isolate_(isolate)
    , mutex_(mutex)
    , locked_(locked)
{
...
JSAtomicsMutex::LockGuard::LockGuard(Isolate* isolate, Handle<JSAtomicsMutex> mutex, std::optional<base::TimeDelta> timeout)
    : LockGuardBase(isolate, mutex, JSAtomicsMutex::Lock(isolate, mutex, timeout))
{
}

JSAtomicsMutex::TryLockGuard::TryLockGuard(Isolate* isolate, Handle<JSAtomicsMutex> mutex)
    : LockGuardBase(isolate, mutex, mutex->TryLock())
{
}

// static
bool JSAtomicsMutex::LockImpl(
    Isolate* requester, DirectHandle<JSAtomicsMutex> mutex, std::optional<base::TimeDelta> timeout, LockSlowPathWrapper slow_path_wrapper)
{
    ...
    bool locked;
    if (V8_LIKELY(state->compare_exchange_weak(expected, kLockedUncontended, std::memory_order_acquire, std::memory_order_relaxed))) {
        locked = true;
    } else {
...
// static
bool JSAtomicsMutex::Lock(Isolate* requester, Handle<JSAtomicsMutex> mutex, std::optional<base::TimeDelta> timeout)
{
    return LockImpl(requester, mutex, timeout, [=](std::atomic<StateT>* state) { return LockSlowPath(requester, mutex, state, timeout); });
}
...
bool JSAtomicsMutex::TryLock()
{
    DisallowGarbageCollection no_gc;
    StateT expected = kUnlockedUncontended;
    if (V8_LIKELY(AtomicStatePtr()->compare_exchange_strong(expected, kLockedUncontended, std::memory_order_acquire, std::memory_order_relaxed))) {
...
void JSAtomicsMutex::Unlock(Isolate* requester)
{
    ...
    if (V8_LIKELY(state->compare_exchange_strong(expected, kUnlockedUncontended, std::memory_order_release, std::memory_order_relaxed))) {
        return;
    }
...
{
    return IsLockedField::decode(AtomicStatePtr()->load(std::memory_order_relaxed));
}

bool JSAtomicsMutex::IsCurrentThreadOwner()
{
    bool result = AtomicOwnerThreadIdPtr()->load(std::memory_order_relaxed) == ThreadId::Current().ToInteger();
...
{
    AtomicOwnerThreadIdPtr()->store(ThreadId::Current().ToInteger(), std::memory_order_relaxed);
}

void JSAtomicsMutex::ClearOwnerThread()
{
    AtomicOwnerThreadIdPtr()->store(ThreadId::Invalid().ToInteger(), std::memory_order_relaxed);
}

std::atomic<int32_t>* JSAtomicsMutex::AtomicOwnerThreadIdPtr()
{
    int32_t* owner_thread_id_ptr = reinterpret_cast<int32_t*>(field_address(kOwnerThreadIdOffset));
