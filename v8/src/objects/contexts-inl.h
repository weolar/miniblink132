...
Tagged<NameToIndexHashTable> ScriptContextTable::names_to_context_index() const
{
    return names_to_context_index_.load();
}
void ScriptContextTable::set_names_to_context_index(Tagged<NameToIndexHashTable> value, WriteBarrierMode mode)
{
...
{
    DCHECK_LT(static_cast<unsigned int>(index), static_cast<unsigned int>(length(kRelaxedLoad)));
...
Tagged<Object> Context::get(PtrComprCageBase cage_base, int index, AcquireLoadTag) const
{
    DCHECK_LT(static_cast<unsigned int>(index), static_cast<unsigned int>(length(kRelaxedLoad)));
    return ACQUIRE_READ_FIELD(*this, OffsetOfElementAt(index));
}

void Context::set(int index, Tagged<Object> value, WriteBarrierMode mode, ReleaseStoreTag)
{
    DCHECK_LT(static_cast<unsigned int>(index), static_cast<unsigned int>(length(kRelaxedLoad)));
    const int offset = OffsetOfElementAt(index);
...
void NativeContext::set(int index, Tagged<Object> value, WriteBarrierMode mode, ReleaseStoreTag tag)
{
...
bool Context::IsDetached() const
{
    return global_object()->IsDetached();
}
...
int Context::FunctionMapIndex(LanguageMode language_mode, FunctionKind kind, bool has_shared_name)
{
    ...
    if (IsGeneratorFunction(kind)) {
        CHECK_FOLLOWS2(GENERATOR_FUNCTION_MAP_INDEX, GENERATOR_FUNCTION_WITH_NAME_MAP_INDEX);
        CHECK_FOLLOWS2(ASYNC_GENERATOR_FUNCTION_MAP_INDEX, ASYNC_GENERATOR_FUNCTION_WITH_NAME_MAP_INDEX);

        base = IsAsyncFunction(kind) ? ASYNC_GENERATOR_FUNCTION_MAP_INDEX : GENERATOR_FUNCTION_MAP_INDEX;
        ...
        CHECK_FOLLOWS2(STRICT_FUNCTION_WITHOUT_PROTOTYPE_MAP_INDEX, METHOD_WITH_NAME_MAP_INDEX);
        ...
        base = is_strict(language_mode) ? STRICT_FUNCTION_MAP_INDEX : SLOPPY_FUNCTION_MAP_INDEX;
    }
...
EXTERNAL_POINTER_ACCESSORS(NativeContext, microtask_queue, MicrotaskQueue*, kMicrotaskQueueOffset, kNativeContextMicrotaskQueueTag)

void NativeContext::synchronized_set_script_context_table(Tagged<ScriptContextTable> script_context_table)
{
    set(SCRIPT_CONTEXT_TABLE_INDEX, script_context_table, UPDATE_WRITE_BARRIER, kReleaseStore);
}

Tagged<ScriptContextTable> NativeContext::synchronized_script_context_table() const
{
    return Cast<ScriptContextTable>(get(SCRIPT_CONTEXT_TABLE_INDEX, kAcquireLoad));
}

Tagged<Map> NativeContext::TypedArrayElementsKindToCtorMap(ElementsKind element_kind) const
{
    int ctor_index = Context::FIRST_FIXED_TYPED_ARRAY_FUN_INDEX + element_kind - ElementsKind::FIRST_FIXED_TYPED_ARRAY_ELEMENTS_KIND;
...
Tagged<Map> NativeContext::TypedArrayElementsKindToRabGsabCtorMap(ElementsKind element_kind) const
{
    int ctor_index = Context::FIRST_RAB_GSAB_TYPED_ARRAY_MAP_INDEX + element_kind - ElementsKind::FIRST_FIXED_TYPED_ARRAY_ELEMENTS_KIND;
    Tagged<Map> map = Cast<Map>(get(ctor_index));
    DCHECK_EQ(map->elements_kind(), GetCorrespondingRabGsabElementsKind(element_kind));
    DCHECK(InstanceTypeChecker::IsJSTypedArray(map));
    return map;
}
