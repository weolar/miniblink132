...
template <typename KeyT> class V8_EXPORT_PRIVATE BaseShape {
public:
    using Key = KeyT;
...
protected:
    ...
    inline static InternalIndex NextProbe(InternalIndex last, uint32_t number, uint32_t size)
    {
        return InternalIndex((last.as_uint32() + number) & (size - 1));
    }
};

template <typename Derived, typename ShapeT> class EXPORT_TEMPLATE_DECLARE(V8_EXPORT_PRIVATE) HashTable : public HashTableBase {
public:
    ...
    V8_WARN_UNUSED_RESULT static Handle<Derived> New(IsolateT* isolate, int at_least_space_for, AllocationType allocation = AllocationType::kYoung,
    ...
    inline InternalIndex FindEntry(PtrComprCageBase cage_base, ReadOnlyRoots roots, Key key, int32_t hash);
    template <typename IsolateT> inline InternalIndex FindEntry(IsolateT* isolate, Key key);
    ...
    inline bool ToKey(ReadOnlyRoots roots, InternalIndex entry, Tagged<Object>* out_k);
    inline bool ToKey(PtrComprCageBase cage_base, InternalIndex entry, Tagged<Object>* out_k);
    ...
    inline Tagged<Object> KeyAt(PtrComprCageBase cage_base, InternalIndex entry, RelaxedLoadTag tag);

    inline void SetKeyAt(InternalIndex entry, Tagged<Object> value, WriteBarrierMode mode = UPDATE_WRITE_BARRIER);
    ...
    static const int kElementsStartOffset = OFFSET_OF_DATA_START(HashTableBase) + kElementsStartIndex * kTaggedSize;
    ...
    {
        return static_cast<int>((slot - object - sizeof(HashTableBase)) / kTaggedSize);
    }

    // Ensure enough space for n additional elements.
    template <typename IsolateT>
    V8_WARN_UNUSED_RESULT static Handle<Derived> EnsureCapacity(
        IsolateT* isolate, Handle<Derived> table, int n = 1, AllocationType allocation = AllocationType::kYoung);
...
protected:
    ...
    template <typename IsolateT> V8_WARN_UNUSED_RESULT static Handle<Derived> NewInternal(IsolateT* isolate, int capacity, AllocationType allocation);

    // Find the entry at which to insert element with the given key that
    // has the given hash value.
    InternalIndex FindInsertionEntry(PtrComprCageBase cage_base, ReadOnlyRoots roots, uint32_t hash);
    template <typename IsolateT> InternalIndex FindInsertionEntry(IsolateT* isolate, uint32_t hash);
    ...
    static int ComputeCapacityWithShrink(int current_capacity, int at_least_room_for);

    // Shrink the hash table.
    V8_WARN_UNUSED_RESULT static Handle<Derived> Shrink(Isolate* isolate, Handle<Derived> table, int additionalCapacity = 0);
...
private:
    ...
    static_assert(EntryToIndex(InternalIndex(kMaxRegularCapacity)) < FixedArray::kMaxRegularLength);
    ...
    static const int kMaxRegularIndex = EntryToIndex(InternalIndex(kMaxRegularEntry));
    ...
    // number _probe_.
    InternalIndex EntryForProbe(ReadOnlyRoots roots, Tagged<Object> k, int probe, InternalIndex expected);

    void Swap(InternalIndex entry1, InternalIndex entry2, WriteBarrierMode mode);
};

#define EXTERN_DECLARE_HASH_TABLE(DERIVED, SHAPE)                                                                                                              \
    extern template class EXPORT_TEMPLATE_DECLARE(V8_EXPORT_PRIVATE) HashTable<class DERIVED, SHAPE>;                                                          \
                                                                                                                                                               \
    extern template EXPORT_TEMPLATE_DECLARE(V8_EXPORT_PRIVATE) Handle<DERIVED> HashTable<DERIVED, SHAPE>::New(Isolate*, int, AllocationType, MinimumCapacity); \
    extern template EXPORT_TEMPLATE_DECLARE(V8_EXPORT_PRIVATE) Handle<DERIVED> HashTable<DERIVED, SHAPE>::New(                                                 \
        LocalIsolate*, int, AllocationType, MinimumCapacity);                                                                                                  \
                                                                                                                                                               \
    extern template EXPORT_TEMPLATE_DECLARE(V8_EXPORT_PRIVATE) Handle<DERIVED> HashTable<DERIVED, SHAPE>::EnsureCapacity(                                      \
...
template <typename Derived, typename Shape> class EXPORT_TEMPLATE_DECLARE(V8_EXPORT_PRIVATE) ObjectHashTableBase : public HashTable<Derived, Shape> {
public:
    // Looks up the value associated with the given key. The hole value is
    // returned in case the key is not present.
    Tagged<Object> Lookup(Handle<Object> key);
    Tagged<Object> Lookup(Handle<Object> key, int32_t hash);
    Tagged<Object> Lookup(PtrComprCageBase cage_base, Handle<Object> key, int32_t hash);
    ...
    static Handle<Derived> Put(Isolate* isolate, Handle<Derived> table, Handle<Object> key, DirectHandle<Object> value, int32_t hash);
...
#define EXTERN_DECLARE_OBJECT_BASE_HASH_TABLE(DERIVED, SHAPE)                                                                                                  \
    ...
    extern template class EXPORT_TEMPLATE_DECLARE(V8_EXPORT_PRIVATE) ObjectHashTableBase<class DERIVED, SHAPE>;

EXTERN_DECLARE_OBJECT_BASE_HASH_TABLE(ObjectHashTable, ObjectHashTableShape)

// ObjectHashTable maps keys that are arbitrary objects to object values by
// using the identity hash of the key for hashing purposes.
class V8_EXPORT_PRIVATE ObjectHashTable : public ObjectHashTableBase<ObjectHashTable, ObjectHashTableShape> {
public:
...
class V8_EXPORT_PRIVATE EphemeronHashTable : public ObjectHashTableBase<EphemeronHashTable, ObjectHashTableShape> {
public:
    static inline Handle<Map> GetMap(ReadOnlyRoots roots);
...
template <int N> class ObjectMultiHashTableShape : public ObjectHashTableShape {
public:
    static const int kEntrySize = 1 + N;
};

template <typename Derived, int N> class ObjectMultiHashTableBase : public HashTable<Derived, ObjectMultiHashTableShape<N>> {
public:
    ...
    std::array<Tagged<Object>, N> Lookup(PtrComprCageBase cage_base, Handle<Object> key);

    // Adds or overwrites the values associated with the given key.
    static Handle<Derived> Put(Isolate* isolate, Handle<Derived> table, Handle<Object> key, const std::array<Handle<Object>, N>& values);

private:
    void SetEntryValues(InternalIndex entry, const std::array<Handle<Object>, N>& values);

    static constexpr inline int EntryToValueIndexStart(InternalIndex entry)
    {
        return HashTable<Derived, ObjectMultiHashTableShape<N>>::EntryToIndex(entry) + ObjectMultiHashTableShape<N>::kEntryValueIndex;
    }
};

class ObjectTwoHashTable : public ObjectMultiHashTableBase<ObjectTwoHashTable, 2> { };
...
class V8_EXPORT_PRIVATE ObjectHashSet : public HashTable<ObjectHashSet, ObjectHashSetShape> {
public:
    static Handle<ObjectHashSet> Add(Isolate* isolate, Handle<ObjectHashSet> set, Handle<Object> key);
...
class V8_EXPORT_PRIVATE NameToIndexHashTable : public HashTable<NameToIndexHashTable, NameToIndexShape> {
public:
    ...
    template <typename IsolateT>
    static Handle<NameToIndexHashTable> Add(IsolateT* isolate, Handle<NameToIndexHashTable> table, IndirectHandle<Name> key, int32_t value);
...
class RegisteredSymbolTable : public HashTable<RegisteredSymbolTable, RegisteredSymbolTableShape> {
public:
    ...
    static Handle<RegisteredSymbolTable> Add(Isolate* isolate, Handle<RegisteredSymbolTable> table, IndirectHandle<String> key, DirectHandle<Symbol>);
