...
static constexpr int kFeedbackSlotKindCount = static_cast<int>(FeedbackSlotKind::kLast) + 1;
...
{
    return kind == FeedbackSlotKind::kSetNamedSloppy || kind == FeedbackSlotKind::kSetNamedStrict;
}
...
{
    return kind == FeedbackSlotKind::kSetKeyedSloppy || kind == FeedbackSlotKind::kSetKeyedStrict;
}
...
inline TypeofMode GetTypeofModeFromSlotKind(FeedbackSlotKind kind)
{
    ...
    return (kind == FeedbackSlotKind::kLoadGlobalInsideTypeof) ? TypeofMode::kInside : TypeofMode::kNotInside;
}

inline LanguageMode GetLanguageModeFromSlotKind(FeedbackSlotKind kind)
{
    DCHECK(IsSetNamedICKind(kind) || IsDefineNamedOwnICKind(kind) || IsStoreGlobalICKind(kind) || IsKeyedStoreICKind(kind) || IsDefineKeyedOwnICKind(kind));
    static_assert(FeedbackSlotKind::kStoreGlobalSloppy <= FeedbackSlotKind::kLastSloppyKind);
    static_assert(FeedbackSlotKind::kSetKeyedSloppy <= FeedbackSlotKind::kLastSloppyKind);
    static_assert(FeedbackSlotKind::kSetNamedSloppy <= FeedbackSlotKind::kLastSloppyKind);
    return (kind <= FeedbackSlotKind::kLastSloppyKind) ? LanguageMode::kSloppy : LanguageMode::kStrict;
}

V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, FeedbackSlotKind kind);
...
public:
    ...
    static constexpr RootIndex kMapRootIndex = RootIndex::kClosureFeedbackCellArrayMap;
...
// closure.
class ClosureFeedbackCellArray : public TaggedArrayBase<ClosureFeedbackCellArray, ClosureFeedbackCellArrayShape> {
    using Super = TaggedArrayBase<ClosureFeedbackCellArray, ClosureFeedbackCellArrayShape>;
...
public:
    ...
    V8_EXPORT_PRIVATE static Handle<ClosureFeedbackCellArray> New(
        Isolate* isolate, DirectHandle<SharedFunctionInfo> shared, AllocationType allocation = AllocationType::kYoung);
...
public:
    ...
    static constexpr uint32_t kFlagsMaybeHasTurbofanCode = FeedbackVector::MaybeHasTurbofanCodeBit::kMask;
    static constexpr uint32_t kFlagsMaybeHasMaglevCode = FeedbackVector::MaybeHasMaglevCodeBit::kMask;
    static constexpr uint32_t kFlagsHasAnyOptimizedCode = FeedbackVector::MaybeHasMaglevCodeBit::kMask | FeedbackVector::MaybeHasTurbofanCodeBit::kMask;
#endif // !V8_ENABLE_LEAPTIERING
    static constexpr uint32_t kFlagsTieringStateIsAnyRequested = kNoneOrInProgressMask << FeedbackVector::TieringStateBits::kShift;
    static constexpr uint32_t kFlagsLogNextExecution = FeedbackVector::LogNextExecutionBit::kMask;

    static constexpr inline uint32_t FlagMaskForNeedsProcessingCheckFrom(CodeKind code_kind);
    ...
    inline std::optional<Tagged<Code>> GetOptimizedOsrCode(Isolate* isolate, FeedbackSlot slot);
    ...
    inline void SynchronizedSet(FeedbackSlot slot, Tagged<MaybeObject> value, WriteBarrierMode mode = UPDATE_WRITE_BARRIER);

    inline Tagged<MaybeObject> Get(FeedbackSlot slot) const;
    inline Tagged<MaybeObject> Get(PtrComprCageBase cage_base, FeedbackSlot slot) const;

    // Returns the feedback cell at |index| that is used to create the
    // closure.
    inline Handle<FeedbackCell> GetClosureFeedbackCell(Isolate* isolate, int index) const;
    ...
    V8_EXPORT_PRIVATE static Handle<FeedbackVector> New(Isolate* isolate, DirectHandle<SharedFunctionInfo> shared,
        DirectHandle<ClosureFeedbackCellArray> closure_feedback_cell_array, DirectHandle<FeedbackCell> parent_feedback_cell,
        IsCompiledScope* is_compiled_scope);

    V8_EXPORT_PRIVATE static Handle<FeedbackVector> NewForTesting(Isolate* isolate, const FeedbackVectorSpec* spec);
    V8_EXPORT_PRIVATE static Handle<FeedbackVector> NewWithOneBinarySlotForTesting(Zone* zone, Isolate* isolate);
    V8_EXPORT_PRIVATE static Handle<FeedbackVector> NewWithOneCompareSlotForTesting(Zone* zone, Isolate* isolate);

#define DEFINE_SLOT_KIND_PREDICATE(Name)                                                                                                                       \
    bool Name(FeedbackSlot slot) const                                                                                                                         \
    {                                                                                                                                                          \
        return Name##Kind(GetKind(slot));                                                                                                                      \
    }
...
#undef DEFINE_SLOT_KIND_PREDICATE
    ...
    static void TraceFeedbackChange(Isolate* isolate, Tagged<FeedbackVector> vector, FeedbackSlot slot, const char* reason);
#endif
    ...
    static_assert(kHeaderSize % kObjectAlignment == 0, "Header must be padded for alignment");
...
public:
    explicit FeedbackVectorSpec(Zone* zone)
        : slot_kinds_(zone)
        , create_closure_parameter_counts_(zone)
    {
    ...
    {
        return AddSlot(typeof_mode == TypeofMode::kInside ? FeedbackSlotKind::kLoadGlobalInsideTypeof : FeedbackSlotKind::kLoadGlobalNotInsideTypeof);
    }
    ...
    FeedbackSlotKind GetStoreICSlot(LanguageMode language_mode)
    {
        ...
        return is_strict(language_mode) ? FeedbackSlotKind::kSetNamedStrict : FeedbackSlotKind::kSetNamedSloppy;
    }
    ...
    {
        static_assert(LanguageModeSize == 2);
        return AddSlot(is_strict(language_mode) ? FeedbackSlotKind::kStoreGlobalStrict : FeedbackSlotKind::kStoreGlobalSloppy);
    }

    FeedbackSlotKind GetKeyedStoreICSlotKind(LanguageMode language_mode)
    {
        static_assert(LanguageModeSize == 2);
        return is_strict(language_mode) ? FeedbackSlotKind::kSetKeyedStrict : FeedbackSlotKind::kSetKeyedSloppy;
    }
    ...
    {
        return AddSlot(FeedbackSlotKind::kTypeOf);
    }
...
public:
    ...
    template <typename IsolateT> V8_EXPORT_PRIVATE static Handle<FeedbackMetadata> New(IsolateT* isolate, const FeedbackVectorSpec* spec);
    ...
    {
        return OBJECT_POINTER_ALIGN(kHeaderSize + word_count(slot_count) * kInt32Size + create_closure_slot_count * kUInt16Size);
    }
...
private:
    ...
    using VectorICComputer = base::BitSetComputer<FeedbackSlotKind, kFeedbackSlotKindBits, kInt32Size * kBitsPerByte, uint32_t>;
...
public:
    ...
    template <typename T> Handle<T> NewHandle(Tagged<T> object) const;

    bool can_write() const
    {
        return mode() == MainThread;
    }

    inline Tagged<MaybeObject> GetFeedback(Tagged<FeedbackVector> vector, FeedbackSlot slot) const;
    inline void SetFeedback(Tagged<FeedbackVector> vector, FeedbackSlot slot, Tagged<MaybeObject> object, WriteBarrierMode mode = UPDATE_WRITE_BARRIER) const;

    std::pair<Tagged<MaybeObject>, Tagged<MaybeObject>> GetFeedbackPair(Tagged<FeedbackVector> vector, FeedbackSlot slot) const;
    void SetFeedbackPair(Tagged<FeedbackVector> vector, FeedbackSlot start_slot, Tagged<MaybeObject> feedback, WriteBarrierMode mode,
...
public:
    ...
    int ExtractMapsAndHandlers(std::vector<MapAndHandler>* maps_and_handlers, TryUpdateHandler map_handler = TryUpdateHandler()) const;
    ...
    template <typename F> void IterateMapsWithUnclearedHandler(F) const;
    ...
    inline std::pair<Tagged<MaybeObject>, Tagged<MaybeObject>> GetFeedbackPair() const;

    void ConfigureMonomorphic(Handle<Name> name, DirectHandle<Map> receiver_map, const MaybeObjectHandle& handler);

    void ConfigurePolymorphic(Handle<Name> name, std::vector<MapAndHandler> const& maps_and_handlers);
    ...
    void ConfigureCloneObject(Handle<Map> source_map, const MaybeObjectHandle& handler);
...
private:
    template <typename FeedbackType> inline void SetFeedback(Tagged<FeedbackType> feedback, WriteBarrierMode mode = UPDATE_WRITE_BARRIER);
    template <typename FeedbackType, typename FeedbackExtraType>
    inline void SetFeedback(
        Tagged<FeedbackType> feedback, WriteBarrierMode mode, Tagged<FeedbackExtraType> feedback_extra, WriteBarrierMode mode_extra = UPDATE_WRITE_BARRIER);
    ...
    mutable std::optional<std::pair<MaybeObjectHandle, MaybeObjectHandle>> feedback_cache_;
    NexusConfig config_;
    Isolate* isolate_;
};
