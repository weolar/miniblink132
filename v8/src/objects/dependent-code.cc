...
Tagged<DependentCode> DependentCode::GetDependentCode(Tagged<HeapObject> object)
{
    if (IsMap(object)) {
...
void DependentCode::SetDependentCode(Handle<HeapObject> object, DirectHandle<DependentCode> dep)
{
    if (IsMap(*object)) {
...
{
    while (groups != 0) {
        auto group = static_cast<DependentCode::DependencyGroup>(1 << base::bits::CountTrailingZeros(static_cast<uint32_t>(groups)));
...
void DependentCode::InstallDependency(Isolate* isolate, Handle<Code> code, Handle<HeapObject> object, DependencyGroups groups)
{
    if (V8_UNLIKELY(v8_flags.trace_compilation_dependencies)) {
        StdoutStream {} << "Installing dependency of [" << code << "] on [" << object << "] in groups [";
        PrintDependencyGroups(groups);
        StdoutStream {} << "]\n";
    }
    Handle<DependentCode> old_deps(DependentCode::GetDependentCode(*object), isolate);
    Handle<DependentCode> new_deps = InsertWeakCode(isolate, old_deps, groups, code);
...
Handle<DependentCode> DependentCode::InsertWeakCode(Isolate* isolate, Handle<DependentCode> entries, DependencyGroups groups, DirectHandle<Code> code)
{
    if (entries->length() == entries->capacity()) {
        // We'd have to grow - try to compact first.
        entries->IterateAndCompact(isolate, [](Tagged<Code>, DependencyGroups) { return false; });
    }
    ...
    entries = Cast<DependentCode>(WeakArrayList::AddToEnd(isolate, entries, code_slot, Smi::FromInt(groups)));
    return entries;
}

template <typename Function> void DependentCode::IterateAndCompact(IsolateForSandbox isolate, const Function& fn)
{
    ...
    while (i >= 0) {
        ...
        if (fn(Cast<CodeWrapper>(obj.GetHeapObjectAssumeWeak())->code(isolate), static_cast<DependencyGroups>(Get(i + kGroupsSlotOffset).ToSmi().value()))) {
            len = FillEntryFromBack(i, len);
        }
...
bool DependentCode::MarkCodeForDeoptimization(Isolate* isolate, DependentCode::DependencyGroups deopt_groups)
{
    DisallowGarbageCollection no_gc;
    ...
    IterateAndCompact(isolate, [&](Tagged<Code> code, DependencyGroups groups) {
        ...
        if (!code->marked_for_deoptimization()) {
            ...
            DependencyGroup first_group = static_cast<DependencyGroup>(1 << base::bits::CountTrailingZeros32(groups & deopt_groups));
...
int DependentCode::FillEntryFromBack(int index, int length)
{
    ...
    for (int i = length - kSlotsPerEntry; i > index; i -= kSlotsPerEntry) {
        ...
        Set(index + kGroupsSlotOffset, Get(i + kGroupsSlotOffset), SKIP_WRITE_BARRIER);
        return i;
    }
    return index; // No non-cleared entry found.
}

void DependentCode::DeoptimizeDependencyGroups(Isolate* isolate, DependentCode::DependencyGroups groups)
{
...
// static
Tagged<DependentCode> DependentCode::empty_dependent_code(const ReadOnlyRoots& roots)
{
