...
public:
    ...
    template <typename TypeTag> void* Allocate(size_t size)
    {
    ...
    // associated with the provided TypeTag type.
    template <typename TypeTag = void> void Delete(void* pointer, size_t size)
    {
    ...
    template <typename T, typename TypeTag = T[]> T* AllocateArray(size_t length)
    {
        static_assert(alignof(T) <= kAlignmentInBytes);
    ...
    template <typename T, typename TypeTag = T[]> base::Vector<T> AllocateVector(size_t length)
    {
    ...
    template <typename T, typename TypeTag = T[]> base::Vector<T> NewVector(size_t length)
    {
        T* new_array = AllocateArray<T, TypeTag>(length);
    ...
    // 'value'.
    template <typename T, typename TypeTag = T[]> base::Vector<T> NewVector(size_t length, T value)
    {
        T* new_array = AllocateArray<T, TypeTag>(length);
    ...
    template <typename T, typename TypeTag = std::remove_const_t<T>[]> base::Vector<std::remove_const_t<T>> CloneVector(base::Vector<T> v)
    {
    ...
    // When V8_ENABLE_PRECISE_ZONE_STATS is defined, the deallocated bytes are
    // associated with the provided TypeTag type.
    template <typename T, typename TypeTag = T[]> void DeleteArray(T* pointer, size_t length)
    {
    ...
    {
        return segment_bytes_allocated_;
    }
...
#ifdef V8_ENABLE_PRECISE_ZONE_STATS
    const TypeStats& type_stats() const
    {
        return type_stats_;
    }
#endif
