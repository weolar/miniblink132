...
#if (defined(__clang__) || defined(__GLIBCXX__)) && !defined(_MSC_VER)
        int status = -1;
        char* result = abi::__cxa_demangle(type_id.name(), buffer_, &buffer_len_, &status);
        if (status == 0) {
...
void TypeStats::Dump() const
{
    ...
    for (auto const& item : map_) {
        ...
        PrintF("%12zu | %12zu | %10zu | %6zu | %s\n", entry.allocated_bytes, entry.deallocated_bytes, entry.allocation_count, entry.instance_size,
            d.demangle(item.first));
    }
    PrintF("%12" PRIu64 " | %12" PRIu64 " | %10" PRIu64 " | ===== TOTAL STATS =====\n", total_allocated_bytes, total_deallocated_bytes, total_allocation_count);
}
