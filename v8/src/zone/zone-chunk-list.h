...
template <typename T, bool backwards, bool modifiable> class ZoneChunkListIterator;
...
template <typename T> class ZoneChunkList : public ZoneObject {
public:
    ...
    explicit ZoneChunkList(Zone* zone)
        : zone_(zone)
    {
    }
...
private:
    template <typename S, bool backwards, bool modifiable> friend class ZoneChunkListIterator;
...
template <typename T, bool backwards, bool modifiable> class ZoneChunkListIterator : public base::iterator<std::bidirectional_iterator_tag, T> {
private:
    template <typename S> using maybe_const = typename std::conditional<modifiable, S, typename std::add_const<S>::type>::type;
...
private:
    ...
    static ZoneChunkListIterator Begin(ChunkList* list)
    {
        ...
        return ZoneChunkListIterator(list->last_nonempty_, list->last_nonempty_->position_ - 1);
    }
    ...
    template <bool move_backward> void Move()
    {
        if (move_backward) {
            // Move backwards.
            if (position_ == 0) {
...
template <typename T> T& ZoneChunkList<T>::front()
{
    DCHECK(!empty());
    return *begin();
}

template <typename T> const T& ZoneChunkList<T>::front() const
{
    DCHECK(!empty());
    return *begin();
}

template <typename T> T& ZoneChunkList<T>::back()
{
    DCHECK(!empty());
...
template <typename T> const T& ZoneChunkList<T>::back() const
{
    DCHECK(!empty());
    // Avoid the branch in `ZoneChunkListIterator::Begin()`.
    V8_ASSUME(size_ != 0);
...
template <typename T> void ZoneChunkList<T>::push_back(const T& item)
{
...
template <typename T> void ZoneChunkList<T>::push_front(const T& item)
{
    if (front_ == nullptr) {
        // Initially empty chunk list.
        front_ = NewChunk(kInitialChunkCapacity);
...
template <typename T> typename ZoneChunkList<T>::SeekResult ZoneChunkList<T>::SeekIndex(size_t index) const
{
...
template <typename T> void ZoneChunkList<T>::Rewind(const size_t limit)
{
    if (limit >= size())
        return;
    ...
    for (Chunk* current = seek_result.chunk_->next_; current != nullptr; current = current->next_) {
        current->position_ = 0;
    }
...
template <typename T> typename ZoneChunkList<T>::iterator ZoneChunkList<T>::Find(const size_t index)
{
    SeekResult seek_result = SeekIndex(index);
    return typename ZoneChunkList<T>::iterator(seek_result.chunk_, seek_result.chunk_index_);
}

template <typename T> typename ZoneChunkList<T>::const_iterator ZoneChunkList<T>::Find(const size_t index) const
{
    SeekResult seek_result = SeekIndex(index);
    return typename ZoneChunkList<T>::const_iterator(seek_result.chunk_, seek_result.chunk_index_);
}

template <typename T> ZoneChunkList<T> ZoneChunkList<T>::SplitAt(iterator split_begin)
{
    ...
    uint32_t new_chunk_size = static_cast<uint32_t>(chunk_split_end - chunk_split_begin);
    uint32_t new_chunk_capacity = std::max(kInitialChunkCapacity, base::bits::RoundUpToPowerOfTwo32(new_chunk_size));
    ...
    result.last_nonempty_ = (last_nonempty_ == split_chunk) ? new_chunk : last_nonempty_;
...
template <typename T> void ZoneChunkList<T>::Append(ZoneChunkList<T>& other)
{
    DCHECK_EQ(zone_, other.zone_);
...
template <typename T> void ZoneChunkList<T>::CopyTo(T* ptr)
{
