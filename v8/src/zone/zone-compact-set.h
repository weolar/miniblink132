...
template <typename T, typename Enable = void> struct ZoneCompactSetTraits;

template <typename T> struct ZoneCompactSetTraits<Handle<T>> {
...
template <typename T> class ZoneCompactSet final {
...
public:
    ...
    template <class It, typename = typename std::iterator_traits<It>::iterator_category> explicit ZoneCompactSet(It first, It last, Zone* zone)
    {
        auto size = last - first;
        if (size == 0) {
            data_ = EmptyValue();
        } else if (size == 1) {
            data_ = PointerWithPayload(Traits::HandleToPointer(*first), kSingletonTag);
        } else {
    ...
    void insert(T handle, Zone* zone)
    {
        ...
        if (is_empty()) {
            ...
            auto it = std::lower_bound(current_list->begin(), current_list->end(), value);
    ...
    void remove(T handle, Zone* zone)
    {
        ...
        const List* current_list = list();
        auto found_it = std::lower_bound(current_list->begin(), current_list->end(), pointer);
    ...
    friend bool operator==(ZoneCompactSet<T> const& lhs, ZoneCompactSet<T> const& rhs)
    {
        ...
        if (lhs.is_list() && rhs.is_list()) {
            ...
            return std::equal(lhs_list->begin(), lhs_list->end(), rhs_list->begin(), rhs_list->end());
        }
        return false;
    }
...
private:
    ...
    {
        return data_.GetPayload() == kSingletonTag;
    }
    bool is_list() const
    {
        return data_.GetPayload() == kListTag;
    }
    ...
    {
        return static_cast<data_type*>(data_.GetPointerWithKnownPayload(kSingletonTag));
    }
...
template <typename T> std::ostream& operator<<(std::ostream& os, ZoneCompactSet<T> set)
{
...
template <typename T> class ZoneCompactSet<T>::const_iterator {
public:
...
template <typename T> typename ZoneCompactSet<T>::const_iterator ZoneCompactSet<T>::begin() const
{
    return ZoneCompactSet<T>::const_iterator(this, 0);
}

template <typename T> typename ZoneCompactSet<T>::const_iterator ZoneCompactSet<T>::end() const
{
    return ZoneCompactSet<T>::const_iterator(this, size());
}

template <typename T> using ZoneHandleSet = ZoneCompactSet<Handle<T>>;
