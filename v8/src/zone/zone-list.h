...
template <typename T> class ZoneList final : public ZoneObject {
public:
    ...
    {
        return base::Vector<T>(data_, length_);
    }
    ...
    template <class Visitor> void Iterate(Visitor* visitor);

    // Sort all list entries (using QuickSort)
    template <typename CompareFunction> void Sort(CompareFunction cmp);
    template <typename CompareFunction> void StableSort(CompareFunction cmp, size_t start, size_t length);
