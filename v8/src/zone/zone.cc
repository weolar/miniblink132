...
Zone::Zone(AccountingAllocator* allocator, const char* name, bool support_compression)
...
void* Zone::AsanNew(size_t size)
{
    ...
    ASAN_POISON_MEMORY_REGION(reinterpret_cast<void*>(redzone_position), kASanRedzoneBytes);
...
void Zone::Reset()
{
    ...
    ASAN_UNPOISON_MEMORY_REGION(reinterpret_cast<void*>(keep->start()), keep->capacity());
    keep->ZapContents();
...
bool Zone::Contains(const void* ptr) const
{
    ...
    for (Segment* segment = segment_head_; segment != nullptr; segment = segment->next()) {
...
void Zone::ReleaseSegment(Segment* segment)
{
    ...
    ASAN_UNPOISON_MEMORY_REGION(reinterpret_cast<void*>(segment->start()), segment->capacity());
...
void Zone::Expand(size_t size)
{
    ...
    Segment* segment = allocator_->AllocateSegment(new_size, supports_compression());
    if (segment == nullptr) {
        V8::FatalProcessOutOfMemory(nullptr, "Zone");
    }
