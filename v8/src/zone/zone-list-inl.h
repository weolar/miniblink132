...
template <typename T> void ZoneList<T>::Add(const T& element, Zone* zone)
{
...
template <typename T> void ZoneList<T>::AddAll(const ZoneList<T>& other, Zone* zone)
{
    AddAll(other.ToVector(), zone);
}

template <typename T> void ZoneList<T>::AddAll(base::Vector<const T> other, Zone* zone)
{
...
template <typename T> void ZoneList<T>::ResizeAdd(const T& element, Zone* zone)
{
    ResizeAddInternal(element, zone);
}

template <typename T> void ZoneList<T>::ResizeAddInternal(const T& element, Zone* zone)
{
...
template <typename T> void ZoneList<T>::Resize(int new_capacity, Zone* zone)
{
    ...
    if (data_)
        zone->DeleteArray<T>(data_, capacity_);
    data_ = new_data;
    capacity_ = new_capacity;
}

template <typename T> base::Vector<T> ZoneList<T>::AddBlock(T value, int count, Zone* zone)
{
    int start = length_;
...
template <typename T> void ZoneList<T>::Set(int index, const T& elm)
{
    DCHECK(index >= 0 && index <= length_);
    data_[index] = elm;
}

template <typename T> void ZoneList<T>::InsertAt(int index, const T& elm, Zone* zone)
{
    DCHECK(index >= 0 && index <= length_);
    Add(elm, zone);
...
template <typename T> T ZoneList<T>::Remove(int i)
{
    T element = at(i);
    length_--;
    while (i < length_) {
...
template <typename T> void ZoneList<T>::Clear(Zone* zone)
{
    if (data_)
        zone->DeleteArray<T>(data_, capacity_);
    DropAndClear();
...
template <typename T> void ZoneList<T>::Rewind(int pos)
{
    DCHECK(0 <= pos && pos <= length_);
    length_ = pos;
}

template <typename T> template <class Visitor> void ZoneList<T>::Iterate(Visitor* visitor)
{
