...
public:
    ...
    JsonString(int start, int length, bool needs_conversion, bool internalize, bool has_escape)
        : start_(start)
        , length_(length)
...
public:
    static MaybeHandle<Object> Internalize(
...
private:
    ...
    MaybeHandle<Object> InternalizeJsonProperty(Handle<JSReceiver> holder, Handle<String> key, Handle<Object> val_node, Handle<Object> snapshot);
...
template <typename Char> class JsonParser final {
public:
    using SeqString = typename CharTraits<Char>::String;
    using SeqExternalString = typename CharTraits<Char>::ExternalString;

    V8_WARN_UNUSED_RESULT static bool CheckRawJson(Isolate* isolate, Handle<String> source)
    {
        return JsonParser(isolate, source).ParseRawJson();
    }

    V8_WARN_UNUSED_RESULT static MaybeHandle<Object> Parse(Isolate* isolate, Handle<String> source, Handle<Object> reviver)
    {
        ...
        if (IsCallable(*reviver)) {
            return JsonParseInternalizer::Internalize(isolate, result, reviver, source, val_node);
        }
        return result;
    }

    static constexpr base::uc32 kEndOfString = static_cast<base::uc32>(-1);
    static constexpr base::uc32 kInvalidUnicodeCharacter = static_cast<base::uc32>(-1);

private:
    class NamedPropertyIterator;

    template <typename T> using SmallVector = base::SmallVector<T, 16>;
    ...
    void Expect(JsonToken token, std::optional<MessageTemplate> errorMessage = std::nullopt)
    {
        if (V8_LIKELY(peek() == token)) {
            ...
            errorMessage ? ReportUnexpectedToken(peek(), errorMessage.value()) : ReportUnexpectedToken(peek());
        }
    }
    ...
    template <size_t N> void ScanLiteral(const char (&s)[N])
    {
        DCHECK(!is_at_end());
        ...
        if (V8_LIKELY(remaining >= N - 1 && CompareCharsEqual(s + 1, cursor_ + 1, N - 2))) {
            cursor_ += N - 1;
            return;
        }
    ...
    template <typename SinkChar> void DecodeString(SinkChar* sink, int start, int length);

    template <typename SinkSeqString> Handle<String> DecodeString(const JsonString& string, Handle<SinkSeqString> intermediate, Handle<String> hint);
    ...
    template <bool should_track_json_source> MaybeHandle<Object> ParseJsonValue();
    ...
    template <bool should_track_json_source> Handle<JSObject> BuildJsonObject(const JsonContinuation& cont, Handle<Map> feedback);
    Handle<Object> BuildJsonArray(size_t start);

    static const int kMaxContextCharacters = 10;
    static const int kMinOriginalSourceLengthForContext = (kMaxContextCharacters * 2) + 1;
    ...
    inline ReadOnlyRoots roots()
    {
        return ReadOnlyRoots(isolate_);
    }
    inline Handle<JSFunction> object_constructor()
    {
...
private:
    ...
    int position() const
    {
        return static_cast<int>(cursor_ - chars_);
    }
