...
public:
    ...
    template <typename Char> static bool Parse(Isolate* isolate, base::Vector<Char> str, double* output);
...
private:
    ...
    template <typename Char> class InputReader {
    public:
        explicit InputReader(base::Vector<Char> s)
            : index_(0)
            , buffer_(s)
        {
            Next();
    ...
    template <typename Char> class DateStringTokenizer {
    public:
