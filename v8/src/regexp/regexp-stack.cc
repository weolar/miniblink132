...
void RegExpStack::ThreadLocal::ResetToStaticStack(RegExpStack* regexp_stack)
{
    ...
    limit_ = reinterpret_cast<Address>(regexp_stack->static_stack_) + kStackLimitSlackSize;
    owns_memory_ = false;
}
...
Address RegExpStack::EnsureCapacity(size_t size)
{
    ...
    if (thread_local_.memory_size_ < size) {
        if (size < kMinimumDynamicStackSize)
            size = kMinimumDynamicStackSize;
        uint8_t* new_memory = NewArray<uint8_t>(size);
        if (thread_local_.memory_size_ > 0) {
            // Copy original memory into top of new memory.
            MemCopy(new_memory + size - thread_local_.memory_size_, thread_local_.memory_, thread_local_.memory_size_);
            if (thread_local_.owns_memory_)
        ...
        thread_local_.limit_ = reinterpret_cast<Address>(new_memory) + kStackLimitSlackSize;
