...
public:
    ...
    static constexpr int kStackLimitSlackSize = kStackLimitSlackSlotCount * kSlotSize;
    ...
    Address end() const
    {
        ...
        DCHECK_EQ(thread_local_.memory_top_, thread_local_.memory_ + thread_local_.memory_size_);
    ...
    {
        thread_local_.ResetToStaticStack(this);
    }
...
private:
    ...
    void ResetIfEmpty()
    {
        thread_local_.ResetToStaticStackIfEmpty(this);
    }

    // Whether the ThreadLocal storage has been invalidated.
    bool IsValid() const
    {
        return thread_local_.memory_ != nullptr;
    }
