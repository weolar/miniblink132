...
public:
    ...
    static inline ZoneList<CharacterRange>* List(Zone* zone, CharacterRange range)
    {
        ZoneList<CharacterRange>* list = zone->New<ZoneList<CharacterRange>>(1, zone);
        list->Add(range, zone);
        return list;
    }
    ...
    V8_EXPORT_PRIVATE static void AddCaseEquivalents(Isolate* isolate, Zone* zone, ZoneList<CharacterRange>* ranges, bool is_one_byte);
    ...
    V8_EXPORT_PRIVATE static bool IsCanonical(const ZoneList<CharacterRange>* ranges);
...
public:
    ...
    explicit RegExpAssertion(Type type)
...
public:
    ...
    {
        return standard_set_type_.has_value();
    }
...
public:
    ...
    RegExpClassRanges(Zone* zone, ZoneList<CharacterRange>* ranges, ClassRangesFlags class_ranges_flags = ClassRangesFlags())
        : set_(ranges)
        , class_ranges_flags_(class_ranges_flags)
    {
    ...
    ZoneList<CharacterRange>* ranges(Zone* zone)
    {
        return set_.ranges(zone);
    }

    bool is_negated() const
    {
        return (class_ranges_flags_ & NEGATED) != 0;
    }
...
using CharacterClassStrings = ZoneMap<base::Vector<const base::uc32>, RegExpTree*, CharacterClassStringLess>;
...
public:
    ...
    {
        return strings_ != nullptr && !strings_->empty();
    }
...
public:
    ...
    class Builder {
    public:
        Builder(bool is_positive, RegExpNode* on_success, int stack_pointer_register, int position_register, int capture_register_count = 0,
...
public:
    explicit RegExpBackReference(Zone* zone)
        : captures_(1, zone)
    {
    }
