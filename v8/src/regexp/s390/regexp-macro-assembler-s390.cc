...
RegExpMacroAssemblerS390::RegExpMacroAssemblerS390(Isolate* isolate, Zone* zone, Mode mode, int registers_to_save)
    : NativeRegExpMacroAssembler(isolate, zone)
    , masm_(std::make_unique<MacroAssembler>(isolate, CodeObjectRequired::kYes, NewAssemblerBuffer(kRegExpCodeSize)))
...
void RegExpMacroAssemblerS390::CheckCharacterGT(base::uc16 limit, Label* on_greater)
{
...
void RegExpMacroAssemblerS390::CheckAtStart(int cp_offset, Label* on_at_start)
{
    ...
    __ AddS64(r2, current_input_offset(), Operand(-char_size() + cp_offset * char_size()));
    __ CmpS64(r2, r3);
    BranchOrBacktrack(eq, on_at_start);
}

void RegExpMacroAssemblerS390::CheckNotAtStart(int cp_offset, Label* on_not_at_start)
{
    __ LoadU64(r3, MemOperand(frame_pointer(), kStringStartMinusOneOffset));
    __ AddS64(r2, current_input_offset(), Operand(-char_size() + cp_offset * char_size()));
    __ CmpS64(r2, r3);
...
void RegExpMacroAssemblerS390::CheckCharacterLT(base::uc16 limit, Label* on_less)
{
    __ CmpU64(current_character(), Operand(limit));
...
void RegExpMacroAssemblerS390::CheckNotBackReferenceIgnoreCase(int start_reg, bool read_backward, bool unicode, Label* on_no_match)
{
    Label fallthrough;
...
void RegExpMacroAssemblerS390::CheckNotBackReference(int start_reg, bool read_backward, Label* on_no_match)
{
    Label fallthrough;
...
void RegExpMacroAssemblerS390::CheckNotCharacter(unsigned c, Label* on_not_equal)
{
    __ CmpU64(current_character(), Operand(c));
    BranchOrBacktrack(ne, on_not_equal);
}

void RegExpMacroAssemblerS390::CheckCharacterAfterAnd(uint32_t c, uint32_t mask, Label* on_equal)
{
...
void RegExpMacroAssemblerS390::CheckNotCharacterAfterAnd(unsigned c, unsigned mask, Label* on_not_equal)
{
    __ AndP(r2, current_character(), Operand(mask));
    if (c != 0) {
        __ CmpU64(r2, Operand(c));
...
void RegExpMacroAssemblerS390::CheckNotCharacterAfterMinusAnd(base::uc16 c, base::uc16 minus, base::uc16 mask, Label* on_not_equal)
{
...
void RegExpMacroAssemblerS390::CheckCharacterInRange(base::uc16 from, base::uc16 to, Label* on_in_range)
{
...
void RegExpMacroAssemblerS390::CheckCharacterNotInRange(base::uc16 from, base::uc16 to, Label* on_not_in_range)
{
    __ lay(r2, MemOperand(current_character(), -from));
    __ CmpU64(r2, Operand(to - from));
...
void RegExpMacroAssemblerS390::CallIsCharacterInRangeArray(const ZoneList<CharacterRange>* ranges)
{
...
bool RegExpMacroAssemblerS390::CheckCharacterInRangeArray(const ZoneList<CharacterRange>* ranges, Label* on_in_range)
{
...
bool RegExpMacroAssemblerS390::CheckCharacterNotInRangeArray(const ZoneList<CharacterRange>* ranges, Label* on_not_in_range)
{
    CallIsCharacterInRangeArray(ranges);
    __ CmpS64(r2, Operand::Zero());
...
void RegExpMacroAssemblerS390::CheckBitInTable(Handle<ByteArray> table, Label* on_bit_set)
{
    ...
    __ LoadU8(r2, MemOperand(r2, index, (OFFSET_OF_DATA_START(ByteArray) - kHeapObjectTag)));
    __ CmpS64(r2, Operand::Zero());
    BranchOrBacktrack(ne, on_bit_set);
}

void RegExpMacroAssemblerS390::SkipUntilBitInTable(int cp_offset, Handle<ByteArray> table, Handle<ByteArray> nibble_table, int advance_by)
{
...
bool RegExpMacroAssemblerS390::CheckSpecialClassRanges(StandardCharacterSet type, Label* on_no_match)
{
...
void RegExpMacroAssemblerS390::StoreRegExpStackPointerToMemory(Register src, Register scratch)
{
    ExternalReference ref = ExternalReference::address_of_regexp_stack_stack_pointer(isolate());
...
void RegExpMacroAssemblerS390::PushRegExpBasePointer(Register stack_pointer, Register scratch)
{
...
void RegExpMacroAssemblerS390::PopRegExpBasePointer(Register stack_pointer_out, Register scratch)
{
    ExternalReference ref = ExternalReference::address_of_regexp_stack_memory_top_address(isolate());
...
Handle<HeapObject> RegExpMacroAssemblerS390::GetCode(Handle<String> source, RegExpFlags flags)
{
    Label return_r2;
    ...
    __ LoadMultipleP(r5, r10, MemOperand(r4, (12 * kSystemPointerSize) + kStackPointerBias + (kXPLINKStackFrameExtraParamSlot * kSystemPointerSize)));
    ...
    __ LoadMultipleP(r7, r9, MemOperand(sp, kStackFrameExtraParamSlot * kSystemPointerSize));
#endif
        ...
        if (num_saved_registers_ > 0) {
            ...
            for (int i = 0; i < num_saved_registers_;) {
                if ((false) && i < num_saved_registers_ - 4) {
                    ...
                    __ AddS64(r5, r0);
                    __ AddS64(r6, r0);
                    __ StoreU32(r3, MemOperand(r2, -(num_saved_registers_ - i - 3) * kIntSize));
                    __ StoreU32(r4, MemOperand(r2, -(num_saved_registers_ - i - 2) * kIntSize));
                    __ StoreU32(r5, MemOperand(r2, -(num_saved_registers_ - i - 1) * kIntSize));
                    __ StoreU32(r6, MemOperand(r2, -(num_saved_registers_ - i) * kIntSize));
                    i += 4;
                } else {
                    ...
                    __ AddS64(r3, r0);
                    __ AddS64(r4, r0);
                    __ StoreU32(r3, MemOperand(r2, -(num_saved_registers_ - i - 1) * kIntSize));
                    __ StoreU32(r4, MemOperand(r2, -(num_saved_registers_ - i) * kIntSize));
                    i += 2;
                }
            }
...
void RegExpMacroAssemblerS390::IfRegisterGE(int reg, int comparand, Label* if_ge)
{
    __ LoadU64(r2, register_location(reg), r0);
...
void RegExpMacroAssemblerS390::IfRegisterLT(int reg, int comparand, Label* if_lt)
{
    __ LoadU64(r2, register_location(reg), r0);
    __ CmpS64(r2, Operand(comparand));
...
RegExpMacroAssembler::IrregexpImplementation RegExpMacroAssemblerS390::Implementation()
{
...
void RegExpMacroAssemblerS390::PushBacktrack(Label* label)
{
    if (label->is_bound()) {
        ...
        __ mov(r2, Operand(target + InstructionStream::kHeaderSize - kHeapObjectTag));
    } else {
...
void RegExpMacroAssemblerS390::PushRegister(int register_index, StackCheckFlag check_stack_limit)
{
...
void RegExpMacroAssemblerS390::WriteCurrentPositionToRegister(int reg, int cp_offset)
{
    if (cp_offset == 0) {
...
void RegExpMacroAssemblerS390::CallCheckStackGuardState(Register scratch, Operand extra_space)
{
    ...
    ExternalReference stack_guard_check = ExternalReference::re_check_stack_guard_state();
    ...
    __ lay(r4, MemOperand(sp, -((stack_slots * kSystemPointerSize) + kStackPointerBias)));
    __ StoreMultipleP(r5, r6, MemOperand(r4, kStackPointerBias + kXPLINKStackFrameExtraParamSlot * kSystemPointerSize));
    ...
    __ LoadMultipleP(r5, r6, MemOperand(r4, kStackPointerBias + kXPLINKStackFrameExtraParamSlot * kSystemPointerSize));
    ...
    if (base::OS::ActivationFrameAlignment() > kSystemPointerSize) {
        __ LoadU64(sp, MemOperand(sp, (kNumRequiredStackFrameSlots * kSystemPointerSize)));
    } else {
        __ la(sp, MemOperand(sp, (kNumRequiredStackFrameSlots * kSystemPointerSize)));
    }
...
int RegExpMacroAssemblerS390::CheckStackGuardState(Address* return_address, Address raw_code, Address re_frame, uintptr_t extra_space)
{
    Tagged<InstructionStream> re_code = Cast<InstructionStream>(Tagged<Object>(raw_code));
    return NativeRegExpMacroAssembler::CheckStackGuardState(frame_entry<Isolate*>(re_frame, kIsolateOffset), frame_entry<intptr_t>(re_frame, kStartIndexOffset),
        static_cast<RegExp::CallOrigin>(frame_entry<intptr_t>(re_frame, kDirectCallOffset)), return_address, re_code,
        frame_entry_address<Address>(re_frame, kInputStringOffset), frame_entry_address<const uint8_t*>(re_frame, kInputStartOffset),
        frame_entry_address<const uint8_t*>(re_frame, kInputEndOffset), extra_space);
}
...
void RegExpMacroAssemblerS390::CheckPosition(int cp_offset, Label* on_outside_input)
{
    if (cp_offset >= 0) {
...
void RegExpMacroAssemblerS390::BranchOrBacktrack(Condition condition, Label* to, CRegister cr)
{
...
void RegExpMacroAssemblerS390::SafeCall(Label* to, Condition cond, CRegister cr)
{
    Label skip;
...
void RegExpMacroAssemblerS390::Push(Register source)
{
    ...
    __ lay(backtrack_stackpointer(), MemOperand(backtrack_stackpointer(), -kSystemPointerSize));
...
void RegExpMacroAssemblerS390::Pop(Register target)
{
    ...
    __ la(backtrack_stackpointer(), MemOperand(backtrack_stackpointer(), kSystemPointerSize));
}
...
void RegExpMacroAssemblerS390::CallCFunctionFromIrregexpCode(ExternalReference function, int num_arguments)
{
    ...
    __ CallCFunction(function, num_arguments, ABI_USES_FUNCTION_DESCRIPTORS, SetIsolateDataSlots::kNo);
}
...
void RegExpMacroAssemblerS390::CallCFunctionUsingStub(ExternalReference function, int num_arguments)
{
...
void RegExpMacroAssemblerS390::LoadCurrentCharacterUnchecked(int cp_offset, int characters)
{
    if (mode_ == LATIN1) {
        // using load reverse for big-endian platforms
        if (characters == 4) {
            __ LoadU32LE(current_character(), MemOperand(current_input_offset(), end_of_input_address(), cp_offset * char_size()));
        } else if (characters == 2) {
            __ LoadU16LE(current_character(), MemOperand(current_input_offset(), end_of_input_address(), cp_offset * char_size()));
        } else {
            DCHECK_EQ(1, characters);
            __ LoadU8(current_character(), MemOperand(current_input_offset(), end_of_input_address(), cp_offset * char_size()));
        }
    } else {
        DCHECK(mode_ == UC16);
        if (characters == 2) {
            __ LoadU32(current_character(), MemOperand(current_input_offset(), end_of_input_address(), cp_offset * char_size()));
            ...
            DCHECK_EQ(1, characters);
            __ LoadU16(current_character(), MemOperand(current_input_offset(), end_of_input_address(), cp_offset * char_size()));
        }
    }
}
