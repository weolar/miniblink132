...
void RegExpMacroAssemblerPPC::AdvanceCurrentPosition(int by)
{
    if (by != 0) {
        if (is_int16(by * char_size())) {
            __ addi(current_input_offset(), current_input_offset(), Operand(by * char_size()));
        } else {
...
void RegExpMacroAssemblerPPC::CheckCharacterGT(base::uc16 limit, Label* on_greater)
{
...
void RegExpMacroAssemblerPPC::CheckAtStart(int cp_offset, Label* on_at_start)
{
    ...
    __ addi(r3, current_input_offset(), Operand(-char_size() + cp_offset * char_size()));
    __ CmpS64(r3, r4);
    BranchOrBacktrack(eq, on_at_start);
}

void RegExpMacroAssemblerPPC::CheckNotAtStart(int cp_offset, Label* on_not_at_start)
{
    __ LoadU64(r4, MemOperand(frame_pointer(), kStringStartMinusOneOffset));
    __ addi(r3, current_input_offset(), Operand(-char_size() + cp_offset * char_size()));
    __ CmpS64(r3, r4);
...
void RegExpMacroAssemblerPPC::CheckCharacterLT(base::uc16 limit, Label* on_less)
{
    __ CmpU64(current_character(), Operand(limit), r0);
...
void RegExpMacroAssemblerPPC::CheckGreedyLoop(Label* on_equal)
{
    ...
    __ addi(backtrack_stackpointer(), backtrack_stackpointer(), Operand(kSystemPointerSize));

    __ bind(&backtrack_non_equal);
    BranchOrBacktrack(eq, on_equal);
}

void RegExpMacroAssemblerPPC::CheckNotBackReferenceIgnoreCase(int start_reg, bool read_backward, bool unicode, Label* on_no_match)
{
    Label fallthrough;
...
void RegExpMacroAssemblerPPC::CheckNotBackReference(int start_reg, bool read_backward, Label* on_no_match)
{
    Label fallthrough;
...
void RegExpMacroAssemblerPPC::CheckNotCharacter(unsigned c, Label* on_not_equal)
{
    __ CmpU64(current_character(), Operand(c), r0);
    BranchOrBacktrack(ne, on_not_equal);
}

void RegExpMacroAssemblerPPC::CheckCharacterAfterAnd(uint32_t c, uint32_t mask, Label* on_equal)
{
...
void RegExpMacroAssemblerPPC::CheckNotCharacterAfterAnd(unsigned c, unsigned mask, Label* on_not_equal)
{
    __ mov(r0, Operand(mask));
    if (c == 0) {
        __ and_(r3, current_character(), r0, SetRC);
...
void RegExpMacroAssemblerPPC::CheckNotCharacterAfterMinusAnd(base::uc16 c, base::uc16 minus, base::uc16 mask, Label* on_not_equal)
{
...
void RegExpMacroAssemblerPPC::CheckCharacterInRange(base::uc16 from, base::uc16 to, Label* on_in_range)
{
...
void RegExpMacroAssemblerPPC::CheckCharacterNotInRange(base::uc16 from, base::uc16 to, Label* on_not_in_range)
{
    __ mov(r0, Operand(from));
    __ sub(r3, current_character(), r0);
    __ CmpU64(r3, Operand(to - from), r0);
...
void RegExpMacroAssemblerPPC::CallIsCharacterInRangeArray(const ZoneList<CharacterRange>* ranges)
{
...
bool RegExpMacroAssemblerPPC::CheckCharacterInRangeArray(const ZoneList<CharacterRange>* ranges, Label* on_in_range)
{
...
bool RegExpMacroAssemblerPPC::CheckCharacterNotInRangeArray(const ZoneList<CharacterRange>* ranges, Label* on_not_in_range)
{
    CallIsCharacterInRangeArray(ranges);
    __ cmpi(r3, Operand::Zero());
...
void RegExpMacroAssemblerPPC::CheckBitInTable(Handle<ByteArray> table, Label* on_bit_set)
{
    ...
    if (mode_ != LATIN1 || kTableMask != String::kMaxOneByteCharCode) {
        ...
        __ addi(r4, current_character(), Operand(OFFSET_OF_DATA_START(ByteArray) - kHeapObjectTag));
    }
...
void RegExpMacroAssemblerPPC::SkipUntilBitInTable(int cp_offset, Handle<ByteArray> table, Handle<ByteArray> nibble_table, int advance_by)
{
...
bool RegExpMacroAssemblerPPC::CheckSpecialClassRanges(StandardCharacterSet type, Label* on_no_match)
{
...
void RegExpMacroAssemblerPPC::StoreRegExpStackPointerToMemory(Register src, Register scratch)
{
    ExternalReference ref = ExternalReference::address_of_regexp_stack_stack_pointer(isolate());
...
void RegExpMacroAssemblerPPC::PushRegExpBasePointer(Register stack_pointer, Register scratch)
{
...
void RegExpMacroAssemblerPPC::PopRegExpBasePointer(Register stack_pointer_out, Register scratch)
{
    ExternalReference ref = ExternalReference::address_of_regexp_stack_memory_top_address(isolate());
...
Handle<HeapObject> RegExpMacroAssemblerPPC::GetCode(Handle<String> source, RegExpFlags flags)
{
    Label return_r3;
        ...
        if (success_label_.is_linked()) {
            ...
            if (global()) {
                ...
                if (global_with_zero_length_check()) {
                    ...
                    Label advance;
                    __ bind(&advance);
                    __ addi(current_input_offset(), current_input_offset(), Operand((mode_ == UC16) ? 2 : 1));
                    if (global_unicode())
...
void RegExpMacroAssemblerPPC::IfRegisterGE(int reg, int comparand, Label* if_ge)
{
    __ LoadU64(r3, register_location(reg), r0);
...
void RegExpMacroAssemblerPPC::IfRegisterLT(int reg, int comparand, Label* if_lt)
{
    __ LoadU64(r3, register_location(reg), r0);
    __ CmpS64(r3, Operand(comparand), r0);
...
RegExpMacroAssembler::IrregexpImplementation RegExpMacroAssemblerPPC::Implementation()
{
...
void RegExpMacroAssemblerPPC::PushRegister(int register_index, StackCheckFlag check_stack_limit)
{
...
void RegExpMacroAssemblerPPC::WriteCurrentPositionToRegister(int reg, int cp_offset)
{
    if (cp_offset == 0) {
...
void RegExpMacroAssemblerPPC::CallCheckStackGuardState(Register scratch, Operand extra_space)
{
    ...
    if (frame_alignment > kSystemPointerSize) {
        ...
        __ mr(scratch, sp);
        __ addi(sp, sp, Operand(-(stack_passed_arguments + 1) * kSystemPointerSize));
        DCHECK(base::bits::IsPowerOfTwo(frame_alignment));
        __ ClearRightImm(sp, sp, Operand(base::bits::WhichPowerOfTwo(frame_alignment)));
        __ StoreU64(scratch, MemOperand(sp, stack_passed_arguments * kSystemPointerSize));
    } else {
...
int RegExpMacroAssemblerPPC::CheckStackGuardState(Address* return_address, Address raw_code, Address re_frame, uintptr_t extra_space)
{
...
void RegExpMacroAssemblerPPC::CheckPosition(int cp_offset, Label* on_outside_input)
{
    if (cp_offset >= 0) {
...
void RegExpMacroAssemblerPPC::BranchOrBacktrack(Condition condition, Label* to, CRegister cr)
{
...
void RegExpMacroAssemblerPPC::Push(Register source)
{
    ...
    __ StoreU64WithUpdate(source, MemOperand(backtrack_stackpointer(), -kSystemPointerSize));
}
...
void RegExpMacroAssemblerPPC::LoadCurrentCharacterUnchecked(int cp_offset, int characters)
{
