...
private:
    explicit CanBeHandledVisitor(RegExpFlags flags)
        : flags_(flags)
    {
    }
    ...
    static bool AreSuitableFlags(RegExpFlags flags)
    {
        ...
        static constexpr RegExpFlags kAllowedFlags
            = RegExpFlag::kGlobal | RegExpFlag::kSticky | RegExpFlag::kMultiline | RegExpFlag::kDotAll | RegExpFlag::kLinear;
        // We support Unicode iff kUnicode is among the supported flags.
        static_assert(ExperimentalRegExp::kSupportsUnicode == IsUnicode(kAllowedFlags));
    ...
    void* VisitClassSetExpression(RegExpClassSetExpression* node, void*) override
    {
        result_ = false;
        return nullptr;
    }
    ...
    void* VisitQuantifier(RegExpQuantifier* node, void*) override
    {
        ...
        if (node->min() > kMaxReplicationFactor || (node->max() != RegExpTree::kInfinity && node->max() > kMaxReplicationFactor)) {
            result_ = false;
            return nullptr;
        }
    ...
    void* VisitLookaround(RegExpLookaround* node, void*) override
    {
        ...
        // flags.
        if (node->type() == RegExpLookaround::Type::LOOKAHEAD || IsGlobal(flags()) || IsSticky(flags())) {
            result_ = false;
        } else {
            node->body()->Accept(this, nullptr);
        }
    ...
    void* VisitEmpty(RegExpEmpty* node, void*) override
    {
        return nullptr;
    }
...
bool ExperimentalRegExpCompiler::CanBeHandled(RegExpTree* tree, RegExpFlags flags, int capture_count)
{
...
public:
    ...
    void Accept()
    {
        code_.Add(RegExpInstruction::Accept(), zone_);
    }
    ...
    void Bind(Label& target)
    {
        ...
        while (target.unbound_patch_list_begin_ != -1) {
            ...
            DCHECK(inst.opcode == RegExpInstruction::FORK || inst.opcode == RegExpInstruction::JMP || inst.opcode == RegExpInstruction::FILTER_CHILD);
...
public:
    static void CompileFilter(Zone* zone, RegExpTree* tree, BytecodeAssembler& assembler, const ZoneMap<int, int>& quantifier_id_remapping)
    {
...
private:
    FilterGroupsCompileVisitor(BytecodeAssembler& assembler, Zone* zone, const ZoneMap<int, int>& quantifier_id_remapping)
        : zone_(zone)
...
private:
    ...
    class BFEntry {
    public:
        explicit BFEntry(RegExpTree* node)
            : label()
            , node(node)
        {
        }
...
public:
    static ZoneList<RegExpInstruction> Compile(RegExpTree* tree, RegExpFlags flags, Zone* zone)
    {
        ...
        if (!IsSticky(flags) && !tree->IsAnchoredAtStart()) {
            ...
            compiler.CompileNonGreedyStar([&]() { compiler.assembler_.ConsumeAnyChar(); });
        }
        ...
        if (v8_flags.experimental_regexp_engine_capture_group_opt) {
            FilterGroupsCompileVisitor::CompileFilter(zone, tree, compiler.assembler_, compiler.quantifier_id_remapping_.value());
        }
...
private:
    ...
    void* VisitDisjunction(RegExpDisjunction* node, void*) override
    {
        ...
        CompileDisjunction(alts.length(), [&](int i) { alts[i]->Accept(this, nullptr); });
        return nullptr;
    }
    ...
    void CompileCharacterRanges(ZoneList<CharacterRange>* ranges, bool negated)
    {
        ...
        if (negated) {
            ...
            ZoneList<CharacterRange>* negated = zone_->New<ZoneList<CharacterRange>>(ranges->length() + 1, zone_);
        ...
        CompileDisjunction(ranges->length(), [&](int i) {
            ...
            static_assert(kMaxSupportedCodepoint <= std::numeric_limits<base::uc16>::max());
            ...
            base::uc16 to_uc16 = static_cast<base::uc16>(std::min(to, kMaxSupportedCodepoint));
    ...
    template <class F> void CompileGreedyRepetition(F&& emit_body, int max_repetition_num)
    {
        // This is compiled into
        //
        //     FORK end
    ...
    // Emit bytecode corresponding to /<emit_body>{0, max_repetition_num}?/.
    template <class F> void CompileNonGreedyRepetition(F&& emit_body, int max_repetition_num)
    {
        // This is compiled into
        //
        //     FORK body0
    ...
    void* VisitQuantifier(RegExpQuantifier* node, void*) override
    {
        ...
        if (v8_flags.experimental_regexp_engine_capture_group_opt && node->max() == 0) {
            if (!node->CaptureRegisters().is_empty()) {
        ...
        bool can_be_reduced_to_non_nullable_plus = node->min() > 0 && node->max() == RegExpTree::kInfinity && node->min_match() > 0;
...
ZoneList<RegExpInstruction> ExperimentalRegExpCompiler::Compile(RegExpTree* tree, RegExpFlags flags, Zone* zone)
{
