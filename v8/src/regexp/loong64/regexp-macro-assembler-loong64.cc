...
RegExpMacroAssemblerLOONG64::RegExpMacroAssemblerLOONG64(Isolate* isolate, Zone* zone, Mode mode, int registers_to_save)
    ...
    , masm_(std::make_unique<MacroAssembler>(isolate, CodeObjectRequired::kYes, NewAssemblerBuffer(kInitialBufferSize)))
...
{
    if (by != 0) {
        __ Add_d(current_input_offset(), current_input_offset(), Operand(by * char_size()));
    }
}
...
void RegExpMacroAssemblerLOONG64::Bind(Label* label)
{
    __ bind(label);
}
...
void RegExpMacroAssemblerLOONG64::CheckCharacterGT(base::uc16 limit, Label* on_greater)
{
...
void RegExpMacroAssemblerLOONG64::CheckAtStart(int cp_offset, Label* on_at_start)
{
    ...
    __ Add_d(a0, current_input_offset(), Operand(-char_size() + cp_offset * char_size()));
    BranchOrBacktrack(on_at_start, eq, a0, Operand(a1));
}

void RegExpMacroAssemblerLOONG64::CheckNotAtStart(int cp_offset, Label* on_not_at_start)
{
    __ Ld_d(a1, MemOperand(frame_pointer(), kStringStartMinusOneOffset));
    __ Add_d(a0, current_input_offset(), Operand(-char_size() + cp_offset * char_size()));
...
void RegExpMacroAssemblerLOONG64::CheckCharacterLT(base::uc16 limit, Label* on_less)
{
...
void RegExpMacroAssemblerLOONG64::CheckGreedyLoop(Label* on_equal)
{
    ...
    __ Add_d(backtrack_stackpointer(), backtrack_stackpointer(), Operand(kIntSize));
    __ bind(&backtrack_non_equal);
    BranchOrBacktrack(on_equal, eq, current_input_offset(), Operand(a0));
}

void RegExpMacroAssemblerLOONG64::CheckNotBackReferenceIgnoreCase(int start_reg, bool read_backward, bool unicode, Label* on_no_match)
{
    Label fallthrough;
...
void RegExpMacroAssemblerLOONG64::CheckNotBackReference(int start_reg, bool read_backward, Label* on_no_match)
{
    Label fallthrough;
...
void RegExpMacroAssemblerLOONG64::CheckNotCharacter(uint32_t c, Label* on_not_equal)
{
    BranchOrBacktrack(on_not_equal, ne, current_character(), Operand(c));
}

void RegExpMacroAssemblerLOONG64::CheckCharacterAfterAnd(uint32_t c, uint32_t mask, Label* on_equal)
{
...
void RegExpMacroAssemblerLOONG64::CheckNotCharacterAfterAnd(uint32_t c, uint32_t mask, Label* on_not_equal)
{
    __ And(a0, current_character(), Operand(mask));
    Operand rhs = (c == 0) ? Operand(zero_reg) : Operand(c);
...
void RegExpMacroAssemblerLOONG64::CheckNotCharacterAfterMinusAnd(base::uc16 c, base::uc16 minus, base::uc16 mask, Label* on_not_equal)
{
...
void RegExpMacroAssemblerLOONG64::CheckCharacterInRange(base::uc16 from, base::uc16 to, Label* on_in_range)
{
...
void RegExpMacroAssemblerLOONG64::CheckCharacterNotInRange(base::uc16 from, base::uc16 to, Label* on_not_in_range)
{
    __ Sub_d(a0, current_character(), Operand(from));
...
void RegExpMacroAssemblerLOONG64::CallIsCharacterInRangeArray(const ZoneList<CharacterRange>* ranges)
{
...
bool RegExpMacroAssemblerLOONG64::CheckCharacterInRangeArray(const ZoneList<CharacterRange>* ranges, Label* on_in_range)
{
...
bool RegExpMacroAssemblerLOONG64::CheckCharacterNotInRangeArray(const ZoneList<CharacterRange>* ranges, Label* on_not_in_range)
{
    CallIsCharacterInRangeArray(ranges);
...
void RegExpMacroAssemblerLOONG64::CheckBitInTable(Handle<ByteArray> table, Label* on_bit_set)
{
...
void RegExpMacroAssemblerLOONG64::SkipUntilBitInTable(int cp_offset, Handle<ByteArray> table, Handle<ByteArray> nibble_table, int advance_by)
{
...
bool RegExpMacroAssemblerLOONG64::CheckSpecialClassRanges(StandardCharacterSet type, Label* on_no_match)
{
...
void RegExpMacroAssemblerLOONG64::LoadRegExpStackPointerFromMemory(Register dst)
{
...
void RegExpMacroAssemblerLOONG64::StoreRegExpStackPointerToMemory(Register src, Register scratch)
{
    ExternalReference ref = ExternalReference::address_of_regexp_stack_stack_pointer(isolate());
    __ li(scratch, ref);
...
void RegExpMacroAssemblerLOONG64::PushRegExpBasePointer(Register stack_pointer, Register scratch)
{
...
void RegExpMacroAssemblerLOONG64::PopRegExpBasePointer(Register stack_pointer_out, Register scratch)
{
    ExternalReference ref = ExternalReference::address_of_regexp_stack_memory_top_address(isolate());
...
Handle<HeapObject> RegExpMacroAssemblerLOONG64::GetCode(Handle<String> source, RegExpFlags flags)
{
    Label return_v0;
    if (0 /* todo masm_->has_exception()*/) {
...

        {
            ...
            ExternalReference stack_limit = ExternalReference::address_of_jslimit(masm_->isolate());
        ...
        if (success_label_.is_linked()) {
            ...
            if (global()) {
                ...
                if (global_with_zero_length_check()) {
                    ...
                    __ Branch(&reload_string_start_minus_one, ne, current_input_offset(), Operand(t3));
                    // Offset from the end is zero if we already reached the end.
                    __ Branch(&exit_label_, eq, current_input_offset(), Operand(zero_reg));
                    // Advance current position after a zero-length match.
                    Label advance;
                    __ bind(&advance);
                    __ Add_d(current_input_offset(), current_input_offset(), Operand((mode_ == UC16) ? 2 : 1));
                    if (global_unicode())
    ...
    LOG(masm_->isolate(), RegExpCodeCreateEvent(Cast<AbstractCode>(code), source, flags));
...
void RegExpMacroAssemblerLOONG64::IfRegisterGE(int reg, int comparand, Label* if_ge)
{
    __ Ld_d(a0, register_location(reg));
    BranchOrBacktrack(if_ge, ge, a0, Operand(comparand));
}

void RegExpMacroAssemblerLOONG64::IfRegisterLT(int reg, int comparand, Label* if_lt)
{
    __ Ld_d(a0, register_location(reg));
...
RegExpMacroAssembler::IrregexpImplementation RegExpMacroAssemblerLOONG64::Implementation()
{
...
void RegExpMacroAssemblerLOONG64::PushBacktrack(Label* label)
{
    if (label->is_bound()) {
        ...
        __ li(a0, Operand(target + InstructionStream::kHeaderSize - kHeapObjectTag));
    } else {
...
void RegExpMacroAssemblerLOONG64::PushRegister(int register_index, StackCheckFlag check_stack_limit)
{
...
void RegExpMacroAssemblerLOONG64::SetCurrentPositionFromEnd(int by)
{
    ...
    __ Branch(&after_position, ge, current_input_offset(), Operand(-by * char_size()));
...
void RegExpMacroAssemblerLOONG64::WriteCurrentPositionToRegister(int reg, int cp_offset)
{
    if (cp_offset == 0) {
...
void RegExpMacroAssemblerLOONG64::CallCheckStackGuardState(Register scratch, Operand extra_space)
{
...
int64_t RegExpMacroAssemblerLOONG64::CheckStackGuardState(Address* return_address, Address raw_code, Address re_frame, uintptr_t extra_space)
{
    ...
    return NativeRegExpMacroAssembler::CheckStackGuardState(frame_entry<Isolate*>(re_frame, kIsolateOffset),
        static_cast<int>(frame_entry<int64_t>(re_frame, kStartIndexOffset)), static_cast<RegExp::CallOrigin>(frame_entry<int64_t>(re_frame, kDirectCallOffset)),
        return_address, re_code, frame_entry_address<Address>(re_frame, kInputStringOffset), frame_entry_address<const uint8_t*>(re_frame, kInputStartOffset),
...
void RegExpMacroAssemblerLOONG64::CheckPosition(int cp_offset, Label* on_outside_input)
{
    if (cp_offset >= 0) {
        BranchOrBacktrack(on_outside_input, ge, current_input_offset(), Operand(-cp_offset * char_size()));
    } else {
        __ Ld_d(a1, MemOperand(frame_pointer(), kStringStartMinusOneOffset));
        __ Add_d(a0, current_input_offset(), Operand(cp_offset * char_size()));
...
void RegExpMacroAssemblerLOONG64::BranchOrBacktrack(Label* to, Condition condition, Register rs, const Operand& rt)
{
...
void RegExpMacroAssemblerLOONG64::SafeCall(Label* to, Condition cond, Register rs, const Operand& rt)
{
...
{
    ExternalReference stack_limit = ExternalReference::address_of_regexp_stack_limit_address(masm_->isolate());
...
void RegExpMacroAssemblerLOONG64::LoadCurrentCharacterUnchecked(int cp_offset, int characters)
{
