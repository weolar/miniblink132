...
{
    if (by != 0) {
        __ AddWord(current_input_offset(), current_input_offset(), Operand(by * char_size()));
    }
}
...
void RegExpMacroAssemblerRISCV::Bind(Label* label)
{
    __ bind(label);
}
...
void RegExpMacroAssemblerRISCV::CheckCharacterGT(base::uc16 limit, Label* on_greater)
{
...
void RegExpMacroAssemblerRISCV::CheckAtStart(int cp_offset, Label* on_at_start)
{
    ...
    __ AddWord(a0, current_input_offset(), Operand(-char_size() + cp_offset * char_size()));
    BranchOrBacktrack(on_at_start, eq, a0, Operand(a1));
}

void RegExpMacroAssemblerRISCV::CheckNotAtStart(int cp_offset, Label* on_not_at_start)
{
    __ LoadWord(a1, MemOperand(frame_pointer(), kStringStartMinusOneOffset));
    __ AddWord(a0, current_input_offset(), Operand(-char_size() + cp_offset * char_size()));
...
void RegExpMacroAssemblerRISCV::CheckGreedyLoop(Label* on_equal)
{
    ...
    __ AddWord(backtrack_stackpointer(), backtrack_stackpointer(), Operand(kIntSize));
    __ bind(&backtrack_non_equal);
    BranchOrBacktrack(on_equal, eq, current_input_offset(), Operand(a0));
}

void RegExpMacroAssemblerRISCV::CallIsCharacterInRangeArray(const ZoneList<CharacterRange>* ranges)
{
...
bool RegExpMacroAssemblerRISCV::CheckCharacterInRangeArray(const ZoneList<CharacterRange>* ranges, Label* on_in_range)
{
...
bool RegExpMacroAssemblerRISCV::CheckCharacterNotInRangeArray(const ZoneList<CharacterRange>* ranges, Label* on_not_in_range)
{
    CallIsCharacterInRangeArray(ranges);
...
void RegExpMacroAssemblerRISCV::CheckNotBackReferenceIgnoreCase(int start_reg, bool read_backward, bool unicode, Label* on_no_match)
{
    Label fallthrough;
    ...
    if (mode_ == LATIN1) {
        ...
        if (read_backward) {
            ...
            __ LoadWord(a2, register_location(start_reg + 1)); // Index of end of capture.
...
void RegExpMacroAssemblerRISCV::CheckNotBackReference(int start_reg, bool read_backward, Label* on_no_match)
{
    Label fallthrough;
...
void RegExpMacroAssemblerRISCV::CheckNotCharacter(uint32_t c, Label* on_not_equal)
{
    BranchOrBacktrack(on_not_equal, ne, current_character(), Operand(c));
}

void RegExpMacroAssemblerRISCV::CheckCharacterAfterAnd(uint32_t c, uint32_t mask, Label* on_equal)
{
...
void RegExpMacroAssemblerRISCV::CheckNotCharacterAfterAnd(uint32_t c, uint32_t mask, Label* on_not_equal)
{
    __ And(a0, current_character(), Operand(mask));
    Operand rhs = (c == 0) ? Operand(zero_reg) : Operand(c);
...
void RegExpMacroAssemblerRISCV::CheckCharacterInRange(base::uc16 from, base::uc16 to, Label* on_in_range)
{
...
void RegExpMacroAssemblerRISCV::CheckCharacterNotInRange(base::uc16 from, base::uc16 to, Label* on_not_in_range)
{
    __ SubWord(a0, current_character(), Operand(from));
...
void RegExpMacroAssemblerRISCV::CheckBitInTable(Handle<ByteArray> table, Label* on_bit_set)
{
...
void RegExpMacroAssemblerRISCV::SkipUntilBitInTable(int cp_offset, Handle<ByteArray> table, Handle<ByteArray> nibble_table, int advance_by)
{
...
bool RegExpMacroAssemblerRISCV::CheckSpecialClassRanges(StandardCharacterSet type, Label* on_no_match)
{
    ...
    case StandardCharacterSet::kWord: {
        if (mode_ != LATIN1) {
            ...
            BranchOrBacktrack(on_no_match, Ugreater, current_character(), Operand('z'));
        }
...
void RegExpMacroAssemblerRISCV::StoreRegExpStackPointerToMemory(Register src, Register scratch)
{
    ExternalReference ref = ExternalReference::address_of_regexp_stack_stack_pointer(isolate());
...
void RegExpMacroAssemblerRISCV::PushRegExpBasePointer(Register stack_pointer, Register scratch)
{
...
void RegExpMacroAssemblerRISCV::PopRegExpBasePointer(Register stack_pointer_out, Register scratch)
{
    ExternalReference ref = ExternalReference::address_of_regexp_stack_memory_top_address(isolate());
...
Handle<HeapObject> RegExpMacroAssemblerRISCV::GetCode(Handle<String> source, RegExpFlags flags)
{
    Label return_a0;
    if (masm_->has_exception()) {
        ...
        // or dont save.
        RegList registers_to_retain = { fp, s1, s2, s3, s4, s5, s6, s7, s8, s9, s10, s11 };
        ...
        if (success_label_.is_linked()) {
            ...
            if (global()) {
                ...
                if (global_with_zero_length_check()) {
                    ...
                    __ Branch(&reload_string_start_minus_one, ne, current_input_offset(), Operand(s3));
                    // Offset from the end is zero if we already reached the end.
                    __ Branch(&exit_label_, eq, current_input_offset(), Operand(zero_reg));
                    // Advance current position after a zero-length match.
                    Label advance;
                    __ bind(&advance);
                    __ AddWord(current_input_offset(), current_input_offset(), Operand((mode_ == UC16) ? 2 : 1));
                    if (global_unicode())
...
void RegExpMacroAssemblerRISCV::IfRegisterGE(int reg, int comparand, Label* if_ge)
{
    __ LoadWord(a0, register_location(reg));
    BranchOrBacktrack(if_ge, ge, a0, Operand(comparand));
}

void RegExpMacroAssemblerRISCV::IfRegisterLT(int reg, int comparand, Label* if_lt)
{
    __ LoadWord(a0, register_location(reg));
...
RegExpMacroAssembler::IrregexpImplementation RegExpMacroAssemblerRISCV::Implementation()
{
...
void RegExpMacroAssemblerRISCV::PushRegister(int register_index, StackCheckFlag check_stack_limit)
{
...
void RegExpMacroAssemblerRISCV::SetCurrentPositionFromEnd(int by)
{
    ...
    __ BranchShort(&after_position, ge, current_input_offset(), Operand(-by * char_size()));
...
void RegExpMacroAssemblerRISCV::WriteCurrentPositionToRegister(int reg, int cp_offset)
{
    if (cp_offset == 0) {
...
bool RegExpMacroAssemblerRISCV::CanReadUnaligned() const
{
    return false;
}
#endif
...
int64_t RegExpMacroAssemblerRISCV::CheckStackGuardState(Address* return_address, Address raw_code, Address re_frame, uintptr_t extra_space)
{
...
void RegExpMacroAssemblerRISCV::CheckPosition(int cp_offset, Label* on_outside_input)
{
    if (cp_offset >= 0) {
...
void RegExpMacroAssemblerRISCV::BranchOrBacktrack(Label* to, Condition condition, Register rs, const Operand& rt)
{
...
void RegExpMacroAssemblerRISCV::SafeCall(Label* to, Condition cond, Register rs, const Operand& rt)
{
...
void RegExpMacroAssemblerRISCV::CheckStackLimit()
{
    ...
    __ li(a0, Operand(stack_limit));
    __ LoadWord(a0, MemOperand(a0));
    SafeCall(&stack_overflow_label_, Uless_equal, backtrack_stackpointer(), Operand(a0));
}

void RegExpMacroAssemblerRISCV::LoadCurrentCharacterUnchecked(int cp_offset, int characters)
{
    Register offset = current_input_offset();
    if (cp_offset != 0) {
        // kScratchReg2 is not being used to store the capture start index at this
        // point.
        __ AddWord(kScratchReg2, current_input_offset(), Operand(cp_offset * char_size()));
        offset = kScratchReg2;
    }
