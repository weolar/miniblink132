...
public:
    ...
    virtual Handle<HeapObject> GetCode(Handle<String> source, RegExpFlags flags) = 0;
    ...
    V8_EXPORT_PRIVATE void LoadCurrentCharacter(
        int cp_offset, Label* on_end_of_input, bool check_bounds = true, int characters = 1, int eats_at_least = kUseCharactersValue);
...
#undef IMPLEMENTATIONS_LIST
    ...
    inline void set_global_mode(GlobalMode mode)
    {
        global_mode_ = mode;
    }
    ...
    inline bool global_unicode() const
    {
...
protected:
    ...
    {
        return backtrack_limit_;
    }

    bool can_fallback() const
    {
        return can_fallback_;
    }
...
public:
    ...
    NativeRegExpMacroAssembler(Isolate* isolate, Zone* zone)
        ...
        , range_array_cache_(zone)
    {
    }
    ~NativeRegExpMacroAssembler() override = default;

    // Returns a {Result} sentinel, or the number of successful matches.
    static int Match(DirectHandle<IrRegExpData> regexp_data, DirectHandle<String> subject, int* offsets_vector, int offsets_vector_length, int previous_index,
        Isolate* isolate);

    V8_EXPORT_PRIVATE static int ExecuteForTesting(Tagged<String> input, int start_offset, const uint8_t* input_start, const uint8_t* input_end, int* output,
    ...
    // Called from generated code.
    static int CheckStackGuardState(Isolate* isolate, int start_index, RegExp::CallOrigin call_origin, Address* return_address,
...
private:
    // Returns a {Result} sentinel, or the number of successful matches.
    static int Execute(Tagged<String> input, int start_offset, const uint8_t* input_start, const uint8_t* input_end, int* output, int output_size,
