...
RUNTIME_FUNCTION(Runtime_AccessCheck)
{
    ...
    if (!isolate->MayAccess(isolate->native_context(), object)) {
        RETURN_FAILURE_ON_EXCEPTION(isolate, isolate->ReportFailedAccessCheck(object));
        UNREACHABLE();
    }
...
RUNTIME_FUNCTION(Runtime_ThrowSymbolAsyncIteratorInvalid)
{
    HandleScope scope(isolate);
    DCHECK_EQ(0, args.length());
    THROW_NEW_ERROR_RETURN_FAILURE(isolate, NewTypeError(MessageTemplate::kSymbolAsyncIteratorInvalid));
}
...
Tagged<Object> NewError(Isolate* isolate, RuntimeArguments args, Handle<JSFunction> (Isolate::*constructor_fn)())
{
    HandleScope scope(isolate);
    ...
    return *isolate->factory()->NewError((isolate->*constructor_fn)(), message_id, base::VectorOf(message_args, num_message_args));
}

Tagged<Object> ThrowError(Isolate* isolate, RuntimeArguments args, Handle<JSFunction> (Isolate::*constructor_fn)())
{
...
RUNTIME_FUNCTION(Runtime_ThrowRangeError)
{
    if (v8_flags.correctness_fuzzer_suppressions) {
        ...
        if (MessageTemplateFromInt(message_id_smi) == MessageTemplate::kBigIntTooBig) {
...
RUNTIME_FUNCTION(Runtime_ThrowInvalidTypedArrayAlignment)
{
    ...
    THROW_NEW_ERROR_RETURN_FAILURE(isolate, NewRangeError(MessageTemplate::kInvalidTypedArrayAlignment, problem_string, type, element_size));
}
...
RUNTIME_FUNCTION(Runtime_ThrowReferenceError)
{
    ...
    THROW_NEW_ERROR_RETURN_FAILURE(isolate, NewReferenceError(MessageTemplate::kNotDefined, name));
}
...
RUNTIME_FUNCTION(Runtime_ThrowIteratorResultNotAnObject)
{
    ...
    THROW_NEW_ERROR_RETURN_FAILURE(isolate, NewTypeError(MessageTemplate::kIteratorResultNotAnObject, value));
}
...
RUNTIME_FUNCTION(Runtime_ThrowThrowMethodMissing)
{
    HandleScope scope(isolate);
    DCHECK_EQ(0, args.length());
    THROW_NEW_ERROR_RETURN_FAILURE(isolate, NewTypeError(MessageTemplate::kThrowMethodMissing));
}
...
RUNTIME_FUNCTION(Runtime_ThrowSymbolIteratorInvalid)
{
    HandleScope scope(isolate);
    DCHECK_EQ(0, args.length());
    THROW_NEW_ERROR_RETURN_FAILURE(isolate, NewTypeError(MessageTemplate::kSymbolIteratorInvalid));
}
...
RUNTIME_FUNCTION(Runtime_ThrowNoAccess)
{
    ...
    SaveAndSwitchContext save(isolate, impl->LastEnteredContext()->native_context());
    THROW_NEW_ERROR_RETURN_FAILURE(isolate, NewTypeError(MessageTemplate::kNoAccess));
}
...
RUNTIME_FUNCTION(Runtime_ThrowNotConstructor)
{
    ...
    THROW_NEW_ERROR_RETURN_FAILURE(isolate, NewTypeError(MessageTemplate::kNotConstructor, object));
}
...
RUNTIME_FUNCTION(Runtime_ThrowApplyNonFunction)
{
    ...
    if (IsNull(*object)) {
        ...
        msg = isolate->factory()->NewConsString(isolate->factory()->NewStringFromAsciiChecked("a "), type).ToHandleChecked();
    }
    THROW_NEW_ERROR_RETURN_FAILURE(isolate, NewTypeError(MessageTemplate::kApplyNonFunction, object, msg));
}
...
RUNTIME_FUNCTION(Runtime_HandleNoHeapWritesInterrupts)
{
    ...
    return isolate->stack_guard()->HandleInterrupts(StackGuard::InterruptLevel::kNoHeapWrites);
}
...
Tagged<Object> BytecodeBudgetInterruptWithStackCheck(Isolate* isolate, RuntimeArguments& args, CodeKind code_kind)
{
    HandleScope scope(isolate);
    DCHECK_EQ(1, args.length());
...
Tagged<Object> BytecodeBudgetInterrupt(Isolate* isolate, RuntimeArguments& args, CodeKind code_kind)
{
    HandleScope scope(isolate);
    DCHECK_EQ(1, args.length());
    DirectHandle<JSFunction> function = args.at<JSFunction>(0);
...
{
    return BytecodeBudgetInterruptWithStackCheck(isolate, args, CodeKind::INTERPRETED_FUNCTION);
}
...
{
    return BytecodeBudgetInterruptWithStackCheck(isolate, args, CodeKind::BASELINE);
}
...
RUNTIME_FUNCTION(Runtime_AllocateInYoungGeneration)
{
    ...
    AllocationAlignment alignment = AllocateDoubleAlignFlag::decode(flags) ? kDoubleAligned : kTaggedAligned;
    ...
    return *isolate->factory()->NewFillerObject(size, alignment, AllocationType::kYoung, AllocationOrigin::kGeneratedCode);
}
...
RUNTIME_FUNCTION(Runtime_AllocateInOldGeneration)
{
    ...
    CHECK(IsAligned(size, kTaggedSize));
    CHECK_GT(size, 0);
    return *isolate->factory()->NewFillerObject(size, alignment, AllocationType::kOld, AllocationOrigin::kGeneratedCode);
}
...
RUNTIME_FUNCTION(Runtime_ThrowCalledNonCallable)
{
    ...
    DCHECK_EQ(1, args.length());
    Handle<Object> object = args.at(0);
    return isolate->Throw(*ErrorUtils::NewCalledNonCallableError(isolate, object));
}
...
RUNTIME_FUNCTION(Runtime_ThrowConstructedNonConstructable)
{
    ...
    DCHECK_EQ(1, args.length());
    Handle<Object> object = args.at(0);
    return isolate->Throw(*ErrorUtils::NewConstructedNonConstructable(isolate, object));
}
...
RUNTIME_FUNCTION(Runtime_ThrowPatternAssignmentNonCoercible)
{
    ...
    DCHECK_EQ(1, args.length());
    Handle<Object> object = args.at(0);
    return ErrorUtils::ThrowLoadFromNullOrUndefined(isolate, object, MaybeHandle<Object>());
}
...
RUNTIME_FUNCTION(Runtime_ThrowConstructorReturnedNonObject)
{
    ...
    THROW_NEW_ERROR_RETURN_FAILURE(isolate, NewTypeError(MessageTemplate::kDerivedConstructorReturnedNonObject));
}
...
RUNTIME_FUNCTION(Runtime_GetAndResetTurboProfilingData)
{
    ...
    if (!BasicBlockProfiler::Get()->HasData(isolate)) {
        THROW_NEW_ERROR_RETURN_FAILURE(isolate,
            NewTypeError(MessageTemplate::kInvalid, isolate->factory()->NewStringFromAsciiChecked("Runtime Call"),
                isolate->factory()->NewStringFromAsciiChecked("V8 was not built with v8_enable_builtins_profiling=true")));
    }

    std::stringstream stats_stream;
    BasicBlockProfiler::Get()->Log(isolate, stats_stream);
    DirectHandle<String> result = isolate->factory()->NewStringFromAsciiChecked(stats_stream.str().c_str());
...
RUNTIME_FUNCTION(Runtime_GetAndResetRuntimeCallStats)
{
    ...
    if (!v8_flags.runtime_call_stats) {
        THROW_NEW_ERROR_RETURN_FAILURE(isolate,
            NewTypeError(MessageTemplate::kInvalid, isolate->factory()->NewStringFromAsciiChecked("Runtime Call"),
                isolate->factory()->NewStringFromAsciiChecked("--runtime-call-stats is not set")));
    }
    // Append any worker thread runtime call stats to the main table before
    // printing.
    isolate->counters()->worker_thread_runtime_call_stats()->AddToMainTable(isolate->counters()->runtime_call_stats());
    ...
    THROW_NEW_ERROR_RETURN_FAILURE(isolate,
        NewTypeError(MessageTemplate::kInvalid, isolate->factory()->NewStringFromAsciiChecked("Runtime Call"),
            isolate->factory()->NewStringFromAsciiChecked("RCS was disabled at compile-time")));
...
RUNTIME_FUNCTION(Runtime_OrdinaryHasInstance)
{
    ...
    RETURN_RESULT_OR_FAILURE(isolate, Object::OrdinaryHasInstance(isolate, callable, object));
}
...
RUNTIME_FUNCTION(Runtime_AllowDynamicFunction)
{
    ...
    return *isolate->factory()->ToBoolean(Builtins::AllowDynamicFunction(isolate, target, global_proxy));
}
...
RUNTIME_FUNCTION(Runtime_CreateAsyncFromSyncIterator)
{
    ...
    Handle<Object> next;
    ASSIGN_RETURN_FAILURE_ON_EXCEPTION(isolate, next, Object::GetProperty(isolate, sync_iterator, isolate->factory()->next_string()));
...
RUNTIME_FUNCTION(Runtime_GetTemplateObject)
{
    ...
    DirectHandle<TemplateObjectDescription> description = args.at<TemplateObjectDescription>(0);
    ...
    return *TemplateObjectDescription::GetTemplateObject(isolate, native_context, description, shared_info, slot_id);
}
...
RUNTIME_FUNCTION(Runtime_ReportMessageFromMicrotask)
{
    ...
    DirectHandle<JSMessageObject> message = isolate->CreateMessageOrAbort(exception, no_location);
...
RUNTIME_FUNCTION(Runtime_GetInitializerFunction)
{
    ...
    DirectHandle<Object> initializer = JSReceiver::GetDataProperty(isolate, constructor, key);
    return *initializer;
}
...
RUNTIME_FUNCTION(Runtime_DoubleToStringWithRadix)
{
    ...
    DirectHandle<String> result = isolate->factory()->NewStringFromAsciiChecked(str);
    DeleteArray(str);
    return *result;
}
...
RUNTIME_FUNCTION(Runtime_SharedValueBarrierSlow)
{
    ...
    ASSIGN_RETURN_FAILURE_ON_EXCEPTION(isolate, shared_value, Object::ShareSlow(isolate, value, kThrowOnError));
    return *shared_value;
}
...
RUNTIME_FUNCTION(Runtime_InvalidateDependentCodeForScriptContextSlot)
{
    HandleScope scope(isolate);
    DCHECK_EQ(1, args.length());
    auto const_tracking_let_cell = Cast<ContextSidePropertyCell>(args.at<HeapObject>(0));
    DependentCode::DeoptimizeDependencyGroups(isolate, *const_tracking_let_cell, DependentCode::kScriptContextSlotPropertyChangedGroup);
    return ReadOnlyRoots(isolate).undefined_value();
}
