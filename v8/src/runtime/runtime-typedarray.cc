...
RUNTIME_FUNCTION(Runtime_ArrayBufferDetach)
{
    ...
    if (args.length() < 1 || !IsJSArrayBuffer(*args.at(0))) {
        THROW_NEW_ERROR_RETURN_FAILURE(isolate, NewTypeError(MessageTemplate::kNotTypedArray));
    }
    auto array_buffer = Cast<JSArrayBuffer>(args.at(0));
    constexpr bool kForceForWasmMemory = false;
    MAYBE_RETURN(JSArrayBuffer::Detach(array_buffer, kForceForWasmMemory, args.atOrUndefined(isolate, 1)), ReadOnlyRoots(isolate).exception());
...
template <typename T> bool CompareNum(T x, T y)
{
    if (x < y) {
        return true;
    } else if (x > y) {
...
RUNTIME_FUNCTION(Runtime_TypedArraySortFast)
{
    ...
    DirectHandle<JSArrayBuffer> buffer(Cast<JSArrayBuffer>(array->buffer()), isolate);
    ...
    if (copy_data) {
        ...
        if (bytes <= static_cast<unsigned>(ByteArray::LengthFor(kMaxRegularHeapObjectSize))) {
        ...
        base::Relaxed_Memcpy(static_cast<base::Atomic8*>(data_copy_ptr), static_cast<base::Atomic8*>(array->DataPtr()), bytes);
    }
...
#define TYPED_ARRAY_SORT(Type, type, TYPE, ctype)                                                                                                              \
    case kExternal##Type##Array: {                                                                                                                             \
        ctype* data = copy_data ? reinterpret_cast<ctype*>(data_copy_ptr) : static_cast<ctype*>(array->DataPtr());                                             \
        if (kExternal##Type##Array == kExternalFloat64Array || kExternal##Type##Array == kExternalFloat32Array                                                 \
            ...
            if (COMPRESS_POINTERS_BOOL && alignof(ctype) > kTaggedSize) {                                                                                      \
                ...
                std::sort(UnalignedSlot<ctype>(data), UnalignedSlot<ctype>(data + length), CompareNum<ctype>);                                                 \
            } else {                                                                                                                                           \
            ...
            if (COMPRESS_POINTERS_BOOL && alignof(ctype) > kTaggedSize) {                                                                                      \
                /* TODO(ishell, v8:8875): See UnalignedSlot<T> for details. */                                                                                 \
                std::sort(UnalignedSlot<ctype>(data), UnalignedSlot<ctype>(data + length));                                                                    \
            } else {                                                                                                                                           \
...
#undef TYPED_ARRAY_SORT
    ...
    if (copy_data) {
        ...
        const size_t bytes = array->GetByteLength();
        base::Relaxed_Memcpy(static_cast<base::Atomic8*>(array->DataPtr()), static_cast<base::Atomic8*>(data_copy_ptr), bytes);
    }
