...
#define F(name, number_of_args, result_size) Address Runtime_##name(int args_length, Address* args_object, Isolate* isolate);
FOR_EACH_INTRINSIC_RETURN_OBJECT(F)
#undef F

#define P(name, number_of_args, result_size) ObjectPair Runtime_##name(int args_length, Address* args_object, Isolate* isolate);
FOR_EACH_INTRINSIC_RETURN_PAIR(P)
#undef P

#define F(name, number_of_args, result_size) { Runtime::k##name, Runtime::RUNTIME, #name, FUNCTION_ADDR(Runtime_##name), number_of_args, result_size },
...
struct IntrinsicFunctionIdentifier {
    ...
    uint32_t Hash()
    {
        return StringHasher::HashSequentialString<uint8_t>(data_, length_, v8::internal::kZeroHashSeed);
    }
...
void InitializeIntrinsicFunctionNames()
{
    ...
    for (size_t i = 0; i < arraysize(kIntrinsicFunctions); ++i) {
        ...
        IntrinsicFunctionIdentifier* identifier
            = new IntrinsicFunctionIdentifier(reinterpret_cast<const unsigned char*>(function->name), static_cast<int>(strlen(function->name)));
