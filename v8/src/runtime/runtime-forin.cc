...
MaybeHandle<HeapObject> Enumerate(Isolate* isolate, Handle<JSReceiver> receiver)
{
    ...
    if (!accumulator.is_receiver_simple_enum()) {
        ...
        ASSIGN_RETURN_ON_EXCEPTION(
            isolate, keys, accumulator.GetKeys(accumulator.may_have_elements() ? GetKeysConversion::kConvertToString : GetKeysConversion::kNoNumbers));
...
MaybeHandle<Object> HasEnumerableProperty(Isolate* isolate, Handle<JSReceiver> receiver, Handle<Object> key)
{
    bool success = false;
    ...
    for (;; it.Next()) {
        ...
        case LookupIterator::JSPROXY: {
            ...
            if (result.FromJust() == ABSENT) {
                ...
                ASSIGN_RETURN_ON_EXCEPTION(isolate, prototype, JSProxy::GetPrototype(proxy));
                if (IsNull(*prototype, isolate)) {
                    return isolate->factory()->undefined_value();
                }
                // We already have a stack-check in JSProxy::GetPrototype.
                return HasEnumerableProperty(isolate, Cast<JSReceiver>(prototype), key);
        ...
        case LookupIterator::WASM_OBJECT:
            THROW_NEW_ERROR(isolate, NewTypeError(MessageTemplate::kWasmObjectsAreOpaque));
...
RUNTIME_FUNCTION(Runtime_ForInHasProperty)
{
    ...
    ASSIGN_RETURN_FAILURE_ON_EXCEPTION(isolate, result, HasEnumerableProperty(isolate, receiver, key));
