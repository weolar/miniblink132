...
RUNTIME_FUNCTION(Runtime_DynamicImportCall)
{
    ...
    ModuleImportPhase phase = static_cast<ModuleImportPhase>(args.smi_value_at(2));
    ...
    Handle<Script> referrer_script = handle(Cast<Script>(function->shared()->script())->GetEvalOrigin(), isolate);
    RETURN_RESULT_OR_FAILURE(isolate, isolate->RunHostImportModuleDynamicallyCallback(referrer_script, specifier, phase, import_options));
}
...
RUNTIME_FUNCTION(Runtime_GetImportMetaObject)
{
    ...
    RETURN_RESULT_OR_FAILURE(isolate, SourceTextModule::GetImportMeta(isolate, module));
}
...
RUNTIME_FUNCTION(Runtime_GetModuleNamespaceExport)
{
    ...
    DirectHandle<JSModuleNamespace> module_namespace = args.at<JSModuleNamespace>(0);
