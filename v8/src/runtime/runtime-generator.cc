...
RUNTIME_FUNCTION(Runtime_CreateJSGeneratorObject)
{
    ...
    CHECK_IMPLIES(IsAsyncFunction(function->shared()->kind()), IsAsyncGeneratorFunction(function->shared()->kind()));
    ...
    int length;
    {
        ...
        length = bytecode->parameter_count_without_receiver() + bytecode->register_count();
    }
    DirectHandle<FixedArray> parameters_and_registers = isolate->factory()->NewFixedArray(length);

    DirectHandle<JSGeneratorObject> generator = isolate->factory()->NewJSGeneratorObject(function);
