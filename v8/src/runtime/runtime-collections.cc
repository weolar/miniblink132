...
RUNTIME_FUNCTION(Runtime_OrderedHashSetGrow)
{
    ...
    if (!table_candidate.ToHandle(&table)) {
        THROW_NEW_ERROR_RETURN_FAILURE(isolate, NewRangeError(MessageTemplate::kOutOfMemory, method_name));
    }
    return *table;
}
...
RUNTIME_FUNCTION(Runtime_SetGrow)
{
    ...
    MaybeHandle<OrderedHashSet> table_candidate = OrderedHashSet::EnsureCapacityForAdding(isolate, table);
    if (!table_candidate.ToHandle(&table)) {
        THROW_NEW_ERROR_RETURN_FAILURE(isolate, NewRangeError(MessageTemplate::kCollectionGrowFailed, isolate->factory()->NewStringFromAsciiChecked("Set")));
    }
...
RUNTIME_FUNCTION(Runtime_MapGrow)
{
    ...
    if (!table_candidate.ToHandle(&table)) {
        THROW_NEW_ERROR_RETURN_FAILURE(isolate, NewRangeError(MessageTemplate::kCollectionGrowFailed, isolate->factory()->NewStringFromAsciiChecked("Map")));
    }
    holder->set_table(*table);
    return ReadOnlyRoots(isolate).undefined_value();
}
...
RUNTIME_FUNCTION(Runtime_OrderedHashMapGrow)
{
    ...
    MaybeHandle<OrderedHashMap> table_candidate = OrderedHashMap::EnsureCapacityForAdding(isolate, table);
    if (!table_candidate.ToHandle(&table)) {
        THROW_NEW_ERROR_RETURN_FAILURE(isolate, NewRangeError(MessageTemplate::kOutOfMemory, methodName));
    }
    return *table;
}
...
RUNTIME_FUNCTION(Runtime_WeakCollectionDelete)
{
    ...
    DirectHandle<EphemeronHashTable> table(Cast<EphemeronHashTable>(weak_collection->table()), isolate);
