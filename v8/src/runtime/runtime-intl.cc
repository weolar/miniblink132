...
RUNTIME_FUNCTION(Runtime_FormatList)
{
    ...
    RETURN_RESULT_OR_FAILURE(isolate, JSListFormat::FormatList(isolate, list_format, list));
}
...
RUNTIME_FUNCTION(Runtime_FormatListToParts)
{
    ...
    DirectHandle<JSListFormat> list_format = args.at<JSListFormat>(0);
    DirectHandle<FixedArray> list = args.at<FixedArray>(1);
    RETURN_RESULT_OR_FAILURE(isolate, JSListFormat::FormatListToParts(isolate, list_format, list));
}
...
RUNTIME_FUNCTION(Runtime_StringToLocaleLowerCase)
{
    ...
    RETURN_RESULT_OR_FAILURE(isolate, Intl::StringLocaleConvertCase(isolate, s, false, locale));
}
