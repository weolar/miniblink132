...
RUNTIME_FUNCTION(Runtime_FunctionGetScriptSource)
{
    ...
    if (IsJSFunction(*function)) {
        Handle<Object> script(Cast<JSFunction>(function)->shared()->script(), isolate);
        if (IsScript(*script))
...
RUNTIME_FUNCTION(Runtime_FunctionGetSourceCode)
{
    ...
    if (IsJSFunction(*function)) {
        DirectHandle<SharedFunctionInfo> shared(Cast<JSFunction>(function)->shared(), isolate);
...
RUNTIME_FUNCTION(Runtime_Call)
{
    ...
    RETURN_RESULT_OR_FAILURE(isolate, Execution::Call(isolate, target, receiver, argc, argv.begin()));
}
