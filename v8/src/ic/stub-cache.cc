...
int StubCache::PrimaryOffset(Tagged<Name> name, Tagged<Map> map)
{
    ...
    uint32_t map_low32bits = static_cast<uint32_t>(map.ptr() ^ (map.ptr() >> kPrimaryTableBits));
...
void StubCache::Set(Tagged<Name> name, Tagged<Map> map, Tagged<MaybeObject> handler)
{
    ...
    Tagged<MaybeObject> old_handler(TaggedValue::ToMaybeObject(isolate(), primary->value));
    ...
    if (!old_handler.SafeEquals(isolate()->builtins()->code(Builtin::kIllegal)) && !primary->map.IsSmi()) {
        Tagged<Map> old_map = Cast<Map>(StrongTaggedValue::ToObject(isolate(), primary->map));
        Tagged<Name> old_name = Cast<Name>(StrongTaggedValue::ToObject(isolate(), primary->key));
