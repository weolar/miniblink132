...
void ICStats::Dump()
{
    ...
    TRACE_EVENT_INSTANT1(TRACE_DISABLED_BY_DEFAULT("v8.ic_stats"), "V8.ICStats", TRACE_EVENT_SCOPE_THREAD, "ic-stats", std::move(value));
    Reset();
}
...
const char* ICStats::GetOrCacheScriptName(Tagged<Script> script)
{
    ...
    if (IsString(script_name_raw)) {
        ...
        script_name_map_.insert(std::make_pair(script_ptr, std::unique_ptr<char[]>(c_script_name)));
        return c_script_name;
    }
    script_name_map_.insert(std::make_pair(script_ptr, std::unique_ptr<char[]>(nullptr)));
    return nullptr;
}

const char* ICStats::GetOrCacheFunctionName(IsolateForSandbox isolate, Tagged<JSFunction> function)
{
...
void ICInfo::AppendToTracedValue(v8::tracing::TracedValue* value) const
{
    ...
    if (column_num != -1)
        value->SetInteger("columnNum", column_num);
    if (is_constructor)
