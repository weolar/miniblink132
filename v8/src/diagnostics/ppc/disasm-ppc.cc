...
uint64_t Decoder::GetPrefixValue()
{
    return PrefixValue;
}
...
int Decoder::FormatOption(Instruction* instr, const char* format)
{
    ...
    case 'i': { // int16
        ...
        out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "%ld", value);
        return 5;
    }
    case 'I': { // IMM8
    ...
    case 'c': { // 'cr: condition register of branch instruction
        ...
        if (code != 7) {
            out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, " cr%d", code);
        }
        return 2;
    }
    case 't': { // 'target: target of branch instructions
        ...
        if ((format[6] == '2') && (format[7] == '6')) {
            ...
            out_buffer_pos_
                += base::SNPrintF(out_buffer_ + out_buffer_pos_, "%+d -> %s", off, converter_.NameOfAddress(reinterpret_cast<uint8_t*>(instr) + off));
            return 8;
...
void Decoder::DecodeExtP(Instruction* instr)
{
    ...
    case PLOAD_STORE_MLS: {
        ...
        Instruction* next_instr = reinterpret_cast<Instruction*>(reinterpret_cast<intptr_t>(instr) + kInstrSize);
...
int Decoder::InstructionDecode(uint8_t* instr_ptr)
{
    ...
    if (opcode != EXTP) {
        ...
        Instruction* next_instr = reinterpret_cast<Instruction*>(reinterpret_cast<intptr_t>(instr) + kInstrSize);
        out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "%08x|%08x ", instr->InstructionBits(), next_instr->InstructionBits());
    }
