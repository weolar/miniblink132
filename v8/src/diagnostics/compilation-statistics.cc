...
void CompilationStatistics::RecordPhaseStats(const char* phase_kind_name, const char* phase_name, const BasicStats& stats)
{
...
void CompilationStatistics::RecordPhaseKindStats(const char* phase_kind_name, const BasicStats& stats)
{
    base::MutexGuard guard(&record_mutex_);
    ...
    if (it == phase_kind_map_.end()) {
        ...
        it = phase_kind_map_.insert(std::make_pair(phase_kind_name_str, phase_kind_stats)).first;
    }
    it->second.Accumulate(stats);
}
...
#define MEMBER(s) QUOTE(s) << ":"
    ...
    stream << DICT(MEMBER("function_name") << QUOTE(function_name_) << "," MEMBER("total_allocated_bytes") << total_allocated_bytes_
                                           << "," MEMBER("max_allocated_bytes") << max_allocated_bytes_ << "," MEMBER("absolute_max_allocated_bytes")
...
static void WriteLine(std::ostream& os, bool machine_format, const char* name, const char* compiler, const CompilationStatistics::BasicStats& stats,
    const CompilationStatistics::BasicStats& total_stats)
{
    ...
    double growth = static_cast<double>(stats.output_graph_size_) / stats.input_graph_size_;
    double mops_per_s = (stats.output_graph_size_ / 1000000.0) / (ms / 1000.0);

    if (machine_format) {
        base::OS::SNPrintF(buffer, kBufferSize, "\"%s_%s_time\"=%.3f\n\"%s_%s_space\"=%zu", compiler, name, ms, compiler, name, stats.total_allocated_bytes_);
        os << buffer;
    } else {
        if (stats.output_graph_size_ != 0) {
            base::OS::SNPrintF(buffer, kBufferSize, "%34s %10.3f (%4.1f%%)  %10zu (%4.1f%%) %10zu %10zu   %5.3f %6.2f", name, ms, percent,
                stats.total_allocated_bytes_, size_percent, stats.max_allocated_bytes_, stats.absolute_max_allocated_bytes_, growth, mops_per_s);
        } else {
            base::OS::SNPrintF(buffer, kBufferSize, "%34s %10.3f (%4.1f%%)  %10zu (%4.1f%%) %10zu %10zu               ", name, ms, percent,
                stats.total_allocated_bytes_, size_percent, stats.max_allocated_bytes_, stats.absolute_max_allocated_bytes_);
        }
...
std::ostream& operator<<(std::ostream& os, const AsPrintableStatistics& ps)
{
    ...
    using SortedPhaseKinds = std::vector<CompilationStatistics::PhaseKindMap::const_iterator>;
    SortedPhaseKinds sorted_phase_kinds(s.phase_kind_map_.size());
    for (auto it = s.phase_kind_map_.begin(); it != s.phase_kind_map_.end(); ++it) {
        sorted_phase_kinds[it->second.insert_order_] = it;
    }

    using SortedPhases = std::vector<CompilationStatistics::PhaseMap::const_iterator>;
    ...
    for (const auto& phase_kind_it : sorted_phase_kinds) {
        ...
        if (!ps.machine_output) {
            for (const auto& phase_it : sorted_phases) {
                ...
                WriteLine(os, ps.machine_output, phase_name.c_str(), ps.compiler, phase_stats, s.total_stats_);
            }
            WritePhaseKindBreak(os);
        }
        const auto& phase_kind_stats = phase_kind_it->second;
        WriteLine(os, ps.machine_output, phase_kind_name.c_str(), ps.compiler, phase_kind_stats, s.total_stats_);
        os << '\n';
    }

    if (!ps.machine_output)
        WriteFullLine(os);
    WriteLine(os, ps.machine_output, "totals", ps.compiler, s.total_stats_, s.total_stats_);
