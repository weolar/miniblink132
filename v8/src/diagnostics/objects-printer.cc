...
void PrintFunctionCallbackInfo(Address* implicit_args, Address* js_args, Address length, std::ostream& os)
{
...
void PrintHeapObjectHeaderWithoutMap(Tagged<HeapObject> object, std::ostream& os, const char* id)
{
...
template <typename T> void PrintDictionaryContents(std::ostream& os, Tagged<T> dict)
{
...
template <typename T> void PrintByteArrayElements(std::ostream& os, const T* array)
{
    ...
    while (i < length) {
        ...
        for (; i < line_end; ++i) {
            os << " " << std::setfill('0') << std::setw(2) << std::hex << static_cast<int>(array->get(i));
        }
        os << "\n";
    }
}
...
bool JSObject::PrintProperties(std::ostream& os)
{
    if (HasFastProperties()) {
        ...
        PrintDictionaryContents(os, Cast<JSGlobalObject>(*this)->global_dictionary(kAcquireLoad));
...
template <class T> bool IsTheHoleAt(Tagged<T> array, int index)
{
    return false;
}

template <> bool IsTheHoleAt(Tagged<FixedDoubleArray> array, int index)
{
    return array->is_the_hole(index);
}

template <class T> double GetScalarElement(Tagged<T> array, int index)
{
...
template <class T> void DoPrintElements(std::ostream& os, Tagged<Object> object, int length)
{
    ...
    for (i = 1; i <= length; i++) {
        ...
        if (i != length && (previous_value == value || values_are_nan) && IsTheHoleAt(array, i - 1) == IsTheHoleAt(array, i)) {
            continue;
        }
        os << "\n";
        std::stringstream ss;
...
struct Fp16Printer {
    uint16_t val;
    explicit Fp16Printer(float f)
        : val(fp16_ieee_from_fp32_value(f))
    {
    }
...
template <typename ElementType> void PrintTypedArrayElements(std::ostream& os, const ElementType* data_ptr, size_t length, bool is_on_heap)
{
    if (length == 0)
        return;
...
template <typename T> void PrintFixedArrayElements(std::ostream& os, Tagged<T> array, int capacity, Tagged<Object> (*get)(Tagged<T>, int))
{
...
template <typename T> void PrintFixedArrayElements(std::ostream& os, Tagged<T> array)
{
    PrintFixedArrayElements<T>(os, array, array->length(), [](Tagged<T> xs, int i) { return Cast<Object>(xs->get(i)); });
}
...
void PrintSloppyArgumentElements(std::ostream& os, ElementsKind kind, Tagged<SloppyArgumentsElements> elements)
{
    ...
    if (arguments_store->length() == 0)
        return;
    os << "\n }"
       << "\n - arguments_store: " << Brief(arguments_store) << " " << ElementsKindToString(arguments_store->map()->elements_kind()) << " {";
...
void PrintEmbedderData(IsolateForSandbox isolate, std::ostream& os, EmbedderDataSlot slot)
{
    DisallowGarbageCollection no_gc;
...
#define PRINT_ELEMENTS(Type, type, TYPE, elementType)                                                                                                          \
    case TYPE##_ELEMENTS: {                                                                                                                                    \
        ...
        const elementType* data_ptr = static_cast<const elementType*>(Cast<JSTypedArray>(*this)->DataPtr());                                                   \
...
#undef PRINT_ELEMENTS
    ...
    case SLOW_SLOPPY_ARGUMENTS_ELEMENTS:
        PrintSloppyArgumentElements(os, map()->elements_kind(), Cast<SloppyArgumentsElements>(elements()));
        break;
...
void JSObjectPrintHeader(std::ostream& os, Tagged<JSObject> obj, const char* id)
{
    ...
    os << "\n - elements: " << Brief(obj->elements()) << " [" << ElementsKindToString(obj->map()->elements_kind());
...
void JSAPIObjectWithEmbedderSlotsPrintHeader(std::ostream& os, Tagged<JSObject> obj, const char* id = nullptr)
{
    JSObjectPrintHeader(os, obj, id);
    os << "\n - cpp_heap_wrappable: " << obj->ReadField<uint32_t>(JSAPIObjectWithEmbedderSlots::kCppHeapWrappableOffset);
}

void JSObjectPrintBody(std::ostream& os, Tagged<JSObject> obj, bool print_elements = true)
{
    os << "\n - properties: ";
    ...
    if (print_elements) {
        size_t length = IsJSTypedArray(obj) ? Cast<JSTypedArray>(obj)->GetLength() : obj->elements()->length();
        if (length > 0)
...
void JSGeneratorObject::JSGeneratorObjectPrint(std::ostream& os)
{
    ...
    if (is_suspended()) {
        ...
        if (fun_info->HasSourceCode()) {
            ...
            Tagged<String> script_name = IsString(script->name()) ? Cast<String>(script->name()) : GetReadOnlyRoots().empty_string();

            os << "\n - source position: ";
            // Can't collect source positions here if not available as that would
            // allocate memory.
            Isolate* isolate = GetIsolate();
            if (fun_info->HasBytecodeArray() && fun_info->GetBytecodeArray(isolate)->HasSourcePositionTable()) {
                os << source_position();
                os << " (";
...
void DescriptorArray::DescriptorArrayPrint(std::ostream& os)
{
    ...
    os << "\n - raw gc state: mc epoch " << DescriptorArrayMarkingState::Epoch::decode(raw) << ", marked " << DescriptorArrayMarkingState::Marked::decode(raw)
       << ", delta " << DescriptorArrayMarkingState::Delta::decode(raw);
    PrintDescriptors(os);
}

namespace {
template <typename T> void PrintFixedArrayWithHeader(std::ostream& os, T* array, const char* type)
{
...
template <typename T> void PrintWeakArrayElements(std::ostream& os, T* array)
{
    // Print in array notation for non-sparse arrays.
    Tagged<MaybeObject> previous_value = array->length() > 0 ? array->get(0) : Tagged<MaybeObject>(kNullAddress);
...
void ObjectBoilerplateDescription::ObjectBoilerplateDescriptionPrint(std::ostream& os)
{
    ...
    os << "\n - flags: " << flags();
    os << "\n - elements:";
    PrintFixedArrayElements<ObjectBoilerplateDescription>(os, this, capacity(), [](Tagged<ObjectBoilerplateDescription> xs, int i) { return xs->get(i); });
    os << "\n";
}
...
void RegExpBoilerplateDescription::RegExpBoilerplateDescriptionPrint(std::ostream& os)
{
    IsolateForSandbox isolate = GetIsolateForSandbox(*this);
...
void ArrayList::ArrayListPrint(std::ostream& os)
{
    ...
    os << "\n - length: " << length();
    os << "\n - elements:";
    PrintFixedArrayElements<ArrayList>(os, this, length(), [](Tagged<ArrayList> xs, int i) { return xs->get(i); });
    os << "\n";
}
...
void ScriptContextTable::ScriptContextTablePrint(std::ostream& os)
{
    ...
    os << "\n - elements:";
    PrintFixedArrayElements<ScriptContextTable>(os, this, length(kAcquireLoad), [](Tagged<ScriptContextTable> xs, int i) { return Cast<Object>(xs->get(i)); });
    os << "\n";
}
...
void RegExpMatchInfo::RegExpMatchInfoPrint(std::ostream& os)
{
    ...
    os << "\n - captures:";
    PrintFixedArrayElements<RegExpMatchInfo>(os, this, capacity(), [](Tagged<RegExpMatchInfo> xs, int i) { return Cast<Object>(xs->get(i)); });
    os << "\n";
}
...
void SloppyArgumentsElements::SloppyArgumentsElementsPrint(std::ostream& os)
{
    ...
    PrintFixedArrayElements<SloppyArgumentsElements>(
        os, this, length(), [](Tagged<SloppyArgumentsElements> xs, int i) { return Cast<Object>(xs->mapped_entries(i, kRelaxedLoad)); });
    os << '\n';
}
...
void AccessorInfo::AccessorInfoPrint(std::ostream& os)
{
    ...
    os << "\n - getter_side_effect_type: " << SideEffectType2String(getter_side_effect_type());
    os << "\n - setter_side_effect_type: " << SideEffectType2String(setter_side_effect_type());
    ...
    if (GetIsolateFromHeapObject(*this, &isolate)) {
        ...
        if (USE_SIMULATOR_BOOL) {
            os << "\n - maybe_redirected_getter: " << reinterpret_cast<void*>(maybe_redirected_getter(isolate));
        }
...
{
    TorqueGeneratedFunctionTemplateInfo<FunctionTemplateInfo, TemplateInfo>::FunctionTemplateInfoPrint(os);

    Isolate* isolate;
    if (GetIsolateFromHeapObject(*this, &isolate)) {
        os << " - callback: " << reinterpret_cast<void*>(callback(isolate));
        if (USE_SIMULATOR_BOOL) {
            os << "\n - maybe_redirected_callback: " << reinterpret_cast<void*>(maybe_redirected_callback(isolate));
        }
    } else {
    ...
    if (allowed_receiver_instance_type_range_start() || allowed_receiver_instance_type_range_end()) {
        os << "\n - allowed_receiver_instance_type_range: [" << allowed_receiver_instance_type_range_start() << ", "
...
namespace {
void PrintContextWithHeader(std::ostream& os, Tagged<Context> context, const char* type)
{
...
template <typename T> void PrintTableContentsGeneric(std::ostream& os, T* dict, DataPrinter print_data_at)
{
    DisallowGarbageCollection no_gc;
    ReadOnlyRoots roots = dict->GetReadOnlyRoots();
...
template <typename T> void PrintDictionaryContentsFull(std::ostream& os, T* dict)
{
    os << "\n - elements: {";
...
template <typename T> void PrintHashMapContentsFull(std::ostream& os, T* dict)
{
    os << "\n - elements: {";
...
template <typename T> void PrintHashSetContentsFull(std::ostream& os, T* dict)
{
    os << "\n - elements: {";
...
template <typename T> void PrintOrderedHashTableHeaderAndBuckets(std::ostream& os, T* table, const char* type)
{
    DisallowGarbageCollection no_gc;
...
template <typename T> void PrintHashTableHeader(std::ostream& os, T* table, const char* type)
{
    PrintHeapObjectHeaderWithoutMap(table, os, type);
    os << "\n - FixedArray length: " << table->length();
    os << "\n - elements: " << table->NumberOfElements();
...
void SmallOrderedNameDictionary::SmallOrderedNameDictionaryPrint(std::ostream& os)
{
...
{
    os << "0x" << std::setfill('0') << std::setw(2) << std::right << std::hex << (value & 0xff) << std::setfill(' ');
}

void SwissNameDictionary::SwissNameDictionaryPrint(std::ostream& os)
{
    this->PrintHeader(os, "SwissNameDictionary");
    os << "\n - meta table ByteArray: " << reinterpret_cast<void*>(this->meta_table().ptr());
    ...
    for (int bucket = 0; bucket < this->Capacity(); ++bucket) {
        Tagged<Object> k;
        if (!this->ToKey(this->GetReadOnlyRoots(), bucket, &k))
            continue;
...
void FeedbackVector::FeedbackVectorPrint(std::ostream& os)
{
    ...
    if (has_optimized_code()) {
        os << "\n - optimized code: " << Brief(optimized_code(GetIsolateForSandbox(*this)));
    } else {
        os << "\n - no optimized code";
    }
    ...
    while (iter.HasNext()) {
        ...
        for (int i = 0; i < entry_size; i++) {
            ...
            os << "\n     [" << slot_with_offset.ToInt() << "]: " << Brief(Get(slot_with_offset));
        }
        if (entry_size > 0)
            os << "\n  }";
    }
...
void FeedbackNexus::Print(std::ostream& os)
{
    ...
    case FeedbackSlotKind::kStoreGlobalStrict: {
        ...
        if (ic_state() == InlineCacheState::MONOMORPHIC) {
            ...
            if (GetFeedback().IsCleared()) {
                ...
                os << "Handler(Lexical variable mode)(context ix = " << FeedbackNexus::ContextIndexBits::decode(handler)
    ...
    case FeedbackSlotKind::kLoadProperty: {
        ...
        if (ic_state() == InlineCacheState::MONOMORPHIC) {
            ...
            if (IsWeakFixedArray(handler) && !Cast<WeakFixedArray>(handler)->get(0).IsCleared()) {
    ...
    case FeedbackSlotKind::kSetKeyedStrict: {
        ...
        if (ic_state() == InlineCacheState::MONOMORPHIC) {
            ...
            if (IsName(feedback)) {
                os << " with name " << Brief(feedback);
                Tagged<WeakFixedArray> array = Cast<WeakFixedArray>(GetFeedbackExtra().GetHeapObject());
                os << "\n   " << Brief(array->get(0)) << ": ";
...
void JSDate::JSDatePrint(std::ostream& os)
{
    ...
    if (!IsSmi(year())) {
        ...
        SNPrintF(buf, "\n - time = %s %04d/%02d/%02d %02d:%02d:%02d\n", weekdays[IsSmi(weekday()) ? Smi::ToInt(weekday()) + 1 : 0],
            IsSmi(year()) ? Smi::ToInt(year()) : -1, IsSmi(month()) ? Smi::ToInt(month()) : -1, IsSmi(day()) ? Smi::ToInt(day()) : -1,
            IsSmi(hour()) ? Smi::ToInt(hour()) : -1, IsSmi(min()) ? Smi::ToInt(min()) : -1, IsSmi(sec()) ? Smi::ToInt(sec()) : -1);
        os << buf.begin();
    }
    JSObjectPrintBody(os, *this);
}
...
void JSCollectionIterator::JSCollectionIteratorPrint(std::ostream& os, const char* name)
{
...
void JSIteratorHelper::JSIteratorHelperPrintHeader(std::ostream& os, const char* helper_name)
{
...
void JSTypedArray::JSTypedArrayPrint(std::ostream& os)
{
    ...
    os << "\n   - base_pointer: " << reinterpret_cast<void*>(static_cast<Address>(base_ptr));
    os << "\n   - external_pointer: " << reinterpret_cast<void*>(external_pointer());
...
void JSFunction::JSFunctionPrint(std::ostream& os)
{
    ...
    int formal_parameter_count = shared()->internal_formal_parameter_count_with_receiver();
    ...
    if (has_feedback_vector() && raw_feedback_cell()->dispatch_handle() != dispatch_handle()) {
        os << "\n - canonical feedback cell dispatch_handle: 0x" << std::hex << raw_feedback_cell()->dispatch_handle() << std::dec;
    }
#endif // V8_ENABLE_LEAPTIERING
    ...
    if (WasmExportedFunction::IsWasmExportedFunction(*this)) {
        ...
        Tagged<WasmExportedFunctionData> data = function->shared()->wasm_exported_function_data();
    ...
    if (WasmJSFunction::IsWasmJSFunction(*this)) {
        ...
        os << "\n - Wasm wrapper around: " << Brief(function->shared()->wasm_js_function_data()->GetCallable());
    }
#endif // V8_ENABLE_WEBASSEMBLY
...
void SharedFunctionInfo::SharedFunctionInfoPrint(std::ostream& os)
{
    ...
    os << "\n - expected_nof_properties: " << static_cast<int>(expected_nof_properties());
    os << "\n - language_mode: " << language_mode();
    os << "\n - trusted_function_data: " << Brief(GetTrustedData(GetIsolateForSandbox(*this)));
...
void SharedFunctionInfoWrapper::SharedFunctionInfoWrapperPrint(std::ostream& os)
{
...
void Code::CodePrint(std::ostream& os, const char* name, Address current_pc)
{
    ...
    os << "\n - deoptimization_data_or_interpreter_data: " << Brief(raw_deoptimization_data_or_interpreter_data());
    os << "\n - position_table: " << Brief(raw_position_table());
    os << "\n - parameter_count: " << parameter_count();
    os << "\n - instruction_stream: " << Brief(raw_instruction_stream());
    os << "\n - instruction_start: " << reinterpret_cast<void*>(instruction_start());
    ...
    if (kind() != CodeKind::WASM_TO_JS_FUNCTION) {
        ...
        os << "\n - wasm_js_tagged_parameter_count: " << wasm_js_tagged_parameter_count();
        os << "\n - wasm_js_first_tagged_parameter: " << wasm_js_first_tagged_parameter();
    }
    ...
    if (has_instruction_stream()) {
        ...
        os << "\n - instruction_stream.relocation_info: " << Brief(istream->relocation_info());
...
void Foreign::ForeignPrint(std::ostream& os)
{
    ...
    os << "\n - foreign address: " << reinterpret_cast<void*>(foreign_address_unchecked());
    os << "\n";
}
...
void ArrayBoilerplateDescription::ArrayBoilerplateDescriptionPrint(std::ostream& os)
{
...
void WasmStruct::WasmStructPrint(std::ostream& os)
{
    ...
    for (uint32_t i = 0; i < struct_type->field_count(); i++) {
        ...
        case wasm::kI64:
            ...
            break;
        case wasm::kF16:
            os << fp16_ieee_to_fp32_value(base::ReadUnalignedValue<uint16_t>(field_address));
            break;
        case wasm::kF32:
...
#endif
                os << std::setw(2) << static_cast<int>(reinterpret_cast<uint8_t*>(field_address)[j]);
            }
...
void WasmArray::WasmArrayPrint(std::ostream& os)
{
    ...
    case wasm::kI32:
        PrintTypedArrayElements(os, reinterpret_cast<int32_t*>(data_ptr), len, true);
        break;
    case wasm::kI64:
        PrintTypedArrayElements(os, reinterpret_cast<int64_t*>(data_ptr), len, true);
        break;
    case wasm::kF16:
        PrintTypedArrayElements(os, reinterpret_cast<Fp16Printer*>(data_ptr), len, true);
        break;
    case wasm::kF32:
        PrintTypedArrayElements(os, reinterpret_cast<float*>(data_ptr), len, true);
        break;
    case wasm::kF64:
        PrintTypedArrayElements(os, reinterpret_cast<double*>(data_ptr), len, true);
        break;
    case wasm::kI8:
        PrintTypedArrayElements(os, reinterpret_cast<int8_t*>(data_ptr), len, true);
        break;
    case wasm::kI16:
        PrintTypedArrayElements(os, reinterpret_cast<int16_t*>(data_ptr), len, true);
        break;
    case wasm::kRef:
    case wasm::kRefNull: {
        os << "\n - elements:";
        constexpr uint32_t kWasmArrayMaximumPrintedElements = 5;
        for (uint32_t i = 0; i < std::min(this->length(), kWasmArrayMaximumPrintedElements); i++) {
            os << "\n   " << static_cast<int>(i) << " - " << Brief(TaggedField<Object>::load(*this, this->element_offset(i)));
        }
        if (this->length() > kWasmArrayMaximumPrintedElements)
            os << "\n   ...";
        break;
    }
    case wasm::kS128: {
        ...
        constexpr uint32_t kWasmArrayMaximumPrintedElements = 5;
        for (uint32_t i = 0; i < std::min(this->length(), kWasmArrayMaximumPrintedElements); i++) {
            os << "\n   " << static_cast<int>(i) << " - 0x" << std::hex << std::setfill('0');
#ifdef V8_TARGET_BIG_ENDIAN
            for (int j = 0; j < kSimd128Size; j++) {
#else
            for (int j = kSimd128Size - 1; j >= 0; j--) {
#endif
                os << std::setw(2) << static_cast<int>(reinterpret_cast<uint8_t*>(this->ElementAddress(i))[j]);
            }
            os << std::dec << std::setfill(' ');
        }
        if (this->length() > kWasmArrayMaximumPrintedElements)
...
{
#define PRINT_WASM_INSTANCE_FIELD(name, convert) os << "\n - " #name ": " << convert(name());
...
void WasmDispatchTable::WasmDispatchTablePrint(std::ostream& os)
{
    ...
    for (int i = 0; i < printed; ++i) {
        os << "\n " << std::setw(8) << i << ": sig: " << sig(i) << "; target: " << AsHex::Address(target(i)) << "; implicit_arg: " << Brief(implicit_arg(i));
    }
    if (printed != len)
        os << "\n  [...]";
    os << "\n";
...
void WasmImportData::WasmImportDataPrint(std::ostream& os)
{
    ...
    os << "\n - sig: " << sig() << " (" << sig()->parameter_count() << " params, " << sig()->return_count() << " returns)";
    os << "\n";
}
...
void AllocationSite::AllocationSitePrint(std::ostream& os)
{
    ...
    os << "\n - memento found count: " << Brief(Smi::FromInt(memento_found_count()));
    os << "\n - memento create count: " << Brief(Smi::FromInt(memento_create_count()));
    os << "\n - pretenure decision: " << Brief(Smi::FromInt(pretenure_decision()));
    os << "\n - transition_info: ";
    if (!PointsToLiteral()) {
...
void Script::ScriptPrint(std::ostream& os)
{
    ...
    os << "\n - compiled lazy function positions: " << compiled_lazy_function_positions();
    bool is_wasm = false;
#if V8_ENABLE_WEBASSEMBLY
...
namespace {
void PrintScopeInfoList(Tagged<ScopeInfo> scope_info, std::ostream& os, const char* list_name, int length)
{
    DisallowGarbageCollection no_gc;
    if (length <= 0)
        return;
...
void ScopeInfo::ScopeInfoPrint(std::ostream& os)
{
    ...
    if (HasInlinedLocalNames()) {
        ...
        os << "\n - local names in a hashtable: " << Brief(context_local_names_hashtable());
    }
    ...
    if (ClassScopeHasPrivateBrand())
        os << "\n - class scope has private brand";
    if (HasSavedClassVariable())
...
void HeapObject::HeapObjectShortPrint(std::ostream& os)
{
    ...
    case SCRIPT_CONTEXT_TABLE_TYPE:
        os << "<ScriptContextTable[" << Cast<ScriptContextTable>(*this)->capacity() << "]>";
        break;
    case HASH_TABLE_TYPE:
    ...
    case SWISS_NAME_DICTIONARY_TYPE:
        os << "<SwissNameDictionary[" << Cast<SwissNameDictionary>(*this)->Capacity() << "]>";
        break;
    case GLOBAL_DICTIONARY_TYPE:
    ...
    case OBJECT_BOILERPLATE_DESCRIPTION_TYPE:
        os << "<ObjectBoilerplateDescription[" << Cast<ObjectBoilerplateDescription>(*this)->capacity() << "]>";
        break;
    ...
    case DESCRIPTOR_ARRAY_TYPE:
        os << "<DescriptorArray[" << Cast<DescriptorArray>(*this)->number_of_descriptors() << "]>";
        break;
    ...
    case CLOSURE_FEEDBACK_CELL_ARRAY_TYPE:
        os << "<ClosureFeedbackCellArray[" << Cast<ClosureFeedbackCellArray>(*this)->length() << "]>";
        break;
    ...
    case PREPARSE_DATA_TYPE: {
        ...
        os << "<PreparseData[data=" << data->data_length() << " children=" << data->children_length() << "]>";
        break;
    }

    case UNCOMPILED_DATA_WITHOUT_PREPARSE_DATA_TYPE: {
        Tagged<UncompiledDataWithoutPreparseData> data = Cast<UncompiledDataWithoutPreparseData>(*this);
        os << "<UncompiledDataWithoutPreparseData (" << data->start_position() << ", " << data->end_position() << ")]>";
        break;
    }

    case UNCOMPILED_DATA_WITH_PREPARSE_DATA_TYPE: {
        Tagged<UncompiledDataWithPreparseData> data = Cast<UncompiledDataWithPreparseData>(*this);
        os << "<UncompiledDataWithPreparseData (" << data->start_position() << ", " << data->end_position() << ") preparsed=" << Brief(data->preparse_data())
           << ">";
        break;
    }
...
void HeapNumber::HeapNumberShortPrint(std::ostream& os)
{
    ...
    static constexpr int64_t kMinSafeInteger = static_cast<int64_t>(kUint64AllBitsSet << 53);
    static constexpr int64_t kMaxSafeInteger = -(kMinSafeInteger + 1);

    double val = value();
    if (i::IsMinusZero(val)) {
        os << "-0.0";
    } else if (val == DoubleToInteger(val) && val >= static_cast<double>(kMinSafeInteger) && val <= static_cast<double>(kMaxSafeInteger)) {
...
int Name::NameShortPrint(base::Vector<char> str)
{
    if (IsString(this)) {
        ...
        if (IsUndefined(s->description())) {
            ...
            return SNPrintF(str, "<%s>", Cast<String>(s->description())->ToCString().get());
        }
    }
}
...
void Map::MapPrint(std::ostream& os)
{
    ...
    os << "\n - prototype_validity cell: " << Brief(prototype_validity_cell(kRelaxedLoad));
    os << "\n - instance descriptors " << (owns_descriptors() ? "(own) " : "") << "#" << NumberOfOwnDescriptors() << ": " << Brief(instance_descriptors());
    ...
    if (!HeapLayout::InReadOnlySpace(*this)) {
        ...
        if (nof_transitions > 0 || transitions.HasPrototypeTransitions() || transitions.HasSideStepTransitions()) {
...
void DescriptorArray::PrintDescriptorDetails(std::ostream& os, InternalIndex descriptor, PropertyDetails::PrintMode mode)
{
    ...
    case PropertyLocation::kDescriptor:
        ...
        if (IsAccessorPair(value)) {
            ...
            os << "(get: " << Brief(pair->getter()) << ", set: " << Brief(pair->setter()) << ")";
        }
        break;
    }
}
...
// static
void TransitionsAccessor::PrintOneTransition(std::ostream& os, Tagged<Name> key, Tagged<Map> target)
{
    os << "\n     ";
#ifdef OBJECT_PRINT
    key->NamePrint(os);
    ...
    if (key == roots.nonextensible_symbol()) {
        ...
        descriptors->PrintDescriptorDetails(os, descriptor, PropertyDetails::kForTransitions);
        os << ")";
    }
    os << " -> " << Brief(target);
}
...
void TransitionArray::PrintInternal(std::ostream& os)
{
    ...
    if (HasPrototypeTransitions()) {
        ...
        os << "\n   Prototype transitions #" << num_transitions << ": " << Brief(prototype_transitions);
        for (int i = 0; i < num_transitions; i++) {
            auto maybe = prototype_transitions->get(TransitionArray::kProtoTransitionHeaderSize + i);
            Tagged<HeapObject> target;
            if (maybe.GetHeapObjectIfWeak(&target)) {
                auto map = Cast<Map>(target);
                os << "\n     " << Brief(map->prototype()) << " -> " << Brief(Cast<Map>(target));
            }
        }
    }
    ...
    if (HasSideStepTransitions()) {
        ...
        os << "\n   Sidestep transitions #" << num_transitions << ": " << Brief(sidestep_transitions);
        for (int i = 0; i < num_transitions; i++) {
...
void TransitionsAccessor::PrintTransitions(std::ostream& os)
{
    ...
    case kMigrationTarget:
        return;
    case kWeakRef: {
        Tagged<Map> target = Cast<Map>(raw_transitions_.GetHeapObjectAssumeWeak());
...
{
    StdoutStream os;
    os << (IsUndefined(map_->GetBackPointer()) ? "root_" : "") << "map= " << Brief(map_);
    DisallowGarbageCollection no_gc;
    PrintTransitionTree(os, 0, &no_gc);
    os << "\n" << std::flush;
}

void TransitionsAccessor::PrintTransitionTree(std::ostream& os, int level, DisallowGarbageCollection* no_gc)
{
    ...
    ForEachTransitionWithKey(
        ...
        [&](Tagged<Name> key, Tagged<Map> target) {
            os << std::endl << "  " << level << "/" << pos << ":" << std::setw(level * 2 + 2) << " ";
            pos++;
            std::stringstream ss;
            ss << Brief(target);
            ...
            if (key == roots.nonextensible_symbol()) {
                ...
                os << "to ";
                InternalIndex descriptor = target->LastAdded();
                Tagged<DescriptorArray> descriptors = target->instance_descriptors(isolate_);
                descriptors->PrintDescriptorDetails(os, descriptor, PropertyDetails::kForTransitions);
            }
        ...
        [&](Tagged<Map> target) {
            os << std::endl << "  " << level << "/p" << proto_pos << ":" << std::setw(level * 2 + 2) << " ";
            proto_pos++;
            std::stringstream ss;
            ss << Brief(target);
        ...
        [&](SideStepTransition::Kind kind, Tagged<Object> side_step) {
            os << std::endl << "  " << level << "/s:" << std::setw(level * 2 + 2) << " ";
            std::stringstream ss;
            ss << Brief(side_step);
...
inline i::Tagged<i::Object> GetObjectFromRaw(void* object)
{
    ...
    if (RoundDown<i::kPtrComprCageBaseAlignment>(object_ptr) == i::kNullAddress) {
        ...
        if (isolate != nullptr) {
            object_ptr = i::V8HeapCompressionScheme::DecompressTagged(isolate, static_cast<i::Tagged_t>(object_ptr));
        } else {
            i::PtrComprCageBase cage_base = i::GetPtrComprCageBase();
            object_ptr = i::V8HeapCompressionScheme::DecompressTagged(cage_base, static_cast<i::Tagged_t>(object_ptr));
        }
    }
#endif
...
V8_EXPORT_PRIVATE extern i::Tagged<i::Object> _v8_internal_Get_Object(void* object)
{
...
V8_DONT_STRIP_SYMBOL
V8_EXPORT_PRIVATE extern std::string _v8_internal_Print_Object_To_String(void* object)
{
...
V8_EXPORT_PRIVATE extern void _v8_internal_Print_Code(void* object)
{
...
#if V8_ENABLE_WEBASSEMBLY
    {
        if (auto* wasm_code = i::wasm::GetWasmCodeManager()->LookupCode(isolate, address)) {
            i::StdoutStream os;
    ...
    std::optional<i::Tagged<i::Code>> lookup_result = isolate->heap()->TryFindCodeForInnerPointerForPrinting(address);
    if (!lookup_result.has_value()) {
        i::PrintF("%p is not within the current isolate's code or embedded spaces\n", object);
        return;
    }
...
#ifdef V8_ENABLE_LEAPTIERING
V8_DONT_STRIP_SYMBOL
V8_EXPORT_PRIVATE extern void _v8_internal_Print_Dispatch_Handle(uint32_t handle)
{
...
V8_DONT_STRIP_SYMBOL
V8_EXPORT_PRIVATE extern void _v8_internal_Print_OnlyCode(void* object, size_t range_limit)
{
    i::Address address = reinterpret_cast<i::Address>(object);
    i::Isolate* isolate = i::Isolate::Current();
    ...
    i::StdoutStream os;
    lookup_result.value()->DisassembleOnlyCode(nullptr, os, isolate, address, range_limit);
#endif
}
...
V8_DONT_STRIP_SYMBOL
V8_EXPORT_PRIVATE extern std::vector<_v8_internal_debugonly::StackTraceDebugDetails> _v8_internal_Expand_StackTrace(i::Isolate* isolate)
{
...
V8_DONT_STRIP_SYMBOL
V8_EXPORT_PRIVATE extern void _v8_internal_Print_TransitionTree(void* object, bool start_at_root = false)
{
...
{
#ifdef OBJECT_PRINT
    const auto mark_bit = v8::internal::MarkBit::From(reinterpret_cast<i::Address>(object));
    i::StdoutStream os;
    os << "Object " << object << " is " << (mark_bit.Get() ? "marked" : "unmarked") << std::endl;
    os << "  mark-bit cell: " << mark_bit.CellAddress() << ", mask: " << mark_bit.Mask() << std::endl;
#endif
}

V8_DONT_STRIP_SYMBOL
V8_EXPORT_PRIVATE void _v8_internal_Print_FunctionCallbackInfo(void* function_callback_info)
{
#ifdef OBJECT_PRINT
...
V8_DONT_STRIP_SYMBOL
V8_EXPORT_PRIVATE void _v8_internal_Print_PropertyCallbackInfo(void* property_callback_info)
{
#ifdef OBJECT_PRINT
