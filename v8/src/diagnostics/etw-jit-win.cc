...
public:
    ...
    static void EnableLogWithFilterData(Isolate* isolate, size_t event_id, const std::string& EnableLogWithFilterData,
        std::weak_ptr<EtwIsolateCaptureStateMonitor> weak_monitor, uint32_t options);
...
private:
    ...
    void EnqueueEnableLog(std::weak_ptr<EtwIsolateCaptureStateMonitor> weak_monitor, uint32_t options)
    {
        ...
        isolate_->RequestInterrupt(
            ...
            [](v8::Isolate* v8_isolate, void* data) {
                std::unique_ptr<EnableInterruptData> interrupt_data(reinterpret_cast<EnableInterruptData*>(data));
                ...
                EnableLog(reinterpret_cast<Isolate*>(v8_isolate), event_id, weak_monitor, options);
            },
    ...
    void EnqueueDisableLog()
    {
        ...
        isolate_->RequestInterrupt(
            // Executed in the isolate thread.
            [](v8::Isolate* v8_isolate, void* data) { DisableLog(reinterpret_cast<Isolate*>(v8_isolate), reinterpret_cast<size_t>(data)); },
    ...
    void EnqueueEnableLogWithFilterData(const std::string& etw_filter_payload, std::weak_ptr<EtwIsolateCaptureStateMonitor> weak_monitor, uint32_t options)
    {
        ...
        isolate_->RequestInterrupt(
            // Executed in the isolate thread.
            [](v8::Isolate* v8_isolate, void* data) {
                std::unique_ptr<EnableWithFilterDataInterruptData> interrupt_data(reinterpret_cast<EnableWithFilterDataInterruptData*>(data));
                size_t event_id = interrupt_data->event_id;
                std::string etw_filter_payload = interrupt_data->payload;
                auto weak_monitor = interrupt_data->weak_monitor;
                uint32_t options = interrupt_data->options;
                EnableLogWithFilterData(reinterpret_cast<Isolate*>(v8_isolate), event_id, etw_filter_payload, weak_monitor, options);
            },
            new EnableWithFilterDataInterruptData { event_id + 1, etw_filter_payload, weak_monitor, options });
    }
...
using IsolateMapType = std::unordered_map<v8::internal::Isolate*, IsolateLoadScriptData>;
static base::LazyInstance<IsolateMapType>::type isolate_map = LAZY_INSTANCE_INITIALIZER;

using FilterDataType = std::string;
// Used when Isolates are created during an ETW tracing session.
static base::LazyInstance<FilterDataType>::type etw_filter_payload = LAZY_INSTANCE_INITIALIZER;
...
// static
void IsolateLoadScriptData::EnableLog(Isolate* isolate, size_t event_id, std::weak_ptr<EtwIsolateCaptureStateMonitor> weak_monitor, uint32_t options)
{
    {
        ETWTRACEDBG << "EnableLog called with event_id==" << event_id << " and options==" << options << " taking mutex" << std::endl;
        base::MutexGuard guard(isolates_mutex.Pointer());
...
void IsolateLoadScriptData::EnableLogWithFilterData(
    Isolate* isolate, size_t event_id, const std::string& etw_filter_payload, std::weak_ptr<EtwIsolateCaptureStateMonitor> weak_monitor, uint32_t options)
{
    {
        ETWTRACEDBG << "EnableLogWithFilterData called with event_id==" << event_id << " and options==" << options << " taking mutex" << std::endl;
        base::MutexGuard guard(isolates_mutex.Pointer());
        auto& data = GetData(isolate);
        if (event_id > 0 && data.CurrentEventId() != event_id) {
            // This interrupt was canceled by a newer interrupt.
...
// static
void IsolateLoadScriptData::EnableLogWithFilterDataOnAllIsolates(const uint8_t* data, size_t size, uint32_t options)
{
    base::MutexGuard guard(isolates_mutex.Pointer());

    std::string etw_filter_payload;
    etw_filter_payload.assign(data, data + size);
    auto monitor = std::make_shared<EtwIsolateCaptureStateMonitor>(isolates_mutex.Pointer(), isolate_map.Pointer()->size());
    bool capture_state = (options & kJitCodeEventEnumExisting) == kJitCodeEventEnumExisting;
    std::weak_ptr<EtwIsolateCaptureStateMonitor> weak_monitor = monitor;
    std::for_each(isolate_map.Pointer()->begin(), isolate_map.Pointer()->end(), [&etw_filter_payload, weak_monitor, options](auto& pair) {
        auto& isolate_data = pair.second;
        isolate_data.EnqueueEnableLogWithFilterData(etw_filter_payload, weak_monitor, options);
    });
    ...
    ETWTRACEDBG << "EnableLogWithFilterDataOnAllIsolates WaitFor " << (timeout ? "timeout" : "completed") << std::endl;
}

// static
void IsolateLoadScriptData::UpdateAllIsolates(bool etw_enabled, uint32_t options)
{
    ETWTRACEDBG << "UpdateAllIsolates with etw_enabled==" << etw_enabled << " and options==" << options << " acquiring mutex" << std::endl;
    base::MutexGuard guard(isolates_mutex.Pointer());
    ETWTRACEDBG << "UpdateAllIsolates Isolate count==" << isolate_map.Pointer()->size() << std::endl;
    auto monitor = std::make_shared<EtwIsolateCaptureStateMonitor>(isolates_mutex.Pointer(), isolate_map.Pointer()->size());
    bool capture_state = (options & kJitCodeEventEnumExisting) == kJitCodeEventEnumExisting;
    std::weak_ptr<EtwIsolateCaptureStateMonitor> weak_monitor = monitor;
    std::for_each(isolate_map.Pointer()->begin(), isolate_map.Pointer()->end(), [etw_enabled, weak_monitor, options](auto& pair) {
        auto& isolate_data = pair.second;
        if (etw_enabled) {
    ...
    bool timeout = !monitor->WaitFor(kCaptureStateTimeout);
    ETWTRACEDBG << "UpdateAllIsolates WaitFor " << (timeout ? "timeout" : "completed") << std::endl;
}

// static
bool IsolateLoadScriptData::MaybeAddLoadedScript(Isolate* isolate, int script_id)
{
    base::MutexGuard guard(isolates_mutex.Pointer());
    auto& data = GetData(isolate);
...
void MaybeSetHandlerNow(Isolate* isolate)
{
    ...
    if (is_etw_enabled && !isolate->heap()->read_only_space()->writable()) {
        if (etw_filter_payload.Pointer()->empty()) {
            IsolateLoadScriptData::EnableLog(isolate, 0, std::weak_ptr<EtwIsolateCaptureStateMonitor>(), kJitCodeEventDefault);
        } else {
            IsolateLoadScriptData::EnableLogWithFilterData(
                isolate, 0, *etw_filter_payload.Pointer(), std::weak_ptr<EtwIsolateCaptureStateMonitor>(), kJitCodeEventDefault);
        }
    }
}
...
{
    return event->script.IsEmpty() ? Tagged<SharedFunctionInfo>() : *Utils::OpenDirectHandle(*event->script);
}
...
std::wstring GetScriptMethodNameFromEvent(const JitCodeEvent* event)
{
    ...
    MultiByteToWideChar(CP_UTF8, 0, event->name.str, name_len,
        ...
        const_cast<LPWSTR>(method_name.data()), static_cast<int>(method_name.size()));
    return method_name;
}
...
std::wstring GetScriptMethodNameFromSharedFunctionInfo(Tagged<SharedFunctionInfo> sfi)
{
    ...
    MultiByteToWideChar(CP_UTF8, 0, sfi_name.get(), method_name_length, const_cast<LPWSTR>(method_name.data()), static_cast<int>(method_name.length()));
    return method_name;
}
...
std::wstring GetScriptMethodName(const JitCodeEvent* event)
{
    ...
    return sfi.is_null() ? GetScriptMethodNameFromEvent(event) : GetScriptMethodNameFromSharedFunctionInfo(sfi);
}
...
void WINAPI ETWEnableCallback(LPCGUID /* source_id */, ULONG is_enabled, UCHAR level, ULONGLONG match_any_keyword, ULONGLONG match_all_keyword,
    PEVENT_FILTER_DESCRIPTOR filter_data, PVOID /* callback_context */)
{
    DCHECK(v8_flags.enable_etw_stack_walking);
    ETWTRACEDBG << "ETWEnableCallback called with is_enabled==" << is_enabled << std::endl;

    bool is_etw_enabled_now = is_enabled && level >= kTraceLevel && (match_any_keyword & kJScriptRuntimeKeyword)
    ...
    if (!is_etw_enabled_now || !filter_data || filter_data->Type != EVENT_FILTER_TYPE_SCHEMATIZED) {
        etw_filter->clear();
    ...
    EVENT_FILTER_HEADER* filter_event_header = reinterpret_cast<EVENT_FILTER_HEADER*>(filter_data->Ptr);
    if (filter_event_header->Size < sizeof(EVENT_FILTER_HEADER)) {
        return; // Invalid data
    }

    const uint8_t* payload_start = reinterpret_cast<uint8_t*>(filter_event_header) + sizeof(EVENT_FILTER_HEADER);
    const size_t payload_size = filter_event_header->Size - sizeof(EVENT_FILTER_HEADER);
    ...
    IsolateLoadScriptData::EnableLogWithFilterDataOnAllIsolates(reinterpret_cast<const uint8_t*>(etw_filter->data()), etw_filter->size(), options);
}
...
void EventHandler(const JitCodeEvent* event)
{
    if (!is_etw_enabled)
        return;
    if (event->code_type != v8::JitCodeEvent::CodeType::JIT_CODE)
        return;
    ...
    if (!sfi.is_null() && IsScript(sfi->script())) {
        ...
        if (IsolateLoadScriptData::MaybeAddLoadedScript(isolate, script_id)) {
            ...
            if (IsString(script_name)) {
                ...
                uint16_t* wstr_data = const_cast<uint16_t*>(reinterpret_cast<const uint16_t*>(wstr_name.data()));
                String::WriteToFlat(v8str_name, wstr_data, 0, v8str_name->length());
            }

            constexpr static auto source_load_event_meta = EventMetadata(kSourceLoadEventID, kJScriptRuntimeKeyword);
            constexpr static auto source_load_event_fields = EventFields("SourceLoad", Field("SourceID", TlgInUINT64), Field("ScriptContextID", TlgInPOINTER),
                Field("SourceFlags", TlgInUINT32), Field("Url", TlgInUNICODESTRING));
            LogEventData(g_v8Provider, &source_load_event_meta, &source_load_event_fields, (uint64_t)script_id, script_context,
    ...
    auto code = isolate->heap()->GcSafeTryFindCodeForInnerPointer(Address(event->code_start));
    if (code && code.value()->is_builtin()) {
        ...
        if (code.value()->has_instruction_stream()) {
            ...
            DCHECK_IMPLIES(
                code.value()->is_builtin(), code.value()->builtin_id() == Builtin::kInterpreterEntryTrampoline && v8_flags.interpreted_frames_native_stack);
        } else {
            DCHECK(code.value()->is_builtin());
        }

        // If the builtin has been relocated, we still need to emit the address
        if (skip_emitting_builtin && V8_SHORT_BUILTIN_CALLS_BOOL && v8_flags.short_builtin_calls) {
    ...
    constexpr static auto method_load_event_meta = EventMetadata(kMethodLoadEventID, kJScriptRuntimeKeyword);
    constexpr static auto method_load_event_fields
        = EventFields("MethodLoad", Field("ScriptContextID", TlgInPOINTER), Field("MethodStartAddress", TlgInPOINTER), Field("MethodSize", TlgInUINT64),
            Field("MethodID", TlgInUINT32), Field("MethodFlags", TlgInUINT16), Field("MethodAddressRangeID", TlgInUINT16), Field("SourceID", TlgInUINT64),
            Field("Line", TlgInUINT32), Field("Column", TlgInUINT32), Field("MethodName", TlgInUNICODESTRING));

    LogEventData(g_v8Provider, &method_load_event_meta, &method_load_event_fields, script_context, event->code_start, (uint64_t)event->code_len,
        (uint32_t)0, // MethodId
