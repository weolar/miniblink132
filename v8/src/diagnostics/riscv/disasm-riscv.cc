...
void Decoder::PrintTarget(Instruction* instr)
{
    if (Assembler::IsJalr(instr->InstructionBits())) {
        if (Assembler::IsAuipc((instr - 4)->InstructionBits()) && (instr - 4)->RdValue() == instr->Rs1Value()) {
            int32_t imm = Assembler::BrachlongOffset((instr - 4)->InstructionBits(), instr->InstructionBits());
            const char* target = converter_.NameOfAddress(reinterpret_cast<uint8_t*>(instr - 4) + imm);
            out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, " -> %s", target);
            return;
        }
    }
}
...
void Decoder::PrintBranchOffset(Instruction* instr)
{
    ...
    const char* target = converter_.NameOfAddress(reinterpret_cast<uint8_t*>(instr) + imm);
    out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "%d -> %s", imm, target);
}
...
void Decoder::PrintCSRReg(Instruction* instr)
{
    ...
    out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "%s", s.c_str());
}
...
// complexity of FormatOption.
int Decoder::FormatFPURegisterOrRoundMode(Instruction* instr, const char* format)
{
...
void Decoder::DecodeIType(Instruction* instr)
{
    ...
    case RO_JALR:
        if (instr->RdValue() == zero_reg.code() && instr->Rs1Value() == ra.code() && instr->Imm12Value() == 0)
            Format(instr, "ret");
    ...
    case RO_ADDI:
        if (instr->Imm12Value() == 0) {
            if (instr->RdValue() == zero_reg.code() && instr->Rs1Value() == zero_reg.code())
                Format(instr, "nop");
            else
...
#endif /*V8_TARGET_ARCH_64_BIT*/
    case RO_FENCE:
        if (instr->MemoryOrder(true) == PSIORW && instr->MemoryOrder(false) == PSIORW)
            Format(instr, "fence");
        else
...
{
    char str[50];
    uint32_t instr_temp = instr->InstructionBits() & (kRvvMopMask | kRvvNfMask | kBaseOpcodeMask);
    ...
    if (RO_V_VL == instr_temp) {
        if (!(instr->InstructionBits() & (kRvvRs2Mask))) {
            snprintf(str, sizeof(str), "vle%d.v    'vd, ('rs1)'vm", instr->vl_vs_width());
            Format(instr, str);
        } else {
            snprintf(str, sizeof(str), "vle%dff.v  'vd, ('rs1)'vm", instr->vl_vs_width());
            Format(instr, str);
        }
    } else if (RO_V_VLS == instr_temp) {
        snprintf(str, sizeof(str), "vlse%d.v   'vd, ('rs1), 'rs2'vm", instr->vl_vs_width());
        Format(instr, str);

    } else if (RO_V_VLX == instr_temp) {
        snprintf(str, sizeof(str), "vlxei%d.v  'vd, ('rs1), 'vs2'vm", instr->vl_vs_width());
        Format(instr, str);
        ...
        if (!(instr->InstructionBits() & (kRvvRs2Mask))) {
            snprintf(str, sizeof(str), "vlseg%de%d.v  'vd, ('rs1)'vm", switch_nf(instr), instr->vl_vs_width());
        } else {
            snprintf(str, sizeof(str), "vlseg%de%dff.v  'vd, ('rs1)'vm", switch_nf(instr), instr->vl_vs_width());
        }
        Format(instr, str);
        ...
        snprintf(str, sizeof(str), "vlsseg%de%d.v  'vd, ('rs1), 'rs2'vm", switch_nf(instr), instr->vl_vs_width());
        Format(instr, str);
        ...
        snprintf(str, sizeof(str), "vlxseg%dei%d.v  'vd, ('rs1), 'vs2'vm", switch_nf(instr), instr->vl_vs_width());
        Format(instr, str);
    }
}
...
{
    int width = 0;
    if ((instr->InstructionBits() & kBaseOpcodeMask) != LOAD_FP && (instr->InstructionBits() & kBaseOpcodeMask) != STORE_FP)
        return -1;
...
void Decoder::DecodeRvvVS(Instruction* instr)
{
    ...
    if (RO_V_VS == instr_temp) {
        snprintf(str, sizeof(str), "vse%d.v    'vd, ('rs1)'vm", instr->vl_vs_width());
        Format(instr, str);
    } else if (RO_V_VSS == instr_temp) {
        snprintf(str, sizeof(str), "vsse%d.v      'vd, ('rs1), 'rs2'vm", instr->vl_vs_width());
        Format(instr, str);
    } else if (RO_V_VSX == instr_temp) {
        snprintf(str, sizeof(str), "vsxei%d.v      'vd, ('rs1), 'vs2'vm", instr->vl_vs_width());
        Format(instr, str);
    } else if (RO_V_VSU == instr_temp) {
        snprintf(str, sizeof(str), "vsuxei%d.v      'vd, ('rs1), 'vs2'vm", instr->vl_vs_width());
        Format(instr, str);
        ...
        snprintf(str, sizeof(str), "vsseg%de%d.v      'vd, ('rs1)'vm", switch_nf(instr), instr->vl_vs_width());
        Format(instr, str);
        ...
        snprintf(str, sizeof(str), "vssseg%de%d.v      'vd, ('rs1), 'rs2'vm", switch_nf(instr), instr->vl_vs_width());
        Format(instr, str);
        ...
        snprintf(str, sizeof(str), "vsxseg%dei%d.v      'vd, ('rs1), 'vs2'vm", switch_nf(instr), instr->vl_vs_width());
        Format(instr, str);
    }
}
...
{
    return v8::internal::Assembler::ConstantPoolSizeAt(reinterpret_cast<v8::internal::Instruction*>(instruction));
}
...
void Disassembler::Disassemble(FILE* f, uint8_t* begin, uint8_t* end, UnimplementedOpcodeAction unimplemented_action)
{
    ...
    for (uint8_t* pc = begin; pc < end;) {
        ...
        uint8_t* prev_pc = pc;
        pc += d.InstructionDecode(buffer, pc);
        v8::internal::PrintF(f, "%p    %08x      %s\n", static_cast<void*>(prev_pc), *reinterpret_cast<uint32_t*>(prev_pc), buffer.begin());
    }
}
