...
public:
    ...
    template <typename T> Slot<T> SlotAt(uintptr_t offset)
    {
        Ensure(offset + sizeof(T));
        return Slot<T>(this, offset);
    }

    template <typename T> Slot<T> CreateSlotHere()
    {
        return CreateSlotsHere<T>(1);
    }

    template <typename T> Slot<T> CreateSlotsHere(uint32_t count)
    {
...
private:
    ...
    template <typename T> Address AddressAt(uintptr_t offset)
    {
        DCHECK(offset < capacity_ && offset + sizeof(T) <= capacity_);
        return reinterpret_cast<Address>(&buffer_[offset]);
    }

    template <typename T> T* RawSlotAt(uintptr_t offset)
    {
        DCHECK(offset < capacity_ && offset + sizeof(T) <= capacity_);
...
template <typename THeader> class DebugSectionBase : public ZoneObject {
public:
...
public:
    ...
    MachOSection(const char* name, const char* segment, uint32_t align, uint32_t flags)
        : name_(name)
        , segment_(segment)
        , align_(align)
        , flags_(flags)
...
public:
    MachOTextSection(uint32_t align, uintptr_t addr, uintptr_t size)
        : MachOSection("__text", "__TEXT", align, MachOSection::S_REGULAR | MachOSection::S_ATTR_SOME_INSTRUCTIONS | MachOSection::S_ATTR_PURE_INSTRUCTIONS)
        , addr_(addr)
        , size_(size)
    {
    }
...
public:
    FullHeaderELFSection(const char* name, Type type, uintptr_t align, uintptr_t addr, uintptr_t offset, uintptr_t size, uintptr_t flags)
...
public:
    ...
    void Write(Writer* w, uintptr_t code_start, uintptr_t code_size)
    {
        ...
        Writer::Slot<MachOSegmentCommand> cmd = WriteSegmentCommand(w, code_start, code_size);
...
private:
    ...
    enum MachOLoadCommandCmd { LC_SEGMENT_32 = 0x00000001u, LC_SEGMENT_64 = 0x00000019u };
...
#error Unsupported target architecture.
    ...
    Writer::Slot<MachOSegmentCommand> WriteSegmentCommand(Writer* w, uintptr_t code_start, uintptr_t code_size)
    {
    ...
    void WriteSections(Writer* w, Writer::Slot<MachOSegmentCommand> cmd, Writer::Slot<MachOHeader> header, uintptr_t load_command_start)
    {
        Writer::Slot<MachOSection::Header> headers = w->CreateSlotsHere<MachOSection::Header>(static_cast<uint32_t>(sections_.size()));
...
private:
    ...
    void WriteHeader(Writer* w)
    {
        ...
        const uint8_t ident[16] = { 0x7F, 'E', 'L', 'F', 2, 2, 1, 3, 0, 0, 0, 0, 0, 0, 0, 0 };
#else
#error Unsupported target architecture.
#endif
...
#error Unsupported target architecture.
    ...
    void WriteSectionTable(Writer* w)
    {
        ...
        Writer::Slot<ELFSection::Header> headers = w->CreateSlotsHere<ELFSection::Header>(static_cast<uint32_t>(sections_.size()));
...
class ELFSymbol {
public:
    enum Type { TYPE_NOTYPE = 0, TYPE_OBJECT = 1, TYPE_FUNC = 2, TYPE_SECTION = 3, TYPE_FILE = 4, TYPE_LOPROC = 13, TYPE_HIPROC = 15 };

    enum Binding { BIND_LOCAL = 0, BIND_GLOBAL = 1, BIND_WEAK = 2, BIND_LOPROC = 13, BIND_HIPROC = 15 };

    ELFSymbol(const char* name, uintptr_t value, uintptr_t size, Binding binding, Type type, uint16_t section)
        : name(name)
        , value(value)
        , size(size)
        , info((binding << 4) | type)
...
public:
    ...
    void WriteBody(Writer::Slot<Header> header, Writer* w) override
    {
        ...
        Writer::Slot<ELFSymbol::SerializedLayout> symbols = w->CreateSlotsHere<ELFSymbol::SerializedLayout>(static_cast<uint32_t>(total_symbols));
        ...
        strtab->AttachWriter(w);
        symbols.at(0).set(ELFSymbol::SerializedLayout(0, 0, 0, ELFSymbol::BIND_LOCAL, ELFSymbol::TYPE_NOTYPE, 0));
        WriteSymbolsList(&locals_, symbols.at(1), strtab);
        WriteSymbolsList(&globals_, symbols.at(static_cast<uint32_t>(locals_.size() + 1)), strtab);
        strtab->DetachWriter();
    }
...
private:
    void WriteSymbolsList(const ZoneChunkList<ELFSymbol>* src, Writer::Slot<ELFSymbol::SerializedLayout> dst, ELFStringTable* strtab)
    {
        int i = 0;
...
public:
#if V8_TARGET_ARCH_X64
    enum StackState { POST_RBP_PUSH, POST_RBP_SET, POST_RBP_POP, STACK_STATE_MAX };
#endif

    CodeDescription(const char* name, base::AddressRegion region, Tagged<SharedFunctionInfo> shared, LineInfo* lineinfo, bool is_function)
        : name_(name)
        , shared_info_(shared)
        , lineinfo_(lineinfo)
...
#if defined(__ELF)
static void CreateSymbolsTable(CodeDescription* desc, Zone* zone, ELF* elf, size_t text_section_index)
{
    ...
    symtab->Add(ELFSymbol("V8 Code", 0, 0, ELFSymbol::BIND_LOCAL, ELFSymbol::TYPE_FILE, ELFSection::INDEX_ABSOLUTE));

    symtab->Add(ELFSymbol(desc->name(), 0, desc->CodeSize(), ELFSymbol::BIND_GLOBAL, ELFSymbol::TYPE_FUNC, text_section_index));
}
#endif // defined(__ELF)
...
public:
    explicit DebugInfoSection(CodeDescription* desc)
        ...
        ,
#else
        : MachOSection("__debug_info", "__DWARF", 1, MachOSection::S_REGULAR | MachOSection::S_ATTR_DEBUG)
        ,
#endif
        desc_(desc)
    {
...
#error Unsupported target architecture.
            ...
            for (int param = 0; param < params; ++param) {
                ...
                w->WriteSLEB128(StandardFrameConstants::kFixedFrameSizeAboveFp + kSystemPointerSize * (params - param - 1));
...
public:
    explicit DebugAbbrevSection(CodeDescription* desc)
        ...
        ,
#else
        : MachOSection("__debug_abbrev", "__DWARF", 1, MachOSection::S_REGULAR | MachOSection::S_ATTR_DEBUG)
        ,
#endif
        desc_(desc)
    {
    ...
    void WriteVariableAbbreviation(Writer* w, int abbreviation_code, bool has_value, bool is_parameter)
    {
    ...
    bool WriteBodyInternal(Writer* w) override
    {
        ...
        if (extra_info) {
            ...
            for (int internal_slot = 0; internal_slot < internal_slots; ++internal_slot) {
...
public:
    explicit DebugLineSection(CodeDescription* desc)
        ...
        ,
#else
        : MachOSection("__debug_line", "__DWARF", 1, MachOSection::S_REGULAR | MachOSection::S_ATTR_DEBUG)
        ,
#endif
        desc_(desc)
    {
    ...
    enum DWARF2Opcodes { DW_LNS_COPY = 1, DW_LNS_ADVANCE_PC = 2, DW_LNS_ADVANCE_LINE = 3, DW_LNS_SET_FILE = 4, DW_LNS_SET_COLUMN = 5, DW_LNS_NEGATE_STMT = 6 };

    // DWARF2 standard, figure 35.
    enum DWARF2ExtendedOpcode { DW_LNE_END_SEQUENCE = 1, DW_LNE_SET_ADDRESS = 2, DW_LNE_DEFINE_FILE = 3 };
...
private:
    ...
    static bool ComparePCInfo(const LineInfo::PCInfo& a, const LineInfo::PCInfo& b)
    {
        if (a.pc_ == b.pc_) {
...
public:
    ...
    void WriteLength(Writer* w, Writer::Slot<uint32_t>* length_slot, int initial_position);
...
private:
    ...
    enum CFIConstants { CIE_ID = 0, CIE_VERSION = 1, CODE_ALIGN_FACTOR = 1, DATA_ALIGN_FACTOR = 1, RETURN_ADDRESS_REGISTER = AMD64_RA };
};

void UnwindInfoSection::WriteLength(Writer* w, Writer::Slot<uint32_t>* length_slot, int initial_position)
{
...
UnwindInfoSection::UnwindInfoSection(CodeDescription* desc)
    ...
    ,
#else
    : MachOSection("__eh_frame", "__TEXT", sizeof(uintptr_t), MachOSection::S_REGULAR)
    ,
#endif
    desc_(desc)
{
...
void UnwindInfoSection::WriteFDEStateOnEntry(Writer* w)
{
    ...
    w->Write<uint64_t>(desc_->GetStackStateStartAddress(CodeDescription::POST_RBP_PUSH));
}
...
void UnwindInfoSection::WriteFDEStateAfterRBPPush(Writer* w)
{
    ...
    // Last location described by this entry.
    w->Write<uint8_t>(DW_CFA_SET_LOC);
    w->Write<uint64_t>(desc_->GetStackStateStartAddress(CodeDescription::POST_RBP_SET));
}
...
void UnwindInfoSection::WriteFDEStateAfterRBPSet(Writer* w)
{
    ...
    // Last location described by this entry.
    w->Write<uint8_t>(DW_CFA_SET_LOC);
    w->Write<uint64_t>(desc_->GetStackStateStartAddress(CodeDescription::POST_RBP_POP));
}
...
static void CreateDWARFSections(CodeDescription* desc, Zone* zone, DebugObject* obj)
{
...
static JITCodeEntry* CreateCodeEntry(Address symfile_addr, uintptr_t symfile_size)
{
    JITCodeEntry* entry = static_cast<JITCodeEntry*>(base::Malloc(sizeof(JITCodeEntry) + symfile_size));

    entry->symfile_addr_ = reinterpret_cast<Address>(entry + 1);
    entry->symfile_size_ = symfile_size;
    MemCopy(reinterpret_cast<void*>(entry->symfile_addr_), reinterpret_cast<void*>(symfile_addr), symfile_size);
...
static JITCodeEntry* CreateELFObject(CodeDescription* desc, Isolate* isolate)
{
    ...
    static_assert(code_alignment == kCodeAlignment, "Unsupported code alignment value");
    mach_o.AddSection(zone.New<MachOTextSection>(code_alignment, desc->CodeStart(), desc->CodeSize()));
    ...
    size_t text_section_index = elf.AddSection(zone.New<FullHeaderELFSection>(
        ".text", ELFSection::TYPE_NOBITS, kCodeAlignment, desc->CodeStart(), 0, desc->CodeSize(), ELFSection::FLAG_ALLOC | ELFSection::FLAG_EXEC));
...
struct AddressRegionLess {
    bool operator()(const base::AddressRegion& a, const base::AddressRegion& b) const
    {
        if (a.begin() == b.begin())
            return a.end() < b.end();
...
static void PutLineInfo(Address addr, LineInfo* info)
{
    ...
    base::HashMap::Entry* e = line_map->LookupOrInsert(reinterpret_cast<void*>(addr), HashCodeAddress(addr));
    if (e->value != nullptr)
...
{
    void* value = GetLineMap()->Remove(reinterpret_cast<void*>(addr), HashCodeAddress(addr));
...
static void AddUnwindInfo(CodeDescription* desc)
{
    ...
    if (desc->is_function()) {
        ...
        desc->SetStackStateStartAddress(CodeDescription::POST_RBP_PUSH, frame_pointer_push_address);
        desc->SetStackStateStartAddress(CodeDescription::POST_RBP_SET, frame_pointer_set_address);
        desc->SetStackStateStartAddress(CodeDescription::POST_RBP_POP, frame_pointer_pop_address);
    } else {
        desc->SetStackStateStartAddress(CodeDescription::POST_RBP_PUSH, desc->CodeStart());
        desc->SetStackStateStartAddress(CodeDescription::POST_RBP_SET, desc->CodeStart());
        desc->SetStackStateStartAddress(CodeDescription::POST_RBP_POP, desc->CodeEnd());
    }
#endif // V8_TARGET_ARCH_X64
}
...
static std::optional<std::pair<CodeMap::iterator, CodeMap::iterator>> GetOverlappingRegions(CodeMap* map, const base::AddressRegion region)
{
...
static void RemoveJITCodeEntries(CodeMap* map, const base::AddressRegion region)
{
...
static void AddJITCodeEntry(CodeMap* map, const base::AddressRegion region, JITCodeEntry* entry, bool dump_if_enabled, const char* name_hint)
{
    ...
    if (v8_flags.gdbjit_dump && dump_if_enabled) {
        ...
        SNPrintF(base::Vector<char>(file_name, kMaxFileNameSize), "/tmp/elfdump%s%d.o", (name_hint != nullptr) ? name_hint : "", file_num++);
        WriteBytes(file_name, reinterpret_cast<uint8_t*>(entry->symfile_addr_), static_cast<int>(entry->symfile_size_));
    }
#endif
...
static void AddCode(const char* name, base::AddressRegion region, Tagged<SharedFunctionInfo> shared, LineInfo* lineinfo, Isolate* isolate, bool is_function)
{
...
void EventHandler(const v8::JitCodeEvent* event)
{
    if (!v8_flags.gdbjit)
        return;
    if ((event->code_type != v8::JitCodeEvent::JIT_CODE) && (event->code_type != v8::JitCodeEvent::WASM_CODE)) {
        return;
    }
    ...
    case v8::JitCodeEvent::CODE_ADDED: {
        ...
        Tagged<SharedFunctionInfo> shared = event->script.IsEmpty() ? Tagged<SharedFunctionInfo>() : *Utils::OpenDirectHandle(*event->script);
        ...
        if (event->code_type == v8::JitCodeEvent::JIT_CODE) {
            Tagged<Code> lookup_result = isolate->heap()->FindCodeForInnerPointer(addr);
            is_function = CodeKindIsOptimizedJSFunction(lookup_result->kind());
        }
        AddCode(event_name.c_str(), { addr, event->code_len }, shared, lineinfo, isolate, is_function);
        break;
    }
