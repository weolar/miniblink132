...
void Decoder::PrintMSARegister(int wreg)
{
    Print(MSARegisters::Name(wreg));
}
...
void Decoder::PrintSs1(Instruction* instr)
{
    ...
    out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "%d", msbd + 1);
}
...
void Decoder::PrintSs2(Instruction* instr)
{
    ...
    out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "%d", msb - lsb + 1);
}
...
void Decoder::PrintSs3(Instruction* instr)
{
    ...
    out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "%d", msbdminus32 + 32 + 1);
}
...
void Decoder::PrintSs4(Instruction* instr)
{
    ...
    int lsb = instr->SaValue();
    out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "%d", msbminus32 + 32 - lsb + 1);
}
...
void Decoder::PrintSs5(Instruction* instr)
{
    ...
    out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "%d", lsbminus32 + 32);
}
...
void Decoder::PrintCc(Instruction* instr)
{
    ...
    out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "cc(%d)", cc);
}
...
void Decoder::PrintPCImm16(Instruction* instr, int delta_pc, int n_bits)
{
    ...
    out_buffer_pos_
        += base::SNPrintF(out_buffer_ + out_buffer_pos_, "%s", converter_.NameOfAddress(reinterpret_cast<uint8_t*>(instr) + delta_pc + (offset << n_bits)));
}
...
void Decoder::PrintPCImm21(Instruction* instr, int delta_pc, int n_bits)
{
    ...
    imm21 >>= (32 - kImm21Bits);
    out_buffer_pos_
        += base::SNPrintF(out_buffer_ + out_buffer_pos_, "%s", converter_.NameOfAddress(reinterpret_cast<uint8_t*>(instr) + delta_pc + (imm21 << n_bits)));
}

// Print 26-bit hex immediate value.
void Decoder::PrintXImm26(Instruction* instr)
{
    uint64_t target = static_cast<uint64_t>(instr->Imm26Value()) << kImmFieldShift;
    target = (reinterpret_cast<uint64_t>(instr) & ~0xFFFFFFF) | target;
    out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "0x%" PRIx64, target);
}
...
void Decoder::PrintPCImm26(Instruction* instr, int delta_pc, int n_bits)
{
    ...
    imm26 >>= (32 - kImm26Bits);
    out_buffer_pos_
        += base::SNPrintF(out_buffer_ + out_buffer_pos_, "%s", converter_.NameOfAddress(reinterpret_cast<uint8_t*>(instr) + delta_pc + (imm26 << n_bits)));
}
...
void Decoder::PrintPCImm26(Instruction* instr)
{
    ...
    out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "%s", converter_.NameOfAddress((reinterpret_cast<uint8_t*>(pc))));
}
...
void Decoder::PrintCode(Instruction* instr)
{
    ...
    case BREAK: {
        ...
        out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "0x%05x (%d)", code, code);
        break;
    }
    case TGE:
    case TGEU:
    ...
    case TNE: {
        ...
        out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "0x%03x", code);
        break;
    }
...
void Decoder::PrintMsaCopy(Instruction* instr)
{
    ...
    int32_t n = instr->MsaElmNValue();
    out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "%s, %s[%u]", converter_.NameOfCPURegister(rd), MSARegisters::Name(ws), n);
}
...
int Decoder::DecodeBreakInstr(Instruction* instr)
{
    ...
    if (instr->Bits(25, 6) == static_cast<int>(kMaxStopCode)) {
        ...
        out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "\n%p       %08" PRIx64,
            static_cast<void*>(reinterpret_cast<int32_t*>(instr + kInstrSize)), reinterpret_cast<uint64_t>(*reinterpret_cast<char**>(instr + kInstrSize)));
