...
EtwIsolateCaptureStateMonitor::EtwIsolateCaptureStateMonitor(base::Mutex* mutex, size_t pending_isolate_count)
    : mutex_(mutex)
...
bool EtwIsolateCaptureStateMonitor::WaitFor(const base::TimeDelta& delta)
{
    ...
    ETWTRACEDBG << "Waiting for " << pending_isolate_count_ << " isolates for up to " << remaining.InMilliseconds() << std::endl;
    while (isolates_ready_cv_.WaitFor(mutex_, remaining)) {
        ETWTRACEDBG << "WaitFor woke up: " << pending_isolate_count_ << " isolates remaining " << std::endl;
        ...
        if (elapsed >= remaining) {
            ETWTRACEDBG << "Elapsed is " << elapsed.InMilliseconds() << " greater than reminaing " << remaining.InMilliseconds() << std::endl;
            return false;
        }

        // If the condition variable was woken up spuriously, adjust the timeout.
        remaining -= elapsed;
        ETWTRACEDBG << "New remaining " << remaining.InMilliseconds() << " resuming waiting" << std::endl;
    }
...
{
    {
        ...
        ETWTRACEDBG << "Got mutex and isolate count reduced to " << pending_isolate_count_ << std::endl;
    }
