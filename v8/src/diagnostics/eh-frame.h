...
class V8_EXPORT_PRIVATE EhFrameConstants final : public NON_EXPORTED_BASE(AllStatic) {
public:
...
private:
    ...
    {
        eh_frame_buffer_.push_back(value);
    }
    ...
    {
        DCHECK_EQ(base::ReadUnalignedValue<uint32_t>(reinterpret_cast<Address>(eh_frame_buffer_.data()) + base_offset), kInt32Placeholder);
        DCHECK_LT(base_offset + kInt32Size, eh_frame_offset());
        base::WriteUnalignedValue<uint32_t>(reinterpret_cast<Address>(eh_frame_buffer_.data()) + base_offset, value);
    }
...
public:
    ...
    void SkipCie()
    {
        ...
        next_ += base::ReadUnalignedValue<uint32_t>(reinterpret_cast<Address>(next_)) + kInt32Size;
    }
...
private:
    ...
    template <typename T> T GetNextValue()
    {
        T result;
...
private:
    static void DumpDwarfDirectives(std::ostream& stream, const uint8_t* start, const uint8_t* end);
