...
public:
        ...
        , scope_depth_(0)
    {
        ...
        if (v8_flags.redirect_code_traces_to != nullptr) {
            base::StrNCpy(filename_, v8_flags.redirect_code_traces_to, filename_.length());
        } else if (isolate_id >= 0) {
            base::SNPrintF(filename_, "code-%d-%d.asm", base::OS::GetCurrentProcessId(), isolate_id);
        } else {
    ...
    public:
        explicit StreamScope(CodeTracer* tracer)
            : Scope(tracer)
        {
            FILE* file = this->file();
            if (file == stdout) {
...
private:
    ...
    {
        return v8_flags.redirect_code_traces;
    }
