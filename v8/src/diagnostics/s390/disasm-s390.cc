...
int Decoder::FormatMask(Instruction* instr, const char* format)
{
    ...
    if ((format[1] == '1')) { // prints the mask format in bits 8-12
        ...
        out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "0x%x", value);
        return 2;
...
int Decoder::FormatImmediate(Instruction* instr, const char* format)
{
    ...
    if (format[1] == '1') { // immediate in 16-31
        ...
        out_buffer_pos_
            += base::SNPrintF(out_buffer_ + out_buffer_pos_, "%d -> %s", value, converter_.NameOfAddress(reinterpret_cast<uint8_t*>(instr) + value));
        return 2;
...
int Decoder::InstructionDecode(uint8_t* instr_ptr)
{
    ...
    if (instrLength == 2) {
        out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "%04x           ", instr->InstructionBits<TwoByteInstr>());
    } else if (instrLength == 4) {
        out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "%08x       ", instr->InstructionBits<FourByteInstr>());
    } else {
        out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "%012" PRIx64 "   ", instr->InstructionBits<SixByteInstr>());
    }
