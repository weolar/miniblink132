...
void HeapObject::HeapObjectVerify(Isolate* isolate)
{
    ...
    CHECK_IMPLIES(!IsTrustedObject(*this) && !IsFreeSpaceOrFiller(*this), !HeapLayout::InTrustedSpace(*this));
...
#undef STRING_TYPE_CASE
    ...
    case WASM_TRUSTED_INSTANCE_DATA_TYPE:
        Cast<WasmTrustedInstanceData>(*this)->WasmTrustedInstanceDataVerify(isolate);
        break;
...
bool JSObject::ElementsAreSafeToExamine(PtrComprCageBase cage_base) const
{
    ...
    return elements(cage_base) != GetReadOnlyRoots(cage_base).one_pointer_filler_map();
}
...
void JSObject::JSObjectVerify(Isolate* isolate)
{
    ...
    if (HasFastProperties()) {
        ...
        for (InternalIndex i : map()->IterateOwnDescriptors()) {
            ...
            if (details.location() == PropertyLocation::kField) {
                ...
                if (r.IsNone()) {
                    ...
                    CHECK(!type_is_none);
                    if (!type_is_any) {
                        CHECK_IMPLIES(FieldType::NowStable(field_type), map()->is_deprecated() || FieldType::NowContains(field_type, value));
                    }
                } else {
                    CHECK(type_is_any);
                }
                CHECK_IMPLIES(is_transitionable_fast_elements_kind, Map::IsMostGeneralFieldType(r, field_type));
            }
        }
        ...
        if (map()->EnumLength() != kInvalidEnumCacheSentinel) {
            ...
            CHECK_IMPLIES(indices != ReadOnlyRoots(isolate).empty_fixed_array(), keys->length() == indices->length());
        }
    }
    ...
    if (ElementsAreSafeToExamine(isolate)) {
        CHECK_EQ((map()->has_fast_smi_or_object_elements() || map()->has_any_nonextensible_elements() || (elements() == GetReadOnlyRoots().empty_fixed_array())
...
void Map::MapVerify(Isolate* isolate)
{
    ...
    if (IsContextMap(*this)) {
        ...
        // null.
        CHECK(IsNull(native_context_or_null()) || IsNativeContext(native_context_or_null()));
        ...
        if (IsUndefined(GetBackPointer(), isolate)) {
            ...
            // belong to the map.
            CHECK_EQ(NumberOfOwnDescriptors(), instance_descriptors(isolate)->number_of_descriptors());
        } else {
            ...
            if (!is_deprecated() && !parent->is_deprecated()) {
                CHECK_EQ(IsInobjectSlackTrackingInProgress(), parent->IsInobjectSlackTrackingInProgress());
            }
    ...
    SLOW_DCHECK(TransitionsAccessor(isolate, *this).IsConsistentWithBackPointers());
    ...
    if (InstanceTypeChecker::IsNativeContextSpecific(instance_type())) {
        ...
        if (instance_type() == NATIVE_CONTEXT_TYPE || instance_type() == JS_GLOBAL_PROXY_TYPE) {
            ...
            CHECK(IsNull(map()->native_context_or_null()) || IsNativeContext(map()->native_context_or_null()));
            ...
            CHECK_IMPLIES(map() != GetReadOnlyRoots().meta_map(), IsNativeContext(map()->native_context_or_null()));
    ...
    if (IsJSObjectMap(*this)) {
        ...
        CHECK_EQ(header_end_offset + JSObject::GetEmbedderFieldCount(*this) * kEmbedderDataSlotSize, inobject_fields_start_offset);

        if (IsJSSharedStructMap(*this) || IsJSSharedArrayMap(*this) || IsJSAtomicsMutex(*this) || IsJSAtomicsCondition(*this)) {
        ...
        if (IsJSFunctionMap(*this) && !IsMap(constructor_or_back_pointer())) {
            ...
            CHECK(IsJSFunction(maybe_constructor) || IsFunctionTemplateInfo(maybe_constructor) ||
                // The above check might fail until empty function setup is done.
                IsUndefined(isolate->raw_native_context()->get(Context::EMPTY_FUNCTION_INDEX)));
        }
    }
    ...
    CHECK_IMPLIES(IsJSObjectMap(*this) && !CanHaveFastTransitionableElementsKind(),
        IsDictionaryElementsKind(elements_kind()) || IsTerminalElementsKind(elements_kind()) || IsAnyHoleyNonextensibleElementsKind(elements_kind())
...
void Map::DictionaryMapVerify(Isolate* isolate)
{
    ...
    CHECK_EQ(ReadOnlyRoots(isolate).empty_descriptor_array(), instance_descriptors(isolate));
...
void FeedbackCell::FeedbackCellVerify(Isolate* isolate)
{
    ...
    CHECK(kind == CodeKind::FOR_TESTING || kind == CodeKind::BUILTIN || kind == CodeKind::INTERPRETED_FUNCTION || kind == CodeKind::BASELINE
        || kind == CodeKind::MAGLEV || kind == CodeKind::TURBOFAN_JS);
#endif
}

void ClosureFeedbackCellArray::ClosureFeedbackCellArrayVerify(Isolate* isolate)
{
    CHECK(IsSmi(length_.load()));
    for (int i = 0; i < length(); ++i) {
        Object::VerifyPointer(isolate, get(i));
...
void ArrayList::ArrayListVerify(Isolate* isolate)
{
    ...
    CHECK_IMPLIES(capacity() == 0, this == ReadOnlyRoots(isolate).empty_array_list());
    for (int i = 0; i < capacity(); ++i) {
        Object::VerifyPointer(isolate, get(i));
    }
}
...
void FixedDoubleArray::FixedDoubleArrayVerify(Isolate* isolate)
{
    for (int i = 0; i < length(); i++) {
        if (!is_the_hole(i)) {
            ...
            uint64_t unexpected = base::bit_cast<uint64_t>(std::numeric_limits<double>::quiet_NaN()) & uint64_t { 0x7FF8000000000000 };
...
void DescriptorArray::DescriptorArrayVerify(Isolate* isolate)
{
    ...
    if (number_of_all_descriptors() == 0) {
        ...
        for (InternalIndex descriptor : InternalIndex::Range(number_of_descriptors())) {
            Tagged<Object> key = *(GetDescriptorSlot(descriptor.as_int()) + kEntryKeyIndex);
            ...
            if (details.location() == PropertyLocation::kField) {
                ...
                CHECK(value == FieldType::None() || value == FieldType::Any() || IsMap(value.GetHeapObjectAssumeWeak()));
...
void TransitionArray::TransitionArrayVerify(Isolate* isolate)
{
        ...
        for (int i = 0; i < length; ++i) {
            ...
            if (maybe_target.GetHeapObjectIfWeak(&target)) {
                if (v8_flags.move_prototype_transitions_first) {
                    Tagged<Map> parent = Cast<Map>(Cast<Map>(target)->constructor_or_back_pointer());
                    if (owner.is_null()) {
    ...
    if (HasSideStepTransitions()) {
        ...
        for (uint32_t index = SideStepTransition::kFirstMapIdx; index <= SideStepTransition::kLastMapIdx; ++index) {
            ...
            if (maybe_target.GetHeapObjectIfWeak(&target)) {
                ...
                CHECK(maybe_target == SideStepTransition::Unreachable || maybe_target == SideStepTransition::Empty || maybe_target.IsCleared());
            }
        }
        Tagged<MaybeObject> maybe_cell = side_trans->get(SideStepTransition::index_of(SideStepTransition::Kind::kObjectAssignValidityCell));
...
namespace {
void SloppyArgumentsElementsVerify(Isolate* isolate, Tagged<SloppyArgumentsElements> elements, Tagged<JSObject> holder)
{
...
void JSArgumentsObject::JSArgumentsObjectVerify(Isolate* isolate)
{
    ...
    if (IsSloppyArgumentsElementsKind(GetElementsKind())) {
        SloppyArgumentsElementsVerify(isolate, Cast<SloppyArgumentsElements>(elements()), *this);
    }
    Tagged<NativeContext> native_context = map()->map()->native_context();
    if (map() == native_context->get(Context::SLOPPY_ARGUMENTS_MAP_INDEX) || map() == native_context->get(Context::SLOW_ALIASED_ARGUMENTS_MAP_INDEX)
    ...
    } else if (map() == native_context->get(Context::STRICT_ARGUMENTS_MAP_INDEX)) {
...
void ConsString::ConsStringVerify(Isolate* isolate)
{
    ...
    if (IsFlat()) {
        ...
        CHECK(IsSeqString(first()) || IsExternalString(first()) || IsThinString(first()));
    }
}
...
void JSFunction::JSFunctionVerify(Isolate* isolate)
{
    ...
    static_assert(JSFunction::TorqueGeneratedClass::kHeaderSize == 8 * kTaggedSize);
    ...
    CHECK_EQ(parameter_count, shared(isolate)->internal_formal_parameter_count_with_receiver());
    Tagged<Code> code_from_table = jdt->GetCode(handle);
    CHECK(code_from_table->parameter_count() == kDontAdaptArgumentsSentinel || code_from_table->parameter_count() == parameter_count);

    // Currently, a JSFunction must have the same dispatch entry as its
    // FeedbackCell, unless the FeedbackCell has no entry.
    JSDispatchHandle feedback_cell_handle = raw_feedback_cell(isolate)->dispatch_handle();
    CHECK_EQ(raw_feedback_cell(isolate) == isolate->heap()->many_closures_cell(), feedback_cell_handle == kNullJSDispatchHandle);
    ...
    LookupIterator it(isolate, function, isolate->factory()->prototype_string(), LookupIterator::OWN_SKIP_INTERCEPTOR);
    ...
    if (has_prototype_property()) {
        ...
        CHECK(!it.IsFound() || it.state() != LookupIterator::ACCESSOR || !IsAccessorInfo(*it.GetAccessors()));
    }

    CHECK_IMPLIES(shared()->HasBuiltinId(),
        Builtins::CheckFormalParameterCount(shared()->builtin_id(), shared()->length(), shared()->internal_formal_parameter_count_with_receiver()));
}
...
bool ShouldVerifySharedFunctionInfoFunctionIndex(Tagged<SharedFunctionInfo> sfi)
{
    if (!sfi->HasBuiltinId())
        return true;
...
void SharedFunctionInfo::SharedFunctionInfoVerify(LocalIsolate* isolate)
{
...
#if V8_ENABLE_WEBASSEMBLY
    bool is_wasm = HasWasmExportedFunctionData() || HasAsmWasmData() || HasWasmJSFunctionData() || HasWasmCapiFunctionData() || HasWasmResumeData();
#else
    bool is_wasm = false;
#endif // V8_ENABLE_WEBASSEMBLY
    CHECK(is_wasm || IsApiFunction() || HasBytecodeArray() || HasBuiltinId() || HasUncompiledDataWithPreparseData() || HasUncompiledDataWithoutPreparseData());
    ...
    if (!is_compiled()) {
        ...
        CHECK(IsScopeInfo(outer_scope_info()) || IsTheHole(outer_scope_info(), roots));
    ...
    if (HasBytecodeArray() && !IsDontAdaptArguments()) {
        CHECK_EQ(GetBytecodeArray(isolate)->parameter_count(), internal_formal_parameter_count_with_receiver());
    }

    if (ShouldVerifySharedFunctionInfoFunctionIndex(*this)) {
        int expected_map_index = Context::FunctionMapIndex(language_mode(), kind(), HasSharedName());
    ...
    if (IsApiFunction()) {
        ...
        if (builtin_id() != Builtin::kCompileLazy && builtin_id() != Builtin::kEmptyFunction) {
            CHECK(construct_as_builtin());
        } else {
            CHECK(!construct_as_builtin());
        }
    }
    CHECK_IMPLIES(HasBuiltinId(), Builtins::CheckFormalParameterCount(builtin_id(), length(), internal_formal_parameter_count_with_receiver()));
}
...
void SharedFunctionInfoWrapper::SharedFunctionInfoWrapperVerify(Isolate* isolate)
{
...
void JSGlobalObject::JSGlobalObjectVerify(Isolate* isolate)
{
    ...
    if (global_dictionary(kAcquireLoad)->NumberOfElements() == 0 && elements()->length() == 0) {
        return;
    }
...
void ExposedTrustedObject::ExposedTrustedObjectVerify(Isolate* isolate)
{
    ...
    IndirectPointerSlot slot = RawIndirectPointerField(kSelfIndirectPointerOffset, tag);
    ...
    if (tag == kCodeIndirectPointerTag) {
        CodePointerTable::Space* space = IsolateForSandbox(isolate).GetCodePointerTableSpaceFor(slot.address());
        // During snapshot creation, the code pointer space of the read-only heap is
        // not marked as an internal read-only space.
        bool is_space_read_only = space == isolate->read_only_heap()->code_pointer_space();
...
void Code::CodeVerify(Isolate* isolate)
{
    ...
    if (has_instruction_stream()) {
        ...
        if (istream->instruction_start() == instruction_start()) {
            ...
            Tagged<Code> lookup_result = isolate->heap()->FindCodeForInnerPointer(instruction_start());
...
void InstructionStream::InstructionStreamVerify(Isolate* isolate)
{
    TrustedObjectVerify(isolate);
    Tagged<Code> code;
    if (!TryGetCode(&code, kAcquireLoad))
        return;
    CHECK(IsAligned(code->instruction_size(), static_cast<unsigned>(InstructionStream::kMetadataAlignment)));
    ...
    CHECK_IMPLIES(!ReadOnlyHeap::Contains(*this), IsAligned(instruction_start(), kCodeAlignment));
#endif // (!defined(_MSC_VER) || defined(__clang__)) && !defined(V8_OS_ZOS)
    CHECK_IMPLIES(!ReadOnlyHeap::Contains(*this), IsAligned(instruction_start(), kCodeAlignment));
    CHECK_EQ(*this, code->instruction_stream());
    CHECK(Size() <= MemoryChunkLayout::MaxRegularCodeObjectSize() || isolate->heap()->InSpace(*this, CODE_LO_SPACE));
...
void JSArray::JSArrayVerify(Isolate* isolate)
{
    ...
    if (!ElementsAreSafeToExamine(isolate))
        return;
    if (IsUndefined(elements(), isolate))
        return;
    ...
    if (IsSmi(length()) && (HasFastElements() || HasAnyNonextensibleElements())) {
        if (elements()->length() > 0) {
            ...
            CHECK_IMPLIES(HasSmiOrObjectElements() || HasAnyNonextensibleElements(), IsFixedArray(elements()));
        }
...
void JSSharedStruct::JSSharedStructVerify(Isolate* isolate)
{
    ...
    Tagged<DescriptorArray> descriptors = struct_map->instance_descriptors(isolate);
    for (InternalIndex i : struct_map->IterateOwnDescriptors()) {
        ...
        } else if (JSSharedStruct::IsElementsTemplateDescriptor(isolate, struct_map, i)) {
            CHECK_EQ(PropertyLocation::kDescriptor, details.location());
...
void WeakCell::WeakCellVerify(Isolate* isolate)
{
    ...
    CHECK_IMPLIES(IsUndefined(unregister_token(), isolate), IsUndefined(key_list_prev(), isolate));
    CHECK_IMPLIES(IsUndefined(unregister_token(), isolate), IsUndefined(key_list_next(), isolate));
    ...
    CHECK(IsUndefined(finalization_registry(), isolate) || IsJSFinalizationRegistry(finalization_registry()));
}
...
template <typename Derived> void SmallOrderedHashTable<Derived>::SmallOrderedHashTableVerify(Isolate* isolate)
{
    ...
    for (int entry = NumberOfElements() + NumberOfDeletedElements(); entry < Capacity(); entry++) {
        for (int offset = 0; offset < Derived::kEntrySize; offset++) {
            Tagged<Object> val = GetDataEntry(entry, offset);
...
void SmallOrderedHashMap::SmallOrderedHashMapVerify(Isolate* isolate)
{
    ...
    SmallOrderedHashTable<SmallOrderedHashMap>::SmallOrderedHashTableVerify(isolate);
    for (int entry = NumberOfElements(); entry < NumberOfDeletedElements(); entry++) {
...
void SmallOrderedHashSet::SmallOrderedHashSetVerify(Isolate* isolate)
{
    ...
    SmallOrderedHashTable<SmallOrderedHashSet>::SmallOrderedHashTableVerify(isolate);
    for (int entry = NumberOfElements(); entry < NumberOfDeletedElements(); entry++) {
        for (int offset = 0; offset < kEntrySize; offset++) {
            Tagged<Object> val = GetDataEntry(entry, offset);
            CHECK(IsTheHole(val, isolate));
...
void SmallOrderedNameDictionary::SmallOrderedNameDictionaryVerify(Isolate* isolate)
{
    CHECK(IsSmallOrderedNameDictionary(*this));
    SmallOrderedHashTable<SmallOrderedNameDictionary>::SmallOrderedHashTableVerify(isolate);
    for (int entry = NumberOfElements(); entry < NumberOfDeletedElements(); entry++) {
        for (int offset = 0; offset < kEntrySize; offset++) {
            Tagged<Object> val = GetDataEntry(entry, offset);
            CHECK(IsTheHole(val, isolate) || (PropertyDetails::Empty().AsSmi() == Cast<Smi>(val)));
        }
    }
}
...
void SwissNameDictionary::SwissNameDictionaryVerify(Isolate* isolate, bool slow_checks)
{
    ...
    if (slow_checks) {
        ...
        // table.
        for (int i = 0; i < std::min(static_cast<int>(Group::kWidth), Capacity()); ++i) {
...
void IrRegExpData::IrRegExpDataVerify(Isolate* isolate)
{
    ...
    CHECK_IMPLIES(IsSmi(capture_name_map()), Smi::ToInt(capture_name_map()) == JSRegExp::kUninitializedValue || capture_name_map() == Smi::zero());
    ...
    case RegExpData::Type::EXPERIMENTAL: {
        if (has_latin1_code()) {
            CHECK_EQ(latin1_code(isolate)->builtin_id(), Builtin::kRegExpExperimentalTrampoline);
...
void JSArrayBuffer::JSArrayBufferVerify(Isolate* isolate)
{
    ...
    if (FIELD_SIZE(kOptionalPaddingOffset) != 0) {
        ...
        CHECK_EQ(0, *reinterpret_cast<uint32_t*>(address() + kOptionalPaddingOffset));
    }
}
...
void JSDataView::JSDataViewVerify(Isolate* isolate)
{
    ...
    if (!WasDetached()) {
        CHECK_EQ(reinterpret_cast<uint8_t*>(Cast<JSArrayBuffer>(buffer())->backing_store()) + byte_offset(), data_pointer());
    }
}
...
void SourceTextModuleInfoEntry::SourceTextModuleInfoEntryVerify(Isolate* isolate)
{
    TorqueGeneratedClassVerifiers::SourceTextModuleInfoEntryVerify(*this, isolate);
    CHECK_IMPLIES(IsString(import_name()), module_request() >= 0);
    CHECK_IMPLIES(IsString(export_name()) && IsString(import_name()), IsUndefined(local_name(), isolate));
}
...
void Module::ModuleVerify(Isolate* isolate)
{
    ...
    CHECK(IsUndefined(module_namespace(), isolate) || IsJSModuleNamespace(module_namespace()));
    ...
    if (!(status() == kErrored || status() == kEvaluating || status() == kEvaluatingAsync || status() == kEvaluated)) {
...
void ModuleRequest::ModuleRequestVerify(Isolate* isolate)
{
    ...
    CHECK_EQ(0, import_attributes()->length() % ModuleRequest::kAttributeEntrySize);

    for (int i = 0; i < import_attributes()->length(); i += ModuleRequest::kAttributeEntrySize) {
...
void SourceTextModule::SourceTextModuleVerify(Isolate* isolate)
{
    ...
    } else if (status() == kEvaluating || status() == kEvaluatingAsync || status() == kEvaluated) {
...
void PrototypeUsers::Verify(Tagged<WeakArrayList> array)
{
    ...
    for (int i = kFirstIndex; i < array->length(); ++i) {
        ...
        if ((object.GetHeapObjectIfWeak(&heap_object) && IsMap(heap_object)) || object.IsCleared()) {
            ++weak_maps_count;
        } else {
            CHECK(IsSmi(object));
        }
...
void ObjectBoilerplateDescription::ObjectBoilerplateDescriptionVerify(Isolate* isolate)
{
    CHECK(IsSmi(length_.load()));
...
void RegExpBoilerplateDescription::RegExpBoilerplateDescriptionVerify(Isolate* isolate)
{
    {
        auto o = data(isolate);
        Object::VerifyPointer(isolate, o);
...
void WasmTrustedInstanceData::WasmTrustedInstanceDataVerify(Isolate* isolate)
{
    ...
    for (int i = 0; i < num_dispatch_tables; ++i) {
        ...
        if (i == 0)
            CHECK_EQ(table, dispatch_table0());
    }
    if (num_dispatch_tables == 0)
        CHECK_EQ(0, dispatch_table0()->length());
}
...
void WasmDispatchTable::WasmDispatchTableVerify(Isolate* isolate)
{
    ...
    for (int i = 0; i < len; ++i) {
        ...
        CHECK(IsWasmTrustedInstanceData(arg) || IsWasmImportData(arg) || arg == Smi::zero());
...
void WasmExportedFunctionData::WasmExportedFunctionDataVerify(Isolate* isolate)
{
    TorqueGeneratedClassVerifiers::WasmExportedFunctionDataVerify(*this, isolate);
    Tagged<Code> wrapper = wrapper_code(isolate);
    CHECK(wrapper->kind() == CodeKind::JS_TO_WASM_FUNCTION || wrapper->kind() == CodeKind::C_WASM_ENTRY
        || (wrapper->is_builtin()
            && (wrapper->builtin_id() == Builtin::kJSToWasmWrapper ||
#if V8_ENABLE_DRUMBRAKE
                wrapper->builtin_id() == Builtin::kGenericJSToWasmInterpreterWrapper ||
#endif // V8_ENABLE_DRUMBRAKE
                wrapper->builtin_id() == Builtin::kWasmPromising || wrapper->builtin_id() == Builtin::kWasmStressSwitch)));
}
...
void DataHandler::DataHandlerVerify(Isolate* isolate)
{
    ...
    CHECK_IMPLIES(!IsSmi(smi_handler()), IsStoreHandler(*this) && IsCode(smi_handler()));
...
void AllocationSite::AllocationSiteVerify(Isolate* isolate)
{
    ...
    CHECK(IsSmi(transition_info_or_boilerplate()) || IsJSObject(transition_info_or_boilerplate()));
...
void Script::ScriptVerify(Isolate* isolate)
{
    ...
    for (int i = 0; i < infos()->length(); ++i) {
        ...
        CHECK(!maybe_object.GetHeapObjectIfWeak(isolate, &heap_object)
            || (maybe_object.GetHeapObjectIfStrong(&heap_object) && IsUndefined(heap_object, isolate)) || Is<SharedFunctionInfo>(heap_object)
            || Is<ScopeInfo>(heap_object));
    }
}
    ...
    if (v8_flags.enable_slow_asserts) {
        for (int i = 0; i < length(); i++) {
            ...
            if (e.GetHeapObjectIfWeak(&heap_object)) {
                ...
                CHECK(e.IsCleared() || (e.GetHeapObjectIfStrong(&heap_object) && IsUndefined(heap_object, isolate)));
            }
        }
    }
}
...
void FunctionTemplateRareData::FunctionTemplateRareDataVerify(Isolate* isolate)
{
    CHECK(IsFixedArray(c_function_overloads()) || IsUndefined(c_function_overloads(), isolate));
}
...
void JSObject::IncrementSpillStatistics(Isolate* isolate, SpillInformation* info)
{
    ...
    if (HasFastProperties()) {
        ...
        Tagged<GlobalDictionary> dict = Cast<JSGlobalObject>(*this)->global_dictionary(kAcquireLoad);
...
void JSObject::SpillInformation::Print()
{
    ...
    PrintF("    - slow elements (#%d): %d (used) %d (unused)\n", number_of_objects_ - number_of_objects_with_fast_elements_, number_of_slow_used_elements_,
...
bool TransitionArray::IsSortedNoDuplicates()
{
    ...
    for (int i = 0; i < number_of_transitions(); i++) {
        ...
        if (!TransitionsAccessor::IsSpecialTransition(key->GetReadOnlyRoots(), key)) {
            Tagged<Map> target = GetTarget(i);
        ...
        int cmp = CompareKeys(prev_key, prev_hash, prev_kind, prev_attributes, key, hash, kind, attributes);
        if (cmp >= 0) {
            Print(this);
            return false;
        }
...
bool TransitionsAccessor::IsConsistentWithBackPointers()
{
    ...
    auto CheckTarget = [&](Tagged<Map> target) {
        ...
        if (!map_->is_deprecated() && !target->is_deprecated()) {
            ...
            DCHECK_IMPLIES(map_->prototype() != target->prototype(), IsUndefined(map_->GetBackPointer()));
        }
    ...
    ForEachTransition(
        ...
        [&](Tagged<Object> side_step) {
            if (!side_step.IsSmi()) {
                ...
                DCHECK_EQ(Cast<Map>(side_step)->GetInObjectProperties() - Cast<Map>(side_step)->UnusedInObjectProperties(),
