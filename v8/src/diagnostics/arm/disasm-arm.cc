...
private:
    ...
    int FormatVFPRegister(Instruction* instr, const char* format, VFPRegPrecision precision);
...
#define STRING_STARTS_WITH(string, compare_string) (strncmp(string, compare_string, strlen(compare_string)) == 0)
...
// formatting. See for example the command "objdump -d <binary file>".
static const char* const shift_names[kNumberOfShifts] = { "lsl", "lsr", "asr", "ror" };
...
void Decoder::PrintShiftRm(Instruction* instr)
{
    ...
    if (instr->RegShiftValue() == 0) {
        ...
        out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, ", %s #%d", shift_names[shift_index], shift_amount);
    } else {
        // by register
        int rs = instr->RsValue();
        out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, ", %s ", shift_names[shift_index]);
        PrintRegister(rs);
    }
}
...
void Decoder::PrintShiftSat(Instruction* instr)
{
    ...
    if (shift > 0) {
        out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, ", %s #%d", shift_names[instr->Bit(6) * 2], instr->Bits(11, 7));
    }
}
...
void Decoder::PrintSoftwareInterrupt(SoftwareInterruptCodes svc)
{
    ...
    default:
        if (svc >= kStopCode) {
            out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "%d - 0x%x", svc & kStopCodeMask, svc & kStopCodeMask);
        } else {
            out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "%d", svc);
        }
        return;
    }
}
...
// complexity of FormatOption.
int Decoder::FormatVFPRegister(Instruction* instr, const char* format, VFPRegPrecision precision)
{
    int retval = 2;
    int reg = -1;
    if (format[1] == 'n') {
...
{
    if (type == nlt_1) {
        out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "{d%d}", Vd);
    } else if (type == nlt_2) {
        out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "{d%d, d%d}", Vd, Vd + 1);
    } else if (type == nlt_3) {
        out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "{d%d, d%d, d%d}", Vd, Vd + 1, Vd + 2);
    } else if (type == nlt_4) {
        out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "{d%d, d%d, d%d, d%d}", Vd, Vd + 1, Vd + 2, Vd + 3);
    }
}

void Decoder::FormatNeonMemory(int Rn, int align, int Rm)
{
    out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "[%s", converter_.NameOfCPURegister(Rn));
    if (align != 0) {
        out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, ":%d", (1 << align) << 6);
    }
    if (Rm == 15) {
        Print("]");
    } else if (Rm == 13) {
        ...
        out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "], %s", converter_.NameOfCPURegister(Rm));
    }
}
...
int Decoder::FormatOption(Instruction* instr, const char* format)
{
    ...
    case 'd': { // 'd: vmov double immediate.
        ...
        out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "#%g", d);
        return 1;
    }
    case 'f': { // 'f: bitfield instructions - v7 and above.
        ...
        out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "#%d, #%d", lsbit, width);
        return 1;
    }
    ...
    case 'i': { // 'i: immediate value from adjacent bits.
        ...
        out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "%d", instr->Bits(width + lsb - 1, lsb));
        return 8;
    }
    ...
    case 'm': {
        ...
        out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "%s", converter_.NameInCode(str));
        return 3;
    }
    case 'o': {
        if ((format[3] == '1') && (format[4] == '2')) {
            ...
            out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "%d", instr->Offset12Value());
            return 5;
        } else if (format[3] == '0') {
            // 'off0to3and8to19 16-bit immediate encoded in bits 19-8 and 3-0.
            DCHECK(STRING_STARTS_WITH(format, "off0to3and8to19"));
            out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "%d", (instr->Bits(19, 8) << 4) + instr->Bits(3, 0));
            return 15;
        }
        // 'off8: 8-bit offset for extra load and store instructions
        DCHECK(STRING_STARTS_WITH(format, "off8"));
        int offs8 = (instr->ImmedHValue() << 4) | instr->ImmedLValue();
        out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "%d", offs8);
        return 4;
    }
    ...
    case 's': {
        if (format[1] == 'h') { // 'shift_op or 'shift_rm or 'shift_sat.
            ...
            if (format[2] == 'g') {
                ...
                DCHECK(STRING_STARTS_WITH(format, "size2") || STRING_STARTS_WITH(format, "size3"));
                int sz = 8 << (format[4] == '2' ? instr->Bits(19, 18) : instr->Bits(21, 20));
                out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "%d", sz);
                return 5;
            }
        } else if (format[1] == 'p') {
    ...
    case 't': { // 'target: target of branch instructions
        ...
        out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "%+d -> %s", off, converter_.NameOfAddress(reinterpret_cast<uint8_t*>(instr) + off));
        return 6;
    }
    ...
    case 'A': {
        ...
        uint8_t* pc = reinterpret_cast<uint8_t*>(instr) + Instruction::kPcLoadDelta;
        uint8_t* addr;
        switch (instr->PUField()) {
        case db_x: {
            addr = pc - offset;
        ...
        out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "0x%08" PRIxPTR, reinterpret_cast<uintptr_t>(addr));
        return 1;
    }
    case 'S':
    ...
    if (instr->Bit(4) == 0) {
        if (instr->Opc1Value() == 0x7) {
            ...
            if ((instr->Opc2Value() == 0x0) && (instr->Opc3Value() == 0x1)) {
                ...
                out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, ", #%d", fraction_bits);
        ...
        if ((instr->VCValue() == 0x0) && (instr->VAValue() == 0x0)) {
            ...
            if (instr->Bit(23) == 0) {
                ...
                if ((opc1_opc2 & 0xB) == 0) {
                    ...
                    if ((opc1_opc2 & 0x8) != 0) {
                        // NeonS8 / NeonU8
                        int i = opc1_opc2 & 0x7;
                        out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "vmov.8 d%d[%d], %s", vd, i, rt_name);
                    } else if ((opc1_opc2 & 0x1) != 0) {
                        // NeonS16 / NeonU16
                        int i = (opc1_opc2 >> 1) & 0x3;
                        out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "vmov.16 d%d[%d], %s", vd, i, rt_name);
                    } else {
                        Unknown(instr);
                    }
                }
                ...
                out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "vdup.%i q%d, %s", size, Vd, rt_name);
            }
            ...
            if ((opc1_opc2 & 0xB) == 0) {
                ...
                if ((opc1_opc2 & 0x8) != 0) {
                    // NeonS8 / NeonU8
                    int i = opc1_opc2 & 0x7;
                    out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "vmov.%c8 %s, d%d[%d]", sign, rt_name, vn, i);
                } else if ((opc1_opc2 & 0x1) != 0) {
                    // NeonS16 / NeonU16
                    int i = (opc1_opc2 >> 1) & 0x3;
                    out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "vmov.%c16 %s, d%d[%d]", sign, rt_name, vn, i);
                } else {
                    Unknown(instr);
                }
            }
...
void Decoder::DecodeVMOVBetweenCoreAndSinglePrecisionRegisters(Instruction* instr)
{
...
void Decoder::DecodeVmovImmediate(Instruction* instr)
{
    ...
    case 0: {
        uint32_t imm32 = imm;
        out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "vmov.i32 q%d, %d", vd, imm32);
        break;
    }
    case 0xe: {
        out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "vmov.i8 q%d, %d", vd, imm);
        break;
    }
    default:
        Unknown(instr);
        ...
        if (instr->Bits(21, 19) == 0 && instr->Bit(7) == 0) {
            ...
            if (imm3H_L != 0 && opc == 0) {
                ...
                if (q) {
                    ...
                    out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "vshr.%s%d q%d, q%d, #%d", u ? "u" : "s", size, Vd, Vm, shift);
                } else {
                    int Vd = instr->VFPDRegValue(kDoublePrecision);
                    int Vm = instr->VFPMRegValue(kDoublePrecision);
                    out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "vshr.%s%d d%d, d%d, #%d", u ? "u" : "s", size, Vd, Vm, shift);
                }
                ...
                if (q) {
                    int Vd = instr->VFPDRegValue(kSimd128Precision);
                    int Vm = instr->VFPMRegValue(kSimd128Precision);
                    out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "vsra.%s%d q%d, q%d, #%d", u ? "u" : "s", size, Vd, Vm, shift);
                } else {
                    int Vd = instr->VFPDRegValue(kDoublePrecision);
                    int Vm = instr->VFPMRegValue(kDoublePrecision);
                    out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "vsra.%s%d d%d, d%d, #%d", u ? "u" : "s", size, Vd, Vm, shift);
                }
                ...
                int Vm = instr->VFPMRegValue(kDoublePrecision);
                int imm3H = instr->Bits(21, 19);
                out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "vmovl.%s%d q%d, d%d", u ? "u" : "s", imm3H * 8, Vd, Vm);
                ...
                int Vd = instr->VFPDRegValue(kSimd128Precision);
                int Vm = instr->VFPMRegValue(kSimd128Precision);
                out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "vshl.i%d q%d, q%d, #%d", size, Vd, Vm, shift);
                ...
                int Vd = instr->VFPDRegValue(kDoublePrecision);
                int Vm = instr->VFPMRegValue(kDoublePrecision);
                out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "vs%ci.%d d%d, d%d, #%d", direction, size, Vd, Vm, shift);
            }
        }
    } else {
        Unknown(instr);
...
void Decoder::DecodeAdvancedSIMDTwoOrThreeRegisters(Instruction* instr)
{
    ...
    if (!op0 && op1 == 0b11) {
        ...
        out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "vext.8 q%d, q%d, q%d, #%d", Vd, Vn, Vm, imm4);
        ...
        if (opc1 == 0 && (opc2 >> 2) == 0) {
            ...
            // vrev<op>.<esize> Qd, Qm.
            out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "vrev%d.%d q%d, q%d", op, esize, Vd, Vm);
            ...
            DCHECK_NE(0b11, size);
            Format(instr, q ? "vceq.s'size2 'Qd, 'Qm, #0" : "vceq.s.'size2 'Dd, 'Dm, #0");
        } else if (opc1 == 0b01 && opc2 == 0b0100) {
            DCHECK_NE(0b11, size);
            Format(instr, q ? "vclt.s'size2 'Qd, 'Qm, #0" : "vclt.s.'size2 'Dd, 'Dm, #0");
            ...
            out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "%s.%c%i d%d, q%d", name, type, esize << 1, Vd, Vm);
            ...
            out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "vcvt.%s q%d, q%d", suffix, Vd, Vm);
        }
        ...
        out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "%s d%d, ", instr->Bit(6) == 0 ? "vtbl.8" : "vtbx.8", Vd);
        ...
        if (instr->Bits(9, 7) == 0) {
            ...
            if (instr->Bit(6) == 0) {
                int Vd = instr->VFPDRegValue(kDoublePrecision);
                out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "vdup.%i d%d, d%d[%d]", esize, Vd, Vm, index);
            } else {
                int Vd = instr->VFPDRegValue(kSimd128Precision);
                out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "vdup.%i q%d, d%d[%d]", esize, Vd, Vm, index);
            }
        } else {
            Unknown(instr);
        }
        ...
        int opc = instr->Bits(11, 8);
        if (opc == 0b1000) {
            Format(instr, u ? "vmlal.u'size3 'Qd, 'Dn, 'Dm" : "vmlal.s'size3 'Qd, 'Dn, 'Dm");
        } else if (opc == 0b1100) {
            Format(instr, u ? "vmull.u'size3 'Qd, 'Dn, 'Dm" : "vmull.s'size3 'Qd, 'Dn, 'Dm");
        }
    } else if (op1 != 0b11 && op3) {
...
void Decoder::DecodeMemoryHintsAndBarriers(Instruction* instr)
{
    ...
    if (op0 == 0b01011) {
        ...
        case 4:
            out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "dsb %s", barrier_option_names[option]);
            break;
        case 5:
            out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "dmb %s", barrier_option_names[option]);
            break;
        case 6:
            out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "isb %s", barrier_option_names[option]);
            break;
        default:
            Unknown(instr);
        }
        ...
        if (offset == 0) {
            out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "pld [%s]", rn_name);
        } else if (instr->Bit(23) == 0) {
            out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "pld [%s, #-%d]", rn_name, offset);
        } else {
            out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "pld [%s, #+%d]", rn_name, offset);
        }
    } else {
...
void Decoder::DecodeAdvancedSIMDElementOrStructureLoadStore(Instruction* instr)
{
    int op0 = instr->Bit(23);
    ...
    if (op0 == 0) {
        ...
        if (itype == nlt_1 || itype == nlt_2 || itype == nlt_3 || itype == nlt_4) {
            ...
            out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, op, (1 << size) << 3);
        ...
        if (l && n == 0b00) {
            ...
            out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "vld1.%d ", (1 << size) << 3);
        ...
        if (n == 0b00) {
            ...
            // Omit alignment.
            out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "v%s1.%d {d%d[%d]}", (l ? "ld" : "st"), (1 << size) << 3, Vd, index);
            Print(", ");
            FormatNeonMemory(Rn, 0, Rm);
        } else {
            Unknown(instr);
...
int Decoder::InstructionDecode(uint8_t* instr_ptr)
{
    ...
    out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "%08x       ", instr->InstructionBits());
    ...
    if ((instruction_bits & kConstantPoolMarkerMask) == kConstantPoolMarker) {
        out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "constant pool begin (length %d)", DecodeConstantPoolLength(instruction_bits));
        return kInstrSize;
    }
...
void Disassembler::Disassemble(FILE* f, uint8_t* begin, uint8_t* end, UnimplementedOpcodeAction unimplemented_action)
{
    ...
    for (uint8_t* pc = begin; pc < end;) {
        ...
        uint8_t* prev_pc = pc;
        pc += d.InstructionDecode(buffer, pc);
        v8::internal::PrintF(f, "%p    %08x      %s\n", static_cast<void*>(prev_pc), *reinterpret_cast<int32_t*>(prev_pc), buffer.begin());
    }
}
