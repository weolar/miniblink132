...
#if !defined(V8_TARGET_ARCH_X64) && !defined(V8_TARGET_ARCH_ARM) && !defined(V8_TARGET_ARCH_ARM64) && !defined(V8_TARGET_ARCH_S390X)                           \
...
STATIC_CONST_MEMBER_DEFINITION const int EhFrameConstants::kEhFrameTerminatorSize;
...
void EhFrameWriter::WriteCie()
{
    ...
    DCHECK_EQ(eh_frame_offset() - size_offset, EhFrameConstants::kInitialStateOffsetInCie);
...
void EhFrameWriter::WriteEhFrameHdr(int code_size)
{
    ...
    DCHECK_EQ(eh_frame_offset() - eh_frame_size, EhFrameConstants::kEhFrameHdrSize);
}
...
void EhFrameWriter::AdvanceLocation(int pc_offset)
{
    ...
    if (factored_delta <= EhFrameConstants::kLocationMask) {
        WriteByte((EhFrameConstants::kLocationTag << EhFrameConstants::kLocationMaskSize) | (factored_delta & EhFrameConstants::kLocationMask));
...
void EhFrameWriter::SetBaseAddressRegisterAndOffset(Register base_register, int base_offset)
{
    DCHECK_EQ(writer_state_, InternalState::kInitialized);
    DCHECK_GE(base_offset, 0);
    int code = RegisterToDwarfCode(base_register);
...
void EhFrameWriter::RecordRegisterSavedToStack(int dwarf_register_code, int offset)
{
    DCHECK_EQ(writer_state_, InternalState::kInitialized);
    ...
    if (factored_offset >= 0) {
        ...
        WriteByte(
            (EhFrameConstants::kSavedRegisterTag << EhFrameConstants::kSavedRegisterMaskSize) | (dwarf_register_code & EhFrameConstants::kSavedRegisterMask));
...
void EhFrameWriter::RecordRegisterFollowsInitialRule(int dwarf_register_code)
{
    ...
    if (dwarf_register_code <= EhFrameConstants::kFollowInitialRuleMask) {
        WriteByte((EhFrameConstants::kFollowInitialRuleTag << EhFrameConstants::kFollowInitialRuleMaskSize)
...
void EhFrameWriter::Finish(int code_size)
{
    ...
    PatchInt32(GetProcedureAddressOffset(), -(RoundUp(code_size, 8) + GetProcedureAddressOffset()));
    PatchInt32(GetProcedureSizeOffset(), code_size);

    // Terminate the .eh_frame.
    static const uint8_t kTerminator[EhFrameConstants::kEhFrameTerminatorSize] = { 0 };
...
// static
uint32_t EhFrameIterator::DecodeULeb128(const uint8_t* encoded, int* encoded_size)
{
...
// static
int32_t EhFrameIterator::DecodeSLeb128(const uint8_t* encoded, int* encoded_size)
{
...
// static
void EhFrameDisassembler::DumpDwarfDirectives(std::ostream& stream, const uint8_t* start, const uint8_t* end)
{
    ...
    while (!eh_frame_iterator.Done()) {
        ...
        if (((bytecode >> EhFrameConstants::kSavedRegisterMaskSize) & 0xFF) == EhFrameConstants::kSavedRegisterTag) {
            ...
            stream << "| " << DwarfRegisterCodeToString(bytecode & EhFrameConstants::kLocationMask) << " saved at base" << std::showpos
                   << decoded_offset * EhFrameConstants::kDataAlignmentFactor << std::noshowpos << '\n';
            continue;
        }

        if (((bytecode >> EhFrameConstants::kFollowInitialRuleMaskSize) & 0xFF) == EhFrameConstants::kFollowInitialRuleTag) {
            stream << "| " << DwarfRegisterCodeToString(bytecode & EhFrameConstants::kLocationMask) << " follows rule in CIE\n";
            continue;
        }

        switch (static_cast<EhFrameConstants::DwarfOpcodes>(bytecode)) {
        case EhFrameConstants::DwarfOpcodes::kOffsetExtendedSf: {
            stream << "| " << DwarfRegisterCodeToString(eh_frame_iterator.GetNextULeb128());
            int32_t decoded_offset = eh_frame_iterator.GetNextSLeb128();
            stream << " saved at base" << std::showpos << decoded_offset * EhFrameConstants::kDataAlignmentFactor << std::noshowpos << '\n';
            break;
        }
        ...
        case EhFrameConstants::DwarfOpcodes::kDefCfa: {
            ...
            stream << "| base_register=" << DwarfRegisterCodeToString(base_register) << ", base_offset=" << base_offset << '\n';
            break;
        }
        ...
        case EhFrameConstants::DwarfOpcodes::kDefCfaRegister: {
            stream << "| base_register=" << DwarfRegisterCodeToString(eh_frame_iterator.GetNextULeb128()) << '\n';
            break;
        }
        case EhFrameConstants::DwarfOpcodes::kSameValue: {
            stream << "| " << DwarfRegisterCodeToString(eh_frame_iterator.GetNextULeb128()) << " not modified from previous frame\n";
            break;
        }
...
void EhFrameDisassembler::DisassembleToStream(std::ostream& stream)
{
    ...
    const int cie_size = base::ReadUnalignedValue<uint32_t>(reinterpret_cast<Address>(start_)) + kInt32Size;
    const int fde_offset = cie_size;

    const uint8_t* cie_directives_start = start_ + EhFrameConstants::kInitialStateOffsetInCie;
    ...
    int32_t procedure_offset = base::ReadUnalignedValue<int32_t>(procedure_offset_address);

    Address procedure_size_address = reinterpret_cast<Address>(start_) + fde_offset + EhFrameConstants::kProcedureSizeOffsetInFde;
    uint32_t procedure_size = base::ReadUnalignedValue<uint32_t>(procedure_size_address);

    const uint8_t* fde_start = start_ + fde_offset;
    stream << reinterpret_cast<const void*>(fde_start) << "  .eh_frame: FDE\n"
           << reinterpret_cast<const void*>(procedure_offset_address) << "  | procedure_offset=" << procedure_offset << '\n'
           << reinterpret_cast<const void*>(procedure_size_address) << "  | procedure_size=" << procedure_size << '\n';

    const int fde_directives_offset = fde_offset + 4 * kInt32Size + 1;

    const uint8_t* fde_directives_start = start_ + fde_directives_offset;
    const uint8_t* fde_directives_end = end_ - EhFrameConstants::kEhFrameHdrSize - EhFrameConstants::kEhFrameTerminatorSize;
    ...
    stream << reinterpret_cast<const void*>(fde_terminator_start) << "  .eh_frame: terminator\n";

    const uint8_t* eh_frame_hdr_start = fde_terminator_start + EhFrameConstants::kEhFrameTerminatorSize;
    stream << reinterpret_cast<const void*>(eh_frame_hdr_start) << "  .eh_frame_hdr\n";
}
