...
void BaselineCompiler::PrologueFillFrame()
{
    ...
    if (v8_flags.debug_code) {
        __ masm()->CompareRoot(kInterpreterAccumulatorRegister, RootIndex::kUndefinedValue);
    ...
    if (has_new_target) {
        ...
        for (; before_new_target_count + 2 <= new_target_index; before_new_target_count += 2) {
            __ masm()->Push(kInterpreterAccumulatorRegister, kInterpreterAccumulatorRegister);
        }
        if (before_new_target_count == new_target_index) {
            __ masm()->Push(kJavaScriptCallNewTargetRegister, kInterpreterAccumulatorRegister);
        } else {
            DCHECK_EQ(before_new_target_count + 1, new_target_index);
            __ masm()->Push(kInterpreterAccumulatorRegister, kJavaScriptCallNewTargetRegister);
        }
...
{
    ASM_CODE_COMMENT(&masm_);
    __ masm()->Add(x15, sp, RoundUp(InterpreterFrameConstants::kFixedFrameSizeFromFp + bytecode_->frame_size(), 2 * kSystemPointerSize));
    __ masm()->Cmp(x15, fp);
