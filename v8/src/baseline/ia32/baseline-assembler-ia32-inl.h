...
{
    return __ lea(rscratch, MemOperand(ebp, interpreter_register.ToOperand() * kSystemPointerSize));
}
    ...
    Register scratch_and_result, Register feedback_vector, FeedbackSlot slot, Label* on_result, Label::Distance distance)
{
    ...
    // Is it marked_for_deoptimization? If yes, clear the slot.
    {
        ...
        __ mov(scratch_and_result, FieldOperand(scratch_and_result, CodeWrapper::kCodeOffset));

        __ TestCodeIsMarkedForDeoptimization(scratch_and_result);
        __ j(equal, on_result, distance);
        __ mov(FieldOperand(feedback_vector, FeedbackVector::OffsetOfElementAt(slot.ToInt())), __ ClearedValue());
    }
...
void BaselineAssembler::AddToInterruptBudgetAndJumpIfNotExceeded(int32_t weight, Label* skip_interrupt_label)
{
    ...
    __ add(FieldOperand(feedback_cell, FeedbackCell::kInterruptBudgetOffset), Immediate(weight));
...
void BaselineAssembler::AddToInterruptBudgetAndJumpIfNotExceeded(Register weight, Label* skip_interrupt_label)
{
    ...
    LoadFeedbackCell(feedback_cell);
    __ add(FieldOperand(feedback_cell, FeedbackCell::kInterruptBudgetOffset), weight);
    if (skip_interrupt_label)
        __ j(greater_equal, skip_interrupt_label);
}
...
void BaselineAssembler::EmitReturn(MacroAssembler* masm)
{
    ...
    __ masm()->mov(actual_params_size, MemOperand(ebp, StandardFrameConstants::kArgCOffset));
