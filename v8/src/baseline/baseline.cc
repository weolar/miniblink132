...
bool CanCompileWithBaseline(Isolate* isolate, Tagged<SharedFunctionInfo> shared)
{
    ...
    if (!v8_flags.sparkplug)
        return false;

    // Check that short builtin calls are enabled if needed.
    if (v8_flags.sparkplug_needs_short_builtins && !isolate->is_short_builtin_calls_enabled()) {
        return false;
    }
    ...
    if (auto debug_info = shared->TryGetDebugInfo(isolate)) {
        ...
        if (debug_info.value()->HasBreakInfo())
            return false;
    ...
    if (!shared->PassesFilter(v8_flags.sparkplug_filter))
        return false;
