...
{
    return __ leaq(rscratch, MemOperand(rbp, interpreter_register.ToOperand() * kSystemPointerSize));
}
...
void BaselineAssembler::JumpIfSmi(Condition cc, Register lhs, Tagged<Smi> smi, Label* target, Label::Distance distance)
{
...
template <typename Arg> struct PushHelper {
...
template <> struct PushHelper<interpreter::RegisterList> {
...
void BaselineAssembler::LoadTaggedField(TaggedRegister output, Register source, int offset)
{
    __ LoadTaggedField(output, FieldOperand(source, offset));
}

void BaselineAssembler::LoadTaggedField(TaggedRegister output, TaggedRegister source, int offset)
{
    __ LoadTaggedField(output, FieldOperand(source, offset));
}

void BaselineAssembler::LoadTaggedField(Register output, TaggedRegister source, int offset)
{
    __ LoadTaggedField(output, FieldOperand(source, offset));
}

void BaselineAssembler::LoadFixedArrayElement(Register output, TaggedRegister array, int32_t index)
{
    LoadTaggedField(output, array, OFFSET_OF_DATA_START(FixedArray) + index * kTaggedSize);
}

void BaselineAssembler::LoadFixedArrayElement(TaggedRegister output, TaggedRegister array, int32_t index)
{
    LoadTaggedField(output, array, OFFSET_OF_DATA_START(FixedArray) + index * kTaggedSize);
}
...
{
    __ MacroAssembler::TryLoadOptimizedOsrCode(scratch_and_result, CodeKind::MAGLEV, feedback_vector, slot, on_result, distance);
}
...
void BaselineAssembler::AddToInterruptBudgetAndJumpIfNotExceeded(int32_t weight, Label* skip_interrupt_label)
{
    ...
    __ addl(FieldOperand(feedback_cell, FeedbackCell::kInterruptBudgetOffset), Immediate(weight));
...
void BaselineAssembler::AddToInterruptBudgetAndJumpIfNotExceeded(Register weight, Label* skip_interrupt_label)
{
    ...
    Register feedback_cell = scratch_scope.AcquireScratch();
    LoadFeedbackCell(feedback_cell);
    __ addl(FieldOperand(feedback_cell, FeedbackCell::kInterruptBudgetOffset), weight);
    if (skip_interrupt_label)
        __ j(greater_equal, skip_interrupt_label);
}
...
void BaselineAssembler::EmitReturn(MacroAssembler* masm)
{
    ...
    __ masm()->movq(actual_params_size, MemOperand(rbp, StandardFrameConstants::kArgCOffset));
