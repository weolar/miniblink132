...
void BaselineCompiler::PrologueFillFrame()
{
    ...
    if (v8_flags.debug_code) {
        __ masm()->Cmp(kInterpreterAccumulatorRegister, handle(ReadOnlyRoots(local_isolate_).undefined_value(), local_isolate_));
...
void BaselineCompiler::VerifyFrameSize()
{
    ...
    __ masm()->addq(kScratchRegister, Immediate(InterpreterFrameConstants::kFixedFrameSizeFromFp + bytecode_->frame_size()));
