...
{
    return __ AddS64(rscratch, fp, Operand(interpreter_register.ToOperand() * kSystemPointerSize));
}
...
void BaselineAssembler::TryLoadOptimizedOsrCode(Register scratch_and_result, Register feedback_vector, FeedbackSlot slot, Label* on_result, Label::Distance)
{
    ...
    // Is it marked_for_deoptimization? If yes, clear the slot.
    {
        ...
        __ LoadCodePointerField(scratch_and_result, FieldMemOperand(scratch_and_result, CodeWrapper::kCodeOffset), r0);
...
void BaselineAssembler::AddToInterruptBudgetAndJumpIfNotExceeded(int32_t weight, Label* skip_interrupt_label)
{
    ...
    __ LoadU32(interrupt_budget, FieldMemOperand(feedback_cell, FeedbackCell::kInterruptBudgetOffset), r0);
