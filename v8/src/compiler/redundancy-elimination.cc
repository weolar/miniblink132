...
RedundancyElimination::RedundancyElimination(Editor* editor, JSGraph* jsgraph, Zone* zone)
...
// static
RedundancyElimination::EffectPathChecks* RedundancyElimination::EffectPathChecks::Copy(Zone* zone, EffectPathChecks const* checks)
{
    return zone->New<EffectPathChecks>(*checks);
}

// static
RedundancyElimination::EffectPathChecks const* RedundancyElimination::EffectPathChecks::Empty(Zone* zone)
{
    return zone->New<EffectPathChecks>(nullptr, 0);
}

bool RedundancyElimination::EffectPathChecks::Equals(EffectPathChecks const* that) const
{
...
void RedundancyElimination::EffectPathChecks::Merge(EffectPathChecks const* that)
{
...
RedundancyElimination::EffectPathChecks const* RedundancyElimination::EffectPathChecks::AddCheck(Zone* zone, Node* node) const
{
...
struct Subsumption {
    ...
    static Subsumption None()
    {
        return Subsumption(Kind::kNone, nullptr);
    }
...
Subsumption CheckSubsumes(Node const* a, Node const* b, MachineOperatorBuilder* machine)
{
    ...
    if (a->op() != b->op()) {
        ...
        } else if (a->opcode() == IrOpcode::kCheckString && b->opcode() == IrOpcode::kCheckStringOrStringWrapper) {
            // CheckString(node) implies CheckStringOrStringWrapper(node)
        } else if (a->opcode() == IrOpcode::kCheckInternalizedString && b->opcode() == IrOpcode::kCheckStringOrStringWrapper) {
        ...
        } else if (a->opcode() == IrOpcode::kCheckedTaggedSignedToInt32 && b->opcode() == IrOpcode::kCheckedTaggedToArrayIndex) {
            // CheckedTaggedSignedToInt32(node) implies
            // CheckedTaggedToArrayIndex(node)
            if (machine->Is64()) {
                // On 64 bit architectures, ArrayIndex is 64 bit.
                subsumption = Subsumption::WithConversion(machine->ChangeInt32ToInt64());
            }
        } else if (a->opcode() == IrOpcode::kCheckedTaggedToInt32 && b->opcode() == IrOpcode::kCheckedTaggedToArrayIndex) {
...
Node* RedundancyElimination::EffectPathChecks::LookupCheck(Node* node, JSGraph* jsgraph) const
{
    for (Check const* check = head_; check != nullptr; check = check->next) {
        Subsumption subsumption = CheckSubsumes(check->node, node, jsgraph->machine());
        if (!subsumption.IsNone() && TypeSubsumes(node, check->node)) {
            DCHECK(!check->node->IsDead());
            Node* result = check->node;
            if (subsumption.IsWithConversion()) {
                result = jsgraph->graph()->NewNode(subsumption.conversion_operator(), result);
            }
            return result;
        }
    }
...
Node* RedundancyElimination::EffectPathChecks::LookupBoundsCheckFor(Node* node) const
{
    for (Check const* check = head_; check != nullptr; check = check->next) {
        if (check->node->opcode() == IrOpcode::kCheckBounds && check->node->InputAt(0) == node && TypeSubsumes(node, check->node)
            && !(CheckBoundsParametersOf(check->node->op()).flags() & CheckBoundsFlag::kConvertStringAndMinusZero)) {
            return check->node;
        }
    }
    return nullptr;
...
RedundancyElimination::EffectPathChecks const* RedundancyElimination::PathChecksForEffectNodes::Get(Node* node) const
{
...
void RedundancyElimination::PathChecksForEffectNodes::Set(Node* node, EffectPathChecks const* checks)
{
    size_t const id = node->id();
    if (id >= info_for_node_.size())
...
Reduction RedundancyElimination::ReduceEffectPhi(Node* node)
{
    ...
    // from other inputs.
    EffectPathChecks* checks = EffectPathChecks::Copy(zone(), node_checks_.Get(NodeProperties::GetEffectInput(node, 0)));
        ...
        if (!first_type.Is(Type::UnsignedSmall())) {
            if (Node* check = checks->LookupBoundsCheckFor(first)) {
                if (!first_type.Is(NodeProperties::GetType(check))) {
                    ...
                    return Changed(node).FollowedBy(ReduceSpeculativeNumberComparison(node));
                }
            }
        }
...
{
    DCHECK(node->opcode() == IrOpcode::kSpeculativeNumberAdd || node->opcode() == IrOpcode::kSpeculativeNumberSubtract
        || node->opcode() == IrOpcode::kSpeculativeSafeIntegerAdd || node->opcode() == IrOpcode::kSpeculativeSafeIntegerSubtract
