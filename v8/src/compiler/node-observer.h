...
inline bool operator==(const ObservableNodeState& lhs, const ObservableNodeState& rhs)
{
    return lhs.id() == rhs.id() && lhs.op() == rhs.op() && lhs.type() == rhs.type();
}
...
public:
    ...
    {
        has_observed_changes_ = true;
    }
    bool has_observed_changes() const
    {
        return has_observed_changes_;
    }
...
public:
    explicit ObserveNodeManager(Zone* zone)
        : zone_(zone)
        , observations_(zone)
    {
    }
...
struct ObserveNodeInfo {
    ObserveNodeInfo()
        : observe_node_manager(nullptr)
        , node_observer(nullptr)
    {
    }
    ObserveNodeInfo(ObserveNodeManager* manager, NodeObserver* observer)
        : observe_node_manager(manager)
