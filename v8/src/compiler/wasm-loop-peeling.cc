    ...
    SourcePositionTable* source_positions, NodeOriginTable* node_origins)
{
    ...
    for (Node* node : loop_node->uses()) {
        ...
        Node* merge_node = graph->NewNode(common->Merge(2), node, copier.map(node)->InputAt(0));
        ...
        for (Edge use_edge : node->use_edges()) {
            ...
            if (loop->count(use) == 1) {
                ...
                DCHECK(use->opcode() == IrOpcode::kLoopExitEffect || use->opcode() == IrOpcode::kLoopExitValue);
                const Operator* phi_operator
                    = use->opcode() == IrOpcode::kLoopExitEffect ? common->EffectPhi(2) : common->Phi(LoopExitValueRepresentationOf(use->op()), 2);
                Node* phi = graph->NewNode(phi_operator, use, copier.map(use)->InputAt(0), merge_node);
                use->ReplaceUses(phi);
    ...
    NodeProperties::ChangeOp(peeled_iteration_header, common->Merge(peeled_iteration_header->InputCount()));

    // Remove the non-recursive input.
    for (Edge use_edge : peeled_iteration_header->use_edges()) {
        DCHECK(NodeProperties::IsPhi(use_edge.from()));
        use_edge.from()->RemoveInput(0);
        const Operator* phi = common->ResizeMergeOrPhi(use_edge.from()->op(), use_edge.from()->InputCount() - /* control input */ 1);
