...
public:
    FrameStateFunctionInfo(FrameStateType type, uint16_t parameter_count, uint16_t max_arguments, int local_count,
        IndirectHandle<SharedFunctionInfo> shared_info, MaybeIndirectHandle<BytecodeArray> bytecode_array, uint32_t wasm_liftoff_frame_size = 0,
    ...
    static bool IsJSFunctionType(FrameStateType type)
    {
        ...
        return type == FrameStateType::kUnoptimizedFunction || type == FrameStateType::kJavaScriptBuiltinContinuation
...
public:
    JSToWasmFrameStateFunctionInfo(
        FrameStateType type, uint16_t parameter_count, int local_count, IndirectHandle<SharedFunctionInfo> shared_info, const wasm::CanonicalSig* signature)
        : FrameStateFunctionInfo(type, parameter_count, 0, local_count, shared_info, {})
...
public:
    ...
    FrameStateType type() const
    {
        return info_ == nullptr ? FrameStateType::kUnoptimizedFunction : info_->type();
    }
    ...
    {
        return info_ == nullptr ? MaybeIndirectHandle<SharedFunctionInfo>() : info_->shared_info();
    }
    MaybeIndirectHandle<BytecodeArray> bytecode_array() const
    {
        return info_ == nullptr ? MaybeIndirectHandle<BytecodeArray>() : info_->bytecode_array();
    }
    uint16_t parameter_count() const
    {
...
FrameState CreateStubBuiltinContinuationFrameState(JSGraph* graph, Builtin name, Node* context, Node* const* parameters, int parameter_count,
    Node* outer_frame_state, ContinuationFrameStateMode mode, const wasm::CanonicalSig* signature = nullptr);
...
FrameState CreateJavaScriptBuiltinContinuationFrameState(JSGraph* graph, SharedFunctionInfoRef shared, Builtin name, Node* target, Node* context,
