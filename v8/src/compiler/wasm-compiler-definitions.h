...
template <typename T>
CallDescriptor* GetWasmCallDescriptor(Zone* zone, const Signature<T>* signature, WasmCallKind kind = kWasmFunction, bool need_frame_state = false);

extern template EXPORT_TEMPLATE_DECLARE(V8_EXPORT_PRIVATE) CallDescriptor* GetWasmCallDescriptor(Zone*, const Signature<wasm::ValueType>*, WasmCallKind, bool);

template <typename T> LocationSignature* BuildLocations(Zone* zone, const Signature<T>* sig, bool extra_callable_param, int* parameter_slots, int* return_slots)
{
    int extra_params = extra_callable_param ? 2 : 1;
    LocationSignature::Builder locations(zone, sig->return_count(), sig->parameter_count() + extra_params);
    int untagged_parameter_slots; // Unused.
    int untagged_return_slots; // Unused.
    wasm::IterateSignatureImpl(sig, extra_callable_param, locations, &untagged_parameter_slots, parameter_slots, &untagged_return_slots, return_slots);
