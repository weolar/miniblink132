...
{
    if (node->opcode() == IrOpcode::kProtectedLoad || node->opcode() == IrOpcode::kLoad || node->opcode() == IrOpcode::kLoadTransform) {
        return true;
    }
    return false;
}
...
{
    DCHECK(IsSupportedLoad(node) || node->opcode() == IrOpcode::kStore || node->opcode() == IrOpcode::kProtectedStore);

    Node* offset = node->InputAt(0);
    if (offset->opcode() == IrOpcode::kLoadFromObject || offset->opcode() == IrOpcode::kLoad) {
        return 0;
    }
...
bool IsContinuousAccess(const ZoneVector<Node*>& node_group)
{
    ...
    for (size_t i = 1; i < node_group.size(); ++i) {
        ...
        if (diff == 8 && node_group[0]->opcode() == IrOpcode::kLoadTransform) {
            LoadTransformParameters params = LoadTransformParametersOf(node_group[0]->op());
            if (params.transformation < LoadTransformation::kFirst128Extend || params.transformation > LoadTransformation::kLast128Extend) {
...
bool AllSameAddress(const ZoneVector<Node*>& nodes)
{
    ...
    for (size_t i = 1; i < nodes.size(); i++) {
        if (GetNodeAddress(nodes[i]) != address) {
            TRACE("Diff address #%d,#%d!\n", address->id(), GetNodeAddress(nodes[i])->id());
            return false;
        }
    }
    return true;
...
V8_INLINE static bool OperatorCanBePacked(const Operator* lhs, const Operator* rhs)
{
    return lhs->opcode() == rhs->opcode() && lhs->properties() == rhs->properties();
}
...
#define CHECK_SIGN_EXTENSION_CASE(op_low, op_high, not_used)                                                                                                   \
    case IrOpcode::k##op_low: {                                                                                                                                \
        if (node_group[1]->opcode() == IrOpcode::k##op_high && node_group[0]->InputAt(0) == node_group[1]->InputAt(0)) {                                       \
            return true;                                                                                                                                       \
        }                                                                                                                                                      \
        return false;                                                                                                                                          \
    }
...
public:
    explicit EffectChainIterator(Node* node)
        : node_(node)
        , prev_(nullptr)
    {
    }
...
void PackNode::Print() const
{
    if (revectorized_node_ != nullptr) {
        TRACE("0x%p #%d:%s(%d %d, %s)\n", this, revectorized_node_->id(), revectorized_node_->op()->mnemonic(), nodes_[0]->id(), nodes_[1]->id(),
            nodes_[0]->op()->mnemonic());
    } else {
        TRACE("0x%p null(%d %d, %s)\n", this, nodes_[0]->id(), nodes_[1]->id(), nodes_[0]->op()->mnemonic());
    }
}
...
bool SLPTree::CanBePacked(const ZoneVector<Node*>& node_group)
{
    ...
    if (!NodeProperties::IsSimd128Operation(node_group[0]) && (op != IrOpcode::kStore) && (op != IrOpcode::kProtectedStore) && (op != IrOpcode::kLoad)
        && (op != IrOpcode::kProtectedLoad) && (op != IrOpcode::kPhi) && (op != IrOpcode::kLoopExitValue) && (op != IrOpcode::kExtractF128)) {
        return false;
    }

    // TODO(jiepan): add support for Constant
    if (AllConstant(node_group)) {
        TRACE("%s(#%d, #%d) are constantant, not supported yet!\n", node_group[0]->op()->mnemonic(), node_group[0]->id(), node_group[1]->id());
        return false;
    }
...
PackNode* SLPTree::NewPackNodeAndRecurs(const ZoneVector<Node*>& node_group, int start_index, int count, unsigned recursion_depth)
{
...
{
    TRACE("Stack Push (%d %s, %d %s)\n", node_group[0]->id(), node_group[0]->op()->mnemonic(), node_group[1]->id(), node_group[1]->op()->mnemonic());
    for (auto node : node_group) {
...
void SLPTree::TryReduceLoadChain(const ZoneVector<Node*>& loads)
{
    ...
    for (Node* load : loads) {
        if (visited.find(load) != visited.end())
            continue;
        visited.insert(load);
...
bool SLPTree::IsSideEffectFreeLoad(const ZoneVector<Node*>& node_group)
{
    ...
    for (size_t i = 0, e = node_group.size(); i < e; i++) {
        ...
        for (int j = 0; j < NodeProperties::FirstControlIndex(load); ++j) {
            ...
            if (std::find(node_group.begin(), node_group.end(), input) == node_group.end()) {
                to_visit.push(input);
            }
        }
    }
...
PackNode* SLPTree::BuildTreeRec(const ZoneVector<Node*>& node_group, unsigned recursion_depth)
{
    TRACE("Enter %s\n", __func__);
    DCHECK_EQ(node_group.size(), 2);
    ...
    if (AllOnStack(node_group)) {
        if (!StackTopIsPhi()) {
            TRACE("Failed due to (%d %s, %d %s) on stack!\n", node0->id(), node0->op()->mnemonic(), node1->id(), node1->op()->mnemonic());
            return nullptr;
        }
    }
    ...
    DCHECK(AllConstant(node_group) || AllPackableOperator(node_group) || MaybePackSignExtensionOp(node_group));
    ...
    if (node0->opcode() == IrOpcode::kExtractF128) {
        ...
        TRACE("Extract leaf node from #%d,%s!\n", source->id(), source->op()->mnemonic());
        // For 256 only, check whether they are from the same source
        if (node0->InputAt(0) == node1->InputAt(0)
            && (node0->InputAt(0)->opcode() == IrOpcode::kLoadTransform ? node0 == node1
                                                                        : OpParameter<int32_t>(node0->op()) + 1 == OpParameter<int32_t>(node1->op()))) {
    ...
    if (IsSupportedLoad(node0)) {
        ...
        if (!IsSplat(node_group)) {
            if (node0->opcode() == IrOpcode::kProtectedLoad && LoadRepresentationOf(node0->op()).representation() != MachineRepresentation::kSimd128) {
                PopStack();
                return nullptr;
            }
            ...
            std::partial_sort_copy(node_group.begin(), node_group.end(), sorted_node_group.begin(), sorted_node_group.end(), MemoryOffsetComparer());
...
#define SIGN_EXTENSION_CASE(op_low, not_used1, not_used2) case IrOpcode::k##op_low:
    switch (node0->opcode()) {
    case IrOpcode::kPhi: {
    ...
    case IrOpcode::kI8x16Shuffle: {
        ...
        if (IsSplat(node_group)) {
            ...
            int index;
            if ((wasm::SimdShuffle::TryMatchSplat<4>(shuffle, &index) && node0->InputAt(index >> 2)->opcode() == IrOpcode::kProtectedLoad)
                || (wasm::SimdShuffle::TryMatchSplat<2>(shuffle, &index) && node0->InputAt(index >> 1)->opcode() == IrOpcode::kProtectedLoad)) {
                PopStack();
            ...
            PopStack();
            return NewPackNodeAndRecurs(node_group, 0, value_in_count, recursion_depth);
        }
    }
        // clang-format on
        SIMPLE_SIMD_OP(CASE)
        ...
        SIMD_SHIFT_OP(CASE)
        {
            if (ShiftBySameScalar(node_group)) {
                TRACE("Added a vector of %s.\n", node0->op()->mnemonic());
                PackNode* pnode = NewPackNodeAndRecurs(node_group, 0, 1, recursion_depth);
                PopStack();
                return pnode;
            }
...
template <typename FunctionType> void SLPTree::ForEach(FunctionType callback)
{
...
Revectorizer::Revectorizer(Zone* zone, Graph* graph, MachineGraph* mcgraph, SourcePositionTable* source_positions)
    : zone_(zone)
    , graph_(graph)
    , mcgraph_(mcgraph)
    ...
    slp_tree_->ForEach([&](PackNode const* pnode) {
        ...
        for (size_t i = 0; i < nodes.size(); i++) {
            ...
            for (auto edge : nodes[i]->use_edges()) {
                ...
                if (!GetPackNode(useNode) && !(useNode->uses().empty()) && useNode->opcode() != IrOpcode::kLoopExitValue) {
                    TRACE("External use edge: (%d:%s) -> (%d:%s)\n", useNode->id(), useNode->op()->mnemonic(), nodes[i]->id(), nodes[i]->op()->mnemonic());
                    cost++;
...
void Revectorizer::SetEffectInput(PackNode* pnode, int index, Node*& input)
{
    ...
    DCHECK(nodes[0] == nodes[1] || NodeProperties::GetEffectInput(nodes[0]) == nodes[1] || NodeProperties::GetEffectInput(nodes[1]) == nodes[0]);
...
void Revectorizer::SetMemoryOpInputs(base::SmallVector<Node*, 2>& inputs, PackNode* pnode, int effect_index)
{
...
Node* Revectorizer::VectorizeTree(PackNode* pnode)
{
    ...
    TRACE("Vectorize #%d:%s, input count: %d\n", node0->id(), node0->op()->mnemonic(), input_count);
    ...
    switch (op) {
    case IrOpcode::kPhi: {
        DCHECK_EQ(PhiRepresentationOf(node0->op()), MachineRepresentation::kSimd128);
        new_op = mcgraph_->common()->Phi(MachineRepresentation::kSimd256, input_count - 1);
        inputs[input_count - 1] = NodeProperties::GetControlInput(node0);
        break;
    }
    case IrOpcode::kLoopExitValue: {
        DCHECK_EQ(LoopExitValueRepresentationOf(node0->op()), MachineRepresentation::kSimd128);
        new_op = mcgraph_->common()->LoopExitValue(MachineRepresentation::kSimd256);
        inputs[input_count - 1] = NodeProperties::GetControlInput(node0);
        break;
    }
...
#undef SIMD_SPLAT_OP
    ...
    case IrOpcode::kProtectedLoad: {
        DCHECK_EQ(LoadRepresentationOf(node0->op()).representation(), MachineRepresentation::kSimd128);
    ...
    case IrOpcode::kProtectedStore: {
        DCHECK_EQ(StoreRepresentationOf(node0->op()).representation(), MachineRepresentation::kSimd128);
        new_op = mcgraph_->machine()->ProtectedStore(MachineRepresentation::kSimd256);
        SetMemoryOpInputs(inputs, pnode, 3);
        break;
    }
    case IrOpcode::kStore: {
        DCHECK_EQ(StoreRepresentationOf(node0->op()).representation(), MachineRepresentation::kSimd128);
        WriteBarrierKind write_barrier_kind = StoreRepresentationOf(node0->op()).write_barrier_kind();
        new_op = mcgraph_->machine()->Store(StoreRepresentation(MachineRepresentation::kSimd256, write_barrier_kind));
        SetMemoryOpInputs(inputs, pnode, 3);
        break;
    }
    case IrOpcode::kLoadTransform: {
      ...
      switch (params.transformation) {
        ...
        new_op = mcgraph_->machine()->LoadTransform(params.kind, new_transformation);
        SetMemoryOpInputs(inputs, pnode, 2);
        break;
    }
    ...
    if (new_op != nullptr) {
        ...
        Node* new_node = graph()->NewNode(new_op, input_count, inputs.begin(), true);
        ...
        for (size_t i = 0; i < nodes.size(); i++) {
            ...
            for (auto edge : nodes[i]->use_edges()) {
                ...
                if (!GetPackNode(useNode)) {
                    if (NodeProperties::IsValueEdge(edge)) {
                        // Extract use
                        TRACE("Replace Value Edge from %d:%s, to %d:%s\n", useNode->id(), useNode->op()->mnemonic(), edge.to()->id(),
                            edge.to()->op()->mnemonic());

                        if (!input_128) {
                            TRACE("Create ExtractF128(%lu) node from #%d\n", i, new_node->id());
                            input_128 = graph()->NewNode(mcgraph()->machine()->ExtractF128(static_cast<int32_t>(i)), new_node);
                        }
        ...
        if (op == IrOpcode::kI8x16Shuffle && IsSplat(nodes)) {
            ...
            TRACE("Replace Effect Edge from %d:%s, to %d:%s\n", source->id(), source->op()->mnemonic(), new_node->id(), new_node->op()->mnemonic());
...
{
    for (auto it = graph_->GetSimdStoreNodes().begin(); it != graph_->GetSimdStoreNodes().end(); ++it) {
        Node* node = *it;
        ...
        if (second_level_iter == store_nodes->end()) {
            second_level_iter = store_nodes->insert({ address, StoreNodeSet(zone()) }).first;
        }
        second_level_iter->second.insert(node);
    }
}

bool Revectorizer::ReduceStoreChains(ZoneMap<Node*, StoreNodeSet>* store_chains)
{
    TRACE("Enter %s\n", __func__);
    bool changed = false;
    for (auto chain_iter = store_chains->cbegin(); chain_iter != store_chains->cend(); ++chain_iter) {
        if (chain_iter->second.size() >= 2 && chain_iter->second.size() % 2 == 0) {
            ZoneVector<Node*> store_chain(chain_iter->second.begin(), chain_iter->second.end(), zone_);
            for (auto it = store_chain.begin(); it < store_chain.end(); it = it + 2) {
                ZoneVector<Node*> stores_unit(it, it + 2, zone_);
                if ((NodeProperties::GetEffectInput(stores_unit[0]) == stores_unit[1] || NodeProperties::GetEffectInput(stores_unit[1]) == stores_unit[0])
...
{
    TRACE("Enter %s, root@ (#%d,#%d)\n", __func__, Stores[0]->id(), Stores[1]->id());
