...
SimplifiedOperatorReducer::SimplifiedOperatorReducer(Editor* editor, JSGraph* jsgraph, JSHeapBroker* broker, BranchSemantics branch_semantics)
...
Reduction SimplifiedOperatorReducer::Reduce(Node* node)
{
    ...
    case IrOpcode::kChangeTaggedToBit: {
        HeapObjectMatcher m(node->InputAt(0));
        if (m.HasResolvedValue()) {
            std::optional<bool> maybe_result = m.Ref(broker()).TryGetBooleanValue(broker());
    ...
    case IrOpcode::kChangeFloat64ToTagged: {
        ...
        if (m.HasResolvedValue())
            return ReplaceNumber(m.ResolvedValue());
    ...
    case IrOpcode::kCheckedFloat64ToInt32: {
        ...
        if (m.HasResolvedValue() && IsInt32Double(m.ResolvedValue())) {
            Node* value = jsgraph()->Int32Constant(static_cast<int32_t>(m.ResolvedValue()));
    ...
    case IrOpcode::kCheckedInt32Add: {
        ...
        if (m.right().HasResolvedValue()) {
            ...
            if (checked_int32_add->opcode() == IrOpcode::kCheckedInt32Add) {
                ...
                if (n.right().HasResolvedValue() && (n.right().ResolvedValue() >= 0) == (m.right().ResolvedValue() >= 0)) {
                    int32_t val;
                    bool overflow = base::bits::SignedAddOverflow32(n.right().ResolvedValue(), m.right().ResolvedValue(), &val);
                    if (!overflow) {
...
Reduction SimplifiedOperatorReducer::Change(Node* node, const Operator* op, Node* a)
{
