    ...
    , cache_(TypeCache::Get())
{
    ...
    singleton_empty_string_ = Type::Constant(broker, broker->empty_string(), zone);
    ...
    falsish_ = Type::Union(Type::Undetectable(), Type::Union(Type::Union(singleton_false_, cache_->kZeroish, zone), singleton_empty_string_, zone), zone);
    truish_ = Type::Union(singleton_true_, Type::Union(Type::DetectableReceiver(), Type::Symbol(), zone), zone);
}
...
Type OperationTyper::AddRanger(double lhs_min, double lhs_max, double rhs_min, double rhs_max)
{
    double results[4];
...
Type OperationTyper::SubtractRanger(double lhs_min, double lhs_max, double rhs_min, double rhs_max)
{
    double results[4];
    results[0] = lhs_min - rhs_min;
    results[1] = lhs_min - rhs_max;
...
Type OperationTyper::MultiplyRanger(double lhs_min, double lhs_max, double rhs_min, double rhs_max)
{
    double results[4];
    results[0] = lhs_min * rhs_min;
    results[1] = lhs_min * rhs_max;
    ...
    if (((lhs_min == -V8_INFINITY || lhs_max == V8_INFINITY) && (rhs_min <= 0.0 && 0.0 <= rhs_max))
        || ((rhs_min == -V8_INFINITY || rhs_max == V8_INFINITY) && (lhs_min <= 0.0 && 0.0 <= lhs_max))) {
        type = Type::Union(type, Type::NaN(), zone());
    }
    return type;
}
...
Type OperationTyper::ToBigIntConvertNumber(Type type)
{
    ...
    bool maybe_number = type.Maybe(Type::Number()) || type.Maybe(Type::Receiver());
...
Type OperationTyper::ToNumeric(Type type)
{
    ...
    return Type::Union(ToNumber(Type::Intersect(type, Type::NonBigInt(), zone())), Type::Intersect(type, Type::BigInt(), zone()), zone());
}
...
Type OperationTyper::NumberToInt32(Type type)
{
    ...
    if (type.Is(signed32ish_)) {
        return Type::Intersect(Type::Union(type, cache_->kSingletonZero, zone()), Type::Signed32(), zone());
    }
...
Type OperationTyper::NumberToUint32(Type type)
{
    ...
    if (type.Is(unsigned32ish_)) {
        return Type::Intersect(Type::Union(type, cache_->kSingletonZero, zone()), Type::Unsigned32(), zone());
    }
...
Type OperationTyper::CheckBigInt(Type type)
{
    return Type::BigInt();
}
...
Type OperationTyper::NumberMultiply(Type lhs, Type rhs)
{
    ...
    bool maybe_nan = lhs.Maybe(Type::NaN()) || rhs.Maybe(Type::NaN())
        || (lhs.Maybe(cache_->kZeroish) && (rhs.Min() == -V8_INFINITY || rhs.Max() == V8_INFINITY))
        || (rhs.Maybe(cache_->kZeroish) && (lhs.Min() == -V8_INFINITY || lhs.Max() == V8_INFINITY));
    ...
    // Try to rule out -0.
    bool maybe_minuszero = lhs.Maybe(Type::MinusZero()) || rhs.Maybe(Type::MinusZero()) || (lhs.Maybe(cache_->kZeroish) && rhs.Min() < 0.0)
    ...
    Type type = (lhs.Is(cache_->kInteger) && rhs.Is(cache_->kInteger)) ? MultiplyRanger(lhs.Min(), lhs.Max(), rhs.Min(), rhs.Max()) : Type::OrderedNumber();
...
Type OperationTyper::NumberDivide(Type lhs, Type rhs)
{
    ...
    bool maybe_nan = lhs.Maybe(Type::NaN()) || rhs.Maybe(cache_->kZeroish)
        || ((lhs.Min() == -V8_INFINITY || lhs.Max() == +V8_INFINITY) && (rhs.Min() == -V8_INFINITY || rhs.Max() == +V8_INFINITY));
    lhs = Type::Intersect(lhs, Type::OrderedNumber(), zone());
    DCHECK(!lhs.IsNone());
    rhs = Type::Intersect(rhs, Type::OrderedNumber(), zone());
    DCHECK(!rhs.IsNone());
    ...
    bool maybe_minuszero
        = !lhs.Is(cache_->kInteger) || (lhs.Maybe(cache_->kZeroish) && rhs.Min() < 0.0) || (rhs.Min() == -V8_INFINITY || rhs.Max() == +V8_INFINITY);
...
Type OperationTyper::NumberShiftLeft(Type lhs, Type rhs)
{
    ...
    double min = std::min(static_cast<int32_t>(static_cast<uint32_t>(min_lhs) << min_rhs), static_cast<int32_t>(static_cast<uint32_t>(min_lhs) << max_rhs));
    double max = std::max(static_cast<int32_t>(static_cast<uint32_t>(max_lhs) << min_rhs), static_cast<int32_t>(static_cast<uint32_t>(max_lhs) << max_rhs));
...
Type OperationTyper::NumberMax(Type lhs, Type rhs)
{
    ...
    if (!lhs.Is(cache_->kIntegerOrMinusZeroOrNaN) || !rhs.Is(cache_->kIntegerOrMinusZeroOrNaN)) {
...
#define MACHINE_BINOP(Name)                                                                                                                                    \
    Type OperationTyper::Name(Type, Type)                                                                                                                      \
    {                                                                                                                                                          \
        return Type::Machine();                                                                                                                                \
    }
...
OperationTyper::ComparisonOutcome OperationTyper::Invert(ComparisonOutcome outcome)
{
...
Type OperationTyper::FalsifyUndefined(ComparisonOutcome outcome)
{
    if ((outcome & kComparisonFalse) != 0 || (outcome & kComparisonUndefined) != 0) {
        return (outcome & kComparisonTrue) != 0 ? Type::Boolean() : singleton_false();
    }
