...
public:
    WasmIntoJSInlinerImpl(Zone* zone, const wasm::WasmModule* module, MachineGraph* mcgraph, const wasm::FunctionBody& body, base::Vector<const uint8_t> bytes,
        SourcePositionTable* source_position_table, int inlining_id)
        ...
        , inlining_id_(inlining_id)
    {
        ...
        Node* start = graph_->NewNode(mcgraph->common()->Start(static_cast<int>(params)));
    ...
    Node* Param(int index, const char* debug_name = nullptr)
    {
        ...
        if (parameters_[array_index] == nullptr) {
            Node* param = graph_->NewNode(mcgraph_->common()->Parameter(index, debug_name), graph_->start());
    ...
    bool TryInlining()
    {
        ...
        while (is_inlineable_) {
            ...
            case wasm::kExprRefCastNull:
                DCHECK(!stack.empty());
                stack.back() = ParseRefCast(stack.back(), opcode == wasm::kExprRefCastNull);
                continue;
            ...
            case wasm::kExprEnd: {
                ...
                Node* ret = graph_->NewNode(mcgraph_->common()->Return(return_count), return_count + 3, buf.data());
...
private:
    ...
    {
        DCHECK(input.type.is_reference_to(wasm::HeapType::kExtern) || input.type.is_reference_to(wasm::HeapType::kNoExtern));
        wasm::ValueType result_type
            = wasm::ValueType::RefMaybeNull(wasm::HeapType::kAny, input.type.is_nullable() ? wasm::Nullability::kNullable : wasm::Nullability::kNonNullable);
    ...
    Value ParseExternConvertAny(Value input)
    {
        ...
        wasm::ValueType result_type
            = wasm::ValueType::RefMaybeNull(wasm::HeapType::kExtern, input.type.is_nullable() ? wasm::Nullability::kNullable : wasm::Nullability::kNonNullable);
    ...
    Value ParseStructGet(Value struct_val, WasmOpcode opcode)
    {
        ...
        const CheckForNull null_check = struct_val.type.is_nullable() ? kWithNullCheck : kWithoutNullCheck;
        Node* member = gasm_.StructGet(struct_val.node, struct_type, field_index, is_signed, null_check);
    ...
    void ParseStructSet(Value wasm_struct, Value value)
    {
        ...
        uint32_t field_index = consume_u32v();
        DCHECK_GT(struct_type->field_count(), field_index);
        const CheckForNull null_check = wasm_struct.type.is_nullable() ? kWithNullCheck : kWithoutNullCheck;
        gasm_.StructSet(wasm_struct.node, value.node, struct_type, field_index, null_check);
    ...
    Value ParseRefCast(Value input, bool null_succeeds)
    {
        ...
        if (heap_index < 0) {
            ...
            gasm_.TrapUnless(gasm_.HasInstanceType(input.node, WASM_ARRAY_TYPE), TrapId::kTrapIllegalCast);
            SetSourcePosition(gasm_.effect());
            gasm_.Goto(&done);
            gasm_.Bind(&done);
            // Add TypeGuard for graph typing.
            Graph* graph = mcgraph_->graph();
            wasm::ValueType result_type = wasm::ValueType::RefMaybeNull(wasm::HeapType::kArray, null_succeeds ? wasm::kNullable : wasm::kNonNullable);
            Node* type_guard
                = graph->NewNode(mcgraph_->common()->TypeGuard(Type::Wasm(result_type, module_, graph->zone())), input.node, gasm_.effect(), gasm_.control());
            gasm_.InitializeEffectControl(type_guard, gasm_.control());
            return TypeNode(type_guard, result_type);
        }
        if (module_->has_signature(wasm::ModuleTypeIndex { static_cast<uint32_t>(heap_index) })) {
            is_inlineable_ = false;
            return {};
        }
        wasm::ValueType target_type
            = wasm::ValueType::RefMaybeNull(wasm::ModuleTypeIndex { static_cast<uint32_t>(heap_index) }, null_succeeds ? wasm::kNullable : wasm::kNonNullable);
        Node* rtt = mcgraph_->graph()->NewNode(gasm_.simplified()->RttCanon(target_type.ref_index()), trusted_data_node_);
    ...
    {
        DCHECK(wasm::IsHeapSubtypeOf(input.type.heap_type(), wasm::HeapType(wasm::HeapType::kArray), module_));
        const CheckForNull null_check = input.type.is_nullable() ? kWithNullCheck : kWithoutNullCheck;
    ...
    Value ParseArrayGet(Value array, Value index, WasmOpcode opcode)
    {
        ...
        const CheckForNull null_check = array.type.is_nullable() ? kWithNullCheck : kWithoutNullCheck;
        // Perform bounds check.
        Node* length = gasm_.ArrayLength(array.node, null_check);
        SetSourcePosition(length);
        gasm_.TrapUnless(gasm_.Uint32LessThan(index.node, length), TrapId::kTrapArrayOutOfBounds);
        SetSourcePosition(gasm_.effect());
        // Perform array.get.
        Node* element = gasm_.ArrayGet(array.node, index.node, array_type, is_signed);
    ...
    WasmOpcode ReadOpcode()
    {
        ...
        auto [opcode_with_prefix, length] = read_prefixed_opcode<ValidationTag>(pc_);
        pc_ += length;
    ...
    {
        compiler::NodeProperties::SetType(node, compiler::Type::Wasm(type, module_, graph_->zone()));
        return { node, type };
    }

    void SetSourcePosition(Node* node)
    {
        if (!source_position_table_->IsEnabled())
            return;
        int offset = static_cast<int>(instruction_start_ - start());
        source_position_table_->SetSourcePosition(node, SourcePosition(offset, inlining_id_));
    }
...
bool WasmIntoJSInliner::TryInlining(Zone* zone, const wasm::WasmModule* module, MachineGraph* mcgraph, const wasm::FunctionBody& body,
    base::Vector<const uint8_t> bytes, SourcePositionTable* source_position_table, int inlining_id)
{
    WasmIntoJSInlinerImpl inliner(zone, module, mcgraph, body, bytes, source_position_table, inlining_id);
