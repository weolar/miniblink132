...
public:
    ...
    template <typename FunctionType> void ForEach(FunctionType callback);
...
class V8_EXPORT_PRIVATE Revectorizer final : public NON_EXPORTED_BASE(ZoneObject) {
public:
    Revectorizer(Zone* zone, Graph* graph, MachineGraph* mcgraph, SourcePositionTable* source_positions);
