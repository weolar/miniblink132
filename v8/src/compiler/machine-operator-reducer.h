...
// machine operators.
class V8_EXPORT_PRIVATE MachineOperatorReducer final : public NON_EXPORTED_BASE(AdvancedReducer) {
public:
    enum SignallingNanPropagation { kSilenceSignallingNan, kPropagateSignallingNan };

    explicit MachineOperatorReducer(Editor* editor, MachineGraph* mcgraph, SignallingNanPropagation signalling_nan_propagation);
...
private:
    ...
    template <typename WordNAdapter> Reduction ReduceWordNAnd(Node* node);
    template <typename WordNAdapter> Reduction ReduceWordNOr(Node* node);
    template <typename WordNAdapter> Reduction ReduceWordNXor(Node* node);
    template <typename WordNAdapter> Reduction ReduceUintNLessThanOrEqual(Node* node);
    ...
    template <typename WordNAdapter> std::optional<Node*> ReduceConditionalN(Node* node);
