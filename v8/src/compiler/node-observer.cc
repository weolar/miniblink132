...
void ObserveNodeManager::StartObserving(Node* node, NodeObserver* observer)
{
    ...
    if (observation == NodeObserver::Observation::kContinue) {
        observations_[node->id()] = zone_->New<NodeObservation>(observer, node, zone_);
    } else {
        DCHECK_EQ(observation, NodeObserver::Observation::kStop);
    }
}

void ObserveNodeManager::OnNodeChanged(const char* reducer_name, const Node* old_node, const Node* new_node)
{
    ...
    NodeObserver::Observation result = observation->observer->OnNodeChanged(reducer_name, new_node, old_state);
