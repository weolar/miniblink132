...
public:
    ...
    JSInliningHeuristic(Editor* editor, Zone* local_zone, OptimizedCompilationInfo* info, JSGraph* jsgraph, JSHeapBroker* broker,
        SourcePositionTable* source_positions, NodeOriginTable* node_origins, Mode mode,
        // The two following arguments should be `nullptr` iff
        // inlining with `mode == kJSOnly`.
        const wasm::WasmModule* wasm_module, JsWasmCallsSidetable* js_wasm_calls_sidetable)
        : AdvancedReducer(editor)
        , inliner_(editor, local_zone, info, jsgraph, broker, source_positions, node_origins, wasm_module, js_wasm_calls_sidetable, mode == kWasmFullInlining)
        ...
        , info_(info)
        , mode_(mode)
        , max_inlined_bytecode_size_cumulative_(v8_flags.max_inlined_bytecode_size_cumulative)
        , max_inlined_bytecode_size_absolute_(v8_flags.max_inlined_bytecode_size_absolute)
    {
        DCHECK_EQ(mode == kWasmWrappersOnly || mode == kWasmFullInlining, wasm_module != nullptr && js_wasm_calls_sidetable != nullptr);
    }
