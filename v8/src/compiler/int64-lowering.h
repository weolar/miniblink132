...
class Int64Lowering {
public:
    Int64Lowering(Graph* graph, MachineOperatorBuilder* machine, CommonOperatorBuilder* common, SimplifiedOperatorBuilder* simplified_, Zone* zone,
...
private:
    static int GetParameterCountAfterLowering(Signature<MachineRepresentation>* signature);
    ...
    Signature<MachineRepresentation>* signature() const
    {
        return signature_;
    }
    ...
    void LowerComparison(Node* node, const Operator* signed_op, const Operator* unsigned_op);
    void LowerWord64AtomicBinop(Node* node, const Operator* op);
    void LowerWord64AtomicNarrowOp(Node* node, const Operator* op);
    void LowerLoadOperator(Node* node, MachineRepresentation rep, const Operator* load_op);
    void LowerStoreOperator(Node* node, MachineRepresentation rep, const Operator* store_op);
