...
public:
    OneOrTwoByteAnalysis(Graph* graph, Zone* zone, JSHeapBroker* broker)
        : states_(graph->NodeCount(), State::kUnknown, zone)
        , broker_(broker)
    {
    }
...
private:
    ...
    void SetStatus(Node* node, State state, int id = kInvalidId)
    {
        ...
        if (node->id() >= status_.size()) {
            ...
            status_.resize(node->id() * growth_factor, Status { kInvalidId, State::kUnvisited });
        }
    ...
    {
        return node->opcode() == IrOpcode::kPhi && schedule()->block(node)->IsLoopHeader();
    }
