...
{
    return OperatorProperties::HasFrameStateInput(node->op()) ? CallDescriptor::kNeedsFrameState : CallDescriptor::kNoFlags;
}

} // namespace

JSGenericLowering::JSGenericLowering(JSGraph* jsgraph, Editor* editor, JSHeapBroker* broker)
...
void JSGenericLowering::ReplaceWithBuiltinCall(Node* node, Callable callable, CallDescriptor::Flags flags)
{
    ReplaceWithBuiltinCall(node, callable, flags, node->op()->properties());
}

void JSGenericLowering::ReplaceWithBuiltinCall(Node* node, Callable callable, CallDescriptor::Flags flags, Operator::Properties properties)
{
    const CallInterfaceDescriptor& descriptor = callable.descriptor();
    auto call_descriptor = Linkage::GetStubCallDescriptor(zone(), descriptor, descriptor.GetStackParameterCount(), flags, properties);
    Node* stub_code = jsgraph()->HeapConstantNoHole(callable.code());
    node->InsertInput(zone(), 0, stub_code);
    NodeProperties::ChangeOp(node, common()->Call(call_descriptor));
}

void JSGenericLowering::ReplaceWithRuntimeCall(Node* node, Runtime::FunctionId f, int nargs_override)
{
    CallDescriptor::Flags flags = FrameStateFlagForCall(node);
...
void JSGenericLowering::ReplaceUnaryOpWithBuiltinCall(Node* node, Builtin builtin_without_feedback, Builtin builtin_with_feedback)
{
    ...
    if (CollectFeedbackInGenericLowering() && p.feedback().IsValid()) {
        ...
        const CallInterfaceDescriptor& descriptor = callable.descriptor();
        CallDescriptor::Flags flags = FrameStateFlagForCall(node);
        auto call_descriptor = Linkage::GetStubCallDescriptor(zone(), descriptor, descriptor.GetStackParameterCount(), flags, node->op()->properties());
        Node* stub_code = jsgraph()->HeapConstantNoHole(callable.code());
...
#define DEF_UNARY_LOWERING(Name)                                                                                                                               \
    void JSGenericLowering::LowerJS##Name(Node* node)                                                                                                          \
    {                                                                                                                                                          \
        ReplaceUnaryOpWithBuiltinCall(node, Builtin::k##Name, Builtin::k##Name##_WithFeedback);                                                                \
    }
...
void JSGenericLowering::ReplaceBinaryOpWithBuiltinCall(Node* node, Builtin builtin_without_feedback, Builtin builtin_with_feedback)
{
...
#define DEF_BINARY_LOWERING(Name)                                                                                                                              \
    void JSGenericLowering::LowerJS##Name(Node* node)                                                                                                          \
    {                                                                                                                                                          \
        ReplaceBinaryOpWithBuiltinCall(node, Builtin::k##Name, Builtin::k##Name##_WithFeedback);                                                               \
    }
// Binary ops.
...
void JSGenericLowering::LowerJSStrictEqual(Node* node)
{
    ...
    Callable callable = Builtins::CallableFor(isolate(), builtin);
    ReplaceWithBuiltinCall(node, callable, CallDescriptor::kNoFlags, Operator::kEliminatable);
}
...
bool ShouldUseMegamorphicAccessBuiltin(FeedbackSource const& source, OptionalNameRef name, AccessMode mode, JSHeapBroker* broker)
{
    ProcessedFeedback const& feedback = broker->GetFeedbackForPropertyAccess(source, mode, name);
...
void JSGenericLowering::LowerJSHasProperty(Node* node)
{
    ...
    if (!p.feedback().IsValid()) {
        ...
        n->InsertInput(zone(), 2, jsgraph()->TaggedIndexConstant(p.feedback().index()));
...
void JSGenericLowering::LowerJSLoadProperty(Node* node)
{
    ...
    if (outer_state->opcode() != IrOpcode::kFrameState) {
        ...
        n->InsertInput(zone(), 2, jsgraph()->TaggedIndexConstant(p.feedback().index()));
        ReplaceWithBuiltinCall(node,
            ShouldUseMegamorphicAccessBuiltin(p.feedback(), {}, AccessMode::kLoad, broker()) ? Builtin::kKeyedLoadICTrampoline_Megamorphic
                                                                                             : Builtin::kKeyedLoadICTrampoline);
    } else {
        n->InsertInput(zone(), 2, jsgraph()->TaggedIndexConstant(p.feedback().index()));
        ReplaceWithBuiltinCall(
            node, ShouldUseMegamorphicAccessBuiltin(p.feedback(), {}, AccessMode::kLoad, broker()) ? Builtin::kKeyedLoadIC_Megamorphic : Builtin::kKeyedLoadIC);
    }
}
...
void JSGenericLowering::LowerJSLoadNamed(Node* node)
{
    ...
    if (!p.feedback().IsValid()) {
        ...
        n->RemoveInput(n.FeedbackVectorIndex());
        node->InsertInput(zone(), 1, jsgraph()->ConstantNoHole(p.name(), broker()));
        node->InsertInput(zone(), 2, jsgraph()->TaggedIndexConstant(p.feedback().index()));
        ReplaceWithBuiltinCall(node,
            ShouldUseMegamorphicAccessBuiltin(p.feedback(), p.name(), AccessMode::kLoad, broker()) ? Builtin::kLoadICTrampoline_Megamorphic
                                                                                                   : Builtin::kLoadICTrampoline);
    } else {
        node->InsertInput(zone(), 1, jsgraph()->ConstantNoHole(p.name(), broker()));
        node->InsertInput(zone(), 2, jsgraph()->TaggedIndexConstant(p.feedback().index()));
        ReplaceWithBuiltinCall(
            node, ShouldUseMegamorphicAccessBuiltin(p.feedback(), p.name(), AccessMode::kLoad, broker()) ? Builtin::kLoadIC_Megamorphic : Builtin::kLoadIC);
    }
}
...
void JSGenericLowering::LowerJSLoadNamedFromSuper(Node* node)
{
    ...
    // FeedbackVector.
    Node* home_object_map = effect = graph()->NewNode(jsgraph()->simplified()->LoadField(AccessBuilder::ForMap()), n.home_object(), effect, control);
    Node* home_object_proto = effect = graph()->NewNode(jsgraph()->simplified()->LoadField(AccessBuilder::ForMapPrototype()), home_object_map, effect, control);
    ...
    node->InsertInput(zone(), 3, jsgraph()->TaggedIndexConstant(p.feedback().index()));
...
void JSGenericLowering::LowerJSLoadGlobal(Node* node)
{
    ...
    if (outer_state->opcode() != IrOpcode::kFrameState) {
        ...
        node->InsertInput(zone(), 1, jsgraph()->TaggedIndexConstant(p.feedback().index()));
...
void JSGenericLowering::LowerJSGetIterator(Node* node)
{
    ...
    Node* load_slot = jsgraph()->TaggedIndexConstant(p.loadFeedback().slot.ToInt());
...
void JSGenericLowering::LowerJSSetKeyedProperty(Node* node)
{
    ...
    if (outer_state->opcode() != IrOpcode::kFrameState) {
        ...
        // can be called here.
        ReplaceWithBuiltinCall(node,
            ShouldUseMegamorphicAccessBuiltin(p.feedback(), {}, AccessMode::kStore, broker()) ? Builtin::kKeyedStoreICTrampoline_Megamorphic
                                                                                              : Builtin::kKeyedStoreICTrampoline);
    } else {
        node->InsertInput(zone(), 3, jsgraph()->TaggedIndexConstant(p.feedback().index()));
        ReplaceWithBuiltinCall(node,
            ShouldUseMegamorphicAccessBuiltin(p.feedback(), {}, AccessMode::kStore, broker()) ? Builtin::kKeyedStoreIC_Megamorphic : Builtin::kKeyedStoreIC);
    }
}
...
void JSGenericLowering::LowerJSDefineKeyedOwnProperty(Node* node)
{
    ...
    if (outer_state->opcode() != IrOpcode::kFrameState) {
        n->RemoveInput(n.FeedbackVectorIndex());
        node->InsertInput(zone(), 4, jsgraph()->TaggedIndexConstant(p.feedback().index()));
...
void JSGenericLowering::LowerJSSetNamedProperty(Node* node)
{
    ...
    if (!p.feedback().IsValid()) {
        ...
        // be called here.
        ReplaceWithBuiltinCall(node,
            ShouldUseMegamorphicAccessBuiltin(p.feedback(), {}, AccessMode::kStore, broker()) ? Builtin::kStoreICTrampoline_Megamorphic
                                                                                              : Builtin::kStoreICTrampoline);
    } else {
        node->InsertInput(zone(), 1, jsgraph()->ConstantNoHole(p.name(), broker()));
        node->InsertInput(zone(), 3, jsgraph()->TaggedIndexConstant(p.feedback().index()));
        ReplaceWithBuiltinCall(
            node, ShouldUseMegamorphicAccessBuiltin(p.feedback(), {}, AccessMode::kStore, broker()) ? Builtin::kStoreIC_Megamorphic : Builtin::kStoreIC);
    }
}
...
void JSGenericLowering::LowerJSDefineKeyedOwnPropertyInLiteral(Node* node)
{
    ...
    static_assert(n.FeedbackVectorIndex() == 4);
    RelaxControls(node);
    node->InsertInput(zone(), 5, jsgraph()->TaggedIndexConstant(p.feedback().index()));
...
void JSGenericLowering::LowerJSGetSuperConstructor(Node* node)
{
    ...
    Node* function_map = effect = graph()->NewNode(jsgraph()->simplified()->LoadField(AccessBuilder::ForMap()), active_function, effect, control);
    ...
    NodeProperties::ChangeOp(node, jsgraph()->simplified()->LoadField(AccessBuilder::ForMapPrototype()));
}

void JSGenericLowering::LowerJSFindNonDefaultConstructorOrConstruct(Node* node)
{
...
void JSGenericLowering::LowerJSCreateArray(Node* node)
{
    ...
    auto call_descriptor = Linkage::GetStubCallDescriptor(zone(), interface_descriptor, arity + 1, CallDescriptor::kNeedsFrameState, node->op()->properties());
    ...
    Node* type_info = site.has_value() ? jsgraph()->ConstantNoHole(site.value(), broker()) : jsgraph()->UndefinedConstant();
...
void JSGenericLowering::LowerJSCreateClosure(Node* node)
{
    ...
    node->InsertInput(zone(), 0, jsgraph()->ConstantNoHole(shared_info, broker()));
...
void JSGenericLowering::LowerJSCreateLiteralArray(Node* node)
{
    ...
    static_assert(n.FeedbackVectorIndex() == 0);
    node->InsertInput(zone(), 1, jsgraph()->TaggedIndexConstant(p.feedback().index()));
    node->InsertInput(zone(), 2, jsgraph()->ConstantNoHole(p.constant(), broker()));
...
void JSGenericLowering::LowerJSGetTemplateObject(Node* node)
{
    ...
    node->InsertInput(zone(), 1, jsgraph()->ConstantNoHole(description, broker()));
    node->InsertInput(zone(), 2, jsgraph()->UintPtrConstant(p.feedback().index()));
...
void JSGenericLowering::LowerJSConstructForwardVarargs(Node* node)
{
    ...
    auto call_descriptor = Linkage::GetStubCallDescriptor(zone(), callable.descriptor(), arg_count + 1, flags);
    Node* stub_code = jsgraph()->HeapConstantNoHole(callable.code());
...
void JSGenericLowering::LowerJSConstructForwardAllArgs(Node* node)
{
    ...
    // JSCallReducer.
    DCHECK(FrameState { NodeProperties::GetFrameStateInput(node) }.outer_frame_state()->opcode() != IrOpcode::kFrameState);

    JSConstructForwardAllArgsNode n(node);

    // Call a builtin for forwarding the arguments of non-inlined (i.e. outermost)
    // frames.
    Callable callable = Builtins::CallableFor(isolate(), Builtin::kConstructForwardAllArgs);
    DCHECK_EQ(callable.descriptor().GetStackParameterCount(), 0);
    auto call_descriptor = Linkage::GetStubCallDescriptor(zone(), callable.descriptor(), 0, CallDescriptor::kNeedsFrameState);
...
void JSGenericLowering::LowerJSConstruct(Node* node)
{
    ...
    auto call_descriptor = Linkage::GetStubCallDescriptor(zone(), callable.descriptor(), stack_argument_count, flags);
    Node* stub_code = jsgraph()->HeapConstantNoHole(callable.code());
    Node* stub_arity = jsgraph()->Int32Constant(JSParameterCount(arg_count));
    Node* receiver = jsgraph()->UndefinedConstant();
    node->RemoveInput(n.FeedbackVectorIndex());
...
void JSGenericLowering::LowerJSConstructWithArrayLike(Node* node)
{
    ...
    Callable callable = Builtins::CallableFor(isolate(), Builtin::kConstructWithArrayLike);
    // If this fails, we might need to update the parameter reordering code
    // to ensure that the additional arguments passed via stack are pushed
    // between top of stack and JS arguments.
    DCHECK_EQ(callable.descriptor().GetStackParameterCount(), 0);
...
void JSGenericLowering::LowerJSConstructWithSpread(Node* node)
{
    ...
    Node* stub_arity = jsgraph()->Int32Constant(JSParameterCount(arg_count - kTheSpread));
    Node* receiver = jsgraph()->UndefinedConstant();
...
void JSGenericLowering::LowerJSStackCheck(Node* node)
{
    ...
    if (stack_check_kind == StackCheckKind::kJSIterationBody) {
        check = effect = graph()->NewNode(machine()->Load(MachineType::Uint8()),
            jsgraph()->ExternalConstant(ExternalReference::address_of_no_heap_write_interrupt_request(isolate())), jsgraph()->IntPtrConstant(0), effect,
            control);
        check = graph()->NewNode(machine()->Word32Equal(), check, jsgraph()->Int32Constant(0));
    } else {
        Node* limit = effect = graph()->NewNode(machine()->Load(MachineType::Pointer()),
            jsgraph()->ExternalConstant(ExternalReference::address_of_jslimit(isolate())), jsgraph()->IntPtrConstant(0), effect, control);

        check = effect = graph()->NewNode(machine()->StackPointerGreaterThan(stack_check_kind), limit, effect);
    }
    ...
    if (stack_check_kind == StackCheckKind::kJSFunctionEntry) {
        node->InsertInput(zone(), 0, graph()->NewNode(machine()->LoadStackCheckOffset()));
    }
...
Zone* JSGenericLowering::zone() const
{
    return graph()->zone();
}

Isolate* JSGenericLowering::isolate() const
{
    return jsgraph()->isolate();
}

Graph* JSGenericLowering::graph() const
{
