...
public:
    LoopFinderImpl(Graph* graph, LoopTree* loop_tree, TickCounter* tick_counter, Zone* zone)
        : zone_(zone)
        , end_(graph->end())
        , queue_(zone)
        , queued_(graph, 2)
        ...
        while (!queue_.empty()) {
            ...
            for (int i = 0; i < node->InputCount(); i++) {
                ...
                if (IsBackedge(node, i)) {
                    ...
                    if (SetBackwardMark(input, loop_num) || !info(input).backwards_visited) {
                        Queue(input);
                    }
                } else {
                    ...
                    if (PropagateBackwardMarks(node, input, loop_num) || !info(input).backwards_visited) {
                        Queue(input);
                    }
                }
            }
...
LoopTree* LoopFinder::BuildLoopTree(Graph* graph, TickCounter* tick_counter, Zone* zone)
{
    LoopTree* loop_tree = graph->zone()->New<LoopTree>(graph->NodeCount(), graph->zone());
...
// static
ZoneUnorderedSet<Node*>* LoopFinder::FindSmallInnermostLoopFromHeader(Node* loop_header, AllNodes& all_nodes, Zone* zone, size_t max_size, Purpose purpose)
{
...
#define ENQUEUE_USES(use_name, condition)                                                                                                                      \
    ...
    while (!queue.empty()) {
        ...
        case IrOpcode::kLoopExit:
            // Found nested loop.
            if (node->InputAt(1) != loop_header)
                return nullptr;
            // LoopExitValue/Effect uses are inside the loop. The rest are not.
            ENQUEUE_USES(use, (use->opcode() == IrOpcode::kLoopExitEffect || use->opcode() == IrOpcode::kLoopExitValue))
            break;
        ...
        case IrOpcode::kCall: {
            ...
            if (callee->opcode() != IrOpcode::kRelocatableInt32Constant && callee->opcode() != IrOpcode::kRelocatableInt64Constant) {
                return nullptr;
            }
            Builtin builtin = static_cast<Builtin>(OpParameter<RelocatablePtrConstantInfo>(callee->op()).value());
            constexpr Builtin unrollable_builtins[] = { // Exists in every stack check.
                Builtin::kWasmStackGuard,
                // Fast table operations.
                Builtin::kWasmTableGet, Builtin::kWasmTableSet, Builtin::kWasmTableGetFuncRef, Builtin::kWasmTableSetFuncRef, Builtin::kWasmTableGrow,
                // Atomics.
                Builtin::kWasmI32AtomicWait, Builtin::kWasmI64AtomicWait,
                // Exceptions.
                Builtin::kWasmAllocateFixedArray, Builtin::kWasmThrow, Builtin::kWasmRethrow, Builtin::kWasmRethrowExplicitContext,
            ...
            if (std::count(std::begin(unrollable_builtins), std::end(unrollable_builtins), builtin) == 0) {
                return nullptr;
            }
            ENQUEUE_USES(use, true)
            break;
        ...
        case IrOpcode::kWasmStructGet: {
            ...
            if (object->opcode() == IrOpcode::kWasmStructGet && visited->find(object) != visited->end()) {
    ...
    for (Node* node : *visited) {
        ...
        if (!all_nodes.IsLive(node))
            continue;

        for (Edge edge : node->input_edges()) {
            Node* input = edge.to();
            if (NodeProperties::IsControlEdge(edge) && visited->count(input) == 0 && input->opcode() != IrOpcode::kStart) {
                FATAL("Floating control detected in wasm turbofan graph: Node #%d:%s is "
                      "inside loop headed by #%d, but its control dependency #%d:%s is "
                      "outside",
                    node->id(), node->op()->mnemonic(), loop_header->id(), input->id(), input->op()->mnemonic());
            }
        }
    }
...
bool LoopFinder::HasMarkedExits(LoopTree* loop_tree, const LoopTree::Loop* loop)
{
    ...
    for (Node* node : loop_tree->LoopNodes(loop)) {
        for (Node* use : node->uses()) {
            if (!loop_tree->Contains(loop, use)) {
                ...
                if (unmarked_exit) {
                    if (v8_flags.trace_turbo_loop) {
                        PrintF("Cannot peel loop %i. Loop exit without explicit mark: Node %i "
                               "(%s) is inside loop, but its use %i (%s) is outside.\n",
                            loop_node->id(), node->id(), node->op()->mnemonic(), use->id(), use->op()->mnemonic());
                    }
                    return false;
                }
            }
