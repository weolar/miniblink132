...
public:
    ...
    {
        return JSConstructNode { call_ }.new_target();
    }
    ...
    {
        return (call_->opcode() == IrOpcode::kJSCall) ? JSCallNode { call_ }.ArgumentCount() : JSConstructNode { call_ }.ArgumentCount();
    }

    CallFrequency const& frequency() const
    {
        return (call_->opcode() == IrOpcode::kJSCall) ? JSCallNode { call_ }.Parameters().frequency() : JSConstructNode { call_ }.Parameters().frequency();
    }
...
#if V8_ENABLE_WEBASSEMBLY
Reduction JSInliner::InlineJSWasmCall(
    Node* call, Node* new_target, Node* context, Node* frame_state, StartNode start, Node* end, Node* exception_target, const NodeVector& uncaught_subcalls)
{
    JSWasmCallNode n(call);
    return InlineCall(call, new_target, context, frame_state, start, end, exception_target, uncaught_subcalls,
        static_cast<int>(n.Parameters().signature()->parameter_count()));
}
#endif // V8_ENABLE_WEBASSEMBLY

Reduction JSInliner::InlineCall(Node* call, Node* new_target, Node* context, Node* frame_state, StartNode start, Node* end, Node* exception_target,
    const NodeVector& uncaught_subcalls, int argument_count)
{
    DCHECK_IMPLIES(IrOpcode::IsInlineeOpcode(call->opcode()), argument_count == JSCallAccessor(call).argument_count());
    ...
    const int inliner_inputs = argument_count + JSCallOrConstructNode::kExtraInputCount - JSCallOrConstructNode::kFeedbackVectorInputCount;
    ...
    if (exception_target != nullptr) {
        ...
        if (subcall_count > 0) {
            TRACE("Inlinee contains " << subcall_count << " calls without local exception handler; "
        ...
        if (subcall_count > 0) {
            Node* control_output = graph()->NewNode(common()->Merge(subcall_count), subcall_count, &on_exception_nodes.front());
            NodeVector values_effects(local_zone_);
            values_effects = on_exception_nodes;
            values_effects.push_back(control_output);
            Node* value_output = graph()->NewNode(common()->Phi(MachineRepresentation::kTagged, subcall_count), subcall_count + 1, &values_effects.front());
            Node* effect_output = graph()->NewNode(common()->EffectPhi(subcall_count), subcall_count + 1, &values_effects.front());
            ReplaceWithValue(exception_target, value_output, effect_output, control_output);
        } else {
            ReplaceWithValue(exception_target, exception_target, exception_target, jsgraph()->Dead());
        }
    }
    ...
    if (!values.empty()) {
        ...
        Node* control_output = graph()->NewNode(common()->Merge(input_count), input_count, &controls.front());
        values.push_back(control_output);
        effects.push_back(control_output);
        Node* value_output = graph()->NewNode(common()->Phi(MachineRepresentation::kTagged, input_count), static_cast<int>(values.size()), &values.front());
        Node* effect_output = graph()->NewNode(common()->EffectPhi(input_count), static_cast<int>(effects.size()), &effects.front());
        ReplaceWithValue(call, value_output, effect_output, control_output);
        return Changed(value_output);
    } else {
        ReplaceWithValue(call, jsgraph()->Dead(), jsgraph()->Dead(), jsgraph()->Dead());
        return Changed(call);
    }
}

FrameState JSInliner::CreateArtificialFrameState(
    Node* node, FrameState outer_frame_state, int parameter_count, FrameStateType frame_state_type, SharedFunctionInfoRef shared, Node* context, Node* callee)
{
    const int parameter_count_with_receiver = parameter_count + JSCallOrConstructNode::kReceiverOrNewTargetInputCount;
    CHECK_LE(parameter_count_with_receiver, kMaxUInt16);
    const FrameStateFunctionInfo* state_info
        = common()->CreateFrameStateFunctionInfo(frame_state_type, parameter_count_with_receiver, 0, 0, shared.object(), {});
...
#if V8_ENABLE_WEBASSEMBLY
    const bool skip_params = frame_state_type == FrameStateType::kWasmInlinedIntoJS;
#else
    ...
    if (skip_params) {
        ...
        // performance?
        const Operator* op_param = common()->StateValues(1, SparseInputMask::Dense());
        ...
        params_node = graph()->NewNode(op_param, static_cast<int>(params.size()), &params.front());
    }
    if (context == nullptr)
    ...
    return FrameState { graph()->NewNode(op, params_node, node0, node0, context, callee, outer_frame_state) };
}
...
bool NeedsImplicitReceiver(SharedFunctionInfoRef shared_info)
{
    ...
    return !shared_info.construct_as_builtin() && !IsDerivedConstructor(shared_info.kind());
}
...
OptionalSharedFunctionInfoRef JSInliner::DetermineCallTarget(Node* node)
{
    ...
    if (match.HasResolvedValue() && match.Ref(broker()).IsJSFunction()) {
        ...
        if (!function.native_context(broker()).equals(broker()->target_native_context())) {
            return std::nullopt;
        }
...
FeedbackCellRef JSInliner::DetermineCallContext(Node* node, Node** context_out)
{
    DCHECK(IrOpcode::IsInlineeOpcode(node->opcode()));
    Node* target = node->InputAt(JSCallOrConstructNode::TargetIndex());
    HeapObjectMatcher match(target);
    ...
    if (match.HasResolvedValue() && match.Ref(broker()).IsJSFunction()) {
        ...
        *context_out = jsgraph()->ConstantNoHole(function.context(broker()), broker());
    ...
    if (match.IsJSCreateClosure()) {
        ...
        *context_out = effect = graph()->NewNode(simplified()->LoadField(AccessBuilder::ForJSFunctionContext()), match.node(), effect, control);
...
#if V8_ENABLE_WEBASSEMBLY
JSInliner::WasmInlineResult JSInliner::TryWasmInlining(const JSWasmCallNode& call_node)
{
    ...
    TRACE("Considering wasm function [" << fct_index << "] " << WasmFunctionNameForTrace(native_module, fct_index) << " of module " << wasm_call_params.module()
                                        << " for inlining");
    ...
    WasmGraphBuilder builder(
        nullptr, zone(), jsgraph(), sig, source_positions_, WasmGraphBuilder::kJSFunctionAbiMode, isolate(), native_module->enabled_features());
    ...
    bool can_inline_body = builder.TryWasmInlining(fct_index, native_module, inlining_id);
    if (can_inline_body) {
        int actual_id = info_->AddInlinedFunction(wasm_call_params.shared_fct_info().object(), Handle<BytecodeArray>(), call_pos);
...
Reduction JSInliner::ReduceJSWasmCall(Node* node)
{
    ...
    size_t subgraph_min_node_id;
    {
        ...
        Node* continuation_frame_state = CreateJSWasmCallBuiltinContinuationFrameState(jsgraph(), call_node.context(), call_node.frame_state(), sig);
        ...
        bool set_in_wasm_flag = !(inline_result.can_inline_body || v8_flags.turboshaft_wasm_in_js_inlining);
        BuildInlinedJSToWasmWrapper(graph()->zone(), jsgraph(), sig, isolate(), source_positions_, continuation_frame_state, set_in_wasm_flag);
    ...
    if (inline_result.can_inline_body || v8_flags.turboshaft_wasm_in_js_inlining) {
        AllNodes inlined_nodes(local_zone_, wrapper_end_node, graph());
        for (Node* subnode : inlined_nodes.reachable) {
            // Ignore nodes that are not part of the inlinee.
            if (subnode->id() < subgraph_min_node_id)
            ...
            if (subnode->opcode() == IrOpcode::kCall && CallDescriptorOf(subnode->op())->kind() == CallDescriptor::kCallWasmFunction) {
        ...
        if (v8_flags.turboshaft_wasm_in_js_inlining && wasm_fct_call) {
            auto [it, inserted] = js_wasm_calls_sidetable_->insert({ wasm_fct_call->id(), &wasm_call_params });
            USE(it);
            DCHECK(inserted);
        }
    }
    ...
    Reduction r = InlineJSWasmCall(node, new_target, context, frame_state, start, wrapper_end_node, exception_target, uncaught_subcalls);
    // Inline the wrapped wasm body if supported.
    if (inline_result.can_inline_body) {
        InlineWasmFunction(wasm_fct_call, inline_result.body_start, inline_result.body_end, call_node.frame_state(), wasm_call_params.shared_fct_info(),
            call_node.ArgumentCount(), context);
    }
    return r;
}

void JSInliner::InlineWasmFunction(
    Node* call, Node* inlinee_start, Node* inlinee_end, Node* frame_state, SharedFunctionInfoRef shared_fct_info, int argument_count, Node* context)
{
    ...
    Node* frame_state_inside
        = CreateArtificialFrameState(call, FrameState { frame_state }, argument_count, FrameStateType::kWasmInlinedIntoJS, shared_fct_info, context, callee);
    Node* check_point = graph()->NewNode(common()->Checkpoint(), frame_state_inside, effect, control);
    effect = check_point;
    ...
    for (Edge edge : inlinee_start->use_edges()) {
        ...
        default:
            if (NodeProperties::IsEffectEdge(edge)) {
                ...
                edge.UpdateTo(use->opcode() == IrOpcode::kProjection ? graph()->start() : control);
            } else {
                UNREACHABLE();
            }
            Revisit(edge.from());
    ...
    for (Edge use_edge : call->use_edges()) {
        if (NodeProperties::IsValueEdge(use_edge)) {
            ...
            ReplaceWithValue(use, return_count == 1 ? terminator->InputAt(1) : jsgraph()->UndefinedConstant());
        }
    }
...
Reduction JSInliner::ReduceJSCall(Node* node)
{
    ...
    SharedFunctionInfoRef outer_shared_info = MakeRef(broker(), info_->shared_info());

    SharedFunctionInfo::Inlineability inlineability = shared_info->GetInlineability(broker());
    if (inlineability != SharedFunctionInfo::kIsInlineable) {
        ...
        TRACE("Not inlining " << *shared_info << " into " << outer_shared_info << " because it had its optimization disabled.");
        return NoChange();
    }
    ...
    if (node->opcode() == IrOpcode::kJSConstruct && !IsConstructable(shared_info->kind())) {
        TRACE("Not inlining " << *shared_info << " into " << outer_shared_info << " because constructor is not constructable.");
        return NoChange();
    }
    ...
    if (node->opcode() == IrOpcode::kJSCall && IsClassConstructor(shared_info->kind())) {
        TRACE("Not inlining " << *shared_info << " into " << outer_shared_info << " because callee is a class constructor.");
        return NoChange();
    }
    ...
    // the nested calls.
    int nesting_level = 0;
    for (Node* frame_state = call.frame_state(); frame_state->opcode() == IrOpcode::kFrameState; frame_state = FrameState { frame_state }.outer_frame_state()) {
        nesting_level++;
    ...
    if (info_->source_positions() && !shared_info->object()->AreSourcePositionsAvailable(broker()->local_isolate_or_isolate())) {
        ...
        TRACE("Not inlining " << *shared_info << " into " << outer_shared_info << " because source positions are missing.");
        return NoChange();
    }
    ...
    TRACE("Inlining " << *shared_info << " into " << outer_shared_info << ((exception_target != nullptr) ? " (inside try-block)" : ""));
    // ----------------------------------------------------------------
    ...
    int inlining_id = info_->AddInlinedFunction(shared_info->object(), bytecode_array.object(), source_positions_->GetSourcePosition(node));
    if (v8_flags.profile_guided_optimization && feedback_cell.feedback_vector(broker()).has_value()
        && feedback_cell.feedback_vector(broker()).value().object()->invocation_count_before_stable(kRelaxedLoad)
    ...
    Node* end;
    {
        ...
        Graph::SubgraphScope scope(graph());
        BytecodeGraphBuilderFlags flags(BytecodeGraphBuilderFlag::kSkipFirstStackAndTierupCheck);
        ...
        }
        {
            ...
            BuildGraphFromBytecode(broker(), zone(), *shared_info, feedback_cell, BytecodeOffset::None(), jsgraph(), frequency, source_positions_,
                node_origins_, inlining_id, info_->code_kind(), flags, &info_->tick_counter());
        }
    ...
    if (node->opcode() == IrOpcode::kJSConstruct) {
        ...
        if (NeedsImplicitReceiver(*shared_info)) {
            ...
            if (m.HasResolvedValue() && m.Ref(broker()).IsJSFunction()) {
                ...
                frame_state_inside
                    = CreateArtificialFrameState(node, frame_state, n.ArgumentCount(), FrameStateType::kConstructCreateStub, *shared_info, caller_context);
            }
            Node* create = graph()->NewNode(javascript()->Create(), call.target(), new_target, caller_context, frame_state_inside, effect, control);
            ...
            result = graph()->NewNode(common()->Select(MachineRepresentation::kTagged), check, node, create);
            ...
            branch_is_receiver_false = graph()->NewNode(javascript()->CallRuntime(Runtime::kThrowConstructorReturnedNonObject), caller_context,
                NodeProperties::GetFrameStateInput(node), node, branch_is_receiver_false);
            uncaught_subcalls.push_back(branch_is_receiver_false);
            branch_is_receiver_false = graph()->NewNode(common()->Throw(), branch_is_receiver_false, branch_is_receiver_false);
            MergeControlToEnd(graph(), common(), branch_is_receiver_false);

            ReplaceWithValue(node_success, node_success, node_success, branch_is_receiver_true);
        ...
        frame_state = CreateArtificialFrameState(node, frame_state, 0, FrameStateType::kConstructInvokeStub, *shared_info, caller_context);
    }
    ...
    if (node->opcode() == IrOpcode::kJSCall && is_sloppy(shared_info->language_mode()) && !shared_info->native()) {
        ...
        if (NodeProperties::CanBePrimitive(broker(), call.receiver(), effect)) {
            ...
            Node* global_proxy = jsgraph()->ConstantNoHole(broker()->target_native_context().global_proxy_object(broker()), broker());
            Node* receiver = effect = graph()->NewNode(simplified()->ConvertReceiver(p.convert_mode()), call.receiver(),
                jsgraph()->ConstantNoHole(broker()->target_native_context(), broker()), global_proxy, effect, start);
            NodeProperties::ReplaceValueInput(node, receiver, JSCallNode::ReceiverIndex());
            NodeProperties::ReplaceEffectInput(node, effect);
        }
    }
    ...
    DCHECK_EQ(parameter_count, shared_info->internal_formal_parameter_count_without_receiver());
    DCHECK_EQ(parameter_count, start.FormalParameterCountWithoutReceiver());
    if (call.argument_count() != parameter_count) {
        frame_state = CreateArtificialFrameState(node, frame_state, call.argument_count(), FrameStateType::kInlinedExtraArguments, *shared_info);
    }

    return InlineCall(node, new_target, context, frame_state, start, end, exception_target, uncaught_subcalls, call.argument_count());
}
