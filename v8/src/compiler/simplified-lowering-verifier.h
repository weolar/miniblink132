...
public:
    ...
    void RecordMachineUsesOfConstant(Node* constant, Node::Uses uses)
    {
        ...
        if (it == machine_uses_of_constants_.end()) {
            it = machine_uses_of_constants_.emplace(constant, ZoneVector<Node*>(zone_)).first;
        }
...
private:
    void ResizeDataIfNecessary(Node* node)
    {
        ...
        DCHECK_EQ(data_[node->id()].truncation, Truncation::Any(IdentifyZeros::kDistinguishZeros));
    }
    ...
    {
        static const Truncation any_truncation = Truncation::Any(IdentifyZeros::kDistinguishZeros);
