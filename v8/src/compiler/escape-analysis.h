...
public:
    ...
    EffectGraphReducer(Graph* graph, std::function<void(Node*, Reduction*)> reduce, TickCounter* tick_counter, Zone* zone);

    void ReduceGraph()
    {
        ReduceFrom(graph_->end());
    }
...
public:
    ...
    int size() const
    {
        return static_cast<int>(kTaggedSize * fields_.size());
    }
...
class V8_EXPORT_PRIVATE EscapeAnalysis final : public NON_EXPORTED_BASE(EffectGraphReducer) {
public:
