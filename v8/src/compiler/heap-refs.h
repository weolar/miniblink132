...
{
    return mode == AccessMode::kStore || mode == AccessMode::kStoreInLiteral || mode == AccessMode::kDefine;
}
...
#define IGNORE_CASE(...)
#define HEAP_BROKER_BACKGROUND_SERIALIZED_OBJECT_LIST(V) HEAP_BROKER_OBJECT_LIST_BASE(V, IGNORE_CASE)
...
template <class T> struct is_ref : public std::false_type { };

#define DEFINE_IS_REF(Name)                                                                                                                                    \
    template <> struct is_ref<Name##Ref> : public std::true_type { };
...
#define BACKGROUND_SERIALIZED_REF_TRAITS(Name)                                                                                                                 \
    template <> struct ref_traits<Name> {                                                                                                                      \
        using ref_type = Name##Ref;                                                                                                                            \
        using data_type = Name##Data;                                                                                                                          \
        static constexpr RefSerializationKind ref_serialization_kind = RefSerializationKind::kBackgroundSerialized;                                            \
    };
...
#define NEVER_SERIALIZED_REF_TRAITS(Name)                                                                                                                      \
    template <> struct ref_traits<Name> {                                                                                                                      \
        ...
        static constexpr RefSerializationKind ref_serialization_kind = RefSerializationKind::kNeverSerialized;                                                 \
    };

HEAP_BROKER_OBJECT_LIST_BASE(BACKGROUND_SERIALIZED_REF_TRAITS, NEVER_SERIALIZED_REF_TRAITS)
#undef NEVER_SERIALIZED_REF_TRAITS
#undef BACKGROUND_SERIALIZED_REF_TRAITS

template <> struct ref_traits<Object> {
...
// existing Ref.
template <> struct ref_traits<Oddball> : public ref_traits<HeapObject> { };
template <> struct ref_traits<Null> : public ref_traits<HeapObject> { };
template <> struct ref_traits<Undefined> : public ref_traits<HeapObject> { };
template <> struct ref_traits<True> : public ref_traits<HeapObject> { };
template <> struct ref_traits<False> : public ref_traits<HeapObject> { };
template <> struct ref_traits<Hole> : public ref_traits<HeapObject> { };
template <> struct ref_traits<EnumCache> : public ref_traits<HeapObject> { };
template <> struct ref_traits<PropertyArray> : public ref_traits<HeapObject> { };
template <> struct ref_traits<ByteArray> : public ref_traits<HeapObject> { };
template <> struct ref_traits<TrustedFixedArray> : public ref_traits<HeapObject> { };
template <> struct ref_traits<ClosureFeedbackCellArray> : public ref_traits<HeapObject> { };
template <> struct ref_traits<NumberDictionary> : public ref_traits<HeapObject> { };
template <> struct ref_traits<OrderedHashMap> : public ref_traits<HeapObject> { };
template <> struct ref_traits<OrderedHashSet> : public ref_traits<HeapObject> { };
template <> struct ref_traits<FeedbackMetadata> : public ref_traits<HeapObject> { };
template <> struct ref_traits<NameDictionary> : public ref_traits<HeapObject> { };
template <> struct ref_traits<OrderedNameDictionary> : public ref_traits<HeapObject> { };
template <> struct ref_traits<SwissNameDictionary> : public ref_traits<HeapObject> { };
template <> struct ref_traits<InterceptorInfo> : public ref_traits<HeapObject> { };
template <> struct ref_traits<ArrayList> : public ref_traits<HeapObject> { };
template <> struct ref_traits<WeakFixedArray> : public ref_traits<HeapObject> { };
template <> struct ref_traits<WeakArrayList> : public ref_traits<HeapObject> { };
template <> struct ref_traits<RegisteredSymbolTable> : public ref_traits<HeapObject> { };
#if V8_ENABLE_WEBASSEMBLY
template <> struct ref_traits<WasmNull> : public ref_traits<HeapObject> { };
#endif // V8_ENABLE_WEBASSEMBLY
template <> struct ref_traits<Smi> : public ref_traits<Object> { };
template <> struct ref_traits<Boolean> : public ref_traits<HeapObject> { };
template <> struct ref_traits<JSProxy> : public ref_traits<JSReceiver> { };
template <> struct ref_traits<JSWrappedFunction> : public ref_traits<JSFunction> { };

template <class... T> struct ref_traits<Union<T...>> {
    // There's no good way in C++ to find a common base class, so just test a few
    // common cases.
    static constexpr bool kAllJSReceiverRef = (std::is_base_of_v<JSReceiverRef, typename ref_traits<T>::ref_type> && ...);
    static constexpr bool kAllHeapObjectRef = (std::is_base_of_v<JSReceiverRef, typename ref_traits<T>::ref_type> && ...);

    using ref_type = std::conditional_t<kAllJSReceiverRef, JSReceiverRef, std::conditional_t<kAllHeapObjectRef, HeapObjectRef, ObjectRef>>;
    using data_type = std::conditional_t<kAllJSReceiverRef, JSReceiverData, std::conditional_t<kAllHeapObjectRef, HeapObjectData, ObjectData>>;

    static constexpr RefSerializationKind ref_serialization_kind = ((ref_traits<T>::ref_serialization_kind == RefSerializationKind::kNeverSerialized) && ...)
...
template <typename TRef> class OptionalRef {
public:
    ...
    private:
        ...
        explicit ArrowOperatorHelper(TRef object)
            : object_(object)
        {
        }
    ...
    template <typename SRef, typename = typename std::enable_if<std::is_convertible<SRef*, TRef*>::value>::type>
    // NOLINTNEXTLINE
...
private:
    explicit OptionalRef(ObjectData* data)
        : data_(data)
    {
    ...
    template <typename SRef> friend class OptionalRef;
};

template <typename T> inline bool operator==(OptionalRef<T> lhs, OptionalRef<T> rhs)
{
    return lhs.equals(rhs);
}

template <typename T> inline size_t hash_value(OptionalRef<T> ref)
{
...
public:
    explicit ObjectRef(ObjectData* data, bool check_type = true)
        : data_(data)
    {
        CHECK_NOT_NULL(data_);
    }
    ...
    size_t hash_value() const
    {
        return base::hash_combine(object().address());
    }
...
private:
    ...
    template <typename TRef> friend class OptionalRef;

    friend std::ostream& operator<<(std::ostream& os, ObjectRef ref);
    friend bool operator<(ObjectRef lhs, ObjectRef rhs);
    template <typename T, typename Enable> friend struct ::v8::internal::ZoneCompactSetTraits;
};
...
inline size_t hash_value(ObjectRef ref)
{
    return ref.hash_value();
}

template <class T> using ZoneRefUnorderedSet = ZoneUnorderedSet<T, ObjectRef::Hash>;

template <class K, class V> using ZoneRefMap = ZoneMap<K, V>;
...
public:
    ...
    HeapObjectType(InstanceType instance_type, ElementsKind elements_kind, Flags flags, OddballType oddball_type, HoleType hole_type)
        ...
        , flags_(flags)
    {
        DCHECK_EQ(instance_type == ODDBALL_TYPE, oddball_type != OddballType::kNone);
    }
...
template <typename T> struct ZoneCompactSetTraits<T, std::enable_if_t<compiler::is_ref<T>::value>> {
    using handle_type = T;
...
template <typename T> using ZoneRefSet = ZoneCompactSet<typename ref_traits<T>::ref_type>;

inline bool AnyMapIsHeapNumber(const ZoneRefSet<Map>& maps)
{
    return std::any_of(maps.begin(), maps.end(), [](MapRef map) { return map.IsHeapNumberMap(); });
}
