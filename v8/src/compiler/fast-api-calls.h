...
struct OverloadsResolutionResult {
    ...
    {
        DCHECK(distinguishable_arg_index_ < 0 || element_type_ != CTypeInfo::Type::kVoid);
    }

    bool is_valid() const
    {
        return distinguishable_arg_index >= 0;
    }
...
using GetParameter = std::function<Node*(int, OverloadsResolutionResult&, GraphAssemblerLabel<0>*)>;
