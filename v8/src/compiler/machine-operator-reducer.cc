...
public:
    ...
    explicit Word32Adapter(MachineOperatorReducer* reducer)
        : r_(reducer)
    {
    }

    template <typename T> static bool IsWordNAnd(const T& x)
    {
        return x.IsWord32And();
    }
    template <typename T> static bool IsWordNShl(const T& x)
    {
        return x.IsWord32Shl();
    }
    template <typename T> static bool IsWordNShr(const T& x)
    {
        return x.IsWord32Shr();
    }
    template <typename T> static bool IsWordNSar(const T& x)
    {
        return x.IsWord32Sar();
    }
    static bool IsWordNSarShiftOutZeros(const Operator* op)
    {
        return op->opcode() == IrOpcode::kWord32Sar && OpParameter<ShiftKind>(op) == ShiftKind::kShiftOutZeros;
    }
    template <typename T> static bool IsWordNXor(const T& x)
    {
        return x.IsWord32Xor();
    }
    template <typename T> static bool IsIntNAdd(const T& x)
    {
        return x.IsInt32Add();
    }
    template <typename T> static bool IsIntNMul(const T& x)
    {
        return x.IsInt32Mul();
    }
    ...
    {
        return r_->ReplaceInt32(value);
    }
    Reduction ReduceWordNAnd(Node* node)
    {
        return r_->ReduceWord32And(node);
    }
    Reduction ReduceIntNAdd(Node* node)
    {
        return r_->ReduceInt32Add(node);
    }
    Reduction TryMatchWordNRor(Node* node)
    {
        return r_->TryMatchWord32Ror(node);
    }

    Node* IntNConstant(int32_t value)
    {
        return r_->Int32Constant(value);
    }
    Node* UintNConstant(uint32_t value)
    {
        return r_->Uint32Constant(value);
    }
    Node* WordNAnd(Node* lhs, Node* rhs)
    {
        return r_->Word32And(lhs, rhs);
    }
...
public:
    ...
    explicit Word64Adapter(MachineOperatorReducer* reducer)
        : r_(reducer)
    {
    }
    ...
    static bool IsWordNSarShiftOutZeros(const Operator* op)
    {
        return op->opcode() == IrOpcode::kWord64Sar && OpParameter<ShiftKind>(op) == ShiftKind::kShiftOutZeros;
    }
    template <typename T> static bool IsWordNXor(const T& x)
    {
    ...
    {
        return r_->ReplaceInt64(value);
    }
    Reduction ReduceWordNAnd(Node* node)
    {
        return r_->ReduceWord64And(node);
    }
    Reduction ReduceIntNAdd(Node* node)
    {
        return r_->ReduceInt64Add(node);
    }
    Reduction TryMatchWordNRor(Node* node)
    {
    ...
    {
        return r_->Int64Constant(value);
    }
    Node* UintNConstant(uint64_t value)
    {
        return r_->Uint64Constant(value);
    }
    Node* WordNAnd(Node* lhs, Node* rhs)
    {
        return r_->Word64And(lhs, rhs);
    }
...
MachineOperatorReducer::MachineOperatorReducer(Editor* editor, MachineGraph* mcgraph, SignallingNanPropagation signalling_nan_propagation)
...
Node* MachineOperatorReducer::Float64PowHalf(Node* value)
{
    ...
    Diamond d(graph(), common(), graph()->NewNode(machine()->Float64LessThanOrEqual(), value, Float64Constant(-V8_INFINITY)), BranchHint::kFalse);
    return d.Phi(MachineRepresentation::kFloat64, Float64Constant(V8_INFINITY), graph()->NewNode(machine()->Float64Sqrt(), value));
}
...
Node* MachineOperatorReducer::Int32Div(Node* dividend, int32_t divisor)
{
    ...
    base::MagicNumbersForDivision<uint32_t> const mag = base::SignedDivisionByConstant(base::bit_cast<uint32_t>(divisor));
    Node* quotient = graph()->NewNode(machine()->Int32MulHigh(), dividend, Uint32Constant(mag.multiplier));
...
Node* MachineOperatorReducer::Int64Div(Node* dividend, int64_t divisor)
{
    ...
    base::MagicNumbersForDivision<uint64_t> const mag = base::SignedDivisionByConstant(base::bit_cast<uint64_t>(divisor));
    Node* quotient = graph()->NewNode(machine()->Int64MulHigh(), dividend, Uint64Constant(mag.multiplier));
...
Node* MachineOperatorReducer::Uint32Div(Node* dividend, uint32_t divisor)
{
    ...
    Node* quotient = graph()->NewNode(machine()->Uint32MulHigh(), dividend, Uint32Constant(mag.multiplier));
    if (mag.add) {
        DCHECK_LE(1u, mag.shift);
        quotient = Word32Shr(Int32Add(Word32Shr(Int32Sub(dividend, quotient), 1), quotient), mag.shift - 1);
    } else {
...
Node* MachineOperatorReducer::Uint64Div(Node* dividend, uint64_t divisor)
{
    ...
    divisor >>= shift;
    // Compute the magic number for the (shifted) divisor.
    base::MagicNumbersForDivision<uint64_t> const mag = base::UnsignedDivisionByConstant(divisor, shift);
    Node* quotient = graph()->NewNode(machine()->Uint64MulHigh(), dividend, Uint64Constant(mag.multiplier));
    if (mag.add) {
        DCHECK_LE(1u, mag.shift);
        quotient = Word64Shr(Int64Add(Word64Shr(Int64Sub(dividend, quotient), 1), quotient), mag.shift - 1);
    } else {
...
Reduction MachineOperatorReducer::Reduce(Node* node)
{
    ...
    case IrOpcode::kWord32Ror: {
        ...
        if (m.IsFoldable()) { // K ror K => K  (K stands for arbitrary constants)
            return ReplaceInt32(base::bits::RotateRight32(m.left().ResolvedValue(), m.right().ResolvedValue() & 31));
        }
        break;
    }
    ...
    case IrOpcode::kInt32Mul: {
        ...
        if (m.IsFoldable()) { // K * K => K  (K stands for arbitrary constants)
            return ReplaceInt32(base::MulWithWraparound(m.left().ResolvedValue(), m.right().ResolvedValue()));
        }
        ...
        if (m.right().IsPowerOf2()) { // x * 2^n => x << n
            node->ReplaceInput(1, Int32Constant(base::bits::WhichPowerOfTwo(m.right().ResolvedValue())));
        ...
        if (m.right().HasResolvedValue() && m.left().IsInt32Mul()) {
            ...
            if (n.right().HasResolvedValue() && m.OwnsInput(m.left().node())) {
                node->ReplaceInput(1, Int32Constant(base::MulWithWraparound(m.right().ResolvedValue(), n.right().ResolvedValue())));
    ...
    case IrOpcode::kFloat32Sub: {
        ...
        if (signalling_nan_propagation_ == kPropagateSignallingNan && m.right().Is(0) && (std::copysign(1.0, m.right().ResolvedValue()) > 0)) {
        ...
        if (m.IsFoldable()) { // L - R => (L - R)
            return ReplaceFloat32(m.left().ResolvedValue() - m.right().ResolvedValue());
        }
        if (signalling_nan_propagation_ == kPropagateSignallingNan && m.left().IsMinusZero()) {
            ...
            if (machine()->Float32RoundUp().IsSupported() && m.right().IsFloat32RoundDown()) {
                if (m.right().InputAt(0)->opcode() == IrOpcode::kFloat32Sub) {
                    ...
                    if (mright0.left().IsMinusZero()) {
                        return Replace(graph()->NewNode(machine()->Float32RoundUp().op(), mright0.right().node()));
                    }
                }
            }
            // -0.0 - R => -R
    ...
    case IrOpcode::kFloat64Add: {
        ...
        if (m.IsFoldable()) { // K + K => K  (K stands for arbitrary constants)
            return ReplaceFloat64(m.left().ResolvedValue() + m.right().ResolvedValue());
        }
        break;
    }
    case IrOpcode::kFloat64Sub: {
        Float64BinopMatcher m(node);
        if (signalling_nan_propagation_ == kPropagateSignallingNan && m.right().Is(0) && (base::Double(m.right().ResolvedValue()).Sign() > 0)) {
            return Replace(m.left().node()); // x - 0 => x
        }
        if (m.right().IsNaN()) { // x - NaN => NaN
            return ReplaceFloat64(SilenceNaN(m.right().ResolvedValue()));
        ...
        if (signalling_nan_propagation_ == kPropagateSignallingNan && m.left().IsMinusZero()) {
            ...
            if (machine()->Float64RoundUp().IsSupported() && m.right().IsFloat64RoundDown()) {
                if (m.right().InputAt(0)->opcode() == IrOpcode::kFloat64Sub) {
                    ...
                    if (mright0.left().IsMinusZero()) {
                        return Replace(graph()->NewNode(machine()->Float64RoundUp().op(), mright0.right().node()));
                    }
                }
            }
            // -0.0 - R => -R
    ...
    case IrOpcode::kFloat64Mul: {
        Float64BinopMatcher m(node);
        if (signalling_nan_propagation_ == kPropagateSignallingNan && m.right().Is(1))
    ...
    case IrOpcode::kFloat64Div: {
        ...
        if (m.IsFoldable()) { // K / K => K  (K stands for arbitrary constants)
            return ReplaceFloat64(base::Divide(m.left().ResolvedValue(), m.right().ResolvedValue()));
        }
        if (signalling_nan_propagation_ == kPropagateSignallingNan && m.right().Is(-1)) { // x / -1.0 => -x
            node->RemoveInput(1);
            NodeProperties::ChangeOp(node, machine()->Float64Neg());
            return Changed(node);
        }
    ...
    case IrOpcode::kFloat64Mod: {
        ...
        if (m.IsFoldable()) { // K % K => K  (K stands for arbitrary constants)
            return ReplaceFloat64(Modulo(m.left().ResolvedValue(), m.right().ResolvedValue()));
        }
        break;
    }
    ...
    case IrOpcode::kFloat64Atan2: {
        ...
        if (m.IsFoldable()) {
            return ReplaceFloat64(base::ieee754::atan2(m.left().ResolvedValue(), m.right().ResolvedValue()));
        }
        break;
    }
    ...
    case IrOpcode::kFloat64Pow: {
        Float64BinopMatcher m(node);
        if (m.IsFoldable()) {
            return ReplaceFloat64(base::ieee754::pow(m.left().ResolvedValue(), m.right().ResolvedValue()));
    ...
    case IrOpcode::kChangeFloat32ToFloat64: {
        ...
        if (m.HasResolvedValue()) {
            if (signalling_nan_propagation_ == kSilenceSignallingNan && std::isnan(m.ResolvedValue())) {
    ...
    case IrOpcode::kBitcastWord32ToWord64: {
        Int32Matcher m(node->InputAt(0));
        if (m.HasResolvedValue())
            return ReplaceInt64(m.ResolvedValue());
    ...
    case IrOpcode::kTruncateFloat64ToFloat32: {
        ...
        if (signalling_nan_propagation_ == kPropagateSignallingNan && m.IsChangeFloat32ToFloat64())
            return Replace(m.node()->InputAt(0));
        break;
    }
...
Reduction MachineOperatorReducer::ReduceTruncateInt64ToInt32(Node* node)
{
    ...
    if (m.IsBitcastTaggedToWordForTagAndSmiBits() && m.node()->UseCount() == 1) {
        ...
        if (input->opcode() == IrOpcode::kLoad || input->opcode() == IrOpcode::kLoadImmutable) {
            ...
            if (ElementSizeLog2Of(load_rep.representation()) == 2) {
                ...
                if (value_edges == 1) {
                    ...
                    NodeProperties::ChangeOp(input,
                        input->opcode() == IrOpcode::kLoad ? machine()->Load(LoadRepresentation::Int32())
...
Reduction MachineOperatorReducer::ReduceInt32Add(Node* node)
{
    ...
    if (m.right().Is(0))
        return Replace(m.left().node()); // x + 0 => x
    if (m.IsFoldable()) { // K + K => K  (K stands for arbitrary constants)
        return ReplaceInt32(base::AddWithWraparound(m.left().ResolvedValue(), m.right().ResolvedValue()));
    }
    ...
    if (m.right().HasResolvedValue() && m.left().IsInt32Add()) {
        Int32BinopMatcher n(m.left().node());
        if (n.right().HasResolvedValue() && m.OwnsInput(m.left().node())) {
            node->ReplaceInput(1, Int32Constant(base::AddWithWraparound(m.right().ResolvedValue(), n.right().ResolvedValue())));
            node->ReplaceInput(0, n.left().node());
            return Changed(node);
        }
    }
...
Reduction MachineOperatorReducer::ReduceInt64Add(Node* node)
{
    ...
    if (m.IsFoldable()) {
        return ReplaceInt64(base::AddWithWraparound(m.left().ResolvedValue(), m.right().ResolvedValue()));
    }
    ...
    if (m.right().HasResolvedValue() && m.left().IsInt64Add()) {
        ...
        if (n.right().HasResolvedValue() && m.OwnsInput(m.left().node())) {
            node->ReplaceInput(1, Int64Constant(base::AddWithWraparound(m.right().ResolvedValue(), n.right().ResolvedValue())));
            node->ReplaceInput(0, n.left().node());
            return Changed(node);
        }
    }
...
Reduction MachineOperatorReducer::ReduceInt32Sub(Node* node)
{
    ...
    if (m.right().Is(0))
        return Replace(m.left().node()); // x - 0 => x
    if (m.IsFoldable()) { // K - K => K  (K stands for arbitrary constants)
        return ReplaceInt32(base::SubWithWraparound(m.left().ResolvedValue(), m.right().ResolvedValue()));
    }
    if (m.LeftEqualsRight())
        return ReplaceInt32(0); // x - x => 0
    if (m.right().HasResolvedValue()) { // x - K => x + -K
        node->ReplaceInput(1, Int32Constant(base::NegateWithWraparound(m.right().ResolvedValue())));
...
Reduction MachineOperatorReducer::ReduceInt64Sub(Node* node)
{
    ...
    if (m.right().Is(0))
        return Replace(m.left().node()); // x - 0 => x
    if (m.IsFoldable()) { // K - K => K  (K stands for arbitrary constants)
        return ReplaceInt64(base::SubWithWraparound(m.left().ResolvedValue(), m.right().ResolvedValue()));
    }
    if (m.LeftEqualsRight())
        return Replace(Int64Constant(0)); // x - x => 0
    if (m.right().HasResolvedValue()) { // x - K => x + -K
        node->ReplaceInput(1, Int64Constant(base::NegateWithWraparound(m.right().ResolvedValue())));
...
Reduction MachineOperatorReducer::ReduceInt64Mul(Node* node)
{
    ...
    if (m.right().Is(1))
        return Replace(m.left().node()); // x * 1 => x
    if (m.IsFoldable()) { // K * K => K  (K stands for arbitrary constants)
        return ReplaceInt64(base::MulWithWraparound(m.left().ResolvedValue(), m.right().ResolvedValue()));
    }
    if (m.right().Is(-1)) { // x * -1 => 0 - x
    ...
    if (m.right().IsPowerOf2()) { // x * 2^n => x << n
        node->ReplaceInput(1, Int64Constant(base::bits::WhichPowerOfTwo(m.right().ResolvedValue())));
    ...
    if (m.right().HasResolvedValue() && m.left().IsInt64Mul()) {
        Int64BinopMatcher n(m.left().node());
        if (n.right().HasResolvedValue() && m.OwnsInput(m.left().node())) {
            node->ReplaceInput(1, Int64Constant(base::MulWithWraparound(m.right().ResolvedValue(), n.right().ResolvedValue())));
            node->ReplaceInput(0, n.left().node());
            return Changed(node);
        }
    }
...
Reduction MachineOperatorReducer::ReduceInt32Div(Node* node)
{
    ...
    if (m.right().Is(1))
        return Replace(m.left().node()); // x / 1 => x
    if (m.IsFoldable()) { // K / K => K  (K stands for arbitrary constants)
        return ReplaceInt32(base::bits::SignedDiv32(m.left().ResolvedValue(), m.right().ResolvedValue()));
    }
...
Reduction MachineOperatorReducer::ReduceInt64Div(Node* node)
{
    ...
    if (m.right().Is(1))
        return Replace(m.left().node()); // x / 1 => x
    if (m.IsFoldable()) { // K / K => K  (K stands for arbitrary constants)
        return ReplaceInt64(base::bits::SignedDiv64(m.left().ResolvedValue(), m.right().ResolvedValue()));
    }
    if (m.LeftEqualsRight()) { // x / x => x != 0
        Node* const zero = Int64Constant(0);
        // {Word64Equal} can get reduced to a bool/int32, but we need this
        // operation to produce an int64.
        return Replace(ChangeInt32ToInt64(Word64Equal(Word64Equal(m.left().node(), zero), zero)));
    }
    if (m.right().Is(-1)) { // x / -1 => 0 - x
        node->ReplaceInput(0, Int64Constant(0));
        node->ReplaceInput(1, m.left().node());
...
Reduction MachineOperatorReducer::ReduceUint32Div(Node* node)
{
    ...
    if (m.right().Is(1))
        return Replace(m.left().node()); // x / 1 => x
    if (m.IsFoldable()) { // K / K => K  (K stands for arbitrary constants)
        return ReplaceUint32(base::bits::UnsignedDiv32(m.left().ResolvedValue(), m.right().ResolvedValue()));
    }
    if (m.LeftEqualsRight()) { // x / x => x != 0
        Node* const zero = Int32Constant(0);
        return Replace(Word32Equal(Word32Equal(m.left().node(), zero), zero));
    ...
    if (m.right().HasResolvedValue()) {
        ...
        if (base::bits::IsPowerOfTwo(divisor)) { // x / 2^n => x >> n
            node->ReplaceInput(1, Uint32Constant(base::bits::WhichPowerOfTwo(m.right().ResolvedValue())));
            node->TrimInputCount(2);
...
Reduction MachineOperatorReducer::ReduceUint64Div(Node* node)
{
    ...
    if (m.right().Is(1))
        return Replace(m.left().node()); // x / 1 => x
    if (m.IsFoldable()) { // K / K => K  (K stands for arbitrary constants)
        return ReplaceUint64(base::bits::UnsignedDiv64(m.left().ResolvedValue(), m.right().ResolvedValue()));
    }
    if (m.LeftEqualsRight()) { // x / x => x != 0
        Node* const zero = Int64Constant(0);
        // {Word64Equal} can get reduced to a bool/int32, but we need this
    ...
    if (m.right().HasResolvedValue()) {
        ...
        if (base::bits::IsPowerOfTwo(divisor)) { // x / 2^n => x >> n
            node->ReplaceInput(1, Uint64Constant(base::bits::WhichPowerOfTwo(m.right().ResolvedValue())));
            node->TrimInputCount(2);
...
Reduction MachineOperatorReducer::ReduceInt32Mod(Node* node)
{
    ...
    if (m.LeftEqualsRight())
        return ReplaceInt32(0); // x % x  => 0
    if (m.IsFoldable()) { // K % K => K  (K stands for arbitrary constants)
        return ReplaceInt32(base::bits::SignedMod32(m.left().ResolvedValue(), m.right().ResolvedValue()));
    }
    if (m.right().HasResolvedValue()) {
        Node* const dividend = m.left().node();
        ...
        if (base::bits::IsPowerOfTwo(divisor)) {
            ...
            Node* const zero = Int32Constant(0);
            Diamond d(graph(), common(), graph()->NewNode(machine()->Int32LessThan(), dividend, zero), BranchHint::kFalse);
            return Replace(d.Phi(MachineRepresentation::kWord32, Int32Sub(zero, Word32And(Int32Sub(zero, dividend), mask)), Word32And(dividend, mask)));
        } else {
...
Reduction MachineOperatorReducer::ReduceInt64Mod(Node* node)
{
    ...
    if (m.right().Is(0))
        return Replace(m.right().node()); // x % 0  => 0
    if (m.right().Is(1))
        return ReplaceInt64(0); // x % 1  => 0
    if (m.right().Is(-1))
        return ReplaceInt64(0); // x % -1 => 0
    if (m.LeftEqualsRight())
        return ReplaceInt64(0); // x % x  => 0
    if (m.IsFoldable()) { // K % K => K  (K stands for arbitrary constants)
        return ReplaceInt64(base::bits::SignedMod64(m.left().ResolvedValue(), m.right().ResolvedValue()));
    }
    if (m.right().HasResolvedValue()) {
        Node* const dividend = m.left().node();
        ...
        if (base::bits::IsPowerOfTwo(divisor)) {
            ...
            Node* const zero = Int64Constant(0);
            Diamond d(graph(), common(), graph()->NewNode(machine()->Int64LessThan(), dividend, zero), BranchHint::kFalse);
            return Replace(d.Phi(MachineRepresentation::kWord64, Int64Sub(zero, Word64And(Int64Sub(zero, dividend), mask)), Word64And(dividend, mask)));
        } else {
...
Reduction MachineOperatorReducer::ReduceUint32Mod(Node* node)
{
    ...
    if (m.LeftEqualsRight())
        return ReplaceUint32(0); // x % x  => 0
    if (m.IsFoldable()) { // K % K => K  (K stands for arbitrary constants)
        return ReplaceUint32(base::bits::UnsignedMod32(m.left().ResolvedValue(), m.right().ResolvedValue()));
    }
    if (m.right().HasResolvedValue()) {
        Node* const dividend = m.left().node();
        uint32_t const divisor = m.right().ResolvedValue();
...
Reduction MachineOperatorReducer::ReduceUint64Mod(Node* node)
{
    ...
    if (m.right().Is(0))
        return Replace(m.right().node()); // x % 0 => 0
    if (m.right().Is(1))
        return ReplaceUint64(0); // x % 1 => 0
    if (m.LeftEqualsRight())
        return ReplaceUint64(0); // x % x  => 0
    if (m.IsFoldable()) { // K % K => K  (K stands for arbitrary constants)
        return ReplaceUint64(base::bits::UnsignedMod64(m.left().ResolvedValue(), m.right().ResolvedValue()));
    }
    if (m.right().HasResolvedValue()) {
        Node* const dividend = m.left().node();
        uint64_t const divisor = m.right().ResolvedValue();
...
Reduction MachineOperatorReducer::ReduceStore(Node* node)
{
    ...
    MachineRepresentation rep = nm.IsStore() ? StoreRepresentationOf(node->op()).representation() : UnalignedStoreRepresentationOf(node->op());
    ...
    case IrOpcode::kWord32And: {
        ...
        if (m.right().HasResolvedValue()
            && ((rep == MachineRepresentation::kWord8 && (m.right().ResolvedValue() & 0xFF) == 0xFF)
                || (rep == MachineRepresentation::kWord16 && (m.right().ResolvedValue() & 0xFFFF) == 0xFFFF))) {
    ...
    case IrOpcode::kWord32Sar: {
        ...
        if (m.left().IsWord32Shl()
            && ((rep == MachineRepresentation::kWord8 && m.right().IsInRange(1, 24))
                || (rep == MachineRepresentation::kWord16 && m.right().IsInRange(1, 16)))) {
            Int32BinopMatcher mleft(m.left().node());
...
Reduction MachineOperatorReducer::ReduceProjection(size_t index, Node* node)
{
    ...
    case IrOpcode::kInt32AddWithOverflow: {
        ...
        if (m.IsFoldable()) {
            int32_t val;
            bool ovf = base::bits::SignedAddOverflow32(m.left().ResolvedValue(), m.right().ResolvedValue(), &val);
    ...
    case IrOpcode::kInt32SubWithOverflow: {
        ...
        if (m.IsFoldable()) {
            int32_t val;
            bool ovf = base::bits::SignedSubOverflow32(m.left().ResolvedValue(), m.right().ResolvedValue(), &val);
            return ReplaceInt32(index == 0 ? val : ovf);
        }
        if (m.right().Is(0)) {
            return Replace(index == 0 ? m.left().node() : m.right().node());
    ...
    case IrOpcode::kInt32MulWithOverflow: {
        ...
        if (m.IsFoldable()) {
            int32_t val;
            bool ovf = base::bits::SignedMulOverflow32(m.left().ResolvedValue(), m.right().ResolvedValue(), &val);
            return ReplaceInt32(index == 0 ? val : ovf);
        }
        if (m.right().Is(0)) {
...
// overflow".
template <typename T> bool CanRevertLeftShiftWithRightShift(T value, T shift)
{
    ...
    if (static_cast<T>(static_cast<unsigned_T>(value) << shift) >> shift != static_cast<T>(value)) {
        return false;
    }
    return true;
}
...
{
    DCHECK(node->opcode() == IrOpcode::kInt32LessThan || node->opcode() == IrOpcode::kInt32LessThanOrEqual || node->opcode() == IrOpcode::kUint32LessThan
        || node->opcode() == IrOpcode::kUint32LessThanOrEqual);
    Int32BinopMatcher m(node);
    // (x >> K) < (y >> K) => x < y   if only zeros shifted out
    if (m.left().op() == machine()->Word32SarShiftOutZeros() && m.right().op() == machine()->Word32SarShiftOutZeros()) {
        Int32BinopMatcher mleft(m.left().node());
        Int32BinopMatcher mright(m.right().node());
        if (mleft.right().HasResolvedValue() && mright.right().Is(mleft.right().ResolvedValue())) {
            node->ReplaceInput(0, mleft.left().node());
    ...
    if (m.right().HasResolvedValue() && m.left().op() == machine()->Word32SarShiftOutZeros() && m.left().node()->UseCount() == 1) {
    ...
    // computed here at compile time.
    if (m.left().HasResolvedValue() && m.right().op() == machine()->Word32SarShiftOutZeros() && m.right().node()->UseCount() == 1) {
...
const Operator* MachineOperatorReducer::Map64To32Comparison(const Operator* op, bool sign_extended)
{
    switch (op->opcode()) {
    case IrOpcode::kInt64LessThan:
        return sign_extended ? machine()->Int32LessThan() : machine()->Uint32LessThan();
    case IrOpcode::kInt64LessThanOrEqual:
        return sign_extended ? machine()->Int32LessThanOrEqual() : machine()->Uint32LessThanOrEqual();
    case IrOpcode::kUint64LessThan:
...
{
    DCHECK(node->opcode() == IrOpcode::kInt64LessThan || node->opcode() == IrOpcode::kInt64LessThanOrEqual || node->opcode() == IrOpcode::kUint64LessThan
        || node->opcode() == IrOpcode::kUint64LessThanOrEqual);
    Int64BinopMatcher m(node);

    bool sign_extended = m.left().IsChangeInt32ToInt64() && m.right().IsChangeInt32ToInt64();
    if (sign_extended || (m.left().IsChangeUint32ToUint64() && m.right().IsChangeUint32ToUint64())) {
        ...
        NodeProperties::ChangeOp(node, Map64To32Comparison(node->op(), sign_extended));
        return Changed(node).FollowedBy(Reduce(node));
    }

    // (x >> K) < (y >> K) => x < y   if only zeros shifted out
    // This is useful for Smi untagging, which results in such a shift.
    if (m.left().op() == machine()->Word64SarShiftOutZeros() && m.right().op() == machine()->Word64SarShiftOutZeros()) {
    ...
    // Simplifying (x >> n) <= k into x <= (k << n), with "k << n" being
    // computed here at compile time.
    if (m.right().HasResolvedValue() && m.left().op() == machine()->Word64SarShiftOutZeros() && m.left().node()->UseCount() == 1) {
        Int64BinopMatcher mleft(m.left().node());
        ...
        if (mleft.right().HasResolvedValue()) {
            ...
            if (CanRevertLeftShiftWithRightShift<int64_t>(right, shift)) {
                ...
                if ((sign_extended || mleft.left().IsChangeUint32ToUint64()) && CanTruncate(static_cast<int64_t>(value))) {
                    NodeProperties::ChangeOp(node, Map64To32Comparison(node->op(), sign_extended));
    ...
    // Simplifying k <= (x >> n) into (k << n) <= x, with "k << n" being
    // computed here at compile time.
    if (m.left().HasResolvedValue() && m.right().op() == machine()->Word64SarShiftOutZeros() && m.right().node()->UseCount() == 1) {
        ...
        if (mright.right().HasResolvedValue()) {
            ...
            if (CanRevertLeftShiftWithRightShift<int64_t>(left, shift)) {
                ...
                // Reducing to 32-bit comparison when possible.
                if ((sign_extended || mright.left().IsChangeUint32ToUint64()) && CanTruncate(static_cast<int64_t>(value))) {
                    NodeProperties::ChangeOp(node, Map64To32Comparison(node->op(), sign_extended));
    ...
    -------------------------------------------------
  */
    if (node->opcode() == IrOpcode::kInt64LessThan || node->opcode() == IrOpcode::kInt64LessThanOrEqual) {
        // Int64LessThan(Int32ToInt64(a), Int64Constant(c))
    ...
    -------------------------------------------------
  */
    if (node->opcode() == IrOpcode::kUint64LessThan || node->opcode() == IrOpcode::kUint64LessThanOrEqual) {
        ...
        if (m.left().IsChangeUint32ToUint64() && m.right().HasResolvedValue()) {
            ...
            if (right_value == static_cast<uint32_t>(right_value)) {
                ...
                NodeProperties::ChangeOp(node, new_op);
                node->ReplaceInput(0, m.left().InputAt(0));
                node->ReplaceInput(1, Uint32Constant(static_cast<uint32_t>(right_value)));
                return Changed(node);
            } else {
                // left < right always
                node->TrimInputCount(0);
        ...
        if (m.right().IsChangeUint32ToUint64() && m.left().HasResolvedValue()) {
            ...
            if (left_value == static_cast<uint32_t>(left_value)) {
                ...
                NodeProperties::ChangeOp(node, new_op);
                node->ReplaceInput(1, m.right().InputAt(0));
                node->ReplaceInput(0, Uint32Constant(static_cast<uint32_t>(left_value)));
                return Changed(node);
            } else {
                // left > right always
                node->TrimInputCount(0);
...
Reduction MachineOperatorReducer::ReduceWord32Shl(Node* node)
{
    ...
    if (m.right().Is(0))
        return Replace(m.left().node()); // x << 0 => x
    if (m.IsFoldable()) { // K << K => K  (K stands for arbitrary constants)
        return ReplaceInt32(base::ShlWithWraparound(m.left().ResolvedValue(), m.right().ResolvedValue()));
    }
    if (m.right().IsInRange(1, 31)) {
        if (m.left().IsWord32Sar() || m.left().IsWord32Shr()) {
            ...
            // signed shifts.
            if (mleft.op() == machine()->Word32SarShiftOutZeros() && mleft.right().IsInRange(1, 31)) {
            ...
            if (mleft.right().Is(m.right().ResolvedValue())) {
                node->ReplaceInput(0, mleft.left().node());
                node->ReplaceInput(1, Uint32Constant(std::numeric_limits<uint32_t>::max() << m.right().ResolvedValue()));
                NodeProperties::ChangeOp(node, machine()->Word32And());
...
Reduction MachineOperatorReducer::ReduceWord64Shl(Node* node)
{
    ...
    if (m.right().Is(0))
        return Replace(m.left().node()); // x << 0 => x
    if (m.IsFoldable()) { // K << K => K  (K stands for arbitrary constants)
        return ReplaceInt64(base::ShlWithWraparound(m.left().ResolvedValue(), m.right().ResolvedValue()));
    }
    if (m.right().IsInRange(1, 63) && (m.left().IsWord64Sar() || m.left().IsWord64Shr())) {
        Int64BinopMatcher mleft(m.left().node());
        ...
        // Since this is used for Smi untagging, we currently only need it for
        // signed shifts.
        if (mleft.op() == machine()->Word64SarShiftOutZeros() && mleft.right().IsInRange(1, 63)) {
            Node* x = mleft.left().node();
        ...
        if (mleft.right().Is(m.right().ResolvedValue())) {
            node->ReplaceInput(0, mleft.left().node());
            node->ReplaceInput(1, Uint64Constant(std::numeric_limits<uint64_t>::max() << m.right().ResolvedValue()));
            NodeProperties::ChangeOp(node, machine()->Word64And());
...
Reduction MachineOperatorReducer::ReduceWord32Shr(Node* node)
{
    ...
    if (m.right().Is(0))
        return Replace(m.left().node()); // x >>> 0 => x
    if (m.IsFoldable()) { // K >>> K => K  (K stands for arbitrary constants)
        return ReplaceInt32(m.left().ResolvedValue() >> (m.right().ResolvedValue() & 31));
    }
...
Reduction MachineOperatorReducer::ReduceWord64Shr(Node* node)
{
    ...
    if (m.right().Is(0))
        return Replace(m.left().node()); // x >>> 0 => x
    if (m.IsFoldable()) { // K >> K => K  (K stands for arbitrary constants)
        return ReplaceInt64(m.left().ResolvedValue() >> (m.right().ResolvedValue() & 63));
    }
    return NoChange();
}
...
template <typename WordNAdapter> Reduction MachineOperatorReducer::ReduceWordNAnd(Node* node)
{
    ...
    if (m.right().Is(1)) {
        ...
        while (left->opcode() == IrOpcode::kTruncateInt64ToInt32 || left->opcode() == IrOpcode::kChangeInt32ToInt64
            || left->opcode() == IrOpcode::kChangeUint32ToUint64) {
            left = left->InputAt(0);
        }
        if ((left->opcode() == IrOpcode::kInt32Add || left->opcode() == IrOpcode::kInt64Add) && left->InputAt(0) == left->InputAt(1)) {
    ...
    if (A::IsWordNAnd(m.left()) && m.right().HasResolvedValue()) {
        ...
        if (mleft.right().HasResolvedValue()) { // (x & K) & K => x & K
            node->ReplaceInput(0, mleft.left().node());
            node->ReplaceInput(1, a.IntNConstant(m.right().ResolvedValue() & mleft.right().ResolvedValue()));
    ...
    if (m.right().IsNegativePowerOf2()) {
        ...
        if (A::IsWordNShl(m.left())) {
            ...
            if (mleft.right().HasResolvedValue() && (mleft.right().ResolvedValue() & (A::WORD_SIZE - 1)) >= base::bits::CountTrailingZeros(mask)) {
                // (x << L) & (-1 << K) => x << L iff L >= K
                return Replace(mleft.node());
            }
        } else if (A::IsIntNAdd(m.left())) {
            typename A::IntNBinopMatcher mleft(m.left().node());
            if (mleft.right().HasResolvedValue() && (mleft.right().ResolvedValue() & mask) == mleft.right().ResolvedValue()) {
                // (x + (K << L)) & (-1 << L) => (x & (-1 << L)) + (K << L)
                node->ReplaceInput(0, a.WordNAnd(mleft.left().node(), m.right().node()));
                node->ReplaceInput(1, mleft.right().node());
            ...
            if (A::IsIntNMul(mleft.left())) {
                ...
                if (mleftleft.right().IsMultipleOf(neg_mask)) {
                    ...
                    node->ReplaceInput(0, a.WordNAnd(mleft.right().node(), m.right().node()));
...
template <typename WordNAdapter> Reduction MachineOperatorReducer::ReduceUintNLessThanOrEqual(Node* node)
{
    using A = WordNAdapter;
    A a(this);

    typename A::UintNBinopMatcher m(node);
    typename A::uintN_t kMaxUIntN = std::numeric_limits<typename A::uintN_t>::max();
    if (m.left().Is(0))
        return ReplaceBool(true); // 0 <= x  =>  true
    if (m.right().Is(kMaxUIntN))
...
struct BitfieldCheck {
    ...
    BitfieldCheck(Node* source, uint32_t mask, uint32_t masked_value, bool truncate_from_64_bit)
        : source(source)
        , mask(mask)
    ...
    static std::optional<BitfieldCheck> Detect(Node* node)
    {
        ...
        if (node->opcode() == IrOpcode::kWord32Equal) {
            ...
            if (eq.left().IsWord32And()) {
                ...
                if (mand.right().HasResolvedValue() && eq.right().HasResolvedValue()) {
                    ...
                    if (mand.left().IsTruncateInt64ToInt32()) {
                        return BitfieldCheck(NodeProperties::GetValueInput(mand.left().node(), 0), mask, masked_value, true);
                    } else {
            ...
            if (node->opcode() == IrOpcode::kTruncateInt64ToInt32) {
                return TryDetectShiftAndMaskOneBit<Word64Adapter>(NodeProperties::GetValueInput(node, 0));
            } else {
...
private:
    template <typename WordNAdapter> static std::optional<BitfieldCheck> TryDetectShiftAndMaskOneBit(Node* node)
    {
        ...
        if (WordNAdapter::IsWordNAnd(NodeMatcher(node))) {
            ...
            if (mand.right().HasResolvedValue() && mand.right().ResolvedValue() == 1) {
                if (WordNAdapter::IsWordNShr(mand.left()) || WordNAdapter::IsWordNSar(mand.left())) {
                    typename WordNAdapter::UintNBinopMatcher shift(mand.left().node());
                    if (shift.right().HasResolvedValue() && shift.right().ResolvedValue() < 32u) {
                        uint32_t mask = 1 << shift.right().ResolvedValue();
                        return BitfieldCheck { shift.left().node(), mask, mask, WordNAdapter::WORD_SIZE == 64 };
                    }
                }
                return BitfieldCheck { mand.left().node(), 1, 1, WordNAdapter::WORD_SIZE == 64 };
            }
        }
        return {};
    }
...
Reduction MachineOperatorReducer::TryMatchWord32Ror(Node* node)
{
    ...
    if (mshl.right().HasResolvedValue() && mshr.right().HasResolvedValue()) {
        ...
        if (node->opcode() == IrOpcode::kWord32Xor && (mshl.right().ResolvedValue() & 31) == 0) {
            return NoChange();
        }
    } else {
        Node* sub = nullptr;
...
template <typename WordNAdapter> Reduction MachineOperatorReducer::ReduceWordNOr(Node* node)
{
    using A = WordNAdapter;
    A a(this);
...
template <typename WordNAdapter> Reduction MachineOperatorReducer::ReduceWordNXor(Node* node)
{
    using A = WordNAdapter;
    A a(this);
    ...
    if (m.LeftEqualsRight())
        return Replace(a.IntNConstant(0)); // x ^ x => 0
    ...
    if (m.right().HasResolvedValue()) {
        ...
        if (m.left().IsTruncateInt64ToInt32()) {
            replacements = ReduceWordEqualForConstantRhs<Word64Adapter, uint32_t>(
                NodeProperties::GetValueInput(m.left().node(), 0), static_cast<uint32_t>(m.right().ResolvedValue()));
        } else {
            replacements = ReduceWordEqualForConstantRhs<Word32Adapter, uint32_t>(m.left().node(), static_cast<uint32_t>(m.right().ResolvedValue()));
        }
        if (replacements) {
        ...
        if (m.left().IsInt32Add() && m.right().IsInt32Constant()) {
            ...
            if (m_add.right().IsInt32Constant()) {
                ...
                node->ReplaceInput(1, Int32Constant(static_cast<uint32_t>(lte_right) - static_cast<uint32_t>(add_right)));
                return Changed(node);
            }
        }
    }
...
Reduction MachineOperatorReducer::ReduceWord64Equal(Node* node)
{
    ...
    if (m.right().HasResolvedValue()) {
        std::optional<std::pair<Node*, uint64_t>> replacements
            = ReduceWordEqualForConstantRhs<Word64Adapter, uint64_t>(m.left().node(), static_cast<uint64_t>(m.right().ResolvedValue()));
        if (replacements) {
            node->ReplaceInput(0, replacements->first);
        ...
        if (m.left().IsInt64Add() && m.right().IsInt64Constant()) {
            ...
            if (m_add.right().IsInt64Constant()) {
                ...
                // No need to consider overflow in this condition (==).
                node->ReplaceInput(0, m_add.left().node());
                node->ReplaceInput(1, Int64Constant(static_cast<uint64_t>(lte_right) - static_cast<uint64_t>(add_right)));
                return Changed(node);
            }
        }
...
Reduction MachineOperatorReducer::ReduceFloat64InsertLowWord32(Node* node)
{
    ...
    if (mlhs.HasResolvedValue() && mrhs.HasResolvedValue()) {
        return ReplaceFloat64(
            base::bit_cast<double>((base::bit_cast<uint64_t>(mlhs.ResolvedValue()) & uint64_t { 0xFFFFFFFF00000000 }) | mrhs.ResolvedValue()));
    }
    return NoChange();
}
...
Reduction MachineOperatorReducer::ReduceFloat64InsertHighWord32(Node* node)
{
    ...
    if (mlhs.HasResolvedValue() && mrhs.HasResolvedValue()) {
        return ReplaceFloat64(base::bit_cast<double>(
            (base::bit_cast<uint64_t>(mlhs.ResolvedValue()) & uint64_t { 0xFFFFFFFF }) | (static_cast<uint64_t>(mrhs.ResolvedValue()) << 32)));
    }
    return NoChange();
}
...
Reduction MachineOperatorReducer::ReduceFloat64Compare(Node* node)
{
    ...
    if (m.IsFoldable()) {
    ...
    } else if ((m.left().IsChangeFloat32ToFloat64() && m.right().IsChangeFloat32ToFloat64())
        || (m.left().IsChangeFloat32ToFloat64() && IsFloat64RepresentableAsFloat32(m.right()))
        || (IsFloat64RepresentableAsFloat32(m.left()) && m.right().IsChangeFloat32ToFloat64())) {
        ...
        node->ReplaceInput(0, m.left().HasResolvedValue() ? Float32Constant(static_cast<float>(m.left().ResolvedValue())) : m.left().InputAt(0));
        node->ReplaceInput(1, m.right().HasResolvedValue() ? Float32Constant(static_cast<float>(m.right().ResolvedValue())) : m.right().InputAt(0));
        return Changed(node);
    }
    return NoChange();
}
...
Node* TrySimplifyCompareForTestBit(Node* cond)
{
    ...
    if (word_equal_left->opcode() != IrOpcode::kWord32And || word_equal_right->opcode() != IrOpcode::kInt32Constant) {
        return nullptr;
    }
...
void MachineOperatorReducer::SwapBranches(Node* node)
{
    ...
    NodeProperties::ChangeOp(node, common()->Branch(NegateBranchHint(BranchHintOf(node->op()))));
}
    ...
    if (auto simplified = TrySimplifyCompareZero(cond)) {
        ...
        if (simplified->is_inverted) {
            ...
            case IrOpcode::kTrapIf: {
                ...
                NodeProperties::ChangeOp(node, common()->TrapUnless(TrapIdOf(node->op()), has_frame_state));
                break;
            }
            case IrOpcode::kTrapUnless: {
                const bool has_frame_state = node->op()->ValueInputCount() > 1;
                NodeProperties::ChangeOp(node, common()->TrapIf(TrapIdOf(node->op()), has_frame_state));
                break;
            }
#endif // V8_ENABLE_WEBASSEMBLY
            case IrOpcode::kDeoptimizeIf: {
                DeoptimizeParameters p = DeoptimizeParametersOf(node->op());
                NodeProperties::ChangeOp(node, common()->DeoptimizeUnless(p.reason(), p.feedback()));
                break;
            }
            case IrOpcode::kDeoptimizeUnless: {
                DeoptimizeParameters p = DeoptimizeParametersOf(node->op());
                NodeProperties::ChangeOp(node, common()->DeoptimizeIf(p.reason(), p.feedback()));
                break;
            }
            default:
...
{
    DCHECK(node->opcode() == IrOpcode::kBranch || node->opcode() == IrOpcode::kDeoptimizeIf || node->opcode() == IrOpcode::kDeoptimizeUnless
    ...
    if (condition.IsTruncateInt64ToInt32()) {
        if (auto replacement = ReduceConditionalN<Word64Adapter>(condition.node())) {
...
template <typename WordNAdapter> std::optional<Node*> MachineOperatorReducer::ReduceConditionalN(Node* node)
{
    NodeMatcher condition(NodeProperties::GetValueInput(node, 0));
    ...
    // reduce to branch(y).
    auto replacements = ReduceWordEqualForConstantRhs<WordNAdapter, uint32_t>(condition.node(), 0);
    if (replacements && replacements->second == 0)
        return replacements->first;
    return {};
}

template <typename WordNAdapter, typename uintN_t, typename intN_t>
std::optional<std::pair<Node*, uintN_t>> MachineOperatorReducer::ReduceWordEqualForConstantRhs(Node* lhs, uintN_t rhs)
{
    if (WordNAdapter::IsWordNAnd(NodeMatcher(lhs))) {
        typename WordNAdapter::UintNBinopMatcher mand(lhs);
        if ((WordNAdapter::IsWordNShr(mand.left()) || WordNAdapter::IsWordNSar(mand.left())) && mand.right().HasResolvedValue()) {
            ...
            if (mshift.right().HasResolvedValue()) {
                ...
                if (shift_bits <= base::bits::CountLeadingZeros(mask) && shift_bits <= base::bits::CountLeadingZeros(rhs)
                    && (std::is_same_v<uintN_t, uint64_t> || mask << shift_bits <= std::numeric_limits<uintN_t>::max())) {
                    Node* new_input = mshift.left().node();
                    uintN_t new_mask = static_cast<uintN_t>(mask << shift_bits);
                    uintN_t new_rhs = rhs << shift_bits;
                    if (std::is_same_v<uintN_t, uint32_t> && WordNAdapter::WORD_SIZE == 64) {
                        ...
                        WordNAdapter a(this);
                        return std::make_pair(a.WordNAnd(new_input, a.UintNConstant(new_mask)), new_rhs);
                    }
                }
            }
        }
    ...
    if (std::is_same_v<intN_t, typename WordNAdapter::intN_t> && WordNAdapter::IsWordNSarShiftOutZeros(lhs->op()) && lhs->UseCount() == 1) {
