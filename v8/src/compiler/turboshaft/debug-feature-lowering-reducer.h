...
template <typename Next> class DebugFeatureLoweringReducer : public Next {
public:
    ...
    OpIndex REDUCE(DebugPrint)(OpIndex input, RegisterRepresentation rep)
    {
        if (isolate_ != nullptr) {
            ...
            case RegisterRepresentation::WordPtr():
                __ CallBuiltin_DebugPrintWordPtr(isolate_, __ NoContextConstant(), input);
                break;
            case RegisterRepresentation::Float64():
                __ CallBuiltin_DebugPrintFloat64(isolate_, __ NoContextConstant(), input);
                break;
            ...
            switch (rep.value()) {
            case RegisterRepresentation::Float64():
                __ template WasmCallBuiltinThroughJumptable<BuiltinCallDescriptor::DebugPrintFloat64>(__ NoContextConstant(), { input });
                break;
            case RegisterRepresentation::WordPtr():
                __ template WasmCallBuiltinThroughJumptable<BuiltinCallDescriptor::DebugPrintWordPtr>(__ NoContextConstant(), { input });
                break;
            default:
    ...
    OpIndex REDUCE(CheckTurboshaftTypeOf)(OpIndex input, RegisterRepresentation rep, Type type, bool successful)
    {
        ...
        UnparkedScopeIfNeeded scope(broker_);
        AllowHandleDereference allow_handle_dereference;
        FATAL("Checking type %s of operation %d:%s failed!", type.ToString().c_str(), input.id(), __ output_graph().Get(input).ToString().c_str());
    }
