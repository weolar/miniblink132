...
template <typename> class VariableReducer;
template <typename> class GraphVisitor;
...
struct BitfieldCheck {
    ...
    BitfieldCheck(OpIndex source, uint32_t mask, uint32_t masked_value, bool truncate_from_64_bit)
        : source(source)
        , mask(mask)
    ...
    static std::optional<BitfieldCheck> Detect(const OperationMatcher& matcher, const Graph& graph, OpIndex index)
    {
        ...
        if (const ComparisonOp* equal = op.TryCast<Opmask::kWord32Equal>()) {
            if (const WordBinopOp* left_and = graph.Get(equal->left()).TryCast<Opmask::kWord32BitwiseAnd>()) {
                uint32_t mask;
                uint32_t masked_value;
                if (matcher.MatchIntegralWord32Constant(left_and->right(), &mask) && matcher.MatchIntegralWord32Constant(equal->right(), &masked_value)) {
                    if ((masked_value & ~mask) != 0)
                        return std::nullopt;
                    if (const ChangeOp* truncate = graph.Get(left_and->left()).TryCast<Opmask::kTruncateWord64ToWord32>()) {
        ...
        } else if (const ChangeOp* truncate = op.TryCast<Opmask::kTruncateWord64ToWord32>()) {
    ...
    {
        if (source != other.source || truncate_from_64_bit != other.truncate_from_64_bit) {
            return std::nullopt;
        }
        ...
        // positions.
        if ((masked_value & overlapping_bits) != (other.masked_value & overlapping_bits)) {
            return std::nullopt;
        }
        return BitfieldCheck { source, mask | other.mask, masked_value | other.masked_value, truncate_from_64_bit };
    }

private:
    template <typename WordType> static std::optional<BitfieldCheck> TryDetectShiftAndMaskOneBit(const OperationMatcher& matcher, OpIndex index)
    {
        constexpr WordRepresentation Rep = V<WordType>::rep;
        // Look for the pattern `(val >> shift) & 1`. The shift may be omitted.
        V<WordType> value;
        uint64_t constant;
        if (matcher.MatchBitwiseAndWithConstant(index, &value, &constant, Rep) && constant == 1) {
            OpIndex input;
            if (int shift_amount; matcher.MatchConstantRightShift(value, &input, Rep, &shift_amount) && shift_amount >= 0 && shift_amount < 32) {
                uint32_t mask = 1 << shift_amount;
                return BitfieldCheck { input, mask, mask, Rep == WordRepresentation::Word64() };
            }
...
template <class Next> class MachineOptimizationReducer : public Next {
public:
    ...
    static_assert(reducer_list_contains<ReducerList, VariableReducer>::value || reducer_list_contains<ReducerList, GraphVisitor>::value);
#endif
    ...
    OpIndex REDUCE(Change)(OpIndex input, ChangeOp::Kind kind, ChangeOp::Assumption assumption, RegisterRepresentation from, RegisterRepresentation to)
    {
        ...
        if (uint64_t value; from.IsWord() && matcher_.MatchIntegralWordConstant(input, WordRepresentation(from), &value)) {
            ...
            case multi(Kind::kBitcast, Rep::Word32(), Rep::Float32()):
                return __ Float32Constant(i::Float32::FromBits(static_cast<uint32_t>(value)));
            ...
            case multi(Kind::kSignedToFloat, Rep::Word32(), Rep::Float64()):
                return __ Float64Constant(static_cast<double>(static_cast<int32_t>(value)));
            case multi(Kind::kSignedToFloat, Rep::Word64(), Rep::Float64()):
                return __ Float64Constant(static_cast<double>(static_cast<int64_t>(value)));
            case multi(Kind::kUnsignedToFloat, Rep::Word32(), Rep::Float64()):
                return __ Float64Constant(static_cast<double>(static_cast<uint32_t>(value)));
        ...
        if (i::Float32 value; from == RegisterRepresentation::Float32() && matcher_.MatchFloat32Constant(input, &value)) {
            if (kind == Kind::kFloatConversion && to == RegisterRepresentation::Float64()) {
        ...
        if (i::Float64 value; from == RegisterRepresentation::Float64() && matcher_.MatchFloat64Constant(input, &value)) {
            if (kind == Kind::kFloatConversion && to == RegisterRepresentation::Float32()) {
            ...
            if (kind == Kind::kSignedFloatTruncateOverflowToMin) {
                ...
                if (to == WordRepresentation::Word64()) {
                    ...
                    if (truncated >= std::numeric_limits<int64_t>::min() && truncated <= kMaxDoubleRepresentableInt64) {
                ...
                if (to == WordRepresentation::Word32()) {
                    ...
                    if (truncated >= std::numeric_limits<int32_t>::min() && truncated <= std::numeric_limits<int32_t>::max()) {
                        result = static_cast<int32_t>(truncated);
                    }
                    return __ Word32Constant(result);
                }
            }
            if (kind == Kind::kJSFloatTruncate && to == WordRepresentation::Word32()) {
        ...
        if (float value; from == RegisterRepresentation::Float32() && matcher_.MatchFloat32Constant(input, &value)) {
            if (kind == Kind::kFloatConversion && to == RegisterRepresentation::Float64()) {
        ...
        if (const ChangeOp* change_op = input_op.TryCast<ChangeOp>()) {
            if (change_op->from == to && change_op->to == from && change_op->IsReversibleBy(kind, signalling_nan_possible)) {
    ...
    V<Float64> REDUCE(BitcastWord32PairToFloat64)(V<Word32> hi_word32, V<Word32> lo_word32)
    {
        ...
        uint32_t lo, hi;
        if (matcher_.MatchIntegralWord32Constant(hi_word32, &hi) && matcher_.MatchIntegralWord32Constant(lo_word32, &lo)) {
            return __ Float64Constant(base::bit_cast<double>(uint64_t { hi } << 32 | uint64_t { lo }));
        }
        return Next::ReduceBitcastWord32PairToFloat64(hi_word32, lo_word32);
    }

    OpIndex REDUCE(TaggedBitcast)(OpIndex input, RegisterRepresentation from, RegisterRepresentation to, TaggedBitcastOp::Kind kind)
    {
        if (ShouldSkipOptimizationStep()) {
        ...
        if (auto* input_bitcast = matcher_.TryCast<TaggedBitcastOp>(input)) {
            if (all_of(input_bitcast->to, from) == RegisterRepresentation::WordPtr() && all_of(input_bitcast->from, to) == RegisterRepresentation::Tagged()) {
                return input_bitcast->input();
            }
        }
        // An Untagged -> Smi -> Untagged sequence can be short-cut.
        if (auto* input_bitcast = matcher_.TryCast<TaggedBitcastOp>(input);
            input_bitcast && to.IsWord() && (kind == TaggedBitcastOp::Kind::kSmi || input_bitcast->kind == TaggedBitcastOp::Kind::kSmi)) {
            if (input_bitcast->from == to)
                return input_bitcast->input();
            ...
            DCHECK(input_bitcast->from == RegisterRepresentation::Word64() && to == RegisterRepresentation::Word32());
        ...
        if (to.IsWord()) {
            if (const ConstantOp* cst = matcher_.TryCast<ConstantOp>(input)) {
                if (cst->kind == ConstantOp::Kind::kWord32 || cst->kind == ConstantOp::Kind::kWord64) {
        ...
        if (const ConstantOp* cst = matcher_.TryCast<ConstantOp>(input)) {
            ...
            if (cst->IsIntegral() && to == RegisterRepresentation::Tagged()) {
                if (Smi::IsValid(cst->integral())) {
                    return __ SmiConstant(i::Tagged<Smi>(static_cast<intptr_t>(cst->integral())));
                }
            }
    ...
    V<Float> REDUCE(FloatUnary)(V<Float> input, FloatUnaryOp::Kind kind, FloatRepresentation rep)
    {
        ...
        if (float k; rep == FloatRepresentation::Float32() && matcher_.MatchFloat32Constant(input, &k)) {
        ...
        } else if (double k; rep == FloatRepresentation::Float64() && matcher_.MatchFloat64Constant(input, &k)) {
            if (std::isnan(k) && !signalling_nan_possible) {
    ...
    V<Word> REDUCE(WordUnary)(V<Word> input, WordUnaryOp::Kind kind, WordRepresentation rep)
    {
        ...
        if (uint32_t k; rep == WordRepresentation::Word32() && matcher_.MatchIntegralWord32Constant(input, &k)) {
            switch (kind) {
        ...
        } else if (uint64_t k; rep == WordRepresentation::Word64() && matcher_.MatchIntegralWord64Constant(input, &k)) {
            switch (kind) {
            case WordUnaryOp::Kind::kReverseBytes:
    ...
    V<Float> REDUCE(FloatBinop)(V<Float> lhs, V<Float> rhs, FloatBinopOp::Kind kind, FloatRepresentation rep)
    {
        ...
        if (FloatBinopOp::IsCommutative(kind) && matcher_.Is<ConstantOp>(lhs) && !matcher_.Is<ConstantOp>(rhs)) {
            return ReduceFloatBinop(rhs, lhs, kind, rep);
        }

        // constant folding
        if (float k1, k2; rep == FloatRepresentation::Float32() && matcher_.MatchFloat32Constant(lhs, &k1) && matcher_.MatchFloat32Constant(rhs, &k2)) {
            switch (kind) {
            case Kind::kAdd:
        ...
        if (double k1, k2; rep == FloatRepresentation::Float64() && matcher_.MatchFloat64Constant(lhs, &k1) && matcher_.MatchFloat64Constant(rhs, &k2)) {
            switch (kind) {
            case Kind::kAdd:
        ...
        // lhs <op> NaN  =>  NaN
        if (matcher_.MatchNaN(rhs) || (matcher_.MatchNaN(lhs) && kind != Kind::kPower)) {
        ...
        if (matcher_.Is<ConstantOp>(rhs)) {
            ...
            if (kind == Kind::kDiv) {
                ...
                if (rep == FloatRepresentation::Float32()) {
                    if (float k; matcher_.MatchFloat32Constant(rhs, &k) && std::isnormal(k) && k != 0 && std::isfinite(k)
                    ...
                    if (double k; matcher_.MatchFloat64Constant(rhs, &k) && std::isnormal(k) && k != 0 && std::isfinite(k)
                        && base::bits::IsPowerOfTwo(base::Double(k).Significand())) {
                        return __ FloatMul(lhs, __ FloatConstant(1.0 / k, rep), rep);
                    }
                }
            ...
            if (kind == Kind::kPower) {
                ...
                if (matcher_.MatchFloat(rhs, 0.5)) {
                    ...
                    IF(UNLIKELY(__ FloatLessThanOrEqual(lhs, __ FloatConstant(-V8_INFINITY, rep), rep)))
                    {
        ...
        if (!signalling_nan_possible && kind == Kind::kSub && matcher_.MatchFloat(lhs, -0.0)) {
            // -0.0 - round_down(-0.0 - y) => round_up(y)
            if (V<Float> a, b, c; FloatUnaryOp::IsSupported(FloatUnaryOp::Kind::kRoundUp, rep) && matcher_.MatchFloatRoundDown(rhs, &a, rep)
                && matcher_.MatchFloatSub(a, &b, &c, rep) && matcher_.MatchFloat(b, -0.0)) {
    ...
    V<Word> REDUCE(WordBinop)(V<Word> left, V<Word> right, WordBinopOp::Kind kind, WordRepresentation rep)
    {
        ...
        DCHECK_EQ(rep, any_of(WordRepresentation::Word32(), WordRepresentation::Word64()));
        ...
        // Place constant on the right for commutative operators.
        if (WordBinopOp::IsCommutative(kind) && matcher_.Is<ConstantOp>(left) && !matcher_.Is<ConstantOp>(right)) {
            return ReduceWordBinop(right, left, kind, rep);
        }
        // constant folding
        if (uint64_t k1, k2; matcher_.MatchIntegralWordConstant(left, rep, &k1) && matcher_.MatchIntegralWordConstant(right, rep, &k2)) {
            switch (kind) {
            case Kind::kAdd:
            ...
            case Kind::kSignedMulOverflownBits:
                return __ WordConstant(is_64 ? base::bits::SignedMulHigh64(static_cast<int64_t>(k1), static_cast<int64_t>(k2))
                                             : base::bits::SignedMulHigh32(static_cast<int32_t>(k1), static_cast<int32_t>(k2)),
                    rep);
            case Kind::kUnsignedMulOverflownBits:
                return __ WordConstant(
                    is_64 ? base::bits::UnsignedMulHigh64(k1, k2) : base::bits::UnsignedMulHigh32(static_cast<uint32_t>(k1), static_cast<uint32_t>(k2)), rep);
            case Kind::kSignedDiv:
                return __ WordConstant(
                    is_64 ? base::bits::SignedDiv64(k1, k2) : base::bits::SignedDiv32(static_cast<int32_t>(k1), static_cast<int32_t>(k2)), rep);
            case Kind::kUnsignedDiv:
                return __ WordConstant(
                    is_64 ? base::bits::UnsignedDiv64(k1, k2) : base::bits::UnsignedDiv32(static_cast<uint32_t>(k1), static_cast<uint32_t>(k2)), rep);
            case Kind::kSignedMod:
                return __ WordConstant(
                    is_64 ? base::bits::SignedMod64(k1, k2) : base::bits::SignedMod32(static_cast<int32_t>(k1), static_cast<int32_t>(k2)), rep);
            case Kind::kUnsignedMod:
                return __ WordConstant(
                    is_64 ? base::bits::UnsignedMod64(k1, k2) : base::bits::UnsignedMod32(static_cast<uint32_t>(k1), static_cast<uint32_t>(k2)), rep);
            }
        }

        if (kind == WordBinopOp::Kind::kBitwiseAnd && rep == WordRepresentation::Word32()) {
            if (auto right_bitfield = detail::BitfieldCheck::Detect(matcher_, __ output_graph(), right)) {
                if (auto left_bitfield = detail::BitfieldCheck::Detect(matcher_, __ output_graph(), left)) {
                    if (auto combined_bitfield = left_bitfield->TryCombine(*right_bitfield)) {
                        ...
                        return __ Word32Equal(__ Word32BitwiseAnd(source, combined_bitfield->mask), combined_bitfield->masked_value);
                    }
                }
            }
        }

        if (uint64_t right_value; matcher_.MatchIntegralWordConstant(right, rep, &right_value)) {
            // TODO(jkummerow): computing {right_value_signed} could probably be
            // handled by the 4th argument to {MatchIntegralWordConstant}.
            int64_t right_value_signed = is_64 ? static_cast<int64_t>(right_value) : int64_t { static_cast<int32_t>(right_value) };
            // (a <op> k1) <op> k2  =>  a <op> (k1 <op> k2)
            if (V<Word> a, k1; WordBinopOp::IsAssociative(kind) && matcher_.MatchWordBinop(left, &a, &k1, kind, rep) && matcher_.Is<ConstantOp>(k1)) {
                V<Word> k2 = right;
                ...
                if (matcher_.Get(left).saturated_use_count.IsZero()) {
                    return ReduceWordBinop(a, ReduceWordBinop(k1, k2, kind, rep), kind, rep);
                }
            }
            switch (kind) {
            case Kind::kSub:
                // left - k  =>  left + -k
                return ReduceWordBinop(left, __ WordConstant(-right_value, rep), Kind::kAdd, rep);
            case Kind::kAdd:
                // left + 0  =>  left
                if (right_value == 0) {
                    return left;
            ...
            case Kind::kBitwiseXor:
                ...
                // (x ^ -1) ^ -1  =>  x
                {
                    V<Word> x, y;
                    int64_t k;
                    if (right_value_signed == -1 && matcher_.MatchBitwiseAnd(left, &x, &y, rep) && matcher_.MatchIntegralWordConstant(y, rep, &k) && k == -1) {
                        return x;
                    }
                }
                break;
            case Kind::kBitwiseOr:
                ...
                // This case can be constructed by UpdateWord and UpdateWord32 in CSA.
                {
                    ...
                    if (matcher_.MatchBitwiseAnd(left, &x, &y, rep) && matcher_.MatchIntegralWordConstant(y, rep, &k1) && (k1 | k2) == rep.MaxUnsignedValue()) {
            ...
            case Kind::kMul:
                ...
                if (base::bits::IsPowerOfTwo(right_value)) {
                    return __ ShiftLeft(left, base::bits::WhichPowerOfTwo(right_value), rep);
                }
                break;
            case Kind::kBitwiseAnd:
                // left & -1 => left
                ...
                if (right_value == 1) {
                    // (x + x) & 1  =>  0
                    V<Word> left_ignore_extensions = IsWord32ConvertedToWord64(left) ? UndoWord32ToWord64Conversion(left) : left;
                    if (V<Word> a, b; matcher_.MatchWordAdd(left_ignore_extensions, &a, &b, WordRepresentation::Word32()) && a == b) {
                        return __ WordConstant(0, rep);
                    }
                ...
                if (IsNegativePowerOfTwo(right_value_signed)) {
                    ...
                    V<Word> x, y;
                    {
                        ...
                        //   (x << L) & (-1 << K)
                        // => x << L               iff L >= K
                        if (matcher_.MatchConstantLeftShift(left, &x, rep, &L) && L >= K) {
                            return left;
                        }
                    }
                    ...
                    if (matcher_.MatchWordAdd(left, &x, &y, rep)) {
                        ...
                        //    (x              + (M << K)) & (-1 << K)
                        // => (x & (-1 << K)) + (M << K)
                        if (matcher_.MatchIntegralWordConstant(y, rep, &L) && (L & mask) == L) {
                            return __ WordAdd(__ WordBitwiseAnd(x, right, rep), __ WordConstant(L, rep), rep);
                        }

                        //   (x1 * (M << K) + y) & (-1 << K)
                        // => x1 * (M << K) + (y & (-1 << K))
                        V<Word> x1, x2, y1, y2;
                        if (matcher_.MatchWordMul(x, &x1, &x2, rep) && matcher_.MatchIntegralWordConstant(x2, rep, &L) && (L & mask) == L) {
                            return __ WordAdd(x, __ WordBitwiseAnd(y, right, rep), rep);
                        }
                        // Same as above with swapped order:
                        //    (x              + y1 * (M << K)) & (-1 << K)
                        // => (x & (-1 << K)) + y1 * (M << K)
                        if (matcher_.MatchWordMul(y, &y1, &y2, rep) && matcher_.MatchIntegralWordConstant(y2, rep, &L) && (L & mask) == L) {
                        ...
                        // => (x1 << K) + (y & (-1 << K))
                        int K2;
                        if (matcher_.MatchConstantLeftShift(x, &x1, rep, &K2) && K2 == K) {
                            return __ WordAdd(x, __ WordBitwiseAnd(y, right, rep), rep);
                        }
                        // Same as above with swapped order:
                        //    (x +              (y1 << K)) & (-1 << K)
                        // => (x & (-1 << K)) + (y1 << K)
                        if (matcher_.MatchConstantLeftShift(y, &y1, rep, &K2) && K2 == K) {
                            return __ WordAdd(__ WordBitwiseAnd(x, right, rep), y, rep);
                        }
            ...
            case WordBinopOp::Kind::kSignedMod:
                ...
                if (base::bits::IsPowerOfTwo(right_value_signed)) {
                    ...
                    V<Word> m = __ ShiftRightLogical(__ ShiftRightArithmetic(left, bits - 1, rep), bits - n, rep);
                    return __ WordSub(__ WordBitwiseAnd(__ WordAdd(left, m, rep), __ WordConstant(right_value_signed - 1, rep), rep), m, rep);
                }
                // The `IntDiv` with a constant right-hand side will be turned into a
                // multiplication, avoiding the expensive integer division.
                return __ WordSub(left, __ WordMul(__ IntDiv(left, right, rep), right, rep), rep);
            case WordBinopOp::Kind::kUnsignedMod:
                ...
                // x % 2^n => x & (2^n - 1)
                if (base::bits::IsPowerOfTwo(right_value)) {
                    return __ WordBitwiseAnd(left, __ WordConstant(right_value - 1, rep), rep);
                }
                // The `UintDiv` with a constant right-hand side will be turned into a
                // multiplication, avoiding the expensive integer division.
                return __ WordSub(left, __ WordMul(right, __ UintDiv(left, right, rep), rep), rep);
        ...
        if (kind == Kind::kAdd) {
            V<Word> x, y, zero;
            // (0 - x) + y => y - x
            if (matcher_.MatchWordSub(left, &zero, &x, rep) && matcher_.MatchZero(zero)) {
                y = right;
                return __ WordSub(y, x, rep);
            }
            // x + (0 - y) => x - y
            if (matcher_.MatchWordSub(right, &zero, &y, rep) && matcher_.MatchZero(zero)) {
                x = left;
        ...
        // 0 / right  =>  0
        // 0 % right  =>  0
        if (matcher_.MatchZero(left) && kind == any_of(Kind::kSignedDiv, Kind::kUnsignedDiv, Kind::kUnsignedMod, Kind::kSignedMod)) {
            return __ WordConstant(0, rep);
        }
    ...
    bool TryMatchHeapObject(V<Any> idx, int depth = 0)
    {
        ...
        if (matcher_.MatchHeapConstant(idx))
            return true;
        if (matcher_.Is<AllocateOp>(idx))
            return true;
        if (matcher_.Is<Opmask::kTaggedBitcastHeapObject>(idx))
            return true;

        // A Phi whose inputs are all HeapObject is itself a HeapObject.
        if (const PhiOp* phi = matcher_.TryCast<Opmask::kTaggedPhi>(idx)) {
            return base::all_of(phi->inputs(), [depth, this](V<Any> input) { return TryMatchHeapObject(input, depth + 1); });
        }
    ...
    std::optional<V<Word>> TryReduceToRor(V<Word> left, V<Word> right, WordBinopOp::Kind kind, WordRepresentation rep)
    {
        ...
        if (!(kind == any_of(WordBinopOp::Kind::kBitwiseOr, WordBinopOp::Kind::kBitwiseXor))) {
            return {};
        }
        ...
        if (high->kind != ShiftOp::Kind::kShiftLeft || low->kind != ShiftOp::Kind::kShiftRightLogical) {
            return {};
        }
        V<Word> x = high->left();
        if (low->left() != x)
        ...
        uint64_t k;
        if (V<Word> a, b;
            matcher_.MatchWordSub(high->right(), &a, &b, rep) && matcher_.MatchIntegralWordConstant(a, rep, &k) && b == low->right() && k == rep.bit_width()) {
            amount = b;
        } else if (V<Word> a, b; matcher_.MatchWordSub(low->right(), &a, &b, rep) && a == high->right() && matcher_.MatchIntegralWordConstant(b, rep, &k)
                   && k == rep.bit_width()) {
            amount = low->right();
        } else if (uint64_t k1, k2; matcher_.MatchIntegralWordConstant(high->right(), rep, &k1) && matcher_.MatchIntegralWordConstant(low->right(), rep, &k2)
    ...
    V<Tuple<Word, Word32>> REDUCE(OverflowCheckedBinop)(V<Word> left, V<Word> right, OverflowCheckedBinopOp::Kind kind, WordRepresentation rep)
    {
        ...
        if (OverflowCheckedBinopOp::IsCommutative(kind) && matcher_.Is<ConstantOp>(left) && !matcher_.Is<ConstantOp>(right)) {
        ...
        // constant folding
        if (rep == WordRepresentation::Word32()) {
            if (int32_t k1, k2; matcher_.MatchIntegralWord32Constant(left, &k1) && matcher_.MatchIntegralWord32Constant(right, &k2)) {
                bool overflow;
                int32_t res;
                switch (kind) {
            ...
            if (int64_t k1, k2; matcher_.MatchIntegralWord64Constant(left, &k1) && matcher_.MatchIntegralWord64Constant(right, &k2)) {
                bool overflow;
                int64_t res;
                switch (kind) {
                case OverflowCheckedBinopOp::Kind::kSignedAdd:
        ...
        // left + 0  =>  (left, false)
        // left - 0  =>  (left, false)
        if (kind == any_of(Kind::kSignedAdd, Kind::kSignedSub) && matcher_.MatchZero(right)) {
        ...
        if (kind == Kind::kSignedAdd && left == right) {
            uint16_t amount;
            if (V<Word32> x; matcher_.MatchConstantShiftRightArithmeticShiftOutZeros(left, &x, WordRepresentation::Word32(), &amount) && amount == 1) {
    ...
    V<Word32> REDUCE(Comparison)(V<Any> left, V<Any> right, ComparisonOp::Kind kind, RegisterRepresentation rep)
    {
        ...
        if (left == right && !(rep == any_of(RegisterRepresentation::Float32(), RegisterRepresentation::Float64()))
            && kind == any_of(Kind::kSignedLessThanOrEqual, Kind::kUnsignedLessThanOrEqual)) {
            switch (kind) {
            case Kind::kEqual:
                UNREACHABLE();
        ...
        if (matcher_.Is<ConstantOp>(right) && matcher_.Is<ConstantOp>(left)) {
            ...
            case RegisterRepresentation::Word64(): {
                if (kind == any_of(Kind::kSignedLessThan, Kind::kSignedLessThanOrEqual)) {
                    if (int64_t k1, k2; matcher_.MatchIntegralWordConstant(left, WordRepresentation(rep), &k1)
                        && matcher_.MatchIntegralWordConstant(right, WordRepresentation(rep), &k2)) {
                        switch (kind) {
                    ...
                    if (uint64_t k1, k2; matcher_.MatchIntegralWordConstant(left, WordRepresentation(rep), &k1)
                        && matcher_.MatchIntegralWordConstant(right, WordRepresentation(rep), &k2)) {
                        switch (kind) {
                        case ComparisonOp::Kind::kUnsignedLessThan:
                            return __ Word32Constant(k1 < k2);
            ...
            case RegisterRepresentation::Float32(): {
                if (float k1, k2; matcher_.MatchFloat32Constant(left, &k1) && matcher_.MatchFloat32Constant(right, &k2)) {
                    switch (kind) {
                    case ComparisonOp::Kind::kSignedLessThan:
                        return __ Word32Constant(k1 < k2);
                    case ComparisonOp::Kind::kSignedLessThanOrEqual:
            ...
            case RegisterRepresentation::Float64(): {
                if (double k1, k2; matcher_.MatchFloat64Constant(left, &k1) && matcher_.MatchFloat64Constant(right, &k2)) {
                    switch (kind) {
                    case ComparisonOp::Kind::kSignedLessThan:
                        return __ Word32Constant(k1 < k2);
                    case ComparisonOp::Kind::kSignedLessThanOrEqual:
        ...
        if (rep == RegisterRepresentation::Float64() && IsFloat32ConvertedToFloat64(left) && IsFloat32ConvertedToFloat64(right)) {
            return __ Comparison(UndoFloat32ToFloat64Conversion(left), UndoFloat32ToFloat64Conversion(right), kind, RegisterRepresentation::Float32());
        }
        if (rep.IsWord()) {
            WordRepresentation rep_w { rep };
            if (kind == Kind::kUnsignedLessThanOrEqual) {
                // 0 <= x  =>  true
                if (uint64_t k; matcher_.MatchIntegralWordConstant(left, rep_w, &k) && k == 0) {
                    return __ Word32Constant(1);
                }
                // x <= MaxUint  =>  true
                if (uint64_t k; matcher_.MatchIntegralWordConstant(right, rep_w, &k) && k == rep.MaxUnsignedValue()) {
                    return __ Word32Constant(1);
                }
                // x <= 0  =>  x == 0
                if (uint64_t k; matcher_.MatchIntegralWordConstant(right, rep_w, &k) && k == 0) {
            ...
            if (kind == Kind::kUnsignedLessThan) {
                ...
                // MaxUint < x  =>  true
                if (uint64_t k; matcher_.MatchIntegralWordConstant(left, rep_w, &k) && k == rep.MaxUnsignedValue()) {
                    return __ Word32Constant(0);
                }
            }
            {
                ...
                V<Word> x, y;
                uint16_t k1, k2;
                if (matcher_.MatchConstantShiftRightArithmeticShiftOutZeros(left, &x, rep_w, &k1)
                    && matcher_.MatchConstantShiftRightArithmeticShiftOutZeros(right, &y, rep_w, &k2) && k1 == k2) {
            ...
            }
            {
                ...
                uint16_t k1;
                int64_t k2;
                if (matcher_.MatchConstantShiftRightArithmeticShiftOutZeros(left, &x, rep_w, &k1) && matcher_.MatchIntegralWordConstant(right, rep_w, &k2)
                    && CountLeadingSignBits(k2, rep_w) > k1) {
                    if (matcher_.Get(left).saturated_use_count.IsZero()) {
                        return __ Comparison(x, __ WordConstant(base::bits::Unsigned(k2) << k1, rep_w), kind, rep_w);
                    } else if constexpr (reducer_list_contains<ReducerList, ValueNumberingReducer>::value) {
                        // If the shift has uses, we only apply the transformation if the
                        // result would be GVNed away.
                        OpIndex rhs = __ WordConstant(base::bits::Unsigned(k2) << k1, rep_w);
                        ...
                        ComparisonOp* cmp = CreateOperation<ComparisonOp>(storage, x, rhs, kind, rep_w);
                        if (__ WillGVNOp(*cmp)) {
                ...
                // Only perform the transformation if the shift is not used yet, to
                // avoid keeping both the shift and x alive.
                if (matcher_.MatchConstantShiftRightArithmeticShiftOutZeros(right, &x, rep_w, &k1) && matcher_.MatchIntegralWordConstant(left, rep_w, &k2)
                    && CountLeadingSignBits(k2, rep_w) > k1) {
                    if (matcher_.Get(right).saturated_use_count.IsZero()) {
                        return __ Comparison(__ WordConstant(base::bits::Unsigned(k2) << k1, rep_w), x, kind, rep_w);
                    } else if constexpr (reducer_list_contains<ReducerList, ValueNumberingReducer>::value) {
                        // If the shift has uses, we only apply the transformation if the
                        // result would be GVNed away.
                        OpIndex lhs = __ WordConstant(base::bits::Unsigned(k2) << k1, rep_w);
                        static_assert(ComparisonOp::input_count == 2);
                        static_assert(sizeof(ComparisonOp) == 8);
                        base::SmallVector<OperationStorageSlot, 32> storage;
                        ComparisonOp* cmp = CreateOperation<ComparisonOp>(storage, lhs, x, kind, rep_w);
                        if (__ WillGVNOp(*cmp)) {
            ...
            if (rep_w == WordRepresentation::Word64()) {
                ...
                if (IsWord32ConvertedToWord64(left, &left_sign_extended) && IsWord32ConvertedToWord64(right, &right_sign_extended)) {
                    if (left_sign_extended != true && right_sign_extended != true) {
                        ...
                        auto SetSigned = [](Kind kind, bool is_signed) {
                            ...
                            case Kind::kSignedLessThanOrEqual:
                            case Kind::kUnsignedLessThanOrEqual:
                                return is_signed ? Kind::kSignedLessThanOrEqual : Kind::kUnsignedLessThanOrEqual;
                            case Kind::kEqual:
                                UNREACHABLE();
                            }
                        };
                        return __ Comparison(UndoWord32ToWord64Conversion(V<Word64>::Cast(left)), UndoWord32ToWord64Conversion(V<Word64>::Cast(right)),
                            SetSigned(kind, false), WordRepresentation::Word32());
                    } else if (left_sign_extended != false && right_sign_extended != false) {
                        // Both sides were sign-extended, this preserves both signed and
                        // unsigned comparisons.
                        return __ Comparison(UndoWord32ToWord64Conversion(V<Word64>::Cast(left)), UndoWord32ToWord64Conversion(V<Word64>::Cast(right)), kind,
    ...
    OpIndex REDUCE(Shift)(OpIndex left, OpIndex right, ShiftOp::Kind kind, WordRepresentation rep)
    {
        if (ShouldSkipOptimizationStep()) {
        ...
        if (matcher_.MatchIntegralWordConstant(left, rep, &c_unsigned, &c_signed)) {
            if (uint32_t amount; matcher_.MatchIntegralWord32Constant(right, &amount)) {
                ...
                case Kind::kShiftRightArithmetic:
                    ...
                    case WordRepresentation::Word32():
                        return __ Word32Constant(static_cast<int32_t>(c_signed) >> amount);
                ...
                case Kind::kShiftRightLogical:
                    switch (rep.value()) {
                    case WordRepresentation::Word32():
                        return __ Word32Constant(static_cast<uint32_t>(c_unsigned) >> amount);
                    case WordRepresentation::Word64():
                ...
                case Kind::kRotateRight:
                    switch (rep.value()) {
                    case WordRepresentation::Word32():
                        return __ Word32Constant(base::bits::RotateRight32(static_cast<uint32_t>(c_unsigned), amount));
                    case WordRepresentation::Word64():
                        return __ Word64Constant(base::bits::RotateRight64(c_unsigned, amount));
                    }
                case Kind::kRotateLeft:
                    switch (rep.value()) {
                    case WordRepresentation::Word32():
                        return __ Word32Constant(base::bits::RotateLeft32(static_cast<uint32_t>(c_unsigned), amount));
                    case WordRepresentation::Word64():
                        return __ Word64Constant(base::bits::RotateLeft64(c_unsigned, amount));
                    }
                }
            }
        }
        if (int32_t amount; matcher_.MatchIntegralWord32Constant(right, &amount) && 0 <= amount && amount < rep.bit_width()) {
            if (amount == 0) {
                return left;
            }
            if (kind == Kind::kShiftLeft) {
                ...
                int k;
                OpIndex x;
                if (matcher_.MatchConstantShift(left, &x, Kind::kShiftRightArithmeticShiftOutZeros, rep, &k)) {
                    int32_t l = amount;
                    if (k == l) {
                        return x;
                    } else if (k > l) {
                        return __ ShiftRightArithmeticShiftOutZeros(x, __ Word32Constant(k - l), rep);
                    } else if (k < l) {
                ...
                // (x >>> K) << K => x & ~(2^K - 1)
                // (x >> K) << K => x & ~(2^K - 1)
                if (matcher_.MatchConstantRightShift(left, &x, rep, &k) && k == amount) {
                    return __ WordBitwiseAnd(x, __ WordConstant(rep.MaxUnsignedValue() << k, rep), rep);
                }
            }
            if (kind == any_of(Kind::kShiftRightArithmetic, Kind::kShiftRightArithmeticShiftOutZeros)) {
                OpIndex x;
                int left_shift_amount;
                // (x << k) >> k
                if (matcher_.MatchConstantShift(left, &x, ShiftOp::Kind::kShiftLeft, rep, &left_shift_amount) && amount == left_shift_amount) {
            ...
            if (rep == WordRepresentation::Word32() && SupportedOperations::word32_shift_is_safe()) {
                ...
                if (V<Word32> a, b; matcher_.MatchBitwiseAnd(right, &a, &b, WordRepresentation::Word32())) {
#if defined(__clang__)
                    static_assert(0x1f == WordRepresentation::Word32().bit_width() - 1);
#endif
                    if (uint32_t b_value; matcher_.MatchIntegralWord32Constant(b, &b_value) && b_value == 0x1f) {
    ...
    OpIndex REDUCE(Branch)(OpIndex condition, Block* if_true, Block* if_false, BranchHint hint)
    {
        ...
        bool negated = false;
        if (std::optional<OpIndex> new_condition = ReduceBranchCondition(condition, &negated)) {
            if (negated) {
    ...
    V<None> REDUCE(DeoptimizeIf)(V<Word32> condition, V<FrameState> frame_state, bool negated, const DeoptimizeParameters* parameters)
    {
        ...
        if (std::optional<V<Word32>> new_condition = ReduceBranchCondition(condition, &negated)) {
            return __ ReduceDeoptimizeIf(new_condition.value(), frame_state, negated, parameters);
        } else {
            return Next::ReduceDeoptimizeIf(condition, frame_state, negated, parameters);
        }
    }
    ...
    V<None> REDUCE(TrapIf)(V<Word32> condition, OptionalV<FrameState> frame_state, bool negated, TrapId trap_id)
    {
        ...
        if (std::optional<V<Word32>> new_condition = ReduceBranchCondition(condition, &negated)) {
            return __ ReduceTrapIf(new_condition.value(), frame_state, negated, trap_id);
        } else {
            goto no_change;
        }
    }
    ...
    OpIndex REDUCE(Store)(OpIndex base_idx, OptionalOpIndex index, OpIndex value, StoreOp::Kind kind, MemoryRepresentation stored_rep,
        WriteBarrierKind write_barrier, int32_t offset, uint8_t element_scale, bool maybe_initializing_or_transitioning,
        IndirectPointerTag maybe_indirect_pointer_tag)
    {
        LABEL_BLOCK(no_change)
        {
            return Next::ReduceStore(base_idx, index, value, kind, stored_rep, write_barrier, offset, element_scale, maybe_initializing_or_transitioning,
        ...
        index = ReduceMemoryIndex(index.value_or_invalid(), &offset, &element_scale, kind.tagged_base);
        switch (stored_rep) {
        case MemoryRepresentation::Uint8():
        case MemoryRepresentation::Int8():
            value = ReduceWithTruncation(value, std::numeric_limits<uint8_t>::max(), WordRepresentation::Word32());
            break;
        case MemoryRepresentation::Uint16():
        case MemoryRepresentation::Int16():
            value = ReduceWithTruncation(value, std::numeric_limits<uint16_t>::max(), WordRepresentation::Word32());
            break;
        case MemoryRepresentation::Uint32():
        case MemoryRepresentation::Int32():
            value = ReduceWithTruncation(value, std::numeric_limits<uint32_t>::max(), WordRepresentation::Word32());
            break;
        default:
            break;
        }
        ...
        if (!index.valid() && matcher_.Is<Opmask::kWord64Add>(base_idx)) {
            ...
            __ Store(base_idx, index, value, kind, stored_rep, write_barrier, offset, element_scale, maybe_initializing_or_transitioning,
                maybe_indirect_pointer_tag);
            return OpIndex::Invalid();
        }

        return Next::ReduceStore(
            base_idx, index, value, kind, stored_rep, write_barrier, offset, element_scale, maybe_initializing_or_transitioning, maybe_indirect_pointer_tag);
    }

    OpIndex REDUCE(Load)(OpIndex base_idx, OptionalOpIndex index, LoadOp::Kind kind, MemoryRepresentation loaded_rep, RegisterRepresentation result_rep,
        int32_t offset, uint8_t element_scale)
    {
        LABEL_BLOCK(no_change)
        {
            return Next::ReduceLoad(base_idx, index, kind, loaded_rep, result_rep, offset, element_scale);
        }
        if (ShouldSkipOptimizationStep())
            goto no_change;
#if V8_TARGET_ARCH_32_BIT
        ...
        while (true) {
            ...
            if (!kind.tagged_base && !index.valid()) {
                if (V<WordPtr> left, right; matcher_.MatchWordAdd(base_idx, &left, &right, WordRepresentation::WordPtr())
                    && TryAdjustOffset(&offset, matcher_.Get(right), element_scale, kind.tagged_base)) {
                    base_idx = left;
                    continue;
                }
            }
        ...
        if (!index.valid() && matcher_.Is<ConstantOp>(base_idx)) {
            ...
            if (base.kind == any_of(ConstantOp::Kind::kHeapObject, ConstantOp::Kind::kCompressedHeapObject)) {
                if (offset == HeapObject::kMapOffset) {
                    ...
                    DCHECK_IMPLIES(__ data()->pipeline_kind() != TurboshaftPipelineKind::kCSA, broker != nullptr);
                    if (broker != nullptr) {
        ...
        if (!index.valid() && matcher_.Is<Opmask::kWord64Add>(base_idx)) {
            ...
            return __ Load(base_idx, index, kind, loaded_rep, result_rep, offset, element_scale);
        }
    ...
    V<Any> REDUCE(Simd128ExtractLane)(V<Simd128> input, Simd128ExtractLaneOp::Kind kind, uint8_t lane)
    {
        ...
        auto MatchUnaryShuffle = [this](V<Simd128> maybe_shuffle) -> const Simd128ShuffleOp* {
            if (const Simd128ShuffleOp* shuffle = matcher_.TryCast<Simd128ShuffleOp>(maybe_shuffle)) {
        ...
        auto MatchBinop = [this](V<Simd128> maybe_binop, Simd128BinopOp::Kind required_binop_kind) -> const Simd128BinopOp* {
            if (const Simd128BinopOp* binop = matcher_.TryCast<Simd128BinopOp>(maybe_binop)) {
        ...
        while (!binops.empty()) {
            ...
            for (unsigned i = 0; i < 2; ++i) {
                ...
                if (const Simd128ShuffleOp* shuffle = MatchUnaryShuffle(operand)) {
                    ...
                    if (const Simd128BinopOp* other_binop = MatchBinop(shuffle_in, binop->kind)) {
        ...
        case MachineRepresentation::kWord8: {
            if (shuffles.size() == 4) {
                ...
                if (wasm::SimdShuffle::TryMatch8x16UpperToLowerReduce(shuffle1, shuffle2, shuffle3, shuffle4)) {
                    V<Simd128> reduce = __ Simd128Reduce(reduce_input, Simd128ReduceOp::Kind::kI8x16AddReduce);
        ...
        case MachineRepresentation::kWord16: {
            if (shuffles.size() == 3) {
                ...
                const uint8_t* shuffle2 = shuffles[1]->shuffle;
                const uint8_t* shuffle3 = shuffles[2]->shuffle;
                if (wasm::SimdShuffle::TryMatch16x8UpperToLowerReduce(shuffle1, shuffle2, shuffle3)) {
                    V<Simd128> reduce = __ Simd128Reduce(reduce_input, Simd128ReduceOp::Kind::kI16x8AddReduce);
                    return __ Simd128ExtractLane(reduce, kind, 0);
                }
            }
            break;
        ...
        case MachineRepresentation::kWord32: {
            if (shuffles.size() == 2) {
                const uint8_t* shuffle1 = shuffles[0]->shuffle;
                const uint8_t* shuffle2 = shuffles[1]->shuffle;
                if (wasm::SimdShuffle::TryMatch32x4UpperToLowerReduce(shuffle1, shuffle2)) {
                    V<Simd128> reduce = __ Simd128Reduce(reduce_input, Simd128ReduceOp::Kind::kI32x4AddReduce);
                    return __ Simd128ExtractLane(reduce, kind, 0);
                }
            }
            break;
        ...
        case MachineRepresentation::kFloat32: {
            if (shuffles.size() == 2) {
                const uint8_t* shuffle1 = shuffles[0]->shuffle;
                const uint8_t* shuffle2 = shuffles[1]->shuffle;
                if (wasm::SimdShuffle::TryMatch32x4PairwiseReduce(shuffle1, shuffle2)) {
                    V<Simd128> reduce = __ Simd128Reduce(reduce_input, Simd128ReduceOp::Kind::kF32x4AddReduce);
                    return __ Simd128ExtractLane(reduce, kind, 0);
                }
            }
            break;
        ...
        case MachineRepresentation::kFloat64: {
            if (shuffles.size() == 1) {
                ...
                if (wasm::SimdShuffle::TryMatch64x2Shuffle(shuffles[0]->shuffle, shuffle64x2) && wasm::SimdShuffle::TryMatch64x2Reduce(shuffle64x2)) {
                    V<Simd128> reduce = rep == MachineRepresentation::kWord64 ? __ Simd128Reduce(reduce_input, Simd128ReduceOp::Kind::kI64x2AddReduce)
                                                                              : __ Simd128Reduce(reduce_input, Simd128ReduceOp::Kind::kF64x2AddReduce);
                    return __ Simd128ExtractLane(reduce, kind, 0);
                }
            }
            break;
...
private:
        ...
        if (matcher_.Is<ConstantOp>(right)) {
            if (matcher_.Is<ConstantOp>(left)) {
                ...
                case RegisterRepresentation::Tagged(): {
                    if (Handle<HeapObject> o1, o2; matcher_.MatchHeapConstant(left, &o1) && matcher_.MatchHeapConstant(right, &o2)) {
            ...
            if (rep.IsWord()) {
                WordRepresentation rep_w { rep };
                // x - y == 0  =>  x == y
                if (V<Word> x, y; matcher_.MatchWordSub(left, &x, &y, rep_w) && matcher_.MatchZero(right)) {
                ...
                }
                {
                    ...
                    int shift_amount;
                    uint64_t mask, k;
                    if (matcher_.MatchBitwiseAnd(left, &shift, &mask_op, rep_w) && matcher_.MatchConstantRightShift(shift, &x, rep_w, &shift_amount)
                        && matcher_.MatchIntegralWordConstant(mask_op, rep_w, &mask) && matcher_.MatchIntegralWordConstant(right, rep_w, &k)
                        && mask <= rep.MaxUnsignedValue() >> shift_amount && k <= rep.MaxUnsignedValue() >> shift_amount) {
                        return ReduceCompareEqual(
                            __ WordBitwiseAnd(x, __ WordConstant(mask << shift_amount, rep_w), rep_w), __ WordConstant(k << shift_amount, rep_w), rep_w);
                    }
                }
                {
                    // (x >> k1) == k2  =>  x == (k2 << k1)  if shifts reversible
                    ...
                    int64_t k2;
                    if (matcher_.MatchConstantShiftRightArithmeticShiftOutZeros(left, &x, rep_w, &k1) && matcher_.MatchIntegralWordConstant(right, rep_w, &k2)
                        && CountLeadingSignBits(k2, rep_w) > k1 && matcher_.Get(left).saturated_use_count.IsZero()) {
                        return __ Equal(x, __ WordConstant(base::bits::Unsigned(k2) << k1, rep_w), rep_w);
                    }
                }
                ...
                if (rep_w == WordRepresentation::Word64()) {
                    ...
                    if (IsWord32ConvertedToWord64(left, &left_sign_extended) && IsWord32ConvertedToWord64(right, &right_sign_extended)) {
                        if (left_sign_extended == right_sign_extended) {
                            return __ Equal(UndoWord32ToWord64Conversion(V<Word64>::Cast(left)), UndoWord32ToWord64Conversion(V<Word64>::Cast(right)),
                                WordRepresentation::Word32());
                        }
                    }
                }
    ...
    {
        if (!maybe_constant.Is<ConstantOp>())
            return false;
        const ConstantOp& constant = maybe_constant.Cast<ConstantOp>();
        if (constant.rep != WordRepresentation::WordPtr() || !constant.IsIntegral()) {
        ...
        if (element_scale > 31)
            return false;
        if (diff <= (std::numeric_limits<int32_t>::max() >> element_scale) && diff >= (std::numeric_limits<int32_t>::min() >> element_scale)
            && !base::bits::SignedAddOverflow32(*offset, static_cast<int32_t>(base::bits::Unsigned(diff) << element_scale), &new_offset)
    ...
    bool TryAdjustIndex(int32_t offset, OpIndex* index, const Operation& maybe_constant, uint8_t element_scale)
    {
        ...
        int64_t diff = constant.signed_integral();
        int64_t new_index;
        if (!base::bits::SignedAddOverflow64(offset, diff << element_scale, &new_index)) {
    ...
    {
        uint64_t diff;
        if (!matcher_.MatchIntegralWordConstant(maybe_constant, WordRepresentation::WordPtr(), &diff)) {
            return false;
        }
        DCHECK_LT(*element_scale, WordRepresentation::WordPtr().bit_width());
        if (diff < (WordRepresentation::WordPtr().bit_width() - uint64_t { *element_scale })) {
            *element_scale += diff;
            return true;
        }
        return false;
        ...
        while (index.valid()) {
            ...
            if (TryAdjustOffset(offset, index_op, *element_scale, tagged_base)) {
                ...
                *offset = 0;
                ...
                if (shift_op->kind == ShiftOp::Kind::kShiftLeft && TryAdjustElementScale(element_scale, shift_op->right())) {
                    index = shift_op->left();
                    continue;
                }
            } else if (const WordBinopOp* binary_op = index_op.TryCast<WordBinopOp>()) {
                ...
                if (binary_op->kind == WordBinopOp::Kind::kAdd && TryAdjustOffset(offset, matcher_.Get(binary_op->right()), *element_scale, tagged_base)) {
    ...
    {
        if (OpIndex input;
            matcher_.MatchChange(value, &input, ChangeOp::Kind::kFloatConversion, RegisterRepresentation::Float32(), RegisterRepresentation::Float64())) {
            return true;
        }
        if (double c; matcher_.MatchFloat64Constant(value, &c) && DoubleToFloat32(c) == c) {
            return true;
        }
        return false;
    }
    ...
    {
        return matcher_.Is<ComparisonOp>(value);
    }
    ...
    {
        if (auto* op = matcher_.TryCast<LoadOp>(value)) {
            return op->loaded_rep == any_of(MemoryRepresentation::Int16(), MemoryRepresentation::Int8());
        } else if (auto* op = matcher_.TryCast<LoadOp>(value)) {
            return op->loaded_rep == any_of(MemoryRepresentation::Int16(), MemoryRepresentation::Int8());
        }
        return false;
    ...
    bool IsWord32ConvertedToWord64(OpIndex value, std::optional<bool>* sign_extended = nullptr)
    {
        if (const ChangeOp* change_op = matcher_.TryCast<ChangeOp>(value)) {
            if (change_op->from == WordRepresentation::Word32() && change_op->to == WordRepresentation::Word64()) {
        ...
        if (int64_t c; matcher_.MatchIntegralWord64Constant(value, &c) && c >= std::numeric_limits<int32_t>::min()) {
            if (c < 0) {
                if (sign_extended)
                    *sign_extended = true;
                return true;
    ...
    {
        if (const ChangeOp* op = matcher_.TryCast<ChangeOp>(value)) {
            if (op->from == WordRepresentation::Word32() && op->to == WordRepresentation::Word64()
                && op->kind == any_of(ChangeOp::Kind::kZeroExtend, ChangeOp::Kind::kSignExtend)) {
                return V<Word32>::Cast(op->input());
            }
        }
        return value;
    ...
    V<Word> ReduceWithTruncation(V<Word> value, uint64_t truncation_mask, WordRepresentation rep)
    {
        { // Remove bitwise-and with a mask whose zero-bits are not observed.
            V<Word> input, mask;
            uint64_t mask_value;
            if (matcher_.MatchBitwiseAnd(value, &input, &mask, rep) && matcher_.MatchIntegralWordConstant(mask, rep, &mask_value)) {
        ...
        }
        {
            ...
            if (matcher_.MatchConstantShift(value, &left_shift, &right_shift_kind, &rep, &right_shift_amount) && ShiftOp::IsRightShift(right_shift_kind)
                && matcher_.MatchConstantShift(left_shift, &left_shift_input, ShiftOp::Kind::kShiftLeft, rep, &left_shift_amount)
                && ((rep.MaxUnsignedValue() >> right_shift_amount) & truncation_mask) == truncation_mask) {
                if (left_shift_amount == right_shift_amount) {
                    return left_shift_input;
                } else if (left_shift_amount < right_shift_amount) {
                    OpIndex shift_amount = __ WordConstant(right_shift_amount - left_shift_amount, rep);
                    return __ Shift(left_shift_input, shift_amount, right_shift_kind, rep);
                } else if (left_shift_amount > right_shift_amount) {
                    OpIndex shift_amount = __ WordConstant(left_shift_amount - right_shift_amount, rep);
                    return __ Shift(left_shift_input, shift_amount, ShiftOp::Kind::kShiftLeft, rep);
                }
            }
        }
        return value;
    ...
    OpIndex ReduceSignedDiv(OpIndex left, int64_t right, WordRepresentation rep)
    {
        ...
        if (right == rep.MinSignedValue()) {
            ...
            return rep == WordRepresentation::Word64() ? __ ChangeUint32ToUint64(equal_op) : equal_op;
        }
        // left / -right  => -(left / right)
        if (right < 0) {
            DCHECK_NE(right, rep.MinSignedValue());
            return __ WordSub(__ WordConstant(0, rep), ReduceSignedDiv(left, Abs(right), rep), rep);
        }
        ...
        auto LowerToMul = [this, left](auto right, WordRepresentation rep) {
            base::MagicNumbersForDivision<decltype(right)> magic = base::SignedDivisionByConstant(right);
            OpIndex quotient = __ IntMulOverflownBits(left, __ WordConstant(magic.multiplier, rep), rep);
            if (magic.multiplier < 0) {
                quotient = __ WordAdd(quotient, left, rep);
            }
            OpIndex sign_bit = __ ShiftRightLogical(left, rep.bit_width() - 1, rep);
            return __ WordAdd(__ ShiftRightArithmetic(quotient, magic.shift, rep), sign_bit, rep);
        };
        if (rep == WordRepresentation::Word32()) {
            return LowerToMul(static_cast<int32_t>(right), WordRepresentation::Word32());
        } else {
            DCHECK_EQ(rep, WordRepresentation::Word64());
            return LowerToMul(static_cast<int64_t>(right), WordRepresentation::Word64());
        }
    }

    OpIndex ReduceUnsignedDiv(OpIndex left, uint64_t right, WordRepresentation rep)
    {
        // left / 0 => 0
        if (right == 0) {
            return __ WordConstant(0, rep);
        ...
        // left / 2^k  => left >> k
        if (base::bits::IsPowerOfTwo(right)) {
            return __ ShiftRightLogical(left, base::bits::WhichPowerOfTwo(right), rep);
        }
        DCHECK_GT(right, 0);
        ...
        auto LowerToMul = [this, left, shift](auto right, WordRepresentation rep) {
            base::MagicNumbersForDivision<decltype(right)> const mag = base::UnsignedDivisionByConstant(right, shift);
            OpIndex quotient = __ UintMulOverflownBits(left, __ WordConstant(mag.multiplier, rep), rep);
            if (mag.add) {
                DCHECK_GE(mag.shift, 1);
                // quotient = (((left - quotient) >> 1) + quotient) >> (mag.shift -
                // 1)
                quotient = __ ShiftRightLogical(__ WordAdd(__ ShiftRightLogical(__ WordSub(left, quotient, rep), 1, rep), quotient, rep), mag.shift - 1, rep);
            } else {
        ...
        if (rep == WordRepresentation::Word32()) {
            return LowerToMul(static_cast<uint32_t>(right), WordRepresentation::Word32());
        } else {
            DCHECK_EQ(rep, WordRepresentation::Word64());
            return LowerToMul(static_cast<uint64_t>(right), WordRepresentation::Word64());
        }
    }
    ...
    std::optional<V<Word32>> ReduceBranchCondition(V<Word32> condition, bool* negated)
    {
        ...
        while (true) {
            ...
            if (V<Word32> left, right; matcher_.MatchEqual(condition, &left, &right) && matcher_.MatchZero(right)) {
                reduced = true;
                condition = left;
                *negated = !*negated;
                continue;
            ...
            if (V<Word32> left, right; matcher_.MatchWordSub(condition, &left, &right, WordRepresentation::Word32())) {
                reduced = true;
            ...
            // x & (1 << k) == (1 << k)  =>  x & (1 << k)
            if (V<Word32> left, right; matcher_.MatchEqual(condition, &left, &right)) {
                V<Word32> x, mask;
                uint32_t k1, k2;
                if (matcher_.MatchBitwiseAnd(left, &x, &mask, WordRepresentation::Word32()) && matcher_.MatchIntegralWord32Constant(mask, &k1)
                    && matcher_.MatchIntegralWord32Constant(right, &k2) && k1 == k2 && base::bits::IsPowerOfTwo(k1)) {
                    reduced = true;
                    condition = left;
                    continue;
                }
            ...
            // (x >> k1) & k2   =>   x & (k2 << k1)
            {
                ...
                int k1_int;
                uint32_t k1, k2;
                if (matcher_.MatchBitwiseAnd(condition, &shift, &k2_index, WordRepresentation::Word32())
                    && matcher_.MatchConstantRightShift(shift, &x, WordRepresentation::Word32(), &k1_int)
                    && matcher_.MatchIntegralWord32Constant(k2_index, &k2)) {
                    k1 = static_cast<uint32_t>(k1_int);
                    if (k1 <= base::bits::CountLeadingZeros(k2) && (static_cast<uint64_t>(k2) << k1 <= std::numeric_limits<uint32_t>::max())) {
    ...
    {
        if (uint32_t value; matcher_.MatchIntegralWord32Constant(condition, &value)) {
            return value != 0;
        }
        return std::nullopt;
    }
    ...
    bool MapLoadCanBeConstantFolded(OptionalMapRef map)
    {
        ...
        if (map->instance_type() == any_of(BIG_INT_BASE_TYPE, HEAP_NUMBER_TYPE, ODDBALL_TYPE)) {
            return true;
        }
