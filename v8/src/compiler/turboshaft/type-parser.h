...
private:
    ...
    template <typename T> std::optional<T> ParseRange()
    {
        if (!ConsumeIf("["))
            return std::nullopt;
        ...
        if (!ConsumeIf("]"))
            return std::nullopt;
        if constexpr (!std::is_same_v<T, Word32Type> && !std::is_same_v<T, Word64Type>) {
            CHECK_LE(*from, *to);
        }
        return T::Range(*from, *to, zone_);
    }

    template <typename T> std::optional<T> ParseSet()
    {
        if (!ConsumeIf("{"))
            return std::nullopt;
    ...
    template <typename T> std::optional<std::vector<T>> ParseSetElements()
    {
