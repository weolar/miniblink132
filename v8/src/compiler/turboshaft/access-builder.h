...
// namespaces.
template <typename Class, typename T> struct FieldAccessTS : public compiler::FieldAccess {
    using type = T;
...
// namespaces.
template <typename Class, typename T> struct ElementAccessTS : public compiler::ElementAccess {
    using type = T;
...
private:
    friend class AccessBuilderTS;
    explicit ElementAccessTS(const compiler::ElementAccess& base, bool is_array_buffer_load)
...
public:
    template <typename Class> static constexpr bool is_array_buffer_v = std::is_same_v<Class, ArrayBuffer>;
...
#define TF_FIELD_ACCESS(Class, T, name)                                                                                                                        \
    ...
    TF_FIELD_ACCESS(HeapNumberOrOddballOrHole, Float64, ForHeapNumberOrOddballOrHoleValue)
#undef TF_ACCESS
    static FieldAccessTS<Object, Map> ForMap(WriteBarrierKind write_barrier = kMapWriteBarrier)
    {
        return FieldAccessTS<Object, Map>(compiler::AccessBuilder::ForMap(write_barrier));
    }
    static FieldAccessTS<FeedbackVector, Word32> ForFeedbackVectorLength()
    {
        return FieldAccessTS<FeedbackVector, Word32>(compiler::FieldAccess { BaseTaggedness::kTaggedBase, FeedbackVector::kLengthOffset, Handle<Name>(),
            OptionalMapRef(), TypeCache::Get()->kInt32, MachineType::Int32(), WriteBarrierKind::kNoWriteBarrier });
    }
...
#undef TF_ELEMENT_ACCESS
    ...
    template <IsTagged T> static ElementAccessTS<FixedArray, T> ForFixedArrayElement()
    {
        static_assert(!is_array_buffer_v<FixedArray>);
        return ElementAccessTS<FixedArray, T> { compiler::AccessBuilder::ForFixedArrayElement(), false };
    }
};
