...
struct LoadStoreSimplificationConfiguration {
...
#if V8_TARGET_ARCH_ARM64 || V8_TARGET_ARCH_ARM || V8_TARGET_ARCH_RISCV64 || V8_TARGET_ARCH_LOONG64 || V8_TARGET_ARCH_MIPS64 || V8_TARGET_ARCH_PPC64            \
    ...
    static constexpr int32_t kMinOffset = -(static_cast<int32_t>(1) << (kDisplacementBits - 1));
    static constexpr int32_t kMaxOffset = (static_cast<int32_t>(1) << (kDisplacementBits - 1)) - 1;
    static constexpr int kMaxElementSizeLog2 = 0;
#else
    static constexpr bool kNeedsUntaggedBase = false;
...
template <class Next> class LoadStoreSimplificationReducer : public Next, LoadStoreSimplificationConfiguration {
public:
    ...
    {
        SimplifyLoadStore(base, index, kind, offset, element_size_log2);
        if (write_barrier != WriteBarrierKind::kNoWriteBarrier && !index.has_value() && __ Get(base).template Is<ConstantOp>()) {
            const ConstantOp& const_base = __ Get(base).template Cast<ConstantOp>();
            if (const_base.IsIntegral() || const_base.kind == ConstantOp::Kind::kSmi) {
    ...
    OpIndex REDUCE(AtomicWord32Pair)(V<WordPtr> base, OptionalV<WordPtr> index, OptionalV<Word32> value_low, OptionalV<Word32> value_high,
        OptionalV<Word32> expected_low, OptionalV<Word32> expected_high, AtomicWord32PairOp::Kind kind, int32_t offset)
    {
        if (kind == AtomicWord32PairOp::Kind::kStore || kind == AtomicWord32PairOp::Kind::kLoad) {
            if (!index.valid()) {
        ...
        return Next::ReduceAtomicWord32Pair(base, index, value_low, value_high, expected_low, expected_high, kind, offset);
    }
...
private:
    ...
    void SimplifyLoadStore(OpIndex& base, OptionalOpIndex& index, LoadOp::Kind& kind, int32_t& offset, uint8_t& element_size_log2)
    {
        ...
        // is ported to x64.
        if (!CanEncodeOffset(offset, kind.tagged_base) || (kind.is_atomic && !CanEncodeAtomic(index, element_size_log2, offset))) {
    ...
    || defined(V8_TARGET_ARCH_PPC64) || defined(V8_TARGET_ARCH_S390X) || defined(V8_TARGET_ARCH_LOONG64) || defined(V8_TARGET_ARCH_MIPS64)                     \
    || defined(V8_TARGET_ARCH_RISCV64) || defined(V8_TARGET_ARCH_RISCV32)
    bool lowering_enabled_ = (is_wasm_ && v8_flags.turboshaft_wasm_instruction_selection_staged) || (!is_wasm_ && v8_flags.turboshaft_instruction_selection);
#else
    bool lowering_enabled_
        = (is_wasm_ && v8_flags.turboshaft_wasm_instruction_selection_experimental) || (!is_wasm_ && v8_flags.turboshaft_instruction_selection);
#endif
