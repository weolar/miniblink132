...
class HeapObject;
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, AbortReason reason);
} // namespace v8::internal
...
#else
#define TURBOSHAFT_SIMD256_OPERATION_LIST(V) TURBOSHAFT_SIMD256_COMMOM_OPERATION_LIST(V)
...
namespace detail {
template <class Op> struct operation_to_opcode_map { };

#define OPERATION_OPCODE_MAP_CASE(Name)                                                                                                                        \
    template <> struct operation_to_opcode_map<Name##Op> : std::integral_constant<Opcode, Opcode::k##Name> { };
TURBOSHAFT_OPERATION_LIST(OPERATION_OPCODE_MAP_CASE)
#undef OPERATION_OPCODE_MAP_CASE
} // namespace detail

template <typename Op> struct operation_to_opcode : detail::operation_to_opcode_map<std::remove_cvref_t<Op>> { };
template <typename Op> constexpr Opcode operation_to_opcode_v = operation_to_opcode<Op>::value;

template <typename Op, uint64_t Mask, uint64_t Value> struct OpMaskT {
    using operation = Op;
...
constexpr uint16_t kNumberOfBlockTerminatorOpcodes = 0 TURBOSHAFT_OPERATION_LIST_BLOCK_TERMINATOR(COUNT_OPCODES);
#undef COUNT_OPCODES

#define COUNT_OPCODES(Name) +1
constexpr uint16_t kNumberOfOpcodes = 0 TURBOSHAFT_OPERATION_LIST(COUNT_OPCODES);
#undef COUNT_OPCODES
...
#define TURBOSHAFT_THROWING_STATIC_OUTPUTS_OPERATIONS_LIST(V) TURBOSHAFT_JS_THROWING_OPERATION_LIST(V)
...
#define THROWING_OP_BOILERPLATE(...)                                                                                                                           \
    ...
    static constexpr base::Vector<const RegisterRepresentation> kOutReps = base::VectorOf(kOutputRepsStorage, arraysize(kOutputRepsStorage));                  \
...
template <typename T> inline base::Vector<T> InitVectorOf(ZoneVector<T>& storage, std::initializer_list<RegisterRepresentation> values)
{
...
public:
    constexpr static base::Vector<const MaybeRegisterRepresentation> SingleRep(RegisterRepresentation rep)
    {
        return base::VectorOf(ToMaybeRepPointer(rep), 1);
    }

    constexpr static base::Vector<const MaybeRegisterRepresentation> PairOf(RegisterRepresentation rep)
    {
        return base::VectorOf(ToMaybeRepPointer(rep), 2);
    }

protected:
    constexpr static const MaybeRegisterRepresentation* ToMaybeRepPointer(RegisterRepresentation rep)
    {
...
struct EffectDimensions {
    ...
    Bits bits() const
    {
        return base::bit_cast<Bits>(*this);
    }
...
struct OpEffects {
    ...
    {
        return CanReadMemory().CanWriteMemory().CanAllocate().CanChangeControlFlow().CanDependOnChecks().RequiredWhenUnused();
    }
    ...
    {
        return IsSubsetOf(OpEffects().CanDependOnChecks().CanChangeControlFlow().CanAllocateWithoutIdentity());
    }
...
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, OpEffects op_effects);
...
private:
    explicit SaturatedUint8(uint8_t val)
        : val(val)
    {
    }
    uint8_t val = 0;
...
template <typename T> struct underlying_operation {
    using type = T;
};
template <typename T, uint64_t M, uint64_t V> struct underlying_operation<OpMaskT<T, M, V>> {
    using type = T;
};
template <typename T> using underlying_operation_t = typename underlying_operation<T>::type;
...
struct alignas(OpIndex) Operation {
    struct IdentityMapper {
        ...
        template <size_t N> base::SmallVector<OpIndex, N> Map(base::Vector<const OpIndex> indices)
        {
    ...
    template <class Op> bool Is() const
    {
    ...
    template <class Op> underlying_operation_t<Op>& Cast()
    {
        DCHECK(Is<Op>());
        return *static_cast<underlying_operation_t<Op>*>(this);
    }
    template <class Op> const underlying_operation_t<Op>& Cast() const
    {
        DCHECK(Is<Op>());
        return *static_cast<const underlying_operation_t<Op>*>(this);
    }
    template <class Op> const underlying_operation_t<Op>* TryCast() const
    {
        if (!Is<Op>())
            return nullptr;
        return static_cast<const underlying_operation_t<Op>*>(this);
    }
    template <class Op> underlying_operation_t<Op>* TryCast()
    {
        if (!Is<Op>())
            return nullptr;
        return static_cast<underlying_operation_t<Op>*>(this);
...
protected:
    ...
    {
        DCHECK_LE(input_count, std::numeric_limits<decltype(this->input_count)>::max());
    }
    ...
    {
        static_assert(std::is_same_v<underlying_operation_t<OpmaskT>, typename OpMaskT<typename OpmaskT::operation, OpmaskT::mask, OpmaskT::value>::operation>);
...
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, OperationPrintStyle op);
...
template <class Op, class = void> struct HasStaticEffects : std::bool_constant<false> { };
template <class Op> struct HasStaticEffects<Op, std::void_t<decltype(Op::effects)>> : std::bool_constant<true> { };
...
template <class Derived> struct OperationT : Operation {
    ...
    bool IsRequiredWhenUnused() const
    {
        return IsBlockTerminator() || derived_this().Effects().is_required_when_unused();
    }
    ...
    {
        return { reinterpret_cast<OpIndex*>(reinterpret_cast<char*>(this) + sizeof(Derived)), derived_this().input_count };
    }
    base::Vector<const OpIndex> inputs() const
    {
        return { reinterpret_cast<const OpIndex*>(reinterpret_cast<const char*>(this) + sizeof(Derived)), derived_this().input_count };
    }

    V8_INLINE OpIndex& input(size_t i)
    {
        return derived_this().inputs()[i];
    }
    ...
    template <typename T> V8_INLINE V<T> input(size_t i) const
    {
    ...
    static size_t StorageSlotCount(size_t input_count)
    {
        ...
        size_t result = std::max<size_t>(2, (r - 1 + sizeof(Derived) / sizeof(OpIndex) + input_count) / r);
        DCHECK_EQ(result, Operation::StorageSlotCount(opcode, input_count));
        return result;
    }
    size_t StorageSlotCount() const
    {
        return StorageSlotCount(input_count);
    }

    template <class... Args> static Derived& New(Graph* graph, size_t input_count, Args... args)
    {
        ...
        for (size_t i = 0; i < end; ++i) {
            if (expected[i] == MaybeRegisterRepresentation::None())
                continue;
            DCHECK(ValidOpInputRep(*graph, result->inputs()[i], RegisterRepresentation(expected[i])));
        }
#endif
    ...
    template <class... Args> static Derived& New(Graph* graph, ShadowyOpIndexVectorWrapper inputs, Args... args)
    {
        return New(graph, inputs.size(), inputs, args...);
    }

    explicit OperationT(size_t input_count)
        : Operation(opcode, input_count)
    {
    ...
    {
        this->inputs().OverwriteWith(static_cast<base::Vector<const OpIndex>>(inputs));
    }
    ...
    {
        return derived_this().inputs() == other.derived_this().inputs() && derived_this().options() == other.derived_this().options();
    }
    template <typename... Args> size_t HashWithOptions(const Args&... args) const
    {
    ...
    void PrintOptions(std::ostream& os) const
    {
        ...
        constexpr size_t options_count = std::tuple_size<std::remove_reference_t<decltype(options)>>::value;
        if (options_count == 0) {
            return;
        }
...
private:
    template <class... T, size_t... I> static void PrintOptionsHelper(std::ostream& os, const std::tuple<T...>& options, std::index_sequence<I...>)
    {
        os << "[";
        bool first = true;
        USE(first);
        ((first ? (first = false, os << std::get<I>(options)) : os << ", " << std::get<I>(options)), ...);
        os << "]";
    }
...
template <size_t InputCount, class Derived> struct FixedArityOperationT : OperationT<Derived> {
    ...
    template <class... Args> static Derived& New(Graph* graph, Args... args)
    {
        Derived& result = OperationT<Derived>::New(graph, InputCount, std::move(args)...);
        return result;
    }
    ...
    V8_INLINE auto ExplodeImpl(Fn fn, Mapper& mapper, std::index_sequence<InputI...>, std::index_sequence<OptionI...>) const
    {
        ...
        USE(options);
        return fn(mapper.Map(this->input(InputI))..., std::get<OptionI>(options)...);
    }

    template <typename Fn, typename Mapper> V8_INLINE auto Explode(Fn fn, Mapper& mapper) const
    {
        return ExplodeImpl(
            fn, mapper, std::make_index_sequence<input_count>(), std::make_index_sequence<std::tuple_size_v<decltype(this->derived_this().options())>>());
    }
};
...
template <RegisterRepresentation::Enum... reps> base::Vector<const RegisterRepresentation> RepVector()
{
    static constexpr std::array<RegisterRepresentation, sizeof...(reps)> rep_array { RegisterRepresentation { reps }... };
    return base::VectorOf(rep_array);
}

template <MaybeRegisterRepresentation::Enum... reps> base::Vector<const MaybeRegisterRepresentation> MaybeRepVector()
{
    static constexpr std::array<MaybeRegisterRepresentation, sizeof...(reps)> rep_array { MaybeRegisterRepresentation { reps }... };
    return base::VectorOf(rep_array);
}
...
struct AbortCSADcheckOp : FixedArityOperationT<1, AbortCSADcheckOp> {
    static constexpr OpEffects effects = OpEffects().RequiredWhenUnused().CanLeaveCurrentFunction();
    ...
    explicit AbortCSADcheckOp(V<String> message)
        : Base(message)
    {
    }
...
#undef DEFINE_KIND
    ...
    base::Vector<const MaybeRegisterRepresentation> inputs_rep(ZoneVector<MaybeRegisterRepresentation>& storage) const
    {
        return MaybeRepVector<MaybeRegisterRepresentation::Tagged(), MaybeRegisterRepresentation::Tagged()>();
    }
    ...
    {
        return input<FrameState>(2);
    }
    V<Context> context() const
    {
        return input<Context>(3);
    }
    ...
    auto options() const
    {
        return std::tuple { kind, lazy_deopt_on_throw };
    }
};
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, GenericBinopOp::Kind kind);
...
#undef DEFINE_KIND
    ...
    V<FrameState> frame_state() const
    {
        return Base::input<FrameState>(1);
    }
    V<Context> context() const
    {
        return Base::input<Context>(2);
    }
...
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, GenericUnopOp::Kind kind);
...
struct ToNumberOrNumericOp : FixedArityOperationT<3, ToNumberOrNumericOp> {
    ...
    ToNumberOrNumericOp(V<Object> input, OpIndex frame_state, V<Context> context, Object::Conversion kind, LazyDeoptOnThrow lazy_deopt_on_throw)
        : Base(input, frame_state, context)
        , kind(kind)
        , lazy_deopt_on_throw(lazy_deopt_on_throw)
    {
...
struct Word32SignHintOp : FixedArityOperationT<1, Word32SignHintOp> {
    ...
    auto options() const
    {
        return std::tuple { sign };
    }
};
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, Word32SignHintOp::Sign sign);
...
struct WordBinopOp : FixedArityOperationT<2, WordBinopOp> {
    ...
    template <class WordType = Word, typename = std::enable_if_t<IsWord<WordType>()>> V<WordType> left() const
    {
        return input<WordType>(0);
    }
    template <class WordType = Word, typename = std::enable_if_t<IsWord<WordType>()>> V<WordType> right() const
    {
...
struct FloatBinopOp : FixedArityOperationT<2, FloatBinopOp> {
    ...
    V<Float> left() const
    {
        return input<Float>(0);
    }
    V<Float> right() const
    {
        return input<Float>(1);
    }
    ...
    void Validate(const Graph& graph) const
    {
        DCHECK_IMPLIES(kind == any_of(Kind::kPower, Kind::kAtan2, Kind::kMod), rep == FloatRepresentation::Float64());
    }
    auto options() const
    {
        return std::tuple { kind, rep };
...
struct Word32PairBinopOp : FixedArityOperationT<4, Word32PairBinopOp> {
    ...
    base::Vector<const RegisterRepresentation> outputs_rep() const
    {
        return RepVector<RegisterRepresentation::Word32(), RegisterRepresentation::Word32()>();
    }

    base::Vector<const MaybeRegisterRepresentation> inputs_rep(const ZoneVector<MaybeRegisterRepresentation>& storage) const
    {
        return MaybeRepVector<MaybeRegisterRepresentation::Word32(), MaybeRegisterRepresentation::Word32(), MaybeRegisterRepresentation::Word32(),
            MaybeRegisterRepresentation::Word32()>();
    }

    OpIndex left_low() const
    {
        return input(0);
    }
    OpIndex left_high() const
    {
        return input(1);
    }
    OpIndex right_low() const
    {
        return input(2);
    }
    OpIndex right_high() const
    {
        return input(3);
    }

    Word32PairBinopOp(OpIndex left_low, OpIndex left_high, OpIndex right_low, OpIndex right_high, Kind kind)
...
struct WordBinopDeoptOnOverflowOp : FixedArityOperationT<3, WordBinopDeoptOnOverflowOp> {
    enum class Kind : uint8_t { kSignedAdd, kSignedMul, kSignedSub, kSignedDiv, kSignedMod, kUnsignedDiv, kUnsignedMod };
    Kind kind;
    WordRepresentation rep;
    ...
    WordBinopDeoptOnOverflowOp(
        V<Word> left, V<Word> right, V<FrameState> frame_state, Kind kind, WordRepresentation rep, FeedbackSource feedback, CheckForMinusZeroMode mode)
    ...
    void Validate(const Graph& graph) const
    {
        DCHECK_IMPLIES(kind == Kind::kUnsignedDiv || kind == Kind::kUnsignedMod, rep == WordRepresentation::Word32());
    }
    auto options() const
    {
        return std::tuple { kind, rep, feedback, mode };
    }
    void PrintOptions(std::ostream& os) const;
};

struct OverflowCheckedBinopOp : FixedArityOperationT<2, OverflowCheckedBinopOp> {
    ...
    base::Vector<const RegisterRepresentation> outputs_rep() const
    {
        ...
        case WordRepresentation::Word64():
            return RepVector<RegisterRepresentation::Word64(), RegisterRepresentation::Word32()>();
        }
    }
...
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, WordUnaryOp::Kind kind);

struct OverflowCheckedUnaryOp : FixedArityOperationT<1, OverflowCheckedUnaryOp> {
    static constexpr int kValueIndex = 0;
    static constexpr int kOverflowIndex = 1;
    ...
    explicit OverflowCheckedUnaryOp(V<Word> input, Kind kind, WordRepresentation rep)
        : Base(input)
        , kind(kind)
        , rep(rep)
    {
...
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, OverflowCheckedUnaryOp::Kind kind);
...
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, FloatUnaryOp::Kind kind);

struct ShiftOp : FixedArityOperationT<2, ShiftOp> {
    enum class Kind : uint8_t { kShiftRightArithmeticShiftOutZeros, kShiftRightArithmetic, kShiftRightLogical, kShiftLeft, kRotateRight, kRotateLeft };
    Kind kind;
    WordRepresentation rep;
    ...
    base::Vector<const MaybeRegisterRepresentation> inputs_rep(ZoneVector<MaybeRegisterRepresentation>& storage) const
    {
        return InitVectorOf(storage, { static_cast<const RegisterRepresentation&>(rep), RegisterRepresentation::Word32() });
    }
...
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, ShiftOp::Kind kind);

struct ComparisonOp : FixedArityOperationT<2, ComparisonOp> {
    enum class Kind : uint8_t { kEqual, kSignedLessThan, kSignedLessThanOrEqual, kUnsignedLessThan, kUnsignedLessThanOrEqual };
    Kind kind;
    ...
    void Validate(const Graph& graph) const
    {
        if (kind == Kind::kEqual) {
            DCHECK(rep
                == any_of(RegisterRepresentation::Word32(), RegisterRepresentation::Word64(), RegisterRepresentation::Float32(),
                    RegisterRepresentation::Float64(), RegisterRepresentation::Tagged()));
            ...
            DCHECK_EQ(rep,
                any_of(
                    RegisterRepresentation::Word32(), RegisterRepresentation::Word64(), RegisterRepresentation::Float32(), RegisterRepresentation::Float64()));
            DCHECK_IMPLIES(rep == any_of(RegisterRepresentation::Float32(), RegisterRepresentation::Float64()),
...
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, ComparisonOp::Kind kind);
...
struct ChangeOp : FixedArityOperationT<1, ChangeOp> {
    ...
    static bool IsReversible(
        Kind kind, Assumption assumption, RegisterRepresentation from, RegisterRepresentation to, Kind reverse_kind, bool signalling_nan_possible)
    {
        switch (kind) {
        case Kind::kFloatConversion:
            return from == RegisterRepresentation::Float32() && to == RegisterRepresentation::Float64() && reverse_kind == Kind::kFloatConversion
                && !signalling_nan_possible;
        case Kind::kSignedFloatTruncateOverflowToMin:
            return assumption == Assumption::kReversible && reverse_kind == Kind::kSignedToFloat;
        case Kind::kUnsignedFloatTruncateOverflowToMin:
            return assumption == Assumption::kReversible && reverse_kind == Kind::kUnsignedToFloat;
        case Kind::kJSFloatTruncate:
            return false;
        case Kind::kSignedToFloat:
            if (from == RegisterRepresentation::Word32() && to == RegisterRepresentation::Float64()) {
                return reverse_kind == any_of(Kind::kSignedFloatTruncateOverflowToMin, Kind::kJSFloatTruncate);
            } else {
                return assumption == Assumption::kReversible && reverse_kind == any_of(Kind::kSignedFloatTruncateOverflowToMin);
            }
        case Kind::kUnsignedToFloat:
            if (from == RegisterRepresentation::Word32() && to == RegisterRepresentation::Float64()) {
                return reverse_kind == any_of(Kind::kUnsignedFloatTruncateOverflowToMin, Kind::kJSFloatTruncate);
            } else {
                return assumption == Assumption::kReversible && reverse_kind == Kind::kUnsignedFloatTruncateOverflowToMin;
            }
    ...
    {
        return IsReversible(kind, assumption, from, to, reverse_kind, signalling_nan_possible);
    }
    ...
    void Validate(const Graph& graph) const
    {
        ...
        // GCs).
        DCHECK_IMPLIES(kind == Kind::kBitcast, from != RegisterRepresentation::Tagged() && to != RegisterRepresentation::Tagged());
    }
    auto options() const
    {
        return std::tuple { kind, assumption, from, to };
    }
};
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, ChangeOp::Kind kind);
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, ChangeOp::Assumption assumption);
...
struct ChangeOrDeoptOp : FixedArityOperationT<2, ChangeOrDeoptOp> {
    ...
    auto options() const
    {
        return std::tuple { kind, minus_zero_mode, feedback };
    }
};
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, ChangeOrDeoptOp::Kind kind);
...
struct TryChangeOp : FixedArityOperationT<1, TryChangeOp> {
    ...
    auto options() const
    {
        return std::tuple { kind, from, to };
    }
};
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, TryChangeOp::Kind kind);

struct BitcastWord32PairToFloat64Op : FixedArityOperationT<2, BitcastWord32PairToFloat64Op> {
    static constexpr OpEffects effects = OpEffects();
    base::Vector<const RegisterRepresentation> outputs_rep() const
    {
        return RepVector<RegisterRepresentation::Float64()>();
    ...
    base::Vector<const MaybeRegisterRepresentation> inputs_rep(ZoneVector<MaybeRegisterRepresentation>& storage) const
    {
        return MaybeRepVector<MaybeRegisterRepresentation::Word32(), MaybeRegisterRepresentation::Word32()>();
    }

    V<Word32> high_word32() const
    {
        return input<Word32>(0);
    }
    V<Word32> low_word32() const
    {
        return input<Word32>(1);
    }
...
struct TaggedBitcastOp : FixedArityOperationT<1, TaggedBitcastOp> {
    ...
    {
        if (kind == Kind::kSmi) {
            DCHECK((from.IsWord() && to.IsTaggedOrCompressed()) || (from.IsTaggedOrCompressed() && to.IsWord()));
            DCHECK_IMPLIES(from == RegisterRepresentation::Word64() || to == RegisterRepresentation::Word64(), Is64());
        } else {
            // TODO(nicohartmann@): Without implicit trucation, the first case might
            // not be correct anymore.
            DCHECK((from.IsWord() && to == RegisterRepresentation::Tagged())
                || (from == RegisterRepresentation::Tagged() && to == RegisterRepresentation::WordPtr())
                || (from == RegisterRepresentation::Compressed() && to == RegisterRepresentation::Word32()));
        }
    }
    auto options() const
    {
        return std::tuple { from, to, kind };
    }
};
...
struct SelectOp : FixedArityOperationT<3, SelectOp> {
    ...
    void Validate(const Graph& graph) const
    {
        DCHECK_IMPLIES(implem == Implementation::kCMove,
            (rep == RegisterRepresentation::Word32() && SupportedOperations::word32_select())
                || (rep == RegisterRepresentation::Word64() && SupportedOperations::word64_select())
                || (rep == RegisterRepresentation::Float32() && SupportedOperations::float32_select())
                || (rep == RegisterRepresentation::Float64() && SupportedOperations::float64_select()));
    }
...
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, SelectOp::Implementation kind);
...
struct PhiOp : OperationT<PhiOp> {
    ...
    void Validate(const Graph& graph) const
    {
        DCHECK_GT(input_count, 0);
    }
    auto options() const
    {
        return std::tuple { rep };
    }
};
...
struct ConstantOp : FixedArityOperationT<0, ConstantOp> {
    ...
    void Validate(const Graph& graph) const
    {
        DCHECK_IMPLIES(kind == Kind::kWord32, storage.integral <= WordRepresentation::Word32().MaxUnsignedValue());
        DCHECK_IMPLIES(kind == Kind::kRelocatableWasmCanonicalSignatureId, storage.integral <= WordRepresentation::Word32().MaxSignedValue());
    }
    ...
    {
        DCHECK(kind == Kind::kHeapObject || kind == Kind::kCompressedHeapObject || kind == Kind::kTrustedHeapObject);
    ...
    {
        return kind == Kind::kWord32 || kind == Kind::kWord64 || kind == Kind::kRelocatableWasmCall || kind == Kind::kRelocatableWasmStubCall
            || kind == Kind::kRelocatableWasmCanonicalSignatureId || kind == Kind::kRelocatableWasmIndirectCallTarget;
    }

    auto options() const
    {
        return std::tuple { kind, storage };
    }
    ...
    size_t hash_value(HashingStrategy strategy = HashingStrategy::kDefault) const
    {
        ...
        case Kind::kExternal:
            return HashWithOptions(strategy == HashingStrategy::kMakeSnapshotStable ? 0 : storage.external.raw());
        case Kind::kHeapObject:
        case Kind::kCompressedHeapObject:
        case Kind::kTrustedHeapObject:
    ...
    bool operator==(const ConstantOp& other) const
    {
        ...
        case Kind::kFloat32:
            ...
            // them specially here.
            return base::bit_cast<uint32_t>(storage.float32) == base::bit_cast<uint32_t>(other.storage.float32);
        case Kind::kFloat64:
        case Kind::kNumber:
            ...
            // Since we canonicalize NaNs for JS anyway, we don't need to treat
            // them specially here.
            return base::bit_cast<uint64_t>(storage.float64) == base::bit_cast<uint64_t>(other.storage.float64);
        case Kind::kExternal:
...
struct LoadOp : OperationT<LoadOp> {
    struct Kind {
        ...
        {
            return rep == MemoryRepresentation::Int8() || rep == MemoryRepresentation::Uint8() || SupportedOperations::IsUnalignedLoadSupported(rep)
        ...
        {
            return tagged_base == other.tagged_base && maybe_unaligned == other.maybe_unaligned && with_trap_handler == other.with_trap_handler
                && load_eliminable == other.load_eliminable && is_immutable == other.is_immutable && is_atomic == other.is_atomic
                && trap_on_null == other.trap_on_null;
        }
    };
    Kind kind;
    ...
    base::Vector<const MaybeRegisterRepresentation> inputs_rep(ZoneVector<MaybeRegisterRepresentation>& storage) const
    {
        base::Vector<const MaybeRegisterRepresentation> result = kind.tagged_base
            ? MaybeRepVector<MaybeRegisterRepresentation::Tagged(), MaybeRegisterRepresentation::WordPtr()>()
            : MaybeRepVector<MaybeRegisterRepresentation::WordPtr(), MaybeRegisterRepresentation::WordPtr()>();
    ...
    LoadOp(OpIndex base, OptionalOpIndex index, Kind kind, MemoryRepresentation loaded_rep, RegisterRepresentation result_rep, int32_t offset,
    ...
    template <typename Fn, typename Mapper> V8_INLINE auto Explode(Fn fn, Mapper& mapper) const
    {
        return fn(mapper.Map(base()), mapper.Map(index()), kind, loaded_rep, result_rep, offset, element_size_log2);
    }

    void Validate(const Graph& graph) const
    {
        DCHECK(loaded_rep.ToRegisterRepresentation() == result_rep || (loaded_rep.IsCompressibleTagged() && result_rep == RegisterRepresentation::Compressed())
            || kind.is_atomic);
        DCHECK_IMPLIES(element_size_log2 > 0, index().valid());
        DCHECK_IMPLIES(kind.maybe_unaligned, !SupportedOperations::IsUnalignedLoadSupported(loaded_rep));
        DCHECK(OffsetIsValid(offset, kind.tagged_base));
    }
    static LoadOp& New(Graph* graph, OpIndex base, OptionalOpIndex index, Kind kind, MemoryRepresentation loaded_rep, RegisterRepresentation result_rep,
        int32_t offset, uint8_t element_size_log2)
    {
        return Base::New(graph, 1 + index.valid(), base, index, kind, loaded_rep, result_rep, offset, element_size_log2);
    }
    void PrintInputs(std::ostream& os, const std::string& op_index_prefix) const;
    void PrintOptions(std::ostream& os) const;
    auto options() const
...
{
    return base::hash_value(static_cast<int>(kind.tagged_base) | (kind.maybe_unaligned << 1) | (kind.load_eliminable << 2) | (kind.is_immutable << 3)
        | (kind.with_trap_handler << 4) | (kind.is_atomic << 5));
}

struct AtomicRMWOp : OperationT<AtomicRMWOp> {
    enum class BinOp : uint8_t { kAdd, kSub, kAnd, kOr, kXor, kExchange, kCompareExchange };
    BinOp bin_op;
    ...
    OpEffects Effects() const
    {
        OpEffects effects = OpEffects().CanWriteMemory().CanDependOnChecks().CanReadMemory();
    ...
    base::Vector<const MaybeRegisterRepresentation> inputs_rep(ZoneVector<MaybeRegisterRepresentation>& storage) const
    {
        if (bin_op == BinOp::kCompareExchange) {
            return InitVectorOf(storage, { RegisterRepresentation::WordPtr(), RegisterRepresentation::WordPtr(), in_out_rep, in_out_rep });
        }
        return InitVectorOf(storage, { RegisterRepresentation::WordPtr(), RegisterRepresentation::WordPtr(), in_out_rep });
    }
    ...
    AtomicRMWOp(OpIndex base, OpIndex index, OpIndex value, OptionalOpIndex expected, BinOp bin_op, RegisterRepresentation in_out_rep,
    ...
    template <typename Fn, typename Mapper> V8_INLINE auto Explode(Fn fn, Mapper& mapper) const
    {
        return fn(mapper.Map(base()), mapper.Map(index()), mapper.Map(value()), mapper.Map(expected()), bin_op, in_out_rep, memory_rep, memory_access_kind);
    }

    static AtomicRMWOp& New(Graph* graph, OpIndex base, OpIndex index, OpIndex value, OptionalOpIndex expected, BinOp bin_op, RegisterRepresentation result_rep,
        MemoryRepresentation input_rep, MemoryAccessKind kind)
    {
        return Base::New(graph, 3 + expected.valid(), base, index, value, expected, bin_op, result_rep, input_rep, kind);
    }
...
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, AtomicRMWOp::BinOp kind);

struct AtomicWord32PairOp : OperationT<AtomicWord32PairOp> {
    enum class Kind : uint8_t { kAdd, kSub, kAnd, kOr, kXor, kExchange, kCompareExchange, kLoad, kStore };
    ...
    base::Vector<const MaybeRegisterRepresentation> inputs_rep(ZoneVector<MaybeRegisterRepresentation>& storage) const
    {
        ...
        if (kind != Kind::kLoad) {
            ...
            if (kind == Kind::kCompareExchange) {
                storage[3 + has_index] = RegisterRepresentation::Word32(); // expected_low
                storage[4 + has_index] = RegisterRepresentation::Word32(); // expected_high
            }
        }
        return base::VectorOf(storage);
    }
    ...
    {
        return kind != Kind::kLoad ? input<Word32>(1 + HasIndex()) : V<Word32>::Invalid();
    }
    OptionalV<Word32> value_high() const
    {
        return kind != Kind::kLoad ? input<Word32>(2 + HasIndex()) : V<Word32>::Invalid();
    }
    OptionalV<Word32> expected_low() const
    {
        return kind == Kind::kCompareExchange ? input<Word32>(3 + HasIndex()) : V<Word32>::Invalid();
    }
    OptionalV<Word32> expected_high() const
    {
        return kind == Kind::kCompareExchange ? input<Word32>(4 + HasIndex()) : V<Word32>::Invalid();
    }
    ...
    AtomicWord32PairOp(V<WordPtr> base, OptionalV<WordPtr> index, OptionalV<Word32> value_low, OptionalV<Word32> value_high, OptionalV<Word32> expected_low,
        OptionalV<Word32> expected_high, Kind kind, int32_t offset)
        : Base(InputCount(kind, index.has_value()))
        , kind(kind)
        , offset(offset)
    {
    ...
    template <typename Fn, typename Mapper> V8_INLINE auto Explode(Fn fn, Mapper& mapper) const
    {
        return fn(mapper.Map(base()), mapper.Map(index()), mapper.Map(value_low()), mapper.Map(value_high()), mapper.Map(expected_low()),
    ...
    bool HasIndex() const
    {
        return input_count == InputCount(kind, true);
    }

    static AtomicWord32PairOp& New(Graph* graph, V<WordPtr> base, OptionalV<WordPtr> index, OptionalV<Word32> value_low, OptionalV<Word32> value_high,
        OptionalV<Word32> expected_low, OptionalV<Word32> expected_high, Kind kind, int32_t offset)
    {
        return Base::New(graph, InputCount(kind, index.has_value()), base, index, value_low, value_high, expected_low, expected_high, kind, offset);
    }
    ...
    auto options() const
    {
        return std::tuple { kind, offset };
    }
};

V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, AtomicWord32PairOp::Kind kind);

struct MemoryBarrierOp : FixedArityOperationT<0, MemoryBarrierOp> {
    AtomicMemoryOrder memory_order;

    static constexpr OpEffects effects = OpEffects().CanReadHeapMemory().CanWriteMemory();
...
struct StoreOp : OperationT<StoreOp> {
    ...
    uint16_t shifted_indirect_pointer_tag; // for indirect pointer stores, the
    ...
    base::Vector<const MaybeRegisterRepresentation> inputs_rep(ZoneVector<MaybeRegisterRepresentation>& storage) const
    {
        RegisterRepresentation base = kind.tagged_base ? RegisterRepresentation::Tagged() : RegisterRepresentation::WordPtr();
        if (index() == OpIndex::Invalid()) {
            return InitVectorOf(storage, { base, stored_rep.ToRegisterRepresentationForStore() });
        }
        return InitVectorOf(storage, { base, stored_rep.ToRegisterRepresentationForStore(), RegisterRepresentation::WordPtr() });
    }
    ...
    StoreOp(OpIndex base, OptionalOpIndex index, OpIndex value, Kind kind, MemoryRepresentation stored_rep, WriteBarrierKind write_barrier, int32_t offset,
        uint8_t element_size_log2, bool maybe_initializing_or_transitioning, IndirectPointerTag maybe_indirect_pointer_tag = kIndirectPointerNullTag)
        : Base(2 + index.valid())
        , kind(kind)
        ...
        , element_size_log2(element_size_log2)
        , offset(offset)
        , maybe_initializing_or_transitioning(maybe_initializing_or_transitioning)
        , shifted_indirect_pointer_tag(maybe_indirect_pointer_tag >> kIndirectPointerTagShift)
    {
    ...
    template <typename Fn, typename Mapper> V8_INLINE auto Explode(Fn fn, Mapper& mapper) const
    {
        return fn(mapper.Map(base()), mapper.Map(index()), mapper.Map(value()), kind, stored_rep, write_barrier, offset, element_size_log2,
    ...
    {
        DCHECK_IMPLIES(element_size_log2 > 0, index().valid());
        DCHECK_IMPLIES(kind.maybe_unaligned, !SupportedOperations::IsUnalignedLoadSupported(stored_rep));
        DCHECK(LoadOp::OffsetIsValid(offset, kind.tagged_base));
    }
    static StoreOp& New(Graph* graph, OpIndex base, OptionalOpIndex index, OpIndex value, Kind kind, MemoryRepresentation stored_rep,
    ...
    {
        return Base::New(graph, 2 + index.valid(), base, index, value, kind, stored_rep, write_barrier, offset, element_size_log2,
    ...
    auto options() const
    {
        return std::tuple { kind, stored_rep, write_barrier, offset, element_size_log2, maybe_initializing_or_transitioning };
    }
};
...
struct DecodeExternalPointerOp : FixedArityOperationT<1, DecodeExternalPointerOp> {
...
struct JSStackCheckOp : OperationT<JSStackCheckOp> {
    ...
    OpEffects Effects() const
    {
        ...
        case Kind::kLoop:
            ...
            return OpEffects().CanDependOnChecks().CanDeopt().CanReadHeapMemory().CanAllocate();
        }
    }

    V<Context> native_context() const
    {
        return Base::input<Context>(0);
    }
    OptionalV<FrameState> frame_state() const
    {
        return input_count > 1 ? Base::input<FrameState>(1) : OptionalV<FrameState>::Nullopt();
    }
    ...
    explicit JSStackCheckOp(V<Context> context, OptionalV<FrameState> frame_state, Kind kind)
        : Base(1 + frame_state.has_value())
        , kind(kind)
    {
        input(0) = context;
    ...
    {
        return Base::New(graph, 1 + frame_state.has_value(), context, frame_state, kind);
    }
...
struct RetainOp : FixedArityOperationT<1, RetainOp> {
    ...
    explicit RetainOp(V<Object> retained)
        : Base(retained)
    {
    }
...
struct StackPointerGreaterThanOp : FixedArityOperationT<1, StackPointerGreaterThanOp> {
    StackCheckKind kind;
...
struct StackSlotOp : FixedArityOperationT<0, StackSlotOp> {
    ...
    auto options() const
    {
        return std::tuple { size, alignment, is_tagged };
    }
};
...
struct FrameConstantOp : FixedArityOperationT<0, FrameConstantOp> {
    ...
    explicit FrameConstantOp(Kind kind)
        : Base()
        , kind(kind)
    {
    }
...
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, FrameConstantOp::Kind kind);
...
struct FrameStateOp : OperationT<FrameStateOp> {
    ...
    {
        return RegisterRepresentation::FromMachineRepresentation(data->machine_types[idx].representation());
    }
...
struct WasmStackCheckOp : FixedArityOperationT<0, WasmStackCheckOp> {
    ...
    auto options() const
    {
        return std::tuple { kind, parameter_slots };
    }
};

V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, WasmStackCheckOp::Kind kind);
struct TrapIfOp : OperationT<TrapIfOp> {
    ...
    OptionalV<FrameState> frame_state() const
    {
        return input_count > 1 ? input<FrameState>(1) : OptionalV<FrameState>::Nullopt();
    }

    TrapIfOp(V<Word32> condition, OptionalV<FrameState> frame_state, bool negated, const TrapId trap_id)
    ...
    template <typename Fn, typename Mapper> V8_INLINE auto Explode(Fn fn, Mapper& mapper) const
    {
        return fn(mapper.Map(condition()), mapper.Map(frame_state()), negated, trap_id);
    }

    static TrapIfOp& New(Graph* graph, V<Word32> condition, OptionalV<FrameState> frame_state, bool negated, const TrapId trap_id)
    {
        return Base::New(graph, 1 + frame_state.valid(), condition, frame_state, negated, trap_id);
    }
...
struct StaticAssertOp : FixedArityOperationT<1, StaticAssertOp> {
    const char* source;
    static constexpr OpEffects effects = OpEffects().CanDependOnChecks().RequiredWhenUnused();
...
struct ParameterOp : FixedArityOperationT<0, ParameterOp> {
    ...
    explicit ParameterOp(int32_t parameter_index, RegisterRepresentation rep, const char* debug_name = "")
        : Base()
...
struct OsrValueOp : FixedArityOperationT<0, OsrValueOp> {
    ...
    explicit OsrValueOp(int32_t index)
        : Base()
        , index(index)
    {
    }
...
struct TSCallDescriptor : public NON_EXPORTED_BASE(ZoneObject) {
    ...
    TSCallDescriptor(const CallDescriptor* descriptor, base::Vector<const RegisterRepresentation> in_reps, base::Vector<const RegisterRepresentation> out_reps,
        CanThrow can_throw, LazyDeoptOnThrow lazy_deopt_on_throw, const JSWasmCallParameters* js_wasm_call_parameters)
    ...
    static const TSCallDescriptor* Create(const CallDescriptor* descriptor, CanThrow can_throw, LazyDeoptOnThrow lazy_deopt_on_throw, Zone* graph_zone,
        const JSWasmCallParameters* js_wasm_call_parameters = nullptr)
    {
        DCHECK_IMPLIES(can_throw == CanThrow::kNo, lazy_deopt_on_throw == LazyDeoptOnThrow::kNo);
        base::Vector<RegisterRepresentation> in_reps = graph_zone->AllocateVector<RegisterRepresentation>(descriptor->ParameterCount());
        for (size_t i = 0; i < descriptor->ParameterCount(); ++i) {
            in_reps[i] = RegisterRepresentation::FromMachineRepresentation(descriptor->GetParameterType(i).representation());
        }
        base::Vector<RegisterRepresentation> out_reps = graph_zone->AllocateVector<RegisterRepresentation>(descriptor->ReturnCount());
        for (size_t i = 0; i < descriptor->ReturnCount(); ++i) {
            out_reps[i] = RegisterRepresentation::FromMachineRepresentation(descriptor->GetReturnType(i).representation());
        }
        return graph_zone->New<TSCallDescriptor>(descriptor, in_reps, out_reps, can_throw, lazy_deopt_on_throw, js_wasm_call_parameters);
    }
};

template <> struct fast_hash<TSCallDescriptor> {
    size_t operator()(const TSCallDescriptor& v)
    {
        ...
        // not too critical).
        return fast_hash_combine(d.kind(), d.tag(), d.ReturnCount(), d.ParameterCount(), d.GPParameterCount(), d.FPParameterCount(), d.ParameterSlotCount(),
...
struct CallOp : OperationT<CallOp> {
    ...
    base::Vector<const MaybeRegisterRepresentation> inputs_rep(ZoneVector<MaybeRegisterRepresentation>& storage) const
    {
        storage.resize(input_count);
        size_t i = 0;
        if (descriptor->descriptor->IsCodeObjectCall() || descriptor->descriptor->IsJSFunctionCall() || descriptor->descriptor->IsBuiltinPointerCall()) {
    ...
    {
        return input<CallTarget>(0);
    }
    OptionalV<FrameState> frame_state() const
    {
        return HasFrameState() ? input<FrameState>(1) : OptionalV<FrameState>::Nullopt();
    }
    ...
    CallOp(
        V<CallTarget> callee, OptionalV<FrameState> frame_state, base::Vector<const OpIndex> arguments, const TSCallDescriptor* descriptor, OpEffects effects)
        ...
        , callee_effects(effects)
    {
        ...
        inputs.SubVector(1 + frame_state.valid(), inputs.size()).OverwriteWith(arguments);
    }
    ...
    template <typename Fn, typename Mapper> V8_INLINE auto Explode(Fn fn, Mapper& mapper) const
    {
        ...
        return fn(mapped_callee, mapped_frame_state, base::VectorOf(mapped_arguments), descriptor, Effects());
    }
    ...
    static CallOp& New(Graph* graph, V<CallTarget> callee, OptionalV<FrameState> frame_state, base::Vector<const OpIndex> arguments,
        const TSCallDescriptor* descriptor, OpEffects effects)
    {
        return Base::New(graph, 1 + frame_state.valid() + arguments.size(), callee, frame_state, arguments, descriptor, effects);
    }
...
struct DidntThrowOp : FixedArityOperationT<1, DidntThrowOp> {
    ...
    explicit DidntThrowOp(
        OpIndex throwing_operation, bool has_catch_block, const base::Vector<const RegisterRepresentation>* results_rep, OpEffects throwing_op_effects)
        : Base(throwing_operation)
...
struct TailCallOp : OperationT<TailCallOp> {
    ...
    {
        return Base::New(graph, 1 + arguments.size(), callee, arguments, descriptor);
    }
    auto options() const
    {
...
struct UnreachableOp : FixedArityOperationT<0, UnreachableOp> {
    static constexpr OpEffects effects = OpEffects().CanDependOnChecks().CanLeaveCurrentFunction();
    base::Vector<const RegisterRepresentation> outputs_rep() const
    {
        return {};
    }
...
struct ReturnOp : OperationT<ReturnOp> {
    ...
    static ReturnOp& New(Graph* graph, V<Word32> pop_count, base::Vector<const OpIndex> return_values)
    {
...
struct GotoOp : FixedArityOperationT<0, GotoOp> {
    ...
    auto options() const
    {
        return std::tuple { destination, is_backedge };
    }
};
...
struct SwitchOp : FixedArityOperationT<1, SwitchOp> {
    struct Case {
        ...
        {
            return value == other.value && destination == other.destination && hint == other.hint;
        }
    };
    ...
    auto options() const
    {
        return std::tuple { cases, default_case, default_hint };
    }
};

template <> struct fast_hash<SwitchOp::Case> {
...
V8_EXPORT_PRIVATE base::SmallVector<Block*, 4> SuccessorBlocks(const Block& block, const Graph& graph);
...
struct TupleOp : OperationT<TupleOp> {
    ...
    explicit TupleOp(base::Vector<const V<Any>> inputs)
        : Base(inputs)
    {
    }
...
struct ProjectionOp : FixedArityOperationT<1, ProjectionOp> {
    ...
    auto options() const
    {
        return std::tuple { index, rep };
    }
};

struct CheckTurboshaftTypeOfOp : FixedArityOperationT<1, CheckTurboshaftTypeOfOp> {
    RegisterRepresentation rep;
    Type type;
    bool successful;

    static constexpr OpEffects effects = OpEffects().CanDependOnChecks().CanReadImmutableMemory().RequiredWhenUnused();
    base::Vector<const RegisterRepresentation> outputs_rep() const
    {
        return {};
    }
    ...
    auto options() const
    {
        return std::tuple { rep, type, successful };
    }
};
...
struct ObjectIsOp : FixedArityOperationT<1, ObjectIsOp> {
    ...
    static constexpr OpEffects effects = OpEffects().CanDependOnChecks().CanReadImmutableMemory();
    base::Vector<const RegisterRepresentation> outputs_rep() const
    {
        return RepVector<RegisterRepresentation::Word32()>();
    }
    ...
    auto options() const
    {
        return std::tuple { kind, input_assumptions };
    }
};
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, ObjectIsOp::Kind kind);
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, ObjectIsOp::InputAssumptions input_assumptions);
...
struct ObjectIsNumericValueOp : FixedArityOperationT<1, ObjectIsNumericValueOp> {
    NumericKind kind;
    ...
    auto options() const
    {
        return std::tuple { kind, input_rep };
    }
};
...
struct ConvertOp : FixedArityOperationT<1, ConvertOp> {
    ...
    auto options() const
    {
        return std::tuple { from, to };
    }
};
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, ConvertOp::Kind kind);

struct ConvertUntaggedToJSPrimitiveOp : FixedArityOperationT<1, ConvertUntaggedToJSPrimitiveOp> {
    ...
    ConvertUntaggedToJSPrimitiveOp(V<Untagged> input, JSPrimitiveKind kind, RegisterRepresentation input_rep, InputInterpretation input_interpretation,
        CheckForMinusZeroMode minus_zero_mode)
        : Base(input)
        , kind(kind)
        , input_rep(input_rep)
    ...
    void Validate(const Graph& graph) const
    {
        ...
        case JSPrimitiveKind::kBigInt:
            ...
            DCHECK_EQ(minus_zero_mode, CheckForMinusZeroMode::kDontCheckForMinusZero);
            break;
        ...
        case JSPrimitiveKind::kHeapNumber:
            DCHECK_IMPLIES(minus_zero_mode == CheckForMinusZeroMode::kCheckForMinusZero, input_rep == RegisterRepresentation::Float64());
            break;
        case JSPrimitiveKind::kHeapNumberOrUndefined:
            DCHECK_IMPLIES(minus_zero_mode == CheckForMinusZeroMode::kDontCheckForMinusZero, input_rep == RegisterRepresentation::Float64());
            break;
        ...
        case JSPrimitiveKind::kString:
            DCHECK_EQ(input_rep, WordRepresentation::Word32());
            DCHECK_EQ(input_interpretation, any_of(InputInterpretation::kCharCode, InputInterpretation::kCodePoint));
            break;
        }
    }
...
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, ConvertUntaggedToJSPrimitiveOp::JSPrimitiveKind kind);

struct ConvertUntaggedToJSPrimitiveOrDeoptOp : FixedArityOperationT<2, ConvertUntaggedToJSPrimitiveOrDeoptOp> {
    enum class JSPrimitiveKind : uint8_t {
        kSmi,
    };
    enum class InputInterpretation : uint8_t {
    ...
    ConvertUntaggedToJSPrimitiveOrDeoptOp(V<Untagged> input, V<FrameState> frame_state, JSPrimitiveKind kind, RegisterRepresentation input_rep,
...
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, ConvertUntaggedToJSPrimitiveOrDeoptOp::JSPrimitiveKind kind);
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, ConvertUntaggedToJSPrimitiveOrDeoptOp::InputInterpretation input_interpretation);

struct ConvertJSPrimitiveToUntaggedOp : FixedArityOperationT<1, ConvertJSPrimitiveToUntaggedOp> {
    ...
    {
        return Base::input<JSPrimitive>(0);
    }
...
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, ConvertJSPrimitiveToUntaggedOp::UntaggedKind kind);
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, ConvertJSPrimitiveToUntaggedOp::InputAssumptions input_assumptions);

struct ConvertJSPrimitiveToUntaggedOrDeoptOp : FixedArityOperationT<2, ConvertJSPrimitiveToUntaggedOrDeoptOp> {
    enum class UntaggedKind : uint8_t {
        kInt32,
        kInt64,
        kFloat64,
    ...
    base::Vector<const RegisterRepresentation> outputs_rep() const
    {
        ...
        case UntaggedKind::kArrayIndex:
            return Is64() ? RepVector<RegisterRepresentation::Word64()>() : RepVector<RegisterRepresentation::Word32()>();
        }
    }
    ...
    ConvertJSPrimitiveToUntaggedOrDeoptOp(V<Object> input, V<FrameState> frame_state, JSPrimitiveKind from_kind, UntaggedKind to_kind,
...
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, ConvertJSPrimitiveToUntaggedOrDeoptOp::JSPrimitiveKind kind);
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, ConvertJSPrimitiveToUntaggedOrDeoptOp::UntaggedKind kind);

struct TruncateJSPrimitiveToUntaggedOp : FixedArityOperationT<1, TruncateJSPrimitiveToUntaggedOp> {
    enum class UntaggedKind : uint8_t {
        kInt32,
        kInt64,
        kBit,
...
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, TruncateJSPrimitiveToUntaggedOp::UntaggedKind kind);
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, TruncateJSPrimitiveToUntaggedOp::InputAssumptions input_assumptions);

struct TruncateJSPrimitiveToUntaggedOrDeoptOp : FixedArityOperationT<2, TruncateJSPrimitiveToUntaggedOrDeoptOp> {
    enum class UntaggedKind : uint8_t {
        kInt32,
    };
    using InputRequirement = ConvertJSPrimitiveToUntaggedOrDeoptOp::JSPrimitiveKind;
    UntaggedKind kind;
    ...
    TruncateJSPrimitiveToUntaggedOrDeoptOp(
        V<JSPrimitive> input, V<FrameState> frame_state, UntaggedKind kind, InputRequirement input_requirement, const FeedbackSource& feedback)
        : Base(input, frame_state)
        , kind(kind)
    ...
    auto options() const
    {
        return std::tuple { kind, input_requirement, feedback };
    }
};
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, TruncateJSPrimitiveToUntaggedOrDeoptOp::UntaggedKind kind);

struct ConvertJSPrimitiveToObjectOp : FixedArityOperationT<3, ConvertJSPrimitiveToObjectOp> {
    ...
    ConvertJSPrimitiveToObjectOp(V<JSPrimitive> value, V<Context> native_context, V<JSGlobalProxy> global_proxy, ConvertReceiverMode mode)
        : Base(value, native_context, global_proxy)
        , mode(mode)
    {
    }
...
struct NewConsStringOp : FixedArityOperationT<3, NewConsStringOp> {
    ...
    base::Vector<const MaybeRegisterRepresentation> inputs_rep(ZoneVector<MaybeRegisterRepresentation>& storage) const
    {
        return MaybeRepVector<MaybeRegisterRepresentation::Word32(), MaybeRegisterRepresentation::Tagged(), MaybeRegisterRepresentation::Tagged()>();
    }
    ...
    {
        return Base::input<String>(1);
    }
    V<String> second() const
    {
        return Base::input<String>(2);
    }
...
struct NewArrayOp : FixedArityOperationT<1, NewArrayOp> {
    ...
    auto options() const
    {
        return std::tuple { kind, allocation_type };
    }
};
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, NewArrayOp::Kind kind);
...
struct DoubleArrayMinMaxOp : FixedArityOperationT<1, DoubleArrayMinMaxOp> {
    ...
    DoubleArrayMinMaxOp(OpIndex array, Kind kind)
        : Base(array)
        , kind(kind)
    {
    }
...
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, DoubleArrayMinMaxOp::Kind kind);
...
struct LoadFieldByIndexOp : FixedArityOperationT<2, LoadFieldByIndexOp> {
    ...
    base::Vector<const MaybeRegisterRepresentation> inputs_rep(ZoneVector<MaybeRegisterRepresentation>& storage) const
    {
        return MaybeRepVector<MaybeRegisterRepresentation::Tagged(), MaybeRegisterRepresentation::Word32()>();
    }
...
struct DebugPrintOp : FixedArityOperationT<1, DebugPrintOp> {
    ...
    static constexpr OpEffects effects = OpEffects().CanChangeControlFlow().CanDependOnChecks().CanReadMemory().RequiredWhenUnused();
    base::Vector<const RegisterRepresentation> outputs_rep() const
    {
        return {};
    }
...
struct BigIntBinopOp : FixedArityOperationT<3, BigIntBinopOp> {
    ...
    V<BigInt> left() const
    {
        return Base::input<BigInt>(0);
    }
    V<BigInt> right() const
    {
        return Base::input<BigInt>(1);
    }
    V<FrameState> frame_state() const
    {
        return Base::input<FrameState>(2);
    }
...
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, BigIntBinopOp::Kind kind);
...
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, BigIntComparisonOp::Kind kind);
...
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, BigIntUnaryOp::Kind kind);
...
struct StringAtOp : FixedArityOperationT<2, StringAtOp> {
    ...
    base::Vector<const MaybeRegisterRepresentation> inputs_rep(ZoneVector<MaybeRegisterRepresentation>& storage) const
    {
        return MaybeRepVector<MaybeRegisterRepresentation::Tagged(), MaybeRegisterRepresentation::WordPtr()>();
    }
...
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, StringAtOp::Kind kind);
...
struct StringToCaseIntlOp : FixedArityOperationT<1, StringToCaseIntlOp> {
    ...
    StringToCaseIntlOp(V<String> string, Kind kind)
        : Base(string)
        , kind(kind)
    {
    }
...
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, StringToCaseIntlOp::Kind kind);
#endif // V8_INTL_SUPPORT
...
struct StringLengthOp : FixedArityOperationT<1, StringLengthOp> {
    ...
    explicit StringLengthOp(V<String> string)
        : Base(string)
    {
    }
...
struct StringIndexOfOp : FixedArityOperationT<3, StringIndexOfOp> {
    ...
    base::Vector<const MaybeRegisterRepresentation> inputs_rep(ZoneVector<MaybeRegisterRepresentation>& storage) const
    {
        return MaybeRepVector<MaybeRegisterRepresentation::Tagged(), MaybeRegisterRepresentation::Tagged(), MaybeRegisterRepresentation::Tagged()>();
    }
...
struct StringFromCodePointAtOp : FixedArityOperationT<2, StringFromCodePointAtOp> {
    static constexpr OpEffects effects = OpEffects()
                                             // String content is immutable, the allocated result does not have
                                             // identity.
                                             .CanAllocateWithoutIdentity()
...
struct StringSubstringOp : FixedArityOperationT<3, StringSubstringOp> {
    ...
    base::Vector<const MaybeRegisterRepresentation> inputs_rep(ZoneVector<MaybeRegisterRepresentation>& storage) const
    {
        return MaybeRepVector<MaybeRegisterRepresentation::Tagged(), MaybeRegisterRepresentation::Word32(), MaybeRegisterRepresentation::Word32()>();
    }
...
struct StringConcatOp : FixedArityOperationT<2, StringConcatOp> {
    ...
    StringConcatOp(V<String> left, V<String> right)
        : Base(left, right)
    {
    }
...
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, StringComparisonOp::Kind kind);
...
struct ArgumentsLengthOp : FixedArityOperationT<0, ArgumentsLengthOp> {
    ...
    Kind kind;
    int formal_parameter_count = 0; // This field is unused for kind == kArguments.
    ...
    auto options() const
    {
        return std::tuple { kind, formal_parameter_count };
    }
};
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, ArgumentsLengthOp::Kind kind);

struct NewArgumentsElementsOp : FixedArityOperationT<1, NewArgumentsElementsOp> {
    ...
    auto options() const
    {
        return std::tuple { type, formal_parameter_count };
    }
};

inline constexpr RegisterRepresentation RegisterRepresentationForArrayType(ExternalArrayType array_type)
{
    switch (array_type) {
...
inline base::Vector<const RegisterRepresentation> VectorForRep(RegisterRepresentation rep)
{
    static constexpr std::array<RegisterRepresentation, 6> table { RegisterRepresentation::Word32(), RegisterRepresentation::Word64(),
        RegisterRepresentation::Float32(), RegisterRepresentation::Float64(), RegisterRepresentation::Tagged(), RegisterRepresentation::Compressed() };
...
struct LoadTypedElementOp : FixedArityOperationT<4, LoadTypedElementOp> {
    ...
    base::Vector<const MaybeRegisterRepresentation> inputs_rep(ZoneVector<MaybeRegisterRepresentation>& storage) const
    {
        return MaybeRepVector<MaybeRegisterRepresentation::Tagged(), MaybeRegisterRepresentation::Tagged(), MaybeRegisterRepresentation::WordPtr(),
            MaybeRegisterRepresentation::WordPtr()>();
    }

    OpIndex buffer() const
    {
        return Base::input(0);
    }
    OpIndex base() const
    ...
    LoadTypedElementOp(OpIndex buffer, OpIndex base, OpIndex external, OpIndex index, ExternalArrayType array_type)
        : Base(buffer, base, external, index)
...
struct LoadStackArgumentOp : FixedArityOperationT<2, LoadStackArgumentOp> {
    ...
    base::Vector<const MaybeRegisterRepresentation> inputs_rep(ZoneVector<MaybeRegisterRepresentation>& storage) const
    {
        return MaybeRepVector<MaybeRegisterRepresentation::WordPtr(), MaybeRegisterRepresentation::WordPtr()>();
    }
...
struct StoreTypedElementOp : FixedArityOperationT<5, StoreTypedElementOp> {
    ...
    base::Vector<const MaybeRegisterRepresentation> inputs_rep(ZoneVector<MaybeRegisterRepresentation>& storage) const
    {
        return InitVectorOf(storage,
            { RegisterRepresentation::Tagged(), RegisterRepresentation::Tagged(), RegisterRepresentation::WordPtr(), RegisterRepresentation::WordPtr(),
    ...
    StoreTypedElementOp(OpIndex buffer, OpIndex base, OpIndex external, OpIndex index, OpIndex value, ExternalArrayType array_type)
        : Base(buffer, base, external, index, value)
        , array_type(array_type)
    {
    }
...
struct StoreDataViewElementOp : FixedArityOperationT<5, StoreDataViewElementOp> {
    ExternalArrayType element_type;
    ...
    {
        return InitVectorOf(storage,
            { RegisterRepresentation::Tagged(), RegisterRepresentation::Tagged(), RegisterRepresentation::WordPtr(),
                RegisterRepresentationForArrayType(element_type), RegisterRepresentation::Word32() });
    }
...
struct TransitionAndStoreArrayElementOp : FixedArityOperationT<3, TransitionAndStoreArrayElementOp> {
    enum class Kind : uint8_t {
        kElement,
        kNumberElement,
        kOddballElement,
    ...
    base::Vector<const MaybeRegisterRepresentation> inputs_rep(ZoneVector<MaybeRegisterRepresentation>& storage) const
    {
        return InitVectorOf(storage, { RegisterRepresentation::Tagged(), RegisterRepresentation::WordPtr(), value_representation() });
    }
    ...
    TransitionAndStoreArrayElementOp(
        OpIndex array, OpIndex index, OpIndex value, Kind kind, MaybeIndirectHandle<Map> fast_map, MaybeIndirectHandle<Map> double_map)
    ...
    {
        return kind == other.kind && fast_map.equals(other.fast_map) && double_map.equals(other.double_map);
    }

    auto options() const
    {
        return std::tuple { kind, fast_map, double_map };
    }
};

V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, TransitionAndStoreArrayElementOp::Kind kind);
...
struct CompareMapsOp : FixedArityOperationT<1, CompareMapsOp> {
    ...
    {
        return Base::input<HeapObject>(0);
    }
...
struct CheckMapsOp : FixedArityOperationT<2, CheckMapsOp> {
    ...
    static constexpr OpEffects effects = OpEffects().CanDependOnChecks().CanDeopt().CanReadHeapMemory().CanWriteHeapMemory();
    base::Vector<const RegisterRepresentation> outputs_rep() const
    {
        return {};
    }
    ...
    CheckMapsOp(V<HeapObject> heap_object, V<FrameState> frame_state, ZoneRefSet<Map> maps, CheckMapsFlags flags, const FeedbackSource& feedback)
    ...
    auto options() const
    {
        return std::tuple { maps, flags, feedback };
    }
    void PrintOptions(std::ostream& os) const;
};
...
struct AssumeMapOp : FixedArityOperationT<1, AssumeMapOp> {
    ...
    static constexpr OpEffects effects = OpEffects().CanDependOnChecks().CanReadHeapMemory().CanChangeControlFlow();
    base::Vector<const RegisterRepresentation> outputs_rep() const
    {
        return {};
    }
...
struct CheckEqualsInternalizedStringOp : FixedArityOperationT<3, CheckEqualsInternalizedStringOp> {
    static constexpr OpEffects effects = OpEffects().CanDeopt();
    base::Vector<const RegisterRepresentation> outputs_rep() const
    {
        return {};
...
struct LoadMessageOp : FixedArityOperationT<1, LoadMessageOp> {
    ...
    explicit LoadMessageOp(V<WordPtr> offset)
        : Base(offset)
    {
    }
...
struct StoreMessageOp : FixedArityOperationT<2, StoreMessageOp> {
    ...
    base::Vector<const MaybeRegisterRepresentation> inputs_rep(ZoneVector<MaybeRegisterRepresentation>& storage) const
    {
        return MaybeRepVector<MaybeRegisterRepresentation::WordPtr(), MaybeRegisterRepresentation::Tagged()>();
    }
...
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, SameValueOp::Mode mode);
...
struct Float64SameValueOp : FixedArityOperationT<2, Float64SameValueOp> {
    ...
    base::Vector<const MaybeRegisterRepresentation> inputs_rep(ZoneVector<MaybeRegisterRepresentation>& storage) const
    {
        return MaybeRepVector<MaybeRegisterRepresentation::Float64(), MaybeRegisterRepresentation::Float64()>();
    }
...
struct FastApiCallParameters : public NON_EXPORTED_BASE(ZoneObject) {
    ...
    {
        return c_functions[0].signature;
    }

    FastApiCallParameters(const FastApiCallFunctionVector& c_functions, const fast_api_call::OverloadsResolutionResult& resolution_result)
    ...
    static const FastApiCallParameters* Create(
        const FastApiCallFunctionVector& c_functions, const fast_api_call::OverloadsResolutionResult& resolution_result, Zone* graph_zone)
    {
        return graph_zone->New<FastApiCallParameters>(std::move(c_functions), resolution_result);
    }
};
...
struct FastApiCallOp : OperationT<FastApiCallOp> {
    ...
    base::Vector<const MaybeRegisterRepresentation> inputs_rep(ZoneVector<MaybeRegisterRepresentation>& storage) const
    {
        DCHECK_EQ(inputs().size(), kNumNonParamInputs + parameters->c_signature()->ArgumentCount());
    ...
    {
        const CTypeInfo& arg_type = parameters->c_signature()->ArgumentInfo(argument_index);
        uint8_t flags = static_cast<uint8_t>(arg_type.GetFlags());
        START_ALLOW_USE_DEPRECATED()
        switch (arg_type.GetSequenceType()) {
        case CTypeInfo::SequenceType::kScalar:
            if (flags & (static_cast<uint8_t>(CTypeInfo::Flags::kEnforceRangeBit) | static_cast<uint8_t>(CTypeInfo::Flags::kClampBit))) {
    ...
    FastApiCallOp(V<FrameState> frame_state, V<Object> data_argument, V<Context> context, base::Vector<const OpIndex> arguments,
        ...
        , lazy_deopt_on_throw(LazyDeoptOnThrow::kNo)
    {
        ...
        inputs[2] = context;
        inputs.SubVector(kNumNonParamInputs, kNumNonParamInputs + arguments.size()).OverwriteWith(arguments);
    }
    ...
    template <typename Fn, typename Mapper> V8_INLINE auto Explode(Fn fn, Mapper& mapper) const
    {
        ...
        return fn(mapped_frame_state, mapped_data_argument, mapped_context, base::VectorOf(mapped_arguments), parameters, out_reps);
    }
    ...
    static FastApiCallOp& New(Graph* graph, V<FrameState> frame_state, V<Object> data_argument, V<Context> context, base::Vector<const OpIndex> arguments,
        const FastApiCallParameters* parameters, base::Vector<const RegisterRepresentation> out_reps)
    {
        return Base::New(graph, kNumNonParamInputs + arguments.size(), frame_state, data_argument, context, arguments, parameters, out_reps);
    }
...
struct RuntimeAbortOp : FixedArityOperationT<0, RuntimeAbortOp> {
    ...
    explicit RuntimeAbortOp(AbortReason reason)
        : reason(reason)
    {
    }
...
struct EnsureWritableFastElementsOp : FixedArityOperationT<2, EnsureWritableFastElementsOp> {
...
struct MaybeGrowFastElementsOp : FixedArityOperationT<5, MaybeGrowFastElementsOp> {
    ...
    base::Vector<const MaybeRegisterRepresentation> inputs_rep(ZoneVector<MaybeRegisterRepresentation>& storage) const
    {
        return MaybeRepVector<MaybeRegisterRepresentation::Tagged(), MaybeRegisterRepresentation::Tagged(), MaybeRegisterRepresentation::Word32(),
            MaybeRegisterRepresentation::Word32()>();
    }
    ...
    {
        return Base::input<Word32>(3);
    }
    V<FrameState> frame_state() const
    {
        return Base::input<FrameState>(4);
    }

    MaybeGrowFastElementsOp(V<Object> object, V<Object> elements, V<Word32> index, V<Word32> elements_length, V<FrameState> frame_state,
    ...
    auto options() const
    {
        return std::tuple { mode, feedback };
    }
};

struct TransitionElementsKindOp : FixedArityOperationT<1, TransitionElementsKindOp> {
...
struct FindOrderedHashEntryOp : FixedArityOperationT<2, FindOrderedHashEntryOp> {
    enum class Kind : uint8_t {
    ...
    static constexpr OpEffects effects = OpEffects().CanDependOnChecks().CanReadMemory().CanAllocate();
    base::Vector<const RegisterRepresentation> outputs_rep() const
    {
        switch (kind) {
    ...
    base::Vector<const MaybeRegisterRepresentation> inputs_rep(ZoneVector<MaybeRegisterRepresentation>& storage) const
    {
        return kind == Kind::kFindOrderedHashMapEntryForInt32Key
            ? MaybeRepVector<MaybeRegisterRepresentation::Tagged(), MaybeRegisterRepresentation::Word32()>()
            : MaybeRepVector<MaybeRegisterRepresentation::Tagged(), MaybeRegisterRepresentation::Tagged()>();
    }
...
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, FindOrderedHashEntryOp::Kind kind);
...
struct CommentOp : FixedArityOperationT<0, CommentOp> {
    ...
    explicit CommentOp(const char* message)
        : message(message)
    {
    }
...
struct SpeculativeNumberBinopOp : FixedArityOperationT<3, SpeculativeNumberBinopOp> {
    enum class Kind : uint8_t {
        kSafeIntegerAdd,
    };
    ...
    SpeculativeNumberBinopOp(OpIndex left, OpIndex right, OpIndex frame_state, Kind kind)
        : Base(left, right, frame_state)
        , kind(kind)
    {
    }
...
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, SpeculativeNumberBinopOp::Kind kind);

#if V8_ENABLE_WEBASSEMBLY

V8_EXPORT_PRIVATE const RegisterRepresentation& RepresentationFor(wasm::ValueType type);
...
struct GlobalGetOp : FixedArityOperationT<1, GlobalGetOp> {
    ...
    GlobalGetOp(V<WasmTrustedInstanceData> instance, const wasm::WasmGlobal* global)
        : Base(instance)
        , global(global)
    {
    }
...
struct GlobalSetOp : FixedArityOperationT<2, GlobalSetOp> {
    ...
    explicit GlobalSetOp(V<WasmTrustedInstanceData> instance, V<Any> value, const wasm::WasmGlobal* global)
        : Base(instance, value)
        , global(global)
    {
    }
...
struct NullOp : FixedArityOperationT<0, NullOp> {
    ...
    explicit NullOp(wasm::ValueType type)
        : Base()
        , type(type)
    {
    }
...
struct IsNullOp : FixedArityOperationT<1, IsNullOp> {
    ...
    IsNullOp(V<Object> object, wasm::ValueType type)
        : Base(object)
        , type(type)
    {
    }
...
struct RttCanonOp : FixedArityOperationT<1, RttCanonOp> {
    ...
    V<FixedArray> rtts() const
    {
        return input<FixedArray>(0);
    }
...
struct WasmTypeCheckOp : OperationT<WasmTypeCheckOp> {
    ...
    WasmTypeCheckOp(V<Object> object, OptionalV<Map> rtt, WasmTypeCheckConfig config)
        : Base(1 + rtt.valid())
        , config(config)
    {
        input(0) = object;
    ...
    base::Vector<const MaybeRegisterRepresentation> inputs_rep(ZoneVector<MaybeRegisterRepresentation>& storage) const
    {
        return input_count > 1 ? MaybeRepVector<MaybeRegisterRepresentation::Tagged(), MaybeRegisterRepresentation::Tagged()>()
    ...
    static WasmTypeCheckOp& New(Graph* graph, V<Object> object, OptionalV<Map> rtt, WasmTypeCheckConfig config)
    {
...
struct WasmTypeCastOp : OperationT<WasmTypeCastOp> {
    ...
    WasmTypeCastOp(V<Object> object, OptionalV<Map> rtt, WasmTypeCheckConfig config)
        : Base(1 + rtt.valid())
        , config(config)
    {
        input(0) = object;
    ...
    static WasmTypeCastOp& New(Graph* graph, V<Object> object, OptionalV<Map> rtt, WasmTypeCheckConfig config)
    {
        return Base::New(graph, 1 + rtt.valid(), object, rtt, config);
    }
};
...
struct AnyConvertExternOp : FixedArityOperationT<1, AnyConvertExternOp> {
    static constexpr OpEffects effects = SmiValuesAre31Bits() ? OpEffects().CanReadMemory() : OpEffects().CanReadMemory().CanAllocate();

    explicit AnyConvertExternOp(V<Object> object)
        : Base(object)
    {
    }
...
struct ExternConvertAnyOp : FixedArityOperationT<1, ExternConvertAnyOp> {
    ...
    explicit ExternConvertAnyOp(V<Object> object)
        : Base(object)
    {
    }
...
struct StructGetOp : FixedArityOperationT<1, StructGetOp> {
    ...
    V<WasmStructNullable> object() const
    {
        return input<WasmStructNullable>(0);
    }
...
struct ArrayGetOp : FixedArityOperationT<2, ArrayGetOp> {
    ...
    ArrayGetOp(V<WasmArrayNullable> array, V<Word32> index, const wasm::ArrayType* array_type, bool is_signed)
        : Base(array, index)
        , is_signed(is_signed)
        , array_type(array_type)
    {
    ...
    V<WasmArrayNullable> array() const
    {
        return input<WasmArrayNullable>(0);
    }
    V<Word32> index() const
    {
        return input<Word32>(1);
    ...
    auto options() const
    {
        return std::tuple { array_type, is_signed };
    }
    void PrintOptions(std::ostream& os) const;
};
...
struct ArraySetOp : FixedArityOperationT<3, ArraySetOp> {
    ...
    ArraySetOp(V<WasmArrayNullable> array, V<Word32> index, V<Any> value, wasm::ValueType element_type)
        : Base(array, index, value)
        , element_type(element_type)
    {
    }
    ...
    base::Vector<const MaybeRegisterRepresentation> inputs_rep(ZoneVector<MaybeRegisterRepresentation>& storage) const
    {
        return InitVectorOf(storage, { RegisterRepresentation::Tagged(), RegisterRepresentation::Word32(), RepresentationFor(element_type) });
    }
...
struct WasmAllocateArrayOp : FixedArityOperationT<2, WasmAllocateArrayOp> {
    static constexpr OpEffects effects = OpEffects().CanAllocate().CanLeaveCurrentFunction();

    const wasm::ArrayType* array_type;

    explicit WasmAllocateArrayOp(V<Map> rtt, V<Word32> length, const wasm::ArrayType* array_type)
        : Base(rtt, length)
        , array_type(array_type)
    {
    }
...
struct WasmRefFuncOp : FixedArityOperationT<1, WasmRefFuncOp> {
    ...
    explicit WasmRefFuncOp(V<WasmTrustedInstanceData> wasm_instance, uint32_t function_index)
        : Base(wasm_instance)
...
struct StringAsWtf16Op : FixedArityOperationT<1, StringAsWtf16Op> {
    ...
    explicit StringAsWtf16Op(V<String> string)
        : Base(string)
    {
    }
...
struct StringPrepareForGetCodeUnitOp : FixedArityOperationT<1, StringPrepareForGetCodeUnitOp> {
    static constexpr OpEffects effects = OpEffects()
                                             // This should not float above a protective null/length check.
                                             .CanDependOnChecks();

    explicit StringPrepareForGetCodeUnitOp(V<Object> string)
        : Base(string)
    {
    }

    OpIndex string() const
    {
        return input(0);
    }
    ...
    base::Vector<const RegisterRepresentation> outputs_rep() const
    {
        return RepVector<RegisterRepresentation::Tagged(), RegisterRepresentation::WordPtr(), RegisterRepresentation::Word32()>();
    }
...
#define DEFINE_KIND(kind) k##kind,
        FOREACH_SIMD_128_BINARY_OPCODE(DEFINE_KIND) kFirstSignExtensionOp = kI16x8ExtMulLowI8x16S,
...
#undef DEFINE_KIND
    ...
    base::Vector<const MaybeRegisterRepresentation> inputs_rep(ZoneVector<MaybeRegisterRepresentation>& storage) const
    {
        return MaybeRepVector<RegisterRepresentation::Simd128(), RegisterRepresentation::Simd128()>();
    }
    ...
    V<Simd128> left() const
    {
        return input<Simd128>(0);
    }
    V<Simd128> right() const
    {
...
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, Simd128BinopOp::Kind kind);
...
#undef DEFINE_KIND
    ...
    Simd128UnaryOp(V<Simd128> input, Kind kind)
        : Base(input)
        , kind(kind)
    {
    }

    V<Simd128> input() const
    {
...
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, Simd128UnaryOp::Kind kind);
...
#undef DEFINE_KIND
    ...
    Simd128ReduceOp(V<Simd128> input, Kind kind)
        : Base(input)
        , kind(kind)
    {
    }
...
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, Simd128ReduceOp::Kind kind);
...
#undef DEFINE_KIND
    ...
    base::Vector<const MaybeRegisterRepresentation> inputs_rep(ZoneVector<MaybeRegisterRepresentation>& storage) const
    {
        return MaybeRepVector<RegisterRepresentation::Simd128(), RegisterRepresentation::Word32()>();
    }
...
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, Simd128ShiftOp::Kind kind);
...
#undef DEFINE_KIND
    ...
    Simd128TestOp(V<Simd128> input, Kind kind)
        : Base(input)
        , kind(kind)
    {
    }
...
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, Simd128TestOp::Kind kind);
...
#undef DEFINE_KIND
    ...
    Simd128SplatOp(V<Any> input, Kind kind)
        : Base(input)
        , kind(kind)
    {
    }
...
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, Simd128SplatOp::Kind kind);
...
#undef DEFINE_KIND
    ...
    base::Vector<const MaybeRegisterRepresentation> inputs_rep(ZoneVector<MaybeRegisterRepresentation>& storage) const
    {
        return MaybeRepVector<RegisterRepresentation::Simd128(), RegisterRepresentation::Simd128(), RegisterRepresentation::Simd128()>();
    }

    Simd128TernaryOp(V<Simd128> first, V<Simd128> second, V<Simd128> third, Kind kind)
        : Base(first, second, third)
        , kind(kind)
    {
    }

    V<Simd128> first() const
    {
        return input<Simd128>(0);
    }
    V<Simd128> second() const
    {
        return input<Simd128>(1);
    }
    V<Simd128> third() const
    {
...
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, Simd128TernaryOp::Kind kind);
...
struct Simd128ReplaceLaneOp : FixedArityOperationT<2, Simd128ReplaceLaneOp> {
    ...
    base::Vector<const MaybeRegisterRepresentation> inputs_rep(ZoneVector<MaybeRegisterRepresentation>& storage) const
    {
        return InitVectorOf(storage, { RegisterRepresentation::Simd128(), new_lane_rep() });
    }

    Simd128ReplaceLaneOp(V<Simd128> into, V<Any> new_lane, Kind kind, uint8_t lane)
        : Base(into, new_lane)
        , kind(kind)
        , lane(lane)
    {
    ...
    V<Simd128> into() const
    {
        return input<Simd128>(0);
    }
    V<Any> new_lane() const
    {
        return input<Any>(1);
    }
...
struct Simd128LaneMemoryOp : FixedArityOperationT<3, Simd128LaneMemoryOp> {
    ...
    OpEffects Effects() const
    {
        OpEffects effects = mode == Mode::kLoad ? OpEffects().CanReadMemory() : OpEffects().CanWriteMemory();
    ...
    base::Vector<const RegisterRepresentation> outputs_rep() const
    {
        return mode == Mode::kLoad ? RepVector<RegisterRepresentation::Simd128()>() : RepVector<>();
    }

    base::Vector<const MaybeRegisterRepresentation> inputs_rep(ZoneVector<MaybeRegisterRepresentation>& storage) const
    {
        return MaybeRepVector<RegisterRepresentation::WordPtr(), RegisterRepresentation::WordPtr(), RegisterRepresentation::Simd128()>();
    }

    Simd128LaneMemoryOp(OpIndex base, OpIndex index, OpIndex value, Mode mode, Kind kind, LaneKind lane_kind, uint8_t lane, int offset)
    ...
    {
        return 1 << static_cast<uint8_t>(lane_kind);
    }
...
struct Simd128LoadTransformOp : FixedArityOperationT<2, Simd128LoadTransformOp> {
...
#undef DEFINE_KIND
    ...
    base::Vector<const MaybeRegisterRepresentation> inputs_rep(ZoneVector<MaybeRegisterRepresentation>& storage) const
    {
        return MaybeRepVector<RegisterRepresentation::WordPtr(), RegisterRepresentation::WordPtr()>();
    }

    Simd128LoadTransformOp(V<WordPtr> base, V<WordPtr> index, LoadKind load_kind, TransformKind transform_kind, int offset)
        : Base(base, index)
        , load_kind(load_kind)
    ...
    auto options() const
    {
        return std::tuple { load_kind, transform_kind, offset };
    }
    void PrintOptions(std::ostream& os) const;
};
...
struct Simd128ShuffleOp : FixedArityOperationT<2, Simd128ShuffleOp> {
    ...
    Simd128ShuffleOp(V<Simd128> left, V<Simd128> right, const uint8_t incoming_shuffle[kSimd128Size])
        : Base(left, right)
    {
...
struct Simd256Extract128LaneOp : FixedArityOperationT<1, Simd256Extract128LaneOp> {
    uint8_t lane;
...
struct Simd256LoadTransformOp : FixedArityOperationT<2, Simd256LoadTransformOp> {
    using LoadKind = LoadOp::Kind;
    enum class TransformKind : uint8_t {
#define DEFINE_KIND(kind) k##kind,
...
#undef DEFINE_KIND
    ...
    Simd256LoadTransformOp(V<WordPtr> base, V<WordPtr> index, LoadKind load_kind, TransformKind transform_kind, int offset)
        : Base(base, index)
        , load_kind(load_kind)
        , transform_kind(transform_kind)
        , offset(offset)
...
#undef DEFINE_KIND
    ...
    Simd256UnaryOp(OpIndex input, Kind kind)
        : Base(input)
        , kind(kind)
    {
    }
...
#undef DEFINE_KIND
    ...
    base::Vector<const MaybeRegisterRepresentation> inputs_rep(ZoneVector<MaybeRegisterRepresentation>& storage) const
    {
        if (kind >= Kind::kFirstSignExtensionOp) {
            ...
            return MaybeRepVector<RegisterRepresentation::Simd256(), RegisterRepresentation::Simd256()>();
        }
    }
...
#undef DEFINE_KIND
    ...
    base::Vector<const MaybeRegisterRepresentation> inputs_rep(ZoneVector<MaybeRegisterRepresentation>& storage) const
    {
        return MaybeRepVector<RegisterRepresentation::Simd256(), RegisterRepresentation::Word32()>();
    }
...
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, Simd256ShiftOp::Kind kind);
...
#undef DEFINE_KIND
    ...
    base::Vector<const MaybeRegisterRepresentation> inputs_rep(ZoneVector<MaybeRegisterRepresentation>& storage) const
    {
        return MaybeRepVector<RegisterRepresentation::Simd256(), RegisterRepresentation::Simd256(), RegisterRepresentation::Simd256()>();
    }

    Simd256TernaryOp(V<Simd256> first, V<Simd256> second, V<Simd256> third, Kind kind)
        : Base(first, second, third)
        , kind(kind)
    {
    }

    V<Simd256> first() const
    {
        return input<Simd256>(0);
    }
    V<Simd256> second() const
    {
        return input<Simd256>(1);
    }
    V<Simd256> third() const
    {
...
#undef DEFINE_KIND
    ...
    Simd256SplatOp(OpIndex input, Kind kind)
        : Base(input)
        , kind(kind)
    {
    }
...
struct SimdPack128To256Op : FixedArityOperationT<2, SimdPack128To256Op> {
    ...
    SimdPack128To256Op(V<Simd128> left, V<Simd128> right)
        : Base(left, right)
    {
    }
    V<Simd128> left() const
...
struct Simd256ShufpsOp : FixedArityOperationT<2, Simd256ShufpsOp> {
    ...
    V<Simd256> left() const
    {
        return Base::input<Simd256>(0);
    }
    V<Simd256> right() const
    {
...
struct SetStackPointerOp : FixedArityOperationT<1, SetStackPointerOp> {
    ...
    explicit SetStackPointerOp(OpIndex value)
        : Base(value)
    {
    }
...
#ifdef V8_ENABLE_CONTINUATION_PRESERVED_EMBEDDER_DATA
struct GetContinuationPreservedEmbedderDataOp : FixedArityOperationT<0, GetContinuationPreservedEmbedderDataOp> {
    ...
    GetContinuationPreservedEmbedderDataOp()
        : Base()
    {
    }
...
struct SetContinuationPreservedEmbedderDataOp : FixedArityOperationT<1, SetContinuationPreservedEmbedderDataOp> {
...
static constexpr std::optional<OpEffects> kOperationEffectsTable[kNumberOfOpcodes] = { TURBOSHAFT_OPERATION_LIST(OPERATION_EFFECTS_CASE) };
#undef OPERATION_EFFECTS_CASE

template <class Op> const Opcode OperationT<Op>::opcode = operation_to_opcode<Op>::value;

template <Opcode opcode> struct opcode_to_operation_map { };

#define OPERATION_OPCODE_MAP_CASE(Name)                                                                                                                        \
    template <> struct opcode_to_operation_map<Opcode::k##Name> {                                                                                              \
        using Op = Name##Op;                                                                                                                                   \
    };
TURBOSHAFT_OPERATION_LIST(OPERATION_OPCODE_MAP_CASE)
#undef OPERATION_OPCODE_MAP_CASE

template <class Op, class = void> struct static_operation_input_count : std::integral_constant<uint32_t, 0> { };
template <class Op>
struct static_operation_input_count<Op, std::void_t<decltype(Op::inputs)>> : std::integral_constant<uint32_t, sizeof(Op::inputs) / sizeof(OpIndex)> { };
...
inline base::Vector<const OpIndex> Operation::inputs() const
{
    ...
    const OpIndex* ptr = reinterpret_cast<const OpIndex*>(reinterpret_cast<const char*>(this) + kOperationSizeTable[OpcodeIndex(opcode)]);
...
inline base::Vector<const RegisterRepresentation> Operation::outputs_rep() const
{
    switch (opcode) {
#define CASE(type)                                                                                                                                             \
    case Opcode::k##type: {                                                                                                                                    \
...
inline base::Vector<const MaybeRegisterRepresentation> Operation::inputs_rep(ZoneVector<MaybeRegisterRepresentation>& storage) const
{
    switch (opcode) {
#define CASE(type)                                                                                                                                             \
    case Opcode::k##type: {                                                                                                                                    \
...
template <typename T, typename = std::enable_if_t<std::is_enum_v<T> || std::is_integral_v<T> || std::is_floating_point_v<T>>> constexpr size_t input_count(T)
{
    return 0;
}
...
template <template <typename> typename HandleType, typename T,
    typename
    = std::enable_if_t<std::disjunction_v<std::is_convertible<HandleType<T>, DirectHandle<T>>, std::is_convertible<HandleType<T>, MaybeDirectHandle<T>>>>>
constexpr size_t input_count(const HandleType<T>)
{
    return 0;
}
template <typename T> constexpr size_t input_count(const base::Flags<T>)
{
    return 0;
}
...
inline size_t input_count(const ElementsTransition)
{
    return 0;
}
inline size_t input_count(const FeedbackSource)
{
    return 0;
}
inline size_t input_count(const ZoneRefSet<Map>)
{
    return 0;
}
inline size_t input_count(ConstantOp::Storage)
{
    return 0;
}
inline size_t input_count(Type)
{
    return 0;
}
...
#ifdef V8_ENABLE_WEBASSEMBLY
constexpr size_t input_count(const wasm::WasmGlobal*)
{
    return 0;
}
...
constexpr size_t input_count(const wasm::ArrayType*)
{
    return 0;
}
constexpr size_t input_count(wasm::ValueType)
{
    return 0;
}
...
constexpr size_t input_count(wasm::ModuleTypeIndex)
{
    return 0;
}
#endif
...
template <typename T> constexpr size_t input_count(base::Vector<const V<T>> inputs)
{
    return inputs.size();
}
} // namespace detail

template <typename Op, typename... Args> Op* CreateOperation(base::SmallVector<OperationStorageSlot, 32>& storage, Args... args)
{
...
template <typename F> auto VisitOperation(const Operation& op, F&& f)
{
    switch (op.opcode) {
#define CASE(name)                                                                                                                                             \
    case Opcode::k##name:                                                                                                                                      \
...
template <typename T, typename Tuple> struct TupleHasType;

template <typename T, typename... Ts> struct TupleHasType<T, std::tuple<Ts...>> {
    static constexpr bool value = (std::is_same_v<T, Ts> || ...);
};

template <typename Op, typename = void> struct ThrowingOpHasProperMembers : std::false_type { };
template <typename Op>
struct ThrowingOpHasProperMembers<Op, std::void_t<std::conjunction<decltype(Op::kOutputRepsStorage), decltype(Op::lazy_deopt_on_throw)>>> : std::true_type { };

template <typename Op, typename = void> struct ThrowingOpHasLazyDeoptOption : std::false_type { };

template <typename Op>
struct ThrowingOpHasLazyDeoptOption<Op, std::enable_if_t<TupleHasType<LazyDeoptOnThrow, decltype(std::declval<Op>().options())>::value>> : std::true_type { };

// CallOp has special handling because its outputs_rep are dynamic (and found on
// its call descriptor).
template <> struct ThrowingOpHasLazyDeoptOption<CallOp, void> : std::true_type { };
template <> struct ThrowingOpHasProperMembers<CallOp, void> : std::true_type { };

template <> struct ThrowingOpHasProperMembers<FastApiCallOp, void> : std::true_type { };
} // namespace details
