...
template <typename> class TypeInferenceReducer;

template <typename Next> class TypedOptimizationsReducer : public UniformReducerAdapter<TypedOptimizationsReducer, Next> {
    ...
    OpIndex ReduceInputGraphBranch(OpIndex ig_index, const BranchOp& operation)
    {
        if (!ShouldSkipOptimizationStep()) {
            ...
            if (!condition_type.IsInvalid()) {
                ...
                condition_type = Typer::TruncateWord32Input(condition_type, true, Asm().graph_zone());
    ...
    template <typename Op, typename Continuation> OpIndex ReduceInputGraphOperation(OpIndex ig_index, const Op& operation)
    {
        if (!ShouldSkipOptimizationStep()) {
            ...
            if (type.IsNone()) {
                ...
                if (OpIndex constant = TryAssembleConstantForType(type); constant.valid()) {
                    return constant;
                }
            }
        }
