...
const TSCallDescriptor* CreateAllocateBuiltinDescriptor(Zone* zone, Isolate* isolate)
{
    return TSCallDescriptor::Create(
        Linkage::GetStubCallDescriptor(zone, AllocateDescriptor {}, AllocateDescriptor {}.GetStackParameterCount(), CallDescriptor::kCanUseRoots,
            Operator::kNoThrow, isolate != nullptr ? StubCallMode::kCallCodeObject : StubCallMode::kCallBuiltinPointer),
...
void MemoryAnalyzer::Run()
{
    ...
    while (current_block < end) {
        ...
        auto operations_range = input_graph.operations(input_graph.Get(current_block));
...
void MemoryAnalyzer::ProcessBlockTerminator(const Operation& op)
{
    if (auto* goto_op = op.TryCast<GotoOp>()) {
        if (input_graph.IsLoopBackedge(*goto_op)) {
            std::optional<BlockState>& target_state = block_states[goto_op->destination->index()];
            ...
            for (const Operation& op : input_graph.operations(*goto_op->destination)) {
...
void MemoryAnalyzer::ProcessAllocation(const AllocateOp& alloc)
{
    ...
    if (auto* size = input_graph.Get(alloc.size()).template TryCast<ConstantOp>()) {
    ...
    if (allocation_folding == AllocationFolding::kDoAllocationFolding && state.last_allocation && new_size.has_value() && state.reserved_size.has_value()
        && alloc.type == state.last_allocation->type && *new_size <= kMaxRegularHeapObjectSize - *state.reserved_size) {
        state.reserved_size = static_cast<uint32_t>(*state.reserved_size + *new_size);
...
void MemoryAnalyzer::MergeCurrentStateIntoSuccessor(const Block* successor)
{
    ...
    // write barriers.
    if (target_state->reserved_size.has_value() && state.reserved_size.has_value()) {
        target_state->reserved_size = std::max(*target_state->reserved_size, *state.reserved_size);
    } else {
        target_state->reserved_size = std::nullopt;
    }
}
