...
template <class Next> class TypeInferenceReducer;
...
public:
    template <class Reducer> explicit DisableValueNumbering(Reducer* reducer)
    {
        if constexpr (reducer_list_contains<typename Reducer::ReducerList, ValueNumberingReducer>::value) {
...
template <class Next> class ValueNumberingReducer : public Next {
#if defined(__clang__)
    static_assert(next_is_bottom_of_assembler_stack<Next>::value || next_reducer_is<Next, TypeInferenceReducer>::value);
#endif

public:
    TURBOSHAFT_REDUCER_BOILERPLATE(ValueNumbering)

    template <typename Op> static constexpr bool CanBeGVNed()
    {
...
#undef EMIT_OP
    ...
    template <class Op> bool WillGVNOp(const Op& op)
    {
        Entry* entry = Find(op);
...
private:
    ...
    template <class Op> OpIndex AddOrFind(OpIndex op_idx)
    {
        if (is_disabled())
            return op_idx;
        ...
        if (std::is_same_v<Op, PendingLoopPhiOp> || op.IsBlockTerminator()
            || (!op.Effects().repetition_is_eliminatable() && !std::is_same_v<Op, DeoptimizeIfOp>)) {
        ...
        if (entry->IsEmpty()) {
            ...
            *entry = Entry { op_idx, Asm().current_block()->index(), hash, depths_heads_.back() };
    ...
    template <class Op> Entry* Find(const Op& op, size_t* hash_ret = nullptr)
    {
        ...
        for (size_t i = start_index;; i = NextEntryIndex(i)) {
            ...
            if (entry.hash == hash) {
                ...
                if (entry_op.Is<Op>() && (!same_block_only || entry.block == Asm().current_block()->index()) && entry_op.Cast<Op>().EqualsForGVN(op)) {
                    return &entry;
                }
            }
    ...
    void RehashIfNeeded()
    {
        if (V8_LIKELY(table_.size() - (table_.size() / 4) > entry_count_))
            return;
        base::Vector<Entry> new_table = table_ = Asm().phase_zone()->template NewVector<Entry>(table_.size() * 2);
    ...
    bool is_disabled()
    {
        return disabled_scope_.is_active();
    }

    ZoneVector<Block*> dominator_path_ { Asm().phase_zone() };
    base::Vector<Entry> table_
        = Asm().phase_zone()->template NewVector<Entry>(base::bits::RoundUpToPowerOfTwo(std::max<size_t>(128, Asm().input_graph().op_id_capacity() / 2)));
