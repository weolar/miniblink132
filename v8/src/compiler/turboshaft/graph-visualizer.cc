...
JSONTurboshaftGraphWriter::JSONTurboshaftGraphWriter(std::ostream& os, const Graph& turboshaft_graph, NodeOriginTable* origins, Zone* zone)
    : os_(os)
    , zone_(zone)
        ...
        for (const Operation& op : turboshaft_graph_.operations(block)) {
            ...
            if (auto* store = op.TryCast<StoreOp>()) {
                if (store->index().valid()) {
                    ...
                    inputs = { store->base(), store->index().value_or_invalid(), store->value() };
                }
            }
...
{
    JSONTurboshaftGraphWriter writer(os, ad.turboshaft_graph, ad.origins, ad.temp_zone);
    writer.Print();
    return os;
}
    ...
    std::ofstream& stream, const char* data_name, const Graph& graph, std::function<bool(std::ostream&, const Graph&, OpIndex)> printer)
{
    ...
    for (auto index : graph.AllOperationIndices()) {
        ...
        if (printer(sstream, graph, index)) {
            stream << (first ? "\n" : ",\n") << "{\"key\":" << index.id() << ", \"value\":\"" << sstream.str() << "\"}";
            first = false;
        }
    }
    stream << "]},\n";
