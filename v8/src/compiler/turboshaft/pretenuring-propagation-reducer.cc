...
void PretenuringPropagationAnalyzer::ProcessStore(const StoreOp& store)
{
    ...
    if (value.Is<AllocateOp>() && value.Cast<AllocateOp>().type == AllocationType::kOld) {
...
void PretenuringPropagationAnalyzer::ProcessAllocate(const AllocateOp& allocate)
{
...
{
    queue_.clear();
    if (!PushContainedValues(old_alloc))
        return;
    ...
    while (!queue_.empty()) {
        ...
        if (AllocateOp* alloc = op.TryCast<AllocateOp>()) {
            if (alloc->type == AllocationType::kOld)
                continue;
            alloc->type = AllocationType::kOld;
            PushContainedValues(idx);
        } else {
            DCHECK(op.Is<PhiOp>());
            if (old_phis_.find(idx) != old_phis_.end())
                continue;
