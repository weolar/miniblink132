    ...
    template <typename Derived> struct Descriptor {
        ...
        static void Verify(const CallDescriptor* desc)
        {
            ...
            if constexpr (std::is_same_v<result_t, void>) {
                ...
                DCHECK(result_t::allows_representation(RegisterRepresentation::FromMachineRepresentation(desc->GetReturnType(0).representation())));
            }
            DCHECK_EQ(desc->NeedsFrameState(), Derived::kNeedsFrameState);
            DCHECK_EQ(desc->properties(), Derived::kProperties);
            constexpr int additional_stub_arguments = 3; // function id, argument count, context (or NoContextConstant)
            DCHECK_EQ(desc->ParameterCount(), std::tuple_size_v<arguments_t> + additional_stub_arguments);
...
public:
    ...
    struct HandleNoHeapWritesInterrupts : public Descriptor<HandleNoHeapWritesInterrupts> {
    ...
    struct ThrowAccessedUninitializedVariable : public Descriptor<ThrowAccessedUninitializedVariable> {
        static constexpr auto kFunction = Runtime::kThrowAccessedUninitializedVariable;
        using arguments_t = std::tuple<V<Object>>;
    ...
    struct ThrowConstructorReturnedNonObject : public Descriptor<ThrowConstructorReturnedNonObject> {
        static constexpr auto kFunction = Runtime::kThrowConstructorReturnedNonObject;
        using arguments_t = std::tuple<>;
        // Doesn't actually return something, but the actual runtime call descriptor
        // (returned by Linkage::GetRuntimeCallDescriptor) returns 1 instead of 0.
        using result_t = Never;
    ...
    struct ThrowNotSuperConstructor : public Descriptor<ThrowNotSuperConstructor> {
    ...
    struct ThrowSuperAlreadyCalledError : public Descriptor<ThrowSuperAlreadyCalledError> {
    ...
    struct ThrowInvalidStringLength : public Descriptor<ThrowInvalidStringLength> {
