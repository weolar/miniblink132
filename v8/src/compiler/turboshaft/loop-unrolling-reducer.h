...
public:
    ...
    {
        return IterationCount(Kind::kUnknown);
    }
    ...
    explicit IterationCount(Kind kind)
        : kind_(kind)
    {
    ...
    bool IsExact() const
    {
        return kind_ == Kind::kExact;
    }
    bool IsApprox() const
    {
        return kind_ == Kind::kApprox;
    }
...
public:
    ...
    enum class BinOp { kAdd, kMul, kSub, kBitwiseAnd, kBitwiseOr, kBitwiseXor, kOverflowCheckedAdd, kOverflowCheckedMul, kOverflowCheckedSub };
...
private:
    ...
    IterationCount CountIterations(
        uint64_t equal_cst, CmpOp cmp_op, uint64_t initial_input, uint64_t binop_cst, BinOp binop_op, WordRepresentation binop_rep, bool loop_if_cond_is) const;
    template <class Int>
    IterationCount CountIterationsImpl(Int init, Int max, CmpOp cmp_op, Int binop_cst, StaticCanonicalForLoopMatcher::BinOp binop_op,
...
std::ostream& operator<<(std::ostream& os, const StaticCanonicalForLoopMatcher::CmpOp& cmp);
std::ostream& operator<<(std::ostream& os, const StaticCanonicalForLoopMatcher::BinOp& binop);
...
public:
    ...
    bool ShouldFullyUnrollLoop(const Block* loop_header) const
    {
        ...
        if (header_info.has_inner_loops)
            return false;
        if (header_info.op_count > kMaxLoopSizeForFullUnrolling)
            return false;

        auto iter_count = GetIterationCount(loop_header);
        return iter_count.IsExact() && iter_count.exact_count() < kMaxLoopIterationsForFullUnrolling;
    }
    ...
    bool ShouldPartiallyUnrollLoop(const Block* loop_header) const
    {
        ...
        return !info.has_inner_loops && info.op_count < kMaxLoopSizeForPartialUnrolling;
    }
    ...
    size_t GetPartialUnrollCount(const Block* loop_header) const
    {
        ...
        if (is_wasm_) {
            LoopFinder::LoopInfo info = loop_finder_.GetLoopInfo(loop_header);
            return std::min(LoopUnrollingAnalyzer::kMaxPartialUnrollingCount, LoopUnrollingAnalyzer::kWasmMaxUnrolledLoopSize / info.op_count);
        }
    ...
    IterationCount GetIterationCount(const Block* loop_header) const
    {
        ...
        if (it == loop_iteration_count_.end())
    ...
    {
        return can_unroll_at_least_one_loop_;
    }
...
private:
    ...
    const bool is_wasm_;
    const size_t kMaxLoopSizeForPartialUnrolling = is_wasm_ ? kWasmMaxLoopSizeForPartialUnrolling : kJSMaxLoopSizeForPartialUnrolling;
    bool can_unroll_at_least_one_loop_ = false;

    ZoneAbslFlatHashSet<uint32_t>& stack_checks_to_remove_;
};

template <class Next> class LoopPeelingReducer;

template <class Next> class LoopStackCheckElisionReducer : public Next {
public:
    ...
    V<AnyOrNone> REDUCE_INPUT_GRAPH(Call)(V<AnyOrNone> ig_idx, const CallOp& call)
    {
        ...
        if (skip_next_stack_check_ && call.IsStackCheck(__ input_graph(), broker_, StackCheckKind::kJSIterationBody)) {
    ...
    {
        if (skip_next_stack_check_ && stack_check.kind == JSStackCheckOp::Kind::kLoop) {
            skip_next_stack_check_ = false;
            return {};
        }
    ...
    V<None> REDUCE_INPUT_GRAPH(WasmStackCheck)(V<None> ig_idx, const WasmStackCheckOp& stack_check)
    {
        if (skip_next_stack_check_ && stack_check.kind == WasmStackCheckOp::Kind::kLoop) {
            skip_next_stack_check_ = false;
            return {};
        }
...
private:
    ...
    // should be removed.
    const ZoneAbslFlatHashSet<uint32_t>& stack_checks_to_remove_ = __ input_graph().stack_checks_to_remove();
    bool remove_stack_checks_ = !stack_checks_to_remove_.empty();

    JSHeapBroker* broker_ = __ data()->broker();
};

template <class Next> class LoopUnrollingReducer : public Next {
public:
    ...
    V<None> REDUCE_INPUT_GRAPH(Goto)(V<None> ig_idx, const GotoOp& gto)
    {
        ...
        if (unrolling_ == UnrollingStatus::kNotUnrolling && dst->IsLoop() && !gto.is_backedge) {
        ...
        } else if ((unrolling_ == UnrollingStatus::kUnrolling) && dst == current_loop_header_) {
    ...
    OpIndex REDUCE_INPUT_GRAPH(Branch)(OpIndex ig_idx, const BranchOp& branch)
    {
        ...
        if (unrolling_ == UnrollingStatus::kRemoveLoop) {
            ...
            bool is_false_in_loop = analyzer_.GetLoopHeader(branch.if_false) == header;
...
#if V8_ENABLE_WEBASSEMBLY
    V<None> REDUCE_INPUT_GRAPH(WasmStackCheck)(V<None> ig_idx, const WasmStackCheckOp& check)
    {
        if (ShouldSkipOptimizationStep() || !skip_next_stack_check_) {
...
private:
    ...
    LoopUnrollingAnalyzer& analyzer_ = *__ input_graph().loop_unrolling_analyzer();
...
template <class Next> void LoopUnrollingReducer<Next>::PartiallyUnrollLoop(const Block* header)
{
    ...
    ScopedModification<bool> set_true(__ turn_loop_without_backedge_into_merge(), false);
    ...
    Block* output_graph_header = __ CloneSubGraph(loop_body, /* keep_loop_kinds */ true);
    ...
    for (size_t i = 0; i < unroll_count - 1; i++) {
        ...
        ScopedModification<bool> skip_stack_checks(&skip_next_stack_check_, !is_last_iteration);
...
template <class Next> void LoopUnrollingReducer<Next>::FixLoopPhis(const Block* input_graph_loop, Block* output_graph_loop, const Block* backedge_block)
{
    ...
    for (const Operation& op : __ input_graph().operations(input_graph_loop->begin(), input_graph_loop->end())) {
        if (auto* input_phi = op.TryCast<PhiOp>()) {
            OpIndex phi_index = __ template MapToNewGraph<true>(__ input_graph().Index(*input_phi));
...
template <class Next> void LoopUnrollingReducer<Next>::RemoveLoop(const Block* header)
{
    DCHECK_EQ(unrolling_, UnrollingStatus::kNotUnrolling);
    DCHECK(!skip_next_stack_check_);
...
template <class Next> void LoopUnrollingReducer<Next>::FullyUnrollLoop(const Block* header)
{
    DCHECK_EQ(unrolling_, UnrollingStatus::kNotUnrolling);
    DCHECK(!skip_next_stack_check_);
    ScopedModification<bool> skip_stack_checks(&skip_next_stack_check_, true);
