...
template <class Key, class Value> class LayeredHashMap {
public:
...
LayeredHashMap<Key, Value>::LayeredHashMap(Zone* zone, uint32_t initial_capacity)
    : entry_count_(0)
    , depths_heads_(zone)
    , zone_(zone)
{
...
template <class Key, class Value> void LayeredHashMap<Key, Value>::StartLayer()
{
    depths_heads_.push_back(nullptr);
}

template <class Key, class Value> void LayeredHashMap<Key, Value>::DropLastLayer()
{
...
template <class Key, class Value> typename LayeredHashMap<Key, Value>::Entry* LayeredHashMap<Key, Value>::FindEntryForKey(Key key, size_t hash)
{
    for (size_t i = hash & mask_;; i = NextEntryIndex(i)) {
        if (table_[i].hash == 0)
            return &table_[i];
        if (table_[i].hash == hash && table_[i].key == key)
            return &table_[i];
    }
}

template <class Key, class Value> void LayeredHashMap<Key, Value>::InsertNewKey(Key key, Value value)
{
    ResizeIfNeeded();
...
template <class Key, class Value> std::optional<Value> LayeredHashMap<Key, Value>::Get(Key key)
{
...
template <class Key, class Value> bool LayeredHashMap<Key, Value>::Contains(Key key)
{
    return Get(key).has_value();
}

template <class Key, class Value> void LayeredHashMap<Key, Value>::ResizeIfNeeded()
{
    if (table_.size() * kNeedResizePercentage > entry_count_)
        return;
    CHECK_LE(table_.size(), std::numeric_limits<size_t>::max() / kGrowthFactor);
    table_ = zone_->NewVector<Entry>(table_.size() * kGrowthFactor);
    mask_ = table_.size() - 1;
    DCHECK_EQ(base::bits::CountPopulation(mask_), sizeof(mask_) * 8 - base::bits::CountLeadingZeros(mask_));
