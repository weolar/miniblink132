...
struct NoChangeCallback {
    template <class Key, class Value> void operator()(Key key, const Value& old_value, const Value& new_value) const
    {
    }
};

template <class Value, class KeyData> class SnapshotTable;

// Place `KeyData` in a superclass to benefit from empty-base optimization.
template <class Value, class KeyData> struct SnapshotTableEntry : KeyData {
    Value value;
    ...
    static constexpr uint32_t kNoMergeOffset = std::numeric_limits<uint32_t>::max();
    static constexpr uint32_t kNoMergedPredecessor = std::numeric_limits<uint32_t>::max();
};
...
template <class Value, class KeyData> class SnapshotTableKey {
public:
...
template <class Value, class KeyData = NoKeyData> class SnapshotTable {
private:
    struct LogEntry;
    struct SnapshotData;
...
public:
    ...
    private:
        ...
        SnapshotData* data_;
        explicit Snapshot(SnapshotData& data)
            : data_(&data)
        {
        }
        explicit Snapshot(SnapshotData* data)
    ...
    public:
        ...
        explicit MaybeSnapshot(Snapshot snapshot)
            : data_(snapshot.data_)
        {
        }
    ...
    template <class ChangeCallback = NoChangeCallback, std::enable_if_t<std::is_invocable_v<ChangeCallback, Key, Value, Value>>* = nullptr>
    ...
    template <class MergeFun, class ChangeCallback = NoChangeCallback,
        std::enable_if_t<std::is_invocable_v<MergeFun, Key, base::Vector<const Value>> && std::is_invocable_v<ChangeCallback, Key, Value, Value>>* = nullptr>
    ...
    const Value& GetPredecessorValue(Key key, int predecessor_index)
    {
        ...
        if (key.entry_->merge_offset == kNoMergeOffset)
            return Get(key);
    ...
    explicit SnapshotTable(Zone* zone)
        : zone_(zone)
    {
    ...
    {
        return Key { table_.emplace_back(TableEntry { std::move(initial_value), std::move(data) }) };
    }
    ...
    bool IsSealed()
    {
        return current_snapshot_->IsSealed();
    }
...
private:
    ...
    template <class ChangeCallback = NoChangeCallback> void RevertCurrentSnapshot(ChangeCallback& change_callback)
    {
        DCHECK(current_snapshot_->IsSealed());
    ...
    template <class ChangeCallback = NoChangeCallback> void ReplaySnapshot(SnapshotData* snapshot, ChangeCallback& change_callback)
    {
    ...
    template <class ChangeCallback> SnapshotData& MoveToNewSnapshot(base::Vector<const Snapshot> predecessors, const ChangeCallback& change_callback);
...
template <class Value, class KeyData> struct SnapshotTable<Value, KeyData>::LogEntry {
    TableEntry& table_entry;
    Value old_value;
    Value new_value;
};

template <class Value, class KeyData> struct SnapshotTable<Value, KeyData>::SnapshotData {
    SnapshotData* parent;
...
void SnapshotTable<Value, KeyData>::RecordMergeValue(TableEntry& entry, const Value& value, uint32_t predecessor_index, uint32_t predecessor_count)
{
    ...
    if (entry.merge_offset == kNoMergeOffset) {
        ...
        CHECK_LE(merge_values_.size() + predecessor_count, std::numeric_limits<uint32_t>::max());
...
typename SnapshotTable<Value, KeyData>::SnapshotData& SnapshotTable<Value, KeyData>::MoveToNewSnapshot(
    base::Vector<const Snapshot> predecessors, const ChangeCallback& change_callback)
{
    DCHECK_WITH_MSG(current_snapshot_->IsSealed(), "A new Snapshot was opened before the previous Snapshot was sealed");
...
void SnapshotTable<Value, KeyData>::MergePredecessors(
    base::Vector<const Snapshot> predecessors, const MergeFun& merge_fun, const ChangeCallback& change_callback)
{
    ...
    for (uint32_t i = 0; i < predecessor_count; ++i) {
        for (SnapshotData* predecessor = predecessors[i].data_; predecessor != common_ancestor; predecessor = predecessor->parent) {
            base::Vector<LogEntry> log_entries = LogEntries(predecessor);
            for (const LogEntry& entry : base::Reversed(log_entries)) {
                RecordMergeValue(entry.table_entry, entry.new_value, i, predecessor_count);
            }
        }
    }
    ...
    for (TableEntry* entry : merging_entries_) {
        Key key { *entry };
        Value value = merge_fun(key, base::VectorOf<const Value>(&merge_values_[entry->merge_offset], predecessor_count));
...
template <class Derived, class Value, class KeyData = NoKeyData> class ChangeTrackingSnapshotTable : public SnapshotTable<Value, KeyData> {
public:
    ...
    {
        Super::StartNewSnapshot(predecessors,
            [this](Key key, const Value& old_value, const Value& new_value) { static_cast<Derived*>(this)->OnValueChange(key, old_value, new_value); });
    }
    ...
    {
        StartNewSnapshot({ parent });
    }
    template <class MergeFun, std::enable_if_t<std::is_invocable_v<MergeFun, Key, base::Vector<const Value>>>* = nullptr>
    void StartNewSnapshot(base::Vector<const Snapshot> predecessors, const MergeFun& merge_fun)
    {
        Super::StartNewSnapshot(predecessors, merge_fun,
            [this](Key key, const Value& old_value, const Value& new_value) { static_cast<Derived*>(this)->OnValueChange(key, old_value, new_value); });
    }
    template <class MergeFun, std::enable_if_t<std::is_invocable_v<MergeFun, Key, base::Vector<const Value>>>* = nullptr>
    ...
    void Set(Key key, Value new_value)
    {
        ...
        if (Super::Set(key, std::move(new_value))) {
            static_cast<Derived*>(this)->OnValueChange(key, old_value, Super::Get(key));
        }
    }
