...
struct DecompressionAnalyzer {
    ...
    template <bool is_loop> void ProcessBlock(const Block& block, int32_t* next_block_id)
    {
        for (const Operation& op : base::Reversed(graph.operations(block))) {
            if (is_loop && op.Is<PhiOp>() && NeedsDecompression(op)) {
                ...
                if (!NeedsDecompression(phi.input(1))) {
                    ...
                    *next_block_id = std::max<int32_t>(*next_block_id, backedge->index().id());
                }
            }
            ProcessOperation(op);
        }
...
void DecompressionAnalyzer::ProcessOperation(const Operation& op)
{
    ...
    case Opcode::kTaggedBitcast: {
        ...
        if (bitcast.kind != TaggedBitcastOp::Kind::kSmi && NeedsDecompression(op)) {
    ...
    case Opcode::kLoad: {
        ...
        if (DECOMPRESS_POINTER_BY_ADDRESSING_MODE && !load.index().valid() && graph.Get(load.base()).saturated_use_count.IsOne()) {
...
void DecompressionAnalyzer::MarkAddressingBase(OpIndex base_idx)
{
    ...
    if (const LoadOp* load = base.TryCast<LoadOp>(); load && load->loaded_rep.IsCompressibleTagged()) {
    ...
    if (base.Is<PhiOp>()) {
        ...
        for (OpIndex input_idx : base.inputs()) {
            ...
            if (!input.Is<LoadOp>() || !base.IsOnlyUserOf(input, graph) || !input.Cast<LoadOp>().loaded_rep.IsCompressibleTagged()) {
...
void RunDecompressionOptimization(Graph& graph, Zone* phase_zone)
{
    ...
    for (OpIndex op_idx : analyzer.candidates) {
        ...
        if (analyzer.NeedsDecompression(op))
            continue;
        switch (op.opcode) {
        case Opcode::kConstant: {
        ...
        case Opcode::kLoad: {
            ...
            if (load.loaded_rep.IsCompressibleTagged()) {
                DCHECK_EQ(load.result_rep, any_of(RegisterRepresentation::Tagged(), RegisterRepresentation::Compressed()));
        ...
        case Opcode::kTaggedBitcast: {
            ...
            if (bitcast.from == RegisterRepresentation::Tagged()
                && (bitcast.to == RegisterRepresentation::WordPtr() || bitcast.kind == TaggedBitcastOp::Kind::kSmi)) {
