...
template <class Next> class LoopUnrollingReducer;
...
template <class Next> class LoopPeelingReducer : public Next {
public:
    ...
    static_assert(!reducer_list_contains<ReducerList, LoopUnrollingReducer>::value);
#endif
    ...
    V<AnyOrNone> REDUCE_INPUT_GRAPH(Call)(V<AnyOrNone> ig_idx, const CallOp& call)
    {
        ...
        if (IsPeeling() && call.IsStackCheck(__ input_graph(), broker_, StackCheckKind::kJSIterationBody)) {
    ...
    {
        if (!IsEmittingUnpeeledBody() || __ current_input_block() != current_loop_header_) {
        ...
        return __ PendingLoopPhi(__ MapToNewGraph(phi.input(PhiOp::kLoopPhiBackEdgeIndex)), phi.rep);
    }
...
private:
    ...
    void PeelFirstIteration(const Block* header)
    {
        ...
        ScopedModification<PeelingStatus> scope(&peeling_, PeelingStatus::kEmittingPeeledLoop);
