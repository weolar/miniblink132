...
public:
    ...
    StringView(const DisallowGarbageCollection& can_rely_on_no_gc, V<String> string, String::Encoding encoding, ConstOrV<WordPtr> start_index = 0,
    ...
    template <typename A> iterator_type Begin(A& assembler)
    {
        ...
        V<WordPtr> begin_offset = assembler.WordPtrAdd(assembler.BitcastTaggedToWordPtr(string_),
            assembler.WordPtrAdd(data_offset - kHeapObjectTag, assembler.WordPtrMul(assembler.resolve(start_index_), stride)));
        V<WordPtr> count;
        ...
        end_offset_ = assembler.WordPtrAdd(begin_offset, assembler.WordPtrMul(count, stride));
        return begin_offset;
    }
    ...
    {
        const auto loaded_rep = encoding_ == String::ONE_BYTE_ENCODING ? MemoryRepresentation::Uint8() : MemoryRepresentation::Uint16();
        return assembler.Load(current_iterator, LoadOp::Kind::RawAligned(), loaded_rep);
    }
