...
template <class Next> class BranchEliminationReducer : public Next {
    // # General overview
    //
...
public:
    ...
    void Bind(Block* new_block)
    {
        ...
        if (new_block->IsBranchTarget()) {
            ...
            const Operation& op = new_block->LastPredecessor()->LastOperation(__ output_graph());
    ...
    OpIndex REDUCE(Branch)(OpIndex cond, Block* if_true, Block* if_false, BranchHint hint)
    {
        ...
        if (const Block* if_true_origin = __ OriginForBlockStart(if_true)) {
            if (const Block* if_false_origin = __ OriginForBlockStart(if_false)) {
                const Operation& first_op_true = if_true_origin->FirstOperation(__ input_graph());
                const Operation& first_op_false = if_false_origin->FirstOperation(__ input_graph());
                ...
                if (true_goto && false_goto && true_goto->destination == false_goto->destination) {
    ...
    V<None> REDUCE(Goto)(Block* destination, bool is_backedge)
    {
        ...
        const Operation& last_op = destination_origin->LastOperation(__ input_graph());
        ...
        if (const BranchOp* branch = last_op.template TryCast<BranchOp>()) {
            V<Word32> condition = __ template MapToNewGraph<true>(branch->condition());
            if (condition.valid()) {
                ...
                if (destination_origin->Contains(branch->condition())) {
                    if (__ input_graph().Get(branch->condition()).template Is<PhiOp>()) {
                        __ CloneBlockAndGoto(destination_origin);
                        return {};
                    } else if (CanBeConstantFolded(branch->condition(), destination_origin)) {
            ...
            // [...]                   [...]
            // ReturnOp
            if (Asm().current_block()->PredecessorCount() == 1 && Asm().current_block()->begin() == __ output_graph().next_operation_index()) {
                const Block* prev_block = Asm().current_block()->LastPredecessor();
                if (prev_block->LastOperation(__ output_graph()).template Is<SwitchOp>()) {
                    goto no_change;
                }
            }
    ...
    V<None> REDUCE(TrapIf)(V<Word32> condition, OptionalV<FrameState> frame_state, bool negated, const TrapId trap_id)
    {
        LABEL_BLOCK(no_change)
        {
...
private:
    ...
    void ResetToBlock(Block* block)
    {
        ...
        while (!dominator_path_.empty() && target != nullptr && dominator_path_.back() != target) {
  ...
  //               B7    B8
    ...
    void ReplayMissingPredecessors(Block* new_block)
    {
        ...
        for (Block* dom = new_block->GetDominator(); dom != nullptr && dom != dominator_path_.back(); dom = dom->GetDominator()) {
        ...
        for (auto it = missing_blocks.rbegin(); it != missing_blocks.rend(); ++it) {
            ...
            if (block->IsBranchTarget()) {
                const Operation& op = block->LastPredecessor()->LastOperation(__ output_graph());
                if (const BranchOp* branch = op.TryCast<BranchOp>()) {
                    DCHECK(branch->if_true->index() == block->index() || branch->if_false->index() == block->index());
                    bool condition_value
                        = branch->if_true->index().valid() ? branch->if_true->index() == block->index() : branch->if_false->index() != block->index();
                    known_conditions_.InsertNewKey(branch->condition(), condition_value);
                }
            }
        }
    ...
    bool CanBeConstantFolded(OpIndex idx, const Block* cond_input_block, bool has_phi = false, int depth = 0)
    {
        ...
        if (op.Is<PhiOp>()) {
            int curr_block_pred_idx = cond_input_block->GetPredecessorIndex(__ current_block()->OriginForBlockEnd());
            // There is no need to increment {depth} on this recursive call, because
            // it will anyways exit early because {idx} won't be in
            // {cond_input_block}.
            return CanBeConstantFolded(op.input(curr_block_pred_idx), cond_input_block, /*has_phi*/ true, depth);
        ...
        for (int i = 0; i < op.input_count; i++) {
            if (!CanBeConstantFolded(op.input(i), cond_input_block, has_phi, depth + 1)) {
                return false;
            }
        }
    ...
    LayeredHashMap<OpIndex, bool> known_conditions_ { __ phase_zone(), __ input_graph().DominatorTreeDepth() * 2 };
};
