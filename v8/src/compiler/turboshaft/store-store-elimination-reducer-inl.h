...
inline std::ostream& operator<<(std::ostream& os, StoreObservability observability)
{
...
class MaybeRedundantStoresTable : public ChangeTrackingSnapshotTable<MaybeRedundantStoresTable, StoreObservability, MaybeRedundantStoresKeyData> {
    using super = ChangeTrackingSnapshotTable<MaybeRedundantStoresTable, StoreObservability, MaybeRedundantStoresKeyData>;
...
public:
    ...
    void BeginBlock(const Block* block)
    {
        ...
        // Collect the snapshots of all successors.
        {
            ...
            for (const Block* s : successors) {
                std::optional<Snapshot> s_snapshot = block_to_snapshot_mapping_[s->index()];
        ...
        // successors.
        StartNewSnapshot(base::VectorOf(successor_snapshots_), [](Key, base::Vector<const StoreObservability> successors) {
            return static_cast<StoreObservability>(*std::max_element(successors.begin(), successors.end()));
        });
    ...
    void Seal(bool* snapshot_has_changed = nullptr)
    {
        ...
        if (!snapshot_has_changed) {
            ...
            *snapshot_has_changed = false;
            StartNewSnapshot(base::VectorOf({ snapshot.value(), new_snapshot }), [&](Key key, base::Vector<const StoreObservability> successors) {
    ...
    void Print(std::ostream& os, const char* sep = "\n") const
    {
        bool first = true;
        for (Key key : active_keys_) {
            os << (first ? "" : sep) << key.data().base.id() << "@" << key.data().offset << ": " << Get(key);
            first = false;
        }
    }
...
private:
    Key map_to_key(OpIndex base, int32_t offset, uint8_t size)
    {
        ...
        if (it != key_mapping_.end())
            return it->second;
        Key new_key = NewKey(MaybeRedundantStoresKeyData { base, offset, size }, StoreObservability::kObservable);
                ...
                if (is_on_heap_store && is_field_store) {
                    ...
                    case StoreObservability::kGCObservable:
                        if (store.maybe_initializing_or_transitioning) {
                            ...
                            table_.MarkStoreAsUnobservable(store.base(), store.offset, size);
                        } else {
                            eliminable_stores_->insert(index);
                            last_field_initialization_store_ = OpIndex::Invalid();
                            is_eliminable_store = true;
                    ...
                    // one.
                    if (COMPRESS_POINTERS_BOOL && !is_eliminable_store && store.maybe_initializing_or_transitioning && store.kind == StoreOp::Kind::TaggedBase()
                        && store.write_barrier == WriteBarrierKind::kNoWriteBarrier && store.stored_rep.IsCompressibleTagged()) {
                        if (last_field_initialization_store_.valid() && graph_.NextIndex(index) == last_field_initialization_store_) {
                            const StoreOp& store0 = store;
                            const StoreOp& store1 = graph_.Get(last_field_initialization_store_).Cast<StoreOp>();

                            DCHECK(!store0.index().valid());
                            DCHECK(!store1.index().valid());

                            const ConstantOp* c0 = graph_.Get(store0.value()).TryCast<ConstantOp>();
                            const ConstantOp* c1 = graph_.Get(store1.value()).TryCast<ConstantOp>();
                            ...
                            // instance).
                            if (c0 && c1 && c0->kind == ConstantOp::Kind::kHeapObject && c1->kind == ConstantOp::Kind::kHeapObject
                                && store1.offset - store0.offset == 4 && HeapLayout::InReadOnlySpace(*c0->handle())
...
template <class Next> class StoreStoreEliminationReducer : public Next {
public:
    ...
    OpIndex REDUCE_INPUT_GRAPH(Store)(OpIndex ig_index, const StoreOp& store)
    {
        if (eliminable_stores_.count(ig_index) > 0) {
            ...
            __ Store(__ MapToNewGraph(store.base()), value, StoreOp::Kind::TaggedBase(), MemoryRepresentation::Uint64(), WriteBarrierKind::kNoWriteBarrier,
                store.offset);
            return OpIndex::Invalid();
        }
