...
public:
    ...
    {
        return revectorized_node_;
    }
    void SetRevectorizedNode(V<Simd256> node)
    {
        revectorized_node_ = node;
    }
    ...
    void set_force_pack_type(ForcePackType type)
    {
        force_pack_type_ = type;
    }
    ForcePackType force_pack_type()
    {
        return force_pack_type_;
    }
...
public:
    ...
    public:
        ...
        {
            DCHECK(kind_ == Kind::kS256Load32Transform || kind_ == Kind::kS256Load64Transform);
...
public:
    ...
    {
        return is_sign_extract_;
    }
    bool is_sign_convert() const
    {
        return is_sign_convert_;
    }
...
private:
    ...
    PackNode* NewForcePackNode(const NodeGroup& node_group, PackNode::ForcePackType type, const Graph& graph);
    ...
    ShufflePackNode* X64TryMatch256Shuffle(const NodeGroup& node_group, const uint8_t* shuffle0, const uint8_t* shuffle1);
#endif // V8_TARGET_ARCH_X64
...
template <class Next> class WasmRevecReducer : public UniformReducerAdapter<WasmRevecReducer, Next> {
public:
    TURBOSHAFT_REDUCER_BOILERPLATE(WasmRevec)
    using Adapter = UniformReducerAdapter<WasmRevecReducer, Next>;

    OpIndex GetExtractOpIfNeeded(PackNode* pnode, OpIndex ig_index, OpIndex og_index)
    {
        const auto lane = base::checked_cast<uint8_t>(std::find(pnode->nodes().begin(), pnode->nodes().end(), ig_index) - pnode->nodes().begin());

        // Force PackNode has a dedicated use in SimdPack128To256Op.
        if (pnode->is_force_pack()) {
            SimdPack128To256Op& op = __ output_graph().Get(pnode->RevectorizedNode()).template Cast<SimdPack128To256Op>();
        ...
        for (auto use : analyzer_.uses(ig_index)) {
            ...
            if (use_pnode != nullptr && !use_pnode->is_force_pack()) {
                ...
                if (__ input_graph().Get(use).opcode != Opcode::kStore || use_pnode->nodes()[0] != use || use_pnode->nodes()[0] > use_pnode->nodes()[1])
                    continue;
            }
    ...
    V<Simd128> REDUCE_INPUT_GRAPH(Simd128Constant)(V<Simd128> ig_index, const Simd128ConstantOp& constant_op)
    {
        if (auto pnode = analyzer_.GetPackNode(ig_index)) {
            ...
            if (!og_index.valid()) {
                ...
                const Simd128ConstantOp& op0 = __ input_graph().Get(inputs[0]).template Cast<Simd128ConstantOp>();
                const Simd128ConstantOp& op1 = __ input_graph().Get(inputs[1]).template Cast<Simd128ConstantOp>();
    ...
    V<Simd128> REDUCE_INPUT_GRAPH(Simd128LoadTransform)(V<Simd128> ig_index, const Simd128LoadTransformOp& load_transform)
    {
        if (auto pnode = analyzer_.GetPackNode(ig_index)) {
            if (pnode->is_force_pack()) {
                return Adapter::ReduceInputGraphSimd128LoadTransform(ig_index, load_transform);
            }
            V<Simd256> og_index = pnode->RevectorizedNode();
            // Skip revectorized node.
            if (!og_index.valid()) {
                ...
                og_index
                    = __ Simd256LoadTransform(base, index, load_transform.load_kind, Get256LoadTransformKindFrom128(load_transform.transform_kind), offset);
                pnode->SetRevectorizedNode(og_index);
            }
            return GetExtractOpIfNeeded(pnode, ig_index, og_index);
        }
    ...
    OpIndex REDUCE_INPUT_GRAPH(Load)(OpIndex ig_index, const LoadOp& load)
    {
        if (auto pnode = analyzer_.GetPackNode(ig_index)) {
            ...
            if (!og_index.valid()) {
                const LoadOp* start = analyzer_.GetStartOperation(pnode, ig_index, load).TryCast<LoadOp>();
                ...
                og_index = __ Load(base, index, load.kind, MemoryRepresentation::Simd256(), start->offset);
                pnode->SetRevectorizedNode(og_index);
            }
    ...
    OpIndex REDUCE_INPUT_GRAPH(Store)(OpIndex ig_index, const StoreOp& store)
    {
        if (auto pnode = analyzer_.GetPackNode(ig_index)) {
            ...
            // Emit revectorized op.
            if (!og_index.valid()) {
                const StoreOp* start = (analyzer_.GetStartOperation(pnode, ig_index, store)).TryCast<StoreOp>();
                ...
                __ Store(base, index, value, store.kind, MemoryRepresentation::Simd256(), store.write_barrier, start->offset);
            ...
            if (auto pnode = analyzer_.GetPackNode(ig_index)) {
                ...
                if (!og_index.valid()) {
                    ...
                    og_index = __ ResolvePhi(
                        phi, [&](OpIndex ind, int block_id) { return analyzer_.GetReduced(ind); }, RegisterRepresentation::Simd256());
                    pnode->SetRevectorizedNode(og_index);
                }

                OpIndex extract_op_index = GetExtractOpIfNeeded(pnode, ig_index, og_index);
    ...
    void FixLoopPhi(const PhiOp& input_phi, OpIndex output_index, Block* output_graph_loop)
    {
        if (input_phi.rep == RegisterRepresentation::Simd128()) {
            ...
            if (auto* pnode = analyzer_.GetPackNode(phi_index)) {
                ...
                if (pending_index.valid() && output_graph_loop->Contains(pending_index)) {
                    // Need skip replaced op
                    if (auto* pending_phi = __ output_graph().Get(pending_index).template TryCast<PendingLoopPhiOp>()) {
                        __ output_graph().template Replace<PhiOp>(pending_index,
                            base::VectorOf({ pending_phi->first(), analyzer_.GetReduced(input_phi.input(1)) }), RegisterRepresentation::Simd256());
                        return;
                    }
                }
            }
    ...
    V<Simd128> REDUCE_INPUT_GRAPH(Simd128Ternary)(V<Simd128> ig_index, const Simd128TernaryOp& ternary)
    {
        if (auto pnode = analyzer_.GetPackNode(ig_index)) {
            ...
            if (!og_index.valid()) {
                ...
                og_index = __ Simd256Ternary(first, second, third, GetSimd256TernaryKind(ternary.kind));
    ...
    V<Simd128> REDUCE_INPUT_GRAPH(Simd128Splat)(V<Simd128> ig_index, const Simd128SplatOp& op)
    {
        if (auto pnode = analyzer_.GetPackNode(ig_index)) {
            ...
            // Skip revectorized node.
            if (!og_index.valid()) {
                og_index = __ Simd256Splat(__ MapToNewGraph(op.input()), Get256SplatOpKindFrom128(op.kind));
    ...
    {
        if (ShufflePackNode* pnode = static_cast<ShufflePackNode*>(analyzer_.GetPackNode(ig_index))) {
            V<Simd256> og_index = pnode->RevectorizedNode();
            // Skip revectorized node.
            if (!og_index.valid()) {
                ...
                case ShufflePackNode::SpecificInfo::Kind::kS256Load64Transform: {
                    const bool is_32 = kind == ShufflePackNode::SpecificInfo::Kind::kS256Load32Transform;

                    const OpIndex load_index = op.input(pnode->info().splat_index() >> (is_32 ? 2 : 1));
                    const LoadOp& load = __ input_graph().Get(load_index).template Cast<LoadOp>();
                    ...
                    V<WordPtr> base = __ WordPtrAdd(__ MapToNewGraph(load.base()), __ IntPtrConstant(offset));

                    V<WordPtr> index = load.index().has_value() ? __ MapToNewGraph(load.index().value()) : __ IntPtrConstant(0);

                    const Simd256LoadTransformOp::TransformKind transform_kind
                        = is_32 ? Simd256LoadTransformOp::TransformKind::k32Splat : Simd256LoadTransformOp::TransformKind::k64Splat;
                    og_index = __ Simd256LoadTransform(base, index, load.kind, transform_kind, 0);
                    pnode->SetRevectorizedNode(og_index);
                    break;
                }
                case ShufflePackNode::SpecificInfo::Kind::kS256Load8x8U: {
                    const Simd128ShuffleOp& op0 = __ input_graph().Get(pnode->nodes()[0]).template Cast<Simd128ShuffleOp>();

                    V<Simd128> load_transform_idx = __ input_graph().Get(op0.left()).template Is<Simd128LoadTransformOp>() ? op0.left() : op0.right();
                    const Simd128LoadTransformOp& load_transform = __ input_graph().Get(load_transform_idx).template Cast<Simd128LoadTransformOp>();
                    DCHECK_EQ(load_transform.transform_kind, Simd128LoadTransformOp::TransformKind::k64Zero);
                    V<WordPtr> base = __ MapToNewGraph(load_transform.base());
                    V<WordPtr> index = __ MapToNewGraph(load_transform.index());
                    og_index
                        = __ Simd256LoadTransform(base, index, load_transform.load_kind, Simd256LoadTransformOp::TransformKind::k8x8U, load_transform.offset);
                    pnode->SetRevectorizedNode(og_index);
                    break;
                }
#ifdef V8_TARGET_ARCH_X64
                ...
                case ShufflePackNode::SpecificInfo::Kind::kShufps: {
                    ...
                    og_index = __ Simd256Shufps(og_left, og_right, pnode->info().shufps_control());
                    pnode->SetRevectorizedNode(og_index);
                    break;
                }
                case ShufflePackNode::SpecificInfo::Kind::kS32x8UnpackLow: {
                    V<Simd256> og_left = analyzer_.GetReduced(op.left());
                    V<Simd256> og_right = analyzer_.GetReduced(op.right());
                    og_index = __ Simd256Unpack(og_left, og_right, Simd256UnpackOp::Kind::k32x8Low);
                    pnode->SetRevectorizedNode(og_index);
                    break;
                }
                case ShufflePackNode::SpecificInfo::Kind::kS32x8UnpackHigh: {
                    V<Simd256> og_left = analyzer_.GetReduced(op.left());
                    V<Simd256> og_right = analyzer_.GetReduced(op.right());
                    og_index = __ Simd256Unpack(og_left, og_right, Simd256UnpackOp::Kind::k32x8High);
                    pnode->SetRevectorizedNode(og_index);
                    break;
                }
#endif // V8_TARGET_ARCH_X64
    ...
    OpIndex REDUCE_INPUT_GRAPH(Simd128ReplaceLane)(OpIndex ig_index, const Simd128ReplaceLaneOp& replace)
    {
        ...
        if (pnode && !pnode->is_force_pack()) {
            ...
            // Don't reduce revectorized node.
            if (!og_index.valid()) {
                const BundlePackNode* bundle_pnode = static_cast<const BundlePackNode*>(pnode);
                DCHECK(bundle_pnode);
                ...
                if (bundle_pnode->is_sign_extract()) {
                    if (bundle_pnode->lane_size() == 1) {
                        if (bundle_pnode->offset() == 0) {
                            i16x8_index = __ Simd128Unary(base_index, Simd128UnaryOp::Kind::kI16x8SConvertI8x16Low);
                        } else {
                            DCHECK_EQ(bundle_pnode->offset(), 8);
                            i16x8_index = __ Simd128Unary(base_index, Simd128UnaryOp::Kind::kI16x8SConvertI8x16High);
                        }
                    }
                    i32x8_index = __ Simd256Unary(i16x8_index, Simd256UnaryOp::Kind::kI32x8SConvertI16x8);
                } else {
                    if (bundle_pnode->lane_size() == 1) {
                        if (bundle_pnode->offset() == 0) {
                            i16x8_index = __ Simd128Unary(base_index, Simd128UnaryOp::Kind::kI16x8UConvertI8x16Low);
                        } else {
                            DCHECK_EQ(bundle_pnode->offset(), 8);
                            i16x8_index = __ Simd128Unary(base_index, Simd128UnaryOp::Kind::kI16x8UConvertI8x16High);
                        }
                    }
                    i32x8_index = __ Simd256Unary(i16x8_index, Simd256UnaryOp::Kind::kI32x8UConvertI16x8);
                }

                if (bundle_pnode->is_sign_convert()) {
                    og_index = __ Simd256Unary(i32x8_index, Simd256UnaryOp::Kind::kF32x8SConvertI32x8);
                } else {
                    og_index = __ Simd256Unary(i32x8_index, Simd256UnaryOp::Kind::kF32x8UConvertI32x8);
                }
    ...
    template <typename Op, typename Continuation> OpIndex ReduceInputGraphOperation(OpIndex ig_index, const Op& op)
    {
        if (PackNode* pnode = analyzer_.GetPackNode(ig_index); pnode && pnode->is_force_pack()) {
            // Handle force packing nodes.
            OpIndex og_index = pnode->RevectorizedNode();
            if (!og_index.valid()) {
                switch (pnode->force_pack_type()) {
                case PackNode::ForcePackType::kSplat: {
                    OpIndex og_index = Continuation { this }.ReduceInputGraph(ig_index, op);
                ...
                case PackNode::ForcePackType::kGeneral: {
                    ...
                    for (int i = 0; i < static_cast<int>(pnode->nodes().size()); i++) {
                        ...
                        const Op& next_op = Asm().input_graph().Get(next_index).template Cast<Op>();
