...
struct FrameElisionPhase {
    ...
    void Run(PipelineData* data, Zone* temp_zone)
    {
        ...
        const bool is_wasm_to_js = data->info()->code_kind() == CodeKind::WASM_TO_JS_FUNCTION || data->info()->builtin() == Builtin::kWasmToJsWrapperCSA;
#else
...
struct FinalizeCodePhase {
    DECL_TURBOSHAFT_MAIN_THREAD_PIPELINE_PHASE_CONSTANTS_WITH_LEGACY_NAME(FinalizeCode)
    static constexpr bool kOutputIsTraceableGraph = false;
