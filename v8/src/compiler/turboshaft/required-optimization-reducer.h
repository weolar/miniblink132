...
template <class Next> class RequiredOptimizationReducer : public Next {
public:
    ...
    OpIndex REDUCE(Phi)(base::Vector<const OpIndex> inputs, RegisterRepresentation rep)
    {
        ...
        if (const ConstantOp* first_constant = __ Get(first).template TryCast<ConstantOp>()) {
            for (const OpIndex& input : inputs.SubVectorFrom(1)) {
                const ConstantOp* maybe_constant = __ Get(input).template TryCast<ConstantOp>();
        ...
        if (const RttCanonOp* first_rtt = __ Get(first).template TryCast<RttCanonOp>()) {
            for (const OpIndex& input : inputs.SubVectorFrom(1)) {
                const RttCanonOp* maybe_rtt = __ Get(input).template TryCast<RttCanonOp>();
                if (!(maybe_rtt && maybe_rtt->rtts() == first_rtt->rtts() && maybe_rtt->type_index == first_rtt->type_index)) {
                    goto no_change;
                }
            }
