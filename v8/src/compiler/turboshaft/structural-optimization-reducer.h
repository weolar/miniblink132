...
template <class Next> class StructuralOptimizationReducer : public Next {
public:
    ...
    OpIndex ReduceInputGraphBranch(OpIndex input_index, const BranchOp& branch)
    {
        ...
        while (true) {
            ...
            // into a switch case.
            const Operation& cond = Asm().input_graph().Get(current_branch->condition());
            ...
            if (!cond.template Is<ComparisonOp>()) {
                ...
                if (!ContainsOnlyPureOps(current_branch->if_true, Asm().input_graph())) {
                ...
                current_branch_hint = hint == BranchHint::kNone ? BranchHint::kNone : hint == BranchHint::kTrue ? BranchHint::kFalse : BranchHint::kTrue;
            } else {
                const ComparisonOp* equal = cond.template TryCast<Opmask::kWord32Equal>();
                if (!equal) {
                    TRACE("\t [bailout] Branch with different condition than Word32 "
                          "Equal.\n");
                    break;
                }
            ...
            const Operation& maybe_branch = current_if_false->LastOperation(Asm().input_graph());
        ...
        Asm().Switch(Asm().MapToNewGraph(switch_var), Asm().output_graph().graph_zone()->CloneVector(base::VectorOf(cases)), Asm().MapToNewGraph(default_block),
            next_hint);
...
private:
    ...
    {
        base::iterator_range<Graph::OpIndexIterator> all_ops = Asm().input_graph().OperationIndices(*input_block);
