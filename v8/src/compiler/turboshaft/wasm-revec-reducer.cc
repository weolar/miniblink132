...
std::string GetSimdOpcodeName(Operation const& op)
{
    ...
    if (op.Is<Simd128BinopOp>() || op.Is<Simd128UnaryOp>() || op.Is<Simd128ShiftOp>() || op.Is<Simd128TestOp>() || op.Is<Simd128TernaryOp>()) {
        op.PrintOptions(oss);
    } else {
...
public:
        ...
        , offset_(op->offset)
    {
        ...
        if constexpr (std::is_same_v<Op, Simd128LoadTransformOp>) {
            ...
            if (!add_op || add_op->kind != WordBinopOp::Kind::kAdd || add_op->rep != WordRepresentation::Word64()) {
                SetInvalid();
                return;
            }
            base_ = &graph->Get(add_op->left());
            const ConstantOp* const_op = graph->Get(add_op->right()).TryCast<ConstantOp>();
            if (!const_op) {
                SetInvalid();
                return;
            }
        ...
        if (const ConstantOp* const_op = change_input->TryCast<ConstantOp>()) {
            ...
            int new_offset;
            if (base::bits::SignedAddOverflow32(static_cast<int>(const_op->word32()), offset_, &new_offset)) {
                // offset is overflow
                SetInvalid();
                return;
            }
    ...
    std::optional<int> operator-(const StoreLoadInfo<Op>& rhs) const
    {
        ...
        if constexpr (std::is_same_v<Op, Simd128LoadTransformOp>) {
            calculatable &= (op_->load_kind == rhs.op_->load_kind && op_->transform_kind == rhs.op_->transform_kind);
        } else {
...
template <class Op, class Info> bool LoadStrideEqualTo(const Graph& graph, const NodeGroup& node_group, int stride)
{
...
template <typename T> bool IsSplat(const T& node_group)
{
...
void PackNode::Print(Graph* graph) const
{
    ...
    TRACE("%s(#%d, #%d)\n", GetSimdOpcodeName(op).c_str(), nodes_[0].id(), nodes_[1].id());
}
...
template <typename FunctionType> void ForEach(FunctionType callback, ZoneUnorderedMap<OpIndex, PackNode*>& node_map)
{
...
void SLPTree::Print(const char* info)
{
    ...
    ForEach([this](PackNode const* pnode) { pnode->Print(&graph_); }, node_to_packnode_);
}
...
{
    TRACE("PackNode %s(#%d, #%d)\n", GetSimdOpcodeName(graph_.Get(node_group[0])).c_str(), node_group[0].id(), node_group[1].id());
...
PackNode* SLPTree::NewForcePackNode(const NodeGroup& node_group, PackNode::ForcePackType type, const Graph& graph)
{
    ...
    if (HasInputDependencies(node_group)) {
        TRACE("ForcePackNode %s(#%d, #%d) failed due to input dependencies.\n", GetSimdOpcodeName(graph_.Get(node_group[0])).c_str(), node_group[0].id(),
            node_group[1].id());
        return nullptr;
    }
...
BundlePackNode* SLPTree::NewBundlePackNode(
    const NodeGroup& node_group, OpIndex base, int8_t offset, uint8_t lane_size, bool is_sign_extract, bool is_sign_convert)
{
    Operation& op = graph_.Get(node_group[0]);
    TRACE("PackNode %s(#%d:, #%d)\n", GetSimdOpcodeName(op).c_str(), node_group[0].id(), node_group[1].id());
    BundlePackNode* pnode = phase_zone_->New<BundlePackNode>(phase_zone_, node_group, base, offset, lane_size, is_sign_extract, is_sign_convert);
    for (OpIndex node : node_group) {
        node_to_packnode_[node] = pnode;
    }
    return pnode;
...
PackNode* SLPTree::NewCommutativePackNodeAndRecurs(const NodeGroup& node_group, unsigned depth)
{
    PackNode* pnode = NewPackNode(node_group);
    ...
    bool same_kind = (op0.left() == op1.left()) || IsSameOpAndKind(graph_.Get(op0.left()), graph_.Get(op1.left()));
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        NodeGroup operands(graph_.Get(node_group[0]).input(i), graph_.Get(node_group[1]).input(node1_input_index));
...
PackNode* SLPTree::NewPackNodeAndRecurs(const NodeGroup& node_group, int start_index, int count, unsigned depth)
{
    PackNode* pnode = NewPackNode(node_group);
    for (int i = start_index; i < start_index + count; ++i) {
        // Prepare the operand vector.
        NodeGroup operands(graph_.Get(node_group[0]).input(i), graph_.Get(node_group[1]).input(i));
...
ShufflePackNode* SLPTree::NewShufflePackNode(const NodeGroup& node_group, ShufflePackNode::SpecificInfo::Kind kind)
{
    Operation& op = graph_.Get(node_group[0]);
    TRACE("PackNode %s(#%d:, #%d)\n", GetSimdOpcodeName(op).c_str(), node_group[0].id(), node_group[1].id());
    ShufflePackNode* pnode = phase_zone_->New<ShufflePackNode>(phase_zone_, node_group, kind);
    for (OpIndex node : node_group) {
        node_to_packnode_[node] = pnode;
    }
    return pnode;
...
ShufflePackNode* SLPTree::Try256ShuffleMatchLoad8x8U(const NodeGroup& node_group, const uint8_t* shuffle0, const uint8_t* shuffle1)
{
...
#define CANONICALIZE_SHUFFLE(n)                                                                                                                                \
    wasm::SimdShuffle::CanonicalizeShuffle(false, shuffle_copy##n, &need_swap, &is_swizzle);                                                                   \
    if (is_swizzle) {                                                                                                                                          \
        /* Here shuffle couldn't be swizzle*/                                                                                                                  \
        return nullptr;                                                                                                                                        \
    }                                                                                                                                                          \
...
#undef CANONICALIZE_SHUFFLE
    ...
    const Simd128LoadTransformOp* load_transform = graph_.Get(shuffle0_left_idx).TryCast<Simd128LoadTransformOp>();
    ...
    Simd128ConstantOp* shuffle0_const = graph_.Get(shuffle0_right_idx).TryCast<Simd128ConstantOp>();
    Simd128ConstantOp* shuffle1_const = graph_.Get(shuffle1_right_idx).TryCast<Simd128ConstantOp>();

    if (!shuffle0_const || !shuffle1_const || !shuffle0_const->IsZero() || !shuffle1_const->IsZero()) {
        // Shuffle right is not zero
        return nullptr;
    }

    if (load_transform->transform_kind == Simd128LoadTransformOp::TransformKind::k64Zero) {
        /*
    ...
    */
        ...
        for (int i = 0; i < kSimd128Size / 4; ++i) {
            ...
            if (shuffle_copy0[i * 4 + 1] < kSimd128Size || shuffle_copy0[i * 4 + 2] < kSimd128Size || shuffle_copy0[i * 4 + 3] < kSimd128Size
                || shuffle_copy1[i * 4 + 1] < kSimd128Size || shuffle_copy1[i * 4 + 2] < kSimd128Size || shuffle_copy1[i * 4 + 3] < kSimd128Size) {
                // not match
                return nullptr;
            }
        }
        TRACE("match load extend 8x8->32x8\n");
        return NewShufflePackNode(node_group, ShufflePackNode::SpecificInfo::Kind::kS256Load8x8U);
    }
    return nullptr;
}

#ifdef V8_TARGET_ARCH_X64
ShufflePackNode* SLPTree::X64TryMatch256Shuffle(const NodeGroup& node_group, const uint8_t* shuffle0, const uint8_t* shuffle1)
{
    DCHECK_EQ(node_group.size(), 2);
    ...
    if (op0.left() == op0.right() && op1.left() == op1.right()) {
        ...
        if (uint8_t shuffle32x8[8]; wasm::SimdShuffle::TryMatch32x8Shuffle(shuffle8x32, shuffle32x8)) {
            uint8_t control;
            if (wasm::SimdShuffle::TryMatchVpshufd(shuffle32x8, &control)) {
                ShufflePackNode* pnode = NewShufflePackNode(node_group, ShufflePackNode::SpecificInfo::Kind::kShufd);
        ...
        if (const wasm::ShuffleEntry<kSimd256Size>* arch_shuffle; wasm::SimdShuffle::TryMatchArchShuffle(shuffle8x32, false, &arch_shuffle)) {
            ...
            return pnode;
        } else if (uint8_t shuffle32x8[8]; wasm::SimdShuffle::TryMatch32x8Shuffle(shuffle8x32, shuffle32x8)) {
            uint8_t control;
            if (wasm::SimdShuffle::TryMatchShufps256(shuffle32x8, &control)) {
                ShufflePackNode* pnode = NewShufflePackNode(node_group, ShufflePackNode::SpecificInfo::Kind::kShufps);
...
//         (local.get 7))))
std::optional<SLPTree::ExtendIntToF32x4Info> SLPTree::TryGetExtendIntToF32x4Info(OpIndex index)
{
    ...
    for (int lane_index = 3; lane_index > 0; lane_index--) {
        const Simd128ReplaceLaneOp* replace_lane = graph_.Get(current).TryCast<turboshaft::Opmask::kSimd128ReplaceLaneF32x4>();
        if (!replace_lane) {
            TRACE("Mismatch in replace lane\n");
            return {};
        }
        const ChangeOp* change = graph_.Get(replace_lane->new_lane()).TryCast<ChangeOp>();
        if (!change) {
            TRACE("Mismatch in type convert\n");
            return {};
        }
        const Simd128ExtractLaneOp* extract_lane = graph_.Get(change->input()).TryCast<Simd128ExtractLaneOp>();
        if (!extract_lane) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (lane_extend_info[i].change_kind != lane_extend_info[0].change_kind
            || (lane_extend_info[i].change_kind != ChangeOp::Kind::kSignedToFloat && lane_extend_info[i].change_kind != ChangeOp::Kind::kUnsignedToFloat)) {
            return {};
        }
        ...
        if (lane_extend_info[i].extract_kind != lane_extend_info[0].extract_kind
            || (lane_extend_info[i].extract_kind != Simd128ExtractLaneOp::Kind::kI8x16S
                && lane_extend_info[i].extract_kind != Simd128ExtractLaneOp::Kind::kI8x16U
                && lane_extend_info[i].extract_kind != Simd128ExtractLaneOp::Kind::kI16x8S
                && lane_extend_info[i].extract_kind != Simd128ExtractLaneOp::Kind::kI16x8U)) {
            return {};
        }
        if (lane_extend_info[i].extract_lane_index != lane_extend_info[0].extract_lane_index + i) {
            return {};
        }
    }
    ...
    if (lane_extend_info[0].extract_kind == Simd128ExtractLaneOp::Kind::kI8x16S || lane_extend_info[0].extract_kind == Simd128ExtractLaneOp::Kind::kI8x16U) {
        info.lane_size = 1;
    } else {
        info.lane_size = 2;
    }
    info.is_sign_extract
        = lane_extend_info[0].extract_kind == Simd128ExtractLaneOp::Kind::kI8x16S || lane_extend_info[0].extract_kind == Simd128ExtractLaneOp::Kind::kI16x8S;
    info.is_sign_convert = lane_extend_info[0].change_kind == ChangeOp::Kind::kSignedToFloat;

    return info;
}

bool SLPTree::TryMatchExtendIntToF32x4(const NodeGroup& node_group, ExtendIntToF32x4Info* info)
{
    ...
    if (info0.value().extend_from != info1.value().extend_from || info0.value().is_sign_extract != info1.value().is_sign_extract
        || info0.value().lane_size != info1.value().lane_size || info0.value().is_sign_convert != info1.value().is_sign_convert) {
        return false;
    }

    uint32_t min_lane_index = std::min(info0.value().start_lane, info1.value().start_lane);
...
bool SLPTree::IsSideEffectFree(OpIndex first, OpIndex second)
{
    ...
    while (prev_node != first) {
        ...
        if ((IsProtectedLoad(graph().Get(second)) && IsProtectedLoad(graph().Get(prev_node))) ? CannotSwapProtectedLoads(prev_effects, effects)
...
bool IsSignExtensionOp(Operation& op)
{
    if (const Simd128UnaryOp* unop = op.TryCast<Simd128UnaryOp>()) {
        return unop->kind >= Simd128UnaryOp::Kind::kFirstSignExtensionOp && unop->kind <= Simd128UnaryOp::Kind::kLastSignExtensionOp;
    } else if (const Simd128BinopOp* binop = op.TryCast<Simd128BinopOp>()) {
        return binop->kind >= Simd128BinopOp::Kind::kFirstSignExtensionOp && binop->kind <= Simd128BinopOp::Kind::kLastSignExtensionOp;
    }
    return false;
}
...
bool SLPTree::CanBePacked(const NodeGroup& node_group)
{
    ...
    if (!is_sign_ext && !IsSameOpAndKind(op0, op1)) {
        TRACE("(%s, %s) have different op\n", GetSimdOpcodeName(op0).c_str(), GetSimdOpcodeName(op1).c_str());
        return false;
    }

    if (node0.offset() <= node1.offset() ? !IsSideEffectFree(node0, node1) : !IsSideEffectFree(node1, node0)) {
...
PackNode* SLPTree::BuildTreeRec(const NodeGroup& node_group, unsigned recursion_depth)
{
    DCHECK_EQ(node_group.size(), 2);
    ...
    case Opcode::kSimd128LoadTransform: {
        const Simd128LoadTransformOp& transform_op0 = op0.Cast<Simd128LoadTransformOp>();
        const Simd128LoadTransformOp& transform_op1 = op1.Cast<Simd128LoadTransformOp>();
        ...
        if (IsLoadSplat(transform_op0)) {
            ...
            if (IsSplat(node_group) || (stride.has_value() && stride.value() == 0)) {
                return NewPackNode(node_group);
            }
            return NewForcePackNode(node_group, PackNode::ForcePackType::kGeneral, graph_);
            ...
            if (stride.has_value()) {
                ...
                if (value == kSimd128Size / 2) {
                    ...
                    return NewForcePackNode(node_group, PackNode::ForcePackType::kSplat, graph_);
                }
            }
            return NewForcePackNode(node_group, PackNode::ForcePackType::kGeneral, graph_);
        } else {
            TRACE("Load Transfrom k64Zero/k32Zero!\n");
            DCHECK(transform_op0.transform_kind == Simd128LoadTransformOp::TransformKind::k32Zero
                || transform_op0.transform_kind == Simd128LoadTransformOp::TransformKind::k64Zero);
    ...
    case Opcode::kLoad: {
        ...
        if (load0.loaded_rep != MemoryRepresentation::Simd128() || load1.loaded_rep != MemoryRepresentation::Simd128()) {
    ...
    case Opcode::kPhi: {
        ...
        if (phi.rep != RegisterRepresentation::Simd128() || op0.input_count != op1.input_count) {
            TRACE("Failed due to invalid phi\n");
            return nullptr;
        }
        PackNode* pnode = NewPackNodeAndRecurs(node_group, 0, value_in_count, recursion_depth);
        return pnode;
    }
...
#define UNARY_SIGN_EXTENSION_CASE(op_low, not_used1, op_high)                                                                                                  \
    case Simd128UnaryOp::Kind::k##op_low: {                                                                                                                    \
        if (const Simd128UnaryOp* unop1 = op1.TryCast<Opmask::kSimd128##op_high>(); unop1 && op0.Cast<Simd128UnaryOp>().input() == unop1->input()) {           \
            return NewPackNode(node_group);                                                                                                                    \
        }                                                                                                                                                      \
        [[fallthrough]];                                                                                                                                       \
    }                                                                                                                                                          \
    case Simd128UnaryOp::Kind::k##op_high: {                                                                                                                   \
        if (op1.Cast<Simd128UnaryOp>().kind == op0.Cast<Simd128UnaryOp>().kind) {                                                                              \
            auto force_pack_type = node0 == node1 ? PackNode::ForcePackType::kSplat : PackNode::ForcePackType::kGeneral;                                       \
...
#define BINOP_SIGN_EXTENSION_CASE(op_low, not_used1, op_high)                                                                                                  \
    case Simd128BinopOp::Kind::k##op_low: {                                                                                                                    \
        if (const Simd128BinopOp* binop1 = op1.TryCast<Opmask::kSimd128##op_high>();                                                                           \
            binop1 && op0.Cast<Simd128BinopOp>().left() == binop1->left() && op0.Cast<Simd128BinopOp>().right() == binop1->right()) {                          \
            return NewPackNode(node_group);                                                                                                                    \
        }                                                                                                                                                      \
        [[fallthrough]];                                                                                                                                       \
    }                                                                                                                                                          \
    case Simd128BinopOp::Kind::k##op_high: {                                                                                                                   \
        ...
        switch (op0.Cast<Simd128BinopOp>().kind) {
            ...
            SIMD256_BINOP_SIMPLE_OP(BINOP_CASE)
            {
                ...
                PackNode* pnode = NewCommutativePackNodeAndRecurs(node_group, recursion_depth);
                return pnode;
            }
        default: {
...
#define SHIFT_CASE(op_128, not_used) case Simd128ShiftOp::Kind::k##op_128:
                SIMD256_SHIFT_OP(SHIFT_CASE)
                {
                    ...
                    PackNode* pnode = NewPackNodeAndRecurs(node_group, 0, kShiftValueInCount, recursion_depth);
                    return pnode;
                }
#undef SHIFT_CASE
            default: {
...
#undef TERNARY_CASE
    ...
    case Opcode::kSimd128Shuffle: {
        ...
        if (CompareCharsEqual(shuffle0, shuffle1, kSimd128Size)) {
            if (IsSplat(node_group)) {
                ...
                int index;
                if (wasm::SimdShuffle::TryMatchSplat<4>(shuffle0, &index) && graph_.Get(op0.input(index >> 2)).opcode == Opcode::kLoad) {
                    ShufflePackNode* pnode = NewShufflePackNode(node_group, ShufflePackNode::SpecificInfo::Kind::kS256Load32Transform);
                    pnode->info().set_splat_index(index);
                    return pnode;
                } else if (wasm::SimdShuffle::TryMatchSplat<2>(shuffle0, &index) && graph_.Get(op0.input(index >> 1)).opcode == Opcode::kLoad) {
                    ShufflePackNode* pnode = NewShufflePackNode(node_group, ShufflePackNode::SpecificInfo::Kind::kS256Load64Transform);
                    pnode->info().set_splat_index(index);
                    return pnode;
                }
            } else {
#ifdef V8_TARGET_ARCH_X64
                if (ShufflePackNode* pnode = X64TryMatch256Shuffle(node_group, shuffle0, shuffle1)) {
    ...
    case Opcode::kSimd128ReplaceLane: {
        ...
        if (TryMatchExtendIntToF32x4(node_group, &info)) {
            ...
            PackNode* p = NewBundlePackNode(node_group, info.extend_from, info.start_lane, info.lane_size, info.is_sign_extract, info.is_sign_convert);
            return p;
        }
        return nullptr;
    }
    ...
    default:
        TRACE("Default branch #%d:%s\n", node0.id(), GetSimdOpcodeName(op0).c_str());
        break;
    }
    return nullptr;
}
...
bool WasmRevecAnalyzer::CanMergeSLPTrees()
{
    for (auto& entry : slp_tree_->GetNodeMapping()) {
        ...
        if (itr != revectorizable_node_.end() && !itr->second->IsSame(*entry.second)) {
...
void WasmRevecAnalyzer::ProcessBlock(const Block& block)
{
    ...
    for (const Operation& op : base::Reversed(graph_.operations(block))) {
        ...
        if (IsSupportedReduceSeed(op)) {
            ...
            if (left_index != right_index && left_op.opcode == right_op.opcode && IsSameOpAndKind(left_op, right_op)) {
    ...
    if (simd128_stores.size() >= 2) {
        for (auto it = std::next(simd128_stores.cbegin()), end = simd128_stores.cend(); it != end;) {
            ...
            if (diff.has_value()) {
                ...
                if (value == kSimd128Size) {
                    store_seeds_.push_back({ graph_.Index(*info0.op()), graph_.Index(*info1.op()) });
...
void WasmRevecAnalyzer::Run()
{
    ...
    ZoneVector<std::pair<OpIndex, OpIndex>> all_seeds(store_seeds_.begin(), store_seeds_.end(), phase_zone_);
