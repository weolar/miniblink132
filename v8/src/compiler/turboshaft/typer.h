...
template <typename T, size_t N> T array_min(const std::array<T, N>& a)
{
    DCHECK_NE(0, N);
...
// There must be at least one non-NaN element.
// Any -0 is converted to 0.
template <typename T, size_t N> T array_max(const std::array<T, N>& a)
{
    DCHECK_NE(0, N);
    T x = -std::numeric_limits<T>::infinity();
    for (size_t i = 0; i < N; ++i) {
...
template <size_t Bits> struct WordOperationTyper {
    ...
    static type_t FromElements(ElementsVector elements, Zone* zone)
    {
        ...
        DCHECK(base::all_of(elements, [&](word_t e) { return result.Contains(e); }));
        return result;
    }
    ...
    static std::pair<word_t, word_t> MakeRange(base::Vector<const word_t> elements)
    {
        ...
        while (to_index + 1 < from_index) {
            if ((elements[to_index + 1] - elements[to_index]) < (elements[from_index] - elements[from_index - 1])) {
                ++to_index;
            } else {
                --from_index;
            }
    ...
    {
        return { type_t::Range(rhs.unsigned_min(), max, zone), type_t::Range(0, lhs.unsigned_max(), zone) };
    }
    ...
    {
        return { type_t::Range(0, rhs.unsigned_max(), zone), type_t::Range(lhs.unsigned_min(), max, zone) };
    }
    ...
    static type_t WidenExponential(const type_t& old_type, type_t new_type, Zone* zone)
    {
        ...
        if (is_wrapping(old_from, old_to)) {
            ...
            if (new_from < old_from) {
                ...
                word_t result_from = std::numeric_limits<word_t>::max() - (min_size - new_to);
                ...
                word_t result_to = min_size - (std::numeric_limits<word_t>::max() - new_from);
        ...
        if (new_from < old_from) {
            ...
            if (new_from <= std::numeric_limits<word_t>::max() - min_size) {
                ...
                return type_t::Range(std::numeric_limits<word_t>::max() - min_size, std::numeric_limits<word_t>::max(), zone);
            }
        }
    }
};

template <size_t Bits> struct FloatOperationTyper {
    static_assert(Bits == 32 || Bits == 64);
    ...
    {
        DCHECK_LE(min, max);
        DCHECK_IMPLIES(detail::is_minus_zero(min), (special_values & type_t::kMinusZero));
        DCHECK_IMPLIES(detail::is_minus_zero(max), (special_values & type_t::kMinusZero));
        if (min == max)
            return Set({ min + float_t { 0 } }, special_values, zone);
    ...
    static type_t Set(std::vector<float_t> elements, uint32_t special_values, Zone* zone)
    {
        ...
        if (base::erase_if(elements, [](float_t v) { return IsMinusZero(v); }) > 0) {
    ...
    static bool IsIntegerSet(const type_t& t)
    {
        ...
        if (std::modf(min, &unused_ipart) != 0.0)
            return false;
        if (min == -inf)
            return false;
        float_t max = t.set_element(size - 1);
        if (std::modf(max, &unused_ipart) != 0.0)
            return false;
        if (max == inf)
            return false;

        for (int i = 1; i < size - 1; ++i) {
            if (std::modf(t.set_element(i), &unused_ipart) != 0.0)
                return false;
        }
        return true;
    }
    ...
    static Type ProductSet(const type_t& l, const type_t& r, uint32_t special_values, Zone* zone, std::function<float_t(float_t, float_t)> combine)
    {
        ...
        auto CombineWithLeft = [&](float_t left) {
            ...
            if (r.has_minus_zero())
                results.push_back(combine(left, -0.0));
            if (r.has_nan())
                results.push_back(combine(left, nan_v<Bits>));
        };
    ...
    static Type Add(type_t l, type_t r, Zone* zone)
    {
        ...
        uint32_t special_values = (maybe_nan ? type_t::kNaN : 0) | (maybe_minuszero ? type_t::kMinusZero : 0);
    ...
    static Type Multiply(type_t l, type_t r, Zone* zone)
    {
        ...
        if (l.is_only_nan() || r.is_only_nan())
            return type_t::NaN();
        bool maybe_nan
            = l.has_nan() || r.has_nan() || (IsZeroish(l) && (r.min() == -inf || r.max() == inf)) || (IsZeroish(r) && (l.min() == -inf || r.max() == inf));

        // Try to rule out -0.
        bool maybe_minuszero = l.has_minus_zero() || r.has_minus_zero() || (IsZeroish(l) && r.min() < 0.0) || (IsZeroish(r) && l.min() < 0.0);
        if (l.has_minus_zero()) {
            l = type_t::LeastUpperBound(l, type_t::Constant(0), zone);
        }
        if (r.has_minus_zero()) {
        ...
        if (result_min <= 0.0 && 0.0 <= result_max && (l_min < 0.0 || r_min < 0.0)) {
            special_values |= type_t::kMinusZero;
            // Remove -0.
            result_min += 0.0;
            result_max += 0.0;
        ...
        if (((l_min == -inf || l_max == inf) && (r_min <= 0.0 && 0.0 <= r_max)) || ((r_min == -inf || r_max == inf) && (l_min <= 0.0 && 0.0 <= l_max))) {
            special_values |= type_t::kNaN;
        }
    ...
    static Type Divide(const type_t& l, const type_t& r, Zone* zone)
    {
        ...
        bool maybe_nan = l.has_nan() || IsZeroish(r) || ((l_min == -inf || l_max == inf) && (r_min == -inf || r_max == inf));
    ...
    static Type Modulus(type_t l, type_t r, Zone* zone)
    {
        ...
        if (l.is_only_nan() || r.is_only_nan())
            return type_t::NaN();
        bool maybe_nan = l.has_nan() || IsZeroish(r) || l.min() == -inf || l.max() == inf;
    ...
    static Type Power(const type_t& l, const type_t& r, Zone* zone)
    {
        ...
        if (r.is_only_nan())
            return type_t::NaN();
        // x ** +-0 => 1.
        if (r.is_constant(0) || r.is_only_minus_zero())
            return type_t::Constant(1);
        if (l.is_only_nan()) {
            // NaN ** 0 => 1.
        ...
        if (r.Contains(-inf) || r.Contains(inf)) {
            if (l.Contains(1) || l.Contains(-1))
                maybe_nan = true;
        }

        // a ** b produces NaN if a < 0 && b is fraction.
        if (l.min() < 0.0 && !IsIntegerSet(r))
            maybe_nan = true;
        ...
        uint32_t special_values = (maybe_nan ? type_t::kNaN : 0) | (maybe_minus_zero ? type_t::kMinusZero : 0) | l.special_values();
    ...
    static Type LessThanOrEqual(const type_t& lhs, const type_t& rhs, Zone* zone)
    {
        ...
        if (lhs.is_only_special_values()) {
            if (lhs.has_minus_zero()) {
                can_be_true = (!rhs.is_only_special_values() && rhs.max() >= 0.0) || rhs.has_minus_zero();
                can_be_false = rhs.min() < 0.0;
            } else {
                DCHECK(lhs.is_only_nan());
            }
            ...
            if (rhs.has_minus_zero()) {
                can_be_true = (!lhs.is_only_special_values() && lhs.min() <= 0.0) || lhs.has_minus_zero();
                can_be_false = lhs.max() > 0.0;
            } else {
                DCHECK(rhs.is_only_nan());
            }
    ...
    static std::pair<Type, Type> RestrictionForLessThan_False(const type_t& lhs, const type_t& rhs, Zone* zone)
    {
        ...
        if (rhs.has_nan()) {
            ...
            uint32_t lhs_sv = type_t::kNaN | (rhs.min() <= 0 ? type_t::kMinusZero : type_t::kNoSpecialValues);
        ...
        if (lhs.has_nan()) {
            ...
            uint32_t rhs_sv = type_t::kNaN | (lhs.max() >= 0 ? type_t::kMinusZero : type_t::kNoSpecialValues);
    ...
    static std::pair<Type, Type> RestrictionForLessThanOrEqual_True(const type_t& lhs, const type_t& rhs, Zone* zone)
    {
        ...
        uint32_t lhs_sv = rhs.max() >= 0 ? type_t::kMinusZero : type_t::kNoSpecialValues;
        uint32_t rhs_sv = lhs.min() <= 0 ? type_t::kMinusZero : type_t::kNoSpecialValues;
        return { type_t::Range(-inf, rhs.max(), lhs_sv, zone), type_t::Range(lhs.min(), inf, rhs_sv, zone) };
    }
    ...
    static std::pair<Type, Type> RestrictionForLessThanOrEqual_False(const type_t& lhs, const type_t& rhs, Zone* zone)
    {
        ...
        if (rhs.has_nan()) {
            ...
            uint32_t sv = type_t::kNaN | (min <= 0 ? type_t::kMinusZero : type_t::kNoSpecialValues);
        ...
        if (lhs.has_nan()) {
            ...
            uint32_t sv = type_t::kNaN | (max >= 0 ? type_t::kMinusZero : type_t::kNoSpecialValues);
...
public:
    ...
    static Type TypeForRepresentation(base::Vector<const RegisterRepresentation> reps, Zone* zone)
    {
        ...
        if (reps.size() == 1)
            return TypeForRepresentation(reps[0]);
        base::SmallVector<Type, 4> tuple_types;
        for (auto rep : reps)
            tuple_types.push_back(TypeForRepresentation(rep));
    ...
    static Type TypeConstant(ConstantOp::Kind kind, ConstantOp::Storage value)
    {
        ...
        case ConstantOp::Kind::kFloat32:
            if (value.float32.is_nan())
                return Float32Type::NaN();
        ...
        case ConstantOp::Kind::kFloat64:
            if (value.float64.is_nan())
                return Float64Type::NaN();
    ...
    static Type TypeWord64Add(const Type& lhs, const Type& rhs, Zone* zone)
    {
        if (lhs.IsNone() || rhs.IsNone())
            return Type::None();
        if (!InputIs(lhs, Type::Kind::kWord64) || !InputIs(rhs, Type::Kind::kWord64)) {
            return Word64Type::Any();
        }
...
#define FLOAT_BINOP(op, bits, float_typer_handler)                                                                                                             \
    static Type TypeFloat##bits##op(const Type& lhs, const Type& rhs, Zone* zone)                                                                              \
    {                                                                                                                                                          \
        if (lhs.IsNone() || rhs.IsNone())                                                                                                                      \
            return Type::None();                                                                                                                               \
        if (!InputIs(lhs, Type::Kind::kFloat##bits) || !InputIs(rhs, Type::Kind::kFloat##bits)) {                                                              \
...
#undef FLOAT_BINOP
    ...
    static Type TypeOverflowCheckedBinop(const Type& left_type, const Type& right_type, OverflowCheckedBinopOp::Kind kind, WordRepresentation rep, Zone* zone)
    {
        ...
        if (rep == WordRepresentation::Word32()) {
            ...
            case OverflowCheckedBinopOp::Kind::kSignedMul:
                ...
                return TupleType::Tuple(Word32Type::Any(), Word32Type::Set({ 0, 1 }, zone), zone);
            }
        } else {
            DCHECK_EQ(rep, WordRepresentation::Word64());
            switch (kind) {
            ...
            case OverflowCheckedBinopOp::Kind::kSignedMul:
                // TODO(nicohartmann@): Support these.
                return TupleType::Tuple(Word64Type::Any(), Word32Type::Set({ 0, 1 }, zone), zone);
            }
        }
    }
    ...
    static Type TypeWord32OverflowCheckedAdd(const Type& lhs, const Type& rhs, Zone* zone)
    {
        ...
        if ((0xC0000000 & l.unsigned_max()) == 0 && (0xC0000000 & r.unsigned_max()) == 0) {
            // Cannot overflow.
    ...
    static Type TypeWord64Comparison(const Type& lhs, const Type& rhs, ComparisonOp::Kind kind, Zone* zone)
    {
        ...
        case ComparisonOp::Kind::kSignedLessThanOrEqual:
            ...
            return Word32Type::Set({ 0, 1 }, zone);
        case ComparisonOp::Kind::kUnsignedLessThan:
            return WordOperationTyper<64>::UnsignedLessThan(lhs.AsWord64(), rhs.AsWord64(), zone);
        case ComparisonOp::Kind::kUnsignedLessThanOrEqual:
            return WordOperationTyper<64>::UnsignedLessThanOrEqual(lhs.AsWord64(), rhs.AsWord64(), zone);
        }
        UNREACHABLE();
    }
    ...
    static Type TypeFloat32Comparison(const Type& lhs, const Type& rhs, ComparisonOp::Kind kind, Zone* zone)
    {
        ...
        case ComparisonOp::Kind::kEqual:
            ...
            return Word32Type::Set({ 0, 1 }, zone);
        case ComparisonOp::Kind::kSignedLessThan:
            return FloatOperationTyper<32>::LessThan(lhs.AsFloat32(), rhs.AsFloat32(), zone);
        case ComparisonOp::Kind::kSignedLessThanOrEqual:
            return FloatOperationTyper<32>::LessThanOrEqual(lhs.AsFloat32(), rhs.AsFloat32(), zone);
        case ComparisonOp::Kind::kUnsignedLessThan:
        case ComparisonOp::Kind::kUnsignedLessThanOrEqual:
            UNREACHABLE();
        }
    ...
    static Type TypeFloat64Comparison(const Type& lhs, const Type& rhs, ComparisonOp::Kind kind, Zone* zone)
    {
        ...
        case ComparisonOp::Kind::kEqual:
            ...
            return Word32Type::Set({ 0, 1 }, zone);
        case ComparisonOp::Kind::kSignedLessThan:
            return FloatOperationTyper<64>::LessThan(lhs.AsFloat64(), rhs.AsFloat64(), zone);
        case ComparisonOp::Kind::kSignedLessThanOrEqual:
            return FloatOperationTyper<64>::LessThanOrEqual(lhs.AsFloat64(), rhs.AsFloat64(), zone);
        case ComparisonOp::Kind::kUnsignedLessThan:
        case ComparisonOp::Kind::kUnsignedLessThanOrEqual:
            UNREACHABLE();
        }
    ...
    static Word64Type ExtendWord32ToWord64(const Word32Type& t, Zone* zone)
    {
        ...
        if (t.is_wrapping())
            return Word64Type::Any();
        return Word64Type::Range(static_cast<uint64_t>(t.unsigned_min()), std::numeric_limits<uint64_t>::max(), zone);
    }
    ...
    static Word32Type TruncateWord32Input(const Type& input, bool implicit_word64_narrowing, Zone* zone)
    {
        ...
        if (input.IsAny()) {
            ...
            if (w64.is_any() || w64.is_wrapping())
                return Word32Type::Any();

            if (w64.range_to() <= std::numeric_limits<uint32_t>::max()) {
                DCHECK_LE(w64.range_from(), std::numeric_limits<uint32_t>::max());
                return Word32Type::Range(static_cast<uint32_t>(w64.range_from()), static_cast<uint32_t>(w64.range_to()), zone);
            }
        ...
        FATAL("Missing proper type for TruncateWord32Input. Type is: %s", input.ToString().c_str());
    }
    ...
    private:
        template <bool allow_implicit_word64_truncation> Type RefineWord32Type(const Type& type, const Type& refinement, Zone* zone)
        {
            // If refinement is Type::None(), the operation/branch is unreachable.
            if (refinement.IsNone())
                return Type::None();
            ...
            if constexpr (allow_implicit_word64_truncation) {
                ...
                if (type.IsWord64()) {
                    return Word64Type::Intersect(
                        type.AsWord64(), Typer::ExtendWord32ToWord64(refinement.AsWord32(), zone), Type::ResolutionMode::kOverApproximate, zone);
                }
            }
            // We limit the values of {type} to those in {refinement}.
            return Word32Type::Intersect(type.AsWord32(), refinement.AsWord32(), Type::ResolutionMode::kOverApproximate, zone);
        }
    ...
    static bool InputIs(const Type& input, Type::Kind expected)
    {
        ...
        std::stringstream s;
        s << expected;
        FATAL("Missing proper type (%s). Type is: %s", s.str().c_str(), input.ToString().c_str());
    }
