...
template <class AfterNext> class VariableReducer : public RequiredOptimizationReducer<AfterNext> {
    ...
    struct VariableTable : ChangeTrackingSnapshotTable<VariableTable, OpIndex, VariableData> {
        explicit VariableTable(Zone* zone)
            : ChangeTrackingSnapshotTable<VariableTable, OpIndex, VariableData>(zone)
...
public:
    ...
    void Bind(Block* new_block)
    {
        ...
        for (const Block* pred : new_block->PredecessorsIterable()) {
            std::optional<Snapshot> pred_snapshot = block_to_snapshot_mapping_[pred->index()];
        ...
        auto merge_variables = [&](Variable var, base::Vector<const OpIndex> predecessors) -> OpIndex {
            for (OpIndex idx : predecessors) {
                if (!idx.valid()) {
                    // If any of the predecessors' value is Invalid, then we shouldn't
                    // merge {var}.
                    return OpIndex::Invalid();
                } else if (__ output_graph().Get(idx).template Is<LoadRootRegisterOp>()) {
        ...
        if (new_block->IsLoop()) {
            ...
            if (active_loop_variables_begin != active_loop_variables_end) {
                ...
                for (Variable var : table_.active_loop_variables) {
                    ...
                    V<Any> pending_loop_phi = __ PendingLoopPhi(table_.Get(var), RegisterRepresentation(rep));
    ...
    V<None> REDUCE(Goto)(Block* destination, bool is_backedge)
    {
        ...
        if (loop_pending_phis_.contains(destination->index())) {
            for (auto [var, pending_phi_idx] : loop_pending_phis_[destination->index()].value()) {
                const PendingLoopPhiOp& pending_phi = __ Get(pending_phi_idx).template Cast<PendingLoopPhiOp>();
                __ output_graph().template Replace<PhiOp>(pending_phi_idx, base::VectorOf({ pending_phi.first(), GetVariable(var) }), pending_phi.rep);
            }
        }
    ...
    {
        DCHECK(!is_temporary_);
        if (V8_UNLIKELY(__ generating_unreachable_operations()))
            return;
        table_.Set(var, new_index);
    }
    template <typename Rep> void Set(Variable var, V<Rep> value)
    {
        DCHECK(!is_temporary_);
        if (V8_UNLIKELY(__ generating_unreachable_operations()))
            return;
        DCHECK(V<Rep>::allows_representation(RegisterRepresentation(var.data().rep)));
...
private:
    ...
    OpIndex MergeFrameState(base::Vector<const OpIndex> frame_states_indices)
    {
        ...
        for (OpIndex idx : frame_states_indices) {
            frame_states.push_back(&__ output_graph().Get(idx).template Cast<FrameStateOp>());
        }
        ...
        if (first_frame->inlined) {
            ...
            for (auto frame_state : frame_states) {
                ...
                all_parent_frame_states_are_the_same
                    = all_parent_frame_states_are_the_same && first_frame->parent_frame_state() == frame_state->parent_frame_state();
            }
            if (all_parent_frame_states_are_the_same) {
                new_inputs.push_back(first_frame->parent_frame_state());
            } else {
                OpIndex merged_parent_frame_state = MergeFrameState(base::VectorOf(indices_to_merge));
        ...
        for (int i = 0; i < first_frame->state_values_count(); i++) {
            ...
            for (auto frame_state : frame_states) {
                ...
                all_inputs_are_the_same = all_inputs_are_the_same && first_frame->state_value(i) == frame_state->state_value(i);
            }
            if (all_inputs_are_the_same) {
                ...
                OpIndex new_input = MergeOpIndices(base::VectorOf(indices_to_merge), rep);
                new_inputs.push_back(new_input);
            }
        }

        return __ FrameState(base::VectorOf(new_inputs), first_frame->inlined, first_frame->data);
    }

    VariableTable table_ { __ phase_zone() };
    const Block* current_block_ = nullptr;
    GrowingBlockSidetable<std::optional<Snapshot>> block_to_snapshot_mapping_ { __ input_graph().block_count(), std::nullopt, __ phase_zone() };
