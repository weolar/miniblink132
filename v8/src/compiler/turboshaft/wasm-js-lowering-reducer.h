...
template <class Next> class WasmJSLoweringReducer : public Next {
public:
    ...
    V<None> REDUCE(TrapIf)(V<Word32> condition, OptionalV<FrameState> frame_state, bool negated, TrapId trap_id)
    {
        ...
        const CallDescriptor* tf_descriptor
            = GetBuiltinCallDescriptor(trap, Asm().graph_zone(), StubCallMode::kCallBuiltinPointer, needs_frame_state, Operator::kNoProperties);
        const TSCallDescriptor* ts_descriptor = TSCallDescriptor::Create(tf_descriptor, CanThrow::kYes, LazyDeoptOnThrow::kNo, Asm().graph_zone());

        V<FrameState> new_frame_state = CreateFrameStateWithUpdatedBailoutId(frame_state.value());
...
private:
    OpIndex CreateFrameStateWithUpdatedBailoutId(OpIndex frame_state)
    {
        ...
        // the trap location).
        const FrameStateOp& frame_state_op = Asm().output_graph().Get(frame_state).template Cast<FrameStateOp>();
        ...
        const FrameStateInfo* new_info = Asm().graph_zone()->template New<FrameStateInfo>(BytecodeOffset(offset), info.state_combine(), info.function_info());
        FrameStateData* new_data
            = Asm().graph_zone()->template New<FrameStateData>(FrameStateData { *new_info, data->instructions, data->machine_types, data->int_operands });
        return __ FrameState(frame_state_op.inputs(), frame_state_op.inlined, new_data);
    }
