...
public:
    explicit TypeInferenceAnalysis(const Graph& graph, Zone* phase_zone)
        ...
        , block_to_snapshot_mapping_(graph.block_count(), std::nullopt, phase_zone)
    ...
    GrowingOpIndexSidetable<Type> Run(GrowingBlockSidetable<std::vector<std::pair<OpIndex, Type>>>* block_refinements = nullptr)
    {
        ...
        for (uint32_t unprocessed_index = 0; unprocessed_index < graph_.block_count();) {
    ...
    template <bool revisit_loop_header> void ProcessBlock(const Block& block, uint32_t* unprocessed_index)
    {
        ...
        // Collect the snapshots of all predecessors.
        {
            ...
            for (const Block* pred : block.PredecessorsIterable()) {
                ...
                if (pred_snapshot.has_value()) {
                    ...
                    DCHECK(block.IsLoop() && pred == block.LastPredecessor() && !revisit_loop_header);
                }
            }
        ...
        // predecessors.
        {
            auto MergeTypes = [&](table_t::Key, base::Vector<const Type> predecessors) -> Type {
                ...
                for (size_t i = 1; i < predecessors.size(); ++i) {
                    result_type = Type::LeastUpperBound(result_type, predecessors[i], graph_zone_);
                }
                return result_type;
            };
        ...
        for (auto it = op_range.begin(); it != op_range.end(); ++it) {
            ...
            case Opcode::kCheckTurboshaftTypeOf:
                ProcessCheckTurboshaftTypeOf(index, op.Cast<CheckTurboshaftTypeOfOp>());
                break;
            ...
            case Opcode::kPhi:
                if constexpr (revisit_loop_header) {
                    loop_needs_revisit = ProcessLoopPhi(index, op.Cast<PhiOp>()) || loop_needs_revisit;
                } else {
            ...
            default:
                ...
                if (op.outputs_rep().size() > 0) {
                    ...
                    SetType(index, Typer::TypeForRepresentation(op.outputs_rep(), graph_zone_), allow_narrowing, is_fallback_for_unsupported_operation);
                }
                break;
            case Opcode::kLoadRootRegister:
                SetType(index, Typer::TypeForRepresentation(op.outputs_rep(), graph_zone_));
                break;
            }
        }
        ...
        if constexpr (revisit_loop_header) {
            if (loop_needs_revisit) {
                ...
                *unprocessed_index = std::min(*unprocessed_index, block.index().id() + 1);
            }
        }
    }
    ...
    void ProcessCheckTurboshaftTypeOf(OpIndex index, const CheckTurboshaftTypeOfOp& check)
    {
        ...
        if (input_type.IsSubtypeOf(check.type)) {
            TURBOSHAFT_TRACE_TYPING_OK("CTOF %3d:%-40s\n  P: %3d:%-40s ~~> %s\n", index.id(), graph_.Get(index).ToString().substr(0, 40).c_str(),
                check.input().id(), graph_.Get(check.input()).ToString().substr(0, 40).c_str(), input_type.ToString().c_str());
        } else if (check.successful) {
            FATAL("Checking type %s of operation %d:%s failed after it passed in a "
                  "previous phase",
                check.type.ToString().c_str(), check.input().id(), graph_.Get(check.input()).ToString().c_str());
        } else {
    ...
    void ProcessComparison(OpIndex index, const ComparisonOp& comparison)
    {
        ...
        Type result_type = Typer::TypeComparison(left_type, right_type, comparison.rep, comparison.kind, graph_zone_);
        SetType(index, result_type);
    }

    void ProcessConstant(OpIndex index, const ConstantOp& constant)
    {
        if (constant.kind == ConstantOp::Kind::kFloat64 && constant.float64().is_hole_nan()) {
    ...
    void ProcessFloatBinop(OpIndex index, const FloatBinopOp& binop)
    {
        ...
        Type result_type = Typer::TypeFloatBinop(left_type, right_type, binop.kind, binop.rep, graph_zone_);
        SetType(index, result_type);
    }
    ...
    bool ProcessLoopPhi(OpIndex index, const PhiOp& phi)
    {
        ...
        if (new_type.IsSubtypeOf(old_type)) {
            TURBOSHAFT_TRACE_TYPING_OK("LOOP %3d:%-40s (FIXPOINT)\n  N:     %-40s ~~> %-40s\n", index.id(), graph_.Get(index).ToString().substr(0, 40).c_str(),
        ...
        TURBOSHAFT_TRACE_TYPING_OK("LOOP %3d:%-40s (REVISIT)\n  W:     %-40s ~~> %-40s\n", index.id(), graph_.Get(index).ToString().substr(0, 40).c_str(),
            old_type.ToString().c_str(), new_type.ToString().c_str());
    ...
    void ProcessOverflowCheckedBinop(OpIndex index, const OverflowCheckedBinopOp& binop)
    {
        ...
        Type result_type = Typer::TypeOverflowCheckedBinop(left_type, right_type, binop.kind, binop.rep, graph_zone_);
        SetType(index, result_type);
    }
    ...
    void ProcessProjection(OpIndex index, const ProjectionOp& projection)
    {
        ...
        if (input_type.IsNone()) {
            ...
            DCHECK(result_type.IsSubtypeOf(Typer::TypeForRepresentation(projection.rep)));
        } else {
    ...
    void ProcessWordBinop(V<Word> index, const WordBinopOp& binop)
    {
        ...
        Type result_type = Typer::TypeWordBinop(left_type, right_type, binop.kind, binop.rep, graph_zone_);
        SetType(index, result_type);
    }
    ...
    Type ComputeTypeForPhi(const PhiOp& phi)
    {
        ...
        Type result_type = MaybeTruncate(GetTypeOrDefault(phi.inputs()[0], Type::None()));
        for (size_t i = 1; i < phi.inputs().size(); ++i) {
            Type input_type = MaybeTruncate(GetTypeOrDefault(phi.inputs()[i], Type::None()));
    ...
    {
        TURBOSHAFT_TRACE_TYPING_OK("Br   %3d:%-40s\n", graph_.Index(*branch).id(), branch->ToString().substr(0, 40).c_str());
    ...
    void RefineOperationType(const Block* new_block, OpIndex op, const Type& type, char case_for_tracing)
    {
        ...
        TURBOSHAFT_TRACE_TYPING_OK(
            "  %c: %3d:%-40s ~~> %s\n", case_for_tracing, op.id(), graph_.Get(op).ToString().substr(0, 40).c_str(), type.ToString().c_str());
    ...
    void SetType(OpIndex index, Type result_type, bool allow_narrowing = false, bool is_fallback_for_unsupported_operation = false)
    {
        ...
        if (!is_fallback_for_unsupported_operation) {
            TURBOSHAFT_TRACE_TYPING_OK(
                "Type %3d:%-40s ==> %s\n", index.id(), graph_.Get(index).ToString().substr(0, 40).c_str(), result_type.ToString().c_str());
        } else {
    ...
    Type Widen(const Type& old_type, const Type& new_type)
    {
        ...
        case Type::Kind::kWord32:
            ...
            return WordOperationTyper<32>::WidenMaximal(old_type.AsWord32(), new_type.AsWord32(), graph_zone_);
        case Type::Kind::kWord64:
            ...
            return WordOperationTyper<64>::WidenMaximal(old_type.AsWord64(), new_type.AsWord64(), graph_zone_);
