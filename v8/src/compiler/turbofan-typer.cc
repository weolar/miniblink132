...
Typer::Typer(JSHeapBroker* broker, Flags flags, Graph* graph, TickCounter* tick_counter)
    : flags_(flags)
    , graph_(graph)
    , decorator_(nullptr)
...
#undef DECLARE_METHOD
    ...
    {
        return NodeProperties::IsTyped(node) ? NodeProperties::GetType(node) : Type::None();
    }
    ...
    JSHeapBroker* broker()
    {
        return typer_->broker();
    }

    void SetWeakened(NodeId node_id)
    {
        weakened_nodes_.insert(node_id);
    }
...
#undef DECLARE_METHOD
#define DECLARE_METHOD(Name, ...)                                                                                                                              \
    inline Type Type##Name(Type input)                                                                                                                         \
    {                                                                                                                                                          \
...
void Typer::Decorator::Decorate(Node* node)
{
    if (node->op()->ValueOutputCount() > 0) {
        ...
        if (is_typed || NodeProperties::AllValueInputsAreTyped(node)) {
            ...
            if (is_typed) {
                type = Type::Intersect(type, NodeProperties::GetType(node), typer_->zone());
            }
...
{
    return left.IsNone() || right.IsNone() ? Type::None() : f(left, right, typer_);
}

Type Typer::Visitor::BinaryNumberOpTyper(Type lhs, Type rhs, Typer* t, BinaryTyperFun f)
{
    lhs = ToNumeric(lhs, t);
    rhs = ToNumeric(rhs, t);
...
Typer::Visitor::ComparisonOutcome Typer::Visitor::Invert(ComparisonOutcome outcome, Typer* t)
{
...
Type Typer::Visitor::FalsifyUndefined(ComparisonOutcome outcome, Typer* t)
{
    ...
    if ((outcome & kComparisonFalse) != 0 || (outcome & kComparisonUndefined) != 0) {
        return (outcome & kComparisonTrue) != 0 ? Type::Boolean() : t->singleton_false_;
    }
...
Type Typer::Visitor::ToInteger(Type type, Typer* t)
{
    ...
    if (type.Is(t->cache_->kIntegerOrMinusZeroOrNaN)) {
        return Type::Union(Type::Intersect(type, t->cache_->kInteger, t->zone()), t->cache_->kSingletonZero, t->zone());
    }
...
Type Typer::Visitor::ObjectIsConstructor(Type type, Typer* t)
{
    ...
    CHECK(!type.IsNone());
    if (type.IsHeapConstant() && type.AsHeapConstant()->Ref().map(t->broker()).is_constructor()) {
        return t->singleton_true_;
    }
    if (!type.Maybe(Type::Callable()))
        return t->singleton_false_;
...
Type Typer::Visitor::TypeStart(Node* node)
{
...
Type Typer::Visitor::TypeTaggedIndexConstant(Node* node)
{
    UNREACHABLE();
}
...
Type Typer::Visitor::TypeCompressedHeapConstant(Node* node)
{
    UNREACHABLE();
}
...
Type Typer::Visitor::TypeInductionVariablePhi(Node* node)
{
    ...
    if (initial_type.IsNone() || increment_type.Is(typer_->cache_->kSingletonZero) || !initial_type.Is(typer_->cache_->kInteger)
        || !increment_type.Is(typer_->cache_->kInteger) || increment_type.Min() == -V8_INFINITY || increment_type.Max() == +V8_INFINITY) {
        ...
        Type type = NodeProperties::IsTyped(node) ? NodeProperties::GetType(node) : Type::None();
    ...
    if (v8_flags.trace_turbo_loop) {
        StdoutStream {} << std::setprecision(10) << "Loop (" << NodeProperties::GetControlInput(node)->id() << ") variable bounds in "
                        << (arithmetic_type == InductionVariable::ArithmeticType::kAddition ? "addition" : "subtraction") << " for phi " << node->id() << ": ("
                        << min << ", " << max << ")\n";
    }
...
bool Typer::Visitor::InductionVariablePhiTypeIsPrefixedPoint(InductionVariable* induction_var)
{
    ...
    for (auto bound : induction_var->upper_bounds()) {
        ...
        if (!bound_type.Is(typer_->cache_->kInteger))
            continue;
        if (!bound_type.IsNone()) {
            bound_type = Type::Range(-V8_INFINITY, bound_type.Max() - (bound.kind == InductionVariable::kStrict), zone());
        }
    ...
    for (auto bound : induction_var->lower_bounds()) {
        ...
        if (!bound_type.Is(typer_->cache_->kInteger))
            continue;
        if (!bound_type.IsNone()) {
            bound_type = Type::Range(bound_type.Min() + (bound.kind == InductionVariable::kStrict), +V8_INFINITY, typer_->zone());
        }
        type = Type::Intersect(type, bound_type, typer_->zone());
    }
...
Type Typer::Visitor::TypeLoopExitValue(Node* node)
{
...
Type Typer::Visitor::TypeFinishRegion(Node* node)
{
    return Operand(node, 0);
}
...
Type Typer::Visitor::TypeObjectId(Node* node)
{
    UNREACHABLE();
}
...
Type Typer::Visitor::TypeCall(Node* node)
{
    return Type::Any();
}
...
Type Typer::Visitor::TypeMapGuard(Node* node)
{
    UNREACHABLE();
}
...
Type Typer::Visitor::TypeDead(Node* node)
{
    return Type::None();
}
Type Typer::Visitor::TypeDeadValue(Node* node)
{
    return Type::None();
}
Type Typer::Visitor::TypeUnreachable(Node* node)
{
    return Type::None();
}

Type Typer::Visitor::TypePlug(Node* node)
{
    UNREACHABLE();
}
Type Typer::Visitor::TypeStaticAssert(Node* node)
{
    UNREACHABLE();
}
Type Typer::Visitor::TypeSLVerifierHint(Node* node)
{
    UNREACHABLE();
}
...
Typer::Visitor::ComparisonOutcome Typer::Visitor::JSCompareTyper(Type lhs, Type rhs, Typer* t)
{
...
Typer::Visitor::ComparisonOutcome Typer::Visitor::NumberCompareTyper(Type lhs, Type rhs, Typer* t)
{
...
Type Typer::Visitor::TypeJSCreate(Node* node)
{
    return Type::Object();
}
...
{
    SharedFunctionInfoRef shared = JSCreateClosureNode { node }.Parameters().shared_info();
...
Type Typer::Visitor::TypeJSParseInt(Type input)
{
    return Type::Number();
}

Type Typer::Visitor::TypeJSRegExpTest(Node* node)
{
    return Type::Boolean();
}
...
{
    static const double kWeakenMinLimits[] = { 0.0, -1073741824.0, -2147483648.0, -4294967296.0, -8589934592.0, -17179869184.0, -34359738368.0, -68719476736.0,
        -137438953472.0, -274877906944.0, -549755813888.0, -1099511627776.0, -2199023255552.0, -4398046511104.0, -8796093022208.0, -17592186044416.0,
        -35184372088832.0, -70368744177664.0, -140737488355328.0, -281474976710656.0, -562949953421312.0 };
    static const double kWeakenMaxLimits[] = { 0.0, 1073741823.0, 2147483647.0, 4294967295.0, 8589934591.0, 17179869183.0, 34359738367.0, 68719476735.0,
        137438953471.0, 274877906943.0, 549755813887.0, 1099511627775.0, 2199023255551.0, 4398046511103.0, 8796093022207.0, 17592186044415.0, 35184372088831.0,
        70368744177663.0, 140737488355327.0, 281474976710655.0, 562949953421311.0 };
    ...
    return Type::Union(current_type, Type::Range(new_min, new_max, typer_->zone()), typer_->zone());
}

Type Typer::Visitor::TypeJSSetKeyedProperty(Node* node)
{
    UNREACHABLE();
}

Type Typer::Visitor::TypeJSDefineKeyedOwnProperty(Node* node)
{
    UNREACHABLE();
}

Type Typer::Visitor::TypeJSSetNamedProperty(Node* node)
{
    UNREACHABLE();
}
...
Type Typer::Visitor::TypeJSDefineNamedOwnProperty(Node* node)
{
    UNREACHABLE();
}
...
Type Typer::Visitor::TypeJSStoreInArrayLiteral(Node* node)
{
    UNREACHABLE();
}
...
Type Typer::Visitor::TypeJSStoreScriptContext(Node* node)
{
    UNREACHABLE();
}
...
Type Typer::Visitor::TypeJSConstruct(Node* node)
{
    return Type::Receiver();
}
...
Type Typer::Visitor::TypeDateNow(Node* node)
{
    return Type::Number();
}

Type Typer::Visitor::TypeDoubleArrayMin(Node* node)
{
    return Type::Number();
}

Type Typer::Visitor::TypeDoubleArrayMax(Node* node)
{
    return Type::Number();
}
...
Type Typer::Visitor::JSCallTyper(Type fun, Typer* t)
{
    ...
    case Builtin::kDatePrototypeGetMilliseconds:
        return Type::Union(Type::Range(0.0, 999.0, t->zone()), Type::NaN(), t->zone());
    ...
    case Builtin::kStringPrototypeCharCodeAt:
        return Type::Union(Type::Range(0, kMaxUInt16, t->zone()), Type::NaN(), t->zone());
    case Builtin::kStringCharAt:
        return Type::String();
    case Builtin::kStringPrototypeCodePointAt:
        return Type::Union(Type::Range(0.0, String::kMaxCodePoint, t->zone()), Type::Undefined(), t->zone());
    ...
    case Builtin::kTypedArrayPrototypeToStringTag:
        return Type::Union(Type::InternalizedString(), Type::Undefined(), t->zone());
...
Type Typer::Visitor::TypeJSLoadMessage(Node* node)
{
    return Type::Any();
}
...
Type Typer::Visitor::TypeJSLoadModule(Node* node)
{
    return Type::Any();
}
...
Type Typer::Visitor::TypeJSGetImportMeta(Node* node)
{
    return Type::Any();
}
...
Type Typer::Visitor::TypeJSStackCheck(Node* node)
{
    return Type::Any();
}

Type Typer::Visitor::TypeJSDebugger(Node* node)
{
    return Type::Any();
}
...
{
    return FalsifyUndefined(NumberCompareTyper(ToNumber(lhs, t), ToNumber(rhs, t), t), t);
}

// static
Type Typer::Visitor::NumberLessThanOrEqualTyper(Type lhs, Type rhs, Typer* t)
{
    return FalsifyUndefined(Invert(JSCompareTyper(ToNumber(rhs, t), ToNumber(lhs, t), t), t), t);
}
...
Type Typer::Visitor::TypeNumberSameValue(Node* node)
{
    UNREACHABLE();
}
...
Type Typer::Visitor::TypeStringSubstring(Node* node)
{
    return Type::String();
}
...
{
    return typer_->operation_typer_.CheckBounds(Operand(node, 0), Operand(node, 1));
}
...
Type Typer::Visitor::TypeCompareMaps(Node* node)
{
    return Type::Boolean();
}
...
Type Typer::Visitor::TypeCheckEqualsSymbol(Node* node)
{
    UNREACHABLE();
}
...
Type Typer::Visitor::TypeAllocateRaw(Node* node)
{
    UNREACHABLE();
}
...
Type Typer::Visitor::TypeLoadMessage(Node* node)
{
    return Type::Any();
}
...
Type Typer::Visitor::TypeLoadFromObject(Node* node)
{
    UNREACHABLE();
}
Type Typer::Visitor::TypeLoadImmutableFromObject(Node* node)
{
    UNREACHABLE();
}
...
Type Typer::Visitor::TypeStoreMessage(Node* node)
{
    UNREACHABLE();
}
...
Type Typer::Visitor::TypeStoreToObject(Node* node)
{
    UNREACHABLE();
}
...
Type Typer::Visitor::TypeStoreSignedSmallElement(Node* node)
{
    UNREACHABLE();
}
...
Type Typer::Visitor::TypeStoreDataViewElement(Node* node)
{
    UNREACHABLE();
}
...
Type Typer::Visitor::TypeNumberIsFinite(Node* node)
{
    return Type::Boolean();
}
...
Type Typer::Visitor::TypeNumberIsInteger(Node* node)
{
    UNREACHABLE();
}
...
Type Typer::Visitor::TypeNumberIsSafeInteger(Node* node)
{
    UNREACHABLE();
}
...
Type Typer::Visitor::TypeRuntimeAbort(Node* node)
{
    UNREACHABLE();
}

Type Typer::Visitor::TypeAssertType(Node* node)
{
    UNREACHABLE();
}
...
Type Typer::Visitor::TypeJSGetIterator(Node* node)
{
    return Type::Any();
}
