...
Node* Graph::NewNode(const Operator* op, int input_count, Node* const* inputs, bool incomplete)
{
...
Node* Graph::NewNodeUnchecked(const Operator* op, int input_count, Node* const* inputs, bool incomplete)
{
...
{
    StdoutStream {} << AsRPO(*this);
}

void Graph::RecordSimdStore(Node* store)
{
    simd_stores_.push_back(store);
}
