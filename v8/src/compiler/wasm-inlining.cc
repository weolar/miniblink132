...
{
    DCHECK(call->opcode() == IrOpcode::kCall || call->opcode() == IrOpcode::kTailCall);
    ...
    IrOpcode::Value reloc_opcode = mcgraph_->machine()->Is32() ? IrOpcode::kRelocatableInt32Constant : IrOpcode::kRelocatableInt64Constant;
    ...
    if (function_inlining_count_[inlinee_index] >= kMaximumInlinedCallsPerFunction) {
        Trace(call, inlinee_index, "too many inlined calls to (recursive?) function");
        return NoChange();
    }

    CHECK_LT(inlinee_index, module()->functions.size());
    const wasm::WasmFunction* inlinee = &module()->functions[inlinee_index];
    base::Vector<const uint8_t> function_bytes = data_.wire_bytes_storage->GetCode(inlinee->code);

    int call_count = GetCallCount(call);

    int wire_byte_size = static_cast<int>(function_bytes.size());
    int min_count_for_inlining = v8_flags.wasm_inlining_ignore_call_counts ? 0 : wire_byte_size / 2;

    // If liftoff ran and collected call counts, only inline calls that have been
    // invoked often, except for truly tiny functions.
    if (v8_flags.liftoff && v8_flags.wasm_inlining && wire_byte_size >= 12 && call_count < min_count_for_inlining) {
    ...
    CandidateInfo candidate { call, inlinee_index, call_count, function_bytes.length() };

    inlining_candidates_.push(candidate);
    return NoChange();
}

bool SmallEnoughToInline(const wasm::WasmModule* module, size_t current_graph_size, uint32_t candidate_size, size_t initial_graph_size)
{
    if (candidate_size > v8_flags.wasm_inlining_max_size) {
        return false;
    }
    if (WasmInliner::graph_size_allows_inlining(module, current_graph_size + candidate_size, initial_graph_size)) {
        return true;
    }
    // For truly tiny functions, let's be a bit more generous.
    return candidate_size <= 12 && WasmInliner::graph_size_allows_inlining(module, current_graph_size - 100, initial_graph_size);
}

bool WasmInliner::graph_size_allows_inlining(const wasm::WasmModule* module, size_t graph_size, size_t initial_graph_size)
{
    size_t budget = std::max<size_t>(v8_flags.wasm_inlining_min_budget, v8_flags.wasm_inlining_factor * initial_graph_size);
    ...
    double small_function_percentage = module->num_small_functions * 100.0 / module->num_declared_functions;
...
{
    TRACE("  [function %d: considering candidate {@%d, index=%d, count=%d, "
          "size=%d, score=%" PRId64 "} graphsize=%zu: %s]\n",
        data_.func_index, candidate.node->id(), candidate.inlinee_index, candidate.call_count, candidate.wire_byte_size, candidate.score(), current_graph_size_,
        decision);
}
...
void WasmInliner::Finalize()
{
    TRACE("[function %d (%s): %s]\n", data_.func_index, debug_name_,
        inlining_candidates_.empty() ? "no inlining candidates" : "going through inlining candidates");
    if (inlining_candidates_.empty())
        return;
    while (!inlining_candidates_.empty()) {
        ...
        if (!SmallEnoughToInline(module(), current_graph_size_, candidate.wire_byte_size, initial_graph_size_)) {
        ...
        for (uint32_t i = 0; i < inlinee->sig->parameter_count(); i++) {
            if (!NodeProperties::IsTyped(call->InputAt(i + 2)))
                continue;
            wasm::TypeInModule param_type = NodeProperties::GetType(call->InputAt(i + 2)).AsWasm();
            CHECK(IsSubtypeOf(param_type.type, inlinee->sig->GetParam(i), param_type.module, module()));
        }
#endif
        ...
        const wasm::FunctionBody inlinee_body { inlinee->sig, inlinee->code.offset(), function_bytes.begin(), function_bytes.end(), is_shared };

        // If the inlinee was not validated before, do that now.
        if (V8_UNLIKELY(!module()->function_was_validated(candidate.inlinee_index))) {
            if (ValidateFunctionBody(zone(), env_->enabled_features, module(), detected_, inlinee_body).failed()) {
        ...
        Node* inlinee_start;
        Node* inlinee_end;
        SourcePosition caller_pos = data_.source_positions->GetSourcePosition(candidate.node);
        inlining_positions_->push_back({ static_cast<int>(candidate.inlinee_index), call->opcode() == IrOpcode::kTailCall, caller_pos });
        int inlining_position_id = static_cast<int>(inlining_positions_->size()) - 1;
        WasmGraphBuilder builder(env_, zone(), mcgraph_, inlinee_body.sig, data_.source_positions, WasmGraphBuilder::kInstanceParameterMode,
            nullptr /* isolate */, env_->enabled_features);
        builder.set_inlining_id(inlining_position_id);
        {
            Graph::SubgraphScope scope(graph());
            wasm::BuildTFGraph(zone()->allocator(), env_->enabled_features, module(), &builder, detected_, inlinee_body, &inlinee_loop_infos,
                &dangling_exceptions, data_.node_origins, candidate.inlinee_index, data_.assumptions,
                NodeProperties::IsExceptionalCall(call) ? wasm::kInlinedHandledCall : wasm::kInlinedNonHandledCall);
        ...
        if (call->opcode() == IrOpcode::kCall) {
            InlineCall(call, inlinee_start, inlinee_end, inlinee->sig, caller_pos, &dangling_exceptions);
        } else {
            InlineTailCall(call, inlinee_start, inlinee_end);
        }
        call->Kill();
        data_.loop_infos->insert(data_.loop_infos->end(), inlinee_loop_infos.begin(), inlinee_loop_infos.end());
...
void WasmInliner::InlineTailCall(Node* call, Node* callee_start, Node* callee_end)
{
...
void WasmInliner::InlineCall(Node* call, Node* callee_start, Node* callee_end, const wasm::FunctionSig* inlinee_sig, SourcePosition parent_pos,
    wasm::DanglingExceptions* dangling_exceptions)
{
    ...
    for (Node* const input : callee_end->inputs()) {
        ...
        case IrOpcode::kTailCall: {
            ...
            if (is_exceptional_call) {
                ...
                Node* if_exception = graph()->NewNode(mcgraph()->common()->IfException(), input, control);
            ...
            if (return_arity == 1) {
                ...
                // node.
                NodeProperties::SetType(input, Type::Wasm({ inlinee_sig->GetReturn(0), module() }, graph()->zone()));
                return_inputs.push_back(input);
            } else if (return_arity > 1) {
                for (int i = 0; i < return_arity; i++) {
                    Node* ith_projection = graph()->NewNode(common()->Projection(i), input, control);
                    // Similarly here we have to type the call's projections.
                    NodeProperties::SetType(ith_projection, Type::Wasm({ inlinee_sig->GetReturn(i), module() }, graph()->zone()));
            ...
            Node* ret = graph()->NewNode(common()->Return(return_arity), static_cast<int>(return_inputs.size()), return_inputs.data());
    ...
    if (is_exceptional_call) {
        ...
        if (handler_count > 0) {
            Node* control_output = graph()->NewNode(common()->Merge(handler_count), handler_count, dangling_exceptions->controls.data());
            ...
            Node* value_output = graph()->NewNode(common()->Phi(MachineRepresentation::kTagged, handler_count), handler_count + 1, values.data());
            Node* effect_output = graph()->NewNode(common()->EffectPhi(handler_count), handler_count + 1, effects.data());
            ...
            ReplaceWithValue(handler, mcgraph()->Dead(), mcgraph()->Dead(), mcgraph()->Dead());
        }
    }
     ...
     * and merges. */
        ...
        Node* control_output = graph()->NewNode(common()->Merge(return_count), return_count, &controls.front());
        effects.push_back(control_output);
        Node* effect_output = graph()->NewNode(common()->EffectPhi(return_count), static_cast<int>(effects.size()), &effects.front());
        ...
        for (int i = 0; i < return_arity; i++) {
            ...
            // inlinee signature.
            MachineRepresentation repr = inlinee_sig->GetReturn(i).machine_representation();
            Node* ith_value_output = graph()->NewNode(common()->Phi(repr, return_count), static_cast<int>(ith_values.size()), &ith_values.front());
            values.push_back(ith_value_output);
        }
        for (Node* return_node : return_nodes)
            return_node->Kill();
        ...
        ReplaceWithValue(call, mcgraph()->Dead(), mcgraph()->Dead(), mcgraph()->Dead());
    }
}
