...
template <class T> struct may_be_unequal {
...
//              something similar.
template <class Key, class Value, class Hasher = base::hash<Key>> class PersistentMap {
public:
    using key_type = Key;
...
public:
    ...
    template <class F> void Modify(Key key, F f);
    ...
    struct ZipIterable {
        ...
        {
            return double_iterator(a.begin(), b.begin());
        }
        double_iterator end()
        {
            return double_iterator(a.end(), b.end());
        }
    };
...
template <class Key, class Value, class Hasher> struct may_be_unequal<PersistentMap<Key, Value, Hasher>> {
...
// lower than before.
template <class Key, class Value, class Hasher> struct PersistentMap<Key, Value, Hasher>::FocusedTree {
    KeyValue key_value;
    ...
    {
        DCHECK(i < length);
        return reinterpret_cast<const FocusedTree**>(reinterpret_cast<uint8_t*>(this) + offsetof(FocusedTree, path_array))[i];
    }
    const FocusedTree* path(int i) const
    {
        DCHECK(i < length);
        return reinterpret_cast<const FocusedTree* const*>(reinterpret_cast<const uint8_t*>(this) + offsetof(FocusedTree, path_array))[i];
    }
};

template <class Key, class Value, class Hasher> class PersistentMap<Key, Value, Hasher>::HashValue {
public:
    ...
    Bit operator[](int pos) const
    {
        ...
        return bits_ & (static_cast<decltype(bits_)>(1) << (kHashBits - pos - 1)) ? kRight : kLeft;
    }
...
template <class Key, class Value, class Hasher> class PersistentMap<Key, Value, Hasher>::iterator {
public:
...
template <class Key, class Value, class Hasher> class PersistentMap<Key, Value, Hasher>::double_iterator {
public:
    std::tuple<Key, Value, Value> operator*()
    {
        if (first_current_) {
            auto pair = *first_;
            return std::make_tuple(pair.first, pair.second, second_current_ ? (*second_).second : second_.def_value());
        } else {
...
template <class Key, class Value, class Hasher> void PersistentMap<Key, Value, Hasher>::Set(Key key, Value new_value)
{
    Modify(key, [&](Value* value) { *value = std::move(new_value); });
}

template <class Key, class Value, class Hasher> template <class F> void PersistentMap<Key, Value, Hasher>::Modify(Key key, F f)
{
    ...
    f(&new_value);
    if (!may_be_unequal<Value>()(old_value, new_value))
        return;
    ...
    size_t size = sizeof(FocusedTree) + std::max(0, length - 1) * sizeof(const FocusedTree*);
    FocusedTree* tree = new (zone_->Allocate<FocusedTree>(size))
        FocusedTree { KeyValue(std::move(key), std::move(new_value)), static_cast<int8_t>(length), key_hash, more, {} };
...
const typename PersistentMap<Key, Value, Hasher>::FocusedTree* PersistentMap<Key, Value, Hasher>::FindHash(HashValue hash) const
{
...
template <class Key, class Value, class Hasher>
const typename PersistentMap<Key, Value, Hasher>::FocusedTree* PersistentMap<Key, Value, Hasher>::FindHash(
    HashValue hash, std::array<const FocusedTree*, kHashBits>* path, int* length) const
{
    const FocusedTree* tree = tree_;
    int level = 0;
    while (tree && hash != tree->key_hash) {
...
template <class Key, class Value, class Hasher> const Value& PersistentMap<Key, Value, Hasher>::GetFocusedValue(const FocusedTree* tree, const Key& key) const
{
    if (!tree) {
        return def_value_;
    }
...
template <class Key, class Value, class Hasher>
const typename PersistentMap<Key, Value, Hasher>::FocusedTree* PersistentMap<Key, Value, Hasher>::GetChild(const FocusedTree* tree, int level, Bit bit)
{
...
template <class Key, class Value, class Hasher>
const typename PersistentMap<Key, Value, Hasher>::FocusedTree* PersistentMap<Key, Value, Hasher>::FindLeftmost(
    const FocusedTree* start, int* level, std::array<const FocusedTree*, kHashBits>* path)
{
    ...
    while (*level < current->length) {
        if (const FocusedTree* left_child = GetChild(current, *level, kLeft)) {
            ...
            current = left_child;
            ++*level;
        } else if (const FocusedTree* right_child = GetChild(current, *level, kRight)) {
...
template <class Key, class Value, class Hasher> std::ostream& operator<<(std::ostream& os, const PersistentMap<Key, Value, Hasher>& map)
{
    os << "{";
    bool first = true;
    for (auto pair : map) {
