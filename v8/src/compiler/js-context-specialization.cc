...
Reduction JSContextSpecialization::ReduceParameter(Node* node)
{
    ...
    if (index == Linkage::kJSCallClosureParamIndex) {
        ...
        if (closure().ToHandle(&function)) {
            Node* value = jsgraph()->ConstantNoHole(MakeRef(broker_, function), broker());
...
Reduction JSContextSpecialization::SimplifyJSLoadContext(Node* node, Node* new_context, size_t new_depth)
{
    ...
    const Operator* op = jsgraph_->javascript()->LoadContext(new_depth, access.index(), access.immutable());
    NodeProperties::ReplaceContextInput(node, new_context);
    NodeProperties::ChangeOp(node, op);
    return Changed(node);
}

Reduction JSContextSpecialization::SimplifyJSLoadScriptContext(Node* node, Node* new_context, size_t new_depth)
{
    ...
    const Operator* op = jsgraph_->javascript()->LoadScriptContext(new_depth, access.index());
    NodeProperties::ReplaceContextInput(node, new_context);
    NodeProperties::ChangeOp(node, op);
    return Changed(node);
}

Reduction JSContextSpecialization::SimplifyJSStoreContext(Node* node, Node* new_context, size_t new_depth)
{
...
{
    DCHECK_EQ(IrOpcode::kParameter, node->opcode());
    return ParameterIndexOf(node->op()) == StartNode { NodeProperties::GetValueInput(node, 0) }.ContextParameterIndex_MaybeNonStandardLayout();
}
...
OptionalContextRef GetSpecializationContext(JSHeapBroker* broker, Node* node, size_t* distance, Maybe<OuterContext> maybe_outer)
{
    switch (node->opcode()) {
    case IrOpcode::kHeapConstant: {
        ...
        // unnecessary.
        HeapObjectRef object = MakeRefAssumeMemoryFence(broker, HeapConstantOf(node->op()));
        if (object.IsContext())
...
Reduction JSContextSpecialization::ReduceJSLoadContext(Node* node)
{
    ...
    OptionalContextRef maybe_concrete = GetSpecializationContext(broker(), context, &depth, outer());
    ...
    if (depth > 0) {
        ...
        return SimplifyJSLoadContext(node, jsgraph()->ConstantNoHole(concrete, broker()), depth);
    }

    if (!access.immutable()
        && !broker()->dependencies()->DependOnScriptContextSlotProperty(concrete, access.index(), ContextSidePropertyCell::kConst, broker())) {
    ...
    if (!maybe_value.has_value()) {
        TRACE_BROKER_MISSING(broker(), "slot value " << access.index() << " for context " << concrete);
        return SimplifyJSLoadContext(node, jsgraph()->ConstantNoHole(concrete, broker()), depth);
    }
...
Reduction JSContextSpecialization::ReduceJSLoadScriptContext(Node* node)
{
    ...
    if (depth > 0) {
        TRACE_BROKER_MISSING(broker(), "previous value for context " << concrete);
        return SimplifyJSLoadScriptContext(node, jsgraph()->ConstantNoHole(concrete, broker()), depth);
    }

    DCHECK(concrete.object()->IsScriptContext());
    auto property = concrete.object()->GetScriptContextSideProperty(access.index());
    switch (property) {
    case ContextSidePropertyCell::kConst: {
        broker()->dependencies()->DependOnScriptContextSlotProperty(concrete, access.index(), property, broker());
        OptionalObjectRef maybe_value = concrete.get(broker(), static_cast<int>(access.index()));
    ...
    case ContextSidePropertyCell::kSmi: {
        ...
        Node* load = effect = jsgraph_->graph()->NewNode(
            jsgraph_->simplified()->LoadField(AccessBuilder::ForContextSlotSmi(access.index())), jsgraph_->ConstantNoHole(concrete, broker()), effect, control);
    ...
    case ContextSidePropertyCell::kMutableHeapNumber: {
        ...
        Node* heap_number = effect = jsgraph_->graph()->NewNode(
            jsgraph_->simplified()->LoadField(AccessBuilder::ForContextSlot(access.index())), jsgraph_->ConstantNoHole(concrete, broker()), effect, control);
        Node* double_load = effect
            = jsgraph_->graph()->NewNode(jsgraph_->simplified()->LoadField(AccessBuilder::ForHeapNumberValue()), heap_number, effect, control);
...
Reduction JSContextSpecialization::ReduceJSStoreContext(Node* node)
{
    ...
    if (depth > 0) {
        TRACE_BROKER_MISSING(broker(), "previous value for context " << concrete);
        return SimplifyJSStoreContext(node, jsgraph()->ConstantNoHole(concrete, broker()), depth);
    }
...
Reduction JSContextSpecialization::ReduceJSStoreScriptContext(Node* node)
{
    ...
    OptionalContextRef maybe_context = GetSpecializationContext(broker(), context, &depth, outer());
    if (IsConstTrackingLetVariableSurelyNotConstant(maybe_context, depth, side_data_index, broker())) {
        // The value is not a constant any more, so we don't need to generate
        // code for invalidating the side data.
        const Operator* op = jsgraph_->javascript()->StoreContext(depth, access.index());
        Node* new_store = effect = jsgraph_->graph()->NewNode(op, NodeProperties::GetValueInput(node, 0), context, effect, control);
    ...
    for (size_t i = 0; i < depth; ++i) {
        context = effect = jsgraph_->graph()->NewNode(
            jsgraph_->simplified()->LoadField(AccessBuilder::ForContextSlotKnownPointer(Context::PREVIOUS_INDEX)), context, effect, control);
    }

    GenerateCheckConstTrackingLetSideData(context, &effect, &control, side_data_index, jsgraph_);
...
OptionalContextRef GetModuleContext(JSHeapBroker* broker, Node* node, Maybe<OuterContext> maybe_context)
{
    ...
    case IrOpcode::kHeapConstant: {
        ...
        // ObjectData pointer instead, which would make new ref construction here
        // unnecessary.
        HeapObjectRef object = MakeRefAssumeMemoryFence(broker, HeapConstantOf(context->op()));
        if (object.IsContext()) {
...
Reduction JSContextSpecialization::ReduceJSGetImportMeta(Node* node)
{
    ...
    if (!module.has_value())
        return NoChange();
    OptionalObjectRef import_meta = module->AsSourceTextModule().import_meta(broker());
    if (!import_meta.has_value())
        return NoChange();
