...
#define TRACE_BROKER_MISSING(broker, x)                                                                                                                        \
    do {                                                                                                                                                       \
        if (broker->tracing_enabled())                                                                                                                         \
            StdoutStream {} << broker->Trace() << "Missing " << x << " (" << __FILE__ << ":" << __LINE__ << ")" << std::endl;                                  \
    } while (false)
...
struct PropertyAccessTarget {
    ...
    struct Hash {
        ...
        {
            return base::hash_combine(base::hash_combine(pair.map.object().address(), pair.name.object().address()), static_cast<int>(pair.mode));
        }
    };
    struct Equal {
        bool operator()(const PropertyAccessTarget& lhs, const PropertyAccessTarget& rhs) const
        {
            return lhs.map.equals(rhs.map) && lhs.name.equals(rhs.name) && lhs.mode == rhs.mode;
        }
    };
};
...
public:
    ...
    JSHeapBroker(Isolate* isolate, Zone* broker_zone)
        : JSHeapBroker(isolate, broker_zone, v8_flags.trace_heap_broker, CodeKind::TURBOFAN_JS)
    {
    }
    ...
    {
        return IsMainThread() ? NexusConfig::FromMainThread(isolate()) : NexusConfig::FromBackgroundThread(isolate(), local_isolate()->heap());
    }
    ...
    template <typename CompilationInfoT> void AttachCompilationInfo(CompilationInfoT* info)
    {
    ...
    ObjectData* GetOrCreateData(Tagged<Object> object, GetOrCreateDataFlags flags = {});
    ...
    ObjectData* TryGetOrCreateData(Tagged<Object> object, GetOrCreateDataFlags flags = {});
    ...
    {
        return local_isolate() != nullptr ? local_isolate() : isolate()->AsLocalIsolate();
    }
    ...
    template <typename T> Handle<T> CanonicalPersistentHandle(Tagged<T> object)
    {
        ...
        if (local_isolate()) {
            *find_result.entry = local_isolate()->heap()->NewPersistentHandle(obj).location();
        } else {
    ...
    template <typename T> Handle<T> CanonicalPersistentHandle(Handle<T> object)
    {
        if (object.is_null())
            return object; // Can't deref a null handle.
        return CanonicalPersistentHandle<T>(*object);
    }

    // Checks if a canonical persistent handle for {object} exists.
    template <typename T> bool IsCanonicalHandle(Handle<T> handle)
    {
        DCHECK_NOT_NULL(canonical_handles_);
    ...
    protected:
        V8_INLINE RecursiveSharedMutexGuardIfNeeded(LocalIsolate* local_isolate, base::SharedMutex* mutex, int* mutex_depth_address);
    ...
    class MapUpdaterGuardIfNeeded final : public RecursiveSharedMutexGuardIfNeeded {
    public:
        V8_INLINE explicit MapUpdaterGuardIfNeeded(JSHeapBroker* broker);
    };

    class BoilerplateMigrationGuardIfNeeded final : public RecursiveSharedMutexGuardIfNeeded {
    public:
...
#define V(Type, name, Name) OptionalRef<typename ref_traits<Type>::ref_type> name##_;
    READ_ONLY_ROOT_LIST(V)
#undef V
...
public:
    ...
    ~TraceScope()
    {
        broker_->DecrementTracingIndentation();
    }
...
public:
    explicit UnparkedScopeIfNeeded(JSHeapBroker* broker, bool extra_condition = true)
    {
...
public:
    JSHeapBrokerScopeForTesting(JSHeapBroker* broker, Isolate* isolate, Zone* zone)
        : JSHeapBrokerScopeForTesting(broker, std::make_unique<CanonicalHandlesMap>(isolate->heap(), ZoneAllocationPolicy(zone)))
    {
    }
    ...
    {
        broker_->set_canonical_handles(nullptr);
    }
...
template <class T, typename = std::enable_if_t<is_subtype_v<T, Object>>> typename ref_traits<T>::ref_type MakeRef(JSHeapBroker* broker, Tagged<T> object)
{
    return TryMakeRef(broker, object, kCrashOnError).value();
}

template <class T, typename = std::enable_if_t<is_subtype_v<T, Object>>> typename ref_traits<T>::ref_type MakeRef(JSHeapBroker* broker, Handle<T> object)
{
    return TryMakeRef(broker, object, kCrashOnError).value();
}

template <class T, typename = std::enable_if_t<is_subtype_v<T, Object>>>
typename ref_traits<T>::ref_type MakeRefAssumeMemoryFence(JSHeapBroker* broker, Tagged<T> object)
{
    return TryMakeRef(broker, object, kAssumeMemoryFence | kCrashOnError).value();
}

template <class T, typename = std::enable_if_t<is_subtype_v<T, Object>>>
typename ref_traits<T>::ref_type MakeRefAssumeMemoryFence(JSHeapBroker* broker, Handle<T> object)
{
    return TryMakeRef(broker, object, kAssumeMemoryFence | kCrashOnError).value();
}
