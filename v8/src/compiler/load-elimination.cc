...
LoadElimination::AbstractState const LoadElimination::AbstractState::empty_state_;

Node* LoadElimination::AbstractElements::Lookup(Node* object, Node* index, MachineRepresentation representation) const
{
...
LoadElimination::AbstractElements const* LoadElimination::AbstractElements::Kill(Node* object, Node* index, Zone* zone) const
{
    for (Element const element : this->elements_) {
        ...
        if (MayAlias(object, element.object)) {
            ...
            for (Element const element2 : this->elements_) {
                if (element2.object == nullptr)
                    continue;
                DCHECK_NOT_NULL(element2.index);
                DCHECK_NOT_NULL(element2.value);
                if (!MayAlias(object, element2.object) || !NodeProperties::GetType(index).Maybe(NodeProperties::GetType(element2.index))) {
...
bool LoadElimination::AbstractElements::Equals(AbstractElements const* that) const
{
    if (this == that)
        return true;
...
LoadElimination::AbstractElements const* LoadElimination::AbstractElements::Merge(AbstractElements const* that, Zone* zone) const
{
    if (this->Equals(that))
        return this;
...
void LoadElimination::AbstractElements::Print() const
{
    for (Element const& element : elements_) {
        if (element.object) {
            PrintF("    #%d:%s @ #%d:%s -> #%d:%s\n", element.object->id(), element.object->op()->mnemonic(), element.index->id(),
                element.index->op()->mnemonic(), element.value->id(), element.value->op()->mnemonic());
        }
    }
}

LoadElimination::FieldInfo const* LoadElimination::AbstractField::Lookup(Node* object) const
{
...
LoadElimination::AbstractField const* LoadElimination::AbstractField::KillConst(Node* object, Zone* zone) const
{
    for (auto info1 : this->info_for_node_) {
        if (info1.first->IsDead())
            continue;
...
LoadElimination::AbstractField const* LoadElimination::AbstractField::Kill(const AliasStateInfo& alias_info, MaybeHandle<Name> name, Zone* zone) const
{
    for (auto info1 : this->info_for_node_) {
        if (info1.first->IsDead())
            continue;
        if (alias_info.MayAlias(info1.first)) {
            AbstractField* that = zone->New<AbstractField>(zone);
            for (auto info2 : this->info_for_node_) {
                if (!alias_info.MayAlias(info2.first) || !MayAlias(name, info2.second.name)) {
                    that->info_for_node_.insert(info2);
                }
            }
            return that;
...
{
    for (auto pair : info_for_node_) {
        PrintF("    #%d:%s -> #%d:%s [repr=%s]\n", pair.first->id(), pair.first->op()->mnemonic(), pair.second.value->id(), pair.second.value->op()->mnemonic(),
...
LoadElimination::AbstractMaps::AbstractMaps(Node* object, ZoneRefSet<Map> maps, Zone* zone)
    : info_for_node_(zone)
{
...
bool LoadElimination::AbstractMaps::Lookup(Node* object, ZoneRefSet<Map>* object_maps) const
{
...
LoadElimination::AbstractMaps const* LoadElimination::AbstractMaps::Kill(const AliasStateInfo& alias_info, Zone* zone) const
{
    for (auto info1 : this->info_for_node_) {
        if (alias_info.MayAlias(info1.first)) {
...
LoadElimination::AbstractMaps const* LoadElimination::AbstractMaps::Merge(AbstractMaps const* that, Zone* zone) const
{
    if (this->Equals(that))
        return this;
...
LoadElimination::AbstractMaps const* LoadElimination::AbstractMaps::Extend(Node* object, ZoneRefSet<Map> maps, Zone* zone) const
{
...
void LoadElimination::AbstractMaps::Print() const
{
    ...
    StdoutStream os;
    for (auto pair : info_for_node_) {
        os << "    #" << pair.first->id() << ":" << pair.first->op()->mnemonic() << std::endl;
...
bool LoadElimination::AbstractState::FieldsEquals(AbstractFields const& this_fields, AbstractFields const& that_fields) const
{
...
bool LoadElimination::AbstractState::Equals(AbstractState const* that) const
{
    ...
    if (!FieldsEquals(this->fields_, that->fields_) || !FieldsEquals(this->const_fields_, that->const_fields_)) {
        return false;
    }
    if (this->maps_) {
...
void LoadElimination::AbstractState::FieldsMerge(AbstractFields* this_fields, AbstractFields const& that_fields, Zone* zone)
{
...
void LoadElimination::AbstractState::Merge(AbstractState const* that, Zone* zone)
{
    // Merge the information we have about the elements.
    if (this->elements_) {
        this->elements_ = that->elements_ ? that->elements_->Merge(this->elements_, zone) : nullptr;
    }
...
bool LoadElimination::AbstractState::LookupMaps(Node* object, ZoneRefSet<Map>* object_map) const
{
    return this->maps_ && this->maps_->Lookup(object, object_map);
}

LoadElimination::AbstractState const* LoadElimination::AbstractState::SetMaps(Node* object, ZoneRefSet<Map> maps, Zone* zone) const
{
...
LoadElimination::AbstractState const* LoadElimination::AbstractState::KillMaps(const AliasStateInfo& alias_info, Zone* zone) const
{
    if (this->maps_) {
...
LoadElimination::AbstractState const* LoadElimination::AbstractState::KillMaps(Node* object, Zone* zone) const
{
    AliasStateInfo alias_info(this, object);
    return KillMaps(alias_info, zone);
}

Node* LoadElimination::AbstractState::LookupElement(Node* object, Node* index, MachineRepresentation representation) const
{
    if (this->elements_) {
...
LoadElimination::AbstractState const* LoadElimination::AbstractState::AddElement(
    Node* object, Node* index, Node* value, MachineRepresentation representation, Zone* zone) const
{
    AbstractState* that = zone->New<AbstractState>(*this);
    if (that->elements_) {
        that->elements_ = that->elements_->Extend(object, index, value, representation, zone);
    } else {
        that->elements_ = zone->New<AbstractElements>(object, index, value, representation, zone);
    }
    return that;
}

LoadElimination::AbstractState const* LoadElimination::AbstractState::KillElement(Node* object, Node* index, Zone* zone) const
{
    if (this->elements_) {
...
LoadElimination::AbstractState const* LoadElimination::AbstractState::AddField(
    Node* object, IndexRange index_range, LoadElimination::FieldInfo info, Zone* zone) const
{
    AbstractState* that = zone->New<AbstractState>(*this);
    ...
    for (int index : index_range) {
        ...
        if (fields[index]) {
            fields[index] = fields[index]->Extend(object, info, zone, that->fields_count_);
        } else {
            fields[index] = zone->New<AbstractField>(object, info, zone);
        }
        int added = fields[index]->count() - count_before;
        if (is_const)
            that->const_fields_count_ += added;
        that->fields_count_ += added;
    }
    return that;
}

LoadElimination::AbstractState const* LoadElimination::AbstractState::KillConstField(Node* object, IndexRange index_range, Zone* zone) const
{
    AliasStateInfo alias_info(this, object);
    ...
    for (int index : index_range) {
        if (AbstractField const* this_field = this->const_fields_[index]) {
            ...
            if (this->const_fields_[index] != this_field) {
                if (!that)
                    that = zone->New<AbstractState>(*this);
                that->const_fields_[index] = this_field;
                int removed = this->const_fields_[index]->count() - that->const_fields_[index]->count();
...
LoadElimination::AbstractState const* LoadElimination::AbstractState::KillField(Node* object, IndexRange index_range, MaybeHandle<Name> name, Zone* zone) const
{
    AliasStateInfo alias_info(this, object);
...
LoadElimination::AbstractState const* LoadElimination::AbstractState::KillField(
    const AliasStateInfo& alias_info, IndexRange index_range, MaybeHandle<Name> name, Zone* zone) const
{
    AbstractState* that = nullptr;
    for (int index : index_range) {
        if (AbstractField const* this_field = this->fields_[index]) {
            ...
            if (this->fields_[index] != this_field) {
                ...
                int removed = this->fields_[index]->count() - that->fields_[index]->count();
                that->fields_count_ -= removed;
            }
        }
    }
...
LoadElimination::AbstractState const* LoadElimination::AbstractState::KillFields(Node* object, MaybeHandle<Name> name, Zone* zone) const
{
    AliasStateInfo alias_info(this, object);
    for (size_t i = 0;; ++i) {
        if (i == fields_.size()) {
...
LoadElimination::AbstractState const* LoadElimination::AbstractState::KillAll(Zone* zone) const
{
...
LoadElimination::FieldInfo const* LoadElimination::AbstractState::LookupField(Node* object, IndexRange index_range, ConstFieldInfo const_field_info) const
{
    ...
    for (int index : index_range) {
        ...
        if (const_field_info.IsConst()) {
            ...
            if (!(info && info->const_field_info == const_field_info))
                return nullptr;
        } else {
...
LoadElimination::AbstractState const* LoadElimination::AbstractStateForEffectNodes::Get(Node* node) const
{
...
void LoadElimination::AbstractStateForEffectNodes::Set(Node* node, AbstractState const* state)
{
    size_t const id = node->id();
    if (id >= info_for_node_.size())
...
Reduction LoadElimination::ReduceEnsureWritableFastElements(Node* node)
{
    ...
    state = state->KillField(object, FieldIndexOf(JSObject::kElementsOffset, kTaggedSize), MaybeHandle<Name>(), zone());
    // Add the new elements on {object}.
    state = state->AddField(object, FieldIndexOf(JSObject::kElementsOffset, kTaggedSize), { node, MachineRepresentation::kTaggedPointer }, zone());
    return UpdateState(node, state);
}
...
Reduction LoadElimination::ReduceMaybeGrowFastElements(Node* node)
{
    ...
    if (params.mode() == GrowFastElementsMode::kDoubleElements) {
        ...
        state = state->SetMaps(node, ZoneRefSet<Map>(broker()->fixed_double_array_map()), zone());
    } else {
        ...
        ZoneRefSet<Map> fixed_array_maps({ broker()->fixed_array_map(), broker()->fixed_cow_array_map() }, zone());
        state = state->SetMaps(node, fixed_array_maps, zone());
    }
    // Kill the previous elements on {object}.
    state = state->KillField(object, FieldIndexOf(JSObject::kElementsOffset, kTaggedSize), MaybeHandle<Name>(), zone());
...
Reduction LoadElimination::ReduceTransitionElementsKind(Node* node)
{
    ...
    case ElementsTransition::kSlowTransition:
        ...
        state = state->KillField(alias_info, FieldIndexOf(JSObject::kElementsOffset, kTaggedSize), MaybeHandle<Name>(), zone());
        break;
    }
    ZoneRefSet<Map> object_maps;
    if (state->LookupMaps(object, &object_maps)) {
...
Reduction LoadElimination::ReduceLoadField(Node* node, FieldAccess const& access)
{
    ...
    if (access.offset == HeapObject::kMapOffset && access.base_is_tagged == kTaggedBase) {
        ...
        if (field_index != IndexRange::Invalid()) {
            ...
            FieldInfo const* lookup_result = state->LookupField(object, field_index, access.const_field_info);
            if (!lookup_result && access.const_field_info.IsConst()) {
                ...
                lookup_result = state->LookupField(object, field_index, ConstFieldInfo::None());
            }
            if (lookup_result) {
                ...
                if (IsCompatible(representation, lookup_result->representation) && !replacement->IsDead()) {
                    // Introduce a TypeGuard if the type of the {replacement} node is not
                    // a subtype of the original {node}'s type.
                    if (!NodeProperties::GetType(replacement).Is(NodeProperties::GetType(node))) {
                        Type replacement_type = Type::Intersect(NodeProperties::GetType(node), NodeProperties::GetType(replacement), graph()->zone());
                        replacement = effect = graph()->NewNode(common()->TypeGuard(replacement_type), replacement, effect, control);
            ...
            FieldInfo info(node, representation, access.name, access.const_field_info);
...
Reduction LoadElimination::ReduceStoreField(Node* node, FieldAccess const& access)
{
    ...
    if (access.offset == HeapObject::kMapOffset && access.base_is_tagged == kTaggedBase) {
        ...
        if (new_value_type.IsHeapConstant()) {
            ...
            ZoneRefSet<Map> object_maps(new_value_type.AsHeapConstant()->Ref().AsMap());
            state = state->SetMaps(object, object_maps, zone());
        }
    } else {
        IndexRange field_index = FieldIndexOf(access);
        if (field_index != IndexRange::Invalid()) {
            ...
            if (lookup_result && (!is_const_store || V8_ENABLE_DOUBLE_CONST_STORE_CHECK_BOOL)) {
                ...
                bool incompatible_representation = !lookup_result->name.is_null() && !IsCompatible(representation, lookup_result->representation);
                bool illegal_double_const_store = is_const_store && !access.is_store_in_literal;
            ...
            FieldInfo new_info(new_value, representation, access.name, access.const_field_info);
...
Reduction LoadElimination::ReduceLoadElement(Node* node)
{
    ...
    case MachineRepresentation::kMapWord:
        if (Node* replacement = state->LookupElement(object, index, access.machine_type.representation())) {
        ...
        state = state->AddElement(object, index, node, access.machine_type.representation(), zone());
        return UpdateState(node, state);
    }
    return NoChange();
}
...
Reduction LoadElimination::ReduceStoreElement(Node* node)
{
    ...
    case MachineRepresentation::kTagged:
    case MachineRepresentation::kMapWord:
        state = state->AddElement(object, index, new_value, access.machine_type.representation(), zone());
        break;
    }
    return UpdateState(node, state);
}
...
LoadElimination::AbstractState const* LoadElimination::UpdateStateForPhi(AbstractState const* state, Node* effect_phi, Node* phi)
{
...
LoadElimination::AbstractState const* LoadElimination::ComputeLoopStateForStoreField(
    Node* current, LoadElimination::AbstractState const* state, FieldAccess const& access) const
{
...
LoadElimination::AbstractState const* LoadElimination::ComputeLoopState(Node* node, AbstractState const* state) const
{
    Node* const control = NodeProperties::GetControlInput(node);
        ...
        if (visited.find(current) == visited.end()) {
            ...
            if (!current->op()->HasProperty(Operator::kNoWrite)) {
                ...
                case IrOpcode::kTransitionElementsKind: {
                    ...
                    Node* const object = NodeProperties::GetValueInput(current, 0);
                    ZoneRefSet<Map> object_maps;
                    if (!state->LookupMaps(object, &object_maps) || !ZoneRefSet<Map>(transition.target()).contains(object_maps)) {
...
// static
LoadElimination::IndexRange LoadElimination::FieldIndexOf(int offset, int representation_size)
{
...
// static
LoadElimination::IndexRange LoadElimination::FieldIndexOf(FieldAccess const& access)
{
    ...
    // configurations.
    DCHECK(rep != MachineRepresentation::kIndirectPointer || representation_size == kTaggedSize);
