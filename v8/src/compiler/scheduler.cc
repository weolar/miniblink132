...
Scheduler::Scheduler(
    Zone* zone, Graph* graph, Schedule* schedule, Flags flags, size_t node_count_hint, TickCounter* tick_counter, const ProfileDataFromFile* profile_data)
    : zone_(zone)
    , graph_(graph)
    , schedule_(schedule)
    , flags_(flags)
...
Schedule* Scheduler::ComputeSchedule(Zone* zone, Graph* graph, Flags flags, TickCounter* tick_counter, const ProfileDataFromFile* profile_data)
{
    ...
    Schedule* schedule = schedule_zone->New<Schedule>(schedule_zone, node_count_hint);
    Scheduler scheduler(zone, graph, schedule, flags, node_count_hint, tick_counter, profile_data);
...
void Scheduler::IncrementUnscheduledUseCount(Node* node, Node* from)
{
    ...
    if (v8_flags.trace_turbo_scheduler) {
        TRACE("  Use count of #%d:%s (used by #%d:%s)++ = %d\n", node->id(), node->op()->mnemonic(), from->id(), from->op()->mnemonic(),
            GetData(node)->unscheduled_count_);
    }
}
...
#undef CONNECT_BLOCK_JS_CASE
    ...
    BasicBlock* BuildBlockForNode(Node* node)
    {
        ...
        if (block == nullptr) {
            ...
            TRACE("Create block id:%d for #%d:%s\n", block->id().ToInt(), node->id(), node->op()->mnemonic());
            FixNode(block, node);
        }
        return block;
    }
    ...
    void ConnectCall(Node* call)
    {
        ...
        schedule_->AddCall(call_block, call, successor_blocks[0], successor_blocks[1]);
    }

    void ConnectBranch(Node* branch)
    {
        BasicBlock* successor_blocks[2];
        CollectSuccessorBlocks(branch, successor_blocks, arraysize(successor_blocks));

        BranchHint hint_from_profile = BranchHint::kNone;
        if (const ProfileDataFromFile* profile_data = scheduler_->profile_data()) {
            hint_from_profile = profile_data->GetHint(successor_blocks[0]->id().ToSize(), successor_blocks[1]->id().ToSize());
        }
        ...
        if (branch == component_entry_) {
            ...
            schedule_->InsertBranch(component_start_, component_end_, branch, successor_blocks[0], successor_blocks[1]);
        } else {
            ...
            schedule_->AddBranch(branch_block, branch, successor_blocks[0], successor_blocks[1]);
        }
    }
    ...
    void ConnectSwitch(Node* sw)
    {
        ...
        BasicBlock** successor_blocks = zone_->AllocateArray<BasicBlock*>(successor_count);
        ...
        if (sw == component_entry_) {
            ...
            schedule_->InsertSwitch(component_start_, component_end_, sw, successor_blocks, successor_count);
        } else {
    ...
    void TraceConnect(Node* node, BasicBlock* block, BasicBlock* succ)
    {
        ...
        if (succ == nullptr) {
            ...
            TRACE("Connect #%d:%s, id:%d -> id:%d\n", node->id(), node->op()->mnemonic(), block->id().ToInt(), succ->id().ToInt());
        }
    }
...
private:
    ...
    void ComputeAndInsertSpecialRPO(BasicBlock* entry, BasicBlock* end)
    {
        ...
        for (BasicBlock* b = order; b != insertion_point; b = b->rpo_next()) {
            ...
            if (HasLoopNumber(current)) {
                ...
                current->set_loop_end(loop_end == nullptr ? BeyondEndSentinel() : loop_end);
            ...
            if (current->loop_header() == nullptr) {
                TRACE("id:%d is not in a loop (depth == %d)\n", current->id().ToInt(), current->loop_depth());
            } else {
                TRACE("id:%d has loop header id:%d, (depth == %d)\n", current->id().ToInt(), current->loop_header()->id().ToInt(), current->loop_depth());
            }
        }
    }
    ...
    void ComputeLoopInfo(ZoneVector<SpecialRPOStackFrame>* queue, size_t num_loops, ZoneVector<Backedge>* backedges)
    {
        ...
        for (LoopInfo& loop : loops_) {
            loop.members->Resize(static_cast<int>(schedule_->BasicBlockCount()), zone_);
        }
        ...
        for (size_t i = 0; i < backedges->size(); i++) {
            ...
            if (loops_[loop_num].header == nullptr) {
                ...
                loops_[loop_num].members = zone_->New<BitVector>(static_cast<int>(schedule_->BasicBlockCount()), zone_);
            }
    ...
    void PrintRPO()
    {
        ...
        for (BasicBlock* block = order_; block != nullptr; block = block->rpo_next()) {
            ...
            if (block->loop_end() != nullptr) {
                os << " range: [B" << block->rpo_number() << ", B" << block->loop_end()->rpo_number() << ")";
            }
...
BasicBlock* Scheduler::GetCommonDominatorIfCached(BasicBlock* b1, BasicBlock* b2)
{
    auto entry1 = common_dominator_cache_.find(b1->id().ToInt());
    if (entry1 == common_dominator_cache_.end())
        return nullptr;
    auto entry2 = entry1->second->find(b2->id().ToInt());
    if (entry2 == entry1->second->end())
        return nullptr;
...
BasicBlock* Scheduler::GetCommonDominator(BasicBlock* b1, BasicBlock* b2)
{
    ...
    if (depth_difference > -kCacheGranularity && depth_difference < kCacheGranularity) {
    ...
    for (/*nop*/; block != nullptr; block = block->rpo_next()) {
        ...
        for (++pred; pred != end; ++pred) {
            ...
            if ((*pred)->dominator_depth() < 0)
                continue;
            if ((*pred)->dominator_depth() > 3 && ((*pred)->dominator()->dominator() == cache || (*pred)->dominator()->dominator()->dominator() == cache)) {
        ...
        TRACE("Block id:%d's idom is id:%d, depth = %d\n", block->id().ToInt(), dominator->id().ToInt(), block->dominator_depth());
    }
}
...
private:
    void InitializePlacement(Node* node)
    {
        ...
        if (scheduler_->InitializePlacement(node) == Scheduler::kFixed) {
            ...
            if (!schedule_->IsScheduled(node)) {
                ...
                BasicBlock* block = opcode == IrOpcode::kParameter ? schedule_->start() : schedule_->block(NodeProperties::GetControlInput(node));
                DCHECK_NOT_NULL(block);
                schedule_->AddNode(block, node);
            }
        }
    ...
    void VisitInputs(Node* node)
    {
        ...
        std::optional<int> coupled_control_edge = scheduler_->GetCoupledControlEdge(node);
        for (auto edge : node->input_edges()) {
            ...
            TRACE("PostEdge #%d:%s->#%d:%s\n", node->id(), node->op()->mnemonic(), to->id(), to->op()->mnemonic());
    ...
    {
        return visited_.Contains(node->id());
    }
...
private:
    ...
    void VisitNode(Node* node)
    {
        ...
        if (scheduler_->GetPlacement(node) == Scheduler::kFixed) {
            ...
            TRACE("Fixing #%d:%s minimum_block = id:%d, dominator_depth = %d\n", node->id(), node->op()->mnemonic(), data->minimum_block_->id().ToInt(),
...
private:
    ...
    void VisitNode(Node* node)
    {
        ...
        TRACE("Schedule late of #%d:%s is id:%d at loop depth %d, minimum = id:%d\n", node->id(), node->op()->mnemonic(), block->id().ToInt(),
        ...
        if (hoist_block && hoist_block->dominator_depth() >= min_block->dominator_depth()) {
            ...
            do {
                TRACE("  hoisting #%d:%s to block id:%d\n", node->id(), node->op()->mnemonic(), hoist_block->id().ToInt());
        ...
        for (Edge edge : node->use_edges()) {
            ...
            if (use_node == nullptr) {
                if (dominators.size() == 1u) {
                    ...
                    TRACE("  cloning #%d:%s for id:%d\n", use_node->id(), use_node->op()->mnemonic(), use_block->id().ToInt());
    ...
    {
        if (!scheduler_->special_rpo_->HasLoopBlocks())
            return nullptr;
        ...
        if (BasicBlock* header_block = block->loop_header()) {
            for (BasicBlock* outgoing_block : scheduler_->special_rpo_->GetOutgoingBlocks(header_block)) {
    ...
    BasicBlock* GetCommonDominatorOfUses(Node* node)
    {
        ...
        for (Edge edge : node->use_edges()) {
            if (!scheduler_->IsLive(edge.from()))
                continue;
            BasicBlock* use_block = GetBlockForUse(edge);
            block = block == nullptr ? use_block : use_block == nullptr ? block : scheduler_->GetCommonDominator(block, use_block);
        }
        return block;
    }
    ...
    BasicBlock* GetBlockForUse(Edge edge)
    {
        ...
        if (IrOpcode::IsPhiOpcode(use->opcode())) {
            ...
            if (scheduler_->GetPlacement(use) == Scheduler::kFixed) {
                TRACE("  input@%d into a fixed phi #%d:%s\n", edge.index(), use->id(), use->op()->mnemonic());
        ...
        if (result == nullptr)
            return nullptr;
        TRACE("  must dominate use #%d:%s in id:%d\n", use->id(), use->op()->mnemonic(), result->id().ToInt());
        return result;
    }
    ...
    Node* CloneNode(Node* node)
    {
        ...
        TRACE(("clone #%d:%s -> #%d\n"), node->id(), node->op()->mnemonic(), copy->id());
        scheduler_->node_data_.resize(copy->id() + 1, scheduler_->DefaultSchedulerData());
...
void Scheduler::SealFinalSchedule()
{
    ...
    if (const ProfileDataFromFile* profile_data = this->profile_data()) {
        for (BasicBlock* block : *schedule_->all_blocks()) {
            uint64_t executed_count = profile_data->GetExecutedCount(block->id().ToSize());
...
void Scheduler::MovePlannedNodes(BasicBlock* from, BasicBlock* to)
{
    ...
    if (to_nodes) {
        ...
        std::swap(scheduled_nodes_[from->id().ToSize()], scheduled_nodes_[to->id().ToSize()]);
    }
}
