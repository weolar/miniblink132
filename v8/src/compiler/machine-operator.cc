...
{
    return lhs.store_representation() == rhs.store_representation() && lhs.order() == rhs.order() && lhs.kind() == rhs.kind();
}
...
{
    return base::hash_combine(hash_value(params.store_representation()), params.order(), params.kind());
}
...
{
    return lhs.representation() == rhs.representation() && lhs.order() == rhs.order() && lhs.kind() == rhs.kind();
}
...
{
    return base::hash_combine(params.representation(), params.order(), params.kind());
}
...
size_t hash_value(MemoryAccessKind kind)
{
...
{
    return os << "(" << params.kind << " " << params.rep << " " << static_cast<uint32_t>(params.laneidx) << ")";
}
...
{
    return lhs.kind == rhs.kind && lhs.rep == rhs.rep && lhs.laneidx == rhs.laneidx;
}
...
{
    return os << "(" << params.kind << " " << params.rep << " " << static_cast<unsigned int>(params.laneidx) << ")";
}
...
{
    DCHECK(IrOpcode::kLoad == op->opcode() || IrOpcode::kProtectedLoad == op->opcode() || IrOpcode::kLoadTrapOnNull == op->opcode()
        || IrOpcode::kUnalignedLoad == op->opcode() || IrOpcode::kLoadImmutable == op->opcode());
    return OpParameter<LoadRepresentation>(op);
}

AtomicLoadParameters AtomicLoadParametersOf(Operator const* op)
{
    DCHECK(IrOpcode::kWord32AtomicLoad == op->opcode() || IrOpcode::kWord64AtomicLoad == op->opcode());
...
{
    DCHECK(IrOpcode::kStore == op->opcode() || IrOpcode::kProtectedStore == op->opcode() || IrOpcode::kStoreTrapOnNull == op->opcode()
...
{
    DCHECK(IrOpcode::kWord32Sar == op->opcode() || IrOpcode::kWord64Sar == op->opcode());
...
#define PURE_SIMD_OP_LIST(V)                                                                                                                                   \
    ...
    IF_WASM(V, F64x2Min, Operator::kAssociative | Operator::kCommutative, 2, 0, 1)                                                                             \
    IF_WASM(V, F64x2Max, Operator::kAssociative | Operator::kCommutative, 2, 0, 1)                                                                             \
    ...
    IF_WASM(V, F32x4Min, Operator::kAssociative | Operator::kCommutative, 2, 0, 1)                                                                             \
    IF_WASM(V, F32x4Max, Operator::kAssociative | Operator::kCommutative, 2, 0, 1)                                                                             \
    ...
    IF_WASM(V, F16x8Min, Operator::kAssociative | Operator::kCommutative, 2, 0, 1)                                                                             \
    IF_WASM(V, F16x8Max, Operator::kAssociative | Operator::kCommutative, 2, 0, 1)                                                                             \
    ...
    IF_WASM(V, S128And, Operator::kAssociative | Operator::kCommutative, 2, 0, 1)                                                                              \
    IF_WASM(V, S128Or, Operator::kAssociative | Operator::kCommutative, 2, 0, 1)                                                                               \
    IF_WASM(V, S128Xor, Operator::kAssociative | Operator::kCommutative, 2, 0, 1)                                                                              \
    ...
    IF_WASM(V, F64x4Min, Operator::kAssociative | Operator::kCommutative, 2, 0, 1)                                                                             \
    IF_WASM(V, F64x4Max, Operator::kAssociative | Operator::kCommutative, 2, 0, 1)                                                                             \
    ...
    IF_WASM(V, F32x8Min, Operator::kAssociative | Operator::kCommutative, 2, 0, 1)                                                                             \
    IF_WASM(V, F32x8Max, Operator::kAssociative | Operator::kCommutative, 2, 0, 1)                                                                             \
    ...
    IF_WASM(V, S256And, Operator::kAssociative | Operator::kCommutative, 2, 0, 1)                                                                              \
    IF_WASM(V, S256Or, Operator::kAssociative | Operator::kCommutative, 2, 0, 1)                                                                               \
    IF_WASM(V, S256Xor, Operator::kAssociative | Operator::kCommutative, 2, 0, 1)                                                                              \
...
#define SIMD_I8x16_LANES(V) SIMD_I16x8_LANES(V) V(8) V(9) V(10) V(11) V(12) V(13) V(14) V(15)
...
struct StackSlotOperator : public Operator1<StackSlotRepresentation> {
    explicit StackSlotOperator(int size, int alignment, bool is_tagged)
        : Operator1<StackSlotRepresentation>(
            IrOpcode::kStackSlot, Operator::kNoDeopt | Operator::kNoThrow, "StackSlot", 0, 0, 0, 1, 0, 0, StackSlotRepresentation(size, alignment, is_tagged))
    {
    }
};
...
#define PURE(Name, properties, value_input_count, control_input_count, output_count)                                                                           \
    struct Name##Operator final : public Operator {                                                                                                            \
        Name##Operator()                                                                                                                                       \
            : Operator(IrOpcode::k##Name, Operator::kPure | properties, #Name, value_input_count, 0, control_input_count, output_count, 0, 0)                  \
        {                                                                                                                                                      \
        }                                                                                                                                                      \
    };                                                                                                                                                         \
    ...
    struct NormalWord32SarOperator final : public Operator1<ShiftKind> {
        NormalWord32SarOperator()
            : Operator1<ShiftKind>(IrOpcode::kWord32Sar, Operator::kPure, "Word32Sar", 2, 0, 0, 1, 0, 0, ShiftKind::kNormal)
        {
        }
    };
    ...
    struct ShiftOutZerosWord32SarOperator final : public Operator1<ShiftKind> {
        ShiftOutZerosWord32SarOperator()
            : Operator1<ShiftKind>(IrOpcode::kWord32Sar, Operator::kPure, "Word32Sar", 2, 0, 0, 1, 0, 0, ShiftKind::kShiftOutZeros)
        {
        }
    };
    ...
    struct NormalWord64SarOperator final : public Operator1<ShiftKind> {
        NormalWord64SarOperator()
            : Operator1<ShiftKind>(IrOpcode::kWord64Sar, Operator::kPure, "Word64Sar", 2, 0, 0, 1, 0, 0, ShiftKind::kNormal)
        {
        }
    };
    ...
    struct ShiftOutZerosWord64SarOperator final : public Operator1<ShiftKind> {
        ShiftOutZerosWord64SarOperator()
            : Operator1<ShiftKind>(IrOpcode::kWord64Sar, Operator::kPure, "Word64Sar", 2, 0, 0, 1, 0, 0, ShiftKind::kShiftOutZeros)
        {
        }
    };
    ShiftOutZerosWord64SarOperator kShiftOutZerosWord64Sar;

    struct ArchitectureDefaultTruncateFloat32ToUint32Operator final : public Operator1<TruncateKind> {
        ArchitectureDefaultTruncateFloat32ToUint32Operator()
            : Operator1<TruncateKind>(
                IrOpcode::kTruncateFloat32ToUint32, Operator::kPure, "TruncateFloat32ToUint32", 1, 0, 0, 1, 0, 0, TruncateKind::kArchitectureDefault)
        {
        }
    };
    ArchitectureDefaultTruncateFloat32ToUint32Operator kArchitectureDefaultTruncateFloat32ToUint32;
    struct SetOverflowToMinTruncateFloat32ToUint32Operator final : public Operator1<TruncateKind> {
        SetOverflowToMinTruncateFloat32ToUint32Operator()
            : Operator1<TruncateKind>(
                IrOpcode::kTruncateFloat32ToUint32, Operator::kPure, "TruncateFloat32ToUint32", 1, 0, 0, 1, 0, 0, TruncateKind::kSetOverflowToMin)
        {
        }
    };
    SetOverflowToMinTruncateFloat32ToUint32Operator kSetOverflowToMinTruncateFloat32ToUint32;

    struct ArchitectureDefaultTruncateFloat32ToInt32Operator final : public Operator1<TruncateKind> {
        ArchitectureDefaultTruncateFloat32ToInt32Operator()
            : Operator1<TruncateKind>(
                IrOpcode::kTruncateFloat32ToInt32, Operator::kPure, "TruncateFloat32ToInt32", 1, 0, 0, 1, 0, 0, TruncateKind::kArchitectureDefault)
        {
        }
    };
    ArchitectureDefaultTruncateFloat32ToInt32Operator kArchitectureDefaultTruncateFloat32ToInt32;
    struct SetOverflowToMinTruncateFloat32ToInt32Operator final : public Operator1<TruncateKind> {
        SetOverflowToMinTruncateFloat32ToInt32Operator()
            : Operator1<TruncateKind>(
                IrOpcode::kTruncateFloat32ToInt32, Operator::kPure, "TruncateFloat32ToInt32", 1, 0, 0, 1, 0, 0, TruncateKind::kSetOverflowToMin)
        {
        }
    };
    SetOverflowToMinTruncateFloat32ToInt32Operator kSetOverflowToMinTruncateFloat32ToInt32;

    struct ArchitectureDefaultTruncateFloat64ToInt64Operator final : public Operator1<TruncateKind> {
        ArchitectureDefaultTruncateFloat64ToInt64Operator()
            : Operator1(IrOpcode::kTruncateFloat64ToInt64, Operator::kPure, "TruncateFloat64ToInt64", 1, 0, 0, 1, 0, 0, TruncateKind::kArchitectureDefault)
        {
        }
    };
    ArchitectureDefaultTruncateFloat64ToInt64Operator kArchitectureDefaultTruncateFloat64ToInt64;
    struct SetOverflowToMinTruncateFloat64ToInt64Operator final : public Operator1<TruncateKind> {
        SetOverflowToMinTruncateFloat64ToInt64Operator()
            : Operator1(IrOpcode::kTruncateFloat64ToInt64, Operator::kPure, "TruncateFloat64ToInt64", 1, 0, 0, 1, 0, 0, TruncateKind::kSetOverflowToMin)
        {
        }
    };
...
#define OVERFLOW_OP(Name, properties)                                                                                                                          \
    struct Name##Operator final : public Operator {                                                                                                            \
        Name##Operator()                                                                                                                                       \
            : Operator(IrOpcode::k##Name, Operator::kEliminatable | Operator::kNoRead | properties, #Name, 2, 0, 1, 2, 0, 0)                                   \
        {                                                                                                                                                      \
        }                                                                                                                                                      \
    };                                                                                                                                                         \
    Name##Operator k##Name;
...
#define LOAD(Type)                                                                                                                                             \
    struct Load##Type##Operator final : public Operator1<LoadRepresentation> {                                                                                 \
        Load##Type##Operator()                                                                                                                                 \
            : Operator1<LoadRepresentation>(IrOpcode::kLoad, Operator::kEliminatable, "Load", 2, 1, 1, 1, 1, 0, MachineType::Type())                           \
        {                                                                                                                                                      \
        }                                                                                                                                                      \
    };                                                                                                                                                         \
    struct UnalignedLoad##Type##Operator final : public Operator1<LoadRepresentation> {                                                                        \
        UnalignedLoad##Type##Operator()                                                                                                                        \
            : Operator1<LoadRepresentation>(IrOpcode::kUnalignedLoad, Operator::kEliminatable, "UnalignedLoad", 2, 1, 1, 1, 1, 0, MachineType::Type())         \
        {                                                                                                                                                      \
        }                                                                                                                                                      \
    };                                                                                                                                                         \
    struct ProtectedLoad##Type##Operator final : public Operator1<LoadRepresentation> {                                                                        \
        ProtectedLoad##Type##Operator()                                                                                                                        \
            : Operator1<LoadRepresentation>(                                                                                                                   \
                IrOpcode::kProtectedLoad, Operator::kNoDeopt | Operator::kNoThrow, "ProtectedLoad", 2, 1, 1, 1, 1, 0, MachineType::Type())                     \
        {                                                                                                                                                      \
        }                                                                                                                                                      \
    };                                                                                                                                                         \
    struct LoadTrapOnNull##Type##Operator final : public Operator1<LoadRepresentation> {                                                                       \
        LoadTrapOnNull##Type##Operator()                                                                                                                       \
            : Operator1<LoadRepresentation>(                                                                                                                   \
                IrOpcode::kLoadTrapOnNull, Operator::kNoDeopt | Operator::kNoThrow, "LoadTrapOnNull", 2, 1, 1, 1, 1, 0, MachineType::Type())                   \
        {                                                                                                                                                      \
        }                                                                                                                                                      \
    };                                                                                                                                                         \
    struct LoadImmutable##Type##Operator final : public Operator1<LoadRepresentation> {                                                                        \
        LoadImmutable##Type##Operator()                                                                                                                        \
            : Operator1<LoadRepresentation>(IrOpcode::kLoadImmutable, Operator::kPure, "LoadImmutable", 2, 0, 0, 1, 0, 0, MachineType::Type())                 \
        {                                                                                                                                                      \
        }                                                                                                                                                      \
    };                                                                                                                                                         \
...
#define LOAD_TRANSFORM_KIND(TYPE, KIND)                                                                                                                        \
    struct KIND##LoadTransform##TYPE##Operator final : public Operator1<LoadTransformParameters> {                                                             \
        KIND##LoadTransform##TYPE##Operator()                                                                                                                  \
            : Operator1<LoadTransformParameters>(IrOpcode::kLoadTransform,                                                                                     \
                MemoryAccessKind::k##KIND == MemoryAccessKind::kProtected ? Operator::kNoDeopt | Operator::kNoThrow : Operator::kEliminatable,                 \
                #KIND "LoadTransform", 2, 1, 1, 1, 1, 0, LoadTransformParameters { MemoryAccessKind::k##KIND, LoadTransformation::k##TYPE })                   \
        {                                                                                                                                                      \
        }                                                                                                                                                      \
    };                                                                                                                                                         \
...
#define STACKSLOT(Size, Alignment, IsTagged)                                                                                                                   \
    struct StackSlotOfSize##Size##OfAlignment##Alignment##IsTagged##Operator final : public StackSlotOperator {                                                \
        StackSlotOfSize##Size##OfAlignment##Alignment##IsTagged##Operator()                                                                                    \
            : StackSlotOperator(Size, Alignment, IsTagged)                                                                                                     \
        {                                                                                                                                                      \
        }                                                                                                                                                      \
    };                                                                                                                                                         \
    StackSlotOfSize##Size##OfAlignment##Alignment##IsTagged##Operator kStackSlotOfSize##Size##OfAlignment##Alignment##IsTagged;
...
#define STORE(Type)                                                                                                                                            \
    struct Store##Type##Operator : public Operator1<StoreRepresentation> {                                                                                     \
        explicit Store##Type##Operator(WriteBarrierKind write_barrier_kind)                                                                                    \
            : Operator1<StoreRepresentation>(IrOpcode::kStore, Operator::kNoDeopt | Operator::kNoRead | Operator::kNoThrow, "Store", 3, 1, 1, 0, 1, 0,         \
                StoreRepresentation(MachineRepresentation::Type, write_barrier_kind))                                                                          \
        {                                                                                                                                                      \
        }                                                                                                                                                      \
    };                                                                                                                                                         \
    struct Store##Type##NoWriteBarrier##Operator final : public Store##Type##Operator {                                                                        \
        Store##Type##NoWriteBarrier##Operator()                                                                                                                \
            : Store##Type##Operator(kNoWriteBarrier)                                                                                                           \
        {                                                                                                                                                      \
        }                                                                                                                                                      \
    };                                                                                                                                                         \
    struct Store##Type##AssertNoWriteBarrier##Operator final : public Store##Type##Operator {                                                                  \
        Store##Type##AssertNoWriteBarrier##Operator()                                                                                                          \
            : Store##Type##Operator(kAssertNoWriteBarrier)                                                                                                     \
        {                                                                                                                                                      \
        }                                                                                                                                                      \
    };                                                                                                                                                         \
    struct Store##Type##MapWriteBarrier##Operator final : public Store##Type##Operator {                                                                       \
        Store##Type##MapWriteBarrier##Operator()                                                                                                               \
            : Store##Type##Operator(kMapWriteBarrier)                                                                                                          \
        {                                                                                                                                                      \
        }                                                                                                                                                      \
    };                                                                                                                                                         \
    struct Store##Type##PointerWriteBarrier##Operator final : public Store##Type##Operator {                                                                   \
        Store##Type##PointerWriteBarrier##Operator()                                                                                                           \
            : Store##Type##Operator(kPointerWriteBarrier)                                                                                                      \
        {                                                                                                                                                      \
        }                                                                                                                                                      \
    };                                                                                                                                                         \
    struct Store##Type##EphemeronKeyWriteBarrier##Operator final : public Store##Type##Operator {                                                              \
        Store##Type##EphemeronKeyWriteBarrier##Operator()                                                                                                      \
            : Store##Type##Operator(kEphemeronKeyWriteBarrier)                                                                                                 \
        {                                                                                                                                                      \
        }                                                                                                                                                      \
    };                                                                                                                                                         \
    struct Store##Type##FullWriteBarrier##Operator final : public Store##Type##Operator {                                                                      \
        Store##Type##FullWriteBarrier##Operator()                                                                                                              \
            : Store##Type##Operator(kFullWriteBarrier)                                                                                                         \
        {                                                                                                                                                      \
        }                                                                                                                                                      \
    };                                                                                                                                                         \
    struct UnalignedStore##Type##Operator final : public Operator1<UnalignedStoreRepresentation> {                                                             \
        UnalignedStore##Type##Operator()                                                                                                                       \
            : Operator1<UnalignedStoreRepresentation>(IrOpcode::kUnalignedStore, Operator::kNoDeopt | Operator::kNoRead | Operator::kNoThrow,                  \
                "UnalignedStore", 3, 1, 1, 0, 1, 0, MachineRepresentation::Type)                                                                               \
        {                                                                                                                                                      \
        }                                                                                                                                                      \
    };                                                                                                                                                         \
    struct ProtectedStore##Type##Operator : public Operator1<StoreRepresentation> {                                                                            \
        explicit ProtectedStore##Type##Operator()                                                                                                              \
            : Operator1<StoreRepresentation>(IrOpcode::kProtectedStore, Operator::kNoDeopt | Operator::kNoRead | Operator::kNoThrow, "ProtectedStore", 3, 1,   \
                1, 0, 1, 0, StoreRepresentation(MachineRepresentation::Type, kNoWriteBarrier))                                                                 \
        {                                                                                                                                                      \
        }                                                                                                                                                      \
    };                                                                                                                                                         \
    struct StoreTrapOnNull##Type##FullWriteBarrier##Operator : public Operator1<StoreRepresentation> {                                                         \
        explicit StoreTrapOnNull##Type##FullWriteBarrier##Operator()                                                                                           \
            : Operator1<StoreRepresentation>(IrOpcode::kStoreTrapOnNull, Operator::kNoDeopt | Operator::kNoRead | Operator::kNoThrow, "StoreTrapOnNull", 3, 1, \
                1, 0, 1, 0, StoreRepresentation(MachineRepresentation::Type, kFullWriteBarrier))                                                               \
        {                                                                                                                                                      \
        }                                                                                                                                                      \
    };                                                                                                                                                         \
    struct StoreTrapOnNull##Type##NoWriteBarrier##Operator : public Operator1<StoreRepresentation> {                                                           \
    ...
    Store##Type##AssertNoWriteBarrier##Operator kStore##Type##AssertNoWriteBarrier;                                                                            \
    Store##Type##MapWriteBarrier##Operator kStore##Type##MapWriteBarrier;                                                                                      \
    Store##Type##PointerWriteBarrier##Operator kStore##Type##PointerWriteBarrier;                                                                              \
    Store##Type##EphemeronKeyWriteBarrier##Operator kStore##Type##EphemeronKeyWriteBarrier;                                                                    \
    Store##Type##FullWriteBarrier##Operator kStore##Type##FullWriteBarrier;                                                                                    \
    UnalignedStore##Type##Operator kUnalignedStore##Type;                                                                                                      \
    ProtectedStore##Type##Operator kProtectedStore##Type;                                                                                                      \
    StoreTrapOnNull##Type##FullWriteBarrier##Operator kStoreTrapOnNull##Type##FullWriteBarrier;                                                                \
    StoreTrapOnNull##Type##NoWriteBarrier##Operator kStoreTrapOnNull##Type##NoWriteBarrier;
...
#define STORE_PAIR(Type1, Type2)                                                                                                                               \
    struct StorePair##Type1##Type2##Operator : public Operator1<StorePairRepresentation> {                                                                     \
        explicit StorePair##Type1##Type2##Operator(WriteBarrierKind write_barrier_kind1, WriteBarrierKind write_barrier_kind2)                                 \
            : Operator1<StorePairRepresentation>(IrOpcode::kStorePair, Operator::kNoDeopt | Operator::kNoRead | Operator::kNoThrow, "StorePair", 4, 1, 1, 0,   \
                1, 0,                                                                                                                                          \
                {                                                                                                                                              \
                    StoreRepresentation(MachineRepresentation::Type1, write_barrier_kind1),                                                                    \
                    StoreRepresentation(MachineRepresentation::Type2, write_barrier_kind2),                                                                    \
                })                                                                                                                                             \
        {                                                                                                                                                      \
        }                                                                                                                                                      \
    };                                                                                                                                                         \
    struct StorePair##Type1##Type2##NoWriteBarrier##Operator final : public StorePair##Type1##Type2##Operator {                                                \
        StorePair##Type1##Type2##NoWriteBarrier##Operator()                                                                                                    \
            : StorePair##Type1##Type2##Operator(kNoWriteBarrier, kNoWriteBarrier)                                                                              \
        {                                                                                                                                                      \
        }                                                                                                                                                      \
    };                                                                                                                                                         \
    StorePair##Type1##Type2##NoWriteBarrier##Operator kStorePair##Type1##Type2##NoWriteBarrier;
...
#undef STORE_PAIR
    ...
    struct StoreIndirectPointerOperator : public Operator1<StoreRepresentation> {
        explicit StoreIndirectPointerOperator(WriteBarrierKind write_barrier_kind)
            : Operator1<StoreRepresentation>(IrOpcode::kStoreIndirectPointer, Operator::kNoDeopt | Operator::kNoRead | Operator::kNoThrow,
                "StoreIndirectPointer", 4, 1, 1, 0, 1, 0, StoreRepresentation(MachineRepresentation::kIndirectPointer, write_barrier_kind))
        {
        }
    };
    struct StoreIndirectPointerNoWriteBarrierOperator final : public StoreIndirectPointerOperator {
        StoreIndirectPointerNoWriteBarrierOperator()
            : StoreIndirectPointerOperator(kNoWriteBarrier)
        {
        }
    };
    struct StoreIndirectPointerWithIndirectPointerWriteBarrierOperator final : public StoreIndirectPointerOperator {
...
#define ATOMIC_LOAD_WITH_KIND(Type, Kind)                                                                                                                      \
    struct Word32SeqCstLoad##Type##Kind##Operator : public Operator1<AtomicLoadParameters> {                                                                   \
        Word32SeqCstLoad##Type##Kind##Operator()                                                                                                               \
            : Operator1<AtomicLoadParameters>(IrOpcode::kWord32AtomicLoad, Operator::kNoProperties, "Word32AtomicLoad", 2, 1, 1, 1, 1, 0,                      \
                AtomicLoadParameters(MachineType::Type(), AtomicMemoryOrder::kSeqCst, MemoryAccessKind::k##Kind))                                              \
        {                                                                                                                                                      \
        }                                                                                                                                                      \
    };                                                                                                                                                         \
...
#define ATOMIC_LOAD_WITH_KIND(Type, Kind)                                                                                                                      \
    struct Word64SeqCstLoad##Type##Kind##Operator : public Operator1<AtomicLoadParameters> {                                                                   \
        Word64SeqCstLoad##Type##Kind##Operator()                                                                                                               \
            : Operator1<AtomicLoadParameters>(IrOpcode::kWord64AtomicLoad, Operator::kNoProperties, "Word64AtomicLoad", 2, 1, 1, 1, 1, 0,                      \
                AtomicLoadParameters(MachineType::Type(), AtomicMemoryOrder::kSeqCst, MemoryAccessKind::k##Kind))                                              \
        {                                                                                                                                                      \
        }                                                                                                                                                      \
    };                                                                                                                                                         \
...
#define ATOMIC_STORE_WITH_KIND(Type, Kind)                                                                                                                     \
    struct Word32SeqCstStore##Type##Kind##Operator : public Operator1<AtomicStoreParameters> {                                                                 \
        Word32SeqCstStore##Type##Kind##Operator()                                                                                                              \
            : Operator1<AtomicStoreParameters>(IrOpcode::kWord32AtomicStore, Operator::kNoDeopt | Operator::kNoRead | Operator::kNoThrow, "Word32AtomicStore", \
                3, 1, 1, 0, 1, 0, AtomicStoreParameters(MachineRepresentation::Type, kNoWriteBarrier, AtomicMemoryOrder::kSeqCst, MemoryAccessKind::k##Kind))  \
        {                                                                                                                                                      \
        }                                                                                                                                                      \
    };                                                                                                                                                         \
...
#define ATOMIC_STORE_WITH_KIND(Type, Kind)                                                                                                                     \
    struct Word64SeqCstStore##Type##Kind##Operator : public Operator1<AtomicStoreParameters> {                                                                 \
        Word64SeqCstStore##Type##Kind##Operator()                                                                                                              \
            : Operator1<AtomicStoreParameters>(IrOpcode::kWord64AtomicStore, Operator::kNoDeopt | Operator::kNoRead | Operator::kNoThrow, "Word64AtomicStore", \
                3, 1, 1, 0, 1, 0, AtomicStoreParameters(MachineRepresentation::Type, kNoWriteBarrier, AtomicMemoryOrder::kSeqCst, MemoryAccessKind::k##Kind))  \
        {                                                                                                                                                      \
        }                                                                                                                                                      \
    };                                                                                                                                                         \
...
#define ATOMIC_OP(op, type, kind)                                                                                                                              \
    struct op##type##kind##Operator : public Operator1<AtomicOpParameters> {                                                                                   \
        op##type##kind##Operator()                                                                                                                             \
            : Operator1<AtomicOpParameters>(IrOpcode::k##op, Operator::kNoDeopt | Operator::kNoThrow, #op, 3, 1, 1, 1, 1, 0,                                   \
                AtomicOpParameters(MachineType::type(), MemoryAccessKind::k##kind))                                                                            \
        {                                                                                                                                                      \
        }                                                                                                                                                      \
    };                                                                                                                                                         \
...
#define ATOMIC_COMPARE_EXCHANGE_WITH_KIND(Type, Kind)                                                                                                          \
    struct Word32AtomicCompareExchange##Type##Kind##Operator : public Operator1<AtomicOpParameters> {                                                          \
        Word32AtomicCompareExchange##Type##Kind##Operator()                                                                                                    \
            : Operator1<AtomicOpParameters>(IrOpcode::kWord32AtomicCompareExchange, Operator::kNoDeopt | Operator::kNoThrow, "Word32AtomicCompareExchange", 4, \
                1, 1, 1, 1, 0, AtomicOpParameters(MachineType::Type(), MemoryAccessKind::k##Kind))                                                             \
        {                                                                                                                                                      \
        }                                                                                                                                                      \
    };                                                                                                                                                         \
    Word32AtomicCompareExchange##Type##Kind##Operator kWord32AtomicCompareExchange##Type##Kind;
...
#define ATOMIC_COMPARE_EXCHANGE_WITH_KIND(Type, Kind)                                                                                                          \
    struct Word64AtomicCompareExchange##Type##Kind##Operator : public Operator1<AtomicOpParameters> {                                                          \
        Word64AtomicCompareExchange##Type##Kind##Operator()                                                                                                    \
            : Operator1<AtomicOpParameters>(IrOpcode::kWord64AtomicCompareExchange, Operator::kNoDeopt | Operator::kNoThrow, "Word64AtomicCompareExchange", 4, \
                1, 1, 1, 1, 0, AtomicOpParameters(MachineType::Type(), MemoryAccessKind::k##Kind))                                                             \
        {                                                                                                                                                      \
        }                                                                                                                                                      \
    };                                                                                                                                                         \
    Word64AtomicCompareExchange##Type##Kind##Operator kWord64AtomicCompareExchange##Type##Kind;
#define ATOMIC_COMPARE_EXCHANGE(Type)                                                                                                                          \
    ATOMIC_COMPARE_EXCHANGE_WITH_KIND(Type, Normal)                                                                                                            \
    ATOMIC_COMPARE_EXCHANGE_WITH_KIND(Type, Protected)
...
#undef ATOMIC_COMPARE_EXCHANGE
    ...
    struct Word32SeqCstPairLoadOperator : public Operator1<AtomicMemoryOrder> {
        Word32SeqCstPairLoadOperator()
            : Operator1<AtomicMemoryOrder>(
                IrOpcode::kWord32AtomicPairLoad, Operator::kNoDeopt | Operator::kNoThrow, "Word32AtomicPairLoad", 2, 1, 1, 2, 1, 0, AtomicMemoryOrder::kSeqCst)
        {
        }
    };
    ...
    struct Word32SeqCstPairStoreOperator : public Operator1<AtomicMemoryOrder> {
        Word32SeqCstPairStoreOperator()
            : Operator1<AtomicMemoryOrder>(IrOpcode::kWord32AtomicPairStore, Operator::kNoDeopt | Operator::kNoThrow, "Word32AtomicPairStore", 4, 1, 1, 0, 1, 0,
...
#define ATOMIC_PAIR_OP(op)                                                                                                                                     \
    struct Word32AtomicPair##op##Operator : public Operator {                                                                                                  \
        Word32AtomicPair##op##Operator()                                                                                                                       \
            : Operator(IrOpcode::kWord32AtomicPair##op, Operator::kNoDeopt | Operator::kNoThrow, "Word32AtomicPair" #op, 4, 1, 1, 2, 1, 0)                     \
        {                                                                                                                                                      \
        }                                                                                                                                                      \
    };                                                                                                                                                         \
...
#undef ATOMIC_PAIR_BINOP_LIST
    ...
    struct Word32AtomicPairCompareExchangeOperator : public Operator {
        Word32AtomicPairCompareExchangeOperator()
            : Operator(IrOpcode::kWord32AtomicPairCompareExchange, Operator::kNoDeopt | Operator::kNoThrow, "Word32AtomicPairCompareExchange", 6, 1, 1, 2, 1, 0)
        {
        }
    };
    Word32AtomicPairCompareExchangeOperator kWord32AtomicPairCompareExchange;

    template <AtomicMemoryOrder order> struct MemoryBarrierOperator : public Operator1<AtomicMemoryOrder> {
        MemoryBarrierOperator()
            : Operator1<AtomicMemoryOrder>(IrOpcode::kMemoryBarrier, Operator::kNoDeopt | Operator::kNoThrow, "SeqCstMemoryBarrier", 0, 1, 1, 0, 1, 0, order)
        {
        }
    };
    ...
    struct BitcastWordToTaggedOperator : public Operator {
        BitcastWordToTaggedOperator()
            : Operator(IrOpcode::kBitcastWordToTagged, Operator::kEliminatable | Operator::kNoWrite, "BitcastWordToTagged", 1, 1, 1, 1, 1, 0)
        {
        }
    };
    ...
    struct BitcastTaggedToWordOperator : public Operator {
        BitcastTaggedToWordOperator()
            : Operator(IrOpcode::kBitcastTaggedToWord, Operator::kEliminatable | Operator::kNoWrite, "BitcastTaggedToWord", 1, 1, 1, 1, 1, 0)
        {
        }
    };
    ...
    struct BitcastMaybeObjectToWordOperator : public Operator {
        BitcastMaybeObjectToWordOperator()
            : Operator(IrOpcode::kBitcastTaggedToWord, Operator::kEliminatable | Operator::kNoWrite, "BitcastMaybeObjectToWord", 1, 1, 1, 1, 1, 0)
        {
        }
    };
    ...
    struct AbortCSADcheckOperator : public Operator {
        AbortCSADcheckOperator()
            : Operator(IrOpcode::kAbortCSADcheck, Operator::kNoThrow, "AbortCSADcheck", 1, 1, 1, 0, 1, 0)
        {
        }
    };
    ...
    struct DebugBreakOperator : public Operator {
        DebugBreakOperator()
            : Operator(IrOpcode::kDebugBreak, Operator::kNoThrow, "DebugBreak", 0, 1, 1, 0, 1, 0)
        {
        }
    };
    ...
    struct StackPointerGreaterThanOperator : public Operator1<StackCheckKind> {
        explicit StackPointerGreaterThanOperator(StackCheckKind kind)
            : Operator1<StackCheckKind>(IrOpcode::kStackPointerGreaterThan, Operator::kEliminatable, "StackPointerGreaterThan", 1, 1, 0, 1, 1, 0, kind)
        {
        }
    };
#define STACK_POINTER_GREATER_THAN(Kind)                                                                                                                       \
    struct StackPointerGreaterThan##Kind##Operator final : public StackPointerGreaterThanOperator {                                                            \
...
#undef STACK_POINTER_GREATER_THAN
    ...
    struct I8x16SwizzleOperator final : public Operator1<bool> {
        I8x16SwizzleOperator()
            : Operator1<bool>(IrOpcode::kI8x16Swizzle, Operator::kPure, "I8x16Swizzle", 2, 0, 0, 1, 0, 0, false)
        {
        }
    };
    ...
    struct I8x16RelaxedSwizzleOperator final : public Operator1<bool> {
        I8x16RelaxedSwizzleOperator()
            : Operator1<bool>(IrOpcode::kI8x16Swizzle, Operator::kPure, "I8x16RelaxedSwizzle", 2, 0, 0, 1, 0, 0, true)
        {
        }
    };
...
struct CommentOperator : public Operator1<const char*> {
    explicit CommentOperator(const char* msg)
        : Operator1<const char*>(IrOpcode::kComment, Operator::kNoThrow | Operator::kNoWrite, "Comment", 0, 1, 1, 0, 1, 0, msg)
    {
    }
};

namespace {
DEFINE_LAZY_LEAKY_OBJECT_GETTER(MachineOperatorGlobalCache, GetMachineOperatorGlobalCache)
}

MachineOperatorBuilder::MachineOperatorBuilder(Zone* zone, MachineRepresentation word, Flags flags, AlignmentRequirements alignmentRequirements)
    : zone_(zone)
...
const Operator* MachineOperatorBuilder::UnalignedStore(UnalignedStoreRepresentation rep)
{
    switch (rep) {
#define STORE(kRep)                                                                                                                                            \
...
const Operator* MachineOperatorBuilder::TruncateFloat32ToUint32(TruncateKind kind)
{
    switch (kind) {
    case TruncateKind::kArchitectureDefault:
...
const Operator* MachineOperatorBuilder::TruncateFloat64ToInt64(TruncateKind kind)
{
    switch (kind) {
    case TruncateKind::kArchitectureDefault:
...
const Operator* MachineOperatorBuilder::TruncateFloat32ToInt32(TruncateKind kind)
{
    switch (kind) {
    case TruncateKind::kArchitectureDefault:
...
{
    return zone_->New<Operator1<uint32_t>>(IrOpcode::kTraceInstruction, Operator::kNoDeopt | Operator::kNoThrow, "TraceInstruction", 0, 1, 1, 0, 1, 0, markid);
}
...
#if V8_ENABLE_WEBASSEMBLY
const Operator* MachineOperatorBuilder::LoadTransform(MemoryAccessKind kind, LoadTransformation transform)
{
#define LOAD_TRANSFORM_KIND(TYPE, KIND)                                                                                                                        \
    if (kind == MemoryAccessKind::k##KIND && transform == LoadTransformation::k##TYPE) {                                                                       \
...
const Operator* MachineOperatorBuilder::LoadLane(MemoryAccessKind kind, LoadRepresentation rep, uint8_t laneidx)
{
#define LOAD_LANE_KIND(TYPE, KIND, LANEIDX)                                                                                                                    \
    if (kind == MemoryAccessKind::k##KIND && rep == MachineType::TYPE() && laneidx == LANEIDX) {                                                               \
        return zone_->New<Operator1<LoadLaneParameters>>(IrOpcode::kLoadLane,                                                                                  \
            MemoryAccessKind::k##KIND == MemoryAccessKind::kProtected ? Operator::kNoDeopt | Operator::kNoThrow : Operator::kEliminatable, "LoadLane", 3, 1,   \
            1, 1, 1, 0, LoadLaneParameters { MemoryAccessKind::k##KIND, LoadRepresentation::TYPE(), LANEIDX });                                                \
    }
...
const Operator* MachineOperatorBuilder::StoreLane(MemoryAccessKind kind, MachineRepresentation rep, uint8_t laneidx)
{
#define STORE_LANE_KIND(REP, KIND, LANEIDX)                                                                                                                    \
    if (kind == MemoryAccessKind::k##KIND && rep == MachineRepresentation::REP && laneidx == LANEIDX) {                                                        \
        return zone_->New<Operator1<StoreLaneParameters>>(IrOpcode::kStoreLane, Operator::kNoDeopt | Operator::kNoRead | Operator::kNoThrow, "StoreLane", 3,   \
            1, 1, 0, 1, 0, StoreLaneParameters { MemoryAccessKind::k##KIND, MachineRepresentation::REP, LANEIDX });                                            \
    }
...
const Operator* MachineOperatorBuilder::StackSlot(int size, int alignment, bool is_tagged)
{
    DCHECK_LE(0, size);
...
const Operator* MachineOperatorBuilder::StackSlot(MachineRepresentation rep, int alignment)
{
...
const Operator* MachineOperatorBuilder::Store(StoreRepresentation store_rep)
{
    ...
    DCHECK_NE(store_rep.representation(), MachineRepresentation::kIndirectPointer);
...
const Operator* MachineOperatorBuilder::StoreIndirectPointer(WriteBarrierKind write_barrier_kind)
{
...
std::optional<const Operator*> MachineOperatorBuilder::TryStorePair(StoreRepresentation store_rep1, StoreRepresentation store_rep2)
{
    DCHECK_NE(store_rep1.representation(), MachineRepresentation::kMapWord);

#define STORE(kRep1, kRep2)                                                                                                                                    \
    static_assert(ElementSizeLog2Of(MachineRepresentation::kRep1) == ElementSizeLog2Of(MachineRepresentation::kRep2));                                         \
    if (MachineRepresentation::kRep1 == store_rep1.representation() && MachineRepresentation::kRep2 == store_rep2.representation()) {                          \
        if (store_rep1.write_barrier_kind() != kNoWriteBarrier || store_rep2.write_barrier_kind() != kNoWriteBarrier) {                                        \
            return {};                                                                                                                                         \
        }                                                                                                                                                      \
...
const Operator* MachineOperatorBuilder::ProtectedStore(MachineRepresentation rep)
{
    switch (rep) {
#define STORE(kRep)                                                                                                                                            \
    case MachineRepresentation::kRep:                                                                                                                          \
...
const Operator* MachineOperatorBuilder::StoreTrapOnNull(StoreRepresentation rep)
{
...
const Operator* MachineOperatorBuilder::StackPointerGreaterThan(StackCheckKind kind)
{
    switch (kind) {
...
const Operator* MachineOperatorBuilder::Word32AtomicLoad(AtomicLoadParameters params)
{
#define CACHED_LOAD_WITH_KIND(Type, Kind)                                                                                                                      \
    if (params.representation() == MachineType::Type() && params.order() == AtomicMemoryOrder::kSeqCst && params.kind() == MemoryAccessKind::k##Kind) {        \
...
#define LOAD(Type)                                                                                                                                             \
    if (params.representation() == MachineType::Type()) {                                                                                                      \
        return zone_->New<Operator1<AtomicLoadParameters>>(                                                                                                    \
            IrOpcode::kWord32AtomicLoad, Operator::kNoProperties, "Word32AtomicLoad", 2, 1, 1, 1, 1, 0, params);                                               \
    }
...
const Operator* MachineOperatorBuilder::Word32AtomicStore(AtomicStoreParameters params)
{
#define CACHED_STORE_WITH_KIND(kRep, Kind)                                                                                                                     \
    if (params.representation() == MachineRepresentation::kRep && params.order() == AtomicMemoryOrder::kSeqCst                                                 \
...
#define STORE(kRep)                                                                                                                                            \
    if (params.representation() == MachineRepresentation::kRep) {                                                                                              \
        return zone_->New<Operator1<AtomicStoreParameters>>(                                                                                                   \
            IrOpcode::kWord32AtomicStore, Operator::kNoDeopt | Operator::kNoRead | Operator::kNoThrow, "Word32AtomicStore", 3, 1, 1, 0, 1, 0, params);         \
    }
...
const Operator* MachineOperatorBuilder::Word32AtomicExchange(AtomicOpParameters params)
{
#define EXCHANGE_WITH_KIND(kType, Kind)                                                                                                                        \
    if (params.type() == MachineType::kType() && params.kind() == MemoryAccessKind::k##Kind) {                                                                 \
...
const Operator* MachineOperatorBuilder::Word32AtomicCompareExchange(AtomicOpParameters params)
{
...
const Operator* MachineOperatorBuilder::Word32AtomicAdd(AtomicOpParameters params)
{
...
const Operator* MachineOperatorBuilder::Word32AtomicSub(AtomicOpParameters params)
{
#define OP_WITH_KIND(kType, Kind)                                                                                                                              \
    if (params.type() == MachineType::kType() && params.kind() == MemoryAccessKind::k##Kind) {                                                                 \
...
const Operator* MachineOperatorBuilder::Word32AtomicAnd(AtomicOpParameters params)
{
#define OP_WITH_KIND(kType, Kind)                                                                                                                              \
    if (params.type() == MachineType::kType() && params.kind() == MemoryAccessKind::k##Kind) {                                                                 \
...
const Operator* MachineOperatorBuilder::Word32AtomicOr(AtomicOpParameters params)
{
#define OP_WITH_KIND(kType, Kind)                                                                                                                              \
    if (params.type() == MachineType::kType() && params.kind() == MemoryAccessKind::k##Kind) {                                                                 \
...
const Operator* MachineOperatorBuilder::Word32AtomicXor(AtomicOpParameters params)
{
#define OP_WITH_KIND(kType, Kind)                                                                                                                              \
    if (params.type() == MachineType::kType() && params.kind() == MemoryAccessKind::k##Kind) {                                                                 \
...
const Operator* MachineOperatorBuilder::Word64AtomicLoad(AtomicLoadParameters params)
{
#define CACHED_LOAD_WITH_KIND(Type, Kind)                                                                                                                      \
    if (params.representation() == MachineType::Type() && params.order() == AtomicMemoryOrder::kSeqCst && params.kind() == MemoryAccessKind::k##Kind) {        \
...
#define LOAD(Type)                                                                                                                                             \
    if (params.representation() == MachineType::Type()) {                                                                                                      \
        return zone_->New<Operator1<AtomicLoadParameters>>(                                                                                                    \
            IrOpcode::kWord64AtomicLoad, Operator::kNoProperties, "Word64AtomicLoad", 2, 1, 1, 1, 1, 0, params);                                               \
    }
...
const Operator* MachineOperatorBuilder::Word64AtomicStore(AtomicStoreParameters params)
{
#define CACHED_STORE_WITH_KIND(kRep, Kind)                                                                                                                     \
    if (params.representation() == MachineRepresentation::kRep && params.order() == AtomicMemoryOrder::kSeqCst                                                 \
        && params.kind() == MemoryAccessKind::k##Kind) {                                                                                                       \
...
#define STORE(kRep)                                                                                                                                            \
    if (params.representation() == MachineRepresentation::kRep) {                                                                                              \
        return zone_->New<Operator1<AtomicStoreParameters>>(                                                                                                   \
            IrOpcode::kWord64AtomicStore, Operator::kNoDeopt | Operator::kNoRead | Operator::kNoThrow, "Word64AtomicStore", 3, 1, 1, 0, 1, 0, params);         \
    }
...
const Operator* MachineOperatorBuilder::Word64AtomicAdd(AtomicOpParameters params)
{
#define OP_WITH_KIND(kType, Kind)                                                                                                                              \
    if (params.type() == MachineType::kType() && params.kind() == MemoryAccessKind::k##Kind) {                                                                 \
...
const Operator* MachineOperatorBuilder::Word64AtomicSub(AtomicOpParameters params)
{
#define OP_WITH_KIND(kType, Kind)                                                                                                                              \
    if (params.type() == MachineType::kType() && params.kind() == MemoryAccessKind::k##Kind) {                                                                 \
...
const Operator* MachineOperatorBuilder::Word64AtomicAnd(AtomicOpParameters params)
{
#define OP_WITH_KIND(kType, Kind)                                                                                                                              \
    if (params.type() == MachineType::kType() && params.kind() == MemoryAccessKind::k##Kind) {                                                                 \
...
const Operator* MachineOperatorBuilder::Word64AtomicOr(AtomicOpParameters params)
{
#define OP_WITH_KIND(kType, Kind)                                                                                                                              \
    if (params.type() == MachineType::kType() && params.kind() == MemoryAccessKind::k##Kind) {                                                                 \
...
const Operator* MachineOperatorBuilder::Word64AtomicXor(AtomicOpParameters params)
{
#define OP_WITH_KIND(kType, Kind)                                                                                                                              \
    if (params.type() == MachineType::kType() && params.kind() == MemoryAccessKind::k##Kind) {                                                                 \
...
const Operator* MachineOperatorBuilder::Word64AtomicExchange(AtomicOpParameters params)
{
#define OP_WITH_KIND(kType, Kind)                                                                                                                              \
    if (params.type() == MachineType::kType() && params.kind() == MemoryAccessKind::k##Kind) {                                                                 \
...
const Operator* MachineOperatorBuilder::Word64AtomicCompareExchange(AtomicOpParameters params)
{
#define OP_WITH_KIND(kType, Kind)                                                                                                                              \
    if (params.type() == MachineType::kType() && params.kind() == MemoryAccessKind::k##Kind) {                                                                 \
...
const Operator* MachineOperatorBuilder::Word32AtomicPairLoad(AtomicMemoryOrder order)
{
    if (order == AtomicMemoryOrder::kSeqCst) {
        return &cache_.kWord32SeqCstPairLoad;
    }
    return zone_->New<Operator1<AtomicMemoryOrder>>(
        IrOpcode::kWord32AtomicPairLoad, Operator::kNoDeopt | Operator::kNoThrow, "Word32AtomicPairLoad", 2, 1, 1, 2, 1, 0, order);
}

const Operator* MachineOperatorBuilder::Word32AtomicPairStore(AtomicMemoryOrder order)
{
    if (order == AtomicMemoryOrder::kSeqCst) {
        return &cache_.kWord32SeqCstPairStore;
    }
    return zone_->New<Operator1<AtomicMemoryOrder>>(
        IrOpcode::kWord32AtomicPairStore, Operator::kNoDeopt | Operator::kNoThrow, "Word32AtomicPairStore", 4, 1, 1, 0, 1, 0, order);
}
...
#define EXTRACT_LANE_OP(Type, Sign, lane_count)                                                                                                                \
    const Operator* MachineOperatorBuilder::Type##ExtractLane##Sign(int32_t lane_index)                                                                        \
    {                                                                                                                                                          \
        DCHECK(0 <= lane_index && lane_index < lane_count);                                                                                                    \
        return zone_->New<Operator1<int32_t>>(                                                                                                                 \
            IrOpcode::k##Type##ExtractLane##Sign, Operator::kPure, "" #Type "ExtractLane" #Sign, 1, 0, 0, 1, 0, 0, lane_index);                                \
    }
...
#define REPLACE_LANE_OP(Type, lane_count)                                                                                                                      \
    const Operator* MachineOperatorBuilder::Type##ReplaceLane(int32_t lane_index)                                                                              \
    {                                                                                                                                                          \
        DCHECK(0 <= lane_index && lane_index < lane_count);                                                                                                    \
        return zone_->New<Operator1<int32_t>>(IrOpcode::k##Type##ReplaceLane, Operator::kPure, "Replace lane", 2, 0, 0, 1, 0, 0, lane_index);                  \
    }
SIMD_LANE_OP_LIST(REPLACE_LANE_OP)
#undef REPLACE_LANE_OP

const Operator* MachineOperatorBuilder::I64x2ReplaceLaneI32Pair(int32_t lane_index)
{
    DCHECK(0 <= lane_index && lane_index < 2);
    return zone_->New<Operator1<int32_t>>(IrOpcode::kI64x2ReplaceLaneI32Pair, Operator::kPure, "Replace lane", 3, 0, 0, 1, 0, 0, lane_index);
}

S128ImmediateParameter const& S128ImmediateParameterOf(Operator const* op)
{
    DCHECK(IrOpcode::kI8x16Shuffle == op->opcode() || IrOpcode::kS128Const == op->opcode());
    return OpParameter<S128ImmediateParameter>(op);
}

S256ImmediateParameter const& S256ImmediateParameterOf(Operator const* op)
{
    DCHECK(IrOpcode::kI8x32Shuffle == op->opcode() || IrOpcode::kS256Const == op->opcode());
    return OpParameter<S256ImmediateParameter>(op);
}

const Operator* MachineOperatorBuilder::S128Const(const uint8_t value[16])
{
    return zone_->New<Operator1<S128ImmediateParameter>>(IrOpcode::kS128Const, Operator::kPure, "Immediate", 0, 0, 0, 1, 0, 0, S128ImmediateParameter(value));
}

const Operator* MachineOperatorBuilder::S256Const(const uint8_t value[32])
{
    return zone_->New<Operator1<S256ImmediateParameter>>(
        IrOpcode::kS256Const, Operator::kPure, "Immediate256", 0, 0, 0, 1, 0, 0, S256ImmediateParameter(value));
}
...
const Operator* MachineOperatorBuilder::I8x16Shuffle(const uint8_t shuffle[16])
{
    return zone_->New<Operator1<S128ImmediateParameter>>(
        IrOpcode::kI8x16Shuffle, Operator::kPure, "I8x16Shuffle", 2, 0, 0, 1, 0, 0, S128ImmediateParameter(shuffle));
}
...
const Operator* MachineOperatorBuilder::I8x32Shuffle(const uint8_t shuffle[32])
{
    return zone_->New<Operator1<S256ImmediateParameter>>(
        IrOpcode::kI8x32Shuffle, Operator::kPure, "I8x32Shuffle", 2, 0, 0, 1, 0, 0, S256ImmediateParameter(shuffle));
}
...
const Operator* MachineOperatorBuilder::ExtractF128(int32_t lane_index)
{
    ...
    public:
        explicit ExtractF128Operator(int32_t lane_index)
            : Operator1<int32_t>(IrOpcode::kExtractF128, Operator::kPure, "ExtractF128", 1, 0, 0, 1, 0, 0, lane_index)
        {
...
const Operator* MachineOperatorBuilder::LoadStackPointer()
{
    ...
    public:
        LoadStackPointerOperator()
            : Operator(IrOpcode::kLoadStackPointer, kNoProperties, "LoadStackPointer", 0, 1, 0, 1, 1, 0)
        {
        }
    };
...
const Operator* MachineOperatorBuilder::SetStackPointer()
{
    ...
    public:
        SetStackPointerOperator()
            : Operator(IrOpcode::kSetStackPointer, kNoProperties, "SetStackPointer", 1, 1, 0, 0, 1, 0)
        {
        }
    };
