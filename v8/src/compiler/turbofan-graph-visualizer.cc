...
const char* get_cached_trace_turbo_filename(OptimizedCompilationInfo* info)
{
    if (!info->trace_turbo_filename()) {
        info->set_trace_turbo_filename(GetVisualizerLogFileName(info, v8_flags.trace_turbo_path, nullptr, "json"));
    }
    return info->trace_turbo_filename();
}

TurboJsonFile::TurboJsonFile(OptimizedCompilationInfo* info, std::ios_base::openmode mode)
...
TurboCfgFile::TurboCfgFile(Isolate* isolate)
    : std::ofstream(Isolate::GetTurboCfgFileName(isolate).c_str(), std::ios_base::app)
{
}
...
void JsonPrintBytecodeSource(std::ostream& os, int source_id, std::unique_ptr<char[]> function_name, DirectHandle<BytecodeArray> bytecode_array)
{
    os << "\"" << source_id << "\" : {";
...
void JsonPrintFunctionSource(std::ostream& os, int source_id, std::unique_ptr<char[]> function_name, Handle<Script> script, Isolate* isolate,
    Handle<SharedFunctionInfo> shared, bool with_key)
{
    ...
    int start = 0;
    int end = 0;
    if (!script.is_null() && !IsUndefined(*script, isolate) && !shared.is_null()) {
        ...
        os << "\"";
        {
            ...
            if (!IsUndefined(script->source())) {
                ...
                wasm::NativeModule* native_module = function_data->instance_data()->native_module();
                const wasm::WasmModule* module = native_module->module();
                std::ostringstream str;
                wasm::DisassembleFunction(module, function_data->function_index(), native_module->wire_bytes(), native_module->GetNamesProvider(), str);
                os << JSONEscaped(str);
...
void JsonPrintInlinedFunctionInfo(std::ostream& os, int source_id, int inlining_id, const OptimizedCompilationInfo::InlinedFunctionHolder& h)
{
    os << "\"" << inlining_id << "\" : ";
...
void JsonPrintAllBytecodeSources(std::ostream& os, OptimizedCompilationInfo* info)
{
    ...
    JsonPrintBytecodeSource(os, -1, info->shared_info()->DebugNameCStr(), info->bytecode_array());
    ...
    for (unsigned id = 0; id < inlined.size(); id++) {
        ...
        JsonPrintBytecodeSource(os, source_id, shared_info->DebugNameCStr(), inlined[id].bytecode_array);
    }
...
{
    os << "\"sources\" : {";
    Handle<Script> script = (info->shared_info().is_null() || info->shared_info()->script() == Tagged<Object>())
        ? Handle<Script>()
        : handle(Cast<Script>(info->shared_info()->script()), isolate);
    JsonPrintFunctionSource(os, -1, info->shared_info().is_null() ? std::unique_ptr<char[]>(new char[1] { 0 }) : info->shared_info()->DebugNameCStr(), script,
        isolate, info->shared_info(), true);
    const auto& inlined = info->inlined_functions();
    SourceIdAssigner id_assigner(info->inlined_functions().size());
    for (unsigned id = 0; id < inlined.size(); id++) {
        os << ", ";
        ...
        JsonPrintFunctionSource(os, source_id, shared->DebugNameCStr(), handle(Cast<Script>(shared->script()), isolate), isolate, shared, true);
    }
    os << "}, ";
    os << "\"inlinings\" : {";
...
void JsonPrintAllSourceWithPositionsWasm(
    std::ostream& os, const wasm::WasmModule* module, const wasm::WireBytesStorage* wire_bytes, base::Vector<WasmInliningPosition> positions)
{
    ...
    for (WasmInliningPosition pos : positions) {
        auto [_, inserted] = source_map.emplace(pos.inlinee_func_index, sources.size());
        if (inserted) {
    ...
    for (size_t i = 0; i < sources.size(); ++i) {
        ...
        os << '"' << i << "\": {\"sourceId\": " << i << ", \"functionName\": \"" << fct.func_index << "\", \"sourceName\": \"\", \"sourceText\": \"";
        base::Vector<const uint8_t> module_bytes { nullptr, 0 };
        std::optional<wasm::ModuleWireBytes> maybe_wire_bytes = wire_bytes->GetModuleBytes();
        if (maybe_wire_bytes)
            module_bytes = maybe_wire_bytes->module_bytes();
        std::ostringstream wasm_str;
        wasm::DisassembleFunction(module, function_id, wire_bytes->GetCode(fct.code), module_bytes, fct.code.offset(), wasm_str);
    ...
    for (size_t i = 0; i < positions.size(); ++i) {
        ...
        os << '"' << i << "\": {\"inliningId\": " << i << ", \"sourceId\": " << source_id << ", \"inliningPosition\": " << AsJSON(inlining_pos) << "}";
    }
}
#endif
...
std::unique_ptr<char[]> GetVisualizerLogFileName(OptimizedCompilationInfo* info, const char* optional_base_dir, const char* phase, const char* suffix)
{
    ...
    if (strlen(debug_name.get()) > 0) {
        ...
        SNPrintF(filename, "%s-%s-%i", file_prefix, debug_name.get(), optimization_id);
    } else if (info->has_shared_info()) {
        SNPrintF(filename, "%s-%p-%i", file_prefix, reinterpret_cast<void*>(info->shared_info()->address()), optimization_id);
    } else {
        SNPrintF(filename, "%s-none-%i", file_prefix, optimization_id);
    }
    base::EmbeddedVector<char, 256> source_file(0);
    bool source_available = false;
    if (v8_flags.trace_file_names && info->has_shared_info() && IsScript(info->shared_info()->script())) {
        Tagged<Object> source_name = Cast<Script>(info->shared_info()->script())->name();
        if (IsString(source_name)) {
            Tagged<String> str = Cast<String>(source_name);
            if (str->length() > 0) {
                SNPrintF(source_file, "%s", str->ToCString().get());
                std::replace(source_file.begin(), source_file.begin() + source_file.length(), '/', '_');
                source_available = true;
            }
        }
    }
    std::replace(filename.begin(), filename.begin() + filename.length(), ' ', '_');
    ...
    if (optional_base_dir != nullptr) {
        SNPrintF(base_dir, "%s%c", optional_base_dir, base::OS::DirectorySeparator());
    } else {
        base_dir[0] = '\0';
    }
    ...
    if (phase == nullptr && !source_available) {
        SNPrintF(full_filename, "%s%s.%s", base_dir.begin(), filename.begin(), suffix);
    } else if (phase != nullptr && !source_available) {
        SNPrintF(full_filename, "%s%s-%s.%s", base_dir.begin(), filename.begin(), phase, suffix);
    } else if (phase == nullptr && source_available) {
        SNPrintF(full_filename, "%s%s_%s.%s", base_dir.begin(), filename.begin(), source_file.begin(), suffix);
    } else {
        SNPrintF(full_filename, "%s%s_%s-%s.%s", base_dir.begin(), filename.begin(), source_file.begin(), phase, suffix);
    }
...
JSONGraphWriter::JSONGraphWriter(std::ostream& os, const Graph* graph, const SourcePositionTable* positions, const NodeOriginTable* origins)
    : os_(os)
    , zone_(nullptr)
    , graph_(graph)
...
void JSONGraphWriter::PrintNode(Node* node, bool is_live)
{
    ...
    os_ << "{\"id\":" << SafeId(node) << ",\"label\":\"" << JSONEscaped(label) << "\""
        ...
        << ",\"live\": " << (is_live ? "true" : "false") << ",\"properties\":\"" << JSONEscaped(properties) << "\"";
    ...
    os_ << ",\"opinfo\":\"" << node->op()->ValueInputCount() << " v " << node->op()->EffectInputCount() << " eff " << node->op()->ControlInputCount()
        << " ctrl in, " << node->op()->ValueOutputCount() << " v " << node->op()->EffectOutputCount() << " eff " << node->op()->ControlOutputCount()
        << " ctrl out\"";
...
void JSONGraphWriter::PrintEdge(Node* from, int index, Node* to)
{
    ...
    os_ << "{\"source\":" << SafeId(to) << ",\"target\":" << SafeId(from) << ",\"index\":" << index << ",\"type\":\"" << edge_type << "\"}";
}
...
private:
    ...
    template <typename InputIterator> void PrintInputs(InputIterator* i, int count, const char* prefix);
...
void GraphC1Visualizer::PrintStringProperty(const char* name, const char* value)
{
    PrintIndent();
    os_ << name << " \"" << value << "\"\n";
}
...
void GraphC1Visualizer::PrintCompilation(const OptimizedCompilationInfo* info)
{
    ...
    PrintLongProperty("date", V8::GetCurrentPlatform()->CurrentClockTimeMilliseconds());
}
...
template <typename InputIterator> void GraphC1Visualizer::PrintInputs(InputIterator* i, int count, const char* prefix)
{
    if (count > 0) {
        os_ << prefix;
    }
...
void GraphC1Visualizer::PrintInputs(Node* node)
{
    ...
    PrintInputs(&i, OperatorProperties::GetContextInputCount(node->op()), " Ctx:");
    PrintInputs(&i, OperatorProperties::GetFrameStateInputCount(node->op()), " FS:");
...
void GraphC1Visualizer::PrintSchedule(
    const char* phase, const Schedule* schedule, const SourcePositionTable* positions, const InstructionSequence* instructions)
{
    Tag tag(this, "cfg");
    ...
    for (size_t i = 0; i < rpo->size(); i++) {
        ...
        const InstructionBlock* instruction_block = instructions->InstructionBlockAt(RpoNumber::FromInt(current->rpo_number()));
        if (instruction_block->code_start() >= 0) {
            ...
            PrintIntProperty("first_lir_id", LifetimePosition::GapFromInstructionIndex(first_index).value());
            PrintIntProperty("last_lir_id", LifetimePosition::InstructionFromInstructionIndex(last_index).value());
        }

        {
            ...
            int total = 0;
            for (BasicBlock::const_iterator it = current->begin(); it != current->end(); ++it) {
                if ((*it)->opcode() == IrOpcode::kPhi)
                    total++;
            }
        ...
        if (instructions != nullptr) {
            ...
            for (int j = instruction_block->first_instruction_index(); j <= instruction_block->last_instruction_index(); j++) {
                PrintIndent();
...
void GraphC1Visualizer::PrintLiveRanges(const char* phase, const RegisterAllocationData* data)
{
...
void GraphC1Visualizer::PrintLiveRangeChain(const TopLevelLiveRange* range, const char* type)
{
...
void GraphC1Visualizer::PrintLiveRange(const LiveRange* range, const char* type, int vreg)
{
    if (range != nullptr && !range->IsEmpty()) {
        PrintIndent();
        ...
        if (range->HasRegisterAssigned()) {
            ...
            if (top->HasSpillRange()) {
                ...
                os_ << " \"const(nostack):" << ConstantOperand::cast(top->GetSpillOperand())->virtual_register() << "\"";
            } else {
...
std::ostream& operator<<(std::ostream& os, const AsC1V& ac)
{
    AccountingAllocator allocator;
    Zone tmp_zone(&allocator, ZONE_NAME);
    GraphC1Visualizer(os, &tmp_zone).PrintSchedule(ac.phase_, ac.schedule_, ac.positions_, ac.instructions_);
    return os;
}
...
std::ostream& operator<<(std::ostream& os, const LiveRangeAsJSON& live_range_json)
{
    ...
    if (range.HasRegisterAssigned()) {
        ...
        os << "\"assigned\",\"op\":" << InstructionOperandAsJSON { &op, &(live_range_json.code_) };
    } else if (range.spilled() && !range.TopLevel()->HasNoSpillType()) {
        const TopLevelLiveRange* top = range.TopLevel();
        if (top->HasSpillOperand()) {
            os << "\"assigned\",\"op\":" << InstructionOperandAsJSON { top->GetSpillOperand(), &(live_range_json.code_) };
        } else {
...
std::ostream& operator<<(std::ostream& os, const TopLevelLiveRangeAsJSON& top_level_live_range_json)
{
    ...
    for (const LiveRange* child = &(top_level_live_range_json.range_); child != nullptr; child = child->next()) {
    ...
    if (top_level_live_range_json.range_.IsFixed()) {
        os << ", \"is_deferred\": " << (top_level_live_range_json.range_.IsDeferredFixed() ? "true" : "false");
    }
    os << ", \"instruction_range\": [" << instruction_range[0] << "," << instruction_range[1] << "]}";
    return os;
}
...
std::ostream& operator<<(std::ostream& os, const InstructionOperandAsJSON& o)
{
    ...
    case InstructionOperand::UNALLOCATED: {
        ...
        case UnallocatedOperand::FIXED_REGISTER: {
            os << ",\"tooltip\": \"FIXED_REGISTER: " << Register::from_code(unalloc->fixed_register_index()) << "\"";
            break;
        }
        case UnallocatedOperand::FIXED_FP_REGISTER: {
            os << ",\"tooltip\": \"FIXED_FP_REGISTER: " << DoubleRegister::from_code(unalloc->fixed_register_index()) << "\"";
            break;
        }
    ...
    case InstructionOperand::ALLOCATED: {
        ...
        os << "\",";
        os << "\"tooltip\": \"" << MachineReprToString(allocated->representation()) << "\"";
        break;
    }
...
std::ostream& operator<<(std::ostream& os, const InstructionAsJSON& i_json)
{
    ...
    for (int i = Instruction::FIRST_GAP_POSITION; i <= Instruction::LAST_GAP_POSITION; i++) {
        ...
        for (MoveOperands* move : *pm) {
            ...
            os << "[" << InstructionOperandAsJSON { &move->destination(), i_json.code_ } << "," << InstructionOperandAsJSON { &move->source(), i_json.code_ }
               << "]";
        }
        os << "]";
    }
...
std::ostream& operator<<(std::ostream& os, const InstructionSequenceAsJSON& s)
{
    ...
    for (int i = 0; i < code->InstructionBlockCount(); i++) {
        if (need_comma)
            os << ",";
        need_comma = true;
        os << InstructionBlockAsJSON { code->InstructionBlockAt(RpoNumber::FromInt(i)), code };
    }
    os << "]";
