...
PeeledIteration* LoopPeeler::Peel(LoopTree::Loop* loop)
{
    ...
    copier.CopyNodes(graph_, tmp_zone_, dead, loop_tree_->BodyNodes(loop), source_positions_, node_origins_);
    ...
    for (Node* exit : loop_tree_->ExitNodes(loop)) {
        ...
        case IrOpcode::kLoopExitValue:
            ...
            NodeProperties::ChangeOp(exit, common_->Phi(LoopExitValueRepresentationOf(exit->op()), 2));
            break;
    ...
    for (Edge edge : node->use_edges()) {
        if (NodeProperties::IsControlEdge(edge)) {
            ...
            if (marker->opcode() == IrOpcode::kLoopExitValue) {
                ...
                NodeProperties::ReplaceUses(marker, nullptr, NodeProperties::GetEffectInput(marker));
                marker->Kill();
            }
        }
    }
    NodeProperties::ReplaceUses(node, nullptr, nullptr, NodeProperties::GetControlInput(node, 0));
    node->Kill();
}
