...
BranchElimination::BranchElimination(Editor* editor, JSGraph* js_graph, Zone* zone, Phase phase)
  ...
  //                                   second_true  second_false
    ...
    auto SemanticsOf = [phase = this->phase_](Node* branch) {
        ...
        if (semantics == BranchSemantics::kUnspecified) {
            semantics = (phase == kEARLY ? BranchSemantics::kJS : BranchSemantics::kMachine);
        }
        return semantics;
    };
    ...
    for (int i = 0; i != input_count; ++i) {
        ...
        if (!branch_condition.IsSet())
            return;
        if (SemanticsOf(branch_condition.branch) != semantics)
            return;
        ...
        if (semantics == BranchSemantics::kJS) {
            ...
            phi_inputs.emplace_back(
                condition_value ? graph->NewNode(jsgraph()->common()->Int32Constant(1)) : graph->NewNode(jsgraph()->common()->Int32Constant(0)));
        }
    }
    phi_inputs.emplace_back(merge);
    Node* new_phi
        = graph->NewNode(common()->Phi(semantics == BranchSemantics::kJS ? MachineRepresentation::kTagged : MachineRepresentation::kWord32, input_count),
...
bool BranchElimination::TryEliminateBranchWithPhiCondition(Node* branch, Node* phi, Node* merge)
{
  ...
  //   false   true
    ...
    if (!phi->OwnedBy(branch))
        return false;
    if (phi->InputCount() != 3)
        return false;
    if (phi->InputAt(2) != merge)
        return false;
    if (merge->UseCount() != 2)
        return false;
    ...
    if (first_value->opcode() != IrOpcode::kInt32Constant || second_value->opcode() != IrOpcode::kInt32Constant) {
        return false;
    }
...
Reduction BranchElimination::ReduceBranch(Node* node)
{
    ...
    if (condition->opcode() == IrOpcode::kPhi && control_input->opcode() == IrOpcode::kMerge) {
...
{
    DCHECK(node->opcode() == IrOpcode::kTrapIf || node->opcode() == IrOpcode::kTrapUnless);
    ...
    return UpdateStatesHelper(node, from_input, condition, node, !trapping_condition, false);
}
...
Reduction BranchElimination::ReduceDeoptimizeConditional(Node* node)
{
    ...
    return UpdateStatesHelper(node, conditions, condition, node, condition_is_true, false);
}
...
Reduction BranchElimination::ReduceIf(Node* node, bool is_true_branch)
{
    ...
    Node* condition = branch->InputAt(0);
    return UpdateStatesHelper(node, from_branch, condition, branch, is_true_branch, true);
}
...
Graph* BranchElimination::graph() const
{
    return jsgraph()->graph();
}

Isolate* BranchElimination::isolate() const
{
