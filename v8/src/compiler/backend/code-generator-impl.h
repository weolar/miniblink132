...
public:
    ...
    CodeEntrypointTag InputCodeEntrypointTag(size_t index)
    {
        ...
        return static_cast<CodeEntrypointTag>(shifted_tag << kCodeEntrypointTagShift);
    }
    ...
    Constant ToConstant(InstructionOperand* op) const
    {
        ...
        return gen_->instructions()->GetConstant(ConstantOperand::cast(op)->virtual_register());
    }
    ...
    {
        return ToConstant(op).ToFloat32();
    }
...
public:
    explicit DeoptimizationExit(
        SourcePosition pos, BytecodeOffset bailout_id, int translation_id, int pc_offset, DeoptimizeKind kind, DeoptimizeReason reason, NodeId node_id)
