...
private:
    ...
    template <typename QueueType> void Schedule();
    ...
    {
        return instr->IsTrap() || (instr->HasMemoryAccessMode() && instr->memory_access_mode() != kMemoryAccessDirect);
    }
    ...
    {
        return MayNeedDeoptOrTrapCheck(instr) || instr->IsDeoptimizeCall() || CanTrap(instr) || HasSideEffect(instr) || IsLoadOperation(instr);
    }
    ...
    {
        return (instr->arch_opcode() == kArchNop) && (instr->OutputCount() == 1) && (instr->OutputAt(0)->IsUnallocated())
