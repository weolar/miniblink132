...
bool IsMacroFused(FirstMacroFusionInstKind first_kind, SecondMacroFusionInstKind second_kind)
{
    ...
    case FirstMacroFusionInstKind::kAddSub:
        return second_kind == SecondMacroFusionInstKind::kAB || second_kind == SecondMacroFusionInstKind::kELG;
...
public:
    ...
    Immediate ToImmediate(InstructionOperand* operand)
    {
        ...
        if (constant.type() == Constant::kCompressedHeapObject) {
            ...
            return Immediate(MacroAssemblerBase::ReadOnlyRootPtr(root_index, gen_->isolate()));
        }
    ...
    Operand SlotToOperand(int slot_index, int extra = 0)
    {
        ...
        return Operand(offset.from_stack_pointer() ? rsp : rbp, offset.offset() + extra);
    }
...
public:
    OutOfLineTruncateDoubleToI(CodeGenerator* gen, Register result, XMMRegister input, StubCallMode stub_mode, UnwindingInfoWriter* unwinding_info_writer)
        : OutOfLineCode(gen)
        , result_(result)
        , input_(input)
        ,
    ...
    void Generate() final
    {
        ...
        unwinding_info_writer_->MaybeIncreaseBaseOffsetAt(__ pc_offset(), kDoubleSize);
        ...
        if (stub_mode_ == StubCallMode::kCallWasmRuntimeStub) {
            ...
            // wasm code space.
            __ near_call(static_cast<intptr_t>(Builtin::kDoubleToI), RelocInfo::WASM_STUB_CALL);
#else
        // For balance.
        if (false) {
#endif // V8_ENABLE_WEBASSEMBLY
...
public:
    OutOfLineRecordWrite(CodeGenerator* gen, Register object, Operand operand, Register value, Register scratch0, Register scratch1, RecordWriteMode mode,
        StubCallMode stub_mode, IndirectPointerTag indirect_pointer_tag = kIndirectPointerNullTag)
        : OutOfLineCode(gen)
        , object_(object)
        , operand_(operand)
        , value_(value)
    ...
    void Generate() final
    {
        ...
        if (COMPRESS_POINTERS_BOOL && mode_ != RecordWriteMode::kValueIsIndirectPointer) {
        ...
        if (mode_ != RecordWriteMode::kValueIsIndirectPointer) {
            ...
            __ CheckPageFlag(value_, scratch0_, MemoryChunk::kIsInReadOnlyHeapMask, not_zero, exit());
            ...
            __ CheckPageFlag(value_, scratch0_, MemoryChunk::kPointersToHereAreInterestingMask, zero, exit());
#endif // !V8_ENABLE_STICKY_MARK_BITS_BOOL
        }

        __ leaq(scratch1_, operand_);

        SaveFPRegsMode const save_fp_mode = frame()->DidAllocateDoubleRegisters() ? SaveFPRegsMode::kSave : SaveFPRegsMode::kIgnore;
        ...
        if (mode_ == RecordWriteMode::kValueIsEphemeronKey) {
            ...
            __ CallIndirectPointerBarrier(object_, scratch1_, save_fp_mode, indirect_pointer_tag_);
#if V8_ENABLE_WEBASSEMBLY
        } else if (stub_mode_ == StubCallMode::kCallWasmRuntimeStub) {
            // A direct call to a wasm runtime stub defined in this module.
            // Just encode the stub index. This will be patched when the code
            // is added to the native module and copied into wasm code space.
            __ CallRecordWriteStubSaveRegisters(object_, scratch1_, save_fp_mode, StubCallMode::kCallWasmRuntimeStub);
#endif // V8_ENABLE_WEBASSEMBLY
        } else {
...
template <std::memory_order order> int EmitStore(MacroAssembler* masm, Operand operand, Register value, MachineRepresentation rep)
{
...
template <std::memory_order order> int EmitStore(MacroAssembler* masm, Operand operand, Immediate value, MachineRepresentation rep);

template <> int EmitStore<std::memory_order_relaxed>(MacroAssembler* masm, Operand operand, Immediate value, MachineRepresentation rep)
{
...
protected:
    ...
    void GenerateWithTrapId(TrapId trap_id)
    {
...
void RecordTrapInfoIfNeeded(Zone* zone, CodeGenerator* codegen, InstructionCode opcode, Instruction* instr, int pc)
{
    const MemoryAccessMode access_mode = instr->memory_access_mode();
    if (access_mode == kMemoryAccessProtectedMemOutOfBounds || access_mode == kMemoryAccessProtectedNullDereference) {
...
#ifdef V8_IS_TSAN
void EmitMemoryProbeForTrapHandlerIfNeeded(MacroAssembler* masm, Register scratch, Operand operand, StubCallMode mode, int size)
{
    ...
    // OOB SIGSEGVs.
    if (trap_handler::IsTrapHandlerEnabled() && mode == StubCallMode::kCallWasmRuntimeStub) {
        switch (size) {
        case kInt8Size:
...
public:
    OutOfLineTSANStore(CodeGenerator* gen, Operand operand, Register value, Register scratch0, StubCallMode stub_mode, int size, std::memory_order order)
        : OutOfLineCode(gen)
        , operand_(operand)
        , value_(value)
        , scratch0_(scratch0)
    ...
    void Generate() final
    {
        const SaveFPRegsMode save_fp_mode = frame()->DidAllocateDoubleRegisters() ? SaveFPRegsMode::kSave : SaveFPRegsMode::kIgnore;
        ...
        if (stub_mode_ == StubCallMode::kCallWasmRuntimeStub) {
            ...
            // Just encode the stub index. This will be patched when the code
            // is added to the native module and copied into wasm code space.
            masm()->CallTSANStoreStub(scratch0_, value_, save_fp_mode, size_, StubCallMode::kCallWasmRuntimeStub, memory_order_);
            return;
        }
#endif // V8_ENABLE_WEBASSEMBLY

        masm()->CallTSANStoreStub(scratch0_, value_, save_fp_mode, size_, StubCallMode::kCallBuiltinPointer, memory_order_);
    }
...
void EmitTSANStoreOOL(Zone* zone, CodeGenerator* codegen, MacroAssembler* masm, Operand operand, Register value_reg, X64OperandConverter& i, StubCallMode mode,
    int size, std::memory_order order)
{
    ...
    auto tsan_ool = zone->New<OutOfLineTSANStore>(codegen, operand, value_reg, scratch0, mode, size, order);
    masm->jmp(tsan_ool->entry());
    masm->bind(tsan_ool->exit());
}

template <std::memory_order order> Register GetTSANValueRegister(MacroAssembler* masm, Register value, X64OperandConverter& i, MachineRepresentation rep)
{
    if (rep == MachineRepresentation::kSandboxedPointer) {
        ...
        Register value_reg = i.TempRegister(1);
        masm->movl(value_reg, FieldOperand(value, ExposedTrustedObject::kSelfIndirectPointerOffset));
        return value_reg;
    }
    return value;
}

template <std::memory_order order> Register GetTSANValueRegister(MacroAssembler* masm, Immediate value, X64OperandConverter& i, MachineRepresentation rep);

template <> Register GetTSANValueRegister<std::memory_order_relaxed>(MacroAssembler* masm, Immediate value, X64OperandConverter& i, MachineRepresentation rep)
{
    Register value_reg = i.TempRegister(1);
    masm->movq(value_reg, value);
    if (rep == MachineRepresentation::kSandboxedPointer) {
        ...
        // Indirect pointer fields contain an index to a pointer table entry, which
        // is obtained from the referenced object.
        masm->movl(value_reg, FieldOperand(value_reg, ExposedTrustedObject::kSelfIndirectPointerOffset));
    }
    return value_reg;
}

template <std::memory_order order, typename ValueT>
void EmitTSANAwareStore(Zone* zone, CodeGenerator* codegen, MacroAssembler* masm, Operand operand, ValueT value, X64OperandConverter& i,
    StubCallMode stub_call_mode, MachineRepresentation rep, Instruction* instr)
{
    // The FOR_TESTING code doesn't initialize the root register. We can't call
    // the TSAN builtin since we need to load the external reference through the
    // root register.
    ...
    if (codegen->code_kind() != CodeKind::FOR_TESTING) {
        if (instr->HasMemoryAccessMode()) {
            RecordTrapInfoIfNeeded(zone, codegen, instr->opcode(), instr, masm->pc_offset());
        }
        int size = ElementSizeInBytes(rep);
        EmitMemoryProbeForTrapHandlerIfNeeded(masm, i.TempRegister(0), operand, stub_call_mode, size);
        Register value_reg = GetTSANValueRegister<order>(masm, value, i, rep);
        EmitTSANStoreOOL(zone, codegen, masm, operand, value_reg, i, stub_call_mode, size, order);
    } else {
        int store_instr_offset = EmitStore<order>(masm, operand, value, rep);
        if (instr->HasMemoryAccessMode()) {
            RecordTrapInfoIfNeeded(zone, codegen, instr->opcode(), instr, store_instr_offset);
        }
    }
}
...
public:
    OutOfLineTSANRelaxedLoad(CodeGenerator* gen, Operand operand, Register scratch0, StubCallMode stub_mode, int size)
        : OutOfLineCode(gen)
        , operand_(operand)
        , scratch0_(scratch0)
        ,
    ...
    void Generate() final
    {
        ...
        if (stub_mode_ == StubCallMode::kCallWasmRuntimeStub) {
            ...
            // Just encode the stub index. This will be patched when the code
            // is added to the native module and copied into wasm code space.
            __ CallTSANRelaxedLoadStub(scratch0_, save_fp_mode, size_, StubCallMode::kCallWasmRuntimeStub);
            return;
        }
#endif // V8_ENABLE_WEBASSEMBLY

        __ CallTSANRelaxedLoadStub(scratch0_, save_fp_mode, size_, StubCallMode::kCallBuiltinPointer);
    }
...
void EmitTSANRelaxedLoadOOLIfNeeded(
    Zone* zone, CodeGenerator* codegen, MacroAssembler* masm, Operand operand, X64OperandConverter& i, StubCallMode mode, int size)
{
    // The FOR_TESTING code doesn't initialize the root register. We can't call
    // the TSAN builtin since we need to load the external reference through the
    // root register.
    ...
    Register scratch0 = i.TempRegister(0);
    auto tsan_ool = zone->New<OutOfLineTSANRelaxedLoad>(codegen, operand, scratch0, mode, size);
    masm->jmp(tsan_ool->entry());
    masm->bind(tsan_ool->exit());
}
...
void EmitTSANAwareStore(Zone* zone, CodeGenerator* codegen, MacroAssembler* masm, Operand operand, ValueT value, X64OperandConverter& i,
    StubCallMode stub_call_mode, MachineRepresentation rep, Instruction* instr)
{
    DCHECK(order == std::memory_order_relaxed || order == std::memory_order_seq_cst);
    int store_instr_off = EmitStore<order>(masm, operand, value, rep);
    if (instr->HasMemoryAccessMode()) {
        RecordTrapInfoIfNeeded(zone, codegen, instr->opcode(), instr, store_instr_off);
    }
}
...
#define ASSEMBLE_MULT(asm_instr)                                                                                                                               \
    do {                                                                                                                                                       \
        if (HasImmediateInput(instr, 1)) {                                                                                                                     \
            if (HasRegisterInput(instr, 0)) {                                                                                                                  \
                __ asm_instr(i.OutputRegister(), i.InputRegister(0), i.InputImmediate(1));                                                                     \
            } else {                                                                                                                                           \
                __ asm_instr(i.OutputRegister(), i.InputOperand(0), i.InputImmediate(1));                                                                      \
            }                                                                                                                                                  \
        } else {                                                                                                                                               \
            if (HasRegisterInput(instr, 1)) {                                                                                                                  \
...
#define ASSEMBLE_AVX_BINOP(asm_instr)                                                                                                                          \
    do {                                                                                                                                                       \
        ...
        if (HasAddressingMode(instr)) {                                                                                                                        \
            ...
            if (instr->InputAt(1)->IsFPRegister()) {                                                                                                           \
                __ asm_instr(i.OutputDoubleRegister(), i.InputDoubleRegister(0), i.InputDoubleRegister(1));                                                    \
            } else {                                                                                                                                           \
                __ asm_instr(i.OutputDoubleRegister(), i.InputDoubleRegister(0), i.InputOperand(1));                                                           \
            }                                                                                                                                                  \
        }                                                                                                                                                      \
    } while (false)
...
#define ASSEMBLE_SIMD_BINOP(opcode)                                                                                                                            \
    do {                                                                                                                                                       \
        if (CpuFeatures::IsSupported(AVX)) {                                                                                                                   \
            CpuFeatureScope avx_scope(masm(), AVX);                                                                                                            \
            __ v##opcode(i.OutputSimd128Register(), i.InputSimd128Register(0), i.InputSimd128Register(1));                                                     \
        } else {                                                                                                                                               \
...
#define ASSEMBLE_SIMD256_BINOP(opcode, cpu_feature)                                                                                                            \
    do {                                                                                                                                                       \
        ...
        __ v##opcode(i.OutputSimd256Register(), i.InputSimd256Register(0), i.InputSimd256Register(1));                                                         \
    } while (false)
...
#define ASSEMBLE_SIMD_PUNPCK_SHUFFLE(opcode)                                                                                                                   \
    do {                                                                                                                                                       \
        ...
        if (CpuFeatures::IsSupported(AVX)) {                                                                                                                   \
            ...
            __ v##opcode(dst, i.InputSimd128Register(0), i.InputSimd128Register(input_index));                                                                 \
        } else {                                                                                                                                               \
...
#define ASSEMBLE_SIMD_SHIFT(opcode, width)                                                                                                                     \
    do {                                                                                                                                                       \
        ...
        if (HasImmediateInput(instr, 1)) {                                                                                                                     \
            if (CpuFeatures::IsSupported(AVX)) {                                                                                                               \
                CpuFeatureScope avx_scope(masm(), AVX);                                                                                                        \
                __ v##opcode(dst, i.InputSimd128Register(0), uint8_t { i.InputInt##width(1) });                                                                \
            } else {                                                                                                                                           \
                DCHECK_EQ(dst, i.InputSimd128Register(0));                                                                                                     \
...
#define ASSEMBLE_SEQ_CST_STORE(rep)                                                                                                                            \
    do {                                                                                                                                                       \
        ...
        EmitTSANAwareStore<std::memory_order_seq_cst>(zone(), this, masm(), operand, value, i, DetermineStubCallMode(), rep, instr);                           \
    } while (false)
...
void AdjustStackPointerForTailCall(Instruction* instr, MacroAssembler* assembler, Linkage* linkage, OptimizedCompilationInfo* info, FrameAccessState* state,
    int new_slot_above_sp, bool allow_shrinkage = true)
{
    ...
    if (instr->HasCallDescriptorFlag(CallDescriptor::kIsTailCallForTierUp)) {
        ...
        DCHECK(linkage->GetIncomingDescriptor()->CalleeSavedFPRegisters().is_empty());
        DCHECK_EQ(state->frame()->GetReturnSlotCount(), 0);
        stack_slot_delta = (state->frame()->GetTotalFrameSlotCount() - kReturnAddressStackSlotCount) * -1;
...
void SetupSimdImmediateInRegister(MacroAssembler* assembler, uint32_t* imms, XMMRegister reg)
{
    assembler->Move(reg, make_uint64(imms[3], imms[2]), make_uint64(imms[1], imms[0]));
}

void SetupSimd256ImmediateInRegister(MacroAssembler* assembler, uint32_t* imms, YMMRegister reg, XMMRegister scratch)
{
    bool is_splat = std::all_of(imms, imms + kSimd256Size, [imms](uint32_t v) { return v == imms[0]; });
    if (is_splat) {
        ...
        assembler->Move(reg, make_uint64(imms[3], imms[2]), make_uint64(imms[1], imms[0]));
        assembler->Move(scratch, make_uint64(imms[7], imms[6]), make_uint64(imms[5], imms[4]));
        CpuFeatureScope avx_scope(assembler, AVX2);
...
void CodeGenerator::AssembleTailCallBeforeGap(Instruction* instr, int first_unused_slot_offset)
{
    CodeGenerator::PushTypeFlags flags(kImmediatePush | kScalarPush);
    ZoneVector<MoveOperands*> pushes(zone());
    GetPushCompatibleMoves(instr, flags, &pushes);

    if (!pushes.empty() && (LocationOperand::cast(pushes.back()->destination()).index() + 1 == first_unused_slot_offset)) {
        DCHECK(!instr->HasCallDescriptorFlag(CallDescriptor::kIsTailCallForTierUp));
        X64OperandConverter g(this, instr);
        for (auto move : pushes) {
            LocationOperand destination_location(LocationOperand::cast(move->destination()));
...
void CodeGenerator::AssembleTailCallAfterGap(Instruction* instr, int first_unused_slot_offset)
{
...
void CodeGenerator::AssembleDispatchHandleRegisterCheck()
{
    ...
    __ LoadParameterCountFromJSDispatchTable(rbx, kJavaScriptCallDispatchHandleRegister);
...
{
    if (info()->shadow_stack_compliant_lazy_deopt() && instr->HasCallDescriptorFlag(CallDescriptor::kNeedsFrameState)) {
...
CodeGenerator::CodeGenResult CodeGenerator::AssembleArchInstruction(Instruction* instr)
{
    ...
    case kArchCallCodeObject: {
        if (HasImmediateInput(instr, 0)) {
            ...
            CodeEntrypointTag tag = i.InputCodeEntrypointTag(instr->CodeEnrypointTagInputIndex());
            DCHECK_IMPLIES(instr->HasCallDescriptorFlag(CallDescriptor::kFixedTargetRegister), reg == kJavaScriptCallCodeStartRegister);
    ...
    case kArchCallJSFunction: {
        ...
        uint32_t num_arguments = i.InputUint32(instr->JSCallArgumentCountInputIndex());
    ...
    case kArchSaveCallerRegisters: {
        ...
        DCHECK(fp_mode_ == SaveFPRegsMode::kIgnore || fp_mode_ == SaveFPRegsMode::kSave);
    ...
    case kArchCallCFunction: {
        ...
        if (linkage()->GetIncomingDescriptor()->IsWasmCapiFunction()) {
            ...
            __ movq(MemOperand(rbp, WasmExitFrameConstants::kCallingPCOffset), kScratchRegister);
        ...
        if (HasImmediateInput(instr, 0)) {
            ...
            pc_offset = __ CallCFunction(ref, num_gp_parameters + num_fp_parameters, set_isolate_data_slots, &return_location);
        } else {
            Register func = i.InputRegister(0);
            pc_offset = __ CallCFunction(func, num_gp_parameters + num_fp_parameters, set_isolate_data_slots, &return_location);
        }

        RecordSafepoint(instr->reference_map(), pc_offset);

        bool const needs_frame_state = (arch_opcode == kArchCallCFunctionWithFrameState);
    ...
    case kArchTableSwitch:
        ...
        break;
    case kArchComment:
        __ RecordComment(reinterpret_cast<const char*>(i.InputInt64(0)), SourceLocation());
        break;
    ...
    case kArchDeoptimize: {
        DeoptimizationExit* exit = BuildTranslation(instr, -1, 0, 0, OutputFrameStateCombine::Ignore());
        __ jmp(exit->label());
        break;
    }
    case kArchRet:
    ...
    case kArchTruncateDoubleToI: {
        ...
        auto ool = zone()->New<OutOfLineTruncateDoubleToI>(this, result, input, DetermineStubCallMode(), &unwinding_info_writer_);
    ...
    case kArchAtomicStoreWithWriteBarrier: {
        ...
        auto ool = zone()->New<OutOfLineRecordWrite>(this, object, operand, value, scratch0, scratch1, mode, DetermineStubCallMode());
        if (arch_opcode == kArchStoreWithWriteBarrier) {
            EmitTSANAwareStore<std::memory_order_relaxed>(
                zone(), this, masm(), operand, value, i, DetermineStubCallMode(), MachineRepresentation::kTagged, instr);
        } else {
...
#if V8_ENABLE_STICKY_MARK_BITS_BOOL
        __ CheckPageFlag(object, scratch0, MemoryChunk::kIncrementalMarking, not_zero, ool->stub_call());
        __ CheckMarkBit(object, scratch0, scratch1, carry, ool->entry());
#else // !V8_ENABLE_STICKY_MARK_BITS_BOOL
        __ CheckPageFlag(object, scratch0, MemoryChunk::kPointersFromHereAreInterestingMask, not_zero, ool->entry());
#endif // !V8_ENABLE_STICKY_MARK_BITS_BOOL
        __ bind(ool->exit());
        break;
    }
    case kArchStoreIndirectWithWriteBarrier: {
        ...
        Operand operand = i.MemoryOperand(&index);
        Register value = i.InputRegister(index++);
        IndirectPointerTag tag = static_cast<IndirectPointerTag>(i.InputInt64(index));
        DCHECK(IsValidIndirectPointerTag(tag));
        Register scratch0 = i.TempRegister(0);
        Register scratch1 = i.TempRegister(1);

        auto ool = zone()->New<OutOfLineRecordWrite>(this, object, operand, value, scratch0, scratch1, mode, DetermineStubCallMode(), tag);
        EmitTSANAwareStore<std::memory_order_relaxed>(
            zone(), this, masm(), operand, value, i, DetermineStubCallMode(), MachineRepresentation::kIndirectPointer, instr);
    ...
    case kSSEFloat64Mod: {
        ...
        if (CpuFeatures::IsSupported(SAHF)) {
            ...
            __ pushq(rax);
            unwinding_info_writer_.MaybeIncreaseBaseOffsetAt(__ pc_offset(), kSystemPointerSize);
            __ popfq();
            unwinding_info_writer_.MaybeIncreaseBaseOffsetAt(__ pc_offset(), -kSystemPointerSize);
        }
    ...
    case kSSEFloat32Max: {
        ...
        auto ool = zone()->New<OutOfLineLoadFloat32NaN>(this, i.OutputDoubleRegister());
    ...
    case kSSEFloat64Max: {
        ...
        auto ool = zone()->New<OutOfLineLoadFloat64NaN>(this, i.OutputDoubleRegister());
        __ j(parity_even, ool->entry());
        __ j(above, &done_compare, Label::kNear);
        __ j(below, &compare_swap, Label::kNear);
    ...
    case kSSEFloat64ToFloat16: {
        __ Cvtpd2ph(i.OutputDoubleRegister(), i.InputDoubleRegister(0), i.TempRegister(0));
        break;
    }
    ...
    case kAVXFloat64Div:
        ...
        break;
    case kX64Float32Abs: {
        __ Absps(i.OutputDoubleRegister(), i.InputDoubleRegister(0), kScratchRegister);
        break;
    }
    case kX64Float32Neg: {
        __ Negps(i.OutputDoubleRegister(), i.InputDoubleRegister(0), kScratchRegister);
        break;
    }
    case kX64FAbs: {
        ...
        if (vec_len == kV128) {
            ...
            case kL16: {
                // F16x8Abs
                CpuFeatureScope avx_scope(masm(), AVX);
                __ Absph(i.OutputSimd128Register(), i.InputSimd128Register(0), kScratchRegister);
                break;
            }
            case kL32: {
                // F32x4Abs
                __ Absps(i.OutputSimd128Register(), i.InputSimd128Register(0), kScratchRegister);
                break;
            }
            case kL64: {
                // F64x2Abs
                __ Abspd(i.OutputDoubleRegister(), i.InputDoubleRegister(0), kScratchRegister);
                break;
            }
            default:
                UNREACHABLE();
            ...
            case kL32: {
                ...
                CpuFeatureScope avx_scope(masm(), AVX2);
                if (dst == src) {
                    __ vpcmpeqd(kScratchSimd256Reg, kScratchSimd256Reg, kScratchSimd256Reg);
            ...
            case kL64: {
                ...
                CpuFeatureScope avx_scope(masm(), AVX2);
                if (dst == src) {
                    __ vpcmpeqq(kScratchSimd256Reg, kScratchSimd256Reg, kScratchSimd256Reg);
    ...
    case kX64FNeg: {
        ...
        if (vec_len == kV128) {
            ...
            case kL16: {
                // F16x8Neg
                CpuFeatureScope avx_scope(masm(), AVX);
                __ Negph(i.OutputSimd128Register(), i.InputSimd128Register(0), kScratchRegister);
                break;
            }
            case kL32: {
                // F32x4Neg
                __ Negps(i.OutputSimd128Register(), i.InputSimd128Register(0), kScratchRegister);
                break;
            }
            case kL64: {
                // F64x2Neg
                __ Negpd(i.OutputDoubleRegister(), i.InputDoubleRegister(0), kScratchRegister);
                break;
            }
            default:
                UNREACHABLE();
    ...
    case kX64Movb: {
        ...
        if (HasImmediateInput(instr, index)) {
            ...
            EmitTSANAwareStore<std::memory_order_relaxed>(
                zone(), this, masm(), operand, value, i, DetermineStubCallMode(), MachineRepresentation::kWord8, instr);
        } else {
            Register value(i.InputRegister(index));
            EmitTSANAwareStore<std::memory_order_relaxed>(
                zone(), this, masm(), operand, value, i, DetermineStubCallMode(), MachineRepresentation::kWord8, instr);
    ...
    case kX64Movw: {
        ...
        if (HasImmediateInput(instr, index)) {
            ...
            EmitTSANAwareStore<std::memory_order_relaxed>(
                zone(), this, masm(), operand, value, i, DetermineStubCallMode(), MachineRepresentation::kWord16, instr);
        } else {
            Register value(i.InputRegister(index));
            EmitTSANAwareStore<std::memory_order_relaxed>(
                zone(), this, masm(), operand, value, i, DetermineStubCallMode(), MachineRepresentation::kWord16, instr);
    ...
    case kX64Movl:
        if (instr->HasOutput()) {
            ...
            if (HasAddressingMode(instr)) {
                ...
                EmitTSANRelaxedLoadOOLIfNeeded(zone(), this, masm(), address, i, DetermineStubCallMode(), kInt32Size);
            } else {
                if (HasRegisterInput(instr, 0)) {
            ...
            if (HasImmediateInput(instr, index)) {
                ...
                EmitTSANAwareStore<std::memory_order_relaxed>(
                    zone(), this, masm(), operand, value, i, DetermineStubCallMode(), MachineRepresentation::kWord32, instr);
            } else {
                Register value(i.InputRegister(index));
                EmitTSANAwareStore<std::memory_order_relaxed>(
                    zone(), this, masm(), operand, value, i, DetermineStubCallMode(), MachineRepresentation::kWord32, instr);
    ...
    case kX64MovqDecompressTaggedSigned: {
        ...
        EmitTSANRelaxedLoadOOLIfNeeded(zone(), this, masm(), address, i, DetermineStubCallMode(), kTaggedSize);
        break;
    }
    ...
    case kX64MovqDecodeSandboxedPointer: {
        ...
        EmitTSANRelaxedLoadOOLIfNeeded(zone(), this, masm(), address, i, DetermineStubCallMode(), kSystemPointerSize);
        break;
    }
    case kX64MovqEncodeSandboxedPointer: {
        ...
        Register value(i.InputRegister(index));
        EmitTSANAwareStore<std::memory_order_relaxed>(
            zone(), this, masm(), operand, value, i, DetermineStubCallMode(), MachineRepresentation::kSandboxedPointer, instr);
        break;
    }
    case kX64Movq:
        if (instr->HasOutput()) {
            ...
            EmitTSANRelaxedLoadOOLIfNeeded(zone(), this, masm(), address, i, DetermineStubCallMode(), kInt64Size);
        } else {
            size_t index = 0;
            Operand operand = i.MemoryOperand(&index);
            if (HasImmediateInput(instr, index)) {
                Immediate value(i.InputImmediate(index));
                EmitTSANAwareStore<std::memory_order_relaxed>(
                    zone(), this, masm(), operand, value, i, DetermineStubCallMode(), MachineRepresentation::kWord64, instr);
            } else {
                Register value(i.InputRegister(index));
                EmitTSANAwareStore<std::memory_order_relaxed>(
                    zone(), this, masm(), operand, value, i, DetermineStubCallMode(), MachineRepresentation::kWord64, instr);
        ...
        if (i.InputRegister(0) == i.OutputRegister()) {
            if (mode == kMode_MRI) {
                ...
                if (constant_summand > 0) {
                    ...
                    __ subl(i.OutputRegister(), Immediate(base::NegateWithWraparound(constant_summand)));
                }
    ...
    case kX64Push: {
        ...
        if (HasAddressingMode(instr)) {
            ...
            } else if (input->IsStackSlot() || input->IsFloatStackSlot() || input->IsDoubleStackSlot()) {
                __ AllocateStackSpace(stack_decrement - kSystemPointerSize);
        ...
        unwinding_info_writer_.MaybeIncreaseBaseOffsetAt(__ pc_offset(), stack_decrement);
        break;
    }
    case kX64Poke: {
        ...
        if (HasImmediateInput(instr, 0)) {
            ...
            if (op->representation() == MachineRepresentation::kFloat64) {
                __ Movsd(Operand(rsp, slot * kSystemPointerSize), i.InputDoubleRegister(0));
            } else {
                DCHECK_EQ(MachineRepresentation::kFloat32, op->representation());
                __ Movss(Operand(rsp, slot * kSystemPointerSize), i.InputFloatRegister(0));
            }
        } else {
    ...
    case kX64FSplat: {
        ...
        if (vec_len == kV128) {
            ...
            case kL16: {
                CpuFeatureScope f16c_scope(masm(), F16C);
                CpuFeatureScope avx2_scope(masm(), AVX2);
                __ vcvtps2ph(i.OutputDoubleRegister(0), i.InputDoubleRegister(0), 0);
                __ vpbroadcastw(i.OutputSimd128Register(), i.OutputDoubleRegister(0));
                break;
            }
            case kL32: {
                // F32x4Splat
    ...
    case kX64FExtractLane: {
        ...
        if (vec_len == kV128) {
            ...
            case kL16: {
                ...
                CpuFeatureScope f16c_scope(masm(), F16C);
                CpuFeatureScope avx_scope(masm(), AVX);
                __ Pextrw(kScratchRegister, i.InputSimd128Register(0), i.InputUint8(1));
            ...
            case kL32: {
                // F32x4ExtractLane
                __ F32x4ExtractLane(i.OutputFloatRegister(), i.InputSimd128Register(0), i.InputUint8(1));
                break;
            }
            case kL64: {
                // F64X2ExtractLane
                __ F64x2ExtractLane(i.OutputDoubleRegister(), i.InputDoubleRegister(0), i.InputUint8(1));
                break;
            }
            default:
                UNREACHABLE();
    ...
    case kX64FReplaceLane: {
        ...
        if (vec_len == kV128) {
            ...
            case kL16: {
                ...
                __ vpinsrw(i.OutputSimd128Register(), i.InputSimd128Register(0), kScratchRegister, i.InputInt8(1));
                break;
            }
            case kL32: {
                // F32x4ReplaceLane
            ...
            case kL64: {
                // F64X2ReplaceLane
                __ F64x2ReplaceLane(i.OutputSimd128Register(), i.InputSimd128Register(0), i.InputDoubleRegister(2), i.InputInt8(1));
                break;
            }
            default:
                UNREACHABLE();
    ...
    case kX64FMin: {
        ...
        if (vec_len == kV128) {
            ...
            case kL16: {
                ...
                YMMRegister ydst = YMMRegister::from_code(i.OutputSimd128Register().code());
                __ F16x8Min(ydst, i.InputSimd128Register(0), i.InputSimd128Register(1), i.TempSimd256Register(0), i.TempSimd256Register(1));
                break;
            }
            case kL32: {
                // F32x4Min
                __ F32x4Min(i.OutputSimd128Register(), i.InputSimd128Register(0), i.InputSimd128Register(1), kScratchDoubleReg);
                break;
            }
            case kL64: {
                // F64x2Min
                ...
                DCHECK_EQ(i.OutputSimd128Register(), i.InputSimd128Register(0));
                __ F64x2Min(i.OutputSimd128Register(), i.InputSimd128Register(0), i.InputSimd128Register(1), kScratchDoubleReg);
                break;
            }
            default:
                UNREACHABLE();
            ...
            case kL32: {
                // F32x8Min
                __ F32x8Min(i.OutputSimd256Register(), i.InputSimd256Register(0), i.InputSimd256Register(1), kScratchSimd256Reg);
                break;
            }
            case kL64: {
                // F64x4Min
                DCHECK_EQ(i.OutputSimd256Register(), i.InputSimd256Register(0));
                __ F64x4Min(i.OutputSimd256Register(), i.InputSimd256Register(0), i.InputSimd256Register(1), kScratchSimd256Reg);
                break;
            }
            default:
                UNREACHABLE();
    ...
    case kX64FMax: {
        ...
        if (vec_len == kV128) {
            ...
            case kL16: {
                ...
                // YMM register during computation. Cast dst to YMM here.
                YMMRegister ydst = YMMRegister::from_code(i.OutputSimd128Register().code());
                __ F16x8Max(ydst, i.InputSimd128Register(0), i.InputSimd128Register(1), i.TempSimd256Register(0), i.TempSimd256Register(1));
                break;
            }
            case kL32: {
                // F32x4Max
                __ F32x4Max(i.OutputSimd128Register(), i.InputSimd128Register(0), i.InputSimd128Register(1), kScratchDoubleReg);
                break;
            }
            case kL64: {
                // F64x2Max
                // Avoids a move in no-AVX case if dst = src0.
                DCHECK_EQ(i.OutputSimd128Register(), i.InputSimd128Register(0));
                __ F64x2Max(i.OutputSimd128Register(), i.InputSimd128Register(0), i.InputSimd128Register(1), kScratchDoubleReg);
                break;
            }
            default:
                UNREACHABLE();
            ...
            case kL32: {
                // F32x8Max
                __ F32x8Max(i.OutputSimd256Register(), i.InputSimd256Register(0), i.InputSimd256Register(1), kScratchSimd256Reg);
                break;
            }
            case kL64: {
                // F64x4Max
                DCHECK_EQ(i.OutputSimd256Register(), i.InputSimd256Register(0));
                __ F64x4Max(i.OutputSimd256Register(), i.InputSimd256Register(0), i.InputSimd256Register(1), kScratchSimd256Reg);
                break;
            }
            default:
                UNREACHABLE();
    ...
    case kX64F64x2Qfma: {
        __ F64x2Qfma(i.OutputSimd128Register(), i.InputSimd128Register(0), i.InputSimd128Register(1), i.InputSimd128Register(2), kScratchDoubleReg);
        break;
    }
    case kX64F64x2Qfms: {
        __ F64x2Qfms(i.OutputSimd128Register(), i.InputSimd128Register(0), i.InputSimd128Register(1), i.InputSimd128Register(2), kScratchDoubleReg);
        break;
    }
    case kX64F64x4Qfma: {
        __ F64x4Qfma(i.OutputSimd256Register(), i.InputSimd256Register(0), i.InputSimd256Register(1), i.InputSimd256Register(2), kScratchSimd256Reg);
        break;
    }
    case kX64F64x4Qfms: {
        __ F64x4Qfms(i.OutputSimd256Register(), i.InputSimd256Register(0), i.InputSimd256Register(1), i.InputSimd256Register(2), kScratchSimd256Reg);
        break;
    }
    ...
    case kX64F64x2ConvertLowI32x4U: {
        __ F64x2ConvertLowI32x4U(i.OutputSimd128Register(), i.InputSimd128Register(0), kScratchRegister);
        break;
    }
    ...
    case kX64I32x4TruncSatF64x2SZero: {
        __ I32x4TruncSatF64x2SZero(i.OutputSimd128Register(), i.InputSimd128Register(0), kScratchDoubleReg, kScratchRegister);
        break;
    }
    case kX64I32x4TruncSatF64x2UZero: {
        __ I32x4TruncSatF64x2UZero(i.OutputSimd128Register(), i.InputSimd128Register(0), kScratchDoubleReg, kScratchRegister);
        break;
    }
    ...
    case kX64I16x8SConvertF16x8: {
        ...
        YMMRegister ydst = YMMRegister::from_code(i.OutputSimd128Register().code());
        __ I16x8SConvertF16x8(ydst, i.InputSimd128Register(0), kScratchSimd256Reg, kScratchRegister);
        break;
    }
    ...
    case kX64F32x8UConvertI32x8: {
        ...
        __ vpblendw(kScratchSimd256Reg, kScratchSimd256Reg, dst, uint8_t { 0x55 }); // get lo 16 bits
    ...
    case kX64F32x4Qfma: {
        __ F32x4Qfma(i.OutputSimd128Register(), i.InputSimd128Register(0), i.InputSimd128Register(1), i.InputSimd128Register(2), kScratchDoubleReg);
        break;
    }
    case kX64F32x4Qfms: {
        __ F32x4Qfms(i.OutputSimd128Register(), i.InputSimd128Register(0), i.InputSimd128Register(1), i.InputSimd128Register(2), kScratchDoubleReg);
        break;
    }
    case kX64F32x8Qfma: {
        __ F32x8Qfma(i.OutputSimd256Register(), i.InputSimd256Register(0), i.InputSimd256Register(1), i.InputSimd256Register(2), kScratchSimd256Reg);
        break;
    }
    case kX64F32x8Qfms: {
        __ F32x8Qfms(i.OutputSimd256Register(), i.InputSimd256Register(0), i.InputSimd256Register(1), i.InputSimd256Register(2), kScratchSimd256Reg);
        break;
    }
    case kX64F16x8Qfma: {
        YMMRegister ydst = YMMRegister::from_code(i.OutputSimd128Register().code());
        __ F16x8Qfma(ydst, i.InputSimd128Register(0), i.InputSimd128Register(1), i.InputSimd128Register(2), i.TempSimd256Register(0), i.TempSimd256Register(1));
        break;
    }
    case kX64F16x8Qfms: {
        YMMRegister ydst = YMMRegister::from_code(i.OutputSimd128Register().code());
        __ F16x8Qfms(ydst, i.InputSimd128Register(0), i.InputSimd128Register(1), i.InputSimd128Register(2), i.TempSimd256Register(0), i.TempSimd256Register(1));
        break;
    }
    case kX64Minps: {
        VectorLength vec_len = VectorLengthField::decode(opcode);
    ...
    case kX64IShl: {
        ...
        if (vec_len == kV128) {
            ...
            case kL8: {
                ...
                if (HasImmediateInput(instr, 1)) {
                    __ I8x16Shl(dst, src, i.InputInt3(1), kScratchRegister, kScratchDoubleReg);
                } else {
                    __ I8x16Shl(dst, src, i.InputRegister(1), kScratchRegister, kScratchDoubleReg, i.TempSimd128Register(0));
                }
                break;
            }
            case kL16: {
        ...
        if (vec_len == kV128) {
            ...
            case kL8: {
                ...
                if (HasImmediateInput(instr, 1)) {
                    ...
                    __ I8x16ShrS(dst, src, i.InputRegister(1), kScratchRegister, kScratchDoubleReg, i.TempSimd128Register(0));
                }
                break;
            }
            case kL16: {
            ...
            case kL64: {
                ...
                if (HasImmediateInput(instr, 1)) {
                    ...
                    __ I64x2ShrS(dst, src, i.InputRegister(1), kScratchDoubleReg, i.TempSimd128Register(0), kScratchRegister);
                }
                break;
            }
            default:
    ...
    case kX64IMul: {
        ...
        if (vec_len == kV128) {
            ...
            case kL64: {
                // I64x2Mul
                __ I64x2Mul(i.OutputSimd128Register(), i.InputSimd128Register(0), i.InputSimd128Register(1), i.TempSimd128Register(0), kScratchDoubleReg);
                break;
            }
            default:
                UNREACHABLE();
            ...
            case kL64: {
                // I64x4Mul
                __ I64x4Mul(i.OutputSimd256Register(), i.InputSimd256Register(0), i.InputSimd256Register(1), i.TempSimd256Register(0), kScratchSimd256Reg);
                break;
            }
            default:
                UNREACHABLE();
    ...
    case kX64INe: {
        ...
        if (vec_len == kV128) {
            ...
            case kL8: {
                ...
                __ vpcmpeqb(kScratchSimd256Reg, kScratchSimd256Reg, kScratchSimd256Reg);
                __ vpxor(dst, dst, kScratchSimd256Reg);
                break;
            }
            case kL16: {
                ...
                __ vpcmpeqw(kScratchSimd256Reg, kScratchSimd256Reg, kScratchSimd256Reg);
                __ vpxor(dst, dst, kScratchSimd256Reg);
                break;
            }
            case kL32: {
    ...
    case kX64IGtS: {
        ...
        if (vec_len == kV128) {
            ...
            case kL64: {
                // I64x2GtS
                __ I64x2GtS(i.OutputSimd128Register(), i.InputSimd128Register(0), i.InputSimd128Register(1), kScratchDoubleReg);
                break;
            }
            default:
                UNREACHABLE();
    ...
    case kX64IGeS: {
        ...
        if (vec_len == kV128) {
            ...
            case kL64: {
                // I64x2GeS
                __ I64x2GeS(i.OutputSimd128Register(), i.InputSimd128Register(0), i.InputSimd128Register(1), kScratchDoubleReg);
                break;
            }
            default:
                UNREACHABLE();
            ...
            case kL64: {
                // I64x4GeS
                __ vpcmpgtq(dst, i.InputSimd256Register(1), i.InputSimd256Register(0));
                __ vpcmpeqq(kScratchSimd256Reg, kScratchSimd256Reg, kScratchSimd256Reg);
                __ vpxor(dst, dst, kScratchSimd256Reg);
                break;
            }
    ...
    case kX64IShrU: {
        ...
        if (vec_len == kV128) {
            ...
            case kL8: {
                ...
                DCHECK_IMPLIES(!CpuFeatures::IsSupported(AVX), dst == src);
                if (HasImmediateInput(instr, 1)) {
                    __ I8x16ShrU(dst, src, i.InputInt3(1), kScratchRegister, kScratchDoubleReg);
                } else {
                    __ I8x16ShrU(dst, src, i.InputRegister(1), kScratchRegister, kScratchDoubleReg, i.TempSimd128Register(0));
                }
                break;
            }
            case kL16: {
    ...
    case kX64I64x2ExtMulLowI32x4S: {
        __ I64x2ExtMul(i.OutputSimd128Register(), i.InputSimd128Register(0), i.InputSimd128Register(1), kScratchDoubleReg, /*low=*/true,
            /*is_signed=*/true);
        break;
    }
    case kX64I64x2ExtMulHighI32x4S: {
        __ I64x2ExtMul(i.OutputSimd128Register(), i.InputSimd128Register(0), i.InputSimd128Register(1), kScratchDoubleReg,
            /*low=*/false,
            /*is_signed=*/true);
        break;
    }
    ...
    case kX64I64x2SConvertI32x4High: {
        __ I64x2SConvertI32x4High(i.OutputSimd128Register(), i.InputSimd128Register(0));
        break;
    }
    ...
    case kX64I64x2UConvertI32x4High: {
        __ I64x2UConvertI32x4High(i.OutputSimd128Register(), i.InputSimd128Register(0), kScratchDoubleReg);
        break;
    }
    ...
    case kX64I32x4SConvertF32x4: {
        __ I32x4SConvertF32x4(i.OutputSimd128Register(), i.InputSimd128Register(0), kScratchDoubleReg, kScratchRegister);
        break;
    }
    case kX64I32x8SConvertF32x8: {
        __ I32x8SConvertF32x8(i.OutputSimd256Register(), i.InputSimd256Register(0), kScratchSimd256Reg, kScratchRegister);
        break;
    }
    ...
    case kX64I32x4SConvertI16x8High: {
        __ I32x4SConvertI16x8High(i.OutputSimd128Register(), i.InputSimd128Register(0));
        break;
    }
    ...
    case kX64I32x4UConvertI16x8High: {
        __ I32x4UConvertI16x8High(i.OutputSimd128Register(), i.InputSimd128Register(0), kScratchDoubleReg);
        break;
    }
    ...
    case kX64I32x4DotI8x16I7x16AddS: {
        ...
        __ I32x4DotI8x16I7x16AddS(
            i.OutputSimd128Register(), i.InputSimd128Register(0), i.InputSimd128Register(1), i.InputSimd128Register(2), kScratchDoubleReg, tmp);
        break;
    }
    case kX64I32x8DotI8x32I7x32AddS: {
        ...
        __ I32x8DotI8x32I7x32AddS(
            i.OutputSimd256Register(), i.InputSimd256Register(0), i.InputSimd256Register(1), i.InputSimd256Register(2), kScratchSimd256Reg, tmp);
        break;
    }
    case kX64I32x4ExtAddPairwiseI16x8S: {
        __ I32x4ExtAddPairwiseI16x8S(i.OutputSimd128Register(), i.InputSimd128Register(0), kScratchRegister);
        break;
    }
    case kX64I32x8ExtAddPairwiseI16x16S: {
        __ I32x8ExtAddPairwiseI16x16S(i.OutputSimd256Register(), i.InputSimd256Register(0), kScratchSimd256Reg);
        break;
    }
    case kX64I32x4ExtAddPairwiseI16x8U: {
        __ I32x4ExtAddPairwiseI16x8U(i.OutputSimd128Register(), i.InputSimd128Register(0), kScratchDoubleReg);
        break;
    }
    case kX64I32x8ExtAddPairwiseI16x16U: {
        __ I32x8ExtAddPairwiseI16x16U(i.OutputSimd256Register(), i.InputSimd256Register(0), kScratchSimd256Reg);
        break;
    }
    ...
    case kX64I16x8SConvertI8x16High: {
        __ I16x8SConvertI8x16High(i.OutputSimd128Register(), i.InputSimd128Register(0));
        break;
    }
    ...
    case kX64I16x8UConvertI8x16High: {
        __ I16x8UConvertI8x16High(i.OutputSimd128Register(), i.InputSimd128Register(0), kScratchDoubleReg);
        break;
    }
    ...
    case kX64I16x8ExtMulLowI8x16S: {
        __ I16x8ExtMulLow(i.OutputSimd128Register(), i.InputSimd128Register(0), i.InputSimd128Register(1), kScratchDoubleReg,
            /*is_signed=*/true);
        break;
    }
    case kX64I16x8ExtMulHighI8x16S: {
        __ I16x8ExtMulHighS(i.OutputSimd128Register(), i.InputSimd128Register(0), i.InputSimd128Register(1), kScratchDoubleReg);
        break;
    }
    ...
    case kX64I16x8ExtMulHighI8x16U: {
        __ I16x8ExtMulHighU(i.OutputSimd128Register(), i.InputSimd128Register(0), i.InputSimd128Register(1), kScratchDoubleReg);
        break;
    }
    case kX64I16x8ExtAddPairwiseI8x16S: {
        __ I16x8ExtAddPairwiseI8x16S(i.OutputSimd128Register(), i.InputSimd128Register(0), kScratchDoubleReg, kScratchRegister);
        break;
    }
    case kX64I16x16ExtAddPairwiseI8x32S: {
        __ I16x16ExtAddPairwiseI8x32S(i.OutputSimd256Register(), i.InputSimd256Register(0), kScratchSimd256Reg);
        break;
    }
    case kX64I16x8ExtAddPairwiseI8x16U: {
        __ I16x8ExtAddPairwiseI8x16U(i.OutputSimd128Register(), i.InputSimd128Register(0), kScratchRegister);
        break;
    }
    case kX64I16x16ExtAddPairwiseI8x32U: {
        __ I16x16ExtAddPairwiseI8x32U(i.OutputSimd256Register(), i.InputSimd256Register(0), kScratchSimd256Reg);
        break;
    }
    case kX64I16x8Q15MulRSatS: {
        __ I16x8Q15MulRSatS(i.OutputSimd128Register(), i.InputSimd128Register(0), i.InputSimd128Register(1), kScratchDoubleReg);
        break;
    }
    case kX64I16x8RelaxedQ15MulRS: {
        __ Pmulhrsw(i.OutputSimd128Register(), i.InputSimd128Register(0), i.InputSimd128Register(1));
        break;
    }
    case kX64I16x8DotI8x16I7x16S: {
        __ I16x8DotI8x16I7x16S(i.OutputSimd128Register(), i.InputSimd128Register(0), i.InputSimd128Register(1));
        break;
    }
    case kX64I16x16DotI8x32I7x32S: {
        CpuFeatureScope avx_scope(masm(), AVX2);
        __ vpmaddubsw(i.OutputSimd256Register(), i.InputSimd256Register(1), i.InputSimd256Register(0));
        break;
    }
    case kX64Pextrb: {
        RecordTrapInfoIfNeeded(zone(), this, opcode, instr, __ pc_offset());
        ...
        if (HasAddressingMode(instr)) {
            Operand operand = i.MemoryOperand(&index);
            __ Pextrb(operand, i.InputSimd128Register(index), i.InputUint8(index + 1));
        } else {
            __ Pextrb(i.OutputRegister(), i.InputSimd128Register(0), i.InputUint8(1));
        }
        break;
    }
    case kX64Pextrw: {
        ...
        if (HasAddressingMode(instr)) {
            Operand operand = i.MemoryOperand(&index);
            __ Pextrw(operand, i.InputSimd128Register(index), i.InputUint8(index + 1));
        } else {
            __ Pextrw(i.OutputRegister(), i.InputSimd128Register(0), i.InputUint8(1));
        }
        break;
    }
    case kX64Pinsrb: {
    ...
    case kX64I32x4ExtMulLowI16x8S: {
        __ I32x4ExtMul(i.OutputSimd128Register(), i.InputSimd128Register(0), i.InputSimd128Register(1), kScratchDoubleReg,
            /*low=*/true,
            /*is_signed=*/true);
        break;
    }
    ...
    case kX64SNot: {
        ...
        if (vec_len == kV128) { // S128Not
            __ S128Not(i.OutputSimd128Register(), i.InputSimd128Register(0), kScratchDoubleReg);
        } else if (vec_len == kV256) { // S256Not
            __ S256Not(i.OutputSimd256Register(), i.InputSimd256Register(0), kScratchSimd256Reg);
        } else {
            UNREACHABLE();
        }
        break;
    ...
    case kX64SSelect: {
        ...
        if (vec_len == kV128) { // S128Select
            __ S128Select(i.OutputSimd128Register(), i.InputSimd128Register(0), i.InputSimd128Register(1), i.InputSimd128Register(2), kScratchDoubleReg);
        } else if (vec_len == kV256) { // S256Select
            __ S256Select(i.OutputSimd256Register(), i.InputSimd256Register(0), i.InputSimd256Register(1), i.InputSimd256Register(2), kScratchSimd256Reg);
        } else {
            UNREACHABLE();
        }
        break;
    ...
    case kX64I8x16Swizzle: {
        __ I8x16Swizzle(i.OutputSimd128Register(), i.InputSimd128Register(0), i.InputSimd128Register(1), kScratchDoubleReg, kScratchRegister,
    ...
    case kX64I8x16Popcnt: {
        __ I8x16Popcnt(i.OutputSimd128Register(), i.InputSimd128Register(0), i.TempSimd128Register(0), kScratchDoubleReg, kScratchRegister);
        break;
    }
    case kX64S128Load8Splat: {
        RecordTrapInfoIfNeeded(zone(), this, opcode, instr, __ pc_offset());
        __ S128Load8Splat(i.OutputSimd128Register(), i.MemoryOperand(), kScratchDoubleReg);
        break;
    }
    case kX64S128Load16Splat: {
        RecordTrapInfoIfNeeded(zone(), this, opcode, instr, __ pc_offset());
        __ S128Load16Splat(i.OutputSimd128Register(), i.MemoryOperand(), kScratchDoubleReg);
        break;
    }
    ...
    case kX64Shufps: {
        if (instr->Output()->IsSimd128Register()) {
            __ Shufps(i.OutputSimd128Register(), i.InputSimd128Register(0), i.InputSimd128Register(1), i.InputUint8(2));
        } else {
            DCHECK(instr->Output()->IsSimd256Register());
            DCHECK(CpuFeatures::IsSupported(AVX));
            CpuFeatureScope scope(masm(), AVX);
            __ vshufps(i.OutputSimd256Register(), i.InputSimd256Register(0), i.InputSimd256Register(1), i.InputUint8(2));
        }
        break;
    }
    ...
    case kX64S32x4Swizzle: {
        ...
        ASSEMBLE_SIMD_IMM_INSTR(Pshufd, i.OutputSimd128Register(), 0, i.InputUint8(1));
        break;
    }
    ...
    case kX64Blendvpd: {
        VectorLength vec_len = VectorLengthField::decode(opcode);
        if (vec_len == kV128) {
            __ Blendvpd(i.OutputSimd128Register(), i.InputSimd128Register(0), i.InputSimd128Register(1), i.InputSimd128Register(2));
        } else {
            DCHECK_EQ(vec_len, kV256);
            CpuFeatureScope avx_scope(masm(), AVX);
            __ vblendvpd(i.OutputSimd256Register(), i.InputSimd256Register(0), i.InputSimd256Register(1), i.InputSimd256Register(2));
        }
        break;
    }
    case kX64Blendvps: {
        VectorLength vec_len = VectorLengthField::decode(opcode);
        if (vec_len == kV128) {
            __ Blendvps(i.OutputSimd128Register(), i.InputSimd128Register(0), i.InputSimd128Register(1), i.InputSimd128Register(2));
        } else {
            DCHECK_EQ(vec_len, kV256);
            CpuFeatureScope avx_scope(masm(), AVX);
            __ vblendvps(i.OutputSimd256Register(), i.InputSimd256Register(0), i.InputSimd256Register(1), i.InputSimd256Register(2));
        }
        break;
    }
    case kX64Pblendvb: {
        VectorLength vec_len = VectorLengthField::decode(opcode);
        if (vec_len == kV128) {
            __ Pblendvb(i.OutputSimd128Register(), i.InputSimd128Register(0), i.InputSimd128Register(1), i.InputSimd128Register(2));
        } else {
            DCHECK_EQ(vec_len, kV256);
            CpuFeatureScope avx_scope(masm(), AVX2);
            __ vpblendvb(i.OutputSimd256Register(), i.InputSimd256Register(0), i.InputSimd256Register(1), i.InputSimd256Register(2));
        }
        break;
    }
    ...
    case kX64I32x4TruncF32x4U: {
        __ I32x4TruncF32x4U(i.OutputSimd128Register(), i.InputSimd128Register(0), kScratchDoubleReg, i.TempSimd128Register(0));
        break;
    }
    case kX64I32x8TruncF32x8U: {
        __ I32x8TruncF32x8U(i.OutputSimd256Register(), i.InputSimd256Register(0), kScratchSimd256Reg, i.TempSimd256Register(0));
        break;
    }
    case kX64Cvttps2dq: {
        VectorLength vec_len = VectorLengthField::decode(opcode);
...
#undef ATOMIC_BINOP_CASE
    ...
    case kX64I64x4ExtMulI32x4S: {
        __ I64x4ExtMul(i.OutputSimd256Register(), i.InputSimd128Register(0), i.InputSimd128Register(1), kScratchSimd256Reg,
            /*is_signed=*/true);
        break;
    }
    ...
    case kX64I32x8ExtMulI16x8S: {
        __ I32x8ExtMul(i.OutputSimd256Register(), i.InputSimd128Register(0), i.InputSimd128Register(1), kScratchSimd256Reg,
            /*is_signed=*/true);
        break;
    }
    ...
    case kX64I16x16ExtMulI8x16S: {
        __ I16x16ExtMul(i.OutputSimd256Register(), i.InputSimd128Register(0), i.InputSimd128Register(1), kScratchSimd256Reg,
            /*is_signed=*/true);
        break;
    }
    ...
    case kX64ExtractF128: {
        ...
        __ vextractf128(i.OutputSimd128Register(), i.InputSimd256Register(0), lane);
        break;
    }
    case kX64InsertI128: {
        CpuFeatureScope avx_scope(masm(), AVX2);
        ...
        if (input0->IsSimd128Register()) {
            __ vinserti128(i.OutputSimd256Register(), YMMRegister::from_xmm(i.InputSimd128Register(0)), i.InputSimd128Register(1), imm);
        } else {
            DCHECK(instr->InputAt(0)->IsSimd256Register());
            __ vinserti128(i.OutputSimd256Register(), i.InputSimd256Register(0), i.InputSimd128Register(1), imm);
        }
        break;
    }
    }
...
void CodeGenerator::AssembleArchDeoptBranch(Instruction* instr, BranchInfo* branch)
{
    Label::Distance flabel_distance = branch->fallthru ? Label::kNear : Label::kFar;
    Label* tlabel = branch->true_label;
    Label* flabel = branch->false_label;
    ...
    if (v8_flags.deopt_every_n_times > 0) {
        if (isolate() != nullptr) {
            ...
            __ movq(rbx, Operand(rbx, WasmTrustedInstanceData::kStressDeoptCounterOffset - 1));
...
void CodeGenerator::AssembleArchJumpRegardlessOfAssemblyOrder(RpoNumber target)
{
    __ jmp(GetLabel(target));
}

#if V8_ENABLE_WEBASSEMBLY
void CodeGenerator::AssembleArchTrap(Instruction* instr, FlagsCondition condition)
{
...
void CodeGenerator::AssembleArchBoolean(Instruction* instr, FlagsCondition condition)
{
    X64OperandConverter i(this, instr);
    Label done;
...
void CodeGenerator::AssembleArchConditionalBranch(Instruction* instr, BranchInfo* branch)
{
    UNREACHABLE();
}

void CodeGenerator::AssembleArchBinarySearchSwitchRange(
    Register input, RpoNumber def_block, std::pair<int32_t, Label*>* begin, std::pair<int32_t, Label*>* end, std::optional<int32_t>& last_cmp_value)
{
    ...
    AssembleArchBinarySearchSwitchRange(input, def_block, middle, end, last_cmp_value);
    masm()->bind(&less_label);
    AssembleArchBinarySearchSwitchRange(input, def_block, begin, middle, last_cmp_value);
}
...
void CodeGenerator::AssembleArchBinarySearchSwitch(Instruction* instr)
{
    ...
    AssembleArchBinarySearchSwitchRange(input, i.InputRpo(1), cases.data(), cases.data() + cases.size(), last_cmp_value);
}
...
void CodeGenerator::AssembleArchSelect(Instruction* instr, FlagsCondition condition)
{
    X64OperandConverter i(this, instr);
    MachineRepresentation rep = LocationOperand::cast(instr->OutputAt(0))->representation();
...
void CodeGenerator::FinishFrame(Frame* frame)
{
    ...
    if (!saves_fp.is_empty()) { // Save callee-saved XMM registers.
        ...
        frame->AllocateSavedCalleeRegisterSlots(saves_fp_count * (kQuadWordSize / kSystemPointerSize));
    }
...
void CodeGenerator::AssembleConstructFrame()
{
...
#if V8_ENABLE_WEBASSEMBLY
            if (call_descriptor->IsWasmFunctionCall() || call_descriptor->IsWasmImportWrapper() || call_descriptor->IsWasmCapiFunction()) {
    ...
    int required_slots = frame()->GetTotalFrameSlotCount() - frame()->GetFixedSlotCount();
    ...
    if (required_slots > 0) {
        ...
        if (info()->IsWasm() && required_slots * kSystemPointerSize > 4 * KB) {
            ...
            if (required_slots * kSystemPointerSize < v8_flags.stack_size * KB) {
                ...
                __ addq(kScratchRegister, Immediate(required_slots * kSystemPointerSize));
            ...
            if (v8_flags.experimental_wasm_growable_stacks) {
                ...
                __ movq(WasmHandleStackOverflowDescriptor::GapRegister(), Immediate(required_slots * kSystemPointerSize));
                ...
                __ addq(WasmHandleStackOverflowDescriptor::FrameBaseRegister(),
                    Immediate(static_cast<int32_t>(call_descriptor->ParameterSlotCount() * kSystemPointerSize + CommonFrameConstants::kFixedFrameSizeAboveFp)));
                __ CallBuiltin(Builtin::kWasmHandleStackOverflow);
                __ PopAll(regs_to_save);
            } else {
                __ near_call(static_cast<intptr_t>(Builtin::kWasmStackOverflow), RelocInfo::WASM_STUB_CALL);
...
void CodeGenerator::AssembleReturn(InstructionOperand* additional_pop_count)
{
...
#if V8_ENABLE_WEBASSEMBLY
    if (call_descriptor->IsWasmFunctionCall() && v8_flags.experimental_wasm_growable_stacks) {
        __ movq(kScratchRegister, MemOperand(rbp, TypedFrameConstants::kFrameTypeOffset));
        __ cmpq(kScratchRegister, Immediate(StackFrame::TypeToMarker(StackFrame::WASM_SEGMENT_START)));
        Label done;
        __ j(not_equal, &done);
        RegList regs_to_save;
        for (auto reg : wasm::kGpReturnRegisters)
            regs_to_save.set(reg);
        __ PushAll(regs_to_save);
        __ PrepareCallCFunction(1);
    ...
    // itself.
    const bool drop_jsargs = parameter_slots != 0 && frame_access_state()->has_frame() && call_descriptor->IsJSFunctionCall();
    if (call_descriptor->IsCFunctionCall()) {
        AssembleDeconstructFrame();
    } else if (frame_access_state()->has_frame()) {
        if (additional_pop_count->IsImmediate() && g.ToConstant(additional_pop_count).ToInt32() == 0) {
    ...
    if (drop_jsargs) {
        ...
        __ leaq(rsp, Operand(rsp, pop_reg, times_system_pointer_size, static_cast<int>(pop_size)));
...
void CodeGenerator::PrepareForDeoptimizationExits(ZoneDeque<DeoptimizationExit*>* exits)
{
}

void CodeGenerator::IncrementStackAccessCounter(InstructionOperand* source, InstructionOperand* destination)
{
    ...
    auto IncrementCounter = [&](ExternalReference counter) { __ incl(__ ExternalReferenceAsOperand(counter)); };
    if (source->IsAnyStackSlot()) {
        IncrementCounter(ExternalReference::address_of_load_from_stack_count(debug_name_));
    }
    if (destination->IsAnyStackSlot()) {
        IncrementCounter(ExternalReference::address_of_store_to_stack_count(debug_name_));
    }
}
...
AllocatedOperand CodeGenerator::Push(InstructionOperand* source)
{
    ...
    X64OperandConverter g(this, nullptr);
    int last_frame_slot_id = frame_access_state_->frame()->GetTotalFrameSlotCount() - 1;
    ...
    } else if (source->IsStackSlot() || source->IsFloatStackSlot() || source->IsDoubleStackSlot()) {
...
void CodeGenerator::Pop(InstructionOperand* dest, MachineRepresentation rep)
{
    ...
    } else if (dest->IsStackSlot() || dest->IsFloatStackSlot() || dest->IsDoubleStackSlot()) {
        frame_access_state()->IncreaseSPDelta(-dropped_slots);
...
void CodeGenerator::MoveToTempLocation(InstructionOperand* source, MachineRepresentation rep)
{
    // Must be kept in sync with {MoveTempLocationTo}.
    DCHECK(!source->IsImmediate());
    if ((IsFloatingPoint(rep) && !move_cycle_.pending_double_scratch_register_use) || (!IsFloatingPoint(rep) && !move_cycle_.pending_scratch_register_use)) {
        // The scratch register for this rep is available.
        int scratch_reg_code = !IsFloatingPoint(rep) ? kScratchRegister.code() : kScratchDoubleReg.code();
...
void CodeGenerator::MoveTempLocationTo(InstructionOperand* dest, MachineRepresentation rep)
{
    if ((IsFloatingPoint(rep) && !move_cycle_.pending_double_scratch_register_use) || (!IsFloatingPoint(rep) && !move_cycle_.pending_scratch_register_use)) {
        int scratch_reg_code = !IsFloatingPoint(rep) ? kScratchRegister.code() : kScratchDoubleReg.code();
        AllocatedOperand scratch(LocationOperand::REGISTER, rep, scratch_reg_code);
...
{
    MoveType::Type move_type = MoveType::InferMove(&move->source(), &move->destination());
    if (move_type == MoveType::kConstantToStack) {
        X64OperandConverter g(this, nullptr);
        Constant src = g.ToConstant(&move->source());
        if (move->destination().IsStackSlot() && (!RelocInfo::IsNoInfo(src.rmode()) || (src.type() != Constant::kInt32 && src.type() != Constant::kInt64))) {
...
bool Is32BitOperand(InstructionOperand* operand)
{
    ...
    return mr == MachineRepresentation::kWord32 || mr == MachineRepresentation::kCompressed || mr == MachineRepresentation::kCompressedPointer;
}
...
void CodeGenerator::AssembleMove(InstructionOperand* source, InstructionOperand* destination)
{
    X64OperandConverter g(this, nullptr);
        ...
        if (source->IsRegister()) {
            ...
            if (rep == MachineRepresentation::kSimd256) {
                ...
                if (source->IsSimd256Register() && destination->IsSimd128Register()) {
                    __ vmovapd(g.ToSimd128Register(destination), g.ToSimd128Register(source));
                } else {
                    __ vmovapd(g.ToSimd256Register(destination), g.ToSimd256Register(source));
                }
            } else {
    ...
    case MoveType::kRegisterToStack: {
        ...
        if (source->IsRegister()) {
            ...
            if (rep == MachineRepresentation::kSimd128) {
                ...
                CpuFeatureScope avx_scope(masm(), AVX);
                // Whether the ymm source should be used as a xmm.
                if (source->IsSimd256Register() && destination->IsSimd128StackSlot()) {
    ...
    case MoveType::kStackToRegister: {
        ...
        if (source->IsStackSlot()) {
            ...
            if (rep == MachineRepresentation::kSimd128) {
                ...
                CpuFeatureScope avx_scope(masm(), AVX);
                if (source->IsSimd256StackSlot() && destination->IsSimd128Register()) {
    ...
    case MoveType::kStackToStack: {
        ...
        if (source->IsStackSlot()) {
            ...
            if (rep == MachineRepresentation::kSimd128) {
                ...
                CpuFeatureScope avx_scope(masm(), AVX);
                if (source->IsSimd256StackSlot() && destination->IsSimd128StackSlot()) {
...
void CodeGenerator::AssembleSwap(InstructionOperand* source, InstructionOperand* destination)
{
    if (v8_flags.trace_turbo_stack_accesses) {
        IncrementStackAccessCounter(source, destination);
