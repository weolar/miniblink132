...
struct MoveKey {
    ...
    bool operator==(const MoveKey& other) const
    {
        return std::tie(this->source, this->destination) == std::tie(other.source, other.destination);
    }
};
...
public:
    ...
    bool ContainsOpOrAlias(const InstructionOperand& op) const
    {
        ...
        if (kFPAliasing == AliasingKind::kCombine && op.IsFPRegister()) {
            ...
            while (aliases--) {
                if (Contains(AllocatedOperand(LocationOperand::REGISTER, other_rep1, base + aliases))) {
                    return true;
                }
            }
            ...
            DCHECK(aliases > 0 || (aliases == 0 && base == -1));
            while (aliases--) {
                if (Contains(AllocatedOperand(LocationOperand::REGISTER, other_rep2, base + aliases))) {
                    return true;
                }
            }
        }
...
void MoveOptimizer::CompressGaps(Instruction* instruction)
{
    ...
    if (i == Instruction::LAST_GAP_POSITION) {
        ...
        CompressMoves(instruction->parallel_moves()[Instruction::FIRST_GAP_POSITION], instruction->parallel_moves()[Instruction::LAST_GAP_POSITION]);
    }
...
const Instruction* MoveOptimizer::LastInstruction(const InstructionBlock* block) const
{
...
void MoveOptimizer::OptimizeMerge(InstructionBlock* block)
{
    ...
    ParallelMove* moves = instr->GetOrCreateParallelMove(static_cast<Instruction::GapPosition>(0), code_zone());
...
bool Is64BitsWide(const InstructionOperand& op)
{
    ...
    return (rep == MachineRepresentation::kTagged || rep == MachineRepresentation::kTaggedPointer || rep == MachineRepresentation::kWord64);
#else
...
bool LoadCompare(const MoveOperands* a, const MoveOperands* b)
{
    ...
    if (a->destination().IsLocationOperand() && b->destination().IsLocationOperand()) {
...
void MoveOptimizer::FinalizeMoves(Instruction* instr)
{
    ...
    for (MoveOperands* load : loads) {
        ...
        ParallelMove* slot_1 = instr->GetOrCreateParallelMove(static_cast<Instruction::GapPosition>(1), code_zone());
