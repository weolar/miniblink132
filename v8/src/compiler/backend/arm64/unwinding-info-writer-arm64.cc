...
void UnwindingInfoWriter::EndInstructionBlock(const InstructionBlock* block)
{
    ...
    for (const RpoNumber& successor : block->successors()) {
        ...
        if (existing_state) {
            ...
            block_initial_states_[successor_index] = zone_->New<BlockInitialState>(saved_lr_);
        }
    }
}
...
void UnwindingInfoWriter::MarkLinkRegisterOnTopOfStack(int pc_offset, const Register& sp)
{
    if (!enabled())
        return;
