...
bool IsBitcast(Node* node)
{
    ...
    // tables.
    return node->opcode() == IrOpcode::kBitcastTaggedToWordForTagAndSmiBits || node->opcode() == IrOpcode::kBitcastWordToTaggedSigned;
}
...
void BitcastElider::VisitNode(Node* node)
{
    for (int i = 0; i < node->InputCount(); i++) {
        ...
        if (input == nullptr)
            continue;
        if (input->opcode() == IrOpcode::kTruncateInt64ToInt32 && OwnedByWord32Op(input)) {
