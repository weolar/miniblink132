...
FrameElider::FrameElider(InstructionSequence* code, bool has_dummy_end_block, bool is_wasm_to_js)
    : code_(code)
...
void FrameElider::MarkBlocks()
{
    for (InstructionBlock* block : instruction_blocks()) {
        ...
        for (int i = block->code_start(); i < block->code_end(); ++i) {
            ...
            if (instr->IsCall() || instr->IsDeoptimizeCall() || instr->arch_opcode() == ArchOpcode::kArchStackPointerGreaterThan
                || instr->arch_opcode() == ArchOpcode::kArchFramePointer) {
                block->mark_needs_frame();
                break;
            }
            if (instr->arch_opcode() == ArchOpcode::kArchStackSlot
                && ((instr->InputAt(0)->IsImmediate() && code_->GetImmediate(ImmediateOperand::cast(instr->InputAt(0))).ToInt32() > 0) || is_wasm_to_js_)) {
