...
{
    return move1->source() == move2->destination() && move2->source() == move1->destination();
}
...
void GapResolver::PerformCycle(const std::vector<MoveOperands*>& cycle)
{
    ...
    MachineRepresentation rep = LocationOperand::cast(move1->destination()).representation();
...
MoveOperands* GapResolver::PerformMoveHelper(ParallelMove* moves, MoveOperands* move, std::vector<MoveOperands*>* cycle)
{
