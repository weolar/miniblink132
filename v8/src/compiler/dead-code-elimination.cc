...
DeadCodeElimination::DeadCodeElimination(Editor* editor, Graph* graph, CommonOperatorBuilder* common, Zone* temp_zone)
...
{
    return node->opcode() == IrOpcode::kDead || node->opcode() == IrOpcode::kUnreachable || node->opcode() == IrOpcode::kDeadValue
...
Reduction DeadCodeElimination::ReduceEffectPhi(Node* node)
{
    ...
    DCHECK(merge->opcode() == IrOpcode::kMerge || merge->opcode() == IrOpcode::kLoop);
...
Reduction DeadCodeElimination::ReduceEffectNode(Node* node)
{
    ...
    if (Node* input = FindDeadInput(node)) {
        ...
        Node* control = node->op()->ControlInputCount() == 1 ? NodeProperties::GetControlInput(node, 0) : graph()->start();
...
Reduction DeadCodeElimination::ReduceDeoptimizeOrReturnOrTerminateOrTailCall(Node* node)
{
    DCHECK(node->opcode() == IrOpcode::kDeoptimize || node->opcode() == IrOpcode::kReturn || node->opcode() == IrOpcode::kTerminate
...
Reduction DeadCodeElimination::ReduceBranchOrSwitch(Node* node)
{
    ...
    if (condition->opcode() == IrOpcode::kDeadValue) {
        ...
        NodeProperties::CollectControlProjections(node, projections, projection_cnt);
