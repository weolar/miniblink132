...
private:
    ...
    inline static Node* NewImpl(Zone* zone, NodeId id, const Operator* op, int input_count, NodePtrT const* inputs, bool has_extensible_inputs);
    ...
    struct Use {
        ...
        ZoneNodePtr* input_ptr()
        {
            ...
            ZoneNodePtr* inputs = is_inline_use() ? reinterpret_cast<Node*>(start)->inline_inputs() : reinterpret_cast<OutOfLineInputs*>(start)->inputs();
            return &inputs[index];
        }
        ...
        Node* from()
        {
            ...
            return is_inline_use() ? reinterpret_cast<Node*>(start) : reinterpret_cast<OutOfLineInputs*>(start)->node_;
        }
    ...
    {
        return has_inline_inputs() ? &(inline_inputs()[input_index]) : &(outline_inputs()->inputs()[input_index]);
    }
    ...
    {
        Use* ptr = has_inline_inputs() ? reinterpret_cast<Use*>(this) : reinterpret_cast<Use*>(outline_inputs());
...
{
    return reinterpret_cast<ZoneNodePtr*>(reinterpret_cast<Address>(this) + sizeof(Node::OutOfLineInputs));
}
...
class NodeWrapper {
public:
    explicit constexpr NodeWrapper(Node* node)
        : node_(node)
    {
    }
    operator Node*() const
...
public:
        ...
        : NodeWrapper(node)
    {
        ...
        SLOW_DCHECK(node == nullptr || node->op()->opcode() == IrOpcode::kEnd || node->op()->EffectOutputCount() > 0);
    }
...
public:
    ...
    {
        // TODO(jgruber): Remove the End special case.
        SLOW_DCHECK(node == nullptr || node->opcode() == IrOpcode::kEnd || node->op()->ControlOutputCount() > 0);
    }
...
Node::InputEdges Node::input_edges()
{
    ...
    if (inline_count != kOutlineMarker) {
        return InputEdges(inline_inputs(), reinterpret_cast<Use*>(this) - 1, inline_count);
    } else {
        return InputEdges(outline_inputs()->inputs(), reinterpret_cast<Use*>(outline_inputs()) - 1, outline_inputs()->count_);
    }
}
...
base::Vector<Node*> Node::inputs_vector() const
{
    ...
    if (inline_count != kOutlineMarker) {
        ...
        return base::VectorOf<Node*>(outline_inputs()->inputs(), outline_inputs()->count_);
    }
}
