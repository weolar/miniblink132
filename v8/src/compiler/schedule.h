...
class V8_EXPORT_PRIVATE BasicBlock final : public NON_EXPORTED_BASE(ZoneObject) {
public:
    ...
    template <class InputIterator> void InsertNodes(iterator insertion_point, InputIterator insertion_start, InputIterator insertion_end)
    {
    ...
    uint64_t pgo_execution_count()
    {
        return pgo_execution_count_;
    }
    void set_pgo_execution_count(uint64_t count)
    {
        pgo_execution_count_ = count;
    }
#endif
