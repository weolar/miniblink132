...
class V8_EXPORT_PRIVATE WasmLoadElimination final : public NON_EXPORTED_BASE(AdvancedReducer) {
public:
...
private:
    struct FieldOrElementValue {
        ...
        explicit FieldOrElementValue(Node* value)
            : value(value)
        {
        }
    ...
    public:
        ...
        bool IsEmpty() const
        {
            return fields_.begin() == fields_.end() && elements_.begin() == elements_.end();
        }
    ...
    private:
        ...
        template <typename OuterKey> static void Update(OuterMap<OuterKey>& map, OuterKey outer_key, Node* inner_key, FieldOrElementValue info)
        {
