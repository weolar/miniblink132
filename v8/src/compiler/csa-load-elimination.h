...
class V8_EXPORT_PRIVATE CsaLoadElimination final : public NON_EXPORTED_BASE(AdvancedReducer) {
public:
...
private:
    ...
    public:
        ...
        {
            return fresh_entries_ == that->fresh_entries_ && constant_entries_ == that->constant_entries_ && arbitrary_entries_ == that->arbitrary_entries_
                && fresh_unknown_entries_ == that->fresh_unknown_entries_ && constant_unknown_entries_ == that->constant_unknown_entries_
    ...
    private:
        ...
        template <typename OuterKey> using OuterMap = PersistentMap<OuterKey, InnerMap>;
        // offset -> object -> info
        ...
        template <typename OuterKey> static void Update(OuterMap<OuterKey>& map, OuterKey outer_key, Node* inner_key, FieldInfo info)
        {
        ...
        template <typename OuterKey> static void IntersectWith(OuterMap<OuterKey>& to, const OuterMap<OuterKey>& from);
    ...
    struct AbstractState : public ZoneObject {
        ...
        {
            return this->immutable_state.Equals(&that->immutable_state) && this->mutable_state.Equals(&that->mutable_state);
        }
