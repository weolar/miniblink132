...
void AddTypeAssertionsImpl::ProcessBlock(BasicBlock* block)
{
    ...
    for (Node* node : *block) {
        ...
        if (node->op()->EffectOutputCount() == 1 && node->op()->EffectInputCount() == 1) {
            for (Node* pending_node : pending) {
                InsertAssertion(pending_node, node);
            }
            pending.clear();
        }
        if (node->opcode() == IrOpcode::kAssertType || node->opcode() == IrOpcode::kAllocate || node->opcode() == IrOpcode::kObjectState
            || node->opcode() == IrOpcode::kObjectId || node->opcode() == IrOpcode::kPhi || !NodeProperties::IsTyped(node)
...
void AddTypeAssertionsImpl::InsertAssertion(Node* asserted, Node* effect_successor)
{
    Node* assertion = graph->NewNode(simplified->AssertType(NodeProperties::GetType(asserted)), asserted, NodeProperties::GetEffectInput(effect_successor));
