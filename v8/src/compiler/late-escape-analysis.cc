...
LateEscapeAnalysis::LateEscapeAnalysis(Editor* editor, Graph* graph, CommonOperatorBuilder* common, Zone* zone)
...
bool LateEscapeAnalysis::IsEscaping(Node* node)
{
    ...
    if (escaping == escaping_allocations_.end())
        return false;
...
void LateEscapeAnalysis::RemoveAllocation(Node* node)
{
    ...
    for (Edge edge : node->use_edges()) {
        ...
        if (std::optional<Node*> stored_value = TryGetStoredValue(use);
            stored_value.has_value() && stored_value.value()->opcode() == IrOpcode::kAllocateRaw && stored_value.value() != node) {
