...
TypedOptimization::TypedOptimization(Editor* editor, CompilationDependencies* dependencies, JSGraph* jsgraph, JSHeapBroker* broker)
    : AdvancedReducer(editor)
    , dependencies_(dependencies)
    , jsgraph_(jsgraph)
    , broker_(broker)
    , true_type_(Type::Constant(broker, broker->true_value(), graph()->zone()))
    , false_type_(Type::Constant(broker, broker->false_value(), graph()->zone()))
...
OptionalMapRef GetStableMapFromObjectType(JSHeapBroker* broker, Type object_type)
{
...
Reduction TypedOptimization::ReduceMaybeGrowFastElements(Node* node)
{
    ...
    if (!index_type.IsNone() && !length_type.IsNone() && index_type.Max() < length_type.Min()) {
        if (v8_flags.turbo_typer_hardening) {
            Node* check_bounds
                = graph()->NewNode(simplified()->CheckBounds(FeedbackSource {}, CheckBoundsFlag::kAbortOnOutOfBounds), index, length, effect, control);
...
Reduction TypedOptimization::ReduceCheckBounds(Node* node)
{
    ...
    Node* const input = NodeProperties::GetValueInput(node, 0);
    Type const input_type = NodeProperties::GetType(input);
    if (p.flags() & CheckBoundsFlag::kConvertStringAndMinusZero && !input_type.Maybe(Type::String()) && !input_type.Maybe(Type::MinusZero())) {
        NodeProperties::ChangeOp(
            node, simplified()->CheckBounds(p.check_parameters().feedback(), p.flags().without(CheckBoundsFlag::kConvertStringAndMinusZero)));
        return Changed(node);
    }
    return NoChange();
}
...
Reduction TypedOptimization::ReduceNumberFloor(Node* node)
{
    ...
    if (input_type.Is(Type::PlainNumber()) && (input->opcode() == IrOpcode::kNumberDivide || input->opcode() == IrOpcode::kSpeculativeNumberDivide)) {
        ...
        if (lhs_type.IsNone() || rhs_type.IsNone())
            return NoChange();
...
Reduction TypedOptimization::ReducePhi(Node* node)
{
    ...
    for (int i = 1; i < arity; ++i) {
        type = Type::Union(type, NodeProperties::GetType(node->InputAt(i)), graph()->zone());
    }
...
Reduction TypedOptimization::ReduceReferenceEqual(Node* node)
{
    ...
    if (rhs_type.Is(Type::Boolean()) && rhs_type.IsHeapConstant() && lhs_type.Is(Type::Boolean())) {
        std::optional<bool> maybe_result = rhs_type.AsHeapConstant()->Ref().TryGetBooleanValue(broker());
...
Reduction TypedOptimization::TryReduceStringComparisonOfStringFromSingleCharCodeToConstant(Node* comparison, StringRef string, bool inverted)
{
...
Reduction TypedOptimization::TryReduceStringComparisonOfStringFromSingleCharCode(Node* comparison, Node* from_char_code, Type constant_type, bool inverted)
{
    ...
    Reduction red = TryReduceStringComparisonOfStringFromSingleCharCodeToConstant(comparison, string, inverted);
    if (red.Changed())
        return red;
    ...
    if (!from_char_code_repl_type.Is(type_cache_->kUint16)) {
        ...
        from_char_code_repl
            = graph()->NewNode(simplified()->NumberBitwiseAnd(), from_char_code_repl, jsgraph()->ConstantNoHole(std::numeric_limits<uint16_t>::max()));
    }
    if (!string.GetFirstChar(broker()).has_value())
        return NoChange();
    Node* constant_repl = jsgraph()->ConstantNoHole(string.GetFirstChar(broker()).value());
    ...
    if (lhs->opcode() == IrOpcode::kStringFromSingleCharCode) {
        if (rhs->opcode() == IrOpcode::kStringFromSingleCharCode) {
            ...
            if (!left_type.Is(type_cache_->kUint16)) {
                ...
                left = graph()->NewNode(simplified()->NumberBitwiseAnd(), left, jsgraph()->ConstantNoHole(std::numeric_limits<uint16_t>::max()));
            }
            if (!right_type.Is(type_cache_->kUint16)) {
                // Convert to signed int32 to satisfy type of {NumberBitwiseAnd}.
                right = graph()->NewNode(simplified()->NumberToInt32(), right);
                right = graph()->NewNode(simplified()->NumberBitwiseAnd(), right, jsgraph()->ConstantNoHole(std::numeric_limits<uint16_t>::max()));
            }
            Node* equal = graph()->NewNode(NumberComparisonFor(node->op()), left, right);
            ReplaceWithValue(node, equal);
            return Replace(equal);
        } else {
            return TryReduceStringComparisonOfStringFromSingleCharCode(node, lhs, rhs_type, false);
        }
    } else if (rhs->opcode() == IrOpcode::kStringFromSingleCharCode) {
        return TryReduceStringComparisonOfStringFromSingleCharCode(node, rhs, lhs_type, true);
    }
    return NoChange();
}
...
Reduction TypedOptimization::ReduceTypeOf(Node* node)
{
    ...
    if (type.Is(Type::Boolean())) {
        return Replace(jsgraph()->ConstantNoHole(broker()->boolean_string(), broker()));
    } else if (type.Is(Type::Number())) {
        return Replace(jsgraph()->ConstantNoHole(broker()->number_string(), broker()));
    } else if (type.Is(Type::String())) {
        return Replace(jsgraph()->ConstantNoHole(broker()->string_string(), broker()));
    } else if (type.Is(Type::BigInt())) {
        return Replace(jsgraph()->ConstantNoHole(broker()->bigint_string(), broker()));
    } else if (type.Is(Type::Symbol())) {
        return Replace(jsgraph()->ConstantNoHole(broker()->symbol_string(), broker()));
    } else if (type.Is(Type::OtherUndetectableOrUndefined())) {
        return Replace(jsgraph()->ConstantNoHole(broker()->undefined_string(), broker()));
    } else if (type.Is(Type::NonCallableOrNull())) {
        return Replace(jsgraph()->ConstantNoHole(broker()->object_string(), broker()));
    } else if (type.Is(Type::Function())) {
        return Replace(jsgraph()->ConstantNoHole(broker()->function_string(), broker()));
    }
    return NoChange();
}
...
Reduction TypedOptimization::ReduceToBoolean(Node* node)
{
    ...
    if (input_type.Is(Type::Boolean())) {
        ...
        node->ReplaceInput(0, graph()->NewNode(simplified()->ObjectIsUndetectable(), input));
        node->TrimInputCount(1);
        NodeProperties::ChangeOp(node, simplified()->BooleanNot());
        return Changed(node);
    } else if (input_type.Is(Type::String())) {
        // ToBoolean(x:string) => BooleanNot(ReferenceEqual(x,""))
        node->ReplaceInput(0, graph()->NewNode(simplified()->ReferenceEqual(), input, jsgraph()->EmptyStringConstant()));
        node->TrimInputCount(1);
        NodeProperties::ChangeOp(node, simplified()->BooleanNot());
        return Changed(node);
    }
...
const Operator* NumberOpFromSpeculativeNumberOp(SimplifiedOperatorBuilder* simplified, const Operator* op)
{
    switch (op->opcode()) {
    case IrOpcode::kSpeculativeNumberEqual:
        return simplified->NumberEqual();
...
Reduction TypedOptimization::ReduceSpeculativeNumberAdd(Node* node)
{
    ...
    if ((hint == NumberOperationHint::kNumber || hint == NumberOperationHint::kNumberOrOddball) && BothAre(lhs_type, rhs_type, Type::PlainPrimitive())
...
Reduction TypedOptimization::ReduceSpeculativeNumberBinop(Node* node)
{
    ...
    Type const rhs_type = NodeProperties::GetType(rhs);
    NumberOperationHint hint = NumberOperationHintOf(node->op());
    if ((hint == NumberOperationHint::kNumber || hint == NumberOperationHint::kNumberOrOddball) && BothAre(lhs_type, rhs_type, Type::NumberOrOddball())) {
        ...
        Node* const toNum_lhs = ConvertPlainPrimitiveToNumber(lhs);
        Node* const toNum_rhs = ConvertPlainPrimitiveToNumber(rhs);
        Node* const value = graph()->NewNode(NumberOpFromSpeculativeNumberOp(simplified(), node->op()), toNum_lhs, toNum_rhs);
        ReplaceWithValue(node, value);
        return Replace(value);
    }
    return NoChange();
...
Reduction TypedOptimization::ReduceSpeculativeNumberComparison(Node* node)
{
    ...
    if (BothAre(lhs_type, rhs_type, Type::Signed32()) || BothAre(lhs_type, rhs_type, Type::Unsigned32())) {
        Node* const value = graph()->NewNode(NumberOpFromSpeculativeNumberOp(simplified(), node->op()), lhs, rhs);
        ReplaceWithValue(node, value);
        return Replace(value);
    }
    return NoChange();
