...
#define DEFINE_IS_OPCODE(Opcode, ...)                                                                                                                          \
    ...
    {                                                                                                                                                          \
        return opcode() == IrOpcode::k##Opcode;                                                                                                                \
    }
...
template <typename T, IrOpcode::Value kOpcode> struct ValueMatcher : public NodeMatcher {
...
template <>
inline ValueMatcher<uint32_t, IrOpcode::kInt32Constant>::ValueMatcher(Node* node)
    : NodeMatcher(node)
    , resolved_value_()
    , has_resolved_value_(false)
{
...
template <>
inline ValueMatcher<uint64_t, IrOpcode::kInt64Constant>::ValueMatcher(Node* node)
    : NodeMatcher(node)
    , resolved_value_()
    , has_resolved_value_(false)
{
...
template <typename T, IrOpcode::Value kOpcode> struct IntMatcher final : public ValueMatcher<T, kOpcode> {
    ...
    {
        return this->HasResolvedValue() && base::IsInRange(this->ResolvedValue(), low, high);
    }
    ...
    {
        return this->HasResolvedValue() && this->ResolvedValue() > 0 && (this->ResolvedValue() & (this->ResolvedValue() - 1)) == 0;
    }
    bool IsNegativePowerOf2() const
    {
        return this->HasResolvedValue() && this->ResolvedValue() < 0
            && ((this->ResolvedValue() == std::numeric_limits<T>::min()) || (-this->ResolvedValue() & (-this->ResolvedValue() - 1)) == 0);
    }
...
using V128ConstMatcher = ValueMatcher<S128ImmediateParameter, IrOpcode::kS128Const>;
...
template <typename T, IrOpcode::Value kOpcode> struct FloatMatcher final : public ValueMatcher<T, kOpcode> {
    ...
    {
        return this->HasResolvedValue() && std::nearbyint(this->ResolvedValue()) == this->ResolvedValue();
    }
...
template <IrOpcode::Value kHeapConstantOpcode> struct HeapObjectMatcherImpl final : public ValueMatcher<IndirectHandle<HeapObject>, kHeapConstantOpcode> {
    ...
    {
        return this->HasResolvedValue() && this->ResolvedValue().address() == value.address();
    }
...
using CompressedHeapObjectMatcher = HeapObjectMatcherImpl<IrOpcode::kCompressedHeapConstant>;

// A pattern matcher for external reference constants.
struct ExternalReferenceMatcher final : public ValueMatcher<ExternalReference, IrOpcode::kExternalConstant> {
...
template <typename Object> struct LoadMatcher : public NodeMatcher {
...
template <typename Left, typename Right, MachineRepresentation rep> struct BinopMatcher : public NodeMatcher {
...
using Int32BinopMatcher = BinopMatcher<Int32Matcher, Int32Matcher, MachineRepresentation::kWord32>;
using Uint32BinopMatcher = BinopMatcher<Uint32Matcher, Uint32Matcher, MachineRepresentation::kWord32>;
using Int64BinopMatcher = BinopMatcher<Int64Matcher, Int64Matcher, MachineRepresentation::kWord64>;
using Uint64BinopMatcher = BinopMatcher<Uint64Matcher, Uint64Matcher, MachineRepresentation::kWord64>;
using IntPtrBinopMatcher = BinopMatcher<IntPtrMatcher, IntPtrMatcher, MachineType::PointerRepresentation()>;
using UintPtrBinopMatcher = BinopMatcher<UintPtrMatcher, UintPtrMatcher, MachineType::PointerRepresentation()>;
using Float32BinopMatcher = BinopMatcher<Float32Matcher, Float32Matcher, MachineRepresentation::kFloat32>;
using Float64BinopMatcher = BinopMatcher<Float64Matcher, Float64Matcher, MachineRepresentation::kFloat64>;
using NumberBinopMatcher = BinopMatcher<NumberMatcher, NumberMatcher, MachineRepresentation::kTagged>;
using HeapObjectBinopMatcher = BinopMatcher<HeapObjectMatcher, HeapObjectMatcher, MachineRepresentation::kTagged>;
using CompressedHeapObjectBinopMatcher = BinopMatcher<CompressedHeapObjectMatcher, CompressedHeapObjectMatcher, MachineRepresentation::kCompressed>;

template <class BinopMatcher, IrOpcode::Value kMulOpcode, IrOpcode::Value kShiftOpcode> struct ScaleMatcher {
        ...
        , power_of_two_plus_one_(false)
    {
        ...
        if (node->opcode() == kShiftOpcode) {
            if (m.right().HasResolvedValue()) {
                typename BinopMatcher::RightMatcher::ValueType value = m.right().ResolvedValue();
                if (value >= 0 && value <= 3) {
...
private:
    void Initialize(Node* node, bool allow_input_swap)
    {
        ...
        if ((this->left().opcode() != kSubOpcode && this->left().opcode() != kAddOpcode)
            && (this->right().opcode() == kAddOpcode || this->right().opcode() == kSubOpcode)) {
            this->SwapInputs();
        }
    }
...
using Int32AddMatcher = AddMatcher<Int32BinopMatcher, IrOpcode::kInt32Add, IrOpcode::kInt32Sub, IrOpcode::kInt32Mul, IrOpcode::kWord32Shl>;
using Int64AddMatcher = AddMatcher<Int64BinopMatcher, IrOpcode::kInt64Add, IrOpcode::kInt64Sub, IrOpcode::kInt64Mul, IrOpcode::kWord64Shl>;
...
template <class AddMatcher> struct BaseWithIndexAndDisplacementMatcher {
    ...
    {
        Initialize(
            node, AddressOption::kAllowScale | (node->op()->HasProperty(Operator::kCommutative) ? AddressOption::kAllowInputSwap : AddressOption::kAllowNone));
    }
        ...
        if (m.HasIndexInput() && OwnedByAddressingOperand(left)) {
            ...
            if (left->opcode() == AddMatcher::kSubOpcode && OwnedByAddressingOperand(left)) {
                ...
                if (left_matcher.right().HasResolvedValue()) {
                    if (left_matcher.HasIndexInput() && OwnedByAddressingOperand(left_left)) {
                        // ((S - D) + B)
...
struct V8_EXPORT_PRIVATE DiamondMatcher : public NON_EXPORTED_BASE(NodeMatcher) {
...
#if V8_ENABLE_WEBASSEMBLY
struct LoadTransformMatcher : ValueMatcher<LoadTransformParameters, IrOpcode::kLoadTransform> {
    explicit LoadTransformMatcher(Node* node)
        : ValueMatcher(node)
    {
    }
