...
protected:
    ...
    {
        V8FileLogger::CallEventLogger(isolate, histogram_->name(), v8::LogEventStatus::kStart, true);
    }

    V8_INLINE void LogEnd(Isolate* isolate)
    {
        V8FileLogger::CallEventLogger(isolate, histogram_->name(), v8::LogEventStatus::kEnd, true);
    }
...
public:
    explicit TimedHistogramScope(TimedHistogram* histogram, Isolate* isolate = nullptr, int64_t* result_in_microseconds = nullptr)
...
class V8_NODISCARD OptionalTimedHistogramScope : public BaseTimedHistogramScope {
public:
...
public:
    explicit NestedTimedHistogramScope(NestedTimedHistogram* histogram, Isolate* isolate = nullptr)
        : BaseTimedHistogramScope(histogram)
        , isolate_(isolate)
    {
        Start();
...
private:
    ...
    void RecordLongTaskTime(base::TimeDelta elapsed) const
    {
        if (histogram_ == isolate_->counters()->execute()) {
            isolate_->GetCurrentLongTaskStats()->v8_execute_us += elapsed.InMicroseconds();
        }
    }
