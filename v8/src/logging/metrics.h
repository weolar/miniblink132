...
public:
    ...
    template <class T> void AddMainThreadEvent(const T& event, v8::metrics::Recorder::ContextId id)
    {
        if (embedder_recorder_)
            embedder_recorder_->AddMainThreadEvent(event, id);
    }

    template <class T> void DelayMainThreadEvent(const T& event, v8::metrics::Recorder::ContextId id)
    {
        if (!embedder_recorder_)
            return;
    ...
    template <class T> void AddThreadSafeEvent(const T& event)
    {
        if (embedder_recorder_)
            embedder_recorder_->AddThreadSafeEvent(event);
    }
...
private:
    ...
    template <class T> class DelayedEvent : public DelayedEventBase {
    public:
...
template <class T, int64_t (base::TimeDelta::*precision)() const = &base::TimeDelta::InMicroseconds> class V8_NODISCARD TimedScope {
public:
    explicit TimedScope(T* event)
        : event_(event)
    {
        Start();
    }
