...
#define FOR_EACH_THREAD_SPECIFIC_COUNTER(V)                                                                                                                    \
    ...
    ADD_THREAD_SPECIFIC_COUNTER(V, Optimize, ScheduledEffectControlLinearization)                                                                              \
    ...
    ADD_THREAD_SPECIFIC_COUNTER(V, Optimize, TurboshaftCodeEliminationAndSimplification)                                                                       \
    ...
    ADD_THREAD_SPECIFIC_COUNTER(V, Optimize, TurboshaftCsaEarlyMachineOptimization)                                                                            \
    ...
    ADD_THREAD_SPECIFIC_COUNTER(V, Optimize, TurboshaftDecompressionOptimization)                                                                              \
...
public:
    ...
    V8_EXPORT_PRIVATE void CorrectCurrentCounterId(RuntimeCallCounterId counter_id, CounterMode mode = kExact);
    ...
    V8_EXPORT_PRIVATE bool IsBackgroundThreadSpecificVariant(RuntimeCallCounterId id);
    V8_EXPORT_PRIVATE bool HasThreadSpecificCounterVariants(RuntimeCallCounterId id);
    ...
    RuntimeCallCounterId CounterIdForThread(RuntimeCallCounterId id)
    {
        ...
        return thread_type_ == kWorkerThread ? static_cast<RuntimeCallCounterId>(id + 1) : id;
    }
    ...
    bool IsCounterAppropriateForThread(RuntimeCallCounterId id)
    {
        ...
        // the correct thread.
        if (!HasThreadSpecificCounterVariants(id))
            return true;
        return IsBackgroundThreadSpecificVariant(id) == (thread_type_ == kWorkerThread);
    }
...
public:
    ...
    explicit WorkerThreadRuntimeCallStatsScope(WorkerThreadRuntimeCallStats* off_thread_stats);
    ~WorkerThreadRuntimeCallStatsScope();

    WorkerThreadRuntimeCallStatsScope(WorkerThreadRuntimeCallStatsScope&&) = delete;
    WorkerThreadRuntimeCallStatsScope(const WorkerThreadRuntimeCallStatsScope&) = delete;

    RuntimeCallStats* Get() const
    {
        return table_;
    }
...
#define TRACE_HANDLER_STATS(isolate, counter_name)                                                                                                             \
    CHANGE_CURRENT_RUNTIME_COUNTER(isolate->counters()->runtime_call_stats(), RuntimeCallCounterId::kHandler_##counter_name)

// A RuntimeCallTimerScopes wraps around a RuntimeCallTimer to measure the
// the time of C++ scope.
class V8_NODISCARD RuntimeCallTimerScope {
public:
    inline RuntimeCallTimerScope(Isolate* isolate, RuntimeCallCounterId counter_id);
    inline RuntimeCallTimerScope(
        LocalIsolate* isolate, RuntimeCallCounterId counter_id, RuntimeCallStats::CounterMode mode = RuntimeCallStats::CounterMode::kExact);
    inline RuntimeCallTimerScope(
        RuntimeCallStats* stats, RuntimeCallCounterId counter_id, RuntimeCallStats::CounterMode mode = RuntimeCallStats::CounterMode::kExact)
    {
