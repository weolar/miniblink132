...
#define RCS_SCOPE(...) v8::internal::RuntimeCallTimerScope CONCAT(rcs_timer_scope, __LINE__)(__VA_ARGS__)

RuntimeCallTimerScope::RuntimeCallTimerScope(Isolate* isolate, RuntimeCallCounterId counter_id)
{
...
RuntimeCallTimerScope::RuntimeCallTimerScope(LocalIsolate* isolate, RuntimeCallCounterId counter_id, RuntimeCallStats::CounterMode mode)
{
    if (V8_LIKELY(!TracingFlags::is_runtime_stats_enabled()))
        return;
