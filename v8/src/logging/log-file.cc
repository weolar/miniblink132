...
void LogFile::WriteLogHeader()
{
    ...
    msg << "v8-version" << kNext << Version::GetMajor() << kNext << Version::GetMinor() << kNext << Version::GetBuild() << kNext << Version::GetPatch();
...
std::unique_ptr<LogFile::MessageBuilder> LogFile::NewMessageBuilder()
{
    ...
    std::unique_ptr<LogFile::MessageBuilder> result(new LogFile::MessageBuilder(this));
...
void LogFile::MessageBuilder::AppendString(Tagged<String> str, std::optional<int> length_limit)
{
    if (str.is_null())
        return;
...
{
    base::Vector<char> buf(log_->format_buffer_.get(), LogFile::kMessageBufferSize);
    int length = base::VSNPrintF(buf, format, args);
    // |length| is -1 if output was truncated.
    if (length == -1)
        length = LogFile::kMessageBufferSize;
...
template <> LogFile::MessageBuilder& LogFile::MessageBuilder::operator<< <const char*>(const char* string)
{
    this->AppendString(string);
    return *this;
}

template <> LogFile::MessageBuilder& LogFile::MessageBuilder::operator<< <void*>(void* pointer)
{
    OFStream& os = log_->os_;
...
template <> LogFile::MessageBuilder& LogFile::MessageBuilder::operator<< <char>(char c)
{
    this->AppendCharacter(c);
    return *this;
}

template <> LogFile::MessageBuilder& LogFile::MessageBuilder::operator<< <Tagged<String>>(Tagged<String> string)
{
    this->AppendString(string);
    return *this;
}

template <> LogFile::MessageBuilder& LogFile::MessageBuilder::operator<< <Tagged<Symbol>>(Tagged<Symbol> symbol)
{
    this->AppendSymbolName(symbol);
    return *this;
}

template <> LogFile::MessageBuilder& LogFile::MessageBuilder::operator<< <Tagged<Name>>(Tagged<Name> name)
{
    if (IsString(name)) {
...
template <> LogFile::MessageBuilder& LogFile::MessageBuilder::operator<< <LogSeparator>(LogSeparator separator)
{
