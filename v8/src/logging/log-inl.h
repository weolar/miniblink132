...
LogEventListener::CodeTag V8FileLogger::ToNativeByScript(LogEventListener::CodeTag tag, Tagged<Script> script)
{
    if (script->type() != Script::Type::kNative)
        return tag;
    switch (tag) {
...
{
    V8FileLogger::CallEventLogger(isolate_, TimerEvent::name(), se, TimerEvent::expose_to_api());
}
