...
public:
    ...
    {
        return lookup_function_ != nullptr;
    }
...
template <typename Histogram> class AggregatedMemoryHistogram {
public:
    // Note: public for testing purposes only.
...
template <typename Histogram> void AggregatedMemoryHistogram<Histogram>::AddSample(double current_ms, double current_value)
{
    if (!is_initialized_) {
        ...
        if (current_ms < last_ms_ + kEpsilon) {
            ...
            aggregate_value_ = current_ms > start_ms_ + kEpsilon ? Aggregate(current_ms, current_value) : aggregate_value_;
            last_value_ = current_value;
            last_ms_ = current_ms;
        }
    }
...
template <typename Histogram> double AggregatedMemoryHistogram<Histogram>::Aggregate(double current_ms, double current_value)
{
...
#define RATE_ID(name, caption, max, res) k_##name,
        NESTED_TIMED_HISTOGRAM_LIST(RATE_ID) NESTED_TIMED_HISTOGRAM_LIST_SLOW(RATE_ID) TIMED_HISTOGRAM_LIST(RATE_ID)
#undef RATE_ID
...
#define COUNTER_ID(name, caption) k_##name,
                        STATS_COUNTER_LIST(COUNTER_ID) STATS_COUNTER_NATIVE_CODE_LIST(COUNTER_ID)
#undef COUNTER_ID
#define COUNTER_ID(name) kCountOf##name, kSizeOf##name,
                            INSTANCE_TYPE_LIST(COUNTER_ID)
#undef COUNTER_ID
#define COUNTER_ID(name) kCountOfCODE_TYPE_##name, kSizeOfCODE_TYPE_##name,
...
public:
    explicit CountersVisitor(Counters* counters)
        : counters_(counters)
    {
    }
