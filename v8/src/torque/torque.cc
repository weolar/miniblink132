...
int WrappedMain(int argc, const char** argv)
{
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (argument == "-o") {
            ...
            if (options.strip_v8_root && argument.substr(0, options.v8_root.size()) == options.v8_root) {
            ...
            if (!StringEndsWith(files.back(), ".tq")) {
                std::cerr << "Unexpected command-line argument \"" << files.back() << "\", expected a .tq file.\n";
                base::OS::Abort();
            }
        }
    }
