...
protected:
    ...
    std::string DefinitionToVariable(const DefinitionLocation& location)
    {
        if (location.IsPhi()) {
            ...
            stream << "phi_bb" << location.GetPhiBlock()->id() << "_" << location.GetPhiIndex();
            return stream.str();
            ...
            if (it == location_map_.end()) {
                it = location_map_.insert(std::make_pair(location, FreshNodeName())).first;
            }
            return it->second;
        }
    }
    ...
    {
        return "label" + std::to_string(fresh_id_++);
    }
    ...
    template <typename T> void EmitIRAnnotation(const T& instruction, Stack<std::string>* stack)
    {
        out() << "    // " << instruction << ", starting stack size: " << stack->Size() << "\n";
    }

#define EMIT_INSTRUCTION_DECLARATION(T) void EmitInstruction(const T& instruction, Stack<std::string>* stack);
    TORQUE_BACKEND_AGNOSTIC_INSTRUCTION_LIST(EMIT_INSTRUCTION_DECLARATION)
#undef EMIT_INSTRUCTION_DECLARATION

#define EMIT_INSTRUCTION_DECLARATION(T) virtual void EmitInstruction(const T& instruction, Stack<std::string>* stack) = 0;
