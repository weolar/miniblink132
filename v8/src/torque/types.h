...
public:
    ...
    {
        return IsStructType() || IsClassType();
    }
...
template <typename T> struct SpecializationKey {
    T* generic;
...
public:
    ...
    {
        aliases_.insert(std::move(alias));
    }
    size_t id() const
    {
        return id_;
...
private:
    friend class TypeOracle;
    AbstractType(const Type* parent, AbstractTypeFlags flags, const std::string& name, const std::string& generated_type, const Type* non_constexpr_version,
    ...
    {
        if (parent)
            DCHECK_EQ(parent->IsConstexpr(), IsConstexpr());
...
public:
    ...
    void Extend(const Type* t)
    {
        if (const UnionType* union_type = UnionType::DynamicCast(t)) {
            ...
            EraseIf(&types_, [&](const Type* member) { return member->IsSubtypeOf(t); });
            types_.insert(t);
        }
    }
    std::string ToExplicitString() const override;
...
public:
    ...
    {
        for (std::optional<const AggregateType*> current = this; current.has_value(); current = (*current)->parent()->AggregateSupertype()) {
...
inline std::optional<ObjectSlotKind> Combine(ObjectSlotKind a, ObjectSlotKind b)
{
    if (a == b)
        return { a };
    if (std::min(a, b) == ObjectSlotKind::kStrongPointer && std::max(a, b) == ObjectSlotKind::kMaybeObjectPointer) {
...
public:
    ...
    bool IsExtern() const
    {
        return flags_ & ClassFlag::kExtern;
    }
    bool ShouldGeneratePrint() const
    {
        if (flags_ & ClassFlag::kCppObjectDefinition)
            return false;
        if (flags_ & ClassFlag::kCppObjectLayoutDefinition)
            return false;
        if (!IsExtern())
            return true;
        if (!ShouldGenerateCppClassDefinitions())
            return false;
    ...
    bool ShouldGenerateBodyDescriptor() const
    {
        ...
        if (flags_ & ClassFlag::kCppObjectLayoutDefinition)
            return false;
        if (flags_ & ClassFlag::kGenerateBodyDescriptor)
            return true;
    ...
    bool IsTransient() const override
    {
        return flags_ & ClassFlag::kTransient;
    }
    bool IsAbstract() const
    {
        return flags_ & ClassFlag::kAbstract;
    }
    ...
    {
        return flags_ & ClassFlag::kCppObjectLayoutDefinition && flags_ & ClassFlag::kGenerateCppClassDefinitions;
    }
    ...
    {
        return (flags_ & ClassFlag::kGenerateUniqueMap) || (!IsExtern() && !IsAbstract());
    }
    bool ShouldGenerateFactoryFunction() const
    {
        return (flags_ & ClassFlag::kGenerateFactoryFunction) || (ShouldExport() && !IsAbstract());
    }
    bool ShouldExport() const
    {
        return flags_ & ClassFlag::kExport;
    }
    bool IsShape() const
    {
        return flags_ & ClassFlag::kIsShape;
    }
...
private:
    ...
    ClassType(const Type* parent, Namespace* nspace, const std::string& name, ClassFlags flags, const std::string& generates, const ClassDeclaration* decl,
...
template <bool success = false> std::ostream& operator<<(std::ostream& os, const Type* t)
{
...
public:
    ...
    {
        return stack_range_ != std::nullopt;
    }
...
struct Signature {
    Signature(
        NameVector n, std::optional<std::string> arguments_variable, ParameterTypes p, size_t i, const Type* r, LabelDeclarationVector l, bool transitioning)
    ...
    {
        return types().size() - implicit_count;
    }
    ...
    {
        return TypeVector(parameter_types.types.begin(), parameter_types.types.begin() + implicit_count);
    }
    TypeVector GetExplicitTypes() const
    {
        return TypeVector(parameter_types.types.begin() + implicit_count, parameter_types.types.end());
    }
    bool HasContextParameter() const;
};
...
TypeVector LowerParameterTypes(const ParameterTypes& parameter_types, size_t vararg_count = 0);
