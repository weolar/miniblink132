...
void Function::PrintDeclarationHeader(std::ostream& stream, int indentation) const
{
...
void Function::PrintDefinition(std::ostream& stream, const std::function<void(std::ostream&)>& builder, int indentation) const
{
...
void Function::PrintInlineDefinition(std::ostream& stream, const std::function<void(std::ostream&)>& builder, int indentation) const
{
    PrintDeclarationHeader(stream, indentation);
    stream << " {\n";
    if (builder) {
...
void Function::PrintBeginDefinition(std::ostream& stream, int indentation) const
{
    stream << std::string(indentation, ' ') << "// " << pos_ << "\n";
    std::string scope;
    if (owning_class_) {
        scope = owning_class_->GetName();
        const auto class_template_parameters = owning_class_->GetTemplateParameters();
    ...
    stream << std::string(indentation, ' ') << return_type_ << " " << scope << name_ << "(";
    bool first = true;
    for (const auto& p : parameters_) {
        if (!first)
            stream << ", ";
