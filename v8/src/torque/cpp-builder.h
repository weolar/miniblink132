...
#define ACCESSOR(name, value)                                                                                                                                  \
    void Set##name(bool v = true)                                                                                                                              \
    {                                                                                                                                                          \
        SetFlag(k##name, v);                                                                                                                                   \
    }                                                                                                                                                          \
    bool Is##name() const                                                                                                                                      \
    {                                                                                                                                                          \
        return HasFlag(k##name);                                                                                                                               \
    }
...
#undef ACCESSOR
    ...
    {
        parameters_.emplace_back(std::move(type), std::move(name), std::move(default_value));
    }
    void InsertParameter(int index, std::string type, std::string name = {}, std::string default_value = {})
    {
        DCHECK_GE(index, 0);
        DCHECK_LE(index, parameters_.size());
        parameters_.insert(parameters_.begin() + index, Parameter(std::move(type), std::move(name), std::move(default_value)));
    }
    ...
    std::vector<std::string> GetParameterNames() const
    {
        ...
        std::transform(parameters_.begin(), parameters_.end(), std::back_inserter(names), [](const Parameter& p) { return p.name; });
        return names;
    }
...
class File {
public:
    explicit File(std::ostream& stream)
        : stream_(&stream)
    {
    }
    ...
    template <typename T> File& operator<<(const T& value)
    {
        s() << value;
        return *this;
    }
...
public:
    ...
    IncludeGuardScope(IncludeGuardScope&& other) V8_NOEXCEPT : file_(nullptr), name_()
    {
