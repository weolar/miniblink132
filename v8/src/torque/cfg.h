...
class Block {
public:
    explicit Block(ControlFlowGraph* cfg, size_t id, std::optional<Stack<const Type*>> input_types, bool is_deferred)
        : cfg_(cfg)
    ...
    {
        return input_types_ != std::nullopt;
    }
    ...
    void MergeInputDefinitions(const Stack<DefinitionLocation>& input_definitions, Worklist<Block*>* worklist)
    {
        ...
        for (BottomOffset i = { 0 }; i < input_definitions.AboveTop(); ++i) {
            ...
            if (current == input)
                continue;
            if (current == DefinitionLocation::Phi(this, i.offset))
                continue;
...
public:
    ...
    {
        blocks_.emplace_back(this, next_block_id_++, std::move(input_types), is_deferred);
        return &blocks_.back();
    }
    void PlaceBlock(Block* block)
    {
        placed_blocks_.push_back(block);
    }
    template <typename UnaryPredicate> void UnplaceBlockIf(UnaryPredicate&& predicate)
    {
        auto newEnd = std::remove_if(placed_blocks_.begin(), placed_blocks_.end(), std::forward<UnaryPredicate>(predicate));
...
public:
    ...
    {
        return std::all_of(
            cfg_.blocks().begin(), cfg_.blocks().end(), [this](Block* block) { return (cfg_.end() && *cfg_.end() == block) || block->IsComplete(); });
    }
