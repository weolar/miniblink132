...
public:
    template <class T> T GetObject(const std::string& property)
    {
...
template <class T> class Request : public Message {
public:
...
template <class T> class Response : public Message {
public:
...
template <class T> class ResponseArrayResult : public Message {
public:
    explicit ResponseArrayResult(JsonValue value)
        : Message(std::move(value))
    {
    }
    ResponseArrayResult()
