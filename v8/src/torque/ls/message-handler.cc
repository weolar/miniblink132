...
public:
    ...
    {
        if (!ShouldAddMessageOfKind(message.kind))
            return;

        SourceId id = message.position ? message.position->source : SourceId::Invalid();
...
private:
    PublishDiagnosticsNotification& GetOrCreateNotificationForSource(SourceId id)
    {
        ...
        std::string file = id.IsValid() ? SourceFileMap::AbsolutePath(id) : "<unknown>";
    ...
    bool ShouldAddMessageOfKind(TorqueMessage::Kind kind)
    {
        ...
        case TorqueMessage::Kind::kLint:
            if (suppress_lint_messages_)
                return false;
            return true;
        }
    }

    void PopulateRangeFromSourcePosition(Range range, const SourcePosition& position)
    {
...
void SendCompilationDiagnostics(const TorqueCompilerResult& result, MessageWriter writer)
{
    ...
    for (auto& pair : collector.notifications()) {
        ...
        if (source.IsValid())
            DiagnosticsFiles::Get().push_back(source);
    }
}
...
void HandleTorqueFileListNotification(TorqueFileListNotification notification, MessageWriter writer)
{
    ...
    for (const auto& file_json : notification.params().object()["files"].ToArray()) {
...
void HandleGotoDefinitionRequest(GotoDefinitionRequest request, MessageWriter writer)
{
    ...
    auto pos = LineAndColumn::WithUnknownOffset(request.params().position().line(), request.params().position().character());
...
void HandleChangeWatchedFilesNotification(DidChangeWatchedFilesNotification notification, MessageWriter writer)
{
...
void HandleDocumentSymbolRequest(DocumentSymbolRequest request, MessageWriter writer)
{
    ...
    for (const auto& symbol : LanguageServerData::SymbolsForSourceId(id)) {
        ...
        if (symbol->IsMacro()) {
            ...
            SymbolKind kind = type->IsClassType() ? SymbolKind::kClass : SymbolKind::kStruct;
...
void HandleMessage(JsonValue raw_message, MessageWriter writer)
{
    ...
    if (method == "initialize") {
        HandleInitializeRequest(InitializeRequest(std::move(request.GetJsonValue())), writer);
    } else if (method == "initialized") {
        HandleInitializedNotification(writer);
    } else if (method == "torque/fileList") {
        HandleTorqueFileListNotification(TorqueFileListNotification(std::move(request.GetJsonValue())), writer);
    } else if (method == "textDocument/definition") {
        HandleGotoDefinitionRequest(GotoDefinitionRequest(std::move(request.GetJsonValue())), writer);
    } else if (method == "workspace/didChangeWatchedFiles") {
        HandleChangeWatchedFilesNotification(DidChangeWatchedFilesNotification(std::move(request.GetJsonValue())), writer);
    } else if (method == "textDocument/documentSymbol") {
        HandleDocumentSymbolRequest(DocumentSymbolRequest(std::move(request.GetJsonValue())), writer);
    } else {
