...
void Block::SetInputTypes(const Stack<const Type*>& input_types)
{
    ...
    for (intptr_t i = std::max(input_types_->Size(), input_types.Size()) - 1; i >= 0; --i) {
...
StackRange CfgAssembler::Goto(Block* block, size_t preserved_slots)
{
    ...
    Emit(DeleteRangeInstruction { StackRange { block->InputTypes().AboveTop() - preserved_slots, CurrentStack().AboveTop() - preserved_slots } });
...
StackRange CfgAssembler::Peek(StackRange range, std::optional<const Type*> type)
{
    ...
    for (size_t i = 0; i < range.Size(); ++i) {
        Emit(PeekInstruction { range.begin() + i, type ? lowered_types[i] : std::optional<const Type*> {} });
    }
    return TopRange(range.Size());
}

void CfgAssembler::Poke(StackRange destination, StackRange origin, std::optional<const Type*> type)
{
    ...
    for (intptr_t i = origin.Size() - 1; i >= 0; --i) {
        Emit(PokeInstruction { destination.begin() + i, type ? lowered_types[i] : std::optional<const Type*> {} });
    }
}
...
{
    Emit(AbortInstruction { AbortInstruction::Kind::kAssertionFailure, std::move(message) });
}
...
void CfgAssembler::OptimizeCfg()
{
    ...
    for (Block* block : cfg_.blocks()) {
        if (cfg_.end() && *cfg_.end() == block)
            continue;
        if (predecessor_count[block->id()] == 0)
            continue;
        ...
        while (!block->instructions().empty()) {
            ...
            if (destination == block)
                break;
            if (cfg_.end() && *cfg_.end() == destination)
                break;
            DCHECK_GT(predecessor_count[destination->id()], 0);
            if (predecessor_count[destination->id()] != 1)
                break;

            DCHECK_GT(destination->instructions().size(), 0);
            block->instructions().pop_back();
            block->instructions().insert(block->instructions().end(), destination->instructions().begin(), destination->instructions().end());
    ...
    cfg_.UnplaceBlockIf([&](Block* b) { return predecessor_count[b->id()] == 0; });
}
...
void CfgAssembler::ComputeInputDefinitions()
{
    ...
    for (Block* block : cfg_.blocks()) {
        DCHECK_IMPLIES(!block->IsDead(), block->InputDefinitions().Size() == block->InputTypes().Size());
        USE(block);
    }
}
