...
TypeArgumentInference::TypeArgumentInference(const GenericParameters& type_parameters, const TypeVector& explicit_type_arguments,
...
TypeVector TypeArgumentInference::GetResult() const
{
    ...
    std::transform(inferred_.begin(), inferred_.end(), result.begin(), [](std::optional<const Type*> maybe_type) { return *maybe_type; });
    return result;
}

void TypeArgumentInference::Match(TypeExpression* parameter, const Type* argument_type)
{
    if (BasicTypeExpression* basic = BasicTypeExpression::DynamicCast(parameter)) {
        ...
        if (basic->namespace_qualification.empty() && !basic->is_constexpr) {
            ...
            if (result != type_parameter_from_name_.end()) {
                ...
                std::optional<const Type*>& maybe_inferred = inferred_[type_parameter_index];
...
void TypeArgumentInference::MatchGeneric(BasicTypeExpression* parameter, const Type* argument_type)
{
    QualifiedName qualified_name { parameter->namespace_qualification, parameter->name->value };
    GenericType* generic_type = Declarations::LookupUniqueGenericType(qualified_name);
    ...
    if (parameters.size() != argument_types.size()) {
        Error("cannot infer types from generic-struct-typed parameter with "
