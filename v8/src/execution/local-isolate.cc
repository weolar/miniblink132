...
LocalIsolate::LocalIsolate(Isolate* isolate, ThreadKind kind)
    ...
    , stack_limit_(kind == ThreadKind::kMain ? isolate->stack_guard()->real_climit() : GetCurrentStackPosition() - v8_flags.stack_size * KB)
...
{
    return isolate_->GetNextScriptId();
}
...
{
    const std::string& res = is_main_thread() ? isolate_->DefaultLocale() : default_locale_;
    DCHECK(!res.empty());
    return res;
}
#endif
