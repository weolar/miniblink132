...
void StackGuard::update_interrupt_requests_and_stack_limits(const ExecutionAccess& lock)
{
    ...
    for (InterruptLevel level : std::array { InterruptLevel::kNoGC, InterruptLevel::kNoHeapWrites, InterruptLevel::kAnyEffect }) {
        thread_local_.set_interrupt_requested(level, InterruptLevelMask(level) & thread_local_.interrupt_flags_);
    }
}
...
void StackGuard::SetStackLimit(uintptr_t limit)
{
    ...
    SetStackLimitInternal(access, limit, SimulatorStack::JsLimitFromCLimit(isolate_, limit));
}

void StackGuard::SetStackLimitInternal(const ExecutionAccess& lock, uintptr_t limit, uintptr_t jslimit)
{
...
{
    uintptr_t climit = SimulatorStack::ShouldSwitchCStackForWasmStackSwitching() ? limit : thread_local_.real_climit_;
    // Try to compare and swap the new jslimit and climit without the
    // ExecutionAccess lock.
    uintptr_t old_jslimit = base::Relaxed_CompareAndSwap(&thread_local_.jslimit_, thread_local_.real_jslimit_, limit);
    USE(old_jslimit);
    DCHECK_IMPLIES(old_jslimit != thread_local_.real_jslimit_, old_jslimit == kInterruptLimit);
    uintptr_t old_climit = base::Relaxed_CompareAndSwap(&thread_local_.climit_, thread_local_.real_climit_, climit);
    USE(old_climit);
    DCHECK_IMPLIES(old_climit != thread_local_.real_climit_, old_climit == kInterruptLimit);
...
void StackGuard::PushInterruptsScope(InterruptsScope* scope)
{
    ...
    if (scope->mode_ == InterruptsScope::kPostponeInterrupts) {
        ...
        uint32_t intercepted = thread_local_.interrupt_flags_ & scope->intercept_mask_;
        ...
        for (InterruptsScope* current = thread_local_.interrupt_scopes_; current != nullptr; current = current->prev_) {
...
void StackGuard::PopInterruptsScope()
{
    ...
    if (top->mode_ == InterruptsScope::kPostponeInterrupts) {
        ...
        if (top->prev_) {
            for (uint32_t interrupt = 1; interrupt < ALL_INTERRUPTS; interrupt = interrupt << 1) {
...
void StackGuard::ThreadLocal::Initialize(Isolate* isolate, const ExecutionAccess& lock)
{
...
#else // DEBUG
    explicit ShouldBeZeroOnReturnScope(int* v)
        : v_(v)
    {
    }
...
Tagged<Object> StackGuard::HandleInterrupts(InterruptLevel level)
{
    ...
    if (TestAndClear(&interrupt_flags, DEOPT_MARKED_ALLOCATION_SITES)) {
        TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("v8.gc"), "V8.GCDeoptMarkedAllocationSites");
        isolate_->heap()->DeoptMarkedAllocationSites();
    }

    if (TestAndClear(&interrupt_flags, INSTALL_CODE)) {
        TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("v8.compile"), "V8.InstallOptimizedFunctions");
    ...
    if (TestAndClear(&interrupt_flags, INSTALL_BASELINE_CODE)) {
        TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("v8.compile"), "V8.FinalizeBaselineConcurrentCompilation");
    ...
    if (TestAndClear(&interrupt_flags, INSTALL_MAGLEV_CODE)) {
        TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("v8.compile"), "V8.FinalizeMaglevConcurrentCompilation");
    ...
    if (TracingFlags::is_runtime_stats_enabled() && Protectors::IsNoProfilingIntact(isolate_)) {
