...
void Simulator::AdvancedSIMDElementOrStructureLoadStoreWriteback(int Rn, int Rm, int ebytes)
{
    if (Rm != 15) {
        if (Rm == 13) {
...
// (simulator) builds.
void SetInstructionBitsInCodeSpace(Instruction* instr, Instr value, Heap* heap)
{
    CodePageMemoryModificationScopeForDebugging scope(MemoryChunkMetadata::FromAddress(reinterpret_cast<Address>(instr)));
...
{
    if (sim_->break_pc_ != nullptr) {
        SetInstructionBitsInCodeSpace(sim_->break_pc_, sim_->break_instr_, sim_->isolate_->heap());
    }
}
...
{
    if (sim_->break_pc_ != nullptr) {
        SetInstructionBitsInCodeSpace(sim_->break_pc_, kBreakpointInstr, sim_->isolate_->heap());
    }
}
...
#define XSTR(a) STR(a)
    ...
    if ((strcmp(cmd, "si") == 0) || (strcmp(cmd, "stepi") == 0)) {
        ...
        if (argc == 2 || (argc == 3 && strcmp(arg2, "fp") == 0)) {
            ...
            if (strcmp(arg1, "all") == 0) {
                for (int i = 0; i < kNumRegisters; i++) {
                    ...
                    PrintF("%3s: 0x%08x %10d", RegisterName(Register::from_code(i)), value, value);
                ...
                for (int i = 0; i < DwVfpRegister::SupportedRegisterCount(); i++) {
                    ...
                    PrintF("%3s: %f 0x%08x %08x\n", VFPRegisters::Name(i, true), dvalue, static_cast<uint32_t>(as_words >> 32),
                ...
                if (GetValue(arg1, &value)) {
                    ...
                    uint64_t as_words = base::bit_cast<uint64_t>(dvalue);
                    PrintF("%s: %f 0x%08x %08x\n", arg1, dvalue, static_cast<uint32_t>(as_words >> 32), static_cast<uint32_t>(as_words & 0xFFFFFFFF));
                } else {
        ...
        while (cur < end) {
            PrintF("  0x%08" V8PRIxPTR ":  0x%08x %10d", reinterpret_cast<intptr_t>(cur), *cur, *cur);
        ...
        while (cur < end) {
            ...
            PrintF("  0x%08" V8PRIxPTR "  %s\n", reinterpret_cast<intptr_t>(prev), buffer.begin());
        }
        ...
        if ((argc == 2) && (strcmp(arg1, "unstop") == 0)) {
            ...
            if (stop_instr->IsStop()) {
                SetInstructionBitsInCodeSpace(stop_instr, kNopInstr, sim_->isolate_->heap());
            } else {
...
bool Simulator::InstructionTracingEnabled()
{
...
void Simulator::FlushOnePage(base::CustomMatcherHashMap* i_cache, intptr_t start, int size)
{
...
template <typename T, int SIZE> void Simulator::get_neon_register(int reg, T (&value)[SIZE / sizeof(T)])
{
...
template <typename T, int SIZE> void Simulator::set_neon_register(int reg, const T (&value)[SIZE / sizeof(T)])
{
    DCHECK(SIZE == kSimd128Size || SIZE == kDoubleSize);
    DCHECK_LE(0, reg);
    DCHECK_GT(SIZE == kSimd128Size ? num_q_registers : num_d_registers, reg);
...
template <class InputType, int register_size> void Simulator::SetVFPRegister(int reg_index, const InputType& value)
{
...
template <class ReturnType, int register_size> ReturnType Simulator::GetFromVFPRegister(int reg_index)
{
    unsigned bytes = register_size * sizeof(vfp_registers_[0]);
...
void Simulator::SetSpecialRegister(SRegisterFieldMask reg_and_mask, uint32_t value)
{
...
bool Simulator::OverflowFrom(int32_t alu_out, int32_t left, int32_t right, bool addition)
{
    bool overflow;
    if (addition) {
...
int32_t Simulator::ProcessPU(Instruction* instr, int num_regs, int reg_size, intptr_t* start_address, intptr_t* end_address)
{
...
void Simulator::HandleVList(Instruction* instr)
{
    ...
    for (int reg = vd; reg < vd + num_regs; reg++) {
        if (precision == kSinglePrecision) {
            if (load) {
                set_s_register_from_sinteger(reg, ReadW(reinterpret_cast<int32_t>(address)));
            } else {
                WriteW(reinterpret_cast<int32_t>(address), get_sinteger_from_s_register(reg));
            }
            address += 1;
        } else {
            if (load) {
                int32_t data[] = { ReadW(reinterpret_cast<int32_t>(address)), ReadW(reinterpret_cast<int32_t>(address + 1)) };
...
using SimulatorRuntimeCall
    = int64_t (*)(int32_t arg0, int32_t arg1, int32_t arg2, int32_t arg3, int32_t arg4, int32_t arg5, int32_t arg6, int32_t arg7, int32_t arg8, int32_t arg9,
        int32_t arg10, int32_t arg11, int32_t arg12, int32_t arg13, int32_t arg14, int32_t arg15, int32_t arg16, int32_t arg17, int32_t arg18, int32_t arg19);
...
// one is ever used.
using SimulatorRuntimeFPTaggedCall = double (*)(int32_t arg0, int32_t arg1, int32_t arg2, int32_t arg3);
...
int64_t UnsafeGenericFunctionCall(intptr_t function, int32_t arg0, int32_t arg1, int32_t arg2, int32_t arg3, int32_t arg4, int32_t arg5, int32_t arg6,
    int32_t arg7, int32_t arg8, int32_t arg9, int32_t arg10, int32_t arg11, int32_t arg12, int32_t arg13, int32_t arg14, int32_t arg15, int32_t arg16,
...
void Simulator::SoftwareInterrupt(Instruction* instr)
{
    ...
    case kCallRtRedirected: {
        ...
        bool stack_aligned = (get_register(sp) & (v8_flags.sim_stack_alignment - 1)) == 0;
        ...
        if (fp_call) {
            ...
            if (InstructionTracingEnabled() || !stack_aligned) {
                PrintF("Call to host function at %p args %08x", reinterpret_cast<void*>(external), arg0);
                if (!stack_aligned) {
                    PrintF(" with unaligned stack %08x\n", get_register(sp));
                }
                PrintF("\n");
            ...
            if (InstructionTracingEnabled() || !stack_aligned) {
                PrintF("Call to host function at %p args %08x %08x", reinterpret_cast<void*>(external), arg0, arg1);
                if (!stack_aligned) {
                    PrintF(" with unaligned stack %08x\n", get_register(sp));
                }
                PrintF("\n");
            ...
            if (InstructionTracingEnabled() || !stack_aligned) {
                PrintF("Call to host function at %p "
                       ...
                       "%08x",
                    reinterpret_cast<void*>(external), arg0, arg1, arg2, arg3, arg4, arg5, arg6, arg7, arg8, arg9, arg10, arg11, arg12, arg13, arg14, arg15,
...
void Simulator::DecodeType01(Instruction* instr)
{
    ...
    if ((type == 0) && instr->IsSpecialType0()) {
        ...
        case ADC: {
            ...
            alu_out = base::AddWithWraparound(base::AddWithWraparound(rn_val, shifter_operand), GetCarry());
            set_register(rd, alu_out);
            if (instr->HasS()) {
                SetNZFlags(alu_out);
        ...
        case SBC: {
            ...
            alu_out = base::SubWithWraparound(base::SubWithWraparound(rn_val, shifter_operand), (GetCarry() ? 0 : 1));
            set_register(rd, alu_out);
            if (instr->HasS()) {
                SetNZFlags(alu_out);
    ...
    case db_x: {
        ...
        if (instr->Bits(5, 4) == 0x1) {
            if ((instr->Bit(22) == 0x0) && (instr->Bit(20) == 0x1)) {
                ...
                // udiv
                if (instr->Bit(21) == 0x1) {
                    ret_val = base::bit_cast<int32_t>(base::bits::UnsignedDiv32(base::bit_cast<uint32_t>(rm_val), base::bit_cast<uint32_t>(rs_val)));
                } else {
...
void Simulator::DecodeType5(Instruction* instr)
{
    ...
    int off = static_cast<int>(static_cast<uint32_t>(instr->SImmed24Value()) << 2);
    ...
    if (instr->Bit(4) == 0) {
        ...
        if ((instr->VCValue() == 0x0) && (instr->VAValue() == 0x0)) {
            ...
            if (rt == 0xF) {
                ...
                uint32_t fpscr = (n_flag_FPSCR_ << 31) | (z_flag_FPSCR_ << 30) | (c_flag_FPSCR_ << 29) | (v_flag_FPSCR_ << 28) | (FPSCR_default_NaN_mode_ << 25)
                    | (inexact_vfp_flag_ << 4) | (underflow_vfp_flag_ << 3) | (overflow_vfp_flag_ << 2) | (div_zero_vfp_flag_ << 1) | (inv_op_vfp_flag_ << 0)
...
void Simulator::DecodeVMOVBetweenCoreAndSinglePrecisionRegisters(Instruction* instr)
{
...
bool get_inv_op_vfp_flag(VFPRoundingMode mode, double val, bool unsigned_)
{
    ...
    switch (mode) {
    case RN:
        return unsigned_ ? (val >= (max_uint + 0.5)) || (val < -0.5) : (val >= (max_int + 0.5)) || (val < (min_int - 0.5));

    case RM:
        return unsigned_ ? (val >= (max_uint + 1.0)) || (val < 0) : (val >= (max_int + 1.0)) || (val < min_int);

    case RZ:
        return unsigned_ ? (val >= (max_uint + 1.0)) || (val <= -1) : (val >= (max_int + 1.0)) || (val <= (min_int - 1.0));
    default:
        UNREACHABLE();
    }
}
...
int32_t Simulator::ConvertDoubleToInt(double val, bool unsigned_integer, VFPRoundingMode mode)
{
    int32_t result;
    if (unsigned_integer) {
    ...
    double abs_diff = unsigned_integer ? std::fabs(val - static_cast<uint32_t>(result)) : std::fabs(val - result);
    ...
    if (inv_op_vfp_flag_) {
        ...
        case RN: {
            ...
            if (abs_diff > 0.5) {
                ...
                result = ((result % 2) == 0) ? result : base::AddWithWraparound(result, val_sign);
            }
            break;
        }
...
void Simulator::DecodeVCVTBetweenFloatingPointAndInteger(Instruction* instr)
{
    ...
    if (to_integer) {
        ...
        double val = double_precision ? get_double_from_d_register(src).get_scalar() : get_float_from_s_register(src).get_scalar();
...
template <typename T, int SIZE = kSimd128Size> void Unop(Simulator* simulator, int Vd, int Vm, std::function<T(T)> unop)
{
...
template <typename T, int SIZE = kSimd128Size> void Binop(Simulator* simulator, int Vd, int Vm, int Vn, std::function<T(T, T)> binop)
{
    static const int kLanes = SIZE / sizeof(T);
...
template <typename T, typename U> U Widen(T value)
{
...
template <typename T, typename U> void Widen(Simulator* simulator, int Vd, int Vm)
{
...
template <typename T, int SIZE> void Abs(Simulator* simulator, int Vd, int Vm)
{
    Unop<T>(simulator, Vd, Vm, [](T x) { return std::abs(x); });
}

template <typename T, int SIZE> void Neg(Simulator* simulator, int Vd, int Vm)
{
...
template <typename T, typename U> void SaturatingNarrow(Simulator* simulator, int Vd, int Vm)
{
...
template <typename T> void AddSat(Simulator* simulator, int Vd, int Vm, int Vn)
{
    Binop<T>(simulator, Vd, Vm, Vn, SaturateAdd<T>);
}

template <typename T> void SubSat(Simulator* simulator, int Vd, int Vm, int Vn)
{
    Binop<T>(simulator, Vd, Vm, Vn, SaturateSub<T>);
}

template <typename T, int SIZE> void Zip(Simulator* simulator, int Vd, int Vm)
{
...
template <typename T, int SIZE> void Unzip(Simulator* simulator, int Vd, int Vm)
{
    static const int kElems = SIZE / sizeof(T);
    static const int kPairs = kElems / 2;
    T src1[kElems], src2[kElems], dst1[kElems], dst2[kElems];
...
template <typename T, int SIZE> void Transpose(Simulator* simulator, int Vd, int Vm)
{
    static const int kElems = SIZE / sizeof(T);
    static const int kPairs = kElems / 2;
...
template <typename T, int SIZE> void Test(Simulator* simulator, int Vd, int Vm, int Vn)
{
    auto test = [](T x, T y) { return (x & y) ? -1 : 0; };
    Binop<T>(simulator, Vd, Vm, Vn, test);
}

template <typename T, int SIZE> void Add(Simulator* simulator, int Vd, int Vm, int Vn)
{
    Binop<T>(simulator, Vd, Vm, Vn, std::plus<T>());
}

template <typename T, int SIZE> void Sub(Simulator* simulator, int Vd, int Vm, int Vn)
{
...
template <typename T, int SIZE> void Mul(Simulator* simulator, int Vd, int Vm, int Vn)
{
    static const int kElems = SIZE / sizeof(T);
    T src1[kElems], src2[kElems];
    simulator->get_neon_register<T, SIZE>(Vn, src1);
...
template <typename T, int SIZE> void ShiftLeft(Simulator* simulator, int Vd, int Vm, int shift)
{
    Unop<T>(simulator, Vd, Vm, [shift](T x) { return x << shift; });
}

template <typename T, int SIZE> void LogicalShiftRight(Simulator* simulator, int Vd, int Vm, int shift)
{
    Unop<T, SIZE>(simulator, Vd, Vm, [shift](T x) { return x >> shift; });
}

template <typename T, int SIZE> void ArithmeticShiftRight(Simulator* simulator, int Vd, int Vm, int shift)
{
    auto shift_fn = std::bind(ArithmeticShiftRight<T>, std::placeholders::_1, shift);
    Unop<T, SIZE>(simulator, Vd, Vm, shift_fn);
}

template <typename T, int SIZE> void ShiftRight(Simulator* simulator, int Vd, int Vm, int shift, bool is_unsigned)
{
    if (is_unsigned) {
...
template <typename T, int SIZE> void ShiftRightAccumulate(Simulator* simulator, int Vd, int Vm, int shift)
{
    Binop<T, SIZE>(simulator, Vd, Vm, Vd, [shift](T a, T x) { return a + (x >> shift); });
}

template <typename T, int SIZE> void ArithmeticShiftRightAccumulate(Simulator* simulator, int Vd, int Vm, int shift)
{
...
template <typename T, int SIZE> void ShiftLeftAndInsert(Simulator* simulator, int Vd, int Vm, int shift)
{
    static const int kElems = SIZE / sizeof(T);
    T src[kElems];
    T dst[kElems];
...
template <typename T, int SIZE> void ShiftRightAndInsert(Simulator* simulator, int Vd, int Vm, int shift)
{
    static const int kElems = SIZE / sizeof(T);
    T src[kElems];
    T dst[kElems];
...
template <typename T, typename S_T, int SIZE> void ShiftByRegister(Simulator* simulator, int Vd, int Vm, int Vn)
{
    static const int kElems = SIZE / sizeof(T);
    T src[kElems];
    S_T shift[kElems];
...
template <typename T, int SIZE> void CompareEqual(Simulator* simulator, int Vd, int Vm, int Vn)
{
    Binop<T>(simulator, Vd, Vm, Vn, [](T x, T y) { return x == y ? -1 : 0; });
}

template <typename T, int SIZE> void CompareGreater(Simulator* simulator, int Vd, int Vm, int Vn, bool ge)
{
    if (ge) {
        Binop<T>(simulator, Vd, Vm, Vn, [](T x, T y) { return x >= y ? -1 : 0; });
    } else {
...
template <typename T> T MinMax(T a, T b, bool is_min)
{
    return is_min ? std::min(a, b) : std::max(a, b);
}

template <typename T, int SIZE> void MinMax(Simulator* simulator, int Vd, int Vm, int Vn, bool min)
{
    if (min) {
        Binop<T>(simulator, Vd, Vm, Vn, [](auto x, auto y) { return std::min<T>(x, y); });
    } else {
        Binop<T>(simulator, Vd, Vm, Vn, [](auto x, auto y) { return std::max<T>(x, y); });
    }
}

template <typename T> void PairwiseMinMax(Simulator* simulator, int Vd, int Vm, int Vn, bool min)
{
...
template <typename T> void PairwiseAdd(Simulator* simulator, int Vd, int Vm, int Vn)
{
    static const int kElems = kDoubleSize / sizeof(T);
    static const int kPairs = kElems / 2;
    T dst[kElems], src1[kElems], src2[kElems];
...
template <typename NarrowType, typename WideType, int SIZE = kSimd128Size> void PairwiseAddLong(Simulator* simulator, int Vd, int Vm)
{
...
template <typename NarrowType, typename WideType, int SIZE = kSimd128Size> void PairwiseAddAccumulateLong(Simulator* simulator, int Vd, int Vm)
{
    DCHECK_EQ(sizeof(WideType), 2 * sizeof(NarrowType));
    static constexpr int kSElems = SIZE / sizeof(NarrowType);
    static constexpr int kTElems = SIZE / sizeof(WideType);
...
template <typename NarrowType, typename WideType> void MultiplyLong(Simulator* simulator, int Vd, int Vn, int Vm)
{
    DCHECK_EQ(sizeof(WideType), 2 * sizeof(NarrowType));
...
void Simulator::DecodeAdvancedSIMDTwoOrThreeRegisters(Instruction* instr)
{
    ...
    if (!op0 && op1 == 0b11) {
        ...
        if (opc1 == 0 && (opc2 >> 2) == 0) {
            ...
            switch (size) {
            case Neon8:
                is_signed ? PairwiseAddLong<int8_t, int16_t>(this, Vd, Vm) : PairwiseAddLong<uint8_t, uint16_t>(this, Vd, Vm);
                break;
            case Neon16:
                is_signed ? PairwiseAddLong<int16_t, int32_t>(this, Vd, Vm) : PairwiseAddLong<uint16_t, uint32_t>(this, Vd, Vm);
                break;
            case Neon32:
                is_signed ? PairwiseAddLong<int32_t, int64_t>(this, Vd, Vm) : PairwiseAddLong<uint32_t, uint64_t>(this, Vd, Vm);
                break;
            case Neon64:
                UNREACHABLE();
            }
            ...
            switch (size) {
            case Neon8:
                is_signed ? PairwiseAddAccumulateLong<int8_t, int16_t>(this, Vd, Vm) : PairwiseAddAccumulateLong<uint8_t, uint16_t>(this, Vd, Vm);
                break;
            case Neon16:
                is_signed ? PairwiseAddAccumulateLong<int16_t, int32_t>(this, Vd, Vm) : PairwiseAddAccumulateLong<uint16_t, uint32_t>(this, Vd, Vm);
                break;
            case Neon32:
                is_signed ? PairwiseAddAccumulateLong<int32_t, int64_t>(this, Vd, Vm) : PairwiseAddAccumulateLong<uint32_t, uint64_t>(this, Vd, Vm);
                break;
            case Neon64:
                UNREACHABLE();
            }
            ...
            for (int i = 0; i < 4; i++) {
                ...
                case 0:
                    ...
                    q_data[i] = base::bit_cast<uint32_t>(std::round(static_cast<float>(base::bit_cast<int32_t>(q_data[i]))));
                    break;
                case 1:
                    // f32 <- u32, round towards nearest.
                    q_data[i] = base::bit_cast<uint32_t>(std::round(static_cast<float>(q_data[i])));
                    break;
                case 2:
                    // s32 <- f32, round to zero.
                    q_data[i] = static_cast<uint32_t>(ConvertDoubleToInt(base::bit_cast<float>(q_data[i]), false, RZ));
                    break;
                case 3:
                    // u32 <- f32, round to zero.
                    q_data[i] = static_cast<uint32_t>(ConvertDoubleToInt(base::bit_cast<float>(q_data[i]), true, RZ));
                    break;
                }
            }
            set_neon_register(Vd, q_data);
        ...
        for (int i = 0; i < kDoubleSize; i++) {
            ...
            if (index < table_len) {
                ...
                result |= ((table >> ((index % kDoubleSize) * kBitsPerByte)) & 0xFF) << shift;
            } else if (vtbx) {
...
void Simulator::DecodeAdvancedSIMDDataProcessing(Instruction* instr)
{
    ...
    if (op0 == 0) {
        ...
        if (!u && opc == 0 && op1) {
            ...
            Binop<uint32_t>(this, Vd, Vm, Vn, [](uint32_t x, uint32_t y) { return x | (~y); });
        ...
        if (instr->Bits(21, 19) == 0 && instr->Bit(7) == 0) {
            ...
            NeonSize ns = static_cast<NeonSize>(base::bits::WhichPowerOfTwo(size >> 3));
            ...
            if (imm3H_L != 0 && opc == 0) {
                ...
                switch (ns) {
                case Neon8:
                    q ? ShiftRight<int8_t, kSimd128Size>(this, Vd, Vm, shift, u) : ShiftRight<int8_t, kDoubleSize>(this, Vd, Vm, shift, u);
                    break;
                case Neon16:
                    q ? ShiftRight<int16_t, kSimd128Size>(this, Vd, Vm, shift, u) : ShiftRight<int16_t, kDoubleSize>(this, Vd, Vm, shift, u);
                    break;
                case Neon32:
                    q ? ShiftRight<int32_t, kSimd128Size>(this, Vd, Vm, shift, u) : ShiftRight<int32_t, kDoubleSize>(this, Vd, Vm, shift, u);
                    break;
                case Neon64:
                    q ? ShiftRight<int64_t, kSimd128Size>(this, Vd, Vm, shift, u) : ShiftRight<int64_t, kDoubleSize>(this, Vd, Vm, shift, u);
                    break;
                }
                ...
                if (u) {
                    ...
                    switch (ns) {
                    case Neon8:
                        ArithmeticShiftRightAccumulate<int8_t, kDoubleSize>(this, Vd, Vm, shift);
                        break;
                    case Neon16:
                        ArithmeticShiftRightAccumulate<int16_t, kDoubleSize>(this, Vd, Vm, shift);
                        break;
                    case Neon32:
                        ArithmeticShiftRightAccumulate<int32_t, kDoubleSize>(this, Vd, Vm, shift);
                        break;
                    case Neon64:
                        ArithmeticShiftRightAccumulate<int64_t, kDoubleSize>(this, Vd, Vm, shift);
                        break;
                    }
                }
...
void Simulator::DecodeAdvancedSIMDElementOrStructureLoadStore(Instruction* instr)
{
    int op0 = instr->Bit(23);
...
void Simulator::DecodeAdvancedSIMDLoadStoreMultipleStructures(Instruction* instr)
{
    int Vd = instr->VFPDRegValue(kDoublePrecision);
...
void Simulator::DecodeAdvancedSIMDLoadSingleStructureToAllLanes(Instruction* instr)
{
...
void Simulator::DecodeAdvancedSIMDLoadStoreSingleStructureToOneLane(Instruction* instr)
{
    int L = instr->Bit(21);
...
void Simulator::DecodeSpecialCondition(Instruction* instr)
{
    ...
    } else if ((instr->Bits(27, 26) == 0b11) && (op0 == 0b10) && ((op1 >> 1) == 0b10) && !op2) {
...
intptr_t Simulator::CallImpl(Address entry, int argument_count, const intptr_t* arguments)
{
    ...
    memcpy(reinterpret_cast<intptr_t*>(entry_stack), arguments + reg_arg_count, (argument_count - reg_arg_count) * sizeof(*arguments));
...
void Simulator::LocalMonitor::NotifyLoadExcl(int32_t addr, TransactionSize size)
{
...
bool Simulator::LocalMonitor::NotifyStoreExcl(int32_t addr, TransactionSize size)
{
    if (access_state_ == MonitorAccess::Exclusive) {
...
void Simulator::GlobalMonitor::Processor::NotifyStore_Locked(int32_t addr, bool is_requesting_processor)
{
    if (access_state_ == MonitorAccess::Exclusive) {
...
bool Simulator::GlobalMonitor::Processor::NotifyStoreExcl_Locked(int32_t addr, bool is_requesting_processor)
{
    if (access_state_ == MonitorAccess::Exclusive) {
...
void Simulator::GlobalMonitor::NotifyLoadExcl_Locked(int32_t addr, Processor* processor)
{
    processor->NotifyLoadExcl_Locked(addr);
    PrependProcessor_Locked(processor);
}

void Simulator::GlobalMonitor::NotifyStore_Locked(int32_t addr, Processor* processor)
{
...
bool Simulator::GlobalMonitor::NotifyStoreExcl_Locked(int32_t addr, Processor* processor)
{
