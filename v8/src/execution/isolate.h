...
void DefaultWasmAsyncResolvePromiseCallback(v8::Isolate* isolate, v8::Local<v8::Context> context, v8::Local<v8::Promise::Resolver> resolver,
...
#define RETURN_VALUE_IF_EXCEPTION_DETECTOR(isolate, detector, value) RETURN_VALUE_IF_EXCEPTION(isolate, (detector.AcceptSideEffects(), value))

#define RETURN_EXCEPTION_IF_EXCEPTION(isolate) RETURN_VALUE_IF_EXCEPTION(isolate, kNullMaybeHandle)
...
#define ASSIGN_RETURN_FAILURE_ON_EXCEPTION(isolate, dst, call)                                                                                                 \
    do {                                                                                                                                                       \
        ...
        ASSIGN_RETURN_ON_EXCEPTION_VALUE(__isolate__, dst, call, ReadOnlyRoots(__isolate__).exception());                                                      \
    } while (false)

#define ASSIGN_RETURN_ON_EXCEPTION(isolate, dst, call) ASSIGN_RETURN_ON_EXCEPTION_VALUE(isolate, dst, call, kNullMaybeHandle)
...
#define THROW_NEW_ERROR(isolate, call) THROW_NEW_ERROR_RETURN_VALUE(isolate, call, kNullMaybeHandle)
...
#define RETURN_FAILURE_ON_EXCEPTION(isolate, call)                                                                                                             \
    do {                                                                                                                                                       \
        Isolate* __isolate__ = (isolate);                                                                                                                      \
        RETURN_ON_EXCEPTION_VALUE(__isolate__, call, ReadOnlyRoots(__isolate__).exception());                                                                  \
    } while (false);
 ...
 */
#define RETURN_ON_EXCEPTION(isolate, call) RETURN_ON_EXCEPTION_VALUE(isolate, call, kNullMaybeHandle)
...
#define WHILE_WITH_HANDLE_SCOPE(isolate, limit_check, body)                                                                                                    \
    do {                                                                                                                                                       \
        ...
        while (limit_check) {                                                                                                                                  \
            HandleScope loop_scope(for_with_handle_isolate);                                                                                                   \
            for (int for_with_handle_it = 0; limit_check && for_with_handle_it < 1024; ++for_with_handle_it) {                                                 \
                body                                                                                                                                           \
            }                                                                                                                                                  \
        }                                                                                                                                                      \
    } while (false)
...
#define ISOLATE_INIT_LIST(V)                                                                                                                                   \
    ...
    V(ModifyCodeGenerationFromStringsCallback2, modify_code_gen_callback, nullptr)                                                                             \
    V(AllowWasmCodeGenerationCallback, allow_wasm_code_gen_callback, nullptr)                                                                                  \
    V(ExtensionCallback, wasm_module_callback, &NoExtension)                                                                                                   \
    V(ExtensionCallback, wasm_instance_callback, &NoExtension)                                                                                                 \
    V(SharedArrayBufferConstructorEnabledCallback, sharedarraybuffer_constructor_enabled_callback, nullptr)                                                    \
    V(WasmStreamingCallback, wasm_streaming_callback, nullptr)                                                                                                 \
    V(WasmAsyncResolvePromiseCallback, wasm_async_resolve_promise_callback, DefaultWasmAsyncResolvePromiseCallback)                                            \
    V(WasmLoadSourceMapCallback, wasm_load_source_map_callback, nullptr)                                                                                       \
    V(WasmImportedStringsEnabledCallback, wasm_imported_strings_enabled_callback, nullptr)                                                                     \
    V(JavaScriptCompileHintsMagicEnabledCallback, compile_hints_magic_enabled_callback, nullptr)                                                               \
...
#define THREAD_LOCAL_TOP_ACCESSOR(type, name)                                                                                                                  \
    inline void set_##name(type v)                                                                                                                             \
    {                                                                                                                                                          \
        thread_local_top()->name##_ = v;                                                                                                                       \
    }                                                                                                                                                          \
    inline type name() const                                                                                                                                   \
    {                                                                                                                                                          \
        return thread_local_top()->name##_;                                                                                                                    \
    }

#define THREAD_LOCAL_TOP_ADDRESS(type, name)                                                                                                                   \
    inline type* name##_address()                                                                                                                              \
    {                                                                                                                                                          \
        return &thread_local_top()->name##_;                                                                                                                   \
    }
...
public:
    ...
    void SetUpFromReadOnlyArtifacts(ReadOnlyArtifacts* artifacts, ReadOnlyHeap* ro_heap);
    ...
    base::SharedMutex* map_updater_access()
    {
        return &map_updater_access_;
    }
    ...
    {
        return OwnsStringTables() ? string_table_.get() : shared_space_isolate()->string_table_.get();
    }
    StringForwardingTable* string_forwarding_table() const
    {
        return OwnsStringTables() ? string_forwarding_table_.get() : shared_space_isolate()->string_forwarding_table_.get();
    }

    SharedStructTypeRegistry* shared_struct_type_registry() const
    {
        return is_shared_space_isolate() ? shared_struct_type_registry_.get() : shared_space_isolate()->shared_struct_type_registry_.get();
    }
    ...
    {
        return thread_local_top()->context_;
    }
    ...
    static uint32_t c_entry_fp_offset()
    {
        return static_cast<uint32_t>(
            OFFSET_OF(Isolate, isolate_data_) + OFFSET_OF(IsolateData, thread_local_top_) + OFFSET_OF(ThreadLocalTop, c_entry_fp_) - isolate_root_bias());
    }
    inline Address* handler_address()
    {
        return &thread_local_top()->handler_;
    }
    ...
    bool WalkCallStackAndPromiseTree(MaybeHandle<JSPromise> rejected_promise, const std::function<void(PromiseHandler)>& callback);
    ...
    V8_NOINLINE void PushStackTraceAndDie(
        void* ptr1 = nullptr, void* ptr2 = nullptr, void* ptr3 = nullptr, void* ptr4 = nullptr, void* ptr5 = nullptr, void* ptr6 = nullptr);
    // Similar to the above but without collecting the stack trace.
    V8_NOINLINE void PushParamsAndDie(
        void* ptr1 = nullptr, void* ptr2 = nullptr, void* ptr3 = nullptr, void* ptr4 = nullptr, void* ptr5 = nullptr, void* ptr6 = nullptr);
    ...
    V8_NOINLINE void PushParamsAndContinue(
        void* ptr1 = nullptr, void* ptr2 = nullptr, void* ptr3 = nullptr, void* ptr4 = nullptr, void* ptr5 = nullptr, void* ptr6 = nullptr);
    Handle<StackTraceInfo> CaptureDetailedStackTrace(int limit, StackTrace::StackTraceOptions options);
    MaybeHandle<JSObject> CaptureAndSetErrorStack(Handle<JSObject> error_object, FrameSkipMode mode, Handle<Object> caller);
    ...
    // set.
    bool MayAccess(Handle<NativeContext> accessing_context, Handle<JSObject> receiver);

    void SetFailedAccessCheckCallback(v8::FailedAccessCheckCallback callback);
    V8_WARN_UNUSED_RESULT MaybeHandle<Object> ReportFailedAccessCheck(Handle<JSObject> receiver);
    ...
    void OnAsyncFunctionSuspended(Handle<JSPromise> promise, Handle<JSPromise> parent);
    ...
    bool ComputeLocationFromSimpleStackTrace(MessageLocation* target, Handle<Object> exception);
    bool ComputeLocationFromDetailedStackTrace(MessageLocation* target, Handle<Object> exception);

    Handle<JSMessageObject> CreateMessage(Handle<Object> exception, MessageLocation* location);
    Handle<JSMessageObject> CreateMessageOrAbort(Handle<Object> exception, MessageLocation* location);
...
#undef NATIVE_CONTEXT_FIELD_ACCESSOR
    ...
    {
        DCHECK_IMPLIES(!COMPRESS_POINTERS_BOOL, isolate_data()->cage_base() == kNullAddress);
    ...
    {
        return base::AddressRegion(reinterpret_cast<Address>(&isolate_data_), sizeof(IsolateData));
    }
    ...
    {
        return isolate_data_.builtin_entry_table();
    }
    ...
    static constexpr bool kBuiltinDispatchHandlesAreStatic = ReadOnlyHeap::IsReadOnlySpaceShared();

    static V8_INLINE JSDispatchHandle builtin_dispatch_handle(JSBuiltinDispatchHandleRoot::Idx idx)
    {
    ...
    {
        return builtin_dispatch_handle(JSBuiltinDispatchHandleRoot::to_idx(builtin));
    }
#endif
    V8_INLINE Address* builtin_table()
    {
        return isolate_data_.builtin_table();
    }
    ...
    static constexpr uint32_t thread_in_wasm_flag_address_offset()
    {
        ...
        return static_cast<uint32_t>(OFFSET_OF(Isolate, isolate_data_) + OFFSET_OF(IsolateData, thread_local_top_)
            + OFFSET_OF(ThreadLocalTop, thread_in_wasm_flag_address_) - isolate_root_bias());
    }

    constexpr static uint32_t context_offset()
    {
        return static_cast<uint32_t>(
            OFFSET_OF(Isolate, isolate_data_) + OFFSET_OF(IsolateData, thread_local_top_) + OFFSET_OF(ThreadLocalTop, context_) - isolate_root_bias());
    }

    static uint32_t error_message_param_offset()
    {
        return static_cast<uint32_t>(OFFSET_OF(Isolate, isolate_data_) + OFFSET_OF(IsolateData, error_message_param_) - isolate_root_bias());
    }

    uint8_t error_message_param()
    {
        return isolate_data_.error_message_param_;
    }
    ...
    {
        DCHECK_EQ(value == nullptr, regexp_static_result_offsets_vector_ != nullptr);
    ...
    {
        return isolate_data_.execution_mode_ & IsolateExecutionModeFlag::kIsProfiling;
    }
    ...
    void SetIsProfiling(bool enabled)
    {
        ...
        isolate_data_.execution_mode_.set(IsolateExecutionModeFlag::kIsProfiling, enabled);
    ...
    {
        return isolate_data_.execution_mode_ & IsolateExecutionModeFlag::kCheckSideEffects;
    }

    DebugInfo::ExecutionMode debug_execution_mode() const
    {
        return should_check_side_effects() ? DebugInfo::kSideEffects : DebugInfo::kBreakpoints;
    }
    void set_debug_execution_mode(DebugInfo::ExecutionMode debug_execution_mode)
    {
        ...
        isolate_data_.execution_mode_.set(IsolateExecutionModeFlag::kCheckSideEffects, check_side_effects);
    }
    ...
    bool has_turbofan_string_builders()
    {
    ...
    enum class ICUObjectCacheType { kDefaultCollator, kDefaultNumberFormat, kDefaultSimpleDateFormat, kDefaultSimpleDateFormatForTime,
        kDefaultSimpleDateFormatForDate };
    static constexpr int kICUObjectCacheTypeCount = 5;

    icu::UMemory* get_cached_icu_object(ICUObjectCacheType cache_type, Handle<Object> locales);
    void set_icu_object_in_cache(ICUObjectCacheType cache_type, DirectHandle<Object> locales, std::shared_ptr<icu::UMemory> obj);
    ...
    bool concurrent_recompilation_enabled()
    {
        ...
        DCHECK(optimizing_compile_dispatcher_ == nullptr || v8_flags.concurrent_recompilation);
    ...
    {
        promise_hook_flags_ = PromiseHookFields::HasContextPromiseHook::update(promise_hook_flags_, context_promise_hook);
        PromiseHookStateUpdated();
    }
    ...
    {
        return PromiseHookFields::HasContextPromiseHook::decode(promise_hook_flags_);
    }
    ...
    void RunAtomicsWaitCallback(v8::Isolate::AtomicsWaitEvent event, Handle<JSArrayBuffer> array_buffer, size_t offset_in_bytes, int64_t value,
    ...
    void RunAllPromiseHooks(PromiseHookType type, Handle<JSPromise> promise, Handle<Object> parent);
    ...
    MaybeHandle<JSObject> RunHostInitializeImportMetaObjectCallback(Handle<SourceTextModule> module);
    ...
    void SetBuiltinUnwindData(Builtin builtin, const win64_unwindinfo::BuiltinUnwindInfo& unwinding_info);
#endif // V8_OS_WIN64

    void SetPrepareStackTraceCallback(PrepareStackTraceCallback callback);
    MaybeHandle<Object> RunPrepareStackTraceCallback(Handle<NativeContext>, Handle<JSObject> Error, Handle<JSArray> sites);
    ...
    {
        return v8_flags.efficiency_mode_for_tiering_heuristics && EfficiencyModeEnabled();
    }
    ...
    {
        thread_local_top()->top_backup_incumbent_scope_ = top_backup_incumbent_scope;
    }
    ...
    {
        return main_thread_local_isolate();
    }
    ...
    {
        return reinterpret_cast<Address>(&isolate_data_.shared_external_pointer_table_);
    }
    ...
    {
        return reinterpret_cast<Address>(&isolate_data_.shared_trusted_pointer_table_);
    }
#endif // V8_ENABLE_SANDBOX

    Address continuation_preserved_embedder_data_address()
    {
        return reinterpret_cast<Address>(&isolate_data_.continuation_preserved_embedder_data_);
    }
    ...
    {
        return owns_shareable_data_;
    }

    bool log_object_relocation() const
    {
        return log_object_relocation_;
    }
    ...
    void LocalsBlockListCacheSet(Handle<ScopeInfo> scope_info, Handle<ScopeInfo> outer_scope_info, Handle<StringSet> locals_blocklist);
    ...
    void ReportExceptionPropertyCallback(Handle<JSReceiver> holder, Handle<Name> name, v8::ExceptionContext callback_kind);
    void SetExceptionPropagationCallback(ExceptionPropagationCallback callback);

#ifdef V8_ENABLE_WASM_SIMD256_REVEC
    void set_wasm_revec_verifier_for_test(compiler::turboshaft::WasmRevecVerifier* verifier)
    {
        wasm_revec_verifier_for_test_ = verifier;
    }

    compiler::turboshaft::WasmRevecVerifier* wasm_revec_verifier_for_test() const
    {
...
private:
    ...
    {
        return PromiseHookFields::HasIsolatePromiseHook::decode(promise_hook_flags_);
    }

    bool HasAsyncEventDelegate() const
    {
        return PromiseHookFields::HasAsyncEventDelegate::decode(promise_hook_flags_);
    }
    ...
    std::atomic<debug::CoverageMode> code_coverage_mode_ { debug::CoverageMode::kBestEffort };
    ...
    MaybeHandle<FixedArray> GetImportAttributesFromArgument(MaybeHandle<Object> maybe_import_options_argument);
    ...
    std::atomic<v8::Isolate::Priority> priority_ = v8::Isolate::Priority::kUserBlocking;
...
#define ISOLATE_FIELD_OFFSET(type, name, ignored) static const intptr_t name##_debug_offset_;
...
#undef ISOLATE_FIELD_OFFSET
    ...
    void InitializeNextUniqueSfiId(uint32_t id)
    {
        ...
        bool successfully_exchanged = next_unique_sfi_id_.compare_exchange_strong(expected, id, std::memory_order_relaxed, std::memory_order_relaxed);
...
#ifdef V8_ENABLE_WASM_SIMD256_REVEC
    compiler::turboshaft::WasmRevecVerifier* wasm_revec_verifier_for_test_ = nullptr;
#endif // V8_ENABLE_WASM_SIMD256_REVEC
...
extern thread_local Isolate::PerIsolateThreadData* g_current_per_isolate_thread_data_ V8_CONSTINIT;
...
public:
    ...
    ~AssertNoContextChange()
    {
        ...
        if (!isolate_->topmost_script_having_context().is_null()) {
            CHECK_EQ(isolate_->topmost_script_having_context(), *topmost_script_having_context_);
        }
    }
...
public:
    ...
    {
        isolate->break_access()->Lock();
    }
    static void Unlock(Isolate* isolate)
    {
        isolate->break_access()->Unlock();
    }
...
#define STACK_CHECK(isolate, result_value)                                                                                                                     \
    do {                                                                                                                                                       \
        ...
        if (V8_UNLIKELY(stack_check.InterruptRequested()) && V8_UNLIKELY(stack_check.HandleStackOverflowAndTerminationRequest())) {                            \
            return result_value;                                                                                                                               \
        }                                                                                                                                                      \
    } while (false)
...
public:
    ...
    explicit StackTraceFailureMessage(Isolate* isolate, StackTraceMode mode, void* ptr1 = nullptr, void* ptr2 = nullptr, void* ptr3 = nullptr,
...
template <base::MutexSharedType kIsShared> class V8_NODISCARD SharedMutexGuardIfOffThread<Isolate, kIsShared> final {
public:
