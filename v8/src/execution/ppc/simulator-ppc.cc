                ...
                if (argc == 2 || (argc == 3 && strcmp(arg2, "fp") == 0)) {
                    ...
                    if (strcmp(arg1, "all") == 0) {
                        ...
                        PrintF("  pc: %08" V8PRIxPTR "  lr: %08" V8PRIxPTR "  "
                            ...
                            sim_->special_reg_pc_, sim_->special_reg_lr_, sim_->special_reg_ctr_, sim_->special_reg_xer_, sim_->condition_reg_);
                        ...
                        && (arg1[1] >= '0' && arg1[1] <= '9' && (arg1[2] == '\0' || (arg1[2] >= '0' && arg1[2] <= '9' && arg1[3] == '\0')))) {
...
intptr_t Simulator::get_lr() const
{
...
template <typename A, typename T, typename Operation> void VectorCompareOp(Simulator* sim, Instruction* instr, bool is_fp, Operation op)
{
...
template <typename S, typename T> void VectorConverFromFPSaturate(Simulator* sim, Instruction* instr, T min_val, T max_val, bool even_lane_result = false)
{
    int t = instr->RTValue();
    int b = instr->RBValue();
    FOR_EACH_LANE(i, S)
...
template <typename S, typename T> void VectorPackSaturate(Simulator* sim, Instruction* instr, S min_val, S max_val)
{
    DECODE_VX_INSTRUCTION(t, a, b, T)
    int src = a;
    int count = 0;
    ...
    FOR_EACH_LANE(i, T)
    {
        sim->set_simd_register_by_lane<T>(t, i, temps[i]);
    }
}

template <typename T> T VSXFPMin(T x, T y)
{
    // Handle NaN.
    ...
    if (std::signbit(x) < std::signbit(y))
        return y;
    if (std::signbit(y) < std::signbit(x))
        return x;
    return std::fmin(x, y);
}

template <typename T> T VSXFPMax(T x, T y)
{
    // Handle NaN.
    // TODO(miladfarca): include the payload of src1.
    if (std::isnan(x) && std::isnan(y))
...
void Simulator::ExecuteGeneric(Instruction* instr)
{
    ...
    case PLOAD_STORE_MLS: {
        ...
        Instruction* next_instr = reinterpret_cast<Instruction*>(get_pc() + kInstrSize);
    ...
    case MODSD: {
        ...
        int64_t alu_out = (rb_val == 0 || (ra_val == kMinLongLong && rb_val == -1)) ? -1 : ra_val % rb_val;
        set_register(rt, alu_out);
        break;
    }
    case SRAW: {
    ...
    case MTVSRD: {
        ...
        if (!instr->Bit(0)) {
            ...
            // if simd reg (TX=1).
            DCHECK_EQ(instr->Bit(0), 1);
            set_simd_register_by_lane<int64_t>(frt, 0, static_cast<int64_t>(ra_val));
            // Low 64 bits of the result is undefined,
            // Which is simulated here by adding random bits.
            set_simd_register_by_lane<int64_t>(frt, 1, static_cast<int64_t>(0x123456789ABCD));
        }
        break;
    }
    case MTVSRDD: {
        ...
        int ra = instr->RAValue();
        int rb = instr->RBValue();
        set_simd_register_by_lane<int64_t>(xt, 0, static_cast<int64_t>(get_register(ra)));
        set_simd_register_by_lane<int64_t>(xt, 1, static_cast<int64_t>(get_register(rb)));
        break;
    }
    case MTVSRWA: {
        DCHECK(!instr->Bit(0));
    ...
    case FCTIDZ: {
        ...
        int frb = instr->RBValue();
        double frb_val = get_double_from_d_register(frb);
        int mode = (opcode == FCTIDZ) ? kRoundToZero : (fp_condition_reg_ & kFPRoundingModeMask);
        int64_t frt_val;
        int64_t one = 1; // work-around gcc
    ...
    case FCTIDUZ: {
        ...
        int frb = instr->RBValue();
        double frb_val = get_double_from_d_register(frb);
        int mode = (opcode == FCTIDUZ) ? kRoundToZero : (fp_condition_reg_ & kFPRoundingModeMask);
        uint64_t frt_val;
        uint64_t kMinVal = 0;
    ...
    case FCTIWZ: {
        ...
        int frb = instr->RBValue();
        double frb_val = get_double_from_d_register(frb);
        int mode = (opcode == FCTIWZ) ? kRoundToZero : (fp_condition_reg_ & kFPRoundingModeMask);
        int64_t frt_val;
    ...
    case FCTIWUZ: {
        ...
        int frb = instr->RBValue();
        double frb_val = get_double_from_d_register(frb);
        int mode = (opcode == FCTIWUZ) ? kRoundToZero : (fp_condition_reg_ & kFPRoundingModeMask);
        uint64_t frt_val;
    ...
    case MTCRF: {
        ...
        for (int i = 0; i <= max_field_index; i++) {
            ...
            result |= ((source << i * field_bit_count) >> i * field_bit_count) >> (max_field_index - i) * field_bit_count;
            bit_mask >>= 1;
        }
    ...
    case LXVD: {
        ...
        set_simd_register_by_lane<int64_t>(xt, 1, ReadDW(ra_val + rb_val + kSystemPointerSize));
        break;
    }
    case LXVX: {
        DECODE_VX_INSTRUCTION(vrt, ra, rb, T)
    ...
    case STXVD: {
        ...
        WriteDW(ra_val + rb_val + kSystemPointerSize, get_simd_register_by_lane<int64_t>(xs, 1));
        break;
    }
    case STXVX: {
        DECODE_VX_INSTRUCTION(vrs, ra, rb, S)
...
#define VSPLT(type)                                                                                                                                            \
    ...
    {                                                                                                                                                          \
        set_simd_register_by_lane<type>(vrt, i, value);                                                                                                        \
    }
    case VSPLTW: {
        VSPLT(int32_t)
        break;
...
#define VSPLTI(type)                                                                                                                                           \
    ...
    FOR_EACH_LANE(i, type)                                                                                                                                     \
    {                                                                                                                                                          \
        set_simd_register_by_lane<type>(vrt, i, sim);                                                                                                          \
    }
    case VSPLTISW: {
        VSPLTI(int32_t)
        break;
...
#define VINSERT(type, element)                                                                                                                                 \
    ...
    int vrt = instr->RTValue();                                                                                                                                \
    int vrb = instr->RBValue();                                                                                                                                \
    set_simd_register_bytes<type>(vrt, uim, get_simd_register_by_lane<type>(vrb, element));
    case VINSERTD: {
        VINSERT(int64_t, 0)
        break;
    }
...
#define VECTOR_ARITHMETIC_OP(type, op)                                                                                                                         \
    ...
    FOR_EACH_LANE(i, type)                                                                                                                                     \
    {                                                                                                                                                          \
        set_simd_register_by_lane<type>(t, i, get_simd_register_by_lane<type>(a, i) op get_simd_register_by_lane<type>(b, i));                                 \
    }
    case XVADDDP: {
...
#define VECTOR_MULTIPLY_EVEN_ODD(input_type, result_type, is_odd)                                                                                              \
    ...
    for (; j < kSimd128Size; i += 2, j += lane_size * 2, k++) {                                                                                                \
        result_type src0 = static_cast<result_type>(get_simd_register_by_lane<input_type>(a, i));                                                              \
        result_type src1 = static_cast<result_type>(get_simd_register_by_lane<input_type>(b, i));                                                              \
...
#define VECTOR_MERGE(type, is_low_side)                                                                                                                        \
    ...
    for (size_t i = 0, source_index = is_low_side ? i + index_limit : i; i < index_limit; i++, source_index++) {                                               \
        set_simd_register_by_lane<type>(t, 2 * i, get_simd_register_by_lane<type>(a, source_index));                                                           \
        set_simd_register_by_lane<type>(t, (2 * i) + 1, get_simd_register_by_lane<type>(b, source_index));                                                     \
    }
    case VMRGLW: {
...
#define VECTOR_SHIFT_OP(type, op, mask)                                                                                                                        \
    ...
    FOR_EACH_LANE(i, type)                                                                                                                                     \
    {                                                                                                                                                          \
        set_simd_register_by_lane<type>(t, i, get_simd_register_by_lane<type>(a, i) op(get_simd_register_by_lane<type>(b, i) & mask));                         \
    }
    case VSLD: {
...
#define VECTOR_COMPARE_OP(type_in, type_out, is_fp, op) VectorCompareOp<type_in, type_out>(this, instr, is_fp, [](type_in a, type_in b) { return a op b; });
    case XVCMPEQDP: {
...
#undef VECTOR_COMPARE_OP
    ...
    case XVCVDPSXWS: {
        VectorConverFromFPSaturate<double, int32_t>(this, instr, kMinInt, kMaxInt, true);
        break;
    }
    case XVCVDPUXWS: {
        VectorConverFromFPSaturate<double, uint32_t>(this, instr, 0, kMaxUInt32, true);
        break;
    }
    case XVCVSXWSP: {
        int t = instr->RTValue();
    ...
    case XSCVSPDPN: {
        ...
        float f = base::bit_cast<float, uint32_t>(static_cast<uint32_t>(double_bits >> 32));
    ...
    case XSCVDPSPN: {
        ...
        int b = instr->RBValue();
        double b_val = get_double_from_d_register(b);
        uint64_t float_bits = static_cast<uint64_t>(base::bit_cast<uint32_t, float>(static_cast<float>(b_val)));
        // Preserve snan.
        if (is_snan(b_val)) {
...
#define VECTOR_ADD_SUB_SATURATE(intermediate_type, result_type, op, min_val, max_val)                                                                          \
    DECODE_VX_INSTRUCTION(t, a, b, T)                                                                                                                          \
    FOR_EACH_LANE(i, result_type)                                                                                                                              \
    {                                                                                                                                                          \
        intermediate_type a_val = static_cast<intermediate_type>(get_simd_register_by_lane<result_type>(a, i));                                                \
        intermediate_type b_val = static_cast<intermediate_type>(get_simd_register_by_lane<result_type>(b, i));                                                \
        intermediate_type t_val = a_val op b_val;                                                                                                              \
        if (t_val > max_val)                                                                                                                                   \
            t_val = max_val;                                                                                                                                   \
        else if (t_val < min_val)                                                                                                                              \
            t_val = min_val;                                                                                                                                   \
        set_simd_register_by_lane<result_type>(t, i, static_cast<result_type>(t_val));                                                                         \
    }
    case VADDSHS: {
...
#undef VECTOR_FP_ROUNDING
    case VSEL: {
        ...
        unsigned __int128 src_1 = base::bit_cast<__int128>(get_simd_register(vra).int8);
        unsigned __int128 src_2 = base::bit_cast<__int128>(get_simd_register(vrb).int8);
        unsigned __int128 src_3 = base::bit_cast<__int128>(get_simd_register(vrc).int8);
    ...
    case VBPERMQ: {
        ...
        unsigned __int128 src_bits = base::bit_cast<__int128>(get_simd_register(a).int8);
...
#undef VECTOR_FP_QF
    ...
    case VMSUMMBM: {
        ...
        FOR_EACH_LANE(i, int32_t)
        {
            int8_t vra_1_val = get_simd_register_by_lane<int8_t>(vra, 4 * i), vra_2_val = get_simd_register_by_lane<int8_t>(vra, (4 * i) + 1),
                   vra_3_val = get_simd_register_by_lane<int8_t>(vra, (4 * i) + 2), vra_4_val = get_simd_register_by_lane<int8_t>(vra, (4 * i) + 3);
            uint8_t vrb_1_val = get_simd_register_by_lane<uint8_t>(vrb, 4 * i), vrb_2_val = get_simd_register_by_lane<uint8_t>(vrb, (4 * i) + 1),
                    vrb_3_val = get_simd_register_by_lane<uint8_t>(vrb, (4 * i) + 2), vrb_4_val = get_simd_register_by_lane<uint8_t>(vrb, (4 * i) + 3);
            int32_t vrc_val = get_simd_register_by_lane<int32_t>(vrc, i);
            int32_t temp1 = vra_1_val * vrb_1_val, temp2 = vra_2_val * vrb_2_val, temp3 = vra_3_val * vrb_3_val, temp4 = vra_4_val * vrb_4_val;
    ...
    case VMSUMSHM: {
        ...
        FOR_EACH_LANE(i, int32_t)
        {
            ...
            int16_t vra_2_val = get_simd_register_by_lane<int16_t>(vra, (2 * i) + 1);
            int16_t vrb_1_val = get_simd_register_by_lane<int16_t>(vrb, 2 * i);
            int16_t vrb_2_val = get_simd_register_by_lane<int16_t>(vrb, (2 * i) + 1);
            int32_t vrc_val = get_simd_register_by_lane<int32_t>(vrc, i);
    ...
    case VMLADDUHM: {
        ...
        FOR_EACH_LANE(i, uint16_t)
        {
            ...
            set_simd_register_by_lane<uint16_t>(vrt, i, (vra_val * vrb_val) + vrc_val);
        }
        break;
    }
#define VECTOR_UNARY_OP(type, op)                                                                                                                              \
    ...
    FOR_EACH_LANE(i, type)                                                                                                                                     \
    {                                                                                                                                                          \
        set_simd_register_by_lane<type>(t, i, op(get_simd_register_by_lane<type>(b, i)));                                                                      \
    }
    case XVABSDP: {
...
#undef VECTOR_ROUNDING_AVERAGE
    case VPOPCNTB: {
        ...
        {
            set_simd_register_by_lane<uint8_t>(t, i, base::bits::CountPopulation(get_simd_register_by_lane<uint8_t>(b, i)));
        }
        break;
    }
...
void Simulator::GlobalMonitor::NotifyLoadExcl(uintptr_t addr, TransactionSize size, ThreadId thread_id)
{
...
void Simulator::GlobalMonitor::NotifyStore(uintptr_t addr, TransactionSize size, ThreadId thread_id)
{
...
bool Simulator::GlobalMonitor::NotifyStoreExcl(uintptr_t addr, TransactionSize size, ThreadId thread_id)
{
