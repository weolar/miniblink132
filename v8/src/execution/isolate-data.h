...
#define ISOLATE_DATA_FAST_C_CALL_PADDING(V) V(kFastCCallAlignmentPaddingOffset, kSystemPointerSize, fast_c_call_alignment_padding)
#endif // V8_HOST_ARCH_64_BIT

#ifdef V8_ENABLE_LEAPTIERING

#define BUILTINS_WITH_DISPATCH_ADAPTER(V, CamelName, underscore_name, ...) V(CamelName, CamelName##SharedFun)

#define BUILTINS_WITH_DISPATCH_LIST(V) BUILTINS_WITH_SFI_LIST_GENERATOR(BUILTINS_WITH_DISPATCH_ADAPTER, V)
...
#define CASE(builtin_name, ...) Builtin::k##builtin_name,
        return std::array<Builtin, Idx::kCount> { BUILTINS_WITH_DISPATCH_LIST(CASE) }[idx];
#undef CASE
    }
...
#define ISOLATE_DATA_FIELDS(V)                                                                                                                                 \
    ...
    V(BuiltinTier0EntryTable, Builtins::kBuiltinTier0Count* kSystemPointerSize, builtin_tier0_entry_table)                                                     \
    V(BuiltinsTier0Table, Builtins::kBuiltinTier0Count* kSystemPointerSize, builtin_tier0_table)                                                               \
    /* Misc. fields. */                                                                                                                                        \
    ...
    V(EmbedderData, Internals::kNumIsolateDataSlots* kSystemPointerSize, embedder_data)                                                                        \
    ...
    V(ContinuationPreservedEmbedderData, kSystemPointerSize, continuation_preserved_embedder_data)                                                             \
    /* Full tables (arbitrary size, potentially slower access). */                                                                                             \
    V(RootsTable, RootsTable::kEntriesCount* kSystemPointerSize, roots_table)                                                                                  \
    V(ExternalReferenceTable, ExternalReferenceTable::kSizeInBytes, external_reference_table)                                                                  \
    V(BuiltinEntryTable, Builtins::kBuiltinCount* kSystemPointerSize, builtin_entry_table)                                                                     \
...
#define EXTERNAL_REFERENCE_LIST_ISOLATE_FIELDS(V) V(isolate_address, "isolate address", IsolateAddress)
...
#undef V
    ...
    static constexpr int BuiltinEntrySlotOffset(Builtin id)
    {
        ...
        return (Builtins::IsTier0(id) ? builtin_tier0_entry_table_offset() : builtin_entry_table_offset()) + Builtins::ToInt(id) * kSystemPointerSize;
    }
    ...
    {
        return (Builtins::IsTier0(id) ? builtin_tier0_table_offset() : builtin_table_offset()) + Builtins::ToInt(id) * kSystemPointerSize;
    }
...
#define V(Offset, Size, Name)                                                                                                                                  \
    ...
    {                                                                                                                                                          \
        return reinterpret_cast<Address>(&Name##_);                                                                                                            \
    }
    ISOLATE_DATA_FIELDS(V)
#undef V
    ...
    {
        return fast_c_call_caller_fp_;
    }
    Address fast_c_call_caller_pc() const
    {
        return fast_c_call_caller_pc_;
    }
    Address fast_api_call_target() const
    {
        return fast_api_call_target_;
    }
    ...
    {
        return builtin_tier0_entry_table_;
    }
    Address* builtin_tier0_table()
    {
        return builtin_tier0_table_;
    }
    RootsTable& roots()
    {
        return roots_table_;
    ...
    {
        return builtin_entry_table_;
    }
    Address* builtin_table()
    {
        return builtin_table_;
    }
...
#define THREAD_LOCAL_TOP_MEMBER_OFFSET(Name)                                                                                                                   \
    ...
    {                                                                                                                                                          \
        return static_cast<uint32_t>(IsolateData::thread_local_top_offset() + OFFSET_OF(ThreadLocalTop, Name##_));                                             \
    }
...
#define FIELDS(V)                                                                                                                                              \
    ...
    V(Padding, 8 + RoundUp<8>(static_cast<int>(kPaddingOffset)) - kPaddingOffset)                                                                              \
    /* Total size. */                                                                                                                                          \
    V(Size, 0)
...
#define V(PureName, Size, Name)                                                                                                                                \
    static_assert(std::is_standard_layout<decltype(IsolateData::Name##_)>::value);                                                                             \
