...
#if V8_TARGET_ARCH_ARM64 || V8_TARGET_ARCH_MIPS64 || V8_TARGET_ARCH_LOONG64 || V8_TARGET_ARCH_RISCV64
...
public:
    ...
    static base::Mutex* redirection_mutex()
    {
    ...
    static void set_redirection(Redirection* r)
    {
        redirection_ = r;
    }

    static base::Mutex* i_cache_mutex()
    {
        return i_cache_mutex_;
    }
    static base::CustomMatcherHashMap* i_cache()
    {
        return i_cache_;
    }
    ...
    static Address RedirectExternalReference(Address external_function, ExternalReference::Type type);
...
protected:
    ...
    static Return VariadicCall(SimT* sim, CallImpl call, Address entry, Args... args)
    {
    ...
    template <typename T> static typename std::enable_if<std::is_integral<T>::value, T>::type ConvertReturn(intptr_t ret)
    {
    ...
    template <typename T> static typename std::enable_if<std::is_pointer<T>::value, T>::type ConvertReturn(intptr_t ret)
    {
        return reinterpret_cast<T>(ret);
    }

    template <typename T> static typename std::enable_if<std::is_base_of<Object, T>::value, T>::type ConvertReturn(intptr_t ret)
    {
        return Tagged<Object>(ret);
    }

#if V8_TARGET_ARCH_ARM64 || V8_TARGET_ARCH_MIPS64 || V8_TARGET_ARCH_LOONG64 || V8_TARGET_ARCH_RISCV64
    template <typename T> static typename std::enable_if<std::is_same<T, v8::AnyCType>::value, T>::type ConvertReturn(intptr_t ret)
    {
        v8::AnyCType result;
    ...
    template <typename T> static typename std::enable_if<std::is_void<T>::value, T>::type ConvertReturn(intptr_t ret)
    {
    }
    ...
    template <typename T> static typename std::enable_if<std::is_integral<T>::value, intptr_t>::type ConvertArg(T arg)
    {
        static_assert(sizeof(T) <= sizeof(intptr_t), "type bigger than ptrsize");
    ...
    template <typename T> static typename std::enable_if<std::is_pointer<T>::value, intptr_t>::type ConvertArg(T arg)
    {
        return reinterpret_cast<intptr_t>(arg);
    }

    template <typename T> static typename std::enable_if<std::is_floating_point<T>::value, intptr_t>::type ConvertArg(T arg)
    {
        UNREACHABLE();
    }
...
public:
    ...
    static Redirection* Get(Address external_function, ExternalReference::Type type);
    ...
    {
        Redirection* redirection = FromInstruction(reinterpret_cast<Instruction*>(reinterpret_cast<void*>(reg)));
...
public:
    ...
    void RegisterFunctionsAndSignatures(Address* c_functions, const CFunctionInfo* const* c_signatures, unsigned num_functions);
    ...
    void AddSignatureForTargetForTesting(Address target, const EncodedCSignature& signature)
    {
        AddSignatureForTarget(target, signature);
    }

private:
    void AddSignatureForTarget(Address target, const EncodedCSignature& signature)
    {
