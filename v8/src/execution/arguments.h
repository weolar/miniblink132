...
template <ArgumentsType arguments_type> class Arguments {
public:
    // Scope to temporarily change the value of an argument.
    class ChangeValueScope {
    public:
        inline ChangeValueScope(Isolate* isolate, Arguments* args, int index, Tagged<Object> value);
        ~ChangeValueScope()
        {
            *location_ = (*old_value_).ptr();
        }
    ...
    V8_INLINE Address* address_of_arg_at(int index) const
    {
        ...
        return reinterpret_cast<Address*>(reinterpret_cast<Address>(arguments_) - offset);
    }
...
template <ArgumentsType T> template <class S> Handle<S> Arguments<T>::at(int index) const
{
    Handle<Object> obj = Handle<Object>(address_of_arg_at(index));
    return Cast<S>(obj);
}

template <ArgumentsType T> FullObjectSlot Arguments<T>::slot_from_address_at(int index, int offset) const
{
...
#define RUNTIME_ENTRY_WITH_RCS(Type, InternalType, Convert, Name)                                                                                              \
    V8_NOINLINE static Type Stats_##Name(int args_length, Address* args_object, Isolate* isolate)                                                              \
    {                                                                                                                                                          \
        RCS_SCOPE(isolate, RuntimeCallCounterId::k##Name);                                                                                                     \
        TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("v8.runtime"), "V8.Runtime_" #Name);                                                                            \
...
#define RUNTIME_FUNCTION_RETURNS_TYPE(Type, InternalType, Convert, Name)                                                                                       \
    static V8_INLINE InternalType __RT_impl_##Name(RuntimeArguments args, Isolate* isolate);                                                                   \
...
#define RUNTIME_FUNCTION(Name) RUNTIME_FUNCTION_RETURNS_TYPE(Address, Tagged<Object>, BUILTIN_CONVERT_RESULT, Name)

#define RUNTIME_FUNCTION_RETURN_PAIR(Name) RUNTIME_FUNCTION_RETURNS_TYPE(ObjectPair, ObjectPair, BUILTIN_CONVERT_RESULT_PAIR, Name)
