...
#define INTERRUPT_LIST(V)                                                                                                                                      \
    ...
    V(DEOPT_MARKED_ALLOCATION_SITES, DeoptMarkedAllocationSites, 5, InterruptLevel::kNoHeapWrites)                                                             \
    ...
    V(START_INCREMENTAL_MARKING, StartIncrementalMarking, 11, InterruptLevel::kNoHeapWrites)

#define V(NAME, Name, id, interrupt_level)                                                                                                                     \
    inline bool Check##Name()                                                                                                                                  \
    {                                                                                                                                                          \
        return CheckInterrupt(NAME);                                                                                                                           \
    }                                                                                                                                                          \
    inline void Request##Name()                                                                                                                                \
    {                                                                                                                                                          \
...
#undef V
    ...
    static_assert(InterruptFlag::ALL_INTERRUPTS < std::numeric_limits<uint32_t>::max());

    static constexpr InterruptFlag InterruptLevelMask(InterruptLevel level)
    {
#define V(NAME, Name, id, interrupt_level) | (interrupt_level <= level ? NAME : 0)
...
#undef V
    ...
    {
        return reinterpret_cast<Address>(&thread_local_.interrupt_requested_[static_cast<int>(level)]);
    }

    static constexpr int jslimit_offset()
    {
        return offsetof(StackGuard, thread_local_) + offsetof(ThreadLocal, jslimit_);
    }

    static constexpr int real_jslimit_offset()
    {
        return offsetof(StackGuard, thread_local_) + offsetof(ThreadLocal, real_jslimit_);
    }
...
private:
    ...
    // You should hold the ExecutionAccess lock when calling this method.
    inline void update_interrupt_requests_and_stack_limits(const ExecutionAccess& lock);
    ...
    public:
        ...
        {
            return base::Relaxed_Store(&jslimit_, static_cast<base::AtomicWord>(limit));
        }
        uintptr_t climit()
        {
        ...
        {
            return base::Relaxed_Store(&climit_, static_cast<base::AtomicWord>(limit));
        }

        // Interrupt request bytes can be read without any lock.
        // Writing requires the ExecutionAccess lock.
        base::Atomic8 interrupt_requested_[kNumberOfInterruptLevels] = { false, false, false };

        void set_interrupt_requested(InterruptLevel level, bool requested)
        {
            base::Relaxed_Store(&interrupt_requested_[static_cast<int>(level)], requested);
        }
