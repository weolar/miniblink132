...
#undef DECLARE_TYPE
    ...
    static constexpr bool IsTypeMarker(uintptr_t function_or_marker)
    {
        ...
        static_assert((std::numeric_limits<uintptr_t>::max() >> kSmiTagSize) > Type::NUMBER_OF_TYPES);
        return (function_or_marker & kSmiTagMask) == kSmiTag && function_or_marker < (Type::NUMBER_OF_TYPES << kSmiTagSize);
    }

    // Copy constructor; it breaks the connection to host iterator
    // (as an iterator usually lives on stack).
    StackFrame(const StackFrame& original) V8_NOEXCEPT : iterator_(nullptr), isolate_(original.isolate_), state_(original.state_)
    {
    }
    ...
    bool is_c_wasm_entry() const
    {
        return type() == C_WASM_ENTRY;
    }
    bool is_wasm_liftoff_setup() const
    {
...
#endif // V8_ENABLE_DRUMBRAKE
    bool is_wasm_debug_break() const
    {
    ...
    {
        return type() == API_ACCESSOR_EXIT;
    }
    bool is_api_callback_exit() const
    {
        return type() == API_CALLBACK_EXIT;
    }
    bool is_irregexp() const
    {
        return type() == IRREGEXP;
    }
    ...
    {
        return state_.callee_fp;
    }
    Address callee_pc() const
    {
        return state_.callee_pc;
    }
    ...
    {
        return state_.is_stack_exit_frame;
    }
    ...
    {
        return static_cast<StackFrameId>(caller_sp());
    }
...
#define FRAME_SUMMARY_VARIANTS(F)                                                                                                                              \
    ...
    IF_WASM_DRUMBRAKE(F, WASM_INTERPRETED, WasmInterpretedFrameSummary, wasm_interpreted_summary_, WasmInterpreted)                                            \
    IF_WASM(F, WASM_INLINED, WasmInlinedFrameSummary, wasm_inlined_summary_, WasmInlined)
...
#undef FRAME_SUMMARY_KIND
    ...
    public:
        JavaScriptFrameSummary(Isolate* isolate, Tagged<Object> receiver, Tagged<JSFunction> function, Tagged<AbstractCode> abstract_code, int code_offset,
        ...
        Handle<FixedArray> parameters() const
        {
            return parameters_;
        }
    ...
    public:
        WasmFrameSummary(Isolate* isolate, Handle<WasmTrustedInstanceData> instance_data, wasm::WasmCode* code, int byte_offset, int function_index,
            bool at_to_number_conversion);

        Handle<Object> receiver() const;
        uint32_t function_index() const;
        wasm::WasmCode* code() const
        {
            return code_;
        }
    ...
    public:
        WasmInlinedFrameSummary(Isolate* isolate, Handle<WasmTrustedInstanceData> instance_data, int function_index, int op_wire_bytes_offset);
    ...
    public:
        WasmInterpretedFrameSummary(Isolate*, Handle<WasmInstanceObject>, uint32_t function_index, int byte_offset);
        Handle<WasmInstanceObject> wasm_instance() const
        {
...
public:
    // Accessors.
    virtual Tagged<Object> context() const; // TODO(victorgomes): CommonFrames don't have context.
...
protected:
    inline explicit TurbofanStubWithContextFrame(StackFrameIteratorBase* iterator);
...
public:
    ...
    {
        return is_unoptimized_js();
    }
    bool is_optimized() const
    {
        return is_optimized_js();
    }
    bool is_turbofan() const
    {
        return is_turbofan_js();
    }
    ...
    static void PrintFunctionAndOffset(
...
public:
    ...
    wasm::WasmCode* wasm_code() const override
    {
        UNREACHABLE();
    }
...
protected:
    inline explicit JavaScriptBuiltinContinuationFrame(StackFrameIteratorBase* iterator);

private:
    friend class StackFrameIteratorBase;
};

class JavaScriptBuiltinContinuationWithCatchFrame : public JavaScriptBuiltinContinuationFrame {
public:
    Type type() const override
    {
...
protected:
    inline explicit JavaScriptBuiltinContinuationWithCatchFrame(StackFrameIteratorBase* iterator);
...
#if V8_ENABLE_WEBASSEMBLY
    wasm::StackMemory* wasm_stack() const
    {
        return wasm_stack_;
    }
#endif
...
public:
    ...
    struct NoHandles { };
    V8_EXPORT_PRIVATE StackFrameIterator(Isolate* isolate, ThreadLocalTop* top, NoHandles);
#if V8_ENABLE_WEBASSEMBLY
    ...
    V8_EXPORT_PRIVATE StackFrameIterator(Isolate* isolate, ThreadLocalTop* t, FirstStackOnly);
    // An iterator that iterates over a given wasm stack segment.
    V8_EXPORT_PRIVATE StackFrameIterator(Isolate* isolate, wasm::StackMemory* stack);
#endif
...
public:
    explicit JavaScriptStackFrameIterator(Isolate* isolate)
        : iterator_(isolate)
    {
        if (!done())
            Advance();
...
class V8_EXPORT_PRIVATE StackFrameIteratorForProfilerForTesting : public StackFrameIteratorForProfiler {
public:
...
public:
    ...
    {
        return { parameters_count_with_receiver, translation_height, is_topmost, pad_arguments, FrameInfoKind::kPrecise };
    }

    static UnoptimizedFrameInfo Conservative(int parameters_count_with_receiver, int locals_count)
    {
        return { parameters_count_with_receiver, locals_count, false, true, FrameInfoKind::kConservative };
    }
...
public:
    static BuiltinContinuationFrameInfo Precise(int translation_height, const CallInterfaceDescriptor& continuation_descriptor,
    ...
    {
        return { translation_height, continuation_descriptor, register_config, is_topmost, deopt_kind, continuation_mode, FrameInfoKind::kPrecise };
    }

    static BuiltinContinuationFrameInfo Conservative(
        int parameters_count, const CallInterfaceDescriptor& continuation_descriptor, const RegisterConfiguration* register_config)
    {
        ...
        return { parameters_count, continuation_descriptor, register_config, false, DeoptimizeKind::kEager, BuiltinContinuationMode::STUB,
...
private:
    BuiltinContinuationFrameInfo(int translation_height, const CallInterfaceDescriptor& continuation_descriptor, const RegisterConfiguration* register_config,
        bool is_topmost, DeoptimizeKind deopt_kind, BuiltinContinuationMode continuation_mode, FrameInfoKind frame_info_kind);
