...
public:
    ...
    template <bool clear_exception, typename Scope> void IncrementCallDepth(Scope* stack_allocated_scope)
    {
        ...
        if constexpr (clear_exception) {
            exception_ = Tagged<Object>(Internals::GetRoot(reinterpret_cast<v8::Isolate*>(isolate_), Internals::kTheHoleValueRootIndex));
        }
    }
    ...
    template <typename Scope> void DecrementCallDepth(Scope* stack_allocated_scope)
    {
    ...
    {
        return last_api_entry_ == kNullAddress;
    }
