...
Arguments<T>::ChangeValueScope::ChangeValueScope(Isolate* isolate, Arguments* args, int index, Tagged<Object> value)
...
template <ArgumentsType T> int Arguments<T>::smi_value_at(int index) const
{
...
template <ArgumentsType T> uint32_t Arguments<T>::positive_smi_value_at(int index) const
{
...
template <ArgumentsType T> int Arguments<T>::tagged_index_value_at(int index) const
{
    return static_cast<int>(Cast<TaggedIndex>((*this)[index]).value());
}

template <ArgumentsType T> double Arguments<T>::number_value_at(int index) const
{
    return Object::NumberValue((*this)[index]);
}

template <ArgumentsType T> Handle<Object> Arguments<T>::atOrUndefined(Isolate* isolate, int index) const
{
    if (index >= length_) {
