    ...
    if (regnum != kNoRegister) {
        *value = GetRegisterValue(regnum);
        ...
        if (strncmp(desc, "0x", 2) == 0) {
            return SScanF(desc + 2, "%" V8PRIxPTR, reinterpret_cast<uintptr_t*>(value)) == 1;
        } else {
    ...
    while (!done && !sim_->has_bad_pc()) {
        ...
        if (line == nullptr) {
            ...
            if ((strcmp(cmd, "si") == 0) || (strcmp(cmd, "stepi") == 0)) {
                ...
                if ((reinterpret_cast<Instruction*>(sim_->get_pc()))->InstructionBits() == 0x7D821008) {
                    sim_->set_pc(sim_->get_pc() + sizeof(FourByteInstr));
                } else {
                    sim_->ExecuteInstruction(reinterpret_cast<Instruction*>(sim_->get_pc()));
                }
                ...
                if (argc == 2 || (argc == 3 && strcmp(arg2, "fp") == 0)) {
                    ...
                    if (strcmp(arg1, "all") == 0) {
                        for (int i = 0; i < kNumRegisters; i++) {
                            ...
                            PrintF("    %3s: %08" V8PRIxPTR, RegisterName(Register::from_code(i)), value);
                        ...
                        for (int i = 0; i < kNumRegisters; i++) {
                            value = GetRegisterValue(i);
                            PrintF("     %3s: %08" V8PRIxPTR " %11" V8PRIdPTR, RegisterName(Register::from_code(i)), value, value);
                            if ((argc == 3 && strcmp(arg2, "fp") == 0) && i < 8 && (i % 2) == 0) {
                                dvalue = GetRegisterPairDoubleValue(i);
                                PrintF(" (%f)\n", dvalue);
                            } else if (!((i + 1) % 2)) {
                        ...
                        for (int i = 0; i < DoubleRegister::kNumRegisters; i++) {
                            ...
                            PrintF("%3s: %f 0x%08x\n", RegisterName(DoubleRegister::from_code(i)), fvalue, as_words);
                        }
                    } else if (strcmp(arg1, "alld") == 0) {
                        for (int i = 0; i < DoubleRegister::kNumRegisters; i++) {
                            dvalue = GetFPDoubleRegisterValue(i);
                            uint64_t as_words = base::bit_cast<uint64_t>(dvalue);
                            PrintF("%3s: %f 0x%08x %08x\n", RegisterName(DoubleRegister::from_code(i)), dvalue, static_cast<uint32_t>(as_words >> 32),
                                static_cast<uint32_t>(as_words & 0xFFFFFFFF));
                        }
                    } else if (arg1[0] == 'r'
                        && (arg1[1] >= '0' && arg1[1] <= '2' && (arg1[2] == '\0' || (arg1[2] >= '0' && arg1[2] <= '5' && arg1[3] == '\0')))) {
                ...
                while (cur < end) {
                    PrintF("  0x%08" V8PRIxPTR ":  0x%08" V8PRIxPTR " %10" V8PRIdPTR, reinterpret_cast<intptr_t>(cur), *cur, *cur);
                ...
                intptr_t value;
                intptr_t stop_pc = sim_->get_pc() - (sizeof(FourByteInstr) + kSystemPointerSize);
...
void Simulator::CheckICache(base::CustomMatcherHashMap* i_cache, Instruction* instr)
{
    ...
    if (cache_hit) {
        ...
        CHECK_EQ(memcmp(reinterpret_cast<void*>(instr), cache_page->CachedData(offset), sizeof(FourByteInstr)), 0);
    } else {
...
#define CREATE_EVALUATE_TABLE(name, op_name, op_value) EvalTable[op_name] = &Simulator::Evaluate_##op_name;
...
template <typename T> T Simulator::get_low_register(int reg) const
{
    DCHECK((reg >= 0) && (reg < kNumGPRs));
...
template <typename T> T Simulator::get_high_register(int reg) const
{
    DCHECK((reg >= 0) && (reg < kNumGPRs));
    // Stupid code added to avoid bug in GCC.
    // See: http://gcc.gnu.org/bugzilla/show_bug.cgi?id=43949
...
template <class T, class R> static R ComputeSignedRoundingResult(T a, T n)
{
    constexpr T NINF = -std::numeric_limits<T>::infinity();
    constexpr T PINF = std::numeric_limits<T>::infinity();
    constexpr long double MN = static_cast<long double>(std::numeric_limits<R>::min());
    constexpr long double MP = static_cast<long double>(std::numeric_limits<R>::max());
...
template <class T, class R> static R ComputeLogicalRoundingResult(T a, T n)
{
    constexpr T NINF = -std::numeric_limits<T>::infinity();
    constexpr T PINF = std::numeric_limits<T>::infinity();
    constexpr long double MP = static_cast<long double>(std::numeric_limits<R>::max());
...
template <typename T1> bool Simulator::OverflowFromSigned(T1 alu_out, T1 left, T1 right, bool addition)
{
    bool overflow;
    if (addition) {
...
using SimulatorRuntimeCall = intptr_t (*)(intptr_t arg0, intptr_t arg1, intptr_t arg2, intptr_t arg3, intptr_t arg4, intptr_t arg5, intptr_t arg6,
    intptr_t arg7, intptr_t arg8, intptr_t arg9, intptr_t arg10, intptr_t arg11, intptr_t arg12, intptr_t arg13, intptr_t arg14, intptr_t arg15, intptr_t arg16,
    intptr_t arg17, intptr_t arg18, intptr_t arg19);
using SimulatorRuntimePairCall = ObjectPair (*)(intptr_t arg0, intptr_t arg1, intptr_t arg2, intptr_t arg3, intptr_t arg4, intptr_t arg5, intptr_t arg6,
    intptr_t arg7, intptr_t arg8, intptr_t arg9, intptr_t arg10, intptr_t arg11, intptr_t arg12, intptr_t arg13, intptr_t arg14, intptr_t arg15, intptr_t arg16,
    intptr_t arg17, intptr_t arg18, intptr_t arg19);
...
void Simulator::SoftwareInterrupt(Instruction* instr)
{
    ...
    case kCallRtRedirected: {
        ...
        for (int i = kRegisterArgCount; i < kArgCount; i++) {
            arg[i] = stack_pointer[(kCalleeRegisterSaveAreaSize / kSystemPointerSize) + (i - kRegisterArgCount)];
        }
        ...
        *reinterpret_cast<intptr_t*>(get_register(sp) + kStackFrameRASlot * kSystemPointerSize) = get_register(r14);
        ...
        if (fp_call) {
            ...
            if (v8_flags.trace_sim || !stack_aligned) {
                ...
                case ExternalReference::BUILTIN_FP_INT_CALL:
                    PrintF("Call to host function at %p with args %f, %" V8PRIdPTR, reinterpret_cast<void*>(FUNCTION_ADDR(generic_target)), dval0, ival);
                    break;
                default:
                    UNREACHABLE();
                }
            ...
            if (v8_flags.trace_sim || !stack_aligned) {
                PrintF("Call to host function at %p args %08" V8PRIxPTR, reinterpret_cast<void*>(external), arg[0]);
                if (!stack_aligned) {
                    PrintF(" with unaligned stack %08" V8PRIxPTR "\n", get_register(sp));
                }
                PrintF("\n");
            ...
            if (v8_flags.trace_sim || !stack_aligned) {
                PrintF("Call to host function at %p args %08" V8PRIxPTR " %08" V8PRIxPTR, reinterpret_cast<void*>(external), arg[0], arg[1]);
                if (!stack_aligned) {
                    PrintF(" with unaligned stack %08" V8PRIxPTR "\n", static_cast<intptr_t>(get_register(sp)));
                }
                PrintF("\n");
            ...
            if (v8_flags.trace_sim || !stack_aligned) {
                ...
                PrintF("Call to host function at %p,\n"
                       "\t\t\t\targs %08" V8PRIxPTR ", %08" V8PRIxPTR ", %08" V8PRIxPTR ", %08" V8PRIxPTR ", %08" V8PRIxPTR ", %08" V8PRIxPTR ", %08" V8PRIxPTR
                       ", %08" V8PRIxPTR ", %08" V8PRIxPTR ", %08" V8PRIxPTR ", %08" V8PRIxPTR ", %08" V8PRIxPTR ", %08" V8PRIxPTR ", %08" V8PRIxPTR
                       ", %08" V8PRIxPTR ", %08" V8PRIxPTR ", %08" V8PRIxPTR ", %08" V8PRIxPTR ", %08" V8PRIxPTR ", %08" V8PRIxPTR,
                    reinterpret_cast<void*>(FUNCTION_ADDR(target)), arg[0], arg[1], arg[2], arg[3], arg[4], arg[5], arg[6], arg[7], arg[8], arg[9], arg[10],
            ...
            if (redirection->type() == ExternalReference::BUILTIN_CALL_PAIR) {
                ...
                ObjectPair result = target(arg[0], arg[1], arg[2], arg[3], arg[4], arg[5], arg[6], arg[7], arg[8], arg[9], arg[10], arg[11], arg[12], arg[13],
                ...
                if (ABI_RETURNS_OBJECTPAIR_IN_REGS) {
                    ...
                    memcpy(reinterpret_cast<void*>(result_buffer), &result, sizeof(ObjectPair));
                ...
                DCHECK(redirection->type() == ExternalReference::BUILTIN_CALL || redirection->type() == ExternalReference::FAST_C_CALL);
                SimulatorRuntimeCall target = reinterpret_cast<SimulatorRuntimeCall>(external);
                intptr_t result = target(arg[0], arg[1], arg[2], arg[3], arg[4], arg[5], arg[6], arg[7], arg[8], arg[9], arg[10], arg[11], arg[12], arg[13],
                    arg[14], arg[15], arg[16], arg[17], arg[18], arg[19]);
                if (v8_flags.trace_sim) {
        ...
        int64_t saved_lr = *reinterpret_cast<intptr_t*>(get_register(sp) + kStackFrameRASlot * kSystemPointerSize);
        set_pc(saved_lr);
        break;
    }
    case kBreakpoint:
...
#define CheckOverflowForIntAdd(src1, src2, type) OverflowFromSigned<type>(src1 + src2, src1, src2, true);

#define CheckOverflowForIntSub(src1, src2, type) OverflowFromSigned<type>(src1 - src2, src1, src2, false);

// Method for checking overflow on unsigned addition
#define CheckOverflowForUIntAdd(src1, src2) ((src1) + (src2) < (src1) || (src1) + (src2) < (src2))
...
#define CheckOverflowForShiftRight(src1, src2) (((src1) >> (src2)) << (src2) != (src1))

// Method for checking overflow on shift left
#define CheckOverflowForShiftLeft(src1, src2) (((src1) << (src2)) >> (src2) != (src1))
...
void Simulator::ExecuteInstruction(Instruction* instr, bool auto_incr_pc)
{
    ...
    if (v8_flags.trace_sim) {
        ...
        PrintF("%05" PRId64 "  %08" V8PRIxPTR "  %s\n", icount_, reinterpret_cast<intptr_t>(instr), buffer.begin());
...
intptr_t Simulator::CallImpl(Address entry, int argument_count, const intptr_t* arguments)
{
    ...
    memcpy(stack_argument, arguments + reg_arg_count, stack_arg_count * sizeof(*arguments));
...
#define GET_ADDRESS(index_reg, base_reg, offset) (((index_reg) == 0) ? 0 : get_register(index_reg)) + (((base_reg) == 0) ? 0 : get_register(base_reg)) + offset
...
#define VECTOR_LOAD_POSITIVE(r1, r2, type)                                                                                                                     \
    for (size_t i = 0, j = 0; j < kSimd128Size; i++, j += sizeof(type)) {                                                                                      \
        set_simd_register_by_lane<type>(r1, i, abs(get_simd_register_by_lane<type>(r2, i)));                                                                   \
    }
EVALUATE(VLP)
{
    DCHECK_OPCODE(VLP);
...
#define VECTOR_AVERAGE_U(r1, r2, r3, type)                                                                                                                     \
    for (size_t i = 0, j = 0; j < kSimd128Size; i++, j += sizeof(type)) {                                                                                      \
        ...
        set_simd_register_by_lane<type>(r1, i, (static_cast<type>(src0) + static_cast<type>(src1) + 1) >> 1);                                                  \
    }
EVALUATE(VAVGL)
{
    DCHECK_OPCODE(VAVGL);
...
#define CASE(i, type)                                                                                                                                          \
    case i:                                                                                                                                                    \
        set_simd_register_by_lane<type>(r1, index, static_cast<type>(get_register(r3)));                                                                       \
        break;
    switch (m4) {
        CASE(0, uint8_t);
        CASE(1, uint16_t);
...
#define FOR_EACH_LANE(i, type) for (uint32_t i = 0; i < kSimd128Size / sizeof(type); i++)
...
#define CASE(i, type)                                                                                                                                          \
    case i: {                                                                                                                                                  \
        ...
        {                                                                                                                                                      \
            set_simd_register_by_lane<type>(r1, j, get_simd_register_by_lane<type>(r3, i2));                                                                   \
        }                                                                                                                                                      \
        break;                                                                                                                                                 \
    }
    switch (m4) {
...
template <class T, class Operation> inline static void VectorBinaryOp(Simulator* sim, int dst, int src1, int src2, Operation op)
{
    FOR_EACH_LANE(i, T)
    {
...
#define VECTOR_BINARY_OP_FOR_TYPE(type, op) VectorBinaryOp<type>(this, r1, r2, r3, [](type a, type b) { return a op b; });
...
#define VECTOR_MULTIPLY_EVEN_ODD_TYPE(r1, r2, r3, input_type, result_type, is_odd)                                                                             \
    size_t i = 0, j = 0, k = 0;                                                                                                                                \
    ...
    for (; j < kSimd128Size; i += 2, j += lane_size * 2, k++) {                                                                                                \
        result_type src0 = static_cast<result_type>(get_simd_register_by_lane<input_type>(r2, i));                                                             \
        result_type src1 = static_cast<result_type>(get_simd_register_by_lane<input_type>(r3, i));                                                             \
        set_simd_register_by_lane<result_type>(r1, k, src0 * src1);                                                                                            \
    }
#define VECTOR_MULTIPLY_EVEN_ODD(r1, r2, r3, is_odd, sign)                                                                                                     \
    switch (m4) {                                                                                                                                              \
    case 0: {                                                                                                                                                  \
        VECTOR_MULTIPLY_EVEN_ODD_TYPE(r1, r2, r3, sign##int8_t, sign##int16_t, is_odd)                                                                         \
        break;                                                                                                                                                 \
    }                                                                                                                                                          \
    case 1: {                                                                                                                                                  \
        VECTOR_MULTIPLY_EVEN_ODD_TYPE(r1, r2, r3, sign##int16_t, sign##int32_t, is_odd)                                                                        \
        break;                                                                                                                                                 \
    }                                                                                                                                                          \
    case 2: {                                                                                                                                                  \
        VECTOR_MULTIPLY_EVEN_ODD_TYPE(r1, r2, r3, sign##int32_t, sign##int64_t, is_odd)                                                                        \
        break;                                                                                                                                                 \
    }                                                                                                                                                          \
    default:                                                                                                                                                   \
        UNREACHABLE();                                                                                                                                         \
...
template <class S, class D> void VectorSum(Simulator* sim, int dst, int src1, int src2)
{
    D value = 0;
    FOR_EACH_LANE(i, S)
    {
        ...
        if ((i + 1) % (sizeof(D) / sizeof(S)) == 0) {
            ...
            sim->set_simd_register_by_lane<D>(dst, i / (sizeof(D) / sizeof(S)), value);
            value = 0;
        }
    }
}
...
#define VECTOR_MERGE(type, is_low_side)                                                                                                                        \
    ...
    for (size_t i = 0, source_index = is_low_side ? i + index_limit : i; i < index_limit; i++, source_index++) {                                               \
        set_simd_register_by_lane<type>(r1, 2 * i, get_simd_register_by_lane<type>(r2, source_index));                                                         \
        set_simd_register_by_lane<type>(r1, (2 * i) + 1, get_simd_register_by_lane<type>(r3, source_index));                                                   \
    }
...
template <class S, class D> void VectorPack(Simulator* sim, int dst, int src1, int src2, bool saturate, const D& max = 0, const D& min = 0)
{
    int src = src1;
    int count = 0;
    S value = 0;
    ...
    FOR_EACH_LANE(i, D)
    {
        sim->set_simd_register_by_lane<D>(dst, i, temps[i]);
    }
}
...
#undef CASE
template <class S, class D> void VectorUnpackHigh(Simulator* sim, int dst, int src)
{
    ...
    FOR_EACH_LANE(i, D)
    {
        temps[i] = sim->get_simd_register_by_lane<S>(src, i);
    }
    FOR_EACH_LANE(i, D)
    {
        sim->set_simd_register_by_lane<D>(dst, i, temps[i]);
...
template <class S> void VectorPopulationCount(Simulator* sim, int dst, int src)
{
    FOR_EACH_LANE(i, S)
    {
        sim->set_simd_register_by_lane<S>(dst, i, base::bits::CountPopulation(sim->get_simd_register_by_lane<S>(src, i)));
    }
}
...
#define CASE(i, S, D)                                                                                                                                          \
    case i: {                                                                                                                                                  \
        ...
        {                                                                                                                                                      \
            set_simd_register_by_lane<D>(r1, index, static_cast<D>(get_simd_register_by_lane<S>(r2, index)));                                                  \
        }                                                                                                                                                      \
        break;                                                                                                                                                 \
    }
EVALUATE(VCDG)
...
#define CASE(i, S, D, type)                                                                                                                                    \
    case i: {                                                                                                                                                  \
        ...
        {                                                                                                                                                      \
            ...
            set_simd_register_by_lane<D>(r1, index, static_cast<D>(Compute##type##RoundingResult<S, D>(a, n)));                                                \
        }                                                                                                                                                      \
        break;                                                                                                                                                 \
    }
EVALUATE(VCGD)
...
template <class S, class D> void VectorUnpackLow(Simulator* sim, int dst, int src)
{
    constexpr size_t kItemCount = kSimd128Size / sizeof(D);
    D temps[kItemCount] = { 0 };
    // About overwriting if src and dst are the same register.
...
#define VECTOR_MAX_MIN_FOR_TYPE(type, op) VectorBinaryOp<type>(this, r1, r2, r3, [](type a, type b) { return (a op b) ? a : b; });
...
#define VECTOR_COMPARE_FOR_TYPE(type, op) VectorBinaryOp<type>(this, r1, r2, r3, [](type a, type b) { return (a op b) ? -1 : 0; });
...
template <class T> void VectorLoadComplement(Simulator* sim, int dst, int src)
{
    FOR_EACH_LANE(i, T)
    {
...
EVALUATE(VBPERM)
{
    ...
    unsigned __int128 src_bits = base::bit_cast<__int128>(get_simd_register(r2).int8);
    for (int i = 0; i < kSimd128Size; i++) {
        result_bits <<= 1;
        uint8_t selected_bit_index = get_simd_register_by_lane<uint8_t>(r3, i);
        if (selected_bit_index < (kSimd128Size * kBitsPerByte)) {
            unsigned __int128 bit_value = (src_bits << selected_bit_index) >> (kSimd128Size * kBitsPerByte - 1);
    ...
    set_simd_register_by_lane<uint8_t>(r1, 6, static_cast<uint8_t>(result_bits >> 8));
    set_simd_register_by_lane<uint8_t>(r1, 7, static_cast<uint8_t>((result_bits << 8) >> 8));
    return length;
}
...
EVALUATE(VSEL)
{
    ...
    USE(m5);
    USE(m6);
    unsigned __int128 src_1 = base::bit_cast<__int128>(get_simd_register(r2).int8);
    unsigned __int128 src_2 = base::bit_cast<__int128>(get_simd_register(r3).int8);
    unsigned __int128 src_3 = base::bit_cast<__int128>(get_simd_register(r4).int8);
...
template <class T, class Operation> void VectorShift(Simulator* sim, int dst, int src, unsigned int shift, Operation op)
{
    FOR_EACH_LANE(i, T)
    {
        T src_val = sim->get_simd_register_by_lane<T>(src, i);
...
#define VECTOR_SHIFT_FOR_TYPE(type, op, shift) VectorShift<type>(this, r1, r3, shift, [](type a, unsigned int shift) { return a op shift; });
...
#define VECTOR_SHIFT_WITH_OPERAND_TYPE(r1, r2, r3, type, op)                                                                                                   \
    for (size_t i = 0, j = 0; j < kSimd128Size; i++, j += sizeof(type)) {                                                                                      \
        type src0 = get_simd_register_by_lane<type>(r2, i);                                                                                                    \
        type src1 = get_simd_register_by_lane<type>(r3, i);                                                                                                    \
        set_simd_register_by_lane<type>(r1, i, src0 op(src1 % (sizeof(type) * 8)));                                                                            \
    }
...
#define VECTOR_FP_MULTIPLY_QFMS_OPERATION(type, op, sign, first_lane_only, function)                                                                           \
    for (size_t i = 0, j = 0; j < kSimd128Size; i++, j += sizeof(type)) {                                                                                      \
        type src0 = get_simd_register_by_lane<type>(r2, i);                                                                                                    \
        type src1 = get_simd_register_by_lane<type>(r3, i);                                                                                                    \
...
template <class FP_Type> static FP_Type JavaMathMax(FP_Type x, FP_Type y)
{
...
template <class FP_Type> static FP_Type IEEE_maxNum(FP_Type x, FP_Type y)
{
    if (x > y)
        return x;
    if (x < y)
...
template <class FP_Type> static FP_Type FPMax(int m6, FP_Type lhs, FP_Type rhs)
{
    switch (m6) {
    case 0:
...
template <class FP_Type> static FP_Type JavaMathMin(FP_Type x, FP_Type y)
{
    if (isnan(x) || isnan(y))
        return NAN;
...
template <class FP_Type> static FP_Type IEEE_minNum(FP_Type x, FP_Type y)
{
    if (x > y)
        return y;
    if (x < y)
...
template <class FP_Type> static FP_Type FPMin(int m6, FP_Type lhs, FP_Type rhs)
{
    switch (m6) {
    case 0:
...
template <class FP_Type, class Operation> static void FPMinMaxForEachLane(Simulator* sim, Operation Op, int dst, int lhs, int rhs, int m5, int m6)
{
    DCHECK(m5 == 8 || m5 == 0);
    if (m5 == 8) {
...
template <class S, class D, class Operation> void VectorFPCompare(Simulator* sim, int dst, int src1, int src2, int m6, Operation op)
{
    static_assert(sizeof(S) == sizeof(D), "Expect input type size == output type size");
...
#define VECTOR_FP_COMPARE_FOR_TYPE(S, D, op) VectorFPCompare<S, D>(this, r1, r2, r3, m6, [](S a, S b) { return (a op b) ? -1 : 0; });
...
template <class T, class Op> static void VectorUnaryOp(Simulator* sim, int dst, int src, int sec, Op op)
{
    if (sec == 8) {
...
template <class T> void VectorSignOp(Simulator* sim, int dst, int src, int m4, int m5)
{
    switch (m5) {
...
EVALUATE(EX)
{
    ...
    SixByteInstr the_instr = Instruction::InstructionBits(reinterpret_cast<const uint8_t*>(b2_val + x2_val + d2_val));
    int inst_length = Instruction::InstructionLength(reinterpret_cast<const uint8_t*>(b2_val + x2_val + d2_val));

    char new_instr_buf[8];
    char* addr = reinterpret_cast<char*>(&new_instr_buf[0]);
    the_instr |= static_cast<SixByteInstr>(r1_val & 0xFF) << (8 * inst_length - 16);
    Instruction::SetInstructionBits<SixByteInstr>(reinterpret_cast<uint8_t*>(addr), static_cast<SixByteInstr>(the_instr));
...
inline static int TestUnderMask(uint16_t val, uint16_t mask, bool is_tm_or_tmy)
{
...
template <class T, class R> static int ComputeSignedRoundingConditionCode(T a, T n)
{
    constexpr T NINF = -std::numeric_limits<T>::infinity();
    constexpr T PINF = std::numeric_limits<T>::infinity();
    constexpr long double MN = static_cast<long double>(std::numeric_limits<R>::min());
...
template <class T, class R> static int ComputeLogicalRoundingConditionCode(T a, T n)
{
    constexpr T NINF = -std::numeric_limits<T>::infinity();
    constexpr T PINF = std::numeric_limits<T>::infinity();
    constexpr long double MP = static_cast<long double>(std::numeric_limits<R>::max());
...
EVALUATE(CS)
{
    ...
    bool is_success = __atomic_compare_exchange_n(reinterpret_cast<int32_t*>(target_addr), &r1_val, r3_val, true, __ATOMIC_SEQ_CST, __ATOMIC_SEQ_CST);
    if (!is_success) {
        set_low_register(r1, r1_val);
        condition_reg_ = 0x4;
    } else {
...
EVALUATE(CSG)
{
    ...
    DCHECK_EQ(target_addr & 0x3, 0);
    bool is_success = __atomic_compare_exchange_n(reinterpret_cast<int64_t*>(target_addr), &r1_val, r3_val, true, __ATOMIC_SEQ_CST, __ATOMIC_SEQ_CST);
    if (!is_success) {
        set_register(r1, r1_val);
        condition_reg_ = 0x4;
    } else {
...
#define ATOMIC_LOAD_AND_UPDATE_WORD32(op)                                                                                                                      \
    ...
    int32_t r3_val = get_low_register<int32_t>(r3);                                                                                                            \
    DCHECK_EQ(addr & 0x3, 0);                                                                                                                                  \
    int32_t r1_val = op(reinterpret_cast<int32_t*>(addr), r3_val, __ATOMIC_SEQ_CST);                                                                           \
    set_low_register(r1, r1_val);
