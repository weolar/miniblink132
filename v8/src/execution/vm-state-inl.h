...
template <StateTag Tag> VMState<Tag>::~VMState()
{
    isolate_->set_current_vm_state(previous_tag_);
}

ExternalCallbackScope::ExternalCallbackScope(Isolate* isolate, Address callback, v8::ExceptionContext exception_context, const void* callback_info)
    : callback_(callback)
    ...
    , pause_timed_histogram_scope_(isolate->counters()->execute())
{
    ...
    TRACE_EVENT_BEGIN0(TRACE_DISABLED_BY_DEFAULT("v8.runtime"), "V8.ExternalCallback");
#endif
...
ExternalCallbackScope::~ExternalCallbackScope()
{
    ...
    vm_state_.isolate_->clear_topmost_script_having_context();
#ifdef V8_RUNTIME_CALL_STATS
    TRACE_EVENT_END0(TRACE_DISABLED_BY_DEFAULT("v8.runtime"), "V8.ExternalCallback");
#endif
#if USE_SIMULATOR || V8_USE_ADDRESS_SANITIZER || V8_USE_SAFE_STACK
