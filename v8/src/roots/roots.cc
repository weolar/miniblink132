...
{
    visitor->VisitRootPointers(Root::kReadOnlyRootList, nullptr, FullObjectSlot(read_only_roots_), FullObjectSlot(&read_only_roots_[kEntriesCount]));
...
void ReadOnlyRoots::VerifyNameForProtectors()
{
    ...
    Tagged<Name> prev;
    for (RootIndex root_index = RootIndex::kFirstNameForProtector; root_index <= RootIndex::kLastNameForProtector; ++root_index) {
        ...
        if (root_index != RootIndex::kFirstNameForProtector) {
            ...
            Address computed_address = prev.address() + ALIGN_TO_ALLOCATION_ALIGNMENT(prev->Size());
...
#define ROOT_TYPE_CHECK(Type, name, CamelName)                                                                                                                 \
     ...
     * compatibility static_asserts in the Tagged class. */                                                                            \
        if (std::is_same_v<Type, Undefined>) {                                                                                                                 \
            return Cast<Oddball>(Tagged<Object>(value))->kind() == Oddball::kUndefined;                                                                        \
...
IndirectHandle<HeapNumber> ReadOnlyRoots::FindHeapNumber(double value)
{
    ...
    for (auto pos = RootIndex::kFirstHeapNumberRoot; pos <= RootIndex::kLastHeapNumberRoot; ++pos) {
