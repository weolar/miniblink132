...
bool RootsTable::IsRootHandleLocation(Address* handle_location, RootIndex* index) const
{
...
template <typename T> bool RootsTable::IsRootHandle(IndirectHandle<T> handle, RootIndex* index) const
{
...
ReadOnlyRoots::ReadOnlyRoots(const Isolate* isolate)
    : read_only_roots_(reinterpret_cast<Address*>(isolate->roots_table().read_only_roots_begin().address()))
{
}
...
{
    return value ? IndirectHandle<Boolean>(true_value_handle()) : IndirectHandle<Boolean>(false_value_handle());
}
...
Address* ReadOnlyRoots::GetLocation(RootIndex root_index) const
{
    ...
    DCHECK_IMPLIES(*location == kNullAddress, root_index == RootIndex::kFreeSpaceMap);
    return location;
}
...
{
    return base::IsInRange(object.ptr(), first_name_for_protector(), last_name_for_protector());
}
...
void ReadOnlyRoots::VerifyNameForProtectorsPages() const
{
    ...
    CHECK_EQ(PageMetadata::FromAddress(first_name_for_protector()), PageMetadata::FromAddress(last_name_for_protector()));
}
...
Address ReadOnlyRoots::address_at(RootIndex root_index) const
{
    ...
    return V8HeapCompressionScheme::DecompressTagged(V8HeapCompressionScheme::base(), StaticReadOnlyRootsPointerTable[static_cast<int>(root_index)]);
#else
