...
template <int radix> char* BasecaseFixedLast(digit_t chunk, char* out)
{
    while (chunk != 0) {
...
template <digit_t radix> char* DivideByMagic(RWDigits rest, Digits input, char* output)
{
    constexpr uint8_t max_bits_per_char = kMaxBitsPerChar[radix];
    constexpr int chunk_chars = kHalfDigitBits * kBitsPerCharTableMultiplier / max_bits_per_char;
...
public:
    ToStringFormatter(Digits X, int radix, bool sign, char* out, uint32_t chars_available, ProcessorImpl* processor)
        : digits_(X)
        , radix_(radix)
        , sign_(sign)
        , out_start_(out)
    ...
    void Fast();
    char* FillWithZeros(RecursionLevel* level, char* prev_cursor, char* out, bool is_last_on_level);
    char* ProcessLevel(RecursionLevel* level, Digits chunk, char* out, bool is_last_on_level);
...
class RecursionLevel {
public:
    static RecursionLevel* CreateLevels(digit_t base_divisor, int base_char_count, int target_bit_length, ProcessorImpl* processor);
    ~RecursionLevel()
    {
        delete next_;
    }
...
// static
RecursionLevel* RecursionLevel::CreateLevels(digit_t base_divisor, int base_char_count, int target_bit_length, ProcessorImpl* processor)
{
...
void RecursionLevel::ComputeInverse(ProcessorImpl* processor, int dividend_length)
{
...
{
    std::unique_ptr<RecursionLevel> recursion_levels(RecursionLevel::CreateLevels(chunk_divisor_, chunk_chars_, BitLength(digits_), processor_));
...
char* ToStringFormatter::FillWithZeros(RecursionLevel* level, char* right_boundary, char* out, bool is_last_on_level)
{
...
char* ToStringFormatter::ProcessLevel(RecursionLevel* level, Digits chunk, char* out, bool is_last_on_level)
{
    ...
    ShiftedDigits chunk_shifted(chunk, level->leading_zero_shift_, allow_inplace_modification);
    chunk = chunk_shifted;
    chunk.Normalize();
    ...
    if (inverse_len == 0) {
        ...
        processor_->DivideBarrett(left, right, chunk, level->divisor_, inverse, scratch);
        if (processor_->should_terminate())
            return out;
    }
    ...
    return ProcessLevel(level->next_, left, out - level->char_count_, is_last_on_level);
}
...
void ProcessorImpl::ToString(char* out, uint32_t* out_length, Digits X, int radix, bool sign)
{
...
Status Processor::ToString(char* out, uint32_t* out_length, Digits X, int radix, bool sign)
{
...
uint32_t ToStringResultLength(Digits X, int radix, bool sign)
{
    ...
    if (IsPowerOfTwo(radix)) {
        ...
        DCHECK(chars_required < static_cast<uint64_t>(std::numeric_limits<uint32_t>::max()));
