...
void KaratsubaSubtractionHelper(RWDigits result, Digits X, Digits Y, int* sign)
{
    X.Normalize();
    Y.Normalize();
    digit_t borrow = 0;
...
void ProcessorImpl::KaratsubaStart(RWDigits Z, Digits X, Digits Y, RWDigits scratch, int k)
{
    KaratsubaMain(Z, X, Y, scratch, k);
    MAYBE_TERMINATE
    for (int i = 2 * k; i < Z.len(); i++)
        Z[i] = 0;
...
void ProcessorImpl::KaratsubaChunk(RWDigits Z, Digits X, Digits Y, RWDigits scratch)
{
    X.Normalize();
    Y.Normalize();
    if (X.len() == 0 || Y.len() == 0)
...
void ProcessorImpl::KaratsubaMain(RWDigits Z, Digits X, Digits Y, RWDigits scratch, int n)
{
