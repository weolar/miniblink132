...
inline digit_t digit_sub2(digit_t a, digit_t b, digit_t borrow_in, digit_t* borrow_out)
{
#if HAVE_TWODIGIT_T
...
inline digit_t digit_mul(digit_t a, digit_t b, digit_t* high)
{
    ...
    *high = result >> kDigitBits;
    ...
    digit_t carry = 0;
    digit_t low = digit_add3(r_low, r_mid1 << kHalfDigitBits, r_mid2 << kHalfDigitBits, &carry);
...
static inline digit_t digit_div(digit_t high, digit_t low, digit_t divisor, digit_t* remainder)
{
#if defined(DCHECK)
    DCHECK(high < divisor);
    DCHECK(divisor != 0);
    ...
    *remainder = rem;
    ...
    static_assert(sizeof(intptr_t) == sizeof(digit_t), "intptr_t and digit_t must have the same size");
