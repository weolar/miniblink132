...
public:
    ...
    Digits()
        : Digits(static_cast<digit_t*>(nullptr), 0)
    {
    }
...
public:
    ...
    public:
        ...
        {
            memcpy(ptr_, &digit, sizeof(digit));
        }
    ...
    void Clear()
    {
        memset(digits_, 0, len_ * sizeof(digit_t));
    }
};
...
{
    return same_sign ? AddResultLength(x_length, y_length) : std::max(x_length, y_length);
}
inline int SubtractResultLength(int x_length, int y_length)
{
    return x_length;
}
inline int SubtractSignedResultLength(int x_length, int y_length, bool same_sign)
{
    return same_sign ? std::max(x_length, y_length) : AddResultLength(x_length, y_length);
}
...
public:
    ...
    template <class CharIt> ALWAYS_INLINE CharIt Parse(CharIt start, CharIt end, digit_t radix);
...
private:
    ...
    template <class CharIt> ALWAYS_INLINE CharIt ParsePowerTwo(CharIt start, CharIt end, digit_t radix);
...
template <class CharIt> CharIt FromStringAccumulator::ParsePowerTwo(CharIt current, CharIt end, digit_t radix)
{
...
template <class CharIt> CharIt FromStringAccumulator::Parse(CharIt start, CharIt end, digit_t radix)
{
    ...
    do {
        ...
        if (!AddPart(multiplier, part, done))
            return current;
    } while (!done);
    return current;
}

bool FromStringAccumulator::AddPart(digit_t multiplier, digit_t part, bool is_last)
{
#if HAVE_TWODIGIT_T
