...
// set.
constexpr PerThreadAsserts kInitialValue = ~PerThreadAsserts { HANDLE_USAGE_ON_ALL_THREADS_ASSERT };
...
template <bool kAllow, PerThreadAssertType... kTypes> PerThreadAssertScope<kAllow, kTypes...>::~PerThreadAssertScope()
{
    Release();
}

template <bool kAllow, PerThreadAssertType... kTypes> void PerThreadAssertScope<kAllow, kTypes...>::Release()
{
    if (old_data_ == kClearedValue)
        return;
...
// static
template <bool kAllow, PerThreadAssertType... kTypes> bool PerThreadAssertScope<kAllow, kTypes...>::IsAllowed()
{
...
#define PER_ISOLATE_ASSERT_SCOPE_DEFINITION(ScopeType, field, enable)                                                                                          \
    ...
    ScopeType::~ScopeType()                                                                                                                                    \
    {                                                                                                                                                          \
        isolate_->set_##field(old_data_);                                                                                                                      \
    }                                                                                                                                                          \
                                                                                                                                                               \
    /* static */                                                                                                                                               \
    bool ScopeType::IsAllowed(Isolate* isolate)                                                                                                                \
    {                                                                                                                                                          \
        return isolate->field();                                                                                                                               \
    }                                                                                                                                                          \
                                                                                                                                                               \
    /* static */                                                                                                                                               \
...
#define PER_ISOLATE_ASSERT_ENABLE_SCOPE_DEFINITION(EnableType, _, field, enable) PER_ISOLATE_ASSERT_SCOPE_DEFINITION(EnableType, field, enable)

#define PER_ISOLATE_ASSERT_DISABLE_SCOPE_DEFINITION(_, DisableType, field, enable) PER_ISOLATE_ASSERT_SCOPE_DEFINITION(DisableType, field, enable)
...
template class PerThreadAssertScope<false, SAFEPOINTS_ASSERT, HEAP_ALLOCATION_ASSERT>;
template class PerThreadAssertScope<true, SAFEPOINTS_ASSERT, HEAP_ALLOCATION_ASSERT>;
template class PerThreadAssertScope<false, CODE_DEPENDENCY_CHANGE_ASSERT, HANDLE_DEREFERENCE_ASSERT, HANDLE_ALLOCATION_ASSERT, HEAP_ALLOCATION_ASSERT>;
template class PerThreadAssertScope<true, CODE_DEPENDENCY_CHANGE_ASSERT, HANDLE_DEREFERENCE_ASSERT, HANDLE_ALLOCATION_ASSERT, HEAP_ALLOCATION_ASSERT>;

static_assert(Internals::kDisallowGarbageCollectionAlign == alignof(DisallowGarbageCollectionInRelease));
static_assert(Internals::kDisallowGarbageCollectionSize == sizeof(DisallowGarbageCollectionInRelease));
