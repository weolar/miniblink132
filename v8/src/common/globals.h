...
#if defined(USE_SIMULATOR)                                                                                                                                     \
    && (defined(V8_TARGET_ARCH_ARM64) || defined(V8_TARGET_ARCH_MIPS64) || defined(V8_TARGET_ARCH_LOONG64) || defined(V8_TARGET_ARCH_RISCV64))
...
static_assert(V8_DEFAULT_STACK_SIZE_KB * KB + kStackLimitSlackForDeoptimizationInBytes <= MB);
...
#if (defined(V8_SHORT_BUILTIN_CALLS) && !defined(V8_COMPRESS_POINTERS)) || defined(V8_EXTERNAL_CODE_SPACE)
...
//
#if V8_HAS_PTHREAD_JIT_WRITE_PROTECT && !(defined(V8_COMPRESS_POINTERS) && !defined(V8_EXTERNAL_CODE_SPACE))
...
#if V8_HAS_BECORE_JIT_WRITE_PROTECT && !(defined(V8_COMPRESS_POINTERS) && !defined(V8_EXTERNAL_CODE_SPACE))
...
#if V8_HAS_PKU_JIT_WRITE_PROTECT && !(defined(V8_COMPRESS_POINTERS) && !defined(V8_EXTERNAL_CODE_SPACE))
...
// (using FP stack).
#if defined(V8_TARGET_ARCH_X64) || defined(V8_TARGET_ARCH_ARM64) || defined(V8_TARGET_ARCH_MIPS64) || defined(V8_TARGET_ARCH_LOONG64)                          \
...
constexpr size_t kMaximalCodeRangeSize = (COMPRESS_POINTERS_BOOL && !V8_EXTERNAL_CODE_SPACE_BOOL) ? 128 * MB : 256 * MB;
constexpr size_t kMinExpectedOSPageSize = 4 * KB; // OS page.
#elif V8_TARGET_ARCH_X64
constexpr size_t kMaximalCodeRangeSize = (COMPRESS_POINTERS_BOOL && !V8_EXTERNAL_CODE_SPACE_BOOL) ? 128 * MB : 512 * MB;
constexpr size_t kMinExpectedOSPageSize = 4 * KB; // OS page.
#else
...
#ifdef V8_COMPRESS_POINTERS
static_assert(kSystemPointerSize == kInt64Size, "Pointer compression can be enabled only for 64-bit architectures");
...
constexpr JSDispatchHandle kInvalidDispatchHandle = static_cast<JSDispatchHandle>(0xffffffff << kJSDispatchHandleShift);
...
constexpr size_t kExternalAllocationSoftLimit = internal::Internals::kExternalAllocationSoftLimit;
...
constexpr int kBitsPerSystemPointerLog2 = kSystemPointerSizeLog2 + kBitsPerByteLog2;
...
template <typename F> F FUNCTION_CAST(uint8_t* addr)
{
    return reinterpret_cast<F>(reinterpret_cast<Address>(addr));
}

template <typename F> F FUNCTION_CAST(Address addr)
{
...
#if V8_HOST_ARCH_PPC64 && (V8_OS_AIX || (V8_TARGET_ARCH_PPC64 && V8_TARGET_BIG_ENDIAN && (!defined(_CALL_ELF) || _CALL_ELF == 1)))
#define USES_FUNCTION_DESCRIPTORS 1
#define FUNCTION_ENTRYPOINT_ADDRESS(f) (reinterpret_cast<v8::internal::Address*>(&(reinterpret_cast<intptr_t*>(f)[0])))
#else
...
// otherwise.
inline LanguageMode stricter_language_mode(LanguageMode mode1, LanguageMode mode2)
{
...
inline std::ostream& operator<<(std::ostream& os, const LookupHoistingMode& mode)
{
    switch (mode) {
...
static_assert((kSmiValueSize + kSmiShiftSize + kSmiTagSize) % 32 == 0, "Unsupported Smi tagging scheme");
...
static_assert(!SmiValuesAre32Bits() == SmiValuesAre31Bits(), "Unsupported Smi tagging scheme");
static_assert(SmiValuesAre32Bits() == kIsSmiValueInUpper32Bits, "Unsupported Smi tagging scheme");
static_assert(SmiValuesAre31Bits() == kIsSmiValueInLower32Bits, "Unsupported Smi tagging scheme");

// Mask for the sign bit in a smi.
constexpr intptr_t kSmiSignMask = static_cast<intptr_t>(uintptr_t { 1 } << (kSmiValueSize + kSmiShiftSize + kSmiTagSize - 1));
...
#ifdef V8_COMPRESS_POINTERS_8GB
static_assert(kObjectAlignment8GbHeap == 2 * kTaggedSize,
...
constexpr uint64_t kTracedHandleEagerResetZapValue = uint64_t { 0x1beffedaabaffedf };
constexpr uint64_t kTracedHandleMinorGCResetZapValue = uint64_t { 0x1beffedeebaffedf };
constexpr uint64_t kTracedHandleMinorGCWeakResetZapValue = uint64_t { 0x1beffed11baffedf };
constexpr uint64_t kTracedHandleFullGCResetZapValue = uint64_t { 0x1beffed77baffedf };
...
class AccessorInfo;
template <ArgumentsType> class Arguments;
...
template <typename T> class DirectHandleVector;
#endif
...
class HeapObject;
class IC;
template <typename T> using IndirectHandle = Handle<T>;
...
template <typename T> class MaybeDirectHandle;
template <typename T> using MaybeIndirectHandle = MaybeHandle<T>;
...
template <HeapObjectReferenceType kRefType, typename StorageType> class TaggedImpl;
...
template <typename Cage> class V8HeapCompressionSchemeImpl;
class MainCage;
...
template <typename CompressionScheme> class OffHeapCompressedObjectSlot;
...
class Smi;
template <typename Config, class Allocator = FreeStoreAllocationPolicy> class SplayTree;
class String;
class StringStream;
class Struct;
class Symbol;
...
using JSPrimitive = Union<Smi, HeapNumber, BigInt, String, Symbol, Boolean, Null, Undefined>;
// A user-exposed JavaScript value, as opposed to V8-internal values like Holes
// or a FixedArray.
using JSAny = Union<Smi, HeapNumber, BigInt, String, Symbol, Boolean, Null, Undefined, JSReceiver>;
using JSAnyNotNumeric = Union<String, Symbol, Boolean, Null, Undefined, JSReceiver>;
using JSAnyNotNumber = Union<BigInt, String, Symbol, Boolean, Null, Undefined, JSReceiver>;
using JSCallable = Union<JSBoundFunction, JSFunction, JSObject, JSProxy, JSWrappedFunction>;
...
struct SlotTraits {
    ...
    using TOffHeapObjectSlot = OffHeapCompressedObjectSlot<V8HeapCompressionScheme>;
#ifdef V8_EXTERNAL_CODE_SPACE
    using TInstructionStreamSlot = OffHeapCompressedObjectSlot<ExternalCodeCompressionScheme>;
#else
...
#endif // V8_COMPRESS_POINTERS
#ifdef V8_ENABLE_SANDBOX
    using TProtectedPointerSlot = OffHeapCompressedObjectSlot<TrustedSpaceCompressionScheme>;
#else
...
{
    return space == TRUSTED_SPACE || space == TRUSTED_LO_SPACE || space == SHARED_TRUSTED_SPACE || space == SHARED_TRUSTED_LO_SPACE;
}
constexpr bool IsAnySharedSpace(AllocationSpace space)
{
    return space == SHARED_SPACE || space == SHARED_LO_SPACE || space == SHARED_TRUSTED_SPACE || space == SHARED_TRUSTED_LO_SPACE;
}
...
static_assert(kGarbageCollectionReasonMaxValue == static_cast<int>(GarbageCollectionReason::NUM_REASONS) - 1,
...
inline std::ostream& operator<<(std::ostream& os, GarbageCollectionReason reason)
{
...
{
    return kind == AllocationType::kSharedOld || kind == AllocationType::kSharedMap;
}
...
enum class ScriptEventType { kReserveId, kCreate, kDeserialize, kBackgroundCompile, kStreamingCompileBackground, kStreamingCompileForeground };
...
#define HAS_SMI_TAG(value) ((static_cast<i::Tagged_t>(value) & ::i::kSmiTagMask) == ::i::kSmiTag)

#define HAS_STRONG_HEAP_OBJECT_TAG(value) (((static_cast<i::Tagged_t>(value) & ::i::kHeapObjectTagMask) == ::i::kHeapObjectTag))

#define HAS_WEAK_HEAP_OBJECT_TAG(value) (((static_cast<i::Tagged_t>(value) & ::i::kHeapObjectTagMask) == ::i::kWeakHeapObjectTag))

// OBJECT_POINTER_ALIGN returns the value aligned as a HeapObject pointer
#define OBJECT_POINTER_ALIGN(value) (((value) + ::i::kObjectAlignmentMask) & ~::i::kObjectAlignmentMask)
...
#ifdef V8_COMPRESS_POINTERS_8GB
#define ALIGN_TO_ALLOCATION_ALIGNMENT(value) (((value) + ::i::kObjectAlignment8GbHeapMask) & ~::i::kObjectAlignment8GbHeapMask)
#else
...
#define POINTER_SIZE_ALIGN(value) (((value) + ::i::kPointerAlignmentMask) & ~::i::kPointerAlignmentMask)
...
#define CODE_POINTER_ALIGN(value) (((value) + ::i::kCodeAlignmentMask) & ~::i::kCodeAlignmentMask)
...
#define DOUBLE_POINTER_ALIGN(value) (((value) + ::i::kDoubleAlignmentMask) & ~::i::kDoubleAlignmentMask)
...
#if (V8_TARGET_ARCH_MIPS64 && !defined(_MIPS_ARCH_MIPS64R6) && (!defined(USE_SIMULATOR) || !defined(_MIPS_TARGET_SIMULATOR)))
...
enum VariableKind : uint8_t { NORMAL_VARIABLE, PARAMETER_VARIABLE, THIS_VARIABLE, SLOPPY_BLOCK_FUNCTION_VARIABLE, SLOPPY_FUNCTION_NAME_VARIABLE };
...
{
    return mode >= VariableMode::kPrivateSetterOnly && mode <= VariableMode::kPrivateGetterAndSetter;
}
...
{
    return IsPrivateMethodVariableMode(mode) || IsPrivateAccessorVariableMode(mode);
}

inline bool IsSerializableVariableMode(VariableMode mode)
{
    return IsDeclaredVariableMode(mode) || IsPrivateMethodOrAccessorVariableMode(mode);
}

inline bool IsImmutableLexicalVariableMode(VariableMode mode)
{
    return mode == VariableMode::kConst || mode == VariableMode::kUsing || mode == VariableMode::kAwaitUsing;
}

inline bool IsImmutableLexicalOrPrivateVariableMode(VariableMode mode)
{
    return IsImmutableLexicalVariableMode(mode) || IsPrivateMethodOrAccessorVariableMode(mode);
}
...
inline std::ostream& operator<<(std::ostream& os, InterpreterPushArgsMode mode)
{
    switch (mode) {
...
static_assert((static_cast<int>(ForInFeedback::kNone) | static_cast<int>(ForInFeedback::kEnumCacheKeysAndIndices))
    == static_cast<int>(ForInFeedback::kEnumCacheKeysAndIndices));
static_assert((static_cast<int>(ForInFeedback::kEnumCacheKeysAndIndices) | static_cast<int>(ForInFeedback::kEnumCacheKeys))
    == static_cast<int>(ForInFeedback::kEnumCacheKeys));
static_assert((static_cast<int>(ForInFeedback::kEnumCacheKeys) | static_cast<int>(ForInFeedback::kAny)) == static_cast<int>(ForInFeedback::kAny));
...
using DefineKeyedOwnPropertyInLiteralFlags = base::Flags<DefineKeyedOwnPropertyInLiteralFlag>;
...
inline std::ostream& operator<<(std::ostream& os, const AssemblerDebugInfo& info)
{
...
static_assert(static_cast<int>(TieringState::kNone) == 0b00 && static_cast<int>(TieringState::kInProgress) == 0b01);
...
{
    return IsRequestMaglev_Concurrent(state) || IsRequestMaglev_Synchronous(state);
}
constexpr bool IsRequestTurbofan(TieringState state)
{
    return IsRequestTurbofan_Concurrent(state) || IsRequestTurbofan_Synchronous(state);
}
...
inline std::ostream& operator<<(std::ostream& os, SpeculationMode speculation_mode)
{
...
inline KeyedAccessLoadMode CreateKeyedAccessLoadMode(bool handle_oob, bool handle_holes)
{
    return static_cast<KeyedAccessLoadMode>(static_cast<int>(handle_oob) | (static_cast<int>(handle_holes) << 1));
}

inline KeyedAccessLoadMode GeneralizeKeyedAccessLoadMode(KeyedAccessLoadMode mode1, KeyedAccessLoadMode mode2)
{
    using T = std::underlying_type<KeyedAccessLoadMode>::type;
    return static_cast<KeyedAccessLoadMode>(static_cast<T>(mode1) | static_cast<T>(mode2));
}

inline bool LoadModeHandlesOOB(KeyedAccessLoadMode load_mode)
{
    using T = std::underlying_type<KeyedAccessLoadMode>::type;
    return (static_cast<T>(load_mode) & static_cast<T>(KeyedAccessLoadMode::kHandleOOB)) != 0;
}

inline bool LoadModeHandlesHoles(KeyedAccessLoadMode load_mode)
{
    using T = std::underlying_type<KeyedAccessLoadMode>::type;
    return (static_cast<T>(load_mode) & static_cast<T>(KeyedAccessLoadMode::kHandleHoles)) != 0;
}
...
{
    return store_mode == KeyedAccessStoreMode::kHandleCOW || store_mode == KeyedAccessStoreMode::kGrowAndHandleCOW;
}

inline bool StoreModeSupportsTypeArray(KeyedAccessStoreMode store_mode)
{
    return store_mode == KeyedAccessStoreMode::kInBounds || store_mode == KeyedAccessStoreMode::kIgnoreTypedArrayOOB;
}
