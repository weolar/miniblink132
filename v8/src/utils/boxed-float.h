...
public:
    ...
    explicit Float64(base::Double value)
        : bit_pattern_(value.AsUint64())
    {
    }
    ...
    {
        return base::bit_cast<double>(bit_pattern_);
    }
...
{
    return f64.is_nan() ? hash_value(f64.is_hole_nan()) : hash_value(f64.get_bits());
}
