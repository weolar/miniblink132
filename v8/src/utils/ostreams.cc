...
int DbgStreamBuf::sync()
{
    ...
    if (pbase() != pptr()) {
        OutputDebugStringA(std::string(pbase(), static_cast<std::string::size_type>(pptr() - pbase())).c_str());
        setp(pbase(), epptr());
    }
#endif
    return 0;
...
{
    return static_cast<std::streamsize>(std::fwrite(s, 1, static_cast<size_t>(n), f_));
}
...
DEFINE_LAZY_LEAKY_OBJECT_GETTER(base::RecursiveMutex, StdoutStream::GetStdoutMutex)
...
std::ostream& PrintUC16ForJSON(std::ostream& os, uint16_t c, bool (*pred)(uint16_t))
{
...
std::ostream& operator<<(std::ostream& os, const AsHex& hex)
{
    ...
    snprintf(buf, kMaxHexLength, "%s%.*" PRIx64, hex.with_prefix ? "0x" : "", hex.min_width, hex.value);
    return os << buf;
}
