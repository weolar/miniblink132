...
class SparseBitVector : public ZoneObject {
    ...
    struct Segment {
        ...
        bool empty() const
        {
            return std::all_of(std::begin(words), std::end(words), [](auto segment) { return segment == 0; });
        }
    };
...
public:
    ...
    public:
        ...
        explicit Iterator(const Segment* segment)
        {
            ...
            do {
                for (int word = 0; word < kNumWordsPerSegment; ++word) {
                    if (segment->words[word] == 0)
                        continue;
                    int bit_in_word = base::bits::CountTrailingZeros(segment->words[word]);
                    segment_ = segment;
            ...
            DCHECK_IMPLIES(segment_, contains(segment_, segment_->offset + bit_in_segment_));
        }
        ...
        {
            return segment_ == other.segment_ && bit_in_segment_ == other.bit_in_segment_;
        }
        ...
        void operator++()
        {
            ...
            if (bit_in_word < kBitsPerWord - 1) {
                uintptr_t remaining_bits = segment_->words[word] & (std::numeric_limits<uintptr_t>::max() << (1 + bit_in_word));
                if (remaining_bits) {
    ...
    explicit SparseBitVector(Zone* zone)
        : zone_(zone)
    {
    }
    ...
    V8_INLINE void Add(int i)
    {
        ...
        if (V8_UNLIKELY(i >= kNumBitsPerSegment)) {
            do {
                last = segment;
                segment = segment->next;
                if (V8_UNLIKELY(!segment))
                    return InsertBitAfter(last, i);
            } while (segment->offset <= i - kNumBitsPerSegment);
            if (V8_UNLIKELY(segment->offset > i))
                return InsertBitAfter(last, i);
        }
        set(segment, i);
    }
    ...
    void Union(const SparseBitVector& other)
    {
        ...
        for (const Segment* other_segment = &other.first_segment_; other_segment; other_segment = other_segment->next) {
            ...
            if (segment && segment->offset == other_segment->offset) {
                std::transform(std::begin(segment->words), std::end(segment->words), std::begin(other_segment->words), std::begin(segment->words),
                    std::bit_or<uintptr_t> {});
                ...
                std::copy(std::begin(other_segment->words), std::end(other_segment->words), std::begin(new_segment->words));
    ...
    {
        return Iterator { &first_segment_ };
    }
    Iterator end() const
    {
        return Iterator { Iterator::EndTag {} };
    }
...
private:
    ...
    {
        if ((segment->offset % kNumBitsPerSegment) != 0)
            return false;
        if (segment->next && segment->next->offset <= segment->offset)
            return false;
        return true;
    }
