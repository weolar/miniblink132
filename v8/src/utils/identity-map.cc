...
std::pair<int, bool> IdentityMapBase::ScanKeysFor(Address address, uint32_t hash) const
{
...
std::pair<int, bool> IdentityMapBase::InsertKey(Address address, uint32_t hash)
{
...
IdentityMapFindResult<uintptr_t> IdentityMapBase::FindOrInsertEntry(Address key)
{
    DCHECK_NE(heap_->gc_state(), Heap::MARK_COMPACT);
...
IdentityMapBase::RawEntry IdentityMapBase::InsertEntry(Address key)
{
    ...
    if (capacity_ == 0) {
        ...
        strong_roots_entry_ = heap_->RegisterStrongRoots("IdentityMapBase", FullObjectSlot(keys_), FullObjectSlot(keys_ + capacity_));
    } else {
...
void IdentityMapBase::Resize(int new_capacity)
{
    ...
    DCHECK_NOT_NULL(strong_roots_entry_);
    heap_->UpdateStrongRoots(strong_roots_entry_, FullObjectSlot(keys_), FullObjectSlot(keys_ + capacity_));
