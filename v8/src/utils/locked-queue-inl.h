...
template <typename Record> struct LockedQueue<Record>::Node : Malloced {
    Node()
        : next(nullptr)
    {
    }
...
template <typename Record> inline LockedQueue<Record>::LockedQueue()
{
    head_ = new Node();
...
template <typename Record> inline void LockedQueue<Record>::Enqueue(Record record)
{
    Node* n = new Node();
    CHECK_NOT_NULL(n);
...
template <typename Record> inline bool LockedQueue<Record>::Dequeue(Record* record)
{
...
template <typename Record> inline bool LockedQueue<Record>::IsEmpty() const
{
    base::MutexGuard guard(&head_mutex_);
    return head_->next.Value() == nullptr;
}

template <typename Record> inline bool LockedQueue<Record>::Peek(Record* record) const
{
    base::MutexGuard guard(&head_mutex_);
    Node* const next_node = head_->next.Value();
    if (next_node == nullptr)
...
template <typename Record> inline size_t LockedQueue<Record>::size() const
{
    return size_;
}
