...
struct AsHexBytes {
    ...
    explicit AsHexBytes(uint64_t v, uint8_t min_bytes = 1, ByteOrder byte_order = kLittleEndian)
        : value(v)
        , min_bytes(min_bytes)
...
template <typename T> struct PrintIteratorRange {
    T start;
    T end;
...
auto PrintCollection(const T& collection)
    -> PrintIteratorRange<typename std::common_type<decltype(std::begin(collection)), decltype(std::end(collection))>::type>
{
...
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, const AsEscapedUC16ForJSON& c);
...
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, const AsHexBytes& v);

template <typename T> std::ostream& operator<<(std::ostream& os, const PrintIteratorRange<T>& range)
{
    const char* separator = "";
