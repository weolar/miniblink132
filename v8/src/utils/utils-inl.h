...
template <typename Char> bool TryAddArrayIndexChar(uint32_t* index, Char c)
{
    if (!IsDecimalDigit(c))
        return false;
    int d = c - '0';
...
template <typename Char> bool TryAddIntegerIndexChar(uint64_t* index, Char c)
{
    if (!IsDecimalDigit(c))
        return false;
    int d = c - '0';
...
template <typename Stream, typename index_t, enum ToIndexMode mode> bool StringToIndex(Stream* stream, index_t* index)
{
    ...
    while (stream->HasMore()) {
        ...
        if (sizeof(result) == 8) {
            ...
            if (!TryAddIntegerIndexChar(reinterpret_cast<uint64_t*>(&result), stream->GetNext())) {
                return false;
            }
        } else {
            // Either mode is fine here.
            if (!TryAddArrayIndexChar(reinterpret_cast<uint32_t*>(&result), stream->GetNext()))
                return false;
        }
    }
