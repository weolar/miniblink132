...
template <typename T> T* NewArray(size_t size)
{
    ...
    if (V8_UNLIKELY(result == nullptr)) {
        ...
        if (result == nullptr)
            V8::FatalProcessOutOfMemory(nullptr, "NewArray");
    }
    return result;
}

template <typename T, typename = typename std::enable_if<base::is_trivially_copyable<T>::value>::type> T* NewArray(size_t size, T default_val)
{
...
template <typename T> using ArrayUniquePtr = std::unique_ptr<T, ArrayDeleter<T>>;
...
V8_EXPORT_PRIVATE v8::PageAllocator* SetPlatformPageAllocatorForTesting(v8::PageAllocator* page_allocator);
...
V8_EXPORT_PRIVATE
V8_WARN_UNUSED_RESULT void* AllocatePages(v8::PageAllocator* page_allocator, void* address, size_t size, size_t alignment, PageAllocator::Permission access);
...
// false.
V8_EXPORT_PRIVATE
V8_WARN_UNUSED_RESULT bool SetPermissions(v8::PageAllocator* page_allocator, void* address, size_t size, PageAllocator::Permission access);
inline bool SetPermissions(v8::PageAllocator* page_allocator, Address address, size_t size, PageAllocator::Permission access)
{
    return SetPermissions(page_allocator, reinterpret_cast<void*>(address), size, access);
}
...
public:
    ...
    V8_EXPORT_PRIVATE VirtualMemory(
        v8::PageAllocator* page_allocator, size_t size, void* hint, size_t alignment = 1, PageAllocator::Permission permissions = PageAllocator::kNoAccess);
