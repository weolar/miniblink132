...
#if defined(V8_TARGET_ARCH_ARM64) && (defined(__ARM_NEON) || defined(__ARM_NEON__))
...
template <typename T> static T ArithmeticShiftRight(T x, int shift)
{
    DCHECK_LE(0, shift);
    if (x < 0) {
...
template <typename T> T JSMax(T x, T y)
{
    if (std::isnan(x))
        return x;
    if (std::isnan(y))
...
// Returns the maximum of the two parameters according to JavaScript semantics.
template <typename T> T JSMin(T x, T y)
{
    if (std::isnan(x))
        return x;
    if (std::isnan(y))
...
template <typename T, typename = typename std::enable_if<std::is_signed<T>::value>::type> typename std::make_unsigned<T>::type Abs(T a)
{
...
template <typename T> T SaturateAdd(T a, T b)
{
...
template <typename T> T SaturateSub(T a, T b)
{
    if (std::is_signed<T>::value) {
        if (a >= 0 && b < 0) {
            if (a > std::numeric_limits<T>::max() + b) {
...
template <typename T> T SaturateRoundingQMul(T a, T b)
{
...
template <typename Wide, typename Narrow> Wide MultiplyLong(Narrow a, Narrow b)
{
    static_assert(std::is_integral<Narrow>::value && std::is_integral<Wide>::value, "only integral types");
    static_assert(std::is_signed<Narrow>::value == std::is_signed<Wide>::value, "both must have same signedness");
...
// Put Wide first so we can use function template argument deduction for Narrow,
// and callers can provide only Wide.
template <typename Wide, typename Narrow> Wide AddLong(Narrow a, Narrow b)
{
    static_assert(std::is_integral<Narrow>::value && std::is_integral<Wide>::value, "only integral types");
    static_assert(std::is_signed<Narrow>::value == std::is_signed<Wide>::value, "both must have same signedness");
    static_assert(sizeof(Narrow) * 2 == sizeof(Wide), "only twice as long");
...
template <typename T> inline T RoundingAverageUnsigned(T a, T b)
{
...
#define DEFINE_FIELD_OFFSET_CONSTANTS(StartOffset, LIST_MACRO) enum { LIST_MACRO##_StartOffset = StartOffset - 1, LIST_MACRO(DEFINE_ONE_FIELD_OFFSET) };

#define DEFINE_ONE_FIELD_OFFSET_PURE_NAME(CamelName, Size, ...) k##CamelName##Offset, k##CamelName##OffsetEnd = k##CamelName##Offset + (Size)-1,

#define DEFINE_FIELD_OFFSET_CONSTANTS_WITH_PURE_NAME(StartOffset, LIST_MACRO)                                                                                  \
    enum { LIST_MACRO##_StartOffset = StartOffset - 1, LIST_MACRO(DEFINE_ONE_FIELD_OFFSET_PURE_NAME) };
...
#define STATIC_ASSERT_FIELD_OFFSETS_EQUAL(Offset1, Offset2) static_assert(static_cast<int>(Offset1) == Offset2)
// ----------------------------------------------------------------------------
// Hash function.
...
template <typename T> class SetOncePointer {
public:
...
template <typename Char> V8_INLINE bool SimdMemEqual(const Char* lhs, const Char* rhs, size_t count, size_t order)
{
    ...
    if (order == 5) { // count: [17, 32]
        ...
        const __m128i lhs128_start = _mm_lddqu_si128(reinterpret_cast<const __m128i*>(lhs));
        const __m128i lhs128_end = _mm_lddqu_si128(reinterpret_cast<const __m128i*>(lhs + count - sizeof(__m128i)));
        const __m128i rhs128_start = _mm_lddqu_si128(reinterpret_cast<const __m128i*>(rhs));
        const __m128i rhs128_end = _mm_lddqu_si128(reinterpret_cast<const __m128i*>(rhs + count - sizeof(__m128i)));
        ...
        const uint32_t res = _mm_movemask_epi8(res_start) << 16 | _mm_movemask_epi8(res_end);
        return res == kSIMDMatched32Mask;
    }

    // count: [33, ...]
    const __m128i lhs128_unrolled = _mm_lddqu_si128(reinterpret_cast<const __m128i*>(lhs));
    const __m128i rhs128_unrolled = _mm_lddqu_si128(reinterpret_cast<const __m128i*>(rhs));
    ...
    if (res_unrolled_mask != kSIMDMatched16Mask)
        return false;

    for (size_t i = count % sizeof(__m128i); i < count; i += sizeof(__m128i)) {
        const __m128i lhs128 = _mm_lddqu_si128(reinterpret_cast<const __m128i*>(lhs + i));
        const __m128i rhs128 = _mm_lddqu_si128(reinterpret_cast<const __m128i*>(rhs + i));
        const __m128i res = _mm_cmpeq_epi8(lhs128, rhs128);
        const uint16_t res_mask = _mm_movemask_epi8(res);
        if (res_mask != kSIMDMatched16Mask)
            return false;
    }
    return true;
}
...
template <typename Char> V8_INLINE V8_CLANG_NO_SANITIZE("alignment") bool SimdMemEqual(const Char* lhs, const Char* rhs, size_t count, size_t order)
{
    static_assert(sizeof(Char) == 1);
    DCHECK_GE(order, 5);
    ...
    if (static_cast<bool>(vgetq_lane_u64(vreinterpretq_u64_u8(vpmaxq_u8(xored, xored)), 0)))
        return false;
    for (size_t i = count % sizeof(uint8x16_t); i < count; i += sizeof(uint8x16_t)) {
...
template <typename IntType, typename Char> V8_CLANG_NO_SANITIZE("alignment") V8_INLINE bool OverlappingCompare(const Char* lhs, const Char* rhs, size_t count)
{
    static_assert(sizeof(Char) == 1);
    return *reinterpret_cast<const IntType*>(lhs) == *reinterpret_cast<const IntType*>(rhs)
        && *reinterpret_cast<const IntType*>(lhs + count - sizeof(IntType)) == *reinterpret_cast<const IntType*>(rhs + count - sizeof(IntType));
}

template <typename Char> V8_CLANG_NO_SANITIZE("alignment") V8_INLINE bool SimdMemEqual(const Char* lhs, const Char* rhs, size_t count)
{
    static_assert(sizeof(Char) == 1);
    if (count == 0) {
        return true;
    ...
    const size_t order = sizeof(count) * CHAR_BIT - base::bits::CountLeadingZeros(count - 1);
    switch (order) {
    case 1: // count: [2, 2]
        return *reinterpret_cast<const uint16_t*>(lhs) == *reinterpret_cast<const uint16_t*>(rhs);
    case 2: // count: [3, 4]
...
template <typename lchar, typename rchar> inline bool CompareCharsEqualUnsigned(const lchar* lhs, const rchar* rhs, size_t chars)
{
...
template <typename lchar, typename rchar> inline bool CompareCharsEqual(const lchar* lhs, const rchar* rhs, size_t chars)
{
    using ulchar = typename std::make_unsigned<lchar>::type;
    using urchar = typename std::make_unsigned<rchar>::type;
    return CompareCharsEqualUnsigned(reinterpret_cast<const ulchar*>(lhs), reinterpret_cast<const urchar*>(rhs), chars);
}

// Compare 8bit/16bit chars to 8bit/16bit chars.
template <typename lchar, typename rchar> inline int CompareCharsUnsigned(const lchar* lhs, const rchar* rhs, size_t chars)
{
    static_assert(std::is_unsigned<lchar>::value);
    static_assert(std::is_unsigned<rchar>::value);
...
template <typename lchar, typename rchar> inline int CompareChars(const lchar* lhs, const rchar* rhs, size_t chars)
{
    using ulchar = typename std::make_unsigned<lchar>::type;
    using urchar = typename std::make_unsigned<rchar>::type;
    return CompareCharsUnsigned(reinterpret_cast<const ulchar*>(lhs), reinterpret_cast<const urchar*>(rhs), chars);
}
...
template <class T> inline constexpr T truncate_to_intn(T x, unsigned n)
{
    DCHECK((0 < n) && (n < (sizeof(x) * kBitsPerByte)));
...
#define INT_1_TO_63_LIST(V)                                                                                                                                    \
    V(1)                                                                                                                                                       \
    V(2) V(3) V(4) V(5) V(6) V(7) V(8) V(9) V(10) V(11) V(12) V(13) V(14) V(15) V(16) V(17) V(18) V(19) V(20) V(21) V(22) V(23) V(24) V(25) V(26) V(27) V(28)  \
        V(29) V(30) V(31) V(32) V(33) V(34) V(35) V(36) V(37) V(38) V(39) V(40) V(41) V(42) V(43) V(44) V(45) V(46) V(47) V(48) V(49) V(50) V(51) V(52) V(53)  \
            V(54) V(55) V(56) V(57) V(58) V(59) V(60) V(61) V(62) V(63)
// clang-format on

#define DECLARE_IS_INT_N(N)                                                                                                                                    \
    inline constexpr bool is_int##N(int64_t x)                                                                                                                 \
    {                                                                                                                                                          \
        return is_intn(x, N);                                                                                                                                  \
    }
#define DECLARE_IS_UINT_N(N)                                                                                                                                   \
    template <class T> inline constexpr bool is_uint##N(T x)                                                                                                   \
    {                                                                                                                                                          \
        return is_uintn(x, N);                                                                                                                                 \
    }
#define DECLARE_TRUNCATE_TO_INT_N(N)                                                                                                                           \
    template <class T> inline constexpr T truncate_to_int##N(T x)                                                                                              \
    {                                                                                                                                                          \
        return truncate_to_intn(x, N);                                                                                                                         \
    }

#define DECLARE_CHECKED_TRUNCATE_TO_INT_N(N)                                                                                                                   \
    template <class T> inline constexpr T checked_truncate_to_int##N(T x)                                                                                      \
    {                                                                                                                                                          \
        CHECK(is_int##N(x));                                                                                                                                   \
        return truncate_to_intn(x, N);                                                                                                                         \
    }
...
public:
    explicit constexpr BytecodeOffset(int id)
        : id_(id)
    {
    }
    ...
    {
        return BytecodeOffset(1);
    }
    static BytecodeOffset ConstructStubInvoke()
    {
        return BytecodeOffset(2);
    }
...
V8_EXPORT_PRIVATE void PRINTF_FORMAT(2, 3) PrintF(FILE* out, const char* format, ...);
...
int WriteChars(const char* filename, const char* str, int size, bool verbose = true);

// Write size bytes to the file given by filename.
// The file is overwritten. Returns the number of bytes written.
int WriteBytes(const char* filename, const uint8_t* bytes, int size, bool verbose = true);

// Simple support to read a file into std::string.
// On return, *exits tells whether the file existed.
V8_EXPORT_PRIVATE std::string ReadFile(const char* filename, bool* exists, bool verbose = true);
V8_EXPORT_PRIVATE std::string ReadFile(FILE* file, bool* exists, bool verbose = true);

bool DoubleToBoolean(double d);

template <typename Char> bool TryAddIndexChar(uint32_t* index, Char c);

enum ToIndexMode { kToArrayIndex, kToIntegerIndex };

// {index_t} is meant to be {uint32_t} or {size_t}.
template <typename Stream, typename index_t, enum ToIndexMode mode = kToArrayIndex> bool StringToIndex(Stream* stream, index_t* index);
...
static inline uint64_t ByteReverse64(uint64_t value)
{
    ...
    return value << (bits_of_v - 8) | ((value << (bits_of_v - 24)) & 0x00FF000000000000) | ((value << (bits_of_v - 40)) & 0x0000FF0000000000)
        | ((value << (bits_of_v - 56)) & 0x000000FF00000000) | ((value >> (bits_of_v - 56)) & 0x00000000FF000000)
        | ((value >> (bits_of_v - 40)) & 0x0000000000FF0000) | ((value >> (bits_of_v - 24)) & 0x000000000000FF00)
        | ((value >> (bits_of_v - 8)) & 0x00000000000000FF);
#endif
}

template <typename V> static inline V ByteReverse(V value)
{
...
template <typename T> T FpOpWorkaround(T input, T value)
{
...
template <> inline Float16 FpOpWorkaround(Float16 input, Float16 value)
{
...
V8_EXPORT_PRIVATE bool PassesFilter(base::Vector<const char> name, base::Vector<const char> filter);
...
inline bool RoundUpToPageSize(size_t byte_length, size_t page_size, size_t max_allowed_byte_length, size_t* pages)
{
