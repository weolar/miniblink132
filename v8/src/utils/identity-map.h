...
template <typename T> struct IdentityMapFindResult {
    T* entry;
    bool already_exists;
};
...
template <typename V, class AllocationPolicy> class IdentityMap : public IdentityMapBase {
public:
    static_assert(sizeof(V) <= sizeof(uintptr_t));
    static_assert(std::is_trivially_copyable<V>::value);
    static_assert(std::is_trivially_destructible<V>::value);

    explicit IdentityMap(Heap* heap, AllocationPolicy allocator = AllocationPolicy())
    ...
    public:
        ...
        Iterator begin()
        {
            return Iterator(map_, map_->NextIndex(-1));
        }
        Iterator end()
        {
            return Iterator(map_, map_->capacity());
        }
...
protected:
    ...
    {
        uintptr_t* result = allocator_.template AllocateArray<uintptr_t, Buffer>(length);
