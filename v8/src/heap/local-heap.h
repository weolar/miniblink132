...
__attribute__((tls_model(V8_TLS_MODEL))) extern thread_local LocalHeap* g_current_local_heap_ V8_CONSTINIT;
...
public:
    ...
    explicit LocalHeap(Heap* heap, ThreadKind kind, std::unique_ptr<PersistentHandles> persistent_handles = nullptr);
    ~LocalHeap();
    ...
    template <typename T> IndirectHandle<T> NewPersistentHandle(Tagged<T> object)
    {
    ...
    template <typename T> IndirectHandle<T> NewPersistentHandle(T object)
    {
    ...
    template <typename T, template <typename> typename MaybeHandleType,
        typename = std::enable_if_t<std::is_convertible_v<MaybeHandleType<T>, MaybeDirectHandle<T>>>>
...
#undef ROOT_ACCESSOR
    ...
    template <typename Callback> V8_INLINE void ExecuteWhileParked(Callback callback);
    template <typename Callback> V8_INLINE void ExecuteMainThreadWhileParked(Callback callback);
    template <typename Callback> V8_INLINE void ExecuteBackgroundThreadWhileParked(Callback callback);
...
private:
    ...
    public:
        ...
        {
            return IsRunning() && (raw_state_ & (SafepointRequestedBit::kMask | CollectionRequestedBit::kMask));
        }

    private:
        constexpr explicit ThreadState(uint8_t value)
            : raw_state_(value)
        {
        }
    ...
    public:
        ...
        {
            return raw_state_.compare_exchange_strong(expected.raw_state_, updated.raw());
        }

        bool CompareExchangeWeak(ThreadState& expected, ThreadState updated)
        {
            return raw_state_.compare_exchange_weak(expected.raw_state_, updated.raw());
        }
    ...
    template <typename Callback> V8_INLINE void ExecuteWithStackMarker(Callback callback);
    ...
    template <typename Callback> V8_INLINE void ParkAndExecuteCallback(Callback callback);
