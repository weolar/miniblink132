...
EvacuationAllocator::EvacuationAllocator(Heap* heap, CompactionSpaceKind compaction_space_kind)
    : heap_(heap)
    ...
    , compaction_spaces_(heap, compaction_space_kind)
{
    ...
    old_space_allocator_.emplace(heap, compaction_spaces_.Get(OLD_SPACE), MainAllocator::kInGC);
    code_space_allocator_.emplace(heap, compaction_spaces_.Get(CODE_SPACE), MainAllocator::kInGC);
    if (heap_->isolate()->has_shared_space()) {
        shared_space_allocator_.emplace(heap, compaction_spaces_.Get(SHARED_SPACE), MainAllocator::kInGC);
    }
    trusted_space_allocator_.emplace(heap, compaction_spaces_.Get(TRUSTED_SPACE), MainAllocator::kInGC);
}

void EvacuationAllocator::FreeLast(AllocationSpace space, Tagged<HeapObject> object, int object_size)
{
...
void EvacuationAllocator::FreeLastInMainAllocator(MainAllocator* allocator, Tagged<HeapObject> object, int object_size)
{
...
void EvacuationAllocator::Finalize()
{
    ...
    if (shared_space_allocator_) {
        ...
        heap_->shared_allocation_space()->MergeCompactionSpace(compaction_spaces_.Get(SHARED_SPACE));
    }

    trusted_space_allocator()->FreeLinearAllocationArea();
    heap_->trusted_space()->MergeCompactionSpace(compaction_spaces_.Get(TRUSTED_SPACE));
}
