...
public:
    ...
    {
        return ChunkAddress() + MemoryChunkLayout::ObjectStartOffsetInMemoryChunk(RO_SPACE);
    }
...
public:
    ...
    void set_accounting_stats(const AllocationStats& stats)
    {
        stats_ = stats;
    }
    ...
    void set_shared_read_only_space(std::unique_ptr<SharedReadOnlySpace> shared_space)
    {
...
public:
    ...
    V8_EXPORT_PRIVATE
    AllocationResult AllocateRaw(int size_in_bytes, AllocationAlignment alignment);
    ...
    {
        return pages_.front()->ChunkAddress();
    }
...
public:
    explicit SharedReadOnlySpace(Heap* heap)
        : ReadOnlySpace(heap)
    {
        is_marked_read_only_ = true;
    }

    SharedReadOnlySpace(Heap* heap, std::vector<ReadOnlyPageMetadata*>&& new_pages,
        std::vector<std::unique_ptr<::v8::PageAllocator::SharedMemoryMapping>>&& mappings, AllocationStats&& new_stats);
    ...
    // deallocated.
    std::vector<std::unique_ptr<v8::PageAllocator::SharedMemoryMapping>> shared_memory_mappings_;
};
...
template <> struct hash<i::ReadOnlyPageMetadata*> : hash<i::MemoryChunkMetadata*> { };
template <> struct hash<const i::ReadOnlyPageMetadata*> : hash<const i::MemoryChunkMetadata*> { };
} // namespace base
