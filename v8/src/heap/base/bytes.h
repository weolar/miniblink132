...
inline double AverageSpeed(const BytesAndDurationBuffer& buffer, const BytesAndDuration& initial, std::optional<v8::base::TimeDelta> selected_duration,
    size_t min_non_empty_speed = 0, size_t max_speed = std::numeric_limits<size_t>::max())
{
    const BytesAndDuration sum = buffer.Reduce(
        [selected_duration](const BytesAndDuration& a, const BytesAndDuration& b) {
            if (selected_duration.has_value() && a.duration >= selected_duration.value()) {
                return a;
            }
    ...
    return std::max(
        std::min(static_cast<double>(sum.bytes) / duration.InMillisecondsF(), static_cast<double>(max_speed)), static_cast<double>(min_non_empty_speed));
}
...
public:
    ...
    void Update(BytesAndDuration bytes_and_duration)
    {
        ...
        const double new_throughput = bytes_and_duration.bytes / bytes_and_duration.duration.InMillisecondsF();
        throughput_ = new_throughput + Decay(throughput_ - new_throughput, bytes_and_duration.duration);
    }
...
private:
    ...
    {
        return throughput * exp2(-delay.InMillisecondsF() / decay_.InMillisecondsF());
    }
