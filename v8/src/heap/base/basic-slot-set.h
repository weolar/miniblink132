...
template <size_t SlotGranularity> class BasicSlotSet {
...
public:
    ...
    static void Delete(BasicSlotSet* slot_set, size_t buckets)
    {
        ...
        v8::base::AlignedFree(reinterpret_cast<uint8_t*>(slot_set) - kInitialBucketsSize);
    }

    constexpr static size_t BucketsForSize(size_t size)
    {
        return (size + (SlotGranularity * kBitsPerBucket) - 1) / (SlotGranularity * kBitsPerBucket);
    }
    ...
    // or not.
    template <AccessMode access_mode> void Insert(size_t slot_offset)
    {
        size_t bucket_index;
    ...
    {
        return Iterate<access_mode>(chunk_start, start_bucket, end_bucket, callback, [this, mode](size_t bucket_index) {
    ...
    static constexpr int kBitsPerBucketLog2 = kCellsPerBucketLog2 + kBitsPerCellLog2;
    ...
    public:
        ...
        template <AccessMode access_mode = AccessMode::ATOMIC> uint32_t LoadCell(int cell_index)
        {
        ...
        template <AccessMode access_mode = AccessMode::ATOMIC> void SetCellBits(int cell_index, uint32_t mask)
        {
            if constexpr (access_mode == AccessMode::ATOMIC) {
        ...
        template <AccessMode access_mode = AccessMode::ATOMIC> void ClearCellBits(int cell_index, uint32_t mask)
        {
            if constexpr (access_mode == AccessMode::ATOMIC) {
...
protected:
    ...
    {
        size_t new_count = 0;
        for (size_t bucket_index = start_bucket; bucket_index < end_bucket; bucket_index++) {
            Bucket* bucket = LoadBucket<access_mode>(bucket_index);
            if (bucket != nullptr) {
    ...
    template <AccessMode access_mode = AccessMode::ATOMIC> void ReleaseBucket(size_t bucket_index)
    {
        Bucket* bucket = LoadBucket<access_mode>(bucket_index);
    ...
    template <AccessMode access_mode = AccessMode::ATOMIC> Bucket* LoadBucket(Bucket** bucket)
    {
    ...
    template <AccessMode access_mode = AccessMode::ATOMIC> Bucket* LoadBucket(size_t bucket_index)
    {
        return LoadBucket(bucket(bucket_index));
    }

    template <AccessMode access_mode = AccessMode::ATOMIC> void StoreBucket(Bucket** bucket, Bucket* value)
    {
        if (access_mode == AccessMode::ATOMIC) {
    ...
    template <AccessMode access_mode = AccessMode::ATOMIC> void StoreBucket(size_t bucket_index, Bucket* value)
    {
        StoreBucket(bucket(bucket_index), value);
    }

    template <AccessMode access_mode = AccessMode::ATOMIC> bool SwapInNewBucket(size_t bucket_index, Bucket* value)
    {
        Bucket** b = bucket(bucket_index);
        if (access_mode == AccessMode::ATOMIC) {
            return v8::base::AsAtomicPointer::Release_CompareAndSwap(b, nullptr, value) == nullptr;
        } else {
            DCHECK_NULL(*b);
            *b = value;
            return true;
    ...
    static void SlotToIndices(size_t slot_offset, size_t* bucket_index, int* cell_index, int* bit_index)
    {
        DCHECK(IsAligned(slot_offset, SlotGranularity));
    ...
    Bucket** buckets()
    {
        return reinterpret_cast<Bucket**>(this);
    }
