...
public:
    struct StepInfo final {
        ...
        {
            return scheduled_delta_bytes() < 0;
        }
    };
    ...
    // marking steps.
    static constexpr v8::base::TimeDelta kEstimatedMarkingTime = v8::base::TimeDelta::FromMilliseconds(500);

    // Minimum number of bytes that should be marked during an incremental
    // marking step.
    static constexpr size_t kDefaultMinimumMarkedBytesPerIncrementalStep = 64 * 1024;
    ...
    static std::unique_ptr<IncrementalMarkingSchedule> CreateWithDefaultMinimumMarkedBytesPerStep(bool predictable_schedule = false);
    static std::unique_ptr<IncrementalMarkingSchedule> CreateWithZeroMinimumMarkedBytesPerStep(bool predictable_schedule = false);
