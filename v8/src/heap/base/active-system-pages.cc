...
size_t ActiveSystemPages::Add(uintptr_t start, uintptr_t end, size_t page_size_bits)
{
    ...
    const uintptr_t start_page_bit = RoundDown(start, page_size) >> page_size_bits;
    ...
    const bitset_t mask = bits == kMaxPages ? int64_t { -1 } : ((uint64_t { 1 } << bits) - 1) << start_page_bit;
