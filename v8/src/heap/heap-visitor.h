...
public:
    explicit MaybeObjectSize(size_t size)
        : raw_size_(size)
    {
        DCHECK_GT(size, 0);
    }
...
//     ...
//   }
template <typename ConcreteVisitor> class HeapVisitor : public ObjectVisitorWithCageBases {
public:
    inline HeapVisitor(PtrComprCageBase cage_base, PtrComprCageBase code_cage_base);
...
protected:
    ...
    template <VisitorId visitor_id> V8_INLINE void VisitMapPointerIfNeeded(Tagged<HeapObject> host);

    // If this predicate returns true, the visitor will visit the full JSObject
    // (including slack).
    V8_INLINE static constexpr bool ShouldVisitFullJSObject()
    {
        return false;
    }
...
#define VISIT(TypeName) V8_INLINE size_t Visit##TypeName(Tagged<Map> map, Tagged<TypeName> object, MaybeObjectSize maybe_object_size);
...
#undef VISIT
    ...
    template <typename T> static V8_INLINE Tagged<T> Cast(Tagged<HeapObject> object);

    // Inspects the slot and filters some well-known RO objects and Smis in a fast
    // way. May still return Smis or RO objects.
    template <typename TSlot> std::optional<Tagged<Object>> GetObjectFilterReadOnlyAndSmiFast(TSlot slot) const;
};
...
template <typename ConcreteVisitor> class ConcurrentHeapVisitor : public HeapVisitor<ConcreteVisitor> {
public:
...
#define VISIT_AS_LOCKED_STRING(VisitorId, TypeName)                                                                                                            \
    V8_INLINE size_t Visit##TypeName(Tagged<Map> map, Tagged<TypeName> object, MaybeObjectSize maybe_object_size);
...
private:
    template <typename T> V8_INLINE size_t VisitStringLocked(Tagged<T> object);

    friend class HeapVisitor<ConcreteVisitor>;
};

template <typename ConcreteVisitor> class NewSpaceVisitor : public ConcurrentHeapVisitor<ConcreteVisitor> {
public:
...
template <class T> Tagged<Object> VisitWeakList(Heap* heap, Tagged<Object> list, WeakObjectRetainer* retainer);
} // namespace internal
} // namespace v8
