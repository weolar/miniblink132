...
{
    MutablePageMetadata::FromHeapObject(heap_object)->shared_mutex()->LockExclusive();
}

// static
void ExclusiveObjectLock::Unlock(Tagged<HeapObject> heap_object)
{
    MutablePageMetadata::FromHeapObject(heap_object)->shared_mutex()->UnlockExclusive();
}
...
{
    MutablePageMetadata::FromHeapObject(heap_object)->shared_mutex()->UnlockShared();
}
