...
#define TRACE_GC(tracer, scope_id)                                                                                                                             \
    ...
    GCTracer::Scope UNIQUE_IDENTIFIER(gc_tracer_scope)(tracer, GCTracer::Scope::ScopeId(scope_id), ThreadKind::kMain);                                         \
    TRACE_EVENT0(TRACE_GC_CATEGORIES, GCTracer::Scope::Name(GCTracer::Scope::ScopeId(scope_id)))

#define TRACE_GC_ARG1(tracer, scope_id, arg0_name, arg0_value)                                                                                                 \
    DCHECK_NE(GCTracer::Scope::MC_SWEEP, scope_id);                                                                                                            \
    DCHECK_NE(GCTracer::Scope::MC_BACKGROUND_SWEEPING, scope_id);                                                                                              \
    GCTracer::Scope UNIQUE_IDENTIFIER(gc_tracer_scope)(tracer, GCTracer::Scope::ScopeId(scope_id), ThreadKind::kMain);                                         \
    TRACE_EVENT1(TRACE_GC_CATEGORIES, GCTracer::Scope::Name(GCTracer::Scope::ScopeId(scope_id)), arg0_name, arg0_value)

#define TRACE_GC_WITH_FLOW(tracer, scope_id, bind_id, flow_flags)                                                                                              \
    DCHECK_NE(GCTracer::Scope::MC_SWEEP, scope_id);                                                                                                            \
    DCHECK_NE(GCTracer::Scope::MC_BACKGROUND_SWEEPING, scope_id);                                                                                              \
    GCTracer::Scope UNIQUE_IDENTIFIER(gc_tracer_scope)(tracer, GCTracer::Scope::ScopeId(scope_id), ThreadKind::kMain);                                         \
    TRACE_EVENT_WITH_FLOW0(TRACE_GC_CATEGORIES, GCTracer::Scope::Name(GCTracer::Scope::ScopeId(scope_id)), bind_id, flow_flags)

#define TRACE_GC1(tracer, scope_id, thread_kind)                                                                                                               \
    GCTracer::Scope UNIQUE_IDENTIFIER(gc_tracer_scope)(tracer, GCTracer::Scope::ScopeId(scope_id), thread_kind);                                               \
    TRACE_EVENT0(TRACE_GC_CATEGORIES, GCTracer::Scope::Name(GCTracer::Scope::ScopeId(scope_id)))
...
#define TRACE_GC_EPOCH(tracer, scope_id, thread_kind)                                                                                                          \
    GCTracer::Scope UNIQUE_IDENTIFIER(gc_tracer_scope)(tracer, GCTracer::Scope::ScopeId(scope_id), thread_kind);                                               \
    TRACE_EVENT1(TRACE_GC_CATEGORIES, GCTracer::Scope::Name(GCTracer::Scope::ScopeId(scope_id)), "epoch", tracer->CurrentEpoch(scope_id))

#define TRACE_GC_EPOCH_WITH_FLOW(tracer, scope_id, thread_kind, bind_id, flow_flags)                                                                           \
    GCTracer::Scope UNIQUE_IDENTIFIER(gc_tracer_scope)(tracer, GCTracer::Scope::ScopeId(scope_id), thread_kind);                                               \
    TRACE_EVENT_WITH_FLOW1(                                                                                                                                    \
        TRACE_GC_CATEGORIES, GCTracer::Scope::Name(GCTracer::Scope::ScopeId(scope_id)), bind_id, flow_flags, "epoch", tracer->CurrentEpoch(scope_id))
...
#undef DEFINE_SCOPE
    ...
    static constexpr base::TimeDelta kThroughputTimeFrame = base::TimeDelta::FromSeconds(5);
    ...
    double YoungGenerationSpeedInBytesPerMillisecond(YoungGenerationSpeedMode mode) const;
...
private:
    ...
    static constexpr base::TimeDelta kSmoothedAllocationSpeedDecayRate = v8::base::TimeDelta::FromMilliseconds(100);

    SmoothedBytesAndDuration new_generation_allocations_ { kSmoothedAllocationSpeedDecayRate };
    SmoothedBytesAndDuration old_generation_allocations_ { kSmoothedAllocationSpeedDecayRate };
    SmoothedBytesAndDuration embedder_generation_allocations_ { kSmoothedAllocationSpeedDecayRate };
