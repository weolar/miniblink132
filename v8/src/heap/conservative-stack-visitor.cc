...
ConservativeStackVisitor::ConservativeStackVisitor(Isolate* isolate, RootVisitor* delegate)
    : ConservativeStackVisitor(isolate, delegate, delegate->collector())
{
}

ConservativeStackVisitor::ConservativeStackVisitor(Isolate* isolate, RootVisitor* delegate, GarbageCollector collector)
    : cage_base_(isolate)
    ,
...
#ifdef V8_COMPRESS_POINTERS
bool ConservativeStackVisitor::IsInterestingCage(PtrComprCageBase cage_base) const
{
...
Address ConservativeStackVisitor::FindBasePtr(Address maybe_inner_ptr, PtrComprCageBase cage_base) const
{
#ifdef V8_COMPRESS_POINTERS
    ...
    const MemoryChunk* chunk = nullptr;
#if defined(V8_ENABLE_CONSERVATIVE_STACK_SCANNING) || defined(DEBUG)
    chunk = allocator_->LookupChunkContainingAddress(maybe_inner_ptr);
#endif
    if (chunk == nullptr)
        return kNullAddress;
    ...
    if (chunk->IsLargePage()) {
        ...
        Tagged<HeapObject> obj(static_cast<const LargePageMetadata*>(chunk_metadata)->GetObject());
    ...
    if (v8_flags.sticky_mark_bits) {
        if (Heap::IsYoungGenerationCollector(collector_) && chunk->IsFlagSet(MemoryChunk::CONTAINS_ONLY_OLD))
            return kNullAddress;
    } else {
        if (Heap::IsYoungGenerationCollector(collector_) && !chunk->InYoungGeneration())
            return kNullAddress;
    ...
    Address base_ptr = MarkingBitmap::FindPreviousValidObject(page, maybe_inner_ptr);
...
#ifdef V8_COMPRESS_POINTERS
    V8HeapCompressionScheme::ProcessIntermediatePointers(cage_base_, address, [this](Address ptr) { VisitConservativelyIfPointer(ptr, cage_base_); });
#ifdef V8_EXTERNAL_CODE_SPACE
    ExternalCodeCompressionScheme::ProcessIntermediatePointers(
        code_cage_base_, address, [this](Address ptr) { VisitConservativelyIfPointer(ptr, code_cage_base_); });
#endif // V8_EXTERNAL_CODE_SPACE
#ifdef V8_ENABLE_SANDBOX
    TrustedSpaceCompressionScheme::ProcessIntermediatePointers(
        trusted_cage_base_, address, [this](Address ptr) { VisitConservativelyIfPointer(ptr, trusted_cage_base_); });
...
void ConservativeStackVisitor::VisitConservativelyIfPointer(Address address)
{
    ...
    // otherwise bail out.
    if (V8HeapCompressionScheme::GetPtrComprCageBaseAddress(address) == cage_base_.address()) {
...
void ConservativeStackVisitor::VisitConservativelyIfPointer(Address address, PtrComprCageBase cage_base)
{
    ...
    delegate_->VisitRootPointer(Root::kStackRoots, nullptr, FullObjectSlot(&root));
