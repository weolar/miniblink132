...
public:
    ...
    static constexpr int kNumTypes = static_cast<int>(ExternalBackingStoreType::kNumValues);
    static constexpr int kMemoryChunkAlignment = sizeof(size_t);
#define FIELD(Type, Name) k##Name##Offset, k##Name##End = k##Name##Offset + sizeof(Type) - 1
    enum Header {
        ...
        kMemoryChunkHeaderSize = kEndOfMetadataPointer
            + ((kEndOfMetadataPointer % kMemoryChunkAlignment) == 0 ? 0 : kMemoryChunkAlignment - (kEndOfMetadataPointer % kMemoryChunkAlignment)),
    };
    ...
    enum Metadata {
        ...
        kMutablePageMetadataSize = kEndOfMarkingBitmap
            + ((kEndOfMarkingBitmap % kMemoryChunkAlignment) == 0 ? 0 : kMemoryChunkAlignment - (kEndOfMarkingBitmap % kMemoryChunkAlignment)),
    };
#undef FIELD
    ...
    static constexpr intptr_t ObjectStartOffsetInCodePage()
    {
        ...
        // kCodeAlignment.
        return RoundUp(sizeof(MemoryChunk) + InstructionStream::kHeaderSize, kCodeAlignment) - InstructionStream::kHeaderSize;
    }
    ...
    {
        return RoundUp(int { kMemoryChunkHeaderSize }, ALIGN_TO_ALLOCATION_ALIGNMENT(kDoubleSize));
    }

    static constexpr size_t AllocatableMemoryInDataPage()
    {
        constexpr size_t kAllocatableMemoryInDataPage = kRegularPageSize - ObjectStartOffsetInDataPage();
    ...
    {
        constexpr int kMaxRegularCodeObjectSize = static_cast<int>(RoundDown(AllocatableMemoryInCodePage() / 2, kTaggedSize));
