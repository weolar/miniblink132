...
void FinalizationRegistryCleanupTask::RunInternal()
{
    ...
    TRACE_EVENT_CALL_STATS_SCOPED(isolate, "v8", "V8.FinalizationRegistryCleanupTask");
    ...
    // list.
    if (!heap_->DequeueDirtyJSFinalizationRegistry().ToHandle(&finalization_registry)) {
        return;
    }
    ...
    Handle<NativeContext> native_context(finalization_registry->native_context(), isolate);
    ...
    if (!microtask_queue)
        microtask_queue = isolate->default_microtask_queue();
    if (microtask_queue && microtask_queue->microtasks_policy() == v8::MicrotasksPolicy::kScoped) {
        ...
        microtasks_scope.reset(new v8::MicrotasksScope(v8_isolate, microtask_queue, v8::MicrotasksScope::kDoNotRunMicrotasks));
    }
    ...
    InvokeFinalizationRegistryCleanupFromTask(native_context, finalization_registry, callback);
    if (finalization_registry->NeedsCleanup() && !finalization_registry->scheduled_for_cleanup()) {
