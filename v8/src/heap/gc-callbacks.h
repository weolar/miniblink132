    ...
    void Invoke(GCType gc_type, GCCallbackFlags gc_callback_flags) const
    {
        ...
        for (const CallbackData& callback_data : callbacks_) {
            if (gc_type & callback_data.gc_type) {
                callback_data.callback(callback_data.isolate, gc_type, gc_callback_flags, callback_data.user_data);
            }
        }
    }
...
private:
    ...
    std::vector<CallbackData>::iterator FindCallback(CallbackType callback, void* data)
    {
        return std::find_if(callbacks_.begin(), callbacks_.end(),
            [callback, data](CallbackData& callback_data) { return callback_data.callback == callback && callback_data.user_data == data; });
    }
