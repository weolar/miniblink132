...
LargePageMetadata::LargePageMetadata(
    Heap* heap, BaseSpace* space, size_t chunk_size, Address area_start, Address area_end, VirtualMemory reservation, Executability executable)
    : MutablePageMetadata(heap, space, chunk_size, area_start, area_end, std::move(reservation), PageSize::kLarge)
{
    static_assert(LargePageMetadata::kMaxCodePageSize <= TypedSlotSet::kMaxOffset);
...
MemoryChunk::MainThreadFlags LargePageMetadata::InitialFlags(Executability executable) const
{
    return MutablePageMetadata::InitialFlags(executable) | MemoryChunk::LARGE_PAGE;
}
...
void LargePageMetadata::ClearOutOfLiveRangeSlots(Address free_start)
{
    ...
    Address aligned_area_end = ChunkAddress() + SlotSet::OffsetForBucket(buckets());
    DCHECK_LE(area_end(), aligned_area_end);
    RememberedSet<OLD_TO_SHARED>::RemoveRange(this, free_start, aligned_area_end, SlotSet::FREE_EMPTY_BUCKETS);
