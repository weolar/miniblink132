...
public:
    ...
    {
        return MemoryChunk::FromAddress(address1) == MemoryChunk::FromAddress(address2);
    }
    ...
    template <typename Callback> inline void ForAllFreeListCategories(Callback callback);
    ...
    {
        return active_system_pages_;
    }

    template <RememberedSetType remembered_set> void ClearTypedSlotsInFreeMemory(const TypedSlotSet::FreeRangesMap& ranges)
    {
...
static_assert(sizeof(MemoryChunkMetadata) <= MemoryChunkLayout::kMemoryChunkMetadataSize);
static_assert(sizeof(MutablePageMetadata) <= MemoryChunkLayout::kMutablePageMetadataSize);
static_assert(sizeof(PageMetadata) <= MemoryChunkLayout::kMutablePageMetadataSize);
...
template <> struct hash<i::PageMetadata*> : hash<i::MemoryChunkMetadata*> { };
template <> struct hash<const i::PageMetadata*> : hash<const i::MemoryChunkMetadata*> { };
} // namespace base
