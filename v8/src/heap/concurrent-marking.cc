...
using MemoryChunkDataMap = ::heap::base::CachedUnorderedMap<MutablePageMetadata*, MemoryChunkData>;

class ConcurrentMarkingVisitor final : public FullMarkingVisitorBase<ConcurrentMarkingVisitor> {
public:
    ConcurrentMarkingVisitor(MarkingWorklists::Local* local_marking_worklists, WeakObjects::Local* local_weak_objects, Heap* heap, unsigned mark_compact_epoch,
        base::EnumSet<CodeFlushMode> code_flush_mode, bool should_keep_ages_unchanged, uint16_t code_flushing_increase, MemoryChunkDataMap* memory_chunk_data)
    ...
    using FullMarkingVisitorBase<ConcurrentMarkingVisitor>::VisitMapPointerIfNeeded;
...
private:
    void RecordRelocSlot(Tagged<InstructionStream> host, RelocInfo* rinfo, Tagged<HeapObject> target)
    {
        ...
        MarkCompactCollector::RecordRelocSlotInfo info = MarkCompactCollector::ProcessRelocInfo(host, rinfo, target);
...
struct ConcurrentMarking::TaskState {
    ...
    PretenuringHandler::PretenuringFeedbackMap local_pretenuring_feedback { PretenuringHandler::kInitialFeedbackCapacity };
};

class ConcurrentMarking::JobTaskMajor : public v8::JobTask {
public:
    JobTaskMajor(
        ConcurrentMarking* concurrent_marking, unsigned mark_compact_epoch, base::EnumSet<CodeFlushMode> code_flush_mode, bool should_keep_ages_unchanged)
        : concurrent_marking_(concurrent_marking)
        , mark_compact_epoch_(mark_compact_epoch)
        , code_flush_mode_(code_flush_mode)
        , should_keep_ages_unchanged_(should_keep_ages_unchanged)
        , trace_id_(reinterpret_cast<uint64_t>(concurrent_marking) ^ concurrent_marking->heap_->tracer()->CurrentEpoch(GCTracer::Scope::MC_BACKGROUND_MARKING))
    {
    }
    ...
    void Run(JobDelegate* delegate) override
    {
        ...
        PtrComprCageAccessScope ptr_compr_cage_access_scope(concurrent_marking_->heap_->isolate());

        if (delegate->IsJoiningThread()) {
            // TRACE_GC is not needed here because the caller opens the right scope.
            concurrent_marking_->RunMajor(delegate, code_flush_mode_, mark_compact_epoch_, should_keep_ages_unchanged_);
        } else {
            TRACE_GC_EPOCH_WITH_FLOW(
                concurrent_marking_->heap_->tracer(), GCTracer::Scope::MC_BACKGROUND_MARKING, ThreadKind::kBackground, trace_id_, TRACE_EVENT_FLAG_FLOW_IN);
            concurrent_marking_->RunMajor(delegate, code_flush_mode_, mark_compact_epoch_, should_keep_ages_unchanged_);
        }
    }
...
public:
    explicit JobTaskMinor(ConcurrentMarking* concurrent_marking)
        : concurrent_marking_(concurrent_marking)
        , trace_id_(reinterpret_cast<uint64_t>(concurrent_marking) ^ concurrent_marking->heap_->tracer()->CurrentEpoch(GCTracer::Scope::MINOR_MS_MARK_PARALLEL))
    {
    }
    ...
    void Run(JobDelegate* delegate) override
    {
        ...
        if (delegate->IsJoiningThread()) {
            TRACE_GC_WITH_FLOW(concurrent_marking_->heap_->tracer(), GCTracer::Scope::MINOR_MS_MARK_PARALLEL, trace_id_, TRACE_EVENT_FLAG_FLOW_IN);
            // TRACE_GC is not needed here because the caller opens the right scope.
            concurrent_marking_->RunMinor(delegate);
        } else {
            TRACE_GC_EPOCH_WITH_FLOW(concurrent_marking_->heap_->tracer(), GCTracer::Scope::MINOR_MS_BACKGROUND_MARKING, ThreadKind::kBackground, trace_id_,
...
void ConcurrentMarking::RunMajor(
    JobDelegate* delegate, base::EnumSet<CodeFlushMode> code_flush_mode, unsigned mark_compact_epoch, bool should_keep_ages_unchanged)
{
    ...
    MarkingWorklists::Local local_marking_worklists(
        marking_worklists_, cpp_heap ? cpp_heap->CreateCppMarkingState() : MarkingWorklists::Local::kNoCppMarkingState);
    WeakObjects::Local local_weak_objects(weak_objects_);
    ConcurrentMarkingVisitor visitor(&local_marking_worklists, &local_weak_objects, heap_, mark_compact_epoch, code_flush_mode, should_keep_ages_unchanged,
    ...
    MainAllocator* const new_space_allocator = heap_->use_new_space() ? heap_->allocator()->new_space_allocator() : nullptr;

    {
        ...
        while (!done) {
            ...
            while (current_marked_bytes < kBytesUntilInterruptCheck && objects_processed < kObjectsUntilInterruptCheck) {
                ...
                if ((new_space_top <= addr && addr < new_space_limit) || addr == new_large_object) {
                    ...
                    visitor.IncrementLiveBytesCached(
                        MutablePageMetadata::cast(MemoryChunkMetadata::FromHeapObject(object)), ALIGN_TO_ALLOCATION_ALIGNMENT(visited_size));
                    if (is_per_context_mode) {
                        native_context_stats.IncrementSize(local_marking_worklists.Context(), map, object, visited_size);
                    }
            ...
            base::AsAtomicWord::Relaxed_Store<size_t>(&task_state->marked_bytes, marked_bytes);
    ...
    if (v8_flags.trace_concurrent_marking) {
        heap_->isolate()->PrintWithTimestamp("Major task %d concurrently marked %dKB in %.2fms\n", task_id, static_cast<int>(marked_bytes / KB), time_ms);
    }
...
V8_INLINE bool IsYoungObjectInLab(MainAllocator* new_space_allocator, NewLargeObjectSpace* new_lo_space, Tagged<HeapObject> heap_object)
{
    // The order of the two loads is important.
    ...
    return (new_space_top <= addr && addr < new_space_limit) || addr == new_large_object;
}

} // namespace

template <YoungGenerationMarkingVisitationMode marking_mode> V8_INLINE size_t ConcurrentMarking::RunMinorImpl(JobDelegate* delegate, TaskState* task_state)
{
    ...
    size_t current_marked_bytes = 0;
    int objects_processed = 0;
    YoungGenerationMarkingVisitor<marking_mode> visitor(heap_, &task_state->local_pretenuring_feedback);
    YoungGenerationRememberedSetsMarkingWorklist::Local remembered_sets(heap_->minor_mark_sweep_collector()->remembered_sets_marking_handler());
    auto& marking_worklists_local = visitor.marking_worklists_local();
    Isolate* isolate = heap_->isolate();
    minor_marking_state_->MarkerStarted();
    MainAllocator* const new_space_allocator = heap_->allocator()->new_space_allocator();
    ...
    do {
        ...
        TRACE_GC_EPOCH(heap_->tracer(), GCTracer::Scope::MINOR_MS_BACKGROUND_MARKING_CLOSURE, ThreadKind::kBackground);
        while (marking_worklists_local.Pop(&heap_object)) {
            if (IsYoungObjectInLab(new_space_allocator, new_lo_space, heap_object)) {
                ...
                if (visited_size) {
                    current_marked_bytes += visited_size;
                    visitor.IncrementLiveBytesCached(MutablePageMetadata::FromHeapObject(heap_object), ALIGN_TO_ALLOCATION_ALIGNMENT(visited_size));
                }
            }

            if (current_marked_bytes >= kBytesUntilInterruptCheck || ++objects_processed >= kObjectsUntilInterruptCheck) {
                marked_bytes += current_marked_bytes;
                if (delegate->ShouldYield()) {
...
void ConcurrentMarking::RunMinor(JobDelegate* delegate)
{
...

    {
        ...
        if (heap_->minor_mark_sweep_collector()->is_in_atomic_pause()) {
            ...
            marked_bytes = RunMinorImpl<YoungGenerationMarkingVisitationMode::kParallel>(delegate, task_state);
        } else {
            marked_bytes = RunMinorImpl<YoungGenerationMarkingVisitationMode::kConcurrent>(delegate, task_state);
        }
    }

    if (v8_flags.trace_concurrent_marking) {
        heap_->isolate()->PrintWithTimestamp("Minor task %d concurrently marked %dKB in %.2fms\n", task_id, static_cast<int>(marked_bytes / KB), time_ms);
    }
...
size_t ConcurrentMarking::GetMajorMaxConcurrency(size_t worker_count)
{
    ...
    const size_t work = std::max<size_t>({ marking_items, weak_objects_->discovered_ephemerons.Size(), weak_objects_->current_ephemerons.Size() });
...
{
    const size_t marking_items
        = marking_worklists_->shared()->Size() + heap_->minor_mark_sweep_collector()->remembered_sets_marking_handler()->RemainingRememberedSetsMarkingIteams();
...
void ConcurrentMarking::TryScheduleJob(GarbageCollector garbage_collector, TaskPriority priority)
{
    DCHECK(v8_flags.parallel_marking || v8_flags.concurrent_marking || v8_flags.concurrent_minor_ms_marking);
    DCHECK(!heap_->IsTearingDown());
    DCHECK(IsStopped());

    DCHECK_NE(garbage_collector, GarbageCollector::SCAVENGER);
    if (garbage_collector == GarbageCollector::MARK_COMPACTOR && !heap_->mark_compact_collector()->UseBackgroundThreadsInCycle()) {
        return;
    }
    if (garbage_collector == GarbageCollector::MINOR_MARK_SWEEPER && !heap_->minor_mark_sweep_collector()->UseBackgroundThreadsInCycle()) {
        return;
    }
    ...
    // stopped.
    DCHECK_IMPLIES(minor_marking_state_,
        garbage_collector_.has_value() && (*garbage_collector_ == garbage_collector) && (garbage_collector == GarbageCollector::MINOR_MARK_SWEEPER));
    DCHECK_IMPLIES(!garbage_collector_.has_value() || *garbage_collector_ == GarbageCollector::MARK_COMPACTOR,
        std::all_of(task_state_.begin(), task_state_.end(), [](auto& task_state) { return task_state->local_pretenuring_feedback.empty(); }));
    ...
    if (garbage_collector == GarbageCollector::MARK_COMPACTOR) {
        ...
        auto job = std::make_unique<JobTaskMajor>(
            this, heap_->mark_compact_collector()->epoch(), heap_->mark_compact_collector()->code_flush_mode(), heap_->ShouldCurrentGCKeepAgesUnchanged());
        current_job_trace_id_.emplace(job->trace_id());
        TRACE_GC_NOTE_WITH_FLOW("Major concurrent marking started", job->trace_id(), TRACE_EVENT_FLAG_FLOW_OUT);
        ...
        marking_worklists_ = heap_->minor_mark_sweep_collector()->marking_worklists();
        auto job = std::make_unique<JobTaskMinor>(this);
        current_job_trace_id_.emplace(job->trace_id());
        TRACE_GC_NOTE_WITH_FLOW("Minor concurrent marking started", job->trace_id(), TRACE_EVENT_FLAG_FLOW_OUT);
        job_handle_ = V8::GetCurrentPlatform()->PostJob(priority, std::move(job));
    }
...
bool ConcurrentMarking::IsWorkLeft() const
{
    ...
    if (garbage_collector_ == GarbageCollector::MARK_COMPACTOR) {
        return !marking_worklists_->shared()->IsEmpty() || !weak_objects_->current_ephemerons.IsEmpty() || !weak_objects_->discovered_ephemerons.IsEmpty();
    }
    DCHECK_EQ(GarbageCollector::MINOR_MARK_SWEEPER, garbage_collector_);
    return !marking_worklists_->shared()->IsEmpty()
        || (heap_->minor_mark_sweep_collector()->remembered_sets_marking_handler()->RemainingRememberedSetsMarkingIteams() > 0);
}

void ConcurrentMarking::RescheduleJobIfNeeded(GarbageCollector garbage_collector, TaskPriority priority)
{
    DCHECK(v8_flags.parallel_marking || v8_flags.concurrent_marking || v8_flags.concurrent_minor_ms_marking);
    ...
    if (IsStopped()) {
        ...
        DCHECK_IMPLIES(garbage_collector_.has_value(), garbage_collector == garbage_collector_);
        ...
        TRACE_GC_NOTE_WITH_FLOW(
            garbage_collector_ == GarbageCollector::MARK_COMPACTOR ? "Major concurrent marking rescheduled" : "Minor concurrent marking rescheduled",
            current_job_trace_id_.value(), TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT);
...
void ConcurrentMarking::FlushPretenuringFeedback()
{
    ...
    for (auto& task_state : task_state_) {
        pretenuring_handler->MergeAllocationSitePretenuringFeedback(task_state->local_pretenuring_feedback);
...
{
    DCHECK(v8_flags.parallel_marking || v8_flags.concurrent_marking || v8_flags.concurrent_minor_ms_marking);
    DCHECK_IMPLIES(garbage_collector_ == GarbageCollector::MARK_COMPACTOR, heap_->mark_compact_collector()->UseBackgroundThreadsInCycle());
...
bool ConcurrentMarking::Pause()
{
    ...
    job_handle_->Cancel();
    DCHECK(current_job_trace_id_.has_value());
    TRACE_GC_NOTE_WITH_FLOW(garbage_collector_ == GarbageCollector::MARK_COMPACTOR ? "Major concurrent marking paused" : "Minor concurrent marking paused",
        current_job_trace_id_.value(), TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT);
    return true;
}
...
void ConcurrentMarking::Resume()
{
    DCHECK(garbage_collector_.has_value());
    DCHECK(current_job_trace_id_.has_value());
    TRACE_GC_NOTE_WITH_FLOW(garbage_collector_ == GarbageCollector::MARK_COMPACTOR ? "Major concurrent marking resumed" : "Minor concurrent marking resumed",
        current_job_trace_id_.value(), TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT);
...
void ConcurrentMarking::FlushMemoryChunkData()
{
    ...
    for (size_t i = 1; i < task_state_.size(); i++) {
        ...
        for (auto& pair : memory_chunk_data) {
            ...
            if (data.typed_slots) {
                RememberedSet<OLD_TO_OLD>::MergeTyped(memory_chunk, std::move(data.typed_slots));
            }
        }
...
ConcurrentMarking::PauseScope::PauseScope(ConcurrentMarking* concurrent_marking)
    : concurrent_marking_(concurrent_marking)
    , resume_on_exit_(v8_flags.concurrent_marking && concurrent_marking_->Pause())
{
...
{
    if (resume_on_exit_) {
        DCHECK_EQ(concurrent_marking_->garbage_collector_, GarbageCollector::MARK_COMPACTOR);
