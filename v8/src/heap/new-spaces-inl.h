...
bool SemiSpace::Contains(Tagged<HeapObject> o) const
{
    ...
    if (memory_chunk->IsLargePage())
        return false;
    return id_ == kToSpace ? memory_chunk->IsToPage() : memory_chunk->IsFromPage();
}
...
template <typename T> inline bool SemiSpace::Contains(Tagged<T> o) const
{
...
bool SemiSpace::ContainsSlow(Address a) const
{
    for (const PageMetadata* p : *this) {
        if (p == MemoryChunkMetadata::FromAddress(a))
            return true;
    }
    return false;
}
...
void SemiSpaceNewSpace::IncrementAllocationTop(Address new_top)
{
    ...
    DCHECK_EQ(PageMetadata::FromAllocationAreaAddress(allocation_top_), PageMetadata::FromAllocationAreaAddress(new_top));
