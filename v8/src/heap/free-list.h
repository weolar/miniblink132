...
public:
    ...
    template <typename Callback> void IterateNodesForTesting(Callback callback)
    {
...
public:
    ...
    V8_EXPORT_PRIVATE static std::unique_ptr<FreeList> CreateFreeListForNewSpace();
    ...
    V8_EXPORT_PRIVATE virtual PageMetadata* GetPageForSize(size_t size_in_bytes) = 0;
    ...
    template <typename Callback> void ForAllFreeListCategories(FreeListCategoryType type, Callback callback)
    {
    ...
    template <typename Callback> void ForAllFreeListCategories(Callback callback)
    {
...
protected:
    ...
    static constexpr unsigned int categories_min[kNumberOfCategories]
        = { 24, 32, 48, 64, 80, 96, 112, 128, 144, 160, 176, 192, 208, 224, 240, 256, 512, 1024, 2048, 4096, 8192, 16384, 32768, 65536 };
    ...
    FreeListCategoryType SelectFreeListCategoryType(size_t size_in_bytes) override
    {
        if (size_in_bytes <= kPreciseCategoryMaxSize) {
            if (size_in_bytes < categories_min[1])
                return 0;
...
protected:
    ...
    {
        for (int i = cat; i >= kFirstCategory && next_nonempty_category[i] > cat; i--) {
    ...
    void CheckCacheIntegrity()
    {
        for (int i = 0; i <= last_category_; i++) {
            DCHECK(next_nonempty_category[i] == last_category_ + 1 || categories_[next_nonempty_category[i]] != nullptr);
...
class V8_EXPORT_PRIVATE FreeListManyCachedFastPathBase : public FreeListManyCached {
public:
        ...
        : small_blocks_mode_(small_blocks_mode)
    {
        if (small_blocks_mode_ == SmallBlocksMode::kProhibit) {
            min_block_size_ = (v8_flags.minor_ms && (v8_flags.minor_ms_min_lab_size_kb > 0)) ? (v8_flags.minor_ms_min_lab_size_kb * KB) : kFastPathStart;
        }
    }
...
protected:
    ...
    static_assert(categories_min[kFastPathFallBackTiny] == kTinyObjectMaxSize * 2);
...
class FreeListManyCachedFastPathForNewSpace : public FreeListManyCachedFastPathBase {
public:
...
class V8_EXPORT_PRIVATE FreeListManyCachedOrigin : public FreeListManyCachedFastPath {
public:
    V8_WARN_UNUSED_RESULT Tagged<FreeSpace> Allocate(size_t size_in_bytes, size_t* node_size, AllocationOrigin origin) override;
};
