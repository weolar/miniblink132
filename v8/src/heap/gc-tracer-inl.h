...
constexpr GCTracer::IncrementalInfos& GCTracer::IncrementalInfos::operator+=(base::TimeDelta delta)
{
    steps++;
    duration += delta;
    ...
    , start_time_(base::TimeTicks::Now())
{
    ...
    if (thread_kind_ == ThreadKind::kMain) {
        ...
        runtime_call_stats_scope_.emplace(tracer->worker_thread_runtime_call_stats());
...
GCTracer::Scope::~Scope()
{
    ...
    if (thread_kind_ == ThreadKind::kMain) {
        if (scope_ == ScopeId::MC_INCREMENTAL || scope_ == ScopeId::MC_INCREMENTAL_START || scope_ == ScopeId::MC_INCREMENTAL_FINALIZE) {
            auto* long_task_stats = tracer_->heap_->isolate_->GetCurrentLongTaskStats();
            long_task_stats->gc_full_incremental_wall_clock_duration_us += duration.InMicroseconds();
        }
    }
...
constexpr bool GCTracer::Event::IsYoungGenerationEvent(Type type)
{
    ...
    return type == Type::SCAVENGER || type == Type::MINOR_MARK_SWEEPER || type == Type::INCREMENTAL_MINOR_MARK_SWEEPER;
}
...
constexpr const GCTracer::IncrementalInfos& GCTracer::incremental_scope(Scope::ScopeId id) const
{
...
{
    if (Scope::FIRST_INCREMENTAL_SCOPE <= id && id <= Scope::LAST_INCREMENTAL_SCOPE) {
        incremental_scopes_[Scope::IncrementalOffset(id)] += duration;
    } else if (Scope::FIRST_BACKGROUND_SCOPE <= id && id <= Scope::LAST_BACKGROUND_SCOPE) {
...
RuntimeCallCounterId GCTracer::RCSCounterFromScope(Scope::ScopeId id)
{
    ...
    return static_cast<RuntimeCallCounterId>(static_cast<int>(RuntimeCallCounterId::kGC_MC_INCREMENTAL) + static_cast<int>(id));
}
#endif // defined(V8_RUNTIME_CALL_STATS)
