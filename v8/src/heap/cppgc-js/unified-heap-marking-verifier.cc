...
public:
    ...
    void VisitWeak(const void*, cppgc::TraceDescriptor desc, cppgc::WeakCallback, const void*) final
    {
    ...
    void VisitWeakContainer(const void* object, cppgc::TraceDescriptor, cppgc::TraceDescriptor weak_desc, cppgc::WeakCallback, const void*) final
    {
        if (!object)
            return;
...
void UnifiedHeapVerificationState::VerifyMarkedTracedReference(const TracedReferenceBase& ref) const
{
    ...
    Address* traced_handle_location = BasicTracedReferenceExtractor::GetObjectSlotForMarking(ref);
    ...
    if (!TracedHandles::IsValidInUseNode(traced_handle_location)) {
        FATAL("MarkingVerifier: Encountered unmarked TracedReference.\n"
              "#\n"
              "# Hint:\n"
              "#   %s (%p)\n"
              "#     \\-> TracedReference (%p)",
            parent_ ? parent_->GetName(cppgc::internal::HeapObjectNameForUnnamedObject::kUseClassNameIfSupported).value : "Stack",
            parent_ ? parent_->ObjectStart() : nullptr, &ref);
    }
}

UnifiedHeapMarkingVerifier::UnifiedHeapMarkingVerifier(cppgc::internal::HeapBase& heap_base, cppgc::internal::CollectionType collection_type)
    : MarkingVerifierBase(heap_base, collection_type, state_, std::make_unique<UnifiedHeapVerificationVisitor>(state_))
{
}
