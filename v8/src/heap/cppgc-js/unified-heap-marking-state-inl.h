...
void UnifiedHeapMarkingState::MarkAndPush(const TracedReferenceBase& reference)
{
    ...
    Address* traced_handle_location = BasicTracedReferenceExtractor::GetObjectSlotForMarking(reference);
    ...
    Tagged<Object> object = TracedHandles::Mark(traced_handle_location, mark_mode_);
    ...
    const auto worklist_target = MarkingHelper::ShouldMarkObject(heap_, heap_object);
    if (worklist_target) {
        MarkingHelper::TryMarkAndPush(heap_, local_marking_worklist_, marking_state_, worklist_target.value(), heap_object);
    }
}
