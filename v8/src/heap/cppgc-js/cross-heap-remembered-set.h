...
public:
    ...
    template <typename F> void Visit(Isolate&, F);

    bool IsEmpty() const
    {
        return remembered_v8_to_cppgc_references_.empty();
    }
...
template <typename F> void CrossHeapRememberedSet::Visit(Isolate& isolate, F f)
{
