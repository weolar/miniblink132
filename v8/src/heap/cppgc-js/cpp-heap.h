...
static_assert(false, "V8 targets can not be built with cppgc_is_standalone set to true.");
#endif
...
public:
    ...
    public:
        ...
        explicit MetricRecorderAdapter(CppHeap& cpp_heap)
            : cpp_heap_(cpp_heap)
        {
        }
    ...
    template <typename F> inline void VisitCrossHeapRememberedSetIfNeeded(F f);
...
private:
    ...
    {
        return collection_type_.has_value();
    }
...
void CppHeap::RememberCrossHeapReferenceIfNeeded(v8::internal::Tagged<v8::internal::JSObject> host_obj, void* value)
{
    if (!generational_gc_supported())
        return;
    DCHECK(isolate_);
    cross_heap_remembered_set_.RememberReferenceIfNeeded(*isolate_, host_obj, value);
}

template <typename F> void CppHeap::VisitCrossHeapRememberedSetIfNeeded(F f)
{
    if (!generational_gc_supported())
        return;
    DCHECK(isolate_);
