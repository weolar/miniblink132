...
public:
    explicit CppMarkingState(cppgc::internal::MarkingStateBase& main_thread_marking_state)
        : owned_marking_state_(nullptr)
        , marking_state_(main_thread_marking_state)
    {
    }

    explicit CppMarkingState(std::unique_ptr<cppgc::internal::MarkingStateBase> concurrent_marking_state)
