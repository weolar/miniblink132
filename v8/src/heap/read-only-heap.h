...
public:
    ...
    // must be initialized
    V8_EXPORT_PRIVATE inline static ReadOnlyRoots GetReadOnlyRoots(Tagged<HeapObject> object);
    ...
    V8_EXPORT_PRIVATE inline static ReadOnlyRoots EarlyGetReadOnlyRoots(Tagged<HeapObject> object);
...
protected:
    ...
    static ReadOnlyHeap* CreateInitialHeapForBootstrapping(Isolate* isolate, ReadOnlyArtifacts* artifacts);
...
public:
    explicit ReadOnlyPageObjectIterator(const ReadOnlyPageMetadata* page, SkipFreeSpaceOrFiller skip_free_space_or_filler = SkipFreeSpaceOrFiller::kYes);
    ReadOnlyPageObjectIterator(
        const ReadOnlyPageMetadata* page, Address current_addr, SkipFreeSpaceOrFiller skip_free_space_or_filler = SkipFreeSpaceOrFiller::kYes);
