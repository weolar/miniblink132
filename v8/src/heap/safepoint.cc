...
void IsolateSafepoint::InitiateGlobalSafepointScope(Isolate* initiator, PerClientSafepointData* client_data)
{
...
void IsolateSafepoint::TryInitiateGlobalSafepointScope(Isolate* initiator, PerClientSafepointData* client_data)
{
    shared_space_isolate()->global_safepoint()->AssertActive();
...
void IsolateSafepoint::InitiateGlobalSafepointScopeRaw(Isolate* initiator, PerClientSafepointData* client_data)
{
    CHECK_EQ(++active_safepoint_scopes_, 1);
    barrier_.Arm();

    size_t running = SetSafepointRequestedFlags(ShouldIncludeMainThread(initiator));
    ...
    if (isolate() != initiator) {
        ...
        // wake it up.
        isolate()->heap()->GetForegroundTaskRunner()->PostTask(std::make_unique<GlobalSafepointInterruptTask>(heap_));
...
IsolateSafepoint::IncludeMainThread IsolateSafepoint::ShouldIncludeMainThread(Isolate* initiator)
{
    const bool is_initiator = isolate() == initiator;
    return is_initiator ? IncludeMainThread::kNo : IncludeMainThread::kYes;
}

size_t IsolateSafepoint::SetSafepointRequestedFlags(IncludeMainThread include_main_thread)
{
    size_t running = 0;
    ...
    for (LocalHeap* local_heap = local_heaps_head_; local_heap; local_heap = local_heap->next_) {
        if (local_heap->is_main_thread() && include_main_thread == IncludeMainThread::kNo) {
            continue;
        }

        const LocalHeap::ThreadState old_state = local_heap->state_.SetSafepointRequested();

        if (old_state.IsRunning())
            running++;
        CHECK_IMPLIES(old_state.IsCollectionRequested(), local_heap->is_main_thread());
...
void IsolateSafepoint::ClearSafepointRequestedFlags(IncludeMainThread include_main_thread)
{
    for (LocalHeap* local_heap = local_heaps_head_; local_heap; local_heap = local_heap->next_) {
        if (local_heap->is_main_thread() && include_main_thread == IncludeMainThread::kNo) {
            continue;
        ...
        const LocalHeap::ThreadState old_state = local_heap->state_.ClearSafepointRequested();
...
{
    barrier_.WaitInSafepoint();
}

void IsolateSafepoint::WaitInUnpark()
{
    barrier_.WaitInUnpark();
}
...
void IsolateSafepoint::WaitUntilRunningThreadsInSafepoint(const PerClientSafepointData* client_data)
{
...
void IsolateSafepoint::Barrier::WaitUntilRunningThreadsInSafepoint(size_t running)
{
    base::MutexGuard guard(&mutex_);
    DCHECK(IsArmed());
...
{
    const auto scoped_blocking_call = V8::GetCurrentPlatform()->CreateBlockingScope(BlockingType::kWillBlock);
    base::MutexGuard guard(&mutex_);
    CHECK(IsArmed());
    stopped_++;
    cv_stopped_.NotifyOne();
...
{
    AssertActive();
    for (LocalHeap* current = local_heaps_head_; current; current = current->next_) {
...
void GlobalSafepoint::RemoveClient(Isolate* client)
{
    ...
    if (client->global_safepoint_next_client_isolate_) {
        client->global_safepoint_next_client_isolate_->global_safepoint_prev_client_isolate_ = client->global_safepoint_prev_client_isolate_;
    }

    if (client->global_safepoint_prev_client_isolate_) {
        client->global_safepoint_prev_client_isolate_->global_safepoint_next_client_isolate_ = client->global_safepoint_next_client_isolate_;
    } else {
...
void GlobalSafepoint::EnterGlobalSafepointScope(Isolate* initiator)
{
    ...
    if (!clients_mutex_.TryLock()) {
        ...
        initiator->main_thread_local_heap()->ExecuteWhileParked([this]() { clients_mutex_.Lock(); });
    }

    if (++active_safepoint_scopes_ > 1)
        return;

    TimedHistogramScope timer(initiator->counters()->gc_time_to_global_safepoint());
    TRACE_GC(initiator->heap()->tracer(), GCTracer::Scope::TIME_TO_GLOBAL_SAFEPOINT);
    ...
    IterateSharedSpaceAndClientIsolates([&clients, initiator](Isolate* client) {
        ...
        client->heap()->safepoint()->TryInitiateGlobalSafepointScope(initiator, &clients.back());
    });
...
{
    shared_space_isolate_->global_safepoint()->EnterGlobalSafepointScope(initiator_);
}

GlobalSafepointScope::~GlobalSafepointScope()
{
    shared_space_isolate_->global_safepoint()->LeaveGlobalSafepointScope(initiator_);
}
...
SafepointScope::SafepointScope(Isolate* initiator, GlobalSafepointForSharedSpaceIsolateTag)
{
