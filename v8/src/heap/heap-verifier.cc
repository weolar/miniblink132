...
namespace {
thread_local Tagged<HeapObject> pending_layout_change_object = Tagged<HeapObject>();
} // namespace
...
class VerifyPointersVisitor : public HeapVisitor<VerifyPointersVisitor>, public RootVisitor {
public:
...
protected:
    ...
    virtual void VerifyPointers(Tagged<HeapObject> host, MaybeObjectSlot start, MaybeObjectSlot end);

    Heap* heap_;
};

void VerifyPointersVisitor::VisitPointers(Tagged<HeapObject> host, ObjectSlot start, ObjectSlot end)
{
    VerifyPointers(host, MaybeObjectSlot(start), MaybeObjectSlot(end));
}

void VerifyPointersVisitor::VisitPointers(Tagged<HeapObject> host, MaybeObjectSlot start, MaybeObjectSlot end)
{
    VerifyPointers(host, start, end);
}

void VerifyPointersVisitor::VisitInstructionStreamPointer(Tagged<Code> host, InstructionStreamSlot slot)
{
...
void VerifyPointersVisitor::VisitRootPointers(Root root, const char* description, FullObjectSlot start, FullObjectSlot end)
{
    VerifyPointersImpl(start, end);
}

void VerifyPointersVisitor::VisitRootPointers(Root root, const char* description, OffHeapObjectSlot start, OffHeapObjectSlot end)
{
    VerifyPointersImpl(start, end);
}
...
void VerifyPointersVisitor::VerifyHeapObjectImpl(Tagged<HeapObject> heap_object)
{
    ...
    CHECK_IMPLIES(!v8_flags.sticky_mark_bits && HeapLayout::InYoungGeneration(heap_object), Heap::InToPage(heap_object));
}

void VerifyPointersVisitor::VerifyCodeObjectImpl(Tagged<HeapObject> heap_object)
{
    CHECK(IsValidCodeObject(heap_, heap_object));
    CHECK(IsMap(heap_object->map(cage_base())));
    CHECK(heap_object->map(cage_base())->instance_type() == INSTRUCTION_STREAM_TYPE);
}

template <typename TSlot> void VerifyPointersVisitor::VerifyPointersImpl(TSlot start, TSlot end)
{
    for (TSlot slot = start; slot < end; ++slot) {
        ...
        if (object.GetHeapObject(&heap_object)) {
            ...
            CHECK(IsSmi(object) || object.IsCleared() || MapWord::IsPacked(object.ptr()));
        }
    }
}

void VerifyPointersVisitor::VerifyPointers(Tagged<HeapObject> host, MaybeObjectSlot start, MaybeObjectSlot end)
{
    ...
    CHECK_EQ(ObjectFields::kMaybePointers, Map::ObjectFieldsFrom(host->map(cage_base())->visitor_id()));
    VerifyPointersImpl(start, end);
}

void VerifyPointersVisitor::VisitCodeTarget(Tagged<InstructionStream> host, RelocInfo* rinfo)
{
    Tagged<InstructionStream> target = InstructionStream::FromTargetAddress(rinfo->target_address());
    VerifyHeapObjectImpl(target);
}

void VerifyPointersVisitor::VisitEmbeddedPointer(Tagged<InstructionStream> host, RelocInfo* rinfo)
{
...
protected:
    void VerifyPointers(Tagged<HeapObject> host, MaybeObjectSlot start, MaybeObjectSlot end) override
    {
        if (!host.is_null()) {
    ...
    void VerifyPointers(Tagged<HeapObject> host, MaybeObjectSlot start, MaybeObjectSlot end) override
    {
        ...
        for (MaybeObjectSlot current = start; current < end; ++current) {
            ...
            if ((*current).GetHeapObject(&heap_object)) {
                ...
                CHECK(ReadOnlyHeap::Contains(heap_object) || shared_space_->Contains(heap_object) || shared_lo_space_->Contains(heap_object)
                    || shared_trusted_space_->Contains(heap_object) || shared_trusted_lo_space_->Contains(heap_object));
            }
        }
    }
...
private:
    ...
    LargeObjectSpace* lo_space() const
    {
...
void HeapVerification::Verify()
{
    ...
    SafepointKind safepoint_kind = isolate()->is_shared_space_isolate() ? SafepointKind::kGlobal : SafepointKind::kIsolate;
    ...
    heap()->IterateRoots(&visitor, base::EnumSet<SkipRoot> { SkipRoot::kConservativeStack });

    if (!isolate()->context().is_null() && !isolate()->raw_native_context().is_null()) {
        Tagged<Object> normalized_map_cache = isolate()->raw_native_context()->normalized_map_cache();

        if (IsNormalizedMapCache(normalized_map_cache)) {
            Cast<NormalizedMapCache>(normalized_map_cache)->NormalizedMapCacheVerify(isolate());
        }
    }
...
void HeapVerification::VerifyObjectMap(Tagged<HeapObject> object)
{
    ...
    CHECK(ReadOnlyHeap::Contains(map) || old_space()->Contains(map) || (shared_space() && shared_space()->Contains(map)));
...
public:
    SlotVerifyingVisitor(Isolate* isolate, std::set<Address>* untyped, std::set<std::pair<SlotType, Address>>* typed, std::set<Address>* protected_pointer)
        : HeapVisitor(isolate)
        , untyped_(untyped)
        , typed_(typed)
    ...
    virtual bool ShouldHaveBeenRecorded(Tagged<HeapObject> host, Tagged<MaybeObject> target) = 0;
    ...
    void VisitCodeTarget(Tagged<InstructionStream> host, RelocInfo* rinfo) override
    {
        Tagged<Object> target = InstructionStream::FromTargetAddress(rinfo->target_address());
        if (ShouldHaveBeenRecorded(host, target)) {
            CHECK(InTypedSet(SlotType::kCodeEntry, rinfo->pc())
                || (rinfo->IsInConstantPool() && InTypedSet(SlotType::kConstPoolCodeEntry, rinfo->constant_pool_entry_address())));
        }
    }
    ...
    void VisitEmbeddedPointer(Tagged<InstructionStream> host, RelocInfo* rinfo) override
    {
        ...
        if (ShouldHaveBeenRecorded(host, target)) {
            CHECK(InTypedSet(SlotType::kEmbeddedObjectFull, rinfo->pc()) || InTypedSet(SlotType::kEmbeddedObjectCompressed, rinfo->pc())
                || (rinfo->IsInConstantPool() && InTypedSet(SlotType::kConstPoolEmbeddedObjectCompressed, rinfo->constant_pool_entry_address()))
                || (rinfo->IsInConstantPool() && InTypedSet(SlotType::kConstPoolEmbeddedObjectFull, rinfo->constant_pool_entry_address())));
        }
    }
...
public:
    OldToNewSlotVerifyingVisitor(
        Isolate* isolate, std::set<Address>* untyped, std::set<std::pair<SlotType, Address>>* typed, EphemeronRememberedSet::TableMap* ephemeron_remembered_set)
        : SlotVerifyingVisitor(isolate, untyped, typed, nullptr)
        , ephemeron_remembered_set_(ephemeron_remembered_set)
    {
    }

    bool ShouldHaveBeenRecorded(Tagged<HeapObject> host, Tagged<MaybeObject> target) override
    {
        // Heap::InToPage() is not available with sticky mark-bits.
        CHECK_IMPLIES(!v8_flags.sticky_mark_bits && target.IsStrongOrWeak() && HeapLayout::InYoungGeneration(target), Heap::InToPage(target));
        return target.IsStrongOrWeak() && HeapLayout::InYoungGeneration(target) && !HeapLayout::InYoungGeneration(host);
    }

    void VisitEphemeron(Tagged<HeapObject> host, int index, ObjectSlot key, ObjectSlot target) override
    {
        ...
        Tagged<Object> k = *key;
        if (!HeapLayout::InYoungGeneration(host) && HeapLayout::InYoungGeneration(k)) {
            ...
            int slot_index = EphemeronHashTable::SlotToIndex(table.address(), key.address());
...
public:
    OldToSharedSlotVerifyingVisitor(
        Isolate* isolate, std::set<Address>* untyped, std::set<std::pair<SlotType, Address>>* typed, std::set<Address>* protected_pointer)
    ...
    bool ShouldHaveBeenRecorded(Tagged<HeapObject> host, Tagged<MaybeObject> target) override
    {
        ...
        return target.GetHeapObject(&target_heap_object) && HeapLayout::InWritableSharedSpace(target_heap_object)
            && !(v8_flags.black_allocated_pages && HeapLayout::InBlackAllocatedPage(target_heap_object)) && !HeapLayout::InYoungGeneration(host)
            && !HeapLayout::InWritableSharedSpace(host);
    }
};

template <RememberedSetType direction>
void CollectSlots(MutablePageMetadata* chunk, Address start, Address end, std::set<Address>* untyped, std::set<std::pair<SlotType, Address>>* typed)
{
    ...
    RememberedSet<direction>::IterateTyped(chunk, [=](SlotType type, Address slot) {
...
public:
    explicit SlotCollectingVisitor(Isolate* isolate)
        : HeapVisitor(isolate)
    {
    }
    ...
    MaybeObjectSlot slot(int i)
    {
        return slots_[i];
    }
#ifdef V8_EXTERNAL_CODE_SPACE
    InstructionStreamSlot code_slot(int i)
    {
        return code_slots_[i];
    }
    int number_of_code_slots()
    {
...
{
    if (current_space_identity() == RO_SPACE || v8_flags.verify_heap_skip_remembered_set) {
        return;
    }
    ...
    CollectSlots<OLD_TO_NEW_BACKGROUND>(chunk, start, end, &old_to_new, &typed_old_to_new);

    OldToNewSlotVerifyingVisitor old_to_new_visitor(isolate(), &old_to_new, &typed_old_to_new, heap()->ephemeron_remembered_set()->tables());
    old_to_new_visitor.Visit(object);

    std::set<Address> old_to_shared;
    std::set<std::pair<SlotType, Address>> typed_old_to_shared;
    CollectSlots<OLD_TO_SHARED>(chunk, start, end, &old_to_shared, &typed_old_to_shared);
    std::set<Address> trusted_to_shared_trusted;
    CollectSlots<TRUSTED_TO_SHARED_TRUSTED>(chunk, start, end, &trusted_to_shared_trusted, nullptr);
    OldToSharedSlotVerifyingVisitor old_to_shared_visitor(isolate(), &old_to_shared, &typed_old_to_shared, &trusted_to_shared_trusted);
...
// static
void HeapVerifier::VerifyObjectLayoutChangeIsAllowed(Heap* heap, Tagged<HeapObject> object)
{
    if (HeapLayout::InWritableSharedSpace(object)) {
        ...
        if (IsShared(object)) {
            ...
            Isolate* shared_space_isolate = isolate->is_shared_space_isolate() ? isolate : isolate->shared_space_isolate();
...
// static
void HeapVerifier::SetPendingLayoutChangeObject(Heap* heap, Tagged<HeapObject> object)
{
...
// static
void HeapVerifier::VerifyObjectLayoutChange(Heap* heap, Tagged<HeapObject> object, Tagged<Map> new_map)
{
...
// static
void HeapVerifier::VerifySafeMapTransition(Heap* heap, Tagged<HeapObject> object, Tagged<Map> new_map)
{
    PtrComprCageBase cage_base(heap->isolate());
    ...
    if (IsString(object, cage_base)
        && (new_map == ReadOnlyRoots(heap).thin_two_byte_string_map() || new_map == ReadOnlyRoots(heap).thin_one_byte_string_map())) {
    ...
    if (v8_flags.shared_string_table && IsString(object, cage_base) && InstanceTypeChecker::IsInternalizedString(new_map->instance_type())) {
...
#ifdef V8_EXTERNAL_CODE_SPACE
    CHECK_EQ(new_visitor.number_of_code_slots(), old_visitor.number_of_code_slots());
