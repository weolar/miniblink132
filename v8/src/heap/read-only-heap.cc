...
ReadOnlyHeap::~ReadOnlyHeap()
{
    ...
    IsolateGroup::current()->code_pointer_table()->TearDownSpace(&code_pointer_space_);
#endif
#ifdef V8_ENABLE_LEAPTIERING
    GetProcessWideJSDispatchTable()->DetachSpaceFromReadOnlySegment(&js_dispatch_table_space_);
    GetProcessWideJSDispatchTable()->TearDownSpace(&js_dispatch_table_space_);
#endif
}

// static
void ReadOnlyHeap::SetUp(Isolate* isolate, SnapshotData* read_only_snapshot_data, bool can_rehash)
{
    ...
    if (IsReadOnlySpaceShared()) {
        ...
        if (read_only_snapshot_data != nullptr) {
            ...
            if (!artifacts) {
                ...
                ro_heap->DeserializeIntoIsolate(isolate, read_only_snapshot_data, can_rehash);
                ...
                isolate->external_pointer_table().SetUpFromReadOnlyArtifacts(isolate->heap()->read_only_external_pointer_space(), artifacts);
        ...
        ReadOnlyHeap* ro_heap = new ReadOnlyHeap(new ReadOnlySpace(isolate->heap()));
...
void ReadOnlyHeap::TearDown(Isolate* isolate)
{
    ...
    if (group->DecrementIsolateCount() == 0) {
        ...
        if (isolate->is_shared_space_isolate())
            group->ClearSharedSpaceIsolate();
        group->ClearReadOnlyArtifacts();
    }
}

void ReadOnlyHeap::DeserializeIntoIsolate(Isolate* isolate, SnapshotData* read_only_snapshot_data, bool can_rehash)
{
...
void ReadOnlyHeap::OnCreateHeapObjectsComplete(Isolate* isolate)
{
    ...
    isolate->heap()->EnsureSweepingCompleted(Heap::SweepingForcedFinalizationMode::kV8Only);
...
// static
ReadOnlyHeap* ReadOnlyHeap::CreateInitialHeapForBootstrapping(Isolate* isolate, ReadOnlyArtifacts* artifacts)
{
...
{
    void* const isolate_ro_roots = isolate->roots_table().read_only_roots_begin().location();
    std::memcpy(isolate_ro_roots, read_only_roots_, kEntriesCount * sizeof(Address));
}

void ReadOnlyHeap::InitializeFromIsolateRoots(Isolate* isolate)
{
    void* const isolate_ro_roots = isolate->roots_table().read_only_roots_begin().location();
    std::memcpy(read_only_roots_, isolate_ro_roots, kEntriesCount * sizeof(Address));
}
...
void ReadOnlyHeap::InitFromIsolate(Isolate* isolate)
{
    ...
    if (IsReadOnlySpaceShared()) {
        ReadOnlyArtifacts* artifacts = isolate->isolate_group()->read_only_artifacts();
...
{
#ifdef V8_ENABLE_SANDBOX
    IsolateGroup::current()->code_pointer_table()->InitializeSpace(&code_pointer_space_);
    ...
    GetProcessWideJSDispatchTable()->AttachSpaceToReadOnlySegment(&js_dispatch_table_space_);
    GetProcessWideJSDispatchTable()->PreAllocateEntries(
        &js_dispatch_table_space_, JSBuiltinDispatchHandleRoot::kCount, Isolate::kBuiltinDispatchHandlesAreStatic);
#endif // V8_ENABLE_LEAPTIERING
}

// static
void ReadOnlyHeap::PopulateReadOnlySpaceStatistics(SharedMemoryStatistics* statistics)
{
...
ReadOnlyHeapObjectIterator::ReadOnlyHeapObjectIterator(const ReadOnlyHeap* ro_heap)
    : ReadOnlyHeapObjectIterator(ro_heap->read_only_space())
{
}

ReadOnlyHeapObjectIterator::ReadOnlyHeapObjectIterator(const ReadOnlySpace* ro_space)
    : ro_space_(ro_space)
    , current_page_(ro_space->pages().begin())
    , page_iterator_(current_page_ == ro_space->pages().end() ? nullptr : *current_page_)
{
}
...
ReadOnlyPageObjectIterator::ReadOnlyPageObjectIterator(const ReadOnlyPageMetadata* page, SkipFreeSpaceOrFiller skip_free_space_or_filler)
    : ReadOnlyPageObjectIterator(page, page == nullptr ? kNullAddress : page->GetAreaStart(), skip_free_space_or_filler)
{
}

ReadOnlyPageObjectIterator::ReadOnlyPageObjectIterator(const ReadOnlyPageMetadata* page, Address current_addr, SkipFreeSpaceOrFiller skip_free_space_or_filler)
    : page_(page)
...
Tagged<HeapObject> ReadOnlyPageObjectIterator::Next()
{
    ...
    for (;;) {
        ...
        if (skip_free_space_or_filler_ == SkipFreeSpaceOrFiller::kYes && IsFreeSpaceOrFiller(object)) {
            continue;
        }
