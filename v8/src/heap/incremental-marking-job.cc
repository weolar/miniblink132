...
IncrementalMarkingJob::IncrementalMarkingJob(Heap* heap)
    : heap_(heap)
    , user_blocking_task_runner_(heap->GetForegroundTaskRunner(TaskPriority::kUserBlocking))
    , user_visible_task_runner_(heap->GetForegroundTaskRunner(TaskPriority::kUserVisible))
{
...
void IncrementalMarkingJob::ScheduleTask(TaskPriority priority)
{
    ...
    v8::TaskRunner* task_runner
        = v8_flags.incremental_marking_start_user_visible && incremental_marking->IsStopped() && (priority != TaskPriority::kUserBlocking)
        ? user_visible_task_runner_.get()
        : user_blocking_task_runner_.get();
    const bool non_nestable_tasks_enabled = task_runner->NonNestableTasksEnabled();
    auto task = std::make_unique<Task>(heap_->isolate(), this, non_nestable_tasks_enabled ? StackState::kNoHeapPointers : StackState::kMayContainHeapPointers);
    ...
    if (V8_UNLIKELY(v8_flags.trace_incremental_marking)) {
        heap_->isolate()->PrintWithTimestamp("[IncrementalMarking] Job: Schedule\n");
    }
}
...
void IncrementalMarkingJob::Task::RunInternal()
{
    ...
    TRACE_EVENT_CALL_STATS_SCOPED(isolate(), "v8", "V8.IncrementalMarkingJob.Task");
...

    {
        ...
        heap->tracer()->RecordTimeToIncrementalMarkingTask(v8::base::TimeTicks::Now() - job_->scheduled_time_);
        job_->scheduled_time_ = v8::base::TimeTicks();
    }

    EmbedderStackStateScope scope(heap, EmbedderStackStateOrigin::kImplicitThroughTask, stack_state_);
    ...
    // new task when starting incremental marking from a task.
    {
        base::MutexGuard guard(&job_->mutex_);
        if (V8_UNLIKELY(v8_flags.trace_incremental_marking)) {
            job_->heap_->isolate()->PrintWithTimestamp("[IncrementalMarking] Job: Run\n");
        }
    ...
    if (incremental_marking->IsMajorMarking()) {
        ...
        if (incremental_marking->IsMajorMarking()) {
            if (V8_UNLIKELY(v8_flags.trace_incremental_marking)) {
                isolate()->PrintWithTimestamp("[IncrementalMarking] Using regular task based on flags\n");
            }
            job_->ScheduleTask();
        }
    }
...
std::optional<base::TimeDelta> IncrementalMarkingJob::CurrentTimeToTask() const
{
...
std::optional<v8::base::TimeDelta> IncrementalMarkingJob::AverageTimeToTask() const
{
