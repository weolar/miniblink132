...
// inlining.
template <typename Derived> class HeapVisitor {
public:
    void Traverse(RawHeap& heap)
    {
        if (VisitHeapImpl(heap))
            return;
        for (auto& space : heap) {
            Traverse(*space.get());
        }
    ...
    void Traverse(BaseSpace& space)
    {
        const bool is_stopped
            = space.is_large() ? VisitLargePageSpaceImpl(LargePageSpace::From(space)) : VisitNormalPageSpaceImpl(NormalPageSpace::From(space));
        if (is_stopped)
            return;
        for (auto* page : space) {
            Traverse(*page);
...
private:
    ...
    {
        return ToDerived().VisitHeap(heap);
    }
