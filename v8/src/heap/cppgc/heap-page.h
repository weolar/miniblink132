...
public:
    ...
    template <AccessMode = AccessMode::kNonAtomic> HeapObjectHeader& ObjectHeaderFromInnerAddress(void* address) const;
    template <AccessMode = AccessMode::kNonAtomic> const HeapObjectHeader& ObjectHeaderFromInnerAddress(const void* address) const;
    ...
    const HeapObjectHeader* TryObjectHeaderFromInnerAddress(const void* address) const;
    ...
    {
        const size_t old_marked_bytes = marked_bytes_.fetch_sub(value, std::memory_order_relaxed);
        USE(old_marked_bytes);
    ...
    {
        return contains_young_objects_;
    }
    ...
    V8_INLINE SlotSet* slot_set() const
    {
...
class V8_EXPORT_PRIVATE NormalPage final : public BasePage {
    template <typename T> class IteratorImpl : v8::base::iterator<std::forward_iterator_tag, T> {
    public:
        explicit IteratorImpl(T* p, ConstAddress lab_start = nullptr, size_t lab_size = 0)
            : p_(p)
            , lab_start_(lab_start)
            , lab_size_(lab_size)
        {
            DCHECK(p);
...
public:
    ...
    {
        return const_iterator(reinterpret_cast<const HeapObjectHeader*>(PayloadEnd()));
    }
    ...
    size_t AllocatedBytesAtLastGC() const
    {
        return allocated_bytes_at_last_gc_;
    }
...
public:
    static constexpr size_t PageHeaderSize()
    {
        ...
        return RoundUp<kGuaranteedObjectAlignment>(sizeof(LargePage) + sizeof(HeapObjectHeader)) - sizeof(HeapObjectHeader);
    }
...
private:
    static constexpr size_t kGuaranteedObjectAlignment = 2 * kAllocationGranularity;
...
template <AccessMode mode = AccessMode::kNonAtomic> const HeapObjectHeader* ObjectHeaderFromInnerAddressImpl(const BasePage* page, const void* address)
{
    if (page->is_large()) {
        return LargePage::From(page)->ObjectHeader();
    }
    const PlatformAwareObjectStartBitmap& bitmap = NormalPage::From(page)->object_start_bitmap();
    const HeapObjectHeader* header = bitmap.FindHeader<mode>(static_cast<ConstAddress>(address));
    DCHECK_LT(address, reinterpret_cast<ConstAddress>(header) + header->AllocatedSize<AccessMode::kAtomic>());
    return header;
}

template <AccessMode mode> HeapObjectHeader& BasePage::ObjectHeaderFromInnerAddress(void* address) const
{
    return const_cast<HeapObjectHeader&>(ObjectHeaderFromInnerAddress<mode>(const_cast<const void*>(address)));
}

template <AccessMode mode> const HeapObjectHeader& BasePage::ObjectHeaderFromInnerAddress(const void* address) const
{
    ...
    SynchronizedLoad();
    const HeapObjectHeader* header = ObjectHeaderFromInnerAddressImpl<mode>(this, address);
...
template <> struct hash<const cppgc::internal::BasePage*> {
...
#endif
        return static_cast<AddressType>(reinterpret_cast<uintptr_t>(base_page)) >> cppgc::internal::api_constants::kPageSizeBits;
    }
};
