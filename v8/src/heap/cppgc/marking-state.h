...
MarkingStateBase::MarkingStateBase(HeapBase& heap, MarkingWorklists& marking_worklists)
    : heap_(heap)
...
void MarkingStateBase::MarkAndPush(const void* object, TraceDescriptor desc)
{
    ...
    MarkAndPush(HeapObjectHeader::FromObject(const_cast<void*>(desc.base_object_payload)), desc);
}

void MarkingStateBase::MarkAndPush(HeapObjectHeader& header, TraceDescriptor desc)
{
...
{
    MarkAndPush(header, { header.ObjectStart(), GlobalGCInfoTable::GCInfoFromIndex(header.GetGCInfoIndex()).trace });
}

void MarkingStateBase::PushMarked(HeapObjectHeader& header, TraceDescriptor desc)
{
...
public:
    ...
    inline void RegisterWeakReferenceIfNeeded(const void*, TraceDescriptor, WeakCallback, const void*);
    ...
    {
        if (V8_LIKELY(!movable_slots_worklist_))
            return;
#if defined(CPPGC_CAGED_HEAP)
        if (V8_UNLIKELY(!CagedHeapBase::IsWithinCage(slot)))
            return;
#else // !defined(CPPGC_CAGED_HEAP)
        if (V8_UNLIKELY(heap::base::Stack::IsOnStack(slot)))
            return;
    ...
    inline void ProcessWeakContainer(const void*, TraceDescriptor, WeakCallback, const void*);

    inline void ProcessEphemeron(const void*, const void*, TraceDescriptor, Visitor&);

    inline void AccountMarkedBytes(const HeapObjectHeader&);
    inline void AccountMarkedBytes(BasePage*, size_t);
    size_t marked_bytes() const
    {
        return marked_bytes_;
    }
...
void BasicMarkingState::RegisterWeakReferenceIfNeeded(const void* object, TraceDescriptor desc, WeakCallback weak_callback, const void* parameter)
{
    // Filter out already marked values. The write barrier for WeakMember
    // ensures that any newly set value after this point is kept alive and does
    // not require the callback.
    const HeapObjectHeader& header = HeapObjectHeader::FromObject(desc.base_object_payload);
    if (!header.IsInConstruction<AccessMode::kAtomic>() && header.IsMarked<AccessMode::kAtomic>())
        return;
    parallel_weak_callback_worklist_.Push({ weak_callback, parameter });
}

void BasicMarkingState::RegisterWeakContainerCallback(WeakCallback callback, const void* object)
{
    DCHECK_NOT_NULL(callback);
    weak_container_callback_worklist_.Push({ callback, object });
}

void BasicMarkingState::RegisterWeakCustomCallback(WeakCallback callback, const void* object)
{
    DCHECK_NOT_NULL(callback);
...
void BasicMarkingState::ProcessWeakContainer(const void* object, TraceDescriptor desc, WeakCallback callback, const void* data)
{
    DCHECK_NOT_NULL(object);

    HeapObjectHeader& header = HeapObjectHeader::FromObject(const_cast<void*>(object));
...
void BasicMarkingState::ProcessEphemeron(const void* key, const void* value, TraceDescriptor value_desc, Visitor& visitor)
{
    ...
    const bool key_in_construction = HeapObjectHeader::FromObject(key).IsInConstruction<AccessMode::kAtomic>();
    const bool key_considered_as_live = key_in_construction ? in_atomic_pause_ : HeapObjectHeader::FromObject(key).IsMarked<AccessMode::kAtomic>();
    DCHECK_IMPLIES(key_in_construction && in_atomic_pause_, HeapObjectHeader::FromObject(key).IsMarked<AccessMode::kAtomic>());
...
{
    const size_t marked_bytes = header.IsLargeObject<AccessMode::kAtomic>() ? reinterpret_cast<const LargePage*>(BasePage::FromPayload(&header))->PayloadSize()
                                                                            : header.AllocatedSize<AccessMode::kAtomic>();
    auto* base_page = BasePage::FromPayload(&const_cast<HeapObjectHeader&>(header));
    AccountMarkedBytes(base_page, marked_bytes);
}

void BasicMarkingState::AccountMarkedBytes(BasePage* base_page, size_t marked_bytes)
{
...
public:
    MutatorMarkingState(HeapBase& heap, MarkingWorklists& marking_worklists, CompactionWorklists* compaction_worklists)
        ...
        , retrace_marked_objects_worklist_(*marking_worklists.retrace_marked_objects_worklist())
    {
    }
    ...
    inline void InvokeWeakRootsCallbackIfNeeded(const void*, TraceDescriptor, WeakCallback, const void*);
...
void MutatorMarkingState::ReTraceMarkedWeakContainer(cppgc::Visitor& visitor, HeapObjectHeader& header)
{
...
{
    HeapObjectHeader& header = BasePage::FromPayload(address)->ObjectHeaderFromInnerAddress(const_cast<Address>(address));
    DCHECK(!header.IsInConstruction());
    if (MarkNoPush(header)) {
        marking_worklist_.Push({ reinterpret_cast<void*>(header.ObjectStart()), GlobalGCInfoTable::GCInfoFromIndex(header.GetGCInfoIndex()).trace });
    }
}

void MutatorMarkingState::InvokeWeakRootsCallbackIfNeeded(const void* object, TraceDescriptor desc, WeakCallback weak_callback, const void* parameter)
{
...
{
    const bool result = weak_containers_worklist_.Contains<AccessMode::kAtomic>(&header) && !recently_retraced_weak_containers_.Contains(&header);
    DCHECK_IMPLIES(result, header.IsMarked<AccessMode::kAtomic>());
    DCHECK_IMPLIES(result, !header.IsInConstruction());
    return result;
}

bool MutatorMarkingState::RecentlyRetracedWeakContainers::Contains(const HeapObjectHeader* header) const
{
    return std::find(recently_retraced_cache_.begin(), recently_retraced_cache_.end(), header) != recently_retraced_cache_.end();
}

void MutatorMarkingState::RecentlyRetracedWeakContainers::Insert(const HeapObjectHeader* header)
{
...
public:
    ...
    inline void AccountDeferredMarkedBytes(BasePage* base_page, size_t deferred_bytes)
    {
...
template <size_t deadline_check_interval, typename WorklistLocal, typename Callback, typename Predicate>
bool DrainWorklistWithPredicate(Predicate should_yield, WorklistLocal& worklist_local, Callback callback)
{
...
template <AccessMode mode> void DynamicallyTraceMarkedObject(Visitor& visitor, const HeapObjectHeader& header)
{
