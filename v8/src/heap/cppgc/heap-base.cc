    ...
    bool VisitPage(BasePage& page)
    {
        if (!page.contains_young_objects()) {
            ...
            DCHECK_EQ(AgeTable::Age::kOld,
                age_table_.GetAgeForRange(CagedHeap::OffsetFromAddress(page.PayloadStart()), CagedHeap::OffsetFromAddress(page.PayloadEnd())));
#endif // defined(DEBUG)
            return true;
        }
        ...
        age_table_.SetAgeForRange(CagedHeap::OffsetFromAddress(page.PayloadStart()), CagedHeap::OffsetFromAddress(page.PayloadEnd()), AgeTable::Age::kOld,
...
HeapBase::HeapBase(std::shared_ptr<cppgc::Platform> platform, const std::vector<std::unique_ptr<CustomSpaceBase>>& custom_spaces, StackSupport stack_support,
    MarkingType marking_support, SweepingType sweeping_support, GarbageCollector& garbage_collector)
    : raw_heap_(this, custom_spaces)
    , platform_(std::move(platform))
    , oom_handler_(std::make_unique<FatalOutOfMemoryHandler>(this))
    ,
#if defined(LEAK_SANITIZER)
    lsan_page_allocator_(std::make_unique<v8::base::LsanPageAllocator>(platform_->GetPageAllocator()))
    ,
    ...
    , object_allocator_(raw_heap_, *page_backend_, *stats_collector_, *prefinalizer_handler_, *oom_handler_, garbage_collector)
    , sweeper_(*this)
...
{
    stats_collector_->RegisterObserver(&allocation_observer_for_PROCESS_HEAP_STATISTICS_);
...
// static
std::unique_ptr<PageBackend> HeapBase::InitializePageBackend(PageAllocator& allocator)
{
#if defined(CPPGC_CAGED_HEAP)
    auto& caged_heap = CagedHeap::Instance();
    return std::make_unique<PageBackend>(caged_heap.page_allocator(), caged_heap.page_allocator());
...
void HeapBase::EnableGenerationalGC()
{
    ...
    if (HeapHandle::is_young_generation_enabled_)
        return;
#if defined(CPPGC_CAGED_HEAP)
...
void HeapBase::ResetRememberedSet()
{
    ...
    protected:
        ...
        {
            some_lab_is_set_ |= static_cast<bool>(space.linear_allocation_buffer().size());
            return true;
        }
...
void HeapBase::Terminate()
{
    ...
    do {
        ...
        stats_collector()->NotifyMarkingStarted(CollectionType::kMajor, GCConfig::MarkingType::kAtomic, GCConfig::IsForcedGC::kForced);
        ...
        sweeper().Start({ SweepingConfig::SweepingType::kAtomic, SweepingConfig::CompactableSpaceHandling::kSweep });
        in_atomic_pause_ = false;
        sweeper().FinishIfRunning();
        more_termination_gcs_needed = strong_persistent_region_.NodesInUse() || weak_persistent_region_.NodesInUse() || [this]() {
            PersistentRegionLock guard;
            return strong_cross_thread_persistent_region_.NodesInUse() || weak_cross_thread_persistent_region_.NodesInUse();
        }();
        gc_count++;
...
HeapStatistics HeapBase::CollectStatistics(HeapStatistics::DetailLevel detail_level)
{
    if (detail_level == HeapStatistics::DetailLevel::kBrief) {
        const size_t pooled_memory = page_backend_->page_pool().PooledMemory();
        const size_t committed_memory = stats_collector_->allocated_memory_size() + pooled_memory;
        const size_t resident_memory = stats_collector_->resident_memory_size() + pooled_memory;

        return { committed_memory, resident_memory, stats_collector_->allocated_object_size(), pooled_memory, HeapStatistics::DetailLevel::kBrief, {}, {} };
    }
...
void HeapBase::CallMoveListeners(Address from, Address to, size_t size_including_header)
{
...
void HeapBase::RegisterMoveListener(MoveListener* listener)
{
    ...
    DCHECK_EQ(std::find(move_listeners_.begin(), move_listeners_.end(), listener), move_listeners_.end());
    move_listeners_.push_back(listener);
}

void HeapBase::UnregisterMoveListener(MoveListener* listener)
{
    auto it = std::remove(move_listeners_.begin(), move_listeners_.end(), listener);
    move_listeners_.erase(it, move_listeners_.end());
}

bool HeapBase::IsGCForbidden() const
{
    return disallow_gc_scope_ > 0;
}
...
{
    heap_.set_name_of_unnamed_object(HeapObjectNameForUnnamedObject::kUseClassNameIfSupported);
}
