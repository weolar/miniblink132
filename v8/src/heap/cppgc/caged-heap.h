...
public:
    ...
    template <typename RetType = uintptr_t> static RetType OffsetFromAddress(const void* address)
    {
        static_assert(std::numeric_limits<RetType>::max() >= (api_constants::kCagedHeapMaxReservationSize - 1), "The return type should be large enough");
    ...
    {
        DCHECK_EQ(reserved_area_.address(), reinterpret_cast<void*>(CagedHeapBase::GetBase()));
        return reinterpret_cast<void*>(BaseFromAddress(address)) == reserved_area_.address();
    }
...
private:
    ...
    explicit CagedHeap(PageAllocator& platform_allocator, size_t desired_heap_size);
