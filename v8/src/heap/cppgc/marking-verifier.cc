...
{
    const HeapObjectHeader& child_header = HeapObjectHeader::FromObject(base_object_payload);
    ...
    if (!child_header.IsMarked()) {
        FATAL("MarkingVerifier: Encountered unmarked object.\n"
              ...
              "#   %s (%p)\n"
              "#     \\-> %s (%p)",
            parent_ ? parent_->GetName(HeapObjectNameForUnnamedObject::kUseClassNameIfSupported).value : "Stack", parent_ ? parent_->ObjectStart() : nullptr,
            child_header.GetName(HeapObjectNameForUnnamedObject::kUseClassNameIfSupported).value, child_header.ObjectStart());
    }
}

MarkingVerifierBase::MarkingVerifierBase(
    HeapBase& heap, CollectionType collection_type, VerificationState& verification_state, std::unique_ptr<cppgc::Visitor> visitor)
...
void MarkingVerifierBase::Run(StackState stack_state, std::optional<size_t> expected_marked_bytes)
{
...
// before V8 compaction or compaction never runs with stack.
    ...
    if (stack_state == StackState::kMayContainHeapPointers) {
        ...
        CHECK_LE(in_construction_objects_stack_.size(), in_construction_objects_heap_.size());
        for (auto* header : in_construction_objects_stack_) {
            CHECK_NE(in_construction_objects_heap_.end(), in_construction_objects_heap_.find(header));
        }
    }
#endif // !defined(THREAD_SANITIZER)
    if (expected_marked_bytes && verifier_found_marked_bytes_are_exact_) {
        // Report differences in marked objects, if possible.
        if (V8_UNLIKELY(expected_marked_bytes.value() != verifier_found_marked_bytes_) && collection_type_ != CollectionType::kMinor) {
        ...
        if (collection_type_ != CollectionType::kMinor) {
            CHECK_EQ(expected_marked_bytes.value(), verifier_found_marked_bytes_in_pages_);
        }
    }
}

void MarkingVerifierBase::VisitInConstructionConservatively(HeapObjectHeader& header, TraceConservativelyCallback callback)
{
    if (in_construction_objects_->find(&header) != in_construction_objects_->end())
        return;
...
bool MarkingVerifierBase::VisitHeapObjectHeader(HeapObjectHeader& header)
{
    ...
    if (collection_type_ == CollectionType::kMinor) {
        ...
        const auto age = CagedHeapLocalData::Get().age_table.GetAge(caged_heap.OffsetFromAddress(header.ObjectStart()));
    ...
    verifier_found_marked_bytes_ += ObjectView<>(header).Size() + sizeof(HeapObjectHeader);
...
void MarkingVerifierBase::ReportDifferences(size_t expected_marked_bytes) const
{
    v8::base::OS::PrintError("\n<--- Mismatch in marking verifier --->\n");
    v8::base::OS::PrintError("Marked bytes: expected %zu vs. verifier found %zu, difference %zd\n", expected_marked_bytes, verifier_found_marked_bytes_,
        expected_marked_bytes - verifier_found_marked_bytes_);
    v8::base::OS::PrintError("A list of pages with possibly mismatched marked objects follows.\n");
    for (auto& space : heap_.raw_heap()) {
        for (auto* page : *space) {
            ...
            if (page->is_large()) {
                ...
                if (header.IsMarked())
                    marked_bytes_on_page += ObjectView<>(header).Size() + sizeof(HeapObjectHeader);
...
void MarkingVerifierBase::ReportNormalPage(const NormalPage& page, size_t marked_bytes_on_page) const
{
    v8::base::OS::PrintError("\nNormal page in space %zu:\n"
                             "Marked bytes: expected %zu vs. verifier found %zu, difference %zd\n",
        page.space().index(), page.marked_bytes(), marked_bytes_on_page, page.marked_bytes() - marked_bytes_on_page);
}

void MarkingVerifierBase::ReportLargePage(const LargePage& page, size_t marked_bytes_on_page) const
{
...
void MarkingVerifierBase::ReportHeapObjectHeader(const HeapObjectHeader& header) const
{
    const char* name = header.IsFree() ? "free space" : header.GetName(HeapObjectNameForUnnamedObject::kUseClassNameIfSupported).value;
    v8::base::OS::PrintError("- %s at %p, size %zu, %s\n", name, header.ObjectStart(), header.ObjectSize(), header.IsMarked() ? "marked" : "unmarked");
}
...
public:
    ...
    void VisitWeak(const void*, TraceDescriptor desc, WeakCallback, const void*) final
    {
    ...
    void VisitWeakContainer(const void* object, TraceDescriptor, TraceDescriptor weak_desc, WeakCallback, const void*) final
    {
        if (!object)
            return;
...
MarkingVerifier::MarkingVerifier(HeapBase& heap_base, CollectionType collection_type)
    : MarkingVerifierBase(heap_base, collection_type, state_, std::make_unique<VerificationVisitor>(state_))
{
}
