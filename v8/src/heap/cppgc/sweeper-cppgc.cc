...
public:
    explicit DeadlineChecker(v8::base::TimeTicks end)
        : end_(end)
    {
    }
    ...
    bool Check()
    {
        return V8_UNLIKELY(++count_ % kInterval == 0) && (end_ < v8::base::TimeTicks::Now());
    }
...
class ObjectStartBitmapVerifier final : private HeapVisitor<ObjectStartBitmapVerifier> {
...
public:
    ...
    virtual void FreeFreeList(std::vector<FreeList::Block>& unfinalized_free_list) = 0;
};

class DiscardingFreeHandler : public FreeHandlerBase {
public:
    DiscardingFreeHandler(PageAllocator& page_allocator, FreeList& free_list, BasePage& page)
    ...
    void Free(FreeList::Block block)
    {
        ...
        const uintptr_t aligned_begin_unused = RoundUp(reinterpret_cast<uintptr_t>(unused_range.first), page_allocator_.CommitPageSize());
        const uintptr_t aligned_end_unused = RoundDown(reinterpret_cast<uintptr_t>(unused_range.second), page_allocator_.CommitPageSize());
        if (aligned_begin_unused < aligned_end_unused) {
            const size_t discarded_size = aligned_end_unused - aligned_begin_unused;
            page_allocator_.DiscardSystemPages(reinterpret_cast<void*>(aligned_begin_unused), aligned_end_unused - aligned_begin_unused);
            page_.IncrementDiscardedMemory(discarded_size);
            page_.space().raw_heap()->heap()->stats_collector()->IncrementDiscardedMemory(discarded_size);
        }
    }
...
public:
    RegularFreeHandler(PageAllocator& page_allocator, FreeList& free_list, BasePage& page)
        : free_list_(free_list)
    {
    }
...
template <typename FreeHandler> class InlinedFinalizationBuilder final : public InlinedFinalizationBuilderBase, public FreeHandler {
public:
    InlinedFinalizationBuilder(BasePage& page, PageAllocator& page_allocator)
        : FreeHandler(page_allocator, NormalPageSpace::From(page.space()).free_list(), page)
    {
    }
    ...
    void AddFreeListEntry(Address start, size_t size)
    {
        ...
        result_.largest_new_free_list_entry = std::max(result_.largest_new_free_list_entry, size);
    }
...
template <typename FreeHandler> class DeferredFinalizationBuilder final : public FreeHandler {
public:
...
typename FinalizationBuilder::ResultType SweepNormalPage(NormalPage* page, PageAllocator& page_allocator, StickyBits sticky_bits)
{
    ...
    const auto clear_bit_if_coalesced_entry = [&bitmap, &start_of_gap](Address address) {
    ...
    for (Address begin = page->PayloadStart(), end = page->PayloadEnd(); begin != end;) {
        ...
        if (start_of_gap != header_address) {
            const size_t new_free_list_entry_size = static_cast<size_t>(header_address - start_of_gap);
    ...
    if (!is_empty && start_of_gap != page->PayloadEnd()) {
        builder.AddFreeListEntry(start_of_gap, static_cast<size_t>(page->PayloadEnd() - start_of_gap));
        DCHECK(bitmap.CheckBit<AccessMode::kAtomic>(start_of_gap));
    }
...
public:
    SweepFinalizer(cppgc::Platform* platform, StatsCollector* stats_collector, BaseSpace* space, size_t* unused_destroyed_normal_pages,
        FreeMemoryHandling free_memory_handling, EmptyPageHandling empty_page_handling_type)
        : platform_(platform)
    ...
    bool FinalizeWithDeadlineAndSize(StatsCollector::ScopeId scope_id, SweepingState& state, v8::base::TimeTicks deadline, size_t size)
    {
    ...
    bool FinalizeWithDeadline(StatsCollector::ScopeId scope_id, SweepingState& state, v8::base::TimeTicks deadline)
    {
        if (state.swept_unfinalized_pages.IsEmpty()) {
            return true;
        }
...
private:
    void FinalizePage(SweepingState::SweptPageState* page_state)
    {
        ...
        for (auto* unfinalized_header = page_state->unfinalized_objects_head; unfinalized_header; unfinalized_header = next_unfinalized) {
            next_unfinalized = unfinalized_header->GetNextUnfinalized(cage_base);
            finalize_header(unfinalized_header);
        }
#else // !defined(CPPGC_CAGED_HEAP)
        for (HeapObjectHeader* unfinalized_header : page_state->unfinalized_objects) {
            finalize_header(unfinalized_header);
        }
#endif // !defined(CPPGC_CAGED_HEAP)
        ...
        if (page_state->is_empty) {
            DCHECK_IMPLIES(page->is_large(), empty_page_handling_ == EmptyPageHandling::kDestroy);
            if (empty_page_handling_ == EmptyPageHandling::kDestroy) {
                if (!page->is_large()) {
                    ...
                    largest_consecutive_block_ = std::max(LargePage::From(page)->PayloadSize(), largest_consecutive_block_);
                }
            ...
            page_state->cached_free_list.Add({ normal_page->PayloadStart(), normal_page->PayloadSize() });
        ...
        if (!page_state->unfinalized_free_list.empty()) {
            std::unique_ptr<FreeHandlerBase> handler = (free_memory_handling_ == FreeMemoryHandling::kDiscardWherePossible)
                ? std::unique_ptr<FreeHandlerBase>(new DiscardingFreeHandler(*platform_->GetPageAllocator(), space_freelist, *page))
                : std::unique_ptr<FreeHandlerBase>(new RegularFreeHandler(*platform_->GetPageAllocator(), space_freelist, *page));
            handler->FreeFreeList(page_state->unfinalized_free_list);
        }

        largest_consecutive_block_ = std::max(page_state->largest_new_free_list_entry, largest_consecutive_block_);
...
public:
    MutatorThreadSweeper(HeapBase* heap, cppgc::Platform* platform, StatsCollector* stats_collector, BaseSpace* space, size_t* unused_destroyed_normal_pages,
        FreeMemoryHandling free_memory_handling, EmptyPageHandling empty_page_handling)
        : platform_(platform)
        , stats_collector_(stats_collector)
        , space_(space)
        , unused_destroyed_normal_pages_(unused_destroyed_normal_pages)
    ...
    bool FinalizeAndSweepWithDeadline(
        StatsCollector::ScopeId scope_id, SweepingState& state, v8::base::TimeTicks deadline, MutatorThreadSweepingMode sweeping_mode)
    {
        // First, prioritize finalization of pages that were swept concurrently.
        SweepFinalizer finalizer(platform_, stats_collector_, space_, unused_destroyed_normal_pages_, free_memory_handling_, EmptyPageHandling::kDestroy);
    ...
    bool SweepWithDeadlineAndSize(StatsCollector::ScopeId scope_id, SweepingState& state, v8::base::TimeTicks deadline, size_t size)
    {
...
private:
    bool SweepSpaceWithDeadline(SweepingState* state, v8::base::TimeTicks deadline)
    {
        DeadlineChecker deadline_check(deadline);
        while (auto page = state->unswept_pages.Pop()) {
            Traverse(**page);
    ...
    bool VisitNormalPage(NormalPage& page)
    {
        ...
        const auto result = (free_memory_handling_ == FreeMemoryHandling::kDiscardWherePossible)
            ? SweepNormalPage<InlinedFinalizationBuilder<DiscardingFreeHandler>>(&page, *platform_->GetPageAllocator(), sticky_bits_)
            : SweepNormalPage<InlinedFinalizationBuilder<RegularFreeHandler>>(&page, *platform_->GetPageAllocator(), sticky_bits_);
        if (result.is_empty && empty_page_handling_ == EmptyPageHandling::kDestroy) {
            ...
            largest_consecutive_block_ = std::max(result.is_empty ? page.PayloadSize() : result.largest_new_free_list_entry, largest_consecutive_block_);
        }
        return true;
    }
    ...
    bool VisitLargePage(LargePage& page)
    {
        ...
        header->Finalize();
        largest_consecutive_block_ = std::max(page.PayloadSize(), largest_consecutive_block_);
...
class ConcurrentSweepTask final : public cppgc::JobTask, private HeapVisitor<ConcurrentSweepTask> {
    friend class HeapVisitor<ConcurrentSweepTask>;

    using FreeMemoryHandling = SweepingConfig::FreeMemoryHandling;

public:
    ConcurrentSweepTask(Platform* platform, HeapBase& heap, SpaceStates* space_states, SweepingState* empty_normal_pages, SweepingState* empty_large_pages,
    ...
    {
        StatsCollector::EnabledConcurrentScope stats_scope(heap_.stats_collector(), StatsCollector::kConcurrentSweep);

        // Sweep empty normal pages first. These pages can be reused across all
        // regular spaces.
        if (!SweepStateOrYield(delegate, *empty_normal_pages_))
            return;
        for (SweepingState& state : *space_states_) {
            if (!SweepStateOrYield(delegate, state))
                return;
        }
        ...
        // normal pages.
        if (!SweepStateOrYield(delegate, *empty_large_pages_))
            return;
...
private:
    ...
    bool VisitNormalPage(NormalPage& page)
    {
        ...
        SweepingState::SweptPageState sweep_result = (free_memory_handling_ == FreeMemoryHandling::kDiscardWherePossible)
            ? SweepNormalPage<DeferredFinalizationBuilder<DiscardingFreeHandler>>(&page, page_allocator_, sticky_bits_)
            : SweepNormalPage<DeferredFinalizationBuilder<RegularFreeHandler>>(&page, page_allocator_, sticky_bits_);
        current_sweeping_state_->swept_unfinalized_pages.Push(std::move(sweep_result));
        return true;
    }
    ...
    bool VisitLargePage(LargePage& page)
    {
        ...
        DCHECK_EQ(page.marked_bytes(), 0u);
#if defined(CPPGC_CAGED_HEAP)
        HeapObjectHeader* const unfinalized_objects = header->IsFinalizable() ? page.ObjectHeader() : nullptr;
#else // !defined(CPPGC_CAGED_HEAP)
        std::vector<HeapObjectHeader*> unfinalized_objects;
        if (header->IsFinalizable()) {
        ...
        current_sweeping_state_->swept_unfinalized_pages.Push({ &page, std::move(unfinalized_objects), {}, {}, true });
        return true;
    }
...
// - ASAN: Poisons all unmarked object payloads.
class PrepareForSweepVisitor final : protected HeapVisitor<PrepareForSweepVisitor> {
    friend class HeapVisitor<PrepareForSweepVisitor>;
    using CompactableSpaceHandling = SweepingConfig::CompactableSpaceHandling;

public:
    PrepareForSweepVisitor(HeapBase* heap, SpaceStates* space_states, SweepingState* empty_normal_pages, SweepingState* empty_large_pages,
...
protected:
    ...
    {
        if ((compactable_space_handling_ == CompactableSpaceHandling::kIgnore) && space.is_compactable()) {
            return true;
        }
        ...
        std::sort(space_pages.begin(), space_pages.end(), [](const BasePage* a, const BasePage* b) { return a->marked_bytes() < b->marked_bytes(); });
        auto first_non_empty_page = std::find_if(space_pages.begin(), space_pages.end(), [](const BasePage* page) { return page->marked_bytes() != 0; });
        empty_normal_pages_->unswept_pages.Insert(space_pages.begin(), first_non_empty_page);
        (*space_states_)[space.index()].unswept_pages.Insert(first_non_empty_page, space_pages.end());
    ...
    bool VisitLargePageSpace(LargePageSpace& space)
    {
        ...
        for (BasePage* page : space_pages) {
            ...
            if (page->marked_bytes() != 0) {
                MutatorThreadSweeper::SweepLiveLargePage(*LargePage::From(page), heap_->sticky_bits());
            } else {
...
public:
    ...
    {
        StatsCollector::EnabledScope stats_scope(stats_collector_, StatsCollector::kAtomicSweep);
        ...
        if (!foreground_task_runner_) {
            ...
            // sweep.
            foreground_task_runner_ = platform_->GetForegroundTaskRunner(kForegroundRegularPriority);
            low_priority_foreground_task_runner_ = platform_->GetForegroundTaskRunner(kForegroundLowPriority);
            // Having a low priority runner implies having a regular runner as well.
            CHECK_IMPLIES(low_priority_foreground_task_runner_.get(), foreground_task_runner_.get());
        }
        ...
        if (config_.free_memory_handling == FreeMemoryHandling::kDiscardWherePossible) {
            // The discarded counter will be recomputed.
            heap_.heap()->stats_collector()->ResetDiscardedMemory();
        }

        PrepareForSweepVisitor(heap_.heap(), &space_states_, &empty_normal_pages_, &empty_large_pages_, config.compactable_space_handling).Run(heap_);
        ...
        if (config.sweeping_type >= SweepingConfig::SweepingType::kIncrementalAndConcurrent) {
    ...
    void SweepForTask(v8::base::TimeDelta max_duration)
    {
        ...
        switch (SweepInForegroundTaskImpl(max_duration, StatsCollector::kSweepInTask)) {
    ...
    void SweepForLowPriorityTask(v8::base::TimeDelta max_duration)
    {
        ...
        switch (SweepInForegroundTaskImpl(max_duration, StatsCollector::kSweepInLowPriorityTask)) {
        case SweepResult::kFullyDone:
            return;
        case SweepResult::kInProgress:
            ...
            return;
        case SweepResult::kMainThreadDoneConcurrentInProgress:
            ScheduleLowPriorityIncrementalSweeping(kDelayWhileLowPrioritySweepingMakesProgress);
            return;
        }
        UNREACHABLE();
    }
    ...
    bool SweepForLargeAllocation(BaseSpace* space, size_t size, v8::base::TimeDelta max_duration)
    {
        ...
        if (empty_large_pages_.swept_unfinalized_pages.IsEmpty() && empty_large_pages_.unswept_pages.IsEmpty()) {
            return false;
        }

        StatsCollector::EnabledScope incremental_sweep_scope(stats_collector_, StatsCollector::kIncrementalSweep);
        StatsCollector::DisabledScope sweep_on_allocation_scope(stats_collector_, StatsCollector::kSweepOnAllocation);
        ...
        SweepFinalizer finalizer(
            platform_, stats_collector_, space, &unused_destroyed_normal_pages_, config_.free_memory_handling, EmptyPageHandling::kDestroy);
        // Check empty pages first. Try to just finalize a page without sweeping.
        // If there's a single page in there we will use it.
        if (finalizer.FinalizeWithDeadlineAndSize(StatsCollector::kSweepFinalizeEmptyPages, empty_large_pages_, deadline, size)) {
            return true;
        }
        MutatorThreadSweeper sweeper(
            heap_.heap(), platform_, stats_collector_, space, &unused_destroyed_normal_pages_, config_.free_memory_handling, EmptyPageHandling::kDestroy);
        // Sweeping an empty page in case there's nothing with finalizers. If
        // there's a single page in there we will use it.
        if (sweeper.SweepWithDeadlineAndSize(StatsCollector::kSweepEmptyPages, empty_large_pages_, deadline, size)) {
            return true;
        }
    ...
    bool SweepForNormalAllocation(BaseSpace* space, size_t size, v8::base::TimeDelta max_duration)
    {
        ...
        if (empty_normal_pages_.swept_unfinalized_pages.IsEmpty() && empty_normal_pages_.unswept_pages.IsEmpty()
            && space_state.swept_unfinalized_pages.IsEmpty() && space_state.unswept_pages.IsEmpty()) {
            return false;
        }
        ...
        SweepFinalizer finalizer(platform_, stats_collector_, space, &unused_destroyed_normal_pages_, config_.free_memory_handling, EmptyPageHandling::kReturn);
        // Check empty pages first. Try to just finalize a page without sweeping.
        // If there's a single page in there we will use it.
        if (finalizer.FinalizeWithDeadlineAndSize(StatsCollector::kSweepFinalizeEmptyPages, empty_normal_pages_, deadline, size)) {
            return true;
        }
        MutatorThreadSweeper sweeper(
            heap_.heap(), platform_, stats_collector_, space, &unused_destroyed_normal_pages_, config_.free_memory_handling, EmptyPageHandling::kReturn);
        // Sweeping an empty page in case there's nothing with finalizers. If
        // there's a single page in there we will use it.
        if (sweeper.SweepWithDeadlineAndSize(StatsCollector::kSweepEmptyPages, empty_normal_pages_, deadline, size)) {
            return true;
        }

        // Process unfinalized non-empty pages as finalizing a page is generally
        // faster than sweeping.
        if (finalizer.FinalizeWithDeadlineAndSize(StatsCollector::kSweepFinalizeSweptPages, space_state, deadline, size)) {
            return true;
        }
        ...
        if (sweeper.SweepWithDeadlineAndSize(StatsCollector::kSweepPages, space_state, deadline, size)) {
            return true;
        }
        return false;
    }
    ...
    bool SweepForAllocationIfRunning(BaseSpace* space, size_t size, v8::base::TimeDelta max_duration)
    {
        ...
        return space->is_large() ? SweepForLargeAllocation(space, size, max_duration) : SweepForNormalAllocation(space, size, max_duration);
    }
    ...
    bool FinishIfRunning()
    {
...

        {
            ...
            if (config_.sweeping_type != SweepingConfig::SweepingType::kAtomic) {
                stats_scope.emplace(stats_collector_, StatsCollector::kIncrementalSweep);
            }
            StatsCollector::EnabledScope inner_scope(stats_collector_, StatsCollector::kSweepFinish);
            if (concurrent_sweeper_handle_ && concurrent_sweeper_handle_->IsValid() && concurrent_sweeper_handle_->UpdatePriorityEnabled()) {
    ...
    {
        return !concurrent_sweeper_handle_ || !concurrent_sweeper_handle_->IsValid() || !concurrent_sweeper_handle_->IsActive();
    }
    ...
    void FinishIfOutOfWork()
    {
        ...
        // steps.
        if (!concurrent_sweeper_handle_ || !concurrent_sweeper_handle_->IsValid() || concurrent_sweeper_handle_->IsActive()) {
            return;
        }
        ...
        // swept pages.
        DCHECK(std::all_of(space_states_.begin(), space_states_.end(), [](const SweepingState& state) { return state.unswept_pages.IsEmpty(); }));
        DCHECK(empty_normal_pages_.unswept_pages.IsEmpty());
        DCHECK(empty_large_pages_.unswept_pages.IsEmpty());
        if (std::any_of(space_states_.begin(), space_states_.end(), [](const SweepingState& state) { return !state.swept_unfinalized_pages.IsEmpty(); })) {
            return;
        }
        if (!empty_normal_pages_.swept_unfinalized_pages.IsEmpty() || !empty_large_pages_.swept_unfinalized_pages.IsEmpty()) {
            return;
        }
        ...
        // done.
        {
            StatsCollector::EnabledScope stats_scope(stats_collector_, StatsCollector::kSweepFinishIfOutOfWork);
            FinalizeSweep();
        }
        NotifyDone();
    }
    ...
    void Finish()
    {
        ...
        SweepFinalizer finalizer(platform_, stats_collector_, kSweepWithoutSpaceAssignment, &unused_destroyed_normal_pages_, config_.free_memory_handling,
        ...
        MutatorThreadSweeper sweeper(heap_.heap(), platform_, stats_collector_, kSweepWithoutSpaceAssignment, &unused_destroyed_normal_pages_,
    ...
    void WaitForConcurrentSweepingForTesting()
    {
        if (concurrent_sweeper_handle_)
            concurrent_sweeper_handle_->Join();
    }
    ...
    bool PerformSweepOnMutatorThread(v8::base::TimeDelta max_duration, StatsCollector::ScopeId internal_scope_id, MutatorThreadSweepingMode sweeping_mode)
    {
        ...
        {
            StatsCollector::EnabledScope stats_scope(stats_collector_, StatsCollector::kIncrementalSweep);

            MutatorThreadSweeper sweeper(heap_.heap(), platform_, stats_collector_, kSweepWithoutSpaceAssignment, &unused_destroyed_normal_pages_,
                config_.free_memory_handling, EmptyPageHandling::kDestroy);
            {
                StatsCollector::EnabledScope inner_stats_scope(
                    stats_collector_, internal_scope_id, "max_duration_ms", max_duration.InMillisecondsF(), "sweeping_mode", ToString(sweeping_mode));
                const auto deadline = v8::base::TimeTicks::Now() + max_duration;
                if (!sweeper.FinalizeAndSweepWithDeadline(StatsCollector::kSweepFinalizeEmptyPages, empty_normal_pages_, deadline, sweeping_mode)) {
                    return false;
                }
                for (auto& state : space_states_) {
                    if (!sweeper.FinalizeAndSweepWithDeadline(StatsCollector::kSweepFinalizeSweptPages, state, deadline, sweeping_mode)) {
                        return false;
                    }
                }
                if (!sweeper.FinalizeAndSweepWithDeadline(StatsCollector::kSweepFinalizeEmptyPages, empty_large_pages_, deadline, sweeping_mode)) {
                    return false;
                }
    ...
    void AddMutatorThreadSweepingObserver(Sweeper::SweepingOnMutatorThreadObserver* observer)
    {
        DCHECK_EQ(mutator_thread_sweeping_observers_.end(),
            std::find(mutator_thread_sweeping_observers_.begin(), mutator_thread_sweeping_observers_.end(), observer));
    ...
    {
        const auto it = std::find(mutator_thread_sweeping_observers_.begin(), mutator_thread_sweeping_observers_.end(), observer);
...
private:
    ...
    public:
        ...
        static constexpr auto kMaxSweepDuration = v8::base::TimeDelta::FromMilliseconds(5);
        ...
        static Handle Post(
            SweeperImpl& sweeper, const std::shared_ptr<cppgc::TaskRunner>& runner, cppgc::TaskPriority priority, std::optional<v8::base::TimeDelta> delay = {})
        {
    ...
    static constexpr auto kDelayWhileLowPrioritySweepingMakesProgress = v8::base::TimeDelta::FromMilliseconds(100);

    static constexpr auto kDelayWhileConcurrentSweepingMakesProgress = v8::base::TimeDelta::FromMilliseconds(5);

    // We use a small delay here to allow lower priority tasks to interrupt
    // sweeping and take over.
    static constexpr auto kDelayForRegularPrioritySweeping = v8::base::TimeDelta::FromMilliseconds(1);
    ...
    SweepResult SweepInForegroundTaskImpl(v8::base::TimeDelta max_duration, StatsCollector::ScopeId scope)
    {
        ...
        bool main_thread_sweep_complete = PerformSweepOnMutatorThread(
            max_duration, scope, concurrent_sweep_complete ? MutatorThreadSweepingMode::kAll : MutatorThreadSweepingMode::kOnlyFinalizers);
        if (main_thread_sweep_complete && !concurrent_sweep_complete && IsConcurrentSweepingDone()) {
            ...
            if (max_duration > v8::base::TimeDelta::FromMilliseconds(0)) {
                ...
                main_thread_sweep_complete = PerformSweepOnMutatorThread(max_duration, scope, MutatorThreadSweepingMode::kAll);
            }
        }
    ...
    {
        DCHECK_GE(config_.sweeping_type, SweepingConfig::SweepingType::kIncremental);
        ...
        incremental_sweeper_handle_ = IncrementalSweepTask::Post(*this, foreground_task_runner_, kForegroundRegularPriority, delay);
    }
    ...
    void ScheduleLowPriorityIncrementalSweeping(std::optional<v8::base::TimeDelta> delay = {})
    {
        ...
        incremental_sweeper_low_priority_handle_ = IncrementalSweepTask::Post(*this, low_priority_foreground_task_runner_, kForegroundLowPriority, delay);
    }

    void ScheduleConcurrentSweeping()
    {
        DCHECK_GE(config_.sweeping_type, SweepingConfig::SweepingType::kIncrementalAndConcurrent);

        concurrent_sweeper_handle_ = platform_->PostJob(kBackgroundRegularPriority,
            std::make_unique<ConcurrentSweepTask>(
                platform_, *heap_.heap(), &space_states_, &empty_normal_pages_, &empty_large_pages_, config_.free_memory_handling));
    }
...
Sweeper::Sweeper(HeapBase& heap)
    : heap_(heap)
    , impl_(std::make_unique<SweeperImpl>(heap))
{
}
...
{
    return impl_->FinishIfRunning();
}

void Sweeper::FinishIfOutOfWork()
{
    impl_->FinishIfOutOfWork();
}
...
bool Sweeper::SweepForAllocationIfRunning(BaseSpace* space, size_t size, v8::base::TimeDelta max_duration)
{
...
bool Sweeper::PerformSweepOnMutatorThread(v8::base::TimeDelta max_duration, StatsCollector::ScopeId scope_id)
{
    return impl_->PerformSweepOnMutatorThread(max_duration, scope_id, MutatorThreadSweepingMode::kAll);
}

Sweeper::SweepingOnMutatorThreadObserver::SweepingOnMutatorThreadObserver(Sweeper& sweeper)
    : sweeper_(sweeper)
{
