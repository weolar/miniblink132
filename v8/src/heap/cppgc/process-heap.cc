...
{
    static v8::base::LazyInstance<HeapRegistry::Storage>::type heap_registry = LAZY_INSTANCE_INITIALIZER;
...
HeapBase* HeapRegistry::TryFromManagedPointer(const void* needle)
{
    ...
    for (auto* heap : GetHeapRegistryStorage()) {
        const auto address = heap->page_backend()->Lookup(reinterpret_cast<ConstAddress>(needle));
        if (address)
            return heap;
    }
    return nullptr;
