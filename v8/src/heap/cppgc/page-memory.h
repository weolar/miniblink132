...
public:
    ...
    {
        return (reinterpret_cast<uintptr_t>(addr) - reinterpret_cast<uintptr_t>(base_)) < size_;
    }
...
public:
    ...
    const MemoryRegion writeable_region() const
    {
        return writable_;
    }
    const MemoryRegion overall_region() const
    {
        return overall_;
    }
...
public:
    ...
    const MemoryRegion reserved_region() const
    {
    ...
    {
        return PageMemory(MemoryRegion(reserved_region().base(), reserved_region().size()),
            MemoryRegion(reserved_region().base() + kGuardPageSize, reserved_region().size() - 2 * kGuardPageSize));
    }
...
{
    return kGuardPageSize != 0 && kGuardPageSize % allocator.CommitPageSize() == 0;
}
