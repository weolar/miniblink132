...
template <MarkerBase::WriteBarrierType type> void ProcessMarkValue(HeapObjectHeader& header, MarkerBase* marker, const void* value)
{
...
{
    if (!value || value == kSentinelPointer)
        return;
...
void WriteBarrier::DijkstraMarkingBarrierSlow(const void* value)
{
    ...
    auto& header = const_cast<HeapObjectHeader&>(page->ObjectHeaderFromInnerAddress(value));
    if (!header.TryMarkAtomic())
        return;

    ProcessMarkValue<MarkerBase::WriteBarrierType::kDijkstra>(header, heap.marker(), value);
}
...
void WriteBarrier::SteeleMarkingBarrierSlow(const void* value)
{
    ...
    ProcessMarkValue<MarkerBase::WriteBarrierType::kSteele>(header, heap.marker(), value);
}

#if defined(CPPGC_YOUNG_GENERATION)
// static
void WriteBarrier::GenerationalBarrierSlow(
...
// static
void WriteBarrier::GenerationalBarrierForSourceObjectSlow(const CagedHeapLocalData& local_data, const void* inner_pointer, HeapHandle* heap_handle)
{
    ...
    auto& object_header = BasePage::FromInnerAddress(&heap, inner_pointer)->ObjectHeaderFromInnerAddress<AccessMode::kAtomic>(inner_pointer);

    // Record the source object.
    heap.remembered_set().AddSourceObject(const_cast<HeapObjectHeader&>(object_header));
}
...
// static
template <WriteBarrierSlotType SlotType> void WriteBarrier::CombinedWriteBarrierSlow(const void* slot)
{
    ...
    if constexpr (SlotType == WriteBarrierSlotType::kCompressed) {
        value = CompressedPointer::Decompress(*static_cast<const CompressedPointer::IntegralType*>(slot));
    } else {
...
template V8_EXPORT_PRIVATE void WriteBarrier::CombinedWriteBarrierSlow<WriteBarrierSlotType::kUncompressed>(const void* slot);
#if defined(CPPGC_POINTER_COMPRESSION)
template V8_EXPORT_PRIVATE void WriteBarrier::CombinedWriteBarrierSlow<WriteBarrierSlotType::kCompressed>(const void* slot);
