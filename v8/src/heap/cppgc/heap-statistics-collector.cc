...
std::string GetNormalPageSpaceName(size_t index)
{
    ...
    return "CustomSpace" + std::to_string(index - RawHeap::kNumberOfRegularSpaces);
}

HeapStatistics::SpaceStatistics* InitializeSpace(HeapStatistics* stats, std::string name)
{
...
HeapStatistics::PageStatistics* InitializePage(HeapStatistics::SpaceStatistics* stats)
{
...
void FinalizePage(HeapStatistics::SpaceStatistics* space_stats, HeapStatistics::PageStatistics** page_stats)
{
    if (*page_stats) {
...
void FinalizeSpace(HeapStatistics* stats, HeapStatistics::SpaceStatistics** space_stats, HeapStatistics::PageStatistics** page_stats)
{
...
void RecordObjectType(std::unordered_map<const void*, size_t>& type_map, std::vector<HeapStatistics::ObjectStatsEntry>& object_statistics,
...
HeapStatistics HeapStatisticsCollector::CollectDetailedStatistics(HeapBase* heap)
{
    ...
    DCHECK_GE(heap->stats_collector()->allocated_memory_size(), stats.resident_size_bytes);
...
bool HeapStatisticsCollector::VisitNormalPageSpace(NormalPageSpace& space)
{
    ...
    current_space_stats_ = InitializeSpace(current_stats_, GetNormalPageSpaceName(space.index()));
...
bool HeapStatisticsCollector::VisitNormalPage(NormalPage& page)
{
    ...
    current_page_stats_->resident_size_bytes = kPageSize - page.discarded_memory();
    return false;
}
...
bool HeapStatisticsCollector::VisitHeapObjectHeader(HeapObjectHeader& header)
{
    ...
    const size_t allocated_object_size
        = header.IsLargeObject() ? LargePage::From(BasePage::FromPayload(const_cast<HeapObjectHeader*>(&header)))->PayloadSize() : header.AllocatedSize();
    RecordObjectType(type_name_to_index_map_, current_page_stats_->object_statistics, &header, allocated_object_size);
