...
{
    return v8::base::bits::WhichPowerOfTwo(v8::base::bits::RoundDownToPowerOfTwo32(size));
}
} // namespace
...
private:
    explicit Entry(size_t size)
        : HeapObjectHeader(size, kFreeListGCInfoIndex)
    {
...
FreeList::FreeList(FreeList&& other) V8_NOEXCEPT : free_list_heads_(std::move(other.free_list_heads_)),
...
std::pair<Address, Address> FreeList::AddReturningUnusedBounds(Block block)
{
    ...
    if (size < sizeof(Entry)) {
        ...
        USE(filler);
        DCHECK_EQ(reinterpret_cast<Address>(block.address) + size, filler.ObjectEnd());
        DCHECK_EQ(reinterpret_cast<Address>(&filler + 1), filler.ObjectEnd());
        return { reinterpret_cast<Address>(&filler + 1), reinterpret_cast<Address>(&filler + 1) };
    }
    ...
    return { reinterpret_cast<Address>(&entry + 1), reinterpret_cast<Address>(&entry) + size };
}
...
FreeList::Block FreeList::Allocate(size_t allocation_size)
{
    ...
    for (; index > 0; --index, bucket_size >>= 1) {
        ...
        if (allocation_size > bucket_size) {
            ...
            if (!entry || entry->AllocatedSize() < allocation_size)
                break;
        }
        if (entry) {
            if (!entry->Next()) {
...
{
    return std::all_of(free_list_heads_.cbegin(), free_list_heads_.cend(), [](const auto* entry) { return !entry; });
}
...
bool FreeList::ContainsForTesting(Block block) const
{
    for (Entry* list : free_list_heads_) {
        for (Entry* entry = list; entry; entry = entry->Next()) {
            if (entry <= block.address && (reinterpret_cast<Address>(block.address) + block.size <= reinterpret_cast<Address>(entry) + entry->AllocatedSize()))
                return true;
        }
    }
    return false;
...
bool FreeList::IsConsistent(size_t index) const
{
    ...
    return (!free_list_heads_[index] && !free_list_tails_[index]) || (free_list_heads_[index] && free_list_tails_[index] && !free_list_tails_[index]->Next());
}

void FreeList::CollectStatistics(HeapStatistics::FreeListStatistics& free_list_stats)
{
