...
bool InGC(HeapHandle& heap_handle)
{
    ...
    return heap.in_atomic_pause() || heap.marker() || heap.sweeper().IsSweepingInProgress();
}

} // namespace

void ExplicitManagementImpl::FreeUnreferencedObject(HeapHandle& heap_handle, void* object)
{
    if (InGC(heap_handle)) {
        return;
    }
    ...
    if (auto& heap_base = HeapBase::From(heap_handle); heap_base.generational_gc_supported()) {
        heap_base.remembered_set().InvalidateRememberedSlotsInRange(object, reinterpret_cast<uint8_t*>(object) + object_size);
        ...
        if (header.IsMarked()) {
            base_page->DecrementMarkedBytes(header.IsLargeObject<AccessMode::kNonAtomic>()
                    ? reinterpret_cast<const LargePage*>(BasePage::FromPayload(&header))->PayloadSize()
    ...
    if (base_page->is_large()) { // Large object.
        ...
        base_page->heap().stats_collector()->NotifyExplicitFree(LargePage::From(base_page)->PayloadSize());
...
bool Grow(HeapObjectHeader& header, BasePage& base_page, size_t new_size, size_t size_delta)
{
...
#if defined(CPPGC_YOUNG_GENERATION)
        if (auto& heap_base = *normal_space.raw_heap()->heap(); heap_base.generational_gc_supported()) {
            if (header.IsMarked()) {
                base_page.IncrementMarkedBytes(header.AllocatedSize<AccessMode::kNonAtomic>());
            }
        }
#endif // defined(CPPGC_YOUNG_GENERATION)
        return true;
...
bool Shrink(HeapObjectHeader& header, BasePage& base_page, size_t new_size, size_t size_delta)
{
    ...
    if (heap.generational_gc_supported()) {
        heap.remembered_set().InvalidateRememberedSlotsInRange(free_start, free_start + size_delta);
        if (header.IsMarked()) {
            base_page.DecrementMarkedBytes(header.AllocatedSize<AccessMode::kNonAtomic>());
        }
    }
#endif // defined(CPPGC_YOUNG_GENERATION)
...
bool ExplicitManagementImpl::Resize(void* object, size_t new_object_size)
{
    ...
    const size_t new_size = RoundUp<kAllocationGranularity>(sizeof(HeapObjectHeader) + new_object_size);
    auto& header = HeapObjectHeader::FromObject(object);
