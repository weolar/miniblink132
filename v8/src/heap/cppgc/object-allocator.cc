...
void MarkRangeAsYoung(BasePage& page, Address begin, Address end)
{
    ...
    // marked as young.
    const bool new_page = (begin == page.PayloadStart()) && (end == page.PayloadEnd());

    auto& age_table = CagedHeapLocalData::Get().age_table;
    age_table.SetAgeForRange(CagedHeap::OffsetFromAddress(begin), CagedHeap::OffsetFromAddress(end), AgeTable::Age::kYoung,
        new_page ? AgeTable::AdjacentCardsPolicy::kIgnore : AgeTable::AdjacentCardsPolicy::kConsider);
...
void AddToFreeList(NormalPageSpace& space, Address start, size_t size)
{
    ...
    NormalPage::From(BasePage::FromPayload(start))->object_start_bitmap().SetBit<AccessMode::kAtomic>(start);
}

void ReplaceLinearAllocationBuffer(NormalPageSpace& space, StatsCollector& stats_collector, Address new_buffer, size_t new_size)
{
...
LargePage* TryAllocateLargeObjectImpl(PageBackend& page_backend, LargePageSpace& space, size_t size)
{
    ...
    // allocation.
    if (sweeper.SweepForAllocationIfRunning(&space, size, v8::base::TimeDelta::FromMicroseconds(500))
    ...
    // cheaper.
    if (sweeper.SweepForAllocationIfRunning(&space, size, v8::base::TimeDelta::Max()) && (page = LargePage::TryCreate(page_backend, space, size))) {
        return page;
    }

    if (sweeper.FinishIfRunning() && (page = LargePage::TryCreate(page_backend, space, size))) {
        return page;
    }
...
void* TryAllocateLargeObject(PageBackend& page_backend, LargePageSpace& space, StatsCollector& stats_collector, size_t size, GCInfoIndex gcinfo)
{
    ...
    auto* header = new (page->ObjectHeader()) HeapObjectHeader(HeapObjectHeader::kLargeObjectSizeInHeader, gcinfo);
...
ObjectAllocator::ObjectAllocator(RawHeap& heap, PageBackend& page_backend, StatsCollector& stats_collector, PreFinalizerHandler& prefinalizer_handler,
    FatalOutOfMemoryHandler& oom_handler, GarbageCollector& garbage_collector)
    : raw_heap_(heap)
...
void ObjectAllocator::OutOfLineAllocateGCSafePoint(NormalPageSpace& space, size_t size, AlignVal alignment, GCInfoIndex gcinfo, void** object)
{
...
namespace {
constexpr GCConfig kOnAllocationFailureGCConfig = { CollectionType::kMajor, StackState::kMayContainHeapPointers, GCConfig::MarkingType::kAtomic,
    GCConfig::SweepingType::kIncrementalAndConcurrent, GCConfig::FreeMemoryHandling::kDiscardWherePossible };
} // namespace

void* ObjectAllocator::OutOfLineAllocateImpl(NormalPageSpace& space, size_t size, AlignVal alignment, GCInfoIndex gcinfo)
{
    ...
    if (size >= kLargeObjectSizeThreshold) {
        auto& large_space = LargePageSpace::From(*raw_heap_.Space(RawHeap::RegularSpaceType::kLarge));
        // LargePage has a natural alignment that already satisfies
        // `kMaxSupportedAlignment`.
        void* result = TryAllocateLargeObject(page_backend_, large_space, stats_collector_, size, gcinfo);
        if (!result) {
            auto config = kOnAllocationFailureGCConfig;
            garbage_collector_.CollectGarbage(config);
            result = TryAllocateLargeObject(page_backend_, large_space, stats_collector_, size, gcinfo);
            if (!result) {
#if defined(CPPGC_CAGED_HEAP)
                const auto last_alloc_status = CagedHeap::Instance().page_allocator().get_last_allocation_status();
                const std::string suffix = v8::base::BoundedPageAllocator::AllocationStatusToString(last_alloc_status);
    ...
    void* result
        = (dynamic_alignment == kAllocationGranularity) ? AllocateObjectOnSpace(space, size, gcinfo) : AllocateObjectOnSpace(space, size, alignment, gcinfo);
    CHECK(result);
    return result;
}

bool ObjectAllocator::TryExpandAndRefillLinearAllocationBuffer(NormalPageSpace& space)
{
    ...
    ReplaceLinearAllocationBuffer(space, stats_collector_, new_page->PayloadStart(), new_page->PayloadSize());
    return true;
}

bool ObjectAllocator::TryRefillLinearAllocationBuffer(NormalPageSpace& space, size_t size)
{
    // Try to allocate from the freelist.
    if (TryRefillLinearAllocationBufferFromFreeList(space, size))
        return true;
    ...
    if (TryExpandAndRefillLinearAllocationBuffer(space))
        return true;

    // Expansion failed. Before finishing all sweeping, finish sweeping of a given
    // space which is cheaper.
    if (sweeper.SweepForAllocationIfRunning(&space, size, v8::base::TimeDelta::Max()) && TryRefillLinearAllocationBufferFromFreeList(space, size)) {
        return true;
    }
...
bool ObjectAllocator::TryRefillLinearAllocationBufferFromFreeList(NormalPageSpace& space, size_t size)
{
    ...
    ReplaceLinearAllocationBuffer(space, stats_collector_, static_cast<Address>(entry.address), entry.size);
    return true;
}
...
void ObjectAllocator::ResetLinearAllocationBuffers()
{
    ...
    public:
        explicit Resetter(StatsCollector& stats)
