...
BasicMarkingState::BasicMarkingState(HeapBase& heap, MarkingWorklists& marking_worklists, CompactionWorklists* compaction_worklists)
    ...
    , discovered_ephemeron_pairs_worklist_(*marking_worklists.discovered_ephemeron_pairs_worklist())
    , ephemeron_pairs_for_processing_worklist_(*marking_worklists.ephemeron_pairs_for_processing_worklist())
    , weak_containers_worklist_(*marking_worklists.weak_containers_worklist())
{
    if (compaction_worklists) {
        movable_slots_worklist_ = std::make_unique<CompactionWorklists::MovableReferencesWorklist::Local>(*compaction_worklists->movable_slots_worklist());
    }
}
...
void BasicMarkingState::Publish()
{
    ...
    if (movable_slots_worklist_)
        movable_slots_worklist_->Publish();
...
{
    std::unordered_set<HeapObjectHeader*> objects = not_fully_constructed_worklist_.Extract<AccessMode::kAtomic>();
...
{
    StatsCollector::EnabledScope stats_scope(heap_.stats_collector(), StatsCollector::kMarkFlushEphemerons);
    discovered_ephemeron_pairs_worklist_.Publish();
    if (!discovered_ephemeron_pairs_worklist_.IsGlobalEmpty()) {
        ephemeron_pairs_for_processing_worklist_.Merge(discovered_ephemeron_pairs_worklist_);
    }
}
