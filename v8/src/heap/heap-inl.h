...
template <typename T> Tagged<T> ForwardingAddress(Tagged<T> heap_obj)
{
...
{
    return Cast<FixedArray>(Tagged<Object>(roots_table()[RootIndex::kSingleCharacterStringTable]));
}
...
#define DCHECK_STATIC_ROOT(obj, name)                                                                                                                          \
    if constexpr (RootsTable::IsReadOnly(RootIndex::k##name) && RootIndex::k##name != RootIndex::kException) {                                                 \
        DCHECK_WITH_MSG(V8HeapCompressionScheme::CompressObject(obj.ptr()) == StaticReadOnlyRootsPointerTable[static_cast<size_t>(RootIndex::k##name)],        \
...
#define ROOT_ACCESSOR(type, name, CamelName)                                                                                                                   \
    ...
    {                                                                                                                                                          \
        ...
        DCHECK_IMPLIES(deserialization_complete(), !RootsTable::IsImmortalImmovable(RootIndex::k##CamelName));                                                 \
...
void Heap::SetFunctionsMarkedForManualOptimization(Tagged<Object> hash_table)
{
    ...
    roots_table()[RootIndex::kFunctionsMarkedForManualOptimization] = hash_table.ptr();
}

#if V8_ENABLE_WEBASSEMBLY
void Heap::SetWasmCanonicalRttsAndJSToWasmWrappers(Tagged<WeakFixedArray> rtts, Tagged<WeakFixedArray> js_to_wasm_wrappers)
{
...
{
    DCHECK(idx == OLD_SPACE || idx == CODE_SPACE || idx == SHARED_SPACE || idx == TRUSTED_SPACE || idx == SHARED_TRUSTED_SPACE);
    return static_cast<PagedSpace*>(space_[idx].get());
}

Space* Heap::space(int idx) const
{
    return space_[idx].get();
}

Address* Heap::NewSpaceAllocationTopAddress()
{
    return new_space_ || v8_flags.sticky_mark_bits ? isolate()->isolate_data()->new_allocation_info_.top_address() : nullptr;
}

Address* Heap::NewSpaceAllocationLimitAddress()
{
    return new_space_ || v8_flags.sticky_mark_bits ? isolate()->isolate_data()->new_allocation_info_.limit_address() : nullptr;
}
...
int Heap::MaxRegularHeapObjectSize(AllocationType allocation)
{
    if (allocation == AllocationType::kCode) {
        DCHECK_EQ(MemoryChunkLayout::MaxRegularCodeObjectSize(), max_regular_code_object_size_);
        return max_regular_code_object_size_;
    }
    return kMaxRegularHeapObjectSize;
}

AllocationResult Heap::AllocateRaw(int size_in_bytes, AllocationType type, AllocationOrigin origin, AllocationAlignment alignment)
{
    return heap_allocator_->AllocateRaw(size_in_bytes, type, origin, alignment);
}

Address Heap::AllocateRawOrFail(int size, AllocationType allocation, AllocationOrigin origin, AllocationAlignment alignment)
{
    return heap_allocator_->AllocateRawWith<HeapAllocator::kRetryOrFail>(size, allocation, origin, alignment).address();
}
...
void Heap::FinalizeExternalString(Tagged<String> string)
{
    ...
    page->DecrementExternalBackingStoreBytes(ExternalBackingStoreType::kExternalString, ext_string->ExternalPayloadSize());
    ext_string->DisposeResource(isolate());
}

Address Heap::NewSpaceTop()
{
    return new_space_ || v8_flags.sticky_mark_bits ? allocator()->new_space_allocator()->top() : kNullAddress;
}

Address Heap::NewSpaceLimit()
{
    return new_space_ || v8_flags.sticky_mark_bits ? allocator()->new_space_allocator()->limit() : kNullAddress;
}
...
{
    return old_space_->Contains(object) && (!v8_flags.sticky_mark_bits || !HeapLayout::InYoungGeneration(object));
}
...
bool Heap::IsPendingAllocationInternal(Tagged<HeapObject> object)
{
    ...
    case NEW_LO_SPACE: {
        LargeObjectSpace* large_space = static_cast<LargeObjectSpace*>(base_space);
        base::SharedMutexGuard<base::kShared> guard(large_space->pending_allocation_mutex());
...
void Heap::ExternalStringTable::AddString(Tagged<String> string)
{
    ...
    if (v8_flags.shared_string_table && heap_->isolate()->is_shared_space_isolate()) {
...
int Heap::MaxNumberToStringCacheSize() const
{
    ...
    number_string_cache_size
        = std::max(static_cast<size_t>(kInitialNumberStringCacheSize * 2), std::min(static_cast<size_t>(0x4000), number_string_cache_size));
    // There is a string and a number per entry so the length is twice the number
    // of entries.
    return static_cast<int>(number_string_cache_size * 2);
}

void Heap::IncrementExternalBackingStoreBytes(ExternalBackingStoreType type, size_t amount)
{
    base::CheckedIncrement(&backing_store_bytes_, static_cast<uint64_t>(amount), std::memory_order_relaxed);
    // TODO(mlippautz): Implement interrupt for global memory allocations that can
    // trigger garbage collections.
}

void Heap::DecrementExternalBackingStoreBytes(ExternalBackingStoreType type, size_t amount)
{
    base::CheckedDecrement(&backing_store_bytes_, static_cast<uint64_t>(amount), std::memory_order_relaxed);
}
