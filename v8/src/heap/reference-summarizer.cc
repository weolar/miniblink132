...
public:
    ...
    void AddStrongReferenceForReferenceSummarizer(Tagged<HeapObject> host, Tagged<HeapObject> obj)
    {
    ...
    void AddWeakReferenceForReferenceSummarizer(Tagged<HeapObject> host, Tagged<HeapObject> obj)
    {
    ...
    {
        return &local_weak_objects_;
    }

private:
    void AddReference(Tagged<HeapObject> host, Tagged<HeapObject> obj, ReferenceSummary::UnorderedHeapObjectSet& references)
    {
...
class ReferenceSummarizerMarkingVisitor : public MarkingVisitorBase<ReferenceSummarizerMarkingVisitor> {
public:
    ReferenceSummarizerMarkingVisitor(Heap* heap, ReferenceSummarizerMarkingState* marking_state)
        : MarkingVisitorBase(marking_state->local_marking_worklists(), marking_state->local_weak_objects(), heap, 0 /*mark_compact_epoch*/,
            {} /*code_flush_mode*/, true /*should_keep_ages_unchanged*/, 0 /*code_flushing_increase*/)
    ...
    void MarkPointerTableEntry(Tagged<HeapObject> host, IndirectPointerSlot slot)
    {
    }
...
ReferenceSummary ReferenceSummary::SummarizeReferencesFrom(Heap* heap, Tagged<HeapObject> obj)
{
