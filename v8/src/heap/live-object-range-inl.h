...
LiveObjectRange::iterator::iterator(const PageMetadata* page)
    ...
    , current_cell_index_(MarkingBitmap::IndexToCell(MarkingBitmap::AddressToIndex(page->area_start())))
...
void LiveObjectRange::iterator::AdvanceToNextValidObject()
{
    ...
    while (AdvanceToNextMarkedObject() && InstanceTypeChecker::IsFreeSpaceOrFiller(current_map_)) {
    }
}
...
bool LiveObjectRange::iterator::AdvanceToNextMarkedObject()
{
    ...
    if (!current_object_.is_null()) {
        ...
        DCHECK_GE(MarkingBitmap::IndexToCell(next_markbit_index), current_cell_index_);
        ...
        const MarkBit::CellType mask = MarkingBitmap::IndexInCellMask(next_markbit_index);
    ...
    while (true) {
        if (current_cell_) {
            ...
            Address current_cell_base = chunk->address() + MarkingBitmap::CellToBase(current_cell_index_);
            ...
            current_size_ = ALIGN_TO_ALLOCATION_ALIGNMENT(current_object_->SizeFromMap(current_map_));
            CHECK(page_->ContainsLimit(object_address + current_size_));
            return true;
        }
        if (++current_cell_index_ >= MarkingBitmap::kCellsCount)
            break;
        current_cell_ = cells_[current_cell_index_];
    }
    return false;
