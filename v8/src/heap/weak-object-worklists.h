...
template <typename SlotType> struct HeapObjectAndSlotPOD {
...
#define WEAK_OBJECT_WORKLISTS_GENERIC(F)                                                                                                                       \
    ...
    F(HeapObjectAndSlot, weak_references_non_trivial_unmarked, WeakReferencesNonTrivialUnmarked)                                                               \
    ...
    F(Tagged<SharedFunctionInfo>, code_flushing_candidates, CodeFlushingCandidates)                                                                            \
...
#define WEAK_OBJECT_WORKLISTS(F)                                                                                                                               \
    ...
    F(Tagged<JSFunction>, baseline_flushing_candidates, BaselineFlushingCandidates)
...
public:
    template <typename Type> using WeakObjectWorklist = ::heap::base::Worklist<Type, 64>;
...
#define DECLARE_WORKLIST(Type, name, _) WeakObjectWorklist<Type>::Local name##_local;
...
private:
#define DECLARE_UPDATE_METHODS(Type, _, Name) static void Update##Name(WeakObjectWorklist<Type>&);
