...
public:
    ...
    // Only works if the object is in the first kPageSize of the MemoryChunk.
    V8_INLINE static MemoryChunkMetadata* FromHeapObject(const HeapObjectLayout* o);
    ...
    {
        return ChunkAddress() + high_water_mark_;
    }
...
// cage base.
template <> struct hash<const i::MemoryChunkMetadata*> {
...
template <> struct hash<i::MemoryChunkMetadata*> {
