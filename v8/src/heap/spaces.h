...
#define DCHECK_OBJECT_SIZE(size) DCHECK((0 < size) && (size <= kMaxRegularHeapObjectSize))

#define DCHECK_CODEOBJECT_SIZE(size) DCHECK((0 < size) && (size <= MemoryChunkLayout::MaxRegularCodeObjectSize()))

template <typename Enum, typename Callback> void ForAll(Callback callback)
{
...
public:
    static inline void MoveExternalBackingStoreBytes(ExternalBackingStoreType type, Space* from, Space* to, size_t amount);
...
template <class PageType> class PageIteratorImpl : public base::iterator<std::forward_iterator_tag, PageType> {
public:
    explicit PageIteratorImpl(PageType* p)
        : p_(p)
    {
    }
...
public:
    explicit MemoryChunkIterator(Heap* heap)
        : space_iterator_(heap)
    {
    }
