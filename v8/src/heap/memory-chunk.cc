...
// static
constexpr MemoryChunk::MainThreadFlags MemoryChunk::kPointersToHereAreInterestingMask;
// static
constexpr MemoryChunk::MainThreadFlags MemoryChunk::kPointersFromHereAreInterestingMask;
// static
...
// static
constexpr MemoryChunk::MainThreadFlags MemoryChunk::kSkipEvacuationSlotsRecordingMask;
...
{
#ifdef V8_ENABLE_SANDBOX
    DCHECK_IMPLIES(metadata_pointer_table_[metadata_index_] != nullptr, metadata_pointer_table_[metadata_index_] == metadata);
...
{
    uint32_t index;
    if (V8HeapCompressionScheme::GetPtrComprCageBaseAddress(chunk_address) == V8HeapCompressionScheme::base()) {
    ...
    } else if (TrustedRange::GetProcessWideTrustedRange()->region().contains(chunk_address)) {
...
void MemoryChunk::InitializationMemoryFence()
{
    ...
    base::Release_Store(reinterpret_cast<base::AtomicWord*>(&metadata_), reinterpret_cast<base::AtomicWord>(metadata_));
#else
    ...
    base::Release_Store(reinterpret_cast<base::AtomicWord*>(&metadata_pointer_table_[metadata_index_]),
        reinterpret_cast<base::AtomicWord>(metadata_pointer_table_[metadata_index_]));
    base::Release_Store(reinterpret_cast<base::Atomic32*>(&metadata_index_), metadata_index_);
#endif
#endif
}
...
void MemoryChunk::SynchronizedLoad() const
{
    ...
    MemoryChunkMetadata* metadata
        = reinterpret_cast<MemoryChunkMetadata*>(base::Acquire_Load(reinterpret_cast<base::AtomicWord*>(&(const_cast<MemoryChunk*>(this)->metadata_))));
#else
    static_assert(sizeof(base::AtomicWord) == sizeof(metadata_pointer_table_[0]));
    static_assert(sizeof(base::Atomic32) == sizeof(metadata_index_));
    uint32_t metadata_index = base::Acquire_Load(reinterpret_cast<base::Atomic32*>(&(const_cast<MemoryChunk*>(this)->metadata_index_)));
    MemoryChunkMetadata* metadata
        = reinterpret_cast<MemoryChunkMetadata*>(base::Acquire_Load(reinterpret_cast<base::AtomicWord*>(&metadata_pointer_table_[metadata_index])));
#endif
...
// static
MemoryChunk::MainThreadFlags MemoryChunk::OldGenerationPageFlags(MarkingMode marking_mode, AllocationSpace space)
{
    ...
    if (marking_mode == MarkingMode::kMajorMarking) {
        flags_to_set |= MemoryChunk::POINTERS_TO_HERE_ARE_INTERESTING | MemoryChunk::POINTERS_FROM_HERE_ARE_INTERESTING | MemoryChunk::INCREMENTAL_MARKING
...
// static
MemoryChunk::MainThreadFlags MemoryChunk::YoungGenerationPageFlags(MarkingMode marking_mode)
{
...
void MemoryChunk::SetOldGenerationPageFlags(MarkingMode marking_mode, AllocationSpace space)
{
    ...
    if (marking_mode != MarkingMode::kMajorMarking) {
        if (IsAnySharedSpace(space)) {
            ...
            flags_to_clear |= MemoryChunk::POINTERS_FROM_HERE_ARE_INTERESTING | MemoryChunk::INCREMENTAL_MARKING;
        } else {
...
bool MemoryChunk::SandboxSafeInReadOnlySpace() const
{
    ...
    SBXCHECK_EQ(static_cast<const ReadOnlyPageMetadata*>(metadata)->ChunkAddress(), address());
