...
public:
    ...
    // remembered set.
    template <AccessMode access_mode> static void Insert(SlotSet* slot_set, size_t slot_offset)
    {
        slot_set->Insert<access_mode == v8::internal::AccessMode::ATOMIC ? v8::internal::SlotSet::AccessMode::ATOMIC
                                                                         : v8::internal::SlotSet::AccessMode::NON_ATOMIC>(slot_offset);
    }
    ...
    static int Iterate(SlotSet* slot_set, const MutablePageMetadata* chunk, Callback callback, SlotSet::EmptyBucketMode mode)
    {
        int slots = 0;
        if (slot_set != nullptr) {
            slots += slot_set->Iterate<access_mode>(chunk->ChunkAddress(), 0, chunk->buckets(), callback, mode);
        }
        return slots;
    }
    ...
    static void RemoveRange(SlotSet* slot_set, MutablePageMetadata* page, Address start, Address end, SlotSet::EmptyBucketMode mode)
    {
        if (slot_set != nullptr) {
            ...
            slot_set->RemoveRange(static_cast<int>(start_offset), static_cast<int>(end_offset), page->buckets(), mode);
        }
    }
    ...
    static void CheckNoneInRange(SlotSet* slot_set, MemoryChunk* chunk, Address start, Address end)
    {
        if (slot_set != nullptr) {
            ...
            size_t end_bucket = SlotSet::BucketForSlot(chunk->OffsetMaybeOutOfRange(end) - kTaggedSize) + 1;
            slot_set->Iterate(
...
template <RememberedSetType type> class RememberedSet : public AllStatic {
public:
    // Given a page and a slot in that page, this function adds the slot to the
    // remembered set.
    template <AccessMode access_mode> static void Insert(MutablePageMetadata* page, size_t slot_offset)
    {
    ...
    {
        static_assert(type == RememberedSetType::OLD_TO_NEW || type == RememberedSetType::OLD_TO_NEW_BACKGROUND);
    ...
    static void MergeAndDeleteTyped(MutablePageMetadata* chunk, TypedSlotSet&& other_typed_slot_set)
    {
        ...
        TypedSlotSet* typed_slot_set = chunk->typed_slot_set<type, AccessMode::NON_ATOMIC>();
        if (typed_slot_set == nullptr) {
            chunk->set_typed_slot_set<RememberedSetType::OLD_TO_NEW, AccessMode::NON_ATOMIC>(&other_typed_slot_set);
            return;
        }
    ...
    static void CheckNoneInRange(MutablePageMetadata* page, Address start, Address end)
    {
        ...
        RememberedSetOperations::CheckNoneInRange(slot_set, page->Chunk(), start, end);
    }
    ...
    template <typename Callback> static void IterateMemoryChunks(Heap* heap, Callback callback)
    {
    ...
    static int Iterate(SlotSet* slot_set, const MutablePageMetadata* chunk, Callback callback, SlotSet::EmptyBucketMode mode)
    {
        return RememberedSetOperations::Iterate<access_mode>(slot_set, chunk, callback, mode);
    }
    ...
    static int IterateAndTrackEmptyBuckets(MutablePageMetadata* chunk, Callback callback, ::heap::base::Worklist<MutablePageMetadata*, 64>::Local* empty_chunks)
    {
        ...
        int slots = 0;
        if (slot_set != nullptr) {
            PossiblyEmptyBuckets* possibly_empty_buckets = chunk->possibly_empty_buckets();
            slots += slot_set->IterateAndTrackEmptyBuckets(chunk->ChunkAddress(), 0, chunk->buckets(), callback, possibly_empty_buckets);
    ...
    static bool CheckPossiblyEmptyBuckets(MutablePageMetadata* chunk)
    {
        ...
        SlotSet* slot_set = chunk->slot_set<type, AccessMode::NON_ATOMIC>();
        if (slot_set != nullptr && slot_set->CheckPossiblyEmptyBuckets(chunk->buckets(), chunk->possibly_empty_buckets())) {
    ...
    static void RemoveRangeTyped(MutablePageMetadata* page, Address start, Address end)
    {
        TypedSlotSet* slot_set = page->typed_slot_set<type>();
        if (slot_set != nullptr) {
            slot_set->Iterate([=](SlotType slot_type, Address slot_addr) { return start <= slot_addr && slot_addr < end ? REMOVE_SLOT : KEEP_SLOT; },
    ...
    template <typename Callback> static int IterateTyped(MutablePageMetadata* chunk, Callback callback)
    {
    ...
    template <typename Callback> static int IterateTyped(TypedSlotSet* slot_set, Callback callback)
    {
...
public:
    ...
    inline static Tagged<HeapObject> GetTargetObject(Heap* heap, SlotType slot_type, Address addr);

private:
    // Updates a code entry slot using an untyped slot callback.
    // The callback accepts FullMaybeObjectSlot and returns SlotCallbackResult.
    template <typename Callback> static SlotCallbackResult UpdateCodeEntry(Address entry_address, Callback callback)
    {
        Tagged<InstructionStream> code = InstructionStream::FromEntryAddress(entry_address);
    ...
    // The callback accepts FullMaybeObjectSlot and returns SlotCallbackResult.
    template <typename Callback> static SlotCallbackResult UpdateCodeTarget(WritableRelocInfo* rinfo, Callback callback)
    {
        DCHECK(RelocInfo::IsCodeTargetMode(rinfo->rmode()));
        Tagged<InstructionStream> old_target = InstructionStream::FromTargetAddress(rinfo->target_address());
        Tagged<InstructionStream> new_target = old_target;
        SlotCallbackResult result = callback(FullMaybeObjectSlot(&new_target));
        DCHECK(!HasWeakHeapObjectTag(new_target));
        if (new_target != old_target) {
            rinfo->set_target_address(Cast<InstructionStream>(new_target)->instruction_start());
        }
        return result;
    }
    ...
    // The callback accepts FullMaybeObjectSlot and returns SlotCallbackResult.
    template <typename Callback> static SlotCallbackResult UpdateEmbeddedPointer(Heap* heap, WritableRelocInfo* rinfo, Callback callback)
    {
