...
void CodeStatistics::RecordCodeAndMetadataStatistics(Tagged<HeapObject> object, Isolate* isolate)
{
    ...
    if (IsScript(object, cage_base)) {
        ...
        if (IsExternalString(source, cage_base)) {
            Tagged<ExternalString> external_source_string = Cast<ExternalString>(source);
...
void CodeStatistics::CollectCodeStatistics(PagedSpace* space, Isolate* isolate)
{
    PagedSpaceObjectIterator obj_it(isolate->heap(), space);
    for (Tagged<HeapObject> obj = obj_it.Next(); !obj.is_null(); obj = obj_it.Next()) {
...
// - by code kind (only in debug mode)
// - by code comment (only in debug mode)
void CodeStatistics::CollectCodeStatistics(OldLargeObjectSpace* space, Isolate* isolate)
{
...
void CodeStatistics::ReportCodeStatistics(Isolate* isolate)
{
    ...
    for (int i = 0; i < kCodeKindCount; i++) {
        if (code_kind_statistics[i] > 0) {
            PrintF("     %-20s: %10d bytes\n", CodeKindToString(static_cast<CodeKind>(i)), code_kind_statistics[i]);
        }
    }
    PrintF("\n");
    ...
    for (int i = 0; i <= CommentStatistic::kMaxComments; i++) {
        ...
        if (cs.size > 0) {
            PrintF("   %-30s: %10d/%6d     (%d)\n", cs.comment, cs.size, cs.count, cs.size / cs.count);
        }
    }
    PrintF("\n");
}
...
void CodeStatistics::EnterComment(Isolate* isolate, const char* comment, int delta)
{
    CommentStatistic* comments_statistics = isolate->paged_space_comments_statistics();
...
void CodeStatistics::CollectCommentStatistics(Isolate* isolate, CodeCommentsIterator* cit)
{
...
void CodeStatistics::CollectCodeCommentStatistics(Tagged<AbstractCode> obj, Isolate* isolate)
{
    // Bytecode objects do not contain RelocInfo.
    PtrComprCageBase cage_base { isolate };
    if (!IsCode(obj, cage_base))
        return;
