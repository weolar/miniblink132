...
SlotCallbackResult UpdateTypedSlotHelper::UpdateTypedSlot(
    WritableJitAllocation& jit_allocation, Heap* heap, SlotType slot_type, Address addr, Callback callback)
{
    switch (slot_type) {
    ...
    case SlotType::kEmbeddedObjectCompressed: {
        WritableRelocInfo rinfo(jit_allocation, addr, RelocInfo::COMPRESSED_EMBEDDED_OBJECT);
        return UpdateEmbeddedPointer(heap, &rinfo, callback);
    }
    case SlotType::kEmbeddedObjectFull: {
        WritableRelocInfo rinfo(jit_allocation, addr, RelocInfo::FULL_EMBEDDED_OBJECT);
        return UpdateEmbeddedPointer(heap, &rinfo, callback);
    }
    case SlotType::kConstPoolEmbeddedObjectCompressed: {
        Tagged<HeapObject> old_target
            = Cast<HeapObject>(Tagged<Object>(V8HeapCompressionScheme::DecompressTagged(heap->isolate(), base::Memory<Tagged_t>(addr))));
        Tagged<HeapObject> new_target = old_target;
        SlotCallbackResult result = callback(FullMaybeObjectSlot(&new_target));
        DCHECK(!HasWeakHeapObjectTag(new_target));
        if (new_target != old_target) {
            base::Memory<Tagged_t>(addr) = V8HeapCompressionScheme::CompressObject(new_target.ptr());
        }
        return result;
    }
...
Tagged<HeapObject> UpdateTypedSlotHelper::GetTargetObject(Heap* heap, SlotType slot_type, Address addr)
{
    switch (slot_type) {
    case SlotType::kCodeEntry: {
    ...
    case SlotType::kConstPoolEmbeddedObjectCompressed: {
        Address full = V8HeapCompressionScheme::DecompressTagged(heap->isolate(), base::Memory<Tagged_t>(addr));
