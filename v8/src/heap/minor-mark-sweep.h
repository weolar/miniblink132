...
using YoungGenerationMainMarkingVisitor = YoungGenerationMarkingVisitor<YoungGenerationMarkingVisitationMode::kParallel>;
...
public:
    ...
    public:
        ...
        template <typename Visitor> bool ProcessNextItem(Visitor* visitor)
        {
    ...
    {
        return remaining_remembered_sets_marking_items_.load(std::memory_order_relaxed);
    }
...
private:
    ...
    public:
        ...
        template <typename Visitor> void Process(Visitor* visitor);
    ...
    private:
        ...
        template <typename Visitor> void MarkUntypedPointers(Visitor* visitor);
        template <typename Visitor> void MarkTypedPointers(Visitor* visitor);
        template <typename Visitor, typename TSlot> V8_INLINE SlotCallbackResult CheckAndMarkObject(Visitor* visitor, TSlot slot);
    ...
    template <typename Visitor> bool ProcessNextItem(Visitor* visitor, std::optional<size_t>& index);
...
public:
    explicit YoungGenerationRootMarkingVisitor(MinorMarkSweepCollector* collector);
...
private:
    template <typename TSlot> void VisitPointersImpl(Root root, TSlot start, TSlot end);
