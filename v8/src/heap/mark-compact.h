...
public:
    ...
    enum class MarkingWorklistProcessingMode { kDefault, kTrackNewlyDiscoveredObjects };
    ...
    std::pair<size_t, size_t> ProcessMarkingWorklist(v8::base::TimeDelta max_duration, size_t max_bytes_to_process, MarkingWorklistProcessingMode mode);
    ...
    template <typename THeapObjectSlot> V8_INLINE static void RecordSlot(Tagged<HeapObject> object, THeapObjectSlot slot, Tagged<HeapObject> target);
    template <typename THeapObjectSlot> V8_INLINE static void RecordSlot(MemoryChunk* source_chunk, THeapObjectSlot slot, Tagged<HeapObject> target);
    ...
    {
        return local_weak_objects_.get();
    }
...
private:
    ...
    void ReportAbortedEvacuationCandidateDueToFlags(Address failed_start, PageMetadata* page);
    ...
    std::vector<std::pair<Address, PageMetadata*>> aborted_evacuation_candidates_due_to_flags_;
