...
PagedSpaceObjectIterator::PagedSpaceObjectIterator(Heap* heap, const PagedSpaceBase* space)
    : space_(space)
...
PagedSpaceBase::PagedSpaceBase(
    Heap* heap, AllocationSpace space, Executability executable, std::unique_ptr<FreeList> free_list, CompactionSpaceKind compaction_space_kind)
...
PageMetadata* PagedSpaceBase::InitializePage(MutablePageMetadata* mutable_page_metadata)
{
...
void PagedSpaceBase::TearDown()
{
    while (!memory_chunk_list_.Empty()) {
        ...
        heap()->memory_allocator()->Free(MemoryAllocator::FreeMode::kImmediately, chunk);
    }
    accounting_stats_.Clear();
}
...
void PagedSpaceBase::MergeCompactionSpace(CompactionSpace* other)
{
    ...
    for (auto it = other->begin(); it != other->end();) {
        ...
        other->RemovePage(p);
        AddPage(p);
        DCHECK_IMPLIES(!p->Chunk()->IsFlagSet(MemoryChunk::NEVER_ALLOCATE_ON_PAGE), p->AvailableInFreeList() == p->AvailableInFreeListFromAllocatedBytes());
    ...
    const bool is_from_client_heap = (other->destination_heap() == CompactionSpace::DestinationHeap::kSharedSpaceHeap);
    DCHECK_IMPLIES(is_from_client_heap, identity() == SHARED_SPACE);
    for (auto p : other->GetNewPages()) {
        heap()->NotifyOldGenerationExpansion(heap()->main_thread_local_heap(), identity(), p,
            is_from_client_heap ? Heap::OldGenerationExpansionNotificationOrigin::kFromClientHeap
...
{
    if (!base::OS::HasLazyCommits() || increment_value == 0)
        return;
    size_t old_value = committed_physical_memory_.fetch_add(increment_value, std::memory_order_relaxed);
    USE(old_value);
...
{
    if (!base::OS::HasLazyCommits() || decrement_value == 0)
        return;
    size_t old_value = committed_physical_memory_.fetch_sub(decrement_value, std::memory_order_relaxed);
    USE(old_value);
...
void PagedSpaceBase::AddPageImpl(PageMetadata* page)
{
    ...
    CHECK(page->SweepingDone());
    page->set_owner(this);
    DCHECK_IMPLIES(identity() == NEW_SPACE, page->Chunk()->IsFlagSet(MemoryChunk::TO_PAGE));
    DCHECK_IMPLIES(identity() != NEW_SPACE, !page->Chunk()->IsFlagSet(MemoryChunk::TO_PAGE));
...
void PagedSpaceBase::RemovePage(PageMetadata* page)
{
    ...
    DCHECK_IMPLIES(identity() == NEW_SPACE, heap()->gc_state() != Heap::NOT_IN_GC);
    if (identity() == NEW_SPACE) {
...
bool PagedSpaceBase::TryExpand(LocalHeap* local_heap, AllocationOrigin origin)
{
    ...
    const size_t accounted_size = MemoryChunkLayout::AllocatableMemoryInMemoryChunk(identity());
    if (origin != AllocationOrigin::kGC && identity() != NEW_SPACE) {
        base::MutexGuard expansion_guard(heap_->heap_expansion_mutex());
        if (!heap()->IsOldGenerationExpansionAllowed(accounted_size, expansion_guard)) {
            return false;
        }
    }
    const MemoryAllocator::AllocationMode allocation_mode
        = (identity() == NEW_SPACE || identity() == OLD_SPACE) ? MemoryAllocator::AllocationMode::kUsePool : MemoryAllocator::AllocationMode::kRegular;
    PageMetadata* page = heap()->memory_allocator()->AllocatePage(allocation_mode, this, executable());
    if (page == nullptr)
        return false;
...
void PagedSpaceBase::ReleasePageImpl(PageMetadata* page, MemoryAllocator::FreeMode free_mode)
{
    DCHECK(page->SweepingDone());
...
void PagedSpaceBase::Verify(Isolate* isolate, SpaceVerificationVisitor* visitor) const
{
    ...
    for (const PageMetadata* page : *this) {
        size_t external_page_bytes[static_cast<int>(ExternalBackingStoreType::kNumValues)] = { 0 };
        ...
        for (Tagged<HeapObject> object : HeapObjectRange(page)) {
            ...
            if (IsExternalString(object, cage_base)) {
                ...
                external_page_bytes[static_cast<int>(ExternalBackingStoreType::kExternalString)] += payload_size;
            }
        }
        ForAll<ExternalBackingStoreType>([page, external_page_bytes, &external_space_bytes](ExternalBackingStoreType type, int index) {
            CHECK_EQ(external_page_bytes[index], page->ExternalBackingStoreBytes(type));
...
void PagedSpaceBase::VerifyCountersBeforeConcurrentSweeping() const
{
    ...
    size_t total_allocated = 0;
    for (const PageMetadata* page : *this) {
        size_t page_allocated = page->SweepingDone() ? page->allocated_bytes() : page->live_bytes();
        total_capacity += page->area_size();
...
void PagedSpaceBase::AddRangeToActiveSystemPages(PageMetadata* page, Address start, Address end)
{
...
void PagedSpaceBase::ReduceActiveSystemPages(PageMetadata* page, ActiveSystemPages active_system_pages)
{
    const size_t reduced_pages = page->active_system_pages()->Reduce(active_system_pages);
    DecrementCommittedPhysicalMemory(reduced_pages * MemoryAllocator::GetCommitPageSize());
}

void PagedSpaceBase::UnlinkFreeListCategories(PageMetadata* page)
{
    DCHECK_EQ(this, page->owner());
    page->ForAllFreeListCategories([this](FreeListCategory* category) { free_list()->RemoveCategory(category); });
}
...
size_t PagedSpaceBase::RelinkFreeListCategories(PageMetadata* page)
{
    ...
    DCHECK_IMPLIES(
        !page->Chunk()->IsFlagSet(MemoryChunk::NEVER_ALLOCATE_ON_PAGE), page->AvailableInFreeList() == page->AvailableInFreeListFromAllocatedBytes());
    return added;
}
...
void PagedSpaceBase::RefillFreeList()
{
    ...
    DCHECK(identity() == OLD_SPACE || identity() == CODE_SPACE || identity() == SHARED_SPACE || identity() == NEW_SPACE || identity() == TRUSTED_SPACE);
...
void CompactionSpace::NotifyNewPage(PageMetadata* page)
{
    ...
    DCHECK_IMPLIES(
        identity() != SHARED_SPACE || destination_heap() != DestinationHeap::kSharedSpaceHeap, !page->Chunk()->IsFlagSet(MemoryChunk::BLACK_ALLOCATED));
...
void CompactionSpace::RefillFreeList()
{
    ...
    while ((added <= kCompactionMemoryWanted) && (p = sweeper->GetSweptPageSafe(this))) {
        // We regularly sweep NEVER_ALLOCATE_ON_PAGE pages. We drop the freelist
        // entries here to make them unavailable for allocations.
        if (p->Chunk()->IsFlagSet(MemoryChunk::NEVER_ALLOCATE_ON_PAGE)) {
...
CompactionSpaceCollection::CompactionSpaceCollection(Heap* heap, CompactionSpaceKind compaction_space_kind)
    : old_space_(heap, OLD_SPACE, Executability::NOT_EXECUTABLE, compaction_space_kind, CompactionSpace::DestinationHeap::kSameHeap)
    , code_space_(heap, CODE_SPACE, Executability::EXECUTABLE, compaction_space_kind, CompactionSpace::DestinationHeap::kSameHeap)
    , trusted_space_(heap, TRUSTED_SPACE, Executability::NOT_EXECUTABLE, compaction_space_kind, CompactionSpace::DestinationHeap::kSameHeap)
{
    if (heap->isolate()->has_shared_space()) {
        const CompactionSpace::DestinationHeap dest_heap
            = heap->isolate()->is_shared_space_isolate() ? CompactionSpace::DestinationHeap::kSameHeap : CompactionSpace::DestinationHeap::kSharedSpaceHeap;
        shared_space_.emplace(heap->isolate()->shared_space_isolate()->heap(), SHARED_SPACE, Executability::NOT_EXECUTABLE, compaction_space_kind, dest_heap);
    }
}
