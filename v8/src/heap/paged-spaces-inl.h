    ...
    while (cur_addr_ != cur_end_) {
        ...
        if (IsFreeSpaceOrFiller(obj, cage_base())) {
            ...
            if (IsInstructionStream(obj, cage_base())) {
                DCHECK_EQ(PageMetadata::FromHeapObject(obj)->owner_identity(), CODE_SPACE);
...
template <bool during_sweep> size_t PagedSpaceBase::FreeInternal(Address start, size_t size_in_bytes)
{
    if (size_in_bytes == 0)
        return 0;
    size_t wasted;
    if (executable_) {
        ...
        wasted = free_list_->Free(free_space, during_sweep ? kDoNotLinkCategory : kLinkCategory);
    } else {
        WritableFreeSpace free_space = WritableFreeSpace::ForNonExecutableMemory(start, size_in_bytes);
        heap()->CreateFillerObjectAtBackground(free_space);
        wasted = free_list_->Free(free_space, during_sweep ? kDoNotLinkCategory : kLinkCategory);
    }
