...
bool NativeContextInferrer::Infer(PtrComprCageBase cage_base, Tagged<Map> map, Tagged<HeapObject> object, Address* native_context)
{
    Tagged<Object> maybe_native_context = map->map()->raw_native_context_or_null();
...
V8_INLINE bool NativeContextStats::HasExternalBytes(Tagged<Map> map)
{
    ...
    return (instance_type == JS_ARRAY_BUFFER_TYPE || InstanceTypeChecker::IsExternalString(instance_type));
}

V8_INLINE void NativeContextStats::IncrementSize(Address context, Tagged<Map> map, Tagged<HeapObject> object, size_t size)
{
