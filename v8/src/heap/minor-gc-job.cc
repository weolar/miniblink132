...
{
    if (v8_flags.sticky_mark_bits) {
        return heap->sticky_space()->young_objects_size() >= YoungGenerationTaskTriggerSize(heap);
    } else {
...
{
    if (!v8_flags.minor_gc_task)
        return;
    if (current_task_id_ != CancelableTaskManager::kInvalidTaskId)
        return;
...
void MinorGCJob::Task::RunInternal()
{
    ...
    if (v8_flags.separate_gc_phases && isolate()->heap()->incremental_marking()->IsMajorMarking()) {
