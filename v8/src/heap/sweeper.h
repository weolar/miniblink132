...
public:
    ...
    private:
        ...
        template <typename ShouldYieldCallback> bool ContributeAndWaitForPromotedPagesIterationImpl(ShouldYieldCallback should_yield_callback);
        template <typename ShouldYieldCallback> bool ParallelIteratePromotedPagesImpl(ShouldYieldCallback should_yield_callback);
    ...
    {
        if (space == NEW_SPACE)
            return minor_sweeping_in_progress();
    ...
    V8_EXPORT_PRIVATE static std::optional<base::AddressRegion> ComputeDiscardMemoryArea(Address start, Address end);
...
private:
    ...
    static constexpr int kNumberOfSweepingSpaces = LAST_SWEEPABLE_SPACE - FIRST_SWEEPABLE_SPACE + 1;

    template <typename Callback> void ForAllSweepingSpaces(Callback callback) const
    {
    ...
    void CleanupRememberedSetEntriesForFreedMemory(Address free_start, Address free_end, PageMetadata* page, bool record_free_ranges,
    ...
    template <SweepingScope scope> class SweepingState {
        using ConcurrentSweeper = typename std::conditional<scope == SweepingScope::kMinor, ConcurrentMinorSweeper, ConcurrentMajorSweeper>::type;
        using SweeperJob = typename std::conditional<scope == SweepingScope::kMinor, MinorSweeperJob, MajorSweeperJob>::type;
...
template <typename ShouldYieldCallback> bool Sweeper::LocalSweeper::ContributeAndWaitForPromotedPagesIterationImpl(ShouldYieldCallback should_yield_callback)
{
    if (!sweeper_->sweeping_in_progress())
        return true;
    if (!sweeper_->IsIteratingPromotedPages())
        return true;
    if (!ParallelIteratePromotedPagesImpl(should_yield_callback))
        return false;
    base::MutexGuard guard(&sweeper_->promoted_pages_iteration_notification_mutex_);
    ...
    sweeper_->promoted_pages_iteration_notification_variable_.Wait(&sweeper_->promoted_pages_iteration_notification_mutex_);
    return true;
}

template <typename ShouldYieldCallback> bool Sweeper::LocalSweeper::ParallelIteratePromotedPagesImpl(ShouldYieldCallback should_yield_callback)
{
