...
template <typename TSlot> void MarkingBarrier::Write(Tagged<HeapObject> host, TSlot slot, Tagged<HeapObject> value)
{
...
void MarkingBarrier::MarkValue(Tagged<HeapObject> host, Tagged<HeapObject> value)
{
    ...
    if (V8_UNLIKELY(uses_shared_heap_) && !is_shared_space_isolate_) {
        ...
        if (v8_flags.black_allocated_pages && HeapLayout::InBlackAllocatedPage(value)) {
            return;
        }
    ...
    DCHECK_IMPLIES(HeapLayout::InWritableSharedSpace(host), is_shared_space_isolate_);
    DCHECK_IMPLIES(HeapLayout::InWritableSharedSpace(value), is_shared_space_isolate_);
...
void MarkingBarrier::MarkValueLocal(Tagged<HeapObject> value)
{
    ...
    if (is_minor()) {
        ...
        if (HeapLayout::InYoungGeneration(value)) {
            MarkingHelper::TryMarkAndPush(heap_, current_worklists_.get(), &marking_state_, MarkingHelper::WorklistTarget::kRegular, value);
        }
    } else {
        ...
        if (!target_worklist)
            return;
        MarkingHelper::TryMarkAndPush(heap_, current_worklists_.get(), &marking_state_, target_worklist.value(), value);
    }
}

template <typename TSlot> inline void MarkingBarrier::MarkRange(Tagged<HeapObject> host, TSlot start, TSlot end)
{
    auto* isolate = heap_->isolate();
    const bool record_slots = IsCompacting(host) && !MemoryChunk::FromHeapObject(host)->ShouldSkipEvacuationSlotRecording();
...
bool MarkingBarrier::IsCompacting(Tagged<HeapObject> object) const
{
    ...
    return shared_heap_worklists_.has_value() && HeapLayout::InWritableSharedSpace(object);
}
