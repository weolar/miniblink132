...
#ifdef VERIFY_HEAP
class MarkingVerifierBase : public ObjectVisitorWithCageBases, public RootVisitor {
public:
    virtual void Run() = 0;
...
protected:
    ...
    void VerifyRoots();
    void VerifyMarkingOnPage(const PageMetadata* page, Address start, Address end);
...
template <ExternalStringTableCleaningMode mode> class ExternalStringTableCleanerVisitor final : public RootVisitor {
public:
    explicit ExternalStringTableCleanerVisitor(Heap* heap)
        : heap_(heap)
    {
    }
...
template class ExternalStringTableCleanerVisitor<ExternalStringTableCleaningMode::kAll>;
template class ExternalStringTableCleanerVisitor<ExternalStringTableCleaningMode::kYoungOnly>;
