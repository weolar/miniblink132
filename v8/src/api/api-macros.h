...
#define API_RCS_SCOPE(i_isolate, class_name, function_name) RCS_SCOPE(i_isolate, i::RuntimeCallCounterId::kAPI_##class_name##_##function_name);

#define ENTER_V8_BASIC(i_isolate)                                                                                                                              \
    /* Embedders should never enter V8 after terminating it */                                                                                                 \
    DCHECK_IMPLIES(i::v8_flags.strict_termination_checks, !i_isolate->is_execution_terminating());                                                             \
...
#define PREPARE_FOR_EXECUTION(context, class_name, function_name)                                                                                              \
    ...
    ENTER_V8_HELPER_INTERNAL(i_isolate, context, class_name, function_name, InternalEscapableScope, false);
...
#define ENTER_V8_NO_SCRIPT_NO_EXCEPTION(i_isolate) i::VMState<v8::OTHER> __state__((i_isolate));

#define ENTER_V8_FOR_NEW_CONTEXT(i_isolate) i::VMState<v8::OTHER> __state__((i_isolate));
#endif // DEBUG
