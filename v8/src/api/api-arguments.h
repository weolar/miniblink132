...
template <typename T> class CustomArguments : public CustomArgumentsBase {
public:
...
protected:
    ...
    template <typename V> Handle<V> GetReturnValue(Isolate* isolate) const;
    ...
    inline FullObjectSlot slot_at(int index) const
    {
        ...
        DCHECK_LE(static_cast<unsigned>(index), static_cast<unsigned>(T::kArgsLength));
...
class PropertyCallbackArguments final : public CustomArguments<PropertyCallbackInfo<Value>> {
public:
    ...
    inline Handle<JSAny> CallAccessorGetter(DirectHandle<AccessorInfo> info, Handle<Name> name);
    // Returns the result of [[Set]] operation or throws an exception.
    V8_WARN_UNUSED_RESULT
    inline bool CallAccessorSetter(DirectHandle<AccessorInfo> info, Handle<Name> name, Handle<Object> value);
    ...
    inline Handle<Object> CallNamedQuery(Handle<InterceptorInfo> interceptor, Handle<Name> name);
    inline Handle<JSAny> CallNamedGetter(Handle<InterceptorInfo> interceptor, Handle<Name> name);
    ...
    inline v8::Intercepted CallNamedSetter(DirectHandle<InterceptorInfo> interceptor, Handle<Name> name, Handle<Object> value);
    inline v8::Intercepted CallNamedDefiner(DirectHandle<InterceptorInfo> interceptor, Handle<Name> name, const v8::PropertyDescriptor& desc);
    inline v8::Intercepted CallNamedDeleter(DirectHandle<InterceptorInfo> interceptor, Handle<Name> name);

    // Empty handle means that the request was not intercepted.
    // Pending exception handling should be done by the caller.
    inline Handle<JSAny> CallNamedDescriptor(Handle<InterceptorInfo> interceptor, Handle<Name> name);
    // Returns JSArray-like object with property names or undefined.
    inline Handle<JSObjectOrUndefined> CallNamedEnumerator(Handle<InterceptorInfo> interceptor);
    ...
    // Empty handle means that the request was not intercepted.
    // Pending exception handling should be done by the caller.
    inline Handle<Object> CallIndexedQuery(Handle<InterceptorInfo> interceptor, uint32_t index);
    inline Handle<JSAny> CallIndexedGetter(Handle<InterceptorInfo> interceptor, uint32_t index);

    // Calls Setter/Definer/Deleter callback and returns whether the request
    // was intercepted.
    // Pending exception handling and interpretation of the result should be
    // done by the caller using GetBooleanReturnValue(..).
    inline v8::Intercepted CallIndexedSetter(DirectHandle<InterceptorInfo> interceptor, uint32_t index, Handle<Object> value);
    inline v8::Intercepted CallIndexedDefiner(DirectHandle<InterceptorInfo> interceptor, uint32_t index, const v8::PropertyDescriptor& desc);
    inline v8::Intercepted CallIndexedDeleter(Handle<InterceptorInfo> interceptor, uint32_t index);

    // Empty handle means that the request was not intercepted.
    // Pending exception handling should be done by the caller.
    inline Handle<JSAny> CallIndexedDescriptor(Handle<InterceptorInfo> interceptor, uint32_t index);
    // Returns JSArray-like object with property names or undefined.
    inline Handle<JSObjectOrUndefined> CallIndexedEnumerator(Handle<InterceptorInfo> interceptor);
    ...
    inline Maybe<InterceptorResult> GetBooleanReturnValue(
        v8::Intercepted intercepted, const char* callback_kind_for_error_message, bool ignore_return_value = false);

    // TODO(ishell): cleanup this hack by embedding the PropertyCallbackInfo
    // into PropertyCallbackArguments object.
    template <typename T> const v8::PropertyCallbackInfo<T>& GetPropertyCallbackInfo()
    {
    ...
    template <typename T> static Tagged<Object> GetPropertyKey(const PropertyCallbackInfo<T>& info)
    {
        return Tagged<Object>(info.args_[kPropertyKeyIndex]);
    }
    template <typename T> static Handle<Object> GetPropertyKeyHandle(const PropertyCallbackInfo<T>& info)
    {
    ...
    template <typename T> static uint32_t GetPropertyIndex(const PropertyCallbackInfo<T>& info)
    {
        ...
        Address ptr = reinterpret_cast<Address>(&info.args_) - offsetof(PropertyCallbackArguments, values_);
...
private:
    // Returns JSArray-like object with property names or undefined.
    inline Handle<JSObjectOrUndefined> CallPropertyEnumerator(Handle<InterceptorInfo> interceptor);
...
class FunctionCallbackArguments : public CustomArguments<FunctionCallbackInfo<Value>> {
public:
    ...
    FunctionCallbackArguments(
        Isolate* isolate, Tagged<FunctionTemplateInfo> target, Tagged<Object> holder, Tagged<HeapObject> new_target, Address* argv, int argc);
   ...
   */
    inline Handle<Object> CallOrConstruct(Tagged<FunctionTemplateInfo> function, bool is_construct);

    // Unofficial way of getting target FunctionTemplateInfo from
    // v8::FunctionCallbackInfo<T>.
    template <typename T> static Tagged<Object> GetTarget(const FunctionCallbackInfo<T>& info)
    {
...
static_assert(BuiltinArguments::kNumExtraArgs == BuiltinExitFrameConstants::kNumExtraArgs);
static_assert(BuiltinArguments::kNumExtraArgsWithReceiver == BuiltinExitFrameConstants::kNumExtraArgsWithReceiver);
