...
void MaterializedObjectStore::Set(Address fp, DirectHandle<FixedArray> materialized_objects)
{
    int index = StackIdToIndex(fp);
    if (index == -1) {
...
{
    auto it = std::find(frame_fps_.begin(), frame_fps_.end(), fp);
    return it == frame_fps_.end() ? -1 : static_cast<int>(std::distance(frame_fps_.begin(), it));
}

Handle<FixedArray> MaterializedObjectStore::GetStackEntries()
{
    return Handle<FixedArray>(isolate()->heap()->materialized_objects(), isolate());
}
...
Handle<FixedArray> MaterializedObjectStore::EnsureStackEntries(int length)
{
    ...
    Tagged<HeapObject> undefined_value = ReadOnlyRoots(isolate()).undefined_value();
