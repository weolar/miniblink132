...
// guarantees:
#define ASSERT_OFFSET(BuiltinName) static_assert(IsolateData::builtin_tier0_entry_table_offset() + Builtins::ToInt(BuiltinName) * kSystemPointerSize <= 0x7F)
...
void Deoptimizer::PatchJumpToTrampoline(Address pc, Address new_pc)
{
    ...
    Assembler masm(AssemblerOptions {}, ExternalAssemblerBuffer(reinterpret_cast<uint8_t*>(pc), kSize));
...
{
    return base::ReadUnalignedValue<Float32>(reinterpret_cast<Address>(simd128_registers_ + n));
}

Float64 RegisterValues::GetDoubleRegister(unsigned n) const
{
    V8_ASSUME(n < arraysize(simd128_registers_));
    return base::ReadUnalignedValue<Float64>(reinterpret_cast<Address>(simd128_registers_ + n));
}

void RegisterValues::SetDoubleRegister(unsigned n, Float64 value)
{
    base::WriteUnalignedValue<Float64>(reinterpret_cast<Address>(simd128_registers_ + n), value);
}
