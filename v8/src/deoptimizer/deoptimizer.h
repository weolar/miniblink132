...
public:
    ...
    static void DeoptimizeFunction(Tagged<JSFunction> function, Tagged<Code> code = {});
    ...
    {
        return offsetof(Deoptimizer, input_);
    }
    ...
    // Tracing.
    static void TraceMarkForDeoptimization(Isolate* isolate, Tagged<Code> code, const char* reason);
...
private:
    ...
    FrameDescription* DoComputeWasmLiftoffFrame(TranslatedFrame& frame, wasm::NativeModule* native_module,
        Tagged<WasmTrustedInstanceData> wasm_trusted_instance, int frame_index, std::stack<intptr_t>& shadow_stack);

    void GetWasmStackSlotsCounts(const wasm::FunctionSig* sig, int* parameter_stack_slots, int* return_stack_slots);
#endif
...
#if V8_ENABLE_WEBASSEMBLY
    TranslatedValue TranslatedValueForWasmReturnKind(std::optional<wasm::ValueKind> wasm_call_return_kind);
#endif // V8_ENABLE_WEBASSEMBLY
    ...
    // stored off-heap.
    std::optional<SandboxHardwareSupport::BlockAccessScope> no_heap_access_during_wasm_deopt_;
#endif
