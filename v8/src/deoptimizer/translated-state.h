...
private:
    ...
    void mark_allocated()
    {
        materialization_state_ = kAllocated;
    }
...
public:
    ...
    private:
        ...
        explicit iterator(std::deque<TranslatedValue>::iterator position, int input_index = 0)
            : position_(position)
...
private:
    ...
    static TranslatedFrame UnoptimizedJSFrame(BytecodeOffset bytecode_offset, Tagged<SharedFunctionInfo> shared_info, Tagged<BytecodeArray> bytecode_array,
    ...
    static TranslatedFrame InlinedExtraArguments(Tagged<SharedFunctionInfo> shared_info, int height);
    static TranslatedFrame ConstructCreateStubFrame(Tagged<SharedFunctionInfo> shared_info, int height);
    static TranslatedFrame ConstructInvokeStubFrame(Tagged<SharedFunctionInfo> shared_info);
    static TranslatedFrame BuiltinContinuationFrame(BytecodeOffset bailout_id, Tagged<SharedFunctionInfo> shared_info, int height);
#if V8_ENABLE_WEBASSEMBLY
    static TranslatedFrame WasmInlinedIntoJSFrame(BytecodeOffset bailout_id, Tagged<SharedFunctionInfo> shared_info, int height);
    static TranslatedFrame JSToWasmBuiltinContinuationFrame(
        BytecodeOffset bailout_id, Tagged<SharedFunctionInfo> shared_info, int height, std::optional<wasm::ValueKind> return_type);
    static TranslatedFrame LiftoffFrame(BytecodeOffset bailout_id, int height, int function_index);
#endif // V8_ENABLE_WEBASSEMBLY
    static TranslatedFrame JavaScriptBuiltinContinuationFrame(BytecodeOffset bailout_id, Tagged<SharedFunctionInfo> shared_info, int height);
    static TranslatedFrame JavaScriptBuiltinContinuationWithCatchFrame(BytecodeOffset bailout_id, Tagged<SharedFunctionInfo> shared_info, int height);
    ...
    explicit TranslatedFrame(Kind kind, Tagged<SharedFunctionInfo> raw_shared_info = {}, Tagged<BytecodeArray> raw_bytecode_array = {}, int height = 0,
        int return_value_offset = 0, int return_value_count = 0)
        : kind_(kind)
...
public:
    explicit DeoptimizationLiteralProvider(Tagged<DeoptimizationLiteralArray> literal_array);

    explicit DeoptimizationLiteralProvider(std::vector<DeoptimizationLiteral> literals);
...
public:
    ...
    void Init(Isolate* isolate, Address input_frame_pointer, Address stack_frame_pointer, DeoptTranslationIterator* iterator,
...
private:
    ...
    TranslatedFrame CreateNextTranslatedFrame(DeoptTranslationIterator* iterator, Tagged<ProtectedDeoptimizationLiteralArray> protected_literal_array,
        const DeoptimizationLiteralProvider& literal_array, Address fp, FILE* trace_file);
    int CreateNextTranslatedValue(int frame_index, DeoptTranslationIterator* iterator, const DeoptimizationLiteralProvider& literal_array, Address fp,
