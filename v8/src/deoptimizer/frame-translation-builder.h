...
public:
    ...
    Handle<DeoptimizationFrameTranslation> ToFrameTranslation(LocalFactory* factory);
...
#if V8_ENABLE_WEBASSEMBLY
    void BeginJSToWasmBuiltinContinuationFrame(BytecodeOffset bailout_id, int literal_id, unsigned height, std::optional<wasm::ValueKind> return_kind);
    void BeginWasmInlinedIntoJSFrame(BytecodeOffset bailout_id, int literal_id, unsigned height);
    void BeginLiftoffFrame(BytecodeOffset bailout_id, unsigned height, uint32_t wasm_function_index);
#endif // V8_ENABLE_WEBASSEMBLY
...
private:
    ...
    template <typename... T> void Add(TranslationOpcode opcode, T... operands);
    ...
    template <typename... T> void AddRawToContents(TranslationOpcode opcode, T... operands);
    ...
    template <typename... T> void AddRawToContentsForCompression(TranslationOpcode opcode, T... operands);
    ...
    template <typename... T> void AddRawBegin(bool update_feedback, T... operands);

    int Size() const
    {
        return V8_UNLIKELY(v8_flags.turbo_compress_frame_translations) ? static_cast<int>(contents_for_compression_.size())
                                                                       : static_cast<int>(contents_.size());
    }
    int SizeInBytes() const
    {
        return V8_UNLIKELY(v8_flags.turbo_compress_frame_translations) ? Size() * kInt32Size : Size();
    }
