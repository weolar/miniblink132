...
{
    V8_ASSUME(n < arraysize(simd128_registers_));
    base::WriteUnalignedValue(reinterpret_cast<Address>(simd128_registers_ + n), value);
}

void FrameDescription::SetCallerPc(unsigned offset, intptr_t value)
{
    Address new_context = static_cast<Address>(GetTop()) + offset + kPCOnStackSize;
...
void FrameDescription::SetPc(intptr_t pc)
{
    ...
    if (ENABLE_CONTROL_FLOW_INTEGRITY_BOOL) {
        Deoptimizer::EnsureValidReturnAddress(isolate_, PointerAuthentication::StripPAC(pc));
    }
    pc_ = pc;
}
