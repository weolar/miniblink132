 ...
 */
template <typename Entry, size_t size> class V8_EXPORT_PRIVATE ExternalEntityTable : public SegmentedTable<Entry, size> {
protected:
    ...
    template <typename Callback> uint32_t GenericSweep(Space* space, Callback marked);
    ...
    template <typename Callback> void IterateEntriesIn(Space* space, Callback callback);
    ...
    // to recognize.
    static constexpr FreelistHead kEntryAllocationIsForbiddenMarker = FreelistHead(-1, -1);
