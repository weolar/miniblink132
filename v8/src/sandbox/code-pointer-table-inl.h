...
void CodePointerTableEntry::MakeCodePointerEntry(Address code, Address entrypoint, CodeEntrypointTag tag, bool mark_as_alive)
{
...
void CodePointerTableEntry::SetEntrypoint(Address value, CodeEntrypointTag tag)
{
    DCHECK(!IsFreelistEntry());
...
void CodePointerTableEntry::Mark()
{
    ...
    // the marking bit.
    bool success = code_.compare_exchange_strong(old_value, new_value, std::memory_order_relaxed);
...
Address CodePointerTable::GetEntrypoint(CodePointerHandle handle, CodeEntrypointTag tag) const
{
...
void CodePointerTable::SetEntrypoint(CodePointerHandle handle, Address value, CodeEntrypointTag tag)
{
...
CodePointerHandle CodePointerTable::AllocateAndInitializeEntry(Space* space, Address code, Address entrypoint, CodeEntrypointTag tag)
{
    DCHECK(space->BelongsTo(this));
    uint32_t index = AllocateEntry(space);
    CFIMetadataWriteScope write_scope("CodePointerTable write");
    at(index).MakeCodePointerEntry(code, entrypoint, tag, space->allocate_black());
...
template <typename Callback> void CodePointerTable::IterateActiveEntriesIn(Space* space, Callback callback)
{
...
uint32_t CodePointerTable::HandleToIndex(CodePointerHandle handle) const
{
    ...
    DCHECK_EQ(handle, (index << kCodePointerHandleShift) | kCodePointerHandleMarker);
    return index;
}
