...
private:
    ...
    struct Payload {
        ...
        {
            return (pointer << kCppHeapPointerPayloadShift) | (static_cast<uint16_t>(tag) << kCppHeapPointerTagShift);
        }
        ...
        void SetMarkBit()
        {
            encoded_word_ |= kCppHeapPointerMarkBit;
        }
        ...
        {
            return static_cast<uint32_t>(encoded_word_ >> kCppHeapPointerPayloadShift);
        }
 ...
 *
 */
class V8_EXPORT_PRIVATE CppHeapPointerTable : public CompactibleExternalEntityTable<CppHeapPointerTableEntry, kCppHeapPointerTableReservationSize> {
    using Base = CompactibleExternalEntityTable<CppHeapPointerTableEntry, kCppHeapPointerTableReservationSize>;
...
public:
    ...
    inline Address Get(CppHeapPointerHandle handle, CppHeapPointerTagRange tag_range) const;

    // Sets the entry referenced by the given handle.
    //
    // This method is atomic and can be called from background threads.
    inline void Set(CppHeapPointerHandle handle, Address value, CppHeapPointerTag tag);

    // Allocates a new entry in the given space. The caller must provide the
    // initial value and tag for the entry.
    //
    // This method is atomic and can be called from background threads.
    inline CppHeapPointerHandle AllocateAndInitializeEntry(Space* space, Address initial_value, CppHeapPointerTag tag);
    ...
    //
    // This method is atomic and can be called from background threads.
    inline void Mark(Space* space, CppHeapPointerHandle handle, Address handle_location);
...
private:
    ...
    void ResolveEvacuationEntryDuringSweeping(uint32_t index, CppHeapPointerHandle* handle_location, uint32_t start_of_evacuation_area);
};
