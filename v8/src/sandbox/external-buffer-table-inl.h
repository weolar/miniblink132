...
void ExternalBufferTableEntry::MakeExternalBufferEntry(std::pair<Address, size_t> buffer, ExternalBufferTag tag)
{
...
std::pair<Address, size_t> ExternalBufferTableEntry::GetExternalBuffer(ExternalBufferTag tag) const
{
...
std::pair<Address, size_t> ExternalBufferTable::Get(ExternalBufferHandle handle, ExternalBufferTag tag) const
{
...
ExternalBufferHandle ExternalBufferTable::AllocateAndInitializeEntry(Space* space, std::pair<Address, size_t> initial_buffer, ExternalBufferTag tag)
{
...
void ExternalBufferTable::Mark(Space* space, ExternalBufferHandle handle, Address handle_location)
{
    DCHECK(space->BelongsTo(this));

    // The null entry is immortal and immutable, so no need to mark it as alive.
    if (handle == kNullExternalBufferHandle)
        return;

    // The handle_location must always contain the given handle.
    DCHECK_EQ(handle, base::AsAtomic32::Acquire_Load(reinterpret_cast<ExternalBufferHandle*>(handle_location)));
