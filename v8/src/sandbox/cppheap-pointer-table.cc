...
uint32_t CppHeapPointerTable::SweepAndCompact(Space* space, Counters* counters)
{
    ...
    for (auto segment : base::Reversed(space->segments_)) {
        ...
        for (uint32_t i = segment.last_entry(); i >= segment.first_entry(); i--) {
            ...
            if (payload.ContainsEvacuationEntry()) {
                ...
                ResolveEvacuationEntryDuringSweeping(i, reinterpret_cast<CppHeapPointerHandle*>(handle_location), start_of_evacuation_area);
...
void CppHeapPointerTable::ResolveEvacuationEntryDuringSweeping(uint32_t new_index, CppHeapPointerHandle* handle_location, uint32_t start_of_evacuation_area)
{
