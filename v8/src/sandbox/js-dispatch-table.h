...
struct JSDispatchEntry {
    ...
    inline void MakeJSDispatchEntry(Address object, Address entrypoint, uint16_t parameter_count, bool mark_as_alive);
    ...
    inline void SetCodeAndEntrypointPointer(Address new_object, Address new_entrypoint);
 ...
 */
class V8_EXPORT_PRIVATE JSDispatchTable : public ExternalEntityTable<JSDispatchEntry, kJSDispatchTableReservationSize> {
    using Base = ExternalEntityTable<JSDispatchEntry, kJSDispatchTableReservationSize>;
...
public:
    ...
    inline void SetCodeNoWriteBarrier(JSDispatchHandle handle, Tagged<Code> new_code);

    // Allocates a new entry in the table and initialize it.
    //
    // This method is atomic and can be called from background threads.
    inline JSDispatchHandle AllocateAndInitializeEntry(Space* space, uint16_t parameter_count);
    inline JSDispatchHandle AllocateAndInitializeEntry(Space* space, uint16_t parameter_count, Tagged<Code> code);

    // The following methods are used to pre allocate entries and then initialize
    // them later.
    JSDispatchHandle PreAllocateEntries(Space* space, int num, bool ensure_static_handles);
    bool PreAllocatedEntryNeedsInitialization(Space* space, JSDispatchHandle handle);
    void InitializePreAllocatedEntry(Space* space, JSDispatchHandle handle, Tagged<Code> code, uint16_t parameter_count);
    ...
    {
        return static_cast<JSDispatchHandle>(kInternalNullEntryIndex + 1 + index) << kJSDispatchHandleShift;
    }
    ...
    template <typename Callback> uint32_t Sweep(Space* space, Counters* counters, Callback callback);
    ...
    template <typename Callback> void IterateActiveEntriesIn(Space* space, Callback callback);

    template <typename Callback> void IterateMarkedEntriesIn(Space* space, Callback callback);
...
private:
    ...
    static inline bool IsCompatibleCode(Tagged<Code> code, uint16_t parameter_count);
