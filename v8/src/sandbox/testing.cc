...
void SandboxMemoryView(const v8::FunctionCallbackInfo<v8::Value>& info)
{
    ...
    if (!info[0]->ToInteger(context).ToLocal(&arg1) || !info[1]->ToInteger(context).ToLocal(&arg2)) {
    ...
    if (offset > sandbox->size() || size > sandbox->size() || (offset + size) > sandbox->size()) {
        isolate->ThrowError("The MemoryView must be entirely contained within the sandbox");
        return;
    }

    Factory* factory = reinterpret_cast<Isolate*>(isolate)->factory();
    std::unique_ptr<BackingStore> memory = BackingStore::WrapAllocation(
        reinterpret_cast<void*>(sandbox->base() + offset), size, v8::BackingStore::EmptyDeleter, nullptr, SharedFlag::kNotShared);
    if (!memory) {
...
// in both cases.
using ArgumentObjectExtractorFunction = std::function<bool(const v8::FunctionCallbackInfo<v8::Value>&, Tagged<HeapObject>* out)>;

static bool GetArgumentObjectPassedAsReference(const v8::FunctionCallbackInfo<v8::Value>& info, Tagged<HeapObject>* out)
{
    v8::Isolate* isolate = info.GetIsolate();
...
static bool GetArgumentObjectPassedAsAddress(const v8::FunctionCallbackInfo<v8::Value>& info, Tagged<HeapObject>* out)
{
    Sandbox* sandbox = GetProcessWideSandbox();
    v8::Isolate* isolate = info.GetIsolate();
    Local<v8::Context> context = isolate->GetCurrentContext();
...
static void SandboxIsWritableImpl(const v8::FunctionCallbackInfo<v8::Value>& info, ArgumentObjectExtractorFunction getArgumentObject)
{
    DCHECK(ValidateCallbackInfo(info));
...
void SandboxIsWritableObjectAt(const v8::FunctionCallbackInfo<v8::Value>& info)
{
    SandboxIsWritableImpl(info, &GetArgumentObjectPassedAsAddress);
}

static void SandboxGetSizeOfImpl(const v8::FunctionCallbackInfo<v8::Value>& info, ArgumentObjectExtractorFunction getArgumentObject)
{
    DCHECK(ValidateCallbackInfo(info));
...
static void SandboxGetInstanceTypeOfImpl(const v8::FunctionCallbackInfo<v8::Value>& info, ArgumentObjectExtractorFunction getArgumentObject)
{
    DCHECK(ValidateCallbackInfo(info));
    v8::Isolate* isolate = info.GetIsolate();
    ...
    out << type;
    MaybeLocal<v8::String> result = v8::String::NewFromUtf8(isolate, out.str().c_str());
...
void SandboxGetInstanceTypeOfObjectAt(const v8::FunctionCallbackInfo<v8::Value>& info)
{
    SandboxGetInstanceTypeOfImpl(info, &GetArgumentObjectPassedAsAddress);
}

static void SandboxGetInstanceTypeIdOfImpl(const v8::FunctionCallbackInfo<v8::Value>& info, ArgumentObjectExtractorFunction getArgumentObject)
{
    DCHECK(ValidateCallbackInfo(info));
...
void SandboxGetInstanceTypeIdOf(const v8::FunctionCallbackInfo<v8::Value>& info)
{
    SandboxGetInstanceTypeIdOfImpl(info, &GetArgumentObjectPassedAsReference);
}

// Sandbox.getInstanceTypeIdOfObjectAt(Number) -> Number
void SandboxGetInstanceTypeIdOfObjectAt(const v8::FunctionCallbackInfo<v8::Value>& info)
{
    SandboxGetInstanceTypeIdOfImpl(info, &GetArgumentObjectPassedAsAddress);
}

// Sandbox.getInstanceTypeIdFor(String) -> Number
void SandboxGetInstanceTypeIdFor(const v8::FunctionCallbackInfo<v8::Value>& info)
{
    DCHECK(ValidateCallbackInfo(info));
    v8::Isolate* isolate = info.GetIsolate();
    ...
    if (all_types.find(*type_name) == all_types.end()) {
        isolate->ThrowError("Unknown type name. If needed, add it in "
...
void SandboxGetFieldOffset(const v8::FunctionCallbackInfo<v8::Value>& info)
{
    ...
    if (all_fields.find(instance_type) == all_fields.end()) {
        isolate->ThrowError("Unknown object type. If needed, add it in "
    ...
    if (obj_fields.find(*field_name) == obj_fields.end()) {
        isolate->ThrowError("Unknown field. If needed, add it in "
                            "SandboxTesting::GetFieldOffsetMap");
        return;
    }
...
Handle<FunctionTemplateInfo> NewFunctionTemplate(Isolate* isolate, FunctionCallback func, ConstructorBehavior constructor_behavior)
{
    // Use the API functions here as they are more convenient to use.
    v8::Isolate* api_isolate = reinterpret_cast<v8::Isolate*>(isolate);
    Local<FunctionTemplate> function_template = FunctionTemplate::New(api_isolate, func, {}, {}, 0, constructor_behavior, SideEffectType::kHasSideEffect);
    return v8::Utils::OpenHandle(*function_template);
}

Handle<JSFunction> CreateFunc(Isolate* isolate, FunctionCallback func, Handle<String> name, bool is_constructor)
{
    ConstructorBehavior constructor_behavior = is_constructor ? ConstructorBehavior::kAllow : ConstructorBehavior::kThrow;
    Handle<FunctionTemplateInfo> function_template = NewFunctionTemplate(isolate, func, constructor_behavior);
    return ApiNatives::InstantiateFunction(isolate, function_template, name).ToHandleChecked();
}

void InstallFunc(Isolate* isolate, Handle<JSObject> holder, FunctionCallback func, const char* name, int num_parameters, bool is_constructor)
{
    Factory* factory = isolate->factory();
    Handle<String> function_name = factory->NewStringFromAsciiChecked(name);
    Handle<JSFunction> function = CreateFunc(isolate, func, function_name, is_constructor);
    function->shared()->set_length(num_parameters);
    JSObject::AddProperty(isolate, holder, function_name, function, NONE);
}

void InstallGetter(Isolate* isolate, Handle<JSObject> object, FunctionCallback func, const char* name)
{
    Factory* factory = isolate->factory();
    ...
    JSObject::DefineOwnAccessorIgnoreAttributes(object, property_name, getter, setter, FROZEN);
}

void InstallFunction(Isolate* isolate, Handle<JSObject> holder, FunctionCallback func, const char* name, int num_parameters)
{
    InstallFunc(isolate, holder, func, name, num_parameters, false);
}

void InstallConstructor(Isolate* isolate, Handle<JSObject> holder, FunctionCallback func, const char* name, int num_parameters)
{
...
#error "This function should not be available in any shipping build "          \
    ...
    Handle<JSObject> sandbox = isolate->factory()->NewJSObject(isolate->object_function(), AllocationType::kOld);
    ...
    InstallFunction(isolate, sandbox, SandboxIsValidObjectAt, "isValidObjectAt", 1);
    InstallFunction(isolate, sandbox, SandboxIsWritable, "isWritable", 1);
    InstallFunction(isolate, sandbox, SandboxIsWritableObjectAt, "isWritableObjectAt", 1);
    InstallFunction(isolate, sandbox, SandboxGetSizeOf, "getSizeOf", 1);
    InstallFunction(isolate, sandbox, SandboxGetSizeOfObjectAt, "getSizeOfObjectAt", 1);
    InstallFunction(isolate, sandbox, SandboxGetInstanceTypeOf, "getInstanceTypeOf", 1);
    InstallFunction(isolate, sandbox, SandboxGetInstanceTypeOfObjectAt, "getInstanceTypeOfObjectAt", 1);
    InstallFunction(isolate, sandbox, SandboxGetInstanceTypeIdOf, "getInstanceTypeIdOf", 1);
    InstallFunction(isolate, sandbox, SandboxGetInstanceTypeIdOfObjectAt, "getInstanceTypeIdOfObjectAt", 1);
    InstallFunction(isolate, sandbox, SandboxGetInstanceTypeIdFor, "getInstanceTypeIdFor", 1);
    ...
    Handle<String> name = isolate->factory()->NewStringFromAsciiChecked("Sandbox");
...
[[noreturn]] void FilterCrash(const char* reason)
{
    ...
    int status = SandboxTesting::mode() == SandboxTesting::Mode::kForFuzzing ? -1 : 0;
    _exit(status);
}
...
struct sigaction g_old_sigabrt_handler, g_old_sigtrap_handler, g_old_sigbus_handler, g_old_sigsegv_handler;
...
void CrashFilter(int signal, siginfo_t* info, void* void_context)
{
    ...
    if (GetProcessWideSandbox()->Contains(faultaddr)) {
        FilterCrash("Caught harmless memory access violaton (inside sandbox address "
    ...
    if (info->si_code == SI_KERNEL && faultaddr == 0) {
        ...
        FilterCrash("Caught harmless memory access violaton (non-canonical address). "
                    "Exiting process...\n");
    }
    ...
    if (faultaddr >= 0x8000'0000'0000'0000ULL) {
        ...
        FilterCrash("Caught harmless memory access violatation (kernel space address). "
                    "Exiting process...\n");
    }
    ...
    if (faultaddr < 0x1000) {
        ...
        FilterCrash("Caught harmless memory access violaton (nullptr dereference). Exiting "
                    "process...\n");
    }
    ...
    if (faultaddr < 4ULL * GB) {
        ...
        FilterCrash("Caught harmless memory access violaton (first 4GB of virtual address "
                    "space). Exiting process...\n");
    }
    ...
    if (!pthread_error) {
        ...
        size_t stack_size;
        pthread_error = pthread_attr_getstack(&attr, reinterpret_cast<void**>(&stack_base), &stack_size);
        ...
        if (!pthread_error && stack_guard_region_start <= faultaddr && faultaddr < stack_guard_region_end) {
            FilterCrash("Caught harmless stack overflow. Exiting process...\n");
        }
    }

    if (info->si_code == SEGV_ACCERR && !SandboxTesting::IsInsideTargetPage(faultaddr)) {
        ...
        FilterCrash("Caught harmless memory access violaton (memory permission violation). "
                    "Exiting process...\n");
    }
    ...
    if (SandboxTesting::mode() == SandboxTesting::Mode::kForTesting) {
        ...
        if (!SandboxTesting::IsInsideTargetPage(faultaddr)) {
            FilterCrash("Detected invalid sandbox violation (not on target page). Exiting "
                        "process...\n");
        }
    }
...
void AsanFaultHandler()
{
    ...
    if (faultaddr == kNullAddress) {
        FilterCrash("Caught ASan fault without a fault address. Ignoring it as we cannot "
                    "check if it is a sandbox violation. Exiting process...\n");
    }

    if (GetProcessWideSandbox()->Contains(faultaddr)) {
        FilterCrash("Caught harmless ASan fault (inside sandbox address space). Exiting "
                    "process...\n");
    }
...
void InstallCrashFilter()
{
    ...
    Address alternate_stack = vas->AllocatePages(VirtualAddressSpace::kNoHint, SIGSTKSZ, vas->page_size(), PagePermissions::kReadWrite);
...
#elif defined(V8_USE_MEMORY_SANITIZER) || defined(V8_USE_UNDEFINED_BEHAVIOR_SANITIZER)
...
void SandboxTesting::Enable(Mode mode)
{
    ...
    if (mode == Mode::kForTesting) {
        ...
        FATAL("The sandbox testing mode is currently incompatible with "
              "AddressSanitizer");
#else
        ...
        target_page_base_ = vas->AllocatePages(vas->RandomPageAddress(), target_page_size_, target_page_size_, PagePermissions::kRead);
        CHECK_NE(target_page_base_, kNullAddress);
        fprintf(stderr,
            "Sandbox testing mode is enabled. Write to the page starting at "
            "0x%" V8PRIxPTR " (available from JavaScript as `Sandbox.targetPage`) to "
...
{
    return base::IsInHalfOpenRange(faultaddr, target_page_base(), target_page_base() + target_page_size());
}
...
SandboxTesting::InstanceTypeMap& SandboxTesting::GetInstanceTypeMap()
{
    ...
    if (!is_initialized) {
        ...
        types["INTERNALIZED_ONE_BYTE_STRING_TYPE"] = INTERNALIZED_ONE_BYTE_STRING_TYPE;
...
SandboxTesting::FieldOffsetMap& SandboxTesting::GetFieldOffsetMap()
{
    ...
    if (!is_initialized) {
        ...
        fields[JS_FUNCTION_TYPE]["dispatch_handle"] = JSFunction::kDispatchHandleOffset;
#endif // V8_ENABLE_LEAPTIERING
        fields[JS_FUNCTION_TYPE]["shared_function_info"] = JSFunction::kSharedFunctionInfoOffset;
        fields[JS_ARRAY_TYPE]["length"] = JSArray::kLengthOffset;
        fields[SEQ_ONE_BYTE_STRING_TYPE]["length"] = offsetof(SeqOneByteString, length_);
        fields[INTERNALIZED_ONE_BYTE_STRING_TYPE]["length"] = offsetof(InternalizedString, length_);
        fields[SLICED_ONE_BYTE_STRING_TYPE]["parent"] = offsetof(SlicedString, parent_);
        ...
        fields[SHARED_FUNCTION_INFO_TYPE]["trusted_function_data"] = SharedFunctionInfo::kTrustedFunctionDataOffset;
        fields[SHARED_FUNCTION_INFO_TYPE]["length"] = SharedFunctionInfo::kLengthOffset;
        fields[SHARED_FUNCTION_INFO_TYPE]["formal_parameter_count"] = SharedFunctionInfo::kFormalParameterCountOffset;
        fields[SCRIPT_TYPE]["wasm_managed_native_module"] = Script::kEvalFromPositionOffset;
#ifdef V8_ENABLE_WEBASSEMBLY
        fields[WASM_MODULE_OBJECT_TYPE]["managed_native_module"] = WasmModuleObject::kManagedNativeModuleOffset;
        fields[WASM_MODULE_OBJECT_TYPE]["script"] = WasmModuleObject::kScriptOffset;
        fields[WASM_INSTANCE_OBJECT_TYPE]["module_object"] = WasmInstanceObject::kModuleObjectOffset;
        fields[WASM_FUNC_REF_TYPE]["trusted_internal"] = WasmFuncRef::kTrustedInternalOffset;
        fields[WASM_TABLE_OBJECT_TYPE]["entries"] = WasmTableObject::kEntriesOffset;
        fields[WASM_TABLE_OBJECT_TYPE]["current_length"] = WasmTableObject::kCurrentLengthOffset;
        fields[WASM_TABLE_OBJECT_TYPE]["maximum_length"] = WasmTableObject::kMaximumLengthOffset;
        fields[WASM_TABLE_OBJECT_TYPE]["raw_type"] = WasmTableObject::kRawTypeOffset;
#endif // V8_ENABLE_WEBASSEMBLY
    }
    return fields;
}
