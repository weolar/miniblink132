...
uint32_t ExternalBufferTable::SweepAndCompact(Space* space, Counters* counters)
{
    ...
    for (auto segment : base::Reversed(space->segments_)) {
        bool segment_will_be_evacuated = evacuation_was_successful && segment.first_entry() >= start_of_evacuation_area;
        ...
        for (uint32_t i = segment.last_entry(); i >= segment.first_entry(); i--) {
            ...
            if (payload.ContainsEvacuationEntry()) {
                ...
                if (!space->FieldWasInvalidated(handle_location)) {
                    entry_was_resolved
                        = TryResolveEvacuationEntryDuringSweeping(i, reinterpret_cast<ExternalBufferHandle*>(handle_location), start_of_evacuation_area);
                }
...
bool ExternalBufferTable::TryResolveEvacuationEntryDuringSweeping(uint32_t new_index, ExternalBufferHandle* handle_location, uint32_t start_of_evacuation_area)
{
