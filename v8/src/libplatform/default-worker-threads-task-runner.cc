...
DefaultWorkerThreadsTaskRunner::DefaultWorkerThreadsTaskRunner(uint32_t thread_pool_size, TimeFunction time_function, base::Thread::Priority priority)
...
void DefaultWorkerThreadsTaskRunner::PostTaskImpl(std::unique_ptr<Task> task, const SourceLocation& location)
{
    base::MutexGuard guard(&lock_);
    if (terminated_)
        return;
...
void DefaultWorkerThreadsTaskRunner::PostDelayedTaskImpl(std::unique_ptr<Task> task, double delay_in_seconds, const SourceLocation& location)
{
    base::MutexGuard guard(&lock_);
    if (terminated_)
        return;
...
void DefaultWorkerThreadsTaskRunner::PostIdleTaskImpl(std::unique_ptr<IdleTask> task, const SourceLocation& location)
{
...
DefaultWorkerThreadsTaskRunner::WorkerThread::WorkerThread(DefaultWorkerThreadsTaskRunner* runner, base::Thread::Priority priority)
...
void DefaultWorkerThreadsTaskRunner::WorkerThread::Run()
{
    ...
    while (true) {
        ...
        case DelayedTaskQueue::MaybeNextTask::kWaitDelayed:
            ...
            // uses.
            runner_->idle_threads_.push_back(this);
            bool notified = condition_var_.WaitFor(&runner_->lock_, next_task.wait_time);
            USE(notified);
            continue;
        }
    }
