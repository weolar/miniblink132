...
Tagged<JSReceiver> GetCompatibleReceiver(Isolate* isolate, Tagged<FunctionTemplateInfo> info, Tagged<JSReceiver> receiver)
{
    RCS_SCOPE(isolate, RuntimeCallCounterId::kGetCompatibleReceiver);
    Tagged<Object> recv_type = info->signature();
    // No signature, return holder.
    if (!IsFunctionTemplateInfo(recv_type))
        return receiver;
    ...
    Tagged<FunctionTemplateInfo> signature = Cast<FunctionTemplateInfo>(recv_type);
    ...
    if (V8_UNLIKELY(IsJSGlobalProxy(js_obj_receiver))) {
        ...
        if (!IsNull(prototype, isolate)) {
            ...
            if (signature->IsTemplateFor(js_obj_prototype))
                return js_obj_prototype;
        }
    }
    return JSReceiver();
}
...
V8_WARN_UNUSED_RESULT MaybeHandle<Object> HandleApiCallHelper(
    Isolate* isolate, Handle<HeapObject> new_target, DirectHandle<FunctionTemplateInfo> fun_data, Handle<Object> receiver, Address* argv, int argc)
{
    ...
    if (is_construct) {
        ...
        if (IsUndefined(fun_data->GetInstanceTemplate(), isolate)) {
            v8::Local<ObjectTemplate> templ = ObjectTemplate::New(reinterpret_cast<v8::Isolate*>(isolate), ToApiHandle<v8::FunctionTemplate>(fun_data));
            FunctionTemplateInfo::SetInstanceTemplate(isolate, fun_data, Utils::OpenHandle(*templ));
        }
        Handle<ObjectTemplateInfo> instance_template(Cast<ObjectTemplateInfo>(fun_data->GetInstanceTemplate()), isolate);
        ASSIGN_RETURN_ON_EXCEPTION(isolate, js_receiver, ApiNatives::InstantiateObject(isolate, instance_template, Cast<JSReceiver>(new_target)));
        ...
        if (!fun_data->accept_any_receiver() && IsAccessCheckNeeded(*js_receiver)) {
            ...
            if (!isolate->MayAccess(isolate->native_context(), js_object)) {
                RETURN_ON_EXCEPTION(isolate, isolate->ReportFailedAccessCheck(js_object));
                UNREACHABLE();
            }
        }
    ...
    if (fun_data->has_callback(isolate)) {
        FunctionCallbackArguments custom(isolate, *fun_data, raw_holder, *new_target, argv, argc);
...
BUILTIN(HandleApiConstruct)
{
    ...
    DirectHandle<FunctionTemplateInfo> fun_data(args.target()->shared()->api_func_data(), isolate);
    int argc = args.length() - 1;
    Address* argv = args.address_of_first_argument();
    RETURN_RESULT_OR_FAILURE(isolate, HandleApiCallHelper<true>(isolate, new_target, fun_data, receiver, argv, argc));
}
...
public:
    ...
    {
        v->VisitRootPointers(Root::kRelocatable, nullptr, FullObjectSlot(&arguments_[0]), FullObjectSlot(&arguments_[length_]));
    }
...
MaybeHandle<Object> Builtins::InvokeApiFunction(Isolate* isolate, bool is_construct, Handle<FunctionTemplateInfo> function, Handle<Object> receiver, int argc,
    Handle<Object> args[], Handle<HeapObject> new_target)
{
    ...
    if (is_construct) {
        return HandleApiCallHelper<true>(isolate, new_target, function, receiver, argv.data() + 1, argc);
    }
    return HandleApiCallHelper<false>(isolate, new_target, function, receiver, argv.data() + 1, argc);
}

// Helper function to handle calls to non-function objects created through the
// API. The object can be called as either a constructor (using new) or just as
// a function (without new).
V8_WARN_UNUSED_RESULT static Tagged<Object> HandleApiCallAsFunctionOrConstructorDelegate(Isolate* isolate, bool is_construct_call, BuiltinArguments args)
{
    ...
    Tagged<JSFunction> constructor = Cast<JSFunction>(obj->map()->GetConstructor());
    DCHECK(constructor->shared()->IsApiFunction());
    Tagged<Object> handler = constructor->shared()->api_func_data()->GetInstanceCallHandler();
    ...
    {
        HandleScope scope(isolate);
        FunctionCallbackArguments custom(isolate, templ, obj, new_target, args.address_of_first_argument(), args.length() - 1);
        Handle<Object> result_handle = custom.CallOrConstruct(templ, is_construct_call);
...
{
    isolate->CountUsage(v8::Isolate::UseCounterFeature::kDocumentAllLegacyConstruct);
