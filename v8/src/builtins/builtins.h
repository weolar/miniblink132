...
template <typename T> static constexpr T FirstFromVarArgs(T x, ...) noexcept
{
    return x;
}
...
#define DEF_ENUM(Name, ...) k##Name,
    BUILTIN_LIST(DEF_ENUM, DEF_ENUM, DEF_ENUM, DEF_ENUM, DEF_ENUM, DEF_ENUM, DEF_ENUM, DEF_ENUM, DEF_ENUM)
#undef DEF_ENUM
...
#define ADD_ONE(Name, ...) +1
    static constexpr int kBuiltinCount = 0 BUILTIN_LIST(ADD_ONE, ADD_ONE, ADD_ONE, ADD_ONE, ADD_ONE, ADD_ONE, ADD_ONE, ADD_ONE, ADD_ONE);
    static constexpr int kBuiltinTier0Count = 0 BUILTIN_LIST_TIER0(ADD_ONE, ADD_ONE, ADD_ONE, ADD_ONE, ADD_ONE, ADD_ONE, ADD_ONE);
#undef ADD_ONE

    static constexpr Builtin kFirst = static_cast<Builtin>(0);
    static constexpr Builtin kLast = static_cast<Builtin>(kBuiltinCount - 1);
    static constexpr Builtin kLastTier0 = static_cast<Builtin>(kBuiltinTier0Count - 1);

    static constexpr int kFirstWideBytecodeHandler = static_cast<int>(Builtin::kFirstBytecodeHandler) + kNumberOfBytecodeHandlers;
    static constexpr int kFirstExtraWideBytecodeHandler = kFirstWideBytecodeHandler + kNumberOfWideBytecodeHandlers;
    static constexpr int kLastBytecodeHandlerPlusOne = kFirstExtraWideBytecodeHandler + kNumberOfWideBytecodeHandlers;
    static constexpr bool kBytecodeHandlersAreSortedLast = kLastBytecodeHandlerPlusOne == kBuiltinCount;
    ...
    static constexpr Builtin kWasmIndirectlyCallableBuiltins[] = { Builtin::kWasmToJsWrapperInvalidSig, Builtin::kWasmToJsWrapperAsm };
    static constexpr size_t kNumWasmIndirectlyCallableBuiltins = arraysize(kWasmIndirectlyCallableBuiltins);
    using WasmBuiltinHandleArray = wasm::WasmCodePointerTable::Handle[kNumWasmIndirectlyCallableBuiltins];
    // TODO(sroettger): this can be consteval, but the gcc bot doesn't support it.
    template <Builtin builtin> static constexpr size_t WasmBuiltinHandleArrayIndex();
#endif
    ...
    static constexpr bool IsBuiltinId(int maybe_id)
    {
        ...
        return static_cast<uint32_t>(maybe_id) < static_cast<uint32_t>(kBuiltinCount);
    }
    ...
    static inline constexpr Builtin IndirectPointerBarrier(SaveFPRegsMode fp_mode);
    static inline constexpr Builtin EphemeronKeyBarrier(SaveFPRegsMode fp_mode);

    static inline constexpr Builtin AdaptorWithBuiltinExitFrame(int formal_parameter_count);

    static inline constexpr Builtin CallFunction(ConvertReceiverMode = ConvertReceiverMode::kAny);
    static inline constexpr Builtin Call(ConvertReceiverMode = ConvertReceiverMode::kAny);
    // Whether the given builtin is one of the JS function call builtins.
    static inline constexpr bool IsAnyCall(Builtin builtin);

    static inline constexpr Builtin NonPrimitiveToPrimitive(ToPrimitiveHint hint = ToPrimitiveHint::kDefault);
    static inline constexpr Builtin OrdinaryToPrimitive(OrdinaryToPrimitiveHint hint);

    static inline constexpr Builtin StringAdd(StringAddFlags flags = STRING_ADD_CHECK_NONE);

    static inline constexpr Builtin LoadGlobalIC(TypeofMode typeof_mode);
    static inline constexpr Builtin LoadGlobalICInOptimizedCode(TypeofMode typeof_mode);

    static inline constexpr Builtin CEntry(int result_size, ArgvMode argv_mode, bool builtin_exit_frame = false, bool switch_to_central_stack = false);

    static inline constexpr Builtin RuntimeCEntry(int result_size, bool switch_to_central_stack = false);

    static inline constexpr Builtin InterpreterCEntry(int result_size);
    static inline constexpr Builtin InterpreterPushArgsThenCall(ConvertReceiverMode receiver_mode, InterpreterPushArgsMode mode);
    static inline constexpr Builtin InterpreterPushArgsThenConstruct(InterpreterPushArgsMode mode);
    ...
    V8_EXPORT_PRIVATE static Callable CallableFor(Isolate* isolate, Builtin builtin);
    ...
    V8_EXPORT_PRIVATE static const char* NameForStackTrace(Isolate* isolate, Builtin builtin);
    ...
    template <Builtin builtin> static inline wasm::WasmCodePointerTable::Handle WasmBuiltinHandleOf(Isolate* isolate);
#endif
    ...
    V8_WARN_UNUSED_RESULT static MaybeHandle<Object> InvokeApiFunction(Isolate* isolate, bool is_construct, Handle<FunctionTemplateInfo> function,
        Handle<Object> receiver, int argc, Handle<Object> args[], Handle<HeapObject> new_target);
    ...
    static bool AllowDynamicFunction(Isolate* isolate, DirectHandle<JSFunction> target, Handle<JSObject> target_global_proxy);

    // Creates a copy of InterpreterEntryTrampolineForProfiling in the code space.
    static Handle<Code> CreateInterpreterEntryTrampolineForProfiling(Isolate* isolate);
    ...
    void SetCWasmInterpreterEntryHandlerOffset(int offset)
    {
        ...
        // generate multiple variants of this builtin but they should all have the
        // same handler offset).
        CHECK(cwasm_interpreter_entry_handler_offset_ == 0 || cwasm_interpreter_entry_handler_offset_ == offset);
...
#define DECLARE_ASM(Name, ...) static void Generate_##Name(MacroAssembler* masm);
#define DECLARE_TF(Name, ...) static void Generate_##Name(compiler::CodeAssemblerState* state);
#define DECLARE_TS(Name, ...)                                                                                                                                  \
    static void Generate_##Name(compiler::turboshaft::PipelineData* data, Isolate* isolate, compiler::turboshaft::Graph& graph, Zone* zone);

    BUILTIN_LIST(IGNORE_BUILTIN, DECLARE_TS, DECLARE_TF, DECLARE_TS, DECLARE_TF, DECLARE_TF, DECLARE_TF, IGNORE_BUILTIN, DECLARE_ASM)
...
V8_INLINE constexpr bool IsInterpreterTrampolineBuiltin(Builtin builtin_id)
{
    ...
    return builtin_id != Builtin::kNoBuiltinId
        && (builtin_id == Builtin::kInterpreterEntryTrampoline || builtin_id == Builtin::kInterpreterEnterAtBytecode
...
V8_INLINE constexpr bool IsBaselineTrampolineBuiltin(Builtin builtin_id)
{
    ...
    // InstructionStream object is not a builtin.
    return builtin_id != Builtin::kNoBuiltinId
        && (builtin_id == Builtin::kBaselineOutOfLinePrologue || builtin_id == Builtin::kBaselineOutOfLinePrologueDeopt
            || builtin_id == Builtin::kBaselineOrInterpreterEnterAtBytecode || builtin_id == Builtin::kBaselineOrInterpreterEnterAtNextBytecode);
}
...
#define SIN_IMPL(X) v8_flags.use_libm_trig_functions ? base::ieee754::libm_sin(X) : base::ieee754::fdlibm_sin(X)
#define COS_IMPL(X) v8_flags.use_libm_trig_functions ? base::ieee754::libm_cos(X) : base::ieee754::fdlibm_cos(X)
#else
