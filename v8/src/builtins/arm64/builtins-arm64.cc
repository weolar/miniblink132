...
void Generate_JSBuiltinsConstructStubHelper(MacroAssembler* masm)
{
    ...
    // Enter a construct frame.
    {
        ...
        // Copy arguments to the expression stack.
        {
            ...
            __ Add(src, fp,
                StandardFrameConstants::kCallerSPOffset + kSystemPointerSize); // Skip receiver.
...
void Builtins::Generate_JSConstructStubGeneric(MacroAssembler* masm)
{
    ...
    __ JumpIfIsInRange(w4, static_cast<uint32_t>(FunctionKind::kDefaultDerivedConstructor), static_cast<uint32_t>(FunctionKind::kDerivedConstructor),
    ...
    // Copy the arguments.
    {
        ...
        __ Add(src, fp, StandardFrameConstants::kCallerSPOffset + kSystemPointerSize);
        __ CopyDoubleWords(dst, src, count);
    }
...
static void CheckSharedFunctionInfoBytecodeOrBaseline(MacroAssembler* masm, Register data, Register scratch, Label* is_baseline, Label* is_bytecode)
{
    ...
    __ CompareInstanceTypeWithUniqueCompressedMap(scratch, Register::no_reg(), INTERPRETER_DATA_TYPE);
#else
...
void Builtins::Generate_ResumeGeneratorTrampoline(MacroAssembler* masm)
{
    ...
    __ StoreTaggedField(x0, FieldMemOperand(x1, JSGeneratorObject::kInputOrDebugPosOffset));
    __ RecordWriteField(x1, JSGeneratorObject::kInputOrDebugPosOffset, x0, kLRHasNotBeenSaved, SaveFPRegsMode::kIgnore);
    // Check that x1 is still valid, RecordWrite might have clobbered it.
    __ AssertGeneratorObject(x1);

    // Load suspended function and context.
    __ LoadTaggedField(x5, FieldMemOperand(x1, JSGeneratorObject::kFunctionOffset));
    ...
    __ CompareAndBranch(x10, Operand(x1), eq, &prepare_step_in_suspended_generator);
    ...
    __ Ldr(dispatch_handle.W(), FieldMemOperand(x5, JSFunction::kDispatchHandleOffset));
    __ LoadEntrypointAndParameterCountFromJSDispatchTable(code, argc, dispatch_handle, scratch);
...
#else
    __ LoadTaggedField(argc, FieldMemOperand(x5, JSFunction::kSharedFunctionInfoOffset));
    __ Ldrh(argc.W(), FieldMemOperand(argc, SharedFunctionInfo::kFormalParameterCountOffset));
    ...
    __ LoadTaggedField(x6, FieldMemOperand(x1, JSGeneratorObject::kReceiverOffset));
    ...
    // Copy the function arguments from the generator object's register file.
    {
        ...
        __ Cbz(x10, &done);
        __ LoadTaggedField(x6, FieldMemOperand(x1, JSGeneratorObject::kParametersAndRegistersOffset));
    ...
    if (v8_flags.debug_code) {
        ...
        __ LoadTaggedField(sfi, FieldMemOperand(x5, JSFunction::kSharedFunctionInfoOffset));
        GetSharedFunctionInfoBytecodeOrBaseline(masm, sfi, bytecode, scratch, &is_baseline, &is_unavailable);
        __ B(&ok);
...
void Generate_JSEntryVariant(MacroAssembler* masm, StackFrame::Type type, Builtin entry_trampoline)
{
...

    {
        ...
        if (xdata_encoder) {
            xdata_encoder->onFramePointerAdjustment(EntryFrameConstants::kDirectCallerFPOffset, EntryFrameConstants::kDirectCallerSPOffset);
        }
#endif
    ...
    static_assert(EntryFrameConstants::kCalleeSavedRegisterBytesPushedAfterFpLrPair == 0);
    ...
    // that.
    {
        ...
        __ Mov(x10, ExternalReference::Create(IsolateAddressId::kExceptionAddress, masm->isolate()));
    }
    ...
    static_assert(StackHandlerConstants::kSize == 2 * kSystemPointerSize, "Unexpected offset for StackHandlerConstants::kSize");
    static_assert(StackHandlerConstants::kNextOffset == 0 * kSystemPointerSize, "Unexpected offset for StackHandlerConstants::kNextOffset");
    ...
    static_assert(EntryFrameConstants::kFixedFrameSize % (2 * kSystemPointerSize) == 0, "Size of entry frame is not a multiple of 16 bytes");
    // fast_c_call_caller_fp and fast_c_call_caller_pc have already been popped.
    int drop_count = (EntryFrameConstants::kFixedFrameSize / kSystemPointerSize) - 2;
    __ Drop(drop_count);
...
static void LeaveInterpreterFrame(MacroAssembler* masm, Register scratch1, Register scratch2)
{
    ...
    __ Ldr(params_size, MemOperand(fp, InterpreterFrameConstants::kBytecodeArrayFromFp));
    __ Ldrh(params_size.W(), FieldMemOperand(params_size, BytecodeArray::kParameterSizeOffset));

    Register actual_params_size = scratch2;
    // Compute the size of the actual parameters + receiver (in bytes).
    __ Ldr(actual_params_size, MemOperand(fp, StandardFrameConstants::kArgCOffset));
    ...
    MacroAssembler* masm, Register bytecode_array, Register bytecode_offset, Register bytecode, Register scratch1, Register scratch2, Label* if_return)
{
    ...
    DCHECK(!AreAliased(bytecode_array, bytecode_offset, bytecode_size_table, bytecode, original_bytecode_offset));
    ...
    __ Add(bytecode_size_table, bytecode_size_table, Operand(kByteSize * interpreter::Bytecodes::kBytecodeCount));
    __ B(&process_bytecode);

    __ Bind(&extra_wide);
    // Update table to the extra wide scaled table.
    __ Add(bytecode_size_table, bytecode_size_table, Operand(2 * kByteSize * interpreter::Bytecodes::kBytecodeCount));
...
void ResetFeedbackVectorOsrUrgency(MacroAssembler* masm, Register feedback_vector, Register scratch)
{
    ...
    __ Ldrb(scratch, FieldMemOperand(feedback_vector, FeedbackVector::kOsrStateOffset));
    __ And(scratch, scratch, Operand(~FeedbackVector::OsrUrgencyBits::kMask));
    __ Strb(scratch, FieldMemOperand(feedback_vector, FeedbackVector::kOsrStateOffset));
}
...
void Builtins::Generate_BaselineOutOfLinePrologue(MacroAssembler* masm)
{
    ...
    // Increment invocation count for the function.
    {
        ...
        __ Ldr(invocation_count, FieldMemOperand(feedback_vector, FeedbackVector::kInvocationCountOffset));
        __ Add(invocation_count, invocation_count, Operand(1));
        __ Str(invocation_count, FieldMemOperand(feedback_vector, FeedbackVector::kInvocationCountOffset));
    }
    ...
    __ bind(&call_stack_guard);
    {
        ...
        Register maybe_dispatch_handle = V8_ENABLE_LEAPTIERING_BOOL ? kJavaScriptCallDispatchHandleRegister : padreg;
...
void Builtins::Generate_BaselineOutOfLinePrologueDeopt(MacroAssembler* masm)
{
    ...
    __ Pop(padreg, kJavaScriptCallArgCountRegister, kJavaScriptCallTargetRegister, kContextRegister);
...
void Builtins::Generate_InterpreterEntryTrampoline(MacroAssembler* masm, InterpreterEntryTrampolineMode mode)
{
    ...
    GetSharedFunctionInfoBytecodeOrBaseline(masm, sfi, kInterpreterBytecodeArrayRegister, x11, &is_baseline, &compile_lazy);
    ...
    __ Ldrh(x7, FieldMemOperand(kInterpreterBytecodeArrayRegister, BytecodeArray::kParameterSizeOffset));
    __ Cmp(x6, x7);
    ...
    __ Mov(kInterpreterBytecodeOffsetRegister, Operand(BytecodeArray::kHeaderSize - kHeapObjectTag));
    ...
    Label stack_overflow;
    {
        ...
        __ Ldrh(w12, FieldMemOperand(kInterpreterBytecodeArrayRegister, BytecodeArray::kMaxArgumentsOffset));
    ...
    __ Ldrsw(x10, FieldMemOperand(kInterpreterBytecodeArrayRegister, BytecodeArray::kIncomingNewTargetOrGeneratorRegisterOffset));
    ...
    Label do_dispatch;
    __ bind(&do_dispatch);
    __ Mov(kInterpreterDispatchTableRegister, ExternalReference::interpreter_dispatch_table_address(masm->isolate()));
    ...
    __ Ldr(kJavaScriptCallCodeStartRegister, MemOperand(kInterpreterDispatchTableRegister, x1));
    ...
    __ Ldr(kInterpreterBytecodeArrayRegister, MemOperand(fp, InterpreterFrameConstants::kBytecodeArrayFromFp));
    ...
    AdvanceBytecodeOffsetOrReturn(masm, kInterpreterBytecodeArrayRegister, kInterpreterBytecodeOffsetRegister, x1, x2, x3, &do_return);
    __ B(&do_dispatch);
    ...
    __ Mov(kInterpreterBytecodeOffsetRegister, Operand(Smi::FromInt(BytecodeArray::kHeaderSize - kHeapObjectTag + kFunctionEntryBytecodeOffset)));
    __ Str(kInterpreterBytecodeOffsetRegister, MemOperand(fp, InterpreterFrameConstants::kBytecodeOffsetFromFp));
...
static void GenerateInterpreterPushArgs(
    MacroAssembler* masm, Register num_args, Register first_arg_index, Register spread_arg_out, ConvertReceiverMode receiver_mode, InterpreterPushArgsMode mode)
{
    ...
    DCHECK(!AreAliased(num_args, first_arg_index, last_arg_addr, stack_addr, slots_to_claim, slots_to_copy));
    // spread_arg_out may alias with the first_arg_index input.
    DCHECK(!AreAliased(spread_arg_out, last_arg_addr, stack_addr, slots_to_claim, slots_to_copy));
    ...
    __ Sub(last_arg_addr, first_arg_index, Operand(slots_to_copy, LSL, kSystemPointerSizeLog2));
...
void Builtins::Generate_InterpreterPushArgsThenCallImpl(MacroAssembler* masm, ConvertReceiverMode receiver_mode, InterpreterPushArgsMode mode)
{
    ...
    GenerateInterpreterPushArgs(masm, num_args, first_arg_index, spread_arg_out, receiver_mode, mode);
...
void Builtins::Generate_InterpreterPushArgsThenConstructImpl(MacroAssembler* masm, InterpreterPushArgsMode mode)
{
    ...
    Register spread_arg_out = (mode == InterpreterPushArgsMode::kWithFinalSpread) ? x2 : no_reg;
    GenerateInterpreterPushArgs(masm, num_args, first_arg_index, spread_arg_out, ConvertReceiverMode::kNullOrUndefined, mode);
...
void Builtins::Generate_ConstructForwardAllArgsImpl(MacroAssembler* masm, ForwardWhichFrame which_frame)
{
    ...
    // receiver.
    __ Add(x4, x4, Operand((StandardFrameConstants::kFixedSlotCountAboveFp + 1) * kSystemPointerSize));
...
void NewImplicitReceiver(MacroAssembler* masm)
{
    ...
    __ Str(implicit_receiver, MemOperand(fp, FastConstructFrameConstants::kImplicitReceiverOffset));
...
void Builtins::Generate_InterpreterPushArgsThenFastConstructFunction(MacroAssembler* masm)
{
    ...
    __ TestAndBranchIfAllClear(x2, Map::Bits1::IsConstructorBit::kMask, &non_constructor);
    ...
    GenerateInterpreterPushArgs(masm, x0, x4, Register::no_reg(), ConvertReceiverMode::kNullOrUndefined, InterpreterPushArgsMode::kOther);
    ...
    __ TestAndBranchIfAnySet(w2, SharedFunctionInfo::ConstructAsBuiltinBit::kMask, &builtin_call);

    // Check if we need to create an implicit receiver.
    Label not_create_implicit_receiver;
    __ DecodeField<SharedFunctionInfo::FunctionKindBits>(w2);
    __ JumpIfIsInRange(w2, static_cast<uint32_t>(FunctionKind::kDefaultDerivedConstructor), static_cast<uint32_t>(FunctionKind::kDerivedConstructor),
        &not_create_implicit_receiver);
    ...
    // on-stack receiver as the result.
    __ Bind(&use_receiver);
    __ Ldr(x0, MemOperand(fp, FastConstructFrameConstants::kImplicitReceiverOffset));
    __ CompareRoot(x0, RootIndex::kTheHoleValue);
    __ B(eq, &do_throw);
...
static void Generate_InterpreterEnterBytecode(MacroAssembler* masm)
{
    ...
    if (v8_flags.debug_code) {
        ...
        __ AssertNotSmi(kInterpreterBytecodeArrayRegister, AbortReason::kFunctionDataShouldBeBytecodeArrayOnInterpreterEntry);
        __ IsObjectType(kInterpreterBytecodeArrayRegister, x1, x1, BYTECODE_ARRAY_TYPE);
        __ Assert(eq, AbortReason::kFunctionDataShouldBeBytecodeArrayOnInterpreterEntry);
    }
    ...
    if (v8_flags.debug_code) {
        Label okay;
        __ cmp(kInterpreterBytecodeOffsetRegister, Operand(BytecodeArray::kHeaderSize - kHeapObjectTag));
        __ B(ge, &okay);
        __ Unreachable();
        __ bind(&okay);
    }
    ...
    __ LoadTrustedPointerField(x1, FieldMemOperand(x1, SharedFunctionInfo::kTrustedFunctionDataOffset), kUnknownIndirectPointerTag);
    __ IsObjectType(x1, kInterpreterDispatchTableRegister, kInterpreterDispatchTableRegister, INTERPRETER_DATA_TYPE);
    __ B(ne, &builtin_trampoline);

    __ LoadProtectedPointerField(x1, FieldMemOperand(x1, InterpreterData::kInterpreterTrampolineOffset));
...
void Builtins::Generate_InterpreterEnterAtNextBytecode(MacroAssembler* masm)
{
    ...
    __ cmp(kInterpreterBytecodeOffsetRegister, Operand(BytecodeArray::kHeaderSize - kHeapObjectTag + kFunctionEntryBytecodeOffset));
    ...
    Label if_return;
    AdvanceBytecodeOffsetOrReturn(masm, kInterpreterBytecodeArrayRegister, kInterpreterBytecodeOffsetRegister, x1, x2, x3, &if_return);
...
void Generate_ContinueToBuiltinHelper(MacroAssembler* masm, bool javascript_builtin, bool with_result)
{
    ...
    int frame_size = BuiltinContinuationFrameConstants::kFixedFrameSizeFromFp
        + (allocatable_register_count + BuiltinContinuationFrameConstants::PaddingSlotCount(allocatable_register_count)) * kSystemPointerSize;
    ...
    if (javascript_builtin && with_result) {
        ...
        constexpr int return_offset = BuiltinContinuationFrameConstants::kCallerSPOffset / kSystemPointerSize - kJSArgcReceiverSlots;
    ...
    __ Ldr(builtin, MemOperand(fp, BuiltinContinuationFrameConstants::kBuiltinIndexOffset));
...
void OnStackReplacement(MacroAssembler* masm, OsrSourceTier source, Register maybe_target_code)
{
    ...
    __ LoadProtectedPointerField(x1, FieldMemOperand(x0, Code::kDeoptimizationDataOrInterpreterDataOffset));

    // Load the OSR entrypoint offset from the deoptimization data.
    // <osr_offset> = <deopt_data>[#header_size + #osr_pc_offset]
    __ SmiUntagField(x1, FieldMemOperand(x1, TrustedFixedArray::OffsetOfElementAt(DeoptimizationData::kOsrPcOffsetIndex)));
...
void Builtins::Generate_BaselineOnStackReplacement(MacroAssembler* masm)
{
    ...
    __ ldr(kContextRegister, MemOperand(fp, BaselineFrameConstants::kContextOffset));
...
void Builtins::Generate_FunctionPrototypeCall(MacroAssembler* masm)
{
    ...
    // That is if padding exists or not.
    {
        ...
        __ CopyDoubleWords(copy_to, copy_from, count, MacroAssembler::kSrcLessThanDst);
        __ Drop(2);
    }
...
void Generate_PrepareForCopyingVarargs(MacroAssembler* masm, Register argc, Register len)
{
...
void Builtins::Generate_CallOrConstructVarargs(MacroAssembler* masm, Builtin target_builtin)
{
    ...
    // Push varargs.
    {
        ...
        __ Add(src, arguments_list, OFFSET_OF_DATA_START(FixedArray) - kHeapObjectTag);
#if !V8_STATIC_ROOTS_BOOL
...
void Builtins::Generate_CallOrConstructForwardVarargs(MacroAssembler* masm, CallOrConstructMode mode, Builtin target_builtin)
{
    ...
    if (mode == CallOrConstructMode::kConstruct) {
        ...
        __ TestAndBranchIfAnySet(x5, Map::Bits1::IsConstructorBit::kMask, &new_target_constructor);
    ...
    // Push varargs.
    {
        ...
        __ Add(args_fp, fp, CommonFrameConstants::kFixedFrameSizeAboveFp + kSystemPointerSize);
...
void Builtins::Generate_CallFunction(MacroAssembler* masm, ConvertReceiverMode mode)
{
    ...
    __ TestAndBranchIfAnySet(w3, SharedFunctionInfo::IsNativeBit::kMask | SharedFunctionInfo::IsStrictBit::kMask, &done_convert);
    {
        // ----------- S t a t e -------------
        //  -- x0 : the number of arguments
        //  -- x1 : the function to call (checked to be a JSFunction)
...
#else
    __ Ldrh(x2, FieldMemOperand(x2, SharedFunctionInfo::kFormalParameterCountOffset));
...
void Generate_PushBoundArguments(MacroAssembler* masm)
{
    ...
    __ Cbz(bound_argc, &no_bound_arguments);
    {
        ...
        // Copy [[BoundArguments]] to the stack (below the receiver).
        {
            ...
            Register copy_to = x12;
            __ Add(bound_argv, bound_argv, OFFSET_OF_DATA_START(FixedArray) - kHeapObjectTag);
...
void Builtins::Generate_Call(MacroAssembler* masm, ConvertReceiverMode mode)
{
    ...
    __ CompareInstanceTypeRange(map, instance_type, FIRST_CALLABLE_JS_FUNCTION_TYPE, LAST_CALLABLE_JS_FUNCTION_TYPE);
    ...
    // Check if target has a [[Call]] internal method.
    {
        ...
        map = no_reg;
        __ TestAndBranchIfAllClear(flags, Map::Bits1::IsCallableBit::kMask, &non_callable);
    }
...
void Builtins::Generate_ConstructFunction(MacroAssembler* masm)
{
    ...
    __ LoadTaggedField(x4, FieldMemOperand(x1, JSFunction::kSharedFunctionInfoOffset));
    __ Ldr(w4, FieldMemOperand(x4, SharedFunctionInfo::kFlagsOffset));
    __ TestAndBranchIfAllClear(w4, SharedFunctionInfo::ConstructAsBuiltinBit::kMask, &call_generic_stub);
...
void Builtins::Generate_Construct(MacroAssembler* masm)
{
    ...
    __ LoadTaggedField(map, FieldMemOperand(target, HeapObject::kMapOffset));
    {
        ...
        __ Ldrb(flags, FieldMemOperand(map, Map::kBitFieldOffset));
        __ TestAndBranchIfAllClear(flags, Map::Bits1::IsConstructorBit::kMask, &non_constructor);
    }

    // Dispatch based on instance type.
    __ CompareInstanceTypeRange(map, instance_type, FIRST_JS_FUNCTION_TYPE, LAST_JS_FUNCTION_TYPE);
...
void Builtins::Generate_WasmCompileLazy(MacroAssembler* masm)
{
    ...
    __ sxtw(kWasmCompileLazyFuncIndexRegister, kWasmCompileLazyFuncIndexRegister.W());
    ...
    temps.Exclude(x17);
    {
        ...
        __ Str(kWasmImplicitArgRegister, MemOperand(fp, WasmFrameConstants::kWasmInstanceDataOffset));
    ...
    __ ldr(temp, FieldMemOperand(kWasmImplicitArgRegister, WasmTrustedInstanceData::kJumpTableStartOffset));
...
void LoadTargetJumpBuffer(MacroAssembler* masm, Register target_continuation, Register tmp, wasm::JumpBuffer::StackState expected_state)
{
    ...
    __ Str(xzr, MemOperand(fp, StackSwitchFrameConstants::kGCScanSlotCountOffset));
    // Switch stack!
...
// stack pool.
void SwitchStacks(
    MacroAssembler* masm, Register finished_continuation, const CPURegister& keep1 = NoReg, const CPURegister& keep2 = padreg, const CPURegister& keep3 = NoReg)
{
...
public:
    ...
    void Free(Register* reg)
    {
        ...
        *reg = no_reg;
        allocated_registers_.erase(find(allocated_registers_.begin(), allocated_registers_.end(), reg));
    }
    ...
    void Reserve(const Register& reg1, const Register& reg2, const Register& reg3 = NoReg, const Register& reg4 = NoReg, const Register& reg5 = NoReg,
    ...
    void ResetExcept(const Register& reg1 = NoReg, const Register& reg2 = NoReg, const Register& reg3 = NoReg, const Register& reg4 = NoReg,
...
void Builtins::Generate_WasmToJsWrapperAsm(MacroAssembler* masm)
{
    ...
    __ Push(wasm::kFpParamRegisters[7], wasm::kFpParamRegisters[6], wasm::kFpParamRegisters[5], wasm::kFpParamRegisters[4]);
    __ Push(wasm::kFpParamRegisters[3], wasm::kFpParamRegisters[2], wasm::kFpParamRegisters[1], wasm::kFpParamRegisters[0]);

    __ Push(wasm::kGpParamRegisters[6], wasm::kGpParamRegisters[5], wasm::kGpParamRegisters[4], wasm::kGpParamRegisters[3]);
...
{
    __ Add(lr, kWasmTrapHandlerFaultAddressRegister, WasmFrameConstants::kProtectedInstructionReturnAddressOffset);
...
void Builtins::Generate_WasmSuspend(MacroAssembler* masm)
{
    ...
    __ Sub(sp, sp, Immediate(StackSwitchFrameConstants::kNumSpillSlots * kSystemPointerSize));
...
void Generate_WasmResumeHelper(MacroAssembler* masm, wasm::OnResume on_resume)
{
    ...
    __ RecordWriteField(suspender, WasmSuspenderObject::kParentOffset, active_suspender, kLRHasBeenSaved, SaveFPRegsMode::kIgnore);
    ...
    __ RecordWriteField(target_continuation, WasmContinuationObject::kParentOffset, active_continuation, kLRHasBeenSaved, SaveFPRegsMode::kIgnore);
    FREE_REG(active_continuation);
    int32_t active_continuation_offset = MacroAssembler::RootRegisterOffsetForRootIndex(RootIndex::kActiveContinuation);
    __ Str(target_continuation, MemOperand(kRootRegister, active_continuation_offset));
...
namespace {
void SwitchToAllocatedStack(MacroAssembler* masm, RegisterAllocator& regs, Register wasm_instance, Register wrapper_buffer, Register& original_fp,
    Register& new_wrapper_buffer, Label* suspend)
{
    ...
    __ Ldr(scratch, MemOperand(wrapper_buffer, JSToWasmWrapperFrameConstants::kWrapperBufferReturnCount));
    __ Str(scratch, MemOperand(new_wrapper_buffer, JSToWasmWrapperFrameConstants::kWrapperBufferReturnCount));
    __ Ldr(scratch, MemOperand(wrapper_buffer, JSToWasmWrapperFrameConstants::kWrapperBufferSigRepresentationArray));
    __ Str(scratch, MemOperand(new_wrapper_buffer, JSToWasmWrapperFrameConstants::kWrapperBufferSigRepresentationArray));
}

void SwitchBackAndReturnPromise(MacroAssembler* masm, RegisterAllocator& regs, wasm::Promise mode, Label* return_promise)
{
    regs.ResetExcept();
    ...
    __ Ldr(kContextRegister, MemOperand(fp, StackSwitchFrameConstants::kImplicitArgOffset));
    ...
    if (mode == wasm::kPromise) {
        __ Mov(tmp, 1);
        __ Str(tmp, MemOperand(fp, StackSwitchFrameConstants::kGCScanSlotCountOffset));
...
void GenerateExceptionHandlingLandingPad(MacroAssembler* masm, RegisterAllocator& regs, Label* return_promise)
{
    regs.ResetExcept();
    ...
    __ Ldr(thread_in_wasm_flag_addr, MemOperand(kRootRegister, Isolate::thread_in_wasm_flag_address_offset()));
...
void JSToWasmWrapperHelper(MacroAssembler* masm, wasm::Promise mode)
{
    ...
    __ Ldr(implicit_arg, MemOperand(fp, JSToWasmWrapperFrameConstants::kImplicitArgOffset));

    DEFINE_PINNED(wrapper_buffer, WasmJSToWasmWrapperDescriptor::WrapperBufferRegister());

    Label suspend;
    Register original_fp = no_reg;
    Register new_wrapper_buffer = no_reg;
    if (stack_switch) {
        SwitchToAllocatedStack(masm, regs, implicit_arg, wrapper_buffer, original_fp, new_wrapper_buffer, &suspend);
    } else {
        original_fp = fp;
        new_wrapper_buffer = wrapper_buffer;
    }

    regs.ResetExcept(original_fp, wrapper_buffer, implicit_arg, new_wrapper_buffer);

    {
        __ Str(new_wrapper_buffer, MemOperand(fp, JSToWasmWrapperFrameConstants::kWrapperBufferOffset));
        if (stack_switch) {
            __ Str(implicit_arg, MemOperand(fp, StackSwitchFrameConstants::kImplicitArgOffset));
            DEFINE_SCOPED(scratch)
            __ Ldr(scratch, MemOperand(original_fp, JSToWasmWrapperFrameConstants::kResultArrayParamOffset));
            __ Str(scratch, MemOperand(fp, StackSwitchFrameConstants::kResultArrayOffset));
        }
    }
    {
        DEFINE_SCOPED(result_size);
        __ Ldr(result_size, MemOperand(wrapper_buffer, JSToWasmWrapperFrameConstants::kWrapperBufferStackReturnBufferSize));
    ...
    }
    {
        DEFINE_SCOPED(scratch);
        __ Mov(scratch, sp);
        __ Str(scratch, MemOperand(new_wrapper_buffer, JSToWasmWrapperFrameConstants::kWrapperBufferStackReturnBufferStart));
    }
    if (stack_switch) {
...

    {
        ...
        __ Ldr(params_start, MemOperand(wrapper_buffer, JSToWasmWrapperFrameConstants::kWrapperBufferParamStart));
        {
            // Push stack parameters on the stack.
            DEFINE_SCOPED(params_end);
            __ Ldr(params_end, MemOperand(wrapper_buffer, JSToWasmWrapperFrameConstants::kWrapperBufferParamEnd));
            ...
            // Push parameter
            {
                ...
                __ Ldp(scratch2, scratch1, MemOperand(params_end, -2 * kSystemPointerSize, PreIndex));
        ...
        for (size_t i = 1; i < arraysize(wasm::kGpParamRegisters); i += 2) {
            ...
            __ Ldp(wasm::kGpParamRegisters[i], wasm::kGpParamRegisters[i + 1], MemOperand(params_start, next_offset));
            next_offset += 2 * kSystemPointerSize;
        }

        for (size_t i = 0; i < arraysize(wasm::kFpParamRegisters); i += 2) {
            __ Ldp(wasm::kFpParamRegisters[i], wasm::kFpParamRegisters[i + 1], MemOperand(params_start, next_offset));
    ...
    __ Ldr(wrapper_buffer, MemOperand(fp, JSToWasmWrapperFrameConstants::kWrapperBufferOffset));

    __ Str(wasm::kFpReturnRegisters[0], MemOperand(wrapper_buffer, JSToWasmWrapperFrameConstants::kWrapperBufferFPReturnRegister1));
    __ Str(wasm::kFpReturnRegisters[1], MemOperand(wrapper_buffer, JSToWasmWrapperFrameConstants::kWrapperBufferFPReturnRegister2));
    __ Str(wasm::kGpReturnRegisters[0], MemOperand(wrapper_buffer, JSToWasmWrapperFrameConstants::kWrapperBufferGPReturnRegister1));
    __ Str(wasm::kGpReturnRegisters[1], MemOperand(wrapper_buffer, JSToWasmWrapperFrameConstants::kWrapperBufferGPReturnRegister2));
    ...
    if (stack_switch) {
        ...
        __ Ldr(x1, MemOperand(fp, JSToWasmWrapperFrameConstants::kResultArrayParamOffset));
        __ Ldr(x0, MemOperand(fp, JSToWasmWrapperFrameConstants::kImplicitArgOffset));
    }
    Register scratch = x3;
...
void Builtins::Generate_CEntry(MacroAssembler* masm, int result_size, ArgvMode argv_mode, bool builtin_exit_frame, bool switch_to_central_stack)
{
    ...
    FrameScope scope(masm, StackFrame::MANUAL);
    __ EnterExitFrame(x10, kNoExtraSpace, builtin_exit_frame ? StackFrame::BUILTIN_EXIT : StackFrame::EXIT);
    ...
    // contain the current exception, don't clobber it.
    {
        ...
        __ Mov(x1, 0); // argv.
        __ Mov(x2, ER::isolate_address());
        __ CallCFunction(ER::Create(Runtime::kUnwindAndFindExceptionHandler), 3, SetIsolateDataSlots::kNo);
    }

    // Retrieve the handler context, SP and FP.
    __ Mov(cp, ER::Create(IsolateAddressId::kPendingHandlerContextAddress, masm->isolate()));
    __ Ldr(cp, MemOperand(cp));
    {
        UseScratchRegisterScope temps(masm);
        Register scratch = temps.AcquireX();
        __ Mov(scratch, ER::Create(IsolateAddressId::kPendingHandlerSPAddress, masm->isolate()));
        __ Ldr(scratch, MemOperand(scratch));
        __ Mov(sp, scratch);
    }
    __ Mov(fp, ER::Create(IsolateAddressId::kPendingHandlerFPAddress, masm->isolate()));
...

    {
        ...
        UseScratchRegisterScope temps(masm);
        Register scratch = temps.AcquireX();
        __ Mov(scratch, ER::Create(IsolateAddressId::kCEntryFPAddress, masm->isolate()));
    ...
    UseScratchRegisterScope temps(masm);
    temps.Exclude(x17);
    __ Mov(x17, ER::Create(IsolateAddressId::kPendingHandlerEntrypointAddress, masm->isolate()));
...
void Builtins::Generate_WasmHandleStackOverflow(MacroAssembler* masm)
{
    ...
    {
        __ Ldr(kWasmImplicitArgRegister, MemOperand(fp, WasmFrameConstants::kWasmInstanceDataOffset));
        __ LoadTaggedField(cp, FieldMemOperand(kWasmImplicitArgRegister, WasmTrustedInstanceData::kNativeContextOffset));
        FrameScope scope(masm, StackFrame::MANUAL);
        __ EnterFrame(StackFrame::INTERNAL);
        __ SmiTag(gap);
        __ PushArgument(gap);
...
void Builtins::Generate_DoubleToI(MacroAssembler* masm)
{
    ...
    __ Ubfx(exponent, result, HeapNumber::kMantissaBits, HeapNumber::kExponentBits);
    ...
    __ Sub(exponent, exponent, HeapNumber::kExponentBias + HeapNumber::kMantissaBits);
...
void Builtins::Generate_CallApiCallbackImpl(MacroAssembler* masm, CallApiCallbackMode mode)
{
    ...
    if (v8_flags.debug_code) {
        ...
        __ Add(scratch, sp, FCA::kLengthOffset + kSystemPointerSize - FC::kFCIArgcOffset);
        __ cmp(scratch, fp);
        __ B(eq, &ok);
        __ DebugBreak();
        __ Bind(&ok);
    ...
    __ Add(function_callback_info_arg, fp, Operand(FC::kFunctionCallbackInfoOffset));
...
void Builtins::Generate_CallApiGetter(MacroAssembler* masm)
{
    ...
    __ LoadTaggedField(name_arg, FieldMemOperand(callback, AccessorInfo::kNameOffset));
    ...
    DCHECK(!AreAliased(api_function_address, property_callback_info_arg, name_arg, callback, scratch, scratch2));
...
void CopyRegListToFrame(
    MacroAssembler* masm, const Register& dst, int dst_offset, const CPURegList& reg_list, const RegisterT& temp0, const RegisterT& temp1, int src_offset = 0)
{
    ASM_CODE_COMMENT(masm);
...
void RestoreRegList(MacroAssembler* masm, const CPURegList& reg_list, const Register& src_base, int src_offset)
{
    ASM_CODE_COMMENT(masm);
    DCHECK_EQ(reg_list.Count() % 2, 0);
    UseScratchRegisterScope temps(masm);
...
void Generate_DeoptimizationEntry(MacroAssembler* masm, DeoptimizeKind deopt_kind)
{
    ...
    CPURegList saved_simd128_registers(kQRegSizeInBits, DoubleRegList::FromBits(RegisterConfiguration::Default()->allocatable_simd128_codes_mask()));
    ...
    const int kSavedRegistersAreaSize = (saved_registers.Count() * kXRegSize) + (saved_simd128_registers.Count() * kQRegSize);
    ...
    CopyRegListToFrame(masm, x1, FrameDescription::registers_offset(), saved_registers, x2, x3);

    // Copy simd128 / double registers to the input frame.
    CopyRegListToFrame(masm, x1, FrameDescription::simd128_registers_offset(), saved_simd128_registers, q2, q3, kSimd128RegistersOffset);
    ...
    RestoreRegList(masm, saved_simd128_registers, current_frame, FrameDescription::simd128_registers_offset());
    ...
    RestoreRegList(masm, saved_registers, last_output_frame, FrameDescription::registers_offset());
...
void Generate_BaselineOrInterpreterEntry(MacroAssembler* masm, bool next_bytecode, bool is_osr = false)
{
    ...
    __ Str(feedback_cell, MemOperand(fp, BaselineFrameConstants::kFeedbackCellFromFp));
    feedback_cell = no_reg;
    // Update feedback vector cache.
    static_assert(InterpreterFrameConstants::kFeedbackVectorFromFp == BaselineFrameConstants::kFeedbackVectorFromFp);
    __ Str(feedback_vector, MemOperand(fp, InterpreterFrameConstants::kFeedbackVectorFromFp));
    ...
    __ Sub(kInterpreterBytecodeOffsetRegister, kInterpreterBytecodeOffsetRegister, (BytecodeArray::kHeaderSize - kHeapObjectTag));
    ...
    if (!is_osr) {
        ...
        if (next_bytecode) {
            __ Mov(get_baseline_pc, ExternalReference::baseline_pc_for_bytecode_offset());
        }
...
#ifdef V8_ENABLE_LEAPTIERING
    __ InvokeFunction(x1, x0, InvokeType::kJump, ArgumentAdaptionMode::kDontAdapt);
#else
