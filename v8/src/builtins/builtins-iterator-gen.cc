...
TNode<Object> IteratorBuiltinsAssembler::GetIteratorMethod(TNode<Context> context, TNode<Object> object)
{
    return GetProperty(context, object, factory()->iterator_symbol());
}

IteratorRecord IteratorBuiltinsAssembler::GetIterator(TNode<Context> context, TNode<Object> object)
{
    TNode<Object> method = GetIteratorMethod(context, object);
    return GetIterator(context, object, method);
}

IteratorRecord IteratorBuiltinsAssembler::GetIterator(TNode<Context> context, TNode<Object> object, TNode<Object> method)
{
    ...
    BIND(&if_callable);
    {
        ...
        BIND(&get_next);
        TNode<Object> next = GetProperty(context, iterator, factory()->next_string());
...
TNode<JSReceiver> IteratorBuiltinsAssembler::IteratorStep(
    TNode<Context> context, const IteratorRecord& iterator, Label* if_done, std::optional<TNode<Map>> fast_iterator_result_map)
{
    ...
    IteratorComplete(context, heap_object_result, if_done, fast_iterator_result_map);
    Goto(&return_result);
...
void IteratorBuiltinsAssembler::IteratorComplete(
    TNode<Context> context, const TNode<HeapObject> iterator, Label* if_done, std::optional<TNode<Map>> fast_iterator_result_map)
{
    DCHECK_NOT_NULL(if_done);
    ...
    if (fast_iterator_result_map) {
        ...
        // 2. Return ToBoolean(? Get(iterResult, "done")).
        TNode<Object> done = LoadObjectField(iterator, JSIteratorResult::kDoneOffset);
    ...
    {
        // 2. Return ToBoolean(? Get(iterResult, "done")).
        TNode<Object> done = GetProperty(context, iterator, factory()->done_string());
        BranchIfToBooleanIsTrue(done, if_done, &return_result);
    }
...
TNode<Object> IteratorBuiltinsAssembler::IteratorValue(TNode<Context> context, TNode<JSReceiver> result, std::optional<TNode<Map>> fast_iterator_result_map)
{
    Label exit(this);
...
void IteratorBuiltinsAssembler::Iterate(TNode<Context> context, TNode<Object> iterable, std::function<void(TNode<Object>)> func,
    std::initializer_list<compiler::CodeAssemblerVariable*> merged_variables)
{
    Iterate(context, iterable, GetIteratorMethod(context, iterable), func, merged_variables);
}

void IteratorBuiltinsAssembler::Iterate(TNode<Context> context, TNode<Object> iterable, TNode<Object> iterable_fn, std::function<void(TNode<Object>)> func,
    std::initializer_list<compiler::CodeAssemblerVariable*> merged_variables)
{
    Label done(this);
...
TNode<JSArray> IteratorBuiltinsAssembler::IterableToList(TNode<Context> context, TNode<Object> iterable, TNode<Object> iterator_fn)
{
...
TNode<FixedArray> IteratorBuiltinsAssembler::IterableToFixedArray(TNode<Context> context, TNode<Object> iterable, TNode<Object> iterator_fn)
{
    GrowableFixedArray values(state());
    FillFixedArrayFromIterable(context, iterable, iterator_fn, &values);
...
void IteratorBuiltinsAssembler::FillFixedArrayFromIterable(
    TNode<Context> context, TNode<Object> iterable, TNode<Object> iterator_fn, GrowableFixedArray* values)
{
...
TF_BUILTIN(IterableToFixedArrayForWasm, IteratorBuiltinsAssembler)
{
    ...
    GotoIf(WordEqual(PositiveSmiUntag(expected_length), values.var_length()->value()), &done);
    Return(CallRuntime(Runtime::kThrowTypeError, context, SmiConstant(MessageTemplate::kWasmTrapMultiReturnLengthMismatch)));
...
TNode<FixedArray> IteratorBuiltinsAssembler::StringListFromIterable(TNode<Context> context, TNode<Object> iterable)
{
    Label done(this);
...
void IteratorBuiltinsAssembler::FastIterableToList(TNode<Context> context, TNode<Object> iterable, TVariable<JSArray>* var_result, Label* slow)
{
    ...
    GotoIfNot(Word32Or(IsFastJSArrayWithNoCustomIteration(context, iterable), IsFastJSArrayForReadWithNoCustomIteration(context, iterable)), &check_string);

    // Fast path for fast JSArray.
    *var_result = CAST(CallBuiltin(Builtin::kCloneFastJSArrayFillingHoles, context, iterable));
    Goto(&done);
    ...
    BIND(&check_string);
    {
        ...
        string_assembler.BranchIfStringPrimitiveWithNoCustomIteration(iterable, context, &string_maybe_fast_call, &check_map);
        ...
        GotoIf(Uint32GreaterThan(length, Uint32Constant(JSArray::kMaxFastArrayLength)), slow);
    ...
    BIND(&check_map);
    {
        ...
        BranchIfIterableWithOriginalKeyOrValueMapIterator(state(), iterable, context, &map_fast_call, &check_set);

        BIND(&map_fast_call);
        *var_result = CAST(CallBuiltin(Builtin::kMapIteratorToList, context, iterable));
        Goto(&done);
    }
    ...
    BIND(&check_set);
    {
        ...
        BranchIfIterableWithOriginalValueSetIterator(state(), iterable, context, &set_fast_call, slow);

        BIND(&set_fast_call);
        *var_result = CAST(CallBuiltin(Builtin::kSetOrSetIteratorToList, context, iterable));
        Goto(&done);
    }
...
TNode<JSArray> IteratorBuiltinsAssembler::FastIterableToList(TNode<Context> context, TNode<Object> iterable, Label* slow)
{
...
TF_BUILTIN(GetIteratorWithFeedbackLazyDeoptContinuation, IteratorBuiltinsAssembler)
{
    ...
    TNode<Object> result = CallBuiltin(Builtin::kCallIteratorWithFeedback, context, receiver, iterator_method, call_slot_smi, feedback);
    Return(result);
}
...
TF_BUILTIN(CallIteratorWithFeedbackLazyDeoptContinuation, IteratorBuiltinsAssembler)
{
    ...
    ThrowIfNotJSReceiver(context, iterator, MessageTemplate::kSymbolIteratorInvalid, "");
    Return(iterator);
}
...
TF_BUILTIN(IterableToFixedArrayWithSymbolLookupSlow, IteratorBuiltinsAssembler)
{
    ...
    TNode<Object> iterator_fn = GetIteratorMethod(context, iterable);
    TailCallBuiltin(Builtin::kIterableToFixedArray, context, iterable, iterator_fn);
}
