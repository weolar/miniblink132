...
inline bool IsJSArrayFastElementMovingAllowed(Isolate* isolate, Tagged<JSArray> receiver)
{
...
{
    return !IsCustomElementsReceiverMap(current->map()) && !current->GetElementsAccessor()->HasAccessors(current);
}

inline bool HasOnlySimpleReceiverElements(Isolate* isolate, Tagged<JSObject> receiver)
{
...
inline bool HasOnlySimpleElements(Isolate* isolate, Tagged<JSReceiver> receiver)
{
...
void MatchArrayElementsKindToArguments(Isolate* isolate, Handle<JSArray> array, BuiltinArguments* args, int first_arg_index, int num_arguments)
{
...
inline bool EnsureJSArrayWithWritableFastElements(Isolate* isolate, Handle<Object> receiver, BuiltinArguments* args, int first_arg_index, int num_arguments)
{
    ...
    if (isolate->IsInitialArrayPrototype(*array))
        return false;

    // Need to ensure that the arguments passed in args can be contained in
    // the array.
    MatchArrayElementsKindToArguments(isolate, array, args, first_arg_index, num_arguments);
    return true;
}
...
V8_WARN_UNUSED_RESULT Maybe<double> GetRelativeIndex(Isolate* isolate, double length, Handle<Object> index, double init_if_undefined)
{
    double relative_index = init_if_undefined;
    if (!IsUndefined(*index)) {
        Handle<Object> relative_index_obj;
        ASSIGN_RETURN_ON_EXCEPTION_VALUE(isolate, relative_index_obj, Object::ToInteger(isolate, index), Nothing<double>());
...
V8_WARN_UNUSED_RESULT Maybe<double> GetLengthProperty(Isolate* isolate, Handle<JSReceiver> receiver)
{
    if (IsJSArray(*receiver)) {
    ...
    ASSIGN_RETURN_ON_EXCEPTION_VALUE(isolate, raw_length_number, Object::GetLengthFromArrayLike(isolate, receiver), Nothing<double>());
...
V8_WARN_UNUSED_RESULT MaybeHandle<Object> SetLengthProperty(Isolate* isolate, Handle<JSReceiver> receiver, double length)
{
    if (IsJSArray(*receiver)) {
        Handle<JSArray> array = Cast<JSArray>(receiver);
    ...
    return Object::SetProperty(isolate, receiver, isolate->factory()->length_string(), isolate->factory()->NewNumber(length), StoreOrigin::kMaybeKeyed,
        Just(ShouldThrow::kThrowOnError));
}

V8_WARN_UNUSED_RESULT Tagged<Object> GenericArrayFill(Isolate* isolate, Handle<JSReceiver> receiver, Handle<Object> value, double start, double end)
{
    // 7. Repeat, while k < final.
    while (start < end) {
        // a. Let Pk be ! ToString(k).
        Handle<String> index = isolate->factory()->NumberToString(isolate->factory()->NewNumber(start));
...
V8_WARN_UNUSED_RESULT Maybe<bool> TryFastArrayFill(
    Isolate* isolate, BuiltinArguments* args, Handle<JSReceiver> receiver, Handle<Object> value, double start_index, double end_index)
{
    ...
    RETURN_ON_EXCEPTION_VALUE(isolate, accessor->Fill(array, value, start, end), Nothing<bool>());
...
BUILTIN(ArrayPrototypeFill)
{
    ...
    double length;
    MAYBE_ASSIGN_RETURN_FAILURE_ON_EXCEPTION(isolate, length, GetLengthProperty(isolate, receiver));
    ...
    double start_index;
    MAYBE_ASSIGN_RETURN_FAILURE_ON_EXCEPTION(isolate, start_index, GetRelativeIndex(isolate, length, start, 0));
    ...
    double end_index;
    MAYBE_ASSIGN_RETURN_FAILURE_ON_EXCEPTION(isolate, end_index, GetRelativeIndex(isolate, length, end, length));
    ...
    bool success;
    MAYBE_ASSIGN_RETURN_FAILURE_ON_EXCEPTION(isolate, success, TryFastArrayFill(isolate, &args, receiver, value, start_index, end_index));
    if (success)
        return *receiver;
...
namespace {
V8_WARN_UNUSED_RESULT Tagged<Object> GenericArrayPush(Isolate* isolate, BuiltinArguments* args)
{
    // 1. Let O be ? ToObject(this value).
    Handle<JSReceiver> receiver;
    ASSIGN_RETURN_FAILURE_ON_EXCEPTION(isolate, receiver, Object::ToObject(isolate, args->receiver()));
    ...
    // 2. Let len be ? ToLength(? Get(O, "length")).
    Handle<Object> raw_length_number;
    ASSIGN_RETURN_FAILURE_ON_EXCEPTION(isolate, raw_length_number, Object::GetLengthFromArrayLike(isolate, receiver));
    ...
    for (int i = 0; i < arg_count; ++i) {
        ...
        if (length <= JSObject::kMaxElementIndex) {
            RETURN_FAILURE_ON_EXCEPTION(isolate, Object::SetElement(isolate, receiver, length, element, ShouldThrow::kThrowOnError));
        } else {
            PropertyKey key(isolate, length);
            LookupIterator it(isolate, receiver, key);
            MAYBE_RETURN(Object::SetProperty(&it, element, StoreOrigin::kMaybeKeyed, Just(ShouldThrow::kThrowOnError)), ReadOnlyRoots(isolate).exception());
        }
    ...
    RETURN_FAILURE_ON_EXCEPTION(isolate,
        Object::SetProperty(isolate, receiver, isolate->factory()->length_string(), final_length, StoreOrigin::kMaybeKeyed, Just(ShouldThrow::kThrowOnError)));
...
BUILTIN(ArrayPush)
{
    ...
    if (!EnsureJSArrayWithWritableFastElements(isolate, receiver, &args, 1, args.length() - 1)) {
    ...
    ElementsAccessor* accessor = array->GetElementsAccessor();
    uint32_t new_length;
    MAYBE_ASSIGN_RETURN_FAILURE_ON_EXCEPTION(isolate, new_length, accessor->Push(array, &args, to_add));
    return *isolate->factory()->NewNumberFromUint((new_length));
}

namespace {

V8_WARN_UNUSED_RESULT Tagged<Object> GenericArrayPop(Isolate* isolate, BuiltinArguments* args)
{
    // 1. Let O be ? ToObject(this value).
    Handle<JSReceiver> receiver;
    ASSIGN_RETURN_FAILURE_ON_EXCEPTION(isolate, receiver, Object::ToObject(isolate, args->receiver()));
    ...
    if (length == 0) {
        ...
        RETURN_FAILURE_ON_EXCEPTION(isolate,
            Object::SetProperty(isolate, receiver, isolate->factory()->length_string(), Handle<Smi>(Smi::zero(), isolate), StoreOrigin::kMaybeKeyed,
                Just(ShouldThrow::kThrowOnError)));
    ...
    ASSIGN_RETURN_FAILURE_ON_EXCEPTION(isolate, element, Object::GetPropertyOrElement(isolate, receiver, index));

    // d. Perform ? DeletePropertyOrThrow(O, index).
    MAYBE_RETURN(JSReceiver::DeletePropertyOrElement(isolate, receiver, index, LanguageMode::kStrict), ReadOnlyRoots(isolate).exception());

    // e. Perform ? Set(O, "length", newLen, true).
    RETURN_FAILURE_ON_EXCEPTION(isolate,
        Object::SetProperty(isolate, receiver, isolate->factory()->length_string(), new_length, StoreOrigin::kMaybeKeyed, Just(ShouldThrow::kThrowOnError)));
...
BUILTIN(ArrayPop)
{
    HandleScope scope(isolate);
    Handle<Object> receiver = args.receiver();
    if (!EnsureJSArrayWithWritableFastElements(isolate, receiver, nullptr, 0, 0)) {
    ...
    if (IsJSArrayFastElementMovingAllowed(isolate, Cast<JSArray>(*receiver))) {
        // Fast Elements Path
        ASSIGN_RETURN_FAILURE_ON_EXCEPTION(isolate, result, array->GetElementsAccessor()->Pop(array));
    } else {
        // Use Slow Lookup otherwise
        uint32_t new_length = len - 1;
        ASSIGN_RETURN_FAILURE_ON_EXCEPTION(isolate, result, JSReceiver::GetElement(isolate, array, new_length));

        // The length could have become read-only during the last GetElement() call,
        // so check again.
        if (JSArray::HasReadOnlyLength(array)) {
            THROW_NEW_ERROR_RETURN_FAILURE(
                isolate, NewTypeError(MessageTemplate::kStrictReadOnlyProperty, isolate->factory()->length_string(), Object::TypeOf(isolate, array), array));
        }
        bool set_len_ok;
        MAYBE_ASSIGN_RETURN_FAILURE_ON_EXCEPTION(isolate, set_len_ok, JSArray::SetLength(array, new_length));
    }
...
V8_WARN_UNUSED_RESULT bool CanUseFastArrayShift(Isolate* isolate, Handle<JSReceiver> receiver)
{
    if (V8_COMPRESS_POINTERS_8GB_BOOL)
        return false;

    if (!EnsureJSArrayWithWritableFastElements(isolate, receiver, nullptr, 0, 0) || !IsJSArrayFastElementMovingAllowed(isolate, Cast<JSArray>(*receiver))) {
        return false;
    }
...
V8_WARN_UNUSED_RESULT Tagged<Object> GenericArrayShift(Isolate* isolate, Handle<JSReceiver> receiver, double length)
{
    // 4. Let first be ? Get(O, "0").
    Handle<Object> first;
    ASSIGN_RETURN_FAILURE_ON_EXCEPTION(isolate, first, Object::GetElement(isolate, receiver, 0));
    ...
    while (k < length) {
        ...
        Handle<String> to = isolate->factory()->NumberToString(isolate->factory()->NewNumber(k - 1));

        // c. Let fromPresent be ? HasProperty(O, from).
        bool from_present;
        MAYBE_ASSIGN_RETURN_FAILURE_ON_EXCEPTION(isolate, from_present, JSReceiver::HasProperty(isolate, receiver, from));

        // d. If fromPresent is true, then.
        if (from_present) {
            // i. Let fromVal be ? Get(O, from).
            Handle<Object> from_val;
            ASSIGN_RETURN_FAILURE_ON_EXCEPTION(isolate, from_val, Object::GetPropertyOrElement(isolate, receiver, from));

            // ii. Perform ? Set(O, to, fromVal, true).
            RETURN_FAILURE_ON_EXCEPTION(isolate, Object::SetPropertyOrElement(isolate, receiver, to, from_val, Just(ShouldThrow::kThrowOnError)));
        } else { // e. Else fromPresent is false,
            // i. Perform ? DeletePropertyOrThrow(O, to).
            MAYBE_RETURN(JSReceiver::DeletePropertyOrElement(isolate, receiver, to, LanguageMode::kStrict), ReadOnlyRoots(isolate).exception());
        }
    ...
    Handle<String> new_length = isolate->factory()->NumberToString(isolate->factory()->NewNumber(length - 1));
    MAYBE_RETURN(JSReceiver::DeletePropertyOrElement(isolate, receiver, new_length, LanguageMode::kStrict), ReadOnlyRoots(isolate).exception());

    // 8. Perform ? Set(O, "length", len-1, true).
    RETURN_FAILURE_ON_EXCEPTION(isolate, SetLengthProperty(isolate, receiver, length - 1));
...
BUILTIN(ArrayShift)
{
    ...
    if (length == 0) {
        // a. Perform ? Set(O, "length", 0, true).
        RETURN_FAILURE_ON_EXCEPTION(isolate, SetLengthProperty(isolate, receiver, length));
    ...
    if (CanUseFastArrayShift(isolate, receiver)) {
        Handle<JSArray> array = Cast<JSArray>(receiver);
        RETURN_RESULT_OR_FAILURE(isolate, array->GetElementsAccessor()->Shift(array));
    }
...
BUILTIN(ArrayUnshift)
{
    ...
    MatchArrayElementsKindToArguments(isolate, array, &args, 1, args.length() - 1);
    ...
    ElementsAccessor* accessor = array->GetElementsAccessor();
    uint32_t new_length;
    MAYBE_ASSIGN_RETURN_FAILURE_ON_EXCEPTION(isolate, new_length, accessor->Unshift(array, &args, to_add));
...
public:
    ArrayConcatVisitor(Isolate* isolate, DirectHandle<UnionOf<JSReceiver, FixedArray, NumberDictionary>> storage, bool fast_elements)
        : isolate_(isolate)
        , storage_(isolate->global_handles()->Create(*storage))
        , index_offset_(0u)
        , bit_field_(FastElementsField::encode(fast_elements) | ExceedsLimitField::encode(false) | IsFixedArrayField::encode(IsFixedArray(*storage, isolate))
              | HasSimpleElementsField::encode(IsFixedArray(*storage, isolate) ||
                  // Don't take fast path for storages that might have
                  // side effects when storing to them.
                  (!IsCustomElementsReceiverMap(storage->map(isolate)) && !IsJSTypedArray(*storage, isolate))))
    {
    ...
    V8_WARN_UNUSED_RESULT bool visit(uint32_t i, Handle<Object> elm)
    {
        ...
        if (!is_fixed_array()) {
            MAYBE_RETURN(JSReceiver::CreateDataProperty(isolate_, Cast<JSReceiver>(storage_), PropertyKey(isolate_, index), elm, Just(kThrowOnError)), false);
            return true;
        }
        ...
        Handle<NumberDictionary> result = NumberDictionary::Set(isolate_, dict, index, elm, not_a_prototype_holder);
    ...
    void increase_index_offset(uint32_t delta)
    {
        ...
        if (fast_elements() && index_offset_ > static_cast<uint32_t>(Cast<FixedArrayBase>(*storage_)->length())) {
            SetDictionaryMode();
        }
    }
    ...
    Handle<JSArray> ToArray()
    {
        ...
        DirectHandle<Number> length = isolate_->factory()->NewNumber(static_cast<double>(index_offset_));
        DirectHandle<Map> map = JSObject::GetElementsTransitionMap(array, fast_elements() ? HOLEY_ELEMENTS : DICTIONARY_ELEMENTS);
        {
            DisallowGarbageCollection no_gc;
    ...
    V8_WARN_UNUSED_RESULT MaybeHandle<JSReceiver> ToJSReceiver()
    {
        ...
        RETURN_ON_EXCEPTION(isolate_,
            Object::SetProperty(isolate_, result, isolate_->factory()->length_string(), length, StoreOrigin::kMaybeKeyed, Just(ShouldThrow::kThrowOnError)));
        return result;
    }
...
private:
    ...
    void SetDictionaryMode()
    {
        ...
        Handle<NumberDictionary> slow_storage(NumberDictionary::New(isolate_, current_storage->length()));
        uint32_t current_length = static_cast<uint32_t>(current_storage->length());
        FOR_WITH_HANDLE_SCOPE(isolate_, uint32_t, i = 0, i, i < current_length, i++, {
            Handle<Object> element(current_storage->get(i), isolate_);
            if (!IsTheHole(*element, isolate_)) {
                // The object holding this backing store has just been allocated, so
                // it cannot yet be used as a prototype.
                Handle<JSObject> not_a_prototype_holder;
                Handle<NumberDictionary> new_storage = NumberDictionary::Set(isolate_, slow_storage, i, element, not_a_prototype_holder);
    ...
    {
        GlobalHandles::Destroy(storage_.location());
    }
    ...
    Isolate* isolate_;
    Handle<UnionOf<JSReceiver, FixedArray, NumberDictionary>> storage_; // Always a global handle.
...
uint32_t EstimateElementCount(Isolate* isolate, DirectHandle<JSArray> array)
{
    ...
    case DICTIONARY_ELEMENTS: {
        Tagged<NumberDictionary> dictionary = Cast<NumberDictionary>(array->elements());
...
void CollectElementIndices(Isolate* isolate, Handle<JSObject> object, uint32_t range, std::vector<uint32_t>* indices)
{
    ...
    case DICTIONARY_ELEMENTS: {
        DisallowGarbageCollection no_gc;
        Tagged<NumberDictionary> dict = Cast<NumberDictionary>(object->elements());
...
#undef TYPED_ARRAY_CASE
    ...
    case SHARED_ARRAY_ELEMENTS: {
        ...
        for (uint32_t i = 0; i < length; i++) {
            ...
            SLOW_DCHECK(object->GetElementsAccessor()->HasElement(*object, i, object->elements()));
            indices->push_back(i);
        }
        if (length == range)
            return;
    ...
    if (!iter.IsAtEnd()) {
        ...
        CollectElementIndices(isolate, PrototypeIterator::GetCurrent<JSObject>(iter), range, indices);
    }
}

bool IterateElementsSlow(Isolate* isolate, Handle<JSReceiver> receiver, uint32_t length, ArrayConcatVisitor* visitor)
{
    FOR_WITH_HANDLE_SCOPE(isolate, uint32_t, i = 0, i, i < length, ++i, {
        ...
        if (maybe.FromJust()) {
            ...
            ASSIGN_RETURN_ON_EXCEPTION_VALUE(isolate, element_value, JSReceiver::GetElement(isolate, receiver, i), false);
...
bool IterateElements(Isolate* isolate, Handle<JSReceiver> receiver, ArrayConcatVisitor* visitor)
{
    ...
    if (IsJSArray(*receiver)) {
        ...
        Handle<Object> val;
        ASSIGN_RETURN_ON_EXCEPTION_VALUE(isolate, val, Object::GetLengthFromArrayLike(isolate, receiver), false);
        if (visitor->index_offset() + Object::NumberValue(*val) > kMaxSafeInteger) {
            isolate->Throw(*isolate->factory()->NewTypeError(MessageTemplate::kInvalidArrayLength));
            return false;
        }
    ...
    case HOLEY_ELEMENTS: {
        ...
        DirectHandle<FixedArray> elements(Cast<FixedArray>(array->elements()), isolate);
        int fast_length = static_cast<int>(length);
        ...
        FOR_WITH_HANDLE_SCOPE(isolate, int, j = 0, j, j < fast_length, j++, {
            ...
            if (!IsTheHole(*element_value, isolate)) {
                ...
                if (maybe.FromJust()) {
                    ...
                    ASSIGN_RETURN_ON_EXCEPTION_VALUE(isolate, element_value, JSReceiver::GetElement(isolate, array, j), false);
                    if (!visitor->visit(j, element_value))
                        return false;
                }
            }
    ...
    case DICTIONARY_ELEMENTS: {
        ...
        FOR_WITH_HANDLE_SCOPE(isolate, size_t, j = 0, j, j < n, (void)0, {
            ...
            Handle<Object> element;
            ASSIGN_RETURN_ON_EXCEPTION_VALUE(isolate, element, JSReceiver::GetElement(isolate, array, index), false);
    ...
    case FAST_SLOPPY_ARGUMENTS_ELEMENTS:
    case SLOW_SLOPPY_ARGUMENTS_ELEMENTS: {
        FOR_WITH_HANDLE_SCOPE(isolate, uint32_t, index = 0, index, index < length, index++, {
            Handle<Object> element;
            ASSIGN_RETURN_ON_EXCEPTION_VALUE(isolate, element, JSReceiver::GetElement(isolate, array, index), false);
            if (!visitor->visit(index, element))
                return false;
...
static Maybe<bool> IsConcatSpreadable(Isolate* isolate, Handle<Object> obj)
{
    ...
    Handle<JSReceiver> receiver;
    if (!TryCast<JSReceiver>(obj, &receiver))
        return Just(false);
    if (!Protectors::IsIsConcatSpreadableLookupChainIntact(isolate) || receiver->HasProxyInPrototype(isolate)) {
        ...
        Handle<Object> value;
        MaybeHandle<Object> maybeValue = i::Runtime::GetObjectProperty(isolate, receiver, key);
...
Tagged<Object> Slow_ArrayConcat(BuiltinArguments* args, Handle<Object> species, Isolate* isolate)
{
    ...
    FOR_WITH_HANDLE_SCOPE(isolate, int, i = 0, i, i < argument_count, i++, {
        ...
        if (IsJSArray(*obj)) {
            ...
            length_estimate = static_cast<uint32_t>(Object::NumberValue(array->length()));
            ...
            if (IsHeapObject(*obj)) {
                kind = GetMoreGeneralElementsKind(kind, IsNumber(*obj) ? PACKED_DOUBLE_ELEMENTS : PACKED_ELEMENTS);
            }
            length_estimate = 1;
            element_estimate = 1;
        }
    ...
    // dictionary.
    bool fast_case = is_array_species && (estimate_nof * 2) >= estimate_result_length && Protectors::IsIsConcatSpreadableLookupChainIntact(isolate);
    ...
    if (fast_case) {
        ...
        ASSIGN_RETURN_FAILURE_ON_EXCEPTION(isolate, storage_object, Execution::New(isolate, species, species, 1, &length));
...
BUILTIN(ArrayConcat)
{
    ...
    ASSIGN_RETURN_FAILURE_ON_EXCEPTION(isolate, receiver, Object::ToObject(isolate, args.receiver(), "Array.prototype.concat"));
    BuiltinArguments::ChangeValueScope set_receiver_value_scope(isolate, &args, BuiltinArguments::kReceiverIndex, *receiver);

    Handle<JSArray> result_array;

    // Avoid a real species read to avoid extra lookups to the array constructor
    if (V8_LIKELY(IsJSArray(*receiver) && Cast<JSArray>(receiver)->HasArrayPrototype(isolate) && Protectors::IsArraySpeciesLookupChainIntact(isolate))) {
    ...
    ASSIGN_RETURN_FAILURE_ON_EXCEPTION(isolate, species, Object::ArraySpeciesConstructor(isolate, receiver));
