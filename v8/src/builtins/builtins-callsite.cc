...
#define CHECK_CALLSITE(frame, method)                                                                                                                          \
    ...
    LookupIterator it(isolate, receiver, isolate->factory()->call_site_info_symbol(), LookupIterator::OWN_SKIP_INTERCEPTOR);                                   \
    if (it.state() != LookupIterator::DATA) {                                                                                                                  \
        THROW_NEW_ERROR_RETURN_FAILURE(isolate, NewTypeError(MessageTemplate::kCallSiteMethod, isolate->factory()->NewStringFromAsciiChecked(method)));        \
    }                                                                                                                                                          \
...
BUILTIN(CallSitePrototypeGetEnclosingColumnNumber)
{
    ...
    return PositiveNumberOrNull(CallSiteInfo::GetEnclosingColumnNumber(frame), isolate);
}
...
BUILTIN(CallSitePrototypeGetEnclosingLineNumber)
{
    ...
    return PositiveNumberOrNull(CallSiteInfo::GetEnclosingLineNumber(frame), isolate);
}
...
BUILTIN(CallSitePrototypeGetFunction)
{
    ...
    if (NativeContextIsForShadowRealm(isolate->raw_native_context())
        || (IsJSFunction(frame->function()) && NativeContextIsForShadowRealm(Cast<JSFunction>(frame->function())->native_context()))) {
        THROW_NEW_ERROR_RETURN_FAILURE(
            isolate, NewTypeError(MessageTemplate::kCallSiteMethodUnsupportedInShadowRealm, isolate->factory()->NewStringFromAsciiChecked(method_name)));
    }
    if (frame->IsStrict() || (IsJSFunction(frame->function()) && Cast<JSFunction>(frame->function())->shared()->is_toplevel())) {
...
BUILTIN(CallSitePrototypeGetThis)
{
    ...
    if (frame->IsAsmJsWasm()) {
        return frame->GetWasmInstance()->trusted_data(isolate)->native_context()->global_proxy();
    }
