...
struct BuiltinMetadata {
    ...
    union KindSpecificData {
        ...
        constexpr KindSpecificData()
            : cpp_entry(kNullAddress)
        {
        }
        constexpr KindSpecificData(Address cpp_entry)
            : cpp_entry(cpp_entry)
        {
        }
        constexpr KindSpecificData(int parameter_count, int /* To disambiguate from above */)
            : parameter_count(static_cast<int16_t>(parameter_count))
        {
        }
        constexpr KindSpecificData(interpreter::Bytecode bytecode, interpreter::OperandScale scale)
...
#define DECL_CPP(Name, Argc) { #Name, Builtins::CPP, { FUNCTION_ADDR(Builtin_##Name) } },
...
#define DECL_BCH(Name, OperandScale, Bytecode) { #Name, Builtins::BCH, { Bytecode, OperandScale } },
#define DECL_ASM(Name, ...) { #Name, Builtins::ASM, {} },
const BuiltinMetadata builtin_metadata[] = { BUILTIN_LIST(DECL_CPP, DECL_TSJ, DECL_TFJ, DECL_TSC, DECL_TFC, DECL_TFS, DECL_TFH, DECL_BCH, DECL_ASM) };
#undef DECL_CPP
#undef DECL_TFJ
#undef DECL_TSC
#undef DECL_TFC
...
{
    DCHECK(Builtins::KindOf(builtin) == TFJ || Builtins::KindOf(builtin) == TFC || Builtins::KindOf(builtin) == TFS);
    return BytecodeOffset(BytecodeOffset::kFirstBuiltinContinuationId + ToInt(builtin));
}

Builtin Builtins::GetBuiltinFromBytecodeOffset(BytecodeOffset id)
{
    Builtin builtin = Builtins::FromInt(id.ToInt() - BytecodeOffset::kFirstBuiltinContinuationId);
    DCHECK(Builtins::KindOf(builtin) == TFJ || Builtins::KindOf(builtin) == TFC || Builtins::KindOf(builtin) == TFS);
    return builtin;
}
...
const char* Builtins::Lookup(Address pc)
{
    ...
    if (!initialized_)
        return nullptr;
    for (Builtin builtin_ix = Builtins::kFirst; builtin_ix <= Builtins::kLast; ++builtin_ix) {
...
FullObjectSlot Builtins::builtin_tier0_slot(Builtin builtin)
{
    ...
    Address* location = &isolate_->builtin_tier0_table()[Builtins::ToInt(builtin)];
    return FullObjectSlot(location);
}
...
void ParameterCountToString(char* buffer, size_t buffer_size, int parameter_count)
{
...
// static
bool Builtins::CheckFormalParameterCount(Builtin builtin, int function_length, int formal_parameter_count_with_receiver)
{
    ...
    if (kind == CPP) {
        ...
        if (parameter_count != formal_parameter_count_with_receiver) {
            if ((false)) {
                ...
                ParameterCountToString(expected_count, kBufSize, formal_parameter_count_with_receiver);
                PrintF("\n##### "
                       "sed -i -z -r 's/%s\\(%s,[\\\\\\n[:space:]]+%s\\)/%s(%s, %s)/g' "
                       "src/builtins/builtins-definitions.h\n",
                    KindNameOf(builtin), name(builtin), actual_count, KindNameOf(builtin), name(builtin), expected_count);
            }
            return false;
        }
    }
...
#define CASE_OTHER(Name, ...)                                                                                                                                  \
    case Builtin::k##Name: {                                                                                                                                   \
        ...
        BUILTIN_LIST(IGNORE_BUILTIN, IGNORE_BUILTIN, IGNORE_BUILTIN, CASE_OTHER, CASE_OTHER, CASE_OTHER, CASE_OTHER, IGNORE_BUILTIN, CASE_OTHER)
#undef CASE_OTHER
    default:
...
void Builtins::PrintBuiltinCode()
{
    ...
    for (Builtin builtin = Builtins::kFirst; builtin <= Builtins::kLast; ++builtin) {
        ...
        if (PassesFilter(base::CStrVector(builtin_name), base::CStrVector(v8_flags.print_builtin_code_filter))) {
...
bool Builtins::IsIsolateIndependentBuiltin(Tagged<Code> code)
{
    ...
    return Builtins::IsBuiltinId(builtin) && Builtins::IsIsolateIndependent(builtin);
}
...
void Builtins::InitializeIsolateDataTables(Isolate* isolate)
{
    ...
    for (Builtin i = Builtins::kFirst; i <= Builtins::kLast; ++i) {
        ...
        isolate_data->builtin_entry_table()[ToInt(i)] = embedded_data.InstructionStartOf(i);
    }

    // T0 tables.
    for (Builtin i = Builtins::kFirst; i <= Builtins::kLastTier0; ++i) {
        const int ii = ToInt(i);
        isolate_data->builtin_tier0_entry_table()[ii] = isolate_data->builtin_entry_table()[ii];
...
void Builtins::EmitCodeCreateEvents(Isolate* isolate)
{
    ...
    for (; i < ToInt(Builtin::kFirstBytecodeHandler); i++) {
        ...
        PROFILE(isolate, CodeCreateEvent(LogEventListener::CodeTag::kBuiltin, code, Builtins::name(FromInt(i))));
    }
    ...
    for (; i < kBuiltinCount; i++) {
        Handle<Code> builtin_code(&builtins[i]);
        Handle<AbstractCode> code = Cast<AbstractCode>(builtin_code);
        interpreter::Bytecode bytecode = builtin_metadata[i].data.bytecode_and_scale.bytecode;
        interpreter::OperandScale scale = builtin_metadata[i].data.bytecode_and_scale.scale;
        PROFILE(isolate, CodeCreateEvent(LogEventListener::CodeTag::kBytecodeHandler, code, interpreter::Bytecodes::ToString(bytecode, scale).c_str()));
    }
}

// static
Handle<Code> Builtins::CreateInterpreterEntryTrampolineForProfiling(Isolate* isolate)
{
    DCHECK_NOT_NULL(isolate->embedded_blob_code());
    DCHECK_NE(0, isolate->embedded_blob_code_size());

    Tagged<Code> code = isolate->builtins()->code(Builtin::kInterpreterEntryTrampolineForProfiling);
...
// static
bool Builtins::AllowDynamicFunction(Isolate* isolate, DirectHandle<JSFunction> target, Handle<JSObject> target_global_proxy)
{
    if (v8_flags.allow_unsafe_function_constructor)
        return true;
