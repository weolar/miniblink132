...
public:
    ...
    // Zero index states for receiver.
    template <class S = Object> Handle<S> at(int index) const
    {
        DCHECK_LT(index, length());
...
static_assert(BuiltinArguments::kNewTargetIndex == BuiltinExitFrameConstants::kNewTargetIndex);
static_assert(BuiltinArguments::kTargetIndex == BuiltinExitFrameConstants::kTargetIndex);
static_assert(BuiltinArguments::kArgcIndex == BuiltinExitFrameConstants::kArgcIndex);
static_assert(BuiltinArguments::kPaddingIndex == BuiltinExitFrameConstants::kPaddingIndex);
...
#define BUILTIN_RCS(name)                                                                                                                                      \
    V8_WARN_UNUSED_RESULT static Tagged<Object> Builtin_Impl_##name(BuiltinArguments args, Isolate* isolate);                                                  \
                                                                                                                                                               \
    V8_NOINLINE static Address Builtin_Impl_Stats_##name(int args_length, Address* args_object, Isolate* isolate)                                              \
    {                                                                                                                                                          \
        BuiltinArguments args(args_length, args_object);                                                                                                       \
        RCS_SCOPE(isolate, RuntimeCallCounterId::kBuiltin_##name);                                                                                             \
        TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("v8.runtime"), "V8.Builtin_" #name);                                                                            \
        return BUILTIN_CONVERT_RESULT(Builtin_Impl_##name(args, isolate));                                                                                     \
    }                                                                                                                                                          \
                                                                                                                                                               \
    V8_WARN_UNUSED_RESULT Address Builtin_##name(int args_length, Address* args_object, Isolate* isolate)                                                      \
    {                                                                                                                                                          \
...
#define CHECK_RECEIVER(Type, name, method)                                                                                                                     \
    if (!Is##Type(*args.receiver())) {                                                                                                                         \
        THROW_NEW_ERROR_RETURN_FAILURE(                                                                                                                        \
            isolate, NewTypeError(MessageTemplate::kIncompatibleMethodReceiver, isolate->factory()->NewStringFromAsciiChecked(method), args.receiver()));      \
    }                                                                                                                                                          \
...
#define TO_THIS_STRING(name, method)                                                                                                                           \
    if (IsNullOrUndefined(*args.receiver(), isolate)) {                                                                                                        \
        THROW_NEW_ERROR_RETURN_FAILURE(                                                                                                                        \
            isolate, NewTypeError(MessageTemplate::kCalledOnNullOrUndefined, isolate->factory()->NewStringFromAsciiChecked(method)));                          \
    }                                                                                                                                                          \
    Handle<String> name;                                                                                                                                       \
    ASSIGN_RETURN_FAILURE_ON_EXCEPTION(isolate, name, Object::ToString(isolate, args.receiver()))
