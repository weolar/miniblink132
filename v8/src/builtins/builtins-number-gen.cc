...
#define DEF_BINOP(Name, Generator)                                                                                                                             \
    ...
    {                                                                                                                                                          \
        ...
        TNode<Object> result = binop_asm.Generator(                                                                                                            \
            [&]() { return context; }, lhs, rhs, slot, [&]() { return feedback_vector; }, UpdateFeedbackMode::kGuaranteedFeedback, false);                     \
                                                                                                                                                               \
        Return(result);                                                                                                                                        \
    }
...
DEF_BINOP(ShiftRightLogical_WithFeedback, Generate_ShiftRightLogicalWithFeedback)
#undef DEF_BINOP
...
#define DEF_BINOP(Name, Generator)                                                                                                                             \
    ...
    {                                                                                                                                                          \
        ...
        auto slot = UncheckedParameter<UintPtrT>(Descriptor::kSlot);                                                                                           \
                                                                                                                                                               \
        BinaryOpAssembler binop_asm(state());                                                                                                                  \
        TNode<Object> result = binop_asm.Generator([&]() { return LoadContextFromBaseline(); }, lhs, rhs, slot,                                                \
            [&]() { return LoadFeedbackVectorFromBaseline(); }, UpdateFeedbackMode::kGuaranteedFeedback, false);                                               \
                                                                                                                                                               \
        Return(result);                                                                                                                                        \
    }
...
#define DEF_BINOP_RHS_SMI(Name, Generator)                                                                                                                     \
    ...
    {                                                                                                                                                          \
        ...
        BinaryOpAssembler binop_asm(state());                                                                                                                  \
        TNode<Object> result = binop_asm.Generator([&]() { return LoadContextFromBaseline(); }, lhs, rhs, slot,                                                \
            [&]() { return LoadFeedbackVectorFromBaseline(); }, UpdateFeedbackMode::kGuaranteedFeedback, true);                                                \
                                                                                                                                                               \
        Return(result);                                                                                                                                        \
    }
...
DEF_BINOP_RHS_SMI(ShiftRightLogicalSmi_Baseline, Generate_ShiftRightLogicalWithFeedback)
...
#define DEF_UNOP(Name, Generator)                                                                                                                              \
    ...
    {                                                                                                                                                          \
        ...
        TNode<Object> result = a.Generator(context, value, slot, feedback_vector, UpdateFeedbackMode::kGuaranteedFeedback);                                    \
                                                                                                                                                               \
        Return(result);                                                                                                                                        \
    }
...
#define DEF_COMPARE(Name)                                                                                                                                      \
    ...
    {                                                                                                                                                          \
        ...
        {                                                                                                                                                      \
            ...
            result = RelationalComparison(Operation::k##Name, lhs, rhs, context, &var_type_feedback);                                                          \
        }                                                                                                                                                      \
    ...
    {                                                                                                                                                          \
        ...
        {                                                                                                                                                      \
            ...
            result = RelationalComparison(                                                                                                                     \
                Operation::k##Name, lhs, rhs, [&]() { return LoadContextFromBaseline(); }, &var_type_feedback);                                                \
        }                                                                                                                                                      \
        auto feedback_vector = LoadFeedbackVectorFromBaseline();                                                                                               \
        UpdateFeedback(var_type_feedback.value(), feedback_vector, slot);                                                                                      \
                                                                                                                                                               \
        ...
        {                                                                                                                                                      \
            auto feedback_vector = LoadFeedbackVectorFromBaseline();                                                                                           \
            UpdateFeedback(var_type_feedback.value(), feedback_vector, slot);                                                                                  \
            CallRuntime(Runtime::kReThrow, LoadContextFromBaseline(), var_exception.value());                                                                  \
            Unreachable();                                                                                                                                     \
        }                                                                                                                                                      \
    }
DEF_COMPARE(LessThan)
...
TF_BUILTIN(Equal_Baseline, CodeStubAssembler)
{
    ...
    TNode<Boolean> result;
    {
        ScopedExceptionHandler handler(this, &if_exception, &var_exception);
        result = Equal(
            lhs, rhs, [&]() { return LoadContextFromBaseline(); }, &var_type_feedback);
    }
    auto feedback_vector = LoadFeedbackVectorFromBaseline();
    UpdateFeedback(var_type_feedback.value(), feedback_vector, slot);
    Return(result);
