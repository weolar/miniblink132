...
void WriteBytecode(std::ofstream& out, Bytecode bytecode, OperandScale operand_scale, int* count, int offset_table[], int table_index)
{
    ...
    if (Bytecodes::BytecodeHasHandler(bytecode, operand_scale)) {
        ...
        out << " \\\n  V(" << name << "Handler, interpreter::OperandScale::k" << operand_scale << ", interpreter::Bytecode::k" << Bytecodes::ToString(bytecode)
            << ")";
...
#define ADD_BYTECODES(Name, ...) WriteBytecode(out, Bytecode::k##Name, operand_scale, &count, offset_table, index++);
...
#undef ADD_BYTECODES
    ...
    CHECK_EQ(single_count, Bytecodes::kBytecodeCount - Bytecodes::kShortStarCount + 1);
    ...
    out << "\n\nconstexpr int kNumberOfBytecodeHandlers = " << single_count << ";\n"
        ...
        << "constexpr uint8_t kIllegalBytecodeHandlerEncoding = " << kIllegalBytecodeHandlerEncoding << ";\n\n"
        << "// Mapping from Bytecode to a dense form with all the illegal\n"
        << "// wide Bytecodes removed. Used to index into the builtins table.\n"
        << "constexpr uint8_t kWideBytecodeToBuiltinsMapping[" << Bytecodes::kBytecodeCount << "] = {    \n";

    for (int i = Bytecodes::kBytecodeCount; i < 2 * Bytecodes::kBytecodeCount; ++i) {
