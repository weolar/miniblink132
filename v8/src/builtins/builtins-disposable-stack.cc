...
BUILTIN(DisposableStackConstructor)
{
    ...
    DirectHandle<JSSyncDisposableStack> disposable_stack = isolate->factory()->NewJSSyncDisposableStack(map);
...
BUILTIN(DisposableStackPrototypeUse)
{
    ...
    ASSIGN_RETURN_FAILURE_ON_EXCEPTION(isolate, method, JSDisposableStackBase::CheckValueAndGetDisposeMethod(isolate, value, DisposeMethodHint::kSyncDispose));

    // 4. Perform ? AddDisposableResource(disposableStack.[[DisposeCapability]],
    //    value, sync-dispose).
    JSDisposableStackBase::Add(isolate, disposable_stack, value, method, DisposeMethodCallType::kValueIsReceiver, DisposeMethodHint::kSyncDispose);
...
BUILTIN(DisposableStackPrototypeDispose)
{
    ...
    ASSIGN_RETURN_FAILURE_ON_EXCEPTION(
        isolate, result, JSDisposableStackBase::DisposeResources(isolate, disposable_stack, MaybeHandle<Object>(), DisposableStackResourcesType::kAllSync));
...
BUILTIN(DisposableStackPrototypeAdopt)
{
    ...
    if (!IsCallable(*on_dispose)) {
        THROW_NEW_ERROR_RETURN_FAILURE(isolate, NewTypeError(MessageTemplate::kNotCallable, on_dispose));
    }
    ...
    JSDisposableStackBase::Add(isolate, disposable_stack, value, on_dispose, DisposeMethodCallType::kValueIsArgument, DisposeMethodHint::kSyncDispose);
...
BUILTIN(DisposableStackPrototypeDefer)
{
    ...
    JSDisposableStackBase::Add(isolate, disposable_stack, ReadOnlyRoots(isolate).undefined_value_handle(), on_dispose, DisposeMethodCallType::kValueIsReceiver,
...
BUILTIN(DisposableStackPrototypeMove)
{
    ...
    Tagged<JSFunction> constructor_function = Cast<JSFunction>(isolate->native_context()->get(Context::JS_DISPOSABLE_STACK_FUNCTION_INDEX));
    DirectHandle<Map> map(constructor_function->initial_map(), isolate);

    DirectHandle<JSSyncDisposableStack> new_disposable_stack = isolate->factory()->NewJSSyncDisposableStack(map);
