...
#else
namespace detail {
template <typename T, size_t N, size_t... I> constexpr std::array<std::remove_cv_t<T>, N> to_array_impl(T (&a)[N], std::index_sequence<I...>)
{
    return { { a[I]... } };
}
} // namespace detail
template <typename T, size_t N> constexpr std::array<std::remove_cv_t<T>, N> to_array(T (&a)[N])
{
...
template <size_t N> class StringLiteral {
public:
...
template <size_t N> StringLiteral(const char (&)[N]) -> StringLiteral<N>;
