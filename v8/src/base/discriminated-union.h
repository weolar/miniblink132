...
//     }
template <typename TagEnum, typename... Ts> class DiscriminatedUnion {
public:
    ...
    template <typename T> constexpr explicit DiscriminatedUnion(Tag tag, T&& data) V8_NOEXCEPT
    {
    ...
    template <typename T> constexpr explicit DiscriminatedUnion(T&& data) V8_NOEXCEPT
    {
        constexpr size_t index = index_of_type_v<std::decay_t<T>, Ts...>;
        static_assert(index < sizeof...(Ts));
        static_assert(index < std::numeric_limits<uint8_t>::max());
    ...
    template <Tag tag> constexpr const auto& get() const
    {
    ...
    // Get union member by tag.
    template <Tag tag> constexpr auto& get()
    {
        using T = nth_type_t<static_cast<size_t>(tag), Ts...>;
        DCHECK_EQ(tag, this->tag());
    ...
    template <typename T> constexpr const auto& get() const
    {
