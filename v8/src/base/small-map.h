...
template <typename NormalMap> class SmallMapDefaultInit {
public:
...
class SmallMap {
    static constexpr size_t kUsingFullMapSentinel = std::numeric_limits<size_t>::max();
...
public:
    ...
    private:
        ...
        V8_INLINE explicit iterator(value_type* init)
            : array_iter_(init)
        {
        }
    ...
    private:
        ...
        V8_INLINE explicit const_iterator(const typename NormalMap::const_iterator& init)
            : array_iter_(nullptr)
            , map_iter_(init)
        {
        }
    ...
    template <typename... Args> std::pair<iterator, bool> emplace(Args&&... args)
    {
        ...
        if (UsingFullMap()) {
            std::pair<typename NormalMap::iterator, bool> ret = map_.emplace(std::forward<Args>(args)...);
            return std::make_pair(iterator(ret.first), ret.second);
        }
    ...
    template <typename... Args> std::pair<iterator, bool> try_emplace(const key_type& key, Args&&... args)
    {
        ...
        if (UsingFullMap()) {
            std::pair<typename NormalMap::iterator, bool> ret = map_.try_emplace(key, std::forward<Args>(args)...);
            return std::make_pair(iterator(ret.first), ret.second);
        }
