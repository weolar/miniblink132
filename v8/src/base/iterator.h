...
template <class Category, class Type, class Diff = std::ptrdiff_t, class Pointer = Type*, class Reference = Type&> struct iterator {
...
template <typename ForwardIterator> class iterator_range {
public:
...
template <typename ForwardIterator> auto make_iterator_range(ForwardIterator begin, ForwardIterator end)
{
    return iterator_range<ForwardIterator> { begin, end };
}

template <class T> struct DerefPtrIterator : base::iterator<std::bidirectional_iterator_tag, T> {
    T* const* ptr;

    explicit DerefPtrIterator(T* const* ptr)
        : ptr(ptr)
    {
    }
...
template <typename T> auto Reversed(T& t)
{
...
// object itself.
template <typename T> auto Reversed(const iterator_range<T>& t)
{
    return make_iterator_range(std::rbegin(t), std::rend(t));
}
...
template <typename T> auto IterateWithoutLast(T& t)
{
...
template <typename T> auto IterateWithoutLast(const iterator_range<T>& t)
{
...
// order. It performs a check whether the container is empty.
template <typename T> auto IterateWithoutFirst(T& t)
{
    DCHECK_NE(std::begin(t), std::end(t));
...
template <typename T> auto IterateWithoutFirst(const iterator_range<T>& t)
{
    iterator_range<T> range_copy = { t.begin(), t.end() };
...
class TupleIterator : public base::iterator<std::bidirectional_iterator_tag, std::tuple<typename std::iterator_traits<Iterators>::reference...>> {
public:
    using value_type = std::tuple<typename std::iterator_traits<Iterators>::reference...>;

    explicit TupleIterator(Iterators... its)
        : its_(its...)
    {
    }
    ...
    {
        return std::apply([](auto&... this_iterators) { return value_type { *this_iterators... }; }, its_);
    }

private:
    template <class Other, size_t... indices> bool not_equal_impl(const Other& other, std::index_sequence<indices...>) const
    {
...
template <class... Containers> auto zip(Containers&... containers)
{
    using TupleIt = TupleIterator<decltype(std::declval<Containers>().begin())...>;
    return base::make_iterator_range(TupleIt(containers.begin()...), TupleIt(containers.end()...));
}
