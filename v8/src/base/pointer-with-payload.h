...
template <typename PointerType> struct PointerWithPayloadTraits {
    static constexpr int kAvailableBits = alignof(PointerType) >= 8 ? 3 : alignof(PointerType) >= 4 ? 2 : 1;
};
...
template <> struct PointerWithPayloadTraits<void> : public PointerWithPayloadTraits<void*> { };
...
template <typename PointerType, typename PayloadType, int NumPayloadBits> class PointerWithPayload {
public:
    ...
    {
        DCHECK_EQ(GetPayload(), payload);
        return reinterpret_cast<PointerType*>(pointer_with_payload_ - static_cast<uintptr_t>(payload));
    }
    ...
    {
        pointer_with_payload_ = reinterpret_cast<uintptr_t>(new_pointer) | static_cast<uintptr_t>(new_payload);
    ...
    V8_INLINE void SetPointer(PointerType* newptr)
    {
        ...
        pointer_with_payload_ = reinterpret_cast<uintptr_t>(newptr) | (pointer_with_payload_ & kPayloadMask);
    ...
    V8_INLINE void SetPayload(PayloadType new_payload)
    {
        ...
        pointer_with_payload_ = (pointer_with_payload_ & kPointerMask) | new_payload_ptr;
        DCHECK_EQ(GetPayload(), new_payload);
    }

    uintptr_t raw() const
    {
        return pointer_with_payload_;
    }

private:
    static constexpr int kAvailableBits = PointerWithPayloadTraits<typename std::remove_const<PointerType>::type>::kAvailableBits;
    static_assert(kAvailableBits >= NumPayloadBits,
    ...
    static constexpr uintptr_t kPayloadMask = (uintptr_t { 1 } << NumPayloadBits) - 1;
