...
public:
    ...
    base::AddressRegion GetOverlap(AddressRegion region) const
    {
        ...
        Address overlap_end = std::max(overlap_start, std::min(end(), region.end()));
...
template <typename T> inline AddressRegion AddressRegionOf(T* ptr, size_t size)
{
    return AddressRegion { reinterpret_cast<AddressRegion::Address>(ptr), sizeof(T) * size };
}

// Construct an AddressRegion from anything providing a {data()} and {size()}
// accessor.
template <typename Container> inline auto AddressRegionOf(Container&& c) -> decltype(AddressRegionOf(c.data(), c.size()))
{
...
{
    return out << "[" << reinterpret_cast<void*>(region.begin()) << "+" << region.size() << "]";
}
