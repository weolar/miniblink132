...
#error "ASAN_POISON_MEMORY_REGION and ASAN_UNPOISON_MEMORY_REGION must be defined"
#endif
...
#define ASAN_CHECK_WHOLE_MEMORY_REGION_IS_POISONED(start, size)                                                                                                \
    do {                                                                                                                                                       \
        for (size_t i = 0; i < size; i++) {                                                                                                                    \
            CHECK(__asan_address_is_poisoned(reinterpret_cast<const char*>(start) + i));                                                                       \
        }                                                                                                                                                      \
    } while (0)
...
public:
    AsanUnpoisonScope(const void* addr, size_t size)
        : addr_(addr)
        , size_(size)
        , was_poisoned_(__asan_region_is_poisoned(const_cast<void*>(addr_), size_))
    {
        if (was_poisoned_) {
...
#define ASAN_POISON_MEMORY_REGION(start, size)                                                                                                                 \
    static_assert(std::is_pointer<decltype(start)>::value, "static type violation");                                                                           \
    static_assert(std::is_convertible<decltype(size), size_t>::value, "static type violation");                                                                \
    USE(start, size)

#define ASAN_UNPOISON_MEMORY_REGION(start, size) ASAN_POISON_MEMORY_REGION(start, size)

#define ASAN_CHECK_WHOLE_MEMORY_REGION_IS_POISONED(start, size) ASAN_POISON_MEMORY_REGION(start, size)
