...
private:
    template <class T, class GetIntrusiveSetIndex, class Container> friend class IntrusiveSet;
...
// Contain(T): O(1)
// Remove(T):  O(1)
template <class T, class GetIntrusiveSetIndex, class Container> class IntrusiveSet {
public:
    // This is not needed for soundness, but rather serves as a hint that `T`
    // should be a lightweight pointer-like value.
    static_assert(std::is_trivially_copyable_v<T>);

    explicit IntrusiveSet(Container container, GetIntrusiveSetIndex index_functor = {})
        : elements_(std::move(container))
        , index_functor_(index_functor)
    {
        static_assert(std::is_same_v<decltype(index_functor(std::declval<T>())), IntrusiveSetIndex&>);
    }

    bool Contains(T x) const
    {
        return Index(x) != IntrusiveSetIndex::kNotInSet;
    }
    ...
    class iterator {
    public:
        explicit iterator(const IntrusiveSet& set)
            : set_(set)
        {
        }
        T operator*()
        ...
        iterator& operator++()
        {
            ...
            if (index_ < set_.elements_.size() && last_index_location_ == &set_.Index(set_.elements_[index_])) {
                index_++;
            }
            return *this;
        }
