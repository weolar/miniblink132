...
RegionAllocator::RegionAllocator(Address memory_region_begin, size_t memory_region_size, size_t page_size)
    : whole_region_(memory_region_begin, memory_region_size, RegionState::kFree)
    , region_size_in_pages_(size() / page_size)
    , max_load_for_randomization_(static_cast<size_t>(size() * kMaxLoadFactorForRandomization))
    , free_size_(0)
...
RegionAllocator::AllRegionsSet::iterator RegionAllocator::FindRegion(Address address)
{
...
RegionAllocator::Region* RegionAllocator::Split(Region* region, size_t new_size)
{
...
void RegionAllocator::Merge(AllRegionsSet::iterator prev_iter, AllRegionsSet::iterator next_iter)
{
    ...
    if (on_merge_)
        on_merge_(prev->begin(), prev->size() + next->size());
...
RegionAllocator::Address RegionAllocator::AllocateRegion(RandomNumberGenerator* rng, size_t size)
{
...
bool RegionAllocator::AllocateRegionAt(Address requested_address, size_t size, RegionState region_state)
{
...
RegionAllocator::Address RegionAllocator::AllocateAlignedRegion(size_t size, size_t alignment)
{
    DCHECK(IsAligned(size, page_size_));
...
RegionAllocator::Address RegionAllocator::AllocateRegion(Address hint, size_t size, size_t alignment)
{
