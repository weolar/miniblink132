...
template <typename Function> inline void VLQEncodeUnsigned(Function&& process_byte, uint32_t value)
{
    ...
    if (value < 1 << (kDataBitsPerByte))
        goto write_one_byte;
    if (value < 1 << (2 * kDataBitsPerByte))
        goto write_two_bytes;
    if (value < 1 << (3 * kDataBitsPerByte))
        goto write_three_bytes;
    if (value < 1 << (4 * kDataBitsPerByte))
...
template <typename Function> inline void VLQEncode(Function&& process_byte, int32_t value)
{
...
template <typename A> inline void VLQEncode(std::vector<uint8_t, A>* data, int32_t value)
{
...
template <typename A> inline void VLQEncodeUnsigned(std::vector<uint8_t, A>* data, uint32_t value)
{
...
inline typename std::enable_if<std::is_same<decltype(std::declval<GetNextFunction>()()), uint8_t>::value, uint32_t>::type VLQDecodeUnsigned(
    GetNextFunction&& get_next)
{
