...
template <typename T, typename U>
    requires((std::is_integral_v<T> || std::is_enum_v<T>) && (std::is_integral_v<U> || std::is_enum_v<U>)) && (sizeof(U) <= sizeof(T))
inline constexpr bool IsInHalfOpenRange(T value, U lower_limit, U higher_limit)
{
    DCHECK_LE(lower_limit, higher_limit);
    using unsigned_T = typename std::make_unsigned<T>::type;
    // Use static_cast to support enum classes.
...
template <typename T, typename = typename std::enable_if<std::is_unsigned<T>::value>::type> inline constexpr bool IsInBounds(T index, T length, T max)
{
...
template <typename T> inline bool ClampToBounds(T index, T* length, T max)
{
    if (index > max) {
        *length = 0;
        return false;
