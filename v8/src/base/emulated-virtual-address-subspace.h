 ...
 */
class V8_BASE_EXPORT EmulatedVirtualAddressSubspace final : public NON_EXPORTED_BASE(::v8::VirtualAddressSpace) {
public:
...
private:
    ...
    {
        return base() + mapped_size_;
    }

    bool Contains(Address outer_start, size_t outer_size, Address inner_start, size_t inner_size) const
    {
        return (inner_start >= outer_start) && ((inner_start + inner_size) <= (outer_start + outer_size));
    }
