...
template <class E, class T = int> class EnumSet {
...
public:
    ...
    {
        return EnumSet(~bits_);
    }

    constexpr bool operator==(EnumSet set) const
    {
        return bits_ == set.bits_;
    }
    constexpr bool operator!=(EnumSet set) const
    {
        return bits_ != set.bits_;
...
template <typename E, typename T> std::ostream& operator<<(std::ostream& os, EnumSet<E, T> set)
{
    os << "{";
    bool first = true;
    while (!set.empty()) {
