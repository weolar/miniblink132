...
template <class T, int shift, int size, class U = uint32_t> class BitField final {
public:
    ...
    template <class T2, int size2> using Next = BitField<T2, kShift + kSize, size2, U>;
...
template <typename A, typename B> class BitFieldUnion final {
public:
    static_assert(std::is_same<typename A::BaseType, typename B::BaseType>::value);
    ...
    static constexpr int kSize = A::kSize + B::kSize + (std::max(A::kShift, B::kShift) - kShift);
};

template <class T, int shift, int size> using BitField8 = BitField<T, shift, size, uint8_t>;

template <class T, int shift, int size> using BitField16 = BitField<T, shift, size, uint16_t>;

template <class T, int shift, int size> using BitField64 = BitField<T, shift, size, uint64_t>;
...
//
#define DEFINE_BIT_FIELD_RANGE_TYPE(Name, Type, Size, _) k##Name##Start, k##Name##End = k##Name##Start + Size - 1,
...
#define DEFINE_BIT_FIELD_TYPE(Name, Type, Size, RangesName) using Name = base::BitField<Type, RangesName::k##Name##Start, Size>;

#define DEFINE_BIT_FIELD_64_TYPE(Name, Type, Size, RangesName) using Name = base::BitField64<Type, RangesName::k##Name##Start, Size>;
...
//
template <class T, int kBitsPerItem, int kBitsPerWord, class U> class BitSetComputer {
public:
