...
template <class Derived, class VarType> class V8_EXPORT_PRIVATE ContextualVariable {
public:
    using VarT = VarType;
    ...
    private:
        ...
        static_assert(std::is_base_of<ContextualVariable, Derived>::value, "Curiously Recurring Template Pattern");
...
#define EXPORT_CONTEXTUAL_VARIABLE(VarName)                                                                                                                    \
    namespace v8::base {                                                                                                                                       \
    template <> V8_EXPORT_PRIVATE typename VarName::Scope*& ContextualVariable<VarName, typename VarName::VarT>::ExportedTop()                                 \
    {                                                                                                                                                          \
        return top_;                                                                                                                                           \
    }                                                                                                                                                          \
    }
...
template <class T> using ContextualClass = ContextualVariable<T, T>;
...
class V8_EXPORT_PRIVATE ContextualVariableWithDefault : public ContextualVariable<Derived, VarType> {
public:
    static VarType& Get()
    {
...
#define DECLARE_CONTEXTUAL_VARIABLE_WITH_DEFAULT(VarName, ...)                                                                                                 \
    struct VarName : ::v8::base::ContextualVariableWithDefault<VarName, __VA_ARGS__> { }
