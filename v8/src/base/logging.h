...
#else
[[noreturn]] PRINTF_FORMAT(1, 2) V8_BASE_EXPORT V8_NOINLINE void V8_Fatal(const char* format, ...);
...
#define FATAL(...)                                                                                                                                             \
    FATAL_HELPER(__VA_ARGS__, V8_Fatal, V8_Fatal, V8_Fatal, V8_Fatal, V8_Fatal, V8_Fatal, FATAL_DISCARD_ARG)                                                   \
    (__VA_ARGS__)
...
V8_BASE_EXPORT void SetDcheckFunction(void (*dcheck_Function)(const char*, int, const char*));

// Override the default function invoked during V8_Fatal.
V8_BASE_EXPORT void SetFatalFunction(void (*fatal_Function)(const char*, int, const char*));
...
#define DCHECK_WITH_LOC(condition, loc) DCHECK_WITH_MSG_AND_LOC(condition, #condition, loc)
...
#define CHECK_OP(name, op, lhs, rhs)                                                                                                                           \
    do {                                                                                                                                                       \
        if (std::string* _msg = ::v8::base::Check##name##Impl<typename ::v8::base::pass_value_or_ref<decltype(lhs)>::type,                                     \
                typename ::v8::base::pass_value_or_ref<decltype(rhs)>::type>((lhs), (rhs), #lhs " " #op " " #rhs)) {                                           \
...
#define CHECK_OP(name, op, lhs, rhs)                                                                                                                           \
    do {                                                                                                                                                       \
        bool _cmp = ::v8::base::Cmp##name##Impl<typename ::v8::base::pass_value_or_ref<decltype(lhs)>::type,                                                   \
            typename ::v8::base::pass_value_or_ref<decltype(rhs)>::type>((lhs), (rhs));                                                                        \
...
namespace detail {
template <typename... Ts> std::string PrintToString(Ts&&... ts)
{
...
template <typename T> auto GetUnderlyingEnumTypeForPrinting(T val)
{
...
template <typename T> std::string PrintCheckOperand(T val)
{
    return "<unprintable>";
}
...
std::string PrintCheckOperand(T val)
{
    std::string int_str = detail::PrintToString(detail::GetUnderlyingEnumTypeForPrinting(val));
    if constexpr (has_output_operator<T, CheckMessageStream>) {
        ...
        if ((val_str.length() == 1 && !std::isprint(val_str[0])) || val_str == int_str) {
            return int_str;
        }
...
#define DEFINE_PRINT_CHECK_OPERAND_CHAR(type)                                                                                                                  \
    template <> V8_BASE_EXPORT std::string PrintCheckOperand<type>(type ch);                                                                                   \
    template <> V8_BASE_EXPORT std::string PrintCheckOperand<type*>(type * cstr);                                                                              \
...
template <typename Lhs, typename Rhs> V8_NOINLINE std::string* MakeCheckOpString(Lhs lhs, Rhs rhs, char const* msg)
{
    ...
    if (lhs_str.size() <= kMaxInlineLength && rhs_str.size() <= kMaxInlineLength) {
...
#define EXPLICIT_CHECK_OP_INSTANTIATION(type)                                                                                                                  \
    extern template V8_BASE_EXPORT std::string* MakeCheckOpString<type, type>(type, type, char const*);                                                        \
...
template <typename T> struct comparison_underlying_type {
    ...
    using underlying = typename std::underlying_type<typename std::conditional<is_enum, decay, Dummy>::type>::type;
    using type_or_bool = typename std::conditional<is_enum, underlying, decay>::type;
    using type = typename std::conditional<std::is_same_v<type_or_bool, bool>, unsigned int, type_or_bool>::type;
};
// Cast a value to its underlying type
#define MAKE_UNDERLYING(Type, value) static_cast<typename comparison_underlying_type<Type>::type>(value)

// is_signed_vs_unsigned::value is true if both types are integral, Lhs is
// signed, and Rhs is unsigned. False in all other cases.
template <typename Lhs, typename Rhs> struct is_signed_vs_unsigned {
    ...
    static constexpr bool value
        = std::is_integral_v<lhs_underlying> && std::is_integral_v<rhs_underlying> && std::is_signed_v<lhs_underlying> && std::is_unsigned_v<rhs_underlying>;
};
// Same thing, other way around: Lhs is unsigned, Rhs signed.
template <typename Lhs, typename Rhs> struct is_unsigned_vs_signed : public is_signed_vs_unsigned<Rhs, Lhs> { };
...
#define DEFINE_CMP_IMPL(NAME, op)                                                                                                                              \
    template <typename Lhs, typename Rhs> V8_INLINE constexpr bool Cmp##NAME##Impl(Lhs lhs, Rhs rhs)                                                           \
    {                                                                                                                                                          \
        return lhs op rhs;                                                                                                                                     \
    }
DEFINE_CMP_IMPL(EQ, ==)
...
// `requires` clause).
#define MAKE_UNSIGNED(Type, value) static_cast<typename std::make_unsigned<typename comparison_underlying_type<Type>::type>::type>(value)
...
DEFINE_SIGNED_MISMATCH_COMP(is_signed_vs_unsigned, EQ, lhs >= 0 && MAKE_UNSIGNED(Lhs, lhs) == MAKE_UNDERLYING(Rhs, rhs))
DEFINE_SIGNED_MISMATCH_COMP(is_signed_vs_unsigned, LT, lhs < 0 || MAKE_UNSIGNED(Lhs, lhs) < MAKE_UNDERLYING(Rhs, rhs))
DEFINE_SIGNED_MISMATCH_COMP(is_signed_vs_unsigned, LE, lhs <= 0 || MAKE_UNSIGNED(Lhs, lhs) <= MAKE_UNDERLYING(Rhs, rhs))
...
#define DEFINE_CHECK_OP_IMPL(NAME)                                                                                                                             \
    template <typename Lhs, typename Rhs> V8_INLINE constexpr std::string* Check##NAME##Impl(Lhs lhs, Rhs rhs, char const* msg)                                \
    {                                                                                                                                                          \
        ...
        return V8_LIKELY(cmp) ? nullptr : MakeCheckOpString<LhsPassT, RhsPassT>(lhs, rhs, msg);                                                                \
    }
...
template <typename T> constexpr std::make_unsigned_t<T> ToUnsigned(T val)
{
...
#define CHECK_IMPLIES(lhs, rhs) CHECK_WITH_MSG(!(lhs) || (rhs), #lhs " implies " #rhs)
// Performs a single (unsigned) comparison to check that {index} is
// in range [0, limit).
#define CHECK_BOUNDS(index, limit) CHECK_LT(v8::base::ToUnsigned(index), v8::base::ToUnsigned(limit))
...
#define DCHECK_BOUNDS(index, limit) DCHECK_LT(v8::base::ToUnsigned(index), v8::base::ToUnsigned(limit))
#else
