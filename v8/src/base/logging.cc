...
#define DEFINE_PRINT_CHECK_OPERAND_CHAR(type)                                                                                                                  \
    ...
    template <> std::string PrintCheckOperand<type*>(type * cstr)                                                                                              \
    {                                                                                                                                                          \
        return PrintCheckOperand<void*>(cstr);                                                                                                                 \
    }                                                                                                                                                          \
    template <> std::string PrintCheckOperand<const type*>(const type* cstr)                                                                                   \
    {                                                                                                                                                          \
...
public:
    explicit FailureMessage(const char* format, va_list arguments)
    {
        ...
        v8::base::OS::VSNPrintF(&message_[0], arraysize(message_), format, arguments);
    }
...
void V8_Dcheck(const char* file, int line, const char* message)
{
    if (v8::base::DcheckFailuresAreIgnored()) {
        ...
        v8::base::OS::PrintError("# Ignoring debug check failure in %s, line %d: %s\n", file, line, message);
        return;
    }
