...
static V8_INLINE void __cpuidex(int cpu_info[4], int info_type, int sub_info_type)
{
...
#else
    __asm__ volatile("cpuid \n\t" : "=a"(cpu_info[0]), "=b"(cpu_info[1]), "=c"(cpu_info[2]), "=d"(cpu_info[3]) : "a"(info_type), "c"(sub_info_type));
#endif // defined(__i386__) && defined(__pic__)
}

#endif // !V8_LIBC_MSVCRT

#elif V8_HOST_ARCH_ARM || V8_HOST_ARCH_ARM64 || V8_HOST_ARCH_MIPS64 || V8_HOST_ARCH_RISCV64
    ...
    , has_zbs_(false)
{
    ...
    if (num_ids > 0) {
        ...
        if (family_ == 0x6) {
            ...
            case 0x5E:
                ...
                break;
            case 0x8E:
                has_intel_jcc_erratum_ = stepping_ == 0x9 || stepping_ == 0xA || stepping_ == 0xB || stepping_ == 0xC;
                break;
            case 0x9E:
                has_intel_jcc_erratum_ = stepping_ == 0x9 || stepping_ == 0xA || stepping_ == 0xB || stepping_ == 0xD;
                break;
            case 0xA6:
    ...
    has_jscvt_ = IsProcessorFeaturePresent(PF_ARM_V83_JSCVT_INSTRUCTIONS_AVAILABLE);
    ...
    if (sysctlbyname("hw.optional.arm.FEAT_JSCVT", &feat_jscvt, &feat_jscvt_size, nullptr, 0) == -1) {
        has_jscvt_ = false;
    } else {
    ...
    if (sysctlbyname("hw.optional.arm.FEAT_DotProd", &feat_dot_prod, &feat_dot_prod_size, nullptr, 0) == -1) {
        has_dot_prod_ = false;
    } else {
    ...
    if (sysctlbyname("hw.optional.arm.FEAT_LSE", &feat_lse, &feat_lse_size, nullptr, 0) == -1) {
        has_lse_ = false;
    } else {
        has_lse_ = feat_lse;
    }
    ...
    if (sysctlbyname("hw.optional.arm.FEAT_PMULL", &feat_pmull, &feat_pmull_size, nullptr, 0) == -1) {
        has_pmull1q_ = false;
    } else {
    ...
    if (sysctlbyname("hw.optional.arm.FEAT_FP16", &fp16, &fp16_size, nullptr, 0) == -1) {
        has_fp16_ = false;
    } else {
        has_fp16_ = fp16;
    }
...
#include <asm/unistd.h>
    ...
    if (!syscall(__NR_riscv_hwprobe, &pairs, sizeof(pairs) / sizeof(riscv_hwprobe), 0, nullptr, 0)) {
