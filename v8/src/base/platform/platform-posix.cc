...
int GetFlagsForMemoryPermission(OS::MemoryPermission access, PageType page_type)
{
    ...
    if (access == OS::MemoryPermission::kNoAccess || access == OS::MemoryPermission::kNoAccessWillJitLater) {
    ...
    if (access == OS::MemoryPermission::kNoAccessWillJitLater || access == OS::MemoryPermission::kReadWriteExecute) {
        flags |= MAP_JIT;
    }
#endif // V8_OS_DARWIN
    return flags;
...
void* Allocate(void* hint, size_t size, OS::MemoryPermission access, PageType page_type)
{
    ...
    if (result != nullptr && size >= kHugePageSize) {
        const uintptr_t huge_start = RoundUp(reinterpret_cast<uintptr_t>(result), kHugePageSize);
        const uintptr_t huge_end = RoundDown(reinterpret_cast<uintptr_t>(result) + size, kHugePageSize);
        if (huge_end > huge_start) {
            // Bail out in case the aligned addresses do not provide a block of at
            // least kHugePageSize size.
            madvise(reinterpret_cast<void*>(huge_start), huge_end - huge_start, MADV_HUGEPAGE);
        }
    }
#endif
...
// static
void* OS::AllocateShared(void* hint, size_t size, MemoryPermission access, PlatformSharedMemoryHandle handle, uint64_t offset)
{
    DCHECK_EQ(0, size % AllocatePageSize());
    int prot = GetProtectionFromMemoryPermission(access);
...
bool OS::DecommitPages(void* address, size_t size)
{
    ...
    void* ret = mmap(address, size, PROT_NONE, MAP_FIXED | MAP_ANONYMOUS | MAP_PRIVATE, -1, 0);
...
PlatformSharedMemoryHandle OS::CreateSharedMemoryHandleForTesting(size_t size)
{
    ...
    memfd_create_t memfd_create = reinterpret_cast<memfd_create_t>(dlsym(RTLD_DEFAULT, "memfd_create"));
    int fd = -1;
    if (memfd_create) {
...
OS::MemoryMappedFile* OS::MemoryMappedFile::create(const char* name, size_t size, void* initial)
{
    if (FILE* file = fopen(name, "w+")) {
        ...
        if (result == size && !ferror(file)) {
            void* memory = mmap(OS::GetRandomMmapAddr(), result, PROT_READ | PROT_WRITE, MAP_SHARED, fileno(file), 0);
            if (memory != MAP_FAILED) {
...
bool AddressSpaceReservation::AllocateShared(void* address, size_t size, OS::MemoryPermission access, PlatformSharedMemoryHandle handle, uint64_t offset)
{
    ...
    int prot = GetProtectionFromMemoryPermission(access);
    int fd = FileDescriptorFromSharedMemoryHandle(handle);
    return mmap(address, size, prot, MAP_SHARED | MAP_FIXED, fd, offset) != MAP_FAILED;
}
#endif // !defined(V8_OS_DARWIN)
...
{
    DCHECK(Contains(address, size));
    return mmap(address, size, PROT_NONE, MAP_FIXED | MAP_ANONYMOUS | MAP_PRIVATE, -1, 0) == address;
}
#endif // !V8_OS_ZOS
    ...
    , start_semaphore_(nullptr)
{
    ...
    if (stack_size_ > 0)
        stack_size_ = std::max(stack_size_, min_stack_size);
