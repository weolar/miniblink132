...
static_assert(offsetof(V8_CRITICAL_SECTION, DebugInfo) == offsetof(CRITICAL_SECTION, DebugInfo));
static_assert(offsetof(V8_CRITICAL_SECTION, LockCount) == offsetof(CRITICAL_SECTION, LockCount));
static_assert(offsetof(V8_CRITICAL_SECTION, RecursionCount) == offsetof(CRITICAL_SECTION, RecursionCount));
static_assert(offsetof(V8_CRITICAL_SECTION, OwningThread) == offsetof(CRITICAL_SECTION, OwningThread));
static_assert(offsetof(V8_CRITICAL_SECTION, LockSemaphore) == offsetof(CRITICAL_SECTION, LockSemaphore));
static_assert(offsetof(V8_CRITICAL_SECTION, SpinCount) == offsetof(CRITICAL_SECTION, SpinCount));
...
public:
    ...
    void InitializeIfNeeded()
    {
        ...
        WideCharToMultiByte(CP_UTF8, 0, tzinfo_.StandardName, -1, std_tz_name_, kTzNameSize, nullptr, nullptr);
        std_tz_name_[kTzNameSize - 1] = '\0';
        WideCharToMultiByte(CP_UTF8, 0, tzinfo_.DaylightName, -1, dst_tz_name_, kTzNameSize, nullptr, nullptr);
        ...
        if (std_tz_name_[0] == '\0' || std_tz_name_[0] == '@') {
            OS::SNPrintF(std_tz_name_, kTzNameSize - 1, "%s Standard Time", GuessTimezoneNameFromBias(tzinfo_.Bias));
        }
        if (dst_tz_name_[0] == '\0' || dst_tz_name_[0] == '@') {
            OS::SNPrintF(dst_tz_name_, kTzNameSize - 1, "%s Daylight Time", GuessTimezoneNameFromBias(tzinfo_.Bias));
        }
...
int OS::GetUserTime(uint32_t* secs, uint32_t* usecs)
{
    ...
    if (!GetThreadTimes(GetCurrentThread(), &dummy, &dummy, &dummy, reinterpret_cast<FILETIME*>(&usertime)))
        return -1;
...
int OS::GetPeakMemoryUsageKb()
{
    ...
    ret = GetProcessMemoryInfo(GetCurrentProcess(), &mem_counters, sizeof(mem_counters));
    if (ret == 0)
        return -1;
...
DEFINE_LAZY_LEAKY_OBJECT_GETTER(RandomNumberGenerator, GetPlatformRandomNumberGenerator)
...
{
    auto is_user_cet_available_in_environment = reinterpret_cast<decltype(&IsUserCetAvailableInEnvironment)>(
        ::GetProcAddress(::GetModuleHandleW(L"kernel32.dll"), "IsUserCetAvailableInEnvironment"));
    auto get_process_mitigation_policy
        = reinterpret_cast<decltype(&GetProcessMitigationPolicy)>(::GetProcAddress(::GetModuleHandle(L"Kernel32.dll"), "GetProcessMitigationPolicy"));

    if (!is_user_cet_available_in_environment || !get_process_mitigation_policy) {
        return false;
    }

    if (!is_user_cet_available_in_environment(USER_CET_ENVIRONMENT_WIN32_PROCESS)) {
        return false;
    }

    PROCESS_MITIGATION_USER_SHADOW_STACK_POLICY uss_policy;
    if (!get_process_mitigation_policy(GetCurrentProcess(), ProcessUserShadowStackPolicy, &uss_policy, sizeof(uss_policy))) {
        return false;
    }
...
void OS::EnsureWin32MemoryAPILoaded()
{
    ...
    if (!loaded) {
        VirtualAlloc2 = (VirtualAlloc2_t)GetProcAddress(GetModuleHandle(L"kernelbase.dll"), "VirtualAlloc2");

        MapViewOfFile3 = (MapViewOfFile3_t)GetProcAddress(GetModuleHandle(L"kernelbase.dll"), "MapViewOfFile3");

        UnmapViewOfFile2 = (UnmapViewOfFile2_t)GetProcAddress(GetModuleHandle(L"kernelbase.dll"), "UnmapViewOfFile2");
...
void* VirtualAllocWrapper(void* address, size_t size, DWORD flags, DWORD protect)
{
    if (VirtualAlloc2) {
        return VirtualAlloc2(GetCurrentProcess(), address, size, flags, protect, NULL, 0);
    } else {
        return VirtualAlloc(address, size, flags, protect);
    }
}

uint8_t* VirtualAllocWithHint(size_t size, DWORD flags, DWORD protect, void* hint)
{
...
void* AllocateInternal(void* hint, size_t size, size_t alignment, size_t page_size, DWORD flags, DWORD protect)
{
    ...
    for (int i = 0; i < kMaxAttempts; ++i) {
        ...
        CHECK(VirtualFree(base, 0, MEM_RELEASE));
        aligned_base = reinterpret_cast<uint8_t*>(RoundUp(reinterpret_cast<uintptr_t>(base), alignment));
        base = reinterpret_cast<uint8_t*>(VirtualAllocWrapper(aligned_base, size, flags, protect));
...
void* OS::Allocate(void* hint, size_t size, size_t alignment, MemoryPermission access)
{
    ...
    DWORD flags = (access == OS::MemoryPermission::kNoAccess) ? MEM_RESERVE : MEM_RESERVE | MEM_COMMIT;
...
// static
void* OS::AllocateShared(void* hint, size_t size, MemoryPermission permission, PlatformSharedMemoryHandle handle, uint64_t offset)
{
    ...
    void* result = MapViewOfFileEx(file_mapping, access, off_hi, off_lo, size, hint);
...
bool OS::DiscardSystemPages(void* address, size_t size)
{
    ...
    static std::atomic<DiscardVirtualMemoryFunction> discard_virtual_memory(reinterpret_cast<DiscardVirtualMemoryFunction>(-1));
    if (discard_virtual_memory == reinterpret_cast<DiscardVirtualMemoryFunction>(-1))
        discard_virtual_memory = reinterpret_cast<DiscardVirtualMemoryFunction>(GetProcAddress(GetModuleHandle(L"Kernel32.dll"), "DiscardVirtualMemory"));
...
// static
std::optional<AddressSpaceReservation> OS::CreateAddressSpaceReservation(void* hint, size_t size, size_t alignment, MemoryPermission max_permission)
{
    ...
    void* reservation = AllocateInternal(hint, size, alignment, page_size, MEM_RESERVE | MEM_RESERVE_PLACEHOLDER, PAGE_NOACCESS);
    if (!reservation)
        return {};
...
PreciseSleepTimer& PreciseSleepTimer::operator=(PreciseSleepTimer&& other) V8_NOEXCEPT
{
    Close();
    timer_ = other.timer_;
    other.timer_ = NULL;
...
bool PreciseSleepTimer::IsInitialized() const
{
    return timer_ != NULL;
}
...
void PreciseSleepTimer::TryInit()
{
    ...
    timer_ = CreateWaitableTimerExW(NULL, // Cannot be inherited by child processes
...
void PreciseSleepTimer::Sleep(TimeDelta interval) const
{
    ...
    CHECK(SetWaitableTimer(timer_, &due_time, period, completion_routine, arg_to_completion_routine, resume));
...
OS::MemoryMappedFile* OS::MemoryMappedFile::open(const char* name, FileMode mode)
{
    ...
    HANDLE file = CreateFileW(utf16_name.c_str(), access, FILE_SHARE_READ | FILE_SHARE_WRITE, nullptr, OPEN_EXISTING, 0, nullptr);
...
OS::MemoryMappedFile* OS::MemoryMappedFile::create(const char* name, size_t size, void* initial)
{
    std::wstring utf16_name = ConvertUtf8StringToUtf16(name);
    // Open a physical file.
    HANDLE file = CreateFileW(utf16_name.c_str(), GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, nullptr, OPEN_ALWAYS, 0, nullptr);
    if (file == nullptr)
        return nullptr;
    if (size == 0)
        return new Win32MemoryMappedFile(file, nullptr, nullptr, 0);
    // Create a file mapping for the physical file.
    HANDLE file_mapping = CreateFileMapping(file, nullptr, PAGE_READWRITE, 0, static_cast<DWORD>(size), nullptr);
    if (file_mapping == nullptr)
        return nullptr;
    // Map a view of the file into memory.
...
std::optional<AddressSpaceReservation> AddressSpaceReservation::CreateSubReservation(void* address, size_t size, OS::MemoryPermission max_permission)
{
...
bool AddressSpaceReservation::FreeSubReservation(AddressSpaceReservation reservation)
{
    // Nothing to do.
...
bool AddressSpaceReservation::Allocate(void* address, size_t size, OS::MemoryPermission access)
{
    DCHECK(Contains(address, size));
    CHECK(VirtualAlloc2);
    DWORD flags = (access == OS::MemoryPermission::kNoAccess) ? MEM_RESERVE | MEM_REPLACE_PLACEHOLDER : MEM_RESERVE | MEM_COMMIT | MEM_REPLACE_PLACEHOLDER;
    DWORD protect = GetProtectionFromMemoryPermission(access);
    return VirtualAlloc2(GetCurrentProcess(), address, size, flags, protect, nullptr, 0);
}
...
bool AddressSpaceReservation::AllocateShared(void* address, size_t size, OS::MemoryPermission access, PlatformSharedMemoryHandle handle, uint64_t offset)
{
    DCHECK(Contains(address, size));
    ...
    DWORD protect = GetProtectionFromMemoryPermission(access);
    HANDLE file_mapping = FileMappingFromSharedMemoryHandle(handle);
    return MapViewOfFile3(file_mapping, GetCurrentProcess(), address, offset, size, MEM_REPLACE_PLACEHOLDER, protect, nullptr, 0);
}
...
bool AddressSpaceReservation::FreeShared(void* address, size_t size)
{
    ...
    return UnmapViewOfFile2(GetCurrentProcess(), address, MEM_PRESERVE_PLACEHOLDER);
}

bool AddressSpaceReservation::SetPermissions(void* address, size_t size, OS::MemoryPermission access)
{
    DCHECK(Contains(address, size));
    return OS::SetPermissions(address, size, access);
}

bool AddressSpaceReservation::RecommitPages(void* address, size_t size, OS::MemoryPermission access)
{
    DCHECK(Contains(address, size));
...
#ifndef __MINGW32__
// DbgHelp.h functions.
using DLL_FUNC_TYPE(SymInitialize) = BOOL(__stdcall*)(IN HANDLE hProcess, IN PSTR UserSearchPath, IN BOOL fInvadeProcess);
...
using DLL_FUNC_TYPE(SymGetSearchPath) = BOOL(__stdcall*)(IN HANDLE hProcess, OUT PSTR SearchPath, IN DWORD SearchPathLength);
using DLL_FUNC_TYPE(SymLoadModule64)
    = DWORD64(__stdcall*)(IN HANDLE hProcess, IN HANDLE hFile, IN PSTR ImageName, IN PSTR ModuleName, IN DWORD64 BaseOfDll, IN DWORD SizeOfDll);
using DLL_FUNC_TYPE(StackWalk64) = BOOL(__stdcall*)(DWORD MachineType, HANDLE hProcess, HANDLE hThread, LPSTACKFRAME64 StackFrame, PVOID ContextRecord,
...
using DLL_FUNC_TYPE(SymGetSymFromAddr64) = BOOL(__stdcall*)(IN HANDLE hProcess, IN DWORD64 qwAddr, OUT PDWORD64 pdwDisplacement, OUT PIMAGEHLP_SYMBOL64 Symbol);
using DLL_FUNC_TYPE(SymGetLineFromAddr64) = BOOL(__stdcall*)(IN HANDLE hProcess, IN DWORD64 qwAddr, OUT PDWORD pdwDisplacement, OUT PIMAGEHLP_LINE64 Line64);
// DbgHelp.h typedefs. Implementation found in dbghelp.dll.
using DLL_FUNC_TYPE(SymFunctionTableAccess64) = PVOID(__stdcall*)(HANDLE hProcess,
    DWORD64 AddrBase); // DbgHelp.h typedef PFUNCTION_TABLE_ACCESS_ROUTINE64
using DLL_FUNC_TYPE(SymGetModuleBase64) = DWORD64(__stdcall*)(HANDLE hProcess,
...
using DLL_FUNC_TYPE(Module32FirstW) = BOOL(__stdcall*)(HANDLE hSnapshot, LPMODULEENTRY32W lpme);
using DLL_FUNC_TYPE(Module32NextW) = BOOL(__stdcall*)(HANDLE hSnapshot, LPMODULEENTRY32W lpme);
...
#define LOAD_DLL_FUNC(name) DLL_FUNC_VAR(name) = reinterpret_cast<DLL_FUNC_TYPE(name)>(GetProcAddress(module, #name));
...
#define DLL_FUNC_LOADED(name) (DLL_FUNC_VAR(name) != nullptr)&&
        ...
        DBGHELP_FUNCTION_LIST(DLL_FUNC_LOADED) TLHELP32_FUNCTION_LIST(DLL_FUNC_LOADED)
...
static std::vector<OS::SharedLibraryAddress> LoadSymbols(HANDLE process_handle)
{
    ...
    HANDLE snapshot = _CreateToolhelp32Snapshot(TH32CS_SNAPMODULE, // dwFlags
    ...
    while (cont) {
        ...
        base = _SymLoadModule64(process_handle, // hProcess
            0, // hFile
        ...
        int lib_name_length = WideCharToMultiByte(CP_UTF8, 0, module_entry.szExePath, -1, nullptr, 0, nullptr, nullptr);
        std::string lib_name(lib_name_length, 0);
        WideCharToMultiByte(CP_UTF8, 0, module_entry.szExePath, -1, &lib_name[0], lib_name_length, nullptr, nullptr);
        result.push_back(OS::SharedLibraryAddress(
            lib_name, reinterpret_cast<uintptr_t>(module_entry.modBaseAddr), reinterpret_cast<uintptr_t>(module_entry.modBaseAddr + module_entry.modBaseSize)));
...
bool Thread::Start()
{
    uintptr_t result = _beginthreadex(nullptr, static_cast<unsigned>(stack_size_), ThreadEntry, this, 0, &data_->thread_id_);
...
std::optional<OS::MemoryRange> OS::GetFirstFreeMemoryRangeWithin(OS::Address boundary_start, OS::Address boundary_end, size_t minimum_size, size_t alignment)
{
    ...
    while (vm_start < boundary_end && VirtualQuery(reinterpret_cast<LPCVOID>(vm_start), &mi, sizeof(mi)) != 0) {
        ...
        if (mi.State == MEM_FREE) {
            ...
            // enough space.
            const uintptr_t overlap_start = RoundUp(std::max(vm_start, boundary_start), alignment);
            const uintptr_t overlap_end = RoundDown(std::min(vm_end, boundary_end), alignment);
...
Stack::StackSlot Stack::ObtainCurrentThreadStackStart()
{
    ...
    return reinterpret_cast<void*>(reinterpret_cast<NT_TIB64*>(NtCurrentTeb())->StackBase);
#elif defined(V8_TARGET_ARCH_32_BIT)
    return reinterpret_cast<void*>(reinterpret_cast<NT_TIB*>(NtCurrentTeb())->StackBase);
