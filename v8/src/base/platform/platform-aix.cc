...
Stack::StackSlot Stack::ObtainCurrentThreadStackStart()
{
    ...
    const int rc = pthread_getthrds_np(&tid, PTHRDSINFO_QUERY_ALL, &buf, sizeof(buf), regbuf, &regbufsize);
    CHECK(!rc);
...
#define MMAP() mmap(address, size, PROT_NONE, MAP_FIXED | MAP_ANONYMOUS | MAP_PRIVATE, -1, 0)
