...
ConditionVariable::~ConditionVariable()
{
    ...
    // Darwin kernel. http://crbug.com/517681.
    {
        ...
        ts.tv_sec = 0;
        ts.tv_nsec = 1;
        pthread_cond_timedwait_relative_np(&native_handle_, &lock.native_handle(), &ts);
    }
#endif
...
bool ConditionVariable::WaitFor(Mutex* mutex, const TimeDelta& rel_time)
{
    ...
    result = pthread_cond_timedwait_relative_np(&native_handle_, &mutex->native_handle(), &ts);
#else
#if (V8_OS_FREEBSD || V8_OS_NETBSD || V8_OS_OPENBSD || (V8_OS_LINUX && V8_LIBC_GLIBC))
    // On Free/Net/OpenBSD and Linux with glibc we can change the time
    // source for pthread_cond_timedwait() to use the monotonic clock.
    ...
    result = pthread_cond_timedwait(&native_handle_, &mutex->native_handle(), &ts);
#endif // V8_OS_DARWIN
    mutex->AssertUnheldAndMark();
    if (result == ETIMEDOUT) {
        return false;
...
{
    mutex->AssertHeldAndUnmark();
    SleepConditionVariableSRW(V8ToWindowsType(&native_handle_), V8ToWindowsType(&mutex->native_handle()), INFINITE, 0);
    mutex->AssertUnheldAndMark();
}
...
bool ConditionVariable::WaitFor(Mutex* mutex, const TimeDelta& rel_time)
{
    ...
    SbConditionVariableResult result = SbConditionVariableWaitTimed(&native_handle_, &mutex->native_handle(), microseconds);
