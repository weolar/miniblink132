...
int64_t ComputeThreadTicks()
{
    ...
    kern_return_t kr
        = thread_info(pthread_mach_thread_np(pthread_self()), THREAD_BASIC_INFO, reinterpret_cast<thread_info_t>(&thread_info_data), &thread_info_count);
    CHECK_EQ(kr, KERN_SUCCESS);

    // We can add the seconds into a {int64_t} without overflow.
    CHECK_LE(thread_info_data.user_time.seconds, std::numeric_limits<int64_t>::max() - thread_info_data.system_time.seconds);
...
V8_INLINE int64_t GetFuchsiaThreadTicks()
{
    ...
    zx_status_t status = zx_object_get_info(thrd_get_zx_handle(thrd_current()), ZX_INFO_THREAD_STATS, &info, sizeof(info), nullptr, nullptr);
...
{
#if (defined(_POSIX_MONOTONIC_CLOCK) && _POSIX_MONOTONIC_CLOCK >= 0) || defined(V8_OS_BSD) || defined(V8_OS_ANDROID) || defined(V8_OS_ZOS)
    ...
    if (clk_id == CLOCK_THREAD_CPUTIME_ID) {
        ...
        return (tc.stime / v8::base::Time::kNanosecondsPerMicrosecond) + (tc.utime / v8::base::Time::kNanosecondsPerMicrosecond);
...
V8_INLINE int64_t NanosecondsNow()
{
    ...
    return int64_t { ts.tv_sec } * v8::base::Time::kNanosecondsPerSecond + ts.tv_nsec;
}
...
{
    return GetClock()->NowFromSystemTime();
}
...
TimeTicks RolloverProtectedNow()
{
    ...
    while (true) {
        ...
        if (g_last_time_and_rollovers.compare_exchange_weak(original, state.as_opaque_32, std::memory_order_acq_rel)) {
            break;
        }
...
TimeDelta QPCValueToTimeDelta(LONGLONG qpc_value)
{
    ...
    if (qpc_value < TimeTicks::kQPCOverflowThreshold) {
        return TimeDelta::FromMicroseconds(qpc_value * TimeTicks::kMicrosecondsPerSecond / g_qpc_ticks_per_second);
    }
...
bool ThreadTicks::IsSupported()
{
    ...
    return false;
#elif (defined(_POSIX_THREAD_CPUTIME) && (_POSIX_THREAD_CPUTIME >= 0)) || defined(V8_OS_DARWIN) || defined(V8_OS_ANDROID) || defined(V8_OS_SOLARIS)            \
    || defined(V8_OS_ZOS)
    return true;
...
#elif (defined(_POSIX_THREAD_CPUTIME) && (_POSIX_THREAD_CPUTIME >= 0)) || defined(V8_OS_ANDROID) || defined(V8_OS_ZOS)
...
ThreadTicks ThreadTicks::GetForThread(const HANDLE& thread_handle)
{
    ...
    ::GetThreadTimes(thread_handle, &creation_time, &exit_time, &kernel_time, &user_time);
    ...
    return ThreadTicks(static_cast<int64_t>(thread_time_seconds * Time::kMicrosecondsPerSecond));
#endif
}
