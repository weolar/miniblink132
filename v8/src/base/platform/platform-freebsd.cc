...
std::vector<OS::SharedLibraryAddress> OS::GetSharedLibraryAddresses()
{
    ...
    if (sysctl(mib, miblen, nullptr, &buffer_size, nullptr, 0) == 0) {
        ...
        if (ret == 0 || (ret == -1 && errno == ENOMEM)) {
            ...
            while (start < end) {
                struct kinfo_vmentry* map = reinterpret_cast<struct kinfo_vmentry*>(start);
                const size_t ssize = map->kve_structsize;
                char* path = map->kve_path;

                CHECK_NE(0, ssize);

                if ((map->kve_protection & KVME_PROT_READ) != 0 && (map->kve_protection & KVME_PROT_EXEC) != 0 && path[0] != '\0') {
                    ...
                    result.push_back(SharedLibraryAddress(lib_name, reinterpret_cast<uintptr_t>(map->kve_start), reinterpret_cast<uintptr_t>(map->kve_end)));
                }
