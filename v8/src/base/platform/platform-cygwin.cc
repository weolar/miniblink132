...
uint8_t* RandomizedVirtualAlloc(size_t size, DWORD flags, DWORD protect, void* hint)
{
    LPVOID base = nullptr;
...
void* OS::Allocate(void* hint, size_t size, size_t alignment, MemoryPermission access)
{
    ...
    for (int i = 0; i < kMaxAttempts; ++i) {
        ...
        base = reinterpret_cast<uint8_t*>(VirtualAlloc(aligned_base, size, flags, protect));
