...
V8_INLINE intptr_t InternalGetExistingThreadLocal(intptr_t index)
{
    ...
    if (index < kMaxInlineSlots) {
        return static_cast<intptr_t>(__readfsdword(kTibInlineTlsOffset + kSystemPointerSize * index));
    }
...
public:
    ...
    static PRINTF_FORMAT(2, 0) void VFPrint(FILE* out, const char* format, va_list args);
    ...
    static PRINTF_FORMAT(3, 4) int SNPrintF(char* str, int length, const char* format, ...);
    static PRINTF_FORMAT(3, 0) int VSNPrintF(char* str, int length, const char* format, va_list args);
    ...
    {
#if (defined(V8_OS_DARWIN) || defined(V8_OS_LINUX)) && !(defined(V8_TARGET_ARCH_PPC64) || defined(V8_TARGET_ARCH_S390X))
        return true;
#else
        return false;
#endif
    ...
    V8_WARN_UNUSED_RESULT static bool RemapPages(const void* address, size_t size, void* new_address, MemoryPermission access);
...
private:
    ...
    V8_WARN_UNUSED_RESULT static void* Allocate(void* address, size_t size, size_t alignment, MemoryPermission access);

    V8_WARN_UNUSED_RESULT static void* AllocateShared(size_t size, MemoryPermission access);

    V8_WARN_UNUSED_RESULT static void* RemapShared(void* old_address, void* new_address, size_t size);

    static void Free(void* address, size_t size);

    V8_WARN_UNUSED_RESULT static void* AllocateShared(
        void* address, size_t size, OS::MemoryPermission access, PlatformSharedMemoryHandle handle, uint64_t offset);

    static void FreeShared(void* address, size_t size);

    static void Release(void* address, size_t size);

    V8_WARN_UNUSED_RESULT static bool SetPermissions(void* address, size_t size, MemoryPermission access);

    V8_WARN_UNUSED_RESULT static bool RecommitPages(void* address, size_t size, MemoryPermission access);

    V8_WARN_UNUSED_RESULT static bool DiscardSystemPages(void* address, size_t size);
    ...
    V8_WARN_UNUSED_RESULT static std::optional<AddressSpaceReservation> CreateAddressSpaceReservation(
...
public:
    ...
    bool Contains(void* region_addr, size_t region_size) const
    {
        ...
        return (region_base >= base) && ((region_base + region_size) <= (base + size_));
    }
    ...
    V8_WARN_UNUSED_RESULT static bool FreeSubReservation(AddressSpaceReservation reservation);
...
public:
    ...
    {
        void* result = reinterpret_cast<void*>(InternalGetExistingThreadLocal(static_cast<intptr_t>(key)));
...
public:
    ...
    static StackSlot GetRealStackAddressForSlot(StackSlot slot)
    {
        ...
        void* real_frame = __asan_addr_is_in_fake_stack(__asan_get_current_fake_stack(), slot, nullptr, nullptr);
        return real_frame ? StackSlot(static_cast<char*>(real_frame) + kAsanRealFrameOffsetBytes) : slot;
#endif // V8_USE_ADDRESS_SANITIZER
        return slot;
    }
