...
public:
    ...
    RandomNumberGenerator();
    explicit RandomNumberGenerator(int64_t seed)
    {
        SetSeed(seed);
    }
    ...
    std::vector<uint64_t> NextSampleSlow(
        uint64_t max, size_t n, const std::unordered_set<uint64_t>& excluded = std::unordered_set<uint64_t> {}) V8_WARN_UNUSED_RESULT;
