...
RandomNumberGenerator::RandomNumberGenerator()
{
    // Check if embedder supplied an entropy source.
    {
        ...
        if (entropy_source != nullptr) {
            int64_t seed;
            if (entropy_source(reinterpret_cast<unsigned char*>(&seed), sizeof(seed))) {
                SetSeed(seed);
                return;
            }
        }
...
std::vector<uint64_t> RandomNumberGenerator::NextSample(uint64_t max, size_t n)
{
    CHECK_LE(n, max);
    ...
    size_t smaller_part = static_cast<size_t>(std::min(max - static_cast<uint64_t>(n), static_cast<uint64_t>(n)));
...
std::vector<uint64_t> RandomNumberGenerator::NextSampleSlow(uint64_t max, size_t n, const std::unordered_set<uint64_t>& excluded)
{
    ...
    size_t larger_part = static_cast<size_t>(std::max(max - static_cast<uint64_t>(n), static_cast<uint64_t>(n)));
    ...
    if (result.size() != n) {
        return ComplementSample(std::unordered_set<uint64_t>(result.begin(), result.end()), max);
    }
    return result;
}
