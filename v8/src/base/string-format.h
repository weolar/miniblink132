...
template <const std::string_view&... strs> struct JoinedStringViews {
    ...
    // included.
    static constexpr std::string_view string_view = { array.data(), array.size() - 1 };
};

template <typename T>
concept FixedSizeString = std::is_bounded_array_v<T> && std::is_same_v<char, std::remove_cv_t<std::remove_pointer_t<std::decay_t<T>>>>;

template <typename T> struct FormattedStringPart; // Specializations below.
...
struct FormattedStringPart<I> {
    ...
    static constexpr std::string_view kFormats[2][2] { { "%" PRIu32, "%" PRId32 }, { "%" PRIu64, "%" PRId64 } };
    static constexpr std::string_view kFormatPart = kFormats[kIs64Bit][kIsSigned];

    using StorageType = std::conditional_t<kIs64Bit, std::conditional_t<kIsSigned, int64_t, uint64_t>, std::conditional_t<kIsSigned, int32_t, uint32_t>>;
    StorageType value;
};
...
template <const std::string_view& kFormat, int kMaxLen, typename... Parts> std::array<char, kMaxLen> PrintFormattedStringToArray(Parts... parts)
{
    std::array<char, kMaxLen> message;

    static_assert(kMaxLen > 0);
    static_assert(kMaxLen < 128,
    ...
    if constexpr (kFormat.size() == 0) {
        ...
        int characters = base::OS::SNPrintF(message.data(), kMaxLen, kFormat.data(), parts.value...);
...
template <typename... Ts> class FormattedString {
    template <typename T> using Part = impl::FormattedStringPart<T>;

    static_assert(std::conjunction_v<std::is_trivial<Part<Ts>>...>, "All parts needs to be trivial to guarantee optimal code");

public:
    static constexpr int kMaxLen = (1 + ... + Part<Ts>::kMaxLen);
    static constexpr std::string_view kFormat = impl::JoinedStringViews<Part<Ts>::kFormatPart...>::string_view;
    ...
    template <typename T> V8_WARN_UNUSED_RESULT auto operator<<(T&& t) const&&
    {
        using PlainT = std::remove_cv_t<std::remove_reference_t<T>>;
        return FormattedString<Ts..., PlainT> { std::tuple_cat(parts_, std::make_tuple(Part<PlainT> { t })) };
    }
    ...
    {
        return std::apply(impl::PrintFormattedStringToArray<kFormat, kMaxLen, Part<Ts>...>, parts_);
    }

private:
    template <typename... Us> friend class FormattedString;

    explicit FormattedString(std::tuple<Part<Ts>...> parts)
        : parts_(parts)
    {
    }
