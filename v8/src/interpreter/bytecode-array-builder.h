...
public:
    BytecodeArrayBuilder(Zone* zone, int parameter_count, int locals_count, FeedbackVectorSpec* feedback_vector_spec = nullptr,
    ...
    {
        DCHECK_LE(fixed_register_count(), register_allocator()->maximum_register_count());
    ...
    BytecodeArrayBuilder& SetNamedProperty(Register object, size_t constant_pool_entry, int feedback_slot, LanguageMode language_mode);
...
#undef ENTRY_GETTER
    ...
    {
        if (!latest_source_info_.is_valid() || latest_source_info_.source_position() < scope_start) {
            return std::nullopt;
        }
...
private:
    ...
    template <Bytecode bytecode, ImplicitRegisterUse implicit_register_use, OperandType... operand_types> friend class BytecodeNodeBuilder;
...
#define DECLARE_BYTECODE_OUTPUT(Name, ...)                                                                                                                     \
    template <typename... Operands> V8_INLINE BytecodeNode Create##Name##Node(Operands... operands);                                                           \
    template <typename... Operands> V8_INLINE void Output##Name(Operands... operands);                                                                         \
    template <typename... Operands> V8_INLINE void Output##Name(BytecodeLabel* label, Operands... operands);
...
#undef DECLARE_OPERAND_TYPE_INFO
    ...
    template <Bytecode bytecode, ImplicitRegisterUse implicit_register_use> void PrepareToOutputBytecode();
...
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, const BytecodeArrayBuilder::ToBooleanMode& mode);
