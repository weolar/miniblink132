...
BreakableControlFlowBuilder::~BreakableControlFlowBuilder()
{
    ...
    if (block_coverage_builder_ != nullptr) {
        block_coverage_builder_->IncrementBlockCounter(node_, SourceRangeKind::kContinuation);
    }
}
...
void BreakableControlFlowBuilder::EmitJumpIfTrue(BytecodeArrayBuilder::ToBooleanMode mode, BytecodeLabels* sites)
{
    builder()->JumpIfTrue(mode, sites->New());
}

void BreakableControlFlowBuilder::EmitJumpIfFalse(BytecodeArrayBuilder::ToBooleanMode mode, BytecodeLabels* sites)
{
...
void BreakableControlFlowBuilder::EmitJumpIfForInDone(BytecodeLabels* sites, Register index, Register cache_length)
{
...
void LoopBuilder::LoopHeader()
{
    ...
    DCHECK(break_labels_.empty() && continue_labels_.empty() && end_labels_.empty());
...
{
    BindLoopEnd();
    if (parent_loop && loop_header_.offset() == parent_loop->loop_header_.offset()) {
        ...
        builder()->JumpLoop(&loop_header_, std::min(loop_depth, FeedbackVector::kMaxOsrUrgency - 1), source_position_, slot_index);
    }
}

void LoopBuilder::BindContinueTarget()
{
    continue_labels_.Bind(builder());
}
...
void SwitchBuilder::BindCaseTargetForJumpTable(int case_value, CaseClause* clause)
{
    builder()->Bind(jump_table_, case_value);
    BuildBlockCoverage(clause);
}

void SwitchBuilder::BindCaseTargetForCompareJump(int index, CaseClause* clause)
{
    builder()->Bind(&case_sites_.at(index));
    BuildBlockCoverage(clause);
}

void SwitchBuilder::JumpToCaseIfTrue(BytecodeArrayBuilder::ToBooleanMode mode, int index)
{
    builder()->JumpIfTrue(mode, &case_sites_.at(index));
}

// Precondition: tag is in the accumulator
void SwitchBuilder::EmitJumpTableIfExists(int min_case, int max_case, std::map<int, CaseClause*>& covered_cases)
{
...
{
    this->EmitJumpIfFalse(BytecodeArrayBuilder::ToBooleanMode::kAlreadyBoolean, &fall_through_);
}

TryCatchBuilder::~TryCatchBuilder()
{
    if (block_coverage_builder_ != nullptr) {
        block_coverage_builder_->IncrementBlockCounter(statement_, SourceRangeKind::kContinuation);
    }
}
...
void TryCatchBuilder::EndTry()
{
    ...
    if (block_coverage_builder_ != nullptr) {
        block_coverage_builder_->IncrementBlockCounter(statement_, SourceRangeKind::kCatch);
    }
}
...
void TryFinallyBuilder::BeginFinally()
{
    ...
    if (block_coverage_builder_ != nullptr) {
        block_coverage_builder_->IncrementBlockCounter(statement_, SourceRangeKind::kFinally);
    }
}
...
void ConditionalChainControlFlowBuilder::ThenAt(size_t index)
{
    ...
    if (block_coverage_builder_) {
        block_coverage_builder_->IncrementBlockCounter(block_coverage_then_slot_at(index));
    }
}
...
void ConditionalChainControlFlowBuilder::ElseAt(size_t index)
{
    ...
    if (block_coverage_builder_) {
        block_coverage_builder_->IncrementBlockCounter(block_coverage_else_slot_at(index));
    }
}
