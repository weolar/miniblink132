...
#define BYTECODE_LIST_WITH_UNIQUE_HANDLERS_IMPL(V, V_TSA)                                                                                                      \
    ...
    V(CallRuntimeForPair, ImplicitRegisterUse::kClobberAccumulator, OperandType::kRuntimeId, OperandType::kRegList, OperandType::kRegCount,                    \
...
#define BYTECODE_LIST_WITH_UNIQUE_HANDLERS(V, V_TSA) BYTECODE_LIST_WITH_UNIQUE_HANDLERS_IMPL(V, V_TSA)
#else
#define BYTECODE_LIST_WITH_UNIQUE_HANDLERS(V, V_TSA) BYTECODE_LIST_WITH_UNIQUE_HANDLERS_IMPL(V, V)
#endif
...
#define COUNT_BYTECODE(x, ...) +1
    ...
    kLast
    = -1 BYTECODE_LIST(COUNT_BYTECODE, COUNT_BYTECODE),
...
public:
    ...
    static const int kShortStarCount = static_cast<int>(Bytecode::kLastShortStar) - static_cast<int>(Bytecode::kFirstShortStar) + 1;
    ...
    static OperandScale PrefixBytecodeToOperandScale(Bytecode bytecode)
    {
        ...
        } else if (bytecode == Bytecode::kExtraWide || bytecode == Bytecode::kDebugBreakExtraWide) {
    ...
    {
        return BytecodeOperands::WritesAccumulator(GetImplicitRegisterUse(bytecode));
    }

    // Returns true if |bytecode| writes the accumulator.
    static bool ClobbersAccumulator(Bytecode bytecode)
    {
        return BytecodeOperands::ClobbersAccumulator(GetImplicitRegisterUse(bytecode));
    }

    // Returns true if |bytecode| writes the accumulator.
    static bool WritesOrClobbersAccumulator(Bytecode bytecode)
    {
        return BytecodeOperands::WritesOrClobbersAccumulator(GetImplicitRegisterUse(bytecode));
    }
    ...
    {
        return BytecodeOperands::WritesImplicitRegister(GetImplicitRegisterUse(bytecode));
    }
    ...
    static constexpr bool IsAccumulatorLoadWithoutEffects(Bytecode bytecode)
    {
        ...
        return bytecode >= Bytecode::kLdar && bytecode <= Bytecode::kLdaImmutableCurrentContextSlot;
    }
    ...
    static constexpr bool IsCompareWithoutEffects(Bytecode bytecode)
    {
        ...
        return bytecode >= Bytecode::kTestReferenceEqual && bytecode <= Bytecode::kTestTypeOf;
    }

    static constexpr bool IsShortStar(Bytecode bytecode)
    {
        return bytecode >= Bytecode::kFirstShortStar && bytecode <= Bytecode::kLastShortStar;
    }
    ...
    {
        return IsShortStar(bytecode) || (bytecode >= Bytecode::kStar && bytecode <= Bytecode::kPopContext);
    }
    ...
    {
        return bytecode >= Bytecode::kJumpIfToBooleanTrue && bytecode <= Bytecode::kJumpIfForInDone;
    }
    ...
    {
        return bytecode >= Bytecode::kJumpIfNullConstant && bytecode <= Bytecode::kJumpIfForInDone;
    }
    ...
    {
        return bytecode >= Bytecode::kJumpLoop && bytecode <= Bytecode::kJumpIfForInDone;
    }
    ...
    {
        return bytecode >= Bytecode::kJump && bytecode <= Bytecode::kJumpIfForInDone;
    }
    ...
    {
        return IsJump(bytecode) && !IsJumpIfToBoolean(bytecode) && bytecode != Bytecode::kJumpLoop;
    }
    ...
    {
        return (IsAccumulatorLoadWithoutEffects(bytecode) || IsRegisterLoadWithoutEffects(bytecode) || IsCompareWithoutEffects(bytecode)
            || IsJumpWithoutEffects(bytecode) || IsSwitch(bytecode) || bytecode == Bytecode::kReturn);
    }
    ...
    {
        return bytecode == Bytecode::kCallAnyReceiver || bytecode == Bytecode::kCallProperty || bytecode == Bytecode::kCallProperty0
            || bytecode == Bytecode::kCallProperty1 || bytecode == Bytecode::kCallProperty2 || bytecode == Bytecode::kCallUndefinedReceiver
            || bytecode == Bytecode::kCallUndefinedReceiver0 || bytecode == Bytecode::kCallUndefinedReceiver1 || bytecode == Bytecode::kCallUndefinedReceiver2
            || bytecode == Bytecode::kConstruct || bytecode == Bytecode::kCallWithSpread || bytecode == Bytecode::kConstructWithSpread
            || bytecode == Bytecode::kConstructForwardAllArgs || bytecode == Bytecode::kCallJSRuntime;
    }
    ...
    {
        return bytecode == Bytecode::kExtraWide || bytecode == Bytecode::kWide || bytecode == Bytecode::kDebugBreakExtraWide
...
#undef OR_BYTECODE
    ...
    {
        return reinterpret_cast<Address>(const_cast<uint8_t*>(&kBytecodeSizes[0][0]));
    }
...
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, const Bytecode& bytecode);
