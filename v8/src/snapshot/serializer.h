...
private:
    ...
    public:
        ...
        ~NameMap()
        {
            for (base::HashMap::Entry* p = impl_.Start(); p != nullptr; p = impl_.Next(p)) {
        ...
        const char* Lookup(Address code_address)
        {
            ...
            return (entry != nullptr) ? static_cast<const char*>(entry->value) : nullptr;
        }
    ...
    private:
        ...
        {
            return impl_.LookupOrInsert(reinterpret_cast<void*>(code_address), ComputeAddressHash(code_address));
        }

        base::HashMap::Entry* FindEntry(Address code_address)
        {
            return impl_.Lookup(reinterpret_cast<void*>(code_address), ComputeAddressHash(code_address));
        }
    ...
    void LogRecordedBuffer(Tagged<AbstractCode> code, MaybeHandle<SharedFunctionInfo>, const char* name, int length) override
    {
...
public:
    explicit ObjectCacheIndexMap(Heap* heap)
        : map_(heap)
        , next_index_(0)
    {
    }
...
public:
    ...
    ~Serializer() override
    {
        DCHECK_EQ(unresolved_forward_refs_, 0);
    }
...
protected:
    ...
    {
        return (flags_ & Snapshot::kReconstructReadOnlyAndSharedObjectCachesForTesting) != 0;
    }

    bool deferred_objects_empty()
    {
        return deferred_objects_.size() == 0;
    }
...
private:
    ...
    GlobalHandleVector<HeapObject> deferred_objects_; // To handle stack overflow.
