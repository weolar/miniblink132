...
public:
    ...
    void WriteSnapshot(v8::StartupData blob) const
    {
        ...
        v8::base::Vector<const uint8_t> blob_vector(reinterpret_cast<const uint8_t*>(blob.data), blob.raw_size);
...
private:
    ...
    static void WriteSnapshotFileSuffix(FILE* fp)
    {
        ...
        fprintf(fp, "}\n");
        fprintf(fp, "\n");
        fprintf(fp, "bool Snapshot::ShouldVerifyChecksum(const v8::StartupData* data) {\n");
    ...
    static void WriteSnapshotFileData(FILE* fp, v8::base::Vector<const uint8_t> blob)
    {
        fprintf(fp, "alignas(kPointerAlignment) static const uint8_t blob_data[] = {\n");
    ...
    static void WriteBinaryContentsAsCArray(FILE* fp, v8::base::Vector<const uint8_t> blob)
    {
...
v8::StartupData CreateSnapshotDataBlob(v8::SnapshotCreator& snapshot_creator, const char* embedded_source)
{
    v8::base::ElapsedTimer timer;
    timer.Start();

    v8::StartupData result = i::CreateSnapshotDataBlobInternal(v8::SnapshotCreator::FunctionCodeHandling::kClear, embedded_source, snapshot_creator);
...
v8::StartupData WarmUpSnapshotDataBlob(v8::StartupData cold_snapshot_blob, const char* warmup_source)
{
    v8::base::ElapsedTimer timer;
    timer.Start();

    v8::StartupData result = i::WarmUpSnapshotDataBlobInternal(cold_snapshot_blob, warmup_source);
...
int main(int argc, char** argv)
{
    ...
    std::string usage = "Usage: " + std::string(argv[0]) + " [--startup-src=file]" + " [--startup-blob=file]" + " [--embedded-src=file]"
        + " [--embedded-variant=label]" + " [--static-roots-src=file]" + " [--target-arch=arch]" + " [--target-os=os] [extras]\n\n";
    int result = i::FlagList::SetFlagsFromCommandLine(&argc, argv, true, HelpOptions(HelpOptions::kExit, usage.c_str()));
    if (result > 0 || (argc > 3)) {
...

    {
        ...
        std::unique_ptr<char[]> embed_script = GetExtraCode(argc >= 2 ? argv[1] : nullptr, "embedding");
        std::unique_ptr<char[]> warmup_script = GetExtraCode(argc >= 3 ? argv[2] : nullptr, "warm up");
        ...
        v8::StartupData blob;
        {
            ...
            size_t code_range_size_mb
                = i::kMaximalCodeRangeSize == 0 ? i::kMaxPCRelativeCodeRangeInMB : std::min(i::kMaximalCodeRangeSize / i::MB, i::kMaxPCRelativeCodeRangeInMB);
            create_params.constraints.set_code_range_size_in_bytes(code_range_size_mb * i::MB);
