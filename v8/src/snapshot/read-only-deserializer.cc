...
private:
    ...
    void AllocatePage(bool fixed_offset)
    {
        ...
        ro_space()->InitializePageForDeserialization(PageAt(actual_page_index), area_size_in_bytes);
    }
    ...
    void DeserializeSegment()
    {
        ...
        if (!V8_STATIC_ROOTS_BOOL) {
            ...
            uint8_t* data = const_cast<uint8_t*>(source_->data() + source_->position());
    ...
    void DecodeTaggedSlots(Address segment_start, const ro::BitSet& tagged_slots)
    {
        ...
        for (size_t i = 0; i < tagged_slots.size_in_bits(); i++) {
            ...
            if (!tagged_slots.contains(static_cast<int>(i)))
                continue;
            ...
            *dst = COMPRESS_POINTERS_BOOL ? V8HeapCompressionScheme::CompressObject(obj_ptr) : static_cast<Tagged_t>(obj_ptr);
        }
    }
...
ReadOnlyDeserializer::ReadOnlyDeserializer(Isolate* isolate, const SnapshotData* data, bool can_rehash)
...
void ReadOnlyDeserializer::DeserializeIntoIsolate()
{
    ...
    if (V8_UNLIKELY(v8_flags.profile_deserialization))
        timer.Start();
    NestedTimedHistogramScope histogram_timer(isolate()->counters()->snapshot_deserialize_rospace());
...
public:
    explicit ObjectPostProcessor(Isolate* isolate)
        : isolate_(isolate)
        , embedded_data_(EmbeddedData::FromBlob(isolate_))
    {
    }
    ...
    void Finalize()
    {
        ...
        for (auto& slot : external_pointer_slots_) {
            registry.emplace_back(slot.Relaxed_LoadHandle(), slot.load(isolate_), slot.tag());
        }

        isolate_->read_only_artifacts()->set_external_pointer_registry(std::move(registry));
...
private:
    Address GetAnyExternalReferenceAt(int index, bool is_api_reference) const
    {
        if (is_api_reference) {
            ...
            Address address = refs == nullptr ? reinterpret_cast<Address>(NoExternalReferencesCallback) : static_cast<Address>(refs[index]);
    ...
    void DecodeExternalPointerSlot(Tagged<HeapObject> host, ExternalPointerSlot slot)
    {
        ...
        auto encoded = ro::EncodedExternalReference::FromUint32(slot.GetContentAsIndexAfterDeserialization(no_gc));
        Address slot_value = GetAnyExternalReferenceAt(encoded.index, encoded.is_api_reference);
    ...
    {
        DecodeExternalPointerSlot(o, o->RawExternalPointerField(AccessorInfo::kSetterOffset, kAccessorInfoSetterTag));
        DecodeExternalPointerSlot(o, o->RawExternalPointerField(AccessorInfo::kMaybeRedirectedGetterOffset, kAccessorInfoGetterTag));
        if (USE_SIMULATOR_BOOL)
            o->init_getter_redirection(isolate_);
    }
    void PostProcessFunctionTemplateInfo(Tagged<FunctionTemplateInfo> o)
    {
        DecodeExternalPointerSlot(o, o->RawExternalPointerField(FunctionTemplateInfo::kMaybeRedirectedCallbackOffset, kFunctionTemplateInfoCallbackTag));
        if (USE_SIMULATOR_BOOL)
            o->init_callback_redirection(isolate_);
    }
    void PostProcessCode(Tagged<Code> o)
    {
        ...
        o->SetInstructionStartForOffHeapBuiltin(isolate_, EmbeddedData::FromBlob(isolate_).InstructionStartOf(o->builtin_id()));
    }
