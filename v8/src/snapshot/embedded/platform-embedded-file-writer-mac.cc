...
void PlatformEmbeddedFileWriterMac::DeclareUint32(const char* name, uint32_t value)
{
...
void PlatformEmbeddedFileWriterMac::SourceInfo(int fileid, const char* filename, int line)
{
    fprintf(fp_, ".loc %d %d\n", fileid, line);
}

// TODO(mmarchini): investigate emitting size annotations for OS X
void PlatformEmbeddedFileWriterMac::DeclareFunctionBegin(const char* name, uint32_t size)
{
    DeclareLabel(name);
...
void PlatformEmbeddedFileWriterMac::DeclareExternalFilename(int fileid, const char* filename)
{
...
int PlatformEmbeddedFileWriterMac::IndentedDataDirective(DataDirective directive)
{
