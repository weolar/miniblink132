...
void PlatformEmbeddedFileWriterAIX::DeclareUint32(const char* name, uint32_t value)
{
...
void PlatformEmbeddedFileWriterAIX::SourceInfo(int fileid, const char* filename, int line)
{
    fprintf(fp_, ".xline %d, \"%s\"\n", line, filename);
}

// TODO(mmarchini): investigate emitting size annotations for AIX
void PlatformEmbeddedFileWriterAIX::DeclareFunctionBegin(const char* name, uint32_t size)
{
    Newline();
...
void PlatformEmbeddedFileWriterAIX::DeclareExternalFilename(int fileid, const char* filename)
{
...
int PlatformEmbeddedFileWriterAIX::IndentedDataDirective(DataDirective directive)
{
