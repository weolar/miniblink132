...
void SnapshotByteSink::PutN(int number_of_bytes, const uint8_t v, const char* description)
{
...
void SnapshotByteSink::PutUint30(uint32_t integer, const char* description)
{
    ...
    if (bytes > 1)
        Put(static_cast<uint8_t>((integer >> 8) & 0xFF), "IntPart2");
    if (bytes > 2)
...
void SnapshotByteSink::PutRaw(const uint8_t* data, int number_of_bytes, const char* description)
{
