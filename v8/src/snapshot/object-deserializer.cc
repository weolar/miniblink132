...
ObjectDeserializer::ObjectDeserializer(Isolate* isolate, const SerializedCodeData* data)
    : Deserializer(isolate, data->Payload(), data->GetMagicNumber(), true, false)
{
}

MaybeDirectHandle<SharedFunctionInfo> ObjectDeserializer::DeserializeSharedFunctionInfo(Isolate* isolate, const SerializedCodeData* data, Handle<String> source)
{
    ...
    return d.Deserialize().ToHandle(&result) ? Cast<SharedFunctionInfo>(result) : MaybeDirectHandle<SharedFunctionInfo>();
}
...
OffThreadObjectDeserializer::OffThreadObjectDeserializer(LocalIsolate* isolate, const SerializedCodeData* data)
    : Deserializer(isolate, data->Payload(), data->GetMagicNumber(), true, false)
{
}

MaybeDirectHandle<SharedFunctionInfo> OffThreadObjectDeserializer::DeserializeSharedFunctionInfo(
...
MaybeDirectHandle<HeapObject> OffThreadObjectDeserializer::Deserialize(std::vector<IndirectHandle<Script>>* deserialized_scripts)
{
    DCHECK(deserializing_user_code());
    ...
    for (DirectHandle<Script> script : new_scripts()) {
        ...
        script->set_id(isolate()->GetNextScriptId());
        LogScriptEvents(*script);
        deserialized_scripts->push_back(isolate()->heap()->NewPersistentHandle(script));
    }
