...
public:
    ...
    public:
        Inner() = default;
        explicit Inner(ZoneVector<uint8_t>* data)
            : data_(data)
        {
        }
...
template <class Data> class BaseConsumedPreparseData : public ConsumedPreparseData {
public:
    ...
    ProducedPreparseData* GetDataForSkippableFunction(Zone* zone, int start_position, int* end_position, int* num_parameters, int* function_length,
        int* num_inner_functions, bool* uses_super_property, LanguageMode* language_mode) final;
...
class OnHeapConsumedPreparseData final : public BaseConsumedPreparseData<Tagged<PreparseData>> {
public:
...
ZonePreparseData* PreparseDataBuilder::ByteData::CopyToZone(Zone* zone, int children_length)
{
...
class ZoneConsumedPreparseData final : public BaseConsumedPreparseData<ZoneVectorWrapper> {
public:
