...
public:
    ...
    ScopedExternalStringLock(const ScopedExternalStringLock& other) V8_NOEXCEPT : resource_(other.resource_)
    {
        resource_->Lock();
    }
...
template <typename Char> struct Range {
    const Char* start;
    const Char* end;
...
template <typename Char> class OnHeapStream {
public:
    ...
    {
        return { &string_->GetChars(*no_gc)[start_offset_ + std::min(length_, pos)], &string_->GetChars(*no_gc)[start_offset_ + length_] };
    }
...
template <typename Char> class ExternalStringStream {
...
public:
    ...
    ExternalStringStream(const ExternalStringStream& other) V8_NOEXCEPT : lock_(other.lock_), data_(other.data_), length_(other.length_)
    {
    }
...
template <typename Char> class TestingStream {
public:
...
template <typename Char> class ChunkedStream {
public:
    ...
    ChunkedStream(const ChunkedStream& other) V8_NOEXCEPT : source_(nullptr), chunks_(other.chunks_)
    {
    }
...
private:
    ...
    Chunk& FindChunk(size_t position, RuntimeCallStats* stats)
    {
        ...
        // Walk backwards.
        for (auto reverse_it = chunks_->rbegin(); reverse_it != chunks_->rend(); ++reverse_it) {
    ...
    virtual void ProcessChunk(const uint8_t* data, size_t position, size_t length)
    {
        ...
        chunks_->emplace_back(reinterpret_cast<const Char*>(data), position, length / sizeof(Char));
    }
...
template <template <typename T> class ByteStream> class BufferedCharacterStream : public Utf16CharacterStream {
public:
    ...
    {
        CHECK(can_be_cloned());
        return std::unique_ptr<Utf16CharacterStream>(new BufferedCharacterStream<ByteStream>(*this));
    }
...
// ByteStream.
template <template <typename T> class ByteStream> class UnbufferedCharacterStream : public Utf16CharacterStream {
public:
    template <class... TArgs>
    ...
    std::unique_ptr<Utf16CharacterStream> Clone() const override
    {
        return std::unique_ptr<Utf16CharacterStream>(new UnbufferedCharacterStream<ByteStream>(*this));
    }
...
// Provides an unbuffered utf-16 view on the bytes from the underlying
// ByteStream.
class RelocatingCharacterStream final : public UnbufferedCharacterStream<OnHeapStream> {
public:
    template <class... TArgs>
    RelocatingCharacterStream(Isolate* isolate, size_t pos, TArgs... args)
        ...
        , isolate_(isolate)
    {
        isolate->main_thread_local_heap()->AddGCEpilogueCallback(UpdateBufferPointersCallback, this);
    }

private:
    ~RelocatingCharacterStream() final
    {
        isolate_->main_thread_local_heap()->RemoveGCEpilogueCallback(UpdateBufferPointersCallback, this);
    }

    static void UpdateBufferPointersCallback(void* stream)
    {
        reinterpret_cast<RelocatingCharacterStream*>(stream)->UpdateBufferPointers();
    }

    void UpdateBufferPointers()
    {
        DisallowGarbageCollection no_gc;
        Range<uint16_t> range = byte_stream_.GetDataAt(buffer_pos_, runtime_call_stats(), &no_gc);
...
public:
    ...
    {
        CHECK(can_be_cloned());
        return std::unique_ptr<Utf16CharacterStream>(new Windows1252CharacterStream(*this));
    }
...
protected:
    bool ReadBlock(size_t position) final
    {
        ...
        size_t length = std::min({ kBufferSize, range.length() });
        std::transform(range.start, range.start + length, &buffer_[0], [](uint8_t c) { return kWindows1252ToUC16[c]; });
        buffer_end_ = &buffer_[length];
        return true;
    }
...
private:
    Windows1252CharacterStream(const Windows1252CharacterStream& other) V8_NOEXCEPT : byte_stream_(other.byte_stream_)
    {
    }
...
private:
    ...
    Utf8ExternalStreamingStream(const Utf8ExternalStreamingStream& source_stream) V8_NOEXCEPT : chunks_(source_stream.chunks_),
                                                                                                current_({ 0, { 0, 0, 0, unibrow::Utf8::State::kAccept } }),
    ...
    if (V8_UNLIKELY(current_.pos.bytes < 3 && current_.pos.chars == 0)) {
        while (cursor < end) {
            ...
            if (V8_LIKELY(t < kUtf8Bom)) {
                *(output_cursor++) = static_cast<base::uc16>(t); // The most frequent case.
...
void Utf8ExternalStreamingStream::SearchPosition(size_t position)
{
    ...
    if (chunk_no + 1 < chunks_->size()) {
        ...
        bool ascii_only_chunk = GetChunk(chunk_no).start.incomplete_char == 0
            && (GetChunk(chunk_no + 1).start.bytes - GetChunk(chunk_no).start.bytes) == (GetChunk(chunk_no + 1).start.chars - GetChunk(chunk_no).start.chars);
        if (ascii_only_chunk) {
            size_t skip = position - GetChunk(chunk_no).start.chars;
            current_ = { chunk_no, { GetChunk(chunk_no).start.bytes + skip, GetChunk(chunk_no).start.chars + skip, 0, unibrow::Utf8::State::kAccept } };
        } else {
            current_ = { chunk_no, GetChunk(chunk_no).start };
...
size_t Utf8ExternalStreamingStream::FillBuffer(size_t position)
{
    ...
    bool out_of_data = current_.chunk_no != chunks_->size() && GetChunk(current_.chunk_no).length == 0 && current_.pos.incomplete_char == 0;
    ...
    DCHECK_EQ(current_.pos.chars - position, static_cast<size_t>(buffer_end_ - buffer_cursor_));
...
Utf16CharacterStream* ScannerStream::For(Isolate* isolate, Handle<String> data, int start_pos, int end_pos)
{
    ...
    if (IsExternalOneByteString(*data)) {
        return new BufferedCharacterStream<ExternalStringStream>(
            static_cast<size_t>(start_pos), Cast<ExternalOneByteString>(*data), start_offset, static_cast<size_t>(end_pos));
    } else if (IsExternalTwoByteString(*data)) {
        return new UnbufferedCharacterStream<ExternalStringStream>(
            static_cast<size_t>(start_pos), Cast<ExternalTwoByteString>(*data), start_offset, static_cast<size_t>(end_pos));
    } else if (IsSeqOneByteString(*data)) {
        return new BufferedCharacterStream<OnHeapStream>(
            static_cast<size_t>(start_pos), Cast<SeqOneByteString>(data), start_offset, static_cast<size_t>(end_pos));
    } else if (IsSeqTwoByteString(*data)) {
        return new RelocatingCharacterStream(isolate, static_cast<size_t>(start_pos), Cast<SeqTwoByteString>(data), start_offset, static_cast<size_t>(end_pos));
    } else {
        UNREACHABLE();
    }
}

std::unique_ptr<Utf16CharacterStream> ScannerStream::ForTesting(const char* data)
{
    return ScannerStream::ForTesting(data, strlen(data));
}

std::unique_ptr<Utf16CharacterStream> ScannerStream::ForTesting(const char* data, size_t length)
{
    if (data == nullptr) {
        DCHECK_EQ(length, 0);
    ...
    return std::unique_ptr<Utf16CharacterStream>(new BufferedCharacterStream<TestingStream>(0, reinterpret_cast<const uint8_t*>(data), length));
}

std::unique_ptr<Utf16CharacterStream> ScannerStream::ForTesting(const uint16_t* data, size_t length)
{
    if (data == nullptr) {
        DCHECK_EQ(length, 0);
    ...
    return std::unique_ptr<Utf16CharacterStream>(new UnbufferedCharacterStream<TestingStream>(0, data, length));
}

Utf16CharacterStream* ScannerStream::For(ScriptCompiler::ExternalSourceStream* source_stream, v8::ScriptCompiler::StreamedSource::Encoding encoding)
{
    switch (encoding) {
    case v8::ScriptCompiler::StreamedSource::TWO_BYTE:
        return new UnbufferedCharacterStream<ChunkedStream>(static_cast<size_t>(0), source_stream);
    case v8::ScriptCompiler::StreamedSource::ONE_BYTE:
        return new BufferedCharacterStream<ChunkedStream>(static_cast<size_t>(0), source_stream);
    case v8::ScriptCompiler::StreamedSource::WINDOWS_1252:
        return new Windows1252CharacterStream(static_cast<size_t>(0), source_stream);
