...
#define EXPAND_BINOP_ASSIGN_TOKEN(T, name, string, precedence) T(kAssign##name, string "=", 2)

#define EXPAND_BINOP_TOKEN(T, name, string, precedence) T(k##name, string, precedence)
...
#undef T
    ...
    V8_INLINE static bool IsValidIdentifier(Value token, LanguageMode language_mode, bool is_generator, bool disallow_await)
    {
        if (V8_LIKELY(base::IsInRange(token, kIdentifier, kAsync)))
            return true;
        if (token == kAwait)
