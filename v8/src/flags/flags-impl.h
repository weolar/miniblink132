...
struct Flag {
    ...
    bool ImpliedBy(const void* ptr) const
    {
        ...
        while (current != nullptr) {
            ...
            if (current->PointsTo(ptr))
                return true;
            current = current->implied_by_ptr_;
            if (visited_flags.contains(current))
                break;
        }
        return false;
    }
    ...
    {
        return GetValue<TYPE_BOOL, bool>();
    }
    ...
    {
        return GetValue<TYPE_UINT64, uint64_t>();
    }
    ...
    {
        return GetValue<TYPE_FLOAT, double>();
    }
    ...
    {
        return GetValue<TYPE_SIZE_T, size_t>();
    }
    ...
    template <FlagType flag_type, typename T> T GetValue() const
    {
    ...
    template <FlagType flag_type, typename T> void SetValue(T new_value, SetBy set_by)
    {
        DCHECK_EQ(flag_type, type_);
