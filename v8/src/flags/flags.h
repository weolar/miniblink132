...
template <typename T> class FlagValue {
    ...
    static_assert(std::is_arithmetic_v<T> || std::is_same_v<std::optional<bool>, T> || std::is_same_v<const char*, T>);

public:
    using underlying_type = T;
    explicit constexpr FlagValue(T value)
        : value_(value)
    {
    }
...
public:
    ...
    public:
        ...
        explicit HelpOptions(ExitBehavior exit_behavior = kExit, const char* usage = nullptr)
...
private:
    ...
    template <typename T> friend class FlagValue;
};

template <typename T> FlagValue<T>& FlagValue<T>::operator=(T new_value)
{
    if (new_value != value_) {
