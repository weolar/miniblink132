...
auto callback(void* env, const wasm::Val args[], wasm::Val results[]) -> wasm::own<wasm::Trap>
{
...
void run(wasm::Engine* engine, const wasm::Shared<wasm::Module>* shared, std::mutex* mutex, int id)
{
    // Create store.
    ...
    for (int i = 0; i < N_REPS; ++i) {
        ...
        // Create imports.
        auto func_type = wasm::FuncType::make(wasm::ownvec<wasm::ValType>::make(wasm::ValType::make(wasm::I32)), wasm::ownvec<wasm::ValType>::make());
        auto func = wasm::Func::make(store, func_type.get(), callback, mutex);

        auto global_type = wasm::GlobalType::make(wasm::ValType::make(wasm::I32), wasm::CONST);
        auto global = wasm::Global::make(store, global_type.get(), wasm::Val::i32(i));
