...
template <class T, class U> void check(T actual, U expected)
{
    if (actual != expected) {
...
void run()
{
    ...
    auto const_f32_type = wasm::GlobalType::make(wasm::ValType::make(wasm::F32), wasm::CONST);
    auto const_i64_type = wasm::GlobalType::make(wasm::ValType::make(wasm::I64), wasm::CONST);
    auto var_f32_type = wasm::GlobalType::make(wasm::ValType::make(wasm::F32), wasm::VAR);
    auto var_i64_type = wasm::GlobalType::make(wasm::ValType::make(wasm::I64), wasm::VAR);
    ...
    wasm::Extern* imports[] = { const_f32_import.get(), const_i64_import.get(), var_f32_import.get(), var_i64_import.get() };
