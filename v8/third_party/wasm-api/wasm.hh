...
static_assert(sizeof(intptr_t) == sizeof(int32_t) || sizeof(intptr_t) == sizeof(int64_t), "incompatible pointer type");
...
template <class T> class vec {
    ...
    vec(size_t size)
        : vec(size, size ? new(std::nothrow) T[size] : nullptr)
    {
        make_data();
    }
...
public:
    ...
    vec(vec<T>&& that)
        : vec(that.size_, that.data_.release())
    {
    }
    ...
    template <class... Ts> static auto make(Ts&&... args) -> vec
    {
...
template <class T> auto make_own(T* x) -> own<T>
{
    return own<T>(x);
}
...
public:
    ...
    {
        return wasm::is_num(kind());
    }
    auto is_ref() const -> bool
    {
        return wasm::is_ref(kind());
    }
};

// External Types

enum ExternKind : uint8_t { EXTERN_FUNC, EXTERN_GLOBAL, EXTERN_TABLE, EXTERN_MEMORY };
...
public:
    ...
    static auto make(ownvec<ValType>&& params = ownvec<ValType>::make(), ownvec<ValType>&& results = ownvec<ValType>::make()) -> own<FuncType>;
...
public:
    ...
    auto is_num() const -> bool
    {
        return wasm::is_num(kind_);
    }
    auto is_ref() const -> bool
    {
        return wasm::is_ref(kind_);
    }

    static auto i32(int32_t x) -> Val
    {
        return Val(x);
    }
    static auto i64(int64_t x) -> Val
    {
        return Val(x);
    }
    static auto f32(float32_t x) -> Val
    {
        return Val(x);
    }
    static auto f64(float64_t x) -> Val
    {
        return Val(x);
    }
    static auto ref(own<Ref>&& x) -> Val
    {
...
public:
    ...
    static auto make(Store*, const FuncType*, callback_with_env, void*, void (*finalizer)(void*) = nullptr) -> own<Func>;
...
public:
    ...
    static auto make(Store*, const TableType*, const Ref* init = nullptr) -> own<Table>;
...
public:
    ...
    static auto make(Store*, const Module*, const Extern* const[], own<Trap>* = nullptr) -> own<Instance>;
