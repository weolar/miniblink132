...
inline void assertions()
{
    ...
    static_assert(sizeof(intptr_t) == sizeof(uint32_t) || sizeof(intptr_t) == sizeof(uint64_t), "incompatible pointer type");
}
...
#define WASM_DECLARE_VEC(name, ptr_or_none)                                                                                                                    \
    ...
    void wasm_##name##_vec_new_uninitialized(own wasm_##name##_vec_t* out, size_t);                                                                            \
    void wasm_##name##_vec_new(own wasm_##name##_vec_t* out, size_t, own wasm_##name##_t ptr_or_none const[]);                                                 \
    void wasm_##name##_vec_copy(own wasm_##name##_vec_t* out, wasm_##name##_vec_t*);                                                                           \
...
static inline void wasm_name_new_from_string(own wasm_name_t* out, const char* s)
{
...
typedef own wasm_trap_t* (*wasm_func_callback_with_env_t)(void* env, const wasm_val_t args[], wasm_val_t results[]);

own wasm_func_t* wasm_func_new(wasm_store_t*, const wasm_functype_t*, wasm_func_callback_t);
own wasm_func_t* wasm_func_new_with_env(wasm_store_t*, const wasm_functype_t* type, wasm_func_callback_with_env_t, void* env, void (*finalizer)(void*));
...
own wasm_trap_t* wasm_func_call(const wasm_func_t*, const wasm_val_t args[], wasm_val_t results[]);

// Global Instances

WASM_DECLARE_REF(global)

own wasm_global_t* wasm_global_new(wasm_store_t*, const wasm_globaltype_t*, const wasm_val_t*);
...
own wasm_table_t* wasm_table_new(wasm_store_t*, const wasm_tabletype_t*, wasm_ref_t* init);
...
own wasm_instance_t* wasm_instance_new(wasm_store_t*, const wasm_module_t*, const wasm_extern_t* const imports[], own wasm_trap_t**);
...
static inline own wasm_functype_t* wasm_functype_new_3_2(
    own wasm_valtype_t* p1, own wasm_valtype_t* p2, own wasm_valtype_t* p3, own wasm_valtype_t* r1, own wasm_valtype_t* r2)
{
    wasm_valtype_t* ps[3] = { p1, p2, p3 };
    wasm_valtype_t* rs[2] = { r1, r2 };
    wasm_valtype_vec_t params, results;
