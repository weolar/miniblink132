...
public:
    ...
    constexpr span(const T* data, index_type size)
        : data_(data)
        , size_(size)
    {
    }
    ...
    constexpr const T& operator[](index_type idx) const
    {
        return data_[idx];
    }
...
template <size_t N> constexpr span<char> MakeSpan(const char (&str)[N])
{
    return span<char>(str, N - 1);
}

template <size_t N> constexpr span<uint8_t> SpanFrom(const char (&str)[N])
{
...
{
    return str ? span<uint8_t>(reinterpret_cast<const uint8_t*>(str), strlen(str)) : span<uint8_t>();
}
...
template <typename C, typename = std::enable_if_t<std::is_unsigned<typename C::value_type> {} && std::is_member_function_pointer<decltype(&C::size)> {}>>
