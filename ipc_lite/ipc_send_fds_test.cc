...
void TestDescriptorServer(IPC::Channel& chan, base::ProcessHandle process_handle)
{
...
int TestDescriptorClient(ino_t expected_inode_num)
{
    ...
    // Setup IPC channel.
    IPC::Channel chan(kTestClientChannel, IPC::Channel::MODE_CLIENT, &listener);
    CHECK(chan.Connect());
...
MULTIPROCESS_IPC_TEST_MAIN(RunTestDescriptorClientSandboxed)
{
    ...
    int error = sandbox_init(kSBXProfilePureComputation, SANDBOX_NAMED, &error_buff);
...
TEST_F(IPCChannelTest, DescriptorTestSandboxed)
{
    ...
    IPC::Channel chan(kTestClientChannel, IPC::Channel::MODE_SERVER, &listener);
    ASSERT_TRUE(chan.Connect());

    base::ProcessHandle process_handle = SpawnChild(TEST_DESCRIPTOR_CLIENT_SANDBOXED, &chan);
...
TEST_F(IPCChannelTest, DescriptorTest)
{
    ...
    base::ProcessHandle process_handle = SpawnChild(TEST_DESCRIPTOR_CLIENT, &chan);
    TestDescriptorServer(chan, process_handle);
}
