...
#error "What platform?"
    ...
    void SetHandle(base::SyncSocket::Handle handle)
    {
        ...
        EXPECT_EQ(sync_socket.Send(kHelloString, kHelloStringLength), kHelloStringLength);
...
TEST_F(SyncSocketTest, SanityTest)
{
    ...
    IPC::Channel chan(kSyncSocketChannel, IPC::Channel::MODE_SERVER, &listener);
    ...
    BOOL retval = DuplicateHandle(GetCurrentProcess(), pair[1].handle(), server_process, &target_handle, 0, FALSE, DUPLICATE_SAME_ACCESS);
    EXPECT_TRUE(retval);
    ...
    pair[0].Close();
    pair[1].Close();
    EXPECT_TRUE(base::WaitForSingleProcess(server_process, base::TimeDelta::FromSeconds(5)));
...
static void BlockingRead(base::SyncSocket* socket, char* buf, size_t length, size_t* received)
{
    DCHECK(buf != NULL);
...
TEST_F(SyncSocketTest, DisconnectTest)
{
    ...
    worker.message_loop()->PostTask(FROM_HERE, base::Bind(&BlockingRead, &pair[0], &buf[0], arraysize(buf), &received));
...
TEST_F(SyncSocketTest, BlockingReceiveTest)
{
    ...
    size_t received = 1U; // Initialize to an unexpected value.
    worker.message_loop()->PostTask(FROM_HERE, base::Bind(&BlockingRead, &pair[0], &buf[0], kHelloStringLength, &received));
