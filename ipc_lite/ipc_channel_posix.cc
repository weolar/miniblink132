...
#include "ipc_lite/ipc_channel_posix.h"
...
#include "ipc_lite/ipc_descriptors.h"
#include "ipc_lite/ipc_logging.h"
#include "ipc_lite/ipc_message_utils.h"
#include "ipc_lite/ipc_switches.h"
...
public:
    ...
    {
        return base::Singleton<PipeMap>::get();
    }
...
COMPILE_ASSERT(sizeof(((sockaddr_un*)0)->sun_path) >= kMaxPipeNameLength, BAD_SUN_PATH_LENGTH);

// Creates a unix domain socket bound to the specified name that is listening
// for connections.
bool CreateServerUnixDomainSocket(const std::string& pipe_name, int* server_listen_fd)
{
    ...
    int path_len = snprintf(unix_addr.sun_path, IPC::kMaxPipeNameLength, "%s", pipe_name.c_str());
    DCHECK_EQ(static_cast<int>(pipe_name.length()), path_len);
    size_t unix_addr_len = offsetof(struct sockaddr_un, sun_path) + path_len + 1;

    // Bind the socket.
    if (bind(fd, reinterpret_cast<const sockaddr*>(&unix_addr), unix_addr_len) != 0) {
...
bool CreateClientUnixDomainSocket(const std::string& pipe_name, int* client_socket)
{
    ...
    int path_len = snprintf(server_unix_addr.sun_path, IPC::kMaxPipeNameLength, "%s", pipe_name.c_str());
    DCHECK_EQ(static_cast<int>(pipe_name.length()), path_len);
    size_t server_unix_addr_len = offsetof(struct sockaddr_un, sun_path) + path_len + 1;

    if (HANDLE_EINTR(connect(fd, reinterpret_cast<sockaddr*>(&server_unix_addr), server_unix_addr_len)) != 0) {
...
Channel::ChannelImpl::ChannelImpl(const IPC::ChannelHandle& channel_handle, Mode mode, Listener* listener)
    : ChannelReader(listener)
    , mode_(mode)
    , peer_pid_(base::kNullProcessId)
    , server_listen_connection_watcher_(FROM_HERE)
    , read_watcher_(FROM_HERE)
    , write_watcher_(/*base::ThreadTaskRunnerHandle::Get()*/ FROM_HERE)
    ...
    , must_unlink_(false)
{
    ...
    if (!CreatePipe(channel_handle)) {
        ...
        LOG(WARNING) << "Unable to create pipe named \"" << channel_handle.name << "\" in " << modestr << " mode";
    }
}
        ...
        if (mode_ & MODE_CLIENT_FLAG) {
            if (local_pipe != -1) {
                ...
                if (used_initial_channel) {
                    LOG(FATAL) << "Denying attempt to reuse initial IPC channel for " << pipe_name_;
                    return false;
                }
                used_initial_channel = true;

                local_pipe = base::GlobalDescriptors::GetInstance()->Get(kPrimaryIPCChannel);
            }
...
bool Channel::ChannelImpl::Connect()
{
    ...
    if (server_listen_pipe_ != -1) {
        ...
        base::CurrentIOThread::Get()->WatchFileDescriptor(
            server_listen_pipe_, true, base::WatchableIOMessagePumpPosix::WATCH_READ, &server_listen_connection_watcher_, this);
    } else {
...
bool Channel::ChannelImpl::ProcessOutgoingMessages()
{
    ...
    while (!output_queue_.empty()) {
        ...
        const char* out_bytes = reinterpret_cast<const char*>(msg->data()) + message_send_bytes_written_;

        struct msghdr msgh = { 0 };
        struct iovec iov = { const_cast<char*>(out_bytes), amt_to_write };
        msgh.msg_iov = &iov;
        msgh.msg_iovlen = 1;
        char buf[CMSG_SPACE(sizeof(int) * FileDescriptorSet::kMaxDescriptorsPerMessage)];

        ssize_t bytes_written = 1;
        int fd_written = -1;

        if (message_send_bytes_written_ == 0 && !msg->file_descriptor_set()->empty()) {
            ...
            msg->file_descriptor_set()->GetDescriptors(reinterpret_cast<int*>(CMSG_DATA(cmsg)));
        ...
        if (bytes_written < 0 && !SocketWriteErrorIsRecoverable()) {
            ...
            PLOG(ERROR) << "pipe error on " << fd_written << " Currently writing message of size: " << msg->size();
            return false;
        }
        ...
        if (static_cast<size_t>(bytes_written) != amt_to_write) {
            ...
            base::CurrentIOThread::Get()->WatchFileDescriptor(pipe_,
                false, // One shot
                base::WatchableIOMessagePumpPosix::WATCH_WRITE, &write_watcher_, this);
            return true;
        } else {
            message_send_bytes_written_ = 0;

            // Message sent OK!
            DVLOG(2) << "sent message @" << msg << " on channel @" << this << " with type " << msg->type() << " on fd " << pipe_;
...
// static
bool Channel::ChannelImpl::IsNamedServerInitialized(const std::string& channel_id)
{
...
{
    base::CurrentIOThread::Get()->WatchFileDescriptor(pipe_, true, base::WatchableIOMessagePumpPosix::WATCH_READ, &read_watcher_, this);
    QueueHelloMessage();
...
void Channel::ChannelImpl::QueueHelloMessage()
{
    ...
    std::unique_ptr<Message> msg(new Message(MSG_ROUTING_NONE, HELLO_MESSAGE_TYPE, IPC::Message::PRIORITY_NORMAL));
    msg->WriteInt(GetHelloMessageProcId());

#if defined(IPC_USES_READWRITE)
    std::unique_ptr<Message> hello;
    if (remote_fd_pipe_ != -1) {
        //         if (!msg->WriteFileDescriptor(base::FileDescriptor(remote_fd_pipe_, false))) {
...
Channel::ChannelImpl::ReadState Channel::ChannelImpl::ReadData(char* buffer, int buffer_len, int* bytes_read)
{
    if (pipe_ == -1)
        return READ_FAILED;
...
bool Channel::ChannelImpl::WillDispatchInputMessage(Message* msg)
{
    ...
    if (error) {
        LOG(WARNING) << error << " channel:" << this << " message-type:" << msg->type() << " header()->num_fds:" << header_fds;
    ...
    msg->file_descriptor_set()->SetDescriptors(&input_fds_.front(), header_fds);
...
bool Channel::ChannelImpl::ExtractFileDescriptorsFromMsghdr(msghdr* msg)
{
    ...
    for (cmsghdr* cmsg = CMSG_FIRSTHDR(msg); cmsg; cmsg = CMSG_NXTHDR(msg, cmsg)) {
        if (cmsg->cmsg_level == SOL_SOCKET && cmsg->cmsg_type == SCM_RIGHTS) {
            ...
            input_fds_.insert(input_fds_.end(), file_descriptors, file_descriptors + num_file_descriptors);
