...
#include "ipc_lite/base/atomic_sequence_num.h"
...
template <typename Type> struct DefaultLazyInstanceTraits {
    ...
    {
        DCHECK_EQ(reinterpret_cast<uintptr_t>(instance) & (ALIGNOF(Type) - 1), 0u) << ": Bad boy, the buffer passed to placement new is not aligned!\n"
...
struct WaitableEventLazyInstanceTraits : public base::DefaultLazyInstanceTraits<base::WaitableEvent> {
    static base::WaitableEvent* New(void* instance)
    {
        // Use placement new to initialize our instance in our preallocated space.
        //return new (instance) base::WaitableEvent(true, true);
        return new (instance) base::WaitableEvent(base::WaitableEvent::ResetPolicy::MANUAL, base::WaitableEvent::InitialState::SIGNALED);
    }
};

base::LazyInstance<base::WaitableEvent, WaitableEventLazyInstanceTraits> dummy_event = LAZY_INSTANCE_INITIALIZER;
...
SyncMessage::SyncMessage(int32 routing_id, uint32 type, PriorityValue priority, MessageReplyDeserializer* deserializer)
