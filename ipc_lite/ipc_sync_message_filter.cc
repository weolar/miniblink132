...
#include "ipc_lite/ipc_sync_message_filter.h"
...
SyncMessageFilter::SyncMessageFilter(base::WaitableEvent* shutdown_event)
    : channel_(NULL)
    , listener_loop_(/*MessageLoopProxy::current()*/ base::SingleThreadTaskRunner::GetCurrentDefault())
...
bool SyncMessageFilter::Send(Message* message)
{
    ...
    if (!message->is_sync()) {
        io_loop_->PostTask(FROM_HERE, base::BindOnce(&SyncMessageFilter::SendOnIOThread, base::RetainedRef(this), message));
        return true;
    }

    //base::WaitableEvent done_event(true, false);
    base::WaitableEvent done_event(base::WaitableEvent::ResetPolicy::MANUAL, base::WaitableEvent::InitialState::NOT_SIGNALED);
    PendingSyncMsg pending_message(SyncMessage::GetMessageId(*message), reinterpret_cast<SyncMessage*>(message)->GetReplyDeserializer(), &done_event);

    {
        ...
        DCHECK(base::SequencedTaskRunner::GetCurrentDefault() != listener_loop_);
        DCHECK(base::SequencedTaskRunner::GetCurrentDefault() != io_loop_);
...
void SyncMessageFilter::OnFilterAdded(Channel* channel)
{
    channel_ = channel;
    base::AutoLock auto_lock(lock_);
    io_loop_ = base::SingleThreadTaskRunner::GetCurrentDefault(); // MessageLoopProxy::current();
}
...
{
    base::AutoLock auto_lock(lock_);
    for (PendingSyncMessages::iterator iter = pending_sync_messages_.begin(); iter != pending_sync_messages_.end(); ++iter) {
