...
class Channel::ChannelImpl::ReaderThreadRunner : public base::DelegateSimpleThread::Delegate {
public:
    ...
    //                      above callbacks.
    ReaderThreadRunner(int pipe, base::Callback<void(scoped_ptr<MessageContents>)> data_read_callback, base::Callback<void()> failure_callback,
...
Channel::ChannelImpl::ReaderThreadRunner::ReaderThreadRunner(int pipe, base::Callback<void(scoped_ptr<MessageContents>)> data_read_callback,
    base::Callback<void()> failure_callback, scoped_refptr<base::MessageLoopProxy> main_message_loop)
    : pipe_(pipe)
...
void Channel::ChannelImpl::ReaderThreadRunner::Run()
{
    while (true) {
        ...
        if (success) {
            main_message_loop_->PostTask(FROM_HERE, base::Bind(data_read_callback_, base::Passed(msg_contents.Pass())));
        } else {
...
bool Channel::ChannelImpl::Connect()
{
    ...
    reader_thread_runner_.reset(new ReaderThreadRunner(pipe_, base::Bind(&Channel::ChannelImpl::DidRecvMsg, weak_ptr_factory_.GetWeakPtr()),
        base::Bind(&Channel::ChannelImpl::ReadDidFail, weak_ptr_factory_.GetWeakPtr()), base::MessageLoopProxy::current()));
    reader_thread_.reset(new base::DelegateSimpleThread(reader_thread_runner_.get(), "ipc_channel_nacl reader thread"));
...
{
    DVLOG(2) << "sending message @" << message << " on channel @" << this << " with type " << message->type();
...
void Channel::ChannelImpl::DidRecvMsg(scoped_ptr<MessageContents> contents)
{
    ...
    input_fds_.insert(input_fds_.end(), contents->fds.begin(), contents->fds.end());
    contents->fds.clear();
...
bool Channel::ChannelImpl::ProcessOutgoingMessages()
{
    ...
    while (!output_queue_.empty()) {
        ...
        if (bytes_written < 0) {
            ...
            DCHECK(errno == EPIPE);
            Close();
            PLOG(ERROR) << "pipe_ error on " << pipe_ << " Currently writing message of size: " << msg->size();
            return false;
        } else {
        ...
        // Message sent OK!
        DVLOG(2) << "sent message @" << msg.get() << " with type " << msg->type() << " on fd " << pipe_;
    }
    return true;
}
