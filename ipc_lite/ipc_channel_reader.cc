...
bool ChannelReader::DispatchInputData(const char* input_data, int input_data_len)
{
    const char* p;
    const char* end;
    ...
    if (input_overflow_buf_.empty()) {
        ...
        if (input_overflow_buf_.size() > Channel::kMaximumMessageSize - input_data_len) {
...
#else
                //TRACE_EVENT2("ipc", "ChannelReader::DispatchInputData", "class", IPC_MESSAGE_ID_CLASS(m.type()), "line", IPC_MESSAGE_ID_LINE(m.type()));
#endif
            m.TraceMessageEnd();
            if (IsHelloMessage(m))
