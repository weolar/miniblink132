...
FileDescriptorSet::~FileDescriptorSet()
{
    ...
    // kernel resources.
    for (unsigned i = consumed_descriptor_highwater_; i < descriptors_.size(); ++i) {
...
{
    for (std::vector<base::FileDescriptor>::const_iterator i = descriptors_.begin(); i != descriptors_.end(); ++i) {
        *(buffer++) = i->fd;
    }
}
...
{
    for (std::vector<base::FileDescriptor>::iterator i = descriptors_.begin(); i != descriptors_.end(); ++i) {
        if (i->auto_close)
