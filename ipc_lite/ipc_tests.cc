...
#if defined(OS_WIN)
base::ProcessHandle IPCChannelTest::SpawnChild(ChildType child_type, IPC::Channel* channel)
{
    // kDebugChildren support.
    bool debug_on_start = CommandLine::ForCurrentProcess()->HasSwitch(switches::kDebugChildren);
...
base::ProcessHandle IPCChannelTest::SpawnChild(ChildType child_type, IPC::Channel* channel)
{
    ...
    switch (child_type) {
    case TEST_CLIENT:
        ret = MultiProcessTest::SpawnChild("RunTestClient", fds_to_map, debug_on_start);
        break;
    case TEST_DESCRIPTOR_CLIENT:
        ret = MultiProcessTest::SpawnChild("RunTestDescriptorClient", fds_to_map, debug_on_start);
        break;
    case TEST_DESCRIPTOR_CLIENT_SANDBOXED:
        ret = MultiProcessTest::SpawnChild("RunTestDescriptorClientSandboxed", fds_to_map, debug_on_start);
        break;
    case TEST_REFLECTOR:
        ret = MultiProcessTest::SpawnChild("RunReflector", fds_to_map, debug_on_start);
        break;
    case FUZZER_SERVER:
        ret = MultiProcessTest::SpawnChild("RunFuzzServer", fds_to_map, debug_on_start);
        break;
    case SYNC_SOCKET_SERVER:
        ret = MultiProcessTest::SpawnChild("RunSyncSocketServer", fds_to_map, debug_on_start);
        break;
    default:
...
TEST_F(IPCChannelTest, ChannelTest)
{
    ...
    // Setup IPC channel.
    IPC::Channel chan(kTestClientChannel, IPC::Channel::MODE_SERVER, &channel_listener);
    ...
    EXPECT_TRUE(base::WaitForSingleProcess(process_handle, base::TimeDelta::FromSeconds(5)));
...
TEST_F(IPCChannelTest, ChannelTestExistingPipe)
{
    ...
    HANDLE pipe = CreateNamedPipeA(name.c_str(), open_mode, PIPE_TYPE_BYTE | PIPE_READMODE_BYTE, 1, 4096, 4096, 5000, NULL);
    IPC::Channel chan(IPC::ChannelHandle(pipe), IPC::Channel::MODE_SERVER, &channel_listener);
...
TEST_F(IPCChannelTest, ChannelProxyTest)
{
    ...
    thread.StartWithOptions(options);
    {
        ...
        IPC::ChannelProxy chan(kTestClientChannel, IPC::Channel::MODE_SERVER, &channel_listener, thread.message_loop_proxy());
        ...
        base::ProcessHandle process_handle = MultiProcessTest::SpawnChild("RunTestClient", fds_to_map, debug_on_start);
#endif // defined(OS_POSIX)
...
TEST_F(IPCChannelTest, MAYBE_SendMessageInChannelConnected)
{
    ...
    // Setup IPC channel.
    IPC::Channel channel(kTestClientChannel, IPC::Channel::MODE_SERVER, &channel_listener);
...
MULTIPROCESS_IPC_TEST_MAIN(RunTestClient)
{
    ...
    // setup IPC channel
    IPC::Channel chan(kTestClientChannel, IPC::Channel::MODE_CLIENT, &channel_listener);
    CHECK(chan.Connect());
    channel_listener.Init(&chan);
