...
#include "ipc_lite/ipc_channel_win.h"

#include <windows.h>

#include "base/auto_reset.h"
#include "base/bind.h"
#include "ipc_lite/base/ipc_compiler_specific.h"
...
{
    COMPILE_ASSERT(!offsetof(Channel::ChannelImpl::State, context), starts_with_io_context);
}
...
{
    if (WaitNamedPipeW((const WCHAR*)(PipeName(channel_id, NULL).c_str()), 1))
        return true;
...
Channel::ChannelImpl::ReadState Channel::ChannelImpl::ReadData(char* buffer, int buffer_len, int* /* bytes_read */)
{
    ...
    DWORD bytes_read = 0;
    BOOL ok = ReadFile(pipe_, buffer, buffer_len, &bytes_read, &input_state_.context.overlapped);
    if (!ok) {
...
// static
const std::wstring Channel::ChannelImpl::PipeName(const std::string& channel_id, int32* secret)
{
...
bool Channel::ChannelImpl::CreatePipe(const IPC::ChannelHandle& channel_handle, Mode mode)
{
    ...
    if (channel_handle.pipe.handle) {
        ...
        if (!DuplicateHandle(GetCurrentProcess(), channel_handle.pipe.handle, GetCurrentProcess(), &pipe_, 0, FALSE, DUPLICATE_SAME_ACCESS)) {
        ...
        pipe_ = CreateNamedPipeW(
            pipe_name.c_str(), open_mode, PIPE_TYPE_BYTE | PIPE_READMODE_BYTE, 1, Channel::kReadBufferSize, Channel::kReadBufferSize, 5000, NULL);
    } else if (mode & MODE_CLIENT_FLAG) {
        DCHECK(!channel_handle.pipe.handle);
        pipe_name = PipeName(channel_handle.name, &client_secret_);
        pipe_ = CreateFileW(pipe_name.c_str(), GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING,
    ...
    if (pipe_ == INVALID_HANDLE_VALUE) {
        ...
        LOG(WARNING) << "Unable to create pipe \"" << pipe_name << "\" in " << (mode & MODE_SERVER_FLAG ? "server" : "client")
                     << " mode. Error :" << GetLastError();
        return false;
    }

    // Create the Hello message to be sent when Connect is called
    std::unique_ptr<Message> m(new Message(MSG_ROUTING_NONE, HELLO_MESSAGE_TYPE, IPC::Message::PRIORITY_NORMAL));
    ...
    //         return false;
    //     }
    m->WriteInt(GetCurrentProcessId());
...
bool Channel::ChannelImpl::Connect()
{
    ...
    base::CurrentIOThread::Get()->RegisterIOHandler(pipe_, this);
    ...
    if (!input_state_.is_pending) {
        ...
        //base::MessagePumpForIO::current()->PostTask(
        base::SequencedTaskRunner::GetCurrentDefault()->PostTask(
            FROM_HERE, base::BindOnce(&Channel::ChannelImpl::OnIOCompleted, weak_factory_.GetWeakPtr(), &input_state_.context, 0, 0));
    }
...
bool Channel::ChannelImpl::ProcessOutgoingMessages(base::MessagePumpForIO::IOContext* context, DWORD bytes_written)
{
    ...
    BOOL ok = WriteFile(pipe_, m->data(), static_cast<int>(m->size()), &bytes_written, &output_state_.context.overlapped);
    if (!ok) {
        DWORD err = GetLastError();
        if (err == ERROR_IO_PENDING) {
            output_state_.is_pending = true;

            DVLOG(2) << "sent pending message @" << m << " on channel @" << this << " with type " << m->type();
    ...
    DVLOG(2) << "sent message @" << m << " on channel @" << this << " with type " << m->type();

    output_state_.is_pending = true;
    return true;
}

void Channel::ChannelImpl::OnIOCompleted(base::MessagePumpForIO::IOContext* context, DWORD bytes_transfered, DWORD error)
{
    bool ok = true;
    DCHECK(thread_check_->CalledOnValidThread());
    if (context == &input_state_.context) {
        ...
        base::AutoReset<bool> auto_reset_processing_incoming(&processing_incoming_, true);
