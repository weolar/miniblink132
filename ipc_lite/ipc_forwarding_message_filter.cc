...
#include "ipc_lite/ipc_forwarding_message_filter.h"
...
ForwardingMessageFilter::ForwardingMessageFilter(const uint32* message_ids_to_filter, size_t num_message_ids_to_filter, base::TaskRunner* target_task_runner)
...
{
    if (message_ids_to_filter_.find(message.type()) == message_ids_to_filter_.end())
        return false;
    ...
    target_task_runner_->PostTask(FROM_HERE, base::BindOnce(handler, message));
    return true;
}
