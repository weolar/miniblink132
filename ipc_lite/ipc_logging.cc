...
void Logging::OnPostDispatchMessage(const Message& message, const std::string& channel_id)
{
    ...
    if (main_thread_->BelongsToCurrentThread()) {
        ...
        main_thread_->PostTask(FROM_HERE, base::BindOnce(&Logging::Log, base::Unretained(this), data));
    }
}

void Logging::GetMessageText(uint32 type, std::string* name, const Message* message, std::string* params)
{
        ...
        if (sender_) {
            ...
            if (!queue_invoke_later_pending_) {
                ...
                base::SequencedTaskRunner::GetCurrentDefault()->PostDelayedTask(
                    FROM_HERE, base::BindOnce(&Logging::OnSendLogs, base::Unretained(this)), base::Milliseconds(kLogSendDelayMs));
            }
        }
    }
...
//         fprintf(stderr, "ipc %s %d %s %s%s %s%s\n  %18.5f %s%18.5f %s%18.5f%s\n", data.channel.c_str(), data.routing_id, data.flags.c_str(),
//             ANSIEscape(sender_ ? ANSI_COLOR_BLUE : ANSI_COLOR_CYAN), message_name.c_str(), ANSIEscape(ANSI_COLOR_RESET), data.params.c_str(),
//             Time::FromInternalValue(data.sent).ToDoubleT(), ANSIEscape(DelayColor(receive_delay)), Time::FromInternalValue(data.receive).ToDoubleT(),
//             ANSIEscape(DelayColor(dispatch_delay)), Time::FromInternalValue(data.dispatch).ToDoubleT(), ANSIEscape(ANSI_COLOR_RESET));
    }
}

void GenerateLogData(const std::string& channel, const Message& message, LogData* data, bool get_params)
{
    if (message.is_reply()) {
        ...
        Logging::GetMessageText(message.type(), &message_name, &message, get_params ? &params : NULL);
