...
public:
    enum STATUS { DISCONNECTED, MESSAGE_RECEIVED, CHANNEL_ERROR, CONNECTED, DENIED, LISTEN_ERROR };
...
public:
    static void SetUpSocket(IPC::ChannelHandle* handle, IPC::Channel::Mode mode);
...
void IPCChannelPosixTest::SetUpSocket(IPC::ChannelHandle* handle, IPC::Channel::Mode mode)
{
    ...
    int path_len = snprintf(server_address.sun_path, IPC::kMaxPipeNameLength, "%s", name.c_str());
    DCHECK_EQ(static_cast<int>(name.length()), path_len);
    size_t server_address_len = offsetof(struct sockaddr_un, sun_path) + path_len + 1;
    ...
    if (mode == IPC::Channel::MODE_NAMED_SERVER) {
        ...
        ASSERT_GE(bind(socket_fd, reinterpret_cast<struct sockaddr*>(&server_address), server_address_len), 0)
            << server_address.sun_path << ": " << strerror(errno) << "(" << errno << ")";
        ASSERT_GE(listen(socket_fd, SOMAXCONN), 0) << server_address.sun_path << ": " << strerror(errno) << "(" << errno << ")";
    } else if (mode == IPC::Channel::MODE_NAMED_CLIENT) {
        ASSERT_GE(connect(socket_fd, reinterpret_cast<struct sockaddr*>(&server_address), server_address_len), 0)
            << server_address.sun_path << ": " << strerror(errno) << "(" << errno << ")";
    } else {
...
void IPCChannelPosixTest::SpinRunLoop(base::TimeDelta delay)
{
    ...
    loop->PostDelayedTask(FROM_HERE, MessageLoop::QuitClosure(), delay);
    loop->Run();
}
...
TEST_F(IPCChannelPosixTest, AdvancedConnected)
{
    ...
    base::ProcessHandle handle = SpawnChild("IPCChannelPosixTestConnectionProc", false);
    ASSERT_TRUE(handle);
...
TEST_F(IPCChannelPosixTest, ResetState)
{
    ...
    base::ProcessHandle handle2 = SpawnChild("IPCChannelPosixTestConnectionProc", false);
    ASSERT_TRUE(handle2);
    SpinRunLoop(TestTimeouts::action_max_timeout());
    ASSERT_EQ(IPCChannelPosixTestListener::CONNECTED, listener.status());
    ASSERT_TRUE(channel.HasAcceptedConnection());
...
TEST_F(IPCChannelPosixTest, MultiConnection)
{
    ...
    ASSERT_EQ(IPCChannelPosixTestListener::CONNECTED, listener.status());
    ASSERT_TRUE(channel.HasAcceptedConnection());
    base::ProcessHandle handle2 = SpawnChild("IPCChannelPosixFailConnectionProc", false);
    ASSERT_TRUE(handle2);
    SpinRunLoop(TestTimeouts::action_max_timeout());
    int exit_code = 0;
    EXPECT_TRUE(base::WaitForExitCode(handle2, &exit_code));
...
TEST_F(IPCChannelPosixTest, IsNamedServerInitialized)
{
    ...
    ASSERT_FALSE(IPC::Channel::IsNamedServerInitialized(connection_socket_name));
    IPC::Channel channel(chan_handle, IPC::Channel::MODE_NAMED_SERVER, &listener);
    ASSERT_TRUE(IPC::Channel::IsNamedServerInitialized(connection_socket_name));
    channel.Close();
    ASSERT_FALSE(IPC::Channel::IsNamedServerInitialized(connection_socket_name));
}
