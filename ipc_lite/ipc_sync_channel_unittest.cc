...
public:
    ...
    virtual ~Worker()
    {
        ...
        ListenerThread()->message_loop()->PostTask(FROM_HERE, base::Bind(&Worker::OnListenerThreadShutdown1, this, &listener_done, &ipc_done));
        listener_done.Wait();
        ipc_done.Wait();
        ipc_thread_.Stop();
    ...
    void Start()
    {
        ...
        ListenerThread()->message_loop()->PostTask(FROM_HERE, base::Bind(&Worker::OnStart, this));
    }
...
protected:
    ...
    virtual void OnAnswer(int* answer)
    {
        NOTREACHED();
    }
    ...
    virtual void OnDouble(int in, int* out)
    {
        NOTREACHED();
    }
    ...
    {
        return new SyncChannel(channel_name_, mode_, this, ipc_thread_.message_loop_proxy(), true, &shutdown_event_);
    }
    ...
    const base::Thread& ipc_thread() const
    {
        return ipc_thread_;
    }
...
private:
    ...
    void OnListenerThreadShutdown1(WaitableEvent* listener_event, WaitableEvent* ipc_event)
    {
        ...
        ipc_thread_.message_loop()->PostTask(FROM_HERE, base::Bind(&Worker::OnIPCThreadShutdown, this, listener_event, ipc_event));
    }

    void OnIPCThreadShutdown(WaitableEvent* listener_event, WaitableEvent* ipc_event)
    {
        MessageLoop::current()->RunAllPending();
        ipc_event->Signal();

        listener_thread_.message_loop()->PostTask(FROM_HERE, base::Bind(&Worker::OnListenerThreadShutdown2, this, listener_event));
    }
    ...
    bool OnMessageReceived(const Message& message)
    {
        ...
        IPC_MESSAGE_HANDLER_DELAY_REPLY(SyncChannelNestedTestMsg_String, OnNestedTestMsg)
        IPC_END_MESSAGE_MAP()
        return true;
    }
...
public:
    ...
    virtual SyncChannel* CreateChannel()
    {
        SyncChannel* channel = new SyncChannel(this, ipc_thread().message_loop_proxy(), shutdown_event());
...
void Recursive(bool server_pump_first, bool server_pump_second, bool client_pump)
{
    std::vector<Worker*> workers;
    workers.push_back(new RecursiveServer(true, server_pump_first, server_pump_second));
...
void RecursiveNoHang(bool server_pump_first, bool server_pump_second, bool client_pump)
{
    std::vector<Worker*> workers;
    workers.push_back(new RecursiveServer(false, server_pump_first, server_pump_second));
...
public:
    MultipleClient1(WaitableEvent* client1_msg_received, WaitableEvent* client1_can_reply)
...
public:
    MultipleClient2(WaitableEvent* client1_msg_received, WaitableEvent* client1_can_reply, bool pump_during_send)
...
void Multiple(bool server_pump, bool client_pump)
{
    ...
    worker = new MultipleClient2(&client1_msg_received, &client1_can_reply, client_pump);
    workers.push_back(worker);
...
public:
    QueuedReplyServer(base::Thread* listener_thread, const std::string& channel_name, const std::string& reply_text)
...
public:
    QueuedReplyClient(base::Thread* listener_thread, const std::string& channel_name, const std::string& expected_text, bool pump_during_send)
...
void QueuedReply(bool client_pump)
{
    ...
    worker = new QueuedReplyServer(&server_worker_thread, "QueuedReply_Server1", "Got first message");
    workers.push_back(worker);

    worker = new QueuedReplyServer(&server_worker_thread, "QueuedReply_Server2", "Got second message");
    workers.push_back(worker);

    worker = new QueuedReplyClient(&client_worker_thread, "QueuedReply_Server1", "Got first message", client_pump);
    workers.push_back(worker);

    worker = new QueuedReplyClient(&client_worker_thread, "QueuedReply_Server2", "Got second message", client_pump);
    workers.push_back(worker);
...
public:
    TimeoutServer(int timeout_ms, std::vector<bool> timeout_seq, bool pump_during_send)
    ...
    void Run()
    {
        for (std::vector<bool>::const_iterator iter = timeout_seq_.begin(); iter != timeout_seq_.end(); ++iter) {
...
public:
    ...
    void Run()
    {
        MessageLoop::current()->PostTask(FROM_HERE, base::Bind(&NestedCallback, this));
        MessageLoop::current()->PostDelayedTask(FROM_HERE, base::Bind(&TimeoutCallback), base::TimeDelta::FromSeconds(9));
...
public:
    TestSyncMessageFilter(base::WaitableEvent* shutdown_event, Worker* worker, scoped_refptr<base::MessageLoopProxy> message_loop)
    ...
    virtual void OnFilterAdded(Channel* channel)
    {
        ...
        message_loop_->PostTask(FROM_HERE, base::Bind(&TestSyncMessageFilter::SendMessageOnHelperThread, this));
    }
...
public:
        ...
        , thread_("helper_thread")
    {
        ...
        filter_ = new TestSyncMessageFilter(shutdown_event(), this, thread_.message_loop_proxy());
    }
...
public:
    ...
    bool SendDummy()
    {
        ListenerThread()->message_loop()->PostTask(FROM_HERE, base::Bind(base::IgnoreResult(&ServerSendAfterClose::Send), this, new SyncChannelTestMsg_NoArgs));
        return true;
    }
...
public:
    RestrictedDispatchServer(WaitableEvent* sent_ping_event, WaitableEvent* wait_event)
    ...
    void OnDoPing(int ping)
    {
        ...
        // IPC thread.
        ipc_thread().message_loop()->PostTask(FROM_HERE, base::Bind(&RestrictedDispatchServer::OnPingSent, this));
    }
...
public:
    RestrictedDispatchClient(WaitableEvent* sent_ping_event, RestrictedDispatchServer* server, NonRestrictedDispatchServer* server2, int* success)
    ...
    void Run()
    {
        ...
        server_->ListenerThread()->message_loop()->PostTask(FROM_HERE, base::Bind(&RestrictedDispatchServer::OnDoPing, server_, 1));
        ...
        non_restricted_channel_.reset(
            new SyncChannel("non_restricted_channel", Channel::MODE_CLIENT, this, ipc_thread().message_loop_proxy(), true, shutdown_event()));

        server_->ListenerThread()->message_loop()->PostTask(FROM_HERE, base::Bind(&RestrictedDispatchServer::OnDoPing, server_, 2));
        sent_ping_event_->Wait();
        ...
        server2_->ListenerThread()->message_loop()->PostTask(FROM_HERE, base::Bind(&NonRestrictedDispatchServer::OnDoPingTTL, server2_, 3));
        int value = 0;
...
TEST_F(IPCSyncChannelTest, RestrictedDispatch)
{
    ...
    workers.push_back(new RestrictedDispatchClient(&sent_ping_event, server, server2, &success));
    RunTest(workers);
    EXPECT_EQ(4, success);
}
...
public:
    RestrictedDispatchDeadlockServer(int server_num, WaitableEvent* server_ready_event, WaitableEvent** events, RestrictedDispatchDeadlockServer* peer)
...
public:
    RestrictedDispatchDeadlockClient2(RestrictedDispatchDeadlockServer* server, WaitableEvent* server_ready_event, WaitableEvent** events)
...
public:
    RestrictedDispatchDeadlockClient1(
        RestrictedDispatchDeadlockServer* server, RestrictedDispatchDeadlockClient2* peer, WaitableEvent* server_ready_event, WaitableEvent** events)
    ...
    {
        server_ready_event_->Wait();
        server_->ListenerThread()->message_loop()->PostTask(FROM_HERE, base::Bind(&RestrictedDispatchDeadlockServer::OnDoServerTask, server_));
        peer_->ListenerThread()->message_loop()->PostTask(FROM_HERE, base::Bind(&RestrictedDispatchDeadlockClient2::OnDoClient2Task, peer_));
        events_[0]->Wait();
        events_[1]->Wait();
        DCHECK(received_msg_ == false);
...
TEST_F(IPCSyncChannelTest, RestrictedDispatchDeadlock)
{
    ...
    server2 = new RestrictedDispatchDeadlockServer(2, &server2_ready, events, NULL);
    server2->OverrideThread(&worker_thread);
    workers.push_back(server2);

    client2 = new RestrictedDispatchDeadlockClient2(server2, &server2_ready, events);
    workers.push_back(client2);

    server1 = new RestrictedDispatchDeadlockServer(1, &server1_ready, events, server2);
    server1->OverrideThread(&worker_thread);
    workers.push_back(server1);

    client1 = new RestrictedDispatchDeadlockClient1(server1, client2, &server1_ready, events);
...
public:
    RestrictedDispatchPipeWorker(
        const std::string& channel1, WaitableEvent* event1, const std::string& channel2, WaitableEvent* event2, int group, int* success)
    ...
    void Run()
    {
        ...
        if (is_first())
            event1_->Signal();
        event2_->Wait();
        other_channel_.reset(new SyncChannel(other_channel_name_, Channel::MODE_CLIENT, this, ipc_thread().message_loop_proxy(), true, shutdown_event()));
...
TEST_F(IPCSyncChannelTest, RestrictedDispatch4WayDeadlock)
{
    ...
    workers.push_back(new RestrictedDispatchPipeWorker("channel0", &event0, "channel1", &event1, 1, &success));
    workers.push_back(new RestrictedDispatchPipeWorker("channel1", &event1, "channel2", &event2, 2, NULL));
    workers.push_back(new RestrictedDispatchPipeWorker("channel2", &event2, "channel3", &event3, 3, NULL));
    workers.push_back(new RestrictedDispatchPipeWorker("channel3", &event3, "channel0", &event0, 4, NULL));
    RunTest(workers);
    EXPECT_EQ(3, success);
}
...
public:
    ...
    void Run()
    {
        server2_channel_.reset(new SyncChannel("reentrant_reply2", Channel::MODE_CLIENT, this, ipc_thread().message_loop_proxy(), true, shutdown_event()));
...
private:
    bool OnMessageReceived(const Message& message)
    {
        ...
        IPC_MESSAGE_HANDLER_DELAY_REPLY(SyncChannelTestMsg_Reentrant1, OnReentrant1)
...
public:
    VerifiedServer(base::Thread* listener_thread, const std::string& channel_name, const std::string& reply_text)
        : Worker(channel_name, Channel::MODE_SERVER)
        , reply_text_(reply_text)
    {
        Worker::OverrideThread(listener_thread);
...
public:
    VerifiedClient(base::Thread* listener_thread, const std::string& channel_name, const std::string& expected_text)
        : Worker(channel_name, Channel::MODE_CLIENT)
        , expected_text_(expected_text)
    {
        Worker::OverrideThread(listener_thread);
...
void Verified()
{
    ...
    worker = new VerifiedServer(&server_worker_thread, channel_id, "Got first message");
    workers.push_back(worker);

    worker = new VerifiedClient(&client_worker_thread, channel_id, "Got first message");
    workers.push_back(worker);
