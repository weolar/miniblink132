...
ChannelProxy::Context::Context(Listener* listener, scoped_refptr<base::SingleThreadTaskRunner> ipc_task_runner)
...
void ChannelProxy::Context::CreateChannel(const IPC::ChannelHandle& handle, const Channel::Mode& mode)
{
...
bool ChannelProxy::Context::OnMessageReceivedNoFilter(const Message& message)
{
    ...
    listener_task_runner_->PostTask(FROM_HERE, base::BindOnce(&Context::OnDispatchMessage, this, message));
    return true;
}
...
void ChannelProxy::Context::OnChannelConnected(int32 peer_pid)
{
    ...
    listener_task_runner_->PostTask(FROM_HERE, base::BindOnce(&Context::OnDispatchConnected, this));
}
...
void ChannelProxy::Context::OnChannelError()
{
    ...
    // See above comment about using listener_task_runner_ here.
    listener_task_runner_->PostTask(FROM_HERE, base::BindOnce(&Context::OnDispatchError, this));
}
...
void ChannelProxy::Context::OnDispatchMessage(const Message& message)
{
    ...
    //TRACE_EVENT1("task", "ChannelProxy::Context::OnDispatchMessage", "name", name);
#else
//     TRACE_EVENT2(
//         "task", "ChannelProxy::Context::OnDispatchMessage", "class", IPC_MESSAGE_ID_CLASS(message.type()), "line", IPC_MESSAGE_ID_LINE(message.type()));
#endif
...
ChannelProxy::ChannelProxy(
    const IPC::ChannelHandle& channel_handle, Channel::Mode mode, Listener* listener, scoped_refptr<base::SingleThreadTaskRunner> ipc_task_runner)
...
void ChannelProxy::Init(const IPC::ChannelHandle& channel_handle, Channel::Mode mode, bool create_pipe_now)
{
    DCHECK(!did_init_);
#if defined(OS_POSIX)
    ...
    if (create_pipe_now) {
        ...
        context_->ipc_task_runner()->PostTask(FROM_HERE, base::BindOnce(&Context::CreateChannel, context_.get(), channel_handle, mode));
    }

    // complete initialization on the background thread
    context_->ipc_task_runner()->PostTask(FROM_HERE, base::BindOnce(&Context::OnChannelOpened, context_.get()));
...
void ChannelProxy::Close()
{
    ...
    if (context_->ipc_task_runner()) {
        context_->ipc_task_runner()->PostTask(FROM_HERE, base::BindOnce(&Context::OnChannelClosed, context_.get()));
    }
}
...
bool ChannelProxy::Send(Message* message)
{
    ...
    context_->ipc_task_runner()->PostTask(FROM_HERE, base::BindOnce(&ChannelProxy::Context::OnSendMessage, context_, (std::unique_ptr<Message>(message))));
    return true;
}
...
{
    //context_->ipc_task_runner()->PostTask(FROM_HERE, base::BindOnce(&Context::OnRemoveFilter, context_.get(), make_scoped_refptr(filter)));
    context_->ipc_task_runner()->PostTask(FROM_HERE, base::BindOnce(&Context::OnRemoveFilter, context_, base::RetainedRef(filter)));
}
