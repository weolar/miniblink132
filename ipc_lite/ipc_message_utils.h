...
#ifndef LITE_IPC_IPC_MESSAGE_UTILS_H_
#define LITE_IPC_IPC_MESSAGE_UTILS_H_
...
#include "ipc_lite/base/tuple.h"
#include "ipc_lite/ipc_param_traits.h"
#include "ipc_lite/ipc_sync_message.h"
...
private:
    mutable base::PickleIterator iter_;
};
...
template <class P> static inline void WriteParam(Message* m, const P& p)
{
    typedef typename SimilarTypeTraits<P>::Type Type;
    ParamTraits<Type>::Write(m, static_cast<const Type&>(p));
}

template <class P> static inline bool WARN_UNUSED_RESULT ReadParam(const Message* m, base::PickleIterator* iter, P* p)
{
    typedef typename SimilarTypeTraits<P>::Type Type;
    return ParamTraits<Type>::Read(m, iter, reinterpret_cast<Type*>(p));
}

template <class P> static inline void LogParam(const P& p, std::string* l)
{
    typedef typename SimilarTypeTraits<P>::Type Type;
...
template <> struct ParamTraits<unsigned int> {
...
template <> struct ParamTraits<unsigned long> {
...
template <> struct ParamTraits<long long> {
...
template <> struct ParamTraits<unsigned long long> {
...
template <> struct IPC_EXPORT ParamTraits<unsigned short> {
...
// IPC.
template <> struct IPC_EXPORT ParamTraits<float> {
...
template <> struct IPC_EXPORT ParamTraits<double> {
...
#ifdef _WIN32
template <> struct ParamTraits<std::wstring> {
    typedef std::wstring param_type;
    static void Write(Message* m, const param_type& p)
    {
        //m->WriteWString(p);
        m->WriteString16(std::u16string_view((const char16_t*)(p.c_str()), p.length()));
    }
    static bool Read(const Message* m, base::PickleIterator* iter, param_type* r)
    {
        ...
        if (iter->ReadString16(&result)) {
            *r = std::wstring((const WCHAR*)(result.c_str()), result.length());
            return true;
        }
        return false;
    }
...
// need this trait.
#if 0 // !defined(WCHAR_T_IS_UTF16)
template <> struct ParamTraits<std::u16string> {
...
template <> struct IPC_EXPORT ParamTraits<std::vector<char>> {
...
template <> struct IPC_EXPORT ParamTraits<std::vector<unsigned char>> {
...
template <> struct IPC_EXPORT ParamTraits<std::vector<bool>> {
...
template <class P> struct ParamTraits<std::vector<P>> {
...
template <class P> struct ParamTraits<std::set<P>> {
...
template <class K, class V> struct ParamTraits<std::map<K, V>> {
...
template <class A, class B> struct ParamTraits<std::pair<A, B>> {
...
template <> struct IPC_EXPORT ParamTraits<base::Value::Dict> {
...
template <> struct IPC_EXPORT ParamTraits<base::FileDescriptor> {
...
template <> struct IPC_EXPORT ParamTraits<base::FilePath> {
...
template <> struct IPC_EXPORT ParamTraits<base::Value::List> {
...
template <> struct IPC_EXPORT ParamTraits<NullableString16> {
...
template <> struct IPC_EXPORT ParamTraits<base::Time> {
...
template <> struct IPC_EXPORT ParamTraits<base::TimeDelta> {
...
template <> struct IPC_EXPORT ParamTraits<base::TimeTicks> {
...
template <> struct ParamTraits<Tuple0> {
...
template <class A> struct ParamTraits<Tuple1<A>> {
...
template <class A, class B> struct ParamTraits<Tuple2<A, B>> {
...
template <class A, class B, class C> struct ParamTraits<Tuple3<A, B, C>> {
    ...
    static bool Read(const Message* m, base::PickleIterator* iter, param_type* r)
    {
        return (ReadParam(m, iter, &r->a) && ReadParam(m, iter, &r->b) && ReadParam(m, iter, &r->c));
    }
    static void Log(const param_type& p, std::string* l)
    {
        LogParam(p.a, l);
...
template <class A, class B, class C, class D> struct ParamTraits<Tuple4<A, B, C, D>> {
    ...
    static bool Read(const Message* m, base::PickleIterator* iter, param_type* r)
    {
        return (ReadParam(m, iter, &r->a) && ReadParam(m, iter, &r->b) && ReadParam(m, iter, &r->c) && ReadParam(m, iter, &r->d));
    }
    static void Log(const param_type& p, std::string* l)
    {
        LogParam(p.a, l);
...
template <class A, class B, class C, class D, class E> struct ParamTraits<Tuple5<A, B, C, D, E>> {
    ...
    static bool Read(const Message* m, base::PickleIterator* iter, param_type* r)
    {
        return (ReadParam(m, iter, &r->a) && ReadParam(m, iter, &r->b) && ReadParam(m, iter, &r->c) && ReadParam(m, iter, &r->d) && ReadParam(m, iter, &r->e));
    }
    static void Log(const param_type& p, std::string* l)
    {
        LogParam(p.a, l);
...
template <> struct IPC_EXPORT ParamTraits<IPC::ChannelHandle> {
...
template <> struct IPC_EXPORT ParamTraits<LogData> {
...
template <> struct IPC_EXPORT ParamTraits<Message> {
...
#if defined(OS_WIN)
template <> struct IPC_EXPORT ParamTraits<HANDLE> {
...
template <> struct IPC_EXPORT ParamTraits<LOGFONT> {
...
template <> struct IPC_EXPORT ParamTraits<MSG> {
...
template <class ParamType> class MessageSchema {
public:
...
IPC_EXPORT void GenerateLogData(const std::string& channel, const Message& message, LogData* data, bool get_params);
...
template <class ReplyParamType> inline void LogReplyParamsToMessage(const ReplyParamType& reply_params, const Message* msg)
{
...
template <class RefTuple> class ParamDeserializer : public MessageReplyDeserializer {
public:
    explicit ParamDeserializer(const RefTuple& out)
        : out_(out)
    {
    }

    bool SerializeOutputParameters(const IPC::Message& msg, base::PickleIterator iter)
    {
...
template <class SendParamType, class ReplyParamType> class SyncMessageSchema {
public:
    ...
    static bool ReadReplyParam(const Message* msg, typename TupleTypes<ReplyParam>::ValueTuple* p) IPC_MSG_NOINLINE;

    template <class T, class S, class Method>
    static bool DispatchWithSendParams(bool ok, const SendParam& send_params, const Message* msg, T* obj, S* sender, Method func)
    {
    ...
    static bool DispatchDelayReplyWithSendParams(bool ok, const SendParam& send_params, const Message* msg, T* obj, Method func)
    {
        Message* reply = SyncMessage::GenerateReply(msg);
        if (ok) {
