...
#include "ipc_lite/ipc_sync_channel.h"
...
public:
    ...
    void QueueMessage(const Message& msg, SyncChannel::SyncContext* context)
    {
        ...
        if (!was_task_pending) {
            listener_task_runner_->PostTask(
                FROM_HERE, base::BindOnce(&ReceivedSyncMsgQueue::DispatchMessagesTask, base::RetainedRef(this), scoped_refptr<SyncContext>(context)));
        }
    }
    ...
    // messages.
    void DispatchMessagesTask(scoped_refptr<SyncContext> context)
    {
        {
            base::AutoLock auto_lock(message_lock_);
            task_pending_ = false;
    ...
    static base::LazyInstance<base::ThreadLocalPointer<ReceivedSyncMsgQueue>>::DestructorAtExit lazy_tls_ptr_;
...
private:
    ...
    ReceivedSyncMsgQueue()
        ...
        , dispatch_event_(base::WaitableEvent::ResetPolicy::MANUAL, base::WaitableEvent::InitialState::NOT_SIGNALED) //(true, false)
...
base::LazyInstance<base::ThreadLocalPointer<SyncChannel::ReceivedSyncMsgQueue>>::DestructorAtExit SyncChannel::ReceivedSyncMsgQueue::lazy_tls_ptr_
    = LAZY_INSTANCE_INITIALIZER;

SyncChannel::SyncContext::SyncContext(Listener* listener, scoped_refptr<base::SingleThreadTaskRunner> ipc_task_runner, WaitableEvent* shutdown_event)
...
void SyncChannel::SyncContext::Push(SyncMessage* sync_msg)
{
    ...
    PendingSyncMsg pending(SyncMessage::GetMessageId(*sync_msg), sync_msg->GetReplyDeserializer(),
        new WaitableEvent(base::WaitableEvent::ResetPolicy::MANUAL, base::WaitableEvent::InitialState::NOT_SIGNALED) /*(true, false)*/);
...
bool SyncChannel::SyncContext::Pop()
{
    ...
    ipc_task_runner()->PostTask(FROM_HERE, base::BindOnce(&ReceivedSyncMsgQueue::DispatchReplies, received_sync_msgs_.get()));
...
void SyncChannel::SyncContext::OnChannelOpened()
{
    ...
    if (shutdown_event_) {
        shutdown_watcher_.StartWatching(shutdown_event_, base::BindOnce(&SyncChannel::SyncContext::OnWaitableEventSignaled, base::Unretained(this)),
...
SyncChannel::SyncChannel(const IPC::ChannelHandle& channel_handle, Channel::Mode mode, Listener* listener, base::SingleThreadTaskRunner* ipc_task_runner,
    bool create_pipe_now, WaitableEvent* shutdown_event)
...
SyncChannel::SyncChannel(Listener* listener, base::SingleThreadTaskRunner* ipc_task_runner, WaitableEvent* shutdown_event)
    : ChannelProxy(new SyncContext(listener, ipc_task_runner, shutdown_event))
    , sync_messages_with_no_timeout_allowed_(true)
{
    StartWatching();
...
bool SyncChannel::SendWithTimeout(Message* message, int timeout_ms)
{
    ...
    TRACE_EVENT1("task", "SyncChannel::SendWithTimeout", "name", name);
#else
    TRACE_EVENT2("task", "SyncChannel::SendWithTimeout", "class", IPC_MESSAGE_ID_CLASS(message->type()), "line", IPC_MESSAGE_ID_LINE(message->type()));
#endif
    if (!message->is_sync()) {
    ...
    if (timeout_ms != base::kNoTimeout) {
        ...
        context->ipc_task_runner()->PostDelayedTask(
            FROM_HERE, base::BindOnce(&SyncContext::OnSendTimeout, context.get(), message_id), base::Milliseconds(timeout_ms));
    }
...
void SyncChannel::WaitForReply(SyncContext* context, WaitableEvent* pump_messages_event)
{
    context->DispatchMessages();
    while (true) {
        WaitableEvent* objects[] = { context->GetDispatchEvent(), context->GetSendDoneEvent(), pump_messages_event };
...
void SyncChannel::StartWatching()
{
    ...
    dispatch_watcher_.StartWatching(
        sync_context()->GetDispatchEvent(), base::BindOnce(&SyncChannel::OnWaitableEventSignaled, base::Unretained(this)), sync_context()->ipc_task_runner());
}
