...
public:
    ...
    TestURLRequestInterceptor& operator=(const TestURLRequestInterceptor&) = delete;
...
TEST(URLRequestFilter, BasicMatching)
{
    ...
    std::unique_ptr<URLRequest> request1(context->CreateRequest(kUrl1, DEFAULT_PRIORITY, &delegate, TRAFFIC_ANNOTATION_FOR_TESTS));

    const GURL kUrl2("http://bar.com/");
    std::unique_ptr<URLRequest> request2(context->CreateRequest(kUrl2, DEFAULT_PRIORITY, &delegate, TRAFFIC_ANNOTATION_FOR_TESTS));

    // Check AddUrlInterceptor checks for invalid URLs.
    EXPECT_FALSE(filter->AddUrlInterceptor(GURL(), std::make_unique<TestURLRequestInterceptor>()));
    ...
    {
        std::unique_ptr<URLRequestJob> found = filter->MaybeInterceptRequest(request1.get());
    ...
    filter->AddHostnameInterceptor(kUrl1.scheme(), kUrl1.host(), std::move(interceptor2));
    {
        std::unique_ptr<URLRequestJob> found = filter->MaybeInterceptRequest(request1.get());
