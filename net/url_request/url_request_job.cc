...
public:
    ...
    URLRequestJobSourceStream& operator=(const URLRequestJobSourceStream&) = delete;
    ...
    {
        DCHECK(job_);
        return job_->ReadRawDataHelper(dest_buffer, buffer_size, std::move(callback));
    }
...
int URLRequestJob::Read(IOBuffer* buf, int buf_size)
{
    ...
    int result = source_stream_->Read(buf, buf_size, base::BindOnce(&URLRequestJob::SourceStreamReadComplete, weak_factory_.GetWeakPtr(), false));
...
bool URLRequestJob::IsRedirectResponse(GURL* location, int* http_status_code, bool* insecure_scheme_was_upgraded)
{
...
void URLRequestJob::ContinueWithCertificate(scoped_refptr<X509Certificate> client_cert, scoped_refptr<SSLPrivateKey> client_private_key)
{
...
// static
GURL URLRequestJob::ComputeReferrerForPolicy(ReferrerPolicy policy, const GURL& original_referrer, const GURL& destination, bool* same_origin_out_for_metrics)
{
    ...
    if (base::FeatureList::IsEnabled(features::kCapReferrerToOriginOnCrossOrigin) && !same_origin) {
        should_strip_to_origin = true;
    }

    bool secure_referrer_but_insecure_destination = original_referrer.SchemeIsCryptographic() && !destination.SchemeIsCryptographic();
...
int URLRequestJob::NotifyConnected(const TransportInfo& info, CompletionOnceCallback callback)
{
    return request_->NotifyConnected(info, std::move(callback));
}

void URLRequestJob::NotifyCertificateRequested(SSLCertRequestInfo* cert_request_info)
{
    request_->NotifyCertificateRequested(cert_request_info);
}

void URLRequestJob::NotifySSLCertificateError(int net_error, const SSLInfo& ssl_info, bool fatal)
{
    request_->NotifySSLCertificateError(net_error, ssl_info, fatal);
}

bool URLRequestJob::CanSetCookie(const net::CanonicalCookie& cookie, CookieOptions* options, const net::FirstPartySetMetadata& first_party_set_metadata,
    CookieInclusionStatus* inclusion_status) const
{
    return request_->CanSetCookie(cookie, options, first_party_set_metadata, inclusion_status);
}
...
void URLRequestJob::NotifyHeadersComplete()
{
    ...
    request_->response_info_.response_time = request_->response_info_.original_response_time = base::Time::Now();
    ...
    if (NeedsAuth()) {
        CHECK(!IsRedirectResponse(&new_location, &http_status_code, &insecure_scheme_was_upgraded));
    ...
    if (IsRedirectResponse(&new_location, &http_status_code, &insecure_scheme_was_upgraded)) {
        CHECK(!NeedsAuth());
        ...
        RedirectInfo redirect_info = RedirectInfo::ComputeRedirectInfo(request_->method(), request_->url(), request_->site_for_cookies(),
            request_->first_party_url_policy(), request_->referrer_policy(), request_->referrer(), http_status_code, new_location,
            net::RedirectUtil::GetReferrerPolicyHeader(request_->response_headers()), insecure_scheme_was_upgraded, CopyFragmentOnRedirect(new_location));
    ...
    if (request_->status() == OK) {
        ...
        if (source_stream_->type() == SourceStream::TYPE_NONE) {
            ...
            if (expected_content_size_ == -1 && request_->response_headers()) {
                ...
                expected_content_size_ = request_->response_headers()->GetContentLength();
            }
        } else {
            request_->net_log().AddEvent(NetLogEventType::URL_REQUEST_FILTERS_SET, [&] { return SourceStreamSetParams(source_stream_.get()); });
        }
    }
...
void URLRequestJob::OnDone(int net_error, bool notify_done)
{
    ...
    if (!request_->failed()) {
        if (net_error != net::OK && net_error != ERR_ABORTED) {
            request_->net_log().AddEventWithNetErrorCode(NetLogEventType::FAILED, net_error);
        }
    ...
    if (notify_done) {
        ...
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(&URLRequestJob::NotifyDone, weak_factory_.GetWeakPtr()));
    }
}
...
void URLRequestJob::SourceStreamReadComplete(bool synchronous, int result)
{
    ...
    if (result > 0 && request()->net_log().IsCapturing()) {
        request()->net_log().AddByteTransferEvent(NetLogEventType::URL_REQUEST_JOB_FILTERED_BYTES_READ, result, pending_read_buffer_->data());
    }
...
int URLRequestJob::ReadRawDataHelper(IOBuffer* buf, int buf_size, CompletionOnceCallback callback)
{
...
void URLRequestJob::GatherRawReadStats(int bytes_read)
{
    ...
    if (bytes_read > 0) {
        ...
        if (source_stream_->type() != SourceStream::TYPE_NONE && request()->net_log().IsCapturing()) {
            request()->net_log().AddByteTransferEvent(NetLogEventType::URL_REQUEST_JOB_BYTES_READ, bytes_read, raw_read_buffer_->data());
        }
...
void URLRequestJob::RecordBytesRead(int bytes_read)
{
    ...
    if (request_->context()->network_quality_estimator()) {
        if (prefilter_bytes_read() == bytes_read) {
            request_->context()->network_quality_estimator()->NotifyHeadersReceived(*request_, prefilter_bytes_read());
        } else {
            request_->context()->network_quality_estimator()->NotifyBytesRead(*request_, prefilter_bytes_read());
        }
    }

    DVLOG(2) << __FUNCTION__ << "() "
             << "\"" << request_->url().spec() << "\""
             << " pre bytes read = " << bytes_read << " pre total = " << prefilter_bytes_read() << " post total = " << postfilter_bytes_read();
}
