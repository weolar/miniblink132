...
base::Value NetLogRejectedRequestParams(const std::string* url_id, int num_failures, const base::TimeDelta& release_after)
{
    base::Value::Dict dict;
    dict.Set("url", *url_id);
    dict.Set("num_failures", num_failures);
    dict.Set("release_after_ms", static_cast<int>(release_after.InMilliseconds()));
    return base::Value(std::move(dict));
}

URLRequestThrottlerEntry::URLRequestThrottlerEntry(URLRequestThrottlerManager* manager, const std::string& url_id)
    : sliding_window_period_(base::Milliseconds(kDefaultSlidingWindowPeriodMs))
    , max_send_threshold_(kDefaultMaxSendThreshold)
    , backoff_entry_(&backoff_policy_)
    , manager_(manager)
    , url_id_(url_id)
    , net_log_(NetLogWithSource::Make(manager->net_log(), NetLogSourceType::EXPONENTIAL_BACKOFF_THROTTLING))
{
    DCHECK(manager_);
    Initialize();
}

URLRequestThrottlerEntry::URLRequestThrottlerEntry(URLRequestThrottlerManager* manager, const std::string& url_id, int sliding_window_period_ms,
    int max_send_threshold, int initial_backoff_ms, double multiply_factor, double jitter_factor, int maximum_backoff_ms)
...
bool URLRequestThrottlerEntry::ShouldRejectRequest(const URLRequest& request) const
{
    ...
    if (!is_backoff_disabled_ && GetBackoffEntry()->ShouldRejectRequest()) {
        net_log_.AddEvent(NetLogEventType::THROTTLING_REJECTED_REQUEST,
            [&] { return NetLogRejectedRequestParams(&url_id_, GetBackoffEntry()->failure_count(), GetBackoffEntry()->GetTimeUntilRelease()); });
        reject_request = true;
    }
...
int64_t URLRequestThrottlerEntry::ReserveSendingTimeForNextRequest(const base::TimeTicks& earliest_time)
{
    ...
    base::TimeTicks recommended_sending_time
        = std::max(std::max(now, earliest_time), std::max(GetBackoffEntry()->GetReleaseTime(), sliding_window_release_time_));
...
base::TimeTicks URLRequestThrottlerEntry::GetExponentialBackoffReleaseTime() const
{
