...
base::LazyInstance<URLRequestJobList>::Leaky g_pending_jobs = LAZY_INSTANCE_INITIALIZER;
...
URLRequestTestJob::URLRequestTestJob(URLRequest* request, const std::string& response_headers, const std::string& response_data, bool auto_advance)
    : URLRequestJob(request)
    , auto_advance_(auto_advance)
    , response_data_(response_data)
    , response_headers_(base::MakeRefCounted<net::HttpResponseHeaders>(net::HttpUtil::AssembleRawHeaders(response_headers)))
...
void URLRequestTestJob::Start()
{
    ...
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(&URLRequestTestJob::StartAsync, weak_factory_.GetWeakPtr()));
}
...
void URLRequestTestJob::SetResponseHeaders(const std::string& response_headers)
{
    response_headers_ = base::MakeRefCounted<HttpResponseHeaders>(net::HttpUtil::AssembleRawHeaders(response_headers));
    response_headers_length_ = response_headers.size();
}
    ...
    if (stage_ == WAITING || async_reads_) {
        ...
        if (stage_ != WAITING) {
            ...
            base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
                FROM_HERE, base::BindOnce(&URLRequestTestJob::ProcessNextOperation, weak_factory_.GetWeakPtr()));
        }
...
void URLRequestTestJob::GetLoadTimingInfo(LoadTimingInfo* load_timing_info) const
{
...
bool URLRequestTestJob::IsRedirectResponse(GURL* location, int* http_status_code, bool* insecure_scheme_was_upgraded)
{
    if (!response_headers_.get())
        return false;
