...
public:
    ...
    explicit TestURLRequestContextGetter(const scoped_refptr<base::SingleThreadTaskRunner>& network_task_runner);

    // Use to pass a pre-initialized |context|.
    TestURLRequestContextGetter(const scoped_refptr<base::SingleThreadTaskRunner>& network_task_runner, std::unique_ptr<URLRequestContext> context);
...
public:
    ...
    void set_on_connected_result(int result)
    {
        on_connected_result_ = result;
    }

    void set_cancel_in_received_redirect(bool val)
    {
        cancel_in_rr_ = val;
    }
    void set_cancel_in_response_started(bool val)
    {
        cancel_in_rs_ = val;
    }
    void set_cancel_in_received_data(bool val)
    {
        cancel_in_rd_ = val;
    }
    ...
    const std::vector<TransportInfo>& transports() const
    {
        return transports_;
    }
    ...
    {
        return static_cast<int>(data_received_.size());
    }
    int response_started_count() const
    {
        return response_started_count_;
    }
    int received_bytes_count() const
    {
        return received_bytes_count_;
    }
    int received_redirect_count() const
    {
        return received_redirect_count_;
    }
    ...
    bool request_failed() const
    {
        return request_failed_;
    }
    bool have_certificate_errors() const
    {
        return have_certificate_errors_;
    }
    bool certificate_errors_are_fatal() const
    {
        return certificate_errors_are_fatal_;
    }
    int certificate_net_error() const
    {
        return certificate_net_error_;
    }
    bool auth_required_called() const
    {
        return auth_required_;
    }
    bool response_completed() const
    {
        return response_completed_;
    }
...
public:
    ...
    bool GetLoadTimingInfoBeforeRedirect(LoadTimingInfo* load_timing_info_before_redirect) const;

    // Will redirect once to the given URL when the next set of headers are
    // received.
    void set_redirect_on_headers_received_url(GURL redirect_on_headers_received_url)
    {
    ...
    void set_preserve_fragment_on_redirect_url(const std::optional<GURL>& preserve_fragment_on_redirect_url)
    {
        preserve_fragment_on_redirect_url_ = preserve_fragment_on_redirect_url;
    }

    void set_cookie_options(int o)
    {
        cookie_options_bit_mask_ = o;
    }
    ...
    int destroyed_requests() const
    {
        return destroyed_requests_;
    }
    int completed_requests() const
    {
        return completed_requests_;
    }
    int canceled_requests() const
    {
        return canceled_requests_;
    }
    int blocked_annotate_cookies_count() const
    {
        return blocked_annotate_cookies_count_;
    }
    int blocked_set_cookie_count() const
    {
        return blocked_set_cookie_count_;
    }
    int set_cookie_count() const
    {
        return set_cookie_count_;
    }
    ...
    int headers_received_count() const
    {
        return headers_received_count_;
    }
    ...
    const std::vector<CookieSettingOverrides>& cookie_setting_overrides_records() const
    {
        return cookie_setting_overrides_records_;
    }

    void set_storage_access_status(std::optional<cookie_util::StorageAccessStatus> status)
    {
...
protected:
    ...
    bool OnAnnotateAndMoveUserBlockedCookies(const URLRequest& request, const net::FirstPartySetMetadata& first_party_set_metadata,
        net::CookieAccessResultList& maybe_included_cookies, net::CookieAccessResultList& excluded_cookies) override;
    NetworkDelegate::PrivacySetting OnForcePrivacyMode(const URLRequest& request) const override;
    bool OnCanSetCookie(const URLRequest& request, const net::CanonicalCookie& cookie, CookieOptions* options,
        const net::FirstPartySetMetadata& first_party_set_metadata, CookieInclusionStatus* inclusion_status) override;
    bool OnCancelURLRequestWithPolicyViolatingReferrerHeader(const URLRequest& request, const GURL& target_url, const GURL& referrer_url) const override;
    std::optional<cookie_util::StorageAccessStatus> OnGetStorageAccessStatus(
        const URLRequest& request, base::optional_ref<const RedirectInfo> redirect_info) const override;
    bool OnIsStorageAccessHeaderEnabled(const url::Origin* top_frame_origin, const GURL& url) const override;
    ...
    bool cancel_request_with_policy_violating_referrer_ = false; // false by default
    ...
    std::optional<cookie_util::StorageAccessStatus> storage_access_status_ = std::nullopt;
...
public:
    ...
    int set_cookie_called_count()
    {
        return set_cookie_called_count_;
    }

    int blocked_set_cookie_count()
    {
        return blocked_set_cookie_count_;
    }

    void ResetSetCookieCalledCount()
    {
        set_cookie_called_count_ = 0;
    }

    void ResetBlockedSetCookieCount()
    {
    ...
    void set_block_annotate_cookies()
    {
        block_annotate_cookies_ = true;
    }

    void unset_block_annotate_cookies()
    {
        block_annotate_cookies_ = false;
    }
    ...
    void ResetAnnotateCookiesCalledCount()
    {
        annotate_cookies_called_count_ = 0;
    }
    ...
    void set_force_privacy_mode(bool enabled)
    {
        force_privacy_mode_ = enabled;
    }
...
public:
    ...
    TestScopedURLInterceptor(const GURL& url, std::unique_ptr<URLRequestJob> intercept_job);
