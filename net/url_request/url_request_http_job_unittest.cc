...
const char kSimpleHeadMockWrite[] = "HEAD / HTTP/1.1\r\n"
                                    "Host: www.example.com\r\n"
                                    "Connection: keep-alive\r\n"
                                    "User-Agent: \r\n"
                                    "Accept-Encoding: gzip, deflate\r\n"
...
const char kTrustAnchorRequestHistogram[] = "Net.Certificate.TrustAnchor.Request";
...
public:
    explicit TestURLRequestHttpJob(URLRequest* request)
        : URLRequestHttpJob(request, request->context()->http_user_agent_settings())
    {
    }
...
TEST_F(URLRequestHttpJobSetUpSourceTest, SetUpSourceFails)
{
    ...
    std::unique_ptr<URLRequest> request = context_->CreateRequest(GURL("http://www.example.com"), DEFAULT_PRIORITY, &delegate_, TRAFFIC_ANNOTATION_FOR_TESTS);
...
public:
    explicit URLRequestHttpJobWithProxy(std::unique_ptr<ProxyResolutionService> proxy_resolution_service)
    {
        auto context_builder = CreateTestURLRequestContextBuilder();
        context_builder->set_client_socket_factory_for_testing(&socket_factory_);
        if (proxy_resolution_service) {
            context_builder->set_proxy_resolution_service(std::move(proxy_resolution_service));
        }
        context_ = context_builder->Build();
    }

    URLRequestHttpJobWithProxy(const URLRequestHttpJobWithProxy&) = delete;
    URLRequestHttpJobWithProxy& operator=(const URLRequestHttpJobWithProxy&) = delete;
...
TEST_F(URLRequestHttpJobWithProxyTest, TestFailureWithoutProxy)
{
    ...
    std::unique_ptr<URLRequest> request
        = http_job_with_proxy.context_->CreateRequest(GURL("http://www.example.com"), DEFAULT_PRIORITY, &delegate, TRAFFIC_ANNOTATION_FOR_TESTS);
...
{
    const char kSimpleProxyGetMockWrite[] = "GET http://www.example.com/ HTTP/1.1\r\n"
                                            "Host: www.example.com\r\n"
    ...
    const ProxyChain proxy_chain = ProxyUriToProxyChain("http://origin.net:80", ProxyServer::SCHEME_HTTP);

    std::unique_ptr<ProxyResolutionService> proxy_resolution_service
        = ConfiguredProxyResolutionService::CreateFixedFromPacResultForTest(ProxyServerToPacResultElement(proxy_chain.First()), TRAFFIC_ANNOTATION_FOR_TESTS);
    ...
    URLRequestHttpJobWithProxy http_job_with_proxy(std::move(proxy_resolution_service));
    http_job_with_proxy.socket_factory_.AddSocketDataProvider(&socket_data);
...
TEST_F(URLRequestHttpJobWithProxyTest, TestContentLengthSuccessfulRequestWithTwoProxies)
{
    ...
    // DIRECT.
    std::unique_ptr<ProxyResolutionService> proxy_resolution_service = ConfiguredProxyResolutionService::CreateFixedFromPacResultForTest(
        ProxyServerToPacResultElement(proxy_chain.First()) + "; DIRECT", TRAFFIC_ANNOTATION_FOR_TESTS);
...
TEST_F(URLRequestHttpJobWithProxyTest, IpProtectionDirectOnlyProxyMetricsRecorded)
{
    base::test::ScopedFeatureList scoped_feature_list;
    scoped_feature_list.InitAndEnableFeatureWithParameters(net::features::kEnableIpProtectionProxy, { { net::features::kIpPrivacyDirectOnly.name, "true" } });
    const auto kIpProtectionDirectChain = ProxyChain::ForIpProtection(std::vector<ProxyServer>());

    std::unique_ptr<ProxyResolutionService> proxy_resolution_service
        = ConfiguredProxyResolutionService::CreateFixedForTest("https://not-used:70", TRAFFIC_ANNOTATION_FOR_TESTS);
    ...
    histogram_tester.ExpectUniqueSample("Net.HttpJob.IpProtection.BytesSent", std::size(kSimpleGetMockWrite),
        /*expected_bucket_count=*/1);

    histogram_tester.ExpectUniqueSample("Net.HttpJob.IpProtection.PrefilterBytesRead.Net",
        /*sample=*/12, /*expected_bucket_count=*/1);

    histogram_tester.ExpectUniqueSample("Net.HttpJob.IpProtection.JobResult",
...
{
    base::test::ScopedFeatureList scoped_feature_list;
    scoped_feature_list.InitAndEnableFeatureWithParameters(net::features::kEnableIpProtectionProxy, { { net::features::kIpPrivacyDirectOnly.name, "false" } });
    const auto kIpProtectionDirectChain = ProxyChain::ForIpProtection(std::vector<ProxyServer>());
    ...
    histogram_tester.ExpectTotalCount("Net.HttpJob.IpProtection.PrefilterBytesRead.Net", 0);
}
...
protected:
    ...
    {
        auto context_builder = CreateTestURLRequestContextBuilder();
        context_builder->SetHttpTransactionFactoryForTesting(std::make_unique<MockNetworkLayer>());
        context_builder->DisableHttpCache();
        context_builder->set_net_log(NetLog::Get());
        context_ = context_builder->Build();

        req_ = context_->CreateRequest(GURL("http://www.example.com"), DEFAULT_PRIORITY, &delegate_, TRAFFIC_ANNOTATION_FOR_TESTS);
    }

    MockNetworkLayer& network_layer()
    {
        // This cast is safe because we set a MockNetworkLayer in the constructor.
        return *static_cast<MockNetworkLayer*>(context_->http_transaction_factory());
    }
...
TEST_F(URLRequestHttpJobWithMockSocketsTest, TestContentLengthSuccessfulRequest)
{
    MockWrite writes[] = { MockWrite(kSimpleGetMockWrite) };
    MockRead reads[] = { MockRead("HTTP/1.1 200 OK\r\n"
                                  "Content-Length: 12\r\n\r\n"),
    ...
    TestDelegate delegate;
    std::unique_ptr<URLRequest> request = context_->CreateRequest(GURL("http://www.example.com"), DEFAULT_PRIORITY, &delegate, TRAFFIC_ANNOTATION_FOR_TESTS);
...
{
    const url::Origin kOrigin1 = url::Origin::Create(GURL("http://www.example.com"));
    const IsolationInfo kTestIsolationInfo = IsolationInfo::CreateForInternalRequest(kOrigin1);
    ...
    // Cache the response.
    {
        ...
        TestDelegate delegate;
        std::unique_ptr<URLRequest> request
            = context_->CreateRequest(GURL("http://www.example.com"), DEFAULT_PRIORITY, &delegate, TRAFFIC_ANNOTATION_FOR_TESTS);
...
TEST_F(URLRequestHttpJobWithMockSocketsTest, TestContentLengthSuccessfulHttp09Request)
{
    MockWrite writes[] = { MockWrite(kSimpleGetMockWrite) };
    MockRead reads[] = { MockRead("Test Content"), MockRead(net::SYNCHRONOUS, net::OK) };
...
TEST_F(URLRequestHttpJobWithMockSocketsTest, TestContentLengthFailedRequest)
{
    ...
    MockRead reads[] = { MockRead("HTTP/1.1 200 OK\r\n"
                                  "Content-Length: 20\r\n\r\n"),
        MockRead("Test Content"), MockRead(net::SYNCHRONOUS, net::ERR_FAILED) };
...
TEST_F(URLRequestHttpJobWithMockSocketsTest, TestContentLengthCancelledRequest)
{
    MockWrite writes[] = { MockWrite(kSimpleGetMockWrite) };
    MockRead reads[] = { MockRead("HTTP/1.1 200 OK\r\n"
                                  "Content-Length: 20\r\n\r\n"),
        MockRead("Test Content"), MockRead(net::SYNCHRONOUS, net::ERR_IO_PENDING) };
...
TEST_F(URLRequestHttpJobWithMockSocketsTest, TestNetworkBytesRedirectedRequest)
{
    ...
    StaticSocketDataProvider redirect_socket_data(redirect_reads, redirect_writes);
    ...
    TestDelegate delegate;
    std::unique_ptr<URLRequest> request = context_->CreateRequest(GURL("http://www.redirect.com"), DEFAULT_PRIORITY, &delegate, TRAFFIC_ANNOTATION_FOR_TESTS);
...
TEST_F(URLRequestHttpJobWithMockSocketsTest, TestNetworkBytesCancelledAfterHeaders)
{
    MockWrite writes[] = { MockWrite(kSimpleGetMockWrite) };
    MockRead reads[] = { MockRead("HTTP/1.1 200 OK\r\n\r\n") };
    StaticSocketDataProvider socket_data(reads, writes);
...
TEST_F(URLRequestHttpJobWithMockSocketsTest, TestNetworkBytesCancelledImmediately)
{
...
TEST_F(URLRequestHttpJobWithMockSocketsTest, TestHttpTimeToFirstByteForCancelledTask)
{
    base::HistogramTester histograms;
    MockWrite writes[] = { MockWrite(kSimpleGetMockWrite) };
    MockRead reads[] = { MockRead("HTTP/1.1 200 OK\r\n"
...
TEST_F(URLRequestHttpJobWithMockSocketsTest, TestHttpJobSuccessPriorityKeyedTotalTime)
{
    base::HistogramTester histograms;
    ...
    for (int priority = 0; priority < net::NUM_PRIORITIES; ++priority) {
        for (int request_index = 0; request_index <= priority; ++request_index) {
            ...
            std::unique_ptr<URLRequest> request = context_->CreateRequest(
                GURL("http://www.example.com/"), static_cast<net::RequestPriority>(priority), &delegate, TRAFFIC_ANNOTATION_FOR_TESTS);
    ...
    for (int priority = 0; priority < net::NUM_PRIORITIES; ++priority) {
        histograms.ExpectTotalCount("Net.HttpJob.TotalTimeSuccess.Priority" + base::NumberToString(priority), priority + 1);
    }
}

TEST_F(URLRequestHttpJobWithMockSocketsTest, TestHttpJobRecordsTrustAnchorHistograms)
{
    SSLSocketDataProvider ssl_socket_data(net::ASYNC, net::OK);
    ssl_socket_data.ssl_info.cert = ImportCertFromFile(GetTestCertsDirectory(), "ok_cert.pem");
    ...
    ssl_socket_data.ssl_info.public_key_hashes.push_back(HashValue(intermediate_hash));
    ...
    TestDelegate delegate;
    std::unique_ptr<URLRequest> request = context_->CreateRequest(GURL("https://www.example.com/"), DEFAULT_PRIORITY, &delegate, TRAFFIC_ANNOTATION_FOR_TESTS);
    request->Start();
    delegate.RunUntilComplete();
    EXPECT_THAT(delegate.request_status(), IsOk());

    histograms.ExpectTotalCount(kTrustAnchorRequestHistogram, 1);
    histograms.ExpectUniqueSample(kTrustAnchorRequestHistogram, kGTSRootR4HistogramID, 1);
}

TEST_F(URLRequestHttpJobWithMockSocketsTest, TestHttpJobDoesNotRecordTrustAnchorHistogramsWhenNoNetworkLoad)
{
    SSLSocketDataProvider ssl_socket_data(net::ASYNC, net::OK);
    ssl_socket_data.ssl_info.cert = ImportCertFromFile(GetTestCertsDirectory(), "ok_cert.pem");
...
TEST_F(URLRequestHttpJobWithMockSocketsTest, TestHttpJobRecordsMostSpecificTrustAnchorHistograms)
{
    SSLSocketDataProvider ssl_socket_data(net::ASYNC, net::OK);
    ssl_socket_data.ssl_info.cert = ImportCertFromFile(GetTestCertsDirectory(), "ok_cert.pem");
    // Simulate a certificate chain issued by "C=US, O=Google Trust Services LLC,
    ...
    ssl_socket_data.ssl_info.public_key_hashes.push_back(HashValue(leaf_hash));
    ssl_socket_data.ssl_info.public_key_hashes.push_back(HashValue(intermediate_hash));
    ssl_socket_data.ssl_info.public_key_hashes.push_back(HashValue(gts_root_r3_hash));
    ssl_socket_data.ssl_info.public_key_hashes.push_back(HashValue(gts_root_r4_hash));
    ...
    histograms.ExpectTotalCount(kTrustAnchorRequestHistogram, 1);
    histograms.ExpectUniqueSample(kTrustAnchorRequestHistogram, kGTSRootR3HistogramID, 1);
}
...
TEST_F(URLRequestHttpJobTest, TestCancelWhileReadingCookies)
{
    ...
    TestDelegate delegate;
    std::unique_ptr<URLRequest> request = context->CreateRequest(GURL("http://www.example.com"), DEFAULT_PRIORITY, &delegate, TRAFFIC_ANNOTATION_FOR_TESTS);
...
{
    TestScopedURLInterceptor interceptor(req_->url(), std::make_unique<TestURLRequestHttpJob>(req_.get()));
...
{
    // Setup HSTS state.
    context_->transport_security_state()->AddHSTS("upgrade.test", base::Time::Now() + base::Seconds(10), true);
    ASSERT_TRUE(context_->transport_security_state()->ShouldUpgradeToSSL("upgrade.test"));
    ASSERT_FALSE(context_->transport_security_state()->ShouldUpgradeToSSL("no-upgrade.test"));
    ...
    for (const auto& test : cases) {
        ...
        std::unique_ptr<URLRequest> r(context_->CreateRequest(url, DEFAULT_PRIORITY, &d, TRAFFIC_ANNOTATION_FOR_TESTS, is_for_websockets));
        ...
        if (test.upgrade_expected) {
            auto entries = net_log_observer_.GetEntriesWithType(net::NetLogEventType::URL_REQUEST_REDIRECT_JOB);
...
{
    // Setup HSTS state.
    context_->transport_security_state()->AddHSTS("upgrade.test", base::Time::Now() + base::Seconds(30), true);
    ASSERT_TRUE(context_->transport_security_state()->ShouldUpgradeToSSL("upgrade.test"));
    ...
    struct TestCase {
        ...
        = { { "http://upgrade.test/example.crl", true, "http://upgrade.test/example.crl" },
              // This test ensures that the HSTS check and upgrade happens prior to cache
              // and socket pool checks
              { "http://upgrade.test/example.crl", false, "https://upgrade.test/example.crl" },
...
class URLRequestHttpJobWithMockSocketsDeviceBoundSessionServiceTest : public TestWithTaskEnvironment {
protected:
    URLRequestHttpJobWithMockSocketsDeviceBoundSessionServiceTest()
    {
        auto context_builder = CreateTestURLRequestContextBuilder();
        context_builder->set_client_socket_factory_for_testing(&socket_factory_);
        context_builder->set_device_bound_session_service(std::make_unique<testing::StrictMock<device_bound_sessions::SessionServiceMock>>());
        context_ = context_builder->Build();
        request_ = context_->CreateRequest(GURL("http://www.example.com"), DEFAULT_PRIORITY, &delegate_, TRAFFIC_ANNOTATION_FOR_TESTS);
    }

    device_bound_sessions::SessionServiceMock& GetMockService()
    {
        return *static_cast<device_bound_sessions::SessionServiceMock*>(context_->device_bound_session_service());
    }
...
TEST_F(URLRequestHttpJobWithMockSocketsDeviceBoundSessionServiceTest, ShouldRespondToDeviceBoundSessionHeader)
{
    const MockWrite writes[] = { MockWrite("GET / HTTP/1.1\r\n"
                                           "Host: www.example.com\r\n"
                                           "Connection: keep-alive\r\n"
                                           "User-Agent: \r\n"
                                           "Accept-Encoding: gzip, deflate\r\n"
    ...
    EXPECT_CALL(GetMockService(), GetAnySessionRequiringDeferral).WillOnce(Return(std::nullopt));
    request_->Start();
...
TEST_F(URLRequestHttpJobWithMockSocketsDeviceBoundSessionServiceTest, DeferRequestIfNeeded)
{
    const MockWrite writes[] = { MockWrite("GET / HTTP/1.1\r\n"
                                           "Host: www.example.com\r\n"
                                           "Connection: keep-alive\r\n"
    ...
    {
        InSequence s;
        EXPECT_CALL(GetMockService(), GetAnySessionRequiringDeferral).WillOnce(Invoke([](Unused) {
            std::optional<device_bound_sessions::Session::Id> tag("test");
            return tag;
        }));
        EXPECT_CALL(GetMockService(), DeferRequestForRefresh).WillOnce(base::test::RunOnceClosure<3>());
    }
...
TEST_F(URLRequestHttpJobWithMockSocketsDeviceBoundSessionServiceTest, DontDeferRequestIfNotNeeded)
{
    const MockWrite writes[] = { MockWrite("GET / HTTP/1.1\r\n"
                                           "Host: www.example.com\r\n"
                                           "Connection: keep-alive\r\n"
    ...
    EXPECT_CALL(GetMockService(), GetAnySessionRequiringDeferral).WillOnce(Invoke([](Unused) { return std::nullopt; }));
    request_->Start();
    delegate_.RunUntilComplete();
    EXPECT_THAT(delegate_.request_status(), IsOk());
}

TEST_F(URLRequestHttpJobWithMockSocketsDeviceBoundSessionServiceTest, ShouldNotRespondWithoutDeviceBoundSessionHeader)
{
    const MockWrite writes[] = { MockWrite("GET / HTTP/1.1\r\n"
                                           "Host: www.example.com\r\n"
                                           "Connection: keep-alive\r\n"
...
TEST_F(URLRequestHttpJobWithMockSocketsDeviceBoundSessionServiceTest, ShouldProcessDeviceBoundSessionChallengeHeader)
{
    const MockWrite writes[] = { MockWrite("GET / HTTP/1.1\r\n"
                                           "Host: www.example.com\r\n"
                                           "Connection: keep-alive\r\n"
...
namespace {
std::unique_ptr<test_server::HttpResponse> HandleRequest(const std::string_view& content, const test_server::HttpRequest& request)
{
...
TEST_F(URLRequestHttpJobTest, ShouldBypassHSTSResponseAndConnectionNotReused)
{
    ...
    context->transport_security_state()->AddHSTS("127.0.0.1", base::Time::Now() + base::Seconds(30), true);
    ASSERT_TRUE(context->transport_security_state()->ShouldUpgradeToSSL("127.0.0.1"));
    ...
    // and ensure that the response is stored in the cache.
    {
        ...
        http_server.RegisterRequestHandler(base::BindRepeating(&HandleRequest, kInsecureContent));
        ...
        net_log_observer_.Clear();
        TestDelegate delegate;
        std::unique_ptr<URLRequest> req(context->CreateRequest(insecure_url, DEFAULT_PRIORITY, &delegate, TRAFFIC_ANNOTATION_FOR_TESTS));
        ...
        EXPECT_EQ(2u, net_log_observer_.GetEntriesWithType(net::NetLogEventType::HTTP_CACHE_ADD_TO_ENTRY).size());
    ...
    // re-use an existing socket connection.
    {
        ...
        https_server.RegisterRequestHandler(base::BindRepeating(&HandleRequest, kSecureContent));
...
TEST_F(URLRequestHttpJobTest, HSTSInternalRedirectCallback)
{
    ...
    context->transport_security_state()->AddHSTS("127.0.0.1", base::Time::Now() + base::Seconds(10), true);
    ASSERT_TRUE(context->transport_security_state()->ShouldUpgradeToSSL("127.0.0.1"));
    ...
    {
        GURL url(https_test.GetURL("/echoheader").ReplaceComponents(replace_scheme));
        TestDelegate delegate;
        ...
        std::unique_ptr<URLRequest> r(context->CreateRequest(url, DEFAULT_PRIORITY, &delegate, TRAFFIC_ANNOTATION_FOR_TESTS));
    ...
    {
        GURL url(https_test.GetURL("/echoheader?foo=bar").ReplaceComponents(replace_scheme));
        TestDelegate delegate;
    ...
    {
        GURL url(https_test.GetURL("/echoheader#foo").ReplaceComponents(replace_scheme));
        TestDelegate delegate;
...
TEST_F(URLRequestHttpJobWithBrotliSupportTest, BrotliAdvertisement)
{
    ...
    ssl_socket_data_provider.ssl_info.cert = ImportCertFromFile(GetTestCertsDirectory(), "unittest.selfsigned.der");
...
TEST_F(URLRequestHttpJobWithBrotliSupportTest, DefaultAcceptEncodingOverriden)
{
    struct {
        ...
        { { net::SourceStream::SourceType::TYPE_GZIP },
            ...
            "Accept-Language: en-us,fr\r\n\r\n" },
        { { net::SourceStream::SourceType::TYPE_GZIP, net::SourceStream::SourceType::TYPE_DEFLATE },
            "GET / HTTP/1.1\r\n"
            "Host: www.example.com\r\n"
            "Connection: keep-alive\r\n"
            "User-Agent: \r\n"
        ...
        { { net::SourceStream::SourceType::TYPE_BROTLI },
            ...
            "Accept-Encoding: br\r\n"
            "Accept-Language: en-us,fr\r\n\r\n" },
        { { net::SourceStream::SourceType::TYPE_BROTLI, net::SourceStream::SourceType::TYPE_GZIP, net::SourceStream::SourceType::TYPE_DEFLATE },
            "GET / HTTP/1.1\r\n"
            "Host: www.example.com\r\n"
            "Connection: keep-alive\r\n"
            "User-Agent: \r\n"
    ...
    for (auto test : kTestCases) {
        ...
        TestDelegate delegate;
        std::unique_ptr<URLRequest> request
            = context_->CreateRequest(GURL("https://www.example.com"), DEFAULT_PRIORITY, &delegate, TRAFFIC_ANNOTATION_FOR_TESTS);
...
#if BUILDFLAG(IS_ANDROID)
class URLRequestHttpJobWithCheckClearTextPermittedTest : public TestWithTaskEnvironment {
protected:
...
TEST_F(URLRequestHttpJobWithCheckClearTextPermittedTest, AndroidCleartextPermittedTest)
{
    ...
    for (const TestCase& test : kTestCases) {
        Java_AndroidNetworkLibraryTestUtil_setUpSecurityPolicyForTesting(env, test.cleartext_permitted);

        TestDelegate delegate;
        std::unique_ptr<URLRequest> request = context_->CreateRequest(GURL(test.url), DEFAULT_PRIORITY, &delegate, TRAFFIC_ANNOTATION_FOR_TESTS);
        request->Start();
        delegate.RunUntilComplete();

        if (test.should_block) {
            EXPECT_THAT(delegate.request_status(), IsError(ERR_CLEARTEXT_NOT_PERMITTED));
        } else {
            // Should fail since there's no test server running
            EXPECT_THAT(delegate.request_status(), IsError(ERR_FAILED));
        }
        EXPECT_EQ(Java_AndroidNetworkLibraryTestUtil_getPerHostCleartextCheckCount(env), test.expected_per_host_call_count);
        EXPECT_EQ(Java_AndroidNetworkLibraryTestUtil_getDefaultCleartextCheckCount(env), test.expected_default_call_count);
    }
}
#endif
...
protected:
    URLRequestHttpJobWebSocketTest()
    {
        ...
        context_builder->set_client_socket_factory_for_testing(&socket_factory_);
        context_ = context_builder->Build();
        req_ = context_->CreateRequest(GURL("ws://www.example.org"), DEFAULT_PRIORITY, &delegate_, TRAFFIC_ANNOTATION_FOR_TESTS,
...
TEST_F(URLRequestHttpJobWebSocketTest, CreateHelperPassedThrough)
{
    ...
    req_->SetUserData(kWebSocketHandshakeUserDataKey, std::move(websocket_stream_create_helper));
...
bool CreateAndSetCookie(CookieStore* cs, const GURL& url, const std::string& cookie_line)
{
    ...
    DCHECK(cs);
    ResultSavingCookieCallback<CookieAccessResult> callback;
    cs->SetCanonicalCookieAsync(std::move(cookie), url, CookieOptions::MakeAllInclusive(), callback.MakeCallback());
    callback.WaitUntilDone();
    return callback.result().status.IsInclude();
}
...
{
    TestDelegate delegate;
    std::unique_ptr<URLRequest> request = context->CreateRequest(url, DEFAULT_PRIORITY, &delegate, TRAFFIC_ANNOTATION_FOR_TESTS);
...
TEST_F(URLRequestHttpJobTest, CookieSchemeRequestSchemeHistogram)
{
    ...
    std::unique_ptr<CanonicalCookie> unset_cookie1 = CanonicalCookie::CreateForTesting(secure_url_for_unset1, "NoSourceSchemeHttps=val", base::Time::Now());
    ...
    histograms.ExpectBucketCount(test_histogram, URLRequestHttpJob::CookieRequestScheme::kUnsetCookieScheme, 1);
    RunRequest(context.get(), secure_url_for_unset1);
    histograms.ExpectBucketCount(test_histogram, URLRequestHttpJob::CookieRequestScheme::kUnsetCookieScheme, 2);
    ...
    std::unique_ptr<CanonicalCookie> unset_cookie2 = CanonicalCookie::CreateForTesting(nonsecure_url_for_unset2, "NoSourceSchemeHttp=val", base::Time::Now());
    ...
    histograms.ExpectBucketCount(test_histogram, URLRequestHttpJob::CookieRequestScheme::kUnsetCookieScheme, 3);
    RunRequest(context.get(), secure_url_for_unset2);
    histograms.ExpectBucketCount(test_histogram, URLRequestHttpJob::CookieRequestScheme::kUnsetCookieScheme, 4);
    ...
    EXPECT_TRUE(CreateAndSetCookie(cookie_store, secure_url_for_secure_set, "SecureScheme=val"));
    RunRequest(context.get(), nonsecure_url_for_secure_set);
    histograms.ExpectBucketCount(test_histogram, URLRequestHttpJob::CookieRequestScheme::kSecureSetNonsecureRequest, 1);
    RunRequest(context.get(), secure_url_for_secure_set);
    histograms.ExpectBucketCount(test_histogram, URLRequestHttpJob::CookieRequestScheme::kSecureSetSecureRequest, 1);
    ...
    EXPECT_TRUE(CreateAndSetCookie(cookie_store, nonsecure_url_for_nonsecure_set, "NonSecureScheme=val"));
    RunRequest(context.get(), nonsecure_url_for_nonsecure_set);
    histograms.ExpectBucketCount(test_histogram, URLRequestHttpJob::CookieRequestScheme::kNonsecureSetNonsecureRequest, 1);
    RunRequest(context.get(), secure_url_for_nonsecure_set);
    histograms.ExpectBucketCount(test_histogram, URLRequestHttpJob::CookieRequestScheme::kNonsecureSetSecureRequest, 1);
}
...
TEST_F(URLRequestHttpJobTest, PrivacyMode_ExclusionReason)
{
    ...
    {
        TestDelegate d;
        GURL test_url = test_server.GetURL("/set-cookie?one=1&"
                                           "two=2&"
                                           "three=3");
    ...
    EXPECT_THAT(req->maybe_sent_cookies(),
        UnorderedElementsAre(MatchesCookieWithAccessResult(MatchesCookieWithNameSourceType("one", CookieSourceType::kHTTP),
                                 MatchesCookieAccessResult(HasExactlyExclusionReasonsForTesting(std::vector<CookieInclusionStatus::ExclusionReason> {
                                                               CookieInclusionStatus::EXCLUDE_USER_PREFERENCES }),
                                     _, _, _)),
            MatchesCookieWithAccessResult(MatchesCookieWithNameSourceType("two", CookieSourceType::kHTTP),
                MatchesCookieAccessResult(HasExactlyExclusionReasonsForTesting(
                                              std::vector<CookieInclusionStatus::ExclusionReason> { CookieInclusionStatus::EXCLUDE_USER_PREFERENCES }),
                    _, _, _)),
            MatchesCookieWithAccessResult(MatchesCookieWithNameSourceType("three", CookieSourceType::kHTTP),
                MatchesCookieAccessResult(HasExactlyExclusionReasonsForTesting(
                                              std::vector<CookieInclusionStatus::ExclusionReason> { CookieInclusionStatus::EXCLUDE_USER_PREFERENCES }),
                    _, _, _))));
...
TEST_F(URLRequestHttpJobTest, IndividuallyBlockedCookies)
{
    ...
    {
        TestDelegate d;
        GURL test_url = test_server.GetURL("/set-cookie?blocked_one=1;SameSite=Lax;Secure&"
    ...
    EXPECT_THAT(req->maybe_sent_cookies(),
        UnorderedElementsAre(MatchesCookieWithAccessResult(MatchesCookieWithNameSourceType("blocked_one", CookieSourceType::kHTTP),
                                 MatchesCookieAccessResult(HasExactlyExclusionReasonsForTesting(std::vector<CookieInclusionStatus::ExclusionReason> {
                                                               CookieInclusionStatus::EXCLUDE_USER_PREFERENCES }),
                                     _, _, _)),
            MatchesCookieWithAccessResult(MatchesCookieWithNameSourceType("blocked_two", CookieSourceType::kHTTP),
                MatchesCookieAccessResult(HasExactlyExclusionReasonsForTesting(
                                              std::vector<CookieInclusionStatus::ExclusionReason> { CookieInclusionStatus::EXCLUDE_USER_PREFERENCES }),
                    _, _, _)),
            MatchesCookieWithAccessResult(
                MatchesCookieWithNameSourceType("allowed", CookieSourceType::kHTTP), MatchesCookieAccessResult(IsInclude(), _, _, _))));
}

namespace {

int content_count = 0;
std::unique_ptr<test_server::HttpResponse> IncreaseOnRequest(const test_server::HttpRequest& request)
{
...
TEST_F(URLRequestHttpJobTest, GetFirstPartySetsCacheFilterMatchInfo)
{
    ...
    TestCookieAccessDelegate* raw_cookie_access_delegate = cookie_access_delegate.get();
    ...
    const IsolationInfo kTestIsolationInfo = IsolationInfo::CreateForInternalRequest(url::Origin::Create(kTestUrl));
    {
        TestDelegate delegate;
        std::unique_ptr<URLRequest> req(context->CreateRequest(kTestUrl, DEFAULT_PRIORITY, &delegate, TRAFFIC_ANNOTATION_FOR_TESTS));
    ...
    FirstPartySetsCacheFilter cache_filter({ { SchemefulSite(kTestUrl), kClearAtRunId } }, kBrowserRunId);
    raw_cookie_access_delegate->set_first_party_sets_cache_filter(std::move(cache_filter));
    {
        TestDelegate delegate;
        std::unique_ptr<URLRequest> req(context->CreateRequest(kTestUrl, DEFAULT_PRIORITY, &delegate, TRAFFIC_ANNOTATION_FOR_TESTS));
        req->SetLoadFlags(LOAD_SKIP_CACHE_VALIDATION);
...
TEST_F(URLRequestHttpJobTest, SetPartitionedCookie)
{
    ...
    const url::Origin kTopFrameOrigin = url::Origin::Create(GURL("https://www.toplevelsite.com"));
    const IsolationInfo kTestIsolationInfo = IsolationInfo::CreateForInternalRequest(kTopFrameOrigin);

    {
        TestDelegate delegate;
        std::unique_ptr<URLRequest> req(context->CreateRequest(https_test.GetURL("/set-cookie?__Host-foo=bar;SameSite=None;Secure;Path=/"
                                                                                 ";Partitioned;"),
    ...
    { // Test request from the same top-level site.
        ...
        std::unique_ptr<URLRequest> req(
            context->CreateRequest(https_test.GetURL("/echoheader?Cookie"), DEFAULT_PRIORITY, &delegate, TRAFFIC_ANNOTATION_FOR_TESTS));
        req->set_isolation_info(kTestIsolationInfo);
        req->Start();
        delegate.RunUntilComplete();
    ...
    { // Test request from a different top-level site.
        const url::Origin kOtherTopFrameOrigin = url::Origin::Create(GURL("https://www.anothertoplevelsite.com"));
        const IsolationInfo kOtherTestIsolationInfo = IsolationInfo::CreateForInternalRequest(kOtherTopFrameOrigin);
    ...
    { // Test request from same top-level eTLD+1 but different scheme. Note that
        ...
        const url::Origin kHttpTopFrameOrigin = url::Origin::Create(GURL("http://www.toplevelsite.com"));
        const IsolationInfo kHttpTestIsolationInfo = IsolationInfo::CreateForInternalRequest(kHttpTopFrameOrigin);
...
TEST_F(URLRequestHttpJobTest, PartitionedCookiePrivacyMode)
{
    ...
    auto context_builder = CreateTestURLRequestContextBuilder();
    context_builder->SetCookieStore(std::make_unique<CookieMonster>(/*store=*/nullptr, /*net_log=*/nullptr));
    auto& network_delegate = *context_builder->set_network_delegate(std::make_unique<FilteringTestNetworkDelegate>());
    auto context = context_builder->Build();
...

    {
        ...
        TestDelegate delegate;
        std::unique_ptr<URLRequest> req(context->CreateRequest(https_test.GetURL("/set-cookie?__Host-partitioned=0;SameSite=None;Secure;Path=/"
    ...
    { // Get both cookies when privacy mode is disabled.
        ...
        req->Start();
        delegate.RunUntilComplete();
        EXPECT_EQ("__Host-partitioned=0; __Host-unpartitioned=1", delegate.data_received());
    }
    ...
    { // Get cookies with privacy mode enabled and partitioned state allowed.
        ...
        auto want_exclusion_reasons = std::vector<CookieInclusionStatus::ExclusionReason> {};

        EXPECT_THAT(req->maybe_sent_cookies(),
            UnorderedElementsAre(MatchesCookieWithAccessResult(MatchesCookieWithNameSourceType("__Host-partitioned", CookieSourceType::kHTTP),
                                     MatchesCookieAccessResult(HasExactlyExclusionReasonsForTesting(want_exclusion_reasons), _, _, _)),
                MatchesCookieWithAccessResult(MatchesCookieWithNameSourceType("__Host-unpartitioned", CookieSourceType::kHTTP),
                    MatchesCookieAccessResult(HasExactlyExclusionReasonsForTesting(
                                                  std::vector<CookieInclusionStatus::ExclusionReason> { CookieInclusionStatus::EXCLUDE_USER_PREFERENCES }),
                        _, _, _))));
    }
