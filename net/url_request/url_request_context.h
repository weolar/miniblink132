...
// complete.
    ...
    // Linux & Windows.
    std::unique_ptr<URLRequest> CreateRequest(const GURL& url, RequestPriority priority, URLRequest::Delegate* delegate) const;
#endif
    ...
    std::unique_ptr<URLRequest> CreateRequest(const GURL& url, RequestPriority priority, URLRequest::Delegate* delegate,
        NetworkTrafficAnnotationTag traffic_annotation, bool is_for_websockets = false,
        const std::optional<net::NetLogSource> net_log_source = std::nullopt) const;
    ...
    const URLRequestJobFactory* job_factory() const
    {
        return job_factory_;
    }

    QuicContext* quic_context() const
    {
        return quic_context_.get();
    }
    ...
    bool check_cleartext_permitted() const
    {
        return check_cleartext_permitted_;
    }
    ...
    handles::NetworkHandle bound_network() const
    {
...
private:
    ...
    void set_http_auth_handler_factory(std::unique_ptr<HttpAuthHandlerFactory> factory);
    void set_http_network_session(std::unique_ptr<HttpNetworkSession> http_network_session);
    void set_http_transaction_factory(std::unique_ptr<HttpTransactionFactory> factory);
    void set_network_delegate(std::unique_ptr<NetworkDelegate> network_delegate);
    void set_http_server_properties(std::unique_ptr<HttpServerProperties> http_server_properties);
    void set_cookie_store(std::unique_ptr<CookieStore> cookie_store);
    void set_transport_security_state(std::unique_ptr<TransportSecurityState> state);
    ...
    void set_http_user_agent_settings(std::unique_ptr<const HttpUserAgentSettings> http_user_agent_settings);
    ...
    void set_network_error_logging_service(std::unique_ptr<NetworkErrorLoggingService> network_error_logging_service);
#endif // BUILDFLAG(ENABLE_REPORTING)
    ...
    void set_require_network_anonymization_key(bool require_network_anonymization_key)
    {
    ...
    void set_transport_security_persister(std::unique_ptr<TransportSecurityPersister> transport_security_persister);

    raw_ptr<NetLog> net_log_ = nullptr;
#if BUILDFLAG(ENABLE_DEVICE_BOUND_SESSIONS)
    void set_device_bound_session_store(std::unique_ptr<device_bound_sessions::SessionStore> device_bound_session_store);
    ...
    std::unique_ptr<std::set<raw_ptr<const URLRequest, SetExperimental>>> url_requests_;

#if BUILDFLAG(ENABLE_DEVICE_BOUND_SESSIONS)
    std::unique_ptr<device_bound_sessions::SessionStore> device_bound_session_store_;
