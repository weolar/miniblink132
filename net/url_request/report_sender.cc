...
public:
    CallbackInfo(net::ReportSender::SuccessCallback success_callback, net::ReportSender::ErrorCallback error_callback)
    ...
    void RunErrorCallback(const GURL& url, int net_error, int http_response_code)
    {
...
ReportSender::ReportSender(URLRequestContext* request_context, net::NetworkTrafficAnnotationTag traffic_annotation)
...
void ReportSender::Send(const GURL& report_uri, base::StringPiece content_type, base::StringPiece report,
    const NetworkAnonymizationKey& network_anonymization_key, SuccessCallback success_callback, ErrorCallback error_callback)
{
    DCHECK(!content_type.empty());
    std::unique_ptr<URLRequest> url_request = request_context_->CreateRequest(report_uri, DEFAULT_PRIORITY, this, traffic_annotation_);
    url_request->SetUserData(&kUserDataKey, std::make_unique<CallbackInfo>(std::move(success_callback), std::move(error_callback)));
    url_request->SetLoadFlags(kLoadFlags);
    url_request->set_allow_credentials(false);
    url_request->set_isolation_info_from_network_anonymization_key(network_anonymization_key);
    ...
    url_request->set_upload(ElementsUploadDataStream::CreateWithReader(std::move(reader), 0));
...
void ReportSender::OnResponseStarted(URLRequest* request, int net_error)
{
    ...
    CallbackInfo* callback_info = static_cast<CallbackInfo*>(request->GetUserData(&kUserDataKey));
    ...
    if (net_error != OK) {
        ...
        callback_info->RunErrorCallback(request->url(), OK, request->GetResponseCode());
    } else {
