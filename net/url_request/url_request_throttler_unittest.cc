...
public:
    ...
    MockURLRequestThrottlerEntry(URLRequestThrottlerManager* manager, const TimeTicks& exponential_backoff_release_time,
        const TimeTicks& sliding_window_release_time, const TimeTicks& fake_now)
        : URLRequestThrottlerEntry(manager, std::string())
    ...
    BackoffEntry* GetBackoffEntry() override
    {
        return &backoff_entry_;
    }
    ...
    {
        return fake_clock_.NowTicks();
    }

    void set_fake_now(const TimeTicks& now)
    {
        fake_clock_.set_now(now);
    }
...
public:
    ...
    {
        return GetNumberOfEntriesForTests();
    }
    ...
    void CreateEntry(bool is_outdated)
    {
        ...
        if (is_outdated) {
            time -= base::Milliseconds(MockURLRequestThrottlerEntry::kDefaultEntryLifetimeMs + 1000);
        }
        ...
        OverrideEntryForTests(fake_url, base::MakeRefCounted<MockURLRequestThrottlerEntry>(this, time, TimeTicks::Now(), TimeTicks::Now()));
    }
...
struct GurlAndString {
    GurlAndString(const GURL& url_value, const std::string& expected, int line_num)
    {
        url = url_value;
        result = expected;
        line = line_num;
...
protected:
    URLRequestThrottlerEntryTest()
        ...
        , request_(context_->CreateRequest(GURL(), DEFAULT_PRIORITY, nullptr, TRAFFIC_ANNOTATION_FOR_TESTS))
    {
    }
...
TEST_F(URLRequestThrottlerEntryTest, InterfaceDuringExponentialBackoff)
{
    ...
    entry_->set_exponential_backoff_release_time(entry_->ImplGetTimeNow() + base::Milliseconds(1));
...
TEST_F(URLRequestThrottlerEntryTest, InterfaceUpdateFailure)
{
    ...
    EXPECT_GT(entry_->GetExponentialBackoffReleaseTime(), entry_->ImplGetTimeNow()) << "A failure should increase the release_time";
}

TEST_F(URLRequestThrottlerEntryTest, InterfaceUpdateSuccess)
{
    entry_->UpdateWithResponse(200);
    EXPECT_EQ(entry_->GetExponentialBackoffReleaseTime(), entry_->ImplGetTimeNow()) << "A success should not add any delay";
}
...
TEST_F(URLRequestThrottlerEntryTest, InterfaceUpdateSuccessThenFailure)
{
    entry_->UpdateWithResponse(200);
    entry_->UpdateWithResponse(503);
    EXPECT_GT(entry_->GetExponentialBackoffReleaseTime(), entry_->ImplGetTimeNow()) << "This scenario should add delay";
    entry_->UpdateWithResponse(200);
}

TEST_F(URLRequestThrottlerEntryTest, IsEntryReallyOutdated)
{
    base::TimeDelta lifetime = base::Milliseconds(MockURLRequestThrottlerEntry::kDefaultEntryLifetimeMs);
    const base::TimeDelta kFiveMs = base::Milliseconds(5);

    TimeAndBool test_values[] = { TimeAndBool(now_, false, __LINE__), TimeAndBool(now_ - kFiveMs, false, __LINE__),
        TimeAndBool(now_ + kFiveMs, false, __LINE__), TimeAndBool(now_ - (lifetime - kFiveMs), false, __LINE__), TimeAndBool(now_ - lifetime, true, __LINE__),
...
TEST_F(URLRequestThrottlerEntryTest, MaxAllowedBackoff)
{
    ...
    EXPECT_EQ(delay.InMilliseconds(), MockURLRequestThrottlerEntry::kDefaultMaximumBackoffMs);
}
...
TEST_F(URLRequestThrottlerEntryTest, SlidingWindow)
{
    ...
    TimeTicks time_1 = entry_->ImplGetTimeNow() + base::Milliseconds(sliding_window / 3);
    TimeTicks time_2 = entry_->ImplGetTimeNow() + base::Milliseconds(2 * sliding_window / 3);
    TimeTicks time_3 = entry_->ImplGetTimeNow() + base::Milliseconds(sliding_window);
    TimeTicks time_4 = entry_->ImplGetTimeNow() + base::Milliseconds(sliding_window + 2 * sliding_window / 3);

    entry_->set_exponential_backoff_release_time(time_1);

    for (int i = 0; i < max_send / 2; ++i) {
        EXPECT_EQ(2 * sliding_window / 3, entry_->ReserveSendingTimeForNextRequest(time_2));
    }
...
TEST_F(URLRequestThrottlerManagerTest, IsUrlStandardised)
{
    ...
    GurlAndString test_values[] = { GurlAndString(GURL("http://www.example.com"), std::string("http://www.example.com/"), __LINE__),
        GurlAndString(GURL("http://www.Example.com"), std::string("http://www.example.com/"), __LINE__),
        GurlAndString(GURL("http://www.ex4mple.com/Pr4c71c41"), std::string("http://www.ex4mple.com/pr4c71c41"), __LINE__),
        GurlAndString(GURL("http://www.example.com/0/token/false"), std::string("http://www.example.com/0/token/false"), __LINE__),
        GurlAndString(GURL("http://www.example.com/index.php?code=javascript"), std::string("http://www.example.com/index.php"), __LINE__),
        GurlAndString(GURL("http://www.example.com/index.php?code=1#superEntry"), std::string("http://www.example.com/index.php"), __LINE__),
        GurlAndString(GURL("http://www.example.com/index.php#superEntry"), std::string("http://www.example.com/index.php"), __LINE__),
        GurlAndString(GURL("http://www.example.com:1234/"), std::string("http://www.example.com:1234/"), __LINE__) };
...
TEST_F(URLRequestThrottlerManagerTest, LocalHostOptedOut)
{
    ...
    EXPECT_GT(TimeTicks::Now() + base::Milliseconds(100), localhost_entry->GetExponentialBackoffReleaseTime());
}
...
TEST_F(URLRequestThrottlerManagerTest, ClearOnNetworkChange)
{
    for (int i = 0; i < 3; ++i) {
        ...
        case 0:
            ...
            break;
        case 1:
            manager.OnConnectionTypeChanged(NetworkChangeNotifier::CONNECTION_UNKNOWN);
            break;
        case 2:
