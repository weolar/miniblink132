...
public:
    ...
    virtual bool IsRedirectResponse(GURL* location, int* http_status_code, bool* insecure_scheme_was_upgraded);
    ...
    virtual void ContinueWithCertificate(scoped_refptr<X509Certificate> client_cert, scoped_refptr<SSLPrivateKey> client_private_key);
    ...
    int64_t expected_content_size() const
    {
    ...
    // received.
    virtual void SetEarlyResponseHeadersCallback(ResponseHeadersCallback callback)
    {
    }
    ...
    virtual void SetIsSharedDictionaryReadAllowedCallback(base::RepeatingCallback<bool()> callback)
    {
    }
    ...
    virtual void SetSharedDictionaryGetter(SharedDictionaryGetter dictionary_getter)
    {
    }
    ...
    static GURL ComputeReferrerForPolicy(
        ReferrerPolicy policy, const GURL& original_referrer, const GURL& destination, bool* same_origin_out_for_metrics = nullptr);
...
private:
    ...
    // synchronously.
    int ReadRawDataHelper(IOBuffer* buf, int buf_size, CompletionOnceCallback callback);
