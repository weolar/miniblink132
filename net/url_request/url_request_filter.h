...
public:
    ...
    void AddHostnameInterceptor(const std::string& scheme, const std::string& hostname, std::unique_ptr<URLRequestInterceptor> interceptor);
    void RemoveHostnameHandler(const std::string& scheme, const std::string& hostname);

    // Returns true if we successfully added the URL handler.  This will replace
    // old handlers for the URL if one existed.
    bool AddUrlInterceptor(const GURL& url, std::unique_ptr<URLRequestInterceptor> interceptor);
...
private:
    ...
    using HostnameInterceptorMap = std::map<std::pair<std::string, std::string>, std::unique_ptr<URLRequestInterceptor>>;
