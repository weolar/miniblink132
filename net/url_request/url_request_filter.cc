...
void URLRequestFilter::AddHostnameInterceptor(const std::string& scheme, const std::string& hostname, std::unique_ptr<URLRequestInterceptor> interceptor)
{
    DCHECK(OnMessageLoopForInterceptorAddition());
    DCHECK_EQ(0u, hostname_interceptor_map_.count(std::pair(scheme, hostname)));
    hostname_interceptor_map_[std::pair(scheme, hostname)] = std::move(interceptor);
...
void URLRequestFilter::RemoveHostnameHandler(const std::string& scheme, const std::string& hostname)
{
...
bool URLRequestFilter::AddUrlInterceptor(const GURL& url, std::unique_ptr<URLRequestInterceptor> interceptor)
{
    DCHECK(OnMessageLoopForInterceptorAddition());
    if (!url.is_valid())
        return false;
    ...
    DCHECK_EQ(0u, hostname_interceptor_map_.count(std::pair(url.scheme(), url.host())));
...
std::unique_ptr<URLRequestJob> URLRequestFilter::MaybeInterceptRequest(URLRequest* request) const
{
