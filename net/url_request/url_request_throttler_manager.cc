...
scoped_refptr<URLRequestThrottlerEntryInterface> URLRequestThrottlerManager::RegisterRequestUrl(const GURL& url)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    if (entry.get() == nullptr) {
        ...
        if (IsLocalhost(url)) {
            if (!logged_for_localhost_disabled_ && IsLocalhost(url)) {
                ...
                net_log_.AddEventWithStringParams(NetLogEventType::THROTTLING_DISABLED_FOR_HOST, "host", url.host());
            }
...
void URLRequestThrottlerManager::OverrideEntryForTests(const GURL& url, scoped_refptr<URLRequestThrottlerEntry> entry)
{
    // Normalize the url.
    std::string url_id = GetIdFromUrl(url);
...
{
    DCHECK(net_log);
    net_log_ = NetLogWithSource::Make(net_log, NetLogSourceType::EXPONENTIAL_BACKOFF_THROTTLING);
}
...
void URLRequestThrottlerManager::OnConnectionTypeChanged(NetworkChangeNotifier::ConnectionType type)
{
    OnNetworkChange();
}
