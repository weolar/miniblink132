...
perf_test::PerfResultReporter SetUpURLRequestQuicReporter(const std::string& story)
{
...
std::unique_ptr<test_server::HttpResponse> HandleRequest(const test_server::HttpRequest& request)
{
    auto http_response = std::make_unique<test_server::BasicHttpResponse>();
    std::string alpn = quic::AlpnForVersion(DefaultSupportedQuicVersions().front());
    http_response->AddCustomHeader("Alt-Svc", base::StringPrintf("%s=\"%s:%d\"", alpn.c_str(), kAltSvcHost, kAltSvcPort));
...
protected:
    URLRequestQuicPerfTest()
        : task_environment_(std::make_unique<base::test::SingleThreadTaskEnvironment>(base::test::SingleThreadTaskEnvironment::MainThreadType::IO))
        , cert_verifier_(std::make_unique<MockCertVerifier>())
    {
        ...
        auto host_resolver = std::make_unique<MappedHostResolver>(std::move(resolver));
        std::string map_rule = base::StringPrintf("MAP %s 127.0.0.1:%d", kOriginHost, tcp_server_->port());
    ...
    {
        return context_->CreateRequest(url, priority, delegate, TRAFFIC_ANNOTATION_FOR_TESTS);
    }
...
private:
    void StartQuicServer()
    {
        ...
        memory_cache_backend_.AddSimpleResponse(kOriginHost, kHelloPath, kHelloStatus, kHelloAltSvcResponse);
        quic_server_ = std::make_unique<QuicSimpleServer>(net::test::ProofSourceForTestingChromium(), config, quic::QuicCryptoServerConfig::ConfigOptions(),
            quic::AllSupportedVersions(), &memory_cache_backend_);
        int rv = quic_server_->Listen(net::IPEndPoint(net::IPAddress::IPv4AllZeros(), kAltSvcPort));
        ...
        cert_verifier().AddResultForCert(verify_result.verified_cert.get(), verify_result, OK);
    }

    void StartTcpServer()
    {
        tcp_server_ = std::make_unique<EmbeddedTestServer>(net::EmbeddedTestServer::TYPE_HTTPS);
        ...
        cert_verifier().AddResultForCert(tcp_server_->GetCertificate(), verify_result, OK);
    }
...
TEST_F(URLRequestQuicPerfTest, TestGetRequest)
{
    ...
    for (int i = 0; i < kNumRequest; ++i) {
        ...
        std::unique_ptr<URLRequest> request = CreateRequest(url, DEFAULT_PRIORITY, &delegate);
    ...
    reporter.AddResult(kMetricRequestTimeMs, (end - start).InMillisecondsF() / kNumRequest);
