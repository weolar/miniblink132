...
class NET_EXPORT ReportSender : public URLRequest::Delegate, public TransportSecurityState::ReportSenderInterface {
public:
    static const int kLoadFlags;

    using SuccessCallback = base::OnceCallback<void()>;
    using ErrorCallback = base::OnceCallback<void(const GURL&, int /* net_error */, int /* http_response_code */)>;
    ...
    explicit ReportSender(URLRequestContext* request_context, net::NetworkTrafficAnnotationTag traffic_annotation);
    ...
    void Send(const GURL& report_uri, base::StringPiece content_type, base::StringPiece report, const NetworkAnonymizationKey& network_anonymization_key,
        SuccessCallback success_callback, ErrorCallback error_callback) override;
