...
void GZipServer(const HttpRequestInfo* request, std::string* response_status, std::string* response_headers, std::string* response_data)
{
    response_data->assign(kGzipData, sizeof(kGzipData));
}

void GZipHelloServer(const HttpRequestInfo* request, std::string* response_status, std::string* response_headers, std::string* response_data)
{
    response_data->assign(kGzipHelloData, sizeof(kGzipHelloData) - 1);
}

void BigGZipServer(const HttpRequestInfo* request, std::string* response_status, std::string* response_headers, std::string* response_data)
{
    response_data->assign(kGzipDataWithName, sizeof(kGzipDataWithName));
    response_data->insert(10, 64 * 1024, 'a');
}

void BrotliHelloServer(const HttpRequestInfo* request, std::string* response_status, std::string* response_headers, std::string* response_data)
{
    response_data->assign(kBrotliHelloData, sizeof(kBrotliHelloData) - 1);
}

void MakeMockReferrerPolicyTransaction(const char* referer_header, const char* response_headers, MockTransaction* transaction)
{
...
TEST_F(URLRequestJobTest, TransactionNoFilter)
{
    ...
    auto* network_layer = context_builder->SetHttpTransactionFactoryForTesting(std::make_unique<MockNetworkLayer>());
    context_builder->DisableHttpCache();
    auto context = context_builder->Build();

    ScopedMockTransaction transaction(kNoFilterTransaction);
    TestDelegate d;
    std::unique_ptr<URLRequest> req(context->CreateRequest(GURL(transaction.url), DEFAULT_PRIORITY, &d, TRAFFIC_ANNOTATION_FOR_TESTS));
...
TEST_F(URLRequestJobTest, SyncTransactionNotifiedWhenDone)
{
    ...
    ScopedMockTransaction transaction(kGZipTransaction);
    TestDelegate d;
    std::unique_ptr<URLRequest> req(context->CreateRequest(GURL(kGZipTransaction.url), DEFAULT_PRIORITY, &d, TRAFFIC_ANNOTATION_FOR_TESTS));
...
TEST_F(URLRequestJobTest, RedirectTransactionWithReferrerPolicyHeader)
{
    ...
    const TestCase kTests[] = {
        ...
        { "http://foo.test/one" /* original url */, "http://foo.test/one" /* original referrer */, "Location: http://foo.test/test\n",
            ...
            ReferrerPolicy::CLEAR_ON_TRANSITION_FROM_SECURE_TO_INSECURE, "http://foo.test/one" /* expected final referrer */ },
    };
    ...
    for (const auto& test : kTests) {
        ...
        MakeMockReferrerPolicyTransaction(request_headers.c_str(), test.response_headers, &transaction);
...
TEST(URLRequestJobComputeReferrer, SetsSameOriginForMetricsOnSameOrigin)
{
    ...
    URLRequestJob::ComputeReferrerForPolicy(ReferrerPolicy(),
...
{
    // Shouldn't segfault.
    URLRequestJob::ComputeReferrerForPolicy(ReferrerPolicy(), GURL(), GURL(), nullptr);
}

TEST(URLRequestJobComputeReferrer, FilesystemDestination)
{
    EXPECT_EQ(URLRequestJob::ComputeReferrerForPolicy(
                  ReferrerPolicy::NEVER_CLEAR, GURL("https://referrer.example"), GURL("filesystem:https://destination.example"), nullptr),
...
TEST(URLRequestJobComputeReferrer, TruncatesLongReferrer)
{
    ...
    EXPECT_EQ(
        URLRequestJob::ComputeReferrerForPolicy(ReferrerPolicy::NEVER_CLEAR, kOriginalReferrer, GURL("https://google.com")), GURL("https://referrer.example/"));
}
...
TEST(URLRequestJobComputeReferrer, DoesntTruncateShortReferrer)
{
    ...
    EXPECT_EQ(URLRequestJob::ComputeReferrerForPolicy(ReferrerPolicy::NEVER_CLEAR, kOriginalReferrer, GURL("https://google.com")), kOriginalReferrer);
}
...
TEST(URLRequestJobComputeReferrer, DoesntTruncateReferrerWithLongRef)
{
    ...
    EXPECT_EQ(URLRequestJob::ComputeReferrerForPolicy(
                  ReferrerPolicy::NEVER_CLEAR, GURL(std::string("https://referrer.example/path#") + std::string(5000, 'a')), GURL("https://google.com")),
...
TEST(URLRequestJobComputeReferrer, InvalidSchemeReferrer)
{
    ...
    ASSERT_FALSE(url::IsReferrerScheme(kOriginalReferrer.spec().data(), kOriginalReferrer.parsed_for_possibly_invalid_spec().scheme));

    EXPECT_EQ(URLRequestJob::ComputeReferrerForPolicy(ReferrerPolicy::NEVER_CLEAR, kOriginalReferrer, GURL("https://google.com")), GURL());

    EXPECT_EQ(URLRequestJob::ComputeReferrerForPolicy(ReferrerPolicy::ORIGIN, kOriginalReferrer, GURL("https://google.com")), GURL());
}

TEST(URLRequestJobComputeReferrer, CapReferrerOnCrossOrigin)
{
    base::test::ScopedFeatureList feature_list;
    feature_list.InitAndEnableFeature(features::kCapReferrerToOriginOnCrossOrigin);

    const GURL kOriginalReferrer("https://boggle.com/path");

    EXPECT_EQ(URLRequestJob::ComputeReferrerForPolicy(ReferrerPolicy::NEVER_CLEAR, kOriginalReferrer, GURL("https://google.com")), GURL("https://boggle.com/"));
}

TEST(URLRequestJobComputeReferrer, CapReferrerOnCrossOriginRespectsStricterPolicy)
{
    base::test::ScopedFeatureList feature_list;
    feature_list.InitAndEnableFeature(features::kCapReferrerToOriginOnCrossOrigin);
    ...
    EXPECT_EQ(URLRequestJob::ComputeReferrerForPolicy(ReferrerPolicy::NO_REFERRER, kOriginalReferrer, GURL("https://google.com")), GURL());
}

TEST(URLRequestJobComputeReferrer, CapReferrerOnCrossOriginDoesntCapOnSameOrigin)
{
    base::test::ScopedFeatureList feature_list;
    feature_list.InitAndEnableFeature(features::kCapReferrerToOriginOnCrossOrigin);
    ...
    EXPECT_EQ(URLRequestJob::ComputeReferrerForPolicy(ReferrerPolicy::NEVER_CLEAR, kOriginalReferrer, GURL("https://boggle.com")), kOriginalReferrer);
}
