...
public:
    ...
    public:
        ...
        virtual int OnConnected(URLRequest* request, const TransportInfo& info, CompletionOnceCallback callback);
        ...
        // deferring redirect.
        virtual void OnReceivedRedirect(URLRequest* request, const RedirectInfo& redirect_info, bool* defer_redirect);
        ...
        virtual void OnCertificateRequested(URLRequest* request, SSLCertRequestInfo* cert_request_info);
        ...
        virtual void OnSSLCertificateError(URLRequest* request, int net_error, const SSLInfo& ssl_info, bool fatal);
    ...
    URLRequest(base::PassKey<URLRequestContext> pass_key, const GURL& url, RequestPriority priority, Delegate* delegate, const URLRequestContext* context,
        NetworkTrafficAnnotationTag traffic_annotation, bool is_for_websockets, std::optional<net::NetLogSource> net_log_source);
    ...
    void set_isolation_info(const IsolationInfo& isolation_info, std::optional<GURL> redirect_info_new_url = std::nullopt);
    ...
    void set_isolation_info_from_network_anonymization_key(const NetworkAnonymizationKey& network_anonymization_key);
    ...
    void set_first_party_url_policy(RedirectInfo::FirstPartyURLPolicy first_party_url_policy);
    ...
    int reporting_upload_depth() const
    {
        return reporting_upload_depth_;
    }
    ...
    // request is started.
    ReferrerPolicy referrer_policy() const
    {
    ...
    // follow it.
    void SetExtraRequestHeaderByName(std::string_view name, std::string_view value, bool overwrite);
    ...
    {
        return response_info_.was_cached;
    }
    ...
    {
        return partial_load_flags_ | per_hop_load_flags_;
    }
    ...
    void set_per_hop_load_flags(int flags)
    {
        per_hop_load_flags_ = flags;
    }
    ...
    void FollowDeferredRedirect(const std::optional<std::vector<std::string>>& removed_headers, const std::optional<net::HttpRequestHeaders>& modified_headers);
    ...
    void ContinueWithCertificate(scoped_refptr<X509Certificate> client_cert, scoped_refptr<SSLPrivateKey> client_private_key);
    ...
    const std::optional<base::flat_set<net::SourceStream::SourceType>>& accepted_stream_types() const
    {
        return accepted_stream_types_;
    }

    void set_accepted_stream_types(const std::optional<base::flat_set<net::SourceStream::SourceType>>& types)
    {
        if (types) {
    ...
    void SetIsSharedDictionaryReadAllowedCallback(base::RepeatingCallback<bool()> callback);
    ...
    void SetDeviceBoundSessionAccessCallback(base::RepeatingCallback<void(const device_bound_sessions::SessionKey&)> callback);
    base::RepeatingCallback<void(const device_bound_sessions::SessionKey&)> device_bound_session_access_callback()
    {
    ...
    // to advertising.
    void set_ad_tagged(bool ad_tagged)
    {
        ad_tagged_ = ad_tagged;
    }
    bool ad_tagged() const
    {
        return ad_tagged_;
    }
    ...
    bool send_client_certs() const
    {
        return send_client_certs_;
    }

    bool is_for_websockets() const
    {
        return is_for_websockets_;
    }

    void SetIdempotency(Idempotency idempotency)
    {
        idempotency_ = idempotency;
    }
    Idempotency GetIdempotency() const
    {
        return idempotency_;
    }
    ...
    void SetSharedDictionaryGetter(SharedDictionaryGetter shared_dictionary_getter);
    ...
    std::optional<cookie_util::StorageAccessStatus> storage_access_status() const
    {
    ...
    std::optional<net::cookie_util::StorageAccessStatus> CalculateStorageAccessStatus(
        base::optional_ref<const RedirectInfo> redirect_info = base::optional_ref<const RedirectInfo>(std::nullopt)) const;
...
protected:
    ...
    void Redirect(const RedirectInfo& redirect_info, const std::optional<std::vector<std::string>>& removed_headers,
...
private:
    ...
    int NotifyConnected(const TransportInfo& info, CompletionOnceCallback callback);
    void NotifyAuthRequired(std::unique_ptr<AuthChallengeInfo> auth_info);
    void NotifyCertificateRequested(SSLCertRequestInfo* cert_request_info);
    void NotifySSLCertificateError(int net_error, const SSLInfo& ssl_info, bool fatal);
    ...
    // been called.
    bool CanSetCookie(const net::CanonicalCookie& cookie, CookieOptions* options, const net::FirstPartySetMetadata& first_party_set_metadata,
    ...
    net::IsolationInfo CreateIsolationInfoFromNetworkAnonymizationKey(const NetworkAnonymizationKey& network_anonymization_key);
    ...
    ReferrerPolicy referrer_policy_ = ReferrerPolicy::CLEAR_ON_TRANSITION_FROM_SECURE_TO_INSECURE;
    RedirectInfo::FirstPartyURLPolicy first_party_url_policy_ = RedirectInfo::FirstPartyURLPolicy::NEVER_CHANGE_URL;
    ...
    std::optional<base::flat_set<net::SourceStream::SourceType>> accepted_stream_types_;
    ...
    base::RepeatingCallback<void(const device_bound_sessions::SessionKey&)> device_bound_session_access_callback_;
