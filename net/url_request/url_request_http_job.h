...
protected:
    URLRequestHttpJob(URLRequest* request, const HttpUserAgentSettings* http_user_agent_settings);
...
private:
    ...
    FRIEND_TEST_ALL_PREFIXES(URLRequestHttpJobTest, CookieSchemeRequestSchemeHistogram);
    ...
    void AnnotateAndMoveUserBlockedCookies(CookieAccessResultList& maybe_included_cookies, CookieAccessResultList& excluded_cookies) const;
    ...
    void NotifyBeforeStartTransactionCallback(int result, const std::optional<HttpRequestHeaders>& headers);
    ...
    int NotifyConnectedCallback(const TransportInfo& info, CompletionOnceCallback callback);
    ...
    void ContinueWithCertificate(scoped_refptr<X509Certificate> client_cert, scoped_refptr<SSLPrivateKey> client_private_key) override;
    ...
    void SetSharedDictionaryGetter(SharedDictionaryGetter shared_dictionary_getter) override;
    ...
    void SetCookieHeaderAndStart(const CookieOptions& options, const CookieAccessResultList& cookie_list, const CookieAccessResultList& excluded_list);

    // Another Cookie Monster callback
    void OnSetCookieResult(const CookieOptions& options, std::optional<CanonicalCookie> cookie, std::string cookie_string, CookieAccessResult access_result);
    ...
    void OnGotFirstPartySetMetadata(FirstPartySetMetadata first_party_set_metadata, FirstPartySetsCacheFilter::MatchInfo match_info);
