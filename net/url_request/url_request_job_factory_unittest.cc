...
public:
    explicit MockURLRequestJob(URLRequest* request)
    ...
    void Start() override
    {
        ...
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(&MockURLRequestJob::StartAsync, weak_factory_.GetWeakPtr()));
    }
...
TEST(URLRequestJobFactoryTest, NoProtocolHandler)
{
    ...
    std::unique_ptr<URLRequest> request(request_context->CreateRequest(GURL("foo://bar"), DEFAULT_PRIORITY, &delegate, TRAFFIC_ANNOTATION_FOR_TESTS));
    request->Start();
...
TEST(URLRequestJobFactoryTest, BasicProtocolHandler)
{
    ...
    context_builder->SetProtocolHandler("foo", std::make_unique<DummyProtocolHandler>());
