...
public:
    ...
    void RunSimulation(const base::TimeDelta& maximum_simulated_duration, const base::TimeDelta& time_between_ticks)
    {
...
public:
    Server(int max_queries_per_tick, double request_drop_ratio)
        ...
        , mock_request_(context_->CreateRequest(GURL(), DEFAULT_PRIORITY, nullptr, TRAFFIC_ANNOTATION_FOR_TESTS))
    {
    }
    ...
    void SetDowntime(const TimeTicks& start_time, const base::TimeDelta& duration)
    {
...
public:
    Requester(MockURLRequestThrottlerEntry* throttler_entry, const base::TimeDelta& time_between_requests, Server* server, RequesterResults* results)
    ...
    void PerformAction() override
    {
        ...
        const base::TimeDelta effective_delay = time_between_requests_ + (base::RandInt(0, 1) ? -current_jitter : current_jitter);
    ...
    void SetStartupJitter(const base::TimeDelta& max_delay)
    {
        ...
        time_of_last_attempt_ = TimeTicks() + base::Milliseconds(delay_ms) - time_between_requests_;
    }
...
void SimulateAttack(Server* server, RequesterResults* attacker_results, RequesterResults* client_results, bool enable_throttling)
{
    ...
    for (size_t i = 0; i < kNumAttackers; ++i) {
        ...
        auto throttler_entry = base::MakeRefCounted<MockURLRequestThrottlerEntry>(&manager);
        if (!enable_throttling)
            throttler_entry->DisableBackoffThrottling();

        auto attacker = std::make_unique<Requester>(throttler_entry.get(), base::Milliseconds(1), server, attacker_results);
    ...
    for (size_t i = 0; i < kNumClients; ++i) {
        ...
        auto client = std::make_unique<Requester>(throttler_entry.get(), base::Minutes(2), server, client_results);
...
TEST(URLRequestThrottlerSimulation, HelpsInAttack)
{
    ...
    SimulateAttack(&unprotected_server, &unprotected_attacker_results, &unprotected_client_results, false);
    SimulateAttack(&protected_server, &protected_attacker_results, &protected_client_results, true);

    // These assert that the DDoS protection actually benefits the
    // server. Manual inspection of the traffic graphs will show this
    // even more clearly.
    EXPECT_GT(unprotected_server.num_overloaded_ticks(), protected_server.num_overloaded_ticks());
    EXPECT_GT(unprotected_server.max_experienced_queries_per_tick(), protected_server.max_experienced_queries_per_tick());

    // These assert that the DDoS protection actually benefits non-malicious
    // (and non-degenerate/accidentally DDoSing) users.
    EXPECT_LT(protected_client_results.GetBlockedRatio(), protected_attacker_results.GetBlockedRatio());
    EXPECT_GT(protected_client_results.GetSuccessRatio(), unprotected_client_results.GetSuccessRatio());
    ...
    unprotected_attacker_results.PrintResults("attackers attacking unprotected server.");
    unprotected_client_results.PrintResults("normal clients making requests to unprotected server.");
    protected_attacker_results.PrintResults("attackers attacking protected server.");
    protected_client_results.PrintResults("normal clients making requests to protected server.");
}

// Returns the downtime perceived by the client, as a ratio of the
// actual downtime.
double SimulateDowntime(const base::TimeDelta& duration, const base::TimeDelta& average_client_interval, bool enable_throttling)
{
    ...
    Requester requester(throttler_entry.get(), average_client_interval, &server, nullptr);
    ...
    return static_cast<double>(requester.last_downtime_duration().InMilliseconds()) / static_cast<double>(duration.InMilliseconds());
}
...
TEST(URLRequestThrottlerSimulation, PerceivedDowntimeRatio)
{
    ...
    struct Stats {
        ...
        {
            VerboseOut("  Perceived downtime with throttling is %.4f times without.\n", increase_ratio);
    ...
    struct Trial {
        ...
        void PrintTrialDescription()
        {
            ...
            const double interval_minutes = average_client_interval / base::Minutes(1);
    ...
    while (global_stats.num_runs < 100000) {
        for (auto& trial : trials) {
            ...
            double ratio_unprotected = SimulateDowntime(trial.duration, trial.average_client_interval, false);
            double ratio_protected = SimulateDowntime(trial.duration, trial.average_client_interval, true);
