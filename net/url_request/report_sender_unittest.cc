...
void CheckUploadData(const URLRequest& request, std::set<std::string>* expect_reports)
{
...
void ErrorCallback(bool* called, const GURL& report_uri, int net_error, int http_response_code)
{
...
// errors.
void ServerErrorResponseCallback(bool* called, const GURL& report_uri, int net_error, int http_response_code)
{
...
public:
    explicit MockServerErrorJob(URLRequest* request)
...
protected:
    ...
    {
        info->headers = base::MakeRefCounted<HttpResponseHeaders>("HTTP/1.1 500 Internal Server Error\n"
...
public:
    ...
    MockServerErrorJobInterceptor& operator=(const MockServerErrorJobInterceptor&) = delete;
...
public:
    ...
    TestReportSenderNetworkDelegate(const TestReportSenderNetworkDelegate&) = delete;
    TestReportSenderNetworkDelegate& operator=(const TestReportSenderNetworkDelegate&) = delete;
    ...
    void set_all_url_requests_destroyed_callback(base::RepeatingClosure callback)
    {
    ...
    void set_expect_url(const GURL& expect_url)
    {
        expect_url_ = expect_url;
    }
    ...
    void set_expected_network_anonymization_key(const NetworkAnonymizationKey& expected_network_anonymization_key)
    {
    ...
    int OnBeforeURLRequest(URLRequest* request, CompletionOnceCallback callback, GURL* new_url) override
    {
        ...
        EXPECT_EQ(expected_network_anonymization_key_, request->isolation_info().network_anonymization_key());
        EXPECT_EQ(IsolationInfo::RequestType::kOther, request->isolation_info().request_type());
        EXPECT_TRUE(request->site_for_cookies().IsNull());

        const HttpRequestHeaders& extra_headers = request->extra_request_headers();
        std::string content_type;
        EXPECT_TRUE(extra_headers.GetHeader(HttpRequestHeaders::kContentType, &content_type));
...
public:
    ReportSenderTest()
    {
        ...
        builder->set_network_delegate(std::make_unique<TestReportSenderNetworkDelegate>());
    ...
    void SetUp() override
    {
        ...
        URLRequestFilter::GetInstance()->AddHostnameInterceptor("http", kServerErrorHostname, std::make_unique<MockServerErrorJobInterceptor>());
    }
    ...
    TestReportSenderNetworkDelegate& network_delegate()
    {
        ...
        // constructor.
        return *static_cast<TestReportSenderNetworkDelegate*>(context_->network_delegate());
    }

protected:
    void SendReport(ReportSender* reporter, const std::string& report, const GURL& url, size_t request_sequence_number,
        base::OnceCallback<void()> success_callback, base::OnceCallback<void(const GURL&, int, int)> error_callback)
    {
        NetworkAnonymizationKey network_anonymization_key = NetworkAnonymizationKey::CreateTransient();
        ...
        network_delegate().set_expected_network_anonymization_key(network_anonymization_key);

        EXPECT_EQ(request_sequence_number, network_delegate().num_requests());

        reporter->Send(url, "application/foobar", report, network_anonymization_key, std::move(success_callback), std::move(error_callback));
    ...
    void SendReport(ReportSender* reporter, const std::string& report, const GURL& url, size_t request_sequence_number)
    {
        SendReport(reporter, report, url, request_sequence_number, base::OnceCallback<void()>(), base::OnceCallback<void(const GURL&, int, int)>());
    }
...
{
    base::RunLoop run_loop;
    network_delegate().set_all_url_requests_destroyed_callback(run_loop.QuitClosure());
    ...
    reporter.Send(
        url, "application/foobar", kDummyReport, NetworkAnonymizationKey(), base::OnceCallback<void()>(), base::OnceCallback<void(const GURL&, int, int)>());
    reporter.Send(url, "application/foobar", kSecondDummyReport, NetworkAnonymizationKey(), base::OnceCallback<void()>(),
...
TEST_F(ReportSenderTest, PendingRequestGetsDeleted)
{
    ...
    network_delegate().set_url_request_destroyed_callback(base::BindRepeating(&MarkURLRequestDestroyed, base::Unretained(&url_request_destroyed)));

    GURL url = URLRequestFailedJob::GetMockHttpUrlWithFailurePhase(URLRequestFailedJob::START, ERR_IO_PENDING);
    network_delegate().set_expect_url(url);
    network_delegate().ExpectReport(kDummyReport);
    network_delegate().set_expected_content_type("application/foobar");
    ...
    auto reporter = std::make_unique<ReportSender>(context(), TRAFFIC_ANNOTATION_FOR_TESTS);
    reporter->Send(
        url, "application/foobar", kDummyReport, NetworkAnonymizationKey(), base::OnceCallback<void()>(), base::OnceCallback<void(const GURL&, int, int)>());
    reporter.reset();
...
TEST_F(ReportSenderTest, ErroredRequestCallsErrorCallback)
{
    ...
    // SendReport will block until the URLRequest is destroyed.
    SendReport(
        &reporter, kDummyReport, url, 0, base::BindOnce(SuccessCallback, &success_callback_called), base::BindOnce(ErrorCallback, &error_callback_called));
...
TEST_F(ReportSenderTest, BadResponseCodeCallsErrorCallback)
{
    ...
    ReportSender reporter(context(), TRAFFIC_ANNOTATION_FOR_TESTS);
    // SendReport will block until the URLRequest is destroyed.
    SendReport(&reporter, kDummyReport, url, 0, base::BindOnce(SuccessCallback, &success_callback_called),
