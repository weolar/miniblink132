...
URLRequestRedirectJob::URLRequestRedirectJob(
    URLRequest* request, const GURL& redirect_destination, RedirectUtil::ResponseCode response_code, const std::string& redirect_reason)
...
void URLRequestRedirectJob::GetLoadTimingInfo(LoadTimingInfo* load_timing_info) const
{
...
{
    request()->net_log().AddEventWithStringParams(NetLogEventType::URL_REQUEST_REDIRECT_JOB, "reason", redirect_reason_);
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(&URLRequestRedirectJob::StartAsync, weak_factory_.GetWeakPtr()));
}
...
void URLRequestRedirectJob::StartAsync()
{
    ...
    fake_headers_ = RedirectUtil::SynthesizeRedirectHeaders(redirect_destination_, response_code_, redirect_reason_, request_headers);

    NetLogResponseHeaders(request()->net_log(), NetLogEventType::URL_REQUEST_FAKE_RESPONSE_HEADERS_CREATED, fake_headers_.get());
    ...
    if (request_headers_callback_) {
        ...
        raw_request_headers.set_request_line(base::StringPrintf("%s %s HTTP/1.1\r\n", request_->method().c_str(), request_->url().PathForRequest().c_str()));
...
void URLRequestRedirectJob::SetRequestHeadersCallback(RequestHeadersCallback callback)
{
