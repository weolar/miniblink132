...
TEST(LocalSetDeclarationTest, Valid_Basic)
{
    ...
    EXPECT_THAT(LocalSetDeclaration(entries, /*aliases=*/ {}).entries(),
        UnorderedElementsAre(Pair(primary, FirstPartySetEntry(primary, SiteType::kPrimary, std::nullopt)),
            Pair(associated, FirstPartySetEntry(primary, SiteType::kAssociated, 0))));
}
...
TEST(LocalSetDeclarationTest, Valid_BasicWithAliases)
{
    ...
    base::flat_map<SchemefulSite, SchemefulSite> aliases({ { primary_cctld, primary }, { associated_cctld, associated } });
    ...
    EXPECT_THAT(local_set.aliases(), UnorderedElementsAre(Pair(associated_cctld, associated), Pair(primary_cctld, primary)));
}
