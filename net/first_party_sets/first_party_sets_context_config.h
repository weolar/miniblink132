...
public:
    ...
    explicit FirstPartySetsContextConfig(base::flat_map<SchemefulSite, FirstPartySetEntryOverride> customizations);
    ...
    std::optional<FirstPartySetEntryOverride> FindOverride(const SchemefulSite& site) const;
    ...
    bool ForEachCustomizationEntry(base::FunctionRef<bool(const SchemefulSite&, const FirstPartySetEntryOverride&)> f) const;

private:
    // mojo (de)serialization needs access to private details.
    friend struct mojo::StructTraits<network::mojom::FirstPartySetsContextConfigDataView, FirstPartySetsContextConfig>;
