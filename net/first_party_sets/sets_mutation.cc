...
SetsMutation::SetsMutation(std::vector<base::flat_map<SchemefulSite, FirstPartySetEntry>> replacement_sets,
    ...
    , additions_(std::move(addition_sets))
{
    ...
    CHECK(base::ranges::all_of(site_counts, [](const std::pair<const SchemefulSite, int>& p) { return p.second == 1; }));
}
