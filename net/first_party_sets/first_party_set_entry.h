...
public:
    ...
    // entry.
    FirstPartySetEntry(SchemefulSite primary, SiteType site_type, std::optional<SiteIndex> site_index);
    FirstPartySetEntry(SchemefulSite primary, SiteType site_type, uint32_t site_index);
    ...
    const std::optional<SiteIndex>& site_index() const
    {
        return site_index_;
    }
...
NET_EXPORT std::ostream& operator<<(std::ostream& os, const FirstPartySetEntry::SiteIndex& site_index);
NET_EXPORT std::ostream& operator<<(std::ostream& os, const FirstPartySetEntry& fpse);
