...
namespace mojo {
template <typename DataViewType, typename T> struct StructTraits;
} // namespace mojo
...
public:
    ...
    // insecure.
    std::optional<FirstPartySetEntry> FindEntry(const SchemefulSite& site, const FirstPartySetsContextConfig& config) const;

    // Batched version of `FindEntry`. Where `FindEntry` would have returned
    // nullopt, this just omits from the result map.
    base::flat_map<SchemefulSite, FirstPartySetEntry> FindEntries(const base::flat_set<SchemefulSite>& sites, const FirstPartySetsContextConfig& config) const;
    ...
    void ApplyManuallySpecifiedSet(const LocalSetDeclaration& local_set_declaration);
    ...
    bool ForEachPublicSetEntry(base::FunctionRef<bool(const SchemefulSite&, const FirstPartySetEntry&)> f) const;
    ...
    // order.
    bool ForEachManualConfigEntry(base::FunctionRef<bool(const SchemefulSite&, const FirstPartySetEntryOverride&)> f) const;
    ...
    // order.
    bool ForEachEffectiveSetEntry(const FirstPartySetsContextConfig& config, base::FunctionRef<bool(const SchemefulSite&, const FirstPartySetEntry&)> f) const;

    // Whether the global sets are empty.
    bool empty() const
    {
        return entries_.empty() && manual_config_.empty();
    }
...
private:
    ...
    friend struct mojo::StructTraits<network::mojom::GlobalFirstPartySetsDataView, GlobalFirstPartySets>;

    friend NET_EXPORT std::ostream& operator<<(std::ostream& os, const GlobalFirstPartySets& sets);

    GlobalFirstPartySets(base::Version public_sets_version, base::flat_map<SchemefulSite, FirstPartySetEntry> entries,
    ...
    // additions.
    base::flat_map<SchemefulSite, FirstPartySetEntry> FindPrimariesAffectedByAdditions(const FlattenedSets& additions) const;
    ...
    std::pair<base::flat_map<SchemefulSite, base::flat_set<SchemefulSite>>, base::flat_set<SchemefulSite>> FindPrimariesAffectedByReplacements(
        const FlattenedSets& replacements, const FlattenedSets& additions,
    ...
    std::vector<base::flat_map<SchemefulSite, FirstPartySetEntry>> NormalizeAdditionSets(
    ...
    // canonical` pair.
    void ForEachAlias(base::FunctionRef<void(const SchemefulSite&, const SchemefulSite&)> f) const;
...
NET_EXPORT std::ostream& operator<<(std::ostream& os, const GlobalFirstPartySets& sets);
