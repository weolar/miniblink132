...
public:
    ...
    ReportingBrowsingDataRemover& operator=(const ReportingBrowsingDataRemover&) = delete;
    ...
    static void RemoveBrowsingData(ReportingCache* cache, uint64_t data_type_mask, const base::RepeatingCallback<bool(const url::Origin&)>& origin_filter);
    ...
    // RemoveBrowsingData.
    static void RemoveAllBrowsingData(ReportingCache* cache, uint64_t data_type_mask);
};
