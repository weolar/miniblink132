...
public:
    ...
    virtual void CanSendReports(std::set<url::Origin> origins, base::OnceCallback<void(std::set<url::Origin>)> result_callback) const = 0;

    // Checks whether |origin| can set |endpoint| to be used for future report
    // deliveries.
    virtual bool CanSetClient(const url::Origin& origin, const GURL& endpoint) const = 0;

    // Checks whether |origin| can use |endpoint| for a report delivery right now.
    virtual bool CanUseClient(const url::Origin& origin, const GURL& endpoint) const = 0;

    static std::unique_ptr<ReportingDelegate> Create(URLRequestContext* request_context);
};
