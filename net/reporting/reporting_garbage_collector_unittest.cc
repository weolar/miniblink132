...
TEST_F(ReportingGarbageCollectorTest, Timer)
{
    ...
    cache()->AddReport(
        std::nullopt, kNak_, kUrl_, kUserAgent_, kGroup_, kType_, base::Value::Dict(), 0, tick_clock()->NowTicks(), 0, ReportingTargetType::kDeveloper);
...
{
    ReportingEndpointGroupKey group_key(kNak_, kReportingSource_, url::Origin::Create(kUrl_), kGroup_, ReportingTargetType::kDeveloper);
    cache()->SetV1EndpointForTesting(group_key, *kReportingSource_, kIsolationInfo_, kUrl_);
