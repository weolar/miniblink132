...
std::unique_ptr<test_server::HttpResponse> AllowPreflight(const test_server::HttpRequest& request)
{
...
std::unique_ptr<test_server::HttpResponse> ReturnResponse(HttpStatusCode code, const test_server::HttpRequest& request)
{
    auto response = std::make_unique<test_server::BasicHttpResponse>();
...
public:
    ...
    {
        return base::BindOnce(&TestUploadCallback::OnUploadComplete, base::Unretained(this));
    }
    ...
    ReportingUploader::Outcome outcome() const
    {
        return outcome_;
    }
...
TEST_F(ReportingUploaderTest, Upload)
{
    ...
    uploader_->StartUpload(kOrigin, server_.GetURL("/"), IsolationInfo::CreateTransient(), kUploadBody, 0, false, callback.callback());
...
TEST_F(ReportingUploaderTest, NetworkError1)
{
    ...
    TestUploadCallback callback;
    uploader_->StartUpload(kOrigin, url, IsolationInfo::CreateTransient(), kUploadBody, 0, false, callback.callback());
    callback.WaitForCall();
...
{
    server_.RegisterRequestHandler(base::BindRepeating(&AllowPreflight));
    server_.RegisterRequestHandler(base::BindRepeating(&ReturnResponse, HTTP_INTERNAL_SERVER_ERROR));
    ASSERT_TRUE(server_.Start());
...
std::unique_ptr<test_server::HttpResponse> VerifyPreflight(bool* preflight_received_out, const test_server::HttpRequest& request)
{
    if (request.method_string != "OPTIONS") {
        return nullptr;
    }
...
TEST_F(ReportingUploaderTest, VerifyPreflight)
{
    ...
    server_.RegisterRequestHandler(base::BindRepeating(&VerifyPreflight, &preflight_received));
    server_.RegisterRequestHandler(base::BindRepeating(&ReturnResponse, HTTP_OK));
    ASSERT_TRUE(server_.Start());
...
TEST_F(ReportingUploaderTest, SkipPreflightForSameOrigin)
{
    ...
    uploader_->StartUpload(server_origin, server_.GetURL("/"), IsolationInfo::CreateTransient(), kUploadBody, 0, false, callback.callback());
    callback.WaitForCall();
...
std::unique_ptr<test_server::HttpResponse> ReturnPreflightError(const test_server::HttpRequest& request)
{
    if (request.method_string != "OPTIONS") {
        return nullptr;
    }
...
std::unique_ptr<test_server::HttpResponse> ReturnPreflightWithoutOrigin(const test_server::HttpRequest& request)
{
    if (request.method_string != "OPTIONS") {
        return nullptr;
    }
...
{
    server_.RegisterRequestHandler(base::BindRepeating(&ReturnPreflightWithoutOrigin));
    server_.RegisterRequestHandler(base::BindRepeating(&ReturnResponse, HTTP_OK));
    ASSERT_TRUE(server_.Start());
...
std::unique_ptr<test_server::HttpResponse> ReturnPreflightWithoutMethods(const test_server::HttpRequest& request)
{
    if (request.method_string != "OPTIONS") {
        return nullptr;
    }
...
{
    server_.RegisterRequestHandler(base::BindRepeating(&ReturnPreflightWithoutMethods));
    server_.RegisterRequestHandler(base::BindRepeating(&ReturnResponse, HTTP_OK));
    ASSERT_TRUE(server_.Start());
...
std::unique_ptr<test_server::HttpResponse> ReturnPreflightWithWildcardMethods(const test_server::HttpRequest& request)
{
    if (request.method_string != "OPTIONS") {
        return nullptr;
    }
...
{
    server_.RegisterRequestHandler(base::BindRepeating(&ReturnPreflightWithWildcardMethods));
    server_.RegisterRequestHandler(base::BindRepeating(&ReturnResponse, HTTP_OK));
    ASSERT_TRUE(server_.Start());
...
std::unique_ptr<test_server::HttpResponse> ReturnPreflightWithoutHeaders(const test_server::HttpRequest& request)
{
    if (request.method_string != "OPTIONS") {
        return nullptr;
    }
...
{
    server_.RegisterRequestHandler(base::BindRepeating(&ReturnPreflightWithoutHeaders));
    server_.RegisterRequestHandler(base::BindRepeating(&ReturnResponse, HTTP_OK));
    ASSERT_TRUE(server_.Start());
...
std::unique_ptr<test_server::HttpResponse> ReturnPreflightWithWildcardHeaders(const test_server::HttpRequest& request)
{
    if (request.method_string != "OPTIONS") {
        return nullptr;
    }
...
{
    server_.RegisterRequestHandler(base::BindRepeating(&ReturnPreflightWithWildcardHeaders));
    server_.RegisterRequestHandler(base::BindRepeating(&ReturnResponse, HTTP_OK));
    ASSERT_TRUE(server_.Start());
...
{
    server_.RegisterRequestHandler(base::BindRepeating(&AllowPreflight));
    server_.RegisterRequestHandler(base::BindRepeating(&ReturnResponse, HTTP_GONE));
    ASSERT_TRUE(server_.Start());
...
std::unique_ptr<test_server::HttpResponse> ReturnRedirect(const std::string& location, const test_server::HttpRequest& request)
{
    ...
    response->set_content("Thank you, Mario! But our Princess is in another castle.");
    response->set_content_type("text/plain");
    return std::move(response);
}
...
TEST_F(ReportingUploaderTest, DontFollowHttpRedirect)
{
    ...
    http_server_.RegisterRequestHandler(base::BindRepeating(&CheckRedirect, &followed));
...
TEST_F(ReportingUploaderTest, DontSendCookies)
{
    ...
    GURL url = server_.GetURL("/");
    auto cookie = CanonicalCookie::CreateForTesting(url, "foo=bar", base::Time::Now());
    context_->cookie_store()->SetCanonicalCookieAsync(std::move(cookie), url, CookieOptions::MakeAllInclusive(), cookie_callback.MakeCallback());
    cookie_callback.WaitUntilDone();
    ASSERT_TRUE(cookie_callback.result().status.IsInclude());

    TestUploadCallback upload_callback;
    uploader_->StartUpload(kOrigin, server_.GetURL("/"), IsolationInfo::CreateTransient(), kUploadBody, 0, false, upload_callback.callback());
    upload_callback.WaitForCall();
}

std::unique_ptr<test_server::HttpResponse> SendCookie(const test_server::HttpRequest& request)
{
    auto response = std::make_unique<test_server::BasicHttpResponse>();
    response->set_code(HTTP_OK);
...
TEST_F(ReportingUploaderTest, DontSaveCookies)
{
    ...
    context_->cookie_store()->GetCookieListWithOptionsAsync(server_.GetURL("/"), CookieOptions::MakeAllInclusive(), CookiePartitionKeyCollection(),
        base::BindOnce(&GetCookieListCallback::Run, base::Unretained(&cookie_callback)));
    cookie_callback.WaitUntilDone();

    EXPECT_TRUE(cookie_callback.cookies().empty());
}

std::unique_ptr<test_server::HttpResponse> ReturnCacheableResponse(int* request_count_out, const test_server::HttpRequest& request)
{
    ++*request_count_out;
    auto response = std::make_unique<test_server::BasicHttpResponse>();
    response->set_code(HTTP_OK);
...
TEST_F(ReportingUploaderTest, DontCacheResponse)
{
    int request_count = 0;
    server_.RegisterRequestHandler(base::BindRepeating(&AllowPreflight));
    server_.RegisterRequestHandler(base::BindRepeating(&ReturnCacheableResponse, &request_count));
    ASSERT_TRUE(server_.Start());
...
TEST_F(ReportingUploaderTest, RespectsNetworkAnonymizationKey)
{
    ...
    const IsolationInfo kIsolationInfo1 = IsolationInfo::Create(net::IsolationInfo::RequestType::kOther, kSiteOrigin1, kSiteOrigin1, net::SiteForCookies());
    const IsolationInfo kIsolationInfo2 = IsolationInfo::Create(net::IsolationInfo::RequestType::kOther, kSiteOrigin2, kSiteOrigin2, net::SiteForCookies());
    ...
    std::unique_ptr<ReportingUploader> uploader1 = ReportingUploader::Create(context.get());
    uploader1->StartUpload(kOrigin, GURL("https://origin/1"), kIsolationInfo1, kUploadBody, 0, false, callback1.callback());
    ...
    std::unique_ptr<ReportingUploader> uploader2 = ReportingUploader::Create(context.get());
    uploader2->StartUpload(kOrigin, GURL("https://origin/2"), kIsolationInfo2, kUploadBody, 0, false, callback2.callback());
    TestUploadCallback callback3;
    std::unique_ptr<ReportingUploader> uploader3 = ReportingUploader::Create(context.get());
    uploader3->StartUpload(kOrigin, GURL("https://origin/3"), kIsolationInfo1, kUploadBody, 0, false, callback3.callback());
