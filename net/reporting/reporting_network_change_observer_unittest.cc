...
protected:
    void SimulateNetworkChange()
    {
        ...
        // recreate NCN.
        NetworkChangeNotifier::NotifyObserversOfNetworkChangeForTests(NetworkChangeNotifier::CONNECTION_NONE);
        base::RunLoop().RunUntilIdle();
        NetworkChangeNotifier::NotifyObserversOfNetworkChangeForTests(NetworkChangeNotifier::CONNECTION_WIFI);
        base::RunLoop().RunUntilIdle();
    }

    void SetEndpoint()
    {
        ASSERT_TRUE(SetEndpointInCache(kGroupKey_, kEndpoint_, base::Time::Now() + base::Days(7)));
    }
    ...
    const ReportingEndpointGroupKey kGroupKey_ = ReportingEndpointGroupKey(NetworkAnonymizationKey(), kOrigin_, kGroup_, ReportingTargetType::kDeveloper);
};
...
TEST_F(ReportingNetworkChangeObserverTest, ClearNothing)
{
    ...
    cache()->AddReport(
        kReportingSource_, kNak_, kUrl_, kUserAgent_, kGroup_, kType_, base::Value::Dict(), 0, tick_clock()->NowTicks(), 0, ReportingTargetType::kDeveloper);
    SetEndpoint();
