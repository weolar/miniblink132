...
protected:
    void RemoveBrowsingData(bool remove_reports, bool remove_clients, std::string host)
    {
        ...
        if (!host.empty()) {
            base::RepeatingCallback<bool(const url::Origin&)> origin_filter = base::BindRepeating(&ReportingBrowsingDataRemoverTest::HostIs, host);
            ReportingBrowsingDataRemover::RemoveBrowsingData(cache(), data_type_mask, origin_filter);
        } else {
            ReportingBrowsingDataRemover::RemoveAllBrowsingData(cache(), data_type_mask);
        }
    }
    ...
    {
        cache()->AddReport(std::nullopt, NetworkAnonymizationKey(), url, kUserAgent_, kGroup_, kType_, base::Value::Dict(), 0, tick_clock()->NowTicks(), 0,
    ...
    {
        SetEndpointInCache(ReportingEndpointGroupKey(NetworkAnonymizationKey(), origin, kGroup_, ReportingTargetType::kDeveloper), kEndpoint_,
...
TEST_F(ReportingBrowsingDataRemoverTest, RemoveSomeClients)
{
    ...
    RemoveBrowsingData(/* remove_reports= */ false, /* remove_clients= */ true,
        /* host= */ kUrl1_.host());
    EXPECT_EQ(2u, report_count());
    EXPECT_FALSE(FindEndpointInCache(ReportingEndpointGroupKey(NetworkAnonymizationKey(), kOrigin1_, kGroup_, ReportingTargetType::kDeveloper), kEndpoint_));
    EXPECT_TRUE(FindEndpointInCache(ReportingEndpointGroupKey(NetworkAnonymizationKey(), kOrigin2_, kGroup_, ReportingTargetType::kDeveloper), kEndpoint_));
}
