...
public:
    ...
    static std::unique_ptr<ReportingEndpointManager> Create(const ReportingPolicy* policy, const base::TickClock* tick_clock, const ReportingDelegate* delegate,
        ReportingCache* cache, const RandIntCallback& rand_callback);
    ...
    virtual const ReportingEndpoint FindEndpointForDelivery(const ReportingEndpointGroupKey& group_key) = 0;
    ...
    virtual void InformOfEndpointRequest(const NetworkAnonymizationKey& network_anonymization_key, const GURL& endpoint, bool succeeded) = 0;
};
