...
public:
    ...
    TestReportingCache(const url::Origin& expected_origin, const std::string& expected_group)
        : expected_origin_(expected_origin)
    ...
    {
        reporting_endpoints_[reporting_endpoint.group_key.network_anonymization_key].push_back(reporting_endpoint);
    }

    // ReportingCache implementation:

    std::vector<ReportingEndpoint> GetCandidateEndpointsForDelivery(const ReportingEndpointGroupKey& group_key) override
    {
    ...
    void AddReport(const std::optional<base::UnguessableToken>& reporting_source, const NetworkAnonymizationKey& network_anonymization_key, const GURL& url,
        const std::string& user_agent, const std::string& group_name, const std::string& type, base::Value::Dict body, int depth, base::TimeTicks queued,
        int attempts, ReportingTargetType target_type) override
    {
        NOTREACHED();
    }
    void GetReports(std::vector<raw_ptr<const ReportingReport, VectorExperimental>>* reports_out) const override
    {
        NOTREACHED();
    }
    ...
    std::vector<raw_ptr<const ReportingReport, VectorExperimental>> GetReportsToDeliver() override
    {
        NOTREACHED();
    }
    std::vector<raw_ptr<const ReportingReport, VectorExperimental>> GetReportsToDeliverForSource(const base::UnguessableToken& reporting_source) override
    {
        NOTREACHED();
    }
    void ClearReportsPending(const std::vector<raw_ptr<const ReportingReport, VectorExperimental>>& reports) override
    {
        NOTREACHED();
    }
    void IncrementReportsAttempts(const std::vector<raw_ptr<const ReportingReport, VectorExperimental>>& reports) override
    {
        NOTREACHED();
    }
    base::flat_map<url::Origin, std::vector<ReportingEndpoint>> GetV1ReportingEndpointsByOrigin() const override
    {
        NOTREACHED();
    }
    void IncrementEndpointDeliveries(const ReportingEndpointGroupKey& group_key, const GURL& url, int reports_delivered, bool successful) override
    {
        NOTREACHED();
    }
    void SetExpiredSource(const base::UnguessableToken& reporting_source) override
    {
        NOTREACHED();
    }
    const base::flat_set<base::UnguessableToken>& GetExpiredSources() const override
    {
        NOTREACHED();
    }
    void RemoveReports(const std::vector<raw_ptr<const ReportingReport, VectorExperimental>>& reports) override
    {
        NOTREACHED();
    }
    void RemoveReports(const std::vector<raw_ptr<const ReportingReport, VectorExperimental>>& reports, bool delivery_success) override
    {
        NOTREACHED();
    }
    ...
    size_t GetReportCountWithStatusForTesting(ReportingReport::Status status) const override
    {
        NOTREACHED();
    }
    ...
    void OnParsedHeader(
        const NetworkAnonymizationKey& network_anonymization_key, const url::Origin& origin, std::vector<ReportingEndpointGroup> parsed_header) override
    {
        NOTREACHED();
    }
    void OnParsedReportingEndpointsHeader(
        const base::UnguessableToken& reporting_source, const IsolationInfo& isolation_info, std::vector<ReportingEndpoint> endpoints) override
    {
        NOTREACHED();
    }
    ...
    void RemoveClient(const NetworkAnonymizationKey& network_anonymization_key, const url::Origin& origin) override
    {
        NOTREACHED();
    }
    ...
    void RemoveSourceAndEndpoints(const base::UnguessableToken& reporting_source) override
    {
        NOTREACHED();
    }
    void AddClientsLoadedFromStore(std::vector<ReportingEndpoint> loaded_endpoints, std::vector<CachedReportingEndpointGroup> loaded_endpoint_groups) override
    {
        NOTREACHED();
    }
    ...
    ReportingEndpoint GetV1EndpointForTesting(const base::UnguessableToken& reporting_source, const std::string& endpoint_name) const override
    {
        NOTREACHED();
    }
    ReportingEndpoint GetEndpointForTesting(const ReportingEndpointGroupKey& group_key, const GURL& url) const override
    {
        NOTREACHED();
    }
    std::vector<ReportingEndpoint> GetEnterpriseEndpointsForTesting() const override
    {
        NOTREACHED();
    }
    bool EndpointGroupExistsForTesting(const ReportingEndpointGroupKey& group_key, OriginSubdomains include_subdomains, base::Time expires) const override
    {
        NOTREACHED();
    }
    bool ClientExistsForTesting(const NetworkAnonymizationKey& network_anonymization_key, const url::Origin& origin) const override
    {
        NOTREACHED();
    }
    ...
    void SetEndpointForTesting(
        const ReportingEndpointGroupKey& group_key, const GURL& url, OriginSubdomains include_subdomains, base::Time expires, int priority, int weight) override
    {
        NOTREACHED();
    }
    void SetV1EndpointForTesting(const ReportingEndpointGroupKey& group_key, const base::UnguessableToken& reporting_source,
        const IsolationInfo& isolation_info, const GURL& url) override
    {
        NOTREACHED();
    }
    void SetEnterpriseEndpointForTesting(const ReportingEndpointGroupKey& group_key, const GURL& url) override
    {
        NOTREACHED();
    }
    IsolationInfo GetIsolationInfoForEndpoint(const ReportingEndpoint& endpoint) const override
    {
        NOTREACHED();
    }
...
private:
    ...
    std::map<NetworkAnonymizationKey, std::vector<ReportingEndpoint>> reporting_endpoints_;
...
public:
        ...
        : cache_(kOrigin, kGroup)
    {
        ...
        endpoint_manager_ = ReportingEndpointManager::Create(&policy_, &clock_, &delegate_, &cache_, TestReportingRandIntCallback());
    }

protected:
    void SetEndpoint(const GURL& endpoint, int priority = ReportingEndpoint::EndpointInfo::kDefaultPriority,
        int weight = ReportingEndpoint::EndpointInfo::kDefaultWeight, const NetworkAnonymizationKey& network_anonymization_key = NetworkAnonymizationKey())
    {
        ReportingEndpointGroupKey group_key(kGroupKey);
        group_key.network_anonymization_key = network_anonymization_key;
        cache_.SetEndpoint(ReportingEndpoint(group_key, ReportingEndpoint::EndpointInfo { endpoint, priority, weight }));
    }

    void SetEnterpriseEndpoint(const GURL& endpoint, int priority = ReportingEndpoint::EndpointInfo::kDefaultPriority,
        int weight = ReportingEndpoint::EndpointInfo::kDefaultWeight, const NetworkAnonymizationKey& network_anonymization_key = NetworkAnonymizationKey())
    {
    ...
    const ReportingEndpointGroupKey kGroupKey = ReportingEndpointGroupKey(kNak, kOrigin, kGroup, ReportingTargetType::kDeveloper);
    const ReportingEndpointGroupKey kEnterpriseGroupKey = ReportingEndpointGroupKey(kNak,
        /*origin=*/std::nullopt, kGroup, ReportingTargetType::kEnterprise);
...
{
    ReportingEndpoint endpoint = endpoint_manager_->FindEndpointForDelivery(kGroupKey);
...
TEST_F(ReportingEndpointManagerTest, EnterpriseEndpoint)
{
    ...
    ReportingEndpoint endpoint = endpoint_manager_->FindEndpointForDelivery(kEnterpriseGroupKey);
    ASSERT_TRUE(endpoint);
    EXPECT_EQ(kEndpoint, endpoint.info.url);
...
TEST_F(ReportingEndpointManagerTest, BackedOffEndpoint)
{
    ...
    base::TimeDelta initial_delay = base::Milliseconds(policy_.endpoint_backoff_policy.initial_delay_ms);

    SetEndpoint(kEndpoint);

    endpoint_manager_->InformOfEndpointRequest(NetworkAnonymizationKey(), kEndpoint, false);
    ...
    ReportingEndpoint endpoint2 = endpoint_manager_->FindEndpointForDelivery(kGroupKey);
    ...
    ReportingEndpoint endpoint3 = endpoint_manager_->FindEndpointForDelivery(kGroupKey);
    EXPECT_FALSE(endpoint3);

    clock_.Advance(initial_delay);

    // Next backoff is longer -- 2x the first -- so endpoint isn't usable yet.
    ReportingEndpoint endpoint4 = endpoint_manager_->FindEndpointForDelivery(kGroupKey);
    EXPECT_FALSE(endpoint4);

    clock_.Advance(initial_delay);

    // After 2x the initial delay, the endpoint is usable again.
    ReportingEndpoint endpoint5 = endpoint_manager_->FindEndpointForDelivery(kGroupKey);
    ASSERT_TRUE(endpoint5);
    EXPECT_EQ(kEndpoint, endpoint5.info.url);

    endpoint_manager_->InformOfEndpointRequest(NetworkAnonymizationKey(), kEndpoint, true);
    endpoint_manager_->InformOfEndpointRequest(NetworkAnonymizationKey(), kEndpoint, true);
    ...
    ReportingEndpoint endpoint6 = endpoint_manager_->FindEndpointForDelivery(kGroupKey);
    EXPECT_FALSE(endpoint6);

    clock_.Advance(initial_delay);

    ReportingEndpoint endpoint7 = endpoint_manager_->FindEndpointForDelivery(kGroupKey);
...
TEST_F(ReportingEndpointManagerTest, RandomEndpoint)
{
    ...
    for (int i = 0; i < kMaxAttempts; ++i) {
        ReportingEndpoint endpoint = endpoint_manager_->FindEndpointForDelivery(kGroupKey);
        ASSERT_TRUE(endpoint);
        ASSERT_TRUE(endpoint.info.url == kEndpoint1 || endpoint.info.url == kEndpoint2);
...
TEST_F(ReportingEndpointManagerTest, Priority)
{
    ...
    SetEndpoint(kPrimaryEndpoint, 10 /* priority */, ReportingEndpoint::EndpointInfo::kDefaultWeight);
    ...
    endpoint_manager_->InformOfEndpointRequest(NetworkAnonymizationKey(), kPrimaryEndpoint, false);
    ReportingEndpoint endpoint2 = endpoint_manager_->FindEndpointForDelivery(kGroupKey);
    ASSERT_TRUE(endpoint2);
...
TEST_F(ReportingEndpointManagerTest, Weight)
{
    ...
    SetEndpoint(kEndpoint1, ReportingEndpoint::EndpointInfo::kDefaultPriority, kEndpoint1Weight);
    SetEndpoint(kEndpoint2, ReportingEndpoint::EndpointInfo::kDefaultPriority, kEndpoint2Weight);
...
TEST_F(ReportingEndpointManagerTest, ZeroWeights)
{
    ...
    SetEndpoint(kEndpoint1, ReportingEndpoint::EndpointInfo::kDefaultPriority, 0 /* weight */);
    SetEndpoint(kEndpoint2, ReportingEndpoint::EndpointInfo::kDefaultPriority, 0 /* weight */);
...
TEST_F(ReportingEndpointManagerTest, NetworkAnonymizationKey)
{
    ...
    const auto kNetworkAnonymizationKey1 = NetworkAnonymizationKey::CreateSameSite(kSite);
    const auto kNetworkAnonymizationKey2 = NetworkAnonymizationKey::CreateSameSite(kSite2);
    const ReportingEndpointGroupKey kGroupKey1(kNetworkAnonymizationKey1, kOrigin, kGroup, ReportingTargetType::kDeveloper);
    const ReportingEndpointGroupKey kGroupKey2(kNetworkAnonymizationKey2, kOrigin, kGroup, ReportingTargetType::kDeveloper);

    // An Endpoint set for kNetworkAnonymizationKey1 should not affect
    // kNetworkAnonymizationKey2.
    SetEndpoint(kEndpoint, ReportingEndpoint::EndpointInfo::kDefaultPriority, 0 /* weight */, kNetworkAnonymizationKey1);
    ReportingEndpoint endpoint = endpoint_manager_->FindEndpointForDelivery(kGroupKey1);
    ASSERT_TRUE(endpoint);
    EXPECT_EQ(kEndpoint, endpoint.info.url);
    ...
    SetEndpoint(kEndpoint, ReportingEndpoint::EndpointInfo::kDefaultPriority, 0 /* weight */, kNetworkAnonymizationKey2);
    ...
    endpoint_manager_->InformOfEndpointRequest(kNetworkAnonymizationKey1, kEndpoint, false);
...
TEST_F(ReportingEndpointManagerTest, NetworkAnonymizationKeyWithMultipleEndpoints)
{
    const SchemefulSite kSite2(GURL("https://origin2/"));
    ...
    // kNetworkAnonymizationKey2.
    SetEndpoint(kEndpoint1, ReportingEndpoint::EndpointInfo::kDefaultPriority, ReportingEndpoint::EndpointInfo::kDefaultWeight, kNetworkAnonymizationKey1);
    SetEndpoint(kEndpoint2, ReportingEndpoint::EndpointInfo::kDefaultPriority, ReportingEndpoint::EndpointInfo::kDefaultWeight, kNetworkAnonymizationKey1);
    SetEndpoint(kEndpoint3, ReportingEndpoint::EndpointInfo::kDefaultPriority, ReportingEndpoint::EndpointInfo::kDefaultWeight, kNetworkAnonymizationKey2);
    ...
    ReportingEndpoint endpoint = endpoint_manager_->FindEndpointForDelivery(kGroupKey2);
    ASSERT_TRUE(endpoint);
...
TEST_F(ReportingEndpointManagerTest, CacheEviction)
{
    ...
    for (int i = 0; i < ReportingEndpointManager::kMaxEndpointBackoffCacheSize; ++i) {
    ...
    for (int i = 0; i < ReportingEndpointManager::kMaxEndpointBackoffCacheSize; ++i) {
        ...
        endpoint_manager_->InformOfEndpointRequest(NetworkAnonymizationKey(), endpoint.info.url, false);
    }
    // All endpoints should now be marked as bad.
    EXPECT_FALSE(endpoint_manager_->FindEndpointForDelivery(kGroupKey));

    // Add another endpoint with a different NetworkAnonymizationKey;
    const auto kDifferentNetworkAnonymizationKey = NetworkAnonymizationKey::CreateSameSite(kSite);
    const ReportingEndpointGroupKey kDifferentGroupKey(kDifferentNetworkAnonymizationKey, kOrigin, kGroup, ReportingTargetType::kDeveloper);
    SetEndpoint(
        kEndpoint, ReportingEndpoint::EndpointInfo::kDefaultPriority, ReportingEndpoint::EndpointInfo::kDefaultWeight, kDifferentNetworkAnonymizationKey);
    ...
    endpoint_manager_->InformOfEndpointRequest(kDifferentNetworkAnonymizationKey, kEndpoint, false);
    ...
    endpoint_manager_->InformOfEndpointRequest(NetworkAnonymizationKey(), endpoint.info.url, true);
