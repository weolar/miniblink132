...
public:
    ...
    static std::unique_ptr<ReportingContext> Create(const ReportingPolicy& policy, URLRequestContext* request_context,
    ...
    void NotifyEndpointsUpdatedForOrigin(const std::vector<ReportingEndpoint>& endpoints);
...
protected:
    ReportingContext(const ReportingPolicy& policy, base::Clock* clock, const base::TickClock* tick_clock, const RandIntCallback& rand_callback,
...
private:
    ...
    base::ObserverList<ReportingCacheObserver, /* check_empty= */ true>::Unchecked cache_observers_;
