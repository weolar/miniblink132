...
class ReportingServiceTest : public ::testing::TestWithParam<bool>, public WithTaskEnvironment {
protected:
    ...
    const NetworkAnonymizationKey kNak2_ = NetworkAnonymizationKey::CreateSameSite(SchemefulSite(kOrigin2_));
    ...
    const ReportingEndpointGroupKey kGroupKey2_ = ReportingEndpointGroupKey(kNak2_, kOrigin2_, kGroup_, ReportingTargetType::kDeveloper);
    ...
    void Init()
    {
        ...
        auto test_context = std::make_unique<TestReportingContext>(&clock_, &tick_clock_, ReportingPolicy(), store_.get());
...
{
    service()->QueueReport(kUrl_, kReportingSource_, kNak_, kUserAgent_, kGroup_, kType_, base::Value::Dict(), 0, ReportingTargetType::kDeveloper);
...
{
    service()->QueueReport(kUrl_, kReportingSource_, kNak_, kUserAgent_, kGroup_, kType_, base::Value::Dict(), 0, ReportingTargetType::kEnterprise);
    FinishLoading(true /* load_success */);
...
TEST_P(ReportingServiceTest, QueueReportSanitizeUrl)
{
    ...
    service()->QueueReport(url, kReportingSource_, kNak_, kUserAgent_, kGroup_, kType_, base::Value::Dict(), 0, ReportingTargetType::kDeveloper);
    FinishLoading(true /* load_success */);
...
TEST_P(ReportingServiceTest, ProcessReportToHeader)
{
    ...
    EXPECT_TRUE(context()->cache()->GetEndpointForTesting(ReportingEndpointGroupKey(kNak_, kOrigin_, kGroup_, ReportingTargetType::kDeveloper), kEndpoint_));
}
...
TEST_P(ReportingServiceTest, ProcessReportingEndpointsHeader)
{
    ...
    auto parsed_header = ParseReportingEndpoints(kGroup_ + "=\"" + kEndpoint_.spec() + "\"");
    ASSERT_TRUE(parsed_header.has_value());
    service()->SetDocumentReportingEndpoints(*kReportingSource_, kOrigin_, kIsolationInfo_, *parsed_header);
    FinishLoading(true /* load_success */);
    ...
    ReportingEndpoint cached_endpoint = context()->cache()->GetV1EndpointForTesting(*kReportingSource_, kGroup_);
...
TEST_P(ReportingServiceTest, ProcessReportingEndpointsHeaderNetworkIsolationKeyDisabled)
{
    base::test::ScopedFeatureList feature_list;
    feature_list.InitWithFeatures({ net::features::kDocumentReporting }, { features::kPartitionConnectionsByNetworkIsolationKey });
...
TEST_P(ReportingServiceTest, SendReportsAndRemoveSource)
{
    base::test::ScopedFeatureList feature_list;
    feature_list.InitAndEnableFeature(net::features::kDocumentReporting);
    auto parsed_header = ParseReportingEndpoints(kGroup_ + "=\"" + kEndpoint_.spec() + "\", " + kGroup2_ + "=\"" + kEndpoint2_.spec() + "\"");
    ASSERT_TRUE(parsed_header.has_value());
    service()->SetDocumentReportingEndpoints(*kReportingSource_, kOrigin_, kIsolationInfo_, *parsed_header);
    ...
    context()->cache()->GetReports(&reports);
    ASSERT_EQ(1u, reports.size());
    EXPECT_EQ(0u, context()->cache()->GetReportCountWithStatusForTesting(ReportingReport::Status::QUEUED));
    ...
    // still be pending.
    EXPECT_EQ(0u, context()->cache()->GetReportCountWithStatusForTesting(ReportingReport::Status::QUEUED));
    EXPECT_EQ(1u, context()->cache()->GetReportCountWithStatusForTesting(ReportingReport::Status::PENDING));
    // Source should be marked as expired.
    ASSERT_TRUE(context()->cache()->GetExpiredSources().contains(*kReportingSource_));
}
...
#else
#define MAYBE_SendReportsAndRemoveSourceWithPendingReports SendReportsAndRemoveSourceWithPendingReports
#endif
TEST_P(ReportingServiceTest, MAYBE_SendReportsAndRemoveSourceWithPendingReports)
{
    base::test::ScopedFeatureList feature_list;
    feature_list.InitAndEnableFeature(net::features::kDocumentReporting);
    auto parsed_header = ParseReportingEndpoints(kGroup_ + "=\"" + kEndpoint_.spec() + "\", " + kGroup2_ + "=\"" + kEndpoint2_.spec() + "\"");
    ...
    service()->QueueReport(kUrl_, kReportingSource_, kNak_, kUserAgent_, kGroup_, kType_, base::Value::Dict(), 0, ReportingTargetType::kDeveloper);
    EXPECT_EQ(1u, context()->cache()->GetReportCountWithStatusForTesting(ReportingReport::Status::QUEUED));
    EXPECT_EQ(1u, context()->cache()->GetReportCountWithStatusForTesting(ReportingReport::Status::PENDING));
...
TEST_P(ReportingServiceTest, ProcessReportingEndpointsHeaderPathAbsolute)
{
    ...
    EXPECT_EQ(0u, context()->cache()->GetEndpointCount());
    // Endpoint should be associated with the reporting source.
    ReportingEndpoint endpoint = context()->cache()->GetV1EndpointForTesting(*kReportingSource_, kGroup_);
...
{
    const std::string header_too_deep = "{\"endpoints\":[{\"url\":\"" + kEndpoint_.spec()
        + "\"}],"
          "\"group\":\""
        + kGroup_
        + "\","
...
TEST_P(ReportingServiceTest, ProcessReportToHeaderNetworkIsolationKeyDisabled)
{
    ...
    EXPECT_EQ(1u, context()->cache()->GetEndpointCount());
    EXPECT_FALSE(context()->cache()->GetEndpointForTesting(ReportingEndpointGroupKey(kNak_, kOrigin_, kGroup_, ReportingTargetType::kDeveloper), kEndpoint_));
    EXPECT_TRUE(context()->cache()->GetEndpointForTesting(
        ReportingEndpointGroupKey(NetworkAnonymizationKey(), kOrigin_, kGroup_, ReportingTargetType::kDeveloper), kEndpoint_));
}
...
TEST_P(ReportingServiceTest, WriteToStore)
{
    ...
    EXPECT_THAT(store()->GetAllCommands(), testing::UnorderedElementsAreArray(expected_commands));

    // Unblock the load. The will let the remaining calls to the service complete
    // without blocking.
    FinishLoading(true /* load_success */);
    expected_commands.emplace_back(CommandType::ADD_REPORTING_ENDPOINT, kGroupKey_, kEndpoint_);
    expected_commands.emplace_back(CommandType::ADD_REPORTING_ENDPOINT_GROUP, kGroupKey_);
    EXPECT_THAT(store()->GetAllCommands(), testing::UnorderedElementsAreArray(expected_commands));
    ...
    service()->ProcessReportToHeader(kOrigin2_, kNak2_,
        "{\"endpoints\":[{\"url\":\"" + kEndpoint_.spec()
            ...
            + "\","
              "\"max_age\":86400}");
    expected_commands.emplace_back(CommandType::ADD_REPORTING_ENDPOINT, kGroupKey2_, kEndpoint_);
    expected_commands.emplace_back(CommandType::ADD_REPORTING_ENDPOINT_GROUP, kGroupKey2_);
    EXPECT_THAT(store()->GetAllCommands(), testing::UnorderedElementsAreArray(expected_commands));

    service()->QueueReport(kUrl_, kReportingSource_, kNak_, kUserAgent_, kGroup_, kType_, base::Value::Dict(), 0, ReportingTargetType::kDeveloper);
    expected_commands.emplace_back(CommandType::UPDATE_REPORTING_ENDPOINT_GROUP_ACCESS_TIME, kGroupKey_);
    EXPECT_THAT(store()->GetAllCommands(), testing::UnorderedElementsAreArray(expected_commands));

    service()->RemoveBrowsingData(
        ReportingBrowsingDataRemover::DATA_TYPE_CLIENTS, base::BindRepeating([](const url::Origin& origin) { return origin.host() == "origin"; }));
    expected_commands.emplace_back(CommandType::DELETE_REPORTING_ENDPOINT, kGroupKey_, kEndpoint_);
    expected_commands.emplace_back(CommandType::DELETE_REPORTING_ENDPOINT_GROUP, kGroupKey_);
    expected_commands.emplace_back(CommandType::FLUSH);
    EXPECT_THAT(store()->GetAllCommands(), testing::UnorderedElementsAreArray(expected_commands));

    service()->RemoveAllBrowsingData(ReportingBrowsingDataRemover::DATA_TYPE_CLIENTS);
    expected_commands.emplace_back(CommandType::DELETE_REPORTING_ENDPOINT, kGroupKey2_, kEndpoint_);
    expected_commands.emplace_back(CommandType::DELETE_REPORTING_ENDPOINT_GROUP, kGroupKey2_);
    expected_commands.emplace_back(CommandType::FLUSH);
    EXPECT_THAT(store()->GetAllCommands(), testing::UnorderedElementsAreArray(expected_commands));
}
...
TEST_P(ReportingServiceTest, SetEnterpriseReportingEndpointsWithFeatureEnabled)
{
    base::test::ScopedFeatureList feature_list;
    ...
    std::vector<ReportingEndpoint> expected_enterprise_endpoints = { { ReportingEndpointGroupKey(NetworkAnonymizationKey(),
    ...
    EXPECT_EQ(expected_enterprise_endpoints, context()->cache()->GetEnterpriseEndpointsForTesting());
}

TEST_P(ReportingServiceTest, SetEnterpriseReportingEndpointsWithFeatureDisabled)
{
    EXPECT_EQ(0u, context()->cache()->GetEnterpriseEndpointsForTesting().size());
    base::flat_map<std::string, GURL> test_enterprise_endpoints {
        { "endpoint-1", GURL("https://example.com/reports") },
...
TEST_P(ReportingServiceTest, ReportingServiceConstructionWithFeatureEnabled)
{
    ...
    EXPECT_EQ(0u, service()->GetContextForTesting()->cache()->GetEnterpriseEndpointsForTesting().size());
    std::unique_ptr<URLRequestContext> url_request_context = CreateTestURLRequestContextBuilder()->Build();
    std::unique_ptr<ReportingService> reporting_service_ptr
        = ReportingService::Create(ReportingPolicy(), url_request_context.get(), store(), test_enterprise_endpoints);
    ...
    EXPECT_EQ(expected_enterprise_endpoints, reporting_service_ptr->GetContextForTesting()->cache()->GetEnterpriseEndpointsForTesting());
}

TEST_P(ReportingServiceTest, ReportingServiceConstructionWithFeatureDisabled)
{
    base::test::ScopedFeatureList feature_list;
    feature_list.InitAndDisableFeature(net::features::kReportingApiEnableEnterpriseCookieIssues);
    base::flat_map<std::string, GURL> test_enterprise_endpoints {
        { "endpoint-1", GURL("https://example.com/reports") },
        { "endpoint-2", GURL("https://reporting.example/cookie-issues") },
        { "endpoint-3", GURL("https://report-collector.example") },
    ...
    EXPECT_EQ(0u, reporting_service_ptr->GetContextForTesting()->cache()->GetEnterpriseEndpointsForTesting().size());
}

INSTANTIATE_TEST_SUITE_P(ReportingServiceStoreTest, ReportingServiceTest, ::testing::Bool());
} // namespace
} // namespace net
