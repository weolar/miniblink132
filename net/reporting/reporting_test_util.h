...
public:
    ...
    void StartUpload(const url::Origin& report_origin, const GURL& url, const IsolationInfo& isolation_info, const std::string& json, int max_depth,
        bool eligible_for_credentials, UploadCallback callback) override;
...
public:
    ...
    void CanSendReports(std::set<url::Origin> origins, base::OnceCallback<void(std::set<url::Origin>)> result_callback) const override;

    bool PermissionsCheckPaused() const;
    void ResumePermissionsCheck();

    bool CanSetClient(const url::Origin& origin, const GURL& endpoint) const override;

    bool CanUseClient(const url::Origin& origin, const GURL& endpoint) const override;
...
private:
    ...
    mutable base::OnceCallback<void(std::set<url::Origin>)> permissions_check_callback_;
};
...
public:
    TestReportingContext(base::Clock* clock, const base::TickClock* tick_clock, const ReportingPolicy& policy,
        ReportingCache::PersistentReportingStore* store = nullptr, const base::flat_map<std::string, GURL>& enterprise_reporting_endpoints = {});
    ...
    base::MockOneShotTimer* test_delivery_timer()
    {
        return delivery_timer_;
    }
...
protected:
    ...
    void UseStore(std::unique_ptr<ReportingCache::PersistentReportingStore> store);

    // Finds a particular endpoint in the cache and returns it (or an invalid
    // ReportingEndpoint, if not found).
    const ReportingEndpoint FindEndpointInCache(const ReportingEndpointGroupKey& group_key, const GURL& url);
    ...
    bool SetEndpointInCache(const ReportingEndpointGroupKey& group_key, const GURL& url, base::Time expires,
        OriginSubdomains include_subdomains = OriginSubdomains::DEFAULT, int priority = ReportingEndpoint::EndpointInfo::kDefaultPriority,
    ...
    void SetEnterpriseEndpointInCache(const ReportingEndpointGroupKey& group_key, const GURL& url);

    // Returns whether an endpoint with the given properties exists in the cache.
    bool EndpointExistsInCache(const ReportingEndpointGroupKey& group_key, const GURL& url);

    // Gets the statistics for a given endpoint, if it exists.
    ReportingEndpoint::Statistics GetEndpointStatistics(const ReportingEndpointGroupKey& group_key, const GURL& url);
    ...
    // checked.
    bool EndpointGroupExistsInCache(const ReportingEndpointGroupKey& group_key, OriginSubdomains include_subdomains, base::Time expires = base::Time());
    ...
    TestReportingUploader* uploader()
    {
        return context_->test_uploader();
    }
    ...
    const std::vector<std::unique_ptr<TestReportingUploader::PendingUpload>>& pending_uploads()
    {
        return uploader()->pending_uploads();
    }

private:
    void CreateContext(const ReportingPolicy& policy, base::Time now, base::TimeTicks now_ticks);
...
public:
    struct Report {
        ...
        Report(const GURL& url, const NetworkAnonymizationKey& network_anonymization_key, const std::string& user_agent, const std::string& group,
            const std::string& type, std::unique_ptr<const base::Value> body, int depth);
    ...
    void ProcessReportToHeader(const url::Origin& url, const NetworkAnonymizationKey& network_anonymization_key, const std::string& header_value) override;
