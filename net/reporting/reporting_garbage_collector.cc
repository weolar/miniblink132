...
class ReportingGarbageCollectorImpl : public ReportingGarbageCollector, public ReportingCacheObserver {
public:
    ...
    void OnEndpointsUpdatedForOrigin(const std::vector<ReportingEndpoint>& endpoints) override
    {
        EnsureTimerIsRunning();
    }
...
private:
    ...
    void CollectGarbage()
    {
        ...
        base::flat_set<base::UnguessableToken> sources_to_remove = context_->cache()->GetExpiredSources();

        std::vector<raw_ptr<const ReportingReport, VectorExperimental>> all_reports;
        context_->cache()->GetReports(&all_reports);

        std::vector<raw_ptr<const ReportingReport, VectorExperimental>> failed_reports;
        std::vector<raw_ptr<const ReportingReport, VectorExperimental>> expired_reports;
    ...
    void EnsureTimerIsRunning()
    {
        ...
        timer_->Start(
            FROM_HERE, context_->policy().garbage_collection_interval, base::BindOnce(&ReportingGarbageCollectorImpl::CollectGarbage, base::Unretained(this)));
    }
...
// static
std::unique_ptr<ReportingGarbageCollector> ReportingGarbageCollector::Create(ReportingContext* context)
{
