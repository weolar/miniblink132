...
public:
    explicit ReportingCacheImpl(ReportingContext* context, const base::flat_map<std::string, GURL>& enterprise_reporting_endpoints);
    ...
    void OnParsedReportingEndpointsHeader(
        const base::UnguessableToken& reporting_source, const IsolationInfo& isolation_info, std::vector<ReportingEndpoint> parsed_header) override;
    ...
    void SetEndpointForTesting(const ReportingEndpointGroupKey& group_key, const GURL& url, OriginSubdomains include_subdomains, base::Time expires,
        int priority, int weight) override;
...
private:
    ...
    struct Client {
        Client(const NetworkAnonymizationKey& network_anonymization_key, const url::Origin& origin);
    ...
    using ReportSet = base::flat_set<std::unique_ptr<ReportingReport>, base::UniquePtrComparator>;
    using ClientMap = std::multimap<std::string, Client>;
    using EndpointGroupMap = std::map<ReportingEndpointGroupKey, CachedReportingEndpointGroup>;
    using EndpointMap = std::multimap<ReportingEndpointGroupKey, ReportingEndpoint>;
    ...
    size_t ConsistencyCheckClient(const std::string& domain, const Client& client) const;

    // Returns the number of endpoints found in |group|.
    size_t ConsistencyCheckEndpointGroup(const ReportingEndpointGroupKey& key, const CachedReportingEndpointGroup& group) const;

    void ConsistencyCheckEndpoint(const ReportingEndpointGroupKey& key, const ReportingEndpoint& endpoint, EndpointMap::const_iterator endpoint_it) const;
#endif // DCHECK_IS_ON()
    ...
    ClientMap::iterator FindClientIt(const NetworkAnonymizationKey& network_anonymization_key, const url::Origin& origin);
    ...
    EndpointGroupMap::iterator FindEndpointGroupIt(const ReportingEndpointGroupKey& group_key);
    ...
    // found.
    EndpointMap::iterator FindEndpointIt(const ReportingEndpointGroupKey& group_key, const GURL& url);
    ...
    void RemoveEndpointsInGroupOtherThan(const ReportingEndpointGroupKey& group_key, const std::set<GURL>& endpoints_to_keep_urls);
    ...
    std::vector<ReportingEndpoint> GetEndpointsInGroup(const ReportingEndpointGroupKey& group_key) const;

    // Gets the number of endpoints for the given origin and group.
    size_t GetEndpointCountInGroup(const ReportingEndpointGroupKey& group_key) const;

    // Updates the last_used time for the given origin and endpoint group.
    void MarkEndpointGroupAndClientUsed(ClientMap::iterator client_it, EndpointGroupMap::iterator group_it, base::Time now);
    ...
    std::optional<EndpointGroupMap::iterator> RemoveEndpointGroupInternal(
        ClientMap::iterator client_it, EndpointGroupMap::iterator group_it, size_t* num_endpoints_removed = nullptr);
    ...
    void EvictEndpointsFromClient(ClientMap::iterator client_it, size_t endpoints_to_evict);
    ...
    void EvictEndpointFromGroup(ClientMap::iterator client_it, EndpointGroupMap::iterator group_it);
    ...
    bool RemoveExpiredOrStaleGroups(ClientMap::iterator client_it, size_t* num_endpoints_removed);
    ...
    ReportingEndpoint::Statistics* GetEndpointStats(const ReportingEndpointGroupKey& group_key, const GURL& url);

    // Helper methods for GetClientsAsValue().
    base::Value GetClientAsValue(const Client& client) const;
    base::Value GetEndpointGroupAsValue(const CachedReportingEndpointGroup& group) const;
    ...
    std::map<base::UnguessableToken, std::vector<ReportingEndpoint>> document_endpoints_;
