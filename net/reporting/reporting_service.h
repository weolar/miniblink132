...
public:
    ...
    static std::unique_ptr<ReportingService> Create(const ReportingPolicy& policy, URLRequestContext* request_context,
    ...
    // destroyed.
    static std::unique_ptr<ReportingService> CreateForTesting(std::unique_ptr<ReportingContext> reporting_context);
    ...
    // will be copied.
    virtual void QueueReport(const GURL& url, const std::optional<base::UnguessableToken>& reporting_source,
    ...
    // header.
    virtual void ProcessReportToHeader(const url::Origin& origin, const NetworkAnonymizationKey& network_anonymization_key, const std::string& header_value)
        = 0;
    ...
    virtual void SetDocumentReportingEndpoints(const base::UnguessableToken& reporting_source, const url::Origin& origin, const IsolationInfo& isolation_info,
    ...
    virtual void RemoveBrowsingData(uint64_t data_type_mask, const base::RepeatingCallback<bool(const url::Origin&)>& origin_filter) = 0;
    ...
    virtual std::vector<raw_ptr<const ReportingReport, VectorExperimental>> GetReports() const = 0;
    ...
    virtual void RemoveReportingCacheObserver(ReportingCacheObserver* observer) = 0;
