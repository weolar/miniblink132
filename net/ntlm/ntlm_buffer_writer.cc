...
{
    return WriteUInt16(sec_buf.length) && WriteUInt16(sec_buf.length) && WriteUInt32(sec_buf.offset);
}
...
#else
    memcpy(reinterpret_cast<void*>(GetBufferPtrAtCursor()), str.c_str(), num_bytes);
...
template <typename T> bool NtlmBufferWriter::WriteUInt(T value)
{
