...
public:
    ...
    // not move the cursor.
    [[nodiscard]] bool ReadBytesFrom(const SecurityBuffer& sec_buf, base::span<uint8_t> buffer);
    ...
    [[nodiscard]] bool ReadPayloadAsBufferReader(const SecurityBuffer& sec_buf, NtlmBufferReader* reader);
    ...
    // should be discarded.
    [[nodiscard]] bool ReadTargetInfo(size_t target_info_len, std::vector<AvPair>* av_pairs);
...
private:
    ...
    template <typename T> bool ReadUInt(T* value);
    ...
    // method.
    void AdvanceCursor(size_t count)
    {
    ...
    // position.
    const uint8_t* GetBufferAtCursor() const
    {
        return GetBufferPtr() + cursor_;
    }
