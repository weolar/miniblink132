...
bool NtlmBufferReader::ReadBytesFrom(const SecurityBuffer& sec_buf, base::span<uint8_t> buffer)
{
...
bool NtlmBufferReader::ReadPayloadAsBufferReader(const SecurityBuffer& sec_buf, NtlmBufferReader* reader)
{
    if (!CanReadFrom(sec_buf))
        return false;

    *reader = NtlmBufferReader(base::make_span(GetBufferPtr() + sec_buf.offset, sec_buf.length));
    return true;
}

bool NtlmBufferReader::ReadSecurityBuffer(SecurityBuffer* sec_buf)
{
    return ReadUInt16(&sec_buf->length) && SkipBytes(sizeof(uint16_t)) && ReadUInt32(&sec_buf->offset);
}
...
bool NtlmBufferReader::ReadTargetInfo(size_t target_info_len, std::vector<AvPair>* av_pairs)
{
    ...
    while ((GetCursor() < target_info_end)) {
        ...
        case TargetInfoAvId::kFlags:
            ...
            if (pair.avlen != sizeof(uint32_t) || !ReadUInt32(reinterpret_cast<uint32_t*>(&pair.flags)))
                return false;
            break;
...
bool NtlmBufferReader::ReadMessageType(MessageType* message_type)
{
    ...
    *message_type = static_cast<MessageType>(raw_message_type);
    ...
    if (*message_type != MessageType::kNegotiate && *message_type != MessageType::kChallenge && *message_type != MessageType::kAuthenticate)
        return false;
...
bool NtlmBufferReader::MatchMessageType(MessageType message_type)
{
    ...
    return ReadMessageType(&actual_message_type) && (actual_message_type == message_type);
}
...
{
    SecurityBuffer sec_buf;
    return ReadSecurityBuffer(&sec_buf) && (sec_buf.offset <= GetLength()) && (sec_buf.length == 0);
}

template <typename T> bool NtlmBufferReader::ReadUInt(T* value)
{
