...
std::vector<uint8_t> GenerateAuthMsg(const NtlmClient& client, base::span<const uint8_t> challenge_msg)
{
    return client.GenerateAuthenticateMessage(test::kNtlmDomain, test::kUser, test::kPassword, test::kHostnameAscii,
        reinterpret_cast<const char*>(test::kChannelBindings), test::kNtlmSpn, test::kClientTimestamp, test::kClientChallenge, challenge_msg);
}

std::vector<uint8_t> GenerateAuthMsg(const NtlmClient& client, const NtlmBufferWriter& challenge_writer)
{
    return GenerateAuthMsg(client, challenge_writer.GetBuffer());
}

bool GetAuthMsgResult(const NtlmClient& client, const NtlmBufferWriter& challenge_writer)
{
...
bool ReadBytesPayload(NtlmBufferReader* reader, base::span<uint8_t> buffer)
{
    ...
    return reader->ReadSecurityBuffer(&sec_buf) && (sec_buf.length == buffer.size()) && reader->ReadBytesFrom(sec_buf, buffer);
}
...
void MakeV2ChallengeMessage(size_t target_info_len, std::vector<uint8_t>* out)
{
    ...
    ASSERT_TRUE(challenge.WriteSecurityBuffer(SecurityBuffer(0, 0))); // target name
    ...
    ASSERT_TRUE(challenge.WriteSecurityBuffer(SecurityBuffer(kChallengeV2HeaderLen, target_info_len))); // target info
    ASSERT_TRUE(challenge.WriteZeros(8)); // version
    ASSERT_EQ(kChallengeV2HeaderLen, challenge.GetCursor());
    ASSERT_TRUE(challenge.WriteAvPair(AvPair(TargetInfoAvId::kServerName, std::vector<uint8_t>(server_name_len, 'a'))));
...
TEST(NtlmClientTest, VerifyNegotiateMessageV1)
{
    ...
    ASSERT_EQ(0, memcmp(test::kExpectedNegotiateMsg, result.data(), kNegotiateMessageLen));
}
...
TEST(NtlmClientTest, MinimalStructurallyValidChallenge)
{
    ...
    ASSERT_TRUE(writer.WriteBytes(base::make_span(test::kMinChallengeMessage).subspan<0, kMinChallengeHeaderLen>()));
...
TEST(NtlmClientTest, ChallengeMsgTooShort)
{
    ...
    ASSERT_TRUE(writer.WriteBytes(base::make_span(test::kMinChallengeMessage).subspan<0, kMinChallengeHeaderLen - 1>()));
...
TEST(NtlmClientTest, Type3UnicodeWithSessionSecuritySpecTest)
{
    ...
    ASSERT_EQ(std::size(test::kExpectedAuthenticateMsgSpecResponseV1), result.size());
    ASSERT_EQ(0, memcmp(test::kExpectedAuthenticateMsgSpecResponseV1, result.data(), result.size()));
}
...
TEST(NtlmClientTest, Type3WithoutUnicode)
{
    ...
    std::vector<uint8_t> result = GenerateAuthMsg(client, base::make_span(test::kMinChallengeMessageNoUnicode).subspan<0, kMinChallengeHeaderLen>());
    ASSERT_FALSE(result.empty());
    ...
    ASSERT_EQ(0, memcmp(test::kExpectedLmResponseWithV1SS, actual_lm_response, kResponseLenV1));
    ASSERT_EQ(0, memcmp(test::kExpectedNtlmResponseWithV1SS, actual_ntlm_response, kResponseLenV1));
...
TEST(NtlmClientTest, ClientDoesNotDowngradeSessionSecurity)
{
    ...
    std::vector<uint8_t> result = GenerateAuthMsg(client, base::make_span(test::kMinChallengeMessageNoSS).subspan<0, kMinChallengeHeaderLen>());
    ASSERT_FALSE(result.empty());
    ...
    ASSERT_EQ(NegotiateFlags::kExtendedSessionSecurity, flags & NegotiateFlags::kExtendedSessionSecurity);
}
...
TEST(NtlmClientTest, VerifyNegotiateMessageV2)
{
    ...
    ASSERT_EQ(0, memcmp(test::kExpectedNegotiateMsg, result.data(), result.size()));
}
...
TEST(NtlmClientTest, VerifyAuthenticateMessageV2)
{
    ...
    // message.
    NtlmClient client(NtlmFeatures(true));
    std::vector<uint8_t> result = GenerateAuthMsg(client, test::kChallengeMsgFromSpecV2);
    ASSERT_FALSE(result.empty());
    ASSERT_EQ(std::size(test::kExpectedAuthenticateMsgSpecResponseV2), result.size());
    ASSERT_EQ(0, memcmp(test::kExpectedAuthenticateMsgSpecResponseV2, result.data(), result.size()));
}

TEST(NtlmClientTest, VerifyAuthenticateMessageInResponseToChallengeWithoutTargetInfoV2)
{
    ...
    ASSERT_EQ(std::size(test::kExpectedAuthenticateMsgToOldV1ChallegeV2), result.size());
    ASSERT_EQ(0, memcmp(test::kExpectedAuthenticateMsgToOldV1ChallegeV2, result.data(), result.size()));
}
