...
{
    return AvPair(TargetInfoAvId::kDomainName, std::vector<uint8_t> { std::begin(test::kNtlmDomainRaw), std::end(test::kNtlmDomainRaw) });
}

AvPair MakeServerAvPair()
{
    return AvPair(TargetInfoAvId::kServerName, std::vector<uint8_t> { std::begin(test::kServerRaw), std::end(test::kServerRaw) });
}
...
TEST(NtlmTest, MapHashToDesKeysAllOnes)
{
    ...
    const uint8_t hash[16] = { 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff };
    const uint8_t expected[24]
        = { 0xfe, 0xfe, 0xfe, 0xfe, 0xfe, 0xfe, 0xfe, 0xfe, 0xfe, 0xfe, 0xfe, 0xfe, 0xfe, 0xfe, 0xfe, 0xfe, 0xfe, 0xfe, 0xc0, 0x00, 0x00, 0x00, 0x00, 0x00 };
...
TEST(NtlmTest, MapHashToDesKeysAlternatingBits)
{
    ...
    const uint8_t hash[16] = { 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa };
    const uint8_t expected[24]
        = { 0xaa, 0x54, 0xaa, 0x54, 0xaa, 0x54, 0xaa, 0x54, 0xaa, 0x54, 0xaa, 0x54, 0xaa, 0x54, 0xaa, 0x54, 0xaa, 0x54, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00 };
...
TEST(NtlmTest, GenerateResponsesV1SpecTests)
{
    ...
    GenerateResponsesV1(test::kPassword, test::kServerChallenge, lm_response, ntlm_response);

    ASSERT_EQ(0, memcmp(test::kExpectedNtlmResponseV1, ntlm_response, kResponseLenV1));
    ...
    ASSERT_EQ(0, memcmp(test::kExpectedNtlmResponseV1, lm_response, kResponseLenV1));
}
...
TEST(NtlmTest, GenerateResponsesV1WithSessionSecuritySpecTests)
{
    uint8_t lm_response[kResponseLenV1];
    uint8_t ntlm_response[kResponseLenV1];
    GenerateResponsesV1WithSessionSecurity(test::kPassword, test::kServerChallenge, test::kClientChallenge, lm_response, ntlm_response);

    ASSERT_EQ(0, memcmp(test::kExpectedLmResponseWithV1SS, lm_response, kResponseLenV1));
    ASSERT_EQ(0, memcmp(test::kExpectedNtlmResponseWithV1SS, ntlm_response, kResponseLenV1));
}
...
TEST(NtlmTest, GenerateResponsesV1WithSessionSecurityClientChallengeUsed)
{
    ...
    GenerateResponsesV1WithSessionSecurity(test::kPassword, test::kServerChallenge, client_challenge1, lm_response1, ntlm_response1);
    GenerateResponsesV1WithSessionSecurity(test::kPassword, test::kServerChallenge, client_challenge2, lm_response2, ntlm_response2);
...
TEST(NtlmTest, GenerateResponsesV1WithSessionSecurityVerifySSUsed)
{
    ...
    GenerateResponsesV1WithSessionSecurity(test::kPassword, test::kServerChallenge, test::kClientChallenge, lm_response1, ntlm_response1);
    GenerateResponsesV1(test::kPassword, test::kServerChallenge, lm_response2, ntlm_response2);
...
TEST(NtlmTest, GenerateProofInputV2SpecTests)
{
    ...
    proof_input = GenerateProofInputV2(test::kServerTimestamp, test::kClientChallenge);
    ...
    ASSERT_EQ(0, memcmp(test::kExpectedTempFromSpecV2, proof_input.data(), proof_input.size()));
}
...
TEST(NtlmTest, GenerateNtlmProofV2SpecTests)
{
    ...
    GenerateNtlmProofV2(test::kExpectedNtlmHashV2, test::kServerChallenge, base::make_span(test::kExpectedTempFromSpecV2).subspan<0, kProofInputLenV2>(),
        test::kExpectedTargetInfoFromSpecV2, v2_proof);

    ASSERT_EQ(0, memcmp(test::kExpectedProofFromSpecV2, v2_proof, kNtlmProofLenV2));
}
...
TEST(NtlmTest, GenerateSessionBaseKeyV2SpecTests)
{
    ...
    GenerateSessionBaseKeyV2(test::kExpectedNtlmHashV2, test::kExpectedProofFromSpecV2, session_base_key);

    // Verify the session base key.
    ASSERT_EQ(0, memcmp(test::kExpectedSessionBaseKeyFromSpecV2, session_base_key, kSessionKeyLenV2));
}
...
TEST(NtlmTest, GenerateSessionBaseKeyWithClientTimestampV2SpecTests)
{
    // Generate the session base key.
    uint8_t session_base_key[kSessionKeyLenV2];
    GenerateSessionBaseKeyV2(test::kExpectedNtlmHashV2, test::kExpectedProofSpecResponseWithClientTimestampV2, session_base_key);

    // Verify the session base key.
    ASSERT_EQ(0, memcmp(test::kExpectedSessionBaseKeyWithClientTimestampV2, session_base_key, kSessionKeyLenV2));
}

TEST(NtlmTest, GenerateChannelBindingHashV2SpecTests)
{
    uint8_t v2_channel_binding_hash[kChannelBindingsHashLen];
    GenerateChannelBindingHashV2(reinterpret_cast<const char*>(test::kChannelBindings), v2_channel_binding_hash);

    ASSERT_EQ(0, memcmp(test::kExpectedChannelBindingHashV2, v2_channel_binding_hash, kChannelBindingsHashLen));
}
...
TEST(NtlmTest, GenerateMicV2Simple)
{
    ...
    uint8_t expected_mic[kMicLenV2] = { 0x71, 0xfe, 0xef, 0xd7, 0x76, 0xd4, 0x42, 0xa8, 0x5f, 0x6e, 0x18, 0x0a, 0x6b, 0x02, 0x47, 0x20 };
...
{
    std::vector<uint8_t> authenticate_msg(std::begin(test::kExpectedAuthenticateMsgSpecResponseV2), std::end(test::kExpectedAuthenticateMsgSpecResponseV2));
    memset(&authenticate_msg[kMicOffsetV2], 0x00, kMicLenV2);

    uint8_t mic[kMicLenV2];
    GenerateMicV2(test::kExpectedSessionBaseKeyWithClientTimestampV2, test::kExpectedNegotiateMsg, test::kChallengeMsgFromSpecV2, authenticate_msg, mic);
...
TEST(NtlmTest, GenerateUpdatedTargetInfo)
{
    ...
    std::vector<uint8_t> updated_target_info
        = GenerateUpdatedTargetInfo(true, true, reinterpret_cast<const char*>(test::kChannelBindings), test::kNtlmSpn, server_av_pairs, &server_timestamp);
    ...
    ASSERT_EQ(std::size(test::kExpectedTargetInfoSpecResponseV2), updated_target_info.size());
    ASSERT_EQ(0, memcmp(test::kExpectedTargetInfoSpecResponseV2, updated_target_info.data(), updated_target_info.size()));
}
...
TEST(NtlmTest, GenerateUpdatedTargetInfoNoEpaOrMic)
{
    ...
    // the client.
    std::vector<uint8_t> updated_target_info
        = GenerateUpdatedTargetInfo(false, false, reinterpret_cast<const char*>(test::kChannelBindings), test::kNtlmSpn, server_av_pairs, &server_timestamp);
    ASSERT_EQ(std::size(test::kExpectedTargetInfoFromSpecV2), updated_target_info.size());
    ASSERT_EQ(0, memcmp(test::kExpectedTargetInfoFromSpecV2, updated_target_info.data(), updated_target_info.size()));
}
...
TEST(NtlmTest, GenerateUpdatedTargetInfoWithServerTimestamp)
{
    ...
    AvPair pair(TargetInfoAvId::kTimestamp, std::vector<uint8_t>(sizeof(uint64_t), 0));
    ...
    ASSERT_EQ(std::size(test::kExpectedTargetInfoFromSpecPlusServerTimestampV2), updated_target_info.size());
    ASSERT_EQ(0, memcmp(test::kExpectedTargetInfoFromSpecPlusServerTimestampV2, updated_target_info.data(), updated_target_info.size()));
}
...
TEST(NtlmTest, GenerateUpdatedTargetInfoWhenServerSendsNoTargetInfo)
{
    ...
    ASSERT_EQ(std::size(test::kExpectedTargetInfoSpecResponseV2) - kMissingServerPairsLength, updated_target_info.size());
    ASSERT_EQ(0, memcmp(test::kExpectedTargetInfoSpecResponseV2 + kMissingServerPairsLength, updated_target_info.data(), updated_target_info.size()));
}
...
TEST(NtlmTest, GenerateNtlmProofV2)
{
    ...
    ASSERT_EQ(0, memcmp(test::kExpectedProofSpecResponseV2, proof, kNtlmProofLenV2));
}
...
TEST(NtlmTest, GenerateNtlmProofWithClientTimestampV2)
{
    ...
    GenerateNtlmProofV2(test::kExpectedNtlmHashV2, test::kServerChallenge,
        base::make_span(test::kExpectedTempWithClientTimestampV2).subspan<0, kProofInputLenV2>(), test::kExpectedTargetInfoSpecResponseV2, proof);
    ASSERT_EQ(0, memcmp(test::kExpectedProofSpecResponseWithClientTimestampV2, proof, kNtlmProofLenV2));
}
