...
TEST(NtlmBufferReaderTest, ReadPayloadBadOffset)
{
    ...
    NtlmBufferReader sub_reader;
    ASSERT_FALSE(reader.ReadPayloadAsBufferReader(SecurityBuffer(4, 1), &sub_reader));
}
...
TEST(NtlmBufferReaderTest, ReadPayloadBadLength)
{
    ...
    NtlmBufferReader sub_reader;
    ASSERT_FALSE(reader.ReadPayloadAsBufferReader(SecurityBuffer(3, 2), &sub_reader));
}
...
TEST(NtlmBufferReaderTest, SkipSecurityBufferWithValidationValid)
{
    ...
    const uint8_t buf[kSecurityBufferLen + 1] = { 0x01, 0, 0x01, 0, kSecurityBufferLen, 0, 0, 0, 0xFF };
...
TEST(NtlmBufferReaderTest, SkipSecurityBufferWithValidationPayloadLengthPastEob)
{
    // Security buffer with length that points past the end of buffer.
    const uint8_t buf[kSecurityBufferLen + 1] = { 0x02, 0, 0x02, 0, kSecurityBufferLen, 0, 0, 0, 0xFF };

    NtlmBufferReader reader(buf);
    ASSERT_FALSE(reader.SkipSecurityBufferWithValidation());
}

TEST(NtlmBufferReaderTest, SkipSecurityBufferWithValidationPayloadOffsetPastEob)
{
    // Security buffer with offset that points past the end of buffer.
    const uint8_t buf[kSecurityBufferLen + 1] = { 0x02, 0, 0x02, 0, kSecurityBufferLen + 1, 0, 0, 0, 0xFF };

    NtlmBufferReader reader(buf);
    ASSERT_FALSE(reader.SkipSecurityBufferWithValidation());
}

TEST(NtlmBufferReaderTest, SkipSecurityBufferWithValidationZeroLengthPayloadOffsetPastEob)
{
    // Security buffer with offset that points past the end of buffer but
    // length is 0.
    const uint8_t buf[kSecurityBufferLen] = { 0, 0, 0, 0, kSecurityBufferLen + 1, 0, 0, 0 };
...
{
    const uint8_t buf[4] = { static_cast<uint8_t>(MessageType::kNegotiate), 0, 0, 0 };
    NtlmBufferReader reader(buf);
...
{
    const uint8_t buf[4] = { static_cast<uint8_t>(MessageType::kChallenge), 0, 0, 0 };
    NtlmBufferReader reader(buf);
...
TEST(NtlmBufferReaderTest, ReadTargetInfoTimestampAndEolOnly)
{
    ...
    const uint8_t buf[16] = { 0x07, 0, 0x08, 0, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0, 0, 0, 0 };
...
TEST(NtlmBufferReaderTest, ReadTargetInfoInvalidTimestampSize)
{
    ...
    const uint8_t buf[15] = { 0x07, 0, 0x07, 0, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0, 0, 0, 0 };
...
TEST(NtlmBufferReaderTest, ReadTargetInfoInvalidTimestampPastEob)
{
    ...
    const uint8_t buf[11] = { 0x07, 0, 0x08, 0, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77 };
...
TEST(NtlmBufferReaderTest, ReadTargetInfoOtherField)
{
    ...
    const uint8_t buf[16] = { 0x02, 0, 0x08, 0, 'A', 0, 'B', 0, 'C', 0, 'D', 0, 0, 0, 0, 0 };
...
TEST(NtlmBufferReaderTest, ReadTargetInfoTerminatorAtLocationOtherThanEnd)
{
    ...
    const uint8_t buf[] = { 0x06, 0, 0x04, 0, 0x02, 0, 0, 0, 0, 0, 0, 0, 0x02, 0, 0x08, 0, 'A', 0, 'B', 0, 'C', 0, 'D', 0, 0, 0, 0, 0 };
...
TEST(NtlmBufferReaderTest, ReadTargetInfoTerminatorNonZeroLength2)
{
    ...
    const uint8_t buf[] = { 0x06, 0, 0x04, 0, 0x02, 0, 0, 0, 0, 0, 0x01, 0, 0xff, 0, 0, 0, 0 };
...
TEST(NtlmBufferReaderTest, ReadTargetInfoEolOnlyPayload)
{
    ...
    const uint8_t buf[] = { 0x04, 0x00, 0x04, 0x00, 0x08, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
...
TEST(NtlmBufferReaderTest, ReadTargetInfoTooShortPayload)
{
    ...
    const uint8_t buf[] = { 0x03, 0x00, 0x03, 0x00, 0x08, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
...
TEST(NtlmBufferReaderTest, ReadTargetInfoFlagsPayload)
{
    ...
    const uint8_t buf[] = { 0x0c, 0x00, 0x0c, 0x00, 0x08, 0x00, 0x00, 0x00, 0x06, 0, 0x04, 0, 0x02, 0, 0, 0, 0, 0, 0, 0 };
...
TEST(NtlmBufferReaderTest, ReadTargetInfoFlagsPayloadWithPaddingBetween)
{
    ...
    // is not contiguous.
    const uint8_t buf[] = { 0x0c, 0x00, 0x0c, 0x00, 0x0c, 0x00, 0x00, 0x00, 0xff, 0xff, 0xff, 0xff, 0x06, 0, 0x04, 0, 0x02, 0, 0, 0, 0, 0, 0, 0 };
    NtlmBufferReader reader(buf);
...
{
    const uint8_t buf[4] = { static_cast<uint8_t>(MessageType::kAuthenticate), 0, 0, 0 };
    NtlmBufferReader reader(buf);
...
{
    const uint8_t buf[12] = { 'N', 'T', 'L', 'M', 'S', 'S', 'P', 0, static_cast<uint8_t>(MessageType::kAuthenticate), 0, 0, 0 };
    NtlmBufferReader reader(buf);
...
{
    const uint8_t buf[12] = { 'N', 'T', 'L', 'M', 'S', 'S', 'P', 0, static_cast<uint8_t>(MessageType::kChallenge), 0, 0, 0 };
    NtlmBufferReader reader(buf);
...
{
    const uint8_t buf[kSecurityBufferLen + 1] = { 0x01, 0, 0, 0, 0x08, 0, 0, 0, 0xff };
