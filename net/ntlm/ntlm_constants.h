...
struct NtlmFeatures {
    explicit NtlmFeatures(bool enable_NTLMv2)
        : enable_NTLMv2(enable_NTLMv2)
    {
    }
...
constexpr NegotiateFlags operator|(NegotiateFlags lhs, NegotiateFlags rhs)
{
    ...
    return static_cast<NegotiateFlags>(static_cast<TFlagsInt>(lhs) | static_cast<TFlagsInt>(rhs));
}
...
constexpr TargetInfoAvFlags operator|(TargetInfoAvFlags lhs, TargetInfoAvFlags rhs)
{
    return static_cast<TargetInfoAvFlags>(static_cast<TAvFlagsInt>(lhs) | static_cast<TAvFlagsInt>(rhs));
}
...
static constexpr size_t kSecurityBufferLen = (2 * sizeof(uint16_t)) + sizeof(uint32_t);
...
static constexpr size_t kNtlmResponseHeaderLenV2 = kNtlmProofLenV2 + kProofInputLenV2;
...
static constexpr NegotiateFlags kNegotiateMessageFlags = NegotiateFlags::kUnicode | NegotiateFlags::kOem | NegotiateFlags::kRequestTarget
    | NegotiateFlags::kNtlm | NegotiateFlags::kAlwaysSign | NegotiateFlags::kExtendedSessionSecurity;
