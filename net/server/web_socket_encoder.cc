...
WebSocketParseResult DecodeFrameHybi17(std::string_view frame, bool client_frame, int* bytes_consumed, std::string* output, bool* compressed)
{
    ...
    *compressed = reserved1;
    ...
    case WebSocketFrameHeader::OpCodeEnum::kOpCodeContinuation: // Treated in the same as kOpCodeText.
    ...
    *bytes_consumed = pos;
    ...
    return final ? WebSocketParseResult::FRAME_OK_FINAL : WebSocketParseResult::FRAME_OK_MIDDLE;
}

void EncodeFrameHybi17(std::string_view message, int masking_key, bool compressed, WebSocketFrameHeader::OpCodeEnum op_code, std::string* output)
{
...
// static
std::unique_ptr<WebSocketEncoder> WebSocketEncoder::CreateServer(const std::string& extensions, WebSocketDeflateParameters* deflate_parameters)
{
    ...
    for (const auto& extension : parser.extensions()) {
        ...
        auto deflater = std::make_unique<WebSocketDeflater>(response.server_context_take_over_mode());
        auto inflater = std::make_unique<WebSocketInflater>(kInflaterChunkSize, kInflaterChunkSize);
...
std::unique_ptr<WebSocketEncoder> WebSocketEncoder::CreateClient(const std::string& response_extensions)
{
    ...
    auto deflater = std::make_unique<WebSocketDeflater>(params.client_context_take_over_mode());
    auto inflater = std::make_unique<WebSocketInflater>(kInflaterChunkSize, kInflaterChunkSize);
...
WebSocketEncoder::WebSocketEncoder(Type type, std::unique_ptr<WebSocketDeflater> deflater, std::unique_ptr<WebSocketInflater> inflater)
    : type_(type)
...
WebSocketParseResult WebSocketEncoder::DecodeFrame(std::string_view frame, int* bytes_consumed, std::string* output)
{
    bool compressed;
    std::string current_output;
    WebSocketParseResult result = DecodeFrameHybi17(frame, type_ == FOR_SERVER, bytes_consumed, &current_output, &compressed);
    switch (result) {
...
void WebSocketEncoder::EncodeTextFrame(std::string_view frame, int masking_key, std::string* output)
{
...
void WebSocketEncoder::EncodeCloseFrame(std::string_view frame, int masking_key, std::string* output)
{
    constexpr auto op_code = WebSocketFrameHeader::OpCodeEnum::kOpCodeClose;
    EncodeFrameHybi17(frame, masking_key, false, op_code, output);
}

void WebSocketEncoder::EncodePongFrame(std::string_view frame, int masking_key, std::string* output)
{
...
bool WebSocketEncoder::Inflate(std::string* message)
{
    ...
    *message = output.size() ? std::string(output.data(), output.size()) : std::string();
    return true;
}
