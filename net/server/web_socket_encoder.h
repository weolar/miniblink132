...
public:
    ...
    static std::unique_ptr<WebSocketEncoder> CreateServer(const std::string& extensions, WebSocketDeflateParameters* params);
    static std::unique_ptr<WebSocketEncoder> CreateClient(const std::string& response_extensions);

    WebSocketParseResult DecodeFrame(std::string_view frame, int* bytes_consumed, std::string* output);
    void EncodeTextFrame(std::string_view frame, int masking_key, std::string* output);
    void EncodePongFrame(std::string_view frame, int masking_key, std::string* output);
    void EncodeCloseFrame(std::string_view frame, int masking_key, std::string* output);

    bool deflate_enabled() const
    {
        return !!deflater_;
    }
...
private:
    ...
    WebSocketEncoder(Type type, std::unique_ptr<WebSocketDeflater> deflater, std::unique_ptr<WebSocketInflater> inflater);
