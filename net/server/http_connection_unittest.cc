...
{
    scoped_refptr<HttpConnection::ReadIOBuffer> buffer = base::MakeRefCounted<HttpConnection::ReadIOBuffer>();
    EXPECT_EQ(HttpConnection::ReadIOBuffer::kInitialBufSize + 0, buffer->GetCapacity());
    EXPECT_EQ(HttpConnection::ReadIOBuffer::kInitialBufSize + 0, buffer->RemainingCapacity());
...
TEST(HttpConnectionTest, ReadIOBuffer_SetCapacity_WithData)
{
    ...
    const std::string kReadData(GetTestString(HttpConnection::ReadIOBuffer::kInitialBufSize));
    memcpy(buffer->data(), kReadData.data(), kReadData.size());
    buffer->DidRead(kReadData.size());
    EXPECT_EQ(HttpConnection::ReadIOBuffer::kInitialBufSize + 0, buffer->GetCapacity());
    EXPECT_EQ(HttpConnection::ReadIOBuffer::kInitialBufSize - static_cast<int>(kReadData.size()), buffer->RemainingCapacity());
    EXPECT_EQ(static_cast<int>(kReadData.size()), buffer->GetSize());
    EXPECT_EQ(kReadData, std::string_view(buffer->StartOfBuffer(), buffer->GetSize()));

    // Check if read data in the buffer is same after SetCapacity().
    const int kNewCapacity = HttpConnection::ReadIOBuffer::kInitialBufSize + 128;
    buffer->SetCapacity(kNewCapacity);
    EXPECT_EQ(kNewCapacity, buffer->GetCapacity());
    EXPECT_EQ(kNewCapacity - static_cast<int>(kReadData.size()), buffer->RemainingCapacity());
    EXPECT_EQ(static_cast<int>(kReadData.size()), buffer->GetSize());
    EXPECT_EQ(kReadData, std::string_view(buffer->StartOfBuffer(), buffer->GetSize()));
}
...
TEST(HttpConnectionTest, ReadIOBuffer_IncreaseCapacity)
{
    ...
    EXPECT_EQ(HttpConnection::ReadIOBuffer::kDefaultMaxBufferSize + 0, buffer->max_buffer_size());
    EXPECT_EQ(HttpConnection::ReadIOBuffer::kDefaultMaxBufferSize + 0, buffer->GetCapacity());

    // Enlarge capacity limit.
    buffer->set_max_buffer_size(buffer->max_buffer_size() * 2);
    EXPECT_TRUE(buffer->IncreaseCapacity());
    EXPECT_EQ(HttpConnection::ReadIOBuffer::kDefaultMaxBufferSize * HttpConnection::ReadIOBuffer::kCapacityIncreaseFactor, buffer->GetCapacity());

    // Shrink capacity limit. It doesn't change capacity itself.
    buffer->set_max_buffer_size(HttpConnection::ReadIOBuffer::kDefaultMaxBufferSize / 2);
    EXPECT_FALSE(buffer->IncreaseCapacity());
    EXPECT_EQ(HttpConnection::ReadIOBuffer::kDefaultMaxBufferSize * HttpConnection::ReadIOBuffer::kCapacityIncreaseFactor, buffer->GetCapacity());
}
...
TEST(HttpConnectionTest, ReadIOBuffer_IncreaseCapacity_WithData)
{
    ...
    buffer->DidRead(kReadData.size());
    EXPECT_EQ(kExpectedInitialBufSize, buffer->GetCapacity());
    EXPECT_EQ(kExpectedInitialBufSize - static_cast<int>(kReadData.size()), buffer->RemainingCapacity());
    EXPECT_EQ(static_cast<int>(kReadData.size()), buffer->GetSize());
    EXPECT_EQ(kReadData, std::string_view(buffer->StartOfBuffer(), buffer->GetSize()));
    ...
    EXPECT_EQ(HttpConnection::ReadIOBuffer::kDefaultMaxBufferSize + 0, buffer->max_buffer_size());
    EXPECT_EQ(HttpConnection::ReadIOBuffer::kDefaultMaxBufferSize + 0, buffer->GetCapacity());
    EXPECT_EQ(HttpConnection::ReadIOBuffer::kDefaultMaxBufferSize - static_cast<int>(kReadData.size()), buffer->RemainingCapacity());
    EXPECT_EQ(static_cast<int>(kReadData.size()), buffer->GetSize());
    EXPECT_EQ(kReadData, std::string_view(buffer->StartOfBuffer(), buffer->GetSize()));
}
...
TEST(HttpConnectionTest, ReadIOBuffer_DidRead_DidConsume)
{
    ...
    EXPECT_EQ(HttpConnection::ReadIOBuffer::kInitialBufSize - kReadLength, buffer->RemainingCapacity());
    ...
    EXPECT_EQ(HttpConnection::ReadIOBuffer::kInitialBufSize / HttpConnection::ReadIOBuffer::kCapacityIncreaseFactor, buffer->GetCapacity());
    // Change in unused capacity because of read data.
    ...
    // Change in read data.
    EXPECT_EQ(kReadData.substr(kConsumedLength), std::string(buffer->StartOfBuffer(), buffer->GetSize()));
    ...
    // Change in unused capacity because of read data.
    EXPECT_EQ(
        HttpConnection::ReadIOBuffer::kInitialBufSize / HttpConnection::ReadIOBuffer::kCapacityIncreaseFactor - kReadLength + kConsumedLength - kReadLength2,
        buffer->RemainingCapacity());
    // Change in read size
    ...
    EXPECT_EQ(start_of_buffer, buffer->StartOfBuffer());
    // Change in start pointer of unused buffer.
    EXPECT_EQ(start_of_buffer + kReadLength - kConsumedLength + kReadLength2, buffer->data());
...
{
    scoped_refptr<HttpConnection::QueuedWriteIOBuffer> buffer = base::MakeRefCounted<HttpConnection::QueuedWriteIOBuffer>();
    ...
    EXPECT_EQ(static_cast<int>(kData.size() + kData2.size()), buffer->total_size());
    ...
    EXPECT_EQ(static_cast<int>(kData.size()) - kConsumedLength, buffer->GetSizeToWrite());
    // Change in total size.
    EXPECT_EQ(static_cast<int>(kData.size() + kData2.size()) - kConsumedLength, buffer->total_size());
    // First data to write has shrinked.
    EXPECT_EQ(kData.substr(kConsumedLength), std::string_view(buffer->data(), buffer->GetSizeToWrite()));
...
{
    scoped_refptr<HttpConnection::QueuedWriteIOBuffer> buffer = base::MakeRefCounted<HttpConnection::QueuedWriteIOBuffer>();
    EXPECT_EQ(HttpConnection::QueuedWriteIOBuffer::kDefaultMaxBufferSize + 0, buffer->max_buffer_size());
