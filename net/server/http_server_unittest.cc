...
public:
    ...
    int ConnectAndWait(const IPEndPoint& address)
    {
        ...
        NetLogSource source;
        socket_ = std::make_unique<TCPClientSocket>(addresses, nullptr, nullptr, nullptr, source);
    ...
    {
        write_buffer_ = base::MakeRefCounted<DrainableIOBuffer>(base::MakeRefCounted<StringIOBuffer>(data), data.length());
        Write();
    }
...
private:
    void Write()
    {
        int result = socket_->Write(write_buffer_.get(), write_buffer_->BytesRemaining(), base::BindOnce(&TestHttpClient::OnWrite, base::Unretained(this)),
    ...
    {
        read_buffer_ = base::MakeRefCounted<IOBufferWithSize>(kMaxExpectedResponseLength);
        int result = socket_->Read(read_buffer_.get(), kMaxExpectedResponseLength, callback->callback());
        if (result != ERR_IO_PENDING) {
    ...
    bool IsCompleteResponse(const std::string& response)
    {
        ...
        size_t end_of_headers = HttpUtil::LocateEndOfHeaders(base::as_byte_span(response));
        ...
        auto headers = base::MakeRefCounted<HttpResponseHeaders>(HttpUtil::AssembleRawHeaders(std::string_view(response.data(), end_of_headers)));
...
class HttpServerTest : public TestWithTaskEnvironment, public HttpServer::Delegate {
public:
    HttpServerTest() = default;

    void SetUp() override
    {
        auto server_socket = std::make_unique<TCPServerSocket>(nullptr, NetLogSource());
    ...
    void OnHttpRequest(int connection_id, const HttpServerRequestInfo& info) override
    {
        received_request_.SetValue({ .info = info, .connection_id = connection_id });
    }

    void OnWebSocketRequest(int connection_id, const HttpServerRequestInfo& info) override
    {
        NOTREACHED();
    }
    ...
    ReceivedRequest WaitForRequest()
    {
        return received_request_.Take();
    }

    bool HasRequest() const
    {
        return received_request_.IsReady();
    }
    ...
    void RunUntilConnectionIdClosed(int connection_id)
    {
        ...
        base::AutoReset<base::OnceClosure> run_loop_quit_func(&run_loop_quit_func_, run_loop.QuitClosure());
        run_loop.Run();
    ...
    std::unordered_map<int, bool>& connection_map()
    {
...
std::string EncodeFrame(std::string message, WebSocketFrameHeader::OpCodeEnum op_code, bool mask, bool finish)
{
    ...
    if (mask) {
        ...
        WriteWebSocketFrameHeader(header, &masking_key, base::as_writable_byte_span(frame_header));
        MaskWebSocketFramePayload(masking_key, 0, base::as_writable_byte_span(message));
    } else {
        WriteWebSocketFrameHeader(header, nullptr, base::as_writable_byte_span(frame_header));
    }
...
TEST_F(HttpServerTest, RequestWithDuplicateHeaders)
{
    ...
    const char* const kHeaders[][3] = {
        // clang-format on
        { "FirstHeader", ": ", "1" }, { "DuplicateHeader", ": ", "2" }, { "MiddleHeader", ": ", "3" }, { "DuplicateHeader", ": ", "4" },
        { "LastHeader", ": ", "5" },
        // clang-format on
    };
    std::string headers;
...
TEST_F(HttpServerTest, RequestWithBody)
{
    ...
    client.Send(base::StringPrintf("GET /test HTTP/1.1\r\n"
                                   "SomeHeader: 1\r\n"
...
TEST_F(WebSocketAcceptingTest, SendPingFrameWithNoMessage)
{
    ...
    const std::string ping_frame = EncodeFrame(message, WebSocketFrameHeader::OpCodeEnum::kOpCodePing,
        /* mask= */ true, /* finish= */ true);
    const std::string pong_frame = EncodeFrame(message, WebSocketFrameHeader::OpCodeEnum::kOpCodePong,
...
TEST_F(WebSocketAcceptingTest, SendLongTextFrame)
{
    ...
    const std::string text_encoded_frame = EncodeFrame(text_frame, WebSocketFrameHeader::OpCodeEnum::kOpCodeText,
        /* mask= */ true,
        /* finish= */ false);
    const std::string continuation_encoded_frame = EncodeFrame(continuation_frame, WebSocketFrameHeader::OpCodeEnum::kOpCodeContinuation,
        /* mask= */ true, /* finish= */ true);
    client.Send(text_encoded_frame);
    client.Send(continuation_encoded_frame);
    std::string received_message = GetMessage();
    EXPECT_EQ(received_message.size(), text_frame.size() + continuation_frame.size());
...
TEST_F(WebSocketAcceptingTest, SendTwoTextFrame)
{
    ...
    const std::string text_encoded_frame_first = EncodeFrame(text_frame_first, WebSocketFrameHeader::OpCodeEnum::kOpCodeText,
        /* mask= */ true,
        /* finish= */ false);
    const std::string continuation_encoded_frame_first = EncodeFrame(continuation_frame_first, WebSocketFrameHeader::OpCodeEnum::kOpCodeContinuation,
        /* mask= */ true, /* finish= */ true);

    const std::string text_frame_second = "FOO";
    const std::string continuation_frame_second = "BAR";
    const std::string text_encoded_frame_second = EncodeFrame(text_frame_second, WebSocketFrameHeader::OpCodeEnum::kOpCodeText,
        /* mask= */ true,
        /* finish= */ false);
    const std::string continuation_encoded_frame_second = EncodeFrame(continuation_frame_second, WebSocketFrameHeader::OpCodeEnum::kOpCodeContinuation,
        /* mask= */ true, /* finish= */ true);
...
TEST_F(WebSocketAcceptingTest, SendPingPongFrame)
{
    ...
    const std::string ping_frame_first = EncodeFrame(ping_message_first, WebSocketFrameHeader::OpCodeEnum::kOpCodePing,
        /* mask= */ true, /* finish= */ true);
    const std::string pong_frame_receive_first = EncodeFrame(ping_message_first, WebSocketFrameHeader::OpCodeEnum::kOpCodePong,
        /* mask= */ false, /* finish= */ true);
    const std::string pong_frame_send = EncodeFrame(
        /* message= */ "", WebSocketFrameHeader::OpCodeEnum::kOpCodePong,
        /* mask= */ true, /* finish= */ true);
    const std::string ping_message_second = "hello";
    const std::string ping_frame_second = EncodeFrame(ping_message_second, WebSocketFrameHeader::OpCodeEnum::kOpCodePing,
        /* mask= */ true, /* finish= */ true);
    const std::string pong_frame_receive_second = EncodeFrame(ping_message_second, WebSocketFrameHeader::OpCodeEnum::kOpCodePong,
        /* mask= */ false, /* finish= */ true);
...
TEST_F(WebSocketAcceptingTest, SendTextAndPingFrame)
{
    ...
    const std::string ping_frame = EncodeFrame(ping_message, WebSocketFrameHeader::OpCodeEnum::kOpCodePing,
        /* mask= */ true, /* finish= */ true);
    const std::string pong_frame = EncodeFrame(ping_message, WebSocketFrameHeader::OpCodeEnum::kOpCodePong,
        /* mask= */ false, /* finish= */ true);
...
TEST_F(WebSocketAcceptingTest, SendTextAndPongFrame)
{
    ...
    const std::string pong_frame = EncodeFrame(pong_message, WebSocketFrameHeader::OpCodeEnum::kOpCodePong,
        /* mask= */ true, /* finish= */ true);
...
TEST_F(WebSocketAcceptingTest, SendTextPingPongFrame)
{
    ...
    const std::string ping_frame_first = EncodeFrame(ping_message_first, WebSocketFrameHeader::OpCodeEnum::kOpCodePing,
        /* mask= */ true, /* finish= */ true);
    const std::string pong_frame_first = EncodeFrame(ping_message_first, WebSocketFrameHeader::OpCodeEnum::kOpCodePong,
        /* mask= */ false, /* finish= */ true);

    const std::string ping_message_second = "HELLO";
    const std::string ping_frame_second = EncodeFrame(ping_message_second, WebSocketFrameHeader::OpCodeEnum::kOpCodePing,
        /* mask= */ true, /* finish= */ true);
    const std::string pong_frame_second = EncodeFrame(ping_message_second, WebSocketFrameHeader::OpCodeEnum::kOpCodePong,
        /* mask= */ false, /* finish= */ true);
...
TEST_F(HttpServerTest, Send200)
{
    ...
    client.Send("GET /test HTTP/1.1\r\n\r\n");
    auto request = WaitForRequest();
    server_->Send200(request.connection_id, "Response!", "text/plain", TRAFFIC_ANNOTATION_FOR_TESTS);
...
TEST_F(HttpServerTest, SendRaw)
{
    ...
    server_->SendRaw(request.connection_id, "More Data", TRAFFIC_ANNOTATION_FOR_TESTS);
    server_->SendRaw(request.connection_id, "Third Piece of Data", TRAFFIC_ANNOTATION_FOR_TESTS);
...
{
    constexpr char kNullByteInHeader[] = "GET / HTTP/1.1\r\n"
...
TEST_F(HttpServerTest, NullByteInBody)
{
    ...
    // request.
    constexpr char kNullByteInBody[] = "POST /body HTTP/1.1\r\n"
                                       "User-Agent: Mozilla\r\n"
                                       "Content-Length: 1\r\n"
                                       "\r\n";
    TestHttpClient client;
...
public:
    ...
    int Read(IOBuffer* buf, int buf_len, CompletionOnceCallback callback) override
    {
        ...
        DCHECK_GT(buf_len, 0);
        int read_len = std::min(static_cast<int>(pending_read_data_.size()), buf_len);
...
TEST_F(HttpServerTest, RequestWithBodySplitAcrossPackets)
{
    ...
    std::string request_text = base::StringPrintf("GET /test HTTP/1.1\r\n"
                                                  "SomeHeader: 1\r\n"
                                                  "Content-Length: %" PRIuS "\r\n\r\n%s",
        body.length(), body.c_str());
...
TEST_F(HttpServerTest, MultipleRequestsOnSameConnection)
{
    ...
    server_->Send200(client_connection_id, "Content for /test", "text/plain", TRAFFIC_ANNOTATION_FOR_TESTS);
    ...
    server_->Send200(client_connection_id, "Content for /test3", "text/plain", TRAFFIC_ANNOTATION_FOR_TESTS);
