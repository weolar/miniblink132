...
std::string ExtensionsHeaderString(const std::vector<WebSocketExtension>& extensions)
{
    if (extensions.empty())
        return std::string();
...
std::string ValidResponseString(const std::string& accept_hash, const std::vector<WebSocketExtension>& extensions)
{
...
void WebSocket::Accept(const HttpServerRequestInfo& request, const NetworkTrafficAnnotationTag traffic_annotation)
{
    ...
    std::string encoded_hash = base::Base64Encode(base::SHA1HashString(key + websockets::kWebSocketGuid));
    ...
    traffic_annotation_ = std::make_unique<NetworkTrafficAnnotationTag>(NetworkTrafficAnnotationTag(traffic_annotation));
}
...
void WebSocket::Send(std::string_view message, WebSocketFrameHeader::OpCodeEnum op_code, const NetworkTrafficAnnotationTag traffic_annotation)
{
    if (closed_)
        return;
    std::string encoded;
...
void WebSocket::SendErrorResponse(const std::string& message, const NetworkTrafficAnnotationTag traffic_annotation)
{
    if (closed_)
        return;
    closed_ = true;
