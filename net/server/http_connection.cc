...
bool HttpConnection::ReadIOBuffer::IncreaseCapacity()
{
    if (GetCapacity() >= max_buffer_size_) {
        LOG(ERROR) << "Too large read data is pending: capacity=" << GetCapacity() << ", max_buffer_size=" << max_buffer_size_ << ", read=" << GetSize();
        return false;
    }
...
bool HttpConnection::QueuedWriteIOBuffer::Append(const std::string& data)
{
    ...
    if (total_size_ + static_cast<int>(data.size()) > max_buffer_size_) {
        LOG(ERROR) << "Too large write data is pending: size=" << total_size_ + data.size() << ", max_buffer_size=" << max_buffer_size_;
        return false;
    }
...
void HttpConnection::QueuedWriteIOBuffer::DidConsume(int size)
{
    ...
    if (size < GetSizeToWrite()) {
        ...
        data_ = nullptr;
        pending_data_.pop();
        data_ = IsEmpty() ? nullptr : const_cast<char*>(pending_data_.front()->data());
    }
    total_size_ -= size;
}
