...
public:
    WaitTillHttpCloseDelegate(FuzzedDataProvider* data_provider, base::OnceClosure done_closure)
    ...
    WaitTillHttpCloseDelegate& operator=(const WaitTillHttpCloseDelegate&) = delete;

    void set_server(net::HttpServer* server)
    {
        server_ = server;
    }
    ...
    void OnHttpRequest(int connection_id, const net::HttpServerRequestInfo& info) override
    {
        ...
        if (action_flags_ & REPLY_TO_MESSAGE) {
            server_->Send200(connection_id, data_provider_->ConsumeRandomLengthString(64), "text/html", TRAFFIC_ANNOTATION_FOR_TESTS);
        }
    }

    void OnWebSocketRequest(int connection_id, const net::HttpServerRequestInfo& info) override
    {
    ...
    void OnWebSocketMessage(int connection_id, std::string data) override
    {
        ...
        if (action_flags_ & REPLY_TO_MESSAGE) {
            server_->SendOverWebSocket(connection_id, data_provider_->ConsumeRandomLengthString(64), TRAFFIC_ANNOTATION_FOR_TESTS);
        }
    }
...
private:
    enum { ACCEPT_CONNECTION = 1, ACCEPT_MESSAGE = 2, REPLY_TO_MESSAGE = 4, ACCEPT_WEBSOCKET = 8, CLOSE_WEBSOCKET_RATHER_THAN_ACCEPT = 16 };
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    std::unique_ptr<net::ServerSocket> server_socket(std::make_unique<net::FuzzedServerSocket>(&data_provider, net::NetLog::Get()));
    CHECK_EQ(net::OK, server_socket->ListenWithAddressAndPort("127.0.0.1", 80, 5));
