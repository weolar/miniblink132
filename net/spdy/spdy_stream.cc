...
base::Value::Dict NetLogSpdyStreamErrorParams(spdy::SpdyStreamId stream_id, int net_error, std::string_view description)
{
    return base::Value::Dict().Set("stream_id", static_cast<int>(stream_id)).Set("net_error", ErrorToShortString(net_error)).Set("description", description);
}

base::Value::Dict NetLogSpdyStreamWindowUpdateParams(spdy::SpdyStreamId stream_id, int32_t delta, int32_t window_size)
{
    return base::Value::Dict().Set("stream_id", static_cast<int>(stream_id)).Set("delta", delta).Set("window_size", window_size);
}
...
SpdyStream::SpdyStream(SpdyStreamType type, const base::WeakPtr<SpdySession>& session, const GURL& url, RequestPriority priority,
    int32_t initial_send_window_size, int32_t max_recv_window_size, const NetLogWithSource& net_log, const NetworkTrafficAnnotationTag& traffic_annotation,
...
{
    CHECK(type_ == SPDY_BIDIRECTIONAL_STREAM || type_ == SPDY_REQUEST_RESPONSE_STREAM);
...
std::unique_ptr<spdy::SpdySerializedFrame> SpdyStream::ProduceHeadersFrame()
{
    ...
    spdy::SpdyControlFlags flags = (pending_send_status_ == NO_MORE_DATA_TO_SEND) ? spdy::CONTROL_FLAG_FIN : spdy::CONTROL_FLAG_NONE;
    std::unique_ptr<spdy::SpdySerializedFrame> frame(
        session_->CreateHeaders(stream_id_, priority_, flags, std::move(request_headers_), delegate_->source_dependency()));
...
bool SpdyStream::AdjustSendWindowSize(int32_t delta_window_size)
{
    ...
    if (delta_window_size > 0) {
        if (send_window_size_ > std::numeric_limits<int32_t>::max() - delta_window_size) {
            return false;
        }
    } else {
        ...
        DCHECK_LE(std::numeric_limits<int32_t>::min() - delta_window_size, send_window_size_);
    }

    send_window_size_ += delta_window_size;

    net_log_.AddEvent(
        NetLogEventType::HTTP2_STREAM_UPDATE_SEND_WINDOW, [&] { return NetLogSpdyStreamWindowUpdateParams(stream_id_, delta_window_size, send_window_size_); });

    PossiblyResumeIfSendStalled();
    return true;
}

void SpdyStream::OnWriteBufferConsumed(size_t frame_payload_size, size_t consume_size, SpdyBuffer::ConsumeSource consume_source)
{
...
void SpdyStream::IncreaseSendWindowSize(int32_t delta_window_size)
{
    ...
    if (!AdjustSendWindowSize(delta_window_size)) {
        std::string desc = base::StringPrintf("Received WINDOW_UPDATE [delta: %d] for stream %d overflows "
...
void SpdyStream::DecreaseSendWindowSize(int32_t delta_window_size)
{
    ...
    net_log_.AddEvent(NetLogEventType::HTTP2_STREAM_UPDATE_SEND_WINDOW,
        [&] { return NetLogSpdyStreamWindowUpdateParams(stream_id_, -delta_window_size, send_window_size_); });
}

void SpdyStream::OnReadBufferConsumed(size_t consume_size, SpdyBuffer::ConsumeSource consume_source)
{
...
void SpdyStream::IncreaseRecvWindowSize(int32_t delta_window_size)
{
    ...
    DCHECK_GE(delta_window_size, 1);
    // Check for overflow.
    DCHECK_LE(delta_window_size, std::numeric_limits<int32_t>::max() - recv_window_size_);

    recv_window_size_ += delta_window_size;
    net_log_.AddEvent(
        NetLogEventType::HTTP2_STREAM_UPDATE_RECV_WINDOW, [&] { return NetLogSpdyStreamWindowUpdateParams(stream_id_, delta_window_size, recv_window_size_); });
    ...
    if (unacked_recv_window_bytes_ > max_recv_window_size_ / 2 || elapsed >= session_->TimeToBufferSmallWindowUpdates()) {
        last_recv_window_update_ = base::TimeTicks::Now();
        session_->SendStreamWindowUpdate(stream_id_, static_cast<uint32_t>(unacked_recv_window_bytes_));
...
void SpdyStream::DecreaseRecvWindowSize(int32_t delta_window_size)
{
    ...
    if (delta_window_size > recv_window_size_ - unacked_recv_window_bytes_) {
        session_->ResetStream(stream_id_, ERR_HTTP2_FLOW_CONTROL_ERROR,
    ...
    net_log_.AddEvent(NetLogEventType::HTTP2_STREAM_UPDATE_RECV_WINDOW,
        [&] { return NetLogSpdyStreamWindowUpdateParams(stream_id_, -delta_window_size, recv_window_size_); });
}
...
void SpdyStream::OnHeadersReceived(const quiche::HttpHeaderBlock& response_headers, base::Time response_time, base::TimeTicks recv_first_byte_time)
{
    ...
    case READY_FOR_HEADERS: {
        ...
        quiche::HttpHeaderBlock::const_iterator it = response_headers.find(spdy::kHttp2StatusHeader);
        ...
        if (status / 100 != 1) {
            ...
            recv_first_byte_time_for_non_informational_response_ = recv_first_byte_time;
        }
...
void SpdyStream::OnDataReceived(std::unique_ptr<SpdyBuffer> buffer)
{
    ...
    if (!weak_this)
        return;
    buffer->AddConsumeCallback(base::BindRepeating(&SpdyStream::OnReadBufferConsumed, GetWeakPtr()));
...
void SpdyStream::OnFrameWriteComplete(spdy::SpdyFrameType frame_type, size_t frame_size)
{
    if (frame_type != spdy::SpdyFrameType::HEADERS && frame_type != spdy::SpdyFrameType::DATA) {
        return;
    }

    int result = (frame_type == spdy::SpdyFrameType::HEADERS) ? OnHeadersSent() : OnDataSent(frame_size);
...
{
    net_log_.AddEvent(NetLogEventType::HTTP2_STREAM_ERROR, [&] { return NetLogSpdyStreamErrorParams(stream_id_, error, description); });
}
...
int SpdyStream::SendRequestHeaders(quiche::HttpHeaderBlock request_headers, SpdySendStatus send_status)
{
    net_log_.AddEvent(NetLogEventType::HTTP_TRANSACTION_HTTP2_SEND_REQUEST_HEADERS,
        [&](NetLogCaptureMode capture_mode) { return HttpHeaderBlockNetLogParams(&request_headers, capture_mode); });
    ...
    session_->EnqueueStreamWrite(GetWeakPtr(), spdy::SpdyFrameType::HEADERS, std::make_unique<HeadersBufferProducer>(GetWeakPtr()));
    return ERR_IO_PENDING;
}

void SpdyStream::SendData(IOBuffer* data, int length, SpdySendStatus send_status)
{
    CHECK_EQ(pending_send_status_, MORE_DATA_TO_SEND);
    CHECK(io_state_ == STATE_OPEN || io_state_ == STATE_HALF_CLOSED_REMOTE) << io_state_;
    CHECK(!pending_send_data_.get());
...
SpdyStream::ShouldRequeueStream SpdyStream::PossiblyResumeIfSendStalled()
{
    ...
    net_log_.AddEventWithIntParams(NetLogEventType::HTTP2_STREAM_FLOW_CONTROL_UNSTALLED, "stream_id", stream_id_);
...
bool SpdyStream::GetLoadTimingInfo(LoadTimingInfo* load_timing_info) const
{
    ...
    load_timing_info->receive_non_informational_headers_start = recv_first_byte_time_for_non_informational_response_;
...
void SpdyStream::QueueNextDataFrame()
{
    ...
    spdy::SpdyDataFlags flags = (pending_send_status_ == NO_MORE_DATA_TO_SEND) ? spdy::DATA_FLAG_FIN : spdy::DATA_FLAG_NONE;
    int effective_len;
    bool end_stream;
    std::unique_ptr<SpdyBuffer> data_buffer(
        session_->CreateDataBuffer(stream_id_, pending_send_data_.get(), pending_send_data_->BytesRemaining(), flags, &effective_len, &end_stream));
    // We'll get called again by PossiblyResumeIfSendStalled().
    if (!data_buffer)
        return;

    DCHECK_GE(data_buffer->GetRemainingSize(), spdy::kDataFrameMinimumSize);
    size_t payload_size = data_buffer->GetRemainingSize() - spdy::kDataFrameMinimumSize;
    ...
    if (payload_size != 0) {
        ...
        data_buffer->AddConsumeCallback(base::BindRepeating(&SpdyStream::OnWriteBufferConsumed, GetWeakPtr(), payload_size));
    }

    if (session_->GreasedFramesEnabled() && delegate_ && delegate_->CanGreaseFrameType()) {
        session_->EnqueueGreasedFrame(GetWeakPtr());
    }

    session_->net_log().AddEvent(NetLogEventType::HTTP2_SESSION_SEND_DATA, [&] { return NetLogSpdyDataParams(stream_id_, effective_len, end_stream); });

    session_->EnqueueStreamWrite(GetWeakPtr(), spdy::SpdyFrameType::DATA, std::make_unique<SimpleBufferProducer>(std::move(data_buffer)));
}

void SpdyStream::OnEarlyHintsReceived(const quiche::HttpHeaderBlock& response_headers, base::TimeTicks recv_first_byte_time)
{
...
void SpdyStream::SaveResponseHeaders(const quiche::HttpHeaderBlock& response_headers, int status)
{
    if (response_headers.contains("transfer-encoding")) {
        session_->ResetStream(stream_id_, ERR_HTTP2_PROTOCOL_ERROR, "Received transfer-encoding header");
        return;
    }
...
std::string SpdyStream::DescribeState(State state)
{
    ...
    default:
        description = base::StringPrintf("Unknown state 0x%08X (%u)", state, state);
        break;
    }
    return description;
}
