...
void TestLoadTimingNotReused(const HttpStream& stream)
{
    ...
    ExpectConnectTimingHasTimes(load_timing_info.connect_timing, CONNECT_TIMING_HAS_DNS_TIMES | CONNECT_TIMING_HAS_SSL_TIMES);
    ExpectLoadTimingHasOnlyConnectionTimes(load_timing_info);
}
...
public:
    ...
    {
        return base::BindOnce(&CancelStreamCallback::CancelStream, base::Unretained(this));
    }
...
class SpdyHttpStreamTest : public testing::TestWithParam<bool>, public WithTaskEnvironment {
public:
    SpdyHttpStreamTest()
        : spdy_util_(/*use_priority_header=*/true)
        , url_(kDefaultUrl)
        , host_port_pair_(HostPortPair::FromURL(url_))
        , key_(host_port_pair_, PRIVACY_MODE_DISABLED, ProxyChain::Direct(), SessionUsage::kDestination, SocketTag(), NetworkAnonymizationKey(),
...
protected:
    ...
    void InitSession(base::span<const MockRead> reads, base::span<const MockWrite> writes)
    {
...
TEST_P(SpdyHttpStreamTest, SendRequest)
{
    ...
    auto http_stream = std::make_unique<SpdyHttpStream>(session_, net_log.source(),
    ...
    ASSERT_THAT(http_stream->InitializeStream(true, DEFAULT_PRIORITY, net_log, CompletionOnceCallback()), IsOk());
    EXPECT_FALSE(http_stream->GetLoadTimingInfo(&load_timing_info));
    ...
    EXPECT_EQ(static_cast<int64_t>(resp.size()), http_stream->GetTotalReceivedBytes());
}
...
TEST_P(SpdyHttpStreamTest, RequestInfoDestroyedBeforeRead)
{
    ...
    std::unique_ptr<HttpRequestInfo> request = std::make_unique<HttpRequestInfo>();
    request->method = "GET";
    request->url = url_;
    request->traffic_annotation = MutableNetworkTrafficAnnotationTag(TRAFFIC_ANNOTATION_FOR_TESTS);
    TestCompletionCallback callback;
    ...
    ASSERT_EQ(0, http_stream->ReadResponseBody(buf.get(), 1, callback.callback()));

    // Stream 1 has been read to completion.
    TestLoadTimingNotReused(*http_stream);

    EXPECT_EQ(static_cast<int64_t>(req.size()), http_stream->GetTotalSentBytes());
    EXPECT_EQ(static_cast<int64_t>(resp.size() + body.size()), http_stream->GetTotalReceivedBytes());
}
...
TEST_P(SpdyHttpStreamTest, LoadTimingTwoRequests)
{
    ...
    MockRead reads[] = {
        CreateMockRead(resp1, 2), CreateMockRead(body1, 3), CreateMockRead(resp2, 4), CreateMockRead(body2, 5), MockRead(ASYNC, 0, 6) // EOF
    };
    ...
    request1.method = "GET";
    request1.url = url_;
    request1.traffic_annotation = MutableNetworkTrafficAnnotationTag(TRAFFIC_ANNOTATION_FOR_TESTS);
    TestCompletionCallback callback1;
    HttpResponseInfo response1;
    HttpRequestHeaders headers1;
    NetLogWithSource net_log;
    auto http_stream1 = std::make_unique<SpdyHttpStream>(session_, net_log.source(),
        /*dns_aliases=*/std::set<std::string>());

    HttpRequestInfo request2;
    request2.method = "GET";
    request2.url = url_;
    request2.traffic_annotation = MutableNetworkTrafficAnnotationTag(TRAFFIC_ANNOTATION_FOR_TESTS);
    TestCompletionCallback callback2;
    HttpResponseInfo response2;
    HttpRequestHeaders headers2;
    auto http_stream2 = std::make_unique<SpdyHttpStream>(session_, net_log.source(),
        /*dns_aliases=*/std::set<std::string>());

    // First write.
    http_stream1->RegisterRequest(&request1);
    ASSERT_THAT(http_stream1->InitializeStream(true, DEFAULT_PRIORITY, net_log, CompletionOnceCallback()), IsOk());
    EXPECT_THAT(http_stream1->SendRequest(headers1, &response1, callback1.callback()), IsError(ERR_IO_PENDING));
    EXPECT_TRUE(HasSpdySession(http_session_->spdy_session_pool(), key_));
    ...
    ASSERT_THAT(http_stream2->InitializeStream(true, DEFAULT_PRIORITY, net_log, CompletionOnceCallback()), IsOk());
    EXPECT_THAT(http_stream2->SendRequest(headers2, &response2, callback2.callback()), IsError(ERR_IO_PENDING));
    EXPECT_TRUE(HasSpdySession(http_session_->spdy_session_pool(), key_));
    ...
    ASSERT_EQ(0, http_stream1->ReadResponseBody(buf1.get(), 1, callback1.callback()));

    // Stream 1 has been read to completion.
    TestLoadTimingNotReused(*http_stream1);

    EXPECT_EQ(static_cast<int64_t>(req1.size()), http_stream1->GetTotalSentBytes());
    EXPECT_EQ(static_cast<int64_t>(resp1.size() + body1.size()), http_stream1->GetTotalReceivedBytes());

    // Stream 2 still has queued body data.
    TestLoadTimingReused(*http_stream2);

    EXPECT_EQ(static_cast<int64_t>(req2.size()), http_stream2->GetTotalSentBytes());
    EXPECT_EQ(static_cast<int64_t>(resp2.size() + body2.size()), http_stream2->GetTotalReceivedBytes());
}

TEST_P(SpdyHttpStreamTest, SendChunkedPost)
{
    spdy::SpdySerializedFrame req(spdy_util_.ConstructChunkedSpdyPost(nullptr, 0));
    spdy::SpdySerializedFrame body(spdy_util_.ConstructSpdyDataFrame(1, kUploadData,
        /*fin=*/true));
    MockWrite writes[] = {
    ...
    MockRead reads[] = {
        CreateMockRead(resp, 2), CreateMockRead(body, 3, SYNCHRONOUS), MockRead(SYNCHRONOUS, 0, 4) // EOF
    };
    ...
    http_stream.RegisterRequest(&request);
    ASSERT_THAT(http_stream.InitializeStream(false, DEFAULT_PRIORITY, net_log, CompletionOnceCallback()), IsOk());
    ...
    EXPECT_EQ(static_cast<int64_t>(req.size() + body.size()), http_stream.GetTotalSentBytes());
    EXPECT_EQ(static_cast<int64_t>(resp.size() + body.size()), http_stream.GetTotalReceivedBytes());
...
{
    spdy::SpdySerializedFrame req(spdy_util_.ConstructChunkedSpdyPost(nullptr, 0));
    spdy::SpdySerializedFrame chunk(spdy_util_.ConstructSpdyDataFrame(1, "", true));
    MockWrite writes[] = {
        CreateMockWrite(req, 0), // request
    ...
    spdy::SpdySerializedFrame resp(spdy_util_.ConstructSpdyPostReply(nullptr, 0));
    MockRead reads[] = {
        CreateMockRead(resp, 2), CreateMockRead(chunk, 3, SYNCHRONOUS), MockRead(SYNCHRONOUS, 0, 4) // EOF
    };
    ...
    EXPECT_EQ(static_cast<int64_t>(req.size() + chunk.size()), http_stream.GetTotalSentBytes());
    EXPECT_EQ(static_cast<int64_t>(resp.size() + chunk.size()), http_stream.GetTotalReceivedBytes());
...
TEST_P(SpdyHttpStreamTest, DelayedSendChunkedPost)
{
    ...
    spdy::SpdySerializedFrame chunk2(spdy_util_.ConstructSpdyDataFrame(1, kUploadData1, false));
    ...
    EXPECT_EQ(static_cast<int64_t>(req.size() + chunk1.size() + chunk2.size() + chunk3.size()), http_stream->GetTotalSentBytes());
    EXPECT_EQ(static_cast<int64_t>(resp.size() + chunk1.size() + chunk2.size() + chunk3.size()), http_stream->GetTotalReceivedBytes());
    ...
    ASSERT_EQ(kUploadDataSize, http_stream->ReadResponseBody(buf1.get(), kUploadDataSize, callback.callback()));
    ...
    ASSERT_EQ(kUploadData1Size, http_stream->ReadResponseBody(buf2.get(), kUploadData1Size, callback.callback()));
    ...
    ASSERT_EQ(kUploadDataSize, http_stream->ReadResponseBody(buf3.get(), kUploadDataSize, callback.callback()));
...
TEST_P(SpdyHttpStreamTest, DelayedSendChunkedPostWithEmptyFinalDataFrame)
{
    spdy::SpdySerializedFrame req(spdy_util_.ConstructChunkedSpdyPost(nullptr, 0));
    spdy::SpdySerializedFrame chunk1(spdy_util_.ConstructSpdyDataFrame(1, false));
    spdy::SpdySerializedFrame chunk2(spdy_util_.ConstructSpdyDataFrame(1, "", true));
    MockWrite writes[] = {
        CreateMockWrite(req, 0),
        CreateMockWrite(chunk1, 1), // POST upload frames
        CreateMockWrite(chunk2, 2),
    ...
    spdy::SpdySerializedFrame resp(spdy_util_.ConstructSpdyPostReply(nullptr, 0));
    MockRead reads[] = {
        CreateMockRead(resp, 3), CreateMockRead(chunk1, 4), CreateMockRead(chunk2, 5), MockRead(ASYNC, 0, 6) // EOF
    };
    ...
    EXPECT_EQ(static_cast<int64_t>(req.size() + chunk1.size()), http_stream->GetTotalSentBytes());
    EXPECT_EQ(0, http_stream->GetTotalReceivedBytes());
    ...
    EXPECT_EQ(static_cast<int64_t>(req.size() + chunk1.size() + chunk2.size()), http_stream->GetTotalSentBytes());
    EXPECT_EQ(static_cast<int64_t>(resp.size() + chunk1.size() + chunk2.size()), http_stream->GetTotalReceivedBytes());
    ...
    // Check |chunk2| response.
    ASSERT_EQ(0, http_stream->ReadResponseBody(buf1.get(), kUploadDataSize, callback.callback()));
...
TEST_P(SpdyHttpStreamTest, ChunkedPostWithEmptyPayload)
{
    ...
    spdy::SpdySerializedFrame resp(spdy_util_.ConstructSpdyPostReply(nullptr, 0));
    MockRead reads[] = {
        CreateMockRead(resp, 2), CreateMockRead(chunk, 3), MockRead(ASYNC, 0, 4) // EOF
    };
...
TEST_P(SpdyHttpStreamTest, SpdyURLTest)
{
    ...
    spdy::SpdySerializedFrame req(spdy_util_.ConstructSpdyGet(base_url, 1, LOWEST));
    MockWrite writes[] = {
        CreateMockWrite(req, 0),
    };
    spdy::SpdySerializedFrame resp(spdy_util_.ConstructSpdyGetReply(nullptr, 0, 1));
...
TEST_P(SpdyHttpStreamTest, DelayedSendChunkedPostWithWindowUpdate)
{
    ...
    spdy::SpdySerializedFrame resp(spdy_util_.ConstructSpdyPostReply(nullptr, 0));
    spdy::SpdySerializedFrame window_update(spdy_util_.ConstructSpdyWindowUpdate(1, kUploadDataSize));
    MockRead reads[] = {
        CreateMockRead(window_update, 2), MockRead(ASYNC, ERR_IO_PENDING, 3), CreateMockRead(resp, 4), CreateMockRead(chunk1, 5), MockRead(ASYNC, 0, 6) // EOF
    };
    ...
    EXPECT_NE(static_cast<int>(kDefaultInitialWindowSize), http_stream->stream()->send_window_size());
    ...
    ASSERT_TRUE(http_stream->stream() != nullptr);
    EXPECT_EQ(static_cast<int>(kDefaultInitialWindowSize), http_stream->stream()->send_window_size());
    ...
    EXPECT_EQ(static_cast<int64_t>(req.size() + chunk1.size()), http_stream->GetTotalSentBytes());
    EXPECT_EQ(static_cast<int64_t>(resp.size() + chunk1.size()), http_stream->GetTotalReceivedBytes());
...
TEST_P(SpdyHttpStreamTest, DataReadErrorSynchronous)
{
    ...
    spdy::SpdySerializedFrame rst_frame(spdy_util_.ConstructSpdyRstStream(1, spdy::ERROR_CODE_INTERNAL_ERROR));
...
TEST_P(SpdyHttpStreamTest, RequestCallbackCancelsStream)
{
    ...
    MockWrite writes[] = { CreateMockWrite(req, 0), CreateMockWrite(chunk, 1), CreateMockWrite(rst, 2) };
    ...
    ASSERT_THAT(upload_stream.Init(upload_callback.callback(), NetLogWithSource()), IsOk());
    upload_stream.AppendData(base::byte_span_from_cstring(""), true);
...
TEST_P(SpdyHttpStreamTest, DownloadWithEmptyDataFrame)
{
    ...
    spdy::SpdySerializedFrame req = spdy_util_.ConstructSpdyHeaders(1, std::move(request_headers), LOWEST, /* fin = */ false);

    // Empty DATA frame with END_STREAM
    spdy::SpdySerializedFrame empty_body(spdy_util_.ConstructSpdyDataFrame(1, "", /* fin = */ true));

    MockWrite writes[] = { CreateMockWrite(req, 0), CreateMockWrite(empty_body, 1) };
    ...
    http_stream->RegisterRequest(&request);
    int rv = http_stream->InitializeStream(true, DEFAULT_PRIORITY, net_log, CompletionOnceCallback());
    EXPECT_THAT(rv, IsOk());
