...
std::string ElideGoAwayDebugDataForNetLogAsString(NetLogCaptureMode capture_mode, std::string_view debug_data)
{
...
TEST(SpdyLogUtilTest, ElideGoAwayDebugDataForNetLog)
{
    ...
    EXPECT_EQ("[6 bytes were stripped]", ElideGoAwayDebugDataForNetLogAsString(NetLogCaptureMode::kDefault, "foobar"));
    EXPECT_EQ("foobar", ElideGoAwayDebugDataForNetLogAsString(NetLogCaptureMode::kIncludeSensitive, "foobar"));
    EXPECT_EQ("%ESCAPED:\xE2\x80\x8B %FE%FF", ElideGoAwayDebugDataForNetLogAsString(NetLogCaptureMode::kIncludeSensitive, "\xfe\xff\x00"));
}
...
TEST(SpdyLogUtilTest, ElideHttpHeaderBlockForNetLog)
{
    ...
    base::Value::List list = ElideHttpHeaderBlockForNetLog(headers, NetLogCaptureMode::kDefault);
    ...
    list = ElideHttpHeaderBlockForNetLog(headers, NetLogCaptureMode::kIncludeSensitive);
...
TEST(SpdyLogUtilTest, HttpHeaderBlockNetLogParams)
{
    ...
    base::Value::Dict dict = HttpHeaderBlockNetLogParams(&headers, NetLogCaptureMode::kDefault);
    ...
    dict = HttpHeaderBlockNetLogParams(&headers, NetLogCaptureMode::kIncludeSensitive);
