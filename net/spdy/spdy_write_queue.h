...
NET_EXPORT_PRIVATE bool IsSpdyFrameTypeWriteCapped(spdy::SpdyFrameType frame_type);
...
public:
    ...
    void Enqueue(RequestPriority priority, spdy::SpdyFrameType frame_type, std::unique_ptr<SpdyBufferProducer> frame_producer,
    ...
    bool Dequeue(spdy::SpdyFrameType* frame_type, std::unique_ptr<SpdyBufferProducer>* frame_producer, base::WeakPtr<SpdyStream>* stream,
    ...
    void RemovePendingWritesForStreamsAfter(spdy::SpdyStreamId last_good_stream_id);

    // Change priority of all pending writes for the given stream.  Frames will be
    // queued after other writes with |new_priority|.
    void ChangePriorityOfWritesForStream(SpdyStream* stream, RequestPriority old_priority, RequestPriority new_priority);
    ...
    // priorities.
    int num_queued_capped_frames() const
    {
        return num_queued_capped_frames_;
    }
...
private:
    ...
    struct PendingWrite {
        ...
        PendingWrite();
        PendingWrite(spdy::SpdyFrameType frame_type, std::unique_ptr<SpdyBufferProducer> frame_producer, const base::WeakPtr<SpdyStream>& stream,
