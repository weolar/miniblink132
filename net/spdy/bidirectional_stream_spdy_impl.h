...
class NET_EXPORT_PRIVATE BidirectionalStreamSpdyImpl : public BidirectionalStreamImpl, public SpdyStream::Delegate {
public:
    BidirectionalStreamSpdyImpl(const base::WeakPtr<SpdySession>& spdy_session, NetLogSource source_dependency);

    BidirectionalStreamSpdyImpl(const BidirectionalStreamSpdyImpl&) = delete;
    BidirectionalStreamSpdyImpl& operator=(const BidirectionalStreamSpdyImpl&) = delete;

    ~BidirectionalStreamSpdyImpl() override;

    // BidirectionalStreamImpl implementation:
    void Start(const BidirectionalStreamRequestInfo* request_info, const NetLogWithSource& net_log, bool send_request_headers_automatically,
        BidirectionalStreamImpl::Delegate* delegate, std::unique_ptr<base::OneShotTimer> timer, const NetworkTrafficAnnotationTag& traffic_annotation) override;
    void SendRequestHeaders() override;
    int ReadData(IOBuffer* buf, int buf_len) override;
    void SendvData(const std::vector<scoped_refptr<IOBuffer>>& buffers, const std::vector<int>& lengths, bool end_stream) override;
