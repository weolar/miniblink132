...
int main(int argc, char** argv)
{
    ...
    if (!command_line.HasSwitch(kFileToWrite) || !command_line.HasSwitch(kExampleCount)) {
        LOG(ERROR) << "Usage: " << argv[0] << " --" << kFileToWrite << "=/path/to/file.out"
                   << " --" << kExampleCount << "=1000";
        return -1;
    }
    ...
    int example_count = 0;
    base::StringToInt(command_line.GetSwitchValueASCII(kExampleCount), &example_count);

    DVLOG(1) << "Writing output to " << file_to_write;
    base::File file_out(base::FilePath::FromUTF8Unsafe(file_to_write), base::File::FLAG_CREATE_ALWAYS | base::File::FLAG_WRITE);
    ...
    for (int i = 0; i != example_count; ++i) {
        quiche::HttpHeaderBlock headers = HpackFuzzUtil::NextGeneratedHeaderSet(&context);
