...
// static
HttpHeaderBlock HpackFuzzUtil::NextGeneratedHeaderSet(GeneratorContext* context)
{
    ...
    for (size_t j = 0; j != header_count; ++j) {
        ...
        if (name_index >= context->names.size()) {
            context->names.push_back(RandBytesAsString(1 + SampleExponential(kNameLengthMean, kNameLengthMax)));
            name = context->names.back();
        } else {
            name = context->names[name_index];
        }
        if (value_index >= context->values.size()) {
            context->values.push_back(RandBytesAsString(1 + SampleExponential(kValueLengthMean, kValueLengthMax)));
...
size_t HpackFuzzUtil::SampleExponential(size_t mean, size_t sanity_bound)
{
    ...
    return std::min(static_cast<size_t>(-std::log(1 - base::RandDouble()) * mean), sanity_bound);
}
...
{
    std::array<uint8_t, 4u> buf = base::U32ToBigEndian(base::checked_cast<uint32_t>(block_size));
...
void HpackFuzzUtil::InitializeFuzzerContext(FuzzerContext* context)
{
    ...
    context->first_stage->HandleControlFrameHeadersStart(context->first_stage_handler.get());
    ...
    context->third_stage->HandleControlFrameHeadersStart(context->third_stage_handler.get());
}

// static
bool HpackFuzzUtil::RunHeaderBlockThroughFuzzerStages(FuzzerContext* context, std::string_view input_block)
{
    // First stage: Decode the input header block. This may fail on invalid input.
    if (!context->first_stage->HandleControlFrameHeadersData(input_block.data(), input_block.size())) {
        return false;
    }
    ...
    std::string second_stage_out = context->second_stage->EncodeHeaderBlock(context->first_stage_handler->decoded_block());
    ...
    if (!context->third_stage->HandleControlFrameHeadersData(second_stage_out.data(), second_stage_out.length())) {
        return false;
    }
...
// static
void HpackFuzzUtil::FlipBits(uint8_t* buffer, size_t buffer_length, size_t flip_per_thousand)
{
