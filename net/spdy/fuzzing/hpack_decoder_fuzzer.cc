...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    size_t max_string_size = fuzzed_data_provider.ConsumeIntegralInRange<size_t>(1, 10 * size);
    http2::HpackDecoder decoder(http2::HpackDecoderNoOpListener::NoOpListener(), max_string_size);
    ...
    while (fuzzed_data_provider.remaining_bytes() > 0) {
        ...
        all_chunks.emplace_back(fuzzed_data_provider.ConsumeBytes<char>(chunk_size));
