...
public:
    ...
    static quiche::HttpHeaderBlock NextGeneratedHeaderSet(GeneratorContext* context);
    ...
    struct Input {
        ...
        template <size_t bytes> base::span<const uint8_t, bytes> ReadSpan()
        {
            auto out = RemainingBytes().first<bytes>();
            offset += bytes;
            return out;
    ...
    static bool RunHeaderBlockThroughFuzzerStages(FuzzerContext* context, std::string_view input_block);
    ...
    // rounding up.
    static void FlipBits(uint8_t* buffer, size_t buffer_length, size_t flip_per_thousand);
};
