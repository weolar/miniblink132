...
{
    EXPECT_EQ(std::string("\x00\x00\x00\x00", 4), HpackFuzzUtil::HeaderBlockPrefix(0));
    EXPECT_EQ(std::string("\x00\x00\x00\x05", 4), HpackFuzzUtil::HeaderBlockPrefix(5));
...
TEST(HpackFuzzUtilTest, PassValidInputThroughAllStages)
{
    ...
    EXPECT_TRUE(HpackFuzzUtil::RunHeaderBlockThroughFuzzerStages(&context, input));
...
TEST(HpackFuzzUtilTest, ValidFuzzExamplesRegressionTest)
{
    ...
    HpackFuzzUtil::Input input;
    ASSERT_TRUE(base::ReadFileToString(source_root.Append(FILE_PATH_LITERAL("net"))
    ...
    while (HpackFuzzUtil::NextHeaderBlock(&input, &block)) {
        ...
        EXPECT_TRUE(HpackFuzzUtil::RunHeaderBlockThroughFuzzerStages(&context, block));
    }
}
...
TEST(HpackFuzzUtilTest, FlipBitsMutatesBuffer)
{
    ...
    HpackFuzzUtil::FlipBits(reinterpret_cast<uint8_t*>(buffer), std::size(buffer) - 1, 1);
