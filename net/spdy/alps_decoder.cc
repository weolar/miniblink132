...
bool ReadUint16PrefixedStringPiece(std::string_view* payload, std::string_view* output)
{
    if (payload->size() < 2) {
        return false;
    }
    const uint16_t length = (static_cast<uint16_t>((*payload)[0]) << 8) + (static_cast<uint8_t>((*payload)[1]));
...
AlpsDecoder::Error AlpsDecoder::Decode(base::span<const char> data)
{
    ...
    base::UmaHistogramEnumeration("Net.SpdySession.AlpsDecoderStatus.Bypassed", accept_ch_parser_.error_bypass());
    ...
    if (decoder_adapter_.state() != http2::Http2DecoderAdapter::SPDY_READY_FOR_FRAME) {
...
void AlpsDecoder::SettingsParser::OnCommonHeader(spdy::SpdyStreamId /*stream_id*/, size_t /*length*/, uint8_t type, uint8_t /*flags*/)
{
    if (type == static_cast<uint8_t>(http2::Http2FrameType::DATA) || type == static_cast<uint8_t>(http2::Http2FrameType::HEADERS)
        || type == static_cast<uint8_t>(http2::Http2FrameType::PRIORITY) || type == static_cast<uint8_t>(http2::Http2FrameType::RST_STREAM)
        || type == static_cast<uint8_t>(http2::Http2FrameType::PUSH_PROMISE) || type == static_cast<uint8_t>(http2::Http2FrameType::PING)
        || type == static_cast<uint8_t>(http2::Http2FrameType::GOAWAY) || type == static_cast<uint8_t>(http2::Http2FrameType::WINDOW_UPDATE)
...
void AlpsDecoder::SettingsParser::OnSetting(spdy::SpdySettingsId id, uint32_t value)
{
    settings_[id] = value;
}
...
bool AlpsDecoder::AcceptChParser::OnFrameHeader(spdy::SpdyStreamId stream_id, size_t length, uint8_t type, uint8_t flags)
{
...
void AlpsDecoder::AcceptChParser::OnFramePayload(const char* data, size_t len)
{
    ...
    while (!payload.empty()) {
        ...
        if (!ReadUint16PrefixedStringPiece(&payload, &origin) || !ReadUint16PrefixedStringPiece(&payload, &value)) {
            if (base::FeatureList::IsEnabled(features::kShouldKillSessionOnAcceptChMalformed)) {
        ...
        accept_ch_.push_back(spdy::AcceptChOriginValuePair { std::string(origin), std::string(value) });
    }
}
