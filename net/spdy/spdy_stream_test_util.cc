...
void ClosingDelegate::OnEarlyHintsReceived(const quiche::HttpHeaderBlock& headers)
{
}

void ClosingDelegate::OnHeadersReceived(const quiche::HttpHeaderBlock& response_headers)
{
}
...
void StreamDelegateBase::OnEarlyHintsReceived(const quiche::HttpHeaderBlock& headers)
{
    EXPECT_TRUE(send_headers_completed_);
    early_hints_.push_back(headers.Clone());
}

void StreamDelegateBase::OnHeadersReceived(const quiche::HttpHeaderBlock& response_headers)
{
    EXPECT_TRUE(send_headers_completed_);
...
std::string StreamDelegateBase::GetResponseHeaderValue(const std::string& name) const
{
    quiche::HttpHeaderBlock::const_iterator it = response_headers_.find(name);
    return (it == response_headers_.end()) ? std::string() : std::string(it->second);
}
...
StreamDelegateDoNothing::StreamDelegateDoNothing(const base::WeakPtr<SpdyStream>& stream)
...
StreamDelegateConsumeData::StreamDelegateConsumeData(const base::WeakPtr<SpdyStream>& stream)
    : StreamDelegateBase(stream)
{
}
...
void StreamDelegateConsumeData::OnDataReceived(std::unique_ptr<SpdyBuffer> buffer)
{
    buffer->Consume(buffer->GetRemainingSize());
}

StreamDelegateSendImmediate::StreamDelegateSendImmediate(const base::WeakPtr<SpdyStream>& stream, std::string_view data)
    : StreamDelegateBase(stream)
    , data_(data)
{
}
...
void StreamDelegateSendImmediate::OnHeadersReceived(const quiche::HttpHeaderBlock& response_headers)
{
    StreamDelegateBase::OnHeadersReceived(response_headers);
    if (data_.data()) {
        scoped_refptr<StringIOBuffer> buf = base::MakeRefCounted<StringIOBuffer>(std::string(data_));
        stream()->SendData(buf.get(), buf->size(), MORE_DATA_TO_SEND);
    }
}

StreamDelegateWithBody::StreamDelegateWithBody(const base::WeakPtr<SpdyStream>& stream, std::string_view data)
    : StreamDelegateBase(stream)
...
StreamDelegateCloseOnHeaders::StreamDelegateCloseOnHeaders(const base::WeakPtr<SpdyStream>& stream)
    : StreamDelegateBase(stream)
{
}
...
void StreamDelegateCloseOnHeaders::OnHeadersReceived(const quiche::HttpHeaderBlock& response_headers)
{
    stream()->Cancel(ERR_ABORTED);
}

StreamDelegateDetectEOF::StreamDelegateDetectEOF(const base::WeakPtr<SpdyStream>& stream)
    : StreamDelegateBase(stream)
{
}
...
void StreamDelegateDetectEOF::OnDataReceived(std::unique_ptr<SpdyBuffer> buffer)
{
    if (!buffer)
        eof_detected_ = true;
}
