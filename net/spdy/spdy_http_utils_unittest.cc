...
TEST_P(SpdyHttpUtilsTestParam, CreateSpdyHeadersFromHttpRequestHTTP2)
{
    ...
    CreateSpdyHeadersFromHttpRequest(request, RequestPriority::HIGHEST, request.extra_headers, &headers);
...
TEST_P(SpdyHttpUtilsTestParam, CreateSpdyHeadersFromHttpRequestForExtendedConnect)
{
    GURL url("https://www.google.com/index.html");
    HttpRequestInfo request;
    ...
    request.extra_headers.SetHeader(HttpRequestHeaders::kUserAgent, "Chrome/1.1");
    quiche::HttpHeaderBlock headers;
    CreateSpdyHeadersFromHttpRequestForExtendedConnect(request, RequestPriority::HIGHEST, "connect-ftp", request.extra_headers, &headers);
    CheckOrdering(headers);
...
TEST_P(SpdyHttpUtilsTestParam, CreateSpdyHeadersWithDefaultPriority)
{
    ...
    request.extra_headers.SetHeader(HttpRequestHeaders::kUserAgent, "Chrome/1.1");
    quiche::HttpHeaderBlock headers;
    CreateSpdyHeadersFromHttpRequest(request, RequestPriority::DEFAULT_PRIORITY, request.extra_headers, &headers);
    CheckOrdering(headers);
    EXPECT_EQ("GET", headers[":method"]);
    EXPECT_EQ("https", headers[":scheme"]);
    EXPECT_EQ("www.google.com", headers[":authority"]);
...
TEST_P(SpdyHttpUtilsTestParam, CreateSpdyHeadersWithExistingPriority)
{
    ...
    request.priority_incremental = true;
    request.extra_headers.SetHeader(HttpRequestHeaders::kUserAgent, "Chrome/1.1");
    request.extra_headers.SetHeader(net::kHttp2PriorityHeader, "explicit-priority");
    quiche::HttpHeaderBlock headers;
    CreateSpdyHeadersFromHttpRequest(request, RequestPriority::HIGHEST, request.extra_headers, &headers);
    CheckOrdering(headers);
    EXPECT_EQ("GET", headers[":method"]);
...
enum class SpdyHeadersToHttpResponseHeadersFeatureConfig { kUseRawString, kUseBuilder };

std::string PrintToString(SpdyHeadersToHttpResponseHeadersFeatureConfig config)
{
    switch (config) {
...
class SpdyHeadersToHttpResponseTest : public ::testing::TestWithParam<SpdyHeadersToHttpResponseHeadersFeatureConfig> {
public:
    SpdyHeadersToHttpResponseTest()
    {
        switch (GetParam()) {
        case SpdyHeadersToHttpResponseHeadersFeatureConfig::kUseRawString:
            feature_list_.InitWithFeatures({}, { features::kSpdyHeadersToHttpResponseUseBuilder });
            break;
...
{
    constexpr char kExpectedSimpleString[] = "HTTP/1.1 200\n"
    ...
    input.AppendValueOrAddHeader("set-cookie", "test_cookie=1234567890; Max-Age=3600; Secure; HttpOnly");
    input.AppendValueOrAddHeader("set-cookie", "session_id=abcdefghijklmnopqrstuvwxyz; Path=/; HttpOnly");
...
INSTANTIATE_TEST_SUITE_P(SpdyHttpUtils, SpdyHeadersToHttpResponseTest,
    Values(SpdyHeadersToHttpResponseHeadersFeatureConfig::kUseRawString, SpdyHeadersToHttpResponseHeadersFeatureConfig::kUseBuilder),
...
using SpdyHeadersToHttpResponseHeadersFunctionPtrType = base::expected<scoped_refptr<HttpResponseHeaders>, int> (*)(const quiche::HttpHeaderBlock&);

class SpdyHeadersToHttpResponseHeadersTest : public testing::TestWithParam<SpdyHeadersToHttpResponseHeadersFunctionPtrType> {
public:
    base::expected<scoped_refptr<HttpResponseHeaders>, int> PerformConversion(const quiche::HttpHeaderBlock& headers)
    {
...
{
    quiche::HttpHeaderBlock headers;
    EXPECT_THAT(PerformConversion(headers), base::test::ErrorIs(ERR_INCOMPLETE_HTTP2_HEADERS));
}
...
TEST_P(SpdyHeadersToHttpResponseHeadersTest, MultipleLocation)
{
    ...
    EXPECT_THAT(PerformConversion(headers), base::test::ErrorIs(ERR_RESPONSE_HEADERS_MULTIPLE_LOCATION));
}
...
TEST_P(SpdyHeadersToHttpResponseHeadersTest, DifferentLocationHeaderTriggersError)
{
    quiche::HttpHeaderBlock headers;
    headers[spdy::kHttp2StatusHeader] = "200";
...
TEST_P(SpdyHeadersToHttpResponseHeadersTest, LocationEquivalenceIgnoresSurroundingSpace)
{
    constexpr char kRawHeaders[] = "HTTP/1.1 200\n"
                                   "location: https://same/\n"
                                   "location: https://same/\n";
...
INSTANTIATE_TEST_SUITE_P(SpdyHttpUtils, SpdyHeadersToHttpResponseHeadersTest,
    Values(SpdyHeadersToHttpResponseHeadersUsingRawString, SpdyHeadersToHttpResponseHeadersUsingBuilder),
    [](const testing::TestParamInfo<SpdyHeadersToHttpResponseHeadersTest::ParamType>& info) {
        return info.param == SpdyHeadersToHttpResponseHeadersUsingRawString ? "SpdyHeadersToHttpResponseHeadersUsingRawString"
