...
quiche::HttpHeaderBlock MakeHeaderBlock()
{
    ...
    headers["strict-transport-security"] = "max-age=106384710; includeSubDomains; preload";
    headers["report-to"] = "{ \"group\": \"wm_nel\", \"max_age\": 604800, \"endpoints\": [{ "
                           "\"url\": "
                           "\"https://nel.example.net/v1/"
                           "events?stream=w3c.reportingapi.network_error&schema_uri=/w3c/"
                           "reportingapi/network_error/1.0.0\" }] }";
    headers["nel"] = "{ \"report_to\": \"wm_nel\", \"max_age\": 604800, \"failure_fraction\": "
                     "0.05, \"success_fraction\": 0.0}";
    headers.AppendValueOrAddHeader("set-cookie",
    ...
    headers.AppendValueOrAddHeader("set-cookie", "NetworkProbeLimit=0.001;Path=/;Secure;Max-Age=3600");
...
// sllowing it down.
template <SpdyHeadersToHttpResponseHeadersFunctionPtrType convert> void Benchmark(::benchmark::State& state)
{
...
BENCHMARK(Benchmark<SpdyHeadersToHttpResponseHeadersUsingRawString>)->MinWarmUpTime(1.0);
BENCHMARK(Benchmark<SpdyHeadersToHttpResponseHeadersUsingBuilder>)->MinWarmUpTime(1.0);
...
void BM_CreateSpdyHeadersFromHttpRequest(::benchmark::State& state)
{
    ...
    http_request_headers.SetHeader("Accept",
    ...
    http_request_headers.SetHeader("Cookie",
    ...
    http_request_headers.SetHeader("Sec-Ch-Ua",
    ...
    http_request_headers.SetHeader("User-Agent",
        "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) "
        "Chrome/117.0.0.0 Safari/537.36");

    for (auto _ : state) {
        quiche::HttpHeaderBlock headers;
        CreateSpdyHeadersFromHttpRequest(info, RequestPriority::DEFAULT_PRIORITY, http_request_headers, &headers);
        ::benchmark::DoNotOptimize(headers);
    }
}
