...
public:
    ...
    void ExpectEntry(std::string_view expected_header_name, std::string_view expected_header_value, std::string_view expected_error_message)
    {
        auto entry_list = net_log_observer_.GetEntries();
        ASSERT_EQ(1u, entry_list.size());
        EXPECT_EQ(entry_list[0].type, NetLogEventType::HTTP2_SESSION_RECV_INVALID_HEADER);
        EXPECT_EQ(entry_list[0].source.id, net_log_with_source_.source().id);
        std::string value;
        EXPECT_EQ(expected_header_name, GetStringValueFromParams(entry_list[0], "header_name"));
        EXPECT_EQ(expected_header_value, GetStringValueFromParams(entry_list[0], "header_value"));
        EXPECT_EQ(expected_error_message, GetStringValueFromParams(entry_list[0], "error"));
    }
...
TEST_F(HeaderCoalescerTest, CorrectHeaders)
{
    ...
    EXPECT_THAT(header_block, ElementsAre(Pair(":foo", "bar"), Pair("baz", "qux")));
}
...
TEST_F(HeaderCoalescerTest, Append)
{
    ...
    quiche::HttpHeaderBlock header_block = header_coalescer_.release_headers();
    EXPECT_THAT(header_block, ElementsAre(Pair("foo", std::string_view("bar\0quux", 8)), Pair("cookie", "baz; qux")));
}
...
TEST_F(HeaderCoalescerTest, HeaderNameNotValid)
{
    ...
    EXPECT_TRUE(header_coalescer_.error_seen());
    ExpectEntry("%ESCAPED:\xE2\x80\x8B \x1\x7F%80%FF", "foo", "Invalid character in header name.");
}
...
TEST_F(HeaderCoalescerTest, HeaderNameHasUppercase)
{
    ...
    header_coalescer_.OnHeader(header_name, "foo");
    EXPECT_TRUE(header_coalescer_.error_seen());
    ExpectEntry("ABCDEFGHIJKLMNOPQRSTUVWXYZ", "foo", "Upper case characters in header name.");
}
...
TEST_F(HeaderCoalescerTest, HeaderNameValid)
{
    ...
    std::string_view header_name("abcdefghijklmnopqrstuvwxyz0123456789!#$%&'*+-."
                                 "^_`|~");
    header_coalescer_.OnHeader(header_name, "foo");
    EXPECT_FALSE(header_coalescer_.error_seen());
    quiche::HttpHeaderBlock header_block = header_coalescer_.release_headers();
