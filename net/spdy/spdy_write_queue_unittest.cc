...
std::unique_ptr<SpdyBufferProducer> StringToProducer(const std::string& s)
{
    ...
    auto frame = std::make_unique<spdy::SpdySerializedFrame>(std::move(data), s.size());
...
public:
    explicit RequeingBufferProducer(SpdyWriteQueue* queue)
    {
        ...
        buffer_->AddConsumeCallback(base::BindRepeating(RequeingBufferProducer::ConsumeCallback, queue));
    }
    ...
    static void ConsumeCallback(SpdyWriteQueue* queue, size_t size, SpdyBuffer::ConsumeSource source)
    {
        auto buffer = std::make_unique<SpdyBuffer>(kRequeued, std::size(kRequeued));
        auto buffer_producer = std::make_unique<SimpleBufferProducer>(std::move(buffer));

        queue->Enqueue(MEDIUM, spdy::SpdyFrameType::RST_STREAM, std::move(buffer_producer), base::WeakPtr<SpdyStream>(), TRAFFIC_ANNOTATION_FOR_TESTS);
    }
...
{
    return std::make_unique<SpdyStream>(SPDY_BIDIRECTIONAL_STREAM, base::WeakPtr<SpdySession>(), GURL(), priority, 0, 0, NetLogWithSource(),
        TRAFFIC_ANNOTATION_FOR_TESTS, false /* detect_broken_connection */);
}
...
TEST_F(SpdyWriteQueueTest, DequeuesByPriority)
{
    ...
    write_queue.Enqueue(LOW, spdy::SpdyFrameType::HEADERS, std::move(producer_low), base::WeakPtr<SpdyStream>(), TRAFFIC_ANNOTATION_FOR_TESTS);
    write_queue.Enqueue(MEDIUM, spdy::SpdyFrameType::HEADERS, std::move(producer_medium), stream_medium->GetWeakPtr(), TRAFFIC_ANNOTATION_FOR_TESTS);
    write_queue.Enqueue(HIGHEST, spdy::SpdyFrameType::RST_STREAM, std::move(producer_highest), stream_highest->GetWeakPtr(), TRAFFIC_ANNOTATION_FOR_TESTS);

    spdy::SpdyFrameType frame_type = spdy::SpdyFrameType::DATA;
    std::unique_ptr<SpdyBufferProducer> frame_producer;
    base::WeakPtr<SpdyStream> stream;
    MutableNetworkTrafficAnnotationTag traffic_annotation;
    ASSERT_TRUE(write_queue.Dequeue(&frame_type, &frame_producer, &stream, &traffic_annotation));
    ...
    EXPECT_FALSE(write_queue.Dequeue(&frame_type, &frame_producer, &stream, &traffic_annotation));
}
...
TEST_F(SpdyWriteQueueTest, DequeuesFIFO)
{
    ...
    write_queue.Enqueue(DEFAULT_PRIORITY, spdy::SpdyFrameType::HEADERS, std::move(producer1), stream1->GetWeakPtr(), TRAFFIC_ANNOTATION_FOR_TESTS);
    write_queue.Enqueue(DEFAULT_PRIORITY, spdy::SpdyFrameType::HEADERS, std::move(producer2), stream2->GetWeakPtr(), TRAFFIC_ANNOTATION_FOR_TESTS);
    write_queue.Enqueue(DEFAULT_PRIORITY, spdy::SpdyFrameType::RST_STREAM, std::move(producer3), stream3->GetWeakPtr(), TRAFFIC_ANNOTATION_FOR_TESTS);
...
TEST_F(SpdyWriteQueueTest, RemovePendingWritesForStream)
{
    ...
    for (int i = 0; i < 100; ++i) {
        ...
        write_queue.Enqueue(DEFAULT_PRIORITY, spdy::SpdyFrameType::HEADERS, IntToProducer(i), stream, TRAFFIC_ANNOTATION_FOR_TESTS);
    }
    ...
    for (int i = 0; i < 100; i += 3) {
        ...
        EXPECT_EQ(stream1.get(), stream.get());
        EXPECT_EQ(MutableNetworkTrafficAnnotationTag(TRAFFIC_ANNOTATION_FOR_TESTS), traffic_annotation);
    }
...
TEST_F(SpdyWriteQueueTest, RemovePendingWritesForStreamsAfter)
{
    ...
    base::WeakPtr<SpdyStream> streams[] = { stream1->GetWeakPtr(), stream2->GetWeakPtr(), stream3->GetWeakPtr(), stream4->GetWeakPtr() };

    for (int i = 0; i < 100; ++i) {
        write_queue.Enqueue(DEFAULT_PRIORITY, spdy::SpdyFrameType::HEADERS, IntToProducer(i), streams[i % std::size(streams)], TRAFFIC_ANNOTATION_FOR_TESTS);
    }
    ...
    for (int i = 0; i < 100; i += std::size(streams)) {
        ...
        base::WeakPtr<SpdyStream> stream;
        MutableNetworkTrafficAnnotationTag traffic_annotation;
        ASSERT_TRUE(write_queue.Dequeue(&frame_type, &frame_producer, &stream, &traffic_annotation)) << "Unable to Dequeue i: " << i;
        EXPECT_EQ(spdy::SpdyFrameType::HEADERS, frame_type);
        EXPECT_EQ(i, ProducerToInt(std::move(frame_producer)));
        EXPECT_EQ(stream1.get(), stream.get());
        EXPECT_EQ(MutableNetworkTrafficAnnotationTag(TRAFFIC_ANNOTATION_FOR_TESTS), traffic_annotation);
...
TEST_F(SpdyWriteQueueTest, Clear)
{
    ...
    for (int i = 0; i < 100; ++i) {
        write_queue.Enqueue(DEFAULT_PRIORITY, spdy::SpdyFrameType::HEADERS, IntToProducer(i), base::WeakPtr<SpdyStream>(), TRAFFIC_ANNOTATION_FOR_TESTS);
    }
...
TEST_F(SpdyWriteQueueTest, RequeingProducerWithoutReentrance)
{
    ...
    queue.Enqueue(DEFAULT_PRIORITY, spdy::SpdyFrameType::HEADERS, std::make_unique<RequeingBufferProducer>(&queue), base::WeakPtr<SpdyStream>(),
        TRAFFIC_ANNOTATION_FOR_TESTS);
    {
        ...
        EXPECT_TRUE(queue.Dequeue(&frame_type, &producer, &stream, &traffic_annotation));
        EXPECT_TRUE(queue.IsEmpty());
        EXPECT_EQ(std::string(kOriginal), producer->ProduceBuffer()->GetRemainingData());
    }
    ...
    EXPECT_TRUE(queue.Dequeue(&frame_type, &producer, &stream, &traffic_annotation));
    EXPECT_EQ(std::string(kRequeued), producer->ProduceBuffer()->GetRemainingData());
}
...
TEST_F(SpdyWriteQueueTest, ReentranceOnRemovePendingWritesAfter)
{
    ...
    SpdyWriteQueue queue;
    queue.Enqueue(
        DEFAULT_PRIORITY, spdy::SpdyFrameType::HEADERS, std::make_unique<RequeingBufferProducer>(&queue), stream->GetWeakPtr(), TRAFFIC_ANNOTATION_FOR_TESTS);
    ...
    EXPECT_TRUE(queue.Dequeue(&frame_type, &producer, &weak_stream, &traffic_annotation));
    EXPECT_EQ(std::string(kRequeued), producer->ProduceBuffer()->GetRemainingData());
}
...
TEST_F(SpdyWriteQueueTest, ChangePriority)
{
    ...
    write_queue.Enqueue(HIGHEST, spdy::SpdyFrameType::HEADERS, std::move(producer1), stream1->GetWeakPtr(), TRAFFIC_ANNOTATION_FOR_TESTS);
    write_queue.Enqueue(MEDIUM, spdy::SpdyFrameType::DATA, std::move(producer2), stream2->GetWeakPtr(), TRAFFIC_ANNOTATION_FOR_TESTS);
    write_queue.Enqueue(LOW, spdy::SpdyFrameType::RST_STREAM, std::move(producer3), stream3->GetWeakPtr(), TRAFFIC_ANNOTATION_FOR_TESTS);
