...
SpdyProxyClientSocket::SpdyProxyClientSocket(const base::WeakPtr<SpdyStream>& spdy_stream, const ProxyChain& proxy_chain, size_t proxy_chain_index,
    const std::string& user_agent, const HostPortPair& endpoint, const NetLogWithSource& source_net_log, scoped_refptr<HttpAuthController> auth_controller,
    ...
    , net_log_(NetLogWithSource::Make(spdy_stream->net_log().net_log(), NetLogSourceType::PROXY_CLIENT_SOCKET))
    , source_dependency_(source_net_log.source())
{
    ...
    net_log_.AddEventReferencingSource(NetLogEventType::HTTP2_PROXY_CLIENT_SESSION, spdy_stream->net_log().source());
...
const scoped_refptr<HttpAuthController>& SpdyProxyClientSocket::GetAuthController() const
{
    return auth_;
}
...
int SpdyProxyClientSocket::Read(IOBuffer* buf, int buf_len, CompletionOnceCallback callback)
{
...
int SpdyProxyClientSocket::ReadIfReady(IOBuffer* buf, int buf_len, CompletionOnceCallback callback)
{
...
int SpdyProxyClientSocket::Write(IOBuffer* buf, int buf_len, CompletionOnceCallback callback, const NetworkTrafficAnnotationTag& traffic_annotation)
{
...
void SpdyProxyClientSocket::RunWriteCallback(int result)
{
    ...
    if (end_stream_state_ == EndStreamState::kEndStreamReceived) {
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
            FROM_HERE, base::BindOnce(&SpdyProxyClientSocket::MaybeSendEndStream, weak_factory_.GetMutableWeakPtr()));
    }
}
...
int SpdyProxyClientSocket::DoGenerateAuthToken()
{
    ...
    return auth_->MaybeGenerateAuthToken(&request_, base::BindOnce(&SpdyProxyClientSocket::OnIOComplete, weak_factory_.GetWeakPtr()), net_log_);
}
...
int SpdyProxyClientSocket::DoSendRequest()
{
    ...
    return spdy_stream_->SendRequestHeaders(std::move(headers), MORE_DATA_TO_SEND);
}
...
void SpdyProxyClientSocket::OnEarlyHintsReceived(const quiche::HttpHeaderBlock& headers)
{
}

void SpdyProxyClientSocket::OnHeadersReceived(const quiche::HttpHeaderBlock& response_headers)
{
...
{
    if (buffer) {
        net_log_.AddByteTransferEvent(NetLogEventType::SOCKET_BYTES_RECEIVED, buffer->GetRemainingSize(), buffer->GetRemainingData());
        ...
        if (end_stream_state_ == EndStreamState::kNone) {
            ...
            base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
                FROM_HERE, base::BindOnce(&SpdyProxyClientSocket::MaybeSendEndStream, weak_factory_.GetWeakPtr()));
        }
    }
...
void SpdyProxyClientSocket::OnDataSent()
{
    ...
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
        FROM_HERE, base::BindOnce(&SpdyProxyClientSocket::RunWriteCallback, weak_factory_.GetWeakPtr(), rv));
}

void SpdyProxyClientSocket::OnTrailers(const quiche::HttpHeaderBlock& trailers)
{
