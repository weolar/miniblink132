...
SpdyHttpStream::SpdyHttpStream(const base::WeakPtr<SpdySession>& spdy_session, NetLogSource source_dependency, std::set<std::string> dns_aliases)
    : MultiplexedHttpStream(std::make_unique<MultiplexedSessionHandle>(spdy_session))
...
int SpdyHttpStream::InitializeStream(bool can_send_early, RequestPriority priority, const NetLogWithSource& stream_net_log, CompletionOnceCallback callback)
{
    DCHECK(!stream_);
    ...
    priority_ = priority;
    int rv = stream_request_.StartRequest(SPDY_REQUEST_RESPONSE_STREAM, spdy_session_, request_info_->url, can_send_early, priority, request_info_->socket_tag,
        stream_net_log, base::BindOnce(&SpdyHttpStream::OnStreamCreated, weak_factory_.GetWeakPtr(), std::move(callback)),
...
int SpdyHttpStream::ReadResponseBody(IOBuffer* buf, int buf_len, CompletionOnceCallback callback)
{
    if (stream_)
        CHECK(!stream_->IsIdle());
...
bool SpdyHttpStream::GetAlternativeService(AlternativeService* alternative_service) const
{
    return false;
}
...
bool SpdyHttpStream::GetLoadTimingInfo(LoadTimingInfo* load_timing_info) const
{
    ...
    // include that time.
    if (!load_timing_info->connect_timing.ssl_end.is_null() && !stream_request_.confirm_handshake_end().is_null()) {
        load_timing_info->connect_timing.ssl_end = stream_request_.confirm_handshake_end();
        load_timing_info->connect_timing.connect_end = stream_request_.confirm_handshake_end();
    }

    return true;
}

int SpdyHttpStream::SendRequest(const HttpRequestHeaders& request_headers, HttpResponseInfo* response, CompletionOnceCallback callback)
{
    if (stream_closed_) {
        return closed_stream_status_;
    }
    ...
    if (HasUploadData()) {
        request_body_buf_ = base::MakeRefCounted<IOBufferWithSize>(kRequestBodyBufferSize);
    ...
    CreateSpdyHeadersFromHttpRequest(*request_info_, priority_, request_headers, &headers);
    DispatchRequestHeadersCallback(headers);

    bool will_send_data = HasUploadData() || spdy_session_->EndStreamWithDataFrame();
    result = stream_->SendRequestHeaders(std::move(headers), will_send_data ? MORE_DATA_TO_SEND : NO_MORE_DATA_TO_SEND);
...
void SpdyHttpStream::OnEarlyHintsReceived(const quiche::HttpHeaderBlock& headers)
{
...
void SpdyHttpStream::OnHeadersReceived(const quiche::HttpHeaderBlock& response_headers)
{
    DCHECK(!response_headers_complete_);
    DCHECK(response_info_);
    ...
    response_info_->response_time = response_info_->original_response_time = stream_->response_time();
...
void SpdyHttpStream::ReadAndSendRequestBodyData()
{
    ...
    const int rv = request_info_->upload_data_stream->Read(
        request_body_buf_.get(), request_body_buf_->size(), base::BindOnce(&SpdyHttpStream::OnRequestBodyReadCompleted, weak_factory_.GetWeakPtr()));
...
void SpdyHttpStream::OnRequestBodyReadCompleted(int status)
{
    if (status < 0) {
        ...
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
            FROM_HERE, base::BindOnce(&SpdyHttpStream::ResetStream, weak_factory_.GetWeakPtr(), status));
    ...
    stream_->SendData(request_body_buf_.get(), request_body_buf_size_, eof ? NO_MORE_DATA_TO_SEND : MORE_DATA_TO_SEND);
}
...
void SpdyHttpStream::MaybeScheduleBufferedReadCallback()
{
    ...
    if (response_body_queue_.GetTotalSize() >= static_cast<size_t>(user_buffer_len_)) {
        buffered_read_timer_.Start(FROM_HERE, base::TimeDelta() /* no delay */, this, &SpdyHttpStream::DoBufferedReadCallback);
        return;
    }
    ...
    buffered_read_timer_.Start(FROM_HERE, base::Milliseconds(1), this, &SpdyHttpStream::DoBufferedReadCallback);
}
...
void SpdyHttpStream::DoBufferedReadCallback()
{
    ...
    if (!response_body_queue_.IsEmpty()) {
        int rv = response_body_queue_.Dequeue(user_buffer_->data(), user_buffer_len_);
...
void SpdyHttpStream::MaybePostRequestCallback(int rv)
{
    ...
    if (request_callback_)
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
            FROM_HERE, base::BindOnce(&SpdyHttpStream::MaybeDoRequestCallback, weak_factory_.GetWeakPtr(), rv));
}
