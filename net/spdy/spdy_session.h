...
// idle.
constexpr base::TimeDelta kDefaultTimeToBufferSmallWindowUpdates = base::Seconds(5);
...
enum SpdyProtocolErrorDetails {
    ...
    SPDY_ERROR_HPACK_INITIAL_DYNAMIC_TABLE_SIZE_UPDATE_IS_ABOVE_LOW_WATER_MARK = 54,
...
SpdyProtocolErrorDetails NET_EXPORT_PRIVATE MapFramerErrorToProtocolError(http2::Http2DecoderAdapter::SpdyFramerError error);
Error NET_EXPORT_PRIVATE MapFramerErrorToNetError(http2::Http2DecoderAdapter::SpdyFramerError error);
SpdyProtocolErrorDetails NET_EXPORT_PRIVATE MapRstStreamStatusToProtocolError(spdy::SpdyErrorCode error_code);
...
static_assert(28 == http2::Http2DecoderAdapter::LAST_ERROR, "SpdyProtocolErrorDetails / Spdy Errors mismatch");
static_assert(13 == spdy::SpdyErrorCode::ERROR_CODE_MAX, "SpdyProtocolErrorDetails / spdy::SpdyErrorCode mismatch");
...
public:
    ...
    int StartRequest(SpdyStreamType type, const base::WeakPtr<SpdySession>& session, const GURL& url, bool can_send_early, RequestPriority priority,
        const SocketTag& socket_tag, const NetLogWithSource& net_log, CompletionOnceCallback callback, const NetworkTrafficAnnotationTag& traffic_annotation,
        bool detect_broken_connection = false, base::TimeDelta heartbeat_interval = base::Seconds(0));
...
public:
    ...
    static bool CanPool(TransportSecurityState* transport_security_state, const SSLInfo& ssl_info, const SSLConfigService& ssl_config_service,
    ...
    SpdySession(const SpdySessionKey& spdy_session_key, HttpServerProperties* http_server_properties, TransportSecurityState* transport_security_state,
        ...
        bool enable_ping_based_connection_checking, bool is_http_enabled, bool is_quic_enabled, size_t session_max_recv_window_size,
        ...
        TimeFunc time_func, NetworkQualityEstimator* network_quality_estimator, NetLog* net_log,
    ...
    void InitializeWithSocketHandle(std::unique_ptr<StreamSocketHandle> stream_socket_handle, SpdySessionPool* pool);
    ...
    // itself.
    void InitializeWithSocket(std::unique_ptr<StreamSocket> stream_socket, const LoadTimingInfo::ConnectTiming& connect_timing, SpdySessionPool* pool);
    ...
    void EnqueueStreamWrite(const base::WeakPtr<SpdyStream>& stream, spdy::SpdyFrameType frame_type, std::unique_ptr<SpdyBufferProducer> producer);
    ...
    std::unique_ptr<spdy::SpdySerializedFrame> CreateHeaders(
        spdy::SpdyStreamId stream_id, RequestPriority priority, spdy::SpdyControlFlags flags, quiche::HttpHeaderBlock headers, NetLogSource source_dependency);
    ...
    std::unique_ptr<SpdyBuffer> CreateDataBuffer(
    ...
    void UpdateStreamPriority(SpdyStream* stream, RequestPriority old_priority, RequestPriority new_priority);
    ...
    void ResetStream(spdy::SpdyStreamId stream_id, int error, const std::string& description);
    ...
    void SendStreamWindowUpdate(spdy::SpdyStreamId stream_id, uint32_t delta_window_size);
    ...
    {
        return availability_state_ == STATE_AVAILABLE;
    }
    bool IsGoingAway() const
    {
        return availability_state_ == STATE_GOING_AWAY;
    }
    bool IsDraining() const
    {
        return availability_state_ == STATE_DRAINING;
    }
    ...
    bool GetLoadTimingInfo(spdy::SpdyStreamId stream_id, LoadTimingInfo* load_timing_info) const;
    ...
    bool support_websocket() const
    {
        return support_websocket_;
    }
    ...
    {
        return session_send_window_size_ == 0;
    }
...
private:
    ...
    using PendingStreamRequestQueue = base::circular_deque<base::WeakPtr<SpdyStreamRequest>>;
    ...
    int TryCreateStream(const base::WeakPtr<SpdyStreamRequest>& request, base::WeakPtr<SpdyStream>* stream);

    // Actually create a stream into |stream|. Returns OK if successful;
    // otherwise, returns an error and |stream| is not filled.
    int CreateStream(const SpdyStreamRequest& request, base::WeakPtr<SpdyStream>* stream);
    ...
    void ChangeStreamRequestPriority(const base::WeakPtr<SpdyStreamRequest>& request, RequestPriority priority);
    ...
    void ResetStreamIterator(ActiveStreamMap::iterator it, int status, const std::string& description);
    ...
    void EnqueueResetStreamFrame(spdy::SpdyStreamId stream_id, RequestPriority priority, spdy::SpdyErrorCode error_code, const std::string& description);

    // Send a PRIORITY frame with the given parameters.
    void EnqueuePriorityFrame(spdy::SpdyStreamId stream_id, spdy::SpdyStreamId dependency_id, int weight, bool exclusive);
    ...
    void SendWindowUpdateFrame(spdy::SpdyStreamId stream_id, uint32_t delta_window_size, RequestPriority priority);
    ...
    void EnqueueSessionWrite(RequestPriority priority, spdy::SpdyFrameType frame_type, std::unique_ptr<spdy::SpdySerializedFrame> frame);

    // Puts |producer| associated with |stream| onto the write queue
    // with the given priority.
    void EnqueueWrite(RequestPriority priority, spdy::SpdyFrameType frame_type, std::unique_ptr<SpdyBufferProducer> producer,
    ...
    void LogAbandonedActiveStream(ActiveStreamMap::const_iterator it, Error status);

    // Invokes a user callback for stream creation.  We provide this method so it
    // can be deferred to the MessageLoop, so we avoid re-entrancy problems.
    void CompleteStreamRequest(const base::WeakPtr<SpdyStreamRequest>& pending_request);
    ...
    void OnSendCompressedFrame(spdy::SpdyStreamId stream_id, spdy::SpdyFrameType type, size_t payload_len, size_t frame_len) override;
    void OnReceiveCompressedFrame(spdy::SpdyStreamId stream_id, spdy::SpdyFrameType type, size_t frame_len) override;
    ...
    void OnWriteBufferConsumed(size_t frame_payload_size, size_t consume_size, SpdyBuffer::ConsumeSource consume_source);
    ...
    //
    // If session flow control is turned off, this must not be called.
    void OnReadBufferConsumed(size_t consume_size, SpdyBuffer::ConsumeSource consume_source);
    ...
    // in the past.
    base::circular_deque<spdy::SpdyStreamId> stream_send_unstall_queue_[NUM_PRIORITIES];
