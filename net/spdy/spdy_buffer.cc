...
std::unique_ptr<spdy::SpdySerializedFrame> MakeSpdySerializedFrame(const char* data, size_t size)
{
    DCHECK(data);
    CHECK_GT(size, 0u);
    ...
    return std::make_unique<spdy::SpdySerializedFrame>(std::move(frame_data), size);
}
...
public:
    SharedFrameIOBuffer(const scoped_refptr<SharedFrame>& shared_frame, size_t offset)
...
void SpdyBuffer::ConsumeHelper(size_t consume_size, ConsumeSource consume_source)
{
