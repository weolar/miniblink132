...
BidirectionalStreamSpdyImpl::BidirectionalStreamSpdyImpl(const base::WeakPtr<SpdySession>& spdy_session, NetLogSource source_dependency)
...
void BidirectionalStreamSpdyImpl::Start(const BidirectionalStreamRequestInfo* request_info, const NetLogWithSource& net_log,
    ...
    const NetworkTrafficAnnotationTag& traffic_annotation)
{
    ...
    if (!spdy_session_) {
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
            FROM_HERE, base::BindOnce(&BidirectionalStreamSpdyImpl::NotifyError, weak_factory_.GetWeakPtr(), ERR_CONNECTION_CLOSED));
        return;
    }

    request_info_ = request_info;

    int rv = stream_request_.StartRequest(SPDY_BIDIRECTIONAL_STREAM, spdy_session_, request_info_->url, false /* no early data */, request_info_->priority,
        request_info_->socket_tag, net_log, base::BindOnce(&BidirectionalStreamSpdyImpl::OnStreamInitialized, weak_factory_.GetWeakPtr()), traffic_annotation,
        request_info_->detect_broken_connection, request_info_->heartbeat_interval);
...
void BidirectionalStreamSpdyImpl::SendvData(const std::vector<scoped_refptr<IOBuffer>>& buffers, const std::vector<int>& lengths, bool end_stream)
{
    ...
    if (written_end_of_stream_) {
        ...
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
            FROM_HERE, base::BindOnce(&BidirectionalStreamSpdyImpl::NotifyError, weak_factory_.GetWeakPtr(), ERR_UNEXPECTED));
        return;
    }
    ...
    if (buffers.size() == 1) {
        ...
        pending_combined_buffer_ = base::MakeRefCounted<net::IOBufferWithSize>(total_len);
        int len = 0;
        // TODO(xunjieli): Get rid of extra copy. Coalesce headers and data frames.
        for (size_t i = 0; i < buffers.size(); ++i) {
            memcpy(pending_combined_buffer_->data() + len, buffers[i]->data(), lengths[i]);
            len += lengths[i];
        }
    }
    stream_->SendData(pending_combined_buffer_.get(), total_len, end_stream ? NO_MORE_DATA_TO_SEND : MORE_DATA_TO_SEND);
}
...
bool BidirectionalStreamSpdyImpl::GetLoadTimingInfo(LoadTimingInfo* load_timing_info) const
{
    if (stream_closed_) {
...
void BidirectionalStreamSpdyImpl::PopulateNetErrorDetails(NetErrorDetails* details)
{
}
...
void BidirectionalStreamSpdyImpl::OnEarlyHintsReceived(const quiche::HttpHeaderBlock& headers)
{
    DCHECK(stream_);
    // TODO(crbug.com/40496584): Plumb Early Hints to `delegate_` if needed.
}

void BidirectionalStreamSpdyImpl::OnHeadersReceived(const quiche::HttpHeaderBlock& response_headers)
{
    DCHECK(stream_);
...
void BidirectionalStreamSpdyImpl::OnDataReceived(std::unique_ptr<SpdyBuffer> buffer)
{
    DCHECK(stream_);
    DCHECK(!stream_closed_);
...
void BidirectionalStreamSpdyImpl::OnTrailers(const quiche::HttpHeaderBlock& trailers)
{
    DCHECK(stream_);
    DCHECK(!stream_closed_);
...
void BidirectionalStreamSpdyImpl::OnClose(int status)
{
    ...
    closed_has_load_timing_info_ = stream_->GetLoadTimingInfo(&closed_load_timing_info_);
...
void BidirectionalStreamSpdyImpl::ScheduleBufferedRead()
{
    ...
    timer_->Start(FROM_HERE, base::Milliseconds(kBufferTimeMs), base::BindOnce(&BidirectionalStreamSpdyImpl::DoBufferedRead, weak_factory_.GetWeakPtr()));
}
...
bool BidirectionalStreamSpdyImpl::MaybeHandleStreamClosedInSendData()
{
    ...
    if (stream_closed_ && closed_stream_status_ == OK) {
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
            FROM_HERE, base::BindOnce(&BidirectionalStreamSpdyImpl::OnDataSent, weak_factory_.GetWeakPtr()));
        return true;
    }
