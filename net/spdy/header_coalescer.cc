...
void NetLogInvalidHeader(const NetLogWithSource& net_log, std::string_view header_name, std::string_view header_value, const char* error_message)
{
    net_log.AddEvent(NetLogEventType::HTTP2_SESSION_RECV_INVALID_HEADER, [&](NetLogCaptureMode capture_mode) {
        return base::Value::Dict()
            .Set("header_name", NetLogStringValue(header_name))
            .Set("header_value", NetLogStringValue(ElideHeaderValueForNetLog(capture_mode, std::string(header_name), std::string(header_value))))
...
HeaderCoalescer::HeaderCoalescer(uint32_t max_header_list_size, const NetLogWithSource& net_log)
...
bool HeaderCoalescer::AddHeader(std::string_view key, std::string_view value)
{
    ...
    if (key[0] == ':') {
        if (regular_header_seen_) {
            NetLogInvalidHeader(net_log_, key, value, "Pseudo header must not follow regular headers.");
            return false;
        }
    ...
    if (!HttpUtil::IsValidHeaderName(key_name)) {
        NetLogInvalidHeader(net_log_, key, value, "Invalid character in header name.");
        return false;
    }

    if (ContainsUppercaseAscii(key_name)) {
        NetLogInvalidHeader(net_log_, key, value, "Upper case characters in header name.");
        return false;
    }
    ...
    for (const unsigned char c : value) {
        if (c < '\t' || ('\t' < c && c < 0x20) || c == 0x7f) {
            ...
            base::StringAppendF(&error_line, "Invalid character 0x%02X in header value.", c);
