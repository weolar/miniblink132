...
public:
    TestDelegate() = delete;
    explicit TestDelegate(const SpdySessionKey& key)
        : key_(key)
    {
    }
    ~TestDelegate() override = default;

    bool ValidatePushedStream(spdy::SpdyStreamId stream_id, const GURL& url, const HttpRequestInfo& request_info, const SpdySessionKey& key) const override
    {
        return key == key_;
    }
...
protected:
    Http2PushPromiseIndexTest()
        ...
        , key1_(HostPortPair::FromURL(url1_), ProxyServer::Direct(), PRIVACY_MODE_ENABLED, SpdySessionKey::IsProxySession::kFalse, SocketTag(),
              NetworkAnonymizationKey(), SecureDnsPolicy::kAllow)
        , key2_(HostPortPair::FromURL(url2_), ProxyServer::Direct(), PRIVACY_MODE_ENABLED, SpdySessionKey::IsProxySession::kFalse, SocketTag(),
              NetworkAnonymizationKey(), SecureDnsPolicy::kAllow)
    {
    }
...
TEST_F(Http2PushPromiseIndexTest, Empty)
{
    ...
    index_.ClaimPushedStream(key1_, url1_, HttpRequestInfo(), &session, &stream_id);
    EXPECT_EQ(kNoPushedStreamFound, stream_id);

    stream_id = 2;
    index_.ClaimPushedStream(key1_, url2_, HttpRequestInfo(), &session, &stream_id);
    EXPECT_EQ(kNoPushedStreamFound, stream_id);
    ...
    stream_id = 2;
    index_.ClaimPushedStream(key2_, url2_, HttpRequestInfo(), &session, &stream_id);
    EXPECT_EQ(kNoPushedStreamFound, stream_id);
}
...
TEST_F(Http2PushPromiseIndexTest, MultipleStreamsWithDifferentKeys)
{
    ...
    base::WeakPtr<SpdySession> session;
    spdy::SpdyStreamId stream_id = 2;
    index_.ClaimPushedStream(key2_, url1_, HttpRequestInfo(), &session, &stream_id);
    EXPECT_EQ(kNoPushedStreamFound, stream_id);
...
TEST_F(Http2PushPromiseIndexTest, MultipleMatchingStreams)
{
    ...
    index_.ClaimPushedStream(key1_, url1_, HttpRequestInfo(), &session, &stream_id1);
    ...
    index_.ClaimPushedStream(key1_, url1_, HttpRequestInfo(), &session, &stream_id2);
    ...
    index_.ClaimPushedStream(key1_, url1_, HttpRequestInfo(), &session, &stream_id3);
...
TEST(Http2PushPromiseIndexCompareByUrlTest, Reflexivity)
{
    ...
    Http2PushPromiseIndexPeer::UnclaimedPushedStream entry2 { GURL(), nullptr, kNoPushedStreamFound };
    ...
    std::set<Http2PushPromiseIndexPeer::UnclaimedPushedStream, Http2PushPromiseIndexPeer::CompareByUrl> entries;
    bool success;
...
TEST(Http2PushPromiseIndexCompareByUrlTest, LookupByURL)
{
    ...
    EXPECT_TRUE(entries.lower_bound(Http2PushPromiseIndexPeer::UnclaimedPushedStream { url2, nullptr, kNoPushedStreamFound }) == entries.find(entry2));
}
