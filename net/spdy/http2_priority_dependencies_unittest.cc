...
bool operator==(const Http2PriorityDependencies::DependencyUpdate& a, const Http2PriorityDependencies::DependencyUpdate& b)
{
    return a.id == b.id && a.parent_stream_id == b.parent_stream_id && a.weight == b.weight && a.exclusive == b.exclusive;
}

std::ostream& operator<<(std::ostream& os, const std::vector<Http2PriorityDependencies::DependencyUpdate>& v)
{
    for (auto e : v) {
        os << "{" << e.id << "," << e.parent_stream_id << "," << e.weight << "," << (e.exclusive ? "true" : "false") << "}";
    }
    return os;
}
...
public:
    ...
    spdy::SpdyStreamId GetId()
    {
        return ++next_id_;
    }

    void TestStreamCreation(spdy::SpdyStreamId new_id, spdy::SpdyPriority priority, spdy::SpdyStreamId expected_parent_id)
    {
        ...
        dependency_state_.OnStreamCreation(new_id, priority, &parent_id, &weight, &exclusive);
        if (expected_parent_id != parent_id || !exclusive || expected_weight != weight) {
    ...
    void TestStreamUpdate(spdy::SpdyStreamId id, spdy::SpdyPriority new_priority, std::vector<ExpectedDependencyUpdate> expected)
    {
        ...
        for (auto e : expected) {
            expected_value.push_back({ e.id, e.parent_id, e.weight, true /* exclusive */ });
        }
...
TEST_F(HttpPriorityDependencyTest, UpdateThreeStreams)
{
    ...
    TestStreamUpdate(first_id, MEDIUM, { { second_id, 0, medium_weight }, { first_id, second_id, medium_weight } });

    // third moves to top.
    TestStreamUpdate(third_id, HIGHEST, { { third_id, 0, highest_weight } });

    // third moves to bottom.
    TestStreamUpdate(third_id, LOWEST, { { second_id, 0, medium_weight }, { third_id, first_id, lowest_weight } });

    // first moves to top.
    TestStreamUpdate(first_id, HIGHEST, { { third_id, second_id, lowest_weight }, { first_id, 0, highest_weight } });
}
...
TEST_F(HttpPriorityDependencyTest, UpdateComplex)
{
    ...
    // 3 moves under 7
    // 2 moves under 4
    TestStreamUpdate(second_id, HIGHEST, { { third_id, seventh_id, lowest_weight }, { second_id, fourth_id, highest_weight } });
