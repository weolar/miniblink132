...
public:
    TestDelegateBase(base::WeakPtr<SpdySession> session, IOBuffer* read_buf, int read_buf_len)
        : stream_(std::make_unique<BidirectionalStreamSpdyImpl>(session, NetLogSource()))
        , read_buf_(read_buf)
    ...
    void Start(const BidirectionalStreamRequestInfo* request, const NetLogWithSource& net_log)
    {
        stream_->Start(request, net_log,
            /*send_request_headers_automatically=*/false, this, std::make_unique<base::OneShotTimer>(), TRAFFIC_ANNOTATION_FOR_TESTS);
...
class BidirectionalStreamSpdyImplTest : public testing::TestWithParam<bool>, public WithTaskEnvironment {
public:
...
protected:
    ...
    void InitSession(base::span<const MockRead> reads, base::span<const MockWrite> writes)
    {
        ...
        session_ = CreateSpdySession(http_session_.get(), key_, net_log_with_source_);
    }
...
TEST_F(BidirectionalStreamSpdyImplTest, SimplePostRequest)
{
    ...
    spdy::SpdySerializedFrame data_frame(spdy_util_.ConstructSpdyDataFrame(1, std::string_view(kBodyData, kBodyDataSize), /*fin=*/true));
    MockWrite writes[] = {
    ...
    request_info.extra_headers.SetHeader(net::HttpRequestHeaders::kContentLength, base::NumberToString(kBodyDataSize));

    auto read_buffer = base::MakeRefCounted<IOBufferWithSize>(kReadBufferSize);
    auto delegate = std::make_unique<TestDelegateBase>(session_, read_buffer.get(), kReadBufferSize);
    delegate->SetRunUntilCompletion(true);
    delegate->Start(&request_info, net_log_with_source_);
    sequenced_data_->RunUntilPaused();

    scoped_refptr<StringIOBuffer> write_buffer = base::MakeRefCounted<StringIOBuffer>(std::string(kBodyData, kBodyDataSize));
...
TEST_F(BidirectionalStreamSpdyImplTest, LoadTimingTwoRequests)
{
    ...
    auto delegate = std::make_unique<TestDelegateBase>(session_, read_buffer.get(), kReadBufferSize);
    auto delegate2 = std::make_unique<TestDelegateBase>(session_, read_buffer2.get(), kReadBufferSize);
    delegate->SetRunUntilCompletion(true);
...
TEST_F(BidirectionalStreamSpdyImplTest, SendDataAfterStreamFailed)
{
    ...
    request_info.method = "POST";
    request_info.url = default_url_;
    request_info.extra_headers.SetHeader(net::HttpRequestHeaders::kContentLength, base::NumberToString(kBodyDataSize * 3));
    ...
    scoped_refptr<StringIOBuffer> buf = base::MakeRefCounted<StringIOBuffer>("dummy");
...
INSTANTIATE_TEST_SUITE_P(BidirectionalStreamSpdyImplTests, BidirectionalStreamSpdyImplTest, ::testing::Bool());
...
TEST_P(BidirectionalStreamSpdyImplTest, RstWithNoErrorBeforeSendIsComplete)
{
    ...
    MockRead reads[] = { CreateMockRead(resp, 1), MockRead(ASYNC, ERR_IO_PENDING, 2), // Force a pause.
    ...
    if (is_test_sendv) {
        std::vector<scoped_refptr<IOBuffer>> three_buffers = { write_buffer.get(), write_buffer.get(), write_buffer.get() };
        std::vector<int> three_lengths = { write_buffer->size(), write_buffer->size(), write_buffer->size() };
