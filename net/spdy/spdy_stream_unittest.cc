...
protected:
    ...
    typedef base::OnceCallback<void(const base::WeakPtr<SpdyStream>&, int32_t)> UnstallFunction;

    explicit SpdyStreamTest(base::test::TaskEnvironment::TimeSource time_source = base::test::TaskEnvironment::TimeSource::DEFAULT)
    ...
    {
        SpdySessionKey key(HostPortPair::FromURL(url_), PRIVACY_MODE_DISABLED, ProxyChain::Direct(), SessionUsage::kDestination, SocketTag(),
    ...
    void RunResumeAfterUnstallRequestResponseTest(UnstallFunction unstall_function);
    ...
    void AddReadEOF()
    {
        reads_.emplace_back(ASYNC, 0, offset_++);
    }
    ...
    void AddReadPause()
    {
        reads_.emplace_back(ASYNC, ERR_IO_PENDING, offset_++);
    }
    ...
    static SpdySessionPool* spdy_session_pool(base::WeakPtr<SpdySession> session)
    {
...
{
    spdy::SpdySerializedFrame req(spdy_util_.ConstructSpdyPost(kDefaultUrl, 1, kPostBodyLength, LOWEST, nullptr, 0));
    AddWrite(req);

    spdy::SpdySerializedFrame resp(spdy_util_.ConstructSpdyPostReply(nullptr, 0));
    AddRead(resp);

    spdy::SpdySerializedFrame msg(spdy_util_.ConstructSpdyDataFrame(1, kPostBodyStringPiece, false));
    AddWrite(msg);

    spdy::SpdySerializedFrame echo(spdy_util_.ConstructSpdyDataFrame(1, kPostBodyStringPiece, false));
    AddRead(echo);
    ...
    base::WeakPtr<SpdyStream> stream = CreateStreamSynchronously(SPDY_BIDIRECTIONAL_STREAM, session, url_, LOWEST, NetLogWithSource());
    ASSERT_TRUE(stream);
    ...
    quiche::HttpHeaderBlock headers(spdy_util_.ConstructPostHeaderBlock(kDefaultUrl, kPostBodyLength));
    EXPECT_THAT(stream->SendRequestHeaders(std::move(headers), MORE_DATA_TO_SEND), IsError(ERR_IO_PENDING));
    ...
    EXPECT_EQ(std::string(kPostBody, kPostBodyLength), delegate.TakeReceivedData());
...
TEST_F(SpdyStreamTest, BrokenConnectionDetectionSuccessfulRequest)
{
    ...
    base::WeakPtr<SpdyStream> stream = CreateStreamSynchronously(SPDY_BIDIRECTIONAL_STREAM, session, url_, LOWEST, NetLogWithSource(), true, base::Seconds(10));
    ASSERT_TRUE(stream);
...
public:
    StreamDelegateWithTrailers(const base::WeakPtr<SpdyStream>& stream, std::string_view data)
...
TEST_F(SpdyStreamTest, Trailers)
{
    ...
    spdy::SpdySerializedFrame msg(spdy_util_.ConstructSpdyDataFrame(1, kPostBodyStringPiece, true));
    AddWrite(msg);
    ...
    spdy::SpdySerializedFrame trailers(spdy_util_.ConstructSpdyResponseHeaders(1, std::move(late_headers), false));
    AddRead(trailers);
    ...
    base::WeakPtr<SpdyStream> stream = CreateStreamSynchronously(SPDY_REQUEST_RESPONSE_STREAM, session, url_, LOWEST, NetLogWithSource());
    ASSERT_TRUE(stream);
    EXPECT_EQ(kDefaultUrl, stream->url().spec());
...
TEST_F(SpdyStreamTest, StreamError)
{
    ...
    base::WeakPtr<SpdyStream> stream
        = CreateStreamSynchronously(SPDY_BIDIRECTIONAL_STREAM, session, url_, LOWEST, NetLogWithSource::Make(NetLogSourceType::NONE));
    ASSERT_TRUE(stream);
    EXPECT_EQ(kDefaultUrl, stream->url().spec());
    ...
    int pos = ExpectLogContainsSomewhere(entries, 0, NetLogEventType::HTTP2_STREAM_ERROR, NetLogEventPhase::NONE);

    EXPECT_EQ(static_cast<int>(stream_id), GetIntegerValueFromParams(entries[pos], "stream_id"));
}
...
TEST_F(SpdyStreamTest, SendLargeDataAfterOpenRequestResponse)
{
    ...
    spdy::SpdySerializedFrame chunk(spdy_util_.ConstructSpdyDataFrame(1, chunk_data, false));
    AddWrite(chunk);
    AddWrite(chunk);

    spdy::SpdySerializedFrame last_chunk(spdy_util_.ConstructSpdyDataFrame(1, chunk_data, true));
    AddWrite(last_chunk);
...
TEST_F(SpdyStreamTest, UpperCaseHeaders)
{
    ...
    spdy::SpdySerializedFrame reply(spdy_util_.ConstructSpdyGetReply(kExtraHeaders, std::size(kExtraHeaders) / 2, 1));
    AddRead(reply);
    ...
    EXPECT_THAT(stream->SendRequestHeaders(std::move(headers), NO_MORE_DATA_TO_SEND), IsError(ERR_IO_PENDING));
...
TEST_F(SpdyStreamTest, HeadersMustHaveStatus)
{
    ...
    spdy::SpdySerializedFrame reply(spdy_util_.ConstructSpdyReply(1, std::move(header_block_without_status)));
    AddRead(reply);
...
TEST_F(SpdyStreamTest, TrailersMustNotFollowTrailers)
{
    ...
    spdy::SpdySerializedFrame body(spdy_util_.ConstructSpdyDataFrame(1, kPostBodyStringPiece, false));
    AddRead(body);

    quiche::HttpHeaderBlock trailers_block;
    trailers_block["foo"] = "bar";
    spdy::SpdySerializedFrame first_trailers(spdy_util_.ConstructSpdyResponseHeaders(1, std::move(trailers_block), false));
    AddRead(first_trailers);

    // Trailers following trailers: procotol error.
    spdy::SpdySerializedFrame second_trailers(spdy_util_.ConstructSpdyResponseHeaders(1, std::move(trailers_block), true));
...
TEST_F(SpdyStreamTest, DataMustNotFollowTrailers)
{
    ...
    quiche::HttpHeaderBlock trailers_block;
    trailers_block["foo"] = "bar";
    spdy::SpdySerializedFrame trailers(spdy_util_.ConstructSpdyResponseHeaders(1, std::move(trailers_block), false));
    AddRead(trailers);
...
public:
    ...
    void Initialize()
    {
        ...
        base::WeakPtr<SpdySession> session(CreateDefaultSpdySession());
        stream_ = CreateStreamSynchronously(SPDY_REQUEST_RESPONSE_STREAM, session, url_, LOWEST, NetLogWithSource());
        ASSERT_TRUE(stream_);
    ...
    StreamDelegateDoNothing& delegate()
    {
        return *delegate_;
    }
...
TEST_F(SpdyStreamTestWithMockClock, NonInformationalResponseStart)
{
    ...
    spdy::SpdySerializedFrame body(spdy_util_.ConstructSpdyDataFrame(1, kPostBodyStringPiece, true));
    AddRead(body);
    AddReadEOF();
    ...
    EXPECT_EQ(load_timing_info.receive_headers_start, expected_receive_headers_start_time);
    EXPECT_EQ(load_timing_info.receive_non_informational_headers_start, expected_receive_headers_start_time);
}
...
TEST_F(SpdyStreamTestWithMockClock, InformationalHeaders)
{
    ...
    spdy::SpdySerializedFrame informational_response(spdy_util_.ConstructSpdyResponseHeaders(1, std::move(informational_headers), false));
    // Separate the headers into 2 fragments and add pauses between the fragments
    // so that the test runner can advance the mock clock to test timing
    // information.
    ...
    // time.
    base::TimeTicks expected_receive_non_informational_headers_start_time = base::TimeTicks::Now();
    ...
    EXPECT_EQ(load_timing_info.receive_non_informational_headers_start, expected_receive_non_informational_headers_start_time);
    // The first response start time should be earlier than the non-informational
    // response start time.
    EXPECT_LT(load_timing_info.receive_headers_start, load_timing_info.receive_non_informational_headers_start);
}
...
TEST_F(SpdyStreamTestWithMockClock, EarlyHints)
{
    ...
    spdy::SpdySerializedFrame informational_response1(spdy_util_.ConstructSpdyResponseHeaders(1, std::move(informational_headers1), false));
    ...
    spdy::SpdySerializedFrame informational_response2(spdy_util_.ConstructSpdyResponseHeaders(1, std::move(informational_headers2), false));
    ...
    const std::vector<quiche::HttpHeaderBlock>& early_hints = delegate().early_hints();
    EXPECT_EQ(early_hints.size(), static_cast<size_t>(kNumberOfInformationalResponses));
    {
        const quiche::HttpHeaderBlock& hint = delegate().early_hints()[0];
        quiche::HttpHeaderBlock::const_iterator status_iterator = hint.find(spdy::kHttp2StatusHeader);
        ASSERT_TRUE(status_iterator != hint.end());
        EXPECT_EQ(status_iterator->second, "103");

        quiche::HttpHeaderBlock::const_iterator link_header_iterator = hint.find("link");
    ...
    EXPECT_EQ(load_timing_info.first_early_hints_time, expected_receive_headers_start_time);
    // The non-informational response start time should be captured at the time
    // the first header fragment of the non-informational response is received.
    EXPECT_EQ(load_timing_info.receive_non_informational_headers_start, expected_receive_non_informational_headers_start_time);
...
TEST_F(SpdyStreamTest, StatusMustBeNumber)
{
    ...
    spdy::SpdySerializedFrame reply(spdy_util_.ConstructSpdyResponseHeaders(1, std::move(incorrect_headers), false));
    AddRead(reply);
...
TEST_F(SpdyStreamTest, StatusCannotHaveExtraText)
{
    ...
    headers_with_status_text[":status"] = "200 Some random extra text describing status";
    spdy::SpdySerializedFrame reply(spdy_util_.ConstructSpdyResponseHeaders(1, std::move(headers_with_status_text), false));
    AddRead(reply);
...
TEST_F(SpdyStreamTest, StatusMustBePresent)
{
    ...
    spdy::SpdySerializedFrame reply(spdy_util_.ConstructSpdyResponseHeaders(1, std::move(headers_without_status), false));
    AddRead(reply);
...
void IncreaseStreamSendWindowSize(const base::WeakPtr<SpdyStream>& stream, int32_t delta_window_size)
{
...
void AdjustStreamSendWindowSize(const base::WeakPtr<SpdyStream>& stream, int32_t delta_window_size)
{
...
// and unstall.
void SpdyStreamTest::RunResumeAfterUnstallRequestResponseTest(UnstallFunction unstall_function)
{
    spdy::SpdySerializedFrame req(spdy_util_.ConstructSpdyPost(kDefaultUrl, 1, kPostBodyLength, LOWEST, nullptr, 0));
    AddWrite(req);
...
{
    RunResumeAfterUnstallRequestResponseTest(base::BindOnce(&IncreaseStreamSendWindowSize));
}

TEST_F(SpdyStreamTest, ResumeAfterSendWindowSizeAdjustRequestResponse)
{
    RunResumeAfterUnstallRequestResponseTest(base::BindOnce(&AdjustStreamSendWindowSize));
}
...
void SpdyStreamTest::RunResumeAfterUnstallBidirectionalTest(UnstallFunction unstall_function)
{
    spdy::SpdySerializedFrame req(spdy_util_.ConstructSpdyPost(kDefaultUrl, 1, kPostBodyLength, LOWEST, nullptr, 0));
    AddWrite(req);
...
{
    RunResumeAfterUnstallBidirectionalTest(base::BindOnce(&IncreaseStreamSendWindowSize));
}

TEST_F(SpdyStreamTest, ResumeAfterSendWindowSizeAdjustBidirectional)
{
    RunResumeAfterUnstallBidirectionalTest(base::BindOnce(&AdjustStreamSendWindowSize));
}
...
TEST_F(SpdyStreamTest, DataOnHalfClosedRemoveStream)
{
    ...
    spdy::SpdySerializedFrame data_frame(spdy_util_.ConstructSpdyDataFrame(1, kPostBodyStringPiece, true));
    AddRead(data_frame);

    spdy::SpdySerializedFrame rst(spdy_util_.ConstructSpdyRstStream(1, spdy::ERROR_CODE_STREAM_CLOSED));
    AddWrite(rst);
...
TEST_F(SpdyStreamTestWithMockClock, FlowControlSlowReads)
{
    ...
    base::WeakPtr<SpdySession> session(CreateDefaultSpdySession());
    session->SetTimeToBufferSmallWindowUpdates(kDefaultTimeToBufferSmallWindowUpdates);
