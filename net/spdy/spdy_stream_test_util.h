...
public:
    ...
    bool send_headers_completed() const
    {
        return send_headers_completed_;
    }
...
public:
    // |data| can be NULL.
    StreamDelegateSendImmediate(const base::WeakPtr<SpdyStream>& stream, std::string_view data);
...
public:
    StreamDelegateWithBody(const base::WeakPtr<SpdyStream>& stream, std::string_view data);
...
public:
    explicit StreamDelegateCloseOnHeaders(const base::WeakPtr<SpdyStream>& stream);
...
public:
    ...
    bool eof_detected() const
    {
        return eof_detected_;
    }
