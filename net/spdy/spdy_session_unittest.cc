...
public:
    MOCK_METHOD3(IsCTRequiredForHost, CTRequirementLevel(std::string_view host, const X509Certificate* chain, const HashValueVector& hashes));
};
...
class SpdySessionRequestDelegate : public SpdySessionPool::SpdySessionRequest::Delegate {
public:
    SpdySessionRequestDelegate() = default;

    SpdySessionRequestDelegate(const SpdySessionRequestDelegate&) = delete;
    SpdySessionRequestDelegate& operator=(const SpdySessionRequestDelegate&) = delete;
...
public:
    ...
    void StallSessionOnly(SpdyStream* stream)
    {
        StallSessionSend();
    }

    void StallStreamOnly(SpdyStream* stream)
    {
...
protected:
    ...
    explicit SpdySessionTest(base::test::TaskEnvironment::TimeSource time_source = base::test::TaskEnvironment::TimeSource::DEFAULT)
        : WithTaskEnvironment(time_source)
        , old_max_group_sockets_(ClientSocketPoolManager::max_sockets_per_group(HttpNetworkSession::NORMAL_SOCKET_POOL))
        , old_max_pool_sockets_(ClientSocketPoolManager::max_sockets_per_pool(HttpNetworkSession::NORMAL_SOCKET_POOL))
        , test_url_(kDefaultUrl)
        , test_server_(test_url_)
        , key_(HostPortPair::FromURL(test_url_), PRIVACY_MODE_DISABLED, ProxyChain::Direct(), SessionUsage::kDestination, SocketTag(),
    ...
    ~SpdySessionTest() override
    {
        ...
        ClientSocketPoolManager::set_max_sockets_per_pool(HttpNetworkSession::NORMAL_SOCKET_POOL, old_max_pool_sockets_);
        ClientSocketPoolManager::set_max_sockets_per_group(HttpNetworkSession::NORMAL_SOCKET_POOL, old_max_group_sockets_);
    }
    ...
    {
        ssl_.ssl_info.cert = ImportCertFromFile(GetTestCertsDirectory(), "spdy_pooling.pem");
    ...
    {
        DCHECK(!session_);
        session_ = ::net::CreateSpdySession(http_session_.get(), key_, net_log_with_source_);
    }
    ...
    void StallSessionSend()
    {
        ...
        while (session_send_window_size() > 0) {
            DecreaseSendWindowSize(std::min(kMaxSpdyFrameChunkSize, session_send_window_size()));
        }
    }
    ...
    void StallStreamSend(SpdyStream* stream)
    {
        ...
        while (stream->send_window_size() > 0) {
            stream->DecreaseSendWindowSize(std::min(kMaxSpdyFrameChunkSize, stream->send_window_size()));
        }
    }
    ...
    void RunResumeAfterUnstallTest(base::OnceCallback<void(SpdyStream*)> stall_function, base::OnceCallback<void(SpdyStream*, int32_t)> unstall_function);
    ...
    {
        session_->in_io_loop_ = in_io_loop;
    }
    ...
    {
        return session_->max_concurrent_streams_;
    }
    ...
    bool ping_in_flight()
    {
        return session_->ping_in_flight_;
    }

    spdy::SpdyPingId next_ping_id()
    {
        return session_->next_ping_id_;
    }

    base::TimeTicks last_read_time()
    {
        return session_->last_read_time_;
    }
    ...
    {
        return session_->active_streams_.size();
    }

    size_t num_created_streams()
    {
        return session_->created_streams_.size();
    }
...
public:
    ...
    {
        return base::BindOnce(&StreamRequestDestroyingCallback::OnComplete, base::Unretained(this));
    }
...
TEST_F(SpdySessionTest, PendingStreamCancellingAnother)
{
    ...
    for (size_t i = 0; i < kInitialMaxConcurrentStreams; ++i) {
        base::WeakPtr<SpdyStream> spdy_stream = CreateStreamSynchronously(SPDY_BIDIRECTIONAL_STREAM, session_, test_url_, MEDIUM, NetLogWithSource());
    ...
    ASSERT_EQ(ERR_IO_PENDING,
        request1.StartRequest(SPDY_BIDIRECTIONAL_STREAM, session_, test_url_, false, MEDIUM, SocketTag(), NetLogWithSource(), callback1.MakeCallback(),
            TRAFFIC_ANNOTATION_FOR_TESTS));

    // |callback2| is never called.
    TestCompletionCallback callback2;
    ASSERT_EQ(ERR_IO_PENDING,
        request2->StartRequest(SPDY_BIDIRECTIONAL_STREAM, session_, test_url_, false, MEDIUM, SocketTag(), NetLogWithSource(), callback2.callback(),
            TRAFFIC_ANNOTATION_FOR_TESTS));
...
TEST_F(SpdySessionTest, GoAwayWithActiveStreams)
{
    spdy::SpdySerializedFrame goaway(spdy_util_.ConstructSpdyGoAway(1));
    MockRead reads[] = {
        MockRead(ASYNC, ERR_IO_PENDING, 2), CreateMockRead(goaway, 3), MockRead(ASYNC, ERR_IO_PENDING, 4), MockRead(ASYNC, 0, 5) // EOF
    };
    spdy::SpdySerializedFrame req1(spdy_util_.ConstructSpdyGet(nullptr, 0, 1, MEDIUM));
    ...
    base::WeakPtr<SpdyStream> spdy_stream1 = CreateStreamSynchronously(SPDY_REQUEST_RESPONSE_STREAM, session_, test_url_, MEDIUM, NetLogWithSource());
    test::StreamDelegateDoNothing delegate1(spdy_stream1);
    spdy_stream1->SetDelegate(&delegate1);

    base::WeakPtr<SpdyStream> spdy_stream2 = CreateStreamSynchronously(SPDY_REQUEST_RESPONSE_STREAM, session_, test_url_, MEDIUM, NetLogWithSource());
...
TEST_F(SpdySessionTest, GoAwayWithActiveAndCreatedStream)
{
    ...
    test::StreamDelegateDoNothing delegate1(spdy_stream1);
    spdy_stream1->SetDelegate(&delegate1);
    quiche::HttpHeaderBlock headers1(spdy_util_.ConstructGetHeaderBlock(kDefaultUrl));
...
TEST_F(SpdySessionTest, GoAwayTwice)
{
    ...
    MockRead reads[] = {
        MockRead(ASYNC, ERR_IO_PENDING, 2), CreateMockRead(goaway1, 3), MockRead(ASYNC, ERR_IO_PENDING, 4), CreateMockRead(goaway2, 5),
...
{
    spdy::SpdySerializedFrame req(spdy_util_.ConstructSpdyGet(nullptr, 0, 1, MEDIUM));
    MockWrite writes[] = {
    ...
    spdy::SpdySerializedFrame joint_frames(spdy::test::MakeSerializedFrame(buffer.get(), joint_size));

    MockRead reads[] = {
        CreateMockRead(resp, 1), CreateMockRead(joint_frames, 2), MockRead(ASYNC, 0, 3) // EOF
    };
    ...
    base::WeakPtr<SpdyStream> spdy_stream = CreateStreamSynchronously(SPDY_REQUEST_RESPONSE_STREAM, session_, test_url_, MEDIUM, NetLogWithSource());
...
TEST_F(SpdySessionTest, GoAwayWithActiveStreamsThenEndStreams)
{
    ...
    spdy::SpdySerializedFrame req1(spdy_util_.ConstructSpdyGet(nullptr, 0, kStreamId1, MEDIUM));
    spdy::SpdySerializedFrame req2(spdy_util_.ConstructSpdyGet(nullptr, 0, kStreamId2, MEDIUM));
    MockWrite writes[] = {
        CreateMockWrite(req1, 0),
        CreateMockWrite(req2, 1),
    };

    spdy::SpdySerializedFrame resp1(spdy_util_.ConstructSpdyGetReply(nullptr, 0, kStreamId1));
    spdy::SpdySerializedFrame resp2(spdy_util_.ConstructSpdyGetReply(nullptr, 0, kStreamId2));

    spdy::SpdySerializedFrame body1(spdy_util_.ConstructSpdyDataFrame(kStreamId1, true));
    spdy::SpdySerializedFrame body2(spdy_util_.ConstructSpdyDataFrame(kStreamId2, true));

    spdy::SpdySerializedFrame goaway(spdy_util_.ConstructSpdyGoAway(kStreamId2));

    MockRead reads[] = {
        CreateMockRead(resp1, 2), CreateMockRead(resp2, 3), MockRead(ASYNC, ERR_IO_PENDING, 4), // (1)
        CreateMockRead(goaway, 5), CreateMockRead(body1, 6), MockRead(ASYNC, ERR_IO_PENDING, 7), // (2)
...
TEST_F(SpdySessionTest, CreateStreamAfterGoAway)
{
    spdy::SpdySerializedFrame goaway(spdy_util_.ConstructSpdyGoAway(1));
    MockRead reads[] = {
        MockRead(ASYNC, ERR_IO_PENDING, 1), CreateMockRead(goaway, 2), MockRead(ASYNC, ERR_IO_PENDING, 3), MockRead(ASYNC, 0, 4) // EOF
    };
    spdy::SpdySerializedFrame req(spdy_util_.ConstructSpdyGet(nullptr, 0, 1, MEDIUM));
    MockWrite writes[] = {
        CreateMockWrite(req, 0),
    ...
    int rv = stream_request.StartRequest(SPDY_REQUEST_RESPONSE_STREAM, session_, test_url_, false, MEDIUM, SocketTag(), NetLogWithSource(),
...
TEST_F(SpdySessionTest, HeadersAfterGoAway)
{
    ...
    MockRead reads[] = {
        MockRead(ASYNC, ERR_IO_PENDING, 1), CreateMockRead(goaway_received, 2), MockRead(ASYNC, ERR_IO_PENDING, 3), CreateMockRead(push, 4),
        MockRead(ASYNC, 0, 6) // EOF
    };
    spdy::SpdySerializedFrame req(spdy_util_.ConstructSpdyGet(nullptr, 0, 1, MEDIUM));
    spdy::SpdySerializedFrame goaway_sent(spdy_util_.ConstructSpdyGoAway(0, spdy::ERROR_CODE_PROTOCOL_ERROR, "PUSH_PROMISE received"));
    MockWrite writes[] = { CreateMockWrite(req, 0), CreateMockWrite(goaway_sent, 5) };
    SequencedSocketData data(reads, writes);
    session_deps_.socket_factory->AddSocketDataProvider(&data);
...
TEST_F(SpdySessionTestWithMockTime, ClientPing)
{
    ...
    MockRead reads[] = {
        CreateMockRead(read_ping, 1), MockRead(ASYNC, ERR_IO_PENDING, 2), MockRead(ASYNC, 0, 3) // EOF
    };
    ...
    base::WeakPtr<SpdyStream> spdy_stream1 = CreateStreamSynchronously(SPDY_BIDIRECTIONAL_STREAM, session_, test_url_, MEDIUM, NetLogWithSource());
...
TEST_F(SpdySessionTest, PingAndWriteLoop)
{
    ...
    base::WeakPtr<SpdyStream> spdy_stream = CreateStreamSynchronously(SPDY_REQUEST_RESPONSE_STREAM, session_, test_url_, LOWEST, NetLogWithSource());
    test::StreamDelegateDoNothing delegate(spdy_stream);
    spdy_stream->SetDelegate(&delegate);
...
TEST_F(SpdySessionTestWithMockTime, DetectBrokenConnectionPing)
{
    ...
    MockRead reads[] = {
        MockRead(ASYNC, ERR_IO_PENDING, 1), CreateMockRead(read_ping1, 2), MockRead(ASYNC, ERR_IO_PENDING, 3), MockRead(ASYNC, ERR_IO_PENDING, 5),
        CreateMockRead(read_ping2, 6), MockRead(ASYNC, ERR_IO_PENDING, 7), MockRead(ASYNC, 0, 8) // EOF
    };
    ...
    MockWrite writes[] = { CreateMockWrite(write_ping1, 0), CreateMockWrite(write_ping2, 4) };
    SequencedSocketData data(reads, writes);
    session_deps_.socket_factory->AddSocketDataProvider(&data);
    ...
    base::WeakPtr<SpdyStream> spdy_stream1
        = CreateStreamSynchronously(SPDY_BIDIRECTIONAL_STREAM, session_, test_url_, MEDIUM, NetLogWithSource(), true, kHeartbeatInterval);
    ASSERT_TRUE(spdy_stream1);
...
TEST_F(SpdySessionTest, StreamIdSpaceExhausted)
{
    ...
    spdy::SpdySerializedFrame req1(spdy_util_.ConstructSpdyGet(nullptr, 0, kLastStreamId - 2, MEDIUM));
    spdy::SpdySerializedFrame req2(spdy_util_.ConstructSpdyGet(nullptr, 0, kLastStreamId, MEDIUM));
    ...
    spdy::SpdySerializedFrame resp1(spdy_util_.ConstructSpdyGetReply(nullptr, 0, kLastStreamId - 2));
    spdy::SpdySerializedFrame resp2(spdy_util_.ConstructSpdyGetReply(nullptr, 0, kLastStreamId));

    spdy::SpdySerializedFrame body1(spdy_util_.ConstructSpdyDataFrame(kLastStreamId - 2, true));
    spdy::SpdySerializedFrame body2(spdy_util_.ConstructSpdyDataFrame(kLastStreamId, true));

    MockRead reads[] = {
        CreateMockRead(resp1, 2), CreateMockRead(resp2, 3), MockRead(ASYNC, ERR_IO_PENDING, 4), CreateMockRead(body1, 5), CreateMockRead(body2, 6),
    ...
    base::WeakPtr<SpdyStream> stream1 = CreateStreamSynchronously(SPDY_REQUEST_RESPONSE_STREAM, session_, test_url_, MEDIUM, NetLogWithSource());
    test::StreamDelegateDoNothing delegate1(stream1);
    stream1->SetDelegate(&delegate1);

    base::WeakPtr<SpdyStream> stream2 = CreateStreamSynchronously(SPDY_REQUEST_RESPONSE_STREAM, session_, test_url_, MEDIUM, NetLogWithSource());
    test::StreamDelegateDoNothing delegate2(stream2);
    stream2->SetDelegate(&delegate2);

    base::WeakPtr<SpdyStream> stream3 = CreateStreamSynchronously(SPDY_REQUEST_RESPONSE_STREAM, session_, test_url_, MEDIUM, NetLogWithSource());
    ...
    EXPECT_EQ(ERR_IO_PENDING,
        request4.StartRequest(SPDY_REQUEST_RESPONSE_STREAM, session_, test_url_, false, MEDIUM, SocketTag(), NetLogWithSource(), callback4.callback(),
            TRAFFIC_ANNOTATION_FOR_TESTS));
    ...
    stream1->SendRequestHeaders(spdy_util_.ConstructGetHeaderBlock(kDefaultUrl), NO_MORE_DATA_TO_SEND);
    base::RunLoop().RunUntilIdle();
    ...
    stream2->SendRequestHeaders(spdy_util_.ConstructGetHeaderBlock(kDefaultUrl), NO_MORE_DATA_TO_SEND);
    base::RunLoop().RunUntilIdle();
...
TEST_F(SpdySessionTest, MaxConcurrentStreamsZero)
{
    ...
    spdy::SpdySerializedFrame settings_frame_zero(spdy_util_.ConstructSpdySettings(settings_zero));

    // Acknowledge it.
    spdy::SpdySerializedFrame settings_ack0(spdy_util_.ConstructSpdySettingsAck());
    ...
    spdy::SpdySerializedFrame settings_frame_one(spdy_util_.ConstructSpdySettings(settings_one));

    // Acknowledge it.
    spdy::SpdySerializedFrame settings_ack1(spdy_util_.ConstructSpdySettingsAck());
    ...
    MockRead reads[] = { CreateMockRead(settings_frame_zero, 0), MockRead(ASYNC, ERR_IO_PENDING, 2), CreateMockRead(settings_frame_one, 3),
        CreateMockRead(resp, 6), CreateMockRead(body, 7), MockRead(ASYNC, 0, 8) };

    MockWrite writes[] = { CreateMockWrite(settings_ack0, 1), CreateMockWrite(settings_ack1, 4), CreateMockWrite(req, 5) };
    ...
    int rv = request.StartRequest(
        SPDY_REQUEST_RESPONSE_STREAM, session_, test_url_, false, MEDIUM, SocketTag(), NetLogWithSource(), callback.callback(), TRAFFIC_ANNOTATION_FOR_TESTS);
...
TEST_F(SpdySessionTest, UnstallRacesWithStreamCreation)
{
    ...
    TestCompletionCallback callback2;
    EXPECT_EQ(ERR_IO_PENDING,
        request2.StartRequest(SPDY_REQUEST_RESPONSE_STREAM, session_, test_url_, false, MEDIUM, SocketTag(), NetLogWithSource(), callback2.callback(),
            TRAFFIC_ANNOTATION_FOR_TESTS));
...
TEST_F(SpdySessionTestWithMockTime, FailedPing)
{
    ...
    spdy::SpdySerializedFrame write_ping(spdy_util_.ConstructSpdyPing(1, false));
    spdy::SpdySerializedFrame goaway(spdy_util_.ConstructSpdyGoAway(0, spdy::ERROR_CODE_PROTOCOL_ERROR, "Failed ping."));
...
TEST_F(SpdySessionTestWithMockTime, NoPingSentWhenCheckPingPending)
{
    ...
    spdy::SpdySerializedFrame read_ping(spdy_util_.ConstructSpdyPing(1, true));
    MockRead reads[] = { CreateMockRead(read_ping, 1), MockRead(ASYNC, ERR_IO_PENDING, 2), MockRead(ASYNC, 0, 3) };
...
{
    const spdy::SpdySettingsId kSpdySettingsId = spdy::SETTINGS_MAX_CONCURRENT_STREAMS;
    ...
    spdy::SpdySerializedFrame settings_frame(spdy_util_.ConstructSpdySettings(new_settings));
    MockRead reads[] = {
    ...
    SpdyStreamRequest request;
    ASSERT_EQ(ERR_IO_PENDING,
        request.StartRequest(SPDY_BIDIRECTIONAL_STREAM, session_, test_url_, false, MEDIUM, SocketTag(), NetLogWithSource(),
            stream_releaser.MakeCallback(&request), TRAFFIC_ANNOTATION_FOR_TESTS));
...
TEST_F(SpdySessionTest, CancelPendingCreateStream)
{
    ...
    SpdyStreamRequest request;
    ASSERT_THAT(request.StartRequest(SPDY_BIDIRECTIONAL_STREAM, session_, test_url_, false, MEDIUM, SocketTag(), NetLogWithSource(), callback->callback(),
                    TRAFFIC_ANNOTATION_FOR_TESTS),
...
TEST_F(SpdySessionTest, ChangeStreamRequestPriority)
{
    ...
    SpdyStreamRequest request1;
    ASSERT_EQ(OK,
        request1.StartRequest(SPDY_REQUEST_RESPONSE_STREAM, session_, test_url_, false, LOWEST, SocketTag(), NetLogWithSource(), callback1.callback(),
            TRAFFIC_ANNOTATION_FOR_TESTS));
    TestCompletionCallback callback2;
    SpdyStreamRequest request2;
    ASSERT_EQ(ERR_IO_PENDING,
        request2.StartRequest(SPDY_REQUEST_RESPONSE_STREAM, session_, test_url_, false, LOWEST, SocketTag(), NetLogWithSource(), callback2.callback(),
            TRAFFIC_ANNOTATION_FOR_TESTS));
...
bool NetLogSourceFromEventParameters(const base::Value::Dict* event_params, NetLogSource* source)
{
...
TEST_F(SpdySessionTest, Initialize)
{
    ...
    int pos = ExpectLogContainsSomewhere(entries, 0, NetLogEventType::HTTP2_SESSION_INITIALIZED, NetLogEventPhase::NONE);
    EXPECT_LT(0, pos);

    NetLogSource socket_source;
    EXPECT_TRUE(NetLogSourceFromEventParameters(&entries[pos].params, &socket_source));
...
{
    spdy::SpdySerializedFrame goaway(spdy_util_.ConstructSpdyGoAway(42, spdy::ERROR_CODE_ENHANCE_YOUR_CALM, "foo"));
    MockRead reads[] = {
    ...
    int pos = ExpectLogContainsSomewhere(entries, 0, NetLogEventType::HTTP2_SESSION_RECV_GOAWAY, NetLogEventPhase::NONE);
    ASSERT_EQ(0, GetIntegerValueFromParams(entries[pos], "active_streams"));
    ASSERT_EQ("11 (ENHANCE_YOUR_CALM)", GetStringValueFromParams(entries[pos], "error_code"));
    ASSERT_EQ("foo", GetStringValueFromParams(entries[pos], "debug_data"));

    // Check that we logged SPDY_SESSION_CLOSE correctly.
    pos = ExpectLogContainsSomewhere(entries, 0, NetLogEventType::HTTP2_SESSION_CLOSE, NetLogEventPhase::NONE);
...
TEST_F(SpdySessionTest, NetLogOnSessionEOF)
{
    ...
    // Check that we logged SPDY_SESSION_CLOSE correctly.
    int pos = ExpectLogContainsSomewhere(entries, 0, NetLogEventType::HTTP2_SESSION_CLOSE, NetLogEventPhase::NONE);

    if (pos < static_cast<int>(entries.size())) {
        ASSERT_THAT(GetNetErrorCodeFromParams(entries[pos]), IsError(ERR_CONNECTION_CLOSED));
    } else {
        ADD_FAILURE();
    }
}
...
TEST_F(SpdySessionTest, HeadersCompressionHistograms)
{
    ...
    histogram_tester.ExpectBucketCount("Net.SpdyHeadersCompressionPercentage", 76, 1);
...
TEST_F(SpdySessionTest, OutOfOrderHeaders)
{
    ...
    spdy::SpdySerializedFrame req_highest(spdy_util_.ConstructSpdyGet(nullptr, 0, 1, HIGHEST));
    spdy::SpdySerializedFrame req_lowest(spdy_util_.ConstructSpdyGet(nullptr, 0, 3, LOWEST));
    MockWrite writes[] = {
        CreateMockWrite(req_highest, 0),
        CreateMockWrite(req_lowest, 1),
    };

    spdy::SpdySerializedFrame resp_highest(spdy_util_.ConstructSpdyGetReply(nullptr, 0, 1));
    spdy::SpdySerializedFrame body_highest(spdy_util_.ConstructSpdyDataFrame(1, true));
    spdy::SpdySerializedFrame resp_lowest(spdy_util_.ConstructSpdyGetReply(nullptr, 0, 3));
    spdy::SpdySerializedFrame body_lowest(spdy_util_.ConstructSpdyDataFrame(3, true));
    MockRead reads[] = {
        CreateMockRead(resp_highest, 2), CreateMockRead(body_highest, 3), CreateMockRead(resp_lowest, 4), CreateMockRead(body_lowest, 5),
        MockRead(ASYNC, 0, 6) // EOF
    };
    ...
    base::WeakPtr<SpdyStream> spdy_stream_lowest = CreateStreamSynchronously(SPDY_REQUEST_RESPONSE_STREAM, session_, test_url_, LOWEST, NetLogWithSource());
    ...
    base::WeakPtr<SpdyStream> spdy_stream_highest = CreateStreamSynchronously(SPDY_REQUEST_RESPONSE_STREAM, session_, test_url_, HIGHEST, NetLogWithSource());
    ...
    quiche::HttpHeaderBlock headers_lowest(spdy_util_.ConstructGetHeaderBlock(kDefaultUrl));
    spdy_stream_lowest->SendRequestHeaders(std::move(headers_lowest), NO_MORE_DATA_TO_SEND);

    quiche::HttpHeaderBlock headers_highest(spdy_util_.ConstructGetHeaderBlock(kDefaultUrl));
    spdy_stream_highest->SendRequestHeaders(std::move(headers_highest), NO_MORE_DATA_TO_SEND);
...
TEST_F(SpdySessionTest, CancelStream)
{
    ...
    spdy::SpdySerializedFrame req2(spdy_util_.ConstructSpdyGet(nullptr, 0, 1, LOWEST));
    MockWrite writes[] = {
    ...
    MockRead reads[] = {
        CreateMockRead(resp2, 1), MockRead(ASYNC, ERR_IO_PENDING, 2), CreateMockRead(body2, 3), MockRead(ASYNC, 0, 4) // EOF
    };
    ...
    base::WeakPtr<SpdyStream> spdy_stream1 = CreateStreamSynchronously(SPDY_REQUEST_RESPONSE_STREAM, session_, test_url_, HIGHEST, NetLogWithSource());
    ASSERT_TRUE(spdy_stream1);
    EXPECT_EQ(0u, spdy_stream1->stream_id());
    test::StreamDelegateDoNothing delegate1(spdy_stream1);
    spdy_stream1->SetDelegate(&delegate1);

    base::WeakPtr<SpdyStream> spdy_stream2 = CreateStreamSynchronously(SPDY_REQUEST_RESPONSE_STREAM, session_, test_url_, LOWEST, NetLogWithSource());
    ...
    quiche::HttpHeaderBlock headers2(spdy_util_.ConstructGetHeaderBlock(kDefaultUrl));
    spdy_stream2->SendRequestHeaders(std::move(headers2), NO_MORE_DATA_TO_SEND);
...
TEST_F(SpdySessionTest, CloseSessionWithTwoCreatedSelfClosingStreams)
{
    ...
    base::WeakPtr<SpdyStream> spdy_stream1 = CreateStreamSynchronously(SPDY_BIDIRECTIONAL_STREAM, session_, test_url_, HIGHEST, NetLogWithSource());
    ASSERT_TRUE(spdy_stream1);
    EXPECT_EQ(0u, spdy_stream1->stream_id());

    base::WeakPtr<SpdyStream> spdy_stream2 = CreateStreamSynchronously(SPDY_BIDIRECTIONAL_STREAM, session_, test_url_, LOWEST, NetLogWithSource());
    ASSERT_TRUE(spdy_stream2);
    EXPECT_EQ(0u, spdy_stream2->stream_id());
...
public:
    SessionClosingDelegate(const base::WeakPtr<SpdyStream>& stream, const base::WeakPtr<SpdySession>& session_to_close)
...
TEST_F(SpdySessionTest, CloseActivatedStreamThatClosesSession)
{
    ...
    spdy::SpdySerializedFrame goaway(spdy_util_.ConstructSpdyGoAway(0, spdy::ERROR_CODE_PROTOCOL_ERROR, "Error"));
...
TEST_F(SpdySessionTest, CloseTwoStalledCreateStream)
{
    ...
    // this file.
    spdy::SettingsMap new_settings;
    const spdy::SpdySettingsId kSpdySettingsId1 = spdy::SETTINGS_MAX_CONCURRENT_STREAMS;
    ...
    MockRead reads[] = {
        CreateMockRead(settings_frame, 0), CreateMockRead(resp1, 3), CreateMockRead(body1, 4), CreateMockRead(resp2, 6), CreateMockRead(body2, 7),
        CreateMockRead(resp3, 9), CreateMockRead(body3, 10), MockRead(ASYNC, ERR_IO_PENDING, 11), MockRead(ASYNC, 0, 12) // EOF
    };
    ...
    base::WeakPtr<SpdyStream> spdy_stream1 = CreateStreamSynchronously(SPDY_REQUEST_RESPONSE_STREAM, session_, test_url_, LOWEST, NetLogWithSource());
    ASSERT_TRUE(spdy_stream1);
    EXPECT_EQ(0u, spdy_stream1->stream_id());
    test::StreamDelegateDoNothing delegate1(spdy_stream1);
    spdy_stream1->SetDelegate(&delegate1);
    ...
    ASSERT_EQ(ERR_IO_PENDING,
        request3.StartRequest(SPDY_REQUEST_RESPONSE_STREAM, session_, test_url_, false, LOWEST, SocketTag(), NetLogWithSource(), callback3.callback(),
            TRAFFIC_ANNOTATION_FOR_TESTS));
    ...
    test::StreamDelegateDoNothing delegate3(stream3);
    stream3->SetDelegate(&delegate3);
    quiche::HttpHeaderBlock headers3(spdy_util_.ConstructGetHeaderBlock(kDefaultUrl));
...
TEST_F(SpdySessionTest, CancelTwoStalledCreateStream)
{
    ...
    base::WeakPtr<SpdyStream> spdy_stream1 = CreateStreamSynchronously(SPDY_BIDIRECTIONAL_STREAM, session_, test_url_, LOWEST, NetLogWithSource());
    ASSERT_TRUE(spdy_stream1);
    EXPECT_EQ(0u, spdy_stream1->stream_id());
    ...
    SpdyStreamRequest request2;
    ASSERT_EQ(ERR_IO_PENDING,
        request2.StartRequest(SPDY_BIDIRECTIONAL_STREAM, session_, test_url_, false, LOWEST, SocketTag(), NetLogWithSource(), callback2.callback(),
            TRAFFIC_ANNOTATION_FOR_TESTS));

    TestCompletionCallback callback3;
    SpdyStreamRequest request3;
    ASSERT_EQ(ERR_IO_PENDING,
        request3.StartRequest(SPDY_BIDIRECTIONAL_STREAM, session_, test_url_, false, LOWEST, SocketTag(), NetLogWithSource(), callback3.callback(),
            TRAFFIC_ANNOTATION_FOR_TESTS));
...
TEST_F(SpdySessionTest, ReadDataWithoutYielding)
{
    ...
    spdy::SpdySerializedFrame partial_data_frame(spdy_util_.ConstructSpdyDataFrame(1, std::string_view(payload_data, kPayloadSize), /*fin=*/false));
    spdy::SpdySerializedFrame finish_data_frame(spdy_util_.ConstructSpdyDataFrame(1, std::string_view(payload_data, kPayloadSize - 1), /*fin=*/true));
    ...
    // bytes.
    MockRead reads[] = {
        CreateMockRead(resp1, 1), MockRead(ASYNC, ERR_IO_PENDING, 2), CreateMockRead(partial_data_frame, 3), CreateMockRead(partial_data_frame, 4, SYNCHRONOUS),
        CreateMockRead(partial_data_frame, 5, SYNCHRONOUS), CreateMockRead(finish_data_frame, 6, SYNCHRONOUS), MockRead(ASYNC, 0, 7) // EOF
    };
...
TEST_F(SpdySessionTest, TestYieldingSlowSynchronousReads)
{
    ...
    spdy::SpdySerializedFrame partial_data_frame(spdy_util_.ConstructSpdyDataFrame(1, "foo ", /*fin=*/false));
    spdy::SpdySerializedFrame finish_data_frame(spdy_util_.ConstructSpdyDataFrame(1, "bar", /*fin=*/true));

    spdy::SpdySerializedFrame resp1(spdy_util_.ConstructSpdyGetReply(nullptr, 0, 1));

    MockRead reads[] = {
        CreateMockRead(resp1, 1), MockRead(ASYNC, ERR_IO_PENDING, 2), CreateMockRead(partial_data_frame, 3, ASYNC),
        CreateMockRead(partial_data_frame, 4, SYNCHRONOUS), CreateMockRead(partial_data_frame, 5, SYNCHRONOUS),
        CreateMockRead(finish_data_frame, 6, SYNCHRONOUS), MockRead(ASYNC, 0, 7) // EOF
    };
...
TEST_F(SpdySessionTest, TestYieldingDuringReadData)
{
    ...
    spdy::SpdySerializedFrame partial_data_frame(spdy_util_.ConstructSpdyDataFrame(1, std::string_view(payload_data, kPayloadSize), /*fin=*/false));
    spdy::SpdySerializedFrame finish_data_frame(spdy_util_.ConstructSpdyDataFrame(1, "h", /*fin=*/true));
    ...
    MockRead reads[] = {
        CreateMockRead(resp1, 1), MockRead(ASYNC, ERR_IO_PENDING, 2), CreateMockRead(partial_data_frame, 3), CreateMockRead(partial_data_frame, 4, SYNCHRONOUS),
        CreateMockRead(partial_data_frame, 5, SYNCHRONOUS), CreateMockRead(partial_data_frame, 6, SYNCHRONOUS),
        CreateMockRead(finish_data_frame, 7, SYNCHRONOUS), MockRead(ASYNC, 0, 8) // EOF
    };
...
TEST_F(SpdySessionTest, TestYieldingDuringAsyncReadData)
{
    ...
    ASSERT_EQ(32 * 1024, kYieldAfterBytesRead);
    TestDataStream test_stream;
    const int kEightKPayloadSize = kYieldAfterBytesRead / 4 - spdy::kFrameHeaderSize;
    auto eightk_payload = base::MakeRefCounted<IOBufferWithSize>(kEightKPayloadSize);
    ...
    spdy::SpdySerializedFrame eightk_data_frame(spdy_util_.ConstructSpdyDataFrame(1, std::string_view(eightk_payload_data, kEightKPayloadSize),
        /*fin=*/false));
    spdy::SpdySerializedFrame twok_data_frame(spdy_util_.ConstructSpdyDataFrame(1, std::string_view(twok_payload_data, kTwoKPayloadSize),
        /*fin=*/false));
    spdy::SpdySerializedFrame finish_data_frame(spdy_util_.ConstructSpdyDataFrame(1, "h", /*fin=*/true));
    ...
    MockRead reads[] = {
        CreateMockRead(resp1, 1), MockRead(ASYNC, ERR_IO_PENDING, 2), CreateMockRead(eightk_data_frame, 3), CreateMockRead(eightk_data_frame, 4, SYNCHRONOUS),
        CreateMockRead(eightk_data_frame, 5, SYNCHRONOUS), CreateMockRead(twok_data_frame, 6, SYNCHRONOUS), CreateMockRead(eightk_data_frame, 7, ASYNC),
        CreateMockRead(eightk_data_frame, 8, SYNCHRONOUS), CreateMockRead(eightk_data_frame, 9, SYNCHRONOUS),
        CreateMockRead(eightk_data_frame, 10, SYNCHRONOUS), CreateMockRead(twok_data_frame, 11, SYNCHRONOUS),
        CreateMockRead(finish_data_frame, 12, SYNCHRONOUS), MockRead(ASYNC, 0, 13) // EOF
    };
...
{
    ClientSocketPoolManager::set_max_sockets_per_group(HttpNetworkSession::NORMAL_SOCKET_POOL, 1);
    ClientSocketPoolManager::set_max_sockets_per_pool(HttpNetworkSession::NORMAL_SOCKET_POOL, 1);
    ...
    ClientSocketPool* pool = http_session_->GetSocketPool(HttpNetworkSession::NORMAL_SOCKET_POOL, ProxyChain::Direct());
    ...
    EXPECT_EQ(ERR_IO_PENDING,
        connection2->Init(ClientSocketPool::GroupId(url::SchemeHostPort(url::kHttpScheme, "2.com", 80), PrivacyMode::PRIVACY_MODE_DISABLED,
                              NetworkAnonymizationKey(), SecureDnsPolicy::kAllow, /*disable_cert_network_fetches=*/false),
            ClientSocketPool::SocketParams::CreateForHttpForTesting(), std::nullopt /* proxy_annotation_tag */, DEFAULT_PRIORITY, SocketTag(),
            ClientSocketPool::RespectLimits::ENABLED, callback2.callback(), ClientSocketPool::ProxyAuthCallback(), pool, NetLogWithSource()));
...
TEST_F(SpdySessionTest, CloseOneIdleConnectionWithAlias)
{
    ...
    session_deps_.host_resolver->rules()->AddIPLiteralRule("www.example.org", "192.168.0.2", std::string());
    ...
    // Create an idle SPDY session.
    SpdySessionKey key1(HostPortPair("www.example.org", 80), PRIVACY_MODE_DISABLED, ProxyChain::Direct(), SessionUsage::kDestination, SocketTag(),
        NetworkAnonymizationKey(), SecureDnsPolicy::kAllow,
        /*disable_cert_verification_network_fetches=*/false);
    ...
    SpdySessionKey key2(HostPortPair("mail.example.org", 80), PRIVACY_MODE_DISABLED, ProxyChain::Direct(), SessionUsage::kDestination, SocketTag(),
        NetworkAnonymizationKey(), SecureDnsPolicy::kAllow,
        /*disable_cert_verification_network_fetches=*/false);
    ...
    EXPECT_FALSE(spdy_session_pool_->RequestSession(key2, /* enable_ip_based_pooling = */ true,
        /* is_websocket = */ false, NetLogWithSource(),
        /* on_blocking_request_destroyed_callback = */ base::RepeatingClosure(), &request_delegate, &request, &is_blocking_request_for_session));
    EXPECT_TRUE(request);
    ...
    EXPECT_EQ(OnHostResolutionCallbackResult::kMayBeDeletedAsync,
        spdy_session_pool_->OnHostResolutionComplete(key2, /* is_websocket = */ false, { endpoint },
            /*aliases=*/ {}));

    // Get a session for |key2|, which should return the session created earlier.
    base::WeakPtr<SpdySession> session2 = spdy_session_pool_->FindAvailableSession(key2, /* enable_ip_based_pooling = */ true,
        /* is_websocket = */ false, NetLogWithSource());
    ...
    EXPECT_EQ(ERR_IO_PENDING,
        connection3->Init(ClientSocketPool::GroupId(url::SchemeHostPort(url::kHttpScheme, "3.com", 80), PrivacyMode::PRIVACY_MODE_DISABLED,
                              NetworkAnonymizationKey(), SecureDnsPolicy::kAllow, /*disable_cert_network_fetches=*/false),
            ClientSocketPool::SocketParams::CreateForHttpForTesting(), std::nullopt /* proxy_annotation_tag */, DEFAULT_PRIORITY, SocketTag(),
            ClientSocketPool::RespectLimits::ENABLED, callback3.callback(), ClientSocketPool::ProxyAuthCallback(), pool, NetLogWithSource()));
    EXPECT_TRUE(pool->IsStalled());
...
TEST_F(SpdySessionTest, CloseSessionOnIdleWhenPoolStalled)
{
    ...
    spdy::SpdySerializedFrame req1(spdy_util_.ConstructSpdyGet(nullptr, 0, 1, LOWEST));
    spdy::SpdySerializedFrame cancel1(spdy_util_.ConstructSpdyRstStream(1, spdy::ERROR_CODE_CANCEL));
    MockWrite writes[] = {
    ...
    TestCompletionCallback callback1;
    base::WeakPtr<SpdyStream> spdy_stream1 = CreateStreamSynchronously(SPDY_REQUEST_RESPONSE_STREAM, session_, test_url_, DEFAULT_PRIORITY, NetLogWithSource());
...
TEST_F(SpdySessionTest, SpdySessionKeyPrivacyMode)
{
    ...
    SpdySessionKey key_privacy_enabled(host_port_pair, PRIVACY_MODE_ENABLED, ProxyChain::Direct(), SessionUsage::kDestination, SocketTag(),
        NetworkAnonymizationKey(), SecureDnsPolicy::kAllow,
        /*disable_cert_verification_network_fetches=*/false);
    SpdySessionKey key_privacy_disabled(host_port_pair, PRIVACY_MODE_DISABLED, ProxyChain::Direct(), SessionUsage::kDestination, SocketTag(),
        NetworkAnonymizationKey(), SecureDnsPolicy::kAllow,
        /*disable_cert_verification_network_fetches=*/false);
...
public:
    StreamCreatingDelegate(const base::WeakPtr<SpdyStream>& stream, const base::WeakPtr<SpdySession>& session)
        : StreamDelegateDoNothing(stream)
        , session_(session)
    {
    }
    ...
    void OnClose(int status) override
    {
        ...
        std::ignore = CreateStreamSynchronously(SPDY_REQUEST_RESPONSE_STREAM, session_, url, MEDIUM, NetLogWithSource());
    }
...
TEST_F(SpdySessionTest, CreateStreamOnStreamReset)
{
    ...
    spdy::SpdySerializedFrame rst(spdy_util_.ConstructSpdyRstStream(1, spdy::ERROR_CODE_REFUSED_STREAM));
    MockRead reads[] = {
        MockRead(ASYNC, ERR_IO_PENDING, 1), CreateMockRead(rst, 2), MockRead(ASYNC, ERR_IO_PENDING, 3), MockRead(ASYNC, 0, 4) // EOF
    };
    SequencedSocketData data(reads, writes);
    session_deps_.socket_factory->AddSocketDataProvider(&data);
...
TEST_F(SpdySessionTest, UpdateStreamsSendWindowSize)
{
    ...
    spdy::SpdySerializedFrame settings_frame(spdy_util_.ConstructSpdySettings(new_settings));
    MockRead reads[] = {
        CreateMockRead(settings_frame, 0), MockRead(ASYNC, ERR_IO_PENDING, 1), MockRead(ASYNC, 0, 2) // EOF
    };
    ...
    base::WeakPtr<SpdyStream> spdy_stream2 = CreateStreamSynchronously(SPDY_BIDIRECTIONAL_STREAM, session_, test_url_, MEDIUM, NetLogWithSource());
    ASSERT_TRUE(spdy_stream2);
...
TEST_F(SpdySessionTest, AdjustRecvWindowSize)
{
    ...
    spdy::SpdySerializedFrame window_update(spdy_util_.ConstructSpdyWindowUpdate(spdy::kSessionFlowControlStreamId, initial_window_size + delta_window_size));
    MockWrite writes[] = {
    ...
    EXPECT_EQ(initial_window_size + delta_window_size, session_recv_window_size());
    ...
    EXPECT_EQ(initial_window_size + delta_window_size + initial_window_size, session_recv_window_size());
    EXPECT_EQ(0, session_unacked_recv_window_bytes());
    ...
    set_in_io_loop(true);
    DecreaseRecvWindowSize(initial_window_size + delta_window_size + initial_window_size);
...
TEST_F(SpdySessionTestWithMockTime, FlowControlSlowReads)
{
    ...
    session_->SetTimeToBufferSmallWindowUpdates(kDefaultTimeToBufferSmallWindowUpdates);
...
TEST_F(SpdySessionTest, AdjustSendWindowSize)
{
    ...
    IncreaseSendWindowSize(delta_window_size);
    EXPECT_EQ(initial_window_size + delta_window_size, session_send_window_size());
...
TEST_F(SpdySessionTest, SessionFlowControlInactiveStream)
{
    ...
    MockRead reads[] = {
        CreateMockRead(resp, 0), MockRead(ASYNC, ERR_IO_PENDING, 1), MockRead(ASYNC, 0, 2) // EOF
    };
    SequencedSocketData data(reads, base::span<MockWrite>());
    session_deps_.socket_factory->AddSocketDataProvider(&data);
...
TEST_F(SpdySessionTest, SessionFlowControlPadding)
{
    ...
    spdy::SpdySerializedFrame resp(spdy_util_.ConstructSpdyDataFrame(1, kUploadData, false, padding_length));
    MockRead reads[] = {
        CreateMockRead(resp, 0), MockRead(ASYNC, ERR_IO_PENDING, 1), MockRead(ASYNC, 0, 2) // EOF
    };
    SequencedSocketData data(reads, base::span<MockWrite>());
    ...
    EXPECT_EQ(kDefaultInitialWindowSize, session_recv_window_size());
    EXPECT_EQ(kUploadDataSize + padding_length, session_unacked_recv_window_bytes());
...
TEST_F(SpdySessionTest, StreamFlowControlTooMuchData)
{
    ...
    spdy::SpdySerializedFrame data_frame(spdy_util_.ConstructSpdyDataFrame(1, payload, false));
    MockRead reads[] = {
    ...
    session_deps_.http2_settings[spdy::SETTINGS_INITIAL_WINDOW_SIZE] = stream_max_recv_window_size;
    CreateNetworkSession();
...
TEST_F(SpdySessionTest, SessionFlowControlTooMuchDataTwoDataFrames)
{
    ...
    // session level.
    ASSERT_LT(session_max_recv_window_size, first_data_frame_size + second_data_frame_size);
    ...
    spdy::SpdySerializedFrame first(spdy_util_.ConstructSpdyDataFrame(1, first_data_frame, false));
    const std::string second_data_frame(second_data_frame_size, 'b');
    spdy::SpdySerializedFrame second(spdy_util_.ConstructSpdyDataFrame(1, second_data_frame, false));
    MockRead reads[] = {
...
TEST_F(SpdySessionTest, StreamFlowControlTooMuchDataTwoDataFrames)
{
    ...
    ASSERT_LT(stream_max_recv_window_size, first_data_frame_size + second_data_frame_size);
    ...
    base::RunLoop().RunUntilIdle();
    EXPECT_TRUE(spdy_stream->IsLocallyClosed());
    EXPECT_EQ(stream_max_recv_window_size - first_data_frame_size, spdy_stream->recv_window_size());
...
public:
    DropReceivedDataDelegate(const base::WeakPtr<SpdyStream>& stream, std::string_view data)
...
TEST_F(SpdySessionTest, SessionFlowControlNoReceiveLeaks)
{
    ...
    spdy::SpdySerializedFrame req(spdy_util_.ConstructSpdyPost(kDefaultUrl, 1, kMsgDataSize, MEDIUM, nullptr, 0));
    spdy::SpdySerializedFrame msg(spdy_util_.ConstructSpdyDataFrame(1, msg_data, false));
    MockWrite writes[] = {
        CreateMockWrite(req, 0),
    ...
    spdy::SpdySerializedFrame resp(spdy_util_.ConstructSpdyGetReply(nullptr, 0, 1));
    spdy::SpdySerializedFrame echo(spdy_util_.ConstructSpdyDataFrame(1, msg_data, false));
    spdy::SpdySerializedFrame window_update(spdy_util_.ConstructSpdyWindowUpdate(spdy::kSessionFlowControlStreamId, kMsgDataSize));
    MockRead reads[] = {
        CreateMockRead(resp, 1), CreateMockRead(echo, 3), MockRead(ASYNC, ERR_IO_PENDING, 4), MockRead(ASYNC, 0, 5) // EOF
    };
    ...
    base::WeakPtr<SpdyStream> stream = CreateStreamSynchronously(SPDY_BIDIRECTIONAL_STREAM, session_, test_url_, MEDIUM, NetLogWithSource());
    ASSERT_TRUE(stream);
    ...
    quiche::HttpHeaderBlock headers(spdy_util_.ConstructPostHeaderBlock(kDefaultUrl, kMsgDataSize));
    EXPECT_EQ(ERR_IO_PENDING, stream->SendRequestHeaders(std::move(headers), MORE_DATA_TO_SEND));
...
TEST_F(SpdySessionTest, SessionFlowControlNoSendLeaks)
{
    ...
    spdy::SpdySerializedFrame resp(spdy_util_.ConstructSpdyGetReply(nullptr, 0, 1));
    MockRead reads[] = {
        MockRead(ASYNC, ERR_IO_PENDING, 1), CreateMockRead(resp, 2), MockRead(ASYNC, 0, 3) // EOF
    };
...
TEST_F(SpdySessionTest, SessionFlowControlEndToEnd)
{
    ...
    spdy::SpdySerializedFrame window_update(spdy_util_.ConstructSpdyWindowUpdate(spdy::kSessionFlowControlStreamId, kMsgDataSize));
    MockRead reads[] = {
        CreateMockRead(resp, 1), MockRead(ASYNC, ERR_IO_PENDING, 3), CreateMockRead(echo, 4), MockRead(ASYNC, ERR_IO_PENDING, 5),
        CreateMockRead(window_update, 6), MockRead(ASYNC, ERR_IO_PENDING, 7), MockRead(ASYNC, 0, 8) // EOF
    };
...
void SpdySessionTest::RunResumeAfterUnstallTest(
    base::OnceCallback<void(SpdyStream*)> stall_function, base::OnceCallback<void(SpdyStream*, int32_t)> unstall_function)
{
    spdy::SpdySerializedFrame req(spdy_util_.ConstructSpdyPost(kDefaultUrl, 1, kBodyDataSize, LOWEST, nullptr, 0));
    spdy::SpdySerializedFrame body(spdy_util_.ConstructSpdyDataFrame(1, kBodyDataStringPiece, true));
    MockWrite writes[] = {
        CreateMockWrite(req, 0),
    ...
    spdy::SpdySerializedFrame resp(spdy_util_.ConstructSpdyGetReply(nullptr, 0, 1));
    spdy::SpdySerializedFrame echo(spdy_util_.ConstructSpdyDataFrame(1, kBodyDataStringPiece, false));
    MockRead reads[] = {
    ...
    base::WeakPtr<SpdyStream> stream = CreateStreamSynchronously(SPDY_REQUEST_RESPONSE_STREAM, session_, test_url_, LOWEST, NetLogWithSource());
    ASSERT_TRUE(stream);
    ...
    quiche::HttpHeaderBlock headers(spdy_util_.ConstructPostHeaderBlock(kDefaultUrl, kBodyDataSize));
    EXPECT_EQ(ERR_IO_PENDING, stream->SendRequestHeaders(std::move(headers), MORE_DATA_TO_SEND));
...
{
    RunResumeAfterUnstallTest(base::BindOnce(&SpdySessionTest::StallSessionOnly, base::Unretained(this)),
        base::BindOnce(&SpdySessionTest::UnstallSessionOnly, base::Unretained(this)));
}
...
TEST_F(SpdySessionTest, ResumeAfterUnstallStream)
{
    RunResumeAfterUnstallTest(
        base::BindOnce(&SpdySessionTest::StallStreamOnly, base::Unretained(this)), base::BindOnce(&SpdySessionTest::UnstallStreamOnly, base::Unretained(this)));
}

TEST_F(SpdySessionTest, StallSessionStreamResumeAfterUnstallSessionStream)
{
    RunResumeAfterUnstallTest(base::BindOnce(&SpdySessionTest::StallSessionStream, base::Unretained(this)),
        base::BindOnce(&SpdySessionTest::UnstallSessionStream, base::Unretained(this)));
}

TEST_F(SpdySessionTest, StallStreamSessionResumeAfterUnstallSessionStream)
{
    RunResumeAfterUnstallTest(base::BindOnce(&SpdySessionTest::StallStreamSession, base::Unretained(this)),
        base::BindOnce(&SpdySessionTest::UnstallSessionStream, base::Unretained(this)));
}
...
{
    RunResumeAfterUnstallTest(base::BindOnce(&SpdySessionTest::StallStreamSession, base::Unretained(this)),
        base::BindOnce(&SpdySessionTest::UnstallStreamSession, base::Unretained(this)));
}
...
{
    spdy::SpdySerializedFrame req1(spdy_util_.ConstructSpdyPost(kDefaultUrl, 1, kBodyDataSize, LOWEST, nullptr, 0));
    spdy::SpdySerializedFrame req2(spdy_util_.ConstructSpdyPost(kDefaultUrl, 3, kBodyDataSize, MEDIUM, nullptr, 0));
    spdy::SpdySerializedFrame body1(spdy_util_.ConstructSpdyDataFrame(1, kBodyDataStringPiece, true));
    spdy::SpdySerializedFrame body2(spdy_util_.ConstructSpdyDataFrame(3, kBodyDataStringPiece, true));
    MockWrite writes[] = {
        CreateMockWrite(req1, 0),
        CreateMockWrite(req2, 1),
    ...
    spdy::SpdySerializedFrame resp2(spdy_util_.ConstructSpdyGetReply(nullptr, 0, 3));
    MockRead reads[] = {
        CreateMockRead(resp1, 4), CreateMockRead(resp2, 5), MockRead(ASYNC, 0, 6) // EOF
    };
    ...
    base::WeakPtr<SpdyStream> stream1 = CreateStreamSynchronously(SPDY_REQUEST_RESPONSE_STREAM, session_, test_url_, LOWEST, NetLogWithSource());
    ASSERT_TRUE(stream1);
    ...
    quiche::HttpHeaderBlock headers1(spdy_util_.ConstructPostHeaderBlock(kDefaultUrl, kBodyDataSize));
    ...
    quiche::HttpHeaderBlock headers2(spdy_util_.ConstructPostHeaderBlock(kDefaultUrl, kBodyDataSize));
...
{
    spdy::SpdySerializedFrame req1(spdy_util_.ConstructSpdyPost(kDefaultUrl, 1, kBodyDataSize, LOWEST, nullptr, 0));
    spdy::SpdySerializedFrame req2(spdy_util_.ConstructSpdyPost(kDefaultUrl, 3, kBodyDataSize, LOWEST, nullptr, 0));
    spdy::SpdySerializedFrame body1(spdy_util_.ConstructSpdyDataFrame(3, kBodyDataStringPiece, true));
    spdy::SpdySerializedFrame body2(spdy_util_.ConstructSpdyDataFrame(1, kBodyDataStringPiece, true));
    MockWrite writes[] = { CreateMockWrite(req1, 0), CreateMockWrite(req2, 1), CreateMockWrite(body1, 2), CreateMockWrite(body2, 3) };

    spdy::SpdySerializedFrame resp1(spdy_util_.ConstructSpdyGetReply(nullptr, 0, 1));
    spdy::SpdySerializedFrame resp2(spdy_util_.ConstructSpdyGetReply(nullptr, 0, 3));
    MockRead reads[] = { CreateMockRead(resp1, 4), CreateMockRead(resp2, 5), MockRead(ASYNC, 0, 6) };
    ...
    base::WeakPtr<SpdyStream> stream2 = CreateStreamSynchronously(SPDY_REQUEST_RESPONSE_STREAM, session_, test_url_, LOWEST, NetLogWithSource());
    ASSERT_TRUE(stream2);
...
class StreamBrokenConnectionDetectionCheckDelegate : public test::StreamDelegateDoNothing {
public:
    StreamBrokenConnectionDetectionCheckDelegate(
        const base::WeakPtr<SpdyStream>& stream, const base::WeakPtr<SpdySession>& session, bool expected_is_broken_connection_detection_enabled)
        : StreamDelegateDoNothing(stream)
        , session_(session)
        , expected_is_broken_connection_detection_enabled_(expected_is_broken_connection_detection_enabled)
    {
    }
    ...
    void OnClose(int status) override
    {
        ASSERT_EQ(expected_is_broken_connection_detection_enabled_, session_->IsBrokenConnectionDetectionEnabled());
    }
...
TEST_F(SpdySessionTest, BrokenConnectionDetectionEOF)
{
    ...
    base::WeakPtr<SpdyStream> stream
        = CreateStreamSynchronously(SPDY_REQUEST_RESPONSE_STREAM, session_, test_url_, MEDIUM, NetLogWithSource(), true, kHeartbeatInterval);
    ASSERT_TRUE(stream);
    ASSERT_TRUE(session_->IsBrokenConnectionDetectionEnabled());
    StreamBrokenConnectionDetectionCheckDelegate delegate(stream, session_, false);
    stream->SetDelegate(&delegate);
...
TEST_F(SpdySessionTest, BrokenConnectionDetectionMultipleRequests)
{
    ...
    StreamBrokenConnectionDetectionCheckDelegate delegate1(spdy_stream1, session_, false);
    spdy_stream1->SetDelegate(&delegate1);
    ...
    EXPECT_TRUE(spdy_stream2);
    EXPECT_TRUE(session_->IsBrokenConnectionDetectionEnabled());
    StreamBrokenConnectionDetectionCheckDelegate delegate2(spdy_stream2, session_, true);
    spdy_stream2->SetDelegate(&delegate2);
...
public:
    StreamClosingDelegate(const base::WeakPtr<SpdyStream>& stream, std::string_view data)
...
TEST_F(SpdySessionTest, SendWindowSizeIncreaseWithDeletedStreams)
{
    spdy::SpdySerializedFrame req1(spdy_util_.ConstructSpdyPost(kDefaultUrl, 1, kBodyDataSize, LOWEST, nullptr, 0));
    spdy::SpdySerializedFrame req2(spdy_util_.ConstructSpdyPost(kDefaultUrl, 3, kBodyDataSize, LOWEST, nullptr, 0));
    spdy::SpdySerializedFrame req3(spdy_util_.ConstructSpdyPost(kDefaultUrl, 5, kBodyDataSize, LOWEST, nullptr, 0));
    spdy::SpdySerializedFrame body2(spdy_util_.ConstructSpdyDataFrame(3, kBodyDataStringPiece, true));
    MockWrite writes[] = {
        CreateMockWrite(req1, 0),
        CreateMockWrite(req2, 1),
    ...
    spdy::SpdySerializedFrame resp2(spdy_util_.ConstructSpdyGetReply(nullptr, 0, 3));
    MockRead reads[] = {
        CreateMockRead(resp2, 4), MockRead(ASYNC, ERR_IO_PENDING, 5), MockRead(ASYNC, 0, 6) // EOF
    };
    ...
    base::WeakPtr<SpdyStream> stream3 = CreateStreamSynchronously(SPDY_REQUEST_RESPONSE_STREAM, session_, test_url_, LOWEST, NetLogWithSource());
    ASSERT_TRUE(stream3);
    ...
    quiche::HttpHeaderBlock headers3(spdy_util_.ConstructPostHeaderBlock(kDefaultUrl, kBodyDataSize));
...
TEST_F(SpdySessionTest, SendWindowSizeIncreaseWithDeletedSession)
{
    spdy::SpdySerializedFrame req1(spdy_util_.ConstructSpdyPost(kDefaultUrl, 1, kBodyDataSize, LOWEST, nullptr, 0));
    spdy::SpdySerializedFrame req2(spdy_util_.ConstructSpdyPost(kDefaultUrl, 3, kBodyDataSize, LOWEST, nullptr, 0));
    spdy::SpdySerializedFrame body1(spdy_util_.ConstructSpdyDataFrame(1, kBodyDataStringPiece, false));
    MockWrite writes[] = {
        CreateMockWrite(req1, 0),
        CreateMockWrite(req2, 1),
    };
...
TEST_F(SpdySessionTest, EnableWebSocket)
{
    ...
    spdy::SpdySerializedFrame settings(spdy_util_.ConstructSpdySettings(settings_map));
    MockRead reads[] = { CreateMockRead(settings, 0), MockRead(ASYNC, ERR_IO_PENDING, 2), MockRead(ASYNC, 0, 3) };
...
TEST_F(SpdySessionTest, EnableWebSocketThenDisableIsProtocolError)
{
    ...
    spdy::SpdySerializedFrame settings1(spdy_util_.ConstructSpdySettings(settings_map1));
    spdy::SettingsMap settings_map2;
    settings_map2[spdy::SETTINGS_ENABLE_CONNECT_PROTOCOL] = 0;
    spdy::SpdySerializedFrame settings2(spdy_util_.ConstructSpdySettings(settings_map2));
    MockRead reads[] = { CreateMockRead(settings1, 0), MockRead(ASYNC, ERR_IO_PENDING, 2), CreateMockRead(settings2, 3) };
    ...
    spdy::SpdySerializedFrame goaway(
        spdy_util_.ConstructSpdyGoAway(0, spdy::ERROR_CODE_PROTOCOL_ERROR, "Invalid value for spdy::SETTINGS_ENABLE_CONNECT_PROTOCOL."));
    MockWrite writes[] = { CreateMockWrite(ack1, 1), CreateMockWrite(ack2, 4), CreateMockWrite(goaway, 5) };
...
TEST_F(SpdySessionTest, GreaseFrameTypeAfterSettings)
{
    ...
    session_deps_.greased_http2_frame = std::optional<net::SpdySessionPool::GreasedHttp2Frame>({ type, flags, payload });
    ...
    expected_settings[spdy::SETTINGS_MAX_HEADER_LIST_SIZE] = kSpdyMaxHeaderListSize;
    expected_settings[spdy::SETTINGS_ENABLE_PUSH] = 0;
    spdy::SpdySerializedFrame settings_frame(spdy_util_.ConstructSpdySettings(expected_settings));

    spdy::SpdySerializedFrame combined_frame = CombineFrames({ &preface, &settings_frame });
    ...
    MockWrite writes[] = { CreateMockWrite(combined_frame, 0), CreateMockWrite(grease, 1) };
...
class SpdySessionReadIfReadyTest : public SpdySessionTest, public testing::WithParamInterface<ReadIfReadySupport> {
public:
    void SetUp() override
    {
...
INSTANTIATE_TEST_SUITE_P(All, SpdySessionReadIfReadyTest, testing::Values(READ_IF_READY_SUPPORTED, READ_IF_READY_NOT_SUPPORTED));
...
TEST_P(SpdySessionReadIfReadyTest, ReadIfReady)
{
    ...
    base::WeakPtr<SpdyStream> spdy_stream = CreateStreamSynchronously(SPDY_REQUEST_RESPONSE_STREAM, session_, test_url_, HIGHEST, NetLogWithSource());
    ASSERT_TRUE(spdy_stream);
    EXPECT_EQ(0u, spdy_stream->stream_id());
    test::StreamDelegateDoNothing delegate(spdy_stream);
    spdy_stream->SetDelegate(&delegate);
...
TEST_F(SendInitialSettingsOnNewSpdySessionTest, ProtocolDefault)
{
    ...
    session_deps_.http2_settings[spdy::SETTINGS_INITIAL_WINDOW_SIZE] = 64 * 1024 - 1;
...
TEST_F(SendInitialSettingsOnNewSpdySessionTest, OverwriteValues)
{
    ...
    session_deps_.http2_settings[spdy::SETTINGS_MAX_HEADER_LIST_SIZE] = 101 * 1024;
...
public:
    AltSvcFrameTest()
        : alternative_service_(
            quic::AlpnForVersion(DefaultSupportedQuicVersions().front()), "alternative.example.org", 443, 86400, spdy::SpdyAltSvcWireFormat::VersionVector())
    {
    ...
    void AddSocketData(const spdy::SpdyAltSvcIR& altsvc_ir)
    {
        ...
        data_ = std::make_unique<SequencedSocketData>(reads_, base::span<MockWrite>());
        session_deps_.socket_factory->AddSocketDataProvider(data_.get());
    }

    void CreateSpdySession()
    {
        session_ = ::net::CreateSpdySession(http_session_.get(), key_, NetLogWithSource());
    }
...
TEST_F(AltSvcFrameTest, ProcessAltSvcFrame)
{
    ...
    const url::SchemeHostPort session_origin("https", test_url_.host(), test_url_.EffectiveIntPort());
    AlternativeServiceInfoVector altsvc_info_vector
        = spdy_session_pool_->http_server_properties()->GetAlternativeServiceInfos(session_origin, NetworkAnonymizationKey());
    ASSERT_TRUE(altsvc_info_vector.empty());

    altsvc_info_vector = spdy_session_pool_->http_server_properties()->GetAlternativeServiceInfos(url::SchemeHostPort(GURL(origin)), NetworkAnonymizationKey());
    ASSERT_EQ(1u, altsvc_info_vector.size());
    AlternativeService alternative_service(kProtoQUIC, "alternative.example.org", 443u);
...
TEST_F(AltSvcFrameTest, IgnoreQuicAltSvcWithUnsupportedVersion)
{
    ...
    spdy::SpdyAltSvcWireFormat::AlternativeService quic_alternative_service(
        "quic", "alternative.example.org", 443, 86400, spdy::SpdyAltSvcWireFormat::VersionVector());
...
TEST_F(AltSvcFrameTest, DoNotProcessAltSvcFrameForOriginNotCoveredByCert)
{
    ...
    const url::SchemeHostPort session_origin("https", test_url_.host(), test_url_.EffectiveIntPort());
    ASSERT_TRUE(spdy_session_pool_->http_server_properties()->GetAlternativeServiceInfos(session_origin, NetworkAnonymizationKey()).empty());

    ASSERT_TRUE(spdy_session_pool_->http_server_properties()->GetAlternativeServiceInfos(url::SchemeHostPort(GURL(origin)), NetworkAnonymizationKey()).empty());
}
...
TEST_F(AltSvcFrameTest, DoNotProcessAltSvcFrameWithNonEmptyOriginOnNonZeroStream)
{
...
TEST_F(AltSvcFrameTest, ProcessAltSvcFrameOnActiveStream)
{
    ...
    spdy::SpdySerializedFrame rst(spdy_util_.ConstructSpdyRstStream(1, spdy::ERROR_CODE_REFUSED_STREAM));
    MockRead reads[] = {
        CreateMockRead(altsvc_frame, 1), CreateMockRead(rst, 2), MockRead(ASYNC, 0, 3) // EOF
    };

    const char request_origin[] = "https://mail.example.org";
    spdy::SpdySerializedFrame req(spdy_util_.ConstructSpdyGet(request_origin, 1, MEDIUM));
    MockWrite writes[] = {
        CreateMockWrite(req, 0),
    };
    SequencedSocketData data(reads, writes);
    ...
    base::WeakPtr<SpdyStream> spdy_stream1
        = CreateStreamSynchronously(SPDY_REQUEST_RESPONSE_STREAM, session_, GURL(request_origin), MEDIUM, NetLogWithSource());
    test::StreamDelegateDoNothing delegate1(spdy_stream1);
    spdy_stream1->SetDelegate(&delegate1);

    quiche::HttpHeaderBlock headers(spdy_util_.ConstructGetHeaderBlock(request_origin));
    ...
    AlternativeServiceInfoVector altsvc_info_vector
        = spdy_session_pool_->http_server_properties()->GetAlternativeServiceInfos(url::SchemeHostPort(GURL(request_origin)), NetworkAnonymizationKey());
    ASSERT_EQ(1u, altsvc_info_vector.size());
    EXPECT_EQ(kProtoQUIC, altsvc_info_vector[0].alternative_service().protocol);
    EXPECT_EQ("alternative.example.org", altsvc_info_vector[0].alternative_service().host);
    EXPECT_EQ(443u, altsvc_info_vector[0].alternative_service().port);
}

TEST_F(AltSvcFrameTest, ProcessAltSvcFrameOnActiveStreamWithNetworkAnonymizationKey)
{
    ...
    key_ = SpdySessionKey(HostPortPair::FromURL(test_url_), PRIVACY_MODE_DISABLED, ProxyChain::Direct(), SessionUsage::kDestination, SocketTag(),
    ...
    AlternativeServiceInfoVector altsvc_info_vector
        = spdy_session_pool_->http_server_properties()->GetAlternativeServiceInfos(url::SchemeHostPort(GURL(request_origin)), kNetworkAnonymizationKey1);
    ASSERT_EQ(1u, altsvc_info_vector.size());
    EXPECT_EQ(kProtoQUIC, altsvc_info_vector[0].alternative_service().protocol);
    EXPECT_EQ("alternative.example.org", altsvc_info_vector[0].alternative_service().host);
    EXPECT_EQ(443u, altsvc_info_vector[0].alternative_service().port);
    ...
    EXPECT_TRUE(
        spdy_session_pool_->http_server_properties()->GetAlternativeServiceInfos(url::SchemeHostPort(GURL(request_origin)), kNetworkAnonymizationKey2).empty());
    EXPECT_TRUE(
        spdy_session_pool_->http_server_properties()->GetAlternativeServiceInfos(url::SchemeHostPort(GURL(request_origin)), NetworkAnonymizationKey()).empty());
}
...
{
    CHECK_EQ(SPDY_ERROR_INVALID_CONTROL_FRAME, MapFramerErrorToProtocolError(http2::Http2DecoderAdapter::SPDY_INVALID_CONTROL_FRAME));
    CHECK_EQ(SPDY_ERROR_INVALID_DATA_FRAME_FLAGS, MapFramerErrorToProtocolError(http2::Http2DecoderAdapter::SPDY_INVALID_DATA_FRAME_FLAGS));
    CHECK_EQ(SPDY_ERROR_HPACK_NAME_HUFFMAN_ERROR, MapFramerErrorToProtocolError(http2::Http2DecoderAdapter::SPDY_HPACK_NAME_HUFFMAN_ERROR));
    CHECK_EQ(SPDY_ERROR_UNEXPECTED_FRAME, MapFramerErrorToProtocolError(http2::Http2DecoderAdapter::SPDY_UNEXPECTED_FRAME));
}

TEST(MapFramerErrorToNetError, MapsValue)
{
    CHECK_EQ(ERR_HTTP2_PROTOCOL_ERROR, MapFramerErrorToNetError(http2::Http2DecoderAdapter::SPDY_INVALID_CONTROL_FRAME));
    CHECK_EQ(ERR_HTTP2_COMPRESSION_ERROR, MapFramerErrorToNetError(http2::Http2DecoderAdapter::SPDY_DECOMPRESS_FAILURE));
    CHECK_EQ(ERR_HTTP2_FRAME_SIZE_ERROR, MapFramerErrorToNetError(http2::Http2DecoderAdapter::SPDY_CONTROL_PAYLOAD_TOO_LARGE));
    CHECK_EQ(ERR_HTTP2_FRAME_SIZE_ERROR, MapFramerErrorToNetError(http2::Http2DecoderAdapter::SPDY_OVERSIZED_PAYLOAD));
}

TEST(MapRstStreamStatusToProtocolError, MapsValues)
{
    CHECK_EQ(STATUS_CODE_PROTOCOL_ERROR, MapRstStreamStatusToProtocolError(spdy::ERROR_CODE_PROTOCOL_ERROR));
    CHECK_EQ(STATUS_CODE_FRAME_SIZE_ERROR, MapRstStreamStatusToProtocolError(spdy::ERROR_CODE_FRAME_SIZE_ERROR));
    CHECK_EQ(STATUS_CODE_ENHANCE_YOUR_CALM, MapRstStreamStatusToProtocolError(spdy::ERROR_CODE_ENHANCE_YOUR_CALM));
    CHECK_EQ(STATUS_CODE_INADEQUATE_SECURITY, MapRstStreamStatusToProtocolError(spdy::ERROR_CODE_INADEQUATE_SECURITY));
    CHECK_EQ(STATUS_CODE_HTTP_1_1_REQUIRED, MapRstStreamStatusToProtocolError(spdy::ERROR_CODE_HTTP_1_1_REQUIRED));
}

TEST(MapNetErrorToGoAwayStatus, MapsValue)
{
    CHECK_EQ(spdy::ERROR_CODE_INADEQUATE_SECURITY, MapNetErrorToGoAwayStatus(ERR_HTTP2_INADEQUATE_TRANSPORT_SECURITY));
    CHECK_EQ(spdy::ERROR_CODE_FLOW_CONTROL_ERROR, MapNetErrorToGoAwayStatus(ERR_HTTP2_FLOW_CONTROL_ERROR));
    CHECK_EQ(spdy::ERROR_CODE_PROTOCOL_ERROR, MapNetErrorToGoAwayStatus(ERR_HTTP2_PROTOCOL_ERROR));
    CHECK_EQ(spdy::ERROR_CODE_COMPRESSION_ERROR, MapNetErrorToGoAwayStatus(ERR_HTTP2_COMPRESSION_ERROR));
    CHECK_EQ(spdy::ERROR_CODE_FRAME_SIZE_ERROR, MapNetErrorToGoAwayStatus(ERR_HTTP2_FRAME_SIZE_ERROR));
    CHECK_EQ(spdy::ERROR_CODE_PROTOCOL_ERROR, MapNetErrorToGoAwayStatus(ERR_UNEXPECTED));
}
...
public:
    ...
    bool CanShareConnectionWithClientCerts(std::string_view hostname) const override
    {
...
TEST(CanPoolTest, CanPool)
{
    ...
    EXPECT_TRUE(SpdySession::CanPool(&tss, ssl_info, ssl_config_service, "www.example.org", "www.example.org"));
    EXPECT_TRUE(SpdySession::CanPool(&tss, ssl_info, ssl_config_service, "www.example.org", "mail.example.org"));
    EXPECT_TRUE(SpdySession::CanPool(&tss, ssl_info, ssl_config_service, "www.example.org", "mail.example.com"));
    EXPECT_FALSE(SpdySession::CanPool(&tss, ssl_info, ssl_config_service, "www.example.org", "mail.google.com"));
}
...
TEST(CanPoolTest, CanNotPoolWithCertErrors)
{
    ...
    EXPECT_FALSE(SpdySession::CanPool(&tss, ssl_info, ssl_config_service, "www.example.org", "mail.example.org"));
}
...
TEST(CanPoolTest, CanNotPoolWithBadPins)
{
    ...
    EXPECT_FALSE(SpdySession::CanPool(&tss, ssl_info, ssl_config_service, "www.example.org", "example.test"));
}
...
TEST(CanPoolTest, CanNotPoolWithBadCTWhenCTRequired)
{
    ...
    ssl_info.ct_policy_compliance = ct::CTPolicyCompliance::CT_POLICY_NOT_ENOUGH_SCTS;

    MockRequireCTDelegate require_ct_delegate;
    EXPECT_CALL(require_ct_delegate, IsCTRequiredForHost("www.example.org", _, _)).WillRepeatedly(Return(CTRequirementLevel::NOT_REQUIRED));
    EXPECT_CALL(require_ct_delegate, IsCTRequiredForHost("mail.example.org", _, _)).WillRepeatedly(Return(CTRequirementLevel::REQUIRED));
...
TEST(CanPoolTest, CanPoolWithBadCTWhenCTNotRequired)
{
    ...
    MockRequireCTDelegate require_ct_delegate;
    EXPECT_CALL(require_ct_delegate, IsCTRequiredForHost("www.example.org", _, _)).WillRepeatedly(Return(CTRequirementLevel::NOT_REQUIRED));
    EXPECT_CALL(require_ct_delegate, IsCTRequiredForHost("mail.example.org", _, _)).WillRepeatedly(Return(CTRequirementLevel::NOT_REQUIRED));
...
TEST(CanPoolTest, CanPoolWithGoodCTWhenCTRequired)
{
    ...
    ssl_info.is_issued_by_known_root = true;
    ssl_info.public_key_hashes.push_back(test::GetTestHashValue(1));
    ssl_info.ct_policy_compliance = ct::CTPolicyCompliance::CT_POLICY_COMPLIES_VIA_SCTS;
...
TEST(CanPoolTest, CanPoolWithAcceptablePins)
{
    ...
    ASSERT_TRUE(hash.FromString("sha256/Nn8jk5By4Vkq6BeOVZ7R7AC6XUUBZsWmUbJR1f1Y5FY="));
...
TEST(CanPoolTest, CanPoolWithClientCertsAndPolicy)
{
    ...
    TestSSLConfigService ssl_config_service;
    ssl_config_service.SetDomainsForPooling({ "www.example.org", "mail.example.org" });
    ...
    // just one hostname.
    EXPECT_TRUE(SpdySession::CanPool(&tss, ssl_info, ssl_config_service, "www.example.org", "mail.example.org"));
    EXPECT_FALSE(SpdySession::CanPool(&tss, ssl_info, ssl_config_service, "www.example.org", "mail.example.com"));
    EXPECT_FALSE(SpdySession::CanPool(&tss, ssl_info, ssl_config_service, "mail.example.com", "www.example.org"));
}
...
TEST_F(SpdySessionTest, UpdateHeaderTableSize)
{
    ...
    MockRead reads[] = { CreateMockRead(settings_frame, 0), MockRead(ASYNC, ERR_IO_PENDING, 2), MockRead(ASYNC, 0, 3) };
...
TEST_F(SpdySessionTest, SettingsDeprecateHttp2PrioritiesValueMustNotChange)
{
    ...
    MockRead reads[] = { CreateMockRead(settings_frame1, 0), MockRead(ASYNC, ERR_IO_PENDING, 2), CreateMockRead(settings_frame1, 3),
        MockRead(ASYNC, ERR_IO_PENDING, 5), CreateMockRead(settings_frame0, 6) };

    auto settings_ack = spdy_util_.ConstructSpdySettingsAck();
    auto goaway = spdy_util_.ConstructSpdyGoAway(0, spdy::ERROR_CODE_PROTOCOL_ERROR,
        "spdy::SETTINGS_DEPRECATE_HTTP2_PRIORITIES value changed after first "
        "SETTINGS frame.");
    MockWrite writes[] = { CreateMockWrite(settings_ack, 1), CreateMockWrite(settings_ack, 4), CreateMockWrite(settings_ack, 7), CreateMockWrite(goaway, 8) };
...
TEST_F(SpdySessionTest, AlpsEmpty)
{
    ...
    histogram_tester.ExpectUniqueSample("Net.SpdySession.AlpsDecoderStatus", static_cast<int>(AlpsDecoder::Error::kNoError), 1);
    histogram_tester.ExpectUniqueSample("Net.SpdySession.AlpsSettingParameterCount", 0, 1);
    const int kNoEntries = 0;
    histogram_tester.ExpectUniqueSample("Net.SpdySession.AlpsAcceptChEntries", kNoEntries, 1);

    histogram_tester.ExpectTotalCount("Net.SpdySession.AcceptChForOrigin", 0);
    EXPECT_EQ("", session_->GetAcceptChViaAlps(url::SchemeHostPort(GURL("https://www.example.org"))));
    histogram_tester.ExpectUniqueSample("Net.SpdySession.AcceptChForOrigin", false, 1);
}
...
TEST_F(SpdySessionTest, AlpsSettings)
{
    ...
    settings[spdy::SETTINGS_HEADER_TABLE_SIZE] = 12345;
    spdy::SpdySerializedFrame settings_frame(spdy_util_.ConstructSpdySettings(settings));
    ssl_.peer_application_settings = std::string(settings_frame.data(), settings_frame.size());
    ...
    histogram_tester.ExpectUniqueSample("Net.SpdySession.AlpsDecoderStatus", static_cast<int>(AlpsDecoder::Error::kNoError), 1);
    histogram_tester.ExpectUniqueSample("Net.SpdySession.AlpsSettingParameterCount", 1, 1);
}
...
TEST_F(SpdySessionTest, AlpsAcceptCh)
{
    ...
    ssl_.peer_application_settings = HexDecode("00001e" // length
                                               "89" // type ACCEPT_CH
                                               "00" // flags
                                               "00000000" // stream ID
                                               "0017" // origin length
    ...
    histogram_tester.ExpectUniqueSample("Net.SpdySession.AlpsAcceptChEntries", kOnlyValidEntries, 1);

    histogram_tester.ExpectTotalCount("Net.SpdySession.AcceptChForOrigin", 0);

    EXPECT_EQ("foo", session_->GetAcceptChViaAlps(url::SchemeHostPort(GURL("https://www.example.com"))));
    histogram_tester.ExpectUniqueSample("Net.SpdySession.AcceptChForOrigin", true, 1);

    EXPECT_EQ("", session_->GetAcceptChViaAlps(url::SchemeHostPort(GURL("https://www.example.org"))));
    histogram_tester.ExpectTotalCount("Net.SpdySession.AcceptChForOrigin", 2);
    histogram_tester.ExpectBucketCount("Net.SpdySession.AcceptChForOrigin", true, 1);
    histogram_tester.ExpectBucketCount("Net.SpdySession.AcceptChForOrigin", false, 1);
}
...
TEST_F(SpdySessionTest, AlpsAcceptChInvalidOrigin)
{
    ...
    ssl_.peer_application_settings = HexDecode("000017" // length
                                               "89" // type ACCEPT_CH
                                               "00" // flags
                                               "00000000" // stream ID
                                               "0010" // origin length
    ...
    histogram_tester.ExpectUniqueSample("Net.SpdySession.AlpsAcceptChEntries", kOnlyInvalidEntries, 1);
}
