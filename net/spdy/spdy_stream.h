...
public:
    ...
    public:
        ...
        virtual void OnEarlyHintsReceived(const quiche::HttpHeaderBlock& headers) = 0;

        // Called when response headers have been received.
        virtual void OnHeadersReceived(const quiche::HttpHeaderBlock& response_headers) = 0;
    ...
    SpdyStream(SpdyStreamType type, const base::WeakPtr<SpdySession>& session, const GURL& url, RequestPriority priority, int32_t initial_send_window_size,
        int32_t max_recv_window_size, const NetLogWithSource& net_log, const NetworkTrafficAnnotationTag& traffic_annotation, bool detect_broken_connection);
    ...
    void set_stream_id(spdy::SpdyStreamId stream_id)
    {
    ...
    int32_t recv_window_size() const
    {
    ...
    // the stream.
    void OnHeadersReceived(const quiche::HttpHeaderBlock& response_headers, base::Time response_time, base::TimeTicks recv_first_byte_time);
    ...
    int SendRequestHeaders(quiche::HttpHeaderBlock request_headers, SpdySendStatus send_status);
    ...
    int64_t raw_received_bytes() const
    {
        return raw_received_bytes_;
    }
    int64_t raw_sent_bytes() const
    {
        return raw_sent_bytes_;
    }
    int recv_bytes() const
    {
        return recv_bytes_;
    }
    ...
    bool detect_broken_connection() const
    {
        return detect_broken_connection_;
    }
...
private:
    ...
    enum ResponseState { READY_FOR_HEADERS, READY_FOR_DATA_OR_TRAILERS, TRAILERS_RECEIVED };
    ...
    void OnEarlyHintsReceived(const quiche::HttpHeaderBlock& response_headers, base::TimeTicks recv_first_byte_time);

    // Saves the given headers into |response_headers_| and calls
    // OnHeadersReceived() on the delegate if attached.
    void SaveResponseHeaders(const quiche::HttpHeaderBlock& response_headers, int status);
