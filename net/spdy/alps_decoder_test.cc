...
{
    AlpsDecoder decoder;
    AlpsDecoder::Error error = decoder.Decode(HexDecode("000000" // length
                                                        "89" // type ACCEPT_CH
                                                        "00" // flags
                                                        "00000000")); // stream ID
...
TEST(AlpsDecoderTest, ParseSettingsAndAcceptChFrames)
{
    ...
    EXPECT_THAT(decoder.GetAcceptCh(),
        ElementsAre(AcceptChOriginValuePair { "https://www.example.com", "foo" }, AcceptChOriginValuePair { "https://mail.example.com", "bar" }));
    EXPECT_THAT(decoder.GetSettings(), ElementsAre(Pair(0x0dab, 0x01020304), Pair(0x1234, 0xfedcba98)));
    EXPECT_EQ(1, decoder.settings_frame_count());
}
...
TEST(AlpsDecoderTest, ParseLargeAcceptChFrame)
{
    ...
    EXPECT_EQ(AlpsDecoder::Error::kNoError, error);
    EXPECT_THAT(decoder.GetAcceptCh(), ElementsAre(AcceptChOriginValuePair { "https://www.example.com", accept_ch_tokens }));
}
...
{
    AlpsDecoder decoder;
    AlpsDecoder::Error error = decoder.Decode(HexDecode("00000c" // length
                                                        "04" // type
                                                        "00" // flags
                                                        "00000000" // stream ID
                                                        "0dab" // identifier
...
{
    AlpsDecoder decoder;
    AlpsDecoder::Error error = decoder.Decode(HexDecode("000006" // length
                                                        "04" // type SETTINGS
                                                        "00" // flags
                                                        "00000000" // stream ID
                                                        "0dab" // identifier
...
{
    AlpsDecoder decoder;
    AlpsDecoder::Error error = decoder.Decode(HexDecode("00001e" // length
                                                        "89" // type ACCEPT_CH
                                                        "00" // flags
...
{
    AlpsDecoder decoder;
    AlpsDecoder::Error error = decoder.Decode(HexDecode("000003" // length
                                                        "00" // frame type DATA
                                                        "01" // flags END_STREAM
                                                        "00000001" // stream ID
                                                        "666f6f")); // payload "foo"
...
public:
    bool ShouldKillSessionOnAcceptChMalformed()
    {
        return GetParam();
    }
...
private:
    void SetUp() override
    {
        feature_list_.InitWithFeatureState(features::kShouldKillSessionOnAcceptChMalformed, ShouldKillSessionOnAcceptChMalformed());
    }
...
TEST_P(AlpsDecoderTestWithFeature, MalformedAcceptChFrame)
{
    ...
    std::string payload = HexDecode("0017" // origin length
                                    "68747470733a2f2f7777772e"
                                    "6578616d706c652e636f6d" // origin "https://www.example.com"
                                    "0003" // value length
                                    "666f6f"); // value "foo"

    for (uint8_t payload_length = 1; payload_length < payload.length(); payload_length++) {
        ...
        frame.append(HexDecode("89" // type ACCEPT_CH
                               "00" // flags
                               "00000000")); // stream ID
        ...
        if (ShouldKillSessionOnAcceptChMalformed()) {
            ...
            histogram_tester.ExpectUniqueSample("Net.SpdySession.AlpsDecoderStatus.Bypassed", static_cast<int>(AlpsDecoder::Error::kNoError), 1);
        } else {
            EXPECT_EQ(AlpsDecoder::Error::kNoError, error);
            histogram_tester.ExpectUniqueSample("Net.SpdySession.AlpsDecoderStatus.Bypassed", static_cast<int>(AlpsDecoder::Error::kAcceptChMalformed), 1);
        }
    }
}
