...
base::WeakPtr<SpdySession> CreateSpdyProxySession(const url::SchemeHostPort& destination, HttpNetworkSession* http_session, const SpdySessionKey& key,
    const CommonConnectJobParams* common_connect_job_params)
{
    EXPECT_FALSE(
        http_session->spdy_session_pool()->FindAvailableSession(key, true /* enable_ip_based_pooling */, false /* is_websocket */, NetLogWithSource()));

    auto transport_params
        = base::MakeRefCounted<TransportSocketParams>(destination, NetworkAnonymizationKey(), SecureDnsPolicy::kAllow, OnHostResolutionCallback(),
            /*supported_alpns=*/base::flat_set<std::string> { "h2", "http/1.1" });

    SSLConfig ssl_config;
    ssl_config.privacy_mode = key.privacy_mode();
    auto ssl_params = base::MakeRefCounted<SSLSocketParams>(
        ConnectJobParams(transport_params), HostPortPair::FromSchemeHostPort(destination), ssl_config, key.network_anonymization_key());
    TestConnectJobDelegate connect_job_delegate;
    SSLConnectJob connect_job(MEDIUM, SocketTag(), common_connect_job_params, ssl_params, &connect_job_delegate, nullptr /* net_log */);
    connect_job_delegate.StartJobExpectingResult(&connect_job, OK, false /* expect_sync_result */);

    base::expected<base::WeakPtr<SpdySession>, int> spdy_session_result = http_session->spdy_session_pool()->CreateAvailableSessionFromSocket(
        key, connect_job_delegate.ReleaseSocket(), LoadTimingInfo::ConnectTiming(), NetLogWithSource());
...
class SpdyProxyClientSocketTest : public PlatformTest, public WithTaskEnvironment, public ::testing::WithParamInterface<bool> {
public:
    SpdyProxyClientSocketTest();

    SpdyProxyClientSocketTest(const SpdyProxyClientSocketTest&) = delete;
    SpdyProxyClientSocketTest& operator=(const SpdyProxyClientSocketTest&) = delete;
...
protected:
    void Initialize(base::span<const MockRead> reads, base::span<const MockWrite> writes);
    void PopulateConnectRequestIR(quiche::HttpHeaderBlock* syn_ir);
    void PopulateConnectReplyIR(quiche::HttpHeaderBlock* block, const char* status);
    spdy::SpdySerializedFrame ConstructConnectRequestFrame(RequestPriority priority = LOWEST);
    ...
    spdy::SpdySerializedFrame ConstructBodyFrame(const char* data, int length, bool fin = false);
    ...
    void AddAuthToCache()
    {
        ...
        session_->http_auth_cache()->Add(url::SchemeHostPort { GURL(kProxyUrl) }, HttpAuth::AUTH_PROXY, "MyRealm1", HttpAuth::AUTH_SCHEME_BASIC,
            NetworkAnonymizationKey(), "Basic realm=MyRealm1", AuthCredentials(kFoo, kBar), "/");
    }
...
SpdyProxyClientSocketTest::SpdyProxyClientSocketTest()
    ...
    , endpoint_spdy_session_key_(endpoint_host_port_pair_, PRIVACY_MODE_DISABLED, proxy_chain_, SessionUsage::kDestination, SocketTag(),
...
void SpdyProxyClientSocketTest::Initialize(base::span<const MockRead> reads, base::span<const MockWrite> writes)
{
    ...
    spdy_session_ = CreateSpdyProxySession(url::SchemeHostPort(url_), session_.get(), endpoint_spdy_session_key_, common_connect_job_params_.get());

    base::WeakPtr<SpdyStream> spdy_stream(CreateStreamSynchronously(SPDY_BIDIRECTIONAL_STREAM, spdy_session_, url_, LOWEST, net_log_with_source_));
    ASSERT_TRUE(spdy_stream.get() != nullptr);

    // Create the SpdyProxyClientSocket.
    sock_ = std::make_unique<SpdyProxyClientSocket>(spdy_stream, proxy_chain_, /*proxy_chain_index=*/0, user_agent_, endpoint_host_port_pair_,
        net_log_with_source_,
        base::MakeRefCounted<HttpAuthController>(HttpAuth::AUTH_PROXY, GURL("https://" + proxy_host_port_.ToString()), NetworkAnonymizationKey(),
...
scoped_refptr<IOBufferWithSize> SpdyProxyClientSocketTest::CreateBuffer(const char* data, int size)
{
    scoped_refptr<IOBufferWithSize> buf = base::MakeRefCounted<IOBufferWithSize>(size);
...
{
    ASSERT_THAT(sock_->Connect(read_callback_.callback()), IsError(ERR_IO_PENDING));
...
void SpdyProxyClientSocketTest::AssertSyncReadEquals(const char* data, int len)
{
    auto buf = base::MakeRefCounted<IOBufferWithSize>(len);
    if (use_read_if_ready()) {
        ASSERT_EQ(len, sock_->ReadIfReady(buf.get(), len, CompletionOnceCallback()));
    } else {
...
void SpdyProxyClientSocketTest::AssertAsyncReadEquals(const char* data, int len, bool fin)
{
    // Issue the read, which will be completed asynchronously
    auto buf = base::MakeRefCounted<IOBufferWithSize>(len);
    if (use_read_if_ready()) {
        ASSERT_EQ(ERR_IO_PENDING, sock_->ReadIfReady(buf.get(), len, read_callback_.callback()));
    } else {
        ASSERT_EQ(ERR_IO_PENDING, sock_->Read(buf.get(), len, read_callback_.callback()));
    }
    ...
    if (use_read_if_ready()) {
        ...
        ASSERT_EQ(len, sock_->ReadIfReady(buf.get(), len, read_callback_.callback()));
    } else {
...
void SpdyProxyClientSocketTest::AssertReadStarts(const char* data, int len)
{
    ...
    if (use_read_if_ready()) {
        ASSERT_EQ(ERR_IO_PENDING, sock_->ReadIfReady(read_buf_.get(), len, read_callback_.callback()));
    } else {
        ASSERT_EQ(ERR_IO_PENDING, sock_->Read(read_buf_.get(), len, read_callback_.callback()));
    }
    EXPECT_TRUE(sock_->IsConnected());
}
...
void SpdyProxyClientSocketTest::AssertReadReturns(const char* data, int len)
{
    ...
    if (use_read_if_ready()) {
        EXPECT_EQ(OK, read_callback_.WaitForResult());
        ASSERT_EQ(len, sock_->ReadIfReady(read_buf_.get(), len, read_callback_.callback()));
    } else {
        EXPECT_EQ(len, read_callback_.WaitForResult());
    }
...
void SpdyProxyClientSocketTest::AssertAsyncWriteSucceeds(const char* data, int len)
{
    AssertWriteReturns(data, len, ERR_IO_PENDING);
    AssertWriteLength(len);
}

void SpdyProxyClientSocketTest::AssertWriteReturns(const char* data, int len, int rv)
{
    scoped_refptr<IOBufferWithSize> buf(CreateBuffer(data, len));
    EXPECT_EQ(rv, sock_->Write(buf.get(), buf->size(), write_callback_.callback(), TRAFFIC_ANNOTATION_FOR_TESTS));
}
...
void SpdyProxyClientSocketTest::PopulateConnectRequestIR(quiche::HttpHeaderBlock* block)
{
...
void SpdyProxyClientSocketTest::PopulateConnectReplyIR(quiche::HttpHeaderBlock* block, const char* status)
{
    (*block)[spdy::kHttp2StatusHeader] = status;
}

// Constructs a standard SPDY HEADERS frame for a CONNECT request.
spdy::SpdySerializedFrame SpdyProxyClientSocketTest::ConstructConnectRequestFrame(RequestPriority priority)
{
    quiche::HttpHeaderBlock block;
    PopulateConnectRequestIR(&block);
    return spdy_util_.ConstructSpdyHeaders(kStreamId, std::move(block), priority, false);
}

// Constructs a SPDY HEADERS frame for a CONNECT request which includes
// Proxy-Authorization headers.
spdy::SpdySerializedFrame SpdyProxyClientSocketTest::ConstructConnectAuthRequestFrame()
{
    quiche::HttpHeaderBlock block;
    PopulateConnectRequestIR(&block);
    block["proxy-authorization"] = "Basic Zm9vOmJhcg==";
    return spdy_util_.ConstructSpdyHeaders(kStreamId, std::move(block), LOWEST, false);
}

// Constructs a standard SPDY HEADERS frame to match the SPDY CONNECT.
spdy::SpdySerializedFrame SpdyProxyClientSocketTest::ConstructConnectReplyFrame()
{
    quiche::HttpHeaderBlock block;
...
spdy::SpdySerializedFrame SpdyProxyClientSocketTest::ConstructConnectAuthReplyFrame()
{
    quiche::HttpHeaderBlock block;
...
spdy::SpdySerializedFrame SpdyProxyClientSocketTest::ConstructConnectRedirectReplyFrame()
{
    quiche::HttpHeaderBlock block;
...
spdy::SpdySerializedFrame SpdyProxyClientSocketTest::ConstructConnectErrorReplyFrame()
{
    quiche::HttpHeaderBlock block;
...
spdy::SpdySerializedFrame SpdyProxyClientSocketTest::ConstructBodyFrame(const char* data, int length, bool fin)
{
    return spdy_util_.ConstructSpdyDataFrame(kStreamId, std::string_view(data, length), fin);
}

// ----------- Connect

INSTANTIATE_TEST_SUITE_P(All, SpdyProxyClientSocketTest, ::testing::Bool());
...
TEST_P(SpdyProxyClientSocketTest, GetPeerAddressReturnsCorrectValues)
{
    ...
    spdy::SpdySerializedFrame resp(ConstructConnectReplyFrame());
    MockRead reads[] = {
        CreateMockRead(resp, 1, ASYNC), MockRead(ASYNC, ERR_IO_PENDING, 2), MockRead(ASYNC, 0, 3), // EOF
    };
...
TEST_P(SpdyProxyClientSocketTest, WriteSplitsLargeDataIntoMultipleFrames)
{
    ...
    spdy::SpdySerializedFrame conn(ConstructConnectRequestFrame());
    spdy::SpdySerializedFrame chunk(ConstructBodyFrame(chunk_data.data(), chunk_data.length()));
    MockWrite writes[] = { CreateMockWrite(conn, 0, SYNCHRONOUS), CreateMockWrite(chunk, 3, SYNCHRONOUS), CreateMockWrite(chunk, 4, SYNCHRONOUS),
    ...
    scoped_refptr<IOBufferWithSize> buf(CreateBuffer(big_data.data(), big_data.length()));

    EXPECT_EQ(ERR_IO_PENDING, sock_->Write(buf.get(), buf->size(), write_callback_.callback(), TRAFFIC_ANNOTATION_FOR_TESTS));
...
TEST_P(SpdyProxyClientSocketTest, ReadErrorResponseBody)
{
    ...
    spdy::SpdySerializedFrame msg2(ConstructBodyFrame(kMsg2, kLen2));
    MockRead reads[] = {
        CreateMockRead(resp, 1, ASYNC), CreateMockRead(msg1, 2, SYNCHRONOUS), CreateMockRead(msg2, 3, SYNCHRONOUS), MockRead(SYNCHRONOUS, 0, 4), // EOF
    };
...
TEST_P(SpdyProxyClientSocketTest, ReadOnDisconnectSocketReturnsNotConnected)
{
    ...
    if (use_read_if_ready()) {
        ASSERT_EQ(ERR_SOCKET_NOT_CONNECTED, sock_->ReadIfReady(nullptr, 1, CompletionOnceCallback()));
    } else {
        ASSERT_EQ(ERR_SOCKET_NOT_CONNECTED, sock_->Read(nullptr, 1, CompletionOnceCallback()));
    }
...
TEST_P(SpdyProxyClientSocketTest, ReadOnClosedSocketReturnsBufferedData)
{
    ...
    spdy::SpdySerializedFrame msg1(ConstructBodyFrame(kMsg1, kLen1));
    MockRead reads[] = {
        CreateMockRead(resp, 1, ASYNC), MockRead(ASYNC, ERR_IO_PENDING, 2), CreateMockRead(msg1, 3, ASYNC), MockRead(ASYNC, 0, 4), // EOF
    };
...
TEST_P(SpdyProxyClientSocketTest, WriteOnClosedStream)
{
    ...
    EXPECT_EQ(ERR_SOCKET_NOT_CONNECTED, sock_->Write(buf.get(), buf->size(), CompletionOnceCallback(), TRAFFIC_ANNOTATION_FOR_TESTS));
}
...
TEST_P(SpdyProxyClientSocketTest, DisconnectWithReadPending)
{
    ...
    auto buf = base::MakeRefCounted<IOBufferWithSize>(kLen1);
    ASSERT_EQ(ERR_IO_PENDING, sock_->Read(buf.get(), kLen1, read_callback_.callback()));
...
TEST_P(SpdyProxyClientSocketTest, RstWithReadAndWritePending)
{
    ...
    spdy::SpdySerializedFrame rst(spdy_util_.ConstructSpdyRstStream(1, spdy::ERROR_CODE_CANCEL));
    MockRead reads[] = {
        CreateMockRead(resp, 1, ASYNC), MockRead(ASYNC, ERR_IO_PENDING, 2), CreateMockRead(rst, 3, ASYNC), MockRead(ASYNC, 0, 4) // EOF
    };
    ...
    ASSERT_EQ(ERR_IO_PENDING, sock_->Read(read_buf.get(), kLen1, read_callback_.callback()));

    scoped_refptr<IOBufferWithSize> write_buf(CreateBuffer(kMsg1, kLen1));
    EXPECT_EQ(ERR_IO_PENDING, sock_->Write(write_buf.get(), write_buf->size(), write_callback_.callback(), TRAFFIC_ANNOTATION_FOR_TESTS));
...
TEST_P(SpdyProxyClientSocketTest, NetLog)
{
    ...
    EXPECT_TRUE(LogContainsBeginEvent(entry_list, 0, NetLogEventType::SOCKET_ALIVE));
    EXPECT_TRUE(LogContainsEvent(entry_list, 1, NetLogEventType::HTTP2_PROXY_CLIENT_SESSION, NetLogEventPhase::NONE));
    EXPECT_TRUE(LogContainsBeginEvent(entry_list, 2, NetLogEventType::HTTP_TRANSACTION_TUNNEL_SEND_REQUEST));
    EXPECT_TRUE(LogContainsEvent(entry_list, 3, NetLogEventType::HTTP_TRANSACTION_SEND_TUNNEL_HEADERS, NetLogEventPhase::NONE));
    EXPECT_TRUE(LogContainsEndEvent(entry_list, 4, NetLogEventType::HTTP_TRANSACTION_TUNNEL_SEND_REQUEST));
    EXPECT_TRUE(LogContainsBeginEvent(entry_list, 5, NetLogEventType::HTTP_TRANSACTION_TUNNEL_READ_HEADERS));
    EXPECT_TRUE(LogContainsEvent(entry_list, 6, NetLogEventType::HTTP_TRANSACTION_READ_TUNNEL_RESPONSE_HEADERS, NetLogEventPhase::NONE));
    EXPECT_TRUE(LogContainsEndEvent(entry_list, 7, NetLogEventType::HTTP_TRANSACTION_TUNNEL_READ_HEADERS));
    EXPECT_TRUE(LogContainsEvent(entry_list, 8, NetLogEventType::SOCKET_BYTES_RECEIVED, NetLogEventPhase::NONE));
    EXPECT_TRUE(LogContainsEndEvent(entry_list, 9, NetLogEventType::SOCKET_ALIVE));
...
public:
    ...
    {
        return base::BindOnce(&DeleteSockCallback::OnComplete, base::Unretained(this));
    }
...
TEST_P(SpdyProxyClientSocketTest, RstWithReadAndWritePendingDelete)
{
    ...
    auto read_buf = base::MakeRefCounted<IOBufferWithSize>(kLen1);
    ASSERT_EQ(ERR_IO_PENDING, sock_->Read(read_buf.get(), kLen1, read_callback.callback()));
...
{
    spdy::SpdySerializedFrame conn(ConstructConnectRequestFrame());
    spdy::SpdySerializedFrame end_stream(ConstructBodyFrame(/*data=*/nullptr, /*length=*/0, /*fin=*/true));
    MockWrite writes[] = {
        CreateMockWrite(conn, 0, SYNCHRONOUS),
    ...
    spdy::SpdySerializedFrame resp(ConstructConnectReplyFrame());
    spdy::SpdySerializedFrame msg1(ConstructBodyFrame(kMsg1, kLen1));
    spdy::SpdySerializedFrame msg2(ConstructBodyFrame(kMsg2, kLen2, /*fin=*/true));
    MockRead reads[] = {
        CreateMockRead(resp, 1, ASYNC),
        MockRead(ASYNC, ERR_IO_PENDING, 2),
        CreateMockRead(msg1, 3, ASYNC),
...
TEST_P(SpdyProxyClientSocketTest, SendEndStreamAfterWrite)
{
    ...
    spdy::SpdySerializedFrame resp(ConstructConnectReplyFrame());
    spdy::SpdySerializedFrame read_msg(ConstructBodyFrame(kMsg2, kLen2, /*fin=*/true));
    MockRead reads[] = {
        CreateMockRead(resp, 1, ASYNC),
        MockRead(ASYNC, ERR_IO_PENDING, 2),
...
TEST_P(SpdyProxyClientSocketTest, WriteAfterStreamEndSent)
{
    ...
    spdy::SpdySerializedFrame resp(ConstructConnectReplyFrame());
    spdy::SpdySerializedFrame read_msg(ConstructBodyFrame(kMsg1, kLen1, /*fin=*/true));
    MockRead reads[] = {
        CreateMockRead(resp, 1, ASYNC),
        MockRead(ASYNC, ERR_IO_PENDING, 2),
        CreateMockRead(read_msg, 3, ASYNC),
