...
public:
    explicit MultiplexedHttpStream(std::unique_ptr<MultiplexedSessionHandle> session);
...
protected:
    void DispatchRequestHeadersCallback(const quiche::HttpHeaderBlock& spdy_headers);

    MultiplexedSessionHandle* session()
    {
        return session_.get();
    }
    const MultiplexedSessionHandle* session() const
    {
        return session_.get();
    }
