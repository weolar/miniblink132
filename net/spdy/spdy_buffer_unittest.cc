...
{
    SpdyBuffer buffer(std::make_unique<spdy::SpdySerializedFrame>(spdy::test::MakeSerializedFrame(const_cast<char*>(kData), kDataSize)));

    EXPECT_EQ(kDataSize, buffer.GetRemainingSize());
    EXPECT_EQ(std::string_view(kData, kDataSize), std::string_view(buffer.GetRemainingData(), buffer.GetRemainingSize()));
}
...
void IncrementBy(size_t* x, SpdyBuffer::ConsumeSource expected_consume_source, size_t delta, SpdyBuffer::ConsumeSource consume_source)
{
...
TEST_F(SpdyBufferTest, Consume)
{
    ...
    size_t x1 = 0;
    size_t x2 = 0;
    buffer.AddConsumeCallback(base::BindRepeating(&IncrementBy, &x1, SpdyBuffer::CONSUME));
    buffer.AddConsumeCallback(base::BindRepeating(&IncrementBy, &x2, SpdyBuffer::CONSUME));
...
TEST_F(SpdyBufferTest, ConsumeOnDestruction)
{
    ...
    {
        SpdyBuffer buffer(kData, kDataSize);
        buffer.AddConsumeCallback(base::BindRepeating(&IncrementBy, &x, SpdyBuffer::DISCARD));
    }
