...
constexpr net::NetworkTrafficAnnotationTag kSpdySessionCommandsTrafficAnnotation = net::DefineNetworkTrafficAnnotation("spdy_session_control", R"(
        semantics {
          sender: "Spdy Session"
          description:
...
public:
    ...
    GreasedBufferProducer(
        base::WeakPtr<SpdyStream> stream, const SpdySessionPool::GreasedHttp2Frame* greased_http2_frame, BufferedSpdyFramer* buffered_spdy_framer)
        : stream_(stream)
    ...
    std::unique_ptr<SpdyBuffer> ProduceBuffer() override
    {
        ...
        spdy::SpdyUnknownIR frame(stream_id, greased_http2_frame_->type, greased_http2_frame_->flags, greased_http2_frame_->payload);
        auto serialized_frame = std::make_unique<spdy::SpdySerializedFrame>(buffered_spdy_framer_->SerializeFrame(frame));
...
bool IsSpdySettingAtDefaultInitialValue(spdy::SpdySettingsId setting_id, uint32_t value)
{
    switch (setting_id) {
...
{
    std::string histogram_name = base::StrCat({ "Net.SpdySession.GoogleSearch.SessionCreationInitiator", is_used ? ".Used" : ".Unused" });

    base::UmaHistogramEnumeration(histogram_name, session_creation);
}

base::Value::Dict NetLogSpdyHeadersSentParams(const quiche::HttpHeaderBlock* headers, bool fin, spdy::SpdyStreamId stream_id, bool has_priority, int weight,
    spdy::SpdyStreamId parent_stream_id, bool exclusive, NetLogSource source_dependency, NetLogCaptureMode capture_mode)
{
...
base::Value::Dict NetLogSpdyHeadersReceivedParams(
    const quiche::HttpHeaderBlock* headers, bool fin, spdy::SpdyStreamId stream_id, NetLogCaptureMode capture_mode)
{
...
base::Value::Dict NetLogSpdySessionCloseParams(int net_error, const std::string& description)
{
    return base::Value::Dict().Set("net_error", net_error).Set("description", description);
}

base::Value::Dict NetLogSpdySessionParams(const HostPortProxyPair& host_pair)
{
    return base::Value::Dict().Set("host", host_pair.first.ToString()).Set("proxy", host_pair.second.ToDebugString());
}
...
base::Value::Dict NetLogSpdySendSettingsParams(const spdy::SettingsMap* settings)
{
    base::Value::List settings_list;
    for (const auto& setting : *settings) {
        const spdy::SpdySettingsId id = setting.first;
        const uint32_t value = setting.second;
        settings_list.Append(base::StringPrintf("[id:%u (%s) value:%u]", id, spdy::SettingsIdToString(id).c_str(), value));
    }

    return base::Value::Dict().Set("settings", std::move(settings_list));
}

base::Value::Dict NetLogSpdyRecvAcceptChParams(spdy::AcceptChOriginValuePair entry)
{
    return base::Value::Dict().Set("origin", entry.origin).Set("accept_ch", entry.value);
}

base::Value::Dict NetLogSpdyRecvSettingParams(spdy::SpdySettingsId id, uint32_t value)
{
    return base::Value::Dict().Set("id", base::StringPrintf("%u (%s)", id, spdy::SettingsIdToString(id).c_str())).Set("value", static_cast<int>(value));
}

base::Value::Dict NetLogSpdyWindowUpdateFrameParams(spdy::SpdyStreamId stream_id, uint32_t delta)
{
    return base::Value::Dict().Set("stream_id", static_cast<int>(stream_id)).Set("delta", static_cast<int>(delta));
}

base::Value::Dict NetLogSpdySessionWindowUpdateParams(int32_t delta, int32_t window_size)
{
    return base::Value::Dict().Set("delta", delta).Set("window_size", window_size);
}

base::Value::Dict NetLogSpdyDataParams(spdy::SpdyStreamId stream_id, int size, bool fin)
{
    return base::Value::Dict().Set("stream_id", static_cast<int>(stream_id)).Set("size", size).Set("fin", fin);
}

base::Value::Dict NetLogSpdyRecvRstStreamParams(spdy::SpdyStreamId stream_id, spdy::SpdyErrorCode error_code)
{
    return base::Value::Dict()
        .Set("stream_id", static_cast<int>(stream_id))
        .Set("error_code", base::StringPrintf("%u (%s)", error_code, ErrorCodeToString(error_code)));
}

base::Value::Dict NetLogSpdySendRstStreamParams(spdy::SpdyStreamId stream_id, spdy::SpdyErrorCode error_code, const std::string& description)
{
    return base::Value::Dict()
        .Set("stream_id", static_cast<int>(stream_id))
        .Set("error_code", base::StringPrintf("%u (%s)", error_code, ErrorCodeToString(error_code)))
...
base::Value::Dict NetLogSpdyPingParams(spdy::SpdyPingId unique_id, bool is_ack, const char* type)
{
    return base::Value::Dict().Set("unique_id", static_cast<int>(unique_id)).Set("type", type).Set("is_ack", is_ack);
}

base::Value::Dict NetLogSpdyRecvGoAwayParams(
    spdy::SpdyStreamId last_stream_id, int active_streams, spdy::SpdyErrorCode error_code, std::string_view debug_data, NetLogCaptureMode capture_mode)
{
    return base::Value::Dict()
        .Set("last_accepted_stream_id", static_cast<int>(last_stream_id))
        .Set("active_streams", active_streams)
        .Set("error_code", base::StringPrintf("%u (%s)", error_code, ErrorCodeToString(error_code)))
        .Set("debug_data", ElideGoAwayDebugDataForNetLog(capture_mode, debug_data));
}

base::Value::Dict NetLogSpdySessionStalledParams(size_t num_active_streams, size_t num_created_streams, size_t max_concurrent_streams, const std::string& url)
{
    return base::Value::Dict()
...
base::Value::Dict NetLogSpdyPriorityParams(spdy::SpdyStreamId stream_id, spdy::SpdyStreamId parent_stream_id, int weight, bool exclusive)
{
    return base::Value::Dict()
        .Set("stream_id", static_cast<int>(stream_id))
...
base::Value::Dict NetLogSpdyGreasedFrameParams(spdy::SpdyStreamId stream_id, uint8_t type, uint8_t flags, size_t length, RequestPriority priority)
{
    return base::Value::Dict()
        .Set("stream_id", static_cast<int>(stream_id))
        .Set("type", type)
...
SpdyProtocolErrorDetails MapFramerErrorToProtocolError(http2::Http2DecoderAdapter::SpdyFramerError err)
{
    switch (err) {
    ...
    case http2::Http2DecoderAdapter::SPDY_HPACK_MISSING_DYNAMIC_TABLE_SIZE_UPDATE:
    ...
    case http2::Http2DecoderAdapter::SPDY_HPACK_DYNAMIC_TABLE_SIZE_UPDATE_NOT_ALLOWED:
        return SPDY_ERROR_HPACK_DYNAMIC_TABLE_SIZE_UPDATE_NOT_ALLOWED;
    case http2::Http2DecoderAdapter::SPDY_HPACK_INITIAL_DYNAMIC_TABLE_SIZE_UPDATE_IS_ABOVE_LOW_WATER_MARK:
        return SPDY_ERROR_HPACK_INITIAL_DYNAMIC_TABLE_SIZE_UPDATE_IS_ABOVE_LOW_WATER_MARK;
    case http2::Http2DecoderAdapter::SPDY_HPACK_DYNAMIC_TABLE_SIZE_UPDATE_IS_ABOVE_ACKNOWLEDGED_SETTING:
    ...
    case http2::Http2DecoderAdapter::SPDY_HPACK_COMPRESSED_HEADER_SIZE_EXCEEDS_LIMIT:
...
Error MapFramerErrorToNetError(http2::Http2DecoderAdapter::SpdyFramerError err)
{
    switch (err) {
    case http2::Http2DecoderAdapter::SPDY_NO_ERROR:
        return OK;
...
SpdyProtocolErrorDetails MapRstStreamStatusToProtocolError(spdy::SpdyErrorCode error_code)
{
    switch (error_code) {
...
int SpdyStreamRequest::StartRequest(SpdyStreamType type, const base::WeakPtr<SpdySession>& session, const GURL& url, bool can_send_early,
    RequestPriority priority, const SocketTag& socket_tag, const NetLogWithSource& net_log, CompletionOnceCallback callback,
    const NetworkTrafficAnnotationTag& traffic_annotation, bool detect_broken_connection, base::TimeDelta heartbeat_interval)
{
    DCHECK(session);
    DCHECK(!session_);
    DCHECK(!stream_);
    ...
    int rv = OK;
    if (!can_send_early) {
        rv = session_->ConfirmHandshake(base::BindOnce(&SpdyStreamRequest::OnConfirmHandshakeComplete, weak_ptr_factory_.GetWeakPtr()));
    }
    if (rv != OK) {
...
void SpdyStreamRequest::SetPriority(RequestPriority priority)
{
    ...
    if (session_)
        session_->ChangeStreamRequestPriority(weak_ptr_factory_.GetWeakPtr(), priority);
    priority_ = priority;
}

void SpdyStreamRequest::OnRequestCompleteSuccess(const base::WeakPtr<SpdyStream>& stream)
{
    DCHECK(session_);
    DCHECK(!stream_);
    DCHECK(!callback_.is_null());
...
// static
bool SpdySession::CanPool(TransportSecurityState* transport_security_state, const SSLInfo& ssl_info, const SSLConfigService& ssl_config_service,
    std::string_view old_hostname, std::string_view new_hostname)
{
    ...
    if (ssl_info.client_cert_sent
        && !(ssl_config_service.CanShareConnectionWithClientCerts(old_hostname) && ssl_config_service.CanShareConnectionWithClientCerts(new_hostname))) {
        return false;
    }
    ...
    if (transport_security_state->CheckPublicKeyPins(HostPortPair(new_hostname, 0), ssl_info.is_issued_by_known_root, ssl_info.public_key_hashes)
        == TransportSecurityState::PKPStatus::VIOLATED) {
        return false;
    }

    switch (transport_security_state->CheckCTRequirements(
        HostPortPair(new_hostname, 0), ssl_info.is_issued_by_known_root, ssl_info.public_key_hashes, ssl_info.cert.get(), ssl_info.ct_policy_compliance)) {
...
SpdySession::SpdySession(const SpdySessionKey& spdy_session_key, HttpServerProperties* http_server_properties, TransportSecurityState* transport_security_state,
    SSLConfigService* ssl_config_service, const quic::ParsedQuicVersionVector& quic_supported_versions, bool enable_sending_initial_data,
    bool enable_ping_based_connection_checking, bool is_http2_enabled, bool is_quic_enabled, size_t session_max_recv_window_size,
    int session_max_queued_capped_frames, const spdy::SettingsMap& initial_settings, bool enable_http2_settings_grease,
    const std::optional<SpdySessionPool::GreasedHttp2Frame>& greased_http2_frame, bool http2_end_stream_with_data_frame, bool enable_priority_update,
    TimeFunc time_func, NetworkQualityEstimator* network_quality_estimator, NetLog* net_log, MultiplexedSessionCreationInitiator session_creation_initiator)
    ...
    , time_to_buffer_small_window_updates_(kDefaultTimeToBufferSmallWindowUpdates)
    , stream_initial_send_window_size_(kDefaultInitialWindowSize)
    , max_header_table_size_(initial_settings.at(spdy::SETTINGS_HEADER_TABLE_SIZE))
    , stream_max_recv_window_size_(initial_settings.at(spdy::SETTINGS_INITIAL_WINDOW_SIZE))
    , net_log_(NetLogWithSource::Make(net_log, NetLogSourceType::HTTP2_SESSION))
    , quic_supported_versions_(quic_supported_versions)
    , enable_sending_initial_data_(enable_sending_initial_data)
    , enable_ping_based_connection_checking_(enable_ping_based_connection_checking)
    , is_http2_enabled_(is_http2_enabled)
    , is_quic_enabled_(is_quic_enabled)
    , connection_at_risk_of_loss_time_(base::Seconds(kDefaultConnectionAtRiskOfLossSeconds))
    , hung_interval_(base::Seconds(kHungIntervalSeconds))
    , time_func_(time_func)
    , network_quality_estimator_(network_quality_estimator)
    , session_creation_initiator_(session_creation_initiator)
{
    net_log_.BeginEvent(NetLogEventType::HTTP2_SESSION, [&] { return NetLogSpdySessionParams(host_port_proxy_pair()); });
...
void SpdySession::InitializeWithSocketHandle(std::unique_ptr<StreamSocketHandle> stream_socket_handle, SpdySessionPool* pool)
{
...
void SpdySession::InitializeWithSocket(std::unique_ptr<StreamSocket> stream_socket, const LoadTimingInfo::ConnectTiming& connect_timing, SpdySessionPool* pool)
{
    DCHECK(!stream_socket_handle_);
    DCHECK(!owned_stream_socket_);
    DCHECK(!socket_);
    ...
    connect_timing_ = std::make_unique<LoadTimingInfo::ConnectTiming>(connect_timing);
...
int SpdySession::ParseAlps()
{
    ...
    if (error != AlpsDecoder::Error::kNoError) {
        DoDrainSession(ERR_HTTP2_PROTOCOL_ERROR, base::StrCat({ "Error parsing ALPS: ", base::NumberToString(static_cast<int>(error)) }));
        return ERR_HTTP2_PROTOCOL_ERROR;
    }

    base::UmaHistogramCounts100("Net.SpdySession.AlpsSettingParameterCount", alps_decoder.GetSettings().size());
    for (const auto& setting : alps_decoder.GetSettings()) {
        spdy::SpdySettingsId identifier = setting.first;
        uint32_t value = setting.second;
        net_log_.AddEvent(NetLogEventType::HTTP2_SESSION_RECV_SETTING, [&] { return NetLogSpdyRecvSettingParams(identifier, value); });
    ...
    for (const auto& entry : alps_decoder.GetAcceptCh()) {
        ...
        net_log_.AddEvent(NetLogEventType::HTTP2_SESSION_RECV_ACCEPT_CH, [&] { return NetLogSpdyRecvAcceptChParams(entry); });
    }
...
bool SpdySession::VerifyDomainAuthentication(std::string_view domain) const
{
    ...
    return CanPool(transport_security_state_, ssl_info, *ssl_config_service_, host_port_pair().host(), domain);
}

void SpdySession::EnqueueStreamWrite(const base::WeakPtr<SpdyStream>& stream, spdy::SpdyFrameType frame_type, std::unique_ptr<SpdyBufferProducer> producer)
{
    DCHECK(frame_type == spdy::SpdyFrameType::HEADERS || frame_type == spdy::SpdyFrameType::DATA);
    EnqueueWrite(stream->priority(), frame_type, std::move(producer), stream, stream->traffic_annotation());
}
...
void SpdySession::EnqueueGreasedFrame(const base::WeakPtr<SpdyStream>& stream)
{
    ...
    net_log_.AddEvent(NetLogEventType::HTTP2_SESSION_SEND_GREASED_FRAME, [&] {
        return NetLogSpdyGreasedFrameParams(stream->stream_id(), greased_http2_frame_.value().type, greased_http2_frame_.value().flags,
            greased_http2_frame_.value().payload.length(), stream->priority());
    });

    EnqueueWrite(stream->priority(), static_cast<spdy::SpdyFrameType>(greased_http2_frame_.value().type),
        std::make_unique<GreasedBufferProducer>(stream, &greased_http2_frame_.value(), buffered_spdy_framer_.get()), stream, stream->traffic_annotation());
}
...
int SpdySession::ConfirmHandshake(CompletionOnceCallback callback)
{
    ...
    if (!in_confirm_handshake_) {
        rv = socket_->ConfirmHandshake(base::BindOnce(&SpdySession::NotifyRequestsOfConfirmation, weak_factory_.GetWeakPtr()));
    }
...
std::unique_ptr<spdy::SpdySerializedFrame> SpdySession::CreateHeaders(
    spdy::SpdyStreamId stream_id, RequestPriority priority, spdy::SpdyControlFlags flags, quiche::HttpHeaderBlock block, NetLogSource source_dependency)
{
    ...
    spdy::SpdyPriority spdy_priority = ConvertRequestPriorityToSpdyPriority(priority);
    ...
    priority_dependency_state_.OnStreamCreation(stream_id, spdy_priority, &parent_stream_id, &weight, &exclusive);

    net_log_.AddEvent(NetLogEventType::HTTP2_SESSION_SEND_HEADERS, [&](NetLogCaptureMode capture_mode) {
        return NetLogSpdyHeadersSentParams(
            &block, (flags & spdy::CONTROL_FLAG_FIN) != 0, stream_id, has_priority, weight, parent_stream_id, exclusive, source_dependency, capture_mode);
    });
    ...
    return std::make_unique<spdy::SpdySerializedFrame>(buffered_spdy_framer_->SerializeFrame(headers));
}

std::unique_ptr<SpdyBuffer> SpdySession::CreateDataBuffer(
    spdy::SpdyStreamId stream_id, IOBuffer* data, int len, spdy::SpdyDataFlags flags, int* effective_len, bool* end_stream)
{
    if (availability_state_ == STATE_DRAINING) {
        return nullptr;
    }
    ...
    *effective_len = std::min(len, kMaxSpdyFrameChunkSize);
    ...
    UMA_HISTOGRAM_ENUMERATION("Net.SpdyFrameStreamAndSessionFlowControlState", frame_flow_control_state, SEND_STALLED_BY_STREAM_AND_SESSION + 1);
    ...
    if (send_stalled_by_stream) {
        ...
        net_log_.AddEventWithIntParams(NetLogEventType::HTTP2_SESSION_STREAM_STALLED_BY_STREAM_SEND_WINDOW, "stream_id", stream_id);
        return nullptr;
    }
    ...
    *effective_len = std::min(*effective_len, stream->send_window_size());
    ...
    if (send_stalled_by_session) {
        stream->set_send_stalled_by_flow_control(true);
        QueueSendStalledStream(*stream);
        net_log_.AddEventWithIntParams(NetLogEventType::HTTP2_SESSION_STREAM_STALLED_BY_SESSION_SEND_WINDOW, "stream_id", stream_id);
        return nullptr;
    }
    ...
    *effective_len = std::min(*effective_len, session_send_window_size_);
    ...
    std::unique_ptr<spdy::SpdySerializedFrame> frame(
        buffered_spdy_framer_->CreateDataFrame(stream_id, data->data(), static_cast<uint32_t>(*effective_len), flags));
    ...
    if (*effective_len != 0) {
        ...
        data_buffer->AddConsumeCallback(
            base::BindRepeating(&SpdySession::OnWriteBufferConsumed, weak_factory_.GetWeakPtr(), static_cast<size_t>(*effective_len)));
    }
...
void SpdySession::UpdateStreamPriority(SpdyStream* stream, RequestPriority old_priority, RequestPriority new_priority)
{
    ...
    // yet and stream_id == 0).
    write_queue_.ChangePriorityOfWritesForStream(stream, old_priority, new_priority);
    ...
    auto updates = priority_dependency_state_.OnStreamUpdate(stream_id, ConvertRequestPriorityToSpdyPriority(new_priority));
    for (auto u : updates) {
...
void SpdySession::CloseCreatedStream(const base::WeakPtr<SpdyStream>& stream, int status)
{
...
void SpdySession::ResetStream(spdy::SpdyStreamId stream_id, int error, const std::string& description)
{
    DCHECK_NE(stream_id, 0u);
...
std::string_view SpdySession::GetAcceptChViaAlps(const url::SchemeHostPort& scheme_host_port) const
{
...
void SpdySession::SendStreamWindowUpdate(spdy::SpdyStreamId stream_id, uint32_t delta_window_size)
{
    ActiveStreamMap::const_iterator it = active_streams_.find(stream_id);
    CHECK(it != active_streams_.end());
    CHECK_EQ(it->second->stream_id(), stream_id);
...
void SpdySession::CloseSessionOnError(Error err, const std::string& description)
{
...
void SpdySession::StartGoingAway(spdy::SpdyStreamId last_good_stream_id, Error status)
{
...
base::Value::Dict SpdySession::GetInfoAsValue() const
{
    ...
    auto dict = base::Value::Dict()
                    ...
                    .Set("network_anonymization_key", spdy_session_key_.network_anonymization_key().ToDebugString())
                    .Set("active_streams", static_cast<int>(active_streams_.size()))
                    .Set("negotiated_protocol", NextProtoToString(socket_->GetNegotiatedProtocol()))
                    .Set("error", error_on_close_)
                    .Set("max_concurrent_streams", static_cast<int>(max_concurrent_streams_))
...
bool SpdySession::IsReused() const
{
    ...
    return owned_stream_socket_ || stream_socket_handle_->reuse_type() == StreamSocketHandle::SocketReuseType::kUnusedIdle;
}

bool SpdySession::GetLoadTimingInfo(spdy::SpdyStreamId stream_id, LoadTimingInfo* load_timing_info) const
{
    if (stream_socket_handle_) {
        DCHECK(!connect_timing_);
        return stream_socket_handle_->GetLoadTimingInfo(stream_id != kFirstStreamId, load_timing_info);
    }
...
bool SpdySession::HasAcceptableTransportSecurity() const
{
    ...
    if (!IsTLSCipherSuiteAllowedByHTTP2(SSLConnectionStatusToCipherSuite(ssl_info.connection_status))) {
        return false;
    }
...
bool SpdySession::ChangeSocketTag(const SocketTag& new_tag)
{
    ...
    SpdySessionKey new_key(spdy_session_key_.host_port_pair(), spdy_session_key_.privacy_mode(), spdy_session_key_.proxy_chain(),
        spdy_session_key_.session_usage(), new_tag, spdy_session_key_.network_anonymization_key(), spdy_session_key_.secure_dns_policy(),
...
void SpdySession::EnableBrokenConnectionDetection(base::TimeDelta heartbeat_interval)
{
    ...
    heartbeat_interval_ = heartbeat_interval;
    heartbeat_timer_.Start(FROM_HERE, heartbeat_interval_, base::BindOnce(&SpdySession::MaybeCheckConnectionStatus, weak_factory_.GetWeakPtr()));
}
...
void SpdySession::InitializeInternal(SpdySessionPool* pool)
{
    ...
    buffered_spdy_framer_ = std::make_unique<BufferedSpdyFramer>(initial_settings_.find(spdy::SETTINGS_MAX_HEADER_LIST_SIZE)->second, net_log_, time_func_);
    buffered_spdy_framer_->set_visitor(this);
    buffered_spdy_framer_->set_debug_visitor(this);
    buffered_spdy_framer_->UpdateHeaderDecoderTableSize(max_header_table_size_);

    net_log_.AddEvent(NetLogEventType::HTTP2_SESSION_INITIALIZED, [&] { return NetLogSpdyInitializedParams(socket_->NetLog().source()); });
    ...
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
        FROM_HERE, base::BindOnce(&SpdySession::PumpReadLoop, weak_factory_.GetWeakPtr(), READ_STATE_DO_READ, OK));
}
...
int SpdySession::TryCreateStream(const base::WeakPtr<SpdyStreamRequest>& request, base::WeakPtr<SpdyStream>* stream)
{
    DCHECK(request);
    ...
    net_log_.AddEvent(NetLogEventType::HTTP2_SESSION_STALLED_MAX_STREAMS,
        [&] { return NetLogSpdySessionStalledParams(active_streams_.size(), created_streams_.size(), max_concurrent_streams_, request->url().spec()); });
...
int SpdySession::CreateStream(const SpdyStreamRequest& request, base::WeakPtr<SpdyStream>* stream)
{
    ...
    DCHECK(socket_);
    UMA_HISTOGRAM_BOOLEAN("Net.SpdySession.CreateStreamWithSocketConnected", socket_->IsConnected());
    if (!socket_->IsConnected()) {
        DoDrainSession(ERR_CONNECTION_CLOSED, "Tried to create SPDY stream for a closed socket connection.");
        return ERR_CONNECTION_CLOSED;
    }

    auto new_stream = std::make_unique<SpdyStream>(request.type(), GetWeakPtr(), request.url(), request.priority(), stream_initial_send_window_size_,
        stream_max_recv_window_size_, request.net_log(), request.traffic_annotation(), request.detect_broken_connection_);
...
bool SpdySession::CancelStreamRequest(const base::WeakPtr<SpdyStreamRequest>& request)
{
    DCHECK(request);
    RequestPriority priority = request->priority();
    CHECK_GE(priority, MINIMUM_PRIORITY);
    ...
    for (int i = MINIMUM_PRIORITY; i <= MAXIMUM_PRIORITY; ++i) {
        if (priority == i)
            continue;
        DCHECK(!base::Contains(pending_create_stream_queues_[i], request.get(), &base::WeakPtr<SpdyStreamRequest>::get));
    }
#endif
    ...
    // other elements.
    PendingStreamRequestQueue::iterator it = base::ranges::find(*queue, request.get(), &base::WeakPtr<SpdyStreamRequest>::get);
    ...
    if (it != queue->end()) {
        ...
        DCHECK(base::ranges::find(it, queue->end(), request.get(), &base::WeakPtr<SpdyStreamRequest>::get) == queue->end());
        return true;
    }
    return false;
}

void SpdySession::ChangeStreamRequestPriority(const base::WeakPtr<SpdyStreamRequest>& request, RequestPriority priority)
{
...
void SpdySession::ProcessPendingStreamRequests()
{
    ...
    for (size_t i = 0; i < max_requests_to_process; ++i) {
        ...
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
            FROM_HERE, base::BindOnce(&SpdySession::CompleteStreamRequest, weak_factory_.GetWeakPtr(), pending_request));
    }
}

void SpdySession::CloseActiveStreamIterator(ActiveStreamMap::iterator it, int status)
{
...
void SpdySession::CloseCreatedStreamIterator(CreatedStreamSet::iterator it, int status)
{
...
void SpdySession::ResetStreamIterator(ActiveStreamMap::iterator it, int error, const std::string& description)
{
...
void SpdySession::EnqueueResetStreamFrame(
    spdy::SpdyStreamId stream_id, RequestPriority priority, spdy::SpdyErrorCode error_code, const std::string& description)
{
    DCHECK_NE(stream_id, 0u);

    net_log_.AddEvent(NetLogEventType::HTTP2_SESSION_SEND_RST_STREAM, [&] { return NetLogSpdySendRstStreamParams(stream_id, error_code, description); });

    DCHECK(buffered_spdy_framer_.get());
    std::unique_ptr<spdy::SpdySerializedFrame> rst_frame(buffered_spdy_framer_->CreateRstStream(stream_id, error_code));

    EnqueueSessionWrite(priority, spdy::SpdyFrameType::RST_STREAM, std::move(rst_frame));
    RecordProtocolErrorHistogram(MapRstStreamStatusToProtocolError(error_code));
}

void SpdySession::EnqueuePriorityFrame(spdy::SpdyStreamId stream_id, spdy::SpdyStreamId dependency_id, int weight, bool exclusive)
{
    net_log_.AddEvent(NetLogEventType::HTTP2_STREAM_SEND_PRIORITY, [&] { return NetLogSpdyPriorityParams(stream_id, dependency_id, weight, exclusive); });

    DCHECK(buffered_spdy_framer_.get());
    std::unique_ptr<spdy::SpdySerializedFrame> frame(buffered_spdy_framer_->CreatePriority(stream_id, dependency_id, weight, exclusive));
    ...
    // priority.
    EnqueueWrite(HIGHEST, spdy::SpdyFrameType::PRIORITY, std::make_unique<SimpleBufferProducer>(std::make_unique<SpdyBuffer>(std::move(frame))),
...
int SpdySession::DoReadLoop(ReadState expected_read_state, int result)
{
    ...
    const base::TimeTicks yield_after_time = time_func_() + base::Milliseconds(kYieldAfterDurationMilliseconds);
...
int SpdySession::DoRead()
{
    ...
    int rv
        = socket_->ReadIfReady(read_buffer_.get(), kReadBufferSize, base::BindOnce(&SpdySession::PumpReadLoop, weak_factory_.GetWeakPtr(), READ_STATE_DO_READ));
    if (rv == ERR_IO_PENDING) {
    ...
    if (rv == ERR_READ_IF_READY_NOT_IMPLEMENTED) {
        ...
        return socket_->Read(
            read_buffer_.get(), kReadBufferSize, base::BindOnce(&SpdySession::PumpReadLoop, weak_factory_.GetWeakPtr(), READ_STATE_DO_READ_COMPLETE));
    }
    return rv;
}
...
int SpdySession::DoReadComplete(int result)
{
    ...
    if (result < 0) {
        DoDrainSession(static_cast<Error>(result), base::StringPrintf("Error %d reading from socket.", -result));
        return result;
    }
    ...
    while (result > 0) {
        ...
        DCHECK_EQ(buffered_spdy_framer_->spdy_framer_error(), http2::Http2DecoderAdapter::SPDY_NO_ERROR);
    }
...
void SpdySession::MaybePostWriteLoop()
{
    if (write_state_ == WRITE_STATE_IDLE) {
        ...
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
            FROM_HERE, base::BindOnce(&SpdySession::PumpWriteLoop, weak_factory_.GetWeakPtr(), WRITE_STATE_DO_WRITE, OK));
    }
}
...
int SpdySession::DoWrite()
{
    ...
    if (in_flight_write_) {
        ...
        base::WeakPtr<SpdyStream> stream;
        if (!write_queue_.Dequeue(&frame_type, &producer, &stream, &in_flight_write_traffic_annotation_)) {
    ...
    return socket_->Write(write_io_buffer.get(), in_flight_write_->GetRemainingSize(),
        base::BindOnce(&SpdySession::PumpWriteLoop, weak_factory_.GetWeakPtr(), WRITE_STATE_DO_WRITE_COMPLETE),
    ...
    if (result > 0) {
        ...
        if (in_flight_write_->GetRemainingSize() == 0) {
            ...
            if (in_flight_write_stream_.get()) {
                ...
                in_flight_write_stream_->OnFrameWriteComplete(in_flight_write_frame_type_, in_flight_write_frame_size_);
            }
...
void SpdySession::SendInitialData()
{
    ...
    if (enable_http2_settings_grease_) {
        spdy::SpdySettingsId greased_id = 0x0a0a + 0x1000 * base::RandGenerator(0xf + 1) + 0x0010 * base::RandGenerator(0xf + 1);
        uint32_t greased_value = base::RandGenerator(static_cast<uint64_t>(std::numeric_limits<uint32_t>::max()) + 1);
        // Let insertion silently fail if `settings_map` already contains
        // `greased_id`.
        settings_map.emplace(greased_id, greased_value);
    }
    net_log_.AddEvent(NetLogEventType::HTTP2_SESSION_SEND_SETTINGS, [&] { return NetLogSpdySendSettingsParams(&settings_map); });
    std::unique_ptr<spdy::SpdySerializedFrame> settings_frame(buffered_spdy_framer_->CreateSettings(settings_map));
    ...
    const bool send_window_update = session_max_recv_window_size_ > session_recv_window_size_;
    if (send_window_update) {
        const int32_t delta_window_size = session_max_recv_window_size_ - session_recv_window_size_;
        session_recv_window_size_ += delta_window_size;
        net_log_.AddEvent(NetLogEventType::HTTP2_SESSION_UPDATE_RECV_WINDOW,
            [&] { return NetLogSpdySessionWindowUpdateParams(delta_window_size, session_recv_window_size_); });

        last_recv_window_update_ = base::TimeTicks::Now();
        session_unacked_recv_window_bytes_ += delta_window_size;
        net_log_.AddEvent(NetLogEventType::HTTP2_SESSION_SEND_WINDOW_UPDATE,
            [&] { return NetLogSpdyWindowUpdateFrameParams(spdy::kSessionFlowControlStreamId, session_unacked_recv_window_bytes_); });
        window_update_frame = buffered_spdy_framer_->CreateWindowUpdate(spdy::kSessionFlowControlStreamId, session_unacked_recv_window_bytes_);
    ...
    // in a single packet.
    size_t initial_frame_size = spdy::kHttp2ConnectionHeaderPrefixSize + settings_frame->size();
    if (send_window_update)
        initial_frame_size += window_update_frame->size();
    auto initial_frame_data = std::make_unique<char[]>(initial_frame_size);
    size_t offset = 0;

    memcpy(initial_frame_data.get() + offset, spdy::kHttp2ConnectionHeaderPrefix, spdy::kHttp2ConnectionHeaderPrefixSize);
    offset += spdy::kHttp2ConnectionHeaderPrefixSize;

    memcpy(initial_frame_data.get() + offset, settings_frame->data(), settings_frame->size());
    offset += settings_frame->size();

    if (send_window_update) {
        memcpy(initial_frame_data.get() + offset, window_update_frame->data(), window_update_frame->size());
    }

    auto initial_frame = std::make_unique<spdy::SpdySerializedFrame>(std::move(initial_frame_data), initial_frame_size);
    EnqueueSessionWrite(HIGHEST, spdy::SpdyFrameType::SETTINGS, std::move(initial_frame));
}
...
void SpdySession::HandleSetting(uint32_t id, uint32_t value)
{
    ...
    case spdy::SETTINGS_HEADER_TABLE_SIZE:
        ...
        break;
    case spdy::SETTINGS_MAX_CONCURRENT_STREAMS:
        max_concurrent_streams_ = std::min(static_cast<size_t>(value), kMaxConcurrentStreamLimit);
        ProcessPendingStreamRequests();
        break;
    case spdy::SETTINGS_INITIAL_WINDOW_SIZE: {
        if (value > static_cast<uint32_t>(std::numeric_limits<int32_t>::max())) {
            net_log_.AddEventWithIntParams(NetLogEventType::HTTP2_SESSION_INITIAL_WINDOW_SIZE_OUT_OF_RANGE, "initial_window_size", value);
            return;
        }
        ...
        net_log_.AddEventWithIntParams(NetLogEventType::HTTP2_SESSION_UPDATE_STREAMS_SEND_WINDOW_SIZE, "delta_window_size", delta_window_size);
        break;
    }
    case spdy::SETTINGS_ENABLE_CONNECT_PROTOCOL:
        if ((value != 0 && value != 1) || (support_websocket_ && value == 0)) {
            DoDrainSession(ERR_HTTP2_PROTOCOL_ERROR, "Invalid value for spdy::SETTINGS_ENABLE_CONNECT_PROTOCOL.");
            return;
        }
        if (value == 1) {
    ...
    case spdy::SETTINGS_DEPRECATE_HTTP2_PRIORITIES:
        if (value != 0 && value != 1) {
            DoDrainSession(ERR_HTTP2_PROTOCOL_ERROR, "Invalid value for spdy::SETTINGS_DEPRECATE_HTTP2_PRIORITIES.");
            return;
        }
...
void SpdySession::UpdateStreamsSendWindowSize(int32_t delta_window_size)
{
    for (const auto& value : active_streams_) {
        if (!value.second->AdjustSendWindowSize(delta_window_size)) {
            DoDrainSession(ERR_HTTP2_FLOW_CONTROL_ERROR,
                base::StringPrintf("New spdy::SETTINGS_INITIAL_WINDOW_SIZE value overflows "
...
{
    if (ping_in_flight_ || check_ping_status_pending_ || !enable_ping_based_connection_checking_) {
        return;
    }
...
void SpdySession::SendWindowUpdateFrame(spdy::SpdyStreamId stream_id, uint32_t delta_window_size, RequestPriority priority)
{
    ActiveStreamMap::const_iterator it = active_streams_.find(stream_id);
    if (it != active_streams_.end()) {
        CHECK_EQ(it->second->stream_id(), stream_id);
    ...
    net_log_.AddEvent(NetLogEventType::HTTP2_SESSION_SEND_WINDOW_UPDATE, [&] { return NetLogSpdyWindowUpdateFrameParams(stream_id, delta_window_size); });

    DCHECK(buffered_spdy_framer_.get());
    std::unique_ptr<spdy::SpdySerializedFrame> window_update_frame(buffered_spdy_framer_->CreateWindowUpdate(stream_id, delta_window_size));
    EnqueueSessionWrite(priority, spdy::SpdyFrameType::WINDOW_UPDATE, std::move(window_update_frame));
}

void SpdySession::WritePingFrame(spdy::SpdyPingId unique_id, bool is_ack)
{
    DCHECK(buffered_spdy_framer_.get());
    std::unique_ptr<spdy::SpdySerializedFrame> ping_frame(buffered_spdy_framer_->CreatePingFrame(unique_id, is_ack));
    EnqueueSessionWrite(HIGHEST, spdy::SpdyFrameType::PING, std::move(ping_frame));

    net_log_.AddEvent(NetLogEventType::HTTP2_SESSION_PING, [&] { return NetLogSpdyPingParams(unique_id, is_ack, "sent"); });
...
void SpdySession::PlanToCheckPingStatus()
{
    ...
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(
        FROM_HERE, base::BindOnce(&SpdySession::CheckPingStatus, weak_factory_.GetWeakPtr(), time_func_()), hung_interval_);
}
...
void SpdySession::CheckPingStatus(base::TimeTicks last_check_time)
{
    ...
    if (now > last_read_time_ + hung_interval_ || last_read_time_ < last_check_time) {
        check_ping_status_pending_ = false;
    ...
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(
        FROM_HERE, base::BindOnce(&SpdySession::CheckPingStatus, weak_factory_.GetWeakPtr(), now), delay);
}
...
void SpdySession::EnqueueSessionWrite(RequestPriority priority, spdy::SpdyFrameType frame_type, std::unique_ptr<spdy::SpdySerializedFrame> frame)
{
    DCHECK(frame_type == spdy::SpdyFrameType::RST_STREAM || frame_type == spdy::SpdyFrameType::SETTINGS || frame_type == spdy::SpdyFrameType::WINDOW_UPDATE
        || frame_type == spdy::SpdyFrameType::PING || frame_type == spdy::SpdyFrameType::GOAWAY);
    DCHECK(IsSpdyFrameTypeWriteCapped(frame_type));
    if (write_queue_.num_queued_capped_frames() > session_max_queued_capped_frames_) {
        LOG(WARNING) << "Draining session due to exceeding max queued capped frames";
    ...
    EnqueueWrite(
        priority, frame_type, std::make_unique<SimpleBufferProducer>(std::move(buffer)), base::WeakPtr<SpdyStream>(), kSpdySessionCommandsTrafficAnnotation);
    if (greased_http2_frame_ && frame_type == spdy::SpdyFrameType::SETTINGS) {
        net_log_.AddEvent(NetLogEventType::HTTP2_SESSION_SEND_GREASED_FRAME, [&] {
            return NetLogSpdyGreasedFrameParams(
                /* stream_id = */ 0, greased_http2_frame_.value().type, greased_http2_frame_.value().flags, greased_http2_frame_.value().payload.length(),
                priority);
        });

        EnqueueWrite(priority, static_cast<spdy::SpdyFrameType>(greased_http2_frame_.value().type),
            std::make_unique<GreasedBufferProducer>(base::WeakPtr<SpdyStream>(), &greased_http2_frame_.value(), buffered_spdy_framer_.get()),
            base::WeakPtr<SpdyStream>(), kSpdySessionCommandsTrafficAnnotation);
    }
}

void SpdySession::EnqueueWrite(RequestPriority priority, spdy::SpdyFrameType frame_type, std::unique_ptr<SpdyBufferProducer> producer,
    const base::WeakPtr<SpdyStream>& stream, const NetworkTrafficAnnotationTag& traffic_annotation)
{
    if (availability_state_ == STATE_DRAINING)
        return;

    write_queue_.Enqueue(priority, frame_type, std::move(producer), stream, traffic_annotation);
    MaybePostWriteLoop();
}
...
std::unique_ptr<SpdyStream> SpdySession::ActivateCreatedStream(SpdyStream* stream)
{
    CHECK_EQ(stream->stream_id(), 0u);
...
void SpdySession::InsertActivatedStream(std::unique_ptr<SpdyStream> stream)
{
    ...
    std::pair<ActiveStreamMap::iterator, bool> result = active_streams_.emplace(stream_id, stream.get());
    CHECK(result.second);
...
{
    UMA_HISTOGRAM_CUSTOM_COUNTS("Net.SpdyStreamsPerSession", streams_initiated_count_, 1, 300, 50);
    UMA_HISTOGRAM_CUSTOM_COUNTS("Net.SpdyStreamsAbandonedPerSession", streams_abandoned_count_, 1, 300, 50);
    UMA_HISTOGRAM_BOOLEAN("Net.SpdySession.ServerSupportsWebSocket", support_websocket_);
    if (IsGoogleHostWithAlpnH3(spdy_session_key_.host_port_pair().host())) {
        LogSessionCreationInitiatorToHistogram(session_creation_initiator_, streams_initiated_count_ > 0);
    }
}

void SpdySession::RecordProtocolErrorHistogram(SpdyProtocolErrorDetails details)
{
    UMA_HISTOGRAM_ENUMERATION("Net.SpdySessionErrorDetails2", details, NUM_SPDY_PROTOCOL_ERROR_DETAILS);
    if (base::EndsWith(host_port_pair().host(), "google.com", base::CompareCase::INSENSITIVE_ASCII)) {
...
void SpdySession::DoDrainSession(Error err, const std::string& description)
{
    ...
    if (err == ERR_HTTP_1_1_REQUIRED) {
        http_server_properties_->SetHTTP11Required(
            url::SchemeHostPort(url::kHttpsScheme, host_port_pair().host(), host_port_pair().port()), spdy_session_key_.network_anonymization_key());
    }
    ...
    if (err != OK && err != ERR_ABORTED && // Used by SpdySessionPool to close idle sessions.
        ...
        spdy::SpdyGoAwayIR goaway_ir(/* last_good_stream_id = */ 0, MapNetErrorToGoAwayStatus(err), description);
        auto frame = std::make_unique<spdy::SpdySerializedFrame>(buffered_spdy_framer_->SerializeFrame(goaway_ir));
    ...
    net_log_.AddEvent(NetLogEventType::HTTP2_SESSION_CLOSE, [&] { return NetLogSpdySessionCloseParams(err, description); });
...
void SpdySession::LogAbandonedActiveStream(ActiveStreamMap::const_iterator it, Error status)
{
...
void SpdySession::CompleteStreamRequest(const base::WeakPtr<SpdyStreamRequest>& pending_request)
{
...
void SpdySession::OnError(http2::Http2DecoderAdapter::SpdyFramerError spdy_framer_error)
{
    CHECK(in_io_loop_);

    RecordProtocolErrorHistogram(MapFramerErrorToProtocolError(spdy_framer_error));
    std::string description
        = base::StringPrintf("Framer error: %d (%s).", spdy_framer_error, http2::Http2DecoderAdapter::SpdyFramerErrorToString(spdy_framer_error));
    DoDrainSession(MapFramerErrorToNetError(spdy_framer_error), description);
}

void SpdySession::OnStreamError(spdy::SpdyStreamId stream_id, const std::string& description)
{
    CHECK(in_io_loop_);
    ...
    if (it == active_streams_.end()) {
        ...
        EnqueueResetStreamFrame(stream_id, IDLE, spdy::ERROR_CODE_PROTOCOL_ERROR, description);
        return;
    }
...
void SpdySession::OnPing(spdy::SpdyPingId unique_id, bool is_ack)
{
    ...
    net_log_.AddEvent(NetLogEventType::HTTP2_SESSION_PING, [&] { return NetLogSpdyPingParams(unique_id, is_ack, "received"); });
    ...
    if (network_quality_estimator_) {
        network_quality_estimator_->RecordSpdyPingLatency(host_port_pair(), ping_duration);
    }
}

void SpdySession::OnRstStream(spdy::SpdyStreamId stream_id, spdy::SpdyErrorCode error_code)
{
    CHECK(in_io_loop_);
    ...
    net_log_.AddEvent(NetLogEventType::HTTP2_SESSION_RECV_RST_STREAM, [&] { return NetLogSpdyRecvRstStreamParams(stream_id, error_code); });
    ...
    if (error_code == spdy::ERROR_CODE_NO_ERROR) {
        ...
        RecordProtocolErrorHistogram(PROTOCOL_ERROR_RST_STREAM_FOR_NON_ACTIVE_STREAM);
        it->second->LogStreamError(ERR_HTTP2_PROTOCOL_ERROR, "Server reset stream.");
...
void SpdySession::OnGoAway(spdy::SpdyStreamId last_accepted_stream_id, spdy::SpdyErrorCode error_code, std::string_view debug_data)
{
    CHECK(in_io_loop_);
    ...
    net_log_.AddEvent(NetLogEventType::HTTP2_SESSION_RECV_GOAWAY, [&](NetLogCaptureMode capture_mode) {
        return NetLogSpdyRecvGoAwayParams(last_accepted_stream_id, active_streams_.size(), error_code, debug_data, capture_mode);
    });
    MakeUnavailable();
...
void SpdySession::OnDataFrameHeader(spdy::SpdyStreamId stream_id, size_t length, bool fin)
{
    CHECK(in_io_loop_);

    net_log_.AddEvent(NetLogEventType::HTTP2_SESSION_RECV_DATA, [&] { return NetLogSpdyDataParams(stream_id, length, fin); });
...
void SpdySession::OnStreamFrameData(spdy::SpdyStreamId stream_id, const char* data, size_t len)
{
    CHECK(in_io_loop_);
    DCHECK_LT(len, 1u << 24);
    ...
    if (data) {
        ...
        buffer->AddConsumeCallback(base::BindRepeating(&SpdySession::OnReadBufferConsumed, weak_factory_.GetWeakPtr()));
    } else {
        DCHECK_EQ(len, 0u);
    }
...
void SpdySession::OnSettings()
{
    ...
    if (!settings_frame_received_) {
        base::UmaHistogramCounts1000("Net.SpdySession.OnSettings.CreatedStreamCount2", created_streams_.size());
        base::UmaHistogramCounts1000("Net.SpdySession.OnSettings.ActiveStreamCount2", active_streams_.size());
        base::UmaHistogramCounts1000("Net.SpdySession.OnSettings.CreatedAndActiveStreamCount2", created_streams_.size() + active_streams_.size());
        base::UmaHistogramCounts1000("Net.SpdySession.OnSettings.PendingStreamCount2", GetTotalSize(pending_create_stream_queues_));
    }

    // Send an acknowledgment of the setting.
    spdy::SpdySettingsIR settings_ir;
    settings_ir.set_is_ack(true);
    auto frame = std::make_unique<spdy::SpdySerializedFrame>(buffered_spdy_framer_->SerializeFrame(settings_ir));
...
void SpdySession::OnSetting(spdy::SpdySettingsId id, uint32_t value)
{
    ...
    // Log the setting.
    net_log_.AddEvent(NetLogEventType::HTTP2_SESSION_RECV_SETTING, [&] { return NetLogSpdyRecvSettingParams(id, value); });
}
...
void SpdySession::OnWindowUpdate(spdy::SpdyStreamId stream_id, int delta_window_size)
{
    CHECK(in_io_loop_);

    net_log_.AddEvent(NetLogEventType::HTTP2_SESSION_RECV_WINDOW_UPDATE, [&] { return NetLogSpdyWindowUpdateFrameParams(stream_id, delta_window_size); });
    ...
    if (stream_id == spdy::kSessionFlowControlStreamId) {
        ...
        if (delta_window_size < 1) {
            ...
            DoDrainSession(ERR_HTTP2_PROTOCOL_ERROR, "Received WINDOW_UPDATE with an invalid delta_window_size " + base::NumberToString(delta_window_size));
            return;
        }
        ...
        if (delta_window_size < 1) {
            ResetStreamIterator(it, ERR_HTTP2_FLOW_CONTROL_ERROR, "Received WINDOW_UPDATE with an invalid delta_window_size.");
            return;
        }
...
void SpdySession::OnPushPromise(spdy::SpdyStreamId /*stream_id*/, spdy::SpdyStreamId /*promised_stream_id*/, quiche::HttpHeaderBlock /*headers*/)
{
    CHECK(in_io_loop_);
    DoDrainSession(ERR_HTTP2_PROTOCOL_ERROR, "PUSH_PROMISE received");
}

void SpdySession::OnHeaders(spdy::SpdyStreamId stream_id, bool has_priority, int weight, spdy::SpdyStreamId parent_stream_id, bool exclusive, bool fin,
    quiche::HttpHeaderBlock headers, base::TimeTicks recv_first_byte_time)
{
    CHECK(in_io_loop_);

    net_log_.AddEvent(NetLogEventType::HTTP2_SESSION_RECV_HEADERS,
        [&](NetLogCaptureMode capture_mode) { return NetLogSpdyHeadersReceivedParams(&headers, fin, stream_id, capture_mode); });
...
void SpdySession::OnAltSvc(spdy::SpdyStreamId stream_id, std::string_view origin, const spdy::SpdyAltSvcWireFormat::AlternativeServiceVector& altsvc_vector)
{
    ...
    if (stream_id == 0) {
        ...
        if (!CanPool(transport_security_state_, ssl_info, *ssl_config_service_, host_port_pair().host(), gurl.host_piece())) {
            return;
        }
    ...
    http_server_properties_->SetAlternativeServices(scheme_host_port, spdy_session_key_.network_anonymization_key(),
        ProcessAlternativeServices(altsvc_vector, is_http2_enabled_, is_quic_enabled_, quic_supported_versions_));
}

bool SpdySession::OnUnknownFrame(spdy::SpdyStreamId stream_id, uint8_t frame_type)
{
    if (stream_id % 2 == 1) {
...
void SpdySession::OnSendCompressedFrame(spdy::SpdyStreamId stream_id, spdy::SpdyFrameType type, size_t payload_len, size_t frame_len)
{
    ...
    if (payload_len) {
        ...
        UMA_HISTOGRAM_PERCENTAGE("Net.SpdyHeadersCompressionPercentage", compression_pct);
    }
}

void SpdySession::OnReceiveCompressedFrame(spdy::SpdyStreamId stream_id, spdy::SpdyFrameType type, size_t frame_len)
{
    last_compressed_frame_len_ = frame_len;
}

void SpdySession::OnWriteBufferConsumed(size_t frame_payload_size, size_t consume_size, SpdyBuffer::ConsumeSource consume_source)
{
...
void SpdySession::IncreaseSendWindowSize(int delta_window_size)
{
    ...
    if (delta_window_size > max_delta_window_size) {
        RecordProtocolErrorHistogram(PROTOCOL_ERROR_INVALID_WINDOW_UPDATE_SIZE);
        DoDrainSession(ERR_HTTP2_PROTOCOL_ERROR,
            "Received WINDOW_UPDATE [delta: " + base::NumberToString(delta_window_size)
                + "] for session overflows session_send_window_size_ [current: " + base::NumberToString(session_send_window_size_) + "]");
        return;
    }
    ...
    net_log_.AddEvent(
        NetLogEventType::HTTP2_SESSION_UPDATE_SEND_WINDOW, [&] { return NetLogSpdySessionWindowUpdateParams(delta_window_size, session_send_window_size_); });
...
void SpdySession::OnReadBufferConsumed(size_t consume_size, SpdyBuffer::ConsumeSource consume_source)
{
    // We can be called with |in_io_loop_| set if a read SpdyBuffer is
    // deleted (e.g., discarded by a SpdyReadQueue).
    DCHECK_GE(consume_size, 1u);
    DCHECK_LE(consume_size, static_cast<size_t>(std::numeric_limits<int32_t>::max()));
...
void SpdySession::IncreaseRecvWindowSize(int32_t delta_window_size)
{
    ...
    DCHECK_GE(delta_window_size, 1);
    // Check for overflow.
    DCHECK_LE(delta_window_size, std::numeric_limits<int32_t>::max() - session_recv_window_size_);

    session_recv_window_size_ += delta_window_size;
    net_log_.AddEvent(
        NetLogEventType::HTTP2_SESSION_UPDATE_RECV_WINDOW, [&] { return NetLogSpdySessionWindowUpdateParams(delta_window_size, session_recv_window_size_); });
    ...
    if (session_unacked_recv_window_bytes_ > session_max_recv_window_size_ / 2 || elapsed >= time_to_buffer_small_window_updates_) {
        last_recv_window_update_ = base::TimeTicks::Now();
        SendWindowUpdateFrame(spdy::kSessionFlowControlStreamId, session_unacked_recv_window_bytes_, HIGHEST);
        session_unacked_recv_window_bytes_ = 0;
    }
}
...
void SpdySession::DecreaseRecvWindowSize(int32_t delta_window_size)
{
    ...
    // respected.
    int32_t receiving_window_size = session_recv_window_size_ - session_unacked_recv_window_bytes_;
    if (delta_window_size > receiving_window_size) {
        RecordProtocolErrorHistogram(PROTOCOL_ERROR_RECEIVE_WINDOW_VIOLATION);
        DoDrainSession(ERR_HTTP2_FLOW_CONTROL_ERROR,
            "delta_window_size is " + base::NumberToString(delta_window_size) + " in DecreaseRecvWindowSize, which is larger than the receive "
...
spdy::SpdyStreamId SpdySession::PopStreamToPossiblyResume()
{
    for (int i = MAXIMUM_PRIORITY; i >= MINIMUM_PRIORITY; --i) {
        base::circular_deque<spdy::SpdyStreamId>* queue = &stream_send_unstall_queue_[i];
        if (!queue->empty()) {
