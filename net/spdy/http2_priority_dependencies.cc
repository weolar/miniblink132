...
void Http2PriorityDependencies::OnStreamCreation(
    spdy::SpdyStreamId id, spdy::SpdyPriority priority, spdy::SpdyStreamId* parent_stream_id, int* weight, bool* exclusive)
{
...
bool Http2PriorityDependencies::PriorityLowerBound(spdy::SpdyPriority priority, IdList::iterator* bound)
{
...
bool Http2PriorityDependencies::ParentOfStream(spdy::SpdyStreamId id, IdList::iterator* parent)
{
...
bool Http2PriorityDependencies::ChildOfStream(spdy::SpdyStreamId id, IdList::iterator* child)
{
    auto entry = entry_by_stream_id_.find(id);
    CHECK(entry != entry_by_stream_id_.end(), base::NotFatalUntil::M130);
...
std::vector<Http2PriorityDependencies::DependencyUpdate> Http2PriorityDependencies::OnStreamUpdate(spdy::SpdyStreamId id, spdy::SpdyPriority new_priority)
{
    ...
    if ((old_has_parent != new_has_parent) || (old_has_parent && old_parent->first != new_parent->first)) {
