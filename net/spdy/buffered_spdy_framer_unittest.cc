...
public:
    ...
    {
        VLOG(1) << "spdy::SpdyFramer Error on stream: " << stream_id << " " << description;
        error_count_++;
    }
    ...
    void SimulateInFramer(const spdy::SpdySerializedFrame& frame)
    {
        ...
        while (input_remaining > 0 && buffered_spdy_framer_.spdy_framer_error() == http2::Http2DecoderAdapter::SPDY_NO_ERROR) {
...
TEST_F(BufferedSpdyFramerTest, OnSetting)
{
    ...
    spdy::SpdySerializedFrame control_frame(framer.SerializeSettings(settings_ir));
...
TEST_F(BufferedSpdyFramerTest, ValidHeadersAfterInvalidHeaders)
{
    ...
    spdy::SpdySerializedFrame headers_frame(spdy_test_util.ConstructSpdyReply(1, std::move(headers)));
    spdy::SpdySerializedFrame headers_frame2(spdy_test_util.ConstructSpdyReply(2, std::move(headers2)));
...
TEST_F(BufferedSpdyFramerTest, ReadPushPromiseHeaderBlock)
{
    ...
    spdy::SpdySerializedFrame control_frame = framer.SerializeFrame(push_promise_ir);
...
{
    spdy::SpdyGoAwayIR go_ir(/*last_good_stream_id=*/2, spdy::ERROR_CODE_FRAME_SIZE_ERROR, "foo");
    NetLogWithSource net_log;
    BufferedSpdyFramer framer(kMaxHeaderListSizeForTest, net_log);
