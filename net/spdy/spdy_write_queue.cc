...
{
    return frame_type == spdy::SpdyFrameType::RST_STREAM || frame_type == spdy::SpdyFrameType::SETTINGS || frame_type == spdy::SpdyFrameType::WINDOW_UPDATE
        || frame_type == spdy::SpdyFrameType::PING || frame_type == spdy::SpdyFrameType::GOAWAY;
}

SpdyWriteQueue::PendingWrite::PendingWrite() = default;

SpdyWriteQueue::PendingWrite::PendingWrite(spdy::SpdyFrameType frame_type, std::unique_ptr<SpdyBufferProducer> frame_producer,
    const base::WeakPtr<SpdyStream>& stream, const MutableNetworkTrafficAnnotationTag& traffic_annotation)
...
SpdyWriteQueue::PendingWrite& SpdyWriteQueue::PendingWrite::operator=(PendingWrite&& other) = default;
...
void SpdyWriteQueue::Enqueue(RequestPriority priority, spdy::SpdyFrameType frame_type, std::unique_ptr<SpdyBufferProducer> frame_producer,
    const base::WeakPtr<SpdyStream>& stream, const NetworkTrafficAnnotationTag& traffic_annotation)
{
    ...
    queue_[priority].push_back({ frame_type, std::move(frame_producer), stream, MutableNetworkTrafficAnnotationTag(traffic_annotation) });
...
bool SpdyWriteQueue::Dequeue(spdy::SpdyFrameType* frame_type, std::unique_ptr<SpdyBufferProducer>* frame_producer, base::WeakPtr<SpdyStream>* stream,
...
void SpdyWriteQueue::RemovePendingWritesForStreamsAfter(spdy::SpdyStreamId last_good_stream_id)
{
    CHECK(!removing_writes_);
    removing_writes_ = true;
...
void SpdyWriteQueue::ChangePriorityOfWritesForStream(SpdyStream* stream, RequestPriority old_priority, RequestPriority new_priority)
{
    CHECK(!removing_writes_);
    DCHECK(stream);
...
void SpdyWriteQueue::Clear()
{
    ...
    for (int i = MINIMUM_PRIORITY; i <= MAXIMUM_PRIORITY; ++i) {
        for (auto& pending_write : queue_[i]) {
            erased_buffer_producers.push_back(std::move(pending_write.frame_producer));
        }
        queue_[i].clear();
    }
    removing_writes_ = false;
