...
template <size_t N> Input FromStringLiteral(const char (&data)[N])
{
...
TEST(ParseValuesTest, ParseTimes)
{
    ...
    EXPECT_FALSE(ParseGeneralizedTime(FromStringLiteral("20140218161261Z"), &out));

    // Minutes only go up to 59.
    EXPECT_FALSE(ParseGeneralizedTime(FromStringLiteral("20140218166000Z"), &out));

    // Hours only go up to 23.
    EXPECT_FALSE(ParseGeneralizedTime(FromStringLiteral("20140218240000Z"), &out));
    // The 0th day of a month is invalid.
    EXPECT_FALSE(ParseGeneralizedTime(FromStringLiteral("20140200161200Z"), &out));
    // The 0th month is invalid.
    EXPECT_FALSE(ParseGeneralizedTime(FromStringLiteral("20140018161200Z"), &out));
    // Months greater than 12 are invalid.
    EXPECT_FALSE(ParseGeneralizedTime(FromStringLiteral("20141318161200Z"), &out));

    // Some months have 31 days.
    EXPECT_TRUE(ParseGeneralizedTime(FromStringLiteral("20140131000000Z"), &out));

    // September has only 30 days.
    EXPECT_FALSE(ParseGeneralizedTime(FromStringLiteral("20140931000000Z"), &out));

    // February has only 28 days...
    EXPECT_FALSE(ParseGeneralizedTime(FromStringLiteral("20140229000000Z"), &out));
    ...
    EXPECT_FALSE(ParseGeneralizedTime(FromStringLiteral("21000229000000Z"), &out));
    ...
    EXPECT_FALSE(ParseGeneralizedTime(FromStringLiteral("20001231010203Z\0"), &out));
    ...
    EXPECT_FALSE(ParseGeneralizedTime(FromStringLiteral("0x201231000000Z"), &out));

    // The last byte must be 'Z'.
    EXPECT_FALSE(ParseGeneralizedTime(FromStringLiteral("20001231000000X"), &out));

    // Check that the length is validated.
    EXPECT_FALSE(ParseGeneralizedTime(FromStringLiteral("20140218161200"), &out));
    EXPECT_FALSE(ParseGeneralizedTime(FromStringLiteral("20140218161200Z0"), &out));
}
...
TEST(ParseValuesTest, TimesCompare)
{
    ...
    ASSERT_TRUE(ParseGeneralizedTime(FromStringLiteral("20140218161200Z"), &time1));
    // Test that ParseUTCTime correctly normalizes the year.
    ASSERT_TRUE(ParseUTCTime(FromStringLiteral("150218161200Z"), &time2));
    ASSERT_TRUE(ParseGeneralizedTime(FromStringLiteral("20160218161200Z"), &time3));
...
TEST(ParseValuesTest, UTCTimeRange)
{
    ...
    ASSERT_TRUE(ParseGeneralizedTime(FromStringLiteral("20140218161200Z"), &time));
...
const Uint64TestData kUint64TestData[] = {
    ...
    { true, { 0x00, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF }, 9, UINT64_MAX },
...
TEST(ParseValuesTest, ParseUint64)
{
    for (size_t i = 0; i < std::size(kUint64TestData); i++) {
        ...
        uint64_t result;
        EXPECT_EQ(test_case.should_pass, ParseUint64(Input(test_case.input, test_case.length), &result));
...
TEST(ParseValuesTest, ParseUint8)
{
    for (size_t i = 0; i < std::size(kUint8TestData); i++) {
        ...
        uint8_t result;
        EXPECT_EQ(test_case.should_pass, ParseUint8(Input(test_case.input, test_case.length), &result));
        if (test_case.should_pass)
            EXPECT_EQ(test_case.expected_value, result);
    }
}
...
TEST(ParseValuesTest, IsValidInteger)
{
    for (size_t i = 0; i < std::size(kIsValidIntegerTestData); i++) {
        ...
        bool negative;
        EXPECT_EQ(test_case.should_pass, IsValidInteger(Input(test_case.input, test_case.length), &negative));
        if (test_case.should_pass)
...
{
    const Input valid_der({ 0x00, 0x66, 0x00, 0x6f, 0x00, 0x6f, 0x00, 0x62, 0x00, 0x61, 0x00, 0x72 });
    std::string s;
...
TEST(ParseValuesTest, ParseUniversalString)
{
    ...
    const Input valid_non_ascii_der({ 0x0, 0x1, 0xf4, 0xe, 0x0, 0x0, 0x0, 0x20, 0x0, 0x1, 0xd1, 0x1e, 0x0, 0x0, 0x26, 0x69, 0x0, 0x0, 0x26, 0x6b });
