...
public:
    ...
    const Input& bytes() const
    {
        return bytes_;
    }
    uint8_t unused_bits() const
    {
        return unused_bits_;
    }
...
[[nodiscard]] NET_EXPORT absl::optional<BitString> ParseBitString(const Input& in);
...
NET_EXPORT_PRIVATE bool operator<(const GeneralizedTime& lhs, const GeneralizedTime& rhs);
NET_EXPORT_PRIVATE bool operator<=(const GeneralizedTime& lhs, const GeneralizedTime& rhs);
NET_EXPORT_PRIVATE bool operator>(const GeneralizedTime& lhs, const GeneralizedTime& rhs);
NET_EXPORT_PRIVATE bool operator>=(const GeneralizedTime& lhs, const GeneralizedTime& rhs);
...
[[nodiscard]] NET_EXPORT bool ParseUTCTime(const Input& in, GeneralizedTime* out);
...
// fractional seconds.
[[nodiscard]] NET_EXPORT bool ParseGeneralizedTime(const Input& in, GeneralizedTime* out);
...
[[nodiscard]] NET_EXPORT bool ParseTeletexStringAsLatin1(Input in, std::string* out);
