...
template <size_t N> base::StringPiece ToStringPiece(const uint8_t (&data)[N])
{
...
TEST(EncodeValuesTest, EncodeTimeFromBeforeWindowsEpoch)
{
    ...
    constexpr base::Time kStartOfYear1570 = base::Time::UnixEpoch() - base::Seconds(12622780800);
    GeneralizedTime generalized_time;
...
TEST(EncodeValuesTest, GeneralizedTimeToTimeBeforeWindowsEpoch)
{
    ...
    bool platform_can_represent_time = base::Time::FromUTCExploded(exploded, &expected_time);
