...
public:
    ...
    template <size_t N>
    constexpr explicit Input(const uint8_t (&data)[N])
        : data_(data)
        , len_(N)
    {
    }
    ...
    // is not an option.
    constexpr const uint8_t* UnsafeData() const
    {
        return data_;
    }
...
NET_EXPORT_PRIVATE constexpr bool operator<(const Input& lhs, const Input& rhs)
{
