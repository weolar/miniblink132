...
bool AppendArgumentFromJSONValue(const std::string& key, const base::Value& value_node, base::CommandLine* command_line)
{
    ...
    case base::Value::Type::INTEGER: {
        command_line->AppendArg(argument_name + "=" + base::NumberToString(value_node.GetInt()));
        break;
    }
...
LocalTestServer::LocalTestServer(Type type, const SSLOptions& ssl_options, const base::FilePath& document_root)
...
bool LocalTestServer::GetTestServerPath(base::FilePath* testserver_path) const
{
    ...
    testserver_dir = testserver_dir.Append(FILE_PATH_LITERAL("net")).Append(FILE_PATH_LITERAL("tools")).Append(FILE_PATH_LITERAL("testserver"));
...
bool LocalTestServer::Init(const base::FilePath& document_root)
{
    ...
    SetResourcePath(src_dir.Append(document_root), src_dir.AppendASCII("net").AppendASCII("data").AppendASCII("ssl").AppendASCII("certificates"));
    return true;
}

std::optional<std::vector<base::FilePath>> LocalTestServer::GetPythonPath() const
{
...
bool LocalTestServer::AddCommandLineArguments(base::CommandLine* command_line) const
{
