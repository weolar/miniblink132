...
{
#if BUILDFLAG(IS_FUCHSIA)
    std::string spawner_url_base(base::CommandLine::ForCurrentProcess()->GetSwitchValueASCII("remote-test-server-spawner-url-base"));
    LOG_IF(FATAL, spawner_url_base.empty()) << "--remote-test-server-spawner-url-base missing from command line";
...
RemoteTestServer::RemoteTestServer(Type type, const base::FilePath& document_root)
    : BaseTestServer(type)
...
RemoteTestServer::RemoteTestServer(Type type, const SSLOptions& ssl_options, const base::FilePath& document_root)
...
bool RemoteTestServer::StartInBackground()
{
    ...
    start_request_ = std::make_unique<RemoteTestServerSpawnerRequest>(io_thread_.task_runner(), GetSpawnerUrl("start"), arguments_string);
...
bool RemoteTestServer::BlockUntilStarted()
{
    ...
    if (server_data_json.empty() || !SetAndParseServerData(server_data_json, &remote_port_)) {
...
bool RemoteTestServer::Stop()
{
    ...
    if (remote_port_) {
        std::unique_ptr<RemoteTestServerSpawnerRequest> kill_request = std::make_unique<RemoteTestServerSpawnerRequest>(
            io_thread_.task_runner(), GetSpawnerUrl(base::StringPrintf("kill?port=%d", remote_port_)), std::string());
...
bool RemoteTestServer::Init(const base::FilePath& document_root)
{
    ...
    bool thread_started = io_thread_.StartWithOptions(base::Thread::Options(base::MessagePumpType::IO, 0));
