...
public:
    ...
    LocalTestServer(Type type, const SSLOptions& ssl_options, const base::FilePath& document_root);
    ...
    [[nodiscard]] virtual bool GetTestServerPath(base::FilePath* testserver_path) const;

    // Adds the command line arguments for the Python test server to
    // |command_line|. Returns true on success.
    [[nodiscard]] virtual bool AddCommandLineArguments(base::CommandLine* command_line) const;
...
private:
    ...
    [[nodiscard]] bool LaunchPython(const base::FilePath& testserver_path, const std::vector<base::FilePath>& python_path);
