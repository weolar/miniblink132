...
bssl::UniquePtr<EVP_PKEY> LoadEVP_PKEYFromPEM(const base::FilePath& filepath)
{
    ...
    bssl::UniquePtr<BIO> bio(BIO_new_mem_buf(const_cast<char*>(data.data()), static_cast<int>(data.size())));
    if (!bio) {
        LOG(ERROR) << "Could not allocate BIO for buffer?";
        return nullptr;
    }
    bssl::UniquePtr<EVP_PKEY> result(PEM_read_bio_PrivateKey(bio.get(), nullptr, nullptr, nullptr));
    if (!result) {
...
std::string PEMFromPrivateKey(EVP_PKEY* key)
{
    ...
    if (!PEM_write_bio_PrivateKey(temp_memory_bio.get(), key, nullptr, nullptr, 0, nullptr, nullptr)) {
...
scoped_refptr<SSLPrivateKey> LoadPrivateKeyOpenSSL(const base::FilePath& filepath)
{
