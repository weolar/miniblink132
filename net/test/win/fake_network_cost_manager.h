...
public:
    ...
    FakeNetworkCostManagerEnvironment(const FakeNetworkCostManagerEnvironment&) = delete;
    FakeNetworkCostManagerEnvironment& operator=(const FakeNetworkCostManagerEnvironment&) = delete;

private:
    // Creates a fake implementation of `INetworkCostManager`.
    HRESULT FakeCoCreateInstance(REFCLSID class_id, LPUNKNOWN outer_aggregate, DWORD context_flags, REFIID interface_id, LPVOID* result);
    ...
    NetworkChangeNotifier::ConnectionCost connection_cost_ GUARDED_BY(member_lock_) = NetworkChangeNotifier::ConnectionCost::CONNECTION_COST_UNKNOWN;

    // When `FakeNetworkCostManagerEnvironment` creates a new
    // `FakeNetworkCostManager`, the new `FakeNetworkCostManager` will simulate
    // this error.
    NetworkCostManagerStatus error_status_ GUARDED_BY(member_lock_) = NetworkCostManagerStatus::kOk;

    // Holds the fake implementations of `INetworkCostManager` constructed through
    // `FakeCoCreateInstance()`.
    std::vector<Microsoft::WRL::ComPtr<FakeNetworkCostManager>> fake_network_cost_managers_ GUARDED_BY(member_lock_);
};
