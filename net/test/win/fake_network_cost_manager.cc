...
DWORD NlmConnectionCostFlagsFromConnectionCost(NetworkChangeNotifier::ConnectionCost source_cost)
{
    switch (source_cost) {
    ...
    case NetworkChangeNotifier::ConnectionCost::CONNECTION_COST_METERED:
        return (NLM_CONNECTION_COST_VARIABLE | NLM_CONNECTION_COST_ROAMING | NLM_CONNECTION_COST_APPROACHINGDATALIMIT);
...
void DispatchCostChangedEvent(ComPtr<INetworkCostManagerEvents> event_target, DWORD cost)
{
    std::ignore = event_target->CostChanged(cost, /*destination_address=*/nullptr);
}
...
class FakeNetworkCostManager final : public RuntimeClass<RuntimeClassFlags<ClassicCom>, INetworkCostManager, IConnectionPointContainer, IConnectionPoint> {
public:
    FakeNetworkCostManager(NetworkChangeNotifier::ConnectionCost connection_cost, NetworkCostManagerStatus error_status)
    ...
    // sink's task runner.
    void PostCostChangedEvents(NetworkChangeNotifier::ConnectionCost changed_cost)
    {
        DWORD cost_for_changed_event;
        std::map</*event_sink_cookie=*/DWORD, EventSinkRegistration> event_sinks_for_changed_event;
        {
            base::AutoLock auto_lock(member_lock_);
            connection_cost_ = changed_cost;
            cost_for_changed_event = NlmConnectionCostFlagsFromConnectionCost(changed_cost);
        ...
        for (const auto& pair : event_sinks_for_changed_event) {
            ...
            registration.event_sink_task_runner_->PostTask(
                FROM_HERE, base::BindOnce(&DispatchCostChangedEvent, registration.event_sink_, cost_for_changed_event));
        }
    }

    // Implement the `INetworkCostManager` interface.
    HRESULT
    __stdcall GetCost(DWORD* cost, NLM_SOCKADDR* destination_ip_address) override
    {
    ...
    HRESULT __stdcall GetDataPlanStatus(NLM_DATAPLAN_STATUS* data_plan_status, NLM_SOCKADDR* destination_ip_address) override
    {
        NOTIMPLEMENTED();
        return E_NOTIMPL;
    }

    HRESULT __stdcall SetDestinationAddresses(UINT32 length, NLM_SOCKADDR* destination_ip_address_list, VARIANT_BOOL append) override
    {
        NOTIMPLEMENTED();
        return E_NOTIMPL;
    }
    ...
    HRESULT __stdcall FindConnectionPoint(REFIID connection_point_id, IConnectionPoint** result) override
    {
        if (error_status_ == NetworkCostManagerStatus::kErrorFindConnectionPointFailed) {
            return E_ABORT;
        }
    ...
    HRESULT __stdcall EnumConnectionPoints(IEnumConnectionPoints** results) override
    {
        NOTIMPLEMENTED();
        return E_NOTIMPL;
    }
    ...
    HRESULT __stdcall Advise(IUnknown* event_sink, DWORD* event_sink_cookie) override
    {
        ...
        HRESULT hr = event_sink->QueryInterface(IID_PPV_ARGS(&cost_manager_event_sink));
        if (hr != S_OK) {
            return hr;
        }
        ...
        event_sinks_[next_event_sink_cookie_] = { cost_manager_event_sink, base::SequencedTaskRunner::GetCurrentDefault() };
    ...
    HRESULT __stdcall GetConnectionPointContainer(IConnectionPointContainer** result) override
    {
        NOTIMPLEMENTED();
        return E_NOTIMPL;
    }
    ...
    HRESULT __stdcall QueryInterface(REFIID interface_id, void** result) override
    {
...
private:
    ...
    NetworkChangeNotifier::ConnectionCost connection_cost_ GUARDED_BY(member_lock_);
    ...
    std::map</*event_sink_cookie=*/DWORD, EventSinkRegistration> event_sinks_ GUARDED_BY(member_lock_);
};
...
FakeNetworkCostManagerEnvironment::FakeNetworkCostManagerEnvironment()
{
    ...
    NetworkCostChangeNotifierWin::OverrideCoCreateInstanceForTesting(
        base::BindRepeating(&FakeNetworkCostManagerEnvironment::FakeCoCreateInstance, base::Unretained(this)));
}

FakeNetworkCostManagerEnvironment::~FakeNetworkCostManagerEnvironment()
{
    // Restore `NetworkCostChangeNotifierWin` to use the real OS APIs.
    NetworkCostChangeNotifierWin::OverrideCoCreateInstanceForTesting(base::BindRepeating(&CoCreateInstance));
}

HRESULT FakeNetworkCostManagerEnvironment::FakeCoCreateInstance(
    REFCLSID class_id, LPUNKNOWN outer_aggregate, DWORD context_flags, REFIID interface_id, LPVOID* result)
{
    ...
    if (error_status_for_new_instance == NetworkCostManagerStatus::kErrorCoCreateInstanceFailed) {
    ...
    ComPtr<FakeNetworkCostManager> instance = Microsoft::WRL::Make<FakeNetworkCostManager>(connection_cost_for_new_instance, error_status_for_new_instance);
    {
        base::AutoLock auto_lock(member_lock_);
...
void FakeNetworkCostManagerEnvironment::SetCost(NetworkChangeNotifier::ConnectionCost value)
{
    // Update the cost for each `INetworkCostManager` instance in
    // `fake_network_cost_managers_`.
    std::vector<Microsoft::WRL::ComPtr<FakeNetworkCostManager>> fake_network_cost_managers_for_change_event;
    {
        base::AutoLock auto_lock(member_lock_);
    ...
    for (const auto& network_cost_manager : fake_network_cost_managers_for_change_event) {
        network_cost_manager->PostCostChangedEvents(/*connection_cost=*/value);
    }
}

void FakeNetworkCostManagerEnvironment::SimulateError(NetworkCostManagerStatus error_status)
{
    base::AutoLock auto_lock(member_lock_);
    error_status_ = error_status;
}
