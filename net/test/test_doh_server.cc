...
std::unique_ptr<test_server::HttpResponse> MakeHttpErrorResponse(HttpStatusCode status, std::string_view error)
{
...
std::unique_ptr<test_server::HttpResponse> MakeHttpResponseFromDns(const DnsResponse& dns_response)
{
    if (!dns_response.IsValid()) {
        return MakeHttpErrorResponse(HTTP_INTERNAL_SERVER_ERROR, "error making DNS response");
    }

    auto response = std::make_unique<test_server::BasicHttpResponse>();
    response->set_code(HTTP_OK);
    response->set_content(std::string(dns_response.io_buffer()->data(), dns_response.io_buffer_size()));
...
{
    server_.RegisterRequestHandler(base::BindRepeating(&TestDohServer::HandleRequest, base::Unretained(this)));
}
...
void TestDohServer::AddAddressRecord(std::string_view name, const IPAddress& address, base::TimeDelta ttl)
{
...
{
    GURL url = hostname_ ? server_.GetURL(*hostname_, kPath) : server_.GetURL(kPath);
    return url.spec() + "{?dns}";
}
...
int TestDohServer::QueriesServedForSubdomains(std::string_view domain)
{
    ...
    auto is_subdomain = [&domain](std::string_view candidate) { return net::IsSubdomainOf(candidate, domain); };
    base::AutoLock lock(lock_);
    return base::ranges::count_if(query_qnames_, is_subdomain);
}

std::unique_ptr<test_server::HttpResponse> TestDohServer::HandleRequest(const test_server::HttpRequest& request)
{
    ...
    if (request.method == test_server::METHOD_GET) {
        ...
        if (!GetValueForKeyInQuery(request_url, "dns", &query_b64) || !base::Base64UrlDecode(query_b64, base::Base64UrlDecodePolicy::IGNORE_PADDING, &query)) {
            return MakeHttpErrorResponse(HTTP_BAD_REQUEST, "could not decode query string");
        }
    } else if (request.method == test_server::METHOD_POST) {
        auto content_type = request.headers.find("content-type");
        if (content_type == request.headers.end() || content_type->second != "application/dns-message") {
            return MakeHttpErrorResponse(HTTP_BAD_REQUEST, "unsupported content type");
        }
    ...
    std::optional<std::string> name = dns_names_util::NetworkToDottedName(dns_query.qname(), /*require_complete=*/true);
    if (!name) {
        DnsResponse response(dns_query.id(), /*is_authoritative=*/false,
            /*answers=*/ {}, /*authority_records=*/ {},
            /*additional_records=*/ {}, dns_query, dns_protocol::kRcodeFORMERR);
    ...
    LOG(INFO) << "Serving " << answers.size() << " records for " << *name << ", qtype " << dns_query.qtype();
