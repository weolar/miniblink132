...
public:
    ...
    MockJobInterceptor(const base::FilePath& base_path, bool map_all_requests_to_base_path)
    ...
    {
        return std::make_unique<URLRequestMockHTTPJob>(request, map_all_requests_to_base_path_ ? base_path_ : GetOnDiskPath(request));
    }
...
void URLRequestMockHTTPJob::AddUrlHandlers(const base::FilePath& base_path)
{
    ...
    filter->AddHostnameInterceptor("http", kMockHostname, CreateInterceptor(base_path));
...
// static
std::unique_ptr<URLRequestInterceptor> URLRequestMockHTTPJob::CreateInterceptor(const base::FilePath& base_path)
{
    return std::make_unique<MockJobInterceptor>(base_path, false);
}

// static
std::unique_ptr<URLRequestInterceptor> URLRequestMockHTTPJob::CreateInterceptorForSingleFile(const base::FilePath& file)
{
    return std::make_unique<MockJobInterceptor>(file, true);
}

URLRequestMockHTTPJob::URLRequestMockHTTPJob(URLRequest* request, const base::FilePath& file_path)
    : URLRequestTestJobBackedByFile(request, file_path, base::ThreadPool::CreateTaskRunner({ base::MayBlock() }))
{
}
...
bool URLRequestMockHTTPJob::IsRedirectResponse(GURL* location, int* http_status_code, bool* insecure_scheme_was_upgraded)
{
    // Override the URLRequestTestJobBackedByFile implementation to invoke the
    // default one based on HttpResponseInfo.
    return URLRequestJob::IsRedirectResponse(location, http_status_code, insecure_scheme_was_upgraded);
}
...
{
    base::ThreadPool::PostTaskAndReplyWithResult(FROM_HERE, { base::MayBlock() }, base::BindOnce(&DoFileIO, file_path_),
        base::BindOnce(&URLRequestMockHTTPJob::SetHeadersAndStart, weak_ptr_factory_.GetWeakPtr()));
}
