...
public:
    URLRequestTestJobBackedByFile(URLRequest* request, const base::FilePath& file_path, const scoped_refptr<base::TaskRunner>& file_task_runner);

    URLRequestTestJobBackedByFile(const URLRequestTestJobBackedByFile&) = delete;
    URLRequestTestJobBackedByFile& operator=(const URLRequestTestJobBackedByFile&) = delete;
...
private:
    ...
    static std::unique_ptr<FileMetaInfo> FetchMetaInfo(const base::FilePath& file_path);
