...
void URLRequestHangingReadJob::Start()
{
    ...
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(&URLRequestHangingReadJob::StartAsync, weak_factory_.GetWeakPtr()));
}
...
void URLRequestHangingReadJob::GetResponseInfoConst(HttpResponseInfo* info) const
{
...
void URLRequestHangingReadJob::AddUrlHandler()
{
    ...
    filter->AddHostnameInterceptor("http", kMockHostname, std::make_unique<MockJobInterceptor>());
    filter->AddHostnameInterceptor("https", kMockHostname, std::make_unique<MockJobInterceptor>());
}
