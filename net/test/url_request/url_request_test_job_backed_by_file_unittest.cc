...
public:
    ...
    // OnReadComplete.
    TestURLRequestTestJobBackedByFile(URLRequest* request, const base::FilePath& file_path, const scoped_refptr<base::TaskRunner>& file_task_runner,
        int* open_result, int64_t* seek_position, bool* done_reading, std::string* observed_content)
...
protected:
    ...
    // observed.
    void RunSuccessfulRequestWithString(const std::string& content, const Range* range);
    ...
    void RunSuccessfulRequestWithString(
        const std::string& content, const std::string& expected_content, const base::FilePath::StringPieceType& file_extension, const Range* range);
    ...
    // range header.
    void RunRequestWithPath(
        const base::FilePath& path, const std::string& range, int* open_result, int64_t* seek_position, bool* done_reading, std::string* observed_content);
...
URLRequestTestJobBackedByFileEventsTest::URLRequestTestJobBackedByFileEventsTest()
...
void URLRequestTestJobBackedByFileEventsTest::RunSuccessfulRequestWithString(const std::string& content, const Range* range)
{
    RunSuccessfulRequestWithString(content, content, FILE_PATH_LITERAL(""), range);
}

void URLRequestTestJobBackedByFileEventsTest::RunSuccessfulRequestWithString(
    const std::string& raw_content, const std::string& expected_content, const base::FilePath::StringPieceType& file_extension, const Range* range)
{
...

    {
        ...
        RunRequestWithPath(path, range_value, &open_result, &seek_position, &done_reading, &observed_content);

        EXPECT_EQ(OK, open_result);
        EXPECT_FALSE(delegate_.request_failed());
        int expected_length = range ? (range->end - range->start + 1) : expected_content.length();
        EXPECT_EQ(delegate_.bytes_received(), expected_length);

        std::string expected_data_received;
        if (range) {
            expected_data_received.insert(0, expected_content, range->start, expected_length);
    ...
    const base::FilePath& path, const std::string& range, int* open_result, int64_t* seek_position, bool* done_reading, std::string* observed_content)
{
    ...
    std::unique_ptr<URLRequest> request(context_->CreateRequest(kUrl, DEFAULT_PRIORITY, &delegate_, TRAFFIC_ANNOTATION_FOR_TESTS));
    TestScopedURLInterceptor interceptor(kUrl,
        std::make_unique<TestURLRequestTestJobBackedByFile>(
            request.get(), path, base::SingleThreadTaskRunner::GetCurrentDefault(), open_result, seek_position, done_reading, observed_content));
    if (!range.empty()) {
        request->SetExtraRequestHeaderByName(HttpRequestHeaders::kRange, range, true /*overwrite*/);
    }
    request->Start();
...
TEST_F(URLRequestTestJobBackedByFileEventsTest, DecodeSvgzFile)
{
    ...
    unsigned char gzip_data[] = { // From:
        //   echo -n 'Hello, World!' | gzip | xxd -i | sed -e 's/^/  /'
        0x1f, 0x8b, 0x08, 0x00, 0x2b, 0x02, 0x84, 0x55, 0x00, 0x03, 0xf3, 0x48, 0xcd, 0xc9, 0xc9, 0xd7, 0x51, 0x08, 0xcf, 0x2f, 0xca, 0x49, 0x51, 0x04, 0x00,
        0xd0, 0xc3, 0x4a, 0xec, 0x0d, 0x00, 0x00, 0x00
    };
    RunSuccessfulRequestWithString(std::string(reinterpret_cast<char*>(gzip_data), sizeof(gzip_data)), expected_content, FILE_PATH_LITERAL("svgz"), nullptr);
}
...
TEST_F(URLRequestTestJobBackedByFileEventsTest, OpenNonExistentFile)
{
    ...
    path = path.Append(FILE_PATH_LITERAL("net/data/url_request_unittest/non-existent.txt"));

    int open_result;
    int64_t seek_position;
    bool done_reading;
    std::string observed_content;
    RunRequestWithPath(path, std::string(), &open_result, &seek_position, &done_reading, &observed_content);
...
TEST_F(URLRequestTestJobBackedByFileEventsTest, MultiRangeRequestNotSupported)
{
    base::FilePath path;
    base::PathService::Get(base::DIR_SRC_TEST_DATA_ROOT, &path);
    path = path.Append(FILE_PATH_LITERAL("net/data/url_request_unittest/BullRunSpeech.txt"));

    int open_result;
    int64_t seek_position;
    bool done_reading;
    std::string observed_content;
    RunRequestWithPath(path, "bytes=1-5,20-30", &open_result, &seek_position, &done_reading, &observed_content);
...
TEST_F(URLRequestTestJobBackedByFileEventsTest, RangeExceedingFileSize)
{
    ...
    bool done_reading;
    std::string observed_content;
    RunRequestWithPath(path, "bytes=50000-", &open_result, &seek_position, &done_reading, &observed_content);
...
TEST_F(URLRequestTestJobBackedByFileEventsTest, IgnoreRangeParsingError)
{
    base::FilePath path;
    base::PathService::Get(base::DIR_SRC_TEST_DATA_ROOT, &path);
    path = path.Append(FILE_PATH_LITERAL("net/data/url_request_unittest/simple.html"));

    int open_result;
    int64_t seek_position;
    bool done_reading;
    std::string observed_content;
    RunRequestWithPath(path, "bytes=3-z", &open_result, &seek_position, &done_reading, &observed_content);
