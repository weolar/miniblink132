...
URLRequestTestJobBackedByFile::URLRequestTestJobBackedByFile(
    URLRequest* request, const base::FilePath& file_path, const scoped_refptr<base::TaskRunner>& file_task_runner)
...
{
    file_task_runner_->PostTaskAndReplyWithResult(FROM_HERE, base::BindOnce(&URLRequestTestJobBackedByFile::FetchMetaInfo, file_path_),
        base::BindOnce(&URLRequestTestJobBackedByFile::DidFetchMetaInfo, weak_ptr_factory_.GetWeakPtr()));
}
...
int URLRequestTestJobBackedByFile::ReadRawData(IOBuffer* dest, int dest_size)
{
    ...
    int rv
        = stream_->Read(dest, dest_size, base::BindOnce(&URLRequestTestJobBackedByFile::DidRead, weak_ptr_factory_.GetWeakPtr(), base::WrapRefCounted(dest)));
    if (rv >= 0) {
        remaining_bytes_ -= rv;
        DCHECK_GE(remaining_bytes_, 0);
    }
...
void URLRequestTestJobBackedByFile::SetExtraRequestHeaders(const HttpRequestHeaders& headers)
{
...
void URLRequestTestJobBackedByFile::GetResponseInfo(HttpResponseInfo* info)
{
    ...
    headers->AddHeader(net::HttpRequestHeaders::kContentType, meta_info_.mime_type);
...
std::unique_ptr<SourceStream> URLRequestTestJobBackedByFile::SetUpSourceStream()
{
...
std::unique_ptr<URLRequestTestJobBackedByFile::FileMetaInfo> URLRequestTestJobBackedByFile::FetchMetaInfo(const base::FilePath& file_path)
{
    ...
    // done in WorkerPool.
    meta_info->mime_type_result = GetMimeTypeFromFile(file_path, &meta_info->mime_type);
    meta_info->absolute_path = base::MakeAbsoluteFilePath(file_path);
    return meta_info;
}

void URLRequestTestJobBackedByFile::DidFetchMetaInfo(std::unique_ptr<FileMetaInfo> meta_info)
{
    ...
    int rv = stream_->Open(file_path_, flags, base::BindOnce(&URLRequestTestJobBackedByFile::DidOpen, weak_ptr_factory_.GetWeakPtr()));
...
void URLRequestTestJobBackedByFile::DidOpen(int result)
{
    ...
    if (range_parse_result_ != OK || !byte_range_.ComputeBounds(meta_info_.file_size)) {
    ...
    if (remaining_bytes_ > 0 && byte_range_.first_byte_position() != 0) {
        int rv = stream_->Seek(byte_range_.first_byte_position(), base::BindOnce(&URLRequestTestJobBackedByFile::DidSeek, weak_ptr_factory_.GetWeakPtr()));
        if (rv != ERR_IO_PENDING)
            DidSeek(ERR_REQUEST_RANGE_NOT_SATISFIABLE);
    } else {
...
void URLRequestTestJobBackedByFile::DidRead(scoped_refptr<IOBuffer> buf, int result)
{
    if (result >= 0) {
