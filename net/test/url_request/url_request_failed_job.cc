...
static_assert(std::size(kFailurePhase) == URLRequestFailedJob::FailurePhase::MAX_FAILURE_PHASE, "kFailurePhase must match FailurePhase enum");
...
public:
    ...
    std::unique_ptr<URLRequestJob> MaybeInterceptRequest(URLRequest* request) const override
    {
        ...
        for (size_t i = 0; i < std::size(kFailurePhase); i++) {
            ...
            if (GetValueForKeyInQuery(request->url(), kFailurePhase[i], &phase_error_string)) {
...
GURL GetMockUrl(const std::string& scheme, const std::string& hostname, URLRequestFailedJob::FailurePhase phase, int net_error)
{
    ...
    return GURL(scheme + "://" + hostname + "/error?" + kFailurePhase[phase] + "=" + base::NumberToString(net_error));
}

} // namespace

URLRequestFailedJob::URLRequestFailedJob(URLRequest* request, FailurePhase phase, int net_error)
...
{
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(&URLRequestFailedJob::StartAsync, weak_factory_.GetWeakPtr()));
}
...
int URLRequestFailedJob::ReadRawData(IOBuffer* buf, int buf_size)
{
    ...
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
        FROM_HERE, base::BindOnce(&URLRequestFailedJob::ReadRawDataComplete, weak_factory_.GetWeakPtr(), net_error_));
...
void URLRequestFailedJob::PopulateNetErrorDetails(NetErrorDetails* details) const
{
    if (net_error_ == ERR_QUIC_PROTOCOL_ERROR) {
        ...
        details->quic_connection_error = quic::QUIC_CONNECTION_MIGRATION_NO_NEW_NETWORK;
    }
}
...
// static
void URLRequestFailedJob::AddUrlHandlerForHostname(const std::string& hostname)
{
...
// static
GURL URLRequestFailedJob::GetMockHttpUrlWithFailurePhase(FailurePhase phase, int net_error)
{
    return GetMockUrl("http", kMockHostname, phase, net_error);
}

// static
GURL URLRequestFailedJob::GetMockHttpUrlForHostname(int net_error, const std::string& hostname)
{
    return GetMockUrl("http", hostname, START, net_error);
}

// static
GURL URLRequestFailedJob::GetMockHttpsUrlForHostname(int net_error, const std::string& hostname)
{
...
void URLRequestFailedJob::StartAsync()
{
    ...
    response_info_.headers = base::MakeRefCounted<net::HttpResponseHeaders>(headers);
