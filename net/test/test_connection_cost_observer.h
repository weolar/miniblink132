...
class TestConnectionCostObserver final : public NetworkChangeNotifier::ConnectionCostObserver {
public:
    TestConnectionCostObserver();
    ~TestConnectionCostObserver() final;

    void OnConnectionCostChanged(NetworkChangeNotifier::ConnectionCost cost) final;

    void WaitForConnectionCostChanged();

    size_t cost_changed_calls() const;
    std::vector<NetworkChangeNotifier::ConnectionCost> cost_changed_inputs() const;
    ...
    TestConnectionCostObserver& operator=(const TestConnectionCostObserver&) = delete;
