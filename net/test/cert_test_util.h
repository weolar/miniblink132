...
// CertificateList.
CertificateList CreateCertificateListFromFile(const base::FilePath& certs_dir, std::string_view cert_file, int format);
...
// directory.
::testing::AssertionResult LoadCertificateFiles(const std::vector<std::string>& cert_filenames, CertificateList* certs);
...
scoped_refptr<X509Certificate> CreateCertificateChainFromFile(const base::FilePath& certs_dir, std::string_view cert_file, int format);

// Imports a single certificate from |cert_path|.
// If the file contains multiple certificates, the first certificate found
// will be returned.
scoped_refptr<X509Certificate> ImportCertFromFile(const base::FilePath& cert_path);
...
scoped_refptr<X509Certificate> ImportCertFromFile(const base::FilePath& certs_dir, std::string_view cert_file);
...
public:
    ScopedTestEVPolicy(EVRootCAMetadata* ev_root_ca_metadata, const SHA256HashValue& fingerprint, const char* policy);
    ~ScopedTestEVPolicy();
