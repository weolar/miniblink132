...
public:
    ...
    struct SctConfig {
        SctConfig();
        SctConfig(std::string log_id, bssl::UniquePtr<EVP_PKEY> log_key, base::Time timestamp);
    ...
    // |issuer|.
    static std::unique_ptr<CertBuilder> FromFile(const base::FilePath& cert_and_key_file, CertBuilder* issuer);
    ...
    static std::unique_ptr<CertBuilder> FromSubjectPublicKeyInfo(base::span<const uint8_t> spki_der, CertBuilder* issuer);
    ...
    static std::unique_ptr<CertBuilder> FromStaticCert(CRYPTO_BUFFER* cert, EVP_PKEY* key);
    // Like FromStaticCert, but loads the certificate and private key from the
    // PEM file |cert_and_key_file|.
    static std::unique_ptr<CertBuilder> FromStaticCertFile(const base::FilePath& cert_and_key_file);
    ...
    static std::vector<std::unique_ptr<CertBuilder>> CreateSimpleChain(size_t chain_length);
    ...
    static std::optional<bssl::SignatureAlgorithm> DefaultSignatureAlgorithmForKey(EVP_PKEY* key);
    ...
    static bool SignData(bssl::SignatureAlgorithm signature_algorithm, std::string_view tbs_data, EVP_PKEY* key, CBB* out_signature);

    static bool SignDataWithDigest(const EVP_MD* digest, std::string_view tbs_data, EVP_PKEY* key, CBB* out_signature);

    // Returns a DER encoded AlgorithmIdentifier TLV for |signature_algorithm|
    // empty string on error.
    static std::string SignatureAlgorithmToDer(bssl::SignatureAlgorithm signature_algorithm);
    ...
    static std::vector<uint8_t> BuildNameWithCommonNameOfType(std::string_view common_name, unsigned common_name_tag);
    ...
    void SetExtension(const bssl::der::Input& oid, std::string value, bool critical = false);
    ...
    void SetNameConstraintsDnsNames(const std::vector<std::string>& permitted_dns_names, const std::vector<std::string>& excluded_dns_names);
    ...
    // removed.
    void SetCaIssuersAndOCSPUrls(const std::vector<GURL>& ca_issuers_urls, const std::vector<GURL>& ocsp_urls);
    ...
    void SetSubjectAltNames(const std::vector<std::string>& dns_names, const std::vector<IPAddress>& ip_addresses);
    ...
    void SetPolicyMappings(const std::vector<std::pair<std::string, std::string>>& policy_mappings);
    ...
    // will removed.
    void SetPolicyConstraints(std::optional<uint64_t> require_explicit_policy, std::optional<uint64_t> inhibit_policy_mapping);
...
private:
    ...
    CertBuilder(CRYPTO_BUFFER* orig_cert, CertBuilder* issuer, bool unique_subject_key_identifier);
    ...
    void BuildTBSCertificate(std::string_view signature_algorithm_tlv, std::string* out);

    void BuildSctListExtension(const std::string& pre_tbs_certificate, std::string* out);
