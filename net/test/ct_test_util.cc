...
{
    tree_leaf->timestamp = base::Time::FromMillisecondsSinceUnixEpoch(kTestTimestamp);
...
void GetX509CertSCT(scoped_refptr<SignedCertificateTimestamp>* sct_ref)
{
    ...
    *sct_ref = base::MakeRefCounted<SignedCertificateTimestamp>();
    ...
    sct->timestamp = base::Time::UnixEpoch() + base::Milliseconds(INT64_C(1365181456089));
...
void GetPrecertSCT(scoped_refptr<SignedCertificateTimestamp>* sct_ref)
{
    ...
    *sct_ref = base::MakeRefCounted<SignedCertificateTimestamp>();
    ...
    sct->timestamp = base::Time::UnixEpoch() + base::Milliseconds(INT64_C(1365181456275));
    sct->extensions.clear();
...
{
    sth->version = SignedTreeHead::V1;
    sth->timestamp = base::Time::UnixEpoch() + base::Milliseconds(INT64_C(1450443594920));
    sth->tree_size = 0;
    std::string empty_root_hash = HexDecode("e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855");
    memcpy(sth->sha256_root_hash, empty_root_hash.c_str(), kSthRootHashLength);
    sth->log_id = GetTestPublicKeyId();

    std::string tree_head_signature = HexDecode("040300463044022046c26401de9416403da54762dc1f1687c38eafd791b15e484ab4c5f7"
...
{
    sth->version = SignedTreeHead::V1;
    sth->timestamp = base::Time::UnixEpoch() + base::Milliseconds(INT64_C(1450870952897));
    sth->tree_size = 0;
    memset(sth->sha256_root_hash, 'f', kSthRootHashLength);
    sth->log_id = GetTestPublicKeyId();

    std::string tree_head_signature = HexDecode("04030046304402207cab04c62dee5d1cbc95fec30cd8417313f71587b75f133ad2e6f324"
...
{
    return CreateSignedTreeHeadJsonString(kSampleSTHTreeSize, kTestTimestamp, GetSampleSTHSHA256RootHash(), GetSampleSTHTreeHeadSignature());
}

std::string CreateSignedTreeHeadJsonString(size_t tree_size, int64_t timestamp, std::string sha256_root_hash, std::string tree_head_signature)
{
    std::string sth_json = std::string("{\"tree_size\":") + base::NumberToString(tree_size) + std::string(",\"timestamp\":") + base::NumberToString(timestamp);

    if (!sha256_root_hash.empty()) {
        std::string root_hash_b64 = base::Base64Encode(sha256_root_hash);
        sth_json += base::StringPrintf(",\"sha256_root_hash\":\"%s\"", root_hash_b64.c_str());
    }
    if (!tree_head_signature.empty()) {
        std::string tree_head_signature_b64 = base::Base64Encode(tree_head_signature);
        sth_json += base::StringPrintf(",\"tree_head_signature\":\"%s\"", tree_head_signature_b64.c_str());
    }
...
std::string CreateConsistencyProofJsonString(const std::vector<std::string>& raw_nodes)
{
...
bool CheckForSingleVerifiedSCTInResult(const SignedCertificateTimestampAndStatusList& scts, const std::string& log_description)
{
    return (scts.size() == 1 && scts[0].status == ct::SCT_STATUS_OK && scts[0].sct->log_description == log_description);
}

bool CheckForSCTOrigin(const SignedCertificateTimestampAndStatusList& scts, ct::SignedCertificateTimestamp::Origin origin)
{
    for (const auto& sct_and_status : scts)
        if (sct_and_status.status == SCT_STATUS_OK && sct_and_status.sct->origin == origin)
            return true;
