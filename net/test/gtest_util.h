...
MATCHER(IsOk, std::string(negation ? "not " : "") + net::ErrorToString(net::OK))
{
    if (arg <= 0)
        *result_listener << net::ErrorToString(arg);
    return arg == net::OK;
...
public:
    ...
    StringPieceHasSubstrMatcher& operator=(const StringPieceHasSubstrMatcher&) = default;
...
#define GTEST_DFATAL_(statement, matcher, fail)                                                                                                                \
    do {                                                                                                                                                       \
        ...
        using ::testing::_;                                                                                                                                    \
        EXPECT_CALL(gtest_log, Log(_, _, _, _, _)).WillRepeatedly(::testing::Return(false));                                                                   \
        EXPECT_CALL(gtest_log, Log(::logging::LOGGING_DFATAL, _, _, _, matcher)).Times(::testing::AtLeast(1)).WillOnce(::testing::Return(false));              \
...
#define EXPECT_DFATAL_WITH(statement, matcher) GTEST_DFATAL_(statement, matcher, GTEST_NONFATAL_FAILURE_)

#define ASSERT_DFATAL_WITH(statement, matcher) GTEST_DFATAL_(statement, matcher, GTEST_FATAL_FAILURE_)

#define EXPECT_DFATAL(statement, regex) EXPECT_DFATAL_WITH(statement, ::testing::ContainsRegex(regex))

#define ASSERT_DFATAL(statement, regex) ASSERT_DFATAL_WITH(statement, ::testing::ContainsRegex(regex))
