...
RawHttpResponse::RawHttpResponse(const std::string& headers, const std::string& contents)
    : headers_(headers)
    , contents_(contents)
{
}
...
void RawHttpResponse::SendResponse(base::WeakPtr<HttpResponseDelegate> delegate)
{
    if (!headers_.empty()) {
...
void BasicHttpResponse::SendResponse(base::WeakPtr<HttpResponseDelegate> delegate)
{
    delegate->SendHeadersContentAndFinish(code_, reason(), BuildHeaders(), content_);
}
...
void DelayedHttpResponse::SendResponse(base::WeakPtr<HttpResponseDelegate> delegate)
{
    base::SequencedTaskRunner::GetCurrentDefault()->PostDelayedTask(
        FROM_HERE, base::BindOnce(&HttpResponseDelegate::SendHeadersContentAndFinish, delegate, code(), reason(), BuildHeaders(), content()), delay_);
}
...
HungAfterHeadersHttpResponse::HungAfterHeadersHttpResponse(base::StringPairs headers)
    : headers_(headers)
{
}
HungAfterHeadersHttpResponse::~HungAfterHeadersHttpResponse() = default;

void HungAfterHeadersHttpResponse::SendResponse(base::WeakPtr<HttpResponseDelegate> delegate)
{
