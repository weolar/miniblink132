...
public:
    ...
    explicit WebSocketConnection(std::unique_ptr<StreamSocket> socket, std::string_view sec_websocket_key);
    ...
    // the server side.
    void StartClosingHandshake(std::optional<uint16_t> code, std::string_view message);

    // Responds to a CLOSE frame received from the client. If `code` is
    // std::nullopt then an empty CLOSE frame will be sent.
    void RespondToCloseFrame(std::optional<uint16_t> code, std::string_view message);
...
private:
    ...
    // lifecycle.
    enum class WebSocketState { kHandshakeInProgress, kOpen, kWaitingForClientClose, kDisconnectingSoon, kClosed };
    ...
    // current message.
    void HandleFrame(WebSocketFrameHeader::OpCode opcode, base::span<const char> payload, bool is_final) VALID_CONTEXT_REQUIRED(sequence_checker_);

    // Internal helper for building WebSocket frames (both data and control
    // frames).
    scoped_refptr<IOBufferWithSize> BuildWebSocketFrame(base::span<const uint8_t> payload, WebSocketFrameHeader::OpCode op_code)
        VALID_CONTEXT_REQUIRED(sequence_checker_);

    // Methods to create specific WebSocket frames.
    scoped_refptr<IOBufferWithSize> CreateTextFrame(std::string_view message) VALID_CONTEXT_REQUIRED(sequence_checker_);
    scoped_refptr<IOBufferWithSize> CreateBinaryFrame(base::span<const uint8_t> message) VALID_CONTEXT_REQUIRED(sequence_checker_);
    scoped_refptr<IOBufferWithSize> CreateCloseFrame(std::optional<uint16_t> code, std::string_view message) VALID_CONTEXT_REQUIRED(sequence_checker_);
    scoped_refptr<IOBufferWithSize> CreatePingFrame(base::span<const uint8_t> payload) VALID_CONTEXT_REQUIRED(sequence_checker_);
    scoped_refptr<IOBufferWithSize> CreatePongFrame(base::span<const uint8_t> payload) VALID_CONTEXT_REQUIRED(sequence_checker_);

    // Internal function to handle sending buffers.
    // `wait_for_handshake`: If true, the message will be queued until the
    // handshake is complete.
    void SendInternal(scoped_refptr<IOBufferWithSize> buffer, bool wait_for_handshake);
