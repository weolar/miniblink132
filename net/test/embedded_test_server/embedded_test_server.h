...
public:
    ...
    struct OCSPConfig {
        ...
        explicit OCSPConfig(std::vector<SingleResponse> single_responses, Produced produced = Produced::kValid);
    ...
    using UpgradeResultOrHttpResponse = base::expected<UpgradeResult, std::unique_ptr<HttpResponse>>;
    using HandleUpgradeRequestCallback = base::RepeatingCallback<UpgradeResultOrHttpResponse(const HttpRequest& request, HttpConnection* connection)>;
    typedef base::RepeatingCallback<std::unique_ptr<HttpResponse>(const HttpRequest& request)> HandleRequestCallback;
    typedef base::RepeatingCallback<void(const HttpRequest& request)> MonitorRequestCallback;
    ...
    EmbeddedTestServer();
    explicit EmbeddedTestServer(Type type, HttpConnection::Protocol protocol = HttpConnection::Protocol::kHttp1);
    ~EmbeddedTestServer();
    ...
    void HandleRequest(base::WeakPtr<HttpResponseDelegate> delegate, std::unique_ptr<HttpRequest> request, const StreamSocket* socket);
    ...
    void RemoveConnection(HttpConnection* connection, EmbeddedTestServerConnectionListener* listener = nullptr);
    ...
    // the caller.
    [[nodiscard]] bool Start(int port = 0, std::string_view address = "127.0.0.1");
    ...
    [[nodiscard]] bool InitializeAndListen(int port = 0, std::string_view address = "127.0.0.1");

    // Starts the Accept IO Thread and begins accepting connections.
    [[nodiscard]] EmbeddedTestServerHandle StartAcceptingConnectionsAndReturnHandle();
    ...
    bool Started() const
    {
        return listen_socket_.get() != nullptr;
    }
    ...
    url::Origin GetOrigin(const std::optional<std::string>& hostname = std::nullopt) const;
    ...
    void SetSSLConfig(const ServerCertificateConfig& cert_config, const SSLServerConfig& ssl_config);
    void SetSSLConfig(const ServerCertificateConfig& cert_config);

    // TODO(mattm): make this [[nodiscard]]
    bool ResetSSLConfig(ServerCertificate cert, const SSLServerConfig& ssl_config);
    ...
    static base::FilePath GetFullPathFromSourceDirectory(const base::FilePath& relative);
    ...
    void RegisterUpgradeRequestHandler(const HandleUpgradeRequestCallback& callback);
...
private:
    ...
    void SetSSLConfigInternal(ServerCertificate cert, const ServerCertificateConfig* cert_config, const SSLServerConfig& ssl_config);

    // Resets the SSLServerConfig on the IO thread.
    bool ResetSSLConfigOnIOThread(ServerCertificate cert, const SSLServerConfig& ssl_config);

    HttpConnection* GetConnectionForSocket(const StreamSocket* socket);

    // Upgrade the TCP connection to one over SSL.
    std::unique_ptr<SSLServerSocket> DoSSLUpgrade(std::unique_ptr<StreamSocket> connection);
    ...
    [[nodiscard]] bool PostTaskToIOThreadAndWaitWithResult(base::OnceCallback<bool()> task);
    ...
    raw_ptr<EmbeddedTestServerConnectionListener, DanglingUntriaged> connection_listener_ = nullptr;
    uint16_t port_ = 0;
    GURL base_url_;
