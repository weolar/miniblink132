...
HttpRequestParser::ParseResult HttpRequestParser::ParseHeaders()
{
    ...
    std::string request_headers;
    {
        ...
        std::vector<std::string> header_line_tokens = base::SplitString(header_line, " ", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL);
        ...
        if (http_request_->method == METHOD_CONNECT) {
        ...
        } else if (http_request_->method == METHOD_OPTIONS && header_line_tokens[1] == "*") {
            ...
            if (!header_line_tokens[1].empty() && header_line_tokens[1].front() == '/') {
                http_request_->relative_url = header_line_tokens[1];
            } else {
    ...
    // Parse further headers.
    {
        ...
        while (true) {
            ...
            if (header_line[0] == ' ' || header_line[0] == '\t') {
                ...
                std::string header_value = Trim(header_line.substr(delimiter_pos + 1, header_line.size() - delimiter_pos - 1));
                http_request_->headers[header_name] = header_value;
            }
        }
    }
    ...
    if (http_request_->headers.count("Content-Length") > 0) {
        ...
        const bool success = base::StringToSizeT(http_request_->headers["Content-Length"], &declared_content_length_);
        if (!success) {
            declared_content_length_ = 0;
            LOG(WARNING) << "Malformed Content-Length header's value.";
        }
    } else if (http_request_->headers.count("Transfer-Encoding") > 0) {
        if (base::EqualsCaseInsensitiveASCII(http_request_->headers["Transfer-Encoding"], "chunked")) {
            http_request_->has_content = true;
...
HttpRequestParser::ParseResult HttpRequestParser::ParseContent()
{
    ...
    if (chunked_decoder_.get()) {
        int bytes_written = chunked_decoder_->FilterBuf(base::as_writable_byte_span(buffer_).subspan(buffer_position_, available_bytes));
        http_request_->content.append(buffer_.data() + buffer_position_, bytes_written);
    ...
    const size_t fetch_bytes = std::min(available_bytes, declared_content_length_ - http_request_->content.size());
