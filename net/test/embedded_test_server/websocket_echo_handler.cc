...
WebSocketEchoHandler::WebSocketEchoHandler(scoped_refptr<WebSocketConnection> connection)
...
void WebSocketEchoHandler::OnClosingHandshake(std::optional<uint16_t> code, std::string_view message)
{
    DVLOG(3) << "Closing handshake received with code: " << (code.has_value() ? base::NumberToString(code.value()) : "none") << ", message: " << message;

    connection()->RespondToCloseFrame(code, message);
}

EmbeddedTestServer::HandleUpgradeRequestCallback WebSocketEchoHandler::CreateHandler()
{
    return CreateWebSocketHandler(
        "/echo-with-no-extension", base::BindRepeating([](scoped_refptr<WebSocketConnection> connection) -> std::unique_ptr<WebSocketHandler> {
