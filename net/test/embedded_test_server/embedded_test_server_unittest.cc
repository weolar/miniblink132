...
class TestConnectionListener : public net::test_server::EmbeddedTestServerConnectionListener {
public:
    ...
    std::unique_ptr<StreamSocket> AcceptedSocket(std::unique_ptr<StreamSocket> connection) override
    {
    ...
    void OnResponseCompletedSuccessfully(std::unique_ptr<StreamSocket> socket) override
    {
        base::AutoLock lock(lock_);
    ...
    void WaitUntilFirstConnectionAccepted()
    {
        accept_loop_.Run();
    }

    void WaitUntilGotSocketFromResponseCompleted()
    {
        complete_loop_.Run();
    }
...
class EmbeddedTestServerTest : public testing::TestWithParam<EmbeddedTestServerConfig>, public WithTaskEnvironment {
public:
    ...
    void SetUp() override
    {
        server_ = std::make_unique<EmbeddedTestServer>(GetParam().type, GetParam().protocol);
    ...
    std::unique_ptr<HttpResponse> HandleRequest(
        const std::string& path, const std::string& content, const std::string& content_type, HttpStatusCode code, const HttpRequest& request)
    {
...
TEST_P(EmbeddedTestServerTest, GetBaseURL)
{
    ...
    if (GetParam().type == EmbeddedTestServer::TYPE_HTTPS) {
        EXPECT_EQ(base::StringPrintf("https://127.0.0.1:%u/", server_->port()), server_->base_url().spec());
    } else {
        EXPECT_EQ(base::StringPrintf("http://127.0.0.1:%u/", server_->port()), server_->base_url().spec());
    }
}
...
TEST_P(EmbeddedTestServerTest, GetURL)
{
    ASSERT_TRUE(server_->Start());
    if (GetParam().type == EmbeddedTestServer::TYPE_HTTPS) {
        EXPECT_EQ(base::StringPrintf("https://127.0.0.1:%u/path?query=foo", server_->port()), server_->GetURL("/path?query=foo").spec());
    } else {
        EXPECT_EQ(base::StringPrintf("http://127.0.0.1:%u/path?query=foo", server_->port()), server_->GetURL("/path?query=foo").spec());
    }
}
...
TEST_P(EmbeddedTestServerTest, GetURLWithHostname)
{
    ASSERT_TRUE(server_->Start());
    if (GetParam().type == EmbeddedTestServer::TYPE_HTTPS) {
        EXPECT_EQ(base::StringPrintf("https://foo.com:%d/path?query=foo", server_->port()), server_->GetURL("foo.com", "/path?query=foo").spec());
    } else {
        EXPECT_EQ(base::StringPrintf("http://foo.com:%d/path?query=foo", server_->port()), server_->GetURL("foo.com", "/path?query=foo").spec());
    }
}
...
TEST_P(EmbeddedTestServerTest, RegisterRequestHandler)
{
    server_->RegisterRequestHandler(
        base::BindRepeating(&EmbeddedTestServerTest::HandleRequest, base::Unretained(this), "/test", "<b>Worked!</b>", "text/html", HTTP_OK));
    ASSERT_TRUE(server_->Start());

    TestDelegate delegate;
    std::unique_ptr<URLRequest> request(context_->CreateRequest(server_->GetURL("/test?q=foo"), DEFAULT_PRIORITY, &delegate, TRAFFIC_ANNOTATION_FOR_TESTS));
    ...
    EXPECT_EQ(request->response_headers()->GetNormalizedHeader("Content-Type"), "text/html");
...
TEST_P(EmbeddedTestServerTest, ServeFilesFromDirectory)
{
    ...
    server_->ServeFilesFromDirectory(src_dir.AppendASCII("net").AppendASCII("data"));
    ASSERT_TRUE(server_->Start());

    TestDelegate delegate;
    std::unique_ptr<URLRequest> request(context_->CreateRequest(server_->GetURL("/test.html"), DEFAULT_PRIORITY, &delegate, TRAFFIC_ANNOTATION_FOR_TESTS));
...
TEST_P(EmbeddedTestServerTest, MockHeadersWithoutCRLF)
{
    ...
    base::FilePath src_dir;
    ASSERT_TRUE(base::PathService::Get(base::DIR_SRC_TEST_DATA_ROOT, &src_dir));
    server_->ServeFilesFromDirectory(src_dir.AppendASCII("net").AppendASCII("data").AppendASCII("embedded_test_server"));
    ASSERT_TRUE(server_->Start());

    TestDelegate delegate;
    std::unique_ptr<URLRequest> request(
        context_->CreateRequest(server_->GetURL("/mock-headers-without-crlf.html"), DEFAULT_PRIORITY, &delegate, TRAFFIC_ANNOTATION_FOR_TESTS));
...
TEST_P(EmbeddedTestServerTest, DefaultNotFoundResponse)
{
    ...
    TestDelegate delegate;
    std::unique_ptr<URLRequest> request(context_->CreateRequest(server_->GetURL("/non-existent"), DEFAULT_PRIORITY, &delegate, TRAFFIC_ANNOTATION_FOR_TESTS));
...
TEST_P(EmbeddedTestServerTest, ConnectionListenerAccept)
{
    ...
    std::unique_ptr<StreamSocket> socket
        = ClientSocketFactory::GetDefaultFactory()->CreateTransportClientSocket(address_list, nullptr, nullptr, NetLog::Get(), NetLogSource());
...
TEST_P(EmbeddedTestServerTest, UpgradeRequestHandlerEvalContinuesOnKNotHandled)
{
    if (GetParam().protocol == HttpConnection::Protocol::kHttp2) {
    ...
    server_->RegisterUpgradeRequestHandler(
        base::BindLambdaForTesting([&](const HttpRequest& request, HttpConnection* connection) -> EmbeddedTestServer::UpgradeResultOrHttpResponse {
    ...
    TestDelegate delegate;
    std::unique_ptr<URLRequest> request(context_->CreateRequest(a_different_url, DEFAULT_PRIORITY, &delegate, TRAFFIC_ANNOTATION_FOR_TESTS));
...
TEST_P(EmbeddedTestServerTest, UpgradeRequestHandlerTransfersSocket)
{
    ...
    TestDelegate delegate;
    std::unique_ptr<URLRequest> request(context_->CreateRequest(websocket_upgrade_url, DEFAULT_PRIORITY, &delegate, TRAFFIC_ANNOTATION_FOR_TESTS));
...
TEST_P(EmbeddedTestServerTest, UpgradeRequestHandlerEvalStopsOnErrorResponse)
{
    ...
    EXPECT_EQ(net::OK, delegate.request_status());
    ASSERT_TRUE(request->response_headers());
    EXPECT_EQ(HTTP_INTERNAL_SERVER_ERROR, request->response_headers()->response_code());
...
TEST_P(EmbeddedTestServerTest, MAYBE_ConnectionListenerComplete)
{
    ...
    std::unique_ptr<URLRequest> request(
        context_->CreateRequest(server_->GetURL("/set-header?Connection: Keep-Alive"), DEFAULT_PRIORITY, &delegate, TRAFFIC_ANNOTATION_FOR_TESTS));
...
{
    server_->RegisterRequestHandler(
        base::BindRepeating(&EmbeddedTestServerTest::HandleRequest, base::Unretained(this), "/test1", "Raspberry chocolate", "text/html", HTTP_OK));
    server_->RegisterRequestHandler(
        base::BindRepeating(&EmbeddedTestServerTest::HandleRequest, base::Unretained(this), "/test2", "Vanilla chocolate", "text/html", HTTP_OK));
    server_->RegisterRequestHandler(
        base::BindRepeating(&EmbeddedTestServerTest::HandleRequest, base::Unretained(this), "/test3", "No chocolates", "text/plain", HTTP_NOT_FOUND));
    ASSERT_TRUE(server_->Start());

    TestDelegate delegate1;
    std::unique_ptr<URLRequest> request1(context_->CreateRequest(server_->GetURL("/test1"), DEFAULT_PRIORITY, &delegate1, TRAFFIC_ANNOTATION_FOR_TESTS));
    TestDelegate delegate2;
    std::unique_ptr<URLRequest> request2(context_->CreateRequest(server_->GetURL("/test2"), DEFAULT_PRIORITY, &delegate2, TRAFFIC_ANNOTATION_FOR_TESTS));
    TestDelegate delegate3;
    std::unique_ptr<URLRequest> request3(context_->CreateRequest(server_->GetURL("/test3"), DEFAULT_PRIORITY, &delegate3, TRAFFIC_ANNOTATION_FOR_TESTS));
    ...
    EXPECT_EQ(request1->response_headers()->GetNormalizedHeader("Content-Type"), "text/html");
    ...
    EXPECT_EQ(request2->response_headers()->GetNormalizedHeader("Content-Type"), "text/html");
    ...
    EXPECT_EQ(request3->response_headers()->GetNormalizedHeader("Content-Type"), "text/plain");
}
...
public:
    ...
    void OnResponseStarted(URLRequest* request, int net_error) override
    {
        ...
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, run_loop_.QuitClosure(), base::Seconds(1));
    }
...
private:
    void SendInfinite(base::WeakPtr<HttpResponseDelegate> delegate)
    {
        ...
        delegate->SendContents("echo", base::BindOnce(&InfiniteResponse::OnSendDone, weak_ptr_factory_.GetWeakPtr(), delegate));
    }

    void OnSendDone(base::WeakPtr<HttpResponseDelegate> delegate)
    {
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
            FROM_HERE, base::BindOnce(&InfiniteResponse::SendInfinite, weak_ptr_factory_.GetWeakPtr(), delegate));
    }

    base::WeakPtrFactory<InfiniteResponse> weak_ptr_factory_ { this };
};

std::unique_ptr<HttpResponse> HandleInfiniteRequest(const HttpRequest& request)
{
...
TEST_P(EmbeddedTestServerTest, CloseDuringWrite)
{
    ...
    server_->RegisterRequestHandler(base::BindRepeating(&HandlePrefixedRequest, "/infinite", base::BindRepeating(&HandleInfiniteRequest)));
    ASSERT_TRUE(server_->Start());

    std::unique_ptr<URLRequest> request
        = context_->CreateRequest(server_->GetURL("/infinite"), DEFAULT_PRIORITY, &cancel_delegate, TRAFFIC_ANNOTATION_FOR_TESTS);
    request->Start();
...
const struct CertificateValuesEntry {
    ...
    { EmbeddedTestServer::CERT_OK_BY_INTERMEDIATE, false, "127.0.0.1", "Test Intermediate CA", 2 },
    { EmbeddedTestServer::CERT_MISMATCHED_NAME, false, "127.0.0.1", "Test Root CA", 1 },
    { EmbeddedTestServer::CERT_COMMON_NAME_IS_DOMAIN, false, "localhost", "Test Root CA", 1 },
...
TEST_P(EmbeddedTestServerTest, AcceptCHFrame)
{
    ...
    TestDelegate delegate;
    std::unique_ptr<URLRequest> request_a(context_->CreateRequest(server_->GetURL("/non-existent"), DEFAULT_PRIORITY, &delegate, TRAFFIC_ANNOTATION_FOR_TESTS));
    request_a->Start();
    delegate.RunUntilComplete();
...
TEST_P(EmbeddedTestServerTest, AcceptCHFrameDifferentOrigins)
{
...

    {
        ...
        std::unique_ptr<URLRequest> request_a(
            context_->CreateRequest(server_->GetURL("a.test", "/non-existent"), DEFAULT_PRIORITY, &delegate, TRAFFIC_ANNOTATION_FOR_TESTS));
        request_a->Start();
        delegate.RunUntilComplete();
...

    {
        TestDelegate delegate;
        std::unique_ptr<URLRequest> request_a(
            context_->CreateRequest(server_->GetURL("b.test", "/non-existent"), DEFAULT_PRIORITY, &delegate, TRAFFIC_ANNOTATION_FOR_TESTS));
        request_a->Start();
        delegate.RunUntilComplete();
...

    {
        TestDelegate delegate;
        std::unique_ptr<URLRequest> request_a(
            context_->CreateRequest(server_->GetURL("c.b.test", "/non-existent"), DEFAULT_PRIORITY, &delegate, TRAFFIC_ANNOTATION_FOR_TESTS));
        request_a->Start();
        delegate.RunUntilComplete();
...
TEST_P(EmbeddedTestServerTest, LargePost)
{
    ...
    server_->RegisterRequestMonitor(base::BindLambdaForTesting([=](const HttpRequest& request) {
    ...
    auto reader = std::make_unique<UploadBytesElementReader>(base::as_byte_span(large_post_body));
    auto stream = ElementsUploadDataStream::CreateWithReader(std::move(reader));

    TestDelegate delegate;
    std::unique_ptr<URLRequest> request(context_->CreateRequest(server_->GetURL("/test"), DEFAULT_PRIORITY, &delegate, TRAFFIC_ANNOTATION_FOR_TESTS));
...
INSTANTIATE_TEST_SUITE_P(EmbeddedTestServerTestInstantiation, EmbeddedTestServerTest, testing::ValuesIn(EmbeddedTestServerConfigs()));
...
class EmbeddedTestServerThreadingTest : public testing::TestWithParam<ThreadingTestParams>, public WithTaskEnvironment { };

class EmbeddedTestServerThreadingTestDelegate : public base::PlatformThread::Delegate {
public:
    EmbeddedTestServerThreadingTestDelegate(bool message_loop_present_on_initialize, bool message_loop_present_on_shutdown, EmbeddedTestServerConfig config)
    ...
    EmbeddedTestServerThreadingTestDelegate(const EmbeddedTestServerThreadingTestDelegate&) = delete;
    EmbeddedTestServerThreadingTestDelegate& operator=(const EmbeddedTestServerThreadingTestDelegate&) = delete;
    ...
    void ThreadMain() override
    {
        ...
        if (message_loop_present_on_initialize_) {
            executor = std::make_unique<base::SingleThreadTaskExecutor>(base::MessagePumpType::IO);
        }
        ...
        TestDelegate delegate;
        std::unique_ptr<URLRequest> request(context->CreateRequest(server.GetURL("/test?q=foo"), DEFAULT_PRIORITY, &delegate, TRAFFIC_ANNOTATION_FOR_TESTS));
...
TEST_P(EmbeddedTestServerThreadingTest, RunTest)
{
    ...
    EmbeddedTestServerThreadingTestDelegate delegate(std::get<0>(GetParam()), std::get<1>(GetParam()), std::get<2>(GetParam()));
    ASSERT_TRUE(base::PlatformThread::Create(0, &delegate, &thread_handle));
    base::PlatformThread::Join(thread_handle);
}

INSTANTIATE_TEST_SUITE_P(EmbeddedTestServerThreadingTestInstantiation, EmbeddedTestServerThreadingTest,
    testing::Combine(testing::Bool(), testing::Bool(), testing::ValuesIn(EmbeddedTestServerConfigs())));
