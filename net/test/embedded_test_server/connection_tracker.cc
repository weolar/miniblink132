...
void ConnectionTracker::CheckAccepted()
{
    ...
    DCHECK(num_accepted_connections_loop_ || num_accepted_connections_needed_ == 0);
    if (!num_accepted_connections_loop_ || num_accepted_connections_needed_ != num_connected_sockets_) {
        return;
    }
...
ConnectionTracker::ConnectionListener::ConnectionListener(ConnectionTracker* tracker)
...
std::unique_ptr<net::StreamSocket> ConnectionTracker::ConnectionListener::AcceptedSocket(std::unique_ptr<net::StreamSocket> connection)
{
    uint16_t port;
    if (GetPort(*connection, &port)) {
        task_runner_->PostTask(FROM_HERE, base::BindOnce(&ConnectionTracker::AcceptedSocketWithPort, base::Unretained(tracker_), port));
    }
    return connection;
}
...
void ConnectionTracker::ConnectionListener::ReadFromSocket(const net::StreamSocket& connection, int rv)
{
    ...
    uint16_t port;
    if (GetPort(connection, &port)) {
        task_runner_->PostTask(FROM_HERE, base::BindOnce(&ConnectionTracker::ReadFromSocketWithPort, base::Unretained(tracker_), port));
    }
}
