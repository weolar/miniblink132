...
MessageOrError WebSocketMessageAssembler::HandleFrame(bool is_final, WebSocketFrameHeader::OpCode opcode, base::span<const char> payload)
{
    ...
    if (is_final) {
        Message complete_message(is_text_message_, base::make_span(multi_frame_buffer_));
        state_ = MessageState::kFinished;
        return complete_message;
    }

    // Update the state to expect a continuation frame.
    state_ = is_text_message_ ? MessageState::kExpectTextContinuation : MessageState::kExpectBinaryContinuation;
