...
template <class T> using StreamMap = base::flat_map<StreamId, T>;
...
class Http2Connection : public HttpConnection, public http2::adapter::Http2VisitorInterface {
public:
    Http2Connection(std::unique_ptr<StreamSocket> socket, EmbeddedTestServerConnectionListener* connection_listener, EmbeddedTestServer* server_delegate);
    ...
    OnHeaderResult OnHeaderForStream(StreamId stream_id, std::string_view key, std::string_view value) override;
    ...
    bool OnGoAway(StreamId last_accepted_stream_id, http2::adapter::Http2ErrorCode error_code, std::string_view opaque_data) override;
    ...
    bool OnMetadataForStream(StreamId stream_id, std::string_view metadata) override;
