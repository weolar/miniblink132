...
constexpr base::FilePath::CharType kMockHttpHeadersExtension[] = FILE_PATH_LITERAL("mock-http-headers");
...
std::unique_ptr<HttpResponse> HandlePrefixedRequest(
    const std::string& prefix, const EmbeddedTestServer::HandleRequestCallback& handler, const HttpRequest& request)
{
...
RequestQuery ParseQuery(const GURL& url)
{
    ...
    for (QueryIterator it(url); !it.IsAtEnd(); it.Advance()) {
        std::string unescaped_query = base::UnescapeBinaryURLComponent(it.GetKey(), base::UnescapeRule::REPLACE_PLUS_WITH_SPACE);
        queries[unescaped_query].push_back(it.GetUnescapedValue());
    }
    return queries;
}

std::string GetFilePathWithReplacements(const std::string& original_file_path, const base::StringPairs& text_to_replace)
{
...
std::unique_ptr<HttpResponse> HandleFileRequest(const base::FilePath& server_root, const HttpRequest& request)
{
    ...
    base::FilePath headers_path(file_path.AddExtension(kMockHttpHeadersExtension));

    if (base::PathExists(headers_path)) {
        std::string headers_contents;

        if (!base::ReadFileToString(headers_path, &headers_contents) || !UpdateReplacedText(query, &headers_contents)) {
            return nullptr;
        }
    ...
    if (request.headers.find("Range") != request.headers.end()) {
        ...
        if (HttpUtil::ParseRangeHeader(request.headers.at("Range"), &ranges) && ranges.size() == 1) {
            ...
            http_response->AddCustomHeader("Content-Range", base::StringPrintf("bytes %" PRIuS "-%" PRIuS "/%" PRIuS, start, end, file_contents.size()));
