...
class NET_EXPORT_PRIVATE BackendCleanupTracker : public base::RefCounted<BackendCleanupTracker> {
public:
    ...
    // and return null.
    static scoped_refptr<BackendCleanupTracker> TryCreate(const base::FilePath& path, base::OnceClosure retry_closure);
...
private:
    ...
    std::vector<std::pair<scoped_refptr<base::SequencedTaskRunner>, base::OnceClosure>> post_cleanup_cbs_;
