...
// shutting down.
using ApplicationStatusListenerGetter = base::RepeatingCallback<base::android::ApplicationStatusListener*()>;
...
NET_EXPORT void FlushCacheThreadAsynchronouslyForTesting(base::OnceClosure cllback);
...
public:
    ...
    // handle it properly.
    explicit Backend(net::CacheType cache_type)
    ...
    // logic.
    virtual EntryResult OpenOrCreateEntry(const std::string& key, net::RequestPriority priority, EntryResultCallback callback) = 0;
    ...
    virtual EntryResult OpenEntry(const std::string& key, net::RequestPriority priority, EntryResultCallback) = 0;
    ...
    // entry is available. The |priority| of the entry determines its priority in
    // the background worker pools.
    virtual EntryResult CreateEntry(const std::string& key, net::RequestPriority priority, EntryResultCallback callback) = 0;
    ...
    virtual net::Error DoomEntry(const std::string& key, net::RequestPriority priority, CompletionOnceCallback callback) = 0;
    ...
    virtual net::Error DoomEntriesBetween(base::Time initial_time, base::Time end_time, CompletionOnceCallback callback) = 0;

    // Marks all entries accessed since |initial_time| for deletion. The return
    // value is a net error code. If this method returns ERR_IO_PENDING, the
    // |callback| will be invoked when the operation completes.
    // Entries with |initial_time| <= access time are deleted.
    virtual net::Error DoomEntriesSince(base::Time initial_time, CompletionOnceCallback callback) = 0;
    ...
    virtual int64_t CalculateSizeOfAllEntries(Int64CompletionOnceCallback callback) = 0;
    ...
    virtual int64_t CalculateSizeOfEntriesBetween(base::Time initial_time, base::Time end_time, Int64CompletionOnceCallback callback);
...
public:
    ...
    virtual int ReadData(int index, int offset, IOBuffer* buf, int buf_len, CompletionOnceCallback callback) = 0;
    ...
    virtual int WriteData(int index, int offset, IOBuffer* buf, int buf_len, CompletionOnceCallback callback, bool truncate) = 0;
    ...
    // entries.
    virtual int ReadSparseData(int64_t offset, IOBuffer* buf, int buf_len, CompletionOnceCallback callback) = 0;
    ...
    // re-created.
    virtual int WriteSparseData(int64_t offset, IOBuffer* buf, int buf_len, CompletionOnceCallback callback) = 0;
    ...
    virtual RangeResult GetAvailableRange(int64_t offset, int len, RangeResultCallback callback) = 0;
...
struct NET_EXPORT RangeResult {
    RangeResult() = default;
    explicit RangeResult(net::Error error)
        : net_error(error)
    {
    }
...
public:
    struct FileEnumerationEntry {
        ...
        FileEnumerationEntry(base::FilePath path, int64_t size, base::Time last_accessed, base::Time last_modified)
            : path(std::move(path))
            , size(size)
    ...
    virtual bool DeleteFile(const base::FilePath& path, DeleteFileMode mode = DeleteFileMode::kDefault) = 0;

    // Renames a file `from_path` to `to_path`. Returns the error information.
    virtual bool ReplaceFile(const base::FilePath& from_path, const base::FilePath& to_path, base::File::Error* error) = 0;

    // Returns information about the given path.
    virtual std::optional<base::File::Info> GetFileInfo(const base::FilePath& path) = 0;

    // Creates an object that can be used to enumerate files in the specified
    // directory.
    virtual std::unique_ptr<FileEnumerator> EnumerateFiles(const base::FilePath& path) = 0;
    ...
    virtual void CleanupDirectory(const base::FilePath& path, base::OnceCallback<void(bool)> callback) = 0;
...
public:
    ...
    virtual std::unique_ptr<BackendFileOperations> Bind(scoped_refptr<base::SequencedTaskRunner> task_runner) = 0;
};

// A factory interface that creates BackendFileOperations.
class BackendFileOperationsFactory : public base::RefCounted<BackendFileOperationsFactory> {
public:
    // Creates a BackendFileOperations which is bound to `task_runner`.
    virtual std::unique_ptr<BackendFileOperations> Create(scoped_refptr<base::SequencedTaskRunner> task_runner) = 0;
...
public:
    ...
    bool ReplaceFile(const base::FilePath& from_path, const base::FilePath& to_path, base::File::Error* error) override;
    std::optional<base::File::Info> GetFileInfo(const base::FilePath& path) override;
    std::unique_ptr<FileEnumerator> EnumerateFiles(const base::FilePath& path) override;
    void CleanupDirectory(const base::FilePath& path, base::OnceCallback<void(bool)> callback) override;
...
class NET_EXPORT TrivialFileOperationsFactory : public BackendFileOperationsFactory {
public:
    TrivialFileOperationsFactory();

    // BackendFileOperationsFactory implementation:
    std::unique_ptr<BackendFileOperations> Create(scoped_refptr<base::SequencedTaskRunner> task_runner) override;
