...
protected:
    ...
    void CreateEntryWithHeaderBodyAndSideData(const std::string& key, int data_size);
    void TruncateFileFromEnd(int file_index, const std::string& key, int data_size, int truncate_size);
...
void DiskCacheEntryTest::InternalSyncIOBackground(disk_cache::Entry* entry)
{
    ...
    EXPECT_EQ(0, entry->ReadData(0, 0, buffer1.get(), kSize1, net::CompletionOnceCallback()));
    base::strlcpy(buffer1->data(), "the data", kSize1);
    EXPECT_EQ(10, entry->WriteData(0, 0, buffer1.get(), kSize1, net::CompletionOnceCallback(), false));
    memset(buffer1->data(), 0, kSize1);
    EXPECT_EQ(10, entry->ReadData(0, 0, buffer1.get(), kSize1, net::CompletionOnceCallback()));
    ...
    EXPECT_EQ(5000, entry->WriteData(1, 1500, buffer2.get(), kSize2, net::CompletionOnceCallback(), false));
    memset(buffer2->data(), 0, kSize2);
    EXPECT_EQ(4989, entry->ReadData(1, 1511, buffer2.get(), kSize2, net::CompletionOnceCallback()));
    EXPECT_STREQ("big data goes here", buffer2->data());
    EXPECT_EQ(5000, entry->ReadData(1, 0, buffer2.get(), kSize2, net::CompletionOnceCallback()));
    EXPECT_EQ(0, memcmp(buffer2->data(), buffer3->data(), 1500));
    EXPECT_EQ(1500, entry->ReadData(1, 5000, buffer2.get(), kSize2, net::CompletionOnceCallback()));

    EXPECT_EQ(0, entry->ReadData(1, 6500, buffer2.get(), kSize2, net::CompletionOnceCallback()));
    EXPECT_EQ(6500, entry->ReadData(1, 0, buffer3.get(), kSize3, net::CompletionOnceCallback()));
    EXPECT_EQ(8192, entry->WriteData(1, 0, buffer3.get(), 8192, net::CompletionOnceCallback(), false));
    EXPECT_EQ(8192, entry->ReadData(1, 0, buffer3.get(), kSize3, net::CompletionOnceCallback()));
    EXPECT_EQ(8192, entry->GetDataSize(1));

    // We need to delete the memory buffer on this thread.
    EXPECT_EQ(0, entry->WriteData(0, 0, nullptr, 0, net::CompletionOnceCallback(), true));
    EXPECT_EQ(0, entry->WriteData(1, 0, nullptr, 0, net::CompletionOnceCallback(), true));
}
...
void DiskCacheEntryTest::InternalSyncIO()
{
    ...
    RunTaskForTest(base::BindOnce(&DiskCacheEntryTest::InternalSyncIOBackground, base::Unretained(this), entry));
...
void DiskCacheEntryTest::InternalAsyncIO()
{
    ...
    EXPECT_EQ(0, entry->ReadData(0, 15 * 1024, buffer1.get(), kSize1, base::BindOnce(&CallbackTest::Run, base::Unretained(&callback1))));
    base::strlcpy(buffer1->data(), "the data", kSize1);
    int expected = 0;
    int ret = entry->WriteData(0, 0, buffer1.get(), kSize1, base::BindOnce(&CallbackTest::Run, base::Unretained(&callback2)), false);
    ...
    memset(buffer2->data(), 0, kSize2);
    ret = entry->ReadData(0, 0, buffer2.get(), kSize1, base::BindOnce(&CallbackTest::Run, base::Unretained(&callback3)));
    EXPECT_TRUE(10 == ret || net::ERR_IO_PENDING == ret);
    if (net::ERR_IO_PENDING == ret)
        expected++;
    ...
    base::strlcpy(buffer2->data(), "The really big data goes here", kSize2);
    ret = entry->WriteData(1, 1500, buffer2.get(), kSize2, base::BindOnce(&CallbackTest::Run, base::Unretained(&callback4)), true);
    ...
    EXPECT_TRUE(helper.WaitUntilCacheIoFinished(expected));
    memset(buffer3->data(), 0, kSize3);
    ret = entry->ReadData(1, 1511, buffer3.get(), kSize2, base::BindOnce(&CallbackTest::Run, base::Unretained(&callback5)));
    ...
    ret = entry->ReadData(1, 0, buffer2.get(), kSize2, base::BindOnce(&CallbackTest::Run, base::Unretained(&callback6)));
    EXPECT_TRUE(5000 == ret || net::ERR_IO_PENDING == ret);
    if (net::ERR_IO_PENDING == ret)
        expected++;
    ...
    EXPECT_TRUE(helper.WaitUntilCacheIoFinished(expected));
    EXPECT_EQ(0, memcmp(buffer2->data(), buffer3->data(), 1500));
    ret = entry->ReadData(1, 5000, buffer2.get(), kSize2, base::BindOnce(&CallbackTest::Run, base::Unretained(&callback7)));
    EXPECT_TRUE(1500 == ret || net::ERR_IO_PENDING == ret);
    if (net::ERR_IO_PENDING == ret)
        expected++;

    ret = entry->ReadData(1, 0, buffer3.get(), kSize3, base::BindOnce(&CallbackTest::Run, base::Unretained(&callback9)));
    EXPECT_TRUE(6500 == ret || net::ERR_IO_PENDING == ret);
    if (net::ERR_IO_PENDING == ret)
        expected++;

    ret = entry->WriteData(1, 0, buffer3.get(), 8192, base::BindOnce(&CallbackTest::Run, base::Unretained(&callback10)), true);
    EXPECT_TRUE(8192 == ret || net::ERR_IO_PENDING == ret);
    if (net::ERR_IO_PENDING == ret)
        expected++;

    EXPECT_TRUE(helper.WaitUntilCacheIoFinished(expected));
    ret = entry->ReadData(1, 0, buffer3.get(), kSize3, base::BindOnce(&CallbackTest::Run, base::Unretained(&callback11)));
    EXPECT_TRUE(8192 == ret || net::ERR_IO_PENDING == ret);
    if (net::ERR_IO_PENDING == ret)
        expected++;
    ...
    ret = entry->ReadData(0, 0, buffer1.get(), kSize1, base::BindOnce(&CallbackTest::Run, base::Unretained(&callback12)));
    EXPECT_TRUE(10 == ret || net::ERR_IO_PENDING == ret);
    if (net::ERR_IO_PENDING == ret)
        expected++;

    ret = entry->ReadData(1, 0, buffer2.get(), kSize2, base::BindOnce(&CallbackTest::Run, base::Unretained(&callback13)));
    EXPECT_TRUE(5000 == ret || net::ERR_IO_PENDING == ret);
    if (net::ERR_IO_PENDING == ret)
        expected++;
...
void DiskCacheEntryTest::ExternalSyncIOBackground(disk_cache::Entry* entry)
{
    ...
    CacheTestFillBuffer(buffer2->data(), kSize2, false);
    base::strlcpy(buffer1->data(), "the data", kSize1);
    EXPECT_EQ(17000, entry->WriteData(0, 0, buffer1.get(), kSize1, net::CompletionOnceCallback(), false));
    memset(buffer1->data(), 0, kSize1);
    EXPECT_EQ(17000, entry->ReadData(0, 0, buffer1.get(), kSize1, net::CompletionOnceCallback()));
    EXPECT_STREQ("the data", buffer1->data());

    base::strlcpy(buffer2->data(), "The really big data goes here", kSize2);
    EXPECT_EQ(25000, entry->WriteData(1, 10000, buffer2.get(), kSize2, net::CompletionOnceCallback(), false));
    memset(buffer2->data(), 0, kSize2);
    EXPECT_EQ(24989, entry->ReadData(1, 10011, buffer2.get(), kSize2, net::CompletionOnceCallback()));
    EXPECT_STREQ("big data goes here", buffer2->data());
    EXPECT_EQ(25000, entry->ReadData(1, 0, buffer2.get(), kSize2, net::CompletionOnceCallback()));
    EXPECT_EQ(5000, entry->ReadData(1, 30000, buffer2.get(), kSize2, net::CompletionOnceCallback()));

    EXPECT_EQ(0, entry->ReadData(1, 35000, buffer2.get(), kSize2, net::CompletionOnceCallback()));
    EXPECT_EQ(17000, entry->ReadData(1, 0, buffer1.get(), kSize1, net::CompletionOnceCallback()));
    EXPECT_EQ(17000, entry->WriteData(1, 20000, buffer1.get(), kSize1, net::CompletionOnceCallback(), false));
...
void DiskCacheEntryTest::ExternalSyncIO()
{
    ...
    // The bulk of the test runs from within the callback, on the cache thread.
    RunTaskForTest(base::BindOnce(&DiskCacheEntryTest::ExternalSyncIOBackground, base::Unretained(this), entry));
...
void DiskCacheEntryTest::ExternalAsyncIO()
{
    ...
    CacheTestFillBuffer(buffer3->data(), kSize3, false);
    base::strlcpy(buffer1->data(), "the data", kSize1);
    int ret = entry->WriteData(0, 0, buffer1.get(), kSize1, base::BindOnce(&CallbackTest::Run, base::Unretained(&callback1)), false);
    ...
    ret = entry->ReadData(0, 0, buffer2.get(), kSize1, base::BindOnce(&CallbackTest::Run, base::Unretained(&callback2)));
    EXPECT_TRUE(17000 == ret || net::ERR_IO_PENDING == ret);
    if (net::ERR_IO_PENDING == ret)
        expected++;
    ...
    base::strlcpy(buffer2->data(), "The really big data goes here", kSize2);
    ret = entry->WriteData(1, 10000, buffer2.get(), kSize2, base::BindOnce(&CallbackTest::Run, base::Unretained(&callback3)), false);
    ...
    memset(buffer3->data(), 0, kSize3);
    ret = entry->ReadData(1, 10011, buffer3.get(), kSize3, base::BindOnce(&CallbackTest::Run, base::Unretained(&callback4)));
    ...
    EXPECT_TRUE(helper.WaitUntilCacheIoFinished(expected));
    EXPECT_STREQ("big data goes here", buffer3->data());
    ret = entry->ReadData(1, 0, buffer2.get(), kSize2, base::BindOnce(&CallbackTest::Run, base::Unretained(&callback5)));
    EXPECT_TRUE(25000 == ret || net::ERR_IO_PENDING == ret);
    if (net::ERR_IO_PENDING == ret)
        expected++;
    ...
    ret = entry->ReadData(1, 30000, buffer2.get(), kSize2, base::BindOnce(&CallbackTest::Run, base::Unretained(&callback6)));
    EXPECT_TRUE(5000 == ret || net::ERR_IO_PENDING == ret);
    if (net::ERR_IO_PENDING == ret)
        expected++;

    ret = entry->ReadData(1, 35000, buffer2.get(), kSize2, base::BindOnce(&CallbackTest::Run, base::Unretained(&callback7)));
    EXPECT_TRUE(0 == ret || net::ERR_IO_PENDING == ret);
    if (net::ERR_IO_PENDING == ret)
        expected++;

    ret = entry->ReadData(1, 0, buffer1.get(), kSize1, base::BindOnce(&CallbackTest::Run, base::Unretained(&callback8)));
    EXPECT_TRUE(17000 == ret || net::ERR_IO_PENDING == ret);
    if (net::ERR_IO_PENDING == ret)
        expected++;
    ret = entry->WriteData(1, 20000, buffer3.get(), kSize1, base::BindOnce(&CallbackTest::Run, base::Unretained(&callback9)), false);
    EXPECT_TRUE(17000 == ret || net::ERR_IO_PENDING == ret);
    if (net::ERR_IO_PENDING == ret)
        expected++;
...
void DiskCacheEntryTest::ReleaseBuffer(int stream_index)
{
    ...
    int rv = entry->WriteData(stream_index, 0, buffer.get(), kBufferSize, cb.callback(), false);
...
void DiskCacheEntryTest::StreamAccess()
{
    ...
    for (int i = 0; i < kNumStreams; i++) {
        EXPECT_EQ(kBufferSize, WriteData(entry, i, 0, reference_buffers[i].get(), kBufferSize, false));
    ...
    EXPECT_EQ(net::ERR_INVALID_ARGUMENT, ReadData(entry, kNumStreams, 0, buffer1.get(), kBufferSize));
    entry->Close();
    ...
    static_assert(kFinalReadSize < kReadBufferSize, "should be exactly two reads");
    auto buffer2 = base::MakeRefCounted<net::IOBufferWithSize>(kReadBufferSize);
    for (int i = 0; i < kNumStreams; i++) {
        memset(buffer2->data(), 0, kReadBufferSize);
        EXPECT_EQ(kReadBufferSize, ReadData(entry, i, 0, buffer2.get(), kReadBufferSize));
        EXPECT_EQ(0, memcmp(reference_buffers[i]->data(), buffer2->data(), kReadBufferSize));

        memset(buffer2->data(), 0, kReadBufferSize);
        EXPECT_EQ(kFinalReadSize, ReadData(entry, i, kReadBufferSize, buffer2.get(), kReadBufferSize));
        EXPECT_EQ(0, memcmp(reference_buffers[i]->data() + kReadBufferSize, buffer2->data(), kFinalReadSize));
    }
...
void DiskCacheEntryTest::GrowData(int stream_index)
{
    ...
    EXPECT_EQ(2000, WriteData(entry, stream_index, 0, buffer1.get(), 2000, false));
    ...
    EXPECT_EQ(20000, WriteData(entry, stream_index, 0, buffer1.get(), kSize, false));
...
void DiskCacheEntryTest::TruncateData(int stream_index)
{
    ...
    EXPECT_EQ(20000, WriteData(entry, stream_index, 0, buffer1.get(), 20000, true));
    EXPECT_EQ(20000, entry->GetDataSize(stream_index));
    ...
    EXPECT_EQ(18000, WriteData(entry, stream_index, 0, buffer1.get(), 18000, false));
    EXPECT_EQ(20000, entry->GetDataSize(stream_index));
    EXPECT_EQ(18000, WriteData(entry, stream_index, 0, buffer1.get(), 18000, true));
    ...
    EXPECT_EQ(600, WriteData(entry, stream_index, 1000, buffer1.get(), 600, true));
    ...
    EXPECT_TRUE(!memcmp(buffer1->data(), buffer2->data(), 1000)) << "Preserves previous data";
...
void DiskCacheEntryTest::SizeChanges(int stream_index)
{
    ...
    EXPECT_EQ(kSize, WriteData(entry, stream_index, 0, buffer1.get(), kSize, true));
    EXPECT_EQ(kSize, WriteData(entry, stream_index, 17000, buffer1.get(), kSize, true));
    EXPECT_EQ(kSize, WriteData(entry, stream_index, 23000, buffer1.get(), kSize, true));
    entry->Close();

    // Extend the file and read between the old size and the new write.
    ASSERT_THAT(OpenEntry(key, &entry), IsOk());
    EXPECT_EQ(23000 + kSize, entry->GetDataSize(stream_index));
    EXPECT_EQ(kSize, WriteData(entry, stream_index, 25000, buffer1.get(), kSize, true));
    ...
    EXPECT_EQ(kSize, ReadData(entry, stream_index, 23000 + kSize - 35, buffer2.get(), kSize));
    ...
    EXPECT_EQ(kSize, WriteData(entry, stream_index, 26000, buffer1.get(), kSize, true));
    ...
    EXPECT_EQ(kSize, WriteData(entry, stream_index, 25000, buffer1.get(), kSize, true));
    EXPECT_EQ(25000 + kSize, entry->GetDataSize(stream_index));
    EXPECT_EQ(28, ReadData(entry, stream_index, 25000 + kSize - 28, buffer2.get(), kSize));
    EXPECT_TRUE(!memcmp(buffer2->data(), buffer1->data() + kSize - 28, 28));

    // Reduce the size with a buffer that is not extending the size.
    EXPECT_EQ(kSize, WriteData(entry, stream_index, 24000, buffer1.get(), kSize, false));
    EXPECT_EQ(25000 + kSize, entry->GetDataSize(stream_index));
    EXPECT_EQ(kSize, WriteData(entry, stream_index, 24500, buffer1.get(), kSize, true));
    ...
    EXPECT_EQ(kSize, WriteData(entry, stream_index, 19000, buffer1.get(), kSize, true));
    ...
    EXPECT_EQ(0, WriteData(entry, stream_index, 20000 + kSize, buffer1.get(), 0, false));
    EXPECT_EQ(kSize, ReadData(entry, stream_index, 19000 + kSize, buffer1.get(), kSize));
...
void DiskCacheEntryTest::ReuseEntry(int size, int stream_index)
{
    ...
    for (int i = 0; i < 15; i++) {
        ...
        EXPECT_EQ(size, WriteData(entry, stream_index, 0, buffer.get(), size, false));
        entry->Close();
        ASSERT_THAT(OpenEntry(key2, &entry), IsOk());
    }
...
void DiskCacheEntryTest::InvalidData(int stream_index)
{
    ...
    // Simple data grow:
    EXPECT_EQ(200, WriteData(entry, stream_index, 400, buffer1.get(), 200, false));
    ...
    EXPECT_EQ(200, WriteData(entry, stream_index, 800, buffer1.get(), 200, false));
    EXPECT_EQ(1000, entry->GetDataSize(stream_index));
    ...
    EXPECT_EQ(200, WriteData(entry, stream_index, 1800, buffer1.get(), 200, true));
    EXPECT_EQ(2000, entry->GetDataSize(stream_index));
    EXPECT_EQ(100, ReadData(entry, stream_index, 1500, buffer3.get(), 100));
    EXPECT_TRUE(!memcmp(buffer3->data(), buffer2->data(), 100));

    // Go to an external file.
    EXPECT_EQ(200, WriteData(entry, stream_index, 19800, buffer1.get(), 200, false));
    EXPECT_EQ(20000, entry->GetDataSize(stream_index));
    ...
    // Extend it again.
    EXPECT_EQ(600, WriteData(entry, stream_index, 2000, buffer1.get(), 600, false));
    ...
    EXPECT_EQ(600, WriteData(entry, stream_index, 3000, buffer1.get(), 600, true));
...
void DiskCacheEntryTest::ReadWriteDestroyBuffer(int stream_index)
{
    ...
    EXPECT_EQ(net::ERR_IO_PENDING, entry->WriteData(stream_index, 0, buffer.get(), kSize, cb.callback(), false));
    ...
    EXPECT_EQ(net::ERR_IO_PENDING, entry->ReadData(stream_index, 0, buffer.get(), kSize, cb.callback()));
    buffer = nullptr;
    EXPECT_EQ(kSize, cb.WaitForResult());
...
TEST_F(DiskCacheEntryTest, MissingData)
{
    ...
    ASSERT_THAT(OpenEntry(key, &entry), IsOk());
    EXPECT_EQ(net::ERR_FILE_NOT_FOUND, ReadData(entry, 0, 0, buffer.get(), kSize));
    entry->Close();
...
TEST_F(DiskCacheEntryTest, MemoryOnlyEnumerationWithSparseEntries)
{
    ...
    EXPECT_EQ(kSize, parent_entry->WriteSparseData(0, buf.get(), kSize, net::CompletionOnceCallback()));

    // This write creates a child entry and writes to it.
    EXPECT_EQ(kSize, parent_entry->WriteSparseData(8192, buf.get(), kSize, net::CompletionOnceCallback()));
...
void VerifySparseIO(disk_cache::Entry* entry, int64_t offset, net::IOBuffer* buf_1, int size, net::IOBuffer* buf_2)
{
    net::TestCompletionCallback cb;
...
void VerifyContentSparseIO(disk_cache::Entry* entry, int64_t offset, char* buffer, int size)
{
    net::TestCompletionCallback cb;
...
void DiskCacheEntryTest::GetAvailableRangeTest()
{
    ...
    RangeResult result = cb.GetResult(entry->GetAvailableRange(0x20F0000, kSize * 2, cb.callback()));
    ...
    result = cb.GetResult(entry->GetAvailableRange(0x20F0000 - kSize, kSize, cb.callback()));
    EXPECT_EQ(net::OK, result.net_error);
    EXPECT_EQ(0, result.available_len);
    ...
    result = cb.GetResult(entry->GetAvailableRange(0x2100000, kSize, cb.callback()));
    EXPECT_EQ(net::OK, result.net_error);
    EXPECT_EQ(0, result.available_len);
    int rv = entry->ReadSparseData(result.start, buf.get(), kSize, read_cb.callback());
    EXPECT_EQ(0, read_cb.GetResult(rv));

    result = cb.GetResult(entry->GetAvailableRange(0x20F2000, kSize, cb.callback()));
    EXPECT_EQ(net::OK, result.net_error);
    ...
    result = cb.GetResult(entry->GetAvailableRange(0x20F0001 - kSize, kSize, cb.callback()));
    EXPECT_EQ(net::OK, result.net_error);
    ...
    result = cb.GetResult(entry->GetAvailableRange(kTinyLen * 2, kTinyLen, cb.callback()));
    EXPECT_EQ(net::OK, result.net_error);
    EXPECT_EQ(0, result.available_len);
    EXPECT_EQ(kTinyLen * 2, result.start);

    // Get a huge range with maximum boundary
    result = cb.GetResult(entry->GetAvailableRange(0x2100000, std::numeric_limits<int32_t>::max(), cb.callback()));
    EXPECT_EQ(net::OK, result.net_error);
    EXPECT_EQ(0, result.available_len);
...
TEST_F(DiskCacheEntryTest, GetAvailableRangeBlockFileDiscontinuous)
{
    ...
    EXPECT_EQ(2 * 1024,
        WriteSparseData(entry, /* offset = */ 1024, buf_2k.get(),
            /* size = */ 2 * 1024));

    // Now record a partial write from start of the first kb.
    EXPECT_EQ(kSmallSize, WriteSparseData(entry, /* offset = */ 0, buf_small.get(), kSmallSize));
    ...
    TestRangeResultCompletionCallback cb;
    RangeResult result = cb.GetResult(entry->GetAvailableRange(812, 1247, cb.callback()));
    EXPECT_EQ(net::OK, result.net_error);
    ...
    // as the last write.
    EXPECT_EQ(kSmallSize, WriteSparseData(entry, /* offset = */ 3072, buf_small.get(), kSmallSize));
    ...
    EXPECT_EQ(kSmallSize, WriteSparseData(entry, /* offset = */ 4096, buf_small.get(), kSmallSize));
    ...
    // computed.
    EXPECT_EQ(kSmallSize, WriteSparseData(entry, /* offset = */ 0x200400, buf_small.get(), kSmallSize));
    result = cb.GetResult(entry->GetAvailableRange(0x100000, 0x200000, cb.callback()));
    EXPECT_EQ(net::OK, result.net_error);
...
TEST_F(DiskCacheEntryTest, SparseWriteDropped)
{
    ...
    for (int i = 0; i < 5; i++) {
        ...
        result = range_cb.GetResult(entry->GetAvailableRange(offset - 100, kSize, range_cb.callback()));
        EXPECT_EQ(net::OK, result.net_error);
        EXPECT_EQ(0, result.available_len);

        result = range_cb.GetResult(entry->GetAvailableRange(offset, kSize, range_cb.callback()));
...
TEST_F(DiskCacheEntryTest, MemoryOnlyMisalignedSparseIO)
{
    ...
    for (int i = 0; i < kSize; i += 1024) {
        auto buf_3 = base::MakeRefCounted<net::WrappedIOBuffer>(buf_1->span().subspan(i));
...
TEST_F(DiskCacheEntryTest, MemoryOnlyMisalignedGetAvailableRange)
{
    ...
    EXPECT_EQ(1024, entry->WriteSparseData(0, buf.get(), 1024, net::CompletionOnceCallback()));
    EXPECT_EQ(1024, entry->WriteSparseData(5120, buf.get(), 1024, net::CompletionOnceCallback()));
    EXPECT_EQ(1024, entry->WriteSparseData(10000, buf.get(), 1024, net::CompletionOnceCallback()));

    // Writes in the middle of an entry and spans 2 child entries.
    EXPECT_EQ(8192, entry->WriteSparseData(50000, buf.get(), 8192, net::CompletionOnceCallback()));

    TestRangeResultCompletionCallback cb;
    // Test that we stop at a discontinuous child at the second block.
    RangeResult result = cb.GetResult(entry->GetAvailableRange(0, 10000, cb.callback()));
    EXPECT_EQ(net::OK, result.net_error);
    EXPECT_EQ(1024, result.available_len);
...
public:
    explicit SparseTestCompletionCallback(std::unique_ptr<disk_cache::Backend> cache)
        : cache_(std::move(cache))
    {
    }

    SparseTestCompletionCallback(const SparseTestCompletionCallback&) = delete;
    SparseTestCompletionCallback& operator=(const SparseTestCompletionCallback&) = delete;
...
TEST_F(DiskCacheEntryTest, DoomSparseEntry2)
{
    ...
    // Write to a bunch of ranges.
    for (int i = 0; i < 12; i++) {
        EXPECT_EQ(kSize, entry->WriteSparseData(offset, buf.get(), kSize, net::CompletionOnceCallback()));
        offset *= 4;
    }
...
void DiskCacheEntryTest::PartialSparseEntry()
{
    ...
    EXPECT_EQ(kSmallSize, WriteSparseData(entry, 1080321, buf1.get(), kSmallSize));
    entry->Close();
    ASSERT_THAT(OpenEntry(key, &entry), IsOk());
    ...
    result = cb.GetResult(entry->GetAvailableRange(20 * 1024, 10000, cb.callback()));
    EXPECT_EQ(net::OK, result.net_error);
    if (memory_only_ || simple_cache_mode_)
    ...
    if (memory_only_ || simple_cache_mode_) {
        result = cb.GetResult(entry->GetAvailableRange(19400, kSize, cb.callback()));
        EXPECT_EQ(net::OK, result.net_error);
...
void DiskCacheEntryTest::SparseInvalidArg()
{
    ...
    EXPECT_EQ(net::ERR_INVALID_ARGUMENT, WriteSparseData(entry, -1, buf.get(), kSize));
    EXPECT_EQ(net::ERR_INVALID_ARGUMENT, WriteSparseData(entry, 0, buf.get(), -1));

    EXPECT_EQ(net::ERR_INVALID_ARGUMENT, ReadSparseData(entry, -1, buf.get(), kSize));
    EXPECT_EQ(net::ERR_INVALID_ARGUMENT, ReadSparseData(entry, 0, buf.get(), -1));

    int64_t start_out;
    EXPECT_EQ(net::ERR_INVALID_ARGUMENT, GetAvailableRange(entry, -1, kSize, &start_out));
    EXPECT_EQ(net::ERR_INVALID_ARGUMENT, GetAvailableRange(entry, 0, -1, &start_out));

    int rv = WriteSparseData(entry, std::numeric_limits<int64_t>::max() - kSize + 1, buf.get(), kSize);
    ...
    EXPECT_TRUE(rv == net::ERR_INVALID_ARGUMENT || rv == net::ERR_CACHE_OPERATION_NOT_SUPPORTED);
...
void DiskCacheEntryTest::SparseClipEnd(int64_t max_index, bool expect_unsupported)
{
    std::string key("key");
    disk_cache::Entry* entry = nullptr;
    ASSERT_THAT(CreateEntry(key, &entry), IsOk());
    ...
    EXPECT_EQ(rv, expect_unsupported ? net::ERR_CACHE_OPERATION_NOT_SUPPORTED : kSize);
    ...
    TestRangeResultCompletionCallback cb;
    RangeResult result = cb.GetResult(entry->GetAvailableRange(kOffset - kSize, kSize * 3, cb.callback()));
    if (expect_unsupported) {
...
TEST_F(DiskCacheEntryTest, SparseClipEnd2)
{
    ...
    TestRangeResultCompletionCallback cb;
    RangeResult result = cb.GetResult(entry->GetAvailableRange(kLimit, kSize * 3, cb.callback()));
    EXPECT_EQ(net::OK, result.net_error);
    EXPECT_EQ(0, result.available_len);
    entry->Close();
}
...
TEST_F(DiskCacheEntryTest, CancelSparseIO)
{
    ...
    int rv = entry->WriteSparseData(1024 * 1024 - 4096, buf.get(), kSize, cb1.callback());
    EXPECT_THAT(rv, IsError(net::ERR_IO_PENDING));

    TestRangeResultCompletionCallback cb5;
    RangeResult result = cb5.GetResult(entry->GetAvailableRange(0, kSize, cb5.callback()));
    if (!cb1.have_result()) {
    ...
    EXPECT_THAT(entry->ReadyForSparseIO(cb2.callback()), IsError(net::ERR_IO_PENDING));
    EXPECT_THAT(entry->ReadyForSparseIO(cb3.callback()), IsError(net::ERR_IO_PENDING));
    entry->CancelSparseIO(); // Should be a no op at this point.
    EXPECT_THAT(entry->ReadyForSparseIO(cb4.callback()), IsError(net::ERR_IO_PENDING));

    if (!cb1.have_result()) {
        EXPECT_EQ(net::ERR_CACHE_OPERATION_NOT_SUPPORTED, entry->ReadSparseData(result.start, buf.get(), kSize, net::CompletionOnceCallback()));
        EXPECT_EQ(net::ERR_CACHE_OPERATION_NOT_SUPPORTED, entry->WriteSparseData(result.start, buf.get(), kSize, net::CompletionOnceCallback()));
    }
    ...
    result = cb5.GetResult(entry->GetAvailableRange(result.start, kSize, cb5.callback()));
    EXPECT_EQ(net::OK, result.net_error);
    EXPECT_EQ(0, result.available_len);
    entry->Close();
}
...
TEST_F(DiskCacheEntryTest, KeySanityCheck3)
{
    ...
    auto key_file = base::MakeRefCounted<disk_cache::File>(true /* want sync ops*/);
...
TEST_F(DiskCacheEntryTest, SimpleCacheGiantEntry)
{
...

    {
        ...
        EXPECT_EQ(kBufSize, WriteData(entry1, 1 /* stream */, kSize1 - kBufSize, buffer.get(), kBufSize, true /* truncate */));
        entry1->Close();
    }
    ...
    // ... but not bigger than that.
    {
        ...
        EXPECT_EQ(net::ERR_FAILED, WriteData(entry2, 1 /* stream */, kSize2 - kBufSize, buffer.get(), kBufSize, true /* truncate */));
        entry2->Close();
    }
}
...
bool DiskCacheEntryTest::SimpleCacheMakeBadChecksumEntry(const std::string& key, int data_size)
{
    disk_cache::Entry* entry = nullptr;
    ...
    base::FilePath entry_file0_path = cache_path_.AppendASCII(disk_cache::simple_util::GetFilenameFromKeyAndFileIndex(key, 0));
    base::File entry_file0(entry_file0_path, base::File::FLAG_WRITE | base::File::FLAG_OPEN);
    if (!entry_file0.IsValid())
        return false;

    int64_t file_offset = sizeof(disk_cache::SimpleFileHeader) + key.size() + data_size - 2;
...
TEST_F(DiskCacheEntryTest, SimpleCacheBadChecksum)
{
    ...
    EXPECT_EQ(net::ERR_CACHE_CHECKSUM_MISMATCH, ReadData(entry, 1, 0, read_buffer.get(), kLargeSize));
}
...
TEST_F(DiskCacheEntryTest, SimpleCacheCreateAfterDiskLayerDoom)
{
    ...
    base::FilePath entry_file1_path = cache_path_.AppendASCII(disk_cache::simple_util::GetFilenameFromKeyAndFileIndex(key, 1));
    base::File entry_file1(entry_file1_path, base::File::FLAG_WRITE | base::File::FLAG_CREATE);
...
TEST_F(DiskCacheEntryTest, SimpleCacheDoomErrorRace)
{
    ...
    base::FilePath entry_file1_path = cache_path_.AppendASCII(disk_cache::simple_util::GetFilenameFromKeyAndFileIndex(kKey, 1));
    base::File entry_file1(entry_file1_path, base::File::FLAG_WRITE | base::File::FLAG_CREATE);
    ASSERT_TRUE(entry_file1.IsValid());
...
TEST_F(DiskCacheEntryTest, SimpleCacheNoEOF)
{
    ...
    const base::FilePath entry_path = cache_path_.AppendASCII(disk_cache::simple_util::GetFilenameFromKeyAndFileIndex(key, 0));
    const int64_t invalid_size = disk_cache::simple_util::GetFileSizeFromDataSize(key.size(), kTruncationBytes);
...
TEST_F(DiskCacheEntryTest, SimpleCacheNonOptimisticOperationsBasic)
{
    ...
    scoped_refptr<net::IOBufferWithSize> write_buffer = base::MakeRefCounted<net::IOBufferWithSize>(kBufferSize);
    CacheTestFillBuffer(write_buffer->data(), write_buffer->size(), false);
    EXPECT_EQ(write_buffer->size(), WriteData(entry, 1, 0, write_buffer.get(), write_buffer->size(), false));

    scoped_refptr<net::IOBufferWithSize> read_buffer = base::MakeRefCounted<net::IOBufferWithSize>(kBufferSize);
    EXPECT_EQ(read_buffer->size(), ReadData(entry, 1, 0, read_buffer.get(), read_buffer->size()));
}
...
TEST_F(DiskCacheEntryTest, SimpleCacheNonOptimisticOperationsDontBlock)
{
    ...
    int ret = entry->WriteData(1, 0, write_buffer.get(), write_buffer->size(), base::BindOnce(&CallbackTest::Run, base::Unretained(&write_callback)), false);
    ASSERT_THAT(ret, IsError(net::ERR_IO_PENDING));
    helper.WaitUntilCacheIoFinished(++expected_callback_runs);
}

TEST_F(DiskCacheEntryTest, SimpleCacheNonOptimisticOperationsBasicsWithoutWaiting)
{
    // Test sequence:
    // Create, Write, Read, Close.
    SetCacheType(net::APP_CACHE); // APP_CACHE doesn't use optimistic operations.
    ...
    ret = entry->ReadData(1, 0, read_buffer.get(), read_buffer->size(), base::BindOnce(&CallbackTest::Run, base::Unretained(&read_callback)));
    EXPECT_THAT(ret, IsError(net::ERR_IO_PENDING));
    ++expected_callback_runs;

    helper.WaitUntilCacheIoFinished(expected_callback_runs);
    ASSERT_EQ(read_buffer->size(), write_buffer->size());
    EXPECT_EQ(0, memcmp(read_buffer->data(), write_buffer->data(), read_buffer->size()));
}
...
TEST_F(DiskCacheEntryTest, SimpleCacheOptimistic)
{
    ...
    EntryResult result = cache_->CreateEntry(key, net::HIGHEST, base::BindOnce(&CallbackTest::RunWithEntry, base::Unretained(&callback1)));
    ...
    int ret = entry->WriteData(1, 0, buffer1.get(), kSize1, base::BindOnce(&CallbackTest::Run, base::Unretained(&callback2)), false);
    EXPECT_TRUE(kSize1 == ret || net::ERR_IO_PENDING == ret);
    if (net::ERR_IO_PENDING == ret)
        expected++;

    // This Read must not be optimistic, since we don't support that yet.
    EXPECT_EQ(net::ERR_IO_PENDING, entry->ReadData(1, 0, buffer1_read.get(), kSize1, base::BindOnce(&CallbackTest::Run, base::Unretained(&callback3))));
    expected++;
    EXPECT_TRUE(helper.WaitUntilCacheIoFinished(expected));
    ...
    EXPECT_EQ(kSize2, entry->WriteData(1, 0, buffer2.get(), kSize2, base::BindOnce(&CallbackTest::Run, base::Unretained(&callback4)), false));
    ...
    EXPECT_EQ(net::ERR_IO_PENDING, entry->ReadData(1, 0, buffer2_read.get(), kSize2, base::BindOnce(&CallbackTest::Run, base::Unretained(&callback5))));
    expected++;
...
TEST_F(DiskCacheEntryTest, SimpleCacheOptimistic2)
{
    ...
    EntryResult result2 = cache_->OpenEntry(key, net::HIGHEST, base::BindOnce(&CallbackTest::RunWithEntry, base::Unretained(&callback2)));
...
TEST_F(DiskCacheEntryTest, SimpleCacheOptimistic3)
{
    ...
    EntryResult result = cache_->CreateEntry(key, net::HIGHEST, EntryResultCallback());
    ASSERT_EQ(net::OK, result.net_error());
    disk_cache::Entry* entry = result.ReleaseEntry();
    ASSERT_NE(nullptr, entry);
    entry->Close();
...
TEST_F(DiskCacheEntryTest, SimpleCacheOptimistic4)
{
    ...
    EXPECT_EQ(net::ERR_IO_PENDING, entry->WriteData(1, 0, buffer1.get(), kSize1, cb.callback(), false));
    ...
    // entry.
    EXPECT_EQ(kSize1, entry2->WriteData(1, 0, buffer1.get(), kSize1, net::CompletionOnceCallback(), false));

    // Lets do another read so we block until both the write and the read
    // operation finishes and we can then test for HasOneRef() below.
    EXPECT_EQ(net::ERR_IO_PENDING, entry2->ReadData(1, 0, buffer1.get(), kSize1, cb.callback()));
...
TEST_F(DiskCacheEntryTest, SimpleCacheOptimistic5)
{
    ...
    EXPECT_EQ(net::ERR_IO_PENDING, entry->ReadData(1, 0, buffer1.get(), kSize1, cb.callback()));
    EXPECT_EQ(kSize1, cb.GetResult(net::ERR_IO_PENDING));
...
TEST_F(DiskCacheEntryTest, SimpleCacheOptimistic6)
{
    ...
    // This Read must not be optimistic, since we don't support that yet.
    EXPECT_EQ(net::ERR_IO_PENDING, entry->ReadData(1, 0, buffer1_read.get(), kSize1, cb.callback()));
    EXPECT_EQ(kSize1, cb.GetResult(net::ERR_IO_PENDING));
    EXPECT_EQ(0, memcmp(buffer1->data(), buffer1_read->data(), kSize1));
...
TEST_F(DiskCacheEntryTest, SimpleCacheOptimisticWriteReleases)
{
    ...
    // it completes.
    EXPECT_EQ(kWriteSize, WriteData(entry, 1, 0, buffer1.get(), kWriteSize, false));
    EXPECT_TRUE(buffer1->HasOneRef());

    // Finally, we should perform an optimistic write and confirm that all
    // references to the IO buffer have been released.
    EXPECT_EQ(kWriteSize, entry->WriteData(1, 0, buffer1.get(), kWriteSize, net::CompletionOnceCallback(), false));
    EXPECT_TRUE(buffer1->HasOneRef());
}
...
TEST_F(DiskCacheEntryTest, SimpleCacheCreateDoomRace)
{
    ...
    EXPECT_THAT(cache_->DoomEntry(key, net::HIGHEST, cb.callback()), IsError(net::ERR_IO_PENDING));
    EXPECT_THAT(cb.GetResult(net::ERR_IO_PENDING), IsOk());

    EXPECT_EQ(kSize1, entry->WriteData(0, 0, buffer1.get(), kSize1, cb.callback(), false));
    ...
    for (int i = 0; i < disk_cache::kSimpleEntryNormalFileCount; ++i) {
        base::FilePath entry_file_path = cache_path_.AppendASCII(disk_cache::simple_util::GetFilenameFromKeyAndFileIndex(key, i));
        base::File::Info info;
...
TEST_F(DiskCacheEntryTest, SimpleCacheDoomCreateRace)
{
    ...
    EntryResult result1 = create_callback.GetResult(cache_->CreateEntry(key, net::HIGHEST, create_callback.callback()));
    ...
    EXPECT_EQ(net::ERR_IO_PENDING, cache_->DoomEntry(key, net::HIGHEST, doom_callback.callback()));

    EntryResult result2 = create_callback.GetResult(cache_->CreateEntry(key, net::HIGHEST, create_callback.callback()));
...
TEST_F(DiskCacheEntryTest, SimpleCacheDoomCreateOptimistic)
{
    ...
    EntryResult result2 = cache_->CreateEntry(kKey, net::HIGHEST, create_callback.callback());
    ASSERT_EQ(net::OK, result2.net_error());
    disk_cache::Entry* entry2 = result2.ReleaseEntry();
    ...
    EXPECT_EQ(kSize, WriteData(entry2, /* index = */ 1, /* offset = */ 0, buf_1.get(), kSize, /* truncate = */ false));
    EXPECT_EQ(kSize, ReadData(entry2, /* index = */ 1, /* offset = */ 0, buf_2.get(), kSize));
...
TEST_F(DiskCacheEntryTest, SimpleCacheDoomCreateOptimisticMassDoom)
{
    ...
    // immediately, hence us using cache_->CreateEntry directly rather than using
    // the DiskCacheTestWithCache::CreateEntry wrapper which blocks when needed.
    EntryResult result = cache_->CreateEntry(kKey, net::HIGHEST, create_callback.callback());
    ASSERT_EQ(net::OK, result.net_error());
...
TEST_F(DiskCacheEntryTest, SimpleCacheDoomOpenOptimistic)
{
    ...
    EntryResult result2 = cache_->OpenEntry(kKey, net::HIGHEST, EntryResultCallback());
...
TEST_F(DiskCacheEntryTest, SimpleCacheOptimisticCreateFailsOnOpen)
{
    ...
    EXPECT_TRUE(disk_cache::simple_util::CreateCorruptFileForTests(key, cache_path_));
    EntryResult result = cache_->CreateEntry(key, net::HIGHEST, EntryResultCallback());
...
TEST_F(DiskCacheEntryTest, SimpleCacheEvictOldEntries)
{
    ...
    EXPECT_EQ(kWriteSize, WriteData(entry, 1, 0, buffer.get(), kWriteSize, false));
    entry->Close();
    AddDelay();
    ...
    ASSERT_NE(net::OK, OpenEntry(key1, &entry)) << "Should have evicted the old entry";
    for (int i = 0; i < 2; i++) {
        ...
        ASSERT_EQ(net::OK, OpenEntry(key2 + base::NumberToString(entry_no), &entry)) << "Should not have evicted fresh entry " << entry_no;
        entry->Close();
    }
}
...
TEST_F(DiskCacheEntryTest, SimpleCacheInFlightTruncate)
{
    ...
    EXPECT_EQ(kBufferSize, WriteData(entry, 1, 0, write_buffer.get(), kBufferSize, false));
    entry->Close();
    entry = nullptr;
    ...
    auto read_buffer = base::MakeRefCounted<net::IOBufferWithSize>(kReadBufferSize);
    CallbackTest read_callback(&helper, false);
    EXPECT_EQ(
        net::ERR_IO_PENDING, entry->ReadData(1, 0, read_buffer.get(), kReadBufferSize, base::BindOnce(&CallbackTest::Run, base::Unretained(&read_callback))));
    ++expected;

    // Truncate the entry to the length of that read.
    auto truncate_buffer = base::MakeRefCounted<net::IOBufferWithSize>(kReadBufferSize);
    CacheTestFillBuffer(truncate_buffer->data(), kReadBufferSize, false);
    CallbackTest truncate_callback(&helper, false);
    EXPECT_EQ(net::ERR_IO_PENDING,
        entry->WriteData(1, 0, truncate_buffer.get(), kReadBufferSize, base::BindOnce(&CallbackTest::Run, base::Unretained(&truncate_callback)), true));
    ++expected;
    ...
    EXPECT_EQ(0, memcmp(write_buffer->data(), read_buffer->data(), kReadBufferSize));
}
...
TEST_F(DiskCacheEntryTest, SimpleCacheInFlightRead)
{
    ...
    CallbackTest write_callback(&helper, false);
    EXPECT_EQ(net::ERR_IO_PENDING,
        entry->WriteData(1, 0, write_buffer.get(), kBufferSize, base::BindOnce(&CallbackTest::Run, base::Unretained(&write_callback)), true));
    ++expected;

    auto read_buffer = base::MakeRefCounted<net::IOBufferWithSize>(kBufferSize);
    CallbackTest read_callback(&helper, false);
    EXPECT_EQ(net::ERR_IO_PENDING, entry->ReadData(1, 0, read_buffer.get(), kBufferSize, base::BindOnce(&CallbackTest::Run, base::Unretained(&read_callback))));
    ++expected;
...
TEST_F(DiskCacheEntryTest, SimpleCacheNonSequentialWrite)
{
    ...
    for (int i = 0; i < disk_cache::kSimpleEntryStreamCount; ++i) {
        ...
        EXPECT_EQ(buf_len, WriteData(entry, i, offset, buffer2.get(), buf_len, false));
        offset = 0;
        buf_len = kHalfSize;
        EXPECT_EQ(buf_len, WriteData(entry, i, offset, buffer1.get(), buf_len, false));
        entry->Close();
...
TEST_F(DiskCacheEntryTest, SimpleCacheStream1SizeChanges)
{
    ...
    ASSERT_THAT(OpenEntry(key, &entry), IsOk());
    base::FilePath entry_file0_path = cache_path_.AppendASCII(disk_cache::simple_util::GetFilenameFromKeyAndFileIndex(key, 0));
    base::File entry_file0(entry_file0_path, base::File::FLAG_READ | base::File::FLAG_OPEN);
    ...
    disk_cache::SimpleEntryStat entry_stat(base::Time::Now(), base::Time::Now(), data_size, sparse_data_size);
    int eof_offset = entry_stat.GetEOFOffsetInFile(key.size(), 0);
    disk_cache::SimpleFileEOF eof_record;
    ASSERT_EQ(static_cast<int>(sizeof(eof_record)), entry_file0.Read(eof_offset, reinterpret_cast<char*>(&eof_record), sizeof(eof_record)));
...
TEST_F(DiskCacheEntryTest, SimpleCacheCRCRewrite)
{
    ...
    for (int i = 0; i < disk_cache::kSimpleEntryStreamCount; ++i) {
        ...
        EXPECT_EQ(0, memcmp(buffer2->data(), buffer1_read1->data() + kHalfSize, kHalfSize));
...
bool DiskCacheEntryTest::SimpleCacheThirdStreamFileExists(const char* key)
{
    ...
    base::FilePath third_stream_file_path = cache_path_.AppendASCII(disk_cache::simple_util::GetFilenameFromKeyAndFileIndex(key, third_stream_file_index));
...
void DiskCacheEntryTest::CreateEntryWithHeaderBodyAndSideData(const std::string& key, int data_size)
{
    ...
    ASSERT_THAT(CreateEntry(key, &entry), IsOk());
    for (int i = 0; i < disk_cache::kSimpleEntryStreamCount; ++i) {
        EXPECT_EQ(data_size, WriteData(entry, i, /* offset */ 0, buffer.get(), data_size, false));
    }
    entry->Close();
}

void DiskCacheEntryTest::TruncateFileFromEnd(int file_index, const std::string& key, int data_size, int truncate_size)
{
    // Remove last eof bytes from cache file.
    ASSERT_GT(data_size, truncate_size);
    const int64_t new_size = disk_cache::simple_util::GetFileSizeFromDataSize(key.size(), data_size) - truncate_size;
    const base::FilePath entry_path = cache_path_.AppendASCII(disk_cache::simple_util::GetFilenameFromKeyAndFileIndex(key, file_index));
...
TEST_F(DiskCacheEntryTest, SimpleCachePreserveActiveEntries)
{
    ...
    scoped_refptr<disk_cache::SimpleEntryImpl> entry1_refptr = static_cast<disk_cache::SimpleEntryImpl*>(entry1);
...
TEST_F(DiskCacheEntryTest, SimpleCacheNoBodyEOF)
{
    ...
    TruncateFileFromEnd(0 /*header and body file index*/, key, kSize, static_cast<int>(sizeof(disk_cache::SimpleFileEOF)));
    EXPECT_THAT(OpenEntry(key, &entry), IsError(net::ERR_FAILED));
}
...
TEST_F(DiskCacheEntryTest, SimpleCacheNoSideDataEOF)
{
    ...
    TruncateFileFromEnd(1 /*side data file_index*/, key, kSize, static_cast<int>(sizeof(disk_cache::SimpleFileEOF)));
    EXPECT_THAT(OpenEntry(key, &entry), IsOk());
    // The corrupted stream should have been deleted.
    EXPECT_FALSE(SimpleCacheThirdStreamFileExists(key));
    // _0 should still exist.
    base::FilePath path_0 = cache_path_.AppendASCII(disk_cache::simple_util::GetFilenameFromKeyAndFileIndex(key, 0));
...
TEST_F(DiskCacheEntryTest, SimpleCacheReadWithoutKeySHA256)
{
    ...
    auto stream_0_iobuffer = base::MakeRefCounted<net::StringIOBuffer>(stream_0_data);
    EXPECT_EQ(static_cast<int>(stream_0_data.size()), WriteData(entry, 0, 0, stream_0_iobuffer.get(), stream_0_data.size(), false));
    const std::string stream_1_data = "FOR STREAM ONE, QUITE DIFFERENT THINGS";
    auto stream_1_iobuffer = base::MakeRefCounted<net::StringIOBuffer>(stream_1_data);
    EXPECT_EQ(static_cast<int>(stream_1_data.size()), WriteData(entry, 1, 0, stream_1_iobuffer.get(), stream_1_data.size(), false));
    entry->Close();
    ...
    EXPECT_TRUE(disk_cache::simple_util::RemoveKeySHA256FromEntry(key, cache_path_));
    ASSERT_THAT(OpenEntry(key, &entry), IsOk());
    ScopedEntryPtr entry_closer(entry);

    EXPECT_EQ(static_cast<int>(stream_0_data.size()), entry->GetDataSize(0));
    auto check_stream_0_data = base::MakeRefCounted<net::IOBufferWithSize>(stream_0_data.size());
    EXPECT_EQ(static_cast<int>(stream_0_data.size()), ReadData(entry, 0, 0, check_stream_0_data.get(), stream_0_data.size()));
    ...
    auto check_stream_1_data = base::MakeRefCounted<net::IOBufferWithSize>(stream_1_data.size());
    EXPECT_EQ(static_cast<int>(stream_1_data.size()), ReadData(entry, 1, 0, check_stream_1_data.get(), stream_1_data.size()));
...
TEST_F(DiskCacheEntryTest, SimpleCacheReadCorruptKeySHA256)
{
    ...
    EXPECT_TRUE(disk_cache::simple_util::CorruptKeySHA256FromEntry(key, cache_path_));
...
TEST_F(DiskCacheEntryTest, SimpleCacheReadCorruptLength)
{
    ...
    EXPECT_TRUE(disk_cache::simple_util::CorruptStream0LengthFromEntry(key, cache_path_));
    EXPECT_NE(net::OK, OpenEntry(key, &entry));
}
...
TEST_F(DiskCacheEntryTest, SimpleCacheCreateRecoverFromRmdir)
{
    ...
    disk_cache::DeleteCache(cache_path_, true /* delete the dir, what we really want*/);
...
TEST_F(DiskCacheEntryTest, SimpleCacheSparseErrorHandling)
{
    ...
    disk_cache::SimpleFileTracker::EntryFileKey num_key(disk_cache::simple_util::GetEntryHashKey(key));
    base::FilePath path_0 = cache_path_.AppendASCII(disk_cache::simple_util::GetFilenameFromEntryFileKeyAndFileIndex(num_key, 0));
    base::FilePath path_s = cache_path_.AppendASCII(disk_cache::simple_util::GetSparseFilenameFromEntryFileKey(num_key));
    ...
    // read.
    base::File file_s(path_s, base::File::FLAG_OPEN | base::File::FLAG_READ | base::File::FLAG_WRITE);
    ASSERT_TRUE(file_s.IsValid());
    file_s.SetLength(sizeof(disk_cache::SimpleFileHeader) + sizeof(disk_cache::SimpleFileSparseRangeHeader) + key.size());
    file_s.Close();
    ...
    EXPECT_EQ(net::ERR_CACHE_READ_FAILURE, ReadSparseData(entry, 0, buffer.get(), kSize));
    ...
    TestRangeResultCompletionCallback range_cb;
    RangeResult result = range_cb.GetResult(entry->GetAvailableRange(0, 1024, range_cb.callback()));
...
TEST_F(DiskCacheEntryTest, SimpleCacheCreateCollision)
{
    ...
    // of both.
    const char kCollKey1[] = "\xfb\x4e\x9c\x1d\x66\x71\xf7\x54\xa3\x11\xa0\x7e\x16\xa5\x68\xf6";
    const char kCollKey2[] = "\xbc\x60\x64\x92\xbc\xa0\x5c\x15\x17\x93\x29\x2d\xe4\x21\xbd\x03";
...
TEST_F(DiskCacheEntryTest, SimpleCacheConvertToSparseStream2LeftOver)
{
    ...
    EXPECT_EQ(kSize, WriteData(entry, /* stream = */ 2, /* offset = */ 0, buffer.get(), kSize, false));
    EXPECT_EQ(0, WriteData(entry, /* stream = */ 2, /* offset = */ 0, buffer.get(), 0, true));
...
TEST_F(DiskCacheEntryTest, SimpleCacheLazyStream2CreateFailure)
{
    ...
    EXPECT_EQ(net::ERR_CACHE_WRITE_FAILURE, WriteData(entry, /* index = */ 2, /* offset = */ 0, buffer.get(), kSize, /* truncate = */ false));
    entry->Close();
}
...
TEST_F(DiskCacheEntryTest, SimpleCacheChecksumpScrewUp)
{
    ...
    ASSERT_EQ(kDoubleSize, WriteData(entry, 1, 0, big_buffer.get(), kDoubleSize, false));

    // Reset remembered position to 0 by writing at an earlier non-zero offset.
    ASSERT_EQ(1, WriteData(entry, /* stream = */ 1, /* offset = */ 1, big_buffer.get(), /* len = */ 1, false));
...
void DiskCacheEntryTest::SparseOffset64Bit()
{
    ...
    EXPECT_EQ(blockfile ? net::ERR_CACHE_OPERATION_NOT_SUPPORTED : kSize, WriteSparseData(entry, kOffset, buffer.get(), kSize));

    int64_t start_out = -1;
    EXPECT_EQ(0, GetAvailableRange(entry, /* offset = */ 0, kSize, &start_out));

    start_out = -1;
    EXPECT_EQ(blockfile ? 0 : kSize, GetAvailableRange(entry, kOffset, kSize, &start_out));
...
TEST_F(DiskCacheEntryTest, SimpleCacheCloseResurrection)
{
    ...
    int rv = entry->WriteData(1, 0, buffer.get(), kSize, net::CompletionOnceCallback(), false);
    ...
    EntryResult result2 = cache_->OpenEntry(kKey, net::HIGHEST, cb_open.callback());
...
TEST_F(DiskCacheEntryTest, BlockFileSparsePendingAfterDtor)
{
    ...
    EXPECT_EQ(net::ERR_IO_PENDING, entry->WriteSparseData(2560, buf.get(), kSize, base::DoNothing()));
    entry->Close();
    ResetCaches();
...
public:
    ...
    void SetupFullAndTrailerPrefetch(int full_size, int trailer_speculative_size)
    {
        std::map<std::string, std::string> params;
        params[disk_cache::kSimpleCacheFullPrefetchBytesParam] = base::NumberToString(full_size);
        params[disk_cache::kSimpleCacheTrailerPrefetchSpeculativeBytesParam] = base::NumberToString(trailer_speculative_size);
        scoped_feature_list_.InitAndEnableFeatureWithParameters(disk_cache::kSimpleCachePrefetchExperiment, params);
    }
    ...
    void InitCacheAndCreateEntry(const std::string& key)
    {
        ...
        ASSERT_EQ(kEntrySize, WriteData(entry, 1, 0, payload_.get(), kEntrySize, false));
        entry->Close();
    }

    virtual net::CacheType SimpleCacheType() const
    {
    ...
    void InitCacheAndCreateEntryWithNoCrc(const std::string& key)
    {
        ...
        ASSERT_EQ(kRemSize, WriteData(entry, 1, kHalfSize, second_half.get(), kRemSize, false));
        entry->Close();
    }
...
TEST_F(DiskCacheSimplePrefetchTest, NoPrefetch)
{
    ...
    histogram_tester.ExpectUniqueSample("SimpleCache.Http.SyncOpenPrefetchMode", disk_cache::OPEN_PREFETCH_NONE, 1);
}
...
TEST_F(DiskCacheSimplePrefetchTest, YesPrefetch)
{
    ...
    histogram_tester.ExpectUniqueSample("SimpleCache.Http.SyncOpenPrefetchMode", disk_cache::OPEN_PREFETCH_FULL, 1);
}
...
TEST_F(DiskCacheSimplePrefetchTest, ChecksumNoPrefetch)
{
    ...
    histogram_tester.ExpectUniqueSample("SimpleCache.Http.SyncCheckEOFResult", disk_cache::CHECK_EOF_RESULT_SUCCESS, 2);
}
...
TEST_F(DiskCacheSimplePrefetchTest, PrefetchReadsSync)
{
    ...
    // if needed.
    EXPECT_EQ(kEntrySize, entry->ReadData(1, 0, read_buf.get(), kEntrySize, net::CompletionOnceCallback()));
    EXPECT_EQ(0, memcmp(read_buf->data(), payload_->data(), kEntrySize));
    entry->Close();
}
...
TEST_F(DiskCacheSimplePrefetchTest, NoFullSmallSpeculative)
{
    ...
    histogram_tester.ExpectUniqueSample("SimpleCache.Http.SyncOpenPrefetchMode", disk_cache::OPEN_PREFETCH_TRAILER, 1);
}
...
public:
    ...
    net::CacheType SimpleCacheType() const override
    {
        return net::APP_CACHE;
    }
};
...
TEST_F(DiskCacheSimpleAppCachePrefetchTest, NoFullNoSpeculative)
{
    ...
    histogram_tester.ExpectUniqueSample("SimpleCache.App.SyncOpenPrefetchMode", disk_cache::OPEN_PREFETCH_TRAILER, 1);
}
...
TEST_F(DiskCacheSimpleAppCachePrefetchTest, LargeFullNoSpeculative)
{
    ...
    histogram_tester.ExpectUniqueSample("SimpleCache.App.SyncOpenPrefetchMode", disk_cache::OPEN_PREFETCH_FULL, 1);
}
