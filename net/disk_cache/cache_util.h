...
// directory.
NET_EXPORT_PRIVATE bool MoveCache(const base::FilePath& from_path, const base::FilePath& to_path);

// Deletes the cache files stored on |path|, and optionally also attempts to
// delete the folder itself.
NET_EXPORT_PRIVATE void DeleteCache(const base::FilePath& path, bool remove_folder);
...
NET_EXPORT_PRIVATE void CleanupDirectory(const base::FilePath& path, base::OnceCallback<void(bool)> callback);
...
// cache type.
NET_EXPORT_PRIVATE int PreferredCacheSize(int64_t available, net::CacheType type = net::DISK_CACHE);
