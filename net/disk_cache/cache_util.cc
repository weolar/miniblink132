...
base::FilePath GetPrefixedName(const base::FilePath& path, const base::SafeBaseName& basename, int index)
{
...
base::FilePath GetTempCacheName(const base::FilePath& dirname, const base::SafeBaseName& basename)
{
...
void CleanupTemporaryDirectories(const base::FilePath& path)
{
    ...
    const std::optional<base::SafeBaseName> basename = base::SafeBaseName::Create(path);
...
BASE_FEATURE(kChangeDiskCacheSizeExperiment, "ChangeDiskCacheSize", base::FEATURE_DISABLED_BY_DEFAULT);
...
void DeleteCache(const base::FilePath& path, bool remove_folder)
{
    ...
    base::FileEnumerator iter(path,
        /* recursive */ false, base::FileEnumerator::FILES | base::FileEnumerator::DIRECTORIES);
...
void CleanupDirectory(const base::FilePath& path, base::OnceCallback<void(bool)> callback)
{
    auto task_runner = base::ThreadPool::CreateSequencedTaskRunner(
        { base::MayBlock(), base::TaskPriority::USER_BLOCKING, base::TaskShutdownBehavior::CONTINUE_ON_SHUTDOWN });

    task_runner->PostTaskAndReplyWithResult(FROM_HERE, base::BindOnce(CleanupDirectoryInternal, path), std::move(callback));
}
...
int PreferredCacheSize(int64_t available, net::CacheType type)
{
    ...
    if (base::FeatureList::IsEnabled(disk_cache::kChangeDiskCacheSizeExperiment) && type == net::DISK_CACHE) {
        percent_relative_size
            = base::GetFieldTrialParamByFeatureAsInt(disk_cache::kChangeDiskCacheSizeExperiment, "percent_relative_size", 100 /* default value */);
    }
    ...
    base::ClampedNumeric<int64_t> scaled_default_disk_cache_size = (base::ClampedNumeric<int64_t>(disk_cache::kDefaultCacheSize) * percent_relative_size) / 100;

    base::ClampedNumeric<int64_t> preferred_cache_size = scaled_default_disk_cache_size;
    ...
    if (available >= 0) {
        ...
        if (preferred_cache_size < available / 5) {
            ...
            preferred_cache_size = std::min((preferred_cache_size * percent_relative_size) / 100, clamped_available / 5);
        }
    }
    ...
    if (type == net::GENERATED_NATIVE_CODE_CACHE) {
        ...
        size_limit = std::min(size_limit, base::ClampedNumeric<int64_t>(kMaxWebUICodeCacheSize));
    }
