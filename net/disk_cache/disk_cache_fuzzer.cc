...
const uint64_t kFirstSavedTime = 5; // Totally random number chosen by dice roll. ;)
...
const std::string kCollidingKey1 = "\xfb\x4e\x9c\x1d\x66\x71\xf7\x54\xa3\x11\xa0\x7e\x16\xa5\x68\xf6";
const std::string kCollidingKey2 = "\xbc\x60\x64\x92\xbc\xa0\x5c\x15\x17\x93\x29\x2d\xe4\x21\xbd\x03";
...
struct InitGlobals {
    InitGlobals()
    {
        ...
        task_environment_ = std::make_unique<base::test::TaskEnvironment>(
            base::test::TaskEnvironment::MainThreadType::IO, base::test::TaskEnvironment::TimeSource::MOCK_TIME);
        ...
        buffer_ = base::MakeRefCounted<net::IOBufferWithSize>(static_cast<size_t>(kMaxEntrySize));
        CacheTestFillBuffer(buffer_->data(), kMaxEntrySize, false);

#define CREATE_IO_CALLBACK(IO_TYPE) io_callbacks_.push_back(base::BindRepeating(&IOCallback, #IO_TYPE));
...
private:
    ...
    // called the callback).
    disk_cache::EntryResult WaitOnEntry(EntryInfo* ei, disk_cache::EntryResult result = disk_cache::EntryResult::MakeError(net::ERR_IO_PENDING));
    ...
    void OpenCacheEntryCallback(uint64_t entry_id, bool async, bool set_is_sparse, disk_cache::EntryResult result);
    ...
    void CreateBackend(disk_cache_fuzzer::FuzzCommands::CacheBackend cache_backend, uint32_t mask, const disk_cache_fuzzer::SetMaxSize* maybe_max_size,
        net::CacheType type, bool simple_cache_wait_for_index);
    ...
    // protobuf.
    std::map<uint64_t, std::unique_ptr<disk_cache::Backend::Iterator>> open_iterators_;
...
net::RequestPriority GetRequestPriority(disk_cache_fuzzer::RequestPriority lpm_pri)
{
    CHECK(net::MINIMUM_PRIORITY <= static_cast<int>(lpm_pri) && static_cast<int>(lpm_pri) <= net::MAXIMUM_PRIORITY);
    return static_cast<net::RequestPriority>(lpm_pri);
}

net::CacheType GetCacheTypeAndPrint(disk_cache_fuzzer::FuzzCommands::CacheType type, disk_cache_fuzzer::FuzzCommands::CacheBackend backend)
{
    switch (type) {
 ...
 */
template <typename T> typename std::map<uint64_t, T>::iterator GetNextValue(typename std::map<uint64_t, T>* entries, uint64_t val)
{
...
void DiskCacheLPMFuzzer::OpenCacheEntryCallback(uint64_t entry_id, bool async, bool set_is_sparse, disk_cache::EntryResult result)
{
    ...
    if (async) {
        ...
        if (ei->entry_ptr) {
            MAYBE_PRINT << " [Async opening of cache entry for \"" << ei->entry_ptr->GetKey() << "\" callback (rv = " << rv << ")]" << std::endl;
        }
        ...
        ei->tcb->callback().Run(disk_cache::EntryResult::MakeError(net::ERR_FILE_VIRUS_INFECTED));
    } else {
...
disk_cache::EntryResult DiskCacheLPMFuzzer::WaitOnEntry(EntryInfo* ei, disk_cache::EntryResult result)
{
    CHECK(ei->tcb);
...
void DiskCacheLPMFuzzer::RunCommands(const disk_cache_fuzzer::FuzzCommands& commands)
{
    ...
    uint32_t mask = commands.has_set_mask() ? (commands.set_mask() ? 0x1 : 0xf) : 0;
    net::CacheType type = GetCacheTypeAndPrint(commands.cache_type(), commands.cache_backend());
    CreateBackend(
        commands.cache_backend(), mask, commands.has_set_max_size() ? &commands.set_max_size() : nullptr, type, commands.simple_cache_wait_for_index());
    ...
    for (const disk_cache_fuzzer::FuzzCommand& command : commands.fuzz_commands()) {
        ...
        case disk_cache_fuzzer::FuzzCommand::kCreateEntry: {
            ...
            disk_cache::EntryResultCallback cb
                = base::BindOnce(&DiskCacheLPMFuzzer::OpenCacheEntryCallback, base::Unretained(this), entry_id, async, is_sparse);

            MAYBE_PRINT << "CreateEntry(\"" << key_str << "\", set_is_sparse = " << is_sparse << ") = " << std::flush;
            disk_cache::EntryResult result = cache_->CreateEntry(key_str, pri, std::move(cb));
        ...
        case disk_cache_fuzzer::FuzzCommand::kOpenEntry: {
            ...
            entry_info->tcb = std::make_unique<TestEntryResultCompletionCallback>();
            disk_cache::EntryResultCallback cb = base::BindOnce(&DiskCacheLPMFuzzer::OpenCacheEntryCallback, base::Unretained(this), entry_id, async, false);

            auto key_it = GetNextValue(&created_cache_entries_, key_id);
            MAYBE_PRINT << "OpenEntry(\"" << key_it->second << "\") = " << std::flush;
            disk_cache::EntryResult result = cache_->OpenEntry(key_it->second, pri, std::move(cb));
            if (!async || result.net_error() != net::ERR_IO_PENDING) {
                result = WaitOnEntry(entry_info, std::move(result));
                int rv = result.net_error();
                if (rv == net::OK)
        ...
        case disk_cache_fuzzer::FuzzCommand::kOpenOrCreateEntry: {
            ...
            const disk_cache_fuzzer::OpenOrCreateEntry& ooce = command.open_or_create_entry();
            ...
            disk_cache::EntryResult result = cache_->OpenOrCreateEntry(key_str, pri, std::move(cb));
            if (!async || result.net_error() != net::ERR_IO_PENDING) {
                result = WaitOnEntry(entry_info, std::move(result));
                int rv = result.net_error();
        ...
        case disk_cache_fuzzer::FuzzCommand::kCloseEntry: {
            ...
            auto entry_it = GetNextValue(&open_cache_entries_, command.close_entry().entry_id());
            if (!IsValidEntry(&entry_it->second))
                continue;

            MAYBE_PRINT << "CloseEntry(\"" << entry_it->second.entry_ptr->GetKey() << "\")" << std::endl;
        ...
        case disk_cache_fuzzer::FuzzCommand::kDoomEntry: {
            ...
            auto entry_it = GetNextValue(&open_cache_entries_, command.doom_entry().entry_id());
            if (!IsValidEntry(&entry_it->second))
                continue;

            MAYBE_PRINT << "DoomEntry(\"" << entry_it->second.entry_ptr->GetKey() << "\")" << std::endl;
        ...
        case disk_cache_fuzzer::FuzzCommand::kWriteData: {
            ...
            net::CompletionOnceCallback cb = !async ? tcb.callback() : GetIOCallback(IOType::WriteData);

            MAYBE_PRINT << "WriteData(\"" << entry_it->second.entry_ptr->GetKey() << "\", index = " << index << ", offset = " << offset << ", size = " << size
                        << ", truncate = " << wd.truncate() << ")" << std::flush;
            int rv = entry_it->second.entry_ptr->WriteData(index, offset, init_globals->buffer_.get(), size, std::move(cb), wd.truncate());
            if (!async)
                rv = tcb.GetResult(rv);
            MAYBE_PRINT << " = " << rv << std::endl;
            break;
        ...
        case disk_cache_fuzzer::FuzzCommand::kReadData: {
            ...
            net::TestCompletionCallback tcb;
            net::CompletionOnceCallback cb = !async ? tcb.callback() : GetIOCallback(IOType::ReadData);

            MAYBE_PRINT << "ReadData(\"" << entry_it->second.entry_ptr->GetKey() << "\", index = " << index << ", offset = " << offset << ", size = " << size
                        << ")" << std::flush;
            int rv = entry_it->second.entry_ptr->ReadData(index, offset, buffer.get(), size, std::move(cb));
            if (!async)
                rv = tcb.GetResult(rv);
            MAYBE_PRINT << " = " << rv << std::endl;
            break;
        ...
        case disk_cache_fuzzer::FuzzCommand::kWriteSparseData: {
            ...
            const disk_cache_fuzzer::WriteSparseData& wsd = command.write_sparse_data();
            auto entry_it = GetNextValue(&open_cache_entries_, wsd.entry_id());
            if (!IsValidEntry(&entry_it->second) || !sparse_entry_tracker_[entry_it->second.entry_ptr])
                continue;
            ...
            net::TestCompletionCallback tcb;
            net::CompletionOnceCallback cb = !async ? tcb.callback() : GetIOCallback(IOType::WriteSparseData);
            MAYBE_PRINT << "WriteSparseData(\"" << entry_it->second.entry_ptr->GetKey() << "\", offset = " << offset << ", size = " << size << ")"
                        << std::flush;
            int rv = entry_it->second.entry_ptr->WriteSparseData(offset, init_globals->buffer_.get(), size, std::move(cb));
            if (!async)
                rv = tcb.GetResult(rv);
            MAYBE_PRINT << " = " << rv << std::endl;
            break;
        ...
        case disk_cache_fuzzer::FuzzCommand::kReadSparseData: {
            ...
            const disk_cache_fuzzer::ReadSparseData& rsd = command.read_sparse_data();
            ...
            net::TestCompletionCallback tcb;
            net::CompletionOnceCallback cb = !async ? tcb.callback() : GetIOCallback(IOType::ReadSparseData);

            MAYBE_PRINT << "ReadSparseData(\"" << entry_it->second.entry_ptr->GetKey() << "\", offset = " << offset << ", size = " << size << ")" << std::flush;
            int rv = entry_it->second.entry_ptr->ReadSparseData(offset, buffer.get(), size, std::move(cb));
            if (!async)
                rv = tcb.GetResult(rv);
            MAYBE_PRINT << " = " << rv << std::endl;
            break;
        ...
        case disk_cache_fuzzer::FuzzCommand::kDoomAllEntries: {
            ...
            net::TestCompletionCallback tcb;
            net::CompletionOnceCallback cb = !async ? tcb.callback() : GetIOCallback(IOType::DoomAllEntries);
        ...
        case disk_cache_fuzzer::FuzzCommand::kIteratorOpenNextEntry: {
            const disk_cache_fuzzer::IteratorOpenNextEntry& ione = command.iterator_open_next_entry();
            ...
            MAYBE_PRINT << "Iterator(" << ione.it_id() << ").OpenNextEntry() = " << std::flush;
            disk_cache::EntryResult result = iterator_it->second->OpenNextEntry(std::move(cb));
            if (!async || result.net_error() != net::ERR_IO_PENDING) {
                result = WaitOnEntry(entry_info, std::move(result));
                int rv = result.net_error();
                entry_info->entry_ptr = result.ReleaseEntry();
                ...
                if (!entry_info->entry_ptr) {
                    ...
                    MAYBE_PRINT << rv << ", key = " << entry_info->entry_ptr->GetKey() << std::endl;
                }
            } else {
                MAYBE_PRINT << "net::ERR_IO_PENDING (async)" << std::endl;
            }
        ...
        case disk_cache_fuzzer::FuzzCommand::kFastForwardBy: {
            base::TimeDelta to_wait = base::Milliseconds(command.fast_forward_by().capped_num_millis() % kMaxNumMillisToWait);
        ...
        case disk_cache_fuzzer::FuzzCommand::kDoomEntriesSince: {
            ...
            const disk_cache_fuzzer::DoomEntriesSince& des = command.doom_entries_since();
            auto time_it = GetNextValue(&saved_times_, des.time_id());
            bool async = des.async();

            net::TestCompletionCallback tcb;
            net::CompletionOnceCallback cb = !async ? tcb.callback() : GetIOCallback(IOType::DoomEntriesSince);

            MAYBE_PRINT << "DoomEntriesSince(" << time_it->second << ")" << std::flush;
        ...
        case disk_cache_fuzzer::FuzzCommand::kDoomEntriesBetween: {
            ...
            const disk_cache_fuzzer::DoomEntriesBetween& deb = command.doom_entries_between();
            ...
            net::TestCompletionCallback tcb;
            net::CompletionOnceCallback cb = !async ? tcb.callback() : GetIOCallback(IOType::DoomEntriesBetween);

            MAYBE_PRINT << "DoomEntriesBetween(" << time1 << ", " << time2 << ")" << std::flush;
        ...
        case disk_cache_fuzzer::FuzzCommand::kOnExternalCacheHit: {
            ...
            auto key_it = GetNextValue(&created_cache_entries_, key_id);
            MAYBE_PRINT << "OnExternalCacheHit(\"" << key_it->second << "\")" << std::endl;
        ...
        case disk_cache_fuzzer::FuzzCommand::kTrimForTest: {
            ...
            RunTaskForTest(base::BindOnce(&disk_cache::BackendImpl::TrimForTest, base::Unretained(block_impl_), command.trim_for_test().empty()));
            break;
        }
        case disk_cache_fuzzer::FuzzCommand::kTrimDeletedListForTest: {
            ...
            RunTaskForTest(
                base::BindOnce(&disk_cache::BackendImpl::TrimDeletedListForTest, base::Unretained(block_impl_), command.trim_deleted_list_for_test().empty()));
            break;
        }
        case disk_cache_fuzzer::FuzzCommand::kGetAvailableRange: {
            ...
            const disk_cache_fuzzer::GetAvailableRange& gar = command.get_available_range();
            ...
            auto result_checker = base::BindRepeating(
                [](net::CompletionOnceCallback callback, uint32_t offset, uint32_t len, const disk_cache::RangeResult& result) {
                    ...
                    base::CheckedNumeric<uint64_t> range_end64 = range_start + result.available_len;
            ...
            disk_cache::RangeResultCallback cb = !async ? tcb.callback() : result_checker;

            MAYBE_PRINT << "GetAvailableRange(\"" << entry->GetKey() << "\", " << offset << ", " << len << ")" << std::flush;
            disk_cache::RangeResult result = entry->GetAvailableRange(offset, len, std::move(cb));
            ...
            if (async && result.net_error == net::ERR_IO_PENDING) {
                ...
                MAYBE_PRINT << " = " << result.net_error << ", start = " << result.start << ", available_len = " << result.available_len;
                if (result.net_error < 0) {
                    MAYBE_PRINT << ", error to string: " << net::ErrorToShortString(result.net_error) << std::endl;
                } else {
        ...
        case disk_cache_fuzzer::FuzzCommand::kCancelSparseIo: {
            ...
            const disk_cache_fuzzer::CancelSparseIO& csio = command.cancel_sparse_io();
            auto entry_it = GetNextValue(&open_cache_entries_, csio.entry_id());
            if (!IsValidEntry(&entry_it->second))
                continue;

            MAYBE_PRINT << "CancelSparseIO(\"" << entry_it->second.entry_ptr->GetKey() << "\")" << std::endl;
        ...
        case disk_cache_fuzzer::FuzzCommand::kDoomKey: {
            ...
            net::TestCompletionCallback tcb;
            net::CompletionOnceCallback cb = !async ? tcb.callback() : GetIOCallback(IOType::DoomKey);
        ...
        case disk_cache_fuzzer::FuzzCommand::kDestructBackend: {
            ...
            const disk_cache_fuzzer::DestructBackend& db = command.destruct_backend();
        ...
        case disk_cache_fuzzer::FuzzCommand::kRecreateWithSize: {
            ...
            MAYBE_PRINT << "RecreateWithSize(" << command.recreate_with_size().size() << ")" << std::endl;
            ShutdownBackend();
            // re-create backend with same config but (potentially) different size.
            CreateBackend(commands.cache_backend(), mask, &command.recreate_with_size(), type, commands.simple_cache_wait_for_index());
            break;
        }
...
int64_t DiskCacheLPMFuzzer::ComputeMaxSize(const disk_cache_fuzzer::SetMaxSize* maybe_max_size)
{
    if (!maybe_max_size) {
...
void DiskCacheLPMFuzzer::CreateBackend(disk_cache_fuzzer::FuzzCommands::CacheBackend cache_backend, uint32_t mask,
    const disk_cache_fuzzer::SetMaxSize* maybe_max_size, net::CacheType type, bool simple_cache_wait_for_index)
{
    ...
    if (cache_backend != disk_cache_fuzzer::FuzzCommands::IN_MEMORY) {
        ...
        while (true) {
            ...
            cleanup_tracker = disk_cache::BackendCleanupTracker::TryCreate(cache_path_, run_dir_ready.QuitClosure());
            if (cleanup_tracker) {
                break;
            } else {
                run_dir_ready.Run();
    ...
    if (cache_backend == disk_cache_fuzzer::FuzzCommands::IN_MEMORY) {
        ...
        auto cache = disk_cache::MemBackendImpl::CreateBackend(ComputeMaxSize(maybe_max_size), /*net_log=*/nullptr);
        ...
        if (!simple_file_tracker_)
            simple_file_tracker_ = std::make_unique<disk_cache::SimpleFileTracker>(kMaxFdsSimpleCache);
        auto simple_backend = std::make_unique<disk_cache::SimpleBackendImpl>(
            /*file_operations=*/nullptr, cache_path_, std::move(cleanup_tracker), simple_file_tracker_.get(), ComputeMaxSize(maybe_max_size), type,
            /*net_log=*/nullptr);
        ...
        if (simple_cache_wait_for_index) {
            MAYBE_PRINT << "Waiting for simple cache index to be ready..." << std::endl;
            net::TestCompletionCallback wait_for_index_cb;
            simple_cache_impl_->index()->ExecuteWhenReady(wait_for_index_cb.callback());
        ...
        if (mask) {
            ...
            cache = std::make_unique<disk_cache::BackendImpl>(cache_path_, mask,
                /* cleanup_tracker = */ std::move(cleanup_tracker),
                /* runner = */ nullptr, type,
                /* net_log = */ nullptr);
        } else {
            MAYBE_PRINT << "." << std::endl;
            cache = std::make_unique<disk_cache::BackendImpl>(cache_path_,
                /* cleanup_tracker = */ std::move(cleanup_tracker),
                /* runner = */ nullptr, type,
                /* net_log = */ nullptr);
        }
...
void DiskCacheLPMFuzzer::CloseAllRemainingEntries()
{
    for (auto& entry_info : open_cache_entries_) {
        ...
        if (!*entry_ptr)
            continue;
        MAYBE_PRINT << "Destructor CloseEntry(\"" << (*entry_ptr)->GetKey() << "\")" << std::endl;
        (*entry_ptr)->Close();
        *entry_ptr = nullptr;
    }
}
