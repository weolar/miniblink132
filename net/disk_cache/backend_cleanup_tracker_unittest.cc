...
protected:
    ...
    void RecordCall(int val)
    {
        called_.push_back(val);
    }

    base::OnceClosure RecordCallClosure(int val)
    {
        return base::BindOnce(&BackendCleanupTrackerTest::RecordCall, base::Unretained(this), val);
    }
...
{
    scoped_refptr<BackendCleanupTracker> t1 = BackendCleanupTracker::TryCreate(path1_, RecordCallClosure(1));
    scoped_refptr<BackendCleanupTracker> t2 = BackendCleanupTracker::TryCreate(path2_, RecordCallClosure(2));
...
{
    scoped_refptr<BackendCleanupTracker> t1 = BackendCleanupTracker::TryCreate(path1_, RecordCallClosure(1));
    scoped_refptr<BackendCleanupTracker> t2 = BackendCleanupTracker::TryCreate(path1_, RecordCallClosure(2));
