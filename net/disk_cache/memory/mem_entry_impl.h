...
class NET_EXPORT_PRIVATE MemEntryImpl final : public Entry, public base::LinkNode<MemEntryImpl> {
public:
    ...
    MemEntryImpl(base::WeakPtr<MemBackendImpl> backend, const std::string& key, net::NetLog* net_log);

    // Constructor for child entries.
    MemEntryImpl(base::WeakPtr<MemBackendImpl> backend, int64_t child_id, MemEntryImpl* parent, net::NetLog* net_log);
    ...
    const MemEntryImpl* parent() const
    {
        return parent_;
    }
...
private:
    MemEntryImpl(base::WeakPtr<MemBackendImpl> backend, const std::string& key, int64_t child_id, MemEntryImpl* parent, net::NetLog* net_log);
    ...
    int InternalWriteData(int index, int offset, IOBuffer* buf, int buf_len, bool truncate);
    ...
    net::Interval<int64_t> ChildInterval(MemEntryImpl::EntryMap::const_iterator i);
