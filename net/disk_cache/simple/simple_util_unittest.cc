...
{
    EXPECT_EQ("0000000005f5e0ff", ConvertEntryHashKeyToHexString(UINT64_C(99999999)));
    EXPECT_EQ("7fffffffffffffff", ConvertEntryHashKeyToHexString(UINT64_C(9223372036854775807)));
    EXPECT_EQ("8000000000000000", ConvertEntryHashKeyToHexString(UINT64_C(9223372036854775808)));
    EXPECT_EQ("ffffffffffffffff", ConvertEntryHashKeyToHexString(UINT64_C(18446744073709551615)));
}

TEST_F(SimpleUtilTest, GetEntryHashKey)
{
    EXPECT_EQ("7ac408c1dff9c84b", GetEntryHashKeyAsHexString("http://www.amazon.com/"));
    EXPECT_EQ(UINT64_C(0x7ac408c1dff9c84b), GetEntryHashKey("http://www.amazon.com/"));

    EXPECT_EQ("9fe947998c2ccf47", GetEntryHashKeyAsHexString("www.amazon.com"));
    ...
    EXPECT_EQ("a68ac2ecc87dfd04",
        GetEntryHashKeyAsHexString(
            "http://www.domain.com/"
            "uoQ76Kb2QL5hzaVOSAKWeX0W9LfDLqphmRXpsfHN8tgF5lCsfTxlOVWY8vFwzhsRzoNYKhUIOTc5TnUlT0vpdQflPyk2nh7vurXOj60cDnkG3nsrXMhFCsPjhcZAic2jKpF9F9TYRYQwJo81IM"
            "i6gY01RK3ZcNl8WGfqcvoZ702UIdetvR7kiaqo1czwSJCMjRFdG6EgMzgXrwE8DYMz4fWqoa1F1c1qwTCBk3yOcmGTbxsPSJK5QRyNea9IFLrBTjfE7ZlN2vZiI7adcDYJef.htm"));

    EXPECT_EQ(UINT64_C(0xa68ac2ecc87dfd04),
        GetEntryHashKey(
            "http://www.domain.com/"
            "uoQ76Kb2QL5hzaVOSAKWeX0W9LfDLqphmRXpsfHN8tgF5lCsfTxlOVWY8vFwzhsRzoNYKhUIOTc5TnUlT0vpdQflPyk2nh7vurXOj60cDnkG3nsrXMhFCsPjhcZAic2jKpF9F9TYRYQwJo81IM"
            "i6gY01RK3ZcNl8WGfqcvoZ702UIdetvR7kiaqo1czwSJCMjRFdG6EgMzgXrwE8DYMz4fWqoa1F1c1qwTCBk3yOcmGTbxsPSJK5QRyNea9IFLrBTjfE7ZlN2vZiI7adcDYJef.htm"));
