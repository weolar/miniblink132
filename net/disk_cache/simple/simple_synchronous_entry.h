...
NET_EXPORT_PRIVATE extern const char kSimpleCacheTrailerPrefetchSpeculativeBytesParam[];
...
public:
    SimpleEntryStat(base::Time last_used, base::Time last_modified, const int32_t data_size[], const int32_t sparse_data_size);
    ...
    int32_t data_size(int stream_index) const
    {
    ...
    int32_t sparse_data_size() const
    {
        return sparse_data_size_;
    }
...
public:
    ...
    struct WriteRequest {
        WriteRequest(int index_p, int offset_p, int buf_len_p, uint32_t previous_crc32_p, bool truncate_p, bool doomed_p, bool request_update_crc_p);
        int index;
        int offset;
        int buf_len;
        uint32_t previous_crc32;
    ...
    NET_EXPORT_PRIVATE SimpleSynchronousEntry(net::CacheType cache_type, const base::FilePath& path, const std::optional<std::string>& key, uint64_t entry_hash,
        SimpleFileTracker* simple_file_tracker, std::unique_ptr<UnboundBackendFileOperations> file_operations, int32_t stream_0_size);
    ...
    static void OpenEntry(net::CacheType cache_type, const base::FilePath& path, const std::optional<std::string>& key, uint64_t entry_hash,
    ...
    static void CreateEntry(net::CacheType cache_type, const base::FilePath& path, const std::string& key, uint64_t entry_hash, SimpleFileTracker* file_tracker,
        std::unique_ptr<UnboundBackendFileOperations> file_operations, SimpleEntryCreationResults* out_results);

    static void OpenOrCreateEntry(net::CacheType cache_type, const base::FilePath& path, const std::string& key, uint64_t entry_hash,
    ...
    static int DeleteEntryFiles(
        const base::FilePath& path, net::CacheType cache_type, uint64_t entry_hash, std::unique_ptr<UnboundBackendFileOperations> unbound_file_operations);
    ...
    // more.
    static int TruncateEntryFiles(const base::FilePath& path, uint64_t entry_hash, std::unique_ptr<UnboundBackendFileOperations> file_operations);
    ...
    static int DeleteEntrySetFiles(
        const std::vector<uint64_t>* key_hashes, const base::FilePath& path, std::unique_ptr<UnboundBackendFileOperations> unbound_file_operations);
    ...
    void ReadData(const ReadRequest& in_entry_op, SimpleEntryStat* entry_stat, net::IOBuffer* out_buf, ReadResult* out_result);
    void WriteData(const WriteRequest& in_entry_op, net::IOBuffer* in_buf, SimpleEntryStat* out_entry_stat, WriteResult* out_write_result);
    int CheckEOFRecord(BackendFileOperations* file_operations, base::File* file, int stream_index, const SimpleEntryStat& entry_stat, uint32_t expected_crc32);

    void ReadSparseData(const SparseRequest& in_entry_op, net::IOBuffer* out_buf, base::Time* out_last_used, int* out_result);
    ...
    void GetAvailableRange(const SparseRequest& in_entry_op, RangeResult* out_result);

    // Close all streams, and add write EOF records to streams indicated by the
    // CRCRecord entries in |crc32s_to_write|.
    void Close(const SimpleEntryStat& entry_stat, std::unique_ptr<std::vector<CRCRecord>> crc32s_to_write, net::GrowableIOBuffer* stream_0_data,
    ...
    NET_EXPORT_PRIVATE base::FilePath GetFilenameForSubfile(SimpleFileTracker::SubFile sub_file) const;
...
private:
    FRIEND_TEST_ALL_PREFIXES(::DiskCacheBackendTest, SimpleCacheEnumerationLongKeys);
    ...
    bool MaybeOpenFile(BackendFileOperations* file_operations, int file_index, base::File::Error* out_error);
    ...
    bool MaybeCreateFile(BackendFileOperations* file_operations, int file_index, FileRequired file_required, base::File::Error* out_error);
    bool OpenFiles(BackendFileOperations* file_operations, SimpleEntryStat* out_entry_stat);
    bool CreateFiles(BackendFileOperations* file_operations, SimpleEntryStat* out_entry_stat);
    ...
    int InitializeForOpen(BackendFileOperations* file_operations, SimpleEntryStat* out_entry_stat, SimpleStreamPrefetchData stream_prefetch_data[2]);
    ...
    int InitializeForCreate(BackendFileOperations* file_operations, SimpleEntryStat* out_entry_stat);
    ...
    int GetEOFRecordData(base::File* file, PrefetchData* prefetch_data, int file_index, int file_offset, SimpleFileEOF* eof_record);

    // Reads either from |file_0_prefetch| or |file|.
    // Range-checks all the in-memory reads.
    bool ReadFromFileOrPrefetched(base::File* file, PrefetchData* prefetch_data, int file_index, int offset, int size, char* dest);
    ...
    // |eof_record|.
    int PreReadStreamPayload(base::File* file, PrefetchData* prefetch_data, int stream_index, int extra_size, const SimpleEntryStat& entry_stat,
        const SimpleFileEOF& eof_record, SimpleStreamPrefetchData* out);

    // Opens the sparse data file and scans it if it exists.
    bool OpenSparseFileIfExists(BackendFileOperations* file_operations, int32_t* out_sparse_data_size);
    ...
    // verifies the CRC32.
    bool ReadSparseRange(base::File* sparse_file, const SparseRange* range, int offset, int len, char* buf);
    ...
    bool WriteSparseRange(base::File* sparse_file, SparseRange* range, int offset, int len, const char* buf);

    // Appends a new sparse range to the sparse data file.
    bool AppendSparseRange(base::File* sparse_file, int64_t offset, int len, const char* buf);

    static int DeleteEntryFilesInternal(const base::FilePath& path, net::CacheType cache_type, uint64_t entry_hash, BackendFileOperations* file_operations);

    static bool DeleteFileForEntryHash(const base::FilePath& path, uint64_t entry_hash, int file_index, BackendFileOperations* file_operations);
    static bool DeleteFilesForEntryHash(const base::FilePath& path, uint64_t entry_hash, BackendFileOperations* file_operations);
    static bool TruncateFilesForEntryHash(const base::FilePath& path, uint64_t entry_hash, BackendFileOperations* file_operations);
