...
struct NET_EXPORT_PRIVATE SimpleIndexLoadResult {
    ...
    SimpleIndex::IndexWriteToDiskReason index_write_reason = SimpleIndex::INDEX_WRITE_REASON_MAX;
...
public:
    ...
    public:
        IndexMetadata();
        IndexMetadata(SimpleIndex::IndexWriteToDiskReason reason, uint64_t entry_count, uint64_t cache_size);
        ...
        SimpleIndex::IndexWriteToDiskReason reason() const
        {
            return reason_;
        }
        ...
        bool has_entry_in_memory_data() const
        {
            return version_ >= 8;
        }
        bool app_cache_has_trailer_prefetch_size() const
        {
            return version_ >= 9;
        }
    ...
    SimpleIndexFile(scoped_refptr<base::SequencedTaskRunner> cache_runner, scoped_refptr<BackendFileOperationsFactory> file_operations_factory,
    ...
    virtual void LoadIndexEntries(base::Time cache_last_modified, base::OnceClosure callback, SimpleIndexLoadResult* out_result);

    // Writes the specified set of entries to disk.
    virtual void WriteToDisk(net::CacheType cache_type, SimpleIndex::IndexWriteToDiskReason reason, const SimpleIndex::EntrySet& entry_set, uint64_t cache_size,
...
private:
    ...
    using EntryFileCallback = base::RepeatingCallback<void(const base::FilePath&, base::Time last_accessed, base::Time last_modified, int64_t size)>;
    ...
    static void SyncLoadIndexEntries(std::unique_ptr<BackendFileOperations> file_operations, net::CacheType cache_type, base::Time cache_last_modified,
    ...
    static void SyncLoadFromDisk(BackendFileOperations* file_operations, net::CacheType cache_type, const base::FilePath& index_filename,
    ...
    // worker thread.
    static void SerializeFinalData(base::Time cache_modified, base::Pickle* pickle);

    // Given the contents of an index file |data| of length |data_len|, returns
    // the corresponding EntrySet. Returns NULL on error.
    static void Deserialize(net::CacheType cache_type, const char* data, int data_len, base::Time* out_cache_last_modified, SimpleIndexLoadResult* out_result);

    // Writes the index file to disk atomically.
    static void SyncWriteToDisk(std::unique_ptr<BackendFileOperations> file_operations, net::CacheType cache_type, const base::FilePath& cache_directory,
    ...
    // found.
    static void SyncRestoreFromDisk(BackendFileOperations* file_operations, net::CacheType cache_type, const base::FilePath& cache_directory,
    ...
    static bool LegacyIsIndexFileStale(BackendFileOperations* file_operations, base::Time cache_last_modified, const base::FilePath& index_file_path);
