...
TEST(SimpleVersionUpgradeTest, FailsToMigrateBackwards)
{
    ...
    EXPECT_EQ(disk_cache::SimpleCacheConsistencyResult::kVersionFromTheFuture, disk_cache::UpgradeSimpleCacheOnDisk(&file_operations, cache_dir.GetPath()));
}
...
TEST(SimpleVersionUpgradeTest, ExperimentBacktoDefault)
{
    ...
    // having one.
    EXPECT_EQ(disk_cache::SimpleCacheConsistencyResult::kBadZeroCheck, disk_cache::UpgradeSimpleCacheOnDisk(&file_operations, cache_dir.GetPath()));
}
...
TEST(SimpleVersionUpgradeTest, FakeIndexVersionGetsUpdated)
{
    ...
    disk_cache::TrivialFileOperations file_operations;
    // Upgrade.
    ASSERT_EQ(disk_cache::SimpleCacheConsistencyResult::kOK, disk_cache::UpgradeSimpleCacheOnDisk(&file_operations, cache_path));

    // Check that the version in the fake index file is updated.
    std::string new_fake_index_contents;
    ASSERT_TRUE(base::ReadFileToString(cache_path.AppendASCII(kFakeIndexFileName), &new_fake_index_contents));
    const disk_cache::FakeIndexData* fake_index_header;
    EXPECT_EQ(sizeof(*fake_index_header), new_fake_index_contents.size());
    fake_index_header = reinterpret_cast<const disk_cache::FakeIndexData*>(new_fake_index_contents.data());
...
TEST(SimpleVersionUpgradeTest, UpgradeV5V6IndexMustDisappear)
{
    ...
    for (uint64_t entry_hash = 0; entry_hash < kEntries; ++entry_hash) {
        for (int index = 0; index < 3; ++index) {
            ...
            ASSERT_TRUE(base::WriteFile(cache_path.AppendASCII(file_name), entry_contents));
        }
    }
    ...
    for (uint64_t entry_hash = 0; entry_hash < kEntries; ++entry_hash) {
        for (int index = 0; index < 3; ++index) {
            ...
            EXPECT_TRUE(base::ReadFileToString(cache_path.AppendASCII(file_name), &real_contents));
