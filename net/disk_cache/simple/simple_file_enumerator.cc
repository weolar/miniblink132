...
std::optional<SimpleFileEnumerator::Entry> SimpleFileEnumerator::Next()
{
    ...
    while (true) {
        ...
        return std::make_optional<Entry>(std::move(path), file_info.size, file_info.last_accessed, file_info.last_modified);
    }
    dir_ = nullptr;
    return std::nullopt;
}
...
std::optional<SimpleFileEnumerator::Entry> SimpleFileEnumerator::Next()
{
    ...
    return std::make_optional<Entry>(std::move(path), info.GetSize(),
        /*last_accessed=*/base::Time(), info.GetLastModifiedTime());
}
