...
class SimplePostOperationWaiterTable : public base::RefCounted<SimplePostOperationWaiterTable> {
    friend class base::RefCounted<SimplePostOperationWaiterTable>;

public:
    SimplePostOperationWaiterTable();

    SimplePostOperationWaiterTable(const SimplePostOperationWaiterTable&) = delete;
    SimplePostOperationWaiterTable& operator=(const SimplePostOperationWaiterTable&) = delete;
...
private:
    ...
    std::unordered_map<uint64_t, std::vector<base::OnceClosure>> entries_pending_operation_;
};
