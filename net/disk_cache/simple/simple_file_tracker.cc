...
{
    UMA_HISTOGRAM_ENUMERATION("SimpleCache.FileDescriptorLimiterAction", op, FD_LIMIT_OP_MAX);
}
...
void SimpleFileTracker::Register(const SimpleSynchronousEntry* owner, SubFile subfile, std::unique_ptr<base::File> file)
{
    DCHECK(file->IsValid());
...

    {
        ...
        auto insert_status = tracked_files_.emplace(owner->entry_file_key().entry_hash, std::vector<std::unique_ptr<TrackedFiles>>());

        std::vector<std::unique_ptr<TrackedFiles>>& candidates = insert_status.first->second;
...
SimpleFileTracker::FileHandle SimpleFileTracker::Acquire(BackendFileOperations* file_operations, const SimpleSynchronousEntry* owner, SubFile subfile)
{
    std::vector<std::unique_ptr<base::File>> files_to_close;

    {
        ...
        return FileHandle(this, owner, subfile, owners_files->files[file_index].get());
    }
}
...
void SimpleFileTracker::Release(const SimpleSynchronousEntry* owner, SubFile subfile)
{
    std::vector<std::unique_ptr<base::File>> files_to_close;

    {
        ...
        DCHECK(owners_files->state[file_index] == TrackedFiles::TF_ACQUIRED || owners_files->state[file_index] == TrackedFiles::TF_ACQUIRED_PENDING_CLOSE);

        // Prepare to executed deferred close, if any.
        if (owners_files->state[file_index] == TrackedFiles::TF_ACQUIRED_PENDING_CLOSE) {
...
void SimpleFileTracker::Close(const SimpleSynchronousEntry* owner, SubFile subfile)
{
...

    {
        ...
        DCHECK(owners_files->state[file_index] == TrackedFiles::TF_ACQUIRED || owners_files->state[file_index] == TrackedFiles::TF_REGISTERED);
...
void SimpleFileTracker::Doom(const SimpleSynchronousEntry* owner, EntryFileKey* key)
{
    base::AutoLock hold_lock(lock_);
    ...
    for (const std::unique_ptr<TrackedFiles>& file_with_same_hash : iter->second) {
        max_doom_gen = std::max(max_doom_gen, file_with_same_hash->key.doom_generation);
    }
...
SimpleFileTracker::TrackedFiles* SimpleFileTracker::Find(const SimpleSynchronousEntry* owner)
{
...
std::unique_ptr<base::File> SimpleFileTracker::PrepareClose(TrackedFiles* owners_files, int file_index)
{
    std::unique_ptr<base::File> file_out = std::move(owners_files->files[file_index]);
...
void SimpleFileTracker::CloseFilesIfTooManyOpen(std::vector<std::unique_ptr<base::File>>* files_to_close)
{
    auto i = lru_.end();
    while (open_files_ > file_limit_ && i != lru_.begin()) {
        ...
        for (int j = 0; j < kSimpleEntryTotalFileCount; ++j) {
            if (tracked_files->state[j] == TrackedFiles::TF_REGISTERED && tracked_files->files[j] != nullptr) {
...
void SimpleFileTracker::ReopenFile(BackendFileOperations* file_operations, TrackedFiles* owners_files, SubFile subfile)
{
    int file_index = static_cast<int>(subfile);
    ...
    base::FilePath file_path = owners_files->owner->GetFilenameForSubfile(subfile);
    owners_files->files[file_index] = std::make_unique<base::File>(file_operations->OpenFile(file_path, flags));
...
SimpleFileTracker::FileHandle::FileHandle(
    SimpleFileTracker* file_tracker, const SimpleSynchronousEntry* entry, SimpleFileTracker::SubFile subfile, base::File* file)
...
SimpleFileTracker::FileHandle& SimpleFileTracker::FileHandle::operator=(FileHandle&& other)
{
