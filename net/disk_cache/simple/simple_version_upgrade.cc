...
bool WriteFakeIndexFile(disk_cache::BackendFileOperations* file_operations, const base::FilePath& file_name)
{
    base::File file = file_operations->OpenFile(file_name, base::File::FLAG_CREATE | base::File::FLAG_WRITE);
    if (!file.IsValid())
        return false;
    ...
    if (!file.WriteAndCheck(0, base::byte_span_from_ref(file_contents))) {
        LOG(ERROR) << "Failed to write fake index file: " << file_name.LossyDisplayName();
        return false;
    }
    return true;
}
...
bool UpgradeIndexV5V6(BackendFileOperations* file_operations, const base::FilePath& cache_directory)
{
...
//    upgrade steps.
SimpleCacheConsistencyResult UpgradeSimpleCacheOnDisk(BackendFileOperations* file_operations, const base::FilePath& path)
{
    ...
    base::File fake_index_file = file_operations->OpenFile(fake_index, base::File::FLAG_OPEN | base::File::FLAG_READ);
    ...
    if (file_header.initial_magic_number != disk_cache::simplecache_v5::kSimpleInitialMagicNumber) {
...
bool DeleteIndexFilesIfCacheIsEmpty(const base::FilePath& path)
{
    ...
    base::FileEnumerator e(path, /* recursive = */ false, base::FileEnumerator::FILES | base::FileEnumerator::DIRECTORIES);
