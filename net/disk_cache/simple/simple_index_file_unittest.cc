...
{
    SimpleIndexFile::IndexMetadata index_metadata(SimpleIndex::INDEX_WRITE_REASON_SHUTDOWN, 123, 456);
    base::Pickle pickle;
...
public:
    ...
    V6IndexMetadataForTest(uint64_t entry_count, uint64_t cache_size)
        : SimpleIndexFile::IndexMetadata(SimpleIndex::INDEX_WRITE_REASON_SHUTDOWN, entry_count, cache_size)
    {
        version_ = 6;
    }
...
public:
    ...
    explicit WrappedSimpleIndexFile(const base::FilePath& index_file_directory)
        : SimpleIndexFile(
            base::SingleThreadTaskRunner::GetCurrentDefault(), base::MakeRefCounted<TrivialFileOperationsFactory>(), net::DISK_CACHE, index_file_directory)
    {
    }
    ...
    static bool LegacyIsIndexFileStale(base::Time cache_last_modified, const base::FilePath& index_file_path)
    {
        TrivialFileOperations ops;
        return SimpleIndexFile::LegacyIsIndexFileStale(&ops, cache_last_modified, index_file_path);
    }
};
...
public:
    ...
    {
        return a.last_used_time_seconds_since_epoch_ == b.last_used_time_seconds_since_epoch_ && a.entry_size_256b_chunks_ == b.entry_size_256b_chunks_
            && a.in_memory_data_ == b.in_memory_data_;
    }

    bool CompareTwoAppCacheEntryMetadata(const EntryMetadata& a, const EntryMetadata& b)
    {
        return a.trailer_prefetch_size_ == b.trailer_prefetch_size_ && a.entry_size_256b_chunks_ == b.entry_size_256b_chunks_
            && a.in_memory_data_ == b.in_memory_data_;
    }
};
...
TEST_F(SimpleIndexFileTest, Serialize)
{
    ...
    SimpleIndexFile::IndexMetadata index_metadata(SimpleIndex::INDEX_WRITE_REASON_SHUTDOWN, static_cast<uint64_t>(kNumHashes), 456);
    ...
    std::unique_ptr<base::Pickle> pickle = WrappedSimpleIndexFile::Serialize(net::DISK_CACHE, index_metadata, entries);
    ...
    WrappedSimpleIndexFile::Deserialize(net::DISK_CACHE, pickle->data_as_char(), pickle->size(), &when_index_last_saw_cache, &deserialize_result);
...
TEST_F(SimpleIndexFileTest, SerializeAppCache)
{
    ...
    for (size_t i = 0; i < kNumHashes; ++i) {
        uint64_t hash = kHashes[i];
        metadata_entries[i] = EntryMetadata(kTrailerPrefetches[i], static_cast<uint32_t>(hash));
        metadata_entries[i].SetInMemoryData(static_cast<uint8_t>(i));
        SimpleIndex::InsertInEntrySet(hash, metadata_entries[i], &entries);
    }

    std::unique_ptr<base::Pickle> pickle = WrappedSimpleIndexFile::Serialize(net::APP_CACHE, index_metadata, entries);
    EXPECT_TRUE(pickle.get() != nullptr);
    base::Time now = base::Time::Now();
    WrappedSimpleIndexFile::SerializeFinalData(now, pickle.get());
    base::Time when_index_last_saw_cache;
    SimpleIndexLoadResult deserialize_result;
    WrappedSimpleIndexFile::Deserialize(net::APP_CACHE, pickle->data_as_char(), pickle->size(), &when_index_last_saw_cache, &deserialize_result);
    EXPECT_TRUE(deserialize_result.did_load);
    EXPECT_EQ(now, when_index_last_saw_cache);
    const SimpleIndex::EntrySet& new_entries = deserialize_result.entries;
    EXPECT_EQ(entries.size(), new_entries.size());
    ...
    for (size_t i = 0; i < kNumHashes; ++i) {
        auto it = new_entries.find(kHashes[i]);
        EXPECT_TRUE(new_entries.end() != it);
        EXPECT_TRUE(CompareTwoAppCacheEntryMetadata(it->second, metadata_entries[i]));
    }
}
...
TEST_F(SimpleIndexFileTest, ReadV7Format)
{
    ...
    static_assert(std::size(kHashes) == std::size(kSizes), "Need same number of hashes and sizes");
    static const size_t kNumHashes = std::size(kHashes);
    ...
    std::unique_ptr<base::Pickle> pickle = WrappedSimpleIndexFile::Serialize(net::DISK_CACHE, v7_metadata, entries);
...
TEST_F(SimpleIndexFileTest, ReadV8Format)
{
    ...
    SimpleIndex::EntrySet entries;
    for (size_t i = 0; i < kNumHashes; ++i) {
        metadata_entries[i] = EntryMetadata(base::Time::Now(), static_cast<uint32_t>(kHashes[i]));
        metadata_entries[i].SetInMemoryData(static_cast<uint8_t>(i));
        SimpleIndex::InsertInEntrySet(kHashes[i], metadata_entries[i], &entries);
    }
    std::unique_ptr<base::Pickle> pickle = WrappedSimpleIndexFile::Serialize(net::DISK_CACHE, v8_metadata, entries);
    ASSERT_TRUE(pickle.get() != nullptr);
    base::Time now = base::Time::Now();
    WrappedSimpleIndexFile::SerializeFinalData(now, pickle.get());
...
TEST_F(SimpleIndexFileTest, ReadV8FormatAppCache)
{
    ...
    for (size_t i = 0; i < kNumHashes; ++i) {
        ...
        EXPECT_NE(metadata_entries[i].trailer_prefetch_size_, it->second.trailer_prefetch_size_);
        EXPECT_EQ(0, it->second.trailer_prefetch_size_);
        // Other data should be unaffected.
        EXPECT_EQ(metadata_entries[i].entry_size_256b_chunks_, it->second.entry_size_256b_chunks_);
...
TEST_F(SimpleIndexFileTest, LegacyIsIndexFileStale)
{
    ...
    EXPECT_TRUE(WrappedSimpleIndexFile::LegacyIsIndexFileStale(cache_mtime, index_path));
    const std::string kDummyData = "nothing to be seen here";
    EXPECT_TRUE(base::WriteFile(index_path, kDummyData));
    ASSERT_TRUE(base::GetFileInfo(cache_path, &file_info));
    cache_mtime = file_info.last_modified;
    EXPECT_FALSE(WrappedSimpleIndexFile::LegacyIsIndexFileStale(cache_mtime, index_path));
...
TEST_F(SimpleIndexFileTest, WriteThenLoadIndex)
{
    ...
    net::TestClosure closure;
    {
        ...
        simple_index_file.WriteToDisk(net::DISK_CACHE, SimpleIndex::INDEX_WRITE_REASON_SHUTDOWN, entries, kCacheSize, closure.closure());
    ...
    simple_index_file.LoadIndexEntries(fake_cache_mtime, closure.closure(), &load_index_result);
    closure.WaitForResult();
...
TEST_F(SimpleIndexFileTest, LoadCorruptIndex)
{
    ...
    EXPECT_TRUE(base::WriteFile(index_path, kDummyData));
    base::File::Info file_info;
    ASSERT_TRUE(base::GetFileInfo(simple_index_file.GetIndexFilePath(), &file_info));
    base::Time fake_cache_mtime = file_info.last_modified;
    EXPECT_FALSE(WrappedSimpleIndexFile::LegacyIsIndexFileStale(fake_cache_mtime, index_path));
    SimpleIndexLoadResult load_index_result;
    net::TestClosure closure;
    simple_index_file.LoadIndexEntries(fake_cache_mtime, closure.closure(), &load_index_result);
    closure.WaitForResult();
...
TEST_F(SimpleIndexFileTest, SimpleCacheUpgrade)
{
    ...
    base::File file(cache_path.AppendASCII("index"), base::File::FLAG_CREATE | base::File::FLAG_WRITE);
    ...
    int bytes_written = file.Write(0, reinterpret_cast<char*>(&file_contents), sizeof(file_contents));
    ...
    // Upgrade the cache.
    ASSERT_EQ(disk_cache::UpgradeSimpleCacheOnDisk(&file_operations, cache_path), SimpleCacheConsistencyResult::kOK);

    // Create the backend and initiate index flush by destroying the backend.
    scoped_refptr<disk_cache::BackendCleanupTracker> cleanup_tracker = disk_cache::BackendCleanupTracker::TryCreate(cache_path, base::OnceClosure());
    ...
    base::Time when_index_last_saw_cache;
    SimpleIndexLoadResult deserialize_result;
    WrappedSimpleIndexFile::Deserialize(net::DISK_CACHE, contents.data(), contents.size(), &when_index_last_saw_cache, &deserialize_result);
    EXPECT_TRUE(deserialize_result.did_load);
}
...
TEST_F(SimpleIndexFileTest, OverwritesStaleTempFile)
{
    ...
    net::TestClosure closure;
    simple_index_file.WriteToDisk(net::DISK_CACHE, SimpleIndex::INDEX_WRITE_REASON_SHUTDOWN, entries, 120U, closure.closure());
    closure.WaitForResult();
