...
public:
    ...
    {
        EXPECT_LT(kTestLastUsedTime - base::Seconds(2), entry_metadata.GetLastUsedTime());
        EXPECT_GT(kTestLastUsedTime + base::Seconds(2), entry_metadata.GetLastUsedTime());
...
public:
    explicit MockSimpleIndexFile(net::CacheType cache_type)
        : SimpleIndexFile(nullptr, base::MakeRefCounted<TrivialFileOperationsFactory>(), cache_type, base::FilePath())
    {
    }
    ...
    void WriteToDisk(net::CacheType cache_type, SimpleIndex::IndexWriteToDiskReason reason, const SimpleIndex::EntrySet& entry_set, uint64_t cache_size,
    ...
    SimpleIndexLoadResult* load_result() const
    {
        return load_result_;
    }
    int load_index_entries_calls() const
    {
        return load_index_entries_calls_;
    }
    int disk_writes() const
    {
        return disk_writes_;
    }
...
class SimpleIndexTest : public net::TestWithTaskEnvironment, public SimpleIndexDelegate {
protected:
    SimpleIndexTest()
        : hashes_(base::BindRepeating(&HashesInitializer))
    {
    }

    static uint64_t HashesInitializer(size_t hash_index)
    {
        return disk_cache::simple_util::GetEntryHashKey(base::StringPrintf("key%d", static_cast<int>(hash_index)));
    }
    ...
    void SetUp() override
    {
        ...
        index_ = std::make_unique<SimpleIndex>(/* io_thread = */ nullptr,
            /* cleanup_tracker = */ nullptr, this, CacheType(), std::move(index_file));
    ...
    void InsertIntoIndexFileReturn(uint64_t hash_key, base::Time last_used_time, int entry_size)
    {
        index_file_->load_result()->entries.emplace(hash_key, EntryMetadata(last_used_time, base::checked_cast<uint32_t>(entry_size)));
    }
    ...
    const MockSimpleIndexFile* index_file() const
    {
        return index_file_.get();
    }
    ...
    int doom_entries_calls() const
    {
        return doom_entries_calls_;
    }

    virtual net::CacheType CacheType() const
    {
...
TEST_F(EntryMetadataTest, SaturatedLastUsedTime)
{
    ...
    EXPECT_EQ(INT64_C(15939440895000000), entry_metadata.GetLastUsedTime().ToInternalValue());

    // Set a time that is too small to be represented by a unix timestamp (before
    // 1970).
    entry_metadata.SetLastUsedTime(base::Time::FromInternalValue(7u)); // This is a date in 1601.
    EXPECT_EQ(base::Time::UnixEpoch() + base::Seconds(1), entry_metadata.GetLastUsedTime());
}
...
TEST_F(EntryMetadataTest, Serialize)
{
    ...
    EXPECT_EQ(RoundSize(RoundSize(kTestEntrySize) | kTestEntryMemoryData), new_entry_metadata2.GetEntrySize());
...
TEST_F(SimpleIndexTest, IndexSizeCorrectOnMerge)
{
    ...
    EXPECT_EQ(9u * kSizeResolution, index()->cache_size_);
    {
        ...
        result->entries.emplace(new_hash_key, EntryMetadata(base::Time::Now(), 11u * kSizeResolution));
        const uint64_t redundant_hash_key = hashes_.at<4>();
        result->entries.emplace(redundant_hash_key, EntryMetadata(base::Time::Now(), 4u * kSizeResolution));
        index()->MergeInitializingSet(std::move(result));
    }
...
TEST_F(SimpleIndexTest, BasicInit)
{
    ...
    EXPECT_EQ(metadata.GetLastUsedTime(), index()->GetLastUsedTime(hashes_.at<1>()));
    EXPECT_LT(now - base::Days(2) - base::Seconds(1), metadata.GetLastUsedTime());
    EXPECT_GT(now - base::Days(2) + base::Seconds(1), metadata.GetLastUsedTime());
    EXPECT_EQ(RoundSize(10u), metadata.GetEntrySize());
    EXPECT_TRUE(GetEntryForTesting(hashes_.at<2>(), &metadata));
    EXPECT_EQ(metadata.GetLastUsedTime(), index()->GetLastUsedTime(hashes_.at<2>()));
...
TEST_F(SimpleIndexTest, DiskWritePostponed)
{
    ...
    EXPECT_TRUE(index()->HasPendingWrite());
    base::TimeTicks expected_trigger(index()->write_to_disk_timer_.desired_run_time());
