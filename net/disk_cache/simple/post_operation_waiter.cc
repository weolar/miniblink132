...
{
    auto [_, inserted] = entries_pending_operation_.emplace(entry_hash, std::vector<base::OnceClosure>());
    CHECK(inserted);
}
...
std::vector<base::OnceClosure>* SimplePostOperationWaiterTable::Find(uint64_t entry_hash)
{
