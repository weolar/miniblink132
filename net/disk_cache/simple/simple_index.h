...
public:
    EntryMetadata();
    EntryMetadata(base::Time last_used_time, base::StrictNumeric<uint32_t> entry_size);
    EntryMetadata(int32_t trailer_prefetch_size, base::StrictNumeric<uint32_t> entry_size);
    ...
    bool Deserialize(net::CacheType cache_type, base::PickleIterator* it, bool has_entry_in_memory_data, bool app_cache_has_trailer_prefetch_size);
...
public:
    ...
    SimpleIndex(const scoped_refptr<base::SequencedTaskRunner>& task_runner, scoped_refptr<BackendCleanupTracker> cleanup_tracker,
        SimpleIndexDelegate* delegate, net::CacheType cache_type, std::unique_ptr<SimpleIndexFile> simple_index_file);
    ...
    // entry.
    bool UpdateEntrySize(uint64_t entry_hash, base::StrictNumeric<uint32_t> entry_size);
    ...
    static bool InsertInEntrySet(uint64_t entry_hash, const EntryMetadata& entry_metadata, EntrySet* entry_set);
    ...
    void InsertEntryForTesting(uint64_t entry_hash, const EntryMetadata& entry_metadata);
    ...
    // initialized.
    uint64_t GetCacheSizeBetween(const base::Time initial_time, const base::Time end_time) const;
    ...
    IndexInitMethod init_method() const
    {
        return init_method_;
    }
...
private:
    ...
    bool UpdateEntryIteratorSize(EntrySet::iterator* it, base::StrictNumeric<uint32_t> entry_size);
    ...
    std::unique_ptr<base::android::ApplicationStatusListener> owned_app_status_listener_;
