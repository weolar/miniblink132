...
using TrackerMap = std::unordered_map<base::FilePath, raw_ptr<BackendCleanupTracker, CtnExperimental>>;
...
// static
scoped_refptr<BackendCleanupTracker> BackendCleanupTracker::TryCreate(const base::FilePath& path, base::OnceClosure retry_closure)
{
    AllBackendCleanupTrackers* all_trackers = g_all_trackers.Pointer();
    base::AutoLock lock(all_trackers->lock);

    std::pair<TrackerMap::iterator, bool> insert_result = all_trackers->map.insert(std::pair<base::FilePath, BackendCleanupTracker*>(path, nullptr));
    if (insert_result.second) {
        auto tracker = base::WrapRefCounted(new BackendCleanupTracker(path));
        insert_result.first->second = tracker.get();
        return tracker;
    } else {
        insert_result.first->second->AddPostCleanupCallbackImpl(std::move(retry_closure));
        return nullptr;
    }
}
...
{
    post_cleanup_cbs_.emplace_back(base::SequencedTaskRunner::GetCurrentDefault(), std::move(cb));
}
...
BackendCleanupTracker::~BackendCleanupTracker()
{
    ...
    while (!post_cleanup_cbs_.empty()) {
        post_cleanup_cbs_.back().first->PostTask(FROM_HERE, std::move(post_cleanup_cbs_.back().second));
