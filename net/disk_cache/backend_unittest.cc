...
#define MAYBE_NonEmptyCorruptSimpleCacheDoesNotRecover DISABLED_NonEmptyCorruptSimpleCacheDoesNotRecover
...
#define MAYBE_NonEmptyCorruptSimpleCacheDoesNotRecover NonEmptyCorruptSimpleCacheDoesNotRecover
...
protected:
    ...
    bool EnumerateAndMatchKeys(int max_to_open, TestIterator* iter, std::set<std::string>* keys_to_match, size_t* count);
    ...
    void BackendCalculateSizeOfEntriesBetween(bool expect_access_time_range_comparisons);
    ...
    void Test2GiBLimit(net::CacheType type, net::BackendType backend_type, bool expect_limit);
};

void DiskCacheBackendTest::CreateKeyAndCheck(disk_cache::Backend* cache, std::string key)
{
    ...
    disk_cache::EntryResult result = cache->CreateEntry(key, net::HIGHEST, cb_entry.callback());
...
void DiskCacheBackendTest::WaitForSimpleCacheIndexAndCheck(disk_cache::Backend* cache)
{
    net::TestCompletionCallback wait_for_index_cb;
    static_cast<disk_cache::SimpleBackendImpl*>(cache)->index()->ExecuteWhenReady(wait_for_index_cb.callback());
...
int DiskCacheBackendTest::GeneratePendingIO(net::TestCompletionCallback* cb)
{
    ...
    entry_result = cache_->CreateEntry("some key", net::HIGHEST, create_cb.callback());
    ...
    for (int i = 0; i < 10 * 1024 * 1024; i += 64 * 1024) {
        ...
        if (!simple_cache_mode_) {
            rv = static_cast<disk_cache::EntryImpl*>(entry)->WriteDataImpl(0, i, buffer.get(), kSize, cb->callback(), false);
        } else {
...
void DiskCacheBackendTest::InitSparseCache(base::Time* doomed_start, base::Time* doomed_end)
{
    InitCache();
    ...
    ASSERT_EQ(kSize, WriteSparseData(entry0, kOffset + kSize, buffer.get(), kSize));
    entry0->Close();
    ...
    ASSERT_EQ(kSize, WriteSparseData(entry1, kOffset + kSize, buffer.get(), kSize));
    entry1->Close();

    ASSERT_THAT(CreateEntry("second", &entry2), IsOk());
    ASSERT_EQ(kSize, WriteSparseData(entry2, 0, buffer.get(), kSize));
    ASSERT_EQ(kSize, WriteSparseData(entry2, kOffset + kSize, buffer.get(), kSize));
    entry2->Close();
    ...
    ASSERT_EQ(kSize, WriteSparseData(entry3, kOffset + kSize, buffer.get(), kSize));
    ASSERT_EQ(kSize, WriteSparseData(entry4, kOffset + kSize, buffer.get(), kSize));
    entry3->Close();
    entry4->Close();
...
bool DiskCacheBackendTest::CreateSetOfRandomEntries(std::set<std::string>* key_pool)
{
...
bool DiskCacheBackendTest::EnumerateAndMatchKeys(int max_to_open, TestIterator* iter, std::set<std::string>* keys_to_match, size_t* count)
{
    disk_cache::Entry* entry;
...
TEST_F(DiskCacheTest, CreateBackend)
{
...

    {
        ...
        disk_cache::BackendResult rv = disk_cache::CreateCacheBackend(net::DISK_CACHE, net::CACHE_BACKEND_DEFAULT,
            /*file_operations=*/nullptr, cache_path_, 0, disk_cache::ResetHandling::kNeverReset, nullptr, cb.callback());
        rv = cb.GetResult(std::move(rv));
        ASSERT_THAT(rv.net_error, IsOk());
        ASSERT_TRUE(rv.backend);
        rv.backend.reset();

        rv = disk_cache::CreateCacheBackend(net::MEMORY_CACHE, net::CACHE_BACKEND_DEFAULT,
            /*file_operations=*/nullptr, base::FilePath(), 0, disk_cache::ResetHandling::kNeverReset, nullptr, cb.callback());
        rv = cb.GetResult(std::move(rv));
        ASSERT_THAT(rv.net_error, IsOk());
        ASSERT_TRUE(rv.backend);
        rv.backend.reset();
...
TEST_F(DiskCacheTest, MemBackendPostCleanupCallback)
{
    ...
    disk_cache::BackendResult rv = disk_cache::CreateCacheBackend(net::MEMORY_CACHE, net::CACHE_BACKEND_DEFAULT,
        /*file_operations=*/nullptr, base::FilePath(), 0, disk_cache::ResetHandling::kNeverReset, nullptr, on_cleanup.closure(), cb.callback());
    rv = cb.GetResult(std::move(rv));
    ASSERT_THAT(rv.net_error, IsOk());
    ASSERT_TRUE(rv.backend);
...
TEST_F(DiskCacheTest, CreateBackendDouble)
{
    ...
    disk_cache::BackendResult rv = disk_cache::CreateCacheBackend(net::APP_CACHE, net::CACHE_BACKEND_DEFAULT, /*file_operations=*/nullptr, cache_path_, 0,
        disk_cache::ResetHandling::kNeverReset,
        /*net_log=*/nullptr, cb.callback());

    disk_cache::BackendResult rv2 = disk_cache::CreateCacheBackend(net::APP_CACHE, net::CACHE_BACKEND_DEFAULT, /*file_operations=*/nullptr, cache_path_, 0,
        disk_cache::ResetHandling::kNeverReset,
...
TEST_F(DiskCacheBackendTest, CreateBackendDoubleOpenEntry)
{
    ...
    disk_cache::BackendResult rv = disk_cache::CreateCacheBackend(net::APP_CACHE, net::CACHE_BACKEND_SIMPLE, /*file_operations=*/nullptr, cache_path_, 0,
        disk_cache::ResetHandling::kNeverReset,
        /*net_log=*/nullptr, cb.callback());

    disk_cache::BackendResult rv2 = disk_cache::CreateCacheBackend(net::APP_CACHE, net::CACHE_BACKEND_SIMPLE, /*file_operations=*/nullptr, cache_path_, 0,
        disk_cache::ResetHandling::kNeverReset,
        /*net_log=*/nullptr, cb2.callback());
    ...
    EntryResult entry_result = rv.backend->CreateEntry("key", net::HIGHEST, cb3.callback());
...
TEST_F(DiskCacheBackendTest, CreateBackendPostCleanup)
{
    ...
    EntryResult result = rv.backend->CreateEntry("key", net::HIGHEST, cb2.callback());
    ...
    base::FilePath entry_path = cache_path_.AppendASCII(disk_cache::simple_util::GetFilenameFromKeyAndFileIndex("key", 0));
...
TEST_F(DiskCacheBackendTest, SimpleCreateBackendRecoveryAppCache)
{
    ...
    // Create an entry.
    TestEntryResultCompletionCallback cb2;
    disk_cache::EntryResult result = rv.backend->CreateEntry("key", net::HIGHEST, cb2.callback());
    result = cb2.GetResult(std::move(result));
    ASSERT_EQ(net::OK, result.net_error());
    disk_cache::Entry* entry = result.ReleaseEntry();
    EXPECT_EQ(kBufSize, WriteData(entry, 0, 0, buffer.get(), kBufSize, false));
    ...
    // Delete the index.
    base::DeleteFile(cache_path_.AppendASCII("index-dir").AppendASCII("the-real-index"));
    ...
    // such).
    EXPECT_EQ(0, simple_cache_impl_->index()->GetTrailerPrefetchSize(disk_cache::simple_util::GetEntryHashKey("key")));
}
...
TEST_F(DiskCacheBackendTest, CreateBackend_MissingFile)
{
    ...
    std::unique_ptr<disk_cache::BackendImpl> cache(std::make_unique<disk_cache::BackendImpl>(cache_path_, nullptr, nullptr, net::DISK_CACHE, nullptr));
...
TEST_F(DiskCacheBackendTest, MemoryListensToMemoryPressure)
{
    ...
    for (int i = 0; i < 0.9 * (kLimit / kEntrySize); ++i) {
        ...
        EXPECT_EQ(kEntrySize, WriteData(entry, 0, 0, buffer.get(), kEntrySize, true));
        entry->Close();
    }
    ...
    base::MemoryPressureListener::NotifyMemoryPressure(base::MemoryPressureListener::MEMORY_PRESSURE_LEVEL_CRITICAL);
    base::RunLoop().RunUntilIdle();
...
TEST_F(DiskCacheBackendTest, ExternalFiles)
{
    ...
    ASSERT_TRUE(base::WriteFile(filename, std::string_view(buffer1->data(), static_cast<size_t>(kSize))));
...
TEST_F(DiskCacheBackendTest, MultipleInstancesWithPendingFileIO)
{
    ...
    disk_cache::BackendResult backend_rv = disk_cache::CreateCacheBackend(net::DISK_CACHE, net::CACHE_BACKEND_DEFAULT, /*file_operations=*/nullptr,
...
void DiskCacheBackendTest::BackendShutdownWithPendingIO(bool fast)
{
...

    {
        ...
        EntryResult result = cache_->CreateEntry("some key", net::HIGHEST, cb.callback());
...
void DiskCacheBackendTest::BackendShutdownWithPendingDoom()
{
    net::TestCompletionCallback cb;
    {
        ...
        TestEntryResultCompletionCallback cb2;
        EntryResult result = cache_->CreateEntry("some key", net::HIGHEST, cb2.callback());
        result = cb2.GetResult(std::move(result));
        ASSERT_THAT(result.net_error(), IsOk());
        result.ReleaseEntry()->Close();
...
TEST_F(DiskCacheTest, TruncatedIndex)
{
    ...
    disk_cache::BackendResult rv = disk_cache::CreateCacheBackend(net::DISK_CACHE, net::CACHE_BACKEND_BLOCKFILE,
        /*file_operations=*/nullptr, cache_path_, 0, disk_cache::ResetHandling::kNeverReset, /*net_log=*/nullptr, cb.callback());
    rv = cb.GetResult(std::move(rv));
...
void DiskCacheBackendTest::BackendSetSize()
{
    ...
    EXPECT_EQ(cache_size / 10, WriteData(entry, 0, 0, buffer.get(), cache_size / 10, false)) << "normal file";

    EXPECT_EQ(net::ERR_FAILED, WriteData(entry, 1, 0, buffer.get(), cache_size / 5, false)) << "file size above the limit";
    entry->Close();
    ...
    InitCache();
    ASSERT_THAT(CreateEntry(first, &entry), IsOk());
    EXPECT_EQ(cache_size / 5, WriteData(entry, 1, 0, buffer.get(), cache_size / 5, false));
    entry->Close();
    ...
    for (int i = 0; i < (95 / 5); ++i) {
        ...
        EXPECT_EQ(cache_size / 20, WriteData(entry, 0, 0, buffer.get(), cache_size / 20, false));
        entry->Close();
    }

    ASSERT_THAT(CreateEntry(second, &entry), IsOk());
    EXPECT_EQ(cache_size / 10, WriteData(entry, 0, 0, buffer.get(), cache_size / 10, false));

    disk_cache::Entry* entry2;
    ASSERT_THAT(CreateEntry("an extra key", &entry2), IsOk());
    EXPECT_EQ(cache_size / 10, WriteData(entry2, 0, 0, buffer.get(), cache_size / 10, false));
...
void DiskCacheBackendTest::BackendFixEnumerators()
{
    ...
    disk_cache::Entry *entry1, *entry2;
    std::unique_ptr<TestIterator> iter1 = CreateIterator(), iter2 = CreateIterator();
...
TEST_F(DiskCacheBackendTest, InMemorySparseEvict)
{
    ...
    ASSERT_EQ(net::ERR_FAILED, WriteSparseData(entries[0].get(), 10000, buffer.get(), 4));
...
void DiskCacheBackendTest::BackendCalculateSizeOfAllEntries()
{
    ...
    for (std::string key : key_pool) {
        ...
        scoped_refptr<net::StringIOBuffer> buffer = base::MakeRefCounted<net::StringIOBuffer>(data);
    ...
    // if the size is back to original value.
    {
        ...
        std::string key = GenerateKey(true);
        ASSERT_THAT(CreateEntry(key, &entry), IsOk());
        ASSERT_EQ(last_entry_size, WriteData(entry, 0, 0, buffer.get(), last_entry_size, true));
        entry->Close();

        int new_result = CalculateSizeOfAllEntries();
        EXPECT_EQ(result + GetRoundedSize(last_entry_size + GetEntryMetadataSize(key)), new_result);
...
void DiskCacheBackendTest::BackendCalculateSizeOfEntriesBetween(bool expect_access_time_comparisons)
{
    InitCache();
    ...
    ASSERT_EQ(CalculateSizeOfAllEntries(), CalculateSizeOfEntriesBetween(base::Time(), base::Time::Max()));
...
{
    InitCache();
    ASSERT_EQ(net::ERR_NOT_IMPLEMENTED, CalculateSizeOfEntriesBetween(base::Time(), base::Time::Max()));
}
...
void DiskCacheBackendTest::BackendTransaction(const std::string& name, int num_entries, bool load)
{
    success_ = false;
...
TEST_F(DiskCacheBackendTest, DeleteOld)
{
    ...
    TestBackendResultCompletionCallback cb;
    {
        ...
        disk_cache::BackendResult rv = disk_cache::CreateCacheBackend(net::DISK_CACHE, net::CACHE_BACKEND_BLOCKFILE,
            /*file_operations=*/nullptr, path, 0, disk_cache::ResetHandling::kResetOnError, /*net_log=*/nullptr, cb.callback());
        path.clear(); // Make sure path was captured by the previous call.
        rv = cb.GetResult(std::move(rv));
        ASSERT_THAT(rv.net_error, IsOk());
    }
    EXPECT_TRUE(CheckCacheIntegrity(cache_path_, new_eviction_, /*max_size = */ 0, mask_));
}
#endif
...
void DiskCacheBackendTest::BackendEviction()
{
    ...
    static_assert(kWriteEntryCount * kWriteSize > kMaxSize, "must write more than MaxSize");
    ...
    for (int i = 0; i < kWriteEntryCount; ++i) {
        AddDelay();
        disk_cache::Entry* entry = nullptr;
        ASSERT_THAT(CreateEntry(key_prefix + base::NumberToString(i), &entry), IsOk());
...
TEST_F(DiskCacheBackendTest, MemoryOnlyUseAfterFree)
{
    ...
    first_parent->WriteSparseData(32768, buffer.get(), 1024, net::CompletionOnceCallback());
}
...
TEST_F(DiskCacheBackendTest, MemoryCapsWritesToMaxSize)
{
    ...
    EXPECT_THAT(WriteData(final_entry.get(), 1, 0, buffer.get(), kWriteSize, false), IsError(net::ERR_INSUFFICIENT_RESOURCES));
}
...
TEST_F(DiskCacheTest, Backend_UsageStatsTimer)
{
    ...
    std::unique_ptr<disk_cache::BackendImpl> cache(
        std::make_unique<disk_cache::BackendImpl>(cache_path_, nullptr, base::SingleThreadTaskRunner::GetCurrentDefault(), net::DISK_CACHE, nullptr));
...
TEST_F(DiskCacheTest, MultipleInstances)
{
    ...
    disk_cache::BackendResult rv = disk_cache::CreateCacheBackend(net::DISK_CACHE, net::CACHE_BACKEND_DEFAULT, /*file_operations=*/nullptr, store1.GetPath(), 0,
        disk_cache::ResetHandling::kNeverReset,
        /*net_log=*/nullptr, cb.callback());
    rv = cb.GetResult(std::move(rv));
    ASSERT_THAT(rv.net_error, IsOk());
    caches[0] = std::move(rv.backend);
    rv = disk_cache::CreateCacheBackend(net::GENERATED_BYTE_CODE_CACHE, net::CACHE_BACKEND_DEFAULT,
        /*file_operations=*/nullptr, store2.GetPath(), 0, disk_cache::ResetHandling::kNeverReset, /*net_log=*/nullptr, cb.callback());
    rv = cb.GetResult(std::move(rv));
    ASSERT_THAT(rv.net_error, IsOk());
...
TEST_F(DiskCacheTest, AutomaticMaxSize)
{
    ...
    EXPECT_EQ((kDefaultCacheSize - 1) * 8 / 10, disk_cache::PreferredCacheSize(large_size - 1));
    EXPECT_EQ(kDefaultCacheSize * 8 / 10, disk_cache::PreferredCacheSize(large_size));
    EXPECT_EQ(kDefaultCacheSize - 1, disk_cache::PreferredCacheSize(large_size * 10 / 8 - 1));

    // Region 2: expected = default_size
    EXPECT_EQ(kDefaultCacheSize, disk_cache::PreferredCacheSize(large_size * 10 / 8));
    EXPECT_EQ(kDefaultCacheSize, disk_cache::PreferredCacheSize(large_size * 10 - 1));

    // Region 3: expected = available * 0.1
    EXPECT_EQ(kDefaultCacheSize, disk_cache::PreferredCacheSize(large_size * 10));
    EXPECT_EQ((kDefaultCacheSize * 25 - 1) / 10, disk_cache::PreferredCacheSize(large_size * 25 - 1));

    // Region 4: expected = default_size * 2.5
    EXPECT_EQ(kDefaultCacheSize * 25 / 10, disk_cache::PreferredCacheSize(large_size * 25));
    EXPECT_EQ(kDefaultCacheSize * 25 / 10, disk_cache::PreferredCacheSize(large_size * 100 - 1));
    EXPECT_EQ(kDefaultCacheSize * 25 / 10, disk_cache::PreferredCacheSize(large_size * 100));
    EXPECT_EQ(kDefaultCacheSize * 25 / 10, disk_cache::PreferredCacheSize(large_size * 250 - 1));

    // Region 5: expected = available * 0.1
    int64_t largest_size = kDefaultCacheSize * 4;
    EXPECT_EQ(kDefaultCacheSize * 25 / 10, disk_cache::PreferredCacheSize(large_size * 250));
    EXPECT_EQ(largest_size - 1, disk_cache::PreferredCacheSize(largest_size * 100 - 1));
...
TEST_F(DiskCacheBackendTest, TotalBuffersSize1)
{
    ...
    for (int i = 0; i < 10; i++) {
        ...
        EXPECT_EQ(kSize, WriteData(entry, 0, 1024 * 1024, buffer.get(), kSize, false));
        EXPECT_EQ(kSize, WriteData(entry, 1, 1024 * 1024, buffer.get(), kSize, false));
...
TEST_F(DiskCacheBackendTest, FileSharing)
{
...

    {
        ...
        base::win::ScopedHandle file2(CreateFile(name.value().c_str(), access, sharing, nullptr, OPEN_EXISTING, 0, nullptr));
        EXPECT_FALSE(file2.IsValid());

        sharing |= FILE_SHARE_DELETE;
        file2.Set(CreateFile(name.value().c_str(), access, sharing, nullptr, OPEN_EXISTING, 0, nullptr));
...
TEST_F(DiskCacheBackendTest, SimpleCacheOpenMissingFile)
{
    ...
    base::FilePath to_delete_file = cache_path_.AppendASCII(disk_cache::simple_util::GetFilenameFromKeyAndFileIndex(key, 0));
    ...
    for (int i = 1; i < disk_cache::kSimpleEntryNormalFileCount; ++i) {
        base::FilePath should_be_gone_file(cache_path_.AppendASCII(disk_cache::simple_util::GetFilenameFromKeyAndFileIndex(key, i)));
...
TEST_F(DiskCacheBackendTest, SimpleCacheOpenBadFile)
{
    ...
    base::FilePath entry_file1_path = cache_path_.AppendASCII(disk_cache::simple_util::GetFilenameFromKeyAndFileIndex(key, 0));

    disk_cache::SimpleFileHeader header;
    header.initial_magic_number = UINT64_C(0xbadf00d);
    EXPECT_TRUE(base::WriteFile(entry_file1_path, base::byte_span_from_ref(header)));
    ASSERT_THAT(OpenEntry(key, &entry), IsError(net::ERR_FAILED));
}
...
TEST_F(DiskCacheBackendTest, SimpleCacheOverBlockfileCache)
{
    ...
    auto simple_cache = std::make_unique<disk_cache::SimpleBackendImpl>(
        /*file_operations_factory=*/nullptr, cache_path_, nullptr, nullptr, 0, net::DISK_CACHE, nullptr);
    net::TestCompletionCallback cb;
...
TEST_F(DiskCacheBackendTest, BlockfileCacheOverSimpleCache)
{
    ...
    auto cache = std::make_unique<disk_cache::BackendImpl>(cache_path_, nullptr, nullptr, net::DISK_CACHE, nullptr);
    cache->SetUnitTestMode();
    net::TestCompletionCallback cb;
    cache->Init(cb.callback());
    EXPECT_NE(net::OK, cb.WaitForResult());
...
TEST_F(DiskCacheBackendTest, SimpleCacheEnumerationBasics)
{
    ...
    ASSERT_TRUE(EnumerateAndMatchKeys(key_pool.size() / 2, iter.get(), &keys_to_match, &count));
...
TEST_F(DiskCacheBackendTest, SimpleCacheEnumerationCorruption)
{
    ...
    auto buffer = base::MakeRefCounted<net::IOBufferWithSize>(kSize);
    CacheTestFillBuffer(buffer->data(), kSize, false);
    ASSERT_EQ(kSize, WriteData(corrupted_entry, 0, 0, buffer.get(), kSize, false));
...
TEST_F(DiskCacheBackendTest, DISABLED_SimpleCachePrioritizedEntryOrder)
{
    ...
    // priority order.
    disk_cache::SimpleBackendImpl* simple_cache = static_cast<disk_cache::SimpleBackendImpl*>(cache_.get());
    auto task_runner = base::ThreadPool::CreateSequencedTaskRunner({ base::TaskPriority::USER_VISIBLE, base::MayBlock() });
    ...
    // the worker pool.
    base::RunLoop run_loop;
    task_runner->PostTaskAndReply(FROM_HERE, base::DoNothing(), run_loop.QuitClosure());
    run_loop.Run();

    std::vector<int> finished_read_order;
    auto finished_callback = [](std::vector<int>* finished_read_order, int entry_number, base::OnceClosure quit_closure, int rv) {
    ...
    entry2->ReadData(2, 0, read_buf2.get(), kSize, base::BindOnce(finished_callback, &finished_read_order, 2, read_run_loop.QuitClosure()));
    entry3->ReadData(2, 0, read_buf3.get(), kSize, base::BindOnce(finished_callback, &finished_read_order, 3, base::OnceClosure()));
    entry1->ReadData(2, 0, read_buf1.get(), kSize, base::BindOnce(finished_callback, &finished_read_order, 1, base::OnceClosure()));
...
TEST_F(DiskCacheBackendTest, SimpleCacheLateDoom)
{
    ...
    base::File cache_dir(cache_path_, base::File::FLAG_OPEN | base::File::FLAG_READ);
    ...
    DisableFirstCleanup();
    InitCache();
    EXPECT_EQ(disk_cache::SimpleIndex::INITIALIZE_METHOD_LOADED, simple_cache_impl_->index()->init_method());
}
...
TEST_F(DiskCacheBackendTest, SimpleCacheNegMaxSize)
{
    ...
    EXPECT_NE(simple_cache_impl_->index()->max_size(), std::numeric_limits<uint64_t>::max());

    int max_default_size = 2 * disk_cache::PreferredCacheSize(std::numeric_limits<int32_t>::max());

    ASSERT_GE(max_default_size, 0);
    EXPECT_LT(simple_cache_impl_->index()->max_size(), static_cast<unsigned>(max_default_size));
    ...
    // between max_size_without_scaling and max_size_without_scaling*2.
    {
        ...
        scoped_feature_list.InitAndEnableFeatureWithParameters(disk_cache::kChangeDiskCacheSizeExperiment, field_trial_params);
...
TEST_F(DiskCacheBackendTest, SimpleLastModified)
{
    ...
    while (base::Time::NowFromSystemTime() <= (entry1_timestamp + base::Milliseconds(10))) {
...
TEST_F(DiskCacheBackendTest, SimpleFdLimit)
{
    ...
    histogram_tester.ExpectBucketCount("SimpleCache.FileDescriptorLimiterAction", disk_cache::FD_LIMIT_CLOSE_FILE, kLargeNumEntries - 64);
    histogram_tester.ExpectBucketCount("SimpleCache.FileDescriptorLimiterAction", disk_cache::FD_LIMIT_REOPEN_FILE, 0);
    histogram_tester.ExpectBucketCount("SimpleCache.FileDescriptorLimiterAction", disk_cache::FD_LIMIT_FAIL_REOPEN_FILE, 0);
    ...
    histogram_tester.ExpectBucketCount("SimpleCache.FileDescriptorLimiterAction", disk_cache::FD_LIMIT_CLOSE_FILE, kLargeNumEntries - 64 + 1);
    histogram_tester.ExpectBucketCount("SimpleCache.FileDescriptorLimiterAction", disk_cache::FD_LIMIT_REOPEN_FILE, 0);
    histogram_tester.ExpectBucketCount("SimpleCache.FileDescriptorLimiterAction", disk_cache::FD_LIMIT_FAIL_REOPEN_FILE, 0);
    ...
    histogram_tester.ExpectBucketCount(
        "SimpleCache.FileDescriptorLimiterAction", disk_cache::FD_LIMIT_CLOSE_FILE, kLargeNumEntries - 64 + 1 + kLargeNumEntries - 1);
    histogram_tester.ExpectBucketCount("SimpleCache.FileDescriptorLimiterAction", disk_cache::FD_LIMIT_REOPEN_FILE, kLargeNumEntries - 1);
    histogram_tester.ExpectBucketCount("SimpleCache.FileDescriptorLimiterAction", disk_cache::FD_LIMIT_FAIL_REOPEN_FILE, 0);
    ...
    histogram_tester.ExpectBucketCount(
        "SimpleCache.FileDescriptorLimiterAction", disk_cache::FD_LIMIT_CLOSE_FILE, kLargeNumEntries - 64 + 1 + kLargeNumEntries - 1 + 2);
    histogram_tester.ExpectBucketCount("SimpleCache.FileDescriptorLimiterAction", disk_cache::FD_LIMIT_REOPEN_FILE, kLargeNumEntries + 1);
    histogram_tester.ExpectBucketCount("SimpleCache.FileDescriptorLimiterAction", disk_cache::FD_LIMIT_FAIL_REOPEN_FILE, 0);
    ...
    histogram_tester.ExpectBucketCount(
        "SimpleCache.FileDescriptorLimiterAction", disk_cache::FD_LIMIT_CLOSE_FILE, kLargeNumEntries - 64 + 1 + kLargeNumEntries - 1 + 2);
    histogram_tester.ExpectBucketCount(
        "SimpleCache.FileDescriptorLimiterAction", disk_cache::FD_LIMIT_REOPEN_FILE, kLargeNumEntries - 64 + 1 + kLargeNumEntries - 1 + 2);
    histogram_tester.ExpectBucketCount("SimpleCache.FileDescriptorLimiterAction", disk_cache::FD_LIMIT_FAIL_REOPEN_FILE, 0);
}
...
void DiskCacheBackendTest::Test2GiBLimit(net::CacheType type, net::BackendType backend_type, bool expect_limit)
{
    TestBackendResultCompletionCallback cb;
    ASSERT_TRUE(CleanupCacheDir());
    ...
    disk_cache::BackendResult rv = disk_cache::CreateCacheBackend(type, backend_type,
        /*file_operations=*/nullptr, cache_path_, size, disk_cache::ResetHandling::kNeverReset, nullptr, cb.callback());
    rv = cb.GetResult(std::move(rv));
    ASSERT_THAT(rv.net_error, IsOk());
    EXPECT_TRUE(rv.backend);
    rv.backend.reset();
    ...
    size += 1;
    rv = disk_cache::CreateCacheBackend(type, backend_type,
        /*file_operations=*/nullptr, cache_path_, size, disk_cache::ResetHandling::kNeverReset, nullptr, cb.callback());
    rv = cb.GetResult(std::move(rv));
    if (expect_limit) {
...
void DiskCacheBackendTest::BackendOpenOrCreateEntry()
{
    ...
    if (!memory_only_) {
        ...
        EntryResult result = cache_->OpenOrCreateEntry("first", net::HIGHEST, callback.callback());
...
{
    InitCache();
    std::unique_ptr<disk_cache::Backend::Iterator> iter = cache_->CreateIterator();
    ResetCaches();
...
void DiskCacheBackendTest::BackendIteratorConcurrentDoom()
{
    ...
    EXPECT_TRUE(result_iter2.net_error() == net::ERR_FAILED || result_iter2.net_error() == net::OK);
...
TEST_F(DiskCacheBackendTest, SimpleOwnershipTransferBackendDestroyRace)
{
    struct CleanupContext {
        explicit CleanupContext(bool* ran_ptr)
            : ran_ptr(ran_ptr)
        {
        }
        ~CleanupContext()
    ...
    EntryResult result = cache_->OpenEntry(kKey, net::HIGHEST,
        base::BindOnce(
...
TEST_F(DiskCacheBackendTest, SimpleCacheSoftResetKeepsValues)
{
    ...
    { // Do the second cache creation with no reset flag, preserving entries.
        ...
        EXPECT_TRUE(static_cast<disk_cache::SimpleBackendImpl*>(cache.get())->index()->Has(disk_cache::simple_util::GetEntryHashKey("key")));
    }
}
...
TEST_F(DiskCacheBackendTest, SimpleCacheHardResetDropsValues)
{
    ...
    { // Re-load cache with a reset flag, which should ignore existing entries.
        ...
        disk_cache::BackendResult rv = disk_cache::CreateCacheBackend(net::APP_CACHE, net::CACHE_BACKEND_SIMPLE, /*file_operations=*/nullptr, cache_path_, 0,
            disk_cache::ResetHandling::kReset, /*net_log=*/nullptr, cb.callback());
        rv = cb.GetResult(std::move(rv));
        EXPECT_THAT(rv.net_error, IsOk());
        std::unique_ptr<disk_cache::Backend> cache = std::move(rv.backend);
        ASSERT_TRUE(cache.get());
        ...
        EXPECT_FALSE(static_cast<disk_cache::SimpleBackendImpl*>(cache.get())->index()->Has(disk_cache::simple_util::GetEntryHashKey("key")));
...
TEST_F(DiskCacheBackendTest, SimpleCancelOpPendingDoom)
{
    ...
    EntryResult entry_result = cache_->CreateEntry(kKey, net::HIGHEST,
        base::BindOnce(
            [](std::unique_ptr<CleanupContext>, EntryResult result) { ADD_FAILURE() << "This should not actually run"; }, std::move(cleanup_context)));
...
TEST_F(DiskCacheBackendTest, SimpleDontLeakPostDoomCreate)
{
    ...
    EntryResult result = cache_->CreateEntry(kKey, net::LOWEST, base::DoNothing());
    ASSERT_EQ(net::OK, result.net_error());
...
TEST_F(DiskCacheBackendTest, BlockFileInsertAliasing)
{
    ...
    EXPECT_EQ(net::ERR_IO_PENDING, entry->WriteSparseData(8, buffer.get(), 64, cb_write64.callback()));

    net::TestCompletionCallback cb_write61k;
    EXPECT_EQ(net::ERR_IO_PENDING, entry->WriteSparseData(16773118, buffer.get(), 61188, cb_write61k.callback()));
    ...
    disk_cache::EntryResult result = cache_->CreateEntry(kKey, net::HIGHEST, cb_create1.callback());
...
TEST_F(DiskCacheBackendTest, MemCacheBackwardsClock)
{
    ...
    EXPECT_LE(2 * kBufSize, CalculateSizeOfEntriesBetween(base::Time(), base::Time::Max()));
...
TEST_F(DiskCacheBackendTest, SimpleOpenOrCreateIndexError)
{
    ...
    EXPECT_EQ(kBufSize,
        WriteData(entry, /*index=*/1, /*offset=*/0, buffer.get(),
            /*len=*/kBufSize, /*truncate=*/false));
    entry->Close();

    // Mess up the index to say it's not there.
    simple_cache_impl_->index()->Remove(disk_cache::simple_util::GetEntryHashKey(kKey));
...
TEST_F(DiskCacheBackendTest, SimpleDoomAfterBackendDestruction)
{
    ...
    EXPECT_EQ(0,
        WriteData(entry, /*index=*/2, /*offset=*/1, buffer.get(),
...
TEST_F(DiskCacheBackendTest, BlockfileEmptyIndex)
{
    ...
    base::File index(cache_path_.AppendASCII("index"), base::File::FLAG_OPEN | base::File::FLAG_WRITE);
    ASSERT_TRUE(index.IsValid());
    ASSERT_TRUE(index.SetLength(0));
    index.Close();

    // Open the backend again. Fails w/o error-recovery.
    rv = disk_cache::CreateCacheBackend(net::DISK_CACHE, net::CACHE_BACKEND_BLOCKFILE,
        /*file_operations=*/nullptr, cache_path_, 0, disk_cache::ResetHandling::kNeverReset, nullptr, cb.callback());
    rv = cb.GetResult(std::move(rv));
    ...
    // normally use.
    rv = disk_cache::CreateCacheBackend(net::DISK_CACHE, net::CACHE_BACKEND_BLOCKFILE,
        /*file_operations=*/nullptr, cache_path_, 0, disk_cache::ResetHandling::kResetOnError, nullptr, cb.callback());
    rv = cb.GetResult(std::move(rv));
    ASSERT_THAT(rv.net_error, IsOk());
    ASSERT_TRUE(rv.backend);
}
...
TEST_F(DiskCacheBackendTest, SimpleDoomIter)
{
    ...
    disk_cache::EntryResult result = iterator->OpenNextEntry(base::BindLambdaForTesting([&](disk_cache::EntryResult result) {
...
TEST_F(DiskCacheBackendTest, SimpleOpenIter)
{
    ...
    for (bool do_corrupt : { false, true }) {
        ...
        auto iterator = cache_->CreateIterator();
        base::RunLoop run_loop;
        base::RepeatingCallback<void(EntryResult)> collect_entry_key = base::BindLambdaForTesting([&](disk_cache::EntryResult result) {
        ...
        base::RepeatingCallback<void(EntryResult)> handle_entry = base::BindLambdaForTesting([&](disk_cache::EntryResult result) {
            ++iter_opened;
            if (result.net_error() == net::ERR_FAILED) {
        ...
        auto handle_open_result = base::BindLambdaForTesting([&](disk_cache::EntryResult result) {
        ...
        base::RepeatingClosure open_one_entry = base::BindLambdaForTesting([&]() {
            ...
            keys.pop_front();
            disk_cache::EntryResult result = cache_->OpenEntry(key, net::DEFAULT_PRIORITY, handle_open_result);
...
TEST_F(DiskCacheBackendTest, BlockFileImmediateCloseNoDangle)
{
    InitCache();
    base::RunLoop run_loop;
    EntryResult result = cache_->CreateEntry("some key", net::HIGHEST, base::BindLambdaForTesting([&](EntryResult result) {
        ASSERT_EQ(result.net_error(), net::OK);
        result.ReleaseEntry()->Close();
...
TEST_F(DiskCacheBackendTest, SimpleWriteOrderEviction)
{
    ...
    scoped_refptr<net::IOBufferWithSize> buffer = CacheTestCreateAndFillBuffer(kMaxSize, /*no_nulls=*/false);
    ...
    for (int size = 1; size <= kMaxSize; ++size) {
        entry->WriteData(/*index=*/1, /*offset = */ next_offset, buffer.get(),
            /*buf_len=*/size, base::BindLambdaForTesting([&](int result) {
...
TEST_F(DiskCacheBackendTest, SimpleNoCloseFromWithinCreate)
{
    ...
    for (int i = 0; i < 100; ++i) {
        ...
        EntryResult entry_result = cache_->CreateEntry(key, net::HIGHEST, base::DoNothing());
        ...
        entry->WriteData(/*index=*/0, /*offset = */ 0, buffer.get(),
            /*buf_len=*/kDataSize, base::BindLambdaForTesting([&](int result) { EXPECT_EQ(kDataSize, result); }),
            /*truncate=*/true);
        entry->Close();
    }
    RunUntilIdle();
