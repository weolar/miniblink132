...
bool MoveCache(const base::FilePath& from_path, const base::FilePath& to_path)
{
    ...
    base::FileEnumerator iter(from_path, false /* not recursive */, base::FileEnumerator::DIRECTORIES | base::FileEnumerator::FILES);
    for (base::FilePath name = iter.Next(); !name.value().empty(); name = iter.Next()) {
