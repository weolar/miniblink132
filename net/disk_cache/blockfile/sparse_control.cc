...
std::string GenerateChildName(const std::string& base_name, int64_t signature, int64_t child_id)
{
    return base::StringPrintf("Range_%s:%" PRIx64 ":%" PRIx64, base_name.c_str(), signature, child_id);
}

// This class deletes the children of a sparse entry.
class ChildrenDeleter : public base::RefCounted<ChildrenDeleter>, public disk_cache::FileIOCallback {
public:
...
void ChildrenDeleter::Start(std::unique_ptr<char[]> buffer, int len)
{
    ...
    // the child entries.
    disk_cache::SparseData* data = reinterpret_cast<disk_cache::SparseData*>(buffer_.get());
...
void ChildrenDeleter::DeleteChildren()
{
    ...
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(&ChildrenDeleter::DeleteChildren, this));
}

// Returns the NetLog event type corresponding to a SparseOperation.
net::NetLogEventType GetSparseEventType(disk_cache::SparseControl::SparseOperation operation)
{
    switch (operation) {
...
void LogChildOperationEnd(const net::NetLogWithSource& net_log, disk_cache::SparseControl::SparseOperation operation, int result)
{
...
int SparseControl::StartIO(SparseOperation op, int64_t offset, net::IOBuffer* buf, int buf_len, CompletionOnceCallback callback)
{
    DCHECK(init_);
    ...
    operation_ = op;
    offset_ = offset;
    user_buf_ = buf ? base::MakeRefCounted<net::DrainableIOBuffer>(buf, buf_len) : nullptr;
    buf_len_ = buf_len;
    ...
    if (entry_->net_log().IsCapturing()) {
        NetLogSparseOperation(entry_->net_log(), GetSparseEventType(operation_), net::NetLogEventPhase::BEGIN, offset_, buf_len_);
    }
    DoChildrenIO();
...
RangeResult SparseControl::GetAvailableRange(int64_t offset, int len)
{
    ...
    range_found_ = false;
    int result = StartIO(kGetRangeOperation, offset, nullptr, len, CompletionOnceCallback());
    if (range_found_)
...
void SparseControl::DeleteChildren(EntryImpl* entry)
{
    ...
    ChildrenDeleter* deleter = new ChildrenDeleter(entry->backend_.get(), entry->GetKey());
    // The object will self destruct when finished.
    deleter->AddRef();

    if (buffer) {
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(&ChildrenDeleter::Start, deleter, std::move(buffer), data_len));
    } else {
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(&ChildrenDeleter::ReadData, deleter, address, data_len));
    }
}
...
int SparseControl::CreateSparseEntry()
{
    ...
    scoped_refptr<net::IOBuffer> buf = base::MakeRefCounted<net::WrappedIOBuffer>(base::as_chars(base::span_from_ref(sparse_header_)));

    int rv = entry_->WriteData(kSparseIndex, 0, buf.get(), sizeof(sparse_header_), CompletionOnceCallback(), false);
...
int SparseControl::OpenSparseEntry(int data_len)
{
    ...
    // Read header.
    int rv = entry_->ReadData(kSparseIndex, 0, buf.get(), sizeof(sparse_header_), CompletionOnceCallback());
    ...
    rv = entry_->ReadData(kSparseIndex, sizeof(sparse_header_), buf.get(), map_len, CompletionOnceCallback());
    if (rv != map_len)
        return net::ERR_CACHE_READ_FAILURE;
...
bool SparseControl::OpenChild()
{
    ...
    if (!(CHILD_ENTRY & child_->GetEntryFlags()) || child_->GetDataSize(kSparseIndex) < static_cast<int>(sizeof(child_data_)))
        return KillChildAndContinue(key, false);

    auto buf = base::MakeRefCounted<net::WrappedIOBuffer>(base::as_chars(base::span_from_ref(child_data_)));

    // Read signature.
    int rv = child_->ReadData(kSparseIndex, 0, buf.get(), sizeof(child_data_), CompletionOnceCallback());
...
void SparseControl::CloseChild()
{
    ...
    int rv = child_->WriteData(kSparseIndex, 0, buf.get(), sizeof(child_data_), CompletionOnceCallback(), false);
    if (rv != sizeof(child_data_))
...
{
    return GenerateChildName(entry_->GetKey(), sparse_header_.signature, offset_ >> 20);
}
...
{
    auto buf = base::MakeRefCounted<net::WrappedIOBuffer>(base::as_chars(children_map_.GetSpan()));

    int rv = entry_->WriteData(kSparseIndex, sizeof(sparse_header_), buf.get(), buf->size(), CompletionOnceCallback(), false);
    if (rv != buf->size()) {
...
void SparseControl::DoChildrenIO()
{
    ...
    if (kGetRangeOperation == operation_ && entry_->net_log().IsCapturing()) {
        entry_->net_log().EndEvent(
            net::NetLogEventType::SPARSE_GET_RANGE, [&] { return CreateNetLogGetAvailableRangeResultParams(RangeResult(offset_, result_)); });
    }
    if (finished_) {
        if (kGetRangeOperation != operation_ && entry_->net_log().IsCapturing()) {
...
bool SparseControl::DoChildIO()
{
    ...
    if (!user_callback_.is_null()) {
        callback = base::BindOnce(&SparseControl::OnChildIOCompleted, base::Unretained(this));
    }
    ...
    case kReadOperation:
        if (entry_->net_log().IsCapturing()) {
            NetLogSparseReadWrite(
                entry_->net_log(), net::NetLogEventType::SPARSE_READ_CHILD_DATA, net::NetLogEventPhase::BEGIN, child_->net_log().source(), child_len_);
        }
        rv = child_->ReadDataImpl(kSparseData, child_offset_, user_buf_.get(), child_len_, std::move(callback));
        break;
    case kWriteOperation:
        if (entry_->net_log().IsCapturing()) {
            NetLogSparseReadWrite(
                entry_->net_log(), net::NetLogEventType::SPARSE_WRITE_CHILD_DATA, net::NetLogEventPhase::BEGIN, child_->net_log().source(), child_len_);
        }
        rv = child_->WriteDataImpl(kSparseData, child_offset_, user_buf_.get(), child_len_, std::move(callback), false);
        break;
...
int SparseControl::DoGetAvailableRange()
{
    ...
    net::Interval<int> bitmap_range(found * kBlockSize, found * kBlockSize + bits_found * kBlockSize);
    ...
    if (child_data_.header.last_block >= 0) {
        last_write_range
            = net::Interval<int>(child_data_.header.last_block * kBlockSize, child_data_.header.last_block * kBlockSize + child_data_.header.last_block_len);
    }

    // Often |last_write_range| is contiguously after |bitmap_range|, but not
    // always. See if they can be combined.
    if (!last_write_range.Empty() && !bitmap_range.Empty() && bitmap_range.max() == last_write_range.min()) {
    ...
    if (bitmap_range.Empty() || (!last_write_range.Empty() && last_write_range.min() < bitmap_range.min()))
