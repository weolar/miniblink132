...
{
    return std::min(int64_t { std::numeric_limits<int32_t>::max() }, int64_t { table_len } * (k64kEntriesStore / kBaseTableLen));
}
...
void FinalCleanupCallback(disk_cache::BackendImpl* backend, base::WaitableEvent* done)
{
...
public:
    CacheThread()
        : base::Thread("CacheThread_BlockFile")
    {
        CHECK(StartWithOptions(base::Thread::Options(base::MessagePumpType::IO, 0)));
    }
...
static base::LazyInstance<CacheThread>::Leaky g_internal_cache_thread = LAZY_INSTANCE_INITIALIZER;
...
scoped_refptr<base::SingleThreadTaskRunner> FallbackToInternalIfNull(const scoped_refptr<base::SingleThreadTaskRunner>& cache_thread)
{
...
BackendImpl::BackendImpl(const base::FilePath& path, scoped_refptr<BackendCleanupTracker> cleanup_tracker,
    const scoped_refptr<base::SingleThreadTaskRunner>& cache_thread, net::CacheType cache_type, net::NetLog* net_log)
    : Backend(cache_type)
...
BackendImpl::BackendImpl(const base::FilePath& path, uint32_t mask, scoped_refptr<BackendCleanupTracker> cleanup_tracker,
    const scoped_refptr<base::SingleThreadTaskRunner>& cache_thread, net::CacheType cache_type, net::NetLog* net_log)
    : Backend(cache_type)
    , cleanup_tracker_(std::move(cleanup_tracker))
    , background_queue_(this, FallbackToInternalIfNull(cache_thread))
...
BackendImpl::~BackendImpl()
{
    ...
    if (background_queue_.BackgroundIsCurrentSequence()) {
        ...
        background_queue_.background_thread()->PostTask(FROM_HERE, base::BindOnce(&FinalCleanupCallback, base::Unretained(this), base::Unretained(&done)));
...
int BackendImpl::SyncInit()
{
    ...
    if (data_->header.experiment != NO_EXPERIMENT && GetCacheType() != net::DISK_CACHE) {
    ...
    if (!(user_flags_ & kNoRandom) && GetCacheType() == net::DISK_CACHE && !InitExperiment(&data_->header, create_files)) {
        return net::ERR_FAILED;
    }
    ...
    if (!disabled_ && should_create_timer) {
        ...
        timer_->Start(FROM_HERE, base::Milliseconds(timer_delay), this, &BackendImpl::OnStatsTimer);
    }
...
int BackendImpl::SyncOpenEntry(const std::string& key, scoped_refptr<EntryImpl>* entry)
{
    DCHECK(entry);
...
int BackendImpl::SyncCreateEntry(const std::string& key, scoped_refptr<EntryImpl>* entry)
{
    DCHECK(entry);
...
int BackendImpl::SyncDoomEntriesBetween(const base::Time initial_time, const base::Time end_time)
{
...
int BackendImpl::SyncDoomEntriesSince(const base::Time initial_time)
{
    ...
    for (;;) {
        ...
        entry->DoomImpl();
        entry = nullptr;
        SyncEndEnumeration(std::move(iterator)); // The doom invalidated the iterator.
    }
}

int BackendImpl::SyncOpenNextEntry(Rankings::Iterator* iterator, scoped_refptr<EntryImpl>* next_entry)
{
...
void BackendImpl::SyncEndEnumeration(std::unique_ptr<Rankings::Iterator> iterator)
{
    iterator->Reset();
}
...
scoped_refptr<EntryImpl> BackendImpl::CreateEntryImpl(const std::string& key)
{
    ...
    if (entry_address.is_initialized()) {
        ...
        // a hash conflict.
        bool error;
        scoped_refptr<EntryImpl> old_entry = MatchEntry(key, hash, false, Addr(), &error);
        if (old_entry)
    ...
    auto cache_entry = base::MakeRefCounted<EntryImpl>(this, entry_address, false);
    IncreaseNumRefs();
...
scoped_refptr<EntryImpl> BackendImpl::OpenNextEntryImpl(Rankings::Iterator* iterator)
{
    if (disabled_)
        return nullptr;
    ...
    if (!iterator->my_rankings) {
        ...
        for (int i = 0; i < kListsToSearch; i++) {
            ret |= OpenFollowingEntryFromList(static_cast<Rankings::List>(i), &iterator->nodes[i], &entries[i]);
        }
        if (!ret) {
            iterator->Reset();
            return nullptr;
        ...
        for (int i = 0; i < kListsToSearch; i++) {
            if (iterator->list == i) {
                OpenFollowingEntryFromList(iterator->list, &iterator->nodes[i], &entries[i]);
            } else {
                entries[i] = GetEnumeratedEntry(iterator->nodes[i], static_cast<Rankings::List>(i));
            }
        }
    }
...
bool BackendImpl::CreateExternalFile(Addr* address)
{
    ...
    for (int i = 0; i < 0x0fffffff; i++, file_number++) {
        ...
        int flags = base::File::FLAG_READ | base::File::FLAG_WRITE | base::File::FLAG_CREATE | base::File::FLAG_WIN_EXCLUSIVE_WRITE;
...
bool BackendImpl::CreateBlock(FileType block_type, int block_count, Addr* block_address)
{
...
void BackendImpl::InternalDoomEntry(EntryImpl* entry)
{
    ...
    bool error;
    scoped_refptr<EntryImpl> parent_entry = MatchEntry(key, hash, true, entry_addr, &error);
...
void BackendImpl::CriticalError(int error)
{
    ...
    if (!num_refs_)
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(&BackendImpl::RestartCache, GetWeakPtr(), true));
}
...
int BackendImpl::RunTaskForTest(base::OnceClosure task, CompletionOnceCallback callback)
{
...
EntryResult BackendImpl::OpenOrCreateEntry(const std::string& key, net::RequestPriority request_priority, EntryResultCallback callback)
{
...
EntryResult BackendImpl::OpenEntry(const std::string& key, net::RequestPriority request_priority, EntryResultCallback callback)
{
    DCHECK(!callback.is_null());
...
EntryResult BackendImpl::CreateEntry(const std::string& key, net::RequestPriority request_priority, EntryResultCallback callback)
{
    DCHECK(!callback.is_null());
...
net::Error BackendImpl::DoomEntry(const std::string& key, net::RequestPriority priority, CompletionOnceCallback callback)
{
    DCHECK(!callback.is_null());
...
net::Error BackendImpl::DoomEntriesBetween(const base::Time initial_time, const base::Time end_time, CompletionOnceCallback callback)
{
    DCHECK(!callback.is_null());
    background_queue_.DoomEntriesBetween(initial_time, end_time, std::move(callback));
    return net::ERR_IO_PENDING;
}

net::Error BackendImpl::DoomEntriesSince(const base::Time initial_time, CompletionOnceCallback callback)
{
    DCHECK(!callback.is_null());
...
int64_t BackendImpl::CalculateSizeOfAllEntries(Int64CompletionOnceCallback callback)
{
    DCHECK(!callback.is_null());
    background_queue_.CalculateSizeOfAllEntries(
        BindOnce([](Int64CompletionOnceCallback callback, int result) { std::move(callback).Run(static_cast<int64_t>(result)); }, std::move(callback)));
    return net::ERR_IO_PENDING;
}
...
bool BackendImpl::InitBackingStore(bool* file_created)
{
    ...
    int flags = base::File::FLAG_READ | base::File::FLAG_WRITE | base::File::FLAG_OPEN_ALWAYS | base::File::FLAG_WIN_EXCLUSIVE_WRITE;
...
int BackendImpl::NewEntry(Addr address, scoped_refptr<EntryImpl>* entry)
{
    ...
    *entry = nullptr;
    ...
    STRESS_DCHECK(block_files_.IsValid(Addr(cache_entry->entry()->Data()->rankings_node)));
...
scoped_refptr<EntryImpl> BackendImpl::MatchEntry(const std::string& key, uint32_t hash, bool find_parent, Addr entry_addr, bool* match_error)
{
...
bool BackendImpl::OpenFollowingEntryFromList(Rankings::List list, CacheRankingsBlock** from_entry, scoped_refptr<EntryImpl>* next_entry)
{
    if (disabled_)
        return false;
...
scoped_refptr<EntryImpl> BackendImpl::GetEnumeratedEntry(CacheRankingsBlock* next, Rankings::List list)
{
    if (!next || disabled_)
        return nullptr;
...
scoped_refptr<EntryImpl> BackendImpl::ResurrectEntry(scoped_refptr<EntryImpl> deleted_entry)
{
...
void BackendImpl::UpgradeTo3_0()
{
    ...
    DCHECK(kVersion2_0 == data_->header.version || kVersion2_1 == data_->header.version);
...
bool BackendImpl::CheckIndex()
{
    ...
    if (data_->header.version == kVersion2_0 || data_->header.version == kVersion2_1) {
        UpgradeTo3_0();
    }
    ...
    if (data_->header.num_bytes < 0
        || (max_size_ < std::numeric_limits<int32_t>::max() - kDefaultCacheSize && data_->header.num_bytes > max_size_ + kDefaultCacheSize)) {
...
{
    if (!g_internal_cache_thread.IsCreated()) {
        base::SequencedTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, std::move(callback));
        return;
    }

    InternalCacheThread()->PostTaskAndReply(FROM_HERE, base::BindOnce([]() {}), std::move(callback));
}
