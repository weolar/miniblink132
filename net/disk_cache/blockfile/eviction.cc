...
void Eviction::TrimCache(bool empty)
{
    ...
    Rankings::ScopedRankingsBlock next(rankings_, rankings_->GetPrev(node.get(), Rankings::NO_USE));
    ...
    while ((header_->num_bytes > target_size || test_mode_) && next.get()) {
        ...
        if (!empty && (deleted_entries > 20 || (TimeTicks::Now() - start).InMilliseconds() > 20)) {
            base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(&Eviction::TrimCache, ptr_factory_.GetWeakPtr(), false));
            break;
        }
    }
...
void Eviction::PostDelayedTrim()
{
    ...
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(
        FROM_HERE, base::BindOnce(&Eviction::DelayedTrim, ptr_factory_.GetWeakPtr()), base::Milliseconds(1000));
}
...
void Eviction::ReportTrimTimes(EntryImpl* entry)
{
    if (first_trim_) {
        ...
        if (header_->create_time) {
            ...
            const base::Time time_2009_3_1 = base::Time::FromInternalValue(12985574400000000);
...
bool Eviction::EvictEntry(CacheRankingsBlock* node, bool empty, Rankings::List list)
{
...
void Eviction::TrimCacheV2(bool empty)
{
    ...
    for (; list < kListsToSearch; list++) {
        while ((header_->num_bytes > target_size || test_mode_) && next[list].get()) {
            ...
            next[list].reset(rankings_->GetPrev(node.get(), static_cast<Rankings::List>(list)));
            if (node->Data()->dirty != backend_->GetCurrentEntryId() || empty) {
                // This entry is not being used by anybody.
                // Do NOT use node as an iterator after this point.
                rankings_->TrackRankingsBlock(node.get(), false);
    ...
    if (empty) {
        ...
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(&Eviction::TrimDeleted, ptr_factory_.GetWeakPtr(), empty));
    }
...
void Eviction::TrimDeleted(bool empty)
{
    ...
    TimeTicks start = TimeTicks::Now();
    Rankings::ScopedRankingsBlock node(rankings_);
    Rankings::ScopedRankingsBlock next(rankings_, rankings_->GetPrev(node.get(), Rankings::DELETED));
    int deleted_entries = 0;
    ...
    if (deleted_entries && !empty && ShouldTrimDeleted()) {
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(&Eviction::TrimDeleted, ptr_factory_.GetWeakPtr(), false));
    }
...
{
    scoped_refptr<EntryImpl> entry = backend_->GetEnumeratedEntry(node, Rankings::DELETED);
    if (!entry)
        return false;
