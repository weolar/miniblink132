...
public:
    BackendImpl(const base::FilePath& path, scoped_refptr<BackendCleanupTracker> cleanup_tracker,
        const scoped_refptr<base::SingleThreadTaskRunner>& cache_thread, net::CacheType cache_type, net::NetLog* net_log);

    // mask can be used to limit the usable size of the hash table, for testing.
    BackendImpl(const base::FilePath& path, uint32_t mask, scoped_refptr<BackendCleanupTracker> cleanup_tracker,
        const scoped_refptr<base::SingleThreadTaskRunner>& cache_thread, net::CacheType cache_type, net::NetLog* net_log);
    ...
    int SyncOpenNextEntry(Rankings::Iterator* iterator, scoped_refptr<EntryImpl>* next_entry);
    ...
    EntryResult OpenOrCreateEntry(const std::string& key, net::RequestPriority request_priority, EntryResultCallback callback) override;
...
private:
    ...
    scoped_refptr<EntryImpl> MatchEntry(const std::string& key, uint32_t hash, bool find_parent, Addr entry_addr, bool* match_error);
    ...
    bool OpenFollowingEntryFromList(Rankings::List list, CacheRankingsBlock** from_entry, scoped_refptr<EntryImpl>* next_entry);

    // Returns the entry that is pointed by |next|, from the given |list|.
    scoped_refptr<EntryImpl> GetEnumeratedEntry(CacheRankingsBlock* next, Rankings::List list);

    // Re-opens an entry that was previously deleted.
    scoped_refptr<EntryImpl> ResurrectEntry(scoped_refptr<EntryImpl> deleted_entry);
    ...
    bool read_only_ = false; // Prevents updates of the rankings data (used by tools).
    ...
    bool user_load_ = false; // True if we see a high load coming from the caller.
