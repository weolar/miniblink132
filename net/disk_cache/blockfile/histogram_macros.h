...
#define CACHE_HISTOGRAM_CUSTOM_COUNTS(name, sample, min, max, bucket_count)                                                                                    \
    do {                                                                                                                                                       \
        base::HistogramBase* counter = base::Histogram::FactoryGet(name, min, max, bucket_count, base::Histogram::kUmaTargetedHistogramFlag);                  \
        counter->Add(sample);                                                                                                                                  \
    } while (0)

#define CACHE_HISTOGRAM_COUNTS(name, sample) CACHE_HISTOGRAM_CUSTOM_COUNTS(name, sample, 1, 1000000, 50)

#define CACHE_HISTOGRAM_COUNTS_10000(name, sample) CACHE_HISTOGRAM_CUSTOM_COUNTS(name, sample, 1, 10000, 50)

#define CACHE_HISTOGRAM_COUNTS_50000(name, sample) CACHE_HISTOGRAM_CUSTOM_COUNTS(name, sample, 1, 50000000, 50)

#define CACHE_HISTOGRAM_CUSTOM_TIMES(name, sample, min, max, bucket_count)                                                                                     \
    do {                                                                                                                                                       \
        base::HistogramBase* counter = base::Histogram::FactoryTimeGet(name, min, max, bucket_count, base::Histogram::kUmaTargetedHistogramFlag);              \
        counter->AddTime(sample);                                                                                                                              \
    } while (0)

#define CACHE_HISTOGRAM_TIMES(name, sample) CACHE_HISTOGRAM_CUSTOM_TIMES(name, sample, base::Milliseconds(1), base::Seconds(10), 50)

#define CACHE_HISTOGRAM_ENUMERATION(name, sample, boundary_value)                                                                                              \
    do {                                                                                                                                                       \
        base::HistogramBase* counter                                                                                                                           \
            = base::LinearHistogram::FactoryGet(name, 1, boundary_value, boundary_value + 1, base::Histogram::kUmaTargetedHistogramFlag);                      \
        counter->Add(sample);                                                                                                                                  \
    } while (0)

#define CACHE_HISTOGRAM_PERCENTAGE(name, under_one_hundred) CACHE_HISTOGRAM_ENUMERATION(name, under_one_hundred, 101)
...
#define CACHE_HISTOGRAM_AGE(name, initial_time) CACHE_HISTOGRAM_COUNTS_10000(name, (base::Time::Now() - initial_time).InHours())

// HISTOGRAM_AGE_MS will collect time elapsed since |initial_time|, with the
// normal resolution of the UMA_HISTOGRAM_TIMES.
#define CACHE_HISTOGRAM_AGE_MS(name, initial_time) CACHE_HISTOGRAM_TIMES(name, base::TimeTicks::Now() - initial_time)

#define CACHE_HISTOGRAM_CACHE_ERROR(name, sample) CACHE_HISTOGRAM_ENUMERATION(name, sample, 50)
