...
public:
    ...
    SyncCallback(scoped_refptr<disk_cache::EntryImpl> entry, net::IOBuffer* buffer, net::CompletionOnceCallback callback, net::NetLogEventType end_event_type)
...
void SyncCallback::OnFileIOComplete(int bytes_copied)
{
    ...
    if (!callback_.is_null()) {
        if (entry_->net_log().IsCapturing()) {
            disk_cache::NetLogReadWriteComplete(entry_->net_log(), end_event_type_, net::NetLogEventPhase::END, bytes_copied);
        }
...
int EntryImpl::ReadDataImpl(int index, int offset, IOBuffer* buf, int buf_len, CompletionOnceCallback callback)
{
    if (net_log_.IsCapturing()) {
        NetLogReadWriteData(net_log_, net::NetLogEventType::ENTRY_READ_DATA, net::NetLogEventPhase::BEGIN, index, offset, buf_len, false);
    }

    int result = InternalReadData(index, offset, buf, buf_len, std::move(callback));

    if (result != net::ERR_IO_PENDING && net_log_.IsCapturing()) {
        NetLogReadWriteComplete(net_log_, net::NetLogEventType::ENTRY_READ_DATA, net::NetLogEventPhase::END, result);
    }
    return result;
}

int EntryImpl::WriteDataImpl(int index, int offset, IOBuffer* buf, int buf_len, CompletionOnceCallback callback, bool truncate)
{
    if (net_log_.IsCapturing()) {
        NetLogReadWriteData(net_log_, net::NetLogEventType::ENTRY_WRITE_DATA, net::NetLogEventPhase::BEGIN, index, offset, buf_len, truncate);
    }

    int result = InternalWriteData(index, offset, buf, buf_len, std::move(callback), truncate);

    if (result != net::ERR_IO_PENDING && net_log_.IsCapturing()) {
        NetLogReadWriteComplete(net_log_, net::NetLogEventType::ENTRY_WRITE_DATA, net::NetLogEventPhase::END, result);
    }
    return result;
}

int EntryImpl::ReadSparseDataImpl(int64_t offset, IOBuffer* buf, int buf_len, CompletionOnceCallback callback)
{
    ...
    result = sparse_->StartIO(SparseControl::kReadOperation, offset, buf, buf_len, std::move(callback));
    return result;
}

int EntryImpl::WriteSparseDataImpl(int64_t offset, IOBuffer* buf, int buf_len, CompletionOnceCallback callback)
{
    DCHECK(node_.Data()->dirty || read_only_);
    int result = InitSparseData();
    if (net::OK != result)
    ...
    result = sparse_->StartIO(SparseControl::kWriteOperation, offset, buf, buf_len, std::move(callback));
    return result;
}
...
bool EntryImpl::CreateEntry(Addr node_address, const std::string& key, uint32_t hash)
{
...
bool EntryImpl::SanityCheck()
{
    ...
    if (key_addr.is_initialized()
        && ((stored->key_len < kMaxBlockSize && key_addr.is_separate_file()) || (stored->key_len >= kMaxBlockSize && key_addr.is_block_file())))
        return false;
...
void EntryImpl::FixForDelete()
{
    ...
    for (int i = 0; i < kNumStreams; i++) {
        ...
        if (data_addr.is_initialized()) {
            if ((data_size <= kMaxBlockSize && data_addr.is_separate_file()) || (data_size > kMaxBlockSize && data_addr.is_block_file())
...
void EntryImpl::BeginLogging(net::NetLog* net_log, bool created)
{
    ...
    net_log_.BeginEvent(net::NetLogEventType::DISK_CACHE_ENTRY_IMPL, [&] { return CreateNetLogParametersEntryCreationParams(this, created); });
}
...
std::string EntryImpl::GetKey() const
{
    ...
    File* key_file = const_cast<EntryImpl*>(this)->GetBackingFile(address, kKeyFileIndex);
    if (!key_file)
        return std::string();
...
int EntryImpl::ReadData(int index, int offset, IOBuffer* buf, int buf_len, CompletionOnceCallback callback)
{
    if (callback.is_null())
    ...
    background_queue_->ReadData(this, index, offset, buf, buf_len, std::move(callback));
    return net::ERR_IO_PENDING;
}

int EntryImpl::WriteData(int index, int offset, IOBuffer* buf, int buf_len, CompletionOnceCallback callback, bool truncate)
{
    if (callback.is_null()) {
        return WriteDataImpl(index, offset, buf, buf_len, std::move(callback), truncate);
    }
    ...
    background_queue_->WriteData(this, index, offset, buf, buf_len, truncate, std::move(callback));
    return net::ERR_IO_PENDING;
}

int EntryImpl::ReadSparseData(int64_t offset, IOBuffer* buf, int buf_len, CompletionOnceCallback callback)
{
    if (callback.is_null())
    ...
    background_queue_->ReadSparseData(this, offset, buf, buf_len, std::move(callback));
    return net::ERR_IO_PENDING;
}

int EntryImpl::WriteSparseData(int64_t offset, IOBuffer* buf, int buf_len, CompletionOnceCallback callback)
{
    if (callback.is_null())
    ...
    background_queue_->WriteSparseData(this, offset, buf, buf_len, std::move(callback));
    return net::ERR_IO_PENDING;
}

RangeResult EntryImpl::GetAvailableRange(int64_t offset, int len, RangeResultCallback callback)
{
    if (!background_queue_.get())
    ...
    if (doomed_) {
        ...
        for (int index = 0; index < kNumStreams; index++) {
            ...
            if (unreported_size_[index]) {
                backend_->ModifyStorageSize(entry_.Data()->data_size[index] - unreported_size_[index], entry_.Data()->data_size[index]);
            }
        }
...
int EntryImpl::InternalReadData(int index, int offset, IOBuffer* buf, int buf_len, CompletionOnceCallback callback)
{
    DCHECK(node_.Data()->dirty || read_only_);
    ...
    int end_offset;
    if (!base::CheckAdd(offset, buf_len).AssignIfValid(&end_offset) || end_offset > entry_size)
    ...
    if (!null_callback) {
        io_callback = new SyncCallback(base::WrapRefCounted(this), buf, std::move(callback), net::NetLogEventType::ENTRY_READ_DATA);
    }
...
int EntryImpl::InternalWriteData(int index, int offset, IOBuffer* buf, int buf_len, CompletionOnceCallback callback, bool truncate)
{
    DCHECK(node_.Data()->dirty || read_only_);
    ...
    int end_offset;
    if (offset > max_file_size || buf_len > max_file_size || !base::CheckAdd(offset, buf_len).AssignIfValid(&end_offset) || end_offset > max_file_size) {
        int size = base::CheckAdd(offset, buf_len).ValueOrDefault(std::numeric_limits<int32_t>::max());
    ...
    bool null_callback = callback.is_null();
    if (!null_callback) {
        io_callback = new SyncCallback(this, buf, std::move(callback), net::NetLogEventType::ENTRY_WRITE_DATA);
    }

    bool completed;
    if (!file->Write(buf->data(), buf_len, file_offset, io_callback, &completed)) {
        if (io_callback)
            io_callback->Discard();
...
// that case.
bool EntryImpl::PrepareTarget(int index, int offset, int buf_len, bool truncate)
{
    if (truncate)
...
bool EntryImpl::CopyToLocalBuffer(int index)
{
    ...
    if (!file || !file->Read(user_buffers_[index]->Data(), len, offset, nullptr, nullptr)) {
        user_buffers_[index].reset();
        return false;
    }
    return true;
...
void EntryImpl::GetData(int index, std::unique_ptr<char[]>* buffer, Addr* address)
{
    DCHECK(backend_.get());
