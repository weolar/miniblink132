    ...
    , address_(address)
{
    ...
    DCHECK(!address.is_initialized() || sizeof(T) == address.BlockSize()) << address.value();
}
...
template <typename T> void StorageBlock<T>::CopyFrom(StorageBlock<T>* other)
{
    DCHECK(!modified_);
...
template <typename T> bool StorageBlock<T>::LazyInit(MappedFile* file, Addr address)
{
...
template <typename T> bool StorageBlock<T>::Load(FileIOCallback* callback, bool* completed)
{
    if (file_) {
        if (!data_)
            AllocateData();
...
template <typename T> bool StorageBlock<T>::Store(FileIOCallback* callback, bool* completed)
{
    if (file_ && data_) {
        data_->self_hash = CalculateHash();
...
template <typename T> uint32_t StorageBlock<T>::CalculateHash() const
{
