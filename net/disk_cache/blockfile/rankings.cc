...
public:
    ...
    Transaction(volatile disk_cache::LruData* data, disk_cache::Addr addr, Operation op, int list);
...
Transaction::Transaction(volatile disk_cache::LruData* data, disk_cache::Addr addr, Operation op, int list)
    : data_(data)
{
...
Rankings::ScopedRankingsBlock::ScopedRankingsBlock(Rankings* rankings, CacheRankingsBlock* node)
...
void Rankings::TrackRankingsBlock(CacheRankingsBlock* node, bool start_tracking)
{
    if (!node)
        return;
...
bool Rankings::CheckLinks(CacheRankingsBlock* node, CacheRankingsBlock* prev, CacheRankingsBlock* next, List* list)
{
    ...
    if (node_addr != prev->address().value() && node_addr != next->address().value() && prev->Data()->next == next->address().value()
        && next->Data()->prev == prev->address().value()) {
...
bool Rankings::CheckSingleLink(CacheRankingsBlock* prev, CacheRankingsBlock* next)
{
...
int Rankings::CheckListSection(List list, Addr end1, Addr end2, bool forward, Addr* last, Addr* second_last, int* num_items)
{
    ...
    *num_items = 0;
    ...
    do {
        node = std::make_unique<CacheRankingsBlock>(backend_->File(current), current);
        node->Load();
...
void Rankings::UpdateIteratorsForRemoved(CacheAddr address, CacheRankingsBlock* next)
{
