...
MappedFile* BlockFiles::GetFile(Addr address)
{
    ...
    DCHECK_GE(block_files_.size(), static_cast<size_t>(kFirstAdditionalBlockFile));
...
bool BlockFiles::CreateBlock(FileType block_type, int block_count, Addr* block_address)
{
    DCHECK(thread_checker_->CalledOnValidThread());
...
bool BlockFiles::IsValid(Addr address)
{
    ...
    if (read_contents) {
        auto buffer = std::make_unique<char[]>(Addr::BlockSizeForFileType(BLOCK_4K) * 4);
        size_t size = address.BlockSize() * address.num_blocks();
        size_t offset = address.start_block() * address.BlockSize() + kBlockHeaderSize;
