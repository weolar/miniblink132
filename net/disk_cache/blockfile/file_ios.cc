...
public:
    ...
    // operation.
    FileBackgroundIO(
        disk_cache::File* file, const void* buf, size_t buf_len, size_t offset, disk_cache::FileIOCallback* callback, disk_cache::InFlightIO* controller)
...
public:
    ...
    void PostRead(disk_cache::File* file, void* buf, size_t buf_len, size_t offset, disk_cache::FileIOCallback* callback);
    void PostWrite(disk_cache::File* file, const void* buf, size_t buf_len, size_t offset, disk_cache::FileIOCallback* callback);
...
protected:
    ...
    void OnOperationComplete(disk_cache::BackgroundIO* operation, bool cancel) override;
};
...
void FileInFlightIO::PostRead(disk_cache::File* file, void* buf, size_t buf_len, size_t offset, disk_cache::FileIOCallback* callback)
{
    auto operation = base::MakeRefCounted<FileBackgroundIO>(file, buf, buf_len, offset, callback, this);
    file->AddRef(); // Balanced on OnOperationComplete()

    base::ThreadPool::PostTask(
        FROM_HERE, { base::MayBlock(), base::TaskShutdownBehavior::CONTINUE_ON_SHUTDOWN }, base::BindOnce(&FileBackgroundIO::Read, operation.get()));
    OnOperationPosted(operation.get());
}

void FileInFlightIO::PostWrite(disk_cache::File* file, const void* buf, size_t buf_len, size_t offset, disk_cache::FileIOCallback* callback)
{
    auto operation = base::MakeRefCounted<FileBackgroundIO>(file, buf, buf_len, offset, callback, this);
    file->AddRef(); // Balanced on OnOperationComplete()

    base::ThreadPool::PostTask(
        FROM_HERE, { base::MayBlock(), base::TaskShutdownBehavior::CONTINUE_ON_SHUTDOWN }, base::BindOnce(&FileBackgroundIO::Write, operation.get()));
    OnOperationPosted(operation.get());
}

// Runs on the IO thread.
void FileInFlightIO::OnOperationComplete(disk_cache::BackgroundIO* operation, bool cancel)
{
...
bool File::Read(void* buffer, size_t buffer_len, size_t offset)
{
    ...
    int ret = UNSAFE_TODO(base_file_.Read(offset, static_cast<char*>(buffer), buffer_len));
...
bool File::Write(const void* buffer, size_t buffer_len, size_t offset)
{
    ...
    int ret = UNSAFE_TODO(base_file_.Write(offset, static_cast<const char*>(buffer), buffer_len));
    return (static_cast<size_t>(ret) == buffer_len);
}
