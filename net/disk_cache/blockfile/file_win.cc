...
struct MyOverlapped {
    MyOverlapped(disk_cache::File* file, size_t offset, disk_cache::FileIOCallback* callback);
    ~MyOverlapped()
    {
    }
...
static_assert(offsetof(MyOverlapped, context_) == 0, "should start with overlapped");

// Helper class to handle the IO completion notifications from the message loop.
class CompletionHandler final : public base::MessagePumpForIO::IOHandler, public base::RefCounted<CompletionHandler> {
public:
    CompletionHandler()
...
private:
    ...
    void OnIOCompleted(base::MessagePumpForIO::IOContext* context, DWORD actual_bytes, DWORD error) override;
};

CompletionHandler* CompletionHandler::Get()
{
    static base::NoDestructor<scoped_refptr<CompletionHandler>> handler(base::MakeRefCounted<CompletionHandler>());
    return handler->get();
}

void CompletionHandler::OnIOCompleted(base::MessagePumpForIO::IOContext* context, DWORD actual_bytes, DWORD error)
{
    ...
    if (data->callback_)
        data->callback_.ExtractAsDangling()->OnFileIOComplete(static_cast<int>(actual_bytes));

    delete data;
}

MyOverlapped::MyOverlapped(disk_cache::File* file, size_t offset, disk_cache::FileIOCallback* callback)
{
...
bool File::Init(const base::FilePath& name)
{
    ...
    base_file_ = base::File(CreateFile(name.value().c_str(), access, sharing, nullptr, OPEN_EXISTING, FILE_FLAG_OVERLAPPED, nullptr));

    if (!base_file_.IsValid())
        return false;

    if (!base::CurrentIOThread::Get()->RegisterIOHandler(base_file_.GetPlatformFile(), CompletionHandler::Get())) {
        return false;
    }

    init_ = true;
    sync_base_file_ = base::File(CreateFile(name.value().c_str(), access, sharing, nullptr, OPEN_EXISTING, 0, nullptr));
...
bool File::Read(void* buffer, size_t buffer_len, size_t offset)
{
    ...
    int ret = UNSAFE_TODO(sync_base_file_.Read(offset, static_cast<char*>(buffer), buffer_len));
...
bool File::Write(const void* buffer, size_t buffer_len, size_t offset)
{
    ...
    int ret = UNSAFE_TODO(sync_base_file_.Write(offset, static_cast<const char*>(buffer), buffer_len));
    return static_cast<int>(buffer_len) == ret;
}
...
bool File::Read(void* buffer, size_t buffer_len, size_t offset, FileIOCallback* callback, bool* completed)
{
    DCHECK(init_);
    if (!callback) {
        if (completed)
    ...
    DWORD actual;
    if (!ReadFile(base_file_.GetPlatformFile(), buffer, size, &actual, data->overlapped())) {
        *completed = false;
...
bool File::Write(const void* buffer, size_t buffer_len, size_t offset, FileIOCallback* callback, bool* completed)
{
    DCHECK(init_);
    if (!callback) {
        if (completed)
...
bool File::AsyncWrite(const void* buffer, size_t buffer_len, size_t offset, FileIOCallback* callback, bool* completed)
{
    DCHECK(init_);
    DCHECK(callback);
    DCHECK(completed);
    ...
    DWORD actual;
    if (!WriteFile(base_file_.GetPlatformFile(), buffer, size, &actual, data->overlapped())) {
        *completed = false;
        if (GetLastError() == ERROR_IO_PENDING)
            return true;
        delete data;
...
bool File::SetLength(size_t length)
{
    ...
    if (INVALID_SET_FILE_POINTER == SetFilePointer(file, size, nullptr, FILE_BEGIN))
        return false;
