...
public:
    ...
    TestFilterSourceStreamBase& operator=(const TestFilterSourceStreamBase&) = delete;

    ~TestFilterSourceStreamBase() override
    {
...
protected:
    ...
    // |buffer_|.
    size_t WriteBufferToOutput(IOBuffer* output_buffer, size_t output_buffer_size)
    {
...
public:
    NeedsAllInputFilterSourceStream(std::unique_ptr<SourceStream> upstream, size_t expected_input_bytes)
        : TestFilterSourceStreamBase(std::move(upstream))
        , expected_input_bytes_(expected_input_bytes)
    {
    }

    NeedsAllInputFilterSourceStream(const NeedsAllInputFilterSourceStream&) = delete;
    NeedsAllInputFilterSourceStream& operator=(const NeedsAllInputFilterSourceStream&) = delete;

    base::expected<size_t, Error> FilterData(IOBuffer* output_buffer, size_t output_buffer_size, IOBuffer* input_buffer, size_t input_buffer_size,
        size_t* consumed_bytes, bool upstream_eof_reached) override
    {
...
public:
    ...
    PassThroughFilterSourceStream& operator=(const PassThroughFilterSourceStream&) = delete;
...
public:
    ...
    bool consumed_all_input() const
    {
        return consumed_all_input_;
    }
...
class FilterSourceStreamTest : public ::testing::TestWithParam<MockSourceStream::Mode> {
protected:
    ...
    int CompleteReadIfAsync(int previous_result, TestCompletionCallback* callback, MockSourceStream* mock_stream, size_t num_reads)
    {
...
INSTANTIATE_TEST_SUITE_P(FilterSourceStreamTests, FilterSourceStreamTest, ::testing::Values(MockSourceStream::SYNC, MockSourceStream::ASYNC));
...
TEST_P(FilterSourceStreamTest, FilterDataReturnNoBytesExceptLast)
{
    ...
    for (size_t offset = 0; offset < input.length(); offset += read_size) {
        source->AddReadResult(input.data() + offset, std::min(read_size, input.length() - offset), OK, GetParam());
        num_reads++;
    }
    ...
    scoped_refptr<IOBufferWithSize> output_buffer = base::MakeRefCounted<IOBufferWithSize>(kDefaultBufferSize);
...
TEST_P(FilterSourceStreamTest, FilterDataOutputSpace)
{
    ...
    // space.
    scoped_refptr<IOBufferWithSize> output_buffer = base::MakeRefCounted<IOBufferWithSize>(kSmallBufferSize);
    MockSourceStream* mock_stream = source.get();
    PassThroughFilterSourceStream stream(std::move(source));
    TestCompletionCallback callback;
    std::string actual_output;
...
TEST_P(FilterSourceStreamTest, FilterDataReturnError)
{
    ...
    rv = stream.Read(output_buffer.get(), output_buffer->size(), callback.callback());
    EXPECT_EQ(ERR_CONTENT_DECODING_FAILED, rv);
}
...
TEST_P(FilterSourceStreamTest, FilterChaining)
{
    ...
    MockSourceStream* mock_stream = source.get();
    auto pass_through_source = std::make_unique<PassThroughFilterSourceStream>(std::move(source));
    pass_through_source->set_type_string("FIRST_PASS_THROUGH");
    auto needs_all_input_source = std::make_unique<NeedsAllInputFilterSourceStream>(std::move(pass_through_source), input.length());
    needs_all_input_source->set_type_string("NEEDS_ALL");
    auto second_pass_through_source = std::make_unique<PassThroughFilterSourceStream>(std::move(needs_all_input_source));
    ...
    std::string actual_output;
    while (true) {
        int rv = second_pass_through_source->Read(output_buffer.get(), output_buffer->size(), callback.callback());
        if (rv == ERR_IO_PENDING)
    ...
    EXPECT_EQ("FIRST_PASS_THROUGH,NEEDS_ALL,SECOND_PASS_THROUGH", second_pass_through_source->Description());
}
