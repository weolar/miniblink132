...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    if (is_shared_dictionary) {
        ...
        zstd_stream = net::CreateZstdSourceStreamWithDictionary(std::move(fuzzed_source_stream), dictionary_buffer, dictionary.size());
    } else {
        auto fuzzed_source_stream = std::make_unique<net::FuzzedSourceStream>(&data_provider);
    ...
    while (true) {
        ...
        int result = zstd_stream->Read(io_buffer.get(), io_buffer->size(), callback.callback());
