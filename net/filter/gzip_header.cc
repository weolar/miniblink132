...
GZipHeader::Status GZipHeader::ReadMore(const char* inbuf, size_t inbuf_len, const char** header_end)
{
    ...
    while (pos < end) {
        ...
        case IN_FEXTRA: {
            ...
            const uint16_t num_extra_bytes = static_cast<uint16_t>(std::min(static_cast<ptrdiff_t>(extra_length_), (end - pos)));
            pos += num_extra_bytes;
