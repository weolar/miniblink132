...
int FuzzedSourceStream::Read(IOBuffer* buf, int buf_len, CompletionOnceCallback callback)
{
    DCHECK(!read_pending_);
    ...
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
        FROM_HERE, base::BindOnce(&FuzzedSourceStream::OnReadComplete, base::Unretained(this), std::move(callback), data, pending_read_buf, result));
...
void FuzzedSourceStream::OnReadComplete(CompletionOnceCallback callback, const std::string& fuzzed_data, scoped_refptr<IOBuffer> read_buf, int result)
{
    DCHECK(read_pending_);

    if (result > 0) {
        std::copy(fuzzed_data.data(), fuzzed_data.data() + result, read_buf->data());
    } else {
        end_returned_ = true;
    }
    read_pending_ = false;
