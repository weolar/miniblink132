...
struct FreeContextDeleter {
    inline void operator()(ZSTD_DCtx* ptr) const
    {
        ZSTD_freeDCtx(ptr);
    }
};
...
public:
    explicit ZstdSourceStream(std::unique_ptr<SourceStream> upstream, scoped_refptr<IOBuffer> dictionary = nullptr, size_t dictionary_size = 0u)
        ...
        , dictionary_size_(dictionary_size)
    {
        ...
        if (dictionary_) {
            ...
            window_log_max = std::clamp(base::bits::Log2Ceiling(base::checked_cast<uint32_t>(dictionary_size_ * 5 / 4)),
                23, // 8MB
                27); // 128MB
        }
        ...
        if (dictionary_) {
            size_t result = ZSTD_DCtx_loadDictionary_advanced(
                dctx_.get(), reinterpret_cast<const void*>(dictionary_->data()), dictionary_size_, ZSTD_dlm_byRef, ZSTD_dct_rawContent);
    ...
    ~ZstdSourceStream() override
    {
        if (ZSTD_isError(decoding_result_)) {
            ...
            UMA_HISTOGRAM_ENUMERATION("Net.ZstdFilter.ErrorCode", static_cast<int>(error_code), static_cast<int>(ZSTD_ErrorCode::ZSTD_error_maxCode));
        }
        ...
        if (decoding_status_ == ZstdDecodingStatus::kEndOfFrame) {
            ...
            if (produced_bytes_ != 0) {
                UMA_HISTOGRAM_PERCENTAGE("Net.ZstdFilter.CompressionRatio", static_cast<int>((consumed_bytes_ * 100) / produced_bytes_));
            }
        }

        UMA_HISTOGRAM_MEMORY_KB("Net.ZstdFilter.MaxMemoryUsage", (max_allocated_ / 1024));
    }
...
private:
    ...
    base::expected<size_t, Error> FilterData(IOBuffer* output_buffer, size_t output_buffer_size, IOBuffer* input_buffer, size_t input_buffer_size,
        size_t* consumed_bytes, bool upstream_end_reached) override
    {
        CHECK(dctx_);
...
std::unique_ptr<FilterSourceStream> CreateZstdSourceStream(std::unique_ptr<SourceStream> previous)
{
...
{
    return std::make_unique<ZstdSourceStream>(std::move(previous), std::move(dictionary), dictionary_size);
}
