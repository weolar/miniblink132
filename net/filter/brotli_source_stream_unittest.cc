...
protected:
    ...
    {
        return brotli_stream_->Read(out_buffer(), out_data_size(), std::move(callback));
    }
    ...
    SourceStream* brotli_stream()
    {
        return brotli_stream_.get();
    }
...
{
    source()->AddReadResult(encoded_buffer(), encoded_len(), OK, MockSourceStream::SYNC);
...
TEST_F(BrotliSourceStreamTest, IgnoreExtraData)
{
    ...
    source()->AddReadResult(reinterpret_cast<const char*>(kResponse), sizeof(kResponse), OK, MockSourceStream::SYNC);
    // Add an EOF.
    source()->AddReadResult(reinterpret_cast<const char*>(kResponse), 0, OK, MockSourceStream::SYNC);
    out_buffer_ = base::MakeRefCounted<IOBufferWithSize>(kDefaultBufferSize);
...
TEST_F(BrotliSourceStreamTest, IgnoreExtraDataInOneRead)
{
    ...
    source()->AddReadResult(response_with_extra_data.c_str(), response_with_extra_data.length(), OK, MockSourceStream::SYNC);
    // Add an EOF.
    source()->AddReadResult(response_with_extra_data.c_str(), 0, OK, MockSourceStream::SYNC);
    out_buffer_ = base::MakeRefCounted<IOBufferWithSize>(kDefaultBufferSize);
    std::string actual_output;
    while (true) {
        TestCompletionCallback callback;
...
TEST_F(BrotliSourceStreamTest, DecodeBrotliTwoBlockSync)
{
    ...
    source()->AddReadResult(encoded_buffer() + 10, encoded_len() - 10, OK, MockSourceStream::SYNC);
    out_buffer_ = base::MakeRefCounted<IOBufferWithSize>(kDefaultBufferSize);
    TestCompletionCallback callback;
    int bytes_read = ReadStream(callback.callback());
    EXPECT_EQ(static_cast<int>(source_data_len()), bytes_read);
...
{
    source()->AddReadResult(encoded_buffer(), encoded_len(), OK, MockSourceStream::ASYNC);
    out_buffer_ = base::MakeRefCounted<IOBufferWithSize>(kDefaultBufferSize);
    TestCompletionCallback callback;
    int bytes_read = ReadStream(callback.callback());
...
TEST_F(BrotliSourceStreamTest, DecodeWithOneByteBuffer)
{
    ...
    EXPECT_EQ(source_data_len(), total_bytes_read);
    EXPECT_EQ(0, memcmp(buffer->data(), source_data().c_str(), source_data_len()));
    EXPECT_EQ("BROTLI", brotli_stream()->Description());
}
...
TEST_F(BrotliSourceStreamTest, DecodeCorruptedData)
{
    ...
    source()->AddReadResult(corrupt_data, corrupt_data_len, OK, MockSourceStream::SYNC);
    out_buffer_ = base::MakeRefCounted<IOBufferWithSize>(kDefaultBufferSize);
    int error = OK;
    do {
        TestCompletionCallback callback;
...
TEST_F(BrotliSourceStreamTest, WithDictionary)
{
    ...
    std::unique_ptr<SourceStream> brotli_stream = CreateBrotliSourceStreamWithDictionary(std::move(source), dictionary_buffer, dictionary_data.size());

    TestCompletionCallback callback;
    int bytes_read = brotli_stream->Read(out_buffer.get(), kDefaultBufferSize, callback.callback());
