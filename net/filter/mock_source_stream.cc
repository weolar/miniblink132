...
int MockSourceStream::Read(IOBuffer* dest_buffer, int buffer_size, CompletionOnceCallback callback)
{
    DCHECK(!awaiting_completion_);
...
MockSourceStream::QueuedResult::QueuedResult(const char* data, int len, Error error, Mode mode)
    : data(data)
    , len(len)
    , error(error)
    , mode(mode)
...
void MockSourceStream::AddReadResult(const char* data, int len, Error error, Mode mode)
{
    if (error != OK) {
