...
std::unique_ptr<GzipSourceStream> GzipSourceStream::Create(std::unique_ptr<SourceStream> upstream, SourceStream::SourceType type)
{
    DCHECK(type == TYPE_GZIP || type == TYPE_DEFLATE);
    auto source = base::WrapUnique(new GzipSourceStream(std::move(upstream), type));
...
GzipSourceStream::GzipSourceStream(std::unique_ptr<SourceStream> upstream, SourceStream::SourceType type)
...
base::expected<size_t, Error> GzipSourceStream::FilterData(
    IOBuffer* output_buffer, size_t output_buffer_size, IOBuffer* input_buffer, size_t input_buffer_size, size_t* consumed_bytes, bool upstream_end_reached)
{
    *consumed_bytes = 0;
    ...
    while (input_data_size > 0 && bytes_out < output_buffer_size) {
        ...
        case STATE_GZIP_HEADER: {
            ...
            GZipHeader::Status status = gzip_header_.ReadMore(input_data, input_data_size, &end);
        ...
        case STATE_SNIFFING_DEFLATE_HEADER: {
            ...
            zlib_stream_.get()->next_out = reinterpret_cast<Bytef*>(output_buffer->data());
            ...
            if (bytes_out > 0 || bytes_used + replay_data_.size() >= kMaxZlibHeaderSniffBytes || ret == Z_STREAM_END) {
                replay_data_.clear();
                if (ret == Z_STREAM_END) {
        ...
        case STATE_REPLAY_DATA: {
            ...
            input_state_ = replay_state_;
            size_t bytes_used;
            scoped_refptr<IOBuffer> replay_buffer = base::MakeRefCounted<WrappedIOBuffer>(replay_data_);
            base::expected<size_t, Error> result
                = FilterData(output_buffer, output_buffer_size, replay_buffer.get(), replay_data_.size(), &bytes_used, upstream_end_reached);
