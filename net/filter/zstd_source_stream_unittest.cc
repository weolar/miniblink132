...
protected:
    ...
    {
        return zstd_stream_->Read(out_buffer(), out_buffer_size(), std::move(callback));
    }
    ...
    {
        return source_data_.length();
    }

    char* encoded_buffer()
    {
        return &encoded_buffer_[0];
    }
    size_t encoded_buffer_len()
    {
        return encoded_buffer_.length();
    }

    MockSourceStream* source()
    {
        return source_;
    }
    SourceStream* zstd_stream()
    {
        return zstd_stream_.get();
    }
...
TEST_F(ZstdSourceStreamTest, DecodeZstdOneBlockSync)
{
    ...
    source()->AddReadResult(encoded_buffer(), encoded_buffer_len(), OK, MockSourceStream::SYNC);
    ...
    histograms.ExpectUniqueSample("Net.ZstdFilter.Status", static_cast<int>(ZstdDecodingStatus::kEndOfFrame), 1);
}
...
TEST_F(ZstdSourceStreamTest, IgnoreExtraDataInOneRead)
{
    ...
    source()->AddReadResult(response_with_extra_data.data(), response_with_extra_data.length(), OK, MockSourceStream::SYNC);
    // Add an EOF.
    source()->AddReadResult(nullptr, 0, OK, MockSourceStream::SYNC);
...
TEST_F(ZstdSourceStreamTest, IgnoreExtraDataInDifferentRead)
{
    ...
    source()->AddReadResult(encoded_buffer(), encoded_buffer_len(), OK, MockSourceStream::SYNC);
    source()->AddReadResult(extra_data.c_str(), extra_data.length(), OK, MockSourceStream::SYNC);
    // Add an EOF.
...
TEST_F(ZstdSourceStreamTest, DecodeZstdTwoBlockSync)
{
    ...
    source()->AddReadResult(encoded_buffer() + 10, encoded_buffer_len() - 10, OK, MockSourceStream::SYNC);
    TestCompletionCallback callback;
    int bytes_read = ReadStream(callback.callback());
    EXPECT_EQ(static_cast<int>(source_data_len()), bytes_read);
    EXPECT_EQ(0, memcmp(out_data(), source_data().c_str(), source_data_len()));
...
{
    source()->AddReadResult(encoded_buffer(), encoded_buffer_len(), OK, MockSourceStream::ASYNC);
    // Add an EOF.
    source()->AddReadResult(nullptr, 0, OK, MockSourceStream::ASYNC);

    scoped_refptr<IOBuffer> buffer = base::MakeRefCounted<IOBufferWithSize>(source_data_len());
...
TEST_F(ZstdSourceStreamTest, DecodeTwoConcatenatedFrames)
{
    ...
    scoped_refptr<IOBufferWithSize> out_buffer = base::MakeRefCounted<IOBufferWithSize>(kLargeBufferSize);

    // Decompress content.
    auto source = std::make_unique<MockSourceStream>();
    source->AddReadResult(encoded_buffer.c_str(), encoded_buffer.size(), OK, MockSourceStream::SYNC);
    source->AddReadResult(nullptr, 0, OK, MockSourceStream::SYNC);
    source->set_expect_all_input_consumed(false);

    std::unique_ptr<SourceStream> zstd_stream = CreateZstdSourceStream(std::move(source));

    std::string actual_output;
    while (true) {
        TestCompletionCallback callback;
        int bytes_read = zstd_stream->Read(out_buffer.get(), kLargeBufferSize, callback.callback());
        if (bytes_read <= OK) {
            break;
        }
...
TEST_F(ZstdSourceStreamTest, WithDictionary)
{
    ...
    scoped_refptr<net::IOBuffer> dictionary_buffer = base::MakeRefCounted<net::StringIOBuffer>(dictionary_data);

    scoped_refptr<IOBufferWithSize> out_buffer = base::MakeRefCounted<IOBufferWithSize>(kDefaultBufferSize);

    auto source = std::make_unique<MockSourceStream>();
    source->AddReadResult(encoded_buffer.c_str(), encoded_buffer.size(), OK, MockSourceStream::SYNC);

    std::unique_ptr<SourceStream> zstd_stream = CreateZstdSourceStreamWithDictionary(std::move(source), dictionary_buffer, dictionary_data.size());

    TestCompletionCallback callback;
    int bytes_read = zstd_stream->Read(out_buffer.get(), kDefaultBufferSize, callback.callback());

    EXPECT_EQ(static_cast<int>(source_data_len()), bytes_read);
    EXPECT_EQ(0, memcmp(out_buffer->data(), source_data().c_str(), source_data_len()));
}
...
TEST_F(ZstdSourceStreamTest, WindowSizeTooBig)
{
    ...
    constexpr uint8_t kNineMegWindowZstd[] = {  };
    out_data()[0] = 'e';

    source()->AddReadResult(reinterpret_cast<const char*>(kNineMegWindowZstd), sizeof(kNineMegWindowZstd), OK, MockSourceStream::SYNC);
    ...
    histograms.ExpectTotalCount("Net.ZstdFilter.Status", 1);
    histograms.ExpectUniqueSample("Net.ZstdFilter.Status", static_cast<int>(ZstdDecodingStatus::kDecodingError), 1);
}
