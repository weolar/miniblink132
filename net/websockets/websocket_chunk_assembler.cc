...
std::unique_ptr<WebSocketFrame> MakeWebSocketFrame(const WebSocketFrameHeader& header, base::span<uint8_t> payload)
{
...
base::expected<std::unique_ptr<WebSocketFrame>, net::Error> WebSocketChunkAssembler::HandleChunk(std::unique_ptr<WebSocketFrameChunk> chunk)
{
    ...
    const bool is_control_frame = WebSocketFrameHeader::IsKnownControlOpCode(opcode) || WebSocketFrameHeader::IsReservedControlOpCode(opcode);
    const bool is_data_frame = WebSocketFrameHeader::IsKnownDataOpCode(opcode) || WebSocketFrameHeader::IsReservedDataOpCode(opcode);
    ...
    if (is_control_frame && current_frame_header_->payload_length > kMaxControlFramePayload) {
        return base::unexpected(ERR_WS_PROTOCOL_ERROR);
    }
    ...
    const bool is_empty_middle_chunk = !is_first_chunk && !is_final_chunk && chunk->payload.empty();
    ...
    if (is_single_chunk_frame) {
        ...
        auto frame = MakeWebSocketFrame(*current_frame_header_, base::as_writable_bytes(chunk->payload));
    ...
    auto frame = MakeWebSocketFrame(*current_frame_header_, base::as_writable_byte_span(chunk_buffer_));
