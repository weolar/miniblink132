...
constexpr size_t MIN_USEFUL_SIZE = BYTES_CONSUMED_BY_PARAMS + MIN_BYTES_TO_CREATE_A_FRAME - 1;
...
private:
    ...
    {
        WebSocketFrameHeader::OpCode opcode = fuzzed_data_provider_->ConsumeIntegralInRange<WebSocketFrameHeader::OpCode>(
            WebSocketFrameHeader::kOpCodeContinuation, WebSocketFrameHeader::kOpCodeControlUnusedF);
        ...
        uint64_t payload_length = fuzzed_data_provider_->ConsumeIntegralInRange(0, 64);
        std::vector<char> payload = fuzzed_data_provider_->ConsumeBytes<char>(payload_length);
...
void WebSocketDeflateStreamFuzz(const uint8_t* data, size_t size)
{
    ...
    params.Add(WebSocketExtension::Parameter("server_max_window_bits", base::NumberToString(server_max_window_bits)));
    params.Add(WebSocketExtension::Parameter("client_max_window_bits", base::NumberToString(client_max_window_bits)));
    ...
    WebSocketDeflateStream deflate_stream(
        std::make_unique<WebSocketFuzzedStream>(&fuzzed_data_provider), parameters, std::make_unique<WebSocketDeflatePredictorImpl>());
