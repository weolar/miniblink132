...
public:
    ...
    WebSocketDeflatePredictorMock& operator=(const WebSocketDeflatePredictorMock&) = delete;
    ...
    Result Predict(const std::vector<std::unique_ptr<WebSocketFrame>>& frames, size_t frame_index) override
    {
        return result_;
    }
    ...
    void AddFramesToBeInput(const std::vector<std::unique_ptr<WebSocketFrame>>& frames)
    {
        for (const auto& frame : frames)
            AddFrameToBeInput(frame.get());
    }
    void VerifySentFrames(const std::vector<std::unique_ptr<WebSocketFrame>>& frames)
    {
        for (const auto& frame : frames)
...
protected:
    ...
    void Initialize(WebSocketDeflater::ContextTakeOverMode mode, int window_bits)
    {
        ...
        auto mock_stream = std::make_unique<testing::StrictMock<MockWebSocketStream>>();
        auto predictor = std::make_unique<WebSocketDeflatePredictorMock>();
        mock_stream_ = mock_stream.get();
        predictor_ = predictor.get();
        deflate_stream_ = std::make_unique<WebSocketDeflateStream>(std::move(mock_stream), parameters, std::move(predictor));
    }

    void AppendTo(std::vector<std::unique_ptr<WebSocketFrame>>* frames, WebSocketFrameHeader::OpCode opcode, FrameFlag flag)
    {
    ...
    void AppendTo(std::vector<std::unique_ptr<WebSocketFrame>>* frames, WebSocketFrameHeader::OpCode opcode, FrameFlag flag, const std::string& data)
    {
        auto frame = std::make_unique<WebSocketFrame>(opcode);
        frame->header.final = (flag & kFinal);
        frame->header.reserved1 = (flag & kReserved1);
        auto buffer = base::HeapArray<uint8_t>::CopiedFrom(base::as_byte_span(data));
...
// here.
class WebSocketDeflateStreamWithDoNotTakeOverContextTest : public WebSocketDeflateStreamTest {
public:
...
class WebSocketDeflateStreamWithClientWindowBitsTest : public WebSocketDeflateStreamTest {
public:
...
public:
    ...
    ReadFramesStub(int result, std::vector<std::unique_ptr<WebSocketFrame>>* frames_to_output)
        : result_(result)
    {
        frames_to_output_.swap(*frames_to_output);
    }

    int Call(std::vector<std::unique_ptr<WebSocketFrame>>* frames, CompletionOnceCallback callback)
    {
        DCHECK(frames->empty());
...
public:
    ...
    int Call(std::vector<std::unique_ptr<WebSocketFrame>>* frames, CompletionOnceCallback callback)
    {
        frames_.insert(frames_.end(), std::make_move_iterator(frames->begin()), std::make_move_iterator(frames->end()));
        frames->clear();
        callback_ = std::move(callback);
...
TEST_F(WebSocketDeflateStreamTest, ReadFailedImmediately)
{
    ...
    {
        InSequence s;
        EXPECT_CALL(*mock_stream_, ReadFrames(&frames, _)).WillOnce(Return(ERR_FAILED));
    }
    EXPECT_THAT(deflate_stream_->ReadFrames(&frames, CompletionOnceCallback()), IsError(ERR_FAILED));
}
...
TEST_F(WebSocketDeflateStreamTest, ReadUncompressedFrameImmediately)
{
    ...
    AppendTo(&frames_to_output, WebSocketFrameHeader::kOpCodeText, kFinal, "hello");
    ReadFramesStub stub(OK, &frames_to_output);
    std::vector<std::unique_ptr<WebSocketFrame>> frames;

    {
        InSequence s;
        EXPECT_CALL(*mock_stream_, ReadFrames(&frames, _)).WillOnce(Invoke(&stub, &ReadFramesStub::Call));
    }
    ASSERT_THAT(deflate_stream_->ReadFrames(&frames, CompletionOnceCallback()), IsOk());
...
TEST_F(WebSocketDeflateStreamTest, ReadUncompressedFrameAsync)
{
    ...
    ASSERT_THAT(deflate_stream_->ReadFrames(&frames, mock_callback.Get()), IsError(ERR_IO_PENDING));
    ASSERT_EQ(0u, frames.size());

    checkpoint.Run();

    AppendTo(stub.frames_passed(), WebSocketFrameHeader::kOpCodeText, kFinal, "hello");
...
{
    std::vector<std::unique_ptr<WebSocketFrame>> frames_to_output;
    AppendTo(&frames_to_output, WebSocketFrameHeader::kOpCodeText, kFinal | kReserved1, std::string("\xf2\x48\xcd\xc9\xc9\x07\x00", 7));
    ReadFramesStub stub(OK, &frames_to_output);
    std::vector<std::unique_ptr<WebSocketFrame>> frames;
    {
        InSequence s;
...
TEST_F(WebSocketDeflateStreamTest, ReadCompressedFrameAsync)
{
    ...
    AppendTo(stub.frames_passed(), WebSocketFrameHeader::kOpCodeText, kFinal | kReserved1, std::string("\xf2\x48\xcd\xc9\xc9\x07\x00", 7));
    std::move(stub.callback()).Run(OK);
...
TEST_F(WebSocketDeflateStreamTest, ReadCompressedFrameFragmentImmediatelyButInflaterReturnsPending)
{
    std::vector<std::unique_ptr<WebSocketFrame>> frames_to_output;
    const std::string data1("\xf2", 1);
    const std::string data2("\x48\xcd\xc9\xc9\x07\x00", 6);
    AppendTo(&frames_to_output, WebSocketFrameHeader::kOpCodeText, kReserved1, data1);
    ...
    {
        InSequence s;
        EXPECT_CALL(*mock_stream_, ReadFrames(&frames, _)).WillOnce(Invoke(&stub1, &ReadFramesStub::Call)).WillOnce(Invoke(&stub2, &ReadFramesStub::Call));
        EXPECT_CALL(checkpoint, Run());
        EXPECT_CALL(mock_callback, Run(OK));
    }
    ASSERT_THAT(deflate_stream_->ReadFrames(&frames, mock_callback.Get()), IsError(ERR_IO_PENDING));
    ...
    AppendTo(stub2.frames_passed(), WebSocketFrameHeader::kOpCodeText, kFinal, data2);
...
TEST_F(WebSocketDeflateStreamTest, ReadInvalidCompressedPayload)
{
    ...
    std::vector<std::unique_ptr<WebSocketFrame>> frames_to_output;
    AppendTo(&frames_to_output, WebSocketFrameHeader::kOpCodeText, kFinal | kReserved1, data);
    ReadFramesStub stub(OK, &frames_to_output);
    std::vector<std::unique_ptr<WebSocketFrame>> frames;
    ...
    ASSERT_EQ(ERR_WS_PROTOCOL_ERROR, deflate_stream_->ReadFrames(&frames, CompletionOnceCallback()));
    ASSERT_EQ(0u, frames.size());
}
...
TEST_F(WebSocketDeflateStreamTest, MergeMultipleFramesInReadFrames)
{
    ...
    std::vector<std::unique_ptr<WebSocketFrame>> frames_to_output;
    AppendTo(&frames_to_output, WebSocketFrameHeader::kOpCodeText, kReserved1, data1);
    AppendTo(&frames_to_output, WebSocketFrameHeader::kOpCodeContinuation, kFinal, data2);
    ReadFramesStub stub(OK, &frames_to_output);
    std::vector<std::unique_ptr<WebSocketFrame>> frames;
...
TEST_F(WebSocketDeflateStreamTest, ReadUncompressedEmptyFrames)
{
    ...
    AppendTo(&frames_to_output, WebSocketFrameHeader::kOpCodeContinuation, kFinal);
    ReadFramesStub stub(OK, &frames_to_output);
    std::vector<std::unique_ptr<WebSocketFrame>> frames;
    ...
    EXPECT_EQ(WebSocketFrameHeader::kOpCodeContinuation, frames[1]->header.opcode);
...
{
    std::vector<std::unique_ptr<WebSocketFrame>> frames_to_output;
    AppendTo(&frames_to_output, WebSocketFrameHeader::kOpCodeText, kReserved1, std::string("\x02\x00", 1));
    AppendTo(&frames_to_output, WebSocketFrameHeader::kOpCodeContinuation, kFinal);
    ReadFramesStub stub(OK, &frames_to_output);
    std::vector<std::unique_ptr<WebSocketFrame>> frames;
...
TEST_F(WebSocketDeflateStreamTest, ReadCompressedFrameFollowedByEmptyFrame)
{
    ...
    std::vector<std::unique_ptr<WebSocketFrame>> frames_to_output;
    AppendTo(&frames_to_output, WebSocketFrameHeader::kOpCodeText, kReserved1, data);
    AppendTo(&frames_to_output, WebSocketFrameHeader::kOpCodeContinuation, kFinal);
    ReadFramesStub stub(OK, &frames_to_output);
    std::vector<std::unique_ptr<WebSocketFrame>> frames;
...
TEST_F(WebSocketDeflateStreamTest, SplitToMultipleFramesInReadFrames)
{
    ...
    std::vector<std::unique_ptr<WebSocketFrame>> frames_to_output;
    AppendTo(&frames_to_output, WebSocketFrameHeader::kOpCodeBinary, kFinal | kReserved1, ToString(deflater.GetOutput(deflater.CurrentOutputSize())));
    ...
    EXPECT_EQ(WebSocketFrameHeader::kOpCodeContinuation, frames[2]->header.opcode);
    ...
    EXPECT_EQ(original_data, ToString(frames[0]) + ToString(frames[1]) + ToString(frames[2]));
}
...
TEST_F(WebSocketDeflateStreamTest, InflaterInternalDataCanBeEmpty)
{
    ...
    std::vector<std::unique_ptr<WebSocketFrame>> frames_to_output;
    AppendTo(&frames_to_output, WebSocketFrameHeader::kOpCodeBinary, kReserved1, ToString(deflater.GetOutput(deflater.CurrentOutputSize())));
...
TEST_F(WebSocketDeflateStreamTest, Reserved1TurnsOnDuringReadingCompressedContinuationFrame)
{
    const std::string data1("\xf2\x48\xcd", 3);
    const std::string data2("\xc9\xc9\x07\x00", 4);
    std::vector<std::unique_ptr<WebSocketFrame>> frames_to_output;
    AppendTo(&frames_to_output, WebSocketFrameHeader::kOpCodeText, kReserved1, data1);
    AppendTo(&frames_to_output, WebSocketFrameHeader::kOpCodeContinuation, kFinal | kReserved1, data2);
    ReadFramesStub stub(OK, &frames_to_output);
    std::vector<std::unique_ptr<WebSocketFrame>> frames;
...
TEST_F(WebSocketDeflateStreamTest, Reserved1TurnsOnDuringReadingUncompressedContinuationFrame)
{
    std::vector<std::unique_ptr<WebSocketFrame>> frames_to_output;
    AppendTo(&frames_to_output, WebSocketFrameHeader::kOpCodeText, kNoFlag, "hello");
    AppendTo(&frames_to_output, WebSocketFrameHeader::kOpCodeContinuation, kFinal | kReserved1, "world");
    ReadFramesStub stub(OK, &frames_to_output);
    std::vector<std::unique_ptr<WebSocketFrame>> frames;
...
TEST_F(WebSocketDeflateStreamTest, ReadCompressedMessages)
{
    std::vector<std::unique_ptr<WebSocketFrame>> frames_to_output;
    AppendTo(
        &frames_to_output, WebSocketFrameHeader::kOpCodeText, kFinal | kReserved1, std::string("\x4a\xce\xcf\x2d\x28\x4a\x2d\x2e\x4e\x4d\x31\x04\x00", 13));
    AppendTo(&frames_to_output, WebSocketFrameHeader::kOpCodeText, kFinal | kReserved1, std::string("\x4a\x86\x33\x8d\x00\x00", 6));
    ReadFramesStub stub(OK, &frames_to_output);
    std::vector<std::unique_ptr<WebSocketFrame>> frames;
...
{
    std::vector<std::unique_ptr<WebSocketFrame>> frames_to_output;
    AppendTo(&frames_to_output, WebSocketFrameHeader::kOpCodeText, kFinal, "uncompressed1");
    AppendTo(&frames_to_output, WebSocketFrameHeader::kOpCodeText, kFinal, "uncompressed2");
    ReadFramesStub stub(OK, &frames_to_output);
    std::vector<std::unique_ptr<WebSocketFrame>> frames;
...
TEST_F(WebSocketDeflateStreamTest, ReadCompressedMessageThenUncompressedMessage)
{
    std::vector<std::unique_ptr<WebSocketFrame>> frames_to_output;
    AppendTo(&frames_to_output, WebSocketFrameHeader::kOpCodeText, kFinal | kReserved1, std::string("\x4a\xce\xcf\x2d\x28\x4a\x2d\x2e\x4e\x4d\x01\x00", 12));
    AppendTo(&frames_to_output, WebSocketFrameHeader::kOpCodeText, kFinal, "uncompressed");
    ReadFramesStub stub(OK, &frames_to_output);
    std::vector<std::unique_ptr<WebSocketFrame>> frames;
...
TEST_F(WebSocketDeflateStreamTest, ReadUncompressedMessageThenCompressedMessage)
{
    std::vector<std::unique_ptr<WebSocketFrame>> frames_to_output;
    AppendTo(&frames_to_output, WebSocketFrameHeader::kOpCodeText, kFinal, "uncompressed");
    AppendTo(&frames_to_output, WebSocketFrameHeader::kOpCodeText, kFinal | kReserved1, std::string("\x4a\xce\xcf\x2d\x28\x4a\x2d\x2e\x4e\x4d\x01\x00", 12));
...
TEST_F(WebSocketDeflateStreamTest, ReadEmptyAsyncFrame)
{
    ...
    {
        InSequence s;
        EXPECT_CALL(*mock_stream_, ReadFrames(&frames, _)).WillOnce(Invoke(stub_vector[0].get(), &ReadFramesStub::Call));

        EXPECT_CALL(*mock_stream_, ReadFrames(&frames, _)).WillOnce(Invoke(stub_vector[1].get(), &ReadFramesStub::Call));

        EXPECT_CALL(mock_callback, Run(OK));
    }

    ASSERT_THAT(deflate_stream_->ReadFrames(&frames, mock_callback.Get()), IsError(ERR_IO_PENDING));
    AppendTo(stub_vector[0]->frames_passed(), WebSocketFrameHeader::kOpCodeText, kReserved1, std::string());
    std::move(stub_vector[0]->callback()).Run(OK);
    AppendTo(stub_vector[1]->frames_passed(), WebSocketFrameHeader::kOpCodeContinuation, kFinal, std::string("\x02\x00"));
...
TEST_F(WebSocketDeflateStreamTest, WriteEmpty)
{
    ...
    EXPECT_THAT(deflate_stream_->WriteFrames(&frames, CompletionOnceCallback()), IsOk());
}
...
TEST_F(WebSocketDeflateStreamTest, WriteFailedImmediately)
{
    ...
    {
        InSequence s;
        EXPECT_CALL(*mock_stream_, WriteFrames(&frames, _)).WillOnce(Return(ERR_FAILED));
    }

    AppendTo(&frames, WebSocketFrameHeader::kOpCodeText, kFinal, "hello");
    predictor_->AddFramesToBeInput(frames);
    EXPECT_THAT(deflate_stream_->WriteFrames(&frames, CompletionOnceCallback()), IsError(ERR_FAILED));
    predictor_->Clear();
}
...
TEST_F(WebSocketDeflateStreamTest, WriteFrameImmediately)
{
    ...
    {
        InSequence s;
        EXPECT_CALL(*mock_stream_, WriteFrames(_, _)).WillOnce(Invoke(&stub, &WriteFramesStub::Call));
    }
    ASSERT_THAT(deflate_stream_->WriteFrames(&frames, CompletionOnceCallback()), IsOk());
    ...
    EXPECT_EQ(std::string("\xf2\x48\xcd\xc9\xc9\x07\x00", 7), ToString(frames_passed[0]));
}
...
TEST_F(WebSocketDeflateStreamTest, WriteFrameAsync)
{
    ...
    {
        InSequence s;
        EXPECT_CALL(*mock_stream_, WriteFrames(&frames, _)).WillOnce(Invoke(&stub, &WriteFramesStub::Call));
        EXPECT_CALL(checkpoint, Run());
        EXPECT_CALL(mock_callback, Run(OK));
    }
    AppendTo(&frames, WebSocketFrameHeader::kOpCodeText, kFinal, "Hello");
...
TEST_F(WebSocketDeflateStreamTest, WriteControlFrameBetweenDataFrames)
{
    ...
    EXPECT_EQ(std::string("\xf2\x48\xcd\xc9\xc9\x07\x00", 7), ToString(frames_passed[1]));
}
...
TEST_F(WebSocketDeflateStreamTest, WriteUncompressedMessage)
{
    ...
    EXPECT_EQ(WebSocketFrameHeader::kOpCodeContinuation, frames_passed[1]->header.opcode);
    EXPECT_TRUE(frames_passed[1]->header.final);
...
TEST_F(WebSocketDeflateStreamTest, LargeDeflatedFramesShouldBeSplit)
{
    ...
    {
        InSequence s;
        EXPECT_CALL(*mock_stream_, WriteFrames(_, _)).WillRepeatedly(Invoke(&stub, &WriteFramesStub::Call));
    }
    ...
    while (true) {
        ...
        total_compressed_frames.insert(
            total_compressed_frames.end(), std::make_move_iterator(stub.frames()->begin()), std::make_move_iterator(stub.frames()->end()));
        stub.frames()->clear();
        if (is_final)
            break;
    }
    ...
    EXPECT_EQ(total_deflated, ToString(deflater.GetOutput(deflater.CurrentOutputSize())));
}
...
TEST_F(WebSocketDeflateStreamWithDoNotTakeOverContextTest, WriteMultipleMessages)
{
    std::vector<std::unique_ptr<WebSocketFrame>> frames;
    AppendTo(&frames, WebSocketFrameHeader::kOpCodeText, kFinal, "Hello");
    AppendTo(&frames, WebSocketFrameHeader::kOpCodeText, kFinal, "Hello");
...
TEST_F(WebSocketDeflateStreamWithDoNotTakeOverContextTest, WritePossiblyCompressMessages)
{
    std::vector<std::unique_ptr<WebSocketFrame>> frames;
    ...
    EXPECT_EQ(std::string("\x72\x74\x44\x00\x00\x00", 6), ToString(frames_passed[2]));
    ...
    EXPECT_EQ(WebSocketFrameHeader::kOpCodeContinuation, frames_passed[4]->header.opcode);
...
TEST_F(WebSocketDeflateStreamWithClientWindowBitsTest, WindowBits8)
{
    ...
    ASSERT_THAT(deflate_stream_->WriteFrames(&frames_, CompletionOnceCallback()), IsOk());
    const std::vector<std::unique_ptr<WebSocketFrame>>& frames_passed = *stub.frames();
    ASSERT_EQ(1u, frames_passed.size());
...
TEST_F(WebSocketDeflateStreamWithClientWindowBitsTest, WindowBits10)
{
    ...
    const std::vector<std::unique_ptr<WebSocketFrame>>& frames_passed = *stub.frames();
    ASSERT_EQ(1u, frames_passed.size());
    EXPECT_EQ(std::string("r\xce(\xca\xcf\xcd,\xcdM\x1c\xe1\xc0\x19\x1a\x0e\0\0", 17), ToString(frames_passed[0]));
}
