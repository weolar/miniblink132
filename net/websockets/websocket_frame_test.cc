...
TEST(WebSocketFrameHeaderTest, FrameLengths)
{
    ...
    static constexpr TestCase kTests[]
        = { { { "\x81\x00", 2 }, UINT64_C(0) }, { { "\x81\x7D", 2 }, UINT64_C(125) }, { { "\x81\x7E\x00\x7E", 4 }, UINT64_C(126) },
              { { "\x81\x7E\xFF\xFF", 4 }, UINT64_C(0xFFFF) }, { { "\x81\x7F\x00\x00\x00\x00\x00\x01\x00\x00", 10 }, UINT64_C(0x10000) },
              { { "\x81\x7F\x7F\xFF\xFF\xFF\xFF\xFF\xFF\xFF", 10 }, UINT64_C(0x7FFFFFFFFFFFFFFF) } };
    ...
    for (const auto& test : kTests) {
        ...
        std::vector<char> expected_output(test.frame_header.begin(), test.frame_header.end());
        std::vector<char> output(expected_output.size());
        EXPECT_EQ(static_cast<int>(expected_output.size()), WriteWebSocketFrameHeader(header, nullptr, base::as_writable_byte_span(output)));
...
TEST(WebSocketFrameHeaderTest, FrameLengthsWithMasking)
{
    ...
    static_assert(kMaskingKey.size() == WebSocketFrameHeader::kMaskingKeyLength, "incorrect masking key size");

    struct TestCase {
        const std::string_view frame_header;
        uint64_t frame_length;
    };
    static constexpr TestCase kTests[] = { { { "\x81\x80\xDE\xAD\xBE\xEF", 6 }, UINT64_C(0) }, { { "\x81\xFD\xDE\xAD\xBE\xEF", 6 }, UINT64_C(125) },
        { { "\x81\xFE\x00\x7E\xDE\xAD\xBE\xEF", 8 }, UINT64_C(126) }, { { "\x81\xFE\xFF\xFF\xDE\xAD\xBE\xEF", 8 }, UINT64_C(0xFFFF) },
        { { "\x81\xFF\x00\x00\x00\x00\x00\x01\x00\x00\xDE\xAD\xBE\xEF", 14 }, UINT64_C(0x10000) },
        { { "\x81\xFF\x7F\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xDE\xAD\xBE\xEF", 14 }, UINT64_C(0x7FFFFFFFFFFFFFFF) } };

    WebSocketMaskingKey masking_key;
    base::as_writable_byte_span(masking_key.key).copy_from(base::as_byte_span(kMaskingKey));
    ...
    for (const auto& test : kTests) {
        ...
        std::vector<char> expected_output(test.frame_header.begin(), test.frame_header.end());
        std::vector<char> output(expected_output.size());
        EXPECT_EQ(static_cast<int>(expected_output.size()), WriteWebSocketFrameHeader(header, &masking_key, base::as_writable_byte_span(output)));
        EXPECT_EQ(expected_output, output);
    }
}
...
TEST(WebSocketFrameHeaderTest, FrameOpCodes)
{
    ...
    static constexpr TestCase kTests[]
        = { { { "\x80\x00", 2 }, WebSocketFrameHeader::kOpCodeContinuation }, { { "\x81\x00", 2 }, WebSocketFrameHeader::kOpCodeText },
              { { "\x82\x00", 2 }, WebSocketFrameHeader::kOpCodeBinary }, { { "\x88\x00", 2 }, WebSocketFrameHeader::kOpCodeClose },
              { { "\x89\x00", 2 }, WebSocketFrameHeader::kOpCodePing }, { { "\x8A\x00", 2 }, WebSocketFrameHeader::kOpCodePong },
              // These are undefined opcodes, but the builder should accept them anyway.
              { { "\x83\x00", 2 }, 0x3 }, { { "\x84\x00", 2 }, 0x4 }, { { "\x85\x00", 2 }, 0x5 }, { { "\x86\x00", 2 }, 0x6 }, { { "\x87\x00", 2 }, 0x7 },
              { { "\x8B\x00", 2 }, 0xB }, { { "\x8C\x00", 2 }, 0xC }, { { "\x8D\x00", 2 }, 0xD }, { { "\x8E\x00", 2 }, 0xE }, { { "\x8F\x00", 2 }, 0xF } };
...
TEST(WebSocketFrameHeaderTest, FinalBitAndReservedBits)
{
    ...
    static constexpr TestCase kTests[] = { { { "\x81\x00", 2 }, true, false, false, false }, { { "\x01\x00", 2 }, false, false, false, false },
        { { "\xC1\x00", 2 }, true, true, false, false }, { { "\xA1\x00", 2 }, true, false, true, false }, { { "\x91\x00", 2 }, true, false, false, true },
        { { "\x71\x00", 2 }, false, true, true, true }, { { "\xF1\x00", 2 }, true, true, true, true } };
...
TEST(WebSocketFrameHeaderTest, InsufficientBufferSize)
{
    ...
    static constexpr TestCase kTests[]
        = { { UINT64_C(0), false, 2u }, { UINT64_C(125), false, 2u }, { UINT64_C(126), false, 4u }, { UINT64_C(0xFFFF), false, 4u },
              { UINT64_C(0x10000), false, 10u }, { UINT64_C(0x7FFFFFFFFFFFFFFF), false, 10u }, { UINT64_C(0), true, 6u }, { UINT64_C(125), true, 6u },
              { UINT64_C(126), true, 8u }, { UINT64_C(0xFFFF), true, 8u }, { UINT64_C(0x10000), true, 14u }, { UINT64_C(0x7FFFFFFFFFFFFFFF), true, 14u } };
    ...
    for (const auto& test : kTests) {
        ...
        EXPECT_EQ(ERR_INVALID_ARGUMENT, WriteWebSocketFrameHeader(header, nullptr, base::span(dummy_buffer).first(test.expected_header_size - 1)));
    }
}
...
TEST(WebSocketFrameTest, MaskPayload)
{
    ...
    static constexpr TestCase kTests[] = {
        ...
        { { "\x00\x00\x00\x00", WebSocketFrameHeader::kMaskingKeyLength }, 0, "FooBar", "FooBar", 6 },
        { "\xFF\xFF\xFF\xFF", 0, "FooBar", "\xB9\x90\x90\xBD\x9E\x8D", 6 },
    };

    for (const auto& test : kTests) {
        WebSocketMaskingKey masking_key;
        base::as_writable_byte_span(masking_key.key).copy_from(base::as_byte_span(test.masking_key));
        std::vector<char> frame_data(test.input, test.input + test.data_length);
        std::vector<char> expected_output(test.output, test.output + test.data_length);
        MaskWebSocketFramePayload(masking_key, test.frame_offset, base::as_writable_byte_span(frame_data));
...
TEST(WebSocketFrameTest, MaskPayloadAlignment)
{
    ...
    static constexpr size_t kMaskingKeyLength = WebSocketFrameHeader::kMaskingKeyLength;
    static constexpr size_t kScratchBufferSize = kMaxVectorAlignment + kMaxVectorSize * 2;
    ...
    static constexpr char kTestInput[] = { "\x3d\x77\x1d\x1b\x19\x8c\x48\xa3\x19\x6d\xf7\xcc\x39\xe7\x57\x0b"
    ...
    static constexpr char kTestOutput[] = { "\xef\xcd\x47\xa5\xcb\x36\x12\x1d\xcb\xd7\xad\x72\xeb\x5d\x0d\xb5"
    ...
    static_assert(std::size(kTestInput) == std::size(kTestOutput), "output and input arrays should have the same length");
    std::unique_ptr<char, base::AlignedFreeDeleter> scratch(static_cast<char*>(base::AlignedAlloc(kScratchBufferSize, kMaxVectorAlignment)));
    WebSocketMaskingKey masking_key;
    base::as_writable_byte_span(masking_key.key).copy_from(base::as_byte_span(kTestMask));
    for (size_t frame_offset = 0; frame_offset < kMaskingKeyLength; ++frame_offset) {
        for (size_t alignment = 0; alignment < kMaxVectorAlignment; ++alignment) {
            char* const aligned_scratch = scratch.get() + alignment;
            const size_t aligned_len = std::min(kScratchBufferSize - alignment, kTestInputSize - frame_offset);
            for (size_t chunk_size = 1; chunk_size < kMaxVectorSize; ++chunk_size) {
                memcpy(aligned_scratch, kTestInput + frame_offset, aligned_len);
                for (size_t chunk_start = 0; chunk_start < aligned_len; chunk_start += chunk_size) {
                    const size_t this_chunk_size = std::min(chunk_size, aligned_len - chunk_start);
                    MaskWebSocketFramePayload(
                        masking_key, frame_offset + chunk_start, base::as_writable_bytes(base::make_span(aligned_scratch + chunk_start, this_chunk_size)));
                }
                // Stop the test if it fails, since we don't want to spew thousands of
                // failures.
                ASSERT_TRUE(std::equal(aligned_scratch, aligned_scratch + aligned_len, kTestOutput + frame_offset))
                    << "Output failed to match for frame_offset=" << frame_offset << ", alignment=" << alignment << ", chunk_size=" << chunk_size;
            }
        }
    }
}
