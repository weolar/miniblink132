...
HttpRequestHeaders WebSocketExtraHeadersToHttpRequestHeaders(const WebSocketExtraHeaders& headers)
{
...
std::string WebSocketStandardRequest(const std::string& path, const std::string& host, const url::Origin& origin,
    const WebSocketExtraHeaders& send_additional_request_headers, const WebSocketExtraHeaders& extra_headers)
{
    return WebSocketStandardRequestWithCookies(path, host, origin, /*cookies=*/ {}, send_additional_request_headers, extra_headers);
}

std::string WebSocketStandardRequestWithCookies(const std::string& path, const std::string& host, const url::Origin& origin,
    const WebSocketExtraHeaders& cookies, const WebSocketExtraHeaders& send_additional_request_headers, const WebSocketExtraHeaders& extra_headers)
{
    ...
    headers.SetHeader("Sec-WebSocket-Extensions", "permessage-deflate; client_max_window_bits");
...
{
    return base::StrCat({ "HTTP/1.1 101 Switching Protocols\r\n"
                          "Upgrade: websocket\r\n"
                          "Connection: Upgrade\r\n"
...
quiche::HttpHeaderBlock WebSocketHttp2Request(
    const std::string& path, const std::string& authority, const std::string& origin, const WebSocketExtraHeaders& extra_headers)
{
    ...
    request_headers["sec-websocket-extensions"] = "permessage-deflate; client_max_window_bits";
...
quiche::HttpHeaderBlock WebSocketHttp2Response(const WebSocketExtraHeaders& extra_headers)
{
...
WebSocketMockClientSocketFactoryMaker::~WebSocketMockClientSocketFactoryMaker() = default;
...
void WebSocketMockClientSocketFactoryMaker::SetExpectations(const std::string& expect_written, const std::string& return_to_read)
{
    ...
    int sequence = 0;
    detail_->write = MockWrite(SYNCHRONOUS, detail_->expect_written.data(), detail_->expect_written.size(), sequence++);
    ...
    for (size_t place = 0; place < detail_->return_to_read.size(); place += kHttpStreamParserBufferSize) {
        detail_->reads.emplace_back(
            SYNCHRONOUS, detail_->return_to_read.data() + place, std::min(detail_->return_to_read.size() - place, kHttpStreamParserBufferSize), sequence++);
    }
    auto socket_data = std::make_unique<SequencedSocketData>(detail_->reads, base::make_span(&detail_->write, 1u));
    socket_data->set_connect_data(MockConnect(SYNCHRONOUS, OK));
    AddRawExpectations(std::move(socket_data));
}

void WebSocketMockClientSocketFactoryMaker::AddRawExpectations(std::unique_ptr<SequencedSocketData> socket_data)
{
    detail_->factory.AddSocketDataProvider(socket_data.get());
    detail_->socket_data_vector.push_back(std::move(socket_data));
}

void WebSocketMockClientSocketFactoryMaker::AddSSLSocketDataProvider(std::unique_ptr<SSLSocketDataProvider> ssl_socket_data)
{
...
{
    url_request_context_builder_->set_client_socket_factory_for_testing(maker_.factory());
...
WebSocketTestURLRequestContextHost::~WebSocketTestURLRequestContextHost() = default;

void WebSocketTestURLRequestContextHost::AddRawExpectations(std::unique_ptr<SequencedSocketData> socket_data)
{
    maker_.AddRawExpectations(std::move(socket_data));
}

void WebSocketTestURLRequestContextHost::AddSSLSocketDataProvider(std::unique_ptr<SSLSocketDataProvider> ssl_socket_data)
{
    maker_.AddSSLSocketDataProvider(std::move(ssl_socket_data));
}

void WebSocketTestURLRequestContextHost::SetProxyConfig(const std::string& proxy_rules)
{
    DCHECK(!url_request_context_);
    auto proxy_resolution_service = ConfiguredProxyResolutionService::CreateFixedForTest(proxy_rules, TRAFFIC_ANNOTATION_FOR_TESTS);
    url_request_context_builder_->set_proxy_resolution_service(std::move(proxy_resolution_service));
}

void DummyConnectDelegate::OnURLRequestConnected(URLRequest* request, const TransportInfo& info)
{
}

int DummyConnectDelegate::OnAuthRequired(const AuthChallengeInfo& auth_info, scoped_refptr<HttpResponseHeaders> response_headers,
    const IPEndPoint& host_port_pair, base::OnceCallback<void(const AuthCredentials*)> callback, std::optional<AuthCredentials>* credentials)
{
    return OK;
}
...
URLRequestContext* WebSocketTestURLRequestContextHost::GetURLRequestContext()
{
    if (!url_request_context_) {
        url_request_context_builder_->set_network_delegate(std::make_unique<TestNetworkDelegate>());
...
void TestWebSocketStreamRequestAPI::OnBasicHandshakeStreamCreated(WebSocketBasicHandshakeStream* handshake_stream)
{
    handshake_stream->SetWebSocketKeyForTesting("dGhlIHNhbXBsZSBub25jZQ==");
}

void TestWebSocketStreamRequestAPI::OnHttp2HandshakeStreamCreated(WebSocketHttp2HandshakeStream* handshake_stream)
{
}

void TestWebSocketStreamRequestAPI::OnHttp3HandshakeStreamCreated(WebSocketHttp3HandshakeStream* handshake_stream)
{
}
} // namespace net
