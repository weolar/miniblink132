...
WebSocketInflater::WebSocketInflater(size_t input_queue_capacity, size_t output_buffer_capacity)
...
int WebSocketInflater::InflateWithFlush(const char* next_in, size_t avail_in)
{
    ...
    return Inflate(reinterpret_cast<const char*>(stream_->next_in), stream_->avail_in, Z_SYNC_FLUSH);
}

int WebSocketInflater::Inflate(const char* next_in, size_t avail_in, int flush)
{
...
std::pair<char*, size_t> WebSocketInflater::OutputBuffer::GetTail()
{
    ...
    return std::pair(&buffer_[tail_], std::min(capacity_ - Size(), buffer_.size() - tail_));
}
...
std::pair<char*, size_t> WebSocketInflater::InputQueue::Top()
{
    DCHECK(!IsEmpty());
    if (buffers_.size() == 1) {
        return std::pair(&buffers_.front()->data()[head_of_first_buffer_], tail_of_last_buffer_ - head_of_first_buffer_);
    }
    return std::pair(&buffers_.front()->data()[head_of_first_buffer_], capacity_ - head_of_first_buffer_);
}
...
size_t WebSocketInflater::InputQueue::PushToLastBuffer(const char* data, size_t size)
{
    DCHECK(!IsEmpty());
