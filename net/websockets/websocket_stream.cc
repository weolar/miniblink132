...
public:
    ...
    int OnConnected(URLRequest* request, const TransportInfo& info, CompletionOnceCallback callback) override;

    void OnReceivedRedirect(URLRequest* request, const RedirectInfo& redirect_info, bool* defer_redirect) override;

    void OnResponseStarted(URLRequest* request, int net_error) override;

    void OnAuthRequired(URLRequest* request, const AuthChallengeInfo& auth_info) override;
...
private:
    void OnAuthRequiredComplete(URLRequest* request, const AuthCredentials* auth_credentials);
...
public:
    WebSocketStreamRequestImpl(const GURL& url, const std::vector<std::string>& requested_subprotocols, const URLRequestContext* context,
        const url::Origin& origin, const SiteForCookies& site_for_cookies, StorageAccessApiStatus storage_access_api_status,
        const IsolationInfo& isolation_info, const HttpRequestHeaders& additional_headers, NetworkTrafficAnnotationTag traffic_annotation,
        std::unique_ptr<WebSocketStream::ConnectDelegate> connect_delegate, std::unique_ptr<WebSocketStreamRequestAPI> api_delegate)
        : delegate_(this)
        , connect_delegate_(std::move(connect_delegate))
        , url_request_(context->CreateRequest(url, DEFAULT_PRIORITY, &delegate_, traffic_annotation,
              /*is_for_websockets=*/true))
        , api_delegate_(std::move(api_delegate))
    {
        DCHECK_EQ(IsolationInfo::RequestType::kOther, isolation_info.request_type());
        ...
        cookie_util::AddOrRemoveStorageAccessApiOverride(url, storage_access_api_status, url_request_->initiator(), url_request_->cookie_setting_overrides());

        auto create_helper = std::make_unique<WebSocketHandshakeStreamCreateHelper>(connect_delegate_.get(), requested_subprotocols, this);
        url_request_->SetUserData(kWebSocketHandshakeUserDataKey, std::move(create_helper));
    ...
    void OnBasicHandshakeStreamCreated(WebSocketBasicHandshakeStream* handshake_stream) override
    {
        if (api_delegate_) {
    ...
    void OnHttp2HandshakeStreamCreated(WebSocketHttp2HandshakeStream* handshake_stream) override
    {
        if (api_delegate_) {
    ...
    void OnHttp3HandshakeStreamCreated(WebSocketHttp3HandshakeStream* handshake_stream) override
    {
        if (api_delegate_) {
    ...
    void Start(std::unique_ptr<base::OneShotTimer> timer)
    {
        ...
        timer_->Start(FROM_HERE, timeout, base::BindOnce(&WebSocketStreamRequestImpl::OnTimeout, base::Unretained(this)));
        url_request_->Start();
    }
    ...
    void PerformUpgrade()
    {
        ...
        if (!handshake_stream_) {
            ReportFailureWithMessage("No handshake stream has been created or handshake stream is already "
                                     "destroyed.",
        ...
        if (!handshake_stream_->CanReadFromStream()) {
            ReportFailureWithMessage("Handshake stream is not readable.", ERR_CONNECTION_CLOSED, std::nullopt);
            return;
        }
        ...
        auto handshake_response_info = std::make_unique<WebSocketHandshakeResponseInfo>(
            url_request_->url(), url_request_->response_headers(), url_request_->GetResponseRemoteEndpoint(), url_request_->response_time());
        connect_delegate_->OnSuccess(handshake_stream->Upgrade(), std::move(handshake_response_info));
    }
    ...
    std::string FailureMessageFromNetError(int net_error)
    {
        if (net_error == ERR_TUNNEL_CONNECTION_FAILED) {
            ...
            return base::StrCat({ "Error in connection establishment: ", ErrorToString(net_error) });
        }
    }
    ...
    void ReportFailure(int net_error, std::optional<int> response_code)
    {
        ...
        ReportFailureWithMessage(failure_message_, failure_net_error_.value_or(net_error), failure_response_code_ ? failure_response_code_ : response_code);
    }

    void ReportFailureWithMessage(const std::string& failure_message, int net_error, std::optional<int> response_code)
    {
...
private:
    void OnHandshakeStreamCreated(WebSocketHandshakeStreamBase* handshake_stream)
    {
...
int Delegate::OnConnected(URLRequest* request, const TransportInfo& info, CompletionOnceCallback callback)
{
    owner_->connect_delegate()->OnURLRequestConnected(request, info);
    return OK;
}

void Delegate::OnReceivedRedirect(URLRequest* request, const RedirectInfo& redirect_info, bool* defer_redirect)
{
    ...
    if (redirect_info.new_method != "GET" || redirect_info.new_url != expected_url) {
        ...
        DLOG(FATAL) << "Unauthorized WebSocket redirect to " << redirect_info.new_method << " " << redirect_info.new_url.spec();
        request->Cancel();
    }
}
...
void Delegate::OnResponseStarted(URLRequest* request, int net_error)
{
    ...
    const bool is_http2 = request->response_info().connection_info == HttpConnectionInfo::kHTTP2;
    ...
    if (net::IsLocalhost(request->url())) {
        ...
        base::UmaHistogramSparse("Net.WebSocket.ErrorCodes_NotLocalhost", -net_error);
    }
    ...
    case HTTP_UNAUTHORIZED:
        owner_->ReportFailureWithMessage("HTTP Authentication failed; no valid credentials available", net_error, response_code);
        return;

    case HTTP_PROXY_AUTHENTICATION_REQUIRED:
        owner_->ReportFailureWithMessage("Proxy authentication failed", net_error, response_code);
        return;
...
void Delegate::OnAuthRequired(URLRequest* request, const AuthChallengeInfo& auth_info)
{
    ...
    int rv = owner_->connect_delegate()->OnAuthRequired(auth_info, request->response_headers(), request->GetResponseRemoteEndpoint(),
        base::BindOnce(&Delegate::OnAuthRequiredComplete, base::Unretained(this), request), &credentials);
...
void Delegate::OnAuthRequiredComplete(URLRequest* request, const AuthCredentials* credentials)
{
    request->LogUnblocked();
    if (!credentials) {
        request->CancelAuth();
...
void Delegate::OnCertificateRequested(URLRequest* request, SSLCertRequestInfo* cert_request_info)
{
...
void Delegate::OnSSLCertificateError(URLRequest* request, int net_error, const SSLInfo& ssl_info, bool fatal)
{
    owner_->connect_delegate()->OnSSLCertificateError(std::make_unique<SSLErrorCallbacks>(request), net_error, ssl_info, fatal);
}
...
std::unique_ptr<WebSocketStreamRequest> WebSocketStream::CreateAndConnectStream(const GURL& socket_url, const std::vector<std::string>& requested_subprotocols,
    ...
    const HttpRequestHeaders& additional_headers, URLRequestContext* url_request_context, const NetLogWithSource& net_log,
    NetworkTrafficAnnotationTag traffic_annotation, std::unique_ptr<ConnectDelegate> connect_delegate)
{
    auto request = std::make_unique<WebSocketStreamRequestImpl>(socket_url, requested_subprotocols, url_request_context, origin, site_for_cookies,
        storage_access_api_status, isolation_info, additional_headers, traffic_annotation, std::move(connect_delegate), nullptr);
    request->Start(std::make_unique<base::OneShotTimer>());
    return std::move(request);
}

std::unique_ptr<WebSocketStreamRequest> WebSocketStream::CreateAndConnectStreamForTesting(const GURL& socket_url,
    const std::vector<std::string>& requested_subprotocols, const url::Origin& origin, const SiteForCookies& site_for_cookies,
    StorageAccessApiStatus storage_access_api_status, const IsolationInfo& isolation_info, const HttpRequestHeaders& additional_headers,
    URLRequestContext* url_request_context, const NetLogWithSource& net_log, NetworkTrafficAnnotationTag traffic_annotation,
    std::unique_ptr<WebSocketStream::ConnectDelegate> connect_delegate, std::unique_ptr<base::OneShotTimer> timer,
    std::unique_ptr<WebSocketStreamRequestAPI> api_delegate)
{
    auto request = std::make_unique<WebSocketStreamRequestImpl>(socket_url, requested_subprotocols, url_request_context, origin, site_for_cookies,
        storage_access_api_status, isolation_info, additional_headers, traffic_annotation, std::move(connect_delegate), std::move(api_delegate));
