...
WEBSOCKET_BASIC_STREAM_TEST_DEFINE_CONSTANT(PartialLargeFrame,
...
WEBSOCKET_BASIC_STREAM_TEST_DEFINE_CONSTANT(MultipleFrames, "\x81\x01X\x81\x01Y\x81\x01Z");
...
WEBSOCKET_BASIC_STREAM_TEST_DEFINE_CONSTANT(EmptyFinalContinuationFrame, "\x80\x00");
...
// invalid.
WEBSOCKET_BASIC_STREAM_TEST_DEFINE_CONSTANT(InvalidFrame, "\x81\x7E\x00\x07Invalid");
...
WEBSOCKET_BASIC_STREAM_TEST_DEFINE_CONSTANT(126BytePong,
    "\x8a\x7e\x00\x7eZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZ"
    "ZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZ");
WEBSOCKET_BASIC_STREAM_TEST_DEFINE_CONSTANT(CloseFrame, "\x88\x09\x03\xe8occludo");
WEBSOCKET_BASIC_STREAM_TEST_DEFINE_CONSTANT(WriteFrame, "\x81\x85\x00\x00\x00\x00Write");
WEBSOCKET_BASIC_STREAM_TEST_DEFINE_CONSTANT(MaskedEmptyPong, "\x8A\x80\x00\x00\x00\x00");
constexpr WebSocketMaskingKey kNulMaskingKey = { { '\0', '\0', '\0', '\0' } };
constexpr WebSocketMaskingKey kNonNulMaskingKey = { { '\x0d', '\x1b', '\x06', '\x17' } };
...
public:
    StrictStaticSocketDataProvider(base::span<const MockRead> reads, base::span<const MockWrite> writes, bool strict_mode)
...
protected:
    WebSocketBasicStreamSocketTest()
        : common_connect_job_params_(&factory_,
    ...
    std::unique_ptr<ClientSocketHandle> MakeTransportSocket(base::span<const MockRead> reads, base::span<const MockWrite> writes)
    {
        socket_data_ = std::make_unique<StrictStaticSocketDataProvider>(reads, writes, expect_all_io_to_complete_);
        ...
        ClientSocketPool::GroupId group_id(url::SchemeHostPort(url::kHttpScheme, "a", 80), PrivacyMode::PRIVACY_MODE_DISABLED, NetworkAnonymizationKey(),
            SecureDnsPolicy::kAllow, /*disable_cert_network_fetches=*/false);
        transport_socket->Init(group_id, null_params, std::nullopt /* proxy_annotation_tag */, MEDIUM, SocketTag(), ClientSocketPool::RespectLimits::ENABLED,
            CompletionOnceCallback(), ClientSocketPool::ProxyAuthCallback(), &pool_, NetLogWithSource());
    ...
    void CreateStream(base::span<const MockRead> reads, base::span<const MockWrite> writes)
    {
        stream_ = WebSocketBasicStream::CreateWebSocketBasicStreamForTesting(
            MakeTransportSocket(reads, writes), http_read_buffer_, sub_protocol_, extensions_, net_log_, generator_);
    }
...
class WebSocketBasicStreamSocketSingleReadTest : public WebSocketBasicStreamSocketTest {
protected:
...
class WebSocketBasicStreamSocketChunkedReadTest : public WebSocketBasicStreamSocketTest {
protected:
    ...
    void CreateChunkedRead(IoMode mode, const char data[], size_t data_size, int chunk_size, size_t number_of_chunks, LastFrameBehaviour last_frame_behaviour)
    {
        reads_.clear();
...
class WebSocketBasicStreamSocketWriteTest : public WebSocketBasicStreamSocketTest {
protected:
    ...
    {
        auto frame = std::make_unique<WebSocketFrame>(WebSocketFrameHeader::kOpCodeText);
        ...
        buffer->span().copy_from(base::byte_span_from_cstring(kWriteFrame).last(payload_size));
...
protected:
    ...
    {
        static const base::TimeTicks kStartPoint = base::TimeTicks::UnixEpoch() + base::Seconds(60);
...
{
    std::vector<std::unique_ptr<WebSocketFrame>> frames;
    CreateChunkedRead(SYNCHRONOUS, kSampleFrame, kSampleFrameSize, 1, 2, LAST_FRAME_BIG);
    int result = stream_->ReadFrames(&frames, cb_.callback());
    EXPECT_THAT(result, IsOk());
    ASSERT_EQ(1U, frames.size());
    EXPECT_EQ(UINT64_C(6), frames[0]->header.payload_length);
...
{
    std::vector<std::unique_ptr<WebSocketFrame>> frames;
    CreateChunkedRead(ASYNC, kSampleFrame, kSampleFrameSize, 1, 2, LAST_FRAME_BIG);
    int result = stream_->ReadFrames(&frames, cb_.callback());
    ASSERT_THAT(result, IsError(ERR_IO_PENDING));
    EXPECT_THAT(cb_.WaitForResult(), IsOk());
    ASSERT_EQ(1U, frames.size());
...
{
    std::vector<std::unique_ptr<WebSocketFrame>> frames;
    MockRead reads[] = { MockRead(SYNCHRONOUS, kSampleFrame, 1), MockRead(ASYNC, kSampleFrame + 1, kSampleFrameSize - 1) };
...
{
    std::vector<std::unique_ptr<WebSocketFrame>> frames;
    MockRead reads[] = { MockRead(SYNCHRONOUS, kPartialLargeFrame, kLargeFrameHeaderSize - 1), MockRead(SYNCHRONOUS, ERR_IO_PENDING) };
    CreateStream(reads, base::span<MockWrite>());
    EXPECT_THAT(stream_->ReadFrames(&frames, cb_.callback()), IsError(ERR_IO_PENDING));
}
...
TEST_F(WebSocketBasicStreamSocketSingleReadTest, LargeFrameFirstChunk)
{
    ...
    EXPECT_EQ(kPartialLargeFrameSize - kLargeFrameHeaderSize, static_cast<size_t>(frames[0]->header.payload_length));
}
...
{
    std::vector<std::unique_ptr<WebSocketFrame>> frames;
    MockRead reads[] = { MockRead(SYNCHRONOUS, kPartialLargeFrame, kLargeFrameHeaderSize),
        MockRead(ASYNC, kPartialLargeFrame + kLargeFrameHeaderSize, kPartialLargeFrameSize - kLargeFrameHeaderSize) };
    CreateStream(reads, base::span<MockWrite>());
    EXPECT_THAT(stream_->ReadFrames(&frames, cb_.callback()), IsOk());
    ASSERT_EQ(1U, frames.size());
    ASSERT_TRUE(frames[0]->payload.empty());
    ...
    EXPECT_THAT(cb_.WaitForResult(), IsOk());
    ASSERT_EQ(1U, frames.size());
    EXPECT_EQ(kPartialLargeFrameSize - kLargeFrameHeaderSize, frames[0]->header.payload_length);
    EXPECT_EQ(WebSocketFrameHeader::kOpCodeContinuation, frames[0]->header.opcode);
}
...
TEST_F(WebSocketBasicStreamSocketChunkedReadTest, LargeFrameTwoChunks)
{
    ...
    CreateChunkedRead(ASYNC, kPartialLargeFrame, kPartialLargeFrameSize, kChunkSize, 2, LAST_FRAME_NOT_BIG);
    TestCompletionCallback cb[2];

    ASSERT_THAT(stream_->ReadFrames(&frames, cb[0].callback()), IsError(ERR_IO_PENDING));
    EXPECT_THAT(cb[0].WaitForResult(), IsOk());
    ASSERT_EQ(1U, frames.size());
    EXPECT_EQ(kChunkSize - kLargeFrameHeaderSize, frames[0]->header.payload_length);

    frames.clear();
    ASSERT_THAT(stream_->ReadFrames(&frames, cb[1].callback()), IsError(ERR_IO_PENDING));
...
TEST_F(WebSocketBasicStreamSocketChunkedReadTest, OnlyFinalChunkIsFinal)
{
    ...
    CreateChunkedRead(ASYNC, kSampleFrame, kSampleFrameSize, kFirstChunkSize, 2, LAST_FRAME_BIG);
    TestCompletionCallback cb[2];
...
TEST_F(WebSocketBasicStreamSocketChunkedReadTest, ContinuationOpCodeUsed)
{
    ...
    // separate chunks.
    CreateChunkedRead(ASYNC, kSampleFrame, kSampleFrameSize, kFirstChunkSize, kChunkCount, LAST_FRAME_BIG);
    ...
    for (int i = 1; i < kChunkCount; ++i) {
        frames.clear();
        ASSERT_THAT(stream_->ReadFrames(&frames, cb[i].callback()), IsError(ERR_IO_PENDING));
...
TEST_F(WebSocketBasicStreamSocketSingleReadTest, SyncClose)
{
    ...
    EXPECT_EQ(ERR_CONNECTION_CLOSED, stream_->ReadFrames(&frames, cb_.callback()));
}
...
TEST_F(WebSocketBasicStreamSocketSingleReadTest, AsyncClose)
{
    ...
    ASSERT_THAT(stream_->ReadFrames(&frames, cb_.callback()), IsError(ERR_IO_PENDING));
...
TEST_F(WebSocketBasicStreamSocketSingleReadTest, SyncErrorsPassedThrough)
{
    ...
    EXPECT_EQ(ERR_INSUFFICIENT_RESOURCES, stream_->ReadFrames(&frames, cb_.callback()));
}
...
TEST_F(WebSocketBasicStreamSocketChunkedReadTest, CloseAfterFrame)
{
    ...
    // the connection.
    CreateChunkedRead(SYNCHRONOUS, kSampleFrame, kSampleFrameSize, kSampleFrameSize, 2, LAST_FRAME_NOT_BIG);
...
{
    std::vector<std::unique_ptr<WebSocketFrame>> frames;
    MockRead reads[] = { MockRead(ASYNC, kSampleFrame, 1U), MockRead(SYNCHRONOUS, "", 0) };
    CreateStream(reads, base::span<MockWrite>());
...
{
    std::vector<std::unique_ptr<WebSocketFrame>> frames;
    MockRead reads[] = { MockRead(ASYNC, kSampleFrame, 1U), MockRead(SYNCHRONOUS, ERR_CONNECTION_CLOSED) };
    CreateStream(reads, base::span<MockWrite>());
...
{
    std::vector<std::unique_ptr<WebSocketFrame>> frames;
    MockRead reads[] = { MockRead(SYNCHRONOUS, kEmptyFirstFrame, kEmptyFirstFrameSize), MockRead(SYNCHRONOUS, kEmptyMiddleFrame, kEmptyMiddleFrameSize),
...
{
    std::vector<std::unique_ptr<WebSocketFrame>> frames;
    MockRead reads[] = { MockRead(SYNCHRONOUS, kEmptyFirstFrame, kEmptyFirstFrameSize), MockRead(ASYNC, kEmptyMiddleFrame, kEmptyMiddleFrameSize),
...
{
    std::vector<std::unique_ptr<WebSocketFrame>> frames;
    CreateRead(MockRead(SYNCHRONOUS, kEmptyFinalTextFrame, kEmptyFinalTextFrameSize));
...
TEST_F(WebSocketBasicStreamSocketTest, ThreeFrameEmptyMessage)
{
    std::vector<std::unique_ptr<WebSocketFrame>> frames;
    MockRead reads[] = { MockRead(SYNCHRONOUS, kEmptyFirstFrame, kEmptyFirstFrameSize), MockRead(SYNCHRONOUS, kEmptyMiddleFrame, kEmptyMiddleFrameSize),
        MockRead(SYNCHRONOUS, kEmptyFinalContinuationFrame, kEmptyFinalContinuationFrameSize) };
    CreateStream(reads, base::span<MockWrite>());
...
TEST_F(WebSocketBasicStreamSocketSingleReadTest, PartialFrameHeaderInHttpResponse)
{
    std::vector<std::unique_ptr<WebSocketFrame>> frames;
...
// headers works correctly.
TEST_F(WebSocketBasicStreamSocketSingleReadTest, PartialControlFrameInHttpResponse)
{
    std::vector<std::unique_ptr<WebSocketFrame>> frames;
    constexpr size_t kPartialFrameBytes = 3;
    SetHttpReadBuffer(kCloseFrame, kPartialFrameBytes);
    CreateRead(MockRead(ASYNC, kCloseFrame + kPartialFrameBytes, kCloseFrameSize - kPartialFrameBytes));
    ...
    EXPECT_EQ(base::as_string_view(frames[0]->payload), std::string_view(kCloseFrame + 2, kCloseFrameSize - 2));
}

// Check that a control frame which partially arrives at the end of the response
// headers works correctly. Synchronous version (unlikely in practice).
TEST_F(WebSocketBasicStreamSocketSingleReadTest, PartialControlFrameInHttpResponseSync)
{
    std::vector<std::unique_ptr<WebSocketFrame>> frames;
    constexpr size_t kPartialFrameBytes = 3;
    SetHttpReadBuffer(kCloseFrame, kPartialFrameBytes);
    CreateRead(MockRead(SYNCHRONOUS, kCloseFrame + kPartialFrameBytes, kCloseFrameSize - kPartialFrameBytes));
...
TEST_F(WebSocketBasicStreamSocketSingleReadTest, SyncInvalidFrame)
{
    ...
    EXPECT_EQ(ERR_WS_PROTOCOL_ERROR, stream_->ReadFrames(&frames, cb_.callback()));
}
...
{
    std::vector<std::unique_ptr<WebSocketFrame>> frames;
    CreateRead(MockRead(SYNCHRONOUS, kPingFrameWithoutFin, kPingFrameWithoutFinSize));
...
TEST_F(WebSocketBasicStreamSocketChunkedReadTest, SplitOverlongControlFrame)
{
    ...
    CreateChunkedRead(SYNCHRONOUS, k126BytePong, k126BytePongSize, kFirstChunkSize, 2, LAST_FRAME_BIG);

    EXPECT_EQ(ERR_WS_PROTOCOL_ERROR, stream_->ReadFrames(&frames, cb_.callback()));
    EXPECT_TRUE(frames.empty());
}

TEST_F(WebSocketBasicStreamSocketChunkedReadTest, AsyncSplitOverlongControlFrame)
{
    std::vector<std::unique_ptr<WebSocketFrame>> frames;
    constexpr size_t kFirstChunkSize = 16;
    expect_all_io_to_complete_ = false;
    CreateChunkedRead(ASYNC, k126BytePong, k126BytePongSize, kFirstChunkSize, 2, LAST_FRAME_BIG);
...
TEST_F(WebSocketBasicStreamSocketTest, MultipleControlFramesInOneRead)
{
    ...
    MockRead reads[] = { MockRead(SYNCHRONOUS, kMultiplePongFrames, kMultiplePongFramesSize) };
    CreateStream(reads, base::span<MockWrite>());
...
TEST_F(WebSocketBasicStreamSocketChunkedReadTest, SyncControlFrameAssembly)
{
    ...
    CreateChunkedRead(SYNCHRONOUS, kCloseFrame, kCloseFrameSize, kChunkSize, 3, LAST_FRAME_BIG);
...
TEST_F(WebSocketBasicStreamSocketChunkedReadTest, AsyncControlFrameAssembly)
{
    std::vector<std::unique_ptr<WebSocketFrame>> frames;
    constexpr size_t kChunkSize = 3;
    CreateChunkedRead(ASYNC, kCloseFrame, kCloseFrameSize, kChunkSize, 3, LAST_FRAME_BIG);
...
TEST_F(WebSocketBasicStreamSocketChunkedReadTest, OneMegFrame)
{
    ...
    constexpr size_t kExpectedFrameCount = (kWireSize + kReadBufferSize - 1) / kReadBufferSize;
    std::vector<std::unique_ptr<WebSocketFrame>> frames;

    auto big_frame = base::HeapArray<uint8_t>::WithSize(kWireSize);
    auto [extended_header, payload] = big_frame.as_span().split_at(kLargeFrameHeaderSize);
    ...
    CreateChunkedRead(ASYNC, reinterpret_cast<char*>(big_frame.data()), big_frame.size(), kReadBufferSize, kExpectedFrameCount, LAST_FRAME_BIG);
    ...
    for (size_t frame = 0; frame < kExpectedFrameCount; ++frame) {
        ...
        if (frame == 0) {
            ...
            expected_payload_size = kWireSize - kReadBufferSize * (kExpectedFrameCount - 1);
        }
...
TEST_F(WebSocketBasicStreamSocketChunkedReadTest, ReservedFlagCleared)
{
    ...
    CreateChunkedRead(ASYNC, kReservedFlagFrame, kReservedFlagFrameSize, kChunkSize, 2, LAST_FRAME_BIG);
...
TEST_F(WebSocketBasicStreamSocketWriteTest, AsyncWriteAtOnce)
{
    ...
    ASSERT_THAT(stream_->WriteFrames(&frames_, cb_.callback()), IsError(ERR_IO_PENDING));
    EXPECT_THAT(cb_.WaitForResult(), IsOk());
}
...
{
    MockWrite writes[]
        = { MockWrite(SYNCHRONOUS, kWriteFrame, 4), MockWrite(ASYNC, kWriteFrame + 4, 4), MockWrite(ASYNC, kWriteFrame + 8, kWriteFrameSize - 8) };
    CreateStream(base::span<MockRead>(), writes);
...
{
    MockWrite writes[] = { MockWrite(SYNCHRONOUS, kMaskedEmptyPong, kMaskedEmptyPongSize) };
    CreateStream(base::span<MockRead>(), writes);

    auto frame = std::make_unique<WebSocketFrame>(WebSocketFrameHeader::kOpCodePong);
    WebSocketFrameHeader& header = frame->header;
    header.final = true;
    header.masked = true;
...
TEST_F(WebSocketBasicStreamSocketTest, WriteNonNulMask)
{
    ...
    masked_frame += std::string(std::begin(kNonNulMaskingKey.key), std::end(kNonNulMaskingKey.key));
    masked_frame += "jiggered";
    MockWrite writes[] = { MockWrite(SYNCHRONOUS, masked_frame.data(), masked_frame.size()) };
...
{
    EXPECT_EQ(buffer_size_manager_.buffer_size(), WebSocketBasicStream::BufferSize::kSmall);
...
TEST_F(WebSocketBasicStreamSwitchTest, ZeroSecondRead)
{
    ...
    EXPECT_EQ(buffer_size_manager_.buffer_size(), WebSocketBasicStream::BufferSize::kLarge);
}
