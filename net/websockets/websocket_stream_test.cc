...
// ends.
std::unique_ptr<SequencedSocketData> BuildSocketData(base::span<MockRead> reads, base::span<MockWrite> writes)
{
...
static IsolationInfo CreateIsolationInfo()
{
    ...
    return IsolationInfo::Create(IsolationInfo::RequestType::kOther, origin, origin, SiteForCookies::FromOrigin(origin));
}
...
protected:
        ...
        , spdy_util_(/*use_priority_header=*/true)
    {
        ...
        if (PriorityHeaderEnabled()) {
            feature_list_.InitWithFeatures({ features::kPartitionConnectionsByNetworkIsolationKey, net::features::kPriorityHeader }, {});
        } else {
            feature_list_.InitWithFeatures({ features::kPartitionConnectionsByNetworkIsolationKey }, { net::features::kPriorityHeader });
        }
    }
    ...
    void CreateAndConnectStandard(std::string_view url, const std::vector<std::string>& sub_protocols,
        const WebSocketExtraHeaders& send_additional_request_headers, const WebSocketExtraHeaders& extra_request_headers,
        const WebSocketExtraHeaders& extra_response_headers, StorageAccessApiStatus storage_access_api_status = StorageAccessApiStatus::kNone)
    {
        ...
        if (stream_type_ == BASIC_HANDSHAKE_STREAM) {
            url_request_context_host_.SetExpectations(
                WebSocketStandardRequest(socket_path, socket_host, Origin(), send_additional_request_headers, extra_request_headers),
                WebSocketStandardResponse(WebSocketExtraHeadersToString(extra_response_headers)) + additional_data_);
            CreateAndConnectStream(socket_url, sub_protocols, Origin(), SiteForCookies(), storage_access_api_status, CreateIsolationInfo(),
                WebSocketExtraHeadersToHttpRequestHeaders(send_additional_request_headers), std::move(timer_));
            return;
        }
        ...
        frames_.emplace_back(spdy::test::MakeSerializedFrame(const_cast<char*>(spdy::kHttp2ConnectionHeaderPrefix), spdy::kHttp2ConnectionHeaderPrefixSize));
        ...
        write_settings[spdy::SETTINGS_MAX_HEADER_LIST_SIZE] = kSpdyMaxHeaderListSize;
        ...
        const char* const kExtraRequestHeaders[] = { "user-agent", "", "accept-encoding", "gzip, deflate", "accept-language", "en-us,fr" };
        frames_.push_back(spdy_util_.ConstructSpdyGet(kExtraRequestHeaders, std::size(kExtraRequestHeaders) / 2, 1, DEFAULT_PRIORITY));
        AddWrite(&frames_.back());
        ...
        // WebSocket request.
        quiche::HttpHeaderBlock request_headers = WebSocketHttp2Request(socket_path, socket_host, kOrigin, extra_request_headers);
        frames_.push_back(spdy_util_.ConstructSpdyHeaders(3, std::move(request_headers), DEFAULT_PRIORITY, false));
        AddWrite(&frames_.back());

        if (reset_websocket_http2_stream_) {
            frames_.push_back(spdy_util_.ConstructSpdyRstStream(3, spdy::ERROR_CODE_CANCEL));
            AddRead(&frames_.back());
        } else {
            ...
            for (const auto& extra_header : extra_response_headers) {
                ...
                extra_response_header_keys.push_back(base::ToLowerASCII(extra_header.first));
                // Save a pointer to this lowercase copy.
                extra_response_headers_vector.push_back(extra_response_header_keys.back().c_str());
            ...
            frames_.push_back(
                spdy_util_.ConstructSpdyReplyError(http2_response_status_, extra_response_headers_vector.data(), extra_response_headers_vector.size() / 2, 3));
            AddRead(&frames_.back());

            // WebSocket data received.
            if (!additional_data_.empty()) {
                frames_.push_back(spdy_util_.ConstructSpdyDataFrame(3, additional_data_, true));
                AddRead(&frames_.back());
            }
        ...
        URLRequestContext* context = url_request_context_host_.GetURLRequestContext();
        TestDelegate delegate;
        std::unique_ptr<URLRequest> request
            = context->CreateRequest(GURL("https://www.example.org/"), DEFAULT_PRIORITY, &delegate, TRAFFIC_ANNOTATION_FOR_TESTS, /*is_for_websockets=*/false);
    ...
    void CreateAndConnectCustomResponse(std::string_view url, const std::vector<std::string>& sub_protocols,
        const WebSocketExtraHeaders& send_additional_request_headers, const WebSocketExtraHeaders& extra_request_headers, const std::string& response_body,
        StorageAccessApiStatus storage_access_api_status = StorageAccessApiStatus::kNone)
    {
        ...
        url_request_context_host_.SetExpectations(
            WebSocketStandardRequest(socket_path, socket_host, Origin(), send_additional_request_headers, extra_request_headers), response_body);
        CreateAndConnectStream(socket_url, sub_protocols, Origin(), SiteForCookies(), storage_access_api_status, CreateIsolationInfo(),
            WebSocketExtraHeadersToHttpRequestHeaders(send_additional_request_headers), nullptr);
    }
    ...
    // Only for HTTP/1-based WebSockets.
    void CreateAndConnectStringResponse(std::string_view url, const std::vector<std::string>& sub_protocols, const std::string& extra_response_headers,
        StorageAccessApiStatus storage_access_api_status = StorageAccessApiStatus::kNone)
    {
        ASSERT_EQ(BASIC_HANDSHAKE_STREAM, stream_type_);
        ...
        url_request_context_host_.SetExpectations(WebSocketStandardRequest(socket_path, socket_host, Origin(),
                                                      /*send_additional_request_headers=*/ {},
                                                      /*extra_headers=*/ {}),
            WebSocketStandardResponse(extra_response_headers));
        CreateAndConnectStream(
            socket_url, sub_protocols, Origin(), SiteForCookies(), storage_access_api_status, CreateIsolationInfo(), HttpRequestHeaders(), nullptr);
    }

    // Like CreateAndConnectStandard(), but take raw mock data.
    void CreateAndConnectRawExpectations(std::string_view url, const std::vector<std::string>& sub_protocols, const HttpRequestHeaders& additional_headers,
        std::unique_ptr<SequencedSocketData> socket_data, StorageAccessApiStatus storage_access_api_status = StorageAccessApiStatus::kNone)
    {
        ASSERT_EQ(BASIC_HANDSHAKE_STREAM, stream_type_);

        AddRawExpectations(std::move(socket_data));
        CreateAndConnectStream(
            GURL(url), sub_protocols, Origin(), SiteForCookies(), storage_access_api_status, CreateIsolationInfo(), additional_headers, std::move(timer_));
    }
...
private:
    ...
    {
        writes_.emplace_back(ASYNC, frame->data(), frame->size(), sequence_number_++);
    }

    void AddRead(const spdy::SpdySerializedFrame* frame)
    {
        reads_.emplace_back(ASYNC, frame->data(), frame->size(), sequence_number_++);
    }
...
INSTANTIATE_TEST_SUITE_P(All, WebSocketStreamCreateTest, testing::Combine(Values(BASIC_HANDSHAKE_STREAM), testing::Bool()));

using WebSocketMultiProtocolStreamCreateTest = WebSocketStreamCreateTest;

INSTANTIATE_TEST_SUITE_P(
    All, WebSocketMultiProtocolStreamCreateTest, testing::Combine(Values(BASIC_HANDSHAKE_STREAM, HTTP2_HANDSHAKE_STREAM), testing::Bool()));

// There are enough tests of the Sec-WebSocket-Extensions header that they
// deserve their own test fixture.
class WebSocketStreamCreateExtensionTest : public WebSocketMultiProtocolStreamCreateTest {
protected:
    ...
    void CreateAndConnectWithExtensions(const std::string& extensions_header_value)
    {
        AddSSLData();
        CreateAndConnectStandard("wss://www.example.org/testing_path", NoSubProtocols(), {}, {}, { { "Sec-WebSocket-Extensions", extensions_header_value } });
        WaitUntilConnectDone();
    }
};

INSTANTIATE_TEST_SUITE_P(All, WebSocketStreamCreateExtensionTest, testing::Combine(Values(BASIC_HANDSHAKE_STREAM, HTTP2_HANDSHAKE_STREAM), testing::Bool()));
...
public:
    ...
    std::unique_ptr<SequencedSocketData> BuildAuthSocketData(std::string response1, std::string request2, std::string response2)
    {
...
protected:
    void CreateAndConnectAuthHandshake(std::string_view url, std::string_view base64_user_pass, std::string_view response2)
    {
        CreateAndConnectRawExpectations(url, NoSubProtocols(), HttpRequestHeaders(),
            helper_.BuildAuthSocketData(kUnauthorizedResponse, RequestExpectation(base64_user_pass), std::string(response2)));
    }
    ...
    static std::string RequestExpectation(std::string_view base64_user_pass)
    {
        ...
        return base::StringPrintf("GET / HTTP/1.1\r\n"
...
INSTANTIATE_TEST_SUITE_P(All, WebSocketStreamCreateBasicAuthTest, testing::Combine(Values(BASIC_HANDSHAKE_STREAM), testing::Bool()));
...
INSTANTIATE_TEST_SUITE_P(All, WebSocketStreamCreateDigestAuthTest, testing::Combine(Values(BASIC_HANDSHAKE_STREAM), testing::Bool()));
...
TEST_P(WebSocketMultiProtocolStreamCreateTest, SimpleSuccess)
{
    ...
    EXPECT_EQ(ERR_WS_UPGRADE, url_request_context_host_.network_delegate().last_error());

    auto samples = histogram_tester.GetHistogramSamplesSinceCreation("Net.WebSocket.HandshakeResult2");
    EXPECT_EQ(1, samples->TotalCount());
    if (stream_type_ == BASIC_HANDSHAKE_STREAM) {
        EXPECT_EQ(1, samples->GetCount(static_cast<int>(WebSocketHandshakeStreamBase::HandshakeResult::CONNECTED)));
    } else {
        DCHECK_EQ(stream_type_, HTTP2_HANDSHAKE_STREAM);
        EXPECT_EQ(1, samples->GetCount(static_cast<int>(WebSocketHandshakeStreamBase::HandshakeResult::HTTP2_CONNECTED)));
    }
}

TEST_P(WebSocketStreamCreateTest, HandshakeInfo)
{
    static constexpr char kResponse[] = "HTTP/1.1 101 Switching Protocols\r\n"
                                        "Upgrade: websocket\r\n"
                                        "Connection: Upgrade\r\n"
    ...
    CreateAndConnectCustomResponse("ws://www.example.org/", NoSubProtocols(), {}, {}, kResponse);
    EXPECT_FALSE(request_info_);
    EXPECT_FALSE(response_info_);
    WaitUntilConnectDone();
    EXPECT_TRUE(stream_);
    ...
    EXPECT_EQ(HeaderKeyValuePair("Cache-Control", "no-cache"), request_headers[3]);
    EXPECT_EQ(HeaderKeyValuePair("Upgrade", "websocket"), request_headers[4]);
    EXPECT_EQ(HeaderKeyValuePair("Origin", "http://www.example.org"), request_headers[5]);
    EXPECT_EQ(HeaderKeyValuePair("Sec-WebSocket-Version", "13"), request_headers[6]);
    EXPECT_EQ(HeaderKeyValuePair("User-Agent", ""), request_headers[7]);
    EXPECT_EQ(HeaderKeyValuePair("Accept-Encoding", "gzip, deflate"), request_headers[8]);
    EXPECT_EQ(HeaderKeyValuePair("Accept-Language", "en-us,fr"), request_headers[9]);
    EXPECT_EQ("Sec-WebSocket-Key", request_headers[10].first);
    EXPECT_EQ(HeaderKeyValuePair("Sec-WebSocket-Extensions", "permessage-deflate; client_max_window_bits"), request_headers[11]);
...
{
    WebSocketExtraHeaders additional_headers({ { "User-Agent", "OveRrIde" }, { "rAnDomHeader", "foobar" } });
    CreateAndConnectStandard("ws://www.example.org/", NoSubProtocols(), additional_headers, additional_headers, {});
    EXPECT_FALSE(request_info_);
    EXPECT_FALSE(response_info_);
    WaitUntilConnectDone();
    EXPECT_FALSE(has_failed());
...
{
    CreateAndConnectStandard("ws://www.example.org/", NoSubProtocols(), {}, {}, {}, StorageAccessApiStatus::kNone);
    WaitUntilConnectDone();

    EXPECT_THAT(url_request_context_host_.network_delegate().cookie_setting_overrides_records(),
        testing::ElementsAre(CookieSettingOverrides(), CookieSettingOverrides()));
}

TEST_P(WebSocketStreamCreateTest, PlumbsHasStorageAccess)
{
    CreateAndConnectStandard("ws://www.example.org/", NoSubProtocols(), {}, {}, {}, StorageAccessApiStatus::kAccessViaAPI);
    WaitUntilConnectDone();

    CookieSettingOverrides expected_overrides;
    expected_overrides.Put(CookieSettingOverride::kStorageAccessGrantEligible);

    EXPECT_THAT(url_request_context_host_.network_delegate().cookie_setting_overrides_records(), testing::ElementsAre(expected_overrides, expected_overrides));
}
...
{
    AddSSLData();
    CreateAndConnectStandard("wss://www.example.org/testing_path", NoSubProtocols(), {}, {}, {});
    WaitUntilConnectDone();
    EXPECT_FALSE(has_failed());
    EXPECT_TRUE(stream_);
}
...
TEST_P(WebSocketMultiProtocolStreamCreateTest, SubProtocolIsUsed)
{
    ...
    CreateAndConnectStandard("wss://www.example.org/testing_path", sub_protocols, {},
        { { "Sec-WebSocket-Protocol", "chatv11.chromium.org, chatv20.chromium.org" } }, { { "Sec-WebSocket-Protocol", "chatv20.chromium.org" } });
    WaitUntilConnectDone();
    ASSERT_TRUE(stream_);
    EXPECT_FALSE(has_failed());
...
TEST_P(WebSocketMultiProtocolStreamCreateTest, UnsolicitedSubProtocol)
{
    ...
    AddSSLData();
    CreateAndConnectStandard("wss://www.example.org/testing_path", NoSubProtocols(), {}, {}, { { "Sec-WebSocket-Protocol", "chatv20.chromium.org" } });
    WaitUntilConnectDone();
    EXPECT_FALSE(stream_);
    ...
    EXPECT_EQ("Error during WebSocket handshake: "
        ...
        failure_message());
    EXPECT_EQ(ERR_INVALID_RESPONSE, url_request_context_host_.network_delegate().last_error());
    ...
    EXPECT_EQ(1, samples->TotalCount());
    if (stream_type_ == BASIC_HANDSHAKE_STREAM) {
        EXPECT_EQ(1, samples->GetCount(static_cast<int>(WebSocketHandshakeStreamBase::HandshakeResult::FAILED_SUBPROTO)));
    } else {
        DCHECK_EQ(stream_type_, HTTP2_HANDSHAKE_STREAM);
        EXPECT_EQ(1, samples->GetCount(static_cast<int>(WebSocketHandshakeStreamBase::HandshakeResult::HTTP2_FAILED_SUBPROTO)));
    }
}
...
TEST_P(WebSocketMultiProtocolStreamCreateTest, UnacceptedSubProtocol)
{
    ...
    CreateAndConnectStandard("wss://www.example.org/testing_path", sub_protocols, {}, { { "Sec-WebSocket-Protocol", "chat.example.com" } }, {});
    WaitUntilConnectDone();
    EXPECT_FALSE(stream_);
    EXPECT_TRUE(has_failed());
    EXPECT_EQ("Error during WebSocket handshake: "
...
TEST_P(WebSocketMultiProtocolStreamCreateTest, UnmatchedSubProtocolInResponse)
{
    ...
    sub_protocols.push_back("chatv20.chromium.org");
    CreateAndConnectStandard("wss://www.example.org/testing_path", sub_protocols, {},
        { { "Sec-WebSocket-Protocol", "chatv11.chromium.org, chatv20.chromium.org" } }, { { "Sec-WebSocket-Protocol", "chatv21.chromium.org" } });
    WaitUntilConnectDone();
    EXPECT_FALSE(stream_);
    EXPECT_TRUE(has_failed());
    EXPECT_EQ("Error during WebSocket handshake: "
...
{
    CreateAndConnectWithExtensions("permessage-deflate; client_no_context_takeover; "
...
{
    AddSSLData();
    SetAdditionalResponseData(std::string("\xc1\x07" // WebSocket header (FIN + RSV1, Text payload 7 bytes)
                                          "\xf2\x48\xcd\xc9\xc9\x07\x00", // "Hello" DEFLATE compressed
        9));
    CreateAndConnectStandard("wss://www.example.org/testing_path", NoSubProtocols(), {}, {}, { { "Sec-WebSocket-Extensions", "permessage-deflate" } });
    WaitUntilConnectDone();
...
TEST_P(WebSocketStreamCreateExtensionTest, MalformedExtension)
{
    ...
    EXPECT_FALSE(stream_);
    EXPECT_TRUE(has_failed());
    EXPECT_EQ("Error during WebSocket handshake: 'Sec-WebSocket-Extensions' header "
...
TEST_P(WebSocketStreamCreateExtensionTest, OnlyOnePerMessageDeflateAllowed)
{
    ...
    CreateAndConnectWithExtensions("permessage-deflate, permessage-deflate; client_max_window_bits=10");
    EXPECT_FALSE(stream_);
    EXPECT_TRUE(has_failed());
    EXPECT_EQ("Error during WebSocket handshake: "
    ...
    EXPECT_EQ(1, samples->TotalCount());
    if (stream_type_ == BASIC_HANDSHAKE_STREAM) {
        EXPECT_EQ(1, samples->GetCount(static_cast<int>(WebSocketHandshakeStreamBase::HandshakeResult::FAILED_EXTENSIONS)));
    } else {
        DCHECK_EQ(stream_type_, HTTP2_HANDSHAKE_STREAM);
        EXPECT_EQ(1, samples->GetCount(static_cast<int>(WebSocketHandshakeStreamBase::HandshakeResult::HTTP2_FAILED_EXTENSIONS)));
    }
}
...
TEST_P(WebSocketStreamCreateExtensionTest, NoMaxWindowBitsArgument)
{
    ...
    EXPECT_FALSE(stream_);
    EXPECT_TRUE(has_failed());
    EXPECT_EQ("Error during WebSocket handshake: Error in permessage-deflate: "
...
{
    CreateAndConnectStandard("ws://www.example.org/", NoSubProtocols(), {}, {}, { { "Sec-WebSocket-Accept", "s3pPLMBiTxaQ9kYGzzhZRbK+xOo=" } });
    WaitUntilConnectDone();
    EXPECT_FALSE(stream_);
    EXPECT_TRUE(has_failed());
    EXPECT_EQ("Error during WebSocket handshake: "
...
TEST_P(WebSocketMultiProtocolStreamCreateTest, InvalidStatusCode)
{
    ...
    AddSSLData();
    if (stream_type_ == BASIC_HANDSHAKE_STREAM) {
        static constexpr char kInvalidStatusCodeResponse[] = "HTTP/1.1 200 OK\r\n"
                                                             "Upgrade: websocket\r\n"
                                                             "Connection: Upgrade\r\n"
                                                             "Sec-WebSocket-Accept: s3pPLMBiTxaQ9kYGzzhZRbK+xOo=\r\n"
                                                             "\r\n";
        CreateAndConnectCustomResponse("wss://www.example.org/", NoSubProtocols(), {}, {}, kInvalidStatusCodeResponse);
    } else {
        DCHECK_EQ(stream_type_, HTTP2_HANDSHAKE_STREAM);
    ...
    if (stream_type_ == BASIC_HANDSHAKE_STREAM) {
        ...
        EXPECT_EQ(1, samples->GetCount(static_cast<int>(WebSocketHandshakeStreamBase::HandshakeResult::INVALID_STATUS)));
    } else {
        DCHECK_EQ(stream_type_, HTTP2_HANDSHAKE_STREAM);
        EXPECT_EQ("Error during WebSocket handshake: Unexpected response code: 101", failure_message());
        EXPECT_EQ(failure_response_code(), 101);
        EXPECT_EQ(1, samples->GetCount(static_cast<int>(WebSocketHandshakeStreamBase::HandshakeResult::HTTP2_INVALID_STATUS)));
    }
}
...
TEST_P(WebSocketMultiProtocolStreamCreateTest, RedirectsRejected)
{
    AddSSLData();
    if (stream_type_ == BASIC_HANDSHAKE_STREAM) {
        static constexpr char kRedirectResponse[] = "HTTP/1.1 302 Moved Temporarily\r\n"
        ...
        CreateAndConnectCustomResponse("wss://www.example.org/", NoSubProtocols(), {}, {}, kRedirectResponse);
    } else {
        DCHECK_EQ(stream_type_, HTTP2_HANDSHAKE_STREAM);
    ...
    EXPECT_TRUE(has_failed());
    EXPECT_EQ("Error during WebSocket handshake: Unexpected response code: 302", failure_message());
}
...
{
    static constexpr char kMalformedResponse[] = "220 mx.google.com ESMTP\r\n"
                                                 "HTTP/1.1 101 OK\r\n"
                                                 "Upgrade: websocket\r\n"
                                                 "Connection: Upgrade\r\n"
                                                 "Sec-WebSocket-Accept: s3pPLMBiTxaQ9kYGzzhZRbK+xOo=\r\n"
                                                 "\r\n";
    CreateAndConnectCustomResponse("ws://www.example.org/", NoSubProtocols(), {}, {}, kMalformedResponse);
    WaitUntilConnectDone();
    EXPECT_TRUE(has_failed());
    EXPECT_EQ("Error during WebSocket handshake: Invalid status line", failure_message());
}
...
TEST_P(WebSocketStreamCreateTest, MissingUpgradeHeader)
{
    ...
    static constexpr char kMissingUpgradeResponse[] = "HTTP/1.1 101 Switching Protocols\r\n"
                                                      "Connection: Upgrade\r\n"
                                                      "Sec-WebSocket-Accept: s3pPLMBiTxaQ9kYGzzhZRbK+xOo=\r\n"
                                                      "\r\n";
    CreateAndConnectCustomResponse("ws://www.example.org/", NoSubProtocols(), {}, {}, kMissingUpgradeResponse);
    WaitUntilConnectDone();
    EXPECT_TRUE(has_failed());
    EXPECT_EQ("Error during WebSocket handshake: 'Upgrade' header is missing", failure_message());

    stream_request_.reset();

    auto samples = histogram_tester.GetHistogramSamplesSinceCreation("Net.WebSocket.HandshakeResult2");
    EXPECT_EQ(1, samples->TotalCount());
    EXPECT_EQ(1, samples->GetCount(static_cast<int>(WebSocketHandshakeStreamBase::HandshakeResult::FAILED_UPGRADE)));
}

// There must only be one upgrade header.
TEST_P(WebSocketStreamCreateTest, DoubleUpgradeHeader)
{
    CreateAndConnectStandard("ws://www.example.org/", NoSubProtocols(), {}, {}, { { "Upgrade", "HTTP/2.0" } });
    WaitUntilConnectDone();
    EXPECT_TRUE(has_failed());
    EXPECT_EQ("Error during WebSocket handshake: "
...
TEST_P(WebSocketStreamCreateTest, MissingConnectionHeader)
{
    ...
    static constexpr char kMissingConnectionResponse[] = "HTTP/1.1 101 Switching Protocols\r\n"
                                                         "Upgrade: websocket\r\n"
                                                         "Sec-WebSocket-Accept: s3pPLMBiTxaQ9kYGzzhZRbK+xOo=\r\n"
                                                         "\r\n";
    CreateAndConnectCustomResponse("ws://www.example.org/", NoSubProtocols(), {}, {}, kMissingConnectionResponse);
    WaitUntilConnectDone();
    EXPECT_TRUE(has_failed());
    EXPECT_EQ("Error during WebSocket handshake: "
    ...
    auto samples = histogram_tester.GetHistogramSamplesSinceCreation("Net.WebSocket.HandshakeResult2");
    EXPECT_EQ(1, samples->TotalCount());
    EXPECT_EQ(1, samples->GetCount(static_cast<int>(WebSocketHandshakeStreamBase::HandshakeResult::FAILED_CONNECTION)));
}
...
{
    static constexpr char kAdditionalConnectionTokenResponse[] = "HTTP/1.1 101 Switching Protocols\r\n"
                                                                 "Upgrade: websocket\r\n"
                                                                 "Connection: Upgrade, Keep-Alive\r\n"
                                                                 "Sec-WebSocket-Accept: s3pPLMBiTxaQ9kYGzzhZRbK+xOo=\r\n"
                                                                 "\r\n";
    CreateAndConnectCustomResponse("ws://www.example.org/", NoSubProtocols(), {}, {}, kAdditionalConnectionTokenResponse);
    WaitUntilConnectDone();
    EXPECT_FALSE(has_failed());
    EXPECT_TRUE(stream_);
}
...
TEST_P(WebSocketStreamCreateTest, MissingSecWebSocketAccept)
{
    ...
    static constexpr char kMissingAcceptResponse[] = "HTTP/1.1 101 Switching Protocols\r\n"
                                                     "Upgrade: websocket\r\n"
                                                     "Connection: Upgrade\r\n"
                                                     "\r\n";
    CreateAndConnectCustomResponse("ws://www.example.org/", NoSubProtocols(), {}, {}, kMissingAcceptResponse);
    WaitUntilConnectDone();
    EXPECT_TRUE(has_failed());
    EXPECT_EQ("Error during WebSocket handshake: "
    ...
    auto samples = histogram_tester.GetHistogramSamplesSinceCreation("Net.WebSocket.HandshakeResult2");
    EXPECT_EQ(1, samples->TotalCount());
    EXPECT_EQ(1, samples->GetCount(static_cast<int>(WebSocketHandshakeStreamBase::HandshakeResult::FAILED_ACCEPT)));
}

// Sec-WebSocket-Accept header must match the key that was sent.
TEST_P(WebSocketStreamCreateTest, WrongSecWebSocketAccept)
{
    static constexpr char kIncorrectAcceptResponse[] = "HTTP/1.1 101 Switching Protocols\r\n"
                                                       "Upgrade: websocket\r\n"
                                                       "Connection: Upgrade\r\n"
                                                       "Sec-WebSocket-Accept: x/byyPZ2tOFvJCGkkugcKvqhhPk=\r\n"
                                                       "\r\n";
    CreateAndConnectCustomResponse("ws://www.example.org/", NoSubProtocols(), {}, {}, kIncorrectAcceptResponse);
    WaitUntilConnectDone();
    EXPECT_TRUE(has_failed());
    EXPECT_EQ("Error during WebSocket handshake: "
...
TEST_P(WebSocketStreamCreateTest, ConnectionFailure)
{
    ...
    socket_data->set_connect_data(MockConnect(SYNCHRONOUS, ERR_CONNECTION_REFUSED));
    CreateAndConnectRawExpectations("ws://www.example.org/", NoSubProtocols(), HttpRequestHeaders(), std::move(socket_data));
    WaitUntilConnectDone();
    EXPECT_TRUE(has_failed());
    EXPECT_EQ("Error in connection establishment: net::ERR_CONNECTION_REFUSED", failure_message());
    EXPECT_FALSE(request_info_);
    EXPECT_FALSE(response_info_);
}
...
TEST_P(WebSocketStreamCreateTest, ConnectionTimeout)
{
    ...
    WaitUntilConnectDone();
    EXPECT_TRUE(has_failed());
    EXPECT_EQ("Error in connection establishment: net::ERR_CONNECTION_TIMED_OUT", failure_message());
}
...
TEST_P(WebSocketStreamCreateTest, CancellationDuringWrite)
{
    ...
    auto socket_data = std::make_unique<SequencedSocketData>(base::span<MockRead>(), writes);
...
{
    std::string request = WebSocketStandardRequest("/", "www.example.org", Origin(), /*send_additional_request_headers=*/ {},
        /*extra_headers=*/ {});
    MockWrite writes[] = { MockWrite(ASYNC, 0, request.c_str()) };
    MockRead reads[] = {
        MockRead(SYNCHRONOUS, ERR_IO_PENDING, 1),
    };
    std::unique_ptr<SequencedSocketData> socket_data(BuildSocketData(reads, writes));
...
TEST_P(WebSocketStreamCreateTest, VeryLargeResponseHeaders)
{
    ...
    CreateAndConnectStringResponse("ws://www.example.org/", NoSubProtocols(), set_cookie_headers);
    WaitUntilConnectDone();
    EXPECT_TRUE(has_failed());
    EXPECT_FALSE(response_info_);
    ...
    auto samples = histogram_tester.GetHistogramSamplesSinceCreation("Net.WebSocket.HandshakeResult2");
    EXPECT_EQ(1, samples->TotalCount());
    EXPECT_EQ(1, samples->GetCount(static_cast<int>(WebSocketHandshakeStreamBase::HandshakeResult::FAILED)));
}
...
TEST_P(WebSocketStreamCreateTest, NoResponse)
{
    ...
    EXPECT_FALSE(stream_);
    EXPECT_FALSE(response_info_);
    EXPECT_EQ("Connection closed before receiving a handshake response", failure_message());

    stream_request_.reset();

    auto samples = histogram_tester.GetHistogramSamplesSinceCreation("Net.WebSocket.HandshakeResult2");
    EXPECT_EQ(1, samples->TotalCount());
    EXPECT_EQ(1, samples->GetCount(static_cast<int>(WebSocketHandshakeStreamBase::HandshakeResult::EMPTY_RESPONSE)));
}

TEST_P(WebSocketStreamCreateTest, SelfSignedCertificateFailure)
{
    auto ssl_socket_data = std::make_unique<SSLSocketDataProvider>(ASYNC, ERR_CERT_AUTHORITY_INVALID);
    ssl_socket_data->ssl_info.cert = ImportCertFromFile(GetTestCertsDirectory(), "unittest.selfsigned.der");
    ASSERT_TRUE(ssl_socket_data->ssl_info.cert.get());
    url_request_context_host_.AddSSLSocketDataProvider(std::move(ssl_socket_data));
    std::unique_ptr<SequencedSocketData> raw_socket_data(BuildNullSocketData());
    CreateAndConnectRawExpectations("wss://www.example.org/", NoSubProtocols(), HttpRequestHeaders(), std::move(raw_socket_data));
    // WaitUntilConnectDone doesn't work in this case.
    base::RunLoop().RunUntilIdle();
    EXPECT_FALSE(has_failed());
    ASSERT_TRUE(ssl_error_callbacks_);
    ssl_error_callbacks_->CancelSSLRequest(ERR_CERT_AUTHORITY_INVALID, &ssl_info_);
    WaitUntilConnectDone();
    EXPECT_TRUE(has_failed());
}
...
TEST_P(WebSocketStreamCreateTest, SelfSignedCertificateSuccess)
{
    ...
    ASSERT_TRUE(ssl_socket_data->ssl_info.cert.get());
    url_request_context_host_.AddSSLSocketDataProvider(std::move(ssl_socket_data));
    url_request_context_host_.AddSSLSocketDataProvider(std::make_unique<SSLSocketDataProvider>(ASYNC, OK));
...
{
    CreateAndConnectCustomResponse("ws://www.example.org/", NoSubProtocols(), {}, {}, kUnauthorizedResponse);
    WaitUntilConnectDone();
    EXPECT_TRUE(has_failed());
    EXPECT_EQ("HTTP Authentication failed; no valid credentials available", failure_message());
    EXPECT_FALSE(response_info_);
}

TEST_P(WebSocketStreamCreateBasicAuthTest, SuccessPasswordInUrl)
{
    CreateAndConnectAuthHandshake("ws://foo:bar@www.example.org/", "Zm9vOmJhcg==", WebSocketStandardResponse(std::string()));
    WaitUntilConnectDone();
    EXPECT_FALSE(has_failed());
    EXPECT_TRUE(stream_);
    ASSERT_TRUE(response_info_);
...
{
    CreateAndConnectAuthHandshake("ws://foo:baz@www.example.org/", "Zm9vOmJheg==", kUnauthorizedResponse);
    WaitUntilConnectDone();
    EXPECT_TRUE(has_failed());
    EXPECT_FALSE(response_info_);
}
...
{
    std::string request1 = WebSocketStandardRequest("/", "www.example.org", Origin(), /*send_additional_request_headers=*/ {},
        /*extra_headers=*/ {});
    std::string response1 = kUnauthorizedResponse;
    std::string request2 = WebSocketStandardRequest("/", "www.example.org", Origin(), { { "Authorization", "Basic Zm9vOmJhcg==" } }, /*extra_headers=*/ {});
    ...
    CreateAndConnectRawExpectations("ws://foo:bar@www.example.org/", NoSubProtocols(), HttpRequestHeaders(), BuildSocketData(reads, writes));
    WaitUntilConnectDone();
    EXPECT_FALSE(has_failed());
    EXPECT_TRUE(stream_);
    ASSERT_TRUE(response_info_);
...
{
    CreateAndConnectRawExpectations("ws://www.example.org/", NoSubProtocols(), HttpRequestHeaders(),
        helper_.BuildAuthSocketData(kUnauthorizedResponse, RequestExpectation("Zm9vOmJheg=="), WebSocketStandardResponse(std::string())));
...
{
    CreateAndConnectRawExpectations("ws://FooBar:pass@www.example.org/", NoSubProtocols(), HttpRequestHeaders(),
        helper_.BuildAuthSocketData(kUnauthorizedResponse, kAuthorizedRequest, WebSocketStandardResponse(std::string())));
    WaitUntilConnectDone();
    EXPECT_FALSE(has_failed());
    EXPECT_TRUE(stream_);
    ASSERT_TRUE(response_info_);
...
TEST_P(WebSocketMultiProtocolStreamCreateTest, Incomplete)
{
    ...
    AddSSLData();
    if (stream_type_ == BASIC_HANDSHAKE_STREAM) {
        std::string request = WebSocketStandardRequest("/", "www.example.org", Origin(),
        ...
        CreateAndConnectRawExpectations("wss://www.example.org/", NoSubProtocols(), HttpRequestHeaders(), BuildSocketData(reads, writes));
        base::RunLoop().RunUntilIdle();
        stream_request_.reset();

        auto samples = histogram_tester.GetHistogramSamplesSinceCreation("Net.WebSocket.HandshakeResult2");
        EXPECT_EQ(1, samples->TotalCount());
        EXPECT_EQ(1, samples->GetCount(static_cast<int>(WebSocketHandshakeStreamBase::HandshakeResult::INCOMPLETE)));
    } else {
        DCHECK_EQ(stream_type_, HTTP2_HANDSHAKE_STREAM);
        CreateAndConnectStandard("wss://www.example.org/", NoSubProtocols(), {}, {}, {});
        stream_request_.reset();
        ...
        auto samples = histogram_tester.GetHistogramSamplesSinceCreation("Net.WebSocket.HandshakeResult2");
        EXPECT_EQ(1, samples->TotalCount());
        EXPECT_EQ(1, samples->GetCount(static_cast<int>(WebSocketHandshakeStreamBase::HandshakeResult::HTTP2_INCOMPLETE)));
    }
}
...
TEST_P(WebSocketMultiProtocolStreamCreateTest, Http2StreamReset)
{
    ...
    if (stream_type_ == BASIC_HANDSHAKE_STREAM) {
        ...
        EXPECT_TRUE(has_failed());
        EXPECT_EQ("Stream closed with error: net::ERR_HTTP2_PROTOCOL_ERROR", failure_message());

        auto samples = histogram_tester.GetHistogramSamplesSinceCreation("Net.WebSocket.HandshakeResult2");
        EXPECT_EQ(1, samples->TotalCount());
        EXPECT_EQ(1, samples->GetCount(static_cast<int>(WebSocketHandshakeStreamBase::HandshakeResult::HTTP2_FAILED)));
    }
}
...
TEST_P(WebSocketStreamCreateTest, HandleErrConnectionClosed)
{
    ...
    static constexpr char kTruncatedResponse[] = "HTTP/1.1 101 Switching Protocols\r\n"
                                                 "Upgrade: websocket\r\n"
                                                 "Connection: Upgrade\r\n"
                                                 "Sec-WebSocket-Accept: s3pPLMBiTxaQ9kYGzzhZRbK+xOo=\r\n"
    ...
    auto samples = histogram_tester.GetHistogramSamplesSinceCreation("Net.WebSocket.HandshakeResult2");
    EXPECT_EQ(1, samples->TotalCount());
    EXPECT_EQ(1, samples->GetCount(static_cast<int>(WebSocketHandshakeStreamBase::HandshakeResult::FAILED_SWITCHING_PROTOCOLS)));
}

TEST_P(WebSocketStreamCreateTest, HandleErrTunnelConnectionFailed)
{
    static constexpr char kConnectRequest[] = "CONNECT www.example.org:80 HTTP/1.1\r\n"
                                              "Host: www.example.org:80\r\n"
                                              "Proxy-Connection: keep-alive\r\n"
                                              "\r\n";

    static constexpr char kProxyResponse[] = "HTTP/1.1 403 Forbidden\r\n"
                                             "Content-Type: text/html\r\n"
                                             "Content-Length: 9\r\n"
                                             "Connection: keep-alive\r\n"
                                             "\r\n"
    ...
    WaitUntilConnectDone();
    EXPECT_TRUE(has_failed());
    EXPECT_EQ("Establishing a tunnel via proxy server failed.", failure_message());
}
...
TEST_P(WebSocketStreamCreateTest, CancelSSLRequestAfterDelete)
{
    ...
    std::unique_ptr<SequencedSocketData> raw_socket_data(BuildSocketData(reads, writes));
    CreateAndConnectRawExpectations("wss://www.example.org/", NoSubProtocols(), HttpRequestHeaders(), std::move(raw_socket_data));
    base::RunLoop().RunUntilIdle();
    EXPECT_FALSE(has_failed());
    ASSERT_TRUE(ssl_error_callbacks_);
...
TEST_P(WebSocketStreamCreateTest, HandleConnectionCloseInFirstSegment)
{
    ...
    std::string response = WebSocketStandardResponse(std::string()) + "\x88" + static_cast<char>(close_body.size()) + close_body;
    MockRead reads[] = {
