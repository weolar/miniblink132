...
perf_test::PerfResultReporter SetUpWebSocketFrameMaskReporter(const std::string& story)
{
...
protected:
    void Benchmark(const char* const story, const char* const payload, size_t size)
    {
        ...
        for (int x = 0; x < kIterations; ++x) {
            MaskWebSocketFramePayload(masking_key, x % size, base::as_writable_byte_span(scratch));
        }
