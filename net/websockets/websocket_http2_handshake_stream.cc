...
WebSocketHttp2HandshakeStream::WebSocketHttp2HandshakeStream(base::WeakPtr<SpdySession> session, WebSocketStream::ConnectDelegate* connect_delegate,
    std::vector<std::string> requested_sub_protocols, std::vector<std::string> requested_extensions, WebSocketStreamRequestAPI* request,
...
void WebSocketHttp2HandshakeStream::RegisterRequest(const HttpRequestInfo* request_info)
{
    DCHECK(request_info);
...
int WebSocketHttp2HandshakeStream::SendRequest(const HttpRequestHeaders& headers, HttpResponseInfo* response, CompletionOnceCallback callback)
{
    ...
    auto request = std::make_unique<WebSocketHandshakeRequestInfo>(request_info_->url, base::Time::Now());
    request->headers = headers;

    AddVectorHeaders(requested_extensions_, requested_sub_protocols_, &request->headers);

    CreateSpdyHeadersFromHttpRequestForWebSocket(request_info_->url, request->headers, &http2_request_headers_);
    ...
    int rv = spdy_stream_request_->StartRequest(SPDY_BIDIRECTIONAL_STREAM, session_, request_info_->url, true, priority_, request_info_->socket_tag, net_log_,
        base::BindOnce(&WebSocketHttp2HandshakeStream::StartRequestCallback, base::Unretained(this)),
...
int WebSocketHttp2HandshakeStream::ReadResponseHeaders(CompletionOnceCallback callback)
{
    if (stream_closed_)
...
int WebSocketHttp2HandshakeStream::ReadResponseBody(IOBuffer* buf, int buf_len, CompletionOnceCallback callback)
{
...
bool WebSocketHttp2HandshakeStream::GetAlternativeService(AlternativeService* alternative_service) const
{
    return false;
}

bool WebSocketHttp2HandshakeStream::GetLoadTimingInfo(LoadTimingInfo* load_timing_info) const
{
...
void WebSocketHttp2HandshakeStream::PopulateNetErrorDetails(NetErrorDetails* /*details*/)
{
    return;
}
...
std::unique_ptr<HttpStream> WebSocketHttp2HandshakeStream::RenewStreamForAuth()
{
    // Renewing the stream is not supported.
    return nullptr;
}

const std::set<std::string>& WebSocketHttp2HandshakeStream::GetDnsAliases() const
{
    return dns_aliases_;
}
...
std::unique_ptr<WebSocketStream> WebSocketHttp2HandshakeStream::Upgrade()
{
    ...
    std::unique_ptr<WebSocketStream> basic_stream
        = std::make_unique<WebSocketBasicStream>(std::move(stream_adapter_), nullptr, sub_protocol_, extensions_, net_log_);
...
base::WeakPtr<WebSocketHandshakeStreamBase> WebSocketHttp2HandshakeStream::GetWeakPtr()
{
...
void WebSocketHttp2HandshakeStream::OnHeadersReceived(const quiche::HttpHeaderBlock& response_headers)
{
    ...
    const int rv = SpdyHeadersToHttpResponse(response_headers, http_response_info_);
    DCHECK_NE(rv, ERR_INCOMPLETE_HTTP2_HEADERS);

    http_response_info_->response_time = http_response_info_->original_response_time = stream_->response_time();
    ...
    http_response_info_->alpn_negotiated_protocol = HttpConnectionInfoToString(http_response_info_->connection_info);
...
void WebSocketHttp2HandshakeStream::OnClose(int status)
{
    ...
    OnFailure(base::StrCat({ "Stream closed with error: ", ErrorToString(status) }), status, std::nullopt);
...
void WebSocketHttp2HandshakeStream::StartRequestCallback(int rv)
{
    ...
    spdy_stream_request_.reset();
    stream_adapter_ = std::make_unique<WebSocketSpdyStreamAdapter>(stream_, this, net_log_);
    rv = stream_->SendRequestHeaders(std::move(http2_request_headers_), MORE_DATA_TO_SEND);
...
int WebSocketHttp2HandshakeStream::ValidateResponse()
{
    ...
    default:
        OnFailure(base::StringPrintf("Error during WebSocket handshake: Unexpected response code: %d", headers->response_code()), ERR_FAILED,
...
int WebSocketHttp2HandshakeStream::ValidateUpgradeResponse(const HttpResponseHeaders* headers)
{
    ...
    OnFailure("Error during WebSocket handshake: " + failure_message, rv, std::nullopt);
    return rv;
}

void WebSocketHttp2HandshakeStream::OnFailure(const std::string& message, int net_error, std::optional<int> response_code)
{
