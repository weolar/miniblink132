...
public:
    ...
    WebSocketHandshakeStreamBase& operator=(const WebSocketHandshakeStreamBase&) = delete;
    ...
    public:
        ...
        virtual std::unique_ptr<WebSocketHandshakeStreamBase> CreateHttp2Stream(base::WeakPtr<SpdySession> session, std::set<std::string> dns_aliases) = 0;
    ...
    static std::string MultipleHeaderValuesMessage(const std::string& header_name);
...
protected:
    ...
    // `headers`.
    static void AddVectorHeaders(const std::vector<std::string>& extensions, const std::vector<std::string>& protocols, HttpRequestHeaders* headers);
