...
constexpr char kMaskedHelloFrame[] = "\x81\x8D\xDE\xAD\xBE\xEF"
...
constexpr FrameHeaderTestCase kFrameHeaderTests[]
    = { { { "\x81\x00", 2 }, UINT64_C(0), kWebSocketNormalClosure }, { { "\x81\x7D", 2 }, UINT64_C(125), kWebSocketNormalClosure },
          { { "\x81\x7E\x00\x7E", 4 }, UINT64_C(126), kWebSocketNormalClosure }, { { "\x81\x7E\xFF\xFF", 4 }, UINT64_C(0xFFFF), kWebSocketNormalClosure },
          { { "\x81\x7F\x00\x00\x00\x00\x00\x01\x00\x00", 10 }, UINT64_C(0x10000), kWebSocketNormalClosure },
          { { "\x81\x7F\x00\x00\x00\x00\x7F\xFF\xFF\xFF", 10 }, UINT64_C(0x7FFFFFFF), kWebSocketNormalClosure },
          { { "\x81\x7F\x00\x00\x00\x00\x80\x00\x00\x00", 10 }, UINT64_C(0x80000000), kWebSocketErrorMessageTooBig },
          { { "\x81\x7F\x7F\xFF\xFF\xFF\xFF\xFF\xFF\xFF", 10 }, UINT64_C(0x7FFFFFFFFFFFFFFF), kWebSocketErrorMessageTooBig } };
...
TEST(WebSocketFrameParserTest, DecodeMaskedFrame)
{
    ...
    MaskWebSocketFramePayload(header->masking_key, 0, base::as_writable_byte_span(payload));
...
TEST(WebSocketFrameParserTest, DecodeManyFrames)
{
    ...
    static constexpr Input kInputs[] = { // Each |frame| data is split into two string literals because C++ lexers
    ...
    for (int i = 0; i < kNumInputs; ++i) {
        ...
        if (!frame)
            continue;
        EXPECT_TRUE(frame->final_chunk);
        ASSERT_EQ(kInputs[i].expected_payload_length, static_cast<uint64_t>(frame->payload.size()));
        EXPECT_TRUE(std::equal(kInputs[i].expected_payload, kInputs[i].expected_payload + kInputs[i].expected_payload_length, frame->payload.data()));
...
TEST(WebSocketFrameParserTest, DecodePartialFrame)
{
    ...
    for (size_t cutting_pos = 0; cutting_pos < kHelloLength; ++cutting_pos) {
        auto [input1, input2] = base::span(hello_frame_data).split_at(kFrameHeaderSize + cutting_pos);
        ...
        if (expected1.size() == 0) {
            ...
            EXPECT_TRUE(std::equal(expected1.begin(), expected1.end(), frame1->payload.data()));
        }
        ...
        if (expected2.size() == 0) {
            ...
            ASSERT_EQ(expected2.size(), static_cast<uint64_t>(frame2->payload.size()));
            EXPECT_TRUE(std::equal(expected2.begin(), expected2.end(), frame2->payload.data()));
        }
...
TEST(WebSocketFrameParserTest, DecodePartialMaskedFrame)
{
    ...
    std::vector<uint8_t> masked_hello_frame_data = ConvertToUint8Vector(kMaskedHelloFrame);

    for (size_t cutting_pos = 0; cutting_pos < kHelloLength; ++cutting_pos) {
        auto [input1, input2] = base::span(masked_hello_frame_data).split_at(kFrameHeaderSize + cutting_pos);
        ...
        if (expected1.size() == 0) {
            ...
            ASSERT_EQ(expected1.size(), static_cast<uint64_t>(frame1->payload.size()));
            std::vector<char> payload1(frame1->payload.data(), frame1->payload.data() + frame1->payload.size());
            MaskWebSocketFramePayload(header1->masking_key, 0, base::as_writable_byte_span(payload1));
        ...
        if (expected2.size() == 0) {
            ...
            ASSERT_EQ(expected2.size(), static_cast<uint64_t>(frame2->payload.size()));
            std::vector<char> payload2(frame2->payload.data(), frame2->payload.data() + frame2->payload.size());
            MaskWebSocketFramePayload(header1->masking_key, cutting_pos, base::as_writable_byte_span(payload2));
...
TEST(WebSocketFrameParserTest, DecodeFramesOfVariousLengths)
{
    for (const auto& test : kFrameHeaderTests) {
        ...
        std::vector<std::unique_ptr<WebSocketFrameChunk>> frames;
        EXPECT_EQ(test.error_code == kWebSocketNormalClosure, parser.Decode(input, &frames));
        ...
        if (expected_payload.size() == 0) {
            ...
            ASSERT_EQ(expected_payload.size(), static_cast<uint64_t>(frame->payload.size()));
            EXPECT_TRUE(std::equal(expected_payload.begin(), expected_payload.end(), frame->payload.data()));
        }
        const WebSocketFrameHeader* header = frame->header.get();
        EXPECT_TRUE(header != nullptr);
        if (!header)
...
TEST(WebSocketFrameParserTest, DecodePartialHeader)
{
    for (int i = 0; i < kNumFrameHeaderTests; ++i) {
        ...
        for (size_t j = 0; j < frame_header_length; ++j) {
            ...
            EXPECT_EQ(!failed, parser.Decode(base::span(frame_header).subspan(j, 1u), &frames));
            if (failed) {
