...
WebSocketExtension WebSocketDeflateParameters::AsExtension() const
{
    ...
    if (is_server_max_window_bits_specified()) {
        ...
        e.Add(WebSocketExtension::Parameter(kServerMaxWindowBits, base::NumberToString(server_max_window_bits())));
    }
    if (is_client_max_window_bits_specified()) {
        if (has_client_max_window_bits_value()) {
            e.Add(WebSocketExtension::Parameter(kClientMaxWindowBits, base::NumberToString(client_max_window_bits())));
        } else {
...
bool WebSocketDeflateParameters::IsValidAsResponse(std::string* failure_message) const
{
    if (server_max_window_bits_.is_specified) {
        DCHECK(server_max_window_bits_.has_value);
        DCHECK(IsValidWindowBits(server_max_window_bits_.bits));
...
bool WebSocketDeflateParameters::Initialize(const WebSocketExtension& extension, std::string* failure_message)
{
...
bool WebSocketDeflateParameters::IsCompatibleWith(const WebSocketDeflateParameters& response) const
{
