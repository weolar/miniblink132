...
class NET_EXPORT_PRIVATE WebSocketBasicHandshakeStream final : public WebSocketHandshakeStreamBase {
public:
    // |connect_delegate| and |failure_message| must out-live this object.
    WebSocketBasicHandshakeStream(std::unique_ptr<StreamSocketHandle> connection, WebSocketStream::ConnectDelegate* connect_delegate, bool using_proxy,
    ...
    WebSocketBasicHandshakeStream& operator=(const WebSocketBasicHandshakeStream&) = delete;
...
private:
    ...
    HttpStreamParser* parser() const
    {
        return state_.parser();
    }
