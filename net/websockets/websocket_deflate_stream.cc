...
WebSocketDeflateStream::WebSocketDeflateStream(
    std::unique_ptr<WebSocketStream> stream, const WebSocketDeflateParameters& params, std::unique_ptr<WebSocketDeflatePredictor> predictor)
...
int WebSocketDeflateStream::ReadFrames(std::vector<std::unique_ptr<WebSocketFrame>>* frames, CompletionOnceCallback callback)
{
    read_callback_ = std::move(callback);
    inflater_outputs_.clear();
    int result = stream_->ReadFrames(frames, base::BindOnce(&WebSocketDeflateStream::OnReadComplete, base::Unretained(this), base::Unretained(frames)));
    if (result < 0)
        return result;
    DCHECK_EQ(OK, result);
    DCHECK(!frames->empty());
...
int WebSocketDeflateStream::WriteFrames(std::vector<std::unique_ptr<WebSocketFrame>>* frames, CompletionOnceCallback callback)
{
...
void WebSocketDeflateStream::OnReadComplete(std::vector<std::unique_ptr<WebSocketFrame>>* frames, int result)
{
    if (result != OK) {
        frames->clear();
...
int WebSocketDeflateStream::Deflate(std::vector<std::unique_ptr<WebSocketFrame>>* frames)
{
    ...
    for (size_t i = 0; i < frames->size(); ++i) {
        ...
        if (writing_state_ == WRITING_UNCOMPRESSED_MESSAGE) {
            ...
            if (!frame->payload.empty() && !deflater_.AddBytes(base::as_chars(frame->payload).data(), frame->payload.size())) {
            ...
            if (writing_state_ == WRITING_COMPRESSED_MESSAGE) {
                ...
                if (final) {
                    int result = AppendPossiblyCompressedMessage(&frames_of_message, &frames_to_write);
                    if (result != OK)
                        return result;
...
void WebSocketDeflateStream::OnMessageStart(const std::vector<std::unique_ptr<WebSocketFrame>>& frames, size_t index)
{
...
int WebSocketDeflateStream::AppendCompressedFrame(const WebSocketFrameHeader& header, std::vector<std::unique_ptr<WebSocketFrame>>* frames_to_write)
{
...
int WebSocketDeflateStream::AppendPossiblyCompressedMessage(
    std::vector<std::unique_ptr<WebSocketFrame>>* frames, std::vector<std::unique_ptr<WebSocketFrame>>* frames_to_write)
{
    DCHECK(!frames->empty());
    ...
    for (size_t i = 0; i < frames->size(); ++i) {
        ...
        DCHECK_EQ(i == 0, WebSocketFrameHeader::kOpCodeContinuation != frame->header.opcode);
...
int WebSocketDeflateStream::Inflate(std::vector<std::unique_ptr<WebSocketFrame>>* frames)
{
    ...
    for (auto& frame_passed : frames_passed) {
        ...
        DVLOG(3) << "Input frame: opcode=" << frame->header.opcode << " final=" << frame->header.final << " reserved1=" << frame->header.reserved1
        ...
        if (reading_state_ == READING_UNCOMPRESSED_MESSAGE) {
            ...
            if (!frame->payload.empty() && !inflater_.AddBytes(base::as_chars(frame->payload).data(), frame->payload.size())) {
                DVLOG(1) << "WebSocket protocol error. "
            ...
            while (inflater_.CurrentOutputSize() >= kChunkSize || frame->header.final) {
                ...
                auto inflated = std::make_unique<WebSocketFrame>(WebSocketFrameHeader::kOpCodeText);
                ...
                DVLOG(3) << "Inflated frame: opcode=" << inflated->header.opcode << " final=" << inflated->header.final
                         << " reserved1=" << inflated->header.reserved1 << " payload_length=" << inflated->header.payload_length;
...
int WebSocketDeflateStream::InflateAndReadIfNecessary(std::vector<std::unique_ptr<WebSocketFrame>>* frames)
{
    int result = Inflate(frames);
    while (result == ERR_IO_PENDING) {
        DCHECK(frames->empty());

        result = stream_->ReadFrames(frames, base::BindOnce(&WebSocketDeflateStream::OnReadComplete, base::Unretained(this), base::Unretained(frames)));
        if (result < 0)
            break;
        DCHECK_EQ(OK, result);
        DCHECK(!frames->empty());
