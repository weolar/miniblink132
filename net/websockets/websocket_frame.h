...
struct NET_EXPORT WebSocketFrameHeader {
    ...
    {
        return opcode == kOpCodeDataUnused3 || opcode == kOpCodeDataUnused4 || opcode == kOpCodeDataUnused5 || opcode == kOpCodeDataUnused6
    ...
    {
        return opcode == kOpCodeControlUnusedB || opcode == kOpCodeControlUnusedC || opcode == kOpCodeControlUnusedD || opcode == kOpCodeControlUnusedE
...
// and maskedness.
NET_EXPORT size_t GetWebSocketFrameHeaderSize(const WebSocketFrameHeader& header);
...
NET_EXPORT int WriteWebSocketFrameHeader(const WebSocketFrameHeader& header, const WebSocketMaskingKey* masking_key, base::span<uint8_t> buffer);
...
NET_EXPORT void MaskWebSocketFramePayload(const WebSocketMaskingKey& masking_key, uint64_t frame_offset, base::span<uint8_t> data);
...
struct ParseCloseFrameResult {
    ...
    ParseCloseFrameResult(uint16_t close_code, std::string_view close_reason, std::optional<std::string_view> error = std::nullopt)
        : code(close_code)
        , reason(close_reason)
        , error(error)
    {
...
NET_EXPORT ParseCloseFrameResult ParseCloseFrame(base::span<const char> payload);
