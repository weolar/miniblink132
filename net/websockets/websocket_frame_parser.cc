...
constexpr size_t kMaximumFrameHeaderSize
    = net::WebSocketFrameHeader::kBaseHeaderSize + net::WebSocketFrameHeader::kMaximumExtendedLengthSize + net::WebSocketFrameHeader::kMaskingKeyLength;
...
bool WebSocketFrameParser::Decode(base::span<uint8_t> data_span, std::vector<std::unique_ptr<WebSocketFrameChunk>>* frame_chunks)
{
    ...
    if (incomplete_header_buffer_.size() > 0) {
        ...
        base::Extend(incomplete_header_buffer_, data_span.first(std::min(data_span.size(), kMaximumFrameHeaderSize - original_size)));
    ...
    while (data_span.size() > 0 || first_chunk) {
        ...
        DCHECK(incomplete_header_buffer_.empty());
        std::unique_ptr<WebSocketFrameChunk> frame_chunk = DecodeFramePayload(first_chunk, &data_span);
        first_chunk = false;
...
size_t WebSocketFrameParser::DecodeFrameHeader(base::span<const uint8_t> data)
{
    ...
    if (payload_length == kPayloadLengthWithTwoByteExtendedLengthField) {
        if (data.size() < current + 2)
            return 0;
        uint16_t payload_length_16 = base::U16FromBigEndian(data.subspan(current).first<2>());
        current += 2;
    ...
    static constexpr int kMaskingKeyLength = WebSocketFrameHeader::kMaskingKeyLength;
    if (masked) {
        if (data.size() < current + kMaskingKeyLength)
            return 0;
        base::as_writable_byte_span(masking_key.key).copy_from(data.subspan(current, kMaskingKeyLength));
...
std::unique_ptr<WebSocketFrameChunk> WebSocketFrameParser::DecodeFramePayload(bool first_chunk, base::span<uint8_t>* data)
{
    ...
    const int chunk_data_size = static_cast<int>(std::min(static_cast<uint64_t>(data->size()), current_frame_header_->payload_length - frame_offset_));
    ...
    if (chunk_data_size > 0) {
        frame_chunk->payload = base::as_writable_chars(data->subspan(0, chunk_data_size));
