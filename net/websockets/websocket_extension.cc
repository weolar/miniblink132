...
WebSocketExtension::Parameter::Parameter(const std::string& name, const std::string& value)
    : name_(name)
    , value_(value)
{
    DCHECK(!value.empty());
...
bool WebSocketExtension::Parameter::operator==(const Parameter& other) const = default;
...
bool WebSocketExtension::Equivalent(const WebSocketExtension& other) const
{
    ...
    auto extract_name = [](const Parameter& param) -> const std::string& { return param.name(); };
