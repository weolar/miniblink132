...
public:
    ...
    ConnectTestingEventInterface& operator=(const ConnectTestingEventInterface&) = delete;
    ...
    void OnURLRequestConnected(net::URLRequest* request, const net::TransportInfo& info) override
    {
    }

    void OnAddChannelResponse(
        std::unique_ptr<WebSocketHandshakeResponseInfo> response, const std::string& selected_subprotocol, const std::string& extensions) override;
    ...
    void WaitForDropChannel()
    {
        drop_channel_future_.Get();
    }
...
void ConnectTestingEventInterface::OnAddChannelResponse(
    std::unique_ptr<WebSocketHandshakeResponseInfo> response, const std::string& selected_subprotocol, const std::string& extensions)
{
...
void ConnectTestingEventInterface::OnDataFrame(bool fin, WebSocketMessageType type, base::span<const char> payload)
{
    DVLOG(3) << "Received WebSocket data frame with message:" << std::string(payload.begin(), payload.end());
...
void ConnectTestingEventInterface::OnDropChannel(bool was_clean, uint16_t code, const std::string& reason)
{
    DVLOG(3) << "OnDropChannel() invoked, was_clean: " << was_clean << ", code: " << code << ", reason: " << reason;
    if (was_clean) {
...
void ConnectTestingEventInterface::OnFailChannel(const std::string& message, int net_error, std::optional<int> response_code)
{
...
void ConnectTestingEventInterface::OnStartOpeningHandshake(std::unique_ptr<WebSocketHandshakeRequestInfo> request)
{
}

void ConnectTestingEventInterface::OnSSLCertificateError(
    std::unique_ptr<SSLErrorCallbacks> ssl_error_callbacks, const GURL& url, int net_error, const SSLInfo& ssl_info, bool fatal)
{
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
        FROM_HERE, base::BindOnce(&SSLErrorCallbacks::CancelSSLRequest, base::Owned(ssl_error_callbacks.release()), ERR_SSL_PROTOCOL_ERROR, &ssl_info));
}

int ConnectTestingEventInterface::OnAuthRequired(const AuthChallengeInfo& auth_info, scoped_refptr<HttpResponseHeaders> response_headers,
    const IPEndPoint& remote_endpoint, base::OnceCallback<void(const AuthCredentials*)> callback, std::optional<AuthCredentials>* credentials)
{
...
void ConnectTestingEventInterface::SetReceivedMessageFuture(std::string received_message)
{
...
public:
    ...
    TestProxyDelegateWithProxyInfo(const TestProxyDelegateWithProxyInfo&) = delete;
    TestProxyDelegateWithProxyInfo& operator=(const TestProxyDelegateWithProxyInfo&) = delete;
...
protected:
    ...
    void InitialiseContext()
    {
        ...
        context_->proxy_resolution_service()->SetProxyDelegate(proxy_delegate_.get());
    }
    ...
    bool ConnectAndWait(const GURL& socket_url)
    {
        ...
        IsolationInfo isolation_info = IsolationInfo::Create(IsolationInfo::RequestType::kOther, origin, origin, SiteForCookies::FromOrigin(origin));
        auto event_interface = std::make_unique<ConnectTestingEventInterface>();
        event_interface_ = event_interface.get();
        channel_ = std::make_unique<WebSocketChannel>(std::move(event_interface), context_.get());
        channel_->SendAddChannelRequest(GURL(socket_url), sub_protocols_, origin, site_for_cookies, StorageAccessApiStatus::kNone, isolation_info,
            HttpRequestHeaders(), TRAFFIC_ANNOTATION_FOR_TESTS);
        event_interface_->WaitForResponse();
        return !event_interface_->failed();
    }

    [[nodiscard]] WebSocketChannel::ChannelState SendMessage(const std::string& message)
    {
        scoped_refptr<IOBufferWithSize> buffer = base::MakeRefCounted<IOBufferWithSize>(message.size());

        buffer->span().copy_from(base::as_byte_span(message));
        return channel_->SendFrame(true, WebSocketFrameHeader::kOpCodeText, buffer, message.size());
    }
    ...
    std::string ReceiveMessage()
    {
        ...
        if (channel_state != WebSocketChannel::ChannelState::CHANNEL_ALIVE) {
            ADD_FAILURE() << "WebSocket channel is no longer alive after reading frames. State:" << channel_state;
            return {};
        }
    ...
    void CloseWebSocket()
    {
        ...
        DVLOG(3) << "Sending close handshake with code: " << close_code << " and reason: " << close_reason;

        auto channel_state = channel_->StartClosingHandshake(close_code, close_reason);

        EXPECT_EQ(channel_state, WebSocketChannel::ChannelState::CHANNEL_ALIVE) << "WebSocket channel is no longer alive after sending the "
                                                                                   "Close frame. State: "
                                                                                << channel_state;
    }
    ...
    raw_ptr<ConnectTestingEventInterface, DanglingUntriaged> event_interface_; // owned by channel_
...
{
    SpawnedTestServer ws_server(SpawnedTestServer::TYPE_WS, GetWebSocketTestDataDirectory());
...
{
    test_server::EmbeddedTestServer embedded_test_server(net::EmbeddedTestServer::TYPE_HTTP);

    embedded_test_server.RegisterUpgradeRequestHandler(test_server::WebSocketEchoHandler::CreateHandler());

    ASSERT_TRUE(embedded_test_server.Start());

    GURL echo_url = ReplaceUrlScheme(embedded_test_server.GetURL("/echo-with-no-extension"), "ws");
    EXPECT_TRUE(ConnectAndWait(echo_url));
}

TEST_F(WebSocketEndToEndTest, WebSocketEchoHandlerTest)
{
    net::test_server::EmbeddedTestServer embedded_test_server(net::test_server::EmbeddedTestServer::TYPE_HTTP);

    embedded_test_server.RegisterUpgradeRequestHandler(net::test_server::WebSocketEchoHandler::CreateHandler());
...
{
    SpawnedTestServer proxy_server(SpawnedTestServer::TYPE_BASIC_AUTH_PROXY, base::FilePath());
    SpawnedTestServer ws_server(SpawnedTestServer::TYPE_WS, GetWebSocketTestDataDirectory());
    ...
    proxy_config.proxy_rules().ParseFromString("https=" + proxy_server.host_port_pair().ToString());
    ...
    context_builder_->set_proxy_resolution_service(std::move(proxy_resolution_service));
...
{
    SpawnedTestServer proxy_server(SpawnedTestServer::TYPE_BASIC_AUTH_PROXY, base::FilePath());
    SpawnedTestServer wss_server(SpawnedTestServer::TYPE_WSS, GetWebSocketTestDataDirectory());
    ASSERT_TRUE(proxy_server.StartInBackground());
...
{
    SpawnedTestServer proxy_server(SpawnedTestServer::TYPE_PROXY, base::FilePath());
    SpawnedTestServer ws_server(SpawnedTestServer::TYPE_WS, GetWebSocketTestDataDirectory());
    ASSERT_TRUE(proxy_server.StartInBackground());
    ASSERT_TRUE(ws_server.StartInBackground());
    ASSERT_TRUE(proxy_server.BlockUntilStarted());
    ASSERT_TRUE(ws_server.BlockUntilStarted());
    ProxyConfig proxy_config;
    proxy_config.proxy_rules().ParseFromString("https=" + proxy_server.host_port_pair().ToString() + ";" + "http=" + proxy_server.host_port_pair().ToString());
    // TODO(crbug.com/40600992): Don't rely on proxying localhost.
    proxy_config.proxy_rules().bypass_rules.AddRulesToSubtractImplicit();
    ...
    EXPECT_EQ(info.proxy_info.ToDebugString(), base::StrCat({ "PROXY ", proxy_server.host_port_pair().ToString() }));
}
...
std::unique_ptr<HttpResponse> ProxyPacHandler(const HttpRequest& request)
{
    ...
    response->set_content(base::StringPrintf("function FindProxyForURL(url, host) {\n"
                                             "  return 'PROXY %s';\n"
                                             "}\n",
        proxy.c_str()));
    return response;
...
TEST_F(WebSocketEndToEndTest, MAYBE_ProxyPacUsed)
{
    ...
    ProxyConfig proxy_config
        = ProxyConfig::CreateFromCustomPacURL(proxy_pac_server.GetURL(base::StrCat({ "/proxy.pac?proxy=", proxy_server.host_port_pair().ToString() })));
    proxy_config.set_pac_mandatory(true);
    auto proxy_config_service = std::make_unique<ProxyConfigServiceFixed>(ProxyConfigWithAnnotation(proxy_config, TRAFFIC_ANNOTATION_FOR_TESTS));
    std::unique_ptr<ProxyResolutionService> proxy_resolution_service(
        ConfiguredProxyResolutionService::CreateUsingSystemProxyResolver(std::move(proxy_config_service), NetLog::Get(),
    ...
    // use of proxy.pac.
    HostPortPair fake_ws_host_port_pair("stealth-localhost", ws_server.host_port_pair().port());

    GURL ws_url(base::StrCat({ "ws://", fake_ws_host_port_pair.ToString(), "/", kEchoServer }));
    EXPECT_TRUE(ConnectAndWait(ws_url));
...
TEST_F(WebSocketEndToEndTest, HstsHttpsToWebSocket)
{
    ...
    SpawnedTestServer::SSLOptions ssl_options(SpawnedTestServer::SSLOptions::CERT_TEST_NAMES);
    SpawnedTestServer wss_server(SpawnedTestServer::TYPE_WSS, ssl_options, GetWebSocketTestDataDirectory());
    ...
    GURL https_page = https_server.GetURL(test_server_hostname, "/hsts-headers.html");
    std::unique_ptr<URLRequest> request(context_->CreateRequest(https_page, DEFAULT_PRIORITY, &delegate, TRAFFIC_ANNOTATION_FOR_TESTS));
    request->Start();
    ...
    GURL ws_url = ReplaceUrlScheme(wss_server.GetURL(test_server_hostname, kEchoServer), "ws");
    EXPECT_TRUE(ConnectAndWait(ws_url));
}
...
TEST_F(WebSocketEndToEndTest, HstsWebSocketToHttps)
{
    ...
    // Verify via http:
    TestDelegate delegate;
    GURL http_page = ReplaceUrlScheme(https_server.GetURL(test_server_hostname, "/simple.html"), "http");
    std::unique_ptr<URLRequest> request(context_->CreateRequest(http_page, DEFAULT_PRIORITY, &delegate, TRAFFIC_ANNOTATION_FOR_TESTS));
    request->Start();
    delegate.RunUntilComplete();
    EXPECT_EQ(OK, delegate.request_status());
...
TEST_F(WebSocketEndToEndTest, HeaderContinuations)
{
    ...
    EXPECT_TRUE(ConnectAndWait(ws_url));
    EXPECT_EQ("permessage-deflate; server_max_window_bits=10", event_interface_->extensions());
}
...
{
    SpawnedTestServer wss_server(
        SpawnedTestServer::TYPE_WSS, SpawnedTestServer::SSLOptions(base::FilePath(FILE_PATH_LITERAL("test_names.pem"))), GetWebSocketTestDataDirectory());
    ASSERT_TRUE(wss_server.Start());

    GURL wss_url("wss://a.test:" + base::NumberToString(wss_server.host_port_pair().port()) + "/" + kEchoServer);
    GURL::Replacements replacements;
...
TEST_F(WebSocketEndToEndTest, HostResolverEndpointResult)
{
    ...
    GURL wss_url("wss://a.test:" + base::NumberToString(port) + "/" + kEchoServer);
    ...
    host_resolver->rules()->AddRule(std::move(resolve_key), MockHostResolverBase::RuleResolver::RuleResult(std::vector { result }));
    context_builder_->set_host_resolver(std::move(host_resolver));
...
TEST_F(WebSocketEndToEndTest, EncryptedClientHello)
{
    ...
    ssl_server_config.ech_keys = MakeTestEchKeys(kPublicName, /*max_name_len=*/128, &ech_config_list);
    ...
    HostResolverEndpointResult result;
    result.ip_endpoints = { IPEndPoint(IPAddress::IPv4Localhost(), wss_url.IntPort()) };
    result.metadata.supported_protocol_alpns = { "http/1.1" };
    ...
    EXPECT_EQ("Error during WebSocket handshake: Unexpected response code: 404", event_interface_->failure_message());
}
} // namespace
