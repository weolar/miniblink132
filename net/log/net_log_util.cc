...
bool RequestCreatedBefore(const URLRequest* request1, const URLRequest* request2)
{
...
base::Value::Dict GetNetConstants()
{
    ...
    {
        static_assert(CertVerifier::VERIFY_FLAGS_LAST == (1 << 0), "Update with new flags");
        constants_dict.Set("certVerifierFlags", base::Value::Dict().Set("VERIFY_DISABLE_NETWORK_FETCHES", CertVerifier::VERIFY_DISABLE_NETWORK_FETCHES));
    }

    {
        static_assert(CertVerifyProc::VERIFY_FLAGS_LAST == (1 << 4), "Update with new flags");
        constants_dict.Set("certVerifyFlags",
            base::Value::Dict()
                .Set("VERIFY_REV_CHECKING_ENABLED", CertVerifyProc::VERIFY_REV_CHECKING_ENABLED)
                .Set("VERIFY_REV_CHECKING_REQUIRED_LOCAL_ANCHORS", CertVerifyProc::VERIFY_REV_CHECKING_REQUIRED_LOCAL_ANCHORS)
                .Set("VERIFY_ENABLE_SHA1_LOCAL_ANCHORS", CertVerifyProc::VERIFY_ENABLE_SHA1_LOCAL_ANCHORS)
                .Set("VERIFY_DISABLE_SYMANTEC_ENFORCEMENT", CertVerifyProc::VERIFY_DISABLE_SYMANTEC_ENFORCEMENT)
                .Set("VERIFY_DISABLE_NETWORK_FETCHES", CertVerifyProc::VERIFY_DISABLE_NETWORK_FETCHES));
    }

    {
        static_assert(
            bssl::SimplePathBuilderDelegate::DigestPolicy::kMaxValue == bssl::SimplePathBuilderDelegate::DigestPolicy::kWeakAllowSha1, "Update with new flags");

        constants_dict.Set("certPathBuilderDigestPolicy",
            base::Value::Dict()
                .Set("kStrong", static_cast<int>(bssl::SimplePathBuilderDelegate::DigestPolicy::kStrong))
                .Set("kWeakAllowSha1", static_cast<int>(bssl::SimplePathBuilderDelegate::DigestPolicy::kWeakAllowSha1)));
    }
    ...
    // symbolic names.
    {
        ...
        for (quic::QuicErrorCode error = quic::QUIC_NO_ERROR; error < quic::QUIC_LAST_ERROR; error = static_cast<quic::QuicErrorCode>(error + 1)) {
    ...
    // and their symbolic names.
    {
        ...
        for (quic::QuicRstStreamErrorCode error = quic::QUIC_STREAM_NO_ERROR; error < quic::QUIC_STREAM_LAST_ERROR;
    ...
    // symbolic names.
    {
        constants_dict.Set("logEventPhase",
            base::Value::Dict()
    ...
    // their symbolic names.
    {
        constants_dict.Set("addressFamily",
            base::Value::Dict()
    ...
    // logging).
    {
        ...
        constants_dict.Set("timeTickOffset", NetLogNumberValue(tick_to_unix_time_ms));
    }
...
NET_EXPORT base::Value::Dict GetNetInfo(URLRequestContext* context)
{
    ...
    base::Value::Dict net_info_dict = context->proxy_resolution_service()->GetProxyNetLogValues();
    ...
    // Log Host Resolver info.
    {
        ...
        if (cache) {
            ...
            cache->GetList(cache_contents_list, true /* include_staleness */, HostCache::SerializationType::kDebug);

            net_info_dict.Set(kNetInfoHostResolver,
                base::Value::Dict()
        ...
        for (const DohProviderEntry* provider : DohProviderEntry::GetList()) {
            if (!base::FeatureList::IsEnabled(provider->feature.get())) {
                disabled_doh_providers_list.Append(NetLogStringValue(provider->provider));
            }
        }
        net_info_dict.Set(kNetInfoDohProvidersDisabledDueToFeature, base::Value(std::move(disabled_doh_providers_list)));
    }
    ...
    {
        net_info_dict.Set(kNetInfoSocketPool, http_network_session->SocketPoolInfoToValue());
    }

    // Log SPDY Sessions.
    {
        net_info_dict.Set(kNetInfoSpdySessions, base::Value::FromUniquePtrValue(http_network_session->SpdySessionPoolInfoToValue()));
    }
    ...
    // Log SPDY status.
    {
        ...
        status_dict.Set("enable_http2", http_network_session->params().enable_http2);
        ...
        const SSLConfig::ApplicationSettings& application_settings = http_network_session->GetApplicationSettings();
        if (!application_settings.empty()) {
            base::Value::Dict application_settings_dict;
            for (const auto& setting : application_settings) {
                application_settings_dict.Set(NextProtoToString(setting.first), base::HexEncode(setting.second));
            }
            status_dict.Set("application_settings", std::move(application_settings_dict));
        }
    ...
    // Log ALT_SVC mappings.
    {
        ...
        net_info_dict.Set(kNetInfoAltSvcMappings, http_server_properties.GetAlternativeServiceInfoAsValue());
    }
    ...
    // Log Reporting API info.
    {
        ...
        if (reporting_service) {
            ...
            NetworkErrorLoggingService* network_error_logging_service = context->network_error_logging_service();
            if (network_error_logging_service) {
                reporting_value.GetDict().Set("networkErrorLogging", network_error_logging_service->StatusAsValue());
            }
            net_info_dict.Set(kNetInfoReporting, std::move(reporting_value));
        } else {
            net_info_dict.Set(kNetInfoReporting, base::Value::Dict().Set("reportingEnabled", false));
        }
...
NET_EXPORT void CreateNetLogEntriesForActiveObjects(const std::set<URLRequestContext*>& contexts, NetLog::ThreadSafeObserver* observer)
{
    ...
    for (auto* request : requests) {
        NetLogEntry entry(
            NetLogEventType::REQUEST_ALIVE, request->net_log().source(), NetLogEventPhase::BEGIN, request->creation_time(), request->GetStateAsValue());
