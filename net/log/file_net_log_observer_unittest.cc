...
void AddEntries(FileNetLogObserver* logger, int num_entries, size_t entry_size)
{
    // Get base size of event.
    const int kDummyId = 0;
    NetLogSource source(NetLogSourceType::HTTP2_SESSION, kDummyId);
    NetLogEntry base_entry(
        NetLogEventType::PAC_JAVASCRIPT_ERROR, source, NetLogEventPhase::BEGIN, base::TimeTicks::Now(), NetLogParamsWithString("message", ""));
    ...
    for (int i = 0; i < num_entries; i++) {
        ...
        // are the same size.
        std::string message = std::string(entry_size - base_entry_size - id.size() + 1, 'x');
        NetLogEntry entry(
            NetLogEventType::PAC_JAVASCRIPT_ERROR, source, NetLogEventPhase::BEGIN, base::TimeTicks::Now(), NetLogParamsWithString("message", message));
...
// and polled data).
struct ParsedNetLog {
    base::expected<void, std::string> InitFromFileContents(const std::string& input);
...
base::expected<void, std::string> ParsedNetLog::InitFromFileContents(const std::string& input)
{
    if (input.empty()) {
        return base::unexpected("input is empty");
    }

    ASSIGN_OR_RETURN(root, base::JSONReader::ReadAndReturnValueWithError(input), &base::JSONReader::Error::message);
...
// failure.
base::expected<std::unique_ptr<ParsedNetLog>, std::string> ReadNetLogFromDisk(const base::FilePath& log_path)
{
    std::string input;
    if (!base::ReadFileToString(log_path, &input)) {
        return base::unexpected("Failed reading file: " + base::UTF16ToUTF8(log_path.LossyDisplayName()));
    }
...
void VerifyEventsInLog(const ParsedNetLog* log, size_t num_events_emitted, size_t num_events_saved)
{
    ASSERT_TRUE(log);
...
// |value|.
void ExpectDictionaryContainsProperty(const base::Value::Dict& dict, const std::string& key, const std::string& value)
{
...
class FileNetLogObserverTest : public ::testing::TestWithParam<bool>, public WithTaskEnvironment {
public:
    void SetUp() override
    {
    ...
    void CreateAndStartObserving(std::unique_ptr<base::Value::Dict> constants, NetLogCaptureMode capture_mode = NetLogCaptureMode::kDefault)
    {
        if (IsBounded()) {
            logger_ = FileNetLogObserver::CreateBoundedForTests(log_path_, kLargeFileSize, kTotalNumFiles, capture_mode, std::move(constants));
        } else {
            logger_ = FileNetLogObserver::CreateUnbounded(log_path_, capture_mode, std::move(constants));
        }

        logger_->StartObserving(NetLog::Get());
    }

    void CreateAndStartObservingBoundedFile(int max_file_size, std::unique_ptr<base::Value::Dict> constants)
    {
        ...
        logger_ = FileNetLogObserver::CreateBoundedFile(std::move(file), max_file_size, NetLogCaptureMode::kDefault, std::move(constants));

        logger_->StartObserving(NetLog::Get());
    }

    void CreateAndStartObservingPreExisting(std::unique_ptr<base::Value::Dict> constants)
    {
        ...
        if (IsBounded()) {
            logger_ = FileNetLogObserver::CreateBoundedPreExisting(
                scratch_dir_.GetPath(), std::move(file), kLargeFileSize, NetLogCaptureMode::kDefault, std::move(constants));
        } else {
            logger_ = FileNetLogObserver::CreateUnboundedPreExisting(std::move(file), NetLogCaptureMode::kDefault, std::move(constants));
        }
...
class FileNetLogObserverBoundedTest : public ::testing::Test, public WithTaskEnvironment {
public:
    void SetUp() override
    {
        ASSERT_TRUE(temp_dir_.CreateUniqueTempDir());
    ...
    void CreateAndStartObserving(std::unique_ptr<base::Value::Dict> constants, uint64_t total_file_size, int num_files)
    {
        logger_ = FileNetLogObserver::CreateBoundedForTests(log_path_, total_file_size, num_files, NetLogCaptureMode::kDefault, std::move(constants));
        logger_->StartObserving(NetLog::Get());
    }
    ...
    {
        return GetInprogressDirectory().AppendASCII("event_file_" + base::NumberToString(index) + ".json");
    }
...
INSTANTIATE_TEST_SUITE_P(All, FileNetLogObserverTest, ::testing::Values(true, false));
...
TEST_P(FileNetLogObserverTest, ObserverDestroyedWithoutStopObservingPreExisting)
{
...
TEST_P(FileNetLogObserverTest, GeneratesValidJSONWithNoEvents)
{
    ...
    ASSERT_OK_AND_ASSIGN(std::unique_ptr<ParsedNetLog> log, ReadNetLogFromDisk(log_path_));
...
TEST_P(FileNetLogObserverTest, GeneratesValidJSONWithNoEventsCreateBoundedFile)
{
    TestClosure closure;
...
TEST_P(FileNetLogObserverTest, GeneratesValidJSONWithOneEventCreateBoundedFile)
{
    TestClosure closure;
...
TEST_P(FileNetLogObserverTest, PreExistingFileBroken)
{
    ...
    if (IsBounded())
        logger_ = FileNetLogObserver::CreateBoundedPreExisting(scratch_dir_.GetPath(), std::move(file), kLargeFileSize, NetLogCaptureMode::kDefault, nullptr);
    else
        logger_ = FileNetLogObserver::CreateUnboundedPreExisting(std::move(file), NetLogCaptureMode::kDefault, nullptr);
    logger_->StartObserving(NetLog::Get());
...
TEST_P(FileNetLogObserverTest, CustomConstants)
{
    ...
    CreateAndStartObserving(std::make_unique<base::Value::Dict>(std::move(constants)));
    ...
    ExpectDictionaryContainsProperty(*log->constants, kConstantKey, kConstantString);
}
...
TEST_P(FileNetLogObserverTest, GeneratesValidJSONWithPolledData)
{
    ...
    dummy_polled_data.SetByDottedPath(kDummyPolledDataPath, kDummyPolledDataString);

    logger_->StopObserving(std::make_unique<base::Value>(std::move(dummy_polled_data)), closure.closure());
    ...
    ExpectDictionaryContainsProperty(*log->polled_data, kDummyPolledDataPath, kDummyPolledDataString);
}
...
TEST_P(FileNetLogObserverTest, LogModeRecorded)
{
    struct TestCase {
        ...
        { NetLogCaptureMode::kEverything, "Everything" }, { NetLogCaptureMode::kIncludeSensitive, "IncludeSensitive" },
    ...
    for (const auto& test_case : test_cases) {
        ...
        closure.WaitForResult();
        ASSERT_OK_AND_ASSIGN(std::unique_ptr<ParsedNetLog> log, ReadNetLogFromDisk(log_path_));
        ExpectDictionaryContainsProperty(*log->constants, "logCaptureMode", test_case.expected_value);
    }
}
...
TEST_P(FileNetLogObserverTest, AddEventsFromMultipleThreads)
{
    ...
    for (size_t i = 0; i < threads.size(); ++i) {
        threads[i] = std::make_unique<base::Thread>("WorkerThread" + base::NumberToString(i));
        threads[i]->Start();
    ...
    for (size_t i = 0; i < kNumThreads; ++i) {
        threads[i]->task_runner()->PostTask(FROM_HERE, base::BindOnce(&AddEntries, base::Unretained(logger_.get()), kNumEventsAddedPerThread, kDummyEventSize));
    }
...
TEST_F(FileNetLogObserverBoundedTest, DropOldEventsFromWriteQueue)
{
    ...
    // Verify the written log.
    ASSERT_OK_AND_ASSIGN(std::unique_ptr<ParsedNetLog> log, ReadNetLogFromDisk(log_path_));
    VerifyEventsInLog(log.get(), kNumEvents, static_cast<size_t>(kTotalNumFiles * ((kFileSize - 1) / kEventSize + 1)));
}
...
TEST_F(FileNetLogObserverBoundedTest, OverwriteAllFiles)
{
    ...
    int num_events_in_files = (kTotalNumFiles - 1) * events_per_file + events_in_last_file;

    // Verify the written log.
    ASSERT_OK_AND_ASSIGN(std::unique_ptr<ParsedNetLog> log, ReadNetLogFromDisk(log_path_));
    VerifyEventsInLog(log.get(), kNumEvents, static_cast<size_t>(num_events_in_files));
}
...
TEST_F(FileNetLogObserverBoundedTest, PreExistingUsesSpecifiedDir)
{
    ...
    logger_ = FileNetLogObserver::CreateBoundedPreExisting(scratch_dir.GetPath(), std::move(file), kLargeFileSize, NetLogCaptureMode::kDefault, nullptr);
    logger_->StartObserving(NetLog::Get());

    base::ThreadPoolInstance::Get()->FlushForTesting();
    EXPECT_TRUE(base::PathExists(log_path_));
    EXPECT_TRUE(base::PathExists(scratch_dir.GetPath().AppendASCII("constants.json")));
    EXPECT_FALSE(base::PathExists(GetInprogressDirectory()));
...
TEST_P(FileNetLogObserverTest, AddEventsFromMultipleThreadsWithStopObserving)
{
    ...
    // Add events in parallel from all the threads.
    for (size_t i = 0; i < kNumThreads; ++i) {
        threads[i]->task_runner()->PostTask(FROM_HERE, base::BindOnce(&AddEntriesViaNetLog, base::Unretained(NetLog::Get()), kNumEventsAddedPerThread));
    }
...
TEST_P(FileNetLogObserverTest, AddEventsFromMultipleThreadsWithoutStopObserving)
{
    const size_t kNumThreads = 10;
    std::vector<std::unique_ptr<base::Thread>> threads(kNumThreads);
    // Start all the threads. Waiting for them to start is to hopefully improve
