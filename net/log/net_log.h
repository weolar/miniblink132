...
public:
    ...
    public:
        ...
        ThreadSafeCaptureModeObserver(const ThreadSafeCaptureModeObserver&) = delete;
        ThreadSafeCaptureModeObserver& operator=(const ThreadSafeCaptureModeObserver&) = delete;
    ...
    protected:
        ...
        void AddEntryAtTimeWithMaterializedParams(
    ...
    void AddEntry(NetLogEventType type, const NetLogSource& source, NetLogEventPhase phase);
    ...
    template <typename T, typename = void> struct ExpectsCaptureMode : std::false_type { };
    template <typename T> struct ExpectsCaptureMode<T, decltype(void(std::declval<T>()(NetLogCaptureMode::kDefault)))> : std::true_type { };
    ...
    inline typename std::enable_if<!ExpectsCaptureMode<ParametersCallback>::value, void>::type AddEntry(
        NetLogEventType type, const NetLogSource& source, NetLogEventPhase phase, const ParametersCallback& get_params)
    {
    ...
    template <typename ParametersCallback> void AddGlobalEntry(NetLogEventType type, const ParametersCallback& get_params)
    {
        AddEntry(type, NetLogSource(NetLogSourceType::NONE, NextID()), NetLogEventPhase::NONE, get_params);
    }

    void AddGlobalEntryWithStringParams(NetLogEventType type, std::string_view name, std::string_view value);
    ...
    void AddObserver(ThreadSafeObserver* observer, NetLogCaptureMode capture_mode);
...
private:
    ...
    // virtual dispatch.
    void AddEntryInternal(NetLogEventType type, const NetLogSource& source, NetLogEventPhase phase, const GetParamsInterface* get_params);
    ...
    // clang).
    void AddEntryWithMaterializedParams(NetLogEventType type, const NetLogSource& source, NetLogEventPhase phase, base::Value::Dict params);
    ...
    std::vector<raw_ptr<ThreadSafeCaptureModeObserver, VectorExperimental>> capture_mode_observers_;
};
