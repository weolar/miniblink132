...
TraceEntryInfo GetTraceEntryInfoFromValue(const base::Value::Dict& value)
{
    ...
    if (const std::string* type = value.FindStringByDottedPath("args.source_type")) {
...
{
    TraceLog::GetInstance()->SetEnabled(base::trace_event::TraceConfig(category, ""), TraceLog::RECORDING_MODE);
...
{
    std::string disabled_netlog_category = std::string("-") + kNetLogTracingCategory;
...
public:
    ...
    void OnTraceDataCollected(base::RunLoop* run_loop, const scoped_refptr<base::RefCountedString>& events_str, bool has_more_events)
    {
        ...
        trace_value = base::JSONReader::Read(json_output_.json_output, base::JSON_PARSE_RFC);
    ...
    void EndTraceAndFlush()
    {
        ...
        TraceLog::GetInstance()->Flush(
            base::BindRepeating(&TraceNetLogObserverTest::OnTraceDataCollected, base::Unretained(this), base::Unretained(&run_loop)));
        run_loop.Run();
    }

    void set_trace_net_log_observer(std::unique_ptr<TraceNetLogObserver> trace_net_log_observer)
    {
        trace_net_log_observer_ = std::move(trace_net_log_observer);
    }

    static base::Value::List FilterNetLogTraceEvents(const base::Value::List& trace_events)
    {
        ...
        for (const auto& event : trace_events) {
            ...
            const std::string* category = event.GetDict().FindStringByDottedPath("cat");
            if (!category) {
                ADD_FAILURE() << "Unexpected item without a category field in trace_events";
                continue;
            }
    ...
    const base::Value::List& trace_events() const
    {
...
TEST_F(TraceNetLogObserverTest, TraceEventCaptured)
{
    ...
    NetLogWithSource net_log_with_source = NetLogWithSource::Make(NetLog::Get(), net::NetLogSourceType::NONE);
    ...
    EXPECT_EQ(std::string(1, TRACE_EVENT_PHASE_NESTABLE_ASYNC_INSTANT), actual_item1.phase);
    EXPECT_EQ(NetLogEventTypeToString(NetLogEventType::CANCELLED), actual_item1.name);
    EXPECT_EQ(NetLog::SourceTypeToString(entries[0].source.type), actual_item1.source_type);
    ...
    EXPECT_EQ(std::string(1, TRACE_EVENT_PHASE_NESTABLE_ASYNC_BEGIN), actual_item2.phase);
    EXPECT_EQ(NetLogEventTypeToString(NetLogEventType::URL_REQUEST_START_JOB), actual_item2.name);
    EXPECT_EQ(NetLog::SourceTypeToString(entries[1].source.type), actual_item2.source_type);
    ...
    EXPECT_EQ(std::string(1, TRACE_EVENT_PHASE_NESTABLE_ASYNC_END), actual_item3.phase);
    EXPECT_EQ(NetLogEventTypeToString(NetLogEventType::URL_REQUEST_START_JOB), actual_item3.name);
}
...
TEST_F(TraceNetLogObserverTest, EnableAndDisableTracing)
{
    ...
    EXPECT_EQ(std::string(1, TRACE_EVENT_PHASE_NESTABLE_ASYNC_INSTANT), actual_item2.phase);
    EXPECT_EQ(NetLogEventTypeToString(NetLogEventType::URL_REQUEST_START_JOB), actual_item2.name);
    EXPECT_EQ(NetLog::SourceTypeToString(entries[2].source.type), actual_item2.source_type);
}
...
TEST_F(TraceNetLogObserverTest, CreateObserverAfterTracingStartsDisabledCategory)
{
    set_trace_net_log_observer(nullptr);
...
TEST_F(TraceNetLogObserverTest, EventsWithAndWithoutParameters)
{
    ...
    NetLog::Get()->AddGlobalEntryWithStringParams(NetLogEventType::CANCELLED, "foo", "bar");
    NetLog::Get()->AddGlobalEntry(NetLogEventType::REQUEST_ALIVE);
    ...
    EXPECT_EQ(base::StringPrintf("0x%x", entries[1].source.id), actual_item2.id);
    EXPECT_EQ(std::string(1, TRACE_EVENT_PHASE_NESTABLE_ASYNC_INSTANT), actual_item2.phase);
    EXPECT_EQ(NetLogEventTypeToString(NetLogEventType::REQUEST_ALIVE), actual_item2.name);
    EXPECT_EQ(NetLog::SourceTypeToString(entries[1].source.type), actual_item2.source_type);

    const std::string* item1_params = item1->GetDict().FindStringByDottedPath("args.params.foo");
    ASSERT_TRUE(item1_params);
    EXPECT_EQ("bar", *item1_params);

    // Perfetto tracing backend skips empty args.
    const base::Value::Dict* item2_args = item2->GetDict().FindDictByDottedPath("args");
