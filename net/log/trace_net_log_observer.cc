...
public:
    explicit TracedValue(base::Value::Dict value)
...
void TraceNetLogObserver::OnAddEntry(const NetLogEntry& entry)
{
    ...
    case NetLogEventPhase::BEGIN:
        TRACE_EVENT_NESTABLE_ASYNC_BEGIN2(kNetLogTracingCategory, NetLogEventTypeToString(entry.type), entry.source.id, "source_type",
            NetLog::SourceTypeToString(entry.source.type), "params", std::make_unique<TracedValue>(std::move(params)));
        break;
    case NetLogEventPhase::END:
        TRACE_EVENT_NESTABLE_ASYNC_END1(
            kNetLogTracingCategory, NetLogEventTypeToString(entry.type), entry.source.id, "params", std::make_unique<TracedValue>(std::move(params)));
        break;
    case NetLogEventPhase::NONE:
        TRACE_EVENT_NESTABLE_ASYNC_INSTANT2(kNetLogTracingCategory, NetLogEventTypeToString(entry.type), entry.source.id, "source_type",
            NetLog::SourceTypeToString(entry.source.type), "params", std::make_unique<TracedValue>(std::move(params)));
        break;
    }
}
