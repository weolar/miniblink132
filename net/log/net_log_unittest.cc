...
TEST(NetLogTest, BasicEventsWithSource)
{
    ...
    NetLogWithSource source0 = NetLogWithSource::Make(NetLogSourceType::URL_REQUEST);
...
TEST(NetLogTest, CaptureModes)
{
    ...
    for (NetLogCaptureMode mode : kModes) {
        ...
        NetLog::Get()->AddGlobalEntry(NetLogEventType::SOCKET_ALIVE, [&](NetLogCaptureMode capture_mode) { return NetCaptureModeParams(capture_mode); });
        ...
        ASSERT_EQ(CaptureModeToInt(mode), GetIntegerValueFromParams(entries[0], "capture_mode"));
...
public:
    ...
    void OnAddEntry(const NetLogEntry& entry) override
    {
        ...
        std::unique_ptr<base::Value::Dict> dict = std::make_unique<base::Value::Dict>(entry.ToDict());
        ASSERT_TRUE(dict);
...
{
    net_log->AddGlobalEntry(NetLogEventType::CANCELLED, [&](NetLogCaptureMode capture_mode) { return NetCaptureModeParams(capture_mode); });
}
...
public:
    ...
    AddRemoveObserverTestThread& operator=(const AddRemoveObserverTestThread&) = delete;

    ~AddRemoveObserverTestThread() override
    {
...
// to completion.
template <class ThreadType> void RunTestThreads(NetLog* net_log)
{
    // Must outlive `threads`.
    base::WaitableEvent start_event(base::WaitableEvent::ResetPolicy::MANUAL, base::WaitableEvent::InitialState::NOT_SIGNALED);
...
TEST(NetLogTest, NetLogTwoObservers)
{
    ...
    // Add first observer.
    NetLog::Get()->AddObserver(&observer[0], NetLogCaptureMode::kIncludeSensitive);
...
TEST(NetLogTest, NetLogEntryToValueEmptyParams)
{
    ...
    NetLogEntry entry1(NetLogEventType::REQUEST_ALIVE, NetLogSource(), NetLogEventPhase::BEGIN, base::TimeTicks(), base::Value::Dict());
