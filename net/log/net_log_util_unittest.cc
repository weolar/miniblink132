...
TEST(NetLogUtil, GetNetInfo)
{
    ...
    auto [rv, _] = context->http_transaction_factory()->GetCache()->GetBackend(TestGetBackendCompletionCallback().callback());
    EXPECT_EQ(OK, rv);
...
TEST(NetLogUtil, GetNetInfoIncludesFieldTrials)
{
    ...
    scoped_feature_list.InitWithFeatureList(std::make_unique<base::FeatureList>());
    ...
    base::Value::List* trials = net_info.GetDict().FindList("activeFieldTrialGroups");
...
TEST(NetLogUtil, GetNetInfoIncludesDisabledDohProviders)
{
    ...
    for (bool provider_enabled : { false, true }) {
        ...
        auto provider_it = base::ranges::find(provider_list, kArbitraryProvider, &net::DohProviderEntry::provider);
        ...
        base::test::ScopedFeatureList scoped_feature_list;
        scoped_feature_list.InitWithFeatureState(provider_entry.feature.get(), provider_enabled);
        EXPECT_EQ(provider_enabled, base::FeatureList::IsEnabled(provider_entry.feature.get()));
        ...
        base::Value net_info(GetNetInfo(context.get()));
        ASSERT_TRUE(net_info.is_dict());
        const base::Value::List* disabled_doh_providers_list = net_info.GetDict().FindList(kNetInfoDohProvidersDisabledDueToFeature);
        CHECK(disabled_doh_providers_list);
        EXPECT_EQ(!provider_enabled, base::Contains(*disabled_doh_providers_list, base::Value(kArbitraryProvider)));
    }
}
...
TEST(NetLogUtil, CreateNetLogEntriesForActiveObjectsOneContext)
{
    ...
    for (size_t num_requests = 0; num_requests < 5; ++num_requests) {
        ...
        for (size_t i = 0; i < num_requests; ++i) {
            requests.push_back(context->CreateRequest(GURL("about:life"), DEFAULT_PRIORITY, &delegate, TRAFFIC_ANNOTATION_FOR_TESTS));
        }
...
TEST(NetLogUtil, CreateNetLogEntriesForActiveObjectsMultipleContexts)
{
    ...
    for (size_t num_requests = 0; num_requests < 5; ++num_requests) {
        ...
        for (size_t i = 0; i < num_requests; ++i) {
            ...
            requests.push_back(contexts[i]->CreateRequest(GURL("about:hats"), DEFAULT_PRIORITY, &delegate, TRAFFIC_ANNOTATION_FOR_TESTS));
        }
        RecordingNetLogObserver net_log_observer;
