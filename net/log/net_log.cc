...
NetLog::ThreadSafeCaptureModeObserver::ThreadSafeCaptureModeObserver() = default;
NetLog::ThreadSafeCaptureModeObserver::~ThreadSafeCaptureModeObserver() = default;

NetLogCaptureModeSet NetLog::ThreadSafeCaptureModeObserver::GetObserverCaptureModes() const
{
    DCHECK(net_log_);
    return net_log_->GetObserverCaptureModes();
}

void NetLog::ThreadSafeCaptureModeObserver::AddEntryAtTimeWithMaterializedParams(
    NetLogEventType type, const NetLogSource& source, NetLogEventPhase phase, base::TimeTicks time, base::Value::Dict params)
{
    DCHECK(net_log_);
    net_log_->AddEntryAtTimeWithMaterializedParams(type, source, phase, time, std::move(params));
}
...
void NetLog::AddEntry(NetLogEventType type, const NetLogSource& source, NetLogEventPhase phase)
{
...
{
    AddEntry(type, NetLogSource(NetLogSourceType::NONE, NextID()), NetLogEventPhase::NONE);
}

void NetLog::AddGlobalEntryWithStringParams(NetLogEventType type, std::string_view name, std::string_view value)
{
...
void NetLog::AddObserver(NetLog::ThreadSafeObserver* observer, NetLogCaptureMode capture_mode)
{
...
void NetLog::AddCaptureModeObserver(NetLog::ThreadSafeCaptureModeObserver* observer)
{
    base::AutoLock lock(lock_);
...
void NetLog::RemoveCaptureModeObserver(NetLog::ThreadSafeCaptureModeObserver* observer)
{
    base::AutoLock lock(lock_);
...
void NetLog::UpdateObserverCaptureModes()
{
    ...
    for (net::NetLog::ThreadSafeCaptureModeObserver* capture_mode_observer : capture_mode_observers_) {
...
std::string NetLog::TimeToString(const base::Time& time)
{
    ...
    base::TimeTicks time_ticks = base::TimeTicks::UnixEpoch() + (time - base::Time::UnixEpoch());
...
{
    int32_t old_value = base::subtle::NoBarrier_AtomicExchange(&last_id_, std::numeric_limits<base::subtle::Atomic32>::min());
    DCHECK_EQ(old_value, 0) << " NetLog::InitializeSourceIdPartition() called "
                               "after NextID() or called multiple times";
}

void NetLog::AddEntryInternal(NetLogEventType type, const NetLogSource& source, NetLogEventPhase phase, const GetParamsInterface* get_params)
{
    ...
    for (int i = 0; i <= static_cast<int>(NetLogCaptureMode::kLast); ++i) {
        ...
        NetLogEntry entry(type, source, phase, base::TimeTicks::Now(), get_params->GetParams(capture_mode));
...
void NetLog::AddEntryWithMaterializedParams(NetLogEventType type, const NetLogSource& source, NetLogEventPhase phase, base::Value::Dict params)
{
    AddEntryAtTimeWithMaterializedParams(type, source, phase, base::TimeTicks::Now(), std::move(params));
}

void NetLog::AddEntryAtTimeWithMaterializedParams(
    NetLogEventType type, const NetLogSource& source, NetLogEventPhase phase, base::TimeTicks time, base::Value::Dict params)
{
