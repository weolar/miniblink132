...
RecordingNetLogObserver::RecordingNetLogObserver(NetLog* net_log, NetLogCaptureMode capture_mode)
    : net_log_(net_log)
{
...
std::vector<NetLogEntry> RecordingNetLogObserver::GetEntriesForSource(NetLogSource source) const
{
    base::AutoLock lock(lock_);
    std::vector<NetLogEntry> result;
    for (const auto& entry : entry_list_) {
...
std::vector<NetLogEntry> RecordingNetLogObserver::GetEntriesWithType(NetLogEventType type) const
{
    base::AutoLock lock(lock_);
    std::vector<NetLogEntry> result;
    for (const auto& entry : entry_list_) {
...
std::vector<NetLogEntry> RecordingNetLogObserver::GetEntriesForSourceWithType(NetLogSource source, NetLogEventType type, NetLogEventPhase phase) const
{
    base::AutoLock lock(lock_);
    std::vector<NetLogEntry> result;
    for (const auto& entry : entry_list_) {
        if (entry.source.id == source.id && entry.type == type && entry.phase == phase) {
            result.push_back(entry.Clone());
        }
    }
    return result;
...
void RecordingNetLogObserver::OnAddEntry(const NetLogEntry& entry)
{
    ...
    base::RepeatingClosure add_entry_callback;
    {
        ...
        base::AutoLock lock(lock_);
        entry_list_.emplace_back(entry.type, entry.source, entry.phase, entry.time, std::move(params));
...
void RecordingNetLogObserver::SetObserverCaptureMode(NetLogCaptureMode capture_mode)
{
    net_log_->RemoveObserver(this);
    net_log_->AddObserver(this, capture_mode);
}

void RecordingNetLogObserver::SetThreadsafeAddEntryCallback(base::RepeatingClosure add_entry_callback)
{
    base::AutoLock lock(lock_);
