...
::testing::AssertionResult LogContainsEvent(
    const std::vector<NetLogEntry>& entries, int offset, NetLogEventType expected_event, NetLogEventPhase expected_phase)
{
    ...
    if (expected_event != entry.type) {
        return ::testing::AssertionFailure() << "Actual event: " << NetLogEventTypeToString(entry.type)
                                             << ". Expected event: " << NetLogEventTypeToString(expected_event) << ".";
    }
    if (expected_phase != entry.phase) {
        return ::testing::AssertionFailure() << "Actual phase: " << static_cast<int>(entry.phase) << ". Expected phase: " << static_cast<int>(expected_phase)
                                             << ".";
    }
    return ::testing::AssertionSuccess();
}

::testing::AssertionResult LogContainsBeginEvent(const std::vector<NetLogEntry>& entries, int offset, NetLogEventType expected_event)
{
    return LogContainsEvent(entries, offset, expected_event, NetLogEventPhase::BEGIN);
}

::testing::AssertionResult LogContainsEndEvent(const std::vector<NetLogEntry>& entries, int offset, NetLogEventType expected_event)
{
    return LogContainsEvent(entries, offset, expected_event, NetLogEventPhase::END);
}

::testing::AssertionResult LogContainsEntryWithType(const std::vector<NetLogEntry>& entries, int offset, NetLogEventType type)
{
    size_t index = GetIndex(entries, offset);
    if (index >= entries.size())
        return ::testing::AssertionFailure() << index << " is out of bounds.";
...
::testing::AssertionResult LogContainsEntryWithTypeAfter(const std::vector<NetLogEntry>& entries, int start_offset, NetLogEventType type)
{
...
size_t ExpectLogContainsSomewhere(const std::vector<NetLogEntry>& entries, size_t min_offset, NetLogEventType expected_event, NetLogEventPhase expected_phase)
{
...
size_t ExpectLogContainsSomewhereAfter(
    const std::vector<NetLogEntry>& entries, size_t start_offset, NetLogEventType expected_event, NetLogEventPhase expected_phase)
{
...
std::optional<std::string> GetOptionalStringValueFromParams(const NetLogEntry& entry, std::string_view path)
{
...
std::optional<bool> GetOptionalBooleanValueFromParams(const NetLogEntry& entry, std::string_view path)
{
    if (entry.params.empty()) {
        return std::nullopt;
    }
...
std::optional<int> GetOptionalIntegerValueFromParams(const NetLogEntry& entry, std::string_view path)
{
    if (entry.params.empty()) {
        return std::nullopt;
    }
...
std::string GetStringValueFromParams(const NetLogEntry& entry, std::string_view path)
{
