...
bool GetClientCertInfoFromFile(const char* filename, int* out_type, size_t* out_max_length)
{
    scoped_refptr<X509Certificate> cert = ImportCertFromFile(GetTestCertsDirectory(), filename);
    if (!cert) {
...
bool GetPublicKeyInfoFromCertificateFile(const char* filename, int* out_type, size_t* out_max_length)
{
    scoped_refptr<X509Certificate> cert = ImportCertFromFile(GetTestCertsDirectory(), filename);
    if (!cert) {
        ADD_FAILURE() << "Could not read " << filename;
    ...
    if (!asn1::ExtractSPKIFromDERCert(x509_util::CryptoBufferAsStringPiece(cert->cert_buffer()), &spki)) {
...
TEST(SSLPlatformKeyUtil, GetPublicKeyInfo)
{
    ...
    ASSERT_TRUE(GetPublicKeyInfoFromCertificateFile("client_1.pem", &type, &max_length));
    EXPECT_EQ(EVP_PKEY_RSA, type);
    EXPECT_EQ(2048u / 8u, max_length);

    ASSERT_TRUE(GetPublicKeyInfoFromCertificateFile("client_4.pem", &type, &max_length));
    EXPECT_EQ(EVP_PKEY_EC, type);
    EXPECT_EQ(ECDSA_SIG_max_len(BitsToBytes(256)), max_length);

    ASSERT_TRUE(GetPublicKeyInfoFromCertificateFile("client_5.pem", &type, &max_length));
    EXPECT_EQ(EVP_PKEY_EC, type);
    EXPECT_EQ(ECDSA_SIG_max_len(BitsToBytes(384)), max_length);

    ASSERT_TRUE(GetPublicKeyInfoFromCertificateFile("client_6.pem", &type, &max_length));
    EXPECT_EQ(EVP_PKEY_EC, type);
    EXPECT_EQ(ECDSA_SIG_max_len(BitsToBytes(521)), max_length);
}
