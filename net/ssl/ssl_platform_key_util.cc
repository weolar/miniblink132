...
bssl::UniquePtr<EVP_PKEY> GetClientCertPublicKey(const X509Certificate* certificate)
{
    crypto::OpenSSLErrStackTracer tracker(FROM_HERE);

    std::string_view spki;
    if (!asn1::ExtractSPKIFromDERCert(x509_util::CryptoBufferAsStringPiece(certificate->cert_buffer()), &spki)) {
...
bool GetClientCertInfo(const X509Certificate* certificate, int* out_type, size_t* out_max_length)
{
...
bool GetPublicKeyInfo(base::span<const uint8_t> spki, int* out_type, size_t* out_max_length)
{
...
std::optional<std::vector<uint8_t>> AddPSSPadding(EVP_PKEY* pubkey, const EVP_MD* md, base::span<const uint8_t> digest)
{
    ...
    if (digest.size() != EVP_MD_size(md) || !RSA_padding_add_PKCS1_PSS_mgf1(rsa, ret.data(), digest.data(), md, md, -1 /* salt length is digest length */)) {
        return std::nullopt;
    }
    return ret;
}
