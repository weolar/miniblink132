...
public:
    FakeClientCertIdentity(scoped_refptr<X509Certificate> cert, scoped_refptr<SSLPrivateKey> key);
    ...
    static std::unique_ptr<FakeClientCertIdentity> CreateFromCertAndFailSigning(const base::FilePath& dir, const std::string& cert_filename);
    ...
    SSLPrivateKey* ssl_private_key() const
    {
        return key_.get();
    }
