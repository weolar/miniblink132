...
{
    std::unique_ptr<base::SimpleTestClock> clock = std::make_unique<base::SimpleTestClock>();
...
public:
    SSLClientSessionCacheTest()
        : ssl_ctx_(SSL_CTX_new(TLS_method()))
    {
    }

protected:
    bssl::UniquePtr<SSL_SESSION> NewSSLSession(uint16_t version = TLS1_2_VERSION)
    {
    ...
    bssl::UniquePtr<SSL_SESSION> MakeTestSession(base::Time now, base::TimeDelta timeout)
    {
...
TEST_F(SSLClientSessionCacheTest, Expiration)
{
    ...
    for (size_t i = 0; i < kNumEntries - 1; i++) {
        bssl::UniquePtr<SSL_SESSION> session = MakeTestSession(clock->Now(), kTimeout);
...
TEST_F(SSLClientSessionCacheTest, TestFlushOnMemoryNotifications)
{
    ...
    // Insert an entry into the session cache.
    bssl::UniquePtr<SSL_SESSION> session1 = MakeTestSession(clock->Now(), kTimeout);
    cache.Insert(MakeTestKey("key1"), bssl::UpRef(session1));
    EXPECT_EQ(session1.get(), cache.Lookup(MakeTestKey("key1")).get());
    EXPECT_EQ(1u, cache.size());
    ...
    bssl::UniquePtr<SSL_SESSION> session2 = MakeTestSession(clock->Now(), kTimeout);
    cache.Insert(MakeTestKey("key2"), bssl::UpRef(session2));
    EXPECT_EQ(2u, cache.size());
...
TEST_F(SSLClientSessionCacheTest, FlushForServer)
{
    ...
    key2.network_anonymization_key = NetworkAnonymizationKey::CreateSameSite(kSiteB);
    ...
    key5.network_anonymization_key = NetworkAnonymizationKey::CreateSameSite(kSiteA);
