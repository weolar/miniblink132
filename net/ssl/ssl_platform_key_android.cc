...
public:
    SSLPlatformKeyAndroid(bssl::UniquePtr<EVP_PKEY> pubkey, const JavaRef<jobject>& key)
        ...
        , provider_name_(android::GetPrivateKeyClassName(key))
    {
        ...
        for (uint16_t algorithm : SSLPrivateKey::DefaultAlgorithmPreferences(EVP_PKEY_id(pubkey_.get()), true /* include PSS */)) {
            ...
            if (java_algorithm && android::PrivateKeySupportsSignature(key_, java_algorithm)) {
                ...
                if (!supports_rsa_no_padding) {
                    supports_rsa_no_padding = android::PrivateKeySupportsCipher(key_, kRSANoPadding);
                }
                if (*supports_rsa_no_padding) {
                    preferences_.push_back(algorithm);
...
private:
    Error SignPSSFallback(uint16_t algorithm, base::span<const uint8_t> input, std::vector<uint8_t>* signature)
    {
        const EVP_MD* md = SSL_get_signature_algorithm_digest(algorithm);
        uint8_t digest[EVP_MAX_MD_SIZE];
        unsigned digest_len;
        if (!EVP_Digest(input.data(), input.size(), digest, &digest_len, md, nullptr)) {
            return ERR_SSL_CLIENT_AUTH_SIGNATURE_FAILED;
        }

        std::optional<std::vector<uint8_t>> padded = AddPSSPadding(pubkey_.get(), md, base::make_span(digest, digest_len));
        if (!padded) {
            return ERR_SSL_CLIENT_AUTH_SIGNATURE_FAILED;
        }

        if (!android::EncryptWithPrivateKey(key_, kRSANoPadding, *padded, signature)) {
            return ERR_SSL_CLIENT_AUTH_SIGNATURE_FAILED;
        }
        return OK;
    }
...
scoped_refptr<SSLPrivateKey> WrapJavaPrivateKey(const X509Certificate* certificate, const JavaRef<jobject>& key)
{
    bssl::UniquePtr<EVP_PKEY> pubkey = GetClientCertPublicKey(certificate);
    if (!pubkey)
        return nullptr;

    return base::MakeRefCounted<ThreadedSSLPrivateKey>(std::make_unique<SSLPlatformKeyAndroid>(std::move(pubkey), key), GetSSLPlatformKeyTaskRunner());
}

std::vector<std::string> SignatureAlgorithmsToJavaKeyTypes(base::span<const uint16_t> algorithms)
{
