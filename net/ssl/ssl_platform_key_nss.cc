...
public:
    SSLPlatformKeyNSS(int type, scoped_refptr<crypto::CryptoModuleBlockingPasswordDelegate> password_delegate, crypto::ScopedSECKEYPrivateKey key)
        : type_(type)
    ...
    std::string GetProviderName() override
    {
        ...
        return base::StringPrintf("%s, %s", PK11_GetModule(key_->pkcs11Slot)->commonName, PK11_GetSlotName(key_->pkcs11Slot));
    }
    ...
    Error Sign(uint16_t algorithm, base::span<const uint8_t> input, std::vector<uint8_t>* signature) override
    {
        ...
        if (SSL_is_signature_algorithm_rsa_pss(algorithm)) {
        ...
        } else if (SSL_get_signature_algorithm_key_type(algorithm) == EVP_PKEY_RSA) {
            ...
            int is_alloced;
            size_t prefix_len;
            if (!RSA_add_pkcs1_prefix(&digest_item.data, &prefix_len, &is_alloced, hash_nid, digest_item.data, digest_item.len)) {
                return ERR_SSL_CLIENT_AUTH_SIGNATURE_FAILED;
            }
...

        {
            ...
            SECStatus rv = PK11_SignWithMechanism(key_.get(), mechanism, &param, &signature_item, &digest_item);
            if (rv != SECSuccess) {
...
scoped_refptr<SSLPrivateKey> FetchClientCertPrivateKey(
    const X509Certificate* certificate, CERTCertificate* cert_certificate, scoped_refptr<crypto::CryptoModuleBlockingPasswordDelegate> password_delegate)
{
    ...
    crypto::ScopedSECKEYPrivateKey key(PK11_FindKeyByAnyCert(cert_certificate, wincx));
    if (!key)
        return nullptr;
    ...
    return base::MakeRefCounted<ThreadedSSLPrivateKey>(
        std::make_unique<SSLPlatformKeyNSS>(type, std::move(password_delegate), std::move(key)), GetSSLPlatformKeyTaskRunner());
}
