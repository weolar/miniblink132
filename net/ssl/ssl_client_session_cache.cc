...
{
    return std::tie(key.server, key.dest_ip_addr, key.network_anonymization_key, key.privacy_mode);
}
    ...
    , cache_(config.max_entries)
{
    memory_pressure_listener_
        = std::make_unique<base::MemoryPressureListener>(FROM_HERE, base::BindRepeating(&SSLClientSessionCache::OnMemoryPressure, base::Unretained(this)));
}
...
void SSLClientSessionCache::FlushForServers(const base::flat_set<HostPortPair>& servers)
{
...
bool SSLClientSessionCache::IsExpired(SSL_SESSION* session, time_t now)
{
    ...
    return now_u64 < SSL_SESSION_get_time(session) - 1 || now_u64 >= SSL_SESSION_get_time(session) + SSL_SESSION_get_timeout(session);
}
...
void SSLClientSessionCache::OnMemoryPressure(base::MemoryPressureListener::MemoryPressureLevel memory_pressure_level)
{
