...
public:
    ...
    X509Certificate* certificate() const
    {
        return cert_.get();
    }
    ...
    virtual void AcquirePrivateKey(base::OnceCallback<void(scoped_refptr<SSLPrivateKey>)> private_key_callback) = 0;
    ...
    static void SelfOwningAcquirePrivateKey(
        std::unique_ptr<ClientCertIdentity> identity, base::OnceCallback<void(scoped_refptr<SSLPrivateKey>)> private_key_callback);
    ...
    // intermediates list.
    void SetIntermediates(std::vector<bssl::UniquePtr<CRYPTO_BUFFER>> intermediates);
...
public:
    ...
    bool operator()(const std::unique_ptr<ClientCertIdentity>& a, const std::unique_ptr<ClientCertIdentity>& b) const;
