...
class NET_EXPORT SSLPrivateKey : public base::RefCountedThreadSafe<SSLPrivateKey> {
public:
    using SignCallback = base::OnceCallback<void(Error, const std::vector<uint8_t>&)>;
    ...
    virtual void Sign(uint16_t algorithm, base::span<const uint8_t> input, SignCallback callback) = 0;
    ...
    // list.
    static std::vector<uint16_t> DefaultAlgorithmPreferences(int type, bool supports_pss);
