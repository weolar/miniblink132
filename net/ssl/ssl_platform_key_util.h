...
NET_EXPORT_PRIVATE scoped_refptr<base::SingleThreadTaskRunner> GetSSLPlatformKeyTaskRunner();
...
// BoringSSL.
NET_EXPORT_PRIVATE bool GetClientCertInfo(const X509Certificate* certificate, int* out_type, size_t* out_max_length);

// Parses a DER-encoded SPKI buffer and returns the public key as an |EVP_PKEY|,
// or nullptr on error.
NET_EXPORT_PRIVATE bssl::UniquePtr<EVP_PKEY> ParseSpki(base::span<const uint8_t> spki);
...
NET_EXPORT_PRIVATE bool GetPublicKeyInfo(base::span<const uint8_t> spki, int* out_type, size_t* out_max_length);
