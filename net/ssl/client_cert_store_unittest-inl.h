...
template <typename T> class ClientCertStoreTest : public TestWithTaskEnvironment {
public:
    T delegate_;
};
...
TYPED_TEST_P(ClientCertStoreTest, EmptyQuery)
{
    ...
    ClientCertIdentityList selected_identities;
    bool rv = this->delegate_.SelectClientCerts(certs, *request.get(), &selected_identities);
    EXPECT_TRUE(rv);
...
{
    scoped_refptr<X509Certificate> cert(ImportCertFromFile(GetTestCertsDirectory(), "client_1.pem"));
    ...
    EXPECT_TRUE(selected_identities[0]->certificate()->EqualsExcludingChain(cert.get()));
}
...
TYPED_TEST_P(ClientCertStoreTest, CertAuthorityFiltering)
{
    ...
    std::vector<std::string> authority_1(1, std::string(reinterpret_cast<const char*>(kAuthority1DN), sizeof(kAuthority1DN)));
    ...
    EXPECT_TRUE(rv);
    ASSERT_EQ(1u, selected_identities.size());
    EXPECT_TRUE(selected_identities[0]->certificate()->EqualsExcludingChain(cert_1.get()));
}
...
TYPED_TEST_P(ClientCertStoreTest, PrintableStringContainingUTF8)
{
    ...
    scoped_refptr<X509Certificate> cert = X509Certificate::CreateFromBufferUnsafeOptions(std::move(cert_handle), {}, options);
    ASSERT_TRUE(cert);

    auto request = base::MakeRefCounted<SSLCertRequestInfo>();

    ClientCertIdentityList selected_identities;
    bool rv = this->delegate_.SelectClientCerts({ cert }, *request.get(), &selected_identities);
    EXPECT_TRUE(rv);
    ASSERT_EQ(1u, selected_identities.size());
    EXPECT_TRUE(selected_identities[0]->certificate()->EqualsExcludingChain(cert.get()));
}

REGISTER_TYPED_TEST_SUITE_P(ClientCertStoreTest, EmptyQuery, AllIssuersAllowed, CertAuthorityFiltering, PrintableStringContainingUTF8);
