...
NET_EXPORT scoped_refptr<SSLPrivateKey> CreateSSLPrivateKeyForSecKey(const X509Certificate* certificate, SecKeyRef key);

// Returns an `SSLPrivateKey` backed by the platform private key contained in
// `unexportable_key`.
NET_EXPORT scoped_refptr<SSLPrivateKey> WrapUnexportableKey(const crypto::UnexportableSigningKey& unexportable_key);
