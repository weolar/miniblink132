...
NET_EXPORT_PRIVATE scoped_refptr<SSLPrivateKey> WrapCAPIPrivateKey(const X509Certificate* certificate, crypto::ScopedHCRYPTPROV prov, DWORD key_spec);

// Returns an SSLPrivateKey backed by |key|, which must correspond to
// |certificate|'s public key, or nullptr on error.
NET_EXPORT_PRIVATE scoped_refptr<SSLPrivateKey> WrapCNGPrivateKey(const X509Certificate* certificate, crypto::ScopedNCryptKey key);

// Uses `key` to load a second NCrypt key handle and return an
// SSLPrivateKey making use of that new handle.
NET_EXPORT scoped_refptr<SSLPrivateKey> WrapUnexportableKeySlowly(const crypto::UnexportableSigningKey& key);
