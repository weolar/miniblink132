...
public:
    ...
    bool EncryptString(const std::string& plaintext, std::string* ciphertext) override;
    bool DecryptString(const std::string& ciphertext, std::string* plaintext) override;
...
base::OnceClosure CookieCryptor::GetInitClosure(base::OnceClosure callback)
{
    ...
    return base::BindOnce(&CookieCryptor::Init, weak_ptr_factory_.GetWeakPtr(), std::move(callback));
}
...
void CookieCryptor::Init(base::OnceClosure callback)
{
    ...
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(
        FROM_HERE, base::BindOnce(&CookieCryptor::InitComplete, weak_ptr_factory_.GetWeakPtr()), base::Milliseconds(100));
}

bool CookieCryptor::EncryptString(const std::string& plaintext, std::string* ciphertext)
{
    ...
    auto result = crypto::aes_cbc::Encrypt(kFixedKey, kFixedIv, base::as_byte_span(plaintext));
    ciphertext->assign(result.begin(), result.end());
    return true;
}

bool CookieCryptor::DecryptString(const std::string& ciphertext, std::string* plaintext)
{
    // SQLite crypto uses OSCrypt Async Encryptor and the behavior for empty
    ...
    auto result = crypto::aes_cbc::Decrypt(kFixedKey, kFixedIv, base::as_byte_span(ciphertext));
    if (result.has_value()) {
...
MATCHER_P2(MatchesCookieKeyAndLastAccessDate, StrictlyUniqueKey, last_access_date, "")
{
    if (!arg) {
        return false;
    }
    ...
    return testing::ExplainMatchResult(StrictlyUniqueKey, list_cookie.StrictlyUniqueKey(), result_listener)
        && testing::ExplainMatchResult(last_access_date, list_cookie.LastAccessDate(), result_listener);
}
...
public:
    SQLitePersistentCookieStoreTest()
        : loaded_event_(base::WaitableEvent::ResetPolicy::AUTOMATIC, base::WaitableEvent::InitialState::NOT_SIGNALED)
        , db_thread_event_(base::WaitableEvent::ResetPolicy::AUTOMATIC, base::WaitableEvent::InitialState::NOT_SIGNALED)
    {
    }

    void SignalLoadedEvent()
    {
    ...
    CanonicalCookieVector Load()
    {
        ...
        store_->Load(base::BindLambdaForTesting([&](CanonicalCookieVector obtained_cookies) {
    ...
    {
        store_->Load(base::BindOnce(&SQLitePersistentCookieStoreTest::OnLoaded, base::Unretained(this),
                         base::BindOnce(&SQLitePersistentCookieStoreTest::SignalLoadedEvent, base::Unretained(this))),
            NetLogWithSource::Make(NetLogSourceType::NONE));
    }
    ...
    void Create(bool crypt_cookies, bool restore_old_session_cookies, bool use_current_thread, bool enable_exclusive_access)
    {
        store_ = base::MakeRefCounted<SQLitePersistentCookieStore>(temp_dir_.GetPath().Append(kCookieFilename),
            use_current_thread ? base::SingleThreadTaskRunner::GetCurrentDefault() : client_task_runner_, background_task_runner_, restore_old_session_cookies,
            crypt_cookies ? std::make_unique<CookieCryptor>() : nullptr, enable_exclusive_access);
    }

    CanonicalCookieVector CreateAndLoad(bool crypt_cookies, bool restore_old_session_cookies)
    {
    ...
    void AddCookie(const std::string& name, const std::string& value, const std::string& domain, const std::string& path, const base::Time& creation)
    {
        store_->AddCookie(*CanonicalCookie::CreateUnsafeCookieForTesting(name, value, domain, path, creation, /*expiration=*/creation,
            /*last_access=*/base::Time(), /*last_update=*/base::Time(),
            /*secure=*/false,
            /*httponly=*/false, CookieSameSite::NO_RESTRICTION, COOKIE_PRIORITY_DEFAULT));
    }

    void AddCookieWithExpiration(const std::string& name, const std::string& value, const std::string& domain, const std::string& path,
        const base::Time& creation, const base::Time& expiration)
    {
        store_->AddCookie(*CanonicalCookie::CreateUnsafeCookieForTesting(name, value, domain, path, creation, expiration,
            /*last_access=*/base::Time(), /*last_update=*/base::Time(),
            /*secure=*/false,
            /*httponly=*/false, CookieSameSite::NO_RESTRICTION, COOKIE_PRIORITY_DEFAULT));
    }
    ...
    {
        std::string contents;
        if (!base::ReadFileToString(temp_dir_.GetPath().Append(kCookieFilename), &contents))
            return std::string();
        return contents;
    }
    ...
    void TearDown() override
    {
        if (!expect_init_errors_) {
            EXPECT_THAT(histograms_.GetAllSamples("Cookie.ErrorInitializeDB"), ::testing::IsEmpty());
        }
        DestroyStore();
    }
...
protected:
    ...
    const scoped_refptr<base::SequencedTaskRunner> client_task_runner_ = base::ThreadPool::CreateSequencedTaskRunner({ base::MayBlock() });
...
TEST_F(SQLitePersistentCookieStoreTest, TestSessionCookiesDeletedOnStartup)
{
    ...
    store_ = base::MakeRefCounted<SQLitePersistentCookieStore>(
        temp_dir_.GetPath().Append(kCookieFilename), client_task_runner_, background_task_runner_, false, nullptr, false);

    // Posting a blocking task to db_thread_ makes sure that the DB thread waits
    // until both Load and Flush have been posted to its task queue.
    background_task_runner_->PostTask(FROM_HERE, base::BindOnce(&SQLitePersistentCookieStoreTest::WaitOnDBEvent, base::Unretained(this)));
    ...
    store_->Flush(base::BindOnce(&base::WaitableEvent::Signal, base::Unretained(&event)));
    ...
    store_ = base::MakeRefCounted<SQLitePersistentCookieStore>(
        temp_dir_.GetPath().Append(kCookieFilename), client_task_runner_, background_task_runner_, true, nullptr, false);
    LoadAsyncAndSignalEvent();
    loaded_event_.Wait();
...
TEST_F(SQLitePersistentCookieStoreTest, TestLoadCookiesForKey)
{
    ...
    auto init_closure = cookie_crypto_delegate->GetInitClosure(cookie_crypto_loop.QuitClosure());
    ...
    store_ = base::MakeRefCounted<SQLitePersistentCookieStore>(temp_dir_.GetPath().Append(kCookieFilename), base::SingleThreadTaskRunner::GetCurrentDefault(),
    ...
    store_->LoadCookiesForKey("aaa.com", base::BindOnce(&SQLitePersistentCookieStoreTest::OnLoaded, base::Unretained(this), run_loop.QuitClosure()));
    ...
    for (CanonicalCookieVector::const_iterator it = cookies_.begin(); it != cookies_.end(); ++it) {
    ...
    ASSERT_EQ(true, cookies_loaded.find("travel.aaa.com") != cookies_loaded.end());
    ...
    size_t pos = ExpectLogContainsSomewhere(entries, 0, NetLogEventType::COOKIE_PERSISTENT_STORE_LOAD, NetLogEventPhase::BEGIN);
    pos = ExpectLogContainsSomewhere(entries, pos, NetLogEventType::COOKIE_PERSISTENT_STORE_LOAD, NetLogEventPhase::END);
    pos = ExpectLogContainsSomewhere(entries, 0, NetLogEventType::COOKIE_PERSISTENT_STORE_LOAD, NetLogEventPhase::BEGIN);
    pos = ExpectLogContainsSomewhere(entries, pos, NetLogEventType::COOKIE_PERSISTENT_STORE_KEY_LOAD_STARTED, NetLogEventPhase::NONE);
    EXPECT_FALSE(GetOptionalStringValueFromParams(entries[pos], "key"));
    pos = ExpectLogContainsSomewhere(entries, pos, NetLogEventType::COOKIE_PERSISTENT_STORE_KEY_LOAD_COMPLETED, NetLogEventPhase::NONE);
    pos = ExpectLogContainsSomewhere(entries, pos, NetLogEventType::COOKIE_PERSISTENT_STORE_LOAD, NetLogEventPhase::END);
    ExpectLogContainsSomewhere(entries, pos, NetLogEventType::COOKIE_PERSISTENT_STORE_CLOSED, NetLogEventPhase::NONE);
}
...
TEST_F(SQLitePersistentCookieStoreTest, TestBeforeCommitCallback)
{
    ...
    Counter counter;
    store_->SetBeforeCommitCallback(base::BindRepeating(&Counter::increment, base::Unretained(&counter)));
...
TEST_F(SQLitePersistentCookieStoreTest, TestLoadOldSessionCookies)
{
    ...
    store_->AddCookie(*CanonicalCookie::CreateUnsafeCookieForTesting("C", "D", "sessioncookie.com", "/", /*creation=*/base::Time::Now(),
        /*expiration=*/base::Time(),
        /*last_access=*/base::Time(), /*last_update=*/base::Time(),
        /*secure=*/false, /*httponly=*/false, CookieSameSite::NO_RESTRICTION, COOKIE_PRIORITY_DEFAULT));
...
TEST_F(SQLitePersistentCookieStoreTest, FilterBadCookiesAndFixupDb)
{
    ...
    sql::Statement stmt(db->GetUniqueStatement("INSERT INTO cookies (creation_utc, host_key, top_frame_site_key, name, "
    ...
    struct CookieInfo {
        ...
        { "google.izzle", "A=", "B", "/path" }, { "google.izzle", "C ", "D", "/path" },
...
TEST_F(SQLitePersistentCookieStoreTest, PersistIsPersistent)
{
    ...
    // Add a session cookie.
    store_->AddCookie(*CanonicalCookie::CreateUnsafeCookieForTesting(kSessionName, "val", "sessioncookie.com", "/",
        /*creation=*/base::Time::Now(),
        /*expiration=*/base::Time(), /*last_access=*/base::Time(),
        /*last_update=*/base::Time(), /*secure=*/false, /*httponly=*/false, CookieSameSite::NO_RESTRICTION, COOKIE_PRIORITY_DEFAULT));
    // Add a persistent cookie.
    store_->AddCookie(*CanonicalCookie::CreateUnsafeCookieForTesting(kPersistentName, "val", "sessioncookie.com", "/",
...
TEST_F(SQLitePersistentCookieStoreTest, PriorityIsPersistent)
{
    ...
    store_->AddCookie(*CanonicalCookie::CreateUnsafeCookieForTesting(kLowName, kCookieValue, kDomain, kCookiePath,
        ...
        /*expiration=*/base::Time::Now() + base::Days(1),
        /*last_access=*/base::Time(), /*last_update=*/base::Time(),
        /*secure=*/false, /*httponly=*/false, CookieSameSite::NO_RESTRICTION, COOKIE_PRIORITY_LOW));

    // Add a medium-priority persistent cookie.
    store_->AddCookie(*CanonicalCookie::CreateUnsafeCookieForTesting(kMediumName, kCookieValue, kDomain, kCookiePath,
        /*creation=*/base::Time::Now() - base::Minutes(2),
        /*expiration=*/base::Time::Now() + base::Days(1),
        /*last_access=*/base::Time(), /*last_update=*/base::Time(),
        /*secure=*/false, /*httponly=*/false, CookieSameSite::NO_RESTRICTION, COOKIE_PRIORITY_MEDIUM));
    ...
    store_->AddCookie(*CanonicalCookie::CreateUnsafeCookieForTesting(kHighName, kCookieValue, kDomain, kCookiePath,
        ...
        /*expiration=*/base::Time::Now() + base::Days(1),
        /*last_access=*/base::Time(), /*last_update=*/base::Time(),
        /*secure=*/false, /*httponly=*/false, CookieSameSite::NO_RESTRICTION, COOKIE_PRIORITY_HIGH));
...
TEST_F(SQLitePersistentCookieStoreTest, SameSiteIsPersistent)
{
    ...
    store_->AddCookie(*CanonicalCookie::CreateUnsafeCookieForTesting(kLaxName, kCookieValue, kDomain, kCookiePath,
        ...
        /*expiration=*/base::Time::Now() + base::Days(1),
        /*last_access=*/base::Time(), /*last_update=*/base::Time(),
        /*secure=*/false, /*httponly=*/false, CookieSameSite::LAX_MODE, COOKIE_PRIORITY_DEFAULT));
    ...
    store_->AddCookie(*CanonicalCookie::CreateUnsafeCookieForTesting(kStrictName, kCookieValue, kDomain, kCookiePath,
        ...
        /*expiration=*/base::Time::Now() + base::Days(1),
        /*last_access=*/base::Time(), /*last_update=*/base::Time(),
        /*secure=*/false, /*httponly=*/false, CookieSameSite::STRICT_MODE, COOKIE_PRIORITY_DEFAULT));
...
TEST_F(SQLitePersistentCookieStoreTest, SameSiteExtendedTreatedAsUnspecified)
{
    ...
    store_->AddCookie(*CanonicalCookie::CreateUnsafeCookieForTesting(kExtendedName, kCookieValue, kDomain, kCookiePath,
        /*creation=*/base::Time::Now() - base::Minutes(1),
        /*expiration=*/base::Time::Now() + base::Days(1),
        /*last_access=*/base::Time(), /*last_update=*/base::Time(),
        /*secure=*/false, /*httponly=*/false, CookieSameSite::STRICT_MODE, COOKIE_PRIORITY_DEFAULT));
    ...
    std::string update_stmt("UPDATE cookies SET samesite=3" // 3 is Extended.
...
TEST_F(SQLitePersistentCookieStoreTest, SourcePortIsPersistent)
{
    ...
    const std::vector<CookieTestValues> kTestCookies = { { "1", 80 }, { "2", 443 }, { "3", 1234 }, { "4", url::PORT_UNSPECIFIED }, { "5", url::PORT_INVALID } };

    InitializeStore(/*crypt=*/false, /*restore_old_session_cookies=*/true);

    for (const auto& input : kTestCookies) {
        // Add some persistent cookies.
        store_->AddCookie(*CanonicalCookie::CreateUnsafeCookieForTesting(input.name, kCookieValue, kDomain, kCookiePath,
            /*creation=*/base::Time::Now() - base::Minutes(1),
            /*expiration=*/base::Time::Now() + base::Days(1),
            /*last_access=*/base::Time(), /*last_update=*/base::Time(),
            /*secure=*/true, /*httponly=*/false, CookieSameSite::LAX_MODE, COOKIE_PRIORITY_DEFAULT,
            /*partition_key=*/std::nullopt, CookieSourceScheme::kUnset /* Doesn't matter for this test. */, input.port));
    }
...
TEST_F(SQLitePersistentCookieStoreTest, UpdateToEncryption)
{
    ...
    AddCookie("other", "something456ABC", "foo.bar", "/", base::Time::Now() + base::Microseconds(10));
    DestroyStore();
...
bool CompareCookies(const std::unique_ptr<CanonicalCookie>& a, const std::unique_ptr<CanonicalCookie>& b)
{
...
TEST_F(SQLitePersistentCookieStoreTest, IdenticalCreationTimes)
{
    ...
    AddCookieWithExpiration("A", "B", "example.com", "/", cookie_time, cookie_expiry);
    AddCookieWithExpiration("C", "B", "example.com", "/", cookie_time, cookie_expiry);
    AddCookieWithExpiration("A", "B", "example2.com", "/", cookie_time, cookie_expiry);
    AddCookieWithExpiration("C", "B", "example2.com", "/", cookie_time, cookie_expiry);
    AddCookieWithExpiration("A", "B", "example.com", "/path", cookie_time, cookie_expiry);
    AddCookieWithExpiration("C", "B", "example.com", "/path", cookie_time, cookie_expiry);
    Flush();
    DestroyStore();
...
TEST_F(SQLitePersistentCookieStoreTest, KeyInconsistency)
{
    ...
    // and save it.
    std::unique_ptr<CookieMonster> cookie_monster = std::make_unique<CookieMonster>(store_.get(), /*net_log=*/nullptr);
    ResultSavingCookieCallback<bool> cookie_scheme_callback1;
    cookie_monster->SetCookieableSchemes({ "ftp", "http" }, cookie_scheme_callback1.MakeCallback());
    ...
    auto cookie = CanonicalCookie::CreateForTesting(ftp_url, "A=B; max-age=3600", base::Time::Now());
    cookie_monster->SetCanonicalCookieAsync(std::move(cookie), ftp_url, CookieOptions::MakeAllInclusive(), set_cookie_callback.MakeCallback());
    ...
    for (int i = 0; i < 50; ++i) {
        ...
        auto canonical_cookie = CanonicalCookie::CreateForTesting(url, "A=B; max-age=3600", base::Time::Now());
        cookie_monster->SetCanonicalCookieAsync(std::move(canonical_cookie), url, CookieOptions::MakeAllInclusive(), set_cookie_callback2.MakeCallback());
    ...
    Create(/*crypt_cookies=*/true, /*restore_old_session_cookies=*/false,
        /*use_current_thread=*/true, /*enable_exclusive_access=*/false);
    cookie_monster = std::make_unique<CookieMonster>(store_.get(), /*net_log=*/nullptr);
    ResultSavingCookieCallback<bool> cookie_scheme_callback2;
    cookie_monster->SetCookieableSchemes({ "ftp", "http" }, cookie_scheme_callback2.MakeCallback());
    ...
    cookie_monster->GetCookieListWithOptionsAsync(GURL("ftp://subdomain.ftperiffic.com/page"), CookieOptions::MakeAllInclusive(),
        CookiePartitionKeyCollection(), base::BindOnce(&GetCookieListCallback::Run, base::Unretained(&get_callback)));
...
TEST_F(SQLitePersistentCookieStoreTest, OpsIfInitFailed)
{
    ...
    ASSERT_TRUE(base::CreateDirectory(temp_dir_.GetPath().Append(kCookieFilename)));
    ...
    auto cookie = CanonicalCookie::CreateForTesting(url, "A=B; max-age=3600", base::Time::Now());
    cookie_monster->SetCanonicalCookieAsync(std::move(cookie), url, CookieOptions::MakeAllInclusive(), set_cookie_callback.MakeCallback());
    set_cookie_callback.WaitUntilDone();
    EXPECT_TRUE(set_cookie_callback.result().status.IsInclude());
...
TEST_F(SQLitePersistentCookieStoreTest, Coalescing)
{
    ...
    std::vector<TestCase> testcases = { { { Op::kAdd, Op::kDelete }, 1u }, { { Op::kUpdate, Op::kDelete }, 1u }, { { Op::kAdd, Op::kUpdate, Op::kDelete }, 1u },
        { { Op::kUpdate, Op::kUpdate }, 1u }, { { Op::kAdd, Op::kUpdate, Op::kUpdate }, 2u }, { { Op::kDelete, Op::kAdd }, 2u },
        { { Op::kDelete, Op::kAdd, Op::kUpdate }, 3u }, { { Op::kDelete, Op::kAdd, Op::kUpdate, Op::kUpdate }, 3u }, { { Op::kDelete, Op::kDelete }, 1u },
        { { Op::kDelete, Op::kAdd, Op::kDelete }, 1u }, { { Op::kDelete, Op::kAdd, Op::kUpdate, Op::kDelete }, 1u } };

    std::unique_ptr<CanonicalCookie> cookie = CanonicalCookie::CreateForTesting(GURL("http://www.example.com/path"), "Tasty=Yes", base::Time::Now());
    ...
    for (const TestCase& testcase : testcases) {
        ...
        base::RunLoop run_loop;
        store_->Load(base::BindLambdaForTesting([&](CanonicalCookieVector cookies) { run_loop.Quit(); }), NetLogWithSource());
        run_loop.Run();
        ...
        EXPECT_EQ(testcase.expected_queue_length, store_->GetQueueLengthForTesting());
...
TEST_F(SQLitePersistentCookieStoreTest, NoCoalesceUnrelated)
{
    ...
    std::unique_ptr<CanonicalCookie> cookie1 = CanonicalCookie::CreateForTesting(GURL("http://www.example.com/path"), "Tasty=Yes", base::Time::Now());

    std::unique_ptr<CanonicalCookie> cookie2 = CanonicalCookie::CreateForTesting(GURL("http://not.example.com/path"), "Tasty=No", base::Time::Now());
...
class SQLitePersistentCookieStoreExclusiveAccessTest : public SQLitePersistentCookieStoreTest, public ::testing::WithParamInterface<bool> {
protected:
    const bool& ShouldBeExclusive()
    {
        return GetParam();
    }
};
...
TEST_P(SQLitePersistentCookieStoreExclusiveAccessTest, LockedStore)
{
    ...
    {
        base::File file(temp_dir_.GetPath().Append(kCookieFilename), base::File::Flags::FLAG_OPEN_ALWAYS | base::File::Flags::FLAG_READ);
...
TEST_P(SQLitePersistentCookieStoreExclusiveAccessTest, LockedStoreAlreadyOpen)
{
    ...
    base::File file(temp_dir_.GetPath().Append(kCookieFilename), base::File::Flags::FLAG_CREATE | base::File::Flags::FLAG_READ);
    ...
    if (ShouldBeExclusive()) {
        expect_init_errors_ = true;
        histograms.ExpectUniqueSample("Cookie.ErrorInitializeDB", sql::SqliteLoggedResultCode::kCantOpen, 1);
        histograms.ExpectUniqueSample("Cookie.WinGetLastErrorInitializeDB", ERROR_SHARING_VIOLATION, 1);
    }
}

INSTANTIATE_TEST_SUITE_P(
    All, SQLitePersistentCookieStoreExclusiveAccessTest, ::testing::Bool(), [](const auto& info) { return info.param ? "Exclusive" : "NotExclusive"; });
...
{
    base::HistogramTester histograms;
    base::WriteFile(temp_dir_.GetPath().Append(kCookieFilename), "SQLite format 3 foobarfoobarfoobar");
    ...
    expect_init_errors_ = true;
    histograms.ExpectUniqueSample("Cookie.ErrorInitializeDB", sql::SqliteLoggedResultCode::kNotADatabase, 1);
}
...
bool CreateV18Schema(sql::Database* db)
{
    ...
    // Version 18 schema
    static constexpr char kCreateSql[] = "CREATE TABLE cookies("
    ...
    static constexpr char kCreateIndexSql[] = "CREATE UNIQUE INDEX cookies_unique_index "
...
std::vector<CanonicalCookie> CookiesForMigrationTest()
{
    ...
    cookies.push_back(*CanonicalCookie::CreateUnsafeCookieForTesting("A", "B", "example.com", "/", /*creation=*/now, /*expiration=*/now,
        /*last_access=*/now, /*last_update=*/now, /*secure=*/true,
        /*httponly=*/false, CookieSameSite::UNSPECIFIED, COOKIE_PRIORITY_DEFAULT));
    cookies.push_back(*CanonicalCookie::CreateUnsafeCookieForTesting("C", "B", "example.com", "/", /*creation=*/now, /*expiration=*/now,
        /*last_access=*/now, /*last_update=*/now, /*secure=*/true,
        /*httponly=*/false, CookieSameSite::UNSPECIFIED, COOKIE_PRIORITY_DEFAULT));
    cookies.push_back(*CanonicalCookie::CreateUnsafeCookieForTesting("A", "B", "example2.com", "/", /*creation=*/now, /*expiration=*/now,
        /*last_access=*/now, /*last_update=*/now, /*secure=*/true,
        /*httponly=*/false, CookieSameSite::UNSPECIFIED, COOKIE_PRIORITY_DEFAULT));
    cookies.push_back(*CanonicalCookie::CreateUnsafeCookieForTesting("C", "B", "example2.com", "/", /*creation=*/now,
        /*expiration=*/now + base::Days(399), /*last_access=*/now,
        /*last_update=*/now,
        /*secure=*/false, /*httponly=*/false, CookieSameSite::UNSPECIFIED, COOKIE_PRIORITY_DEFAULT));
    cookies.push_back(*CanonicalCookie::CreateUnsafeCookieForTesting("A", "B", "example.com", "/path", /*creation=*/now,
        /*expiration=*/now + base::Days(400), /*last_access=*/now,
        /*last_update=*/now,
        /*secure=*/false, /*httponly=*/false, CookieSameSite::UNSPECIFIED, COOKIE_PRIORITY_DEFAULT));
    cookies.push_back(*CanonicalCookie::CreateUnsafeCookieForTesting("C", "B", "example.com", "/path", /*creation=*/now,
        /*expiration=*/now + base::Days(401), /*last_access=*/now,
        /*last_update=*/now,
        /*secure=*/false, /*httponly=*/false, CookieSameSite::UNSPECIFIED, COOKIE_PRIORITY_DEFAULT));
    cookies.push_back(*CanonicalCookie::CreateUnsafeCookieForTesting("D", "", "empty.com", "/", /*creation=*/now, /*expiration=*/now,
        /*last_access=*/now, /*last_update=*/now, /*secure=*/true,
        /*httponly=*/false, CookieSameSite::UNSPECIFIED, COOKIE_PRIORITY_DEFAULT));
    return cookies;
}
...
bool AddV18CookiesToDB(sql::Database* db, base::TimeDelta max_expiration_delta)
{
    ...
    for (const CanonicalCookie& cookie : cookies) {
        ...
        base::expected<CookiePartitionKey::SerializedCookiePartitionKey, std::string> serialized_partition_key
...
bool AddV22CookiesToDB(sql::Database* db, const std::vector<CanonicalCookie>& cookies)
{
    sql::Statement statement(db->GetCachedStatement(SQL_FROM_HERE,
        "INSERT INTO cookies (creation_utc, top_frame_site_key, host_key, name, "
        "value, encrypted_value, path, expires_utc, is_secure, is_httponly, "
...
bool AddV23CookiesToDB(sql::Database* db, const std::vector<CanonicalCookie>& cookies, CookieCryptoDelegate* crypto, bool place_unencrypted_too)
{
    sql::Statement statement(db->GetCachedStatement(SQL_FROM_HERE,
    ...
    for (const CanonicalCookie& cookie : cookies) {
        ...
        statement.BindString(3, cookie.Name());
        if (crypto) {
            statement.BindString(4, place_unencrypted_too ? cookie.Value() : ""); // value is encrypted. If `place_unencrypted_too` is
        ...
        if (cookie.SourceScheme() == CookieSourceScheme::kUnset && cookie.IsSecure()) {
...
void ConfirmCookiesAfterMigrationTest(std::vector<std::unique_ptr<CanonicalCookie>> read_in_cookies, bool expect_last_update_date = false)
{
    ...
    EXPECT_EQ(read_in_cookies[i]->LastUpdateDate(), expect_last_update_date ? read_in_cookies[i]->CreationDate() : base::Time());
    EXPECT_EQ(read_in_cookies[i]->ExpiryDate(), read_in_cookies[i]->CreationDate());
    ...
    EXPECT_EQ(read_in_cookies[i]->LastUpdateDate(), expect_last_update_date ? read_in_cookies[i]->CreationDate() : base::Time());
    EXPECT_EQ(read_in_cookies[i]->ExpiryDate(), read_in_cookies[i]->CreationDate() + base::Days(400));
    EXPECT_EQ(read_in_cookies[i]->SourceType(), CookieSourceType::kUnknown);
    ...
    EXPECT_LE(read_in_cookies[i]->ExpiryDate(), base::Time::Now() + base::Days(400));
    EXPECT_GE(read_in_cookies[i]->ExpiryDate(), base::Time::Now() + base::Days(400) - base::Minutes(1));
    EXPECT_EQ(read_in_cookies[i]->SourceType(), CookieSourceType::kUnknown);
    ...
    EXPECT_EQ(CookieSourceScheme::kUnset, read_in_cookies[i]->SourceScheme());
    EXPECT_EQ(read_in_cookies[i]->LastUpdateDate(), expect_last_update_date ? read_in_cookies[i]->CreationDate() : base::Time());
    EXPECT_EQ(read_in_cookies[i]->ExpiryDate(), read_in_cookies[i]->CreationDate() + base::Days(399));
    EXPECT_EQ(read_in_cookies[i]->SourceType(), CookieSourceType::kUnknown);
...
void ConfirmDatabaseVersionAfterMigration(const base::FilePath path, int version)
{
    sql::Database connection;
...
{
    // Open db.
    const base::FilePath database_path = temp_dir_.GetPath().Append(kCookieFilename);
    {
        sql::Database connection;
    ...
    CanonicalCookieVector read_in_cookies = CreateAndLoad(
        /*crypt_cookies=*/false, /*restore_old_session_cookies=*/false);
    ASSERT_NO_FATAL_FAILURE(ConfirmCookiesAfterMigrationTest(std::move(read_in_cookies),
        /*expect_last_update_date=*/true));
    DestroyStore();

    ASSERT_NO_FATAL_FAILURE(ConfirmDatabaseVersionAfterMigration(database_path, 19));
}
...
TEST_F(SQLitePersistentCookieStoreTest, UpgradeToSchemaVersion20)
{
    ...
    ASSERT_NO_FATAL_FAILURE(ConfirmDatabaseVersionAfterMigration(database_path, 20));
}
...
TEST_F(SQLitePersistentCookieStoreTest, UpgradeToSchemaVersion21)
{
    ...
    ASSERT_NO_FATAL_FAILURE(ConfirmDatabaseVersionAfterMigration(database_path, 21));
}
...
TEST_F(SQLitePersistentCookieStoreTest, UpgradeToSchemaVersion22)
{
    ...
    ASSERT_NO_FATAL_FAILURE(ConfirmDatabaseVersionAfterMigration(database_path, 22));
}
...
TEST_F(SQLitePersistentCookieStoreTest, UpgradeToSchemaVersion23)
{
    ...
    ASSERT_NO_FATAL_FAILURE(ConfirmDatabaseVersionAfterMigration(database_path, 23));
}

class SQLitePersistentCookieStorev24UpgradeTest : public SQLitePersistentCookieStoreTest,
                                                  public ::testing::WithParamInterface<std::tuple</*crypto_for_encrypt*/ bool,
...
protected:
    void SetUp() override
    {
        features_.InitWithFeatureState(features::kEncryptedAndPlaintextValuesAreInvalid, std::get<3>(GetParam()));
...
TEST_P(SQLitePersistentCookieStorev24UpgradeTest, UpgradeToSchemaVersion24)
{
    ...
    const base::FilePath database_path = temp_dir_.GetPath().Append(kCookieFilename);
    {
        ...
        ASSERT_TRUE(AddV23CookiesToDB(&connection, CookiesForMigrationTest(), crypto_for_encrypt ? cryptor.get() : nullptr, place_unencrypted_too));
    }
    {
        ...
        if (crypto_for_encrypt && !crypto_for_decrypt) {
            ...
            const base::Histogram::Sample expected_bucket = drop_dup_values && place_unencrypted_too
                ? /*CookieLoadProblem::kValuesExistInBothEncryptedAndPlaintext*/ 8
                : /*CookieLoadProblem::kNoCrypto*/ 7;
            histogram_tester.ExpectBucketCount("Cookie.LoadProblem", expected_bucket, CookiesForMigrationTest().size() - 1);
        } else {
    ...
    ASSERT_NO_FATAL_FAILURE(ConfirmDatabaseVersionAfterMigration(database_path, 24));
}

INSTANTIATE_TEST_SUITE_P(
    , SQLitePersistentCookieStorev24UpgradeTest, ::testing::Combine(::testing::Bool(), ::testing::Bool(), ::testing::Bool(), ::testing::Bool()));
...
TEST_F(SQLitePersistentCookieStoreTest, CannotModifyHostName)
{
    ...
    }
    {
        ...
        ASSERT_TRUE(connection.Execute("UPDATE cookies SET host_key='attacker.com' WHERE "
...
TEST_F(SQLitePersistentCookieStoreTest, ShortHash)
{
    ...
    }
    {
        ...
        ASSERT_TRUE(connection.Open(database_path));
        sql::Transaction transaction(&connection);
        sql::Statement set_encrypted_value(connection.GetUniqueStatement("UPDATE cookies SET encrypted_value=? WHERE host_key='sensitive.com'"));
...
TEST_F(SQLitePersistentCookieStoreTest, UpgradeToSchemaVersion23_ConfirmSourceSchemeRecalculation)
{
    const base::FilePath database_path = temp_dir_.GetPath().Append(kCookieFilename);
    const base::Time now = base::Time::Now();
    std::vector<CanonicalCookie> cookies;

    cookies.push_back(*CanonicalCookie::CreateUnsafeCookieForTesting("secure_true", "A", "example.com", "/", now, now, now, now,
        /*secure=*/true, /*httponly=*/false, CookieSameSite::UNSPECIFIED, COOKIE_PRIORITY_DEFAULT, std::optional<CookiePartitionKey>(),
        CookieSourceScheme::kUnset));

    cookies.push_back(*CanonicalCookie::CreateUnsafeCookieForTesting("secure_false", "B", "example.com", "/", now, now, now, now,
        /*secure=*/false, /*httponly=*/false, CookieSameSite::UNSPECIFIED, COOKIE_PRIORITY_DEFAULT, std::optional<CookiePartitionKey>(),
        CookieSourceScheme::kUnset));
    ...
    for (const auto& cookie : cookies) {
        sql::Statement verify_stmt(connection.GetUniqueStatement("SELECT source_scheme FROM cookies WHERE is_secure=?"));
        ...
        EXPECT_TRUE(verify_stmt.Step());
        EXPECT_EQ(static_cast<int>(cookie.SecureAttribute() ? CookieSourceScheme::kSecure : CookieSourceScheme::kUnset), verify_stmt.ColumnInt(0));
        // Confirm that exactly one cookie matches the SQL query
        EXPECT_FALSE(verify_stmt.Step());
    }
}

class SQLitePersistentCookieStoreTest_OriginBoundCookies : public SQLitePersistentCookieStoreTest {
public:
    ...
    void InitializeTest()
    {
        ...
        basic_cookie_ = CanonicalCookie::CreateForTesting(basic_url_, "a=b; max-age=100000", /*creation_time=*/base::Time::Now());
...
// distinct.
TEST_F(SQLitePersistentCookieStoreTest_OriginBoundCookies, UniquenessConstraint)
{
    InitializeTest();
    ...
    auto basic_cookie2 = CanonicalCookie::CreateForTesting(basic_url_, "a=b2; max-age=100000",
    ...
    EXPECT_THAT(cookies_,
        testing::UnorderedElementsAre(MatchesEveryCookieField(*basic_cookie_), MatchesEveryCookieField(*http_cookie_),
            MatchesEveryCookieField(*port_444_cookie_), MatchesEveryCookieField(*http_444_cookie_)));
}
...
TEST_F(SQLitePersistentCookieStoreTest_OriginBoundCookies, DeleteCookie)
{
    ...
    EXPECT_THAT(cookies_,
        testing::UnorderedElementsAre(
            MatchesEveryCookieField(*basic_cookie_), MatchesEveryCookieField(*http_cookie_), MatchesEveryCookieField(*port_444_cookie_)));
}

// Tests that updating a cookie correctly takes the scheme and port into
// account.
TEST_F(SQLitePersistentCookieStoreTest_OriginBoundCookies, UpdateCookieAccessTime)
{
    InitializeTest();
    ...
    EXPECT_THAT(cookies_,
        testing::UnorderedElementsAre(MatchesCookieKeyAndLastAccessDate(basic_cookie_->StrictlyUniqueKey(), basic_last_access),
            MatchesCookieKeyAndLastAccessDate(http_cookie_->StrictlyUniqueKey(), http_last_access),
            MatchesCookieKeyAndLastAccessDate(port_444_cookie_->StrictlyUniqueKey(), port_444_last_access),
            MatchesCookieKeyAndLastAccessDate(http_444_cookie_->StrictlyUniqueKey(), new_last_access)));
}
...
TEST_F(SQLitePersistentCookieStoreTest, SavingPartitionedCookies)
{
    ...
    store_->AddCookie(*CanonicalCookie::CreateUnsafeCookieForTesting("__Host-foo", "bar", GURL("https://example.com/").host(), "/",
        /*creation=*/base::Time::Now(),
        /*expiration=*/base::Time::Now() + base::Days(1),
        /*last_access=*/base::Time::Now(),
        /*last_update=*/base::Time::Now(), /*secure=*/true, /*httponly=*/false, CookieSameSite::UNSPECIFIED, COOKIE_PRIORITY_DEFAULT,
...
TEST_F(SQLitePersistentCookieStoreTest, LoadingPartitionedCookies)
{
    ...
    EXPECT_EQ(CookiePartitionKey::FromURLForTesting(GURL("https://toplevelsite.com")), cc->PartitionKey());
    EXPECT_EQ(last_update, cc->LastUpdateDate());
}

std::unique_ptr<CanonicalCookie> CreatePartitionedCookie(const std::string& name, const std::string& domain, const std::string& top_frame_site_key,
    CookiePartitionKey::AncestorChainBit ancestor_chain_bit, CookieSourceScheme scheme = CookieSourceScheme::kUnset, bool partitioned_cookies_enabled = true)
{
    const base::Time now = base::Time::Now();

    return CanonicalCookie::CreateUnsafeCookieForTesting(name, "B", domain, "/", now, now, now, now, /*secure=*/true,
        /*httponly=*/false, CookieSameSite::UNSPECIFIED, COOKIE_PRIORITY_DEFAULT,
        partitioned_cookies_enabled ? CookiePartitionKey::FromURLForTesting(GURL(top_frame_site_key), ancestor_chain_bit) :
...
std::vector<std::pair<CanonicalCookie, std::string>> GenerateHasCrossSiteAncestorCookiesAndVals(bool migrating = false)
{
    ...
    results.emplace_back(*CreatePartitionedCookie("A", default_domain, "https://www.example.com", CookiePartitionKey::AncestorChainBit::kSameSite), "0");
    if (!migrating) {
        // Key and domain are the same site but with kCrossSite
        results.emplace_back(*CreatePartitionedCookie("B", default_domain, "https://www.example.com", CookiePartitionKey::AncestorChainBit::kCrossSite), "1");
    }
    // Key and domain are different
    results.emplace_back(*CreatePartitionedCookie("C", default_domain, "https://www.toplevelsite.com", CookiePartitionKey::AncestorChainBit::kCrossSite), "1");
    // Domain is a substring
    results.emplace_back(*CreatePartitionedCookie("D", "ample.com", "https://www.example.com", CookiePartitionKey::AncestorChainBit::kCrossSite), "1");
    // http check kNonSecure scheme match.
    results.emplace_back(*CreatePartitionedCookie("E", default_domain, "http://www.example.com", CookiePartitionKey::AncestorChainBit::kSameSite), "0");

    return results;
}

TEST_F(SQLitePersistentCookieStoreTest, UpgradeToSchemaVersion23_AddingHasCrossSiteAncestor)
{
    const base::FilePath database_path = temp_dir_.GetPath().Append(kCookieFilename);

    std::vector<std::pair<CanonicalCookie, std::string>> cookies_and_expected_values = GenerateHasCrossSiteAncestorCookiesAndVals(true);
    ...
    for (const auto& cookie_pair : cookies_and_expected_values) {
        ...
        sql::Statement verify_stmt(connection.GetUniqueStatement("SELECT name FROM cookies WHERE host_key=?"
...
TEST_F(SQLitePersistentCookieStoreTest, TestValueOfHasCrossSiteAncestorOnDoCommit)
{
    InitializeStore(/*crypt=*/false, /*restore_old_session_cookies=*/false);

    std::vector<std::pair<CanonicalCookie, std::string>> cookies_and_expected_values = GenerateHasCrossSiteAncestorCookiesAndVals();
    for (const auto& cookie_pair : cookies_and_expected_values) {
...
TEST_F(SQLitePersistentCookieStoreTest, NoCryptoForDecryption)
{
    ...
    {
        base::HistogramTester histogram_tester;
        const auto cookies = CreateAndLoad(/*crypt=*/false, /*restore_old_session_cookies=*/false);
...
class SQLitePersistentCookieStoreTestWithDropDupDataFeature : public ::testing::WithParamInterface<
                                                                  /*features::kEncryptedAndPlaintextValuesAreInvalid*/ bool>,
                                                              public SQLitePersistentCookieStoreTest {
public:
    void SetUp() override
    {
        features_.InitWithFeatureState(features::kEncryptedAndPlaintextValuesAreInvalid, IsDroppingCookiesEnabled());
        SQLitePersistentCookieStoreTest::SetUp();
    }

protected:
    bool IsDroppingCookiesEnabled() const
    {
        return GetParam();
    }
...
// is dropped.
TEST_P(SQLitePersistentCookieStoreTestWithDropDupDataFeature, OverridePlaintextValue)
{
    {
        CreateAndLoad(/*crypt_cookies=*/true,
            /*restore_old_session_cookies=*/false);
    ...
    }
    {
        ...
        ASSERT_TRUE(connection.Execute("UPDATE cookies SET encrypted_value=x'', "
        ...
        ASSERT_TRUE(connection.Execute("UPDATE cookies SET value='not-F' WHERE host_key='example3.com'"));
        ASSERT_TRUE(transaction.Commit());
    }
    {
        base::HistogramTester histogram_tester;
        auto cookies = CreateAndLoad(/*crypt_cookies=*/true,
            /*restore_old_session_cookies=*/false);
        histogram_tester.ExpectBucketCount("Cookie.EncryptedAndPlaintextValues", true, 1);
...
INSTANTIATE_TEST_SUITE_P(
    , SQLitePersistentCookieStoreTestWithDropDupDataFeature, ::testing::Bool(), [](auto& info) { return info.param ? "Enabled" : "Disabled"; });
