...
base::TaskPriority COMPONENT_EXPORT(NET_EXTRAS) GetCookieStoreBackgroundSequencePriority();

// Implements the PersistentCookieStore interface in terms of a SQLite database.
// For documentation about the actual member functions consult the documentation
// of the parent class |CookieMonster::PersistentCookieStore|.
class COMPONENT_EXPORT(NET_EXTRAS) SQLitePersistentCookieStore : public CookieMonster::PersistentCookieStore {
public:
    ...
    SQLitePersistentCookieStore(const base::FilePath& path, const scoped_refptr<base::SequencedTaskRunner>& client_task_runner,
    ...
    SQLitePersistentCookieStore& operator=(const SQLitePersistentCookieStore&) = delete;
    ...
    void LoadCookiesForKey(const std::string& key, LoadedCallback callback) override;
...
private:
    ...
    void CompleteLoad(LoadedCallback callback, std::vector<std::unique_ptr<CanonicalCookie>> cookie_list);
    void CompleteKeyedLoad(const std::string& key, LoadedCallback callback, std::vector<std::unique_ptr<CanonicalCookie>> cookie_list);
