...
base::Value::Dict CookieKeyedLoadNetLogParams(const std::string& key, net::NetLogCaptureMode capture_mode)
{
...
void HistogramCookieAge(const net::CanonicalCookie& cookie)
{
    if (cookie.IsPersistent()) {
        ...
        if (!cookie.LastUpdateDate().is_null() && cookie.SourceType() == net::CookieSourceType::kScript) {
            const int script_cookie_age_since_last_update_in_hours = (Time::Now() - cookie.LastUpdateDate()).InHours();
            if (script_cookie_age_since_last_update_in_hours > kHoursInOneWeek) {
                UMA_HISTOGRAM_CUSTOM_COUNTS(
                    "Cookie.ScriptAgeSinceLastUpdateInHoursGTOneWeek", script_cookie_age_since_last_update_in_hours, kHoursInOneWeek + 1, kHoursInOneYear, 100);
            } else {
                UMA_HISTOGRAM_CUSTOM_COUNTS(
                    "Cookie.ScriptAgeSinceLastUpdateInHoursLTEOneWeek", script_cookie_age_since_last_update_in_hours, 1, kHoursInOneWeek + 1, 100);
            }
        }
    } else {
        ...
        if (!cookie.CreationDate().is_null()) {
            const int session_cookie_age_in_hours = (Time::Now() - cookie.CreationDate()).InHours();
            if (session_cookie_age_in_hours > kHoursInOneWeek) {
                UMA_HISTOGRAM_CUSTOM_COUNTS("Cookie.SessionAgeInHoursGTOneWeek2", session_cookie_age_in_hours, kHoursInOneWeek + 1, kHoursInOneYear, 100);
            } else {
                UMA_HISTOGRAM_CUSTOM_COUNTS("Cookie.SessionAgeInHoursLTEOneWeek2", session_cookie_age_in_hours, 1, kHoursInOneWeek + 1, 100);
            }
        }
        ...
        if (!cookie.LastUpdateDate().is_null()) {
            const int session_cookie_age_since_last_update_in_hours = (Time::Now() - cookie.LastUpdateDate()).InHours();
            if (session_cookie_age_since_last_update_in_hours > kHoursInOneWeek) {
                UMA_HISTOGRAM_CUSTOM_COUNTS("Cookie.SessionAgeSinceLastUpdateInHoursGTOneWeek", session_cookie_age_since_last_update_in_hours,
                    kHoursInOneWeek + 1, kHoursInOneYear, 100);
            } else {
                UMA_HISTOGRAM_CUSTOM_COUNTS(
                    "Cookie.SessionAgeSinceLastUpdateInHoursLTEOneWeek", session_cookie_age_since_last_update_in_hours, 1, kHoursInOneWeek + 1, 100);
            }
        }
    }
}
...
class SQLitePersistentCookieStore::Backend : public SQLitePersistentStoreBackendBase {
public:
    Backend(const base::FilePath& path, scoped_refptr<base::SequencedTaskRunner> client_task_runner,
        scoped_refptr<base::SequencedTaskRunner> background_task_runner, bool restore_old_session_cookies,
        std::unique_ptr<CookieCryptoDelegate> crypto_delegate, bool enable_exclusive_access)
        : SQLitePersistentStoreBackendBase(path,
            /* histogram_tag = */ "Cookie", kCurrentVersionNumber, kCompatibleVersionNumber, std::move(background_task_runner), std::move(client_task_runner),
    ...
    void LoadCookiesForKey(base::optional_ref<const std::string> key, LoadedCallback loaded_callback);
    ...
    bool MakeCookiesFromSQLStatement(
        std::vector<std::unique_ptr<CanonicalCookie>>& cookies, sql::Statement& statement, std::unordered_set<std::string>& top_frame_site_keys_to_delete);
...
private:
    ...
    public:
        ...
        const CanonicalCookie& cc() const
        {
            return cc_;
        }
...
private:
    ...
    void LoadAndNotifyInBackground(base::optional_ref<const std::string> key, LoadedCallback loaded_callback);
    ...
    void NotifyLoadCompleteInForeground(LoadedCallback loaded_callback, bool load_success);

    // Called from Load when crypto gets obtained.
    void CryptoHasInitFromLoad(base::optional_ref<const std::string> key, LoadedCallback loaded_callback);
    ...
    void DeleteTopFrameSiteKeys(const std::unordered_set<std::string>& top_frame_site_keys);

    // Batch a cookie operation (add or delete)
    void BatchOperation(PendingOperation::OperationType op, const CanonicalCookie& cc);
    ...
    typedef std::map<CanonicalCookie::StrictlyUniqueCookieKey, PendingOperationsForKey> PendingOperationsMap;
...
DBCookieSourceType CookieSourceTypeToDBCookieSourceType(CookieSourceType value)
{
    switch (value) {
...
CookieSourceType DBCookieSourceTypeToCookieSourceType(DBCookieSourceType value)
{
    switch (value) {
...
CookieSourceScheme DBToCookieSourceScheme(int value)
{
    ...
    if (value < 0 || value > enum_max_value) {
        DLOG(WARNING) << "DB read of cookie's source scheme is invalid. Resetting "
                         "value to unset.";
        value = static_cast<int>(CookieSourceScheme::kUnset); // Reset value to a known, useful, state.
    }
...
bool CreateV20Schema(sql::Database* db)
{
    ...
    static constexpr char kCreateTableQuery[] = "CREATE TABLE cookies("
    ...
    static constexpr char kCreateIndexQuery[] = "CREATE UNIQUE INDEX cookies_unique_index "
...
void SQLitePersistentCookieStore::Backend::Load(LoadedCallback loaded_callback)
{
    LoadCookiesForKey(std::nullopt, std::move(loaded_callback));
}

void SQLitePersistentCookieStore::Backend::LoadCookiesForKey(base::optional_ref<const std::string> key, LoadedCallback loaded_callback)
{
    if (crypto_) {
        crypto_->Init(base::BindOnce(&Backend::CryptoHasInitFromLoad, this, key.CopyAsOptional(), std::move(loaded_callback)));
    } else {
        CryptoHasInitFromLoad(key, std::move(loaded_callback));
    }
}

void SQLitePersistentCookieStore::Backend::CryptoHasInitFromLoad(base::optional_ref<const std::string> key, LoadedCallback loaded_callback)
{
    PostBackgroundTask(FROM_HERE, base::BindOnce(&Backend::LoadAndNotifyInBackground, this, key.CopyAsOptional(), std::move(loaded_callback)));
}

void SQLitePersistentCookieStore::Backend::LoadAndNotifyInBackground(base::optional_ref<const std::string> key, LoadedCallback loaded_callback)
{
...
void SQLitePersistentCookieStore::Backend::NotifyLoadCompleteInForeground(LoadedCallback loaded_callback, bool load_success)
{
...
bool SQLitePersistentCookieStore::Backend::DoInitializeDatabase()
{
    ...
    sql::Statement smt(db()->GetUniqueStatement("SELECT DISTINCT host_key FROM cookies"));
...
void SQLitePersistentCookieStore::Backend::ChainLoadCookies(LoadedCallback loaded_callback)
{
    DCHECK(background_task_runner()->RunsTasksInCurrentSequence());
    ...
    if (load_success && keys_to_load_.size() > 0) {
        bool success = background_task_runner()->PostTask(FROM_HERE, base::BindOnce(&Backend::ChainLoadCookies, this, std::move(loaded_callback)));
        if (!success) {
            LOG(WARNING) << "Failed to post task from " << FROM_HERE.ToString() << " to background_task_runner().";
        }
    } else {
...
bool SQLitePersistentCookieStore::Backend::LoadCookiesForDomains(const std::set<std::string>& domains)
{
    DCHECK(background_task_runner()->RunsTasksInCurrentSequence());
    ...
    delete_statement.Assign(db()->GetCachedStatement(SQL_FROM_HERE, "DELETE FROM cookies WHERE host_key = ?"));
    ...
    for (; it != domains.end() && ok; ++it) {
        smt.BindString(0, *it);
        ok = MakeCookiesFromSQLStatement(cookies, smt, top_frame_site_keys_to_delete);
        smt.Reset(true);
    }
...
void SQLitePersistentCookieStore::Backend::DeleteTopFrameSiteKeys(const std::unordered_set<std::string>& top_frame_site_keys)
{
    if (top_frame_site_keys.empty())
        return;

    sql::Statement delete_statement;
    delete_statement.Assign(db()->GetCachedStatement(SQL_FROM_HERE, "DELETE FROM cookies WHERE top_frame_site_key = ?"));
    ...
    std::vector<std::unique_ptr<CanonicalCookie>>& cookies, sql::Statement& statement, std::unordered_set<std::string>& top_frame_site_keys_to_delete)
{
    ...
    while (statement.Step()) {
        ...
        const bool encrypted_and_plaintext_values = !value.empty() && !encrypted_value.empty();
        UMA_HISTOGRAM_BOOLEAN("Cookie.EncryptedAndPlaintextValues", encrypted_and_plaintext_values);

        // Ensure feature is fully activated for all users who load cookies, before
        // checking the validity of the row.
        if (base::FeatureList::IsEnabled(features::kEncryptedAndPlaintextValuesAreInvalid)) {
            if (encrypted_and_plaintext_values) {
                RecordCookieLoadProblem(CookieLoadProblem::kValuesExistInBothEncryptedAndPlaintext);
                ok = false;
                continue;
            }
        }
        ...
        if (!encrypted_value.empty()) {
            ...
            if (!base::StartsWith(value, correct_hash, base::CompareCase::SENSITIVE)) {
        ...
        base::expected<std::optional<CookiePartitionKey>, std::string> partition_key
            = CookiePartitionKey::FromStorage(statement.ColumnString(2), statement.ColumnBool(19));
        ...
        std::unique_ptr<net::CanonicalCookie> cc = CanonicalCookie::FromStorage(
            ...
            /*same_site=*/
            DBCookieSameSiteToCookieSameSite(static_cast<DBCookieSameSite>(statement.ColumnInt(14))), //
            /*priority=*/
            DBCookiePriorityToCookiePriority(static_cast<DBCookiePriority>(statement.ColumnInt(12))), //
            ...
            /*source_type=*/
            DBCookieSourceTypeToCookieSourceType(static_cast<DBCookieSourceType>(statement.ColumnInt(18)))); //
        if (cc) {
            DLOG_IF(WARNING, cc->CreationDate() > Time::Now()) << "CreationDate too recent";
            if (!cc->LastUpdateDate().is_null()) {
                DLOG_IF(WARNING, cc->LastUpdateDate() > Time::Now()) << "LastUpdateDate too recent";
                ...
                UMA_HISTOGRAM_CUSTOM_COUNTS("Cookie.DaysSinceRefreshForRetrieval", (base::Time::Now() - cc->LastUpdateDate()).InDays(), 1, 400, 100);
            }
...
std::optional<int> SQLitePersistentCookieStore::Backend::DoMigrateDatabaseSchema()
{
    ...
    if (cur_version == 18) {
        ...
        sql::Statement update_statement(db()->GetCachedStatement(SQL_FROM_HERE,
    ...
    if (cur_version == 19) {
        ...
        static constexpr char insert_cookies_sql[] = "INSERT OR REPLACE INTO cookies "
    ...
    */
    ...
    if (cur_version == 23) {
        ...
        if (crypto_) {
            ...
            select_smt.Assign(db()->GetCachedStatement(SQL_FROM_HERE, "SELECT rowid, host_key, encrypted_value, value FROM cookies"));

            update_smt.Assign(db()->GetCachedStatement(SQL_FROM_HERE,
            ...
            while (select_smt.Step()) {
                ...
                if (!crypto_->EncryptString(base::StrCat({ crypto::SHA256HashString(domain), decrypted_value }), &new_encrypted_value)) {
...
void SQLitePersistentCookieStore::Backend::AddCookie(const CanonicalCookie& cc)
{
    BatchOperation(PendingOperation::COOKIE_ADD, cc);
}

void SQLitePersistentCookieStore::Backend::UpdateCookieAccessTime(const CanonicalCookie& cc)
{
    BatchOperation(PendingOperation::COOKIE_UPDATEACCESS, cc);
}

void SQLitePersistentCookieStore::Backend::DeleteCookie(const CanonicalCookie& cc)
{
    BatchOperation(PendingOperation::COOKIE_DELETE, cc);
}

void SQLitePersistentCookieStore::Backend::BatchOperation(PendingOperation::OperationType op, const CanonicalCookie& cc)
{
    ...
    PendingOperationsMap::size_type num_pending;
    {
        ...
        if (!iter_and_result.second) {
            ...
            if (po->op() == PendingOperation::COOKIE_DELETE) {
                ...
                if (!ops_for_key.empty() && ops_for_key.back()->op() == PendingOperation::COOKIE_UPDATEACCESS) {
    ...
    if (num_pending == 1) {
        ...
        if (!background_task_runner()->PostDelayedTask(FROM_HERE, base::BindOnce(&Backend::Commit, this), kCommitInterval)) {
...
void SQLitePersistentCookieStore::Backend::DoCommit()
{
    ...
    sql::Statement delete_statement(db()->GetCachedStatement(SQL_FROM_HERE,
        ...
        for (std::unique_ptr<PendingOperation>& po_entry : kv.second) {
            ...
            case PendingOperation::COOKIE_ADD:
                ...
                if (crypto_) {
                    ...
                    if (!crypto_->EncryptString(base::StrCat({ crypto::SHA256HashString(po->cc().Domain()), po->cc().Value() }), &encrypted_value)) {
                ...
                add_statement.BindInt(13, CookiePriorityToDBCookiePriority(po->cc().Priority()));
                add_statement.BindInt(14, CookieSameSiteToDBCookieSameSite(po->cc().SameSite()));
                ...
                add_statement.BindInt(18, CookieSourceTypeToDBCookieSourceType(po->cc().SourceType()));
                add_statement.BindBool(19, serialized_partition_key->has_cross_site_ancestor());
            ...
            case PendingOperation::COOKIE_UPDATEACCESS:
                ...
                update_access_statement.BindString(3, serialized_partition_key->TopLevelSite());
                update_access_statement.BindString(4, po->cc().Path());
                update_access_statement.BindInt(5, static_cast<int>(po->cc().SourceScheme()));
                update_access_statement.BindInt(6, po->cc().SourcePort());
                update_access_statement.BindBool(7, serialized_partition_key->has_cross_site_ancestor());
                if (!update_access_statement.Run()) {
                    DLOG(WARNING) << "Could not update cookie last access time in the DB.";
            ...
            case PendingOperation::COOKIE_DELETE:
                ...
                delete_statement.BindString(2, serialized_partition_key->TopLevelSite());
                delete_statement.BindString(3, po->cc().Path());
                delete_statement.BindInt(4, static_cast<int>(po->cc().SourceScheme()));
                delete_statement.BindInt(5, po->cc().SourcePort());
                delete_statement.BindBool(6, serialized_partition_key->has_cross_site_ancestor());
                if (!delete_statement.Run()) {
...
void SQLitePersistentCookieStore::Backend::DeleteAllInList(const std::list<CookieOrigin>& cookies)
{
    if (cookies.empty())
        return;
    ...
    if (background_task_runner()->RunsTasksInCurrentSequence()) {
        ...
        PostBackgroundTask(FROM_HERE, base::BindOnce(&Backend::BackgroundDeleteAllInList, this, cookies));
    }
}
...
// WHERE clause.
void SQLitePersistentCookieStore::Backend::BackgroundDeleteAllInList(const std::list<CookieOrigin>& cookies)
{
    DCHECK(background_task_runner()->RunsTasksInCurrentSequence());
    ...
    sql::Statement delete_statement(db()->GetCachedStatement(SQL_FROM_HERE, "DELETE FROM cookies WHERE host_key=? AND is_secure=?"));
    if (!delete_statement.is_valid()) {
...
void SQLitePersistentCookieStore::Backend::FinishedLoadingCookies(LoadedCallback loaded_callback, bool success)
{
    PostClientTask(FROM_HERE, base::BindOnce(&Backend::NotifyLoadCompleteInForeground, this, std::move(loaded_callback), success));
}

SQLitePersistentCookieStore::SQLitePersistentCookieStore(const base::FilePath& path, const scoped_refptr<base::SequencedTaskRunner>& client_task_runner,
    const scoped_refptr<base::SequencedTaskRunner>& background_task_runner, bool restore_old_session_cookies,
    std::unique_ptr<CookieCryptoDelegate> crypto_delegate, bool enable_exclusive_access)
    : backend_(base::MakeRefCounted<Backend>(
        path, client_task_runner, background_task_runner, restore_old_session_cookies, std::move(crypto_delegate), enable_exclusive_access))
{
}

void SQLitePersistentCookieStore::DeleteAllInList(const std::list<CookieOrigin>& cookies)
{
    backend_->DeleteAllInList(cookies);
}

void SQLitePersistentCookieStore::Load(LoadedCallback loaded_callback, const NetLogWithSource& net_log)
{
    ...
    backend_->Load(base::BindOnce(&SQLitePersistentCookieStore::CompleteLoad, this, std::move(loaded_callback)));
}

void SQLitePersistentCookieStore::LoadCookiesForKey(const std::string& key, LoadedCallback loaded_callback)
{
    DCHECK(!loaded_callback.is_null());
    net_log_.AddEvent(NetLogEventType::COOKIE_PERSISTENT_STORE_KEY_LOAD_STARTED,
        [&](NetLogCaptureMode capture_mode) { return CookieKeyedLoadNetLogParams(key, capture_mode); });
    // Note that |backend_| keeps |this| alive by keeping a reference count.
    // If this class is ever converted over to a WeakPtr<> pattern (as TODO it
    // should be) this will need to be replaced by a more complex pattern that
    // guarantees |loaded_callback| being called even if the class has been
    // destroyed. |backend_| needs to outlive |this| to commit changes to disk.
    backend_->LoadCookiesForKey(key, base::BindOnce(&SQLitePersistentCookieStore::CompleteKeyedLoad, this, key, std::move(loaded_callback)));
}
...
void SQLitePersistentCookieStore::UpdateCookieAccessTime(const CanonicalCookie& cc)
{
...
void SQLitePersistentCookieStore::SetBeforeCommitCallback(base::RepeatingClosure callback)
{
...
{
    net_log_.AddEventWithStringParams(NetLogEventType::COOKIE_PERSISTENT_STORE_CLOSED, "type", "SQLitePersistentCookieStore");
    backend_->Close();
}

void SQLitePersistentCookieStore::CompleteLoad(LoadedCallback callback, std::vector<std::unique_ptr<CanonicalCookie>> cookie_list)
{
    net_log_.EndEvent(NetLogEventType::COOKIE_PERSISTENT_STORE_LOAD);
    std::move(callback).Run(std::move(cookie_list));
}

void SQLitePersistentCookieStore::CompleteKeyedLoad(const std::string& key, LoadedCallback callback, std::vector<std::unique_ptr<CanonicalCookie>> cookie_list)
{
    net_log_.AddEventWithStringParams(NetLogEventType::COOKIE_PERSISTENT_STORE_KEY_LOAD_COMPLETED, "domain", key);
    std::move(callback).Run(std::move(cookie_list));
}
