...
SQLitePersistentStoreBackendBase::SQLitePersistentStoreBackendBase(const base::FilePath& path, const std::string& histogram_tag,
    const int current_version_number, const int compatible_version_number, scoped_refptr<base::SequencedTaskRunner> background_task_runner,
    scoped_refptr<base::SequencedTaskRunner> client_task_runner, bool enable_exclusive_access)
    : path_(path)
...
void SQLitePersistentStoreBackendBase::Flush(base::OnceClosure callback)
{
    ...
    PostBackgroundTask(FROM_HERE, base::BindOnce(&SQLitePersistentStoreBackendBase::FlushAndNotifyInBackground, this, std::move(callback)));
}
...
void SQLitePersistentStoreBackendBase::Close()
{
    if (background_task_runner_->RunsTasksInCurrentSequence()) {
        ...
        PostBackgroundTask(FROM_HERE, base::BindOnce(&SQLitePersistentStoreBackendBase::DoCloseInBackground, this));
    }
}

void SQLitePersistentStoreBackendBase::SetBeforeCommitCallback(base::RepeatingClosure callback)
{
...
bool SQLitePersistentStoreBackendBase::InitializeDatabase()
{
    ...
    db_ = std::make_unique<sql::Database>(sql::DatabaseOptions { .exclusive_locking = false, .exclusive_database_file_lock = enable_exclusive_access_ });
    ...
    db_->set_error_callback(base::BindRepeating(&SQLitePersistentStoreBackendBase::DatabaseErrorCallback, base::Unretained(this)));
    ...
    if (enable_exclusive_access_) {
        ...
        if (base::PathExists(path_)) {
            ...
            // option.
            base::PreReadFile(path_, /*is_executable=*/false, /*sequential=*/false, kPreReadSize);
        }
    }
    ...
    if (!MigrateDatabaseSchema() || !CreateDatabaseSchema()) {
        DLOG(ERROR) << "Unable to update or initialize " << histogram_tag_ << " DB tables.";
        RecordDBMigrationProblem();
        Reset();
        return false;
    }

    base::UmaHistogramCustomTimes(histogram_tag_ + ".TimeInitializeDB", timer.Elapsed(), base::Milliseconds(1), base::Minutes(1), 50);
...
void SQLitePersistentStoreBackendBase::PostBackgroundTask(const base::Location& origin, base::OnceClosure task)
{
    if (!background_task_runner_->PostTask(origin, std::move(task))) {
        LOG(WARNING) << "Failed to post task from " << origin.ToString() << " to background_task_runner_.";
    }
}

void SQLitePersistentStoreBackendBase::PostClientTask(const base::Location& origin, base::OnceClosure task)
{
    if (!client_task_runner_->PostTask(origin, std::move(task))) {
        LOG(WARNING) << "Failed to post task from " << origin.ToString() << " to client_task_runner_.";
    }
}
...
{
    // Version check.
    if (!meta_table_.Init(db_.get(), current_version_number_, compatible_version_number_)) {
        return false;
    }
    ...
    if (cur_version.value() < current_version_number_) {
        ...
        bool recovered = sql::Database::Delete(path_) && db()->Open(path_) && meta_table_.Init(db(), current_version_number_, compatible_version_number_);
        base::UmaHistogramBoolean(histogram_tag_ + ".CorruptMetaTableRecovered", recovered);
        if (!recovered) {
...
void SQLitePersistentStoreBackendBase::FlushAndNotifyInBackground(base::OnceClosure callback)
{
    DCHECK(background_task_runner_->RunsTasksInCurrentSequence());
...
void SQLitePersistentStoreBackendBase::DatabaseErrorCallback(int error, sql::Statement* stmt)
{
    DCHECK(background_task_runner_->RunsTasksInCurrentSequence());
...
#if BUILDFLAG(IS_WIN)
        base::UmaHistogramSparse(histogram_tag_ + ".WinGetLastErrorInitializeDB", ::GetLastError());
#endif // BUILDFLAG(IS_WIN)
    }
    ...
    PostBackgroundTask(FROM_HERE, base::BindOnce(&SQLitePersistentStoreBackendBase::KillDatabase, this));
}
