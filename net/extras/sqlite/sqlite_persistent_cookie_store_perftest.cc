...
static_assert(kRandomSeed > 10 * kNumDomains, "kRandomSeed not high enough for number of domains");
static_assert(kRandomSeed > 10 * kCookiesPerDomain, "kRandomSeed not high enough for number of cookies per domain");
...
public:
    SQLitePersistentCookieStorePerfTest()
        ...
        , key_loaded_event_(base::WaitableEvent::ResetPolicy::AUTOMATIC, base::WaitableEvent::InitialState::NOT_SIGNALED)
    {
    }
    ...
    void Load()
    {
        store_->Load(base::BindOnce(&SQLitePersistentCookieStorePerfTest::OnLoaded, base::Unretained(this)), NetLogWithSource());
        loaded_event_.Wait();
    }

    CanonicalCookie CookieFromIndices(int domain_num, int cookie_num)
    {
        base::Time t(test_start_ + base::Microseconds(domain_num * kCookiesPerDomain + cookie_num));
        std::string domain_name(base::StringPrintf(".domain_%d.com", domain_num));
        return *CanonicalCookie::CreateUnsafeCookieForTesting(base::StringPrintf("Cookie_%d", cookie_num), "1", domain_name, "/", t, t, t, t, false, false,
    ...
    void SetUp() override
    {
        ...
        store_ = base::MakeRefCounted<SQLitePersistentCookieStore>(temp_dir_.GetPath().Append(cookie_filename), client_task_runner_, background_task_runner_,
...
TEST_F(SQLitePersistentCookieStorePerfTest, TestLoadForKeyPerformance)
{
    ...
    for (int domain_num = 0; domain_num < 3; ++domain_num) {
        ...
        store_->LoadCookiesForKey(domain_name, base::BindOnce(&SQLitePersistentCookieStorePerfTest::OnKeyLoaded, base::Unretained(this)));
