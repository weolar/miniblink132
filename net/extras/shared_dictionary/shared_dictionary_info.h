...
public:
    SharedDictionaryInfo(const GURL& url, base::Time last_fetch_time, base::Time response_time, base::TimeDelta expiration, const std::string& match,
        const std::string& match_dest_string, const std::string& id, base::Time last_used_time, size_t size, const net::SHA256HashValue& hash,
        const base::UnguessableToken& disk_cache_key_token, const std::optional<int64_t>& primary_key_in_database);
    ...
    base::Time last_fetch_time() const
    {
    ...
    base::TimeDelta expiration() const
    {
        return expiration_;
    }
    ...
    const std::string& match_dest_string() const
    {
        return match_dest_string_;
    }
    const std::string& id() const
    {
        return id_;
    }
    base::Time last_used_time() const
    {
    ...
    const net::SHA256HashValue& hash() const
    {
        return hash_;
    }
