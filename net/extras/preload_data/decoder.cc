...
PreloadDecoder::HuffmanDecoder::HuffmanDecoder(const uint8_t* tree, size_t tree_bytes)
    : tree_(tree)
    , tree_bytes_(tree_bytes)
{
}

bool PreloadDecoder::HuffmanDecoder::Decode(PreloadDecoder::BitReader* reader, char* out) const
{
...
PreloadDecoder::PreloadDecoder(const uint8_t* huffman_tree, size_t huffman_tree_size, const uint8_t* trie, size_t trie_bits, size_t trie_root_position)
    ...
    *out_found = false;
    ...
    for (;;) {
        ...
        for (;;) {
            ...
            if (c == kEndOfString) {
                if (!ReadEntry(&bit_reader_, search, current_search_offset, out_found)) {
                    return false;
                }
                if (current_search_offset == 0) {
                    CHECK(*out_found);
            ...
            if (is_first_offset) {
                ...
                uint32_t jump_delta;
                if (!bit_reader_.Read(5, &jump_delta_bits) || !bit_reader_.Read(jump_delta_bits, &jump_delta)) {
                    return false;
                }
                ...
                if (!is_long_jump) {
                    ...
                    uint32_t jump_delta_bits;
                    if (!bit_reader_.Read(4, &jump_delta_bits) || !bit_reader_.Read(jump_delta_bits + 8, &jump_delta)) {
                        return false;
                    }
                }
