...
static constexpr SHA256HashValue kTestHash = { { 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10, 0x11, 0x12,
...
static constexpr unsigned char kWrongBrotliSignature[] = { 0xf0, 0x44, 0x43, 0x42 };
static constexpr unsigned char kBrotliSignatureAndHash[] = {
    // kBrotliSignature
    0xff, 0x44, 0x43, 0x42,
    // kTestHash
    0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1a,
    0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x20
};
static constexpr base::span<const unsigned char> kTooSmallBrotliHeader = base::make_span(kBrotliSignatureAndHash).subspan(sizeof(kBrotliSignatureAndHash) / 2);

static constexpr unsigned char kZstdSignature[] = { 0x5e, 0x2a, 0x4d, 0x18, 0x20, 0x00, 0x00, 0x00 };
// The first byte is different from the correct signature.
static constexpr unsigned char kWrongZstdSignature[] = { 0x50, 0x2a, 0x4d, 0x18, 0x20, 0x00, 0x00, 0x00 };
...
static constexpr base::span<const unsigned char> kTooSmallZstdHeader = base::span(kZstdSignatureAndHash).subspan(sizeof(kZstdSignatureAndHash) / 2u);
...
class SharedDictionaryHeaderCheckerSourceStreamTest : public ::testing::TestWithParam<Type> {
public:
    ...
    SharedDictionaryHeaderCheckerSourceStreamTest(const SharedDictionaryHeaderCheckerSourceStreamTest&) = delete;
    SharedDictionaryHeaderCheckerSourceStreamTest& operator=(const SharedDictionaryHeaderCheckerSourceStreamTest&) = delete;
...
protected:
    ...
    {
        stream_ = std::make_unique<SharedDictionaryHeaderCheckerSourceStream>(std::move(mock_stream_), GetType(), kTestHash);
    }
    SharedDictionaryHeaderCheckerSourceStream* stream()
    {
        return stream_.get();
    }
    ...
    {
        mock_stream_ptr_->CompleteNextRead();
    }
...
INSTANTIATE_TEST_SUITE_P(All, SharedDictionaryHeaderCheckerSourceStreamTest,
    testing::ValuesIn({ Type::kDictionaryCompressedBrotli, Type::kDictionaryCompressedZstd }),
    [](const testing::TestParamInfo<Type>& info) { return ToString(info.param); });
...
TEST_P(SharedDictionaryHeaderCheckerSourceStreamTest, Description)
{
    ...
    EXPECT_EQ(stream()->Description(), "SharedDictionaryHeaderCheckerSourceStream");
}
...
TEST_P(SharedDictionaryHeaderCheckerSourceStreamTest, EmptyStreamAsyncCompleteBeforeRead)
{
...
TEST_P(SharedDictionaryHeaderCheckerSourceStreamTest, EmptyStreamAsyncCompleteAfterRead)
{
    AddReadResult(OK, Mode::ASYNC);
    CreateHeaderCheckerSourceStream();
    CompleteNextRead();
...
TEST_P(SharedDictionaryHeaderCheckerSourceStreamTest, TooSmallHeaderSyncDataSyncComplete)
{
...
TEST_P(SharedDictionaryHeaderCheckerSourceStreamTest, TooSmallHeaderSyncDataAsyncCompleteBeforeRead)
{
    AddReadResult(GetTooSmallHeader(), Mode::SYNC);
    AddReadResult(OK, Mode::ASYNC);
    CreateHeaderCheckerSourceStream();
...
TEST_P(SharedDictionaryHeaderCheckerSourceStreamTest, TooSmallHeaderSyncDataAsyncCompleteAfterRead)
{
    AddReadResult(GetTooSmallHeader(), Mode::SYNC);
    AddReadResult(OK, Mode::ASYNC);
    CreateHeaderCheckerSourceStream();
...
TEST_P(SharedDictionaryHeaderCheckerSourceStreamTest, HeaderSync)
{
    ...
    EXPECT_EQ(base::as_chars(buffer()->span()).first(kTestBodyData.size()), kTestBodyData);
    CheckSyncRead(OK);
}
