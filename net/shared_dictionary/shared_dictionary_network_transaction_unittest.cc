...
const std::string kTestDictionarySha256 = "c19728aed36503cfc81a0f5359e6f472e121f77bf20a2faac7994191293c0623";
// The Structured Field sf-binary hash of sha256 of `kTestDictionaryData`.
const std::string kTestDictionarySha256Base64 = ":wZcortNlA8/IGg9TWeb0cuEh93vyCi+qx5lBkSk8BiM=:";
const std::string kTestData = "HelloこんにちはHallo你好HelloこんにちはHallo你好";
...
// $ xxd -i /tmp/out.dcb
const uint8_t kBrotliEncodedData[] = { 0xff, 0x44, 0x43, 0x42, 0xc1, 0x97, 0x28, 0xae, 0xd3, 0x65, 0x03, 0xcf, 0xc8, 0x1a, 0x0f, 0x53, 0x59, 0xe6, 0xf4, 0x72,
    0xe1, 0x21, 0xf7, 0x7b, 0xf2, 0x0a, 0x2f, 0xaa, 0xc7, 0x99, 0x41, 0x91, 0x29, 0x3c, 0x06, 0x23, 0xa1, 0xe8, 0x01, 0x00, 0x22, 0x8d, 0x54, 0xc6, 0xf6, 0x26,
    0x81, 0x69, 0x46, 0x9d, 0xb2, 0x60, 0x0e, 0x6b, 0xf5, 0x07, 0x02 };
const std::string kBrotliEncodedDataString = std::string(reinterpret_cast<const char*>(kBrotliEncodedData), sizeof(kBrotliEncodedData));
...
// $ xxd -i /tmp/out.dcz
const uint8_t kZstdEncodedData[] = { 0x5e, 0x2a, 0x4d, 0x18, 0x20, 0x00, 0x00, 0x00, 0xc1, 0x97, 0x28, 0xae, 0xd3, 0x65, 0x03, 0xcf, 0xc8, 0x1a, 0x0f, 0x53,
    0x59, 0xe6, 0xf4, 0x72, 0xe1, 0x21, 0xf7, 0x7b, 0xf2, 0x0a, 0x2f, 0xaa, 0xc7, 0x99, 0x41, 0x91, 0x29, 0x3c, 0x06, 0x23, 0x28, 0xb5, 0x2f, 0xfd, 0x24, 0x3e,
    0x85, 0x00, 0x00, 0x28, 0x48, 0x65, 0x6c, 0x6c, 0x6f, 0x03, 0x00, 0x42, 0x35, 0x88, 0x6a, 0x03, 0x87, 0x4c, 0x2d, 0xcd, 0x1e, 0xde, 0x25 };
const std::string kZstdEncodedDataString = std::string(reinterpret_cast<const char*>(kZstdEncodedData), sizeof(kZstdEncodedData));

const size_t kDefaultBufferSize = 1023;

class DummySyncDictionary : public SharedDictionary {
public:
    explicit DummySyncDictionary(const std::string& data_string, const std::string& id = "")
        : data_(base::MakeRefCounted<StringIOBuffer>(data_string))
        , size_(data_string.size())
        , id_(id)
    {
        std::unique_ptr<crypto::SecureHash> secure_hash = crypto::SecureHash::Create(crypto::SecureHash::SHA256);
    ...
    scoped_refptr<IOBuffer> data() const override
    {
        return data_;
    }
    ...
    const SHA256HashValue& hash() const override
    {
        return hash_;
    }
...
{
    return TransportInfo(TransportType::kDirect, IPEndPoint(IPAddress::IPv4Localhost(), 80),
        /*accept_ch_frame_arg=*/"",
        /*cert_is_issued_by_known_root=*/false, kProtoHTTP2);
}

static void BrotliTestTransactionHandler(
    const HttpRequestInfo* request, std::string* response_status, std::string* response_headers, std::string* response_data)
{
    EXPECT_THAT(request->extra_headers.GetHeader(shared_dictionary::kAvailableDictionaryHeaderName), testing::Optional(kTestDictionarySha256Base64));
    *response_data = kBrotliEncodedDataString;
}

static void ZstdTestTransactionHandler(const HttpRequestInfo* request, std::string* response_status, std::string* response_headers, std::string* response_data)
{
    EXPECT_THAT(request->extra_headers.GetHeader(shared_dictionary::kAvailableDictionaryHeaderName), testing::Optional(kTestDictionarySha256Base64));
...
static const auto kTestTransactionHandlerWithoutAvailableDictionary
    = base::BindRepeating([](const HttpRequestInfo* request, std::string* response_status, std::string* response_headers, std::string* response_data) {
          EXPECT_FALSE(request->extra_headers.HasHeader(shared_dictionary::kAvailableDictionaryHeaderName));
...
public:
        ...
        , network_layer_(std::make_unique<MockNetworkLayer>())
    {
        scoped_feature_list_.InitWithFeatures(
            /*enabled_features=*/ {},
            /*disabled_features=*/ { features::kCompressionDictionaryTransportRequireKnownRootCert });
    }
    ~SharedDictionaryNetworkTransactionTest() override = default;

    SharedDictionaryNetworkTransactionTest(const SharedDictionaryNetworkTransactionTest&) = delete;
    SharedDictionaryNetworkTransactionTest& operator=(const SharedDictionaryNetworkTransactionTest&) = delete;
...
TEST_F(SharedDictionaryNetworkTransactionTest, SyncDictionary)
{
    ...
    request.dictionary_getter
        = base::BindRepeating([](const std::optional<SharedDictionaryIsolationKey>& isolation_key, const GURL& request_url) -> scoped_refptr<SharedDictionary> {
    ...
    transaction.SetIsSharedDictionaryReadAllowedCallback(base::BindRepeating([]() { return true; }));

    TestCompletionCallback start_callback;
    ASSERT_THAT(transaction.Start(&request, start_callback.callback(), NetLogWithSource()), test::IsError(ERR_IO_PENDING));
    EXPECT_THAT(start_callback.WaitForResult(), test::IsError(OK));

    scoped_refptr<IOBufferWithSize> buf = base::MakeRefCounted<IOBufferWithSize>(kDefaultBufferSize);
    TestCompletionCallback read_callback;
    ASSERT_THAT(transaction.Read(buf.get(), buf->size(), read_callback.callback()), test::IsError(ERR_IO_PENDING));
...
TEST_F(SharedDictionaryNetworkTransactionTest, NotAllowedToUseDictionary)
{
    ...
    // header.
    scoped_mock_transaction_->handler = kTestTransactionHandlerWithoutAvailableDictionary;
    ...
    SharedDictionaryNetworkTransaction transaction(CreateNetworkTransaction(),
        /*enable_shared_zstd=*/false);
    transaction.SetIsSharedDictionaryReadAllowedCallback(base::BindRepeating([]() { return false; }));
...
TEST_F(SharedDictionaryNetworkTransactionTest, DictionaryId)
{
    ...
    scoped_mock_transaction_->handler
        = base::BindRepeating([](const HttpRequestInfo* request, std::string* response_status, std::string* response_headers, std::string* response_data) {
              EXPECT_THAT(request->extra_headers.GetHeader("dictionary-id"), testing::Optional(std::string("\"test-id\"")));
              *response_data = kBrotliEncodedDataString;
          });
    ...
    request.dictionary_getter
        = base::BindRepeating([](const std::optional<SharedDictionaryIsolationKey>& isolation_key, const GURL& request_url) -> scoped_refptr<SharedDictionary> {
              return base::MakeRefCounted<DummySyncDictionary>(kTestDictionaryData, "test-id");
          });
    SharedDictionaryNetworkTransaction transaction(CreateNetworkTransaction(),
        /*enable_shared_zstd=*/false);
    transaction.SetIsSharedDictionaryReadAllowedCallback(base::BindRepeating([]() { return true; }));
...
TEST_F(SharedDictionaryNetworkTransactionTest, DictionaryIdWithBackSlashAndDquote)
{
    // Change MockTransaction to check the dictionary-id header
    scoped_mock_transaction_->handler
        = base::BindRepeating([](const HttpRequestInfo* request, std::string* response_status, std::string* response_headers, std::string* response_data) {
              EXPECT_THAT(request->extra_headers.GetHeader("dictionary-id"), testing::Optional(std::string("\"test\\\\dictionary\\\"id\"")));
              *response_data = kBrotliEncodedDataString;
          });
    ...
    request.dictionary_getter
        = base::BindRepeating([](const std::optional<SharedDictionaryIsolationKey>& isolation_key, const GURL& request_url) -> scoped_refptr<SharedDictionary> {
              return base::MakeRefCounted<DummySyncDictionary>(kTestDictionaryData, "test\\dictionary\"id");
          });
    SharedDictionaryNetworkTransaction transaction(CreateNetworkTransaction(),
        /*enable_shared_zstd=*/false);
    transaction.SetIsSharedDictionaryReadAllowedCallback(base::BindRepeating([]() { return true; }));
...
TEST_F(SharedDictionaryNetworkTransactionTest, RequireKnownRootCertCheckFailure)
{
    base::test::ScopedFeatureList scoped_feature_list;
    scoped_feature_list.InitAndEnableFeature(features::kCompressionDictionaryTransportRequireKnownRootCert);
    // Change MockTransaction to check that there is no available-dictionary
    // header.
    scoped_mock_transaction_->handler = kTestTransactionHandlerWithoutAvailableDictionary;
...
TEST_F(SharedDictionaryNetworkTransactionTest, RequireKnownRootCertCheckSuccess)
{
    base::test::ScopedFeatureList scoped_feature_list;
    scoped_feature_list.InitAndEnableFeature(features::kCompressionDictionaryTransportRequireKnownRootCert);
...
TEST_F(SharedDictionaryNetworkTransactionTest, RequireKnownRootCertCheckSuccessForLocalhost)
{
    base::test::ScopedFeatureList scoped_feature_list;
    scoped_feature_list.InitAndEnableFeature(features::kCompressionDictionaryTransportRequireKnownRootCert);
    ...
    ScopedMockTransaction scoped_mock_transaction(kBrotliDictionaryTestTransaction, "http:///localhost:1234/test");
...
TEST_F(SharedDictionaryNetworkTransactionTest, NoMatchingDictionary)
{
    ...
    request.dictionary_getter = base::BindRepeating(
        [](const std::optional<SharedDictionaryIsolationKey>& isolation_key, const GURL& request_url) -> scoped_refptr<SharedDictionary> { return nullptr; });
    SharedDictionaryNetworkTransaction transaction(CreateNetworkTransaction(),
        /*enable_shared_zstd=*/false);
    transaction.SetIsSharedDictionaryReadAllowedCallback(base::BindRepeating([]() { return true; }));
...
TEST_F(SharedDictionaryNetworkTransactionTest, WithoutValidLoadFlag)
{
    ...
    request.dictionary_getter = base::BindRepeating(
        [](bool* getter_called, const std::optional<SharedDictionaryIsolationKey>& isolation_key, const GURL& request_url) -> scoped_refptr<SharedDictionary> {
            *getter_called = true;
            return nullptr;
        },
...
TEST_F(SharedDictionaryNetworkTransactionTest, WrongContentDictionaryHeader)
{
    ...
    TestCompletionCallback start_callback;
    ASSERT_THAT(start_callback.GetResult(transaction.Start(&request, start_callback.callback(), NetLogWithSource())), test::IsError(OK));
    scoped_refptr<IOBufferWithSize> buf = base::MakeRefCounted<IOBufferWithSize>(kDefaultBufferSize);
    TestCompletionCallback read_callback;
    ASSERT_THAT(
        read_callback.GetResult(transaction.Read(buf.get(), buf->size(), read_callback.callback())), test::IsError(ERR_UNEXPECTED_CONTENT_DICTIONARY_HEADER));
}
TEST_F(SharedDictionaryNetworkTransactionTest, MultipleContentEncodingWithSbr)
{
    ...
    scoped_mock_transaction_->response_headers = "content-encoding: dcb, deflate\n";
...
TEST_F(SharedDictionaryNetworkTransactionTest, AsyncDictionarySuccessBeforeStartReading)
{
    scoped_refptr<DummyAsyncDictionary> dictionary = base::MakeRefCounted<DummyAsyncDictionary>(kTestDictionaryData);
    DummyAsyncDictionary* dictionary_ptr = dictionary.get();

    MockHttpRequest request(kBrotliDictionaryTestTransaction);
    request.dictionary_getter = base::BindRepeating(
        [](scoped_refptr<DummyAsyncDictionary>* dictionary, const std::optional<SharedDictionaryIsolationKey>& isolation_key,
    ...
    base::OnceCallback<void(int)> dictionary_read_all_callback = dictionary_ptr->TakeReadAllCallback();
...
TEST_F(SharedDictionaryNetworkTransactionTest, AsyncDictionarySuccessAfterStartReading)
{
    scoped_refptr<DummyAsyncDictionary> dictionary = base::MakeRefCounted<DummyAsyncDictionary>(kTestDictionaryData);
    DummyAsyncDictionary* dictionary_ptr = dictionary.get();
...
TEST_F(SharedDictionaryNetworkTransactionTest, AsyncDictionarySuccessAfterTransactionDestroy)
{
    scoped_refptr<DummyAsyncDictionary> dictionary = base::MakeRefCounted<DummyAsyncDictionary>(kTestDictionaryData);
    DummyAsyncDictionary* dictionary_ptr = dictionary.get();
    ...
    std::unique_ptr<SharedDictionaryNetworkTransaction> transaction
        = std::make_unique<SharedDictionaryNetworkTransaction>(CreateNetworkTransaction(), /*enable_shared_zstd=*/false);
    transaction->SetIsSharedDictionaryReadAllowedCallback(base::BindRepeating([]() { return true; }));
...
TEST_F(SharedDictionaryNetworkTransactionTest, AsyncDictionaryFailureBeforeStartReading)
{
    scoped_refptr<DummyAsyncDictionary> dictionary = base::MakeRefCounted<DummyAsyncDictionary>(kTestDictionaryData);
    DummyAsyncDictionary* dictionary_ptr = dictionary.get();
    ...
    scoped_refptr<IOBufferWithSize> buf = base::MakeRefCounted<IOBufferWithSize>(kDefaultBufferSize);
    TestCompletionCallback read_callback;
    ASSERT_THAT(transaction.Read(buf.get(), buf->size(), read_callback.callback()), test::IsError(ERR_DICTIONARY_LOAD_FAILED));
}

TEST_F(SharedDictionaryNetworkTransactionTest, AsyncDictionaryFailureAfterStartReading)
{
    scoped_refptr<DummyAsyncDictionary> dictionary = base::MakeRefCounted<DummyAsyncDictionary>(kTestDictionaryData);
    DummyAsyncDictionary* dictionary_ptr = dictionary.get();
...
TEST_F(SharedDictionaryNetworkTransactionTest, Restart)
{
...

    {
        ...
        ASSERT_THAT(transaction.RestartIgnoringLastError(restart_callback.callback()), test::IsError(ERR_FAILED));
    }
    {
        TestCompletionCallback restart_callback;
        ASSERT_THAT(transaction.RestartWithCertificate(
                        /*client_cert=*/nullptr,
    ...
    {
        TestCompletionCallback restart_callback;
        ASSERT_THAT(transaction.RestartWithAuth(AuthCredentials(), restart_callback.callback()), test::IsError(ERR_FAILED));
    }
...
TEST_F(SharedDictionaryNetworkTransactionTest, GetLoadState)
{
    ...
    scoped_refptr<IOBufferWithSize> buf = base::MakeRefCounted<IOBufferWithSize>(1);
    TestCompletionCallback read_callback;
    ASSERT_THAT(transaction.Read(buf.get(), buf->size(), read_callback.callback()), test::IsError(ERR_IO_PENDING));
    int read_result = read_callback.WaitForResult();
...
TEST_F(SharedDictionaryNetworkTransactionTest, SharedZstd)
{
    ...
    ASSERT_THAT(transaction.Read(buf.get(), buf->size(), read_callback.callback()), test::IsError(ERR_CONTENT_DECODING_FAILED));
...
class SharedDictionaryNetworkTransactionProtocolCheckTest
    : public SharedDictionaryNetworkTransactionTest,
      public testing::WithParamInterface<
          std::tuple<ProtocolCheckHttp1TestCase, ProtocolCheckHttp2TestCase, ProtocolCheckProtocolTestCase, ProtocolCheckHostTestCase>> {
public:
    SharedDictionaryNetworkTransactionProtocolCheckTest()
    {
        ...
        if (AllowHttp1()) {
            enabled_features.push_back(features::kCompressionDictionaryTransportOverHttp1);
        } else {
            disabled_features.push_back(features::kCompressionDictionaryTransportOverHttp1);
        }
        if (AllowHttp2()) {
            enabled_features.push_back(features::kCompressionDictionaryTransportOverHttp2);
        } else {
            disabled_features.push_back(features::kCompressionDictionaryTransportOverHttp2);
        }
        scoped_feature_list_.InitWithFeatures(enabled_features, disabled_features);
    }
    SharedDictionaryNetworkTransactionProtocolCheckTest(const SharedDictionaryNetworkTransactionProtocolCheckTest&) = delete;
    SharedDictionaryNetworkTransactionProtocolCheckTest& operator=(const SharedDictionaryNetworkTransactionProtocolCheckTest&) = delete;
...
protected:
    MockTransaction CreateMockTransaction()
    {
        ...
        if (!ShuoldUseDictionary()) {
            // Change MockTransaction to check that there is no available-dictionary
            // header.
            mock_transaction.handler = kTestTransactionHandlerWithoutAvailableDictionary;
        }
        if (IsHttp2()) {
...
INSTANTIATE_TEST_SUITE_P(All, SharedDictionaryNetworkTransactionProtocolCheckTest,
    ::testing::Combine(::testing::Values(ProtocolCheckHttp1TestCase::kAllowHttp1, ProtocolCheckHttp1TestCase::kDoNotAllowHttp1),
        ::testing::Values(ProtocolCheckHttp2TestCase::kAllowHttp2, ProtocolCheckHttp2TestCase::kDoNotAllowHttp2),
        ::testing::Values(ProtocolCheckProtocolTestCase::kHttp1, ProtocolCheckProtocolTestCase::kHttp2, ProtocolCheckProtocolTestCase::kHttp3),
        ::testing::Values(ProtocolCheckHostTestCase::kLocalHost, ProtocolCheckHostTestCase::kNonLocalhost)),
    [](const testing::TestParamInfo<
        std::tuple<ProtocolCheckHttp1TestCase, ProtocolCheckHttp2TestCase, ProtocolCheckProtocolTestCase, ProtocolCheckHostTestCase>>& info) {
        return ToString(std::get<0>(info.param)) + "_" + ToString(std::get<1>(info.param)) + "_" + ToString(std::get<2>(info.param)) + "_"
