...
public:
    ...
    TestNetworkChangeNotifierWin& operator=(const TestNetworkChangeNotifierWin&) = delete;
    ...
    void RecomputeCurrentConnectionTypeOnBlockingSequence(base::OnceCallback<void(ConnectionType)> reply_callback) const override
    {
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
            FROM_HERE, base::BindOnce(std::move(reply_callback), NetworkChangeNotifier::CONNECTION_UNKNOWN));
    }
...
public:
    ...
    void StartWatchingAndSucceed()
    {
        ...
        EXPECT_CALL(network_change_notifier_, WatchForAddressChangeInternal()).WillOnce(Return(true));
    ...
    void RetryAndSucceed()
    {
        ...
        EXPECT_CALL(test_ip_address_observer_, OnIPAddressChanged()).WillOnce(Invoke(&run_loop, &base::RunLoop::QuitWhenIdle));
        EXPECT_CALL(network_change_notifier_, WatchForAddressChangeInternal()).WillOnce(Return(true));
    ...
    void RetryAndFail()
    {
        ...
        EXPECT_FALSE(network_change_notifier_.is_watching());
        EXPECT_LT(initial_sequential_failures, network_change_notifier_.sequential_failures());
    ...
    NetworkChangeNotifier::ConnectionCost GetCurrentConnectionCostFromDefaultImplementationForTesting()
    {
        return network_change_notifier_.GetCurrentConnectionCostFromDefaultImplementationForTesting();
    }
...
{
    if (base::win::GetVersion() < NetworkCostChangeNotifierWin::kSupportedOsVersion) {
        GTEST_SKIP();
    }

    fake_network_cost_manager_environment_.SetCost(NetworkChangeNotifier::ConnectionCost::CONNECTION_COST_UNMETERED);

    // Wait for `NetworkCostChangeNotifierWin` to finish initializing.
    RunUntilIdle();

    EXPECT_EQ(GetCurrentConnectionCost(), NetworkChangeNotifier::ConnectionCost::CONNECTION_COST_UNMETERED);

    fake_network_cost_manager_environment_.SetCost(NetworkChangeNotifier::ConnectionCost::CONNECTION_COST_METERED);

    // Wait for `NetworkCostChangeNotifierWin` to handle the cost changed event.
    RunUntilIdle();

    EXPECT_EQ(GetCurrentConnectionCost(), NetworkChangeNotifier::ConnectionCost::CONNECTION_COST_METERED);
}
...
TEST_F(NetworkChangeNotifierWinTest, CostChangeObserver)
{
    ...
    EXPECT_EQ(cost_observer.last_cost_changed_input(), NetworkChangeNotifier::ConnectionCost::CONNECTION_COST_METERED);
...
class NetworkChangeNotifierWinCostErrorTest : public NetworkChangeNotifierWinTest {
    void SetUp() override
    {
        if (base::win::GetVersion() < NetworkCostChangeNotifierWin::kSupportedOsVersion) {
            GTEST_SKIP();
        ...
        fake_network_cost_manager_environment_.SimulateError(NetworkCostManagerStatus::kErrorGetCostFailed);
...
TEST_F(NetworkChangeNotifierWinCostErrorTest, CostError)
{
    ...
    EXPECT_EQ(GetCurrentConnectionCost(), GetCurrentConnectionCostFromDefaultImplementationForTesting());
}

// Override the Windows OS version to simulate running on an OS that does not
// support `INetworkCostManager`.
class NetworkChangeNotifierWinCostUnsupportedOsTest : public NetworkChangeNotifierWinTest {
public:
