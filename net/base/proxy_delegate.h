...
public:
    ...
    virtual void OnResolveProxy(const GURL& url, const NetworkAnonymizationKey& network_anonymization_key, const std::string& method,
    ...
    // failing proxy chain.
    virtual void OnSuccessfulRequestAfterFailures(const ProxyRetryInfoMap& proxy_retry_info) = 0;
    ...
    virtual Error OnBeforeTunnelRequest(const ProxyChain& proxy_chain, size_t chain_index, HttpRequestHeaders* extra_headers) = 0;
    ...
    // `response_headers`.
    virtual Error OnTunnelHeadersReceived(const ProxyChain& proxy_chain, size_t chain_index, const HttpResponseHeaders& response_headers) = 0;

    // Associates a `ProxyResolutionService` with this `ProxyDelegate`.
    // `proxy_resolution_service` must outlive `this`.
    virtual void SetProxyResolutionService(ProxyResolutionService* proxy_resolution_service) = 0;
};
