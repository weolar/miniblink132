...
private:
    ...
    static BackoffEntry::Policy PolicyFromProto(const fuzz_proto::BackoffEntryPolicy& policy)
    {
...
void TestDeserialize(const ProtoTranslator& translator)
{
    ...
    std::unique_ptr<BackoffEntry> entry = BackoffEntrySerializer::DeserializeFromList(value->GetList(), &policy, &clock, translator.parse_time());
    if (!entry)
        return;

    base::Value::List reserialized = BackoffEntrySerializer::SerializeToList(*entry, translator.parse_time());
    ...
    std::unique_ptr<BackoffEntry> entry_reparsed = BackoffEntrySerializer::DeserializeFromList(reserialized, &policy, &clock, translator.parse_time());
...
void TestSerialize(const ProtoTranslator& translator)
{
    ...
    base::Value::List serialized = BackoffEntrySerializer::SerializeToList(native_entry, translator.serialize_time());

    MockClock clock;
    clock.SetNow(translator.now_ticks());

    // Deserialize it.
    std::unique_ptr<BackoffEntry> deserialized_entry = BackoffEntrySerializer::DeserializeFromList(serialized, &policy, &clock, translator.parse_time());
...
DEFINE_PROTO_FUZZER(const fuzz_proto::FuzzerInput& input)
{
    ...
    if (translator.now_ticks().is_inf() || translator.parse_time().is_inf() || translator.serialize_time().is_inf()) {
        return;
    }
