...
public:
    ...
    static AddressList CreateFromIPAddress(const IPAddress& address, uint16_t port);

    static AddressList CreateFromIPAddressList(const IPAddressList& addresses, std::vector<std::string> aliases);
    ...
    {
        return std::tie(endpoints_, dns_aliases_) == std::tie(other.endpoints_, other.dns_aliases_);
    }
    bool operator!=(const AddressList& other) const
    {
        return !(*this == other);
    ...
    const std::vector<std::string>& dns_aliases() const
    {
        return dns_aliases_;
    }
    ...
    const std::vector<net::IPEndPoint>& endpoints() const
    {
        return endpoints_;
    }
