...
public:
    ...
    static IsolationInfo CreateForInternalRequest(const url::Origin& top_frame_origin);
    ...
    static IsolationInfo CreateTransientWithNonce(const base::UnguessableToken& nonce);
    ...
    static std::optional<IsolationInfo> Deserialize(const std::string& serialized);
    ...
    static IsolationInfo Create(RequestType request_type, const url::Origin& top_frame_origin, const url::Origin& frame_origin,
        const SiteForCookies& site_for_cookies, const std::optional<base::UnguessableToken>& nonce = std::nullopt);

    // TODO(crbug.com/344943210): Remove this and create a safer way to ensure
    // NIKs created from NAKs aren't used by accident.
    static IsolationInfo DoNotUseCreatePartialFromNak(const net::NetworkAnonymizationKey& network_anonymization_key);
    ...
    static std::optional<IsolationInfo> CreateIfConsistent(RequestType request_type, const std::optional<url::Origin>& top_frame_origin,
        const std::optional<url::Origin>& frame_origin, const SiteForCookies& site_for_cookies,
...
private:
    IsolationInfo(RequestType request_type, const std::optional<url::Origin>& top_frame_origin, const std::optional<url::Origin>& frame_origin,
    ...
    friend struct mojo::StructTraits<network::mojom::IsolationInfoDataView, IsolationInfo>;
};
