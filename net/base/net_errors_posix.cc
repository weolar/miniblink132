...
{
    if (os_error != 0)
        DVLOG(2) << "Error " << os_error << ": " << logging::SystemErrorCodeToString(os_error);
    ...
    case 0:
        return OK;
    default:
        LOG(WARNING) << "Unknown error " << base::safe_strerror(os_error) << " (" << os_error << ") mapped to net::ERR_FAILED";
        return ERR_FAILED;
    }
}
