...
TEST(NetworkInterfacesTest, GetNetworkList)
{
    ...
    for (auto it = list.begin(); it != list.end(); ++it) {
        ...
        EXPECT_TRUE(it->address.IsValid()) << "Invalid address of size " << it->address.size();
        ...
        NET_LUID luid;
        EXPECT_EQ(static_cast<DWORD>(NO_ERROR), ConvertInterfaceIndexToLuid(it->interface_index, &luid));
        GUID guid;
        EXPECT_EQ(static_cast<DWORD>(NO_ERROR), ConvertInterfaceLuidToGuid(&luid, &guid));
