...
class NET_EXPORT_PRIVATE NetworkChangeNotifierWin : public NetworkChangeNotifier, public base::win::ObjectWatcher::Delegate {
public:
...
protected:
    ...
    void set_is_watching(bool is_watching)
    {
        is_watching_ = is_watching;
    }
    int sequential_failures() const
    {
        return sequential_failures_;
    }
...
private:
    ...
    virtual void RecomputeCurrentConnectionTypeOnBlockingSequence(base::OnceCallback<void(ConnectionType)> reply_callback) const;
    ...
    std::atomic<NetworkChangeNotifier::ConnectionCost> last_computed_connection_cost_ = NetworkChangeNotifier::ConnectionCost::CONNECTION_COST_UNKNOWN;
