...
TEST_F(NetworkActivityMontiorTest, Threading)
{
    ...
    for (size_t i = 0; i < num_increments; ++i) {
        ...
        threads[thread_num]->task_runner()->PostTask(FROM_HERE, base::BindOnce(&activity_monitor::IncrementBytesReceived, bytes_received));
        threads[thread_num]->task_runner()->PostTask(FROM_HERE, base::BindOnce(&VerifyBytesReceivedIsMultipleOf, bytes_received));
    }

    threads.clear();

    EXPECT_EQ(num_increments * bytes_received, activity_monitor::GetBytesReceived());
}
