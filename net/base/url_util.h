...
NET_EXPORT GURL AppendQueryParameter(const GURL& url, std::string_view name, std::string_view value);
...
NET_EXPORT GURL AppendOrReplaceQueryParameter(const GURL& url, std::string_view name, std::optional<std::string_view> value);
...
NET_EXPORT GURL AppendOrReplaceRef(const GURL& url, const std::string_view& ref);
...
NET_EXPORT bool GetValueForKeyInQuery(const GURL& url, std::string_view search_key, std::string* out_value);
...
NET_EXPORT bool ParseHostAndPort(std::string_view input, std::string* host, int* port);
...
NET_EXPORT bool IsSubdomainOf(std::string_view subdomain, std::string_view superdomain);
...
NET_EXPORT std::string CanonicalizeHost(std::string_view host, url::CanonHostInfo* host_info);

// Canonicalizes |host| and returns it.  Also fills |host_info| with
// IP address information.  |host_info| must not be NULL.
// Canonicalization will follow the host parsing rules for a file URL.
NET_EXPORT std::string CanonicalizeFileHost(std::string_view host, url::CanonHostInfo* host_info);
...
NET_EXPORT_PRIVATE void GetIdentityFromURL(const GURL& url, std::u16string* username, std::u16string* password);
...
NET_EXPORT_PRIVATE std::string UnescapePercentEncodedUrl(std::string_view input);
