...
void CallClosureAfterCheckingResult(base::OnceClosure closure, bool* did_check_result, int result)
{
...
class ExampleEmployer::ExampleWorker : public base::RefCountedThreadSafe<ExampleWorker> {
public:
...
private:
    ...
    const scoped_refptr<base::SingleThreadTaskRunner> origin_task_runner_ = base::SingleThreadTaskRunner::GetCurrentDefault();
};
...
void ExampleEmployer::ExampleWorker::DoWork()
{
    ...
    origin_task_runner_->PostTask(FROM_HERE, base::BindOnce(&ExampleWorker::DoCallback, this));
}
...
bool ExampleEmployer::DoSomething(CompletionOnceCallback callback)
{
    ...
    if (!base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(&ExampleWorker::DoWork, request_))) {
        NOTREACHED();
    }
...
class TestCompletionCallbackTest : public PlatformTest, public WithTaskEnvironment { };
...
TEST_F(TestCompletionCallbackTest, Closure)
{
    ...
    CompletionOnceCallback completion_callback = base::BindOnce(&CallClosureAfterCheckingResult, closure.closure(), base::Unretained(&did_check_result));
