...
class UploadFileElementReaderTest : public testing::TestWithParam<bool>, public WithTaskEnvironment {
protected:
    void SetUp() override
    {
        // Some tests (*.ReadPartially) rely on bytes_.size() being even.
        bytes_.assign({ '1', '2', '3', '4', '5', '6', '7', '8', '9', 'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i' });

        ASSERT_TRUE(temp_dir_.CreateUniqueTempDir());

        ASSERT_TRUE(base::CreateTemporaryFileInDir(temp_dir_.GetPath(), &temp_file_path_));
        ASSERT_TRUE(base::WriteFile(temp_file_path_, std::string_view(bytes_.data(), bytes_.size())));

        reader_ = CreateReader(0, std::numeric_limits<uint64_t>::max(), base::Time());
    ...
    std::unique_ptr<UploadFileElementReader> CreateReader(int64_t offset, int64_t length, base::Time expected_modification_time)
    {
        if (GetParam()) {
            return std::make_unique<UploadFileElementReader>(
                base::SingleThreadTaskRunner::GetCurrentDefault().get(), temp_file_path_, offset, length, expected_modification_time);
        }

        // The base::File::FLAG_WIN_SHARE_DELETE lets the file be deleted without
        // the test fixture waiting on it to be closed.
        int open_flags = base::File::FLAG_OPEN | base::File::FLAG_READ | base::File::FLAG_WIN_SHARE_DELETE;
#if BUILDFLAG(IS_WIN)
        ...
        return std::make_unique<UploadFileElementReader>(base::SingleThreadTaskRunner::GetCurrentDefault().get(), std::move(file),
            // Use an incorrect path, to make sure that the file is never re-opened.
            base::FilePath(FILE_PATH_LITERAL("this_should_be_ignored")), offset, length, expected_modification_time);
    }
...
TEST_P(UploadFileElementReaderTest, ReadPartially)
{
    ...
    ASSERT_EQ(ERR_IO_PENDING, reader_->Read(wrapped_buffer.get(), buf.size(), read_callback1.callback()));
...
TEST_P(UploadFileElementReaderTest, ReadAll)
{
    ...
    ASSERT_EQ(ERR_IO_PENDING, reader_->Read(wrapped_buffer.get(), buf.size(), read_callback.callback()));
    EXPECT_EQ(static_cast<int>(buf.size()), read_callback.WaitForResult());
    EXPECT_EQ(0U, reader_->BytesRemaining());
    EXPECT_EQ(bytes_, buf);
    // Try to read again.
    EXPECT_EQ(0, reader_->Read(wrapped_buffer.get(), buf.size(), read_callback.callback()));
}
...
TEST_P(UploadFileElementReaderTest, InitDuringAsyncOperation)
{
    ...
    // Start reading all.
    TestCompletionCallback read_callback1;
    EXPECT_EQ(ERR_IO_PENDING, reader_->Read(wrapped_buffer.get(), buf.size(), read_callback1.callback()));
    ...
    EXPECT_THAT(reader_->Init(init_callback2.callback()), IsError(ERR_IO_PENDING));
    ...
    TestCompletionCallback read_callback2;
    EXPECT_EQ(ERR_IO_PENDING, reader_->Read(wrapped_buffer2.get(), buf2.size(), read_callback2.callback()));
    EXPECT_EQ(static_cast<int>(buf2.size()), read_callback2.WaitForResult());
    EXPECT_EQ(bytes_.size() - buf2.size(), reader_->BytesRemaining());
    EXPECT_EQ(std::vector<char>(bytes_.begin(), bytes_.begin() + buf2.size()), buf2);
...
TEST_P(UploadFileElementReaderTest, RepeatedInitDuringInit)
{
    ...
    EXPECT_THAT(reader_->Init(init_callback3.callback()), IsError(ERR_IO_PENDING));
    ...
    // Read all.
    TestCompletionCallback read_callback;
    int result = reader_->Read(wrapped_buffer.get(), buf.size(), read_callback.callback());
...
TEST_P(UploadFileElementReaderTest, Range)
{
    ...
    auto wrapped_buffer = base::MakeRefCounted<WrappedIOBuffer>(buf);
    TestCompletionCallback read_callback;
    ASSERT_EQ(ERR_IO_PENDING, reader_->Read(wrapped_buffer.get(), kLength, read_callback.callback()));
    EXPECT_EQ(static_cast<int>(kLength), read_callback.WaitForResult());
    const std::vector<char> expected(bytes_.begin() + kOffset, bytes_.begin() + kOffset + kLength);
...
TEST_P(UploadFileElementReaderTest, FileChanged)
{
    ...
    const base::Time expected_modification_time = info.last_modified - base::Seconds(1);
    reader_ = CreateReader(0, std::numeric_limits<uint64_t>::max(), expected_modification_time);
    TestCompletionCallback init_callback;
    ASSERT_THAT(reader_->Init(init_callback.callback()), IsError(ERR_IO_PENDING));
...
TEST_P(UploadFileElementReaderTest, InexactExpectedTimeStamp)
{
    ...
    const base::Time expected_modification_time = info.last_modified - base::Milliseconds(900);
    reader_ = CreateReader(0, std::numeric_limits<uint64_t>::max(), expected_modification_time);
    TestCompletionCallback init_callback;
    ASSERT_THAT(reader_->Init(init_callback.callback()), IsError(ERR_IO_PENDING));
    EXPECT_THAT(init_callback.WaitForResult(), IsOk());
...
TEST_P(UploadFileElementReaderTest, WrongPath)
{
    ...
    reader_ = std::make_unique<UploadFileElementReader>(
        base::SingleThreadTaskRunner::GetCurrentDefault().get(), wrong_path, 0, std::numeric_limits<uint64_t>::max(), base::Time());
    TestCompletionCallback init_callback;
    ASSERT_THAT(reader_->Init(init_callback.callback()), IsError(ERR_IO_PENDING));
    EXPECT_THAT(init_callback.WaitForResult(), IsError(ERR_FILE_NOT_FOUND));
}

INSTANTIATE_TEST_SUITE_P(All, UploadFileElementReaderTest, testing::ValuesIn({ false, true }));
