...
public:
    ...
    // `AddInterface`.
    std::optional<ChangeBits> AddInterfaces(std::vector<fuchsia::net::interfaces::Properties> interfaces);

    // Returns `std::nullopt` if `properties` is invalid or incomplete, or if the
    // interface already exists in the cache.
    std::optional<ChangeBits> AddInterface(fuchsia::net::interfaces::Properties properties);

    // Returns `std::nullopt` if `properties` is invalid or does not contain an
    // `id`, or if the interface does not exist in the cache.
    std::optional<ChangeBits> ChangeInterface(fuchsia::net::interfaces::Properties properties);

    // Returns `std::nullopt` if `interface_id` does not exist in the cache.
    std::optional<ChangeBits> RemoveInterface(InterfaceProperties::InterfaceId interface_id);
...
private:
    ...
    bool UpdateConnectionTypeWhileLocked() EXCLUSIVE_LOCKS_REQUIRED(lock_) VALID_CONTEXT_REQUIRED(sequence_checker_);

    std::optional<ChangeBits> AddInterfaceWhileLocked(fuchsia::net::interfaces::Properties properties) EXCLUSIVE_LOCKS_REQUIRED(lock_)
    ...
    base::flat_map<InterfaceProperties::InterfaceId, InterfaceProperties> interfaces_ GUARDED_BY(lock_);

    // The ConnectionType of the default network interface.
    NetworkChangeNotifier::ConnectionType connection_type_ GUARDED_BY(lock_) = NetworkChangeNotifier::CONNECTION_NONE;
