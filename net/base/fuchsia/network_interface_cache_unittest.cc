...
enum : InterfaceProperties::InterfaceId { kDefaultInterfaceId = 1, kSecondaryInterfaceId = 2 };
    ...
    fuchsia::hardware::network::PortClass device_class = fuchsia::hardware::network::PortClass::ETHERNET)
{
    ...
    properties.set_port_class(fuchsia::net::interfaces::PortClass::WithDevice(std::move(device_class)));
    properties.set_addresses(MakeSingleItemVec(InterfaceAddressFrom(kDefaultIPv4Address, kDefaultIPv4Prefix)));
    return properties;
}
...
TEST_F(NetworkInterfaceCacheTest, AddInterface)
{
    ...
    EXPECT_EQ(change_bits.value(), NetworkInterfaceCache::kIpAddressChanged | NetworkInterfaceCache::kConnectionTypeChanged);

    NetworkInterfaceList networks;
    EXPECT_TRUE(cache.GetOnlineInterfaces(&networks));
    EXPECT_EQ(networks.size(), 1u);

    EXPECT_EQ(cache.GetConnectionType(), NetworkChangeNotifier::CONNECTION_ETHERNET);
}
...
TEST_F(NetworkInterfaceCacheTest, ChangeInterface)
{
    ...
    fuchsia::net::interfaces::Properties properties;
    properties.set_id(kDefaultInterfaceId);
    properties.set_port_class(fuchsia::net::interfaces::PortClass::WithLoopback(fuchsia::net::interfaces::Empty()));
