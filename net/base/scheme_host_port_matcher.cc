...
SchemeHostPortMatcher::SchemeHostPortMatcher(SchemeHostPortMatcher&& rhs) = default;
SchemeHostPortMatcher& SchemeHostPortMatcher::operator=(SchemeHostPortMatcher&& rhs) = default;
SchemeHostPortMatcher::~SchemeHostPortMatcher() = default;
...
// static
SchemeHostPortMatcher SchemeHostPortMatcher::FromRawString(const std::string& raw)
{
    SchemeHostPortMatcher result;

    base::StringTokenizer entries(raw, kParseRuleListDelimiterList);
    while (entries.GetNext()) {
        auto rule = SchemeHostPortMatcherRule::FromUntrimmedRawString(entries.token_piece());
        if (rule) {
...
void SchemeHostPortMatcher::AddAsFirstRule(std::unique_ptr<SchemeHostPortMatcherRule> rule)
{
    DCHECK(rule);
    rules_.insert(rules_.begin(), std::move(rule));
}

void SchemeHostPortMatcher::AddAsLastRule(std::unique_ptr<SchemeHostPortMatcherRule> rule)
{
    DCHECK(rule);
    rules_.push_back(std::move(rule));
}

void SchemeHostPortMatcher::ReplaceRule(size_t index, std::unique_ptr<SchemeHostPortMatcherRule> rule)
{
...
SchemeHostPortMatcherResult SchemeHostPortMatcher::Evaluate(const GURL& url) const
{
