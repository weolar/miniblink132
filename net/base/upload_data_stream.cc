...
base::Value::Dict NetLogInitEndInfoParams(int result, int total_size, bool is_chunked)
{
...
UploadDataStream::UploadDataStream(bool is_chunked, bool has_null_source, int64_t identifier)
...
int UploadDataStream::Init(CompletionOnceCallback callback, const NetLogWithSource& net_log)
{
    Reset();
...
int UploadDataStream::Read(IOBuffer* buf, int buf_len, CompletionOnceCallback callback)
{
    DCHECK(!callback.is_null() || IsInMemory());
    DCHECK(initialized_successfully_);
    DCHECK_GT(buf_len, 0);

    net_log_.BeginEvent(NetLogEventType::UPLOAD_DATA_STREAM_READ, [&] { return CreateReadInfoParams(current_position_); });
...
void UploadDataStream::Reset()
{
    ...
    if (!callback_.is_null()) {
        if (!initialized_successfully_) {
            ...
            // initialization.
            net_log_.EndEventWithNetErrorCode(NetLogEventType::UPLOAD_DATA_STREAM_INIT, ERR_ABORTED);
        } else {
            // Otherwise, a read is being aborted.
            net_log_.EndEventWithNetErrorCode(NetLogEventType::UPLOAD_DATA_STREAM_READ, ERR_ABORTED);
        }
    }
...
const std::vector<std::unique_ptr<UploadElementReader>>* UploadDataStream::GetElementReaders() const
{
    return nullptr;
}
...
void UploadDataStream::OnInitCompleted(int result)
{
    ...
    net_log_.EndEvent(NetLogEventType::UPLOAD_DATA_STREAM_INIT, [&] { return NetLogInitEndInfoParams(result, total_size_, is_chunked_); });
...
void UploadDataStream::OnReadCompleted(int result)
{
    ...
    net_log_.EndEventWithNetErrorCode(NetLogEventType::UPLOAD_DATA_STREAM_READ, result);
