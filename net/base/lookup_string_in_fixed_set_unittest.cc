...
void RecursivelyEnumerateDafsaLanguage(const FixedSetIncrementalLookup& lookup, std::vector<char>* sequence, std::vector<std::string>* language)
{
    ...
    for (char c = std::numeric_limits<char>::min();; ++c) {
        ...
        if (continued_lookup.Advance(c)) {
            ...
            CHECK_LT(saved_language_size, language->size())
                << "DAFSA includes a branch to nowhere at node: " << std::string(sequence->begin(), sequence->end());
            sequence->pop_back();
        }
...
std::vector<std::string> EnumerateDafsaLanguage(base::span<const uint8_t> graph)
{
...
INSTANTIATE_TEST_SUITE_P(LookupStringInFixedSetTest, Dafsa1Test, ::testing::ValuesIn(kBasicTestCases));
...
INSTANTIATE_TEST_SUITE_P(LookupStringInFixedSetTest, Dafsa3Test, ::testing::ValuesIn(kTwoByteOffsetTestCases));
...
INSTANTIATE_TEST_SUITE_P(LookupStringInFixedSetTest, Dafsa4Test, ::testing::ValuesIn(kThreeByteOffsetTestCases));
...
INSTANTIATE_TEST_SUITE_P(LookupStringInFixedSetTest, Dafsa5Test, ::testing::ValuesIn(kJoinedPrefixesTestCases));
