...
enum PrivateRegistryFilter { EXCLUDE_PRIVATE_REGISTRIES = 0, INCLUDE_PRIVATE_REGISTRIES };
...
// foobar.baz).
enum UnknownRegistryFilter { EXCLUDE_UNKNOWN_REGISTRIES = 0, INCLUDE_UNKNOWN_REGISTRIES };
...
NET_EXPORT std::string GetDomainAndRegistry(const GURL& gurl, PrivateRegistryFilter filter);

// Like the GURL version, but takes an Origin. Returns an empty string if the
// Origin is opaque.
NET_EXPORT std::string GetDomainAndRegistry(const url::Origin& origin, PrivateRegistryFilter filter);
...
NET_EXPORT std::string GetDomainAndRegistry(std::string_view host, PrivateRegistryFilter filter);

// Same as above, but returns a StringPiece that is backed by the supplied
// url::Origin.
NET_EXPORT std::string_view GetDomainAndRegistryAsStringPiece(const url::Origin& origin, PrivateRegistryFilter filter);
...
NET_EXPORT bool SameDomainOrHost(const GURL& gurl1, const GURL& gurl2, PrivateRegistryFilter filter);
NET_EXPORT bool SameDomainOrHost(const url::Origin& origin1, const url::Origin& origin2, PrivateRegistryFilter filter);
// Note: this returns false if |origin2| is not set.
NET_EXPORT bool SameDomainOrHost(const url::Origin& origin1, const std::optional<url::Origin>& origin2, PrivateRegistryFilter filter);
NET_EXPORT bool SameDomainOrHost(const GURL& gurl, const url::Origin& origin, PrivateRegistryFilter filter);
...
//                                                         bar)
NET_EXPORT size_t GetRegistryLength(const GURL& gurl, UnknownRegistryFilter unknown_filter, PrivateRegistryFilter private_filter);
...
NET_EXPORT bool HostHasRegistryControlledDomain(std::string_view host, UnknownRegistryFilter unknown_filter, PrivateRegistryFilter private_filter);
...
NET_EXPORT bool HostIsRegistryIdentifier(std::string_view canon_host, PrivateRegistryFilter private_filter);
...
NET_EXPORT size_t GetCanonicalHostRegistryLength(std::string_view canon_host, UnknownRegistryFilter unknown_filter, PrivateRegistryFilter private_filter);
...
NET_EXPORT size_t PermissiveGetHostRegistryLength(std::string_view host, UnknownRegistryFilter unknown_filter, PrivateRegistryFilter private_filter);
NET_EXPORT size_t PermissiveGetHostRegistryLength(std::u16string_view host, UnknownRegistryFilter unknown_filter, PrivateRegistryFilter private_filter);
...
NET_EXPORT_PRIVATE void SetFindDomainGraphForTesting(base::span<const uint8_t> domains);
