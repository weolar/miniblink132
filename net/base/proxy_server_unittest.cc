...
TEST(ProxyServerTest, FromSchemeHostAndPort)
{
    ...
    for (size_t i = 0; i < std::size(tests); ++i) {
        ...
        auto proxy = ProxyServer::FromSchemeHostAndPort(tests[i].input_scheme, tests[i].input_host, tests[i].input_port);
        ...
        auto proxy_from_string_port = ProxyServer::FromSchemeHostAndPort(tests[i].input_scheme, tests[i].input_host, tests[i].input_port_str);
...
TEST(ProxyServerTest, InvalidHostname)
{
    ...
    for (size_t i = 0; i < std::size(tests); ++i) {
        ...
        auto proxy = ProxyServer::FromSchemeHostAndPort(ProxyServer::SCHEME_HTTP, tests[i], 80);
...
TEST(ProxyServerTest, InvalidPort)
{
    ...
    for (size_t i = 0; i < std::size(tests); ++i) {
        SCOPED_TRACE(base::NumberToString(i) + ": " + tests[i]);
        auto proxy = ProxyServer::FromSchemeHostAndPort(ProxyServer::SCHEME_HTTP, "foopy", tests[i]);
        EXPECT_FALSE(proxy.is_valid());
    }
}
...
TEST(ProxyServerTest, ComparatorAndEquality)
{
    const struct {
        ...
        { // Equal.
            ProxyUriToProxyServer("foo:11", ProxyServer::SCHEME_HTTP), ProxyUriToProxyServer("http://foo:11", ProxyServer::SCHEME_HTTP), 0 },
        { // Port is different.
            ProxyUriToProxyServer("foo:333", ProxyServer::SCHEME_HTTP), ProxyUriToProxyServer("foo:444", ProxyServer::SCHEME_HTTP), -1 },
        { // Host is different.
            ProxyUriToProxyServer("foo:33", ProxyServer::SCHEME_HTTP), ProxyUriToProxyServer("bar:33", ProxyServer::SCHEME_HTTP), 1 },
        { // Scheme is different.
            ProxyUriToProxyServer("socks4://foo:33", ProxyServer::SCHEME_HTTP), ProxyUriToProxyServer("http://foo:33", ProxyServer::SCHEME_HTTP), 1 },
    };
...
TEST(ProxyServerTest, Properties)
{
    // HTTP proxy.
    {
        auto proxy = ProxyServer::FromSchemeHostAndPort(ProxyServer::SCHEME_HTTP, "foo", std::nullopt);
        ASSERT_TRUE(proxy.is_valid());
    ...
    // HTTPS proxy.
    {
        auto proxy = ProxyServer::FromSchemeHostAndPort(ProxyServer::SCHEME_HTTPS, "foo", std::nullopt);
        ASSERT_TRUE(proxy.is_valid());
    ...
    // QUIC proxy.
    {
        auto proxy = ProxyServer::FromSchemeHostAndPort(ProxyServer::SCHEME_QUIC, "foo", std::nullopt);
        ASSERT_TRUE(proxy.is_valid());
        EXPECT_FALSE(proxy.is_http());
        EXPECT_FALSE(proxy.is_https());
        EXPECT_TRUE(proxy.is_http_like());
    ...
    // SOCKS5 proxy.
    {
        auto proxy = ProxyServer::FromSchemeHostAndPort(ProxyServer::SCHEME_SOCKS5, "foo", std::nullopt);
        ASSERT_TRUE(proxy.is_valid());
        EXPECT_FALSE(proxy.is_http());
        EXPECT_FALSE(proxy.is_https());
