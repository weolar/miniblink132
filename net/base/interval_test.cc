...
protected:
    ...
    void TestIntersect(const Interval<int64_t>& i1, const Interval<int64_t>& i2, bool changes_i1, bool changes_i2, const Interval<int64_t>& result)
    {
        Interval<int64_t> i;
        i.CopyFrom(i1);
...
TEST_F(IntervalTest, Length)
{
    ...
    const Interval<base::TimeDelta> empty4(base::TimeDelta() + base::Seconds(1), base::TimeDelta());
    const Interval<int> d1(1, 2);
    const Interval<int> d2(0, 50);
    const Interval<base::TimeDelta> d3(base::TimeDelta(), base::TimeDelta() + base::Seconds(1));
    const Interval<base::TimeDelta> d4(base::TimeDelta() + base::Hours(1), base::TimeDelta() + base::Minutes(90));
