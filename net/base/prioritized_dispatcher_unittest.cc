...
static_assert(MINIMUM_PRIORITY == 0u && MINIMUM_PRIORITY == THROTTLED && THROTTLED < IDLE && IDLE < LOWEST && LOWEST < HIGHEST && HIGHEST <= MAXIMUM_PRIORITY,
...
public:
    ...
    public:
        TestJob(PrioritizedDispatcher* dispatcher, char tag, Priority priority, std::string* log)
...
protected:
    ...
    {
        auto job = std::make_unique<TestJob>(dispatcher_.get(), data, priority, &log_);
        job->Add(false);
        return job;
    }
...
TEST_F(PrioritizedDispatcherTest, GetLimits)
{
    ...
    ASSERT_EQ(static_cast<size_t>(NUM_PRIORITIES), retrieved_limits.reserved_slots.size());
    for (size_t priority = MINIMUM_PRIORITY; priority <= MAXIMUM_PRIORITY; ++priority) {
        EXPECT_EQ(original_limits.reserved_slots[priority], retrieved_limits.reserved_slots[priority]);
    }
    ...
    ASSERT_EQ(static_cast<size_t>(NUM_PRIORITIES), retrieved_limits.reserved_slots.size());
    for (size_t priority = MINIMUM_PRIORITY; priority <= MAXIMUM_PRIORITY; ++priority) {
        EXPECT_EQ(new_limits.reserved_slots[priority], retrieved_limits.reserved_slots[priority]);
    }
}
