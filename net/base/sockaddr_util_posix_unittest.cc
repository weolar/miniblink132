...
TEST(FillUnixAddressTest, SimpleAddress)
{
    ...
    EXPECT_TRUE(FillUnixAddress(path, /*use_abstract_namespace=*/false, &storage));
    ...
    EXPECT_EQ(path.size() + 1U + offsetof(struct sockaddr_un, sun_path), (unsigned int)storage.addr_len);

    struct sockaddr_un* socket_addr = reinterpret_cast<struct sockaddr_un*>(storage.addr);
...
TEST(FillUnixAddressTest, PathEmpty)
{
    SockaddrStorage storage;
    std::string path = "";
    EXPECT_FALSE(FillUnixAddress(path, /*use_abstract_namespace=*/false, &storage));
}
...
TEST(FillUnixAddressTest, AbstractLinuxAddress)
{
    ...
    std::string unix_path(reinterpret_cast<char*>(socket_addr->sun_path + 1), path.size());
    EXPECT_EQ(unix_path, path);
#else
    // Other platforms don't support the abstract Linux namespace.
    EXPECT_FALSE(FillUnixAddress(path, /*use_abstract_namespace=*/true, &storage));
#endif
}
