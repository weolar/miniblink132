...
struct NET_EXPORT_PRIVATE ConnectionEndpointMetadata {
    ...
    ConnectionEndpointMetadata(std::vector<std::string> supported_protocol_alpns, EchConfigList ech_config_list, std::string target_name);
    ~ConnectionEndpointMetadata();

    ConnectionEndpointMetadata(const ConnectionEndpointMetadata&);
    ConnectionEndpointMetadata& operator=(const ConnectionEndpointMetadata&) = default;
    ...
    {
        return std::tie(supported_protocol_alpns, ech_config_list, target_name) == std::tie(other.supported_protocol_alpns, other.ech_config_list, target_name);
    }

    base::Value ToValue() const;
    static std::optional<ConnectionEndpointMetadata> FromValue(const base::Value& value);
