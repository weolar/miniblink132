...
public:
    ...
    bool GetPreferredExtensionForMimeType(const std::string& mime_type, base::FilePath::StringType* extension) const;

    bool MatchesMimeType(const std::string& mime_type_pattern, const std::string& mime_type) const;

    bool ParseMimeTypeWithoutParameter(base::StringPiece type_string, std::string* top_level_type, std::string* subtype) const;
...
template <size_t num_mappings> static const char* FindMimeType(const MimeInfo (&mappings)[num_mappings], const std::string& ext)
{
...
static base::FilePath::StringType StringToFilePathStringType(const base::StringPiece& string_piece)
{
#if BUILDFLAG(IS_WIN)
...
static bool FindPreferredExtension(const MimeInfo (&mappings)[num_mappings], const std::string& mime_type, base::FilePath::StringType* result)
{
...
bool MimeUtil::GetPreferredExtensionForMimeType(const std::string& mime_type, base::FilePath::StringType* extension) const
{
...
bool MatchesMimeTypeParameters(const std::string& mime_type_pattern, const std::string& mime_type)
{
...
bool MimeUtil::MatchesMimeType(const std::string& mime_type_pattern, const std::string& mime_type) const
{
...
bool ParseMimeType(const std::string& type_str, std::string* mime_type, base::StringPairs* params)
{
...
bool MimeUtil::ParseMimeTypeWithoutParameter(base::StringPiece type_string, std::string* top_level_type, std::string* subtype) const
{
    std::vector<base::StringPiece> components = base::SplitStringPiece(type_string, "/", base::KEEP_WHITESPACE, base::SPLIT_WANT_ALL);
...
bool ParseMimeTypeWithoutParameter(base::StringPiece type_string, std::string* top_level_type, std::string* subtype)
{
...
static const char* const kStandardImageTypes[] = { "image/avif", "image/bmp", "image/cis-cod", "image/gif", "image/ief", "image/jpeg", "image/jxl",
    "image/webp", "image/pict", "image/pipeg", "image/png", "image/svg+xml", "image/tiff", "image/vnd.microsoft.icon", "image/x-cmu-raster", "image/x-cmx",
    "image/x-icon", "image/x-portable-anymap", "image/x-portable-bitmap", "image/x-portable-graymap", "image/x-portable-pixmap", "image/x-rgb",
    ...
    base::span<const MimeInfo> mappings, const std::string& mime_type, bool prefix_match, std::unordered_set<base::FilePath::StringType>* extensions)
{
    for (const auto& mapping : mappings) {
        ...
        if (base::StartsWith(cur_mime_type, mime_type, base::CompareCase::INSENSITIVE_ASCII)
            ...
            for (const base::StringPiece& this_extension : base::SplitStringPiece(mapping.extensions, ",", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL)) {
...
// to fail: (),./:=+
const base::StringPiece kMimeBoundaryCharacters("0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ");
...
{
    return g_mime_util.Get().MatchesMimeType(std::string(mime_type_pattern), std::string(mime_type));
}
