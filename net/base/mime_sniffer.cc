...
template <int MagicSize, int MaskSize> class VerifySizes {
...
#define verified_sizeof(magic, mask) VerifySizes<sizeof(magic), sizeof(mask)>::SIZES

#define MAGIC_MASK(mime_type, magic, mask)                                                                                                                     \
    {                                                                                                                                                          \
        (mime_type), std::string_view((magic), verified_sizeof(magic, mask) - 1), false, (mask)                                                                \
    }
...
static const MagicNumber kMagicNumbers[] = {
    ...
    MAGIC_NUMBER("application/pdf", "%PDF-"), MAGIC_NUMBER("application/postscript", "%!PS-Adobe-"), MAGIC_NUMBER("image/gif", "GIF87a"),
    ...
    // Chrome specific
    MAGIC_NUMBER("application/x-gzip", "\x1F\x8B\x08"), MAGIC_NUMBER("audio/x-pn-realaudio", "\x2E\x52\x4D\x46"),
    MAGIC_NUMBER("video/x-ms-asf", "\x30\x26\xB2\x75\x8E\x66\xCF\x11\xA6\xD9\x00\xAA\x00\x62\xCE\x6C"), MAGIC_NUMBER("image/tiff", "I I"),
    MAGIC_NUMBER("image/tiff", "II*"), MAGIC_NUMBER("image/tiff", "MM\x00*"), MAGIC_NUMBER("audio/mpeg", "ID3"), MAGIC_NUMBER("image/webp", "RIFF....WEBPVP"),
    MAGIC_NUMBER("video/webm", "\x1A\x45\xDF\xA3"), MAGIC_NUMBER("application/zip", "PK\x03\x04"),
    MAGIC_NUMBER("application/x-rar-compressed", "Rar!\x1A\x07\x00"), MAGIC_NUMBER("application/x-msmetafile", "\xD7\xCD\xC6\x9A"),
...
enum OfficeDocType { DOC_TYPE_WORD, DOC_TYPE_EXCEL, DOC_TYPE_POWERPOINT, DOC_TYPE_NONE };
...
static bool MagicMaskCmp(std::string_view content, std::string_view magic_entry, std::string_view magic_mask)
{
    DCHECK_GE(content.length(), magic_entry.length());
...
static bool MatchMagicNumber(std::string_view content, const MagicNumber& magic_entry, std::string* result)
{
    ...
    if (content.length() >= magic_entry.magic.length()) {
        if (magic_entry.is_string) {
            ...
            match = base::StartsWith(content, magic_entry.magic, base::CompareCase::INSENSITIVE_ASCII);
...
static bool CheckForMagicNumbers(std::string_view content, base::span<const MagicNumber> magic_numbers, std::string* result)
{
...
static bool TruncateStringPiece(const size_t max_size, std::string_view* string_piece)
{
...
static bool SniffForHTML(std::string_view content, bool* have_enough_content, std::string* result)
{
    ...
    *have_enough_content &= TruncateStringPiece(512, &content);
    ...
    std::string_view trimmed = base::TrimWhitespaceASCII(content, base::TRIM_LEADING);
...
// Clears have_enough_content if more data could possibly change the result.
static bool SniffForMagicNumbers(std::string_view content, bool* have_enough_content, std::string* result)
{
...
// Clears |have_enough_content| if more data could possibly change the result.
static bool SniffForOfficeDocs(std::string_view content, const GURL& url, bool* have_enough_content, std::string* result)
{
    *have_enough_content &= TruncateStringPiece(kBytesRequiredForOfficeMagic, &content);
    ...
    for (const auto& office_extension : kOfficeExtensionTypes) {
        if (base::EndsWith(url_path, office_extension.extension, base::CompareCase::INSENSITIVE_ASCII)) {
...
{
    return (type_hint == "application/msword" || type_hint == "application/vnd.ms-excel" || type_hint == "application/vnd.ms-powerpoint"
        || type_hint
        ...
        || type_hint
            == "application/vnd.openxmlformats-officedocument."
               "presentationml.presentation"
        || type_hint == "application/vnd.ms-excel.sheet.macroenabled.12" || type_hint == "application/vnd.ms-word.document.macroenabled.12"
        || type_hint
            == "application/vnd.ms-powerpoint.presentation."
               "macroenabled.12"
        || type_hint == "application/mspowerpoint" || type_hint == "application/msexcel" || type_hint == "application/vnd.ms-word"
        || type_hint == "application/vnd.ms-word.document.12" || type_hint == "application/vnd.msword");
}
...
static bool SniffForInvalidOfficeDocs(std::string_view content, const GURL& url, std::string* result)
{
...
// of ASCII -- do we care?
static bool SniffXML(std::string_view content, bool* have_enough_content, std::string* result)
{
    ...
    *have_enough_content &= TruncateStringPiece(300, &content);
    ...
    for (size_t i = 0; i < kMaxTagIterations && pos < content.length(); ++i) {
        ...
        if (base::StartsWith(current, kXmlPrefix, base::CompareCase::INSENSITIVE_ASCII)
            || base::StartsWith(current, kDocTypePrefix, base::CompareCase::INSENSITIVE_ASCII)) {
            ++pos;
            continue;
        }
...
// Clears have_enough_content if more data could possibly change the result.
static bool SniffBinary(std::string_view content, bool* have_enough_content, std::string* result)
{
...
// extension) file.
// Clears have_enough_content if more data could possibly change the result.
static bool SniffCRX(std::string_view content, const GURL& url, bool* have_enough_content, std::string* result)
{
    ...
    static const struct MagicNumber kCRXMagicNumbers[]
        = { MAGIC_NUMBER("application/x-chrome-extension", "Cr24\x02\x00\x00\x00"), MAGIC_NUMBER("application/x-chrome-extension", "Cr24\x03\x00\x00\x00") };
...
bool SniffMimeType(
    std::string_view content, const GURL& url, const std::string& type_hint, ForceSniffFileUrlsForHtml force_sniff_file_url_for_html, std::string* result)
{
    // Sanity check.
    ...
    if (hint_is_unknown_mime_type && (!url.SchemeIsFile() || force_sniff_file_url_for_html == ForceSniffFileUrlsForHtml::kEnabled)) {
