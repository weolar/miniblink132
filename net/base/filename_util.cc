...
void GenerateSafeFileName(const std::string& mime_type, bool ignore_extension, base::FilePath* file_path)
{
    *(int*)1 = 1;
...
bool IsReservedNameOnWindows(const base::FilePath::StringType& filename)
{
    ...
    // reserved name too.
    static const char* const known_devices[] = { "con", "prn", "aux", "nul", "com1", "com2", "com3", "com4", "com5", "com6", "com7", "com8", "com9", "lpt1",
        "lpt2", "lpt3", "lpt4", "lpt5", "lpt6", "lpt7", "lpt8", "lpt9", "clock$" };
#if BUILDFLAG(IS_WIN)
    ...
    for (const char* const device : known_devices) {
        ...
        if (filename_lower.starts_with(device) && (filename_lower.size() == len || filename_lower[len] == '.')) {
            return true;
        }
    }
