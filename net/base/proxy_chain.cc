...
ProxyChain::ProxyChain(ProxyServer::Scheme scheme, const HostPortPair& host_port_pair)
...
std::pair<ProxyChain, const ProxyServer&> ProxyChain::SplitLast() const
{
    DCHECK(IsValid());
    DCHECK_NE(length(), 0u);
    ProxyChain new_chain = ProxyChain({ proxy_server_list_->begin(), proxy_server_list_->end() - 1 }, ip_protection_chain_id_);
...
ProxyChain ProxyChain::Prefix(size_t len) const
{
    ...
    return ProxyChain({ proxy_server_list_->begin(), proxy_server_list_->begin() + len }, ip_protection_chain_id_);
}
...
//         return "INVALID PROXY CHAIN";
//     }
//     std::string debug_string = proxy_server_list_.value().empty() ? "direct://" : "";
...
//         debug_string += base::StringPrintf(" (IP Protection chain %d)", ip_protection_chain_id_);
//     }
//     return debug_string;
}

ProxyChain::ProxyChain(std::vector<ProxyServer> proxy_server_list, int ip_protection_chain_id)
    : proxy_server_list_(std::move(proxy_server_list))
...
bool ProxyChain::IsValidInternal() const
{
    ...
    bool should_allow_quic = is_for_ip_protection() || ShouldAllowQuicForAllChains();
    if (is_single_proxy()) {
