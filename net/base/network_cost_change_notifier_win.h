...
public:
    ...
    static constexpr base::win::Version kSupportedOsVersion = base::win::Version::WIN10_20H1;

    using CostChangedCallback = base::RepeatingCallback<void(NetworkChangeNotifier::ConnectionCost)>;
    ...
    static base::SequenceBound<NetworkCostChangeNotifierWin> CreateInstance(CostChangedCallback cost_changed_callback);

    // Use `CreateInstance()` above.  This constructor is public for use by
    // `base::SequenceBound` only.
    explicit NetworkCostChangeNotifierWin(CostChangedCallback cost_changed_callback);
    ...
    // conditions.
    using CoCreateInstanceCallback = base::RepeatingCallback<HRESULT(REFCLSID, LPUNKNOWN, DWORD, REFIID, LPVOID*)>;
    static void OverrideCoCreateInstanceForTesting(CoCreateInstanceCallback callback_for_testing);

    NetworkCostChangeNotifierWin(const NetworkCostChangeNotifierWin&) = delete;
    NetworkCostChangeNotifierWin& operator=(const NetworkCostChangeNotifierWin&) = delete;
...
private:
    ...
    Microsoft::WRL::ComPtr<NetworkCostManagerEventSinkWin> cost_manager_event_sink_;
