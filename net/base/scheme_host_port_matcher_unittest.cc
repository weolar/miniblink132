...
{
    SchemeHostPortMatcher matcher = SchemeHostPortMatcher::FromRawString(".google.com , .foobar.com:30");
...
{
    SchemeHostPortMatcher matcher = SchemeHostPortMatcher::FromRawString(":// , , .google.com , , http://baz");
...
TEST(SchemeHostPortMatcherTest, DoesNotMimicProxyBypassRules)
{
    ...
    // rule).
    SchemeHostPortMatcher matcher = SchemeHostPortMatcher::FromRawString("<-loopback>");
    ...
    EXPECT_EQ(SchemeHostPortMatcherResult::kNoMatch, matcher.Evaluate(GURL("http://localhost")));
    EXPECT_EQ(SchemeHostPortMatcherResult::kNoMatch, matcher.Evaluate(GURL("http://169.254.1.1")));
}
