...
BackoffEntry::BackoffEntry(const BackoffEntry::Policy* policy, const base::TickClock* clock)
    : policy_(policy)
    , clock_(clock)
{
    DCHECK(policy_);
...
void BackoffEntry::InformOfRequest(bool succeeded)
{
    if (!succeeded) {
        ...
        exponential_backoff_release_time_ = std::max(GetTimeTicksNow() + delay, exponential_backoff_release_time_);
    }
}
...
bool BackoffEntry::CanDiscard() const
{
    ...
    if (failure_count_ > 0) {
        ...
        return unused_since_ms >= std::max(policy_->maximum_backoff_ms, policy_->entry_lifetime_ms);
    }
...
{
    base::ClampedNumeric<int> effective_failure_count = base::ClampSub(failure_count_, policy_->num_errors_to_ignore).Max(0);
    ...
    base::TimeDelta backoff_duration = base::Microseconds(int64_t { backoff_duration_us.ValueOrDefault(std::numeric_limits<int64_t>::max()) });
...
base::TimeTicks BackoffEntry::BackoffDurationToReleaseTime(base::TimeDelta backoff_duration) const
{
    ...
    int64_t release_time_us = std::min(calculated_release_time_us.ValueOrDefault(std::numeric_limits<int64_t>::max()),
        maximum_release_time_us.ValueOrDefault(std::numeric_limits<int64_t>::max()));
