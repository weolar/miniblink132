...
bool GetAddress(const struct nlmsghdr* header, int header_length, IPAddress* out, bool* really_deprecated)
{
    ...
    // remaining bytes.
    const struct ifaddrmsg* msg = reinterpret_cast<const struct ifaddrmsg*>(NLMSG_DATA(header));
    ...
    for (const struct rtattr* attr = reinterpret_cast<const struct rtattr*>(IFA_RTA(msg)); RTA_OK(attr, length); attr = RTA_NEXT(attr, length)) {
        ...
        case IFA_CACHEINFO: {
            if (RTA_PAYLOAD(attr) < sizeof(struct ifa_cacheinfo)) {
                LOG(ERROR) << "attr does not have enough bytes to read an ifa_cacheinfo";
                return false;
            }
...
template <typename T> T* SafelyCastNetlinkMsgData(const struct nlmsghdr* header, int length)
{
...
AddressTrackerLinux::AddressTrackerLinux(const base::RepeatingClosure& address_callback, const base::RepeatingClosure& link_callback,
    const base::RepeatingClosure& tunnel_callback, const std::unordered_set<std::string>& ignored_interfaces,
...
void AddressTrackerLinux::Init()
{
    ...
    DCHECK_LT(base::android::BuildInfo::GetInstance()->sdk_int(), base::android::SDK_VERSION_P);
#endif
    ...
    if (tracking_) {
        ...
        rv = bind(netlink_fd_.get(), reinterpret_cast<struct sockaddr*>(&addr), sizeof(addr));
        if (rv < 0) {
...
std::pair<AddressTrackerLinux::AddressMap, std::unordered_set<int>> AddressTrackerLinux::GetInitialDataAndStartRecordingDiffs()
{
    DCHECK(tracking_);
...
void AddressTrackerLinux::SetDiffCallback(DiffCallback diff_callback)
{
    ...
    if (!sequenced_task_runner_->RunsTasksInCurrentSequence()) {
        sequenced_task_runner_->PostTask(
            FROM_HERE, base::BindOnce(&AddressTrackerLinux::SetDiffCallback, weak_ptr_factory_.GetWeakPtr(), std::move(diff_callback)));
        return;
    }
...
NetworkChangeNotifier::ConnectionType AddressTrackerLinux::GetCurrentConnectionType()
{
...
void AddressTrackerLinux::DumpInitialAddressesAndWatch()
{
    ...
    int rv = HANDLE_EINTR(sendto(netlink_fd_.get(), &request, request.header.nlmsg_len, 0, reinterpret_cast<struct sockaddr*>(&peer), sizeof(peer)));
    if (rv < 0) {
    ...
    rv = HANDLE_EINTR(sendto(netlink_fd_.get(), &request, request.header.nlmsg_len, 0, reinterpret_cast<struct sockaddr*>(&peer), sizeof(peer)));
    if (rv < 0) {
        PLOG(ERROR) << "Could not send NETLINK request";
        AbortAndForceOnline();
        return;
    ...
    if (tracking_) {
        DCHECK(!sequenced_task_runner_ || sequenced_task_runner_->RunsTasksInCurrentSequence());

        watcher_ = base::FileDescriptorWatcher::WatchReadable(
            netlink_fd_.get(), base::BindRepeating(&AddressTrackerLinux::OnFileCanReadWithoutBlocking, base::Unretained(this)));
    }
}

void AddressTrackerLinux::ReadMessages(bool* address_changed, bool* link_changed, bool* tunnel_changed)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    *tunnel_changed = false;
    ...
    std::vector<char> buffer(std::max(base::GetPageSize(), kMinNetlinkBufferSize));

    {
        ...
        for (;;) {
            int rv = HANDLE_EINTR(recv(netlink_fd_.get(), buffer.data(), buffer.size(),
            ...
            HandleMessage(buffer.data(), rv, address_changed, link_changed, tunnel_changed);
        }
    }
...
void AddressTrackerLinux::HandleMessage(const char* buffer, int length, bool* address_changed, bool* link_changed, bool* tunnel_changed)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(buffer);
    // Note that NLMSG_NEXT decrements |length| to reflect the number of bytes
    // remaining in |buffer|.
    for (const struct nlmsghdr* header = reinterpret_cast<const struct nlmsghdr*>(buffer); length >= 0 && NLMSG_OK(header, static_cast<__u32>(length));
        ...
        case NLMSG_DONE:
            return;
        case NLMSG_ERROR: {
            const struct nlmsgerr* msg = SafelyCastNetlinkMsgData<const struct nlmsgerr>(header, length);
            if (msg == nullptr)
                return;
        ...
        case RTM_NEWADDR: {
            ...
            const struct ifaddrmsg* msg = SafelyCastNetlinkMsgData<const struct ifaddrmsg>(header, length);
            if (msg == nullptr)
                return;
...
void AddressTrackerLinux::UpdateCurrentConnectionType()
{
    ...
    if (GetNetworkListImpl(&networks, 0, online_links, address_map, get_interface_name_)) {
        type = NetworkChangeNotifier::ConnectionTypeFromInterfaceList(networks);
    } else {
        type = online_links.empty() ? NetworkChangeNotifier::CONNECTION_NONE : NetworkChangeNotifier::CONNECTION_UNKNOWN;
    }
...
AddressTrackerLinux::AddressTrackerAutoLock::AddressTrackerAutoLock(const AddressTrackerLinux& tracker, base::Lock& lock)
    : tracker_(tracker)
    , lock_(lock)
{
