...
UploadFileElementReader::UploadFileElementReader(base::TaskRunner* task_runner, base::File file, const base::FilePath& path, uint64_t range_offset,
    uint64_t range_length, const base::Time& expected_modification_time)
...
UploadFileElementReader::UploadFileElementReader(
    base::TaskRunner* task_runner, const base::FilePath& path, uint64_t range_offset, uint64_t range_length, const base::Time& expected_modification_time)
    : task_runner_(task_runner)
    , path_(path)
    , range_offset_(range_offset)
    , range_length_(range_length)
...
int UploadFileElementReader::Read(IOBuffer* buf, int buf_length, CompletionOnceCallback callback)
{
    DCHECK(!callback.is_null());
    DCHECK_EQ(next_state_, State::IDLE);
    DCHECK(file_stream_);

    int num_bytes_to_read = static_cast<int>(std::min(BytesRemaining(), static_cast<uint64_t>(buf_length)));
    if (num_bytes_to_read == 0)
        return 0;

    next_state_ = State::READ_COMPLETE;
    int result = file_stream_->Read(
        buf, num_bytes_to_read, base::BindOnce(base::IgnoreResult(&UploadFileElementReader::OnIOComplete), weak_ptr_factory_.GetWeakPtr()));
...
int UploadFileElementReader::DoOpen()
{
    ...
    int result = file_stream_->Open(path_, base::File::FLAG_OPEN | base::File::FLAG_READ | base::File::FLAG_ASYNC,
        base::BindOnce(&UploadFileElementReader::OnIOComplete, weak_ptr_factory_.GetWeakPtr()));
    DCHECK_GT(0, result);
    return result;
}
...
{
    if (result < 0) {
        DLOG(WARNING) << "Failed to open \"" << path_.value() << "\" for reading: " << result;
        file_stream_.reset();
        return result;
    }
...
{
    next_state_ = State::GET_FILE_INFO;
    return file_stream_->Seek(range_offset_,
        base::BindOnce(
            [](base::WeakPtr<UploadFileElementReader> weak_this, int64_t result) {
                if (!weak_this)
                    return;
                weak_this->OnIOComplete(result >= 0 ? OK : static_cast<int>(result));
            },
...
{
    if (result < 0) {
        DLOG(WARNING) << "Failed to seek \"" << path_.value() << "\" to offset: " << range_offset_ << " (" << result << ")";
        return result;
    }
    ...
    result = file_stream_->GetFileInfo(file_info_ptr,
        base::BindOnce(
            [](base::WeakPtr<UploadFileElementReader> weak_this, std::unique_ptr<base::File::Info> file_info, int result) {
                if (!weak_this)
                    return;
...
int UploadFileElementReader::DoGetFileInfoComplete(int result)
{
    ...
    // for sliced files.
    if (!expected_modification_time_.is_null() && (expected_modification_time_ - file_info_.last_modified).magnitude().InSeconds() != 0) {
...
UploadFileElementReader::ScopedOverridingContentLengthForTests::ScopedOverridingContentLengthForTests(uint64_t value)
{
    overriding_content_length = value;
}

UploadFileElementReader::ScopedOverridingContentLengthForTests::~ScopedOverridingContentLengthForTests()
{
