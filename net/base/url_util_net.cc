...
GURL AppendQueryParameter(const GURL& url, const std::string& name, const std::string& value)
{
...
GURL AppendOrReplaceQueryParameter(const GURL& url, const std::string& name, absl::optional<base::StringPiece> value)
{
    bool replaced = false;
    ...
    while (url::ExtractQueryKeyValue(input.data(), &cursor, &key_range, &value_range)) {
        ...
        if (!replaced && key == param_name) {
            ...
            key_value_pair.assign(input, key_range.begin, value_range.end() - key_range.begin);
        }
        if (!output.empty())
            output += "&";
...
bool GetValueForKeyInQuery(const GURL& url, const std::string& search_key, std::string* out_value)
{
...
{
    int default_port = url::DefaultPortForScheme(std::string_view(scheme_host_port.scheme().data(), static_cast<int>(scheme_host_port.scheme().length())));
...
std::string CanonicalizeHost(base::StringPiece host, url::CanonHostInfo* host_info)
{
    ...
    url::CanonicalizeHostVerbose(host.data(), raw_host_component, &canon_host_output, host_info);
...
bool IsLocalHostname(base::StringPiece host)
{
    ...
    return normalized_host == "localhost" || IsNormalizedLocalhostTLD(normalized_host);
}
