...
FileStream::Context::Context(base::File file, scoped_refptr<base::TaskRunner> task_runner)
...
int FileStream::Context::Read(IOBuffer* buf, int buf_len, CompletionOnceCallback callback)
{
    ...
    task_runner_->PostTask(FROM_HERE,
        base::BindOnce(&FileStream::Context::ReadAsync, base::Unretained(this), file_.GetPlatformFile(), base::WrapRefCounted(buf), buf_len,
            &io_context_.overlapped, base::SingleThreadTaskRunner::GetCurrentDefault()));
    return ERR_IO_PENDING;
}

int FileStream::Context::Write(IOBuffer* buf, int buf_len, CompletionOnceCallback callback)
{
    DCHECK(!async_in_progress_);
    ...
    if (!WriteFile(file_.GetPlatformFile(), buf->data(), buf_len, &bytes_written, &io_context_.overlapped)) {
...
FileStream::Context::IOResult FileStream::Context::SeekFileImpl(int64_t offset)
{
...
{
    if (!base::CurrentIOThread::Get()->RegisterIOHandler(file_.GetPlatformFile(), this)) {
        file_.Close();
    }
}

void FileStream::Context::IOCompletionIsPending(CompletionOnceCallback callback, IOBuffer* buf)
{
...
void FileStream::Context::OnIOCompleted(base::MessagePumpForIO::IOContext* context, DWORD bytes_read, DWORD error)
{
...
// static
void FileStream::Context::ReadAsync(FileStream::Context* context, HANDLE file, scoped_refptr<IOBuffer> buf, int buf_len, OVERLAPPED* overlapped,
    scoped_refptr<base::SingleThreadTaskRunner> origin_thread_task_runner)
{
    ...
    origin_thread_task_runner->PostTask(
        FROM_HERE, base::BindOnce(&FileStream::Context::ReadAsyncResult, base::Unretained(context), ret, bytes_read, ::GetLastError()));
}

void FileStream::Context::ReadAsyncResult(BOOL read_file_ret, DWORD bytes_read, DWORD os_error)
{
