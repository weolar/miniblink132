...
{
    NetworkConfigWatcherApple::Delegate* net_config_delegate = static_cast<NetworkConfigWatcherApple::Delegate*>(config_delegate);
...
public:
    explicit NetworkConfigWatcherAppleThread(NetworkConfigWatcherApple::Delegate* delegate);
    NetworkConfigWatcherAppleThread(const NetworkConfigWatcherAppleThread&) = delete;
    NetworkConfigWatcherAppleThread& operator=(const NetworkConfigWatcherAppleThread&) = delete;
...
NetworkConfigWatcherAppleThread::NetworkConfigWatcherAppleThread(NetworkConfigWatcherApple::Delegate* delegate)
...
void NetworkConfigWatcherAppleThread::Init()
{
    ...
    task_runner()->PostDelayedTask(
        FROM_HERE, base::BindOnce(&NetworkConfigWatcherAppleThread::InitNotifications, weak_factory_.GetWeakPtr()), kInitializationDelay);
}
...
void NetworkConfigWatcherAppleThread::InitNotifications()
{
    ...
    if (!success && num_retry_ < kMaxRetry) {
        ...
        task_runner()->PostDelayedTask(
            FROM_HERE, base::BindOnce(&NetworkConfigWatcherAppleThread::InitNotifications, weak_factory_.GetWeakPtr()), kRetryInterval);
        num_retry_++;
        return;
    }
...
bool NetworkConfigWatcherAppleThread::InitNotificationsHelper()
{
    ...
    base::apple::ScopedCFTypeRef<SCDynamicStoreRef> store(SCDynamicStoreCreate(nullptr, CFSTR("org.chromium"), DynamicStoreCallback, &context));
    if (!store) {
        int error = SCError();
...
NetworkConfigWatcherApple::NetworkConfigWatcherApple(Delegate* delegate)
    : notifier_thread_(std::make_unique<NetworkConfigWatcherAppleThread>(delegate))
{
