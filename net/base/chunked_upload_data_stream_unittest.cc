...
std::string ReadSync(UploadDataStream* stream, int buffer_size)
{
    ...
    int result = stream->Read(buf.get(), buffer_size, TestCompletionCallback().callback());
    EXPECT_GE(result, 0);
...
TEST(ChunkedUploadDataStreamTest, AppendOnce)
{
    ...
    ASSERT_THAT(stream.Init(TestCompletionCallback().callback(), NetLogWithSource()), IsOk());
...
TEST(ChunkedUploadDataStreamTest, MultipleAppends)
{
    ...
    for (size_t i = 0; i < kTestDataSize; ++i) {
        ...
        int bytes_read = stream.Read(buf.get(), kTestBufferSize, callback.callback());
        ASSERT_THAT(bytes_read, IsError(ERR_IO_PENDING));
        stream.AppendData(base::byte_span_from_cstring(kTestData).subspan(i, 1u), i == kTestDataSize - 1);
...
TEST(ChunkedUploadDataStreamTest, MultipleAppendsBetweenReads)
{
    ...
    for (size_t i = 0; i < kTestDataSize; ++i) {
        ...
        ASSERT_FALSE(stream.IsEOF());
        stream.AppendData(base::byte_span_from_cstring(kTestData).subspan(i, 1u), i == kTestDataSize - 1);
        int bytes_read = stream.Read(buf.get(), kTestBufferSize, TestCompletionCallback().callback());
...
TEST(ChunkedUploadDataStreamTest, MultipleAppendsBeforeInit)
{
    ...
    stream.AppendData(base::byte_span_from_cstring(kTestData).subspan(1u, 1u), false);
...
TEST(ChunkedUploadDataStreamTest, ChunkedUploadDataStreamWriter)
{
    ...
    std::unique_ptr<ChunkedUploadDataStream::Writer> writer(stream->CreateWriter());

    // Write before Init.
    ASSERT_TRUE(writer->AppendData(base::byte_span_from_cstring(kTestData).first(1u), false));
    ASSERT_THAT(stream->Init(TestCompletionCallback().callback(), NetLogWithSource()), IsOk());

    // Write after Init.
    ASSERT_TRUE(writer->AppendData(base::byte_span_from_cstring(kTestData).subspan(1u), false));
    ...
    stream.reset();
    EXPECT_FALSE(writer->AppendData(base::byte_span_from_cstring(kTestData), true));
}
