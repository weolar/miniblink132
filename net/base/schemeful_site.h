...
public:
    ...
    static std::optional<SchemefulSite> CreateIfHasRegisterableDomain(const url::Origin&);
    ...
    static std::optional<SchemefulSite> DeserializeWithNonce(base::PassKey<NetworkAnonymizationKey>, std::string_view value);
    ...
    std::optional<std::string> SerializeWithNonce(base::PassKey<NetworkAnonymizationKey>);

    bool opaque() const
    {
        return site_as_origin_.opaque();
    }
...
private:
    ...
    friend struct mojo::StructTraits<network::mojom::SchemefulSiteDataView, SchemefulSite>;
    ...
    // Deserializes a string obtained from `SerializeWithNonce()` to a
    // `SchemefulSite`. Returns nullopt if the value was invalid in any way.
    static std::optional<SchemefulSite> DeserializeWithNonce(std::string_view value);
    ...
    const url::Origin& internal_value() const
    {
