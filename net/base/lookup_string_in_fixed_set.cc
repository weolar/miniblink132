...
inline bool GetNextOffset(base::span<const uint8_t>* bytes, base::span<const uint8_t>* offset_bytes)
{
    if (!bytes->size()) {
        return false;
    }
    ...
    case 0x60: // Read three byte offset
        *offset_bytes = offset_bytes->subspan(((bytes->front() & 0x1F) << 16) | ((*bytes)[1] << 8) | (*bytes)[2]);
        bytes_consumed = 3;
        break;
    case 0x40: // Read two byte offset
        *offset_bytes = offset_bytes->subspan(((bytes->front() & 0x1F) << 8) | (*bytes)[1]);
        bytes_consumed = 2;
        break;
    default:
...
FixedSetIncrementalLookup::FixedSetIncrementalLookup(base::span<const uint8_t> graph)
    : bytes_(graph)
    , original_bytes_(graph)
{
}

FixedSetIncrementalLookup::FixedSetIncrementalLookup(const FixedSetIncrementalLookup& other) = default;

FixedSetIncrementalLookup& FixedSetIncrementalLookup::operator=(const FixedSetIncrementalLookup& other) = default;
...
int LookupStringInFixedSet(base::span<const uint8_t> graph, const char* key, size_t key_length)
{
...
int LookupSuffixInReversedSet(base::span<const uint8_t> graph, bool include_private, std::string_view host, size_t* suffix_length)
{
    FixedSetIncrementalLookup lookup(graph);
    *suffix_length = 0;
