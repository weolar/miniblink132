...
const uint16_t* GetPortFieldFromSockaddr(const struct sockaddr* address, socklen_t address_len)
{
...
TEST_F(IPEndPointTest, ToFromSockAddr)
{
    ...
    for (const auto& test : tests) {
        ...
        // Basic verification.
        socklen_t expected_size = test.ipv6 ? sizeof(struct sockaddr_in6) : sizeof(struct sockaddr_in);
...
namespace {
constexpr uint8_t kBluetoothAddrBytes[kBluetoothAddressSize] = { 1, 2, 3, 4, 5, 6 };
constexpr uint8_t kBluetoothAddrBytes2[kBluetoothAddressSize] = { 1, 2, 3, 4, 5, 7 };
...
SOCKADDR_BTH BuildBluetoothSockAddr(const IPAddress& ip_address, uint32_t port)
{
    SOCKADDR_BTH addr = {};
...
TEST_F(IPEndPointTest, WinBluetoothSockAddrCompareWithSelf)
{
    ...
    EXPECT_TRUE(bt_endpoint.FromSockAddr(reinterpret_cast<const struct sockaddr*>(&addr), sizeof(addr)));
    ...
    SockaddrStorage storage;
    EXPECT_DCHECK_DEATH(std::ignore = bt_endpoint.ToSockAddr(storage.addr, &storage.addr_len));
...
TEST_F(IPEndPointTest, WinBluetoothSockAddrCompareWithCopy)
{
    ...
    EXPECT_EQ(bt_endpoint.GetSockAddrFamily(), bt_endpoint_other.GetSockAddrFamily());
    ...
    SockaddrStorage storage;
    EXPECT_DCHECK_DEATH(std::ignore = bt_endpoint_other.ToSockAddr(storage.addr, &storage.addr_len));
...
TEST_F(IPEndPointTest, WinBluetoothSockAddrCompareWithDifferentPort)
{
    ...
    SOCKADDR_BTH addr2 = BuildBluetoothSockAddr(kBluetoothAddress, kBluetoothPort + 1);
    EXPECT_TRUE(bt_endpoint_other.FromSockAddr(reinterpret_cast<const struct sockaddr*>(&addr2), sizeof(addr2)));
    EXPECT_EQ(bt_endpoint.address(), bt_endpoint_other.address());
    EXPECT_EQ(bt_endpoint.GetFamily(), bt_endpoint_other.GetFamily());
    EXPECT_EQ(bt_endpoint.GetSockAddrFamily(), bt_endpoint_other.GetSockAddrFamily());
...
TEST_F(IPEndPointTest, WinBluetoothSockAddrCompareWithDifferentAddress)
{
    ...
    IPEndPoint bt_endpoint_other;
    SOCKADDR_BTH addr2 = BuildBluetoothSockAddr(kBluetoothAddress2, kBluetoothPort);
    EXPECT_TRUE(bt_endpoint_other.FromSockAddr(reinterpret_cast<const struct sockaddr*>(&addr2), sizeof(addr2)));
