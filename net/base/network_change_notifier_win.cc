...
NetworkChangeNotifierWin::NetworkChangeNotifierWin()
    ...
    , blocking_task_runner_(base::ThreadPool::CreateSequencedTaskRunner({ base::MayBlock() }))
    ...
    , sequence_runner_for_registration_(base::SequencedTaskRunner::GetCurrentDefault())
{
    memset(&addr_overlapped_, 0, sizeof addr_overlapped_);
    addr_overlapped_.hEvent = WSACreateEvent();

    cost_change_notifier_
        = NetworkCostChangeNotifierWin::CreateInstance(base::BindRepeating(&NetworkChangeNotifierWin::OnCostChanged, weak_factory_.GetWeakPtr()));
}
...
// static
NetworkChangeNotifier::NetworkChangeCalculatorParams NetworkChangeNotifierWin::NetworkChangeCalculatorParamsWin()
{
...
// static
NetworkChangeNotifier::ConnectionType NetworkChangeNotifierWin::RecomputeCurrentConnectionTypeModern()
{
    using GetNetworkConnectivityHintType = decltype(&::GetNetworkConnectivityHint);
    ...
    GetNetworkConnectivityHintType get_network_connectivity_hint
        = reinterpret_cast<GetNetworkConnectivityHintType>(::GetProcAddress(::GetModuleHandleA("iphlpapi.dll"), "GetNetworkConnectivityHint"));
...
//
// static
NetworkChangeNotifier::ConnectionType NetworkChangeNotifierWin::RecomputeCurrentConnectionType()
{
    if (base::win::GetVersion() >= base::win::Version::WIN10_20H1 && base::FeatureList::IsEnabled(features::kEnableGetNetworkConnectivityHintAPI)) {
    ...
    // TODO(droger): Return something more detailed than CONNECTION_UNKNOWN.
    return found_connection ? ConnectionTypeFromInterfaces() : NetworkChangeNotifier::CONNECTION_NONE;
}

void NetworkChangeNotifierWin::RecomputeCurrentConnectionTypeOnBlockingSequence(base::OnceCallback<void(ConnectionType)> reply_callback) const
{
...
NetworkChangeNotifier::ConnectionCost NetworkChangeNotifierWin::GetCurrentConnectionCost()
{
    if (last_computed_connection_cost_ == ConnectionCost::CONNECTION_COST_UNKNOWN) {
...
void NetworkChangeNotifierWin::OnCostChanged(NetworkChangeNotifier::ConnectionCost new_cost)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
NetworkChangeNotifier::ConnectionType NetworkChangeNotifierWin::GetCurrentConnectionType() const
{
    base::AutoLock auto_lock(last_computed_connection_type_lock_);
    return last_computed_connection_type_;
}

void NetworkChangeNotifierWin::SetCurrentConnectionType(ConnectionType connection_type)
{
    base::AutoLock auto_lock(last_computed_connection_type_lock_);
...
void NetworkChangeNotifierWin::OnObjectSignaled(HANDLE object)
{
    ...
    RecomputeCurrentConnectionTypeOnBlockingSequence(base::BindOnce(&NetworkChangeNotifierWin::NotifyObservers, weak_factory_.GetWeakPtr()));
}
...
void NetworkChangeNotifierWin::NotifyObservers(ConnectionType connection_type)
{
    ...
    // delay again.
    offline_polls_ = 0;
    timer_.Start(FROM_HERE, base::Seconds(1), this, &NetworkChangeNotifierWin::NotifyParentOfConnectionTypeChange);
}
...
void NetworkChangeNotifierWin::WatchForAddressChange()
{
    ...
    if (!WatchForAddressChangeInternal()) {
        ...
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE,
            base::BindOnce(&NetworkChangeNotifierWin::WatchForAddressChange, weak_factory_.GetWeakPtr()),
...
void NetworkChangeNotifierWin::NotifyParentOfConnectionTypeChange()
{
    RecomputeCurrentConnectionTypeOnBlockingSequence(
        base::BindOnce(&NetworkChangeNotifierWin::NotifyParentOfConnectionTypeChangeImpl, weak_factory_.GetWeakPtr()));
}

void NetworkChangeNotifierWin::NotifyParentOfConnectionTypeChangeImpl(ConnectionType connection_type)
{
    SetCurrentConnectionType(connection_type);
    ...
    GetCurrentMaxBandwidthAndConnectionType(&max_bandwidth_mbps, &max_connection_type);
