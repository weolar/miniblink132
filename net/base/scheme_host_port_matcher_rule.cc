...
// static
std::unique_ptr<SchemeHostPortMatcherRule> SchemeHostPortMatcherRule::FromUntrimmedRawString(std::string_view raw_untrimmed)
{
    std::string_view raw = base::TrimWhitespaceASCII(raw_untrimmed, base::TRIM_ALL);
    ...
    if (raw.find('/') != std::string::npos) {
        ...
        return std::make_unique<SchemeHostPortMatcherIPBlockRule>(std::string(raw), scheme, ip_prefix, prefix_length_in_bits);
    }
    ...
    if (ParseHostAndPort(raw, &host, &port)) {
        ...
        if (ip_address.AssignFromIPLiteral(host)) {
            ...
            return std::make_unique<SchemeHostPortMatcherIPHostRule>(scheme, IPEndPoint(ip_address, adjusted_port));
        }
    }
    ...
    if (pos_colon != std::string::npos) {
        if (!ParseInt32(base::MakeStringPiece(raw.begin() + pos_colon + 1, raw.end()), ParseIntFormat::NON_NEGATIVE, &port) || port > 0xFFFF) {
    ...
    return std::make_unique<SchemeHostPortMatcherHostnamePatternRule>(scheme, hostname_pattern, port);
}
...
SchemeHostPortMatcherHostnamePatternRule::SchemeHostPortMatcherHostnamePatternRule(
    const std::string& optional_scheme, const std::string& hostname_pattern, int optional_port)
...
SchemeHostPortMatcherResult SchemeHostPortMatcherHostnamePatternRule::Evaluate(const GURL& url) const
{
    ...
    return base::MatchPattern(url.host(), hostname_pattern_) ? SchemeHostPortMatcherResult::kInclude : SchemeHostPortMatcherResult::kNoMatch;
}
...
std::unique_ptr<SchemeHostPortMatcherHostnamePatternRule> SchemeHostPortMatcherHostnamePatternRule::GenerateSuffixMatchingRule() const
{
    if (!hostname_pattern_.starts_with("*")) {
        return std::make_unique<SchemeHostPortMatcherHostnamePatternRule>(optional_scheme_, "*" + hostname_pattern_, optional_port_);
    }
...
{
    return base::trace_event::EstimateMemoryUsage(optional_scheme_) + base::trace_event::EstimateMemoryUsage(hostname_pattern_);
}
#endif // !BUILDFLAG(CRONET_BUILD)

SchemeHostPortMatcherIPHostRule::SchemeHostPortMatcherIPHostRule(const std::string& optional_scheme, const IPEndPoint& ip_end_point)
    : optional_scheme_(base::ToLowerASCII(optional_scheme))
...
SchemeHostPortMatcherResult SchemeHostPortMatcherIPHostRule::Evaluate(const GURL& url) const
{
    ...
    // Note it is necessary to lower-case the host, since GURL uses capital
    // letters for percent-escaped characters.
    return base::MatchPattern(url.host(), ip_host_) ? SchemeHostPortMatcherResult::kInclude : SchemeHostPortMatcherResult::kNoMatch;
}
...
{
    return base::trace_event::EstimateMemoryUsage(optional_scheme_) + base::trace_event::EstimateMemoryUsage(ip_host_);
}
#endif // !BUILDFLAG(CRONET_BUILD)

SchemeHostPortMatcherIPBlockRule::SchemeHostPortMatcherIPBlockRule(
    const std::string& description, const std::string& optional_scheme, const IPAddress& ip_prefix, size_t prefix_length_in_bits)
...
SchemeHostPortMatcherResult SchemeHostPortMatcherIPBlockRule::Evaluate(const GURL& url) const
{
    ...
    return IPAddressMatchesPrefix(ip_address, ip_prefix_, prefix_length_in_bits_) ? SchemeHostPortMatcherResult::kInclude
...
{
    return base::trace_event::EstimateMemoryUsage(description_) + base::trace_event::EstimateMemoryUsage(optional_scheme_)
