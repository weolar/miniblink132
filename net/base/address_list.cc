...
AddressList::AddressList(const IPEndPoint& endpoint, std::vector<std::string> aliases)
...
// static
AddressList AddressList::CreateFromIPAddress(const IPAddress& address, uint16_t port)
{
    return AddressList(IPEndPoint(address, port));
}

// static
AddressList AddressList::CreateFromIPAddressList(const IPAddressList& addresses, std::vector<std::string> aliases)
{
    AddressList list;
...
void AddressList::AppendDnsAliases(std::vector<std::string> aliases)
{
    ...
    dns_aliases_.insert(dns_aliases_.end(), std::move_iterator<iter_t>(aliases.begin()), std::move_iterator<iter_t>(aliases.end()));
}
