...
// application layer.
bool TryConvertNativeToNetIPAttributes(int native_attributes, int* net_attributes)
{
...
NetworkChangeNotifier::ConnectionType GetInterfaceConnectionType(const std::string& ifname)
{
...
bool GetNetworkListImpl(NetworkInterfaceList* networks, int policy, const std::unordered_set<int>& online_links,
    const internal::AddressTrackerLinux::AddressMap& address_map, GetInterfaceNameFunction get_interface_name)
{
    ...
    for (const auto& it : address_map) {
        ...
        if (!IPEndPoint(it.first, 0).ToSockAddr(reinterpret_cast<sockaddr*>(&sock_addr), &sock_len)) {
            continue;
        }
        ...
        if (it.second.ifa_family == AF_INET6) {
            ...
            if (!TryConvertNativeToNetIPAttributes(it.second.ifa_flags, &ip_attributes))
                continue;
        }
        ...
        networks->push_back(NetworkInterface(ifname, ifname, it.second.ifa_index, type, it.first, it.second.ifa_prefixlen, ip_attributes));
    }

    return true;
}

std::string GetWifiSSIDFromInterfaceListInternal(const NetworkInterfaceList& interfaces, internal::GetInterfaceSSIDFunction get_interface_ssid)
{
...
bool GetNetworkList(NetworkInterfaceList* networks, int policy)
{
    ...
    if (build_info->sdk_int() >= base::android::SDK_VERSION_NOUGAT) {
        ...
        bool use_alternative_getifaddrs = std::string_view(build_info->brand()) == "samsung" && std::string_view(build_info->hardware()).starts_with("mt");
        bool ret = internal::GetNetworkListUsingGetifaddrs(networks, policy, use_alternative_getifaddrs);
    ...
    return internal::GetNetworkListImpl(
        networks, policy, map_owner->GetOnlineLinks(), map_owner->GetAddressMap(), &internal::AddressTrackerLinux::GetInterfaceName);
}
...
// On Android, obtain the SSID using the Android-specific APIs.
    ...
    if (GetNetworkList(&networks, INCLUDE_HOST_SCOPE_VIRTUAL_INTERFACES)) {
        return internal::GetWifiSSIDFromInterfaceListInternal(networks, internal::GetInterfaceSSID);
    }
    return std::string();
#endif
}
