...
HostMappingRules::HostMappingRules(const HostMappingRules& host_mapping_rules) = default;

HostMappingRules::~HostMappingRules() = default;

HostMappingRules& HostMappingRules::operator=(const HostMappingRules& host_mapping_rules) = default;
...
bool HostMappingRules::RewriteHost(HostPortPair* host_port) const
{
    ...
    for (const auto& map_rule : map_rules_) {
        ...
        for (const auto& exclusion_rule : exclusion_rules_) {
            if (base::MatchPattern(host_port->host(), exclusion_rule.hostname_pattern))
                return false;
        }
...
HostMappingRules::RewriteResult HostMappingRules::RewriteUrl(GURL& url) const
{
    ...
    DCHECK_EQ(url.EffectiveIntPort() == url::PORT_UNSPECIFIED, new_url.EffectiveIntPort() == url::PORT_UNSPECIFIED);
...
bool HostMappingRules::AddRuleFromString(std::string_view rule_string)
{
    std::vector<std::string_view> parts
        = base::SplitStringPiece(base::TrimWhitespaceASCII(rule_string, base::TRIM_ALL), " ", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL);

    // Test for EXCLUSION rule.
    if (parts.size() == 2 && base::EqualsCaseInsensitiveASCII(parts[0], "exclude")) {
        ExclusionRule rule;
    ...
    if (parts.size() == 3 && base::EqualsCaseInsensitiveASCII(parts[0], "map")) {
        ...
        if (!ParseHostAndPort(parts[2], &rule.replacement_hostname, &rule.replacement_port)) {
...
void HostMappingRules::SetRulesFromString(std::string_view rules_string)
{
    ...
    std::vector<std::string_view> rules = base::SplitStringPiece(rules_string, ",", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL);
