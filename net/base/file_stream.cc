...
FileStream::FileStream(base::File file, const scoped_refptr<base::TaskRunner>& task_runner)
...
int FileStream::Open(const base::FilePath& path, int open_flags, CompletionOnceCallback callback)
{
    if (IsOpen()) {
...
int FileStream::Read(IOBuffer* buf, int buf_len, CompletionOnceCallback callback)
{
    if (!IsOpen())
        return ERR_UNEXPECTED;
...
int FileStream::Write(IOBuffer* buf, int buf_len, CompletionOnceCallback callback)
{
    if (!IsOpen())
        return ERR_UNEXPECTED;
...
int FileStream::GetFileInfo(base::File::Info* file_info, CompletionOnceCallback callback)
{
    if (!IsOpen())
        return ERR_UNEXPECTED;
...
{
    return IsOpen() ? context_->ConnectNamedPipe(std::move(callback)) : ERR_UNEXPECTED;
}
