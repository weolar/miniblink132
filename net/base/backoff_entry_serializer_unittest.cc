...
const Time kParseTime = Time::FromMillisecondsSinceUnixEpoch(1430907555111); // May 2015 for realism

BackoffEntry::Policy base_policy = {
    0 /* num_errors_to_ignore */, 1000 /* initial_delay_ms */, 2.0 /* multiply_factor */, 0.0 /* jitter_factor */, 20000 /* maximum_backoff_ms */,
    2000 /* entry_lifetime_ms */, false /* always_use_initial_delay */
};
...
TEST(BackoffEntrySerializerTest, SpecialCasesOfBackoffDuration)
{
    ...
    for (const TestCase& test_case : test_cases) {
        ...
        const std::string& serialized_backoff_duration_string = serialized[2].GetString();
        int64_t serialized_backoff_duration_us;
        EXPECT_TRUE(base::StringToInt64(serialized_backoff_duration_string, &serialized_backoff_duration_us));

        base::TimeDelta serialized_backoff_duration = base::Microseconds(serialized_backoff_duration_us);
...
TEST(BackoffEntrySerializerTest, SerializeFiniteReleaseTime)
{
    ...
    std::unique_ptr<BackoffEntry> deserialized = BackoffEntrySerializer::DeserializeFromList(serialized, &base_policy, &original_ticks, kParseTime);
...
TEST(BackoffEntrySerializerTest, SerializeNoFailures)
{
    ...
    std::unique_ptr<BackoffEntry> deserialized = BackoffEntrySerializer::DeserializeFromList(serialized, &base_policy, &original_ticks, original_time);
    ASSERT_TRUE(deserialized.get());
...
TEST(BackoffEntrySerializerTest, DeserializeNeverInfiniteReleaseTime)
{
    ...
    base::Time time_now = base::Time::FromDeltaSinceWindowsEpoch(base::Microseconds(-1));

    std::unique_ptr<BackoffEntry> entry = BackoffEntrySerializer::DeserializeFromList(serialized, &base_policy, &original_ticks, time_now);
    ASSERT_FALSE(entry);
}

TEST(BackoffEntrySerializerTest, SerializeTimeOffsets)
{
    Time original_time = Time::FromMillisecondsSinceUnixEpoch(1430907555111); // May 2015 for realism
    TestTickClock original_ticks;
    BackoffEntry original(&base_policy, &original_ticks);
    // 2 errors.
...

    {
        ...
        std::unique_ptr<BackoffEntry> deserialized = BackoffEntrySerializer::DeserializeFromList(serialized, &base_policy, &original_ticks, later_time);
        ASSERT_TRUE(deserialized.get());
        EXPECT_EQ(original.failure_count(), deserialized->failure_count());
        ...
        EXPECT_GT(original.GetTimeUntilRelease(), deserialized->GetTimeUntilRelease());
        EXPECT_EQ(original.GetReleaseTime() - base::Days(1), deserialized->GetReleaseTime());
    }

    {
        ...
        std::unique_ptr<BackoffEntry> deserialized = BackoffEntrySerializer::DeserializeFromList(serialized, &base_policy, &later_ticks, original_time);
        ASSERT_TRUE(deserialized.get());
        EXPECT_EQ(original.failure_count(), deserialized->failure_count());
        ...
        EXPECT_EQ(original.GetTimeUntilRelease(), deserialized->GetTimeUntilRelease());
        EXPECT_EQ(original.GetReleaseTime() + base::Days(1), deserialized->GetReleaseTime());
    }

    {
        ...
        later_ticks.set_now(TimeTicks() + base::Days(1));
        Time later_time = original_time + base::Days(1);
        std::unique_ptr<BackoffEntry> deserialized = BackoffEntrySerializer::DeserializeFromList(serialized, &base_policy, &later_ticks, later_time);
        ASSERT_TRUE(deserialized.get());
        EXPECT_EQ(original.failure_count(), deserialized->failure_count());
...

    {
        ...
        std::unique_ptr<BackoffEntry> deserialized = BackoffEntrySerializer::DeserializeFromList(serialized, &base_policy, &original_ticks, earlier_time);
        ASSERT_TRUE(deserialized.get());
        EXPECT_EQ(original.failure_count(), deserialized->failure_count());
...
TEST(BackoffEntrySerializerTest, DeserializeUnknownVersion)
{
    ...
    auto deserialized = BackoffEntrySerializer::DeserializeFromList(serialized, &base_policy, nullptr, kParseTime);
