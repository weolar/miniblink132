...
{
    scoped_refptr<base::RefCountedMemory> header(NetModule::GetResource(IDR_DIR_HEADER_HTML));
...
std::string GetDirectoryListingEntry(const std::u16string& name, const std::string& raw_bytes, bool is_dir, int64_t size, base::Time modified)
{
    std::string result;
    ...
    result.append(",");
    if (raw_bytes.empty()) {
        base::EscapeJSONString(base::EscapePath(base::UTF16ToUTF8(name)), true, &result);
    } else {
    ...
    if (modified.is_null()) {
        ...
        raw_time_string_stream << modified.InMillisecondsSinceUnixEpoch() / base::Time::kMillisecondsPerSecond << ",";
