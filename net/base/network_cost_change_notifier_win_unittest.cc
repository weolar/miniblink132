...
{
    fake_network_cost_manager_environment_.SetCost(NetworkChangeNotifier::ConnectionCost::CONNECTION_COST_UNKNOWN);

    TestConnectionCostObserver cost_change_observer;
    auto cost_change_callback = base::BindRepeating(&TestConnectionCostObserver::OnConnectionCostChanged, base::Unretained(&cost_change_observer));

    base::SequenceBound<NetworkCostChangeNotifierWin> cost_change_notifier = NetworkCostChangeNotifierWin::CreateInstance(cost_change_callback);
    ...
    EXPECT_EQ(cost_change_observer.last_cost_changed_input(), NetworkChangeNotifier::ConnectionCost::CONNECTION_COST_UNKNOWN);
}
...
TEST_F(NetworkCostChangeNotifierWinTest, InitialCostKnown)
{
    ...
    EXPECT_EQ(cost_change_observer.last_cost_changed_input(), NetworkChangeNotifier::ConnectionCost::CONNECTION_COST_UNMETERED);
}
...
TEST_F(NetworkCostChangeNotifierWinTest, MultipleCostChangedEvents)
{
    ...
    EXPECT_EQ(cost_change_observer.last_cost_changed_input(), NetworkChangeNotifier::ConnectionCost::CONNECTION_COST_METERED);
...
{
    base::test::ScopedOSInfoOverride os_override(base::test::ScopedOSInfoOverride::Type::kWinServer2016);
