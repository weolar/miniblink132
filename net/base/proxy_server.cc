...
// static
ProxyServer ProxyServer::FromSchemeHostAndPort(Scheme scheme, std::string_view host, std::string_view port_str)
{
    ...
    return FromSchemeHostAndPort(scheme, host, static_cast<uint16_t>(port_number));
}

// static
ProxyServer ProxyServer::FromSchemeHostAndPort(Scheme scheme, std::string_view host, std::optional<uint16_t> port)
{
    // Create INVALID proxies directly using `ProxyServer()`.
    DCHECK_NE(scheme, SCHEME_INVALID);
    ...
    if (!url::CanonicalizeHost(host.data(), url::Component(0, host.size()), &canonicalized_output, &component_output)) {
        return ProxyServer();
    }
