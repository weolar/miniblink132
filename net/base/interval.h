...
public:
    ...
    void CopyFrom(const Interval& i)
    {
        *this = i;
    }
    bool Equals(const Interval& i) const
    {
        return *this == i;
    }
    bool Empty() const
    ...
    {
        return (min_ >= max_ ? min_ : max_) - min_;
    }
...
template <typename T> bool Interval<T>::Intersects(const Interval& i, Interval* out) const
{
    if (!Intersects(i))
        return false;
    if (out != nullptr) {
...
template <typename T> bool Interval<T>::IntersectWith(const Interval& i)
{
    if (Empty())
        return false;
    bool modified = false;
...
template <typename T> bool Interval<T>::SpanningUnion(const Interval& i)
{
    if (i.Empty())
        return false;
    if (Empty()) {
...
template <typename T> bool Interval<T>::Difference(const Interval& i, Interval* lo, Interval* hi) const
{
