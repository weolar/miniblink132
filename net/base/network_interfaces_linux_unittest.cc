...
{
    static_assert(std::size(kIfnameVmnet) < IF_NAMESIZE, "Invalid interface name");
...
TEST(NetworkInterfacesTest, NetworkListTrimmingLinux)
{
    ...
    EXPECT_TRUE(internal::GetNetworkListImpl(&results, INCLUDE_HOST_SCOPE_VIRTUAL_INTERFACES, online_links, address_map, GetInterfaceName));
    ...
    address_map.clear();
    ASSERT_TRUE(address_map.insert(std::pair(ipv6_address, msg)).second);
    EXPECT_TRUE(internal::GetNetworkListImpl(&results, INCLUDE_HOST_SCOPE_VIRTUAL_INTERFACES, online_links, address_map, GetInterfaceNameVM));
    ...
    address_map.clear();
    ASSERT_TRUE(address_map.insert(std::pair(ipv6_address, msg)).second);
    EXPECT_TRUE(internal::GetNetworkListImpl(&results, EXCLUDE_HOST_SCOPE_VIRTUAL_INTERFACES, online_links, address_map, GetInterfaceNameVM));
    EXPECT_EQ(results.size(), 0ul);
    results.clear();
...
TEST(NetworkInterfacesTest, GetWifiSSIDFromInterfaceList)
{
    ...
    EXPECT_EQ(std::string(kWiFiSSID), internal::GetWifiSSIDFromInterfaceListInternal(list, TestGetInterfaceSSID));
