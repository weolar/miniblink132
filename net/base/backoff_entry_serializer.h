...
public:
    ...
    static base::Value::List SerializeToList(const BackoffEntry& entry, base::Time time_now);
    ...
    static std::unique_ptr<BackoffEntry> DeserializeFromList(
        const base::Value::List& serialized, const BackoffEntry::Policy* policy, const base::TickClock* clock, base::Time time_now);
};
