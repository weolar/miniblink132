...
public:
    ElementsUploadDataStream(std::vector<std::unique_ptr<UploadElementReader>> element_readers, int64_t identifier);
    ...
    static std::unique_ptr<UploadDataStream> CreateWithReader(std::unique_ptr<UploadElementReader> reader);

private:
    // UploadDataStream implementation.
    bool IsInMemory() const override;
    const std::vector<std::unique_ptr<UploadElementReader>>* GetElementReaders() const override;
    ...
    // necessary.
    void OnReadElementCompleted(const scoped_refptr<DrainableIOBuffer>& buf, int result);
    ...
    void ProcessReadResult(const scoped_refptr<DrainableIOBuffer>& buf, int result);
