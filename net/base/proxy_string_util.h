...
//   "BLAH xxx:xx"      INVALID
NET_EXPORT ProxyChain PacResultElementToProxyChain(std::string_view pac_result_element);
// TODO(crbug.com/40284947): Remove method once all calls are updated to use
// PacResultElementToProxyChain.
NET_EXPORT ProxyServer PacResultElementToProxyServer(std::string_view pac_result_element);
NET_EXPORT std::string ProxyServerToPacResultElement(const ProxyServer& proxy_server);
...
NET_EXPORT ProxyChain ProxyUriToProxyChain(std::string_view uri, ProxyServer::Scheme default_scheme, bool is_quic_allowed = false);
...
NET_EXPORT ProxyChain MultiProxyUrisToProxyChain(std::string_view uris, ProxyServer::Scheme default_scheme, bool is_quic_allowed = false);
NET_EXPORT ProxyServer ProxyUriToProxyServer(std::string_view uri, ProxyServer::Scheme default_scheme, bool is_quic_allowed = false);
NET_EXPORT std::string ProxyServerToProxyUri(const ProxyServer& proxy_server);
NET_EXPORT ProxyServer ProxySchemeHostAndPortToProxyServer(ProxyServer::Scheme scheme, std::string_view host_and_port);
...
// SCHEME_INVALID.
NET_EXPORT ProxyServer::Scheme GetSchemeFromUriScheme(std::string_view scheme, bool is_quic_allowed = false);
