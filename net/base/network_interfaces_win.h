...
struct NET_EXPORT WlanApi {
    ...
    typedef DWORD(WINAPI* WlanEnumInterfacesFunc)(HANDLE, VOID*, WLAN_INTERFACE_INFO_LIST**);
    typedef DWORD(WINAPI* WlanQueryInterfaceFunc)(HANDLE, const GUID*, WLAN_INTF_OPCODE, VOID*, DWORD*, VOID**, WLAN_OPCODE_VALUE_TYPE*);
    typedef DWORD(WINAPI* WlanSetInterfaceFunc)(HANDLE, const GUID*, WLAN_INTF_OPCODE, DWORD, const VOID*, VOID*);
    ...
    template <typename T> DWORD OpenHandle(DWORD client_version, DWORD* cur_version, T* handle) const
    {
        HANDLE temp_handle;
        DWORD result = open_handle_func(client_version, nullptr, cur_version, &temp_handle);
...
struct WlanApiHandleTraits {
    ...
    {
        return WlanApi::GetInstance().close_handle_func(handle, nullptr) == ERROR_SUCCESS;
    }
...
typedef base::win::GenericScopedHandle<WlanApiHandleTraits, base::win::DummyVerifierTraits> WlanHandle;
...
NET_EXPORT bool GetNetworkListImpl(NetworkInterfaceList* networks, int policy, const IP_ADAPTER_ADDRESSES* ip_adapter_addresses);
