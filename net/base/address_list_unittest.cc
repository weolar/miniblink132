...
TEST(AddressListTest, Canonical)
{
    ...
    EXPECT_THAT(addrlist1.dns_aliases(), UnorderedElementsAre("canonical.bar.com"));

    // Copy the AddressList to another one.
    AddressList addrlist2 = addrlist1;
    EXPECT_THAT(addrlist2.dns_aliases(), UnorderedElementsAre("canonical.bar.com"));
}
...
TEST(AddressListTest, CreateFromIPAddressList)
{
    ...
    AddressList test_list = AddressList::CreateFromIPAddressList(ip_list, std::move(aliases));
...
TEST(AddressListTest, SetDefaultCanonicalNameThenSetDnsAliases)
{
    ...
    EXPECT_THAT(addrlist.dns_aliases(), UnorderedElementsAre("alias1", "alias2", "alias3"));
}
...
TEST(AddressListTest, SetDefaultCanonicalNameThenAppendDnsAliases)
{
    ...
    EXPECT_THAT(addrlist.dns_aliases(), UnorderedElementsAre("1.2.3.4", "alias1", "alias2", "alias3"));
}
...
TEST(AddressListTest, DnsAliases)
{
    ...
    EXPECT_THAT(addrlist.dns_aliases(), UnorderedElementsAre("alias1", "alias2", "alias3", "alias4", "alias5", "alias6"));

    std::vector<std::string> new_aliases({ "alias7", "alias8", "alias9" });
    addrlist.SetDnsAliases(std::move(new_aliases));

    EXPECT_THAT(addrlist.dns_aliases(), UnorderedElementsAre("alias7", "alias8", "alias9"));
}
...
{
    AddressList several;
    several.endpoints() = { IPEndPoint(IPAddress(0, 0, 0, 1), 0), IPEndPoint(IPAddress(0, 0, 0, 2), 0), IPEndPoint(IPAddress(0, 0, 0, 2), 0),
        IPEndPoint(IPAddress(0, 0, 0, 3), 0), IPEndPoint(IPAddress(0, 0, 0, 2), 0), IPEndPoint(IPAddress(0, 0, 0, 1), 0), IPEndPoint(IPAddress(0, 0, 0, 2), 0),
        IPEndPoint(IPAddress(0, 0, 0, 3), 0), IPEndPoint(IPAddress(0, 0, 0, 2), 0) };
    several.Deduplicate();

    // Deduplication should preserve the order of the first instances
    // of the unique addresses.
    EXPECT_THAT(
        several.endpoints(), ElementsAre(IPEndPoint(IPAddress(0, 0, 0, 1), 0), IPEndPoint(IPAddress(0, 0, 0, 2), 0), IPEndPoint(IPAddress(0, 0, 0, 3), 0)));
}
...
{
    std::vector<IPEndPoint> permutation = { IPEndPoint(IPAddress(0, 0, 0, 1), 0), IPEndPoint(IPAddress(0, 0, 0, 1), 0), IPEndPoint(IPAddress(0, 0, 0, 2), 0),
        IPEndPoint(IPAddress(0, 0, 0, 2), 0), IPEndPoint(IPAddress(0, 0, 0, 3), 0) };
