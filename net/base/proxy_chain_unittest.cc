...
{
    std::vector proxy_servers = { ProxyUriToProxyServer("foo:555", ProxyServer::SCHEME_HTTPS), ProxyUriToProxyServer("foo:666", ProxyServer::SCHEME_HTTPS) };
...
{
    ProxyChain proxy_chain = ProxyChain::FromSchemeHostAndPort(ProxyServer::SCHEME_HTTP, "foo", 80);
...
{
    ProxyChain proxy_chain1 = ProxyChain(ProxyUriToProxyServer("foo:333", ProxyServer::SCHEME_SOCKS5));
    ...
    ProxyChain ip_protection_proxy_chain = ProxyChain::ForIpProtection(
        { ProxyUriToProxyServer("foo:444", ProxyServer::SCHEME_HTTPS), ProxyUriToProxyServer("foo:555", ProxyServer::SCHEME_HTTPS) });
    EXPECT_EQ(ip_protection_proxy_chain.ToDebugString(), "[https://foo:444, https://foo:555] (IP Protection)");
...
// builds.
    ...
    ProxyChain proxy_chain2
        = ProxyChain({ ProxyUriToProxyServer("foo:444", ProxyServer::SCHEME_HTTPS), ProxyUriToProxyServer("foo:555", ProxyServer::SCHEME_HTTPS) });
...
TEST(ProxyChainTest, FromSchemeHostAndPort)
{
    const struct {
        ...
        // IPv6 literal
        { ProxyServer::SCHEME_HTTP, "[3ffe:2a00:100:7031::1]", 80, "80", "[3ffe:2a00:100:7031::1]", 80 },
        { ProxyServer::SCHEME_HTTP, "3ffe:2a00:100:7031::1", 80, "80", "[3ffe:2a00:100:7031::1]", 80 },

        // IPv6 literal canonicalization
        { ProxyServer::SCHEME_HTTP, "FEDC:BA98:7654:3210:FEDC:BA98:7654:3210", 80, "80", "[fedc:ba98:7654:3210:fedc:ba98:7654:3210]", 80 },
    ...
    for (size_t i = 0; i < std::size(tests); ++i) {
        SCOPED_TRACE(base::NumberToString(i) + ": " + tests[i].input_host + ":" + base::NumberToString(tests[i].input_port.value_or(-1)));
        auto chain = ProxyChain::FromSchemeHostAndPort(tests[i].input_scheme, tests[i].input_host, tests[i].input_port);
        ...
        auto chain_from_string_port = ProxyChain::FromSchemeHostAndPort(tests[i].input_scheme, tests[i].input_host, tests[i].input_port_str);
...
TEST(ProxyChainTest, InvalidHostname)
{
    ...
    for (size_t i = 0; i < std::size(tests); ++i) {
        ...
        auto proxy = ProxyChain::FromSchemeHostAndPort(ProxyServer::SCHEME_HTTP, tests[i], 80);
...
TEST(ProxyChainTest, InvalidPort)
{
    ...
    for (size_t i = 0; i < std::size(tests); ++i) {
        SCOPED_TRACE(base::NumberToString(i) + ": " + tests[i]);
        auto proxy = ProxyChain::FromSchemeHostAndPort(ProxyServer::SCHEME_HTTP, "foopy", tests[i]);
        EXPECT_FALSE(proxy.IsValid());
    }
}
...
{
    auto proxy_server = ProxyUriToProxyServer("foo:333", ProxyServer::SCHEME_HTTPS);
...
{
    auto proxy_server1 = ProxyUriToProxyServer("foo:333", ProxyServer::SCHEME_HTTPS);
    auto proxy_server2 = ProxyUriToProxyServer("foo:444", ProxyServer::SCHEME_HTTPS);
    auto proxy_server3 = ProxyUriToProxyServer("foo:555", ProxyServer::SCHEME_HTTPS);

    auto chain3 = ProxyChain::ForIpProtection({ proxy_server1, proxy_server2, proxy_server3 });
    EXPECT_EQ(chain3.SplitLast(), std::make_pair(ProxyChain::ForIpProtection({ proxy_server1, proxy_server2 }), proxy_server3));

    auto chain1 = ProxyChain({ proxy_server1 });
    EXPECT_EQ(chain1.SplitLast(), std::make_pair(ProxyChain::Direct(), proxy_server1));
    ...
    EXPECT_EQ(chain2.SplitLast(), std::make_pair(ProxyChain({ proxy_server1 }), proxy_server2));
#endif // BUILDFLAG(ENABLE_BRACKETED_PROXY_URIS)
}
...
TEST(ProxyChainTest, Prefix)
{
    ...
    auto proxy_server2 = ProxyUriToProxyServer("foo:444", ProxyServer::SCHEME_HTTPS);
    auto proxy_server3 = ProxyUriToProxyServer("foo:555", ProxyServer::SCHEME_HTTPS);
    auto chain = ProxyChain::ForIpProtection({ proxy_server1, proxy_server2, proxy_server3 }, /*chain_id=*/2);
    EXPECT_EQ(chain.Prefix(0), ProxyChain::ForIpProtection({}, /*chain_id=*/2));
    EXPECT_EQ(chain.Prefix(1), ProxyChain::ForIpProtection({ proxy_server1 }, /*chain_id=*/2));
    EXPECT_EQ(chain.Prefix(2),
        ProxyChain::ForIpProtection({ proxy_server1, proxy_server2 },
            /*chain_id=*/2));
    EXPECT_EQ(chain.Prefix(3), ProxyChain::ForIpProtection({ proxy_server1, proxy_server2, proxy_server3 }, /*chain_id=*/2));
}
...
TEST(ProxyChainTest, IsForIpProtection)
{
    ...
    auto ip_protection_proxy_chain1 = ProxyChain::ForIpProtection(std::vector<ProxyServer>());
    EXPECT_TRUE(ip_protection_proxy_chain1.is_for_ip_protection());

    auto regular_proxy_chain2
        = ProxyChain({ ProxyUriToProxyServer("foo:555", ProxyServer::SCHEME_HTTPS), ProxyUriToProxyServer("foo:666", ProxyServer::SCHEME_HTTPS) });
    EXPECT_FALSE(regular_proxy_chain2.is_for_ip_protection());

    auto ip_protection_proxy_chain2 = ProxyChain::ForIpProtection(
        { ProxyUriToProxyServer("foo:555", ProxyServer::SCHEME_HTTPS), ProxyUriToProxyServer("foo:666", ProxyServer::SCHEME_HTTPS) });
...
TEST(ProxyChainTest, ForIpProtection)
{
    ...
    EXPECT_TRUE(ip_protection_proxy_chain1.is_for_ip_protection());
    EXPECT_EQ(ip_protection_proxy_chain1.ip_protection_chain_id(), ProxyChain::kDefaultIpProtectionChainId);

    // Ensure that ProxyChain can be reassigned a new value created using its own
    // `proxy_severs()`.
    auto proxy_chain = ProxyChain({ ProxyUriToProxyServer("foo:555", ProxyServer::SCHEME_HTTPS) });
    ...
    EXPECT_EQ(proxy_chain.ip_protection_chain_id(), ProxyChain::kNotIpProtectionChainId);

    // Re-assign new value to `proxy_chain` by using its own proxy servers to
    // create a proxy chain for IP protection.
    proxy_chain = ProxyChain::ForIpProtection(std::move(proxy_chain.proxy_servers()));
    ...
    EXPECT_EQ(proxy_chain.ip_protection_chain_id(), ProxyChain::kDefaultIpProtectionChainId);
...
{
    auto https_server1 = ProxyUriToProxyServer("foo:333", ProxyServer::SCHEME_HTTPS);
    auto https_server2 = ProxyUriToProxyServer("foo:444", ProxyServer::SCHEME_HTTPS);
    auto http_server = ProxyUriToProxyServer("foo:555", ProxyServer::SCHEME_HTTP);
    auto socks_server = ProxyUriToProxyServer("foo:666", ProxyServer::SCHEME_SOCKS4);
    ...
    EXPECT_FALSE(ProxyChain({ https_server1, https_server2 }).is_get_to_proxy_allowed());
}
...
TEST(ProxyChainTest, IsValid)
{
    ...
    EXPECT_TRUE(ProxyChain::ForIpProtection({ quic1, quic2, https1, https2 }).IsValid());
    ...
    EXPECT_CHECK_DEATH(ProxyChain::ForIpProtection({ https1, https2, quic1, quic2 }).IsValid());
    // Socks proxy server is not valid for multi-proxy chain.
    EXPECT_CHECK_DEATH(ProxyChain::ForIpProtection({ socks, https1 }).IsValid());
    EXPECT_CHECK_DEATH(ProxyChain::ForIpProtection({ socks, https1, https2 }).IsValid());
    EXPECT_CHECK_DEATH(ProxyChain::ForIpProtection({ https1, socks }).IsValid());
    EXPECT_CHECK_DEATH(ProxyChain::ForIpProtection({ https1, https2, socks }).IsValid());
    ...
    EXPECT_EQ(ProxyChain({ https1, https2 }).IsValid(), multi_proxy_chain_supported);
...
#if !BUILDFLAG(ENABLE_BRACKETED_PROXY_URIS) || !BUILDFLAG(ENABLE_QUIC_PROXY_SUPPORT)
    bool is_multi_proxy_quic_supported = false;
#else
    bool is_multi_proxy_quic_supported = true;
#endif
    EXPECT_EQ(ProxyChain({ quic1, https1 }).IsValid(), is_multi_proxy_quic_supported);
    EXPECT_EQ(ProxyChain({ quic1, quic2, https1, https2 }).IsValid(), is_multi_proxy_quic_supported);
}
...
TEST(ProxyChainTest, Unequal)
{
    ...
    std::vector<ProxyChain> proxy_chain_list
        = { ProxyChain::Direct(), ProxyUriToProxyChain("foo:333", ProxyServer::SCHEME_HTTP), ProxyUriToProxyChain("foo:444", ProxyServer::SCHEME_HTTP),
              ProxyChain({ ProxyUriToProxyServer("foo:555", ProxyServer::SCHEME_HTTPS), ProxyUriToProxyServer("foo:666", ProxyServer::SCHEME_HTTPS) }),
              ProxyUriToProxyChain("socks4://foo:33", ProxyServer::SCHEME_SOCKS4), ProxyUriToProxyChain("http://foo:33", ProxyServer::SCHEME_HTTP),
              ProxyChain({ ProxyUriToProxyChain("bar:33", ProxyServer::SCHEME_HTTP) }),
              ProxyChain::ForIpProtection(
                  { ProxyUriToProxyServer("foo:555", ProxyServer::SCHEME_HTTPS), ProxyUriToProxyServer("foo:666", ProxyServer::SCHEME_HTTPS) }) };

    // Unordered proxy chains.
    std::set<ProxyChain> proxy_chain_set(proxy_chain_list.begin(), proxy_chain_list.end());
    ...
    for (const ProxyChain& proxy_chain1 : proxy_chain_list) {
        ...
        while (proxy_chain2 != proxy_chain_set.end() && proxy_chain1 < *proxy_chain2) {
            EXPECT_FALSE(*proxy_chain2 < proxy_chain1);
            EXPECT_TRUE(proxy_chain1 < *proxy_chain2);
            EXPECT_FALSE(*proxy_chain2 == proxy_chain1);
            EXPECT_FALSE(proxy_chain1 == *proxy_chain2);
...
{
    ProxyServer proxy_server = ProxyUriToProxyServer("foo:11", ProxyServer::SCHEME_HTTP);

    ProxyChain proxy_chain1 = ProxyChain(proxy_server);
    ProxyChain proxy_chain2 = ProxyChain(std::vector<ProxyServer> { proxy_server });
    ProxyChain proxy_chain3 = ProxyChain(ProxyServer::SCHEME_HTTP, HostPortPair("foo", 11));
...
{
    ProxyChain proxy_chain = ProxyChain(ProxyUriToProxyServer("foo:11", ProxyServer::SCHEME_HTTPS));
    base::Pickle pickle;
    proxy_chain.Persist(&pickle);
    base::PickleIterator iter(pickle);
    ProxyChain proxy_chain_from_pickle;
...
TEST(ProxyChainTest, MultiProxyChainNotForIpProtectionInvalidProxyChain)
{
    ProxyChain invalid_chain
        = ProxyChain({ ProxyUriToProxyServer("foo:11", ProxyServer::SCHEME_HTTPS), ProxyUriToProxyServer("hoo:11", ProxyServer::SCHEME_HTTPS) });
...
TEST(ProxyChainTest, MultiProxyChain)
{
    ...
    std::vector<ProxyServer> proxy_servers = { proxy_server1, proxy_server2, proxy_server3 };
    auto proxy = ProxyChain(proxy_servers);
    ...
    auto ip_protection_proxy_chain = ProxyChain::ForIpProtection({ proxy_server1, proxy_server2 });
    EXPECT_TRUE(ip_protection_proxy_chain.is_for_ip_protection());
    EXPECT_EQ(regular_proxy_chain.proxy_servers(), ip_protection_proxy_chain.proxy_servers());
}
...
TEST(ProxyChainTest, MultiProxyChainsCanBeConvertedToForIpProtection)
{
    ...
    EXPECT_EQ(proxy_chain.ip_protection_chain_id(), ProxyChain::kDefaultIpProtectionChainId);
    EXPECT_FALSE(copied_proxy_chain.is_for_ip_protection());
    EXPECT_EQ(copied_proxy_chain.ip_protection_chain_id(), ProxyChain::kNotIpProtectionChainId);
...
{
    ProxyChain proxy_chain
        = ProxyChain({ ProxyUriToProxyServer("foo:11", ProxyServer::SCHEME_HTTPS), ProxyUriToProxyServer("foo:22", ProxyServer::SCHEME_HTTPS) });
    base::Pickle pickle;
    proxy_chain.Persist(&pickle);
    base::PickleIterator iter(pickle);
    ProxyChain proxy_chain_from_pickle;
