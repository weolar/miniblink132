...
{
    return (body.length() % 4) == 0 && base::ranges::none_of(body, [](char c) { return c == '%' || IsBase64Whitespace(c); });
}

} // namespace

bool DataURL::Parse(const GURL& url, std::string* mime_type, std::string* charset, std::string* data)
{
    ...
    std::vector<std::string_view> meta_data
        = base::SplitStringPiece(base::MakeStringPiece(content.begin(), comma), ";", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL);
    ...
    for (; iter != meta_data.cend(); ++iter) {
        if (!base64_encoded && base::EqualsCaseInsensitiveASCII(*iter, kBase64Tag)) {
            base64_encoded = true;
        } else if (charset_value.empty() && base::StartsWith(*iter, kCharsetTag, base::CompareCase::INSENSITIVE_ASCII)) {
    ...
    } else if (!ParseMimeTypeWithoutParameter(mime_type_value, nullptr, nullptr)) {
        ...
        if (base64_encoded) {
            if (base::FeatureList::IsEnabled(features::kOptimizeParsingDataUrls)) {
                ...
                if (!base::Base64Decode(raw_body, data)) {
                    ...
                    if (!base::Base64Decode(unescaped_body, data, base::Base64DecodePolicy::kForgiving)) {
                        return false;
                    }
                }
            } else {
            ...
            // command line flag.
            if (!base::FeatureList::IsEnabled(features::kKeepWhitespaceForDataUrls) || HasRemoveWhitespaceCommandLineFlag()) {
                if (!(mime_type_value.compare(0, 5, "text/") == 0 || mime_type_value.find("xml") != std::string::npos)) {
...
Error DataURL::BuildResponse(
    const GURL& url, std::string_view method, std::string* mime_type, std::string* charset, std::string* data, scoped_refptr<HttpResponseHeaders>* headers)
{
    DCHECK(data);
    DCHECK(!*headers);
    ...
    if (base::FeatureList::IsEnabled(features::kOptimizeParsingDataUrls)) {
        ...
        *headers = HttpResponseHeaders::TryToCreate("HTTP/1.1 200 OK\r\n"
                                                    "Content-Type:"
            + content_type);
    }
