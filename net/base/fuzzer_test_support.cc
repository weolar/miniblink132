...
struct InitGlobals {
    InitGlobals()
    {
        ...
        task_environment = std::make_unique<base::test::TaskEnvironment>(base::test::TaskEnvironment::MainThreadType::IO);

        increased_timeout_ = std::make_unique<base::test::ScopedRunLoopTimeout>(FROM_HERE, TestTimeouts::action_max_timeout());
