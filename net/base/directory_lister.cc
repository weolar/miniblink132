...
// Static.
bool CompareAlphaDirsFirst(const DirectoryLister::DirectoryListerData& a, const DirectoryLister::DirectoryListerData& b)
{
    ...
    return base::i18n::LocaleAwareCompareFilenames(a.info.GetName(), b.info.GetName());
}

void SortData(std::vector<DirectoryLister::DirectoryListerData>* data, DirectoryLister::ListingType listing_type)
{
...
DirectoryLister::DirectoryLister(const base::FilePath& dir, DirectoryListerDelegate* delegate)
    : DirectoryLister(dir, ALPHA_DIRS_FIRST, delegate)
{
}

DirectoryLister::DirectoryLister(const base::FilePath& dir, ListingType type, DirectoryListerDelegate* delegate)
    : delegate_(delegate)
{
...
{
    base::ThreadPool::PostTask(FROM_HERE, { base::MayBlock(), base::TaskShutdownBehavior::CONTINUE_ON_SHUTDOWN }, base::BindOnce(&Core::Start, core_));
}
...
DirectoryLister::Core::Core(const base::FilePath& dir, ListingType type, DirectoryLister* lister)
    : dir_(dir)
    , type_(type)
...
void DirectoryLister::Core::Start()
{
    ...
    if (!base::DirectoryExists(dir_)) {
        origin_task_runner_->PostTask(FROM_HERE, base::BindOnce(&Core::DoneOnOriginSequence, this, std::move(directory_list), ERR_FILE_NOT_FOUND));
        return;
    }
    ...
    */
    ...
    origin_task_runner_->PostTask(FROM_HERE, base::BindOnce(&Core::DoneOnOriginSequence, this, std::move(directory_list), OK));
}
...
void DirectoryLister::Core::DoneOnOriginSequence(std::unique_ptr<DirectoryList> directory_list, int error) const
{
    DCHECK(origin_task_runner_->RunsTasksInCurrentSequence());
