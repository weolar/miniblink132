...
template <typename T> bool ParseIntHelper(std::string_view input, ParseIntFormat format, T* output, ParseIntError* optional_error)
{
    ...
    bool is_non_negative = (format == ParseIntFormat::NON_NEGATIVE || format == ParseIntFormat::STRICT_NON_NEGATIVE);
    bool is_strict = (format == ParseIntFormat::STRICT_NON_NEGATIVE || format == ParseIntFormat::STRICT_OPTIONALLY_NEGATIVE);
    ...
    std::string_view numeric_portion = starts_with_negative ? input.substr(1) : input;
    ...
    if (!numeric_portion.empty() && numeric_portion.find_first_not_of("0123456789") == std::string::npos) {
        ...
        return SetError(starts_with_negative ? ParseIntError::FAILED_UNDERFLOW : ParseIntError::FAILED_OVERFLOW, optional_error);
    }
...
bool ParseInt32(std::string_view input, ParseIntFormat format, int32_t* output, ParseIntError* optional_error)
{
    return ParseIntHelper(input, format, output, optional_error);
}

bool ParseInt64(std::string_view input, ParseIntFormat format, int64_t* output, ParseIntError* optional_error)
{
    return ParseIntHelper(input, format, output, optional_error);
}

bool ParseUint32(std::string_view input, ParseIntFormat format, uint32_t* output, ParseIntError* optional_error)
{
    CHECK(format == ParseIntFormat::NON_NEGATIVE || format == ParseIntFormat::STRICT_NON_NEGATIVE);
    return ParseIntHelper(input, format, output, optional_error);
}

bool ParseUint64(std::string_view input, ParseIntFormat format, uint64_t* output, ParseIntError* optional_error)
{
    CHECK(format == ParseIntFormat::NON_NEGATIVE || format == ParseIntFormat::STRICT_NON_NEGATIVE);
    return ParseIntHelper(input, format, output, optional_error);
}
