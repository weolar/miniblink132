...
TEST(UrlUtilTest, AppendQueryParameter)
{
    ...
    EXPECT_EQ("http://example.com/path?name=value", AppendQueryParameter(GURL("http://example.com/path"), "name", "value").spec());
    ...
    // appended with '&'.
    EXPECT_EQ("http://example.com/path?existing=one&name=value", AppendQueryParameter(GURL("http://example.com/path?existing=one"), "name", "value").spec());

    // Appending a name-value pair with unsafe characters included. The
    // unsafe characters should be escaped.
    EXPECT_EQ(
        "http://example.com/path?existing=one&na+me=v.alue%3D", AppendQueryParameter(GURL("http://example.com/path?existing=one"), "na me", "v.alue=").spec());
}

TEST(UrlUtilTest, AppendOrReplaceQueryParameter)
{
    // Appending a name-value pair to a URL without a query component.
    EXPECT_EQ("http://example.com/path?name=value", AppendOrReplaceQueryParameter(GURL("http://example.com/path"), "name", "value").spec());

    // Appending a name-value pair to a URL with a query component.
    // The original component should be preserved, and the new pair should be
    // appended with '&'.
    EXPECT_EQ(
        "http://example.com/path?existing=one&name=value", AppendOrReplaceQueryParameter(GURL("http://example.com/path?existing=one"), "name", "value").spec());

    // Appending a name-value pair with unsafe characters included. The
    // unsafe characters should be escaped.
    EXPECT_EQ("http://example.com/path?existing=one&na+me=v.alue%3D",
        AppendOrReplaceQueryParameter(GURL("http://example.com/path?existing=one"), "na me", "v.alue=").spec());

    // Replace value of an existing paramater.
    EXPECT_EQ("http://example.com/path?existing=one&name=new",
        AppendOrReplaceQueryParameter(GURL("http://example.com/path?existing=one&name=old"), "name", "new").spec());

    // Replace a name-value pair with unsafe characters included. The
    // unsafe characters should be escaped.
    EXPECT_EQ("http://example.com/path?na+me=n.ew%3D&existing=one",
        AppendOrReplaceQueryParameter(GURL("http://example.com/path?na+me=old&existing=one"), "na me", "n.ew=").spec());

    // Replace the value of first parameter with this name only.
    EXPECT_EQ("http://example.com/path?name=new&existing=one&name=old",
        AppendOrReplaceQueryParameter(GURL("http://example.com/path?name=old&existing=one&name=old"), "name", "new").spec());
    ...
    EXPECT_EQ("http://example.com/path?bar&name=new&left=&"
              "=right&=&&name=again",
        AppendOrReplaceQueryParameter(GURL("http://example.com/path?bar&name=old&left=&"
                                           "=right&=&&name=again"),
            "name", "new")
            .spec());
    ...
    EXPECT_EQ(
        "http://example.com/path?abc=xyz", AppendOrReplaceQueryParameter(GURL("http://example.com/path?name=value&abc=xyz"), "name", std::nullopt).spec());

    // Removes the name-value pair from the URL.
    EXPECT_EQ("http://example.com/path?", AppendOrReplaceQueryParameter(GURL("http://example.com/path?existing=one"), "existing", std::nullopt).spec());

    // Removes the first name-value pair.
    EXPECT_EQ("http://example.com/path?c=d&e=f", AppendOrReplaceQueryParameter(GURL("http://example.com/path?a=b&c=d&e=f"), "a", std::nullopt).spec());

    // Removes a name-value pair in between two query params.
    EXPECT_EQ("http://example.com/path?existing=one&hello=world",
        AppendOrReplaceQueryParameter(GURL("http://example.com/path?existing=one&replace=sure&hello=world"), "replace", std::nullopt).spec());

    // Removes the last name-value pair.
    EXPECT_EQ("http://example.com/path?existing=one",
        AppendOrReplaceQueryParameter(GURL("http://example.com/path?existing=one&replace=sure"), "replace", std::nullopt).spec());

    // Removing a name-value pair with unsafe characters included. The
    // unsafe characters should be escaped.
    EXPECT_EQ("http://example.com/path?existing=one&hello=world",
        AppendOrReplaceQueryParameter(GURL("http://example.com/"
    ...
    EXPECT_EQ("http://example.com/path?existing=one&name=old",
        AppendOrReplaceQueryParameter(GURL("http://example.com/path?existing=one&name=old"), "old", std::nullopt).spec());

    // Remove the value of first parameter with this name only.
    EXPECT_EQ("http://example.com/path?existing=one&name=old",
        AppendOrReplaceQueryParameter(GURL("http://example.com/path?name=something&existing=one&name=old"), "name", std::nullopt).spec());

    // Preserve the content of the original params regardless of our failure to
    // interpret them correctly.
    EXPECT_EQ("http://example.com/path?bar&left=&"
              "=right&=&&name=again",
        AppendOrReplaceQueryParameter(GURL("http://example.com/path?bar&name=old&left=&"
                                           "=right&=&&name=again"),
            "name", std::nullopt)
...
TEST(UrlUtilTest, AppendOrReplaceRef)
{
    ...
    EXPECT_EQ("http://example.com/path#ref", AppendOrReplaceRef(GURL("http://example.com/path"), "ref").spec());

    // Setting a ref over an existing one should replace it.
    EXPECT_EQ("http://example.com/path#ref", AppendOrReplaceRef(GURL("http://example.com/path#old_ref"), "ref").spec());

    // Setting a ref on a url with existing query parameters should simply append
    // it at the end
    EXPECT_EQ("http://example.com/path?query=value#ref", AppendOrReplaceRef(GURL("http://example.com/path?query=value#ref"), "ref").spec());
    ...
    EXPECT_EQ("http://example.com/special-chars?query=value#ref%20chars%%22;'",
        AppendOrReplaceRef(GURL("http://example.com/special-chars?query=value"), "ref chars%\";'").spec());

    // Testing adding a ref to a URL with specially encoded characters.
    // `special chars%\";'?query=value#ref`
    EXPECT_EQ("http://example.com/special%20chars%%22;'?query=value#ref",
        AppendOrReplaceRef(GURL("http://example.com/special chars%\";'?query=value"), "ref").spec());
}
...
TEST(UrlUtilTest, ParseHostAndPort)
{
    const struct {
        ...
        { "foo:10", true, "foo", 10 },
        { "foo", true, "foo", -1 },
        { "[1080:0:0:0:8:800:200C:4171]:11", true, "1080:0:0:0:8:800:200C:4171", 11 },
        { "[1080:0:0:0:8:800:200C:4171]", true, "1080:0:0:0:8:800:200C:4171", -1 },
...
TEST(UrlUtilTest, GetHostAndOptionalPort)
{
    ...
    for (const auto& test : tests) {
        ...
        EXPECT_EQ(test.expected_host_and_port, GetHostAndOptionalPort(url::SchemeHostPort(test.url)));
    }
}

TEST(UrlUtilTest, GetHostOrSpecFromURL)
{
    EXPECT_EQ("example.com", GetHostOrSpecFromURL(GURL("http://example.com/test")));
    EXPECT_EQ("example.com", GetHostOrSpecFromURL(GURL("http://example.com./test")));
    EXPECT_EQ("file:///tmp/test.html", GetHostOrSpecFromURL(GURL("file:///tmp/test.html")));
}
...
TEST(UrlUtilTest, IsSubdomainOf)
{
    struct {
        ...
        { "bar.foo.com", "foo.com", true }, { "barfoo.com", "foo.com", false }, { "bar.foo.com", "com", true }, { "bar.foo.com", "other.com", false },
        { "bar.foo.com", "bar.foo.com", true }, { "bar.foo.com", "baz.foo.com", false }, { "bar.foo.com", "baz.bar.foo.com", false },
        { "bar.foo.com", "ar.foo.com", false }, { "foo.com", "foo.com.", false }, { "bar.foo.com", "foo.com.", false }, { "", "", true }, { "a", "", false },
        { "", "a", false }, { "127.0.0.1", "0.0.1", true }, // Don't do this...
    };

    for (const auto& test : tests) {
        EXPECT_EQ(test.is_subdomain, IsSubdomainOf(test.subdomain, test.superdomain));
    }
}
...
TEST(UrlUtilTest, CompliantHost)
{
    struct {
        ...
        { "123456789a123456789a123456789a123456789a123456789a123456789a1234", false },
        { "123456789a123456789a123456789a123456789a123456789a123456789a1234.", false },
    ...
    for (const auto& compliant_host : compliant_host_cases) {
        EXPECT_EQ(compliant_host.expected_output, IsCanonicalizedHostCompliant(compliant_host.host)) << compliant_host.host;
    }
}
...
void PrintTo(const NonUniqueNameTestData& data, std::ostream* os)
{
    ...
    *os << " hostname: " << testing::PrintToString(data.hostname) << "; is_unique: " << testing::PrintToString(data.is_unique);
}
...
class UrlUtilNonUniqueNameTest : public testing::TestWithParam<NonUniqueNameTestData> {
public:
...
INSTANTIATE_TEST_SUITE_P(All, UrlUtilNonUniqueNameTest, testing::ValuesIn(kNonUniqueNameTestData));
...
public:
        ...
        : use_standard_compliant_non_special_scheme_url_parsing_(GetParam())
    {
        if (use_standard_compliant_non_special_scheme_url_parsing_) {
            scoped_feature_list_.InitAndEnableFeature(url::kStandardCompliantNonSpecialSchemeURLParsing);
        } else {
            scoped_feature_list_.InitAndDisableFeature(url::kStandardCompliantNonSpecialSchemeURLParsing);
        }
    }
...
TEST(UrlUtilTest, ChangeWebSocketSchemeToHttpScheme)
{
    ...
    } tests[] = { { "ws://google.com:78/path?query=1", "http://google.com:78/path?query=1" },
    ...
    for (const auto& test : tests) {
        ...
        EXPECT_EQ(expected_output_url, ChangeWebSocketSchemeToHttpScheme(input_url));
    }
}
...
TEST(UrlUtilTest, SchemeHasNetworkHost)
{
    ...
    AddStandardScheme(kCustomSchemeWithHostPortAndUserInformation, url::SCHEME_WITH_HOST_PORT_AND_USER_INFORMATION);
    AddStandardScheme(kCustomSchemeWithHostAndPort, url::SCHEME_WITH_HOST_AND_PORT);
    AddStandardScheme(kCustomSchemeWithHost, url::SCHEME_WITH_HOST);
    AddStandardScheme(kCustomSchemeWithoutAuthority, url::SCHEME_WITHOUT_AUTHORITY);
    ...
    EXPECT_TRUE(IsStandardSchemeWithNetworkHost(kCustomSchemeWithHostPortAndUserInformation));
...
TEST(UrlUtilTest, GoogleHostWithAlpnH3)
{
    struct {
        ...
        { "google.CoM", true },
        { "www.Google.cOm", true },
        { "www.google.cat", false },
        { "www.google.co.in", false },
