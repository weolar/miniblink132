...
public:
    ...
    ProxyChain& operator=(ProxyChain&& other) noexcept; // Move assignment operator
    ...
    // ProxyChain.
    static ProxyChain FromSchemeHostAndPort(ProxyServer::Scheme scheme, std::string_view host, std::string_view port_str)
    {
        return ProxyChain(ProxyServer::FromSchemeHostAndPort(scheme, host, port_str));
    }
    static ProxyChain FromSchemeHostAndPort(ProxyServer::Scheme scheme, std::string_view host, std::optional<uint16_t> port)
    {
    ...
    {
        return ProxyChain(std::vector<ProxyServer>());
    }
    ...
    static ProxyChain ForIpProtection(std::vector<ProxyServer> proxy_server_list, int chain_id = 0)
    {
    ...
    const std::optional<std::vector<ProxyServer>>& proxy_servers_if_valid() const
    {
    ...
    size_t length() const
    {
        return proxy_server_list_.has_value() ? proxy_server_list_.value().size() : 0;
    }

    // Returns true if this chain contains more than one proxy.
    bool is_multi_proxy() const
    {
        return proxy_server_list_.has_value() ? proxy_server_list_.value().size() > 1 : false;
    }
    ...
    {
        return proxy_server_list_.has_value() ? proxy_server_list_.value().empty() : false;
    }

    template <class Predicate> bool AnyProxy(Predicate p) const
    {
        return proxy_server_list_.has_value() && std::any_of(proxy_server_list_->begin(), proxy_server_list_->end(), p);
    }
    ...
    int ip_protection_chain_id() const
    {
        return ip_protection_chain_id_;
    }

    bool operator==(const ProxyChain& other) const
    {
        return std::tie(proxy_server_list_, ip_protection_chain_id_) == std::tie(other.proxy_server_list_, other.ip_protection_chain_id_);
    }
...
private:
    explicit ProxyChain(std::vector<ProxyServer> proxy_server_list, int ip_protection_chain_id);
...
NET_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, const ProxyChain& proxy_chain);
