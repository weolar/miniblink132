...
TEST(MimeSnifferTest, PlainTextPerfTest)
{
    ...
    size_t expected_size = plaintext.size() << base::bits::Log2Ceiling(kTargetSize / plaintext.size());
    ...
    reporter.RegisterImportantMetric("throughput", "bytesPerSecond_biggerIsBetter");
    reporter.AddResult("throughput", static_cast<int64_t>(plaintext.size()) * kMeasuredIterations / elapsed_timer.Elapsed().InSecondsF());
}
