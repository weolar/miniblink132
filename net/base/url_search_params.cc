...
UrlSearchParams::UrlSearchParams(const GURL& url)
{
    for (auto it = QueryIterator(url); !it.IsAtEnd(); it.Advance()) {
        ...
        params_.emplace_back(UnescapePercentEncodedUrl(it.GetKey()), UnescapePercentEncodedUrl(it.GetValue()));
    }
}
...
void UrlSearchParams::DeleteAllWithNames(const base::flat_set<std::string>& names)
{
    std::erase_if(params_, [&](const auto& pair) { return names.contains(pair.first); });
}

void UrlSearchParams::DeleteAllExceptWithNames(const base::flat_set<std::string>& names)
{
    std::erase_if(params_, [&](const auto& pair) { return !names.contains(pair.first); });
}

const std::vector<std::pair<std::string, std::string>>& UrlSearchParams::params() const
{
    return params_;
}
