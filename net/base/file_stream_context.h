...
public:
    ...
    bool async_in_progress() const
    {
        return async_in_progress_;
    }
    ...
    void Open(const base::FilePath& path, int open_flags, CompletionOnceCallback callback);
    ...
    void GetFileInfo(base::File::Info* file_info, CompletionOnceCallback callback);
...
private:
    ...
    // network error code.
    void OnAsyncCompleted(Int64CompletionOnceCallback callback, const IOResult& result);
    ...
    void OnIOCompleted(base::MessagePumpForIO::IOContext* context, DWORD bytes_read, DWORD error) override;
    ...
    static void ReadAsync(FileStream::Context* context, HANDLE file, scoped_refptr<IOBuffer> buf, int buf_len, OVERLAPPED* overlapped,
