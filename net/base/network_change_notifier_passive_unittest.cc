...
public:
        ...
        : task_environment_(base::test::TaskEnvironment::TimeSource::MOCK_TIME)
    {
        ...
        notifier_ = base::WrapUnique(
            new NetworkChangeNotifierPassive(NetworkChangeNotifier::CONNECTION_UNKNOWN, NetworkChangeNotifier::SUBTYPE_UNKNOWN, dns_config_notifier_.get()));
        auto dns_config_service = std::make_unique<TestDnsConfigService>();
        dns_config_service_ = dns_config_service.get();
        dns_config_notifier_->SetDnsConfigServiceForTesting(std::move(dns_config_service), base::OnceClosure());
    }
    ...
    NetworkChangeNotifierPassive* notifier()
    {
        return notifier_.get();
    }
    TestDnsConfigService* dns_config_service()
    {
        return dns_config_service_;
    }
...
class MockNetworkChangeObserver : public NetworkChangeNotifier::NetworkChangeObserver {
public:
...
TEST_F(NetworkChangeNotifierPassiveTest, OnNetworkChanged)
{
    ...
    EXPECT_CALL(observer, OnNetworkChanged(NetworkChangeNotifier::CONNECTION_NONE));
...
class MockMaxBandwidthObserver : public NetworkChangeNotifier::MaxBandwidthObserver {
public:
    MOCK_METHOD2(OnMaxBandwidthChanged, void(double, NetworkChangeNotifier::ConnectionType));
};
...
TEST_F(NetworkChangeNotifierPassiveTest, OnMaxBandwidthChanged)
{
    ...
    EXPECT_CALL(observer, OnMaxBandwidthChanged(3.6, NetworkChangeNotifier::CONNECTION_4G));
    notifier()->OnConnectionSubtypeChanged(NetworkChangeNotifier::CONNECTION_4G, NetworkChangeNotifier::SUBTYPE_HSPA);
    FastForwardUntilIdle();
