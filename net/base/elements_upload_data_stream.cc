...
ElementsUploadDataStream::ElementsUploadDataStream(std::vector<std::unique_ptr<UploadElementReader>> element_readers, int64_t identifier)
...
std::unique_ptr<UploadDataStream> ElementsUploadDataStream::CreateWithReader(std::unique_ptr<UploadElementReader> reader)
{
...
const std::vector<std::unique_ptr<UploadElementReader>>* ElementsUploadDataStream::GetElementReaders() const
{
...
int ElementsUploadDataStream::InitElements(size_t start_index)
{
    ...
    for (size_t i = start_index; i < element_readers_.size(); ++i) {
        ...
        int result = reader->Init(base::BindOnce(&ElementsUploadDataStream::OnInitElementCompleted, weak_ptr_factory_.GetWeakPtr(), i));
...
void ElementsUploadDataStream::OnInitElementCompleted(size_t index, int result)
{
...
int ElementsUploadDataStream::ReadElements(const scoped_refptr<DrainableIOBuffer>& buf)
{
    while (read_error_ == OK && element_index_ < element_readers_.size()) {
        ...
        int result = reader->Read(
            buf.get(), buf->BytesRemaining(), base::BindOnce(&ElementsUploadDataStream::OnReadElementCompleted, weak_ptr_factory_.GetWeakPtr(), buf));
        if (result == ERR_IO_PENDING)
...
void ElementsUploadDataStream::OnReadElementCompleted(const scoped_refptr<DrainableIOBuffer>& buf, int result)
{
    ProcessReadResult(buf, result);
...
void ElementsUploadDataStream::ProcessReadResult(const scoped_refptr<DrainableIOBuffer>& buf, int result)
{
    DCHECK_NE(ERR_IO_PENDING, result);
    DCHECK(!read_error_);
