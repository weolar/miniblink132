...
class DataURLTest : public testing::Test, public ::testing::WithParamInterface<std::tuple<bool, bool>> {
public:
    DataURLTest()
    {
        ...
        const auto feature_set = [&](bool flag_on) -> FeatureList& { return flag_on ? enabled_features : disabled_features; };
        feature_set(OptimizedParsing()).push_back(features::kOptimizeParsingDataUrls);
        feature_set(KeepWhitespace()).push_back(features::kKeepWhitespaceForDataUrls);
        feature_list_.InitWithFeatures(enabled_features, disabled_features);
    }

    bool OptimizedParsing() const
    {
        return std::get<0>(GetParam());
    }
    bool KeepWhitespace() const
    {
...
INSTANTIATE_TEST_SUITE_P(DataURLTest, DataURLTest,
    testing::Combine(
...
TEST_P(DataURLTest, Parse)
{
    const ParseTestData tests[] = {
        ...
        { "data:;base64,aGVsbG8gd29ybGQ=", true, "text/plain", "US-ASCII", "hello world" },
        ...
        { "data:f(oo/bar;baz=1;charset=kk,boo", true, "text/plain", "US-ASCII", "boo" },
        ...
        { "data:text/html,<html><body><b>hello world</b></body></html>", true, "text/html", "", "<html><body><b>hello world</b></body></html>" },
        ...
        { "data:image/fractal,a b c d e f g", true, "image/fractal", "", KeepWhitespace() ? "a b c d e f g" : "abcdefg" },

        // Spaces should also be removed from anything base-64 encoded
        { "data:;base64,aGVs bG8gd2  9ybGQ=", true, "text/plain", "US-ASCII", "hello world" },

        // Other whitespace should also be removed from anything base-64 encoded.
        { "data:;base64,aGVs bG8gd2  \n9ybGQ=", true, "text/plain", "US-ASCII", "hello world" },
        ...
        { "data:img/png,A  B  %20  %0A  C", true, "img/png", "", KeepWhitespace() ? "A  B     \n  C" : "AB \nC" },

        { "data:text/plain;charset=utf-8;base64,SGVsbMO2", true, "text/plain", "utf-8", "Hell\xC3\xB6" },

        // no mimetype
        { "data:;charset=utf-8;base64,SGVsbMO2", true, "text/plain", "utf-8", "Hell\xC3\xB6" },

        // Not sufficiently padded.
        { "data:;base64,aGVsbG8gd29ybGQ", true, "text/plain", "US-ASCII", "hello world" },

        // Not sufficiently padded with whitespace.
        { "data:;base64,aGV sbG8g d29ybGQ", true, "text/plain", "US-ASCII", "hello world" },

        // Not sufficiently padded with escaped whitespace.
        { "data:;base64,aGV%20sbG8g%20d29ybGQ", true, "text/plain", "US-ASCII", "hello world" },
        ...
        { "data:text/plain;charset=utf-8,\xE2\x80\x8Ftest", true, "text/plain", "utf-8", "\xE2\x80\x8Ftest" },
        ...
        { "data:text/plain;charset=utf-8,%E2%80%8Ftest", true, "text/plain", "utf-8", "\xE2\x80\x8Ftest" },
        ...
        { "data:text/plain,this/is/a/test/%23include/#dontinclude", true, "text/plain", "", "this/is/a/test/#include/" },

        // More unescaping tests and tests with nulls.
        { "data:%00text/plain%41,foo", true, "%00text/plain%41", "", "foo" },
        { "data:text/plain;charset=%00US-ASCII%41,foo", true, "text/plain", "%00US-ASCII%41", "foo" },
        { "data:text/plain,%00_%41", true, "text/plain", "", std::string("\x00_A", 3) },
        { "data:text/plain;base64,AA//", true, "text/plain", "", std::string("\x00\x0F\xFF", 3) },
...
TEST_P(DataURLTest, BuildResponseSimple)
{
    ...
    ASSERT_EQ(OK, DataURL::BuildResponse(GURL("data:,Hello"), "GET", &mime_type, &charset, &data, &headers));
    ...
    EXPECT_EQ(headers->GetNormalizedHeader("Content-Type"), "text/plain;charset=US-ASCII");
}
...
TEST_P(DataURLTest, BuildResponseHead)
{
    for (const char* method : { "HEAD", "head", "hEaD" }) {
        ...
        std::string data;
        scoped_refptr<HttpResponseHeaders> headers;
        ASSERT_EQ(OK, DataURL::BuildResponse(GURL("data:,Hello"), method, &mime_type, &charset, &data, &headers));
...
TEST_P(DataURLTest, BuildResponseInput)
{
    ...
    ASSERT_EQ(ERR_INVALID_URL, DataURL::BuildResponse(GURL("bogus"), "GET", &mime_type, &charset, &data, &headers));
...
TEST_P(DataURLTest, BuildResponseInvalidMimeType)
{
    ...
    ASSERT_EQ(OK, DataURL::BuildResponse(GURL("data:f(o/b)r,test"), "GET", &mime_type, &charset, &data, &headers));
...
TEST_P(DataURLTest, InvalidCharset)
{
    ...
    ASSERT_EQ(ERR_INVALID_URL, DataURL::BuildResponse(GURL("data:text/html;charset=(),test"), "GET", &mime_type, &charset, &data, &headers));
    EXPECT_FALSE(headers);
    EXPECT_TRUE(mime_type.empty());
    EXPECT_TRUE(charset.empty());
    EXPECT_TRUE(data.empty());
...
TEST_P(DataURLTest, Image)
{
    ...
    GURL image_url("data:image/png;base64,"
    ...
    EXPECT_EQ(OK, DataURL::BuildResponse(image_url, "GET", &mime_type, &charset, &data, &headers));
...
{
    base::CommandLine::ForCurrentProcess()->AppendSwitch(kRemoveWhitespaceForDataURLs);
    const ParseTestData tests[] = {
        { "data:image/fractal,a b c d e f g", true, "image/fractal", "", "abcdefg" },
