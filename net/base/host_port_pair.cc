...
{
    return HostPortPair(url.HostNoBrackets(), static_cast<uint16_t>(url.EffectiveIntPort()));
}

// static
HostPortPair HostPortPair::FromSchemeHostPort(const url::SchemeHostPort& scheme_host_port)
{
...
HostPortPair HostPortPair::FromString(std::string_view str)
{
    ...
    if (base::SplitStringPiece(str, ":", base::KEEP_WHITESPACE, base::SPLIT_WANT_ALL).size() > 2 && str.front() != '[') {
        return HostPortPair();
    }
...
std::optional<HostPortPair> HostPortPair::FromValue(const base::Value& value)
{
    ...
    if (host == nullptr || !port.has_value() || !base::IsValueInRangeForNumericType<uint16_t>(port.value())) {
        return std::nullopt;
    }
