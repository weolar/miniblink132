...
constexpr Priority kPriorities[kNumQueues][kNumElements] = { { 3, 2, 1, 1, 5, 2, 5, 1 }, { 6, 2, 1, 0, 6, 2, 5, 0 }, { 3, 2, 2, 2, 3, 2, 3, 2 } };
constexpr int kFirstMinOrder[kNumQueues][kNumElements] = { { 2, 3, 7, 1, 5, 0, 4, 6 }, { 3, 7, 2, 1, 5, 6, 0, 4 }, { 1, 2, 3, 5, 7, 0, 4, 6 } };
constexpr int kLastMaxOrderErase[kNumQueues][kNumElements] = { { 6, 4, 0, 5, 1, 7, 3, 2 }, { 4, 0, 6, 5, 1, 2, 7, 3 }, { 6, 4, 0, 7, 5, 3, 2, 1 } };
constexpr int kFirstMaxOrder[kNumQueues][kNumElements] = { { 4, 6, 0, 1, 5, 2, 3, 7 }, { 0, 4, 6, 1, 5, 2, 3, 7 }, { 0, 4, 6, 1, 2, 3, 5, 7 } };
constexpr int kLastMinOrder[kNumQueues][kNumElements] = { { 7, 3, 2, 5, 1, 0, 6, 4 }, { 7, 3, 2, 5, 1, 6, 4, 0 }, { 7, 5, 3, 2, 1, 6, 4, 0 } };
...
public:
    ...
    void SetUp() override
    {
        ...
        for (size_t i = 0; i < kNumElements; ++i) {
            ...
            pointers_[i] = queue_.Insert(static_cast<int>(i), kPriorities[GetParam()][i]);
...
{
    for (PriorityQueue<int>::Pointer p = queue_.FirstMax(); !p.Equals(queue_.LastMin()); p = queue_.GetNextTowardsLastMin(p)) {
        for (PriorityQueue<int>::Pointer q = queue_.GetNextTowardsLastMin(p); !q.is_null(); q = queue_.GetNextTowardsLastMin(q)) {
    ...
    for (PriorityQueue<int>::Pointer p = queue_.LastMin(); !p.Equals(queue_.FirstMax()); p = queue_.GetPreviousTowardsFirstMax(p)) {
        for (PriorityQueue<int>::Pointer q = queue_.GetPreviousTowardsFirstMax(p); !q.is_null(); q = queue_.GetPreviousTowardsFirstMax(q)) {
...
TEST_P(PriorityQueueTest, FirstMinOrder)
{
    for (size_t i = 0; i < kNumElements; ++i) {
        ...
        // Also check Equals.
        EXPECT_TRUE(queue_.FirstMin().Equals(pointers_[kFirstMinOrder[GetParam()][i]]));
...
TEST_P(PriorityQueueTest, FirstMaxOrder)
{
    ...
    size_t i = 0;
    for (; !p.is_null() && i < kNumElements; p = queue_.GetNextTowardsLastMin(p), ++i) {
...
TEST_P(PriorityQueueTest, GetPreviousTowardsFirstMaxAndErase)
{
    ...
    for (size_t i = 0; i < kNumElements; ++i) {
        ...
        PriorityQueue<int>::Pointer next = queue_.GetPreviousTowardsFirstMax(current);
        queue_.Erase(current);
        current = next;
    }
    EXPECT_TRUE(current.is_null());
...
TEST_P(PriorityQueueTest, EraseFromMiddle)
{
    ...
    const int expected_order[kNumQueues][kNumElements - 2] = { { 3, 7, 1, 5, 4, 6 }, { 3, 7, 1, 5, 6, 4 }, { 1, 3, 5, 7, 4, 6 } };
...
TEST_P(PriorityQueueTest, InsertAtFront)
{
    ...
    const int expected_order[kNumQueues][kNumElements + 5]
        = { { 10, 12, 11, 2, 3, 7, 9, 1, 5, 0, 4, 6, 8 }, { 10, 3, 7, 12, 11, 2, 9, 1, 5, 6, 8, 0, 4 }, { 10, 12, 11, 9, 1, 2, 3, 5, 7, 0, 4, 6, 8 } };
...
{
    auto pred = [](size_t i, int value) -> bool { return value == static_cast<int>(i); };
    for (size_t i = 0; i < kNumElements; ++i) {
        PriorityQueue<int>::Pointer pointer = queue_.FindIf(base::BindRepeating(pred, i));
...
INSTANTIATE_TEST_SUITE_P(PriorityQueues, PriorityQueueTest, testing::Range(static_cast<size_t>(0), kNumQueues));
