...
protected:
    void SetUp() override
    {
        ...
        reader_ = std::make_unique<UploadBytesElementReader>(base::as_byte_span(bytes_));
...
TEST_F(UploadBytesElementReaderTest, ReadPartially)
{
    ...
    EXPECT_EQ(static_cast<int>(buf.size()), reader_->Read(wrapped_buffer.get(), buf.size(), CompletionOnceCallback()));
...
TEST_F(UploadBytesElementReaderTest, ReadAll)
{
    ...
    EXPECT_EQ(bytes_, buf);
    // Try to read again.
    EXPECT_EQ(0, reader_->Read(wrapped_buffer.get(), buf.size(), CompletionOnceCallback()));
}
...
TEST_F(UploadBytesElementReaderTest, ReadTooMuch)
{
    ...
    auto wrapped_buffer = base::MakeRefCounted<WrappedIOBuffer>(buf);
    EXPECT_EQ(static_cast<int>(bytes_.size()), reader_->Read(wrapped_buffer.get(), buf.size(), CompletionOnceCallback()));
    EXPECT_EQ(0U, reader_->BytesRemaining());
