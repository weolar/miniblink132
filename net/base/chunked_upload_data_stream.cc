...
bool ChunkedUploadDataStream::Writer::AppendData(base::span<const uint8_t> data, bool is_done)
{
...
ChunkedUploadDataStream::Writer::Writer(base::WeakPtr<ChunkedUploadDataStream> upload_data_stream)
    : upload_data_stream_(upload_data_stream)
{
}

ChunkedUploadDataStream::ChunkedUploadDataStream(int64_t identifier, bool has_null_source)
...
std::unique_ptr<ChunkedUploadDataStream::Writer> ChunkedUploadDataStream::CreateWriter()
{
    return base::WrapUnique(new Writer(weak_factory_.GetWeakPtr()));
}

void ChunkedUploadDataStream::AppendData(base::span<const uint8_t> data, bool is_done)
{
    DCHECK(!all_data_appended_);
    DCHECK(!data.empty() || is_done);
    if (!data.empty()) {
        upload_data_.push_back(std::make_unique<std::vector<uint8_t>>(data.begin(), data.end()));
    }
...
int ChunkedUploadDataStream::ReadChunk(IOBuffer* buf, int buf_len)
{
    ...
    while (read_index_ < upload_data_.size() && bytes_read < buf_len) {
        ...
        base::span<const uint8_t> bytes_to_read = data.subspan(read_offset_, std::min(static_cast<size_t>(buf_len - bytes_read), data.size() - read_offset_));
