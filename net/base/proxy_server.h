...
public:
    ...
    // invalid ProxyServer.
    static ProxyServer FromSchemeHostAndPort(Scheme scheme, std::string_view host, std::string_view port_str);
    static ProxyServer FromSchemeHostAndPort(Scheme scheme, std::string_view host, std::optional<uint16_t> port);
    ...
    {
        return is_http() || is_https() || is_quic();
    }
...
NET_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, const ProxyServer& proxy_server);
