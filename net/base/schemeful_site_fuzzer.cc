...
DEFINE_PROTO_FUZZER(const url_proto::Url& url_message)
{
    ...
    std::optional<net::SchemefulSite> site_with_registrable_domain = net::SchemefulSite::CreateIfHasRegisterableDomain(origin);

    if (site_with_registrable_domain) {
        CHECK_EQ(site_with_registrable_domain->GetInternalOriginForTesting(), site.GetInternalOriginForTesting());
