...
public:
    ...
    //
    // Uses |task_runner| for asynchronous operations.
    FileStream(base::File file, const scoped_refptr<base::TaskRunner>& task_runner);
    ...
    // know when).
    virtual int Open(const base::FilePath& path, int open_flags, CompletionOnceCallback callback);
    ...
    virtual int Write(IOBuffer* buf, int buf_len, CompletionOnceCallback callback);
    ...
    virtual int GetFileInfo(base::File::Info* file_info, CompletionOnceCallback callback);
