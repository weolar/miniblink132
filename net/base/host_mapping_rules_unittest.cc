...
TEST(HostMappingRulesTest, SetRulesFromString)
{
    ...
    rules.SetRulesFromString("map *.com baz , map *.net bar:60, EXCLUDE *.foo.com");
...
{
    HostMappingRules rules;
    rules.SetRulesFromString("map *.com:80 baz:111 , map *.com:443 blat:333, EXCLUDE *.foo.com");
...
TEST(HostMappingRulesTest, IgnoresUnmappedUrls)
{
    ...
    EXPECT_EQ(rules.RewriteUrl(url), HostMappingRules::RewriteResult::kNoMatchingRule);
...
TEST(HostMappingRulesTest, IgnoresInvalidReplacementUrls)
{
    ...
    EXPECT_EQ(rules.RewriteUrl(url), HostMappingRules::RewriteResult::kInvalidRewrite);
