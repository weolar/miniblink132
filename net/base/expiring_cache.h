...
template <typename KeyType, typename ValueType, typename ExpirationType> class NoopEvictionHandler {
public:
    void Handle(const KeyType& key, const ValueType& value, const ExpirationType& expiration, const ExpirationType& now, bool onGet) const
    {
    }
};
...
template <typename KeyType, typename ValueType, typename ExpirationType, typename ExpirationCompare,
    typename EvictionHandler = NoopEvictionHandler<KeyType, ValueType, ExpirationType>>
class ExpiringCache {
public:
...
public:
    ...
    public:
        ...
        bool HasNext() const
        {
            return it_ != cache_->entries_.end();
        }
        void Advance()
        {
            ++it_;
    ...
    void Put(const KeyType& key, const ValueType& value, const ExpirationType& now, const ExpirationType& expiration)
    {
        typename EntryMap::iterator it = entries_.find(key);
        if (it == entries_.end()) {
...
private:
    ...
    void Compact(const ExpirationType& now)
    {
        ...
        for (it = entries_.begin(); it != entries_.end() && entries_.size() >= max_entries_;) {
            Evict(it++, now, false);
        }
    }

    void Evict(typename EntryMap::iterator it, const ExpirationType& now, bool on_get)
    {
        eviction_handler_.Handle(it->first, it->second.first, it->second.second, now, on_get);
        entries_.erase(it);
    }
