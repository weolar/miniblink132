...
std::optional<IPEndPoint> IPEndPoint::FromValue(const base::Value& value)
{
    ...
    if (!port.has_value() || !base::IsValueInRangeForNumericType<uint16_t>(port.value())) {
        return std::nullopt;
    }

    return IPEndPoint(address.value(), base::checked_cast<uint16_t>(port.value()));
}
...
bool IPEndPoint::ToSockAddr(struct sockaddr* address, socklen_t* address_length) const
{
    // By definition, socklen_t is large enough to hold both sizes.
    constexpr socklen_t kSockaddrInSize = static_cast<socklen_t>(sizeof(struct sockaddr_in));
    constexpr socklen_t kSockaddrIn6Size = static_cast<socklen_t>(sizeof(struct sockaddr_in6));
    ...
    case IPAddress::kIPv4AddressSize: {
        ...
        *address_length = kSockaddrInSize;
        ...
        memcpy(&addr->sin_addr, address_.bytes().data(), IPAddress::kIPv4AddressSize);
        break;
    }
    case IPAddress::kIPv6AddressSize: {
        ...
        *address_length = kSockaddrIn6Size;
        ...
        memcpy(&addr6->sin6_addr, address_.bytes().data(), IPAddress::kIPv6AddressSize);
        break;
    }
    default:
        return false;
...
bool IPEndPoint::FromSockAddr(const struct sockaddr* sock_addr, socklen_t sock_addr_len)
{
    DCHECK(sock_addr);
    ...
    case AF_INET: {
        ...
        *this = IPEndPoint(
            ...
            IPAddress(base::as_bytes(base::span_from_ref(addr->sin_addr.s_addr))), base::NetToHost16(addr->sin_port));
        return true;
    }
    case AF_INET6: {
        ...
        *this = IPEndPoint(IPAddress(addr->sin6_addr.s6_addr), base::NetToHost16(addr->sin6_port));
        return true;
    }
#if BUILDFLAG(IS_WIN)
    case AF_BTH: {
        ...
        *this = IPEndPoint();
        ...
        // prefix of it.
        address_ = IPAddress(base::as_bytes(base::span_from_ref(addr->btAddr)).first(kBluetoothAddressSize));
