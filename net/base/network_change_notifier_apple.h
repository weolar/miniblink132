...
class NET_EXPORT_PRIVATE NetworkChangeNotifierApple : public NetworkChangeNotifier {
public:
    ...
    public:
        ...
        void SetDynamicStoreNotificationKeys(base::apple::ScopedCFTypeRef<SCDynamicStoreRef> store) override;
...
private:
    ...
    void SetDynamicStoreNotificationKeys(base::apple::ScopedCFTypeRef<SCDynamicStoreRef> store);
    void OnNetworkConfigChange(CFArrayRef changed_keys);
    void CleanUpOnNotifierThread();

    void SetInitialConnectionType();

    static void ReachabilityCallback(SCNetworkReachabilityRef target, SCNetworkConnectionFlags flags, void* notifier);

    static NetworkChangeCalculatorParams NetworkChangeCalculatorParamsMac();

#if BUILDFLAG(IS_MAC)
    void SetCallbacksForTest(base::OnceClosure initialized_callback, base::RepeatingCallback<bool(NetworkInterfaceList*, int)> get_network_list_callback,
        base::RepeatingCallback<std::string(SCDynamicStoreRef)> get_ipv4_primary_interface_name_callback,
        base::RepeatingCallback<std::string(SCDynamicStoreRef)> get_ipv6_primary_interface_name_callback);
    ...
    base::RepeatingCallback<bool(NetworkInterfaceList*, int)> get_network_list_callback_;
    base::RepeatingCallback<std::string(SCDynamicStoreRef)> get_ipv4_primary_interface_name_callback_;
    base::RepeatingCallback<std::string(SCDynamicStoreRef)> get_ipv6_primary_interface_name_callback_;
#endif // BUILDFLAG(IS_MAC)
