...
UploadBytesElementReader::UploadBytesElementReader(base::span<const uint8_t> bytes)
    : bytes_(bytes)
{
}
...
const UploadBytesElementReader* UploadBytesElementReader::AsBytesReader() const
{
    return this;
}
...
int UploadBytesElementReader::Read(IOBuffer* buf, int buf_length, CompletionOnceCallback callback)
{
    DCHECK_LT(0, buf_length);

    base::span<const uint8_t> bytes_to_read = bytes_.subspan(offset_, std::min(BytesRemaining(), static_cast<uint64_t>(buf_length)));
...
UploadOwnedBytesElementReader::UploadOwnedBytesElementReader(std::vector<char>* data)
...
std::unique_ptr<UploadOwnedBytesElementReader> UploadOwnedBytesElementReader::CreateWithString(const std::string& string)
{
