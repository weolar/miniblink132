...
TEST(BackoffEntryTest, CanDiscard)
{
    ...
    EXPECT_FALSE(entry.CanDiscard());
    now_ticks.set_now(entry.GetReleaseTime() + base::Milliseconds(base_policy.maximum_backoff_ms + 1));
    EXPECT_TRUE(entry.CanDiscard());

    // Test the final case (no errors, dependent only on specified lifetime).
    now_ticks.set_now(entry.GetReleaseTime() + base::Milliseconds(base_policy.entry_lifetime_ms - 1));
    entry.InformOfRequest(true);
    EXPECT_FALSE(entry.CanDiscard());
    now_ticks.set_now(entry.GetReleaseTime() + base::Milliseconds(base_policy.entry_lifetime_ms));
    EXPECT_TRUE(entry.CanDiscard());
}
...
TEST(BackoffEntryTest, RetainCustomHorizon)
{
    ...
    custom.InformOfRequest(false);
    EXPECT_EQ(TimeTicks() + base::Days(3) + base::Milliseconds(1000), custom.GetReleaseTime());
}
