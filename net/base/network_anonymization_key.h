...
public:
    ...
    NetworkAnonymizationKey(const NetworkAnonymizationKey& network_anonymization_key);
    NetworkAnonymizationKey(NetworkAnonymizationKey&& network_anonymization_key);

    ~NetworkAnonymizationKey();

    NetworkAnonymizationKey& operator=(const NetworkAnonymizationKey& network_anonymization_key);
    ...
    {
        return std::tie(top_frame_site_, is_cross_site_, nonce_) == std::tie(other.top_frame_site_, other.is_cross_site_, other.nonce_);
    }
    ...
    static NetworkAnonymizationKey CreateSameSite(const SchemefulSite& top_frame_site)
    {
    ...
    static NetworkAnonymizationKey CreateCrossSite(const SchemefulSite& top_frame_site)
    {
    ...
    static NetworkAnonymizationKey CreateFromFrameSite(
        const SchemefulSite& top_frame_site, const SchemefulSite& frame_site, std::optional<base::UnguessableToken> nonce = std::nullopt);
    ...
    static NetworkAnonymizationKey CreateFromNetworkIsolationKey(const net::NetworkIsolationKey& network_isolation_key);
    ...
    static NetworkAnonymizationKey CreateFromParts(
        const SchemefulSite& top_frame_site, bool is_cross_site, std::optional<base::UnguessableToken> nonce = std::nullopt)
    {
    ...
    [[nodiscard]] static bool FromValue(const base::Value& value, NetworkAnonymizationKey* out_network_anonymization_key);
...
private:
    NetworkAnonymizationKey(const SchemefulSite& top_frame_site, bool is_cross_site, std::optional<base::UnguessableToken> nonce = std::nullopt);

    std::string GetSiteDebugString(const std::optional<SchemefulSite>& site) const;

    static std::optional<std::string> SerializeSiteWithNonce(const SchemefulSite& site);
