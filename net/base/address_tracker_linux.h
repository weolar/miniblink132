...
public:
    ...
    AddressTrackerLinux(const base::RepeatingClosure& address_callback, const base::RepeatingClosure& link_callback,
        const base::RepeatingClosure& tunnel_callback, const std::unordered_set<std::string>& ignored_interfaces,
        scoped_refptr<base::SequencedTaskRunner> blocking_thread_runner = nullptr);
    ...
    std::pair<AddressMap, std::unordered_set<int>> GetInitialDataAndStartRecordingDiffs();
...
private:
    ...
    FRIEND_TEST_ALL_PREFIXES(AddressTrackerLinuxNetlinkTest, TestInitializeTwoTrackers);
    FRIEND_TEST_ALL_PREFIXES(AddressTrackerLinuxNetlinkTest, TestInitializeTwoTrackersInPidNamespaces);
    ...
    public:
        AddressTrackerAutoLock(const AddressTrackerLinux& tracker, base::Lock& lock) EXCLUSIVE_LOCK_FUNCTION(lock);
    ...
    // online links.
    void ReadMessages(bool* address_changed, bool* link_changed, bool* tunnel_changed);
    ...
    // nullopt, |*online_links_diff| is populated with the changes to the set of
    // online links.
    void HandleMessage(const char* buffer, int length, bool* address_changed, bool* link_changed, bool* tunnel_changed);
    ...
    base::RepeatingClosure address_callback_ GUARDED_BY_CONTEXT(sequence_checker_);
    ...
    std::unique_ptr<base::FileDescriptorWatcher::Controller> watcher_ GUARDED_BY_CONTEXT(sequence_checker_);
    ...
    NetworkChangeNotifier::ConnectionType current_connection_type_ GUARDED_BY(connection_type_lock_) = NetworkChangeNotifier::CONNECTION_NONE;
    int threads_waiting_for_connection_type_initialization_ GUARDED_BY(connection_type_lock_) = 0;
