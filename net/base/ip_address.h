...
public:
    ...
    // The bytes are expected to be in network byte order.
    IPAddress(uint8_t b0, uint8_t b1, uint8_t b2, uint8_t b3, uint8_t b4, uint8_t b5, uint8_t b6, uint8_t b7, uint8_t b8, uint8_t b9, uint8_t b10, uint8_t b11,
...
NET_EXPORT std::string IPAddressToStringWithPort(const IPAddress& address, uint16_t port);
...
// (IPv6) addresses.
NET_EXPORT bool IPAddressMatchesPrefix(const IPAddress& ip_address, const IPAddress& ip_prefix, size_t prefix_length_in_bits);
...
//    a:b:c::/46
//    ::1/128
NET_EXPORT bool ParseCIDRBlock(std::string_view cidr_literal, IPAddress* ip_address, size_t* prefix_length_in_bits);
...
[[nodiscard]] NET_EXPORT bool ParseURLHostnameToAddress(std::string_view hostname, IPAddress* ip_address);
...
template <size_t N> bool IPAddressStartsWith(const IPAddress& address, const uint8_t (&prefix)[N])
{
    if (address.size() < N)
        return false;
...
enum class Dns64PrefixLength { k32bit, k40bit, k48bit, k56bit, k64bit, k96bit, kInvalid };
...
NET_EXPORT Dns64PrefixLength ExtractPref64FromIpv4onlyArpaAAAA(const IPAddress& address);
...
NET_EXPORT IPAddress ConvertIPv4ToIPv4EmbeddedIPv6(const IPAddress& ipv4_address, const IPAddress& ipv6_address, Dns64PrefixLength prefix_length);
