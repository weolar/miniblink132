...
{
    scoped_refptr<IOBufferWithSize> buf = base::MakeRefCounted<IOBufferWithSize>(kTestDataSize);
...
public:
    ...
    const base::FilePath temp_file_path() const
    {
...
{
    TestCompletionCallback callback;
    auto stream = std::make_unique<FileStream>(base::SingleThreadTaskRunner::GetCurrentDefault());
    int flags = base::File::FLAG_OPEN | base::File::FLAG_READ | base::File::FLAG_ASYNC;
    int rv = stream->Open(temp_file_path(), flags, callback.callback());
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
    EXPECT_THAT(callback.WaitForResult(), IsOk());
...
TEST_F(FileStreamTest, UseFileHandle)
{
    ...
    auto read_stream = std::make_unique<FileStream>(std::move(file1), base::SingleThreadTaskRunner::GetCurrentDefault());
    ASSERT_THAT(read_stream->Seek(0, callback64.callback()), IsError(ERR_IO_PENDING));
    ASSERT_EQ(0, callback64.WaitForResult());
    // Read into buffer and compare.
    scoped_refptr<IOBufferWithSize> read_buffer = base::MakeRefCounted<IOBufferWithSize>(kTestDataSize);
    ...
    auto write_stream = std::make_unique<FileStream>(std::move(file2), base::SingleThreadTaskRunner::GetCurrentDefault());
    ASSERT_THAT(write_stream->Seek(0, callback64.callback()), IsError(ERR_IO_PENDING));
    ASSERT_EQ(0, callback64.WaitForResult());
    scoped_refptr<IOBufferWithSize> write_buffer = CreateTestDataBuffer();
    rv = write_stream->Write(write_buffer.get(), kTestDataSize, callback.callback());
    ASSERT_EQ(kTestDataSize, callback.GetResult(rv));
    write_stream.reset();

    // Read into buffer and compare to make sure the handle worked fine.
    ASSERT_EQ(kTestDataSize, base::ReadFile(temp_file_path(), read_buffer->data(), kTestDataSize));
    ASSERT_EQ(0, memcmp(kTestData, read_buffer->data(), kTestDataSize));
}
...
TEST_F(FileStreamTest, Read)
{
    ...
    for (;;) {
        scoped_refptr<IOBufferWithSize> buf = base::MakeRefCounted<IOBufferWithSize>(4);
        rv = stream.Read(buf.get(), buf->size(), callback.callback());
...
TEST_F(FileStreamTest, Write_FromOffset)
{
    ...
    scoped_refptr<DrainableIOBuffer> drainable = base::MakeRefCounted<DrainableIOBuffer>(std::move(buffer), buffer_size);
    while (total_bytes_written != kTestDataSize) {
        rv = stream.Write(drainable.get(), drainable->BytesRemaining(), callback.callback());
        if (rv == ERR_IO_PENDING)
            rv = callback.WaitForResult();
        EXPECT_LT(0, rv);
        if (rv <= 0)
...
public:
    TestWriteReadCompletionCallback(FileStream* stream, int* total_bytes_written, int* total_bytes_read, std::string* data_read)
        : stream_(stream)
        , total_bytes_written_(total_bytes_written)
        , total_bytes_read_(total_bytes_read)
        , data_read_(data_read)
        , drainable_(base::MakeRefCounted<DrainableIOBuffer>(CreateTestDataBuffer(), kTestDataSize))
    {
    }

    TestWriteReadCompletionCallback(const TestWriteReadCompletionCallback&) = delete;
    TestWriteReadCompletionCallback& operator=(const TestWriteReadCompletionCallback&) = delete;
    ...
    {
        return base::BindOnce(&TestWriteReadCompletionCallback::OnComplete, base::Unretained(this));
    }
    ...
    void OnComplete(int result)
    {
        ...
        *total_bytes_written_ += result;
        ...
        if (*total_bytes_written_ != kTestDataSize) {
            ...
            std::string data_read;
            TestWriteReadCompletionCallback callback(stream_, &total_bytes_written, &total_bytes_read, &data_read);
            rv = stream_->Write(drainable_.get(), drainable_->BytesRemaining(), callback.callback());
...
TEST_F(FileStreamTest, WriteRead)
{
    ...
    TestInt64CompletionCallback callback64;
    EXPECT_THAT(stream->Seek(file_size.value(), callback64.callback()), IsError(ERR_IO_PENDING));
    ...
    std::string data_read;
    {
        ...
        TestWriteReadCompletionCallback callback(stream.get(), &total_bytes_written, &total_bytes_read, &data_read);
...
public:
    ...
    TestWriteCloseCompletionCallback(const TestWriteCloseCompletionCallback&) = delete;
    TestWriteCloseCompletionCallback& operator=(const TestWriteCloseCompletionCallback&) = delete;
    ...
    CompletionOnceCallback callback()
    {
        return base::BindOnce(&TestWriteCloseCompletionCallback::OnComplete, base::Unretained(this));
    }
...
TEST_F(FileStreamTest, WriteError)
{
    ...
    auto stream = std::make_unique<FileStream>(std::move(file), base::SingleThreadTaskRunner::GetCurrentDefault());
...
TEST_F(FileStreamTest, AsyncFlagMismatch)
{
    ...
    FileStream stream(std::move(lying_file), base::SingleThreadTaskRunner::GetCurrentDefault());
...
protected:
    ...
    {
        base::win::ScopedHandle pipe(::CreateNamedPipeW(pipe_name_.c_str(), PIPE_ACCESS_DUPLEX | FILE_FLAG_FIRST_PIPE_INSTANCE | FILE_FLAG_OVERLAPPED,
    ...
    base::File OpenPipe()
    {
        base::win::ScopedHandle pipe(::CreateFileW(pipe_name_.c_str(), GENERIC_READ | GENERIC_WRITE,
            /*dwShareMode=*/0, /*lpSecurityAttributes=*/nullptr, OPEN_EXISTING, /*dwFlagsAndAttributes=*/0,
...
private:
    ...
    const std::wstring pipe_name_ { base::StrCat({ L"\\\\.\\pipe\\chromium.test.", base::ASCIIToWide(base::UnguessableToken::Create().ToString()) }) };
};
...
TEST_F(FileStreamPipeTest, ConnectNamedPipeAfterClient)
{
    ...
    ASSERT_TRUE(pipe.IsValid()) << base::File::ErrorToString(pipe.error_details());

    FileStream pipe_stream(std::move(pipe), base::SingleThreadTaskRunner::GetCurrentDefault());
    ASSERT_TRUE(pipe_stream.IsOpen());

    // Open the client end of the pipe.
    base::File client(OpenPipe());
    ASSERT_TRUE(client.IsValid()) << base::File::ErrorToString(client.error_details());
    ...
    TestCompletionCallback callback;
    ASSERT_THAT(callback.GetResult(pipe_stream.ConnectNamedPipe(callback.callback())), IsOk());

    // Send some data over the pipe to be sure it works.
    scoped_refptr<IOBufferWithSize> write_io_buffer = CreateTestDataBuffer();
    int result = pipe_stream.Write(write_io_buffer.get(), write_io_buffer->size(), callback.callback());
...
TEST_F(FileStreamPipeTest, ConnectNamedPipeBeforeClient)
{
    ...
    TestCompletionCallback callback;
    ASSERT_THAT(pipe_stream.ConnectNamedPipe(callback.callback()), IsError(ERR_IO_PENDING));
...
{
    {
        ...
        // destroyed.
        ASSERT_THAT(pipe_stream.ConnectNamedPipe(base::BindLambdaForTesting([loop_quitter = base::ScopedClosureRunner(QuitClosure())](int error) { FAIL(); })),
...
TEST_F(FileStreamPipeDeathTest, CannotConnectFile)
{
    ...
    base::File exe_file(exe_path, base::File::FLAG_OPEN | base::File::FLAG_READ | base::File::FLAG_ASYNC | base::File::FLAG_WIN_SHARE_DELETE);
    ASSERT_TRUE(exe_file.IsValid()) << base::File::ErrorToString(exe_file.error_details());

    // Pass that file to a FileStream.
    FileStream file_stream(std::move(exe_file), base::SingleThreadTaskRunner::GetCurrentDefault());
    ASSERT_TRUE(file_stream.IsOpen());

    ASSERT_CHECK_DEATH({ file_stream.ConnectNamedPipe(CompletionOnceCallback()); });
}
