...
template <typename T> fuchsia::net::Subnet SubnetFrom(T octets, uint8_t prefix)
{
...
template <typename T> fuchsia::net::interfaces::Address InterfaceAddressFrom(T octets, uint8_t prefix)
{
...
template <typename T> std::vector<T> MakeSingleItemVec(T item)
{
    std::vector<T> vec;
...
fuchsia::net::interfaces::Properties DefaultInterfaceProperties(
    fuchsia::hardware::network::PortClass device_class = fuchsia::hardware::network::PortClass::ETHERNET)
{
    ...
    interface.set_port_class(fuchsia::net::interfaces::PortClass::WithDevice(std::move(device_class)));
    interface.set_addresses(MakeSingleItemVec(InterfaceAddressFrom(kDefaultIPv4Address, kDefaultIPv4Prefix)));
    return interface;
}
...
fuchsia::net::interfaces::Properties SecondaryInterfaceProperties()
{
    ...
    interface.set_port_class(fuchsia::net::interfaces::PortClass::WithDevice([]() { return fuchsia::hardware::network::PortClass::ETHERNET; }()));
    interface.set_addresses(MakeSingleItemVec(InterfaceAddressFrom(kSecondaryIPv4Address, kSecondaryIPv4Prefix)));
    return interface;
}

template <typename F> fuchsia::net::interfaces::Event MakeChangeEvent(uint64_t interface_id, F fn)
{
...
public:
        ...
        : binding_(this)
    {
        ...
        pending_.push(fuchsia::net::interfaces::Event::WithIdle(fuchsia::net::interfaces::Empty {}));
    }
    ...
    void SetInitial(std::vector<fuchsia::net::interfaces::Properties> props)
    {
        ...
        for (auto& prop : props) {
            pending_.push(fuchsia::net::interfaces::Event::WithExisting(std::move(prop)));
        }
        pending_.push(fuchsia::net::interfaces::Event::WithIdle(fuchsia::net::interfaces::Empty {}));
...
public:
    ...
    void Unbind()
    {
        watcher_.AsyncCall(&FakeWatcher::Unbind);
    }
...
template <class T> class ResultReceiver {
public:
    ~ResultReceiver()
    {
        EXPECT_EQ(entries_.size(), 0u);
    }
...
class FakeConnectionTypeObserver final : public NetworkChangeNotifier::ConnectionTypeObserver {
public:
    ...
    bool RunAndExpectConnectionTypes(std::vector<NetworkChangeNotifier::ConnectionType> sequence)
    {
        return receiver_.RunAndExpectEntries(sequence);
    }

    // ConnectionTypeObserver implementation.
    void OnConnectionTypeChanged(NetworkChangeNotifier::ConnectionType type) override
    {
...
// Accumulates the list of ConnectionTypes notified via OnConnectionTypeChanged.
class FakeNetworkChangeObserver final : public NetworkChangeNotifier::NetworkChangeObserver {
public:
    ...
    bool RunAndExpectNetworkChanges(std::vector<NetworkChangeNotifier::ConnectionType> sequence)
    {
        return receiver_.RunAndExpectEntries(sequence);
    }
...
// Accumulates the list of ConnectionTypes notified via OnConnectionTypeChanged.
class FakeIPAddressObserver final : public NetworkChangeNotifier::IPAddressObserver {
public:
    FakeIPAddressObserver()
    {
    ...
    size_t ip_change_count() const
    {
        return ip_change_count_;
    }
...
public:
    ...
    NetworkChangeNotifierFuchsiaTest(const NetworkChangeNotifierFuchsiaTest&) = delete;
    NetworkChangeNotifierFuchsiaTest& operator=(const NetworkChangeNotifierFuchsiaTest&) = delete;
    ...
    void CreateNotifier(bool require_wlan = false, bool disconnect_watcher = false)
    {
        ...
        fidl::InterfaceRequest<fuchsia::net::interfaces::Watcher> watcher_request = watcher.NewRequest();
        ...
        dns_config_notifier_ = std::make_unique<SystemDnsConfigChangeNotifier>(nullptr /* task_runner */, nullptr /* dns_config_service */);
        notifier_ = base::WrapUnique(new NetworkChangeNotifierFuchsia(std::move(watcher), require_wlan, dns_config_notifier_.get()));
...
TEST_F(NetworkChangeNotifierFuchsiaTest, ConnectFail_BeforeGetWatcher)
{
    ...
    // construction.
    EXPECT_EXIT(CreateNotifier(/*require_wlan=*/false, /*disconnect_watcher=*/true), testing::ExitedWithCode(1), "");
}
...
TEST_F(NetworkChangeNotifierFuchsiaTest, ConnectFail_AfterGetWatcher)
{
    ...
    EXPECT_EQ(NetworkChangeNotifier::ConnectionType::CONNECTION_NONE, notifier_->GetCurrentConnectionType());
...
TEST_F(NetworkChangeNotifierFuchsiaTest, InterfacesChangeDuringConstruction)
{
    ...
    watcher_.SetInitial(DefaultInterfaceProperties(fuchsia::hardware::network::PortClass::WLAN_CLIENT));

    // Inject an interfaces change event so that the notifier will receive it
    // immediately after the initial state.
    watcher_.PushEvent(MakeChangeEvent(kDefaultInterfaceId, [](fuchsia::net::interfaces::Properties* props) {
        props->set_addresses(MakeSingleItemVec(InterfaceAddressFrom(kSecondaryIPv4Address, kSecondaryIPv4Prefix)));
    }));
...
TEST_F(NetworkChangeNotifierFuchsiaTest, NotifyNetworkChangeOnInitialIPChange)
{
    ...
    EXPECT_TRUE(network_change_observer.RunAndExpectNetworkChanges({ NetworkChangeNotifier::CONNECTION_NONE, NetworkChangeNotifier::CONNECTION_WIFI }));
    EXPECT_TRUE(ip_observer_->RunAndExpectCallCount(1));
}
...
TEST_F(NetworkChangeNotifierFuchsiaTest, NoChange)
{
    ...
    CreateNotifier();
    EXPECT_EQ(NetworkChangeNotifier::ConnectionType::CONNECTION_ETHERNET, notifier_->GetCurrentConnectionType());
...
TEST_F(NetworkChangeNotifierFuchsiaTest, NoChangeV6)
{
    ...
    initial.set_addresses(MakeSingleItemVec(InterfaceAddressFrom(kDefaultIPv6Address, kDefaultIPv6Prefix)));
...
TEST_F(NetworkChangeNotifierFuchsiaTest, MultiV6IPNoChange)
{
    ...
    props.mutable_addresses()->push_back(InterfaceAddressFrom(kDefaultIPv6Address, kDefaultIPv6Prefix));
    props.mutable_addresses()->push_back(InterfaceAddressFrom(kSecondaryIPv6Address, kSecondaryIPv6Prefix));
...
{
    auto props = DefaultInterfaceProperties();
    props.set_addresses(MakeSingleItemVec(InterfaceAddressFrom(kDefaultIPv6Address, kDefaultIPv6Prefix)));
    watcher_.SetInitial(std::move(props));
    CreateNotifier();
    EXPECT_EQ(NetworkChangeNotifier::ConnectionType::CONNECTION_ETHERNET, notifier_->GetCurrentConnectionType());

    watcher_.PushEvent(MakeChangeEvent(kDefaultInterfaceId, [](fuchsia::net::interfaces::Properties* props) {
        props->set_addresses(MakeSingleItemVec(InterfaceAddressFrom(kSecondaryIPv6Address, kSecondaryIPv6Prefix)));
    }));
...
TEST_F(NetworkChangeNotifierFuchsiaTest, MultiV6IPChanged)
{
    ...
    watcher_.PushEvent(MakeChangeEvent(kDefaultInterfaceId, [](fuchsia::net::interfaces::Properties* props) {
        ...
        addrs.push_back(InterfaceAddressFrom(kSecondaryIPv4Address, kSecondaryIPv4Prefix));
        addrs.push_back(InterfaceAddressFrom(kSecondaryIPv6Address, kSecondaryIPv6Prefix));
...
TEST_F(NetworkChangeNotifierFuchsiaTest, Ipv6AdditionalIpChange)
{
    ...
    watcher_.PushEvent(MakeChangeEvent(kDefaultInterfaceId, [](fuchsia::net::interfaces::Properties* props) {
        ...
        props->mutable_addresses()->push_back(InterfaceAddressFrom(kDefaultIPv4Address, kDefaultIPv4Prefix));
        props->mutable_addresses()->push_back(InterfaceAddressFrom(kDefaultIPv6Address, kDefaultIPv6Prefix));
    }));
...
TEST_F(NetworkChangeNotifierFuchsiaTest, InterfaceDown)
{
    ...
    watcher_.PushEvent(MakeChangeEvent(kDefaultInterfaceId, [](fuchsia::net::interfaces::Properties* props) { props->set_online(false); }));

    EXPECT_TRUE(type_observer_->RunAndExpectConnectionTypes({ NetworkChangeNotifier::ConnectionType::CONNECTION_NONE }));
    EXPECT_TRUE(ip_observer_->RunAndExpectCallCount(1));
}
...
TEST_F(NetworkChangeNotifierFuchsiaTest, InterfaceUp)
{
    ...
    watcher_.PushEvent(MakeChangeEvent(kDefaultInterfaceId, [](fuchsia::net::interfaces::Properties* props) { props->set_online(true); }));

    EXPECT_TRUE(type_observer_->RunAndExpectConnectionTypes({ NetworkChangeNotifier::ConnectionType::CONNECTION_ETHERNET }));
    EXPECT_TRUE(ip_observer_->RunAndExpectCallCount(1));
}
...
TEST_F(NetworkChangeNotifierFuchsiaTest, InterfaceDeleted)
{
    ...
    watcher_.PushEvent(fuchsia::net::interfaces::Event::WithRemoved(kDefaultInterfaceId));
...
TEST_F(NetworkChangeNotifierFuchsiaTest, InterfaceAdded)
{
    ...
    watcher_.PushEvent(fuchsia::net::interfaces::Event::WithAdded(DefaultInterfaceProperties(fuchsia::hardware::network::PortClass::WLAN_CLIENT)));

    EXPECT_TRUE(type_observer_->RunAndExpectConnectionTypes({ NetworkChangeNotifier::ConnectionType::CONNECTION_WIFI }));
    EXPECT_TRUE(ip_observer_->RunAndExpectCallCount(1));
}
...
TEST_F(NetworkChangeNotifierFuchsiaTest, SecondaryInterfaceAddedNoop)
{
    ...
    watcher_.PushEvent(fuchsia::net::interfaces::Event::WithAdded(SecondaryInterfaceProperties()));
}
...
TEST_F(NetworkChangeNotifierFuchsiaTest, SecondaryInterfaceDeletedNoop)
{
    ...
    watcher_.PushEvent(fuchsia::net::interfaces::Event::WithRemoved(kSecondaryInterfaceId));
}
...
TEST_F(NetworkChangeNotifierFuchsiaTest, FoundWiFi)
{
    watcher_.SetInitial(DefaultInterfaceProperties(fuchsia::hardware::network::PortClass::WLAN_CLIENT));
    CreateNotifier();
    EXPECT_EQ(NetworkChangeNotifier::ConnectionType::CONNECTION_WIFI, notifier_->GetCurrentConnectionType());
}
