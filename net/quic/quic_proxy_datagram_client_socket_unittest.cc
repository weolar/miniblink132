...
public:
    ...
    void InitializeClientSocket() override
    {
        sock_ = std::make_unique<QuicProxyDatagramClientSocket>(
            destination_endpoint_.GetURL(), proxy_chain_, user_agent_, NetLogWithSource::Make(NetLogSourceType::NONE), proxy_delegate_.get());
    ...
    void PopulateConnectRequestIR(quiche::HttpHeaderBlock* block, std::optional<const HttpRequestHeaders> extra_headers) override
    {
        ...
        (*block)[":authority"] = port != 443 ? base::StrCat({ host, ":", base::NumberToString(port) }) : host;
        if (extra_headers) {
    ...
    void AssertConnectSucceeds() override
    {
        ...
        ASSERT_THAT(sock_->ConnectViaStream(local_addr_, peer_addr_, std::move(stream_handle_), callback.callback()), IsError(ERR_IO_PENDING));
...
TEST_P(QuicProxyDatagramClientSocketTest, ConnectSendsCorrectRequest)
{
    ...
    mock_quic_data_.AddRead(ASYNC, ConstructServerConnectReplyPacket(/*packet_number=*/1, !kFin));
    mock_quic_data_.AddRead(SYNCHRONOUS, ERR_IO_PENDING);
    mock_quic_data_.AddWrite(SYNCHRONOUS,
        ConstructAckAndRstPacket(packet_number++, quic::QUIC_STREAM_CANCELLED,
...
TEST_P(QuicProxyDatagramClientSocketTest, ProxyDelegateHeaders)
{
    ...
    extra_expected_headers.SetHeader(kTestHeaderName, ProxyServerToProxyUri(proxy_chain_.Last()));
    ...
    mock_quic_data_.AddWrite(SYNCHRONOUS, ConstructSettingsPacket(packet_number++));
    mock_quic_data_.AddWrite(SYNCHRONOUS, ConstructConnectRequestPacket(packet_number++, extra_expected_headers));
    mock_quic_data_.AddRead(ASYNC,
        ConstructServerConnectReplyPacket(
            /*packet_number=*/1, !kFin, /*header_length=*/nullptr, response_headers));
    mock_quic_data_.AddRead(SYNCHRONOUS, ERR_IO_PENDING);
    mock_quic_data_.AddWrite(SYNCHRONOUS,
        ConstructAckAndRstPacket(packet_number++, quic::QUIC_STREAM_CANCELLED,
            /*largest_received=*/1, /*smallest_received=*/1));
    ...
    proxy_delegate_->VerifyOnTunnelHeadersReceived(proxy_chain_, /*chain_index=*/0, kResponseHeaderName, kResponseHeaderValue);
}

TEST_P(QuicProxyDatagramClientSocketTest, ProxyDelegateFails)
{
    proxy_delegate_ = std::make_unique<TestProxyDelegate>();
    proxy_delegate_->MakeOnTunnelHeadersReceivedFail(ERR_TUNNEL_CONNECTION_FAILED);
...
TEST_P(QuicProxyDatagramClientSocketTest, WriteSendsData)
{
    ...
    mock_quic_data_.AddWrite(SYNCHRONOUS,
        ConstructAckAndDatagramPacket(
            packet_number++, /*largest_received=*/1, /*smallest_received=*/1, { quarter_stream_id + context_id + std::string(kMsg1, kLen1) }));
    mock_quic_data_.AddWrite(SYNCHRONOUS, ConstructDatagramPacket(packet_number++, { quarter_stream_id + context_id + std::string(kMsg2, kLen2) }));
    mock_quic_data_.AddWrite(SYNCHRONOUS, ConstructRstPacket(packet_number++, quic::QUIC_STREAM_CANCELLED));

    InitializeSession();

    quic::test::QuicSpdySessionPeer::SetHttpDatagramSupport(session_.get(), quic::HttpDatagramSupport::kRfc);
...
TEST_P(QuicProxyDatagramClientSocketTest, OnHttp3DatagramAddsDatagram)
{
    ...
    sock_->OnHttp3Datagram(0, std::string(1, '\0') /* context_id */ + std::string(kDatagramPayload, kDatagramLen));

    ASSERT_TRUE(!sock_->GetDatagramsForTesting().empty());
    ASSERT_EQ(sock_->GetDatagramsForTesting().front(), "youveGotMail");

    histogram_tester_.ExpectUniqueSample(QuicProxyDatagramClientSocket::kMaxQueueSizeHistogram, false, 1);
}
...
TEST_P(QuicProxyDatagramClientSocketTest, ReadReadsDataInQueue)
{
    ...
    mock_quic_data_.AddRead(ASYNC,
        ConstructServerDatagramPacket(2,
            std::string(1, '\0') /* quarter_stream_id */ + std::string(1, '\0') /* context_id */
                + std::string(kDatagramPayload,
                    kDatagramLen) // Actual message payload
            ));
    mock_quic_data_.AddWrite(SYNCHRONOUS,
        ConstructAckPacket(packet_number++, /*largest_received=*/2,
            /*smallest_received=*/1));
    mock_quic_data_.AddReadPauseForever();
    mock_quic_data_.AddWrite(SYNCHRONOUS, ConstructRstPacket(packet_number++, quic::QUIC_STREAM_CANCELLED));
...
TEST_P(QuicProxyDatagramClientSocketTest, MaxQueueLimitDiscardsIncomingDatagram)
{
    int packet_number = 1;
    ...
    for (size_t i = 0; i < QuicProxyDatagramClientSocket::kMaxDatagramQueueSize + 1; i++) {
        sock_->OnHttp3Datagram(0, std::string(1, '\0') /* context_id */ + std::string(kDatagramPayload, kDatagramLen));
    }

    ASSERT_TRUE(sock_->GetDatagramsForTesting().size() == QuicProxyDatagramClientSocket::kMaxDatagramQueueSize);

    histogram_tester_.ExpectTotalCount(QuicProxyDatagramClientSocket::kMaxQueueSizeHistogram, QuicProxyDatagramClientSocket::kMaxDatagramQueueSize + 1);
    histogram_tester_.ExpectBucketCount(QuicProxyDatagramClientSocket::kMaxQueueSizeHistogram, false, QuicProxyDatagramClientSocket::kMaxDatagramQueueSize);
    histogram_tester_.ExpectBucketCount(QuicProxyDatagramClientSocket::kMaxQueueSizeHistogram, true, 1);
}

INSTANTIATE_TEST_SUITE_P(VersionIncludeStreamDependencySequence, QuicProxyDatagramClientSocketTest, ::testing::ValuesIn(AllSupportedQuicVersions()),
