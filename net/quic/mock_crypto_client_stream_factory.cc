...
void MockCryptoClientStreamFactory::SetConfigForServerId(const quic::QuicServerId& server_id, const quic::QuicConfig& config)
{
    config_for_server_[server_id] = std::make_unique<quic::QuicConfig>(config);
}

std::unique_ptr<quic::QuicCryptoClientStream> MockCryptoClientStreamFactory::CreateQuicCryptoClientStream(const quic::QuicServerId& server_id,
    QuicChromiumClientSession* session, std::unique_ptr<quic::ProofVerifyContext> /*proof_verify_context*/, quic::QuicCryptoClientConfig* crypto_config)
{
    ...
    quic::QuicConfig* config = it == config_for_server_.end() ? config_.get() : it->second.get();

    std::unique_ptr<MockCryptoClientStream> stream = std::make_unique<MockCryptoClientStream>(
        server_id, session, nullptr, *config, crypto_config, handshake_mode_, proof_verify_details, use_mock_crypter_);
