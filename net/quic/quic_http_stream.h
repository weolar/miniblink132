...
public:
    explicit QuicHttpStream(std::unique_ptr<QuicChromiumClientSession::Handle> session, std::set<std::string> dns_aliases);
    ...
    static HttpConnectionInfo ConnectionInfoFromQuicVersion(quic::ParsedQuicVersion quic_version);
...
private:
    ...
    int session_error_ = ERR_UNEXPECTED; // Error code from the connection shutdown.
