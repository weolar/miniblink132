...
void MockQuicData::AddRead(IoMode mode, std::unique_ptr<quic::QuicReceivedPacket> packet)
{
    reads_.emplace_back(mode, packet->data(), packet->length(), sequence_number_++, static_cast<uint8_t>(packet->ecn_codepoint()));
    packets_.push_back(std::move(packet));
}
void MockQuicData::AddRead(IoMode mode, std::unique_ptr<quic::QuicEncryptedPacket> packet)
{
    reads_.emplace_back(mode, packet->data(), packet->length(), sequence_number_++, /*tos=*/0);
    packets_.push_back(std::move(packet));
}
...
void MockQuicData::AddWrite(IoMode mode, std::unique_ptr<quic::QuicEncryptedPacket> packet)
{
    writes_.emplace_back(mode, packet->data(), packet->length(), sequence_number_++);
    packets_.push_back(std::move(packet));
}
...
void MockQuicData::AddWrite(IoMode mode, int rv, std::unique_ptr<quic::QuicEncryptedPacket> packet)
{
    writes_.emplace_back(mode, rv, sequence_number_++);
    packets_.push_back(std::move(packet));
}
