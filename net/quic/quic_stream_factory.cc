...
base::Value NetLogQuicStreamFactoryJobParams(const QuicStreamFactory::QuicSessionAliasKey* key)
{
    base::Value::Dict dict;
    dict.Set("host", key->server_id().host());
    dict.Set("port", key->server_id().port());
    dict.Set("privacy_mode", PrivacyModeToDebugString(key->session_key().privacy_mode()));
    dict.Set("network_anonymization_key", key->session_key().network_anonymization_key().ToDebugString());
...
{
    UMA_HISTOGRAM_ENUMERATION("Net.QuicStreamFactory.DoConnectFailureLocation", location);
}
...
{
    UMA_HISTOGRAM_TIMES("Net.QuicSession.StaleConnectionTime", base::TimeTicks::Now() - start_time);
}

void LogValidConnectionTime(base::TimeTicks start_time)
{
    UMA_HISTOGRAM_TIMES("Net.QuicSession.ValidConnectionTime", base::TimeTicks::Now() - start_time);
}

void LogFreshDnsResolveTime(base::TimeTicks start_time)
{
    UMA_HISTOGRAM_TIMES("Net.QuicSession.FreshDnsResolutionTime", base::TimeTicks::Now() - start_time);
}
...
quic::ParsedQuicVersion SelectQuicVersion(
    const std::vector<HostResolverEndpointResult>& endpoint_results, const quic::ParsedQuicVersionVector& supported_versions)
{
...
public:
    QuicCryptoClientConfigOwner(std::unique_ptr<quic::ProofVerifier> proof_verifier, std::unique_ptr<quic::QuicClientSessionCache> session_cache,
    ...
    QuicCryptoClientConfigOwner& operator=(const QuicCryptoClientConfigOwner&) = delete;
    ...
    quic::QuicCryptoClientConfig* config()
    {
        return &config_;
    }
...
// destruction.
class QuicStreamFactory::CryptoClientConfigHandle : public QuicCryptoClientConfigHandle {
public:
    explicit CryptoClientConfigHandle(const QuicCryptoClientConfigMap::iterator& map_iterator)
    ...
    ~CryptoClientConfigHandle() override
    {
        ...
        if (map_iterator_->second->num_refs() == 0) {
            map_iterator_->second->quic_stream_factory()->OnAllCryptoClientRefReleased(map_iterator_);
        }
    }
...
public:
    Job(QuicStreamFactory* factory, quic::ParsedQuicVersion quic_version, HostResolver* host_resolver, const QuicSessionAliasKey& key,
        std::unique_ptr<CryptoClientConfigHandle> client_config_handle, bool was_alternative_service_recently_broken,
        bool retry_on_alternate_network_before_handshake, bool race_stale_dns_on_connection, RequestPriority priority, bool use_dns_aliases,
        bool require_dns_https_alpn, int cert_verify_flags, const NetLogWithSource& net_log);
...
private:
    ...
    void CloseStaleHostConnection()
    {
        ...
        if (session_) {
            ...
            session->CloseSessionOnErrorLater(ERR_FAILED, quic::QUIC_STALE_CONNECTION_CANCELLED, quic::ConnectionCloseBehavior::SEND_CONNECTION_CLOSE_PACKET);
        }
    }
    ...
    bool DoesPeerAddressMatchWithFreshAddressList()
    {
        ...
        std::vector<net::IPEndPoint> endpoints = fresh_resolve_host_request_->GetAddressResults()->endpoints();

        IPEndPoint stale_address = resolve_host_request_->GetAddressResults()->front();
    ...
    {
        if (used) {
            net_log_.AddEvent(NetLogEventType::QUIC_STREAM_FACTORY_JOB_STALE_HOST_TRIED_ON_CONNECTION);
        } else {
            net_log_.AddEvent(NetLogEventType::QUIC_STREAM_FACTORY_JOB_STALE_HOST_NOT_USED_ON_CONNECTION);
        }
    ...
    {
        if (matched) {
            net_log_.AddEvent(NetLogEventType::QUIC_STREAM_FACTORY_JOB_STALE_HOST_RESOLUTION_MATCHED);
        } else {
            net_log_.AddEvent(NetLogEventType::QUIC_STREAM_FACTORY_JOB_STALE_HOST_RESOLUTION_NO_MATCH);
        }
...
QuicStreamFactory::Job::Job(QuicStreamFactory* factory, quic::ParsedQuicVersion quic_version, HostResolver* host_resolver, const QuicSessionAliasKey& key,
    std::unique_ptr<CryptoClientConfigHandle> client_config_handle, bool was_alternative_service_recently_broken,
    bool retry_on_alternate_network_before_handshake, bool race_stale_dns_on_connection, RequestPriority priority, bool use_dns_aliases,
    bool require_dns_https_alpn, int cert_verify_flags, const NetLogWithSource& net_log)
    : factory_(factory)
    ...
    , was_alternative_service_recently_broken_(was_alternative_service_recently_broken)
    , retry_on_alternate_network_before_handshake_(retry_on_alternate_network_before_handshake)
    , race_stale_dns_on_connection_(race_stale_dns_on_connection)
    , net_log_(NetLogWithSource::Make(net_log.net_log(), NetLogSourceType::QUIC_STREAM_FACTORY_JOB))
    , network_(handles::kInvalidNetworkHandle)
{
    DCHECK_EQ(quic_version.IsKnown(), !require_dns_https_alpn);
    net_log_.BeginEvent(NetLogEventType::QUIC_STREAM_FACTORY_JOB, [&] { return NetLogQuicStreamFactoryJobParams(&key_); });
    // Associate |net_log_| with |net_log|.
    net_log_.AddEventReferencingSource(NetLogEventType::QUIC_STREAM_FACTORY_JOB_BOUND_TO_HTTP_STREAM_JOB, net_log.source());
    net_log.AddEventReferencingSource(NetLogEventType::HTTP_STREAM_JOB_BOUND_TO_QUIC_STREAM_FACTORY_JOB, net_log_.source());
}
...
void QuicStreamFactory::Job::OnSessionClosed(QuicChromiumClientSession* session)
{
...
void QuicStreamFactory::Job::OnResolveHostComplete(int rv)
{
    ...
    if (fresh_resolve_host_request_) {
        ...
        if (rv != OK) {
            ...
            resolve_host_request_ = std::move(fresh_resolve_host_request_);
            io_state_ = STATE_RESOLVE_HOST_COMPLETE;
        } else if (factory_->HasMatchingIpSession(key_, HostResolver::GetNonProtocolEndpoints(*fresh_resolve_host_request_->GetEndpointResults()),
                       *fresh_resolve_host_request_->GetDnsAliasResults(), use_dns_aliases_)) {
...
void QuicStreamFactory::Job::OnConnectComplete(int rv)
{
    ...
    if (rv == ERR_QUIC_PROTOCOL_ERROR) {
        HistogramProtocolErrorLocation(JobProtocolErrorLocation::kCryptoConnectFailedAsync);
    }
...
void QuicStreamFactory::Job::PopulateNetErrorDetails(NetErrorDetails* details) const
{
    if (!session_)
        return;
    details->connection_info = QuicHttpStream::ConnectionInfoFromQuicVersion(session_->connection()->version());
...
int QuicStreamFactory::Job::DoResolveHost()
{
    ...
    if (race_stale_dns_on_connection_) {
        ...
        parameters.cache_usage = HostResolver::ResolveHostParameters::CacheUsage::STALE_ALLOWED;
    }
    parameters.secure_dns_policy = key_.session_key().secure_dns_policy();
    resolve_host_request_ = host_resolver_->CreateRequest(key_.destination(), key_.session_key().network_anonymization_key(), net_log_, parameters);
    ...
    int rv = resolve_host_request_->Start(base::BindOnce(&QuicStreamFactory::Job::OnResolveHostComplete, base::Unretained(this)));

    if (rv == ERR_IO_PENDING || !resolve_host_request_->GetStaleInfo() || !resolve_host_request_->GetStaleInfo().value().is_stale()) {
    ...
    fresh_resolve_host_request_ = host_resolver_->CreateRequest(key_.destination(), key_.session_key().network_anonymization_key(), net_log_, parameters);
    // Unretained is safe because |this| owns the request, ensuring cancellation
    // on destruction.
    // This request will only query fresh host resolution.
    int fresh_rv = fresh_resolve_host_request_->Start(base::BindOnce(&QuicStreamFactory::Job::OnResolveHostComplete, base::Unretained(this)));
...
int QuicStreamFactory::Job::DoResolveHostComplete(int rv)
{
    ...
    if (require_dns_https_alpn_) {
        quic_version_ = SelectQuicVersion(*resolve_host_request_->GetEndpointResults(), factory_->supported_versions());
    ...
    if (factory_->HasMatchingIpSession(key_, HostResolver::GetNonProtocolEndpoints(*resolve_host_request_->GetEndpointResults()),
...
int QuicStreamFactory::Job::DoConnect()
{
    ...
    net_log_.AddEntryWithBoolParams(NetLogEventType::QUIC_STREAM_FACTORY_JOB_CONNECT, NetLogEventPhase::BEGIN, "require_confirmation", require_confirmation);

    DCHECK_NE(quic_version_, quic::ParsedQuicVersion::Unsupported());
    int rv = factory_->CreateSession(key_, quic_version_, cert_verify_flags_, require_confirmation, *resolve_host_request_->GetAddressResults(),
        dns_resolution_start_time_, dns_resolution_end_time_, net_log_, &session_, &network_);
    ...
    if (rv != OK) {
        ...
        DCHECK(!session_);
        if (rv == ERR_QUIC_PROTOCOL_ERROR) {
            HistogramProtocolErrorLocation(JobProtocolErrorLocation::kCreateSessionFailed);
        }
        return rv;
    }
    ...
    if (!session_->connection()->connected()) {
        HistogramProtocolErrorLocation(JobProtocolErrorLocation::kSessionStartReadingFailed);
        return ERR_QUIC_PROTOCOL_ERROR;
    }

    rv = session_->CryptoConnect(base::BindOnce(&QuicStreamFactory::Job::OnConnectComplete, GetWeakPtr()));

    if (!session_->connection()->connected() && session_->error() == quic::QUIC_PROOF_INVALID) {
        return ERR_QUIC_HANDSHAKE_FAILED;
    }

    if (rv == ERR_QUIC_PROTOCOL_ERROR) {
        HistogramProtocolErrorLocation(JobProtocolErrorLocation::kCryptoConnectFailedSync);
    }
    return rv;
}
...
{
    UMA_HISTOGRAM_TIMES("Net.QuicSession.TimeFromResolveHostToConfirmConnection", base::TimeTicks::Now() - dns_resolution_start_time_);
    ...
    if (retry_on_alternate_network_before_handshake_ && session_ && !session_->OneRttKeysAvailable() && network_ == factory_->default_network()) {
        if (session_->error() == quic::QUIC_NETWORK_IDLE_TIMEOUT || session_->error() == quic::QUIC_HANDSHAKE_TIMEOUT
            ...
            UMA_HISTOGRAM_BOOLEAN("Net.QuicStreamFactory.AttemptMigrationBeforeHandshake", connection_retried_);
            UMA_HISTOGRAM_ENUMERATION("Net.QuicStreamFactory.AttemptMigrationBeforeHandshake."
                                      "FailedConnectionType",
                NetworkChangeNotifier::GetNetworkConnectionType(factory_->default_network()), NetworkChangeNotifier::ConnectionType::CONNECTION_LAST + 1);
            if (connection_retried_) {
                UMA_HISTOGRAM_ENUMERATION("Net.QuicStreamFactory.MigrationBeforeHandshake.NewConnectionType",
                    NetworkChangeNotifier::GetNetworkConnectionType(network_), NetworkChangeNotifier::ConnectionType::CONNECTION_LAST + 1);
                net_log_.AddEvent(NetLogEventType::QUIC_STREAM_FACTORY_JOB_RETRY_ON_ALTERNATE_NETWORK);
    ...
    if (connection_retried_) {
        UMA_HISTOGRAM_BOOLEAN("Net.QuicStreamFactory.MigrationBeforeHandshake2", rv == OK);
        if (rv == OK) {
            UMA_HISTOGRAM_BOOLEAN("Net.QuicStreamFactory.NetworkChangeDuringMigrationBeforeHandshake", network_ == factory_->default_network());
        } else {
            base::UmaHistogramSparse("Net.QuicStreamFactory.MigrationBeforeHandshakeFailedReason", -rv);
        }
    } else if (network_ != handles::kInvalidNetworkHandle && network_ != factory_->default_network()) {
        UMA_HISTOGRAM_BOOLEAN("Net.QuicStreamFactory.ConnectionOnNonDefaultNetwork", rv == OK);
    }
    ...
    if (factory_->HasMatchingIpSession(key_, { ToIPEndPoint(session_->connection()->peer_address()) },
            /*aliases=*/ {}, use_dns_aliases_)) {
        LogConnectionIpPooling(true);
        session_->connection()->CloseConnection(
            quic::QUIC_CONNECTION_IP_POOLED, "An active session exists for the given IP.", quic::ConnectionCloseBehavior::SEND_CONNECTION_CLOSE_PACKET);
        session_ = nullptr;
        return OK;
    }
    ...
    std::set<std::string> dns_aliases
        = use_dns_aliases_ && resolve_host_request_->GetDnsAliasResults() ? *resolve_host_request_->GetDnsAliasResults() : std::set<std::string>();
...
int QuicStreamRequest::Request(url::SchemeHostPort destination, quic::ParsedQuicVersion quic_version, PrivacyMode privacy_mode, RequestPriority priority,
    const SocketTag& socket_tag, const NetworkAnonymizationKey& network_anonymization_key, SecureDnsPolicy secure_dns_policy, bool use_dns_aliases,
    bool require_dns_https_alpn, int cert_verify_flags, const GURL& url, const NetLogWithSource& net_log, NetErrorDetails* net_error_details,
    CompletionOnceCallback failed_on_default_network_callback, CompletionOnceCallback callback)
{
    ...
    session_key_ = QuicSessionKey(HostPortPair::FromURL(url), privacy_mode, socket_tag, network_anonymization_key, secure_dns_policy, require_dns_https_alpn);

    int rv = factory_->Create(session_key_, std::move(destination), quic_version, priority, use_dns_aliases, cert_verify_flags, url, net_log, this);
    if (rv == ERR_IO_PENDING) {
        net_log_ = net_log;
        callback_ = std::move(callback);
    } else {
...
std::unique_ptr<QuicChromiumClientSession::Handle> QuicStreamRequest::ReleaseSessionHandle()
{
...
void QuicStreamRequest::SetSession(std::unique_ptr<QuicChromiumClientSession::Handle> session)
{
    session_ = std::move(session);
}

bool QuicStreamRequest::CanUseExistingSession(const GURL& url, PrivacyMode privacy_mode, const SocketTag& socket_tag,
    const NetworkAnonymizationKey& network_anonymization_key, SecureDnsPolicy secure_dns_policy, bool require_dns_https_alpn,
    const url::SchemeHostPort& destination) const
{
    return factory_->CanUseExistingSession(
        QuicSessionKey(HostPortPair::FromURL(url), privacy_mode, socket_tag, network_anonymization_key, secure_dns_policy, require_dns_https_alpn),
        destination);
}

QuicStreamFactory::QuicSessionAliasKey::QuicSessionAliasKey(url::SchemeHostPort destination, QuicSessionKey session_key)
    : destination_(std::move(destination))
    , session_key_(std::move(session_key))
{
}

bool QuicStreamFactory::QuicSessionAliasKey::operator<(const QuicSessionAliasKey& other) const
{
    return std::tie(destination_, session_key_) < std::tie(other.destination_, other.session_key_);
}

bool QuicStreamFactory::QuicSessionAliasKey::operator==(const QuicSessionAliasKey& other) const
{
    return destination_ == other.destination_ && session_key_ == other.session_key_;
}

QuicStreamFactory::QuicStreamFactory(NetLog* net_log, HostResolver* host_resolver, SSLConfigService* ssl_config_service,
    ClientSocketFactory* client_socket_factory, HttpServerProperties* http_server_properties, CertVerifier* cert_verifier, CTPolicyEnforcer* ct_policy_enforcer,
    TransportSecurityState* transport_security_state, SCTAuditingDelegate* sct_auditing_delegate,
    SocketPerformanceWatcherFactory* socket_performance_watcher_factory, QuicCryptoClientStreamFactory* quic_crypto_client_stream_factory,
    ...
    , use_network_anonymization_key_for_crypto_configs_(base::FeatureList::IsEnabled(features::kPartitionHttpServerPropertiesByNetworkIsolationKey))
{
...
bool QuicStreamFactory::CanUseExistingSession(const QuicSessionKey& session_key, const url::SchemeHostPort& destination) const
{
    ...
    for (const auto& key_value : active_sessions_) {
        ...
        if ((it != all_sessions_.end()) && (destination == it->second.destination()) && session->CanPool(session_key.host(), session_key)) {
            return true;
        }
    }
...
int QuicStreamFactory::Create(const QuicSessionKey& session_key, url::SchemeHostPort destination, quic::ParsedQuicVersion quic_version,
    RequestPriority priority, bool use_dns_aliases, int cert_verify_flags, const GURL& url, const NetLogWithSource& net_log, QuicStreamRequest* request)
{
    ...
    for (auto session : active_sessions_) {
        quic::QuicClientPromisedInfo* promised = session.second->GetPromised(url, session_key);
        if (!promised)
            continue;
    ...
    if (active_job != active_jobs_.end()) {
        ...
        job_net_log.AddEventReferencingSource(NetLogEventType::QUIC_STREAM_FACTORY_JOB_BOUND_TO_HTTP_STREAM_JOB, net_log.source());
        net_log.AddEventReferencingSource(NetLogEventType::HTTP_STREAM_JOB_BOUND_TO_QUIC_STREAM_FACTORY_JOB, job_net_log.source());
    ...
    if (!active_sessions_.empty()) {
        for (const auto& key_value : active_sessions_) {
            QuicChromiumClientSession* session = key_value.second;
            if (destination == all_sessions_[session].destination() && session->CanPool(session_key.server_id().host(), session_key)) {
                request->SetSession(session->CreateHandle(std::move(destination)));
                return OK;
            }
        }
    ...
    std::unique_ptr<Job> job = std::make_unique<Job>(this, quic_version, host_resolver_, key, CreateCryptoConfigHandle(session_key.network_anonymization_key()),
        WasQuicRecentlyBroken(session_key), params_.retry_on_alternate_network_before_handshake, params_.race_stale_dns_on_connection, priority,
        use_dns_aliases, session_key.require_dns_https_alpn(), cert_verify_flags, net_log);
    int rv = job->Run(base::BindOnce(&QuicStreamFactory::OnJobComplete, base::Unretained(this), job.get()));
    if (rv == ERR_IO_PENDING) {
...
void QuicStreamFactory::OnBlackholeAfterHandshakeConfirmed(QuicChromiumClientSession* session)
{
...
void QuicStreamFactory::SetRequestPriority(QuicStreamRequest* request, RequestPriority priority)
{
    auto job_iter = active_jobs_.find(request->session_key());
...
void QuicStreamFactory::CloseAllSessions(int error, quic::QuicErrorCode quic_error)
{
    ...
    while (!all_sessions_.empty()) {
        ...
        all_sessions_.begin()->first->CloseSessionOnError(error, quic_error, quic::ConnectionCloseBehavior::SEND_CONNECTION_CLOSE_PACKET);
...
void QuicStreamFactory::ClearCachedStatesInCryptoConfig(const base::RepeatingCallback<bool(const GURL&)>& origin_filter)
{
...
int QuicStreamFactory::ConfigureSocket(DatagramClientSocket* socket, IPEndPoint addr, handles::NetworkHandle network, const SocketTag& socket_tag)
{
...
handles::NetworkHandle QuicStreamFactory::FindAlternateNetwork(handles::NetworkHandle old_network)
{
...
std::unique_ptr<DatagramClientSocket> QuicStreamFactory::CreateSocket(NetLog* net_log, const NetLogSource& source)
{
...
void QuicStreamFactory::OnNetworkConnected(handles::NetworkHandle network)
{
    ...
    if (params_.migrate_sessions_on_network_change_v2) {
        NetLogWithSource net_log = NetLogWithSource::Make(net_log_, NetLogSourceType::QUIC_CONNECTION_MIGRATION);
        net_log.AddEventWithStringParams(NetLogEventType::QUIC_CONNECTION_MIGRATION_PLATFORM_NOTIFICATION, "signal", "OnNetworkConnected");
    }
...
void QuicStreamFactory::OnNetworkDisconnected(handles::NetworkHandle network)
{
    ...
    if (params_.migrate_sessions_on_network_change_v2) {
        NetLogWithSource net_log = NetLogWithSource::Make(net_log_, NetLogSourceType::QUIC_CONNECTION_MIGRATION);
        net_log.AddEventWithStringParams(NetLogEventType::QUIC_CONNECTION_MIGRATION_PLATFORM_NOTIFICATION, "signal", "OnNetworkDisconnected");
    }
...
void QuicStreamFactory::OnNetworkSoonToDisconnect(handles::NetworkHandle network)
{
...
void QuicStreamFactory::OnNetworkMadeDefault(handles::NetworkHandle network)
{
    ...
    if (params_.migrate_sessions_on_network_change_v2) {
        NetLogWithSource net_log = NetLogWithSource::Make(net_log_, NetLogSourceType::QUIC_CONNECTION_MIGRATION);
        net_log.AddEventWithStringParams(NetLogEventType::QUIC_CONNECTION_MIGRATION_PLATFORM_NOTIFICATION, "signal", "OnNetworkMadeDefault");
    }
...
void QuicStreamFactory::set_is_quic_known_to_work_on_current_network(bool is_quic_known_to_work_on_current_network)
{
...
base::TimeDelta QuicStreamFactory::GetTimeDelayForWaitingJob(const QuicSessionKey& session_key)
{
...
const std::set<std::string>& QuicStreamFactory::GetDnsAliasesForSessionKey(const QuicSessionKey& key) const
{
...
bool QuicStreamFactory::HasActiveSession(const QuicSessionKey& session_key) const
{
...
int QuicStreamFactory::CreateSession(const QuicSessionAliasKey& key, quic::ParsedQuicVersion quic_version, int cert_verify_flags, bool require_confirmation,
    const AddressList& address_list, base::TimeTicks dns_resolution_start_time, base::TimeTicks dns_resolution_end_time, const NetLogWithSource& net_log,
    QuicChromiumClientSession** session, handles::NetworkHandle* network)
{
    ...
    int rv = ConfigureSocket(socket.get(), addr, *network, key.session_key().socket_tag());
    if (rv != OK)
        return rv;
    ...
    if (!alarm_factory_.get()) {
        alarm_factory_ = std::make_unique<QuicChromiumAlarmFactory>(base::ThreadTaskRunnerHandle::Get().get(), clock_);
    }
    ...
    if (params_.max_server_configs_stored_in_properties > 0) {
        server_info = std::make_unique<PropertiesBasedQuicServerInfo>(server_id, key.session_key().network_anonymization_key(), http_server_properties_);
    }
    ...
    QuicChromiumPacketWriter* writer = new QuicChromiumPacketWriter(socket.get(), task_runner_);
    quic::QuicConnection* connection = new quic::QuicConnection(connection_id, quic::QuicSocketAddress(), ToQuicSocketAddress(addr), helper_.get(),
    ...
    if (!quic_version.HasIetfInvariantHeader() && !config.HasClientSentConnectionOption(quic::kNSTP, quic::Perspective::IS_CLIENT)) {
    ...
    if (socket_performance_watcher_factory_) {
        socket_performance_watcher
            = socket_performance_watcher_factory_->CreateSocketPerformanceWatcher(SocketPerformanceWatcherFactory::PROTOCOL_QUIC, address_list);
    }
    ...
    *session = new QuicChromiumClientSession(connection, std::move(socket), this, quic_crypto_client_stream_factory_, clock_, transport_security_state_,
        ssl_config_service_, std::move(server_info), key.session_key(), require_confirmation, params_.migrate_sessions_early_v2,
        ...
        params_.allow_port_migration, params_.idle_session_migration_period, params_.max_time_on_non_default_network,
        ...
        yield_after_duration_, params_.headers_include_h2_stream_dependency, cert_verify_flags, config, std::move(crypto_config_handle),
        network_connection_.connection_description(), dns_resolution_start_time, dns_resolution_end_time, std::make_unique<quic::QuicClientPushPromiseIndex>(),
        push_delegate_, tick_clock_, task_runner_, std::move(socket_performance_watcher), net_log.net_log());
...
void QuicStreamFactory::ActivateSession(const QuicSessionAliasKey& key, QuicChromiumClientSession* session, std::set<std::string> dns_aliases)
{
    DCHECK(!HasActiveSession(key.session_key()));
...
void QuicStreamFactory::MarkAllActiveSessionsGoingAway(AllActiveSessionsGoingAwayReason reason)
{
    while (!active_sessions_.empty()) {
...
bool QuicStreamFactory::WasQuicRecentlyBroken(const QuicSessionKey& session_key) const
{
...
void QuicStreamFactory::ProcessGoingAwaySession(QuicChromiumClientSession* session, const quic::QuicServerId& server_id, bool session_was_active)
{
    ...
    HistogramBrokenAlternateProtocolLocation(BROKEN_ALTERNATE_PROTOCOL_LOCATION_QUIC_STREAM_FACTORY);
...
void QuicStreamFactory::MapSessionToAliasKey(QuicChromiumClientSession* session, const QuicSessionAliasKey& key, std::set<std::string> dns_aliases)
{
    session_aliases_[session].insert(key);
    dns_aliases_by_session_key_[key.session_key()] = std::move(dns_aliases);
}

void QuicStreamFactory::UnmapSessionFromSessionAliases(QuicChromiumClientSession* session)
{
...
std::unique_ptr<QuicStreamFactory::CryptoClientConfigHandle> QuicStreamFactory::CreateCryptoConfigHandle(
    const NetworkAnonymizationKey& network_anonymization_key)
{
    ...
    if (mru_iterator != recent_crypto_config_map_.end()) {
        ...
        map_iterator = active_crypto_config_map_.emplace(std::make_pair(actual_network_anonymization_key, std::move(mru_iterator->second))).first;
    ...
    std::unique_ptr<QuicCryptoClientConfigOwner> crypto_config_owner = std::make_unique<QuicCryptoClientConfigOwner>(
        std::make_unique<ProofVerifierChromium>(cert_verifier_, ct_policy_enforcer_, transport_security_state_, sct_auditing_delegate_,
    ...
    if (SSLKeyLoggerManager::IsActive()) {
        SSL_CTX_set_keylog_callback(crypto_config->ssl_ctx(), SSLKeyLoggerManager::KeyLogCallback);
    }
    ...
    map_iterator = active_crypto_config_map_.emplace(std::make_pair(actual_network_anonymization_key, std::move(crypto_config_owner))).first;
    return std::make_unique<CryptoClientConfigHandle>(map_iterator);
}

void QuicStreamFactory::OnAllCryptoClientRefReleased(QuicCryptoClientConfigMap::iterator& map_iterator)
{
...
void QuicStreamFactory::CollectDataOnPlatformNotification(enum QuicPlatformNotification notification, handles::NetworkHandle affected_network) const
{
...
std::unique_ptr<QuicCryptoClientConfigHandle> QuicStreamFactory::GetCryptoConfigForTesting(const NetworkAnonymizationKey& network_anonymization_key)
{
    return CreateCryptoConfigHandle(network_anonymization_key);
}

bool QuicStreamFactory::CryptoConfigCacheIsEmptyForTesting(const quic::QuicServerId& server_id, const NetworkAnonymizationKey& network_anonymization_key)
{
