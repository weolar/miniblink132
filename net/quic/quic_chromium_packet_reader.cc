...
const size_t kReadBufferSize = static_cast<size_t>(quic::kMaxIncomingPacketSize + 1);
} // namespace

QuicChromiumPacketReader::QuicChromiumPacketReader(std::unique_ptr<DatagramClientSocket> socket, const quic::QuicClock* clock, Visitor* visitor,
    int yield_after_packets, quic::QuicTime::Delta yield_after_duration, bool report_ecn, const NetLogWithSource& net_log)
...
void QuicChromiumPacketReader::StartReading()
{
    for (;;) {
        ...
        CHECK(socket_);
        read_pending_ = true;
        int rv = socket_->Read(read_buffer_.get(), read_buffer_->size(), base::BindOnce(&QuicChromiumPacketReader::OnReadComplete, weak_factory_.GetWeakPtr()));
        ...
        if (++num_packets_read_ > yield_after_packets_ || clock_->Now() > yield_after_) {
            ...
            base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
                FROM_HERE, base::BindOnce(&QuicChromiumPacketReader::OnReadComplete, weak_factory_.GetWeakPtr(), rv));
        } else {
...
static_assert(static_cast<EcnCodePoint>(quic::ECN_NOT_ECT) == ECN_NOT_ECT && static_cast<EcnCodePoint>(quic::ECN_ECT1) == ECN_ECT1
        && static_cast<EcnCodePoint>(quic::ECN_ECT0) == ECN_ECT0 && static_cast<EcnCodePoint>(quic::ECN_CE) == ECN_CE,
    "Mismatch ECN codepoint values");
bool QuicChromiumPacketReader::ProcessReadResult(int result)
{
    read_pending_ = false;
    if (result <= 0 && net_log_.IsCapturing()) {
        net_log_.AddEventWithIntParams(NetLogEventType::QUIC_READ_ERROR, "net_error", result);
    }
    if (result == 0) {
    ...
    return visitor_->OnPacket(packet, ToQuicSocketAddress(local_address), ToQuicSocketAddress(peer_address)) && self;
}
