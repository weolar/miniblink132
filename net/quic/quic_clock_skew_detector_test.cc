...
{
    EXPECT_FALSE(detector_.ClockSkewDetected(start_ticks_time_, start_wall_time_));
}

TEST_F(QuicClockSkewDetectorTest, NoOffset)
{
    base::TimeDelta delta = base::Seconds(57);
    EXPECT_FALSE(detector_.ClockSkewDetected(start_ticks_time_ + delta, start_wall_time_ + delta));
}

TEST_F(QuicClockSkewDetectorTest, SmallOffset)
{
    base::TimeDelta delta = base::Milliseconds(57);
    EXPECT_FALSE(detector_.ClockSkewDetected(start_ticks_time_, start_wall_time_ + delta));
}
...
TEST_F(QuicClockSkewDetectorTest, ManySmallOffset)
{
    for (int i = 0; i < 10; ++i) {
        ...
        EXPECT_FALSE(detector_.ClockSkewDetected(start_ticks_time_, start_wall_time_ + i * delta));
    }
}
...
TEST_F(QuicClockSkewDetectorTest, LargeOffset)
{
    ...
    EXPECT_TRUE(detector_.ClockSkewDetected(start_ticks_time_, start_wall_time_ + delta));
}
...
TEST_F(QuicClockSkewDetectorTest, LargeOffsetThenSmallOffset)
{
    ...
    EXPECT_FALSE(detector_.ClockSkewDetected(start_ticks_time_ + small_delta, start_wall_time_ + delta + small_delta));
}
