...
// configuration.
inline NET_EXPORT_PRIVATE quic::ParsedQuicVersionVector DefaultSupportedQuicVersions()
{
...
inline NET_EXPORT_PRIVATE quic::ParsedQuicVersion SupportedQuicVersionForProxying()
{
...
{
    return quic::ParsedQuicVersionVector { quic::ParsedQuicVersion::Q046(), quic::ParsedQuicVersion::Draft29() };
}
...
inline NET_EXPORT_PRIVATE quic::ParsedQuicVersionVector AllSupportedQuicVersions()
{
...
constexpr base::TimeDelta kDefaultIdleSessionMigrationPeriod = base::Seconds(30);
...
// enforce this.
constexpr base::TimeDelta kDefaultRetransmittableOnWireTimeout = base::Milliseconds(200);

// The default maximum time QUIC session could be on non-default network before
// migrate back to default network.
inline constexpr base::TimeDelta kMaxTimeOnNonDefaultNetwork = base::Seconds(128);
...
struct NET_EXPORT QuicParams {
    ...
    quic::ParsedQuicVersionVector supported_versions = DefaultSupportedQuicVersions();
    ...
    // finished.
    base::TimeDelta max_time_before_crypto_handshake = base::Seconds(quic::kMaxTimeForCryptoHandshakeSecs);
    // Maximum idle time before the crypto handshake has completed.
    base::TimeDelta max_idle_time_before_crypto_handshake = base::Seconds(quic::kInitialIdleTimeoutSecs);
    ...
    // discrepancy.
    bool migrate_sessions_on_network_change_v2 = base::FeatureList::IsEnabled(features::kMigrateSessionsOnNetworkChangeV2);
    ...
    base::TimeDelta idle_session_migration_period = kDefaultIdleSessionMigrationPeriod;
    ...
    int max_migrations_to_non_default_network_on_write_error = kMaxMigrationsToNonDefaultNetworkOnWriteError;
    // Maximum number of migrations to the non-default network on path
    // degrading per network for each session.
    int max_migrations_to_non_default_network_on_path_degrading = kMaxMigrationsToNonDefaultNetworkOnPathDegrading;
...
public:
    QuicContext();
    explicit QuicContext(std::unique_ptr<quic::QuicConnectionHelperInterface> helper);
    virtual ~QuicContext();
    ...
    {
        return helper_->GetRandomGenerator();
    }
    ...
    void SetHelperForTesting(std::unique_ptr<quic::QuicConnectionHelperInterface> helper)
    {
...
void ConfigureQuicCryptoClientConfig(quic::QuicCryptoClientConfig& crypto_config);
