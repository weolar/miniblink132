...
public:
    MOCK_METHOD(void, OnConnected, (scoped_refptr<HttpResponseHeaders>), (override));
    MOCK_METHOD(void, OnConnectionFailed, (const WebTransportError&), (override));
    MOCK_METHOD(void, OnClosed, (const std::optional<WebTransportCloseInfo>&), (override));
...
public:
    ...
    void set_wall_now(quic::QuicWallTime now)
    {
        wall_now_ = now;
    }
...
public:
    ...
    void SetUp() override
    {
        ...
        anonymization_key_ = NetworkAnonymizationKey::CreateSameSite(SchemefulSite(origin_));

        // By default, quit on error instead of waiting for RunLoop() to time out.
        ON_CALL(visitor_, OnConnectionFailed(_)).WillByDefault([this](const WebTransportError& error) {
            LOG(ERROR) << "Connection failed: " << error;
            if (run_loop_) {
                run_loop_->Quit();
            }
        });
        ON_CALL(visitor_, OnError(_)).WillByDefault([this](const WebTransportError& error) {
    ...
    void BuildContext(std::unique_ptr<ProxyResolutionService> proxy_resolution_service)
    {
        ...
        // roots in QUIC.
        quic_context->params()->origins_to_force_quic_on.insert(HostPortPair("test.example.com", 0));
    ...
    {
        return GURL { base::StrCat({ "https://test.example.com:", base::NumberToString(port_), suffix }) };
    }
    ...
    void StartServer(std::unique_ptr<quic::ProofSource> proof_source = nullptr)
    {
        ...
        server_ = std::make_unique<QuicSimpleServer>(
            std::move(proof_source), quic::QuicConfig(), quic::QuicCryptoServerConfig::ConfigOptions(), AllSupportedQuicVersions(), &backend_);
        ASSERT_TRUE(server_->CreateUDPSocketAndListen(quic::QuicSocketAddress(quic::QuicIpAddress::Any6(), /*port=*/0)));
...
TEST_F(DedicatedWebTransportHttp3Test, Connect)
{
    StartServer();
    client_
        = std::make_unique<DedicatedWebTransportHttp3Client>(GetURL("/echo"), origin_, &visitor_, anonymization_key_, context_.get(), WebTransportParameters());
...
{
    BuildContext(ConfiguredProxyResolutionService::CreateFixedFromProxyChainsForTest(
        { ProxyChain::FromSchemeHostAndPort(ProxyServer::SCHEME_HTTPS, "test", 80) }, TRAFFIC_ANNOTATION_FOR_TESTS));
    StartServer();
    client_
        = std::make_unique<DedicatedWebTransportHttp3Client>(GetURL("/echo"), origin_, &visitor_, anonymization_key_, context_.get(), WebTransportParameters());
...
TEST_F(DedicatedWebTransportHttp3Test, MAYBE_CloseTimeout)
{
    ...
    auto noop_socket = std::make_unique<UDPServerSocket>(/*net_log=*/nullptr, NetLogSource());
...
TEST_F(DedicatedWebTransportHttp3Test, CloseReason)
{
    ...
    client_ = std::make_unique<DedicatedWebTransportHttp3Client>(
        GetURL("/session-close"), origin_, &visitor_, anonymization_key_, context_.get(), WebTransportParameters());
    ...
    quic::WebTransportStream* stream = client_->session()->OpenOutgoingUnidirectionalStream();
    ...
    EXPECT_CALL(visitor_, OnClosed(_)).WillOnce(DoAll(StopRunning(), SaveArg<0>(&received_close_info)));
    Run();
