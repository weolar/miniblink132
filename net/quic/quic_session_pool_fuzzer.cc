...
struct FuzzerEnvironment {
        ...
        : scheme_host_port(url::kHttpsScheme, kServerHostName, kServerPort)
    {
        ...
        verify_details.cert_verify_result.verified_cert = X509Certificate::CreateFromBytes(kCertData);
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    std::unique_ptr<ContextHostResolver> host_resolver
        = CreateFuzzedContextHostResolver(HostResolver::ManagerOptions(), nullptr, &data_provider, true /* enable_caching */);
    ...
    params.max_server_configs_stored_in_properties = data_provider.ConsumeBool() ? 1 : 0;
    ...
    if (!params.close_sessions_on_ip_change) {
        ...
        if (!params.goaway_sessions_on_ip_change) {
            params.migrate_sessions_on_network_change_v2 = data_provider.ConsumeBool();
            if (params.migrate_sessions_on_network_change_v2) {
                params.migrate_sessions_early_v2 = data_provider.ConsumeBool();
                params.retry_on_alternate_network_before_handshake = data_provider.ConsumeBool();
                params.migrate_idle_sessions = data_provider.ConsumeBool();
            }
        }
    }

    std::unique_ptr<QuicSessionPool> factory
        = std::make_unique<QuicSessionPool>(env.net_log.net_log(), host_resolver.get(), env.ssl_config_service.get(), &socket_factory, &http_server_properties,
            env.cert_verifier.get(), &env.transport_security_state, nullptr, nullptr, nullptr, &env.crypto_client_stream_factory, &env.quic_context);

    QuicSessionRequest request(factory.get());
    TestCompletionCallback callback;
    NetErrorDetails net_error_details;
    quic::ParsedQuicVersionVector versions = AllSupportedQuicVersions();
    quic::ParsedQuicVersion version = versions[data_provider.ConsumeIntegralInRange<size_t>(0, versions.size() - 1)];

    quic::QuicEnableVersion(version);

    request.Request(env.scheme_host_port, version, ProxyChain::Direct(), TRAFFIC_ANNOTATION_FOR_TESTS, /*http_user_agent_settings=*/nullptr,
        SessionUsage::kDestination, PRIVACY_MODE_DISABLED, DEFAULT_PRIORITY, SocketTag(), NetworkAnonymizationKey(), SecureDnsPolicy::kAllow,
        /*require_dns_https_alpn=*/false, kCertVerifyFlags, GURL(kUrl), env.net_log, &net_error_details, MultiplexedSessionCreationInitiator::kUnknown,
        /*failed_on_default_network_callback=*/CompletionOnceCallback(), callback.callback());
    ...
    stream->InitializeStream(true, DEFAULT_PRIORITY, env.net_log, CompletionOnceCallback());

    HttpResponseInfo response;
    HttpRequestHeaders request_headers;
    if (OK != stream->SendRequest(request_headers, &response, callback.callback())) {
        return 0;
    }
