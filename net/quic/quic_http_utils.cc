...
spdy::SpdyPriority ConvertRequestPriorityToQuicPriority(const RequestPriority priority)
{
...
RequestPriority ConvertQuicPriorityToRequestPriority(spdy::SpdyPriority priority)
{
    // Handle invalid values gracefully.
    return (priority >= 5) ? IDLE : static_cast<RequestPriority>(HIGHEST - priority);
}

base::Value::Dict QuicRequestNetLogParams(
    quic::QuicStreamId stream_id, const quiche::HttpHeaderBlock* headers, quic::QuicStreamPriority priority, NetLogCaptureMode capture_mode)
{
    ...
    case quic::QuicPriorityType::kWebTransport: {
        ...
        dict.Set("web_transport_session_id", static_cast<int>(web_transport_priority.session_id));
        ...
        dict.Set("web_transport_send_group_number", static_cast<double>(web_transport_priority.send_group_number));
        dict.Set("web_transport_send_order", static_cast<double>(web_transport_priority.send_order));
        break;
    }
    }
...
base::Value::Dict QuicResponseNetLogParams(
    quic::QuicStreamId stream_id, bool fin_received, const quiche::HttpHeaderBlock* headers, NetLogCaptureMode capture_mode)
{
    base::Value::Dict dict = HttpHeaderBlockNetLogParams(headers, capture_mode);
    dict.Set("quic_stream_id", static_cast<int>(stream_id));
