...
class NET_EXPORT_PRIVATE QuicProxyDatagramClientSocket : public DatagramClientSocket, public quic::QuicSpdyStream::Http3DatagramVisitor {
public:
    ...
    QuicProxyDatagramClientSocket& operator=(const QuicProxyDatagramClientSocket&) = delete;
    ...
    int ConnectViaStream(const IPEndPoint& local_address, const IPEndPoint& proxy_peer_address, std::unique_ptr<QuicChromiumClientStream::Handle> stream,
    ...
    void OnHttp3Datagram(quic::QuicStreamId stream_id, std::string_view payload) override;
    void OnUnknownCapsule(quic::QuicStreamId stream_id, const quiche::UnknownCapsule& capsule) override;

    const HttpResponseInfo* GetConnectResponseInfo() const;
    bool IsConnected() const;

    const std::queue<std::string>& GetDatagramsForTesting()
    {
        return datagrams_;
    }

    static constexpr char kMaxQueueSizeHistogram[] = "Net.QuicProxyDatagramClientSocket.MaxQueueSizeReached";

    // Upper bound for datagrams in queue.
    static constexpr size_t kMaxDatagramQueueSize = 16;

private:
    enum State { STATE_DISCONNECTED, STATE_SEND_REQUEST, STATE_SEND_REQUEST_COMPLETE, STATE_READ_REPLY, STATE_READ_REPLY_COMPLETE, STATE_CONNECT_COMPLETE };
    ...
    int proxy_chain_index()
    {
        return proxy_chain_.length() - 1;
    }
