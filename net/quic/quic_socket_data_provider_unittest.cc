...
public:
    QuicSocketDataProviderTest()
        : packet_maker_(std::make_unique<QuicTestPacketMaker>(version_, quic::QuicUtils::CreateRandomConnectionId(context_.random_generator()),
            context_.clock(), "hostname", quic::Perspective::IS_CLIENT,
    ...
    {
        return packet_maker_->Packet(packet_number).AddMessageFrame(base::NumberToString(packet_number)).Build();
    }
...
TEST_F(QuicSocketDataProviderTest, LinearSequenceSync)
{
    ...
    base::SequencedTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindLambdaForTesting([&]() {
        std::unique_ptr<DatagramClientSocket> socket
            = socket_factory.CreateDatagramClientSocket(DatagramSocket::BindType::DEFAULT_BIND, nullptr, net_log_with_source_.source());
        socket->Connect(IPEndPoint());

        for (uint64_t packet_number = 1; packet_number < 4; packet_number++) {
            std::unique_ptr<quic::QuicReceivedPacket> packet = TestPacket(packet_number);
            scoped_refptr<StringIOBuffer> buffer = base::MakeRefCounted<StringIOBuffer>(std::string(packet->data(), packet->length()));
            EXPECT_EQ(static_cast<int>(packet->length()), socket->Write(buffer.get(), packet->length(), base::DoNothing(), TRAFFIC_ANNOTATION_FOR_TESTS));
        }
    }));
...
TEST_F(QuicSocketDataProviderTest, LinearSequenceAsync)
{
    ...
    int next_packet = 1;
    base::RepeatingCallback<void(int)> callback = base::BindLambdaForTesting([&](int result) {
        EXPECT_GT(result, 0); // Bytes written or, on the first call, one.
        if (next_packet <= 3) {
            std::unique_ptr<quic::QuicReceivedPacket> packet = TestPacket(next_packet++);
            scoped_refptr<StringIOBuffer> buffer = base::MakeRefCounted<StringIOBuffer>(std::string(packet->data(), packet->length()));
            EXPECT_EQ(ERR_IO_PENDING, socket->Write(buffer.get(), packet->length(), callback, TRAFFIC_ANNOTATION_FOR_TESTS));
        }
    });
    callback.Run(1);
    socket_data.RunUntilAllConsumed();
...
TEST_F(QuicSocketDataProviderTest, ReadTos)
{
    ...
    EXPECT_EQ(static_cast<int>(TestPacket(1)->length()), socket->Read(read_buffer.get(), 100, base::DoNothing()));
...
TEST_F(QuicSocketDataProviderTest, AddReadError)
{
    ...
    scoped_refptr<GrowableIOBuffer> read_buffer = base::MakeRefCounted<GrowableIOBuffer>();
    read_buffer->SetCapacity(100);
    EXPECT_EQ(ERR_CONNECTION_ABORTED, socket->Read(read_buffer.get(), 100, base::DoNothing()));
...
TEST_F(QuicSocketDataProviderTest, MismatchedWrite)
{
    ...
    EXPECT_NONFATAL_FAILURE(
        EXPECT_EQ(ERR_UNEXPECTED, socket->Write(buffer.get(), packet->length(), base::DoNothing(), TRAFFIC_ANNOTATION_FOR_TESTS)), "Expectation 'p1' not met.");
}
...
TEST_F(QuicSocketDataProviderTest, ReadBlocksWrite)
{
    ...
    std::unique_ptr<quic::QuicReceivedPacket> packet = TestPacket(1);
    scoped_refptr<StringIOBuffer> buffer = base::MakeRefCounted<StringIOBuffer>(std::string(packet->data(), packet->length()));
    EXPECT_NONFATAL_FAILURE(EXPECT_EQ(ERR_UNEXPECTED, socket->Write(buffer.get(), packet->length(), base::DoNothing(), TRAFFIC_ANNOTATION_FOR_TESTS)),
...
TEST_F(QuicSocketDataProviderTest, WriteDelaysRead)
{
    ...
    base::OnceCallback<void(int)> read_callback = base::BindLambdaForTesting([&](int result) {
        EXPECT_EQ(result, static_cast<int>(TestPacket(22222)->length()));
        read_completed = true;
    });
    scoped_refptr<GrowableIOBuffer> read_buffer = base::MakeRefCounted<GrowableIOBuffer>();
    read_buffer->SetCapacity(100);
    EXPECT_EQ(ERR_IO_PENDING, socket->Read(read_buffer.get(), 100, std::move(read_callback)));
...
TEST_F(QuicSocketDataProviderTest, PauseDelaysCalls)
{
    ...
    base::OnceCallback<void(int)> write_callback = base::BindLambdaForTesting([&](int result) {
        EXPECT_EQ(result, static_cast<int>(TestPacket(3)->length()));
        write_completed = true;
    });
    std::unique_ptr<quic::QuicReceivedPacket> packet = TestPacket(3);
    scoped_refptr<StringIOBuffer> buffer = base::MakeRefCounted<StringIOBuffer>(std::string(packet->data(), packet->length()));
    EXPECT_EQ(ERR_IO_PENDING, socket->Write(buffer.get(), packet->length(), std::move(write_callback), TRAFFIC_ANNOTATION_FOR_TESTS));
...
TEST_F(QuicSocketDataProviderTest, ParallelReadAndWrite)
{
    for (bool read_first : { false, true }) {
        ...
        std::unique_ptr<quic::QuicReceivedPacket> packet = TestPacket(1);
        scoped_refptr<IOBuffer> buffer = base::MakeRefCounted<StringIOBuffer>(std::string(packet->data(), packet->length()));
        EXPECT_EQ(static_cast<int>(packet->length()), socket->Write(buffer.get(), packet->length(), base::DoNothing(), TRAFFIC_ANNOTATION_FOR_TESTS));

        scoped_refptr<GrowableIOBuffer> read_buffer = base::MakeRefCounted<GrowableIOBuffer>();
        read_buffer->SetCapacity(100);
        auto do_read = [&]() { EXPECT_EQ(static_cast<int>(TestPacket(2)->length()), socket->Read(read_buffer.get(), 100, base::DoNothing())); };

        std::unique_ptr<quic::QuicReceivedPacket> write_packet = TestPacket(3);
        buffer = base::MakeRefCounted<StringIOBuffer>(std::string(write_packet->data(), write_packet->length()));

        auto do_write = [&]() {
            EXPECT_EQ(
                static_cast<int>(write_packet->length()), socket->Write(buffer.get(), write_packet->length(), base::DoNothing(), TRAFFIC_ANNOTATION_FOR_TESTS));
        };
...
TEST_F(QuicSocketDataProviderTest, MultipleReadsReady)
{
    ...
    scoped_refptr<StringIOBuffer> buffer = base::MakeRefCounted<StringIOBuffer>(std::string(packet->data(), packet->length()));
    EXPECT_EQ(static_cast<int>(packet->length()), socket->Write(buffer.get(), packet->length(), base::DoNothing(), TRAFFIC_ANNOTATION_FOR_TESTS));
    EXPECT_CHECK_DEATH(socket->Read(buffer.get(), buffer->size(), base::DoNothing()));
}
