...
public:
    TestProofVerifierCallback(TestCompletionCallback* comp_callback, bool* ok, string* error_details)
    ...
    void Run(bool ok, const string& error_details, std::unique_ptr<quic::ProofVerifyDetails>* details) override
    {
        *ok_ = ok;
        *error_details_ = error_details;
...
// matches |expected_ok|.
void RunVerification(quic::ProofVerifier* verifier, const string& hostname, const uint16_t port, const string& server_config,
    quic::QuicTransportVersion quic_version, std::string_view chlo_hash, const std::vector<string>& certs, const string& proof, bool expected_ok)
{
    std::unique_ptr<quic::ProofVerifyDetails> details;
    TestCompletionCallback comp_callback;
    bool ok;
    string error_details;
    std::unique_ptr<quic::ProofVerifyContext> verify_context(quic::test::crypto_test_utils::ProofVerifyContextForTesting());
    auto callback = std::make_unique<TestProofVerifierCallback>(&comp_callback, &ok, &error_details);

    quic::QuicAsyncStatus status = verifier->VerifyProof(
        hostname, port, server_config, quic_version, chlo_hash, certs, "", proof, verify_context.get(), &error_details, &details, std::move(callback));
...
public:
    explicit TestCallback(bool* called, bool* ok, quiche::QuicheReferenceCountedPointer<quic::ProofSource::Chain>* chain, quic::QuicCryptoProof* proof)
        : called_(called)
        , ok_(ok)
        , chain_(chain)
        , proof_(proof)
...
private:
    ...
    raw_ptr<bool> ok_;
    raw_ptr<quiche::QuicheReferenceCountedPointer<quic::ProofSource::Chain>> chain_;
...
INSTANTIATE_TEST_SUITE_P(QuicTransportVersion, ProofTest, ::testing::ValuesIn(AllSupportedQuicVersions()), ::testing::PrintToStringParamName());

TEST_P(ProofTest, Verify)
{
    std::unique_ptr<quic::ProofSource> source(quic::test::crypto_test_utils::ProofSourceForTesting());
    std::unique_ptr<quic::ProofVerifier> verifier(quic::test::crypto_test_utils::ProofVerifierForTesting());
    ...
    auto first_cb = std::make_unique<TestCallback>(&first_called, &first_ok, &first_chain, &first_proof);

    // GetProof here expects the async method to invoke the callback
    // synchronously.
    source->GetProof(server_addr, client_addr, hostname, server_config, quic_version, first_chlo_hash, std::move(first_cb));
    source->GetProof(server_addr, client_addr, hostname, server_config, quic_version, second_chlo_hash, std::move(cb));
    ASSERT_TRUE(called);
    ...
    RunVerification(verifier.get(), hostname, port, server_config, quic_version, first_chlo_hash, chain->certs, proof.signature, true);

    RunVerification(verifier.get(), "foo.com", port, server_config, quic_version, first_chlo_hash, chain->certs, proof.signature, false);

    RunVerification(
        verifier.get(), server_config.substr(1, string::npos), port, server_config, quic_version, first_chlo_hash, chain->certs, proof.signature, false);

    const string corrupt_signature = "1" + proof.signature;
    RunVerification(verifier.get(), hostname, port, server_config, quic_version, first_chlo_hash, chain->certs, corrupt_signature, false);
    ...
    RunVerification(verifier.get(), "foo.com", port, server_config, quic_version, first_chlo_hash, wrong_certs, corrupt_signature, false);
}
...
public:
    TestingSignatureCallback(bool* ok_out, std::string* signature_out)
        : ok_out_(ok_out)
        , signature_out_(signature_out)
    {
    }

    void Run(bool ok, std::string signature, std::unique_ptr<quic::ProofSource::Details> /*details*/) override
    {
        *ok_out_ = ok;
...
TEST_P(ProofTest, TlsSignature)
{
    ...
    quiche::QuicheReferenceCountedPointer<quic::ProofSource::Chain> chain = source->GetCertChain(server_address, client_address, hostname, &cert_matched_sni);
    ...
    string sig;
    bool success;
    std::unique_ptr<TestingSignatureCallback> callback = std::make_unique<TestingSignatureCallback>(&success, &sig);
    source->ComputeTlsSignature(server_address, client_address, hostname, SSL_SIGN_RSA_PSS_SHA256, to_be_signed, std::move(callback));
    EXPECT_TRUE(success);
    ...
    EVP_PKEY_CTX* ctx;
    ASSERT_EQ(EVP_DigestVerifyInit(md_ctx.get(), &ctx, EVP_sha256(), nullptr, pkey.get()), 1);
    ASSERT_EQ(EVP_PKEY_CTX_set_rsa_padding(ctx, RSA_PKCS1_PSS_PADDING), 1);
    ASSERT_EQ(EVP_PKEY_CTX_set_rsa_pss_saltlen(ctx, -1), 1);
    ASSERT_EQ(EVP_DigestVerifyUpdate(md_ctx.get(), to_be_signed.data(), to_be_signed.size()), 1);
    EXPECT_EQ(EVP_DigestVerifyFinal(md_ctx.get(), reinterpret_cast<const uint8_t*>(sig.data()), sig.size()), 1);
}
...
TEST_P(ProofTest, UseAfterFree)
{
    ...
    // GetProof here expects the async method to invoke the callback
    // synchronously.
    source->GetProof(server_addr, client_addr, hostname, server_config, GetParam().transport_version, chlo_hash, std::move(cb));
    ASSERT_TRUE(called);
    ASSERT_TRUE(ok);
