...
class NET_EXPORT_PRIVATE QuicChromiumPacketWriter : public quic::QuicPacketWriter {
public:
    ...
    public:
        ...
        virtual int HandleWriteError(int error_code, scoped_refptr<ReusableIOBuffer> last_packet) = 0;
    ...
    QuicChromiumPacketWriter(DatagramClientSocket* socket, base::SequencedTaskRunner* task_runner);
    ...
    quic::WriteResult WritePacket(const char* buffer, size_t buf_len, const quic::QuicIpAddress& self_address, const quic::QuicSocketAddress& peer_address,
    ...
    quic::QuicPacketBuffer GetNextWriteLocation(const quic::QuicIpAddress& self_address, const quic::QuicSocketAddress& peer_address) override;
