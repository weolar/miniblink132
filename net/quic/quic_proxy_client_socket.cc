...
QuicProxyClientSocket::QuicProxyClientSocket(std::unique_ptr<QuicChromiumClientStream::Handle> stream,
    std::unique_ptr<QuicChromiumClientSession::Handle> session, const ProxyChain& proxy_chain, size_t proxy_chain_index, const std::string& user_agent,
    const HostPortPair& endpoint, const NetLogWithSource& net_log, scoped_refptr<HttpAuthController> auth_controller, ProxyDelegate* proxy_delegate)
    ...
    , net_log_(net_log)
{
    ...
    net_log_.AddEventReferencingSource(NetLogEventType::HTTP2_PROXY_CLIENT_SESSION, stream_->net_log().source());
}
...
const scoped_refptr<HttpAuthController>& QuicProxyClientSocket::GetAuthController() const
{
    return auth_;
}
...
int QuicProxyClientSocket::Read(IOBuffer* buf, int buf_len, CompletionOnceCallback callback)
{
    DCHECK(connect_callback_.is_null());
    ...
    int rv = stream_->ReadBody(buf, buf_len, base::BindOnce(&QuicProxyClientSocket::OnReadComplete, weak_factory_.GetWeakPtr()));

    if (rv == ERR_IO_PENDING) {
        read_callback_ = std::move(callback);
        read_buf_ = buf;
    } else if (rv == 0) {
        net_log_.AddByteTransferEvent(NetLogEventType::SOCKET_BYTES_RECEIVED, 0, nullptr);
    } else if (rv > 0) {
...
void QuicProxyClientSocket::OnReadComplete(int rv)
{
    ...
    if (!read_callback_.is_null()) {
        DCHECK(read_buf_);
        if (rv >= 0) {
            net_log_.AddByteTransferEvent(NetLogEventType::SOCKET_BYTES_RECEIVED, rv, read_buf_->data());
        }
        read_buf_ = nullptr;
...
int QuicProxyClientSocket::Write(IOBuffer* buf, int buf_len, CompletionOnceCallback callback, const NetworkTrafficAnnotationTag& traffic_annotation)
{
    DCHECK(connect_callback_.is_null());
    ...
    int rv = stream_->WriteStreamData(
        std::string_view(buf->data(), buf_len), false, base::BindOnce(&QuicProxyClientSocket::OnWriteComplete, weak_factory_.GetWeakPtr()));
    if (rv == OK)
        return buf_len;
...
{
    return IsConnected() ? session_->GetPeerAddress(address) : ERR_SOCKET_NOT_CONNECTED;
}

int QuicProxyClientSocket::GetLocalAddress(IPEndPoint* address) const
{
    return IsConnected() ? session_->GetSelfAddress(address) : ERR_SOCKET_NOT_CONNECTED;
}
...
int QuicProxyClientSocket::DoGenerateAuthToken()
{
    ...
    return auth_->MaybeGenerateAuthToken(&request_, base::BindOnce(&QuicProxyClientSocket::OnIOComplete, weak_factory_.GetWeakPtr()), net_log_);
}
...
int QuicProxyClientSocket::DoSendRequest()
{
    ...
    if (proxy_delegate_) {
        ...
        int result = proxy_delegate_->OnBeforeTunnelRequest(proxy_chain_, proxy_chain_index_, &proxy_delegate_headers);
        if (result < 0) {
            return result;
        }
    ...
    BuildTunnelRequest(endpoint_, authorization_headers, user_agent_, &request_line, &request_.extra_headers);
    ...
    CreateSpdyHeadersFromHttpRequest(request_, std::nullopt, request_.extra_headers, &headers);
...
int QuicProxyClientSocket::DoReadReply()
{
    ...
    int rv = stream_->ReadInitialHeaders(
        &response_header_block_, base::BindOnce(&QuicProxyClientSocket::OnReadResponseHeadersComplete, weak_factory_.GetWeakPtr()));
    if (rv == ERR_IO_PENDING)
...
int QuicProxyClientSocket::DoReadReplyComplete(int result)
{
    ...
    if (proxy_delegate_) {
        int rv = proxy_delegate_->OnTunnelHeadersReceived(proxy_chain_, proxy_chain_index_, *response_.headers);
        if (rv != OK) {
...
int QuicProxyClientSocket::ProcessResponseHeaders(const quiche::HttpHeaderBlock& headers)
{
