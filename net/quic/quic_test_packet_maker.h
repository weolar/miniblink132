...
public:
    ...
    QuicTestPacketMaker(quic::ParsedQuicVersion version, quic::QuicConnectionId connection_id, const quic::QuicClock* clock, const std::string& host,
        quic::Perspective perspective, bool client_priority_uses_incremental = false, bool use_priority_header = false);
    ~QuicTestPacketMaker();
    ...
    std::unique_ptr<quic::QuicReceivedPacket> MakeDummyCHLOPacket(uint64_t packet_number);
    ...
    std::unique_ptr<quic::QuicReceivedPacket> MakeAckAndRetransmissionPacket(uint64_t packet_number, uint64_t first_received, uint64_t largest_received,
        uint64_t smallest_received, const std::vector<uint64_t>& original_packet_numbers);

    std::unique_ptr<quic::QuicReceivedPacket> MakeRequestHeadersAndMultipleDataFramesPacket(uint64_t packet_number, quic::QuicStreamId stream_id, bool fin,
        spdy::SpdyPriority spdy_priority, quiche::HttpHeaderBlock headers, size_t* spdy_headers_frame_length, const std::vector<std::string>& data_writes);

    // If |spdy_headers_frame_length| is non-null, it will be set to the size of
    // the SPDY headers frame created for this packet.
    std::unique_ptr<quic::QuicReceivedPacket> MakeRequestHeadersPacket(uint64_t packet_number, quic::QuicStreamId stream_id, bool fin,
        spdy::SpdyPriority spdy_priority, quiche::HttpHeaderBlock headers, size_t* spdy_headers_frame_length, bool should_include_priority_frame = true);

    std::unique_ptr<quic::QuicReceivedPacket> MakeRetransmissionAndRequestHeadersPacket(const std::vector<uint64_t>& original_packet_numbers,
    ...
    std::unique_ptr<quic::QuicReceivedPacket> MakeRequestHeadersAndRstPacket(uint64_t packet_number, quic::QuicStreamId stream_id, bool fin,
    ...
    std::unique_ptr<quic::QuicReceivedPacket> MakeInitialSettingsPacket(uint64_t packet_number);

    std::unique_ptr<quic::QuicReceivedPacket> MakePriorityPacket(uint64_t packet_number, quic::QuicStreamId id, spdy::SpdyPriority spdy_priority);

    std::unique_ptr<quic::QuicReceivedPacket> MakeRetransmissionPacket(uint64_t original_packet_number, uint64_t new_packet_number);
    ...
    quiche::HttpHeaderBlock GetRequestHeaders(const std::string& method, const std::string& scheme, const std::string& path) const;

    quiche::HttpHeaderBlock ConnectRequestHeaders(const std::string& host_port) const;

    quiche::HttpHeaderBlock GetResponseHeaders(const std::string& status) const;

    quiche::HttpHeaderBlock GetResponseHeaders(const std::string& status, const std::string& alt_svc) const;
    ...
    quic::QuicLongHeaderType long_header_type()
    {
        return long_header_type_;
    }
    ...
    std::string QpackEncodeHeaders(quic::QuicStreamId stream_id, quiche::HttpHeaderBlock headers, size_t* encoded_data_length);

    void set_ecn_codepoint(quic::QuicEcnCodepoint ecn)
    {
        ecn_codepoint_ = ecn;
    }
...
private:
    void AddPriorityHeader(spdy::SpdyPriority spdy_priority, quiche::HttpHeaderBlock* headers);

    quic::QuicStreamId GetFirstBidirectionalStreamId() const;

    std::string GenerateHttp3PriorityData(spdy::SpdyPriority spdy_priority, quic::QuicStreamId stream_id);
...
class QuicTestPacketBuilder {
public:
    QuicTestPacketBuilder(uint64_t packet_number, QuicTestPacketMaker* maker, QuicTestPacketMaker::ConnectionState* connection_state);
    ...
    QuicTestPacketBuilder& AddNewConnectionIdFrame(const quic::QuicConnectionId& cid, uint64_t sequence_number, uint64_t retire_prior_to);
    QuicTestPacketBuilder& AddMaxStreamsFrame(quic::QuicControlFrameId control_frame_id, quic::QuicStreamCount stream_count, bool unidirectional);
    QuicTestPacketBuilder& AddStreamsBlockedFrame(quic::QuicControlFrameId control_frame_id, quic::QuicStreamCount stream_count, bool unidirectional);
    ...
    // after this call.
    QuicTestPacketBuilder& AddStreamFrame(quic::QuicStreamId stream_id, bool fin, std::string_view data);
    QuicTestPacketBuilder& AddAckFrame(
        uint64_t first_received, uint64_t largest_received, uint64_t smallest_received, std::optional<quic::QuicEcnCounts> ecn = std::nullopt);
    ...
    QuicTestPacketBuilder& AddRstStreamFrame(quic::QuicStreamId stream_id, quic::QuicRstStreamErrorCode error_code);
    QuicTestPacketBuilder& AddConnectionCloseFrame(quic::QuicErrorCode quic_error, const std::string& quic_error_details, uint64_t frame_type = 0);
    QuicTestPacketBuilder& AddGoAwayFrame(quic::QuicErrorCode error_code, std::string reason_phrase);
    QuicTestPacketBuilder& AddPathResponseFrame();
    QuicTestPacketBuilder& AddPathChallengeFrame();
    QuicTestPacketBuilder& AddStopSendingFrame(quic::QuicStreamId stream_id, quic::QuicRstStreamErrorCode error_code);
    QuicTestPacketBuilder& AddCryptoFrame(quic::EncryptionLevel level, quic::QuicStreamOffset offset, const quic::QuicData& data);
    ...
    QuicTestPacketBuilder& AddPacketRetransmission(
        uint64_t packet_number, base::RepeatingCallback<bool(const quic::QuicFrame&)> filter = base::RepeatingCallback<bool(const quic::QuicFrame&)>());
