...
public:
    ...
    MockQuicClientSessionBase& operator=(const MockQuicClientSessionBase&) = delete;
    ...
    MOCK_METHOD2(OnConnectionClosed, void(const quic::QuicConnectionCloseFrame& frame, quic::ConnectionCloseSource source));
    ...
    MOCK_METHOD6(WritevData,
        quic::QuicConsumedData(quic::QuicStreamId id, size_t write_length, quic::QuicStreamOffset offset, quic::StreamSendingState state,
            quic::TransmissionType type, quic::EncryptionLevel level));
    MOCK_METHOD2(WriteControlFrame, bool(const quic::QuicFrame&, quic::TransmissionType));
    MOCK_METHOD4(
        SendRstStream, void(quic::QuicStreamId stream_id, quic::QuicRstStreamErrorCode error, quic::QuicStreamOffset bytes_written, bool send_rst_only));

    MOCK_METHOD2(OnStreamHeaders, void(quic::QuicStreamId stream_id, std::string_view headers_data));
    MOCK_METHOD2(OnStreamHeadersPriority, void(quic::QuicStreamId stream_id, const spdy::SpdyStreamPrecedence& precedence));
    MOCK_METHOD3(OnStreamHeadersComplete, void(quic::QuicStreamId stream_id, bool fin, size_t frame_len));
    ...
    {
        return WriteHeadersOnHeadersStreamMock(id, headers, fin, precedence, std::move(ack_listener));
    }
    MOCK_METHOD5(WriteHeadersOnHeadersStreamMock,
        size_t(quic::QuicStreamId id, const quiche::HttpHeaderBlock& headers, bool fin, const spdy::SpdyStreamPrecedence& precedence,
            const quiche::QuicheReferenceCountedPointer<quic::QuicAckListenerInterface>& ack_listener));
    ...
    static quic::QuicConsumedData ConsumeAllData(
        quic::QuicStreamId id, size_t write_length, quic::QuicStreamOffset offset, bool fin, quic::QuicAckListenerInterface* ack_listener);
...
MockQuicClientSessionBase::MockQuicClientSessionBase(quic::QuicConnection* connection)
    : quic::QuicSpdyClientSessionBase(connection,
        /*visitor=*/nullptr, quic::test::DefaultQuicConfig(), connection->supported_versions())
{
    crypto_stream_ = std::make_unique<quic::test::MockQuicCryptoStream>(this);
    Initialize();
    ON_CALL(*this, WritevData(_, _, _, _, _, _)).WillByDefault(testing::Return(quic::QuicConsumedData(0, false)));
}

MockQuicClientSessionBase::~MockQuicClientSessionBase() = default;

class QuicChromiumClientStreamTest : public ::testing::TestWithParam<quic::ParsedQuicVersion>, public WithTaskEnvironment {
public:
    QuicChromiumClientStreamTest()
        : version_(GetParam())
        , crypto_config_(quic::test::crypto_test_utils::ProofVerifierForTesting())
        , session_(new quic::test::MockQuicConnection(&helper_, &alarm_factory_, quic::Perspective::IS_CLIENT, quic::test::SupportedVersions(version_)))
    {
        quic::test::QuicConfigPeer::SetReceivedInitialSessionFlowControlWindow(session_.config(), quic::kMinimumFlowControlSendWindow);
        quic::test::QuicConfigPeer::SetReceivedInitialMaxStreamDataBytesOutgoingBidirectional(session_.config(), quic::kMinimumFlowControlSendWindow);
        quic::test::QuicConfigPeer::SetReceivedMaxUnidirectionalStreams(session_.config(), 10);
        session_.OnConfigNegotiated();
        stream_ = new QuicChromiumClientStream(quic::test::GetNthClientInitiatedBidirectionalStreamId(version_.transport_version, 0), &session_,
            quic::QuicServerId(), quic::BIDIRECTIONAL, NetLogWithSource(), TRAFFIC_ANNOTATION_FOR_TESTS);
        ...
        session_.connection()->SetEncrypter(quic::ENCRYPTION_FORWARD_SECURE, std::make_unique<quic::test::TaggingEncrypter>(quic::ENCRYPTION_FORWARD_SECURE));
    }
    ...
    quiche::HttpHeaderBlock CreateResponseHeaders(const std::string& status_code)
    {
    ...
    {
        auto buffer = base::MakeRefCounted<IOBufferWithSize>(expected_data.length() + 1);
        EXPECT_EQ(static_cast<int>(expected_data.length()), stream_->Read(buffer.get(), expected_data.length() + 1));
        EXPECT_EQ(expected_data, std::string_view(buffer->data(), expected_data.length()));
    }
    ...
    quic::QuicHeaderList ProcessHeadersFull(const quiche::HttpHeaderBlock& headers)
    {
        quic::QuicHeaderList h = ProcessHeaders(headers);
        TestCompletionCallback callback;
        EXPECT_EQ(static_cast<int>(h.uncompressed_header_bytes()), handle_->ReadInitialHeaders(&headers_, callback.callback()));
    ...
    {
        return quic::test::GetNthClientInitiatedBidirectionalStreamId(session_.connection()->transport_version(), n);
    }

    quic::QuicStreamId GetNthServerInitiatedUnidirectionalStreamId(int n)
    {
        return quic::test::GetNthServerInitiatedUnidirectionalStreamId(session_.connection()->transport_version(), n);
    }
...
INSTANTIATE_TEST_SUITE_P(Version, QuicChromiumClientStreamTest, ::testing::ValuesIn(AllSupportedQuicVersions()), ::testing::PrintToStringParamName());

TEST_P(QuicChromiumClientStreamTest, Handle)
{
    testing::InSequence seq;
    EXPECT_TRUE(handle_->IsOpen());
    EXPECT_EQ(quic::test::GetNthClientInitiatedBidirectionalStreamId(version_.transport_version, 0), handle_->id());
    ...
    quic::QuicStreamFrame frame2(quic::test::GetNthClientInitiatedBidirectionalStreamId(version_.transport_version, 0), true, offset, std::string_view());
    ...
    EXPECT_CALL(session_, WritevData(stream_->id(), _, _, _, quic::NOT_RETRANSMISSION, _)).WillOnce(Return(quic::QuicConsumedData(header.length(), false)));
    EXPECT_CALL(session_, WritevData(stream_->id(), _, _, _, quic::NOT_RETRANSMISSION, _)).WillOnce(Return(quic::QuicConsumedData(kDataLen, true)));
    TestCompletionCallback callback;
    EXPECT_EQ(OK, handle_->WriteStreamData(std::string_view(kData1, kDataLen), true, callback.callback()));
    ...
    EXPECT_EQ(ERR_CONNECTION_CLOSED, handle_->WriteStreamData(std::string_view(kData1, kDataLen), true, callback.callback()));

    std::vector<scoped_refptr<IOBuffer>> buffers = { base::MakeRefCounted<IOBufferWithSize>(10) };
    std::vector<int> lengths = { 10 };
    EXPECT_EQ(ERR_CONNECTION_CLOSED, handle_->WritevStreamData(buffers, lengths, true, callback.callback()));
...
{
    quic::test::QuicConnectionPeer::TearDownLocalConnectionState(session_.connection());
...
TEST_P(QuicChromiumClientStreamTest, HandleAfterStreamReset)
{
    ...
    quic::QuicStopSendingFrame stop_sending_frame(
        quic::kInvalidControlFrameId, quic::test::GetNthClientInitiatedBidirectionalStreamId(version_.transport_version, 0), quic::QUIC_STREAM_CANCELLED);
    ...
    quic::QuicRstStreamFrame rst(
        quic::kInvalidControlFrameId, quic::test::GetNthClientInitiatedBidirectionalStreamId(version_.transport_version, 0), quic::QUIC_STREAM_CANCELLED, 0);
...
TEST_P(QuicChromiumClientStreamTest, OnDataAvailable)
{
    ...
    stream_->OnStreamFrame(quic::QuicStreamFrame(quic::test::GetNthClientInitiatedBidirectionalStreamId(version_.transport_version, 0),
        /*fin=*/false,
    ...
    EXPECT_EQ(data_len, handle_->ReadBody(buffer.get(), 2 * data_len, callback.callback()));
...
TEST_P(QuicChromiumClientStreamTest, OnDataAvailableAfterReadBody)
{
    ...
    TestCompletionCallback callback;
    auto buffer = base::MakeRefCounted<IOBufferWithSize>(2 * data_len);
    EXPECT_EQ(ERR_IO_PENDING, handle_->ReadBody(buffer.get(), 2 * data_len, callback.callback()));
...
TEST_P(QuicChromiumClientStreamTest, ProcessHeadersWithError)
{
    ...
    EXPECT_CALL(*static_cast<quic::test::MockQuicConnection*>(session_.connection()),
        OnStreamReset(quic::test::GetNthClientInitiatedBidirectionalStreamId(version_.transport_version, 0), quic::QUIC_BAD_APPLICATION_PAYLOAD));
...
TEST_P(QuicChromiumClientStreamTest, OnDataAvailableWithError)
{
    ...
    EXPECT_EQ(ERR_IO_PENDING,
        handle_->ReadBody(buffer.get(), 2 * data_len, base::BindOnce(&QuicChromiumClientStreamTest::ResetStreamCallback, base::Unretained(this), stream_)));

    EXPECT_CALL(*static_cast<quic::test::MockQuicConnection*>(session_.connection()),
        OnStreamReset(quic::test::GetNthClientInitiatedBidirectionalStreamId(version_.transport_version, 0), quic::QUIC_STREAM_CANCELLED));
...
TEST_P(QuicChromiumClientStreamTest, OnTrailers)
{
    ...
    EXPECT_EQ(static_cast<int>(t.uncompressed_header_bytes()), handle_->ReadTrailingHeaders(&trailers_, trailers_callback.callback()));

    // Read the body and verify that it arrives correctly.
    EXPECT_EQ(0, handle_->ReadBody(buffer.get(), 2 * data_len, callback.callback()));
...
TEST_P(QuicChromiumClientStreamTest, MarkTrailersConsumedWhenNotifyDelegate)
{
    ...
    EXPECT_EQ(static_cast<int>(t.uncompressed_header_bytes()), handle_->ReadTrailingHeaders(&trailers_, callback.callback()));
...
TEST_P(QuicChromiumClientStreamTest, ReadAfterTrailersReceivedButNotDelivered)
{
    ...
    EXPECT_EQ(static_cast<int>(t.uncompressed_header_bytes()), handle_->ReadTrailingHeaders(&trailers_, callback2.callback()));
...
TEST_P(QuicChromiumClientStreamTest, WriteStreamDataAsync)
{
    ...
    // No data written.
    EXPECT_CALL(session_, WritevData(stream_->id(), _, _, _, quic::NOT_RETRANSMISSION, _)).WillOnce(Return(quic::QuicConsumedData(0, false)));
    TestCompletionCallback callback;
    EXPECT_EQ(ERR_IO_PENDING, handle_->WriteStreamData(std::string_view(kData1, kDataLen), true, callback.callback()));
...
{
    testing::InSequence seq;
    scoped_refptr<StringIOBuffer> buf1 = base::MakeRefCounted<StringIOBuffer>("hello world!");
    scoped_refptr<StringIOBuffer> buf2 = base::MakeRefCounted<StringIOBuffer>("Just a small payload");

    // All data written.
    std::string header = ConstructDataHeader(buf1->size());
    EXPECT_CALL(session_, WritevData(stream_->id(), _, _, _, quic::NOT_RETRANSMISSION, _)).WillOnce(Return(quic::QuicConsumedData(header.length(), false)));
    EXPECT_CALL(session_, WritevData(stream_->id(), _, _, _, quic::NOT_RETRANSMISSION, _)).WillOnce(Return(quic::QuicConsumedData(buf1->size(), false)));
    header = ConstructDataHeader(buf2->size());
    EXPECT_CALL(session_, WritevData(stream_->id(), _, _, _, quic::NOT_RETRANSMISSION, _)).WillOnce(Return(quic::QuicConsumedData(header.length(), false)));
    EXPECT_CALL(session_, WritevData(stream_->id(), _, _, _, quic::NOT_RETRANSMISSION, _)).WillOnce(Return(quic::QuicConsumedData(buf2->size(), true)));
    TestCompletionCallback callback;
    EXPECT_EQ(OK, handle_->WritevStreamData({ buf1, buf2 }, { buf1->size(), buf2->size() }, true, callback.callback()));
}
...
TEST_P(QuicChromiumClientStreamTest, WritevStreamDataAsync)
{
    ...
    std::string header = ConstructDataHeader(buf1->size());
    EXPECT_CALL(session_, WritevData(stream_->id(), _, _, _, quic::NOT_RETRANSMISSION, _)).WillOnce(Return(quic::QuicConsumedData(header.length(), false)));
    EXPECT_CALL(session_, WritevData(stream_->id(), _, _, _, quic::NOT_RETRANSMISSION, _))
        // First piece of data is written.
        .WillOnce(Return(quic::QuicConsumedData(buf1->size(), false)));
    // Second piece of data is queued.
    EXPECT_CALL(session_, WritevData(stream_->id(), _, _, _, quic::NOT_RETRANSMISSION, _)).WillOnce(Return(quic::QuicConsumedData(0, false)));
    TestCompletionCallback callback;
    EXPECT_EQ(ERR_IO_PENDING, handle_->WritevStreamData({ buf1.get(), buf2.get() }, { buf1->size(), buf2->size() }, true, callback.callback()));
    ASSERT_FALSE(callback.have_result());
...
TEST_P(QuicChromiumClientStreamTest, WriteConnectUdpPayload)
{
    ...
    quic::test::QuicSpdySessionPeer::SetHttpDatagramSupport(&session_, quic::HttpDatagramSupport::kRfc);
    EXPECT_CALL(*static_cast<quic::test::MockQuicConnection*>(session_.connection()), SendMessage(1, _, false)).WillOnce(Return(quic::MESSAGE_STATUS_SUCCESS));
    EXPECT_EQ(OK, handle_->WriteConnectUdpPayload(packet));
    histogram_tester_.ExpectBucketCount(QuicChromiumClientStream::kHttp3DatagramDroppedHistogram, false, 1);

    // Packet is dropped if session does not have HTTP3 Datagram support.
    quic::test::QuicSpdySessionPeer::SetHttpDatagramSupport(&session_, quic::HttpDatagramSupport::kNone);
    EXPECT_EQ(OK, handle_->WriteConnectUdpPayload(packet));
    histogram_tester_.ExpectBucketCount(QuicChromiumClientStream::kHttp3DatagramDroppedHistogram, true, 1);
    histogram_tester_.ExpectTotalCount(QuicChromiumClientStream::kHttp3DatagramDroppedHistogram, 2);
}
...
TEST_P(QuicChromiumClientStreamTest, HeadersBeforeHandle)
{
    ...
    QuicChromiumClientStream* stream2
        = new QuicChromiumClientStream(stream_id, &session_, quic::QuicServerId(), quic::READ_UNIDIRECTIONAL, NetLogWithSource(), TRAFFIC_ANNOTATION_FOR_TESTS);
    ...
    stream2->OnStreamHeaderList(true, header_list.uncompressed_header_bytes(), header_list);

    // Now set the delegate and verify that the headers are delivered.
    handle2_ = stream2->CreateHandle();
    TestCompletionCallback callback;
    EXPECT_EQ(static_cast<int>(header_list.uncompressed_header_bytes()), handle2_->ReadInitialHeaders(&headers_, callback.callback()));
...
TEST_P(QuicChromiumClientStreamTest, HeadersAndDataBeforeHandle)
{
    ...
    quic::QuicHeaderList header_list = quic::test::AsHeaderList(headers_);
    stream2->OnStreamHeaderList(false, header_list.uncompressed_header_bytes(), header_list);
    const char data[] = "hello world!";
...
TEST_P(QuicChromiumClientStreamTest, ContinueResponse)
{
    ...
    EXPECT_EQ(static_cast<int>(header_list.uncompressed_header_bytes()), handle_->ReadInitialHeaders(&response_headers, base::DoNothing()));
    base::RunLoop().RunUntilIdle();
...
TEST_P(QuicChromiumClientStreamTest, EarlyHintsResponses)
{
    ...
    EXPECT_EQ(static_cast<int>(hints1_bytes), handle_->ReadInitialHeaders(&headers, base::DoNothing()));
    base::RunLoop().RunUntilIdle();
    ...
    EXPECT_EQ(static_cast<int>(hints2_bytes), handle_->ReadInitialHeaders(&headers, base::DoNothing()));
    base::RunLoop().RunUntilIdle();
    ...
    EXPECT_EQ(static_cast<int>(initial_headers_bytes), handle_->ReadInitialHeaders(&headers, base::DoNothing()));
    base::RunLoop().RunUntilIdle();
    EXPECT_EQ(headers, headers_);
}
...
TEST_P(QuicChromiumClientStreamTest, EarlyHintsAsync)
{
    ...
    EXPECT_EQ(ERR_IO_PENDING, handle_->ReadInitialHeaders(&headers, hints_callback.callback()));
...
TEST_P(QuicChromiumClientStreamTest, EarlyHintsAfterInitialHeaders)
{
    ...
    // like ":status".
    EXPECT_CALL(*static_cast<quic::test::MockQuicConnection*>(session_.connection()),
        CloseConnection(quic::QUIC_INVALID_HEADERS_STREAM_DATA, _, quic::ConnectionCloseBehavior::SEND_CONNECTION_CLOSE_PACKET));
...
TEST_P(QuicChromiumClientStreamTest, TrailersAfterEarlyHintsWithoutRead)
{
    ...
    // Read the Early Hints response from the handle.
    {
        quiche::HttpHeaderBlock headers;
        TestCompletionCallback callback;
        EXPECT_EQ(static_cast<int>(hints_header_list.uncompressed_header_bytes()), handle_->ReadInitialHeaders(&headers, callback.callback()));
        EXPECT_EQ(headers, hints_headers);
    }
    ...
    // Read the initial headers from the handle.
    {
        quiche::HttpHeaderBlock headers;
        TestCompletionCallback callback;
        EXPECT_EQ(static_cast<int>(header_list.uncompressed_header_bytes()), handle_->ReadInitialHeaders(&headers, callback.callback()));
        EXPECT_EQ(headers, headers_);
    }
    ...
    // Read trailers from the handle.
    {
        quiche::HttpHeaderBlock headers;
        TestCompletionCallback callback;
        EXPECT_EQ(static_cast<int>(trailer_header_list.uncompressed_header_bytes()), handle_->ReadTrailingHeaders(&headers, callback.callback()));
