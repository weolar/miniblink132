...
QuicContext::QuicContext()
    : QuicContext(std::make_unique<QuicChromiumConnectionHelper>(quic::QuicChromiumClock::GetInstance(), quic::QuicRandom::GetInstance()))
{
}

QuicContext::QuicContext(std::unique_ptr<quic::QuicConnectionHelperInterface> helper)
...
quic::ParsedQuicVersion QuicContext::SelectQuicVersion(const quic::ParsedQuicVersionVector& advertised_versions)
{
    const quic::ParsedQuicVersionVector& supported_versions = params()->supported_versions;
...
quic::QuicConfig InitializeQuicConfig(const QuicParams& params)
{
    ...
    config.SetIdleNetworkTimeout(quic::QuicTime::Delta::FromMicroseconds(params.idle_connection_timeout.InMicroseconds()));
    config.set_max_time_before_crypto_handshake(quic::QuicTime::Delta::FromMicroseconds(params.max_time_before_crypto_handshake.InMicroseconds()));
    config.set_max_idle_time_before_crypto_handshake(quic::QuicTime::Delta::FromMicroseconds(params.max_idle_time_before_crypto_handshake.InMicroseconds()));
    ...
    config.SetInitialSessionFlowControlWindowToSend(kQuicSessionMaxRecvWindowSize);
