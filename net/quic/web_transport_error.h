...
struct NET_EXPORT WebTransportError {
    ...
    explicit WebTransportError(int net_error)
        : net_error(net_error)
    {
        DCHECK_LT(net_error, 0);
    }
    WebTransportError(int net_error, quic::QuicErrorCode quic_error, std::string_view details, bool safe_to_report_details)
        : net_error(net_error)
