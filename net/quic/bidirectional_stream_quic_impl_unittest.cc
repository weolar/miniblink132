...
enum DelegateMethod { kOnStreamReady, kOnHeadersReceived, kOnTrailersReceived, kOnDataRead, kOnDataSent, kOnFailed };

class TestDelegateBase : public BidirectionalStreamImpl::Delegate {
public:
    TestDelegateBase(IOBuffer* read_buf, int read_buf_len)
        : TestDelegateBase(read_buf, read_buf_len, std::make_unique<base::OneShotTimer>())
    {
    }

    TestDelegateBase(IOBuffer* read_buf, int read_buf_len, std::unique_ptr<base::OneShotTimer> timer)
        : read_buf_(read_buf)
    ...
    void Start(const BidirectionalStreamRequestInfo* request_info, const NetLogWithSource& net_log, std::unique_ptr<QuicChromiumClientSession::Handle> session)
    {
        not_expect_callback_ = true;
        stream_ = std::make_unique<BidirectionalStreamQuicImpl>(std::move(session));
        stream_->Start(request_info, net_log, send_request_headers_automatically_, this, nullptr, TRAFFIC_ANNOTATION_FOR_TESTS);
    ...
    void SendData(const scoped_refptr<IOBuffer>& data, int length, bool end_of_stream)
    {
        SendvData({ data }, { length }, end_of_stream);
    }

    void SendvData(const std::vector<scoped_refptr<IOBuffer>>& data, const std::vector<int>& lengths, bool end_of_stream)
    {
        not_expect_callback_ = true;
    ...
    void WaitUntilNextCallback(DelegateMethod method)
    {
        ...
        EXPECT_EQ(headers_received || (method == kOnHeadersReceived), !response_headers_.empty());
        EXPECT_EQ(trailers_received || (method == kOnTrailersReceived), trailers_received_);
        EXPECT_EQ(on_data_read_count + (method == kOnDataRead ? 1 : 0), on_data_read_count_);
        EXPECT_EQ(on_data_sent_count + (method == kOnDataSent ? 1 : 0), on_data_sent_count_);
    }
    ...
    const std::string& data_received() const
    {
    ...
    const quiche::HttpHeaderBlock& trailers() const
    {
        return trailers_;
    }
    int on_data_read_count() const
    {
        return on_data_read_count_;
    }
    int on_data_sent_count() const
    {
        return on_data_sent_count_;
    }
    bool on_failed_called() const
    {
        return on_failed_called_;
    }
    bool is_ready() const
    {
        return is_ready_;
...
class BidirectionalStreamQuicImplTest : public ::testing::TestWithParam<quic::ParsedQuicVersion>, public WithTaskEnvironment {
protected:
    ...
    BidirectionalStreamQuicImplTest()
        ...
        , server_maker_(version_, connection_id_, &clock_, kDefaultServerHostName, quic::Perspective::IS_SERVER, false)
        , printer_(version_)
        , destination_(url::kHttpsScheme, kDefaultServerHostName, kDefaultServerPort)
    {
    ...
    void Initialize()
    {
        ...
        connection_ = new quic::QuicConnection(connection_id_, quic::QuicSocketAddress(), ToQuicSocketAddress(peer_addr_), helper_.get(), alarm_factory_.get(),
            new QuicChromiumPacketWriter(socket.get(), runner_.get()), true /* owns_writer */, quic::Perspective::IS_CLIENT,
            quic::test::SupportedVersions(version_), connection_id_generator_);
        if (connection_->version().KnowsWhichDecrypterToUse()) {
            connection_->InstallDecrypter(
                quic::ENCRYPTION_FORWARD_SECURE, std::make_unique<quic::test::StrictTaggingDecrypter>(quic::ENCRYPTION_FORWARD_SECURE));
        }
        ...
        session_ = std::make_unique<QuicChromiumClientSession>(connection_, std::move(socket),
            ...
            /*server_preferred_address=*/true, MultiplexedSessionCreationInitiator::kUnknown, NetLogWithSource::Make(NetLogSourceType::NONE));
        session_->Initialize();
    ...
    void SetRequest(const std::string& method, const std::string& path, RequestPriority priority)
    {
        request_headers_ = client_maker_.GetRequestHeaders(method, "http", path);
    }

    quiche::HttpHeaderBlock ConstructResponseHeaders(const std::string& response_code)
    {
    ...
    {
        std::unique_ptr<quic::QuicReceivedPacket> packet(server_maker_.Packet(packet_number).AddStreamFrame(stream_id_, fin, data).Build());
        DVLOG(2) << "packet(" << packet_number << "): " << std::endl << quiche::QuicheTextUtils::HexDump(packet->AsStringPiece());
        return packet;
    }

    std::unique_ptr<quic::QuicReceivedPacket> ConstructClientDataPacket(bool fin, std::string_view data)
    {
        return client_maker_.Packet(++packet_number_).AddStreamFrame(stream_id_, fin, data).Build();
    }

    std::unique_ptr<quic::QuicReceivedPacket> ConstructRequestHeadersPacket(bool fin, RequestPriority request_priority, size_t* spdy_headers_frame_length)
    {
        return ConstructRequestHeadersPacketInner(stream_id_, fin, request_priority, spdy_headers_frame_length);
    }

    std::unique_ptr<quic::QuicReceivedPacket> ConstructRequestHeadersPacketInner(
        quic::QuicStreamId stream_id, bool fin, RequestPriority request_priority, size_t* spdy_headers_frame_length)
    {
        spdy::SpdyPriority priority = ConvertRequestPriorityToQuicPriority(request_priority);
        std::unique_ptr<quic::QuicReceivedPacket> packet(
            client_maker_.MakeRequestHeadersPacket(++packet_number_, stream_id, fin, priority, std::move(request_headers_), spdy_headers_frame_length));
        DVLOG(2) << "packet(" << packet_number_ << "): " << std::endl << quiche::QuicheTextUtils::HexDump(packet->AsStringPiece());
        return packet;
    }

    std::unique_ptr<quic::QuicReceivedPacket> ConstructRequestHeadersAndMultipleDataFramesPacket(
        bool fin, RequestPriority request_priority, size_t* spdy_headers_frame_length, const std::vector<std::string>& data)
    {
        spdy::SpdyPriority priority = ConvertRequestPriorityToQuicPriority(request_priority);
        std::unique_ptr<quic::QuicReceivedPacket> packet(client_maker_.MakeRequestHeadersAndMultipleDataFramesPacket(
            ++packet_number_, stream_id_, fin, priority, std::move(request_headers_), spdy_headers_frame_length, data));
        DVLOG(2) << "packet(" << packet_number_ << "): " << std::endl << quiche::QuicheTextUtils::HexDump(packet->AsStringPiece());
        return packet;
    }

    std::unique_ptr<quic::QuicReceivedPacket> ConstructResponseHeadersPacket(
        uint64_t packet_number, bool fin, quiche::HttpHeaderBlock response_headers, size_t* spdy_headers_frame_length)
    {
        return ConstructResponseHeadersPacketInner(packet_number, stream_id_, fin, std::move(response_headers), spdy_headers_frame_length);
    }

    std::unique_ptr<quic::QuicReceivedPacket> ConstructResponseHeadersPacketInner(
        uint64_t packet_number, quic::QuicStreamId stream_id, bool fin, quiche::HttpHeaderBlock response_headers, size_t* spdy_headers_frame_length)
    {
        return server_maker_.MakeResponseHeadersPacket(packet_number, stream_id, fin, std::move(response_headers), spdy_headers_frame_length);
    }
    ...
    std::unique_ptr<quic::QuicReceivedPacket> ConstructServerRstStreamPacket(uint64_t packet_number)
    {
    ...
    std::unique_ptr<quic::QuicReceivedPacket> ConstructRstStreamCancelledPacket(uint64_t packet_number, QuicTestPacketMaker* maker)
    {
        std::unique_ptr<quic::QuicReceivedPacket> packet(client_maker_.Packet(packet_number)
    ...
    std::unique_ptr<quic::QuicReceivedPacket> ConstructClientAckAndRstStreamPacket(uint64_t largest_received, uint64_t smallest_received)
    {
    ...
    std::unique_ptr<quic::QuicReceivedPacket> ConstructAckAndDataPacket(
        uint64_t packet_number, uint64_t largest_received, uint64_t smallest_received, bool fin, std::string_view data, QuicTestPacketMaker* maker)
    {
        std::unique_ptr<quic::QuicReceivedPacket> packet(
            maker->Packet(packet_number).AddAckFrame(/*first_received=*/1, largest_received, smallest_received).AddStreamFrame(stream_id_, fin, data).Build());
        DVLOG(2) << "packet(" << packet_number << "): " << std::endl << quiche::QuicheTextUtils::HexDump(packet->AsStringPiece());
        return packet;
    }

    std::unique_ptr<quic::QuicReceivedPacket> ConstructClientAckPacket(uint64_t largest_received, uint64_t smallest_received)
    {
        return client_maker_.Packet(++packet_number_).AddAckFrame(1, largest_received, smallest_received).Build();
    }
...
INSTANTIATE_TEST_SUITE_P(Version, BidirectionalStreamQuicImplTest, ::testing::ValuesIn(AllSupportedQuicVersions()), ::testing::PrintToStringParamName());
...
TEST_P(BidirectionalStreamQuicImplTest, GetRequest)
{
    ...
    AddWrite(ConstructRequestHeadersPacketInner(GetNthClientInitiatedBidirectionalStreamId(0), kFin, DEFAULT_PRIORITY, &spdy_request_headers_frame_length));
    ...
    auto delegate = std::make_unique<TestDelegateBase>(read_buffer.get(), kReadBufferSize);
    delegate->set_trailers_expected(true);
    delegate->Start(&request, net_log_with_source(), session()->CreateHandle(destination_));
    ...
    ProcessPacket(ConstructResponseHeadersPacket(2, !kFin, std::move(response_headers), &spdy_response_headers_frame_length));
    ...
    ProcessPacket(ConstructResponseTrailersPacket(4, kFin, trailers.Clone(), &spdy_trailers_frame_length));
    ...
    EXPECT_EQ(static_cast<int64_t>(spdy_request_headers_frame_length), delegate->GetTotalSentBytes());
    EXPECT_EQ(static_cast<int64_t>(spdy_response_headers_frame_length + strlen(kResponseBody) + header.length() + spdy_trailers_frame_length),
...
TEST_P(BidirectionalStreamQuicImplTest, LoadTimingTwoRequests)
{
    ...
    client_maker_.SetEncryptionLevel(quic::ENCRYPTION_ZERO_RTT);
    AddWrite(ConstructInitialSettingsPacket());
    AddWrite(ConstructRequestHeadersPacketInner(GetNthClientInitiatedBidirectionalStreamId(0), kFin, DEFAULT_PRIORITY, nullptr));
    // SetRequest() again for second request as |request_headers_| was moved.
    SetRequest("GET", "/", DEFAULT_PRIORITY);
    AddWrite(ConstructRequestHeadersPacketInner(GetNthClientInitiatedBidirectionalStreamId(1), kFin, DEFAULT_PRIORITY, nullptr));
    client_maker_.SetEncryptionLevel(quic::ENCRYPTION_FORWARD_SECURE);
    AddWrite(ConstructClientAckPacket(3, 1));
    Initialize();
    ...
    auto delegate2 = std::make_unique<TestDelegateBase>(read_buffer2.get(), kReadBufferSize);
    delegate2->Start(&request, net_log_with_source(), session()->CreateHandle(destination_));
    ...
    // Server sends the response headers.
    ProcessPacket(ConstructResponseHeadersPacketInner(2, GetNthClientInitiatedBidirectionalStreamId(0), kFin, ConstructResponseHeaders("200"), nullptr));

    ProcessPacket(ConstructResponseHeadersPacketInner(3, GetNthClientInitiatedBidirectionalStreamId(1), kFin, ConstructResponseHeaders("200"), nullptr));
...
TEST_P(BidirectionalStreamQuicImplTest, CoalesceDataBuffersNotHeadersFrame)
{
    ...
    AddWrite(ConstructRequestHeadersPacketInner(GetNthClientInitiatedBidirectionalStreamId(0), !kFin, DEFAULT_PRIORITY, &spdy_request_headers_frame_length));
    AddWrite(ConstructClientDataPacket(!kFin, header + kBody1 + header2 + kBody2));
    ...
    AddWrite(ConstructClientDataPacket(kFin, header3 + kBody3 + header4 + kBody4 + header5 + kBody5));
    ...
    // Send a Data packet.
    scoped_refptr<StringIOBuffer> buf1 = base::MakeRefCounted<StringIOBuffer>(kBody1);
    scoped_refptr<StringIOBuffer> buf2 = base::MakeRefCounted<StringIOBuffer>(kBody2);
    ...
    scoped_refptr<StringIOBuffer> buf3 = base::MakeRefCounted<StringIOBuffer>(kBody3);
    scoped_refptr<StringIOBuffer> buf4 = base::MakeRefCounted<StringIOBuffer>(kBody4);
    scoped_refptr<StringIOBuffer> buf5 = base::MakeRefCounted<StringIOBuffer>(kBody5);

    delegate->SendvData({ buf3, buf4, buf5 }, { buf3->size(), buf4->size(), buf5->size() }, kFin);
    delegate->WaitUntilNextCallback(kOnDataSent);
    ...
    EXPECT_EQ(kProtoQUIC, delegate->GetProtocol());
    EXPECT_EQ(static_cast<int64_t>(spdy_request_headers_frame_length + kBody1.length() + kBody2.length() + kBody3.length() + kBody4.length() + kBody5.length()
                  + header.length() + header2.length() + header3.length() + header4.length() + header5.length()),
        delegate->GetTotalSentBytes());
    EXPECT_EQ(static_cast<int64_t>(spdy_response_headers_frame_length + strlen(kResponseBody) + header6.length() + spdy_trailers_frame_length),
        delegate->GetTotalReceivedBytes());
}
...
TEST_P(BidirectionalStreamQuicImplTest, SendDataCoalesceDataBufferAndHeaderFrame)
{
    ...
    AddWrite(ConstructRequestHeadersAndMultipleDataFramesPacket(!kFin, DEFAULT_PRIORITY, &spdy_request_headers_frame_length, { header, kBody1 }));
    ...
    EXPECT_EQ(2, delegate->on_data_sent_count());
    EXPECT_EQ(kProtoQUIC, delegate->GetProtocol());
    EXPECT_EQ(static_cast<int64_t>(spdy_request_headers_frame_length + strlen(kBody1) + strlen(kBody2) + header.length() + header2.length()),
        delegate->GetTotalSentBytes());
    EXPECT_EQ(static_cast<int64_t>(spdy_response_headers_frame_length + strlen(kResponseBody) + header3.length() + spdy_trailers_frame_length),
        delegate->GetTotalReceivedBytes());
}

// Tests that when request headers are delayed, SendvData triggers coalescing of
// request headers with data buffers.
TEST_P(BidirectionalStreamQuicImplTest, SendvDataCoalesceDataBuffersAndHeaderFrame)
{
    SetRequest("POST", "/", DEFAULT_PRIORITY);
    size_t spdy_request_headers_frame_length;
    client_maker_.SetEncryptionLevel(quic::ENCRYPTION_ZERO_RTT);
    ...
    AddWrite(ConstructRequestHeadersAndMultipleDataFramesPacket(
        !kFin, DEFAULT_PRIORITY, &spdy_request_headers_frame_length, { header + kBody1 + header2 + kBody2 }));
...
TEST_P(BidirectionalStreamQuicImplTest, SendDataWriteErrorCoalesceDataBufferAndHeaderFrame)
{
    client_maker_.SetEncryptionLevel(quic::ENCRYPTION_ZERO_RTT);
    AddWrite(ConstructInitialSettingsPacket());
    ...
    auto read_buffer = base::MakeRefCounted<IOBufferWithSize>(kReadBufferSize);
    auto delegate = std::make_unique<DeleteStreamDelegate>(read_buffer.get(), kReadBufferSize, DeleteStreamDelegate::ON_FAILED);
    delegate->DoNotSendRequestHeadersAutomatically();
    delegate->Start(&request, net_log_with_source(), session()->CreateHandle(destination_));
    ConfirmHandshake();
    delegate->WaitUntilNextCallback(kOnStreamReady);
...
// headers to be sent, if that write fails the stream does not crash.
TEST_P(BidirectionalStreamQuicImplTest, SendvDataWriteErrorCoalesceDataBufferAndHeaderFrame)
{
    client_maker_.SetEncryptionLevel(quic::ENCRYPTION_ZERO_RTT);
    AddWrite(ConstructInitialSettingsPacket());
    AddWriteError(SYNCHRONOUS, ERR_CONNECTION_REFUSED);
...
TEST_P(BidirectionalStreamQuicImplTest, PostRequest)
{
    ...
    // Send a DATA frame.
    scoped_refptr<StringIOBuffer> buf = base::MakeRefCounted<StringIOBuffer>(kUploadData);
    ...
    EXPECT_EQ(kProtoQUIC, delegate->GetProtocol());
    EXPECT_EQ(static_cast<int64_t>(spdy_request_headers_frame_length + strlen(kUploadData) + header.length()), delegate->GetTotalSentBytes());
    EXPECT_EQ(static_cast<int64_t>(spdy_response_headers_frame_length + strlen(kResponseBody) + header2.length() + spdy_trailers_frame_length),
        delegate->GetTotalReceivedBytes());
}
...
TEST_P(BidirectionalStreamQuicImplTest, InterleaveReadDataAndSendData)
{
    ...
    std::string header = ConstructDataHeader(strlen(kUploadData));
    AddWrite(ConstructAckAndDataPacket(++packet_number_, 2, 1, !kFin, header + std::string(kUploadData), &client_maker_));
    AddWrite(ConstructAckAndDataPacket(++packet_number_, 3, 3, kFin, header + std::string(kUploadData), &client_maker_));
    Initialize();
    ...
    int server_ack = 2;
    ProcessPacket(ConstructAckAndDataPacket(3, server_ack++, 1, !kFin, header2 + kResponseBody, &server_maker_));
    ...
    EXPECT_EQ(2, delegate->on_data_sent_count());
    EXPECT_EQ(kProtoQUIC, delegate->GetProtocol());
    EXPECT_EQ(static_cast<int64_t>(spdy_request_headers_frame_length + 2 * strlen(kUploadData) + 2 * header.length()), delegate->GetTotalSentBytes());
    EXPECT_EQ(static_cast<int64_t>(spdy_response_headers_frame_length + 2 * strlen(kResponseBody) + 2 * header2.length()), delegate->GetTotalReceivedBytes());
}
...
TEST_P(BidirectionalStreamQuicImplTest, ServerSendsRstAfterHeaders)
{
    ...
    // IETF QUIC.
    ProcessPacket(server_maker_.Packet(1).AddRstStreamFrame(GetNthClientInitiatedBidirectionalStreamId(0), quic::QUIC_STREAM_CANCELLED).Build());

    delegate->WaitUntilNextCallback(kOnFailed);

    TestCompletionCallback cb;
    EXPECT_THAT(delegate->ReadData(cb.callback()), IsError(ERR_QUIC_PROTOCOL_ERROR));
...
TEST_P(BidirectionalStreamQuicImplTest, ServerSendsRstAfterReadData)
{
    ...
    // Server sends a Rst. Since the stream has sent fin, the rst is one way in
    // IETF QUIC.
    ProcessPacket(server_maker_.Packet(3).AddRstStreamFrame(GetNthClientInitiatedBidirectionalStreamId(0), quic::QUIC_STREAM_CANCELLED).Build());
    ...
    EXPECT_EQ(0, delegate->on_data_sent_count());
    EXPECT_EQ(static_cast<int64_t>(spdy_request_headers_frame_length), delegate->GetTotalSentBytes());
    EXPECT_EQ(static_cast<int64_t>(spdy_response_headers_frame_length), delegate->GetTotalReceivedBytes());
}
...
TEST_P(BidirectionalStreamQuicImplTest, SessionClosedBeforeReadData)
{
    ...
    int rv = delegate->ReadData(cb.callback());
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
    session()->connection()->CloseConnection(quic::QUIC_NO_ERROR, "test", quic::ConnectionCloseBehavior::SILENT_CLOSE);
    delegate->WaitUntilNextCallback(kOnFailed);
...
TEST_P(BidirectionalStreamQuicImplTest, DeleteStreamDuringOnStreamReady)
{
    ...
    auto read_buffer = base::MakeRefCounted<IOBufferWithSize>(kReadBufferSize);
    auto delegate = std::make_unique<DeleteStreamDelegate>(read_buffer.get(), kReadBufferSize, DeleteStreamDelegate::ON_STREAM_READY);
    delegate->Start(&request, net_log_with_source(), session()->CreateHandle(destination_));
    ConfirmHandshake();
    delegate->WaitUntilNextCallback(kOnStreamReady);
...
TEST_P(BidirectionalStreamQuicImplTest, DeleteStreamDuringOnHeadersReceived)
{
    ...
    auto read_buffer = base::MakeRefCounted<IOBufferWithSize>(kReadBufferSize);
    auto delegate = std::make_unique<DeleteStreamDelegate>(read_buffer.get(), kReadBufferSize, DeleteStreamDelegate::ON_HEADERS_RECEIVED);
    delegate->Start(&request, net_log_with_source(), session()->CreateHandle(destination_));
    ConfirmHandshake();
    delegate->WaitUntilNextCallback(kOnStreamReady);
...
TEST_P(BidirectionalStreamQuicImplTest, DeleteStreamDuringOnDataRead)
{
    ...
    auto read_buffer = base::MakeRefCounted<IOBufferWithSize>(kReadBufferSize);
    auto delegate = std::make_unique<DeleteStreamDelegate>(read_buffer.get(), kReadBufferSize, DeleteStreamDelegate::ON_DATA_READ);
    delegate->Start(&request, net_log_with_source(), session()->CreateHandle(destination_));
    ConfirmHandshake();
    delegate->WaitUntilNextCallback(kOnStreamReady);
...
TEST_P(BidirectionalStreamQuicImplTest, AsyncFinRead)
{
    ...
    scoped_refptr<StringIOBuffer> buf1 = base::MakeRefCounted<StringIOBuffer>(kBody);
...
TEST_P(BidirectionalStreamQuicImplTest, DeleteStreamDuringOnTrailersReceived)
{
    ...
    AddWrite(ConstructInitialSettingsPacket());
    client_maker_.SetEncryptionLevel(quic::ENCRYPTION_FORWARD_SECURE);
    AddWrite(ConstructRequestHeadersPacket(kFin, DEFAULT_PRIORITY, &spdy_request_headers_frame_length));
