...
public:
    ...
    virtual void OnConnected(scoped_refptr<HttpResponseHeaders> response_headers) = 0;
    // CONNECTING -> FAILED
    virtual void OnConnectionFailed(const WebTransportError& error) = 0;
    // CONNECTED -> CLOSED
    virtual void OnClosed(const std::optional<WebTransportCloseInfo>& close_info) = 0;
    // CONNECTED -> FAILED
    ...
    virtual void OnDatagramProcessed(std::optional<quic::MessageStatus> status) = 0;
};
...
public:
    ...
    virtual void Close(const std::optional<WebTransportCloseInfo>& close_info) = 0;
