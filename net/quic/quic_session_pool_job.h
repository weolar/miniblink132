...
public:
    Job(QuicSessionPool* pool, QuicSessionAliasKey key, std::unique_ptr<CryptoClientConfigHandle> client_config_handle, RequestPriority priority,
    ...
    QuicSessionPool* pool() const
    {
        return pool_.get();
    }

    // Associate this job with another source.
    void AssociateWithNetLogSource(const NetLogWithSource& http_stream_job_net_log) const;
...
protected:
    ...
    virtual void UpdatePriority(RequestPriority old_priority, RequestPriority new_priority);
