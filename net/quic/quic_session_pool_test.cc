...
public:
    SessionAttemptHelper(QuicSessionPool* pool, quic::ParsedQuicVersion quic_version)
        : pool_(pool)
        , quic_endpoint(quic_version, IPEndPoint(IPAddress::IPv4Localhost(), QuicSessionPoolTestBase::kDefaultServerPort), ConnectionEndpointMetadata())
    {
        const url::SchemeHostPort destination(url::kHttpsScheme, QuicSessionPoolTestBase::kDefaultServerHostName, QuicSessionPoolTestBase::kDefaultServerPort);
        QuicSessionKey session_key(destination.host(), destination.port(), PrivacyMode::PRIVACY_MODE_DISABLED, ProxyChain::Direct(), SessionUsage::kDestination,
            SocketTag(), NetworkAnonymizationKey(), SecureDnsPolicy::kAllow, /*require_dns_https_alpn=*/false);
    ...
    int Start()
    {
        attempt_ = pool_->CreateSessionAttempt(this, quic_session_alias_key_.session_key(), quic_endpoint,
            /*cert_verify_flags=*/0,
        ...
        return attempt_->Start(base::BindOnce(&SessionAttemptHelper::OnComplete, base::Unretained(this)));
    }
...
public:
    MockQuicSessionPool(NetLog* net_log, HostResolver* host_resolver, SSLConfigService* ssl_config_service, ClientSocketFactory* client_socket_factory,
        HttpServerProperties* http_server_properties, CertVerifier* cert_verifier, TransportSecurityState* transport_security_state,
        ProxyDelegate* proxy_delegate, SCTAuditingDelegate* sct_auditing_delegate, SocketPerformanceWatcherFactory* socket_performance_watcher_factory,
        QuicCryptoClientStreamFactory* quic_crypto_client_stream_factory, QuicContext* context)
        : QuicSessionPool(net_log, host_resolver, ssl_config_service, client_socket_factory, http_server_properties, cert_verifier, transport_security_state,
            proxy_delegate, sct_auditing_delegate, socket_performance_watcher_factory, quic_crypto_client_stream_factory, context)
    {
    }
    ...
    void FinishConnectAndConfigureSocket(CompletionOnceCallback callback, DatagramClientSocket* socket, const SocketTag& socket_tag, int rv) override
    {
        QuicSessionPool::FinishConnectAndConfigureSocket(std::move(callback), socket, socket_tag, rv);
        MockFinishConnectAndConfigureSocket();
    }
};

class QuicSessionPoolTest : public QuicSessionPoolTestBase, public ::testing::TestWithParam<TestParams> {
protected:
    QuicSessionPoolTest()
    ...
    void TestThatBlackHoleIsDisabledOnNoNewNetworkThenResumedAfterConnectingToANetwork(bool is_blackhole_disabled_after_disconnecting);
    ...
    base::RepeatingCallback<bool(const quic::QuicFrame&)> FilterControlStreamOnly()
    {
        quic::QuicStreamId control_stream_id = quic::QuicUtils::GetFirstUnidirectionalStreamId(version_.transport_version, quic::Perspective::IS_CLIENT);
        return base::BindRepeating(
            [](quic::QuicStreamId control_stream_id, const quic::QuicFrame& frame) {
                return frame.type == quic::STREAM_FRAME && frame.stream_frame.stream_id == control_stream_id;
            },
            control_stream_id);
    }
...
void QuicSessionPoolTest::InitializeConnectionMigrationV2Test(NetworkChangeNotifier::NetworkList connected_networks)
{
...
void QuicSessionPoolTest::VerifyServerMigration(const quic::QuicConfig& config, IPEndPoint expected_address)
{
    ...
    socket_data1.AddWrite(ASYNC, client_maker_.MakeDummyCHLOPacket(packet_number++));
    client_maker_.set_save_packet_frames(true);
    // Change the encryption level after handshake is confirmed.
    client_maker_.SetEncryptionLevel(quic::ENCRYPTION_FORWARD_SECURE);
    socket_data1.AddWrite(SYNCHRONOUS, ConstructInitialSettingsPacket(packet_number++));
    ...
    socket_data2.AddWrite(SYNCHRONOUS, client_maker_.Packet(packet_number++).AddPathChallengeFrame().AddPaddingFrame().Build());
    socket_data2.AddRead(ASYNC, server_maker_.Packet(1).AddPathResponseFrame().AddPaddingFrame().Build());
    socket_data2.AddReadPauseForever();
    socket_data2.AddWrite(SYNCHRONOUS, client_maker_.MakeRetransmissionPacket(2, packet_number++));
    socket_data2.AddWrite(SYNCHRONOUS, client_maker_.Packet(packet_number++).AddPingFrame().Build());
    socket_data2.AddWrite(SYNCHRONOUS, client_maker_.Packet(packet_number++).AddRetireConnectionIdFrame(/*sequence_number=*/0u).Build());
    ...
    EXPECT_EQ(actual_address, expected_address) << "Socket connected to: " << actual_address.address().ToString() << " " << actual_address.port()
                                                << "Expected address: " << expected_address.address().ToString() << " " << expected_address.port();
...
// NetworkAnonymizationKeys, but the same server. If false, stores data for
// two different servers, using the same NetworkAnonymizationKey.
void QuicSessionPoolTest::VerifyInitialization(bool vary_network_anonymization_key)
{
    ...
    if (vary_network_anonymization_key) {
        network_anonymization_key2 = NetworkAnonymizationKey::CreateSameSite(kSite2);
    ...
    const quic::QuicConfig* config = QuicSessionPoolPeer::GetConfig(factory_.get());
    EXPECT_EQ(500, config->IdleNetworkTimeout().ToSeconds());

    QuicSessionPoolPeer::SetTaskRunner(factory_.get(), runner_.get());

    const AlternativeService alternative_service1(kProtoQUIC, kDefaultServerHostName, kDefaultServerPort);
    ...
    std::unique_ptr<QuicServerInfo> quic_server_info
        = std::make_unique<PropertiesBasedQuicServerInfo>(quic_server_id1, PRIVACY_MODE_DISABLED, network_anonymization_key1, http_server_properties_.get());
    ...
    std::unique_ptr<QuicServerInfo> quic_server_info2
        = std::make_unique<PropertiesBasedQuicServerInfo>(quic_server_id2, PRIVACY_MODE_DISABLED, network_anonymization_key2, http_server_properties_.get());
    ...
    host_resolver_->rules()->AddIPLiteralRule(kDefaultServerHostName, "192.168.0.1", "");
    ...
    RequestBuilder builder(this);
    builder.destination = url::SchemeHostPort(url::kHttpsScheme, quic_server_id1.host(), quic_server_id1.port());
    builder.network_anonymization_key = network_anonymization_key1;
    EXPECT_EQ(ERR_IO_PENDING, builder.CallRequest());
    EXPECT_THAT(callback_.WaitForResult(), IsOk());

    EXPECT_FALSE(QuicSessionPoolPeer::CryptoConfigCacheIsEmpty(factory_.get(), quic_server_id1, network_anonymization_key1));

    std::unique_ptr<QuicCryptoClientConfigHandle> crypto_config_handle1 = QuicSessionPoolPeer::GetCryptoConfig(factory_.get(), network_anonymization_key1);
    ...
    host_resolver_->rules()->AddIPLiteralRule(kDefaultServerHostName, "192.168.0.2", "");

    RequestBuilder builder2(this);
    builder2.destination = url::SchemeHostPort(url::kHttpsScheme, quic_server_id2.host(), quic_server_id2.port());
    builder2.network_anonymization_key = network_anonymization_key2;
    builder2.url = vary_network_anonymization_key ? GURL(kDefaultUrl) : GURL("https://mail.example.org/");
    EXPECT_EQ(ERR_IO_PENDING, builder2.CallRequest());
    EXPECT_THAT(callback_.WaitForResult(), IsOk());

    EXPECT_FALSE(QuicSessionPoolPeer::CryptoConfigCacheIsEmpty(factory_.get(), quic_server_id2, network_anonymization_key2));
    std::unique_ptr<QuicCryptoClientConfigHandle> crypto_config_handle2 = QuicSessionPoolPeer::GetCryptoConfig(factory_.get(), network_anonymization_key2);
...
INSTANTIATE_TEST_SUITE_P(
    VersionIncludeStreamDependencySequence, QuicSessionPoolTest, ::testing::ValuesIn(GetTestParams()), ::testing::PrintToStringParamName());
...
TEST_P(QuicSessionPoolTest, CachedInitialRtt)
{
    ...
    http_server_properties_->SetServerNetworkStats(url::SchemeHostPort(GURL(kDefaultUrl)), NetworkAnonymizationKey(), stats);
...
TEST_P(QuicSessionPoolTest, CachedInitialRttWithNetworkAnonymizationKey)
{
    ...
    ServerNetworkStats stats;
    stats.srtt = base::Milliseconds(10);
    http_server_properties_->SetServerNetworkStats(url::SchemeHostPort(GURL(kDefaultUrl)), kNetworkAnonymizationKey1, stats);
    quic_params_->estimate_initial_rtt = true;
    Initialize();
    ...
    for (const auto& network_anonymization_key : { kNetworkAnonymizationKey1, kNetworkAnonymizationKey2, NetworkAnonymizationKey() }) {
        ...
        QuicChromiumClientSession* session = GetActiveSession(kDefaultDestination, PRIVACY_MODE_DISABLED, network_anonymization_key);
...
TEST_P(QuicSessionPoolTest, ServerNetworkStatsWithNetworkAnonymizationKey)
{
    ...
    for (size_t i = 0; i < std::size(kNetworkAnonymizationKeys); ++i) {
        ...
        QuicChromiumClientSession* session = GetActiveSession(kDefaultDestination, PRIVACY_MODE_DISABLED, kNetworkAnonymizationKeys[i]);

        session->OnHttp3GoAway(0);

        EXPECT_FALSE(HasActiveSession(kDefaultDestination, PRIVACY_MODE_DISABLED, kNetworkAnonymizationKeys[i]));
        ...
        for (size_t j = 0; j < std::size(kNetworkAnonymizationKeys); ++j) {
            ...
            if (j <= i) {
                EXPECT_TRUE(http_server_properties_->GetServerNetworkStats(url::SchemeHostPort(GURL(kDefaultUrl)), kNetworkAnonymizationKeys[j]));
            } else {
                EXPECT_FALSE(http_server_properties_->GetServerNetworkStats(url::SchemeHostPort(GURL(kDefaultUrl)), kNetworkAnonymizationKeys[j]));
            }
        }
    }
...
TEST_P(QuicSessionPoolTest, PooledWithDifferentIpSession)
{
    ...
    const IPEndPoint kRightIP(*IPAddress::FromIPLiteral("192.168.0.1"), kDefaultServerPort);
    const IPEndPoint kWrongIP(*IPAddress::FromIPLiteral("192.168.0.2"), kDefaultServerPort);
    ...
    url::SchemeHostPort server3(url::kHttpsScheme, kServer3HostName, kDefaultServerPort);
    url::SchemeHostPort server4(url::kHttpsScheme, kServer4HostName, kDefaultServerPort);
    url::SchemeHostPort server5(url::kHttpsScheme, kServer5HostName, kDefaultServerPort);
    host_resolver_->set_synchronous_mode(true);
    host_resolver_->rules()->AddIPLiteralRule(kDefaultServerHostName, "192.168.0.1", "");
    ...
    host_resolver_->rules()->AddRule(server3.host(), MockHostResolverBase::RuleResolver::RuleResult({ std::move(endpoints) }));
    ...
    host_resolver_->rules()->AddRule(server4.host(), MockHostResolverBase::RuleResolver::RuleResult({ std::move(endpoints) }));
    ...
    endpoints[0].metadata.supported_protocol_alpns = { kWrongALPN };
    host_resolver_->rules()->AddRule(server5.host(), MockHostResolverBase::RuleResolver::RuleResult({ std::move(endpoints) }));
    ...
    ASSERT_EQ(4u, GetActiveSession(kDefaultDestination)->received_origins().size());
...
TEST_P(QuicSessionPoolTest, PoolingSkipDns)
{
    ...
    frame.origins.push_back(base::StrCat({ "https://", kServer2HostName }));
    GetActiveSession(kDefaultDestination)->OnOriginFrame(frame);
    ASSERT_EQ(1u, GetActiveSession(kDefaultDestination)->received_origins().size());
...
TEST_P(QuicSessionPoolTest, IgnoreIpMatchingWhenFindingExistingSessionsAndSkipDns)
{
    ...
    EXPECT_TRUE(GetActiveSession(kDefaultDestination)->config()->SetInitialReceivedConnectionOptions(initial_received_options));
...
TEST_P(QuicSessionPoolTest, PoolingWithServerMigration)
{
    ...
    config.SetPreferredAddressConnectionIdAndTokenToSend(kNewCID, quic::QuicUtils::GenerateStatelessResetToken(kNewCID));
...
{
    base::test::ScopedFeatureList scoped_feature_list;
    scoped_feature_list.InitAndEnableFeature(net::features::kStaticKeyPinningEnforcement);
    Initialize();
...
TEST_P(QuicSessionPoolTest, MaxOpenStream)
{
    ...
    socket_data.AddRead(ASYNC,
        server_maker_.Packet(1).AddStopSendingFrame(stream_id, quic::QUIC_STREAM_CANCELLED).AddRstStreamFrame(stream_id, quic::QUIC_STREAM_CANCELLED).Build());
    socket_data.AddRead(ASYNC,
        server_maker_.Packet(2)
            .AddMaxStreamsFrame(/*control_frame_id=*/1,
                /*stream_count=*/52,
                /*unidirectional=*/false)
            .Build());
    socket_data.AddWrite(SYNCHRONOUS, client_maker_.Packet(packet_num++).AddAckFrame(1, 2, 1).Build());
    socket_data.AddReadPauseForever();
    socket_data.AddSocketDataToFactory(socket_factory_.get());
...
TEST_P(QuicSessionPoolTest, CloseAllSessions)
{
    ...
    int packet_num = 1;
    socket_data.AddWrite(SYNCHRONOUS, ConstructInitialSettingsPacket(packet_num++));
    socket_data.AddWrite(SYNCHRONOUS, client_maker_.Packet(packet_num++).AddConnectionCloseFrame(quic::QUIC_PEER_GOING_AWAY, "net error").Build());
    socket_data.AddSocketDataToFactory(socket_factory_.get());
...
TEST_P(QuicSessionPoolTest, WriteErrorInCryptoConnectWithAsyncHostResolutionSyncSessionCreation)
{
    base::test::ScopedFeatureList scoped_feature_list;
    scoped_feature_list.InitAndDisableFeature(net::features::kAsyncQuicSession);
    Initialize();
...
TEST_P(QuicSessionPoolTest, WriteErrorInCryptoConnectWithAsyncHostResolutionAsyncSessionCreation)
{
    Initialize();
    // Use unmocked crypto stream to do crypto connect.
    crypto_client_stream_factory_.set_handshake_mode(MockCryptoClientStream::COLD_START_WITH_CHLO_SENT);
...
TEST_P(QuicSessionPoolTest, WriteErrorInCryptoConnectWithSyncHostResolutionSyncQuicSession)
{
    base::test::ScopedFeatureList scoped_feature_list;
    scoped_feature_list.InitAndDisableFeature(net::features::kAsyncQuicSession);
    Initialize();
...
TEST_P(QuicSessionPoolTest, WriteErrorInCryptoConnectWithSyncHostResolutionAsyncQuicSession)
{
    Initialize();
    // Use unmocked crypto stream to do crypto connect.
    crypto_client_stream_factory_.set_handshake_mode(MockCryptoClientStream::COLD_START_WITH_CHLO_SENT);
...
TEST_P(QuicSessionPoolTest, CloseSessionDuringCreation)
{
    ...
    auto factory = MockQuicSessionPool(net_log_.net_log(), host_resolver_.get(), &ssl_config_service_, socket_factory_.get(), http_server_properties_.get(),
    ...
    socket_data.AddWrite(SYNCHRONOUS, client_maker_.Packet(packet_num).AddConnectionCloseFrame(quic::QUIC_IP_ADDRESS_CHANGED, "net error").Build());
    socket_data.AddSocketDataToFactory(socket_factory_.get());
    ...
    EXPECT_CALL(factory, MockFinishConnectAndConfigureSocket()).WillOnce([] { NetworkChangeNotifier::NotifyObserversOfIPAddressChangeForTests(); });
    ...
    EXPECT_TRUE(QuicSessionPoolPeer::HasActiveSession(&factory, server_id, PRIVACY_MODE_DISABLED, NetworkAnonymizationKey()));
    QuicChromiumClientSession* session = QuicSessionPoolPeer::GetActiveSession(&factory, server_id, PRIVACY_MODE_DISABLED, NetworkAnonymizationKey());
    EXPECT_TRUE(QuicSessionPoolPeer::IsLiveSession(&factory, session));

    base::RunLoop().RunUntilIdle();

    // Session should now be closed.
    EXPECT_FALSE(QuicSessionPoolPeer::HasActiveSession(&factory, server_id, PRIVACY_MODE_DISABLED, NetworkAnonymizationKey()));
}
...
TEST_P(QuicSessionPoolTest, GoAwaySessionsOnIPAddressChanged)
{
    ...
    quic_data1.AddWrite(SYNCHRONOUS, ConstructGetRequestPacket(packet_num++, GetNthClientInitiatedBidirectionalStreamId(0), true));
    quic_data1.AddReadPause();
    quic_data1.AddRead(ASYNC, ConstructOkResponsePacket(1, GetNthClientInitiatedBidirectionalStreamId(0), true));
...
void QuicSessionPoolTest::TestMigrationOnNetworkMadeDefault(IoMode write_mode)
{
    ...
    int packet_num = 1;
    quic_data1.AddWrite(SYNCHRONOUS, ConstructInitialSettingsPacket(packet_num++));
    quic_data1.AddWrite(write_mode, ConstructGetRequestPacket(packet_num++, GetNthClientInitiatedBidirectionalStreamId(0), true));
    quic_data1.AddSocketDataToFactory(socket_factory_.get());
    ...
    quic_data2.AddWrite(SYNCHRONOUS, client_maker_.Packet(packet_num++).AddPathChallengeFrame().AddPaddingFrame().Build());
    quic_data2.AddReadPause();
    // Connectivity probe to receive from the server.
    quic_data2.AddRead(ASYNC, server_maker_.Packet(1).AddPathResponseFrame().AddPaddingFrame().Build());
    ...
    quic_data2.AddRead(ASYNC, ConstructOkResponsePacket(2, GetNthClientInitiatedBidirectionalStreamId(0), false));
    quic_data2.AddReadPauseForever();
...
TEST_P(QuicSessionPoolTest, MigratedToBlockedSocketAfterProbing)
{
    ...
    quic_data2.AddWrite(ASYNC, client_maker_.Packet(packet_num++).AddPathChallengeFrame().AddPaddingFrame().Build());
    quic_data2.AddRead(ASYNC, ConstructOkResponsePacket(2, GetNthClientInitiatedBidirectionalStreamId(0), false));
    quic_data2.AddReadPauseForever();
...
TEST_P(QuicSessionPoolTest, OnNetworkMadeDefaultNonMigratableStream_MigrateIdleSessions)
{
...
TEST_P(QuicSessionPoolTest, OnNetworkMadeDefaultNonMigratableStream_DoNotMigrateIdleSessions)
{
    TestOnNetworkMadeDefaultNonMigratableStream(false);
}

void QuicSessionPoolTest::TestOnNetworkMadeDefaultNonMigratableStream(bool migrate_idle_sessions)
{
    ...
    MockQuicData quic_data1(version_);
    // Connectivity probe to be sent on the new path.
    quic_data1.AddWrite(SYNCHRONOUS, client_maker_.Packet(packet_num++).AddPathChallengeFrame().AddPaddingFrame().Build());
    quic_data1.AddReadPause();
    // Connectivity probe to receive from the server.
    quic_data1.AddRead(ASYNC, server_maker_.Packet(1).AddPathResponseFrame().AddPaddingFrame().Build());
    ...
    if (migrate_idle_sessions) {
        ...
        quic_data1.AddWrite(SYNCHRONOUS,
            client_maker_.Packet(packet_num++)
                ...
                .AddRstStreamFrame(GetNthClientInitiatedBidirectionalStreamId(0), quic::QUIC_STREAM_CANCELLED)
                .Build());
        quic_data1.AddWrite(SYNCHRONOUS, client_maker_.MakeRetransmissionPacket(1, packet_num++));
        // Ping packet to send after migration is completed.
        quic_data1.AddWrite(SYNCHRONOUS, client_maker_.Packet(packet_num++).AddPingFrame().Build());
        quic_data1.AddWrite(SYNCHRONOUS, client_maker_.Packet(packet_num++).AddRetireConnectionIdFrame(0u).Build());
    } else {
        client_maker_.set_connection_id(cid_on_old_path);
        socket_data.AddWrite(SYNCHRONOUS,
            client_maker_.Packet(packet_num++)
                ...
                .AddAckFrame(/*first_received=*/1, /*largest_received=*/1,
                    /*smallest_received=*/1)
                .AddConnectionCloseFrame(quic::QUIC_CONNECTION_MIGRATION_NO_MIGRATABLE_STREAMS, "net error", /*frame_type=*/0x1b)
                .Build());
    }
...
TEST_P(QuicSessionPoolTest, OnNetworkDisconnectedNonMigratableStream_DoNotMigrateIdleSessions)
{
    TestOnNetworkDisconnectedNonMigratableStream(false);
}

TEST_P(QuicSessionPoolTest, OnNetworkDisconnectedNonMigratableStream_MigrateIdleSessions)
{
    TestOnNetworkDisconnectedNonMigratableStream(true);
}

void QuicSessionPoolTest::TestOnNetworkDisconnectedNonMigratableStream(bool migrate_idle_sessions)
{
    quic_params_->migrate_idle_sessions = migrate_idle_sessions;
    InitializeConnectionMigrationV2Test({ kDefaultNetworkForTests, kNewNetworkForTests });
    ProofVerifyDetailsChromium verify_details = DefaultProofVerifyDetails();
    ...
    if (migrate_idle_sessions) {
        ...
        auto packet_two_frames = client_maker_.CloneSavedFrames(2); // STREAM, RST_STREAM
        ...
        socket_data.AddWrite(SYNCHRONOUS, client_maker_.Packet(packet_num++).AddPingFrame().Build());
        socket_data.AddWrite(SYNCHRONOUS, client_maker_.Packet(packet_num++).AddRetireConnectionIdFrame(0u).Build());
        socket_data.AddSocketDataToFactory(socket_factory_.get());
    } else {
        socket_data.AddReadPauseForever();
        int packet_num = 1;
    ...
    EXPECT_EQ(migrate_idle_sessions, QuicSessionPoolPeer::IsLiveSession(factory_.get(), session));
    EXPECT_EQ(migrate_idle_sessions, HasActiveSession(kDefaultDestination));
...
TEST_P(QuicSessionPoolTest, OnNetworkMadeDefaultNoOpenStreams_DoNotMigrateIdleSessions)
{
    TestOnNetworkMadeDefaultNoOpenStreams(false);
}

TEST_P(QuicSessionPoolTest, OnNetworkMadeDefaultNoOpenStreams_MigrateIdleSessions)
{
    TestOnNetworkMadeDefaultNoOpenStreams(true);
}

void QuicSessionPoolTest::TestOnNetworkMadeDefaultNoOpenStreams(bool migrate_idle_sessions)
{
    quic_params_->migrate_idle_sessions = migrate_idle_sessions;
    InitializeConnectionMigrationV2Test({ kDefaultNetworkForTests, kNewNetworkForTests });
    ProofVerifyDetailsChromium verify_details = DefaultProofVerifyDetails();
    ...
    if (!migrate_idle_sessions) {
        socket_data.AddWrite(
            SYNCHRONOUS, client_maker_.Packet(packet_num).AddConnectionCloseFrame(quic::QUIC_CONNECTION_MIGRATION_NO_MIGRATABLE_STREAMS, "net error").Build());
    }
    socket_data.AddSocketDataToFactory(socket_factory_.get());
...
TEST_P(QuicSessionPoolTest, OnNetworkDisconnectedNoOpenStreams_DoNotMigateIdleSessions)
{
    TestOnNetworkDisconnectedNoOpenStreams(false);
}

TEST_P(QuicSessionPoolTest, OnNetworkDisconnectedNoOpenStreams_MigateIdleSessions)
{
    TestOnNetworkDisconnectedNoOpenStreams(true);
}

void QuicSessionPoolTest::TestOnNetworkDisconnectedNoOpenStreams(bool migrate_idle_sessions)
{
    quic_params_->migrate_idle_sessions = migrate_idle_sessions;
    InitializeConnectionMigrationV2Test({ kDefaultNetworkForTests, kNewNetworkForTests });
    ProofVerifyDetailsChromium verify_details = DefaultProofVerifyDetails();
    ...
    if (migrate_idle_sessions) {
        ...
        alternate_socket_data.AddWrite(SYNCHRONOUS, client_maker_.MakeRetransmissionPacket(1, packet_num++));
        // Ping packet to send after migration.
        alternate_socket_data.AddWrite(SYNCHRONOUS, client_maker_.Packet(packet_num++).AddPingFrame().Build());
        alternate_socket_data.AddWrite(SYNCHRONOUS, client_maker_.Packet(packet_num++).AddRetireConnectionIdFrame(0u).Build());
...
void QuicSessionPoolTest::TestMigrationOnNetworkDisconnected(bool async_write_before)
{
    InitializeConnectionMigrationV2Test({ kDefaultNetworkForTests, kNewNetworkForTests });
    ...
    socket_data.AddWrite(SYNCHRONOUS, ConstructGetRequestPacket(packet_number++, GetNthClientInitiatedBidirectionalStreamId(0), true));
    ...
    MockQuicData socket_data1(version_);
    client_maker_.set_connection_id(cid_on_new_path);
    socket_data1.AddWrite(SYNCHRONOUS, client_maker_.MakeCombinedRetransmissionPacket({ 1, 2 }, packet_number++));
    socket_data1.AddWrite(SYNCHRONOUS, client_maker_.Packet(packet_number++).AddPingFrame().Build());
    socket_data1.AddWrite(SYNCHRONOUS, client_maker_.Packet(packet_number++).AddRetireConnectionIdFrame(0u).Build());
    socket_data1.AddRead(ASYNC, ConstructOkResponsePacket(1, GetNthClientInitiatedBidirectionalStreamId(0), false));
    socket_data1.AddReadPauseForever();
...
TEST_P(QuicSessionPoolTest, MigrateOnDefaultNetworkDisconnectedWithProxiedSession)
{
    InitializeConnectionMigrationV2Test({ kDefaultNetworkForTests, kNewNetworkForTests });
    scoped_mock_network_change_notifier_->mock_network_change_notifier()->NotifyNetworkMadeDefault(kDefaultNetworkForTests);
    ...
    auto proxy_chain = ProxyChain::ForIpProtection({
        ProxyServer::FromSchemeHostAndPort(ProxyServer::SCHEME_QUIC, proxy_origin.host(), 443),
    });
    ...
    socket_data.AddWrite("initial-settings", ConstructInitialSettingsPacket(to_proxy_packet_num++)).Sync();
    socket_data
        .AddWrite("connect-udp",
            ConstructConnectUdpRequestPacket(to_proxy_packet_num++, stream_id, proxy.host(), "/.well-known/masque/udp/www.example.org/443/", false))
        .Sync();
    socket_data.AddRead("server-settings", ConstructServerSettingsPacket(from_proxy_packet_num++));
    socket_data.AddRead("connect-ok-response", ConstructOkResponsePacket(from_proxy_packet_num++, stream_id, true));

    socket_data.AddWrite("ack-ok", client_maker_.Packet(to_proxy_packet_num++).AddAckFrame(1, 2, 1).Build());

    quiche::HttpHeaderBlock headers = to_endpoint_maker.GetRequestHeaders("GET", "https", "/");
    spdy::SpdyPriority priority = ConvertRequestPriorityToQuicPriority(DEFAULT_PRIORITY);

    socket_data.AddWrite("initial-settings-and-get",
        client_maker_.Packet(to_proxy_packet_num++)
            .AddMessageFrame(
                ConstructClientH3DatagramFrame(stream_id, kConnectUdpContextId, to_endpoint_maker.MakeInitialSettingsPacket(to_endpoint_packet_num++)))
            .AddMessageFrame(ConstructClientH3DatagramFrame(stream_id, kConnectUdpContextId,
                to_endpoint_maker.MakeRequestHeadersPacket(to_endpoint_packet_num++, stream_id, /*fin=*/true, priority, std::move(headers), nullptr)))
            .Build());
    ...
    QuicChromiumClientSession* destination_session = GetActiveSession(kDefaultDestination, PRIVACY_MODE_DISABLED, NetworkAnonymizationKey(), proxy_chain);
    EXPECT_TRUE(QuicSessionPoolPeer::IsLiveSession(factory_.get(), destination_session));

    // Ensure that the session to the proxy is alive and active.
    QuicChromiumClientSession* proxy_session
        = GetActiveSession(proxy_origin, PRIVACY_MODE_DISABLED, NetworkAnonymizationKey(), ProxyChain::ForIpProtection({}), SessionUsage::kProxy);
    EXPECT_TRUE(QuicSessionPoolPeer::IsLiveSession(factory_.get(), proxy_session));
    quic::QuicConnectionId cid_on_new_path = quic::test::TestConnectionId(12345678);
    ...
    client_maker_.set_connection_id(cid_on_new_path);
    socket_data1.AddWrite("retransmit", client_maker_.MakeCombinedRetransmissionPacket({ 1, 2 }, to_proxy_packet_num++)).Sync();
    socket_data1.AddWrite("ping", client_maker_.Packet(to_proxy_packet_num++).AddPingFrame().Build()).Sync();
    socket_data1.AddWrite("retire-cid", client_maker_.Packet(to_proxy_packet_num++).AddRetireConnectionIdFrame(0u).Build()).Sync();
    quiche::HttpHeaderBlock response_headers = from_endpoint_maker.GetResponseHeaders("200");
    socket_data1.AddRead("proxied-ok-response",
        server_maker_.Packet(from_proxy_packet_num++)
            .AddMessageFrame(ConstructClientH3DatagramFrame(stream_id, kConnectUdpContextId,
                from_endpoint_maker.MakeResponseHeadersPacket(from_endpoint_packet_num++, stream_id, /*fin=*/true, std::move(response_headers), nullptr)))
            .Build());
    ...
    EXPECT_TRUE(QuicSessionPoolPeer::IsLiveSession(factory_.get(), destination_session));
    EXPECT_TRUE(HasActiveSession(kDefaultDestination, PRIVACY_MODE_DISABLED, NetworkAnonymizationKey(), proxy_chain));
...
TEST_P(QuicSessionPoolTest, MigrateOnPathDegradingWithProxiedSession)
{
    ...
    QuicSocketDataProvider socket_data1(version_);
    client_maker_.set_connection_id(cid_on_new_path);
    socket_data1.AddWrite("probe", client_maker_.Packet(to_proxy_packet_num++).AddPathChallengeFrame().AddPaddingFrame(1315).Build()).Sync();
    socket_data1.AddRead("probe-back", server_maker_.Packet(from_proxy_packet_num++).AddPathResponseFrame().AddPaddingFrame(1315).Build());
    socket_data1
        .AddWrite("retransmit-and-retire",
            client_maker_.Packet(to_proxy_packet_num++).AddPacketRetransmission(1).AddPacketRetransmission(2).AddRetireConnectionIdFrame(0u).Build())
        .Sync();
    socket_data1.AddWrite("ping", client_maker_.Packet(to_proxy_packet_num++).AddPingFrame().Build()).Sync();
    quiche::HttpHeaderBlock response_headers = from_endpoint_maker.GetResponseHeaders("200");
    socket_data1.AddRead("proxied-ok-response",
...
TEST_P(QuicSessionPoolTest, NewNetworkConnectedAfterNoNetwork)
{
    ...
    int packet_num = 1;
    socket_data.AddWrite(SYNCHRONOUS, ConstructInitialSettingsPacket(packet_num++));
    socket_data.AddWrite(SYNCHRONOUS, ConstructGetRequestPacket(packet_num++, GetNthClientInitiatedBidirectionalStreamId(0), true));
    socket_data.AddSocketDataToFactory(socket_factory_.get());
    ...
    MockQuicData socket_data1(version_);
    client_maker_.set_connection_id(cid_on_new_path);
    socket_data1.AddWrite(SYNCHRONOUS, client_maker_.MakeCombinedRetransmissionPacket({ 1, 2 }, packet_num++));
    socket_data1.AddWrite(SYNCHRONOUS, client_maker_.Packet(packet_num++).AddPingFrame().Build());
    socket_data1.AddWrite(SYNCHRONOUS, client_maker_.Packet(packet_num++).AddRetireConnectionIdFrame(0u).Build());
    socket_data1.AddRead(ASYNC, ConstructOkResponsePacket(1, GetNthClientInitiatedBidirectionalStreamId(0), false));
    socket_data1.AddReadPauseForever();
    socket_data1.AddWrite(SYNCHRONOUS,
        client_maker_.Packet(packet_num++)
...
TEST_P(QuicSessionPoolTest, MigrateToProbingSocket)
{
    ...
    quic_data1.AddWrite(SYNCHRONOUS, ConstructGetRequestPacket(packet_number++, GetNthClientInitiatedBidirectionalStreamId(0), true));
    quic_data1.AddSocketDataToFactory(socket_factory_.get());
    ...
    client_maker_.set_connection_id(cid_on_new_path);
    // Connectivity probe to be sent on the new path.
    quic_data2.AddWrite(SYNCHRONOUS, client_maker_.Packet(packet_number++).AddPathChallengeFrame().AddPaddingFrame().Build());
    quic_data2.AddReadPause();
    ...
    quic_data2.AddRead(SYNCHRONOUS, server_maker_.Packet(2).AddPathResponseFrame().AddPaddingFrame().Build());
    quic_data2.AddRead(SYNCHRONOUS, server_maker_.Packet(3).AddPathResponseFrame().AddPaddingFrame().Build());
    quic_data2.AddRead(SYNCHRONOUS, server_maker_.Packet(4).AddPathResponseFrame().AddPaddingFrame().Build());
    quic_data2.AddWrite(ASYNC, client_maker_.MakeAckAndRetransmissionPacket(packet_number++, 1, 4, 1, { 1, 2 }));
    quic_data2.AddWrite(SYNCHRONOUS, client_maker_.Packet(packet_number++).AddRetireConnectionIdFrame(0u).Build());
    quic_data2.AddRead(ASYNC, ConstructOkResponsePacket(5, GetNthClientInitiatedBidirectionalStreamId(0), false));
    quic_data2.AddReadPauseForever();
    quic_data2.AddWrite(SYNCHRONOUS,
        client_maker_.Packet(packet_number++)
...
void QuicSessionPoolTest::TestMigrationOnPathDegrading(bool async_write_before)
{
    InitializeConnectionMigrationV2Test({ kDefaultNetworkForTests, kNewNetworkForTests });
    ProofVerifyDetailsChromium verify_details = DefaultProofVerifyDetails();
    crypto_client_stream_factory_.AddProofVerifyDetails(&verify_details);
...
TEST_P(QuicSessionPoolTest, MigrateSessionEarlyProbingWriterError)
{
    ...
    client_maker_.set_connection_id(cid_on_old_path);
    quic_data1.AddWrite(SYNCHRONOUS, client_maker_.Packet(packet_number++).AddRetireConnectionIdFrame(/*sequence_number=*/1u).Build());
...
TEST_P(QuicSessionPoolTest, MigrateSessionEarlyProbingWriterErrorThreeNetworks)
{
    InitializeConnectionMigrationV2Test({ kDefaultNetworkForTests, kNewNetworkForTests });
    ...
    quic_data1.AddWrite(ASYNC, client_maker_.Packet(packet_number++).AddRetireConnectionIdFrame(/*sequence_number=*/1u).Build());
...
TEST_P(QuicSessionPoolTest, MultiPortSessionWithMigration)
{
    ...
    quic_data1.AddWrite(SYNCHRONOUS, ConstructGetRequestPacket(3, GetNthClientInitiatedBidirectionalStreamId(0), true));
    quic_data1.AddSocketDataToFactory(socket_factory_.get());
    ...
    client_maker_.set_connection_id(cid_on_new_path);
    // Connectivity probe to be sent on the new path.
    quic_data2.AddWrite(SYNCHRONOUS, client_maker_.Packet(2).AddPathChallengeFrame().AddPaddingFrame().Build());
    quic_data2.AddReadPause();
    // Connectivity probe to receive from the server.
    quic_data2.AddRead(ASYNC, server_maker_.Packet(1).AddPathResponseFrame().AddPaddingFrame().Build());
    quic_data2.AddReadPause();
    ...
    quic_data2.AddWrite(ASYNC,
        client_maker_.Packet(4)
            ...
            .AddPingFrame()
            .Build());
    quic_data2.AddWrite(SYNCHRONOUS, client_maker_.Packet(5).AddRetireConnectionIdFrame(/*sequence_number=*/0u).Build());
    quic_data2.AddRead(ASYNC, server_maker_.Packet(3).AddAckFrame(1, 5, 1).Build());
    quic_data2.AddReadPauseForever();
    quic_data2.AddWrite(SYNCHRONOUS,
...
TEST_P(QuicSessionPoolTest, SuccessfullyMigratedToServerPreferredAddress)
{
    ...
    crypto_client_stream_factory_.AddProofVerifyDetails(&verify_details);
    quic::QuicConfig config;
    config.SetIPv4AlternateServerAddressToSend(ToQuicSocketAddress(server_preferred_address));
    quic::test::QuicConfigPeer::SetPreferredAddressConnectionIdAndToken(&config, kNewCID, quic::QuicUtils::GenerateStatelessResetToken(kNewCID));
    crypto_client_stream_factory_.SetConfig(config);
    // Use cold start mode to send crypto message for handshake.
    crypto_client_stream_factory_.set_handshake_mode(MockCryptoClientStream::COLD_START_WITH_CHLO_SENT);
    ...
    MockQuicData quic_data1(version_);
    quic_data1.AddReadPauseForever();
    quic_data1.AddWrite(ASYNC, client_maker_.MakeDummyCHLOPacket(packet_number++));
    // Change the encryption level after handshake is confirmed.
    client_maker_.SetEncryptionLevel(quic::ENCRYPTION_FORWARD_SECURE);
    quic_data1.AddWrite(SYNCHRONOUS, ConstructInitialSettingsPacket(packet_number++));
    quic_data1.AddSocketDataToFactory(socket_factory_.get());
    ...
    EXPECT_FALSE(HasActiveJob(kDefaultDestination, PRIVACY_MODE_DISABLED));
    QuicChromiumClientSession* session = GetActiveSession(kDefaultDestination);
    EXPECT_FALSE(session->connection()->GetStats().server_preferred_address_validated);
    EXPECT_FALSE(session->connection()->GetStats().failed_to_validate_server_preferred_address);
    const quic::QuicSocketAddress peer_address = session->peer_address();

    quic_data2.Resume();
    EXPECT_FALSE(session->connection()->HasPendingPathValidation());
    EXPECT_TRUE(session->connection()->GetStats().server_preferred_address_validated);
    EXPECT_FALSE(session->connection()->GetStats().failed_to_validate_server_preferred_address);
    EXPECT_NE(session->peer_address(), peer_address);
    EXPECT_EQ(session->peer_address(), ToQuicSocketAddress(server_preferred_address));
...
TEST_P(QuicSessionPoolTest, FailedToValidateServerPreferredAddress)
{
    ...
    EXPECT_FALSE(session->connection()->HasPendingPathValidation());
    EXPECT_FALSE(session->connection()->GetStats().server_preferred_address_validated);
    EXPECT_TRUE(session->connection()->GetStats().failed_to_validate_server_preferred_address);
    EXPECT_EQ(session->peer_address(), peer_address);
    EXPECT_NE(session->peer_address(), ToQuicSocketAddress(server_preferred_address));
...
TEST_P(QuicSessionPoolTest, ServerMigrationDisabled)
{
    ...
    IPEndPoint expected_address = IPEndPoint(IPAddress(127, 0, 0, 1), kDefaultServerPort);
    EXPECT_EQ(actual_address, expected_address) << "Socket connected to: " << actual_address.address().ToString() << " " << actual_address.port()
                                                << ". Expected address: " << expected_address.address().ToString() << " " << expected_address.port();
...
TEST_P(QuicSessionPoolTest, MigratePortOnPathDegrading_WithoutNetworkHandle_PathValidator)
{
    socket_factory_ = std::make_unique<TestPortMigrationSocketFactory>();
    Initialize();
...
TEST_P(QuicSessionPoolTest, PortMigrationProbingReceivedStatelessReset_PathValidator)
{
    socket_factory_ = std::make_unique<TestPortMigrationSocketFactory>();
    Initialize();
    ProofVerifyDetailsChromium verify_details = DefaultProofVerifyDetails();
...
TEST_P(QuicSessionPoolTest, MigratePortOnPathDegrading_WithNetworkHandle_PathValidator)
{
    scoped_mock_network_change_notifier_ = std::make_unique<ScopedMockNetworkChangeNotifier>();
    MockNetworkChangeNotifier* mock_ncn = scoped_mock_network_change_notifier_->mock_network_change_notifier();
    mock_ncn->ForceNetworkHandlesSupported();
...
TEST_P(QuicSessionPoolTest, MigratePortOnPathDegrading_WithMigration_PathValidator)
{
    scoped_mock_network_change_notifier_ = std::make_unique<ScopedMockNetworkChangeNotifier>();
    MockNetworkChangeNotifier* mock_ncn = scoped_mock_network_change_notifier_->mock_network_change_notifier();
    mock_ncn->ForceNetworkHandlesSupported();
...
TEST_P(QuicSessionPoolTest, TestPostNetworkOnMadeDefaultWhileConnectionMigrationFailOnUnexpectedErrorTwoDifferentSessions)
{
    scoped_mock_network_change_notifier_ = std::make_unique<ScopedMockNetworkChangeNotifier>();
    MockNetworkChangeNotifier* mock_ncn = scoped_mock_network_change_notifier_->mock_network_change_notifier();
    mock_ncn->ForceNetworkHandlesSupported();
    ...
    std::unique_ptr<QuicChromiumClientSession::Handle> handle1 = session1->CreateHandle(server1);
    std::unique_ptr<QuicChromiumClientSession::Handle> handle2 = session2->CreateHandle(server2);
    ...
    EXPECT_EQ(quic::QuicErrorCode::QUIC_CONNECTION_MIGRATION_HANDSHAKE_UNCONFIRMED, details.quic_connection_error);
...
TEST_P(QuicSessionPoolTest, TestPostNetworkMadeDefaultWhileConnectionMigrationFailBeforeHandshake)
{
    scoped_mock_network_change_notifier_ = std::make_unique<ScopedMockNetworkChangeNotifier>();
    MockNetworkChangeNotifier* mock_ncn = scoped_mock_network_change_notifier_->mock_network_change_notifier();
    mock_ncn->ForceNetworkHandlesSupported();
    ...
    crypto_client_stream_factory_.last_stream()->setHandshakeConfirmedForce(false);

    std::unique_ptr<QuicChromiumClientSession::Handle> handle = session->CreateHandle(kDefaultDestination);
    mock_ncn->NotifyNetworkDisconnected(kDefaultNetworkForTests);
...
TEST_P(QuicSessionPoolTest, TestPostNetworkOnMadeDefaultWhileConnectionMigrationFailOnNoActiveStreams)
{
    scoped_mock_network_change_notifier_ = std::make_unique<ScopedMockNetworkChangeNotifier>();
    MockNetworkChangeNotifier* mock_ncn = scoped_mock_network_change_notifier_->mock_network_change_notifier();
    mock_ncn->ForceNetworkHandlesSupported();
    ...
    NetErrorDetails details;
    handle->PopulateNetErrorDetails(&details);
    EXPECT_EQ(quic::QuicErrorCode::QUIC_CONNECTION_MIGRATION_NO_MIGRATABLE_STREAMS, details.quic_connection_error);
    EXPECT_EQ(false, details.quic_connection_migration_successful);
}

// See crbug/1465889 for more details on what scenario is being tested.
TEST_P(QuicSessionPoolTest, TestPostNetworkOnMadeDefaultWhileConnectionMigrationFailOnUnexpectedError)
{
    scoped_mock_network_change_notifier_ = std::make_unique<ScopedMockNetworkChangeNotifier>();
    MockNetworkChangeNotifier* mock_ncn = scoped_mock_network_change_notifier_->mock_network_change_notifier();
    mock_ncn->ForceNetworkHandlesSupported();
    ...
    quic_data.AddReadPauseForever();
    quic_data.AddWrite(SYNCHRONOUS, ConstructInitialSettingsPacket(packet_num++));
    quic_data.AddWrite(SYNCHRONOUS, ConstructGetRequestPacket(packet_num++, GetNthClientInitiatedBidirectionalStreamId(0), true));
    quic_data.AddReadPauseForever();
    ...
    NetErrorDetails details;
    handle->PopulateNetErrorDetails(&details);
    EXPECT_EQ(quic::QuicErrorCode::QUIC_CONNECTION_MIGRATION_INTERNAL_ERROR, details.quic_connection_error);
    EXPECT_EQ(false, details.quic_connection_migration_successful);
}

// See crbug/1465889 for more details on what scenario is being tested.
TEST_P(QuicSessionPoolTest, TestPostNetworkOnMadeDefaultWhileConnectionMigrationIsFailing)
{
    scoped_mock_network_change_notifier_ = std::make_unique<ScopedMockNetworkChangeNotifier>();
    MockNetworkChangeNotifier* mock_ncn = scoped_mock_network_change_notifier_->mock_network_change_notifier();
    mock_ncn->ForceNetworkHandlesSupported();
    ...
    NetErrorDetails details;
    handle->PopulateNetErrorDetails(&details);
    EXPECT_EQ(quic::QuicErrorCode::QUIC_CONNECTION_MIGRATION_TOO_MANY_CHANGES, details.quic_connection_error);
    EXPECT_EQ(false, details.quic_connection_migration_successful);
}
...
TEST_P(QuicSessionPoolTest, TestCloseSessionOnErrorLaterThenConnectionMigrationMigrateToSocket)
{
    scoped_mock_network_change_notifier_ = std::make_unique<ScopedMockNetworkChangeNotifier>();
    MockNetworkChangeNotifier* mock_ncn = scoped_mock_network_change_notifier_->mock_network_change_notifier();
    mock_ncn->ForceNetworkHandlesSupported();
    ...
    HttpRequestHeaders request_headers;
    EXPECT_EQ(OK, stream->SendRequest(request_headers, &response, callback_.callback()));
    session->CloseSessionOnErrorLater(0, quic::QUIC_TOO_MANY_RTOS, quic::ConnectionCloseBehavior::SILENT_CLOSE);
    session->MigrateToSocket(
        quic::QuicSocketAddress(), quic::QuicSocketAddress(), nullptr, std::make_unique<QuicChromiumPacketWriter>(nullptr, task_runner.get()));
}

// Regression test for https://crbug.com/1465889
// Note: This test can be deleted once every instance of
// CloseSessionOnErrorLater has been deleted.
TEST_P(QuicSessionPoolTest, TestCloseSessionOnErrorLaterThenConnectionMigrationMigrate)
{
    scoped_mock_network_change_notifier_ = std::make_unique<ScopedMockNetworkChangeNotifier>();
    MockNetworkChangeNotifier* mock_ncn = scoped_mock_network_change_notifier_->mock_network_change_notifier();
    mock_ncn->ForceNetworkHandlesSupported();
    ...
    std::unique_ptr<DatagramClientSocket> socket(factory_->CreateSocket(net_log_.net_log(), net_log_.source()));
    DatagramClientSocket* socket_ptr = socket.get();
    factory_->ConnectAndConfigureSocket(base::BindLambdaForTesting([&session, &socket](int rv) {
        session->CloseSessionOnErrorLater(0, quic::QUIC_TOO_MANY_RTOS, quic::ConnectionCloseBehavior::SILENT_CLOSE);
        // The QuicSession is closed so FinishMigrate will fail to migrate the
        // socket. Hence the callback should never be called.
        session->FinishMigrate(std::move(socket), ToIPEndPoint(session->connection()->peer_address()), true,
            base::BindLambdaForTesting([](MigrationResult result) { NOTREACHED(); }),
            /* RV = OK */ 0);
    }),
        socket_ptr, ToIPEndPoint(session->connection()->peer_address()), kNewNetworkForTests, SocketTag());
    base::RunLoop().RunUntilIdle();
}

void QuicSessionPoolTest::TestThatBlackHoleIsDisabledOnNoNewNetworkThenResumedAfterConnectingToANetwork(bool is_blackhole_disabled_after_disconnecting)
{
    scoped_mock_network_change_notifier_ = std::make_unique<ScopedMockNetworkChangeNotifier>();
    MockNetworkChangeNotifier* mock_ncn = scoped_mock_network_change_notifier_->mock_network_change_notifier();
    mock_ncn->ForceNetworkHandlesSupported();
    ...
    quic::QuicConnectionId cid_on_new_path = quic::test::TestConnectionId(12345678);
    client_maker_.set_connection_id(cid_on_new_path);
    quic_data2.AddWrite(SYNCHRONOUS, client_maker_.Packet(packet_num++).AddPingFrame().Build());
    quic_data2.AddWrite(SYNCHRONOUS, client_maker_.Packet(packet_num++).AddRetireConnectionIdFrame(/*sequence_number=*/0u).Build());

    quic_data2.AddRead(ASYNC, ConstructOkResponsePacket(1, GetNthClientInitiatedBidirectionalStreamId(0), false));
    quic_data2.AddReadPauseForever();
    quic_data2.AddWrite(SYNCHRONOUS,
        client_maker_.Packet(packet_num++)
            .AddStreamFrame(GetQpackDecoderStreamId(), /*fin=*/false, StreamCancellationQpackDecoderInstruction(0))
    ...
    if (is_blackhole_disabled_after_disconnecting) {
        EXPECT_FALSE(session->connection()->blackhole_detector().IsDetectionInProgress());
    } else {
        EXPECT_TRUE(session->connection()->blackhole_detector().IsDetectionInProgress());
    }
...
// connected to.
TEST_P(QuicSessionPoolTest, VerifyThatBlackHoleIsDisabledOnNoAvailableNetworkThenResumedAfterConnectingToNewNetwork_FeatureDisabled)
{
    TestThatBlackHoleIsDisabledOnNoNewNetworkThenResumedAfterConnectingToANetwork(false);
}

// When the feature is enabled, the blackhole detector should be disabled
// when there is no available network. resumed once a new network has been
// connected to.
TEST_P(QuicSessionPoolTest, VerifyThatBlackHoleIsDisabledOnNoAvailableNetworkThenResumedAfterConnectingToNewNetwork_FeatureEnabled)
{
    base::test::ScopedFeatureList feature_list;
    feature_list.InitWithFeatures(
        ...
        // disabled_features
        {});
    TestThatBlackHoleIsDisabledOnNoNewNetworkThenResumedAfterConnectingToANetwork(true);
}
...
void QuicSessionPoolTest::TestSimplePortMigrationOnPathDegrading()
{
    ...
    quic_data2.AddRead(ASYNC, server_maker_.Packet(1).AddPathResponseFrame().AddPaddingFrame().Build());
    // Ping packet to send after migration is completed.
    quic_data2.AddWrite(ASYNC, client_maker_.Packet(packet_number++).AddPingFrame().Build());
    quic_data2.AddRead(ASYNC, ConstructOkResponsePacket(2, GetNthClientInitiatedBidirectionalStreamId(0), false));
    quic_data2.AddReadPauseForever();
    quic_data2.AddWrite(SYNCHRONOUS,
        client_maker_.Packet(packet_number++)
...
TEST_P(QuicSessionPoolTest, MultiplePortMigrationsExceedsMaxLimit_iQUICStyle)
{
    ...
    for (int i = 0; i <= 4; i++) {
        ...
        quic_data2.AddReadPause();
        // Connectivity probe to receive from the server.
        quic_data2.AddRead(ASYNC, server_maker_.Packet(server_packet_num++).AddPathResponseFrame().AddPaddingFrame().Build());
        if (i == 0) {
            // Retire old connection id and send ping packet after migration is
            // completed.
            quic_data2.AddWrite(SYNCHRONOUS, client_maker_.Packet(packet_number++).AddRetireConnectionIdFrame(/*sequence_number=*/0u).Build());
            quic_data2.AddWrite(SYNCHRONOUS, client_maker_.Packet(packet_number++).AddPingFrame().Build());
        } else if (i != 4) {
            quic_data2.AddWrite(SYNCHRONOUS,
                client_maker_.Packet(packet_number++)
                    .AddAckFrame(/*first_received=*/1,
        ...
        if (i == 4) {
            ...
            quic_data2.AddRead(SYNCHRONOUS, server_maker_.Packet(server_packet_num++).AddPathResponseFrame().AddPaddingFrame().Build());
        } else {
            quic_data2.AddRead(ASYNC, server_maker_.Packet(server_packet_num++).AddPathResponseFrame().AddPaddingFrame().Build());
        }
        ...
        if (i == 3) {
            ...
            // response.
            quic_data2.AddRead(ASYNC, server_maker_.Packet(server_packet_num++).AddPathResponseFrame().AddPaddingFrame().Build());
            quic_data2.AddWrite(SYNCHRONOUS, client_maker_.Packet(packet_number++).AddAckFrame(1, 9, 9).Build());
        }
        quic_data2.AddReadPauseForever();
        quic_data2.AddSocketDataToFactory(socket_factory_.get());
...
TEST_P(QuicSessionPoolTest, MigratePortOnPathDegrading_MigrateIdleSession_PathValidator)
{
    scoped_mock_network_change_notifier_ = std::make_unique<ScopedMockNetworkChangeNotifier>();
    MockNetworkChangeNotifier* mock_ncn = scoped_mock_network_change_notifier_->mock_network_change_notifier();
    mock_ncn->ForceNetworkHandlesSupported();
    ...
    quic_data1.AddRead(ASYNC,
        ConstructOkResponsePacket(1, GetNthClientInitiatedBidirectionalStreamId(0),
            /*fin = */ true));
    quic_data1.AddReadPauseForever();
    quic_data1.AddSocketDataToFactory(socket_factory_.get());
    ...
    quic_data2.AddReadPause();
    // Connectivity probe to receive from the server.
    quic_data2.AddRead(ASYNC, server_maker_.Packet(2).AddPathResponseFrame().AddPaddingFrame().Build());
    quic_data2.AddReadPauseForever();
    // Ping packet to send after migration is completed.
    quic_data2.AddWrite(ASYNC,
        client_maker_.Packet(packet_number++)
...
TEST_P(QuicSessionPoolTest, DoNotMigrateToBadSocketOnPathDegrading)
{
    ...
    quic_data.AddRead(ASYNC, ConstructOkResponsePacket(1, GetNthClientInitiatedBidirectionalStreamId(0), false));
    quic_data.AddReadPauseForever();
...
void QuicSessionPoolTest::TestMigrateSessionWithDrainingStream(IoMode write_mode_for_queued_packet)
{
    InitializeConnectionMigrationV2Test({ kDefaultNetworkForTests, kNewNetworkForTests });
    ProofVerifyDetailsChromium verify_details = DefaultProofVerifyDetails();
    crypto_client_stream_factory_.AddProofVerifyDetails(&verify_details);
    ...
    quic_data1.AddRead(ASYNC, ConstructOkResponsePacket(2, GetNthClientInitiatedBidirectionalStreamId(0),
    ...
    quic_data2.AddReadPause();
    // Connectivity probe to receive from the server.
    quic_data2.AddRead(ASYNC, server_maker_.Packet(3).AddPathResponseFrame().AddPaddingFrame().Build());
    // Ping packet to send after migration is completed.
    ...
    server_maker_.Reset();
    quic_data2.AddRead(ASYNC, ConstructOkResponsePacket(1, GetNthClientInitiatedBidirectionalStreamId(0), false));
    quic_data2.AddWrite(SYNCHRONOUS, client_maker_.Packet(packet_number++).AddAckFrame(1, 3, 1).Build());
    quic_data2.AddReadPauseForever();
    quic_data2.AddSocketDataToFactory(socket_factory_.get());
...
TEST_P(QuicSessionPoolTest, MigrateOnNewNetworkConnectAfterPathDegrading)
{
    ...
    quic_data2.AddWrite(ASYNC,
        client_maker_.MakeCombinedRetransmissionPacket(
            /*original_packet_numbers=*/ { 1, 2 }, packet_num++));
    quic_data2.AddWrite(SYNCHRONOUS, client_maker_.Packet(packet_num++).AddRetireConnectionIdFrame(0u).Build());
    quic_data2.AddRead(ASYNC, ConstructOkResponsePacket(2, GetNthClientInitiatedBidirectionalStreamId(0), false));
    quic_data2.AddReadPauseForever();
    quic_data2.AddWrite(SYNCHRONOUS,
        client_maker_.Packet(packet_num++)
...
// migration signal.
TEST_P(QuicSessionPoolTest, MigrateMultipleSessionsToBadSocketsAfterDisconnected)
{
    InitializeConnectionMigrationV2Test({ kDefaultNetworkForTests });
...
TEST_P(QuicSessionPoolTest, MigrateSessionEarlyNonMigratableStream_DoNotMigrateIdleSessions)
{
    TestMigrateSessionEarlyNonMigratableStream(false);
}

TEST_P(QuicSessionPoolTest, MigrateSessionEarlyNonMigratableStream_MigrateIdleSessions)
{
    TestMigrateSessionEarlyNonMigratableStream(true);
}

void QuicSessionPoolTest::TestMigrateSessionEarlyNonMigratableStream(bool migrate_idle_sessions)
{
    quic_params_->migrate_idle_sessions = migrate_idle_sessions;
    InitializeConnectionMigrationV2Test({ kDefaultNetworkForTests, kNewNetworkForTests });
    ProofVerifyDetailsChromium verify_details = DefaultProofVerifyDetails();
...
TEST_P(QuicSessionPoolTest, MigrateSessionOnAsyncWriteError)
{
    ...
    quic::QuicConnectionId cid_on_new_path = quic::test::TestConnectionId(12345678);
    client_maker_.set_connection_id(cid_on_new_path);
    ConstructGetRequestPacket(packet_num++, GetNthClientInitiatedBidirectionalStreamId(0), true);
    quiche::HttpHeaderBlock headers = client_maker_.GetRequestHeaders("GET", "https", "/");
    spdy::SpdyPriority priority = ConvertRequestPriorityToQuicPriority(DEFAULT_PRIORITY);
    size_t spdy_headers_frame_len;
    socket_data1.AddWrite(SYNCHRONOUS,
        client_maker_.MakeRetransmissionAndRequestHeadersPacket(
            { 1, 2 }, packet_num++, GetNthClientInitiatedBidirectionalStreamId(1), true, priority, std::move(headers), &spdy_headers_frame_len));
    socket_data1.AddWrite(SYNCHRONOUS, client_maker_.Packet(packet_num++).AddPingFrame().Build());
    socket_data1.AddWrite(SYNCHRONOUS, client_maker_.Packet(packet_num++).AddRetireConnectionIdFrame(/*sequence_number=*/0u).Build());
    socket_data1.AddRead(ASYNC, ConstructOkResponsePacket(1, GetNthClientInitiatedBidirectionalStreamId(0), false));
    socket_data1.AddReadPauseForever();
    socket_data1.AddWrite(SYNCHRONOUS,
        client_maker_.Packet(packet_num++)
...
TEST_P(QuicSessionPoolTest, MigrateBackToDefaultPostMigrationOnWriteError)
{
    ...
    quic_data2.AddRead(ASYNC,
        server_maker_.Packet(peer_packet_num++)
            ...
            .AddNewConnectionIdFrame(cid2,
                ...
                /*retire_prior_to=*/1u)
            .Build());
    quic_data2.AddRead(ASYNC, ConstructOkResponsePacket(peer_packet_num++, GetNthClientInitiatedBidirectionalStreamId(0), false));
    quic_data2.AddReadPauseForever();
    quic_data2.AddSocketDataToFactory(socket_factory_.get());
    ...
    // Connectivity probe to be sent on the new path.
    quic_data3.AddWrite(SYNCHRONOUS, client_maker_.Packet(packet_num++).AddPathChallengeFrame().AddPaddingFrame().Build());
    // Connectivity probe to receive from the server.
    quic_data3.AddRead(ASYNC, server_maker_.Packet(peer_packet_num++).AddPathResponseFrame().AddPaddingFrame().Build());
    quic_data3.AddReadPauseForever();
...
TEST_P(QuicSessionPoolTest, NoMigrationOnPathDegradingBeforeHandshakeConfirmed)
{
    FLAGS_quic_enable_chaos_protection = false;
    InitializeConnectionMigrationV2Test({ kDefaultNetworkForTests, kNewNetworkForTests });
...
void QuicSessionPoolTest::TestNoAlternateNetworkBeforeHandshake(quic::QuicErrorCode quic_error)
{
    FLAGS_quic_enable_chaos_protection = false;
...
void QuicSessionPoolTest::TestNewConnectionOnAlternateNetworkBeforeHandshake(quic::QuicErrorCode quic_error)
{
    DCHECK(quic_error == quic::QUIC_NETWORK_IDLE_TIMEOUT || quic_error == quic::QUIC_HANDSHAKE_TIMEOUT);
    FLAGS_quic_enable_chaos_protection = false;
    ...
    socket_data2.AddWrite(ASYNC, ConstructGetRequestPacket(packet_num++, GetNthClientInitiatedBidirectionalStreamId(0), true));
    socket_data2.AddRead(ASYNC, ConstructOkResponsePacket(1, GetNthClientInitiatedBidirectionalStreamId(0), false));
    socket_data2.AddReadPauseForever();
    int probing_packet_num = packet_num++;
    socket_data2.AddWrite(SYNCHRONOUS, client_maker_.Packet(packet_num++).AddRetireConnectionIdFrame(/*sequence_number=*/1u).Build());
    socket_data2.AddWrite(SYNCHRONOUS,
        client_maker_.Packet(packet_num++)
            .AddStreamFrame(GetQpackDecoderStreamId(), /*fin=*/false, StreamCancellationQpackDecoderInstruction(0))
            .AddStopSendingFrame(GetNthClientInitiatedBidirectionalStreamId(0), quic::QUIC_STREAM_CANCELLED)
    ...
    probing_data.AddWrite(SYNCHRONOUS, client_maker_.Packet(probing_packet_num).AddPathChallengeFrame().AddPaddingFrame().Build());
...
TEST_P(QuicSessionPoolTest, RetryConnectionOnWriteErrorBeforeHandshakeConfirmed)
{
    FLAGS_quic_enable_chaos_protection = false;
    quic_params_->retry_on_alternate_network_before_handshake = true;
    InitializeConnectionMigrationV2Test({ kDefaultNetworkForTests, kNewNetworkForTests });
...
void QuicSessionPoolTest::TestMigrationOnWriteError(IoMode write_error_mode)
{
    ...
    socket_data1.AddWrite(SYNCHRONOUS,
        client_maker_.MakeCombinedRetransmissionPacket(
            /*original_packet_numbers=*/ { 1, 2 }, packet_num++));
    socket_data1.AddWrite(ASYNC, client_maker_.Packet(packet_num++).AddPingFrame().Build());
    socket_data1.AddWrite(SYNCHRONOUS, client_maker_.Packet(packet_num++).AddRetireConnectionIdFrame(/*sequence_number=*/0u).Build());
    socket_data1.AddRead(ASYNC, ConstructOkResponsePacket(1, GetNthClientInitiatedBidirectionalStreamId(0), false));
    socket_data1.AddReadPauseForever();
    socket_data1.AddWrite(SYNCHRONOUS,
...
void QuicSessionPoolTest::TestMigrationOnWriteErrorNoNewNetwork(IoMode write_error_mode)
{
    InitializeConnectionMigrationV2Test({ kDefaultNetworkForTests });
    ProofVerifyDetailsChromium verify_details = DefaultProofVerifyDetails();
    crypto_client_stream_factory_.AddProofVerifyDetails(&verify_details);
...
TEST_P(QuicSessionPoolTest, MigrateSessionOnWriteErrorWithMultipleRequestsSync)
{
    TestMigrationOnWriteErrorWithMultipleRequests(SYNCHRONOUS);
}

TEST_P(QuicSessionPoolTest, MigrateSessionOnWriteErrorWithMultipleRequestsAsync)
{
...
// migratable streams.
void QuicSessionPoolTest::TestMigrationOnWriteErrorWithMultipleRequests(IoMode write_error_mode)
{
    InitializeConnectionMigrationV2Test({ kDefaultNetworkForTests, kNewNetworkForTests });
    ProofVerifyDetailsChromium verify_details = DefaultProofVerifyDetails();
    crypto_client_stream_factory_.AddProofVerifyDetails(&verify_details);
...
// error.
void QuicSessionPoolTest::TestMigrationOnWriteErrorMixedStreams(IoMode write_error_mode)
{
    InitializeConnectionMigrationV2Test({ kDefaultNetworkForTests, kNewNetworkForTests });
    ProofVerifyDetailsChromium verify_details = DefaultProofVerifyDetails();
    crypto_client_stream_factory_.AddProofVerifyDetails(&verify_details);
    ...
    socket_data1.AddWrite(SYNCHRONOUS,
        client_maker_.Packet(packet_number++)
            ...
            .AddStreamFrame(GetQpackDecoderStreamId(), /*fin=*/false, StreamCancellationQpackDecoderInstruction(1))
            .Build());
    socket_data1.AddWrite(SYNCHRONOUS, client_maker_.Packet(packet_number++).AddPingFrame().Build());
    socket_data1.AddWrite(SYNCHRONOUS, client_maker_.Packet(packet_number++).AddRetireConnectionIdFrame(/*sequence_number=*/0u).Build());
    socket_data1.AddRead(ASYNC, ConstructOkResponsePacket(1, GetNthClientInitiatedBidirectionalStreamId(0), false));
    socket_data1.AddReadPauseForever();
    socket_data1.AddWrite(SYNCHRONOUS,
        client_maker_.Packet(packet_number++)
            .AddStreamFrame(GetQpackDecoderStreamId(), /*fin=*/false, StreamCancellationQpackDecoderInstruction(0, false))
            .AddStopSendingFrame(GetNthClientInitiatedBidirectionalStreamId(0), quic::QUIC_STREAM_CANCELLED)
            .AddRstStreamFrame(GetNthClientInitiatedBidirectionalStreamId(0), quic::QUIC_STREAM_CANCELLED)
            .Build());
    socket_data1.AddSocketDataToFactory(socket_factory_.get());
...
// error.
void QuicSessionPoolTest::TestMigrationOnWriteErrorMixedStreams2(IoMode write_error_mode)
{
    InitializeConnectionMigrationV2Test({ kDefaultNetworkForTests, kNewNetworkForTests });
    ProofVerifyDetailsChromium verify_details = DefaultProofVerifyDetails();
    crypto_client_stream_factory_.AddProofVerifyDetails(&verify_details);
    ...
    base::RepeatingCallback<bool(const quic::QuicFrame&)> control_stream_only = FilterControlStreamOnly();
    quic::QuicStreamId stream_1 = GetNthClientInitiatedBidirectionalStreamId(1);
    socket_data1.AddWrite(SYNCHRONOUS,
        client_maker_.Packet(packet_number++)
    ...
    socket_data1.AddWrite(SYNCHRONOUS, client_maker_.Packet(packet_number++).AddPingFrame().Build());
    socket_data1.AddWrite(SYNCHRONOUS, client_maker_.Packet(packet_number++).AddRetireConnectionIdFrame(/*sequence_number=*/0u).Build());
    socket_data1.AddWrite(SYNCHRONOUS, ConstructGetRequestPacket(packet_number++, GetNthClientInitiatedBidirectionalStreamId(0), true));
    socket_data1.AddRead(ASYNC, ConstructOkResponsePacket(1, GetNthClientInitiatedBidirectionalStreamId(0), false));
    socket_data1.AddReadPauseForever();
    socket_data1.AddWrite(SYNCHRONOUS,
        client_maker_.Packet(packet_number++)
...
void QuicSessionPoolTest::TestMigrationOnWriteErrorNonMigratableStream(IoMode write_error_mode, bool migrate_idle_sessions)
{
    quic_params_->migrate_idle_sessions = migrate_idle_sessions;
    InitializeConnectionMigrationV2Test({ kDefaultNetworkForTests, kNewNetworkForTests });
    ProofVerifyDetailsChromium verify_details = DefaultProofVerifyDetails();
    ...
    if (migrate_idle_sessions) {
        ...
        socket_data.AddWrite(SYNCHRONOUS,
            client_maker_.Packet(packet_num++)
                ...
                .Build());
        socket_data.AddWrite(SYNCHRONOUS, client_maker_.Packet(packet_num++).AddPingFrame().Build());
        socket_data.AddWrite(SYNCHRONOUS, client_maker_.Packet(packet_num++).AddRetireConnectionIdFrame(/*sequence_number=*/0u).Build());
        socket_data.AddSocketDataToFactory(socket_factory_.get());
    } else {
        socket_data.AddReadPauseForever();
        socket_data.AddWrite(SYNCHRONOUS, ConstructInitialSettingsPacket());
...
TEST_P(QuicSessionPoolTest, MigrateSessionOnWriteErrorNonMigratableStreamSync_DoNotMigrateIdleSessions)
{
    TestMigrationOnWriteErrorNonMigratableStream(SYNCHRONOUS, false);
}

TEST_P(QuicSessionPoolTest, MigrateSessionOnWriteErrorNonMigratableStreamAsync_DoNotMigrateIdleSessions)
{
    TestMigrationOnWriteErrorNonMigratableStream(ASYNC, false);
}

TEST_P(QuicSessionPoolTest, MigrateSessionOnWriteErrorNonMigratableStreamSync_MigrateIdleSessions)
{
    TestMigrationOnWriteErrorNonMigratableStream(SYNCHRONOUS, true);
}

TEST_P(QuicSessionPoolTest, MigrateSessionOnWriteErrorNonMigratableStreamAsync_MigrateIdleSessions)
{
    TestMigrationOnWriteErrorNonMigratableStream(ASYNC, true);
}

void QuicSessionPoolTest::TestMigrationOnWriteErrorMigrationDisabled(IoMode write_error_mode)
{
    InitializeConnectionMigrationV2Test({ kDefaultNetworkForTests, kNewNetworkForTests });
    ProofVerifyDetailsChromium verify_details = DefaultProofVerifyDetails();
    crypto_client_stream_factory_.AddProofVerifyDetails(&verify_details);
...
TEST_P(QuicSessionPoolTest, MigrateSessionOnWriteErrorMigrationDisabledSynchronous)
{
...
void QuicSessionPoolTest::TestMigrationOnMultipleWriteErrors(IoMode write_error_mode_on_old_network, IoMode write_error_mode_on_new_network)
{
    InitializeConnectionMigrationV2Test({ kDefaultNetworkForTests, kNewNetworkForTests });
    ProofVerifyDetailsChromium verify_details = DefaultProofVerifyDetails();
    crypto_client_stream_factory_.AddProofVerifyDetails(&verify_details);
...
void QuicSessionPoolTest::TestMigrationOnNetworkNotificationWithWriteErrorQueuedLater(bool disconnected)
{
    InitializeConnectionMigrationV2Test({ kDefaultNetworkForTests, kNewNetworkForTests });
    ProofVerifyDetailsChromium verify_details = DefaultProofVerifyDetails();
    crypto_client_stream_factory_.AddProofVerifyDetails(&verify_details);
...
TEST_P(QuicSessionPoolTest, MigrateOnNetworkDisconnectedWithWriteErrorQueuedLater)
{
...
TEST_P(QuicSessionPoolTest, MigrateOnWriteErrorWithNetworkMadeDefaultQueuedEarlier)
{
    TestMigrationOnNetworkNotificationWithWriteErrorQueuedLater(
        /*disconnected=*/false);
}
...
void QuicSessionPoolTest::TestMigrationOnWriteErrorWithNotificationQueuedLater(bool disconnected)
{
    InitializeConnectionMigrationV2Test({ kDefaultNetworkForTests, kNewNetworkForTests });
    ProofVerifyDetailsChromium verify_details = DefaultProofVerifyDetails();
    crypto_client_stream_factory_.AddProofVerifyDetails(&verify_details);
...
//   completed.
TEST_P(QuicSessionPoolTest, MigrateOnWriteErrorWithNetworkMadeDefaultQueuedLater)
{
...
//   completed.
TEST_P(QuicSessionPoolTest, MigrateOnWriteErrorWithNetworkDisconnectedQueuedLater)
{
...
// - new network is made default.
void QuicSessionPoolTest::TestMigrationOnWriteErrorPauseBeforeConnected(IoMode write_error_mode)
{
    InitializeConnectionMigrationV2Test({ kDefaultNetworkForTests });
    ProofVerifyDetailsChromium verify_details = DefaultProofVerifyDetails();
    crypto_client_stream_factory_.AddProofVerifyDetails(&verify_details);
    ...
    socket_data1.AddWrite(
        SYNCHRONOUS, client_maker_.Packet(packet_num++).AddPacketRetransmission(1).AddPacketRetransmission(2).AddRetireConnectionIdFrame(0).Build());
    socket_data1.AddWrite(SYNCHRONOUS, client_maker_.Packet(packet_num++).AddPingFrame().Build());
    socket_data1.AddWrite(SYNCHRONOUS,
        client_maker_.Packet(packet_num++)
            .AddStreamFrame(GetQpackDecoderStreamId(), /*fin=*/false, StreamCancellationQpackDecoderInstruction(0))
...
TEST_P(QuicSessionPoolTest, MigrateSessionOnSyncWriteErrorPauseBeforeConnected)
{
    TestMigrationOnWriteErrorPauseBeforeConnected(SYNCHRONOUS);
}

TEST_P(QuicSessionPoolTest, MigrateSessionOnAsyncWriteErrorPauseBeforeConnected)
{
...
TEST_P(QuicSessionPoolTest, IgnoreWriteErrorFromOldWriterAfterMigration)
{
    ...
    socket_data.AddWrite(ASYNC, ERR_ADDRESS_UNREACHABLE, ConstructGetRequestPacket(packet_num++, GetNthClientInitiatedBidirectionalStreamId(0), true));
    socket_data.AddReadPauseForever();
    socket_data.AddSocketDataToFactory(socket_factory_.get());
...
TEST_P(QuicSessionPoolTest, IgnoreReadErrorFromOldReaderAfterMigration)
{
    ...
    MockQuicData socket_data1(version_);
    client_maker_.set_connection_id(cid_on_new_path);
    socket_data1.AddWrite(SYNCHRONOUS, client_maker_.MakeRetransmissionPacket(1, packet_num++));
    socket_data1.AddWrite(SYNCHRONOUS, client_maker_.Packet(packet_num++).AddPingFrame().Build());
    socket_data1.AddWrite(SYNCHRONOUS, ConstructGetRequestPacket(packet_num++, GetNthClientInitiatedBidirectionalStreamId(0), true));
    socket_data1.AddWrite(SYNCHRONOUS, client_maker_.Packet(packet_num++).AddRetireConnectionIdFrame(/*sequence_number=*/0u).Build());
    socket_data1.AddRead(ASYNC, ConstructOkResponsePacket(1, GetNthClientInitiatedBidirectionalStreamId(0), false));
    socket_data1.AddReadPauseForever();
    socket_data1.AddWrite(SYNCHRONOUS,
...
TEST_P(QuicSessionPoolTest, DefaultRetransmittableOnWireTimeoutForMigration)
{
    ...
    auto task_runner = base::MakeRefCounted<base::TestMockTimeTaskRunner>();
    QuicSessionPoolPeer::SetTaskRunner(factory_.get(), task_runner.get());
    QuicSessionPoolPeer::SetAlarmFactory(factory_.get(), std::make_unique<QuicChromiumAlarmFactory>(task_runner.get(), context_.clock()));
    ...
    socket_data.AddRead(ASYNC,
        server_maker_.Packet(peer_packet_num++)
            .AddNewConnectionIdFrame(cid_on_new_path, /*sequence_number=*/1u,
                /*retire_prior_to=*/0u)
            .Build());
    socket_data.AddReadPause();
    socket_data.AddRead(ASYNC, ERR_ADDRESS_UNREACHABLE);
    ...
    socket_data1.AddRead(ASYNC, ConstructOkResponsePacket(peer_packet_num++, GetNthClientInitiatedBidirectionalStreamId(0), false));
    socket_data1.AddRead(
        ASYNC, server_maker_.Packet(peer_packet_num++).AddStreamFrame(GetNthClientInitiatedBidirectionalStreamId(0), false, "Hello World").Build());

    // Read an ACK from server which acks all client data.
    socket_data1.AddRead(SYNCHRONOUS, server_maker_.Packet(peer_packet_num++).AddAckFrame(1, packet_num, 1).Build());
    socket_data1.AddWrite(ASYNC, client_maker_.Packet(packet_num++).AddAckFrame(1, peer_packet_num - 2, 1).Build());
    // The PING packet sent for retransmittable on wire.
    socket_data1.AddWrite(SYNCHRONOUS, client_maker_.Packet(packet_num++).AddPingFrame().Build());
    socket_data1.AddReadPause();
    std::string header = ConstructDataHeader(6);
    socket_data1.AddRead(ASYNC, ConstructServerDataPacket(3, GetNthClientInitiatedBidirectionalStreamId(0), true, header + "hello!"));
    socket_data1.AddReadPauseForever();
    socket_data1.AddWrite(SYNCHRONOUS,
        client_maker_.Packet(packet_num++)
            .AddStreamFrame(GetQpackDecoderStreamId(), /*fin=*/false, StreamCancellationQpackDecoderInstruction(0))
...
TEST_P(QuicSessionPoolTest, CustomRetransmittableOnWireTimeout)
{
    ...
    // Read two packets so that client will send ACK immedaitely.
    socket_data1.AddRead(ASYNC, ConstructOkResponsePacket(1, GetNthClientInitiatedBidirectionalStreamId(0), false));
    socket_data1.AddRead(ASYNC, server_maker_.Packet(2).AddStreamFrame(GetNthClientInitiatedBidirectionalStreamId(0), false, "Hello World").Build());
    // Read an ACK from server which acks all client data.
    socket_data1.AddRead(SYNCHRONOUS, server_maker_.Packet(3).AddAckFrame(1, 2, 1).Build());
    socket_data1.AddWrite(ASYNC, client_maker_.Packet(packet_num++).AddAckFrame(1, 2, 1).Build());
    // The PING packet sent for retransmittable on wire.
    socket_data1.AddWrite(SYNCHRONOUS, client_maker_.Packet(packet_num++).AddPingFrame().Build());
    socket_data1.AddReadPause();
    std::string header = ConstructDataHeader(6);
...
TEST_P(QuicSessionPoolTest, NoRetransmittableOnWireTimeout)
{
    ...
    const quic::QuicAlarmProxy ping_alarm = quic::test::QuicConnectionPeer::GetPingAlarm(session->connection());
...
TEST_P(QuicSessionPoolTest, CustomRetransmittableOnWireTimeoutWithMigrationOnNetworkChangeOnly)
{
    constexpr base::TimeDelta custom_timeout_value = base::Milliseconds(200);
    quic_params_->retransmittable_on_wire_timeout = custom_timeout_value;
    quic_params_->migrate_sessions_on_network_change_v2 = true;
...
TEST_P(QuicSessionPoolTest, NoRetransmittableOnWireTimeoutWithMigrationOnNetworkChangeOnly)
{
    // Use non-default initial srtt so that if QPACK emits additional setting
    // packet, it will not have the same retransmission timeout as the
    // default value of retransmittable-on-wire-ping timeout.
...
// read error on the old reader will be ignored and will not close the
// connection.
TEST_P(QuicSessionPoolTest, IgnoreReadErrorOnOldReaderDuringPendingMigrationOnWriteError)
{
    InitializeConnectionMigrationV2Test({ kDefaultNetworkForTests, kNewNetworkForTests });
    ProofVerifyDetailsChromium verify_details = DefaultProofVerifyDetails();
    crypto_client_stream_factory_.AddProofVerifyDetails(&verify_details);
...
// network connects.
TEST_P(QuicSessionPoolTest, MigrateSessionOnWriteErrorWithDisconnectAfterConnectAsync)
{
...
// network connects.
TEST_P(QuicSessionPoolTest, MigrateSessionOnWriteErrorWithDisconnectAfterConnectSync)
{
...
// network connects.
TEST_P(QuicSessionPoolTest, MigrateSessionOnWriteErrorWithDisconnectBeforeConnectAsync)
{
...
// network connects.
TEST_P(QuicSessionPoolTest, MigrateSessionOnWriteErrorWithDisconnectBeforeConnectSync)
{
...
void QuicSessionPoolTest::TestMigrationOnWriteErrorWithMultipleNotifications(IoMode write_error_mode, bool disconnect_before_connect)
{
    InitializeConnectionMigrationV2Test({ kDefaultNetworkForTests });
    ProofVerifyDetailsChromium verify_details = DefaultProofVerifyDetails();
    crypto_client_stream_factory_.AddProofVerifyDetails(&verify_details);
...
TEST_P(QuicSessionPoolTest, DefaultIdleMigrationPeriod)
{
    ...
    auto task_runner = base::MakeRefCounted<base::TestMockTimeTaskRunner>();
    QuicSessionPoolPeer::SetTaskRunner(factory_.get(), task_runner.get());
    QuicSessionPoolPeer::SetTickClock(factory_.get(), task_runner->GetMockTickClock());
    ...
    alternate_socket_data.AddWrite(SYNCHRONOUS,
        client_maker_.MakeAckAndRetransmissionPacket(packet_num++,
            ...
            /*original_packet_numbers=*/ { 1 }));
    alternate_socket_data.AddWrite(SYNCHRONOUS, client_maker_.Packet(packet_num++).AddPingFrame().Build());
    alternate_socket_data.AddWrite(ASYNC, client_maker_.Packet(packet_num++).AddRetireConnectionIdFrame(/*sequence_number=*/0u).Build());
...
TEST_P(QuicSessionPoolTest, SharedCryptoConfig)
{
    ...
    for (const auto& cannoncial_suffix : cannoncial_suffixes) {
        ...
        std::unique_ptr<QuicCryptoClientConfigHandle> crypto_config_handle = QuicSessionPoolPeer::GetCryptoConfig(factory_.get(), NetworkAnonymizationKey());
        quic::QuicServerId server_id1(scheme_host_port1.host(), scheme_host_port1.port());
        ...
        quic::QuicServerId server_id2(scheme_host_port2.host(), scheme_host_port2.port());
...
TEST_P(QuicSessionPoolTest, ReducePingTimeoutOnConnectionTimeOutOpenStreams)
{
    ...
    // with open stream.
    EXPECT_EQ(quic::QuicTime::Delta::FromSeconds(quic::kPingTimeoutSecs), QuicSessionPoolPeer::GetPingTimeout(factory_.get()));
    RequestBuilder builder(this);
    EXPECT_EQ(ERR_IO_PENDING, builder.CallRequest());
    EXPECT_THAT(callback_.WaitForResult(), IsOk());
    ...
    EXPECT_EQ(quic::QuicTime::Delta::FromSeconds(10), QuicSessionPoolPeer::GetPingTimeout(factory_.get()));
...
TEST_P(QuicSessionPoolTest, CryptoConfigCache)
{
    ...
    const auto kNetworkAnonymizationKey3 = NetworkAnonymizationKey::CreateSameSite(kSite3);

    Initialize();

    // Create a QuicCryptoClientConfigHandle for kNetworkAnonymizationKey1, and
    // set the user agent.
    std::unique_ptr<QuicCryptoClientConfigHandle> crypto_config_handle1 = QuicSessionPoolPeer::GetCryptoConfig(factory_.get(), kNetworkAnonymizationKey1);
    ...
    std::unique_ptr<QuicCryptoClientConfigHandle> crypto_config_handle2 = QuicSessionPoolPeer::GetCryptoConfig(factory_.get(), kNetworkAnonymizationKey2);
    ...
    std::unique_ptr<QuicCryptoClientConfigHandle> crypto_config_handle3 = QuicSessionPoolPeer::GetCryptoConfig(factory_.get(), kNetworkAnonymizationKey3);
...
TEST_P(QuicSessionPoolTest, CryptoConfigCacheWithNetworkAnonymizationKey)
{
    ...
    std::unique_ptr<QuicCryptoClientConfigHandle> crypto_config_handle1_2 = QuicSessionPoolPeer::GetCryptoConfig(factory_.get(), kNetworkAnonymizationKey1);
    std::unique_ptr<QuicCryptoClientConfigHandle> crypto_config_handle2_2 = QuicSessionPoolPeer::GetCryptoConfig(factory_.get(), kNetworkAnonymizationKey2);
    ...
    // same NAKs as before.
    crypto_config_handle2 = QuicSessionPoolPeer::GetCryptoConfig(factory_.get(), kNetworkAnonymizationKey2);
    crypto_config_handle1 = QuicSessionPoolPeer::GetCryptoConfig(factory_.get(), kNetworkAnonymizationKey1);
    crypto_config_handle3 = QuicSessionPoolPeer::GetCryptoConfig(factory_.get(), kNetworkAnonymizationKey3);
    EXPECT_EQ(kUserAgentId1, crypto_config_handle1->GetConfig()->user_agent_id());
    EXPECT_EQ(kUserAgentId2, crypto_config_handle2->GetConfig()->user_agent_id());
    EXPECT_EQ(kUserAgentId3, crypto_config_handle3->GetConfig()->user_agent_id());
}
...
TEST_P(QuicSessionPoolTest, CryptoConfigCacheMRUWithNetworkAnonymizationKey)
{
    ...
    for (int i = 0; i < kNumSessionsToMake; ++i) {
        ...
        network_anonymization_keys.emplace_back(NetworkAnonymizationKey::CreateSameSite(site));

        std::unique_ptr<QuicCryptoClientConfigHandle> crypto_config_handle
            = QuicSessionPoolPeer::GetCryptoConfig(factory_.get(), network_anonymization_keys[i]);
...
TEST_P(QuicSessionPoolTest, CryptoConfigCacheMRUWithRealRequestsAndWithNetworkAnonymizationKey)
{
    const int kNumSessionsToMake = kMaxRecentCryptoConfigs + 5;
    ...
    const quic::QuicServerId kQuicServerId(kDefaultServerHostName, kDefaultServerPort);
    ...
    for (int i = 0; i < kNumSessionsToMake; ++i) {
        ...
        base::Time expiration = base::Time::Now() + base::Days(1);
        alternative_service_info_vector.push_back(AlternativeServiceInfo::CreateQuicAlternativeServiceInfo(alternative_service1, expiration, { version_ }));
        http_server_properties_->SetAlternativeServices(url::SchemeHostPort(GURL(kDefaultUrl)), network_anonymization_keys[i], alternative_service_info_vector);

        http_server_properties_->SetMaxServerConfigsStoredInProperties(kDefaultMaxQuicServerEntries);

        std::unique_ptr<QuicServerInfo> quic_server_info = std::make_unique<PropertiesBasedQuicServerInfo>(
            kQuicServerId, PRIVACY_MODE_DISABLED, network_anonymization_keys[i], http_server_properties_.get());
        ...
        RequestBuilder builder(this);
        builder.destination = url::SchemeHostPort(url::kHttpsScheme, kDefaultServerHostName, kDefaultServerPort);
        ...
        for (int j = 0; j < kNumSessionsToMake; ++j) {
            ...
            EXPECT_EQ(i - (kMaxRecentCryptoConfigs + 1) < j && j <= i,
                !QuicSessionPoolPeer::CryptoConfigCacheIsEmpty(factory_.get(), kQuicServerId, network_anonymization_keys[j]));
        }
...
TEST_P(QuicSessionPoolTest, YieldAfterDuration)
{
    ...
    ProofVerifyDetailsChromium verify_details = DefaultProofVerifyDetails();
    crypto_client_stream_factory_.AddProofVerifyDetails(&verify_details);
    QuicSessionPoolPeer::SetYieldAfterDuration(factory_.get(), quic::QuicTime::Delta::FromMilliseconds(-1));
...
TEST_P(QuicSessionPoolTest, PoolByOrigin)
{
    ...
    EXPECT_EQ(quic::QuicServerId(kDefaultServerHostName, kDefaultServerPort), session1->server_id());
...
class QuicSessionPoolWithDestinationTest : public QuicSessionPoolTestBase, public ::testing::TestWithParam<PoolingTestParams> {
protected:
    ...
    {
        auto sequenced_socket_data = std::make_unique<SequencedSocketData>(base::span_from_ref(hanging_read_), base::span<MockWrite>());
...
INSTANTIATE_TEST_SUITE_P(VersionIncludeStreamDependencySequence, QuicSessionPoolWithDestinationTest, ::testing::ValuesIn(GetPoolingTestParams()),
...
TEST_P(QuicSessionPoolWithDestinationTest, SharedCertificate)
{
    ...
    EXPECT_EQ(quic::QuicServerId(origin1_.host(), origin1_.port()), session1->server_id());
...
TEST_P(QuicSessionPoolWithDestinationTest, DifferentPrivacyMode)
{
    ...
    EXPECT_EQ(quic::QuicServerId(origin1_.host(), origin1_.port()), session1->server_id());
    EXPECT_EQ(quic::QuicServerId(origin2_.host(), origin2_.port()), session2->server_id());
...
TEST_P(QuicSessionPoolWithDestinationTest, DifferentProxyChain)
{
    ...
    QuicTestPacketMaker endpoint_maker1(version_, quic::QuicUtils::CreateRandomConnectionId(context_.random_generator()), context_.clock(), origin1_.host(),
    ...
    socket_data1.AddWrite(SYNCHRONOUS, ConstructConnectUdpRequestPacket(2, stream_id, proxy1.host(), "/.well-known/masque/udp/www.example.org/443/", false));
    ...
    socket_data1.AddReadPauseForever();
    socket_data1.AddWrite(ASYNC, client_maker_.Packet(3).AddAckFrame(3, 4, 3).Build());
    socket_data1.AddWrite(ASYNC, ConstructClientH3DatagramPacket(4, stream_id, kConnectUdpContextId, endpoint_maker1.MakeInitialSettingsPacket(1)));
    socket_data1.AddSocketDataToFactory(socket_factory_.get());

    QuicTestPacketMaker endpoint_maker2(version_, quic::QuicUtils::CreateRandomConnectionId(context_.random_generator()), context_.clock(), origin2_.host(),
        quic::Perspective::IS_CLIENT,
        /*client_priority_uses_incremental=*/true,
        /*use_priority_header=*/true);
    client_maker_.Reset();
    ...
    socket_data2.AddWrite(SYNCHRONOUS, ConstructConnectUdpRequestPacket(2, stream_id, proxy2.host(), "/.well-known/masque/udp/mail.example.org/443/", false));
    ...
    socket_data2.AddReadPauseForever();
    socket_data2.AddWrite(ASYNC, client_maker_.Packet(3).AddAckFrame(3, 4, 3).Build());
    socket_data2.AddWrite(ASYNC, ConstructClientH3DatagramPacket(4, stream_id, kConnectUdpContextId, endpoint_maker2.MakeInitialSettingsPacket(1)));
    socket_data2.AddSocketDataToFactory(socket_factory_.get());

    auto proxy_chain1 = ProxyChain::ForIpProtection({
        ProxyServer::FromSchemeHostAndPort(ProxyServer::SCHEME_QUIC, proxy1_origin.host(), 443),
    });
    EXPECT_TRUE(proxy_chain1.IsValid());

    auto proxy_chain2 = ProxyChain::ForIpProtection({
        ProxyServer::FromSchemeHostAndPort(ProxyServer::SCHEME_QUIC, proxy2_origin.host(), 443),
    });
    ...
    std::unique_ptr<HttpStream> stream1 = CreateStream(&builder1.request);
    EXPECT_TRUE(stream1.get());
    EXPECT_TRUE(HasActiveSession(origin1_, PRIVACY_MODE_DISABLED, NetworkAnonymizationKey(), proxy_chain1));
...
TEST_P(QuicSessionPoolTest, ClearCachedStatesInCryptoConfig)
{
    ...
    struct TestCase {
        TestCase(const std::string& host, int port, quic::QuicCryptoClientConfig* crypto_config)
            : server_id(host, port)
    ...
    } test_cases[] = { TestCase("www.google.com", 443, crypto_config_handle->GetConfig()), TestCase("www.example.com", 443, crypto_config_handle->GetConfig()),
...
TEST_P(QuicSessionPoolTest, HostResolverUsesParams)
{
    const SchemefulSite kSite1(GURL("https://foo.test/"));
    const SchemefulSite kSite2(GURL("https://bar.test/"));
    const auto kNetworkAnonymizationKey = NetworkAnonymizationKey::CreateSameSite(kSite1);
    base::test::ScopedFeatureList feature_list;
    feature_list.InitAndEnableFeature(features::kPartitionConnectionsByNetworkIsolationKey);
...
TEST_P(QuicSessionPoolTest, ResultAfterQuicSessionCreationCallbackFail)
{
    ...
    EXPECT_TRUE(builder.request.WaitForQuicSessionCreation(quic_session_callback.callback()));
    ...
    EXPECT_FALSE(builder.request.WaitForQuicSessionCreation(quic_session_callback.callback()));
...
TEST_P(QuicSessionPoolTest, ResultAfterQuicSessionCreationCallbackSuccessAsync)
{
    Initialize();
    ProofVerifyDetailsChromium verify_details = DefaultProofVerifyDetails();
    crypto_client_stream_factory_.AddProofVerifyDetails(&verify_details);
...
TEST_P(QuicSessionPoolTest, ResultAfterHostResolutionCallbackAsyncSync)
{
    ...
    EXPECT_TRUE(builder.request.WaitForHostResolution(host_resolution_callback.callback()));
    ...
    EXPECT_FALSE(builder.request.WaitForHostResolution(host_resolution_callback.callback()));
...
TEST_P(QuicSessionPoolTest, Tag)
{
    ...
    std::unique_ptr<QuicChromiumClientSession::Handle> stream1 = builder1.request.ReleaseSessionHandle();
    EXPECT_TRUE(stream1);
    ...
    EXPECT_THAT(callback_.GetResult(rv), IsOk());
    std::unique_ptr<QuicChromiumClientSession::Handle> stream2 = builder2.request.ReleaseSessionHandle();
    EXPECT_TRUE(stream2);
    ...
    EXPECT_TRUE(socket_factory->GetLastProducedUDPSocket()->tagged_before_data_transferred());
    std::unique_ptr<QuicChromiumClientSession::Handle> stream3 = builder3.request.ReleaseSessionHandle();
    EXPECT_TRUE(stream3);
...
{
    std::vector<std::string> dns_aliases({ "alias1", "alias2", kDefaultServerHostName });
    host_resolver_->rules()->AddIPLiteralRuleWithDnsAliases(kDefaultServerHostName, "192.168.0.1", std::move(dns_aliases));
    ...
    EXPECT_THAT(stream->GetDnsAliases(), testing::ElementsAre("alias1", "alias2", kDefaultServerHostName));
}
...
TEST_P(QuicSessionPoolTest, NoAdditionalDnsAliases)
{
    ...
    EXPECT_THAT(stream->GetDnsAliases(), testing::ElementsAre(kDefaultServerHostName));
}
...
TEST_P(QuicSessionPoolTest, RequireDnsHttpsAlpnMatch)
{
    ...
    endpoints[0].ip_endpoints = { IPEndPoint(IPAddress::IPv4Localhost(), 0) };
    endpoints[0].metadata.supported_protocol_alpns = { quic::AlpnForVersion(version_) };
...
TEST_P(QuicSessionPoolTest, RequireDnsHttpsAlpnUnknownAndSupportedAlpn)
{
    std::vector<HostResolverEndpointResult> endpoints(2);
    endpoints[0].ip_endpoints = { IPEndPoint(IPAddress::IPv4Localhost(), 0) };
    endpoints[0].metadata.supported_protocol_alpns = { "unknown", quic::AlpnForVersion(version_) };
    // Add a final non-protocol endpoint at the end.
    endpoints[1].ip_endpoints = { IPEndPoint(IPAddress::IPv4Localhost(), 0) };
    TestRequireDnsHttpsAlpn(std::move(endpoints), /*expect_success=*/true);
}
...
TEST_P(QuicSessionPoolTest, RequireDnsHttpsNotAlpnName)
{
    std::vector<HostResolverEndpointResult> endpoints(2);
    endpoints[0].ip_endpoints = { IPEndPoint(IPAddress::IPv4Localhost(), 0) };
    endpoints[0].metadata.supported_protocol_alpns = { quic::ParsedQuicVersionToString(version_) };
    // Add a final non-protocol endpoint at the end.
    endpoints[1].ip_endpoints = { IPEndPoint(IPAddress::IPv4Localhost(), 0) };
    TestRequireDnsHttpsAlpn(std::move(endpoints), /*expect_success=*/false);
}
...
void QuicSessionPoolTest::TestRequireDnsHttpsAlpn(std::vector<HostResolverEndpointResult> endpoints, bool expect_success)
{
    quic_params_->supported_versions = { version_ };
    host_resolver_ = std::make_unique<MockHostResolver>();
    host_resolver_->rules()->AddRule(kDefaultServerHostName,
    ...
    if (expect_success) {
        ...
        EXPECT_THAT(callback_.WaitForResult(), IsError(ERR_DNS_NO_MATCHING_SUPPORTED_ALPN));
    }
}
...
{
    return base::StrCat({ ParsedQuicVersionToString(p.version), "_", (p.use_dns_aliases ? "" : "DoNot"), "UseDnsAliases_1st_", PrintToString(p.dns_aliases1),
        "_2nd_", PrintToString(p.dns_aliases2) });
}
...
std::vector<DnsAliasPoolingTestParams> GetDnsAliasPoolingTestParams()
{
    ...
    quic::ParsedQuicVersionVector all_supported_versions = AllSupportedQuicVersions();
    for (const quic::ParsedQuicVersion& version : all_supported_versions) {
        params.push_back(DnsAliasPoolingTestParams { version, false /* use_dns_aliases */, {} /* dns_aliases1 */, {} /* dns_aliases2 */,
            {} /* expected_dns_aliases1 */, {} /* expected_dns_aliases2 */ });
        params.push_back(DnsAliasPoolingTestParams { version, true /* use_dns_aliases */, {} /* dns_aliases1 */, {} /* dns_aliases2 */,
            { QuicSessionPoolTest::kDefaultServerHostName } /* expected_dns_aliases1 */,
            { QuicSessionPoolTest::kServer2HostName } /* expected_dns_aliases2 */ });
        params.push_back(DnsAliasPoolingTestParams { version, false /* use_dns_aliases */, { "alias1", "alias2", "alias3" }, {} /* dns_aliases2 */,
            {} /* expected_dns_aliases1 */, {} /* expected_dns_aliases2 */ });
        params.push_back(
            DnsAliasPoolingTestParams { version, true /* use_dns_aliases */, { "alias1", "alias2", "alias3" } /* dns_aliases1 */, {} /* dns_aliases2 */,
                { "alias1", "alias2", "alias3" } /* expected_dns_aliases1 */, { QuicSessionPoolTest::kServer2HostName } /* expected_dns_aliases2 */ });
        params.push_back(DnsAliasPoolingTestParams { version, false /* use_dns_aliases */, { "alias1", "alias2", "alias3" } /* dns_aliases1 */,
            { "alias3", "alias4", "alias5" } /* dns_aliases2 */, {} /* expected_dns_aliases1 */, {} /* expected_dns_aliases2 */ });
        params.push_back(DnsAliasPoolingTestParams { version, true /* use_dns_aliases */, { "alias1", "alias2", "alias3" } /* dns_aliases1 */,
            { "alias3", "alias4", "alias5" } /* dns_aliases2 */, { "alias1", "alias2", "alias3" } /* expected_dns_aliases1 */,
            { "alias3", "alias4", "alias5" } /* expected_dns_aliases2 */ });
        params.push_back(DnsAliasPoolingTestParams { version, false /* use_dns_aliases */, {} /* dns_aliases1 */,
            { "alias3", "alias4", "alias5" } /* dns_aliases2 */, {} /* expected_dns_aliases1 */, {} /* expected_dns_aliases2 */ });
        params.push_back(
            DnsAliasPoolingTestParams { version, true /* use_dns_aliases */, {} /* dns_aliases1 */, { "alias3", "alias4", "alias5" } /* dns_aliases2 */,
                { QuicSessionPoolTest::kDefaultServerHostName } /* expected_dns_aliases1 */, { "alias3", "alias4", "alias5" } /* expected_dns_aliases2 */ });
    }
    return params;
}
...
class QuicSessionPoolDnsAliasPoolingTest : public QuicSessionPoolTestBase, public ::testing::TestWithParam<DnsAliasPoolingTestParams> {
protected:
...
INSTANTIATE_TEST_SUITE_P(VersionIncludeStreamDependencySequence, QuicSessionPoolDnsAliasPoolingTest, ::testing::ValuesIn(GetDnsAliasPoolingTestParams()),
    ::testing::PrintToStringParamName());
...
TEST_P(QuicSessionPoolDnsAliasPoolingTest, IPPooling)
{
    ...
    std::unique_ptr<HttpStream> stream1 = CreateStream(&builder1.request);
    EXPECT_TRUE(stream1.get());
    EXPECT_TRUE(HasActiveSession(kOrigin1, PRIVACY_MODE_DISABLED, NetworkAnonymizationKey(), ProxyChain::Direct(), session_usage));
    ...
    std::unique_ptr<HttpStream> stream2 = CreateStream(&builder2.request);
    EXPECT_TRUE(stream2.get());
    EXPECT_TRUE(HasActiveSession(kOrigin2, PRIVACY_MODE_DISABLED, NetworkAnonymizationKey(), ProxyChain::Direct(), session_usage));

    QuicChromiumClientSession::Handle* session1 = QuicHttpStreamPeer::GetSessionHandle(stream1.get());
    QuicChromiumClientSession::Handle* session2 = QuicHttpStreamPeer::GetSessionHandle(stream2.get());
    EXPECT_TRUE(session1->SharesSameSession(*session2));

    EXPECT_EQ(quic::QuicServerId(kOrigin1.host(), kOrigin1.port()), session1->server_id());
...
TEST_P(QuicSessionPoolTest, EchWithQuicFromAltSvc)
{
    ...
    host_resolver_ = std::make_unique<MockHostResolver>();
    host_resolver_->rules()->AddRule(kDefaultServerHostName, MockHostResolverBase::RuleResolver::RuleResult({ endpoint }));
    ...
    ASSERT_TRUE(session);
    quic::QuicSSLConfig config = session->GetSSLConfig();
    EXPECT_EQ(std::string(endpoint.metadata.ech_config_list.begin(), endpoint.metadata.ech_config_list.end()), config.ech_config_list);
}
...
TEST_P(QuicSessionPoolTest, EchWithQuicFromHttpsRecord)
{
    ...
    QuicChromiumClientSession* session
        = GetActiveSession(kDefaultDestination, PRIVACY_MODE_DISABLED, NetworkAnonymizationKey(), ProxyChain::Direct(), SessionUsage::kDestination,
...
TEST_P(QuicSessionPoolTest, EchSvcbReliant)
{
    ...
    host_resolver_ = std::make_unique<MockHostResolver>();
    host_resolver_->rules()->AddRule(kDefaultServerHostName, MockHostResolverBase::RuleResolver::RuleResult(std::move(endpoints)));
