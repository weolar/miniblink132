...
public:
    ...
    void ReallyOnPathDegrading()
    {
        QuicChromiumClientSession::OnPathDegrading();
    }
};

class QuicChromiumClientSessionTest : public ::testing::TestWithParam<quic::ParsedQuicVersion>, public WithTaskEnvironment {
public:
    QuicChromiumClientSessionTest()
        : version_(GetParam())
        , config_(quic::test::DefaultQuicConfig())
        , crypto_config_(quic::test::crypto_test_utils::ProofVerifierForTesting())
        , default_read_(std::make_unique<MockRead>(SYNCHRONOUS, ERR_IO_PENDING, 0))
        , socket_data_(std::make_unique<SequencedSocketData>(base::span_from_ref(*default_read_), base::span<MockWrite>()))
        , helper_(&clock_, &random_)
        , transport_security_state_(std::make_unique<TransportSecurityState>())
        , session_key_(kServerHostname, kServerPort, PRIVACY_MODE_DISABLED, ProxyChain::Direct(), SessionUsage::kDestination, SocketTag(),
              NetworkAnonymizationKey(), SecureDnsPolicy::kAllow,
              /*require_dns_https_alpn=*/false)
        , destination_(url::kHttpsScheme, kServerHostname, kServerPort)
        , default_network_(handles::kInvalidNetworkHandle)
        , client_maker_(version_, quic::QuicUtils::CreateRandomConnectionId(&random_), &clock_, kServerHostname, quic::Perspective::IS_CLIENT)
        , server_maker_(version_, quic::QuicUtils::CreateRandomConnectionId(&random_), &clock_, kServerHostname, quic::Perspective::IS_SERVER, false)
    {
    ...
    void ResetHandleOnError(std::unique_ptr<QuicChromiumClientSession::Handle>* handle, int net_error)
    {
...
protected:
    void Initialize()
    {
        ...
        std::unique_ptr<DatagramClientSocket> socket = socket_factory_.CreateDatagramClientSocket(DatagramSocket::DEFAULT_BIND, NetLog::Get(), NetLogSource());
        socket->Connect(kIpEndPoint);
        QuicChromiumPacketWriter* writer = new net::QuicChromiumPacketWriter(socket.get(), base::SingleThreadTaskRunner::GetCurrentDefault().get());
        quic::QuicConnection* connection
            = new quic::QuicConnection(quic::QuicUtils::CreateRandomConnectionId(&random_), quic::QuicSocketAddress(), ToQuicSocketAddress(kIpEndPoint),
                &helper_, &alarm_factory_, writer, true, quic::Perspective::IS_CLIENT, quic::test::SupportedVersions(version_), connection_id_generator_);
        session_ = std::make_unique<TestingQuicChromiumClientSession>(connection, std::move(socket),
            /*stream_factory=*/nullptr, &crypto_client_stream_factory_, &clock_, transport_security_state_.get(), &ssl_config_service_,
            base::WrapUnique(static_cast<QuicServerInfo*>(nullptr)), QuicSessionAliasKey(url::SchemeHostPort(), session_key_),
            ...
            /*migrate_idle_session=*/false, allow_port_migration_, kDefaultIdleSessionMigrationPeriod, /*multi_port_probing_interval=*/0,
            ...
            /*cert_verify_flags=*/0, config_, std::make_unique<TestQuicCryptoClientConfigHandle>(&crypto_config_), "CONNECTION_UNKNOWN", base::TimeTicks::Now(),
            base::TimeTicks::Now(), base::DefaultTickClock::GetInstance(), base::SingleThreadTaskRunner::GetCurrentDefault().get(),
        ...
        scoped_refptr<X509Certificate> cert(ImportCertFromFile(GetTestCertsDirectory(), "spdy_pooling.pem"));
    ...
    std::unique_ptr<QuicChromiumPacketWriter> CreateQuicChromiumPacketWriter(DatagramClientSocket* socket, QuicChromiumClientSession* session) const
    {
        auto writer = std::make_unique<QuicChromiumPacketWriter>(socket, base::SingleThreadTaskRunner::GetCurrentDefault().get());
    ...
    {
        return quic::test::QuicSessionPeer::ietf_streamid_manager(session_.get())->max_outgoing_bidirectional_streams();
    }
...
INSTANTIATE_TEST_SUITE_P(VersionIncludeStreamDependencySequence, QuicChromiumClientSessionTest, ::testing::ValuesIn(AllSupportedQuicVersions()),
...
TEST_P(QuicChromiumClientSessionTest, GetSSLInfo1)
{
    ...
    details.cert_verify_result.policy_compliance = ct::CTPolicyCompliance::CT_POLICY_COMPLIES_VIA_SCTS;
    ...
    EXPECT_TRUE(ssl_info.cert->EqualsIncludingChain(details.cert_verify_result.verified_cert.get()));
    EXPECT_EQ(details.cert_verify_result.cert_status, ssl_info.cert_status);
    EXPECT_EQ(details.cert_verify_result.is_issued_by_known_root, ssl_info.is_issued_by_known_root);
    EXPECT_EQ(details.cert_verify_result.policy_compliance, ssl_info.ct_policy_compliance);
}
...
TEST_P(QuicChromiumClientSessionTest, GetSSLInfo2)
{
    ...
    details.cert_verify_result.policy_compliance = ct::CTPolicyCompliance::CT_POLICY_NOT_ENOUGH_SCTS;
...
TEST_P(QuicChromiumClientSessionTest, Handle)
{
    ...
    std::unique_ptr<QuicChromiumClientSession::Handle> handle = session_->CreateHandle(destination_);
    ...
    ASSERT_EQ(OK, handle->RequestStream(/*requires_confirmation=*/false, callback.callback(), TRAFFIC_ANNOTATION_FOR_TESTS));
    ...
    EXPECT_TRUE(handle->CreatePacketBundler().get() == nullptr);
    {
        ...
        std::unique_ptr<QuicChromiumClientSession::Handle> handle2 = session_->CreateHandle(destination_);
        EXPECT_FALSE(handle2->IsConnected());
        EXPECT_TRUE(handle2->OneRttKeysAvailable());
        ASSERT_EQ(ERR_CONNECTION_CLOSED, handle2->RequestStream(/*requires_confirmation=*/false, callback.callback(), TRAFFIC_ANNOTATION_FOR_TESTS));
    }
    ...
    EXPECT_EQ(ERR_CONNECTION_CLOSED, handle->GetPeerAddress(&address));
    EXPECT_TRUE(handle->CreatePacketBundler().get() == nullptr);
    ASSERT_EQ(ERR_CONNECTION_CLOSED, handle->RequestStream(/*requires_confirmation=*/false, callback.callback(), TRAFFIC_ANNOTATION_FOR_TESTS));
}
...
TEST_P(QuicChromiumClientSessionTest, ConfirmationRequiredStreamRequest)
{
    ...
    std::unique_ptr<QuicChromiumClientSession::Handle> handle = session_->CreateHandle(destination_);
    TestCompletionCallback callback;
    ASSERT_EQ(OK, handle->RequestStream(/*requires_confirmation=*/true, callback.callback(), TRAFFIC_ANNOTATION_FOR_TESTS));
    EXPECT_TRUE(handle->ReleaseStream() != nullptr);
...
TEST_P(QuicChromiumClientSessionTest, StreamRequestBeforeConfirmation)
{
    ...
    std::unique_ptr<QuicChromiumClientSession::Handle> handle = session_->CreateHandle(destination_);
    TestCompletionCallback callback;
    ASSERT_EQ(ERR_IO_PENDING, handle->RequestStream(/*requires_confirmation=*/true, callback.callback(), TRAFFIC_ANNOTATION_FOR_TESTS));
...
TEST_P(QuicChromiumClientSessionTest, AsyncStreamRequest)
{
    ...
    quic_data.AddWrite(SYNCHRONOUS,
        client_maker_.Packet(packet_num++)
            ...
            .Build());
    quic_data.AddWrite(
        SYNCHRONOUS, client_maker_.Packet(packet_num++).AddRstStreamFrame(GetNthClientInitiatedBidirectionalStreamId(0), quic::QUIC_STREAM_CANCELLED).Build());
    quic_data.AddWrite(
        SYNCHRONOUS, client_maker_.Packet(packet_num++).AddRstStreamFrame(GetNthClientInitiatedBidirectionalStreamId(1), quic::QUIC_STREAM_CANCELLED).Build());
    ...
    std::unique_ptr<QuicChromiumClientSession::Handle> handle = session_->CreateHandle(destination_);
    TestCompletionCallback callback;
    ASSERT_EQ(ERR_IO_PENDING, handle->RequestStream(/*requires_confirmation=*/false, callback.callback(), TRAFFIC_ANNOTATION_FOR_TESTS));
    // Request a second stream and verify that it's also pending.
    std::unique_ptr<QuicChromiumClientSession::Handle> handle2 = session_->CreateHandle(destination_);
    TestCompletionCallback callback2;
    ASSERT_EQ(ERR_IO_PENDING, handle2->RequestStream(/*requires_confirmation=*/false, callback2.callback(), TRAFFIC_ANNOTATION_FOR_TESTS));

    // Close two stream to open up sending credits.
    quic::QuicRstStreamFrame rst(quic::kInvalidControlFrameId, GetNthClientInitiatedBidirectionalStreamId(0), quic::QUIC_STREAM_CANCELLED, 0);
    session_->OnRstStream(rst);
    quic::QuicRstStreamFrame rst2(quic::kInvalidControlFrameId, GetNthClientInitiatedBidirectionalStreamId(1), quic::QUIC_STREAM_CANCELLED, 0);
    session_->OnRstStream(rst2);
    // To close the streams completely, we need to also receive STOP_SENDING
    // frames.
    quic::QuicStopSendingFrame stop_sending(quic::kInvalidControlFrameId, GetNthClientInitiatedBidirectionalStreamId(0), quic::QUIC_STREAM_CANCELLED);
    session_->OnStopSendingFrame(stop_sending);
    quic::QuicStopSendingFrame stop_sending2(quic::kInvalidControlFrameId, GetNthClientInitiatedBidirectionalStreamId(1), quic::QUIC_STREAM_CANCELLED);
...
TEST_P(QuicChromiumClientSessionTest, ReadAfterConnectionClose)
{
    ...
    quic_data.AddRead(ASYNC, server_maker_.Packet(1).AddConnectionCloseFrame(quic::QUIC_CRYPTO_VERSION_NOT_SUPPORTED, "Time to panic!").Build());
    quic_data.AddSocketDataToFactory(&socket_factory_);
    ...
    ASSERT_EQ(ERR_IO_PENDING,
        handle->RequestStream(
            /*requires_confirmation=*/false, base::BindOnce(&QuicChromiumClientSessionTest::ResetHandleOnError, base::Unretained(this), &handle2),
            TRAFFIC_ANNOTATION_FOR_TESTS));

    TestCompletionCallback callback2;
    ASSERT_EQ(ERR_IO_PENDING, handle2->RequestStream(/*requires_confirmation=*/false, callback2.callback(), TRAFFIC_ANNOTATION_FOR_TESTS));

    session_->connection()->CloseConnection(quic::QUIC_NETWORK_IDLE_TIMEOUT, "Timed out", quic::ConnectionCloseBehavior::SILENT_CLOSE);
...
TEST_P(QuicChromiumClientSessionTest, CancelPendingStreamRequest)
{
    ...
    quic_data.AddWrite(
        SYNCHRONOUS, client_maker_.Packet(3).AddRstStreamFrame(GetNthClientInitiatedBidirectionalStreamId(0), quic::QUIC_STREAM_CANCELLED).Build());
    quic_data.AddRead(ASYNC, ERR_IO_PENDING);
    quic_data.AddRead(ASYNC, ERR_CONNECTION_CLOSED);
    quic_data.AddSocketDataToFactory(&socket_factory_);
...
TEST_P(QuicChromiumClientSessionTest, ConnectionCloseBeforeStreamRequest)
{
    ...
    int packet_num = 1;
    quic_data.AddWrite(SYNCHRONOUS, client_maker_.MakeInitialSettingsPacket(packet_num++));
    quic_data.AddWrite(SYNCHRONOUS, client_maker_.Packet(packet_num++).AddPingFrame().Build());
    quic_data.AddRead(ASYNC, server_maker_.Packet(1).AddConnectionCloseFrame(quic::QUIC_CRYPTO_VERSION_NOT_SUPPORTED, "Time to panic!").Build());
...
TEST_P(QuicChromiumClientSessionTest, MaxNumStreams)
{
    ...
    quic_data.AddWrite(SYNCHRONOUS,
        client_maker_.Packet(3)
            .AddStopSendingFrame(GetNthClientInitiatedBidirectionalStreamId(0), quic::QUIC_RST_ACKNOWLEDGEMENT)
            .AddRstStreamFrame(GetNthClientInitiatedBidirectionalStreamId(0), quic::QUIC_RST_ACKNOWLEDGEMENT)
            .Build());
    ...
    for (size_t i = 0; i < kMaxOpenStreams; i++) {
        QuicChromiumClientStream* stream = QuicChromiumClientSessionPeer::CreateOutgoingStream(session_.get());
        EXPECT_TRUE(stream);
        streams.push_back(stream);
    }
    // This stream, the 51st dynamic stream, can not be opened.
    EXPECT_FALSE(QuicChromiumClientSessionPeer::CreateOutgoingStream(session_.get()));
    ...
    EXPECT_FALSE(QuicChromiumClientSessionPeer::CreateOutgoingStream(session_.get()));
    quic::QuicRstStreamFrame rst1(quic::kInvalidControlFrameId, stream_id, quic::QUIC_STREAM_NO_ERROR, 0);
    session_->OnRstStream(rst1);
    EXPECT_EQ(kMaxOpenStreams - 1, session_->GetNumActiveStreams());
    base::RunLoop().RunUntilIdle();
    EXPECT_TRUE(QuicChromiumClientSessionPeer::CreateOutgoingStream(session_.get()));
}
...
TEST_P(QuicChromiumClientSessionTest, PendingStreamOnRst)
{
    MockQuicData quic_data(version_);
    int packet_num = 1;
    quic_data.AddWrite(ASYNC, client_maker_.MakeInitialSettingsPacket(packet_num++));
    quic_data.AddWrite(
        ASYNC, client_maker_.Packet(packet_num++).AddStopSendingFrame(GetNthServerInitiatedUnidirectionalStreamId(0), quic::QUIC_RST_ACKNOWLEDGEMENT).Build());
    quic_data.AddRead(ASYNC, ERR_IO_PENDING);
    quic_data.AddRead(ASYNC, ERR_CONNECTION_CLOSED);
    quic_data.AddSocketDataToFactory(&socket_factory_);
    ...
    quic::QuicStreamFrame data(GetNthServerInitiatedUnidirectionalStreamId(0), false, 1, std::string_view("SP"));
    session_->OnStreamFrame(data);
    EXPECT_EQ(0u, session_->GetNumActiveStreams());
    quic::QuicRstStreamFrame rst(quic::kInvalidControlFrameId, GetNthServerInitiatedUnidirectionalStreamId(0), quic::QUIC_STREAM_CANCELLED, 0);
    session_->OnRstStream(rst);
}
...
TEST_P(QuicChromiumClientSessionTest, GoAwayReceived)
{
    ...
    EXPECT_EQ(nullptr, QuicChromiumClientSessionPeer::CreateOutgoingStream(session_.get()));
}
...
TEST_P(QuicChromiumClientSessionTest, CanPool)
{
    ...
    EXPECT_TRUE(session_->CanPool("www.example.org",
        QuicSessionKey("foo", 1234, PRIVACY_MODE_DISABLED, ProxyChain::Direct(), SessionUsage::kDestination, SocketTag(), NetworkAnonymizationKey(),
            SecureDnsPolicy::kAllow,
            /*require_dns_https_alpn=*/false)));
    EXPECT_FALSE(session_->CanPool("www.example.org",
        QuicSessionKey("foo", 1234, PRIVACY_MODE_ENABLED, ProxyChain::Direct(), SessionUsage::kDestination, SocketTag(), NetworkAnonymizationKey(),
            SecureDnsPolicy::kAllow,
            /*require_dns_https_alpn=*/false)));
    EXPECT_FALSE(session_->CanPool("www.example.org",
        QuicSessionKey("foo", 1234, PRIVACY_MODE_DISABLED, ProxyChain::Direct(), SessionUsage::kDestination, SocketTag(), NetworkAnonymizationKey(),
    ...
    EXPECT_FALSE(session_->CanPool("www.example.org",
        QuicSessionKey("foo", 1234, PRIVACY_MODE_DISABLED, ProxyChain::Direct(), SessionUsage::kDestination, tag1, NetworkAnonymizationKey(),
            SecureDnsPolicy::kAllow,
            /*require_dns_https_alpn=*/false)));
    EXPECT_FALSE(session_->CanPool("www.example.org",
        QuicSessionKey("foo", 1234, PRIVACY_MODE_DISABLED, ProxyChain::Direct(), SessionUsage::kDestination, tag2, NetworkAnonymizationKey(),
            SecureDnsPolicy::kAllow,
            /*require_dns_https_alpn=*/false)));
#endif
    EXPECT_FALSE(session_->CanPool("www.example.org",
        QuicSessionKey("foo", 1234, PRIVACY_MODE_DISABLED, ProxyChain::FromSchemeHostAndPort(ProxyServer::SCHEME_QUIC, "bar", 443), SessionUsage::kDestination,
            SocketTag(), NetworkAnonymizationKey(), SecureDnsPolicy::kAllow,
            /*require_dns_https_alpn=*/false)));
    EXPECT_FALSE(session_->CanPool("www.example.org",
        QuicSessionKey("foo", 1234, PRIVACY_MODE_DISABLED, ProxyChain::Direct(), SessionUsage::kProxy, SocketTag(), NetworkAnonymizationKey(),
            SecureDnsPolicy::kAllow,
            /*require_dns_https_alpn=*/false)));
    ...
    // Check that NetworkAnonymizationKey is respected when feature is enabled.
    {
        ...
        EXPECT_TRUE(session_->CanPool("mail.example.com",
            QuicSessionKey("foo", 1234, PRIVACY_MODE_DISABLED, ProxyChain::Direct(), SessionUsage::kDestination, SocketTag(),
                NetworkAnonymizationKey::CreateSameSite(kSiteFoo), SecureDnsPolicy::kAllow,
                /*require_dns_https_alpn=*/false)));
    }
    {
        base::test::ScopedFeatureList feature_list;
...
TEST_P(QuicChromiumClientSessionTest, CanPoolWithNetworkAnonymizationKey)
{
    ...
    const auto kNetworkAnonymizationKey1 = NetworkAnonymizationKey::CreateSameSite(kSiteFoo);
    const auto kNetworkAnonymizationKey2 = NetworkAnonymizationKey::CreateSameSite(kSiteBar);

    session_key_ = QuicSessionKey(kServerHostname, kServerPort, PRIVACY_MODE_DISABLED, ProxyChain::Direct(), SessionUsage::kDestination, SocketTag(),
    ...
    EXPECT_TRUE(session_->CanPool("www.example.org",
        QuicSessionKey("foo", 1234, PRIVACY_MODE_DISABLED, ProxyChain::Direct(), SessionUsage::kDestination, SocketTag(), kNetworkAnonymizationKey1,
            SecureDnsPolicy::kAllow,
            /*require_dns_https_alpn=*/false)));
    EXPECT_FALSE(session_->CanPool("www.example.org",
        QuicSessionKey("foo", 1234, PRIVACY_MODE_ENABLED, ProxyChain::Direct(), SessionUsage::kDestination, SocketTag(), kNetworkAnonymizationKey1,
            SecureDnsPolicy::kAllow,
            /*require_dns_https_alpn=*/false)));
#if BUILDFLAG(IS_ANDROID)
    SocketTag tag1(SocketTag::UNSET_UID, 0x12345678);
    SocketTag tag2(getuid(), 0x87654321);
    EXPECT_FALSE(session_->CanPool("www.example.org",
        QuicSessionKey("foo", 1234, PRIVACY_MODE_DISABLED, ProxyChain::Direct(), SessionUsage::kDestination, tag1, kNetworkAnonymizationKey1,
            SecureDnsPolicy::kAllow,
            /*require_dns_https_alpn=*/false)));
    EXPECT_FALSE(session_->CanPool("www.example.org",
        QuicSessionKey("foo", 1234, PRIVACY_MODE_DISABLED, ProxyChain::Direct(), SessionUsage::kDestination, tag2, kNetworkAnonymizationKey1,
            SecureDnsPolicy::kAllow,
            /*require_dns_https_alpn=*/false)));
#endif
    EXPECT_TRUE(session_->CanPool("mail.example.org",
    ...
    EXPECT_FALSE(session_->CanPool("mail.example.com",
        QuicSessionKey("foo", 1234, PRIVACY_MODE_DISABLED, ProxyChain::Direct(), SessionUsage::kDestination, SocketTag(), kNetworkAnonymizationKey2,
            SecureDnsPolicy::kAllow,
            /*require_dns_https_alpn=*/false)));
    EXPECT_FALSE(session_->CanPool("mail.example.com",
        QuicSessionKey("foo", 1234, PRIVACY_MODE_DISABLED, ProxyChain::Direct(), SessionUsage::kDestination, SocketTag(), NetworkAnonymizationKey(),
...
TEST_P(QuicChromiumClientSessionTest, ConnectionNotPooledWithDifferentPin)
{
    ...
    details.cert_verify_result.is_issued_by_known_root = true;
    uint8_t bad_pin = 3;
    details.cert_verify_result.public_key_hashes.push_back(GetTestHashValue(bad_pin));
    ...
    EXPECT_FALSE(session_->CanPool(kPreloadedPKPHost,
        QuicSessionKey("foo", 1234, PRIVACY_MODE_DISABLED, ProxyChain::Direct(), SessionUsage::kDestination, SocketTag(), NetworkAnonymizationKey(),
            SecureDnsPolicy::kAllow,
            /*require_dns_https_alpn=*/false)));
}
...
TEST_P(QuicChromiumClientSessionTest, MigrateToSocket)
{
    ...
    quic_data2.AddWrite(SYNCHRONOUS,
        client_maker_.Packet(packet_num++)
            ...
            .Build());
    quic_data2.AddWrite(
        SYNCHRONOUS, client_maker_.Packet(packet_num++).AddStreamFrame(GetNthClientInitiatedBidirectionalStreamId(0), false, std::string_view(data)).Build());
    quic_data2.AddSocketDataToFactory(&socket_factory_);
    // Create connected socket.
    std::unique_ptr<DatagramClientSocket> new_socket = socket_factory_.CreateDatagramClientSocket(DatagramSocket::RANDOM_BIND, NetLog::Get(), NetLogSource());
    EXPECT_THAT(new_socket->Connect(kIpEndPoint), IsOk());

    // Create reader and writer.
    auto new_reader = std::make_unique<QuicChromiumPacketReader>(std::move(new_socket), &clock_, session_.get(), kQuicYieldAfterPacketsRead,
    ...
    std::unique_ptr<QuicChromiumPacketWriter> new_writer(CreateQuicChromiumPacketWriter(new_reader->socket(), session_.get()));
    ...
    // Migrate session.
    EXPECT_TRUE(session_->MigrateToSocket(ToQuicSocketAddress(local_address), ToQuicSocketAddress(peer_address), std::move(new_reader), std::move(new_writer)));
    ...
    session_->WritevData(stream->id(), 4, 0, quic::NO_FIN, quic::NOT_RETRANSMISSION, quic::ENCRYPTION_FORWARD_SECURE);
...
TEST_P(QuicChromiumClientSessionTest, MigrateToSocketMaxReaders)
{
    ...
    int packet_num = 1;
    int peer_packet_num = 1;
    quic::QuicConnectionId next_cid = quic::QuicUtils::CreateRandomConnectionId(quiche::QuicheRandom::GetInstance());
    uint64_t next_cid_sequence_number = 1u;
    quic_data.AddWrite(SYNCHRONOUS, client_maker_.MakeInitialSettingsPacket(packet_num++));
    quic_data.AddRead(ASYNC, ERR_IO_PENDING);
    quic_data.AddRead(ASYNC,
        server_maker_.Packet(peer_packet_num++)
            .AddNewConnectionIdFrame(next_cid, next_cid_sequence_number,
    ...
    for (size_t i = 0; i < kMaxReadersPerQuicSession - 1; ++i) {
        ...
        quic_data2.AddWrite(ASYNC,
            client_maker_.Packet(packet_num++)
                ...
                .Build());
        next_cid = quic::QuicUtils::CreateRandomConnectionId(quiche::QuicheRandom::GetInstance());
        ...
        std::unique_ptr<DatagramClientSocket> new_socket
            = socket_factory_.CreateDatagramClientSocket(DatagramSocket::RANDOM_BIND, NetLog::Get(), NetLogSource());
        EXPECT_THAT(new_socket->Connect(kIpEndPoint), IsOk());
        ...
        // Migrate session.
        EXPECT_TRUE(
            session_->MigrateToSocket(ToQuicSocketAddress(local_address), ToQuicSocketAddress(peer_address), std::move(new_reader), std::move(new_writer)));
        // Spin message loop to complete migration.
        base::RunLoop().RunUntilIdle();
        alarm_factory_.FireAlarm(quic::test::QuicConnectionPeer::GetRetirePeerIssuedConnectionIdAlarm(session_->connection()));
...
TEST_P(QuicChromiumClientSessionTest, MigrateToSocketReadError)
{
    ...
    // Spin message loop to complete migration.
    base::RunLoop().RunUntilIdle();
    EXPECT_TRUE(quic::test::QuicConnectionPeer::GetRetirePeerIssuedConnectionIdAlarm(session_->connection())->IsSet());
...
TEST_P(QuicChromiumClientSessionTest, RetransmittableOnWireTimeout)
{
    ...
    quic_data.AddRead(ASYNC, server_maker_.Packet(1).AddAckFrame(1, packet_num - 1, 1).Build());
    ...
    quic::test::QuicTestAlarmProxy alarm(quic::test::QuicConnectionPeer::GetPingAlarm(session_->connection()));
    ...
    EXPECT_EQ(clock_.ApproximateNow() + quic::QuicTime::Delta::FromMilliseconds(200), alarm.deadline());
...
TEST_P(QuicChromiumClientSessionTest, ResetOnEmptyResponseHeaders)
{
    ...
    quic_data.AddWrite(ASYNC,
        client_maker_.Packet(packet_num++)
            .AddStopSendingFrame(GetNthClientInitiatedBidirectionalStreamId(0), quic::QUIC_STREAM_GENERAL_PROTOCOL_ERROR)
            .AddRstStreamFrame(GetNthClientInitiatedBidirectionalStreamId(0), quic::QUIC_STREAM_GENERAL_PROTOCOL_ERROR)
            .Build());
    quic_data.AddRead(ASYNC, ERR_IO_PENDING);
    quic_data.AddRead(ASYNC, ERR_CONNECTION_CLOSED);
    quic_data.AddSocketDataToFactory(&socket_factory_);
    ...
    TestCompletionCallback callback;
    EXPECT_EQ(OK, session_handle->RequestStream(/*requires_confirmation=*/false, callback.callback(), TRAFFIC_ANNOTATION_FOR_TESTS));

    auto stream_handle = session_handle->ReleaseStream();
    EXPECT_TRUE(stream_handle->IsOpen());

    auto* stream = quic::test::QuicSessionPeer::GetOrCreateStream(session_.get(), stream_handle->id());
    ...
    int rv = stream_handle->ReadInitialHeaders(&header_block, CompletionOnceCallback());
...
TEST_P(QuicChromiumClientSessionTest, DegradingWithoutNetworkChange_NoNetworkHandle)
{
    // Add a connectivity monitor for testing.
    default_network_ = handles::kInvalidNetworkHandle;
    connectivity_monitor_ = std::make_unique<QuicConnectivityMonitor>(default_network_);
...
TEST_P(QuicChromiumClientSessionTest, DegradingWithMultiPortEnabled)
{
    ...
    EXPECT_EQ(UNKNOWN_CAUSE, QuicChromiumClientSessionPeer::GetCurrentMigrationCause(session_.get()));
}
...
// by |default_network_|.
TEST_P(QuicChromiumClientSessionTest, DegradingOnDeafultNetwork_WithoutMigration)
{
...
TEST_P(QuicChromiumClientSessionTest, DegradingWithDeafultNetworkChange_WithoutMigration)
{
    default_network_ = kDefaultNetworkForTests;
    connectivity_monitor_ = std::make_unique<QuicConnectivityMonitor>(default_network_);
...
TEST_P(QuicChromiumClientSessionTest, WriteErrorDuringCryptoConnect)
{
    ...
    Initialize();
    ASSERT_THAT(session_->CryptoConnect(callback_.callback()), IsError(ERR_QUIC_HANDSHAKE_FAILED));
    // Verify error count is properly recorded.
    EXPECT_EQ(1u, connectivity_monitor_->GetCountForWriteErrorCode(ERR_ADDRESS_UNREACHABLE));
    EXPECT_EQ(0u, connectivity_monitor_->GetCountForWriteErrorCode(ERR_CONNECTION_RESET));

    // Simulate a default network change, write error stats should be reset.
    connectivity_monitor_->OnDefaultNetworkUpdated(kNewNetworkForTests);
    EXPECT_EQ(0u, connectivity_monitor_->GetCountForWriteErrorCode(ERR_ADDRESS_UNREACHABLE));
}
...
TEST_P(QuicChromiumClientSessionTest, WriteErrorAfterHandshakeConfirmed)
{
    ...
    // Verify error count is properly recorded.
    EXPECT_EQ(1u, connectivity_monitor_->GetCountForWriteErrorCode(ERR_CONNECTION_RESET));
    EXPECT_EQ(0u, connectivity_monitor_->GetCountForWriteErrorCode(ERR_ADDRESS_UNREACHABLE));
    ...
    // clears all stats.
    size_t expected_error_count = NetworkChangeNotifier::AreNetworkHandlesSupported() ? 1u : 0u;
    EXPECT_EQ(expected_error_count, connectivity_monitor_->GetCountForWriteErrorCode(ERR_CONNECTION_RESET));
}
...
TEST_P(QuicChromiumClientSessionTest, ReportsReceivedEcn)
{
    ...
    mock_quic_data.AddWrite(ASYNC, client_maker_.MakeInitialSettingsPacket(write_packet_num++));
    mock_quic_data.AddRead(ASYNC, server_maker_.MakeInitialSettingsPacket(read_packet_num++));
    server_maker_.set_ecn_codepoint(quic::ECN_ECT0);
    mock_quic_data.AddRead(ASYNC, server_maker_.Packet(read_packet_num++).AddPingFrame().Build());
    mock_quic_data.AddWrite(SYNCHRONOUS, client_maker_.Packet(write_packet_num++).AddAckFrame(0, 1, 0, ecn).Build());
    ...
    ecn.ect1 = 1;
    ecn.ce = 1;
    mock_quic_data.AddWrite(SYNCHRONOUS, client_maker_.Packet(write_packet_num++).AddAckFrame(0, 3, 0, ecn).Build());
