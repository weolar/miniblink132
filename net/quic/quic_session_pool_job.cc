...
base::Value::Dict NetLogQuicSessionPoolJobParams(const QuicSessionAliasKey* key)
{
    const ProxyChain& proxy_chain = key->session_key().proxy_chain();
    return base::Value::Dict()
        .Set("host", key->server_id().host())
        .Set("port", key->server_id().port())
        .Set("privacy_mode", PrivacyModeToDebugString(key->session_key().privacy_mode()))
        .Set("proxy_chain", proxy_chain.IsValid() ? proxy_chain.ToDebugString() : "invalid")
        .Set("network_anonymization_key", key->session_key().network_anonymization_key().ToDebugString());
}

} // namespace

QuicSessionPool::Job::Job(QuicSessionPool* pool, QuicSessionAliasKey key, std::unique_ptr<CryptoClientConfigHandle> client_config_handle,
    RequestPriority priority, const NetLogWithSource& net_log)
    : pool_(pool)
    , key_(std::move(key))
    ...
    , net_log_(net_log)
{
    net_log_.BeginEvent(NetLogEventType::QUIC_SESSION_POOL_JOB, [&] { return NetLogQuicSessionPoolJobParams(&key_); });
}
...
void QuicSessionPool::Job::AssociateWithNetLogSource(const NetLogWithSource& http_stream_job_net_log) const
{
    net_log().AddEventReferencingSource(NetLogEventType::QUIC_SESSION_POOL_JOB_BOUND_TO, http_stream_job_net_log.source());
    http_stream_job_net_log.AddEventReferencingSource(NetLogEventType::BOUND_TO_QUIC_SESSION_POOL_JOB, net_log().source());
}
...
void QuicSessionPool::Job::UpdatePriority(RequestPriority old_priority, RequestPriority new_priority)
{
}
