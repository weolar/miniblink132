...
base::TimeTicks NowInTicks(const quic::MockClock& clock)
{
    ...
    return ticks + base::Microseconds((clock.Now() - quic::QuicTime::Zero()).ToMicroseconds());
}
...
bool TestTaskRunner::PostDelayedTask(const base::Location& from_here, base::OnceClosure task, base::TimeDelta delay)
{
    EXPECT_GE(delay, base::TimeDelta());
    tasks_.emplace_back(from_here, std::move(task), NowInTicks(*clock_), delay, base::TestPendingTask::NESTABLE);
    return true;
}

bool TestTaskRunner::PostNonNestableDelayedTask(const base::Location& from_here, base::OnceClosure task, base::TimeDelta delay)
{
...
quic::QuicTime::Delta TestTaskRunner::NextPendingTaskDelay()
{
    ...
    return quic::QuicTime::Delta::FromMicroseconds((next->GetTimeToRun() - NowInTicks(*clock_)).InMicroseconds());
}
...
void TestTaskRunner::RunNextTask()
{
    ...
    clock_->AdvanceTime(quic::QuicTime::Delta::FromMicroseconds((next->GetTimeToRun() - NowInTicks(*clock_)).InMicroseconds()));
