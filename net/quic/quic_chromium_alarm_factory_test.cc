...
TEST_F(QuicChromiumAlarmFactoryTest, CreateAlarm)
{
    ...
    constexpr quic::QuicTime::Delta kDelta = quic::QuicTime::Delta::FromMicroseconds(1);
...
TEST_F(QuicChromiumAlarmFactoryTest, CreateAlarmAndReset)
{
    ...
    constexpr quic::QuicTime::Delta kNewDelta = quic::QuicTime::Delta::FromMicroseconds(3);
...
TEST_F(QuicChromiumAlarmFactoryTest, CreateAlarmAndResetEarlier)
{
    ...
    constexpr quic::QuicTime::Delta kDelta = quic::QuicTime::Delta::FromMicroseconds(3);
    quic::QuicTime deadline = clock_.Now() + kDelta;
    alarm->Set(deadline);
    EXPECT_TRUE(alarm->IsSet());
    EXPECT_EQ(alarm->deadline(), deadline);
    ...
    constexpr quic::QuicTime::Delta kNewDelta = quic::QuicTime::Delta::FromMicroseconds(1);
    quic::QuicTime new_deadline = clock_.Now() + kNewDelta;
    alarm->Set(new_deadline);
    EXPECT_TRUE(alarm->IsSet());
    EXPECT_EQ(alarm->deadline(), new_deadline);
...
TEST_F(QuicChromiumAlarmFactoryTest, CreateAlarmAndUpdate)
{
    ...
    alarm->Update(new_deadline + quic::QuicTime::Delta::FromMicroseconds(1), quic::QuicTime::Delta::FromMicroseconds(2));
    EXPECT_TRUE(alarm->IsSet());
    EXPECT_EQ(alarm->deadline(), new_deadline);
    EXPECT_FALSE(delegate->fired());
    ...
    alarm->Update(quic::QuicTime::Zero(), quic::QuicTime::Delta::FromMicroseconds(1));
    EXPECT_FALSE(alarm->IsSet());
}
