...
public:
    ...
    MockCryptoClientStreamFactory& operator=(const MockCryptoClientStreamFactory&) = delete;

    ~MockCryptoClientStreamFactory() override;

    std::unique_ptr<quic::QuicCryptoClientStream> CreateQuicCryptoClientStream(const quic::QuicServerId& server_id, QuicChromiumClientSession* session,
        std::unique_ptr<quic::ProofVerifyContext> proof_verify_context, quic::QuicCryptoClientConfig* crypto_config) override;

    void set_handshake_mode(MockCryptoClientStream::HandshakeMode handshake_mode)
    {
    ...
    void AddProofVerifyDetails(const ProofVerifyDetailsChromium* proof_verify_details)
    {
    ...
    void SetConfigForServerId(const quic::QuicServerId& server_id, const quic::QuicConfig& config);

private:
    MockCryptoClientStream::HandshakeMode handshake_mode_ = MockCryptoClientStream::CONFIRM_HANDSHAKE;
    std::vector<base::WeakPtr<MockCryptoClientStream>> streams_;
    base::queue<raw_ptr<const ProofVerifyDetailsChromium, CtnExperimental>> proof_verify_details_queue_;
    std::unique_ptr<quic::QuicConfig> config_;
    std::map<quic::QuicServerId, std::unique_ptr<quic::QuicConfig>> config_for_server_;
