...
QuicHttpStream::QuicHttpStream(std::unique_ptr<QuicChromiumClientSession::Handle> session, std::set<std::string> dns_aliases)
...
HttpConnectionInfo QuicHttpStream::ConnectionInfoFromQuicVersion(quic::ParsedQuicVersion quic_version)
{
...
int QuicHttpStream::InitializeStream(bool can_send_early, RequestPriority priority, const NetLogWithSource& stream_net_log, CompletionOnceCallback callback)
{
    ...
    stream_net_log.AddEventReferencingSource(NetLogEventType::HTTP_STREAM_REQUEST_BOUND_TO_QUIC_SESSION, quic_session()->net_log().source());
    stream_net_log.AddEventWithIntParams(
        NetLogEventType::QUIC_CONNECTION_MIGRATION_MODE, "connection_migration_mode", static_cast<int>(quic_session()->connection_migration_mode()));
...
int QuicHttpStream::SendRequest(const HttpRequestHeaders& request_headers, HttpResponseInfo* response, CompletionOnceCallback callback)
{
    ...
    CreateSpdyHeadersFromHttpRequest(*request_info_, priority_, request_headers, &request_headers_);
    ...
    if (request_body_stream_) {
        ...
        // CPU usage.
        raw_request_body_buf_ = base::MakeRefCounted<IOBufferWithSize>(
            static_cast<size_t>(std::max(10 * quic::kMaxOutgoingPacketSize, std::min(request_body_stream_->size(), 256 * quic::kMaxOutgoingPacketSize))));
        // The request body buffer is empty at first.
        request_body_buf_ = base::MakeRefCounted<DrainableIOBuffer>(raw_request_body_buf_, 0);
    }
...
int QuicHttpStream::ReadResponseHeaders(CompletionOnceCallback callback)
{
    ...
    int rv = stream_->ReadInitialHeaders(&response_header_block_, base::BindOnce(&QuicHttpStream::OnReadResponseHeadersComplete, weak_factory_.GetWeakPtr()));
...
int QuicHttpStream::ReadResponseBody(IOBuffer* buf, int buf_len, CompletionOnceCallback callback)
{
    CHECK(callback_.is_null());
    CHECK(!callback.is_null());
    ...
    int rv = stream_->ReadBody(buf, buf_len, base::BindOnce(&QuicHttpStream::OnReadBodyComplete, weak_factory_.GetWeakPtr()));
    if (rv == ERR_IO_PENDING) {
        callback_ = std::move(callback);
        user_buffer_ = buf;
...
bool QuicHttpStream::GetLoadTimingInfo(LoadTimingInfo* load_timing_info) const
{
    ...
    if (stream_) {
        ...
        load_timing_info->first_early_hints_time = stream_->first_early_hints_time();
        load_timing_info->receive_non_informational_headers_start = stream_->headers_received_start_time();
        load_timing_info->receive_headers_start = load_timing_info->first_early_hints_time.is_null() ? load_timing_info->receive_non_informational_headers_start
...
bool QuicHttpStream::GetAlternativeService(AlternativeService* alternative_service) const
{
...
{
    details->connection_info = ConnectionInfoFromQuicVersion(quic_session()->GetQuicVersion());
    quic_session()->PopulateNetErrorDetails(details);
    if (quic_session()->OneRttKeysAvailable() && stream_ && stream_->connection_error() != quic::QUIC_NO_ERROR) {
...
std::optional<HttpStream::QuicErrorDetails> QuicHttpStream::GetQuicErrorDetails() const
{
...
{
    int rv = stream_->ReadTrailingHeaders(&trailing_header_block_, base::BindOnce(&QuicHttpStream::OnReadTrailingHeadersComplete, weak_factory_.GetWeakPtr()));
...
int QuicHttpStream::DoLoop(int rv)
{
    ...
    std::unique_ptr<quic::QuicConnection::ScopedPacketFlusher> packet_flusher = quic_session()->CreatePacketBundler();
    do {
...
int QuicHttpStream::DoRequestStream()
{
    ...
    return quic_session()->RequestStream(!can_send_early_, base::BindOnce(&QuicHttpStream::OnIOComplete, weak_factory_.GetWeakPtr()),
...
int QuicHttpStream::DoRequestStreamComplete(int rv)
{
    ...
    if (request_info_->load_flags & LOAD_DISABLE_CONNECTION_MIGRATION_TO_CELLULAR) {
...
int QuicHttpStream::DoSetRequestPriority()
{
    ...
    stream_->SetPriority(quic::QuicStreamPriority(quic::HttpStreamPriority { urgency, incremental }));
...
int QuicHttpStream::DoSendHeaders()
{
    uint8_t urgency = ConvertRequestPriorityToQuicPriority(priority_);
    bool incremental = request_info_->priority_incremental;
    quic::QuicStreamPriority priority(quic::HttpStreamPriority { urgency, incremental });
    // Log the actual request with the URL Request's net log.
    stream_net_log_.AddEvent(NetLogEventType::HTTP_TRANSACTION_QUIC_SEND_REQUEST_HEADERS,
        [&](NetLogCaptureMode capture_mode) { return QuicRequestNetLogParams(stream_->id(), &request_headers_, priority, capture_mode); });
    ...
    int rv = stream_->WriteHeaders(std::move(request_headers_), !has_upload_data, nullptr);
    if (rv > 0) {
...
int QuicHttpStream::DoReadRequestBody()
{
    ...
    return request_body_stream_->Read(
        raw_request_body_buf_.get(), raw_request_body_buf_->size(), base::BindOnce(&QuicHttpStream::OnIOComplete, weak_factory_.GetWeakPtr()));
}
...
int QuicHttpStream::DoReadRequestBodyComplete(int rv)
{
    ...
    request_body_buf_ = base::MakeRefCounted<DrainableIOBuffer>(raw_request_body_buf_, rv);
...
int QuicHttpStream::DoSendBody()
{
    ...
    if (len > 0 || eof) {
        ...
        return stream_->WriteStreamData(data, eof, base::BindOnce(&QuicHttpStream::OnIOComplete, weak_factory_.GetWeakPtr()));
    }
...
int QuicHttpStream::ProcessResponseHeaders(const quiche::HttpHeaderBlock& headers)
{
    const int rv = SpdyHeadersToHttpResponse(headers, response_info_);
    base::UmaHistogramBoolean("Net.QuicHttpStream.ProcessResponseHeaderSuccess", rv == OK);
    if (rv != OK) {
    ...
    response_info_->connection_info = ConnectionInfoFromQuicVersion(quic_session()->GetQuicVersion());
    response_info_->was_alpn_negotiated = true;
    response_info_->alpn_negotiated_protocol = HttpConnectionInfoToString(response_info_->connection_info);
    response_info_->response_time = response_info_->original_response_time = base::Time::Now();
    ...
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(&QuicHttpStream::ReadTrailingHeaders, weak_factory_.GetWeakPtr()));
...
int QuicHttpStream::ComputeResponseStatus() const
{
    ...
    base::UmaHistogramEnumeration("Net.QuicHttpStream.ResponseStatus", stream_->stream_error(), quic::QUIC_STREAM_LAST_ERROR);
