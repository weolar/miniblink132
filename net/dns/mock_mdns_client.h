...
public:
    ...
    MOCK_METHOD3(CreateListener, std::unique_ptr<MDnsListener>(uint16_t, const std::string&, MDnsListener::Delegate*));
    MOCK_METHOD4(CreateTransaction, std::unique_ptr<MDnsTransaction>(uint16_t, const std::string&, int, const MDnsTransaction::ResultCallback&));
