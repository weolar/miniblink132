...
using SystemDnsResultsCallback = base::OnceCallback<void(const AddressList& addr_list, int os_error, int net_error)>;
...
public:
    ...
    struct NET_EXPORT_PRIVATE Params {
        ...
        static constexpr base::TimeDelta kDnsDefaultUnresponsiveDelay = base::Seconds(6);

        // Set |max_system_retry_attempts| to this to select a default retry value.
        static constexpr size_t kDefaultRetryAttempts = -1;

        // Sets up defaults.
        Params(scoped_refptr<HostResolverProc> resolver_proc, size_t max_retry_attempts);
    ...
    struct CacheParams {
        CacheParams(HostResolverCache& cache, NetworkAnonymizationKey network_anonymization_key);
    ...
    static std::unique_ptr<HostResolverSystemTask> Create(std::string hostname, AddressFamily address_family, HostResolverFlags flags,
        const Params& params = Params(nullptr, 0), const NetLogWithSource& job_net_log = NetLogWithSource(),
        handles::NetworkHandle network = handles::kInvalidNetworkHandle, std::optional<CacheParams> cache_params = std::nullopt);

    // Same as above but resolves the result of GetHostName() (the machine's own
    // hostname).
    static std::unique_ptr<HostResolverSystemTask> CreateForOwnHostname(AddressFamily address_family, HostResolverFlags flags,
        const Params& params = Params(nullptr, 0), const NetLogWithSource& job_net_log = NetLogWithSource(),
    ...
    HostResolverSystemTask(std::optional<std::string> hostname, AddressFamily address_family, HostResolverFlags flags,
        const Params& params = Params(nullptr, 0), const NetLogWithSource& job_net_log = NetLogWithSource(),
        handles::NetworkHandle network = handles::kInvalidNetworkHandle, std::optional<CacheParams> cache_params = std::nullopt);
...
private:
    ...
    void OnLookupComplete(const uint32_t attempt_number, const AddressList& results, const int os_error, int error);

    void MaybeCacheResults(const AddressList& address_list);
    void CacheEndpoints(std::string domain_name, std::vector<IPEndPoint> endpoints, DnsQueryType query_type);
    void CacheAlias(std::string domain_name, DnsQueryType query_type, std::string target_name);
...
NET_EXPORT_PRIVATE int SystemHostResolverCall(const std::string& host, AddressFamily address_family, HostResolverFlags host_resolver_flags,
    AddressList* addrlist, int* os_error, handles::NetworkHandle network = handles::kInvalidNetworkHandle);
...
NET_EXPORT_PRIVATE void SetSystemDnsResolutionTaskRunnerForTesting(scoped_refptr<base::TaskRunner> task_runner);
...
NET_EXPORT void SetSystemDnsResolverOverride(base::RepeatingCallback<void(const std::optional<std::string>& host, AddressFamily address_family,
