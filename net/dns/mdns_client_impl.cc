...
void RecordQueryMetric(mdnsQueryType query_type, std::string_view host)
{
    ...
    } else if (base::ranges::any_of(kPrintScanServices, [&host](std::string_view service) { return host.ends_with(service); })) {
...
void MDnsSocketFactoryImpl::CreateSockets(std::vector<std::unique_ptr<DatagramServerSocket>>* sockets)
{
    InterfaceIndexFamilyList interfaces(GetMDnsInterfacesToBind());
    for (const auto& interface : interfaces) {
        DCHECK(interface.second == ADDRESS_FAMILY_IPV4 || interface.second == ADDRESS_FAMILY_IPV6);
        std::unique_ptr<DatagramServerSocket> socket(CreateAndBindMDnsSocket(interface.second, interface.first, net_log_));
        if (socket)
            sockets->push_back(std::move(socket));
    }
}

MDnsConnection::SocketHandler::SocketHandler(std::unique_ptr<DatagramServerSocket> socket, MDnsConnection* connection)
...
int MDnsConnection::SocketHandler::DoLoop(int rv)
{
    do {
        ...
        rv = socket_->RecvFrom(response_.io_buffer(), response_.io_buffer_size(), &recv_addr_,
            base::BindOnce(&MDnsConnection::SocketHandler::OnDatagramReceived, base::Unretained(this)));
    } while (rv > 0);
...
void MDnsConnection::SocketHandler::Send(const scoped_refptr<IOBuffer>& buffer, unsigned size)
{
    if (send_in_progress_) {
        send_queue_.emplace(buffer, size);
        return;
    }
    int rv = socket_->SendTo(buffer.get(), size, multicast_addr_, base::BindOnce(&MDnsConnection::SocketHandler::SendDone, base::Unretained(this)));
    if (rv == ERR_IO_PENDING) {
...
int MDnsConnection::Init(MDnsSocketFactory* socket_factory)
{
    ...
    for (std::unique_ptr<DatagramServerSocket>& socket : sockets) {
        socket_handlers_.push_back(std::make_unique<MDnsConnection::SocketHandler>(std::move(socket), this));
    }
...
void MDnsConnection::Send(const scoped_refptr<IOBuffer>& buffer, unsigned size)
{
...
void MDnsConnection::PostOnError(SocketHandler* loop, int rv)
{
    ...
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(&MDnsConnection::OnError, weak_ptr_factory_.GetWeakPtr(), rv));
}
...
void MDnsConnection::OnDatagramReceived(DnsResponse* response, const IPEndPoint& recv_addr, int bytes_read)
{
...
MDnsClientImpl::Core::Core(base::Clock* clock, base::OneShotTimer* timer)
    ...
    , connection_(std::make_unique<MDnsConnection>((MDnsConnection::Delegate*)this))
{
...
{
    std::optional<std::vector<uint8_t>> name_dns = dns_names_util::DottedNameToNetwork(name);
...
void MDnsClientImpl::Core::HandlePacket(DnsResponse* response, int bytes_read)
{
    ...
    for (const auto& update_key : update_keys) {
        ...
        if (record->type() == dns_protocol::kTypeNSEC) {
            ...
            AlertListeners(update_key.second, ListenerKey(record->name(), record->type()), record);
        }
    }
}
...
void MDnsClientImpl::Core::NotifyNsecRecord(const RecordParsed* record)
{
    ...
    for (const auto* record_to_remove : records_to_remove) {
        ...
        if (!rdata->GetBit(record_to_remove->type())) {
            std::unique_ptr<const RecordParsed> record_removed = cache_.RemoveRecord(record_to_remove);
    ...
    for (; i != listeners_.end() && i->first.name_lowercase() == key.name_lowercase(); i++) {
...
MDnsClientImpl::Core::ListenerKey::ListenerKey(const std::string& name, uint16_t type)
    : name_lowercase_(base::ToLowerASCII(name))
    , type_(type)
{
}

bool MDnsClientImpl::Core::ListenerKey::operator<(const MDnsClientImpl::Core::ListenerKey& key) const
{
...
void MDnsClientImpl::Core::AlertListeners(MDnsCache::UpdateType update_type, const ListenerKey& key, const RecordParsed* record)
{
...
void MDnsClientImpl::Core::RemoveListener(MDnsListenerImpl* listener)
{
    ...
    if (observer_list_iterator->second->empty()) {
        ...
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
            FROM_HERE, base::BindOnce(&MDnsClientImpl::Core::CleanupObserverList, weak_ptr_factory_.GetWeakPtr(), key));
    }
}
...
void MDnsClientImpl::Core::ScheduleCleanup(base::Time cleanup)
{
    ...
    if (cleanup != base::Time()) {
        cleanup_timer_->Start(
            FROM_HERE, std::max(base::TimeDelta(), cleanup - clock_->Now()), base::BindOnce(&MDnsClientImpl::Core::DoCleanup, base::Unretained(this)));
    }
}

void MDnsClientImpl::Core::DoCleanup()
{
    cache_.CleanupRecords(clock_->Now(), base::BindRepeating(&MDnsClientImpl::Core::OnRecordRemoved, base::Unretained(this)));
...
{
    AlertListeners(MDnsCache::RecordRemoved, ListenerKey(record->name(), record->type()), record);
}

void MDnsClientImpl::Core::QueryCache(uint16_t rrtype, const std::string& name, std::vector<const RecordParsed*>* records) const
{
...
MDnsClientImpl::MDnsClientImpl(base::Clock* clock, std::unique_ptr<base::OneShotTimer> timer)
    : clock_(clock)
...
std::unique_ptr<MDnsListener> MDnsClientImpl::CreateListener(uint16_t rrtype, const std::string& name, MDnsListener::Delegate* delegate)
{
    return std::make_unique<MDnsListenerImpl>(rrtype, name, clock_, delegate, this);
}

std::unique_ptr<MDnsTransaction> MDnsClientImpl::CreateTransaction(
    uint16_t rrtype, const std::string& name, int flags, const MDnsTransaction::ResultCallback& callback)
{
    return std::make_unique<MDnsTransactionImpl>(rrtype, name, flags, callback, this);
}

MDnsListenerImpl::MDnsListenerImpl(uint16_t rrtype, const std::string& name, base::Clock* clock, MDnsListener::Delegate* delegate, MDnsClientImpl* client)
    : rrtype_(rrtype)
    , name_(name)
    , clock_(clock)
    , client_(client)
...
void MDnsListenerImpl::HandleRecordUpdate(MDnsCache::UpdateType update_type, const RecordParsed* record)
{
    DCHECK(started_);
...
void MDnsListenerImpl::ScheduleNextRefresh()
{
    ...
    next_refresh_.Reset(base::BindRepeating(&MDnsListenerImpl::DoRefresh, weak_ptr_factory_.GetWeakPtr()));
    ...
    base::Time next_refresh1 = last_update_ + base::Milliseconds(static_cast<int>(base::Time::kMillisecondsPerSecond * kListenerRefreshRatio1 * ttl_));

    base::Time next_refresh2 = last_update_ + base::Milliseconds(static_cast<int>(base::Time::kMillisecondsPerSecond * kListenerRefreshRatio2 * ttl_));

    base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, next_refresh_.callback(), next_refresh1 - clock_->Now());

    base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, next_refresh_.callback(), next_refresh2 - clock_->Now());
}
...
MDnsTransactionImpl::MDnsTransactionImpl(
    uint16_t rrtype, const std::string& name, int flags, const MDnsTransaction::ResultCallback& callback, MDnsClientImpl* client)
    : rrtype_(rrtype)
    , name_(name)
    , callback_(callback)
    , client_(client)
...
void MDnsTransactionImpl::TriggerCallback(MDnsTransaction::Result result, const RecordParsed* record)
{
    DCHECK(started_);
    if (!is_active())
        return;
...
void MDnsTransactionImpl::OnRecordUpdate(MDnsListener::UpdateType update, const RecordParsed* record)
{
    DCHECK(started_);
...
bool MDnsTransactionImpl::QueryAndListen()
{
    ...
    timeout_.Reset(base::BindOnce(&MDnsTransactionImpl::SignalTransactionOver, weak_ptr_factory_.GetWeakPtr()));
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, timeout_.callback(), kTransactionTimeout);
