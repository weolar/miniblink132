...
{
    std::optional<std::vector<uint8_t>> dns_name = dns_names_util::DottedNameToNetwork(dotted_name);
...
public:
    ...
    DnsSocketData(uint16_t id, const char* dotted_name, uint16_t qtype, IoMode mode, Transport transport, const OptRecordRdata* opt_rdata = nullptr,
        DnsQuery::PaddingStrategy padding_strategy = DnsQuery::PaddingStrategy::NONE)
        : query_(std::make_unique<DnsQuery>(id, DomainFromDot(dotted_name), qtype, opt_rdata, padding_strategy))
        , transport_(transport)
    {
        if (Transport::TCP == transport_) {
            auto length = std::make_unique<uint16_t>();
            *length = base::HostToNet16(query_->io_buffer()->size());
            writes_.emplace_back(mode, reinterpret_cast<const char*>(length.get()), sizeof(uint16_t), num_reads_and_writes());
            lengths_.push_back(std::move(length));
        }
        writes_.emplace_back(mode, query_->io_buffer()->data(), query_->io_buffer()->size(), num_reads_and_writes());
    }
    ...
    void AddResponseWithLength(std::unique_ptr<DnsResponse> response, IoMode mode, uint16_t tcp_length)
    {
        CHECK(!provider_.get());
        if (Transport::TCP == transport_) {
            auto length = std::make_unique<uint16_t>();
            *length = base::HostToNet16(tcp_length);
            reads_.emplace_back(mode, reinterpret_cast<const char*>(length.get()), sizeof(uint16_t), num_reads_and_writes());
            lengths_.push_back(std::move(length));
        }
        reads_.emplace_back(mode, response->io_buffer()->data(), response->io_buffer_size(), num_reads_and_writes());
    ...
    {
        auto response = std::make_unique<DnsResponse>(query_->io_buffer()->span(), 0);
    ...
    SequencedSocketData* GetProvider()
    {
        ...
        if (transport_ != Transport::HTTPS) {
            reads_.emplace_back(SYNCHRONOUS, ERR_IO_PENDING, writes_.size() + reads_.size());
        }
    ...
    IOBufferWithSize* query_buffer()
    {
        return query_->io_buffer();
    }

private:
    size_t num_reads_and_writes() const
    {
...
public:
    TestUDPClientSocket(TestSocketFactory* factory, SocketDataProvider* data, net::NetLog* net_log)
        : MockUDPClientSocket(data, net_log)
        , factory_(factory)
    {
    }
    ...
    int Connect(const IPEndPoint& endpoint) override;
    int ConnectAsync(const IPEndPoint& address, CompletionOnceCallback callback) override;
...
public:
    ...
    std::unique_ptr<DatagramClientSocket> CreateDatagramClientSocket(DatagramSocket::BindType bind_type, NetLog* net_log, const NetLogSource& source) override
    {
        ...
        auto socket = std::make_unique<TestUDPClientSocket>(this, data_provider, net_log);
...
int TestUDPClientSocket::ConnectAsync(const IPEndPoint& address, CompletionOnceCallback callback)
{
...
public:
    ...
    void StartTransaction(DnsTransactionFactory* factory, const char* hostname, uint16_t qtype, bool secure, ResolveContext* context)
    {
        std::unique_ptr<DnsTransaction> transaction
            = factory->CreateTransaction(hostname, qtype, NetLogWithSource::Make(net::NetLog::Get(), net::NetLogSourceType::NONE), secure,
    ...
    void StartTransaction(std::unique_ptr<DnsTransaction> transaction)
    {
        ...
        transaction_->Start(base::BindOnce(&TransactionHelper::OnTransactionComplete, base::Unretained(this)));
    }
    ...
    void OnTransactionComplete(int rv, const DnsResponse* response)
    {
        ...
        if (expected_answer_count_ >= 0) {
            ...
            EXPECT_EQ(static_cast<unsigned>(expected_answer_count_), response->answer_count());
...
using ResponseModifierCallback = base::RepeatingCallback<void(URLRequest* request, HttpResponseInfo* info)>;

// Callback that allows the test to substitute its own implementation
// of URLRequestJob to handle the request.
using DohJobMakerCallback = base::RepeatingCallback<std::unique_ptr<URLRequestJob>(URLRequest* request, SocketDataProvider* data_provider)>;
...
public:
    URLRequestMockDohJob(URLRequest* request, SocketDataProvider* data_provider, ResponseModifierCallback response_modifier = ResponseModifierCallback(),
    ...
    static void MatchQueryData(URLRequest* request, SocketDataProvider* data_provider)
    {
        ...
        if (request->method() == "GET") {
            ...
            EXPECT_TRUE(base::Base64UrlDecode(encoded_query, base::Base64UrlDecodePolicy::IGNORE_PADDING, &decoded_query));
            ...
            for (auto& reader : *readers) {
                ...
                decoded_query += std::string(base::as_string_view(byte_reader->bytes()));
            }
        }
    ...
    void Start() override
    {
        ...
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(&URLRequestMockDohJob::StartAsync, weak_factory_.GetWeakPtr()));
    }
    ...
    void GetResponseInfo(HttpResponseInfo* info) override
    {
        ...
        info->headers = base::MakeRefCounted<HttpResponseHeaders>(HttpUtil::AssembleRawHeaders(raw_headers));
    ...
    void OnReadComplete(const MockRead& data) override
    {
        ...
        ReadRawDataComplete(DoBufferCopy(data.data, data.data_len, pending_buf_, pending_buf_size_));
    }
...
private:
    ...
    int DoBufferCopy(const char* data, int data_len, IOBuffer* buf, int buf_size)
    {
...
public:
    ...
    void ConfigureNumServers(size_t num_servers)
    {
        ...
        for (size_t i = 0; i < num_servers; ++i) {
            config_.nameservers.emplace_back(IPAddress(192, 168, 1, i), dns_protocol::kDefaultPort);
        }
    }
    ...
    // HTTPResponse.
    void ConfigureDohServers(bool use_post, size_t num_doh_servers = 1, bool make_available = true)
    {
        GURL url(URLRequestMockDohJob::GetMockHttpsUrl("doh_test"));
        URLRequestFilter* filter = URLRequestFilter::GetInstance();
        filter->AddHostnameInterceptor(url.scheme(), url.host(), std::make_unique<DohJobInterceptor>(this));
        ...
        for (size_t i = 0; i < num_doh_servers; ++i) {
            templates.push_back(URLRequestMockDohJob::GetMockHttpsUrl(base::StringPrintf("doh_test_%zu", i)) + (use_post ? "" : "{?dns}"));
        }
        config_.doh_config = *DnsOverHttpsConfig::FromTemplatesForTesting(std::move(templates));
        ConfigureFactory();

        if (make_available) {
            for (size_t server_index = 0; server_index < num_doh_servers; ++server_index) {
                resolve_context_->RecordServerSuccess(server_index, true /* is_doh_server */, session_.get());
            }
        }
    }
    ...
    {
        session_
            = base::MakeRefCounted<DnsSession>(config_, base::BindRepeating(&DnsTransactionTestBase::GetNextId, base::Unretained(this)), nullptr /* NetLog */);
        resolve_context_->InvalidateCachesAndPerSessionData(session_.get(), false /* network_change */);
        transaction_factory_ = DnsTransactionFactory::CreateFactory(session_.get());
    }

    void AddSocketData(std::unique_ptr<DnsSocketData> data, bool enqueue_transaction_id = true)
    {
    ...
    void AddQueryAndResponseNoWrite(uint16_t id, const char* dotted_name, uint16_t qtype, IoMode mode, Transport transport,
        const OptRecordRdata* opt_rdata = nullptr, DnsQuery::PaddingStrategy padding_strategy = DnsQuery::PaddingStrategy::NONE)
    {
        CHECK(socket_factory_.get());
        auto data = std::make_unique<DnsSocketData>(id, dotted_name, qtype, mode, transport, opt_rdata, padding_strategy);
        data->ClearWrites();
    ...
    void AddQueryAndResponse(uint16_t id, const char* dotted_name, uint16_t qtype, base::span<const uint8_t> response_data, IoMode mode, Transport transport,
        const OptRecordRdata* opt_rdata = nullptr, DnsQuery::PaddingStrategy padding_strategy = DnsQuery::PaddingStrategy::NONE,
    ...
    void AddQueryAndErrorResponse(uint16_t id, const char* dotted_name, uint16_t qtype, int error, IoMode mode, Transport transport,
        const OptRecordRdata* opt_rdata = nullptr, DnsQuery::PaddingStrategy padding_strategy = DnsQuery::PaddingStrategy::NONE,
        bool enqueue_transaction_id = true)
    {
        CHECK(socket_factory_.get());
    ...
    void AddAsyncQueryAndResponse(
        uint16_t id, const char* dotted_name, uint16_t qtype, base::span<const uint8_t> data, const OptRecordRdata* opt_rdata = nullptr)
    {
        AddQueryAndResponse(id, dotted_name, qtype, data, ASYNC, Transport::UDP, opt_rdata);
    }

    void AddSyncQueryAndResponse(
        uint16_t id, const char* dotted_name, uint16_t qtype, base::span<const uint8_t> data, const OptRecordRdata* opt_rdata = nullptr)
    {
        AddQueryAndResponse(id, dotted_name, qtype, data, SYNCHRONOUS, Transport::UDP, opt_rdata);
    }

    // Add expected query of |dotted_name| and |qtype| and no response.
    void AddHangingQuery(const char* dotted_name, uint16_t qtype, DnsQuery::PaddingStrategy padding_strategy = DnsQuery::PaddingStrategy::NONE,
        uint16_t id = base::RandInt(0, std::numeric_limits<uint16_t>::max()), bool enqueue_transaction_id = true)
    {
        auto data = std::make_unique<DnsSocketData>(id, dotted_name, qtype, ASYNC, Transport::UDP, nullptr /* opt_rdata */, padding_strategy);
        AddSocketData(std::move(data), enqueue_transaction_id);
    }
    ...
    void AddQueryAndRcode(const char* dotted_name, uint16_t qtype, int rcode, IoMode mode, Transport trans,
        DnsQuery::PaddingStrategy padding_strategy = DnsQuery::PaddingStrategy::NONE, uint16_t id = base::RandInt(0, std::numeric_limits<uint16_t>::max()),
        bool enqueue_transaction_id = true)
    {
        CHECK_NE(dns_protocol::kRcodeNOERROR, rcode);
        auto data = std::make_unique<DnsSocketData>(id, dotted_name, qtype, mode, trans, nullptr /* opt_rdata */, padding_strategy);
        data->AddRcode(rcode, mode);
        AddSocketData(std::move(data), enqueue_transaction_id);
    }

    void AddAsyncQueryAndRcode(const char* dotted_name, uint16_t qtype, int rcode)
    {
        AddQueryAndRcode(dotted_name, qtype, rcode, ASYNC, Transport::UDP);
    }

    void AddSyncQueryAndRcode(const char* dotted_name, uint16_t qtype, int rcode)
    {
    ...
    void CheckServerOrder(const size_t* servers, size_t num_attempts)
    {
        ...
        for (size_t i = 0; i < num_attempts; ++i) {
            if (servers[i] < num_insecure_nameservers) {
                ...
                EXPECT_EQ(socket_factory_->remote_endpoints_[i].insecure_nameserver.value(), session_->config().nameservers[servers[i]]);
            } else {
                // Check secure server match.
                EXPECT_EQ(socket_factory_->remote_endpoints_[i].secure_nameserver.value(),
                    session_->config().doh_config.servers()[servers[i] - num_insecure_nameservers]);
            }
        }
    }
    ...
    std::unique_ptr<URLRequestJob> MaybeInterceptRequest(URLRequest* request)
    {
        ...
        for (auto server : config_.doh_config.servers()) {
            if (server_found)
                break;
            std::string url_base = GetURLFromTemplateWithoutParameters(server.server_template());
        ...
        EXPECT_TRUE(request->isolation_info().network_isolation_key().IsTransient());
        ...
        if (!expect_multiple_isolation_infos_) {
            if (!isolation_info_) {
                isolation_info_ = std::make_unique<IsolationInfo>(request->isolation_info());
            } else {
                EXPECT_TRUE(isolation_info_->IsEqualForTesting(request->isolation_info()));
            }
        }
        ...
        EXPECT_THAT(request->extra_request_headers().GetHeader("Accept"), testing::Optional(std::string("application/dns-message")));

        EXPECT_THAT(request->extra_request_headers().GetHeader("Accept-Language"), testing::Optional(std::string("*")));

        EXPECT_THAT(request->extra_request_headers().GetHeader("User-Agent"), testing::Optional(std::string("Chrome")));
        ...
        return std::make_unique<URLRequestMockDohJob>(request, provider, response_modifier_, on_start_);
    }

    class DohJobInterceptor : public URLRequestInterceptor {
    public:
        explicit DohJobInterceptor(DnsTransactionTestBase* test)
            : test_(test)
        {
        }
    ...
    void SetUp() override
    {
        ...
        context_builder->set_client_socket_factory_for_testing(socket_factory_.get());
        request_context_ = context_builder->Build();
        resolve_context_ = std::make_unique<ResolveContext>(request_context_.get(), false /* enable_caching */);
    ...
    void set_expect_multiple_isolation_infos(bool expect_multiple_isolation_infos)
    {
...
class DnsTransactionTest : public DnsTransactionTestBase, public WithTaskEnvironment {
public:
    DnsTransactionTest() = default;
    ~DnsTransactionTest() override = default;
};

class DnsTransactionTestWithMockTime : public DnsTransactionTestBase, public WithTaskEnvironment {
protected:
...
{
    AddAsyncQueryAndResponse(0 /* id */, kT0HostName, kT0Qtype, kT0ResponseDatagram);

    TransactionHelper helper0(kT0RecordCount);
    helper0.StartTransaction(transaction_factory_.get(), kT0HostName, kT0Qtype, false /* secure */, resolve_context_.get());
...
TEST_F(DnsTransactionTest, LookupWithEDNSOption)
{
    ...
    transaction_factory_->AddEDNSOption(OptRecordRdata::UnknownOpt::CreateForTesting(123, "\xbe\xef"));
    expected_opt_rdata.AddOpt(OptRecordRdata::UnknownOpt::CreateForTesting(123, "\xbe\xef"));

    AddAsyncQueryAndResponse(0 /* id */, kT0HostName, kT0Qtype, kT0ResponseDatagram, &expected_opt_rdata);
...
TEST_F(DnsTransactionTest, LookupWithMultipleEDNSOptions)
{
    ...
    std::vector<std::pair<uint16_t, std::string>> params = { // Two options with the same code, to check that both are included.
        std::pair<uint16_t, std::string>(1, "\xde\xad"), std::pair<uint16_t, std::string>(1, "\xbe\xef"),
        // Try a different code and different length of data.
        std::pair<uint16_t, std::string>(2, "\xff")
    };

    for (auto& param : params) {
        transaction_factory_->AddEDNSOption(OptRecordRdata::UnknownOpt::CreateForTesting(param.first, param.second));
        expected_opt_rdata.AddOpt(OptRecordRdata::UnknownOpt::CreateForTesting(param.first, param.second));
    }
...
{
    AddAsyncQueryAndResponse(0 /* id */, kT0HostName, kT0Qtype, kT0ResponseDatagram);
    AddAsyncQueryAndResponse(1 /* id */, kT1HostName, kT1Qtype, kT1ResponseDatagram);

    TransactionHelper helper0(kT0RecordCount);
    helper0.StartTransaction(transaction_factory_.get(), kT0HostName, kT0Qtype, false /* secure */, resolve_context_.get());
    TransactionHelper helper1(kT1RecordCount);
    helper1.StartTransaction(transaction_factory_.get(), kT1HostName, kT1Qtype, false /* secure */, resolve_context_.get());
...
{
    AddQueryAndResponseNoWrite(0 /* id */, kT0HostName, kT0Qtype, ASYNC, Transport::UDP, nullptr);
...
TEST_F(DnsTransactionTest, MismatchedResponseSync)
{
    ...
    auto data = std::make_unique<DnsSocketData>(0 /* id */, kT0HostName, kT0Qtype, SYNCHRONOUS, Transport::UDP);
    data->AddResponseData(kT1ResponseDatagram, SYNCHRONOUS);
    AddSocketData(std::move(data));

    // Second attempt receives valid response synchronously.
    auto data1 = std::make_unique<DnsSocketData>(0 /* id */, kT0HostName, kT0Qtype, SYNCHRONOUS, Transport::UDP);
...
TEST_F(DnsTransactionTest, MismatchedResponseAsync)
{
    ...
    auto data0 = std::make_unique<DnsSocketData>(0 /* id */, kT0HostName, kT0Qtype, ASYNC, Transport::UDP);
    ...
    auto data1 = std::make_unique<DnsSocketData>(0 /* id */, kT0HostName, kT0Qtype, ASYNC, Transport::UDP);
...
TEST_F(DnsTransactionTest, MismatchedResponseFail)
{
    ...
    // allowed.
    AddAsyncQueryAndResponse(1 /* id */, kT0HostName, kT0Qtype, kT0ResponseDatagram);
...
TEST_F(DnsTransactionTest, ZeroSizeResponseAsync)
{
    ...
    auto data0 = std::make_unique<DnsSocketData>(/*id=*/0, kT0HostName, kT0Qtype, ASYNC, Transport::UDP);
    data0->AddReadError(0, ASYNC);
    AddSocketData(std::move(data0));

    // Second attempt receives valid response asynchronously.
    auto data1 = std::make_unique<DnsSocketData>(/*id=*/0, kT0HostName, kT0Qtype, ASYNC, Transport::UDP);
    data1->AddResponseData(kT0ResponseDatagram, ASYNC);
    AddSocketData(std::move(data1));
...
TEST_F(DnsTransactionTestWithMockTime, Timeout_FastTimeout)
{
    ...
    std::unique_ptr<DnsTransaction> transaction = transaction_factory_->CreateTransaction(
        kT0HostName, kT0Qtype, NetLogWithSource(), false /* secure */, SecureDnsMode::kOff, resolve_context_.get(), true /* fast_timeout */);
    ...
    base::RunLoop().RunUntilIdle();
    EXPECT_FALSE(helper0.has_completed());
    FastForwardBy(resolve_context_->NextClassicFallbackPeriod(0, 0, session_.get()));
    EXPECT_FALSE(helper0.has_completed());
    FastForwardBy(resolve_context_->NextClassicFallbackPeriod(0, 1, session_.get()));
    EXPECT_FALSE(helper0.has_completed());
    FastForwardBy(resolve_context_->NextClassicFallbackPeriod(0, 2, session_.get()));
    EXPECT_TRUE(helper0.has_completed());
}
...
TEST_F(DnsTransactionTest, SuffixSearchAboveNdots)
{
    ...
    helper0.StartTransaction(transaction_factory_.get(), "x.y.z", dns_protocol::kTypeA, false /* secure */, resolve_context_.get());
    helper0.RunUntilComplete();
...
TEST_F(DnsTransactionTest, SuffixSearchBelowNdots)
{
    ...
    AddAsyncQueryAndRcode("x.a", dns_protocol::kTypeA, dns_protocol::kRcodeNXDOMAIN);
    ...
    AddAsyncQueryAndRcode("x", dns_protocol::kTypeAAAA, dns_protocol::kRcodeNXDOMAIN);

    TransactionHelper helper0(ERR_NAME_NOT_RESOLVED);
    helper0.StartTransaction(transaction_factory_.get(), "x.y", dns_protocol::kTypeA, false /* secure */, resolve_context_.get());
    helper0.RunUntilComplete();

    // A single-label name.
    TransactionHelper helper1(ERR_NAME_NOT_RESOLVED);
    helper1.StartTransaction(transaction_factory_.get(), "x", dns_protocol::kTypeA, false /* secure */, resolve_context_.get());
    helper1.RunUntilComplete();

    // A fully-qualified name.
    TransactionHelper helper2(ERR_NAME_NOT_RESOLVED);
    helper2.StartTransaction(transaction_factory_.get(), "x.", dns_protocol::kTypeAAAA, false /* secure */, resolve_context_.get());
...
{
    // Responses for first transaction.
    AddAsyncQueryAndRcode("x", dns_protocol::kTypeA, dns_protocol::kRcodeNXDOMAIN);

    // A fully-qualified name.
    TransactionHelper helper0(ERR_NAME_NOT_RESOLVED);
    helper0.StartTransaction(transaction_factory_.get(), "x.", dns_protocol::kTypeA, false /* secure */, resolve_context_.get());
    helper0.RunUntilComplete();

    // A single label name is not even attempted.
    TransactionHelper helper1(ERR_DNS_SEARCH_EMPTY);
    helper1.StartTransaction(transaction_factory_.get(), "singlelabel", dns_protocol::kTypeA, false /* secure */, resolve_context_.get());
    helper1.RunUntilComplete();
}
...
TEST_F(DnsTransactionTest, DontAppendToMultiLabelName)
{
    ...
    TransactionHelper helper1(ERR_NAME_NOT_RESOLVED);
    helper1.StartTransaction(transaction_factory_.get(), "x.y", dns_protocol::kTypeA, false /* secure */, resolve_context_.get());
    helper1.RunUntilComplete();

    TransactionHelper helper2(ERR_NAME_NOT_RESOLVED);
    helper2.StartTransaction(transaction_factory_.get(), "x", dns_protocol::kTypeA, false /* secure */, resolve_context_.get());
    helper2.RunUntilComplete();
}
...
TEST_F(DnsTransactionTest, SuffixSearchStop)
{
    ...
    AddAsyncQueryAndRcode("x.y.z", dns_protocol::kTypeA, dns_protocol::kRcodeNXDOMAIN);
    AddAsyncQueryAndRcode("x.y.z.a", dns_protocol::kTypeA, dns_protocol::kRcodeNXDOMAIN);
    AddAsyncQueryAndResponse(0 /* id */, "x.y.z.b", dns_protocol::kTypeA, kResponseNoData);
...
TEST_F(DnsTransactionTest, SyncFirstQuery)
{
    ...
    AddSyncQueryAndResponse(0 /* id */, kT0HostName, kT0Qtype, kT0ResponseDatagram);
...
TEST_F(DnsTransactionTest, SyncFirstQueryWithSearch)
{
    ...
    AddSyncQueryAndRcode("www.lab.ccs.neu.edu", kT2Qtype, dns_protocol::kRcodeNXDOMAIN);
    // "www.ccs.neu.edu"
    AddAsyncQueryAndResponse(2 /* id */, kT2HostName, kT2Qtype, kT2ResponseDatagram);

    TransactionHelper helper0(kT2RecordCount);
    helper0.StartTransaction(transaction_factory_.get(), "www", kT2Qtype, false /* secure */, resolve_context_.get());
    helper0.RunUntilComplete();
}
...
TEST_F(DnsTransactionTest, SyncSearchQuery)
{
    ...
    AddAsyncQueryAndRcode("www.lab.ccs.neu.edu", dns_protocol::kTypeA, dns_protocol::kRcodeNXDOMAIN);
    AddSyncQueryAndResponse(2 /* id */, kT2HostName, kT2Qtype, kT2ResponseDatagram);
...
TEST_F(DnsTransactionTest, ConnectFailure)
{
    ...
    helper0.StartTransaction(transaction_factory_.get(), "www.chromium.org", dns_protocol::kTypeA, false /* secure */, resolve_context_.get());
    helper0.RunUntilComplete();
...
TEST_F(DnsTransactionTest, HttpsGetLookup)
{
    ...
    AddQueryAndResponse(0, kT0HostName, kT0Qtype, kT0ResponseDatagram, SYNCHRONOUS, Transport::HTTPS, nullptr /* opt_rdata */,
        DnsQuery::PaddingStrategy::BLOCK_LENGTH_128, false /* enqueue_transaction_id */);
    TransactionHelper helper0(kT0RecordCount);
    helper0.StartTransaction(transaction_factory_.get(), kT0HostName, kT0Qtype, true /* secure */, resolve_context_.get());
    helper0.RunUntilComplete();
}
...
{
    ConfigureDohServers(false /* use_post */);
    AddQueryAndRcode(kT0HostName, kT0Qtype, dns_protocol::kRcodeSERVFAIL, SYNCHRONOUS, Transport::HTTPS, DnsQuery::PaddingStrategy::BLOCK_LENGTH_128,
        0 /* id */, false /* enqueue_transaction_id */);
...
TEST_F(DnsTransactionTest, HttpsGetMalformed)
{
    ...
    AddQueryAndResponse(0 /* id */, kT0HostName, kT0Qtype, kT1ResponseDatagram, SYNCHRONOUS, Transport::HTTPS, nullptr /* opt_rdata */,
        DnsQuery::PaddingStrategy::BLOCK_LENGTH_128, false /* enqueue_transaction_id */);
    TransactionHelper helper0(ERR_DNS_MALFORMED_RESPONSE);
    helper0.StartTransaction(transaction_factory_.get(), kT0HostName, kT0Qtype, true /* secure */, resolve_context_.get());
    helper0.RunUntilComplete();
...
{
    ConfigureDohServers(true /* use_post */);
    AddQueryAndResponse(0, kT0HostName, kT0Qtype, kT0ResponseDatagram, ASYNC, Transport::HTTPS, nullptr /* opt_rdata */,
        DnsQuery::PaddingStrategy::BLOCK_LENGTH_128, false /* enqueue_transaction_id */);
    TransactionHelper helper0(kT0RecordCount);
    helper0.StartTransaction(transaction_factory_.get(), kT0HostName, kT0Qtype, true /* secure */, resolve_context_.get());
    helper0.RunUntilComplete();
...
std::unique_ptr<URLRequestJob> DohJobMakerCallbackFailLookup(URLRequest* request, SocketDataProvider* data)
{
    URLRequestMockDohJob::MatchQueryData(request, data);
    return std::make_unique<URLRequestFailedJob>(request, URLRequestFailedJob::START, ERR_NAME_NOT_RESOLVED);
}
...
std::unique_ptr<URLRequestJob> DohJobMakerCallbackFailStart(URLRequest* request, SocketDataProvider* data)
{
    URLRequestMockDohJob::MatchQueryData(request, data);
    return std::make_unique<URLRequestFailedJob>(request, URLRequestFailedJob::START, ERR_FAILED);
}
...
std::unique_ptr<URLRequestJob> DohJobMakerCallbackFailSync(URLRequest* request, SocketDataProvider* data)
{
    URLRequestMockDohJob::MatchQueryData(request, data);
    return std::make_unique<URLRequestFailedJob>(request, URLRequestFailedJob::READ_SYNC, ERR_FAILED);
}
...
TEST_F(DnsTransactionTest, HttpsPostLookupFailSync)
{
    ...
    auto data = std::make_unique<DnsSocketData>(
        0, kT0HostName, kT0Qtype, SYNCHRONOUS, Transport::HTTPS, nullptr /* opt_rdata */, DnsQuery::PaddingStrategy::BLOCK_LENGTH_128);
...
std::unique_ptr<URLRequestJob> DohJobMakerCallbackFailAsync(URLRequest* request, SocketDataProvider* data)
{
    URLRequestMockDohJob::MatchQueryData(request, data);
    return std::make_unique<URLRequestFailedJob>(request, URLRequestFailedJob::READ_ASYNC, ERR_FAILED);
}
...
TEST_F(DnsTransactionTest, HttpsPostLookupAsyncThenSync)
{
    ...
    auto data = std::make_unique<DnsSocketData>(
        0, kT0HostName, kT0Qtype, SYNCHRONOUS, Transport::HTTPS, nullptr /* opt_rdata */, DnsQuery::PaddingStrategy::BLOCK_LENGTH_128);
    data->AddResponseData(base::span(kT0ResponseDatagram).first(20u), ASYNC);
    data->AddResponseData(base::span(kT0ResponseDatagram).subspan(20u), SYNCHRONOUS);
    AddSocketData(std::move(data), false /* enqueue_transaction_id */);
    TransactionHelper helper0(kT0RecordCount);
    helper0.StartTransaction(transaction_factory_.get(), kT0HostName, kT0Qtype, true /* secure */, resolve_context_.get());
    helper0.RunUntilComplete();
...
TEST_F(DnsTransactionTest, HttpsPostLookupSyncThenAsyncError)
{
    ...
    auto data = std::make_unique<DnsSocketData>(
        0, kT0HostName, kT0Qtype, SYNCHRONOUS, Transport::HTTPS, nullptr /* opt_rdata */, DnsQuery::PaddingStrategy::BLOCK_LENGTH_128);
    data->AddResponseData(base::span(kT0ResponseDatagram).first(20u), SYNCHRONOUS);
    data->AddReadError(ERR_FAILED, ASYNC);
    AddSocketData(std::move(data), false /* enqueue_transaction_id */);
    TransactionHelper helper0(ERR_FAILED);
    helper0.StartTransaction(transaction_factory_.get(), kT0HostName, kT0Qtype, true /* secure */, resolve_context_.get());
...
{
    ConfigureDohServers(true /* use_post */, 1 /* num_doh_servers */, false /* make_available */);
    ASSERT_FALSE(resolve_context_->GetDohServerAvailability(0u /* doh_server_index */, session_.get()));
...
TEST_F(DnsTransactionTest, HttpsMarkHttpsBad)
{
    ...
    AddQueryAndErrorResponse(0, kT0HostName, kT0Qtype, ERR_CONNECTION_REFUSED, SYNCHRONOUS, Transport::HTTPS, nullptr /* opt_rdata */,
        DnsQuery::PaddingStrategy::BLOCK_LENGTH_128, false /* enqueue_transaction_id */);
    AddQueryAndErrorResponse(0, kT0HostName, kT0Qtype, ERR_CONNECTION_REFUSED, SYNCHRONOUS, Transport::HTTPS, nullptr /* opt_rdata */,
        DnsQuery::PaddingStrategy::BLOCK_LENGTH_128, false /* enqueue_transaction_id */);
    AddQueryAndResponse(0, kT0HostName, kT0Qtype, kT0ResponseDatagram, ASYNC, Transport::HTTPS, nullptr /* opt_rdata */,
        DnsQuery::PaddingStrategy::BLOCK_LENGTH_128, false /* enqueue_transaction_id */);
    AddQueryAndErrorResponse(0 /* id */, kT0HostName, kT0Qtype, ERR_CONNECTION_REFUSED, SYNCHRONOUS, Transport::HTTPS, nullptr /* opt_rdata */,
        DnsQuery::PaddingStrategy::BLOCK_LENGTH_128, false /* enqueue_transaction_id */);
    AddQueryAndErrorResponse(0 /* id */, kT0HostName, kT0Qtype, ERR_CONNECTION_REFUSED, SYNCHRONOUS, Transport::HTTPS, nullptr /* opt_rdata */,
        DnsQuery::PaddingStrategy::BLOCK_LENGTH_128, false /* enqueue_transaction_id */);
    AddQueryAndResponse(0 /* id */, kT0HostName, kT0Qtype, kT0ResponseDatagram, ASYNC, Transport::HTTPS, nullptr /* opt_rdata */,
        DnsQuery::PaddingStrategy::BLOCK_LENGTH_128, false /* enqueue_transaction_id */);
    ...
    {
        std::unique_ptr<DnsServerIterator> classic_itr = resolve_context_->GetClassicDnsIterator(session_->config(), session_.get());
        std::unique_ptr<DnsServerIterator> doh_itr = resolve_context_->GetDohIterator(session_->config(), SecureDnsMode::kAutomatic, session_.get());
    ...
    helper1.StartTransaction(transaction_factory_.get(), kT0HostName, kT0Qtype, true /* secure */, resolve_context_.get());
    helper1.RunUntilComplete();
...
TEST_F(DnsTransactionTest, HttpsPostFailThenHTTPFallback)
{
    ...
    AddQueryAndRcode(kT0HostName, kT0Qtype, dns_protocol::kRcodeSERVFAIL, ASYNC, Transport::HTTPS, DnsQuery::PaddingStrategy::BLOCK_LENGTH_128, 0 /* id */,
...
{
    ConfigureDohServers(true /* use_post */, 2 /* num_doh_servers */, false /* make_available */);

    // Make just server 1 available.
    resolve_context_->RecordServerSuccess(1u /* server_index */, true /* is_doh_server*/, session_.get());
...
TEST_F(DnsTransactionTest, HttpsFailureThenNotAvailable_Automatic)
{
    ...
    ConfigureDohServers(true /* use_post */, 3 /* num_doh_servers */, false /* make_available */);

    // Make just server 0 available.
    resolve_context_->RecordServerSuccess(0u /* server_index */, true /* is_doh_server*/, session_.get());
...
TEST_F(DnsTransactionTest, HttpsFailureThenNotAvailable_Secure)
{
    ...
    {
        std::unique_ptr<DnsServerIterator> doh_itr = resolve_context_->GetDohIterator(session_->config(), SecureDnsMode::kSecure, session_.get());
...
TEST_F(DnsTransactionTest, MaxHttpsFailures_NonConsecutive)
{
    ...
    for (size_t i = 0; i < ResolveContext::kAutomaticModeFailureLimit - 1; i++) {
        ...
        failure.StartTransaction(transaction_factory_.get(), kT0HostName, kT0Qtype, true /* secure */, resolve_context_.get());
    ...
    success.StartTransaction(transaction_factory_.get(), kT0HostName, kT0Qtype, true /* secure */, resolve_context_.get());
    ...
    last_failure.StartTransaction(transaction_factory_.get(), kT0HostName, kT0Qtype, true /* secure */, resolve_context_.get());
...
TEST_F(DnsTransactionTest, SuccessfulTransactionStartedBeforeUnavailable)
{
    ...
    auto data = std::make_unique<DnsSocketData>(
        0, kT0HostName, kT0Qtype, ASYNC, Transport::HTTPS, nullptr /* opt_rdata */, DnsQuery::PaddingStrategy::BLOCK_LENGTH_128);
    ...
    delayed_success.StartTransaction(transaction_factory_.get(), kT0HostName, kT0Qtype, true /* secure */, resolve_context_.get());
    base::RunLoop().RunUntilIdle();
    ...
    EXPECT_FALSE(resolve_context_->GetDohServerAvailability(0u /* doh_server_index */, session_.get()));
    ...
    EXPECT_TRUE(resolve_context_->GetDohServerAvailability(0u /* doh_server_index */, session_.get()));
}
...
public:
    CookieCallback()
        : loop_to_quit_(std::make_unique<base::RunLoop>())
    {
    }
    ...
    void GetCookieListCallback(const net::CookieAccessResultList& list, const net::CookieAccessResultList& excluded_cookies)
    {
...
TEST_F(DnsTransactionTest, HttpsPostTestNoCookies)
{
    ...
    AddQueryAndResponse(0, kT0HostName, kT0Qtype, kT0ResponseDatagram, SYNCHRONOUS, Transport::HTTPS, nullptr /* opt_rdata */,
        DnsQuery::PaddingStrategy::BLOCK_LENGTH_128, false /* enqueue_transaction_id */);
    AddQueryAndResponse(0 /* id */, kT0HostName, kT0Qtype, kT0ResponseDatagram, SYNCHRONOUS, Transport::HTTPS, nullptr /* opt_rdata */,
        DnsQuery::PaddingStrategy::BLOCK_LENGTH_128, false /* enqueue_transaction_id */);
    TransactionHelper helper0(kT0RecordCount);
    TransactionHelper helper1(kT0RecordCount);
    ...
    request_context_->cookie_store()->GetCookieListWithOptionsAsync(
        GURL(GetURLFromTemplateWithoutParameters(config_.doh_config.servers()[0].server_template())), CookieOptions::MakeAllInclusive(),
        CookiePartitionKeyCollection(), base::BindOnce(&CookieCallback::GetCookieListCallback, base::Unretained(&callback)));
    callback.WaitUntilDone();
    EXPECT_EQ(0u, callback.cookie_list_size());
    callback.Reset();
    GURL cookie_url(GetURLFromTemplateWithoutParameters(config_.doh_config.servers()[0].server_template()));
    auto cookie = CanonicalCookie::CreateForTesting(cookie_url, "test-cookie=you-still-fail", base::Time::Now());
    request_context_->cookie_store()->SetCanonicalCookieAsync(
        std::move(cookie), cookie_url, CookieOptions(), base::BindOnce(&CookieCallback::SetCookieCallback, base::Unretained(&callback)));
    helper1.StartTransaction(transaction_factory_.get(), kT0HostName, kT0Qtype, true /* secure */, resolve_context_.get());
    helper1.RunUntilComplete();
}
...
void MakeResponseWithBadRequestResponse(URLRequest* request, HttpResponseInfo* info)
{
...
TEST_F(DnsTransactionTest, HttpsPostWithBadRequestResponse)
{
    ...
    AddQueryAndResponse(0, kT0HostName, kT0Qtype, kT0ResponseDatagram, SYNCHRONOUS, Transport::HTTPS, nullptr /* opt_rdata */,
        DnsQuery::PaddingStrategy::BLOCK_LENGTH_128, false /* enqueue_transaction_id */);
    TransactionHelper helper0(ERR_DNS_MALFORMED_RESPONSE);
    SetResponseModifierCallback(base::BindRepeating(MakeResponseWithBadRequestResponse));
    helper0.StartTransaction(transaction_factory_.get(), kT0HostName, kT0Qtype, true /* secure */, resolve_context_.get());
    helper0.RunUntilComplete();
}
...
void MakeResponseRedirect(URLRequest* request, HttpResponseInfo* info)
{
    if (request->url_chain().size() < 2) {
        ...
        info->headers->AddHeader("Location", "/redirect-destination?" + request->url().query());
    }
}
...
void MakeResponseInsecureRedirect(URLRequest* request, HttpResponseInfo* info)
{
    if (request->url_chain().size() < 2) {
        info->headers->ReplaceStatusLine("HTTP/1.1 302 Found");
        const std::string location = URLRequestMockDohJob::GetMockHttpUrl("/redirect-destination?" + request->url().query());
...
TEST_F(DnsTransactionTest, HttpsGetRedirectToInsecureProtocol)
{
    ...
    AddQueryAndResponse(0, kT0HostName, kT0Qtype, kT0ResponseDatagram, SYNCHRONOUS, Transport::HTTPS, /*opt_rdata=*/nullptr,
        DnsQuery::PaddingStrategy::BLOCK_LENGTH_128,
        /*enqueue_transaction_id=*/false);
    TransactionHelper helper0(ERR_ABORTED);
    SetResponseModifierCallback(base::BindRepeating(MakeResponseInsecureRedirect));
    helper0.StartTransaction(transaction_factory_.get(), kT0HostName, kT0Qtype,
...
TEST_F(DnsTransactionTest, HttpsGetContentLengthTooLarge)
{
    ...
    SetResponseModifierCallback(
        base::BindLambdaForTesting([](URLRequest* request, HttpResponseInfo* info) { info->headers->AddHeader("Content-Length", "65536"); }));
    helper0.StartTransaction(transaction_factory_.get(), kT0HostName, kT0Qtype,
        /*secure=*/true, resolve_context_.get());
    helper0.RunUntilComplete();
    ASSERT_EQ(helper0.response(), nullptr);
...
TEST_F(DnsTransactionTest, HttpsGetResponseTooLargeWithoutContentLength)
{
    ...
    AddQueryAndResponse(0, kT0HostName, kT0Qtype, large_response, SYNCHRONOUS, Transport::HTTPS,
...
TEST_F(DnsTransactionTest, CanLookupDohServerName)
{
    ...
    ConfigureDohServers(true /* use_post */);
    AddQueryAndErrorResponse(0, kMockHostname, dns_protocol::kTypeA, ERR_NAME_NOT_RESOLVED, SYNCHRONOUS, Transport::HTTPS, nullptr /* opt_rdata */,
        DnsQuery::PaddingStrategy::BLOCK_LENGTH_128, false /* enqueue_transaction_id */);
    TransactionHelper helper0(ERR_NAME_NOT_RESOLVED);
    helper0.StartTransaction(transaction_factory_.get(), "mock", dns_protocol::kTypeA, true /* secure */, resolve_context_.get());
    helper0.RunUntilComplete();
}
...
TEST_F(DnsTransactionTestWithMockTime, SlowHttpsResponse_SingleAttempt)
{
    ...
    ASSERT_LT(resolve_context_->NextDohFallbackPeriod(0 /* doh_server_index */, session_.get()),
        resolve_context_->SecureTransactionTimeout(SecureDnsMode::kSecure, session_.get()));

    // Simulate a slow response by using an ERR_IO_PENDING read error to delay
    // until SequencedSocketData::Resume() is called.
    auto data = std::make_unique<DnsSocketData>(
        0 /* id */, kT0HostName, kT0Qtype, ASYNC, Transport::HTTPS, nullptr /* opt_rdata */, DnsQuery::PaddingStrategy::BLOCK_LENGTH_128);
    data->AddReadError(ERR_IO_PENDING, ASYNC);
    data->AddResponseData(kT0ResponseDatagram, ASYNC);
    SequencedSocketData* sequenced_socket_data = data->GetProvider();
    AddSocketData(std::move(data), false /* enqueue_transaction_id */);
    ...
    std::unique_ptr<DnsTransaction> transaction = transaction_factory_->CreateTransaction(
        kT0HostName, kT0Qtype, NetLogWithSource(), true /* secure */, SecureDnsMode::kSecure, resolve_context_.get(), false /* fast_timeout */);

    helper.StartTransaction(std::move(transaction));
    base::RunLoop().RunUntilIdle();
    ASSERT_FALSE(helper.has_completed());
    FastForwardBy(resolve_context_->NextDohFallbackPeriod(0 /* doh_server_index */, session_.get()));
...
TEST_F(DnsTransactionTestWithMockTime, SlowHttpsResponse_SingleAttempt_FastTimeout)
{
    config_.doh_attempts = 1;
    ConfigureDohServers(false /* use_post */);

    AddHangingQuery(kT0HostName, kT0Qtype, DnsQuery::PaddingStrategy::BLOCK_LENGTH_128, 0 /* id */, false /* enqueue_transaction_id */);

    TransactionHelper helper(ERR_DNS_TIMED_OUT);
    std::unique_ptr<DnsTransaction> transaction = transaction_factory_->CreateTransaction(
        kT0HostName, kT0Qtype, NetLogWithSource(), true /* secure */, SecureDnsMode::kSecure, resolve_context_.get(), true /* fast_timeout */);
    helper.StartTransaction(std::move(transaction));
    base::RunLoop().RunUntilIdle();
    ASSERT_FALSE(helper.has_completed());
...
TEST_F(DnsTransactionTestWithMockTime, HttpsTimeout)
{
    ...
    base::TimeDelta timeout = resolve_context_->SecureTransactionTimeout(SecureDnsMode::kSecure, session_.get());
...
TEST_F(DnsTransactionTestWithMockTime, HttpsTimeout2)
{
    ...
    base::TimeDelta fallback_period = resolve_context_->NextDohFallbackPeriod(0 /* doh_server_index */, session_.get());
    ...
    fallback_period = resolve_context_->NextDohFallbackPeriod(0 /* doh_server_index */, session_.get());
...
TEST_F(DnsTransactionTestWithMockTime, LongHttpsTimeouts)
{
    ...
    ASSERT_GT(kNumAttempts * resolve_context_->NextDohFallbackPeriod(0 /* doh_server_index */, session_.get()),
        resolve_context_->SecureTransactionTimeout(SecureDnsMode::kSecure, session_.get()));
...
{
    AddAsyncQueryAndRcode(kT0HostName, kT0Qtype, dns_protocol::kRcodeNOERROR | dns_protocol::kFlagTC);
    AddQueryAndResponse(0 /* id */, kT0HostName, kT0Qtype, kT0ResponseDatagram, ASYNC, Transport::TCP);
...
TEST_F(DnsTransactionTest, TcpLookup_LowEntropy)
{
    ...
    for (int i = 0; i <= DnsUdpTracker::kPortReuseThreshold; ++i) {
        AddQueryAndResponse(0 /* id */, kT0HostName, kT0Qtype, kT0ResponseDatagram, ASYNC, Transport::UDP);
    }
    ...
    for (int i = 0; i <= DnsUdpTracker::kPortReuseThreshold; ++i) {
        ...
        udp_helper.StartTransaction(transaction_factory_.get(), kT0HostName, kT0Qtype, false /* secure */, resolve_context_.get());
...
{
    AddAsyncQueryAndRcode(kT0HostName, kT0Qtype, dns_protocol::kRcodeNOERROR | dns_protocol::kFlagTC);
    AddQueryAndRcode(kT0HostName, kT0Qtype, dns_protocol::kRcodeSERVFAIL, ASYNC, Transport::TCP);
...
{
    AddAsyncQueryAndRcode(kT0HostName, kT0Qtype, dns_protocol::kRcodeNOERROR | dns_protocol::kFlagTC);
    auto data = std::make_unique<DnsSocketData>(0 /* id */, kT0HostName, kT0Qtype, ASYNC, Transport::TCP);
    ...
    data->AddResponseWithLength(std::make_unique<DnsResponse>(kT0ResponseDatagram, 0), ASYNC, static_cast<uint16_t>(kT0QuerySize - 1));
    AddSocketData(std::move(data));
...
TEST_F(DnsTransactionTestWithMockTime, TcpTimeout_UdpRetry)
{
    ConfigureFactory();
    AddAsyncQueryAndRcode(kT0HostName, kT0Qtype, dns_protocol::kRcodeNOERROR | dns_protocol::kFlagTC);
    AddSocketData(std::make_unique<DnsSocketData>(1 /* id */, kT0HostName, kT0Qtype, ASYNC, Transport::TCP));
...
TEST_F(DnsTransactionTest, TCPReadReturnsZeroAsync)
{
    ...
    data->AddResponseWithLength(
        std::make_unique<DnsResponse>(response_data.first(response_data.size() - 1), 0), ASYNC, static_cast<uint16_t>(response_data.size()));
...
TEST_F(DnsTransactionTest, TCPReadReturnsZeroSynchronous)
{
    ...
    // Return all but the last byte of the response.
    data->AddResponseWithLength(
        std::make_unique<DnsResponse>(response_data.first(response_data.size() - 1), 0), SYNCHRONOUS, static_cast<uint16_t>(response_data.size()));
    // Then return a 0-length read.
...
TEST_F(DnsTransactionTest, MismatchedThenRefusedThenTCP)
{
    ...
    // Attempt 2.
    AddQueryAndErrorResponse(0 /* id */, kT0HostName, kT0Qtype, ERR_CONNECTION_REFUSED, SYNCHRONOUS, Transport::UDP);
...
TEST_F(DnsTransactionTest, InvalidQuery)
{
    ...
    helper0.StartTransaction(transaction_factory_.get(), ".", dns_protocol::kTypeA, false /* secure */, resolve_context_.get());
    helper0.RunUntilComplete();

    TransactionHelper helper1(ERR_INVALID_ARGUMENT);
    helper1.StartTransaction(transaction_factory_.get(), "foo,bar.com", dns_protocol::kTypeA, false /* secure */, resolve_context_.get());
    helper1.RunUntilComplete();
}
...
TEST_F(DnsTransactionTest, CheckAsync)
{
    ...
    TransactionHelper helper0(kT0RecordCount);
    bool started = false;
    SetUrlRequestStartedCallback(base::BindLambdaForTesting([&] { started = true; }));
    helper0.StartTransaction(transaction_factory_.get(), kT0HostName, kT0Qtype, true /* secure */, resolve_context_.get());
...
TEST_F(DnsTransactionTestWithMockTime, ProbeUntilSuccess)
{
    ...
    AddQueryAndErrorResponse(0 /* id */, kT4HostName, kT4Qtype, ERR_CONNECTION_REFUSED, SYNCHRONOUS, Transport::HTTPS, nullptr /* opt_rdata */,
        DnsQuery::PaddingStrategy::BLOCK_LENGTH_128, false /* enqueue_transaction_id */);
    AddQueryAndErrorResponse(0 /* id */, kT4HostName, kT4Qtype, ERR_CONNECTION_REFUSED, SYNCHRONOUS, Transport::HTTPS, nullptr /* opt_rdata */,
        DnsQuery::PaddingStrategy::BLOCK_LENGTH_128, false /* enqueue_transaction_id */);
    AddQueryAndResponse(0 /* id */, kT4HostName, kT4Qtype, kT4ResponseDatagram, ASYNC, Transport::HTTPS, nullptr /* opt_rdata */,
        DnsQuery::PaddingStrategy::BLOCK_LENGTH_128, false /* enqueue_transaction_id */);

    std::unique_ptr<DnsProbeRunner> runner = transaction_factory_->CreateDohProbeRunner(resolve_context_.get());
...
TEST_F(DnsTransactionTestWithMockTime, ProbeCreationTriggersSuccessMetric)
{
    ...
    ASSERT_EQ(kDohProbeHostname, kT4HostName);
    AddQueryAndResponse(/*id=*/0, kT4HostName, kT4Qtype, kT4ResponseDatagram, ASYNC, Transport::HTTPS,
        /*opt_rdata=*/nullptr, DnsQuery::PaddingStrategy::BLOCK_LENGTH_128,
        /*enqueue_transaction_id=*/false);

    // The metric timer should not have started yet.
    EXPECT_FALSE(resolve_context_->doh_autoupgrade_metrics_timer_is_running_for_testing());
    ...
    // after the timeout.
    EXPECT_TRUE(resolve_context_->doh_autoupgrade_metrics_timer_is_running_for_testing());
    ...
    FastForwardBy(ResolveContext::kDohAutoupgradeSuccessMetricTimeout + base::Milliseconds(1));

    EXPECT_FALSE(resolve_context_->doh_autoupgrade_metrics_timer_is_running_for_testing());

    histogram_tester.ExpectUniqueSample("Net.DNS.ResolveContext.DohAutoupgrade.Other.Status", DohServerAutoupgradeStatus::kSuccessWithNoPriorFailures, 1);
}

TEST_F(DnsTransactionTestWithMockTime, ProbeAttemptConnectionFailureAffectsHistograms)
{
    config_.secure_dns_mode = SecureDnsMode::kAutomatic;
    ConfigureDohServers(/*use_post=*/true, /*num_doh_servers=*/1,
        /*make_available=*/false);
    ASSERT_EQ(kDohProbeHostname, kT4HostName);
    AddQueryAndErrorResponse(/*id=*/0, kT4HostName, kT4Qtype, ERR_CONNECTION_REFUSED, SYNCHRONOUS, Transport::HTTPS, /*opt_rdata=*/nullptr,
        DnsQuery::PaddingStrategy::BLOCK_LENGTH_128,
        /*enqueue_transaction_id=*/false);
    ...
    FastForwardBy(ResolveContext::kDohAutoupgradeSuccessMetricTimeout + base::Milliseconds(1));
    histogram_tester.ExpectUniqueSample("Net.DNS.ResolveContext.DohAutoupgrade.Other.Status", DohServerAutoupgradeStatus::kFailureWithNoPriorSuccesses, 1);
    histogram_tester.ExpectUniqueSample("Net.DNS.DnsTransaction.SecureNotValidated.Other.FailureError", std::abs(Error::ERR_CONNECTION_REFUSED), 1);
}
...
TEST_F(DnsTransactionTestWithMockTime, ProbeAttemptServFailAffectsHistograms)
{
    ...
    ConfigureDohServers(/*use_post=*/true, /*num_doh_servers=*/1,
        /*make_available=*/false);
    ASSERT_EQ(kDohProbeHostname, kT4HostName);
    AddQueryAndRcode(kT4HostName, kT4Qtype, dns_protocol::kRcodeSERVFAIL, SYNCHRONOUS, Transport::HTTPS, DnsQuery::PaddingStrategy::BLOCK_LENGTH_128, /*id=*/0,
        /*enqueue_transaction_id=*/false);
    ...
    FastForwardBy(ResolveContext::kDohAutoupgradeSuccessMetricTimeout + base::Milliseconds(1));
    histogram_tester.ExpectUniqueSample("Net.DNS.ResolveContext.DohAutoupgrade.Other.Status", DohServerAutoupgradeStatus::kFailureWithNoPriorSuccesses, 1);
    histogram_tester.ExpectUniqueSample("Net.DNS.DnsTransaction.SecureNotValidated.Other.FailureError", std::abs(Error::ERR_DNS_SERVER_FAILED), 1);
}

TEST_F(DnsTransactionTestWithMockTime, ProbeAttemptEmptyResponseAffectsHistograms)
{
    config_.secure_dns_mode = SecureDnsMode::kAutomatic;
    ConfigureDohServers(/*use_post=*/true, /*num_doh_servers=*/1,
        /*make_available=*/false);
    ASSERT_EQ(kDohProbeHostname, kT4HostName);
    auto response = std::make_unique<DnsResponse>(BuildTestDnsResponse(kT4HostName, dns_protocol::kTypeA,
        /*answers=*/ {}));
    auto data = std::make_unique<DnsSocketData>(
        /*id=*/0, kT4HostName, dns_protocol::kTypeA, SYNCHRONOUS, Transport::HTTPS, /*opt_rdata=*/nullptr, DnsQuery::PaddingStrategy::BLOCK_LENGTH_128);
    ...
    histogram_tester.ExpectUniqueSample(
        "Net.DNS.DnsTransaction.SecureNotValidated.Other.FailureError", std::abs(Error::ERR_DNS_SECURE_PROBE_RECORD_INVALID), 1);
}
...
TEST_F(DnsTransactionTestWithMockTime, HungProbe)
{
    ...
    // called.
    auto data = std::make_unique<DnsSocketData>(
        0 /* id */, kT4HostName, kT4Qtype, ASYNC, Transport::HTTPS, nullptr /* opt_rdata */, DnsQuery::PaddingStrategy::BLOCK_LENGTH_128);
    data->AddReadError(ERR_IO_PENDING, ASYNC);
...
TEST_F(DnsTransactionTestWithMockTime, ProbeMultipleServers)
{
    ...
    ASSERT_FALSE(resolve_context_->GetDohServerAvailability(0u /* doh_server_index */, session_.get()));
    ASSERT_FALSE(resolve_context_->GetDohServerAvailability(1u /* doh_server_index */, session_.get()));
    ...
    RunUntilIdle();
    EXPECT_TRUE(resolve_context_->GetDohServerAvailability(0u /* doh_server_index */, session_.get()));
    EXPECT_FALSE(resolve_context_->GetDohServerAvailability(1u /* doh_server_index */, session_.get()));

    // On second round of probing, probes for first server should self-cancel and
    // second server should become available.
    FastForwardBy(runner->GetDelayUntilNextProbeForTest(0u /* doh_server_index */));
    EXPECT_EQ(runner->GetDelayUntilNextProbeForTest(0u /* doh_server_index */), base::TimeDelta());
    FastForwardBy(runner->GetDelayUntilNextProbeForTest(1u /* doh_server_index */));
    EXPECT_TRUE(resolve_context_->GetDohServerAvailability(1u /* doh_server_index */, session_.get()));
...
TEST_F(DnsTransactionTestWithMockTime, MultipleProbeRunners)
{
    ...
    std::unique_ptr<DnsProbeRunner> runner1 = transaction_factory_->CreateDohProbeRunner(resolve_context_.get());
    std::unique_ptr<DnsProbeRunner> runner2 = transaction_factory_->CreateDohProbeRunner(resolve_context_.get());
...
TEST_F(DnsTransactionTestWithMockTime, MultipleProbeRunners_SeparateContexts)
{
    ...
    context2.InvalidateCachesAndPerSessionData(session_.get(), false /* network_change */);

    std::unique_ptr<DnsProbeRunner> runner1 = transaction_factory_->CreateDohProbeRunner(resolve_context_.get());
    std::unique_ptr<DnsProbeRunner> runner2 = transaction_factory_->CreateDohProbeRunner(&context2);
    ...
    {
        std::unique_ptr<DnsServerIterator> doh_itr2 = context2.GetDohIterator(session_->config(), SecureDnsMode::kAutomatic, session_.get());
...
{
    ConfigureDohServers(true /* use_post */, 1 /* num_doh_servers */, false /* make_available */);
    AddQueryAndResponse(0 /* id */, kT4HostName, kT4Qtype, kT4ResponseDatagram, SYNCHRONOUS, Transport::HTTPS, nullptr /* opt_rdata */,
        DnsQuery::PaddingStrategy::BLOCK_LENGTH_128, false /* enqueue_transaction_id */);
...
TEST_F(DnsTransactionTestWithMockTime, RestartFinishedProbe)
{
    ...
    for (int i = 0; i < ResolveContext::kAutomaticModeFailureLimit; ++i) {
        resolve_context_->RecordServerFailure(0u /* server_index */, true /* is_doh_server */, ERR_FAILED, session_.get());
    }
    ASSERT_FALSE(resolve_context_->GetDohServerAvailability(0u /* doh_server_index */, session_.get()));
    runner->Start(false /* network_change */);
...
TEST_F(DnsTransactionTestWithMockTime, RejectsQueryingLongNames)
{
    ...
    TransactionHelper helper0(ERR_INVALID_ARGUMENT);
    helper0.StartTransaction(transaction_factory_.get(), long_dotted_name.c_str(), dns_protocol::kTypeA, false /* secure */, resolve_context_.get());
    helper0.RunUntilComplete();
}
...
TEST_F(DnsTransactionTestWithMockTime, TcpConnectionRefusedAfterFallback)
{
    ...
    auto data1 = std::make_unique<DnsSocketData>(0 /* id */, kT0HostName, kT0Qtype, ASYNC, Transport::TCP);
    ...
    auto data2 = std::make_unique<DnsSocketData>(0 /* id */, kT0HostName, kT0Qtype, ASYNC, Transport::TCP);
    ...
    base::TimeDelta timeout = resolve_context_->NextClassicFallbackPeriod(0 /* classic_server_index */, 0 /* attempt */, session_.get());
...
{
    ConfigureDohServers(false /* use_post */, 2 /* num_doh_servers */, true /* make_available */);
    ...
    base::TimeDelta timeout = resolve_context_->NextDohFallbackPeriod(0 /* doh_server_index */, session_.get());
    FastForwardBy(timeout);
