...
HostCache::Entry ParseHostnameResult(const std::string& host, uint16_t port)
{
    ...
    return HostCache::Entry(OK, std::vector<HostPortPair>({ HostPortPair(host, port) }), HostCache::Entry::SOURCE_UNKNOWN);
}
} // namespace
...
public:
    ...
    void Start()
    {
        ...
        int flags = MDnsTransaction::SINGLE_RESULT | MDnsTransaction::QUERY_CACHE | MDnsTransaction::QUERY_NETWORK;
        // If |this| is destroyed, destruction of |internal_transaction_| should
        // cancel and prevent invocation of OnComplete.
        std::unique_ptr<MDnsTransaction> inner_transaction = task_->mdns_client_->CreateTransaction(DnsQueryTypeToQtype(query_type_), task_->hostname_, flags,
            base::BindRepeating(&HostResolverMdnsTask::Transaction::OnComplete, base::Unretained(this)));
    ...
    bool IsDone() const
    {
        return results_.error() != ERR_IO_PENDING;
    }
    bool IsError() const
    {
        return IsDone() && results_.error() != OK && results_.error() != ERR_NAME_NOT_RESOLVED;
    }
    const HostCache::Entry& results() const
    {
        return results_;
    }
...
private:
    void OnComplete(MDnsTransaction::Result result, const RecordParsed* parsed)
    {
        ...
        results_ = HostResolverMdnsTask::ParseResult(error, query_type_, parsed, task_->hostname_);
...
HostResolverMdnsTask::HostResolverMdnsTask(MDnsClient* mdns_client, std::string hostname, DnsQueryTypeSet query_types)
...
HostCache::Entry HostResolverMdnsTask::GetResults() const
{
    ...
    DCHECK(!completion_closure_);
    DCHECK(base::ranges::all_of(transactions_, [](const Transaction& t) { return t.IsDone(); }));

    auto found_error = base::ranges::find_if(transactions_, &Transaction::IsError);
    ...
    for (auto it = ++transactions_.begin(); it != transactions_.end(); ++it) {
        combined_results = HostCache::Entry::MergeEntries(std::move(combined_results), it->results());
    }
...
// static
HostCache::Entry HostResolverMdnsTask::ParseResult(int error, DnsQueryType query_type, const RecordParsed* parsed, const std::string& expected_hostname)
{
    if (error != OK) {
    ...
    case DnsQueryType::HTTPS:
        ...
        NOTREACHED();
    case DnsQueryType::A:
        return HostCache::Entry(OK, { IPEndPoint(parsed->rdata<net::ARecordRdata>()->address(), 0) },
            /*aliases=*/ {}, HostCache::Entry::SOURCE_UNKNOWN);
    case DnsQueryType::AAAA:
        return HostCache::Entry(OK, { IPEndPoint(parsed->rdata<net::AAAARecordRdata>()->address(), 0) },
            /*aliases=*/ {}, HostCache::Entry::SOURCE_UNKNOWN);
    case DnsQueryType::TXT:
        return HostCache::Entry(OK, parsed->rdata<net::TxtRecordRdata>()->texts(), HostCache::Entry::SOURCE_UNKNOWN);
    case DnsQueryType::PTR:
        return ParseHostnameResult(parsed->rdata<PtrRecordRdata>()->ptrdomain(), 0 /* port */);
    case DnsQueryType::SRV:
        return ParseHostnameResult(parsed->rdata<SrvRecordRdata>()->target(), parsed->rdata<SrvRecordRdata>()->port());
    }
}
...
void HostResolverMdnsTask::CheckCompletion(bool post_needed)
{
    ...
    if (base::ranges::any_of(transactions_, [](const Transaction& t) { return t.IsError(); })) {
        Complete(post_needed);
        return;
    }

    if (base::ranges::all_of(transactions_, [](const Transaction& t) { return t.IsDone(); })) {
        Complete(post_needed);
        return;
    }
}
