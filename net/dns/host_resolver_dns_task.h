...
public:
    ...
    struct SingleTransactionResults {
        ...
        SingleTransactionResults& operator=(const SingleTransactionResults&) = delete;
    ...
    class Delegate {
    public:
        virtual void OnDnsTaskComplete(base::TimeTicks start_time, bool allow_fallback, HostCache::Entry results, bool secure) = 0;
        ...
        virtual void OnIntermediateTransactionsComplete(std::optional<SingleTransactionResults> single_transaction_results) = 0;
    ...
    HostResolverDnsTask(DnsClient* client, HostResolver::Host host, NetworkAnonymizationKey anonymization_key, DnsQueryTypeSet query_types,
...
private:
    ...
    struct TransactionInfo {
        explicit TransactionInfo(DnsQueryType type, TransactionErrorBehavior error_behavior = TransactionErrorBehavior::kFallback);
        ~TransactionInfo();
    ...
    void OnDnsTransactionComplete(std::set<TransactionInfo>::iterator transaction_info_it, uint16_t request_port, int net_error, const DnsResponse* response);

    bool IsFatalTransactionFailure(int transaction_error, const TransactionInfo& transaction_info, const DnsResponse* response);

    void SortTransactionAndHandleResults(TransactionInfo transaction_info, Results transaction_results);
    ...
    void HandleTransactionResults(TransactionInfo transaction_info, Results transaction_results);

    void OnTransactionsFinished(std::optional<SingleTransactionResults> single_transaction_results);

    void OnSortComplete(base::TimeTicks sort_start_time, HostCache::Entry results, bool secure, bool success, std::vector<IPEndPoint> sorted);

    bool AnyPotentiallyFatalTransactionsRemain();

    void CancelNonFatalTransactions();

    void OnFailure(int net_error, bool allow_fallback, std::optional<base::TimeDelta> ttl = std::nullopt,
    ...
    bool AnyOfTypeTransactionsRemain(std::initializer_list<DnsQueryType> types) const;
