...
class DnsRequest {
public:
    DnsRequest(net::HostResolver* host_resolver, FuzzedDataProvider* data_provider, std::vector<std::unique_ptr<DnsRequest>>* dns_requests)
    ...
    static void CreateRequest(net::HostResolver* host_resolver, FuzzedDataProvider* data_provider, std::vector<std::unique_ptr<DnsRequest>>* dns_requests)
    {
        auto dns_request = std::make_unique<DnsRequest>(host_resolver, data_provider, dns_requests);
    ...
    static void WaitForRequestComplete(FuzzedDataProvider* data_provider, std::vector<std::unique_ptr<DnsRequest>>* dns_requests)
    {
        if (dns_requests->empty())
            return;
        uint32_t index = data_provider->ConsumeIntegralInRange<uint32_t>(0, dns_requests->size() - 1);
    ...
    static void CancelRequest(net::HostResolver* host_resolver, FuzzedDataProvider* data_provider, std::vector<std::unique_ptr<DnsRequest>>* dns_requests)
    {
        if (dns_requests->empty())
            return;
        uint32_t index = data_provider->ConsumeIntegralInRange<uint32_t>(0, dns_requests->size() - 1);
...
private:
    void OnCallback(int result)
    {
        ...
        for (auto request = dns_requests_->begin(); request != dns_requests_->end(); ++request) {
            if (request->get() != this)
                continue;
    ...
    int Start()
    {
        ...
        std::advance(query_types_it, data_provider_->ConsumeIntegralInRange<size_t>(0, net::kDnsQueryTypes.size() - 1));
        parameters.dns_query_type = query_types_it->first;

        parameters.initial_priority
            = static_cast<net::RequestPriority>(data_provider_->ConsumeIntegralInRange<int32_t>(net::MINIMUM_PRIORITY, net::MAXIMUM_PRIORITY));

        parameters.source = data_provider_->PickValueInArray(net::kHostResolverSources);
        ...
        parameters.cache_usage = data_provider_->ConsumeBool() ? net::HostResolver::ResolveHostParameters::CacheUsage::ALLOWED
        ...
        if (net::IsAddressType(parameters.dns_query_type) && parameters.source != net::HostResolverSource::DNS
        ...
        request_ = host_resolver_->CreateRequest(net::HostPortPair(hostname, 80), net::NetworkAnonymizationKey(), net::NetLogWithSource(), parameters);
        int rv = request_->Start(base::BindOnce(&DnsRequest::OnCallback, base::Unretained(this)));
    ...
    static bool IsParameterCombinationAllowed(net::HostResolver::ResolveHostParameters parameters)
    {
        // SYSTEM requests only support address types.
        if (parameters.source == net::HostResolverSource::SYSTEM && !net::IsAddressType(parameters.dns_query_type)) {
            return false;
        }
        ...
        if (parameters.source == net::HostResolverSource::MULTICAST_DNS
            && (parameters.include_canonical_name || parameters.loopback_only
                || parameters.cache_usage != net::HostResolver::ResolveHostParameters::CacheUsage::ALLOWED
...
{
    {
        ...
        options.max_concurrent_resolves = data_provider.ConsumeIntegralInRange(1, 8);
        ...
        std::unique_ptr<net::ContextHostResolver> host_resolver
            = net::CreateFuzzedContextHostResolver(options, net::NetLog::Get(), &data_provider, enable_caching);
        ...
        while (!done) {
            ...
            case 0:
                ...
                break;
            case 1:
                DnsRequest::CreateRequest(host_resolver.get(), &data_provider, &dns_requests);
                break;
            case 2:
                DnsRequest::WaitForRequestComplete(&data_provider, &dns_requests);
                break;
            case 3:
                DnsRequest::CancelRequest(host_resolver.get(), &data_provider, &dns_requests);
                break;
            }
        }
    }
