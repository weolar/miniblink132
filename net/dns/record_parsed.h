...
public:
    ...
    static std::unique_ptr<const RecordParsed> CreateFrom(DnsRecordParser* parser, base::Time time_created);
    ...
    const RecordRdata* rdata_for_testing() const
    {
        return rdata_.get();
    }
...
private:
    RecordParsed(const std::string& name, uint16_t type, uint16_t klass, uint32_t ttl, std::unique_ptr<const RecordRdata> rdata, base::Time time_created);
