...
std::unique_ptr<HostResolver::ResolveHostRequest> MappedHostResolver::CreateRequest(url::SchemeHostPort host, NetworkAnonymizationKey network_anonymization_key,
    NetLogWithSource source_net_log, std::optional<ResolveHostParameters> optional_parameters)
{
    ...
    case HostMappingRules::RewriteResult::kRewritten:
        ...
        return impl_->CreateRequest(
            url::SchemeHostPort(rewritten_url), std::move(network_anonymization_key), std::move(source_net_log), std::move(optional_parameters));
    ...
    case HostMappingRules::RewriteResult::kNoMatchingRule:
        return impl_->CreateRequest(std::move(host), std::move(network_anonymization_key), std::move(source_net_log), std::move(optional_parameters));
    }
}

std::unique_ptr<HostResolver::ResolveHostRequest> MappedHostResolver::CreateRequest(const HostPortPair& host,
    ...
    const std::optional<ResolveHostParameters>& optional_parameters)
{
    ...
    return impl_->CreateRequest(rewritten, network_anonymization_key, source_net_log, optional_parameters);
}

std::unique_ptr<HostResolver::ServiceEndpointRequest> MappedHostResolver::CreateServiceEndpointRequest(
    Host host, NetworkAnonymizationKey network_anonymization_key, NetLogWithSource net_log, ResolveHostParameters parameters)
{
    ...
    case HostMappingRules::RewriteResult::kRewritten:
        ...
        return impl_->CreateServiceEndpointRequest(
            Host(url::SchemeHostPort(rewritten_url)), std::move(network_anonymization_key), std::move(net_log), std::move(parameters));
    case HostMappingRules::RewriteResult::kInvalidRewrite:
        // Treat any invalid mapping as if it was "^NOTFOUND" (which should itself
        // result in `kInvalidRewrite`).
        return CreateFailingServiceEndpointRequest(ERR_NAME_NOT_RESOLVED);
    case HostMappingRules::RewriteResult::kNoMatchingRule:
        return impl_->CreateServiceEndpointRequest(std::move(host), std::move(network_anonymization_key), std::move(net_log), std::move(parameters));
    }
}

std::unique_ptr<HostResolver::ProbeRequest> MappedHostResolver::CreateDohProbeRequest()
{
