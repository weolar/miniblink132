...
void OnSortComplete(std::vector<IPEndPoint>* sorted_buf, CompletionOnceCallback callback, bool success, std::vector<IPEndPoint> sorted)
{
    if (success)
...
TEST(AddressSorterTest, Sort)
{
    ...
    sorter->Sort(endpoints, base::BindOnce(&OnSortComplete, &result, callback.callback()));
