...
concept HasConnectionEndpointMetadata = requires(T t) {
    {
        t.metadata
    } -> std::same_as<ConnectionEndpointMetadata&>;
};
...
public:
    ...
    public:
        ...
        // requests.
        virtual const std::vector<HostResolverEndpointResult>* GetEndpointResults() const = 0;
        ...
        NET_EXPORT virtual const std::vector<bool>* GetExperimentalResultsForTesting() const;
        ...
        // invoking the callback or by returning a result other than
        // |ERR_IO_PENDING|.
        virtual const std::optional<HostCache::EntryStaleness>& GetStaleInfo() const = 0;
    ...
    struct NET_EXPORT ManagerOptions {
        ...
        size_t max_system_retry_attempts = HostResolverSystemTask::Params::kDefaultRetryAttempts;
    ...
    public:
        ...
        virtual std::unique_ptr<HostResolver> CreateResolver(HostResolverManager* manager, std::string_view host_mapping_rules, bool enable_caching);
    ...
    public:
        ...
        public:
            ...
            virtual void OnAddressResult(MdnsListenerUpdateType update_type, DnsQueryType result_type, IPEndPoint address) = 0;
            virtual void OnTextResult(MdnsListenerUpdateType update_type, DnsQueryType result_type, std::vector<std::string> text_records) = 0;
            virtual void OnHostnameResult(MdnsListenerUpdateType update_type, DnsQueryType result_type, HostPortPair host) = 0;

            // For results which may be valid MDNS but are not handled/parsed by
            // HostResolver, e.g. pointers to the root domain.
            virtual void OnUnhandledResult(MdnsListenerUpdateType update_type, DnsQueryType result_type) = 0;
        };
    ...
    virtual std::unique_ptr<ResolveHostRequest> CreateRequest(url::SchemeHostPort host, NetworkAnonymizationKey network_anonymization_key,
        NetLogWithSource net_log, std::optional<ResolveHostParameters> optional_parameters)
        = 0;
    ...
    virtual std::unique_ptr<ResolveHostRequest> CreateRequest(const HostPortPair& host, const NetworkAnonymizationKey& network_anonymization_key,
        const NetLogWithSource& net_log, const std::optional<ResolveHostParameters>& optional_parameters)
        = 0;
    ...
    virtual std::unique_ptr<MdnsListener> CreateMdnsListener(const HostPortPair& host, DnsQueryType query_type);
    ...
    static std::unique_ptr<HostResolver> CreateResolver(HostResolverManager* manager, std::string_view host_mapping_rules = "", bool enable_caching = true);
    ...
    static std::unique_ptr<HostResolver> CreateStandaloneResolver(
        NetLog* net_log, std::optional<ManagerOptions> options = std::nullopt, std::string_view host_mapping_rules = "", bool enable_caching = true);
    ...
    static std::unique_ptr<ContextHostResolver> CreateStandaloneContextResolver(
        NetLog* net_log, std::optional<ManagerOptions> options = std::nullopt, bool enable_caching = true);
    ...
    static std::unique_ptr<HostResolver> CreateStandaloneNetworkBoundResolver(NetLog* net_log, handles::NetworkHandle network,
        std::optional<ManagerOptions> options = std::nullopt, std::string_view host_mapping_rules = "", bool enable_caching = true);

    // Helpers for interacting with HostCache and ProcResolver.
    static AddressFamily DnsQueryTypeSetToAddressFamily(DnsQueryTypeSet query_types);
    static HostResolverFlags ParametersToHostResolverFlags(const ResolveHostParameters& parameters);
    ...
    static AddressList EndpointResultToAddressList(base::span<const HostResolverEndpointResult> endpoints, const std::set<std::string>& aliases);
    ...
    // resolution.
    static bool MayUseNAT64ForIPv4Literal(HostResolverFlags flags, HostResolverSource source, const IPAddress& ip_address);
...
protected:
    ...
    static std::unique_ptr<ServiceEndpointRequest> CreateFailingServiceEndpointRequest(int error);
};
