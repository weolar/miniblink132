...
public:
    ...
    using ResultsOrError = base::expected<std::set<std::unique_ptr<HostResolverInternalResult>>, ExtractionError>;

    // References must stay alive for the life of the created extractor.
    explicit DnsResponseResultExtractor(const DnsResponse& response, const base::Clock& clock = *base::DefaultClock::GetInstance(),
    ...
    DnsResponseResultExtractor& operator=(const DnsResponseResultExtractor&) = delete;
    ...
    ResultsOrError ExtractDnsResults(DnsQueryType query_type, std::string_view original_domain_name, uint16_t request_port) const;
