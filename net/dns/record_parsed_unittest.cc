...
{
    DnsRecordParser parser1(kT1ResponseDatagram, sizeof(dns_protocol::Header), kT1RecordCount);
    std::string dotted_qname;
    uint16_t qtype;
...
TEST(RecordParsedTest, ParseUnknownRdata)
{
    static const char kRecordData[] =
        ...
        // RDATA="garbage data"
        "garbage data";
    DnsRecordParser parser(base::byte_span_from_cstring(kRecordData), 0 /* offset */,
        /*num_records=*/1);
...
TEST(RecordParsedTest, EqualityHandlesUnknownRdata)
{
    ...
    std::unique_ptr<const RecordParsed> unknown_record = RecordParsed::CreateFrom(&parser, base::Time());
    ASSERT_TRUE(unknown_record);
    ASSERT_FALSE(unknown_record->rdata_for_testing());

    std::unique_ptr<const RecordParsed> known_record = RecordParsed::CreateFrom(&parser, base::Time());
    ASSERT_TRUE(known_record);
    ASSERT_TRUE(known_record->rdata_for_testing());

    EXPECT_TRUE(unknown_record->IsEqual(unknown_record.get(), false /* is_mdns */));
    EXPECT_TRUE(unknown_record->IsEqual(unknown_record.get(), true /* is_mdns */));
    ...
    EXPECT_FALSE(unknown_record->IsEqual(known_record.get(), false /* is_mdns */));
    EXPECT_FALSE(unknown_record->IsEqual(known_record.get(), true /* is_mdns */));
    EXPECT_FALSE(known_record->IsEqual(unknown_record.get(), false /* is_mdns */));
