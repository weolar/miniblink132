...
bool ParseAndCreateDnsQueryFromRawPacket(const uint8_t* data, size_t length, std::unique_ptr<DnsQuery>* out)
{
...
TEST(DnsQueryTest, CopiesAreIndependent)
{
    ...
    EXPECT_EQ(std::string_view(q1.io_buffer()->data(), q1.io_buffer()->size()), std::string_view(q2.io_buffer()->data(), q2.io_buffer()->size()));
...
{
    DnsQuery query(46 /* id */, kQName, dns_protocol::kTypeAAAA, nullptr /* opt_rdata */, DnsQuery::PaddingStrategy::BLOCK_LENGTH_128);
...
{
    std::optional<std::vector<uint8_t>> qname = dns_names_util::DottedNameToNetwork("really.long.domain.name.that.will.push.us.past.the.128.byte.block."
                                                                                    "size.because.it.would.be.nice.to.test.something.realy.long.like."
                                                                                    "that.com");
    ASSERT_TRUE(qname.has_value());
    DnsQuery query(112 /* id */, qname.value(), dns_protocol::kTypeAAAA, nullptr /* opt_rdata */, DnsQuery::PaddingStrategy::BLOCK_LENGTH_128);
...
TEST(DnsQueryParseTest, SingleQuestionForTypeARecord)
{
    ...
    EXPECT_TRUE(ParseAndCreateDnsQueryFromRawPacket(query_data, sizeof(query_data), &query));
...
TEST(DnsQueryParseTest, FailsInvalidQueries)
{
    const struct TestCase {
        ...
        size_t size;
    } testcases[] = { { kQueryTruncatedQuestion, std::size(kQueryTruncatedQuestion) }, { kQueryTwoQuestions, std::size(kQueryTwoQuestions) },
        { kQueryInvalidDNSDomainName1, std::size(kQueryInvalidDNSDomainName1) }, { kQueryInvalidDNSDomainName2, std::size(kQueryInvalidDNSDomainName2) } };
    std::unique_ptr<DnsQuery> query;
    for (const auto& testcase : testcases) {
        EXPECT_FALSE(ParseAndCreateDnsQueryFromRawPacket(testcase.data, testcase.size, &query));
    }
}

TEST(DnsQueryParseTest, ParsesLongName)
{
    const char kHeader[] = "\x6f\x15" // ID
                           "\x00\x00" // FLAGS
                           "\x00\x01" // 1 question
                           "\x00\x00" // 0 answers
    ...
    ASSERT_LE(long_name.size(), static_cast<size_t>(dns_protocol::kMaxNameLength));
...
TEST(DnsQueryParseTest, FailsTooLongName)
{
    ...
    ASSERT_GT(long_name.size(), static_cast<size_t>(dns_protocol::kMaxNameLength));
    long_name.append("\x00", 1);
