...
public:
    ...
    public:
        ...
        uint16_t info_code() const
        {
            return info_code_;
        }
        std::string_view extra_text() const
        {
            return extra_text_;
        }
    ...
    public:
        ...
        // classes).
        static std::unique_ptr<UnknownOpt> CreateForTesting(uint16_t code, std::string data);
    ...
    private:
        ...
        friend std::unique_ptr<OptRecordRdata> OptRecordRdata::Create(std::string_view data);
    };

    static constexpr uint16_t kOptsWithDedicatedClasses[] = { dns_protocol::kEdnsPadding, dns_protocol::kEdnsExtendedDnsError };
    ...
    const std::multimap<uint16_t, const std::unique_ptr<const Opt>>& opts() const
    {
        return opts_;
    }
