...
size_t DnsResourceRecord::CalculateRecordSize() const
{
    ...
    // DNSDomainFromDot.
    return name.size() + (has_final_dot ? 1 : 2) + net::dns_protocol::kResourceRecordSizeInBytesWithoutNameAndRData
...
DnsRecordParser::DnsRecordParser(base::span<const uint8_t> packet, size_t offset, size_t num_records)
    : packet_(packet)
...
unsigned DnsRecordParser::ReadName(const void* const vpos, std::string* out) const
{
    ...
    for (;;) {
        ...
        case dns_protocol::kLabelPointer: {
            if (packet_.size() < sizeof(uint16_t) || offset > packet_.size() - sizeof(uint16_t)) {
            ...
            uint16_t new_offset = base::U16FromBigEndian(packet_.subspan(offset).first<2u>());
...
bool DnsRecordParser::ReadRecord(DnsResourceRecord* out)
{
    ...
    uint16_t rdlen;
    if (reader.ReadU16BigEndian(out->type) && reader.ReadU16BigEndian(out->klass) && reader.ReadU32BigEndian(out->ttl) && //
        reader.ReadU16BigEndian(rdlen) && base::OptionalUnwrapTo(reader.Read(rdlen), out->rdata, [](auto span) { return base::as_string_view(span); })) {
...
bool DnsRecordParser::ReadQuestion(std::string& out_dotted_qname, uint16_t& out_qtype)
{
    ...
    out_qtype = base::U16FromBigEndian(packet_.subspan(cur_ + consumed).first<sizeof(uint16_t)>());
...
DnsResponse::DnsResponse(uint16_t id, bool is_authoritative, const std::vector<DnsResourceRecord>& answers,
    const std::vector<DnsResourceRecord>& authority_records, const std::vector<DnsResourceRecord>& additional_records, const std::optional<DnsQuery>& query,
    uint8_t rcode, bool validate_records, bool validate_names_as_internet_hostnames)
{
    ...
    size_t response_size = has_query ? sizeof(header) + query.value().question_size() : sizeof(header);
    // Add the size of all answers and additional records.
    auto do_accumulation = [](size_t cur_size, const DnsResourceRecord& record) { return cur_size + record.CalculateRecordSize(); };
    response_size = std::accumulate(answers.begin(), answers.end(), response_size, do_accumulation);
    response_size = std::accumulate(authority_records.begin(), authority_records.end(), response_size, do_accumulation);
    response_size = std::accumulate(additional_records.begin(), additional_records.end(), response_size, do_accumulation);
    ...
    for (const auto& answer : answers) {
        success &= WriteAnswer(&writer, answer, query, validate_records, validate_names_as_internet_hostnames);
        DCHECK(success);
    }
    // Start the Authority section.
    for (const auto& record : authority_records) {
        success &= WriteRecord(&writer, record, validate_records, validate_names_as_internet_hostnames);
        DCHECK(success);
    }
...
DnsResponse::DnsResponse()
    : io_buffer_(base::MakeRefCounted<IOBufferWithSize>(dns_protocol::kMaxUDPSize + 1))
...
DnsResponse::DnsResponse(base::span<const uint8_t> data, size_t answer_offset)
    ...
    , parser_(io_buffer_->span(), answer_offset, std::numeric_limits<size_t>::max())
{
    io_buffer_->span().copy_from(data);
}

// static
DnsResponse DnsResponse::CreateEmptyNoDataResponse(uint16_t id, bool is_authoritative, base::span<const uint8_t> qname, uint16_t qtype)
{
...
bool DnsResponse::InitParse(size_t nbytes, const DnsQuery& query)
{
    ...
    base::span<const uint8_t> subspan = io_buffer_->span().subspan(kHeaderSize, question.size());
    // Match the question section.
    if (question != base::as_string_view(subspan)) {
        return false;
    }

    std::optional<std::string> dotted_qname = dns_names_util::NetworkToDottedName(query.qname());
    ...
    parser_ = DnsRecordParser(io_buffer_->span().first(nbytes), kHeaderSize + question.size(), num_records);
    return true;
}
...
bool DnsResponse::InitParseWithoutQuery(size_t nbytes)
{
    ...
    parser_ = DnsRecordParser(io_buffer_->span().first(nbytes), kHeaderSize, num_records);
...
bool DnsResponse::WriteHeader(base::SpanWriter<uint8_t>* writer, const dns_protocol::Header& header)
{
    return writer->WriteU16BigEndian(header.id) && writer->WriteU16BigEndian(header.flags) && writer->WriteU16BigEndian(header.qdcount)
        && writer->WriteU16BigEndian(header.ancount) && writer->WriteU16BigEndian(header.nscount) && writer->WriteU16BigEndian(header.arcount);
}

bool DnsResponse::WriteQuestion(base::SpanWriter<uint8_t>* writer, const DnsQuery& query)
{
    return writer->Write(base::as_byte_span(query.question()));
}

bool DnsResponse::WriteRecord(base::SpanWriter<uint8_t>* writer, const DnsResourceRecord& record, bool validate_record, bool validate_name_as_internet_hostname)
{
    ...
    if (validate_record && !RecordRdata::HasValidSize(record.owned_rdata, record.type)) {
        VLOG(1) << "Invalid RDATA size for a record.";
        return false;
    }

    std::optional<std::vector<uint8_t>> domain_name = dns_names_util::DottedNameToNetwork(record.name, validate_name_as_internet_hostname);
    if (!domain_name.has_value()) {
        VLOG(1) << "Invalid dotted name (as " << (validate_name_as_internet_hostname ? "Internet hostname)." : "DNS name).");
        return false;
    }

    return writer->Write(domain_name.value()) && writer->WriteU16BigEndian(record.type) && writer->WriteU16BigEndian(record.klass)
        && writer->WriteU32BigEndian(record.ttl) && writer->WriteU16BigEndian(record.owned_rdata.size()) &&
        // Use the owned RDATA in the record to construct the response.
        writer->Write(base::as_byte_span(record.owned_rdata));
}

bool DnsResponse::WriteAnswer(base::SpanWriter<uint8_t>* writer, const DnsResourceRecord& answer, const std::optional<DnsQuery>& query, bool validate_record,
    bool validate_name_as_internet_hostname)
{
    ...
    if (validate_record && query.has_value() && answer.type != query.value().qtype() && answer.type != dns_protocol::kTypeCNAME) {
        VLOG(1) << "Mismatched answer resource record type and qtype.";
        return false;
    }
    return WriteRecord(writer, answer, validate_record, validate_name_as_internet_hostname);
}
