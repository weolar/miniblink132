...
HostResolverProc::HostResolverProc(scoped_refptr<HostResolverProc> previous, bool allow_fallback_to_system_or_default)
...
int HostResolverProc::Resolve(const std::string& host, AddressFamily address_family, HostResolverFlags host_resolver_flags, AddressList* addrlist,
    int* os_error, handles::NetworkHandle network)
{
    if (network == handles::kInvalidNetworkHandle)
        return Resolve(host, address_family, host_resolver_flags, addrlist, os_error);
    ...
    const std::string& host, AddressFamily address_family, HostResolverFlags host_resolver_flags, AddressList* addrlist, int* os_error)
{
    if (previous_proc_.get()) {
        return previous_proc_->Resolve(host, address_family, host_resolver_flags, addrlist, os_error);
    }
    ...
    return SystemHostResolverCall(host, address_family, host_resolver_flags, addrlist, os_error);
}
...
void HostResolverProc::SetPreviousProc(scoped_refptr<HostResolverProc> proc)
{
    ...
    previous_proc_ = (GetLastProc(proc.get()) == this) ? std::move(current_previous) : std::move(proc);
}
