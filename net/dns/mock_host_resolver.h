...
public:
    ...
    public:
        struct RuleKey {
            ...
            auto GetTuple() const
            {
                return std::tie(scheme, hostname_pattern, port, query_type, query_source);
            }
        ...
        struct RuleResult {
            RuleResult();
            explicit RuleResult(std::vector<HostResolverEndpointResult> endpoints, std::set<std::string> aliases = std::set<std::string>());
        ...
        explicit RuleResolver(std::optional<RuleResultOrError> default_result = std::nullopt);
        ~RuleResolver();
        ...
        const RuleResultOrError& Resolve(const Host& request_endpoint, DnsQueryTypeSet request_types, HostResolverSource request_source) const;
        ...
        void AddRule(std::string_view hostname_pattern, std::string_view ip_literal);
        ...
        // above.
        void AddIPLiteralRule(std::string_view hostname_pattern, std::string_view ip_literal, std::string_view canonical_name);
        void AddIPLiteralRuleWithDnsAliases(std::string_view hostname_pattern, std::string_view ip_literal, std::vector<std::string> dns_aliases);
        void AddIPLiteralRuleWithDnsAliases(std::string_view hostname_pattern, std::string_view ip_literal, std::set<std::string> dns_aliases);
        ...
        void AddRuleWithFlags(std::string_view host_pattern, std::string_view ip_literal, HostResolverFlags flags, std::vector<std::string> dns_aliases = {});
    ...
    public:
        ...
        bool IsDohProbeRunning() const
        {
            return !!doh_probe_request_;
        }
        ...
        {
            return num_non_local_resolves_;
        }
        ...
        void IncrementNumResolve()
        {
            ++num_resolve_;
        }
        void IncrementNumResolveFromCache()
        {
            ++num_resolve_from_cache_;
        }
        void IncrementNumNonLocalResolves()
        {
            ++num_non_local_resolves_;
        }
        void ClearDohProbeRequest()
        {
    ...
    RuleResolver* rules()
    {
        return &rule_resolver_;
    }

    scoped_refptr<const State> state() const
    {
        return state_;
    }
    ...
    std::unique_ptr<ResolveHostRequest> CreateRequest(url::SchemeHostPort host, NetworkAnonymizationKey network_anonymization_key, NetLogWithSource net_log,
        std::optional<ResolveHostParameters> optional_parameters) override;
    std::unique_ptr<ResolveHostRequest> CreateRequest(const HostPortPair& host, const NetworkAnonymizationKey& network_anonymization_key,
        const NetLogWithSource& net_log, const std::optional<ResolveHostParameters>& optional_parameters) override;
    std::unique_ptr<ServiceEndpointRequest> CreateServiceEndpointRequest(
        Host host, NetworkAnonymizationKey network_anonymization_key, NetLogWithSource net_log, ResolveHostParameters parameters) override;
    std::unique_ptr<ProbeRequest> CreateDohProbeRequest() override;
    std::unique_ptr<MdnsListener> CreateMdnsListener(const HostPortPair& host, DnsQueryType query_type) override;
    ...
    int LoadIntoCache(absl::variant<url::SchemeHostPort, HostPortPair> endpoint, const NetworkAnonymizationKey& network_anonymization_key,
    ...
    bool has_pending_requests() const
    {
        return state_->has_pending_requests();
    }
    ...
    const std::optional<NetworkAnonymizationKey>& last_request_network_anonymization_key()
    {
    ...
    bool IsDohProbeRunning() const
    {
        return state_->IsDohProbeRunning();
    }

    void TriggerMdnsListeners(const HostPortPair& host, DnsQueryType query_type, MdnsListenerUpdateType update_type, const IPEndPoint& address_result);
    void TriggerMdnsListeners(
    ...
    void TriggerMdnsListeners(const HostPortPair& host, DnsQueryType query_type, MdnsListenerUpdateType update_type, const HostPortPair& host_result);
    void TriggerMdnsListeners(const HostPortPair& host, DnsQueryType query_type, MdnsListenerUpdateType update_type);
...
private:
    ...
    // scenarios.
    MockHostResolverBase(bool use_caching, int cache_invalidation_num, RuleResolver rule_resolver);
    ...
    int ResolveFromIPLiteralOrCache(const Host& endpoint, const NetworkAnonymizationKey& network_anonymization_key, DnsQueryType dns_query_type,
        HostResolverFlags flags, HostResolverSource source, HostResolver::ResolveHostParameters::CacheUsage cache_usage,
    ...
    std::optional<NetworkAnonymizationKey> last_request_network_anonymization_key_;
...
public:
    explicit MockHostResolver(std::optional<RuleResolver::RuleResultOrError> default_result = std::nullopt)
        : MockHostResolverBase(/*use_caching=*/false,
            /*cache_invalidation_num=*/0, RuleResolver(std::move(default_result)))
    {
    }
...
public:
    ...
    // be read before it invalidates itself. Useful to force cache expiration
    // scenarios.
    explicit MockCachingHostResolver(int cache_invalidation_num = 0, std::optional<RuleResolver::RuleResultOrError> default_result = std::nullopt)
        : MockHostResolverBase(/*use_caching=*/true, cache_invalidation_num, RuleResolver(std::move(default_result)))
    {
    }
...
public:
    explicit MockHostResolverFactory(
        MockHostResolverBase::RuleResolver rules = MockHostResolverBase::RuleResolver(), bool use_caching = false, int cache_invalidation_num = 0);
    ...
    std::unique_ptr<HostResolver> CreateResolver(HostResolverManager* manager, std::string_view host_mapping_rules, bool enable_caching) override;
    std::unique_ptr<HostResolver> CreateStandaloneResolver(
        NetLog* net_log, const HostResolver::ManagerOptions& options, std::string_view host_mapping_rules, bool enable_caching) override;
...
public:
    ...
    // `previous`.
    explicit RuleBasedHostResolverProc(scoped_refptr<HostResolverProc> previous, bool allow_fallback = true);
    ...
    void AddRuleForAddressFamily(std::string_view host_pattern, AddressFamily address_family, std::string_view ip_literal);
    ...
    void AddIPLiteralRule(std::string_view host_pattern, std::string_view ip_literal, std::string_view canonical_name);
    ...
    void AddIPLiteralRuleWithDnsAliases(std::string_view host_pattern, std::string_view ip_literal, std::vector<std::string> dns_aliases);

    void AddRuleWithLatency(std::string_view host_pattern, std::string_view replacement, int latency_ms);
    ...
    void AddSimulatedFailure(std::string_view host, HostResolverFlags flags = HOST_RESOLVER_LOOPBACK_ONLY);

    // Simulate a lookup timeout failure for |host| (it also can be a pattern).
    void AddSimulatedTimeoutFailure(std::string_view host, HostResolverFlags flags = HOST_RESOLVER_LOOPBACK_ONLY);
    ...
    struct Rule {
        ...
        Rule(ResolverType resolver_type, std::string_view host_pattern, AddressFamily address_family, HostResolverFlags host_resolver_flags,
            std::string_view replacement, std::vector<std::string> dns_aliases, int latency_ms);
...
private:
    ...
    // pattern.
    std::map<std::string_view, size_t> num_resolves_per_host_pattern_ GUARDED_BY(rule_lock_);
...
public:
    ...
    public:
        ...
        int num_cancellations() const
        {
            return num_cancellations_;
        }

        void IncrementNumCancellations()
        {
            ++num_cancellations_;
        }
    ...
    int num_cancellations() const
    {
        return state_->num_cancellations();
    }

    // Return the corresponding values passed to the most recent call to
    // CreateRequest()
    const HostPortPair& last_host() const
    {
        return last_host_;
    }
    ...
    const scoped_refptr<const State> state() const
    {
        return state_;
    }
