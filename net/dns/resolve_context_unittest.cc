...
protected:
    ...
    scoped_refptr<DnsSession> CreateDnsSession(const DnsConfig& config)
    {
        ...
        return base::MakeRefCounted<DnsSession>(config, null_random_callback, nullptr /* netlog */);
    }
...
private:
    std::unique_ptr<MockClientSocketFactory> socket_factory_ = std::make_unique<MockClientSocketFactory>();
};
...
DnsConfig CreateDnsConfig(int num_servers, int num_doh_servers)
{
    ...
    for (int i = 0; i < num_servers; ++i) {
        IPEndPoint dns_endpoint(IPAddress(192, 168, 1, static_cast<uint8_t>(i)), dns_protocol::kDefaultPort);
        config.nameservers.push_back(dns_endpoint);
    }
    std::vector<std::string> templates;
    templates.reserve(num_doh_servers);
    for (int i = 0; i < num_doh_servers; ++i) {
        templates.push_back(base::StringPrintf("https://mock.http/doh_test_%d{?dns}", i));
    }
    config.doh_config = *DnsOverHttpsConfig::FromTemplatesForTesting(std::move(templates));
...
DnsConfig CreateDnsConfigWithKnownDohProviderConfig()
{
    ...
    config.doh_config = DnsOverHttpsConfig(GetDohUpgradeServersFromNameservers(config.nameservers));
...
{
    DnsConfig config = CreateDnsConfig(1 /* num_servers */, 3 /* num_doh_servers */);
    ...
    context.InvalidateCachesAndPerSessionData(session.get(), false /* network_change */);

    // Mark probe success for the "original" (pre-invalidation) session.
    context.RecordServerSuccess(1u /* server_index */, true /* is_doh_server */, session.get());
...
{
    DnsConfig config = CreateDnsConfig(2 /* num_servers */, 2 /* num_doh_servers */);
    scoped_refptr<DnsSession> session = CreateDnsSession(config);
    ...
    std::unique_ptr<DnsServerIterator> doh_itr = context.GetDohIterator(session->config(), SecureDnsMode::kAutomatic, session.get());
    ...
    histogram_tester.ExpectTotalCount("Net.DNS.ResolveContext.DohAutoupgrade.Other.Status", 0);
}
...
TEST_F(ResolveContextTest, DohServerAvailability_RecordedSuccess)
{
    ...
    // occurred (although this may not be strictly necessary).
    FastForwardBy(ResolveContext::kDohAutoupgradeSuccessMetricTimeout + base::Milliseconds(1));
    histogram_tester.ExpectTotalCount("Net.DNS.ResolveContext.DohAutoupgrade.Other.Status", 1);
    histogram_tester.ExpectBucketCount("Net.DNS.ResolveContext.DohAutoupgrade.Other.Status", DohServerAutoupgradeStatus::kSuccessWithNoPriorFailures, 1);
}
...
{
    DnsConfig config1 = CreateDnsConfig(1 /* num_servers */, 3 /* num_doh_servers */);
    scoped_refptr<DnsSession> session1 = CreateDnsSession(config1);

    DnsConfig config2 = CreateDnsConfig(2 /* num_servers */, 2 /* num_doh_servers */);
    scoped_refptr<DnsSession> session2 = CreateDnsSession(config2);

    auto request_context = CreateTestURLRequestContextBuilder()->Build();
    ResolveContext context(request_context.get(), true /* enable_caching */);
    context.InvalidateCachesAndPerSessionData(session2.get(), true /* network_change */);

    // Use current session to set a probe result.
    context.RecordServerSuccess(1u /* server_index */, true /* is_doh_server */, session2.get());

    std::unique_ptr<DnsServerIterator> doh_itr = context.GetDohIterator(session1->config(), SecureDnsMode::kAutomatic, session1.get());
    ...
    for (int i = 0; i < ResolveContext::kAutomaticModeFailureLimit; ++i) {
        context.RecordServerFailure(1u /* server_index */, true /* is_doh_server */, ERR_FAILED, session1.get());
    }
...
TEST_F(ResolveContextTest, DohServerIndexToUse)
{
    ...
    context.RecordServerSuccess(0u /* server_index */, true /* is_doh_server */, session.get());
    EXPECT_EQ(context.NumAvailableDohServers(session.get()), 1u);
    std::unique_ptr<DnsServerIterator> doh_itr = context.GetDohIterator(session->config(), SecureDnsMode::kAutomatic, session.get());
...
TEST_F(ResolveContextTest, DohServerIndexToUse_SecureMode)
{
    ...
    std::unique_ptr<DnsServerIterator> doh_itr = context.GetDohIterator(session->config(), SecureDnsMode::kSecure, session.get());
...
TEST_F(ResolveContextTest, DohServerAvailabilityNotification)
{
    ...
    for (int i = 0; i < ResolveContext::kAutomaticModeFailureLimit; ++i) {
        ...
        context.RecordServerFailure(0u /* server_index */, true /* is_doh_server */, ERR_FAILED, session.get());
        base::RunLoop().RunUntilIdle(); // Notifications are async.
        EXPECT_EQ(1, config_observer.dns_changed_calls());
    }
    ASSERT_EQ(1u, context.NumAvailableDohServers(session.get()));
    ...
    for (int i = 0; i < ResolveContext::kAutomaticModeFailureLimit; ++i) {
        ...
        context.RecordServerFailure(1u /* server_index */, true /* is_doh_server */, ERR_FAILED, session.get());
    }
    ASSERT_EQ(0u, context.NumAvailableDohServers(session.get()));
    base::RunLoop().RunUntilIdle(); // Notifications are async.
...
TEST_F(ResolveContextTest, InvalidateCachesAndPerSessionData)
{
    ...
    HostCache::Key key("example.com", DnsQueryType::UNSPECIFIED, 0, HostResolverSource::ANY, anonymization_key);
    context.host_cache()->Set(
        key, HostCache::Entry(OK, /*ip_endpoints=*/ {}, /*aliases=*/ {}, HostCache::Entry::SOURCE_UNKNOWN), tick_clock.NowTicks(), base::Seconds(10));
    ASSERT_TRUE(context.host_cache()->Lookup(key, tick_clock.NowTicks()));

    context.host_resolver_cache()->Set(
        std::make_unique<HostResolverInternalErrorResult>("domain.test", DnsQueryType::AAAA, tick_clock.NowTicks() + base::Seconds(10),
            clock.Now() + base::Seconds(10), HostResolverInternalResult::Source::kDns, ERR_NAME_NOT_RESOLVED),
        anonymization_key, HostResolverSource::DNS, /*secure=*/false);
    ASSERT_TRUE(context.host_resolver_cache()->Lookup("domain.test", anonymization_key));
    ...
    EXPECT_FALSE(context.host_resolver_cache()->Lookup("domain.test", anonymization_key));

    // Re-add to the caches and now add some DoH server status.
    context.host_cache()->Set(
        key, HostCache::Entry(OK, /*ip_endpoints=*/ {}, /*aliases=*/ {}, HostCache::Entry::SOURCE_UNKNOWN), tick_clock.NowTicks(), base::Seconds(10));
    context.host_resolver_cache()->Set(
        std::make_unique<HostResolverInternalErrorResult>("domain2.test", DnsQueryType::AAAA, tick_clock.NowTicks() + base::Seconds(10),
            clock.Now() + base::Seconds(10), HostResolverInternalResult::Source::kDns, ERR_NAME_NOT_RESOLVED),
        anonymization_key, HostResolverSource::DNS, /*secure=*/false);
    context.RecordServerSuccess(/*server_index=*/0u, /*is_doh_server=*/true, session.get());
    ASSERT_TRUE(context.host_cache()->Lookup(key, tick_clock.NowTicks()));
    ASSERT_TRUE(context.host_resolver_cache()->Lookup("domain2.test", anonymization_key));
    ...
    EXPECT_FALSE(context.host_cache()->Lookup(key, tick_clock.NowTicks()));
    EXPECT_FALSE(context.host_resolver_cache()->Lookup("domain2.test", anonymization_key));
...
TEST_F(ResolveContextTest, InvalidateCachesAndPerSessionDataSameSession)
{
    ...
    NetworkAnonymizationKey anonymization_key;
    HostCache::Key key("example.com", DnsQueryType::UNSPECIFIED, 0, HostResolverSource::ANY, anonymization_key);
    context.host_cache()->Set(key, HostCache::Entry(OK, /*ip_endpoints=*/ {}, /*aliases=*/ { "example.com" }, HostCache::Entry::SOURCE_UNKNOWN),
...
{
    ResolveContext context(nullptr /* url_request_context */, false /* enable_caching */);
    DnsConfig config = CreateDnsConfig(2 /* num_servers */, 2 /* num_doh_servers */);
    scoped_refptr<DnsSession> session = CreateDnsSession(config);
    context.InvalidateCachesAndPerSessionData(session.get(), false /* network_change */);
    ...
    for (int i = 0; i < config.attempts; i++) {
        std::unique_ptr<DnsServerIterator> classic_itr = context.GetClassicDnsIterator(session->config(), session.get());
        ...
        context.RecordServerFailure(1u /* server_index */, false /* is_doh_server */, ERR_FAILED, session.get());
    }
    ...
    context.RecordServerSuccess(1u /* server_index */, false /* is_doh_server */, session.get());
    {
        std::unique_ptr<DnsServerIterator> classic_itr = context.GetClassicDnsIterator(session->config(), session.get());
...
TEST_F(ResolveContextTest, Failures_DifferentSession)
{
    ...
    for (int i = 0; i < config1.attempts; i++) {
        std::unique_ptr<DnsServerIterator> classic_itr = context.GetClassicDnsIterator(session2->config(), session2.get());
        ...
        context.RecordServerFailure(1u /* server_index */, false /* is_doh_server */, ERR_FAILED, session1.get());
    }
    std::unique_ptr<DnsServerIterator> classic_itr = context.GetClassicDnsIterator(session2->config(), session2.get());
...
{
    ResolveContext context(nullptr /* url_request_context */, false /* enable_caching */);
    DnsConfig config = CreateDnsConfig(3 /* num_servers */, 2 /* num_doh_servers */);
    config.attempts = 1;
    scoped_refptr<DnsSession> session = CreateDnsSession(config);
    context.InvalidateCachesAndPerSessionData(session.get(), false /* network_change */);
    ...
    for (int i = 0; i < config.attempts; i++) {
        ...
        context.RecordServerFailure(0u /* server_index */, false /* is_doh_server */, ERR_FAILED, session.get());
        context.RecordServerFailure(1u /* server_index */, false /* is_doh_server */, ERR_FAILED, session.get());
    }
    {
        std::unique_ptr<DnsServerIterator> classic_itr = context.GetClassicDnsIterator(session->config(), session.get());
    ...
    // Expect failures to be reset on successful request.
    context.RecordServerSuccess(0u /* server_index */, false /* is_doh_server */, session.get());
    context.RecordServerSuccess(1u /* server_index */, false /* is_doh_server */, session.get());
    {
        std::unique_ptr<DnsServerIterator> classic_itr = context.GetClassicDnsIterator(session->config(), session.get());
...
TEST_F(ResolveContextTest, DohFailures_Consecutive)
{
    ...
    // occurred (although this may not be strictly necessary).
    FastForwardBy(ResolveContext::kDohAutoupgradeSuccessMetricTimeout + base::Milliseconds(1));
    histogram_tester.ExpectTotalCount("Net.DNS.ResolveContext.DohAutoupgrade.Other.Status",
        /*expected_count=*/1);
    histogram_tester.ExpectBucketCount("Net.DNS.ResolveContext.DohAutoupgrade.Other.Status", DohServerAutoupgradeStatus::kFailureWithSomePriorSuccesses,
        /*expected_count=*/1);
...
TEST_F(ResolveContextTest, DohFailures_NonConsecutive)
{
    ...
    histogram_tester.ExpectTotalCount("Net.DNS.ResolveContext.DohAutoupgrade.Other.Status",
        /*expected_count=*/1);
    histogram_tester.ExpectBucketCount("Net.DNS.ResolveContext.DohAutoupgrade.Other.Status", DohServerAutoupgradeStatus::kSuccessWithSomePriorFailures,
        /*expected_count=*/1);
...
TEST_F(ResolveContextTest, DohFailures_NeverSuccessful)
{
    ...
    context.RecordServerFailure(/*server_index=*/0u, /*is_doh_server=*/true, ERR_FAILED, session.get());
    ...
    histogram_tester.ExpectTotalCount("Net.DNS.ResolveContext.DohAutoupgrade.Other.Status",
        /*expected_count=*/1);
    histogram_tester.ExpectBucketCount("Net.DNS.ResolveContext.DohAutoupgrade.Other.Status", DohServerAutoupgradeStatus::kFailureWithNoPriorSuccesses,
        /*expected_count=*/1);
}
...
TEST_F(ResolveContextTest, DohFailures_NeverSuccessfulKnownProviderConfig)
{
    ...
    // occurred (although this may not be strictly necessary).
    FastForwardBy(ResolveContext::kDohAutoupgradeSuccessMetricTimeout + base::Milliseconds(1));
    histogram_tester.ExpectTotalCount("Net.DNS.ResolveContext.DohAutoupgrade.Google.Status",
        /*expected_count=*/1);
    histogram_tester.ExpectBucketCount("Net.DNS.ResolveContext.DohAutoupgrade.Google.Status", DohServerAutoupgradeStatus::kFailureWithNoPriorSuccesses,
        /*expected_count=*/1);
}
...
{
    ResolveContext context(nullptr /* url_request_context */, false /* enable_caching */);
    DnsConfig config = CreateDnsConfig(2 /* num_servers */, 3 /* num_doh_servers */);
    scoped_refptr<DnsSession> session = CreateDnsSession(config);
    context.InvalidateCachesAndPerSessionData(session.get(), false /* network_change */);

    context.RecordServerSuccess(0u /* server_index */, true /* is_doh_server */, session.get());
    context.RecordServerSuccess(1u /* server_index */, true /* is_doh_server */, session.get());
    context.RecordServerSuccess(2u /* server_index */, true /* is_doh_server */, session.get());
    ...
    histogram_tester.ExpectBucketCount("Net.DNS.ResolveContext.DohAutoupgrade.Other.Status", DohServerAutoupgradeStatus::kSuccessWithSomePriorFailures,
        /*expected_count=*/2);
    histogram_tester.ExpectBucketCount("Net.DNS.ResolveContext.DohAutoupgrade.Other.Status", DohServerAutoupgradeStatus::kSuccessWithNoPriorFailures,
        /*expected_count=*/1);
}
...
TEST_F(ResolveContextTest, FallbackPeriod_Default)
{
    ...
    base::TimeDelta delta = context.NextClassicFallbackPeriod(0 /* server_index */, 0 /* attempt */, session.get()) - config.fallback_period;
    EXPECT_LE(delta, base::Milliseconds(10));
    delta = context.NextDohFallbackPeriod(0 /* doh_server_index */, session.get()) - config.fallback_period;
    EXPECT_LE(delta, base::Milliseconds(10));
}
...
TEST_F(ResolveContextTest, FallbackPeriod_LongConfigured)
{
    ...
    EXPECT_EQ(context.NextClassicFallbackPeriod(0 /* server_index */, 0 /* attempt */, session.get()), config.fallback_period);
    EXPECT_EQ(context.NextDohFallbackPeriod(0 /* doh_server_index */, session.get()), config.fallback_period);
}
...
TEST_F(ResolveContextTest, FallbackPeriod_LongRtt)
{
    ...
    for (int i = 0; i < 50; ++i) {
        context.RecordRtt(0u /* server_index */, false /* is_doh_server */, base::Minutes(10), OK, session.get());
        context.RecordRtt(1u /* server_index */, true /* is_doh_server */, base::Minutes(10), OK, session.get());
    }

    // Expect servers with high recorded RTT to have increased fallback periods
    // (>10ms).
    base::TimeDelta delta = context.NextClassicFallbackPeriod(0u /* server_index */, 0 /* attempt */, session.get()) - config.fallback_period;
    EXPECT_GT(delta, base::Milliseconds(10));
    delta = context.NextDohFallbackPeriod(1u, session.get()) - config.fallback_period;
    EXPECT_GT(delta, base::Milliseconds(10));

    // Servers without recorded RTT expected to remain the same (<=10ms).
    delta = context.NextClassicFallbackPeriod(1u /* server_index */, 0 /* attempt */, session.get()) - config.fallback_period;
    EXPECT_LE(delta, base::Milliseconds(10));
    delta = context.NextDohFallbackPeriod(0u /* doh_server_index */, session.get()) - config.fallback_period;
    EXPECT_LE(delta, base::Milliseconds(10));
}
...
TEST_F(ResolveContextTest, FallbackPeriod_NoSession)
{
    ...
    base::TimeDelta delta = context.NextClassicFallbackPeriod(0u /* server_index */, 0 /* attempt */, session.get()) - config.fallback_period;
    EXPECT_LE(delta, base::Milliseconds(10));
    delta = context.NextDohFallbackPeriod(1u /* doh_server_index */, session.get()) - config.fallback_period;
    EXPECT_LE(delta, base::Milliseconds(10));
}
...
TEST_F(ResolveContextTest, FallbackPeriod_DifferentSession)
{
    ...
    for (int i = 0; i < 50; ++i) {
        context.RecordRtt(0u /* server_index */, false /* is_doh_server */, base::Minutes(10), OK, session2.get());
        context.RecordRtt(1u /* server_index */, true /* is_doh_server */, base::Minutes(10), OK, session2.get());
    }

    // Expect normal short fallback periods for other session.
    base::TimeDelta delta = context.NextClassicFallbackPeriod(0u /* server_index */, 0 /* attempt */, session1.get()) - config1.fallback_period;
    EXPECT_LE(delta, base::Milliseconds(10));
    delta = context.NextDohFallbackPeriod(0u /* doh_server_index */, session1.get()) - config1.fallback_period;
    EXPECT_LE(delta, base::Milliseconds(10));

    // Recording RTT's for other session should have no effect on current session
    // fallback periods.
    base::TimeDelta fallback_period = context.NextClassicFallbackPeriod(0u /* server_index */, 0 /* attempt */, session2.get());
    for (int i = 0; i < 50; ++i) {
        context.RecordRtt(0u /* server_index */, false /* is_doh_server */, base::Milliseconds(1), OK, session1.get());
    }
    EXPECT_EQ(fallback_period, context.NextClassicFallbackPeriod(0u /* server_index */, 0 /* attempt */, session2.get()));
}
...
{
    ResolveContext context(nullptr /* url_request_context */, false /* enable_caching */);
    DnsConfig config = CreateDnsConfig(0 /* num_servers */, 1 /* num_doh_servers */);
    config.fallback_period = base::TimeDelta();
    scoped_refptr<DnsSession> session = CreateDnsSession(config);
    context.InvalidateCachesAndPerSessionData(session.get(), false /* network_change */);

    EXPECT_EQ(context.SecureTransactionTimeout(SecureDnsMode::kSecure, session.get()), features::kDnsMinTransactionTimeout.Get());
}
...
TEST_F(ResolveContextTest, SecureTransactionTimeout_LongFallbackPeriod)
{
    ...
    base::TimeDelta expected = kFallbackPeriod * features::kDnsTransactionTimeoutMultiplier.Get();
    ASSERT_GT(expected, features::kDnsMinTransactionTimeout.Get());

    EXPECT_EQ(context.SecureTransactionTimeout(SecureDnsMode::kSecure, session.get()), expected);
}

TEST_F(ResolveContextTest, SecureTransactionTimeout_LongRtt)
{
    ResolveContext context(nullptr /* url_request_context */, false /* enable_caching */);
    DnsConfig config = CreateDnsConfig(0 /* num_servers */, 2 /* num_doh_servers */);
    config.fallback_period = base::TimeDelta();
    scoped_refptr<DnsSession> session = CreateDnsSession(config);
    context.InvalidateCachesAndPerSessionData(session.get(), false /* network_change */);
    ...
    for (int i = 0; i < 50; ++i) {
        context.RecordRtt(0u /* server_index */, true /* is_doh_server */, base::Minutes(10), OK, session.get());
    }

    // Expect longer timeouts.
    EXPECT_GT(context.SecureTransactionTimeout(SecureDnsMode::kSecure, session.get()), features::kDnsMinTransactionTimeout.Get());
}

TEST_F(ResolveContextTest, SecureTransactionTimeout_DifferentSession)
{
    const base::TimeDelta kFallbackPeriod = base::Minutes(5);
    DnsConfig config1 = CreateDnsConfig(0 /* num_servers */, 1 /* num_doh_servers */);
    ...
    ResolveContext context(nullptr /* url_request_context */, false /* enable_caching */);
    context.InvalidateCachesAndPerSessionData(session1.get(), true /* network_change */);

    // Confirm that if session data were used, the timeout would be higher than
    // the min.
    base::TimeDelta multiplier_expected = kFallbackPeriod * features::kDnsTransactionTimeoutMultiplier.Get();
    ASSERT_GT(multiplier_expected, features::kDnsMinTransactionTimeout.Get());

    // Expect timeout always minimum with wrong session.
    EXPECT_EQ(context.SecureTransactionTimeout(SecureDnsMode::kSecure, session2.get()), features::kDnsMinTransactionTimeout.Get());
}
...
{
    ResolveContext context(nullptr /* url_request_context */, false /* enable_caching */);
    DnsConfig config = CreateDnsConfig(1 /* num_servers */, 0 /* num_doh_servers */);
    config.fallback_period = base::TimeDelta();
    scoped_refptr<DnsSession> session = CreateDnsSession(config);
    context.InvalidateCachesAndPerSessionData(session.get(), false /* network_change */);

    EXPECT_EQ(context.ClassicTransactionTimeout(session.get()), features::kDnsMinTransactionTimeout.Get());
}
...
{
    ResolveContext context(nullptr /* url_request_context */, false /* enable_caching */);
    DnsConfig config = CreateDnsConfig(2 /* num_servers */, 0 /* num_doh_servers */);
    config.fallback_period = base::TimeDelta();
    scoped_refptr<DnsSession> session = CreateDnsSession(config);
    context.InvalidateCachesAndPerSessionData(session.get(), false /* network_change */);
    ...
    // Record long RTTs for only 1 server.
    for (int i = 0; i < 50; ++i) {
        context.RecordRtt(1u /* server_index */, false /* is_doh_server */, base::Minutes(10), OK, session.get());
    }
    ...
    // Expect longer timeouts.
    EXPECT_GT(context.ClassicTransactionTimeout(session.get()), features::kDnsMinTransactionTimeout.Get());
}

TEST_F(ResolveContextTest, ClassicTransactionTimeout_DifferentSession)
{
    const base::TimeDelta kFallbackPeriod = base::Minutes(5);
    DnsConfig config1 = CreateDnsConfig(1 /* num_servers */, 0 /* num_doh_servers */);
    config1.fallback_period = kFallbackPeriod;
    scoped_refptr<DnsSession> session1 = CreateDnsSession(config1);
    ...
    // Expect timeout always minimum with wrong session.
    EXPECT_EQ(context.ClassicTransactionTimeout(session2.get()), features::kDnsMinTransactionTimeout.Get());
}
...
TEST_F(ResolveContextTest, NegativeRtt)
{
    ...
    context.RecordRtt(0 /* server_index */, false /* is_doh_server */, base::Milliseconds(-1), OK /* rv */, session.get());
    context.RecordRtt(0 /* server_index */, true /* is_doh_server */, base::Milliseconds(-1), OK /* rv */, session.get());
}
...
TEST_F(ResolveContextTest, SessionChange_NoSession)
{
    ...
    context.InvalidateCachesAndPerSessionData(nullptr /* new_session */, false /* network_change */);
