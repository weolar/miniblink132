...
class NET_EXPORT_PRIVATE HostsReadingTestDnsConfigService : public TestDnsConfigService {
public:
    using HostsParserFactory = base::RepeatingCallback<std::unique_ptr<DnsHostsParser>(void)>;

    explicit HostsReadingTestDnsConfigService(HostsParserFactory hosts_parser_factory);
...
private:
    ...
    public:
        HostsReader(TestDnsConfigService& service, HostsParserFactory hosts_parser_factory);
