...
bool HaveOnlyLoopbackAddressesUsingGetifaddrs()
{
    ...
    bool result = true;
    for (struct ifaddrs* interface = interface_addr; interface != nullptr; interface = interface->ifa_next) {
...
bool HaveOnlyLoopbackAddressesFast(AddressMapOwnerLinux* address_map_owner)
{
    ...
    AddressMapOwnerLinux::AddressMap address_map = address_map_owner->GetAddressMap();
    ...
    for (const auto& [address, ifaddrmsg] : address_map) {
        ...
        if (online_links.contains(ifaddrmsg.ifa_index) && !address.IsLoopback() && !(address.IsIPv6() && address.IsLinkLocal())) {
            return false;
        }
    }
...
void RunHaveOnlyLoopbackAddressesJob(base::OnceCallback<void(bool)> finished_cb)
{
#if BUILDFLAG(IS_LINUX)
    ...
    AddressMapOwnerLinux* address_map_owner = NetworkChangeNotifier::GetAddressMapOwner();
    if (address_map_owner) {
        ...
        base::SequencedTaskRunner::GetCurrentDefault()->PostTask(
            FROM_HERE, base::BindOnce(std::move(finished_cb), HaveOnlyLoopbackAddressesFast(address_map_owner)));
        return;
    }
#endif // BUILDFLAG(IS_LINUX)
