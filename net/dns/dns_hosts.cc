    ...
    bool Advance()
    {
        ...
        while (pos_ < end_ && pos_ != std::string::npos) {
            ...
            default: {
                ...
                token_ = std::string_view(data_ + token_start, token_end - token_start);
...
void ParseHostsWithCommaMode(const std::string& contents, DnsHosts* dns_hosts, ParseHostsCommaMode comma_mode)
{
    CHECK(dns_hosts);
    ...
    while (parser.Advance()) {
        if (parser.token_is_ip()) {
            ...
            std::string canonicalized_host = CanonicalizeHost(parser.token(), &canonicalization_info);
...
void ParseHostsWithCommaModeForTesting(const std::string& contents, DnsHosts* dns_hosts, ParseHostsCommaMode comma_mode)
{
...
void ParseHosts(const std::string& contents, DnsHosts* dns_hosts)
{
    ...
    // metrics.
    base::UmaHistogramMemoryKB("Net.DNS.DnsHosts.EstimateMemoryUsage", base::trace_event::EstimateMemoryUsage(*dns_hosts));
...
bool DnsHostsFileParser::ParseHosts(DnsHosts* dns_hosts) const
{
    ...
    // TODO(crbug.com/40874231): Remove this when we have enough data.
    base::UmaHistogramCustomCounts("Net.DNS.DnsHosts.FileSize", size.value(), 1, kMaxHostsSize * 2, 50);
