...
public:
    ...
    struct ServiceAction {
        ...
        {
            return std::tie(negated, status, action) == std::tie(other.negated, other.status, other.action);
        }
    ...
    struct NET_EXPORT_PRIVATE ServiceSpecification {
        explicit ServiceSpecification(Service service, std::vector<ServiceAction> actions = {});
        ...
        {
            return std::tie(service, actions) == std::tie(other.service, other.actions);
        }
