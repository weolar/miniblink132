...
bool CompareServiceEndpointAddresses(const ServiceEndpoint& a, const ServiceEndpoint& b)
{
...
bool CompareServiceEndpoint(const ServiceEndpoint& a, const ServiceEndpoint& b)
{
...
DnsTaskResultsManager::DnsTaskResultsManager(Delegate* delegate, HostResolver::Host host, DnsQueryTypeSet query_types, const NetLogWithSource& net_log)
    : delegate_(delegate)
    , host_(std::move(host))
...
void DnsTaskResultsManager::ProcessDnsTransactionResults(DnsQueryType query_type, const std::set<std::unique_ptr<HostResolverInternalResult>>& results)
{
    ...
    for (auto& result : results) {
        ...
        case HostResolverInternalResult::Type::kData: {
            PerDomainResult& per_domain_result = GetOrCreatePerDomainResult(result->domain_name());
            for (const auto& ip_endpoint : result->AsData().endpoints()) {
                ...
                if (ip_endpoint.address().IsIPv4()) {
                    per_domain_result.ipv4_endpoints.emplace_back(ip_endpoint.address(), host_.GetPort());
                } else {
                    CHECK(ip_endpoint.address().IsIPv6());
                    per_domain_result.ipv6_endpoints.emplace_back(ip_endpoint.address(), host_.GetPort());
                }
            }
        ...
        case HostResolverInternalResult::Type::kMetadata: {
            ...
            for (auto [priority, metadata] : result->AsMetadata().metadatas()) {
                ...
                PerDomainResult& per_domain_result = GetOrCreatePerDomainResult(metadata.target_name);
        ...
        case net::HostResolverInternalResult::Type::kError:
            ...
            PerDomainResult& per_domain_result = GetOrCreatePerDomainResult(result->domain_name());
            if (query_type == DnsQueryType::AAAA && result->AsError().error() == ERR_NAME_NOT_RESOLVED && !per_domain_result.ipv4_endpoints.empty()) {
    ...
    const bool waiting_for_aaaa_response = query_types_.Has(DnsQueryType::AAAA) && !aaaa_response_received_;
    if (waiting_for_aaaa_response) {
        if (query_type == DnsQueryType::A && should_update_endpoints) {
            ...
            net_log_.BeginEvent(NetLogEventType::HOST_RESOLVER_SERVICE_ENDPOINTS_RESOLUTION_DELAY);
            // Safe to unretain since `this` owns the timer.
            resolution_delay_timer_.Start(
                FROM_HERE, kResolutionDelay, base::BindOnce(&DnsTaskResultsManager::OnAaaaResolutionTimedout, base::Unretained(this)));
        }
...
const std::vector<ServiceEndpoint>& DnsTaskResultsManager::GetCurrentEndpoints() const
{
...
DnsTaskResultsManager::PerDomainResult& DnsTaskResultsManager::GetOrCreatePerDomainResult(const std::string& domain_name)
{
    auto it = per_domain_results_.find(domain_name);
    if (it == per_domain_results_.end()) {
        it = per_domain_results_.try_emplace(it, domain_name, std::make_unique<PerDomainResult>());
    }
    return *it->second;
}
...
void DnsTaskResultsManager::UpdateEndpoints()
{
    ...
    for (const auto& [domain_name, per_domain_result] : per_domain_results_) {
        if (per_domain_result->ipv4_endpoints.empty() && per_domain_result->ipv6_endpoints.empty()) {
            continue;
        }
    ...
    std::stable_sort(new_endpoints.begin(), new_endpoints.end(), CompareServiceEndpoint);
...
{
    net_log_.EndEvent(NetLogEventType::HOST_RESOLVER_SERVICE_ENDPOINTS_RESOLUTION_DELAY, [&]() {
        base::TimeDelta elapsed = base::TimeTicks::Now() - resolution_delay_start_time_;
        base::Value::Dict dict;
