...
TEST(HttpsRecordRdataTest, ParsesAlias)
{
    ...
    std::unique_ptr<HttpsRecordRdata> rdata = HttpsRecordRdata::Parse(std::string_view(kRdata, sizeof(kRdata) - 1));
    ASSERT_TRUE(rdata);
...
TEST(HttpsRecordRdataTest, ParsesService)
{
    ...
    ServiceFormHttpsRecordRdata expected(1 /* priority */, "chromium.org", std::set<uint16_t>({ 1, 2, 3, 4, 5, 6 }),
        std::vector<std::string>({ "foo", "bar" }) /* alpn_ids */, false /* default_alpn */, std::optional<uint16_t>(46) /* port */,
        std::vector<IPAddress>({ IPAddress(8, 8, 8, 8) }) /* ipv4_hint */, "hello" /* ech_config */, std::vector<IPAddress>({ expected_ipv6 }) /* ipv6_hint */,
    ...
    EXPECT_THAT(service_rdata->mandatory_keys(), testing::ElementsAre(1, 2, 3, 4, 5, 6));
    ...
    EXPECT_THAT(service_rdata->ipv4_hint(), testing::ElementsAre(IPAddress(8, 8, 8, 8)));
    EXPECT_EQ(service_rdata->ech_config(), "hello");
    EXPECT_THAT(service_rdata->ipv6_hint(), testing::ElementsAre(expected_ipv6));
    EXPECT_THAT(service_rdata->unparsed_params(), testing::ElementsAre(testing::Pair(7, "foo")));
...
TEST(HttpsRecordRdataTest, AliasIsEqualRejectsWrongType)
{
    ...
    ServiceFormHttpsRecordRdata service(1u /* priority */, "service.name.test", {} /* mandatory_keys */, {} /* alpn_ids */, true /* default_alpn */,
        std::nullopt /* port */, {} /* ipv4_hint */, "" /* ech_config */, {} /* ipv6_hint */, {} /* unparsed_params */);
