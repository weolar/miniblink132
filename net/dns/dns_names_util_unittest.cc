...
{
    EXPECT_THAT(DottedNameToNetwork("com", /*require_valid_internet_hostname=*/true), Optional(ToBytes("\003com")));
...
{
    EXPECT_THAT(DottedNameToNetwork("com", /*require_valid_internet_hostname=*/false), Optional(ToBytes("\003com")));
    EXPECT_THAT(DottedNameToNetwork("google.com",
...
{
    EXPECT_FALSE(DottedNameToNetwork("", /*require_valid_internet_hostname=*/true).has_value());
    EXPECT_FALSE(DottedNameToNetwork(".", /*require_valid_internet_hostname=*/true).has_value());
    EXPECT_FALSE(DottedNameToNetwork("..", /*require_valid_internet_hostname=*/true).has_value());
...
{
    EXPECT_FALSE(DottedNameToNetwork("", /*require_valid_internet_hostname=*/false).has_value());
    EXPECT_FALSE(DottedNameToNetwork(".", /*require_valid_internet_hostname=*/false).has_value());
    EXPECT_FALSE(DottedNameToNetwork("..", /*require_valid_internet_hostname=*/false).has_value());
    EXPECT_FALSE(DottedNameToNetwork(".google.com",
        /*require_valid_internet_hostname=*/false)
...
TEST(DnsNamesUtilTest, DottedNameToNetworkWithValidationAcceptsEmptyLabelAtEnd)
{
    EXPECT_THAT(DottedNameToNetwork("www.google.com.",
                    /*require_valid_internet_hostname=*/true),
        Optional(ToBytes("\003www\006google\003com")));
...
TEST(DnsNamesUtilTest, DottedNameToNetworkWithValidationAllowsLongNames)
{
    ...
    EXPECT_THAT(DottedNameToNetwork("z23456789a123456789a123456789a123456789a123456789a123456789a123",
                    /*require_valid_internet_hostname=*/true),
    ...
    EXPECT_THAT(DottedNameToNetwork("abcdefghi.abcdefghi.abcdefghi.abcdefghi.abcdefghi.abcdefghi."
...
TEST(DnsNamesUtilTest, DottedNameToNetworkWithValidationRejectsTooLongNames)
{
    ...
    EXPECT_FALSE(DottedNameToNetwork("123456789a123456789a123456789a123456789a123456789a123456789a1234",
        /*require_valid_internet_hostname=*/true)
                     .has_value());
    EXPECT_FALSE(DottedNameToNetwork("123456789a123456789a123456789a123456789a123456789a123456789a1234.",
        /*require_valid_internet_hostname=*/true)
    ...
    EXPECT_FALSE(DottedNameToNetwork("123456789.123456789.123456789.123456789.123456789.123456789."
...
TEST(DnsNamesUtilTest, DottedNameToNetworkWithValidationRejectsRestrictedCharacters)
{
...
TEST(DnsNamesUtilTest, DottedNameToNetworkAcceptsRestrictedCharacters)
{
    ...
    EXPECT_THAT(DottedNameToNetwork("_ipp._tcp.local.foo printer (bar)",
                    /*require_valid_internet_hostname=*/false),
...
TEST(DnsNamesUtilTest, NetworkToDottedNameAdvancesReader)
{
    ...
    {
        auto reader = base::SpanReader(base::byte_span_from_cstring("\003com\0ZZZ"));
        EXPECT_THAT(NetworkToDottedName(reader), Optional(Eq("com")));
...
TEST(DnsNamesUtilTest, NetworkToDottedNameShouldHandleSimpleNames)
{
    ...
    EXPECT_THAT(NetworkToDottedName(base::as_byte_span(dns_name)), Optional(Eq("foo")));

    dns_name += "\003bar";
    EXPECT_THAT(NetworkToDottedName(base::as_byte_span(dns_name)), Optional(Eq("foo.bar")));

    dns_name += "\002uk";
    EXPECT_THAT(NetworkToDottedName(base::as_byte_span(dns_name)), Optional(Eq("foo.bar.uk")));
...
TEST(DnsNamesUtilTest, NetworkToDottedNameShouldHandleEmpty)
{
    ...
    EXPECT_THAT(NetworkToDottedName(base::as_byte_span(dns_name)), Optional(Eq("")));
...
TEST(DnsNamesUtilTest, NetworkToDottedNameShouldRejectEmptyIncomplete)
{
    ...
    EXPECT_THAT(NetworkToDottedName(base::as_byte_span(dns_name), false /* require_complete */), Optional(Eq("")));

    EXPECT_EQ(NetworkToDottedName(base::as_byte_span(dns_name), true /* require_complete */), std::nullopt);
}
...
TEST(DnsNamesUtilTest, NetworkToDottedNameComplete)
{
    ...
    EXPECT_THAT(NetworkToDottedName(base::as_byte_span(dns_name), false /* require_complete */), Optional(Eq("foo.test")));

    EXPECT_THAT(NetworkToDottedName(base::as_byte_span(dns_name), true /* require_complete */), Optional(Eq("foo.test")));
}
...
TEST(DnsNamesUtilTest, NetworkToDottedNameNotComplete)
{
    ...
    EXPECT_THAT(NetworkToDottedName(base::as_byte_span(dns_name), false /* require_complete */), Optional(Eq("boo.test")));

    EXPECT_EQ(NetworkToDottedName(base::as_byte_span(dns_name), true /* require_complete */), std::nullopt);
}

TEST(DnsNamesUtilTest, NetworkToDottedNameShouldRejectEmptyWhenRequiringComplete)
{
    std::string dns_name;
    ...
    dns_name += '\0';
    EXPECT_THAT(NetworkToDottedName(base::as_byte_span(dns_name), true /* require_complete */), Optional(Eq("")));
}
...
TEST(DnsNamesUtilTest, NetworkToDottedNameShouldHandleExcessInput)
{
    ...
    EXPECT_THAT(NetworkToDottedName(base::as_byte_span(dns_name)), Optional(Eq("cool.name.test")));

    dns_name = "\002hi";
    dns_name += '\0';
    dns_name += "goodbye";

    EXPECT_THAT(NetworkToDottedName(base::as_byte_span(dns_name)), Optional(Eq("hi")));
}
...
{
    std::string dns_name;
    for (int i = 0; i < dns_protocol::kMaxNameLength; i += (dns_protocol::kMaxLabelLength + 1)) {
        int label_size = std::min(dns_protocol::kMaxNameLength - 1 - i, dns_protocol::kMaxLabelLength);
...
{
    std::string dns_name;
    for (int i = 0; i < dns_protocol::kMaxNameLength + 1; i += (dns_protocol::kMaxLabelLength + 1)) {
        int label_size = std::min(dns_protocol::kMaxNameLength - i, dns_protocol::kMaxLabelLength);
        dns_name += static_cast<char>(label_size);
        for (int j = 0; j < label_size; ++j) {
            dns_name += 'a';
        }
    }
    ASSERT_EQ(dns_name.size(), static_cast<size_t>(dns_protocol::kMaxNameLength + 1));
...
TEST(DnsNamesUtilTest, NetworkToDottedNameShouldRejectTooLongCompleteName)
{
    ...
    dns_name += '\0';
    ASSERT_EQ(dns_name.size(), static_cast<size_t>(dns_protocol::kMaxNameLength + 2));
...
TEST(DnsNamesUtilTest, ValidDnsNames)
{
    constexpr std::string_view kGoodHostnames[] = {
        ...
        "%20%20noodles.blorg",
        "noo dles.blorg ",
        "noo dles_ipp._tcp.local",
        "www.nood(les).blorg",
        "noo dl(es)._tcp.local",
    };
...
{
    EXPECT_TRUE(IsValidDnsName("z23456789a123456789a123456789a123456789a123456789a123456789a123"));
    EXPECT_TRUE(IsValidDnsName("z23456789a123456789a123456789a123456789a123456789a123456789a123."));

    EXPECT_TRUE(IsValidDnsRecordName("z23456789a123456789a123456789a123456789a123456789a123456789a123"));
    EXPECT_TRUE(IsValidDnsRecordName("z23456789a123456789a123456789a123456789a123456789a123456789a123."));
}

TEST(DnsNameUtilTest, TooLongLabelsInInvalidDnsNames)
{
    EXPECT_FALSE(IsValidDnsName("123456789a123456789a123456789a123456789a123456789a123456789a1234"));
    EXPECT_FALSE(IsValidDnsName("z23456789a123456789a123456789a123456789a123456789a123456789a1234."));

    EXPECT_FALSE(IsValidDnsRecordName("z23456789a123456789a123456789a123456789a123456789a123456789a1234"));
    EXPECT_FALSE(IsValidDnsRecordName("z23456789a123456789a123456789a123456789a123456789a123456789a1234."));
}

TEST(DnsNameUtilTest, LongValidDnsNames)
{
    EXPECT_TRUE(IsValidDnsName("abcdefghi.abcdefghi.abcdefghi.abcdefghi.abcdefghi.abcdefghi."
                               "abcdefghi.abcdefghi.abcdefghi.abcdefghi.abcdefghi.abcdefghi."
                               "abcdefghi.abcdefghi.abcdefghi.abcdefghi.abcdefghi.abcdefghi."
                               "abcdefghi.abcdefghi.abcdefghi.abcdefghi.abcdefghi.abcdefghi."
                               "abcdefghi.abc"));
    ...
    EXPECT_TRUE(IsValidDnsRecordName("abcdefghi.abcdefghi.abcdefghi.abcdefghi.abcdefghi.abcdefghi."
                                     "abcdefghi.abcdefghi.abcdefghi.abcdefghi.abcdefghi.abcdefghi."
                                     "abcdefghi.abcdefghi.abcdefghi.abcdefghi.abcdefghi.abcdefghi."
                                     "abcdefghi.abcdefghi.abcdefghi.abcdefghi.abcdefghi.abcdefghi."
                                     "abcdefghi.abc"));
...
{
    EXPECT_FALSE(IsValidDnsName("abcdefghi.abcdefghi.abcdefghi.abcdefghi.abcdefghi.abcdefghi."
                                "abcdefghi.abcdefghi.abcdefghi.abcdefghi.abcdefghi.abcdefghi."
                                "abcdefghi.abcdefghi.abcdefghi.abcdefghi.abcdefghi.abcdefghi."
                                "abcdefghi.abcdefghi.abcdefghi.abcdefghi.abcdefghi.abcdefghi."
                                "abcdefghi.abcd"));
    ...
    EXPECT_FALSE(IsValidDnsRecordName("abcdefghi.abcdefghi.abcdefghi.abcdefghi.abcdefghi.abcdefghi."
                                      "abcdefghi.abcdefghi.abcdefghi.abcdefghi.abcdefghi.abcdefghi."
                                      "abcdefghi.abcdefghi.abcdefghi.abcdefghi.abcdefghi.abcdefghi."
                                      "abcdefghi.abcdefghi.abcdefghi.abcdefghi.abcdefghi.abcdefghi."
                                      "abcdefghi.abcd"));
