...
public:
    ...
    typedef base::RepeatingCallback<void(const RecordParsed*)> RecordRemovedCallback;
    ...
    void FindDnsRecords(unsigned type, const std::string& name, std::vector<const RecordParsed*>* records, base::Time now) const;

    // Remove expired records, call |record_removed_callback| for every removed
    // record.
    void CleanupRecords(base::Time now, const RecordRemovedCallback& record_removed_callback);
    ...
    base::Time next_expiration() const
    {
