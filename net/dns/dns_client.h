...
public:
    ...
    virtual void SetInsecureEnabled(bool enabled, bool additional_types_enabled) = 0;

    // When true, DoH should not be used in AUTOMATIC mode since no DoH servers
    // have a successful probe state.
    virtual bool FallbackFromSecureTransactionPreferred(ResolveContext* context) const = 0;
    ...
    virtual std::optional<std::vector<IPEndPoint>> GetPresetAddrs(const url::SchemeHostPort& endpoint) const = 0;
    ...
    virtual void SetTransactionFactoryForTesting(std::unique_ptr<DnsTransactionFactory> factory) = 0;
    virtual void SetAddressSorterForTesting(std::unique_ptr<AddressSorter> address_sorter) = 0;
    ...
    static std::unique_ptr<DnsClient> CreateClientForTesting(NetLog* net_log, const RandIntCallback& rand_int_callback);
};
