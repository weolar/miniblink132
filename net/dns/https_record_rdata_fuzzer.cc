...
void ParseAndExercise(FuzzedDataProvider& data_provider)
{
    ...
    std::unique_ptr<HttpsRecordRdata> parsed3 = HttpsRecordRdata::Parse(data_provider.ConsumeRemainingBytesAsString());
    ...
    if (parsed->IsAlias()) {
        ...
        CHECK(!base::Contains(mandatory_keys, dns_protocol::kHttpsServiceParamKeyMandatory));
