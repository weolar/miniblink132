...
bool AddressInfo::const_iterator::operator!=(const AddressInfo::const_iterator& o) const
{
    return ai_ != o.ai_;
}
...
AddressInfo::AddressInfoAndResult AddressInfo::Get(
    const std::string& host, const addrinfo& hints, std::unique_ptr<AddrInfoGetter> getter, handles::NetworkHandle network)
{
    if (getter == nullptr)
        getter = std::make_unique<AddrInfoGetter>();
    int err = OK;
    int os_error = 0;
    std::unique_ptr<addrinfo, FreeAddrInfoFunc> ai = getter->getaddrinfo(host, &hints, &os_error, network);
    ...
    return AddressInfoAndResult(std::optional<AddressInfo>(AddressInfo(std::move(ai), std::move(getter))), OK, 0);
}
...
{
    return (ai_->ai_canonname != nullptr) ? std::optional<std::string>(std::string(ai_->ai_canonname)) : std::optional<std::string>();
}
...
AddressInfo::AddressInfo(std::unique_ptr<addrinfo, FreeAddrInfoFunc> ai, std::unique_ptr<AddrInfoGetter> getter)
    : ai_(std::move(ai))
...
std::unique_ptr<addrinfo, FreeAddrInfoFunc> AddrInfoGetter::getaddrinfo(
    const std::string& host, const addrinfo* hints, int* out_os_error, handles::NetworkHandle network)
{
    addrinfo* ai;
...
#if BUILDFLAG(IS_ANDROID)
        *out_os_error = android::GetAddrInfoForNetwork(network, host.c_str(), nullptr, hints, &ai);
