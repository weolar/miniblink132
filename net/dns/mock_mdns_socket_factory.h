...
public:
    ...
    MOCK_METHOD4(RecvFrom, int(IOBuffer* buffer, int size, IPEndPoint* address, CompletionOnceCallback callback));

    int SendTo(IOBuffer* buf, int buf_len, const IPEndPoint& address, CompletionOnceCallback callback) override;

    MOCK_METHOD3(SendToInternal, int(const std::string& packet, const std::string address, CompletionOnceCallback callback));
    ...
    int HandleRecvNow(IOBuffer* buffer, int size, IPEndPoint* address, CompletionOnceCallback callback);

    int HandleRecvLater(IOBuffer* buffer, int size, IPEndPoint* address, CompletionOnceCallback callback);
...
private:
    int SendToInternal(const std::string& packet, const std::string& address, CompletionOnceCallback callback);

    // The latest receive callback is always saved, since the MDnsConnection
    // does not care which socket a packet is received on.
    int RecvFromInternal(IOBuffer* buffer, int size, IPEndPoint* address, CompletionOnceCallback callback);

    void CreateSocket(AddressFamily address_family, std::vector<std::unique_ptr<DatagramServerSocket>>* sockets);
