...
class MockDnsServerGetter : public base::RefCountedThreadSafe<MockDnsServerGetter> {
public:
    void set_retval(bool retval)
    {
        retval_ = retval;
    }
...
class DnsConfigServiceAndroidTest : public testing::Test, public WithTaskEnvironment {
public:
...
protected:
    ...
    std::unique_ptr<DnsConfigServiceAndroid> service_ = std::make_unique<DnsConfigServiceAndroid>();
    DnsConfig real_config_;

    scoped_refptr<MockDnsServerGetter> mock_dns_server_getter_ = base::MakeRefCounted<MockDnsServerGetter>();
    test::ScopedMockNetworkChangeNotifier mock_notifier_;
};

TEST_F(DnsConfigServiceAndroidTest, HandlesNetworkChangeNotifications)
{
    service_->WatchConfig(base::BindRepeating(&DnsConfigServiceAndroidTest::OnConfigChanged, base::Unretained(this)));
...
TEST_F(DnsConfigServiceAndroidTest, NewConfigReadOnNetworkChange)
{
    ...
    seen_config_ = false;
    NetworkChangeNotifier::NotifyObserversOfConnectionTypeChangeForTests(NetworkChangeNotifier::CONNECTION_WIFI);
    FastForwardBy(DnsConfigServiceAndroid::kConfigChangeDelay);
    RunUntilIdle();
    ASSERT_TRUE(seen_config_);
...
TEST_F(DnsConfigServiceAndroidTest, ReadsSearchSuffixes)
{
    ...
    service_->ReadConfig(base::BindRepeating(&DnsConfigServiceAndroidTest::OnConfigChanged, base::Unretained(this)));
    FastForwardBy(DnsConfigServiceAndroid::kConfigChangeDelay);
    RunUntilIdle();
    ASSERT_TRUE(seen_config_);
