...
public:
    ...
    void set_resolv_reader_for_testing(std::unique_ptr<ResolvReader> resolv_reader)
    {
    ...
    void set_nsswitch_reader_for_testing(std::unique_ptr<NsswitchReader> nsswitch_reader)
    {
        DCHECK(!config_reader_); // Need to call before first read.
...
private:
    FRIEND_TEST_ALL_PREFIXES(DnsConfigServiceLinuxTest, ChangeConfigMultipleTimes);
    class Watcher;
    class ConfigReader;

    std::unique_ptr<ResolvReader> resolv_reader_ = std::make_unique<ResolvReader>();
    std::unique_ptr<NsswitchReader> nsswitch_reader_ = std::make_unique<NsswitchReader>();
