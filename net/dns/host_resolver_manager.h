...
class NET_EXPORT HostResolverManager : public NetworkChangeNotifier::IPAddressObserver,
...
public:
    ...
    HostResolverManager(const HostResolver::ManagerOptions& options, SystemDnsConfigChangeNotifier* system_dns_config_notifier, NetLog* net_log);
    ...
    static std::unique_ptr<HostResolverManager> CreateNetworkBoundHostResolverManager(
    ...
    // `resolve_context`.
    std::unique_ptr<HostResolver::ResolveHostRequest> CreateRequest(absl::variant<url::SchemeHostPort, HostPortPair> host,
    ...
    std::unique_ptr<HostResolver::ResolveHostRequest> CreateRequest(HostResolver::Host host, NetworkAnonymizationKey network_anonymization_key,
    ...
    std::unique_ptr<HostResolver::ProbeRequest> CreateDohProbeRequest(ResolveContext* resolve_context);
    std::unique_ptr<MdnsListener> CreateMdnsListener(const HostPortPair& host, DnsQueryType query_type);

    // Creates a service endpoint resolution request.
    std::unique_ptr<HostResolver::ServiceEndpointRequest> CreateServiceEndpointRequest(url::SchemeHostPort scheme_host_port,
    ...
    virtual void SetInsecureDnsClientEnabled(bool enabled, bool additional_dns_types_enabled);
    ...
    void set_host_resolver_system_params_for_test(const HostResolverSystemTask::Params& host_resolver_system_params)
    {
        host_resolver_system_params_ = host_resolver_system_params;
    }

    void InvalidateCachesForTesting()
    {
        InvalidateCaches();
    }
    ...
    void SetMdnsSocketFactoryForTesting(std::unique_ptr<MDnsSocketFactory> socket_factory);
    ...
    size_t num_jobs_for_testing() const
    {
        return jobs_.size();
    }

    bool check_ipv6_on_wifi_for_testing() const
    {
        return check_ipv6_on_wifi_;
    }
    ...
    HostResolverManager(base::PassKey<HostResolverManager>, const HostResolver::ManagerOptions& options,
...
private:
    ...
    void InitializeJobKeyAndIPAddress(const NetworkAnonymizationKey& network_anonymization_key, const ResolveHostParameters& parameters,
    ...
    HostCache::Entry ResolveLocally(bool only_ipv6_reachable, const JobKey& job_key, const IPAddress& ip_address, ResolveHostParameters::CacheUsage cache_usage,
        SecureDnsPolicy secure_dns_policy, HostResolverSource source, const NetLogWithSource& source_net_log, HostCache* cache, std::deque<TaskType>* out_tasks,
        std::optional<HostCache::EntryStaleness>* out_stale_info);

    // Creates and starts a Job to asynchronously attempt to resolve
    // |request|.
    void CreateAndStartJob(JobKey key, std::deque<TaskType> tasks, RequestImpl* request);

    HostResolverManager::Job* AddJobWithoutRequest(JobKey key, ResolveHostParameters::CacheUsage cache_usage, HostCache* host_cache, std::deque<TaskType> tasks,
        RequestPriority priority, const NetLogWithSource& source_net_log);

    // Similar to CreateAndStartJob(), but for a ServiceEndpointRequest.
    void CreateAndStartJobForServiceEndpointRequest(JobKey key, std::deque<TaskType> tasks, ServiceEndpointRequestImpl* request);

    // Resolves the IP literal hostname represented by `ip_address`.
    HostCache::Entry ResolveAsIP(DnsQueryTypeSet query_types, bool resolve_canonname, const IPAddress& ip_address);
    ...
    std::optional<HostCache::Entry> MaybeServeFromCache(HostCache* cache, const HostCache::Key& key, ResolveHostParameters::CacheUsage cache_usage,
        bool ignore_secure, const NetLogWithSource& source_net_log, std::optional<HostCache::EntryStaleness>* out_stale_info);
    ...
    // bootstrap result.
    void StartBootstrapFollowup(JobKey key, HostCache* host_cache, const NetLogWithSource& source_net_log);
    ...
    std::optional<HostCache::Entry> ServeLocalhost(std::string_view hostname, DnsQueryTypeSet query_types, bool default_family_due_to_no_ipv6);
    ...
    void PushDnsTasks(bool system_task_allowed, SecureDnsMode secure_dns_mode, bool insecure_tasks_allowed, bool allow_cache, bool prioritize_local_lookups,
        ResolveContext* resolve_context, std::deque<TaskType>* out_tasks);
    ...
    int StartIPv6ReachabilityCheck(const NetLogWithSource& net_log, ClientSocketFactory* client_socket_factory, CompletionOnceCallback callback);
    ...
    void CacheResult(HostCache* cache, const HostCache::Key& key, const HostCache::Entry& entry, base::TimeDelta ttl);
    ...
    std::unique_ptr<DnsProbeRunner> CreateDohProbeRunner(ResolveContext* resolve_context);
    ...
    base::ObserverList<ResolveContext, true /* check_empty */, false /* allow_reentrancy */> registered_contexts_;
...
NET_EXPORT_PRIVATE bool ResolveLocalHostname(std::string_view host, std::vector<IPEndPoint>* address_list);
