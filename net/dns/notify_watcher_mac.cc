...
bool NotifyWatcherMac::Watch(const char* key, const CallbackType& callback)
{
    ...
    Cancel();
    uint32_t status = notify_register_file_descriptor(key, &notify_fd_, 0, &notify_token_);
    if (status != NOTIFY_STATUS_OK)
        return false;
    DCHECK_GE(notify_fd_, 0);
    watcher_
        = base::FileDescriptorWatcher::WatchReadable(notify_fd_, base::BindRepeating(&NotifyWatcherMac::OnFileCanReadWithoutBlocking, base::Unretained(this)));
    callback_ = callback;
    return true;
}
