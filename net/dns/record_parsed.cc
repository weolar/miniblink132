...
RecordParsed::RecordParsed(
    const std::string& name, uint16_t type, uint16_t klass, uint32_t ttl, std::unique_ptr<const RecordRdata> rdata, base::Time time_created)
    : name_(name)
    , type_(type)
    , klass_(klass)
    , ttl_(ttl)
...
// static
std::unique_ptr<const RecordParsed> RecordParsed::CreateFrom(DnsRecordParser* parser, base::Time time_created)
{
    ...
    return base::WrapUnique(new RecordParsed(record.name, record.type, record.klass, record.ttl, std::move(rdata), time_created));
}
...
bool RecordParsed::IsEqual(const RecordParsed* other, bool is_mdns) const
{
    ...
    return name_ == other->name_ && klass == other_klass && type_ == other->type_ && !!rdata_ == !!other->rdata_
