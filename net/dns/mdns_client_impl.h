...
public:
    ...
    explicit MDnsSocketFactoryImpl(NetLog* net_log)
        : net_log_(net_log)
    {
    }
...
private:
    class SocketHandler {
    public:
        SocketHandler(std::unique_ptr<DatagramServerSocket> socket, MDnsConnection* connection);
    ...
    void OnDatagramReceived(DnsResponse* response, const IPEndPoint& recv_addr, int bytes_read);
...
public:
    ...
    public:
        ...
        void QueryCache(uint16_t rrtype, const std::string& name, std::vector<const RecordParsed*>* records) const;
    ...
    private:
        ...
        public:
            ...
            const std::string& name_lowercase() const
            {
        ...
        typedef std::map<ListenerKey, std::unique_ptr<ObserverListType>> ListenerMap;

        // Alert listeners of an update to the cache.
        void AlertListeners(MDnsCache::UpdateType update_type, const ListenerKey& key, const RecordParsed* record);
    ...
    MDnsClientImpl(base::Clock* clock, std::unique_ptr<base::OneShotTimer> cleanup_timer);
    ...
    std::unique_ptr<MDnsListener> CreateListener(uint16_t rrtype, const std::string& name, MDnsListener::Delegate* delegate) override;

    std::unique_ptr<MDnsTransaction> CreateTransaction(
        uint16_t rrtype, const std::string& name, int flags, const MDnsTransaction::ResultCallback& callback) override;
...
public:
    MDnsListenerImpl(uint16_t rrtype, const std::string& name, base::Clock* clock, MDnsListener::Delegate* delegate, MDnsClientImpl* client);
    ...
    void HandleRecordUpdate(MDnsCache::UpdateType update_type, const RecordParsed* record_parsed);
...
class MDnsTransactionImpl final : public MDnsTransaction, public MDnsListener::Delegate {
public:
    MDnsTransactionImpl(uint16_t rrtype, const std::string& name, int flags, const MDnsTransaction::ResultCallback& callback, MDnsClientImpl* client);
...
private:
    ...
    void TriggerCallback(MDnsTransaction::Result result, const RecordParsed* record);
