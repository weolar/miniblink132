...
protected:
    ...
    DnsConfig ValidConfigWithDoh(bool doh_only)
    {
        ...
        config.doh_config = *net::DnsOverHttpsConfig::FromString("https://www.doh.com/");
        return config;
    }
...
TEST_F(DnsClientTest, NoConfig)
{
    ...
    EXPECT_TRUE(client_->FallbackFromSecureTransactionPreferred(resolve_context_.get()));
...
TEST_F(DnsClientTest, CanUseSecureDnsTransactions_NoDohServers)
{
    ...
    EXPECT_THAT(client_->GetEffectiveConfig(), testing::Pointee(BasicValidConfig()));
...
TEST_F(DnsClientTest, InsecureNotEnabled)
{
    ...
    EXPECT_THAT(client_->GetEffectiveConfig(), testing::Pointee(ValidConfigWithDoh(false /* doh_only */)));
    EXPECT_TRUE(client_->GetHosts());
    EXPECT_TRUE(client_->GetTransactionFactory());
    EXPECT_EQ(client_->GetCurrentSession()->config(), ValidConfigWithDoh(false /* doh_only */));
}
...
TEST_F(DnsClientTest, CanUseSecureDnsTransactions_ProbeSuccess)
{
    ...
    resolve_context_->InvalidateCachesAndPerSessionData(client_->GetCurrentSession(), true /* network_change */);

    EXPECT_TRUE(client_->CanUseSecureDnsTransactions());
    EXPECT_TRUE(client_->FallbackFromSecureTransactionPreferred(resolve_context_.get()));

    resolve_context_->RecordServerSuccess(0u /* server_index */, true /* is_doh_server */, client_->GetCurrentSession());
    EXPECT_TRUE(client_->CanUseSecureDnsTransactions());
    EXPECT_FALSE(client_->FallbackFromSecureTransactionPreferred(resolve_context_.get()));
}
...
TEST_F(DnsClientTest, AllAllowed)
{
    client_->SetInsecureEnabled(/*enabled=*/true,
        /*additional_types_enabled=*/true);
    client_->SetSystemConfig(ValidConfigWithDoh(false /* doh_only */));
    resolve_context_->InvalidateCachesAndPerSessionData(client_->GetCurrentSession(), false /* network_change */);
    resolve_context_->RecordServerSuccess(0u /* server_index */, true /* is_doh_server */, client_->GetCurrentSession());
...
TEST_F(DnsClientTest, GetPresetAddrs)
{
    ...
    EXPECT_FALSE(client_->GetPresetAddrs(url::SchemeHostPort("https", "otherdomain.com", 443)));
    EXPECT_FALSE(client_->GetPresetAddrs(url::SchemeHostPort("http", "www.doh.com", 443)));
    EXPECT_FALSE(client_->GetPresetAddrs(url::SchemeHostPort("https", "www.doh.com", 9999)));

    std::vector<IPEndPoint> expected({ { { 4, 3, 2, 1 }, 443 }, { { 4, 3, 2, 2 }, 443 } });

    EXPECT_THAT(client_->GetPresetAddrs(url::SchemeHostPort("https", "www.doh.com", 443)), testing::Optional(expected));
}
...
TEST_F(DnsClientTest, Override)
{
    ...
    EXPECT_THAT(client_->GetEffectiveConfig(), testing::Pointee(BasicValidOverrides().ApplyOverrides(BasicValidConfig())));
    EXPECT_EQ(client_->GetCurrentSession()->config(), BasicValidOverrides().ApplyOverrides(BasicValidConfig()));
...
TEST_F(DnsClientTest, OverrideNoConfig)
{
    ...
    auto override_everything = DnsConfigOverrides::CreateOverridingEverythingWithDefaults();
    override_everything.nameservers.emplace({ IPEndPoint(IPAddress(1, 2, 3, 4), 123) });
    client_->SetConfigOverrides(override_everything);
    EXPECT_THAT(client_->GetEffectiveConfig(), testing::Pointee(override_everything.ApplyOverrides(DnsConfig())));
    EXPECT_EQ(client_->GetCurrentSession()->config(), override_everything.ApplyOverrides(DnsConfig()));
}
...
TEST_F(DnsClientTest, OverrideInvalidConfig)
{
    ...
    client_->SetConfigOverrides(BasicValidOverrides());
    EXPECT_THAT(client_->GetEffectiveConfig(), testing::Pointee(BasicValidOverrides().ApplyOverrides(BasicValidConfig())));
    EXPECT_EQ(client_->GetCurrentSession()->config(), BasicValidOverrides().ApplyOverrides(DnsConfig()));
}
...
TEST_F(DnsClientTest, ReplaceCurrentSession)
{
    ...
    base::WeakPtr<DnsSession> session_before = client_->GetCurrentSession()->GetWeakPtr();
