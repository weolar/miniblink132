...
static const uint8_t kTestResponsesDifferentAnswers[] = {
    ...
    3, 'g', 'h', 's', 1, 'l', 6, 'g', 'o', 'o', 'g', 'l', 'e', 3, 'c', 'o', 'm', 0x00, 0x00, 0x01, // TYPE is A.
...
{
    DnsRecordParser parser(kT1ResponseDatagram, sizeof(dns_protocol::Header), kT1RecordCount);
    ...
    cache_.FindDnsRecords(ARecordRdata::kType, "ghs.l.google.com", &results, default_time_);
    ...
    results.clear();
    cache_.FindDnsRecords(PtrRecordRdata::kType, "ghs.l.google.com", &results, default_time_);
...
TEST_F(MDnsCacheTest, Expiration)
{
    ...
    cache_.FindDnsRecords(ARecordRdata::kType, "ghs.l.google.com", &results, default_time_ + ttl2);

    EXPECT_EQ(0u, results.size());

    EXPECT_CALL(record_removal_, OnRecordRemoved(record_to_be_deleted));

    cache_.CleanupRecords(default_time_ + ttl2, base::BindRepeating(&RecordRemovalMock::OnRecordRemoved, base::Unretained(&record_removal_)));
...
TEST_F(MDnsCacheTest, RecordChange)
{
    ...
    EXPECT_EQ(MDnsCache::RecordAdded, cache_.UpdateDnsRecord(std::move(record1)));
    EXPECT_EQ(MDnsCache::RecordChanged, cache_.UpdateDnsRecord(std::move(record2)));
}
...
TEST_F(MDnsCacheTest, GoodbyePacket)
{
    ...
    EXPECT_EQ(base::Time(), cache_.next_expiration());
    EXPECT_EQ(MDnsCache::NoChange, cache_.UpdateDnsRecord(std::move(record_goodbye)));
    EXPECT_EQ(base::Time(), cache_.next_expiration());
    EXPECT_EQ(MDnsCache::RecordAdded, cache_.UpdateDnsRecord(std::move(record_hello)));
    EXPECT_EQ(default_time_ + ttl, cache_.next_expiration());
    EXPECT_EQ(MDnsCache::NoChange, cache_.UpdateDnsRecord(std::move(record_goodbye2)));
...
TEST_F(MDnsCacheTest, AnyRRType)
{
    ...
    EXPECT_EQ(dns_protocol::kTypeA, std::min(results[0]->type(), results[1]->type()));
    EXPECT_EQ(dns_protocol::kTypeAAAA, std::max(results[0]->type(), results[1]->type()));
}
...
TEST_F(MDnsCacheTest, RemoveRecord)
{
    ...
    cache_.FindDnsRecords(dns_protocol::kTypeCNAME, "codereview.chromium.org", &results, default_time_);
...
{
    DnsRecordParser parser(kTestResponseTwoRecords, 0, /*num_records=*/2);
    std::unique_ptr<const RecordParsed> record1 = RecordParsed::CreateFrom(&parser, default_time_);
    const RecordParsed* record1_ptr = record1.get();
    std::unique_ptr<const RecordParsed> record2 = RecordParsed::CreateFrom(&parser, default_time_);
...
TEST_F(MDnsCacheTest, ClearOnOverfilledCleanup)
{
    ...
    cache_.CleanupRecords(default_time_, base::BindRepeating(&RecordRemovalMock::OnRecordRemoved, base::Unretained(&record_removal_)));

    EXPECT_FALSE(cache_.IsCacheOverfilled());
    std::vector<const RecordParsed*> results;
    cache_.FindDnsRecords(dns_protocol::kTypeA, "ghs.l.google.com", &results, default_time_);
    EXPECT_TRUE(results.empty());
    cache_.FindDnsRecords(dns_protocol::kTypeAAAA, "ghs.l.google.com", &results, default_time_);
    EXPECT_TRUE(results.empty());
}
