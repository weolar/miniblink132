...
public:
    ...
    {
        return base::BindRepeating(&CallbackHelper::OnComplete, base::Unretained(this));
    }
...
public:
    ~BlockingHelper()
    {
        EXPECT_EQ(state_, State::kUnblocked);
    }
    ...
    {
        block_event_.Reset();
        task_runner_->PostTask(FROM_HERE, base::BindOnce(&BlockingHelper::OnBlockedCallback, base::Unretained(this)));
        block_event_.Wait();
...
private:
    ...
    scoped_refptr<base::SingleThreadTaskRunner> task_runner_ = base::SingleThreadTaskRunner::GetCurrentDefault();
};
...
class DnsConfigServiceLinuxTest : public ::testing::Test, public WithTaskEnvironment {
public:
...
{
    scoped_refptr<base::SequencedTaskRunner> runner = base::ThreadPool::CreateSequencedTaskRunner({ base::MayBlock() });
    std::unique_ptr<internal::DnsConfigServiceLinux, base::OnTaskRunnerDeleter> service(
        new internal::DnsConfigServiceLinux(), base::OnTaskRunnerDeleter(runner));

    runner->PostTask(
        FROM_HERE, base::BindOnce(&internal::DnsConfigServiceLinux::WatchConfig, base::Unretained(service.get()), base::BindRepeating(&DummyConfigCallback)));
    service.reset();
    RunUntilIdle();
}
...
TEST_F(DnsConfigServiceLinuxTest, IgnoresBasicNsswitchConfigIfResolvConfigUnhandled)
{
    auto res = std::make_unique<struct __res_state>();
    InitializeResState(res.get());
...
TEST_F(DnsConfigServiceLinuxTest, RejectsNsswitchWithoutFiles)
{
    ...
    nsswitch_reader_->set_value({ NsswitchReader::ServiceSpecification(NsswitchReader::Service::kDns) });
...
TEST_F(DnsConfigServiceLinuxTest, RejectsWithExtraFiles)
{
    ...
    nsswitch_reader_->set_value({ NsswitchReader::ServiceSpecification(NsswitchReader::Service::kFiles),
        NsswitchReader::ServiceSpecification(NsswitchReader::Service::kFiles), NsswitchReader::ServiceSpecification(NsswitchReader::Service::kDns) });
...
TEST_F(DnsConfigServiceLinuxTest, IgnoresRedundantActions)
{
    ...
    nsswitch_reader_->set_value({ NsswitchReader::ServiceSpecification(NsswitchReader::Service::kFiles,
                                      { { /*negated=*/false, NsswitchReader::Status::kSuccess, NsswitchReader::Action::kReturn },
                                          { /*negated=*/true, NsswitchReader::Status::kSuccess, NsswitchReader::Action::kContinue } }),
...
TEST_F(DnsConfigServiceLinuxTest, RejectsWithBadDnsSuccessAction)
{
    ...
    nsswitch_reader_->set_value({ NsswitchReader::ServiceSpecification(NsswitchReader::Service::kFiles),
        NsswitchReader::ServiceSpecification(
            NsswitchReader::Service::kDns, { { /*negated=*/false, NsswitchReader::Status::kSuccess, NsswitchReader::Action::kContinue } }) });
...
TEST_F(DnsConfigServiceLinuxTest, RejectsNsswitchWithMisorderedServices)
{
    ...
    nsswitch_reader_->set_value(
        { NsswitchReader::ServiceSpecification(NsswitchReader::Service::kDns), NsswitchReader::ServiceSpecification(NsswitchReader::Service::kFiles) });
...
TEST_F(DnsConfigServiceLinuxTest, RejectsNsswitchMdns)
{
    ...
    nsswitch_reader_->set_value({ NsswitchReader::ServiceSpecification(NsswitchReader::Service::kFiles),
        NsswitchReader::ServiceSpecification(NsswitchReader::Service::kMdns), NsswitchReader::ServiceSpecification(NsswitchReader::Service::kDns) });
...
TEST_F(DnsConfigServiceLinuxTest, RejectsNsswitchMdns4)
{
    ...
    nsswitch_reader_->set_value({ NsswitchReader::ServiceSpecification(NsswitchReader::Service::kFiles),
        NsswitchReader::ServiceSpecification(NsswitchReader::Service::kMdns4), NsswitchReader::ServiceSpecification(NsswitchReader::Service::kDns) });
...
TEST_F(DnsConfigServiceLinuxTest, RejectsNsswitchMdns6)
{
    ...
    nsswitch_reader_->set_value({ NsswitchReader::ServiceSpecification(NsswitchReader::Service::kFiles),
        NsswitchReader::ServiceSpecification(NsswitchReader::Service::kMdns6), NsswitchReader::ServiceSpecification(NsswitchReader::Service::kDns) });
...
TEST_F(DnsConfigServiceLinuxTest, AcceptsNsswitchMdnsMinimal)
{
    ...
    nsswitch_reader_->set_value({ NsswitchReader::ServiceSpecification(NsswitchReader::Service::kFiles),
        ...
        NsswitchReader::ServiceSpecification(NsswitchReader::Service::kMdns6Minimal), NsswitchReader::ServiceSpecification(NsswitchReader::Service::kDns) });
...
TEST_F(DnsConfigServiceLinuxTest, AcceptsNsswitchMdnsMinimalWithCommonActions)
{
    ...
    nsswitch_reader_->set_value({ NsswitchReader::ServiceSpecification(NsswitchReader::Service::kFiles),
        NsswitchReader::ServiceSpecification(
            NsswitchReader::Service::kMdnsMinimal, { { /*negated=*/true, NsswitchReader::Status::kUnavailable, NsswitchReader::Action::kReturn } }),
        NsswitchReader::ServiceSpecification(
            NsswitchReader::Service::kMdns4Minimal, { { /*negated=*/false, NsswitchReader::Status::kNotFound, NsswitchReader::Action::kReturn } }),
        NsswitchReader::ServiceSpecification(
            NsswitchReader::Service::kMdns6Minimal, { { /*negated=*/true, NsswitchReader::Status::kUnavailable, NsswitchReader::Action::kReturn } }),
        NsswitchReader::ServiceSpecification(NsswitchReader::Service::kDns) });
...
TEST_F(DnsConfigServiceLinuxTest, RejectsWithBadMdnsMinimalUnavailableAction)
{
    ...
    nsswitch_reader_->set_value({ NsswitchReader::ServiceSpecification(NsswitchReader::Service::kFiles),
        NsswitchReader::ServiceSpecification(
            NsswitchReader::Service::kMdnsMinimal, { { /*negated=*/false, NsswitchReader::Status::kUnavailable, NsswitchReader::Action::kReturn } }),
        NsswitchReader::ServiceSpecification(NsswitchReader::Service::kDns) });
...
TEST_F(DnsConfigServiceLinuxTest, AcceptsNsswitchMyHostname)
{
    ...
    nsswitch_reader_->set_value({ NsswitchReader::ServiceSpecification(NsswitchReader::Service::kFiles),
        NsswitchReader::ServiceSpecification(NsswitchReader::Service::kMyHostname), NsswitchReader::ServiceSpecification(NsswitchReader::Service::kDns) });
...
TEST_F(DnsConfigServiceLinuxTest, RejectsWithBadMyHostnameNotFoundAction)
{
    ...
    nsswitch_reader_->set_value({ NsswitchReader::ServiceSpecification(NsswitchReader::Service::kFiles),
        NsswitchReader::ServiceSpecification(
            NsswitchReader::Service::kMyHostname, { { /*negated=*/false, NsswitchReader::Status::kNotFound, NsswitchReader::Action::kReturn } }),
        NsswitchReader::ServiceSpecification(NsswitchReader::Service::kDns) });
...
TEST_F(DnsConfigServiceLinuxTest, RejectsNsswitchResolve)
{
    ...
    nsswitch_reader_->set_value({ NsswitchReader::ServiceSpecification(NsswitchReader::Service::kFiles),
        NsswitchReader::ServiceSpecification(NsswitchReader::Service::kResolve), NsswitchReader::ServiceSpecification(NsswitchReader::Service::kDns) });
...
TEST_F(DnsConfigServiceLinuxTest, RejectsNsswitchNis)
{
    ...
    nsswitch_reader_->set_value({ NsswitchReader::ServiceSpecification(NsswitchReader::Service::kFiles),
        NsswitchReader::ServiceSpecification(NsswitchReader::Service::kNis), NsswitchReader::ServiceSpecification(NsswitchReader::Service::kDns) });
...
TEST_F(DnsConfigServiceLinuxTest, RejectsWithBadNisNotFoundAction)
{
    ...
    nsswitch_reader_->set_value({ NsswitchReader::ServiceSpecification(NsswitchReader::Service::kFiles),
        NsswitchReader::ServiceSpecification(
            NsswitchReader::Service::kNis, { { /*negated=*/false, NsswitchReader::Status::kNotFound, NsswitchReader::Action::kReturn } }),
        NsswitchReader::ServiceSpecification(NsswitchReader::Service::kDns) });
...
TEST_F(DnsConfigServiceLinuxTest, RejectsNsswitchUnknown)
{
    ...
    nsswitch_reader_->set_value({ NsswitchReader::ServiceSpecification(NsswitchReader::Service::kFiles),
        NsswitchReader::ServiceSpecification(NsswitchReader::Service::kUnknown), NsswitchReader::ServiceSpecification(NsswitchReader::Service::kDns) });
