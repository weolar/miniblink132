...
public:
    ...
    {
        return base::StrCat({ "Net.DNS.HTTPSSVC.RecordHttps.", secure_ ? "Secure." : "Insecure.", "ExpectNoerror." });
    }

    template <typename T> void ExpectSample(std::string_view name, std::optional<T> sample) const
    {
        if (sample)
    ...
    void ExpectSample(std::string_view name, std::optional<base::TimeDelta> sample) const
    {
    ...
    void VerifyAddressResolveTimeMetric(std::optional<base::TimeDelta> expect_noerror_time = std::nullopt)
    {
        const std::string kExpectNoerror = base::StrCat({ BuildMetricNamePrefix(), "ResolveTimeAddress" });

        ExpectSample(kExpectNoerror, expect_noerror_time);
    }

    void VerifyHttpsMetricsForExpectNoerror(std::optional<HttpssvcDnsRcode> rcode = std::nullopt, std::optional<bool> parsable = std::nullopt,
        std::optional<bool> record_with_error = std::nullopt, std::optional<base::TimeDelta> resolve_time_https = std::nullopt,
        std::optional<int> resolve_time_ratio = std::nullopt) const
    {
        ...
        const std::string kMetricRecordWithError = base::StrCat({ kPrefix, "RecordWithError" });
        const std::string kMetricResolveTimeExperimental = base::StrCat({ kPrefix, "ResolveTimeExperimental" });
        const std::string kMetricResolveTimeRatio = base::StrCat({ kPrefix, "ResolveTimeRatio" });
    ...
    const base::HistogramTester& histo() const
    {
        return histogram_;
    }
...
INSTANTIATE_TEST_SUITE_P(HttpssvcMetricsTestSimple, HttpssvcMetricsTest,
...
TEST_P(HttpssvcMetricsTest, AddressAndHttpsParsable)
{
    ...
    VerifyHttpsMetricsForExpectNoerror({ HttpssvcDnsRcode::kNoError } /* rcode */, { true } /* parsable */, std::nullopt /* record_with_error */,
        { kResolveTimeHttps } /* resolve_time_https */, { 15 } /* resolve_time_ratio */);
}
...
TEST_P(HttpssvcMetricsTest, AddressAndHttpsMissingWithRcode)
{
    ...
    VerifyAddressResolveTimeMetric({ kResolveTime } /* expect_noerror_time */);
    VerifyHttpsMetricsForExpectNoerror({ HttpssvcDnsRcode::kNxDomain } /* rcode */, std::nullopt /* parsable */, std::nullopt /* record_with_error */,
        { kResolveTimeHttps } /* resolve_time_https */, { 15 } /* resolve_time_ratio */);
}
...
TEST_P(HttpssvcMetricsTest, AddressAndHttpsParsableWithRcode)
{
    ...
    VerifyAddressResolveTimeMetric({ kResolveTime } /* expect_noerror_time */);
    VerifyHttpsMetricsForExpectNoerror({ HttpssvcDnsRcode::kNxDomain } /* rcode */,
        // "parsable" metric is omitted because the RCODE is not NOERROR.
        std::nullopt /* parsable */, { true } /* record_with_error */, { kResolveTimeHttps } /* resolve_time_https */, { 15 } /* resolve_time_ratio */);
}
...
TEST_P(HttpssvcMetricsTest, AddressAndHttpsMangledWithRcode)
{
    ...
    const base::TimeDelta kResolveTimeHttps = base::Milliseconds(15);
    auto metrics = std::make_optional<HttpssvcMetrics>(secure_);
    metrics->SaveForHttps(HttpssvcDnsRcode::kNxDomain, { false }, kResolveTimeHttps);
    metrics->SaveForAddressQuery(kResolveTime, HttpssvcDnsRcode::kNoError);
    metrics.reset(); // Record the metrics to UMA.
...
TEST_P(HttpssvcMetricsTest, AddressAndHttpsTimedOut)
{
    ...
    VerifyAddressResolveTimeMetric({ kResolveTime } /* expect_noerror_time */);
    VerifyHttpsMetricsForExpectNoerror({ HttpssvcDnsRcode::kTimedOut } /* rcode */,
        // "parsable" metric is omitted because the RCODE is not NOERROR.
        std::nullopt /* parsable */, std::nullopt /* record_with_error */, { kResolveTimeHttps } /* resolve_time_https */, { 15 } /* resolve_time_ratio */);
}
