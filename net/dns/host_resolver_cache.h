...
public:
    struct StaleLookupResult {
        StaleLookupResult(const HostResolverInternalResult& result, std::optional<base::TimeDelta> expired_by, bool stale_by_generation);
    ...
    explicit HostResolverCache(size_t max_results, const base::Clock& clock = *base::DefaultClock::GetInstance(),
    ...
    // criteria).
    const HostResolverInternalResult* Lookup(std::string_view domain_name, const NetworkAnonymizationKey& network_anonymization_key,
        DnsQueryType query_type = DnsQueryType::UNSPECIFIED, HostResolverSource source = HostResolverSource::ANY,
    ...
    // criteria).
    std::optional<StaleLookupResult> LookupStale(std::string_view domain_name, const NetworkAnonymizationKey& network_anonymization_key,
        DnsQueryType query_type = DnsQueryType::UNSPECIFIED, HostResolverSource source = HostResolverSource::ANY,
        std::optional<bool> secure = std::nullopt) const;
    ...
    bool AtMaxSizeForTesting() const
    {
...
private:
    ...
    struct KeyComparator {
        ...
        {
            return std::tie(lhs.domain_name, lhs.network_anonymization_key) < std::tie(rhs.domain_name, rhs.network_anonymization_key);
        }
    ...
    struct Entry {
        Entry(std::unique_ptr<HostResolverInternalResult> result, HostResolverSource source, bool secure, int staleness_generation);
        ~Entry();

        Entry(Entry&&);
        Entry& operator=(Entry&&);

        bool IsStale(base::Time now, base::TimeTicks now_ticks, int current_staleness_generation) const;
        base::TimeDelta TimeUntilExpiration(base::Time now, base::TimeTicks now_ticks) const;
    ...
    std::vector<EntryMap::const_iterator> LookupInternal(std::string_view domain_name, const NetworkAnonymizationKey& network_anonymization_key,
        DnsQueryType query_type, HostResolverSource source, std::optional<bool> secure) const;

    void Set(std::unique_ptr<HostResolverInternalResult> result, const NetworkAnonymizationKey& network_anonymization_key, HostResolverSource source,
        bool secure, bool replace_existing, int staleness_generation);
    ...
    // restoration.
    base::Value SerializeEntries(bool serialize_staleness_generation, bool require_persistable_anonymization_key) const;
