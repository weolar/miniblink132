...
public:
    void Sort(const std::vector<IPEndPoint>& endpoints, CallbackType callback) const override
    {
        ...
        std::sort(sorted.begin(), sorted.end(), [](const IPEndPoint& a, const IPEndPoint& b) { return a.address() < b.address(); });
...
public:
    ...
    ServiceEndpointRequest* request() const
    {
        return request_.get();
    }

    std::optional<int> finished_result() const
    {
        return finished_result_;
    }
...
public:
    ...
    int Start()
    {
        return request_->Start(base::BindOnce(&LegacyRequester::OnComplete, base::Unretained(this)));
    }
    ...
    std::optional<int> complete_result() const
    {
        return complete_result_;
    }
...
class HostResolverServiceEndpointRequestTest : public HostResolverManagerDnsTest {
public:
...
protected:
    ...
    void set_ipv6_reachable(bool reachable)
    {
        ipv6_reachable_ = reachable;
    }

    void SetDnsRules(MockDnsClientRuleList rules)
    {
        CreateResolverWithOptionsAndParams(DefaultOptions(),
    ...
    void UseNoDomanDnsRules(const std::string& host)
    {
        ...
        AddDnsRule(&rules, host, dns_protocol::kTypeA, MockDnsClientRule::ResultType::kNoDomain, /*delay=*/false);
        AddDnsRule(&rules, host, dns_protocol::kTypeAAAA, MockDnsClientRule::ResultType::kNoDomain, /*delay=*/false);
    ...
    {
        MockDnsClientRuleList rules;
        AddDnsRule(&rules, host, dns_protocol::kTypeA, MockDnsClientRule::ResultType::kTimeout, /*delay=*/false);
        AddDnsRule(&rules, host, dns_protocol::kTypeAAAA, MockDnsClientRule::ResultType::kTimeout, /*delay=*/false);
        SetDnsRules(std::move(rules));
    }
    ...
    {
        MockDnsClientRuleList rules;
        AddDnsRule(&rules, host, dns_protocol::kTypeA, MockDnsClientRule::ResultType::kOk, /*delay=*/false);
        AddDnsRule(&rules, host, dns_protocol::kTypeAAAA, MockDnsClientRule::ResultType::kOk, /*delay=*/false);
        SetDnsRules(std::move(rules));
    }
    ...
    {
        MockDnsClientRuleList rules;
        AddDnsRule(&rules, host, dns_protocol::kTypeA, MockDnsClientRule::ResultType::kOk, /*delay=*/true);
        AddDnsRule(&rules, host, dns_protocol::kTypeAAAA, MockDnsClientRule::ResultType::kOk, /*delay=*/false);
        SetDnsRules(std::move(rules));
    }
    ...
    void UseIpv6DelayedDnsRules(const std::string& host)
    {
        MockDnsClientRuleList rules;
        AddDnsRule(&rules, host, dns_protocol::kTypeA, MockDnsClientRule::ResultType::kOk, /*delay=*/false);
        AddDnsRule(&rules, host, dns_protocol::kTypeAAAA, MockDnsClientRule::ResultType::kOk, /*delay=*/true);
        SetDnsRules(std::move(rules));
    }
    ...
    void UseHttpsDelayedDnsRules(const std::string& host)
    {
        ...
        std::vector<DnsResourceRecord> records = { BuildTestHttpsServiceRecord(host, /*priority=*/1, /*service_name=*/".",
            /*params=*/ {}) };
        rules.emplace_back(host, dns_protocol::kTypeHttps,
            /*secure=*/false, MockDnsClientRule::Result(BuildTestDnsResponse(host, dns_protocol::kTypeHttps, records)),
            /*delay=*/true);
        SetDnsRules(std::move(rules));
    }

    std::unique_ptr<ServiceEndpointRequest> CreateRequest(std::string_view host, ResolveHostParameters parameters = ResolveHostParameters())
    {
        return resolver_->CreateServiceEndpointRequest(
            url::SchemeHostPort(GURL(host)), NetworkAnonymizationKey(), NetLogWithSource(), std::move(parameters), resolve_context_.get());
    }

    Requester CreateRequester(std::string_view host, ResolveHostParameters parameters = ResolveHostParameters())
    {
    ...
    LegacyRequester CreateLegacyRequester(std::string_view host)
    {
        return LegacyRequester(resolver_->CreateRequest(
            url::SchemeHostPort(GURL(host)), NetworkAnonymizationKey(), NetLogWithSource(), ResolveHostParameters(), resolve_context_.get()));
    }
...
TEST_F(HostResolverServiceEndpointRequestTest, NameNotResolved)
{
    ...
    EXPECT_THAT(requester.request()->GetResolveErrorInfo(), ResolveErrorInfo(ERR_NAME_NOT_RESOLVED));
}
...
TEST_F(HostResolverServiceEndpointRequestTest, TimedOut)
{
    ...
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
    requester.WaitForFinished();
    EXPECT_THAT(requester.finished_result(), Optional(IsError(ERR_NAME_NOT_RESOLVED)));
    EXPECT_THAT(requester.request()->GetResolveErrorInfo().error, IsError(ERR_DNS_TIMED_OUT));
}
...
TEST_F(HostResolverServiceEndpointRequestTest, Ok)
{
    ...
    EXPECT_THAT(requester.finished_endpoints(),
        ElementsAre(ExpectServiceEndpoint(ElementsAre(MakeIPEndPoint("127.0.0.1", 443)), ElementsAre(MakeIPEndPoint("::1", 443)))));
}

TEST_F(HostResolverServiceEndpointRequestTest, Ipv6GloballyReachableCheckAsyncOk)
{
...
TEST_F(HostResolverServiceEndpointRequestTest, Ipv6GloballyReachableCheckFail)
{
    ...
    requester.WaitForFinished();
    EXPECT_THAT(*requester.finished_result(), IsOk());
    EXPECT_THAT(requester.finished_endpoints(), ElementsAre(ExpectServiceEndpoint(ElementsAre(MakeIPEndPoint("127.0.0.1", 443)))));
...
TEST_F(HostResolverServiceEndpointRequestTest, ResolveLocally)
{
    ...
    // miss.
    {
        ...
        EXPECT_THAT(requester.request()->GetResolveErrorInfo(), ResolveErrorInfo(ERR_DNS_CACHE_MISS));
    }
...
TEST_F(HostResolverServiceEndpointRequestTest, Ipv6GloballyReachableCheckAsyncLocalOnly)
{
    set_globally_reachable_check_is_async(true);
    UseNonDelayedDnsRules("ok");
...
TEST_F(HostResolverServiceEndpointRequestTest, EndpointsAreSorted)
{
    ...
    DnsResponse a_response = BuildTestDnsResponse(kHost, dns_protocol::kTypeA,
        { BuildTestAddressRecord(kHost, *IPAddress::FromIPLiteral("192.0.2.2")), BuildTestAddressRecord(kHost, *IPAddress::FromIPLiteral("192.0.2.1")) });
    DnsResponse aaaa_response = BuildTestDnsResponse(kHost, dns_protocol::kTypeAAAA,
        { BuildTestAddressRecord(kHost, *IPAddress::FromIPLiteral("2001:db8::2")), BuildTestAddressRecord(kHost, *IPAddress::FromIPLiteral("2001:db8::1")) });
    ...
    CreateResolver();
    UseMockDnsClient(CreateValidDnsConfig(), std::move(rules));
    mock_dns_client_->SetAddressSorterForTesting(std::make_unique<FakeAddressSorter>());
    ...
    EXPECT_THAT(*requester.finished_result(), IsOk());
    EXPECT_THAT(requester.finished_endpoints(),
        ElementsAre(ExpectServiceEndpoint(ElementsAre(MakeIPEndPoint("192.0.2.1", 443), MakeIPEndPoint("192.0.2.2", 443)),
            ElementsAre(MakeIPEndPoint("2001:db8::1", 443), MakeIPEndPoint("2001:db8::2", 443)))));
}
...
TEST_F(HostResolverServiceEndpointRequestTest, Ipv4Slow)
{
    ...
    EXPECT_THAT(requester.request()->GetEndpointResults(), ElementsAre(ExpectServiceEndpoint(IsEmpty(), ElementsAre(MakeIPEndPoint("::1", 443)))));
    EXPECT_THAT(requester.request()->GetDnsAliasResults(), UnorderedElementsAre("4slow_ok"));
...
TEST_F(HostResolverServiceEndpointRequestTest, Ipv6SlowResolutionDelayPassed)
{
    ...
    FastForwardBy(DnsTaskResultsManager::kResolutionDelay + base::Milliseconds(1));
    RunUntilIdle();
    EXPECT_EQ(1u, resolver_->num_running_dispatcher_jobs_for_tests());
    ASSERT_FALSE(requester.finished_result().has_value());
    EXPECT_THAT(requester.request()->GetEndpointResults(), ElementsAre(ExpectServiceEndpoint(ElementsAre(MakeIPEndPoint("127.0.0.1", 443)), IsEmpty())));
...
TEST_F(HostResolverServiceEndpointRequestTest, HttpsSlow)
{
    ...
    EXPECT_THAT(requester.finished_endpoints(),
        ElementsAre(ExpectServiceEndpoint(ElementsAre(MakeIPEndPoint("127.0.0.1", 443)), ElementsAre(MakeIPEndPoint("::1", 443)),
            ...
            // Non-SVCB endpoints.
            ExpectServiceEndpoint(ElementsAre(MakeIPEndPoint("127.0.0.1", 443)), ElementsAre(MakeIPEndPoint("::1", 443)))));
}
...
TEST_F(HostResolverServiceEndpointRequestTest, DestroyResolverWhileUpdating)
{
    ...
    requester.SetOnUpdatedCallback(base::BindLambdaForTesting([&]() { DestroyResolver(); }));

    RunUntilIdle();
    EXPECT_THAT(requester.finished_result(), Optional(IsError(ERR_NAME_NOT_RESOLVED)));
    EXPECT_THAT(requester.request()->GetResolveErrorInfo().error, IsError(ERR_DNS_REQUEST_CANCELLED));
}
...
TEST_F(HostResolverServiceEndpointRequestTest, DestroyResolverWhileFinishing)
{
    ...
    requester.SetOnFinishedCallback(base::BindLambdaForTesting([&]() { DestroyResolver(); }));

    RunUntilIdle();
    EXPECT_THAT(*requester.finished_result(), IsOk());
}

TEST_F(HostResolverServiceEndpointRequestTest, EndpointsCryptoReadySystemTaskOnly)
{
    proc_->AddRuleForAllFamilies("a.test", "192.0.2.1");
    ResolveHostParameters parameters;
    parameters.source = HostResolverSource::SYSTEM;
    Requester requester = CreateRequester("https://a.test", std::move(parameters));
    int rv = requester.Start();
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
...
TEST_F(HostResolverServiceEndpointRequestTest, MultipleRequestsAddRequestInTheMiddleOfResolution)
{
    UseIpv4DelayedDnsRules("4slow_ok");
    ...
    EXPECT_THAT(requester1.request()->GetEndpointResults(), ElementsAre(ExpectServiceEndpoint(IsEmpty(), ElementsAre(MakeIPEndPoint("::1", 443)))));
    ...
    EXPECT_THAT(requester2.request()->GetEndpointResults(), ElementsAre(ExpectServiceEndpoint(IsEmpty(), ElementsAre(MakeIPEndPoint("::1", 443)))));
...
TEST_F(HostResolverServiceEndpointRequestTest, MultipleRequestsAddAndCancelRequestInUpdatedCallback)
{
    UseIpv4DelayedDnsRules("4slow_ok");
...
TEST_F(HostResolverServiceEndpointRequestTest, MultipleRequestsAddRequestInFinishedCallback)
{
    UseNonDelayedDnsRules("ok");
...
TEST_F(HostResolverServiceEndpointRequestTest, MultipleRequestsCancelOneRequestOnUpdated)
{
    UseIpv4DelayedDnsRules("4slow_ok");
...
TEST_F(HostResolverServiceEndpointRequestTest, MultipleRequestsCancelAllRequestOnUpdated)
{
    UseIpv4DelayedDnsRules("4slow_ok");
...
TEST_F(HostResolverServiceEndpointRequestTest, MultipleRequestsCancelAllRequestOnFinished)
{
    UseNonDelayedDnsRules("ok");
...
TEST_F(HostResolverServiceEndpointRequestTest, WithLegacyRequestDestroyResolverOnUpdated)
{
    UseIpv4DelayedDnsRules("4slow_ok");
...
TEST_F(HostResolverServiceEndpointRequestTest, WithLegacyRequestCancelRequestOnUpdated)
{
    UseIpv4DelayedDnsRules("4slow_ok");
...
TEST_F(HostResolverServiceEndpointRequestTest, WithLegacyRequestCancelLegacyRequest)
{
    UseNonDelayedDnsRules("ok");
