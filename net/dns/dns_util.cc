...
DohProviderEntry::List GetDohProviderEntriesFromNameservers(const std::vector<IPEndPoint>& dns_servers)
{
    ...
    for (const auto& server : dns_servers) {
        for (const net::DohProviderEntry* entry : providers) {
            ...
            if (base::Contains(entry->ip_addresses, server.address()) && base::FeatureList::IsEnabled(entry->feature.get())
...
bool GetTimeDeltaForConnectionTypeFromFieldTrial(const char* field_trial, NetworkChangeNotifier::ConnectionType type, base::TimeDelta* out)
{
...
base::TimeDelta GetTimeDeltaForConnectionTypeFromFieldTrialOrDefault(
    const char* field_trial, base::TimeDelta default_delta, NetworkChangeNotifier::ConnectionType type)
{
    base::TimeDelta out;
...
std::vector<DnsOverHttpsServerConfig> GetDohUpgradeServersFromDotHostname(const std::string& dot_server)
{
    ...
    for (const net::DohProviderEntry* entry : DohProviderEntry::GetList()) {
        ...
        if (base::Contains(entry->dns_over_tls_hostnames, dot_server) && base::FeatureList::IsEnabled(entry->feature.get())) {
...
std::vector<DnsOverHttpsServerConfig> GetDohUpgradeServersFromNameservers(const std::vector<IPEndPoint>& dns_servers)
{
    const auto entries = GetDohProviderEntriesFromNameservers(dns_servers);
    std::vector<DnsOverHttpsServerConfig> doh_servers;
    doh_servers.reserve(entries.size());
    base::ranges::transform(entries, std::back_inserter(doh_servers), &DohProviderEntry::doh_server_config);
    return doh_servers;
}

std::string GetDohProviderIdForHistogramFromServerConfig(const DnsOverHttpsServerConfig& doh_server)
{
    const auto& entries = DohProviderEntry::GetList();
    const auto it = base::ranges::find(entries, doh_server, &DohProviderEntry::doh_server_config);
    return it != entries.end() ? (*it)->provider : "Other";
}

std::string GetDohProviderIdForHistogramFromNameserver(const IPEndPoint& nameserver)
{
