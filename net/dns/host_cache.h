...
public:
    struct NET_EXPORT Key {
        ...
        // HostCache.
        Key(absl::variant<url::SchemeHostPort, std::string> host, DnsQueryType dns_query_type, HostResolverFlags host_resolver_flags,
        ...
        {
            return std::tie(key->dns_query_type, key->host_resolver_flags, key->host, key->host_resolver_source, key->network_anonymization_key, key->secure);
        }
    ...
    public:
        ...
        template <typename T>
        Entry(int error, T&& results, Source source, std::optional<base::TimeDelta> ttl)
            : error_(error)
            , source_(source)
        ...
        Entry(int error, std::vector<IPEndPoint> ip_endpoints, std::set<std::string> aliases, Source source, std::optional<base::TimeDelta> ttl = std::nullopt);

        // For errors with no |results|.
        Entry(int error, Source source, std::optional<base::TimeDelta> ttl = std::nullopt);
        ...
        // can be read.
        Entry(const std::set<std::unique_ptr<HostResolverInternalResult>>& results, base::Time now, base::TimeTicks now_ticks,
        ...
        bool operator==(const Entry& other) const
        {
            return ContentsEqual(other)
                && std::tie(source_, pinning_, ttl_, expires_, network_changes_, total_hits_, stale_hits_)
                == std::tie(other.source_, other.pinning_, other.ttl_, other.expires_, other.network_changes_, other.total_hits_, other.stale_hits_);
        }

        bool ContentsEqual(const Entry& other) const
        {
            return std::tie(error_, ip_endpoints_, endpoint_metadatas_, aliases_, text_records_, hostnames_, https_record_compatibility_, canonical_names_)
                == std::tie(other.error_, other.ip_endpoints_, other.endpoint_metadatas_, other.aliases_, other.text_records_, other.hostnames_,
        ...
        {
            return error_ != ERR_NETWORK_CHANGED && error_ != ERR_HOST_RESOLVER_QUEUE_TOO_LARGE;
        }
        ...
        const std::vector<HostPortPair>& hostnames() const
        {
            return hostnames_;
        }
        ...
        void set_https_record_compatibility(std::vector<bool> https_record_compatibility)
        {
            https_record_compatibility_ = std::move(https_record_compatibility);
        }
        std::optional<bool> pinning() const
        {
            return pinning_;
        }
        void set_pinning(std::optional<bool> pinning)
        {
            pinning_ = pinning;
        }
        ...
        void set_ttl(base::TimeDelta ttl)
        {
            ttl_ = ttl;
        }

        base::TimeTicks expires() const
        {
            return expires_;
        }
    ...
    private:
        ...
        Entry(const Entry& entry, base::TimeTicks now, base::TimeDelta ttl, int network_changes);

        Entry(int error, std::vector<IPEndPoint> ip_endpoints, std::multimap<HttpsRecordPriority, ConnectionEndpointMetadata> endpoint_metadatas,
            std::set<std::string> aliases, std::vector<std::string>&& text_results, std::vector<HostPortPair>&& hostnames,
        ...
        void SetResult(std::multimap<HttpsRecordPriority, ConnectionEndpointMetadata> endpoint_metadatas)
        {
        ...
        void GetStaleness(base::TimeTicks now, int network_changes, EntryStaleness* out) const;
        ...
        std::multimap<HttpsRecordPriority, ConnectionEndpointMetadata> endpoint_metadatas_;
    ...
    const std::pair<const Key, Entry>* Lookup(const Key& key, base::TimeTicks now, bool ignore_secure = false);
    ...
    // looking for a match. If there is no matching entry, returns NULL.
    const std::pair<const Key, Entry>* LookupStale(const Key& key, base::TimeTicks now, EntryStaleness* stale_out, bool ignore_secure = false);

    // Overwrites or creates an entry for |key|.
    // |entry| is the value to set, |now| is the current time
    // |ttl| is the "time to live".
    void Set(const Key& key, const Entry& entry, base::TimeTicks now, base::TimeDelta ttl);
    ...
    const HostCache::Key* GetMatchingKeyForTesting(
        std::string_view hostname, HostCache::Entry::Source* source_out = nullptr, HostCache::EntryStaleness* stale_out = nullptr) const;
    ...
    void ClearForHosts(const base::RepeatingCallback<bool(const std::string&)>& host_filter);
    ...
    void GetList(base::Value::List& entry_list, bool include_staleness, SerializationType serialization_type) const;
    ...
    size_t last_restore_size() const
    {
        return restore_size_;
    }
...
private:
    ...
    enum class LookupOutcome { kLookupMissAbsent = 0, kLookupMissStale = 1, kLookupHitValid = 2, kLookupHitStale = 3, kMaxValue = kLookupHitStale };
    ...
    enum class EraseReason { kEraseEvict = 0, kEraseClear = 1, kEraseDestruct = 2, kMaxValue = kEraseDestruct };
    ...
    // nullptr.
    static std::pair<const HostCache::Key, HostCache::Entry>* GetLessStaleMoreSecureResult(
    ...
    std::pair<const Key, Entry>* LookupInternalIgnoringFields(const Key& initial_key, base::TimeTicks now, bool ignore_secure);
    ...
    void RecordLookup(LookupOutcome outcome, base::TimeTicks now, const Key& key, const Entry* entry);

    // Record cache erase metrics for the `entry`.
    void RecordErase(EraseReason reason, base::TimeTicks now, const Key& key, const Entry& entry);
