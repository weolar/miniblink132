...
std::unique_ptr<OptRecordRdata::EdeOpt> OptRecordRdata::EdeOpt::Create(std::string data)
{
    uint16_t info_code;
    ...
    if (!edeReader.ReadU16BigEndian(info_code) || !base::OptionalUnwrapTo(edeReader.Read(edeReader.remaining()), extra_text)) {
        return nullptr;
    }
    ...
    return std::make_unique<EdeOpt>(info_code, std::string(base::as_string_view(extra_text)));
}
...
OptRecordRdata::EdeOpt::EdeInfoCode OptRecordRdata::EdeOpt::GetEnumFromInfoCode() const
{
    return GetEnumFromInfoCode(info_code_);
}

OptRecordRdata::EdeOpt::EdeInfoCode OptRecordRdata::EdeOpt::GetEnumFromInfoCode(uint16_t info_code)
{
    switch (info_code) {
    case 0:
...
std::unique_ptr<OptRecordRdata::UnknownOpt> OptRecordRdata::UnknownOpt::CreateForTesting(uint16_t code, std::string data)
{
    CHECK_IS_TEST();
    return base::WrapUnique(new OptRecordRdata::UnknownOpt(code, std::move(data)));
}
...
std::unique_ptr<OptRecordRdata> OptRecordRdata::Create(std::string_view data)
{
    ...
    while (reader.remaining() > 0u) {
        ...
        if (!reader.ReadU16BigEndian(opt_code) || !reader.ReadU16BigEndian(opt_data_size) || !base::OptionalUnwrapTo(reader.Read(opt_data_size), opt_data)) {
            return nullptr;
        }
        ...
        case dns_protocol::kEdnsPadding:
            opt = std::make_unique<OptRecordRdata::PaddingOpt>(std::string(base::as_string_view(opt_data)));
            break;
        case dns_protocol::kEdnsExtendedDnsError:
            opt = OptRecordRdata::EdeOpt::Create(std::string(base::as_string_view(opt_data)));
            break;
        default:
            opt = base::WrapUnique(new OptRecordRdata::UnknownOpt(opt_code, std::string(base::as_string_view(opt_data))));
            break;
        }
...
void OptRecordRdata::AddOpt(std::unique_ptr<Opt> opt)
{
    ...
    bool success = writer.WriteU16BigEndian(opt->GetCode()) && writer.WriteU16BigEndian(opt_data.size()) && writer.Write(base::as_byte_span(opt_data));
    DCHECK(success);
...
std::vector<const OptRecordRdata::PaddingOpt*> OptRecordRdata::GetPaddingOpts() const
{
