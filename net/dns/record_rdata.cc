...
// static
std::unique_ptr<SrvRecordRdata> SrvRecordRdata::Create(std::string_view data, const DnsRecordParser& parser)
{
    ...
    if (!parser.ReadName(data.substr(kSrvRecordMinimumSize).data(), &rdata->target_)) {
        return nullptr;
    }
...
// static
std::unique_ptr<ARecordRdata> ARecordRdata::Create(std::string_view data, const DnsRecordParser& parser)
{
    if (!HasValidSize(data, kType))
        return nullptr;
...
// static
std::unique_ptr<AAAARecordRdata> AAAARecordRdata::Create(std::string_view data, const DnsRecordParser& parser)
{
    if (!HasValidSize(data, kType))
        return nullptr;
...
// static
std::unique_ptr<CnameRecordRdata> CnameRecordRdata::Create(std::string_view data, const DnsRecordParser& parser)
{
...
// static
std::unique_ptr<PtrRecordRdata> PtrRecordRdata::Create(std::string_view data, const DnsRecordParser& parser)
{
...
// static
std::unique_ptr<TxtRecordRdata> TxtRecordRdata::Create(std::string_view data, const DnsRecordParser& parser)
{
...
// static
std::unique_ptr<NsecRecordRdata> NsecRecordRdata::Create(std::string_view data, const DnsRecordParser& parser)
{
    ...
    const BitmapHeader* header = reinterpret_cast<const BitmapHeader*>(data.data() + next_domain_length);
    ...
    rdata->bitmap_.insert(rdata->bitmap_.begin(), bitmap_data.begin(), bitmap_data.end());
