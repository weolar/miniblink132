...
// static
const base::TimeDelta DnsConfigService::kInvalidationTimeout = base::Milliseconds(150);

DnsConfigService::DnsConfigService(base::FilePath::StringPieceType hosts_file_path, std::optional<base::TimeDelta> config_change_delay)
...
DnsConfigService::HostsReader::HostsReader(base::FilePath::StringPieceType hosts_file_path, DnsConfigService& service)
    : service_(&service)
    , hosts_file_path_(hosts_file_path)
{
}
...
DnsConfigService::HostsReader::WorkItem::WorkItem(std::unique_ptr<DnsHostsParser> dns_hosts_parser)
...
bool DnsConfigService::HostsReader::WorkItem::AddAdditionalHostsTo(DnsHosts& in_out_dns_hosts)
{
...
std::unique_ptr<SerialWorker::WorkItem> DnsConfigService::HostsReader::CreateWorkItem()
{
    return std::make_unique<WorkItem>(std::make_unique<DnsHostsFileParser>(hosts_file_path_));
}

bool DnsConfigService::HostsReader::OnWorkFinished(std::unique_ptr<SerialWorker::WorkItem> serial_worker_work_item)
{
...
void DnsConfigService::ReadHostsNow()
{
    ...
    if (!hosts_reader_) {
        ...
        hosts_reader_ = std::make_unique<HostsReader>(hosts_file_path_.value(), *this);
    }
...
void DnsConfigService::StartTimer()
{
    ...
    timer_.Start(FROM_HERE, kInvalidationTimeout, this, &DnsConfigService::OnTimeout);
}
...
void DnsConfigService::OnConfigChanged(bool succeeded)
{
    if (config_change_delay_) {
        ...
        base::SequencedTaskRunner::GetCurrentDefault()->PostDelayedTask(
            FROM_HERE, base::BindOnce(&DnsConfigService::OnConfigChangedDelayed, weak_factory_.GetWeakPtr(), succeeded), config_change_delay_.value());
    } else {
