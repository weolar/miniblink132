...
std::optional<IPEndPoint> IpEndpointFromValue(const base::Value& value)
{
    ...
    if (!ip_str || !port || !base::IsValueInRangeForNumericType<uint16_t>(port.value())) {
        return std::nullopt;
    }
...
base::Value EndpointMetadataPairToValue(const std::pair<HttpsRecordPriority, ConnectionEndpointMetadata>& pair)
{
    base::Value::Dict dictionary;
...
std::optional<std::pair<HttpsRecordPriority, ConnectionEndpointMetadata>> EndpointMetadataPairFromValue(const base::Value& value)
{
    if (!value.is_dict())
        return std::nullopt;
    ...
    if (!priority || !base::IsValueInRangeForNumericType<HttpsRecordPriority>(priority.value())) {
        return std::nullopt;
    }

    if (!metadata_value)
        return std::nullopt;
    std::optional<ConnectionEndpointMetadata> metadata = ConnectionEndpointMetadata::FromValue(*metadata_value);
    if (!metadata)
        return std::nullopt;

    return std::pair(base::checked_cast<HttpsRecordPriority>(priority.value()), std::move(metadata).value());
}

bool IPEndPointsFromLegacyAddressListValue(const base::Value::List& value, std::vector<IPEndPoint>& ip_endpoints)
{
...
template <typename T> void MergeLists(T& target, const T& source)
{
    target.insert(target.end(), source.begin(), source.end());
}

template <typename T> void MergeContainers(T& target, const T& source)
{
...
bool IsValidHostname(std::string_view hostname)
{
    ...
    if (ip_address.AssignFromIPLiteral(hostname) || ParseURLHostnameToAddress(hostname, &ip_address)) {
        return false;
    }
...
const std::string& GetHostname(const absl::variant<url::SchemeHostPort, std::string>& host)
{
...
const std::string GetHistogramName(std::string_view histogram_name, const HostCache::Key& key)
{
    constexpr std::string_view kHistogramPrefix = "Net.DNS.HostCache.";

    return base::StrCat({ kHistogramPrefix, histogram_name, IsGoogleHostWithAlpnH3(GetHostname(key.host)) ? ".GoogleHost" : "" });
}

} // namespace

// Used in histograms; do not modify existing values.
enum HostCache::SetOutcome : int { SET_INSERT = 0, SET_UPDATE_VALID = 1, SET_UPDATE_STALE = 2, MAX_SET_OUTCOME };

HostCache::Key::Key(absl::variant<url::SchemeHostPort, std::string> host, DnsQueryType dns_query_type, HostResolverFlags host_resolver_flags,
    HostResolverSource host_resolver_source, const NetworkAnonymizationKey& network_anonymization_key)
    : host(std::move(host))
...
HostCache::Entry::Entry(int error, Source source, std::optional<base::TimeDelta> ttl)
    : error_(error)
    , source_(source)
    , ttl_(ttl.value_or(kUnknownTtl))
{
    ...
    static_assert(std::is_same<net::HttpsRecordPriority, HostCache::Entry::HttpsRecordPriority>::value,
        "`net::HttpsRecordPriority` and "
        "`HostCache::Entry::HttpsRecordPriority` must be same type");
}

HostCache::Entry::Entry(const std::set<std::unique_ptr<HostResolverInternalResult>>& results, base::Time now, base::TimeTicks now_ticks, Source empty_source)
{
    ...
    std::optional<base::TimeDelta> smallest_ttl = TtlFromInternalResults(results, now, now_ticks);
    ...
    for (const std::unique_ptr<HostResolverInternalResult>& result : results) {
        ...
        case HostResolverInternalResult::Type::kData:
            if (!result->AsData().endpoints().empty() && result->AsData().endpoints().front().GetFamily() == ADDRESS_FAMILY_IPV6) {
    ...
    if (!data_results.empty() || metadata_result) {
        ...
        // transaction.
        CHECK(base::ranges::all_of(error_results, [](const HostResolverInternalResult* error_result) {
            return error_result->query_type() != DnsQueryType::UNSPECIFIED && error_result->AsError().error() == ERR_NAME_NOT_RESOLVED;
        }));
        ...
        for (const HostResolverInternalResult* error_result : error_results) {
            if (error_result->expiration().has_value() || error_result->timed_expiration().has_value()) {
                any_error_cacheable = true;
            }

            if (error_result->AsError().error() != ERR_NAME_NOT_RESOLVED || error_result->query_type() == DnsQueryType::UNSPECIFIED) {
    ...
    if (!data_results.empty()) {
        for (const HostResolverInternalResult* data_result : data_results) {
            DCHECK(!data_result->AsData().endpoints().empty() || !data_result->AsData().strings().empty() || !data_result->AsData().hosts().empty());
            // Data results should always be cacheable.
            DCHECK(data_result->expiration().has_value() || data_result->timed_expiration().has_value());
    ...
    if (metadata_result) {
        ...
        DCHECK(metadata_result->expiration().has_value() || metadata_result->timed_expiration().has_value());
...
HostCache::Entry HostCache::Entry::MergeEntries(Entry front, Entry back)
{
    ...
    front.error_ = front.error() == OK || back.error() == OK ? OK : ERR_NAME_NOT_RESOLVED;
    ...
    MergeLists(front.https_record_compatibility_, back.https_record_compatibility_);
    ...
    front.network_changes_ = std::max(front.network_changes(), back.network_changes());
...
HostCache::Entry::Entry(int error, std::vector<IPEndPoint> ip_endpoints, std::set<std::string> aliases, Source source, std::optional<base::TimeDelta> ttl)
    : error_(error)
...
HostCache::Entry::Entry(const HostCache::Entry& entry, base::TimeTicks now, base::TimeDelta ttl, int network_changes)
    : error_(entry.error())
...
HostCache::Entry::Entry(int error, std::vector<IPEndPoint> ip_endpoints, std::multimap<HttpsRecordPriority, ConnectionEndpointMetadata> endpoint_metadatas,
    std::set<std::string> aliases, std::vector<std::string>&& text_records, std::vector<HostPortPair>&& hostnames,
    std::vector<bool>&& https_record_compatibility, Source source, base::TimeTicks expires, int network_changes)
    : error_(error)
    , ip_endpoints_(std::move(ip_endpoints))
...
void HostCache::Entry::GetStaleness(base::TimeTicks now, int network_changes, EntryStaleness* out) const
{
    DCHECK(out);
...
base::Value::Dict HostCache::Entry::GetAsValue(bool include_staleness) const
{
    ...
    if (include_staleness) {
        ...
        base::Time expiration_time = base::Time::Now() - (base::TimeTicks::Now() - expires());
        entry_dict.Set(kExpirationKey, base::NumberToString(expiration_time.ToInternalValue()));
    }
    ...
    if (error() != OK) {
        ...
        for (const auto& endpoint_metadata_pair : endpoint_metadatas_) {
            endpoint_metadatas_list.Append(EndpointMetadataPairToValue(endpoint_metadata_pair));
        }
...
std::optional<base::TimeDelta> HostCache::Entry::TtlFromInternalResults(
    const std::set<std::unique_ptr<HostResolverInternalResult>>& results, base::Time now, base::TimeTicks now_ticks)
{
    std::optional<base::TimeDelta> smallest_ttl;
    for (const std::unique_ptr<HostResolverInternalResult>& result : results) {
        if (result->expiration().has_value()) {
            smallest_ttl = std::min(smallest_ttl.value_or(base::TimeDelta::Max()), result->expiration().value() - now_ticks);
        }
        if (result->timed_expiration().has_value()) {
            smallest_ttl = std::min(smallest_ttl.value_or(base::TimeDelta::Max()), result->timed_expiration().value() - now);
        }
    }
    return smallest_ttl;
}
...
// static
const HostCache::EntryStaleness HostCache::kNotStale = { base::Seconds(-1), 0, 0 };
...
const std::pair<const HostCache::Key, HostCache::Entry>* HostCache::Lookup(const Key& key, base::TimeTicks now, bool ignore_secure)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
const std::pair<const HostCache::Key, HostCache::Entry>* HostCache::LookupStale(
    const Key& key, base::TimeTicks now, HostCache::EntryStaleness* stale_out, bool ignore_secure)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    if (caching_is_disabled())
        return nullptr;
    ...
    RecordLookup(is_stale ? LookupOutcome::kLookupHitStale : LookupOutcome::kLookupHitValid, now, result->first, entry);
...
// static
std::pair<const HostCache::Key, HostCache::Entry>* HostCache::GetLessStaleMoreSecureResult(
    base::TimeTicks now, std::pair<const HostCache::Key, HostCache::Entry>* result1, std::pair<const HostCache::Key, HostCache::Entry>* result2)
{
    ...
    if (staleness1.network_changes == staleness2.network_changes) {
        ...
        // non-expired result.
        if (staleness1.expired_by.is_negative() && staleness2.expired_by >= base::TimeDelta()) {
            return result1;
        }
        if (staleness1.expired_by >= base::TimeDelta() && staleness2.expired_by.is_negative()) {
            return result2;
        }
    ...
    return (staleness1.network_changes < staleness2.network_changes) ? result1 : result2;
}

std::pair<const HostCache::Key, HostCache::Entry>* HostCache::LookupInternalIgnoringFields(const Key& initial_key, base::TimeTicks now, bool ignore_secure)
{
    std::pair<const HostCache::Key, HostCache::Entry>* preferred_result = LookupInternal(initial_key);

    if (ignore_secure) {
        Key effective_key = initial_key;
        effective_key.secure = !initial_key.secure;
        preferred_result = GetLessStaleMoreSecureResult(now, preferred_result, LookupInternal(effective_key));
    }

    return preferred_result;
}

std::pair<const HostCache::Key, HostCache::Entry>* HostCache::LookupInternal(const Key& key)
{
    auto it = entries_.find(key);
    return (it != entries_.end()) ? &*it : nullptr;
}

void HostCache::Set(const Key& key, const Entry& entry, base::TimeTicks now, base::TimeDelta ttl)
{
...
const HostCache::Key* HostCache::GetMatchingKeyForTesting(
    std::string_view hostname, HostCache::Entry::Source* source_out, HostCache::EntryStaleness* stale_out) const
{
    for (const EntryMap::value_type& entry : entries_) {
        if (GetHostname(entry.first.host) == hostname) {
            ...
            if (stale_out != nullptr) {
                entry.second.GetStaleness(tick_clock_->NowTicks(), network_changes_, stale_out);
            }
            return &entry.first;
        }
    }
...
void HostCache::ClearForHosts(const base::RepeatingCallback<bool(const std::string&)>& host_filter)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
void HostCache::GetList(base::Value::List& entry_list, bool include_staleness, SerializationType serialization_type) const
{
    entry_list.clear();
    ...
    for (const auto& pair : entries_) {
        ...
        if (serialization_type == SerializationType::kRestorable) {
            ...
            network_anonymization_key_value = base::Value(key.network_anonymization_key.ToDebugString());
        }
        ...
        entry_dict.Set(kDnsQueryTypeKey, base::strict_cast<int>(key.dns_query_type));
        entry_dict.Set(kFlagsKey, key.host_resolver_flags);
        entry_dict.Set(kHostResolverSourceKey, base::strict_cast<int>(key.host_resolver_source));
        entry_dict.Set(kNetworkAnonymizationKey, std::move(network_anonymization_key_value));
...
bool HostCache::RestoreFromListValue(const base::Value::List& old_cache)
{
    ...
    for (const auto& entry : old_cache) {
        ...
        if (scheme_ptr) {
            ...
            url::SchemeHostPort scheme_host_port(*scheme_ptr, *hostname_ptr, port.value());
        ...
        std::optional<int> maybe_dns_query_type = entry_dict.FindInt(kDnsQueryTypeKey);
        if (!maybe_dns_query_type.has_value())
            return false;
        std::optional<DnsQueryType> dns_query_type = GetDnsQueryType(maybe_dns_query_type.value());
        if (!dns_query_type.has_value())
            return false;
        // HostResolverSource is optional.
        int host_resolver_source = entry_dict.FindInt(kHostResolverSourceKey).value_or(base::strict_cast<int>(HostResolverSource::ANY));

        const base::Value* network_anonymization_key_value = entry_dict.Find(kNetworkAnonymizationKey);
        NetworkAnonymizationKey network_anonymization_key;
        if (!network_anonymization_key_value || network_anonymization_key_value->type() == base::Value::Type::STRING
            || !NetworkAnonymizationKey::FromValue(*network_anonymization_key_value, &network_anonymization_key)) {
            return false;
        }
        ...
        if (maybe_error.has_value()) {
            ...
            if ((hostname_records_list == nullptr && host_ports_list != nullptr) || (hostname_records_list != nullptr && host_ports_list == nullptr)) {
                return false;
            }
        }
        ...
        base::TimeTicks expiration_time = tick_clock_->NowTicks() - (base::Time::Now() - base::Time::FromInternalValue(time_internal));

        std::vector<IPEndPoint> ip_endpoints;
        if (ip_endpoints_list) {
            for (const base::Value& ip_endpoint_value : *ip_endpoints_list) {
                std::optional<IPEndPoint> ip_endpoint = IpEndpointFromValue(ip_endpoint_value);
                if (!ip_endpoint)
                    return false;
        ...
        if (endpoint_metadatas_list) {
            for (const base::Value& endpoint_metadata_value : *endpoint_metadatas_list) {
                std::optional<std::pair<HttpsRecordPriority, ConnectionEndpointMetadata>> pair = EndpointMetadataPairFromValue(endpoint_metadata_value);
                if (!pair)
                    return false;
        ...
        if (legacy_addresses_list) {
            ...
            if (!IPEndPointsFromLegacyAddressListValue(*legacy_addresses_list, ip_endpoints)) {
                return false;
            }
        }
        ...
        if (hostname_records_list) {
            ...
            for (size_t i = 0; i < hostname_records_list->size(); ++i) {
                if (!(*hostname_records_list)[i].is_string() || !(*host_ports_list)[i].is_int()
                    || !base::IsValueInRangeForNumericType<uint16_t>((*host_ports_list)[i].GetInt())) {
                    return false;
                }
                hostname_records.emplace_back((*hostname_records_list)[i].GetString(), base::checked_cast<uint16_t>((*host_ports_list)[i].GetInt()));
            }
        }
        ...
        Key key(std::move(host), dns_query_type.value(), flags, static_cast<HostResolverSource>(host_resolver_source), network_anonymization_key);
        key.secure = secure;
        ...
        if (found == entries_.end()) {
            Entry new_entry(error, std::move(ip_endpoints), std::move(endpoint_metadatas), std::move(aliases), std::move(text_records),
                std::move(hostname_records), std::move(experimental_results), Entry::SOURCE_UNKNOWN, expiration_time, network_changes_ - 1);
...
bool HostCache::EvictOneEntry(base::TimeTicks now)
{
    ...
    for (auto it = entries_.begin(); it != entries_.end(); ++it) {
        ...
        if ((entry.expires() < oldest.expires()) && (entry.IsStale(now, network_changes_) || !oldest.IsStale(now, network_changes_))) {
            oldest_it = it;
        }
    }

    if (oldest_it) {
        RecordErase(EraseReason::kEraseEvict, now, (*oldest_it)->first, (*oldest_it)->second);
...
{
    return entry.pinning().value_or(false) && entry.network_changes() == network_changes();
}

void HostCache::RecordLookup(LookupOutcome outcome, base::TimeTicks now, const Key& key, const Entry* entry)
{
    ...
    if (outcome == LookupOutcome::kLookupHitStale) {
        ...
        base::UmaHistogramLongTimes(GetHistogramName("LookupStale.ExpiredBy", key), now - entry->expires());
        base::UmaHistogramCounts1000(GetHistogramName("LookupStale.NetworkChanges", key), network_changes_ - entry->network_changes());
    }
}

void HostCache::RecordErase(EraseReason reason, base::TimeTicks now, const Key& key, const Entry& entry)
{
    HostCache::EntryStaleness stale;
    entry.GetStaleness(now, network_changes_, &stale);
    base::UmaHistogramEnumeration(GetHistogramName("Erase", key), reason);
    if (stale.is_stale()) {
        base::UmaHistogramLongTimes(GetHistogramName("EraseStale.ExpiredBy", key), stale.expired_by);
        base::UmaHistogramCounts1000(GetHistogramName("EraseStale.NetworkChanges", key), stale.network_changes);
        base::UmaHistogramCounts1000(GetHistogramName("EraseStale.StaleHits", key), entry.stale_hits());
    } else {
        base::UmaHistogramLongTimes(GetHistogramName("EraseValid.ValidFor", key), -stale.expired_by);
    }
}
...
std::ostream& operator<<(std::ostream& out, const net::HostCache::EntryStaleness& s)
{
    return out << "EntryStaleness{" << s.expired_by << ", " << s.network_changes << ", " << s.stale_hits << "}";
}
