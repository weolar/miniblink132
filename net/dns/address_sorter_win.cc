...
private:
    ...
    public:
        static void Start(const std::vector<IPEndPoint>& endpoints, CallbackType callback)
        {
            ...
            base::ThreadPool::PostTaskAndReply(FROM_HERE, { base::MayBlock(), base::TaskShutdownBehavior::CONTINUE_ON_SHUTDOWN },
                base::BindOnce(&Job::Run, job), base::BindOnce(&Job::OnComplete, job));
        }
    ...
    private:
        ...
        Job(const std::vector<IPEndPoint>& endpoints, CallbackType callback)
            ...
            , buffer_size_((sizeof(SOCKET_ADDRESS_LIST) + base::CheckedNumeric<DWORD>(endpoints.size()) * (sizeof(SOCKET_ADDRESS) + sizeof(SOCKADDR_STORAGE)))
                               .ValueOrDie<DWORD>())
            , input_buffer_(reinterpret_cast<SOCKET_ADDRESS_LIST*>(malloc(buffer_size_)))
            , output_buffer_(reinterpret_cast<SOCKET_ADDRESS_LIST*>(malloc(buffer_size_)))
        {
            input_buffer_->iAddressCount = base::checked_cast<INT>(endpoints.size());
            SOCKADDR_STORAGE* storage = reinterpret_cast<SOCKADDR_STORAGE*>(input_buffer_->Address + input_buffer_->iAddressCount);

            for (size_t i = 0; i < endpoints.size(); ++i) {
                IPEndPoint ipe = endpoints[i];
                // Addresses must be sockaddr_in6.
                if (ipe.address().IsIPv4()) {
                    ipe = IPEndPoint(ConvertIPv4ToIPv4MappedIPv6(ipe.address()), ipe.port());
                }
        ...
        void Run()
        {
            ...
            DWORD result_size = 0;
            int result
                = WSAIoctl(sock, SIO_ADDRESS_LIST_SORT, input_buffer_.get(), buffer_size_, output_buffer_.get(), buffer_size_, &result_size, nullptr, nullptr);
        ...
        void OnComplete()
        {
            ...
            if (success_) {
                ...
                for (int i = 0; i < output_buffer_->iAddressCount; ++i) {
                    IPEndPoint ipe;
                    bool result = ipe.FromSockAddr(output_buffer_->Address[i].lpSockaddr, output_buffer_->Address[i].iSockaddrLength);
                    ...
                    if (ipe.address().IsIPv4MappedIPv6()) {
                        ipe = IPEndPoint(ConvertIPv4MappedIPv6ToIPv4(ipe.address()), ipe.port());
                    }
