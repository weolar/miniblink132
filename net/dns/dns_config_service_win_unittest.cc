...
TEST(DnsConfigServiceWinTest, ParseSearchList)
{
    const struct TestCase {
        ...
        { L"\u017c\xf3\u0142ta.pi\u0119\u015b\u0107.pl,pl", { "xn--ta-4ja03asj.xn--pi-wla5e0q.pl", "pl" } },
...
std::unique_ptr<IP_ADAPTER_ADDRESSES, base::FreeDeleter> CreateAdapterAddresses(const AdapterInfo* infos)
{
    ...
    std::unique_ptr<IP_ADAPTER_ADDRESSES, base::FreeDeleter> heap(static_cast<IP_ADAPTER_ADDRESSES*>(malloc(heap_size)));
    CHECK(heap.get());
...
TEST(DnsConfigServiceWinTest, ConvertAdapterAddresses)
{
    ...
    const struct TestCase {
    ...
    } cases[] = {
        {
            ...
            {
                { IF_TYPE_SOFTWARE_LOOPBACK, IfOperStatusUp, L"funnyloop", { "2.0.0.2" } },
                { IF_TYPE_FASTETHER, IfOperStatusDormant, L"example.com", { "1.0.0.1" } },
                { IF_TYPE_USB, IfOperStatusUp, L"chromium.org", { "10.0.0.10", "2001:FFFF::1111" } },
                { 0 },
            },
        ...
        },
        {
            ...
            {
                { IF_TYPE_USB, IfOperStatusUp, L"chromium.org", { "10.0.0.10", "2001:FFFF::1111" }, { 1024, 24 } },
                { 0 },
            },
            { "10.0.0.10", "2001:FFFF::1111" },
            "chromium.org",
        ...
        },
        {
            ...
            {
                { IF_TYPE_SOFTWARE_LOOPBACK, IfOperStatusUp, L"funnyloop", { "2.0.0.2" } },
                { IF_TYPE_FASTETHER, IfOperStatusUp, L"example.com", { "1.0.0.1", "fec0:0:0:ffff::2", "8.8.8.8" } },
                { IF_TYPE_USB, IfOperStatusUp, L"chromium.org", { "10.0.0.10", "2001:FFFF::1111" } },
                { 0 },
            },
            { "1.0.0.1", "8.8.8.8" },
        ...
        },
        {
            // No usable adapters.
            {
                { IF_TYPE_SOFTWARE_LOOPBACK, IfOperStatusUp, L"localhost", { "2.0.0.2" } },
                { IF_TYPE_FASTETHER, IfOperStatusDormant, L"example.com", { "1.0.0.1" } },
    ...
    for (const auto& t : cases) {
        ...
        base::expected<DnsConfig, ReadWinSystemDnsSettingsError> config_or_error = internal::ConvertSettingsToDnsConfig(std::move(settings));
        ...
        if (config_or_error.has_value()) {
            ...
            EXPECT_THAT(config_or_error->search, testing::ElementsAre(t.expected_suffix));
        }
    }
}
...
TEST(DnsConfigServiceWinTest, ConvertSuffixSearch)
{
    ...
    const struct TestCase {
        ...
        },
        {
            ...
            {
                std::nullopt, std::nullopt, L"a.b.c.d.e", std::nullopt, { 1, std::nullopt }, // policy_devolution: enabled, level
    ...
    for (auto& t : cases) {
        ...
        ASSERT_OK_AND_ASSIGN(DnsConfig dns_config, internal::ConvertSettingsToDnsConfig(std::move(settings)));
        EXPECT_THAT(dns_config, testing::Field(&DnsConfig::search, testing::ElementsAreArray(t.expected_search)));
    }
}
...
TEST(DnsConfigServiceWinTest, AppendToMultiLabelName)
{
    ...
    for (const auto& t : cases) {
        ...
        ASSERT_OK_AND_ASSIGN(DnsConfig dns_config, internal::ConvertSettingsToDnsConfig(std::move(settings)));
        EXPECT_THAT(dns_config, testing::Field(&DnsConfig::append_to_multi_label_name, testing::Eq(t.expected_output)));
    }
}
...
TEST(DnsConfigServiceWinTest, HaveProxy)
{
    ...
    for (const auto& t : cases) {
        ...
        ASSERT_OK_AND_ASSIGN(DnsConfig dns_config, internal::ConvertSettingsToDnsConfig(std::move(settings)));
        EXPECT_THAT(dns_config, testing::Field(&DnsConfig::unhandled_options, testing::Eq(t.unhandled_options)));
    }
}
...
TEST(DnsConfigServiceWinTest, UsesVpn)
{
    ...
    settings.addresses = CreateAdapterAddresses(infos);
    ASSERT_OK_AND_ASSIGN(DnsConfig dns_config, internal::ConvertSettingsToDnsConfig(std::move(settings)));
    EXPECT_THAT(dns_config, testing::Field(&DnsConfig::unhandled_options, testing::IsTrue()));
}
...
TEST(DnsConfigServiceWinTest, AdapterSpecificNameserversForNo)
{
    AdapterInfo infos[3] = {
        { IF_TYPE_FASTETHER, IfOperStatusUp, L"example.com", { "1.0.0.1", "fec0:0:0:ffff::2", "8.8.8.8" } },
        { IF_TYPE_USB, IfOperStatusDown, L"chromium.org", { "10.0.0.10", "2001:FFFF::1111" } },
        { 0 },
    };
    ...
    settings.addresses = CreateAdapterAddresses(infos);
    ASSERT_OK_AND_ASSIGN(DnsConfig dns_config, internal::ConvertSettingsToDnsConfig(std::move(settings)));
    EXPECT_THAT(dns_config, testing::Field(&DnsConfig::unhandled_options, testing::IsFalse()));
}
