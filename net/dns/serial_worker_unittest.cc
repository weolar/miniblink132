...
static const BackoffEntry::Policy kTestBackoffPolicy = {
    ...
    static_cast<int>(kBackoffInitialDelay.InMilliseconds()), // Initial delay for backoff in ms.
    kBackoffMultiplyFactor, // Factor to multiply for exponential backoff.
    0, // Fuzzing percentage.
    static_cast<int>(base::Seconds(1).InMilliseconds()), // Maximum time to delay requests
    // in ms: 1 second.
...
public:
    ...
    public:
        ...
        public:
            explicit TestWorkItem(SerialWorkerTest* test)
                : test_(test)
            {
            }
        ...
        explicit TestSerialWorker(SerialWorkerTest* t)
            : SerialWorker(/*max_number_of_retries=*/kMaxRetries, &kTestBackoffPolicy)
            , test_(t)
        {
        }
        ...
        bool OnWorkFinished(std::unique_ptr<SerialWorker::WorkItem> work_item) override
        {
            CHECK(test_);
...
protected:
    ...
    {
        task_runner_->PostTask(FROM_HERE, base::BindOnce(&SerialWorkerTest::BreakCallback, base::Unretained(this), b));
    }
    ...
    SerialWorkerTest()
        ...
        , work_allowed_(base::WaitableEvent::ResetPolicy::AUTOMATIC, base::WaitableEvent::InitialState::NOT_SIGNALED)
        , work_called_(base::WaitableEvent::ResetPolicy::AUTOMATIC, base::WaitableEvent::InitialState::NOT_SIGNALED)
    {
    }
    ...
    std::unique_ptr<TestSerialWorker> worker_ = std::make_unique<TestSerialWorker>(this);
...
TEST_F(SerialWorkerTest, MultipleFailureExponentialBackoff)
{
    ...
    for (int retry_attempt_count = 1; retry_attempt_count <= kMaxRetries; retry_attempt_count++) {
        // Confirm it failed and that a retry was scheduled.
        ASSERT_EQ(retry_attempt_count, worker_->GetBackoffEntryForTesting().failure_count());
        EXPECT_TRUE(worker_->GetRetryTimerForTesting().IsRunning());
        base::TimeDelta expected_backoff_delay;
        if (retry_attempt_count == 1) {
            expected_backoff_delay = kBackoffInitialDelay;
        } else {
            expected_backoff_delay = kBackoffInitialDelay * kBackoffMultiplyFactor * (retry_attempt_count - 1);
        }
        EXPECT_EQ(expected_backoff_delay, worker_->GetBackoffEntryForTesting().GetTimeUntilRelease()) << "retry_attempt_count=" << retry_attempt_count;
