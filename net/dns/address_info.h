...
public:
    ...
    // Constructors
    using AddressInfoAndResult = std::tuple<std::optional<AddressInfo>, int /* err */, int /* os_error */>;
    ...
    static AddressInfoAndResult Get(const std::string& host, const addrinfo& hints, std::unique_ptr<AddrInfoGetter> getter = nullptr,
...
private:
    // Constructors
    AddressInfo(std::unique_ptr<addrinfo, FreeAddrInfoFunc> ai, std::unique_ptr<AddrInfoGetter> getter);

    // Data.
    std::unique_ptr<addrinfo, FreeAddrInfoFunc> ai_; // Never null (except after move)
