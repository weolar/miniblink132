...
public:
    ...
    // of this class.
    DnsTaskResultsManager(Delegate* delegate, HostResolver::Host host, DnsQueryTypeSet query_types, const NetLogWithSource& net_log);
    ...
    void ProcessDnsTransactionResults(DnsQueryType query_type, const std::set<std::unique_ptr<HostResolverInternalResult>>& results);
...
private:
    ...
    std::map</*domain_name=*/std::string, std::unique_ptr<PerDomainResult>> per_domain_results_;
