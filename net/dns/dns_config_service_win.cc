...
// Registry key paths.
const wchar_t kTcpipPath[] = L"SYSTEM\\CurrentControlSet\\Services\\Tcpip\\Parameters";
const wchar_t kTcpip6Path[] = L"SYSTEM\\CurrentControlSet\\Services\\Tcpip6\\Parameters";
const wchar_t kDnscachePath[] = L"SYSTEM\\CurrentControlSet\\Services\\Dnscache\\Parameters";
const wchar_t kPolicyPath[] = L"SOFTWARE\\Policies\\Microsoft\\Windows NT\\DNSClient";
...
inline constexpr DnsWindowsCompatibility operator|(DnsWindowsCompatibility a, DnsWindowsCompatibility b)
{
    return static_cast<DnsWindowsCompatibility>(static_cast<int>(a) | static_cast<int>(b));
}

inline DnsWindowsCompatibility& operator|=(DnsWindowsCompatibility& a, DnsWindowsCompatibility b)
{
    return a = a | b;
}
...
std::unique_ptr<IP_ADAPTER_ADDRESSES, base::FreeDeleter> ReadAdapterUnicastAddresses()
{
    ...
    for (unsigned tries = 0; (tries < 3) && (rv == ERROR_BUFFER_OVERFLOW); tries++) {
        ...
        rv = GetAdaptersAddresses(
            AF_UNSPEC, GAA_FLAG_SKIP_ANYCAST | GAA_FLAG_SKIP_DNS_SERVER | GAA_FLAG_SKIP_MULTICAST | GAA_FLAG_SKIP_FRIENDLY_NAME, nullptr, out.get(), &len);
    }
    if (rv != NO_ERROR)
        out.reset();
    return out;
...
bool AddLocalhostEntriesTo(DnsHosts& in_out_hosts)
{
    ...
    in_out_hosts.emplace(DnsHostsKey("localhost", ADDRESS_FAMILY_IPV4), loopback_ipv4);
    in_out_hosts.emplace(DnsHostsKey("localhost", ADDRESS_FAMILY_IPV6), loopback_ipv6);
    ...
    bool have_ipv4 = in_out_hosts.count(DnsHostsKey(localname, ADDRESS_FAMILY_IPV4)) > 0;
    bool have_ipv6 = in_out_hosts.count(DnsHostsKey(localname, ADDRESS_FAMILY_IPV6)) > 0;

    if (have_ipv4 && have_ipv6)
        return true;

    std::unique_ptr<IP_ADAPTER_ADDRESSES, base::FreeDeleter> addresses = ReadAdapterUnicastAddresses();
    if (!addresses.get())
        return false;

    // The order of adapters is the network binding order, so stick to the
    // first good adapter for each family.
    for (const IP_ADAPTER_ADDRESSES* adapter = addresses.get(); adapter != nullptr && (!have_ipv4 || !have_ipv6); adapter = adapter->Next) {
        ...
        for (const IP_ADAPTER_UNICAST_ADDRESS* address = adapter->FirstUnicastAddress; address != nullptr; address = address->Next) {
            IPEndPoint ipe;
            if (!ipe.FromSockAddr(address->Address.lpSockaddr, address->Address.iSockaddrLength)) {
                return false;
            }
            if (!have_ipv4 && (ipe.GetFamily() == ADDRESS_FAMILY_IPV4)) {
                have_ipv4 = true;
                in_out_hosts[DnsHostsKey(localname, ADDRESS_FAMILY_IPV4)] = ipe.address();
            } else if (!have_ipv6 && (ipe.GetFamily() == ADDRESS_FAMILY_IPV6)) {
                have_ipv6 = true;
                in_out_hosts[DnsHostsKey(localname, ADDRESS_FAMILY_IPV6)] = ipe.address();
            }
        }
    }
    return true;
...
public:
    ...
    bool Watch(const wchar_t key[], const CallbackType& callback)
    {
        ...
        return key_.StartWatching(base::BindOnce(&RegistryWatcher::OnObjectSignaled, base::Unretained(this)));
    }
    ...
    void OnObjectSignaled()
    {
        DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
        DCHECK(!callback_.is_null());
        if (key_.StartWatching(base::BindOnce(&RegistryWatcher::OnObjectSignaled, base::Unretained(this)))) {
            callback_.Run(true);
        } else {
            key_.Close();
            callback_.Run(false);
...
base::FilePath GetHostsPath()
{
    ...
    return base::FilePath(buffer).Append(FILE_PATH_LITERAL("drivers\\etc\\hosts"));
}

void ConfigureSuffixSearch(const WinDnsSystemSettings& settings, DnsConfig& in_out_config)
{
    // SearchList takes precedence, so check it first.
    if (settings.policy_search_list.has_value()) {
        std::vector<std::string> search = ParseSearchList(settings.policy_search_list.value());
        if (!search.empty()) {
        ...
        std::vector<std::string> search = ParseSearchList(settings.tcpip_search_list.value());
        if (!search.empty()) {
            in_out_config.search = std::move(search);
            return;
        }
    ...
    WinDnsSystemSettings::DevolutionSetting devolution = settings.policy_devolution;
...
std::optional<std::vector<IPEndPoint>> GetNameServers(const IP_ADAPTER_ADDRESSES* adapter)
{
    std::vector<IPEndPoint> nameservers;
    for (const IP_ADAPTER_DNS_SERVER_ADDRESS* address = adapter->FirstDnsServerAddress; address != nullptr; address = address->Next) {
        IPEndPoint ipe;
        if (ipe.FromSockAddr(address->Address.lpSockaddr, address->Address.iSockaddrLength)) {
...
bool CheckAndRecordCompatibility(bool have_name_resolution_policy, bool have_proxy, bool uses_vpn, bool has_adapter_specific_nameservers)
{
    ...
    if (has_adapter_specific_nameservers) {
        compatibility |= DnsWindowsCompatibility::kIncompatibleAdapterSpecificNameserver;
    }
    base::UmaHistogramEnumeration("Net.DNS.DnsConfig.Windows.Compatibility", compatibility);
...
std::string ParseDomainASCII(std::wstring_view widestr)
{
    ...
    bool success = base::UTF16ToUTF8(punycode.data(), punycode.length(), &converted);
    DCHECK(success);
...
std::vector<std::string> ParseSearchList(std::wstring_view value)
{
    ...
    for (std::wstring_view t : base::SplitStringPiece(value, L",", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL)) {
...
base::expected<DnsConfig, ReadWinSystemDnsSettingsError> ConvertSettingsToDnsConfig(
    const base::expected<WinDnsSystemSettings, ReadWinSystemDnsSettingsError>& settings_or_error)
{
    ...
    // The order of adapters is the network binding order, so stick to the
    // first good adapter.
    for (const IP_ADAPTER_ADDRESSES* adapter = settings.addresses.get(); adapter != nullptr; adapter = adapter->Next) {
        ...
        std::optional<std::vector<IPEndPoint>> nameservers = GetNameServers(adapter);
        if (!nameservers) {
            return base::unexpected(ReadWinSystemDnsSettingsError::kGetNameServersFailed);
        }

        if (!nameservers->empty() && (adapter->OperStatus == IfOperStatusUp)) {
            // Check if the |adapter| has adapter specific nameservers.
            std::set<IPEndPoint> nameservers_set(nameservers->begin(), nameservers->end());
            if (!previous_nameservers_set.empty() && (previous_nameservers_set != nameservers_set)) {
        ...
        if (adapter->OperStatus != IfOperStatusUp || adapter->IfType == IF_TYPE_SOFTWARE_LOOPBACK || !dns_config.nameservers.empty())
            continue;
    ...
    if (!settings.append_to_multi_label_name.has_value()) {
        ...
        dns_config.append_to_multi_label_name = (settings.append_to_multi_label_name.value() != 0);
    }
    ...
    if (!CheckAndRecordCompatibility(settings.have_name_resolution_policy, settings.have_proxy, uses_vpn, has_adapter_specific_nameservers)) {
...
// allows IO.
class DnsConfigServiceWin::Watcher : public NetworkChangeNotifier::IPAddressObserver, public DnsConfigService::Watcher {
public:
    ...
    bool Watch() override
    {
        ...
        RegistryWatcher::CallbackType callback = base::BindRepeating(&Watcher::OnConfigChanged, base::Unretained(this));
        ...
        if (!hosts_watcher_.Watch(GetHostsPath(), base::FilePathWatcher::Type::kNonRecursive,
                base::BindRepeating(&Watcher::OnHostsFilePathWatcherChange, base::Unretained(this)))) {
...
public:
    explicit ConfigReader(DnsConfigServiceWin& service)
        : SerialWorker(/*max_number_of_retries=*/3)
        , service_(&service)
    {
    }
    ...
    bool OnWorkFinished(std::unique_ptr<SerialWorker::WorkItem> serial_worker_work_item) override
    {
        DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
        ...
        base::UmaHistogramEnumeration(base::StrCat({ "Net.DNS.DnsConfig.Windows.ReadSystemSettings", base::NumberToString(GetFailureCount()) }),
            work_item->dns_config_or_error_.has_value() ? ReadWinSystemDnsSettingsError::kOk : work_item->dns_config_or_error_.error());

        if (work_item->dns_config_or_error_.has_value()) {
            service_->OnConfigRead(std::move(work_item->dns_config_or_error_).value());
            return true;
        } else {
...
private:
    ...
    public:
        ...
        {
            dns_config_or_error_ = ConvertSettingsToDnsConfig(ReadWinSystemDnsSettings());
        }

    private:
        friend DnsConfigServiceWin::ConfigReader;
        base::expected<DnsConfig, ReadWinSystemDnsSettingsError> dns_config_or_error_;
    };
...
private:
    ...
    public:
        explicit WorkItem(base::FilePath hosts_file_path)
            : DnsConfigService::HostsReader::WorkItem(std::make_unique<DnsHostsFileParser>(std::move(hosts_file_path)))
        {
        }
...
DnsConfigServiceWin::DnsConfigServiceWin()
    : DnsConfigService(GetHostsPath().value(), std::nullopt /* config_change_delay */)
{
