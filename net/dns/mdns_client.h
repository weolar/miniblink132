...
public:
    ...
    typedef base::RepeatingCallback<void(Result, const RecordParsed*)> ResultCallback;
...
public:
    ...
    // made to a record.
    enum UpdateType { RECORD_ADDED, RECORD_CHANGED, RECORD_REMOVED };
    ...
    public:
        ...
        virtual void OnRecordUpdate(UpdateType update, const RecordParsed* record) = 0;
...
public:
    ...
    virtual void CreateSockets(std::vector<std::unique_ptr<DatagramServerSocket>>* sockets) = 0;
...
public:
    ...
    virtual std::unique_ptr<MDnsListener> CreateListener(uint16_t rrtype, const std::string& name, MDnsListener::Delegate* delegate) = 0;
...
typedef std::vector<std::pair<uint32_t, AddressFamily>> InterfaceIndexFamilyList;
...
NET_EXPORT std::unique_ptr<DatagramServerSocket> CreateAndBindMDnsSocket(AddressFamily address_family, uint32_t interface_index, NetLog* net_log);
