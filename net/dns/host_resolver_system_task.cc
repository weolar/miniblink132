...
{
    static base::NoDestructor<scoped_refptr<base::TaskRunner>> system_dns_resolution_task_runner(nullptr);
...
void PostSystemDnsResolutionTaskAndReply(
    base::OnceCallback<int(AddressList* addrlist, int* os_error)> system_dns_resolution_callback, SystemDnsResultsCallback results_cb)
{
    ...
    // with the results.
    auto call_with_results_cb
        = base::BindOnce([](SystemDnsResultsCallback results_cb, std::unique_ptr<net::AddressList> addr_list, std::unique_ptr<int> os_error,
                             int net_error) { std::move(results_cb).Run(std::move(*addr_list), *os_error, net_error); },
            std::move(results_cb), std::move(addr_list), std::move(os_error));

    scoped_refptr<base::TaskRunner> system_dns_resolution_task_runner = GetSystemDnsResolutionTaskRunnerOverride();
    if (!system_dns_resolution_task_runner) {
        ...
        system_dns_resolution_task_runner
            = base::ThreadPool::CreateTaskRunner({ base::MayBlock(), base::TaskPriority::USER_BLOCKING, base::TaskShutdownBehavior::CONTINUE_ON_SHUTDOWN });
    }
    system_dns_resolution_task_runner->PostTaskAndReplyWithResult(
        FROM_HERE, base::BindOnce(std::move(system_dns_resolution_callback), addr_list_ptr, os_error_ptr), std::move(call_with_results_cb));
}

int ResolveOnWorkerThread(scoped_refptr<HostResolverProc> resolver_proc, std::optional<std::string> hostname, AddressFamily address_family,
    HostResolverFlags flags, handles::NetworkHandle network, AddressList* addrlist, int* os_error)
{
    std::string hostname_str = hostname ? *std::move(hostname) : GetHostName();
    if (resolver_proc) {
        return resolver_proc->Resolve(hostname_str, address_family, flags, addrlist, os_error, network);
    } else {
        return SystemHostResolverCall(hostname_str, address_family, flags, addrlist, os_error, network);
    }
}

// Creates NetLog parameters when the resolve failed.
base::Value::Dict NetLogHostResolverSystemTaskFailedParams(uint32_t attempt_number, int net_error, int os_error)
{
...
using SystemDnsResolverOverrideCallback = base::RepeatingCallback<void(const std::optional<std::string>& host, AddressFamily address_family,
    HostResolverFlags host_resolver_flags, SystemDnsResultsCallback results_cb, handles::NetworkHandle network)>;
...
SystemDnsResolverOverrideCallback& GetSystemDnsResolverOverride()
{
    ...
    if (*dns_override) {
        ...
        base::ScopedValidateSequenceChecker scoped_validated_sequence_checker(*sequence_checker);
    }
#endif
...
void SetSystemDnsResolverOverride(SystemDnsResolverOverrideCallback dns_override)
{
    GetSystemDnsResolverOverride() = std::move(dns_override);
}

HostResolverSystemTask::Params::Params(scoped_refptr<HostResolverProc> resolver_proc, size_t in_max_retry_attempts)
...
HostResolverSystemTask::CacheParams::CacheParams(HostResolverCache& cache, NetworkAnonymizationKey network_anonymization_key)
...
// static
std::unique_ptr<HostResolverSystemTask> HostResolverSystemTask::Create(std::string hostname, AddressFamily address_family, HostResolverFlags flags,
    const Params& params, const NetLogWithSource& job_net_log, handles::NetworkHandle network, std::optional<CacheParams> cache_params)
{
    return std::make_unique<HostResolverSystemTask>(std::move(hostname), address_family, flags, params, job_net_log, network, std::move(cache_params));
}

// static
std::unique_ptr<HostResolverSystemTask> HostResolverSystemTask::CreateForOwnHostname(
    AddressFamily address_family, HostResolverFlags flags, const Params& params, const NetLogWithSource& job_net_log, handles::NetworkHandle network)
{
    return std::make_unique<HostResolverSystemTask>(std::nullopt, address_family, flags, params, job_net_log, network,
        /*cache_params=*/std::nullopt);
}

HostResolverSystemTask::HostResolverSystemTask(std::optional<std::string> hostname, AddressFamily address_family, HostResolverFlags flags, const Params& params,
    const NetLogWithSource& job_net_log, handles::NetworkHandle network, std::optional<CacheParams> cache_params)
    ...
    , cache_params_(std::move(cache_params))
{
    ...
    if (hostname_) {
        ...
        DCHECK(dns_names_util::IsValidDnsName(*hostname_)) << "Invalid hostname: " << *hostname_;
    }
...
void HostResolverSystemTask::StartLookupAttempt()
{
    ...
    if (attempt_number_ <= params_.max_retry_attempts) {
        base::SequencedTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE,
            base::BindOnce(&HostResolverSystemTask::StartLookupAttempt, weak_ptr_factory_.GetWeakPtr()),
            params_.unresponsive_delay * std::pow(params_.retry_factor, attempt_number_ - 1));
    }
    ...
    if (GetSystemDnsResolverOverride()) {
        GetSystemDnsResolverOverride().Run(hostname_, address_family_, flags_, std::move(lookup_complete_cb), network_);
        ...
        base::OnceCallback<int(AddressList * addrlist, int* os_error)> resolve_cb
            = base::BindOnce(&ResolveOnWorkerThread, params_.resolver_proc, hostname_, address_family_, flags_, network_);
        PostSystemDnsResolutionTaskAndReply(std::move(resolve_cb), std::move(lookup_complete_cb));
    }
}

// Callback for when DoLookup() completes.
void HostResolverSystemTask::OnLookupComplete(const uint32_t attempt_number, const AddressList& results, const int os_error, int error)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(!was_completed());

    TRACE_EVENT0(NetTracingCategory(), "HostResolverSystemTask::OnLookupComplete");
    ...
    if (error != OK) {
        net_log_.EndEvent(NetLogEventType::HOST_RESOLVER_SYSTEM_TASK, [&] { return NetLogHostResolverSystemTaskFailedParams(0, error, os_error); });
        net_log_.AddEvent(
            NetLogEventType::HOST_RESOLVER_MANAGER_ATTEMPT_FINISHED, [&] { return NetLogHostResolverSystemTaskFailedParams(attempt_number, error, os_error); });
    } else {
        net_log_.EndEvent(NetLogEventType::HOST_RESOLVER_SYSTEM_TASK, [&] { return results.NetLogParams(); });
        net_log_.AddEventWithIntParams(NetLogEventType::HOST_RESOLVER_MANAGER_ATTEMPT_FINISHED, "attempt_number", attempt_number);
    }
...
void HostResolverSystemTask::MaybeCacheResults(const AddressList& address_list)
{
    if (address_list.empty() || !cache_params_.has_value() || !base::FeatureList::IsEnabled(features::kUseHostResolverCache)) {
        return;
    }
    ...
    if (!address_list.dns_aliases().empty()) {
        ...
        CacheAlias(std::string(domain_name), DnsQueryType::A, address_list.dns_aliases().front());
        CacheAlias(std::string(domain_name), DnsQueryType::AAAA, address_list.dns_aliases().front());
...
void HostResolverSystemTask::CacheEndpoints(std::string domain_name, std::vector<IPEndPoint> endpoints, DnsQueryType query_type)
{
    if (endpoints.empty()) {
        cache_params_.value().cache->Set(std::make_unique<HostResolverInternalErrorResult>(std::move(domain_name), query_type, base::TimeTicks::Now() + kTtl,
                                             base::Time::Now() + kTtl, HostResolverInternalResult::Source::kUnknown, ERR_NAME_NOT_RESOLVED),
            cache_params_.value().network_anonymization_key, HostResolverSource::SYSTEM, /*secure=*/false);
    } else {
        cache_params_.value().cache->Set(
            std::make_unique<HostResolverInternalDataResult>(std::move(domain_name), query_type, base::TimeTicks::Now() + kTtl, base::Time::Now() + kTtl,
                HostResolverInternalResult::Source::kUnknown, std::move(endpoints), std::vector<std::string> {}, std::vector<HostPortPair> {}),
            cache_params_.value().network_anonymization_key, HostResolverSource::SYSTEM, /*secure=*/false);
    }
}

void HostResolverSystemTask::CacheAlias(std::string domain_name, DnsQueryType query_type, std::string target_name)
{
    cache_params_.value().cache->Set(std::make_unique<HostResolverInternalAliasResult>(std::move(domain_name), query_type, base::TimeTicks::Now() + kTtl,
                                         base::Time::Now() + kTtl, HostResolverInternalResult::Source::kUnknown, std::move(target_name)),
        cache_params_.value().network_anonymization_key, HostResolverSource::SYSTEM, /*secure=*/false);
}
...
int SystemHostResolverCall(const std::string& host, AddressFamily address_family, HostResolverFlags host_resolver_flags, AddressList* addrlist,
    int* os_error_opt, handles::NetworkHandle network)
{
    ...
    if ((hints.ai_family != AF_UNSPEC || hints.ai_flags & AI_ADDRCONFIG) && ai && ai->IsAllLocalhostOfOneFamily()) {
