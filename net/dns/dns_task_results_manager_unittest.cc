...
std::unique_ptr<HostResolverInternalDataResult> CreateDataResult(std::string_view domain_name, std::vector<IPEndPoint> ip_endpoints, DnsQueryType query_type)
{
    return std::make_unique<HostResolverInternalDataResult>(std::string(domain_name), query_type, /*expiration=*/base::TimeTicks(),
        /*timed_expiration=*/base::Time(), HostResolverInternalResult::Source::kDns, std::move(ip_endpoints), std::vector<std::string>(),
        std::vector<HostPortPair>());
}

std::unique_ptr<HostResolverInternalErrorResult> CreateNoData(std::string_view domain_name, DnsQueryType query_type)
{
    return std::make_unique<HostResolverInternalErrorResult>(std::string(domain_name), query_type, /*expiration=*/base::TimeTicks(),
        /*timed_expiration=*/base::Time(), HostResolverInternalResult::Source::kDns, ERR_NAME_NOT_RESOLVED);
}

std::unique_ptr<HostResolverInternalMetadataResult> CreateMetadata(
    std::string_view domain_name, std::multimap<HttpsRecordPriority, ConnectionEndpointMetadata> metadatas)
{
    return std::make_unique<HostResolverInternalMetadataResult>(std::string(domain_name), DnsQueryType::HTTPS,
        /*expiration=*/base::TimeTicks(), /*timed_expiration=*/base::Time(), HostResolverInternalResult::Source::kDns, std::move(metadatas));
}

std::unique_ptr<HostResolverInternalAliasResult> CreateAlias(std::string_view domain_name, DnsQueryType query_type, std::string_view alias_target)
{
    return std::make_unique<HostResolverInternalAliasResult>(std::string(domain_name), query_type, /*expiration=*/base::TimeTicks(),
        /*timed_expiration=*/base::Time(), HostResolverInternalResult::Source::kDns, std::string(alias_target));
}

std::vector<IPEndPoint> WithPort(const std::vector<IPEndPoint>& endpoints, uint16_t port)
{
...
static const ConnectionEndpointMetadata kMetadata1(
    ...
    /*ech_config_list=*/ {}, std::string(kHostName));

static const ConnectionEndpointMetadata kMetadata2(
    /*supported_protocol_alpns=*/ { "h2", "http/1.1" },
    /*ech_config_list=*/ {}, std::string(kHostName));

static const std::multimap<HttpsRecordPriority, ConnectionEndpointMetadata> kMetadatas { { 1, kMetadata1 }, { 2, kMetadata2 } };
...
public:
    explicit ManagerFactory(DnsTaskResultsManager::Delegate* delegate)
        : delegate_(delegate)
        , host_(HostResolver::Host(url::SchemeHostPort("https", kHostName, 443)))
    {
    }

    std::unique_ptr<DnsTaskResultsManager> Create()
    {
        return std::make_unique<DnsTaskResultsManager>(delegate_, host_, query_types_, NetLogWithSource());
    }
...
private:
    ...
    DnsQueryTypeSet query_types_ = { DnsQueryType::A, DnsQueryType::AAAA, DnsQueryType::HTTPS };
};
...
protected:
    ...
    {
        return ManagerFactory(delegate_.get());
    }
...
TEST_F(DnsTaskResultsManagerTest, IsMetadataReady)
{
    ...
    std::unique_ptr<DnsTaskResultsManager> manager = factory().query_types({ DnsQueryType::A, DnsQueryType::AAAA }).Create();
    ASSERT_TRUE(manager->IsMetadataReady());

    // HTTPS RR is queried.
    manager = factory().query_types({ DnsQueryType::A, DnsQueryType::AAAA, DnsQueryType::HTTPS }).Create();
...
{
    std::unique_ptr<DnsTaskResultsManager> manager = factory().query_types({ DnsQueryType::A, DnsQueryType::HTTPS }).Create();

    std::set<std::unique_ptr<HostResolverInternalResult>> results;
    results.insert(CreateDataResult(kHostName, { MakeIPEndPoint("192.0.2.1") }, DnsQueryType::A));
    manager->ProcessDnsTransactionResults(DnsQueryType::A, results);

    EXPECT_THAT(manager->GetCurrentEndpoints(), ElementsAre(ExpectServiceEndpoint(ElementsAre(MakeIPEndPoint("192.0.2.1", 443)))));
...
TEST_F(DnsTaskResultsManagerTest, IPv4First)
{
    ...
    results1.insert(CreateDataResult(kHostName, { MakeIPEndPoint("192.0.2.1") }, DnsQueryType::A));
    ...
    results2.insert(CreateDataResult(kHostName, { MakeIPEndPoint("2001:db8::1") }, DnsQueryType::AAAA));
    manager->ProcessDnsTransactionResults(DnsQueryType::AAAA, results2);

    EXPECT_THAT(manager->GetCurrentEndpoints(),
        ElementsAre(ExpectServiceEndpoint(ElementsAre(MakeIPEndPoint("192.0.2.1", 443)), ElementsAre(MakeIPEndPoint("2001:db8::1", 443)))));
}
...
TEST_F(DnsTaskResultsManagerTest, IPv6First)
{
    ...
    std::set<std::unique_ptr<HostResolverInternalResult>> results1;
    results1.insert(CreateDataResult(kHostName, { MakeIPEndPoint("2001:db8::1") }, DnsQueryType::AAAA));
    manager->ProcessDnsTransactionResults(DnsQueryType::AAAA, results1);

    EXPECT_THAT(manager->GetCurrentEndpoints(), ElementsAre(ExpectServiceEndpoint(IsEmpty(), ElementsAre(MakeIPEndPoint("2001:db8::1", 443)))));

    // A is responded. Service endpoints should be updated.
    std::set<std::unique_ptr<HostResolverInternalResult>> results;
    results.insert(CreateDataResult(kHostName, { MakeIPEndPoint("192.0.2.1"), MakeIPEndPoint("192.0.2.2") }, DnsQueryType::A));
    manager->ProcessDnsTransactionResults(DnsQueryType::A, results);

    EXPECT_THAT(manager->GetCurrentEndpoints(),
        ElementsAre(ExpectServiceEndpoint(
            ElementsAre(MakeIPEndPoint("192.0.2.1", 443), MakeIPEndPoint("192.0.2.2", 443)), ElementsAre(MakeIPEndPoint("2001:db8::1", 443)))));
}
...
TEST_F(DnsTaskResultsManagerTest, IPv6NoDataBeforeIPv4)
{
    ...
    // delay.
    std::set<std::unique_ptr<HostResolverInternalResult>> results2;
    results2.insert(CreateDataResult(kHostName, { MakeIPEndPoint("192.0.2.1") }, DnsQueryType::A));
...
TEST_F(DnsTaskResultsManagerTest, MetadataFirst)
{
    ...
    results3.insert(CreateDataResult(kHostName, { MakeIPEndPoint("2001:db8::1") }, DnsQueryType::AAAA));
    manager->ProcessDnsTransactionResults(DnsQueryType::AAAA, results3);

    EXPECT_THAT(manager->GetCurrentEndpoints(),
        ElementsAre(ExpectServiceEndpoint(ElementsAre(MakeIPEndPoint("192.0.2.1", 443)), ElementsAre(MakeIPEndPoint("2001:db8::1", 443)), kMetadata1),
            ExpectServiceEndpoint(ElementsAre(MakeIPEndPoint("192.0.2.1", 443)), ElementsAre(MakeIPEndPoint("2001:db8::1", 443)), kMetadata2)));
}
...
TEST_F(DnsTaskResultsManagerTest, MetadataDifferentTargetName)
{
    ...
    std::set<std::unique_ptr<HostResolverInternalResult>> results1;
    results1.insert(CreateMetadata(kHostName, { { 1, kMetadataDifferentTargetName } }));
    manager->ProcessDnsTransactionResults(DnsQueryType::HTTPS, results1);
...
TEST_F(DnsTaskResultsManagerTest, MetadataAfterIPv6)
{
    ...
    ASSERT_TRUE(manager->IsMetadataReady());
    EXPECT_THAT(manager->GetCurrentEndpoints(),
        ElementsAre(ExpectServiceEndpoint(IsEmpty(), ElementsAre(MakeIPEndPoint("2001:db8::1", 443)), kMetadata1),
            ExpectServiceEndpoint(IsEmpty(), ElementsAre(MakeIPEndPoint("2001:db8::1", 443)), kMetadata2)));
}
...
TEST_F(DnsTaskResultsManagerTest, IPv6TimedoutAfterMetadata)
{
    ...
    ASSERT_TRUE(manager->IsMetadataReady());
    EXPECT_THAT(manager->GetCurrentEndpoints(),
        ElementsAre(ExpectServiceEndpoint(ElementsAre(MakeIPEndPoint("192.0.2.1", 443)), IsEmpty(), kMetadata1),
            ExpectServiceEndpoint(ElementsAre(MakeIPEndPoint("192.0.2.1", 443)), IsEmpty(), kMetadata2)));
}
...
TEST_F(DnsTaskResultsManagerTest, EndpointOrdering)
{
    ...
    const std::vector<IPEndPoint> kSvcbHost1IPv6s = { MakeIPEndPoint("2001:db8::1") };
    const std::vector<IPEndPoint> kSvcbHost2IPv6s = { MakeIPEndPoint("2001:db8::2") };
    ...
    const std::multimap<HttpsRecordPriority, ConnectionEndpointMetadata> kSvcbHost1Metadatas { { 1, kSvcbHost1Metadata1 }, { 2, kSvcbHost1Metadata2 } };
    ...
    for (const auto& testdata : kTestDatas) {
        if (!testdata.ipv4_endpoints.empty()) {
            std::set<std::unique_ptr<HostResolverInternalResult>> results;
            results.insert(CreateDataResult(testdata.host, testdata.ipv4_endpoints, DnsQueryType::A));
            manager->ProcessDnsTransactionResults(DnsQueryType::A, results);
        }
        if (!testdata.ipv6_endpoints.empty()) {
            std::set<std::unique_ptr<HostResolverInternalResult>> results;
            results.insert(CreateDataResult(testdata.host, testdata.ipv6_endpoints, DnsQueryType::AAAA));
    ...
    const std::vector<ServiceEndpoint> kExpects = {
        ServiceEndpoint(WithPort(kSvcbHost1IPv4s, 443), WithPort(kSvcbHost1IPv6s, 443), kSvcbHost1Metadata1),
        ServiceEndpoint(WithPort(kSvcbHost1IPv4s, 443), WithPort(kSvcbHost1IPv6s, 443), kSvcbHost1Metadata2),
        ServiceEndpoint(WithPort(kSvcbHost2IPv4s, 443), WithPort(kSvcbHost2IPv6s, 443), ConnectionEndpointMetadata()),
        ServiceEndpoint(WithPort(kSvcbHost3IPv4s, 443), {}, ConnectionEndpointMetadata()),
    };
...
TEST_F(DnsTaskResultsManagerTest, Aliases)
{
    ...
    results1.insert(CreateAlias(kAliasTarget1, DnsQueryType::AAAA, kAliasTarget2));
    results1.insert(CreateDataResult(kHostName, { MakeIPEndPoint("2001:db8::1") }, DnsQueryType::AAAA));
    manager->ProcessDnsTransactionResults(DnsQueryType::AAAA, results1);

    EXPECT_THAT(manager->GetCurrentEndpoints(), ElementsAre(ExpectServiceEndpoint(IsEmpty(), ElementsAre(MakeIPEndPoint("2001:db8::1", 443)))));
    EXPECT_THAT(manager->GetAliases(), UnorderedElementsAre(kHostName, kAliasTarget1, kAliasTarget2));
}
...
TEST_F(DnsTaskResultsManagerTest, Ipv4MappedIpv6)
{
    ...
    manager->ProcessDnsTransactionResults(DnsQueryType::AAAA, results);
    EXPECT_THAT(manager->GetCurrentEndpoints(), ElementsAre(ExpectServiceEndpoint(ElementsAre(MakeIPEndPoint("192.0.2.1", 443)), IsEmpty())));
}
