...
const DnsOverHttpsServerConfig kServerConfig1 = *DnsOverHttpsServerConfig::FromString("https://example1.test");
const DnsOverHttpsServerConfig kServerConfig2 = *DnsOverHttpsServerConfig::FromString("https://example2.test");
...
TEST(DnsOverHttpsConfigTest, MultiValue)
{
    ...
    EXPECT_EQ(kServerConfig1.server_template() + "\n" + kServerConfig2.server_template(), config.ToString());
...
{
    auto config = DnsOverHttpsConfig::FromString(kServerConfig1.server_template());
...
TEST(DnsOverHttpsConfigTest, FromStringMultiValue)
{
    ...
    EXPECT_THAT(config, testing::Optional(DnsOverHttpsConfig({ kServerConfig1, kServerConfig2 })));
}

TEST(DnsOverHttpsConfigTest, FromStringExtraWhitespace)
{
    auto config = DnsOverHttpsConfig::FromString("  \t" + kServerConfig1.server_template() + "    " + kServerConfig2.server_template() + "\n ");
    EXPECT_THAT(config, testing::Optional(DnsOverHttpsConfig({ kServerConfig1, kServerConfig2 })));

    auto config2 = DnsOverHttpsConfig::FromString(kServerConfig1.server_template() + "\t" + kServerConfig2.server_template());
...
{
    std::string some_invalid = "foo " + kServerConfig1.server_template() + " bar " + kServerConfig2.server_template() + " baz";
    EXPECT_FALSE(DnsOverHttpsConfig::FromString(some_invalid));
    EXPECT_EQ(DnsOverHttpsConfig({ kServerConfig1, kServerConfig2 }), DnsOverHttpsConfig::FromStringLax(some_invalid));
}
...
TEST(DnsOverHttpsConfigTest, JsonLax)
{
    ...
    DnsOverHttpsConfig reparsed = DnsOverHttpsConfig::FromStringLax(parsed.ToString());
