...
struct NET_EXPORT_PRIVATE HostResolverEndpointResult {
    ...
    HostResolverEndpointResult& operator=(const HostResolverEndpointResult&) = default;
    ...
    {
        return std::tie(ip_endpoints, metadata) == std::tie(other.ip_endpoints, other.metadata);
    }
    bool operator!=(const HostResolverEndpointResult& other) const
    {
        return !(*this == other);
...
using HostResolverEndpointResults = std::vector<net::HostResolverEndpointResult>;
...
struct NET_EXPORT_PRIVATE ServiceEndpoint {
    ...
    ServiceEndpoint(std::vector<IPEndPoint> ipv4_endpoints, std::vector<IPEndPoint> ipv6_endpoints, ConnectionEndpointMetadata metadata);
    ...
    bool operator==(const ServiceEndpoint& other) const
    {
        return std::forward_as_tuple(ipv4_endpoints, ipv6_endpoints, metadata)
            == std::forward_as_tuple(other.ipv4_endpoints, other.ipv6_endpoints, other.metadata);
    }
    bool operator!=(const ServiceEndpoint& other) const
    {
        return !(*this == other);
