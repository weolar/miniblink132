...
public:
    ...
    static std::optional<DnsOverHttpsConfig> FromTemplatesForTesting(std::vector<std::string> servers);

    // Constructs a Config from its text form if valid.  Returns `nullopt` if the
    // input is empty or invalid (even partly invalid).
    static std::optional<DnsOverHttpsConfig> FromString(std::string_view doh_config);
...
private:
    // Constructs a Config from URI templates of zero or more servers.
    // Returns `nullopt` if any string is invalid.
    static std::optional<DnsOverHttpsConfig> FromTemplates(std::vector<std::string> servers);
