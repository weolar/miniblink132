...
IPEndPoint GetMdnsIPEndPoint(const char* address)
{
    ...
    DCHECK(success);
    return IPEndPoint(multicast_group_number, dns_protocol::kDefaultPortMulticast);
}
...
// crbug.com/899310.
    ...
    switch (address_family) {
    case ADDRESS_FAMILY_IPV4:
        return IPEndPoint(IPAddress::IPv4AllZeros(), dns_protocol::kDefaultPortMulticast);
    case ADDRESS_FAMILY_IPV6:
        return IPEndPoint(IPAddress::IPv6AllZeros(), dns_protocol::kDefaultPortMulticast);
    default:
        NOTREACHED();
    }
...
std::string GetNameForHttpsQuery(const url::SchemeHostPort& scheme_host_port, uint16_t* out_port)
{
    DCHECK(!scheme_host_port.host().empty() && scheme_host_port.host().front() != '.');
    ...
    return base::StrCat({ "_", base::NumberToString(scheme_host_port.port()), "._https.", scheme_host_port.host() });
}
