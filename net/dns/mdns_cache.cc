...
MDnsCache::Key::Key(unsigned type, const std::string& name, const std::string& optional)
    : type_(type)
...
MDnsCache::Key& MDnsCache::Key::operator=(const MDnsCache::Key& other) = default;

MDnsCache::Key::~Key() = default;

bool MDnsCache::Key::operator<(const MDnsCache::Key& other) const
{
    return std::tie(name_lowercase_, type_, optional_) < std::tie(other.name_lowercase_, other.type_, other.optional_);
}

bool MDnsCache::Key::operator==(const MDnsCache::Key& key) const
{
    return type_ == key.type_ && name_lowercase_ == key.name_lowercase_ && optional_ == key.optional_;
}
...
MDnsCache::MDnsCache()
    : entry_limit_(kDefaultEntryLimit)
{
}
...
MDnsCache::UpdateType MDnsCache::UpdateDnsRecord(std::unique_ptr<const RecordParsed> record)
{
    ...
    std::pair<RecordMap::iterator, bool> insert_result = mdns_cache_.emplace(cache_key, nullptr);
...
void MDnsCache::CleanupRecords(base::Time now, const RecordRemovedCallback& record_removed_callback)
{
...
void MDnsCache::FindDnsRecords(unsigned type, const std::string& name, std::vector<const RecordParsed*>* results, base::Time now) const
{
    DCHECK(results);
    results->clear();
    ...
    for (; i != mdns_cache_.end(); ++i) {
        if (i->first.name_lowercase() != name_lowercase || (type != 0 && i->first.type() != type)) {
            break;
        }
...
std::unique_ptr<const RecordParsed> MDnsCache::RemoveRecord(const RecordParsed* record)
{
