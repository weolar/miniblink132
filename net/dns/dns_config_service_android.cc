...
constexpr base::FilePath::CharType kFilePathHosts[] = FILE_PATH_LITERAL("/system/etc/hosts");
...
public:
    explicit ConfigReader(DnsConfigServiceAndroid& service, android::DnsServerGetter dns_server_getter)
        ...
        void DoWork() override
        {
            ...
            if (base::android::BuildInfo::GetInstance()->sdk_int() >= base::android::SDK_VERSION_MARSHMALLOW) {
                if (!dns_server_getter_.Run(
                        &dns_config_->nameservers, &dns_config_->dns_over_tls_active, &dns_config_->dns_over_tls_hostname, &dns_config_->search)) {
                    dns_config_.reset();
                }
                return;
            }
...
void DnsConfigServiceAndroid::ReadConfigNow()
{
    ...
    if (!config_reader_) {
        ...
        config_reader_ = std::make_unique<ConfigReader>(*this, std::move(dns_server_getter_));
    }
...
void DnsConfigServiceAndroid::OnNetworkChanged(NetworkChangeNotifier::ConnectionType type)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
