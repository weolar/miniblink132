...
class NET_EXPORT HostResolverProc : public base::RefCountedThreadSafe<HostResolverProc> {
public:
    explicit HostResolverProc(scoped_refptr<HostResolverProc> previous, bool allow_fallback_to_system_or_default = true);
    ...
    virtual int Resolve(const std::string& host, AddressFamily address_family, HostResolverFlags host_resolver_flags, AddressList* addrlist, int* os_error) = 0;
    ...
    virtual int Resolve(const std::string& host, AddressFamily address_family, HostResolverFlags host_resolver_flags, AddressList* addrlist, int* os_error,
