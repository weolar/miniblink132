...
const uint8_t kSamplePacket1[] = {
    ...
    // Answer 1
    0x07, '_', 'p', 'r', 'i', 'v', 'e', 't', 0x04, '_', 't', 'c', 'p', 0x05, 'l', 'o', 'c', 'a', 'l', 0x00, 0x00, 0x0c, // TYPE is PTR.
...
const uint8_t kQueryPacketPrivetA[] = {
    ...
    // Question
    // This part is echoed back from the respective query.
    0x07, '_', 'p', 'r', 'i', 'v', 'e', 't', 0x04, '_', 't', 'c', 'p', 0x05, 'l', 'o', 'c', 'a', 'l', 0x00, 0x00, 0x01, // TYPE is A.
    0x00, 0x01, // CLASS is IN.
};
...
const uint8_t kSamplePacketNsec[] = {
    ...
    // Answer 1
    0x07, '_', 'p', 'r', 'i', 'v', 'e', 't', 0x04, '_', 't', 'c', 'p', 0x05, 'l', 'o', 'c', 'a', 'l', 0x00, 0x00, 0x2f, // TYPE is NSEC.
    0x00, 0x01, // CLASS is IN.
    0x00, 0x01, // TTL (4 bytes) is 20 hours, 47 minutes, 48 seconds.
...
public:
    ...
    MOCK_METHOD2(StartObserver, void(const base::Location& posted_from, base::TimeDelta delay));
};
...
public:
    MOCK_METHOD2(OnRecordUpdate, void(MDnsListener::UpdateType update, const RecordParsed* records));
...
{
    base::CancelableOnceCallback<void()> callback(base::BindOnce(&MDnsTest::Stop, base::Unretained(this)));
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, callback.callback(), time_period);
...
TEST_F(MDnsTest, PassiveListeners)
{
    ...
    std::unique_ptr<MDnsListener> listener_privet = test_client_->CreateListener(dns_protocol::kTypePTR, "_privet._tcp.local", &delegate_privet);
    std::unique_ptr<MDnsListener> listener_printer = test_client_->CreateListener(dns_protocol::kTypePTR, "_printer._tcp.local", &delegate_printer);
    ...
    EXPECT_CALL(delegate_privet, OnRecordUpdate(MDnsListener::RECORD_ADDED, _))
        .Times(Exactly(1))
        .WillOnce(Invoke(&record_privet, &PtrRecordCopyContainer::SaveWithDummyArg));

    EXPECT_CALL(delegate_printer, OnRecordUpdate(MDnsListener::RECORD_ADDED, _))
        .Times(Exactly(1))
        .WillOnce(Invoke(&record_printer, &PtrRecordCopyContainer::SaveWithDummyArg));

    SimulatePacketReceive(kSamplePacket1, sizeof(kSamplePacket1));
    SimulatePacketReceive(kSamplePacket1, sizeof(kSamplePacket1));

    EXPECT_TRUE(record_privet.IsRecordWith("_privet._tcp.local", "hello._privet._tcp.local"));

    EXPECT_TRUE(record_printer.IsRecordWith("_printer._tcp.local", "hello._printer._tcp.local"));
...
TEST_F(MDnsTest, PassiveListenersWithCapitalization)
{
    ...
    SimulatePacketReceive(kSamplePacket1WithCapitalization, sizeof(kSamplePacket1WithCapitalization));
    SimulatePacketReceive(kSamplePacket1WithCapitalization, sizeof(kSamplePacket1WithCapitalization));
...
TEST_F(MDnsTest, PassiveListenersCacheCleanup)
{
    ...
    EXPECT_CALL(delegate_privet, OnRecordUpdate(MDnsListener::RECORD_REMOVED, _))
        .Times(Exactly(1))
        .WillOnce(DoAll(InvokeWithoutArgs(this, &MDnsTest::Stop), Invoke(&record_privet2, &PtrRecordCopyContainer::SaveWithDummyArg)));

    RunFor(base::Seconds(record_privet.ttl() + 1));

    EXPECT_TRUE(record_privet2.IsRecordWith("_privet._tcp.local", "hello._privet._tcp.local"));
}
...
TEST_F(MDnsTest, CacheCleanupWithShortTTL)
{
    ...
    EXPECT_CALL(*clock, Now()).Times(3).WillRepeatedly(Return(start_time)).RetiresOnSaturation();
    ...
    EXPECT_CALL(delegate_privet, OnRecordUpdate(MDnsListener::RECORD_ADDED, _)).Times(Exactly(1));
    EXPECT_CALL(delegate_printer, OnRecordUpdate(MDnsListener::RECORD_ADDED, _)).Times(Exactly(1));

    SimulatePacketReceive(kSamplePacket3, sizeof(kSamplePacket3));

    EXPECT_CALL(delegate_privet, OnRecordUpdate(MDnsListener::RECORD_REMOVED, _)).Times(Exactly(1));
    ...
    EXPECT_CALL(*clock, Now()).WillOnce(Return(start_time + base::Seconds(4))).RetiresOnSaturation();
    EXPECT_CALL(*clock, Now()).WillOnce(Return(start_time + base::Seconds(2))).RetiresOnSaturation();
...
TEST_F(MDnsTest, StopListening_CacheCleanupScheduled)
{
    ...
    test_client_ = std::make_unique<MDnsClientImpl>(clock, std::move(cleanup_timer));
    ASSERT_THAT(test_client_->StartListening(&socket_factory_), test::IsOk());
    ASSERT_TRUE(test_client_->IsListening());
...
TEST_F(MDnsTest, MalformedPacket)
{
    ...
    SimulatePacketReceive(kCorruptedPacketUnsalvagable, sizeof(kCorruptedPacketUnsalvagable));

    // Regression test: send a packet where the question cannot be read.
    SimulatePacketReceive(kCorruptedPacketBadQuestion, sizeof(kCorruptedPacketBadQuestion));

    // Then send salvagable packet to ensure we can extract useful records.
    SimulatePacketReceive(kCorruptedPacketSalvagable, sizeof(kCorruptedPacketSalvagable));
...
TEST_F(MDnsTest, TransactionWithEmptyCache)
{
    ...
    std::unique_ptr<MDnsTransaction> transaction_privet = test_client_->CreateTransaction(dns_protocol::kTypePTR, "_privet._tcp.local",
        MDnsTransaction::QUERY_NETWORK | MDnsTransaction::QUERY_CACHE | MDnsTransaction::SINGLE_RESULT,
        base::BindRepeating(&MDnsTest::MockableRecordCallback, base::Unretained(this)));
...
{
    ExpectPacket(kQueryPacketPrivetWithCapitalization, sizeof(kQueryPacketPrivetWithCapitalization));
...
{
    std::unique_ptr<MDnsTransaction> transaction_privet = test_client_->CreateTransaction(dns_protocol::kTypePTR, "_privet._tcp.local",
        MDnsTransaction::QUERY_CACHE | MDnsTransaction::SINGLE_RESULT, base::BindRepeating(&MDnsTest::MockableRecordCallback, base::Unretained(this)));

    EXPECT_CALL(*this, MockableRecordCallback(MDnsTransaction::RESULT_NO_RESULTS, _)).Times(Exactly(1));
...
TEST_F(MDnsTest, TransactionWithCache)
{
    ...
    std::unique_ptr<MDnsListener> listener_irrelevant = test_client_->CreateListener(dns_protocol::kTypeA, "codereview.chromium.local", &delegate_irrelevant);
    ...
    EXPECT_CALL(*this, MockableRecordCallback(MDnsTransaction::RESULT_RECORD, _)).WillOnce(Invoke(&record_privet, &PtrRecordCopyContainer::SaveWithDummyArg));
...
TEST_F(MDnsTest, AdditionalRecords)
{
    ...
    SimulatePacketReceive(kSamplePacketAdditionalOnly, sizeof(kSamplePacketAdditionalOnly));
...
TEST_F(MDnsTest, TransactionTimeout)
{
    ...
    EXPECT_CALL(*this, MockableRecordCallback(MDnsTransaction::RESULT_NO_RESULTS, nullptr))
        .Times(Exactly(1))
...
TEST_F(MDnsTest, TransactionMultipleRecords)
{
    ...
    std::unique_ptr<MDnsTransaction> transaction_privet = test_client_->CreateTransaction(dns_protocol::kTypePTR, "_privet._tcp.local",
        MDnsTransaction::QUERY_NETWORK | MDnsTransaction::QUERY_CACHE, base::BindRepeating(&MDnsTest::MockableRecordCallback, base::Unretained(this)));
    ...
    EXPECT_CALL(*this, MockableRecordCallback(MDnsTransaction::RESULT_RECORD, _))
        .Times(Exactly(2))
        .WillOnce(Invoke(&record_privet, &PtrRecordCopyContainer::SaveWithDummyArg))
        .WillOnce(Invoke(&record_privet2, &PtrRecordCopyContainer::SaveWithDummyArg));
    ...
    EXPECT_TRUE(record_privet2.IsRecordWith("_privet._tcp.local", "zzzzz._privet._tcp.local"));

    EXPECT_CALL(*this, MockableRecordCallback(MDnsTransaction::RESULT_DONE, nullptr)).WillOnce(InvokeWithoutArgs(this, &MDnsTest::Stop));
...
TEST_F(MDnsTest, TransactionReentrantDelete)
{
    ...
    transaction_ = test_client_->CreateTransaction(dns_protocol::kTypePTR, "_privet._tcp.local",
        MDnsTransaction::QUERY_NETWORK | MDnsTransaction::QUERY_CACHE | MDnsTransaction::SINGLE_RESULT,
        base::BindRepeating(&MDnsTest::MockableRecordCallback, base::Unretained(this)));
    ...
    EXPECT_CALL(*this, MockableRecordCallback(MDnsTransaction::RESULT_NO_RESULTS, nullptr))
        .Times(Exactly(1))
        .WillOnce(DoAll(InvokeWithoutArgs(this, &MDnsTest::DeleteTransaction), InvokeWithoutArgs(this, &MDnsTest::Stop)));
...
TEST_F(MDnsTest, TransactionReentrantDeleteFromCache)
{
    ...
    transaction_ = test_client_->CreateTransaction(dns_protocol::kTypePTR, "_privet._tcp.local", MDnsTransaction::QUERY_NETWORK | MDnsTransaction::QUERY_CACHE,
        base::BindRepeating(&MDnsTest::MockableRecordCallback, base::Unretained(this)));
...
TEST_F(MDnsTest, TransactionReentrantCacheLookupStart)
{
    ...
    std::unique_ptr<MDnsTransaction> transaction1 = test_client_->CreateTransaction(dns_protocol::kTypePTR, "_privet._tcp.local",
        MDnsTransaction::QUERY_NETWORK | MDnsTransaction::QUERY_CACHE | MDnsTransaction::SINGLE_RESULT,
        base::BindRepeating(&MDnsTest::MockableRecordCallback, base::Unretained(this)));

    std::unique_ptr<MDnsTransaction> transaction2 = test_client_->CreateTransaction(dns_protocol::kTypePTR, "_printer._tcp.local",
        MDnsTransaction::QUERY_CACHE | MDnsTransaction::SINGLE_RESULT, base::BindRepeating(&MDnsTest::MockableRecordCallback2, base::Unretained(this)));

    EXPECT_CALL(*this, MockableRecordCallback2(MDnsTransaction::RESULT_RECORD, _)).Times(Exactly(1));

    EXPECT_CALL(*this, MockableRecordCallback(MDnsTransaction::RESULT_RECORD, _))
        .Times(Exactly(1))
        .WillOnce(IgnoreResult(InvokeWithoutArgs(transaction2.get(), &MDnsTransaction::Start)));
...
TEST_F(MDnsTest, ListenerReentrantDelete)
{
    ...
    listener1_ = test_client_->CreateListener(dns_protocol::kTypePTR, "_privet._tcp.local", &delegate_privet);

    listener2_ = test_client_->CreateListener(dns_protocol::kTypePTR, "_privet._tcp.local", &delegate_privet);
...
TEST_F(MDnsTest, DoubleRecordDisagreeing)
{
    ...
    std::unique_ptr<MDnsListener> listener_privet = test_client_->CreateListener(dns_protocol::kTypeA, "privet.local", &delegate_privet);

    ASSERT_TRUE(listener_privet->Start());

    EXPECT_CALL(delegate_privet, OnRecordUpdate(MDnsListener::RECORD_ADDED, _)).Times(Exactly(1)).WillOnce(SaveIPAddress(&address));

    SimulatePacketReceive(kCorruptedPacketDoubleRecord, sizeof(kCorruptedPacketDoubleRecord));
...
{
    StrictMock<MockListenerDelegate> delegate_privet;
    std::unique_ptr<MDnsListener> listener_privet = test_client_->CreateListener(dns_protocol::kTypeA, "_privet._tcp.local", &delegate_privet);
    ...
    std::unique_ptr<MDnsListener> listener_privet2 = test_client_->CreateListener(dns_protocol::kTypePTR, "_privet._tcp.local", &delegate_privet2);

    ASSERT_TRUE(listener_privet->Start());

    EXPECT_CALL(delegate_privet, OnNsecRecord("_privet._tcp.local", dns_protocol::kTypeA));
...
{
    std::unique_ptr<MDnsTransaction> transaction_privet = test_client_->CreateTransaction(dns_protocol::kTypeA, "_privet._tcp.local",
        MDnsTransaction::QUERY_NETWORK | MDnsTransaction::QUERY_CACHE | MDnsTransaction::SINGLE_RESULT,
        base::BindRepeating(&MDnsTest::MockableRecordCallback, base::Unretained(this)));
    ...
    EXPECT_CALL(*this, MockableRecordCallback(MDnsTransaction::RESULT_NSEC, nullptr));
...
TEST_F(MDnsTest, NsecWithTransactionFromCache)
{
    ...
    StrictMock<MockListenerDelegate> delegate_irrelevant;
    std::unique_ptr<MDnsListener> listener_irrelevant = test_client_->CreateListener(dns_protocol::kTypePTR, "_privet._tcp.local", &delegate_irrelevant);
    ...
    std::unique_ptr<MDnsTransaction> transaction_privet_a = test_client_->CreateTransaction(dns_protocol::kTypeA, "_privet._tcp.local",
        MDnsTransaction::QUERY_NETWORK | MDnsTransaction::QUERY_CACHE | MDnsTransaction::SINGLE_RESULT,
        base::BindRepeating(&MDnsTest::MockableRecordCallback, base::Unretained(this)));
    ...
    std::unique_ptr<MDnsTransaction> transaction_privet_ptr = test_client_->CreateTransaction(dns_protocol::kTypePTR, "_privet._tcp.local",
        MDnsTransaction::QUERY_NETWORK | MDnsTransaction::QUERY_CACHE | MDnsTransaction::SINGLE_RESULT,
        base::BindRepeating(&MDnsTest::MockableRecordCallback, base::Unretained(this)));
...
TEST_F(MDnsTest, NsecConflictRemoval)
{
    ...
    EXPECT_CALL(delegate_privet, OnRecordUpdate(MDnsListener::RECORD_ADDED, _)).WillOnce(SaveArg<1>(&record1));

    SimulatePacketReceive(kSamplePacketAPrivet, sizeof(kSamplePacketAPrivet));

    EXPECT_CALL(delegate_privet, OnRecordUpdate(MDnsListener::RECORD_REMOVED, _)).WillOnce(SaveArg<1>(&record2));
...
TEST_F(MDnsTest, MAYBE_RefreshQuery)
{
    ...
    EXPECT_CALL(socket_factory_, OnSendTo(MakeString(kQueryPacketPrivetA, sizeof(kQueryPacketPrivetA)))).Times(4);
...
class FailingSocketFactory : public MDnsSocketFactory {
    void CreateSockets(std::vector<std::unique_ptr<DatagramServerSocket>>* sockets) override
    {
        auto socket = std::make_unique<MockMDnsDatagramServerSocket>(ADDRESS_FAMILY_IPV4);
        EXPECT_CALL(*socket, RecvFrom(_, _, _, _)).WillRepeatedly(Return(ERR_FAILED));
...
TEST_F(MDnsTest, StartListeningFailure)
{
    ...
    EXPECT_THAT(test_client_->StartListening(&socket_factory), test::IsError(ERR_FAILED));
}
...
TEST_F(MDnsTest, ClearOverfilledCache)
{
    ...
    EXPECT_CALL(delegate_privet, OnRecordUpdate(MDnsListener::RECORD_ADDED, _)).Times(AtMost(1)).WillOnce(Assign(&privet_added, true));
    EXPECT_CALL(delegate_privet, OnRecordUpdate(MDnsListener::RECORD_REMOVED, _)).WillRepeatedly(Assign(&privet_added, false));

    bool printer_added = false;
    EXPECT_CALL(delegate_printer, OnRecordUpdate(MDnsListener::RECORD_ADDED, _)).Times(AtMost(1)).WillOnce(Assign(&printer_added, true));
    EXPECT_CALL(delegate_printer, OnRecordUpdate(MDnsListener::RECORD_REMOVED, _)).WillRepeatedly(Assign(&printer_added, false));
...
protected:
    ...
    void SetUp() override
    {
        auto socket_ipv4 = std::make_unique<MockMDnsDatagramServerSocket>(ADDRESS_FAMILY_IPV4);
        auto socket_ipv6 = std::make_unique<MockMDnsDatagramServerSocket>(ADDRESS_FAMILY_IPV6);
...
TEST_F(MDnsConnectionTest, ReceiveSynchronous)
{
    ...
    EXPECT_CALL(*socket_ipv4_ptr_, RecvFrom(_, _, _, _)).WillOnce(Return(ERR_IO_PENDING));
    EXPECT_CALL(*socket_ipv6_ptr_, RecvFrom(_, _, _, _))
        .WillOnce(Invoke(socket_ipv6_ptr_.get(), &MockMDnsDatagramServerSocket::HandleRecvNow))
...
TEST_F(MDnsConnectionTest, ReceiveAsynchronous)
{
    ...
    EXPECT_CALL(*socket_ipv6_ptr_, RecvFrom(_, _, _, _))
        .Times(2)
        .WillOnce(Invoke(socket_ipv6_ptr_.get(), &MockMDnsDatagramServerSocket::HandleRecvLater))
        .WillOnce(Return(ERR_IO_PENDING));
...
TEST_F(MDnsConnectionTest, Error)
{
    ...
    EXPECT_CALL(*socket_ipv4_ptr_, RecvFrom(_, _, _, _)).WillOnce(Return(ERR_IO_PENDING));
    EXPECT_CALL(*socket_ipv6_ptr_, RecvFrom(_, _, _, _)).WillOnce([&](auto, auto, auto, auto cb) {
...
protected:
    void SetUp() override
    {
        ...
        EXPECT_CALL(*socket_ipv4_ptr_, RecvFrom(_, _, _, _)).WillOnce(Return(ERR_IO_PENDING));
        EXPECT_CALL(*socket_ipv6_ptr_, RecvFrom(_, _, _, _)).WillOnce(Return(ERR_IO_PENDING));
        EXPECT_THAT(InitConnection(), test::IsOk());
    }
};
...
{
    EXPECT_CALL(*socket_ipv4_ptr_, SendToInternal(sample_packet_, "224.0.0.251:5353", _));
    EXPECT_CALL(*socket_ipv6_ptr_, SendToInternal(sample_packet_, "[ff02::fb]:5353", _));
...
{
    EXPECT_CALL(*socket_ipv4_ptr_, SendToInternal(sample_packet_, "224.0.0.251:5353", _));
    EXPECT_CALL(*socket_ipv6_ptr_, SendToInternal(sample_packet_, "[ff02::fb]:5353", _)).WillOnce(Return(ERR_SOCKET_NOT_CONNECTED));
...
TEST_F(MDnsConnectionSendTest, SendQueued)
{
    ...
    EXPECT_CALL(*socket_ipv4_ptr_, SendToInternal(sample_packet_, "224.0.0.251:5353", _)).Times(2).WillRepeatedly(Return(OK));

    CompletionOnceCallback callback;
    // Delay sending data. Only the first call should be made.
    EXPECT_CALL(*socket_ipv6_ptr_, SendToInternal(sample_packet_, "[ff02::fb]:5353", _)).WillOnce([&](auto, auto, auto cb) {
        callback = std::move(cb);
        return ERR_IO_PENDING;
    });
    ...
    EXPECT_CALL(*socket_ipv4_ptr_, SendToInternal(sample_packet_, "224.0.0.251:5353", _)).Times(0);
    // Expect call for the second IPv6 packet.
    EXPECT_CALL(*socket_ipv6_ptr_, SendToInternal(sample_packet_, "[ff02::fb]:5353", _)).WillOnce(Return(OK));
...
TEST(MDnsSocketTest, CreateSocket)
{
    ...
    auto socket = CreateAndBindMDnsSocket(AddressFamily::ADDRESS_FAMILY_IPV4, 1, net::NetLog::Get());
    EXPECT_TRUE(socket);
    socket->Close();
}
