...
void ValidateParsedResponse(net::DnsResponse& response, const net::IOBufferWithSize& packet, std::optional<net::DnsQuery> query = std::nullopt)
{
    ...
    if (response.IsValid()) {
        ...
        if (query) {
            ...
            std::optional<std::string> dotted_qname = net::dns_names_util::NetworkToDottedName(query->qname(),
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    auto response_packet = base::MakeRefCounted<net::IOBufferWithSize>(response_string.size());
    memcpy(response_packet->data(), response_string.data(), response_string.size());
    ...
    net::DnsResponse received_response_with_query(response_packet, response_string.size());
    received_response_with_query.InitParse(response_string.size(), query);
    ValidateParsedResponse(received_response_with_query, *response_packet.get(), query);

    net::DnsResponse response(query.id(), true /* is_authoritative */, {} /* answers */, {} /* authority_records */, {} /* additional records */, query);
    std::string out = base::HexEncode(response.io_buffer()->data(), response.io_buffer_size());
