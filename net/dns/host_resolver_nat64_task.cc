...
HostResolverNat64Task::HostResolverNat64Task(std::string_view hostname, NetworkAnonymizationKey network_anonymization_key, NetLogWithSource net_log,
    ResolveContext* resolve_context, base::WeakPtr<HostResolverManager> resolver)
    : hostname_(hostname)
...
void HostResolverNat64Task::Start(base::OnceClosure completion_closure)
{
    ...
    if (rv != ERR_IO_PENDING) {
        base::SequencedTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, std::move(completion_closure_));
    }
}
...
int HostResolverNat64Task::DoResolve()
{
    ...
    request_ipv4onlyarpa_ = resolver_->CreateRequest(HostPortPair("ipv4only.arpa", 80), network_anonymization_key_, net_log_, parameters, resolve_context_);

    return request_ipv4onlyarpa_->Start(base::BindOnce(&HostResolverNat64Task::OnIOComplete, weak_ptr_factory_.GetWeakPtr()));
}
...
int HostResolverNat64Task::DoResolveComplete(int result)
{
    ...
    if (result != OK || request_ipv4onlyarpa_->GetEndpointResults()->empty()) {
        ...
        results_ = HostCache::Entry(OK, { IPEndPoint(ipv4_address, 0) }, std::move(aliases), HostCache::Entry::SOURCE_UNKNOWN);
        return OK;
    }
...
int HostResolverNat64Task::DoSynthesizeToIpv6()
{
    ...
    for (const auto& endpoints : *request_ipv4onlyarpa_->GetEndpointResults()) {
        for (const auto& ip_endpoint : endpoints.ip_endpoints) {
            ...
            Dns64PrefixLength pref64_length = ExtractPref64FromIpv4onlyArpaAAAA(ipv4onlyarpa_AAAA_address);

            IPAddress converted_address = ConvertIPv4ToIPv4EmbeddedIPv6(ipv4_address, ipv4onlyarpa_AAAA_address, pref64_length);
    ...
    results_ = HostCache::Entry(OK, std::move(converted_addresses), std::move(aliases), HostCache::Entry::SOURCE_UNKNOWN);
    return OK;
}
