...
constexpr base::FilePath::CharType kNsswitchPath[] = FILE_PATH_LITERAL(_PATH_NSSWITCH_CONF);
#else
constexpr base::FilePath::CharType kNsswitchPath[] = FILE_PATH_LITERAL("/etc/nsswitch.conf");
#endif
...
{
    std::string file;
    bool result = base::ReadFileToStringWithMaxSize(base::FilePath(kNsswitchPath), &file, kMaxFileSize);
    UMA_HISTOGRAM_BOOLEAN("Net.DNS.DnsConfig.Nsswitch.Read", result || file.size() == kMaxFileSize);
    UMA_HISTOGRAM_BOOLEAN("Net.DNS.DnsConfig.Nsswitch.TooLarge", !result && file.size() == kMaxFileSize);
...
std::string_view FindDatabase(std::string_view text, std::string_view database_name)
{
    DCHECK(!text.empty());
    ...
    while (!text.empty()) {
        ...
        if (base::StartsWith(text, database_name, base::CompareCase::INSENSITIVE_ASCII)) {
...
NsswitchReader::ServiceAction TokenizeAction(std::string_view action_column)
{
    ...
    DCHECK_EQ(action_column.find_first_of(base::kWhitespaceASCII), std::string_view::npos);

    NsswitchReader::ServiceAction result = { /*negated=*/false, NsswitchReader::Status::kUnknown, NsswitchReader::Action::kUnknown };

    std::vector<std::string_view> split = base::SplitStringPiece(action_column, "=", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL);
    if (split.size() != 2)
        return result;
...
std::vector<NsswitchReader::ServiceAction> TokenizeActions(std::string_view actions)
{
    ...
    for (const auto& action_column : base::SplitStringPiece(actions, base::kWhitespaceASCII, base::KEEP_WHITESPACE, base::SPLIT_WANT_NONEMPTY)) {
        DCHECK(!action_column.empty());
...
NsswitchReader::ServiceSpecification TokenizeService(std::string_view service_column)
{
    DCHECK(!service_column.empty());
    DCHECK_EQ(service_column.find_first_of(base::kWhitespaceASCII), std::string_view::npos);
    ...
    if (base::EqualsCaseInsensitiveASCII(service_column, "mdns4")) {
        return NsswitchReader::ServiceSpecification(NsswitchReader::Service::kMdns4);
    }
    if (base::EqualsCaseInsensitiveASCII(service_column, "mdns6")) {
        return NsswitchReader::ServiceSpecification(NsswitchReader::Service::kMdns6);
    }
    if (base::EqualsCaseInsensitiveASCII(service_column, "mdns_minimal")) {
        return NsswitchReader::ServiceSpecification(NsswitchReader::Service::kMdnsMinimal);
    }
    if (base::EqualsCaseInsensitiveASCII(service_column, "mdns4_minimal")) {
        return NsswitchReader::ServiceSpecification(NsswitchReader::Service::kMdns4Minimal);
    }
    if (base::EqualsCaseInsensitiveASCII(service_column, "mdns6_minimal")) {
        return NsswitchReader::ServiceSpecification(NsswitchReader::Service::kMdns6Minimal);
    }
    if (base::EqualsCaseInsensitiveASCII(service_column, "myhostname")) {
        return NsswitchReader::ServiceSpecification(NsswitchReader::Service::kMyHostname);
    }
    if (base::EqualsCaseInsensitiveASCII(service_column, "resolve")) {
        return NsswitchReader::ServiceSpecification(NsswitchReader::Service::kResolve);
    }
    ...
    return NsswitchReader::ServiceSpecification(NsswitchReader::Service::kUnknown);
}
...
std::string_view GetActionsStringAndRemoveBrackets(std::string_view database, size_t& out_num_bytes)
{
    ...
    actions = base::TrimWhitespaceASCII(actions, base::TrimPositions::TRIM_LEADING);
    while (!actions.empty() && actions.front() == '[') {
        actions = base::TrimWhitespaceASCII(actions.substr(1), base::TrimPositions::TRIM_LEADING);
    }

    // Include any trailing ']' and whitespace in `out_num_bytes`.
    while (out_num_bytes < database.size() && (database[out_num_bytes] == ']' || base::IsAsciiWhitespace(database[out_num_bytes]))) {
        ++out_num_bytes;
    }
...
std::vector<NsswitchReader::ServiceSpecification> TokenizeDatabase(std::string_view database)
{
    ...
    while (!database.empty()) {
        ...
        if (database.front() == '[') {
            ...
            std::string_view actions = GetActionsStringAndRemoveBrackets(database, num_actions_bytes);
            ...
            if (!actions.empty()) {
                std::vector<NsswitchReader::ServiceAction> tokenized_actions = TokenizeActions(actions);
                tokenized.back().actions.insert(tokenized.back().actions.end(), tokenized_actions.begin(), tokenized_actions.end());
            }
        } else {
        ...
        database = base::TrimWhitespaceASCII(database, base::TrimPositions::TRIM_LEADING);
    }
...
{
    return { NsswitchReader::ServiceSpecification(NsswitchReader::Service::kFiles), NsswitchReader::ServiceSpecification(NsswitchReader::Service::kDns) };
}

} // namespace

NsswitchReader::ServiceSpecification::ServiceSpecification(Service service, std::vector<ServiceAction> actions)
    : service(service)
...
NsswitchReader::ServiceSpecification::ServiceSpecification(const ServiceSpecification&) = default;

NsswitchReader::ServiceSpecification& NsswitchReader::ServiceSpecification::operator=(const ServiceSpecification&) = default;

NsswitchReader::ServiceSpecification::ServiceSpecification(ServiceSpecification&&) = default;

NsswitchReader::ServiceSpecification& NsswitchReader::ServiceSpecification::operator=(ServiceSpecification&&) = default;
...
std::vector<NsswitchReader::ServiceSpecification> NsswitchReader::ReadAndParseHosts()
{
