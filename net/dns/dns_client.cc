...
void UpdateConfigForDohUpgrade(DnsConfig* config)
{
    ...
    if (!config->unhandled_options && config->allow_dns_over_https_upgrade && !has_doh_servers && config->secure_dns_mode == SecureDnsMode::kAutomatic) {
        ...
        if (!config->dns_over_tls_hostname.empty()) {
            ...
            UMA_HISTOGRAM_BOOLEAN("Net.DNS.UpgradeConfig.DotUpgradeSucceeded", has_doh_servers);
        } else {
            bool all_local = true;
            ...
            UMA_HISTOGRAM_BOOLEAN("Net.DNS.UpgradeConfig.HasPublicInsecureNameserver", !all_local);

            config->doh_config = DnsOverHttpsConfig(GetDohUpgradeServersFromNameservers(config->nameservers));
            has_doh_servers = !config->doh_config.servers().empty();
            UMA_HISTOGRAM_BOOLEAN("Net.DNS.UpgradeConfig.InsecureUpgradeSucceeded", has_doh_servers);
        }
    } else {
        UMA_HISTOGRAM_BOOLEAN("Net.DNS.UpgradeConfig.Ineligible.DohSpecified", has_doh_servers);
        UMA_HISTOGRAM_BOOLEAN("Net.DNS.UpgradeConfig.Ineligible.UnhandledOptions", config->unhandled_options);
    }
}
...
public:
    ...
    {
        const DnsConfig* config = GetEffectiveConfig();
        return config && config->nameservers.size() > 0 && insecure_enabled_ && !config->unhandled_options && !config->dns_over_tls_active;
    }
    ...
    bool FallbackFromSecureTransactionPreferred(ResolveContext* context) const override
    {
    ...
    {
        return !CanUseInsecureDnsTransactions() || insecure_fallback_failures_ >= kMaxInsecureFallbackFailures;
    }
    ...
    base::Value::Dict GetDnsConfigAsValueForNetLog() const override
    {
        ...
        dict.Set("can_use_insecure_dns_transactions", CanUseInsecureDnsTransactions());
        return dict;
    }
...
private:
    ...
    bool UpdateDnsConfig()
    {
        ...
        if (net_log_) {
            net_log_->AddGlobalEntry(NetLogEventType::DNS_CONFIG_CHANGED, [this] { return GetDnsConfigAsValueForNetLog(); });
        }
    ...
    void UpdateSession(std::optional<DnsConfig> new_effective_config)
    {
        ...
        if (new_effective_config) {
            ...
            session_ = base::MakeRefCounted<DnsSession>(std::move(new_effective_config).value(), rand_int_callback_, net_log_);
    ...
    std::unique_ptr<AddressSorter> address_sorter_ = AddressSorter::CreateAddressSorter();
...
{
    return std::make_unique<DnsClientImpl>(net_log, base::BindRepeating(&base::RandInt));
}

// static
std::unique_ptr<DnsClient> DnsClient::CreateClientForTesting(NetLog* net_log, const RandIntCallback& rand_int_callback)
{
