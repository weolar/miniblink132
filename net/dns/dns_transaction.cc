...
constexpr net::NetworkTrafficAnnotationTag kTrafficAnnotation = net::DefineNetworkTrafficAnnotation("dns_transaction", R"(
        semantics {
...
base::Value::Dict NetLogStartParams(const std::string& hostname, uint16_t qtype)
{
...
public:
    ...
    base::Value::Dict NetLogResponseParams(NetLogCaptureMode capture_mode) const
    {
        ...
        if (GetResponse()) {
            ...
            dict.Set("additional_answer_count", static_cast<int>(GetResponse()->additional_answer_count()));
        }
...
public:
    DnsUDPAttempt(size_t server_index, std::unique_ptr<DatagramClientSocket> socket, const IPEndPoint& server, std::unique_ptr<DnsQuery> query,
    ...
    int Start(CompletionOnceCallback callback) override
    {
        ...
        int rv = socket_->ConnectAsync(server_, base::BindOnce(&DnsUDPAttempt::OnIOComplete, base::Unretained(this)));
...
private:
    ...
    int DoSendQuery(int rv)
    {
        ...
        return socket_->Write(
            query_->io_buffer(), query_->io_buffer()->size(), base::BindOnce(&DnsUDPAttempt::OnIOComplete, base::Unretained(this)), kTrafficAnnotation);
    }
    ...
    int DoReadResponse()
    {
        ...
        return socket_->Read(response_->io_buffer(), response_->io_buffer_size(), base::BindOnce(&DnsUDPAttempt::OnIOComplete, base::Unretained(this)));
    }
...
public:
    DnsHTTPAttempt(size_t doh_server_index, std::unique_ptr<DnsQuery> query, const string& server_template, const GURL& gurl_without_parameters, bool use_post,
        URLRequestContext* url_request_context, const IsolationInfo& isolation_info, RequestPriority request_priority_, bool is_probe)
        : DnsAttempt(doh_server_index)
        , query_(std::move(query))
        , net_log_(NetLogWithSource::Make(NetLog::Get(), NetLogSourceType::DNS_OVER_HTTPS))
    {
        GURL url;
        if (use_post) {
            ...
            base::Base64UrlEncode(
                std::string_view(query_->io_buffer()->data(), query_->io_buffer()->size()), base::Base64UrlEncodePolicy::OMIT_PADDING, &encoded_query);
        ...
        net_log_.BeginEvent(NetLogEventType::DOH_URL_REQUEST, [&] {
            ...
            std::optional<std::string> hostname = dns_names_util::NetworkToDottedName(query_->qname());
            DCHECK(hostname.has_value());
            return NetLogStartParams(*hostname, query_->qtype());
        });

        HttpRequestHeaders extra_request_headers;
        extra_request_headers.SetHeader(HttpRequestHeaders::kAccept, kDnsOverHttpResponseContentType);
        ...
        extra_request_headers.SetHeader(HttpRequestHeaders::kAcceptEncoding, "identity");

        DCHECK(url_request_context);
        request_ = url_request_context->CreateRequest(url, request_priority_, this, net::DefineNetworkTrafficAnnotation("dns_over_https", R"(
        semantics {
        ...
        if (use_post) {
            ...
            std::unique_ptr<UploadElementReader> reader = std::make_unique<UploadBytesElementReader>(query_->io_buffer()->span());
            request_->set_upload(ElementsUploadDataStream::CreateWithReader(std::move(reader)));
            extra_request_headers.SetHeader(HttpRequestHeaders::kContentType, kDnsOverHttpResponseContentType);
        }
        ...
        request_->SetLoadFlags(request_->load_flags() | LOAD_DISABLE_CACHE | LOAD_BYPASS_PROXY);
    ...
    int Start(CompletionOnceCallback callback) override
    {
        ...
        // the network stack.
        base::SequencedTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(&DnsHTTPAttempt::StartAsync, weak_factory_.GetWeakPtr()));
    ...
    base::Value GetRawResponseBufferForLog() const override
    {
        ...
        return NetLogBinaryValue(response_->io_buffer()->data(), response_->io_buffer_size());
    }
    const NetLogWithSource& GetSocketNetLog() const override
    {
        return net_log_;
    ...
    void OnResponseStarted(net::URLRequest* request, int net_error) override
    {
        ...
        if (request_->GetResponseCode() != 200 || !request->response_headers()->GetMimeType(&content_type)
        ...
        if (request->response_headers()->HasHeader(HttpRequestHeaders::kContentLength)) {
            if (request_->response_headers()->GetContentLength() > kDnsOverHttpResponseMaximumSize) {
                ResponseCompleted(ERR_DNS_MALFORMED_RESPONSE);
                return;
            }

            buffer_->SetCapacity(request_->response_headers()->GetContentLength() + 1);
        } else {
    ...
    void OnReadCompleted(net::URLRequest* request, int bytes_read) override
    {
        ...
        if (bytes_read > 0) {
            ...
            int read_result = request_->Read(buffer_.get(), buffer_->RemainingCapacity());
            ...
            if (read_result <= 0) {
                ...
                base::SequencedTaskRunner::GetCurrentDefault()->PostTask(
                    FROM_HERE, base::BindOnce(&DnsHTTPAttempt::OnReadCompleted, weak_factory_.GetWeakPtr(), request_.get(), read_result));
            }
        } else {
...
private:
    ...
    {
        net_log_.EndEventWithNetErrorCode(NetLogEventType::DOH_URL_REQUEST, net_error);
        DCHECK_NE(net::ERR_IO_PENDING, net_error);
        if (net_error != OK) {
            return net_error;
        }
...
void ConstructDnsHTTPAttempt(DnsSession* session, size_t doh_server_index, base::span<const uint8_t> qname, uint16_t qtype, const OptRecordRdata* opt_rdata,
    std::vector<std::unique_ptr<DnsAttempt>>* attempts, URLRequestContext* url_request_context, const IsolationInfo& isolation_info,
    RequestPriority request_priority, bool is_probe)
{
    DCHECK(url_request_context);

    std::unique_ptr<DnsQuery> query;
    if (attempts->empty()) {
        query = std::make_unique<DnsQuery>(/*id=*/0, qname, qtype, opt_rdata, DnsQuery::PaddingStrategy::BLOCK_LENGTH_128);
    } else {
        query = std::make_unique<DnsQuery>(*attempts->at(0)->GetQuery());
    }

    DCHECK_LT(doh_server_index, session->config().doh_config.servers().size());
    const DnsOverHttpsServerConfig& doh_server = session->config().doh_config.servers()[doh_server_index];
    GURL gurl_without_parameters(GetURLFromTemplateWithoutParameters(doh_server.server_template()));
    attempts->push_back(std::make_unique<DnsHTTPAttempt>(doh_server_index, std::move(query), doh_server.server_template(), gurl_without_parameters,
        doh_server.use_post(), url_request_context, isolation_info, request_priority, is_probe));
}

class DnsTCPAttempt : public DnsAttempt {
public:
    DnsTCPAttempt(size_t server_index, std::unique_ptr<StreamSocket> socket, std::unique_ptr<DnsQuery> query)
        : DnsAttempt(server_index)
        , socket_(std::move(socket))
        , query_(std::move(query))
    ...
    int Start(CompletionOnceCallback callback) override
    {
        ...
        start_time_ = base::TimeTicks::Now();
        next_state_ = STATE_CONNECT_COMPLETE;
        int rv = socket_->Connect(base::BindOnce(&DnsTCPAttempt::OnIOComplete, base::Unretained(this)));
        if (rv == ERR_IO_PENDING) {
            return rv;
        }
        return DoLoop(rv);
...
private:
    ...
    int DoConnectComplete(int rv)
    {
        ...
        buffer_ = base::MakeRefCounted<DrainableIOBuffer>(length_buffer_, length_buffer_->size());
    ...
    int DoSendLength(int rv)
    {
        ...
        if (buffer_->BytesRemaining() > 0) {
            next_state_ = STATE_SEND_LENGTH;
            return socket_->Write(
                buffer_.get(), buffer_->BytesRemaining(), base::BindOnce(&DnsTCPAttempt::OnIOComplete, base::Unretained(this)), kTrafficAnnotation);
        }
        buffer_ = base::MakeRefCounted<DrainableIOBuffer>(query_->io_buffer(), query_->io_buffer()->size());
        next_state_ = STATE_SEND_QUERY;
        return OK;
    }
    ...
    int DoReadLengthComplete(int rv)
    {
        ...
        buffer_ = base::MakeRefCounted<DrainableIOBuffer>(response_->io_buffer(), response_length_);
        next_state_ = STATE_READ_RESPONSE;
        return OK;
    }
    ...
    int ReadIntoBuffer()
    {
        return socket_->Read(buffer_.get(), buffer_->BytesRemaining(), base::BindOnce(&DnsTCPAttempt::OnIOComplete, base::Unretained(this)));
    }
...
public:
    DnsOverHttpsProbeRunner(base::WeakPtr<DnsSession> session, base::WeakPtr<ResolveContext> context)
        : session_(session)
        , context_(context)
    {
        DCHECK(session_);
        ...
        std::optional<std::vector<uint8_t>> qname = dns_names_util::DottedNameToNetwork(kDohProbeHostname);
        DCHECK(qname.has_value());
        formatted_probe_qname_ = std::move(qname).value();

        for (size_t i = 0; i < session_->config().doh_config.servers().size(); i++) {
    ...
    void Start(bool network_change) override
    {
        ...
        for (size_t i = 0; i < config.servers().size(); i++) {
            if (!probe_stats_list_[i]) {
                ...
                ContinueProbe(i, probe_stats_list_[i]->weak_factory.GetWeakPtr(), network_change, base::TimeTicks::Now() /* sequence_start_time */);
            }
        }
    }

    base::TimeDelta GetDelayUntilNextProbeForTest(size_t doh_server_index) const override
    {
        if (doh_server_index >= probe_stats_list_.size() || !probe_stats_list_[doh_server_index])
            return base::TimeDelta();

        return probe_stats_list_[doh_server_index]->backoff_entry->GetTimeUntilRelease();
    }

private:
    struct ProbeStats {
        ProbeStats()
            : backoff_entry(std::make_unique<net::BackoffEntry>(&kProbeBackoffPolicy))
        {
        }
    ...
    void ContinueProbe(size_t doh_server_index, base::WeakPtr<ProbeStats> probe_stats, bool network_change, base::TimeTicks sequence_start_time)
    {
        ...
        base::SequencedTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE,
            base::BindOnce(
                &DnsOverHttpsProbeRunner::ContinueProbe, weak_ptr_factory_.GetWeakPtr(), doh_server_index, probe_stats, network_change, sequence_start_time),
            probe_stats->backoff_entry->GetTimeUntilRelease());

        unsigned attempt_number = probe_stats->probe_attempts.size();
        ConstructDnsHTTPAttempt(session_.get(), doh_server_index, formatted_probe_qname_, dns_protocol::kTypeA, /*opt_rdata=*/nullptr,
            &probe_stats->probe_attempts, context_->url_request_context(), context_->isolation_info(), RequestPriority::DEFAULT_PRIORITY,
            /*is_probe=*/true);

        DnsAttempt* probe_attempt = probe_stats->probe_attempts.back().get();
        probe_attempt->Start(base::BindOnce(&DnsOverHttpsProbeRunner::ProbeComplete, weak_ptr_factory_.GetWeakPtr(), attempt_number, doh_server_index,
            std::move(probe_stats), network_change, sequence_start_time, base::TimeTicks::Now() /* query_start_time */));
    }

    void ProbeComplete(unsigned attempt_number, size_t doh_server_index, base::WeakPtr<ProbeStats> probe_stats, bool network_change,
        base::TimeTicks sequence_start_time, base::TimeTicks query_start_time, int rv)
    {
        bool success = false;
        while (probe_stats && session_ && context_) {
            if (rv != OK) {
                ...
                context_->RecordServerFailure(doh_server_index, /*is_doh_server=*/true, rv, session_.get());
                break;
            }
            // Check that the response parses properly before considering it a
            // success.
            DCHECK_LT(attempt_number, probe_stats->probe_attempts.size());
            const DnsAttempt* attempt = probe_stats->probe_attempts[attempt_number].get();
            const DnsResponse* response = attempt->GetResponse();
            if (response) {
                DnsResponseResultExtractor extractor(*response);
                DnsResponseResultExtractor::ResultsOrError results = extractor.ExtractDnsResults(DnsQueryType::A,
                    /*original_domain_name=*/kDohProbeHostname,
                    /*request_port=*/0);

                if (results.has_value()) {
                    for (const auto& result : results.value()) {
                        if (result->type() == HostResolverInternalResult::Type::kData && !result->AsData().endpoints().empty()) {
                            context_->RecordServerSuccess(doh_server_index, /*is_doh_server=*/true, session_.get());
                            context_->RecordRtt(doh_server_index, /*is_doh_server=*/true, base::TimeTicks::Now() - query_start_time, rv, session_.get());
                            success = true;
        ...
        base::UmaHistogramLongTimes(
            base::JoinString(
                { "Net.DNS.ProbeSequence", network_change ? "NetworkChange" : "ConfigChange", success ? "Success" : "Failure", "AttemptTime" }, "."),
...
public:
    DnsTransactionImpl(DnsSession* session, std::string hostname, uint16_t qtype, const NetLogWithSource& parent_net_log, const OptRecordRdata* opt_rdata,
        bool secure, SecureDnsMode secure_dns_mode, ResolveContext* resolve_context, bool fast_timeout)
        : session_(session)
        ...
        , net_log_(NetLogWithSource::Make(NetLog::Get(), NetLogSourceType::DNS_TRANSACTION))
        , resolve_context_(resolve_context->AsSafeRef())
    {
        DCHECK(session_.get());
        DCHECK(!hostname_.empty());
        DCHECK(!IsIPLiteral(hostname_));
        parent_net_log.AddEventReferencingSource(NetLogEventType::DNS_TRANSACTION, net_log_.source());
    }
    ...
    ~DnsTransactionImpl() override
    {
        ...
        if (!callback_.is_null()) {
            net_log_.EndEventWithNetErrorCode(NetLogEventType::DNS_TRANSACTION, ERR_ABORTED);
    ...
    void Start(ResponseCallback callback) override
    {
        ...
        net_log_.BeginEvent(NetLogEventType::DNS_TRANSACTION, [&] { return NetLogStartParams(hostname_, qtype_); });
        ...
        if (result.rv != ERR_IO_PENDING) {
            ...
            base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
                FROM_HERE, base::BindOnce(&DnsTransactionImpl::DoCallback, weak_ptr_factory_.GetWeakPtr(), result));
        }
    }
...
private:
    ...
    int PrepareSearch()
    {
        ...
        std::optional<std::vector<uint8_t>> labeled_qname = dns_names_util::DottedNameToNetwork(hostname_,
        ...
        for (const auto& suffix : config.search) {
            std::optional<std::vector<uint8_t>> qname = dns_names_util::DottedNameToNetwork(hostname_ + "." + suffix,
                /*require_valid_internet_hostname=*/true);
    ...
    void DoCallback(AttemptResult result)
    {
        ...
        const DnsResponse* response = result.attempt ? result.attempt->GetResponse() : nullptr;
        CHECK(result.rv != OK || response != nullptr);

        timer_.Stop();

        net_log_.EndEventWithNetErrorCode(NetLogEventType::DNS_TRANSACTION, result.rv);
    ...
    {
        UMA_HISTOGRAM_ENUMERATION("Net.DNS.DnsTransaction.AttemptType", attempt_type);
    }
    ...
    AttemptResult MakeClassicDnsAttempt()
    {
        ...
        std::unique_ptr<DnsQuery> query;
        if (attempts_.empty()) {
            query = std::make_unique<DnsQuery>(id, qnames_.front(), qtype_, opt_rdata_);
        } else {
        ...
        if (result.rv == ERR_IO_PENDING) {
            base::TimeDelta fallback_period = resolve_context_->NextClassicFallbackPeriod(server_index, attempt_number, session_.get());
            timer_.Start(FROM_HERE, fallback_period, this, &DnsTransactionImpl::OnFallbackPeriodExpired);
        }
    ...
    // next nameserver.
    AttemptResult MakeUdpAttempt(size_t server_index, std::unique_ptr<DnsQuery> query)
    {
        DCHECK(!secure_);
        ...
        std::unique_ptr<DatagramClientSocket> socket
            = resolve_context_->url_request_context()->GetNetworkSessionContext()->client_socket_factory->CreateDatagramClientSocket(
                DatagramSocket::RANDOM_BIND, net_log_.net_log(), net_log_.source());

        attempts_.push_back(
            std::make_unique<DnsUDPAttempt>(server_index, std::move(socket), config.nameservers[server_index], std::move(query), session_->udp_tracker()));
        ++attempts_count_;

        DnsAttempt* attempt = attempts_.back().get();
        net_log_.AddEventReferencingSource(NetLogEventType::DNS_TRANSACTION_ATTEMPT, attempt->GetSocketNetLog().source());

        int rv = attempt->Start(
            base::BindOnce(&DnsTransactionImpl::OnAttemptComplete, base::Unretained(this), attempt_number, true /* record_rtt */, base::TimeTicks::Now()));
    ...
    AttemptResult MakeHTTPAttempt()
    {
        ...
        ConstructDnsHTTPAttempt(session_.get(), doh_server_index, qnames_.front(), qtype_, opt_rdata_, &attempts_, resolve_context_->url_request_context(),
            resolve_context_->isolation_info(), request_priority_, /*is_probe=*/false);
        ++attempts_count_;
        DnsAttempt* attempt = attempts_.back().get();
        // Associate this attempt with the DoH request in NetLog.
        net_log_.AddEventReferencingSource(NetLogEventType::DNS_TRANSACTION_HTTPS_ATTEMPT, attempt->GetSocketNetLog().source());
        attempt->GetSocketNetLog().AddEventReferencingSource(NetLogEventType::DNS_TRANSACTION_HTTPS_ATTEMPT, net_log_.source());
        int rv = attempt->Start(
            base::BindOnce(&DnsTransactionImpl::OnAttemptComplete, base::Unretained(this), attempt_number, true /* record_rtt */, base::TimeTicks::Now()));
        if (rv == ERR_IO_PENDING) {
            base::TimeDelta fallback_period = resolve_context_->NextDohFallbackPeriod(doh_server_index, session_.get());
            timer_.Start(FROM_HERE, fallback_period, this, &DnsTransactionImpl::OnFallbackPeriodExpired);
        }
    ...
    AttemptResult RetryUdpAttemptAsTcp(const DnsAttempt* previous_attempt)
    {
        ...
        std::unique_ptr<DnsQuery> query = previous_attempt->GetQuery()->CloneWithNewId(session_->NextQueryId());
    ...
    AttemptResult MakeTcpAttempt(size_t server_index, std::unique_ptr<DnsQuery> query)
    {
        DCHECK(!secure_);
        const DnsConfig& config = session_->config();
        DCHECK_LT(server_index, config.nameservers.size());
        ...
        std::unique_ptr<StreamSocket> socket
            = resolve_context_->url_request_context()->GetNetworkSessionContext()->client_socket_factory->CreateTransportClientSocket(
                AddressList(config.nameservers[server_index]), nullptr, network_quality_estimator, net_log_.net_log(), net_log_.source());

        unsigned attempt_number = attempts_.size();

        attempts_.push_back(std::make_unique<DnsTCPAttempt>(server_index, std::move(socket), std::move(query)));
        ++attempts_count_;

        DnsAttempt* attempt = attempts_.back().get();
        net_log_.AddEventReferencingSource(NetLogEventType::DNS_TRANSACTION_TCP_ATTEMPT, attempt->GetSocketNetLog().source());

        int rv = attempt->Start(
            base::BindOnce(&DnsTransactionImpl::OnAttemptComplete, base::Unretained(this), attempt_number, false /* record_rtt */, base::TimeTicks::Now()));
        return AttemptResult(rv, attempt);
    }
    ...
    {
        std::optional<std::string> dotted_qname = dns_names_util::NetworkToDottedName(qnames_.front());
        net_log_.BeginEventWithStringParams(NetLogEventType::DNS_TRANSACTION_QUERY, "qname", dotted_qname.value_or("???MALFORMED_NAME???"));

        attempts_.clear();
        had_tcp_retry_ = false;
        if (secure_) {
            dns_server_iterator_ = resolve_context_->GetDohIterator(session_->config(), secure_dns_mode_, session_.get());
        } else {
            dns_server_iterator_ = resolve_context_->GetClassicDnsIterator(session_->config(), session_.get());
        }
    ...
    void OnAttemptComplete(unsigned attempt_number, bool record_rtt, base::TimeTicks start, int rv)
    {
        ...
        if (record_rtt && attempt->GetResponse()) {
            resolve_context_->RecordRtt(attempt->server_index(), secure_ /* is_doh_server */, base::TimeTicks::Now() - start, rv, session_.get());
        }
        if (callback_.is_null())
            return;
    ...
    void LogResponse(const DnsAttempt* attempt)
    {
        if (attempt) {
            net_log_.AddEvent(
                NetLogEventType::DNS_TRANSACTION_RESPONSE, [&](NetLogCaptureMode capture_mode) { return attempt->NetLogResponseParams(capture_mode); });
        }
    }
    ...
    AttemptResult ProcessAttemptResult(AttemptResult result)
    {
        while (result.rv != ERR_IO_PENDING) {
            ...
            switch (result.rv) {
            case OK:
                resolve_context_->RecordServerSuccess(result.attempt->server_index(), secure_ /* is_doh_server */, session_.get());
                net_log_.EndEventWithNetErrorCode(NetLogEventType::DNS_TRANSACTION_QUERY, result.rv);
            ...
            case ERR_DNS_TIMED_OUT:
                ...
                if (result.attempt) {
                    ...
                    resolve_context_->RecordServerFailure(result.attempt->server_index(), secure_ /* is_doh_server */, result.rv, session_.get());
                }
    ...
    {
        return base::ranges::any_of(attempts_, [](std::unique_ptr<DnsAttempt>& attempt) { return attempt->IsPending(); });
    }
    ...
    void OnFallbackPeriodExpired()
    {
        if (callback_.is_null())
            return;
        DCHECK(!attempts_.empty());
        AttemptResult result = ProcessAttemptResult(AttemptResult(ERR_DNS_TIMED_OUT, attempts_.back().get()));
        if (result.rv != ERR_IO_PENDING)
            DoCallback(result);
    }
    ...
    void StartTimeoutTimer()
    {
        ...
        if (secure_) {
            timeout = resolve_context_->SecureTransactionTimeout(secure_dns_mode_, session_.get());
        } else {
...
public:
    ...
    std::unique_ptr<DnsTransaction> CreateTransaction(std::string hostname, uint16_t qtype, const NetLogWithSource& net_log, bool secure,
        SecureDnsMode secure_dns_mode, ResolveContext* resolve_context, bool fast_timeout) override
    {
        return std::make_unique<DnsTransactionImpl>(
            session_.get(), std::move(hostname), qtype, net_log, opt_rdata_.get(), secure, secure_dns_mode, resolve_context, fast_timeout);
    }

    std::unique_ptr<DnsProbeRunner> CreateDohProbeRunner(ResolveContext* resolve_context) override
    {
        ...
        return std::make_unique<DnsOverHttpsProbeRunner>(session_->GetWeakPtr(), resolve_context->GetWeakPtr());
    }
...
// static
std::unique_ptr<DnsTransactionFactory> DnsTransactionFactory::CreateFactory(DnsSession* session)
{
