...
bool IsValidDnsRecordName(std::string_view dotted_form_name)
{
    ...
    return IsValidDnsName(dotted_form_name) && !HostStringIsLocalhost(dotted_form_name) && !ip_address.AssignFromIPLiteral(dotted_form_name)
        && !ParseURLHostnameToAddress(dotted_form_name, &ip_address);
}

// Based on DJB's public domain code.
std::optional<std::vector<uint8_t>> DottedNameToNetwork(std::string_view dotted_form_name, bool require_valid_internet_hostname)
{
    ...
    if (require_valid_internet_hostname && !IsCanonicalizedHostCompliant(dotted_form_name))
        return std::nullopt;
    ...
    while (iter != dotted_form_name.end()) {
        ...
        if (name.size() + labellen + 2 > dns_protocol::kMaxNameLength || labellen > dns_protocol::kMaxLabelLength) {
            DCHECK(!require_valid_internet_hostname);
            return std::nullopt;
        }
...
std::optional<std::string> NetworkToDottedName(base::span<const uint8_t> span, bool require_complete)
{
    auto reader = base::SpanReader(span);
    return NetworkToDottedName(reader, require_complete);
}

std::optional<std::string> NetworkToDottedName(base::SpanReader<const uint8_t>& reader, bool require_complete)
{
    std::string ret;
    ...
    while (reader.remaining() > 0u) {
        ...
        if ((reader.remaining_span()[0u] & dns_protocol::kLabelMask) == dns_protocol::kLabelPointer) {
            return std::nullopt;
        }
...
bool ReadU8LengthPrefixed(base::SpanReader<const uint8_t>& reader, base::span<const uint8_t>* out)
{
...
bool ReadU16LengthPrefixed(base::SpanReader<const uint8_t>& reader, base::span<const uint8_t>* out)
{
    base::SpanReader<const uint8_t> inner_reader = reader;
    uint16_t len;
...
std::string UrlCanonicalizeNameIfAble(std::string_view name)
{
    ...
    url::CanonicalizeHostVerbose(name.data(), url::Component(0, name.size()), &output, &host_info);
