...
{
    return IPAddress(data_provider->ConsumeIntegral<uint8_t>(), data_provider->ConsumeIntegral<uint8_t>(), data_provider->ConsumeIntegral<uint8_t>(),
...
{
    return IPAddress(data_provider->ConsumeIntegral<uint8_t>(), data_provider->ConsumeIntegral<uint8_t>(), data_provider->ConsumeIntegral<uint8_t>(),
        data_provider->ConsumeIntegral<uint8_t>(), data_provider->ConsumeIntegral<uint8_t>(), data_provider->ConsumeIntegral<uint8_t>(),
        data_provider->ConsumeIntegral<uint8_t>(), data_provider->ConsumeIntegral<uint8_t>(), data_provider->ConsumeIntegral<uint8_t>(),
        data_provider->ConsumeIntegral<uint8_t>(), data_provider->ConsumeIntegral<uint8_t>(), data_provider->ConsumeIntegral<uint8_t>(),
        data_provider->ConsumeIntegral<uint8_t>(), data_provider->ConsumeIntegral<uint8_t>(), data_provider->ConsumeIntegral<uint8_t>(),
        data_provider->ConsumeIntegral<uint8_t>());
...
DnsConfig GetFuzzedDnsConfig(FuzzedDataProvider* data_provider)
{
    ...
    for (uint32_t i = 0; i < num_nameservers; ++i) {
        config.nameservers.push_back(IPEndPoint(FuzzIPAddress(data_provider), FuzzPort(data_provider)));
    }
    ...
    for (uint8_t i = 0; i < num_hosts_entries; ++i) {
        const char* kHostnames[] = { "foo", "foo.com", "a.foo.com", "bar", "localhost", "localhost6" };
...
public:
    ...
    explicit FuzzedHostResolverProc(base::WeakPtr<FuzzedDataProvider> data_provider)
    ...
    int Resolve(const std::string& host, AddressFamily address_family, HostResolverFlags host_resolver_flags, AddressList* addrlist, int* os_error) override
    {
        ...
        if (address_family == ADDRESS_FAMILY_UNSPECIFIED || address_family == ADDRESS_FAMILY_IPV6) {
            ...
            for (uint8_t i = 0; i < num_ipv6_addresses; ++i) {
                result.push_back(net::IPEndPoint(FuzzIPv6Address(data_provider_.get()), 0));
            }
        }
        ...
        if (address_family == ADDRESS_FAMILY_UNSPECIFIED || address_family == ADDRESS_FAMILY_IPV4) {
            ...
            for (uint8_t i = 0; i < num_ipv4_addresses; ++i) {
                result.push_back(net::IPEndPoint(FuzzIPv4Address(data_provider_.get()), 0));
            }
        }
...
const Error kMdnsErrors[] = { ERR_FAILED, ERR_ACCESS_DENIED, ERR_INTERNET_DISCONNECTED, ERR_TIMED_OUT, ERR_CONNECTION_RESET, ERR_CONNECTION_ABORTED,
...
public:
    ...
    int RecvFrom(IOBuffer* buffer, int buffer_length, IPEndPoint* out_address, CompletionOnceCallback callback) override
    {
        ...
        if (data_provider_->ConsumeBool()) {
            base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE,
                base::BindOnce(
                    &FuzzedMdnsSocket::CompleteRecv, weak_factory_.GetWeakPtr(), std::move(callback), base::RetainedRef(buffer), buffer_length, out_address));
        }

        return ERR_IO_PENDING;
    }

    int SendTo(IOBuffer* buf, int buf_len, const IPEndPoint& address, CompletionOnceCallback callback) override
    {
        if (data_provider_->ConsumeBool()) {
            return data_provider_->ConsumeBool() ? OK : data_provider_->PickValueInArray(kMdnsErrors);
        }

        base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
            FROM_HERE, base::BindOnce(&FuzzedMdnsSocket::CompleteSend, weak_factory_.GetWeakPtr(), std::move(callback)));
        return ERR_IO_PENDING;
    }
...
private:
    void CompleteRecv(CompletionOnceCallback callback, IOBuffer* buffer, int buffer_length, IPEndPoint* out_address)
    {
        int rv = GenerateResponse(buffer, buffer_length, out_address);
        std::move(callback).Run(rv);
    }

    int GenerateResponse(IOBuffer* buffer, int buffer_length, IPEndPoint* out_address)
    {
        if (data_provider_->ConsumeBool()) {
            std::string data = data_provider_->ConsumeRandomLengthString(buffer_length);
            base::ranges::copy(data, buffer->data());
            *out_address = IPEndPoint(FuzzIPAddress(data_provider_), FuzzPort(data_provider_));
            return data.size();
        }
...
public:
    ...
    FuzzedHostResolverManager(const HostResolver::ManagerOptions& options, NetLog* net_log, FuzzedDataProvider* data_provider)
        : HostResolverManager(options, nullptr /* system_dns_config_notifier */, net_log)
        , data_provider_(data_provider)
    ...
    {
        HostResolverSystemTask::Params system_task_params(base::MakeRefCounted<FuzzedHostResolverProc>(data_provider_weak_factory_.GetWeakPtr()),
        ...
        SetMdnsSocketFactoryForTesting(std::make_unique<FuzzedMdnsSocketFactory>(data_provider_));
        std::unique_ptr<DnsClient> dns_client = DnsClient::CreateClientForTesting(
            net_log_, base::BindRepeating(&FuzzedDataProvider::ConsumeIntegralInRange<int32_t>, base::Unretained(data_provider_)));
    ...
    FuzzedHostResolverManager& operator=(const FuzzedHostResolverManager&) = delete;
...
private:
        ...
        const IPAddress& dest, const NetLogWithSource& net_log, ClientSocketFactory* client_socket_factory, CompletionOnceCallback callback) override
    {
        ...
        if (start_globally_reachable_async_) {
            base::SequencedTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(std::move(callback), reachable_rv));
            return ERR_IO_PENDING;
        }
        return reachable_rv;
    }
...
std::unique_ptr<ContextHostResolver> CreateFuzzedContextHostResolver(
    const HostResolver::ManagerOptions& options, NetLog* net_log, FuzzedDataProvider* data_provider, bool enable_caching)
{
    auto manager = std::make_unique<FuzzedHostResolverManager>(options, net_log, data_provider);
    auto resolve_context = std::make_unique<ResolveContext>(nullptr /* url_request_context */, enable_caching);
    return std::make_unique<ContextHostResolver>(std::move(manager), std::move(resolve_context));
}
