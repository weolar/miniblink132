...
int Bind(AddressFamily address_family, uint32_t interface_index, DatagramServerSocket* socket)
{
    ...
    return socket->JoinGroup(dns_util::GetMdnsGroupEndPoint(address_family).address());
}
...
std::unique_ptr<DatagramServerSocket> CreateAndBindMDnsSocket(AddressFamily address_family, uint32_t interface_index, NetLog* net_log)
{
    ...
    if (rv != OK) {
        socket.reset();
        VLOG(1) << "MDNS bind failed, address_family=" << address_family << ", error=" << rv;
    }
    return socket;
}
