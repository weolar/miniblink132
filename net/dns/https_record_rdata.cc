...
bool ReadNextServiceParam(std::optional<uint16_t> last_key, base::SpanReader<const uint8_t>& reader, uint16_t* out_param_key, std::string_view* out_param_value)
{
...
bool ParseMandatoryKeys(std::string_view param_value, std::set<uint16_t>* out_parsed)
{
    DCHECK(out_parsed);
...
bool ParseAlpnIds(std::string_view param_value, std::vector<std::string>* out_parsed)
{
    DCHECK(out_parsed);
...
template <size_t ADDRESS_SIZE> bool ParseIpAddresses(std::string_view param_value, std::vector<IPAddress>* out_addresses)
{
    ...
    do {
        if (auto addr_bytes = reader.template Read<ADDRESS_SIZE>(); !addr_bytes.has_value()) {
            return false;
        } else {
...
// static
std::unique_ptr<HttpsRecordRdata> HttpsRecordRdata::Parse(std::string_view data)
{
...
// static
std::unique_ptr<AliasFormHttpsRecordRdata> AliasFormHttpsRecordRdata::Parse(std::string_view data)
{
    auto reader = base::SpanReader(base::as_byte_span(data));
    ...
    std::optional<std::string> alias_name = dns_names_util::NetworkToDottedName(reader, true /* require_complete */);
    ...
    return std::make_unique<AliasFormHttpsRecordRdata>(std::move(alias_name).value());
}
...
ServiceFormHttpsRecordRdata::ServiceFormHttpsRecordRdata(HttpsRecordPriority priority, std::string service_name, std::set<uint16_t> mandatory_keys,
    std::vector<std::string> alpn_ids, bool default_alpn, std::optional<uint16_t> port, std::vector<IPAddress> ipv4_hint, std::string ech_config,
    std::vector<IPAddress> ipv6_hint, std::map<uint16_t, std::string> unparsed_params)
    : priority_(priority)
    ...
    , unparsed_params_(std::move(unparsed_params))
{
    ...
    DCHECK(!base::Contains(mandatory_keys_, dns_protocol::kHttpsServiceParamKeyMandatory));
...
bool ServiceFormHttpsRecordRdata::IsEqual(const HttpsRecordRdata* other) const
{
    ...
    return priority_ == service->priority_ && service_name_ == service->service_name_ && mandatory_keys_ == service->mandatory_keys_
        && alpn_ids_ == service->alpn_ids_ && default_alpn_ == service->default_alpn_ && port_ == service->port_ && ipv4_hint_ == service->ipv4_hint_
        && ech_config_ == service->ech_config_ && ipv6_hint_ == service->ipv6_hint_;
}
...
// static
std::unique_ptr<ServiceFormHttpsRecordRdata> ServiceFormHttpsRecordRdata::Parse(std::string_view data)
{
    auto reader = base::SpanReader(base::as_byte_span(data));
    ...
    std::optional<std::string> service_name = dns_names_util::NetworkToDottedName(reader, true /* require_complete */);
    if (!service_name.has_value())
        return nullptr;

    if (reader.remaining() == 0u) {
        return std::make_unique<ServiceFormHttpsRecordRdata>(HttpsRecordPriority { priority }, std::move(service_name).value(),
            std::set<uint16_t>() /* mandatory_keys */, std::vector<std::string>() /* alpn_ids */, true /* default_alpn */, std::nullopt /* port */,
            std::vector<IPAddress>() /* ipv4_hint */, std::string() /* ech_config */, std::vector<IPAddress>() /* ipv6_hint */,
            std::map<uint16_t, std::string>() /* unparsed_params */);
    }

    uint16_t param_key = 0;
    std::string_view param_value;
    if (!ReadNextServiceParam(std::nullopt /* last_key */, reader, &param_key, &param_value)) {
        return nullptr;
    }
    ...
    if (param_key == dns_protocol::kHttpsServiceParamKeyMandatory) {
        ...
        if (!ParseMandatoryKeys(param_value, &mandatory_keys))
            return nullptr;
        if (reader.remaining() > 0 && !ReadNextServiceParam(param_key, reader, &param_key, &param_value)) {
            return nullptr;
        }
    }
    ...
    if (param_key == dns_protocol::kHttpsServiceParamKeyPort) {
        ...
        if (param_value.size() != 2)
            return nullptr;
        uint16_t port_val = base::U16FromBigEndian(base::as_byte_span(param_value).first<2>());
        port = port_val;
        if (reader.remaining() > 0 && !ReadNextServiceParam(param_key, reader, &param_key, &param_value)) {
            return nullptr;
        }
    ...
    if (param_key > dns_protocol::kHttpsServiceParamKeyIpv6Hint) {
        for (;;) {
            DCHECK(!IsSupportedKey(param_key));
            CHECK(unparsed_params.emplace(param_key, static_cast<std::string>(param_value)).second);
    ...
    return std::make_unique<ServiceFormHttpsRecordRdata>(HttpsRecordPriority { priority }, std::move(service_name).value(), std::move(mandatory_keys),
        std::move(alpn_ids), default_alpn, port, std::move(ipv4_hint), std::move(ech_config), std::move(ipv6_hint), std::move(unparsed_params));
}

bool ServiceFormHttpsRecordRdata::IsCompatible() const
{
    std::set<uint16_t> supported_keys(std::begin(kSupportedKeys), std::end(kSupportedKeys));
