...
public:
    ...
    explicit TestUDPClientSocket(const AddressMapping* mapping, ConnectMode connect_mode)
        : mapping_(mapping)
    ...
    int Write(IOBuffer*, int, CompletionOnceCallback, const NetworkTrafficAnnotationTag& traffic_annotation) override
    {
        NOTIMPLEMENTED();
        return OK;
    }
    int SetReceiveBufferSize(int32_t) override
    {
        return OK;
    }
    int SetSendBufferSize(int32_t) override
    {
        return OK;
    }
    ...
    int ConnectAsync(const IPEndPoint& address, CompletionOnceCallback callback) override
    {
        ...
        finish_connect_callback_ = base::BindOnce(&TestUDPClientSocket::RunConnectCallback, weak_ptr_factory_.GetWeakPtr(), std::move(callback), rv);
        if (connect_mode_ == ConnectMode::kAsynchronous) {
            base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, std::move(finish_connect_callback_));
    ...
    void FinishConnect()
    {
        std::move(finish_connect_callback_).Run();
    }
...
public:
    ...
    std::unique_ptr<DatagramClientSocket> CreateDatagramClientSocket(DatagramSocket::BindType, NetLog*, const NetLogSource&) override
    {
        auto new_socket = std::make_unique<TestUDPClientSocket>(&mapping_, connect_mode_);
    ...
    std::unique_ptr<TransportClientSocket> CreateTransportClientSocket(
        const AddressList&, std::unique_ptr<SocketPerformanceWatcher>, net::NetworkQualityEstimator*, NetLog*, const NetLogSource&) override
    {
        NOTIMPLEMENTED();
        return nullptr;
    }
    std::unique_ptr<SSLClientSocket> CreateSSLClientSocket(SSLClientContext*, std::unique_ptr<StreamSocket>, const HostPortPair&, const SSLConfig&) override
    {
        NOTIMPLEMENTED();
        return nullptr;
    }
    ...
    void SetSocketCreateCallback(base::RepeatingCallback<void(TestUDPClientSocket*)> socket_create_callback)
    {
...
void OnSortComplete(bool& completed, std::vector<IPEndPoint>* sorted_buf, CompletionOnceCallback callback, bool success, std::vector<IPEndPoint> sorted)
{
    EXPECT_TRUE(success);
    completed = true;
    if (success)
...
protected:
    ...
    AddressSorterPosix::SourceAddressInfo* GetSourceInfo(const std::string& addr)
    {
...
class AddressSorterPosixSyncOrAsyncTest : public AddressSorterPosixTest, public testing::WithParamInterface<TestUDPClientSocket::ConnectMode> {
protected:
    AddressSorterPosixSyncOrAsyncTest()
    {
        SetConnectMode(GetParam());
    }
    ...
    void Verify(const char* const addresses[], const int order[])
    {
        ...
        sorter_->Sort(endpoints, base::BindOnce(&OnSortComplete, std::ref(completed_), &sorted, callback.callback()));
        ...
        for (size_t i = 0; (i < sorted.size()) || (order[i] >= 0); ++i) {
            ...
            EXPECT_TRUE(expected == actual) << "Endpoint out of order at position " << i << "\n"
...
INSTANTIATE_TEST_SUITE_P(AddressSorterPosix, AddressSorterPosixSyncOrAsyncTest,
    ::testing::Values(TestUDPClientSocket::ConnectMode::kSynchronous, TestUDPClientSocket::ConnectMode::kAsynchronous));
...
TEST_P(AddressSorterPosixSyncOrAsyncTest, Rule6)
{
    ...
    const char* const addresses[] = { "2001::1", "::ffff:1234:1", "ff32::1", "::1", nullptr };
...
TEST_P(AddressSorterPosixSyncOrAsyncTest, Rule8)
{
    ...
    const char* const addresses[] = { "ff38::1", "3000::1", "ff35::1", "ff32::1", "fe81::1", nullptr };
...
TEST_P(AddressSorterPosixSyncOrAsyncTest, Rule9)
{
    ...
    const char* const addresses[] = { "4080::1", "4002::1", "4000::1", "3000::1", nullptr };
    const int order[] = { 3, 2, 1, 0, -1 };
    Verify(addresses, order);
}
...
TEST_P(AddressSorterPosixSyncOrAsyncTest, MultipleRules)
{
    ...
    const char* const addresses[] = { "ff3e::1", "ff32::2", "4000::1", "ff32::1", "::1", "8.0.0.1", nullptr };
...
TEST_P(AddressSorterPosixSyncOrAsyncTest, InputPortsAreMaintained)
{
    ...
    std::vector<IPEndPoint> sorted;
    TestCompletionCallback callback;
    sorter_->Sort(input, base::BindOnce(&OnSortComplete, std::ref(completed_), &sorted, callback.callback()));
    callback.WaitForResult();
...
TEST_F(AddressSorterPosixTest, RandomAsyncSocketOrder)
{
    ...
    SetSocketCreateCallback(base::BindRepeating(
        [](std::vector<TestUDPClientSocket*>& created_sockets, TestUDPClientSocket* socket) { created_sockets.push_back(socket); }, std::ref(created_sockets)));
