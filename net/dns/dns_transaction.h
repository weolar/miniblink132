...
public:
    ...
    using ResponseCallback = base::OnceCallback<void(int neterror, const DnsResponse* response)>;
...
public:
    ...
    virtual base::TimeDelta GetDelayUntilNextProbeForTest(size_t doh_server_index) const = 0;
};
...
public:
    ...
    [[nodiscard]] virtual std::unique_ptr<DnsTransaction> CreateTransaction(std::string hostname, uint16_t qtype, const NetLogWithSource& net_log, bool secure,
    ...
    // resolvers.
    [[nodiscard]] virtual std::unique_ptr<DnsProbeRunner> CreateDohProbeRunner(ResolveContext* resolve_context) = 0;
    ...
    // |session|.
    [[nodiscard]] static std::unique_ptr<DnsTransactionFactory> CreateFactory(DnsSession* session);
