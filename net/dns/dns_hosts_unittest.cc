...
void PopulateExpectedHosts(const ExpectedHostsEntry* entries, size_t num_entries, DnsHosts* expected_hosts_out)
{
    for (size_t i = 0; i < num_entries; ++i) {
        ...
        ASSERT_EQ(ip_ref.size(), (entries[i].family == ADDRESS_FAMILY_IPV4) ? 4u : 16u);
    }
}

TEST(DnsHostsTest, ParseHosts)
{
    const std::string kContents = "127.0.0.1       localhost # standard\n"
                                  "\n"
    ...
    histograms.ExpectUniqueSample("Net.DNS.DnsHosts.Count", std::size(kEntries), 1);
#if !BUILDFLAG(CRONET_BUILD)
    histograms.ExpectUniqueSample("Net.DNS.DnsHosts.EstimateMemoryUsage", base::trace_event::EstimateMemoryUsage(actual_hosts), 1);
#endif
}
...
TEST(DnsHostsTest, ParseHosts_CommaIsToken)
{
    ...
    DnsHosts expected_hosts, actual_hosts;
    PopulateExpectedHosts(kEntries, std::size(kEntries), &expected_hosts);
    ParseHostsWithCommaModeForTesting(kContents, &actual_hosts, PARSE_HOSTS_COMMA_IS_TOKEN);
...
TEST(DnsHostsTest, ParseHosts_CommaIsWhitespace)
{
    ...
    DnsHosts expected_hosts, actual_hosts;
    PopulateExpectedHosts(kEntries, std::size(kEntries), &expected_hosts);
    ParseHostsWithCommaModeForTesting(kContents, &actual_hosts, PARSE_HOSTS_COMMA_IS_WHITESPACE);
    ASSERT_EQ(expected_hosts, actual_hosts);
}
