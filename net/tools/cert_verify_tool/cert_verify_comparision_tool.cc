...
public:
    CertVerifyImpl(const std::string& name, scoped_refptr<net::CertVerifyProc> proc)
        : name_(name)
        , proc_(std::move(proc))
    {
    }
    ...
    bool VerifyCert(net::X509Certificate& x509_target_and_intermediates, const std::string& hostname, net::CertVerifyResult* result, int* error)
    {
        if (hostname.empty()) {
        ...
        *error = proc_->Verify(&x509_target_and_intermediates, hostname,
            ...
            /*sct_list=*/std::string(), flags, crl_set.get(), x509_additional_trust_anchors, result, net::NetLogWithSource());
...
std::unique_ptr<CertVerifyImpl> CreateCertVerifyImplFromName(base::StringPiece impl_name, scoped_refptr<net::CertNetFetcher> cert_net_fetcher)
{
#if !(BUILDFLAG(IS_FUCHSIA) || BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS))
    if (impl_name == "platform") {
        return std::make_unique<CertVerifyImpl>("CertVerifyProc (system)", net::CertVerifyProc::CreateSystemVerifyProc(std::move(cert_net_fetcher)));
    }
#endif

    if (impl_name == "builtin") {
#if BUILDFLAG(CHROME_ROOT_STORE_SUPPORTED)
        return std::make_unique<CertVerifyImpl>("CertVerifyProcBuiltin",
            net::CreateCertVerifyProcBuiltin(std::move(cert_net_fetcher), net::CreateSslSystemTrustStoreChromeRoot(std::make_unique<net::TrustStoreChrome>())));
#endif
    }
...
void PrintStats()
{
    ...
    for (const auto& ignorable_diff_stat : ignorable_difference_stats) {
        std::cout << "  " << ignorable_diff_stat.first << ": " << ignorable_diff_stat.second << "\n";
    }
}
...
std::string TrialComparisonResultToString(net::TrialComparisonResult result)
{
    ...
    case net::TrialComparisonResult::kIgnoredMultipleEVPoliciesAndOneMatchesRoot:
    ...
    case net::TrialComparisonResult::kIgnoredBuiltinAuthorityInvalidPlatformSymantec:
...
int RunCert(base::File* input_file, const std::unique_ptr<CertVerifyImpl>& platform_proc, const std::unique_ptr<CertVerifyImpl>& builtin_proc)
{
    ...
    if (size_bytes_read != 4) {
        std::cerr << "Couldn't read 4 byte size field, read only " << size_bytes_read << "\n";
        file_error_stats["size_read_error"]++;
        return -1;
    }

    uint32_t proto_size;
    proto_size = (static_cast<uint8_t>(size_bytes[3]) << 24) + (static_cast<uint8_t>(size_bytes[2]) << 16) + (static_cast<uint8_t>(size_bytes[1]) << 8)
        + (static_cast<uint8_t>(size_bytes[0]));

    // read proto_size bytes, parse to proto.
    std::vector<char> proto_bytes(proto_size);
    int proto_bytes_read = input_file->ReadAtCurrentPos(proto_bytes.data(), proto_size);

    if ((proto_bytes_read - proto_size) != 0) {
        std::cerr << "Couldn't read expected proto of size " << proto_size << "read only " << proto_bytes_read << "\n";
    ...
    if (!cert_chain.ParseFromArray(proto_bytes.data(), proto_bytes_read)) {
        std::cerr << "Proto parse error for proto of size " << proto_size << ", " << proto_bytes_read << " proto bytes read total, " << size_bytes_read
                  << " size bytes read\n\n\n";
    ...
    if (!x509_target_and_intermediates) {
        std::cerr << "X509Certificate::CreateFromDERCertChain failed for host" << cert_chain.host() << "\n\n\n";
    ...
    platform_proc->VerifyCert(*x509_target_and_intermediates, cert_chain.host(), &platform_result, &platform_error);
    builtin_proc->VerifyCert(*x509_target_and_intermediates, cert_chain.host(), &builtin_result, &builtin_error);

    if (net::CertVerifyResultEqual(platform_result, builtin_result) && platform_error == builtin_error) {
        chain_processing_stats["equal"]++;
    } else {
        net::TrialComparisonResult result = net::IsSynchronouslyIgnorableDifference(platform_error, platform_result, builtin_error, builtin_result,
        ...
        // section of code.
        const bool chains_equal = platform_result.verified_cert->EqualsIncludingChain(builtin_result.verified_cert.get());
        ...
        if (!chains_equal && (builtin_error == net::OK || platform_error != net::OK)) {
            net::CertVerifyResult platform_reverification_result;
            int platform_reverification_error;

            platform_proc->VerifyCert(*builtin_result.verified_cert, cert_chain.host(), &platform_reverification_result, &platform_reverification_error);
            if (net::CertVerifyResultEqual(platform_reverification_result, builtin_result) && platform_reverification_error == builtin_error) {
        ...
        std::cout << "\nInput chain: \n " << FingerPrintCryptoBuffer(x509_target_and_intermediates->cert_buffer()) << " "
                  << SubjectFromX509Certificate(x509_target_and_intermediates.get()) << "\n";

        for (const auto& intermediate : x509_target_and_intermediates->intermediate_buffers()) {
            std::cout << " " << FingerPrintCryptoBuffer(intermediate.get()) << " " << SubjectFromCryptoBuffer(intermediate.get()) << "\n";
        }
...
int main(int argc, char** argv)
{
    ...
    base::ThreadPoolInstance::CreateAndStartWithDefaultParams("cert_verify_comparison_tool");
    ...
    std::unique_ptr<base::File> input_file = std::make_unique<base::File>(input_path, flags);

    if (!input_file->IsValid()) {
        std::cerr << "Error: --dump file " << input_path.MaybeAsASCII() << " is not valid \n";
        return 1;
    }
    ...
    std::unique_ptr<CertVerifyImpl> platform_proc = CreateCertVerifyImplFromName("platform", cert_net_fetcher);
    if (!platform_proc) {
        std::cerr << "Error platform proc not sucessfully created";
        return 1;
    }
    std::unique_ptr<CertVerifyImpl> builtin_proc = CreateCertVerifyImplFromName("builtin", cert_net_fetcher);
    if (!builtin_proc) {
