...
public:
    QuicChromeServerDispatchPacketTest()
        : crypto_config_("blah", quic::QuicRandom::GetInstance(), quic::test::crypto_test_utils::ProofSourceForTesting(), quic::KeyExchangeSource::Default())
        , version_manager_(quic::AllSupportedVersions())
        , connection_id_generator_(quic::kQuicDefaultConnectionIdLength)
        , dispatcher_(std::make_unique<quic::test::MockQuicDispatcher>(&config_, &crypto_config_, &version_manager_,
              std::make_unique<quic::test::MockQuicConnectionHelper>(), std::make_unique<QuicSimpleServerSessionHelper>(quic::QuicRandom::GetInstance()),
              std::make_unique<quic::test::MockAlarmFactory>(), &memory_cache_backend_, connection_id_generator_))
    {
    ...
    void DispatchPacket(const quic::QuicReceivedPacket& packet)
    {
        ...
        dispatcher_->ProcessPacket(ToQuicSocketAddress(server_addr), ToQuicSocketAddress(client_addr), packet);
    }
...
TEST_F(QuicChromeServerDispatchPacketTest, DispatchPacket)
{
    ...
    quic::QuicReceivedPacket encrypted_valid_packet(reinterpret_cast<char*>(valid_packet), std::size(valid_packet), quic::QuicTime::Zero(), false);
