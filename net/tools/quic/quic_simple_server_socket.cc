...
std::unique_ptr<UDPServerSocket> CreateQuicSimpleServerSocket(const IPEndPoint& address, IPEndPoint* server_address)
{
    auto socket = std::make_unique<UDPServerSocket>(/*net_log=*/nullptr, NetLogSource());
    ...
    rc = socket->SetReceiveBufferSize(static_cast<int32_t>(quic::kDefaultSocketReceiveBuffer));
    if (rc < 0) {
