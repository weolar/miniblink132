...
class QuicClientMessageLooplNetworkHelper : public quic::QuicClientBase::NetworkHelper, public QuicChromiumPacketReader::Visitor {
public:
    // Create a quic client, which will have events managed by an externally owned
    // EpollServer.
    QuicClientMessageLooplNetworkHelper(quic::QuicChromiumClock* clock, quic::QuicClientBase* client);

    QuicClientMessageLooplNetworkHelper(const QuicClientMessageLooplNetworkHelper&) = delete;
    QuicClientMessageLooplNetworkHelper& operator=(const QuicClientMessageLooplNetworkHelper&) = delete;
    ...
    bool CreateUDPSocketAndBind(quic::QuicSocketAddress server_address, quic::QuicIpAddress bind_to_address, int bind_to_port) override;
