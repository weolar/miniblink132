...
void ResolverThread::Run()
{
    ...
    std::unique_ptr<net::HostResolver> resolver = net::HostResolver::CreateStandaloneResolver(NetLog::Get(), options);
    ...
    std::unique_ptr<net::HostResolver::ResolveHostRequest> request
        = resolver->CreateRequest(scheme_host_port_, NetworkAnonymizationKey(), NetLogWithSource(), std::nullopt);

    base::RunLoop run_loop;
    rv_ = request->Start(base::BindOnce(&ResolverThread::OnResolutionComplete, base::Unretained(this), run_loop.QuitClosure()));
...
int ResolverThread::Resolve(url::SchemeHostPort scheme_host_port, AddressList* addresses)
{
...
// static
int SynchronousHostResolver::Resolve(url::SchemeHostPort scheme_host_port, AddressList* addresses)
{
