...
enum Errors { GENERIC = -1, ALL_GOOD = 0, INVALID_ARGUMENT = 1, CRASH_OVERWRITE, NOT_REACHED };
...
bool CreateTargetFolder(const base::FilePath& path, RankCrashes action, base::FilePath* full_path)
{
    const char* const folders[] = { "", "insert_empty1", "insert_empty2", "insert_empty3", "insert_one1", "insert_one2", "insert_one3", "insert_load1",
        "insert_load2", "remove_one1", "remove_one2", "remove_one3", "remove_one4", "remove_head1", "remove_head2", "remove_head3", "remove_head4",
        "remove_tail1", "remove_tail2", "remove_tail3", "remove_load1", "remove_load2", "remove_load3" };
...
void FlushQueue(disk_cache::Backend* cache)
{
    ...
    int rv = reinterpret_cast<disk_cache::BackendImpl*>(cache)->FlushQueueForTest(cb.callback());
    cb.GetResult(rv); // Ignore the result;
}

bool CreateCache(const base::FilePath& path, base::Thread* thread, disk_cache::Backend** cache, net::TestCompletionCallback* cb)
{
    int size = 1024 * 1024;
    disk_cache::BackendImpl* backend
        = new disk_cache::BackendImpl(path, /* cleanup_tracker = */ nullptr, thread->task_runner().get(), net::DISK_CACHE, /* net_log = */ nullptr);
...
int SimpleInsert(const base::FilePath& path, RankCrashes action, base::Thread* cache_thread)
{
    net::TestCompletionCallback cb;
    ...
    disk_cache::EntryResult result = cb_create.GetResult(cache->CreateEntry(test_name, net::HIGHEST, cb_create.callback()));
    ...
    result = cb_create.GetResult(cache->CreateEntry(test_name, net::HIGHEST, cb_create.callback()));
    if (result.net_error() != net::OK)
        return GENERIC;
...
int SimpleRemove(const base::FilePath& path, RankCrashes action, base::Thread* cache_thread)
{
    ...
    TestEntryResultCompletionCallback cb_create;
    disk_cache::EntryResult result = cb_create.GetResult(cache->CreateEntry(kCrashEntryName, net::HIGHEST, cb_create.callback()));
    if (result.net_error() != net::OK)
        return GENERIC;
    ...
    if (action >= disk_cache::REMOVE_TAIL_1) {
        result = cb_create.GetResult(cache->CreateEntry("some other key", net::HIGHEST, cb_create.callback()));
        if (result.net_error() != net::OK)
            return GENERIC;
    ...
    result = cb_create.GetResult(cache->OpenEntry(kCrashEntryName, net::HIGHEST, cb_create.callback()));
    if (result.net_error() != net::OK)
        return GENERIC;
...
int HeadRemove(const base::FilePath& path, RankCrashes action, base::Thread* cache_thread)
{
    ...
    TestEntryResultCompletionCallback cb_create;
    disk_cache::EntryResult result = cb_create.GetResult(cache->CreateEntry("some other key", net::HIGHEST, cb_create.callback()));
    if (result.net_error() != net::OK)
        return GENERIC;

    result.ReleaseEntry()->Close();
    FlushQueue(cache);
    result = cb_create.GetResult(cache->CreateEntry(kCrashEntryName, net::HIGHEST, cb_create.callback()));
    if (result.net_error() != net::OK)
        return GENERIC;
...
int LoadOperations(const base::FilePath& path, RankCrashes action, base::Thread* cache_thread)
{
    DCHECK(action >= disk_cache::INSERT_LOAD_1);

    // Work with a tiny index table (16 entries).
    disk_cache::BackendImpl* cache
        = new disk_cache::BackendImpl(path, 0xf, /*cleanup_tracker=*/nullptr, cache_thread->task_runner().get(), net::DISK_CACHE, nullptr);
    ...
    for (int i = 0; i < 100; i++) {
        ...
        disk_cache::EntryResult result = cb_create.GetResult(cache->CreateEntry(key, net::HIGHEST, cb_create.callback()));
        if (result.net_error() != net::OK)
            return GENERIC;
        result.ReleaseEntry()->Close();
        FlushQueue(cache);
    ...
    disk_cache::EntryResult result = cb_create.GetResult(cache->OpenEntry(kCrashEntryName, net::HIGHEST, cb_create.callback()));
    if (result.net_error() != net::OK)
        return GENERIC;
...
int SlaveCode(const base::FilePath& path, RankCrashes action)
{
    ...
    if (!cache_thread.StartWithOptions(base::Thread::Options(base::MessagePumpType::IO, 0)))
        return GENERIC;
