...
{
    std::cout << command_line_name << " content_encoding [content_encoding]..." << std::endl << std::endl;
    std::cout << "Decodes the stdin into the stdout using an content_encoding "
              ...
              << "Content-Encoding HTTP response header's value split by ','." << std::endl;
}
...
int main(int argc, char* argv[])
{
    ...
    return !net::ContentDecoderToolProcessInput(content_encodings, &std::cin, &std::cout);
}
