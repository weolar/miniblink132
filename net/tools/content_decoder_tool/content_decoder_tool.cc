...
// static
bool ContentDecoderToolProcessInput(std::vector<std::string> content_encodings, std::istream* input_stream, std::ostream* output_stream)
{
    std::unique_ptr<SourceStream> upstream(std::make_unique<StdinSourceStream>(input_stream));
    for (const auto& content_encoding : base::Reversed(content_encodings)) {
        ...
        if (base::EqualsCaseInsensitiveASCII(content_encoding, kBrotli)) {
            ...
            downstream = GzipSourceStream::Create(std::move(upstream), SourceStream::TYPE_DEFLATE);
        } else if (base::EqualsCaseInsensitiveASCII(content_encoding, kGZip) || base::EqualsCaseInsensitiveASCII(content_encoding, kXGZip)) {
            downstream = GzipSourceStream::Create(std::move(upstream), SourceStream::TYPE_GZIP);
        } else {
    ...
    scoped_refptr<IOBuffer> read_buffer = base::MakeRefCounted<IOBufferWithSize>(kBufferLen);
    while (true) {
        TestCompletionCallback callback;
        int bytes_read = upstream->Read(read_buffer.get(), kBufferLen, callback.callback());
