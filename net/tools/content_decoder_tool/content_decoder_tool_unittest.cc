...
protected:
    ContentDecoderToolTest()
        : gzip_encoded_len_(kBufferSize)
    {
    }
    ...
    void SetUp() override
    {
        ...
        CompressGzip(source_data_.data(), source_data_.size(), gzip_encoded_, &gzip_encoded_len_, true);
    }
    ...
    const char* brotli_encoded()
    {
        return brotli_encoded_.data();
    }
    size_t brotli_encoded_len()
    {
        return brotli_encoded_.size();
    }

    char* gzip_encoded()
    {
        return gzip_encoded_;
    }
    size_t gzip_encoded_len()
    {
        return gzip_encoded_len_;
    }
...
TEST_F(ContentDecoderToolTest, TestBrotli)
{
    ...
    bool brotli_disabled = CreateBrotliSourceStream(std::move(mock_source_stream)) == nullptr;
    if (brotli_disabled)
        return;
