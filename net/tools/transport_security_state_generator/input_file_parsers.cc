...
{
    const char* const important_words[] = { "Universal", "Global", "EV", "G1", "G2", "G3", "G4", "G5" };
...
{
    std::vector<std::string_view> words = base::SplitStringPiece(name, " ", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL);
    if (words.empty()) {
    ...
    if (!base::EqualsCaseInsensitiveASCII(pin_name.substr(0, first_word.size()), first_word)) {
        LOG(ERROR) << "The first word of the certificate name (" << first_word << ") isn't a prefix of the variable name (" << pin_name << ")";
        return false;
    }
    ...
    for (size_t i = 0; i < words.size(); ++i) {
        ...
        if (word == "Class" && (i + 1) < words.size()) {
            ...
            if (pos == std::string::npos) {
                LOG(ERROR) << "Certficate class specification doesn't appear in the variable "
                              "name ("
                           << pin_name << ")";
                return false;
            }
            ...
            if (pos == std::string::npos) {
                LOG(ERROR) << "Number doesn't appear in the certificate variable name (" << pin_name << ")";
                return false;
            }
        } else if (IsImportantWordInCertificateName(word)) {
            pos = pin_name.find(word);
            if (pos == std::string::npos) {
                LOG(ERROR) << std::string(word) + " doesn't appear in the certificate variable name (" << pin_name << ")";
                return false;
            }
        }
    }
...
bool IsValidName(std::string_view candidate)
{
    ...
    for (const char& character : candidate) {
        isValid
            = (character >= '0' && character <= '9') || (character >= 'a' && character <= 'z') || (character >= 'A' && character <= 'Z') || character == '_';
        if (!isValid) {
            return false;
        }
    }
...
bool ParseCertificatesFile(std::string_view certs_input, Pinsets* pinsets, base::Time* timestamp)
{
    ...
    for (std::string_view line : SplitStringPiece(certs_input, "\n", base::KEEP_WHITESPACE, base::SPLIT_WANT_ALL)) {
        ...
        case CertificateParserState::IN_CERTIFICATE:
            ...
            if (!ExtractSubjectNameFromCertificate(certificate.get(), &subject_name)) {
                LOG(ERROR) << "Could not extract name from certificate " << name;
                return false;
            }

            if (!MatchCertificateName(subject_name, name)) {
                LOG(ERROR) << name << " is not a reasonable name for " << subject_name;
                return false;
            }
        ...
        case CertificateParserState::PRE_TIMESTAMP:
            ...
            if (!base::StringToUint64(line, &timestamp_epoch) || !base::IsValueInRangeForNumericType<time_t>(timestamp_epoch)) {
...
bool ParseJSON(std::string_view hsts_json, std::string_view pins_json, TransportSecurityStateEntries* entries, Pinsets* pinsets)
{
    static constexpr auto valid_hsts_keys = base::MakeFixedFlatSet<std::string_view>({
        kNameJSONKey,
        kPolicyJSONKey,
    ...
    static constexpr auto valid_pins_keys = base::MakeFixedFlatSet<std::string_view>({
        kNameJSONKey,
        kIncludeSubdomainsJSONKey,
        kPinsJSONKey,
    });
    ...
    std::set<std::string> valid_policies
        = { "test", "public-suffix", "google", "custom", "bulk-legacy", "bulk-18-weeks", "bulk-1-year", "public-suffix-requested" };
    ...
    const base::Value::List* pinning_entries_list = pins_dict->FindList("entries");
    ...
    for (size_t i = 0; i < pinning_entries_list->size(); ++i) {
        ...
        if (!parsed) {
            LOG(ERROR) << "Could not parse entry " << base::NumberToString(i) << " in the input pins JSON";
            return false;
        }
        const std::string* maybe_hostname = parsed->FindString(kNameJSONKey);
        if (!maybe_hostname) {
            LOG(ERROR) << "Could not extract the hostname for entry " << base::NumberToString(i) << " from the input pins JSON";
            return false;
        }

        if (maybe_hostname->empty()) {
            LOG(ERROR) << "The hostname for entry " << base::NumberToString(i) << " is empty";
            return false;
        }

        for (auto entry_value : *parsed) {
            if (!base::Contains(valid_pins_keys, entry_value.first)) {
                LOG(ERROR) << "The entry for " << *maybe_hostname << " contains an unknown " << entry_value.first << " field";
                return false;
            }
        }
        ...
        if (!maybe_pinset) {
            LOG(ERROR) << "Could not extract the pinset for entry " << base::NumberToString(i) << " from the input pins JSON";
            return false;
        }

        if (pins_map.find(*maybe_hostname) != pins_map.end()) {
            LOG(ERROR) << *maybe_hostname << " has duplicate entries in the input pins JSON";
            return false;
        }

        pins_map[*maybe_hostname] = std::pair(*maybe_pinset, parsed->FindBool(kIncludeSubdomainsJSONKey).value_or(false));
    }

    const base::Value::List* preload_entries_list = hsts_value->GetDict().FindList("entries");
    ...
    for (size_t i = 0; i < preload_entries_list->size(); ++i) {
        ...
        if (!parsed) {
            LOG(ERROR) << "Could not parse entry " << base::NumberToString(i) << " in the input HSTS JSON";
            return false;
        }
        ...
        const std::string* maybe_hostname = parsed->FindString(kNameJSONKey);
        if (!maybe_hostname) {
            LOG(ERROR) << "Could not extract the hostname for entry " << base::NumberToString(i) << " from the input HSTS JSON";
            return false;
        }
        ...
        for (auto entry_value : *parsed) {
            if (!base::Contains(valid_hsts_keys, entry_value.first)) {
                LOG(ERROR) << "The entry for " << entry->hostname << " contains an unknown " << entry_value.first << " field";
                return false;
            }
        }
        ...
        if (!policy || !base::Contains(valid_policies, *policy)) {
            LOG(ERROR) << "The entry for " << entry->hostname << " does not have a valid policy";
            return false;
        }
        ...
        entry->include_subdomains = parsed->FindBool(kIncludeSubdomainsJSONKey).value_or(false);
    ...
    for (size_t i = 0; i < pinsets_list->size(); ++i) {
        ...
        if (!parsed) {
            LOG(ERROR) << "Could not parse pinset " << base::NumberToString(i) << " in the input JSON";
            return false;
        }

        const std::string* maybe_name = parsed->FindString("name");
        if (!maybe_name) {
            LOG(ERROR) << "Could not extract the name for pinset " << base::NumberToString(i) << " from the input JSON";
            return false;
        }
        std::string name = *maybe_name;

        const std::string* maybe_report_uri = parsed->FindString("report_uri");
        std::string report_uri = maybe_report_uri ? *maybe_report_uri : std::string();

        auto pinset = std::make_unique<Pinset>(name, report_uri);

        const base::Value::List* pinset_static_hashes_list = parsed->FindList("static_spki_hashes");
        if (pinset_static_hashes_list) {
            for (const auto& hash : *pinset_static_hashes_list) {
                if (!hash.is_string()) {
                    LOG(ERROR) << "Could not parse static spki hash " << hash.DebugString() << " in the input JSON";
                    return false;
                }
        ...
        const base::Value::List* pinset_bad_static_hashes_list = parsed->FindList("bad_static_spki_hashes");
        if (pinset_bad_static_hashes_list) {
            for (const auto& hash : *pinset_bad_static_hashes_list) {
                if (!hash.is_string()) {
                    LOG(ERROR) << "Could not parse bad static spki hash " << hash.DebugString() << " in the input JSON";
                    return false;
                }
