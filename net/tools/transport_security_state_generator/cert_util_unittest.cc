...
// "Security."
static const char kSelfSignedWithCommonNamePEM[] = "-----BEGIN CERTIFICATE-----\n"
...
// "Security".
static const char kSelfSignedWithoutCommonNamePEM[] = "-----BEGIN CERTIFICATE-----\n"
...
static const char kSelfSignedWithoutSubject[] = "-----BEGIN CERTIFICATE-----\n"
...
static const char kInvalidCertificatePEM[] = "-----BEGIN CERTIFICATE-----\n"
                                             "This is invalid base64.\n"
                                             "It contains some (#$*) invalid characters.\n"
                                             "-----END CERTIFICATE-----\n";

// Valid PEM public key headers but invalid BASE64 content.
static const char kInvalidPublicKeyPEM[] = "-----BEGIN PUBLIC KEY-----\n"
                                           "This is invalid base64.\n"
                                           "It contains some (#$*) invalid characters.\n"
                                           "-----END PUBLIC KEY-----\n";
...
static const char kUnknownPEMHeaders[] = "-----BEGIN OF SOMETHING-----\n"
                                         "MIIBIjANBgkqhkiG9w0BAQEFAAOCAQ8AMIIBCgKCAQEAujzwcb5bJuC/A/Y9izGl\n"
                                         "LlA3fnKGbeyn53BdVznJN4fQwU82WKVYdqt8d/1ZDRdYyhGrTgXJeCURe9VSJyX1\n"
                                         "X2a5EApSFsopP8Yjy0Rl6dNOLO84KCW9dPmfHC3uP0ac4hnHT5dUr05YvhJmHCkf\n"
                                         "as6v/aEgpPLDhRF6UruSUh+gIpUg/F3+vlD99HLfbloukoDtQyxW+86s9sO7RQ00\n"
...
{
    SPKIHash hash1;
    bssl::UniquePtr<X509> cert1 = GetX509CertificateFromPEM(kSelfSignedWithCommonNamePEM);
    ...
    EXPECT_THAT(hash_vector,
        testing::ElementsAreArray({ 0xAC, 0xFB, 0x2B, 0xF3, 0x6A, 0x90, 0x47, 0xF1, 0x74, 0xAE, 0xF1, 0xCE, 0x63, 0x3D, 0xA9, 0x45, 0xCB, 0xA, 0xA7, 0x3F, 0x16,
            0x2A, 0xF3, 0x88, 0x9A, 0xE2, 0x72, 0xC, 0x07, 0x63, 0x45, 0xB0 }));

    SPKIHash hash2;
    bssl::UniquePtr<X509> cert2 = GetX509CertificateFromPEM(kSelfSignedWithoutCommonNamePEM);
    ...
    EXPECT_THAT(hash_vector2,
        testing::ElementsAreArray({ 0x40, 0xBC, 0xD6, 0xE4, 0x10, 0x70, 0x37, 0x3C, 0xF7, 0x21, 0x51, 0xD7, 0x27, 0x64, 0xFD, 0xF1, 0xA, 0x89, 0x0, 0xAD, 0x75,
            0xDF, 0xB3, 0xEA, 0x21, 0xFC, 0x6E, 0x67, 0xD5, 0xAE, 0xA4, 0x94 }));
}
...
TEST(CertUtilTest, CalculateSPKIHashFromKey)
{
    ...
    std::vector<uint8_t> hash_vector(hash1.data(), hash1.data() + hash1.size());
    EXPECT_THAT(hash_vector,
        testing::ElementsAreArray({ 0x63, 0xB0, 0x21, 0x4, 0x3, 0x13, 0x9E, 0x36, 0xEE, 0xCB, 0x6F, 0xA5, 0x7A, 0x94, 0x56, 0x18, 0xBA, 0x41, 0x13, 0x8C, 0x4A,
            0x48, 0x99, 0x80, 0x51, 0x66, 0xF8, 0x85, 0x2, 0xFC, 0x48, 0x9E }));
    SPKIHash hash2;
    EXPECT_FALSE(CalculateSPKIHashFromKey(kInvalidPublicKeyPEM, &hash2));

    SPKIHash hash3;
    EXPECT_FALSE(CalculateSPKIHashFromKey(kSelfSignedWithoutCommonNamePEM, &hash3));
...
TEST(CertUtilTest, ExtractSubjectNameFromCertificate)
{
    ...
    std::string name3;
    bssl::UniquePtr<X509> cert3 = GetX509CertificateFromPEM(kSelfSignedWithoutSubject);
