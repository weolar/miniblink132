...
{
    std::string valid = "# This line should ignored. The rest should result in 3 pins.\n"
                        "PinsListTimestamp\n"
                        "1649894400\n"
                        "TestPublicKey1\n"
    ...
    ASSERT_TRUE(base::Time::FromUTCString("2022-04-14T00:00:00Z", &expected_timestamp));
...
TEST(InputFileParsersTest, ParseCertificatesFileInvalid)
{
    ...
    std::string invalid = "PinsListTimestamp\n"
                          "1649894400\n"
                          "TestName\n"
                          "unexpected";
...
TEST(InputFileParsersTest, ParseCertificatesFileInvalidName)
{
    ...
    std::string invalid_name_small_character = "PinsListTimestamp\n"
                                               "1649894400\n"
                                               "startsWithSmallLetter\n"
                                               "sha256/AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA=\n";
    EXPECT_FALSE(ParseCertificatesFile(invalid_name_small_character, &pinsets, &unused));

    std::string invalid_name_invalid_characters = "PinsListTimestamp\n"
                                                  "1649894400\n"
                                                  "Invalid-Characters-In-Name\n"
                                                  "sha256/AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA=\n";
    EXPECT_FALSE(ParseCertificatesFile(invalid_name_invalid_characters, &pinsets, &unused));

    std::string invalid_name_number = "PinsListTimestamp\n"
                                      "1649894400\n"
                                      "1InvalidName\n"
                                      "sha256/AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA=\n";
    EXPECT_FALSE(ParseCertificatesFile(invalid_name_number, &pinsets, &unused));

    std::string invalid_name_space = "PinsListTimestamp\n"
                                     "1649894400\n"
                                     "Invalid Name\n"
                                     "sha256/AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA=\n";
...
TEST(InputFileParsersTest, ParseCertificatesFileInvalidCertificateName)
{
    Pinsets pinsets;
    base::Time unused;
    std::string timestamp_prefix = "PinsListTimestamp\n"
                                   "1649894400\n";
    ...
    std::string missing_prefix = timestamp_prefix + "Class3_G1_Test\n" + certificate;
    EXPECT_FALSE(ParseCertificatesFile(missing_prefix, &pinsets, &unused));

    std::string missing_class = timestamp_prefix + "Chromium_G1_Test\n" + certificate;
    EXPECT_FALSE(ParseCertificatesFile(missing_class, &pinsets, &unused));

    std::string missing_number = timestamp_prefix + "Chromium_Class3_Test\n" + certificate;
    EXPECT_FALSE(ParseCertificatesFile(missing_number, &pinsets, &unused));

    std::string valid = timestamp_prefix + "Chromium_Class3_G1_Test\n" + certificate;
...
TEST(InputFileParsersTest, ParseCertificatesFileInvalidTimestamp)
{
    ...
    std::string timestamp_prefix = "PinsListTimestamp\n"
                                   "1649894400\n";
    std::string bad_timestamp_prefix = "PinsListTimestamp\n"
                                       "NotReallyTimestamp\n";
    std::string certificate = "Chromium_Class3_G1_Test\n"
                              "-----BEGIN CERTIFICATE-----\n"
                              "MIIDIzCCAgugAwIBAgIJALs84KlxWh4GMA0GCSqGSIb3DQEBCwUAMCgxGTAXBgNV\n"
                              "BAoMEENocm9taXVtIENsYXNzIDMxCzAJBgNVBAsMAkcxMB4XDTE3MDIwMTE5NTUw\n"
                              "NVoXDTE4MDIwMTE5NTUwNVowKDEZMBcGA1UECgwQQ2hyb21pdW0gQ2xhc3MgMzEL\n"
    ...
    std::string multiple_timestamp = timestamp_prefix + timestamp_prefix + certificate;
