...
// |value|.
bool ReplaceTag(const std::string& name, const std::string& value, std::string* tpl)
{
    std::string tag = "[[" + name + "]]";
...
std::string WritePinsetList(const std::string& name, const std::vector<std::string>& pins)
{
...
huffman_trie::HuffmanRepresentationTable ApproximateHuffman(const TransportSecurityStateEntries& entries)
{
...
std::string PreloadedStateGenerator::Generate(
    const std::string& preload_template, const TransportSecurityStateEntries& entries, const Pinsets& pinsets, const base::Time& timestamp)
{
    ...
    for (const auto& entry : entries) {
        auto trie_entry = std::make_unique<TransportSecurityStateTrieEntry>(pinsets_map, entry.get());
    ...
    huffman_trie::HuffmanRepresentationTable optimal_table = huffman_builder.ToTable();
    ...
    ReplaceTag("PINS_LIST_TIMESTAMP", base::NumberToString(timestamp.ToTimeT()), &output);

    return output;
}

void PreloadedStateGenerator::ProcessSPKIHashes(const Pinsets& pinset, std::string* tpl)
{
    std::string output;
...
void PreloadedStateGenerator::ProcessPinsets(const Pinsets& pinset, NameIDMap* pinset_map, std::string* tpl)
{
    ...
    for (const auto& current : pinsets) {
        ...
        const std::string& accepted_pins_names = FormatAcceptedKeyName(uppercased_name);
        certs_output.append(WritePinsetList(accepted_pins_names, pinset_ptr->static_spki_hashes()));
        ...
        if (pinset_ptr->bad_static_spki_hashes().size()) {
            ...
            certs_output.append(WritePinsetList(rejected_pins_names, pinset_ptr->bad_static_spki_hashes()));
            certs_output.append(kNewLine);
        }
        ...
        pinsets_output.append(kIndent);
        pinsets_output.append("{" + accepted_pins_names + ", " + rejected_pins_names + ", " + report_uri + "},");
        pinsets_output.append(kNewLine);

        pinset_map->insert(NameIDPair(pinset_ptr->name(), static_cast<uint32_t>(pinset_map->size())));
    }
