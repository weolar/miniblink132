...
bool CheckForDuplicatePins(const Pinsets& pinsets)
{
    ...
    for (const auto& pin : pinsets.spki_hashes()) {
        ...
        std::string hash = std::string(pin.second.data(), pin.second.data() + pin.second.size());
        auto it = seen_hashes.find(hash);
        if (it != seen_hashes.cend()) {
            LOG(ERROR) << "Duplicate pin hash for " << pin.first << ", already seen as " << it->second;
            return false;
        }
...
bool CheckCertificatesInPinsets(const Pinsets& pinsets)
{
    ...
    for (const auto& pinset : pinsets.pinsets()) {
        ...
        const std::vector<std::string>& good_hashes = pinset.second->static_spki_hashes();
        const std::vector<std::string>& bad_hashes = pinset.second->bad_static_spki_hashes();

        std::vector<std::string> all_pin_names;
        all_pin_names.reserve(good_hashes.size() + bad_hashes.size());
        all_pin_names.insert(all_pin_names.end(), good_hashes.begin(), good_hashes.end());
        all_pin_names.insert(all_pin_names.end(), bad_hashes.begin(), bad_hashes.end());

        for (const auto& pin_name : all_pin_names) {
            if (pin_names.find(pin_name) == pin_names.cend()) {
                LOG(ERROR) << "Pinset " << pinset.second->name() << " references pin " + pin_name << " which doesn't exist";
                return false;
            }
...
bool CheckNoopEntries(const TransportSecurityStateEntries& entries)
{
    for (const auto& entry : entries) {
        if (!entry->force_https && entry->pinset.empty()) {
            ...
            LOG(ERROR) << "Entry for " << entry->hostname << " has no mode and no pins";
            return false;
        }
    }
    return true;
...
bool CheckHostnames(const TransportSecurityStateEntries& entries)
{
    for (const auto& entry : entries) {
        ...
        if (!most_recent_component_started_alphanumeric) {
            LOG(ERROR) << "The last label of " << hostname << " must start with a lowercase alphanumeric character";
            return false;
        }
...
int main(int argc, char* argv[])
{
    ...
    if (!ParseCertificatesFile(certs_input, &pinsets, &timestamp) || !ParseJSON(hsts_json_input, pins_json_input, &entries, &pinsets)) {
        LOG(ERROR) << "Error while parsing the input files.";
        return 1;
    }

    if (!CheckDuplicateEntries(entries) || !CheckNoopEntries(entries) || !CheckForDuplicatePins(pinsets) || !CheckCertificatesInPinsets(pinsets)
    ...
    VLOG(1) << "Wrote trie containing " << entries.size() << " entries, referencing " << pinsets.size() << " pinsets to " << output_path.AsUTF8Unsafe()
            << std::endl;
