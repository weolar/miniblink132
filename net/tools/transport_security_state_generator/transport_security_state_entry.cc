...
{
    return entry->force_https && entry->include_subdomains && entry->pinset.empty();
}
...
TransportSecurityStateTrieEntry::TransportSecurityStateTrieEntry(const NameIDMap& pinsets_map, TransportSecurityStateEntry* entry)
...
bool TransportSecurityStateTrieEntry::WriteEntry(huffman_trie::TrieBitBuffer* writer) const
{
