...
bool ParsePEM(std::string_view pem_input, std::string_view expected_block_type, std::string* der_output)
{
    const std::string& block_start = base::StringPrintf(kPEMBeginBlock, expected_block_type.data());
    const std::string& block_end = base::StringPrintf(kPEMEndBlock, expected_block_type.data());
    ...
    std::string_view base64_encoded = pem_input.substr(base64_start_pos, block_end_pos - base64_start_pos);

    if (!base::Base64Decode(base::CollapseWhitespaceASCII(base64_encoded, true), der_output)) {
        return false;
    }
...
bssl::UniquePtr<X509> GetX509CertificateFromPEM(std::string_view pem_data)
{
    ...
    return bssl::UniquePtr<X509>(d2i_X509(nullptr, &der_data, base::checked_cast<long>(der.size())));
}
...
bool ExtractSubjectNameFromCertificate(X509* certificate, std::string* name)
{
    ...
    if (!ExtractFieldFromX509Name(subject, NID_commonName, &result) || result.empty()) {
        std::string organization;
        if (!ExtractFieldFromX509Name(subject, NID_organizationName, &organization)) {
            return false;
        }

        std::string organizational_unit;
        if (!ExtractFieldFromX509Name(subject, NID_organizationalUnitName, &organizational_unit)) {
            return false;
        }
...
bool CalculateSPKIHashFromCertificate(X509* certificate, SPKIHash* out_hash)
{
    ...
    size_t spki_der_len;
    bssl::ScopedCBB cbb;
    if (!CBB_init(cbb.get(), 0) || !EVP_marshal_public_key(cbb.get(), key.get()) || !CBB_finish(cbb.get(), &spki_der, &spki_der_len)) {
        return false;
    }
...
bool CalculateSPKIHashFromKey(std::string_view pem_key, SPKIHash* out_hash)
{
    ...
    out_hash->CalculateFromBytes(reinterpret_cast<const uint8_t*>(der.data()), der.size());
    return true;
}
