...
const char* const kCommandNames[]
    = { "stop", "get_size", "list_keys", "get_stream", "delete_stream", "delete_key", "update_raw_headers", "list_dups", "set_header" };

// Prints the command line help.
void PrintHelp()
{
    std::cout << "cachetool <cache_path> <cache_backend_type> <subcommand> " << std::endl << std::endl;
    ...
    std::cout << "  delete_stream <key> <index>: Delete a particular stream of a"
              << " given key." << std::endl;
    std::cout << "  get_size: Calculate the total size of the cache in bytes." << std::endl;
    ...
    std::cout << "  stop: Verify that the cache can be opened and return, "
              << "confirming the cache exists and is of the right type." << std::endl;
...
public:
    ProgramArgumentCommandMarshal(Backend* cache_backend, base::CommandLine::StringVector args)
...
void GetSize(CommandMarshal* command_marshal)
{
    ...
    int64_t rv = command_marshal->cache_backend()->CalculateSizeOfAllEntries(cb.callback());
    rv = cb.GetResult(rv);
    if (rv < 0)
...
{
    std::unique_ptr<Backend::Iterator> entry_iterator = command_marshal->cache_backend()->CreateIterator();
...
bool GetResponseInfoForEntry(disk_cache::Entry* entry, net::HttpResponseInfo* response_info)
{
    int size = entry->GetDataSize(kResponseInfoIndex);
    if (size == 0)
        return false;
    scoped_refptr<net::IOBuffer> buffer = base::MakeRefCounted<net::IOBufferWithSize>(size);
    net::TestCompletionCallback cb;

    int bytes_read = 0;
    while (true) {
        int rv = entry->ReadData(kResponseInfoIndex, bytes_read, buffer.get(), size, cb.callback());
        rv = cb.GetResult(rv);
        if (rv < 0) {
            entry->Close();
            return false;
        ...
        if (rv == 0) {
            ...
            if (!net::HttpCache::ParseResponseInfo(buffer->span(), response_info, &truncated_response_info)) {
                return false;
            }
...
std::string GetMD5ForResponseBody(disk_cache::Entry* entry)
{
    ...
    scoped_refptr<net::IOBuffer> buffer = base::MakeRefCounted<net::IOBufferWithSize>(kInitBufferSize);
    net::TestCompletionCallback cb;
    ...
    int bytes_read = 0;
    while (true) {
        int rv = entry->ReadData(kResponseContentIndex, bytes_read, buffer.get(), kInitBufferSize, cb.callback());
        rv = cb.GetResult(rv);
        if (rv < 0) {
            entry->Close();
            return "";
...
void PersistResponseInfo(CommandMarshal* command_marshal, const std::string& key, const net::HttpResponseInfo& response_info)
{
    scoped_refptr<net::PickledIOBuffer> data = base::MakeRefCounted<net::PickledIOBuffer>();
    response_info.Persist(data->pickle(), false, false);
    data->Done();

    TestEntryResultCompletionCallback cb_open;
    EntryResult result = command_marshal->cache_backend()->OpenEntry(key, net::HIGHEST, cb_open.callback());
    ...
    net::TestCompletionCallback cb;
    int rv = cache_entry->WriteData(kResponseInfoIndex, 0, data.get(), data_len, cb.callback(), true);
...
void ListDups(CommandMarshal* command_marshal)
{
    ...
    for (const auto& hash_and_entries : md5_entries) {
        ...
        for (const auto& entry : hash_and_entries.second) {
            std::string out = base::StringPrintf("%d, %s, %s", entry.size, entry.url.c_str(), entry.mime_type.c_str());
    ...
    int64_t rv = command_marshal->cache_backend()->CalculateSizeOfAllEntries(size_cb.callback());
...
scoped_refptr<net::GrowableIOBuffer> GetStreamForKeyBuffer(CommandMarshal* command_marshal, const std::string& key, int index)
{
    ...
    scoped_refptr<net::GrowableIOBuffer> buffer = base::MakeRefCounted<net::GrowableIOBuffer>();
    buffer->SetCapacity(kInitBufferSize);
    net::TestCompletionCallback cb;
    while (true) {
        int rv = cache_entry->ReadData(index, buffer->offset(), buffer.get(), buffer->capacity() - buffer->offset(), cb.callback());
        rv = cb.GetResult(rv);
        if (rv < 0) {
            cache_entry->Close();
...
void GetStreamForKey(CommandMarshal* command_marshal)
{
    ...
    if (command_marshal->has_failed())
        return;
    scoped_refptr<net::GrowableIOBuffer> buffer(GetStreamForKeyBuffer(command_marshal, key, index));
    if (command_marshal->has_failed())
        return;
    if (index == kResponseInfoIndex) {
        net::HttpResponseInfo response_info;
        bool truncated_response_info = false;
        if (!net::HttpCache::ParseResponseInfo(buffer->span_before_offset(), &response_info, &truncated_response_info)) {
            // This can happen when reading data stored by content::CacheStorage.
            std::cerr << "WARNING: Returning empty response info for key: " << key << std::endl;
            command_marshal->ReturnSuccess();
        ...
        command_marshal->ReturnSuccess();
        command_marshal->ReturnString(net::HttpUtil::ConvertHeadersBackToHTTPResponse(response_info.headers->raw_headers()));
    } else {
        command_marshal->ReturnSuccess();
...
void UpdateRawResponseHeaders(CommandMarshal* command_marshal)
{
    ...
    if (command_marshal->has_failed())
        return;
    scoped_refptr<net::GrowableIOBuffer> buffer(GetStreamForKeyBuffer(command_marshal, key, kResponseInfoIndex));
    if (command_marshal->has_failed())
        return;
    net::HttpResponseInfo response_info;
    bool truncated_response_info = false;
    net::HttpCache::ParseResponseInfo(buffer->span_before_offset(), &response_info, &truncated_response_info);
    if (truncated_response_info)
        std::cerr << "WARNING: Truncated HTTP response." << std::endl;

    response_info.headers = base::MakeRefCounted<net::HttpResponseHeaders>(raw_headers);
...
void DeleteStreamForKey(CommandMarshal* command_marshal)
{
    ...
    net::TestCompletionCallback cb;
    scoped_refptr<net::StringIOBuffer> buffer = base::MakeRefCounted<net::StringIOBuffer>("");
    int rv = cache_entry->WriteData(index, 0, buffer.get(), 0, cb.callback(), true);
...
void DeleteKey(CommandMarshal* command_marshal)
{
    ...
    if (command_marshal->has_failed())
        return;
    net::TestCompletionCallback cb;
    int rv = command_marshal->cache_backend()->DoomEntry(key, net::HIGHEST, cb.callback());
    if (cb.GetResult(rv) != net::OK)
...
bool ExecuteCommands(CommandMarshal* command_marshal)
{
    while (!command_marshal->has_failed()) {
        ...
        if (subcommand == "stop") {
            ...
            StreamCommandMarshal stream_command_marshal(command_marshal->cache_backend());
...
int main(int argc, char* argv[])
{
    ...
    BackendResult result = disk_cache::CreateCacheBackend(net::DISK_CACHE, backend_type, /*file_operations=*/nullptr, cache_path, INT_MAX,
        disk_cache::ResetHandling::kNeverReset, /*net_log=*/nullptr, cb.callback());
    ...
    ProgramArgumentCommandMarshal program_argument_marshal(cache_backend.get(), base::CommandLine::StringVector(args.begin() + 2, args.end()));
