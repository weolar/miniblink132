...
void DumpStats(const base::FilePath& path, disk_cache::CacheAddr addr)
{
    ...
    int64_t* counters = reinterpret_cast<int64_t*>(buffer.get() + 2 + disk_cache::Stats::kDataSizesLength);
...
void DumpIndexHeader(const base::FilePath& name, disk_cache::CacheAddr* stats_addr)
{
    disk_cache::IndexHeader header;
    if (!ReadHeader(name, reinterpret_cast<char*>(&header), sizeof(header)))
        return;
...
public:
    ...
    bool LoadRankings(disk_cache::CacheAddr addr, disk_cache::RankingsNode* rankings);
...
bool CacheDumper::Init()
{
    ...
    index_ = reinterpret_cast<disk_cache::Index*>(index_file_->Init(index_name, 0));
    if (!index_) {
...
bool CacheDumper::GetEntry(disk_cache::EntryStore* entry, disk_cache::CacheAddr* addr)
{
...
bool CacheDumper::LoadEntry(disk_cache::CacheAddr addr, disk_cache::EntryStore* entry)
{
    disk_cache::Addr address(addr);
...
bool CacheDumper::LoadRankings(disk_cache::CacheAddr addr, disk_cache::RankingsNode* rankings)
{
    disk_cache::Addr address(addr);
...
{
    return base::UnlocalizedTimeFormatWithPattern(base::Time::FromDeltaSinceWindowsEpoch(base::Microseconds(time_us)), "y/M/d H:m:s.S");
}

void DumpEntry(disk_cache::CacheAddr addr, const disk_cache::EntryStore& entry, bool verbose)
{
    std::string key;
...
void DumpRankings(disk_cache::CacheAddr addr, const disk_cache::RankingsNode& rankings, bool verbose)
{
...
void PrintCSVHeader()
{
    printf("entry,rankings,next,prev,rank-contents,chain,reuse,key,"
           "d0,d1,d2,d3\n");
}

void DumpCSV(disk_cache::CacheAddr addr, const disk_cache::EntryStore& entry, const disk_cache::RankingsNode& rankings)
{
    printf("0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x\n", addr, entry.rankings_node, rankings.next, rankings.prev, rankings.contents,
        entry.next, entry.reuse_count, entry.long_key, entry.data_addr[0], entry.data_addr[1], entry.data_addr[2], entry.data_addr[3]);
...
int DumpHeaders(const base::FilePath& input_path)
{
    ...
    base::FileEnumerator iter(input_path, false, base::FileEnumerator::FILES, FILE_PATH_LITERAL("data_*"));
...
int DumpLists(const base::FilePath& input_path)
{
    ...
    for (int i = 0; i < 5; i++) {
        ...
        while (addr) {
            ...
            printf("%d, 0x%x, 0x%x, 0x%x, 0x%x\n", i, addr, rankings.next, rankings.prev, rankings.contents);
...
int DumpAllocation(const base::FilePath& file)
{
    ...
    std::string out;
    net::ViewCacheHelper::HexDump(reinterpret_cast<char*>(&header.allocation_map), sizeof(header.allocation_map), &out);
    printf("%s\n", out.c_str());
    return 0;
}
