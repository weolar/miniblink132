...
int main(int argc, const char* argv[])
{
    ...
    if (command_line->HasSwitch("ssl-cert")) {
        if ((embedded_test_server_type.has_value() && *embedded_test_server_type != net::EmbeddedTestServer::TYPE_HTTPS)
            || (!embedded_test_server_type.has_value() && !net::SpawnedTestServer::UsingSSL(server_type))) {
    ...
    base::FilePath full_path = net::test_server::EmbeddedTestServer::GetFullPathFromSourceDirectory(doc_root);
    ...
    if (net::SpawnedTestServer::UsingSSL(server_type)) {
        test_server = std::make_unique<net::SpawnedTestServer>(server_type, ssl_options, doc_root);
    } else {
        test_server = std::make_unique<net::SpawnedTestServer>(server_type, doc_root);
    }
