...
void TrieBitBuffer::WritePosition(uint32_t position, int32_t* last_position)
{
    ...
    constexpr uint8_t kMaxBitLength = kShortOffsetMaxLength + (1 << kLongOffsetLengthLength);
    ...
    if (*last_position != -1) {
        ...
        DCHECK(number_of_bits <= kMaxBitLength) << "positive position delta too large.";
        ...
        if (number_of_bits <= kShortOffsetMaxLength) {
            ...
            WriteBits(number_of_bits - kShortOffsetMaxLength - 1, kLongOffsetLengthLength);
...
void TrieBitBuffer::WriteChar(uint8_t byte, const HuffmanRepresentationTable& table, HuffmanBuilder* huffman_builder)
{
...
uint32_t TrieBitBuffer::WriteToBitWriter(BitWriter* writer)
{
    ...
    for (auto const& element : elements_) {
        if (element.number_of_bits) {
            writer->WriteBits(element.bits >> (8 - element.number_of_bits), element.number_of_bits);
        } else {
