...
class TrieWriter {
public:
    TrieWriter(const HuffmanRepresentationTable& huffman_table, HuffmanBuilder* huffman_builder);
    ~TrieWriter();
...
protected:
    ...
    {
        return huffman_builder_;
    }

private:
    bool WriteDispatchTables(ReversedEntries::iterator start, ReversedEntries::iterator end, uint32_t* position);
