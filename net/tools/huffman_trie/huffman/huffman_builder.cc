...
class HuffmanNode {
public:
    HuffmanNode(uint8_t value, uint32_t count, std::unique_ptr<HuffmanNode> left, std::unique_ptr<HuffmanNode> right)
        : value_(value)
        , count_(count)
        , left_(std::move(left))
    ...
    const std::unique_ptr<HuffmanNode>& left() const
    {
        return left_;
    }
    const std::unique_ptr<HuffmanNode>& right() const
    {
        return right_;
    }
...
bool CompareNodes(const std::unique_ptr<HuffmanNode>& lhs, const std::unique_ptr<HuffmanNode>& rhs)
{
...
void HuffmanBuilder::TreeToTable(HuffmanNode* node, uint32_t bits, uint32_t number_of_bits, HuffmanRepresentationTable* table)
{
    if (node->IsLeaf()) {
...
uint32_t HuffmanBuilder::WriteToVector(HuffmanNode* node, std::vector<uint8_t>* vector)
{
    uint8_t left_value;
    uint8_t right_value;
...
std::unique_ptr<HuffmanNode> HuffmanBuilder::BuildTree()
{
    ...
    for (const auto& item : counts_) {
        nodes.push_back(std::make_unique<HuffmanNode>(item.first, item.second, nullptr, nullptr));
    }
    ...
    while (nodes.size() > 1) {
        ...
        auto parent = std::make_unique<HuffmanNode>(0, count_a + count_b, std::move(a), std::move(b));
