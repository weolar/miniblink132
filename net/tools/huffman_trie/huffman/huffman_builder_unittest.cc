...
TEST(HuffmanBuilderTest, NoPrefixCollision)
{
    ...
    for (uint8_t i = 0; i <= 127; i++) {
        ...
        for (uint8_t j = 0; j <= 127; j++) {
            ...
            uint32_t aligned_i = encoding[i].bits << (32 - encoding[i].number_of_bits);
            uint32_t aligned_j = encoding[j].bits << (32 - encoding[j].number_of_bits);
...
TEST(HuffmanBuilderTest, ToVector)
{
  ...
  //         0xE3 (c)    node 0     0xE4 (d)    0xE5 (e)
    //                    /      \
  //                0xE1 (a)    0xE2 (b)
    EXPECT_THAT(output, testing::ElementsAre(0xE1, 0xE2, 0xE3, 0x0, 0xE4, 0xE5, 0x1, 0x2));
}
