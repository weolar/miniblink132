...
std::optional<std::map<std::string, std::string>> DecodeCerts(std::string_view in)
{
    ...
    for (;;) {
        ...
        if (strcmp(name, "CERTIFICATE") != 0) {
            LOG(ERROR) << "Found PEM block of type " << name << " instead of CERTIFICATE";
            return std::nullopt;
        }
        std::string sha256_hex = base::ToLowerASCII(base::HexEncode(crypto::SHA256Hash(base::make_span(data, base::checked_cast<size_t>(len)))));
        certs[sha256_hex] = std::string(data, data + len);
    }
    return std::move(certs);
}

std::optional<RootStore> ReadTextRootStore(const base::FilePath& root_store_path, const base::FilePath& certs_path)
{
    std::string root_store_text;
    if (!base::ReadFileToString(base::MakeAbsoluteFilePath(root_store_path), &root_store_text)) {
        LOG(ERROR) << "Could not read " << root_store_path;
        return std::nullopt;
    }

    RootStore root_store;
    if (!google::protobuf::TextFormat::ParseFromString(root_store_text, &root_store)) {
    ...
    if (!certs_path.empty()) {
        ...
        if (!base::ReadFileToString(base::MakeAbsoluteFilePath(certs_path), &certs_data)) {
    ...
    for (auto& anchor : *root_store.mutable_trust_anchors()) {
        if (anchor.certificate_case() != chrome_root_store::TrustAnchor::kSha256Hex) {
            continue;
        }
    ...
    if (!certs.empty()) {
        LOG(ERROR) << "Unused certificate (SHA-256 hash " << certs.begin()->first << ") in " << certs_path;
        return std::nullopt;
    }
...
{
    return base::StrCat({ "base::Time::UnixEpoch() + base::Seconds(", base::NumberToString(t), ")" });
}
...
bool WriteRootCppFile(const RootStore& root_store, const base::FilePath cpp_path)
{
    ...
    std::string string_to_write = "// This file is auto-generated, DO NOT EDIT.\n\n";
    ...
    for (int i = 0; i < root_store.trust_anchors_size(); i++) {
        ...
        base::StringAppendF(&string_to_write, "constexpr uint8_t kChromeRootCert%d[] = {", i);
        ...
        if (anchor.constraints_size() > 0) {
            ...
            for (const auto& constraint : anchor.constraints()) {
                ...
                constraint_params.push_back(constraint.has_sct_not_after_sec() ? SecondsFromEpochToBaseTime(constraint.sct_not_after_sec()) : kNulloptString);

                constraint_params.push_back(constraint.has_sct_all_after_sec() ? SecondsFromEpochToBaseTime(constraint.sct_all_after_sec()) : kNulloptString);

                constraint_params.push_back(constraint.has_min_version() ? VersionFromString(constraint.min_version()) : kNulloptString);

                constraint_params.push_back(constraint.has_max_version_exclusive() ? VersionFromString(constraint.max_version_exclusive()) : kNulloptString);

                if (constraint.permitted_dns_names_size() > 0) {
                    constraint_params.push_back(base::StringPrintf("kChromeRootConstraint%dNames%d", i, constraint_num));
                } else {
                    constraint_params.push_back("{}");
                }

                constraint_strings.push_back(base::StrCat({ "{", base::JoinString(constraint_params, ","), "}" }));
...
// Returns true if file was correctly written, false otherwise.
bool WriteEvCppFile(const RootStore& root_store, const base::FilePath cpp_path)
{
...
int main(int argc, char** argv)
{
    ...
    base::FilePath root_store_cpp_path = command_line.GetSwitchValuePath("write-cpp-root-store");
    base::FilePath ev_roots_cpp_path = command_line.GetSwitchValuePath("write-cpp-ev-roots");
    base::FilePath root_store_path = command_line.GetSwitchValuePath("root-store");
    base::FilePath certs_path = command_line.GetSwitchValuePath("certs");

    if ((proto_path.empty() && root_store_cpp_path.empty() && ev_roots_cpp_path.empty()) || root_store_path.empty() || command_line.HasSwitch("help")) {
    ...
    std::optional<RootStore> root_store = ReadTextRootStore(root_store_path, certs_path);
    if (!root_store) {
        return 1;
    }
    ...
    if (!proto_path.empty()) {
        ...
        if (!root_store->SerializeToString(&serialized)) {
            LOG(ERROR) << "Error serializing root store proto" << root_store->DebugString();
            return 1;
        }
    ...
    if (!root_store_cpp_path.empty() && !WriteRootCppFile(*root_store, root_store_cpp_path)) {
        PLOG(ERROR) << "Error writing root store C++ include file";
        return 1;
    }
    if (!ev_roots_cpp_path.empty() && !WriteEvCppFile(*root_store, ev_roots_cpp_path)) {
