...
void EntryWrapper::DoOpen(int key)
{
    ...
    disk_cache::EntryResult result
        = g_data->cache->OpenEntry(g_data->keys[key], net::HIGHEST, base::BindOnce(&EntryWrapper::OnOpenDone, base::Unretained(this), key));
...
void EntryWrapper::OnOpenDone(int key, disk_cache::EntryResult result)
{
    ...
    state_ = CREATE;
    result = g_data->cache->CreateEntry(g_data->keys[key], net::HIGHEST, base::BindOnce(&EntryWrapper::OnOpenDone, base::Unretained(this), key));
    if (result.net_error() != net::ERR_IO_PENDING)
        OnOpenDone(key, std::move(result));
}
...
void EntryWrapper::DoRead()
{
    ...
    int rv = entry_->ReadData(0, 0, buffer_.get(), kReadSize, base::BindOnce(&EntryWrapper::OnReadDone, base::Unretained(this)));
...
void EntryWrapper::DoWrite()
{
    ...
    state_ = WRITE;
    base::snprintf(buffer_->data(), kBufferSize, "Write: %d iter: %d, size: %d, truncate: %d     ", g_data->writes, g_data->iteration, size, truncate ? 1 : 0);
    int rv = entry_->WriteData(0, 0, buffer_.get(), size, base::BindOnce(&EntryWrapper::OnWriteDone, base::Unretained(this), size), truncate);
    if (rv != net::ERR_IO_PENDING)
        OnWriteDone(size, rv);
}
...
{
    state_ = DOOM;
    int rv = g_data->cache->DoomEntry(key, net::HIGHEST, base::BindOnce(&EntryWrapper::OnDeleteDone, base::Unretained(this)));
    if (rv != net::ERR_IO_PENDING)
        OnDeleteDone(rv);
}
...
void EntryWrapper::DoIdle()
{
    ...
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(&LoopTask));
}
...
void StressTheCache(int iteration)
{
    ...
    g_data->cache = new disk_cache::BackendImpl(path, mask, /*cleanup_tracker=*/nullptr, cache_thread.task_runner().get(), net::DISK_CACHE, nullptr);
    ...
    printf("Iteration %d, initial entries: %d\n", iteration, g_data->cache->GetEntryCount());
...
void RunSoon(scoped_refptr<base::SingleThreadTaskRunner> task_runner)
{
    ...
    task_runner->PostDelayedTask(FROM_HERE, base::BindOnce(&CrashCallback), kTaskDelay);
}
...
void CrashHandler(const char* file, int line, std::string_view str, std::string_view stack_trace)
{
    g_crashing = true;
...
const GUID kStressCacheTraceProviderName = { 0xb9a153d4, 0x31c3, 0x48e4, { 0x9a, 0xbf, 0xd5, 0x43, 0x83, 0xf1, 0x4a, 0xd } };
#endif
...
int main(int argc, const char* argv[])
{
    ...
    logging::ScopedLogAssertHandler scoped_assert_handler(base::BindRepeating(CrashHandler));
