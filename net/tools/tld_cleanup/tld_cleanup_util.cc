...
NormalizeResult NormalizeDataToRuleMap(const std::string& data, RuleMap& rules)
{
    ...
    for (std::string line; std::getline(data_stream, line, '\n');) {
        ...
        CHECK(!base::Contains(rules, domain)) << "Duplicate rule found for " << domain;
        ...
        if (tld_start != std::string::npos && tld_start + 1 < domain.size()) {
            ...
            bool is_private = in_private_section && (iter == extra_rules.end() || iter->second.is_private);
            extra_rules[extra_rule_domain] = Rule { /*exception=*/false, /*wildcard=*/false, is_private };
        }
    }

    base::ranges::copy_if(extra_rules, std::inserter(rules, rules.end()), [&](const auto& extra_rule) { return !base::Contains(rules, extra_rule.first); });
