...
const char* ConnectionTypeToString(net::NetworkChangeNotifier::ConnectionType type)
{
    switch (type) {
...
const char* ConfigAvailabilityToString(net::ProxyConfigService::ConfigAvailability availability)
{
    switch (availability) {
...
public:
    ...
    void OnConnectionTypeChanged(net::NetworkChangeNotifier::ConnectionType type) override
    {
    ...
    void OnProxyConfigChanged(const net::ProxyConfigWithAnnotation& config, net::ProxyConfigService::ConfigAvailability availability) override
    {
        LOG(INFO) << "OnProxyConfigChanged(" << ProxyConfigToString(config.value()) << ", " << ConfigAvailabilityToString(availability) << ")";
    }
};
...
int main(int argc, char* argv[])
{
    ...
    if (!ignored_netifs_str.empty()) {
        for (const std::string& ignored_netif : base::SplitString(ignored_netifs_str, ",", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL)) {
            LOG(INFO) << "Ignoring: " << ignored_netif;
            ignored_interfaces.insert(ignored_netif);
        }
    }
    auto network_change_notifier = std::make_unique<net::NetworkChangeNotifierLinux>(ignored_interfaces);
#else
    std::unique_ptr<net::NetworkChangeNotifier> network_change_notifier(net::NetworkChangeNotifier::CreateIfNeeded());
#endif

    // Use the network loop as the file loop also.
    std::unique_ptr<net::ProxyConfigService> proxy_config_service(net::ProxyConfigService::CreateSystemProxyConfigService(io_task_executor.task_runner()));
    ...
    LOG(INFO) << "Initial connection type: " << ConnectionTypeToString(net::NetworkChangeNotifier::GetConnectionType());

    {
        net::ProxyConfigWithAnnotation config;
        const net::ProxyConfigService::ConfigAvailability availability = proxy_config_service->GetLatestProxyConfig(&config);
        LOG(INFO) << "Initial proxy config: " << ProxyConfigToString(config.value()) << ", " << ConfigAvailabilityToString(availability);
    }
