...
public:
    ...
    struct NET_EXPORT NelPolicyKey {
        NelPolicyKey();
        NelPolicyKey(const NetworkAnonymizationKey& network_anonymization_key, const url::Origin& origin);
    ...
    struct WildcardNelPolicyKey {
        ...
        WildcardNelPolicyKey(const NetworkAnonymizationKey& network_anonymization_key, const std::string& domain);
    ...
    struct NET_EXPORT SignedExchangeReportDetails {
        ...
        SignedExchangeReportDetails& operator=(const SignedExchangeReportDetails& other);
    ...
    static std::unique_ptr<NetworkErrorLoggingService> Create(PersistentNelStore* store);

    NetworkErrorLoggingService(const NetworkErrorLoggingService&) = delete;
    NetworkErrorLoggingService& operator=(const NetworkErrorLoggingService&) = delete;
    ...
    virtual void OnHeader(
        const NetworkAnonymizationKey& network_anonymization_key, const url::Origin& origin, const IPAddress& received_ip_address, const std::string& value)
        = 0;
    ...
    virtual void QueueSignedExchangeReport(SignedExchangeReportDetails details) = 0;

    // Removes browsing data (origin policies) associated with any origin for
    // which |origin_filter| returns true.
    virtual void RemoveBrowsingData(const base::RepeatingCallback<bool(const url::Origin&)>& origin_filter) = 0;
...
public:
    using NelPoliciesLoadedCallback = base::OnceCallback<void(std::vector<NelPolicy>)>;
