...
MockPersistentNelStore::Command::Command(Type type, NelPoliciesLoadedCallback loaded_callback)
    : type(type)
    , loaded_callback(std::move(loaded_callback))
{
}

MockPersistentNelStore::Command::Command(Type type, const NetworkErrorLoggingService::NelPolicy& policy)
    : type(type)
    , key(policy.key)
{
}
...
bool operator==(const MockPersistentNelStore::Command& lhs, const MockPersistentNelStore::Command& rhs)
{
...
void MockPersistentNelStore::LoadNelPolicies(NelPoliciesLoadedCallback loaded_callback)
{
    DCHECK(!load_started_);
    command_list_.emplace_back(Command::Type::LOAD_NEL_POLICIES, std::move(loaded_callback));
    load_started_ = true;
}

void MockPersistentNelStore::AddNelPolicy(const NetworkErrorLoggingService::NelPolicy& policy)
{
    DCHECK(load_started_);
...
void MockPersistentNelStore::UpdateNelPolicyAccessTime(const NetworkErrorLoggingService::NelPolicy& policy)
{
    DCHECK(load_started_);
    command_list_.emplace_back(Command::Type::UPDATE_NEL_POLICY, policy);
}

void MockPersistentNelStore::DeleteNelPolicy(const NetworkErrorLoggingService::NelPolicy& policy)
{
    DCHECK(load_started_);
...
void MockPersistentNelStore::SetPrestoredPolicies(std::vector<NetworkErrorLoggingService::NelPolicy> policies)
{
    DCHECK(!load_started_);
    ...
    for (size_t i = 0; i < command_list_.size(); ++i) {
        ...
        if (command.type == Command::Type::LOAD_NEL_POLICIES) {
            ...
            if (load_success) {
                ...
                std::move(command.loaded_callback).Run(std::vector<NetworkErrorLoggingService::NelPolicy>());
            }
        }
        if (i > 0) {
...
bool MockPersistentNelStore::VerifyCommands(const CommandList& expected_commands) const
{
    return command_list_ == expected_commands;
}

MockPersistentNelStore::CommandList MockPersistentNelStore::GetAllCommands() const
{
