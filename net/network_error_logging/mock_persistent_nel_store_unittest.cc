...
NetworkErrorLoggingService::NelPolicy MakePolicy(const url::Origin& origin, const net::NetworkAnonymizationKey& network_anonymization_key)
{
...
void RunClosureOnNelPoliciesLoaded(base::OnceClosure closure, std::vector<NetworkErrorLoggingService::NelPolicy>* policies_out,
...
// destruction.
MockPersistentNelStore::NelPoliciesLoadedCallback MakeExpectedRunNelPoliciesLoadedCallback(std::vector<NetworkErrorLoggingService::NelPolicy>* policies_out)
{
    base::OnceClosure closure = base::MakeExpectedRunClosure(FROM_HERE);
    return base::BindOnce(&RunClosureOnNelPoliciesLoaded, std::move(closure), policies_out);
}
...
protected:
    const url::Origin origin_ = url::Origin::Create(GURL("https://example.test/"));
    const NetworkAnonymizationKey network_anonymization_key_ = NetworkAnonymizationKey::CreateCrossSite(SchemefulSite(GURL("https://foo.test/")));
    const NetworkErrorLoggingService::NelPolicy nel_policy_ = MakePolicy(origin_, network_anonymization_key_);
};
...
TEST_F(MockPersistentNelStoreTest, FinishLoading)
{
    ...
    store.LoadNelPolicies(MakeExpectedRunNelPoliciesLoadedCallback(&loaded_policies));
...
TEST_F(MockPersistentNelStoreTest, PreStoredPolicies)
{
    ...
    std::vector<NetworkErrorLoggingService::NelPolicy> prestored_policies = { nel_policy_ };
    ...
    EXPECT_EQ(network_anonymization_key_, loaded_policies[0].key.network_anonymization_key);
...
TEST_F(MockPersistentNelStoreTest, Add)
{
    ...
    expected_commands.emplace_back(MockPersistentNelStore::Command::Type::ADD_NEL_POLICY, policy);
...
TEST_F(MockPersistentNelStoreTest, AddThenDelete)
{
    ...
    expected_commands.emplace_back(MockPersistentNelStore::Command::Type::DELETE_NEL_POLICY, policy);
    EXPECT_EQ(3u, store.GetAllCommands().size());
...
TEST_F(MockPersistentNelStoreTest, AddThenUpdate)
{
    ...
    expected_commands.emplace_back(MockPersistentNelStore::Command::Type::UPDATE_NEL_POLICY, policy);
    EXPECT_EQ(3u, store.GetAllCommands().size());
