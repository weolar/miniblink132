...
// resolve that proxy.
class NET_EXPORT ConfiguredProxyResolutionService : public ProxyResolutionService,
...
public:
    ...
    public:
        ...
        virtual Mode GetNextDelay(int initial_error, base::TimeDelta current_delay, base::TimeDelta* next_delay) const = 0;
    };
    ...
    ConfiguredProxyResolutionService(const ConfiguredProxyResolutionService&) = delete;
    ConfiguredProxyResolutionService& operator=(const ConfiguredProxyResolutionService&) = delete;
    ...
    //   2.  PAC URL
    //   3.  named proxy
    int ResolveProxy(const GURL& url, const std::string& method, const NetworkAnonymizationKey& network_anonymization_key, ProxyInfo* results,
        CompletionOnceCallback callback, std::unique_ptr<ProxyResolutionRequest>* request, const NetLogWithSource& net_log) override;
    ...
    void SetPacFileFetchers(std::unique_ptr<PacFileFetcher> pac_file_fetcher, std::unique_ptr<DhcpPacFileFetcher> dhcp_pac_file_fetcher);
    ...
    // ProxyResolutionService
    [[nodiscard]] bool CastToConfiguredProxyResolutionService(ConfiguredProxyResolutionService** configured_proxy_resolution_service) override;
    ...
    static std::unique_ptr<ConfiguredProxyResolutionService> CreateUsingSystemProxyResolver(
    ...
    // autoconfig.
    static std::unique_ptr<ConfiguredProxyResolutionService> CreateWithoutProxyResolver(
    ...
    static std::unique_ptr<ConfiguredProxyResolutionService> CreateFixedForTest(const ProxyConfigWithAnnotation& pc);
    ...
    static std::unique_ptr<ConfiguredProxyResolutionService> CreateFixedFromPacResultForTest(
    ...
    static std::unique_ptr<ConfiguredProxyResolutionService> CreateFixedFromAutoDetectedPacResultForTest(
        const std::string& pac_string, const NetworkTrafficAnnotationTag& traffic_annotation);

    // This method is used by tests to create a ConfiguredProxyResolutionService
    // that returns a proxy fallback list (|proxy_chain|) for every URL.
    static std::unique_ptr<ConfiguredProxyResolutionService> CreateFixedFromProxyChainsForTest(
    ...
    // active policy.
    static const PacPollPolicy* set_pac_script_poll_policy(const PacPollPolicy* policy);
    ...
    bool quick_check_enabled_for_testing() const
    {
...
private:
    ...
    FRIEND_TEST_ALL_PREFIXES(ProxyResolutionServiceTest, UpdateConfigAfterFailedAutodetect);
    FRIEND_TEST_ALL_PREFIXES(ProxyResolutionServiceTest, UpdateConfigFromPACToDirect);
    class InitProxyResolver;
    class PacFileDeciderPoller;

    typedef std::set<raw_ptr<ConfiguredProxyResolutionRequest, SetExperimental>> PendingRequests;
    ...
    int DidFinishResolvingProxy(const GURL& url, const NetworkAnonymizationKey& network_anonymization_key, const std::string& method, ProxyInfo* result,
        int result_code, const NetLogWithSource& net_log);

    // Start initialization using |fetched_config_|.
    void InitializeUsingLastFetchedConfig();

    // Start the initialization skipping past the "decision" phase.
    void InitializeUsingDecidedConfig(int decider_result, const PacFileDataWithSource& script_data, const ProxyConfigWithAnnotation& effective_config);
    ...
    base::WeakPtrFactory<ConfiguredProxyResolutionService> weak_ptr_factory_ { this };
};
