...
PacFileDataWithSource::~PacFileDataWithSource() = default;
PacFileDataWithSource::PacFileDataWithSource(const PacFileDataWithSource&) = default;
PacFileDataWithSource& PacFileDataWithSource::operator=(const PacFileDataWithSource&) = default;

base::Value::Dict PacFileDecider::PacSource::NetLogParams(const GURL& effective_pac_url) const
{
...
PacFileDecider::PacFileDecider(PacFileFetcher* pac_file_fetcher, DhcpPacFileFetcher* dhcp_pac_file_fetcher, NetLog* net_log)
    : pac_file_fetcher_(pac_file_fetcher)
    , dhcp_pac_file_fetcher_(dhcp_pac_file_fetcher)
    , net_log_(NetLogWithSource::Make(net_log, NetLogSourceType::PAC_FILE_DECIDER))
{
}
...
int PacFileDecider::Start(const ProxyConfigWithAnnotation& config, const base::TimeDelta wait_delay, bool fetch_pac_bytes, CompletionOnceCallback callback)
{
    ...
    traffic_annotation_ = net::MutableNetworkTrafficAnnotationTag(config.traffic_annotation());
...
// (2) WPAD (DNS).
// (3) Custom PAC URL.
PacFileDecider::PacSourceList PacFileDecider::BuildPacSourcesFallbackList(const ProxyConfig& config) const
{
...
int PacFileDecider::DoWait()
{
    ...
    wait_timer_.Start(FROM_HERE, wait_delay_, this, &PacFileDecider::OnWaitTimerFired);
...
int PacFileDecider::DoWaitComplete(int result)
{
    DCHECK_EQ(OK, result);
    if (wait_delay_.ToInternalValue() != 0) {
        net_log_.EndEventWithNetErrorCode(NetLogEventType::PAC_FILE_DECIDER_WAIT, result);
    }
...
int PacFileDecider::DoQuickCheck()
{
    ...
    if (!pac_file_fetcher_ || !pac_file_fetcher_->GetRequestContext() || !pac_file_fetcher_->GetRequestContext()->host_resolver()) {
    ...
    HostResolver* host_resolver = pac_file_fetcher_->GetRequestContext()->host_resolver();
    resolve_request_
        = host_resolver->CreateRequest(HostPortPair(host, 80), pac_file_fetcher_->isolation_info().network_anonymization_key(), net_log_, parameters);

    CompletionRepeatingCallback callback = base::BindRepeating(&PacFileDecider::OnIOCompletion, base::Unretained(this));

    next_state_ = STATE_QUICK_CHECK_COMPLETE;
    quick_check_timer_.Start(FROM_HERE, base::Milliseconds(kQuickCheckDelayMs), base::BindOnce(callback, ERR_NAME_NOT_RESOLVED));
...
int PacFileDecider::DoFetchPacScript()
{
    ...
    net_log_.BeginEvent(NetLogEventType::PAC_FILE_DECIDER_FETCH_PAC_SCRIPT, [&] { return pac_source.NetLogParams(effective_pac_url); });
    ...
    if (pac_source.type == PacSource::WPAD_DHCP) {
        ...
        return dhcp_pac_file_fetcher_->Fetch(
            &pac_script_, base::BindOnce(&PacFileDecider::OnIOCompletion, base::Unretained(this)), net_log_, NetworkTrafficAnnotationTag(traffic_annotation_));
    }
    ...
    return pac_file_fetcher_->Fetch(effective_pac_url, &pac_script_, base::BindOnce(&PacFileDecider::OnIOCompletion, base::Unretained(this)),
...
int PacFileDecider::DoFetchPacScriptComplete(int result)
{
    ...
    net_log_.EndEventWithNetErrorCode(NetLogEventType::PAC_FILE_DECIDER_FETCH_PAC_SCRIPT, result);
    if (result != OK)
        return TryToFallbackPacSource(result);
...
int PacFileDecider::DoVerifyPacScriptComplete(int result)
{
    ...
    if (fetch_pac_bytes_) {
        ...
        script_data_.data = pac_source.type == PacSource::CUSTOM ? PacFileData::FromURL(pac_source.url) : PacFileData::ForAutoDetect();
    }
    ...
    effective_config_ = ProxyConfigWithAnnotation(config, net::NetworkTrafficAnnotationTag(traffic_annotation_));
...
int PacFileDecider::TryToFallbackPacSource(int error)
{
    ...
    net_log_.AddEvent(NetLogEventType::PAC_FILE_DECIDER_FALLING_BACK_TO_NEXT_PAC_SOURCE);
    if (quick_check_enabled_ && current_pac_source().type == PacSource::WPAD_DNS)
        next_state_ = STATE_QUICK_CHECK;
    else
        next_state_ = GetStartState();
...
void PacFileDecider::DetermineURL(const PacSource& pac_source, GURL* effective_pac_url)
{
