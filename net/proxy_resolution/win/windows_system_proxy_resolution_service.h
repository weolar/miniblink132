...
class NET_EXPORT WindowsSystemProxyResolutionService : public ProxyResolutionService {
public:
    ...
    WindowsSystemProxyResolutionService(const WindowsSystemProxyResolutionService&) = delete;
    WindowsSystemProxyResolutionService& operator=(const WindowsSystemProxyResolutionService&) = delete;
...
private:
    ...
    WindowsSystemProxyResolutionService(std::unique_ptr<WindowsSystemProxyResolver> windows_system_proxy_resolver, NetLog* net_log);

    typedef std::set<raw_ptr<WindowsSystemProxyResolutionRequest, SetExperimental>> PendingRequests;

    [[nodiscard]] bool ContainsPendingRequest(WindowsSystemProxyResolutionRequest* req);
    ...
    int DidFinishResolvingProxy(const GURL& url, const std::string& method, ProxyInfo* result, WinHttpStatus winhttp_status, const NetLogWithSource& net_log);
