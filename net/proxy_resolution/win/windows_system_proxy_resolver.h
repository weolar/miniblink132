...
public:
    ...
    WindowsSystemProxyResolver& operator=(const WindowsSystemProxyResolver&) = delete;
    virtual ~WindowsSystemProxyResolver() = default;

    // Asynchronously finds a proxy for `url`. The `callback_target` must outlive
    // `this`.
    virtual std::unique_ptr<Request> GetProxyForUrl(const GURL& url, WindowsSystemProxyResolutionRequest* callback_target) = 0;
};
