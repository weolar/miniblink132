...
public:
    explicit MockDhcpPacFileAdapterFetcher(URLRequestContext* context, scoped_refptr<base::TaskRunner> task_runner)
    ...
    std::unique_ptr<PacFileFetcher> ImplCreateScriptFetcher() override
    {
        ...
        if (fetcher_delay_ms_ != -1) {
            fetcher_timer_.Start(FROM_HERE, base::Milliseconds(fetcher_delay_ms_), this, &MockDhcpPacFileAdapterFetcher::OnFetcherTimer);
        }
        return fetcher;
    }
    ...
    public:
        explicit DelayingDhcpQuery()
            : DhcpQuery()
            , test_finished_event_(base::WaitableEvent::ResetPolicy::MANUAL, base::WaitableEvent::InitialState::NOT_SIGNALED)
        {
        }
    ...
    base::TimeDelta ImplGetTimeout() const override
    {
        return timeout_;
    }
...
public:
    FetcherClient()
        ...
        , fetcher_(std::make_unique<MockDhcpPacFileAdapterFetcher>(
              url_request_context_.get(), base::ThreadPool::CreateSequencedTaskRunner({ base::MayBlock(), base::TaskShutdownBehavior::CONTINUE_ON_SHUTDOWN })))
    {
    }
    ...
    void RunTest()
    {
        fetcher_->Fetch("adapter name", callback_.callback(), TRAFFIC_ANNOTATION_FOR_TESTS);
    }
...
class MockDhcpRealFetchPacFileAdapterFetcher : public MockDhcpPacFileAdapterFetcher {
public:
    explicit MockDhcpRealFetchPacFileAdapterFetcher(URLRequestContext* context, scoped_refptr<base::TaskRunner> task_runner)
...
TEST(DhcpPacFileAdapterFetcher, MockDhcpRealFetch)
{
    ...
    test_server.ServeFilesFromSourceDirectory("net/data/pac_file_fetcher_unittest");
    ...
    client.fetcher_ = std::make_unique<MockDhcpRealFetchPacFileAdapterFetcher>(
        client.url_request_context(), base::ThreadPool::CreateTaskRunner({ base::MayBlock(), base::TaskShutdownBehavior::CONTINUE_ON_SHUTDOWN }));
...
TEST(DhcpPacFileAdapterFetcher, SanitizeDhcpApiString)
{
    ...
    // Default case.
    EXPECT_EQ(BASE_URL, DhcpPacFileAdapterFetcher::SanitizeDhcpApiString(BASE_URL, kBaseUrlLen));
    ...
    // Embedded NULLs.
    EXPECT_EQ(BASE_URL, DhcpPacFileAdapterFetcher::SanitizeDhcpApiString(BASE_URL "\0foo\0blat", kBaseUrlLen + 9));
}
