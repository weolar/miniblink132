...
public:
    ...
    int GetProxyForURL(const GURL& url, const NetworkAnonymizationKey& network_anymization_key, ProxyInfo* results, CompletionOnceCallback /*callback*/,
...
ProxyResolverWinHttp::ProxyResolverWinHttp(const scoped_refptr<PacFileData>& script_data)
    : pac_url_(script_data->type() == PacFileData::TYPE_AUTO_DETECT ? GURL("http://wpad/wpad.dat") : script_data->url())
{
}
...
int ProxyResolverWinHttp::GetProxyForURL(const GURL& query_url, const NetworkAnonymizationKey& network_anonymization_key, ProxyInfo* results,
    CompletionOnceCallback /*callback*/, std::unique_ptr<Request>* /*request*/, const NetLogWithSource& /*net_log*/)
{
    ...
    BOOL ok = WinHttpGetProxyForUrl(session_handle_, base::as_wcstr(base::ASCIIToUTF16(mutable_query_url.spec())), &options, &info);
    if (!ok) {
        if (ERROR_WINHTTP_LOGIN_FAILURE == GetLastError()) {
            options.fAutoLogonIfChallenged = TRUE;
            ok = WinHttpGetProxyForUrl(session_handle_, base::as_wcstr(base::ASCIIToUTF16(mutable_query_url.spec())), &options, &info);
        }
        if (!ok) {
...
{
    DCHECK(!session_handle_);
    session_handle_ = WinHttpOpen(nullptr, WINHTTP_ACCESS_TYPE_NO_PROXY, WINHTTP_NO_PROXY_NAME, WINHTTP_NO_PROXY_BYPASS, 0);
    if (!session_handle_)
        return false;
