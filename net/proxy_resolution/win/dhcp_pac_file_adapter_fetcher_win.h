...
public:
    ...
    DhcpPacFileAdapterFetcher(URLRequestContext* url_request_context, scoped_refptr<base::TaskRunner> task_runner);

    DhcpPacFileAdapterFetcher(const DhcpPacFileAdapterFetcher&) = delete;
    DhcpPacFileAdapterFetcher& operator=(const DhcpPacFileAdapterFetcher&) = delete;
    ...
    virtual void Fetch(const std::string& adapter_name, CompletionOnceCallback callback, const NetworkTrafficAnnotationTag traffic_annotation);
    ...
    static std::string SanitizeDhcpApiString(const char* data, size_t count_bytes);
...
protected:
    ...
    class NET_EXPORT_PRIVATE DhcpQuery : public base::RefCountedThreadSafe<DhcpQuery> {
    public:
        DhcpQuery();
...
private:
    ...
    void OnDhcpQueryDone(scoped_refptr<DhcpQuery> dhcp_query, const NetworkTrafficAnnotationTag traffic_annotation);
    void OnTimeout();
