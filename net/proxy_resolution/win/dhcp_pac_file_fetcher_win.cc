...
struct DhcpAdapterNamesLoggingInfo {
    ...
    DhcpAdapterNamesLoggingInfo& operator=(const DhcpAdapterNamesLoggingInfo&) = delete;
...
public:
    ...
    bool PostDelayedTask(const base::Location& from_here, base::OnceClosure task, base::TimeDelta delay) override
    {
        ...
        base::OnceClosure wrapped_task = base::BindOnce(&TaskRunnerWithCap::RunTaskAndSchedulePendingTask, this, std::move(task));
...
private:
    ...
    void RunTaskAndSchedulePendingTask(base::OnceClosure task)
    {
        ...
        task_runner_->PostTask(task_to_schedule.from_here, std::move(task_to_schedule.task));
    }

    const scoped_refptr<base::TaskRunner> task_runner_
        = base::ThreadPool::CreateTaskRunner({ base::MayBlock(), base::TaskShutdownBehavior::CONTINUE_ON_SHUTDOWN, base::TaskPriority::USER_VISIBLE });
...
base::Value::Dict NetLogGetAdaptersDoneParams(DhcpAdapterNamesLoggingInfo* info)
{
    ...
    for (IP_ADAPTER_ADDRESSES* adapter = info->adapters.get(); adapter; adapter = adapter->Next) {
    ...
    result.Set("origin_to_worker_thread_hop_dt", static_cast<int>((info->worker_thread_start_time - info->origin_thread_start_time).InMilliseconds()));
    result.Set("worker_to_origin_thread_hop_dt", static_cast<int>((info->origin_thread_end_time - info->worker_thread_end_time).InMilliseconds()));
    result.Set("worker_dt", static_cast<int>((info->worker_thread_end_time - info->worker_thread_start_time).InMilliseconds()));
...
DhcpPacFileFetcherWin::DhcpPacFileFetcherWin(URLRequestContext* url_request_context)
    ...
    std::u16string* utf16_text, CompletionOnceCallback callback, const NetLogWithSource& net_log, const NetworkTrafficAnnotationTag traffic_annotation)
{
    ...
    last_query_->logging_info()->origin_thread_start_time = base::TimeTicks::Now();

    task_runner_->PostTaskAndReply(FROM_HERE, base::BindOnce(&DhcpPacFileFetcherWin::AdapterQuery::GetCandidateAdapterNames, last_query_.get()),
        base::BindOnce(&DhcpPacFileFetcherWin::OnGetCandidateAdapterNamesDone, weak_ptr_factory_.GetWeakPtr(), last_query_, traffic_annotation));
...
void DhcpPacFileFetcherWin::CancelImpl()
{
    ...
    if (state_ != STATE_DONE) {
        ...
        for (FetcherVector::iterator it = fetchers_.begin(); it != fetchers_.end(); ++it) {
            (*it)->Cancel();
        }
...
void DhcpPacFileFetcherWin::OnGetCandidateAdapterNamesDone(scoped_refptr<AdapterQuery> query, const NetworkTrafficAnnotationTag traffic_annotation)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    net_log_.EndEvent(NetLogEventType::WPAD_DHCP_WIN_GET_ADAPTERS, [&] { return NetLogGetAdaptersDoneParams(logging_info); });
    ...
    for (const std::string& adapter_name : adapter_names) {
        std::unique_ptr<DhcpPacFileAdapterFetcher> fetcher(ImplCreateAdapterFetcher());
        size_t fetcher_index = fetchers_.size();
        fetcher->Fetch(adapter_name, base::BindOnce(&DhcpPacFileFetcherWin::OnFetcherDone, base::Unretained(this), fetcher_index), traffic_annotation);
...
void DhcpPacFileFetcherWin::OnFetcherDone(size_t fetcher_index, int result)
{
    DCHECK(state_ == STATE_NO_RESULTS || state_ == STATE_SOME_RESULTS);

    net_log_.AddEvent(NetLogEventType::WPAD_DHCP_WIN_ON_FETCHER_DONE, [&] { return NetLogFetcherDoneParams(fetcher_index, result); });
    ...
    if (state_ == STATE_NO_RESULTS) {
        ...
        wait_timer_.Start(FROM_HERE, ImplGetMaxWait(), this, &DhcpPacFileFetcherWin::OnWaitTimer);
    }
}
...
void DhcpPacFileFetcherWin::TransitionToDone()
{
    ...
    net_log_.EndEvent(NetLogEventType::WPAD_DHCP_WIN_FETCH, [&] { return NetLogFetcherDoneParams(used_fetcher_index, result); });
...
std::unique_ptr<DhcpPacFileAdapterFetcher> DhcpPacFileFetcherWin::ImplCreateAdapterFetcher()
{
    return std::make_unique<DhcpPacFileAdapterFetcher>(url_request_context_, task_runner_);
}

scoped_refptr<DhcpPacFileFetcherWin::AdapterQuery> DhcpPacFileFetcherWin::ImplCreateAdapterQuery()
{
...
bool DhcpPacFileFetcherWin::GetCandidateAdapterNames(std::set<std::string>* adapter_names, DhcpAdapterNamesLoggingInfo* info)
{
...
const std::set<std::string>& DhcpPacFileFetcherWin::AdapterQuery::adapter_names() const
{
    return adapter_names_;
}

bool DhcpPacFileFetcherWin::AdapterQuery::ImplGetCandidateAdapterNames(std::set<std::string>* adapter_names, DhcpAdapterNamesLoggingInfo* info)
{
    return DhcpPacFileFetcherWin::GetCandidateAdapterNames(adapter_names, info);
}
