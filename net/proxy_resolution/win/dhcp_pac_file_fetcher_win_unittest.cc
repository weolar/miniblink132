...
public:
        ...
        , fetcher_(std::make_unique<DhcpPacFileFetcherWin>(context_.get()))
    {
        ...
        timeout_.Start(FROM_HERE, base::Seconds(5), this, &RealFetchTester::OnTimeout);
    }

    void RunTest()
    {
        int result = fetcher_->Fetch(
            &pac_text_, base::BindOnce(&RealFetchTester::OnCompletion, base::Unretained(this)), NetLogWithSource(), TRAFFIC_ANNOTATION_FOR_TESTS);
    ...
    void RunTestWithDeferredCancel()
    {
        ...
        cancel_timer_.Start(FROM_HERE, base::Milliseconds(0), this, &RealFetchTester::OnCancelTimer);
        RunTest();
    }
...
public:
    DelayingDhcpPacFileAdapterFetcher(URLRequestContext* url_request_context, scoped_refptr<base::TaskRunner> task_runner)
...
public:
    ...
    std::unique_ptr<DhcpPacFileAdapterFetcher> ImplCreateAdapterFetcher() override
    {
        return std::make_unique<DelayingDhcpPacFileAdapterFetcher>(url_request_context(), GetTaskRunner());
    }
};
...
TEST(DhcpPacFileFetcherWin, RealFetchWithDeferredCancel)
{
    ...
    RealFetchTester fetcher;
    fetcher.fetcher_ = std::make_unique<DelayingDhcpPacFileFetcherWin>(fetcher.context_.get());
...
public:
    DummyDhcpPacFileAdapterFetcher(URLRequestContext* context, scoped_refptr<base::TaskRunner> runner)
        : DhcpPacFileAdapterFetcher(context, runner)
        , pac_script_(u"bingo")
    {
    }

    void Fetch(const std::string& adapter_name, CompletionOnceCallback callback, const NetworkTrafficAnnotationTag traffic_annotation) override
    {
        callback_ = std::move(callback);
        timer_.Start(FROM_HERE, base::Milliseconds(fetch_delay_ms_), this, &DummyDhcpPacFileAdapterFetcher::OnTimer);
    }
    ...
    std::u16string GetPacScript() const override
    {
        return pac_script_;
    }
    ...
    void Configure(bool did_finish, int result, std::u16string pac_script, int fetch_delay_ms)
    {
...
public:
    ...
    public:
        ...
        {
            adapter_names->insert(mock_adapter_names_.begin(), mock_adapter_names_.end());
            return true;
        }
    ...
    MockDhcpPacFileFetcherWin(URLRequestContext* context)
        : DhcpPacFileFetcherWin(context)
        , worker_finished_event_(base::WaitableEvent::ResetPolicy::MANUAL, base::WaitableEvent::InitialState::NOT_SIGNALED)
    {
        ResetTestState();
    }

    ~MockDhcpPacFileFetcherWin() override
    {
        ResetTestState();
    }
    ...
    void PushBackAdapter(const std::string& adapter_name, std::unique_ptr<DhcpPacFileAdapterFetcher> fetcher)
    {
        adapter_query_->mock_adapter_names_.push_back(adapter_name);
        adapter_fetchers_.push_back(std::move(fetcher));
    }

    void ConfigureAndPushBackAdapter(const std::string& adapter_name, bool did_finish, int result, std::u16string pac_script, base::TimeDelta fetch_delay)
    {
        auto adapter_fetcher = std::make_unique<DummyDhcpPacFileAdapterFetcher>(url_request_context(), GetTaskRunner());
        adapter_fetcher->Configure(did_finish, result, pac_script, fetch_delay.InMilliseconds());
...
public:
    ...
    {
        int result = fetcher_.Fetch(
            &pac_text_, base::BindOnce(&FetcherClient::OnCompletion, base::Unretained(this)), NetLogWithSource(), TRAFFIC_ANNOTATION_FOR_TESTS);
...
{
    auto adapter_fetcher = std::make_unique<DummyDhcpPacFileAdapterFetcher>(client->context(), client->GetTaskRunner());
...
{
    client->fetcher_.ConfigureAndPushBackAdapter("most_preferred", true, ERR_PAC_NOT_IN_DHCP, std::u16string(), base::Milliseconds(1));
    client->fetcher_.ConfigureAndPushBackAdapter("second", true, OK, u"bingo", base::Milliseconds(50));
    client->fetcher_.ConfigureAndPushBackAdapter("third", true, OK, u"rocko", base::Milliseconds(1));
    client->RunTest();
    client->RunMessageLoopUntilComplete();
    ASSERT_THAT(client->result_, IsOk());
    ASSERT_EQ(u"bingo", client->pac_text_);
...
void TestNormalCaseURLConfiguredMultipleAdaptersWithTimeout(FetcherClient* client)
{
    client->fetcher_.ConfigureAndPushBackAdapter("most_preferred", true, ERR_PAC_NOT_IN_DHCP, std::u16string(), base::Milliseconds(1));
    // This will time out.
    client->fetcher_.ConfigureAndPushBackAdapter("second", false, ERR_IO_PENDING, u"bingo", TestTimeouts::action_timeout());
    client->fetcher_.ConfigureAndPushBackAdapter("third", true, OK, u"rocko", base::Milliseconds(1));
    client->RunTest();
    client->RunMessageLoopUntilComplete();
    ASSERT_THAT(client->result_, IsOk());
...
TEST(DhcpPacFileFetcherWin, NormalCaseURLConfiguredMultipleAdaptersWithTimeout)
{
    base::test::TaskEnvironment task_environment;
...
void TestFailureCaseURLConfiguredMultipleAdaptersWithTimeout(FetcherClient* client)
{
    client->fetcher_.ConfigureAndPushBackAdapter("most_preferred", true, ERR_PAC_NOT_IN_DHCP, std::u16string(), base::Milliseconds(1));
    // This will time out.
    client->fetcher_.ConfigureAndPushBackAdapter("second", false, ERR_IO_PENDING, u"bingo", TestTimeouts::action_timeout());
    ...
    // should be chosen.
    client->fetcher_.ConfigureAndPushBackAdapter("third", true, ERR_HTTP_RESPONSE_CODE_FAILURE, std::u16string(), base::Milliseconds(1));
    client->fetcher_.ConfigureAndPushBackAdapter("fourth", true, ERR_NOT_IMPLEMENTED, std::u16string(), base::Milliseconds(1));
    client->RunTest();
    client->RunMessageLoopUntilComplete();
...
TEST(DhcpPacFileFetcherWin, FailureCaseURLConfiguredMultipleAdaptersWithTimeout)
{
    base::test::TaskEnvironment task_environment;
...
void TestFailureCaseNoURLConfigured(FetcherClient* client)
{
    ...
    // This is the first non-ERR_PAC_NOT_IN_DHCP error and as such
    // should be chosen.
    client->fetcher_.ConfigureAndPushBackAdapter("third", true, ERR_PAC_NOT_IN_DHCP, std::u16string(), base::Milliseconds(1));
    client->RunTest();
    client->RunMessageLoopUntilComplete();
...
void TestShortCircuitLessPreferredAdapters(FetcherClient* client)
{
    ...
    client->fetcher_.ConfigureAndPushBackAdapter("1", true, ERR_PAC_NOT_IN_DHCP, std::u16string(), base::Milliseconds(1));
    client->fetcher_.ConfigureAndPushBackAdapter("2", true, OK, u"bingo", base::Milliseconds(1));
    client->fetcher_.ConfigureAndPushBackAdapter("3", true, OK, u"wrongo", TestTimeouts::action_max_timeout());
    ...
    // more adapters.
    ASSERT_GT(client->fetcher_.max_wait_ - (client->fetcher_.max_wait_ / 10), timer.Elapsed());
}
...
TEST(DhcpPacFileFetcherWin, ReuseFetcher)
{
    ...
    for (TestVector::const_iterator it = test_functions.begin(); it != test_functions.end(); ++it) {
        (*it)(&client);
