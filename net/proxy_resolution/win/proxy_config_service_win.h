...
class NET_EXPORT_PRIVATE ProxyConfigServiceWin : public PollingProxyConfigService, public NetworkChangeNotifier::NetworkChangeObserver {
public:
...
private:
    ...
    static void GetCurrentProxyConfig(const NetworkTrafficAnnotationTag traffic_annotation, ProxyConfigWithAnnotation* config);

    // Set |config| using the proxy configuration values of |ie_config|.
    static void SetFromIEConfig(ProxyConfig* config, const WINHTTP_CURRENT_USER_IE_PROXY_CONFIG& ie_config);
