...
ProxyConfigServiceWin::ProxyConfigServiceWin(const NetworkTrafficAnnotationTag& traffic_annotation)
    : PollingProxyConfigService(base::Seconds(kPollIntervalSec), base::BindRepeating(&ProxyConfigServiceWin::GetCurrentProxyConfig), traffic_annotation)
{
...
void ProxyConfigServiceWin::OnNetworkChanged(NetworkChangeNotifier::ConnectionType type)
{
...
void ProxyConfigServiceWin::StartWatchingRegistryForChanges()
{
    ...
    AddKeyToWatchList(HKEY_CURRENT_USER, L"Software\\Microsoft\\Windows\\CurrentVersion\\Internet Settings");

    AddKeyToWatchList(HKEY_LOCAL_MACHINE, L"Software\\Microsoft\\Windows\\CurrentVersion\\Internet Settings");
...
bool ProxyConfigServiceWin::AddKeyToWatchList(HKEY rootkey, const wchar_t* subkey)
{
    std::unique_ptr<base::win::RegKey> key = std::make_unique<base::win::RegKey>();
    if (key->Create(rootkey, subkey, KEY_NOTIFY) != ERROR_SUCCESS) {
        return false;
    }

    if (!key->StartWatching(base::BindOnce(&ProxyConfigServiceWin::OnObjectSignaled, base::Unretained(this), base::Unretained(key.get())))) {
        return false;
    }
...
void ProxyConfigServiceWin::OnObjectSignaled(base::win::RegKey* key)
{
    ...
    auto it = base::ranges::find(keys_to_watch_, key, &std::unique_ptr<base::win::RegKey>::get);
    CHECK(it != keys_to_watch_.end(), base::NotFatalUntil::M130);

    // Keep watching the registry key.
    if (!key->StartWatching(base::BindOnce(&ProxyConfigServiceWin::OnObjectSignaled, base::Unretained(this), base::Unretained(key)))) {
...
// static
void ProxyConfigServiceWin::GetCurrentProxyConfig(const NetworkTrafficAnnotationTag traffic_annotation, ProxyConfigWithAnnotation* config)
{
...
// static
void ProxyConfigServiceWin::SetFromIEConfig(ProxyConfig* config, const WINHTTP_CURRENT_USER_IE_PROXY_CONFIG& ie_config)
{
    ...
    if (ie_config.lpszProxy) {
        ...
        config->proxy_rules().ParseFromString(base::WideToUTF8(ie_config.lpszProxy));
    }
    if (ie_config.lpszProxyBypass) {
