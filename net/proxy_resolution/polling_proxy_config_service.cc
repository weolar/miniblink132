...
class PollingProxyConfigService::Core : public base::RefCountedThreadSafe<PollingProxyConfigService::Core> {
public:
    Core(base::TimeDelta poll_interval, GetConfigFunction get_config_func, const NetworkTrafficAnnotationTag& traffic_annotation)
    ...
    void CheckForChangesNow()
    {
        ...
        base::ThreadPool::PostTask(
            FROM_HERE, { base::MayBlock(), base::TaskShutdownBehavior::CONTINUE_ON_SHUTDOWN }, base::BindOnce(&Core::PollAsync, this, get_config_func_));
    }
...
private:
    ...
    void PollAsync(GetConfigFunction func)
    {
        ...
        if (origin_task_runner_.get()) {
            origin_task_runner_->PostTask(FROM_HERE, base::BindOnce(&Core::GetConfigCompleted, this, config));
        }
    }
...
ProxyConfigService::ConfigAvailability PollingProxyConfigService::GetLatestProxyConfig(ProxyConfigWithAnnotation* config)
{
...
PollingProxyConfigService::PollingProxyConfigService(
    base::TimeDelta poll_interval, GetConfigFunction get_config_func, const NetworkTrafficAnnotationTag& traffic_annotation)
    : core_(base::MakeRefCounted<Core>(poll_interval, get_config_func, traffic_annotation))
{
}
