...
ConfiguredProxyResolutionRequest::ConfiguredProxyResolutionRequest(ConfiguredProxyResolutionService* service, const GURL& url, const std::string& method,
    const NetworkAnonymizationKey& network_anonymization_key, ProxyInfo* results, CompletionOnceCallback user_callback, const NetLogWithSource& net_log)
    : service_(service)
...
int ConfiguredProxyResolutionRequest::Start()
{
    ...
    traffic_annotation_ = MutableNetworkTrafficAnnotationTag(service_->config_->traffic_annotation());

    if (service_->ApplyPacBypassRules(url_, results_))
        return OK;

    return service_->GetProxyResolver()->GetProxyForURL(url_, network_anonymization_key_, results_,
        base::BindOnce(&ConfiguredProxyResolutionRequest::QueryComplete, base::Unretained(this)), &resolve_job_, net_log_);
}

void ConfiguredProxyResolutionRequest::StartAndCompleteCheckingForSynchronous()
{
...
int ConfiguredProxyResolutionRequest::QueryDidComplete(int result_code)
{
    ...
    int rv = service_->DidFinishResolvingProxy(url_, network_anonymization_key_, method_, results_, result_code, net_log_);
...
int ConfiguredProxyResolutionRequest::QueryDidCompleteSynchronously(int result_code)
{
