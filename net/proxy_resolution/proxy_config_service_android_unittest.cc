...
public:
    ...
    const ProxyConfigWithAnnotation& config() const
    {
        return config_;
    }
...
public:
    JavaLooperPreparer()
    {
        Java_AndroidProxyConfigServiceTestUtil_prepareLooper(base::android::AttachCurrentThread());
    }
};
...
protected:
    ...
    explicit ProxyConfigServiceAndroidTestBase(const StringMap& initial_configuration)
        : configuration_(initial_configuration)
        , service_(base::SingleThreadTaskRunner::GetCurrentDefault(), base::SingleThreadTaskRunner::GetCurrentDefault(),
              base::BindRepeating(&ProxyConfigServiceAndroidTestBase::GetProperty, base::Unretained(this)))
    {
    }
    ...
    void SetProxyOverride(const ProxyConfigServiceAndroid::ProxyOverrideRule& rule, const std::vector<std::string>& bypass_rules, const bool reverse_bypass,
    ...
    void SetProxyOverride(const std::vector<ProxyConfigServiceAndroid::ProxyOverrideRule>& rules, const std::vector<std::string>& bypass_rules,
        const bool reverse_bypass, base::OnceClosure callback)
    {
        service_.SetProxyOverride(rules, bypass_rules, reverse_bypass, std::move(callback));
        base::RunLoop().RunUntilIdle();
    }
...
class ProxyConfigServiceAndroidWithInitialConfigTest : public ProxyConfigServiceAndroidTestBase {
public:
...
TEST_F(ProxyConfigServiceAndroidTest, TestProxyOverrideSchemes)
{
    ...
    SetProxyOverride({ "*", "httpoverrideproxy.com:200" }, bypass_rules, false, base::DoNothing());
    ...
    SetProxyOverride({ "https", "httpoverrideproxy.com:200" }, bypass_rules, false, base::DoNothing());
    TestMapping("http://example.com/", "DIRECT");
    TestMapping("https://example.com/", "PROXY httpoverrideproxy.com:200");
    TestMapping("ftp://example.com/", "DIRECT");
...
TEST_F(ProxyConfigServiceAndroidTest, TestProxyOverridePorts)
{
    ...
    SetProxyOverride({ "*", "httpoverrideproxy.com" }, bypass_rules, false, base::DoNothing());
    ...
    SetProxyOverride({ "*", "https://httpoverrideproxy.com" }, bypass_rules, false, base::DoNothing());
    ...
    SetProxyOverride({ "*", "https://httpoverrideproxy.com:777" }, bypass_rules, false, base::DoNothing());
...
TEST_F(ProxyConfigServiceAndroidTest, TestProxyOverrideListOfRules)
{
    ...
    TestMapping("https://example.com", "SOCKS5 fallback1:1080;PROXY fallback2:80;DIRECT");
}
...
TEST_F(ProxyConfigServiceAndroidTest, TestReverseBypass)
{
    ...
    SetProxyOverride({ "*", "httpoverrideproxy.com:200" }, bypass_rules, true, base::DoNothing());
