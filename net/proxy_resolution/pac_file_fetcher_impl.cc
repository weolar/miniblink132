...
constexpr bool IsPacMimeType(std::string_view mime_type)
{
    ...
    return base::ranges::any_of(kSupportedPacMimeTypes, [&](auto pac_mime_type) { return base::EqualsCaseInsensitiveASCII(pac_mime_type, mime_type); });
}
...
void ConvertResponseToUTF16(const std::string& charset, const std::string& bytes, std::u16string* utf16)
{
    if (charset.empty()) {
        ...
        for (const auto& bom : kBomMappings) {
            if (bytes_str.starts_with(bom.prefix)) {
                return ConvertResponseToUTF16(bom.charset,
...
std::unique_ptr<PacFileFetcherImpl> PacFileFetcherImpl::Create(URLRequestContext* url_request_context)
{
...
base::TimeDelta PacFileFetcherImpl::SetTimeoutConstraint(base::TimeDelta timeout)
{
...
void PacFileFetcherImpl::OnResponseCompleted(URLRequest* request, int net_error)
{
...
int PacFileFetcherImpl::Fetch(const GURL& url, std::u16string* text, CompletionOnceCallback callback, const NetworkTrafficAnnotationTag traffic_annotation)
{
    ...
    cur_request_ = url_request_context_->CreateRequest(url, MAXIMUM_PRIORITY, this, traffic_annotation);
    ...
    cur_request_->SetLoadFlags(LOAD_BYPASS_PROXY | LOAD_DISABLE_CACHE | LOAD_DISABLE_CERT_NETWORK_FETCHES | LOAD_IGNORE_LIMITS);
    ...
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(
        FROM_HERE, base::BindOnce(&PacFileFetcherImpl::OnTimeout, weak_factory_.GetWeakPtr(), cur_request_id_), max_duration_);
...
void PacFileFetcherImpl::OnReceivedRedirect(URLRequest* request, const RedirectInfo& redirect_info, bool* defer_redirect)
{
    int error = OK;
...
void PacFileFetcherImpl::OnAuthRequired(URLRequest* request, const AuthChallengeInfo& auth_info)
{
    DCHECK_EQ(request, cur_request_.get());
...
void PacFileFetcherImpl::OnSSLCertificateError(URLRequest* request, int net_error, const SSLInfo& ssl_info, bool fatal)
{
    DCHECK_EQ(request, cur_request_.get());
...
void PacFileFetcherImpl::OnResponseStarted(URLRequest* request, int net_error)
{
    ...
    if (request->url().SchemeIsHTTPOrHTTPS()) {
        ...
        if (request->GetResponseCode() != 200) {
            VLOG(1) << "Fetched PAC script had (bad) status line: " << request->response_headers()->GetStatusLine();
        ...
        if (!IsPacMimeType(mime_type)) {
            VLOG(1) << "Fetched PAC script does not have a proper mime type: " << mime_type;
        }
    }
...
void PacFileFetcherImpl::FetchCompleted()
{
    if (result_code_ == OK) {
        ...
        DEPRECATED_UMA_HISTOGRAM_MEDIUM_TIMES("Net.ProxyScriptFetcher.FirstByteDuration", fetch_time_to_first_byte_ - fetch_start_time_);
