...
void ExpectProxyServerEquals(const char* expectation, const ProxyList& proxy_list)
{
...
TEST(ProxyConfigTest, Equals)
{
    ...
    config2.proxy_rules().single_proxies.SetSingleProxyServer(ProxyUriToProxyServer("myproxy:80", ProxyServer::SCHEME_HTTP));

    EXPECT_FALSE(config1.Equals(config2));
    EXPECT_FALSE(config2.Equals(config1));

    config1.proxy_rules().type = ProxyConfig::ProxyRules::Type::PROXY_LIST;
    config1.proxy_rules().single_proxies.SetSingleProxyServer(ProxyUriToProxyServer("myproxy:100", ProxyServer::SCHEME_HTTP));

    EXPECT_FALSE(config1.Equals(config2));
    EXPECT_FALSE(config2.Equals(config1));

    config1.proxy_rules().single_proxies.SetSingleProxyServer(ProxyUriToProxyServer("myproxy", ProxyServer::SCHEME_HTTP));
...
TEST(ProxyConfigTest, EqualsMultiProxyChains)
{
    ...
    config2.proxy_rules().type = ProxyConfig::ProxyRules::Type::PROXY_LIST_PER_SCHEME;
    config2.proxy_rules().proxies_for_https.SetSingleProxyChain(MultiProxyUrisToProxyChain("[https://foopy:443 https://hoopy:443]", ProxyServer::SCHEME_HTTPS));

    EXPECT_FALSE(config1.Equals(config2));
    EXPECT_FALSE(config2.Equals(config1));

    config1.proxy_rules().type = ProxyConfig::ProxyRules::Type::PROXY_LIST_PER_SCHEME;
    config1.proxy_rules().proxies_for_https.SetSingleProxyChain(MultiProxyUrisToProxyChain("[https://foopy:80 https://hoopy:80]", ProxyServer::SCHEME_HTTPS));

    EXPECT_FALSE(config1.Equals(config2));
    EXPECT_FALSE(config2.Equals(config1));

    config1.proxy_rules().proxies_for_https.SetSingleProxyChain(MultiProxyUrisToProxyChain("[https://foopy https://hoopy]", ProxyServer::SCHEME_HTTPS));
...
class ProxyConfigToValueTest : public ::testing::TestWithParam<ProxyConfigToValueTestCase> { };
...
ProxyConfigToValueTestCase GetTestCasePacUrl()
{
    ...
    return { std::move(config), "{\"pac_url\":\"http://www.example.com/test.pac\"}" };
}
...
ProxyConfigToValueTestCase GetTestCasePacUrlAndAutoDetect()
{
    ...
    return { std::move(config), "{\"auto_detect\":true,\"pac_url\":\"http://www.example.com/test.pac\"}" };
}
...
{
    ProxyConfig config;
    config.proxy_rules().ParseFromString("http=https://proxy1:8080;https=socks5://proxy2");
    config.proxy_rules().bypass_rules.AddRuleFromString("*.google.com");
...
{
    ProxyConfig config;
    config.proxy_rules().ParseFromString("https://proxy1:8080,http://proxy2,direct://");
...
{
    ProxyConfig config;
    config.proxy_rules().ParseFromString("http=[https://proxy1:8080 https://proxy2:8080];https=socks5://proxy2",
...
INSTANTIATE_TEST_SUITE_P(All, ProxyConfigToValueTest,
    testing::Values(GetTestCaseDirect(), GetTestCaseAutoDetect(), GetTestCasePacUrl(), GetTestCasePacUrlMandatory(), GetTestCasePacUrlAndAutoDetect(),
        GetTestCaseSingleProxy(), GetTestCaseSingleProxyWithBypass(), GetTestCaseSingleProxyWithReversedBypass(), GetTestCaseProxyPerScheme(),
#if BUILDFLAG(ENABLE_BRACKETED_PROXY_URIS)
...
TEST(ProxyConfigTest, ParseProxyRules)
{
    ...
    } tests[] = { // One HTTP proxy for all schemes.
        {
            "myproxy:80",
    ...
    for (const auto& test : tests) {
        ...
        ExpectProxyServerEquals(test.single_proxy, config.proxy_rules().single_proxies);
        ExpectProxyServerEquals(test.proxy_for_http, config.proxy_rules().proxies_for_http);
        ExpectProxyServerEquals(test.proxy_for_https, config.proxy_rules().proxies_for_https);
        ExpectProxyServerEquals(test.proxy_for_ftp, config.proxy_rules().proxies_for_ftp);
        ExpectProxyServerEquals(test.fallback_proxy, config.proxy_rules().fallback_proxies);
    }
}
...
TEST(ProxyConfigTest, ParseProxyRulesQuicIsNotAllowed)
{
    ...
    EXPECT_EQ(ProxyConfig::ProxyRules::Type::PROXY_LIST, config.proxy_rules().type);
...
TEST(ProxyConfigTest, ParseProxyRulesQuicIsAllowed)
{
    ...
    EXPECT_EQ(ProxyConfig::ProxyRules::Type::PROXY_LIST, config.proxy_rules().type);
    ExpectProxyServerEquals("QUIC foopy:443", config.proxy_rules().single_proxies);
    ExpectProxyServerEquals(nullptr, config.proxy_rules().proxies_for_http);
    ExpectProxyServerEquals(nullptr, config.proxy_rules().proxies_for_https);
    ExpectProxyServerEquals(nullptr, config.proxy_rules().proxies_for_ftp);
    ExpectProxyServerEquals(nullptr, config.proxy_rules().fallback_proxies);
...
// crash.
TEST(ProxyConfigTest, ParseProxyRulesDisallowQuicProxySupportIfBuildFlagDisabled)
{
    ProxyConfig config;

    EXPECT_CHECK_DEATH(config.proxy_rules().ParseFromString("quic://foopy:443",
...
TEST(ProxyConfigTest, ParseProxyRulesDisallowMultiProxyChainsInReleaseBuilds)
{
    ...
    EXPECT_CHECK_DEATH(config.proxy_rules().ParseFromString("http=[https://proxy1:8080 https://proxy2:8080]", true));
}
#endif // !BUILDFLAG(ENABLE_BRACKETED_PROXY_URIS)
...
protected:
    ...
    void Apply(const GURL& gurl)
    {
        rules_.Apply(gurl, &info_);
    }
    std::string ToDebugString() const
    {
        return info_.ToDebugString();
    }
...
{
    ParseFromString("http=proxy:3128 ; https=sslproxy:3128 ; socks=socksproxy:1080");
    Apply(WsUrl());
...
{
    ParseFromString("http=proxy:3128 ; https=sslproxy:3128 ; socks=socks5://socksproxy:1080");
    Apply(WsUrl());
...
TEST_F(ProxyConfigWebSocketTest, PrefersNonSocksFallbackOverHttps)
{
    ...
    ParseFromString("http=proxy:3128 ; https=sslproxy:3128; socks=https://httpsproxy");
