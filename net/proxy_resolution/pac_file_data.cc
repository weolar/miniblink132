...
{
    return base::WrapRefCounted(new PacFileData(TYPE_SCRIPT_CONTENTS, GURL(), base::UTF8ToUTF16(utf8)));
}

// static
scoped_refptr<PacFileData> PacFileData::FromUTF16(const std::u16string& utf16)
{
    return base::WrapRefCounted(new PacFileData(TYPE_SCRIPT_CONTENTS, GURL(), utf16));
}

// static
scoped_refptr<PacFileData> PacFileData::FromURL(const GURL& url)
{
    return base::WrapRefCounted(new PacFileData(TYPE_SCRIPT_URL, url, std::u16string()));
}

// static
scoped_refptr<PacFileData> PacFileData::ForAutoDetect()
{
    return base::WrapRefCounted(new PacFileData(TYPE_AUTO_DETECT, GURL(), std::u16string()));
}
...
PacFileData::PacFileData(Type type, const GURL& url, const std::u16string& utf16)
    : type_(type)
    , url_(url)
    , utf16_(utf16)
{
