...
MockAsyncProxyResolver::Job::Job(MockAsyncProxyResolver* resolver, const GURL& url, ProxyInfo* results, CompletionOnceCallback callback)
    : resolver_(resolver)
    , url_(url)
    , results_(results)
...
int MockAsyncProxyResolver::GetProxyForURL(const GURL& url, const NetworkAnonymizationKey& network_anonymization_key, ProxyInfo* results,
...
MockAsyncProxyResolverFactory::Request::Request(MockAsyncProxyResolverFactory* factory, const scoped_refptr<PacFileData>& script_data,
    std::unique_ptr<ProxyResolver>* resolver, CompletionOnceCallback callback)
    : factory_(factory)
...
void MockAsyncProxyResolverFactory::Request::CompleteNow(int rv, std::unique_ptr<ProxyResolver> resolver)
{
...
void MockAsyncProxyResolverFactory::Request::CompleteNowWithForwarder(int rv, ProxyResolver* resolver)
{
    DCHECK(resolver);
...
class MockAsyncProxyResolverFactory::Job : public ProxyResolverFactory::Request {
public:
    explicit Job(const scoped_refptr<MockAsyncProxyResolverFactory::Request>& request)
        : request_(request)
    {
    }
    ~Job() override
...
MockAsyncProxyResolverFactory::MockAsyncProxyResolverFactory(bool resolvers_expect_pac_bytes)
    : ProxyResolverFactory(resolvers_expect_pac_bytes)
{
}

int MockAsyncProxyResolverFactory::CreateProxyResolver(const scoped_refptr<PacFileData>& pac_script, std::unique_ptr<ProxyResolver>* resolver,
    CompletionOnceCallback callback, std::unique_ptr<ProxyResolverFactory::Request>* request_handle)
{
    auto request = base::MakeRefCounted<Request>(this, pac_script, resolver, std::move(callback));
...
int ForwardingProxyResolver::GetProxyForURL(const GURL& query_url, const NetworkAnonymizationKey& network_anonymization_key, ProxyInfo* results,
    CompletionOnceCallback callback, std::unique_ptr<Request>* request, const NetLogWithSource& net_log)
{
    return impl_->GetProxyForURL(query_url, network_anonymization_key, results, std::move(callback), request, net_log);
}
