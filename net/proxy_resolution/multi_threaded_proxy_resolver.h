...
class NET_EXPORT_PRIVATE MultiThreadedProxyResolverFactory : public ProxyResolverFactory {
public:
    MultiThreadedProxyResolverFactory(size_t max_num_threads, bool factory_expects_bytes);
...
private:
    ...
    virtual std::unique_ptr<ProxyResolverFactory> CreateProxyResolverFactory() = 0;
