...
const base::FilePath::CharType kDocRoot[] = FILE_PATH_LITERAL("net/data/pac_file_fetcher_unittest");
...
public:
    PacFileFetcherImplTest()
    {
        ...
        network_delegate_ = builder->set_network_delegate(std::make_unique<BasicNetworkDelegate>());
...
TEST_F(PacFileFetcherImplTest, FileUrlNotAllowed)
{
    ...
    int result = pac_fetcher->Fetch(GetTestFileUrl("pac.txt"), &text, callback.callback(), TRAFFIC_ANNOTATION_FOR_TESTS);
...
TEST_F(PacFileFetcherImplTest, RedirectToFileUrl)
{
    ...
    std::u16string text;
    TestCompletionCallback callback;
    int result = pac_fetcher->Fetch(url, &text, callback.callback(), TRAFFIC_ANNOTATION_FOR_TESTS);
...
TEST_F(PacFileFetcherImplTest, HttpStatusCode)
{
    ...
    { // Fetch a PAC which gives a 500 -- FAIL
        ...
        int result = pac_fetcher->Fetch(url, &text, callback.callback(), TRAFFIC_ANNOTATION_FOR_TESTS);
        EXPECT_THAT(result, IsError(ERR_IO_PENDING));
        EXPECT_THAT(callback.WaitForResult(), IsError(ERR_HTTP_RESPONSE_CODE_FAILURE));
...
TEST_F(PacFileFetcherImplTest, IsolationInfo)
{
    ...
    std::unique_ptr<net::HostResolver::ResolveHostRequest> host_request = context_->host_resolver()->CreateRequest(
        url::SchemeHostPort(url), pac_fetcher->isolation_info().network_anonymization_key(), net::NetLogWithSource(), params);
    ...
    host_request = context_->host_resolver()->CreateRequest(url::SchemeHostPort(url), NetworkAnonymizationKey(), net::NetLogWithSource(), params);
...
TEST_F(PacFileFetcherImplTest, Hang)
{
    ...
    base::TimeDelta prev_timeout = pac_fetcher->SetTimeoutConstraint(base::Milliseconds(500));
...
TEST_F(PacFileFetcherImplTest, IgnoresLimits)
{
    ...
    int num_requests = 2 + ClientSocketPoolManager::max_sockets_per_group(HttpNetworkSession::NORMAL_SOCKET_POOL);

    net::test_server::SimpleConnectionListener connection_listener(num_requests, net::test_server::SimpleConnectionListener::FAIL_ON_ADDITIONAL_CONNECTIONS);
...
TEST_F(PacFileFetcherImplTest, OnShutdown)
{
    ...
    std::u16string text;
    TestCompletionCallback callback;
    int result = pac_fetcher->Fetch(test_server_.GetURL("/hung"), &text, callback.callback(), TRAFFIC_ANNOTATION_FOR_TESTS);
    EXPECT_THAT(result, IsError(ERR_IO_PENDING));
    ...
    result = pac_fetcher->Fetch(test_server_.GetURL("/hung"), &text, callback.callback(), TRAFFIC_ANNOTATION_FOR_TESTS);
