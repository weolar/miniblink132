...
public:
    ...
    void set_is_mdl_match(bool is_mdl_match)
    {
        is_mdl_match_ = is_mdl_match;
    }
    ...
    template <class Predicate> bool AnyProxyInChain(Predicate p) const
    {
        if (is_empty()) {
            return false;
        }
    ...
    // This is a temporary workaround to gather initial metrics for IP Protection.
    // TODO(crbug.com/40947771): Remove once the experiment is concluded.
    bool is_mdl_match() const
    {
        return is_mdl_match_;
    }
    ...
    void set_proxy_resolve_start_time(const base::TimeTicks& proxy_resolve_start_time)
    {
    ...
    void set_proxy_resolve_end_time(const base::TimeTicks& proxy_resolve_end_time)
    {
    ...
    void set_traffic_annotation(const MutableNetworkTrafficAnnotationTag& traffic_annotation)
    {
