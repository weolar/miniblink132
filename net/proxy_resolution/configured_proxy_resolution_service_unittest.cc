...
class ImmediatePollPolicy : public ConfiguredProxyResolutionService::PacPollPolicy {
public:
    ...
    Mode GetNextDelay(int error, base::TimeDelta current_delay, base::TimeDelta* next_delay) const override
    {
...
class ImmediateAfterActivityPollPolicy : public ConfiguredProxyResolutionService::PacPollPolicy {
public:
    ImmediateAfterActivityPollPolicy() = default;

    ImmediateAfterActivityPollPolicy(const ImmediateAfterActivityPollPolicy&) = delete;
    ImmediateAfterActivityPollPolicy& operator=(const ImmediateAfterActivityPollPolicy&) = delete;
...
class ConfiguredProxyResolutionServiceTest : public ::testing::Test, public WithTaskEnvironment {
protected:
    ...
    {
        testing::Test::SetUp();
        previous_policy_ = ConfiguredProxyResolutionService::set_pac_script_poll_policy(&never_poll_policy_);
    }

    void TearDown() override
    {
        // Restore the original policy.
        ConfiguredProxyResolutionService::set_pac_script_poll_policy(previous_policy_);
        testing::Test::TearDown();
    }

private:
    NeverPollPolicy never_poll_policy_;
    raw_ptr<const ConfiguredProxyResolutionService::PacPollPolicy> previous_policy_;
};
...
public:
    explicit MockProxyConfigService(const ProxyConfig& config)
        : config_(ProxyConfigWithAnnotation(config, TRAFFIC_ANNOTATION_FOR_TESTS))
    {
    }

    explicit MockProxyConfigService(const std::string& pac_url)
        : config_(ProxyConfig::CreateFromCustomPacURL(GURL(pac_url)), TRAFFIC_ANNOTATION_FOR_TESTS)
    {
    }
    ...
    ConfigAvailability GetLatestProxyConfig(ProxyConfigWithAnnotation* results) override
    {
    ...
    {
        SetConfig(ProxyConfigWithAnnotation(ProxyConfig::CreateFromCustomPacURL(GURL(pac_url)), TRAFFIC_ANNOTATION_FOR_TESTS));
    }
...
public:
    ...
    int num_resolve_proxy_called() const
    {
        return num_resolve_proxy_called_;
    }
    ...
    void OnSuccessfulRequestAfterFailures(const ProxyRetryInfoMap& proxy_retry_info) override
    {
    }
    ...
    Error OnBeforeTunnelRequest(const ProxyChain& proxy_chain, size_t chain_index, HttpRequestHeaders* extra_headers) override
    {
        return OK;
    }

    Error OnTunnelHeadersReceived(const ProxyChain& proxy_chain, size_t chain_index, const HttpResponseHeaders& response_headers) override
    {
        return OK;
    }

    void SetProxyResolutionService(ProxyResolutionService* proxy_resolution_service) override
    {
    }
...
public:
    ...
    bool num_proxy_fallback_called() const
    {
        return num_proxy_fallback_called_;
    }
...
JobMap GetJobsForURLs(const JobMap& map, const std::vector<GURL>& urls)
{
    ...
    size_t b = map.size();
    if (a != b) {
        ADD_FAILURE() << "map size (" << map.size() << ") != urls size (" << urls.size() << ")";
        return map;
    }
...
JobMap GetPendingJobsForURLs(const MockAsyncProxyResolver& resolver, const GURL& url1 = GURL(), const GURL& url2 = GURL(), const GURL& url3 = GURL())
{
...
JobMap GetCancelledJobsForURLs(const MockAsyncProxyResolver& resolver, const GURL& url1 = GURL(), const GURL& url2 = GURL(), const GURL& url3 = GURL())
{
    std::vector<GURL> urls;
    if (!url1.is_empty())
        urls.push_back(url1);
    ...
    JobMap map;
    for (const std::unique_ptr<MockAsyncProxyResolver::Job>& it : resolver.cancelled_jobs()) {
        DCHECK(it);
        map[it->url()] = it.get();
    }
...
TEST_F(ConfiguredProxyResolutionServiceTest, Direct)
{
    ...
    ConfiguredProxyResolutionService service(
        std::make_unique<MockProxyConfigService>(ProxyConfig::CreateDirect()), std::move(factory), nullptr, /*quick_check_enabled=*/true);
    ...
    int rv = service.ResolveProxy(
        url, std::string(), NetworkAnonymizationKey(), &info, callback.callback(), &request, NetLogWithSource::Make(NetLogSourceType::NONE));
    EXPECT_THAT(rv, IsOk());
    ...
    EXPECT_TRUE(LogContainsBeginEvent(entries, 0, NetLogEventType::PROXY_RESOLUTION_SERVICE));
    EXPECT_TRUE(LogContainsEvent(entries, 1, NetLogEventType::PROXY_RESOLUTION_SERVICE_RESOLVED_PROXY_LIST, NetLogEventPhase::NONE));
    EXPECT_TRUE(LogContainsEndEvent(entries, 2, NetLogEventType::PROXY_RESOLUTION_SERVICE));
}
...
TEST_F(ConfiguredProxyResolutionServiceTest, OnResolveProxyCallbackAddProxy)
{
    ...
    ConfiguredProxyResolutionService service(std::make_unique<MockProxyConfigService>(config), nullptr, nullptr,
    ...
    std::unique_ptr<ProxyResolutionRequest> request;
    int rv = service.ResolveProxy(url, std::string(), NetworkAnonymizationKey(), &info, callback.callback(), &request, net_log_with_source);
    EXPECT_THAT(rv, IsOk());
    ...
    rv = service.ResolveProxy(url, "GET", NetworkAnonymizationKey(), &info, callback.callback(), &request, net_log_with_source);
    EXPECT_EQ(1, delegate.num_resolve_proxy_called());
    EXPECT_THAT(delegate.proxy_retry_info(), ElementsAre(Key(ProxyChain(ProxyUriToProxyChain("badproxy:8080", ProxyServer::SCHEME_HTTP)))));
    ...
    // Check bypassed URL:
    rv = service.ResolveProxy(bypass_url, "GET", NetworkAnonymizationKey(), &info, callback.callback(), &request, net_log_with_source);
    EXPECT_TRUE(info.is_direct());
}

TEST_F(ConfiguredProxyResolutionServiceTest, OnResolveProxyCallbackRemoveProxy)
{
...
TEST_F(ConfiguredProxyResolutionServiceTest, OnResolveProxyHasNak)
{
    ...
    GURL url("http://www.google.com/");
    NetworkAnonymizationKey network_anonymization_key = NetworkAnonymizationKey::CreateCrossSite(SchemefulSite(GURL("http://example.com")));

    ProxyInfo info;
    TestCompletionCallback callback;
    std::unique_ptr<ProxyResolutionRequest> request;
    service.ResolveProxy(url, std::string(), network_anonymization_key, &info, callback.callback(), &request, NetLogWithSource::Make(NetLogSourceType::NONE));

    EXPECT_EQ(network_anonymization_key, proxy_delegate.network_anonymization_key());
}
...
// requests.
template <typename T> class DeletingCallback : public TestCompletionCallbackBase {
public:
    ...
    {
        return base::BindOnce(&DeletingCallback::DeleteItem, base::Unretained(this));
    }
...
template <typename T> DeletingCallback<T>::~DeletingCallback() = default;
...
{
    auto config_service = std::make_unique<MockProxyConfigService>("http://foopy/proxy.pac");
    MockAsyncProxyResolver resolver;
    auto factory = std::make_unique<MockAsyncProxyResolverFactory>(false);
    auto* factory_ptr = factory.get();

    std::unique_ptr<ConfiguredProxyResolutionService> service
        = std::make_unique<ConfiguredProxyResolutionService>(std::move(config_service), std::move(factory), nullptr,
            /*quick_check_enabled=*/true);
    ...
    net::CompletionOnceCallback callback2 = base::BindOnce([](int result) { ASSERT_FALSE(true); });

    int rv = service->ResolveProxy(url, std::string(), NetworkAnonymizationKey(), &info, callback.callback(), &request, NetLogWithSource());
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));

    rv = service->ResolveProxy(url2, std::string(), NetworkAnonymizationKey(), &info, std::move(callback2), &request2, NetLogWithSource());
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));

    // Run pending requests.
    ASSERT_EQ(1u, factory_ptr->pending_requests().size());
    EXPECT_EQ(GURL("http://foopy/proxy.pac"), factory_ptr->pending_requests()[0]->script_data()->url());
...
TEST_F(ConfiguredProxyResolutionServiceTest, CallbackDeletesRequestDuringDestructor)
{
    auto config_service = std::make_unique<MockProxyConfigService>("http://foopy/proxy.pac");
    ...
    ProxyInfo info;
    std::unique_ptr<ProxyResolutionRequest> request, request2;
    DeletingCallback<ProxyResolutionRequest> callback(&request2), callback2(&request);

    int rv = service->ResolveProxy(url, std::string(), NetworkAnonymizationKey(), &info, callback.callback(), &request, NetLogWithSource());
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));

    rv = service->ResolveProxy(url, std::string(), NetworkAnonymizationKey(), &info, callback2.callback(), &request2, NetLogWithSource());
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
    ...
    if (callback.have_result()) {
        EXPECT_THAT(callback.WaitForResult(), IsError(net::ERR_MANDATORY_PROXY_CONFIGURATION_FAILED));
    }
    if (callback2.have_result()) {
        EXPECT_THAT(callback2.WaitForResult(), IsError(net::ERR_MANDATORY_PROXY_CONFIGURATION_FAILED));
    }
}
...
TEST_F(ConfiguredProxyResolutionServiceTest, CallbackDeletesSelf)
{
    ...
    int rv = service->ResolveProxy(url, std::string(), NetworkAnonymizationKey(), &info, callback1.callback(), &request1, NetLogWithSource());
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));

    GURL url2("http://www.example.com/");
    std::unique_ptr<ProxyResolutionRequest> request2;
    DeletingCallback<ProxyResolutionRequest> callback2(&request2);
    rv = service->ResolveProxy(url2, std::string(), NetworkAnonymizationKey(), &info, callback2.callback(), &request2, NetLogWithSource());
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));

    std::unique_ptr<ProxyResolutionRequest> request3;
    TestCompletionCallback callback3;
    rv = service->ResolveProxy(url, std::string(), NetworkAnonymizationKey(), &info, callback3.callback(), &request3, NetLogWithSource());
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
...
// ConfiguredProxyResolutionService's destructor.
TEST_F(ConfiguredProxyResolutionServiceTest, CallbackDeletesSelfDuringDestructor)
{
    auto config_service = std::make_unique<MockProxyConfigService>("http://foopy/proxy.pac");
    ...
    EXPECT_THAT(callback1.WaitForResult(), IsError(net::ERR_MANDATORY_PROXY_CONFIGURATION_FAILED));
    EXPECT_THAT(callback2.WaitForResult(), IsError(net::ERR_MANDATORY_PROXY_CONFIGURATION_FAILED));
    EXPECT_THAT(callback3.WaitForResult(), IsError(net::ERR_MANDATORY_PROXY_CONFIGURATION_FAILED));
}
...
TEST_F(ConfiguredProxyResolutionServiceTest, ProxyServiceDeletedBeforeRequest)
{
    ...
    int rv;
    {
        ConfiguredProxyResolutionService service(std::move(config_service), std::move(factory), nullptr,
            /*quick_check_enabled=*/true);
...
TEST_F(ConfiguredProxyResolutionServiceTest, CallbackDeletesService)
{
    ...
    std::unique_ptr<ProxyResolutionRequest> request1;
    int rv = service->ResolveProxy(url, std::string(), NetworkAnonymizationKey(), &info, callback.callback(), &request1, NetLogWithSource());
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
    ...
    config_service_ptr->SetConfig(ProxyConfigWithAnnotation(ProxyConfig::CreateDirect(), TRAFFIC_ANNOTATION_FOR_TESTS));
...
TEST_F(ConfiguredProxyResolutionServiceTest, PAC)
{
    ...
    EXPECT_TRUE(LogContainsBeginEvent(entries, 0, NetLogEventType::PROXY_RESOLUTION_SERVICE));
    EXPECT_TRUE(LogContainsBeginEvent(entries, 1, NetLogEventType::PROXY_RESOLUTION_SERVICE_WAITING_FOR_INIT_PAC));
    EXPECT_TRUE(LogContainsEndEvent(entries, 2, NetLogEventType::PROXY_RESOLUTION_SERVICE_WAITING_FOR_INIT_PAC));
    EXPECT_TRUE(LogContainsEndEvent(entries, 4, NetLogEventType::PROXY_RESOLUTION_SERVICE));
}
...
TEST_F(ConfiguredProxyResolutionServiceTest, PAC_NoIdentityOrHash)
{
    ...
    EXPECT_EQ(GURL("http://www.google.com/?ref"), resolver.pending_jobs()[0]->url());
...
TEST_F(ConfiguredProxyResolutionServiceTest, PAC_FailoverAfterDirect)
{
    ...
    // Set the result in proxy resolver.
    resolver.pending_jobs()[0]->results()->UsePacString("DIRECT ; PROXY foobar:10 ; DIRECT ; PROXY foobar:20");
    resolver.pending_jobs()[0]->CompleteNow(OK);
...
TEST_F(ConfiguredProxyResolutionServiceTest, PAC_ConfigSourcePropagates)
{
    ...
    EXPECT_THAT(callback.WaitForResult(), IsOk());
    EXPECT_EQ(MutableNetworkTrafficAnnotationTag(TRAFFIC_ANNOTATION_FOR_TESTS), info.traffic_annotation());
...
TEST_F(ConfiguredProxyResolutionServiceTest, ProxyResolverFails)
{
    ...
    TestCompletionCallback callback1;
    std::unique_ptr<ProxyResolutionRequest> request;
    int rv = service.ResolveProxy(url, std::string(), NetworkAnonymizationKey(), &info, callback1.callback(), &request, NetLogWithSource());
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
    ...
    TestCompletionCallback callback2;
    rv = service.ResolveProxy(url, std::string(), NetworkAnonymizationKey(), &info, callback2.callback(), &request, NetLogWithSource());
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
...
TEST_F(ConfiguredProxyResolutionServiceTest, ProxyResolverTerminatedDuringRequest)
{
...
TEST_F(ConfiguredProxyResolutionServiceTest, ProxyResolverTerminatedDuringRequestWithConcurrentRequest)
{
    // Test what happens when the ProxyResolver fails with a fatal error while
    // a GetProxyForURL() call is in progress.
    ...
    int rv = service.ResolveProxy(url1, std::string(), NetworkAnonymizationKey(), &info, callback1.callback(), &request1, NetLogWithSource());
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
    TestCompletionCallback callback2;
    rv = service.ResolveProxy(url2, std::string(), NetworkAnonymizationKey(), &info, callback2.callback(), &request2, NetLogWithSource());
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
...
TEST_F(ConfiguredProxyResolutionServiceTest, PacFileFetcherFailsDownloadingMandatoryPac)
{
    ...
    // fall-back to DIRECT.
    EXPECT_EQ(ERR_MANDATORY_PROXY_CONFIGURATION_FAILED, callback1.WaitForResult());
    EXPECT_FALSE(info.is_direct());
...
TEST_F(ConfiguredProxyResolutionServiceTest, ProxyResolverFailsParsingJavaScriptMandatoryPac)
{
    ...
    service.SetPacFileFetchers(std::move(fetcher), std::make_unique<DoNothingDhcpPacFileFetcher>());
...
TEST_F(ConfiguredProxyResolutionServiceTest, ProxyResolverFailsInJavaScriptMandatoryPac)
{
    // Test what happens when the ProxyResolver fails that is configured to use a
...
TEST_F(ConfiguredProxyResolutionServiceTest, ProxyFallback)
{
    ...
    // Set the result in proxy resolver.
    resolver.pending_jobs()[0]->results()->UseNamedProxy("foopy1:8080;foopy2:9090");
    resolver.pending_jobs()[0]->CompleteNow(OK);
    ...
    EXPECT_EQ(ERR_PROXY_CONNECTION_FAILED, test_delegate.last_proxy_fallback_net_error());
    service.SetProxyDelegate(nullptr);
    EXPECT_EQ(1u, info.proxy_retry_info().size());
    EXPECT_TRUE(info.proxy_retry_info().contains(ProxyChain::FromSchemeHostAndPort(ProxyServer::SCHEME_HTTP, "foopy1", 8080)));

    TestCompletionCallback callback3;
    rv = service.ResolveProxy(url, std::string(), NetworkAnonymizationKey(), &info, callback3.callback(), &request, NetLogWithSource());
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
    ...
    resolver.pending_jobs()[0]->results()->UseNamedProxy("foopy3:7070;foopy1:8080;foopy2:9090");
    resolver.pending_jobs()[0]->CompleteNow(OK);
    ...
    rv = service.ResolveProxy(url, std::string(), NetworkAnonymizationKey(), &info, callback7.callback(), &request, NetLogWithSource());
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
    ...
    resolver.pending_jobs()[0]->results()->UseNamedProxy("foopy1:8080;foopy3:7070;foopy2:9090;foopy4:9091");
    resolver.pending_jobs()[0]->CompleteNow(OK);
...
TEST_F(ConfiguredProxyResolutionServiceTest, ProxyFallbackToDirect)
{
    ...
    // Set the result in proxy resolver.
    resolver.pending_jobs()[0]->results()->UsePacString("PROXY foopy1:8080; PROXY foopy2:9090; DIRECT");
    resolver.pending_jobs()[0]->CompleteNow(OK);
...
TEST_F(ConfiguredProxyResolutionServiceTest, ProxyFallback_BadConfig)
{
    ...
    ProxyInfo info2;
    TestCompletionCallback callback2;
    rv = service.ResolveProxy(url, std::string(), NetworkAnonymizationKey(), &info2, callback2.callback(), &request, NetLogWithSource());
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
    ...
    TestCompletionCallback callback3;
    std::unique_ptr<ProxyResolutionRequest> request3;
    rv = service.ResolveProxy(url, std::string(), NetworkAnonymizationKey(), &info3, callback3.callback(), &request3, NetLogWithSource());
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
...
TEST_F(ConfiguredProxyResolutionServiceTest, ProxyFallback_BadConfigMandatory)
{
    ...
    TestCompletionCallback callback3;
    std::unique_ptr<ProxyResolutionRequest> request3;
    rv = service.ResolveProxy(url, std::string(), NetworkAnonymizationKey(), &info2, callback3.callback(), &request3, NetLogWithSource());
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
    ...
    EXPECT_EQ(ERR_MANDATORY_PROXY_CONFIGURATION_FAILED, callback3.WaitForResult());
    ...
    rv = service.ResolveProxy(url, std::string(), NetworkAnonymizationKey(), &info3, callback4.callback(), &request4, NetLogWithSource());
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
...
TEST_F(ConfiguredProxyResolutionServiceTest, ProxyBypassList)
{
    ...
    rv = service.ResolveProxy(url1, std::string(), NetworkAnonymizationKey(), &info[0], callback[0].callback(), &request1, NetLogWithSource());
    EXPECT_THAT(rv, IsOk());
    EXPECT_TRUE(info[0].is_direct());

    // Request for a .com domain hits the proxy.
    rv = service.ResolveProxy(url2, std::string(), NetworkAnonymizationKey(), &info[1], callback[1].callback(), &request2, NetLogWithSource());
    EXPECT_THAT(rv, IsOk());
...
TEST_F(ConfiguredProxyResolutionServiceTest, PerProtocolProxyTests)
{
    ...
    std::unique_ptr<ProxyResolutionRequest> request;
    {
        ...
        ProxyInfo info;
        TestCompletionCallback callback;
        int rv = service.ResolveProxy(test_url, std::string(), NetworkAnonymizationKey(), &info, callback.callback(), &request, NetLogWithSource());
        EXPECT_THAT(rv, IsOk());
        EXPECT_FALSE(info.is_direct());
        EXPECT_EQ("[foopy1:8080]", info.proxy_chain().ToDebugString());
    }
...
TEST_F(ConfiguredProxyResolutionServiceTest, ProxyConfigTrafficAnnotationPropagates)
{
...
TEST_F(ConfiguredProxyResolutionServiceTest, DefaultProxyFallbackToSOCKS)
{
    ...
    config.set_auto_detect(false);
    EXPECT_EQ(ProxyConfig::ProxyRules::Type::PROXY_LIST_PER_SCHEME, config.proxy_rules().type);
...
TEST_F(ConfiguredProxyResolutionServiceTest, CancelInProgressRequest)
{
    ...
    TestCompletionCallback callback1;
    std::unique_ptr<ProxyResolutionRequest> request1;
    int rv = service.ResolveProxy(url1, std::string(), NetworkAnonymizationKey(), &info1, callback1.callback(), &request1, NetLogWithSource());
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
    ...
    TestCompletionCallback callback2;
    std::unique_ptr<ProxyResolutionRequest> request2;
    rv = service.ResolveProxy(url2, std::string(), NetworkAnonymizationKey(), &info2, callback2.callback(), &request2, NetLogWithSource());
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
    ...
    TestCompletionCallback callback3;
    std::unique_ptr<ProxyResolutionRequest> request3;
    rv = service.ResolveProxy(url3, std::string(), NetworkAnonymizationKey(), &info3, callback3.callback(), &request3, NetLogWithSource());
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
...
TEST_F(ConfiguredProxyResolutionServiceTest, InitialPACScriptDownload)
{
    ...
    // resolver.
    EXPECT_EQ(kValidPacScript116, factory_ptr->pending_requests()[0]->script_data()->utf16());
    factory_ptr->pending_requests()[0]->CompleteNowWithForwarder(OK, &resolver);
...
TEST_F(ConfiguredProxyResolutionServiceTest, ChangeScriptFetcherWhilePACDownloadInProgress)
{
    const GURL url1("http://request1");
    const GURL url2("http://request2");
    auto config_service = std::make_unique<MockProxyConfigService>("http://foopy/proxy.pac");
...
TEST_F(ConfiguredProxyResolutionServiceTest, CancelWhilePACFetching)
{
    ...
    std::unique_ptr<ProxyResolutionRequest> request1;
    RecordingNetLogObserver net_log_observer;
    int rv = service.ResolveProxy(GURL("http://request1"), std::string(), NetworkAnonymizationKey(), &info1, callback1.callback(), &request1,
    ...
    TestCompletionCallback callback2;
    std::unique_ptr<ProxyResolutionRequest> request2;
    rv = service.ResolveProxy(GURL("http://request2"), std::string(), NetworkAnonymizationKey(), &info2, callback2.callback(), &request2, NetLogWithSource());
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));

    ProxyInfo info3;
    TestCompletionCallback callback3;
    std::unique_ptr<ProxyResolutionRequest> request3;
    rv = service.ResolveProxy(GURL("http://request3"), std::string(), NetworkAnonymizationKey(), &info3, callback3.callback(), &request3, NetLogWithSource());
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
    ...
    EXPECT_TRUE(LogContainsBeginEvent(entries1, 0, NetLogEventType::PROXY_RESOLUTION_SERVICE));
    EXPECT_TRUE(LogContainsBeginEvent(entries1, 1, NetLogEventType::PROXY_RESOLUTION_SERVICE_WAITING_FOR_INIT_PAC));
    // Note that PROXY_RESOLUTION_SERVICE_WAITING_FOR_INIT_PAC is never completed
    // before the cancellation occured.
    EXPECT_TRUE(LogContainsEvent(entries1, 2, NetLogEventType::CANCELLED, NetLogEventPhase::NONE));
    EXPECT_TRUE(LogContainsEndEvent(entries1, 3, NetLogEventType::PROXY_RESOLUTION_SERVICE));
}

// Test that if auto-detect fails, we fall-back to the custom pac.
TEST_F(ConfiguredProxyResolutionServiceTest, FallbackFromAutodetectToCustomPac)
{
    const GURL url1("http://request1");
    const GURL url2("http://request2");
    ProxyConfig config;
...
TEST_F(ConfiguredProxyResolutionServiceTest, FallbackFromAutodetectToCustomPac2)
{
    const GURL url1("http://request1");
    const GURL url2("http://request2");
    ProxyConfig config;
...
TEST_F(ConfiguredProxyResolutionServiceTest, FallbackFromAutodetectToCustomToManual)
{
    ProxyConfig config;
    config.set_auto_detect(true);
    config.set_pac_url(GURL("http://foopy/proxy.pac"));
    ...
    std::unique_ptr<ProxyResolutionRequest> request1;
    int rv
        = service.ResolveProxy(GURL("http://request1"), std::string(), NetworkAnonymizationKey(), &info1, callback1.callback(), &request1, NetLogWithSource());
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
...
TEST_F(ConfiguredProxyResolutionServiceTest, BypassDoesntApplyToPac)
{
    ...
    std::unique_ptr<ProxyResolutionRequest> request1;
    int rv = service.ResolveProxy(
        GURL("http://www.google.com"), std::string(), NetworkAnonymizationKey(), &info1, callback1.callback(), &request1, NetLogWithSource());
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
    ...
    std::unique_ptr<ProxyResolutionRequest> request2;
    rv = service.ResolveProxy(
        GURL("http://www.google.com"), std::string(), NetworkAnonymizationKey(), &info2, callback2.callback(), &request2, NetLogWithSource());
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
...
TEST_F(ConfiguredProxyResolutionServiceTest, DeleteWhileInitProxyResolverHasOutstandingFetch)
{
    ProxyConfig config = ProxyConfig::CreateFromCustomPacURL(GURL("http://foopy/proxy.pac"));
...
// deleted prior to the InitProxyResolver).
TEST_F(ConfiguredProxyResolutionServiceTest, DeleteWhileInitProxyResolverHasOutstandingSet)
{
    auto config_service = std::make_unique<MockProxyConfigService>("http://foopy/proxy.pac");
...
TEST_F(ConfiguredProxyResolutionServiceTest, UpdateConfigFromPACToDirect)
{
    ...
    EXPECT_EQ(PacFileData::TYPE_AUTO_DETECT, factory_ptr->pending_requests()[0]->script_data()->type());
    factory_ptr->pending_requests()[0]->CompleteNowWithForwarder(OK, &resolver);
...
TEST_F(ConfiguredProxyResolutionServiceTest, NetworkChangeTriggersPacRefetch)
{
    ...
    ConfiguredProxyResolutionService service(std::move(config_service), std::move(factory), net::NetLog::Get(),
        /*quick_check_enabled=*/true);
    ...
    EXPECT_EQ(kValidPacScript216, factory_ptr->pending_requests()[0]->script_data()->utf16());
    factory_ptr->pending_requests()[0]->CompleteNowWithForwarder(OK, &resolver);
    ...
    EXPECT_TRUE(LogContainsEntryWithType(entries, 0, NetLogEventType::PROXY_CONFIG_CHANGED));
...
TEST_F(ConfiguredProxyResolutionServiceTest, PACScriptRefetchAfterContentChange)
{
    // Change the retry policy to wait a mere 1 ms before retrying, so the test
    // runs quickly.
    ImmediatePollPolicy poll_policy;
...
TEST_F(ConfiguredProxyResolutionServiceTest, PACScriptRefetchAfterContentUnchanged)
{
    // Change the retry policy to wait a mere 1 ms before retrying, so the test
    // runs quickly.
    ImmediatePollPolicy poll_policy;
...
TEST_F(ConfiguredProxyResolutionServiceTest, PACScriptPollingPolicy)
{
    ...
    std::unique_ptr<ConfiguredProxyResolutionService::PacPollPolicy> policy = ConfiguredProxyResolutionService::CreateDefaultPacPollPolicy();
    ...
    EXPECT_EQ(ConfiguredProxyResolutionService::PacPollPolicy::MODE_USE_TIMER, mode);

    // Poll #1
    mode = policy->GetNextDelay(error, delay, &delay);
    EXPECT_EQ(32, delay.InSeconds());
    EXPECT_EQ(ConfiguredProxyResolutionService::PacPollPolicy::MODE_START_AFTER_ACTIVITY, mode);
...
TEST_F(ConfiguredProxyResolutionServiceTest, IpAddressChangeResetsProxy)
{
    ...
    ConfiguredProxyResolutionService service(std::make_unique<MockProxyConfigService>(ProxyConfig::CreateAutoDetect()), std::move(factory),
    ...
    TestCompletionCallback callback2;
    std::unique_ptr<ProxyResolutionRequest> request2;
    rv = service.ResolveProxy(GURL("http://request1"), std::string(), NetworkAnonymizationKey(), &info2, callback2.callback(), &request2, NetLogWithSource());
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
...
TEST_F(ConfiguredProxyResolutionServiceTest, DnsChangeNoopWithoutResolver)
{
    ...
    MockAsyncProxyResolver resolver;
    ConfiguredProxyResolutionService service(std::make_unique<MockProxyConfigService>(ProxyConfig::CreateAutoDetect()),
...
public:
    SanitizeUrlHelper()
    {
        ...
        auto factory = std::make_unique<MockAsyncProxyResolverFactory>(false);
        auto* factory_ptr = factory.get();
        service_ = std::make_unique<ConfiguredProxyResolutionService>(std::move(config_service), std::move(factory), nullptr,
            /*quick_check_enabled=*/true);
        ...
        TestCompletionCallback callback;
        std::unique_ptr<ProxyResolutionRequest> request;
        int rv = service_->ResolveProxy(url, std::string(), NetworkAnonymizationKey(), &info, callback.callback(), &request, NetLogWithSource());
        EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
    ...
    GURL SanitizeUrl(const GURL& raw_url)
    {
        ...
        TestCompletionCallback callback;
        std::unique_ptr<ProxyResolutionRequest> request1;
        int rv = service_->ResolveProxy(raw_url, std::string(), NetworkAnonymizationKey(), &info, callback.callback(), &request1, NetLogWithSource());
        EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
...
TEST_F(ConfiguredProxyResolutionServiceTest, OnShutdownWithLiveRequest)
{
    ...
    TestCompletionCallback callback;
    std::unique_ptr<ProxyResolutionRequest> request;
    int rv = service.ResolveProxy(GURL("http://request/"), std::string(), NetworkAnonymizationKey(), &info, callback.callback(), &request, NetLogWithSource());
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
...
TEST_F(ConfiguredProxyResolutionServiceTest, ImplicitlyBypassWithManualSettings)
{
    ...
    auto service = ConfiguredProxyResolutionService::CreateFixedForTest(ProxyConfigWithAnnotation(config, TRAFFIC_ANNOTATION_FOR_TESTS));
    ...
    TestCompletionCallback callback1;
    int rv = service->ResolveProxy(
        GURL("http://www.example.com"), std::string(), NetworkAnonymizationKey(), &info1, callback1.callback(), &request1, NetLogWithSource());
    EXPECT_THAT(rv, IsOk());
    ...
    for (auto* host : kImplicityBypassedHosts) {
        for (auto* scheme : kUrlSchemes) {
            ...
            ProxyInfo info;
            TestCompletionCallback callback;
            rv = service->ResolveProxy(url, std::string(), NetworkAnonymizationKey(), &info, callback.callback(), &request, NetLogWithSource());
            EXPECT_THAT(rv, IsOk());
            EXPECT_TRUE(info.is_direct());
        }
    }
...
TEST_F(ConfiguredProxyResolutionServiceTest, CastToConfiguredProxyResolutionService)
{
    auto config_service = std::make_unique<MockProxyConfigService>(ProxyConfig::CreateDirect());

    ConfiguredProxyResolutionService service(std::move(config_service), std::make_unique<MockAsyncProxyResolverFactory>(false), nullptr,
        /*quick_check_enabled=*/true);
