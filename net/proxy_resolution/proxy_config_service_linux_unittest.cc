...
template <typename key_type, typename value_type> struct SettingsTable {
...
#undef ENTRY
    ...
    bool SetVar(std::string_view variable_name, const std::string& new_value) override
    {
        ADD_FAILURE();
        return false;
    }
...
public:
    ...
    MockSettingGetter()
    {
#define ENTRY(key, field) strings_table.settings[SettingGetter::key] = &values.field
...
#undef ENTRY
#define ENTRY(key, field) ints_table.settings[SettingGetter::key] = &values.field
...
#undef ENTRY
#define ENTRY(key, field) bools_table.settings[SettingGetter::key] = &values.field
...
public:
    explicit SyncConfigGetter(std::unique_ptr<ProxyConfigServiceLinux> config_service)
        : event_(base::WaitableEvent::ResetPolicy::AUTOMATIC, base::WaitableEvent::InitialState::NOT_SIGNALED)
        , main_thread_("Main_Thread")
        , config_service_(std::move(config_service))
        , matches_pac_url_event_(base::WaitableEvent::ResetPolicy::AUTOMATIC, base::WaitableEvent::InitialState::NOT_SIGNALED)
    {
        ...
        main_thread_.task_runner()->PostTask(FROM_HERE, base::BindOnce(&SyncConfigGetter::Init, base::Unretained(this)));
        Wait();
    }
    ...
    ~SyncConfigGetter() override
    {
        ...
        main_thread_.task_runner()->PostTask(FROM_HERE, base::BindOnce(&SyncConfigGetter::CleanUp, base::Unretained(this)));
        Wait();
    }
    ...
    void SetupAndInitialFetch()
    {
        config_service_->SetupAndFetchInitialConfig(
            base::SingleThreadTaskRunner::GetCurrentDefault(), main_thread_.task_runner(), TRAFFIC_ANNOTATION_FOR_TESTS);
    }
    // Synchronously gets the proxy config.
    ProxyConfigService::ConfigAvailability SyncGetLatestProxyConfig(ProxyConfigWithAnnotation* config)
    {
        main_thread_.task_runner()->PostTask(FROM_HERE, base::BindOnce(&SyncConfigGetter::GetLatestConfigOnIOThread, base::Unretained(this)));
        Wait();
...
private:
    void OnProxyConfigChanged(const ProxyConfigWithAnnotation& config, ProxyConfigService::ConfigAvailability availability) override
    {
        // If the configuration changed to |expected_pac_url_| signal the event.
        base::AutoLock lock(lock_);
        if (config.value().has_pac_url() && config.value().pac_url() == expected_pac_url_) {
...
class ProxyConfigServiceLinuxTest : public PlatformTest, public WithTaskEnvironment {
protected:
    void SetUp() override
    {
        PlatformTest::SetUp();
        ...
        config_kdedefaults_home_ = config_home_.Append(FILE_PATH_LITERAL("kdedefaults"));
        kioslaverc5_xdg_ = config_xdg_home_.Append(FILE_PATH_LITERAL("kioslaverc"));
        kioslaverc5_kdedefaults_ = config_kdedefaults_home_.Append(FILE_PATH_LITERAL("kioslaverc"));
    }
...
TEST_F(ProxyConfigServiceLinuxTest, BasicGSettingsTest)
{
    ...
    const struct {
...

        {
            ...
            false, // auto_detect
            GURL(), // pac_url
            ProxyRulesExpectation::PerSchemeWithSocks("www.google.com:88", // http
                "www.foo.com:110", // https
                "ftp.foo.com:121", // ftp
    ...
    for (size_t i = 0; i < std::size(tests); ++i) {
        SCOPED_TRACE(base::StringPrintf("Test[%" PRIuS "] %s", i, tests[i].description.c_str()));
        ...
        SyncConfigGetter sync_config_getter(std::make_unique<ProxyConfigServiceLinux>(std::move(env), std::move(setting_getter), TRAFFIC_ANNOTATION_FOR_TESTS));
...
TEST_F(ProxyConfigServiceLinuxTest, BasicEnvTest)
{
    ...
    const struct {
...

        {
            ...
            false, // auto_detect
            GURL(), // pac_url
            ProxyRulesExpectation::Single("socks5://socks.com:888", // single proxy
                ""), // bypass rules
        },

        {
            ...
            false, // auto_detect
            GURL(), // pac_url
            ProxyRulesExpectation::Single("socks4://socks.com:888", // single proxy
                ""), // bypass rules
        },

        {
            ...
            false, // auto_detect
            GURL(), // pac_url
            ProxyRulesExpectation::Single("socks5://socks.com:1080", // single proxy
                ""), // bypass rules
        },

        {
            ...
            false, // auto_detect
            GURL(), // pac_url
            ProxyRulesExpectation::Single("www.google.com:80", "*.google.com,*foo.com:99,1.2.3.4:22,127.0.0.1/8"),
        },
    };
...
TEST_F(ProxyConfigServiceLinuxTest, GSettingsNotification)
{
    ...
    auto setting_getter = std::make_unique<MockSettingGetter>();
    auto* setting_getter_ptr = setting_getter.get();
    auto service = std::make_unique<ProxyConfigServiceLinux>(std::move(env), std::move(setting_getter), TRAFFIC_ANNOTATION_FOR_TESTS);
    ...
    sync_config_getter.SetupAndInitialFetch();
    EXPECT_EQ(ProxyConfigService::CONFIG_VALID, sync_config_getter.SyncGetLatestProxyConfig(&config));
...
TEST_F(ProxyConfigServiceLinuxTest, KDEConfigParser)
{
    ...
    const struct {
...

        {
            ...
            false, // auto_detect
            GURL(), // pac_url
            ProxyRulesExpectation::PerSchemeWithBypassReversed("www.google.com:80", // http
                "", // https
                "", // ftp
                "*.google.com"), // bypass rules
        },
...

        {
            ...
            false, // auto_detect
            GURL(), // pac_url
            ProxyRulesExpectation::PerSchemeWithSocks("www.normal.com:80", // http
    ...
    for (size_t i = 0; i < std::size(tests); ++i) {
        ...
        SyncConfigGetter sync_config_getter(std::make_unique<ProxyConfigServiceLinux>(std::move(env), TRAFFIC_ANNOTATION_FOR_TESTS));
        ProxyConfigWithAnnotation config;
...
TEST_F(ProxyConfigServiceLinuxTest, KDEFileChanged)
{
    ...
    EXPECT_TRUE(base::WriteFile(kioslaverc_,
        "[Proxy Settings]\nProxyType=2\n"
    ...
    EXPECT_TRUE(base::WriteFile(kioslaverc_.AddExtension("new"),
        "[Proxy Settings]\nProxyType=2\n"
