...
bool IsLinkLocalIP(const GURL& url)
{
    ...
    // common case.
    if (!(url.host_piece().starts_with("169.254.") || url.host_piece().starts_with("["))) {
        return false;
    }
...
public:
    ...
    BypassSimpleHostnamesRule& operator=(const BypassSimpleHostnamesRule&) = delete;

    SchemeHostPortMatcherResult Evaluate(const GURL& url) const override
    {
        return ((url.host_piece().find('.') == std::string::npos) && !url.HostIsIPAddress()) ? SchemeHostPortMatcherResult::kInclude
                                                                                             : SchemeHostPortMatcherResult::kNoMatch;
    }

    std::string ToString() const override
    {
        return kBypassSimpleHostnames;
    }
};
...
public:
    ...
    SubtractImplicitBypassesRule& operator=(const SubtractImplicitBypassesRule&) = delete;

    SchemeHostPortMatcherResult Evaluate(const GURL& url) const override
    {
        return ProxyBypassRules::MatchesImplicitRules(url) ? SchemeHostPortMatcherResult::kExclude : SchemeHostPortMatcherResult::kNoMatch;
    }

    std::string ToString() const override
    {
        return kSubtractImplicitBypasses;
    }
};

std::unique_ptr<SchemeHostPortMatcherRule> ParseRule(std::string_view raw_untrimmed)
{
...
void ProxyBypassRules::ReplaceRule(size_t index, std::unique_ptr<SchemeHostPortMatcherRule> rule)
{
...
bool ProxyBypassRules::MatchesImplicitRules(const GURL& url)
{
    ...
    return IsLocalhost(url) || IsIPv4MappedLoopback(url)
        ...
        || (url.host_piece() == "loopback") || (url.host_piece() == "loopback.")
#endif
        ;
}
