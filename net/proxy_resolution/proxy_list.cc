...
void ProxyList::Set(const std::string& proxy_uri_list)
{
    ...
    while (str_tok.GetNext()) {
        ProxyChain chain = ProxyUriToProxyChain(str_tok.token_piece(), ProxyServer::SCHEME_HTTP);
        AddProxyChain(chain);
    }
}
...
void ProxyList::DeprioritizeBadProxyChains(const ProxyRetryInfoMap& proxy_retry_info)
{
    ...
    proxy_chains_.insert(proxy_chains_.end(), bad_chains_to_try.begin(), bad_chains_to_try.end());
}
...
void ProxyList::RemoveProxiesWithoutScheme(int scheme_bit_field)
{
    std::erase_if(proxy_chains_, [&](const ProxyChain& chain) {
        ...
        return std::any_of(proxy_servers.begin(), proxy_servers.end(), [&](const ProxyServer& server) { return !(scheme_bit_field & server.scheme()); });
    });
}
...
void ProxyList::SetFromPacString(const std::string& pac_string)
{
    ...
    while (entry_tok.GetNext()) {
        ProxyChain proxy_chain = PacResultElementToProxyChain(entry_tok.token_piece());
        if (proxy_chain.IsValid()) {
...
std::string ProxyList::ToPacString() const
{
    ...
    for (const ProxyChain& proxy_chain : proxy_chains_) {
        ...
        proxy_list += proxy_chain.is_direct() ? "DIRECT" : ProxyServerToPacResultElement(proxy_chain.First());
    }
    return proxy_list;
}
...
bool ProxyList::Fallback(ProxyRetryInfoMap* proxy_retry_info, int net_error, const NetLogWithSource& net_log)
{
    if (proxy_chains_.empty()) {
        NOTREACHED();
    }
    // By default, proxy chains are not retried for 5 minutes.
    UpdateRetryInfoOnFallback(proxy_retry_info, base::Minutes(5), true, std::vector<ProxyChain>(), net_error, net_log);
...
void ProxyList::AddProxyChainToRetryList(ProxyRetryInfoMap* proxy_retry_info, base::TimeDelta retry_delay, bool try_while_bad,
    const ProxyChain& proxy_chain_to_retry, int net_error, const NetLogWithSource& net_log) const
{
    ...
    net_log.AddEventWithStringParams(NetLogEventType::PROXY_LIST_FALLBACK, "bad_proxy_chain", proxy_chain_to_retry.ToDebugString());
}

void ProxyList::UpdateRetryInfoOnFallback(ProxyRetryInfoMap* proxy_retry_info, base::TimeDelta retry_delay, bool reconsider,
    const std::vector<ProxyChain>& additional_proxies_to_bypass, int net_error, const NetLogWithSource& net_log) const
{
    ...
    if (!first_chain.is_direct()) {
        AddProxyChainToRetryList(proxy_retry_info, retry_delay, reconsider, first_chain, net_error, net_log);
        // If any additional proxies to bypass are specified, add to the retry map
        // as well.
        for (const ProxyChain& additional_proxy_chain : additional_proxies_to_bypass) {
            AddProxyChainToRetryList(proxy_retry_info, retry_delay, reconsider, ProxyChain(additional_proxy_chain.proxy_servers()), net_error, net_log);
        }
    }
}
