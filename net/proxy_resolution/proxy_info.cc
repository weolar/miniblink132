...
bool ProxyInfo::ContainsMultiProxyChain() const
{
    ...
    return std::any_of(proxy_chains.begin(), proxy_chains.end(), [](const ProxyChain& proxy_chain) { return proxy_chain.is_multi_proxy(); });
}
...
void ProxyInfo::DeprioritizeBadProxyChains(const ProxyRetryInfoMap& proxy_retry_info)
{
...
bool ProxyInfo::AllChainProxiesAreHttps() const
{
    ...
    return std::all_of(proxy_servers.begin(), proxy_servers.end(), [](const ProxyServer& proxy_server) { return proxy_server.is_https(); });
}
