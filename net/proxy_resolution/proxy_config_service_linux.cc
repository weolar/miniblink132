...
void RewriteRulesForSuffixMatching(ProxyBypassRules* out)
{
    ...
    for (size_t i = 0; i < out->rules().size(); ++i) {
        ...
        const SchemeHostPortMatcherHostnamePatternRule* prev_rule = static_cast<const SchemeHostPortMatcherHostnamePatternRule*>(out->rules()[i].get());
...
ProxyConfigWithAnnotation GetConfigOrDirect(const std::optional<ProxyConfigWithAnnotation>& optional_config)
{
    if (optional_config)
...
bool ProxyConfigServiceLinux::Delegate::GetProxyFromEnvVarForScheme(std::string_view variable, ProxyServer::Scheme scheme, ProxyChain* result_chain)
{
    ...
    ProxyChain proxy_chain = ProxyUriToProxyChain(env_value, ProxyServer::SCHEME_HTTP);
    if (proxy_chain.IsValid() && (proxy_chain.is_direct() || proxy_chain.is_single_proxy())) {
...
bool ProxyConfigServiceLinux::Delegate::GetProxyFromEnvVar(std::string_view variable, ProxyChain* result_chain)
{
    return GetProxyFromEnvVarForScheme(variable, ProxyServer::SCHEME_HTTP, result_chain);
}

std::optional<ProxyConfigWithAnnotation> ProxyConfigServiceLinux::Delegate::GetConfigFromEnv()
{
    ProxyConfig config;
    ...
    if (env_var_getter_->GetVar("auto_proxy", &auto_proxy)) {
        ...
        return ProxyConfigWithAnnotation(config, NetworkTrafficAnnotationTag(traffic_annotation_));
    }
    ...
    if (GetProxyFromEnvVar("all_proxy", &proxy_chain)) {
        ...
        if (have_http || have_https || have_ftp) {
            ...
            config.proxy_rules().type = ProxyConfig::ProxyRules::Type::PROXY_LIST_PER_SCHEME;
        }
    }
    if (config.proxy_rules().empty()) {
        ...
        if (env_var_getter_->GetVar("SOCKS_VERSION", &env_version) && env_version == "4")
            scheme = ProxyServer::SCHEME_SOCKS4;
    ...
    if (config.proxy_rules().empty()) {
        ...
        // connections.
        return !no_proxy.empty() ? ProxyConfigWithAnnotation(config, NetworkTrafficAnnotationTag(traffic_annotation_))
...
class SettingGetterImplGSettings : public ProxyConfigServiceLinux::SettingGetter {
public:
    ...
    SettingGetterImplGSettings& operator=(const SettingGetterImplGSettings&) = delete;
    ...
    bool Init(const scoped_refptr<base::SingleThreadTaskRunner>& glib_task_runner) override
    {
        DCHECK(glib_task_runner->RunsTasksInCurrentSequence());
        DCHECK(!client_);
        DCHECK(!task_runner_.get());

        if (!g_settings_schema_source_lookup(g_settings_schema_source_get_default(), kProxyGSettingsSchema, TRUE)
    ...
    bool SetUpNotifications(ProxyConfigServiceLinux::Delegate* delegate) override
    {
        DCHECK(client_);
        DCHECK(task_runner_->RunsTasksInCurrentSequence());
        ...
        g_signal_connect(G_OBJECT(client_.get()), "changed", G_CALLBACK(OnGSettingsChangeNotification), this);
        g_signal_connect(G_OBJECT(http_client_.get()), "changed", G_CALLBACK(OnGSettingsChangeNotification), this);
        g_signal_connect(G_OBJECT(https_client_.get()), "changed", G_CALLBACK(OnGSettingsChangeNotification), this);
        g_signal_connect(G_OBJECT(ftp_client_.get()), "changed", G_CALLBACK(OnGSettingsChangeNotification), this);
        g_signal_connect(G_OBJECT(socks_client_.get()), "changed", G_CALLBACK(OnGSettingsChangeNotification), this);
        // Simulate a change to avoid possibly losing updates before this point.
        OnChangeNotification();
        return true;
    }

    const scoped_refptr<base::SequencedTaskRunner>& GetNotificationTaskRunner() override
    {
        return task_runner_;
    }
    ...
    bool GetStringList(StringListSetting key, std::vector<std::string>* result) override
    {
        DCHECK(client_);
        switch (key) {
    ...
    bool UseSuffixMatching() override
    {
        return false;
    }

private:
    bool GetStringByPath(GSettings* client, std::string_view key, std::string* result)
    {
        DCHECK(task_runner_->RunsTasksInCurrentSequence());
    ...
    bool GetStringListByPath(GSettings* client, std::string_view key, std::vector<std::string>* result)
    {
        DCHECK(task_runner_->RunsTasksInCurrentSequence());
    ...
    void OnChangeNotification()
    {
        ...
        debounce_timer_->Start(FROM_HERE, base::Milliseconds(kDebounceTimeoutMilliseconds), this, &SettingGetterImplGSettings::OnDebouncedNotification);
    }

    // gsettings notification callback, dispatched on the default glib main loop.
    static void OnGSettingsChangeNotification(GSettings* client, gchar* key, gpointer user_data)
    {
...
bool SettingGetterImplGSettings::CheckVersion(base::Environment* env)
{
    // CheckVersion() must be called *before* Init()!
    DCHECK(!client_);
...
public:
        ...
        , env_var_getter_(env_var_getter)
    {
        ...
        if (env_var_getter->GetVar("KDEHOME", &home) && !home.empty()) {
            ...
            if (desktop == base::nix::DESKTOP_ENVIRONMENT_KDE3) {
            ...
            } else if (desktop == base::nix::DESKTOP_ENVIRONMENT_KDE5 || desktop == base::nix::DESKTOP_ENVIRONMENT_KDE6) {
                ...
                if (env_var_getter_->GetVar("XDG_CONFIG_DIRS", &config_dirs)) {
                    auto dirs = base::SplitString(config_dirs, ":", base::KEEP_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
    ...
    bool SetUpNotifications(ProxyConfigServiceLinux::Delegate* delegate) override
    {
        ...
        for (const auto& kde_config_dir : kde_config_dirs_) {
            if (inotify_add_watch(inotify_fd_, kde_config_dir.value().c_str(), IN_MODIFY | IN_MOVED_TO) < 0) {
                ++failed_dirs;
            }
        }
        ...
        notify_delegate_ = delegate;
        inotify_watcher_
            = base::FileDescriptorWatcher::WatchReadable(inotify_fd_, base::BindRepeating(&SettingGetterImplKDE::OnChangeNotification, base::Unretained(this)));
        // Simulate a change to avoid possibly losing updates before this point.
        OnChangeNotification();
        return true;
    }
...
private:
    ...
    void AddKDESetting(const std::string& key, const std::string& value)
    {
        if (key == "ProxyType") {
            ...
            reversed_bypass_list_ = (value == "true" || StringToIntOrDefault(value, 0) != 0);
    ...
    void OnChangeNotification()
    {
        ...
        if (kioslaverc_touched) {
            ...
            // (In that case Stop() is a no-op.)
            debounce_timer_->Stop();
            debounce_timer_->Start(FROM_HERE, base::Milliseconds(kDebounceTimeoutMilliseconds), this, &SettingGetterImplKDE::OnDebouncedNotification);
        }
    }

    typedef std::map<StringSetting, std::string> string_map_type;
    typedef std::map<StringListSetting, std::vector<std::string>> strings_map_type;
...
bool ProxyConfigServiceLinux::Delegate::GetProxyFromSettings(SettingGetter::StringSetting host_key, ProxyServer* result_server)
{
    std::string host;
    ...
    ProxyServer proxy_server = ProxyUriToProxyServer(host, ProxyServer::SCHEME_HTTP);
...
std::optional<ProxyConfigWithAnnotation> ProxyConfigServiceLinux::Delegate::GetConfigFromSettings()
{
    ProxyConfig config;
    ...
    if (mode == "auto") {
        ...
        if (setting_getter_->GetString(SettingGetter::PROXY_AUTOCONF_URL, &pac_url_str)) {
    ...
    bool use_http_proxy;
    if (setting_getter_->GetBool(SettingGetter::PROXY_USE_HTTP_PROXY, &use_http_proxy) && !use_http_proxy) {
    ...
    if (setting_getter_->GetStringList(SettingGetter::PROXY_IGNORE_HOSTS, &ignore_hosts_list)) {
...
ProxyConfigServiceLinux::Delegate::Delegate(std::unique_ptr<base::Environment> env_var_getter, std::optional<std::unique_ptr<SettingGetter>> setting_getter,
    std::optional<NetworkTrafficAnnotationTag> traffic_annotation)
    : env_var_getter_(std::move(env_var_getter))
{
    if (traffic_annotation) {
        traffic_annotation_ = MutableNetworkTrafficAnnotationTag(traffic_annotation.value());
    }
    ...
    case base::nix::DESKTOP_ENVIRONMENT_KDE6:
        setting_getter_ = std::make_unique<SettingGetterImplKDE>(env_var_getter_.get());
        break;
...
void ProxyConfigServiceLinux::Delegate::SetUpAndFetchInitialConfig(const scoped_refptr<base::SingleThreadTaskRunner>& glib_task_runner,
    const scoped_refptr<base::SequencedTaskRunner>& main_task_runner, const NetworkTrafficAnnotationTag& traffic_annotation)
{
    ...
    if (cached_config_) {
        VLOG(1) << "Obtained proxy settings from annotation hash code " << cached_config_->traffic_annotation().unique_id_hash_code;
        ...
        if (main_task_runner.get()) {
            scoped_refptr<base::SequencedTaskRunner> required_loop = setting_getter_->GetNotificationTaskRunner();
            if (!required_loop.get() || required_loop->RunsTasksInCurrentSequence()) {
                ...
                required_loop->PostTask(FROM_HERE, base::BindOnce(&ProxyConfigServiceLinux::Delegate::SetUpNotifications, this));
            }
        }
    }
...
ProxyConfigService::ConfigAvailability ProxyConfigServiceLinux::Delegate::GetLatestProxyConfig(ProxyConfigWithAnnotation* config)
{
    // This is called from the main TaskRunner.
    DCHECK(!main_task_runner_.get() || main_task_runner_->RunsTasksInCurrentSequence());
...
void ProxyConfigServiceLinux::Delegate::OnCheckProxyConfigSettings()
{
    ...
    if (new_config.has_value() != reference_config_.has_value() || (new_config.has_value() && !new_config->value().Equals(reference_config_->value()))) {
        // Post a task to the main TaskRunner with the new configuration, so it can
        // update |cached_config_|.
        main_task_runner_->PostTask(FROM_HERE, base::BindOnce(&ProxyConfigServiceLinux::Delegate::SetNewProxyConfig, this, new_config));
...
void ProxyConfigServiceLinux::Delegate::SetNewProxyConfig(const std::optional<ProxyConfigWithAnnotation>& new_config)
{
    DCHECK(main_task_runner_->RunsTasksInCurrentSequence());
    VLOG(1) << "Proxy configuration changed";
    cached_config_ = new_config;
    for (auto& observer : observers_) {
        observer.OnProxyConfigChanged(GetConfigOrDirect(new_config), ProxyConfigService::CONFIG_VALID);
    }
}
...
void ProxyConfigServiceLinux::Delegate::PostDestroyTask()
{
    ...
    scoped_refptr<base::SequencedTaskRunner> shutdown_loop = setting_getter_->GetNotificationTaskRunner();
    if (!shutdown_loop.get() || shutdown_loop->RunsTasksInCurrentSequence()) {
        ...
        shutdown_loop->PostTask(FROM_HERE, base::BindOnce(&ProxyConfigServiceLinux::Delegate::OnDestroy, this));
    }
}
...
ProxyConfigServiceLinux::ProxyConfigServiceLinux()
    : delegate_(base::MakeRefCounted<Delegate>(base::Environment::Create(), std::nullopt, std::nullopt))
{
}
...
ProxyConfigServiceLinux::ProxyConfigServiceLinux(std::unique_ptr<base::Environment> env_var_getter, const NetworkTrafficAnnotationTag& traffic_annotation)
    : delegate_(base::MakeRefCounted<Delegate>(std::move(env_var_getter), std::nullopt, traffic_annotation))
{
}

ProxyConfigServiceLinux::ProxyConfigServiceLinux(
    std::unique_ptr<base::Environment> env_var_getter, std::unique_ptr<SettingGetter> setting_getter, const NetworkTrafficAnnotationTag& traffic_annotation)
    : delegate_(base::MakeRefCounted<Delegate>(std::move(env_var_getter), std::move(setting_getter), traffic_annotation))
{
}
...
ProxyConfigService::ConfigAvailability ProxyConfigServiceLinux::GetLatestProxyConfig(ProxyConfigWithAnnotation* config)
{
