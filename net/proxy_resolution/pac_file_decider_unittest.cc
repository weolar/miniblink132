...
public:
    ...
    {
        rules_.push_back(Rule(GURL(url), kFailedDownloading /*fetch_error*/, false));
    }
...
public:
    ...
    int Fetch(const GURL& url, std::u16string* text, CompletionOnceCallback callback, const NetworkTrafficAnnotationTag traffic_annotation) override
    {
...
public:
    ...
    int Fetch(std::u16string* utf16_text, CompletionOnceCallback callback, const NetLogWithSource& net_log,
...
int MockDhcpPacFileFetcher::Fetch(
    std::u16string* utf16_text, CompletionOnceCallback callback, const NetLogWithSource& net_log, const NetworkTrafficAnnotationTag traffic_annotation)
{
...
void MockDhcpPacFileFetcher::CompleteRequests(int result, const std::u16string& script)
{
    *utf16_text_ = script;
...
TEST(PacFileDeciderTest, CustomPacSucceeds)
{
    ...
    EXPECT_THAT(decider.Start(ProxyConfigWithAnnotation(config, TRAFFIC_ANNOTATION_FOR_TESTS), base::TimeDelta(), true, callback.callback()), IsOk());
    ...
    EXPECT_TRUE(LogContainsBeginEvent(entries, 0, NetLogEventType::PAC_FILE_DECIDER));
    EXPECT_TRUE(LogContainsBeginEvent(entries, 1, NetLogEventType::PAC_FILE_DECIDER_FETCH_PAC_SCRIPT));
    EXPECT_TRUE(LogContainsEndEvent(entries, 2, NetLogEventType::PAC_FILE_DECIDER_FETCH_PAC_SCRIPT));
    EXPECT_TRUE(LogContainsEndEvent(entries, 3, NetLogEventType::PAC_FILE_DECIDER));
...
TEST(PacFileDeciderTest, CustomPacFails1)
{
    ...
    RecordingNetLogObserver observer;
    PacFileDecider decider(&fetcher, &dhcp_fetcher, net::NetLog::Get());
    EXPECT_THAT(decider.Start(ProxyConfigWithAnnotation(config, TRAFFIC_ANNOTATION_FOR_TESTS), base::TimeDelta(), true, callback.callback()),
...
TEST(PacFileDeciderTest, CustomPacFails2)
{
    ...
    EXPECT_THAT(
        decider.Start(ProxyConfigWithAnnotation(config, TRAFFIC_ANNOTATION_FOR_TESTS), base::TimeDelta(), true, callback.callback()), IsError(kFailedParsing));
    EXPECT_FALSE(decider.script_data().data);
}
...
TEST(PacFileDeciderTest, HasNullPacFileFetcher)
{
    ...
    EXPECT_THAT(
        decider.Start(ProxyConfigWithAnnotation(config, TRAFFIC_ANNOTATION_FOR_TESTS), base::TimeDelta(), true, callback.callback()), IsError(ERR_UNEXPECTED));
    EXPECT_FALSE(decider.script_data().data);
}
...
class PacFileDeciderQuickCheckTest : public ::testing::Test, public WithTaskEnvironment {
public:
    ...
    void SetUp() override
    {
        ...
        decider_ = std::make_unique<PacFileDecider>(&fetcher_, &dhcp_fetcher_, nullptr);
    }

    int StartDecider()
    {
        return decider_->Start(ProxyConfigWithAnnotation(config_, TRAFFIC_ANNOTATION_FOR_TESTS), base::TimeDelta(), true, callback_.callback());
    }
...
TEST_F(PacFileDeciderQuickCheckTest, AsyncSuccess)
{
    ...
    EXPECT_EQ(fetcher_.isolation_info().network_anonymization_key(), host_resolver().request_network_anonymization_key(1));
...
TEST_F(PacFileDeciderQuickCheckTest, QuickCheckInhibitsDhcp)
{
    ...
    decider_ = std::make_unique<PacFileDecider>(&fetcher_, &dhcp_fetcher, nullptr);
    EXPECT_THAT(StartDecider(), IsError(ERR_IO_PENDING));
...
TEST_F(PacFileDeciderQuickCheckTest, QuickCheckDisabled)
{
    ...
    decider_ = std::make_unique<PacFileDecider>(&fetcher, &dhcp_fetcher_, nullptr);
    EXPECT_THAT(StartDecider(), IsError(ERR_IO_PENDING));
...
TEST(PacFileDeciderTest, AutodetectFailCustomSuccess2)
{
    ...
    EXPECT_TRUE(decider.effective_config().value().Equals(ProxyConfig::CreateFromCustomPacURL(GURL("http://custom/proxy.pac"))));
    ...
    EXPECT_TRUE(LogContainsBeginEvent(entries, 1, NetLogEventType::PAC_FILE_DECIDER_FETCH_PAC_SCRIPT));
    EXPECT_TRUE(LogContainsEndEvent(entries, 2, NetLogEventType::PAC_FILE_DECIDER_FETCH_PAC_SCRIPT));
    EXPECT_TRUE(LogContainsEvent(entries, 3, NetLogEventType::PAC_FILE_DECIDER_FALLING_BACK_TO_NEXT_PAC_SOURCE, NetLogEventPhase::NONE));
    // This is the DNS phase, which attempts a fetch but fails.
    EXPECT_TRUE(LogContainsBeginEvent(entries, 4, NetLogEventType::PAC_FILE_DECIDER_FETCH_PAC_SCRIPT));
    EXPECT_TRUE(LogContainsEndEvent(entries, 5, NetLogEventType::PAC_FILE_DECIDER_FETCH_PAC_SCRIPT));
    EXPECT_TRUE(LogContainsEvent(entries, 6, NetLogEventType::PAC_FILE_DECIDER_FALLING_BACK_TO_NEXT_PAC_SOURCE, NetLogEventPhase::NONE));
    // Finally, the custom PAC URL phase.
    EXPECT_TRUE(LogContainsBeginEvent(entries, 7, NetLogEventType::PAC_FILE_DECIDER_FETCH_PAC_SCRIPT));
    EXPECT_TRUE(LogContainsEndEvent(entries, 8, NetLogEventType::PAC_FILE_DECIDER_FETCH_PAC_SCRIPT));
    EXPECT_TRUE(LogContainsEndEvent(entries, 9, NetLogEventType::PAC_FILE_DECIDER));
}
...
TEST(PacFileDeciderTest, CustomPacFails1_WithPositiveDelay)
{
    ...
    RecordingNetLogObserver observer;
    PacFileDecider decider(&fetcher, &dhcp_fetcher, net::NetLog::Get());
    EXPECT_THAT(decider.Start(ProxyConfigWithAnnotation(config, TRAFFIC_ANNOTATION_FOR_TESTS), base::Milliseconds(1), true, callback.callback()),
    ...
    EXPECT_TRUE(LogContainsBeginEvent(entries, 0, NetLogEventType::PAC_FILE_DECIDER));
    EXPECT_TRUE(LogContainsBeginEvent(entries, 1, NetLogEventType::PAC_FILE_DECIDER_WAIT));
    EXPECT_TRUE(LogContainsEndEvent(entries, 2, NetLogEventType::PAC_FILE_DECIDER_WAIT));
    EXPECT_TRUE(LogContainsBeginEvent(entries, 3, NetLogEventType::PAC_FILE_DECIDER_FETCH_PAC_SCRIPT));
    EXPECT_TRUE(LogContainsEndEvent(entries, 4, NetLogEventType::PAC_FILE_DECIDER_FETCH_PAC_SCRIPT));
    EXPECT_TRUE(LogContainsEndEvent(entries, 5, NetLogEventType::PAC_FILE_DECIDER));
}
...
TEST(PacFileDeciderTest, CustomPacFails1_WithNegativeDelay)
{
    ...
    RecordingNetLogObserver observer;
    PacFileDecider decider(&fetcher, &dhcp_fetcher, net::NetLog::Get());
    EXPECT_THAT(decider.Start(ProxyConfigWithAnnotation(config, TRAFFIC_ANNOTATION_FOR_TESTS), base::Seconds(-5), true, callback.callback()),
        IsError(kFailedDownloading));
    EXPECT_FALSE(decider.script_data().data);
...
public:
    ...
    SynchronousSuccessDhcpFetcher& operator=(const SynchronousSuccessDhcpFetcher&) = delete;
    ...
    const std::u16string& expected_text() const
    {
...
TEST(PacFileDeciderTest, AutodetectDhcpSuccess)
{
    ...
    EXPECT_TRUE(decider.effective_config().value().has_pac_url());
    EXPECT_EQ(GURL("http://dhcppac/"), decider.effective_config().value().pac_url());
}
...
public:
        ...
        const NetworkTrafficAnnotationTag traffic_annotation) override
    {
        ...
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
            FROM_HERE, base::BindOnce(&AsyncFailDhcpFetcher::CallbackWithFailure, weak_ptr_factory_.GetWeakPtr()));
        return ERR_IO_PENDING;
    }
...
TEST(PacFileDeciderTest, DhcpCancelledByDestructor)
{
    ...
    // Scope so PacFileDecider gets destroyed early.
    {
        ...
        decider.Start(ProxyConfigWithAnnotation(config, TRAFFIC_ANNOTATION_FOR_TESTS), base::TimeDelta(), true, callback.callback());
    }
