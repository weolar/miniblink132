...
void RunLoopObserverCallBackFunc(CFRunLoopObserverRef observer, CFRunLoopActivity activity, void* info);
...
public:
    ...
    SynchronizedRunLoopObserver& operator=(const SynchronizedRunLoopObserver&) = delete;
    ...
    void RunLoopObserverCallBack(CFRunLoopObserverRef observer, CFRunLoopActivity activity);
...
SynchronizedRunLoopObserver::SynchronizedRunLoopObserver(base::Lock& lock)
    : lock_(lock)
{
    CFRunLoopObserverContext observer_context = { 0, this, nullptr, nullptr, nullptr };
    observer_.reset(CFRunLoopObserverCreate(
        kCFAllocatorDefault, kCFRunLoopBeforeSources | kCFRunLoopBeforeWaiting | kCFRunLoopExit, true, 0, RunLoopObserverCallBackFunc, &observer_context));
}
...
void SynchronizedRunLoopObserver::RemoveFromCurrentRunLoop(const CFStringRef mode)
{
    DCHECK(thread_checker_.CalledOnValidThread());
    CFRunLoopRemoveObserver(CFRunLoopGetCurrent(), observer_.get(), mode);
}

void SynchronizedRunLoopObserver::RunLoopObserverCallBack(CFRunLoopObserverRef observer, CFRunLoopActivity activity) NO_THREAD_SAFETY_ANALYSIS
{
    DCHECK(thread_checker_.CalledOnValidThread());
...
ProxyResolverApple::ProxyResolverApple(const scoped_refptr<PacFileData>& script_data)
...
int ProxyResolverApple::GetProxyForURL(const GURL& query_url, const NetworkAnonymizationKey& network_anonymization_key, ProxyInfo* results,
    CompletionOnceCallback /*callback*/, std::unique_ptr<Request>* /*request*/, const NetLogWithSource& net_log)
{
    ...
    if (query_url.SchemeIsWSOrWSS()) {
        ...
        replacements.SetSchemeStr(query_url.SchemeIsCryptographic() ? "https" : "http");
        mutable_query_url = query_url.ReplaceComponents(replacements);
    }

    base::apple::ScopedCFTypeRef<CFStringRef> query_ref(base::SysUTF8ToCFStringRef(mutable_query_url.spec()));
    base::apple::ScopedCFTypeRef<CFURLRef> query_url_ref(CFURLCreateWithString(kCFAllocatorDefault, query_ref.get(), nullptr));
    if (!query_url_ref.get())
        return ERR_FAILED;
    base::apple::ScopedCFTypeRef<CFStringRef> pac_ref(
        base::SysUTF8ToCFStringRef(script_data_->type() == PacFileData::TYPE_AUTO_DETECT ? std::string() : script_data_->url().spec()));
    base::apple::ScopedCFTypeRef<CFURLRef> pac_url_ref(CFURLCreateWithString(kCFAllocatorDefault, pac_ref.get(), nullptr));
    if (!pac_url_ref.get())
        return ERR_FAILED;
    ...
    base::apple::ScopedCFTypeRef<CFDictionaryRef> empty_dictionary(CFDictionaryCreate(nullptr, nullptr, nullptr, 0, nullptr, nullptr));
    base::apple::ScopedCFTypeRef<CFArrayRef> dummy_result(CFNetworkCopyProxiesForURL(query_url_ref.get(), empty_dictionary.get()));
    ...
    base::apple::ScopedCFTypeRef<CFRunLoopSourceRef> runloop_source(
        CFNetworkExecuteProxyAutoConfigurationURL(pac_url_ref.get(), query_url_ref.get(), ResultCallback, &context));
#if LEAK_SANITIZER
    ...
    const CFStringRef private_runloop_mode = CFSTR("org.chromium.ProxyResolverApple");
    ...
    // are added to the run loop concurrently.
    {
        ...
        CFRunLoopAddSource(CFRunLoopGetCurrent(), runloop_source.get(), private_runloop_mode);
    }
    ...
    {
        base::AutoLock lock(g_cfnetwork_pac_runloop_lock.Get());
        CFRunLoopRemoveSource(CFRunLoopGetCurrent(), runloop_source.get(), private_runloop_mode);
    }
    ...
    base::apple::ScopedCFTypeRef<CFArrayRef> proxy_array_ref(base::apple::CFCastStrict<CFArrayRef>(result));
    ...
    for (CFIndex i = 0; i < proxy_array_count; ++i) {
        CFDictionaryRef proxy_dictionary = base::apple::CFCastStrict<CFDictionaryRef>(CFArrayGetValueAtIndex(proxy_array_ref.get(), i));
        ...
        CFStringRef proxy_type = base::apple::GetValueFromDictionary<CFStringRef>(proxy_dictionary, kCFProxyTypeKey);
        ProxyChain proxy_chain = ProxyDictionaryToProxyChain(proxy_type, proxy_dictionary, kCFProxyHostNameKey, kCFProxyPortNumberKey);
