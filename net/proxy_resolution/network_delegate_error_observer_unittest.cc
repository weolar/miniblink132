...
public:
    ...
    bool got_pac_error() const
    {
        return got_pac_error_;
    }
...
TEST(NetworkDelegateErrorObserverTest, CallOnThread)
{
    ...
    NetworkDelegateErrorObserver observer(&network_delegate, base::SingleThreadTaskRunner::GetCurrentDefault().get());
    thread.task_runner()->PostTask(
        FROM_HERE, base::BindOnce(&NetworkDelegateErrorObserver::OnPACScriptError, base::Unretained(&observer), 42, std::u16string()));
    thread.Stop();
...
TEST(NetworkDelegateErrorObserverTest, NoDelegate)
{
    ...
    base::Thread thread("test_thread");
    thread.Start();
    NetworkDelegateErrorObserver observer(nullptr, base::SingleThreadTaskRunner::GetCurrentDefault().get());
    thread.task_runner()->PostTask(
        FROM_HERE, base::BindOnce(&NetworkDelegateErrorObserver::OnPACScriptError, base::Unretained(&observer), 42, std::u16string()));
    thread.Stop();
    base::RunLoop().RunUntilIdle();
