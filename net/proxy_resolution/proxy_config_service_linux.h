...
public:
    ...
    public:
        ...
        virtual bool Init(const scoped_refptr<base::SingleThreadTaskRunner>& glib_task_runner) = 0;
        ...
        virtual const scoped_refptr<base::SequencedTaskRunner>& GetNotificationTaskRunner() = 0;
        ...
        virtual bool GetStringList(StringListSetting key, std::vector<std::string>* result) = 0;
    ...
    public:
        ...
        Delegate(std::unique_ptr<base::Environment> env_var_getter, std::optional<std::unique_ptr<SettingGetter>> setting_getter,
        ...
        void SetUpAndFetchInitialConfig(const scoped_refptr<base::SingleThreadTaskRunner>& glib_task_runner,
        ...
        ProxyConfigService::ConfigAvailability GetLatestProxyConfig(ProxyConfigWithAnnotation* config);
    ...
    private:
        ...
        bool GetProxyFromEnvVarForScheme(std::string_view variable, ProxyServer::Scheme scheme, ProxyChain* result_chain);
        // As above but with scheme set to HTTP, for convenience.
        bool GetProxyFromEnvVar(std::string_view variable, ProxyChain* result_chain);
        ...
        // specification from it and puts it in result. Returns true if the
        // requested variable is defined and the value valid.
        bool GetProxyFromSettings(SettingGetter::StringSetting host_key, ProxyServer* result_server);
        ...
        void SetNewProxyConfig(const std::optional<ProxyConfigWithAnnotation>& new_config);
    ...
    explicit ProxyConfigServiceLinux(std::unique_ptr<base::Environment> env_var_getter, const NetworkTrafficAnnotationTag& traffic_annotation);
    ProxyConfigServiceLinux(std::unique_ptr<base::Environment> env_var_getter, std::unique_ptr<SettingGetter> setting_getter,
    ...
    void SetupAndFetchInitialConfig(const scoped_refptr<base::SingleThreadTaskRunner>& glib_task_runner,
        const scoped_refptr<base::SequencedTaskRunner>& main_task_runner, const NetworkTrafficAnnotationTag& traffic_annotation)
    {
        delegate_->SetUpAndFetchInitialConfig(glib_task_runner, main_task_runner, traffic_annotation);
    }
    void OnCheckProxyConfigSettings()
    {
    ...
    ProxyConfigService::ConfigAvailability GetLatestProxyConfig(ProxyConfigWithAnnotation* config) override;
