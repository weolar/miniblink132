...
class TransportSecurityPersisterTest : public ::testing::Test, public WithTaskEnvironment {
public:
    ...
    void SetUp() override
    {
        ...
        transport_security_file_path_ = temp_dir_.GetPath().AppendASCII("TransportSecurity");
        ...
        persister_ = std::make_unique<TransportSecurityPersister>(state_.get(), std::move(background_runner), transport_security_file_path_);
    }
...
TEST_F(TransportSecurityPersisterTest, SerializeData3)
{
    ...
    EXPECT_TRUE(base::ReadFileToString(transport_security_file_path_, &persisted));
...
TEST_F(TransportSecurityPersisterTest, DeserializeLegacyExpectCTData)
{
    ...
    const std::string kInput = R"({"version":2, "sts": [{ "host": ")" + kHost
    ...
    LOG(ERROR) << kInput;
    persister_->LoadEntries(kInput);
    FastForwardBy(TransportSecurityPersister::GetCommitInterval() + base::Seconds(1));
    EXPECT_EQ(1u, state_->num_sts_entries());
    // Now read the data and check that there are no Expect-CT entries.
    std::string persisted;
    ASSERT_TRUE(base::ReadFileToString(transport_security_file_path_, &persisted));
...
class TransportSecurityPersisterCommitTest : public TransportSecurityPersisterTest, public ::testing::WithParamInterface<std::string> {
public:
    TransportSecurityPersisterCommitTest()
    {
        if (GetParam().empty()) {
            ...
            feature_list_.InitAndEnableFeatureWithParameters(kTransportSecurityFileWriterSchedule, { { "commit_interval", GetParam() } });
        }
    }
...
INSTANTIATE_TEST_SUITE_P(All, TransportSecurityPersisterCommitTest,
    ::testing::Values(
...
TEST_P(TransportSecurityPersisterCommitTest, WriteAtCommitInterval)
{
    ...
    persister_->WriteNow(state_.get(), run_loop.QuitClosure());
    run_loop.Run();
    EXPECT_LT(base::TimeTicks::Now() - time_before_write, TransportSecurityPersister::GetCommitInterval());
    EXPECT_TRUE(base::ReadFileToString(transport_security_file_path_, &persisted));
    EXPECT_FALSE(persisted.empty());
