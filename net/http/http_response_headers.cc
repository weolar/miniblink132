...
const char* const kHopByHopResponseHeaders[] = { "connection", "proxy-connection", "keep-alive", "trailer", "transfer-encoding", "upgrade" };

// These headers are challenge response headers;
// not to be stored by caches.
const char* const kChallengeResponseHeaders[] = { "www-authenticate", "proxy-authenticate" };
...
// this list:
const char* const kNonUpdatedHeaderPrefixes[] = { "x-content-", "x-webkit-" };
...
int ParseStatus(std::string_view status, std::string& append_to)
{
    ...
    auto first_non_digit = std::ranges::find_if(status, [](char c) { return !base::IsAsciiDigit(c); });
    ...
    base::StringToInt(base::MakeStringPiece(status.begin(), first_non_digit), &response_code);
...
HttpResponseHeaders::Builder::Builder(HttpVersion version, std::string_view status)
    : version_(version)
    , status_(status)
{
    DCHECK(version == HttpVersion(1, 0) || version == HttpVersion(1, 1) || version == HttpVersion(2, 0));
}
...
{
    // BuilderPassKey, HttpVersion version, std::string_view status, base::span<const std::pair<std::string_view, std::string_view>> headers)
    base::span<const std::pair<std::string_view, std::string_view>> headers(headers_.data(), headers_.size());
    return base::MakeRefCounted<HttpResponseHeaders>(BuilderPassKey(), version_, status_, headers);
}
...
//         std::optional<HttpStatusCode> status_code = TryToGetHttpStatusCode(response_code_);
//         if (status_code.has_value()) {
//             UMA_HISTOGRAM_ENUMERATION("Net.HttpResponseCode2", status_code.value(), net::HttpStatusCode::HTTP_STATUS_CODE_MAX);
//         }
//     } else {
//         UMA_HISTOGRAM_CUSTOM_ENUMERATION("Net.HttpResponseCode", HttpUtil::MapStatusCodeForHistogram(response_code_),
    ...
    : http_version_(version)
{
    ...
    for (const auto& [key, value] : headers) {
        ...
        AddHeader(name_begin, name_end, values_begin, values_end, header_contains_comma[index] ? ContainsCommas::kYes : ContainsCommas::kNo);
        ++index;
    }
    raw_headers_.push_back('\0');
...
scoped_refptr<HttpResponseHeaders> HttpResponseHeaders::TryToCreate(std::string_view headers)
{
    // Reject strings with nulls.
    if (HasEmbeddedNulls(headers) || headers.size() > std::numeric_limits<int>::max()) {
        return nullptr;
    }

    return base::MakeRefCounted<HttpResponseHeaders>(HttpUtil::AssembleRawHeaders(headers));
}

scoped_refptr<HttpResponseHeaders> HttpResponseHeaders::TryToCreateForDataURL(std::string_view content_type)
{
    // Reject strings with nulls.
    if (HasEmbeddedNulls(content_type) || content_type.size() > std::numeric_limits<int>::max()) {
        return nullptr;
    }

    constexpr char kStatusLineAndHeaderName[] = "HTTP/1.1 200 OK\0Content-Type:";
    std::string raw_headers
        = base::StrCat({ std::string_view(kStatusLineAndHeaderName, sizeof(kStatusLineAndHeaderName) - 1), content_type, std::string_view("\0\0", 2) });

    return base::MakeRefCounted<HttpResponseHeaders>(raw_headers);
}

void HttpResponseHeaders::Persist(base::Pickle* pickle, PersistOptions options)
{
    ...
    for (size_t i = 0; i < parsed_.size(); ++i) {
        ...
        std::string header_name = base::ToLowerASCII(base::MakeStringPiece(parsed_[i].name_begin, parsed_[i].name_end));
...
{
    DCHECK(new_headers.response_code() == HTTP_NOT_MODIFIED || new_headers.response_code() == HTTP_PARTIAL_CONTENT);
    ...
    for (size_t i = 0; i < new_headers.parsed_.size(); ++i) {
        ...
        auto name = base::MakeStringPiece(new_parsed[i].name_begin, new_parsed[i].name_end);
...
void HttpResponseHeaders::MergeWithHeaders(std::string raw_headers, const HeaderSet& headers_to_remove)
{
    for (size_t i = 0; i < parsed_.size(); ++i) {
        DCHECK(!parsed_[i].is_continuation());
        ...
        std::string name = base::ToLowerASCII(base::MakeStringPiece(parsed_[i].name_begin, parsed_[i].name_end));
...
void HttpResponseHeaders::RemoveHeaders(const std::unordered_set<std::string>& header_names)
{
    // Copy up to the null byte.  This just copies the status line.
    std::string new_raw_headers(raw_headers_.c_str());
    new_raw_headers.push_back('\0');
...
void HttpResponseHeaders::RemoveHeaderLine(const std::string& name, const std::string& value)
{
...
void HttpResponseHeaders::AddHeader(std::string_view name, std::string_view value)
{
...
void HttpResponseHeaders::SetHeader(std::string_view name, std::string_view value)
{
    RemoveHeader(name);
...
void HttpResponseHeaders::UpdateWithNewRange(const HttpByteRange& byte_range, int64_t resource_size, bool replace_status_line)
{
    ...
    AddHeader(kRangeHeader, base::StringPrintf("bytes %" PRId64 "-%" PRId64 "/%" PRId64, start, end, resource_size));
...
void HttpResponseHeaders::Parse(const std::string& raw_input)
{
    ...
    HttpUtil::HeadersIterator headers(line_end + 1, raw_headers_.end(), std::string(1, '\0'));
    while (headers.GetNext()) {
        AddHeader(headers.name_begin(), headers.name_end(), headers.values_begin(), headers.values_end(), ContainsCommas::kMaybe);
    }
...
std::optional<std::string> HttpResponseHeaders::GetNormalizedHeader(std::string_view name) const
{
...
bool HttpResponseHeaders::EnumerateHeaderLines(size_t* iter, std::string* name, std::string* value) const
{
    size_t i = *iter;
    if (i == parsed_.size())
        return false;
...
std::optional<std::string_view> HttpResponseHeaders::EnumerateHeader(size_t* iter, std::string_view name) const
{
    size_t i;
    if (!iter || !*iter) {
        i = FindHeader(0, name);
...
bool HttpResponseHeaders::EnumerateHeader(size_t* iter, std::string_view name, std::string* value) const
{
...
bool HttpResponseHeaders::HasHeaderValue(std::string_view name, std::string_view value) const
{
...
// static
HttpVersion HttpResponseHeaders::ParseVersion(std::string::const_iterator line_begin, std::string::const_iterator line_end)
{
    ...
    if (!base::StartsWith(base::MakeStringPiece(line_begin, line_end), "http", base::CompareCase::INSENSITIVE_ASCII)) {
...
// Note: this implementation implicitly assumes that line_end points at a valid
// sentinel character (such as '\0').
void HttpResponseHeaders::ParseStatusLine(std::string::const_iterator line_begin, std::string::const_iterator line_end, bool has_headers)
{
    ...
    if (parsed_http_version != http_version_) {
        DVLOG(1) << "assuming HTTP/" << http_version_.major_value() << "." << http_version_.minor_value();
    }
    ...
    response_code_ = ParseStatus(base::MakeStringPiece(p + 1, line_end), raw_headers_);
}

size_t HttpResponseHeaders::FindHeader(size_t from, std::string_view search) const
{
    for (size_t i = from; i < parsed_.size(); ++i) {
        if (parsed_[i].is_continuation())
            continue;
        auto name = base::MakeStringPiece(parsed_[i].name_begin, parsed_[i].name_end);
...
std::optional<base::TimeDelta> HttpResponseHeaders::GetCacheControlDirective(std::string_view directive) const
{
    ...
    while ((value = EnumerateHeader(&iter, name))) {
        if (!base::StartsWith(*value, directive, base::CompareCase::INSENSITIVE_ASCII)) {
            continue;
        }
        ...
        if (start == end || !std::all_of(start, end, [](char c) { return '0' <= c && c <= '9'; })) {
            continue;
        }
        int64_t seconds = 0;
...
void HttpResponseHeaders::AddHeader(std::string::const_iterator name_begin, std::string::const_iterator name_end, std::string::const_iterator values_begin,
    std::string::const_iterator values_end, ContainsCommas contains_commas)
{
    // If the header can be coalesced, then we should split it up.
    if (values_begin == values_end || HttpUtil::IsNonCoalescingHeader(base::MakeStringPiece(name_begin, name_end)) || contains_commas == ContainsCommas::kNo) {
        ...
        while (it.GetNext()) {
            ...
            std::string::const_iterator sub_value_begin = values_begin + (it.value().data() - values.data());
            std::string::const_iterator sub_value_end = sub_value_begin + it.value().length();
...
void HttpResponseHeaders::AddToParsed(std::string::const_iterator name_begin, std::string::const_iterator name_end, std::string::const_iterator value_begin,
    std::string::const_iterator value_end)
{
    ParsedHeader header;
...
void HttpResponseHeaders::AddNonCacheableHeaders(HeaderSet* result) const
{
    ...
    while ((value = EnumerateHeader(&iter, kCacheControl))) {
        ...
        std::string_view remaining = value->substr(kPrefixLen, value->size() - kPrefixLen - 1);
        // Use base::KEEP_WHITESPACE despite trimming each item so can use the HTTP
        // definition of whitespace.
        std::vector<std::string_view> items = base::SplitStringPiece(remaining, /*separators=*/",", base::KEEP_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
...
void HttpResponseHeaders::GetMimeTypeAndCharset(std::string* mime_type, std::string* charset) const
{
    mime_type->clear();
    charset->clear();
...
bool HttpResponseHeaders::IsRedirect(std::string* location) const
{
    ...
    if (location) {
        auto location_strpiece = base::MakeStringPiece(parsed_[i].value_begin, parsed_[i].value_end);
...
bool HttpResponseHeaders::HasStorageAccessRetryHeader(const std::string* expected_origin) const
{
    *(int*)1 = 1;
    return false;

//     std::optional<std::string> header_value = GetNormalizedHeader(kActivateStorageAccessHeader);
//     if (!header_value) {
//         return false;
//     }
//     const std::optional<structured_headers::ParameterizedItem> item = structured_headers::ParseItem(*header_value);
//     if (!item || !item->item.is_token() || item->item.GetString() != "retry") {
//         return false;
//     }
//     return base::ranges::any_of(item->params, [&](const auto& key_and_value) -> bool {
...
//             return true;
//         }
//         return expected_origin && value.is_string() && value.GetString() == *expected_origin;
//     });
}
...
bool HttpResponseHeaders::IsRedirectResponseCode(int response_code)
{
    ...
    return (response_code == HTTP_MOVED_PERMANENTLY || response_code == HTTP_FOUND || response_code == HTTP_SEE_OTHER
        || response_code == HTTP_TEMPORARY_REDIRECT || response_code == HTTP_PERMANENT_REDIRECT);
}
...
//
ValidationType HttpResponseHeaders::RequiresValidation(const Time& request_time, const Time& response_time, const Time& current_time) const
{
    ...
    base::TimeDelta age = GetCurrentAge(request_time, response_time, current_time);
...
//
HttpResponseHeaders::FreshnessLifetimes HttpResponseHeaders::GetFreshnessLifetimes(const Time& response_time) const
{
    ...
    lifetimes.staleness = must_revalidate ? base::TimeDelta() : GetStaleWhileRevalidateValue().value_or(base::TimeDelta());
    ...
    if ((response_code_ == HTTP_OK || response_code_ == HTTP_NON_AUTHORITATIVE_INFORMATION || response_code_ == HTTP_PARTIAL_CONTENT) && !must_revalidate) {
    ...
    if (response_code_ == HTTP_MULTIPLE_CHOICES || response_code_ == HTTP_MOVED_PERMANENTLY || response_code_ == HTTP_PERMANENT_REDIRECT
...
//
base::TimeDelta HttpResponseHeaders::GetCurrentAge(const Time& request_time, const Time& response_time, const Time& current_time) const
{
    // If there is no Date header, then assume that the server response was
    // generated at the time when we received the response.
    Time date_value = GetDateValue().value_or(response_time);
    ...
    base::TimeDelta apparent_age = std::max(base::TimeDelta(), response_time - date_value);
    base::TimeDelta response_delay = response_time - request_time;
    base::TimeDelta corrected_age_value = age_value + response_delay;
    base::TimeDelta corrected_initial_age = std::max(apparent_age, corrected_age_value);
...
std::optional<base::TimeDelta> HttpResponseHeaders::GetStaleWhileRevalidateValue() const
{
    return GetCacheControlDirective("stale-while-revalidate");
}

std::optional<Time> HttpResponseHeaders::GetTimeValuedHeader(const std::string& name) const
{
    std::optional<std::string_view> value;
    ...
    if (base::FeatureList::IsEnabled(features::kTreatHTTPExpiresHeaderValueZeroAsExpired) && name == "Expires" && *value == "0") {
        return Time::Min();
    }
    ...
    Time result;
    return Time::FromUTCString(std::string(*value).c_str(), &result) ? std::make_optional(result) : std::nullopt;
}
...
bool HttpResponseHeaders::IsKeepAlive() const
{
    ...
    static const char* const kConnectionHeaders[] = { "connection", "proxy-connection" };
    struct KeepAliveToken {
        const char* const token;
        bool keep_alive;
    };
    static const KeepAliveToken kKeepAliveTokens[] = { { "keep-alive", true }, { "close", false } };
...
bool HttpResponseHeaders::HasStrongValidators() const
{
    return HttpUtil::HasStrongValidators(
        GetHttpVersion(), EnumerateHeader(nullptr, "etag"), EnumerateHeader(nullptr, "Last-Modified"), EnumerateHeader(nullptr, "Date"));
}

bool HttpResponseHeaders::HasValidators() const
{
    return HttpUtil::HasValidators(GetHttpVersion(), EnumerateHeader(nullptr, "etag"), EnumerateHeader(nullptr, "Last-Modified"));
}
...
int64_t HttpResponseHeaders::GetInt64HeaderValue(const std::string& header) const
{
    size_t iter = 0;
    std::optional<std::string_view> content_length = EnumerateHeader(&iter, header);
...
bool HttpResponseHeaders::GetContentRangeFor206(int64_t* first_byte_position, int64_t* last_byte_position, int64_t* instance_length) const
{
    size_t iter = 0;
    std::optional<std::string_view> content_range = EnumerateHeader(&iter, kContentRange);
    if (!content_range) {
        *first_byte_position = *last_byte_position = *instance_length = -1;
        return false;
    }

    return HttpUtil::ParseContentRangeHeaderFor206(*content_range, first_byte_position, last_byte_position, instance_length);
}

base::Value::Dict HttpResponseHeaders::NetLogParams(NetLogCaptureMode capture_mode) const
{
    *(int*)1 = 1;
...
bool HttpResponseHeaders::StrictlyEquals(const HttpResponseHeaders& other) const
{
    if (http_version_ != other.http_version_ || response_code_ != other.response_code_ || raw_headers_ != other.raw_headers_
        || parsed_.size() != other.parsed_.size()) {
        return false;
    }

    auto offsets_match = [&](std::string::const_iterator this_offset, std::string::const_iterator other_offset) {
        return this_offset - raw_headers_.begin() == other_offset - other.raw_headers_.begin();
    };
    return std::mismatch(parsed_.begin(), parsed_.end(), other.parsed_.begin(), [&](const ParsedHeader& lhs, const ParsedHeader& rhs) {
        return offsets_match(lhs.name_begin, rhs.name_begin) && offsets_match(lhs.name_end, rhs.name_end) && offsets_match(lhs.value_begin, rhs.value_begin)
