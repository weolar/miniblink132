    ...
    for (auto async : { true, false }) {
        for (auto target : { HttpAuth::AUTH_PROXY, HttpAuth::AUTH_SERVER }) {
            ...
            mock_handler.InitFromChallenge(
                &tokenizer, target, SSLInfo(), NetworkAnonymizationKey(), scheme_host_port, NetLogWithSource::Make(NetLogSourceType::NONE));
            mock_handler.SetGenerateExpectation(async, OK);
            mock_handler.GenerateAuthToken(&credentials, &request, test_callback.callback(), &auth_token);
            if (async)
            ...
            EXPECT_TRUE(LogContainsBeginEvent(entries, 0, NetLogEventType::AUTH_HANDLER_INIT));
            EXPECT_TRUE(LogContainsEndEvent(entries, 1, NetLogEventType::AUTH_HANDLER_INIT));
            EXPECT_TRUE(LogContainsBeginEvent(entries, 2, NetLogEventType::AUTH_GENERATE_TOKEN));
            EXPECT_TRUE(LogContainsEndEvent(entries, 3, NetLogEventType::AUTH_GENERATE_TOKEN));
            EXPECT_TRUE(LogContainsEntryWithType(entries, 4, NetLogEventType::AUTH_HANDLE_CHALLENGE));
        }
    }
}
