...
const MockTransaction kBasicTransaction = {
    ...
    .transport_info = TransportInfo(TransportType::kDirect, IPEndPoint(IPAddress::IPv4Localhost(), 80),
        /*accept_ch_frame_arg=*/"",
...
public:
    ...
    MockNetworkTransactionTest& operator=(const MockNetworkTransactionTest&) = delete;
...
TEST_F(MockNetworkTransactionTest, Basic)
{
    ...
    EXPECT_EQ(mock_transaction.transport_info.endpoint, transaction->GetResponseInfo()->remote_endpoint);
    ...
    EXPECT_EQ(std::string_view(mock_transaction.data), std::string_view(buf->data(), read_result));
}
...
TEST_F(MockNetworkTransactionTest, SyncNetStart)
{
    ...
    auto transaction = CreateNetworkTransaction();
    TestCompletionCallback start_callback;
    ASSERT_THAT(transaction->Start(&request, start_callback.callback(), NetLogWithSource()), test::IsError(OK));
...
TEST_F(MockNetworkTransactionTest, AsyncNetStartFailure)
{
    ...
    TestCompletionCallback start_callback;
    ASSERT_THAT(transaction->Start(&request, start_callback.callback(), NetLogWithSource()), test::IsError(ERR_IO_PENDING));
    EXPECT_THAT(start_callback.WaitForResult(), test::IsError(ERR_NETWORK_ACCESS_DENIED));
}
...
TEST_F(MockNetworkTransactionTest, SyncNetStartFailure)
{
    ...
    auto transaction = CreateNetworkTransaction();
    TestCompletionCallback start_callback;
    ASSERT_THAT(transaction->Start(&request, start_callback.callback(), NetLogWithSource()), test::IsError(ERR_NETWORK_ACCESS_DENIED));
}
...
TEST_F(MockNetworkTransactionTest, BeforeNetworkStartCallback)
{
    ...
    transaction->SetBeforeNetworkStartCallback(base::BindLambdaForTesting([&](bool* defer) { before_network_start_callback_called = true; }));
...
TEST_F(MockNetworkTransactionTest, BeforeNetworkStartCallbackDeferAndResume)
{
    ...
    auto transaction = CreateNetworkTransaction();
    bool before_network_start_callback_called = false;
    transaction->SetBeforeNetworkStartCallback(base::BindLambdaForTesting([&](bool* defer) {
...
TEST_F(MockNetworkTransactionTest, AsyncConnectedCallback)
{
    ...
    transaction->SetConnectedCallback(base::BindLambdaForTesting([&](const TransportInfo& info, CompletionOnceCallback callback) -> int {
...
TEST_F(MockNetworkTransactionTest, AsyncConnectedCallbackFailure)
{
    ...
    EXPECT_THAT(start_callback.WaitForResult(), test::IsError(ERR_INSUFFICIENT_RESOURCES));
}
...
TEST_F(MockNetworkTransactionTest, ModifyRequestHeadersCallback)
{
    ...
    transaction->SetModifyRequestHeadersCallback(base::BindLambdaForTesting([&](HttpRequestHeaders* request_headers) {
