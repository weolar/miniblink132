...
std::string HttpAuthHandlerDigest::DynamicNonceGenerator::GenerateNonce() const
{
...
HttpAuthHandlerDigest::FixedNonceGenerator::FixedNonceGenerator(const std::string& nonce)
    : nonce_(nonce)
{
}
...
void HttpAuthHandlerDigest::Factory::set_nonce_generator(std::unique_ptr<const NonceGenerator> nonce_generator)
{
    nonce_generator_ = std::move(nonce_generator);
}

int HttpAuthHandlerDigest::Factory::CreateAuthHandler(HttpAuthChallengeTokenizer* challenge, HttpAuth::Target target, const SSLInfo& ssl_info,
    ...
    const NetLogWithSource& net_log, HostResolver* host_resolver, std::unique_ptr<HttpAuthHandler>* handler)
{
    ...
    auto tmp_handler = base::WrapUnique(new HttpAuthHandlerDigest(digest_nonce_count, nonce_generator_.get()));
...
bool HttpAuthHandlerDigest::Init(HttpAuthChallengeTokenizer* challenge, const SSLInfo& ssl_info, const NetworkAnonymizationKey& network_anonymization_key)
{
    ...
    const AuthCredentials* credentials, const HttpRequestInfo* request, CompletionOnceCallback callback, std::string* auth_token)
{
    ...
    *auth_token = AssembleCredentials(method, path, *credentials, cnonce, nonce_count_);
    return OK;
}

HttpAuth::AuthorizationResult HttpAuthHandlerDigest::HandleAnotherChallengeImpl(HttpAuthChallengeTokenizer* challenge)
{
...
HttpAuthHandlerDigest::HttpAuthHandlerDigest(int nonce_count, const NonceGenerator* nonce_generator)
...
bool HttpAuthHandlerDigest::ParseChallenge(HttpAuthChallengeTokenizer* challenge)
{
...
bool HttpAuthHandlerDigest::ParseChallengeProperty(std::string_view name, std::string_view value)
{
...
void HttpAuthHandlerDigest::GetRequestMethodAndPath(const HttpRequestInfo* request, std::string* method, std::string* path) const
{
    DCHECK(request);
...
public:
    ...
    std::string HexDigest()
    {
        ...
        return base::ToLowerASCII(base::HexEncode(base::span(md_value).first(out_len_)));
    }
...
std::string HttpAuthHandlerDigest::AssembleResponseDigest(
    const std::string& method, const std::string& path, const AuthCredentials& credentials, const std::string& cnonce, const std::string& nc) const
{
    // ha1 = H(A1)
    DigestContext ha1_ctx(algorithm_);
    ha1_ctx.Update({ base::UTF16ToUTF8(credentials.username()), ":", original_realm_, ":", base::UTF16ToUTF8(credentials.password()) });
    std::string ha1 = ha1_ctx.HexDigest();

    if (algorithm_ == HttpAuthHandlerDigest::Algorithm::MD5_SESS || algorithm_ == HttpAuthHandlerDigest::Algorithm::SHA256_SESS) {
...
std::string HttpAuthHandlerDigest::AssembleCredentials(
    const std::string& method, const std::string& path, const AuthCredentials& credentials, const std::string& cnonce, int nonce_count) const
{
    ...
    std::string authorization = (std::string("Digest username=") + HttpUtil::Quote(username));
    ...
    std::string response = AssembleResponseDigest(method, path, credentials, cnonce, nc);
