...
void NET_EXPORT_PRIVATE SetTransportSecurityStateSourceForTesting(const TransportSecurityStateSource* source);
...
public:
    ...
    public:
        ...
        virtual void WriteNow(TransportSecurityState* state, base::OnceClosure callback) = 0;
    ...
    public:
        ...
        virtual CTRequirementLevel IsCTRequiredForHost(std::string_view hostname, const X509Certificate* chain, const HashValueVector& hashes) = 0;
    ...
    public:
        ...
        const HashedHost& hostname() const
        {
            return iterator_->first;
        }
        const STSState& domain_state() const
        {
    ...
    public:
        PinSet(std::string name, std::vector<std::vector<uint8_t>> static_spki_hashes, std::vector<std::vector<uint8_t>> bad_static_spki_hashes);
    ...
    struct NET_EXPORT PinSetInfo {
        ...
        PinSetInfo(std::string hostname, std::string pinset_name, bool include_subdomains);
    };
    ...
    explicit TransportSecurityState(std::vector<std::string> hsts_host_bypass_list);
    ...
    SSLUpgradeDecision GetSSLUpgradeDecision(const std::string& host, const NetLogWithSource& net_log = NetLogWithSource());
    ...
    bool ShouldUpgradeToSSL(const std::string& host, const NetLogWithSource& net_log = NetLogWithSource());
    PKPStatus CheckPublicKeyPins(const HostPortPair& host_port_pair, bool is_issued_by_known_root, const HashValueVector& hashes);
    ...
    CTRequirementsStatus CheckCTRequirements(const HostPortPair& host_port_pair, bool is_issued_by_known_root, const HashValueVector& public_key_hashes,
        const X509Certificate* validated_certificate_chain, ct::CTPolicyCompliance policy_compliance);
    ...
    void UpdatePinList(const std::vector<PinSet>& pinsets, const std::vector<PinSetInfo>& host_pins, base::Time update_time);
    ...
    // Note: This is only used for serializing/deserializing the
    // TransportSecurityState.
    void AddOrUpdateEnabledSTSHosts(const HashedHost& hashed_host, const STSState& state);
    ...
    void DeleteAllDynamicDataBetween(base::Time start_time, base::Time end_time, base::OnceClosure callback);
    ...
    void AddHSTS(std::string_view host, const base::Time& expiry, bool include_subdomains);

    // Adds explicitly-specified data as if it was processed from an HPKP header.
    // Note: dynamic PKP data is not persisted.
    void AddHPKP(std::string_view host, const base::Time& expiry, bool include_subdomains, const HashValueVector& hashes);
    ...
    // For unit tests only.
    void EnableStaticPinsForTesting()
    {
        enable_static_pins_ = true;
    }
    bool has_dynamic_pkp_state() const
    {
        return !enabled_pkp_hosts_.empty();
    }
...
private:
    ...
    PKPStatus CheckPublicKeyPinsImpl(const HostPortPair& host_port_pair, bool is_issued_by_known_root, const HashValueVector& hashes);
    ...
    void AddHSTSInternal(std::string_view host, STSState::UpgradeMode upgrade_mode, const base::Time& expiry, bool include_subdomains);
    void AddHPKPInternal(
    ...
    // pinsets_ vector.
    std::optional<std::map<std::string, std::pair<const PinSet*, bool>, std::less<>>> host_pins_;
