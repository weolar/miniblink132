...
public:
    ...
    // creates.
    void set_http_auth_preferences(const HttpAuthPreferences* http_auth_preferences)
    {
    ...
    virtual int CreateAuthHandler(HttpAuthChallengeTokenizer* challenge, HttpAuth::Target target, const SSLInfo& ssl_info,
        const NetworkAnonymizationKey& network_anonymization_key, const url::SchemeHostPort& scheme_host_port, CreateReason create_reason,
        int digest_nonce_count, const NetLogWithSource& net_log, HostResolver* host_resolver, std::unique_ptr<HttpAuthHandler>* handler)
        = 0;
    ...
    int CreateAuthHandlerFromString(std::string_view challenge, HttpAuth::Target target, const SSLInfo& ssl_info,
    ...
    // |challenge| and calls |CreateAuthHandler|. See |CreateAuthHandler| for
    // more details on return values.
    int CreatePreemptiveAuthHandlerFromString(const std::string& challenge, HttpAuth::Target target, const NetworkAnonymizationKey& network_anonymization_key,
        const url::SchemeHostPort& scheme_host_port, int digest_nonce_count, const NetLogWithSource& net_log, HostResolver* host_resolver,
    ...
    static std::unique_ptr<HttpAuthHandlerRegistryFactory> CreateDefault(const HttpAuthPreferences* prefs = nullptr
        ...
        ,
        HttpAuthMechanismFactory negotiate_auth_system_factory = HttpAuthMechanismFactory()
#endif
    );
...
public:
    explicit HttpAuthHandlerRegistryFactory(const HttpAuthPreferences* http_auth_preferences);

    HttpAuthHandlerRegistryFactory(const HttpAuthHandlerRegistryFactory&) = delete;
    HttpAuthHandlerRegistryFactory& operator=(const HttpAuthHandlerRegistryFactory&) = delete;

    ~HttpAuthHandlerRegistryFactory() override;

    // Sets the preferences into the factory associated with |scheme|.
    void SetHttpAuthPreferences(const std::string& scheme, const HttpAuthPreferences* prefs);
    ...
    // deleted.
    void RegisterSchemeFactory(const std::string& scheme, std::unique_ptr<HttpAuthHandlerFactory> factory);
    ...
    // |negotiate_auth_system_factory| is used to override the default auth system
    // used by the Negotiate authentication handler.
    static std::unique_ptr<HttpAuthHandlerRegistryFactory> Create(const HttpAuthPreferences* prefs
#if BUILDFLAG(USE_EXTERNAL_GSSAPI)
        ,
        const std::string& gssapi_library_name = ""
#endif
