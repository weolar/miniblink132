...
public:
    HttpBasicState(std::unique_ptr<StreamSocketHandle> connection, bool is_for_get_to_http_proxy);
    ...
    void Initialize(const HttpRequestInfo* request_info, RequestPriority priority, const NetLogWithSource& net_log);
    ...
    bool is_for_get_to_http_proxy() const
    {
        return is_for_get_to_http_proxy_;
    }

    StreamSocketHandle* connection() const
    {
