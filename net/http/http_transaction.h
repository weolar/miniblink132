...
public:
    ...
    using ConnectedCallback = base::RepeatingCallback<int(const TransportInfo& info, CompletionOnceCallback callback)>;
    ...
    virtual int Start(const HttpRequestInfo* request_info, CompletionOnceCallback callback, const NetLogWithSource& net_log) = 0;
    ...
    virtual int RestartWithAuth(const AuthCredentials& credentials, CompletionOnceCallback callback) = 0;
    ...
    virtual void SetWebSocketHandshakeStreamCreateHelper(WebSocketHandshakeStreamBase::CreateHelper* create_helper) = 0;

    // Sets the callback to receive notification just before network use.
    virtual void SetBeforeNetworkStartCallback(BeforeNetworkStartCallback callback) = 0;
    ...
    virtual void SetEarlyResponseHeadersCallback(ResponseHeadersCallback callback) = 0;
    ...
    virtual void SetModifyRequestHeadersCallback(base::RepeatingCallback<void(HttpRequestHeaders*)> callback) = 0;

    virtual void SetIsSharedDictionaryReadAllowedCallback(base::RepeatingCallback<bool()> callback) = 0;
