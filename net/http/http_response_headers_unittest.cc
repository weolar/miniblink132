...
protected:
    ...
    {
        DCHECK(headers_.get()) << "Call InitializeHeadersWithCacheControl() first";
        std::optional<TimeDelta> stale_while_revalidate_value = headers()->GetStaleWhileRevalidateValue();
...
class CommonHttpResponseHeadersTest : public HttpResponseHeadersTest, public ::testing::WithParamInterface<TestData> { };
...
INSTANTIATE_TEST_SUITE_P(HttpResponseHeaders, CommonHttpResponseHeadersTest, testing::ValuesIn(response_headers_tests));
...
class PersistenceTest : public HttpResponseHeadersTest, public ::testing::WithParamInterface<PersistData> { };
...
INSTANTIATE_TEST_SUITE_P(HttpResponseHeaders, PersistenceTest, testing::ValuesIn(persistence_tests));
...
TEST(HttpResponseHeadersTest, EnumerateHeader_Coalesced)
{
    ...
    EXPECT_EQ("no-cache=\"set-cookie,server\"", parsed->EnumerateHeader(&iter, "cache-control"));
    EXPECT_EQ("", parsed->EnumerateHeader(&iter, "cache-control"));
...
TEST(HttpResponseHeadersTest, EnumerateHeader_Challenge)
{
    ...
    size_t iter = 0;
    EXPECT_EQ("Digest realm=foobar, nonce=x, domain=y", parsed->EnumerateHeader(&iter, "WWW-Authenticate"));
    EXPECT_EQ("Basic realm=quatar", parsed->EnumerateHeader(&iter, "WWW-Authenticate"));
...
TEST(HttpResponseHeadersTest, EnumerateHeader_DateValued)
{
    ...
    EXPECT_EQ("Tue, 07 Aug 2007 23:10:55 GMT", parsed->EnumerateHeader(nullptr, "date"));
    EXPECT_EQ("Wed, 01 Aug 2007 23:23:45 GMT", parsed->EnumerateHeader(nullptr, "last-modified"));
...
TEST(HttpResponseHeadersTest, DefaultDateToGMT)
{
    ...
    ASSERT_TRUE(base::Time::FromString("Tue, 07 Aug 2007 23:10:55 GMT", &expected_value));
    ...
    EXPECT_THAT(parsed->GetExpiresValue(), testing::AnyOf(std::nullopt, expected_value));
}
...
TEST(HttpResponseHeadersTest, GetAgeValueOverflow)
{
    ...
    EXPECT_EQ(base::Seconds(static_cast<int64_t>(0xFFFFFFFFL)), parsed->GetAgeValue());
}
...
class ContentTypeTest : public HttpResponseHeadersTest, public ::testing::WithParamInterface<ContentTypeTestData> { };
...
const ContentTypeTestData mimetype_tests[] = {
    ...
    { "HTTP/1.1 200 OK\n"
      ...
      "Content-type: text/plain\n"
      "Content-type: text/html\n",
        "text/html", true, "", false, "text/plain, text/html, text/plain, text/html" },
    // Test charset parsing.
    { "HTTP/1.1 200 OK\n"
      "Content-type: text/html\n"
      "Content-type: text/html; charset=ISO-8859-1\n",
        "text/html", true, "iso-8859-1", true, "text/html, text/html; charset=ISO-8859-1" },
    ...
    { "HTTP/1.1 200 OK\n"
      "Content-type: text/html;charset=utf-8\n"
      "Content-type: text/html;charset=iso-8859-1\n",
        "text/html", true, "iso-8859-1", true, "text/html;charset=utf-8, text/html;charset=iso-8859-1" },
    ...
    { "HTTP/1.1 200 OK\n"
      "Content-type: text/html;charset=utf-8; charset=iso-8859-1\n",
        "text/html", true, "utf-8", true, "text/html;charset=utf-8; charset=iso-8859-1" },
    // Multiple params.
    { "HTTP/1.1 200 OK\n"
      "Content-type: text/html; foo=utf-8; charset=iso-8859-1\n",
        "text/html", true, "iso-8859-1", true, "text/html; foo=utf-8; charset=iso-8859-1" },
    { "HTTP/1.1 200 OK\n"
      "Content-type: text/html ; charset=utf-8 ; bar=iso-8859-1\n",
        "text/html", true, "utf-8", true, "text/html ; charset=utf-8 ; bar=iso-8859-1" },
    // Comma embeded in quotes.
    { "HTTP/1.1 200 OK\n"
      "Content-type: text/html ; charset=\"utf-8,text/plain\" ;\n",
        "text/html", true, "utf-8,text/plain", true, "text/html ; charset=\"utf-8,text/plain\" ;" },
...
INSTANTIATE_TEST_SUITE_P(HttpResponseHeaders, ContentTypeTest, testing::ValuesIn(mimetype_tests));
...
class RequiresValidationTest : public HttpResponseHeadersTest, public ::testing::WithParamInterface<RequiresValidationTestData> { };
...
TEST_P(RequiresValidationTest, RequiresValidation)
{
    ...
    ASSERT_TRUE(base::Time::FromString("Wed, 28 Nov 2007 00:40:09 GMT", &request_time));
    ASSERT_TRUE(base::Time::FromString("Wed, 28 Nov 2007 00:40:12 GMT", &response_time));
    ASSERT_TRUE(base::Time::FromString("Wed, 28 Nov 2007 00:45:20 GMT", &current_time));
...
INSTANTIATE_TEST_SUITE_P(HttpResponseHeaders, RequiresValidationTest, testing::ValuesIn(requires_validation_tests));
...
class UpdateTest : public HttpResponseHeadersTest, public ::testing::WithParamInterface<UpdateTestData> { };
...
INSTANTIATE_TEST_SUITE_P(HttpResponseHeaders, UpdateTest, testing::ValuesIn(update_tests));
...
class EnumerateHeaderLinesTest : public HttpResponseHeadersTest, public ::testing::WithParamInterface<EnumerateHeaderTestData> { };
...
class IsRedirectTest : public HttpResponseHeadersTest, public ::testing::WithParamInterface<IsRedirectTestData> { };
...
INSTANTIATE_TEST_SUITE_P(HttpResponseHeaders, IsRedirectTest, testing::ValuesIn(is_redirect_tests));
...
class HasStorageAccessRetryTest : public HttpResponseHeadersTest, public ::testing::WithParamInterface<HasStorageAccessRetryTestData> { };
...
TEST_P(HasStorageAccessRetryTest, HasStorageAccessRetry)
{
    ...
    EXPECT_EQ(parsed->HasStorageAccessRetryHeader(base::OptionalToPtr(test.expected_origin)), test.want_result);
}
...
INSTANTIATE_TEST_SUITE_P(HttpResponseHeaders, HasStorageAccessRetryTest, testing::ValuesIn(has_storage_access_retry_tests));
...
class GetContentLengthTest : public HttpResponseHeadersTest, public ::testing::WithParamInterface<ContentLengthTestData> { };
...
INSTANTIATE_TEST_SUITE_P(HttpResponseHeaders, GetContentLengthTest, testing::ValuesIn(content_length_tests));
...
class ContentRangeTest : public HttpResponseHeadersTest, public ::testing::WithParamInterface<ContentRangeTestData> { };
...
TEST_P(ContentRangeTest, GetContentRangeFor206)
{
    ...
    bool return_value = parsed->GetContentRangeFor206(&first_byte_position, &last_byte_position, &instance_size);
...
// fails.
void PrintTo(const KeepAliveTestData& keep_alive_test_data, std::ostream* os)
{
    *os << "{\"" << keep_alive_test_data.headers << "\", " << std::boolalpha << keep_alive_test_data.expected_keep_alive << "}";
}

class IsKeepAliveTest : public HttpResponseHeadersTest, public ::testing::WithParamInterface<KeepAliveTestData> { };
...
INSTANTIATE_TEST_SUITE_P(HttpResponseHeaders, IsKeepAliveTest, testing::ValuesIn(keepalive_tests));
...
class HasStrongValidatorsTest : public HttpResponseHeadersTest, public ::testing::WithParamInterface<HasStrongValidatorsTestData> { };
...
const HasStrongValidatorsTestData strong_validators_tests[] = { { "HTTP/0.9 200 OK", false },
    { "HTTP/1.0 200 OK\n"
...
INSTANTIATE_TEST_SUITE_P(HttpResponseHeaders, HasStrongValidatorsTest, testing::ValuesIn(strong_validators_tests));
...
{
    scoped_refptr<HttpResponseHeaders> headers = HttpResponseHeaders::TryToCreate("HTTP/1.1 200 OK\n"
                                                                                  "connection: keep-alive\n"
                                                                                  "Cache-control: max-age=10000\n");
    ASSERT_TRUE(headers);
...
{
    static constexpr char kHeadersWithNuls[] = { "HTTP/1.1 200 OK\0"
                                                 "Content-Type: application/octet-stream\0" };
    // The size must be specified explicitly to include the nul characters.
    static constexpr std::string_view kHeadersWithNulsAsStringPiece(kHeadersWithNuls, sizeof(kHeadersWithNuls));
    scoped_refptr<HttpResponseHeaders> headers = HttpResponseHeaders::TryToCreate(kHeadersWithNulsAsStringPiece);
...
TEST(HttpResponseHeadersTest, TracingSupport)
{
    ...
    EXPECT_EQ(perfetto::TracedValueToString(headers), "{response_code:200,headers:[{name:connection,value:keep-alive}]}");
}
#endif
...
class RemoveHeaderTest : public HttpResponseHeadersTest, public ::testing::WithParamInterface<RemoveHeaderTestData> { };
...
INSTANTIATE_TEST_SUITE_P(HttpResponseHeaders, RemoveHeaderTest, testing::ValuesIn(remove_header_tests));
...
class RemoveHeadersTest : public HttpResponseHeadersTest, public ::testing::WithParamInterface<RemoveHeadersTestData> { };
...
INSTANTIATE_TEST_SUITE_P(HttpResponseHeaders, RemoveHeadersTest, testing::ValuesIn(remove_headers_tests));
...
class RemoveIndividualHeaderTest : public HttpResponseHeadersTest, public ::testing::WithParamInterface<RemoveIndividualHeaderTestData> { };
...
INSTANTIATE_TEST_SUITE_P(HttpResponseHeaders, RemoveIndividualHeaderTest, testing::ValuesIn(remove_individual_header_tests));
...
class ReplaceStatusTest : public HttpResponseHeadersTest, public ::testing::WithParamInterface<ReplaceStatusTestData> { };
...
INSTANTIATE_TEST_SUITE_P(HttpResponseHeaders, ReplaceStatusTest, testing::ValuesIn(replace_status_tests));
...
class UpdateWithNewRangeTest : public HttpResponseHeadersTest, public ::testing::WithParamInterface<UpdateWithNewRangeTestData> { };
...
TEST_P(UpdateWithNewRangeTest, UpdateWithNewRange)
{
    ...
    EXPECT_EQ(std::string(test.expected_headers_with_replaced_status), ToSimpleString(parsed));
}
...
INSTANTIATE_TEST_SUITE_P(HttpResponseHeaders, UpdateWithNewRangeTest, testing::ValuesIn(update_range_tests));
...
TEST_F(HttpResponseHeadersCacheControlTest, MaxAgeWithSpaceBeforeEqualsIsRejected)
{
    InitializeHeadersWithCacheControl("max-age = 7");
    EXPECT_FALSE(headers()->GetMaxAgeValue());
}

TEST_F(HttpResponseHeadersCacheControlTest, MaxAgeWithLeadingandTrailingSpaces)
{
    InitializeHeadersWithCacheControl("max-age= 7  ");
...
TEST_F(HttpResponseHeadersCacheControlTest, MaxAgeBogusFirstMatchUsed)
{
    ...
    InitializeHeadersWithCacheControl("max-age10, max-age=now, max-age=20, max-age=30");
...
TEST_F(HttpResponseHeadersCacheControlTest, MaxAgeOverflow)
{
    ...
    EXPECT_EQ(base::TimeDelta::FiniteMax().InSeconds(), GetMaxAgeValue().InSeconds());
}
...
class MaxAgeEdgeCasesTest : public HttpResponseHeadersCacheControlTest, public ::testing::WithParamInterface<MaxAgeTestData> { };
...
TEST_P(MaxAgeEdgeCasesTest, MaxAgeEdgeCases)
{
    ...
    InitializeHeadersWithCacheControl((max_age + test.max_age_string).c_str());
    if (test.expected_seconds.has_value()) {
        EXPECT_EQ(test.expected_seconds.value(), GetMaxAgeValue().InSeconds()) << " for max-age=" << test.max_age_string;
    } else {
        EXPECT_FALSE(headers()->GetMaxAgeValue());
    }
}
...
const MaxAgeTestData max_age_tests[] = {
    ...
    { "-1", std::nullopt }, { "--1", std::nullopt }, { "2s", std::nullopt }, { "3 days", std::nullopt }, { "'4'", std::nullopt }, { "\"5\"", std::nullopt },
    ...
    { "9223372036853", 9223372036853 }, { "9223372036854", 9223372036854 }, { "9223372036855", 9223372036854 }, { "9223372036854775806", 9223372036854 },
    { "9223372036854775807", 9223372036854 }, { "20000000000000000000", 9223372036854 }, // Overflow int64_t.
};

INSTANTIATE_TEST_SUITE_P(HttpResponseHeadersCacheControl, MaxAgeEdgeCasesTest, testing::ValuesIn(max_age_tests));

TEST_F(HttpResponseHeadersCacheControlTest, AbsentStaleWhileRevalidateReturnsFalse)
{
    InitializeHeadersWithCacheControl("max-age=3600");
    EXPECT_FALSE(headers()->GetStaleWhileRevalidateValue());
}

TEST_F(HttpResponseHeadersCacheControlTest, StaleWhileRevalidateWithoutValueRejected)
{
...
TEST_F(HttpResponseHeadersCacheControlTest, FirstStaleWhileRevalidateValueUsed)
{
    InitializeHeadersWithCacheControl("stale-while-revalidate=1,stale-while-revalidate=7200");
...
class GetCurrentAgeTest : public HttpResponseHeadersTest, public ::testing::WithParamInterface<GetCurrentAgeTestData> { };
...
TEST_P(GetCurrentAgeTest, GetCurrentAge)
{
    ...
    base::TimeDelta age = parsed->GetCurrentAge(request_time, response_time, current_time);
...
const struct GetCurrentAgeTestData get_current_age_tests[] = {
    ...
    { "HTTP/1.1 200 OK\n"
      "Age: 2",
        "Fri, 20 Jan 2011 10:40:08 GMT", "Fri, 20 Jan 2011 10:40:12 GMT", "Fri, 20 Jan 2011 10:40:14 GMT", 8 },
    // Without Age header.
    { "HTTP/1.1 200 OK\n"
      "Date: Fri, 20 Jan 2011 10:40:10 GMT\n",
        "Fri, 20 Jan 2011 10:40:08 GMT", "Fri, 20 Jan 2011 10:40:12 GMT", "Fri, 20 Jan 2011 10:40:14 GMT", 6 },
    ...
    { "HTTP/1.1 200 OK\n"
      ...
      "Age: 0\n",
        "Fri, 20 Jan 2011 10:40:08 GMT", "Fri, 20 Jan 2011 10:40:12 GMT", "Fri, 20 Jan 2011 10:40:14 GMT", 7 }
};
...
{
    for (HttpVersion version : { HttpVersion(1, 0), HttpVersion(1, 1), HttpVersion(2, 0) }) {
        auto headers = HttpResponseHeaders::Builder(version, "200").Build();
        EXPECT_EQ(base::StringPrintf("HTTP/%d.%d 200", version.major_value(), version.minor_value()), headers->GetStatusLine());
...
{
    *os << "\"" << data.status << "\", \"" << data.expected_status_line << "\", " << data.expected_response_code << ", \"" << data.expected_status_text
        << "\"}";
}

class BuilderStatusLineTest : public HttpResponseHeadersTest, public ::testing::WithParamInterface<BuilderStatusLineTestData> { };

TEST_P(BuilderStatusLineTest, Common)
{
    const auto& [status, expected_status_line, expected_response_code, expected_status_text] = GetParam();

    auto http_response_headers = HttpResponseHeaders::Builder({ 1, 1 }, status).Build();
...
INSTANTIATE_TEST_SUITE_P(HttpResponseHeaders, BuilderStatusLineTest, testing::ValuesIn(kBuilderStatusLineTests));
...
class BuilderHeadersTest : public HttpResponseHeadersTest, public ::testing::WithParamInterface<BuilderHeadersTestData> { };
...
INSTANTIATE_TEST_SUITE_P(HttpResponseHeaders, BuilderHeadersTest, testing::ValuesIn(builder_headers_tests));
