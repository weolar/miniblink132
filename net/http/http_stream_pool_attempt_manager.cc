...
constexpr NextProtoSet kTcpBasedProtocols = { NextProto::kProtoUnknown, NextProto::kProtoHTTP11, NextProto::kProtoHTTP2 };

constexpr NextProtoSet kQuicBasedProtocols = { NextProto::kProtoUnknown, NextProto::kProtoQUIC };

StreamSocketHandle::SocketReuseType GetReuseTypeFromIdleStreamSocket(const StreamSocket& stream_socket)
{
    return stream_socket.WasEverUsed() ? StreamSocketHandle::SocketReuseType::kReusedIdle : StreamSocketHandle::SocketReuseType::kUnusedIdle;
}

} // namespace

// Represents an in-flight stream attempt.
class HttpStreamPool::AttemptManager::InFlightAttempt : public TlsStreamAttempt::SSLConfigProvider {
public:
    explicit InFlightAttempt(AttemptManager* manager)
        : manager_(manager)
    {
    }
    ...
    int Start(std::unique_ptr<StreamAttempt> attempt)
    {
        ...
        return attempt_->Start(base::BindOnce(&AttemptManager::OnInFlightAttemptComplete, base::Unretained(manager_), this));
    }
    ...
    void set_is_aborted(bool is_aborted)
    {
        is_aborted_ = is_aborted;
    }
    ...
    base::expected<SSLConfig, TlsStreamAttempt::GetSSLConfigError> GetSSLConfig() override
    {
...
HttpStreamPool::AttemptManager::AttemptManager(Group* group, NetLog* net_log)
    : group_(group)
    , net_log_(NetLogWithSource::Make(net_log, NetLogSourceType::HTTP_STREAM_POOL_ATTEMPT_MANAGER))
...
{
    CHECK(group_);
    net_log_.BeginEvent(NetLogEventType::HTTP_STREAM_POOL_ATTEMPT_MANAGER_ALIVE, [&] {
        base::Value::Dict dict;
        dict.Set("stream_attempt_delay", static_cast<int>(stream_attempt_delay_.InMilliseconds()));
        group_->net_log().source().AddToEventParameters(dict);
        return dict;
    });
    group_->net_log().AddEventReferencingSource(NetLogEventType::HTTP_STREAM_POOL_GROUP_ATTEMPT_MANAGER_CREATED, net_log_.source());
}

HttpStreamPool::AttemptManager::~AttemptManager()
{
    net_log().EndEvent(NetLogEventType::HTTP_STREAM_POOL_ATTEMPT_MANAGER_ALIVE);
    group_->net_log().AddEventReferencingSource(NetLogEventType::HTTP_STREAM_POOL_GROUP_ATTEMPT_MANAGER_DESTROYED, net_log_.source());
}

void HttpStreamPool::AttemptManager::StartJob(Job* job, RequestPriority priority, const std::vector<SSLConfig::CertAndStatus>& allowed_bad_certs,
    RespectLimits respect_limits, bool enable_ip_based_pooling, bool enable_alternative_services, quic::ParsedQuicVersion quic_version,
    const NetLogWithSource& net_log)
{
    MaybeUpdateQuicVersionWhenForced(quic_version);
    net_log_.AddEvent(NetLogEventType::HTTP_STREAM_POOL_ATTEMPT_MANAGER_START_JOB, [&] {
        base::Value::Dict dict;
    ...
    net_log.AddEventReferencingSource(NetLogEventType::HTTP_STREAM_POOL_ATTEMPT_MANAGER_JOB_BOUND, net_log_.source());
    ...
    CHECK(!spdy_session_);
    DCHECK(!spdy_session_pool()->FindAvailableSession(spdy_session_key(), enable_ip_based_pooling_,
    ...
    if (is_failing_) {
        ...
        base::SequencedTaskRunner::GetCurrentDefault()->PostTask(
            FROM_HERE, base::BindOnce(&AttemptManager::NotifyJobOfFailure, weak_ptr_factory_.GetWeakPtr()));
        return;
    }
    ...
    if (stream_socket) {
        ...
        const StreamSocketHandle::SocketReuseType reuse_type = GetReuseTypeFromIdleStreamSocket(*stream_socket);
        ...
        // per-group limit.
        CreateTextBasedStreamAndNotify(std::move(stream_socket), reuse_type, LoadTimingInfo::ConnectTiming());
        return;
    }
...
int HttpStreamPool::AttemptManager::Preconnect(size_t num_streams, quic::ParsedQuicVersion quic_version, CompletionOnceCallback callback)
{
    MaybeUpdateQuicVersionWhenForced(quic_version);
    net_log_.AddEvent(NetLogEventType::HTTP_STREAM_POOL_ATTEMPT_MANAGER_PRECONNECT, [&] {
        base::Value::Dict dict;
    ...
    auto entry = std::make_unique<PreconnectEntry>(num_streams, std::move(callback));
...
bool HttpStreamPool::AttemptManager::IsSvcbOptional()
{
    ...
    base::span<const ServiceEndpoint> endpoints = service_endpoint_request_->GetEndpointResults();
    return !HostResolver::AllProtocolEndpointsHaveEch(endpoints);
}

int HttpStreamPool::AttemptManager::WaitForSSLConfigReady(CompletionOnceCallback callback)
{
...
base::expected<SSLConfig, TlsStreamAttempt::GetSSLConfigError> HttpStreamPool::AttemptManager::GetSSLConfig(InFlightAttempt* attempt)
{
    ...
    for (auto& endpoint : service_endpoint_request_->GetEndpointResults()) {
        ...
        const std::vector<IPEndPoint>& ip_endpoints = attempt->ip_endpoint().address().IsIPv4() ? endpoint.ipv4_endpoints : endpoint.ipv6_endpoints;
...
void HttpStreamPool::AttemptManager::OnJobComplete(Job* job)
{
    ...
    if (notified_it != notified_jobs_.end()) {
        ...
        for (JobQueue::Pointer pointer = jobs_.FirstMax(); !pointer.is_null(); pointer = jobs_.GetNextTowardsLastMin(pointer)) {
...
const QuicSessionAliasKey& HttpStreamPool::AttemptManager::quic_session_alias_key() const
{
...
{
    return pool()->RequiresHTTP11(stream_key().destination(), stream_key().network_anonymization_key());
}
...
LoadState HttpStreamPool::AttemptManager::GetLoadState() const
{
    ...
    for (const auto& in_flight_attempt : in_flight_attempts_) {
        load_state = std::max(load_state, in_flight_attempt->attempt()->GetLoadState());
...
void HttpStreamPool::AttemptManager::OnQuicTaskComplete(int rv, NetErrorDetails details)
{
    ...
    net_log().AddEvent(NetLogEventType::HTTP_STREAM_POOL_ATTEMPT_MANAGER_QUIC_TASK_COMPLETED, [&] {
    ...
    if (rv != OK && (tcp_based_attempt_state_ == TcpBasedAttemptState::kAllAttemptsFailed || group_->force_quic())) {
        error_to_notify_ = rv;
        NotifyFailure();
        return;
    }
    ...
    if (rv != OK || should_block_stream_attempt_) {
        ...
        base::SequencedTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(&AttemptManager::MaybeComplete, weak_ptr_factory_.GetWeakPtr()));
    }
}
...
base::Value::Dict HttpStreamPool::AttemptManager::GetInfoAsValue()
{
    ...
    dict.Set("pending_preconnect_count", static_cast<int>(PendingPreconnectCount()));
...
MultiplexedSessionCreationInitiator HttpStreamPool::AttemptManager::CalculateMultiplexedSessionCreationInitiator()
{
...
void HttpStreamPool::AttemptManager::ResolveServiceEndpoint(RequestPriority initial_priority)
{
    CHECK(!service_endpoint_request_);
    HostResolver::ResolveHostParameters parameters;
    parameters.initial_priority = initial_priority;
    parameters.secure_dns_policy = stream_key().secure_dns_policy();
    service_endpoint_request_ = http_network_session()->host_resolver()->CreateServiceEndpointRequest(
        HostResolver::Host(stream_key().destination()), stream_key().network_anonymization_key(), net_log(), std::move(parameters));
...
void HttpStreamPool::AttemptManager::RestrictAllowedProtocols(NextProtoSet allowed_alpns)
{
...
void HttpStreamPool::AttemptManager::MaybeChangeServiceEndpointRequestPriority()
{
    if (service_endpoint_request_ && !service_endpoint_request_finished_) {
...
bool HttpStreamPool::AttemptManager::CanUseExistingSessionAfterEndpointChanges()
{
    CHECK(service_endpoint_request_);
    ...
    if (CanUseQuic()) {
        for (const auto& endpoint : service_endpoint_request_->GetEndpointResults()) {
            if (quic_session_pool()->HasMatchingIpSessionForServiceEndpoint(
                    quic_session_alias_key(), endpoint, service_endpoint_request_->GetDnsAliasResults(), true)) {
                if (quic_task_) {
                ...
                base::SequencedTaskRunner::GetCurrentDefault()->PostTask(
                    FROM_HERE, base::BindOnce(&AttemptManager::CreateQuicStreamAndNotify, weak_ptr_factory_.GetWeakPtr()));
                return true;
            }
        }
    }
    ...
    for (const auto& endpoint : service_endpoint_request_->GetEndpointResults()) {
        spdy_session_
            = spdy_session_pool()->FindMatchingIpSessionForServiceEndpoint(spdy_session_key(), endpoint, service_endpoint_request_->GetDnsAliasResults());
        if (spdy_session_) {
            HandleSpdySessionReady();
            // Use PostTask() because we could reach here from RequestStream()
            // synchronously when the DNS resolution finishes immediately.
            base::SequencedTaskRunner::GetCurrentDefault()->PostTask(
                FROM_HERE, base::BindOnce(&AttemptManager::CreateSpdyStreamAndNotify, weak_ptr_factory_.GetWeakPtr()));
            return true;
        }
    }
...
{
    if (!should_block_stream_attempt_ || stream_attempt_delay_timer_.IsRunning()) {
        return;
    }
    CHECK(!stream_attempt_delay_.is_zero());
    stream_attempt_delay_timer_.Start(
        FROM_HERE, stream_attempt_delay_, base::BindOnce(&AttemptManager::OnStreamAttemptDelayPassed, weak_ptr_factory_.GetWeakPtr()));
}
...
void HttpStreamPool::AttemptManager::MaybeCalculateSSLConfig()
{
    ...
    ssl_config.disable_cert_verification_network_fetches = stream_key().disable_cert_network_fetches();
    ssl_config.early_data_enabled = http_network_session()->params().enable_early_data;

    ssl_config.alpn_protos = http_network_session()->GetAlpnProtos();
    ssl_config.application_settings = http_network_session()->GetApplicationSettings();
    http_network_session()->http_server_properties()->MaybeForceHTTP11(stream_key().destination(), stream_key().network_anonymization_key(), &ssl_config);

    ssl_config.ignore_certificate_errors = http_network_session()->params().ignore_certificate_errors;
    ssl_config.network_anonymization_key = stream_key().network_anonymization_key();
    ...
    for (auto& in_flight_attempt : in_flight_attempts_) {
        if (!in_flight_attempt->is_slow() && !in_flight_attempt->slow_timer().IsRunning()) {
            ...
            in_flight_attempt->slow_timer().Start(
                FROM_HERE, kConnectionAttemptDelay, base::BindOnce(&AttemptManager::OnInFlightAttemptSlow, base::Unretained(this), in_flight_attempt.get()));
        }
    }
...
{
    CHECK(service_endpoint_request_);
    if (!CanUseQuic() || quic_task_result_.has_value() || !service_endpoint_request_->EndpointsCryptoReady()) {
        return;
    }
...
void HttpStreamPool::AttemptManager::MaybeAttemptConnection(std::optional<size_t> max_attempts)
{
    ...
    if (CanUseQuic() && quic_task_result_.has_value() && *quic_task_result_ == OK) {
        return;
    }
    ...
    if (!ip_endpoint.has_value()) {
        ...
        if (tcp_based_attempt_state_ == TcpBasedAttemptState::kAllAttemptsFailed && !quic_task_) {
            // Tried all endpoints.
            MaybeMarkQuicBroken();
            NotifyFailure();
        }
    ...
    while (IsConnectionAttemptReady()) {
        ...
        if (using_tls) {
            attempt = std::make_unique<TlsStreamAttempt>(pool()->stream_attempt_params(), *ip_endpoint,
            ...
            attempt = std::make_unique<TcpStreamAttempt>(pool()->stream_attempt_params(), *ip_endpoint);
        }

        net_log().AddEventReferencingSource(NetLogEventType::HTTP_STREAM_POOL_ATTEMPT_MANAGER_ATTEMPT_START, attempt->net_log().source());
        net_log().AddEvent(NetLogEventType::HTTP_STREAM_POOL_ATTEMPT_MANAGER_ATTEMPT_START, [&] {
            base::Value::Dict dict = GetStatesAsNetLogParams();
        ...
        raw_attempt->attempt()->net_log().AddEventReferencingSource(NetLogEventType::STREAM_ATTEMPT_BOUND_TO_POOL, net_log().source());
        if (rv != ERR_IO_PENDING) {
            base::SequencedTaskRunner::GetCurrentDefault()->PostTask(
                FROM_HERE, base::BindOnce(&AttemptManager::OnInFlightAttemptComplete, base::Unretained(this), raw_attempt, rv));
        } else {
            raw_attempt->slow_timer().Start(
                FROM_HERE, kConnectionAttemptDelay, base::BindOnce(&AttemptManager::OnInFlightAttemptSlow, base::Unretained(this), raw_attempt));
            if (tls_attempt_ptr && !tls_attempt_ptr->IsTcpHandshakeCompleted()) {
                tls_attempt_ptr->SetTcpHandshakeCompletionCallback(
                    base::BindOnce(&AttemptManager::OnInFlightAttemptTcpHandshakeComplete, base::Unretained(this), raw_attempt));
            }
        }
...
bool HttpStreamPool::AttemptManager::IsConnectionAttemptReady()
{
    ...
    case CanAttemptResult::kThrottledForSpdy:
        ...
        if (!spdy_throttle_timer_.IsRunning()) {
            spdy_throttle_timer_.Start(FROM_HERE, kSpdyThrottleDelay, base::BindOnce(&AttemptManager::OnSpdyThrottleDelayPassed, base::Unretained(this)));
        }
        return false;
    case CanAttemptResult::kReachedGroupLimit:
...
HttpStreamPool::AttemptManager::CanAttemptResult HttpStreamPool::AttemptManager::CanAttemptConnection()
{
...
{
    if (!http_network_session()->http_server_properties()->GetSupportsSpdy(stream_key().destination(), stream_key().network_anonymization_key())) {
        return false;
    }
...
size_t HttpStreamPool::AttemptManager::PendingCountInternal(size_t pending_count) const
{
...
std::optional<IPEndPoint> HttpStreamPool::AttemptManager::GetIPEndPointToAttempt()
{
    if (!service_endpoint_request_ || service_endpoint_request_->GetEndpointResults().empty()) {
        return std::nullopt;
    }
    ...
    for (auto& endpoint : service_endpoint_request_->GetEndpointResults()) {
        ...
        std::optional<IPEndPoint> ip_endpoint
            = prefer_ipv6_ ? FindPreferredIPEndpoint(endpoint.ipv6_endpoints) : FindPreferredIPEndpoint(endpoint.ipv4_endpoints);
        if (ip_endpoint.has_value()) {
            return ip_endpoint;
        }
    }
    ...
    for (auto& endpoint : service_endpoint_request_->GetEndpointResults()) {
        ...
        std::optional<IPEndPoint> ip_endpoint
            = prefer_ipv6_ ? FindPreferredIPEndpoint(endpoint.ipv4_endpoints) : FindPreferredIPEndpoint(endpoint.ipv6_endpoints);
        if (ip_endpoint.has_value()) {
            return ip_endpoint;
        }
    }
...
std::optional<IPEndPoint> HttpStreamPool::AttemptManager::FindPreferredIPEndpoint(const std::vector<IPEndPoint>& ip_endpoints)
{
...
HttpStreamPool::AttemptManager::FailureKind HttpStreamPool::AttemptManager::DetermineFailureKind()
{
...
void HttpStreamPool::AttemptManager::NotifyJobOfFailure()
{
    ...
    base::SequencedTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(&AttemptManager::NotifyJobOfFailure, weak_ptr_factory_.GetWeakPtr()));
    ...
    case FailureKind::kStreamFailed:
        job->OnStreamFailed(error_to_notify_, net_error_details_, resolve_error_info_);
        break;
...
void HttpStreamPool::AttemptManager::NotifyPreconnectsComplete(int rv)
{
    while (!preconnects_.empty()) {
        std::unique_ptr<PreconnectEntry> entry = std::move(preconnects_.extract(preconnects_.begin()).value());
        base::SequencedTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(std::move(entry->callback), rv));
    }
    base::SequencedTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(&AttemptManager::MaybeComplete, weak_ptr_factory_.GetWeakPtr()));
}

void HttpStreamPool::AttemptManager::ProcessPreconnectsAfterAttemptComplete(int rv)
{
    ...
    for (auto* entry_ptr : completed) {
        ...
        std::unique_ptr<PreconnectEntry> entry = std::move(preconnects_.extract(it).value());
        base::SequencedTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(std::move(entry->callback), entry->result));
    }
...
void HttpStreamPool::AttemptManager::CreateTextBasedStreamAndNotify(
    std::unique_ptr<StreamSocket> stream_socket, StreamSocketHandle::SocketReuseType reuse_type, LoadTimingInfo::ConnectTiming connect_timing)
{
    NextProto negotiated_protocol = stream_socket->GetNegotiatedProtocol();
    CHECK_NE(negotiated_protocol, NextProto::kProtoHTTP2);

    std::unique_ptr<HttpStream> http_stream = group_->CreateTextBasedStream(std::move(stream_socket), reuse_type, std::move(connect_timing));
    CHECK(respect_limits_ == RespectLimits::kIgnore || group_->ActiveStreamSocketCount() <= pool()->max_stream_sockets_per_group())
        << "active=" << group_->ActiveStreamSocketCount() << ", limit=" << pool()->max_stream_sockets_per_group();
...
void HttpStreamPool::AttemptManager::CreateSpdyStreamAndNotify()
{
    ...
    std::set<std::string> dns_aliases = http_network_session()->spdy_session_pool()->GetDnsAliasesForSessionKey(spdy_session_key());
    auto http_stream = std::make_unique<SpdyHttpStream>(spdy_session_, net_log().source(), std::move(dns_aliases));
...
void HttpStreamPool::AttemptManager::CreateQuicStreamAndNotify()
{
    QuicChromiumClientSession* quic_session
        = quic_session_pool()->FindExistingSession(quic_session_alias_key().session_key(), quic_session_alias_key().destination());
    CHECK(quic_session);
    ...
    std::set<std::string> dns_aliases = quic_session->GetDnsAliasesForSessionKey(quic_session_alias_key().session_key());
    auto http_stream = std::make_unique<QuicHttpStream>(quic_session->CreateHandle(stream_key().destination()), std::move(dns_aliases));
    NotifyStreamReady(std::move(http_stream), NextProto::kProtoQUIC);
    // `this` may be deleted.
}

void HttpStreamPool::AttemptManager::NotifyStreamReady(std::unique_ptr<HttpStream> stream, NextProto negotiated_protocol)
{
    Job* job = ExtractFirstJobToNotify();
    if (!job) {
...
void HttpStreamPool::AttemptManager::SetJobPriority(Job* job, RequestPriority priority)
{
    for (JobQueue::Pointer pointer = jobs_.FirstMax(); !pointer.is_null(); pointer = jobs_.GetNextTowardsLastMin(pointer)) {
        if (pointer.value() == job) {
...
void HttpStreamPool::AttemptManager::OnInFlightAttemptComplete(InFlightAttempt* raw_attempt, int rv)
{
    net_log().AddEvent(NetLogEventType::HTTP_STREAM_POOL_ATTEMPT_MANAGER_ATTEMPT_END, [&] {
        base::Value::Dict dict = GetStatesAsNetLogParams();
    ...
    std::unique_ptr<InFlightAttempt> in_flight_attempt = std::move(in_flight_attempts_.extract(it).value());
    ...
    LoadTimingInfo::ConnectTiming connect_timing = in_flight_attempt->attempt()->connect_timing();
    ...
    connect_timing.domain_lookup_end
        = dns_resolution_end_time_.is_null() ? connect_timing.connect_start : std::min(connect_timing.connect_start, dns_resolution_end_time_);

    std::unique_ptr<StreamSocket> stream_socket = in_flight_attempt->attempt()->ReleaseStreamSocket();
    CHECK(stream_socket);
    CHECK(service_endpoint_request_);
    ...
    if (stream_socket->GetNegotiatedProtocol() == NextProto::kProtoHTTP2) {
        CHECK(!spdy_session_pool()->FindAvailableSession(group_->spdy_session_key(), enable_ip_based_pooling_,
            /*is_websocket=*/false, net_log()));
        std::unique_ptr<HttpStreamPoolHandle> handle = group_->CreateHandle(std::move(stream_socket), reuse_type, std::move(connect_timing));
        int create_result = spdy_session_pool()->CreateAvailableSessionFromSocketHandle(
            spdy_session_key(), std::move(handle), net_log(), MultiplexedSessionCreationInitiator::kUnknown, &spdy_session_);
        if (create_result != OK) {
    ...
    CreateTextBasedStreamAndNotify(std::move(stream_socket), reuse_type, std::move(connect_timing));
}

void HttpStreamPool::AttemptManager::OnInFlightAttemptTcpHandshakeComplete(InFlightAttempt* raw_attempt, int rv)
{
    auto it = in_flight_attempts_.find(raw_attempt);
    CHECK(it != in_flight_attempts_.end());
...
void HttpStreamPool::AttemptManager::OnInFlightAttemptSlow(InFlightAttempt* raw_attempt)
{
    auto it = in_flight_attempts_.find(raw_attempt);
    CHECK(it != in_flight_attempts_.end());
...
void HttpStreamPool::AttemptManager::HandleAttemptFailure(std::unique_ptr<InFlightAttempt> in_flight_attempt, int rv)
{
    ...
    if (IsCertificateError(rv)) {
        ...
        SSLInfo ssl_info;
        bool has_ssl_info = in_flight_attempt->attempt()->stream_socket()->GetSSLInfo(&ssl_info);
        CHECK(has_ssl_info);
...
base::TimeDelta HttpStreamPool::AttemptManager::GetStreamAttemptDelay()
{
    ...
    return quic_session_pool()->GetTimeDelayForWaitingJob(quic_session_alias_key().session_key());
}
...
{
    net_log().AddEvent(NetLogEventType::HTTP_STREAM_POOL_ATTEMPT_MANAGER_STREAM_ATTEMPT_DELAY_PASSED, [&] {
        base::Value::Dict dict;
        dict.Set("stream_attempt_delay", static_cast<int>(stream_attempt_delay_.InMilliseconds()));
        return dict;
    });
...
void HttpStreamPool::AttemptManager::MaybeUpdateQuicVersionWhenForced(quic::ParsedQuicVersion& quic_version)
{
    if (!quic_version.IsKnown() && group_->force_quic()) {
        quic_version = http_network_session()->context().quic_context->params()->supported_versions[0];
    }
}
...
bool HttpStreamPool::AttemptManager::CanUseQuic()
{
    return allowed_alpns_.HasAny(kQuicBasedProtocols)
        && pool()->CanUseQuic(stream_key().destination(), stream_key().network_anonymization_key(), enable_ip_based_pooling_, enable_alternative_services_);
}

bool HttpStreamPool::AttemptManager::CanUseExistingQuicSession()
{
    return pool()->CanUseExistingQuicSession(quic_session_alias_key(), enable_ip_based_pooling_, enable_alternative_services_);
}

bool HttpStreamPool::AttemptManager::IsEchEnabled() const
{
    return pool()->stream_attempt_params()->ssl_client_context->config().ech_enabled;
}

bool HttpStreamPool::AttemptManager::IsEndpointUsableForTcpBasedAttempt(const ServiceEndpoint& endpoint, bool svcb_optional)
{
    ...
    for (const auto& alpn : endpoint.metadata.supported_protocol_alpns) {
        if (base::Contains(http_network_session()->GetAlpnProtos(), NextProtoFromString(alpn))) {
            return true;
        }
    }
    return false;
...
{
    if (!quic_task_result_.has_value() || tcp_based_attempt_state_ == TcpBasedAttemptState::kAttempting) {
        return;
    }

    if (*quic_task_result_ == OK || *quic_task_result_ == ERR_DNS_NO_MATCHING_SUPPORTED_ALPN || *quic_task_result_ == ERR_NETWORK_CHANGED
    ...
    if (tcp_based_attempt_state_ == TcpBasedAttemptState::kNotStarted || tcp_based_attempt_state_ == TcpBasedAttemptState::kAllAttemptsFailed) {
        return;
    }

    const url::SchemeHostPort& destination = stream_key().destination();
    http_network_session()->http_server_properties()->MarkAlternativeServiceBroken(
        AlternativeService(NextProto::kProtoQUIC, destination.host(), destination.port()), stream_key().network_anonymization_key());
}
...
base::Value::Dict HttpStreamPool::AttemptManager::GetStatesAsNetLogParams()
{
    ...
    dict.Set("num_inflight_attempts", static_cast<int>(in_flight_attempts_.size()));
...
{
    if (!jobs_.empty() || !notified_jobs_.empty() || !preconnects_.empty() || !in_flight_attempts_.empty()) {
        return;
    }
