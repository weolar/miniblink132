...
public:
    ...
    {
        return NetworkAnonymizationKey::CreateFromNetworkIsolationKey(network_isolation_key_);
    }

    void set_network_isolation_key(const net::NetworkIsolationKey& network_isolation_key)
    {
...
void PopulateCacheEntry(HttpCache* cache, const GURL& request_url)
{
    ...
    std::unique_ptr<MockTransaction> mock_trans = CreateMockTransaction(request_url);
...
TEST(HttpCacheLookupManagerTest, ServerPushMissCache)
{
    ...
    std::unique_ptr<MockServerPushHelper> push_helper = std::make_unique<MockServerPushHelper>(request_url);
...
TEST(HttpCacheLookupManagerTest, ServerPushDoNotCreateCacheEntry)
{
    ...
    std::unique_ptr<MockServerPushHelper> push_helper2 = std::make_unique<MockServerPushHelper>(request_url);
...
class HttpCacheLookupManagerTest_NetworkIsolationKey : public ::testing::Test,
                                                       public ::testing::WithParamInterface<bool /* use_same_network_isolation_key */> { };

TEST_P(HttpCacheLookupManagerTest_NetworkIsolationKey, ServerPushCacheStatus)
{
    bool use_same_network_isolation_key = GetParam();
    bool split_cache_enabled = base::FeatureList::IsEnabled(net::features::kSplitCacheByNetworkIsolationKey);
    ...
    if (!use_same_network_isolation_key) {
        ...
        push_helper->set_network_isolation_key(net::NetworkIsolationKey(site, site));
    }

    MockServerPushHelper* push_helper_ptr = push_helper.get();

    int expected_cancel_times = use_same_network_isolation_key || !split_cache_enabled ? 1 : 0;
    ...
    int expected_open_count = use_same_network_isolation_key || !split_cache_enabled ? 1 : 0;
...
INSTANTIATE_TEST_SUITE_P(All, HttpCacheLookupManagerTest_NetworkIsolationKey, ::testing::Bool());
...
TEST(HttpCacheLookupManagerTest, ServerPushLookupOnUrl)
{
    ...
    std::unique_ptr<MockTransaction> mock_trans3 = CreateMockTransaction(request_url2);
    ...
    std::unique_ptr<MockServerPushHelper> push_helper3 = std::make_unique<MockServerPushHelper>(request_url2);
