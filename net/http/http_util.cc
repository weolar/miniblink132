...
template <typename ConstIterator> void TrimLWSImplementation(ConstIterator* begin, ConstIterator* end)
{
    // leading whitespace
...
{
    std::vector<std::string> tokens = base::SplitString(language_code, "-", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL);
...
std::string HttpUtil::GenerateRequestLine(std::string_view method, const GURL& url, bool is_for_get_to_http_proxy)
{
    static constexpr char kSuffix[] = " HTTP/1.1\r\n";
    const std::string path = is_for_get_to_http_proxy ? HttpUtil::SpecForRequest(url) : url.PathForRequest();
...
{
    DCHECK(url.is_valid() && (url.SchemeIsHTTPOrHTTPS() || url.SchemeIsWSOrWSS()));
    return SimplifyUrlForRequest(url).spec();
}

// static
void HttpUtil::ParseContentType(std::string_view content_type_str, std::string* mime_type, std::string* charset, bool* had_charset, std::string* boundary)
{
    ...
    for (const auto& param : params) {
        ...
        if (!type_has_charset && base::EqualsCaseInsensitiveASCII(param.first, "charset")) {
            type_has_charset = true;
            charset_value = std::string(HttpUtil::TrimLWS(param.second));
            continue;
        }

        if (boundary && !type_has_boundary && base::EqualsCaseInsensitiveASCII(param.first, "boundary")) {
...
// static
bool HttpUtil::ParseRangeHeader(const std::string& ranges_specifier, std::vector<HttpByteRange>* ranges)
{
    ...
    std::string_view bytes_unit = std::string_view(ranges_specifier).substr(0, equal_char_offset);
    ...
    std::string::const_iterator byte_range_set_begin = ranges_specifier.begin() + equal_char_offset + 1;
    std::string::const_iterator byte_range_set_end = ranges_specifier.end();

    ValuesIterator byte_range_set_iterator(std::string_view(byte_range_set_begin, byte_range_set_end),
        /*delimiter=*/',');
...
bool HttpUtil::ParseContentRangeHeaderFor206(
    std::string_view content_range_spec, int64_t* first_byte_position, int64_t* last_byte_position, int64_t* instance_length)
{
    *first_byte_position = *last_byte_position = *instance_length = -1;
    ...
    if (!base::EqualsCaseInsensitiveASCII(TrimLWS(content_range_spec.substr(0, space_position)), "bytes")) {
        return false;
    }
    ...
    if (base::StringToInt64(TrimLWS(content_range_spec.substr(space_position + 1, minus_position - (space_position + 1))), first_byte_position)
        && *first_byte_position >= 0
        && base::StringToInt64(TrimLWS(content_range_spec.substr(minus_position + 1, slash_position - (minus_position + 1))), last_byte_position)
        && *last_byte_position >= *first_byte_position && base::StringToInt64(TrimLWS(content_range_spec.substr(slash_position + 1)), instance_length)
...
// static
bool HttpUtil::ParseRetryAfterHeader(const std::string& retry_after_string, base::Time now, base::TimeDelta* retry_after)
{
    uint32_t seconds;
    base::Time time;
...
{
    static constexpr char kWeekdayName[7][4] = { "Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat" };
    static constexpr char kMonthName[12][4] = { "Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec" };
    base::Time::Exploded exploded;
    time.UTCExplode(&exploded);
    return base::StringPrintf("%s, %02d %s %04d %02d:%02d:%02d GMT", kWeekdayName[exploded.day_of_week], exploded.day_of_month, kMonthName[exploded.month - 1],
...
{
    if (base::StartsWith(name, "proxy-", base::CompareCase::INSENSITIVE_ASCII) || base::StartsWith(name, "sec-", base::CompareCase::INSENSITIVE_ASCII))
        return false;
...
bool HttpUtil::IsNonCoalescingHeader(std::string_view name)
{
    ...
    static constexpr std::string_view kNonCoalescingHeaders[] = { "date", "expires", "last-modified",
...
// static
void HttpUtil::TrimLWS(std::string::const_iterator* begin, std::string::const_iterator* end)
{
...
{
    return !(c >= 0x7F || c <= 0x20 || c == '(' || c == ')' || c == '<' || c == '>' || c == '@' || c == ',' || c == ';' || c == ':' || c == '\\' || c == '"'
        || c == '/' || c == '[' || c == ']' || c == '?' || c == '=' || c == '{' || c == '}');
}
...
size_t HttpUtil::LocateStartOfStatusLine(base::span<const uint8_t> buf)
{
    ...
    if (buf.size() >= http_len) {
        ...
        for (size_t i = 0; i <= i_max; ++i) {
            if (base::EqualsCaseInsensitiveASCII(base::as_string_view(buf.subspan(i, http_len)), "http")) {
                return i;
            }
        }
    }
...
static size_t LocateEndOfHeadersHelper(base::span<const uint8_t> buf, size_t i, bool accept_empty_header_list)
{
    char last_c = '\0';
    bool was_lf = false;
...
size_t HttpUtil::LocateEndOfAdditionalHeaders(base::span<const uint8_t> buf, size_t i)
{
...
std::string HttpUtil::AssembleRawHeaders(std::string_view input)
{
    ...
    size_t status_begin_offset = LocateStartOfStatusLine(base::as_byte_span(input));
...
{
    const std::vector<std::string> languages = base::SplitString(language_prefs, ",", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL);
...
std::string HttpUtil::GenerateAcceptLanguageHeader(const std::string& raw_language_list)
{
    ...
    while (t.GetNext()) {
        ...
        if (qvalue10 == 10) {
            ...
            base::StringAppendF(&lang_list_with_q, ",%s;q=0.%d", language.c_str(), qvalue10);
        }
...
bool HttpUtil::HasStrongValidators(HttpVersion version, std::optional<std::string_view> etag_header, std::optional<std::string_view> last_modified_header,
    std::optional<std::string_view> date_header)
{
    ...
    if (!last_modified_header || !base::Time::FromString(std::string(*last_modified_header).c_str(), &last_modified)) {
        return false;
    }

    base::Time date;
    if (!date_header || !base::Time::FromString(std::string(*date_header).c_str(), &date)) {
        return false;
    }
...
bool HttpUtil::HasValidators(HttpVersion version, std::optional<std::string_view> etag_header, std::optional<std::string_view> last_modified_header)
{
...
std::vector<int> HttpUtil::GetStatusCodesForHistogram()
{
    ...
    codes.reserve(HISTOGRAM_MAX_HTTP_STATUS_CODE - HISTOGRAM_MIN_HTTP_STATUS_CODE + 2);
    codes.push_back(0);
    for (int i = HISTOGRAM_MIN_HTTP_STATUS_CODE; i <= HISTOGRAM_MAX_HTTP_STATUS_CODE; ++i)
        codes.push_back(i);
    return codes;
}
...
HttpUtil::HeadersIterator::HeadersIterator(
    std::string::const_iterator headers_begin, std::string::const_iterator headers_end, const std::string& line_delimiter)
...
{
    DCHECK(name != nullptr);
    DCHECK_EQ(0, base::ToLowerASCII(name).compare(name)) << "the header name must be in all lower case";

    while (GetNext()) {
        if (base::EqualsCaseInsensitiveASCII(base::MakeStringPiece(name_begin_, name_end_), name)) {
            return true;
        }
    }
...
HttpUtil::ValuesIterator::ValuesIterator(std::string_view values, char delimiter, bool ignore_empty_values)
...
HttpUtil::NameValuePairsIterator::NameValuePairsIterator(std::string_view value, char delimiter, Values optional_values, Quotes strict_quotes)
...
HttpUtil::NameValuePairsIterator::NameValuePairsIterator(const NameValuePairsIterator& other) = default;
...
bool HttpUtil::NameValuePairsIterator::ParseNameValuePair(std::string_view name_value_pair)
{
...
bool HttpUtil::ParseAcceptEncoding(const std::string& accept_encoding, std::set<std::string>* allowed_encodings)
{
...
bool HttpUtil::ParseContentEncoding(const std::string& content_encoding, std::set<std::string>* used_encodings)
{
    ...
    base::StringTokenizer encoding_tokenizer(content_encoding.begin(), content_encoding.end(), ",");
...
bool HttpUtil::HeadersContainMultipleCopiesOfField(const HttpResponseHeaders& headers, const std::string& field_name)
{
    size_t it = 0;
    std::optional<std::string_view> field_value = headers.EnumerateHeader(&it, field_name);
    if (!field_value) {
        return false;
    }
