...
bool ShouldByPassCacheForFirstPartySets(const std::optional<int64_t>& clear_at_run_id, const std::optional<int64_t>& written_at_run_id)
{
    return clear_at_run_id.has_value() && (!written_at_run_id.has_value() || written_at_run_id.value() < clear_at_run_id.value());
}
...
constexpr HeaderNameAndValue kPassThroughHeaders[] = { { "if-unmodified-since", nullptr }, // causes unexpected 412s
    { "if-match", nullptr }, // causes unexpected 412s
    { "if-range", nullptr }, { nullptr, nullptr } };
...
constexpr HeaderNameAndValue kForceFetchHeaders[] = { { "cache-control", "no-cache" }, { "pragma", "no-cache" }, { nullptr, nullptr } };
...
constexpr HeaderNameAndValue kForceValidateHeaders[] = { { "cache-control", "max-age=0" }, { nullptr, nullptr } };

bool HeaderMatches(const HttpRequestHeaders& headers, const HeaderNameAndValue* search)
{
...
#define CACHE_STATUS_HISTOGRAMS(type) UMA_HISTOGRAM_ENUMERATION("HttpCache.Pattern" type, cache_entry_status_, CacheEntryStatus::ENTRY_MAX)

#define IS_NO_STORE_HISTOGRAMS(type, is_no_store) base::UmaHistogramBoolean("HttpCache.IsNoStore" type, is_no_store)
...
{
    static_assert(HttpCache::Transaction::kNumValidationHeaders == std::size(kValidationHeaders), "invalid number of validation headers");

    io_callback_ = base::BindRepeating(&Transaction::OnIOComplete, weak_factory_.GetWeakPtr());
    cache_io_callback_ = base::BindRepeating(&Transaction::OnCacheIOComplete, weak_factory_.GetWeakPtr());
}
...
int HttpCache::Transaction::Start(const HttpRequestInfo* request, CompletionOnceCallback callback, const NetLogWithSource& net_log)
{
    DCHECK(request);
    DCHECK(request->IsConsistent());
    DCHECK(!callback.is_null());
    TRACE_EVENT_BEGIN("net", "HttpCacheTransaction", perfetto::Track(trace_id_), "url", request->url.spec());
...
int HttpCache::Transaction::RestartIgnoringLastError(CompletionOnceCallback callback)
{
    DCHECK(!callback.is_null());
...
int HttpCache::Transaction::RestartWithCertificate(
    scoped_refptr<X509Certificate> client_cert, scoped_refptr<SSLPrivateKey> client_private_key, CompletionOnceCallback callback)
{
    DCHECK(!callback.is_null());
    ...
    int rv = RestartNetworkRequestWithCertificate(std::move(client_cert), std::move(client_private_key));
...
int HttpCache::Transaction::RestartWithAuth(const AuthCredentials& credentials, CompletionOnceCallback callback)
{
...
int HttpCache::Transaction::Read(IOBuffer* buf, int buf_len, CompletionOnceCallback callback)
{
    TRACE_EVENT_INSTANT("net", "HttpCacheTransaction::Read", perfetto::Track(trace_id_), "buf_len", buf_len);
...
void HttpCache::Transaction::StopCaching()
{
    ...
    // from the net.
    if (cache_.get() && (mode_ & WRITE) && !is_sparse_ && !range_requested_ && network_transaction()) {
...
const HttpResponseInfo* HttpCache::Transaction::GetResponseInfo() const
{
    ...
    if (auth_response_.headers.get()) {
        DCHECK_EQ(cache_entry_status_, auth_response_.cache_entry_status) << "These must be in sync via SetResponse and SetAuthResponse.";
...
void HttpCache::Transaction::SetQuicServerInfo(QuicServerInfo* quic_server_info)
{
}

bool HttpCache::Transaction::GetLoadTimingInfo(LoadTimingInfo* load_timing_info) const
{
    const HttpTransaction* transaction = GetOwnedOrMovedNetworkTransaction();
    if (transaction) {
    ...
    if (network_transaction_info_.old_network_trans_load_timing) {
        *load_timing_info = *network_transaction_info_.old_network_trans_load_timing;
        return true;
    }
...
void HttpCache::Transaction::PopulateNetErrorDetails(NetErrorDetails* details) const
{
    const HttpTransaction* transaction = GetOwnedOrMovedNetworkTransaction();
    if (transaction) {
...
void HttpCache::Transaction::SetWebSocketHandshakeStreamCreateHelper(WebSocketHandshakeStreamBase::CreateHelper* create_helper)
{
...
void HttpCache::Transaction::SetBeforeNetworkStartCallback(BeforeNetworkStartCallback callback)
{
    DCHECK(!network_trans_);
    before_network_start_callback_ = std::move(callback);
}

void HttpCache::Transaction::SetConnectedCallback(const ConnectedCallback& callback)
{
    DCHECK(!network_trans_);
    connected_callback_ = callback;
}

void HttpCache::Transaction::SetRequestHeadersCallback(RequestHeadersCallback callback)
{
    DCHECK(!network_trans_);
    request_headers_callback_ = std::move(callback);
}

void HttpCache::Transaction::SetResponseHeadersCallback(ResponseHeadersCallback callback)
{
    DCHECK(!network_trans_);
    response_headers_callback_ = std::move(callback);
}

void HttpCache::Transaction::SetEarlyResponseHeadersCallback(ResponseHeadersCallback callback)
{
    DCHECK(!network_trans_);
    early_response_headers_callback_ = std::move(callback);
}

void HttpCache::Transaction::SetModifyRequestHeadersCallback(base::RepeatingCallback<void(HttpRequestHeaders*)> callback)
{
    // This method should not be called for this class.
    NOTREACHED();
}

void HttpCache::Transaction::SetIsSharedDictionaryReadAllowedCallback(base::RepeatingCallback<bool()> callback)
{
    DCHECK(!network_trans_);
...
ConnectionAttempts HttpCache::Transaction::GetConnectionAttempts() const
{
    ...
    attempts.insert(attempts.begin(), network_transaction_info_.old_connection_attempts.begin(), network_transaction_info_.old_connection_attempts.end());
    return attempts;
}
...
{
    TRACE_EVENT_INSTANT("net", "HttpCacheTransaction::WriterAboutToBeRemovedFromEntry", perfetto::Track(trace_id_));
    // Since the transaction can no longer access the network transaction, save
    // all network related info now.
    if (moved_network_transaction_to_writers_ && entry_->writers()->network_transaction()) {
...
{
    TRACE_EVENT_INSTANT("net", "HttpCacheTransaction::WriteModeTransactionAboutToBecomeReader", perfetto::Track(trace_id_));
    mode_ = READ;
    if (moved_network_transaction_to_writers_ && entry_->writers()->network_transaction()) {
        SaveNetworkTransactionInfo(*(entry_->writers()->network_transaction()));
    }
...
int HttpCache::Transaction::DoLoop(int result)
{
    ...
    DCHECK(reading_ || rv != OK || !entry_ || state == STATE_FINISH_HEADERS_COMPLETE);
...
int HttpCache::Transaction::DoGetBackendComplete(int result)
{
    ...
    net_log_.EndEventWithNetErrorCode(NetLogEventType::HTTP_CACHE_GET_BACKEND, result);
    ...
    std::optional<std::string> cache_key = HttpCache::GenerateCacheKeyForRequest(request_);
    ...
    if ((method_ == "PUT" || method_ == "DELETE" || method_ == "PATCH") && mode_ != READ_WRITE && mode_ != WRITE) {
        mode_ = NONE;
    }
    ...
    TRACE_EVENT_INSTANT(
        "net", "HttpCacheTransaction::DoGetBackendComplete", perfetto::Track(trace_id_), "mode", mode_, "should_pass_through", should_pass_through);
    return OK;
}

int HttpCache::Transaction::DoInitEntry()
{
    TRACE_EVENT_INSTANT("net", "HttpCacheTransaction::DoInitEntry", perfetto::Track(trace_id_));
    DCHECK(!new_entry_);
...
{
    TRACE_EVENT_INSTANT("net", "HttpCacheTransaction::DoOpenOrCreateEntry", perfetto::Track(trace_id_));
    DCHECK(!new_entry_);
    ...
    // we know is doomed.
    uint8_t in_memory_info = cache_->GetCurrentBackend()->GetEntryInMemoryData(cache_key_);
    bool entry_not_suitable = false;
    if (MaybeRejectBasedOnEntryInMemoryData(in_memory_info)) {
        cache_->GetCurrentBackend()->DoomEntry(cache_key_, priority_, base::DoNothing());
...
{
    TRACE_EVENT_INSTANT("net", "HttpCacheTransaction::DoOpenOrCreateEntryComplete", perfetto::Track(trace_id_), "result",
        (result == OK ? (new_entry_->opened() ? "opened" : "created") : "failed"));

    const bool record_uma
        = record_entry_open_or_creation_time_ && cache_ && cache_->GetCurrentBackend() && cache_->GetCurrentBackend()->GetCacheType() != MEMORY_CACHE;
    record_entry_open_or_creation_time_ = false;
    ...
    if (result == OK) {
        if (new_entry_->opened()) {
            if (record_uma) {
                base::UmaHistogramTimes("HttpCache.OpenDiskEntry", base::TimeTicks::Now() - first_cache_access_since_);
            }
        } else {
            if (record_uma) {
                base::UmaHistogramTimes("HttpCache.CreateDiskEntry", base::TimeTicks::Now() - first_cache_access_since_);
            }
...
{
    TRACE_EVENT_INSTANT("net", "HttpCacheTransaction::DoDoomEntry", perfetto::Track(trace_id_));
...
{
    TRACE_EVENT_INSTANT("net", "HttpCacheTransaction::DoDoomEntryComplete", perfetto::Track(trace_id_), "result", result);
    net_log_.EndEventWithNetErrorCode(NetLogEventType::HTTP_CACHE_DOOM_ENTRY, result);
    cache_pending_ = false;
    TransitionToState(result == ERR_CACHE_RACE ? STATE_HEADERS_PHASE_CANNOT_PROCEED : STATE_CREATE_ENTRY);
    return OK;
}

int HttpCache::Transaction::DoCreateEntry()
{
    TRACE_EVENT_INSTANT("net", "HttpCacheTransaction::DoCreateEntry", perfetto::Track(trace_id_));
    DCHECK(!new_entry_);
...
{
    TRACE_EVENT_INSTANT("net", "HttpCacheTransaction::DoCreateEntryComplete", perfetto::Track(trace_id_), "result", result);
    // It is important that we go to STATE_ADD_TO_ENTRY whenever the result is
    // OK, otherwise the cache will end up with an active entry without any
    // transaction attached.
    net_log_.EndEventWithNetErrorCode(NetLogEventType::HTTP_CACHE_CREATE_ENTRY, result);
    cache_pending_ = false;
    switch (result) {
    case OK:
        TransitionToState(STATE_ADD_TO_ENTRY);
...
{
    TRACE_EVENT_INSTANT("net", "HttpCacheTransaction::DoAddToEntry", perfetto::Track(trace_id_));
    DCHECK(new_entry_);
    cache_pending_ = true;
...
{
    CHECK(next_state_ == STATE_ADD_TO_ENTRY_COMPLETE || next_state_ == STATE_FINISH_HEADERS_COMPLETE);
    if ((bypass_lock_for_test_ && next_state_ == STATE_ADD_TO_ENTRY_COMPLETE)
        || (bypass_lock_after_headers_for_test_ && next_state_ == STATE_FINISH_HEADERS_COMPLETE)) {
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
            FROM_HERE, base::BindOnce(&HttpCache::Transaction::OnCacheLockTimeout, weak_factory_.GetWeakPtr(), entry_lock_waiting_since_));
    } else {
        int timeout_milliseconds = 20 * 1000;
        if (partial_ && entry->HasWriters() && !entry->writers()->IsEmpty() && entry->writers()->IsExclusive()) {
        ...
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE,
            base::BindOnce(&HttpCache::Transaction::OnCacheLockTimeout, weak_factory_.GetWeakPtr(), entry_lock_waiting_since_),
...
{
    TRACE_EVENT_INSTANT("net", "HttpCacheTransaction::DoAddToEntryComplete", perfetto::Track(trace_id_), "result", result);
    net_log_.EndEventWithNetErrorCode(NetLogEventType::HTTP_CACHE_ADD_TO_ENTRY, result);
    if (cache_ && cache_->GetCurrentBackend() && cache_->GetCurrentBackend()->GetCacheType() != MEMORY_CACHE) {
        const base::TimeDelta entry_lock_wait = TimeTicks::Now() - entry_lock_waiting_since_;
...
{
    TRACE_EVENT_INSTANT("net", "HttpCacheTransaction::DoDoneHeadersAddToEntryComplete", perfetto::Track(trace_id_), "result", result);
    ...
    DCHECK(entry_->CanTransactionWriteResponseHeaders(this, partial_ != nullptr, false));
    TransitionToState(STATE_CACHE_WRITE_RESPONSE);
    return OK;
}
...
{
    TRACE_EVENT_INSTANT("net", "HttpCacheTransaction::DoCacheReadResponse", perfetto::Track(trace_id_));
    DCHECK(entry_);
    ...
    return entry_->GetEntry()->ReadData(kResponseInfoIndex, 0, read_buf_.get(), io_buf_len_, io_callback_);
}

int HttpCache::Transaction::DoCacheReadResponseComplete(int result)
{
    TRACE_EVENT_INSTANT(
        "net", "HttpCacheTransaction::DoCacheReadResponseComplete", perfetto::Track(trace_id_), "result", result, "io_buf_len", read_buf_->size());
    net_log_.EndEventWithNetErrorCode(NetLogEventType::HTTP_CACHE_READ_INFO, result);
    ...
    if (result != read_buf_->size() || !HttpCache::ParseResponseInfo(read_buf_->span(), &response_, &truncated_)) {
        return OnCacheReadError(result, true);
    }

    // If the read response matches the clearing filter of FPS, doom the entry
    // and restart transaction.
    if (ShouldByPassCacheForFirstPartySets(initial_request_->fps_cache_filter, response_.browser_run_id)) {
    ...
    if (!entry_->IsWritingInProgress()) {
        ...
        if ((truncated_ || response_.headers->response_code() == HTTP_PARTIAL_CONTENT) && !range_requested_
    ...
    if (response_.restricted_prefetch && !(request_->load_flags & LOAD_CAN_USE_RESTRICTED_PREFETCH_FOR_MAIN_FRAME)) {
        TransitionToState(STATE_SEND_REQUEST);
        return OK;
    }

    // When a restricted prefetch is reused, we lift its reuse restriction.
    bool restricted_prefetch_reuse = response_.restricted_prefetch && request_->load_flags & LOAD_CAN_USE_RESTRICTED_PREFETCH_FOR_MAIN_FRAME;
    DCHECK(!restricted_prefetch_reuse || response_.unused_since_prefetch);

    if (response_.unused_since_prefetch != !!(request_->load_flags & LOAD_PREFETCH)) {
        ...
        updated_prefetch_response_->unused_since_prefetch = !response_.unused_since_prefetch;
        if (response_.restricted_prefetch && request_->load_flags & LOAD_CAN_USE_RESTRICTED_PREFETCH_FOR_MAIN_FRAME) {
...
{
    TRACE_EVENT_INSTANT("net", "HttpCacheTransaction::DoCacheWriteUpdatedPrefetchResponse", perfetto::Track(trace_id_), "result", result);
    DCHECK(updated_prefetch_response_);
    ...
    return WriteResponseInfoToEntry(*updated_prefetch_response_.get(), truncated_);
}

int HttpCache::Transaction::DoCacheWriteUpdatedPrefetchResponseComplete(int result)
{
    TRACE_EVENT_INSTANT("net", "HttpCacheTransaction::DoCacheWriteUpdatedPrefetchResponseComplete", perfetto::Track(trace_id_), "result", result);
...
{
    TRACE_EVENT_INSTANT("net", "HttpCacheTransaction::DoCacheDispatchValidation", perfetto::Track(trace_id_));
    if (!entry_) {
...
int HttpCache::Transaction::DoCompletePartialCacheValidation(int result)
{
    ...
    partial_->PrepareCacheValidation(entry_->GetEntry(), &custom_request_->extra_headers);
...
{
    TRACE_EVENT_INSTANT("net", "HttpCacheTransaction::DoCacheUpdateStaleWhileRevalidateTimeout", perfetto::Track(trace_id_));
    response_.stale_revalidate_timeout = cache_->clock_->Now() + kStaleRevalidateTimeout;
...
int HttpCache::Transaction::DoCacheUpdateStaleWhileRevalidateTimeoutComplete(int result)
{
    TRACE_EVENT_INSTANT("net", "HttpCacheTransaction::DoCacheUpdateStaleWhileRevalidateTimeoutComplete", perfetto::Track(trace_id_), "result", result);
    DCHECK(!reading_);
    TransitionToState(STATE_CONNECTED_CALLBACK);
    return OnWriteResponseInfoToEntryComplete(result);
}
...
{
    TRACE_EVENT_INSTANT("net", "HttpCacheTransaction::DoSendRequest", perfetto::Track(trace_id_));
    DCHECK(mode_ & WRITE || mode_ == NONE);
    DCHECK(!network_trans_.get());
    ...
    network_trans_->SetBeforeNetworkStartCallback(std::move(before_network_start_callback_));
    ...
    network_trans_->SetEarlyResponseHeadersCallback(early_response_headers_callback_);
    network_trans_->SetResponseHeadersCallback(response_headers_callback_);
    if (is_shared_dictionary_read_allowed_callback_) {
        network_trans_->SetIsSharedDictionaryReadAllowedCallback(is_shared_dictionary_read_allowed_callback_);
    }
    ...
    if (websocket_handshake_stream_base_create_helper_) {
        network_trans_->SetWebSocketHandshakeStreamCreateHelper(websocket_handshake_stream_base_create_helper_);
    }
...
{
    TRACE_EVENT_INSTANT("net", "HttpCacheTransaction::DoSendRequestComplete", perfetto::Track(trace_id_), "result", result, "elapsed",
...
int HttpCache::Transaction::DoSuccessfulSendRequest()
{
    ...
    TRACE_EVENT_INSTANT(
        "net", "HttpCacheTransaction::DoSuccessfulSendRequest", perfetto::Track(trace_id_), "response_code", new_response->headers->response_code());

    if (new_response->headers->response_code() == HTTP_UNAUTHORIZED || new_response->headers->response_code() == HTTP_PROXY_AUTHENTICATION_REQUIRED) {
    ...
    if (mode_ == WRITE && (method_ == "PUT" || method_ == "DELETE" || method_ == "PATCH")) {
        if (NonErrorResponse(new_response_->headers->response_code()) && (entry_ && !entry_->IsDoomed())) {
    ...
    if (!(effective_load_flags_ & LOAD_DISABLE_CACHE) && method_ == "POST" && NonErrorResponse(new_response_->headers->response_code())
        && (!HttpCache::IsSplitCacheEnabled() || request_->network_isolation_key.IsFullyPopulated())) {
        cache_->DoomMainEntryForUrl(
            request_->url, request_->network_isolation_key, request_->is_subframe_document_resource, request_->is_main_frame_navigation, request_->initiator);
    }

    if (new_response_->headers->response_code() == HTTP_REQUESTED_RANGE_NOT_SATISFIABLE && (method_ == "GET" || method_ == "POST")) {
    ...
    if (mode_ == READ_WRITE || mode_ == UPDATE) {
        if (new_response->headers->response_code() == HTTP_NOT_MODIFIED || handling_206_) {
...
{
    TRACE_EVENT_INSTANT("net", "HttpCacheTransaction::DoUpdateCachedResponse", perfetto::Track(trace_id_));
    int rv = OK;
...
{
    TRACE_EVENT_INSTANT("net", "HttpCacheTransaction::DoCacheWriteUpdatedResponse", perfetto::Track(trace_id_));
...
{
    TRACE_EVENT_INSTANT("net", "HttpCacheTransaction::DoCacheWriteUpdatedResponseComplete", perfetto::Track(trace_id_), "result", result);
    TransitionToState(STATE_UPDATE_CACHED_RESPONSE_COMPLETE);
    return OnWriteResponseInfoToEntryComplete(result);
}
...
{
    TRACE_EVENT_INSTANT("net", "HttpCacheTransaction::DoUpdateCachedResponseComplete", perfetto::Track(trace_id_), "result", result);
    if (mode_ == UPDATE) {
        ...
        if ((!partial_ && !entry_->IsWritingInProgress()) || (partial_ && partial_->IsLastRange())) {
            mode_ = READ;
        }
...
{
    TRACE_EVENT_INSTANT("net", "HttpCacheTransaction::DoOverwriteCachedResponse", perfetto::Track(trace_id_));
    if (mode_ & READ) {
...
{
    TRACE_EVENT_INSTANT("net", "HttpCacheTransaction::DoCacheWriteResponse", perfetto::Track(trace_id_));
    DCHECK(response_.headers);
    ...
    if (entry_ && !entry_->CanTransactionWriteResponseHeaders(this, partial_ != nullptr, is_match)) {
...
{
    TRACE_EVENT_INSTANT("net", "HttpCacheTransaction::DoCacheWriteResponseComplete", perfetto::Track(trace_id_), "result", result);
...
{
    TRACE_EVENT_INSTANT("net", "HttpCacheTransaction::DoTruncateCachedData", perfetto::Track(trace_id_));
...
{
    TRACE_EVENT_INSTANT("net", "HttpCacheTransaction::DoTruncateCachedDataComplete", perfetto::Track(trace_id_), "result", result);
    EndDiskCacheAccessTimeCount(DiskCacheAccessType::kWrite);
    if (entry_) {
        net_log_.EndEventWithNetErrorCode(NetLogEventType::HTTP_CACHE_WRITE_DATA, result);
    }
...
{
    TRACE_EVENT_INSTANT("net", "HttpCacheTransaction::DoPartialHeadersReceived", perfetto::Track(trace_id_));
    new_response_ = nullptr;
...
{
    TRACE_EVENT_INSTANT("net", "HttpCacheTransaction::DoFinishHeaders", perfetto::Track(trace_id_), "result", result);
...
{
    TRACE_EVENT_INSTANT("net", "HttpCacheTransaction::DoFinishHeadersComplete", perfetto::Track(trace_id_), "result", rv);
    entry_lock_waiting_since_ = TimeTicks();
...
{
    TRACE_EVENT_INSTANT(
        "net", "HttpCacheTransaction::DoNetworkReadCacheWrite", perfetto::Track(trace_id_), "read_offset", read_offset_, "read_buf_len", read_buf_len_);
    DCHECK(InWriters());
...
{
    TRACE_EVENT_INSTANT("net", "HttpCacheTransaction::DoNetworkReadCacheWriteComplete", perfetto::Track(trace_id_), "result", result);
    if (!cache_.get()) {
        TransitionToState(STATE_NONE);
        return ERR_UNEXPECTED;
    }
...
{
    TRACE_EVENT_INSTANT("net", "HttpCacheTransaction::DoNetworkRead", perfetto::Track(trace_id_), "read_offset", read_offset_, "read_buf_len", read_buf_len_);
...
{
    TRACE_EVENT_INSTANT("net", "HttpCacheTransaction::DoNetworkReadComplete", perfetto::Track(trace_id_), "result", result);
...
int HttpCache::Transaction::DoCacheReadData()
{
    ...
    TRACE_EVENT_INSTANT("net", "HttpCacheTransaction::DoCacheReadData", perfetto::Track(trace_id_), "read_offset", read_offset_, "read_buf_len", read_buf_len_);
    ...
    if (partial_) {
        return partial_->CacheRead(entry_->GetEntry(), read_buf_.get(), read_buf_len_, io_callback_);
    }

    BeginDiskCacheAccessTimeCount();
    return entry_->GetEntry()->ReadData(kResponseContentIndex, read_offset_, read_buf_.get(), read_buf_len_, io_callback_);
}
...
int HttpCache::Transaction::DoCacheReadDataComplete(int result)
{
    ...
    TRACE_EVENT_INSTANT("net", "HttpCacheTransaction::DoCacheReadDataComplete", perfetto::Track(trace_id_), "result", result);
    net_log_.EndEventWithNetErrorCode(NetLogEventType::HTTP_CACHE_READ_DATA, result);
...
void HttpCache::Transaction::SetRequest(const NetLogWithSource& net_log)
{
    ...
    for (size_t i = 0; i < std::size(kValidationHeaders); ++i) {
        ...
        if (std::optional<std::string> validation_value = request_->extra_headers.GetHeader(info.request_header_name); validation_value) {
    ...
    if (range_found || special_headers || external_validation_.initialized) {
        ...
        std::string empty;
        NetLogRequestHeaders(net_log_, NetLogEventType::HTTP_CACHE_CALLER_REQUEST_HEADERS, empty, &request_->extra_headers);
    }
...
bool HttpCache::Transaction::ShouldPassThrough()
{
    ...
    } else if (method_ == "POST" && request_->upload_data_stream && request_->upload_data_stream->identifier()) {
...
int HttpCache::Transaction::BeginCacheValidation()
{
    ...
    if ((effective_load_flags_ & LOAD_SUPPORT_ASYNC_REVALIDATION) && required_validation == VALIDATION_ASYNCHRONOUS) {
        ...
        needs_stale_while_revalidate_cache_update = response_.stale_revalidate_timeout.is_null();
    }

    if (method_ == "HEAD" && (truncated_ || response_.headers->response_code() == HTTP_PARTIAL_CONTENT)) {
        DCHECK(!partial_);
        if (skip_validation) {
            DCHECK(!reading_);
            TransitionToState(STATE_CONNECTED_CALLBACK);
    ...
    bool first_read_of_full_from_partial = is_sparse_ && !reading_ && (partial_ && !partial_->range_requested() && !partial_->IsLastRange());

    if (partial_ && (is_sparse_ || truncated_) && (!partial_->IsCurrentRangeCached() || invalid_range_ || first_read_of_full_from_partial)) {
    ...
    if (skip_validation) {
        UpdateCacheEntryStatus(CacheEntryStatus::ENTRY_USED);
        DCHECK(!reading_);
        TransitionToState(needs_stale_while_revalidate_cache_update ? STATE_CACHE_UPDATE_STALE_WHILE_REVALIDATE_TIMEOUT : STATE_CONNECTED_CALLBACK);
        return OK;
    } else {
...
int HttpCache::Transaction::BeginPartialCacheValidation()
{
    ...
    if (response_.headers->response_code() != HTTP_PARTIAL_CONTENT && !partial_ && !truncated_) {
        return BeginCacheValidation();
    }
...
int HttpCache::Transaction::ValidateEntryHeadersAndContinue()
{
    ...
    if (!partial_->UpdateFromStoredHeaders(response_.headers.get(), entry_->GetEntry(), truncated_, entry_->IsWritingInProgress())) {
        return DoRestartPartialRequest();
    }
...
bool HttpCache::Transaction::ExternallyConditionalizedValidationHeadersMatchEntry() const
{
    ...
    for (size_t i = 0; i < std::size(kValidationHeaders); i++) {
        ...
        std::optional<std::string_view> validator = response_.headers->EnumerateHeader(nullptr, kValidationHeaders[i].related_response_header_name);
...
int HttpCache::Transaction::BeginExternallyConditionalizedRequest()
{
    ...
    if (response_.headers->response_code() != HTTP_OK || truncated_ || !ExternallyConditionalizedValidationHeadersMatchEntry()) {
...
int HttpCache::Transaction::RestartNetworkRequestWithCertificate(scoped_refptr<X509Certificate> client_cert, scoped_refptr<SSLPrivateKey> client_private_key)
{
    DCHECK(mode_ & WRITE || mode_ == NONE);
    DCHECK(network_trans_.get());
    DCHECK_EQ(STATE_NONE, next_state_);
    ...
    next_state_ = STATE_SEND_REQUEST_COMPLETE;
    int rv = network_trans_->RestartWithCertificate(std::move(client_cert), std::move(client_private_key), io_callback_);
    if (rv != ERR_IO_PENDING) {
        return DoLoop(rv);
    }
    return rv;
...
int HttpCache::Transaction::RestartNetworkRequestWithAuth(const AuthCredentials& credentials)
{
    DCHECK(mode_ & WRITE || mode_ == NONE);
    DCHECK(network_trans_.get());
    DCHECK_EQ(STATE_NONE, next_state_);
...
ValidationType HttpCache::Transaction::RequiresValidation()
{
    ...
    if (!(effective_load_flags_ & LOAD_SKIP_VARY_CHECK) && response_.vary_data.is_valid()
        && !response_.vary_data.MatchesRequest(*request_, *response_.headers.get())) {
        vary_mismatch_ = true;
    ...
    ValidationType validation_required_by_headers = validate_flag
        ? VALIDATION_SYNCHRONOUS
        : response_.headers->RequiresValidation(response_.request_time, response_.response_time, cache_->clock_->Now());

    base::TimeDelta response_time_in_cache = cache_->clock_->Now() - response_.response_time;

    if (!base::FeatureList::IsEnabled(features::kPrefetchFollowsNormalCacheSemantics) && !(effective_load_flags_ & LOAD_PREFETCH)
        && (response_time_in_cache >= base::TimeDelta())) {
        bool reused_within_time_window = response_time_in_cache < base::Minutes(kPrefetchReuseMins);
    ...
    if (validation_required_by_headers == VALIDATION_ASYNCHRONOUS) {
        ...
        if (!response_.stale_revalidate_timeout.is_null() && response_.stale_revalidate_timeout < cache_->clock_->Now()) {
            return VALIDATION_SYNCHRONOUS;
        }
    }
...
bool HttpCache::Transaction::IsResponseConditionalizable(std::string* etag_value, std::string* last_modified_value) const
{
    DCHECK(response_.headers.get());

    // This only makes sense for cached 200 or 206 responses.
    if (response_.headers->response_code() != HTTP_OK && response_.headers->response_code() != HTTP_PARTIAL_CONTENT) {
        return false;
    }
    ...
    response_.headers->EnumerateHeader(nullptr, "last-modified", last_modified_value);
...
bool HttpCache::Transaction::ShouldOpenOnlyMethods() const
{
    ...
    // fallback to create.
    return method_ == "PUT" || method_ == "DELETE" || method_ == "PATCH" || (method_ == "HEAD" && mode_ == READ_WRITE);
}
...
bool HttpCache::Transaction::ConditionalizeRequest()
{
    ...
    DCHECK(response_.headers->response_code() != HTTP_PARTIAL_CONTENT || response_.headers->HasStrongValidators());
    ...
    if (!etag_value.empty()) {
        if (use_if_range) {
            ...
            custom_request_->extra_headers.SetHeader(HttpRequestHeaders::kIfRange, etag_value);
        } else {
            custom_request_->extra_headers.SetHeader(HttpRequestHeaders::kIfNoneMatch, etag_value);
        }
    ...
    if (!last_modified_value.empty()) {
        if (use_if_range) {
            custom_request_->extra_headers.SetHeader(HttpRequestHeaders::kIfRange, last_modified_value);
        } else {
            custom_request_->extra_headers.SetHeader(HttpRequestHeaders::kIfModifiedSince, last_modified_value);
        }
    }
...
bool HttpCache::Transaction::MaybeRejectBasedOnEntryInMemoryData(uint8_t in_memory_info)
{
    ...
    if (effective_load_flags_ & LOAD_SKIP_CACHE_VALIDATION || effective_load_flags_ & LOAD_ONLY_FROM_CACHE) {
        return false;
    }
...
bool HttpCache::Transaction::ComputeUnusablePerCachingHeaders()
{
    ...
    // override.
    auto freshness_lifetimes = response_.headers->GetFreshnessLifetimes(response_.response_time);
    return freshness_lifetimes.freshness.is_zero() && freshness_lifetimes.staleness.is_zero();
}
...
bool HttpCache::Transaction::ValidatePartialResponse()
{
    ...
    bool failure = response_code == HTTP_OK || response_code == HTTP_REQUESTED_RANGE_NOT_SATISFIABLE;
    ...
    if (partial_->IsCurrentRangeCached()) {
        ...
        if (response_code == HTTP_NOT_MODIFIED && partial_->ResponseHeadersOK(headers)) {
            return true;
        }
    } else {
        ...
        if (!reading_ && !is_sparse_ && !partial_response) {
            ...
            if (response_code == HTTP_OK || (!truncated_ && response_code != HTTP_NOT_MODIFIED && response_code != HTTP_REQUESTED_RANGE_NOT_SATISFIABLE)) {
...
int HttpCache::Transaction::DoConnectedCallback()
{
    ...
    auto type = response_.WasFetchedViaProxy() ? TransportType::kCachedFromProxy : TransportType::kCached;
...
{
    if (result != OK) {
        if (result == ERR_CACHED_IP_ADDRESS_SPACE_BLOCKED_BY_PRIVATE_NETWORK_ACCESS_POLICY) {
            DoomInconsistentEntry();
            UpdateCacheEntryStatus(CacheEntryStatus::ENTRY_OTHER);
            TransitionToState(reading_ ? STATE_SEND_REQUEST : STATE_HEADERS_PHASE_CANNOT_PROCEED);
            return OK;
        }
...
{
    TRACE_EVENT_INSTANT("net", "HttpCacheTransaction::DoSetupEntryForRead", perfetto::Track(trace_id_));
    if (network_trans_) {
        ResetNetworkTransaction();
    }
    ...
    if (partial_) {
        if (truncated_ || is_sparse_
            || (!invalid_range_ && (response_.headers->response_code() == HTTP_OK || response_.headers->response_code() == HTTP_PARTIAL_CONTENT))) {
...
int HttpCache::Transaction::WriteResponseInfoToEntry(const HttpResponseInfo& response, bool truncated)
{
    DCHECK(response.headers);
    TRACE_EVENT_INSTANT("net", "HttpCacheTransaction::WriteResponseInfoToEntry", perfetto::Track(trace_id_), "truncated", truncated);
    ...
    if (IsCertStatusError(response.ssl_info.cert_status) || UpdateAndReportCacheability(*response.headers)) {
        if (partial_) {
            partial_->FixResponseHeaders(response_.headers.get(), true);
        }
        ...
        DCHECK(stopped);
        net_log_.EndEventWithNetErrorCode(NetLogEventType::HTTP_CACHE_WRITE_INFO, OK);
        return OK;
    }
    ...
    if (!entry_->IsDoomed()) {
        cache_->GetCurrentBackend()->SetEntryInMemoryData(cache_key_, ComputeUnusablePerCachingHeaders() ? HINT_UNUSABLE_PER_CACHING_HEADERS : 0);
    }

    BeginDiskCacheAccessTimeCount();
    return entry_->GetEntry()->WriteData(kResponseInfoIndex, 0, data.get(), io_buf_len_, io_callback_, true);
}

int HttpCache::Transaction::OnWriteResponseInfoToEntryComplete(int result)
{
    TRACE_EVENT_INSTANT("net", "HttpCacheTransaction::OnWriteResponseInfoToEntryComplete", perfetto::Track(trace_id_), "result", result);
    EndDiskCacheAccessTimeCount(DiskCacheAccessType::kWrite);
    if (!entry_) {
        return OK;
    }
    net_log_.EndEventWithNetErrorCode(NetLogEventType::HTTP_CACHE_WRITE_INFO, result);
...
{
    TRACE_EVENT_INSTANT("net", "HttpCacheTransaction::DoneWithEntry", perfetto::Track(trace_id_), "entry_is_complete", entry_is_complete);
    if (!entry_) {
        return;
    }
...
int HttpCache::Transaction::OnCacheReadError(int result, bool restart)
{
    ...
    if (restart) {
        ...
        // `HttpCache::Transaction::WriterAboutToBeRemovedFromEntry` which accesses
        // `this`'s `entry_` member.
        cache_->DoneWithEntry(entry_, this, true /* entry_is_complete */, partial_ != nullptr);
        entry_.reset();
        is_sparse_ = false;
...
void HttpCache::Transaction::OnCacheLockTimeout(base::TimeTicks start_time)
{
    ...
    DCHECK(next_state_ == STATE_ADD_TO_ENTRY_COMPLETE || next_state_ == STATE_FINISH_HEADERS_COMPLETE || waiting_for_cache_io_);
...
void HttpCache::Transaction::DoomPartialEntry(bool delete_object)
{
    ...
    // `HttpCache::Transaction::WriterAboutToBeRemovedFromEntry` which accesses
    // `this`'s `entry_` member.
    cache_->DoneWithEntry(entry_, this, false /* entry_is_complete */, partial_ != nullptr);
    entry_.reset();
    is_sparse_ = false;
    truncated_ = false;
    if (delete_object) {
...
const HttpTransaction* HttpCache::Transaction::GetOwnedOrMovedNetworkTransaction() const
{
    if (network_trans_) {
        return network_trans_.get();
    }
...
{
    return const_cast<HttpTransaction*>(static_cast<const Transaction*>(this)->network_transaction());
}
...
void HttpCache::Transaction::SetAuthResponse(const HttpResponseInfo& auth_response)
{
    auth_response_ = auth_response;
    SyncCacheEntryStatusToResponse();
}

void HttpCache::Transaction::UpdateCacheEntryStatus(CacheEntryStatus new_cache_entry_status)
{
...
void HttpCache::Transaction::RecordHistograms()
{
    ...
    if (!cache_.get() || !cache_->GetCurrentBackend() || cache_->GetCurrentBackend()->GetCacheType() != DISK_CACHE || cache_->mode() != NORMAL
        || method_ != "GET") {
        return;
    }
    ...
    if (request_->possibly_top_frame_origin) {
        is_third_party = !request_->possibly_top_frame_origin->IsSameOriginWith(request_->url);
    }

    std::string mime_type;
    HttpResponseHeaders* response_headers = GetResponseInfo()->headers.get();
    const bool is_no_store = response_headers && response_headers->HasHeaderValue("cache-control", "no-store");
    if (response_headers && response_headers->GetMimeType(&mime_type)) {
        ...
        // estimate.
        if (mime_type == "text/html" && (effective_load_flags_ & LOAD_MAIN_FRAME_DEPRECATED)) {
        ...
        } else if (mime_type.ends_with("javascript") || mime_type.ends_with("ecmascript")) {
            if (is_third_party) {
    ...
    UMA_HISTOGRAM_CUSTOM_TIMES("HttpCache.AccessToDone2", total_time, base::Milliseconds(1), base::Seconds(30), 100);
    ...
    DCHECK((did_send_request
               && (cache_entry_status_ == CacheEntryStatus::ENTRY_NOT_IN_CACHE || cache_entry_status_ == CacheEntryStatus::ENTRY_VALIDATED
                   || cache_entry_status_ == CacheEntryStatus::ENTRY_UPDATED || cache_entry_status_ == CacheEntryStatus::ENTRY_CANT_CONDITIONALIZE
                   || cache_entry_status_ == CacheEntryStatus::ENTRY_USED))
        || (!did_send_request && (cache_entry_status_ == CacheEntryStatus::ENTRY_USED || cache_entry_status_ == CacheEntryStatus::ENTRY_CANT_CONDITIONALIZE)));

    if (!did_send_request) {
        if (cache_entry_status_ == CacheEntryStatus::ENTRY_USED) {
            UMA_HISTOGRAM_CUSTOM_TIMES("HttpCache.AccessToDone2.Used", total_time, base::Milliseconds(1), base::Seconds(3), 100);
        }
        return;
    }
    ...
    UMA_HISTOGRAM_CUSTOM_TIMES("HttpCache.AccessToDone2.SentRequest", total_time, base::Milliseconds(1), base::Seconds(30), 100);
    ...
    case CacheEntryStatus::ENTRY_CANT_CONDITIONALIZE: {
        UMA_HISTOGRAM_TIMES("HttpCache.BeforeSend.CantConditionalize", before_send_time);
        break;
    }
    ...
    if (!total_disk_cache_read_time_.is_zero()) {
        base::UmaHistogramTimes("HttpCache.TotalDiskCacheTimePerTransaction.Read", total_disk_cache_read_time_);
    }
    if (!total_disk_cache_write_time_.is_zero()) {
        base::UmaHistogramTimes("HttpCache.TotalDiskCacheTimePerTransaction.Write", total_disk_cache_write_time_);
    }
}

bool HttpCache::Transaction::InWriters() const
{
    return entry_ && entry_->HasWriters() && entry_->writers()->HasTransaction(this);
}

HttpCache::Transaction::NetworkTransactionInfo::NetworkTransactionInfo() = default;
HttpCache::Transaction::NetworkTransactionInfo::~NetworkTransactionInfo() = default;

void HttpCache::Transaction::SaveNetworkTransactionInfo(const HttpTransaction& transaction)
{
    DCHECK(!network_transaction_info_.old_network_trans_load_timing);
    LoadTimingInfo load_timing;
    if (transaction.GetLoadTimingInfo(&load_timing)) {
        network_transaction_info_.old_network_trans_load_timing = std::make_unique<LoadTimingInfo>(load_timing);
    }

    network_transaction_info_.total_received_bytes += transaction.GetTotalReceivedBytes();
    network_transaction_info_.total_sent_bytes += transaction.GetTotalSentBytes();
    network_transaction_info_.received_body_bytes = transaction.GetReceivedBodyBytes();
...
bool HttpCache::Transaction::UpdateAndReportCacheability(const HttpResponseHeaders& headers)
{
...
void HttpCache::Transaction::UpdateSecurityHeadersBeforeForwarding()
{
    ...
    std::string stored_corp_header = response_.headers->GetNormalizedHeader("Cross-Origin-Resource-Policy").value_or(std::string());
    if (!stored_corp_header.empty()) {
        new_response_->headers->SetHeader("Cross-Origin-Resource-Policy", stored_corp_header);
    }
    return;
}
...
void HttpCache::Transaction::EndDiskCacheAccessTimeCount(DiskCacheAccessType type)
{
    ...
    base::TimeDelta elapsed = TimeTicks::Now() - last_disk_cache_access_start_time_;
    switch (type) {
