...
public:
    enum class State { WAIT_FOR_INIT, WAIT_FOR_CHALLENGE, WAIT_FOR_GENERATE_AUTH_TOKEN, TOKEN_PENDING, DONE };
    ...
    public:
        ...
        void AddMockHandler(std::unique_ptr<HttpAuthHandler> handler, HttpAuth::Target target);
    ...
    private:
        std::vector<std::unique_ptr<HttpAuthHandler>> handlers_[HttpAuth::AUTH_NUM_TARGETS];
