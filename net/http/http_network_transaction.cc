...
WebSocketFallbackResult CalculateWebSocketFallbackResult(int result, bool http_1_1_was_required, HttpConnectionInfoCoarse connection_info)
{
    if (result == OK) {
        if (connection_info == HttpConnectionInfoCoarse::kHTTP2) {
            return WebSocketFallbackResult::kSuccessHttp2;
        }
        return http_1_1_was_required ? WebSocketFallbackResult::kSuccessHttp11AfterFallback : WebSocketFallbackResult::kSuccessHttp11;
    }

    return http_1_1_was_required ? WebSocketFallbackResult::kFailureAfterFallback : WebSocketFallbackResult::kFailure;
}

void RecordWebSocketFallbackResult(int result, bool http_1_1_was_required, HttpConnectionInfoCoarse connection_info)
{
    ...
    base::UmaHistogramEnumeration("Net.WebSocket.FallbackResult", CalculateWebSocketFallbackResult(result, http_1_1_was_required, connection_info));
}

const std::string_view NegotiatedProtocolToHistogramSuffix(const HttpResponseInfo& response)
{
...
HttpNetworkTransaction::HttpNetworkTransaction(RequestPriority priority, HttpNetworkSession* session)
    : io_callback_(base::BindRepeating(&HttpNetworkTransaction::OnIOComplete, base::Unretained(this)))
    , session_(session)
    , priority_(priority)
{
}
...
HttpNetworkTransaction::~HttpNetworkTransaction()
{
    ...
    if (stream_.get()) {
        ...
        //                and pass it in.
        if (!stream_->CanReuseConnection() || next_state_ != STATE_NONE || close_connection_on_destruction_) {
...
int HttpNetworkTransaction::Start(const HttpRequestInfo* request_info, CompletionOnceCallback callback, const NetLogWithSource& net_log)
{
    ...
    if (std::optional<std::string> header = request_->extra_headers.GetHeader(HttpRequestHeaders::kReferer); header) {
        request_referrer_.swap(header.value());
    }
    if (std::optional<std::string> header = request_->extra_headers.GetHeader(HttpRequestHeaders::kUserAgent); header) {
    ...
    if (request_->idempotency == IDEMPOTENT || (request_->idempotency == DEFAULT_IDEMPOTENCY && HttpUtil::IsMethodSafe(request_info->method))) {
...
int HttpNetworkTransaction::RestartIgnoringLastError(CompletionOnceCallback callback)
{
    DCHECK(!stream_.get());
    ...
    scoped_refptr<X509Certificate> client_cert, scoped_refptr<SSLPrivateKey> client_private_key, CompletionOnceCallback callback)
{
    ...
    // then pick them up.
    session_->ssl_client_context()->SetClientCertificate(response_.cert_request_info->host_and_port, std::move(client_cert), std::move(client_private_key));
...
int HttpNetworkTransaction::RestartWithAuth(const AuthCredentials& credentials, CompletionOnceCallback callback)
{
    if (!CheckMaxRestarts())
        return ERR_TOO_MANY_RETRIES;
...
void HttpNetworkTransaction::PrepareForAuthRestart(HttpAuth::Target target)
{
    ...
    if (target == HttpAuth::AUTH_SERVER && auth_controllers_[target]->NeedsHTTP11()) {
        ...
        session_->http_server_properties()->SetHTTP11Required(url::SchemeHostPort(rewritten_url), network_anonymization_key_);
    }
    ...
    if (stream_->CanReuseConnection()) {
        ...
        if (!stream_->IsResponseBodyComplete()) {
            ...
            read_buf_ = base::MakeRefCounted<IOBufferWithSize>(kDrainBodyBufferSize); // A bit bucket.
...
int HttpNetworkTransaction::Read(IOBuffer* buf, int buf_len, CompletionOnceCallback callback)
{
    DCHECK(buf);
    DCHECK_LT(0, buf_len);
    ...
    if (headers_valid_ && headers.get() && stream_request_.get()) {
        ...
        DCHECK(proxy_info_.AnyProxyInChain([](const ProxyServer& s) { return s.is_http_like(); }));
...
void HttpNetworkTransaction::SetQuicServerInfo(QuicServerInfo* quic_server_info)
{
}

bool HttpNetworkTransaction::GetLoadTimingInfo(LoadTimingInfo* load_timing_info) const
{
    ...
    if (!dns_resolution_start_time_override_.is_null() && !dns_resolution_end_time_override_.is_null()
        && (dns_resolution_start_time_override_ < load_timing_info->connect_timing.domain_lookup_start)
        && (dns_resolution_end_time_override_ < load_timing_info->connect_timing.domain_lookup_end)) {
        load_timing_info->connect_timing.domain_lookup_start = dns_resolution_start_time_override_;
        load_timing_info->connect_timing.domain_lookup_end = dns_resolution_end_time_override_;
    }
...
void HttpNetworkTransaction::PopulateNetErrorDetails(NetErrorDetails* details) const
{
...
void HttpNetworkTransaction::SetWebSocketHandshakeStreamCreateHelper(WebSocketHandshakeStreamBase::CreateHelper* create_helper)
{
    websocket_handshake_stream_base_create_helper_ = create_helper;
}

void HttpNetworkTransaction::SetBeforeNetworkStartCallback(BeforeNetworkStartCallback callback)
{
    before_network_start_callback_ = std::move(callback);
}

void HttpNetworkTransaction::SetConnectedCallback(const ConnectedCallback& callback)
{
    connected_callback_ = callback;
}

void HttpNetworkTransaction::SetRequestHeadersCallback(RequestHeadersCallback callback)
{
    DCHECK(!stream_);
    request_headers_callback_ = std::move(callback);
}

void HttpNetworkTransaction::SetEarlyResponseHeadersCallback(ResponseHeadersCallback callback)
{
    DCHECK(!stream_);
    early_response_headers_callback_ = std::move(callback);
}

void HttpNetworkTransaction::SetResponseHeadersCallback(ResponseHeadersCallback callback)
{
    DCHECK(!stream_);
    response_headers_callback_ = std::move(callback);
}

void HttpNetworkTransaction::SetModifyRequestHeadersCallback(base::RepeatingCallback<void(HttpRequestHeaders*)> callback)
{
    modify_headers_callbacks_ = std::move(callback);
}

void HttpNetworkTransaction::SetIsSharedDictionaryReadAllowedCallback(base::RepeatingCallback<bool()> callback)
{
...
void HttpNetworkTransaction::OnStreamReady(const ProxyInfo& used_proxy_info, std::unique_ptr<HttpStream> stream)
{
    DCHECK_EQ(STATE_CREATE_STREAM_COMPLETE, next_state_);
    DCHECK(stream_request_.get());
    ...
    response_.was_alpn_negotiated = stream_request_->negotiated_protocol() != kProtoUnknown;
    response_.alpn_negotiated_protocol = NextProtoToString(stream_request_->negotiated_protocol());
    response_.alternate_protocol_usage = stream_request_->alternate_protocol_usage();
    // TODO(crbug.com/40815866): Stop using `was_fetched_via_spdy`.
    response_.was_fetched_via_spdy = stream_request_->negotiated_protocol() == kProtoHTTP2;
    response_.dns_aliases = stream_->GetDnsAliases();

    dns_resolution_start_time_override_ = stream_request_->dns_resolution_start_time_override();
    dns_resolution_end_time_override_ = stream_request_->dns_resolution_end_time_override();

    SetProxyInfoInResponse(used_proxy_info, &response_);
    OnIOComplete(OK);
}

void HttpNetworkTransaction::OnBidirectionalStreamImplReady(const ProxyInfo& used_proxy_info, std::unique_ptr<BidirectionalStreamImpl> stream)
{
    NOTREACHED();
}

void HttpNetworkTransaction::OnWebSocketHandshakeStreamReady(const ProxyInfo& used_proxy_info, std::unique_ptr<WebSocketHandshakeStreamBase> stream)
{
    OnStreamReady(used_proxy_info, std::move(stream));
}

void HttpNetworkTransaction::OnStreamFailed(
    int result, const NetErrorDetails& net_error_details, const ProxyInfo& used_proxy_info, ResolveErrorInfo resolve_error_info)
{
    DCHECK_EQ(STATE_CREATE_STREAM_COMPLETE, next_state_);
    DCHECK_NE(OK, result);
    DCHECK(stream_request_.get());
...
void HttpNetworkTransaction::OnCertificateError(int result, const SSLInfo& ssl_info)
{
    DCHECK_EQ(STATE_CREATE_STREAM_COMPLETE, next_state_);
    DCHECK_NE(OK, result);
    DCHECK(stream_request_.get());
...
void HttpNetworkTransaction::OnNeedsProxyAuth(const HttpResponseInfo& proxy_response, const ProxyInfo& used_proxy_info, HttpAuthController* auth_controller)
{
    DCHECK(stream_request_.get());
    DCHECK_EQ(STATE_CREATE_STREAM_COMPLETE, next_state_);
...
void HttpNetworkTransaction::OnSwitchesToHttpStreamPool(HttpStreamPoolSwitchingInfo switching_info)
{
    CHECK_EQ(STATE_CREATE_STREAM_COMPLETE, next_state_);
    CHECK(stream_request_);
    stream_request_.reset();

    stream_request_ = session_->http_stream_pool()->RequestStream(this, std::move(switching_info), priority_,
        /*allowed_bad_certs=*/observed_bad_certs_, enable_ip_based_pooling_, enable_alternative_services_, net_log_);
...
{
    return proxy_info_.proxy_chain().is_get_to_proxy_allowed() && request_->url.SchemeIs("http");
}
...
int HttpNetworkTransaction::DoLoop(int result)
{
    ...
    do {
        ...
        case STATE_SEND_REQUEST_COMPLETE:
            ...
            net_log_.EndEventWithNetErrorCode(NetLogEventType::HTTP_TRANSACTION_SEND_REQUEST, rv);
            break;
        ...
        case STATE_READ_HEADERS_COMPLETE:
            ...
            net_log_.EndEventWithNetErrorCode(NetLogEventType::HTTP_TRANSACTION_READ_HEADERS, rv);
            break;
        ...
        case STATE_READ_BODY_COMPLETE:
            ...
            net_log_.EndEventWithNetErrorCode(NetLogEventType::HTTP_TRANSACTION_READ_BODY, rv);
            break;
        case STATE_DRAIN_BODY_FOR_AUTH_RESTART:
            DCHECK_EQ(OK, rv);
            net_log_.BeginEvent(NetLogEventType::HTTP_TRANSACTION_DRAIN_BODY_FOR_AUTH_RESTART);
            rv = DoDrainBodyForAuthRestart();
            break;
        case STATE_DRAIN_BODY_FOR_AUTH_RESTART_COMPLETE:
            rv = DoDrainBodyForAuthRestartComplete(rv);
            net_log_.EndEventWithNetErrorCode(NetLogEventType::HTTP_TRANSACTION_DRAIN_BODY_FOR_AUTH_RESTART, rv);
            break;
        default:
...
int HttpNetworkTransaction::DoCreateStream()
{
    ...
    if (ForWebSocketHandshake()) {
        stream_request_ = session_->http_stream_factory()->RequestWebSocketHandshakeStream(*request_, priority_, /*allowed_bad_certs=*/observed_bad_certs_,
            this, websocket_handshake_stream_base_create_helper_, enable_ip_based_pooling_, enable_alternative_services_, net_log_);
    } else {
        stream_request_ = session_->http_stream_factory()->RequestStream(
            *request_, priority_, /*allowed_bad_certs=*/observed_bad_certs_, this, enable_ip_based_pooling_, enable_alternative_services_, net_log_);
    }
    DCHECK(stream_request_.get());
    return ERR_IO_PENDING;
}
...
int HttpNetworkTransaction::DoCreateStreamComplete(int result)
{
    ...
    if (result == OK) {
        ...
        base::UmaHistogramTimes(base::StrCat({ "Net.NetworkTransaction.Create", (ForWebSocketHandshake() ? "WebSocketStreamTime." : "HttpStreamTime."),
                                    (IsGoogleHostWithAlpnH3(url_.host()) ? "GoogleHost." : ""), NegotiatedProtocolToHistogramSuffix(response_) }),
...
int HttpNetworkTransaction::DoInitStream()
{
    ...
    int rv = stream_->InitializeStream(can_send_early_data_, priority_, net_log_, io_callback_);
    ...
    base::UmaHistogramBoolean(base::StrCat({ "Net.NetworkTransaction.InitializeStreamBlocked", IsGoogleHostWithAlpnH3(url_.host()) ? "GoogleHost." : ".",
...
int HttpNetworkTransaction::DoInitStreamComplete(int result)
{
    ...
    if (!blocked_initialize_stream_start_time_.is_null()) {
        base::UmaHistogramTimes(base::StrCat({ "Net.NetworkTransaction.InitializeStreamBlockTime", IsGoogleHostWithAlpnH3(url_.host()) ? "GoogleHost." : ".",
                                    NegotiatedProtocolToHistogramSuffix(response_) }),
...
int HttpNetworkTransaction::DoConnectedCallback()
{
    ...
    return connected_callback_.Run(TransportInfo(type, remote_endpoint_, std::string { stream_->GetAcceptChViaAlps() }, is_issued_by_known_root,
                                       NextProtoFromString(response_.alpn_negotiated_protocol)),
        base::BindOnce(&HttpNetworkTransaction::ResumeAfterConnected, base::Unretained(this)));
}
...
int HttpNetworkTransaction::DoGenerateProxyAuthToken()
{
    ...
    if (!auth_controllers_[target].get())
        auth_controllers_[target] = base::MakeRefCounted<HttpAuthController>(target, AuthURL(target), request_->network_anonymization_key,
            session_->http_auth_cache(), session_->http_auth_handler_factory(), session_->host_resolver());
    int rv = auth_controllers_[target]->MaybeGenerateAuthToken(request_, io_callback_, net_log_);
    // TODO(crbug.com/359404121): Remove this histogram after the investigation
    // completes.
    ...
    base::UmaHistogramBoolean(base::StrCat({ "Net.NetworkTransaction.GenerateProxyAuthTokenBlocked", IsGoogleHostWithAlpnH3(url_.host()) ? "GoogleHost." : ".",
                                  NegotiatedProtocolToHistogramSuffix(response_) }),
        blocked);
    return rv;
}
...
int HttpNetworkTransaction::DoGenerateProxyAuthTokenComplete(int rv)
{
    ...
    if (!blocked_generate_proxy_auth_token_start_time_.is_null()) {
        base::UmaHistogramTimes(base::StrCat({ "Net.NetworkTransaction.GenerateProxyAuthTokenBlockTime",
                                    IsGoogleHostWithAlpnH3(url_.host()) ? "GoogleHost." : ".", NegotiatedProtocolToHistogramSuffix(response_) }),
...
int HttpNetworkTransaction::DoGenerateServerAuthToken()
{
    ...
    base::UmaHistogramBoolean(base::StrCat({ "Net.NetworkTransaction.GenerateServerAuthTokenBlocked", IsGoogleHostWithAlpnH3(url_.host()) ? "GoogleHost." : ".",
                                  NegotiatedProtocolToHistogramSuffix(response_) }),
        blocked);
    return rv;
}
...
int HttpNetworkTransaction::DoGenerateServerAuthTokenComplete(int rv)
{
    ...
    if (!blocked_generate_server_auth_token_start_time_.is_null()) {
        base::UmaHistogramTimes(base::StrCat({ "Net.NetworkTransaction.GenerateServerAuthTokenBlockTime",
                                    IsGoogleHostWithAlpnH3(url_.host()) ? "GoogleHost." : ".", NegotiatedProtocolToHistogramSuffix(response_) }),
            base::TimeTicks::Now() - blocked_generate_server_auth_token_start_time_);
    }
    if (rv == OK)
...
int HttpNetworkTransaction::BuildRequestHeaders(bool using_http_proxy_without_tunnel)
{
    request_headers_.SetHeader(HttpRequestHeaders::kHost, GetHostAndOptionalPort(request_->url));

    // For compat with HTTP/1.0 servers and proxies:
    if (using_http_proxy_without_tunnel) {
        request_headers_.SetHeader(HttpRequestHeaders::kProxyConnection, "keep-alive");
    } else {
    ...
    if (request_->upload_data_stream) {
        if (request_->upload_data_stream->is_chunked()) {
            request_headers_.SetHeader(HttpRequestHeaders::kTransferEncoding, "chunked");
        } else {
            request_headers_.SetHeader(HttpRequestHeaders::kContentLength, base::NumberToString(request_->upload_data_stream->size()));
        }
    ...
    if (ShouldApplyProxyAuth() && HaveAuth(HttpAuth::AUTH_PROXY))
        auth_controllers_[HttpAuth::AUTH_PROXY]->AddAuthorizationHeader(&request_headers_);
    if (ShouldApplyServerAuth() && HaveAuth(HttpAuth::AUTH_SERVER))
        auth_controllers_[HttpAuth::AUTH_SERVER]->AddAuthorizationHeader(&request_headers_);

    if (features::kIpPrivacyAddHeaderToProxiedRequests.Get() && proxy_info_.is_for_ip_protection()) {
    ...
    response_.did_use_http_auth
        = request_headers_.HasHeader(HttpRequestHeaders::kAuthorization) || request_headers_.HasHeader(HttpRequestHeaders::kProxyAuthorization);
    return OK;
}
...
int HttpNetworkTransaction::DoInitRequestBody()
{
    ...
    int rv = OK;
    if (request_->upload_data_stream)
        rv = request_->upload_data_stream->Init(base::BindOnce(&HttpNetworkTransaction::OnIOComplete, base::Unretained(this)), net_log_);
    return rv;
}
...
int HttpNetworkTransaction::DoReadHeadersComplete(int result)
{
    ...
    if (ForWebSocketHandshake()) {
        RecordWebSocketFallbackResult(result, http_1_1_was_required_, HttpConnectionInfoToCoarse(response_.connection_info));
    }
    ...
    if (response_.headers->response_code() == HTTP_EARLY_HINTS) {
        NetLogResponseHeaders(net_log_, NetLogEventType::HTTP_TRANSACTION_READ_EARLY_HINTS_RESPONSE_HEADERS, response_.headers.get());
        ...
        if (EarlyHintsAreAllowedOn(response_.connection_info) && early_response_headers_callback_) {
    ...
    if (response_.headers->response_code() == HTTP_REQUEST_TIMEOUT && HttpConnectionInfoToCoarse(response_.connection_info) == HttpConnectionInfoCoarse::kHTTP1
        && stream_->IsConnectionReused()) {
#if BUILDFLAG(ENABLE_REPORTING)
        GenerateNetworkErrorLoggingReport(OK);
#endif // BUILDFLAG(ENABLE_REPORTING)
        net_log_.AddEventWithNetErrorCode(NetLogEventType::HTTP_TRANSACTION_RESTART_AFTER_ERROR, response_.headers->response_code());
    ...
    NetLogResponseHeaders(net_log_, NetLogEventType::HTTP_TRANSACTION_READ_RESPONSE_HEADERS, response_.headers.get());
    ...
    if (can_send_early_data_ && response_.headers->response_code() == HTTP_TOO_EARLY) {
    ...
    const bool has_body_with_null_source = request_->upload_data_stream && request_->upload_data_stream->has_null_source();
    if (response_.headers->response_code() == 421 && (enable_ip_based_pooling_ || enable_alternative_services_) && !has_body_with_null_source) {
#if BUILDFLAG(ENABLE_REPORTING)
        GenerateNetworkErrorLoggingReport(OK);
#endif // BUILDFLAG(ENABLE_REPORTING)
        ...
        net_log_.AddEvent(NetLogEventType::HTTP_TRANSACTION_RESTART_MISDIRECTED_REQUEST);
    ...
    if (IsSecureRequest()) {
        ...
        if (response_.ssl_info.is_valid() && !IsCertStatusError(response_.ssl_info.cert_status)) {
            session_->http_stream_factory()->ProcessAlternativeServices(
                session_, network_anonymization_key_, response_.headers.get(), url::SchemeHostPort(request_->url));
        }
    }
    ...
    if ((response_code >= 400 && response_code < 600) || response_code == HTTP_NO_CONTENT || response_code == HTTP_RESET_CONTENT
        || response_code == HTTP_NOT_MODIFIED || request_->method == "HEAD" || response_.headers->GetContentLength() == 0
...
int HttpNetworkTransaction::DoReadBody()
{
    ...
    return stream_->ReadResponseBody(read_buf_.get(), read_buf_len_, io_callback_);
}
...
int HttpNetworkTransaction::DoReadBodyComplete(int result)
{
    ...
    if (done) {
        ...
        if (!enable_alternative_services_ && retried_alternative_service_.protocol != kProtoUnknown) {
            HistogramBrokenAlternateProtocolLocation(BROKEN_ALTERNATE_PROTOCOL_LOCATION_HTTP_NETWORK_TRANSACTION);
            session_->http_server_properties()->MarkAlternativeServiceBroken(retried_alternative_service_, network_anonymization_key_);
        }
...
{
    std::optional<std::string> value = response_.headers->GetNormalizedHeader("Report-To");
    if (!value) {
        return;
    }
    ...
    reporting_service->ProcessReportToHeader(url::Origin::Create(url_), network_anonymization_key_, *value);
}

void HttpNetworkTransaction::ProcessNetworkErrorLoggingHeader()
{
    std::optional<std::string> value = response_.headers->GetNormalizedHeader(NetworkErrorLoggingService::kHeaderName);
    if (!value) {
        return;
    }

    NetworkErrorLoggingService* network_error_logging_service = session_->network_error_logging_service();
    ...
    network_error_logging_service->OnHeader(network_anonymization_key_, url::Origin::Create(url_), remote_endpoint_.address(), *value);
}
...
void HttpNetworkTransaction::GenerateNetworkErrorLoggingReport(int rv)
{
    ...
    NetworkErrorLoggingService* service = session_->network_error_logging_service();
    if (!service)
        return;

    // Don't report on proxy auth challenges.
    if (response_.headers && response_.headers->response_code() == HTTP_PROXY_AUTHENTICATION_REQUIRED) {
        return;
    }
...
int HttpNetworkTransaction::HandleSSLClientAuthError(int error)
{
    ...
    if (DCHECK_IS_ON()) {
        ...
        bool proxy_using_tls = proxy_info_.AnyProxyInChain([](const ProxyServer& s) { return s.is_secure_http_like(); });
    ...
    if (session_->ssl_client_context()->ClearClientCertificate(host_port_pair)) {
        ...
        // socket pools.
        if (is_server && error == ERR_SSL_CLIENT_AUTH_SIGNATURE_FAILED && !configured_client_cert_for_server_ && !HasExceededMaxRetries()) {
            retry_attempts_++;
            net_log_.AddEventWithNetErrorCode(NetLogEventType::HTTP_TRANSACTION_RESTART_AFTER_ERROR, error);
            ResetConnectionAndRequestForResend(RetryReason::kSslClientAuthSignatureFailed);
            return OK;
        }
    }
    return error;
...
// static
std::optional<HttpNetworkTransaction::RetryReason> HttpNetworkTransaction::GetRetryReasonForIOError(int error)
{
    switch (error) {
...
int HttpNetworkTransaction::HandleIOError(int error)
{
    ...
    std::optional<HttpNetworkTransaction::RetryReason> retry_reason = GetRetryReasonForIOError(error);
    if (!retry_reason) {
        return error;
    }
    switch (*retry_reason) {
    ...
    case RetryReason::kQuicProtocolError:
        if (HasExceededMaxRetries() || GetResponseHeaders() != nullptr || !stream_->GetAlternativeService(&retried_alternative_service_)) {
        ...
        if (session_->http_server_properties()->IsAlternativeServiceBroken(retried_alternative_service_, network_anonymization_key_)) {
            ...
            ResetConnectionAndRequestForResend(*retry_reason);
            error = OK;
        } else if (session_->context().quic_context->params()->retry_without_alt_svc_on_quic_errors) {
...
void HttpNetworkTransaction::ResetConnectionAndRequestForResend(RetryReason retry_reason)
{
    // TODO:(crbug.com/1495705): Remove this CHECK after fixing the bug.
    CHECK(request_);
    base::UmaHistogramEnumeration(
        IsGoogleHostWithAlpnH3(url_.host()) ? "Net.NetworkTransactionH3SupportedGoogleHost.RetryReason" : "Net.NetworkTransaction.RetryReason", retry_reason);
...
int HttpNetworkTransaction::HandleAuthChallenge()
{
    ...
    int rv = auth_controllers_[target]->HandleAuthChallenge(headers, response_.ssl_info, !ShouldApplyServerAuth(), false, net_log_);
...
GURL HttpNetworkTransaction::AuthURL(HttpAuth::Target target) const
{
    ...
    case HttpAuth::AUTH_PROXY: {
        ...
        const char* scheme = proxy_server.is_secure_http_like() ? "https://" : "http://";
...
bool HttpNetworkTransaction::ContentEncodingsValid() const
{
    ...
    if (!HttpUtil::ParseAcceptEncoding(request_headers_.GetHeader(HttpRequestHeaders::kAcceptEncoding).value_or(std::string()), &allowed_encodings)) {
        return false;
    }

    std::string content_encoding = headers->GetNormalizedHeader("Content-Encoding").value_or(std::string());
    ...
    for (auto const& encoding : used_encodings) {
        SourceStream::SourceType source_type = FilterSourceStream::ParseEncodingType(encoding);
...
// static
void HttpNetworkTransaction::SetProxyInfoInResponse(const ProxyInfo& proxy_info, HttpResponseInfo* response_info)
{
