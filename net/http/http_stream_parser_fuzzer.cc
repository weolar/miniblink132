...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    scoped_refptr<net::GrowableIOBuffer> read_buffer = base::MakeRefCounted<net::GrowableIOBuffer>();
    // Use a NetLog that listens to events, to get coverage of logging
    // callbacks.
    net::HttpStreamParser parser(&fuzzed_socket, false /* is_reused */, GURL("http://localhost/"), "GET",
        /*upload_data_stream=*/nullptr, read_buffer.get(), net_log_with_source);

    net::HttpResponseInfo response_info;
    int result = parser.SendRequest("GET / HTTP/1.1\r\n", net::HttpRequestHeaders(), TRAFFIC_ANNOTATION_FOR_TESTS, &response_info, callback.callback());
    ...
    while (true) {
        ...
        result = parser.ReadResponseBody(io_buffer.get(), io_buffer->size(), callback.callback());
