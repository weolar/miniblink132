...
public:
    ...
    public:
        ...
        void set_nonce_generator(std::unique_ptr<const NonceGenerator> nonce_generator);
...
private:
    ...
    HttpAuth::AuthorizationResult HandleAnotherChallengeImpl(HttpAuthChallengeTokenizer* challenge) override;
    ...
    void GetRequestMethodAndPath(const HttpRequestInfo* request, std::string* method, std::string* path) const;

    // Build up  the 'response' production.
    std::string AssembleResponseDigest(
    ...
    std::string AssembleCredentials(
