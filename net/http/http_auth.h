...
public:
    ...
    static const char* AuthorizationResultToString(AuthorizationResult authorization_result);

    // Returns a value for logging an authorization result to a NetLog.
    static base::Value::Dict NetLogAuthorizationResultParams(const char* name, AuthorizationResult authorization_result);
    ...
    static void ChooseBestChallenge(HttpAuthHandlerFactory* http_auth_handler_factory, const HttpResponseHeaders& response_headers, const SSLInfo& ssl_info,
    ...
    static AuthorizationResult HandleChallengeResponse(HttpAuthHandler* handler, const HttpResponseHeaders& response_headers, Target target,
        const std::set<Scheme>& disabled_schemes, std::string* challenge_used);
};
