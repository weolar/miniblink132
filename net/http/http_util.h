...
public:
    ...
    static std::string GenerateRequestLine(std::string_view method, const GURL& url, bool is_for_get_to_http_proxy);
    ...
    static void ParseContentType(std::string_view content_type_str, std::string* mime_type, std::string* charset, bool* had_charset, std::string* boundary);

    // Parses the value of a "Range" header as defined in RFC 7233 Section 2.1.
    // https://tools.ietf.org/html/rfc7233#section-2.1
    // Returns false on failure.
    static bool ParseRangeHeader(const std::string& range_specifier, std::vector<HttpByteRange>* ranges);
    ...
    static bool ParseRetryAfterHeader(const std::string& retry_after_string, base::Time now, base::TimeDelta* retry_after);
    ...
    static void TrimLWS(std::string::const_iterator* begin, std::string::const_iterator* end);
    ...
    [[nodiscard]] static bool StrictUnquote(std::string_view str, std::string* out);
    ...
    static size_t LocateEndOfAdditionalHeaders(base::span<const uint8_t> buf, size_t i = 0);
    ...
    static std::string GenerateAcceptLanguageHeader(const std::string& raw_language_list);
    ...
    static bool HasStrongValidators(HttpVersion version, std::optional<std::string_view> etag_header, std::optional<std::string_view> last_modified_header,
    ...
    // Non-nullopt times will be converted to std::strings and parsed, which can
    // be somewhat expensive.
    static bool HasValidators(HttpVersion version, std::optional<std::string_view> etag_header, std::optional<std::string_view> last_modified_header);
    ...
    static bool ParseAcceptEncoding(const std::string& accept_encoding, std::set<std::string>* allowed_encodings);
    ...
    // 3.5 of RFC 2616.
    static bool ParseContentEncoding(const std::string& content_encoding, std::set<std::string>* used_encodings);

    // Return true if `headers` contain multiple `field_name` fields with
    // different values.
    static bool HeadersContainMultipleCopiesOfField(const HttpResponseHeaders& headers, const std::string& field_name);
    ...
    public:
        HeadersIterator(std::string::const_iterator headers_begin, std::string::const_iterator headers_end, const std::string& line_delimiter);
        ~HeadersIterator();
    ...
    public:
        ValuesIterator(std::string_view values, char delimiter, bool ignore_empty_values = true);
    ...
    public:
        ...
        NameValuePairsIterator(std::string_view value, char delimiter, Values optional_values = Values::REQUIRED, Quotes strict_quotes = Quotes::NOT_STRICT);
        ...
        bool value_is_quoted() const
        {
            return value_is_quoted_;
        }

        // The value before unquoting (if any).
        std::string_view raw_value() const LIFETIME_BOUND
        {
            return value_;
        }
