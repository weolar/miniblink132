...
public:
    ...
    Transaction::Mode mode() const override
    {
...
public:
    TestHttpCache(std::unique_ptr<HttpTransactionFactory> network_layer, std::unique_ptr<BackendFactory> backend_factory)
        : HttpCache(std::move(network_layer), std::move(backend_factory))
    {
    }

    void WritersDoneWritingToEntry(scoped_refptr<ActiveEntry> entry, bool success, bool should_keep_entry, TransactionSet make_readers) override
    {
    ...
    size_t MakeReadersSize() const
    {
        return make_readers_size_;
    }
...
public:
    ...
    WritersTest()
        ...
        , test_cache_(std::make_unique<MockNetworkLayer>(), std::make_unique<MockBackendFactory>())
        , request_(kSimpleGET_Transaction)
    {
        scoped_transaction_.response_headers = "Last-Modified: Wed, 28 Nov 2007 00:40:09 GMT\n"
                                               "Content-Length: 22\n"
                                               "Etag: \"foopy\"\n";
    ...
    void CreateWriters()
    {
        cache_.CreateBackendEntry(GenerateCacheKey(kSimpleGET_Transaction.url), &disk_entry_.AsEphemeralRawAddr(), nullptr);
        entry_ = new HttpCache::ActiveEntry(cache_.GetWeakPtr(), disk_entry_, false);
        (static_cast<MockDiskEntry*>(disk_entry_))->AddRef();
        writers_ = std::make_unique<HttpCache::Writers>(&test_cache_, base::WrapRefCounted(entry_.get()));
    }
    ...
    void CreateWritersAddTransaction(
        HttpCache::ParallelWritingPattern parallel_writing_pattern_ = HttpCache::PARALLEL_WRITING_JOIN, bool content_encoding_present = false)
    {
        ...
        network_transaction->Start(&request_, callback.callback(), NetLogWithSource());
        ...
        std::unique_ptr<TestHttpCacheTransaction> transaction = std::make_unique<TestHttpCacheTransaction>(DEFAULT_PRIORITY, cache_.http_cache());

        CreateWriters();
        EXPECT_TRUE(writers_->IsEmpty());
        HttpCache::Writers::TransactionInfo info(transaction->partial(), transaction->is_truncated(), response_info_);

        writers_->AddTransaction(transaction.get(), parallel_writing_pattern_, transaction->priority(), info);
        writers_->SetNetworkTransaction(transaction.get(), std::move(network_transaction));
        EXPECT_TRUE(writers_->HasTransaction(transaction.get()));
        transactions_.push_back(std::move(transaction));
    }

    void CreateWritersAddTransactionPriority(
        RequestPriority priority, HttpCache::ParallelWritingPattern parallel_writing_pattern_ = HttpCache::PARALLEL_WRITING_JOIN)
    {
    ...
    void AddTransactionToExistingWriters()
    {
        ...
        HttpCache::Writers::TransactionInfo info(transaction->partial(), transaction->is_truncated(), *(transaction->GetResponseInfo()));
        info.response_info = response_info_;
        writers_->AddTransaction(transaction.get(), HttpCache::PARALLEL_WRITING_JOIN, transaction->priority(), info);
        transactions_.push_back(std::move(transaction));
    }
    ...
    int Read(std::string* result)
    {
        ...
        do {
            ...
            rv = writers_->Read(buf.get(), kDefaultBufferSize, callback.callback(), transaction);
    ...
    void ReadVerifyTwoDifferentBufferLengths(const std::vector<int>& buffer_lengths)
    {
        ...
        for (size_t i = 0; i < transactions_.size(); i++) {
            rv = writers_->Read(bufs[i].get(), buffer_lengths[i], callbacks[i].callback(), transactions_[i].get());
        ...
        std::vector<int> expected_lengths = { buffer_lengths[0], buffer_lengths[0] < buffer_lengths[1] ? buffer_lengths[0] : buffer_lengths[1] };
    ...
    void ReadAllDeleteTransaction(DeleteTransactionType deleteType)
    {
        ...
        do {
            ...
            for (size_t i = 0; i < transactions_.size(); i++) {
                bufs.push_back(base::MakeRefCounted<IOBufferWithSize>(kDefaultBufferSize));

                // If we have deleted a transaction in the first iteration, then do not
                // invoke Read on it, in subsequent iterations.
                if (!first_iter && deleteType != DeleteTransactionType::NONE && i == delete_index) {
                    continue;
                }
                ...
                rv = writers_->Read(bufs[i].get(), kDefaultBufferSize, callbacks[i].callback(), transactions_[i].get());
                EXPECT_EQ(ERR_IO_PENDING, rv); // Since the default is asynchronous.
            }

            if (first_iter && deleteType != DeleteTransactionType::NONE) {
                writers_->RemoveTransaction(transactions_.at(delete_index).get(), false /* success */);
            }
            ...
            if (rv > 0) {
                for (size_t i = 0; i < results.size(); i++) {
                    if (i == delete_index && deleteType != DeleteTransactionType::NONE && deleteType != DeleteTransactionType::ACTIVE) {
                        continue;
                    }
    ...
    void StopMidRead()
    {
        ...
        rv = writers_->Read(buf.get(), 5, base::BindOnce([](int rv) {}), transaction);
        EXPECT_EQ(ERR_IO_PENDING, rv);
    }

    void ReadAll()
    {
        ReadAllDeleteTransaction(DeleteTransactionType::NONE);
    }
    ...
    int ReadCacheWriteFailure(std::vector<std::string>* results)
    {
        ...
        do {
            ...
            disk_cache::Entry* en;
            cache_.OpenBackendEntry(GenerateCacheKey(kSimpleGET_Transaction.url), &en);
            en->Close();
            ...
            for (size_t i = 0; i < transactions_.size(); i++) {
                ...
                rv = writers_->Read(bufs[i].get(), 30, callbacks[i].callback(), transactions_[i].get());
                EXPECT_EQ(ERR_IO_PENDING, rv); // Since the default is asynchronous.
            }
    ...
    bool ShouldKeepEntry() const
    {
        return writers_->should_keep_entry_;
    }
    ...
    bool Truncated() const
    {
        ...
        int rv = disk_entry_->ReadData(kResponseInfoIndex, 0, read_buffer.get(), read_buffer->size(), callback.callback());
        rv = callback.GetResult(rv);
        HttpResponseInfo response_info;
        bool truncated;
        HttpCache::ParseResponseInfo(read_buffer->span(), &response_info, &truncated);
        return truncated;
    }

    bool ShouldTruncate()
    {
        return writers_->ShouldTruncate();
    }
...
{
    ScopedMockTransaction transaction(kSimpleGET_Transaction, "http://failure.example/");
...
{
    CreateWritersAddTransaction(HttpCache::PARALLEL_WRITING_JOIN, true /* content_encoding_present */);
    std::string result;
    ReadFewBytes(&result);
