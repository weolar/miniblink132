...
public:
    ...
    public:
        ...
        virtual disk_cache::BackendResult CreateBackend(NetLog* net_log, base::OnceCallback<void(disk_cache::BackendResult)> callback) = 0;

#if BUILDFLAG(IS_ANDROID)
        virtual void SetAppStatusListenerGetter(disk_cache::ApplicationStatusListenerGetter app_status_listener_getter)
        {
        }
#endif
    };
    ...
    public:
        ...
        DefaultBackend(CacheType type, BackendType backend_type, scoped_refptr<disk_cache::BackendFileOperationsFactory> file_operations_factory,
            const base::FilePath& path, int max_bytes, bool hard_reset);
        ...
        disk_cache::BackendResult CreateBackend(NetLog* net_log, base::OnceCallback<void(disk_cache::BackendResult)> callback) override;

#if BUILDFLAG(IS_ANDROID)
        void SetAppStatusListenerGetter(disk_cache::ApplicationStatusListenerGetter app_status_listener_getter) override;
#endif

    private:
        CacheType type_;
        BackendType backend_type_;
        const scoped_refptr<disk_cache::BackendFileOperationsFactory> file_operations_factory_;
    ...
    HttpCache(std::unique_ptr<HttpTransactionFactory> network_layer, std::unique_ptr<BackendFactory> backend_factory);
    ...
    HttpTransactionFactory* network_layer()
    {
    ...
    static bool ParseResponseInfo(base::span<const uint8_t> data, HttpResponseInfo* response_info, bool* response_truncated);
    ...
    // `include_credentials`.
    void OnExternalCacheHit(const GURL& url, const std::string& http_method, const NetworkIsolationKey& network_isolation_key, bool include_credentials);
    ...
    void SimulateCacheLockTimeoutForTesting()
    {
    ...
    int CreateTransaction(RequestPriority priority, std::unique_ptr<HttpTransaction>* transaction) override;
    ...
    std::unique_ptr<HttpTransactionFactory> SetHttpNetworkTransactionFactoryForTesting(std::unique_ptr<HttpTransactionFactory> new_network_layer);
    ...
    static std::optional<std::string> GenerateCacheKeyForRequest(const HttpRequestInfo* request);
...
private:
    ...
    enum WorkItemOperation { WI_CREATE_BACKEND, WI_OPEN_OR_CREATE_ENTRY, WI_OPEN_ENTRY, WI_CREATE_ENTRY, WI_DOOM_ENTRY };
    ...
    FRIEND_TEST_ALL_PREFIXES(HttpCacheTest_SplitCacheFeatureEnabled, SplitCacheWithNetworkIsolationKey);
    ...
    FRIEND_TEST_ALL_PREFIXES(HttpCacheTest_SplitCacheFeatureEnabled, SplitCacheUsesRegistrableDomain);

    using TransactionList = std::list<raw_ptr<Transaction, CtnExperimental>>;
    using TransactionSet = std::unordered_set<raw_ptr<Transaction, CtnExperimental>>;
    ...
    public:
        ActiveEntry(base::WeakPtr<HttpCache> cache, disk_cache::Entry* entry, bool opened_in);

        ActiveEntry(ActiveEntry const&) = delete;
        ActiveEntry& operator=(ActiveEntry const&) = delete;

        disk_cache::Entry* GetEntry()
        {
            return disk_entry_.get();
        }
        ...
        void set_will_process_queued_transactions(bool will_process_queued_transactions)
        {
        ...
        {
            return add_to_entry_queue_;
        }

        TransactionList& done_headers_queue()
        {
            return done_headers_queue_;
        }
        ...
        bool IsWritingInProgress() const
        {
            return writers_.get();
        }
        ...
        void AddTransactionToWriters(Transaction* transaction, ParallelWritingPattern parallel_writing_pattern);

        // Returns true if this transaction can write headers to the entry.
        bool CanTransactionWriteResponseHeaders(Transaction* transaction, bool is_partial, bool is_match) const;
    ...
    using ActiveEntriesMap = std::unordered_map<std::string, base::raw_ref<ActiveEntry>>;
    using PendingOpsMap = std::unordered_map<std::string, raw_ptr<PendingOp, CtnExperimental>>;
    ...
    // returns false.
    static std::string GenerateCacheKey(const GURL& url, int load_flags, const NetworkIsolationKey& network_isolation_key, int64_t upload_data_identifier,
    ...
    Error CreateAndSetWorkItem(scoped_refptr<ActiveEntry>* entry, Transaction* transaction, WorkItemOperation operation, PendingOp* pending_op);
    ...
    // isolation key.
    void DoomMainEntryForUrl(const GURL& url, const NetworkIsolationKey& isolation_key, bool is_subframe_document_resource, bool is_main_frame_navigation,
    ...
    // cache entry.
    scoped_refptr<ActiveEntry> ActivateEntry(disk_cache::Entry* disk_entry, bool opened);
    ...
    int OpenOrCreateEntry(const std::string& key, scoped_refptr<ActiveEntry>* entry, Transaction* transaction);
    ...
    int OpenEntry(const std::string& key, scoped_refptr<ActiveEntry>* entry, Transaction* transaction);
    ...
    int CreateEntry(const std::string& key, scoped_refptr<ActiveEntry>* entry, Transaction* transaction);
    ...
    int AddTransactionToEntry(scoped_refptr<ActiveEntry>& entry, Transaction* transaction);
    ...
    // ERR_CACHE_RACE.
    int DoneWithResponseHeaders(scoped_refptr<ActiveEntry>& entry, Transaction* transaction, bool is_partial);
    ...
    void DoneWithEntry(scoped_refptr<ActiveEntry>& entry, Transaction* transaction, bool entry_is_complete, bool is_partial);
    ...
    // Virtual so that it can be extended in tests.
    virtual void WritersDoneWritingToEntry(scoped_refptr<ActiveEntry> entry, bool success, bool should_keep_entry, TransactionSet make_readers);
    ...
    ParallelWritingPattern CanTransactionJoinExistingWriters(Transaction* transaction);
    ...
    // |pending_op|.
    bool RemovePendingTransactionFromPendingOp(PendingOp* pending_op, Transaction* transaction);
    ...
    // is called.
    static void OnPendingOpComplete(base::WeakPtr<HttpCache> cache, PendingOp* pending_op, int result);

    // Variant for Open/Create method family, which has a different signature.
    static void OnPendingCreationOpComplete(base::WeakPtr<HttpCache> cache, PendingOp* pending_op, disk_cache::EntryResult result);

    // Variant for CreateCacheBackend, which has a different signature.
    static void OnPendingBackendCreationOpComplete(base::WeakPtr<HttpCache> cache, PendingOp* pending_op, disk_cache::BackendResult result);
