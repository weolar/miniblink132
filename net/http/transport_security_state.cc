...
TransportSecurityState::HashedHost HashHost(base::span<const uint8_t> canonicalized_host)
{
...
std::vector<uint8_t> CanonicalizeHost(std::string_view host)
{
    ...
    std::optional<std::vector<uint8_t>> new_host = dns_names_util::DottedNameToNetwork(lowered_host,
...
public:
    ...
    bool ReadEntry(extras::PreloadDecoder::BitReader* reader, const std::string& search, size_t current_search_offset, bool* out_found) override
    {
        ...
        if (is_simple_entry) {
            ...
            if (tmp.has_pins) {
                if (!reader->Read(4, &tmp.pinset_id) || (!tmp.sts_include_subdomains && !reader->Next(&tmp.pkp_include_subdomains))) {
                    return false;
                }
            }
        }
        ...
        if (current_search_offset == 0 || search[current_search_offset - 1] == '.') {
...
bool DecodeHSTSPreload(const std::string& search_hostname, PreloadResult* out)
{
    ...
    HSTSPreloadDecoder decoder(g_hsts_source->huffman_tree, g_hsts_source->huffman_tree_size, g_hsts_source->preloaded_data, g_hsts_source->preloaded_bits,
        g_hsts_source->root_position);
    if (!decoder.Decode(hostname, &found)) {
        DCHECK(false) << "Internal error in DecodeHSTSPreload for hostname " << hostname;
        return false;
    }
    if (found)
        *out = decoder.result();
...
void SetTransportSecurityStateSourceForTesting(const TransportSecurityStateSource* source)
{
...
TransportSecurityState::TransportSecurityState(std::vector<std::string> hsts_host_bypass_list)
{
...
base::Value::Dict TransportSecurityState::NetLogUpgradeToSSLParam(const std::string& host)
{
    STSState sts_state;
    ...
    dict.Set("host_found_in_hsts_bypass_list", hsts_host_bypass_list_.find(host) != hsts_host_bypass_list_.end());
    return dict;
}

SSLUpgradeDecision TransportSecurityState::GetSSLUpgradeDecision(const std::string& host, const NetLogWithSource& net_log)
{
    net_log.AddEvent(NetLogEventType::TRANSPORT_SECURITY_STATE_SHOULD_UPGRADE_TO_SSL, [&] { return NetLogUpgradeToSSLParam(host); });
    STSState sts_state;
    ...
    if (GetDynamicSTSState(host, &sts_state)) {
        ...
        if (sts_state.ShouldUpgradeToSSL() && !(net::IsLocalHostname(host) && base::FeatureList::IsEnabled(features::kIgnoreHSTSForLocalhost))) {
            ...
            if (GetStaticSTSState(host, &static_sts_state) && static_sts_state.ShouldUpgradeToSSL()) {
...
bool TransportSecurityState::ShouldUpgradeToSSL(const std::string& host, const NetLogWithSource& net_log)
{
...
TransportSecurityState::PKPStatus TransportSecurityState::CheckPublicKeyPins(
    const HostPortPair& host_port_pair, bool is_issued_by_known_root, const HashValueVector& public_key_hashes)
{
    ...
    return CheckPublicKeyPinsImpl(host_port_pair, is_issued_by_known_root, public_key_hashes);
}
...
TransportSecurityState::CTRequirementsStatus TransportSecurityState::CheckCTRequirements(const HostPortPair& host_port_pair, bool is_issued_by_known_root,
    const HashValueVector& public_key_hashes, const X509Certificate* validated_certificate_chain, ct::CTPolicyCompliance policy_compliance)
{
    ...
    bool complies
        = (policy_compliance == ct::CTPolicyCompliance::CT_POLICY_COMPLIES_VIA_SCTS || policy_compliance == ct::CTPolicyCompliance::CT_POLICY_BUILD_NOT_TIMELY);
    ...
    if (require_ct_delegate_) {
        ...
        ct_required = require_ct_delegate_->IsCTRequiredForHost(host_port_pair.host(), validated_certificate_chain, public_key_hashes);
    }
    switch (ct_required) {
...
void TransportSecurityState::SetDelegate(TransportSecurityState::Delegate* delegate)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    delegate_ = delegate;
}
...
void TransportSecurityState::UpdatePinList(const std::vector<PinSet>& pinsets, const std::vector<PinSetInfo>& host_pins, base::Time update_time)
{
    pinsets_ = pinsets;
    ...
    for (const auto& pin : host_pins) {
        ...
        host_pins_.value()[pin.hostname_] = std::pair(pinset_names_map[pin.pinset_name_], pin.include_subdomains_);
    }
}

void TransportSecurityState::AddHSTSInternal(
    std::string_view host, TransportSecurityState::STSState::UpgradeMode upgrade_mode, const base::Time& expiry, bool include_subdomains)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
void TransportSecurityState::AddHPKPInternal(
    std::string_view host, const base::Time& last_observed, const base::Time& expiry, bool include_subdomains, const HashValueVector& hashes)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    const std::vector<uint8_t> canonicalized_host = CanonicalizeHost(host);
    if (canonicalized_host.empty())
...
void TransportSecurityState::SetEnablePublicKeyPinningBypassForLocalTrustAnchors(bool value)
{
    enable_pkp_bypass_for_local_trust_anchors_ = value;
}

TransportSecurityState::PKPStatus TransportSecurityState::CheckPins(
    const HostPortPair& host_port_pair, bool is_issued_by_known_root, const TransportSecurityState::PKPState& pkp_state, const HashValueVector& hashes)
{
...
void TransportSecurityState::DeleteAllDynamicDataBetween(base::Time start_time, base::Time end_time, base::OnceClosure callback)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    while (sts_iterator != enabled_sts_hosts_.end()) {
        if (sts_iterator->second.last_observed >= start_time && sts_iterator->second.last_observed < end_time) {
            dirtied = true;
    ...
    while (pkp_iterator != enabled_pkp_hosts_.end()) {
        if (pkp_iterator->second.last_observed >= start_time && pkp_iterator->second.last_observed < end_time) {
            dirtied = true;
            enabled_pkp_hosts_.erase(pkp_iterator++);
            continue;
        }
...
bool TransportSecurityState::AddHSTSHeader(std::string_view host, std::string_view value)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
void TransportSecurityState::AddHSTS(std::string_view host, const base::Time& expiry, bool include_subdomains)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    AddHSTSInternal(host, STSState::MODE_FORCE_HTTPS, expiry, include_subdomains);
}

void TransportSecurityState::AddHPKP(std::string_view host, const base::Time& expiry, bool include_subdomains, const HashValueVector& hashes)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
TransportSecurityState::PKPStatus TransportSecurityState::CheckPublicKeyPinsImpl(
    const HostPortPair& host_port_pair, bool is_issued_by_known_root, const HashValueVector& hashes)
{
    PKPState pkp_state;
...
bool TransportSecurityState::GetStaticSTSState(const std::string& host, STSState* sts_result) const
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    if (DecodeHSTSPreload(host, &result) && hsts_host_bypass_list_.find(host) == hsts_host_bypass_list_.end() && result.force_https) {
...
bool TransportSecurityState::GetStaticPKPState(const std::string& host, PKPState* pkp_result) const
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);

    if (!enable_static_pins_ || !IsStaticPKPListTimely() || !base::FeatureList::IsEnabled(features::kStaticKeyPinningEnforcement)) {
        return false;
    }
    ...
    if (host_pins_.has_value()) {
        ...
        while (true) {
            ...
            if (iter != host_pins_->end() && (iter->second.second || search_hostname == normalized_host)) {
                ...
                for (const auto& hash : pinset->static_spki_hashes()) {
                    ...
                    // crash.
                    if (hash.size() == 32) {
                        AddHash(reinterpret_cast<const char*>(hash.data()), &pkp_result->spki_hashes);
                    }
                }
                for (const auto& hash : pinset->bad_static_spki_hashes()) {
                    ...
                    // crash.
                    if (hash.size() == 32) {
                        AddHash(reinterpret_cast<const char*>(hash.data()), &pkp_result->bad_spki_hashes);
                    }
                }
                return true;
            }
        ...
        const TransportSecurityStateSource::Pinset* pinset = &g_hsts_source->pinsets[result.pinset_id];
...
bool TransportSecurityState::GetSTSState(const std::string& host, STSState* result)
{
    return GetDynamicSTSState(host, result) || GetStaticSTSState(host, result);
}

bool TransportSecurityState::GetPKPState(const std::string& host, PKPState* result)
{
    return GetDynamicPKPState(host, result) || GetStaticPKPState(host, result);
}

bool TransportSecurityState::GetDynamicSTSState(const std::string& host, STSState* result)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    for (size_t i = 0; canonicalized_host[i]; i += canonicalized_host[i] + 1) {
        base::span<const uint8_t> host_sub_chunk = base::make_span(canonicalized_host).subspan(i);
        ...
        if (i == 0 || j->second.include_subdomains) {
            std::optional<std::string> dotted_name = dns_names_util::NetworkToDottedName(host_sub_chunk);
            if (!dotted_name)
                return false;
...
bool TransportSecurityState::GetDynamicPKPState(const std::string& host, PKPState* result)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
void TransportSecurityState::AddOrUpdateEnabledSTSHosts(const HashedHost& hashed_host, const STSState& state)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
TransportSecurityState::STSStateIterator::STSStateIterator(const TransportSecurityState& state)
...
TransportSecurityState::PinSet::PinSet(
    std::string name, std::vector<std::vector<uint8_t>> static_spki_hashes, std::vector<std::vector<uint8_t>> bad_static_spki_hashes)
    : name_(std::move(name))
...
TransportSecurityState::PinSetInfo::PinSetInfo(std::string hostname, std::string pinset_name, bool include_subdomains)
...
bool TransportSecurityState::PKPState::CheckPublicKeyPins(const HashValueVector& hashes) const
{
...
bool TransportSecurityState::IsStaticPKPListTimely() const
{
    ...
    return (base::Time::Now() - key_pins_list_last_update_time_).InDays() < 70 /* 10 weeks */;
}
