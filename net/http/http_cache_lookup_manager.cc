...
// transaction.
base::Value NetLogPushLookupTransactionParams(const NetLogSource& net_log, const ServerPushDelegate::ServerPushHelper* push_helper)
{
...
HttpCacheLookupManager::LookupTransaction::LookupTransaction(std::unique_ptr<ServerPushHelper> server_push_helper, NetLog* net_log)
    : push_helper_(std::move(server_push_helper))
    , request_(std::make_unique<HttpRequestInfo>())
    , net_log_(NetLogWithSource::Make(net_log, NetLogSourceType::SERVER_PUSH_LOOKUP_TRANSACTION))
{
}

HttpCacheLookupManager::LookupTransaction::~LookupTransaction() = default;

int HttpCacheLookupManager::LookupTransaction::StartLookup(HttpCache* cache, CompletionOnceCallback callback, const NetLogWithSource& session_net_log)
{
    net_log_.BeginEvent(
        NetLogEventType::SERVER_PUSH_LOOKUP_TRANSACTION, [&] { return NetLogPushLookupTransactionParams(session_net_log.source(), push_helper_.get()); });
...
void HttpCacheLookupManager::LookupTransaction::OnLookupComplete(int result)
{
    ...
    net_log_.EndEventWithNetErrorCode(NetLogEventType::SERVER_PUSH_LOOKUP_TRANSACTION, result);
}
...
void HttpCacheLookupManager::OnPush(std::unique_ptr<ServerPushHelper> push_helper, const NetLogWithSource& session_net_log)
{
    ...
    auto lookup = std::make_unique<LookupTransaction>(std::move(push_helper), session_net_log.net_log());
    // TODO(zhongyi): add events in session net log to log the creation of
    // LookupTransaction.

    int rv
        = lookup->StartLookup(http_cache_, base::BindOnce(&HttpCacheLookupManager::OnLookupComplete, weak_factory_.GetWeakPtr(), pushed_url), session_net_log);
