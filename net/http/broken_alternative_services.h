...
typedef std::list<std::pair<BrokenAlternativeService, base::TimeTicks>> BrokenAlternativeServiceList;

// Stores how many times an alternative service has been marked broken.
class RecentlyBrokenAlternativeServices : public base::LRUCache<BrokenAlternativeService, int> {
public:
    explicit RecentlyBrokenAlternativeServices(int max_recently_broken_alternative_service_entries)
...
public:
    ...
    BrokenAlternativeServices(int max_recently_broken_alternative_service_entries, Delegate* delegate, const base::TickClock* clock);
    ...
    // state.
    void MarkBrokenUntilDefaultNetworkChanges(const BrokenAlternativeService& broken_alternative_service);
    ...
    void MarkRecentlyBroken(const BrokenAlternativeService& broken_alternative_service);

    // Returns true if the alternative service is considered broken.
    bool IsBroken(const BrokenAlternativeService& broken_alternative_service) const;
    ...
    bool IsBroken(const BrokenAlternativeService& broken_alternative_service, base::TimeTicks* brokenness_expiration) const;
    ...
    bool WasRecentlyBroken(const BrokenAlternativeService& broken_alternative_service);
    ...
    // with the new value.
    void SetBrokenAndRecentlyBrokenAlternativeServices(std::unique_ptr<BrokenAlternativeServiceList> broken_alternative_service_list,
    ...
    void SetDelayParams(std::optional<base::TimeDelta> initial_delay, std::optional<bool> exponential_backoff_on_initial_delay);

    const BrokenAlternativeServiceList& broken_alternative_service_list() const;

    const RecentlyBrokenAlternativeServices& recently_broken_alternative_services() const;
...
private:
    ...
    typedef std::map<BrokenAlternativeService, BrokenAlternativeServiceList::iterator> BrokenMap;
    ...
    // state.
    void MarkBrokenImpl(const BrokenAlternativeService& broken_alternative_service);
    ...
    std::set<BrokenAlternativeService> broken_alternative_services_on_default_network_;
