...
public:
    static void AddBrokenAlternativeServiceWithExpirationTime(HttpServerProperties* impl, const AlternativeService& alternative_service, base::TimeTicks when,
        const NetworkAnonymizationKey network_anonymization_key = NetworkAnonymizationKey())
    {
        BrokenAlternativeService broken_alternative_service(alternative_service, network_anonymization_key, true /* use_network_anonymization_key */);
        BrokenAlternativeServiceList::iterator unused_it;
        impl->broken_alternative_services_.AddToBrokenListAndMap(broken_alternative_service, when, &unused_it);
        auto it = impl->broken_alternative_services_.recently_broken_alternative_services_.Get(broken_alternative_service);
        if (it == impl->broken_alternative_services_.recently_broken_alternative_services_.end()) {
            impl->broken_alternative_services_.recently_broken_alternative_services_.Put(broken_alternative_service, 1);
        } else {
            it->second++;
        }
    }

    static void ExpireBrokenAlternateProtocolMappings(HttpServerProperties* impl)
    {
...
HttpServerProperties::ServerInfoMapKey CreateSimpleKey(const url::SchemeHostPort& server)
{
    return HttpServerProperties::ServerInfoMapKey(server, NetworkAnonymizationKey(), false /* use_network_anonymization_key */);
}
...
protected:
    HttpServerPropertiesTest()
        ...
        , impl_(nullptr /* pref_delegate */, nullptr /* net_log */, test_tick_clock_, &test_clock_)
    {
        // Set |test_clock_| to some random time.
        test_clock_.Advance(base::Seconds(12345));

        SchemefulSite site1(GURL("https://foo.test/"));
        network_anonymization_key1_ = NetworkAnonymizationKey::CreateSameSite(site1);
        SchemefulSite site2(GURL("https://bar.test/"));
        network_anonymization_key2_ = NetworkAnonymizationKey::CreateSameSite(site2);
    }

    // This is a little awkward, but need to create and configure the
    // ScopedFeatureList before creating the HttpServerProperties.
    static std::unique_ptr<base::test::ScopedFeatureList> CreateFeatureListWithPartitioningDisabled()
    {
        std::unique_ptr<base::test::ScopedFeatureList> feature_list = std::make_unique<base::test::ScopedFeatureList>();
        feature_list->InitAndDisableFeature(features::kPartitionConnectionsByNetworkIsolationKey);
        return feature_list;
    }

    bool HasAlternativeService(const url::SchemeHostPort& origin, const NetworkAnonymizationKey& network_anonymization_key)
    {
        const AlternativeServiceInfoVector alternative_service_info_vector = impl_.GetAlternativeServiceInfos(origin, network_anonymization_key);
        return !alternative_service_info_vector.empty();
    }

    void SetAlternativeService(const url::SchemeHostPort& origin, const AlternativeService& alternative_service)
    {
        const base::Time expiration = test_clock_.Now() + base::Days(1);
        if (alternative_service.protocol == kProtoQUIC) {
            impl_.SetQuicAlternativeService(origin, NetworkAnonymizationKey(), alternative_service, expiration, DefaultSupportedQuicVersions());
        } else {
            impl_.SetHttp2AlternativeService(origin, NetworkAnonymizationKey(), alternative_service, expiration);
        }
    }

    void MarkBrokenAndLetExpireAlternativeServiceNTimes(const AlternativeService& alternative_service, int num_times)
    {
    }
...
TEST_F(HttpServerPropertiesTest, SetSupportsSpdy)
{
    ...
    EXPECT_TRUE(impl_.GetSupportsSpdy(https_www_server, NetworkAnonymizationKey()));
    EXPECT_TRUE(impl_.SupportsRequestPriority(https_www_server, NetworkAnonymizationKey()));
    EXPECT_TRUE(impl_.GetSupportsSpdy(http_photo_server, NetworkAnonymizationKey()));
    EXPECT_TRUE(impl_.SupportsRequestPriority(http_photo_server, NetworkAnonymizationKey()));
    EXPECT_FALSE(impl_.GetSupportsSpdy(https_mail_server, NetworkAnonymizationKey()));
    EXPECT_FALSE(impl_.SupportsRequestPriority(https_mail_server, NetworkAnonymizationKey()));
    EXPECT_FALSE(impl_.GetSupportsSpdy(http_google_server, NetworkAnonymizationKey()));
    EXPECT_FALSE(impl_.SupportsRequestPriority(http_google_server, NetworkAnonymizationKey()));
    EXPECT_FALSE(impl_.GetSupportsSpdy(https_photos_server, NetworkAnonymizationKey()));
    EXPECT_FALSE(impl_.SupportsRequestPriority(https_photos_server, NetworkAnonymizationKey()));
    EXPECT_TRUE(impl_.GetSupportsSpdy(valid_google_server, NetworkAnonymizationKey()));
    EXPECT_TRUE(impl_.SupportsRequestPriority(valid_google_server, NetworkAnonymizationKey()));
    ...
    EXPECT_FALSE(impl_.GetSupportsSpdy(https_www_server, NetworkAnonymizationKey()));
    EXPECT_FALSE(impl_.SupportsRequestPriority(https_www_server, NetworkAnonymizationKey()));
    EXPECT_TRUE(impl_.GetSupportsSpdy(https_mail_server, NetworkAnonymizationKey()));
    EXPECT_TRUE(impl_.SupportsRequestPriority(https_mail_server, NetworkAnonymizationKey()));
}
...
TEST_F(HttpServerPropertiesTest, SetSupportsSpdyWithNetworkIsolationKey)
{
    ...
    EXPECT_FALSE(impl_.SupportsRequestPriority(kServer, network_anonymization_key1_));
    EXPECT_FALSE(impl_.GetSupportsSpdy(kServer, NetworkAnonymizationKey()));
    EXPECT_FALSE(impl_.SupportsRequestPriority(kServer, NetworkAnonymizationKey()));
    ...
    for (const auto& network_anonymization_key_to_set : { NetworkAnonymizationKey(), network_anonymization_key1_ }) {
        ...
        EXPECT_TRUE(impl_.SupportsRequestPriority(kServer, network_anonymization_key1_));
        EXPECT_TRUE(impl_.GetSupportsSpdy(kServer, NetworkAnonymizationKey()));
        EXPECT_TRUE(impl_.SupportsRequestPriority(kServer, NetworkAnonymizationKey()));
    ...
    // one.
    HttpServerProperties properties(nullptr /* pref_delegate */, nullptr /* net_log */, test_tick_clock_, &test_clock_);

    EXPECT_FALSE(properties.GetSupportsSpdy(kServer, network_anonymization_key1_));
    EXPECT_FALSE(properties.SupportsRequestPriority(kServer, network_anonymization_key1_));
    EXPECT_FALSE(properties.GetSupportsSpdy(kServer, NetworkAnonymizationKey()));
    EXPECT_FALSE(properties.SupportsRequestPriority(kServer, NetworkAnonymizationKey()));
    ...
    EXPECT_TRUE(properties.SupportsRequestPriority(kServer, network_anonymization_key1_));
    EXPECT_FALSE(properties.GetSupportsSpdy(kServer, NetworkAnonymizationKey()));
    EXPECT_FALSE(properties.SupportsRequestPriority(kServer, NetworkAnonymizationKey()));
    ...
    EXPECT_TRUE(properties.SupportsRequestPriority(kServer, NetworkAnonymizationKey()));
...
TEST_F(HttpServerPropertiesTest, LoadSupportsSpdy)
{
    ...
    std::unique_ptr<HttpServerProperties::ServerInfoMap> spdy_servers = std::make_unique<HttpServerProperties::ServerInfoMap>();
    impl_.OnServerInfoLoadedForTesting(std::move(spdy_servers));
    EXPECT_FALSE(impl_.GetSupportsSpdy(spdy_server_google, NetworkAnonymizationKey()));

    // Check by initializing www.google.com:443 and photos.google.com:443 as spdy
    // servers.
    std::unique_ptr<HttpServerProperties::ServerInfoMap> spdy_servers1 = std::make_unique<HttpServerProperties::ServerInfoMap>();
    ...
    EXPECT_TRUE(impl_.GetSupportsSpdy(spdy_server_google, NetworkAnonymizationKey()));
    EXPECT_FALSE(impl_.GetSupportsSpdy(spdy_server_photos, NetworkAnonymizationKey()));
    ...
    std::unique_ptr<HttpServerProperties::ServerInfoMap> spdy_servers2 = std::make_unique<HttpServerProperties::ServerInfoMap>();
    ...
    EXPECT_TRUE(impl_.GetSupportsSpdy(spdy_server_mail, NetworkAnonymizationKey()));
    EXPECT_TRUE(impl_.GetSupportsSpdy(spdy_server_docs, NetworkAnonymizationKey()));
    EXPECT_TRUE(impl_.GetSupportsSpdy(spdy_server_google, NetworkAnonymizationKey()));
    EXPECT_FALSE(impl_.GetSupportsSpdy(spdy_server_photos, NetworkAnonymizationKey()));
    ...
    std::unique_ptr<HttpServerProperties::ServerInfoMap> spdy_servers3 = std::make_unique<HttpServerProperties::ServerInfoMap>();
    ...
    EXPECT_FALSE(impl_.GetSupportsSpdy(spdy_server_mail, NetworkAnonymizationKey()));
    EXPECT_TRUE(impl_.GetSupportsSpdy(spdy_server_docs, NetworkAnonymizationKey()));
    EXPECT_TRUE(impl_.GetSupportsSpdy(spdy_server_google, NetworkAnonymizationKey()));
    EXPECT_TRUE(impl_.GetSupportsSpdy(spdy_server_photos, NetworkAnonymizationKey()));
}

TEST_F(HttpServerPropertiesTest, SupportsRequestPriority)
{
    url::SchemeHostPort spdy_server_empty("https", std::string(), 443);
    EXPECT_FALSE(impl_.SupportsRequestPriority(spdy_server_empty, NetworkAnonymizationKey()));

    // Add www.google.com:443 as supporting SPDY.
    url::SchemeHostPort spdy_server_google("https", "www.google.com", 443);
    impl_.SetSupportsSpdy(spdy_server_google, NetworkAnonymizationKey(), true);
    EXPECT_TRUE(impl_.SupportsRequestPriority(spdy_server_google, NetworkAnonymizationKey()));

    // Add mail.google.com:443 as not supporting SPDY.
    url::SchemeHostPort spdy_server_mail("https", "mail.google.com", 443);
    EXPECT_FALSE(impl_.SupportsRequestPriority(spdy_server_mail, NetworkAnonymizationKey()));

    // Add docs.google.com:443 as supporting SPDY.
    url::SchemeHostPort spdy_server_docs("https", "docs.google.com", 443);
    impl_.SetSupportsSpdy(spdy_server_docs, NetworkAnonymizationKey(), true);
    EXPECT_TRUE(impl_.SupportsRequestPriority(spdy_server_docs, NetworkAnonymizationKey()));

    // Add www.youtube.com:443 as supporting QUIC.
    url::SchemeHostPort youtube_server("https", "www.youtube.com", 443);
    const AlternativeService alternative_service1(kProtoQUIC, "www.youtube.com", 443);
    SetAlternativeService(youtube_server, alternative_service1);
    EXPECT_TRUE(impl_.SupportsRequestPriority(youtube_server, NetworkAnonymizationKey()));
...
TEST_F(AlternateProtocolServerPropertiesTest, Basic)
{
    ...
    EXPECT_EQ(alternative_service, alternative_service_info_vector[0].alternative_service());
...
TEST_F(AlternateProtocolServerPropertiesTest, ExcludeOrigin)
{
    ...
    AlternativeServiceInfo alternative_service_info1
        = AlternativeServiceInfo::CreateHttp2AlternativeServiceInfo(AlternativeService(kProtoHTTP2, "foo", 443), expiration);
    ...
    AlternativeServiceInfo alternative_service_info2
        = AlternativeServiceInfo::CreateHttp2AlternativeServiceInfo(AlternativeService(kProtoHTTP2, "bar", 443), expiration);
    ...
    AlternativeServiceInfo alternative_service_info3
        = AlternativeServiceInfo::CreateHttp2AlternativeServiceInfo(AlternativeService(kProtoHTTP2, "foo", 80), expiration);
    ...
    AlternativeServiceInfo alternative_service_info4
        = AlternativeServiceInfo::CreateQuicAlternativeServiceInfo(AlternativeService(kProtoQUIC, "foo", 443), expiration, DefaultSupportedQuicVersions());
    alternative_service_info_vector.push_back(alternative_service_info4);

    url::SchemeHostPort test_server("https", "foo", 443);
    impl_.SetAlternativeServices(test_server, NetworkAnonymizationKey(), alternative_service_info_vector);

    const AlternativeServiceInfoVector alternative_service_info_vector2 = impl_.GetAlternativeServiceInfos(test_server, NetworkAnonymizationKey());
...
TEST_F(AlternateProtocolServerPropertiesTest, Set)
{
    ...
    impl_.SetHttp2AlternativeService(test_server1, NetworkAnonymizationKey(), alternative_service1, expiration1);
    ...
    alternative_service_info_vector.push_back(AlternativeServiceInfo::CreateHttp2AlternativeServiceInfo(alternative_service2, expiration2));
    url::SchemeHostPort test_server2("http", "foo2", 80);
    // 0th entry in the memory.
    impl_.SetAlternativeServices(test_server2, NetworkAnonymizationKey(), alternative_service_info_vector);

    // Prepare |server_info_map| to be loaded by OnServerInfoLoadedForTesting().
    std::unique_ptr<HttpServerProperties::ServerInfoMap> server_info_map = std::make_unique<HttpServerProperties::ServerInfoMap>();
    const AlternativeService alternative_service3(kProtoHTTP2, "bar3", 123);
    base::Time expiration3 = now + base::Days(3);
    const AlternativeServiceInfo alternative_service_info1 = AlternativeServiceInfo::CreateHttp2AlternativeServiceInfo(alternative_service3, expiration3);
    // Simulate updating data for 0th entry with data from Preferences.
    server_info_map->GetOrPut(CreateSimpleKey(test_server2))->second.alternative_services = AlternativeServiceInfoVector(/*size=*/1, alternative_service_info1);
    ...
    const AlternativeServiceInfo alternative_service_info2 = AlternativeServiceInfo::CreateHttp2AlternativeServiceInfo(alternative_service4, expiration4);
    // Add an old entry from Preferences, this will be added to end of recency
    // list.
    server_info_map->GetOrPut(CreateSimpleKey(test_server3))->second.alternative_services = AlternativeServiceInfoVector(/*size=*/1, alternative_service_info2);
    ...
    const HttpServerProperties::ServerInfoMap& map = impl_.server_info_map_for_testing();
    ...
    const AlternativeServiceInfoVector* service_info = &map_it->second.alternative_services.value();
...
TEST_F(AlternateProtocolServerPropertiesTest, SetWebSockets)
{
    ...
    EXPECT_EQ(0u, impl_.GetAlternativeServiceInfos(https_server, NetworkAnonymizationKey()).size());
    EXPECT_EQ(0u, impl_.GetAlternativeServiceInfos(wss_server, NetworkAnonymizationKey()).size());
    EXPECT_EQ(0u, impl_.GetAlternativeServiceInfos(http_server, NetworkAnonymizationKey()).size());
    EXPECT_EQ(0u, impl_.GetAlternativeServiceInfos(ws_server, NetworkAnonymizationKey()).size());

    SetAlternativeService(wss_server, alternative_service);
    EXPECT_EQ(1u, impl_.GetAlternativeServiceInfos(https_server, NetworkAnonymizationKey()).size());
    EXPECT_EQ(1u, impl_.GetAlternativeServiceInfos(wss_server, NetworkAnonymizationKey()).size());
    EXPECT_EQ(0u, impl_.GetAlternativeServiceInfos(http_server, NetworkAnonymizationKey()).size());
    EXPECT_EQ(0u, impl_.GetAlternativeServiceInfos(ws_server, NetworkAnonymizationKey()).size());
    ...
    EXPECT_EQ(1u, impl_.GetAlternativeServiceInfos(https_server, NetworkAnonymizationKey()).size());
    EXPECT_EQ(1u, impl_.GetAlternativeServiceInfos(wss_server, NetworkAnonymizationKey()).size());
    EXPECT_EQ(1u, impl_.GetAlternativeServiceInfos(http_server, NetworkAnonymizationKey()).size());
    EXPECT_EQ(1u, impl_.GetAlternativeServiceInfos(ws_server, NetworkAnonymizationKey()).size());

    impl_.SetAlternativeServices(https_server, NetworkAnonymizationKey(), AlternativeServiceInfoVector());
    EXPECT_EQ(0u, impl_.GetAlternativeServiceInfos(https_server, NetworkAnonymizationKey()).size());
    EXPECT_EQ(0u, impl_.GetAlternativeServiceInfos(wss_server, NetworkAnonymizationKey()).size());
    EXPECT_EQ(1u, impl_.GetAlternativeServiceInfos(http_server, NetworkAnonymizationKey()).size());
    EXPECT_EQ(1u, impl_.GetAlternativeServiceInfos(ws_server, NetworkAnonymizationKey()).size());

    impl_.SetAlternativeServices(ws_server, NetworkAnonymizationKey(), AlternativeServiceInfoVector());
    EXPECT_EQ(0u, impl_.GetAlternativeServiceInfos(https_server, NetworkAnonymizationKey()).size());
    EXPECT_EQ(0u, impl_.GetAlternativeServiceInfos(wss_server, NetworkAnonymizationKey()).size());
    EXPECT_EQ(0u, impl_.GetAlternativeServiceInfos(http_server, NetworkAnonymizationKey()).size());
    EXPECT_EQ(0u, impl_.GetAlternativeServiceInfos(ws_server, NetworkAnonymizationKey()).size());
...
{
    const url::SchemeHostPort kServer("https", "foo.test", 443);
    const AlternativeServiceInfoVector kAlternativeServices({ AlternativeServiceInfo::CreateHttp2AlternativeServiceInfo(
        AlternativeService(kProtoHTTP2, "foo", 443), base::Time::Now() + base::Days(1) /* expiration */) });

    EXPECT_TRUE(impl_.GetAlternativeServiceInfos(kServer, network_anonymization_key1_).empty());
    EXPECT_TRUE(impl_.GetAlternativeServiceInfos(kServer, NetworkAnonymizationKey()).empty());

    // Without network isolation keys enabled for HttpServerProperties, passing in
    // a NetworkAnonymizationKey should have no effect on behavior.
    for (const auto& network_anonymization_key_to_set : { NetworkAnonymizationKey(), network_anonymization_key1_ }) {
        impl_.SetAlternativeServices(kServer, network_anonymization_key_to_set, kAlternativeServices);
        EXPECT_EQ(kAlternativeServices, impl_.GetAlternativeServiceInfos(kServer, network_anonymization_key1_));
        EXPECT_EQ(kAlternativeServices, impl_.GetAlternativeServiceInfos(kServer, NetworkAnonymizationKey()));

        impl_.SetAlternativeServices(kServer, network_anonymization_key_to_set, AlternativeServiceInfoVector());
        EXPECT_TRUE(impl_.GetAlternativeServiceInfos(kServer, network_anonymization_key1_).empty());
        EXPECT_TRUE(impl_.GetAlternativeServiceInfos(kServer, NetworkAnonymizationKey()).empty());
    }
    ...
    properties.SetAlternativeServices(kServer, network_anonymization_key1_, kAlternativeServices);
    EXPECT_EQ(kAlternativeServices, properties.GetAlternativeServiceInfos(kServer, network_anonymization_key1_));
    EXPECT_TRUE(properties.GetAlternativeServiceInfos(kServer, NetworkAnonymizationKey()).empty());

    properties.SetAlternativeServices(kServer, NetworkAnonymizationKey(), kAlternativeServices);
    EXPECT_EQ(kAlternativeServices, properties.GetAlternativeServiceInfos(kServer, network_anonymization_key1_));
    EXPECT_EQ(kAlternativeServices, properties.GetAlternativeServiceInfos(kServer, NetworkAnonymizationKey()));

    properties.SetAlternativeServices(kServer, network_anonymization_key1_, AlternativeServiceInfoVector());
    EXPECT_TRUE(properties.GetAlternativeServiceInfos(kServer, network_anonymization_key1_).empty());
    EXPECT_EQ(kAlternativeServices, properties.GetAlternativeServiceInfos(kServer, NetworkAnonymizationKey()));

    properties.SetAlternativeServices(kServer, NetworkAnonymizationKey(), AlternativeServiceInfoVector());
    EXPECT_TRUE(properties.GetAlternativeServiceInfos(kServer, network_anonymization_key1_).empty());
    EXPECT_TRUE(properties.GetAlternativeServiceInfos(kServer, NetworkAnonymizationKey()).empty());
}
...
TEST_F(AlternateProtocolServerPropertiesTest, SetWithEmptyHostname)
{
    ...
    const AlternativeService alternative_service_with_empty_hostname(kProtoHTTP2, "", 1234);
    const AlternativeService alternative_service_with_foo_hostname(kProtoHTTP2, "foo", 1234);
    SetAlternativeService(server, alternative_service_with_empty_hostname);
    impl_.MarkAlternativeServiceBroken(alternative_service_with_foo_hostname, NetworkAnonymizationKey());

    std::unique_ptr<HttpServerProperties::ServerInfoMap> server_info_map = std::make_unique<HttpServerProperties::ServerInfoMap>();
    impl_.OnServerInfoLoadedForTesting(std::move(server_info_map));

    EXPECT_TRUE(impl_.IsAlternativeServiceBroken(alternative_service_with_foo_hostname, NetworkAnonymizationKey()));
    const AlternativeServiceInfoVector alternative_service_info_vector = impl_.GetAlternativeServiceInfos(server, NetworkAnonymizationKey());
    ASSERT_EQ(1u, alternative_service_info_vector.size());
    EXPECT_EQ(alternative_service_with_foo_hostname, alternative_service_info_vector[0].alternative_service());
}
...
TEST_F(AlternateProtocolServerPropertiesTest, EmptyVector)
{
    ...
    base::Time expiration = test_clock_.Now() - base::Days(1);
    const AlternativeServiceInfo alternative_service_info = AlternativeServiceInfo::CreateHttp2AlternativeServiceInfo(alternative_service, expiration);
    std::unique_ptr<HttpServerProperties::ServerInfoMap> server_info_map = std::make_unique<HttpServerProperties::ServerInfoMap>();
    server_info_map->GetOrPut(CreateSimpleKey(server))->second.alternative_services = AlternativeServiceInfoVector(
    ...
    ASSERT_TRUE(impl_.GetAlternativeServiceInfos(server, NetworkAnonymizationKey()).empty());

    // GetAlternativeServiceInfos() should remove this key from
    // |server_info_map_|, and SetAlternativeServices() should not crash.
    impl_.SetAlternativeServices(server, NetworkAnonymizationKey(), AlternativeServiceInfoVector(/*size=*/1, alternative_service_info));
...
TEST_F(AlternateProtocolServerPropertiesTest, EmptyVectorForCanonical)
{
    ...
    const AlternativeServiceInfo alternative_service_info = AlternativeServiceInfo::CreateHttp2AlternativeServiceInfo(alternative_service, expiration);
    std::unique_ptr<HttpServerProperties::ServerInfoMap> server_info_map = std::make_unique<HttpServerProperties::ServerInfoMap>();
    server_info_map->GetOrPut(CreateSimpleKey(canonical_server))->second.alternative_services
    ...
    // GetAlternativeServiceInfos() should remove this key from
    // |server_info_map_|, and SetAlternativeServices() should not crash.
    impl_.SetAlternativeServices(canonical_server, NetworkAnonymizationKey(), AlternativeServiceInfoVector(/*size=*/1, alternative_service_info));

    // There should still be no alternative service assigned to
    // |canonical_server|.
    ASSERT_TRUE(impl_.GetAlternativeServiceInfos(canonical_server, NetworkAnonymizationKey()).empty());
}
...
TEST_F(AlternateProtocolServerPropertiesTest, ClearServerWithCanonical)
{
    ...
    const AlternativeServiceInfo alternative_service_info
        = AlternativeServiceInfo::CreateQuicAlternativeServiceInfo(alternative_service, expiration, DefaultSupportedQuicVersions());
    ...
    // |server_info_map_|, and SetAlternativeServices() should not crash.
    impl_.SetAlternativeServices(server, NetworkAnonymizationKey(), AlternativeServiceInfoVector());
...
TEST_F(AlternateProtocolServerPropertiesTest, MRUOfGetAlternativeServiceInfos)
{
    ...
    EXPECT_EQ(alternative_service2, it->second.alternative_services.value()[0].alternative_service());

    const AlternativeServiceInfoVector alternative_service_info_vector = impl_.GetAlternativeServiceInfos(test_server1, NetworkAnonymizationKey());
    ASSERT_EQ(1u, alternative_service_info_vector.size());
    EXPECT_EQ(alternative_service1, alternative_service_info_vector[0].alternative_service());
    ...
    ASSERT_TRUE(it->second.alternative_services.has_value());
    ASSERT_EQ(1u, it->second.alternative_services->size());
    EXPECT_EQ(alternative_service1, it->second.alternative_services.value()[0].alternative_service());
}
...
TEST_F(AlternateProtocolServerPropertiesTest, SetBroken)
{
    ...
    AlternativeServiceInfoVector alternative_service_info_vector = impl_.GetAlternativeServiceInfos(test_server, NetworkAnonymizationKey());
    ASSERT_EQ(1u, alternative_service_info_vector.size());
    EXPECT_EQ(alternative_service1, alternative_service_info_vector[0].alternative_service());
    EXPECT_FALSE(impl_.IsAlternativeServiceBroken(alternative_service1, NetworkAnonymizationKey()));

    // GetAlternativeServiceInfos should return the broken alternative service.
    impl_.MarkAlternativeServiceBroken(alternative_service1, NetworkAnonymizationKey());
    alternative_service_info_vector = impl_.GetAlternativeServiceInfos(test_server, NetworkAnonymizationKey());
    ASSERT_EQ(1u, alternative_service_info_vector.size());
    EXPECT_EQ(alternative_service1, alternative_service_info_vector[0].alternative_service());
    EXPECT_TRUE(impl_.IsAlternativeServiceBroken(alternative_service1, NetworkAnonymizationKey()));

    // SetAlternativeServices should add a broken alternative service to the map.
    AlternativeServiceInfoVector alternative_service_info_vector2;
    base::Time expiration = test_clock_.Now() + base::Days(1);
    alternative_service_info_vector2.push_back(AlternativeServiceInfo::CreateHttp2AlternativeServiceInfo(alternative_service1, expiration));
    const AlternativeService alternative_service2(kProtoHTTP2, "foo", 1234);
    alternative_service_info_vector2.push_back(AlternativeServiceInfo::CreateHttp2AlternativeServiceInfo(alternative_service2, expiration));
    impl_.SetAlternativeServices(test_server, NetworkAnonymizationKey(), alternative_service_info_vector2);
    alternative_service_info_vector = impl_.GetAlternativeServiceInfos(test_server, NetworkAnonymizationKey());
    ASSERT_EQ(2u, alternative_service_info_vector.size());
    EXPECT_EQ(alternative_service1, alternative_service_info_vector[0].alternative_service());
    EXPECT_EQ(alternative_service2, alternative_service_info_vector[1].alternative_service());
    EXPECT_TRUE(impl_.IsAlternativeServiceBroken(alternative_service1, NetworkAnonymizationKey()));
    EXPECT_FALSE(impl_.IsAlternativeServiceBroken(alternative_service2, NetworkAnonymizationKey()));
...
TEST_F(AlternateProtocolServerPropertiesTest, SetBrokenUntilDefaultNetworkChanges)
{
    url::SchemeHostPort test_server("http", "foo", 80);
    const AlternativeService alternative_service1(kProtoHTTP2, "foo", 443);
    SetAlternativeService(test_server, alternative_service1);
    ...
    impl_.MarkAlternativeServiceBrokenUntilDefaultNetworkChanges(alternative_service1, NetworkAnonymizationKey());
...
TEST_F(AlternateProtocolServerPropertiesTest, MaxAge)
{
    ...
    const AlternativeService alternative_service1(kProtoHTTP2, "foo", 443);
    alternative_service_info_vector.push_back(AlternativeServiceInfo::CreateHttp2AlternativeServiceInfo(alternative_service1, now - one_day));
    ...
    alternative_service_info_vector.push_back(AlternativeServiceInfo::CreateHttp2AlternativeServiceInfo(alternative_service2, now + one_day));

    url::SchemeHostPort test_server("http", "foo", 80);
    impl_.SetAlternativeServices(test_server, NetworkAnonymizationKey(), alternative_service_info_vector);

    AlternativeServiceInfoVector alternative_service_info_vector2 = impl_.GetAlternativeServiceInfos(test_server, NetworkAnonymizationKey());
    ASSERT_EQ(1u, alternative_service_info_vector2.size());
    EXPECT_EQ(alternative_service2, alternative_service_info_vector2[0].alternative_service());
}
...
TEST_F(AlternateProtocolServerPropertiesTest, MaxAgeCanonical)
{
    ...
    url::SchemeHostPort canonical_server("https", "bar.c.youtube.com", 443);
    impl_.SetAlternativeServices(canonical_server, NetworkAnonymizationKey(), alternative_service_info_vector);
...
TEST_F(AlternateProtocolServerPropertiesTest, AlternativeServiceWithScheme)
{
    ...
    const AlternativeService alternative_service1(kProtoHTTP2, "foo", 443);
    base::Time expiration = test_clock_.Now() + base::Days(1);
    alternative_service_info_vector.push_back(AlternativeServiceInfo::CreateHttp2AlternativeServiceInfo(alternative_service1, expiration));
    const AlternativeService alternative_service2(kProtoHTTP2, "bar", 1234);
    alternative_service_info_vector.push_back(AlternativeServiceInfo::CreateHttp2AlternativeServiceInfo(alternative_service2, expiration));
    // Set Alt-Svc list for |http_server|.
    url::SchemeHostPort http_server("http", "foo", 80);
    impl_.SetAlternativeServices(http_server, NetworkAnonymizationKey(), alternative_service_info_vector);
    ...
    EXPECT_EQ(alternative_service1, it->second.alternative_services.value()[0].alternative_service());
    EXPECT_EQ(alternative_service2, it->second.alternative_services.value()[1].alternative_service());
    ...
    impl_.SetAlternativeServices(https_server, NetworkAnonymizationKey(), alternative_service_info_vector);
    EXPECT_EQ(2u, impl_.GetAlternativeServiceInfos(https_server, NetworkAnonymizationKey()).size());
    EXPECT_EQ(2u, impl_.GetAlternativeServiceInfos(http_server, NetworkAnonymizationKey()).size());

    // Clear Alt-Svc list for |http_server|.
    impl_.SetAlternativeServices(http_server, NetworkAnonymizationKey(), AlternativeServiceInfoVector());
...
TEST_F(AlternateProtocolServerPropertiesTest, ClearAlternativeServices)
{
    ...
    impl_.SetAlternativeServices(test_server, NetworkAnonymizationKey(), AlternativeServiceInfoVector());
...
TEST_F(AlternateProtocolServerPropertiesTest, BrokenShadowsCanonical)
{
    url::SchemeHostPort test_server("https", "foo.c.youtube.com", 443);
    url::SchemeHostPort canonical_server("https", "bar.c.youtube.com", 443);
    AlternativeService canonical_alternative_service(kProtoQUIC, "bar.c.youtube.com", 1234);
    SetAlternativeService(canonical_server, canonical_alternative_service);
    AlternativeServiceInfoVector alternative_service_info_vector = impl_.GetAlternativeServiceInfos(test_server, NetworkAnonymizationKey());
    ASSERT_EQ(1u, alternative_service_info_vector.size());
    EXPECT_EQ(canonical_alternative_service, alternative_service_info_vector[0].alternative_service());

    const AlternativeService broken_alternative_service(kProtoHTTP2, "foo", 443);
    impl_.MarkAlternativeServiceBroken(broken_alternative_service, NetworkAnonymizationKey());
    EXPECT_TRUE(impl_.IsAlternativeServiceBroken(broken_alternative_service, NetworkAnonymizationKey()));

    SetAlternativeService(test_server, broken_alternative_service);
    alternative_service_info_vector = impl_.GetAlternativeServiceInfos(test_server, NetworkAnonymizationKey());
    ASSERT_EQ(1u, alternative_service_info_vector.size());
    EXPECT_EQ(broken_alternative_service, alternative_service_info_vector[0].alternative_service());
    EXPECT_TRUE(impl_.IsAlternativeServiceBroken(broken_alternative_service, NetworkAnonymizationKey()));
}
...
TEST_F(AlternateProtocolServerPropertiesTest, ClearBroken)
{
    ...
    SetAlternativeService(test_server, alternative_service);
    impl_.MarkAlternativeServiceBroken(alternative_service, NetworkAnonymizationKey());
    ASSERT_TRUE(HasAlternativeService(test_server, NetworkAnonymizationKey()));
    EXPECT_TRUE(impl_.IsAlternativeServiceBroken(alternative_service, NetworkAnonymizationKey()));
...
TEST_F(AlternateProtocolServerPropertiesTest, MarkBrokenWithNetworkIsolationKey)
{
    ...
    // should be ignored.
    impl_.SetHttp2AlternativeService(server, network_anonymization_key1_, alternative_service, expiration);
    EXPECT_FALSE(impl_.IsAlternativeServiceBroken(alternative_service, network_anonymization_key1_));
    EXPECT_FALSE(impl_.WasAlternativeServiceRecentlyBroken(alternative_service, network_anonymization_key1_));
    EXPECT_FALSE(impl_.IsAlternativeServiceBroken(alternative_service, network_anonymization_key2_));
    EXPECT_FALSE(impl_.WasAlternativeServiceRecentlyBroken(alternative_service, network_anonymization_key2_));

    impl_.MarkAlternativeServiceBroken(alternative_service, network_anonymization_key1_);
    EXPECT_TRUE(impl_.IsAlternativeServiceBroken(alternative_service, network_anonymization_key1_));
    EXPECT_TRUE(impl_.WasAlternativeServiceRecentlyBroken(alternative_service, network_anonymization_key1_));
    EXPECT_TRUE(impl_.IsAlternativeServiceBroken(alternative_service, network_anonymization_key2_));
    EXPECT_TRUE(impl_.WasAlternativeServiceRecentlyBroken(alternative_service, network_anonymization_key2_));

    impl_.ConfirmAlternativeService(alternative_service, network_anonymization_key2_);
    EXPECT_FALSE(impl_.IsAlternativeServiceBroken(alternative_service, network_anonymization_key1_));
    EXPECT_FALSE(impl_.WasAlternativeServiceRecentlyBroken(alternative_service, network_anonymization_key1_));
    EXPECT_FALSE(impl_.IsAlternativeServiceBroken(alternative_service, network_anonymization_key2_));
    EXPECT_FALSE(impl_.WasAlternativeServiceRecentlyBroken(alternative_service, network_anonymization_key2_));
    ...
    properties.SetHttp2AlternativeService(server, network_anonymization_key1_, alternative_service, expiration);
    ...
    EXPECT_FALSE(properties.WasAlternativeServiceRecentlyBroken(alternative_service, network_anonymization_key1_));
    EXPECT_FALSE(properties.IsAlternativeServiceBroken(alternative_service, network_anonymization_key2_));
    EXPECT_FALSE(properties.WasAlternativeServiceRecentlyBroken(alternative_service, network_anonymization_key2_));

    properties.MarkAlternativeServiceBroken(alternative_service, network_anonymization_key1_);
    EXPECT_TRUE(properties.IsAlternativeServiceBroken(alternative_service, network_anonymization_key1_));
    EXPECT_TRUE(properties.WasAlternativeServiceRecentlyBroken(alternative_service, network_anonymization_key1_));
    EXPECT_FALSE(properties.IsAlternativeServiceBroken(alternative_service, network_anonymization_key2_));
    EXPECT_FALSE(properties.WasAlternativeServiceRecentlyBroken(alternative_service, network_anonymization_key2_));

    properties.MarkAlternativeServiceBroken(alternative_service, network_anonymization_key2_);
    EXPECT_TRUE(properties.IsAlternativeServiceBroken(alternative_service, network_anonymization_key1_));
    EXPECT_TRUE(properties.WasAlternativeServiceRecentlyBroken(alternative_service, network_anonymization_key1_));
    EXPECT_TRUE(properties.IsAlternativeServiceBroken(alternative_service, network_anonymization_key2_));
    EXPECT_TRUE(properties.WasAlternativeServiceRecentlyBroken(alternative_service, network_anonymization_key2_));

    properties.ConfirmAlternativeService(alternative_service, network_anonymization_key1_);
    EXPECT_FALSE(properties.IsAlternativeServiceBroken(alternative_service, network_anonymization_key1_));
    EXPECT_FALSE(properties.WasAlternativeServiceRecentlyBroken(alternative_service, network_anonymization_key1_));
    EXPECT_TRUE(properties.IsAlternativeServiceBroken(alternative_service, network_anonymization_key2_));
    EXPECT_TRUE(properties.WasAlternativeServiceRecentlyBroken(alternative_service, network_anonymization_key2_));

    properties.ConfirmAlternativeService(alternative_service, network_anonymization_key2_);
    EXPECT_FALSE(properties.IsAlternativeServiceBroken(alternative_service, network_anonymization_key1_));
    EXPECT_FALSE(properties.WasAlternativeServiceRecentlyBroken(alternative_service, network_anonymization_key1_));
    EXPECT_FALSE(properties.IsAlternativeServiceBroken(alternative_service, network_anonymization_key2_));
    EXPECT_FALSE(properties.WasAlternativeServiceRecentlyBroken(alternative_service, network_anonymization_key2_));
...
TEST_F(AlternateProtocolServerPropertiesTest, MarkRecentlyBroken)
{
    ...
    EXPECT_FALSE(impl_.IsAlternativeServiceBroken(alternative_service, NetworkAnonymizationKey()));
    EXPECT_FALSE(impl_.WasAlternativeServiceRecentlyBroken(alternative_service, NetworkAnonymizationKey()));

    impl_.MarkAlternativeServiceRecentlyBroken(alternative_service, NetworkAnonymizationKey());
    EXPECT_FALSE(impl_.IsAlternativeServiceBroken(alternative_service, NetworkAnonymizationKey()));
    EXPECT_TRUE(impl_.WasAlternativeServiceRecentlyBroken(alternative_service, NetworkAnonymizationKey()));

    impl_.ConfirmAlternativeService(alternative_service, NetworkAnonymizationKey());
    EXPECT_FALSE(impl_.IsAlternativeServiceBroken(alternative_service, NetworkAnonymizationKey()));
    EXPECT_FALSE(impl_.WasAlternativeServiceRecentlyBroken(alternative_service, NetworkAnonymizationKey()));
}

TEST_F(AlternateProtocolServerPropertiesTest, MarkRecentlyBrokenWithNetworkIsolationKey)
{
    url::SchemeHostPort server("http", "foo", 80);
    const AlternativeService alternative_service(kProtoHTTP2, "foo", 443);
    const base::Time expiration = test_clock_.Now() + base::Days(1);
    ...
    impl_.MarkAlternativeServiceRecentlyBroken(alternative_service, network_anonymization_key1_);
    EXPECT_FALSE(impl_.IsAlternativeServiceBroken(alternative_service, network_anonymization_key1_));
    EXPECT_TRUE(impl_.WasAlternativeServiceRecentlyBroken(alternative_service, network_anonymization_key1_));
    EXPECT_FALSE(impl_.IsAlternativeServiceBroken(alternative_service, network_anonymization_key2_));
    EXPECT_TRUE(impl_.WasAlternativeServiceRecentlyBroken(alternative_service, network_anonymization_key2_));
    ...
    properties.MarkAlternativeServiceRecentlyBroken(alternative_service, network_anonymization_key1_);
    EXPECT_FALSE(properties.IsAlternativeServiceBroken(alternative_service, network_anonymization_key1_));
    EXPECT_TRUE(properties.WasAlternativeServiceRecentlyBroken(alternative_service, network_anonymization_key1_));
    EXPECT_FALSE(properties.IsAlternativeServiceBroken(alternative_service, network_anonymization_key2_));
    EXPECT_FALSE(properties.WasAlternativeServiceRecentlyBroken(alternative_service, network_anonymization_key2_));

    properties.MarkAlternativeServiceRecentlyBroken(alternative_service, network_anonymization_key2_);
    EXPECT_FALSE(properties.IsAlternativeServiceBroken(alternative_service, network_anonymization_key1_));
    EXPECT_TRUE(properties.WasAlternativeServiceRecentlyBroken(alternative_service, network_anonymization_key1_));
    EXPECT_FALSE(properties.IsAlternativeServiceBroken(alternative_service, network_anonymization_key2_));
    EXPECT_TRUE(properties.WasAlternativeServiceRecentlyBroken(alternative_service, network_anonymization_key2_));
...
TEST_F(AlternateProtocolServerPropertiesTest, MarkBrokenUntilDefaultNetworkChanges)
{
    url::SchemeHostPort server("http", "foo", 80);
    const AlternativeService alternative_service(kProtoHTTP2, "foo", 443);
    SetAlternativeService(server, alternative_service);
    ...
    impl_.MarkAlternativeServiceBrokenUntilDefaultNetworkChanges(alternative_service, NetworkAnonymizationKey());
    EXPECT_TRUE(impl_.IsAlternativeServiceBroken(alternative_service, NetworkAnonymizationKey()));
    EXPECT_TRUE(impl_.WasAlternativeServiceRecentlyBroken(alternative_service, NetworkAnonymizationKey()));
...
TEST_F(AlternateProtocolServerPropertiesTest, MarkBrokenUntilDefaultNetworkChangesWithNetworkIsolationKey)
{
    url::SchemeHostPort server("http", "foo", 80);
    const AlternativeService alternative_service(kProtoHTTP2, "foo", 443);
    const base::Time expiration = test_clock_.Now() + base::Days(1);
    ...
    impl_.MarkAlternativeServiceBrokenUntilDefaultNetworkChanges(alternative_service, network_anonymization_key1_);
    EXPECT_TRUE(impl_.IsAlternativeServiceBroken(alternative_service, network_anonymization_key1_));
    EXPECT_TRUE(impl_.WasAlternativeServiceRecentlyBroken(alternative_service, network_anonymization_key1_));
    EXPECT_TRUE(impl_.IsAlternativeServiceBroken(alternative_service, network_anonymization_key2_));
    EXPECT_TRUE(impl_.WasAlternativeServiceRecentlyBroken(alternative_service, network_anonymization_key2_));
    ...
    properties.MarkAlternativeServiceBrokenUntilDefaultNetworkChanges(alternative_service, network_anonymization_key1_);
    EXPECT_TRUE(properties.IsAlternativeServiceBroken(alternative_service, network_anonymization_key1_));
    EXPECT_TRUE(properties.WasAlternativeServiceRecentlyBroken(alternative_service, network_anonymization_key1_));
    EXPECT_FALSE(properties.IsAlternativeServiceBroken(alternative_service, network_anonymization_key2_));
    EXPECT_FALSE(properties.WasAlternativeServiceRecentlyBroken(alternative_service, network_anonymization_key2_));

    properties.MarkAlternativeServiceBrokenUntilDefaultNetworkChanges(alternative_service, network_anonymization_key2_);
    EXPECT_TRUE(properties.IsAlternativeServiceBroken(alternative_service, network_anonymization_key1_));
    EXPECT_TRUE(properties.WasAlternativeServiceRecentlyBroken(alternative_service, network_anonymization_key1_));
    EXPECT_TRUE(properties.IsAlternativeServiceBroken(alternative_service, network_anonymization_key2_));
    EXPECT_TRUE(properties.WasAlternativeServiceRecentlyBroken(alternative_service, network_anonymization_key2_));
...
TEST_F(AlternateProtocolServerPropertiesTest, OnDefaultNetworkChangedWithNetworkIsolationKey)
{
    url::SchemeHostPort server("http", "foo", 80);
    const AlternativeService alternative_service(kProtoHTTP2, "foo", 443);
...
TEST_F(AlternateProtocolServerPropertiesTest, Canonical)
{
    ...
    url::SchemeHostPort canonical_server("https", "bar.c.youtube.com", 443);
    EXPECT_FALSE(HasAlternativeService(canonical_server, NetworkAnonymizationKey()));

    AlternativeServiceInfoVector alternative_service_info_vector;
    const AlternativeService canonical_alternative_service1(kProtoQUIC, "bar.c.youtube.com", 1234);
    base::Time expiration = test_clock_.Now() + base::Days(1);
    alternative_service_info_vector.push_back(
        AlternativeServiceInfo::CreateQuicAlternativeServiceInfo(canonical_alternative_service1, expiration, DefaultSupportedQuicVersions()));
    const AlternativeService canonical_alternative_service2(kProtoHTTP2, "", 443);
    alternative_service_info_vector.push_back(AlternativeServiceInfo::CreateHttp2AlternativeServiceInfo(canonical_alternative_service2, expiration));
    impl_.SetAlternativeServices(canonical_server, NetworkAnonymizationKey(), alternative_service_info_vector);
    ...
    EXPECT_EQ(canonical_alternative_service1, alternative_service_info_vector2[0].alternative_service());
    ...
    // argument.
    EXPECT_EQ(test_server.host(), alternative_service_info_vector2[1].alternative_service().host);
    EXPECT_EQ(canonical_alternative_service2.protocol, alternative_service_info_vector2[1].alternative_service().protocol);
    EXPECT_EQ(canonical_alternative_service2.port, alternative_service_info_vector2[1].alternative_service().port);

    // Verify the canonical suffix.
    EXPECT_EQ(".c.youtube.com", *impl_.GetCanonicalSuffixForTesting(test_server.host()));
    EXPECT_EQ(".c.youtube.com", *impl_.GetCanonicalSuffixForTesting(canonical_server.host()));
}
...
TEST_F(AlternateProtocolServerPropertiesTest, ClearCanonical)
{
    ...
    SetAlternativeService(canonical_server, canonical_alternative_service);
    impl_.SetAlternativeServices(canonical_server, NetworkAnonymizationKey(), AlternativeServiceInfoVector());
    EXPECT_FALSE(HasAlternativeService(test_server, NetworkAnonymizationKey()));
}
...
TEST_F(AlternateProtocolServerPropertiesTest, CanonicalWithNetworkIsolationKey)
{
    ...
    EXPECT_FALSE(HasAlternativeService(canonical_server1, network_anonymization_key1_));
    ...
    const AlternativeService canonical_alternative_service2(kProtoHTTP2, "", 443);
    alternative_service_info_vector.push_back(AlternativeServiceInfo::CreateHttp2AlternativeServiceInfo(canonical_alternative_service2, expiration));
    properties.SetAlternativeServices(canonical_server1, network_anonymization_key1_, alternative_service_info_vector);

    // Since |test_server| does not have an alternative service itself,
    // GetAlternativeServiceInfos should return those of |canonical_server|.
    AlternativeServiceInfoVector alternative_service_info_vector2 = properties.GetAlternativeServiceInfos(test_server, network_anonymization_key1_);
    ASSERT_EQ(2u, alternative_service_info_vector2.size());
    EXPECT_EQ(canonical_alternative_service1, alternative_service_info_vector2[0].alternative_service());

    // Canonical information should not be visible for other NetworkIsolationKeys.
    EXPECT_TRUE(properties.GetAlternativeServiceInfos(test_server, network_anonymization_key2_).empty());
    EXPECT_TRUE(properties.GetAlternativeServiceInfos(test_server, NetworkAnonymizationKey()).empty());
    ...
    properties.SetAlternativeServices(canonical_server2, network_anonymization_key2_, { alternative_service_info_vector[0] });

    // The canonical server information should reachable, and different, for both
    // NetworkIsolationKeys.
    EXPECT_EQ(1u, properties.GetAlternativeServiceInfos(test_server, network_anonymization_key2_).size());
    EXPECT_EQ(2u, properties.GetAlternativeServiceInfos(test_server, network_anonymization_key1_).size());
    EXPECT_TRUE(properties.GetAlternativeServiceInfos(test_server, NetworkAnonymizationKey()).empty());
    ...
    properties.SetAlternativeServices(canonical_server1, network_anonymization_key1_, {});
    EXPECT_EQ(1u, properties.GetAlternativeServiceInfos(test_server, network_anonymization_key2_).size());
    EXPECT_TRUE(properties.GetAlternativeServiceInfos(test_server, network_anonymization_key1_).empty());
    EXPECT_TRUE(properties.GetAlternativeServiceInfos(test_server, NetworkAnonymizationKey()).empty());
}
...
TEST_F(AlternateProtocolServerPropertiesTest, CanonicalBroken)
{
    ...
    impl_.MarkAlternativeServiceBroken(canonical_alternative_service, NetworkAnonymizationKey());
    EXPECT_FALSE(HasAlternativeService(test_server, NetworkAnonymizationKey()));
}

TEST_F(AlternateProtocolServerPropertiesTest, CanonicalBrokenUntilDefaultNetworkChanges)
{
    url::SchemeHostPort test_server("https", "foo.c.youtube.com", 443);
    url::SchemeHostPort canonical_server("https", "bar.c.youtube.com", 443);
    AlternativeService canonical_alternative_service(kProtoQUIC, "bar.c.youtube.com", 1234);
    ...
    SetAlternativeService(canonical_server, canonical_alternative_service);
    EXPECT_TRUE(HasAlternativeService(test_server, NetworkAnonymizationKey()));
    impl_.MarkAlternativeServiceBrokenUntilDefaultNetworkChanges(canonical_alternative_service, NetworkAnonymizationKey());
    EXPECT_FALSE(HasAlternativeService(test_server, NetworkAnonymizationKey()));
}
...
TEST_F(AlternateProtocolServerPropertiesTest, CanonicalOverride)
{
    ...
    AlternativeService bar_alternative_service(kProtoQUIC, "bar.c.youtube.com", 1234);
    SetAlternativeService(bar_server, bar_alternative_service);
    AlternativeServiceInfoVector alternative_service_info_vector = impl_.GetAlternativeServiceInfos(foo_server, NetworkAnonymizationKey());
    ASSERT_EQ(1u, alternative_service_info_vector.size());
    EXPECT_EQ(bar_alternative_service, alternative_service_info_vector[0].alternative_service());

    url::SchemeHostPort qux_server("https", "qux.c.youtube.com", 443);
    AlternativeService qux_alternative_service(kProtoQUIC, "qux.c.youtube.com", 443);
    SetAlternativeService(qux_server, qux_alternative_service);
    alternative_service_info_vector = impl_.GetAlternativeServiceInfos(foo_server, NetworkAnonymizationKey());
    ASSERT_EQ(1u, alternative_service_info_vector.size());
    EXPECT_EQ(qux_alternative_service, alternative_service_info_vector[0].alternative_service());
}
...
TEST_F(AlternateProtocolServerPropertiesTest, ExpireBrokenAlternateProtocolMappings)
{
    ...
    HttpServerPropertiesPeer::AddBrokenAlternativeServiceWithExpirationTime(&impl_, alternative_service, past);
    EXPECT_TRUE(impl_.IsAlternativeServiceBroken(alternative_service, NetworkAnonymizationKey()));
    EXPECT_TRUE(impl_.WasAlternativeServiceRecentlyBroken(alternative_service, NetworkAnonymizationKey()));
...
TEST_F(AlternateProtocolServerPropertiesTest, ExpireBrokenAlternateProtocolMappingsWithNetworkIsolationKey)
{
    url::SchemeHostPort server("https", "foo", 443);
    ...
    properties.SetHttp2AlternativeService(server, network_anonymization_key1_, alternative_service, alt_service_expiration);
    properties.SetHttp2AlternativeService(server, network_anonymization_key2_, alternative_service, alt_service_expiration);

    EXPECT_FALSE(properties.GetAlternativeServiceInfos(server, network_anonymization_key1_).empty());
    EXPECT_FALSE(properties.IsAlternativeServiceBroken(alternative_service, network_anonymization_key1_));
    EXPECT_FALSE(properties.WasAlternativeServiceRecentlyBroken(alternative_service, network_anonymization_key1_));
    EXPECT_FALSE(properties.GetAlternativeServiceInfos(server, network_anonymization_key2_).empty());
    EXPECT_FALSE(properties.IsAlternativeServiceBroken(alternative_service, network_anonymization_key2_));
    EXPECT_FALSE(properties.WasAlternativeServiceRecentlyBroken(alternative_service, network_anonymization_key2_));

    // Set broken alternative service with expiration date in the past for
    // |network_anonymization_key1_|.
    HttpServerPropertiesPeer::AddBrokenAlternativeServiceWithExpirationTime(&properties, alternative_service, past, network_anonymization_key1_);
    EXPECT_TRUE(properties.IsAlternativeServiceBroken(alternative_service, network_anonymization_key1_));
    EXPECT_TRUE(properties.WasAlternativeServiceRecentlyBroken(alternative_service, network_anonymization_key1_));
    EXPECT_FALSE(properties.IsAlternativeServiceBroken(alternative_service, network_anonymization_key2_));
    EXPECT_FALSE(properties.WasAlternativeServiceRecentlyBroken(alternative_service, network_anonymization_key2_));
    ...
    // |network_anonymization_key1_|.
    HttpServerPropertiesPeer::AddBrokenAlternativeServiceWithExpirationTime(&properties, alternative_service, future, network_anonymization_key2_);
    EXPECT_TRUE(properties.IsAlternativeServiceBroken(alternative_service, network_anonymization_key1_));
    EXPECT_TRUE(properties.WasAlternativeServiceRecentlyBroken(alternative_service, network_anonymization_key1_));
    EXPECT_TRUE(properties.IsAlternativeServiceBroken(alternative_service, network_anonymization_key2_));
    EXPECT_TRUE(properties.WasAlternativeServiceRecentlyBroken(alternative_service, network_anonymization_key2_));
    ...
    EXPECT_TRUE(properties.GetAlternativeServiceInfos(server, network_anonymization_key1_).empty());
    EXPECT_FALSE(properties.IsAlternativeServiceBroken(alternative_service, network_anonymization_key1_));
    EXPECT_TRUE(properties.WasAlternativeServiceRecentlyBroken(alternative_service, network_anonymization_key1_));
    EXPECT_FALSE(properties.GetAlternativeServiceInfos(server, network_anonymization_key2_).empty());
    EXPECT_TRUE(properties.IsAlternativeServiceBroken(alternative_service, network_anonymization_key2_));
...
TEST_F(AlternateProtocolServerPropertiesTest, RemoveExpiredBrokenAltSvc)
{
    ...
    base::TimeTicks past = test_tick_clock_->NowTicks() - base::Seconds(42);
    HttpServerPropertiesPeer::AddBrokenAlternativeServiceWithExpirationTime(&impl_, bar_alternative_service, past);
    ...
    EXPECT_TRUE(impl_.WasAlternativeServiceRecentlyBroken(bar_alternative_service, NetworkAnonymizationKey()));
    EXPECT_FALSE(impl_.WasAlternativeServiceRecentlyBroken(baz_alternative_service, NetworkAnonymizationKey()));
}

TEST_F(AlternateProtocolServerPropertiesTest, SetBrokenAlternativeServicesDelayParams1)
{
    url::SchemeHostPort server("https", "foo", 443);
    AlternativeService alternative_service(kProtoQUIC, "foo", 443);
    SetAlternativeService(server, alternative_service);
...
TEST_F(AlternateProtocolServerPropertiesTest, SetBrokenAlternativeServicesDelayParams2)
{
    url::SchemeHostPort server("https", "foo", 443);
    AlternativeService alternative_service(kProtoQUIC, "foo", 443);
    SetAlternativeService(server, alternative_service);
...
TEST_F(AlternateProtocolServerPropertiesTest, RemoveExpiredBrokenAltSvc2)
{
    ...
    impl_.MarkAlternativeServiceBroken(alternative_service1, NetworkAnonymizationKey());
    impl_.MarkAlternativeServiceBroken(alternative_service2, NetworkAnonymizationKey());

    EXPECT_TRUE(impl_.IsAlternativeServiceBroken(alternative_service2, NetworkAnonymizationKey()));
    ...
    FastForwardBy(BROKEN_ALT_SVC_EXPIRE_DELAYS[3] - BROKEN_ALT_SVC_EXPIRE_DELAYS[0]);
...
TEST_F(AlternateProtocolServerPropertiesTest, RemoveExpiredBrokenAltSvc3)
{
    ...
    base::TimeTicks past = test_tick_clock_->NowTicks() - base::Seconds(42);
    HttpServerPropertiesPeer::AddBrokenAlternativeServiceWithExpirationTime(&impl_, kAltService, past);
...
TEST_F(AlternateProtocolServerPropertiesTest, GetAlternativeServiceInfoAsValue)
{
    constexpr base::Time::Exploded kNowExploded = { .year = 2018, .month = 1, .day_of_week = 3, .day_of_month = 24, .hour = 15, .minute = 12, .second = 53 };
    base::Time now;
    ...
    AlternativeServiceInfoVector alternative_service_info_vector;
    alternative_service_info_vector.push_back(
        AlternativeServiceInfo::CreateHttp2AlternativeServiceInfo(AlternativeService(kProtoHTTP2, "foo", 443), now + base::Minutes(1)));
    alternative_service_info_vector.push_back(AlternativeServiceInfo::CreateQuicAlternativeServiceInfo(
        AlternativeService(kProtoQUIC, "bar", 443), now + base::Hours(1), DefaultSupportedQuicVersions()));
    alternative_service_info_vector.push_back(AlternativeServiceInfo::CreateQuicAlternativeServiceInfo(
        AlternativeService(kProtoQUIC, "baz", 443), now + base::Hours(1), DefaultSupportedQuicVersions()));

    impl_.SetAlternativeServices(url::SchemeHostPort("https", "youtube.com", 443), NetworkAnonymizationKey(), alternative_service_info_vector);

    impl_.MarkAlternativeServiceBroken(AlternativeService(kProtoQUIC, "bar", 443), NetworkAnonymizationKey());

    impl_.MarkAlternativeServiceBrokenUntilDefaultNetworkChanges(AlternativeService(kProtoQUIC, "baz", 443), NetworkAnonymizationKey());

    alternative_service_info_vector.clear();
    alternative_service_info_vector.push_back(
        AlternativeServiceInfo::CreateHttp2AlternativeServiceInfo(AlternativeService(kProtoHTTP2, "foo2", 443), now + base::Days(1)));
    impl_.SetAlternativeServices(url::SchemeHostPort("http", "test.com", 80), NetworkAnonymizationKey(), alternative_service_info_vector);
    ...
    base::Value alternative_service_info_value = impl_.GetAlternativeServiceInfoAsValue();
    std::string alternative_service_info_json;
    base::JSONWriter::Write(alternative_service_info_value, &alternative_service_info_json);
...
TEST_F(HttpServerPropertiesTest, LoadServerNetworkStats)
{
    ...
    std::unique_ptr<HttpServerProperties::ServerInfoMap> load_server_info_map = std::make_unique<HttpServerProperties::ServerInfoMap>();
    impl_.OnServerInfoLoadedForTesting(std::move(load_server_info_map));
    const ServerNetworkStats* stats = impl_.GetServerNetworkStats(google_server, NetworkAnonymizationKey());
    ...
    load_server_info_map = std::make_unique<HttpServerProperties::ServerInfoMap>();
    load_server_info_map->GetOrPut(CreateSimpleKey(google_server))->second.server_network_stats = stats_google;
    impl_.OnServerInfoLoadedForTesting(std::move(load_server_info_map));

    // Verify data for www.google.com:443.
    ASSERT_EQ(1u, impl_.server_info_map_for_testing().size());
    EXPECT_EQ(stats_google, *(impl_.GetServerNetworkStats(google_server, NetworkAnonymizationKey())));
    ...
    impl_.SetServerNetworkStats(docs_server, NetworkAnonymizationKey(), stats_docs);
    ...
    new_stats_docs.bandwidth_estimate = quic::QuicBandwidth::FromBitsPerSecond(250);
    server_info_map->GetOrPut(CreateSimpleKey(docs_server))->second.server_network_stats = new_stats_docs;
    ...
    server_info_map->GetOrPut(CreateSimpleKey(mail_server))->second.server_network_stats = stats_mail;
...
TEST_F(HttpServerPropertiesTest, SetServerNetworkStats)
{
    ...
    EXPECT_EQ(nullptr, impl_.GetServerNetworkStats(foo_http_server, NetworkAnonymizationKey()));
    EXPECT_EQ(nullptr, impl_.GetServerNetworkStats(foo_https_server, NetworkAnonymizationKey()));

    ServerNetworkStats stats1;
    stats1.srtt = base::Microseconds(10);
    stats1.bandwidth_estimate = quic::QuicBandwidth::FromBitsPerSecond(100);
    impl_.SetServerNetworkStats(foo_http_server, NetworkAnonymizationKey(), stats1);

    const ServerNetworkStats* stats2 = impl_.GetServerNetworkStats(foo_http_server, NetworkAnonymizationKey());
...
TEST_F(HttpServerPropertiesTest, ClearServerNetworkStats)
{
    ...
    url::SchemeHostPort foo_https_server("https", "foo", 443);
    impl_.SetServerNetworkStats(foo_https_server, NetworkAnonymizationKey(), stats);
...
TEST_F(HttpServerPropertiesTest, OnQuicServerInfoMapLoaded)
{
    ...
    HttpServerProperties::QuicServerInfoMapKey google_key(
        google_quic_server_id, PRIVACY_MODE_ENABLED, NetworkAnonymizationKey(), false /* use_network_anonymization_key */);
    ...
    // Check empty map.
    std::unique_ptr<HttpServerProperties::QuicServerInfoMap> init_quic_server_info_map
        = std::make_unique<HttpServerProperties::QuicServerInfoMap>(kMaxQuicServerEntries);
    impl_.OnQuicServerInfoMapLoadedForTesting(std::move(init_quic_server_info_map));
    EXPECT_EQ(0u, impl_.quic_server_info_map().size());

    // Check by initializing with www.google.com:443.
    std::string google_server_info("google_quic_server_info");
    init_quic_server_info_map = std::make_unique<HttpServerProperties::QuicServerInfoMap>(kMaxQuicServerEntries);
    init_quic_server_info_map->Put(google_key, google_server_info);
    impl_.OnQuicServerInfoMapLoadedForTesting(std::move(init_quic_server_info_map));

    // Verify data for www.google.com:443.
    EXPECT_EQ(1u, impl_.quic_server_info_map().size());
    EXPECT_EQ(google_server_info, *impl_.GetQuicServerInfo(google_quic_server_id, PRIVACY_MODE_ENABLED, NetworkAnonymizationKey()));
    ...
    HttpServerProperties::QuicServerInfoMapKey docs_key(
        docs_quic_server_id, PRIVACY_MODE_ENABLED, NetworkAnonymizationKey(), false /* use_network_anonymization_key */);
    std::string docs_server_info("docs_quic_server_info");
    impl_.SetQuicServerInfo(docs_quic_server_id, PRIVACY_MODE_ENABLED, NetworkAnonymizationKey(), docs_server_info);

    // Recency order will be |docs_server| and |google_server|.
    const HttpServerProperties::QuicServerInfoMap& map = impl_.quic_server_info_map();
    ...
    HttpServerProperties::QuicServerInfoMapKey mail_key(
        mail_quic_server_id, PRIVACY_MODE_ENABLED, NetworkAnonymizationKey(), false /* use_network_anonymization_key */);
    ...
    // Recency order will be |docs_server|, |google_server| and |mail_server|.
    const HttpServerProperties::QuicServerInfoMap& memory_map = impl_.quic_server_info_map();
    ...
    const HttpServerProperties::QuicServerInfoMap& memory_map1 = impl_.quic_server_info_map();
    ...
    EXPECT_EQ(nullptr, impl_.GetQuicServerInfo(mail_quic_server_id, PRIVACY_MODE_ENABLED, NetworkAnonymizationKey()));
}
...
TEST_F(HttpServerPropertiesTest, SetQuicServerInfo)
{
    ...
    // Without network isolation keys enabled for HttpServerProperties, passing in
    // a NetworkAnonymizationKey should have no effect on behavior.
    impl_.SetQuicServerInfo(server1, PRIVACY_MODE_DISABLED, NetworkAnonymizationKey(), quic_server_info1);
    EXPECT_EQ(quic_server_info1, *(impl_.GetQuicServerInfo(server1, PRIVACY_MODE_DISABLED, NetworkAnonymizationKey())));
    EXPECT_FALSE(impl_.GetQuicServerInfo(server2, PRIVACY_MODE_ENABLED, NetworkAnonymizationKey()));
    EXPECT_EQ(quic_server_info1, *(impl_.GetQuicServerInfo(server1, PRIVACY_MODE_DISABLED, network_anonymization_key1_)));
    EXPECT_FALSE(impl_.GetQuicServerInfo(server2, PRIVACY_MODE_ENABLED, network_anonymization_key1_));

    impl_.SetQuicServerInfo(server2, PRIVACY_MODE_ENABLED, network_anonymization_key1_, quic_server_info2);
    EXPECT_EQ(quic_server_info1, *(impl_.GetQuicServerInfo(server1, PRIVACY_MODE_DISABLED, NetworkAnonymizationKey())));
    EXPECT_EQ(quic_server_info2, *(impl_.GetQuicServerInfo(server2, PRIVACY_MODE_ENABLED, NetworkAnonymizationKey())));
    EXPECT_EQ(quic_server_info1, *(impl_.GetQuicServerInfo(server1, PRIVACY_MODE_DISABLED, network_anonymization_key1_)));
    EXPECT_EQ(quic_server_info2, *(impl_.GetQuicServerInfo(server2, PRIVACY_MODE_ENABLED, network_anonymization_key1_)));

    impl_.SetQuicServerInfo(server1, PRIVACY_MODE_DISABLED, network_anonymization_key1_, quic_server_info3);
    EXPECT_EQ(quic_server_info3, *(impl_.GetQuicServerInfo(server1, PRIVACY_MODE_DISABLED, NetworkAnonymizationKey())));
    EXPECT_EQ(quic_server_info2, *(impl_.GetQuicServerInfo(server2, PRIVACY_MODE_ENABLED, NetworkAnonymizationKey())));
    EXPECT_EQ(quic_server_info3, *(impl_.GetQuicServerInfo(server1, PRIVACY_MODE_DISABLED, network_anonymization_key1_)));
    EXPECT_EQ(quic_server_info2, *(impl_.GetQuicServerInfo(server2, PRIVACY_MODE_ENABLED, network_anonymization_key1_)));

    impl_.Clear(base::OnceClosure());
    EXPECT_FALSE(impl_.GetQuicServerInfo(server1, PRIVACY_MODE_DISABLED, NetworkAnonymizationKey()));
    EXPECT_FALSE(impl_.GetQuicServerInfo(server2, PRIVACY_MODE_ENABLED, NetworkAnonymizationKey()));
    EXPECT_FALSE(impl_.GetQuicServerInfo(server1, PRIVACY_MODE_DISABLED, network_anonymization_key1_));
    EXPECT_FALSE(impl_.GetQuicServerInfo(server2, PRIVACY_MODE_ENABLED, network_anonymization_key1_));
    ...
    properties.SetQuicServerInfo(server1, PRIVACY_MODE_DISABLED, NetworkAnonymizationKey(), quic_server_info1);
    EXPECT_EQ(quic_server_info1, *(properties.GetQuicServerInfo(server1, PRIVACY_MODE_DISABLED, NetworkAnonymizationKey())));
    EXPECT_FALSE(properties.GetQuicServerInfo(server2, PRIVACY_MODE_ENABLED, NetworkAnonymizationKey()));
    EXPECT_FALSE(properties.GetQuicServerInfo(server1, PRIVACY_MODE_DISABLED, network_anonymization_key1_));
    EXPECT_FALSE(properties.GetQuicServerInfo(server2, PRIVACY_MODE_ENABLED, network_anonymization_key1_));

    properties.SetQuicServerInfo(server1, PRIVACY_MODE_DISABLED, network_anonymization_key1_, quic_server_info2);
    EXPECT_EQ(quic_server_info1, *(properties.GetQuicServerInfo(server1, PRIVACY_MODE_DISABLED, NetworkAnonymizationKey())));
    EXPECT_FALSE(properties.GetQuicServerInfo(server2, PRIVACY_MODE_ENABLED, NetworkAnonymizationKey()));
    EXPECT_EQ(quic_server_info2, *(properties.GetQuicServerInfo(server1, PRIVACY_MODE_DISABLED, network_anonymization_key1_)));
    EXPECT_FALSE(properties.GetQuicServerInfo(server2, PRIVACY_MODE_ENABLED, network_anonymization_key1_));

    properties.SetQuicServerInfo(server2, PRIVACY_MODE_ENABLED, network_anonymization_key1_, quic_server_info3);
    EXPECT_EQ(quic_server_info1, *(properties.GetQuicServerInfo(server1, PRIVACY_MODE_DISABLED, NetworkAnonymizationKey())));
    EXPECT_FALSE(properties.GetQuicServerInfo(server2, PRIVACY_MODE_ENABLED, NetworkAnonymizationKey()));
    EXPECT_EQ(quic_server_info2, *(properties.GetQuicServerInfo(server1, PRIVACY_MODE_DISABLED, network_anonymization_key1_)));
    EXPECT_EQ(quic_server_info3, *(properties.GetQuicServerInfo(server2, PRIVACY_MODE_ENABLED, network_anonymization_key1_)));

    properties.Clear(base::OnceClosure());
    EXPECT_FALSE(properties.GetQuicServerInfo(server1, PRIVACY_MODE_DISABLED, NetworkAnonymizationKey()));
    EXPECT_FALSE(properties.GetQuicServerInfo(server2, PRIVACY_MODE_ENABLED, NetworkAnonymizationKey()));
    EXPECT_FALSE(properties.GetQuicServerInfo(server1, PRIVACY_MODE_DISABLED, network_anonymization_key1_));
    EXPECT_FALSE(properties.GetQuicServerInfo(server2, PRIVACY_MODE_ENABLED, network_anonymization_key1_));
}
...
TEST_F(HttpServerPropertiesTest, QuicServerInfoCanonicalSuffixMatch)
{
    ...
    impl_.SetQuicServerInfo(foo_server_id, PRIVACY_MODE_DISABLED, NetworkAnonymizationKey(), foo_server_info);
    ...
    impl_.SetQuicServerInfo(baz_server_id, PRIVACY_MODE_DISABLED, NetworkAnonymizationKey(), baz_server_info);
    ...
    const std::string* bar_server_info = impl_.GetQuicServerInfo(bar_server_id, PRIVACY_MODE_DISABLED, NetworkAnonymizationKey());
...
TEST_F(HttpServerPropertiesTest, QuicServerInfoCanonicalSuffixMatchWithNetworkIsolationKey)
{
    ...
    properties.SetQuicServerInfo(server1, PRIVACY_MODE_DISABLED, network_anonymization_key1_, server_info1);
    const std::string* fetched_server_info = properties.GetQuicServerInfo(server1, PRIVACY_MODE_DISABLED, network_anonymization_key1_);
    ASSERT_TRUE(fetched_server_info);
    EXPECT_EQ(server_info1, *fetched_server_info);
    fetched_server_info = properties.GetQuicServerInfo(server2, PRIVACY_MODE_DISABLED, network_anonymization_key1_);
    ASSERT_TRUE(fetched_server_info);
    EXPECT_EQ(server_info1, *fetched_server_info);
    EXPECT_FALSE(properties.GetQuicServerInfo(server1, PRIVACY_MODE_DISABLED, network_anonymization_key2_));
    EXPECT_FALSE(properties.GetQuicServerInfo(server2, PRIVACY_MODE_DISABLED, network_anonymization_key2_));
    EXPECT_FALSE(properties.GetQuicServerInfo(server1, PRIVACY_MODE_DISABLED, NetworkAnonymizationKey()));
    EXPECT_FALSE(properties.GetQuicServerInfo(server2, PRIVACY_MODE_DISABLED, NetworkAnonymizationKey()));
    ...
    properties.SetQuicServerInfo(server1, PRIVACY_MODE_DISABLED, network_anonymization_key2_, server_info2);
    fetched_server_info = properties.GetQuicServerInfo(server1, PRIVACY_MODE_DISABLED, network_anonymization_key1_);
    ASSERT_TRUE(fetched_server_info);
    EXPECT_EQ(server_info1, *fetched_server_info);
    fetched_server_info = properties.GetQuicServerInfo(server2, PRIVACY_MODE_DISABLED, network_anonymization_key1_);
    ASSERT_TRUE(fetched_server_info);
    EXPECT_EQ(server_info1, *fetched_server_info);
    fetched_server_info = properties.GetQuicServerInfo(server1, PRIVACY_MODE_DISABLED, network_anonymization_key2_);
    ASSERT_TRUE(fetched_server_info);
    EXPECT_EQ(server_info2, *fetched_server_info);
    fetched_server_info = properties.GetQuicServerInfo(server2, PRIVACY_MODE_DISABLED, network_anonymization_key2_);
    ASSERT_TRUE(fetched_server_info);
    EXPECT_EQ(server_info2, *fetched_server_info);
    EXPECT_FALSE(properties.GetQuicServerInfo(server1, PRIVACY_MODE_DISABLED, NetworkAnonymizationKey()));
    EXPECT_FALSE(properties.GetQuicServerInfo(server2, PRIVACY_MODE_DISABLED, NetworkAnonymizationKey()));
    ...
    properties.Clear(base::OnceClosure());
    EXPECT_FALSE(properties.GetQuicServerInfo(server1, PRIVACY_MODE_DISABLED, network_anonymization_key1_));
    EXPECT_FALSE(properties.GetQuicServerInfo(server2, PRIVACY_MODE_DISABLED, network_anonymization_key1_));
    EXPECT_FALSE(properties.GetQuicServerInfo(server1, PRIVACY_MODE_DISABLED, network_anonymization_key2_));
    EXPECT_FALSE(properties.GetQuicServerInfo(server2, PRIVACY_MODE_DISABLED, network_anonymization_key2_));
    EXPECT_FALSE(properties.GetQuicServerInfo(server1, PRIVACY_MODE_DISABLED, NetworkAnonymizationKey()));
    EXPECT_FALSE(properties.GetQuicServerInfo(server2, PRIVACY_MODE_DISABLED, NetworkAnonymizationKey()));
...
TEST_F(HttpServerPropertiesTest, QuicServerInfoCanonicalSuffixMatchReturnsMruEntry)
{
    ...
    impl_.SetQuicServerInfo(h1_server_id, PRIVACY_MODE_DISABLED, NetworkAnonymizationKey(), h1_server_info);

    quic::QuicServerId h2_server_id("h2.googlevideo.com", 443);
    std::string h2_server_info("h2_server_info");
    impl_.SetQuicServerInfo(h2_server_id, PRIVACY_MODE_DISABLED, NetworkAnonymizationKey(), h2_server_info);

    // Create quic::QuicServerId to use for the search.
    quic::QuicServerId foo_server_id("foo.googlevideo.com", 443);

    // Check that 'h2' info is returned since it is MRU.
    const std::string* server_info = impl_.GetQuicServerInfo(foo_server_id, PRIVACY_MODE_DISABLED, NetworkAnonymizationKey());
    ASSERT_TRUE(server_info != nullptr);
    EXPECT_EQ(h2_server_info, *server_info);

    // Access 'h1' info, so it becomes MRU.
    impl_.GetQuicServerInfo(h1_server_id, PRIVACY_MODE_DISABLED, NetworkAnonymizationKey());

    // Check that 'h1' info is returned since it is MRU now.
    server_info = impl_.GetQuicServerInfo(foo_server_id, PRIVACY_MODE_DISABLED, NetworkAnonymizationKey());
    ASSERT_TRUE(server_info != nullptr);
...
TEST_F(HttpServerPropertiesTest, QuicServerInfoCanonicalSuffixMatchDoesntChangeOrder)
{
    // Add a host with a matching canonical name.
    quic::QuicServerId h1_server_id("h1.googlevideo.com", 443);
    HttpServerProperties::QuicServerInfoMapKey h1_key(
        h1_server_id, PRIVACY_MODE_DISABLED, NetworkAnonymizationKey(), false /* use_network_anonymization_key */);
    std::string h1_server_info("h1_server_info");
    impl_.SetQuicServerInfo(h1_server_id, PRIVACY_MODE_DISABLED, NetworkAnonymizationKey(), h1_server_info);
    ...
    HttpServerProperties::QuicServerInfoMapKey h2_key(
        h2_server_id, PRIVACY_MODE_DISABLED, NetworkAnonymizationKey(), false /* use_network_anonymization_key */);
    std::string h2_server_info("h2_server_info");
    impl_.SetQuicServerInfo(h2_server_id, PRIVACY_MODE_DISABLED, NetworkAnonymizationKey(), h2_server_info);
...
TEST_F(HttpServerPropertiesTest, QuicServerInfoCanonicalSuffixMatchSetInfoMap)
{
    ...
    HttpServerProperties::QuicServerInfoMapKey h3_key(
        h3_server_id, PRIVACY_MODE_DISABLED, NetworkAnonymizationKey(), false /* use_network_anonymization_key */);
