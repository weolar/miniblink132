...
class HttpStreamPool::AttemptManager : public HostResolver::ServiceEndpointRequest::Delegate {
public:
    ...
    void StartJob(Job* job, RequestPriority priority, const std::vector<SSLConfig::CertAndStatus>& allowed_bad_certs, RespectLimits respect_limits,
    ...
    int Preconnect(size_t num_streams, quic::ParsedQuicVersion quic_version, CompletionOnceCallback callback);
    ...
    size_t InFlightAttemptCount() const
    {
        return in_flight_attempts_.size();
    }
    ...
    std::optional<int> GetQuicTaskResultForTesting()
    {
        return quic_task_result_;
    }

    MultiplexedSessionCreationInitiator CalculateMultiplexedSessionCreationInitiator();
...
private:
    ...
    base::expected<SSLConfig, TlsStreamAttempt::GetSSLConfigError> GetSSLConfig(InFlightAttempt* attempt);
    ...
    // `max_attempts`.
    void MaybeAttemptConnection(std::optional<size_t> max_attempts = std::nullopt);
    ...
    std::optional<IPEndPoint> FindPreferredIPEndpoint(const std::vector<IPEndPoint>& ip_endpoints);
    ...
    void NotifyStreamReady(std::unique_ptr<HttpStream> stream, NextProto negotiated_protocol);
    ...
    void OnInFlightAttemptTcpHandshakeComplete(InFlightAttempt* raw_attempt, int rv);
    void OnInFlightAttemptSlow(InFlightAttempt* raw_attempt);

    void HandleAttemptFailure(std::unique_ptr<InFlightAttempt> in_flight_attempt, int rv);
    ...
    bool IsEndpointUsableForTcpBasedAttempt(const ServiceEndpoint& endpoint, bool svcb_optional);
    ...
    std::set<std::unique_ptr<PreconnectEntry>, base::UniquePtrComparator> preconnects_;

    std::unique_ptr<HostResolver::ServiceEndpointRequest> service_endpoint_request_;
    ...
    std::set<std::unique_ptr<InFlightAttempt>, base::UniquePtrComparator> in_flight_attempts_;
    ...
    TcpBasedAttemptState tcp_based_attempt_state_ = TcpBasedAttemptState::kNotStarted;
