...
HttpAuthFilterAllowlist::HttpAuthFilterAllowlist(const std::string& server_allowlist)
{
...
bool HttpAuthFilterAllowlist::AddFilter(const std::string& filter, HttpAuth::Target target)
{
...
bool HttpAuthFilterAllowlist::IsValid(const url::SchemeHostPort& scheme_host_port, HttpAuth::Target target) const
{
    if ((target != HttpAuth::AUTH_SERVER) && (target != HttpAuth::AUTH_PROXY))
        return false;
    // All proxies pass
...
void HttpAuthFilterAllowlist::SetAllowlist(const std::string& server_allowlist)
{
    ...
    rules_.ParseFromString(ProxyBypassRules::GetRulesToSubtractImplicit() + ";" + server_allowlist);
}
