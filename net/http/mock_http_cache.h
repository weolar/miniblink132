...
class MockDiskEntry : public disk_cache::Entry, public base::RefCounted<MockDiskEntry> {
public:
    enum DeferOp {
        DEFER_NONE,
        DEFER_CREATE,
    ...
    enum FailOp { FAIL_READ = 0x01, FAIL_WRITE = 0x02, FAIL_READ_SPARSE = 0x04, FAIL_WRITE_SPARSE = 0x08, FAIL_GET_AVAILABLE_RANGE = 0x10, FAIL_ALL = 0xFF };
    ...
    int ReadData(int index, int offset, IOBuffer* buf, int buf_len, CompletionOnceCallback callback) override;
    int WriteData(int index, int offset, IOBuffer* buf, int buf_len, CompletionOnceCallback callback, bool truncate) override;
    int ReadSparseData(int64_t offset, IOBuffer* buf, int buf_len, CompletionOnceCallback callback) override;
    int WriteSparseData(int64_t offset, IOBuffer* buf, int buf_len, CompletionOnceCallback callback) override;
    ...
    uint8_t in_memory_data() const
    {
        return in_memory_data_;
    }
    void set_in_memory_data(uint8_t val)
    {
        in_memory_data_ = val;
    }

    // Fail subsequent requests, specified via FailOp bits.
    void set_fail_requests(int mask)
    {
        fail_requests_ = mask;
    }

    void set_fail_sparse_requests()
    {
        fail_sparse_requests_ = true;
    }
    ...
    void SetDefer(DeferOp defer_op)
    {
        defer_op_ = defer_op;
    }
    ...
    void set_max_file_size(int val)
    {
        max_file_size_ = val;
    }
...
private:
    ...
    static void StoreAndDeliverCallbacks(bool store, MockDiskEntry* entry, base::OnceClosure callback);
...
public:
    ...
    EntryResult OpenOrCreateEntry(const std::string& key, RequestPriority request_priority, EntryResultCallback callback) override;
    EntryResult OpenEntry(const std::string& key, RequestPriority request_priority, EntryResultCallback callback) override;
    EntryResult CreateEntry(const std::string& key, RequestPriority request_priority, EntryResultCallback callback) override;
    Error DoomEntry(const std::string& key, RequestPriority request_priority, CompletionOnceCallback callback) override;
    Error DoomAllEntries(CompletionOnceCallback callback) override;
    Error DoomEntriesBetween(base::Time initial_time, base::Time end_time, CompletionOnceCallback callback) override;
    Error DoomEntriesSince(base::Time initial_time, CompletionOnceCallback callback) override;
    int64_t CalculateSizeOfAllEntries(Int64CompletionOnceCallback callback) override;
    ...
    void set_double_create_check(bool value)
    {
        double_create_check_ = value;
    }
    ...
    void set_max_file_size(int new_size)
    {
        max_file_size_ = new_size;
    }
    ...
    void SetDefer(MockDiskEntry::DeferOp defer_op)
    {
        defer_op_ = defer_op;
    }
...
private:
    using EntryMap = std::map<std::string, raw_ptr<MockDiskEntry, CtnExperimental>>;
...
public:
    disk_cache::BackendResult CreateBackend(NetLog* net_log, disk_cache::BackendResultCallback callback) override;
};

class MockHttpCache {
public:
    MockHttpCache();
    explicit MockHttpCache(std::unique_ptr<HttpCache::BackendFactory> disk_cache_factory);
    ...
    static bool ReadResponseInfo(disk_cache::Entry* disk_entry, HttpResponseInfo* response_info, bool* response_truncated);

    // Helper function for writing response info into the disk cache.
    static bool WriteResponseInfo(disk_cache::Entry* disk_entry, const HttpResponseInfo* response_info, bool skip_transient_headers, bool response_truncated);
    ...
    bool CreateBackendEntry(const std::string& key, disk_cache::Entry** entry, NetLog* net_log);
...
using TestGetBackendCompletionCallbackBase = net::internal::TestCompletionCallbackTemplate<HttpCache::GetBackendResult, GetBackendResultIsPendingHelper>;

class TestGetBackendCompletionCallback : public TestGetBackendCompletionCallbackBase {
public:
    TestGetBackendCompletionCallback() = default;

    HttpCache::GetBackendCallback callback()
    {
        return base::BindOnce(&TestGetBackendCompletionCallback::SetResult, base::Unretained(this));
    }
};
