...
HttpStreamRequest::HttpStreamRequest(Helper* helper, WebSocketHandshakeStreamBase::CreateHelper* websocket_handshake_stream_create_helper,
    const NetLogWithSource& net_log, StreamType stream_type)
    : helper_(helper)
    , websocket_handshake_stream_create_helper_(websocket_handshake_stream_create_helper)
    , net_log_(net_log)
...
void HttpStreamRequest::Complete(NextProto negotiated_protocol, AlternateProtocolUsage alternate_protocol_usage)
{
    DCHECK(!completed_);
    completed_ = true;
...
void HttpStreamRequest::AddConnectionAttempts(const ConnectionAttempts& attempts)
{
...
WebSocketHandshakeStreamBase::CreateHelper* HttpStreamRequest::websocket_handshake_stream_create_helper() const
{
    return websocket_handshake_stream_create_helper_;
}

void HttpStreamRequest::SetDnsResolutionTimeOverrides(base::TimeTicks dns_resolution_start_time_override, base::TimeTicks dns_resolution_end_time_override)
{
    CHECK(!dns_resolution_start_time_override.is_null());
    CHECK(!dns_resolution_end_time_override.is_null());
    if (dns_resolution_start_time_override_.is_null() || (dns_resolution_start_time_override < dns_resolution_start_time_override_)) {
        dns_resolution_start_time_override_ = dns_resolution_start_time_override;
    }
    if (dns_resolution_end_time_override_.is_null() || (dns_resolution_end_time_override < dns_resolution_end_time_override_)) {
