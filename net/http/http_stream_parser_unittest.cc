...
std::unique_ptr<StreamSocket> CreateConnectedSocket(SequencedSocketData* data)
{
    ...
    auto socket = std::make_unique<MockTCPClientSocket>(AddressList(), nullptr, data);
...
TEST(HttpStreamParser, DataReadErrorSynchronous)
{
    ...
    ReadErrorUploadDataStream upload_data_stream(ReadErrorUploadDataStream::FailureMode::SYNC);
    ...
    ASSERT_THAT(upload_data_stream.Init(TestCompletionCallback().callback(), NetLogWithSource()), IsOk());
    ...
    scoped_refptr<GrowableIOBuffer> read_buffer = base::MakeRefCounted<GrowableIOBuffer>();
    HttpStreamParser parser(
        stream_socket.get(), false /* is_reused */, GURL("http://localhost"), "POST", &upload_data_stream, read_buffer.get(), NetLogWithSource());
    ...
    TestCompletionCallback callback;
    int result = parser.SendRequest("POST / HTTP/1.1\r\n", headers, TRAFFIC_ANNOTATION_FOR_TESTS, &response, callback.callback());
...
TEST(HttpStreamParser, DataReadErrorAsynchronous)
{
    ...
    ReadErrorUploadDataStream upload_data_stream(ReadErrorUploadDataStream::FailureMode::ASYNC);
    ASSERT_THAT(upload_data_stream.Init(TestCompletionCallback().callback(), NetLogWithSource()), IsOk());
...
public:
    ...
    InitAsyncUploadDataStream& operator=(const InitAsyncUploadDataStream&) = delete;

private:
    void CompleteInit()
    {
        UploadDataStream::OnInitCompleted(OK);
    }

    int InitInternal(const NetLogWithSource& net_log) override
    {
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
            FROM_HERE, base::BindOnce(&InitAsyncUploadDataStream::CompleteInit, weak_factory_.GetWeakPtr()));
        return ERR_IO_PENDING;
    }
...
TEST(HttpStreamParser, InitAsynchronousUploadDataStream)
{
    ...
    int result1 = parser.SendRequest("POST / HTTP/1.1\r\n", headers, TRAFFIC_ANNOTATION_FOR_TESTS, &response, callback1.callback());
...
TEST(HttpStreamParser, EncodeChunk_EmptyPayload)
{
    ...
    const int num_bytes_written = HttpStreamParser::EncodeChunk(kPayload, base::as_writable_byte_span(output));
...
TEST(HttpStreamParser, ShouldMergeRequestHeadersAndBody_NoBody)
{
    ...
    ASSERT_FALSE(HttpStreamParser::ShouldMergeRequestHeadersAndBody("some header", nullptr));
}

TEST(HttpStreamParser, ShouldMergeRequestHeadersAndBody_EmptyBody)
{
    std::vector<std::unique_ptr<UploadElementReader>> element_readers;
    std::unique_ptr<UploadDataStream> body(std::make_unique<ElementsUploadDataStream>(std::move(element_readers), 0));
    ASSERT_THAT(body->Init(CompletionOnceCallback(), NetLogWithSource()), IsOk());
    // Shouldn't be merged if upload data is empty.
    ASSERT_FALSE(HttpStreamParser::ShouldMergeRequestHeadersAndBody("some header", body.get()));
}
...
TEST(HttpStreamParser, ShouldMergeRequestHeadersAndBody_ChunkedBody)
{
    ...
    ASSERT_THAT(body->Init(TestCompletionCallback().callback(), NetLogWithSource()), IsOk());
...
TEST(HttpStreamParser, ShouldMergeRequestHeadersAndBody_FileBody)
{
    ...
    ASSERT_TRUE(base::CreateTemporaryFileInDir(temp_dir.GetPath(), &temp_file_path));

    {
        std::vector<std::unique_ptr<UploadElementReader>> element_readers;

        element_readers.push_back(
            std::make_unique<UploadFileElementReader>(base::SingleThreadTaskRunner::GetCurrentDefault().get(), temp_file_path, 0, 0, base::Time()));

        std::unique_ptr<UploadDataStream> body(std::make_unique<ElementsUploadDataStream>(std::move(element_readers), 0));
        TestCompletionCallback callback;
        ASSERT_THAT(body->Init(callback.callback(), NetLogWithSource()), IsError(ERR_IO_PENDING));
...
TEST(HttpStreamParser, ShouldMergeRequestHeadersAndBody_SmallBodyInMemory)
{
    std::vector<std::unique_ptr<UploadElementReader>> element_readers;
    const std::string payload = "123";
    element_readers.push_back(std::make_unique<UploadBytesElementReader>(base::as_byte_span(payload)));

    std::unique_ptr<UploadDataStream> body(std::make_unique<ElementsUploadDataStream>(std::move(element_readers), 0));
    ASSERT_THAT(body->Init(CompletionOnceCallback(), NetLogWithSource()), IsOk());
    // Yes, should be merged if the in-memory body is small here.
    ASSERT_TRUE(HttpStreamParser::ShouldMergeRequestHeadersAndBody("some header", body.get()));
}
...
TEST(HttpStreamParser, SentBytesNoHeaders)
{
    ...
    scoped_refptr<GrowableIOBuffer> read_buffer = base::MakeRefCounted<GrowableIOBuffer>();
    HttpStreamParser parser(stream_socket.get(), false /* is_reused */, GURL("http://localhost"), "GET",
        /*upload_data_stream=*/nullptr, read_buffer.get(), NetLogWithSource());

    HttpResponseInfo response;
    TestCompletionCallback callback;
    EXPECT_EQ(OK, parser.SendRequest("GET / HTTP/1.1\r\n", HttpRequestHeaders(), TRAFFIC_ANNOTATION_FOR_TESTS, &response, callback.callback()));
...
TEST(HttpStreamParser, SentBytesWithErrorWritingHeaders)
{
    ...
    HttpResponseInfo response;
    TestCompletionCallback callback;
    EXPECT_EQ(ERR_CONNECTION_RESET, parser.SendRequest("GET / HTTP/1.1\r\n", headers, TRAFFIC_ANNOTATION_FOR_TESTS, &response, callback.callback()));
...
TEST(HttpStreamParser, SentBytesPost)
{
    ...
    std::vector<std::unique_ptr<UploadElementReader>> element_readers;
    element_readers.push_back(std::make_unique<UploadBytesElementReader>(base::byte_span_from_cstring("hello world!")));
...
TEST(HttpStreamParser, AsyncSingleChunkAndAsyncSocket)
{
    ...
    ASSERT_THAT(upload_stream.Init(TestCompletionCallback().callback(), NetLogWithSource()), IsOk());
    ...
    scoped_refptr<GrowableIOBuffer> read_buffer = base::MakeRefCounted<GrowableIOBuffer>();
    HttpStreamParser parser(
        stream_socket.get(), false /* is_reused */, GURL("http://localhost"), "POST", &upload_stream, read_buffer.get(), NetLogWithSource());
    ...
    ASSERT_EQ(
        ERR_IO_PENDING, parser.SendRequest("GET /one.html HTTP/1.1\r\n", request_headers, TRAFFIC_ANNOTATION_FOR_TESTS, &response_info, callback.callback()));
    ...
    ASSERT_THAT(parser.ReadResponseHeaders(callback.callback()), IsError(ERR_IO_PENDING));
    ASSERT_THAT(callback.WaitForResult(), IsOk());

    // Finally, attempt to read the response body.
    auto body_buffer = base::MakeRefCounted<IOBufferWithSize>(kBodySize);
    ASSERT_EQ(ERR_IO_PENDING, parser.ReadResponseBody(body_buffer.get(), kBodySize, callback.callback()));
...
TEST(HttpStreamParser, SyncSingleChunkAndAsyncSocket)
{
    ...
    scoped_refptr<GrowableIOBuffer> read_buffer = base::MakeRefCounted<GrowableIOBuffer>();
    HttpStreamParser parser(stream_socket.get(), false /* is_reused */, GURL("http://localhost"), "GET", &upload_stream, read_buffer.get(), NetLogWithSource());
...
TEST(HttpStreamParser, TruncatedHeaders)
{
    MockRead truncated_status_reads[] = {
        MockRead(SYNCHRONOUS, 1, "HTTP/1.1 20"), MockRead(SYNCHRONOUS, 0, 2), // EOF
    };
    ...
    MockRead truncated_in_header_reads[] = {
        MockRead(SYNCHRONOUS, 1, "HTTP/1.1 200 Ok\r\nHead"), MockRead(SYNCHRONOUS, 0, 2), // EOF
    };
    ...
    MockRead truncated_after_final_newline_reads[] = {
        MockRead(SYNCHRONOUS, 1, "HTTP/1.1 200 Ok\r\nHeader: foo\r\n\r"), MockRead(SYNCHRONOUS, 0, 2), // EOF
    };
    ...
    for (size_t protocol = 0; protocol < NUM_PROTOCOLS; protocol++) {
        ...
        for (size_t i = 0; i < std::size(reads); i++) {
            ...
            scoped_refptr<GrowableIOBuffer> read_buffer = base::MakeRefCounted<GrowableIOBuffer>();
            HttpStreamParser parser(
                stream_socket.get(), false /* is_reused */, url, "GET", /*upload_data_stream=*/nullptr, read_buffer.get(), NetLogWithSource());
...
TEST(HttpStreamParser, WebSocket101Response)
{
    ...
    EXPECT_EQ("a fake websocket frame", base::as_string_view(read_buffer->everything()));

    EXPECT_EQ(CountWriteBytes(writes), parser.sent_bytes());
    EXPECT_EQ(CountReadBytes(reads) - static_cast<int64_t>(strlen("a fake websocket frame")), parser.received_bytes());
}
...
public:
    ...
    {
        writes_.emplace_back(MockWrite(SYNCHRONOUS, sequence_number_++, "GET / HTTP/1.1\r\n\r\n"));
    }
    ...
    {
        reads_.emplace_back(SYNCHRONOUS, string_piece.data(), string_piece.length(), sequence_number_++);
    }

    void AddAsyncRead(std::string_view string_piece)
    {
        reads_.emplace_back(ASYNC, string_piece.data(), string_piece.length(), sequence_number_++);
    }
    ...
    void SetupParserAndSendRequest()
    {
        ...
        parser_ = std::make_unique<HttpStreamParser>(stream_socket_.get(), false /* is_reused */, url_, "GET",
            /*upload_data_stream=*/nullptr, read_buffer(), NetLogWithSource());

        TestCompletionCallback callback;
        ASSERT_EQ(OK, parser_->SendRequest("GET / HTTP/1.1\r\n", request_headers_, TRAFFIC_ANNOTATION_FOR_TESTS, &response_info_, callback.callback()));
    }

    void ReadHeadersExpectingError(Error error)
    {
        TestCompletionCallback callback;
        EXPECT_THAT(parser_->ReadResponseHeaders(callback.callback()), IsError(error));
    }

    void ReadHeaders()
    {
        ReadHeadersExpectingError(OK);
    }
    ...
    std::string ReadBody(int user_buf_len, int* read_lengths)
    {
        ...
        std::string body;
        while (true) {
            rv = parser_->ReadResponseBody(buffer.get(), user_buf_len, callback.callback());
...
TEST(HttpStreamParser, Http09PortTests)
{
    ...
    const TestCase kTestCases[] = {
        ...
        { "https://foo.com:8080/", ERR_INVALID_HTTP_RESPONSE, ERR_INVALID_HTTP_RESPONSE },
        { "http://foo.com:443/", ERR_INVALID_HTTP_RESPONSE, OK },
        { "https://foo.com:80/", ERR_INVALID_HTTP_RESPONSE, ERR_INVALID_HTTP_RESPONSE },
    };
    ...
    for (const auto& test_case : kTestCases) {
        ...
        EXPECT_EQ("HTTP/0.9 200 OK", get_runner.response_info()->headers->GetStatusLine());
        ...
        EXPECT_EQ(kResponse.size(), static_cast<size_t>(get_runner.parser()->received_bytes()));
        EXPECT_EQ(HttpConnectionInfo::kHTTP0_9, get_runner.response_info()->connection_info);
    }
    ...
    for (const auto& test_case : kTestCases) {
        ...
        get_runner.ReadHeadersExpectingError(test_case.expected_shoutcast_header_error);
        ...
        EXPECT_EQ(kShoutcastResponse.size(), static_cast<size_t>(get_runner.parser()->received_bytes()));
        EXPECT_EQ(HttpConnectionInfo::kHTTP0_9, get_runner.response_info()->connection_info);
    }
}
...
{
    const std::string kResponse = "HTTP/1.1 100 Continue\r\n\r\nhello\r\nworld\r\n";
    ...
    ASSERT_TRUE(get_runner.response_info()->headers);
    EXPECT_EQ("HTTP/1.1 100 Continue", get_runner.response_info()->headers->GetStatusLine());
...
TEST(HttpStreamParser, ReceivedBytesNormal)
{
    ...
    EXPECT_EQ(HttpConnectionInfo::kHTTP1_0, get_runner.response_info()->connection_info);
}
...
TEST(HttpStreamParser, ReceivedBytesExcludesNextResponse)
{
    ...
    EXPECT_EQ(HttpConnectionInfo::kHTTP1_1, get_runner.response_info()->connection_info);
}
...
TEST(HttpStreamParser, NonInformationalResponseStart)
{
    ...
    int64_t response_headers_size = response_headers1.size() + response_headers2.size();
    ...
    std::unique_ptr<StreamSocket> stream_socket = CreateConnectedSocket(&sequenced_socket_data);

    // Set up the http stream parser.
    auto read_buffer = base::MakeRefCounted<GrowableIOBuffer>();
    HttpStreamParser parser(stream_socket.get(), /*connection_is_reused=*/false, GURL("http://localhost"), "GET",
        /*upload_data_stream=*/nullptr, read_buffer.get(), NetLogWithSource());

    // Send a request.
    HttpResponseInfo response;
    TestCompletionCallback callback;
    EXPECT_THAT(parser.SendRequest("GET / HTTP/1.1\r\n", HttpRequestHeaders(), TRAFFIC_ANNOTATION_FOR_TESTS, &response, callback.callback()), IsOk());

    EXPECT_THAT(parser.ReadResponseHeaders(callback.callback()), IsError(ERR_IO_PENDING));
    ...
    EXPECT_EQ(parser.non_informational_response_start_time(), first_response_start_time);

    // [seq=4] The parser reads the response body.
    auto body_buffer = base::MakeRefCounted<IOBufferWithSize>(response_body.size());
    int result = parser.ReadResponseBody(body_buffer.get(), response_body.size(), callback.callback());
...
TEST(HttpStreamParser, NonInformationalResponseStartWithoutBody)
{
    ...
    int result = parser.ReadResponseBody(body_buffer.get(), body_buffer->size(), callback.callback());
...
TEST(HttpStreamParser, ReceivedBytesIncludesContinueHeader)
{
    ...
    int64_t status100_response_headers_size = status100_response_headers1.size() + status100_response_headers2.size();
    ...
    std::string response_body = "content";
    int64_t response_size = status100_response_headers_size + response_headers_size + response_body.size();
    ...
    // clock.
    base::TimeTicks non_informational_response_start_time = task_environment.NowTicks();
    sequenced_socket_data.Resume();
    sequenced_socket_data.RunUntilPaused();
    task_environment.AdvanceClock(base::Seconds(1));
    ...
    // Check the received headers.
    EXPECT_EQ(200, response.headers->response_code());
    EXPECT_EQ(status100_response_headers_size + response_headers_size, parser.received_bytes());
    ...
    EXPECT_EQ(parser.non_informational_response_start_time(), non_informational_response_start_time);
    // The first response start time should be earlier than the non-informational
    // response start time.
    EXPECT_LT(parser.first_response_start_time(), parser.non_informational_response_start_time());
...
TEST(HttpStreamParser, EarlyHints)
{
    ...
    std::string status103_response_headers2 = "Link: </style.css>; rel=preload; as=style\r\n";
    std::string status103_response_headers3 = "Link: </script.js>; rel=preload; as=script\r\n\r\n";
    int64_t status103_response_headers_size = status103_response_headers1.size() + status103_response_headers2.size() + status103_response_headers3.size();
    ...
    std::string response_body = "content";
    int64_t response_size = status103_response_headers_size + response_headers_size + response_body.size();
    ...
    // Check the received headers.
    EXPECT_EQ(200, response.headers->response_code());
    EXPECT_EQ(status103_response_headers_size + response_headers_size, parser.received_bytes());
...
TEST(HttpStreamParser, MixedResponseHeaders)
{
    ...
    int64_t status100_response_headers_size = status100_response_headers.size() + 12;
...
TEST(HttpStreamParser, ReadAfterUnownedObjectsDestroyed)
{
    ...
    MockRead reads[] = {
        ...
        MockRead(SYNCHRONOUS, 3, "Connection: Keep-Alive\r\n\r\n"), MockRead(SYNCHRONOUS, 4, "1"), MockRead(SYNCHRONOUS, 0, 5), // EOF
    };
    ...
    std::vector<std::unique_ptr<UploadElementReader>> element_readers;
    element_readers.push_back(std::make_unique<UploadBytesElementReader>(base::byte_span_from_cstring("123")));
    auto upload_data_stream = std::make_unique<ElementsUploadDataStream>(std::move(element_readers), 0);
    ASSERT_THAT(upload_data_stream->Init(TestCompletionCallback().callback(), NetLogWithSource()), IsOk());

    scoped_refptr<GrowableIOBuffer> read_buffer = base::MakeRefCounted<GrowableIOBuffer>();
    HttpStreamParser parser(
        stream_socket.get(), false /* is_reused */, GURL("http://somewhere/foo.html"), "POST", upload_data_stream.get(), read_buffer.get(), NetLogWithSource());
    ...
    TestCompletionCallback callback;
    ASSERT_EQ(OK, parser.SendRequest("GET /foo.html HTTP/1.1\r\n", *request_headers, TRAFFIC_ANNOTATION_FOR_TESTS, response_info.get(), callback.callback()));
...
{
    constexpr std::string_view kResponseHeaders = "HTTP/1.0 200 OK\r\n"
                                                  "Foo: Bar\r\n\r\n";
    ...
    get_runner.SetupParserAndSendRequest();
    get_runner.ReadHeaders();
    EXPECT_EQ(get_runner.response_info()->headers->GetNormalizedHeader("Foo"), "Bar");
    int read_lengths[] = { 1, 1, 0 };
    EXPECT_EQ(kResponseBody, get_runner.ReadBody(kResponseBody.size(), read_lengths));
}
