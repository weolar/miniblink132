...
public:
    ...
    virtual bool CanUseDefaultCredentials(const url::SchemeHostPort& auth_scheme_host_port) const;
    virtual HttpAuth::DelegationType GetDelegationType(const url::SchemeHostPort& auth_scheme_host_port) const;
    ...
    {
        return delegate_by_kdc_policy_;
    }
    ...
    {
        return basic_over_http_enabled_;
    }
    ...
    void set_allowed_schemes(const std::optional<std::set<std::string>>& allowed_schemes)
    {
        allowed_schemes_ = allowed_schemes;
    }

    void set_http_auth_scheme_filter(base::RepeatingCallback<bool(const url::SchemeHostPort&)>&& filter)
    {
...
private:
    ...
    base::RepeatingCallback<bool(const url::SchemeHostPort&)> http_auth_scheme_filter_ = base::RepeatingCallback<bool(const url::SchemeHostPort&)>();
};
