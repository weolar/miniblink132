...
// static
HttpAuthNtlmMechanism::GenerateRandomProc g_generate_random_proc = GenerateRandom;

// static
HttpAuthNtlmMechanism::HostNameProc g_host_name_proc = GetHostName;

template <typename T> T SwapOut(T* target, T source)
{
    T t = *target;
    *target = source;
    return t;
...
int SetAuthTokenFromBinaryToken(std::string* auth_token, const std::vector<uint8_t>& next_token)
{
    if (next_token.empty())
        return ERR_UNEXPECTED;

    std::string encode_output = base::Base64Encode(std::string_view(reinterpret_cast<const char*>(next_token.data()), next_token.size()));
...
HttpAuthNtlmMechanism::ScopedProcSetter::ScopedProcSetter(GetMSTimeProc ms_time_proc, GenerateRandomProc random_proc, HostNameProc host_name_proc)
{
...
HttpAuthNtlmMechanism::HttpAuthNtlmMechanism(const HttpAuthPreferences* http_auth_preferences)
    : ntlm_client_(ntlm::NtlmFeatures(http_auth_preferences ? http_auth_preferences->NtlmV2Enabled() : true))
{
}
...
HttpAuth::AuthorizationResult HttpAuthNtlmMechanism::ParseChallenge(HttpAuthChallengeTokenizer* tok)
{
    ...
    return ParseLaterRoundChallenge(HttpAuth::Scheme::AUTH_SCHEME_NTLM, tok, &encoded_token, &challenge_token_);
}

int HttpAuthNtlmMechanism::GenerateAuthToken(const AuthCredentials* credentials, const std::string& spn, const std::string& channel_bindings,
    std::string* auth_token, const NetLogWithSource& net_log, CompletionOnceCallback callback)
{
    if (!credentials) {
    ...
    if (challenge_token_.empty()) {
        ...
        return SetAuthTokenFromBinaryToken(auth_token, ntlm_client_.GetNegotiateMessage());
    }
    ...
    auto next_token = ntlm_client_.GenerateAuthenticateMessage(
        domain, user, credentials->password(), hostname, channel_bindings, spn, g_get_ms_time_proc(), client_challenge, base::as_byte_span(challenge_token_));

    return SetAuthTokenFromBinaryToken(auth_token, next_token);
}

void HttpAuthNtlmMechanism::SetDelegation(HttpAuth::DelegationType delegation_type)
{
    // Nothing to do.
}
