...
public:
    enum SocketPoolType { NORMAL_SOCKET_POOL, WEBSOCKET_SOCKET_POOL, NUM_SOCKET_POOL_TYPES };

    HttpNetworkSession(const HttpNetworkSessionParams& params, const HttpNetworkSessionContext& context);
    ~HttpNetworkSession();

    HttpAuthCache* http_auth_cache()
    {
        return &http_auth_cache_;
    }
    SSLClientContext* ssl_client_context()
    {
        return &ssl_client_context_;
    }
    ...
    // connections.
    ClientSocketPool* GetSocketPool(SocketPoolType pool_type, const ProxyChain& proxy_chain);
    ...
    HttpStreamPool* http_stream_pool()
    {
        return http_stream_pool_.get();
    }
    ...
    const HttpNetworkSessionParams& params() const
    {
        return params_;
    }
    // Returns the original Context used to construct this session.
    const HttpNetworkSessionContext& context() const
    {
        return context_;
    }

    // Returns protocols to be used with ALPN.
    const NextProtoVector& GetAlpnProtos() const
    {
        return next_protos_;
    }
    ...
    bool ShouldForceQuic(const url::SchemeHostPort& destination, const ProxyInfo& proxy_info, bool is_websocket);
    ...
    // will be nullptr.
    CommonConnectJobParams CreateCommonConnectJobParams(bool for_websockets = false);
...
private:
    ...
    std::set<std::unique_ptr<HttpResponseBodyDrainer>, base::UniquePtrComparator> response_drainers_;
