...
static_assert((HttpResponseBodyDrainer::kDrainBodyBufferSize % kMagicChunkSize) == 0, "chunk size needs to divide evenly into buffer size");
...
public:
    ...
    void set_stall_reads_forever()
    {
        stall_reads_forever_ = true;
    }
    ...
    void set_is_last_chunk_zero_size()
    {
        is_last_chunk_zero_size_ = true;
    }
...
int MockHttpStream::ReadResponseBody(IOBuffer* buf, int buf_len, CompletionOnceCallback callback)
{
    ...
    if (!is_sync_) {
        ...
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(&MockHttpStream::CompleteRead, weak_factory_.GetWeakPtr()));
        return ERR_IO_PENDING;
    } else {
...
protected:
    ...
    const raw_ptr<MockHttpStream, AcrossTasksDanglingUntriaged> mock_stream_; // Owned by |drainer_|.
...
{
    mock_stream_->set_num_chunks(HttpResponseBodyDrainer::kDrainBodyBufferSize / kMagicChunkSize);
    session_->StartResponseDrainer(std::move(drainer_));
    EXPECT_FALSE(result_waiter_.WaitForResult());
}
