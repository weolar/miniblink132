...
HttpAuthChallengeTokenizer::HttpAuthChallengeTokenizer(std::string_view challenge)
...
HttpUtil::NameValuePairsIterator HttpAuthChallengeTokenizer::param_pairs() const
{
...
std::string_view HttpAuthChallengeTokenizer::base64_param() const
{
    ...
    while (encoded_length > 0 && encoded_length % 4 != 0 && params_[encoded_length - 1] == '=') {
        --encoded_length;
    }
...
void HttpAuthChallengeTokenizer::Init(std::string_view challenge)
{
    ...
    lower_case_scheme_ = base::ToLowerASCII(base::MakeStringPiece(tok.token_begin(), tok.token_end()));

    params_ = HttpUtil::TrimLWS(std::string_view(tok.token_end(), challenge.end()));
}
