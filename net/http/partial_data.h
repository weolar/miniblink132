...
public:
    ...
    int ShouldValidateCache(disk_cache::Entry* entry, CompletionOnceCallback callback);

    // Builds the required |headers| to perform the proper cache validation for
    // the next range to be fetched.
    void PrepareCacheValidation(disk_cache::Entry* entry, HttpRequestHeaders* headers);
    ...
    bool UpdateFromStoredHeaders(const HttpResponseHeaders* headers, disk_cache::Entry* entry, bool truncated, bool writing_in_progress);
    ...
    // of the operation.
    int CacheRead(disk_cache::Entry* entry, IOBuffer* data, int data_len, CompletionOnceCallback callback);
    ...
    int CacheWrite(disk_cache::Entry* entry, IOBuffer* data, int data_len, CompletionOnceCallback callback);
    ...
    bool initial_validation() const
    {
        return initial_validation_;
    }

    bool range_requested() const
    {
        return range_requested_;
    }
