...
void HttpResponseBodyDrainer::Start(HttpNetworkSession* session)
{
    ...
    if (rv == ERR_IO_PENDING) {
        timer_.Start(FROM_HERE, base::Seconds(kTimeoutInSeconds), this, &HttpResponseBodyDrainer::OnTimerFired);
        return;
    }
...
int HttpResponseBodyDrainer::DoDrainResponseBody()
{
    ...
    return stream_->ReadResponseBody(
        read_buf_.get(), kDrainBodyBufferSize - total_read_, base::BindOnce(&HttpResponseBodyDrainer::OnIOComplete, base::Unretained(this)));
}
