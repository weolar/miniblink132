...
void HistogramAuthEvent(HttpAuthHandler* handler, AuthEvent auth_event)
{
    ...
    //   Negotiate Start: 6
    //   Negotiate Reject: 7
    static constexpr int kEventBucketsEnd = int { HttpAuth::AUTH_SCHEME_MAX } * AUTH_EVENT_MAX;
    int event_bucket = int { auth_scheme } * AUTH_EVENT_MAX + auth_event;
    DCHECK(event_bucket >= 0 && event_bucket < kEventBucketsEnd);
    UMA_HISTOGRAM_ENUMERATION("Net.HttpAuthCount", event_bucket, kEventBucketsEnd);
    ...
    static constexpr int kTargetBucketsEnd = int { HttpAuth::AUTH_SCHEME_MAX } * AUTH_TARGET_MAX;
    ...
    UMA_HISTOGRAM_ENUMERATION("Net.HttpAuthTarget", target_bucket, kTargetBucketsEnd);
}

base::Value::Dict ControllerParamsToValue(HttpAuth::Target target, const GURL& url)
{
...
HttpAuthController::HttpAuthController(HttpAuth::Target target, const GURL& auth_url, const NetworkAnonymizationKey& network_anonymization_key,
    HttpAuthCache* http_auth_cache, HttpAuthHandlerFactory* http_auth_handler_factory, HostResolver* host_resolver)
    : target_(target)
    , auth_url_(auth_url)
...
void HttpAuthController::BindToCallingNetLog(const NetLogWithSource& caller_net_log)
{
    if (!net_log_.source().IsValid()) {
        net_log_ = NetLogWithSource::Make(caller_net_log.net_log(), NetLogSourceType::HTTP_AUTH_CONTROLLER);
        net_log_.BeginEvent(NetLogEventType::AUTH_CONTROLLER, [&] { return ControllerParamsToValue(target_, auth_url_); });
    }
    caller_net_log.AddEventReferencingSource(NetLogEventType::AUTH_BOUND_TO_CONTROLLER, net_log_.source());
}

int HttpAuthController::MaybeGenerateAuthToken(const HttpRequestInfo* request, CompletionOnceCallback callback, const NetLogWithSource& caller_net_log)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    DCHECK(!auth_info_);
    ...
    if (!needs_auth)
        return OK;
    net_log_.BeginEventReferencingSource(NetLogEventType::AUTH_GENERATE_TOKEN, caller_net_log.source());
    ...
    int rv
        = handler_->GenerateAuthToken(credentials, request, base::BindOnce(&HttpAuthController::OnGenerateAuthTokenDone, base::Unretained(this)), &auth_token_);
...
bool HttpAuthController::SelectPreemptiveAuth(const NetLogWithSource& caller_net_log)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    DCHECK(!HaveAuth());
    ...
    HttpAuthCache::Entry* entry = http_auth_cache_->LookupByPath(auth_scheme_host_port_, target_, network_anonymization_key_, auth_path_);
    if (!entry)
        return false;
    ...
    int rv_create = http_auth_handler_factory_->CreatePreemptiveAuthHandlerFromString(entry->auth_challenge(), target_, network_anonymization_key_,
        auth_scheme_host_port_, entry->IncrementNonceCount(), net_log_, host_resolver_, &handler_preemptive);
    if (rv_create != OK)
        return false;
...
void HttpAuthController::AddAuthorizationHeader(HttpRequestHeaders* authorization_headers)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    DCHECK(HaveAuth());
    ...
    if (!auth_token_.empty()) {
        authorization_headers->SetHeader(HttpAuth::GetAuthorizationHeaderName(target_), auth_token_);
        auth_token_.clear();
    }
}

int HttpAuthController::HandleAuthChallenge(scoped_refptr<HttpResponseHeaders> headers, const SSLInfo& ssl_info, bool do_not_send_server_auth,
    bool establishing_tunnel, const NetLogWithSource& caller_net_log)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    DCHECK(headers.get());
    DCHECK(auth_scheme_host_port_.IsValid());
    ...
    BindToCallingNetLog(caller_net_log);
    net_log_.BeginEventReferencingSource(NetLogEventType::AUTH_HANDLE_CHALLENGE, caller_net_log.source());
    ...
    if (HaveAuth()) {
        ...
        HttpAuth::AuthorizationResult result = HttpAuth::HandleChallengeResponse(handler_.get(), *headers, target_, disabled_schemes_, &challenge_used);
        switch (result) {
        ...
        case HttpAuth::AUTHORIZATION_RESULT_STALE:
            if (http_auth_cache_->UpdateStaleChallenge(
                    auth_scheme_host_port_, target_, handler_->realm(), handler_->auth_scheme(), network_anonymization_key_, challenge_used)) {
        ...
        case HttpAuth::AUTHORIZATION_RESULT_DIFFERENT_REALM:
            ...
            InvalidateCurrentHandler((identity_.source == HttpAuth::IDENT_SRC_PATH_LOOKUP) ? INVALIDATE_HANDLER : INVALIDATE_HANDLER_AND_CACHED_CREDENTIALS);
            break;
        default:
            NOTREACHED();
        }
    ...
    do {
        if (!handler_.get() && can_send_auth) {
            ...
            HttpAuth::ChooseBestChallenge(http_auth_handler_factory_, *headers, ssl_info, network_anonymization_key_, target_, auth_scheme_host_port_,
        ...
        if (!handler_.get()) {
            if (establishing_tunnel) {
                ...
                net_log_.EndEventWithNetErrorCode(NetLogEventType::AUTH_HANDLE_CHALLENGE, ERR_PROXY_AUTH_UNSUPPORTED);
...
void HttpAuthController::ResetAuth(const AuthCredentials& credentials)
{
    ...
    case HttpAuth::IDENT_SRC_DEFAULT_CREDENTIALS:
        break;
    default:
        http_auth_cache_->Add(auth_scheme_host_port_, target_, handler_->realm(), handler_->auth_scheme(), network_anonymization_key_, handler_->challenge(),
...
void HttpAuthController::InvalidateCurrentHandler(InvalidateHandlerAction action)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    DCHECK(handler_.get());
...
void HttpAuthController::InvalidateRejectedAuthFromCache()
{
    ...
    http_auth_cache_->Remove(auth_scheme_host_port_, target_, handler_->realm(), handler_->auth_scheme(), network_anonymization_key_, identity_.credentials);
}
...
bool HttpAuthController::SelectNextAuthIdentityToTry()
{
    ...
    HttpAuthCache::Entry* entry
        = http_auth_cache_->Lookup(auth_scheme_host_port_, target_, handler_->realm(), handler_->auth_scheme(), network_anonymization_key_);
...
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    net_log_.EndEventWithNetErrorCode(NetLogEventType::AUTH_GENERATE_TOKEN, result);
    switch (result) {
