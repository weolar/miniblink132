...
base::Value::Dict NetLogSendRequestBodyParams(uint64_t length, bool is_chunked, bool did_merge)
{
...
void NetLogSendRequestBody(const NetLogWithSource& net_log, uint64_t length, bool is_chunked, bool did_merge)
{
    net_log.AddEvent(NetLogEventType::HTTP_TRANSACTION_SEND_REQUEST_BODY, [&] { return NetLogSendRequestBodyParams(length, is_chunked, did_merge); });
}
...
HttpStreamParser::HttpStreamParser(StreamSocket* stream_socket, bool connection_is_reused, const GURL& url, const std::string& method,
    UploadDataStream* upload_data_stream, GrowableIOBuffer* read_buffer, const NetLogWithSource& net_log)
    : url_(url)
    , method_(method)
    ...
    , net_log_(net_log)
    , truncate_to_content_length_enabled_(base::FeatureList::IsEnabled(features::kTruncateBodyToContentLength))
{
    io_callback_ = base::BindRepeating(&HttpStreamParser::OnIOComplete, weak_ptr_factory_.GetWeakPtr());
}

HttpStreamParser::~HttpStreamParser() = default;

int HttpStreamParser::SendRequest(const std::string& request_line, const HttpRequestHeaders& headers, const NetworkTrafficAnnotationTag& traffic_annotation,
    HttpResponseInfo* response, CompletionOnceCallback callback)
{
    ...
    NetLogRequestHeaders(net_log_, NetLogEventType::HTTP_TRANSACTION_SEND_REQUEST_HEADERS, request_line, &headers);
    ...
    if (upload_data_stream_) {
        request_body_send_buf_ = base::MakeRefCounted<SeekableIOBuffer>(kRequestBodyBufferSize);
        if (upload_data_stream_->is_chunked()) {
            ...
            request_body_read_buf_ = base::MakeRefCounted<SeekableIOBuffer>(kRequestBodyBufferSize - kChunkHeaderFooterSize);
        } else {
    ...
    if (ShouldMergeRequestHeadersAndBody(request, upload_data_stream_)) {
        int merged_size = static_cast<int>(request_headers_length_ + upload_data_stream_->size());
        auto merged_request_headers_and_body = base::MakeRefCounted<IOBufferWithSize>(merged_size);
        // We'll repurpose |request_headers_| to store the merged headers and
        // body.
        request_headers_ = base::MakeRefCounted<DrainableIOBuffer>(merged_request_headers_and_body, merged_size);
        ...
        while (todo) {
            int consumed = upload_data_stream_->Read(request_headers_.get(), static_cast<int>(todo), CompletionOnceCallback());
        ...
        NetLogSendRequestBody(net_log_, upload_data_stream_->size(), false, /* not chunked */
            true /* merged */);
    }
    ...
    if (!did_merge) {
        ...
        scoped_refptr<StringIOBuffer> headers_io_buf = base::MakeRefCounted<StringIOBuffer>(std::move(request));
        request_headers_ = base::MakeRefCounted<DrainableIOBuffer>(std::move(headers_io_buf), request_size);
    }
...
{
    int ret = stream_socket_->ConfirmHandshake(base::BindOnce(&HttpStreamParser::RunConfirmHandshakeCallback, weak_ptr_factory_.GetWeakPtr()));
...
int HttpStreamParser::ReadResponseBody(IOBuffer* buf, int buf_len, CompletionOnceCallback callback)
{
    DCHECK(io_state_ == STATE_NONE || io_state_ == STATE_DONE);
    DCHECK(callback_.is_null());
    DCHECK(!callback.is_null());
...
int HttpStreamParser::DoLoop(int result)
{
    do {
        ...
        case STATE_READ_HEADERS_COMPLETE:
            ...
            net_log_.EndEventWithNetErrorCode(NetLogEventType::HTTP_STREAM_PARSER_READ_HEADERS, result);
            break;
...
int HttpStreamParser::DoSendHeaders()
{
    ...
    return stream_socket_->Write(request_headers_.get(), bytes_remaining, io_callback_, NetworkTrafficAnnotationTag(traffic_annotation_));
}
...
int HttpStreamParser::DoSendHeadersComplete(int result)
{
    ...
    if (upload_data_stream_
        ...
        NetLogSendRequestBody(net_log_, upload_data_stream_->size(), upload_data_stream_->is_chunked(), false /* not merged */);
...
int HttpStreamParser::DoSendBody()
{
    if (request_body_send_buf_->BytesRemaining() > 0) {
        ...
        return stream_socket_->Write(
            request_body_send_buf_.get(), request_body_send_buf_->BytesRemaining(), io_callback_, NetworkTrafficAnnotationTag(traffic_annotation_));
    }
    ...
    return upload_data_stream_->Read(
        request_body_read_buf_.get(), request_body_read_buf_->capacity(), base::BindOnce(&HttpStreamParser::OnIOComplete, weak_ptr_factory_.GetWeakPtr()));
}
...
int HttpStreamParser::DoReadHeaders()
{
    ...
    return stream_socket_->Read(read_buf_.get(), read_buf_->RemainingCapacity(), io_callback_);
}
...
int HttpStreamParser::DoReadHeadersComplete(int result)
{
    ...
    if (io_state_ == STATE_DONE || !response_->headers || response_->headers->response_code() / 100 != 1) {
        response_ = nullptr;
    }
...
int HttpStreamParser::DoReadBody()
{
    ...
    if (truncate_to_content_length_enabled_ && !chunked_decoder_.get() && response_body_length_ >= 0) {
    ...
    if (read_buf_->offset()) {
        ...
        if (available) {
            ...
            user_read_buf_->span().copy_prefix_from(read_buf_->everything().subspan(read_buf_unused_offset_, bytes_from_buffer));
            read_buf_unused_offset_ += bytes_from_buffer;
            // Clear out the remaining data if we've reached the end of the body.
            if (truncate_to_content_length_enabled_ && (remaining_body == bytes_from_buffer) && (available > bytes_from_buffer)) {
    ...
    return stream_socket_->Read(user_read_buf_.get(), user_read_buf_len_, io_callback_);
}
...
int HttpStreamParser::DoReadBodyComplete(int result)
{
    ...
    if (truncate_to_content_length_enabled_ && !chunked_decoder_.get() && response_body_length_ >= 0) {
        ...
        int64_t remaining_read_len = std::min(static_cast<int64_t>(user_read_buf_len_), remaining_body);
    ...
    if (result > 0 && chunked_decoder_.get()) {
        result = chunked_decoder_->FilterBuf(user_read_buf_->span().first(base::checked_cast<size_t>(result)));
    ...
    if (result <= 0 || IsResponseBodyComplete()) {
        ...
        if (save_amount) {
            ...
            read_buf_->everything().copy_prefix_from(user_read_buf_->span().subspan(result, save_amount));
        }
        read_buf_->set_offset(save_amount);
        if (additional_save_amount) {
            read_buf_->span().copy_prefix_from(read_buf_->everything().subspan(read_buf_unused_offset_, additional_save_amount));
...
int HttpStreamParser::HandleReadHeaderResult(int result)
{
    ...
    if (read_buf_->offset() == 0) {
        response_->response_time = response_->original_response_time = base::Time::Now();
    ...
    if (end_of_header_offset == -1) {
        ...
        if (response_body_length_ == 0) {
            base::span<uint8_t> extra_bytes = read_buf_->span_before_offset().subspan(end_of_header_offset);
            ...
            if (response_->headers->response_code() / 100 == 1) {
                ...
                if (response_->headers->response_code() == HTTP_EARLY_HINTS && first_early_hints_time_.is_null()) {
...
int HttpStreamParser::FindAndParseResponseHeaders(int new_bytes)
{
    ...
    if (response_header_start_offset_ == std::string::npos) {
        response_header_start_offset_ = HttpUtil::LocateStartOfStatusLine(read_buf_->span_before_offset());
    }
    ...
    if (response_header_start_offset_ != std::string::npos) {
        ...
        size_t lower_bound = (base::ClampedNumeric<size_t>(read_buf_->offset()) - new_bytes - 3).RawValue();
        size_t search_start = std::max(response_header_start_offset_, lower_bound);
        end_offset = HttpUtil::LocateEndOfHeaders(read_buf_->span_before_offset(), search_start);
...
int HttpStreamParser::ParseResponseHeaders(size_t end_offset)
{
    ...
    if (response_header_start_offset_ != std::string::npos) {
        ...
        headers = HttpResponseHeaders::TryToCreate(base::as_string_view(read_buf_->everything().first(end_offset)));
        if (!headers)
        ...
        if (url::DefaultPortForScheme(scheme) != url_.EffectiveIntPort()) {
            ...
            if (read_buf_->offset() < 3 || scheme != "http"
                || !base::EqualsCaseInsensitiveASCII(base::as_string_view(read_buf_->everything().first(3u)), "icy")) {
                return ERR_INVALID_HTTP_RESPONSE;
            }
        }

        headers = base::MakeRefCounted<HttpResponseHeaders>(std::string("HTTP/0.9 200 OK"));
    }
    ...
    if (!headers->IsChunkEncoded()) {
        if (HttpUtil::HeadersContainMultipleCopiesOfField(*headers, "Content-Length"))
    ...
    if (HttpUtil::HeadersContainMultipleCopiesOfField(*headers, "Content-Disposition"))
    ...
    DVLOG(1) << __func__ << "() content_length = \"" << response_->headers->GetContentLength() << "\n\""
...
bool HttpStreamParser::CanReuseConnection() const
{
    ...
    if ((IsResponseBodyComplete() && IsMoreDataBuffered()) || discarded_extra_data_) {
        return false;
    }
...
int HttpStreamParser::EncodeChunk(std::string_view payload, base::span<uint8_t> output)
{
    ...
    // Add the header.
    const std::string header = base::StringPrintf("%X\r\n", static_cast<int>(payload.size()));
...
// static
bool HttpStreamParser::ShouldMergeRequestHeadersAndBody(const std::string& request_headers, const UploadDataStream* request_body)
{
...
{
    return request_headers_ == nullptr && request_body_send_buf_ == nullptr && request_body_read_buf_ == nullptr;
}
