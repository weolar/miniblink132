...
class NET_EXPORT_PRIVATE HttpStreamPool : public NetworkChangeNotifier::IPAddressObserver, public SSLClientContext::Observer {
public:
    ...
    public:
        ...
        virtual std::optional<int> OnPreconnect(const HttpStreamKey& stream_key, size_t num_streams) = 0;
    };

    // Reasons for closing streams.
    static constexpr std::string_view kIpAddressChanged = "IP address changed";
    static constexpr std::string_view kSslConfigChanged = "SSL configuration changed";
    static constexpr std::string_view kIdleTimeLimitExpired = "Idle time limit expired";
    ...
    static constexpr std::string_view kRemoteSideClosedConnection = "Remote side closed connection";
    static constexpr std::string_view kDataReceivedUnexpectedly = "Data received unexpectedly";
    static constexpr std::string_view kClosedConnectionReturnedToPool = "Connection was closed when it was returned to the pool";
    static constexpr std::string_view kSocketGenerationOutOfDate = "Socket generation out of date";
    ...
    static constexpr std::string_view kMaxStreamSocketsPerPoolParamName = "max_stream_per_pool";
    static constexpr std::string_view kMaxStreamSocketsPerGroupParamName = "max_stream_per_group";

    // FeatureParam name for enabling consistency checks.
    static constexpr std::string_view kEnableConsistencyCheckParamName = "enable_consistency_check";

    // The time to wait between connection attempts.
    static constexpr base::TimeDelta kConnectionAttemptDelay = base::Milliseconds(250);
    ...
    explicit HttpStreamPool(HttpNetworkSession* http_network_session, bool cleanup_on_ip_address_change = true);
    ...
    std::unique_ptr<HttpStreamRequest> RequestStream(HttpStreamRequest::Delegate* delegate, HttpStreamPoolSwitchingInfo switching_info,
        RequestPriority priority, const std::vector<SSLConfig::CertAndStatus>& allowed_bad_certs, bool enable_ip_based_pooling,
    ...
    int Preconnect(HttpStreamPoolSwitchingInfo switching_info, size_t num_streams, CompletionOnceCallback callback);
    ...
    size_t TotalIdleStreamCount()
    {
        return total_idle_stream_count_;
    }
    ...
    {
        return total_handed_out_stream_count_ + total_idle_stream_count_ + total_connecting_stream_count_;
    }
    ...
    bool IsQuicBroken(const url::SchemeHostPort& destination, const NetworkAnonymizationKey& network_anonymization_key);

    // Returns true when QUIC can be used for `destination`.
    bool CanUseQuic(const url::SchemeHostPort& destination, const NetworkAnonymizationKey& network_anonymization_key, bool enable_ip_based_pooling,
    ...
    quic::ParsedQuicVersion SelectQuicVersion(const AlternativeServiceInfo& alternative_service_info);

    // Returns true when there is an existing QUIC session for `quic_session_key`.
    bool CanUseExistingQuicSession(const QuicSessionAliasKey& quic_session_alias_key, bool enable_ip_based_pooling, bool enable_alternative_services);
    ...
    void set_max_stream_sockets_per_group_for_testing(size_t max_stream_sockets_per_group)
    {
        max_stream_sockets_per_group_ = max_stream_sockets_per_group;
    }

    Group& GetOrCreateGroup(const HttpStreamKey& stream_key, std::optional<QuicSessionAliasKey> quic_session_alias_key = std::nullopt);
...
private:
    ...
    base::WeakPtr<SpdySession> FindAvailableSpdySession(const HttpStreamKey& stream_key, const SpdySessionKey& spdy_session_key, bool enable_ip_based_pooling,
        const NetLogWithSource& net_log = NetLogWithSource());

    void OnPreconnectComplete(JobController* job_controller, CompletionOnceCallback callback, int rv);
    ...
    std::set<std::unique_ptr<JobController>, base::UniquePtrComparator> job_controllers_;
