...
base::Value::Dict NetLogParameterChannelBindings(const std::string& channel_binding_token, NetLogCaptureMode capture_mode)
{
...
#endif
    const HttpAuthPreferences* prefs, HttpAuthMechanismFactory negotiate_auth_system_factory)
{
    ...
    return std::make_unique<HttpAuthSSPI>(auth_library, HttpAuth::AUTH_SCHEME_NEGOTIATE);
#elif BUILDFLAG(IS_POSIX)
    return std::make_unique<HttpAuthGSSAPI>(auth_library, CHROME_GSS_SPNEGO_MECH_OID_DESC);
#endif
}

} // namespace

HttpAuthHandlerNegotiate::Factory::Factory(HttpAuthMechanismFactory negotiate_auth_system_factory)
...
const std::string& HttpAuthHandlerNegotiate::Factory::GetLibraryNameForTesting() const
{
    return auth_library_->GetLibraryNameForTesting();
}
#endif // !BUILDFLAG(IS_ANDROID) && BUILDFLAG(IS_POSIX)

int HttpAuthHandlerNegotiate::Factory::CreateAuthHandler(HttpAuthChallengeTokenizer* challenge, HttpAuth::Target target, const SSLInfo& ssl_info,
    ...
    const NetLogWithSource& net_log, HostResolver* host_resolver, std::unique_ptr<HttpAuthHandler>* handler)
{
    ...
    std::unique_ptr<HttpAuthHandler> tmp_handler(std::make_unique<HttpAuthHandlerNegotiate>(
        CreateAuthSystem(auth_library_.get(), http_auth_preferences(), negotiate_auth_system_factory_), http_auth_preferences(), host_resolver));
    ...
    //                 method and only constructing when valid.
    std::unique_ptr<HttpAuthHandler> tmp_handler(std::make_unique<HttpAuthHandlerNegotiate>(
        CreateAuthSystem(http_auth_preferences(), negotiate_auth_system_factory_), http_auth_preferences(), host_resolver));
#elif BUILDFLAG(IS_POSIX)
    if (is_unsupported_)
        return ERR_UNSUPPORTED_AUTH_SCHEME;
    ...
    if (!http_auth_preferences() || !http_auth_preferences()->AllowGssapiLibraryLoad()) {
        return ERR_UNSUPPORTED_AUTH_SCHEME;
    }
    ...
    std::unique_ptr<HttpAuthHandler> tmp_handler(std::make_unique<HttpAuthHandlerNegotiate>(
        CreateAuthSystem(auth_library_.get(), http_auth_preferences(), negotiate_auth_system_factory_), http_auth_preferences(), host_resolver));
#endif
    if (!tmp_handler->InitFromChallenge(challenge, target, ssl_info, network_anonymization_key, scheme_host_port, net_log)) {
...
HttpAuthHandlerNegotiate::HttpAuthHandlerNegotiate(std::unique_ptr<HttpAuthMechanism> auth_system, const HttpAuthPreferences* prefs, HostResolver* resolver)
...
bool HttpAuthHandlerNegotiate::Init(HttpAuthChallengeTokenizer* challenge, const SSLInfo& ssl_info, const NetworkAnonymizationKey& network_anonymization_key)
{
    ...
    if (ssl_info.is_valid())
        x509_util::GetTLSServerEndPointChannelBinding(*ssl_info.cert, &channel_bindings_);
    if (!channel_bindings_.empty())
        net_log().AddEvent(NetLogEventType::AUTH_CHANNEL_BINDINGS,
            [&](NetLogCaptureMode capture_mode) { return NetLogParameterChannelBindings(channel_bindings_, capture_mode); });
    return true;
}

int HttpAuthHandlerNegotiate::GenerateAuthTokenImpl(
    const AuthCredentials* credentials, const HttpRequestInfo* request, CompletionOnceCallback callback, std::string* auth_token)
{
    DCHECK(callback_.is_null());
    DCHECK(auth_token_ == nullptr);
    auth_token_ = auth_token;
    if (already_called_) {
        DCHECK((!has_credentials_ && credentials == nullptr) || (has_credentials_ && credentials->Equals(credentials_)));
...
HttpAuth::AuthorizationResult HttpAuthHandlerNegotiate::HandleAnotherChallengeImpl(HttpAuthChallengeTokenizer* challenge)
{
    return auth_system_->ParseChallenge(challenge);
}

std::string HttpAuthHandlerNegotiate::CreateSPN(const std::string& server, const url::SchemeHostPort& scheme_host_port)
{
    ...
    if (port != 80 && port != 443 && (http_auth_preferences_ && http_auth_preferences_->NegotiateEnablePort())) {
        return base::StringPrintf("HTTP%c%s:%d", kSpnSeparator, server.c_str(), port);
    } else {
...
int HttpAuthHandlerNegotiate::DoResolveCanonicalName()
{
    ...
    resolve_host_request_ = resolver_->CreateRequest(scheme_host_port_, network_anonymization_key_, net_log(), parameters);
    return resolve_host_request_->Start(base::BindOnce(&HttpAuthHandlerNegotiate::OnIOComplete, base::Unretained(this)));
}
...
int HttpAuthHandlerNegotiate::DoResolveCanonicalNameComplete(int rv)
{
    ...
    if (resolve_host_request_) {
        if (rv == OK) {
            ...
            VLOG(1) << "Problem finding canonical name for SPN for host " << scheme_host_port_.host() << ": " << ErrorToString(rv);
            rv = OK;
        }
    }
...
int HttpAuthHandlerNegotiate::DoGenerateAuthToken()
{
    ...
    return auth_system_->GenerateAuthToken(
        credentials, spn_, channel_bindings_, auth_token_, net_log(), base::BindOnce(&HttpAuthHandlerNegotiate::OnIOComplete, base::Unretained(this)));
}
