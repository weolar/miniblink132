...
public:
    ...
    {
        return HttpStreamKey(destination_, privacy_mode_, SocketTag(), NetworkAnonymizationKey(), secure_dns_policy_, disable_cert_network_fetches_);
    }
...
public:
    ...
    {
        AlternativeService alternative_service(NextProto::kProtoQUIC, key_builder_.destination().host(), key_builder_.destination().port());
    ...
    {
        return key_builder_.Build();
    }

    int Preconnect(HttpStreamPool& pool)
    {
        int rv = pool.Preconnect(HttpStreamPoolSwitchingInfo(GetStreamKey(), alternative_service_info_, is_http1_allowed_, load_flags_, proxy_info_),
            num_streams_, base::BindOnce(&Preconnector::OnComplete, base::Unretained(this)));
...
public:
    ...
    StreamRequester& set_enable_alternative_services(bool enable_alternative_services)
    {
    ...
    StreamRequester& set_alternative_service_info(AlternativeServiceInfo alternative_service_info)
    {
    ...
    HttpStreamRequest* RequestStream(HttpStreamPool& pool)
    {
        ...
        request_ = pool.RequestStream(this, HttpStreamPoolSwitchingInfo(stream_key, alternative_service_info_, is_http1_allowed_, load_flags_, proxy_info_),
            priority_, allowed_bad_certs_, enable_ip_based_pooling_, enable_alternative_services_, NetLogWithSource());
        return request_.get();
    }
    ...
    void OnSwitchesToHttpStreamPool(HttpStreamPoolSwitchingInfo request_info) override
    {
    }

    std::unique_ptr<HttpStream> ReleaseStream()
    {
    ...
    const SSLInfo& cert_error_ssl_info() const
    {
        return cert_error_ssl_info_;
    }

    scoped_refptr<SSLCertRequestInfo> cert_info() const
    {
        return cert_info_;
    }
...
public:
    explicit TestJobDelegate(std::optional<HttpStreamKey> stream_key = std::nullopt)
    {
    ...
    void CreateAndStartJob(HttpStreamPool& pool)
    {
        ...
        job_->Start(RequestPriority::DEFAULT_PRIORITY, /*allowed_bad_certs=*/ {}, HttpStreamPool::RespectLimits::kRespect,
            /*enable_ip_based_pooling=*/true,
            /*enable_alternative_services=*/true, quic_version_, NetLogWithSource());
    }
    ...
    void OnStreamReady(Job* job, std::unique_ptr<HttpStream> stream, NextProto negotiated_protocol) override
    {
        negotiated_protocol_ = negotiated_protocol;
        SetResult(OK);
    }

    void OnStreamFailed(Job* job, int status, const NetErrorDetails& net_error_details, ResolveErrorInfo resolve_error_info) override
    {
        SetResult(status);
    }

    void OnCertificateError(Job* job, int status, const SSLInfo& ssl_info) override
    {
        SetResult(status);
    }
...
private:
    ...
    {
        result_future_.SetValue(result);
    }

    StreamKeyBuilder key_builder_;

    NextProto expected_protocol_ = NextProto::kProtoUnknown;
    quic::ParsedQuicVersion quic_version_ = quic::ParsedQuicVersion::Unsupported();
...
protected:
    void InitializeSession()
    {
        ...
        quic_context->params()->origins_to_force_quic_on = origins_to_force_quic_on_;
        ...
        session_deps_.ssl_config_service = std::make_unique<TestSSLConfigService>(config);

        http_network_session_ = SpdySessionDependencies::SpdyCreateSession(&session_deps_);
    }

    void DestroyHttpNetworkSession()
    {
        http_network_session_.reset();
    }
    ...
    HttpStreamPool& pool()
    {
        return *http_network_session_->http_stream_pool();
    }
    ...
    {
        return static_cast<TestSSLConfigService*>(session_deps_.ssl_config_service.get());
    }

    MockCryptoClientStreamFactory* crypto_client_stream_factory()
    {
        return static_cast<MockCryptoClientStreamFactory*>(session_deps_.quic_crypto_client_stream_factory.get());
    }
    ...
    base::WeakPtr<SpdySession> CreateFakeSpdySession(const HttpStreamKey& stream_key, IPEndPoint peer_addr = IPEndPoint(IPAddress(192, 0, 2, 1), 443))
    {
        ...
        auto handle = group.CreateHandle(std::move(socket), StreamSocketHandle::SocketReuseType::kUnused, LoadTimingInfo::ConnectTiming());

        base::WeakPtr<SpdySession> spdy_session;
        int rv = spdy_session_pool()->CreateAvailableSessionFromSocketHandle(
            group.spdy_session_key(), std::move(handle), NetLogWithSource(), MultiplexedSessionCreationInitiator::kUnknown, &spdy_session);
        CHECK_EQ(rv, OK);
    ...
    void AddQuicData(std::string_view host = kDefaultServerName, MockConnectCompleter* connect_completer = nullptr)
    {
        auto client_maker
            = std::make_unique<QuicTestPacketMaker>(quic_version(), quic::QuicUtils::CreateRandomConnectionId(session_deps_.quic_context->random_generator()),
                session_deps_.quic_context->clock(), std::string(host), quic::Perspective::IS_CLIENT);
        ...
        quic_data->AddWrite(SYNCHRONOUS,
            client_maker->Packet(packet_number++).AddConnectionCloseFrame(quic::QUIC_CONNECTION_CANCELLED, "net error", quic::NO_IETF_QUIC_ERROR).Build());
    ...
    QuicTestPacketMaker* CreateQuicClientPacketMaker(std::string_view host = kDefaultServerName)
    {
        auto client_maker
            = std::make_unique<QuicTestPacketMaker>(quic_version(), quic::QuicUtils::CreateRandomConnectionId(session_deps_.quic_context->random_generator()),
                session_deps_.quic_context->clock(), std::string(host), quic::Perspective::IS_CLIENT);
...
TEST_F(HttpStreamPoolAttemptManagerTest, ResolveEndpointFailedMultipleRequests)
{
    FakeServiceEndpointRequest* endpoint_request = resolver()->AddFakeRequest();
...
TEST_F(HttpStreamPoolAttemptManagerTest, DnsAliases)
{
    ...
    FakeServiceEndpointRequest* endpoint_request = resolver()->AddFakeRequest();
    endpoint_request->add_endpoint(ServiceEndpointBuilder().add_v4("192.0.2.1").endpoint()).set_aliases(kAliases).CompleteStartSynchronously(OK);
...
TEST_F(HttpStreamPoolAttemptManagerTest, ConnectTiming)
{
    ...
    endpoint_request->add_endpoint(ServiceEndpointBuilder().add_v4("192.0.2.1").endpoint()).set_crypto_ready(false).CallOnServiceEndpointsUpdated();
    RunUntilIdle();
    ASSERT_FALSE(requester.result().has_value());

    FastForwardBy(kDnsFinishDelay);
    endpoint_request->set_crypto_ready(true).CallOnServiceEndpointRequestFinished(OK);
    ASSERT_FALSE(requester.result().has_value());
    ...
    stream->InitializeStream(/*can_send_early=*/false, RequestPriority::IDLE, NetLogWithSource(), base::DoNothing());
    ...
    ASSERT_EQ(connect_timing.domain_lookup_end - connect_timing.domain_lookup_start, kDnsUpdateDelay);
    ASSERT_EQ(connect_timing.connect_end - connect_timing.connect_start, kDnsFinishDelay + kTcpDelay + kTlsDelay);
    ASSERT_EQ(connect_timing.ssl_end - connect_timing.ssl_start, kTlsDelay);
}

TEST_F(HttpStreamPoolAttemptManagerTest, ConnectTimingDnsResolutionNotFinished)
{
    ...
    FastForwardBy(kDnsUpdateDelay);
    endpoint_request->add_endpoint(ServiceEndpointBuilder().add_v4("192.0.2.1").endpoint()).set_crypto_ready(true).CallOnServiceEndpointsUpdated();
    RunUntilIdle();
    FastForwardBy(kDnsUpdateDelay);
    EXPECT_THAT(requester.result(), Optional(IsOk()));
    ...
    LoadTimingInfo timing_info;
    ASSERT_TRUE(stream->GetLoadTimingInfo(&timing_info));
    ASSERT_EQ(timing_info.connect_timing.domain_lookup_end, timing_info.connect_timing.connect_start);
}
...
TEST_F(HttpStreamPoolAttemptManagerTest, PlainHttpWaitForHttpsRecord)
{
    ...
    endpoint_request->add_endpoint(ServiceEndpointBuilder().add_v4("192.0.2.1").endpoint()).CallOnServiceEndpointsUpdated();
    ...
    endpoint_request->CallOnServiceEndpointRequestFinished(ERR_DNS_NAME_HTTPS_ONLY);
    requester.WaitForResult();
...
TEST_F(HttpStreamPoolAttemptManagerTest, SetPriority)
{
    ...
    StreamRequester requester1;
    HttpStreamRequest* request1 = requester1.set_priority(RequestPriority::LOW).RequestStream(pool());
    AttemptManager* manager = pool().GetOrCreateGroupForTesting(requester1.GetStreamKey()).GetAttemptManagerForTesting();
    ...
    StreamRequester requester2;
    HttpStreamRequest* request2 = requester2.set_priority(RequestPriority::IDLE).RequestStream(pool());
    ASSERT_EQ(manager, pool().GetOrCreateGroupForTesting(requester2.GetStreamKey()).GetAttemptManagerForTesting());
    ASSERT_EQ(endpoint_request->priority(), RequestPriority::LOW);
    ASSERT_EQ(manager->GetPriority(), RequestPriority::LOW);
...
TEST_F(HttpStreamPoolAttemptManagerTest, TcpFailSync)
{
    ...
    endpoint_request->add_endpoint(ServiceEndpointBuilder().add_v4("192.0.2.1").endpoint());
...
TEST_F(HttpStreamPoolAttemptManagerTest, TlsOkAsync)
{
    ...
    endpoint_request->add_endpoint(ServiceEndpointBuilder().add_v4("192.0.2.1").endpoint()).CallOnServiceEndpointRequestFinished(OK);
    RunUntilIdle();
    EXPECT_THAT(requester.result(), Optional(IsOk()));
}
...
TEST_F(HttpStreamPoolAttemptManagerTest, TcpSyncTlsAsyncOk)
{
    ...
    resolver()->AddFakeRequest()->add_endpoint(ServiceEndpointBuilder().add_v4("192.0.2.1").endpoint()).CompleteStartSynchronously(OK);
...
TEST_F(HttpStreamPoolAttemptManagerTest, TlsCryptoReadyDelayed)
{
    ...
    StreamRequester requester;
    HttpStreamRequest* request = requester.set_destination("https://a.test").RequestStream(pool());
...
TEST_F(HttpStreamPoolAttemptManagerTest, CertificateError)
{
    ...
    const scoped_refptr<X509Certificate> kCert = ImportCertFromFile(GetTestCertsDirectory(), "ok_cert.pem");
    ...
    ASSERT_TRUE(requester1.cert_error_ssl_info().cert->EqualsIncludingChain(kCert.get()));
    ASSERT_EQ(requester1.connection_attempts().size(), 1u);
    ASSERT_EQ(requester1.connection_attempts()[0].result, ERR_CERT_DATE_INVALID);

    ASSERT_TRUE(requester2.cert_error_ssl_info().cert->EqualsIncludingChain(kCert.get()));
...
TEST_F(HttpStreamPoolAttemptManagerTest, NeedsClientAuth)
{
    ...
    ssl.cert_request_info->host_and_port = HostPortPair::FromSchemeHostPort(kDestination);
    socket_factory()->AddSSLSocketDataProvider(&ssl);
    ...
    endpoint_request->set_crypto_ready(true).CallOnServiceEndpointsUpdated();
    RunUntilIdle();
    EXPECT_EQ(requester1.cert_info()->host_and_port, HostPortPair::FromSchemeHostPort(kDestination));
    EXPECT_EQ(requester2.cert_info()->host_and_port, HostPortPair::FromSchemeHostPort(kDestination));
}
...
TEST_F(HttpStreamPoolAttemptManagerTest, OneIPEndPointFailed)
{
    ...
    endpoint_request->add_endpoint(ServiceEndpointBuilder().add_v6("2001:db8::1").add_v4("192.0.2.1").endpoint());
    endpoint_request->CallOnServiceEndpointRequestFinished(OK);
    RunUntilIdle();
    EXPECT_THAT(requester.result(), Optional(IsOk()));
}
...
TEST_F(HttpStreamPoolAttemptManagerTest, IPEndPointsSlow)
{
    ...
    endpoint_request->add_endpoint(ServiceEndpointBuilder().add_v6("2001:db8::1").add_v6("2001:db8::2").add_v4("192.0.2.1").endpoint());
    endpoint_request->CallOnServiceEndpointRequestFinished(OK);
    RunUntilIdle();
    AttemptManager* manager = pool().GetOrCreateGroupForTesting(requester.GetStreamKey()).GetAttemptManagerForTesting();
...
TEST_F(HttpStreamPoolAttemptManagerTest, PauseSlowTimerAfterTcpHandshakeForTls)
{
    FakeServiceEndpointRequest* endpoint_request = resolver()->AddFakeRequest();
    ...
    auto ssl1 = std::make_unique<SSLSocketDataProvider>(SYNCHRONOUS, ERR_IO_PENDING);
...
TEST_F(HttpStreamPoolAttemptManagerTest, SlowTimerFiredAfterIdleSocketAvailable)
{
    resolver()->AddFakeRequest()->add_endpoint(ServiceEndpointBuilder().add_v4("192.0.2.1").endpoint()).CompleteStartSynchronously(OK);

    HttpStreamKey stream_key = StreamKeyBuilder().set_destination("http://a.test").Build();
    ...
    std::unique_ptr<HttpStream> stream
        = group.CreateTextBasedStream(std::make_unique<FakeStreamSocket>(), StreamSocketHandle::SocketReuseType::kReusedIdle, LoadTimingInfo::ConnectTiming());
    stream.reset();
...
TEST_F(HttpStreamPoolAttemptManagerTest, FeatureParamStreamLimits)
{
    ...
    feature_list.InitAndEnableFeatureWithParameters(features::kHappyEyeballsV3,
        { { std::string(HttpStreamPool::kMaxStreamSocketsPerPoolParamName), "2" }, { std::string(HttpStreamPool::kMaxStreamSocketsPerGroupParamName), "3" } });
    InitializeSession();
...
TEST_F(HttpStreamPoolAttemptManagerTest, ReachedGroupLimit)
{
    ...
    Group& group = pool().GetOrCreateGroupForTesting(requesters[0]->GetStreamKey());
    ...
    ASSERT_EQ(stalled_request->GetLoadState(), LOAD_STATE_WAITING_FOR_AVAILABLE_SOCKET);
    ...
    std::unique_ptr<StreamRequester> released_requester = std::move(requesters.back());
    requesters.pop_back();
    std::unique_ptr<HttpStream> released_stream = released_requester->ReleaseStream();
    ...
    released_stream->InitializeStream(/*can_send_early=*/false, RequestPriority::IDLE, NetLogWithSource(), base::DoNothing());
...
TEST_F(HttpStreamPoolAttemptManagerTest, ReachedPoolLimit)
{
    ...
    const HttpStreamKey key_a(url::SchemeHostPort("http", "a.test", 80), PRIVACY_MODE_DISABLED, SocketTag(), NetworkAnonymizationKey(), SecureDnsPolicy::kAllow,
        /*disable_cert_network_fetches=*/false);

    const HttpStreamKey key_b(url::SchemeHostPort("http", "b.test", 80), PRIVACY_MODE_DISABLED, SocketTag(), NetworkAnonymizationKey(), SecureDnsPolicy::kAllow,
        /*disable_cert_network_fetches=*/false);
    ...
    for (size_t i = 0; i < kMaxPerGroup; ++i) {
        streams_a.emplace_back(
            group_a.CreateTextBasedStream(std::make_unique<FakeStreamSocket>(), StreamSocketHandle::SocketReuseType::kUnused, LoadTimingInfo::ConnectTiming()));
    }
    ...
    data2->set_connect_data(MockConnect(ASYNC, OK));
    socket_factory()->AddSocketDataProvider(data2.get());
    ASSERT_EQ(request2->GetLoadState(), LOAD_STATE_WAITING_FOR_STALLED_SOCKET_POOL);
...
TEST_F(HttpStreamPoolAttemptManagerTest, ReachedPoolLimitHighPriorityGroupFirst)
{
    constexpr size_t kMaxPerGroup = 1;
    ...
    for (const auto& [host, ip_address, priority] : items) {
        FakeServiceEndpointRequest* endpoint_request = resolver()->AddFakeRequest();
        endpoint_request->add_endpoint(ServiceEndpointBuilder().add_v4(ip_address).endpoint());
        endpoint_requests.emplace_back(endpoint_request);

        auto requester = std::make_unique<StreamRequester>();
        requester->set_destination(url::SchemeHostPort("http", host, 80)).set_priority(priority);
        requesters.emplace_back(std::move(requester));
...
TEST_F(HttpStreamPoolAttemptManagerTest, ReachedPerGroupLimitWithIdleStreamSocket)
{
    resolver()->AddFakeRequest()->add_endpoint(ServiceEndpointBuilder().add_v4("192.0.2.1").endpoint()).CompleteStartSynchronously(OK);
    ...
    for (size_t i = 0; i < pool().max_stream_sockets_per_group() + 1; ++i) {
        ...
        ASSERT_LE(group.ActiveStreamSocketCount(), pool().max_stream_sockets_per_group());
    }
...
TEST_F(HttpStreamPoolAttemptManagerTest, UseIdleStreamSocketAfterRelease)
{
    ...
    for (size_t i = 0; i < pool().max_stream_sockets_per_group(); ++i) {
        std::unique_ptr<HttpStream> http_stream
            = group.CreateTextBasedStream(std::make_unique<FakeStreamSocket>(), StreamSocketHandle::SocketReuseType::kUnused, LoadTimingInfo::ConnectTiming());
        streams.emplace_back(std::move(http_stream));
    }
    ASSERT_EQ(group.ActiveStreamSocketCount(), pool().max_stream_sockets_per_group());
    ASSERT_EQ(group.IdleStreamSocketCount(), 0u);
...
TEST_F(HttpStreamPoolAttemptManagerTest, CloseIdleStreamAttemptConnectionReachedPoolLimit)
{
    constexpr size_t kMaxPerGroup = 2;
    constexpr size_t kMaxPerPool = 3;
    pool().set_max_stream_sockets_per_group_for_testing(kMaxPerGroup);
    ...
    std::unique_ptr<HttpStream> stream1
        = group_b.CreateTextBasedStream(std::make_unique<FakeStreamSocket>(), StreamSocketHandle::SocketReuseType::kUnused, LoadTimingInfo::ConnectTiming());
    ASSERT_TRUE(pool().ReachedMaxStreamLimit());
...
TEST_F(HttpStreamPoolAttemptManagerTest, ProcessPendingRequestDnsResolutionOngoing)
{
    FakeServiceEndpointRequest* endpoint_request = resolver()->AddFakeRequest();
...
TEST_F(HttpStreamPoolAttemptManagerTest, CancelAttemptAndRequestsOnIPAddressChange)
{
    ...
    endpoint_request1->add_endpoint(ServiceEndpointBuilder().add_v4("192.0.2.1").endpoint());
    endpoint_request1->CallOnServiceEndpointRequestFinished(OK);
    endpoint_request2->add_endpoint(ServiceEndpointBuilder().add_v4("192.0.2.2").endpoint());
    endpoint_request2->CallOnServiceEndpointRequestFinished(OK);

    AttemptManager* manager1 = pool().GetOrCreateGroupForTesting(requester1.GetStreamKey()).GetAttemptManagerForTesting();
    AttemptManager* manager2 = pool().GetOrCreateGroupForTesting(requester2.GetStreamKey()).GetAttemptManagerForTesting();
...
TEST_F(HttpStreamPoolAttemptManagerTest, IPAddressChangeAfterNeedsClientAuth)
{
    ...
    NetworkChangeNotifier::NotifyObserversOfIPAddressChangeForTests();
    RunUntilIdle();
    EXPECT_THAT(requester1.result(), Optional(IsError(ERR_SSL_CLIENT_AUTH_CERT_NEEDED)));
    EXPECT_THAT(requester2.result(), Optional(IsError(ERR_NETWORK_CHANGED)));
}
...
TEST_F(HttpStreamPoolAttemptManagerTest, SSLConfigChangedReleasedStreamGenerationOutdated)
{
    StreamRequester requester;
    requester.set_destination("https://a.test");
    Group& group = pool().GetOrCreateGroupForTesting(requester.GetStreamKey());
...
TEST_F(HttpStreamPoolAttemptManagerTest, SSLConfigForServersChanged)
{
    ...
    Group& group_a = pool().GetOrCreateGroupForTesting(requester_a.GetStreamKey());
    group_a.AddIdleStreamSocket(std::make_unique<FakeStreamSocket>());
    ASSERT_EQ(group_a.IdleStreamSocketCount(), 1u);

    StreamRequester requester_b;
    requester_b.set_destination("https://b.test");
    Group& group_b = pool().GetOrCreateGroupForTesting(requester_b.GetStreamKey());
    ...
    pool().OnSSLConfigForServersChanged({ HostPortPair::FromSchemeHostPort(requester_a.GetStreamKey().destination()) });
...
{
    StreamRequester requester;
    requester.set_destination("https://a.test").set_enable_ip_based_pooling(false);
...
TEST_F(HttpStreamPoolAttemptManagerTest, SpdyOk)
{
    ...
    for (size_t i = 0; i < kNumRequests; ++i) {
        ...
        auto requester = std::make_unique<StreamRequester>();
        requester->set_destination("https://a.test").set_enable_ip_based_pooling(false).RequestStream(pool());
        requesters.emplace_back(std::move(requester));
    }
...
TEST_F(HttpStreamPoolAttemptManagerTest, RequireHttp11AfterSpdySessionCreated)
{
    ...
    ASSERT_TRUE(spdy_session_pool()->HasAvailableSession(stream_key.CalculateSpdySessionKey(), /*is_websocket=*/false));

    // Disable HTTP/2.
    http_server_properties()->SetHTTP11Required(stream_key.destination(), stream_key.network_anonymization_key());
    ...
    ASSERT_FALSE(spdy_session_pool()->HasAvailableSession(stream_key.CalculateSpdySessionKey(), /*is_websocket=*/false));
}

TEST_F(HttpStreamPoolAttemptManagerTest, RequireHttp11AfterSpdySessionCreatedRequestDestroyed)
{
    const MockWrite writes[] = { MockWrite(SYNCHRONOUS, ERR_IO_PENDING, 1) };
    const MockRead reads[] = { MockRead(SYNCHRONOUS, ERR_IO_PENDING, 0) };
    auto h2_data = std::make_unique<SequencedSocketData>(reads, writes);
...
TEST_F(HttpStreamPoolAttemptManagerTest, CloseIdleSpdySessionWhenPoolStalled)
{
    ...
    const HttpStreamKey stream_key_a = StreamKeyBuilder().set_destination(kDestinationA).Build();
    CreateFakeSpdySession(stream_key_a);
    ASSERT_TRUE(spdy_session_pool()->HasAvailableSession(stream_key_a.CalculateSpdySessionKey(), /*is_websocket=*/false));
    ...
    ASSERT_TRUE(spdy_session_pool()->HasAvailableSession(requester_b.GetStreamKey().CalculateSpdySessionKey(),
        /*is_websocket=*/false));
    ASSERT_FALSE(spdy_session_pool()->HasAvailableSession(stream_key_a.CalculateSpdySessionKey(), /*is_websocket=*/false));
}

TEST_F(HttpStreamPoolAttemptManagerTest, PreconnectRequireHttp11AfterSpdySessionCreated)
{
...
TEST_F(HttpStreamPoolAttemptManagerTest, SpdyReachedPoolLimit)
{
    ...
    StreamRequester requester_a;
    requester_a.set_destination("https://a.test");
    base::WeakPtr<SpdySession> spdy_session_a = CreateFakeSpdySession(requester_a.GetStreamKey(), MakeIPEndPoint("192.0.2.1"));
    ...
    stream_a->InitializeStream(/*can_send_early=*/false, DEFAULT_PRIORITY, NetLogWithSource(), base::DoNothing());

    StreamRequester requester_b;
    requester_b.set_destination("https://b.test");
    CreateFakeSpdySession(requester_b.GetStreamKey(), MakeIPEndPoint("192.0.2.2"));
    ...
    request_info_b.traffic_annotation = MutableNetworkTrafficAnnotationTag(TRAFFIC_ANNOTATION_FOR_TESTS);
    stream_b->RegisterRequest(&request_info_b);
    stream_b->InitializeStream(/*can_send_early=*/false, DEFAULT_PRIORITY, NetLogWithSource(), base::DoNothing());
    ...
    endpoint_request->add_endpoint(ServiceEndpointBuilder().add_v4("192.0.2.1").endpoint()).CallOnServiceEndpointRequestFinished(OK);
    RunUntilIdle();
    Group& group_c = pool().GetOrCreateGroupForTesting(requester_c.GetStreamKey());
...
TEST_F(HttpStreamPoolAttemptManagerTest, SpdyMatchingIpSessionOk)
{
    ...
    endpoint_request->add_endpoint(ServiceEndpointBuilder().add_ip_endpoint(kCommonEndPoint).endpoint()).CallOnServiceEndpointRequestFinished(OK);
    RunUntilIdle();
    EXPECT_THAT(requester_b.result(), Optional(IsOk()));
    ASSERT_EQ(pool().TotalActiveStreamCount(), 1u);
}

TEST_F(HttpStreamPoolAttemptManagerTest, SpdyMatchingIpSessionBecomeUnavailableBeforeNotify)
{
    const IPEndPoint kCommonEndPoint = MakeIPEndPoint("2001:db8::1", 443);
    ...
    StreamRequester requester_b;
    requester_b.set_destination("https://example.test").RequestStream(pool());
    endpoint_request->add_endpoint(ServiceEndpointBuilder().add_ip_endpoint(kCommonEndPoint).endpoint()).CallOnServiceEndpointsUpdated();
    ...
    EXPECT_THAT(requester_b.result(), Optional(IsOk()));
    ASSERT_TRUE(spdy_session_pool()->FindAvailableSession(requester_b.GetStreamKey().CalculateSpdySessionKey(),
        /*enable_ip_based_pooling=*/true, /*is_websocket=*/false, NetLogWithSource()));
}
...
TEST_F(HttpStreamPoolAttemptManagerTest, SpdyMatchingIpSessionAlreadyHaveSession)
{
    const IPEndPoint kCommonEndPoint = MakeIPEndPoint("2001:db8::1", 443);
    ...
    endpoint_request->add_endpoint(ServiceEndpointBuilder().add_ip_endpoint(kCommonEndPoint).endpoint())
        .CallOnServiceEndpointsUpdated()
...
TEST_F(HttpStreamPoolAttemptManagerTest, SpdyMatchingIpSessionDnsResolutionFinishSynchronously)
{
    const IPEndPoint kCommonEndPoint = MakeIPEndPoint("2001:db8::1", 443);
    ...
    FakeServiceEndpointRequest* endpoint_request = resolver()->AddFakeRequest();
    endpoint_request->add_endpoint(ServiceEndpointBuilder().add_ip_endpoint(kCommonEndPoint).endpoint()).set_start_result(OK);
...
TEST_F(HttpStreamPoolAttemptManagerTest, SpdyMatchingIpSessionDisabled)
{
    ...
    StreamRequester requester_b;
    requester_b.set_destination("https://example.test").set_enable_ip_based_pooling(false).RequestStream(pool());
...
TEST_F(HttpStreamPoolAttemptManagerTest, SpdyMatchingIpSessionKeyMismatch)
{
    ...
    requester_a.set_destination("https://www.example.org").set_privacy_mode(PRIVACY_MODE_ENABLED_WITHOUT_CLIENT_CERTS);
...
TEST_F(HttpStreamPoolAttemptManagerTest, SpdyMatchingIpSessionVerifyDomainFailed)
{
    const IPEndPoint kCommonEndPoint = MakeIPEndPoint("192.0.2.1", 443);
    ...
    StreamRequester requester_b;
    requester_b.set_destination("https://non-alternative.test").RequestStream(pool());
...
TEST_F(HttpStreamPoolAttemptManagerTest, ThrottleAttemptForSpdyBlockSecondAttempt)
{
    constexpr std::string_view kDestination = "https://a.test";
    ...
    HttpStreamKey stream_key = requester1.GetStreamKey();
    http_server_properties()->SetSupportsSpdy(stream_key.destination(), stream_key.network_anonymization_key(),
        /*supports_spdy=*/true);
...
TEST_F(HttpStreamPoolAttemptManagerTest, ThrottleAttemptForSpdyDelayPassedHttp2)
{
    constexpr std::string_view kDestination = "https://a.test";
...
TEST_F(HttpStreamPoolAttemptManagerTest, ThrottleAttemptForSpdyDelayPassedHttp1)
{
    constexpr std::string_view kDestination = "https://a.test";
...
TEST_F(HttpStreamPoolAttemptManagerTest, PreconnectMultipleStreamsHttp1)
{
    ...
    endpoint_request->add_endpoint(ServiceEndpointBuilder().add_v4("192.0.2.1").endpoint()).CallOnServiceEndpointRequestFinished(OK);
    Group& group = pool().GetOrCreateGroupForTesting(preconnector.GetStreamKey());
    ASSERT_EQ(group.GetAttemptManagerForTesting()->InFlightAttemptCount(), kNumStreams);
    ASSERT_FALSE(preconnector.result().has_value());
...
TEST_F(HttpStreamPoolAttemptManagerTest, PreconnectMultipleStreamsHttp2)
{
    ...
    EXPECT_THAT(preconnector.result(), Optional(IsOk()));
    ASSERT_EQ(group.IdleStreamSocketCount(), 0u);
    ASSERT_TRUE(spdy_session_pool()->HasAvailableSession(stream_key.CalculateSpdySessionKey(), false));
}
...
TEST_F(HttpStreamPoolAttemptManagerTest, PreconnectRequireHttp1)
{
    ...
    ASSERT_FALSE(spdy_session_pool()->HasAvailableSession(stream_key.CalculateSpdySessionKey(), false));
}
...
TEST_F(HttpStreamPoolAttemptManagerTest, PreconnectMultipleStreamsOkAndFail)
{
    ...
    std::vector<MockConnect> connects = { { MockConnect(ASYNC, OK), MockConnect(ASYNC, ERR_FAILED) } };
    std::vector<std::unique_ptr<SequencedSocketData>> datas;
...
TEST_F(HttpStreamPoolAttemptManagerTest, PreconnectMultipleStreamsFailAndOk)
{
    ...
    std::vector<MockConnect> connects = { { MockConnect(ASYNC, ERR_FAILED), MockConnect(ASYNC, OK) } };
    std::vector<std::unique_ptr<SequencedSocketData>> datas;
    for (const auto& connect : connects) {
        auto data = std::make_unique<SequencedSocketData>();
        data->set_connect_data(connect);
...
TEST_F(HttpStreamPoolAttemptManagerTest, PreconnectMultipleRequests)
{
    ...
    std::vector<MockConnect> connects = { { MockConnect(&completers[0]), MockConnect(&completers[1]) } };
    std::vector<std::unique_ptr<SequencedSocketData>> datas;
    for (const auto& connect : connects) {
        auto data = std::make_unique<SequencedSocketData>();
        data->set_connect_data(connect);
...
TEST_F(HttpStreamPoolAttemptManagerTest, PreconnectReachedGroupLimit)
{
    ...
    RunUntilIdle();
    Group& group = pool().GetOrCreateGroupForTesting(preconnector.GetStreamKey());
    EXPECT_THAT(preconnector.result(), Optional(IsError(ERR_PRECONNECT_MAX_SOCKET_LIMIT)));
    ASSERT_EQ(group.IdleStreamSocketCount(), 1u);
}
...
TEST_F(HttpStreamPoolAttemptManagerTest, PreconnectReachedPoolLimit)
{
    ...
    pool().GetOrCreateGroupForTesting(key_a).CreateTextBasedStream(
        std::make_unique<FakeStreamSocket>(), StreamSocketHandle::SocketReuseType::kUnused, LoadTimingInfo::ConnectTiming());
    ...
    endpoint_request->add_endpoint(ServiceEndpointBuilder().add_v4("192.0.2.1").endpoint()).CallOnServiceEndpointRequestFinished(OK);
    RunUntilIdle();
    Group& group_b = pool().GetOrCreateGroupForTesting(preconnector_b.GetStreamKey());
    EXPECT_THAT(preconnector_b.result(), Optional(IsError(ERR_PRECONNECT_MAX_SOCKET_LIMIT)));
    ASSERT_EQ(group_b.IdleStreamSocketCount(), 1u);
}

TEST_F(HttpStreamPoolAttemptManagerTest, RequestStreamAndPreconnectWhileFailing)
{
    constexpr std::string_view kDestination = "http://a.test";
    ...
    for (size_t i = 0; i < 2; ++i) {
        FakeServiceEndpointRequest* endpoint_request = resolver()->AddFakeRequest();
        endpoint_request->add_endpoint(ServiceEndpointBuilder().add_v4("192.0.2.1").endpoint()).set_start_result(OK);
    }
...
TEST_F(HttpStreamPoolAttemptManagerTest, ReleaseStreamWhileFailing)
{
    ...
    stream1->InitializeStream(/*can_send_early=*/false, RequestPriority::IDLE, NetLogWithSource(), base::DoNothing());
    ...
    requester1.ResetRequest();
    requester2.ResetRequest();
    ASSERT_FALSE(pool().GetOrCreateGroupForTesting(stream_key).GetAttemptManagerForTesting());
}
...
TEST_F(HttpStreamPoolAttemptManagerTest, PreconnectPriority)
{
    ...
    int rv = preconnector.Preconnect(pool());
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
    EXPECT_EQ(pool().GetOrCreateGroupForTesting(preconnector.GetStreamKey()).GetAttemptManagerForTesting()->GetPriority(), RequestPriority::IDLE);
}
...
TEST_F(HttpStreamPoolAttemptManagerTest, FailingIsNotStalled)
{
    ...
    resolver()->AddFakeRequest()->add_endpoint(ServiceEndpointBuilder().add_v4("192.0.2.2").endpoint()).CompleteStartSynchronously(OK);
    ...
    EXPECT_FALSE(pool().GetOrCreateGroupForTesting(requester_a.GetStreamKey()).GetAttemptManagerForTesting()->IsStalledByPoolLimit());
}
...
TEST_F(HttpStreamPoolAttemptManagerTest, HavingSpdySessionIsNotStalled)
{
    ...
    EXPECT_FALSE(pool().GetOrCreateGroupForTesting(requester.GetStreamKey()).GetAttemptManagerForTesting()->IsStalledByPoolLimit());
}
...
TEST_F(HttpStreamPoolAttemptManagerTest, HavingQuicSessionIsNotStalled)
{
    ...
    StreamRequester requester;
    requester.set_destination(kDefaultDestination).set_quic_version(quic_version()).RequestStream(pool());
    RunUntilIdle();
    EXPECT_THAT(requester.result(), Optional(IsOk()));
...
TEST_F(HttpStreamPoolAttemptManagerTest, ReuseTypeUnusedIdle)
{
    ...
    RunUntilIdle();
    EXPECT_THAT(preconnector.result(), Optional(IsOk()));
    ASSERT_EQ(pool().GetOrCreateGroupForTesting(preconnector.GetStreamKey()).IdleStreamSocketCount(), 1u);
...
TEST_F(HttpStreamPoolAttemptManagerTest, QuicOk)
{
    ...
    EXPECT_THAT(requester.result(), Optional(IsOk()));
    EXPECT_THAT(pool().GetOrCreateGroupForTesting(requester.GetStreamKey()).GetAttemptManagerForTesting()->GetQuicTaskResultForTesting(), Optional(IsOk()));
...
TEST_F(HttpStreamPoolAttemptManagerTest, QuicOkDnsAlpn)
{
    ...
    endpoint_request->add_endpoint(ServiceEndpointBuilder().add_v4("192.0.2.1").set_alpns({ "h3", "h2" }).endpoint()).CallOnServiceEndpointRequestFinished(OK);
    RunUntilIdle();

    EXPECT_THAT(requester1.result(), Optional(IsOk()));
    EXPECT_THAT(requester2.result(), Optional(IsOk()));
    EXPECT_THAT(pool().GetOrCreateGroupForTesting(requester1.GetStreamKey()).GetAttemptManagerForTesting()->GetQuicTaskResultForTesting(), Optional(IsOk()));
}
...
TEST_F(HttpStreamPoolAttemptManagerTest, QuicBroken)
{
    ...
    http_server_properties()->MarkAlternativeServiceBroken(alternative_service, NetworkAnonymizationKey());
...
TEST_F(HttpStreamPoolAttemptManagerTest, QuicFailBeforeTls)
{
    ...
    EXPECT_THAT(pool().GetOrCreateGroupForTesting(requester.GetStreamKey()).GetAttemptManagerForTesting()->GetQuicTaskResultForTesting(),
    ...
    const AlternativeService alternative_service(NextProto::kProtoQUIC, HostPortPair::FromSchemeHostPort(requester.GetStreamKey().destination()));
    EXPECT_FALSE(http_server_properties()->IsAlternativeServiceBroken(alternative_service, NetworkAnonymizationKey()));
}
...
TEST_F(HttpStreamPoolAttemptManagerTest, QuicFailNonBrokenErrors)
{
    ...
    for (const int net_error : kErrors) {
        ...
        const AlternativeService alternative_service(NextProto::kProtoQUIC, HostPortPair::FromSchemeHostPort(requester.GetStreamKey().destination()));
        EXPECT_FALSE(http_server_properties()->IsAlternativeServiceBroken(alternative_service, NetworkAnonymizationKey())) << ErrorToString(net_error);
    }
}
...
TEST_F(HttpStreamPoolAttemptManagerTest, QuicNetErrorDetails)
{
    ...
    crypto_client_stream_factory()->set_handshake_mode(MockCryptoClientStream::COLD_START);
    ...
    EXPECT_EQ(requester.net_error_details().quic_connection_error, quic::QUIC_PACKET_READ_ERROR);
}
...
TEST_F(HttpStreamPoolAttemptManagerTest, QuicCanUseExistingSession)
{
    ...
    StreamRequester requester1;
    requester1.set_destination(kDefaultDestination).set_quic_version(quic_version()).RequestStream(pool());
    ...
    // existing session.
    StreamRequester requester2;
    requester2.set_destination(kDefaultDestination).set_quic_version(quic_version()).RequestStream(pool());
    RunUntilIdle();
    EXPECT_THAT(requester2.result(), Optional(IsOk()));
...
{
    FakeServiceEndpointRequest* endpoint_request = resolver()->AddFakeRequest();
    endpoint_request->add_endpoint(ServiceEndpointBuilder().add_v4("192.0.2.1").endpoint()).CompleteStartSynchronously(OK);
    ...
    StreamRequester requester;
    requester.set_destination(kDefaultDestination).set_enable_alternative_services(false).RequestStream(pool());
    RunUntilIdle();

    EXPECT_THAT(requester.result(), Optional(IsOk()));
    ASSERT_FALSE(pool().GetOrCreateGroupForTesting(requester.GetStreamKey()).GetAttemptManagerForTesting()->GetQuicTaskResultForTesting().has_value());
}

TEST_F(HttpStreamPoolAttemptManagerTest, AlternativeSerivcesDisabledQuicSessionExists)
{
    base::test::ScopedFeatureList feature_list;
    feature_list.InitAndDisableFeature(net::features::kAsyncQuicSession);
    ...
    StreamRequester requester2;
    requester2.set_destination(kDefaultDestination).set_enable_alternative_services(false).RequestStream(pool());
    requester2.WaitForResult();
    EXPECT_THAT(requester2.result(), Optional(IsOk()));
...
TEST_F(HttpStreamPoolAttemptManagerTest, QuicEndpointNotFoundNoDnsAlpn)
{
    ...
    StreamRequester requester;
    requester.set_destination(kDefaultDestination).set_quic_version(quic::ParsedQuicVersion::Unsupported()).RequestStream(pool());
    RunUntilIdle();
...
TEST_F(HttpStreamPoolAttemptManagerTest, QuicPreconnect)
{
    ...
    Preconnector preconnector1(kDefaultDestination);
    preconnector1.set_num_streams(2).set_quic_version(quic_version()).Preconnect(pool());
    RunUntilIdle();
    EXPECT_THAT(preconnector1.result(), Optional(IsOk()));
    ...
    Preconnector preconnector2(kDefaultDestination);
    int rv = preconnector2.set_num_streams(1).set_quic_version(quic_version()).Preconnect(pool());
    RunUntilIdle();
    EXPECT_THAT(rv, IsOk());
}
...
TEST_F(HttpStreamPoolAttemptManagerTest, QuicMatchingIpSession)
{
    ...
    FakeServiceEndpointRequest* endpoint_request1 = resolver()->AddFakeRequest();
    endpoint_request1->add_endpoint(ServiceEndpointBuilder().add_ip_endpoint(kCommonEndPoint).endpoint()).CompleteStartSynchronously(OK);
    ...
    FakeServiceEndpointRequest* endpoint_request2 = resolver()->AddFakeRequest();
    endpoint_request2->add_endpoint(ServiceEndpointBuilder().add_ip_endpoint(kCommonEndPoint).endpoint()).CompleteStartSynchronously(OK);

    StreamRequester requester2;
    requester2.set_destination(kAltDestination).set_quic_version(quic_version()).RequestStream(pool());
    RunUntilIdle();
    EXPECT_THAT(requester2.result(), Optional(IsOk()));
    QuicSessionAliasKey quic_key1 = requester1.GetStreamKey().CalculateQuicSessionAliasKey();
    QuicSessionAliasKey quic_key2 = requester2.GetStreamKey().CalculateQuicSessionAliasKey();
    ASSERT_EQ(quic_session_pool()->FindExistingSession(quic_key1.session_key(), quic_key1.destination()),
        quic_session_pool()->FindExistingSession(quic_key2.session_key(), quic_key2.destination()));
}
...
TEST_F(HttpStreamPoolAttemptManagerTest, QuicMatchingIpSessionOnEndpointsUpdated)
{
    base::test::ScopedFeatureList feature_list;
    feature_list.InitAndEnableFeature(net::features::kAsyncQuicSession);
    ...
    StreamRequester requester2;
    requester2.set_destination(kAltDestination).set_quic_version(quic_version()).RequestStream(pool());
    endpoint_request2->add_endpoint(ServiceEndpointBuilder().add_v4("192.0.2.1").endpoint()).set_crypto_ready(true).CallOnServiceEndpointsUpdated();
    ASSERT_FALSE(requester2.result().has_value());

    endpoint_request2->add_endpoint(ServiceEndpointBuilder().add_ip_endpoint(kCommonEndPoint).endpoint()).CallOnServiceEndpointsUpdated();
    RunUntilIdle();
    EXPECT_THAT(requester2.result(), Optional(IsOk()));
    QuicSessionAliasKey quic_key1 = requester1.GetStreamKey().CalculateQuicSessionAliasKey();
    QuicSessionAliasKey quic_key2 = requester2.GetStreamKey().CalculateQuicSessionAliasKey();
    EXPECT_EQ(quic_session_pool()->FindExistingSession(quic_key1.session_key(), quic_key1.destination()),
        quic_session_pool()->FindExistingSession(quic_key2.session_key(), quic_key2.destination()));
}
...
TEST_F(HttpStreamPoolAttemptManagerTest, QuicPreconnectMatchingIpSession)
{
    ...
    EXPECT_THAT(preconnector2.result(), Optional(IsOk()));
    QuicSessionAliasKey quic_key1 = requester1.GetStreamKey().CalculateQuicSessionAliasKey();
    QuicSessionAliasKey quic_key2 = preconnector2.GetStreamKey().CalculateQuicSessionAliasKey();
    EXPECT_EQ(quic_session_pool()->FindExistingSession(quic_key1.session_key(), quic_key1.destination()),
        quic_session_pool()->FindExistingSession(quic_key2.session_key(), quic_key2.destination()));
}
...
TEST_F(HttpStreamPoolAttemptManagerTest, QuicMatchingIpSessionDisabled)
{
    ...
    StreamRequester requester;
    requester.set_destination(kDefaultDestination).set_enable_ip_based_pooling(false).RequestStream(pool());
    RunUntilIdle();
...
TEST_F(HttpStreamPoolAttemptManagerTest, DelayStreamAttemptQuicFail)
{
    ...
    const AlternativeService alternative_service(NextProto::kProtoQUIC, HostPortPair::FromSchemeHostPort(requester.GetStreamKey().destination()));
    EXPECT_TRUE(http_server_properties()->IsAlternativeServiceBroken(alternative_service, NetworkAnonymizationKey()));
}
...
TEST_F(HttpStreamPoolAttemptManagerTest, DelayStreamAttemptDisableAlternativeServicesLater)
{
    base::test::ScopedFeatureList feature_list;
    feature_list.InitAndEnableFeature(net::features::kAsyncQuicSession);
...
{
    origins_to_force_quic_on().insert(HostPortPair::FromURL(GURL(kDefaultDestination)));
    InitializeSession();
...
TEST_F(HttpStreamPoolAttemptManagerTest, QuicSessionGoneBeforeUsing)
{
    ...
    request_info.traffic_annotation = MutableNetworkTrafficAnnotationTag(TRAFFIC_ANNOTATION_FOR_TESTS);
    stream->RegisterRequest(&request_info);
    int rv = stream->InitializeStream(/*can_send_early=*/false, RequestPriority::IDLE, NetLogWithSource(), base::DoNothing());
...
TEST_F(HttpStreamPoolAttemptManagerTest, GetInfoAsValue)
{
    ...
    EXPECT_THAT(info.FindInt("max_socket_count"), Optional(pool().max_stream_sockets_per_pool()));
    EXPECT_THAT(info.FindInt("max_sockets_per_group"), Optional(pool().max_stream_sockets_per_group()));

    base::Value::Dict* groups_info = info.FindDict("groups");
    ASSERT_TRUE(groups_info);

    base::Value::Dict* info_a = groups_info->FindDict(requester_a.GetStreamKey().ToString());
    ASSERT_TRUE(info_a);
    EXPECT_THAT(info_a->FindInt("active_socket_count"), Optional(1));
    EXPECT_THAT(info_a->FindInt("idle_socket_count"), Optional(1));

    base::Value::Dict* info_b = groups_info->FindDict(requester_b.GetStreamKey().ToString());
    ASSERT_TRUE(info_b);
...
{
    const url::SchemeHostPort kOrigin(url::kHttpsScheme, "origin.example.org", 443);
    const HostPortPair kAlternative("alt.example.org", 443);

    const AlternativeService alternative_service(NextProto::kProtoHTTP2, kAlternative);
    const base::Time expiration = base::Time::Now() + base::Days(1);

    StreamRequester requester;
    requester.set_destination(kOrigin).set_alternative_service_info(AlternativeServiceInfo::CreateHttp2AlternativeServiceInfo(alternative_service, expiration));
...
TEST_F(HttpStreamPoolAttemptManagerTest, AltSvcNegotiatedWithH1)
{
    ...
    requester.RequestStream(pool());
    requester.WaitForResult();
    EXPECT_THAT(requester.result(), Optional(IsError(ERR_ALPN_NEGOTIATION_FAILED)));
    requester.ResetRequest();
...
TEST_F(HttpStreamPoolAttemptManagerTest, AltSvcSetPriority)
{
    ...
    HttpStreamRequest* request = requester.set_priority(RequestPriority::LOW).RequestStream(pool());

    AttemptManager* origin_manager = pool().GetOrCreateGroupForTesting(requester.GetStreamKey()).GetAttemptManagerForTesting();
    ASSERT_TRUE(origin_manager);
    EXPECT_EQ(origin_manager->GetPriority(), RequestPriority::LOW);

    HttpStreamKey alt_stream_key = StreamKeyBuilder().set_destination(url::SchemeHostPort(url::kHttpsScheme, kAlternative.host(), kAlternative.port())).Build();
    AttemptManager* alt_manager = pool().GetOrCreateGroupForTesting(alt_stream_key).GetAttemptManagerForTesting();
...
TEST_F(HttpStreamPoolAttemptManagerTest, UnsafePort)
{
    ...
    const url::SchemeHostPort destination = requester.GetStreamKey().destination();
    ASSERT_FALSE(IsPortAllowedForScheme(destination.port(), destination.scheme()));
...
TEST_F(HttpStreamPoolAttemptManagerTest, PreconnectUnsafePort)
{
    ...
    const url::SchemeHostPort destination = preconnector.GetStreamKey().destination();
    ASSERT_FALSE(IsPortAllowedForScheme(destination.port(), destination.scheme()));
...
TEST_F(HttpStreamPoolAttemptManagerTest, ReportBadProxyAfterSuccessOnDirect)
{
    ...
    auto proxy_chain = ProxyChain::FromSchemeHostAndPort(ProxyServer::Scheme::SCHEME_HTTP, "badproxy", 80);
    const ProxyRetryInfoMap retry_info_map = http_network_session()->proxy_resolution_service()->proxy_retry_info();
...
{
    const auto kIpProtectionDirectChain = ProxyChain::ForIpProtection(std::vector<ProxyServer>());
    ProxyInfo proxy_info;
    ...
    requester.WaitForResult();
    EXPECT_THAT(requester.result(), Optional(IsOk()));
    EXPECT_EQ(requester.used_proxy_info().ToDebugString(), proxy_info.ToDebugString());
}
...
TEST_F(HttpStreamPoolAttemptManagerTest, DestroyHttpNetworkSessionWithSpdySession)
{
    ...
    CreateFakeSpdySession(StreamKeyBuilder().set_destination("https://a.test").Build());
...
TEST_F(HttpStreamPoolAttemptManagerTest, AsyncQuicSessionDestroyRequestBeforeSessionCreation)
{
    base::test::ScopedFeatureList feature_list;
    feature_list.InitAndEnableFeature(net::features::kAsyncQuicSession);
    ...
    resolver()->AddFakeRequest()->add_endpoint(ServiceEndpointBuilder().add_ip_endpoint(kCommonEndPoint).endpoint()).CompleteStartSynchronously(OK);
    ...
    endpoint_request->set_crypto_ready(true).add_endpoint(ServiceEndpointBuilder().add_v4("192.0.2.1").endpoint()).CallOnServiceEndpointsUpdated();
    ASSERT_FALSE(requester2.result().has_value());
    ...
    endpoint_request->set_crypto_ready(true).add_endpoint(ServiceEndpointBuilder().add_ip_endpoint(kCommonEndPoint).endpoint()).CallOnServiceEndpointsUpdated();
...
TEST_F(HttpStreamPoolAttemptManagerTest, EchOk)
{
    ...
    ASSERT_TRUE(MakeTestEchKeys("www.example.org", /*max_name_len=*/128, &ech_config_list));
    ...
    resolver()
        ->AddFakeRequest()
        ->add_endpoint(ServiceEndpointBuilder().add_v4("192.0.2.1").set_alpns({ "http/1.1" }).set_ech_config_list(ech_config_list).endpoint())
...
TEST_F(HttpStreamPoolAttemptManagerTest, EchSvcbOptional)
{
    ...
    resolver()
        ->AddFakeRequest()
        ->add_endpoint(ServiceEndpointBuilder().add_v4("192.0.2.1").set_alpns({ "http/1.1" }).set_ech_config_list(ech_config_list).endpoint())
        .add_endpoint(ServiceEndpointBuilder().add_v4("192.0.2.2").set_alpns({ "http/1.1" }).endpoint())
        .CompleteStartSynchronously(OK);
...
TEST_F(HttpStreamPoolAttemptManagerTest, EchSvcbReliant)
{
    ...
    resolver()
        ->AddFakeRequest()
        ->add_endpoint(ServiceEndpointBuilder().add_v4("192.0.2.1").set_alpns({ "h3" }).set_ech_config_list(ech_config_list).endpoint())
        .add_endpoint(ServiceEndpointBuilder().add_v4("192.0.2.2").set_alpns({ "http/1.1" }).set_ech_config_list(ech_config_list).endpoint())
        .CompleteStartSynchronously(OK);
...
TEST_F(HttpStreamPoolAttemptManagerTest, EchSvcbReliantQuicOnlyAbortTcpAttempt)
{
    std::vector<uint8_t> ech_config_list;
    ASSERT_TRUE(MakeTestEchKeys("www.example.org", /*max_name_len=*/128, &ech_config_list));
    ...
    endpoint_request->set_endpoints({ ServiceEndpointBuilder().add_v4("192.0.2.1").endpoint() }).CallOnServiceEndpointsUpdated();
    Group& group = pool().GetOrCreateGroupForTesting(requester.GetStreamKey());
    ...
    endpoint_request->set_endpoints({ ServiceEndpointBuilder().add_v4("192.0.2.1").set_alpns({ "h3" }).set_ech_config_list(ech_config_list).endpoint() })
        .set_crypto_ready(true)
        .CallOnServiceEndpointRequestFinished(OK);
...
TEST_F(HttpStreamPoolAttemptManagerTest, JobAllowH2OnlyCancelQuicAttempt)
{
    ...
    EXPECT_THAT(pool().GetOrCreateGroupForTesting(stream_key).GetAttemptManagerForTesting()->GetQuicTaskResultForTesting(), Optional(IsError(ERR_ABORTED)));
}
