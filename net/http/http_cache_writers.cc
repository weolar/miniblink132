...
bool IsValidResponseForWriter(bool is_partial, const HttpResponseInfo* response_info)
{
    ...
    if (!is_partial && (response_info->headers->response_code() != HTTP_OK && response_info->headers->response_code() != HTTP_NOT_MODIFIED)) {
        return false;
    }
...
HttpCache::Writers::TransactionInfo::TransactionInfo(PartialData* partial_data, const bool is_truncated, HttpResponseInfo info)
...
HttpCache::Writers::TransactionInfo::TransactionInfo(const TransactionInfo&) = default;

HttpCache::Writers::Writers(HttpCache* cache, scoped_refptr<HttpCache::ActiveEntry> entry)
    : cache_(cache)
    , entry_(entry)
{
    DCHECK(cache_);
...
int HttpCache::Writers::Read(scoped_refptr<IOBuffer> buf, int buf_len, CompletionOnceCallback callback, Transaction* transaction)
{
    DCHECK(buf);
    DCHECK_GT(buf_len, 0);
...
void HttpCache::Writers::AddTransaction(
    Transaction* transaction, ParallelWritingPattern initial_writing_pattern, RequestPriority priority, const TransactionInfo& info)
{
    DCHECK(transaction);
    ...
    should_keep_entry_ = IsValidResponseForWriter(info.partial != nullptr, &(info.response_info));
...
void HttpCache::Writers::SetNetworkTransaction(Transaction* transaction, std::unique_ptr<HttpTransaction> network_transaction)
{
...
void HttpCache::Writers::RemoveTransaction(Transaction* transaction, bool success)
{
    ...
    // Destroys `this`.
    cache_->WritersDoneWritingToEntry(entry_, success, should_keep_entry_, TransactionSet());
}

void HttpCache::Writers::EraseTransaction(Transaction* transaction, int result)
{
...
HttpCache::Writers::TransactionMap::iterator HttpCache::Writers::EraseTransaction(TransactionMap::iterator it, int result)
{
...
void HttpCache::Writers::TruncateEntry()
{
    ...
    response_info_truncation_.Persist(data->pickle(), true /* skip_transient_headers*/, true /* response_truncated */);
    data->Done();
    io_buf_len_ = data->pickle()->size();
    entry_->GetEntry()->WriteData(kResponseInfoIndex, 0, data.get(), io_buf_len_, base::DoNothing(), true);
}
...
bool HttpCache::Writers::ShouldTruncate()
{
    ...
    if (response_info_truncation_.headers->GetContentLength() <= 0 || response_info_truncation_.headers->HasHeaderValue("Accept-Ranges", "none")
    ...
    int64_t content_length = response_info_truncation_.headers->GetContentLength();
...
HttpCache::Writers::WaitingForRead::WaitingForRead(scoped_refptr<IOBuffer> buf, int len, CompletionOnceCallback consumer_callback)
...
int HttpCache::Writers::DoNetworkRead()
{
    ...
    CompletionOnceCallback io_callback = base::BindOnce(&HttpCache::Writers::OnIOComplete, weak_factory_.GetWeakPtr());
    return network_transaction_->Read(read_buf_.get(), io_buf_len_, std::move(io_callback));
}
...
int HttpCache::Writers::DoCacheWriteData(int num_bytes)
{
    ...
    if (!partial) {
        ...
        rv = entry_->GetEntry()->WriteData(kResponseContentIndex, current_size, read_buf_.get(), num_bytes, std::move(io_callback), true);
    } else {
        rv = partial->CacheWrite(entry_->GetEntry(), read_buf_.get(), num_bytes, std::move(io_callback));
    }
    return rv;
}
...
int HttpCache::Writers::DoCacheWriteDataComplete(int result)
{
    ...
    if (!last_disk_cache_access_start_time_.is_null() && active_transaction_ && !all_writers_.find(active_transaction_)->second.partial) {
        active_transaction_->AddDiskCacheWriteTime(base::TimeTicks::Now() - last_disk_cache_access_start_time_);
...
void HttpCache::Writers::OnDataReceived(int result)
{
    ...
    bool is_partial = active_transaction_ != nullptr && it->second.partial != nullptr;
    ...
    if (result == 0) {
        ...
        int current_size = entry_->GetEntry()->GetDataSize(kResponseContentIndex);
        DCHECK(network_transaction_);
        const HttpResponseInfo* response_info = network_transaction_->GetResponseInfo();
...
void HttpCache::Writers::CompleteWaitingForReadTransactions(int result)
{
    for (auto it = waiting_for_read_.begin(); it != waiting_for_read_.end();) {
        ...
        if (result >= 0) { // success
            ...
            memcpy(it->second.read_buf->data(), read_buf_->data(), it->second.write_len);
            callback_result = it->second.write_len;
        }

        // Post task to notify transaction.
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(std::move(it->second.callback), callback_result));
...
void HttpCache::Writers::SetCacheCallback(bool success, const TransactionSet& make_readers)
{
    DCHECK(!cache_callback_);
    cache_callback_ = base::BindOnce(&HttpCache::WritersDoneWritingToEntry, cache_->GetWeakPtr(), entry_, success, should_keep_entry_, make_readers);
}
