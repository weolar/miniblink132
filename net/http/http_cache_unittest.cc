...
class DeleteCacheCompletionCallback : public TestGetBackendCompletionCallbackBase {
public:
    ...
    DeleteCacheCompletionCallback& operator=(const DeleteCacheCompletionCallback&) = delete;

    HttpCache::GetBackendCallback callback()
    {
        return base::BindOnce(&DeleteCacheCompletionCallback::OnComplete, base::Unretained(this));
    }
...
void ReadAndVerifyTransaction(HttpTransaction* trans, const MockTransaction& trans_info)
{
    std::string content;
...
void ReadRemainingAndVerifyTransaction(HttpTransaction* trans, const std::string& already_read, const MockTransaction& trans_info)
{
    std::string content;
    int rv = ReadTransaction(trans, &content);
    EXPECT_THAT(rv, IsOk());
...
void RunTransactionTestBase(HttpCache* cache, const MockTransaction& trans_info, const MockHttpRequest& request, HttpResponseInfo* response_info,
    const NetLogWithSource& net_log, LoadTimingInfo* load_timing_info, int64_t* sent_bytes, int64_t* received_bytes, IPEndPoint* remote_endpoint)
{
...
void RunTransactionTestWithRequest(HttpCache* cache, const MockTransaction& trans_info, const MockHttpRequest& request, HttpResponseInfo* response_info)
{
    RunTransactionTestBase(cache, trans_info, request, response_info, NetLogWithSource(), nullptr, nullptr, nullptr, nullptr);
}

void RunTransactionTestAndGetTiming(HttpCache* cache, const MockTransaction& trans_info, const NetLogWithSource& log, LoadTimingInfo* load_timing_info)
{
    RunTransactionTestBase(cache, trans_info, MockHttpRequest(trans_info), nullptr, log, load_timing_info, nullptr, nullptr, nullptr);
}

void RunTransactionTestAndGetTimingAndConnectedSocketAddress(
    HttpCache* cache, const MockTransaction& trans_info, const NetLogWithSource& log, LoadTimingInfo* load_timing_info, IPEndPoint* remote_endpoint)
{
    RunTransactionTestBase(cache, trans_info, MockHttpRequest(trans_info), nullptr, log, load_timing_info, nullptr, nullptr, remote_endpoint);
}

void RunTransactionTest(HttpCache* cache, const MockTransaction& trans_info)
{
    RunTransactionTestAndGetTiming(cache, trans_info, NetLogWithSource(), nullptr);
}

void RunTransactionTestWithLog(HttpCache* cache, const MockTransaction& trans_info, const NetLogWithSource& log)
{
    RunTransactionTestAndGetTiming(cache, trans_info, log, nullptr);
}

void RunTransactionTestWithResponseInfo(HttpCache* cache, const MockTransaction& trans_info, HttpResponseInfo* response)
{
    RunTransactionTestWithRequest(cache, trans_info, MockHttpRequest(trans_info), response);
}
...
{
    RunTransactionTestBase(cache, trans_info, MockHttpRequest(trans_info), response, log, load_timing_info, nullptr, nullptr, nullptr);
}

void RunTransactionTestWithResponse(HttpCache* cache, const MockTransaction& trans_info, std::string* response_headers)
{
...
void RunTransactionTestWithResponseAndGetTiming(
    HttpCache* cache, const MockTransaction& trans_info, std::string* response_headers, const NetLogWithSource& log, LoadTimingInfo* load_timing_info)
{
    HttpResponseInfo response;
    RunTransactionTestBase(cache, trans_info, MockHttpRequest(trans_info), &response, log, load_timing_info, nullptr, nullptr, nullptr);
    *response_headers = ToSimpleString(response.headers);
...
public:
    ...
    void set_no_store(bool value)
    {
        no_store = value;
    }

    static void FastNoStoreHandler(const HttpRequestInfo* request, std::string* response_status, std::string* response_headers, std::string* response_data)
    {
        if (no_store) {
...
public:
    ...
    static void RangeHandler(const HttpRequestInfo* request, std::string* response_status, std::string* response_headers, std::string* response_data);
...
// Static.
void RangeTransactionServer::RangeHandler(
    const HttpRequestInfo* request, std::string* response_status, std::string* response_headers, std::string* response_data)
{
    ...
    std::optional<std::string> range_header = request->extra_headers.GetHeader(HttpRequestHeaders::kRange);
    if (!range_header || !HttpUtil::ParseRangeHeader(*range_header, &ranges) || bad_200_ || ranges.size() != 1
    ...
    std::string content_range = base::StringPrintf("Content-Range: bytes %" PRId64 "-%" PRId64 "/%" PRId64 "\n", start, end, length_);
    ...
    if (!request->extra_headers.HasHeader("If-None-Match") || modified_) {
        ...
        if (end == start) {
            ...
            for (int64_t block_start = start; block_start < end; block_start += 10) {
                base::StringAppendF(&data, "rg: %02" PRId64 "-%02" PRId64 " ", block_start % 100, (block_start + 9) % 100);
            }
        }
        *response_data = data;
        ...
        if (end - start != 9) {
            ...
            std::string content_length = base::StringPrintf("Content-Length: %" PRId64 "\n", len);
            response_headers->replace(response_headers->find("Content-Length:"), content_length.size(), content_length);
        }
    } else {
        response_status->assign("HTTP/1.1 304 Not Modified");
        response_data->clear();
...
{
    auto headers = base::MakeRefCounted<HttpResponseHeaders>(HttpUtil::AssembleRawHeaders(response));

    ASSERT_EQ(206, headers->response_code());

    int64_t range_start, range_end, object_size;
    ASSERT_TRUE(headers->GetContentRangeFor206(&range_start, &range_end, &object_size));
...
void CreateTruncatedEntry(std::string raw_headers, MockHttpCache* cache)
{
    ...
    response.headers = base::MakeRefCounted<HttpResponseHeaders>(HttpUtil::AssembleRawHeaders(raw_headers));
    ...
    int len = static_cast<int>(base::strlcpy(buf->data(), "rg: 00-09 rg: 10-19 ", 100));
...
void VerifyTruncatedFlag(MockHttpCache* cache, const std::string& key, bool flag_value, int data_size)
{
    disk_cache::Entry* entry;
...
class FakeWebSocketHandshakeStreamCreateHelper : public WebSocketHandshakeStreamBase::CreateHelper {
public:
    ~FakeWebSocketHandshakeStreamCreateHelper() override = default;
    std::unique_ptr<WebSocketHandshakeStreamBase> CreateBasicStream(
        std::unique_ptr<ClientSocketHandle> connect, bool using_proxy, WebSocketEndpointLockManager* websocket_endpoint_lock_manager) override
    {
        return nullptr;
    }
...
// these tests.
std::vector<NetLogEntry> GetFilteredNetLogEntries(const RecordingNetLogObserver& net_log_observer)
{
...
bool LogContainsEventType(const RecordingNetLogObserver& net_log_observer, NetLogEventType expected)
{
...
public:
    ...
    int OpenEntry(HttpCache* cache, const std::string& url, scoped_refptr<ActiveEntry>* entry, HttpCache::Transaction* trans)
    {
        return cache->OpenEntry(GenerateCacheKey(url), entry, trans);
    }

    int OpenOrCreateEntry(HttpCache* cache, const std::string& url, scoped_refptr<ActiveEntry>* entry, HttpCache::Transaction* trans)
    {
        return cache->OpenOrCreateEntry(GenerateCacheKey(url), entry, trans);
    }

    int CreateEntry(HttpCache* cache, const std::string& url, scoped_refptr<ActiveEntry>* entry, HttpCache::Transaction* trans)
    {
        return cache->CreateEntry(GenerateCacheKey(url), entry, trans);
    }

    int DoomEntry(HttpCache* cache, const std::string& url, HttpCache::Transaction* trans)
    {
...
public:
    ...
    std::string ComputeCacheKey(const std::string& url_string)
    {
        ...
        request_info.network_anonymization_key = NetworkAnonymizationKey::CreateSameSite(site);
        MockHttpCache cache;
...
TEST_F(HttpCacheTest, GetBackend)
{
    ...
    HttpCache::GetBackendResult result = cache.http_cache()->GetBackend(cb.callback());
...
TEST_F(HttpCacheSimpleGetTest, Basic)
{
    ...
    // Write to the cache.
    RunTransactionTestAndGetTiming(cache.http_cache(), kSimpleGET_Transaction, NetLogWithSource::Make(NetLogSourceType::NONE), &load_timing_info);
...
TEST_F(HttpCacheSimpleGetTest, ConnectedCallback)
{
    ...
    TestCompletionCallback callback;
    ASSERT_THAT(transaction->Start(&request, callback.callback(), NetLogWithSource()), IsError(ERR_IO_PENDING));
...
TEST_F(HttpCacheSimpleGetTest, ConnectedCallbackOnCacheHit)
{
    ...
    EXPECT_THAT(connected_handler.transports(), ElementsAre(CachedTestTransportInfo()));
}
...
TEST_F(HttpCacheSimpleGetTest, ConnectedCallbackOnCacheHitReturnInconsistentIpError)
{
    MockHttpCache cache;
...

    {
        ...
        TestCompletionCallback callback;
        ASSERT_THAT(transaction->Start(&request, callback.callback(), NetLogWithSource()), IsError(ERR_IO_PENDING));
        EXPECT_THAT(callback.WaitForResult(), IsError(ERR_INCONSISTENT_IP_ADDRESS_SPACE));
...
TEST_F(HttpCacheSimpleGetTest, ConnectedCallbackOnCacheHitReturnPrivateNetworkAccessBlockedError)
{
    MockHttpCache cache;
...

    {
        ...
        // connected callback error.
        ConnectedHandler connected_handler;
        connected_handler.set_result(ERR_CACHED_IP_ADDRESS_SPACE_BLOCKED_BY_PRIVATE_NETWORK_ACCESS_POLICY);
        ...
        // Used the cache entry only.
        EXPECT_THAT(connected_handler.transports(), ElementsAre(CachedTestTransportInfo(), TestTransportInfo()));
    }
...
TEST_F(HttpCacheSimpleGetTest, ConnectedCallbackOnCacheHitFromProxy)
{
    ...
    EXPECT_THAT(connected_handler.transports(), ElementsAre(expected_transport_info));
}
...
{
    {
        ...
        feature_list.InitWithFeatures({},
            { net::features::kSplitCacheByCrossSiteMainFrameNavigationBoolean, net::features::kSplitCacheByMainFrameNavigationInitiator,
                net::features::kSplitCacheByNavigationInitiator });

        EXPECT_EQ(HttpCache::ExperimentMode::kStandard, HttpCache::GetExperimentMode());
    }
    {
        base::test::ScopedFeatureList feature_list;
        feature_list.InitWithFeatures({ net::features::kSplitCacheByCrossSiteMainFrameNavigationBoolean },
            { net::features::kSplitCacheByMainFrameNavigationInitiator, net::features::kSplitCacheByNavigationInitiator });

        EXPECT_EQ(HttpCache::ExperimentMode::kCrossSiteInitiatorBoolean, HttpCache::GetExperimentMode());
    }
    {
        base::test::ScopedFeatureList feature_list;
        feature_list.InitWithFeatures({ net::features::kSplitCacheByMainFrameNavigationInitiator },
            { net::features::kSplitCacheByCrossSiteMainFrameNavigationBoolean, net::features::kSplitCacheByNavigationInitiator });

        EXPECT_EQ(HttpCache::ExperimentMode::kMainFrameNavigationInitiator, HttpCache::GetExperimentMode());
    }
    {
        base::test::ScopedFeatureList feature_list;
        feature_list.InitWithFeatures({ net::features::kSplitCacheByNavigationInitiator },
            { net::features::kSplitCacheByCrossSiteMainFrameNavigationBoolean, net::features::kSplitCacheByMainFrameNavigationInitiator });

        EXPECT_EQ(HttpCache::ExperimentMode::kNavigationInitiator, HttpCache::GetExperimentMode());
    }
    {
        base::test::ScopedFeatureList feature_list;
        feature_list.InitWithFeatures(
    ...
    {
        base::test::ScopedFeatureList feature_list;
        feature_list.InitWithFeatures({ net::features::kSplitCacheByMainFrameNavigationInitiator, net::features::kSplitCacheByNavigationInitiator },
...
const struct {
    ...
    = { { SplitCacheTestCase::kEnabledTriplePlusCrossSiteMainFrameNavBool, net::features::kSplitCacheByCrossSiteMainFrameNavigationBoolean },
          { SplitCacheTestCase::kEnabledTriplePlusMainFrameNavInitiator, net::features::kSplitCacheByMainFrameNavigationInitiator },
          { SplitCacheTestCase::kEnabledTriplePlusNavInitiator, net::features::kSplitCacheByNavigationInitiator } };

class HttpCacheTestSplitCacheFeature : public HttpCacheTest, public ::testing::WithParamInterface<SplitCacheTestCase> {
public:
    HttpCacheTestSplitCacheFeature()
        : split_cache_experiment_feature_list_(GetParam(), kTestCaseToFeatureMapping)
    {
        if (IsSplitCacheEnabled()) {
            split_cache_enabled_feature_list_.InitAndEnableFeature(net::features::kSplitCacheByNetworkIsolationKey);
        } else {
            split_cache_enabled_feature_list_.InitAndDisableFeature(net::features::kSplitCacheByNetworkIsolationKey);
        }
    }
...
private:
    net::test::ScopedMutuallyExclusiveFeatureList split_cache_experiment_feature_list_;
...
TEST_P(HttpCacheTestSplitCacheFeature, SimpleGetVerifyGoogleFontMetrics)
{
    ...
    ScopedMockTransaction transaction(kSimpleGET_Transaction, "http://themes.googleusercontent.com/static/fonts/roboto");
    MockHttpRequest request(transaction);
    request.network_isolation_key = NetworkIsolationKey(site_a, site_a);
    request.network_anonymization_key = NetworkAnonymizationKey::CreateSameSite(site_a);

    // Attempt to populate the cache.
    RunTransactionTestWithRequest(cache.http_cache(), transaction, request, nullptr);

    RunTransactionTestWithRequest(cache.http_cache(), transaction, request, nullptr);
}

INSTANTIATE_TEST_SUITE_P(All, HttpCacheTestSplitCacheFeature,
    testing::ValuesIn({ SplitCacheTestCase::kDisabled, SplitCacheTestCase::kEnabledTripleKeyed, SplitCacheTestCase::kEnabledTriplePlusCrossSiteMainFrameNavBool,
        SplitCacheTestCase::kEnabledTriplePlusMainFrameNavInitiator, SplitCacheTestCase::kEnabledTriplePlusNavInitiator }),
...
public:
    ...
    {
        split_cache_always_enabled_feature_list_.InitAndEnableFeature(features::kSplitCacheByNetworkIsolationKey);
    }
...
TEST_F(HttpCacheSimpleGetTest, NoDiskCache)
{
    ...
    EXPECT_TRUE(LogContainsBeginEvent(entries, 0, NetLogEventType::HTTP_CACHE_GET_BACKEND));
    EXPECT_TRUE(LogContainsEndEvent(entries, 1, NetLogEventType::HTTP_CACHE_GET_BACKEND));
    EXPECT_TRUE(LogContainsBeginEvent(entries, 2, NetLogEventType::HTTP_CACHE_OPEN_OR_CREATE_ENTRY));
    EXPECT_TRUE(LogContainsEndEvent(entries, 3, NetLogEventType::HTTP_CACHE_OPEN_OR_CREATE_ENTRY));
...
TEST_F(HttpCacheSimpleGetTest, LoadOnlyFromCacheHit)
{
    ...
    // Write to the cache.
    RunTransactionTestAndGetTiming(cache.http_cache(), kSimpleGET_Transaction, net_log_with_source, &load_timing_info);
    ...
    EXPECT_TRUE(LogContainsBeginEvent(entries, 2, NetLogEventType::HTTP_CACHE_OPEN_OR_CREATE_ENTRY));
    EXPECT_TRUE(LogContainsEndEvent(entries, 3, NetLogEventType::HTTP_CACHE_OPEN_OR_CREATE_ENTRY));
    EXPECT_TRUE(LogContainsBeginEvent(entries, 4, NetLogEventType::HTTP_CACHE_ADD_TO_ENTRY));
    ...
    RunTransactionTestAndGetTiming(cache.http_cache(), transaction, net_log_with_source, &load_timing_info);
...
TEST_F(HttpCacheSimpleGetTest, LoadPreferringCacheVaryMismatch)
{
    ...
    LoadTimingInfo load_timing_info;
    RunTransactionTestAndGetTiming(cache.http_cache(), transaction, NetLogWithSource::Make(NetLogSourceType::NONE), &load_timing_info);
...
TEST_F(HttpCacheSimpleGetTest, NetworkAccessedNetwork)
{
    ...
    RunTransactionTestWithResponseInfo(cache.http_cache(), kSimpleGET_Transaction, &response_info);

    EXPECT_EQ(1, cache.network_layer()->transaction_count());
    EXPECT_EQ(0, cache.disk_cache()->open_count());
    EXPECT_EQ(1, cache.disk_cache()->create_count());
    EXPECT_TRUE(response_info.network_accessed);
    EXPECT_EQ(CacheEntryStatus::ENTRY_NOT_IN_CACHE, response_info.cache_entry_status);
}
...
TEST_F(HttpCacheSimpleGetTest, NetworkAccessedCache)
{
    ...
    HttpResponseInfo response_info;
    RunTransactionTestWithResponseInfo(cache.http_cache(), transaction, &response_info);
...
TEST_F(HttpCacheSimpleGetTest, LoadBypassCache)
{
    ...
    EXPECT_TRUE(LogContainsBeginEvent(entries, 0, NetLogEventType::HTTP_CACHE_GET_BACKEND));
    EXPECT_TRUE(LogContainsEndEvent(entries, 1, NetLogEventType::HTTP_CACHE_GET_BACKEND));
    EXPECT_TRUE(LogContainsBeginEvent(entries, 2, NetLogEventType::HTTP_CACHE_DOOM_ENTRY));
    EXPECT_TRUE(LogContainsEndEvent(entries, 3, NetLogEventType::HTTP_CACHE_DOOM_ENTRY));
    EXPECT_TRUE(LogContainsBeginEvent(entries, 4, NetLogEventType::HTTP_CACHE_CREATE_ENTRY));
    EXPECT_TRUE(LogContainsEndEvent(entries, 5, NetLogEventType::HTTP_CACHE_CREATE_ENTRY));
    EXPECT_TRUE(LogContainsBeginEvent(entries, 6, NetLogEventType::HTTP_CACHE_ADD_TO_ENTRY));
...
TEST_F(HttpCacheSimpleGetTest, LoadValidateCache)
{
    ...
    RunTransactionTestWithResponseInfoAndGetTiming(
        cache.http_cache(), transaction, &response_info, NetLogWithSource::Make(NetLogSourceType::NONE), &load_timing_info);
...
TEST_F(HttpCacheSimpleGetTest, UnusedSincePrefetch)
{
    ...
    RunTransactionTestWithResponseInfoAndGetTiming(
        cache.http_cache(), kSimpleGET_Transaction, &response_info, NetLogWithSource::Make(NetLogSourceType::NONE), nullptr);
    ...
    RunTransactionTestWithResponseInfoAndGetTiming(
        cache.http_cache(), prefetch_transaction, &response_info, NetLogWithSource::Make(NetLogSourceType::NONE), nullptr);
    EXPECT_FALSE(response_info.unused_since_prefetch);
...
TEST_F(HttpCacheSimpleGetTest, RestrictedPrefetchIsRestrictedUntilReuse)
{
    ...
    RunTransactionTestWithResponseInfoAndGetTiming(
        cache.http_cache(), kTypicalGET_Transaction, &response_info, NetLogWithSource::Make(NetLogSourceType::NONE), nullptr);
    ...
    // restricted.
    MockTransaction can_use_restricted_prefetch_transaction(kSimpleGET_Transaction);
    can_use_restricted_prefetch_transaction.load_flags |= LOAD_CAN_USE_RESTRICTED_PREFETCH_FOR_MAIN_FRAME;
    RunTransactionTestWithResponseInfoAndGetTiming(
        cache.http_cache(), can_use_restricted_prefetch_transaction, &response_info, NetLogWithSource::Make(NetLogSourceType::NONE), nullptr);
    EXPECT_TRUE(response_info.restricted_prefetch);
    EXPECT_TRUE(response_info.was_cached);
    EXPECT_FALSE(response_info.network_accessed);
...
TEST_F(HttpCacheTest, PrefetchTruncateCancelInConnectedCallback)
{
    ...
    transaction.load_flags |= LOAD_PREFETCH | LOAD_CAN_USE_RESTRICTED_PREFETCH_FOR_MAIN_FRAME;
    ...
    // Do a truncated read of a prefetch request.
    {
        ...
        scoped_refptr<IOBufferWithSize> buf = base::MakeRefCounted<IOBufferWithSize>(10);
        rv = c.callback.GetResult(c.trans->Read(buf.get(), buf->size(), c.callback.callback()));
    ...
    transaction.load_flags &= ~LOAD_PREFETCH;
    {
        ...
        int rv = cache.CreateTransaction(&c.trans);
        ASSERT_THAT(rv, IsOk());
        c.trans->SetConnectedCallback(base::BindRepeating([](const TransportInfo& info, CompletionOnceCallback callback) -> int { return ERR_ABORTED; }));
        rv = c.callback.GetResult(c.trans->Start(&request, c.callback.callback(), NetLogWithSource()));
...
static const auto kPreserveRequestHeaders
    = base::BindRepeating([](const HttpRequestInfo* request, std::string* response_status, std::string* response_headers, std::string* response_data) {
          EXPECT_TRUE(request->extra_headers.HasHeader(kExtraHeaderKey));
      });
...
TEST_F(HttpCacheSimpleGetTest, ManyReaders)
{
    ...
    for (int i = 0; i < kNumTransactions; ++i) {
        ...
        c->result = c->trans->Start(&request, c->callback.callback(), NetLogWithSource());
    }
...
TEST_F(HttpCacheRangeGetTest, FullAfterPartial)
{
    ...
    // Request a prefix.
    {
        ...
        RunTransactionTestWithRequest(cache.http_cache(), transaction_pre, request_pre, &response_pre);
...

    {
        ...
        RunTransactionTestWithRequest(cache.http_cache(), transaction_all, request_all, &response_all);
...
TEST_F(HttpCacheRangeGetTest, FullAfterPartialReuse)
{
...

    {
        ...
        ScopedMockTransaction transaction_all(kRangeGET_TransactionOK);
        transaction_all.request_headers = EXTRA_HEADER;
        transaction_all.data = "rg: 00-09 rg: 10-19 rg: 20-29 rg: 30-39 rg: 40-49"
...

    {
        ...
        transaction_all2.data = "rg: 00-09 rg: 10-19 rg: 20-29 rg: 30-39 rg: 40-49"
                                " rg: 50-59 rg: 60-69 rg: 70-79 ";
        MockHttpRequest request_all2(transaction_all2);

        HttpResponseInfo response_all2;
        RunTransactionTestWithRequest(cache.http_cache(), transaction_all2, request_all2, &response_all2);
...
TEST_F(HttpCacheRangeGetTest, ConnectedCallbackCalledForEachRange)
{
    ...
    // per subrange, as split up by cache hits.
    {
        ...
        // transaction.
        EXPECT_THAT(connected_handler.transports(), ElementsAre(TestTransportInfo(), CachedTestTransportInfo(), TestTransportInfoWithPort(123)));
    }
}
...
TEST_F(HttpCacheRangeGetTest, ConnectedCallbackReturnInconsistentIpError)
{
    ...
    // invalidated as a result of the specific error code.
    {
        ...
        std::string content;
        EXPECT_THAT(ReadTransaction(transaction.get(), &content), IsError(ERR_INCONSISTENT_IP_ADDRESS_SPACE));

        // A second call that failed.
        EXPECT_THAT(connected_handler.transports(), ElementsAre(TestTransportInfo(), CachedTestTransportInfo()));
    }
...
TEST_F(HttpCacheRangeGetTest, ConnectedCallbackReturnInconsistentIpErrorForNetwork)
{
    MockHttpCache cache;
...
TEST_F(HttpCacheRangeGetTest, ConnectedCallbackReturnErrorSecondTime)
{
    ...
    // cached, such that the cache entry ends up with both the prefix and infix.
    {
        ...
        std::string content;
        EXPECT_THAT(ReadTransaction(transaction.get(), &content), IsError(ERR_NOT_IMPLEMENTED));
...
TEST_F(HttpCacheRangeGetTest, ConnectedCallbackCalledForEachRangeWithPrefix)
{
    ...
    // per subrange, as split up by cache hits.
    {
        ...
        EXPECT_THAT(connected_handler.transports(), ElementsAre(CachedTestTransportInfo(), TestTransportInfoWithPort(123)));
    }
}
...
TEST_F(HttpCacheRangeGetTest, ParallelValidationDifferentRanges)
{
    ...
    MockHttpRequest request1(transaction);
    {
        ...
        c->result = c->trans->Start(&request1, c->callback.callback(), NetLogWithSource());
        base::RunLoop().RunUntilIdle();
    ...
    MockHttpRequest request2(transaction);
    {
        ...
        c->result = c->trans->Start(&request2, c->callback.callback(), NetLogWithSource());
        base::RunLoop().RunUntilIdle();
    ...
    for (int i = 0; i < kNumTransactions; ++i) {
        ...
        if (i == 0) {
            ReadRemainingAndVerifyTransaction(c->trans.get(), first_read, transaction);
            continue;
        }
    ...
    // cached.
    {
        ...
        std::string headers;
        RunTransactionTestWithResponse(cache.http_cache(), range_transaction, &headers);
...
TEST_F(HttpCacheRangeGetTest, DoNotCreateWritersWhenReaderExists)
{
    ...
    context.result = context.trans->Start(&request, context.callback.callback(), NetLogWithSource());
    base::RunLoop().RunUntilIdle();
    std::string cache_key = request.CacheKey();
    ...
    range_context.result = range_context.trans->Start(&range_request, range_context.callback.callback(), NetLogWithSource());
    base::RunLoop().RunUntilIdle();
...
TEST_F(HttpCacheRangeGetTest, ParallelValidationCouldntConditionalize)
{
    ...
    for (int i = 0; i < kNumTransactions; ++i) {
        ...
        if (i == 0) {
            ReadRemainingAndVerifyTransaction(c->trans.get(), first_read, mock_transaction);
            continue;
        }
...
TEST_F(HttpCacheRangeGetTest, ParallelValidationCouldConditionalize)
{
    ...
    std::string response_headers_str = base::StrCat({ "ETag: StrongOne\n", "Content-Length:", base::NumberToString(strlen(kFullRangeData)), "\n" });
    ...
    ReadRemainingAndVerifyTransaction(c0->trans.get(), first_read, mock_transaction);
...
TEST_F(HttpCacheSimpleGetTest, ValidationFailureWithCreateFailure)
{
    ...
    c1->result = c1->trans->Start(&request, c1->callback.callback(), NetLogWithSource());
    ...
    c2->result = c2->trans->Start(&request, c2->callback.callback(), NetLogWithSource());
...
TEST_F(HttpCacheRangeGetTest, Enormous)
{
    ...
    auto backend_factory = std::make_unique<HttpCache::DefaultBackend>(DISK_CACHE, CACHE_BACKEND_BLOCKFILE,
        /*file_operations_factory=*/nullptr, temp_dir.GetPath(), 1024 * 1024, false);
    ...
    // Prime with a range it can store.
    {
        ...
        HttpResponseInfo response;
        RunTransactionTestWithRequest(cache.http_cache(), transaction, request, &response);
    ...
    {
        ScopedMockTransaction transaction(kRangeGET_TransactionOK);
        transaction.request_headers = "Range: bytes = "
...
TEST_F(HttpCacheSimpleGetTest, ParallelValidationNoMatch1)
{
    ...
    EXPECT_EQ(kNumTransactions - 1, cache.GetCountWriterTransactions(validate_request.CacheKey()));
...
TEST_F(HttpCacheSimpleGetTest, ParallelValidationCancelValidated)
{
    ...
    for (int i = 0; i < kNumTransactions; ++i) {
        ...
        c->result = c->trans->Start(current_request, c->callback.callback(), NetLogWithSource());
    }
...
TEST_F(HttpCacheSimpleGetTest, ParallelValidationValidatedTimeout)
{
    ...
    ReadAndVerifyTransaction(context_list[0]->trans.get(), kSimpleGET_Transaction);
}
...
TEST_F(HttpCacheSimpleGetTest, ParallelValidationCancelReader)
{
    ...
    for (int i = 2; i < kNumTransactions; ++i) {
        ReadAndVerifyTransaction(context_list[i]->trans.get(), kSimpleGET_Transaction);
    }
...
TEST_F(HttpCacheSimpleGetTest, HangingCacheWriteCleanup)
{
    ...
    TestCompletionCallback callback;
    int result = transaction->Start(&request, callback.callback(), NetLogWithSource());
    ...
    std::string cache_key = request.CacheKey();
    scoped_refptr<MockDiskEntry> entry = mock_cache.disk_cache()->GetDiskEntryRef(cache_key);
...
TEST_F(HttpCacheSimpleGetTest, ParallelWritingCancelWriter)
{
    ...
    for (int i = 0; i < kNumTransactions; i++) {
        ...
        if (i == 1) {
            ReadRemainingAndVerifyTransaction(context->trans.get(), first_read, kSimpleGET_Transaction);
        } else {
            ReadAndVerifyTransaction(context->trans.get(), kSimpleGET_Transaction);
        }
    }
...
TEST_F(HttpCacheSimpleGetTest, ParallelWritingCacheWriteFailed)
{
    ...
    en->Close();
    const int kBufferSize = 5;
    std::vector<scoped_refptr<IOBuffer>> buffer(3, base::MakeRefCounted<IOBufferWithSize>(kBufferSize));
    for (int i = 0; i < 2; i++) {
        auto& c = context_list[i];
        c->result = c->trans->Read(buffer[i].get(), kBufferSize, c->callback.callback());
        EXPECT_EQ(ERR_IO_PENDING, c->result);
    }
    ...
    // Invoke Read on the 3rd transaction and it should get the error code back.
    auto& c = context_list[2];
    c->result = c->trans->Read(buffer[2].get(), kBufferSize, c->callback.callback());
    EXPECT_EQ(ERR_CACHE_WRITE_FAILURE, c->result);
    ...
    ReadRemainingAndVerifyTransaction(succ_read->trans.get(), first_read, kSimpleGET_Transaction);
}
...
TEST_F(HttpCacheSimplePostTest, ParallelWritingDisallowed)
{
    ...
    element_readers.push_back(std::make_unique<UploadBytesElementReader>(base::byte_span_from_cstring("hello")));
    ElementsUploadDataStream upload_data_stream(std::move(element_readers), kUploadId);
    ...
    ReadAndVerifyTransaction(context_list[0]->trans.get(), kSimplePOST_Transaction);
...
TEST_F(HttpCacheSimpleGetTest, ParallelWritingSuccess)
{
    ...
    for (int i = 0; i < 2; i++) {
        auto& c = context_list[i];
        ReadRemainingAndVerifyTransaction(c->trans.get(), first_read[i], kSimpleGET_Transaction);
        if (i == 0) {
...
TEST_F(HttpCacheSimpleGetTest, ParallelWritingHuge)
{
    ...
    std::string response_headers
        = base::StrCat({ kSimpleGET_Transaction.response_headers, "Content-Length: ", base::NumberToString(strlen(kSimpleGET_Transaction.data)), "\n" });
    ...
    const int kBufferSize = 5;
    std::vector<scoped_refptr<IOBuffer>> buffer(kNumTransactions, base::MakeRefCounted<IOBufferWithSize>(kBufferSize));
    auto& c = context_list[0];
    c->result = c->trans->Read(buffer[0].get(), kBufferSize, c->callback.callback());
    EXPECT_EQ(ERR_IO_PENDING, c->result);
    ...
    for (int i = 0; i < kNumTransactions; i++) {
        ReadRemainingAndVerifyTransaction(context_list[i]->trans.get(), first_read[i], kSimpleGET_Transaction);
    }
...
TEST_F(HttpCacheSimpleGetTest, ParallelWritingVerifyNetworkBytes)
{
    ...
    ReadAndVerifyTransaction(context_list[1]->trans.get(), kSimpleGET_Transaction);
    ...
    EXPECT_GE(total_received_bytes, context_list[0]->trans->GetTotalReceivedBytes());
...
TEST_F(HttpCacheTest, DoomDoesNotSetHints)
{
    ...
    c1.result = cache.CreateTransaction(&c1.trans);
    ASSERT_THAT(c1.result, IsOk());
    c1.trans->SetBeforeNetworkStartCallback(base::BindOnce([](bool* defer) { *defer = true; }));
    ...
    c2.result = cache.CreateTransaction(&c2.trans);
    ASSERT_THAT(c2.result, IsOk());
    c2.result = c2.trans->Start(&request2, c2.callback.callback(), NetLogWithSource());
    ...
    context3.result = context3.trans->Start(&request3, context3.callback.callback(), NetLogWithSource());
    base::RunLoop().RunUntilIdle();
...
TEST_F(HttpCacheSimpleGetTest, WriterTimeout)
{
    ...
    ASSERT_EQ(ERR_IO_PENDING, c1.trans->Start(&request, c1.callback.callback(), NetLogWithSource()));
    ASSERT_THAT(cache.CreateTransaction(&c2.trans), IsOk());
    ASSERT_EQ(ERR_IO_PENDING, c2.trans->Start(&request, c2.callback.callback(), NetLogWithSource()));
...
{
    auto cache = std::make_unique<MockHttpCache>(std::make_unique<MockBackendNoCbFactory>());
...
TEST_F(HttpCacheSimpleGetTest, WaitForBackend)
{
    ...
    context_list[0]->result = context_list[0]->trans->Start(&request0, context_list[0]->callback.callback(), NetLogWithSource());
    context_list[1]->result = context_list[1]->trans->Start(&request1, context_list[1]->callback.callback(), NetLogWithSource());
    context_list[2]->result = context_list[2]->trans->Start(&request2, context_list[2]->callback.callback(), NetLogWithSource());
...
TEST_F(HttpCacheTest, TypicalGetConditionalRequest)
{
    ...
    LoadTimingInfo load_timing_info;
    RunTransactionTestAndGetTiming(cache.http_cache(), kTypicalGET_Transaction, NetLogWithSource::Make(NetLogSourceType::NONE), &load_timing_info);
...
static const auto kETagGetConditionalRequestHandler
    = base::BindRepeating([](const HttpRequestInfo* request, std::string* response_status, std::string* response_headers, std::string* response_data) {
          EXPECT_TRUE(request->extra_headers.HasHeader(HttpRequestHeaders::kIfNoneMatch));
          response_status->assign("HTTP/1.1 304 Not Modified");
...
TEST_F(HttpCacheETagGetTest, ConditionalRequest304)
{
    ...
    RunTransactionTestAndGetTimingAndConnectedSocketAddress(
        cache.http_cache(), transaction, NetLogWithSource::Make(NetLogSourceType::NONE), &load_timing_info, &remote_endpoint);
...
public:
    ...
    bool LastModifiedUsed()
    {
        return last_modified_used_;
    }

    MockTransactionHandler GetHandlerCallback()
    {
        return base::BindLambdaForTesting(
            [this](const HttpRequestInfo* request, std::string* response_status, std::string* response_headers, std::string* response_data) {
...
static void ETagGet_UnconditionalRequest_Handler(
    const HttpRequestInfo* request, std::string* response_status, std::string* response_headers, std::string* response_data)
{
    EXPECT_FALSE(request->extra_headers.HasHeader(HttpRequestHeaders::kIfNoneMatch));
}
...
TEST_F(HttpCacheETagGetTest, Http10)
{
    ...
    transaction.load_flags = LOAD_VALIDATE_CACHE;
    transaction.handler = base::BindRepeating(&ETagGet_UnconditionalRequest_Handler);
    RunTransactionTest(cache.http_cache(), transaction);
...
TEST_F(HttpCacheETagGetTest, ConditionalRequest304NoStore)
{
    ...
    // in a conditional request.
    transaction.load_flags = LOAD_VALIDATE_CACHE;
    transaction.handler = base::BindRepeating(&ETagGet_ConditionalRequest_NoStore_Handler);
    RunTransactionTest(cache.http_cache(), transaction);
...
static void ConditionalizedRequestUpdatesCacheHelper(
    const Response& net_response_1, const Response& net_response_2, const Response& cached_response_2, const char* extra_request_headers)
{
    MockHttpCache cache;
    ...
    static const Response kUnexpectedResponse = { "HTTP/1.1 500 Unexpected", "Server: unexpected_header", "unexpected body" };
    ...
    std::string response_headers;
    RunTransactionTestWithResponse(cache.http_cache(), request, &response_headers);
...
TEST_F(HttpCacheTest, ConditionalizedRequestUpdatesCache1)
{
    ...
    ConditionalizedRequestUpdatesCacheHelper(kNetResponse1, kNetResponse2, kNetResponse2, extra_headers);
}
...
TEST_F(HttpCacheTest, ConditionalizedRequestUpdatesCache3)
{
    ...
    // Second network response for |kUrl|.
    static const Response kNetResponse2 = { "HTTP/1.1 304 Not Modified",
        "Date: Wed, 22 Jul 2009 03:15:26 GMT\n"
        "Server: server2\n"
        "Last-Modified: Wed, 06 Feb 2008 22:38:21 GMT\n",
        "" };

    static const Response kCachedResponse2 = { "HTTP/1.1 200 OK",
        "Date: Wed, 22 Jul 2009 03:15:26 GMT\n"
        "Server: server2\n"
        "Last-Modified: Wed, 06 Feb 2008 22:38:21 GMT\n",
        "body1" };
    ...
    ConditionalizedRequestUpdatesCacheHelper(kNetResponse1, kNetResponse2, kCachedResponse2, extra_headers);
}
...
TEST_F(HttpCacheTest, ConditionalizedRequestUpdatesCache4)
{
    ...
    static const Response kNetResponse = { "HTTP/1.1 304 Not Modified",
        "Date: Wed, 22 Jul 2009 03:15:26 GMT\n"
        "Last-Modified: Wed, 06 Feb 2008 22:38:21 GMT\n",
        "" };
...
TEST_F(HttpCacheTest, ConditionalizedRequestUpdatesCache5)
{
    ...
    static const Response kNetResponse = { "HTTP/1.1 200 OK",
        "Date: Wed, 22 Jul 2009 03:15:26 GMT\n"
        "Last-Modified: Wed, 06 Feb 2008 22:38:21 GMT\n",
        "foobar!!!" };
...
TEST_F(HttpCacheTest, ConditionalizedRequestUpdatesCache6)
{
    ...
    ConditionalizedRequestUpdatesCacheHelper(kNetResponse1, kNetResponse2, kNetResponse1, kExtraRequestHeaders);
}
...
TEST_F(HttpCacheTest, ConditionalizedRequestUpdatesCache8)
{
    ...
    ConditionalizedRequestUpdatesCacheHelper(kNetResponse1, kNetResponse2, kNetResponse2, kExtraRequestHeaders);
}
...
TEST_F(HttpCacheTestSplitCacheFeatureEnabled, SimplePostInvalidate205SplitCache)
{
    ...
    req1.network_anonymization_key = NetworkAnonymizationKey::CreateSameSite(site_a);
    ...
    req1b.network_anonymization_key = NetworkAnonymizationKey::CreateSameSite(site_b);
    RunTransactionTestWithRequest(cache.http_cache(), transaction, req1b, nullptr);
    ...
    req2.network_anonymization_key = NetworkAnonymizationKey::CreateSameSite(site_a);
...
TEST_F(HttpCacheSimpleHeadTest, WithCachedRanges)
{
    ...
    ScopedMockTransaction transaction(kSimpleGET_Transaction, kRangeGET_TransactionOK.url);
    transaction.method = "HEAD";
    transaction.data = "";
    std::string headers;
...
TEST_F(HttpCacheSimpleGetTest, DoesntLogHeaders)
{
    ...
    RecordingNetLogObserver net_log_observer;
    RunTransactionTestWithLog(cache.http_cache(), kSimpleGET_Transaction, NetLogWithSource::Make(NetLogSourceType::NONE));

    EXPECT_FALSE(LogContainsEventType(net_log_observer, NetLogEventType::HTTP_CACHE_CALLER_REQUEST_HEADERS));
}
...
TEST_F(HttpCacheRangeGetTest, LogsHeaders)
{
    ...
    RecordingNetLogObserver net_log_observer;
    RunTransactionTestWithLog(cache.http_cache(), kRangeGET_Transaction, NetLogWithSource::Make(NetLogSourceType::NONE));

    EXPECT_TRUE(LogContainsEventType(net_log_observer, NetLogEventType::HTTP_CACHE_CALLER_REQUEST_HEADERS));
}
...
TEST_F(HttpCacheTest, ExternalValidationLogsHeaders)
{
    ...
    MockTransaction transaction(kSimpleGET_Transaction);
    transaction.request_headers = "If-None-Match: foo\r\n" EXTRA_HEADER;
    RunTransactionTestWithLog(cache.http_cache(), transaction, NetLogWithSource::Make(NetLogSourceType::NONE));
...
TEST_F(HttpCacheRangeGetTest, NoStrongValidators)
{
    ...
    RunTransactionTestWithResponse(cache.http_cache(), kRangeGET_TransactionOK, &headers);
...
TEST_F(HttpCacheRangeGetTest, NoValidationLogsRestart)
{
    ...
    // Now verify that the cached data is not used.
    RecordingNetLogObserver net_log_observer;
    RunTransactionTestWithLog(cache.http_cache(), kRangeGET_TransactionOK, NetLogWithSource::Make(NetLogSourceType::NONE));

    EXPECT_TRUE(LogContainsEventType(net_log_observer, NetLogEventType::HTTP_CACHE_RESTART_PARTIAL_REQUEST));
}
...
TEST_F(HttpCacheRangeGetTest, NoContentLength)
{
    ...
    // Now verify that there's no cached data.
    transaction.handler = base::BindRepeating(&RangeTransactionServer::RangeHandler);
    RunTransactionTestWithResponse(cache.http_cache(), kRangeGET_TransactionOK, &headers);
...
TEST_F(HttpCacheRangeGetTest, OK)
{
    ...
    transaction.data = "rg: 20-29 rg: 30-39 rg: 40-49 rg: 50-59 ";
    LoadTimingInfo load_timing_info;
    RunTransactionTestWithResponseAndGetTiming(cache.http_cache(), transaction, &headers, NetLogWithSource::Make(NetLogSourceType::NONE), &load_timing_info);
...
TEST_F(HttpCacheRangeGetTest, NoStore)
{
    ...
    ScopedMockTransaction transaction(kRangeGET_TransactionOK);
    std::string response_headers = base::StrCat({ kRangeGET_TransactionOK.response_headers, "Cache-Control: no-store\n" });
    transaction.response_headers = response_headers.c_str();
...
TEST_F(HttpCacheRangeGetTest, NoStore304)
{
    ...
    ScopedMockTransaction transaction(kRangeGET_TransactionOK);
    std::string response_headers = base::StrCat({ kRangeGET_TransactionOK.response_headers, "Cache-Control: max-age=0\n" });
    transaction.response_headers = response_headers.c_str();
    ...
    response_headers = base::StrCat({ kRangeGET_TransactionOK.response_headers, "Cache-Control: no-store\n" });
    transaction.response_headers = response_headers.c_str();
    RunTransactionTestWithResponse(cache.http_cache(), transaction, &headers);
    Verify206Response(headers, 40, 49);
...
TEST_F(HttpCacheRangeGetTest, Revalidate1)
{
    ...
    NetLogWithSource net_log_with_source = NetLogWithSource::Make(NetLogSourceType::NONE);
    LoadTimingInfo load_timing_info;
    RunTransactionTestWithResponseAndGetTiming(cache.http_cache(), transaction, &headers, net_log_with_source, &load_timing_info);
...
TEST_F(HttpCacheRangeGetTest, 304)
{
    ...
    // Write to the cache (40-49).
    RunTransactionTestWithResponse(cache.http_cache(), scoped_transaction, &headers);
...
TEST_F(HttpCacheUnknownRangeGetTest, Empty302Replaced)
{
    ...
    EXPECT_EQ("HTTP/1.1 206 Partial Content\n"
...
TEST_F(HttpCacheGetTest, Previous206)
{
    ...
    // Write to the cache (40-49).
    RunTransactionTestWithResponseAndGetTiming(cache.http_cache(), kRangeGET_TransactionOK, &headers, net_log_with_source, &load_timing_info);
...
TEST_F(HttpCacheGetTest, Previous206NewContent)
{
    ...
    handler.set_modified(true);
    LoadTimingInfo load_timing_info;
    RunTransactionTestWithResponseAndGetTiming(cache.http_cache(), transaction2, &headers, NetLogWithSource::Make(NetLogSourceType::NONE), &load_timing_info);
...
TEST_F(HttpCacheGetTest, Previous206NotSparse)
{
    ...
    int len = static_cast<int>(base::strlcpy(buf->data(), kRangeGET_TransactionOK.data, 500));
    TestCompletionCallback cb;
    int rv = entry->WriteData(1, 0, buf.get(), len, cb.callback(), true);
    EXPECT_EQ(len, cb.GetResult(rv));
    entry->Close();
    ...
    RunTransactionTestWithResponseAndGetTiming(
        cache.http_cache(), kSimpleGET_Transaction, &headers, NetLogWithSource::Make(NetLogSourceType::NONE), &load_timing_info);
...
TEST_F(HttpCacheGetTest, Previous206NotValidation)
{
    ...
    // Now see that we don't use the stored entry.
    std::string headers;
    RunTransactionTestWithResponse(cache.http_cache(), kSimpleGET_Transaction, &headers);
...
TEST_F(HttpCacheRangeGetTest, Previous200)
{
...

    {
        ...
        ScopedMockTransaction transaction(kTypicalGET_Transaction, kRangeGET_TransactionOK.url);
        transaction.data = kFullRangeData;
        RunTransactionTest(cache.http_cache(), transaction);
        EXPECT_EQ(1, cache.network_layer()->transaction_count());
        EXPECT_EQ(0, cache.disk_cache()->open_count());
...
TEST_F(HttpCacheTest, RangeRequestResultsIn200)
{
    ...
    ScopedMockTransaction transaction2(kSimpleGET_Transaction, kRangeGET_TransactionOK.url);
    transaction2.request_headers = kRangeGET_TransactionOK.request_headers;
...
TEST_F(HttpCacheRangeGetTest, CancelWhileReading)
{
    ...
    rv = context->trans->Start(&request, context->callback.callback(), NetLogWithSource());
    if (rv == ERR_IO_PENDING) {
    ...
    // Start Read.
    scoped_refptr<IOBufferWithSize> buf = base::MakeRefCounted<IOBufferWithSize>(5);
    rv = context->trans->Read(buf.get(), buf->size(), context->callback.callback());
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
...
TEST_F(HttpCacheRangeGetTest, FastFlakyServer)
{
    ...
    EXPECT_EQ(1, cache.disk_cache()->open_count());
    EXPECT_EQ(1, cache.disk_cache()->create_count());
    EXPECT_TRUE(LogContainsEventType(net_log_observer, NetLogEventType::HTTP_CACHE_RE_SEND_PARTIAL_REQUEST));
}
...
TEST_F(HttpCacheTest, WriteResponseInfoTruncated)
{
    MockHttpCache cache;
    disk_cache::Entry* entry;
    ASSERT_TRUE(cache.CreateBackendEntry(GenerateCacheKey("http://www.google.com"), &entry, nullptr));

    HttpResponseInfo response;
    response.headers = base::MakeRefCounted<HttpResponseHeaders>(HttpUtil::AssembleRawHeaders("HTTP/1.1 200 OK"));
...
TEST_F(HttpCacheTest, PersistHttpResponseInfo)
{
    ...
    response1.headers = base::MakeRefCounted<HttpResponseHeaders>("HTTP/1.1 200 OK");
...
TEST_F(HttpCacheTest, DontSetTruncatedFlag)
{
    ...
    // Read everything.
    scoped_refptr<IOBufferWithSize> buf = base::MakeRefCounted<IOBufferWithSize>(22);
    rv = c->trans->Read(buf.get(), buf->size(), c->callback.callback());
...
TEST_F(HttpCacheTest, SyncRead)
{
    ...
    TestTransactionConsumer c1(DEFAULT_PRIORITY, cache.http_cache()), c2(DEFAULT_PRIORITY, cache.http_cache()), c3(DEFAULT_PRIORITY, cache.http_cache());
...
TEST_F(HttpCacheTestSplitCacheFeatureEnabled, SplitCacheWithNetworkIsolationKey)
{
    MockHttpCache cache;
    HttpResponseInfo response;
    ...
    trans_info.network_anonymization_key = NetworkAnonymizationKey::CreateSameSite(site_a);
    RunTransactionTestWithRequest(cache.http_cache(), kSimpleGET_Transaction, trans_info, &response);
    ...
    trans_info.network_anonymization_key = NetworkAnonymizationKey::CreateCrossSite(site_a);
    RunTransactionTestWithRequest(cache.http_cache(), kSimpleGET_Transaction, trans_info, &response);
    EXPECT_FALSE(response.was_cached);
    ...
    trans_info.network_anonymization_key = NetworkAnonymizationKey::CreateCrossSite(site_b);
    ...
    post_info.network_anonymization_key = NetworkAnonymizationKey::CreateSameSite(site_a);
    post_info.upload_data_stream = &upload_data_stream;

    RunTransactionTestWithRequest(cache.http_cache(), kSimplePOST_Transaction, post_info, &response);
    EXPECT_FALSE(response.was_cached);
}
...
TEST_F(HttpCacheTest, HttpCacheProfileThirdPartyCSS)
{
    ...
    RunTransactionTestWithRequest(cache.http_cache(), transaction, trans_info, &response);
    ...
    trans_info.network_anonymization_key = NetworkAnonymizationKey::CreateSameSite(site_b);
...
TEST_P(HttpCacheTestSplitCacheFeature, SplitCache)
{
    ...
    const SchemefulSite site_data(GURL("data:text/html,<body>Hello World</body>"));
    ...
    RunTransactionTestWithRequest(cache.http_cache(), kSimpleGET_Transaction, subframe_document_trans_info, &response);
    EXPECT_FALSE(response.was_cached);
    ...
    trans_info.network_anonymization_key = NetworkAnonymizationKey::CreateSameSite(site_data);
    EXPECT_EQ(std::nullopt, trans_info.network_isolation_key.ToCacheKeyString());
    RunTransactionTestWithRequest(cache.http_cache(), kSimpleGET_Transaction, trans_info, &response);
    EXPECT_FALSE(response.was_cached);
...
TEST_P(HttpCacheTestSplitCacheFeature, GenerateCacheKeyForRequestFailures)
{
    ...
    cacheable_request.network_anonymization_key = NetworkAnonymizationKey::CreateSameSite(site);
    std::optional<std::string> cache_key = HttpCache::GenerateCacheKeyForRequest(&cacheable_request);
    ...
    opaque_top_level_site_request.network_isolation_key = NetworkIsolationKey(site_data, site);
    opaque_top_level_site_request.network_anonymization_key
        = NetworkAnonymizationKey::CreateFromNetworkIsolationKey(opaque_top_level_site_request.network_isolation_key);
    ...
    cache_key = HttpCache::GenerateCacheKeyForRequest(&opaque_top_level_site_request);
    ...
    cache_key = HttpCache::GenerateCacheKeyForRequest(&opaque_initiator_main_frame_request);
    EXPECT_EQ(is_request_cacheable, cache_key.has_value());
    ...
    cache_key = HttpCache::GenerateCacheKeyForRequest(&opaque_initiator_subframe_request);
    EXPECT_EQ(is_request_cacheable, cache_key.has_value());
}
...
TEST_F(HttpCacheTest, SplitCacheEnabledByDefaultButOverridden)
{
    HttpCache::ClearGlobalsForTesting();
    base::test::ScopedFeatureList feature_list;
    feature_list.InitAndDisableFeature(features::kSplitCacheByNetworkIsolationKey);
...
TEST_F(HttpCacheTest, NonSplitCache)
{
    ...
    trans_info.network_anonymization_key = NetworkAnonymizationKey::CreateSameSite(kSiteA);
    RunTransactionTestWithRequest(cache.http_cache(), kSimpleGET_Transaction, trans_info, &response);
    EXPECT_TRUE(response.was_cached);
}
...
enum class TransactionPhase { BEFORE_FIRST_READ, AFTER_FIRST_READ, AFTER_NETWORK_READ };

using CacheInitializer = void (*)(MockHttpCache*);
using HugeCacheTestConfiguration = std::pair<TransactionPhase, CacheInitializer>;

class HttpCacheHugeResourceTest : public ::testing::TestWithParam<HugeCacheTestConfiguration>, public WithTaskEnvironment {
public:
...
protected:
    static void LargeResourceTransactionHandler(
        const HttpRequestInfo* request, std::string* response_status, std::string* response_headers, std::string* response_data);
    static int LargeBufferReader(int64_t content_length, int64_t offset, IOBuffer* buf, int buf_len);
...
void HttpCacheHugeResourceTest::LargeResourceTransactionHandler(
    const HttpRequestInfo* request, std::string* response_status, std::string* response_headers, std::string* response_data)
{
    std::optional<std::string> if_range = request->extra_headers.GetHeader(HttpRequestHeaders::kIfRange);
    if (!if_range) {
    ...
    std::string range_header = request->extra_headers.GetHeader(HttpRequestHeaders::kRange).value();
    std::vector<HttpByteRange> ranges;
    ...
    *response_headers = base::StringPrintf("Content-Range: bytes %" PRId64 "-%" PRId64 "/%" PRId64 "\n"
                                           "Content-Length: %" PRId64 "\n",
        range.first_byte_position(), last_byte_position, kTotalSize, last_byte_position - range.first_byte_position() + 1);
}

// static
int HttpCacheHugeResourceTest::LargeBufferReader(int64_t content_length, int64_t offset, IOBuffer* buf, int buf_len)
{
...
// static
void HttpCacheHugeResourceTest::SetFlagOnBeforeNetworkStart(bool* started, bool* /* defer */)
{
    *started = true;
}
...
{
    ScopedMockTransaction scoped_transaction(kRangeGET_TransactionOK);
    std::string cached_headers = base::StringPrintf("HTTP/1.1 200 OK\n"
                                                    "Last-Modified: Sat, 18 Apr 2007 01:10:43 GMT\n"
                                                    "ETag: \"foo\"\n"
                                                    "Accept-Ranges: bytes\n"
                                                    "Content-Length: %" PRId64 "\n",
...
// static
void HttpCacheHugeResourceTest::SetupPrefixSparseCacheEntry(MockHttpCache* cache)
{
    ScopedMockTransaction transaction(kRangeGET_TransactionOK);
    transaction.handler = MockTransactionHandler();
    transaction.request_headers = "Range: bytes = 0-9\r\n" EXTRA_HEADER;
...
// static
void HttpCacheHugeResourceTest::SetupInfixSparseCacheEntry(MockHttpCache* cache)
{
    ScopedMockTransaction transaction(kRangeGET_TransactionOK);
    transaction.handler = MockTransactionHandler();
...
// static
std::list<HugeCacheTestConfiguration> HttpCacheHugeResourceTest::GetTestModes()
{
    std::list<HugeCacheTestConfiguration> test_modes;
    const TransactionPhase kTransactionPhases[]
        = { TransactionPhase::BEFORE_FIRST_READ, TransactionPhase::AFTER_FIRST_READ, TransactionPhase::AFTER_NETWORK_READ };
    const CacheInitializer kInitializers[] = { &SetupTruncatedCacheEntry, &SetupPrefixSparseCacheEntry, &SetupInfixSparseCacheEntry };
...
INSTANTIATE_TEST_SUITE_P(_, HttpCacheHugeResourceTest, ::testing::ValuesIn(HttpCacheHugeResourceTest::kTestModes));
...
TEST_P(HttpCacheHugeResourceTest, StopCachingFollowedByReadForHugeTruncatedResource)
{
    ...
    int rv = cache.http_cache()->CreateTransaction(DEFAULT_PRIORITY, &http_transaction);
    ASSERT_EQ(OK, rv);
    ...
    if (stop_caching_phase == TransactionPhase::AFTER_NETWORK_READ) {
        http_transaction->SetBeforeNetworkStartCallback(base::BindOnce(&SetFlagOnBeforeNetworkStart, &network_transaction_started));
    }
    ...
    EXPECT_EQ(kTotalSize, http_transaction->GetResponseInfo()->headers->GetContentLength());
    do {
        ...
        scoped_refptr<IOBuffer> buf = base::MakeRefCounted<IOBufferWithSize>(kBufferSize);
...
TEST_F(HttpCacheTest, SetPriority)
{
    ...
    EXPECT_EQ(DEFAULT_PRIORITY, cache.network_layer()->last_create_transaction_priority());

    info.url = GURL(kSimpleGET_Transaction.url);
    TestCompletionCallback callback;
    EXPECT_EQ(ERR_IO_PENDING, trans->Start(&info, callback.callback(), NetLogWithSource()));
...
TEST_F(HttpCacheTest, SetWebSocketHandshakeStreamCreateHelper)
{
    ...
    EXPECT_EQ(&create_helper, cache.network_layer()->last_transaction()->websocket_handshake_stream_create_helper());
    EXPECT_THAT(callback.WaitForResult(), IsOk());
}
...
void RunTransactionAndGetNetworkBytes(MockHttpCache* cache, const MockTransaction& trans_info, int64_t* sent_bytes, int64_t* received_bytes)
{
    RunTransactionTestBase(
        cache->http_cache(), trans_info, MockHttpRequest(trans_info), nullptr, NetLogWithSource(), nullptr, sent_bytes, received_bytes, nullptr);
}
...
protected:
    ...
    {
        clock_.Advance(base::Seconds(seconds));
    }

    int prefetch_reuse_mins()
    {
        return HttpCache::kPrefetchReuseMins;
    }
...
TEST_F(HttpCachePrefetchValidationTest, SkipValidationOnceReadOnly)
{
    EXPECT_TRUE(TransactionRequiredNetwork(LOAD_PREFETCH));
    AdvanceTime(kRequireValidationSecs);
    EXPECT_FALSE(TransactionRequiredNetwork(LOAD_ONLY_FROM_CACHE | LOAD_SKIP_CACHE_VALIDATION));
    EXPECT_TRUE(TransactionRequiredNetwork(LOAD_NORMAL));
}
...
TEST_F(HttpCachePrefetchValidationTest, SkipValidationOnExistingEntryThatNeedsValidation)
{
    EXPECT_TRUE(TransactionRequiredNetwork(LOAD_NORMAL));
    AdvanceTime(kRequireValidationSecs);
    EXPECT_TRUE(TransactionRequiredNetwork(LOAD_PREFETCH));
...
TEST_F(HttpCachePrefetchValidationTest, SkipValidationOnExistingEntryThatDoesNotNeedValidation)
{
    EXPECT_TRUE(TransactionRequiredNetwork(LOAD_NORMAL));
...
TEST_F(HttpCacheTest, StaleContentNotUsedWhenLoadFlagNotSet)
{
    ...
    ScopedMockTransaction stale_while_revalidate_transaction(kSimpleGET_Transaction);
    ...
    HttpResponseInfo response_info;
    RunTransactionTestWithResponseInfo(cache.http_cache(), stale_while_revalidate_transaction, &response_info);
...
TEST_F(HttpCacheTest, NoStoreResponseShouldNotBlockFollowingRequests)
{
    ...
    first->result = first->trans->Start(&request, first->callback.callback(), NetLogWithSource());
    EXPECT_EQ(LOAD_STATE_WAITING_FOR_CACHE, first->trans->GetLoadState());

    base::RunLoop().RunUntilIdle();
    EXPECT_EQ(LOAD_STATE_IDLE, first->trans->GetLoadState());
    ASSERT_TRUE(first->trans->GetResponseInfo());
    EXPECT_TRUE(first->trans->GetResponseInfo()->headers->HasHeaderValue("Cache-Control", "no-store"));
    ...
    second->result = second->trans->Start(&request, second->callback.callback(), NetLogWithSource());
    ...
    EXPECT_TRUE(second->trans->GetResponseInfo()->headers->HasHeaderValue("Cache-Control", "no-store"));
...
TEST_F(HttpCacheTest, CachePreservesSSLInfo)
{
    ...
    int status = 0;
    SSLConnectionStatusSetCipherSuite(kTLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256, &status);
    ...
    EXPECT_EQ(transaction.ssl_connection_status, response_info.ssl_info.connection_status);
...
TEST_F(HttpCacheTest, RevalidationUpdatesSSLInfo)
{
    ...
    int status2 = 0;
    SSLConnectionStatusSetCipherSuite(kTLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256, &status2);
...
TEST_F(HttpCacheTest, CacheEntryStatusOther)
{
    ...
    HttpResponseInfo response_info;
    RunTransactionTestWithResponseInfo(cache.http_cache(), kRangeGET_Transaction, &response_info);
...
TEST_F(HttpCacheTest, CacheEntryStatusValidated)
{
    ...
    HttpResponseInfo response_info;
    RunTransactionTestWithResponseInfo(cache.http_cache(), still_valid, &response_info);

    EXPECT_TRUE(response_info.was_cached);
    EXPECT_TRUE(response_info.network_accessed);
    EXPECT_EQ(CacheEntryStatus::ENTRY_VALIDATED, response_info.cache_entry_status);
}
...
TEST_F(HttpCacheTest, CacheEntryStatusUpdated)
{
    ...
    HttpResponseInfo response_info;
    RunTransactionTestWithResponseInfo(cache.http_cache(), update, &response_info);
...
TEST_F(HttpCacheTest, CacheEntryStatusCantConditionalize)
{
    ...
    HttpResponseInfo response_info;
    RunTransactionTestWithResponseInfo(cache.http_cache(), kTypicalGET_Transaction, &response_info);

    EXPECT_FALSE(response_info.was_cached);
    EXPECT_TRUE(response_info.network_accessed);
    EXPECT_EQ(CacheEntryStatus::ENTRY_CANT_CONDITIONALIZE, response_info.cache_entry_status);
}
...
TEST_F(HttpSplitCacheKeyTest, GetResourceURLFromHttpCacheKey)
{
    ...
    MockHttpCache cache;
    std::string urls[] = { "http://www.a.com/", "https://b.com/example.html", "http://example.com/Some Path/Some Leaf?some query" };
...
TEST_F(HttpCacheTest, GetResourceURLFromHttpCacheKey)
{
    ...
    for (const auto& test : kTestCase) {
        EXPECT_EQ(test.output, HttpCache::GetResourceURLFromHttpCacheKey(test.input));
    }
}
...
public:
    ...
    {
        return base::BindRepeating(&TestCompletionCallbackForHttpCache::SetResult, base::Unretained(this));
    }
...
TEST_F(HttpCacheIOCallbackTest, FailedDoomFollowedByOpen)
{
    ...
    std::unique_ptr<Transaction> transaction = std::make_unique<Transaction>(DEFAULT_PRIORITY, cache.http_cache());
    ...
    rv = OpenEntry(cache.http_cache(), m_transaction.url, &entry1, transaction.get());
    ASSERT_EQ(rv, ERR_IO_PENDING);
...
TEST_F(HttpCacheIOCallbackTest, FailedDoomFollowedByCreate)
{
    ...
    ASSERT_EQ(rv, ERR_IO_PENDING);
    cache.disk_cache()->set_force_fail_callback_later(false);
    rv = CreateEntry(cache.http_cache(), m_transaction.url, &entry1, transaction.get());
    ASSERT_EQ(rv, ERR_IO_PENDING);
...
TEST_F(HttpCacheIOCallbackTest, FailedOpenFollowedByCreate)
{
    ...
    // Queue up our operations.
    int rv = OpenEntry(cache.http_cache(), m_transaction.url, &entry1, transaction.get());
    ASSERT_EQ(rv, ERR_IO_PENDING);
    cache.disk_cache()->set_force_fail_callback_later(false);
    rv = CreateEntry(cache.http_cache(), m_transaction.url, &entry2, transaction.get());
    ASSERT_EQ(rv, ERR_IO_PENDING);
...
TEST_F(HttpCacheIOCallbackTest, FailedCreateFollowedByOpen)
{
    ...
    // Queue up our operations.
    int rv = CreateEntry(cache.http_cache(), m_transaction.url, &entry1, transaction.get());
    ASSERT_EQ(rv, ERR_IO_PENDING);
    cache.disk_cache()->set_force_fail_callback_later(false);
    rv = OpenEntry(cache.http_cache(), m_transaction.url, &entry2, transaction.get());
    ASSERT_EQ(rv, ERR_IO_PENDING);
...
TEST_F(HttpCacheIOCallbackTest, CreateFollowedByOpenOrCreate)
{
    ...
    int rv = CreateEntry(cache.http_cache(), m_transaction.url, &entry1, transaction.get());
    ASSERT_EQ(rv, ERR_IO_PENDING);
    rv = OpenOrCreateEntry(cache.http_cache(), m_transaction.url, &entry2, transaction.get());
    ASSERT_EQ(rv, ERR_IO_PENDING);
...
TEST_F(HttpCacheIOCallbackTest, OpenFollowedByOpenOrCreate)
{
    ...
    int rv = CreateEntry(cache.http_cache(), m_transaction.url, &entry0, transaction.get());
    ASSERT_EQ(rv, ERR_IO_PENDING);
    cb.GetResult(rv);
...
TEST_F(HttpCacheIOCallbackTest, OpenOrCreateFollowedByCreate)
{
    ...
    // Queue up our operations.
    int rv = OpenOrCreateEntry(cache.http_cache(), m_transaction.url, &entry1, transaction.get());
    ASSERT_EQ(rv, ERR_IO_PENDING);
    rv = CreateEntry(cache.http_cache(), m_transaction.url, &entry2, transaction.get());
    ASSERT_EQ(rv, ERR_IO_PENDING);
...
TEST_F(HttpCacheTest, SecurityHeadersAreCopiedToConditionalizedResponse)
{
    ...
    EXPECT_EQ(response.headers->GetNormalizedHeader("Cross-Origin-Resource-Policy"), "cross-origin");
