...
bool ParseRealm(const HttpAuthChallengeTokenizer& tokenizer, std::string* realm)
{
    CHECK(realm);
    realm->clear();
...
bool HttpAuthHandlerBasic::Init(HttpAuthChallengeTokenizer* challenge, const SSLInfo& ssl_info, const NetworkAnonymizationKey& network_anonymization_key)
{
...
bool HttpAuthHandlerBasic::ParseChallenge(HttpAuthChallengeTokenizer* challenge)
{
...
int HttpAuthHandlerBasic::GenerateAuthTokenImpl(
    const AuthCredentials* credentials, const HttpRequestInfo*, CompletionOnceCallback callback, std::string* auth_token)
{
    DCHECK(credentials);
    ...
    std::string base64_username_password = base::Base64Encode(base::UTF16ToUTF8(credentials->username()) + ":" + base::UTF16ToUTF8(credentials->password()));
    *auth_token = "Basic " + base64_username_password;
    return OK;
}

HttpAuth::AuthorizationResult HttpAuthHandlerBasic::HandleAnotherChallengeImpl(HttpAuthChallengeTokenizer* challenge)
{
...
int HttpAuthHandlerBasic::Factory::CreateAuthHandler(HttpAuthChallengeTokenizer* challenge, HttpAuth::Target target, const SSLInfo& ssl_info,
...
{
    if (http_auth_preferences() && !http_auth_preferences()->basic_over_http_enabled() && scheme_host_port.scheme() == url::kHttpScheme) {
