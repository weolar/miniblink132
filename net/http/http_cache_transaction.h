...
private:
    ...
    int RestartNetworkRequestWithCertificate(scoped_refptr<X509Certificate> client_cert, scoped_refptr<SSLPrivateKey> client_private_key);
    ...
    bool IsResponseConditionalizable(std::string* etag_value, std::string* last_modified_value) const;
    ...
    int WriteResponseInfoToEntry(const HttpResponseInfo& response, bool truncated);
    ...
    void UpdateCacheEntryStatus(HttpResponseInfo::CacheEntryStatus new_cache_entry_status);
    ...
    raw_ptr<const HttpResponseInfo, AcrossTasksDanglingUntriaged> new_response_ = nullptr;
    ...
    bool bypass_lock_after_headers_for_test_ = false; // A test is exercising the
        // cache lock.
    bool fail_conditionalization_for_test_ = false; // Fail ConditionalizeRequest.
    ...
    HttpResponseInfo::CacheEntryStatus cache_entry_status_ = HttpResponseInfo::CacheEntryStatus::ENTRY_UNDEFINED;
