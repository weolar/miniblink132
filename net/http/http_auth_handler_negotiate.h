...
public:
    ...
    HttpAuthHandlerNegotiate(std::unique_ptr<HttpAuthMechanism> auth_system, const HttpAuthPreferences* prefs, HostResolver* host_resolver);
    ...
    const std::string& spn_for_testing() const
    {
        return spn_;
    }
...
private:
    ...
    std::string CreateSPN(const std::string& server, const url::SchemeHostPort& scheme_host_port);
