...
protected:
    ...
    void ConfigureTestDependencies(std::unique_ptr<ConfiguredProxyResolutionService> proxy_resolution_service)
    {
        ...
        network_session_ = std::make_unique<HttpNetworkSession>(HttpNetworkSessionParams(), session_context);
...
TEST_F(HttpNetworkLayerTest, GET)
{
    ...
    request_info.extra_headers.SetHeader(HttpRequestHeaders::kUserAgent, "Foo/1.0");
