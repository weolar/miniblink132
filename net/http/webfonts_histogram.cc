...
bool Consume(const std::string& str, base::StringPiece substr, std::string::size_type* pos)
{
...
void RecordCacheEvent(net::HttpResponseInfo::CacheEntryStatus cache_status, const std::string& histogram_name)
{
    CACHE_HISTOGRAM_ENUMERATION(histogram_name, cache_status, net::HttpResponseInfo::CacheEntryStatus::ENTRY_MAX);
}
...
// "opensans" or "others".
void MaybeRecordCacheStatus(HttpResponseInfo::CacheEntryStatus cache_status, const std::string& key)
{
    std::string::size_type pos = 0;
    if (Consume(key, "https://", &pos) || Consume(key, "http://", &pos)) {
        if (Consume(key, "themes.googleusercontent.com/static/fonts/", &pos) || Consume(key, "ssl.gstatic.com/fonts/", &pos)
