...
public:
    ...
    void OnPush(std::unique_ptr<ServerPushHelper> push_helper, const NetLogWithSource& session_net_log) override;
...
private:
    ...
    public:
        LookupTransaction(std::unique_ptr<ServerPushHelper> push_helper, NetLog* net_log);
        ~LookupTransaction();

        // Issues an HttpCache::Transaction to lookup whether the response is cached
        // without header validation.
        int StartLookup(HttpCache* cache, CompletionOnceCallback callback, const NetLogWithSource& session_net_log);
