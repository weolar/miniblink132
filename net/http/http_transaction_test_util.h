...
enum {
    ...
    TEST_MODE_SYNC_ALL
    = (TEST_MODE_SYNC_NET_START | TEST_MODE_SYNC_NET_READ | TEST_MODE_SYNC_CACHE_START | TEST_MODE_SYNC_CACHE_READ | TEST_MODE_SYNC_CACHE_WRITE),
    TEST_MODE_SLOW_READ = 1 << 5
};

using MockTransactionReadHandler = base::RepeatingCallback<int(int64_t content_length, int64_t offset, IOBuffer* buf, int buf_len)>;

using MockTransactionHandler
    = base::RepeatingCallback<void(const HttpRequestInfo* request, std::string* response_status, std::string* response_headers, std::string* response_data)>;
...
struct ScopedMockTransaction : MockTransaction {
    ...
    explicit ScopedMockTransaction(const MockTransaction& t, const char* url = nullptr);
...
public:
    TestTransactionConsumer(RequestPriority priority, HttpTransactionFactory* factory);
    ...
    const HttpTransaction* transaction() const
    {
        return trans_.get();
    }
...
public:
    ...
    int Start(const HttpRequestInfo* request, CompletionOnceCallback callback, const NetLogWithSource& net_log) override;
    ...
    void SetWebSocketHandshakeStreamCreateHelper(CreateHelper* create_helper) override;
...
private:
    ...
    base::RepeatingCallback<void(HttpRequestHeaders*)> modify_request_headers_callback_;
...
public:
    ...
    bool done_reading_called() const
    {
        return done_reading_called_;
    }
    bool stop_caching_called() const
    {
        return stop_caching_called_;
    }
...
public:
    ...
    {
        return base::BindRepeating(&ConnectedHandler::OnConnected, base::Unretained(this));
    }
    ...
    // directly.
    void set_run_callback(bool run_callback)
    {
