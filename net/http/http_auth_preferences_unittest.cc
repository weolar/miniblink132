...
{
    HttpAuthPreferences http_auth_preferences;
    EXPECT_EQ(std::string(), http_auth_preferences.AuthAndroidNegotiateAccountType());
    http_auth_preferences.set_auth_android_negotiate_account_type("foo");
    EXPECT_EQ(std::string("foo"), http_auth_preferences.AuthAndroidNegotiateAccountType());
}
...
TEST(HttpAuthPreferencesTest, AuthServerAllowlist)
{
    HttpAuthPreferences http_auth_preferences;
    // Check initial value
    EXPECT_FALSE(http_auth_preferences.CanUseDefaultCredentials(url::SchemeHostPort(GURL("abc"))));
    http_auth_preferences.SetServerAllowlist("*");
    EXPECT_TRUE(http_auth_preferences.CanUseDefaultCredentials(url::SchemeHostPort(GURL("abc"))));
}
...
TEST(HttpAuthPreferencesTest, DelegationType)
{
    ...
    HttpAuthPreferences http_auth_preferences;
    // Check initial value
    EXPECT_EQ(DelegationType::kNone, http_auth_preferences.GetDelegationType(url::SchemeHostPort(GURL("abc"))));

    http_auth_preferences.SetDelegateAllowlist("*");
    EXPECT_EQ(DelegationType::kUnconstrained, http_auth_preferences.GetDelegationType(url::SchemeHostPort(GURL("abc"))));

    http_auth_preferences.set_delegate_by_kdc_policy(true);
    EXPECT_EQ(DelegationType::kByKdcPolicy, http_auth_preferences.GetDelegationType(url::SchemeHostPort(GURL("abc"))));
...
TEST(HttpAuthPreferencesTest, HttpAuthSchemesFilter)
{
    ...
    http_auth_preferences.set_http_auth_scheme_filter(
        base::BindRepeating([](const url::SchemeHostPort& scheme_host_port) { return scheme_host_port.GetURL() == GURL("https://www.google.com"); }));
    EXPECT_TRUE(http_auth_preferences.IsAllowedToUseAllHttpAuthSchemes(url::SchemeHostPort(GURL("https://www.google.com"))));
    EXPECT_FALSE(http_auth_preferences.IsAllowedToUseAllHttpAuthSchemes(url::SchemeHostPort(GURL("https://www.example.com"))));
}
