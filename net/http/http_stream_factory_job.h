...
class HttpStreamFactory::Job : public SpdySessionPool::SpdySessionRequest::Delegate {
public:
    ...
    public:
        ...
        virtual void OnBidirectionalStreamImplReady(Job* job, const ProxyInfo& used_proxy_info) = 0;

        // Invoked when |job| has a WebSocketHandshakeStream ready.
        virtual void OnWebSocketHandshakeStreamReady(Job* job, const ProxyInfo& used_proxy_info, std::unique_ptr<WebSocketHandshakeStreamBase> stream) = 0;
        ...
        virtual void OnCertificateError(Job* job, int status, const SSLInfo& ssl_info) = 0;
        ...
        virtual void OnNeedsProxyAuth(Job* job, const HttpResponseInfo& proxy_response, const ProxyInfo& used_proxy_info, HttpAuthController* auth_controller)
            = 0;
        ...
        virtual void AddConnectionAttemptsToRequest(Job* job, const ConnectionAttempts& attempts) = 0;
        ...
        virtual WebSocketHandshakeStreamBase::CreateHelper* websocket_handshake_stream_create_helper() = 0;
    ...
    HttpStreamRequest::StreamType stream_type() const
    {
        return stream_type_;
    }
    ...
    {
        return next_state_ == STATE_WAIT_COMPLETE;
    }
    ...
    bool using_quic() const
    {
        return using_quic_;
    }

    bool should_reconsider_proxy() const
    {
        return should_reconsider_proxy_;
    }
...
private:
    ...
    void OnNeedsProxyAuthCallback(const HttpResponseInfo& response_info, HttpAuthController* auth_controller, base::OnceClosure restart_with_auth_callback);
    ...
    int SetSpdyHttpStreamOrBidirectionalStreamImpl(base::WeakPtr<SpdySession> session);
    ...
    HttpStreamRequest::StreamType stream_type_ = HttpStreamRequest::BIDIRECTIONAL_STREAM;
...
public:
    ...
    virtual std::unique_ptr<HttpStreamFactory::Job> CreateJob(HttpStreamFactory::Job::Delegate* delegate, HttpStreamFactory::JobType job_type,
        HttpNetworkSession* session, const StreamRequestInfo& request_info, RequestPriority priority, const ProxyInfo& proxy_info,
        ...
        bool enable_ip_based_pooling, NetLog* net_log, NextProto alternative_protocol, quic::ParsedQuicVersion quic_version);
};
