...
void RunTest(const char* const inputs[], size_t num_inputs, const char* expected_output, bool expected_eof, int bytes_after_eof)
{
...
void RunTestUntilFailure(const char* const inputs[], size_t num_inputs, size_t fail_index)
{
    HttpChunkedDecoder decoder;
    EXPECT_FALSE(decoder.reached_eof());
...
{
    const char* const inputs[] = { // This chunked body is not terminated.
...
{
    const char* const inputs[] = { // Compatibility [RFC 7230 - Invalid]:
...
TEST(HttpChunkedDecoderTest, ReallyBigChunks)
{
    ...
    for (int64_t chunk_length : kChunkLengths) {
        ...
        std::string chunk_header = base::StringPrintf("%" PRIx64 "\r\n", chunk_length);
        ...
        for (int64_t total_written = 0; total_written < chunk_length; total_written += kWrittenBytesPerIteration) {
            EXPECT_EQ(
                kWrittenBytesPerIteration, base::checked_cast<size_t>(decoder.FilterBuf(base::as_writable_byte_span(data).first(kWrittenBytesPerIteration))));
            EXPECT_FALSE(decoder.reached_eof());
        }
...
TEST(HttpChunkedDecoderTest, LongChunkLengthLine)
{
    ...
    const char* const inputs[] = { big_chunk.get(), "5" };
    RunTestUntilFailure(inputs, std::size(inputs), 1);
}
...
TEST(HttpChunkedDecoderTest, LongLengthLengthLine)
{
    ...
    memset(big_chunk.get(), '0', big_chunk_length);
    big_chunk[big_chunk_length] = 0;
    const char* const inputs[] = { "5;", big_chunk.get() };
    RunTestUntilFailure(inputs, std::size(inputs), 1);
}
