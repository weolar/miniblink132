...
int HttpChunkedDecoder::FilterBuf(base::span<uint8_t> buf)
{
    ...
    while (buf.size() > 0) {
        if (chunk_remaining_ > 0) {
            size_t num = std::min(base::saturated_cast<size_t>(chunk_remaining_), buf.size());
        ...
        base::span<const uint8_t> subspan = buf.subspan(base::checked_cast<size_t>(bytes_consumed));
        if (!subspan.empty()) {
...
//
//         Us: ^\X+[ ]*$
bool HttpChunkedDecoder::ParseChunkSize(base::span<const uint8_t> buf, uint64_t* out)
{
