...
{
    std::optional<std::string> range_header = headers.GetHeader(HttpRequestHeaders::kRange);
    if (!range_header) {
    ...
    if (!HttpUtil::ParseRangeHeader(range_header.value(), &ranges) || ranges.size() != 1) {
        return false;
    }
...
void PartialData::RestoreHeaders(HttpRequestHeaders* headers) const
{
    ...
    int64_t end = byte_range_.IsSuffixByteRange() ? byte_range_.suffix_length() : byte_range_.last_byte_position();
    ...
    *headers = extra_headers_;
    ...
    if (current_range_start_ < 0) {
        headers->SetHeader(HttpRequestHeaders::kRange, HttpByteRange::Suffix(end).GetHeaderValue());
    } else {
        headers->SetHeader(HttpRequestHeaders::kRange, HttpByteRange::Bounded(current_range_start_, end).GetHeaderValue());
    }
}

int PartialData::ShouldValidateCache(disk_cache::Entry* entry, CompletionOnceCallback callback)
{
    ...
    if (sparse_entry_) {
        ...
        disk_cache::RangeResultCallback cb = base::BindOnce(&PartialData::GetAvailableRangeCompleted, weak_factory_.GetWeakPtr());
        disk_cache::RangeResult range = entry->GetAvailableRange(current_range_start_, len, std::move(cb));

        cached_min_len_ = range.net_error == OK ? range.available_len : range.net_error;
...
void PartialData::PrepareCacheValidation(disk_cache::Entry* entry, HttpRequestHeaders* headers)
{
    DCHECK_GE(current_range_start_, 0);
    ...
    if (!len) {
        ...
        headers->SetHeader(HttpRequestHeaders::kRange, user_byte_range_.GetHeaderValue());
        return;
    }
    ...
    *headers = extra_headers_;
    ...
    headers->SetHeader(HttpRequestHeaders::kRange, HttpByteRange::Bounded(current_range_start_, current_range_end_).GetHeaderValue());
}
...
bool PartialData::UpdateFromStoredHeaders(const HttpResponseHeaders* headers, disk_cache::Entry* entry, bool truncated, bool writing_in_progress)
{
    resource_size_ = 0;
    if (truncated) {
...
// requested.
void PartialData::FixResponseHeaders(HttpResponseHeaders* headers, bool success)
{
    if (truncated_)
        return;
    ...
    if (!success) {
        ...
        headers->SetHeader(kRangeHeader, base::StringPrintf("bytes 0-0/%" PRId64, resource_size_));
    ...
    if (byte_range_.IsValid() && resource_size_) {
        ...
        headers->SetHeader(kLengthHeader, base::StringPrintf("%" PRId64, resource_size_));
    }
}
...
int PartialData::CacheRead(disk_cache::Entry* entry, IOBuffer* data, int data_len, CompletionOnceCallback callback)
{
    ...
    int rv = 0;
    if (sparse_entry_) {
        rv = entry->ReadSparseData(current_range_start_, data, read_len, std::move(callback));
    } else {
        if (current_range_start_ > std::numeric_limits<int32_t>::max())
            return ERR_INVALID_ARGUMENT;

        rv = entry->ReadData(kDataStream, static_cast<int>(current_range_start_), data, read_len, std::move(callback));
    }
    return rv;
}

int PartialData::CacheWrite(disk_cache::Entry* entry, IOBuffer* data, int data_len, CompletionOnceCallback callback)
{
    DVLOG(3) << "To write: " << data_len;
    if (sparse_entry_) {
        return entry->WriteSparseData(current_range_start_, data, data_len, std::move(callback));
    } else {
        if (current_range_start_ > std::numeric_limits<int32_t>::max())
            return ERR_INVALID_ARGUMENT;

        return entry->WriteData(kDataStream, static_cast<int>(current_range_start_), data, data_len, std::move(callback), true);
    }
}
...
int PartialData::GetNextRangeLen()
{
    ...
    int64_t range_len = byte_range_.HasLastBytePosition() ? byte_range_.last_byte_position() - current_range_start_ + 1 : std::numeric_limits<int32_t>::max();
    if (range_len > std::numeric_limits<int32_t>::max())
        range_len = std::numeric_limits<int32_t>::max();
    return static_cast<int32_t>(range_len);
}

void PartialData::GetAvailableRangeCompleted(const disk_cache::RangeResult& result)
{
    DCHECK(!callback_.is_null());
    DCHECK_NE(ERR_IO_PENDING, result.net_error);

    int len_or_error = result.net_error == OK ? result.available_len : result.net_error;
