...
public:
    ...
    virtual HttpAuth::AuthorizationResult ParseChallenge(HttpAuthChallengeTokenizer* tok) = 0;
    ...
    virtual int GenerateAuthToken(const AuthCredentials* credentials, const std::string& spn, const std::string& channel_bindings, std::string* auth_token,
...
using HttpAuthMechanismFactory = base::RepeatingCallback<std::unique_ptr<HttpAuthMechanism>(const HttpAuthPreferences*)>;
