...
HttpBasicStream::HttpBasicStream(std::unique_ptr<StreamSocketHandle> connection, bool is_for_get_to_http_proxy)
...
int HttpBasicStream::InitializeStream(bool can_send_early, RequestPriority priority, const NetLogWithSource& net_log, CompletionOnceCallback callback)
{
    ...
    if (!can_send_early) {
        ...
        ret = parser()->ConfirmHandshake(base::BindOnce(&HttpBasicStream::OnHandshakeConfirmed, base::Unretained(this), std::move(callback)));
    }
    return ret;
}

int HttpBasicStream::SendRequest(const HttpRequestHeaders& headers, HttpResponseInfo* response, CompletionOnceCallback callback)
{
    DCHECK(parser());
    ...
    return parser()->SendRequest(
        state_.GenerateRequestLine(), headers, NetworkTrafficAnnotationTag(state_.traffic_annotation()), response, std::move(callback));
}
...
int HttpBasicStream::ReadResponseBody(IOBuffer* buf, int buf_len, CompletionOnceCallback callback)
{
...
std::unique_ptr<HttpStream> HttpBasicStream::RenewStreamForAuth()
{
    ...
    return std::make_unique<HttpBasicStream>(state_.ReleaseConnection(), state_.is_for_get_to_http_proxy());
}
...
bool HttpBasicStream::GetLoadTimingInfo(LoadTimingInfo* load_timing_info) const
{
    ...
    // include that time.
    if (!load_timing_info->connect_timing.ssl_end.is_null() && !confirm_handshake_end_.is_null()) {
    ...
    load_timing_info->receive_headers_start = parser()->first_response_start_time();
    load_timing_info->receive_non_informational_headers_start = parser()->non_informational_response_start_time();
    load_timing_info->first_early_hints_time = parser()->first_early_hints_time();
    return true;
}

bool HttpBasicStream::GetAlternativeService(AlternativeService* alternative_service) const
{
    return false;
}
...
void HttpBasicStream::SetRequestHeadersCallback(RequestHeadersCallback callback)
{
...
void HttpBasicStream::OnHandshakeConfirmed(CompletionOnceCallback callback, int rv)
{
    if (rv == OK) {
