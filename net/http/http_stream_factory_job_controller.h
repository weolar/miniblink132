...
class HttpStreamFactory::JobController : public HttpStreamFactory::Job::Delegate, public HttpStreamRequest::Helper {
public:
    JobController(HttpStreamFactory* factory, HttpStreamRequest::Delegate* delegate, HttpNetworkSession* session, JobFactory* job_factory,
        const HttpRequestInfo& http_request_info, bool is_preconnect, bool is_websocket, bool enable_ip_based_pooling, bool enable_alternative_services,
        bool delay_main_job_with_available_spdy_session, const std::vector<SSLConfig::CertAndStatus>& allowed_bad_certs);

    ~JobController() override;

    // Used in tests only for verification purpose.
    const Job* main_job() const
    {
        return main_job_.get();
    }
    const Job* alternative_job() const
    {
        return alternative_job_.get();
    }
    const Job* dns_alpn_h3_job() const
    {
        return dns_alpn_h3_job_.get();
    }
    ...
    std::unique_ptr<HttpStreamRequest> Start(HttpStreamRequest::Delegate* delegate,
    ...
    void OnBidirectionalStreamImplReady(Job* job, const ProxyInfo& used_proxy_info) override;

    // Invoked when |job| has a WebSocketHandshakeStream ready.
    void OnWebSocketHandshakeStreamReady(Job* job, const ProxyInfo& used_proxy_info, std::unique_ptr<WebSocketHandshakeStreamBase> stream) override;

    // Invoked when a QUIC job finished a DNS resolution.
    void OnQuicHostResolution(
        const url::SchemeHostPort& destination, base::TimeTicks dns_resolution_start_time, base::TimeTicks dns_resolution_end_time) override;
    ...
    void OnNeedsProxyAuth(Job* job, const HttpResponseInfo& proxy_response, const ProxyInfo& used_proxy_info, HttpAuthController* auth_controller) override;
    ...
    void AddConnectionAttemptsToRequest(Job* job, const ConnectionAttempts& attempts) override;
    ...
    WebSocketHandshakeStreamBase::CreateHelper* websocket_handshake_stream_create_helper() override;
...
private:
    ...
    enum State { STATE_RESOLVE_PROXY, STATE_RESOLVE_PROXY_COMPLETE, STATE_CREATE_JOBS, STATE_NONE };
    ...
    AlternativeServiceInfo GetAlternativeServiceInfoFor(const GURL& http_request_info_url, const StreamRequestInfo& request_info,
        HttpStreamRequest::Delegate* delegate, HttpStreamRequest::StreamType stream_type);

    AlternativeServiceInfo GetAlternativeServiceInfoInternal(const GURL& http_request_info_url, const StreamRequestInfo& request_info,
        HttpStreamRequest::Delegate* delegate, HttpStreamRequest::StreamType stream_type);

    // Just calls QuicContext::SelectQuicVersion().
    quic::ParsedQuicVersion SelectQuicVersion(const quic::ParsedQuicVersionVector& advertised_versions);
    ...
    void ReportAlternateProtocolUsage(AlternateProtocolUsage alternate_protocol_usage, bool is_google_host) const;
    ...
    {
        return (main_job_ ? 1 : 0) + (alternative_job_ ? 1 : 0) + (dns_alpn_h3_job_ ? 1 : 0);
    }
    ...
    void CallOnSwitchesToHttpStreamPool(HttpStreamKey stream_key, AlternativeServiceInfo alternative_service_info);
