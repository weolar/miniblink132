...
{
    auto result = mock_transactions.insert(std::make_pair(GURL(trans->url).spec(), trans));
...
static const MockTransaction* const kBuiltinMockTransactions[]
    = { &kSimpleGET_Transaction, &kSimplePOST_Transaction, &kTypicalGET_Transaction, &kETagGET_Transaction, &kRangeGET_Transaction };
...
ScopedMockTransaction::ScopedMockTransaction(const MockTransaction& t, const char* url)
    : MockTransaction(t)
{
    if (url) {
        this->url = url;
...
TestTransactionConsumer::TestTransactionConsumer(RequestPriority priority, HttpTransactionFactory* factory)
{
...
void TestTransactionConsumer::Start(const HttpRequestInfo* request, const NetLogWithSource& net_log)
{
    state_ = State::kStarting;
    int result = trans_->Start(request, base::BindOnce(&TestTransactionConsumer::OnIOComplete, base::Unretained(this)), net_log);
...
void TestTransactionConsumer::Read()
{
    ...
    int result = trans_->Read(read_buf_.get(), 1024, base::BindOnce(&TestTransactionConsumer::OnIOComplete, base::Unretained(this)));
    if (result != ERR_IO_PENDING)
        DidRead(result);
}
...
MockNetworkTransaction::MockNetworkTransaction(RequestPriority priority, MockNetworkLayer* factory)
    : priority_(priority)
...
int MockNetworkTransaction::Start(const HttpRequestInfo* request, CompletionOnceCallback callback, const NetLogWithSource& net_log)
{
    net_log_ = net_log;
...
int MockNetworkTransaction::RestartIgnoringLastError(CompletionOnceCallback callback)
{
    return ERR_FAILED;
}
...
int MockNetworkTransaction::RestartWithAuth(const AuthCredentials& credentials, CompletionOnceCallback callback)
{
...
void MockNetworkTransaction::PopulateNetErrorDetails(NetErrorDetails* /*details*/) const
{
    NOTIMPLEMENTED();
}
...
int MockNetworkTransaction::Read(IOBuffer* buf, int buf_len, CompletionOnceCallback callback)
{
...
void MockNetworkTransaction::SetQuicServerInfo(QuicServerInfo* quic_server_info)
{
}

bool MockNetworkTransaction::GetLoadTimingInfo(LoadTimingInfo* load_timing_info) const
{
...
void MockNetworkTransaction::SetWebSocketHandshakeStreamCreateHelper(WebSocketHandshakeStreamBase::CreateHelper* create_helper)
{
...
int MockNetworkTransaction::StartInternal(HttpRequestInfo request, CompletionOnceCallback callback)
{
...
int MockNetworkTransaction::DoCreateStream()
{
    ...
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
        FROM_HERE, base::BindOnce(&MockNetworkTransaction::OnIOComplete, weak_factory_.GetWeakPtr(), OK));
    return ERR_IO_PENDING;
}
...
int MockNetworkTransaction::DoConnectedCallback()
{
    ...
    const MockTransaction* t = FindMockTransaction(current_request_.url);
    CHECK(t);
    return connected_callback_.Run(t->transport_info, base::BindOnce(&MockNetworkTransaction::OnIOComplete, weak_factory_.GetWeakPtr()));
}
...
int MockNetworkTransaction::DoSendRequest()
{
    ...
    std::string header_data = base::StringPrintf("%s\n%s\n", resp_status.c_str(), resp_headers.c_str());
    ...
    if (t->transport_info.type == TransportType::kDirect) {
        ...
        response_.proxy_chain
            = ProxyChain::FromSchemeHostAndPort(ProxyServer::SCHEME_HTTP, t->transport_info.endpoint.ToStringWithoutPort(), t->transport_info.endpoint.port());
    }
...
void MockNetworkTransaction::SetBeforeNetworkStartCallback(BeforeNetworkStartCallback callback)
{
    before_network_start_callback_ = std::move(callback);
}

void MockNetworkTransaction::SetModifyRequestHeadersCallback(base::RepeatingCallback<void(HttpRequestHeaders*)> callback)
{
    modify_request_headers_callback_ = std::move(callback);
}

void MockNetworkTransaction::SetConnectedCallback(const ConnectedCallback& callback)
{
...
void MockNetworkTransaction::CallbackLater(CompletionOnceCallback callback, int result)
{
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
        FROM_HERE, base::BindOnce(&MockNetworkTransaction::RunCallback, weak_factory_.GetWeakPtr(), std::move(callback), result));
}

void MockNetworkTransaction::RunCallback(CompletionOnceCallback callback, int result)
{
...
int MockNetworkLayer::CreateTransaction(RequestPriority priority, std::unique_ptr<HttpTransaction>* trans)
{
    transaction_count_++;
    last_create_transaction_priority_ = priority;
    auto mock_transaction = std::make_unique<MockNetworkTransaction>(priority, this);
...
ConnectedHandler& ConnectedHandler::operator=(const ConnectedHandler&) = default;
...
int ConnectedHandler::OnConnected(const TransportInfo& info, CompletionOnceCallback callback)
{
    transports_.push_back(info);
    if (run_callback_) {
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(std::move(callback), result_));
        return ERR_IO_PENDING;
    }
    return result_;
}
