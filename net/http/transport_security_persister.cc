...
BASE_FEATURE(kTransportSecurityFileWriterSchedule, "TransportSecurityFileWriterSchedule", base::FEATURE_ENABLED_BY_DEFAULT);
...
const base::FeatureParam<base::TimeDelta> kCommitIntervalParam(&kTransportSecurityFileWriterSchedule, "commit_interval", kMinCommitInterval);
...
std::string HashedDomainToExternalString(const TransportSecurityState::HashedHost& hashed)
{
...
std::optional<TransportSecurityState::HashedHost> ExternalStringToHashedDomain(const std::string& external)
{
...
base::Value::List SerializeSTSData(const TransportSecurityState* state)
{
    ...
    for (; sts_iterator.HasNext(); sts_iterator.Advance()) {
        ...
        serialized.Set(kHostname, HashedDomainToExternalString(sts_iterator.hostname()));
        serialized.Set(kStsIncludeSubdomains, sts_state.include_subdomains);
        serialized.Set(kStsObserved, sts_state.last_observed.InSecondsFSinceUnixEpoch());
...
void DeserializeSTSData(const base::Value& sts_list, TransportSecurityState* state)
{
    if (!sts_list.is_list())
        return;
    ...
    for (const base::Value& sts_entry : sts_list.GetList()) {
        ...
        std::optional<bool> sts_include_subdomains = sts_dict->FindBool(kStsIncludeSubdomains);
        ...
        if (!hostname || !sts_include_subdomains.has_value() || !sts_observed.has_value() || !expiry.has_value() || !mode) {
            continue;
        }

        TransportSecurityState::STSState sts_state;
        sts_state.include_subdomains = *sts_include_subdomains;
        sts_state.last_observed = base::Time::FromSecondsSinceUnixEpoch(*sts_observed);
        ...
        std::optional<TransportSecurityState::HashedHost> hashed = ExternalStringToHashedDomain(*hostname);
        if (!hashed.has_value())
            continue;
...
void OnWriteFinishedTask(scoped_refptr<base::SequencedTaskRunner> task_runner, base::OnceClosure callback, bool result)
{
...
TransportSecurityPersister::TransportSecurityPersister(
    TransportSecurityState* state, const scoped_refptr<base::SequencedTaskRunner>& background_runner, const base::FilePath& data_path)
    : transport_security_state_(state)
    , writer_(data_path, background_runner, GetCommitInterval(), kHistogramSuffix)
    ...
    , background_runner_(background_runner)
{
    ...
    background_runner_->PostTaskAndReplyWithResult(
        FROM_HERE, base::BindOnce(&LoadState, writer_.path()), base::BindOnce(&TransportSecurityPersister::CompleteLoad, weak_ptr_factory_.GetWeakPtr()));
}
...
void TransportSecurityPersister::WriteNow(TransportSecurityState* state, base::OnceClosure callback)
{
    DCHECK(foreground_runner_->RunsTasksInCurrentSequence());
    DCHECK_EQ(transport_security_state_, state);

    writer_.RegisterOnNextWriteCallbacks(base::OnceClosure(),
        base::BindOnce(&OnWriteFinishedTask, foreground_runner_,
            base::BindOnce(&TransportSecurityPersister::OnWriteFinished, weak_ptr_factory_.GetWeakPtr(), std::move(callback))));
...
void TransportSecurityPersister::LoadEntries(const std::string& serialized)
{
    ...
    Deserialize(serialized, transport_security_state_, contains_legacy_expect_ct_data);
...
{
    return std::clamp(kCommitIntervalParam.Get(), kMinCommitInterval, kMaxCommitInterval);
}

void TransportSecurityPersister::Deserialize(const std::string& serialized, TransportSecurityState* state, bool& contains_legacy_expect_ct_data)
{
