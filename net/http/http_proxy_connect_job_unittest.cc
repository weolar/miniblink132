...
const ProxyServer kHttpProxyServer { ProxyServer::SCHEME_HTTP, HostPortPair(kHttpProxyHost, 80) };
const ProxyServer kHttpsProxyServer { ProxyServer::SCHEME_HTTPS, HostPortPair(kHttpsProxyHost, 443) };
const ProxyServer kHttpsNestedProxyServer { ProxyServer::SCHEME_HTTPS, HostPortPair(kHttpsNestedProxyHost, 443) };
...
const ProxyChain kHttpsNestedProxyChain = ProxyChain::ForIpProtection({ { kHttpsProxyServer, kHttpsNestedProxyServer } });
...
MATCHER_P(QSRHasProxyChain, proxy_chain,
    base::StringPrintf("QuicStreamRequest %s ProxyChain %s", negation ? "does not have" : "has", proxy_chain.ToDebugString().c_str()))
{
    *result_listener << "where the proxy chain is " << arg->session_key().proxy_chain().ToDebugString();
    return arg->session_key().proxy_chain() == proxy_chain;
}

MATCHER_P(
    IsQuicVersion, quic_version, base::StringPrintf("QUIC version %s %s", negation ? "is not" : "is", quic::ParsedQuicVersionToString(quic_version).c_str()))
{
    *result_listener << "where the QUIC version is " << quic::ParsedQuicVersionToString(arg);
...
public:
        ...
        : WithTaskEnvironment(base::test::TaskEnvironment::TimeSource::MOCK_TIME)
    {
        ...
        network_quality_estimator_ = std::make_unique<TestNetworkQualityEstimator>();
    ...
    {
        common_connect_job_params_ = std::make_unique<CommonConnectJobParams>(session_->CreateCommonConnectJobParams());
        ...
        common_connect_job_params_->network_quality_estimator = network_quality_estimator_.get();
    }
...
class HttpProxyConnectJobTest : public HttpProxyConnectJobTestBase, public ::testing::TestWithParam<HttpProxyType> {
public:
    ...
    void InitAdaptiveTimeoutFieldTrialWithParams(bool use_default_params, int ssl_http_rtt_multiplier, int non_ssl_http_rtt_multiplier,
        base::TimeDelta min_proxy_connection_timeout, base::TimeDelta max_proxy_connection_timeout)
    {
        ...
        if (!use_default_params) {
            params["ssl_http_rtt_multiplier"] = base::NumberToString(ssl_http_rtt_multiplier);
            params["non_ssl_http_rtt_multiplier"] = base::NumberToString(non_ssl_http_rtt_multiplier);
            params["min_proxy_connection_timeout_seconds"] = base::NumberToString(min_proxy_connection_timeout.InSeconds());
            params["max_proxy_connection_timeout_seconds"] = base::NumberToString(max_proxy_connection_timeout.InSeconds());
        }
        base::FieldTrialParamAssociator::GetInstance()->ClearAllParamsForTesting();
    ...
    scoped_refptr<TransportSocketParams> CreateHttpProxyParams(SecureDnsPolicy secure_dns_policy) const
    {
        if (GetParam() != HTTP) {
            return nullptr;
        }
        return base::MakeRefCounted<TransportSocketParams>(kHttpProxyServer.host_port_pair(), NetworkAnonymizationKey(), secure_dns_policy,
            OnHostResolutionCallback(),
            /*supported_alpns=*/base::flat_set<std::string>());
    }

    scoped_refptr<SSLSocketParams> CreateHttpsProxyParams(SecureDnsPolicy secure_dns_policy) const
    {
        if (GetParam() == HTTP) {
            return nullptr;
        }
        return base::MakeRefCounted<SSLSocketParams>(ConnectJobParams(base::MakeRefCounted<TransportSocketParams>(kHttpsProxyServer.host_port_pair(),
                                                         NetworkAnonymizationKey(), secure_dns_policy, OnHostResolutionCallback(),
                                                         /*supported_alpns=*/base::flat_set<std::string>())),
            HostPortPair(kHttpsProxyHost, 443), SSLConfig(), NetworkAnonymizationKey());
    }

    // Returns a correctly constructed HttpProxyParams for a single HTTP or HTTPS
    // proxy.
    scoped_refptr<HttpProxySocketParams> CreateParams(bool tunnel, SecureDnsPolicy secure_dns_policy)
    {
        ...
        return base::MakeRefCounted<HttpProxySocketParams>(std::move(params), HostPortPair(kEndpointHost, tunnel ? 443 : 80),
            GetParam() == HTTP ? kHttpProxyChain : kHttpsProxyChain,
            /*proxy_chain_index=*/0, tunnel, TRAFFIC_ANNOTATION_FOR_TESTS, NetworkAnonymizationKey(), secure_dns_policy);
    }
    ...
    scoped_refptr<SSLSocketParams> CreateNestedHttpsProxyParams(
        bool tunnel, SecureDnsPolicy secure_dns_policy, const ProxyChain& proxy_chain, size_t proxy_chain_index) const
    {
        DCHECK_NE(GetParam(), HTTP);

        const ProxyServer& proxy_server = proxy_chain.GetProxyServer(proxy_chain_index);
        ...
        if (proxy_chain_index != 0) {
            ...
            return base::MakeRefCounted<SSLSocketParams>(
                ConnectJobParams(CreateNestedParams(tunnel, secure_dns_policy, proxy_chain, previous_hop_proxy_chain_index)), proxy_server.host_port_pair(),
                SSLConfig(), NetworkAnonymizationKey());
        }

        // If we are creating the SSLSocketParams for the first hop, establish a
        // direct encrypted connection to it.
        return base::MakeRefCounted<SSLSocketParams>(ConnectJobParams(base::MakeRefCounted<TransportSocketParams>(proxy_server.host_port_pair(),
                                                         NetworkAnonymizationKey(), secure_dns_policy, OnHostResolutionCallback(),
                                                         /*supported_alpns=*/base::flat_set<std::string>())),
        ...
        bool tunnel, SecureDnsPolicy secure_dns_policy, const ProxyChain& proxy_chain, size_t proxy_chain_index) const
    {
        ...
        scoped_refptr<SSLSocketParams> ssl_params = CreateNestedHttpsProxyParams(tunnel, secure_dns_policy, proxy_chain, proxy_chain_index);
        if (proxy_chain_index + 1 != proxy_chain.length()) {
            ...
            const ProxyServer& next_hop_proxy_server = proxy_chain.GetProxyServer(next_hop_proxy_chain_index);
        ...
        return base::MakeRefCounted<HttpProxySocketParams>(ConnectJobParams(std::move(ssl_params)), connect_host_port_pair, proxy_chain, proxy_chain_index,
            tunnel, TRAFFIC_ANNOTATION_FOR_TESTS, NetworkAnonymizationKey(), secure_dns_policy);
    }

    std::unique_ptr<HttpProxyConnectJob> CreateConnectJobForHttpRequest(
        ConnectJob::Delegate* delegate, RequestPriority priority = DEFAULT_PRIORITY, SecureDnsPolicy secure_dns_policy = SecureDnsPolicy::kAllow)
    {
        return CreateConnectJob(CreateParams(false /* tunnel */, secure_dns_policy), delegate, priority);
    }

    std::unique_ptr<HttpProxyConnectJob> CreateConnectJobForTunnel(
        ConnectJob::Delegate* delegate, RequestPriority priority = DEFAULT_PRIORITY, SecureDnsPolicy secure_dns_policy = SecureDnsPolicy::kAllow)
    {
        return CreateConnectJob(CreateParams(true /* tunnel */, secure_dns_policy), delegate, priority);
    }
        ...
        ConnectJob::Delegate* delegate, RequestPriority priority = DEFAULT_PRIORITY, SecureDnsPolicy secure_dns_policy = SecureDnsPolicy::kAllow)
    {
        ...
        return CreateConnectJob(
            CreateNestedParams(/*tunnel=*/true, secure_dns_policy, kHttpsNestedProxyChain, last_hop_proxy_server_index), delegate, priority);
    }

    std::unique_ptr<HttpProxyConnectJob> CreateConnectJob(
        scoped_refptr<HttpProxySocketParams> http_proxy_socket_params, ConnectJob::Delegate* delegate, RequestPriority priority)
    {
        return std::make_unique<HttpProxyConnectJob>(
            priority, SocketTag(), common_connect_job_params_.get(), std::move(http_proxy_socket_params), delegate, /*net_log=*/nullptr);
    }

    void Initialize(base::span<const MockRead> reads, base::span<const MockWrite> writes, base::span<const MockRead> spdy_reads,
        base::span<const MockWrite> spdy_writes, IoMode connect_and_ssl_io_mode, bool two_ssl_proxies = false)
    {
        if (GetParam() == SPDY) {
        ...
        if (GetParam() != HTTP) {
            ...
            ssl_data_ = std::make_unique<SSLSocketDataProvider>(connect_and_ssl_io_mode, OK);
            if (GetParam() == SPDY) {
        ...
        if (two_ssl_proxies) {
            ...
            // the proxy.
            nested_second_proxy_ssl_data_ = std::make_unique<SSLSocketDataProvider>(connect_and_ssl_io_mode, OK);
            if (GetParam() == SPDY) {
                InitializeSpdySsl(nested_second_proxy_ssl_data_.get());
            }
            session_deps_.socket_factory->AddSSLSocketDataProvider(nested_second_proxy_ssl_data_.get());
        }
    }
    ...
    {
        base::TimeDelta normal_nested_connection_timeout = TransportConnectJob::ConnectionTimeout();
        if (GetParam() != HTTP) {
            normal_nested_connection_timeout += SSLConnectJob::HandshakeTimeoutForTesting();
        }
        ...
        base::TimeDelta alternate_connection_timeout = HttpProxyConnectJob::AlternateNestedConnectionTimeout(
            *CreateParams(true /* tunnel */, SecureDnsPolicy::kAllow), network_quality_estimator_.get());
        ...
        // normal timeout.
        if (!alternate_connection_timeout.is_zero() && alternate_connection_timeout < normal_nested_connection_timeout) {
...
// and SPDY.
INSTANTIATE_TEST_SUITE_P(HttpProxyType, HttpProxyConnectJobTest, ::testing::Values(HTTP, HTTPS, SPDY));
...
TEST_P(HttpProxyConnectJobTest, NoTunnel)
{
    ...
    for (IoMode io_mode : { SYNCHRONOUS, ASYNC }) {
        ...
        Initialize(base::span<MockRead>(), base::span<MockWrite>(), base::span<MockRead>(), base::span<MockWrite>(), io_mode);

        TestConnectJobDelegate test_delegate;
        std::unique_ptr<ConnectJob> connect_job = CreateConnectJobForHttpRequest(&test_delegate);
        test_delegate.StartJobExpectingResult(connect_job.get(), OK, io_mode == SYNCHRONOUS);
        ...
        histogram_tester.ExpectTotalCount("Net.HttpProxy.ConnectLatency.Http1.Http.Success", (!is_secure && !is_http2) ? 1 : 0);
        histogram_tester.ExpectTotalCount("Net.HttpProxy.ConnectLatency.Http1.Https.Success", (is_secure && !is_http2) ? 1 : 0);
        histogram_tester.ExpectTotalCount("Net.HttpProxy.ConnectLatency.Http2.Https.Success", (is_secure && is_http2) ? 1 : 0);
    }
}
...
TEST_P(HttpProxyConnectJobTest, HasEstablishedConnectionTunnel)
{
    ...
    spdy::SpdySerializedFrame req(spdy_util_.ConstructSpdyConnect(nullptr, 0, 1, HttpProxyConnectJob::kH2QuicTunnelPriority, HostPortPair(kEndpointHost, 443)));
    ...
    SSLSocketDataProvider ssl_data(ASYNC, OK);
    ssl_data.ssl_info.cert = ImportCertFromFile(GetTestCertsDirectory(), "ok_cert.pem");
    ...
    TestConnectJobDelegate test_delegate;
    std::unique_ptr<ConnectJob> connect_job = CreateConnectJobForTunnel(&test_delegate);
    ...
    EXPECT_EQ(test_delegate.socket()->GetNegotiatedProtocol(), NextProto::kProtoUnknown);
}
...
TEST_P(HttpProxyConnectJobTest, ProxyDelegateExtraHeaders)
{
    ...
    ProxyServer proxy_server(GetParam() == HTTP ? ProxyServer::SCHEME_HTTP : ProxyServer::SCHEME_HTTPS,
        HostPortPair(GetParam() == HTTP ? kHttpProxyHost : kHttpsProxyHost, GetParam() == HTTP ? 80 : 443));
    std::string proxy_server_uri = ProxyServerToProxyUri(proxy_server);

    std::string http1_request = base::StringPrintf("CONNECT www.endpoint.test:443 HTTP/1.1\r\n"
                                                   "Host: www.endpoint.test:443\r\n"
                                                   "Proxy-Connection: keep-alive\r\n"
                                                   "User-Agent: test-ua\r\n"
                                                   "%s: %s\r\n\r\n",
    ...
    std::string http1_response = base::StringPrintf("HTTP/1.1 200 Connection Established\r\n"
                                                    "%s: %s\r\n\r\n",
    ...
    spdy::SpdySerializedFrame req(spdy_util_.ConstructSpdyConnect(
        kExtraRequestHeaders, std::size(kExtraRequestHeaders) / 2, 1, HttpProxyConnectJob::kH2QuicTunnelPriority, HostPortPair(kEndpointHost, 443)));
    MockWrite spdy_writes[] = { CreateMockWrite(req, 0) };
    spdy::SpdySerializedFrame resp(spdy_util_.ConstructSpdyGetReply(kExtraResponseHeaders, std::size(kExtraResponseHeaders) / 2, 1));
    MockRead spdy_reads[] = {
    ...
    TestConnectJobDelegate test_delegate;
    std::unique_ptr<ConnectJob> connect_job = CreateConnectJobForTunnel(&test_delegate);
    test_delegate.StartJobExpectingResult(connect_job.get(), OK, false /* expect_sync_result */);

    ASSERT_EQ(proxy_delegate_->on_tunnel_headers_received_call_count(), 1u);
    proxy_delegate_->VerifyOnTunnelHeadersReceived(ProxyChain(proxy_server), 0, kResponseHeaderName, kResponseHeaderValue);
}
...
TEST_P(HttpProxyConnectJobTest, NestedProxyProxyDelegateExtraHeaders)
{
    ...
    const ProxyServer& first_hop_proxy_server = kHttpsNestedProxyChain.GetProxyServer(/*chain_index=*/0);
    const ProxyServer& second_hop_proxy_server = kHttpsNestedProxyChain.GetProxyServer(/*chain_index=*/1);

    std::string first_hop_proxy_server_uri = ProxyServerToProxyUri(first_hop_proxy_server);
    std::string second_hop_proxy_server_uri = ProxyServerToProxyUri(second_hop_proxy_server);

    std::string first_hop_http1_request = base::StringPrintf("CONNECT last-hop-https-proxy.example.test:443 HTTP/1.1\r\n"
                                                             "Host: last-hop-https-proxy.example.test:443\r\n"
                                                             "Proxy-Connection: keep-alive\r\n"
                                                             "User-Agent: test-ua\r\n"
                                                             "%s: %s\r\n\r\n",
        kTestHeaderName, first_hop_proxy_server_uri.c_str());
    std::string second_hop_http1_request = base::StringPrintf("CONNECT www.endpoint.test:443 HTTP/1.1\r\n"
                                                              "Host: www.endpoint.test:443\r\n"
                                                              "Proxy-Connection: keep-alive\r\n"
                                                              "User-Agent: test-ua\r\n"
                                                              "%s: %s\r\n\r\n",
    ...
    const char kResponseHeaderName[] = "bar";
    std::string first_hop_http1_response = base::StringPrintf("HTTP/1.1 200 Connection Established\r\n"
                                                              "%s: %s\r\n\r\n",
        kResponseHeaderName, first_hop_proxy_server_uri.c_str());

    std::string second_hop_http1_response = base::StringPrintf("HTTP/1.1 200 Connection Established\r\n"
                                                               "%s: %s\r\n\r\n",
    ...
    spdy::SpdySerializedFrame first_hop_req(spdy_util_.ConstructSpdyConnect(kFirstHopExtraRequestHeaders, std::size(kFirstHopExtraRequestHeaders) / 2, 1,
        HttpProxyConnectJob::kH2QuicTunnelPriority, second_hop_proxy_server.host_port_pair()));

    spdy::SpdySerializedFrame first_hop_resp(spdy_util_.ConstructSpdyGetReply(kFirstHopExtraResponseHeaders, std::size(kFirstHopExtraResponseHeaders) / 2, 1));
    ...
    spdy::SpdySerializedFrame second_hop_req(new_spdy_util.ConstructSpdyConnect(kSecondHopExtraRequestHeaders, std::size(kSecondHopExtraRequestHeaders) / 2, 1,
        HttpProxyConnectJob::kH2QuicTunnelPriority, HostPortPair(kEndpointHost, 443)));
    ...
    // frames.
    spdy::SpdySerializedFrame wrapped_second_hop_req(spdy_util_.ConstructWrappedSpdyFrame(second_hop_req, 1));

    spdy::SpdySerializedFrame second_hop_resp(
        new_spdy_util.ConstructSpdyGetReply(kSecondHopExtraResponseHeaders, std::size(kSecondHopExtraResponseHeaders) / 2, 1));

    spdy::SpdySerializedFrame wrapped_second_hop_resp(spdy_util_.ConstructWrappedSpdyFrame(second_hop_resp, 1));
    ...
    TestConnectJobDelegate test_delegate;
    std::unique_ptr<ConnectJob> connect_job = CreateConnectJobForNestedProxyTunnel(&test_delegate);
    ...
    proxy_delegate_->VerifyOnTunnelHeadersReceived(
        kHttpsNestedProxyChain, /*chain_index=*/0, kResponseHeaderName, first_hop_proxy_server_uri, /*call_index=*/0);
    proxy_delegate_->VerifyOnTunnelHeadersReceived(
        kHttpsNestedProxyChain, /*chain_index=*/1, kResponseHeaderName, second_hop_proxy_server_uri, /*call_index=*/1);
}
...
TEST_P(HttpProxyConnectJobTest, NeedAuth)
{
    for (IoMode io_mode : { SYNCHRONOUS, ASYNC }) {
        ...
        spdy::SpdySerializedFrame connect(
            spdy_util.ConstructSpdyConnect(nullptr, 0, 1, HttpProxyConnectJob::kH2QuicTunnelPriority, HostPortPair(kEndpointHost, 443)));
        spdy::SpdySerializedFrame rst(spdy_util.ConstructSpdyRstStream(1, spdy::ERROR_CODE_CANCEL));
        ...
        spdy::SpdySerializedFrame connect2(spdy_util.ConstructSpdyConnect(
            kSpdyAuthCredentials, std::size(kSpdyAuthCredentials) / 2, 3, HttpProxyConnectJob::kH2QuicTunnelPriority, HostPortPair(kEndpointHost, 443)));
        ...
        spdy::SpdySerializedFrame connect2_resp(spdy_util.ConstructSpdyGetReply(nullptr, 0, 3));
        MockRead spdy_reads[] = {
        ...
        ASSERT_TRUE(test_delegate.auth_response_info().headers->EnumerateHeader(nullptr, "Proxy-Authenticate", &proxy_authenticate));
...
TEST_P(HttpProxyConnectJobTest, NeedAuthTwice)
{
    for (IoMode io_mode : { SYNCHRONOUS, ASYNC }) {
        ...
        spdy::SpdySerializedFrame connect2(spdy_util.ConstructSpdyConnect(
            kSpdyAuthCredentials, std::size(kSpdyAuthCredentials) / 2, 3, HttpProxyConnectJob::kH2QuicTunnelPriority, HostPortPair(kEndpointHost, 443)));
        spdy::SpdySerializedFrame rst2(spdy_util.ConstructSpdyRstStream(3, spdy::ERROR_CODE_CANCEL));
        spdy_util.UpdateWithStreamDestruction(3);

        spdy::SpdySerializedFrame connect3(spdy_util.ConstructSpdyConnect(
            kSpdyAuthCredentials, std::size(kSpdyAuthCredentials) / 2, 5, HttpProxyConnectJob::kH2QuicTunnelPriority, HostPortPair(kEndpointHost, 443)));
        MockWrite spdy_writes[] = {
            CreateMockWrite(connect, 0, io_mode),
            CreateMockWrite(rst, 2, io_mode),
            CreateMockWrite(connect2, 3, io_mode),
        ...
        spdy::SpdySerializedFrame connect_auth_resp(spdy_util.ConstructSpdyReplyError(kAuthStatus, kAuthChallenge, std::size(kAuthChallenge) / 2, 1));
        spdy::SpdySerializedFrame connect2_auth_resp(spdy_util.ConstructSpdyReplyError(kAuthStatus, kAuthChallenge, std::size(kAuthChallenge) / 2, 3));
        spdy::SpdySerializedFrame connect3_resp(spdy_util.ConstructSpdyGetReply(nullptr, 0, 5));
        MockRead spdy_reads[] = {
            CreateMockRead(connect_auth_resp, 1, ASYNC),
...
TEST_P(HttpProxyConnectJobTest, HaveAuth)
{
    ...
    url::SchemeHostPort proxy_scheme_host_port(
        GetParam() == HTTP ? GURL(std::string("http://") + kHttpProxyHost) : GURL(std::string("https://") + kHttpsProxyHost));
    session_->http_auth_cache()->Add(proxy_scheme_host_port, HttpAuth::AUTH_PROXY, "MyRealm1", HttpAuth::AUTH_SCHEME_BASIC, NetworkAnonymizationKey(),
    ...
    for (IoMode io_mode : { SYNCHRONOUS, ASYNC }) {
        ...
        spdy::SpdySerializedFrame connect(spdy_util.ConstructSpdyConnect(
            kSpdyAuthCredentials, std::size(kSpdyAuthCredentials) / 2, 1, HttpProxyConnectJob::kH2QuicTunnelPriority, HostPortPair(kEndpointHost, 443)));

        MockWrite spdy_writes[] = {
            CreateMockWrite(connect, 0, ASYNC),
        };

        spdy::SpdySerializedFrame connect_resp(spdy_util.ConstructSpdyGetReply(nullptr, 0, 1));
        MockRead spdy_reads[] = {
        ...
        test_delegate.StartJobExpectingResult(connect_job.get(), OK, io_mode == SYNCHRONOUS && GetParam() != SPDY);
...
{
    session_deps_.host_resolver->rules()->AddSimulatedTimeoutFailure(kHttpProxyHost);
    session_deps_.host_resolver->rules()->AddSimulatedTimeoutFailure(kHttpsProxyHost);

    TestConnectJobDelegate test_delegate;
    std::unique_ptr<ConnectJob> connect_job = CreateConnectJobForHttpRequest(&test_delegate, DEFAULT_PRIORITY);
    test_delegate.StartJobExpectingResult(connect_job.get(), ERR_PROXY_CONNECTION_FAILED, false /* expect_sync_result */);
    EXPECT_THAT(connect_job->GetResolveErrorInfo().error, test::IsError(ERR_DNS_TIMED_OUT));
}
...
TEST_P(HttpProxyConnectJobTest, RequestPriority)
{
    ...
    for (int initial_priority = MINIMUM_PRIORITY; initial_priority <= MAXIMUM_PRIORITY; ++initial_priority) {
        SCOPED_TRACE(initial_priority);
        for (int new_priority = MINIMUM_PRIORITY; new_priority <= MAXIMUM_PRIORITY; ++new_priority) {
            SCOPED_TRACE(new_priority);
            if (initial_priority == new_priority) {
                continue;
            }
            TestConnectJobDelegate test_delegate;
            std::unique_ptr<ConnectJob> connect_job = CreateConnectJobForHttpRequest(&test_delegate, static_cast<RequestPriority>(initial_priority));
            EXPECT_THAT(connect_job->Connect(), test::IsError(ERR_IO_PENDING));
            EXPECT_FALSE(test_delegate.has_result());
            ...
            connect_job->ChangePriority(static_cast<RequestPriority>(initial_priority));
            EXPECT_EQ(initial_priority, host_resolver->request_priority(request_id));
        }
    }
}
...
TEST_P(HttpProxyConnectJobTest, SecureDnsPolicy)
{
    for (auto secure_dns_policy : { SecureDnsPolicy::kAllow, SecureDnsPolicy::kDisable }) {
        TestConnectJobDelegate test_delegate;
        std::unique_ptr<ConnectJob> connect_job = CreateConnectJobForHttpRequest(&test_delegate, DEFAULT_PRIORITY, secure_dns_policy);
...
TEST_P(HttpProxyConnectJobTest, SpdySessionKeyDisableSecureDns)
{
    ...
    TestConnectJobDelegate test_delegate;
    std::unique_ptr<ConnectJob> connect_job = CreateConnectJobForTunnel(&test_delegate, DEFAULT_PRIORITY, SecureDnsPolicy::kDisable);

    EXPECT_THAT(connect_job->Connect(), test::IsError(ERR_IO_PENDING));
    EXPECT_THAT(test_delegate.WaitForResult(), test::IsOk());
    EXPECT_TRUE(common_connect_job_params_->spdy_session_pool->FindAvailableSession(
        SpdySessionKey(kHttpsProxyServer.host_port_pair(), PRIVACY_MODE_DISABLED, ProxyChain::Direct(), SessionUsage::kProxy, SocketTag(),
            NetworkAnonymizationKey(), SecureDnsPolicy::kDisable,
            /*disable_cert_verification_network_fetches=*/true),
        /* enable_ip_based_pooling = */ false,
        /* is_websocket = */ false, NetLogWithSource()));
    EXPECT_FALSE(common_connect_job_params_->spdy_session_pool->FindAvailableSession(
        SpdySessionKey(kHttpsProxyServer.host_port_pair(), PRIVACY_MODE_DISABLED, ProxyChain::Direct(), SessionUsage::kProxy, SocketTag(),
...
TEST_P(HttpProxyConnectJobTest, SetSpdySessionSocketRequestPriority)
{
    ...
    // establishment.
    spdy::SpdySerializedFrame req(spdy_util_.ConstructSpdyConnect(nullptr /* extra_headers */, 0 /* extra_header_count */, 1 /* stream_id */,
        HttpProxyConnectJob::kH2QuicTunnelPriority, HostPortPair(kEndpointHost, 443)));
    MockWrite spdy_writes[] = { CreateMockWrite(req, 0, ASYNC) };
    spdy::SpdySerializedFrame resp(spdy_util_.ConstructSpdyGetReply(nullptr, 0, 1));
    MockRead spdy_reads[] = { CreateMockRead(resp, 1, ASYNC), MockRead(ASYNC, 0, 2) };

    Initialize(base::span<MockRead>(), base::span<MockWrite>(), spdy_reads, spdy_writes, SYNCHRONOUS);

    TestConnectJobDelegate test_delegate;
    std::unique_ptr<ConnectJob> connect_job = CreateConnectJobForTunnel(&test_delegate, IDLE);
    EXPECT_THAT(connect_job->Connect(), test::IsError(ERR_IO_PENDING));
    EXPECT_FALSE(test_delegate.has_result());
...
TEST_P(HttpProxyConnectJobTest, SpdyInadequateTransportSecurity)
{
    ...
    feature_list.InitAndEnableFeature(features::kSpdySessionForProxyAdditionalChecks);
    ...
    SSLConnectionStatusSetVersion(SSL_CONNECTION_VERSION_TLS1_1, &ssl_data.ssl_info.connection_status);
    session_deps_.socket_factory->AddSSLSocketDataProvider(&ssl_data);
    ...
    EXPECT_THAT(connect_job->Connect(), test::IsError(ERR_IO_PENDING));
    EXPECT_THAT(test_delegate.WaitForResult(), test::IsError(ERR_HTTP2_INADEQUATE_TRANSPORT_SECURITY));
    EXPECT_FALSE(common_connect_job_params_->spdy_session_pool->FindAvailableSession(
        SpdySessionKey(kHttpsProxyServer.host_port_pair(), PRIVACY_MODE_DISABLED, ProxyChain::Direct(), SessionUsage::kProxy, SocketTag(),
            NetworkAnonymizationKey(), SecureDnsPolicy::kDisable,
            /*disable_cert_verification_network_fetches=*/true),
...
TEST_P(HttpProxyConnectJobTest, SpdyValidAlps)
{
    ...
    SSLSocketDataProvider ssl_data(ASYNC, OK);
    InitializeSpdySsl(&ssl_data);
    ssl_data.peer_application_settings = HexDecode("000000" // length
                                                   "04" // type SETTINGS
                                                   "00" // flags
                                                   "00000000"); // stream ID
    session_deps_.socket_factory->AddSSLSocketDataProvider(&ssl_data);
...
TEST_P(HttpProxyConnectJobTest, SpdyInvalidAlpsCheckEnabled)
{
    ...
    EXPECT_THAT(connect_job->Connect(), test::IsError(ERR_IO_PENDING));
    EXPECT_THAT(test_delegate.WaitForResult(), test::IsError(ERR_HTTP2_PROTOCOL_ERROR));
    EXPECT_FALSE(common_connect_job_params_->spdy_session_pool->FindAvailableSession(
        SpdySessionKey(kHttpsProxyServer.host_port_pair(), PRIVACY_MODE_DISABLED, ProxyChain::Direct(), SessionUsage::kProxy, SocketTag(),
            NetworkAnonymizationKey(), SecureDnsPolicy::kDisable,
            /*disable_cert_verification_network_fetches=*/true),
...
{
    base::test::ScopedFeatureList feature_list;
    feature_list.InitAndDisableFeature(features::kSpdySessionForProxyAdditionalChecks);
...
TEST_P(HttpProxyConnectJobTest, TCPError)
{
    ...
    for (IoMode io_mode : { SYNCHRONOUS, ASYNC }) {
        ...
        TestConnectJobDelegate test_delegate;
        std::unique_ptr<ConnectJob> connect_job = CreateConnectJobForHttpRequest(&test_delegate);
        test_delegate.StartJobExpectingResult(connect_job.get(), ERR_PROXY_CONNECTION_FAILED, io_mode == SYNCHRONOUS);

        bool is_secure_proxy = GetParam() == HTTPS;
        histogram_tester.ExpectTotalCount("Net.HttpProxy.ConnectLatency.Http1.Http.Error", is_secure_proxy ? 0 : 1);
        histogram_tester.ExpectTotalCount("Net.HttpProxy.ConnectLatency.Http1.Https.Error", is_secure_proxy ? 1 : 0);
    }
}
...
TEST_P(HttpProxyConnectJobTest, SSLError)
{
    ...
    for (IoMode io_mode : { SYNCHRONOUS, ASYNC }) {
        ...
        TestConnectJobDelegate test_delegate;
        std::unique_ptr<ConnectJob> connect_job = CreateConnectJobForTunnel(&test_delegate);
        test_delegate.StartJobExpectingResult(connect_job.get(), ERR_PROXY_CERTIFICATE_INVALID, io_mode == SYNCHRONOUS);

        histogram_tester.ExpectTotalCount("Net.HttpProxy.ConnectLatency.Http1.Https.Error", 1);
        histogram_tester.ExpectTotalCount("Net.HttpProxy.ConnectLatency.Http1.Http.Error", 0);
        histogram_tester.ExpectTotalCount("Net.HttpProxy.ConnectLatency.Http2.Https.Error", 0);
        histogram_tester.ExpectTotalCount("Net.HttpProxy.ConnectLatency.Http2.Http.Error", 0);
    }
}
...
TEST_P(HttpProxyConnectJobTest, TunnelUnexpectedClose)
{
    for (IoMode io_mode : { SYNCHRONOUS, ASYNC }) {
        ...
        spdy::SpdySerializedFrame req(SpdyTestUtil().ConstructSpdyConnect(nullptr /*extra_headers */, 0 /*extra_header_count */, 1 /* stream_id */,
            HttpProxyConnectJob::kH2QuicTunnelPriority, HostPortPair(kEndpointHost, 443)));
        ...
        if (GetParam() == SPDY) {
            ...
            test_delegate.StartJobExpectingResult(connect_job.get(), ERR_CONNECTION_CLOSED, false /* expect_sync_result */);
        } else {
            test_delegate.StartJobExpectingResult(connect_job.get(), ERR_RESPONSE_HEADERS_TRUNCATED, io_mode == SYNCHRONOUS);
        }
    }
}
...
TEST_P(HttpProxyConnectJobTest, Tunnel1xxResponse)
{
    ...
    for (IoMode io_mode : { SYNCHRONOUS, ASYNC }) {
        ...
        Initialize(reads, writes, base::span<MockRead>(), base::span<MockWrite>(), io_mode);

        TestConnectJobDelegate test_delegate;
        std::unique_ptr<ConnectJob> connect_job = CreateConnectJobForTunnel(&test_delegate);
        test_delegate.StartJobExpectingResult(connect_job.get(), ERR_TUNNEL_CONNECTION_FAILED, io_mode == SYNCHRONOUS);
    }
}
...
TEST_P(HttpProxyConnectJobTest, TunnelSetupError)
{
    for (IoMode io_mode : { SYNCHRONOUS, ASYNC }) {
        ...
        SpdyTestUtil spdy_util;
        spdy::SpdySerializedFrame req(spdy_util.ConstructSpdyConnect(nullptr /* extra_headers */, 0 /* extra_header_count */, 1 /* stream_id */,
            HttpProxyConnectJob::kH2QuicTunnelPriority, HostPortPair("www.endpoint.test", 443)));
        spdy::SpdySerializedFrame rst(spdy_util.ConstructSpdyRstStream(1, spdy::ERROR_CODE_CANCEL));
        MockWrite spdy_writes[] = {
        ...
        TestConnectJobDelegate test_delegate;
        std::unique_ptr<ConnectJob> connect_job = CreateConnectJobForTunnel(&test_delegate, LOW);
        test_delegate.StartJobExpectingResult(connect_job.get(), ERR_TUNNEL_CONNECTION_FAILED, io_mode == SYNCHRONOUS && GetParam() != SPDY);
...
TEST_P(HttpProxyConnectJobTest, SslClientAuth)
{
    ...
    for (IoMode io_mode : { SYNCHRONOUS, ASYNC }) {
        ...
        SequencedSocketData socket_data(MockConnect(io_mode, OK), base::span<const MockRead>(), base::span<const MockWrite>());
        ...
        TestConnectJobDelegate test_delegate;
        std::unique_ptr<ConnectJob> connect_job = CreateConnectJobForTunnel(&test_delegate);
        test_delegate.StartJobExpectingResult(connect_job.get(), ERR_SSL_CLIENT_AUTH_CERT_NEEDED, io_mode == SYNCHRONOUS);
...
TEST_P(HttpProxyConnectJobTest, TunnelSetupRedirect)
{
    ...
    for (IoMode io_mode : { SYNCHRONOUS, ASYNC }) {
        ...
        spdy::SpdySerializedFrame req(
            spdy_util.ConstructSpdyConnect(nullptr /* extra_headers */, 0 /* extra_header_count */, 1, DEFAULT_PRIORITY, HostPortPair(kEndpointHost, 443)));
        spdy::SpdySerializedFrame rst(spdy_util.ConstructSpdyRstStream(1, spdy::ERROR_CODE_CANCEL));
        ...
        spdy::SpdySerializedFrame resp(spdy_util.ConstructSpdyReplyError("302", responseHeaders, responseHeadersSize, 1));
        MockRead spdy_reads[] = {
            CreateMockRead(resp, 1, ASYNC),
            MockRead(ASYNC, 0, 2),
        };
        ...
        TestConnectJobDelegate test_delegate(TestConnectJobDelegate::SocketExpected::ON_SUCCESS_ONLY);
        std::unique_ptr<ConnectJob> connect_job = CreateConnectJobForTunnel(&test_delegate);
        ...
        test_delegate.StartJobExpectingResult(connect_job.get(), ERR_TUNNEL_CONNECTION_FAILED, expect_sync_result);
...
TEST_P(HttpProxyConnectJobTest, TestTimeoutsAuthChallengeNewConnection)
{
    ...
    for (TimeoutPhase timeout_phase : kTimeoutPhases) {
        ...
        // before establishing a connection, so don't need them for
        // TimeoutPhase::CONNECT.
        Initialize(reads, writes, base::span<MockRead>(), base::span<MockWrite>(), SYNCHRONOUS);
        ...
        // before establishing a connection, so don't need them for
        // TimeoutPhase::SECOND_CONNECT.
        Initialize(reads2, writes2, base::span<MockRead>(), base::span<MockWrite>(), SYNCHRONOUS);
...
TEST_P(HttpProxyConnectJobTest, ConnectionTimeoutNoNQE)
{
    ...
    base::TimeDelta alternate_connection_timeout
        = HttpProxyConnectJob::AlternateNestedConnectionTimeout(*CreateParams(true /* tunnel */, SecureDnsPolicy::kAllow),
...
TEST_P(HttpProxyConnectJobTest, ConnectionTimeoutWithExperiment)
{
    ...
    InitAdaptiveTimeoutFieldTrialWithParams(false, kMultiplier, kMultiplier, kMinTimeout, kMaxTimeout);
    EXPECT_LE(base::TimeDelta(), GetNestedConnectionTimeout());
...
TEST_P(HttpProxyConnectJobTest, ConnectionTimeoutWithConnectionProperty)
{
    ...
    InitAdaptiveTimeoutFieldTrialWithParams(false, kSecureMultiplier, kNonSecureMultiplier, kMinTimeout, kMaxTimeout);
    ...
    if (GetParam() != HTTP) {
        ...
        EXPECT_EQ(kNonSecureMultiplier * kRttEstimate, GetNestedConnectionTimeout());
    }
}
...
{
    InitAdaptiveTimeoutFieldTrialWithParams(true, 0, 0, base::TimeDelta(), base::TimeDelta());
    EXPECT_LE(base::TimeDelta(), GetNestedConnectionTimeout());
...
public:
    explicit MockQuicSessionPool(
        HttpServerProperties* http_server_properties, CertVerifier* cert_verifier, TransportSecurityState* transport_security_state, QuicContext* context)
        : QuicSessionPool(/*net_log=*/nullptr,
            /*host_resolver=*/nullptr,
            /*ssl_config_service=*/nullptr,
            /*client_socket_factory=*/nullptr, http_server_properties, cert_verifier, transport_security_state,
            /*proxy_delegate=*/nullptr,
            /*sct_auditing_delegate=*/nullptr,
            /*socket_performance_watcher_factory=*/nullptr,
            /*quic_crypto_client_stream_factory=*/nullptr, context)
    {
    }
    ...
    MOCK_METHOD(int, RequestSession,
        (const QuicSessionKey& session_key, url::SchemeHostPort destination, quic::ParsedQuicVersion quic_version,
            const std::optional<NetworkTrafficAnnotationTag> proxy_annotation_tag, MultiplexedSessionCreationInitiator session_creation_initiator,
...
class HttpProxyConnectQuicJobTest : public HttpProxyConnectJobTestBase, public testing::Test {
public:
    HttpProxyConnectQuicJobTest()
        : mock_quic_session_pool_(
            session_->http_server_properties(), session_->cert_verifier(), session_->context().transport_security_state, session_->context().quic_context)
    {
...
TEST_F(HttpProxyConnectQuicJobTest, RequestQuicProxy)
{
    ...
    ProxyChain proxy_chain = ProxyChain::ForIpProtection({ ProxyServer(ProxyServer::SCHEME_QUIC, HostPortPair(kQuicProxyHost, 443)) });
    SSLConfig quic_ssl_config;
    scoped_refptr<HttpProxySocketParams> http_proxy_socket_params
        = base::MakeRefCounted<HttpProxySocketParams>(quic_ssl_config, HostPortPair(kEndpointHost, 443), proxy_chain,
            /*proxy_chain_index=*/0, /*tunnel=*/true, TRAFFIC_ANNOTATION_FOR_TESTS, NetworkAnonymizationKey(), SecureDnsPolicy::kAllow);

    TestConnectJobDelegate test_delegate;
    auto connect_job = std::make_unique<HttpProxyConnectJob>(DEFAULT_PRIORITY, SocketTag(), common_connect_job_params_.get(),
        std::move(http_proxy_socket_params), &test_delegate,
        /*net_log=*/nullptr);

    // Expect a session to be requested, and then leave it pending.
    EXPECT_CALL(mock_quic_session_pool_, RequestSession(_, _, _, _, _, _, _, _, _, _, _, QSRHasProxyChain(proxy_chain.Prefix(0))))
        .Times(1)
...
TEST_F(HttpProxyConnectQuicJobTest, QuicProxyRequestUsesRfcV1)
{
    ...
    ASSERT_EQ(DefaultSupportedQuicVersions()[0], quic::ParsedQuicVersion::RFCv1());

    auto supported_versions = quic::ParsedQuicVersionVector { quic::ParsedQuicVersion::RFCv2(), quic::ParsedQuicVersion::RFCv1() };
    ...
    EXPECT_CALL(mock_quic_session_pool_,
        RequestSession(_, _, IsQuicVersion(quic::ParsedQuicVersion::RFCv1()), _, _, _, _, _, _, _, _, QSRHasProxyChain(proxy_chain.Prefix(0))))
...
TEST_F(HttpProxyConnectQuicJobTest, RequestMultipleQuicProxies)
{
    ...
    scoped_refptr<HttpProxySocketParams> http_proxy_socket_params
        = base::MakeRefCounted<HttpProxySocketParams>(quic_ssl_config, HostPortPair(kEndpointHost, 443), proxy_chain,
            /*proxy_chain_index=*/1, /*tunnel=*/true, TRAFFIC_ANNOTATION_FOR_TESTS, NetworkAnonymizationKey(), SecureDnsPolicy::kAllow);
    ...
    EXPECT_CALL(mock_quic_session_pool_, RequestSession(_, _, _, _, _, _, _, _, _, _, _, QSRHasProxyChain(proxy_chain.Prefix(1))))
        .Times(1)
        .WillRepeatedly(testing::Return(ERR_IO_PENDING));
