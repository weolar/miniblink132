...
bool URLSecurityManagerWin::CanUseDefaultCredentials(const url::SchemeHostPort& auth_scheme_host_port) const
{
    if (HasDefaultAllowlist())
        return URLSecurityManagerAllowlist::CanUseDefaultCredentials(auth_scheme_host_port);
    ...
    hr = security_manager_->ProcessUrlAction(
        base::as_wcstr(url16), URLACTION_CREDENTIALS_USE, reinterpret_cast<BYTE*>(&policy), sizeof(policy), nullptr, 0, PUAF_NOUI, 0);
    if (FAILED(hr)) {
...
bool URLSecurityManagerWin::EnsureSystemSecurityManager()
{
    if (!security_manager_.Get()) {
        HRESULT hr = CoInternetCreateSecurityManager(nullptr, &security_manager_, 0);
