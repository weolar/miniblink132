...
bool HttpVaryData::Init(const HttpRequestInfo& request_info, const HttpResponseHeaders& response_headers)
{
    base::MD5Context ctx;
    base::MD5Init(&ctx);
...
bool HttpVaryData::MatchesRequest(const HttpRequestInfo& request_info, const HttpResponseHeaders& cached_response_headers) const
{
    // Vary: * never matches.
    ...
    return memcmp(&new_vary_data.request_digest_, &request_digest_, sizeof(request_digest_)) == 0;
}

// static
void HttpVaryData::AddField(const HttpRequestInfo& request_info, std::string_view request_header, base::MD5Context* ctx)
{
    std::string request_value = request_info.extra_headers.GetHeader(request_header).value_or(std::string());
