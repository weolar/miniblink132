...
public:
    ...
    virtual int InitializeStream(bool can_send_early, RequestPriority priority, const NetLogWithSource& net_log, CompletionOnceCallback callback) = 0;
    ...
    virtual int SendRequest(const HttpRequestHeaders& request_headers, HttpResponseInfo* response, CompletionOnceCallback callback) = 0;
    ...
    virtual int ReadResponseBody(IOBuffer* buf, int buf_len, CompletionOnceCallback callback) = 0;
    ...
    virtual bool GetAlternativeService(AlternativeService* alternative_service) const = 0;
