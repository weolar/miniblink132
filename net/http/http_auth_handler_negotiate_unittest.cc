...
class HttpAuthHandlerNegotiateTest : public PlatformTest, public WithTaskEnvironment {
public:
    void SetUp() override
    {
        scoped_feature_list_.InitAndEnableFeature(features::kPartitionConnectionsByNetworkIsolationKey);
        ...
        resolver_->rules()->AddIPLiteralRule("alias", "10.0.0.2", "canonical.example.com");

        http_auth_preferences_ = std::make_unique<MockAllowHttpAuthPreferences>();
        factory_ = std::make_unique<HttpAuthHandlerNegotiate::Factory>(HttpAuthMechanismFactory());
        factory_->set_http_auth_preferences(http_auth_preferences_.get());
#if BUILDFLAG(IS_ANDROID)
        auth_library_for_android_ = std::move(auth_library);
        http_auth_preferences_->set_auth_android_negotiate_account_type("org.chromium.test.DummySpnegoAuthenticator");
    ...
    void SetupMocks(MockAuthLibrary* mock_library)
    {
        ...
        mock_library->ExpectQuerySecurityPackageInfo(SEC_E_OK, security_package_.get());
#else
        ...
        MockAuthLibrary::SecurityContextQuery queries[] = {
            MockAuthLibrary::SecurityContextQuery("Negotiate", // Package name
    ...
    void SetupErrorMocks(MockAuthLibrary* mock_library, int major_status, int minor_status)
    {
        ...
        test::GssContextMockImpl context("localhost", // Source name
            ...
            1, // Locally initiated
            0); // Open
        MockAuthLibrary::SecurityContextQuery query("Negotiate", // Package name
    ...
    int CreateHandler(bool disable_cname_lookup, bool use_port, bool synchronous_resolve_mode, const std::string& url_string,
        std::unique_ptr<HttpAuthHandlerNegotiate>* handler)
    {
        http_auth_preferences_->set_negotiate_disable_cname_lookup(disable_cname_lookup);
        ...
        int rv = factory_->CreateAuthHandlerFromString("Negotiate", HttpAuth::AUTH_SERVER, null_ssl_info, network_anonymization_key(), scheme_host_port,
    ...
    MockAuthLibrary* AuthLibrary()
    {
        return auth_library_;
    }
    MockCachingHostResolver* resolver()
    {
...
TEST_F(HttpAuthHandlerNegotiateTest, DisableCname)
{
    ...
    std::string token;
    EXPECT_EQ(OK, callback.GetResult(auth_handler->GenerateAuthToken(nullptr, &request_info, callback.callback(), &token)));
#if BUILDFLAG(IS_WIN)
...
TEST_F(HttpAuthHandlerNegotiateTest, CnameSync)
{
    ...
    std::unique_ptr<HostResolver::ResolveHostRequest> host_request1
        = resolver()->CreateRequest(scheme_host_port, NetworkAnonymizationKey(), NetLogWithSource(), resolve_params);
    ...
    std::unique_ptr<HostResolver::ResolveHostRequest> host_request2
        = resolver()->CreateRequest(scheme_host_port, network_anonymization_key(), NetLogWithSource(), resolve_params);
...
TEST_F(HttpAuthHandlerNegotiateTest, CnameAsync)
{
    ...
    HttpRequestInfo request_info;
    std::string token;
    EXPECT_EQ(ERR_IO_PENDING, auth_handler->GenerateAuthToken(nullptr, &request_info, callback.callback(), &token));
...
TEST_F(HttpAuthHandlerNegotiateTest, MissingGSSAPI)
{
    ...
    negotiate_factory->set_library(std::make_unique<GSSAPISharedLibrary>("/this/library/does/not/exist"));

    url::SchemeHostPort scheme_host_port(GURL("http://www.example.com"));
    std::unique_ptr<HttpAuthHandler> generic_handler;
    int rv = negotiate_factory->CreateAuthHandlerFromString(
        "Negotiate", HttpAuth::AUTH_SERVER, SSLInfo(), NetworkAnonymizationKey(), scheme_host_port, NetLogWithSource(), resolver(), &generic_handler);
...
public:
    ...
    bool Init(const NetLogWithSource&) override
    {
        return true;
    }
...
TEST_F(HttpAuthHandlerNegotiateTest, OverrideAuthSystem)
{
    auto negotiate_factory = std::make_unique<HttpAuthHandlerNegotiate::Factory>(
        base::BindRepeating([](const HttpAuthPreferences*) -> std::unique_ptr<HttpAuthMechanism> { return std::make_unique<TestAuthSystem>(); }));
    negotiate_factory->set_http_auth_preferences(http_auth_preferences());
#if BUILDFLAG(IS_WIN)
    negotiate_factory->set_library(std::make_unique<MockAuthLibrary>(NEGOSSP_NAME));
#elif !BUILDFLAG(IS_ANDROID)
    ...
    EXPECT_EQ(OK,
        negotiate_factory->CreateAuthHandlerFromString(
            "Negotiate", HttpAuth::AUTH_SERVER, SSLInfo(), NetworkAnonymizationKey(), scheme_host_port, NetLogWithSource(), resolver(), &handler));
    EXPECT_TRUE(handler);

    TestCompletionCallback callback;
    std::string auth_token;
    HttpRequestInfo request_info;
    EXPECT_EQ(OK, callback.GetResult(handler->GenerateAuthToken(nullptr, &request_info, callback.callback(), &auth_token)));
