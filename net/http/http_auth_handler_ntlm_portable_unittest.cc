...
public:
    ...
    HttpAuthHandlerNtlmPortableTest()
    {
        ...
        creds_ = AuthCredentials(base::StrCat({ ntlm::test::kNtlmDomain, u"\\", ntlm::test::kUser }), ntlm::test::kPassword);
    }
    ...
    int CreateHandler()
    {
        ...
        return factory_->CreateAuthHandlerFromString(
            "NTLM", HttpAuth::AUTH_SERVER, null_ssl_info, NetworkAnonymizationKey(), scheme_host_port, NetLogWithSource(), nullptr, &auth_handler_);
    }

    std::string CreateNtlmAuthHeader(base::span<const uint8_t> buffer)
    {
        std::string output = base::Base64Encode(std::string_view(reinterpret_cast<const char*>(buffer.data()), buffer.size()));

        return "NTLM " + output;
    }

    HttpAuth::AuthorizationResult HandleAnotherChallenge(const std::string& challenge)
    {
    ...
    int GenerateAuthToken(std::string* token)
    {
        ...
        return callback.GetResult(GetAuthHandler()->GenerateAuthToken(GetCreds(), &request_info, callback.callback(), token));
    }

    bool ReadBytesPayload(ntlm::NtlmBufferReader* reader, base::span<uint8_t> buffer)
    {
    ...
    void ReadString16Payload(ntlm::NtlmBufferReader* reader, std::u16string* str)
    {
        ntlm::SecurityBuffer sec_buf;
    ...
    static std::string MockGetHostName()
    {
...
TEST_F(HttpAuthHandlerNtlmPortableTest, EmptyTokenFails)
{
    ...
    ASSERT_EQ(HttpAuth::AUTHORIZATION_RESULT_REJECT, HandleAnotherChallenge("NTLM"));
}
...
TEST_F(HttpAuthHandlerNtlmPortableTest, InvalidBase64Encoding)
{
    ...
    ASSERT_EQ(HttpAuth::AUTHORIZATION_RESULT_INVALID, HandleAnotherChallenge("NTLM !!!!!!!!!!!!!"));
}
...
TEST_F(HttpAuthHandlerNtlmPortableTest, CantChangeSchemeMidway)
{
    ...
    ASSERT_EQ(HttpAuth::AUTHORIZATION_RESULT_INVALID, HandleAnotherChallenge("Negotiate SSdtIG5vdCBhIHJlYWwgdG9rZW4h"));
}

TEST_F(HttpAuthHandlerNtlmPortableTest, NtlmV1AuthenticationSuccess)
{
    HttpAuthNtlmMechanism::ScopedProcSetter proc_setter(MockGetMSTime, MockRandom, MockGetHostName);
    ASSERT_EQ(OK, CreateHandler());
    ASSERT_EQ(OK, GetGenerateAuthTokenResult());

    std::string token;
    ASSERT_EQ(HttpAuth::AUTHORIZATION_RESULT_ACCEPT, HandleAnotherChallenge(CreateNtlmAuthHeader(ntlm::test::kChallengeMsgV1)));
    ASSERT_EQ(OK, GenerateAuthToken(&token));

    // Validate the authenticate message
    std::string decoded;
    ASSERT_TRUE(DecodeChallenge(token, &decoded));
    ASSERT_EQ(std::size(ntlm::test::kExpectedAuthenticateMsgSpecResponseV1), decoded.size());
    ASSERT_EQ(0, memcmp(decoded.data(), ntlm::test::kExpectedAuthenticateMsgSpecResponseV1, decoded.size()));
}
