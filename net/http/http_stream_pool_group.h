...
public:
    ...
    static constexpr base::TimeDelta kUsedIdleStreamSocketTimeout = base::Seconds(300);

    // The same timeout as
    // ClientSocketPoolManager::unused_idle_socket_timeout().
    static constexpr base::TimeDelta kUnusedIdleStreamSocketTimeout = base::Seconds(60);

    Group(HttpStreamPool* pool, HttpStreamKey stream_key, std::optional<QuicSessionAliasKey> quic_session_alias_key);
    ...
    std::unique_ptr<Job> CreateJob(Job::Delegate* delegate, NextProto expected_protocol, bool is_http1_allowed, ProxyInfo proxy_info);
    ...
    void ReleaseStreamSocket(std::unique_ptr<StreamSocket> socket, int64_t generation);
    ...
    size_t HandedOutStreamSocketCount() const
    {
        return handed_out_stream_count_;
    }

    // Returns the number of idle streams.
    size_t IdleStreamSocketCount() const
    {
        return idle_stream_sockets_.size();
    }
...
private:
    struct IdleStreamSocket {
        IdleStreamSocket(std::unique_ptr<StreamSocket> stream_socket, base::TimeTicks start_time);
        ~IdleStreamSocket();
    ...
    static base::expected<void, std::string_view> IsIdleStreamSocketUsable(const IdleStreamSocket& idle);

    void CleanupIdleStreamSockets(CleanupMode mode, std::string_view net_log_close_reason_utf8);
