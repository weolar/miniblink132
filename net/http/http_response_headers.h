...
class NET_EXPORT HttpResponseHeaders : public base::RefCountedThreadSafe<HttpResponseHeaders> {
public:
    ...
    HttpResponseHeaders(BuilderPassKey, HttpVersion version, std::string_view status, base::span<const std::pair<std::string_view, std::string_view>> headers);
    ...
    static scoped_refptr<HttpResponseHeaders> TryToCreate(std::string_view headers);
    ...
    // are already known.
    static scoped_refptr<HttpResponseHeaders> TryToCreateForDataURL(std::string_view content_type);
    ...
    void UpdateWithNewRange(const HttpByteRange& byte_range, int64_t resource_size, bool replace_status_line);
    ...
    [[nodiscard]] std::optional<std::string> GetNormalizedHeader(std::string_view name) const;
    ...
    // ("Foo", "c").
    bool EnumerateHeaderLines(size_t* iter, std::string* name, std::string* value) const;
    ...
    std::optional<std::string_view> EnumerateHeader(size_t* iter, std::string_view name) const;
    ...
    // true.
    bool EnumerateHeader(size_t* iter, std::string_view name, std::string* value) const;
    ...
    void GetMimeTypeAndCharset(std::string* mime_type, std::string* charset) const;
    ...
    ValidationType RequiresValidation(const base::Time& request_time, const base::Time& response_time, const base::Time& current_time) const;
    ...
    FreshnessLifetimes GetFreshnessLifetimes(const base::Time& response_time) const;
    ...
    base::TimeDelta GetCurrentAge(const base::Time& request_time, const base::Time& response_time, const base::Time& current_time) const;
    ...
    bool GetContentRangeFor206(int64_t* first_byte_position, int64_t* last_byte_position, int64_t* instance_length) const;
...
private:
    ...
    static HttpVersion ParseVersion(std::string::const_iterator line_begin, std::string::const_iterator line_end);
    ...
    void ParseStatusLine(std::string::const_iterator line_begin, std::string::const_iterator line_end, bool has_headers);
    ...
    std::optional<base::TimeDelta> GetCacheControlDirective(std::string_view directive) const;
    ...
    // optimization.
    void AddHeader(std::string::const_iterator name_begin, std::string::const_iterator name_end, std::string::const_iterator value_begin,
        std::string::const_iterator value_end, ContainsCommas contains_commas);

    // Add to parsed_ given the fields of a ParsedHeader object.
    void AddToParsed(std::string::const_iterator name_begin, std::string::const_iterator name_end, std::string::const_iterator value_begin,
    ...
    void MergeWithHeaders(std::string raw_headers, const HeaderSet& headers_to_remove);
...
using ResponseHeadersCallback = base::RepeatingCallback<void(scoped_refptr<const HttpResponseHeaders>)>;
