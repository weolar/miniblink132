...
public:
    ...
    HttpStreamParser(StreamSocket* stream_socket, bool connection_is_reused, const GURL& url, const std::string& method, UploadDataStream* upload_data_stream,
        GrowableIOBuffer* read_buffer, const NetLogWithSource& net_log);
    ...
    int SendRequest(const std::string& request_line, const HttpRequestHeaders& headers, const NetworkTrafficAnnotationTag& traffic_annotation,
    ...
    int ReadResponseBody(IOBuffer* buf, int buf_len, CompletionOnceCallback callback);
    ...
    static bool ShouldMergeRequestHeadersAndBody(const std::string& request_headers, const UploadDataStream* request_body);
