...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    if (data_provider.ConsumeBool()) {
        scheme
            = std::string(data_provider.PickValueInArray({ net::kBasicAuthScheme, net::kDigestAuthScheme, net::kNtlmAuthScheme, net::kNegotiateAuthScheme }));
    } else {
        scheme = data_provider.ConsumeRandomLengthString(10);
    }
    std::unique_ptr<net::HttpAuthHandlerRegistryFactory> factory = net::HttpAuthHandlerFactory::CreateDefault();
    ...
    factory->CreateAuthHandlerFromString(challenge, net::HttpAuth::AUTH_SERVER, null_ssl_info, net::NetworkAnonymizationKey(), scheme_host_port,
