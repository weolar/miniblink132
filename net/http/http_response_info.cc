...
bool KeyExchangeGroupIsValid(int ssl_connection_status)
{
    ...
    if (SSLConnectionStatusToVersion(ssl_connection_status) >= SSL_CONNECTION_VERSION_TLS1_3) {
        return true;
    }

    // Prior to TLS 1.3, only ECDHE ciphers have groups.
    const SSL_CIPHER* cipher = SSL_get_cipher_by_value(SSLConnectionStatusToCipherSuite(ssl_connection_status));
...
HttpResponseInfo& HttpResponseInfo::operator=(const HttpResponseInfo& rhs) = default;

bool HttpResponseInfo::InitFromPickle(const base::Pickle& pickle, bool* response_truncated)
{
    ...
    if (flags & RESPONSE_INFO_HAS_SSL_CONNECTION_STATUS) {
        ...
        if (SSLConnectionStatusToVersion(connection_status) == SSL_CONNECTION_VERSION_SSL3) {
            return false;
        }
    ...
    if (flags & RESPONSE_INFO_HAS_SIGNED_CERTIFICATE_TIMESTAMPS) {
        ...
        for (int i = 0; i < num_scts; ++i) {
            scoped_refptr<ct::SignedCertificateTimestamp> sct(ct::SignedCertificateTimestamp::CreateFromPickle(&iter));
            uint16_t status;
    ...
    if (flags & RESPONSE_INFO_HAS_CONNECTION_INFO) {
        ...
        if (value > static_cast<int>(HttpConnectionInfo::kUNKNOWN) && value <= static_cast<int>(HttpConnectionInfo::kMaxValue)) {
    ...
    *response_truncated = (flags & RESPONSE_INFO_TRUNCATED) != 0;
    ...
    if (flags & RESPONSE_INFO_HAS_PEER_SIGNATURE_ALGORITHM) {
        ...
        if (!iter.ReadInt(&peer_signature_algorithm) || !base::IsValueInRangeForNumericType<uint16_t>(peer_signature_algorithm)) {
            return false;
        }
        ssl_info.peer_signature_algorithm = base::checked_cast<uint16_t>(peer_signature_algorithm);
    }
    ...
    ssl_info.encrypted_client_hello = (flags & RESPONSE_INFO_ENCRYPTED_CLIENT_HELLO) != 0;
    ...
    did_use_shared_dictionary = (extra_flags & RESPONSE_EXTRA_INFO_DID_USE_SHARED_DICTIONARY) != 0;
...
void HttpResponseInfo::Persist(base::Pickle* pickle, bool skip_transient_headers, bool response_truncated) const
{
    ...
    if (skip_transient_headers) {
        persist_options = HttpResponseHeaders::PERSIST_SANS_COOKIES | HttpResponseHeaders::PERSIST_SANS_CHALLENGES
            | HttpResponseHeaders::PERSIST_SANS_HOP_BY_HOP | HttpResponseHeaders::PERSIST_SANS_NON_CACHEABLE | HttpResponseHeaders::PERSIST_SANS_RANGES
    ...
    if (flags & RESPONSE_INFO_HAS_STALENESS) {
        pickle->WriteInt64((stale_revalidate_timeout - base::Time()).InMicroseconds());
    }
