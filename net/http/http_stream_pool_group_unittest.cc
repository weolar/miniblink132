...
protected:
    ...
    void set_disable_idle_sockets_close_on_memory_pressure(bool disable_idle_sockets_close_on_memory_pressure)
    {
        session_deps_.disable_idle_sockets_close_on_memory_pressure = disable_idle_sockets_close_on_memory_pressure;
    }
    ...
    Group& GetTestGroup()
    {
        const HttpStreamKey key(url::SchemeHostPort("http", "a.test", 80), PRIVACY_MODE_DISABLED, SocketTag(), NetworkAnonymizationKey(),
...
TEST_F(HttpStreamPoolGroupTest, CreateTextBasedStream)
{
    ...
    std::unique_ptr<HttpStream> stream
        = group.CreateTextBasedStream(std::move(stream_socket), StreamSocketHandle::SocketReuseType::kUnused, LoadTimingInfo::ConnectTiming());
    CHECK(stream);
...
TEST_F(HttpStreamPoolGroupTest, ReleaseStreamSocketUsed)
{
    ...
    static_assert(Group::kUnusedIdleStreamSocketTimeout <= Group::kUsedIdleStreamSocketTimeout);
