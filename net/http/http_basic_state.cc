...
HttpBasicState::HttpBasicState(std::unique_ptr<StreamSocketHandle> connection, bool is_for_get_to_http_proxy)
...
void HttpBasicState::Initialize(const HttpRequestInfo* request_info, RequestPriority priority, const NetLogWithSource& net_log)
{
    DCHECK(!parser_.get());
    traffic_annotation_ = request_info->traffic_annotation;
    parser_ = std::make_unique<HttpStreamParser>(connection_->socket(), connection_->reuse_type() == StreamSocketHandle::SocketReuseType::kReusedIdle,
        request_info->url, request_info->method, request_info->upload_data_stream, read_buf_.get(), net_log);
}
...
{
    return HttpUtil::GenerateRequestLine(parser_->method(), parser_->url(), is_for_get_to_http_proxy_);
}

bool HttpBasicState::IsConnectionReused() const
{
    return connection_->reuse_type() == StreamSocketHandle::SocketReuseType::kReusedIdle
        || connection_->reuse_type() == StreamSocketHandle::SocketReuseType::kUnusedIdle;
}
...
const std::set<std::string>& HttpBasicState::GetDnsAliases() const
{
    ...
    return (connection_ && connection_->socket()) ? connection_->socket()->GetDnsAliases() : *emptyset_result;
}
