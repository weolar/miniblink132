...
TEST(HttpStreamRequestTest, SetPriority)
{
    ...
    auto job_controller = std::make_unique<HttpStreamFactory::JobController>(factory, &request_delegate, session.get(), &job_factory, request_info,
        /*is_preconnect=*/false,
        /*is_websocket=*/false,
    ...
    HttpStreamFactory::JobController* job_controller_raw_ptr = job_controller.get();
    factory->job_controller_set_.insert(std::move(job_controller));

    std::unique_ptr<HttpStreamRequest> request(
        job_controller_raw_ptr->Start(&request_delegate, nullptr, NetLogWithSource(), HttpStreamRequest::HTTP_STREAM, DEFAULT_PRIORITY));
