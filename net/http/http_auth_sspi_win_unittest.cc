...
void MatchDomainUserAfterSplit(const std::u16string& combined, const std::u16string& expected_domain, const std::u16string& expected_user)
{
...
TEST(HttpAuthSSPITest, ParseChallenge_FirstRound)
{
    ...
    EXPECT_EQ(HttpAuth::AUTHORIZATION_RESULT_ACCEPT, auth_sspi.ParseChallenge(&challenge));
}
...
TEST(HttpAuthSSPITest, ParseChallenge_TwoRounds)
{
    ...
    EXPECT_EQ(HttpAuth::AUTHORIZATION_RESULT_ACCEPT, auth_sspi.ParseChallenge(&first_challenge));

    // Generate an auth token and create another thing.
    std::string auth_token;
    EXPECT_EQ(OK,
        auth_sspi.GenerateAuthToken(nullptr, "HTTP/intranet.google.com", std::string(), &auth_token, NetLogWithSource(), base::BindOnce(&UnexpectedCallback)));

    HttpAuthChallengeTokenizer second_challenge("Negotiate Zm9vYmFy");
    EXPECT_EQ(HttpAuth::AUTHORIZATION_RESULT_ACCEPT, auth_sspi.ParseChallenge(&second_challenge));
}
...
TEST(HttpAuthSSPITest, ParseChallenge_UnexpectedTokenFirstRound)
{
    ...
    EXPECT_EQ(HttpAuth::AUTHORIZATION_RESULT_INVALID, auth_sspi.ParseChallenge(&challenge));
}
...
TEST(HttpAuthSSPITest, ParseChallenge_MissingTokenSecondRound)
{
    ...
    EXPECT_EQ(HttpAuth::AUTHORIZATION_RESULT_REJECT, auth_sspi.ParseChallenge(&second_challenge));
}
...
TEST(HttpAuthSSPITest, ParseChallenge_NonBase64EncodedToken)
{
    ...
    EXPECT_EQ(HttpAuth::AUTHORIZATION_RESULT_INVALID, auth_sspi.ParseChallenge(&second_challenge));
}
...
TEST(HttpAuthSSPITest, GenerateAuthToken_FullHandshake_AmbientCreds)
{
    ...
    std::string first_challenge_text = "Negotiate";
    HttpAuthChallengeTokenizer first_challenge("Negotiate");
    ASSERT_EQ(HttpAuth::AUTHORIZATION_RESULT_ACCEPT, auth_sspi.ParseChallenge(&first_challenge));
    ...
    ASSERT_EQ(HttpAuth::AUTHORIZATION_RESULT_ACCEPT, auth_sspi.ParseChallenge(&second_challenge));
...
TEST(HttpAuthSSPITest, GenerateAuthToken_FullHandshake_AmbientCreds_Logging)
{
    ...
    std::string auth_token;
    ASSERT_EQ(OK,
        auth_sspi.GenerateAuthToken(nullptr, "HTTP/intranet.google.com", std::string(), &auth_token, net_log_with_source, base::BindOnce(&UnexpectedCallback)));
    ...
    auto entries = net_log_observer.GetEntriesWithType(NetLogEventType::AUTH_LIBRARY_ACQUIRE_CREDS);
    ...
    entries = net_log_observer.GetEntriesWithType(NetLogEventType::AUTH_LIBRARY_INIT_SEC_CTX);
