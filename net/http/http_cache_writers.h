...
public:
    ...
    struct NET_EXPORT_PRIVATE TransactionInfo {
        TransactionInfo(PartialData* partial, bool truncated, HttpResponseInfo info);
        ~TransactionInfo();
    ...
    // |callback|.
    int Read(scoped_refptr<IOBuffer> buf, int buf_len, CompletionOnceCallback callback, Transaction* transaction);
    ...
    void AddTransaction(Transaction* transaction, ParallelWritingPattern initial_writing_pattern, RequestPriority priority, const TransactionInfo& info);
    ...
    void SetNetworkTransaction(Transaction* transaction, std::unique_ptr<HttpTransaction> network_transaction);
    ...
    bool network_read_only() const
    {
        return network_read_only_;
    }

    int GetTransactionsCount() const
    {
        return all_writers_.size();
    }
...
private:
    ...
    struct WaitingForRead {
        ...
        WaitingForRead(scoped_refptr<IOBuffer> read_buf, int len, CompletionOnceCallback consumer_callback);
        ~WaitingForRead();
    ...
    TransactionMap::iterator EraseTransaction(TransactionMap::iterator it, int result);
