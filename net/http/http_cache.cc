...
HttpCache::DefaultBackend::DefaultBackend(CacheType type, BackendType backend_type,
    scoped_refptr<disk_cache::BackendFileOperationsFactory> file_operations_factory, const base::FilePath& path, int max_bytes, bool hard_reset)
    : type_(type)
...
// static
std::unique_ptr<HttpCache::BackendFactory> HttpCache::DefaultBackend::InMemory(int max_bytes)
{
    return std::make_unique<DefaultBackend>(MEMORY_CACHE, CACHE_BACKEND_DEFAULT,
        /*file_operations_factory=*/nullptr, base::FilePath(), max_bytes, false);
}

disk_cache::BackendResult HttpCache::DefaultBackend::CreateBackend(NetLog* net_log, base::OnceCallback<void(disk_cache::BackendResult)> callback)
{
    DCHECK_GE(max_bytes_, 0);
    disk_cache::ResetHandling reset_handling = hard_reset_ ? disk_cache::ResetHandling::kReset : disk_cache::ResetHandling::kResetOnError;
    LOCAL_HISTOGRAM_BOOLEAN("HttpCache.HardReset", hard_reset_);
#if BUILDFLAG(IS_ANDROID)
    if (app_status_listener_getter_) {
        return disk_cache::CreateCacheBackend(
            type_, backend_type_, file_operations_factory_, path_, max_bytes_, reset_handling, net_log, std::move(callback), app_status_listener_getter_);
    }
#endif
    return disk_cache::CreateCacheBackend(type_, backend_type_, file_operations_factory_, path_, max_bytes_, reset_handling, net_log, std::move(callback));
}

#if BUILDFLAG(IS_ANDROID)
void HttpCache::DefaultBackend::SetAppStatusListenerGetter(disk_cache::ApplicationStatusListenerGetter app_status_listener_getter)
{
...
HttpCache::ActiveEntry::ActiveEntry(base::WeakPtr<HttpCache> cache, disk_cache::Entry* entry, bool opened_in)
    : cache_(std::move(cache))
    , disk_entry_(entry)
    , opened_(opened_in)
{
    CHECK(disk_entry_);
    cache_->active_entries_.emplace(disk_entry_->GetKey(), base::raw_ref<ActiveEntry>::from_ptr(this));
}
...
void HttpCache::ActiveEntry::FinalizeDoomed()
{
    ...
    auto it = cache_->doomed_entries_.find(base::raw_ref<ActiveEntry>::from_ptr(this));
...
void HttpCache::ActiveEntry::SlowDeactivate()
{
    ...
    for (auto it = cache_->active_entries_.begin(); it != cache_->active_entries_.end(); ++it) {
...
bool HttpCache::ActiveEntry::TransactionInReaders(Transaction* transaction) const
{
...
void HttpCache::ActiveEntry::AddTransactionToWriters(Transaction* transaction, ParallelWritingPattern parallel_writing_pattern)
{
    CHECK(cache_);
    if (!writers_) {
        writers_ = std::make_unique<Writers>(cache_.get(), base::WrapRefCounted(this));
    } else {
    ...
    Writers::TransactionInfo info(transaction->partial(), transaction->is_truncated(), *(transaction->GetResponseInfo()));

    writers_->AddTransaction(transaction, parallel_writing_pattern, transaction->priority(), info);
}
...
bool HttpCache::ActiveEntry::RemovePendingTransaction(Transaction* transaction)
{
    auto j = find(add_to_entry_queue_.begin(), add_to_entry_queue_.end(), transaction);
...
bool HttpCache::ActiveEntry::CanTransactionWriteResponseHeaders(Transaction* transaction, bool is_partial, bool is_match) const
{
    ...
    if (!is_match) {
        return (!writers_ || writers_->IsEmpty()) && done_headers_queue_.empty() && readers_.empty();
    }
...
public:
    WorkItem(WorkItemOperation operation, Transaction* transaction, scoped_refptr<ActiveEntry>* entry)
        : operation_(operation)
        , transaction_(transaction)
        , entry_(entry)
    {
    }
    WorkItem(WorkItemOperation operation, Transaction* transaction, CompletionOnceCallback callback)
        : operation_(operation)
        , transaction_(transaction)
        , entry_(nullptr)
...
HttpCache::HttpCache(std::unique_ptr<HttpTransactionFactory> network_layer, std::unique_ptr<BackendFactory> backend_factory)
    : net_log_(nullptr)
    ...
    , keys_marked_no_store_(features::kAvoidEntryCreationForNoStoreCacheSize.Get())
{
    g_init_cache = true;
...
HttpCache::GetBackendResult HttpCache::GetBackend(GetBackendCallback callback)
{
    ...
    int rv = CreateBackend(base::BindOnce(&HttpCache::ReportGetBackendResult, GetWeakPtr(), std::move(callback)));
...
void HttpCache::ReportGetBackendResult(GetBackendCallback callback, int net_error)
{
...
// static
bool HttpCache::ParseResponseInfo(base::span<const uint8_t> data, HttpResponseInfo* response_info, bool* response_truncated)
{
    base::Pickle pickle = base::Pickle::WithUnownedBuffer(data);
    return response_info->InitFromPickle(pickle, response_truncated);
}

void HttpCache::CloseAllConnections(int net_error, const char* net_log_reason_utf8)
{
...
void HttpCache::OnExternalCacheHit(const GURL& url, const std::string& http_method, const NetworkIsolationKey& network_isolation_key, bool used_credentials)
{
...
int HttpCache::CreateTransaction(RequestPriority priority, std::unique_ptr<HttpTransaction>* transaction)
{
    ...
    auto new_transaction = std::make_unique<HttpCache::Transaction>(priority, this);
...
std::unique_ptr<HttpTransactionFactory> HttpCache::SetHttpNetworkTransactionFactoryForTesting(std::unique_ptr<HttpTransactionFactory> new_network_layer)
{
    std::unique_ptr<HttpTransactionFactory> old_network_layer(std::move(network_layer_));
    ...
    if (IsSplitCacheEnabled()) {
        ...
        if (request->initiator.has_value() && request->initiator->opaque()) {
            ...
            case HttpCache::ExperimentMode::kNavigationInitiator:
                if (request->is_main_frame_navigation || request->is_subframe_document_resource) {
                    return false;
                }
                break;
            }
...
std::string HttpCache::GenerateCacheKey(const GURL& url, int load_flags, const NetworkIsolationKey& network_isolation_key, int64_t upload_data_identifier,
    bool is_subframe_document_resource, bool is_mainframe_navigation, std::optional<url::Origin> initiator)
{
    ...
    const char credential_key
        = (base::FeatureList::IsEnabled(features::kSplitCacheByIncludeCredentials) && (load_flags & LOAD_DO_NOT_SAVE_COOKIES)) ? '0' : '1';
    ...
    if (IsSplitCacheEnabled()) {
        ...
        if (initiator.has_value() && (is_mainframe_navigation || is_subframe_document_resource)) {
            const auto initiator_site = net::SchemefulSite(*initiator);
            const bool is_initiator_cross_site = initiator_site != net::SchemefulSite(url);
            ...
            if (is_initiator_cross_site) {
                ...
                case HttpCache::ExperimentMode::kMainFrameNavigationInitiator:
                    if (is_mainframe_navigation) {
                        ...
                        navigation_experiment_prefix = base::StrCat({ "mfni_", initiator_site.Serialize(), " " });
                    }
                    break;
                case HttpCache::ExperimentMode::kNavigationInitiator:
                    if (is_mainframe_navigation || is_subframe_document_resource) {
                        CHECK(!initiator_site.opaque());
                        navigation_experiment_prefix = base::StrCat({ "ni_", initiator_site.Serialize(), " " });
                    }
                    break;
                }
            }
        }
        isolation_key = base::StrCat({ kDoubleKeyPrefix, subframe_document_resource_prefix, navigation_experiment_prefix,
    ...
    return base::StringPrintf("%c/%" PRId64 "/%s%s", credential_key, upload_data_identifier, isolation_key.c_str(), HttpUtil::SpecForRequest(url).c_str());
}

// static
HttpCache::ExperimentMode HttpCache::GetExperimentMode()
{
    bool cross_site_main_frame_navigation_boolean_enabled = base::FeatureList::IsEnabled(net::features::kSplitCacheByCrossSiteMainFrameNavigationBoolean);
    bool main_frame_navigation_initiator_enabled = base::FeatureList::IsEnabled(net::features::kSplitCacheByMainFrameNavigationInitiator);
    bool navigation_initiator_enabled = base::FeatureList::IsEnabled(net::features::kSplitCacheByNavigationInitiator);

    if (cross_site_main_frame_navigation_boolean_enabled) {
        if (main_frame_navigation_initiator_enabled || navigation_initiator_enabled) {
...
// static
std::optional<std::string> HttpCache::GenerateCacheKeyForRequest(const HttpRequestInfo* request)
{
    CHECK(request);
    ...
    const int64_t upload_data_identifier = request->upload_data_stream ? request->upload_data_stream->identifier() : int64_t(0);
    return GenerateCacheKey(request->url, request->load_flags, request->network_isolation_key, upload_data_identifier, request->is_subframe_document_resource,
...
void HttpCache::SplitCacheFeatureEnableByDefault()
{
    ...
    if (!base::FeatureList::GetInstance()->IsFeatureOverridden("SplitCacheByNetworkIsolationKey")) {
...
{
    return base::FeatureList::IsEnabled(features::kSplitCacheByNetworkIsolationKey) || g_enable_split_cache;
}
...
Error HttpCache::CreateAndSetWorkItem(scoped_refptr<ActiveEntry>* entry, Transaction* transaction, WorkItemOperation operation, PendingOp* pending_op)
{
...
int HttpCache::CreateBackend(CompletionOnceCallback callback)
{
    ...
    std::unique_ptr<WorkItem> item = std::make_unique<WorkItem>(WI_CREATE_BACKEND, nullptr, std::move(callback));
    ...
    disk_cache::BackendResult result
        = backend_factory_->CreateBackend(net_log_, base::BindOnce(&HttpCache::OnPendingBackendCreationOpComplete, GetWeakPtr(), pending_op));
    ...
    OnPendingBackendCreationOpComplete(GetWeakPtr(), pending_op, std::move(result));
    return rv;
}
...
int HttpCache::GetBackendForTransaction(Transaction* transaction)
{
    ...
    std::unique_ptr<WorkItem> item = std::make_unique<WorkItem>(WI_CREATE_BACKEND, transaction, CompletionOnceCallback());
    PendingOp* pending_op = GetPendingOp(std::string());
...
int HttpCache::AsyncDoomEntry(const std::string& key, Transaction* transaction)
{
    PendingOp* pending_op = GetPendingOp(key);
    int rv = CreateAndSetWorkItem(nullptr, transaction, WI_DOOM_ENTRY, pending_op);
    if (rv != OK) {
        return rv;
    }

    RequestPriority priority = transaction ? transaction->priority() : LOWEST;
    rv = disk_cache_->DoomEntry(key, priority, base::BindOnce(&HttpCache::OnPendingOpComplete, GetWeakPtr(), pending_op));
    if (rv == ERR_IO_PENDING) {
        pending_op->callback_will_delete = true;
        return rv;
    }
...
void HttpCache::DoomMainEntryForUrl(const GURL& url, const NetworkIsolationKey& isolation_key, bool is_subframe_document_resource,
    bool is_main_frame_navigation, const std::optional<url::Origin>& initiator)
{
    if (!disk_cache_) {
        return;
    }
    ...
    temp_info.network_anonymization_key = NetworkAnonymizationKey::CreateFromNetworkIsolationKey(isolation_key);
...
scoped_refptr<HttpCache::ActiveEntry> HttpCache::GetActiveEntry(const std::string& key)
{
    auto it = active_entries_.find(key);
    return it != active_entries_.end() ? base::WrapRefCounted(&it->second.get()) : nullptr;
}

scoped_refptr<HttpCache::ActiveEntry> HttpCache::ActivateEntry(disk_cache::Entry* disk_entry, bool opened)
{
    DCHECK(!HasActiveEntry(disk_entry->GetKey()));
    return base::MakeRefCounted<ActiveEntry>(weak_factory_.GetWeakPtr(), disk_entry, opened);
}
...
int HttpCache::OpenOrCreateEntry(const std::string& key, scoped_refptr<ActiveEntry>* entry, Transaction* transaction)
{
    DCHECK(!HasActiveEntry(key));

    PendingOp* pending_op = GetPendingOp(key);
    int rv = CreateAndSetWorkItem(entry, transaction, WI_OPEN_OR_CREATE_ENTRY, pending_op);
    if (rv != OK) {
        return rv;
    }

    disk_cache::EntryResult entry_result
        = disk_cache_->OpenOrCreateEntry(key, transaction->priority(), base::BindOnce(&HttpCache::OnPendingCreationOpComplete, GetWeakPtr(), pending_op));
    ...
    pending_op->writer->ClearTransaction();
    OnPendingCreationOpComplete(GetWeakPtr(), pending_op, std::move(entry_result));
    return rv;
}

int HttpCache::OpenEntry(const std::string& key, scoped_refptr<ActiveEntry>* entry, Transaction* transaction)
{
    DCHECK(!HasActiveEntry(key));
    ...
    disk_cache::EntryResult entry_result
        = disk_cache_->OpenEntry(key, transaction->priority(), base::BindOnce(&HttpCache::OnPendingCreationOpComplete, GetWeakPtr(), pending_op));
    rv = entry_result.net_error();
    if (rv == ERR_IO_PENDING) {
        pending_op->callback_will_delete = true;
        return ERR_IO_PENDING;
...
int HttpCache::CreateEntry(const std::string& key, scoped_refptr<ActiveEntry>* entry, Transaction* transaction)
{
    ...
    PendingOp* pending_op = GetPendingOp(key);
    int rv = CreateAndSetWorkItem(entry, transaction, WI_CREATE_ENTRY, pending_op);
    if (rv != OK) {
        return rv;
    }

    disk_cache::EntryResult entry_result
        = disk_cache_->CreateEntry(key, transaction->priority(), base::BindOnce(&HttpCache::OnPendingCreationOpComplete, GetWeakPtr(), pending_op));
    rv = entry_result.net_error();
    if (rv == ERR_IO_PENDING) {
        pending_op->callback_will_delete = true;
        return ERR_IO_PENDING;
...
int HttpCache::AddTransactionToEntry(scoped_refptr<ActiveEntry>& entry, Transaction* transaction)
{
    DCHECK(entry);
...
int HttpCache::DoneWithResponseHeaders(scoped_refptr<ActiveEntry>& entry, Transaction* transaction, bool is_partial)
{
    // If |transaction| is the current writer, do nothing. This can happen for
    // range requests since they can go back to headers phase after starting to
    // write.
    ...
    if ((transaction->mode() & Transaction::WRITE) && !entry->HasWriters() && entry->readers().empty()) {
        entry->AddTransactionToWriters(transaction, CanTransactionJoinExistingWriters(transaction));
        ProcessQueuedTransactions(entry);
        return OK;
    }
...
void HttpCache::DoneWithEntry(scoped_refptr<ActiveEntry>& entry, Transaction* transaction, bool entry_is_complete, bool is_partial)
{
    ...
    if (entry->HasWriters() && entry->writers()->HasTransaction(transaction)) {
        entry->writers()->RemoveTransaction(transaction, entry_is_complete /* success */);
        return;
    }
...
void HttpCache::WritersDoneWritingToEntry(scoped_refptr<ActiveEntry> entry, bool success, bool should_keep_entry, TransactionSet make_readers)
{
...
void HttpCache::DoomEntryValidationNoMatch(scoped_refptr<ActiveEntry> entry)
{
    ...
    for (HttpCache::Transaction* transaction : entry->add_to_entry_queue()) {
        ...
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(transaction->cache_io_callback(), ERR_CACHE_RACE));
    }
...
void HttpCache::ProcessQueuedTransactions(scoped_refptr<ActiveEntry> entry)
{
    ...
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
        FROM_HERE, base::BindOnce(&HttpCache::OnProcessQueuedTransactions, GetWeakPtr(), std::move(entry)));
}
...
void HttpCache::ProcessAddToEntryQueue(scoped_refptr<ActiveEntry> entry)
{
    ...
    if (delay_add_transaction_to_entry_for_test_) {
        ...
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
            FROM_HERE, base::BindOnce(&HttpCache::ProcessAddToEntryQueueImpl, GetWeakPtr(), std::move(entry)));
    } else {
...
HttpCache::ParallelWritingPattern HttpCache::CanTransactionJoinExistingWriters(Transaction* transaction)
{
    ...
    if (transaction->GetResponseInfo()->headers && transaction->GetResponseInfo()->headers->GetContentLength() > disk_cache_->MaxFileSize()) {
...
{
    ParallelWritingPattern writers_pattern;
    DCHECK(!entry->HasWriters() || entry->writers()->CanAddWriters(&writers_pattern));
    DCHECK(!entry->done_headers_queue().empty());

    Transaction* transaction = entry->done_headers_queue().front();

    ParallelWritingPattern parallel_writing_pattern = CanTransactionJoinExistingWriters(transaction);
...
LoadState HttpCache::GetLoadStateForPendingTransaction(const Transaction* transaction)
{
...
void HttpCache::RemovePendingTransaction(Transaction* transaction)
{
    ...
    for (auto k = doomed_entries_.begin(); k != doomed_entries_.end() && !found; ++k) {
...
bool HttpCache::RemovePendingTransactionFromPendingOp(PendingOp* pending_op, Transaction* transaction)
{
...
{
    return keys_marked_no_store_.Get(base::SHA1Hash(base::as_byte_span(key))) != keys_marked_no_store_.end();
}
...
// static
void HttpCache::OnPendingOpComplete(base::WeakPtr<HttpCache> cache, PendingOp* pending_op, int rv)
{
    if (cache.get()) {
...
// static
void HttpCache::OnPendingCreationOpComplete(base::WeakPtr<HttpCache> cache, PendingOp* pending_op, disk_cache::EntryResult result)
{
    if (!cache.get()) {
        // The callback was cancelled so we should delete the pending_op that
...
// static
void HttpCache::OnPendingBackendCreationOpComplete(base::WeakPtr<HttpCache> cache, PendingOp* pending_op, disk_cache::BackendResult result)
{
    if (!cache.get()) {
        // The callback was cancelled so we should delete the pending_op that
...
void HttpCache::OnBackendCreated(int result, PendingOp* pending_op)
{
    ...
    if (backend_factory_.get()) {
        ...
        if (result == OK) {
            ...
            UMA_HISTOGRAM_MEMORY_KB("HttpCache.MaxFileSizeOnInit", disk_cache_->MaxFileSize() / 1024);
        }
    }

    if (!pending_op->pending_queue.empty()) {
        std::unique_ptr<WorkItem> pending_item = std::move(pending_op->pending_queue.front());
        ...
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(&HttpCache::OnBackendCreated, GetWeakPtr(), result, pending_op));
    } else {
