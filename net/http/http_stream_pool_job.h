...
public:
    ...
    public:
        ...
        virtual void OnStreamReady(Job* job, std::unique_ptr<HttpStream> stream, NextProto negotiated_protocol) = 0;

        // Called when stream attempts failed.
        virtual void OnStreamFailed(Job* job, int status, const NetErrorDetails& net_error_details, ResolveErrorInfo resolve_error_info) = 0;
    ...
    Job(Delegate* delegate, AttemptManager* attempt_manager, NextProto expected_protocol, bool is_http1_allowed, ProxyInfo proxy_info);
    ...
    // Starts this job.
    void Start(RequestPriority priority, const std::vector<SSLConfig::CertAndStatus>& allowed_bad_certs, RespectLimits respect_limits,
    ...
    void OnStreamReady(std::unique_ptr<HttpStream> stream, NextProto negotiated_protocol);

    // Called by the associated AttemptManager when stream attempts failed.
    void OnStreamFailed(int rv, const NetErrorDetails& net_error_details, ResolveErrorInfo resolve_error_info);
    ...
    const NextProtoSet& allowed_alpns() const
    {
...
private:
    void CallOnStreamReady(std::unique_ptr<HttpStream> stream, NextProto negotiated_protocol);
    void CallOnStreamFailed(int status, const NetErrorDetails& net_error_details, ResolveErrorInfo resolve_error_info);
