...
TEST(HttpUtilTest, IsSafeHeader)
{
    ...
    for (const auto* unsafe_header : unsafe_headers) {
        ...
        EXPECT_FALSE(HttpUtil::IsSafeHeader(base::ToUpperASCII(unsafe_header), "")) << unsafe_header;
    }
    ...
    for (const auto* safe_header : safe_headers) {
        ...
        EXPECT_TRUE(HttpUtil::IsSafeHeader(base::ToUpperASCII(safe_header), "")) << safe_header;
    }
    ...
    static const struct {
        const char* value;
        bool is_safe;
    } disallowed_values[] = { { "connect", false }, { "trace", false }, { "track", false }, { "CONNECT", false }, { "cOnnEcT", false }, { "get", true },
        { "get,post", true }, { "get,connect", false }, { "get, connect", false }, { "get,connect ", false }, { "get,connect ,post", false },
        { "get,,,,connect", false }, { "trace,get,PUT", false } };
    for (const auto* header : disallowed_with_forbidden_methods_headers) {
        for (const auto& test_case : disallowed_values) {
            EXPECT_EQ(test_case.is_safe, HttpUtil::IsSafeHeader(header, test_case.value)) << header << ": " << test_case.value;
        }
    }
}
...
TEST(HttpUtilTest, LocateEndOfAdditionalHeaders)
{
    ...
    for (const auto& test : tests) {
        size_t eoh = HttpUtil::LocateEndOfAdditionalHeaders(base::as_byte_span(test.input));
        EXPECT_EQ(test.expected_result, eoh);
    }
}
...
TEST(HttpUtilTest, GenerateAcceptLanguageHeader)
{
    ...
    EXPECT_EQ(std::string("en-US,fr;q=0.9,de;q=0.8,ko;q=0.7,zh-CN;q=0.6,ja;q=0.5"), header);
}
...
TEST(HttpUtilTest, ParseContentType)
{
  ...
  const struct {
    ...
    for (const auto& test : tests) {
        ...
        std::string boundary;
        HttpUtil::ParseContentType(test.content_type, &mime_type, &charset, &had_charset, &boundary);
        EXPECT_EQ(test.expected_mime_type, mime_type) << "content_type=" << test.content_type;
        EXPECT_EQ(test.expected_charset, charset) << "content_type=" << test.content_type;
        EXPECT_EQ(test.expected_had_charset, had_charset) << "content_type=" << test.content_type;
        EXPECT_EQ(test.expected_boundary, boundary) << "content_type=" << test.content_type;
    }
}
...
TEST(HttpUtilTest, ParseContentResetCharset)
{
    ...
    HttpUtil::ParseContentType("Text/Html", &mime_type, &charset, &had_charset, &boundary);
    EXPECT_EQ("text/html", mime_type);
    EXPECT_EQ("", charset);
    EXPECT_FALSE(had_charset);

    // The same mime, add charset.
    HttpUtil::ParseContentType("tExt/hTml;charset=utf-8", &mime_type, &charset, &had_charset, &boundary);
    EXPECT_EQ("text/html", mime_type);
    ...
    HttpUtil::ParseContentType("texT/plaiN", &mime_type, &charset, &had_charset, &boundary);
...
TEST(HttpUtilTest, ParseContentRangeHeader)
{
    const struct {
        ...
        { "bytes 0-50  /    * ", false, -1, -1, -1 },
        { "bytes 0-10000000000/10000000001", true, 0, 10000000000ll, 10000000001ll },
    ...
    for (const auto& test : tests) {
        ...
        EXPECT_EQ(test.expected_return_value,
            HttpUtil::ParseContentRangeHeaderFor206(test.content_range_header_spec, &first_byte_position, &last_byte_position, &instance_length))
            << test.content_range_header_spec;
        EXPECT_EQ(test.expected_first_byte_position, first_byte_position) << test.content_range_header_spec;
...
TEST(HttpUtilTest, ParseRetryAfterHeader)
{
    ...
    } tests[] = { { "", false, base::TimeDelta() }, { "-3", false, base::TimeDelta() }, { "-2", false, base::TimeDelta() }, { "-1", false, base::TimeDelta() },
        { "+0", false, base::TimeDelta() }, { "+1", false, base::TimeDelta() }, { "0", true, base::Seconds(0) }, { "1", true, base::Seconds(1) },
        { "2", true, base::Seconds(2) }, { "3", true, base::Seconds(3) }, { "60", true, base::Seconds(60) }, { "3600", true, base::Seconds(3600) },
        { "86400", true, base::Seconds(86400) }, { "Thu, 1 Jan 2015 12:34:56 GMT", true, later - now },
        { "Mon, 1 Jan 1900 12:34:56 GMT", false, base::TimeDelta() } };

    for (size_t i = 0; i < std::size(tests); ++i) {
        base::TimeDelta retry_after;
        bool return_value = HttpUtil::ParseRetryAfterHeader(tests[i].retry_after_string, now, &retry_after);
        EXPECT_EQ(tests[i].expected_return_value, return_value)
            << "Test case " << i << ": expected " << tests[i].expected_return_value << " but got " << return_value << ".";
        if (tests[i].expected_return_value && return_value) {
            EXPECT_EQ(tests[i].expected_retry_after, retry_after)
                << "Test case " << i << ": expected " << tests[i].expected_retry_after.InSeconds() << "s but got " << retry_after.InSeconds() << "s.";
        }
    }
}
...
{
    constexpr base::Time::Exploded kTime = { .year = 2011, .month = 4, .day_of_week = 6, .day_of_month = 30, .hour = 22, .minute = 42, .second = 7 };
    base::Time time;
...
void CheckNextNameValuePair(
    HttpUtil::NameValuePairsIterator* parser, bool expect_next, bool expect_valid, std::string expected_name, std::string expected_value)
{
    ...
    CheckCurrentNameValuePair(parser, expect_valid, expected_name, expected_value);
}

void CheckInvalidNameValuePair(std::string valid_part, std::string invalid_part)
{
...
TEST(HttpUtilTest, NameValuePairsIteratorCopyAndAssign)
{
    ...
    ASSERT_NO_FATAL_FAILURE(CheckNextNameValuePair(&parser_a, true, true, "alpha", "\"a\""));

    HttpUtil::NameValuePairsIterator parser_b(parser_a);
    // a and b now point to same location
    ASSERT_NO_FATAL_FAILURE(CheckCurrentNameValuePair(&parser_b, true, "alpha", "\"a\""));
    ASSERT_NO_FATAL_FAILURE(CheckCurrentNameValuePair(&parser_a, true, "alpha", "\"a\""));

    // advance a, no effect on b
    ASSERT_NO_FATAL_FAILURE(CheckNextNameValuePair(&parser_a, true, true, "beta", " b "));
    ASSERT_NO_FATAL_FAILURE(CheckCurrentNameValuePair(&parser_b, true, "alpha", "\"a\""));

    // assign b the current state of a, no effect on a
    parser_b = parser_a;
    ASSERT_NO_FATAL_FAILURE(CheckCurrentNameValuePair(&parser_b, true, "beta", " b "));
    ASSERT_NO_FATAL_FAILURE(CheckCurrentNameValuePair(&parser_a, true, "beta", " b "));

    // advance b, no effect on a
    ASSERT_NO_FATAL_FAILURE(CheckNextNameValuePair(&parser_b, true, true, "cappa", "c;"));
    ASSERT_NO_FATAL_FAILURE(CheckCurrentNameValuePair(&parser_a, true, "beta", " b "));
}
...
TEST(HttpUtilTest, NameValuePairsIteratorEmptyInput)
{
    ...
    ASSERT_NO_FATAL_FAILURE(CheckNextNameValuePair(&parser, false, true, std::string(), std::string()));
}

TEST(HttpUtilTest, NameValuePairsIterator)
{
    std::string data = "alpha=1; beta= 2 ;"
                       "cappa =' 3; foo=';"
                       "cappa =\" 3; foo=\";"
                       "delta= \" \\\"4\\\" \"; e= \" '5'\"; e=6;"
                       "f=\"\\\"\\h\\e\\l\\l\\o\\ \\w\\o\\r\\l\\d\\\"\";"
    ...
    ASSERT_NO_FATAL_FAILURE(CheckNextNameValuePair(&parser, true, true, "alpha", "1"));
    ASSERT_NO_FATAL_FAILURE(CheckNextNameValuePair(&parser, true, true, "beta", "2"));

    // Single quotes shouldn't be treated as quotes.
    ASSERT_NO_FATAL_FAILURE(CheckNextNameValuePair(&parser, true, true, "cappa", "' 3"));
    ASSERT_NO_FATAL_FAILURE(CheckNextNameValuePair(&parser, true, true, "foo", "'"));

    // But double quotes should be, and can contain semi-colons and equal signs.
    ASSERT_NO_FATAL_FAILURE(CheckNextNameValuePair(&parser, true, true, "cappa", " 3; foo="));

    ASSERT_NO_FATAL_FAILURE(CheckNextNameValuePair(&parser, true, true, "delta", " \"4\" "));
    ASSERT_NO_FATAL_FAILURE(CheckNextNameValuePair(&parser, true, true, "e", " '5'"));
    ASSERT_NO_FATAL_FAILURE(CheckNextNameValuePair(&parser, true, true, "e", "6"));
    ASSERT_NO_FATAL_FAILURE(CheckNextNameValuePair(&parser, true, true, "f", "\"hello world\""));
    ASSERT_NO_FATAL_FAILURE(CheckNextNameValuePair(&parser, true, true, "g", std::string()));
    ASSERT_NO_FATAL_FAILURE(CheckNextNameValuePair(&parser, true, true, "h", "hello"));
    ASSERT_NO_FATAL_FAILURE(CheckNextNameValuePair(&parser, false, true, std::string(), std::string()));
}
...
TEST(HttpUtilTest, NameValuePairsIteratorOptionalValues)
{
    ...
    ASSERT_NO_FATAL_FAILURE(CheckNextNameValuePair(&default_parser, true, true, "alpha", "1"));
    ASSERT_NO_FATAL_FAILURE(CheckNextNameValuePair(&default_parser, false, false, std::string(), std::string()));

    HttpUtil::NameValuePairsIterator values_required_parser(
        data, /*delimiter=*/';', HttpUtil::NameValuePairsIterator::Values::REQUIRED, HttpUtil::NameValuePairsIterator::Quotes::NOT_STRICT);
    EXPECT_TRUE(values_required_parser.valid());
    ASSERT_NO_FATAL_FAILURE(CheckNextNameValuePair(&values_required_parser, true, true, "alpha", "1"));
    ASSERT_NO_FATAL_FAILURE(CheckNextNameValuePair(&values_required_parser, false, false, std::string(), std::string()));

    HttpUtil::NameValuePairsIterator parser(
        data, /*delimiter=*/';', HttpUtil::NameValuePairsIterator::Values::NOT_REQUIRED, HttpUtil::NameValuePairsIterator::Quotes::NOT_STRICT);
    EXPECT_TRUE(parser.valid());

    ASSERT_NO_FATAL_FAILURE(CheckNextNameValuePair(&parser, true, true, "alpha", "1"));
    ASSERT_NO_FATAL_FAILURE(CheckNextNameValuePair(&parser, true, true, "beta", std::string()));
    ASSERT_NO_FATAL_FAILURE(CheckNextNameValuePair(&parser, true, true, "cappa", std::string()));
    ASSERT_NO_FATAL_FAILURE(CheckNextNameValuePair(&parser, true, true, "delta", std::string()));
    ASSERT_NO_FATAL_FAILURE(CheckNextNameValuePair(&parser, true, true, "e", std::string()));
    ASSERT_NO_FATAL_FAILURE(CheckNextNameValuePair(&parser, true, true, "f", "1"));
    ASSERT_NO_FATAL_FAILURE(CheckNextNameValuePair(&parser, false, true, std::string(), std::string()));
    EXPECT_TRUE(parser.valid());
}
...
TEST(HttpUtilTest, NameValuePairsIteratorIllegalInputs)
{
    ...
    ASSERT_NO_FATAL_FAILURE(CheckInvalidNameValuePair("alpha=1", ";beta="));
    ASSERT_NO_FATAL_FAILURE(CheckInvalidNameValuePair("alpha=1", ";beta=;cappa=2"));
...
TEST(HttpUtilTest, NameValuePairsIteratorMissingEndQuote)
{
    ...
    ASSERT_NO_FATAL_FAILURE(CheckNextNameValuePair(&parser, true, true, "name", "value"));
    ASSERT_NO_FATAL_FAILURE(CheckNextNameValuePair(&parser, false, true, std::string(), std::string()));
}
...
TEST(HttpUtilTest, NameValuePairsIteratorStrictQuotesEscapedEndQuote)
{
    ...
    HttpUtil::NameValuePairsIterator parser(
        data, /*delimiter=*/';', HttpUtil::NameValuePairsIterator::Values::REQUIRED, HttpUtil::NameValuePairsIterator::Quotes::STRICT_QUOTES);
    EXPECT_TRUE(parser.valid());

    ASSERT_NO_FATAL_FAILURE(CheckNextNameValuePair(&parser, true, true, "foo", "bar"));
    ASSERT_NO_FATAL_FAILURE(CheckNextNameValuePair(&parser, false, false, std::string(), std::string()));
}
...
TEST(HttpUtilTest, NameValuePairsIteratorStrictQuotesSingleQuotes)
{
    ...
    ASSERT_NO_FATAL_FAILURE(CheckNextNameValuePair(&parser, true, true, "foo", "bar"));
    ASSERT_NO_FATAL_FAILURE(CheckNextNameValuePair(&parser, true, true, "name", "'value"));
    ASSERT_NO_FATAL_FAILURE(CheckNextNameValuePair(&parser, true, true, "ok", "it'"));
}
...
TEST(HttpUtilTest, HasValidators)
{
    ...
    EXPECT_FALSE(HttpUtil::HasValidators(v0_9, kEtagStrong, kLastModifiedInvalid));
    ...
    EXPECT_FALSE(HttpUtil::HasValidators(v1_0, kEtagStrong, kLastModifiedInvalid));
...
TEST(HttpUtilTest, IsValidHeaderValue)
{
    ...
    for (const std::string& value : invalid_values) {
        ...
        base::ReplaceSubstringsAfterOffset(&replaced, 0, "${NUL}", std::string(1, '\0'));
...
TEST(HttpUtilTest, ParseAcceptEncoding)
{
    ...
    for (const auto& test : tests) {
        ...
        EXPECT_STREQ(test.expected, reformatted.c_str()) << "value=\"" << value << "\"";
    }
}
...
TEST(HttpUtilTest, ExpandLanguageList)
{
    ...
    EXPECT_EQ("en-US,en-CA,en,fr", HttpUtil::ExpandLanguageList("en-US,en-CA,fr"));
    EXPECT_EQ("en-US,en-CA,en,fr,en-AU", HttpUtil::ExpandLanguageList("en-US,en-CA,fr,en-AU"));
    EXPECT_EQ("en-US,en-CA,en,fr-CA,fr", HttpUtil::ExpandLanguageList("en-US,en-CA,fr-CA"));

    // Add a base language even if it's already in the list.
    EXPECT_EQ("en-US,en,fr-CA,fr,it,es-AR,es,it-IT", HttpUtil::ExpandLanguageList("en-US,fr-CA,it,fr,es-AR,it-IT"));
    // Trims a whitespace.
    EXPECT_EQ("en-US,en,fr", HttpUtil::ExpandLanguageList("en-US, fr"));

    // Do not expand the single character subtag 'x' as a language.
    EXPECT_EQ("x-private-agreement-subtags", HttpUtil::ExpandLanguageList("x-private-agreement-subtags"));
