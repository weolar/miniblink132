...
TEST(HttpBasicStateTest, ConstructsProperly)
{
    ...
    const HttpBasicState state(std::move(handle), true /* is_for_get_to_http_proxy */);
...
{
    const HttpBasicState state(std::make_unique<ClientSocketHandle>(), false /* is_for_get_to_http_proxy */);
...
TEST(HttpBasicStateTest, ReleaseConnectionWorks)
{
    ...
    const std::unique_ptr<StreamSocketHandle> released_connection(state.ReleaseConnection());
...
TEST(HttpBasicStateTest, TrafficAnnotationStored)
{
    ...
    state.Initialize(&request_info, LOW, NetLogWithSource());
    EXPECT_EQ(TRAFFIC_ANNOTATION_FOR_TESTS, NetworkTrafficAnnotationTag(state.traffic_annotation()));
}
