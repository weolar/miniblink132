...
TEST(HTTPRequestInfoTest, IsConsistent)
{
    ...
    with_anon_nak.network_isolation_key = NetworkIsolationKey(kTestSiteA, kTestSiteB);
    EXPECT_FALSE(with_anon_nak.IsConsistent());

    HttpRequestInfo cross_site;
    cross_site.network_isolation_key = NetworkIsolationKey(kTestSiteA, kTestSiteB);
    cross_site.network_anonymization_key = NetworkAnonymizationKey::CreateCrossSite(kTestSiteA);
