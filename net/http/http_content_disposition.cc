...
bool DecodeBQEncoding(std::string_view part, RFC2047EncodingType enc_type, const std::string& charset, std::string* output)
{
    std::string decoded;
...
bool DecodeWord(std::string_view encoded_word, const std::string& referrer_charset, bool* is_rfc2047, std::string* output, int* parse_result_flags)
{
    *is_rfc2047 = false;
    output->clear();
    ...
    *is_rfc2047 = true;
    int part_index = 0;
    std::string charset;
    base::CStringTokenizer t(encoded_word.data(), encoded_word.data() + encoded_word.size(), "?");
    ...
    decoded_word = base::UnescapeBinaryURLComponent(encoded_word, base::UnescapeRule::NORMAL);
...
bool DecodeFilenameValue(std::string_view input, const std::string& referrer_charset, std::string* output, int* parse_result_flags)
{
    ...
    while (t.GetNext()) {
        ...
        // it, either.
        std::string decoded;
        if (!DecodeWord(t.token_piece(), referrer_charset, &is_previous_token_rfc2047, &decoded, &current_parse_result_flags))
            return false;
...
bool ParseExtValueComponents(std::string_view input, std::string* charset, std::string* value_chars)
{
    base::StringViewTokenizer t(input, "'");
...
bool DecodeExtValue(std::string_view param_value, std::string* decoded)
{
    ...
    std::string unescaped = base::UnescapeBinaryURLComponent(value, base::UnescapeRule::NORMAL);
...
HttpContentDisposition::HttpContentDisposition(const std::string& header, const std::string& referrer_charset)
{
...
std::string::const_iterator HttpContentDisposition::ConsumeDispositionType(std::string::const_iterator begin, std::string::const_iterator end)
{
    DCHECK(type_ == INLINE);
...
//
void HttpContentDisposition::Parse(const std::string& header, const std::string& referrer_charset)
{
    DCHECK(type_ == INLINE);
    ...
    while (iter.GetNext()) {
        if (filename.empty() && base::EqualsCaseInsensitiveASCII(iter.name(), "filename")) {
            DecodeFilenameValue(iter.value(), referrer_charset, &filename, &parse_result_flags_);
            if (!filename.empty()) {
                parse_result_flags_ |= HAS_FILENAME;
                if (filename[0] == '\'')
                    parse_result_flags_ |= HAS_SINGLE_QUOTED_FILENAME;
            }
        } else if (ext_filename.empty() && base::EqualsCaseInsensitiveASCII(iter.name(), "filename*")) {
