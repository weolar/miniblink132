...
class NET_EXPORT CachingCertVerifier : public CertVerifier, public CertVerifier::Observer, public CertDatabase::Observer {
public:
...
private:
    ...
    FRIEND_TEST_ALL_PREFIXES(CachingCertVerifierCacheClearingTest, CacheClearedSyncVerification);
    FRIEND_TEST_ALL_PREFIXES(CachingCertVerifierCacheClearingTest, CacheClearedAsyncVerification);
    ...
    struct CacheExpirationFunctor {
        ...
        bool operator()(const CacheValidityPeriod& now, const CacheValidityPeriod& expiration) const;
    };

    using CertVerificationCache = ExpiringCache<RequestParams, CachedResult, CacheValidityPeriod, CacheExpirationFunctor>;
    ...
    void AddResultToCache(uint32_t config_id, const RequestParams& params, base::Time start_time, const CertVerifyResult& verify_result, int error);
