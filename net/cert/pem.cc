...
PEMTokenizer::PEMTokenizer(const StringPiece& str, const std::vector<std::string>& allowed_block_types)
{
...
bool PEMTokenizer::GetNext()
{
    while (pos_ != StringPiece::npos) {
        ...
        for (it = block_types_.begin(); it != block_types_.end(); ++it) {
            ...
            if (!base::Base64Decode(base::CollapseWhitespaceASCII(encoded, true), &data_)) {
...
void PEMTokenizer::Init(const StringPiece& str, const std::vector<std::string>& allowed_block_types)
{
    str_ = str;
    pos_ = 0;
    ...
    for (const auto& allowed_block_type : allowed_block_types) {
        ...
        allowed_type.header = base::StringPrintf(kPEMBeginBlock, allowed_block_type.c_str());
        allowed_type.footer = base::StringPrintf(kPEMEndBlock, allowed_block_type.c_str());
...
std::string PEMEncode(base::StringPiece data, const std::string& type)
{
    ...
    for (size_t i = 0, chunk_offset = 0; i < chunks; ++i, chunk_offset += kChunkSize) {
