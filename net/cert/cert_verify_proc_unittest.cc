...
const char kTrustAnchorVerifyOutOfDateHistogram[] = "Net.Certificate.TrustAnchor.VerifyOutOfDate";
...
{
    constexpr uint8_t kTestOid0SigAlgTLV[] = { 0x30, 0x10, 0x06, 0x0c, 0x2a, 0x86, 0x48, 0x86, 0xf7, 0x12, 0x04, 0x01, 0x84, 0xb7, 0x09, 0x00, 0x05, 0x00 };
    return std::string(std::begin(kTestOid0SigAlgTLV), std::end(kTestOid0SigAlgTLV));
}
...
{
    static uint8_t kTestOid0[] = { 0x06, 0x0c, 0x2a, 0x86, 0x48, 0x86, 0xf7, 0x12, 0x04, 0x01, 0x84, 0xb7, 0x09, 0x00 };
...
private:
    int VerifyInternal(X509Certificate* cert, const std::string& hostname, const std::string& ocsp_response, const std::string& sct_list, int flags,
        CertVerifyResult* verify_result, const NetLogWithSource& net_log) override;

    const CertVerifyResult result_;
    const int error_ = OK;
};

int MockCertVerifyProc::VerifyInternal(X509Certificate* cert, const std::string& hostname, const std::string& ocsp_response, const std::string& sct_list,
    int flags, CertVerifyResult* verify_result, const NetLogWithSource& net_log)
{
...
// tests.
std::string VerifyProcTypeToName(const testing::TestParamInfo<CertVerifyProcType>& params)
{
    switch (params.param) {
...
scoped_refptr<CertVerifyProc> CreateCertVerifyProc(CertVerifyProcType type, scoped_refptr<CertNetFetcher> cert_net_fetcher, scoped_refptr<CRLSet> crl_set,
    CertificateList additional_trust_anchors, CertificateList additional_untrusted_authorities)
{
    CertVerifyProc::InstanceParams instance_params;
    instance_params.additional_trust_anchors = net::x509_util::ParseAllValidCerts(additional_trust_anchors);
    instance_params.additional_untrusted_authorities = net::x509_util::ParseAllValidCerts(additional_untrusted_authorities);
    switch (type) {
#if BUILDFLAG(IS_ANDROID)
    case CERT_VERIFY_PROC_ANDROID:
        return base::MakeRefCounted<CertVerifyProcAndroid>(std::move(cert_net_fetcher), std::move(crl_set));
#elif BUILDFLAG(IS_IOS)
    case CERT_VERIFY_PROC_IOS:
    ...
    case CERT_VERIFY_PROC_BUILTIN:
        return CreateCertVerifyProcBuiltin(std::move(cert_net_fetcher), std::move(crl_set), std::make_unique<DoNothingCTVerifier>(),
            base::MakeRefCounted<DefaultCTPolicyEnforcer>(), CreateSslSystemTrustStore(), instance_params, std::nullopt);
#endif
#if BUILDFLAG(CHROME_ROOT_STORE_SUPPORTED)
    case CERT_VERIFY_PROC_BUILTIN_CHROME_ROOTS:
        return CreateCertVerifyProcBuiltin(std::move(cert_net_fetcher), std::move(crl_set), std::make_unique<DoNothingCTVerifier>(),
            base::MakeRefCounted<DefaultCTPolicyEnforcer>(), CreateSslSystemTrustStoreChromeRoot(std::make_unique<net::TrustStoreChrome>()), instance_params,
            std::nullopt);
#endif
    default:
        return nullptr;
...
{
    return verify_proc_type == CERT_VERIFY_PROC_IOS || verify_proc_type == CERT_VERIFY_PROC_ANDROID;
}
...
// CertVerifyProcType.
bool ScopedTestRootCanTrustIntermediateCert(CertVerifyProcType verify_proc_type)
{
    return verify_proc_type == CERT_VERIFY_PROC_IOS || verify_proc_type == CERT_VERIFY_PROC_BUILTIN || verify_proc_type == CERT_VERIFY_PROC_BUILTIN_CHROME_ROOTS
...
// "VerifyInternal()".
class CertVerifyProcInternalTest : public testing::TestWithParam<CertVerifyProcType> {
protected:
    void SetUp() override
    {
        SetUpCertVerifyProc(CRLSet::BuiltinCRLSet());
    }
    ...
    void SetUpWithCertNetFetcher(scoped_refptr<CertNetFetcher> cert_net_fetcher, scoped_refptr<CRLSet> crl_set, CertificateList additional_trust_anchors,
        CertificateList additional_untrusted_authorities)
    {
        CertVerifyProcType type = verify_proc_type();
        verify_proc_ = CreateCertVerifyProc(type, std::move(cert_net_fetcher), std::move(crl_set), additional_trust_anchors, additional_untrusted_authorities);
    ...
    virtual void SetUpWithAdditionalCerts(CertificateList additional_trust_anchors, CertificateList additional_untrusted_authorities)
    {
        SetUpWithCertNetFetcher(nullptr, CRLSet::BuiltinCRLSet(), additional_trust_anchors, additional_untrusted_authorities);
    }

    int Verify(X509Certificate* cert, const std::string& hostname, int flags, CertVerifyResult* verify_result, const NetLogWithSource& net_log)
    {
        return verify_proc_->Verify(cert, hostname, /*ocsp_response=*/std::string(),
            /*sct_list=*/std::string(), flags, verify_result, net_log);
    }

    int Verify(X509Certificate* cert, const std::string& hostname, int flags, CertVerifyResult* verify_result)
    {
    ...
    CertVerifyProcType verify_proc_type() const
    {
        return GetParam();
    }
    ...
    static bool ParseKeyType(const std::string& key_type, std::string* type, int* size)
    {
    ...
    {
        return verify_proc_type() == CERT_VERIFY_PROC_BUILTIN || verify_proc_type() == CERT_VERIFY_PROC_BUILTIN_CHROME_ROOTS;
    }
    ...
    {
#if BUILDFLAG(IS_IOS)
        if (verify_proc_type() == CERT_VERIFY_PROC_IOS && !base::ios::IsRunningOnIOS16OrLater()) {
            return true;
        }
#endif
        return false;
    ...
    CertVerifyProc* verify_proc() const
    {
        return verify_proc_.get();
    }
...
INSTANTIATE_TEST_SUITE_P(All, CertVerifyProcInternalTest, testing::ValuesIn(kAllCertVerifiers), VerifyProcTypeToName);
...
TEST_P(CertVerifyProcInternalTest, DistrustedIntermediate)
{
    ...
    EXPECT_THAT(Verify(chain.get(), kHostname), IsError(ERR_CERT_AUTHORITY_INVALID));

    // Trusting root should cause chain to verify successfully.
    ScopedTestRoot trust_root(root->GetX509Certificate(), bssl::CertificateTrust::ForTrustAnchor());
    EXPECT_THAT(Verify(chain.get(), kHostname), IsOk());

    ScopedTestRoot distrust_intermediate(intermediate->GetX509Certificate(), bssl::CertificateTrust::ForDistrusted());
...
TEST_P(CertVerifyProcInternalTest, EVVerificationMultipleOID)
{
    ...
    ASSERT_TRUE(asn1::ExtractSPKIFromDERCert(x509_util::CryptoBufferAsStringPiece(root->GetCertBuffer()), &spki));
    ...
    ScopedTestEVPolicy scoped_test_ev_policy(
        EVRootCAMetadata::GetInstance(), X509Certificate::CalculateFingerprint256(root->GetCertBuffer()), kEVTestCertPolicy);
    ScopedTestEVPolicy scoped_test_other_policy(EVRootCAMetadata::GetInstance(), SHA256HashValue(), kOtherTestCertPolicy);
...
TEST_P(CertVerifyProcInternalTest, TrustedTargetCertWithEVPolicy)
{
    ...
    ScopedTestEVPolicy scoped_test_ev_policy(EVRootCAMetadata::GetInstance(), SHA256HashValue(), kEVTestCertPolicy);
...
// explode if it does.
TEST_P(CertVerifyProcInternalTest, TrustedTargetCertWithEVPolicyAndEVFingerprint)
{
    auto [leaf, root] = CertBuilder::CreateSimpleChain2();
    ...
    leaf->SetCertificatePolicies({ kEVTestCertPolicy });
    ScopedTestEVPolicy scoped_test_ev_policy(
        EVRootCAMetadata::GetInstance(), X509Certificate::CalculateFingerprint256(leaf->GetCertBuffer()), kEVTestCertPolicy);
    scoped_refptr<X509Certificate> cert = leaf->GetX509Certificate();
    ScopedTestRoot scoped_test_root(cert);
...
TEST_P(CertVerifyProcInternalTest, TrustedIntermediateCertWithEVPolicy)
{
    ...
    for (bool trust_the_intermediate : { false, true }) {
        ...
        scoped_refptr<X509Certificate> intermediate_cert = intermediate->GetX509Certificate();
...
{
    base::FilePath certs_dir = GetTestNetDataDirectory().AppendASCII("parse_certificate_unittest");
    ...
    scoped_refptr<X509Certificate> ok_cert(ImportCertFromFile(GetTestCertsDirectory(), "ok_cert.pem"));
    ASSERT_TRUE(ok_cert);

    std::vector<bssl::UniquePtr<CRYPTO_BUFFER>> intermediates;
    intermediates.push_back(bssl::UpRef(ok_cert->cert_buffer()));
    scoped_refptr<X509Certificate> cert_with_bad_target(X509Certificate::CreateFromBuffer(bssl::UpRef(bad_cert->cert_buffer()), std::move(intermediates)));
    ASSERT_TRUE(cert_with_bad_target);
    EXPECT_EQ(1U, cert_with_bad_target->intermediate_buffers().size());

    int flags = 0;
    CertVerifyResult verify_result;
    int error = Verify(cert_with_bad_target.get(), "127.0.0.1", flags, &verify_result);
...
{
    ScopedTestRoot test_root(ImportCertFromFile(GetTestCertsDirectory(), "root_ca_cert.pem"));

    base::FilePath certs_dir = GetTestNetDataDirectory().AppendASCII("parse_certificate_unittest");
    bssl::UniquePtr<CRYPTO_BUFFER> bad_cert = x509_util::CreateCryptoBuffer(std::string_view("invalid"));
    ASSERT_TRUE(bad_cert);
    ...
    scoped_refptr<X509Certificate> cert_with_bad_intermediate(X509Certificate::CreateFromBuffer(bssl::UpRef(ok_cert->cert_buffer()), std::move(intermediates)));
    ...
    NetLogWithSource net_log(NetLogWithSource::Make(net::NetLog::Get(), net::NetLogSourceType::CERT_VERIFIER_TASK));
    int flags = 0;
    CertVerifyResult verify_result;
    int error = Verify(cert_with_bad_intermediate.get(), "127.0.0.1", flags, &verify_result, net_log);
    ...
    auto event = base::ranges::find(events, NetLogEventType::CERT_VERIFY_PROC, &NetLogEntry::type);
    ...
    if (VerifyProcTypeIsBuiltin()) {
        event = base::ranges::find(events, NetLogEventType::CERT_VERIFY_PROC_INPUT_CERT, &NetLogEntry::type);
        ASSERT_NE(event, events.end());
        EXPECT_EQ(net::NetLogEventPhase::NONE, event->phase);
        const std::string* errors = event->params.FindString("errors");
        ASSERT_TRUE(errors);
        EXPECT_EQ("ERROR: Failed parsing Certificate SEQUENCE\nERROR: Failed parsing "
                  "Certificate\n",
            *errors);
    }
}
...
TEST_P(CertVerifyProcInternalTest, RejectExpiredCert)
{
    ...
    scoped_refptr<X509Certificate> cert = CreateCertificateChainFromFile(certs_dir, "expired_cert.pem", X509Certificate::FORMAT_AUTO);
    ASSERT_TRUE(cert);
...
TEST_P(CertVerifyProcInternalTest, RejectWeakKeys)
{
    ...
    for (const std::string& ee_type : key_types) {
        for (const std::string& signer_type : key_types) {
            ...
            ASSERT_TRUE(leaf->UseKeyFromFile(certs_dir.AppendASCII(ee_type + "-1.key")));
            ASSERT_TRUE(intermediate->UseKeyFromFile(certs_dir.AppendASCII(signer_type + "-2.key")));

            ScopedTestRoot scoped_root(root->GetX509Certificate());

            CertVerifyResult verify_result;
            int error = Verify(leaf->GetX509CertificateChain().get(), "www.example.com", 0, &verify_result);

            if (IsInvalidKeyType(ee_type) || IsInvalidKeyType(signer_type)) {
                EXPECT_NE(OK, error);
                EXPECT_EQ(CERT_STATUS_INVALID, verify_result.cert_status & CERT_STATUS_INVALID);
            } else if (IsWeakKeyType(ee_type) || IsWeakKeyType(signer_type)) {
                EXPECT_NE(OK, error);
                EXPECT_EQ(CERT_STATUS_WEAK_KEY, verify_result.cert_status & CERT_STATUS_WEAK_KEY);
...
TEST_P(CertVerifyProcInternalTest, ExtraneousRootCert)
{
    ...
    scoped_refptr<X509Certificate> server_cert = leaf_builder->GetX509Certificate();
    ...
    scoped_refptr<X509Certificate> extra_cert = root_builder->GetX509Certificate();

    ScopedTestRoot scoped_root(root_cert);

    std::vector<bssl::UniquePtr<CRYPTO_BUFFER>> intermediates;
    intermediates.push_back(bssl::UpRef(extra_cert->cert_buffer()));
    scoped_refptr<X509Certificate> cert_chain = X509Certificate::CreateFromBuffer(bssl::UpRef(server_cert->cert_buffer()), std::move(intermediates));
    ASSERT_TRUE(cert_chain);

    CertVerifyResult verify_result;
    int flags = 0;
    int error = Verify(cert_chain.get(), "www.example.com", flags, &verify_result);
    EXPECT_THAT(error, IsOk());

    // The extra root should be discarded.
    ASSERT_TRUE(verify_result.verified_cert.get());
    ASSERT_EQ(1u, verify_result.verified_cert->intermediate_buffers().size());
    EXPECT_TRUE(x509_util::CryptoBufferEqual(verify_result.verified_cert->intermediate_buffers().front().get(), root_cert->cert_buffer()));
}
...
TEST_P(CertVerifyProcInternalTest, GoogleDigiNotarTest)
{
    ...
    int error = Verify(cert_chain.get(), "mail.google.com", flags, &verify_result);
    EXPECT_NE(OK, error);
...
TEST_P(CertVerifyProcInternalTest, NameConstraintsOk)
{
    ...
    ASSERT_TRUE(leaf->UseKeyFromFile(GetTestCertsDirectory().AppendASCII("name_constrained_key.pem")));
    // example.com is allowed by kDomainsTest, and notarealtld is not a known
    // TLD, so that's allowed too.
    leaf->SetSubjectAltNames({ "test.ExAmPlE.CoM", "example.notarealtld", "*.test2.ExAmPlE.CoM", "*.example2.notarealtld" }, {});
    ...
    int flags = 0;
    CertVerifyResult verify_result;
    int error = Verify(leaf_cert.get(), "test.example.com", flags, &verify_result);
    EXPECT_THAT(error, IsOk());
    EXPECT_EQ(0U, verify_result.cert_status);

    error = Verify(leaf_cert.get(), "foo.test2.example.com", flags, &verify_result);
    EXPECT_THAT(error, IsOk());
    EXPECT_EQ(0U, verify_result.cert_status);
}
...
TEST_P(CertVerifyProcInternalTest, NameConstraintsFailure)
{
    ...
    EXPECT_EQ(CERT_STATUS_NAME_CONSTRAINT_VIOLATION, verify_result.cert_status & CERT_STATUS_NAME_CONSTRAINT_VIOLATION);
}
...
protected:
    ...
    // algorithm.
    static constexpr bssl::DigestAlgorithm kUnknownDigestAlgorithm = bssl::DigestAlgorithm::Sha384;
    ...
    [[nodiscard]] int VerifyChain(const std::vector<CertParams>& chain_params)
    {
        ...
        std::vector<std::unique_ptr<CertBuilder>> builders = CertBuilder::CreateSimpleChain(chain_params.size());
        for (size_t i = 0; i < chain_params.size(); i++) {
            builders[i]->SetOuterSignatureAlgorithmTLV(base::as_string_view(GetAlgorithmSequence(chain_params[i].cert_algorithm)));
            builders[i]->SetTBSSignatureAlgorithmTLV(base::as_string_view(GetAlgorithmSequence(chain_params[i].tbs_algorithm)));
        }

        scoped_refptr<X509Certificate> chain = builders.front()->GetX509CertificateFullChain();
        if (!chain) {
        ...
        return verify_proc->Verify(chain.get(), "www.example.com", /*ocsp_response=*/std::string(),
            /*sct_list=*/std::string(), flags, &verify_result, NetLogWithSource());
    }

private:
    static base::span<const uint8_t> GetAlgorithmSequence(bssl::DigestAlgorithm algorithm)
    {
        switch (algorithm) {
        case bssl::DigestAlgorithm::Sha1:
            static const uint8_t kSha1WithRSAEncryption[] = { 0x30, 0x0D, 0x06, 0x09, 0x2a, 0x86, 0x48, 0x86, 0xf7, 0x0d, 0x01, 0x01, 0x05, 0x05, 0x00 };
            return kSha1WithRSAEncryption;
        case bssl::DigestAlgorithm::Sha256:
            static const uint8_t kSha256WithRSAEncryption[] = { 0x30, 0x0D, 0x06, 0x09, 0x2a, 0x86, 0x48, 0x86, 0xf7, 0x0d, 0x01, 0x01, 0x0b, 0x05, 0x00 };
            return kSha256WithRSAEncryption;
        case kUnknownDigestAlgorithm:
            static const uint8_t kUnknownAlgorithm[] = { 0x30, 0x0D, 0x06, 0x09, 0x8a, 0x87, 0x18, 0x46, 0xd7, 0x0d, 0x01, 0x01, 0x0b, 0x05, 0x00 };
...
{
    int rv = VerifyLeaf({ bssl::DigestAlgorithm::Sha1, bssl::DigestAlgorithm::Sha1 });
...
{
    int rv = VerifyLeaf({ bssl::DigestAlgorithm::Sha256, bssl::DigestAlgorithm::Sha256 });
    ASSERT_THAT(rv, IsOk());
}
...
{
    int rv = VerifyLeaf({ bssl::DigestAlgorithm::Sha1, bssl::DigestAlgorithm::Sha256 });
...
{
    int rv = VerifyLeaf({ bssl::DigestAlgorithm::Sha256, bssl::DigestAlgorithm::Sha1 });
    ASSERT_THAT(rv, IsError(ERR_CERT_INVALID));
}
...
{
    int rv = VerifyIntermediate({ bssl::DigestAlgorithm::Sha1, bssl::DigestAlgorithm::Sha256 });
    ASSERT_THAT(rv, IsError(ERR_CERT_INVALID));
}
...
{
    int rv = VerifyIntermediate({ bssl::DigestAlgorithm::Sha256, bssl::DigestAlgorithm::Sha1 });
    ASSERT_THAT(rv, IsError(ERR_CERT_INVALID));
}
...
{
    int rv = VerifyRoot({ bssl::DigestAlgorithm::Sha256, bssl::DigestAlgorithm::Sha1 });
    ASSERT_THAT(rv, IsOk());
}
...
TEST(CertVerifyProcTest, TestHasTooLongValidity)
{
    ...
    for (const auto& test : tests) {
        ...
        EXPECT_EQ(test.is_valid_too_long, CertVerifyProc::HasTooLongValidity(*leaf->GetX509Certificate()));
    }
}
...
TEST(CertVerifyProcTest, VerifyCertValidityTooLong)
{
...

    {
        ...
        auto verify_proc = base::MakeRefCounted<MockCertVerifyProc>(dummy_result);
        CertVerifyResult verify_result;
        int error = verify_proc->Verify(leaf->GetX509Certificate().get(), "www.example.com",
...

    {
        ...
        EXPECT_EQ(CERT_STATUS_VALIDITY_TOO_LONG, verify_result.cert_status & CERT_STATUS_ALL_ERRORS);
    }

    {
        ...
        auto verify_proc = base::MakeRefCounted<MockCertVerifyProc>(dummy_result, ERR_CERT_AUTHORITY_INVALID);
        CertVerifyResult verify_result;
        int error = verify_proc->Verify(leaf->GetX509Certificate().get(), "www.example.com",
            /*ocsp_response=*/std::string(),
            /*sct_list=*/std::string(), 0, &verify_result, NetLogWithSource());
        EXPECT_THAT(error, IsError(ERR_CERT_AUTHORITY_INVALID));
        EXPECT_EQ(CERT_STATUS_AUTHORITY_INVALID | CERT_STATUS_VALIDITY_TOO_LONG, verify_result.cert_status & CERT_STATUS_ALL_ERRORS);
    }
}
...
{
    base::FilePath certs_dir = GetTestCertsDirectory();
    scoped_refptr<X509Certificate> cert_chain = CreateCertificateChainFromFile(certs_dir, "leaf_from_known_root.pem", X509Certificate::FORMAT_AUTO);
    ASSERT_TRUE(cert_chain);

    int flags = 0;
    CertVerifyResult verify_result;
    int error = Verify(cert_chain.get(), "timberfirepizza.com", flags, &verify_result);
...
{
    base::FilePath certs_dir = GetTestCertsDirectory();
    CertificateList certs = CreateCertificateListFromFile(certs_dir, "x509_verify_results.chain.pem", X509Certificate::FORMAT_AUTO);
    ...
    scoped_refptr<X509Certificate> cert_chain = X509Certificate::CreateFromBuffer(bssl::UpRef(certs[0]->cert_buffer()), std::move(intermediates));
    ASSERT_TRUE(cert_chain);
    ...
    std::vector<std::string> expected_public_key_hashes = { // Target
    ...
    EXPECT_THAT(expected_public_key_hashes, testing::UnorderedElementsAreArray(public_key_hash_strings));
}
...
TEST_P(CertVerifyProcInternalTest, VerifyReturnChainBasic)
{
    ...
    scoped_refptr<X509Certificate> google_full_chain = X509Certificate::CreateFromBuffer(bssl::UpRef(certs[0]->cert_buffer()), std::move(intermediates));
    ...
    CertVerifyResult verify_result;
    EXPECT_EQ(static_cast<X509Certificate*>(nullptr), verify_result.verified_cert.get());
    int error = Verify(google_full_chain.get(), "127.0.0.1", 0, &verify_result);
    EXPECT_THAT(error, IsOk());
    ASSERT_NE(static_cast<X509Certificate*>(nullptr), verify_result.verified_cert.get());

    EXPECT_TRUE(x509_util::CryptoBufferEqual(google_full_chain->cert_buffer(), verify_result.verified_cert->cert_buffer()));
    const auto& return_intermediates = verify_result.verified_cert->intermediate_buffers();
    ASSERT_EQ(2U, return_intermediates.size());
    EXPECT_TRUE(x509_util::CryptoBufferEqual(return_intermediates[0].get(), certs[1]->cert_buffer()));
    EXPECT_TRUE(x509_util::CryptoBufferEqual(return_intermediates[1].get(), certs[2]->cert_buffer()));
}
...
{
    constexpr SHA256HashValue kSymantecHashValue = { { 0xb2, 0xde, 0xf5, 0x36, 0x2a, 0xd3, 0xfa, 0xcd, 0x04, 0xbd, 0x29, 0x04, 0x7a, 0x43, 0x84, 0x4f, 0x76,
        0x70, 0x34, 0xea, 0x48, 0x92, 0xf8, 0x0e, 0x56, 0xbe, 0xe6, 0x90, 0x24, 0x3e, 0x25, 0x02 } };
    constexpr SHA256HashValue kGoogleHashValue = { { 0xec, 0x72, 0x29, 0x69, 0xcb, 0x64, 0x20, 0x0a, 0xb6, 0x63, 0x8f, 0x68, 0xac, 0x53, 0x8e, 0x40, 0xab, 0xab,
        0x5b, 0x19, 0xa6, 0x48, 0x56, 0x61, 0x04, 0x2a, 0x10, 0x61, 0xc4, 0x61, 0x27, 0x76 } };

    auto [leaf, root] = CertBuilder::CreateSimpleChain2();

    static constexpr base::Time may_1_2016 = base::Time::FromTimeT(1462060800);
    leaf->SetValidity(may_1_2016, may_1_2016 + base::Days(1));
    scoped_refptr<X509Certificate> leaf_pre_june_2016 = leaf->GetX509Certificate();
    ...
    scoped_refptr<X509Certificate> leaf_post_june_2016 = leaf->GetX509Certificate();
    ...
    //                     rejected as of M66.
    for (X509Certificate* cert : { leaf_dec_2017.get(), leaf_pre_june_2016.get() }) {
        ...
        error = verify_proc->Verify(cert, "www.example.com", /*ocsp_response=*/std::string(),
        ...
        error = verify_proc->Verify(cert, "www.example.com", /*ocsp_response=*/std::string(),
            /*sct_list=*/std::string(), CertVerifyProc::VERIFY_DISABLE_SYMANTEC_ENFORCEMENT, &test_result_3, NetLogWithSource());
        EXPECT_THAT(error, IsOk());
    ...
    // ... Or the caller disabled enforcement of Symantec policies.
    CertVerifyResult test_result_3;
    error = verify_proc->Verify(cert.get(), "www.example.com", /*ocsp_response=*/std::string(),
        /*sct_list=*/std::string(), CertVerifyProc::VERIFY_DISABLE_SYMANTEC_ENFORCEMENT, &test_result_3, NetLogWithSource());
    EXPECT_THAT(error, IsOk());
    EXPECT_FALSE(test_result_3.cert_status & CERT_STATUS_SYMANTEC_LEGACY);
}
...
TEST_P(CertVerifyProcInternalTest, VerifyReturnChainFiltersUnrelatedCerts)
{
    ...
    scoped_refptr<X509Certificate> unrelated_certificate2 = ImportCertFromFile(certs_dir, "google.single.pem");
    ASSERT_NE(static_cast<X509Certificate*>(nullptr), unrelated_certificate.get());
    ASSERT_NE(static_cast<X509Certificate*>(nullptr), unrelated_certificate2.get());
...
TEST_P(CertVerifyProcInternalTest, AdditionalTrustAnchors)
{
    ...
    CertificateList ca_cert_list = CreateCertificateListFromFile(GetTestCertsDirectory(), "root_ca_cert.pem", X509Certificate::FORMAT_AUTO);
    ASSERT_EQ(1U, ca_cert_list.size());
    scoped_refptr<X509Certificate> ca_cert(ca_cert_list[0]);

    CertificateList cert_list = CreateCertificateListFromFile(GetTestCertsDirectory(), "ok_cert.pem", X509Certificate::FORMAT_AUTO);
...
TEST_P(CertVerifyProcInternalTest, AdditionalIntermediates)
{
    ...
    EXPECT_THAT(Verify(leaf_cert.get(), kHostname), IsError(ERR_CERT_AUTHORITY_INVALID));
    ...
    EXPECT_TRUE(x509_util::CryptoBufferEqual(verify_result.verified_cert->intermediate_buffers().back().get(), root_cert->cert_buffer()));
    EXPECT_TRUE(x509_util::CryptoBufferEqual(verify_result.verified_cert->intermediate_buffers().front().get(), intermediate_cert->cert_buffer()));
    EXPECT_FALSE(verify_result.is_issued_by_additional_trust_anchor);
}
...
TEST_P(CertVerifyProcInternalTest, AdditionalIntermediateDuplicatesRoot)
{
    ...
    SetUpWithAdditionalCerts({}, { root->GetX509Certificate(), intermediate->GetX509Certificate() });
    CertVerifyResult verify_result;
    int error = Verify(leaf_cert.get(), kHostname, /*flags=*/0, &verify_result);
    EXPECT_THAT(error, IsOk());
    ASSERT_TRUE(verify_result.verified_cert);
...
TEST_P(CertVerifyProcInternalTest, AdditionalTrustAnchorDuplicateIntermediate)
{
    ...
    EXPECT_THAT(Verify(leaf->GetX509Certificate().get(), kHostname), IsError(ERR_CERT_AUTHORITY_INVALID));
...
TEST_P(CertVerifyProcInternalTest, CRLSet)
{
    ...
    EXPECT_TRUE(base::ReadFileToString(GetTestCertsDirectory().AppendASCII("crlset_by_leaf_spki.raw"), &crl_set_bytes));
    ...
    // root.
    crl_set_bytes.clear();
    EXPECT_TRUE(base::ReadFileToString(GetTestCertsDirectory().AppendASCII("crlset_by_root_serial.raw"), &crl_set_bytes));
    ASSERT_TRUE(CRLSet::Parse(crl_set_bytes, &crl_set));
...
TEST_P(CertVerifyProcInternalTest, CRLSetLeafSerial)
{
    ...
    scoped_refptr<X509Certificate> leaf = CreateCertificateChainFromFile(GetTestCertsDirectory(), "ok_cert_by_intermediate.pem", X509Certificate::FORMAT_AUTO);
    ASSERT_TRUE(leaf);
    ...
    scoped_refptr<CRLSet> crl_set;
    std::string crl_set_bytes;
    ASSERT_TRUE(base::ReadFileToString(GetTestCertsDirectory().AppendASCII("crlset_by_intermediate_serial.raw"), &crl_set_bytes));
    ASSERT_TRUE(CRLSet::Parse(crl_set_bytes, &crl_set));
...
TEST_P(CertVerifyProcInternalTest, CRLSetRootReturnsChain)
{
    ...
    scoped_refptr<CRLSet> crl_set;
    std::string crl_set_bytes;
    ASSERT_TRUE(base::ReadFileToString(GetTestCertsDirectory().AppendASCII("crlset_by_root_spki.raw"), &crl_set_bytes));
    ASSERT_TRUE(CRLSet::Parse(crl_set_bytes, &crl_set));
...
TEST_P(CertVerifyProcInternalTest, CRLSetRevokedBySubject)
{
    ...
    scoped_refptr<X509Certificate> root(ImportCertFromFile(GetTestCertsDirectory(), "root_ca_cert.pem"));
    ASSERT_TRUE(root);

    scoped_refptr<X509Certificate> leaf(ImportCertFromFile(GetTestCertsDirectory(), "ok_cert.pem"));
    ASSERT_TRUE(leaf);
    ...
    ASSERT_TRUE(base::ReadFileToString(GetTestCertsDirectory().AppendASCII("crlset_by_leaf_subject_no_spki.raw"), &crl_set_bytes));
    ASSERT_TRUE(CRLSet::Parse(crl_set_bytes, &crl_set));
    ...
    ASSERT_TRUE(base::ReadFileToString(GetTestCertsDirectory().AppendASCII("crlset_by_root_subject_no_spki.raw"), &crl_set_bytes));
    ASSERT_TRUE(CRLSet::Parse(crl_set_bytes, &crl_set));
    ...
    ASSERT_TRUE(base::ReadFileToString(GetTestCertsDirectory().AppendASCII("crlset_by_root_subject.raw"), &crl_set_bytes));
    ASSERT_TRUE(CRLSet::Parse(crl_set_bytes, &crl_set));
...
TEST_P(CertVerifyProcInternalTest, BlockedInterceptionByRoot)
{
    ...
    scoped_refptr<X509Certificate> cert = CreateCertificateChainFromFile(GetTestCertsDirectory(), "ok_cert_by_intermediate.pem", X509Certificate::FORMAT_AUTO);
    ASSERT_TRUE(cert);
    ...
    if (SupportsCRLSet()) {
        ...
        EXPECT_TRUE(verify_result.cert_status & CERT_STATUS_KNOWN_INTERCEPTION_BLOCKED);
    } else {
        EXPECT_THAT(error, IsOk());
        EXPECT_TRUE(verify_result.cert_status & CERT_STATUS_KNOWN_INTERCEPTION_DETECTED);
    }
}
...
TEST_P(CertVerifyProcInternalTest, BlockedInterceptionByIntermediate)
{
    ...
    std::string crl_set_bytes;
    ASSERT_TRUE(base::ReadFileToString(GetTestCertsDirectory().AppendASCII("crlset_blocked_interception_by_intermediate.raw"), &crl_set_bytes));
    ASSERT_TRUE(CRLSet::Parse(crl_set_bytes, &crl_set));
    ...
    scoped_refptr<X509Certificate> second_cert = CreateCertificateChainFromFile(GetTestCertsDirectory(), "ok_cert.pem", X509Certificate::FORMAT_AUTO);
...
TEST_P(CertVerifyProcInternalTest, DetectsInterceptionByRoot)
{
    ...
    // Read in a CRLSet that marks the root as blocked for interception.
    std::string crl_set_bytes;
    ASSERT_TRUE(base::ReadFileToString(GetTestCertsDirectory().AppendASCII("crlset_known_interception_by_root.raw"), &crl_set_bytes));
    ASSERT_TRUE(CRLSet::Parse(crl_set_bytes, &crl_set));
...
TEST_P(CertVerifyProcInternalTest, CRLSetDuringPathBuilding)
{
    ...
    ASSERT_TRUE(LoadCertificateFiles({ "multi-root-A-by-B.pem", "multi-root-B-by-C.pem", "multi-root-C-by-D.pem", "multi-root-D-by-D.pem" }, &path_1_certs));

    CertificateList path_2_certs;
    ASSERT_TRUE(LoadCertificateFiles({ "multi-root-A-by-B.pem", "multi-root-B-by-C.pem", "multi-root-C-by-E.pem", "multi-root-E-by-E.pem" }, &path_2_certs));

    CertificateList path_3_certs;
    ASSERT_TRUE(LoadCertificateFiles({ "multi-root-A-by-B.pem", "multi-root-B-by-F.pem", "multi-root-F-by-E.pem", "multi-root-E-by-E.pem" }, &path_3_certs));
    ...
    std::vector<bssl::UniquePtr<CRYPTO_BUFFER>> intermediates;
    intermediates.push_back(bssl::UpRef(path_1_certs[1]->cert_buffer())); // B-by-C
    intermediates.push_back(bssl::UpRef(path_1_certs[2]->cert_buffer())); // C-by-D
    intermediates.push_back(bssl::UpRef(path_2_certs[2]->cert_buffer())); // C-by-E
    intermediates.push_back(bssl::UpRef(path_3_certs[1]->cert_buffer())); // B-by-F
    intermediates.push_back(bssl::UpRef(path_3_certs[2]->cert_buffer())); // F-by-E
    scoped_refptr<X509Certificate> cert = X509Certificate::CreateFromBuffer(bssl::UpRef(path_1_certs[0]->cert_buffer()), std::move(intermediates));
    ASSERT_TRUE(cert);
    ...
    } kTests[] = { { "multi-root-crlset-D-and-E.raw", false, nullptr }, { "multi-root-crlset-E.raw", true, path_1_certs[2].get() },
        { "multi-root-crlset-CD-and-FE.raw", true, path_2_certs[2].get() }, { "multi-root-crlset-C.raw", true, path_3_certs[2].get() },
    ...
    for (const auto& testcase : kTests) {
        ...
        scoped_refptr<CRLSet> crl_set;
        std::string crl_set_bytes;
        EXPECT_TRUE(base::ReadFileToString(GetTestCertsDirectory().AppendASCII(testcase.crlset), &crl_set_bytes));
        ASSERT_TRUE(CRLSet::Parse(crl_set_bytes, &crl_set));
        ...
        const auto& verified_intermediates = verify_result.verified_cert->intermediate_buffers();
        ASSERT_EQ(3U, verified_intermediates.size());

        scoped_refptr<X509Certificate> intermediate = X509Certificate::CreateFromBuffer(bssl::UpRef(verified_intermediates[1].get()), {});
        ASSERT_TRUE(intermediate);

        EXPECT_TRUE(testcase.expected_intermediate->EqualsExcludingChain(intermediate.get()))
            << "Expected: " << testcase.expected_intermediate->subject().common_name << " issued by " << testcase.expected_intermediate->issuer().common_name
            << "; Got: " << intermediate->subject().common_name << " issued by " << intermediate->issuer().common_name;
    }
}
...
TEST_P(CertVerifyProcInternalTest, FailedIntermediateSignatureValidation)
{
    ...
    intermediate->EraseExtension(bssl::der::Input(bssl::kAuthorityKeyIdentifierOid));
...
TEST_P(CertVerifyProcInternalTest, FailedTargetSignatureValidation)
{
    ...
    intermediate->EraseExtension(bssl::der::Input(bssl::kSubjectKeyIdentifierOid));
    ...
    scoped_refptr<X509Certificate> cert = X509Certificate::CreateFromBuffer(bssl::UpRef(leaf_wrong_signature), std::move(intermediates));
    ASSERT_TRUE(cert.get());
...
protected:
    ...
    {
        if (verify_error == OK) {
            histograms_.ExpectUniqueSample(HistogramName(), CertVerifyProc::NameNormalizationResult::kNormalized, 1);
        } else {
    ...
    {
        histograms_.ExpectUniqueSample(HistogramName(), CertVerifyProc::NameNormalizationResult::kByteEqual, 1);
    }
...
INSTANTIATE_TEST_SUITE_P(All, CertVerifyProcNameNormalizationTest, testing::ValuesIn(kAllCertVerifiers), VerifyProcTypeToName);
...
TEST_P(CertVerifyProcNameNormalizationTest, StringType)
{
    ...
    leaf->SetIssuerTLV(CertBuilder::BuildNameWithCommonNameOfType(issuer_cn, CBS_ASN1_PRINTABLESTRING));
    intermediate->SetSubjectTLV(CertBuilder::BuildNameWithCommonNameOfType(issuer_cn, CBS_ASN1_UTF8STRING));

    ScopedTestRoot scoped_root(root->GetX509Certificate());

    int flags = 0;
    CertVerifyResult verify_result;
    int error = Verify(leaf->GetX509CertificateChain().get(), "www.example.com", flags, &verify_result);
...
TEST_P(CertVerifyProcNameNormalizationTest, CaseFolding)
{
    ...
    leaf->SetIssuerTLV(CertBuilder::BuildNameWithCommonNameOfType("Z" + issuer_hex, CBS_ASN1_PRINTABLESTRING));
    intermediate->SetSubjectTLV(CertBuilder::BuildNameWithCommonNameOfType("z" + issuer_hex, CBS_ASN1_PRINTABLESTRING));
...
TEST_P(CertVerifyProcNameNormalizationTest, ByteEqual)
{
    ...
    std::string issuer_hex = CertBuilder::MakeRandomHexString(12);
    leaf->SetIssuerTLV(CertBuilder::BuildNameWithCommonNameOfType(issuer_hex, CBS_ASN1_PRINTABLESTRING));
    intermediate->SetSubjectTLV(CertBuilder::BuildNameWithCommonNameOfType(issuer_hex, CBS_ASN1_PRINTABLESTRING));
...
{
    const uint8_t kMd5WithRSAEncryption[] = { 0x30, 0x0d, 0x06, 0x09, 0x2a, 0x86, 0x48, 0x86, 0xf7, 0x0d, 0x01, 0x01, 0x04, 0x05, 0x00 };
    return std::string(std::begin(kMd5WithRSAEncryption), std::end(kMd5WithRSAEncryption));
}
...
class CertVerifyProcInternalWithNetFetchingTest : public CertVerifyProcInternalTest {
protected:
    ...
    void SetUp() override
    {
        ...
        base::WaitableEvent initialization_complete_event(base::WaitableEvent::ResetPolicy::MANUAL, base::WaitableEvent::InitialState::NOT_SIGNALED);
        network_thread_->task_runner()->PostTask(
            FROM_HERE, base::BindOnce(&SetUpOnNetworkThread, &context_, &cert_net_fetcher_, &initialization_complete_event));
        ...
        // prior to Start().
        test_server_.RegisterRequestHandler(base::BindRepeating(&CertVerifyProcInternalWithNetFetchingTest::DispatchToRequestHandler, base::Unretained(this)));
    ...
    void TearDown() override
    {
        ...
        network_thread_->task_runner()->PostTask(FROM_HERE, base::BindOnce(&ShutdownOnNetworkThread, &context_, &cert_net_fetcher_));
    ...
    GURL RegisterSimpleTestServerHandler(std::string path, HttpStatusCode status_code, std::string content_type, std::string content)
    {
        GURL handler_url(GetTestServerAbsoluteUrl(path));
        base::AutoLock lock(request_handlers_lock_);
        request_handlers_.push_back(base::BindRepeating(&SimpleTestServerHandler, std::move(path), status_code, std::move(content_type), std::move(content)));
        return handler_url;
    }
    ...
    void CreateSimpleChainWithAIA(scoped_refptr<X509Certificate>* out_leaf, std::string* ca_issuers_path, bssl::UniquePtr<CRYPTO_BUFFER>* out_intermediate,
    ...
    GURL CreateAndServeCrl(
        CertBuilder* crl_issuer, const std::vector<uint64_t>& revoked_serials, std::optional<bssl::SignatureAlgorithm> signature_algorithm = std::nullopt)
    {
        std::string crl = BuildCrl(crl_issuer->GetSubject(), crl_issuer->GetKey(), revoked_serials, signature_algorithm);
        std::string crl_path = MakeRandomPath(".crl");
        return RegisterSimpleTestServerHandler(crl_path, HTTP_OK, "application/pkix-crl", crl);
    }

    GURL CreateAndServeCrlWithAlgorithmTlvAndDigest(
        CertBuilder* crl_issuer, const std::vector<uint64_t>& revoked_serials, const std::string& signature_algorithm_tlv, const EVP_MD* digest)
    {
        std::string crl = BuildCrlWithAlgorithmTlvAndDigest(crl_issuer->GetSubject(), crl_issuer->GetKey(), revoked_serials, signature_algorithm_tlv, digest);
        std::string crl_path = MakeRandomPath(".crl");
        return RegisterSimpleTestServerHandler(crl_path, HTTP_OK, "application/pkix-crl", crl);
    }

private:
    std::unique_ptr<test_server::HttpResponse> DispatchToRequestHandler(const test_server::HttpRequest& request)
    {
    ...
    // requests for |path|.
    static std::unique_ptr<test_server::HttpResponse> SimpleTestServerHandler(const std::string& path, HttpStatusCode status_code,
        const std::string& content_type, const std::string& content, const test_server::HttpRequest& request)
    {
    ...
    static void SetUpOnNetworkThread(std::unique_ptr<URLRequestContext>* context, scoped_refptr<CertNetFetcherURLRequest>* cert_net_fetcher,
        base::WaitableEvent* initialization_complete_event)
    {
        ...
        url_request_context_builder.set_proxy_config_service(std::make_unique<ProxyConfigServiceFixed>(ProxyConfigWithAnnotation()));
    ...
    static void ShutdownOnNetworkThread(std::unique_ptr<URLRequestContext>* context, scoped_refptr<net::CertNetFetcherURLRequest>* cert_net_fetcher)
    {
    ...
    std::vector<test_server::EmbeddedTestServer::HandleRequestCallback> request_handlers_;
};

INSTANTIATE_TEST_SUITE_P(All, CertVerifyProcInternalWithNetFetchingTest, testing::ValuesIn(kAllCertVerifiers), VerifyProcTypeToName);
...
TEST_P(CertVerifyProcInternalWithNetFetchingTest, MAYBE_IntermediateFromAia404)
{
    ...
    RegisterSimpleTestServerHandler(ca_issuers_path, HTTP_NOT_FOUND, "text/plain", "Not Found");
...
#endif
TEST_P(CertVerifyProcInternalWithNetFetchingTest, MAYBE_IntermediateFromAia200Der)
{
    const char kHostname[] = "www.example.com";
    ...
    RegisterSimpleTestServerHandler(ca_issuers_path, HTTP_OK, "application/pkix-cert", std::string(x509_util::CryptoBufferAsStringPiece(intermediate.get())));
    ...
    if (verify_proc_type() != CERT_VERIFY_PROC_IOS) {
        ...
        // can't be found.
        int error = Verify(leaf.get(), kHostname, CertVerifyProc::VERIFY_DISABLE_NETWORK_FETCHES, &verify_result);
        EXPECT_THAT(error, IsError(ERR_CERT_AUTHORITY_INVALID));
...
#endif
TEST_P(CertVerifyProcInternalWithNetFetchingTest, MAYBE_IntermediateFromAia200Pem)
{
    const char kHostname[] = "www.example.com";
    ...
    ASSERT_TRUE(X509Certificate::GetPEMEncoded(intermediate.get(), &intermediate_pem));

    // Setup the test server to reply with the correct intermediate.
    RegisterSimpleTestServerHandler(ca_issuers_path, HTTP_OK, "application/x-x509-ca-cert", intermediate_pem);
...
#endif
TEST_P(CertVerifyProcInternalWithNetFetchingTest, MAYBE_IntermediateFromAia200Pem2)
{
    const char kHostname[] = "www.example.com";
...
TEST_P(CertVerifyProcInternalWithNetFetchingTest, Sha1IntermediateButAIAHasSha256)
{
    const char kHostname[] = "www.example.com";
    ...
    RegisterSimpleTestServerHandler(
        ca_issuers_path, HTTP_OK, "application/pkix-cert", std::string(x509_util::CryptoBufferAsStringPiece(intermediate_sha256.get())));

    // Build a chain to verify that includes the SHA1 intermediate.
    std::vector<bssl::UniquePtr<CRYPTO_BUFFER>> intermediates;
    intermediates.push_back(bssl::UpRef(intermediate_sha1.get()));
    scoped_refptr<X509Certificate> chain_sha1 = X509Certificate::CreateFromBuffer(leaf->DupCertBuffer(), std::move(intermediates));
    ...
    if (VerifyProcTypeIsBuiltin()) {
        ...
        EXPECT_TRUE(x509_util::CryptoBufferEqual(verify_result.verified_cert->intermediate_buffers()[0].get(), intermediate_sha256.get()));
        ASSERT_EQ(2u, verify_result.verified_cert->intermediate_buffers().size());
        ...
        EXPECT_NE(OK, error);
        if (verify_proc_type() == CERT_VERIFY_PROC_ANDROID && error == ERR_CERT_AUTHORITY_INVALID) {
...
TEST_P(CertVerifyProcInternalWithNetFetchingTest, RevocationHardFailNoCrlsDisableNetworkFetches)
{
    ...
    const int flags = CertVerifyProc::VERIFY_REV_CHECKING_REQUIRED_LOCAL_ANCHORS | CertVerifyProc::VERIFY_DISABLE_NETWORK_FETCHES;
    CertVerifyResult verify_result;
    int error = Verify(chain.get(), kHostname, flags, &verify_result);
...
// should succeed.
TEST_P(CertVerifyProcInternalWithNetFetchingTest, RevocationHardFailCrlGoodNoRevokedCertificates)
{
    if (!SupportsRevCheckingRequiredLocalAnchors()) {
        LOG(INFO) << "Skipping test as verifier doesn't support "
                     "VERIFY_REV_CHECKING_REQUIRED_LOCAL_ANCHORS";
...
TEST_P(CertVerifyProcInternalWithNetFetchingTest, RevocationHardFailCrlGoodIrrelevantSerialsRevoked)
{
    if (!SupportsRevCheckingRequiredLocalAnchors()) {
        LOG(INFO) << "Skipping test as verifier doesn't support "
                     "VERIFY_REV_CHECKING_REQUIRED_LOCAL_ANCHORS";
    ...
    intermediate->SetCrlDistributionPointUrl(CreateAndServeCrl(root.get(), { leaf->GetSerialNumber() }));

    // Intermediate-issued CRL revokes intermediates's serial number. This is
    // irrelevant.
    leaf->SetCrlDistributionPointUrl(CreateAndServeCrl(intermediate.get(), { intermediate->GetSerialNumber() }));
...
TEST_P(CertVerifyProcInternalWithNetFetchingTest, RevocationHardFailLeafRevokedByCrl)
{
    if (!SupportsRevCheckingRequiredLocalAnchors()) {
        LOG(INFO) << "Skipping test as verifier doesn't support "
                     "VERIFY_REV_CHECKING_REQUIRED_LOCAL_ANCHORS";
    ...
    leaf->SetCrlDistributionPointUrl(CreateAndServeCrl(intermediate.get(), { leaf->GetSerialNumber() }));
...
TEST_P(CertVerifyProcInternalWithNetFetchingTest, RevocationHardFailIntermediateRevokedByCrl)
{
    if (!SupportsRevCheckingRequiredLocalAnchors()) {
        LOG(INFO) << "Skipping test as verifier doesn't support "
                     "VERIFY_REV_CHECKING_REQUIRED_LOCAL_ANCHORS";
    ...
    intermediate->SetCrlDistributionPointUrl(CreateAndServeCrl(root.get(), { intermediate->GetSerialNumber() }));
...
// fail.
TEST_P(CertVerifyProcInternalWithNetFetchingTest, RevocationHardFailLeafCrlDpHttpError)
{
    if (!SupportsRevCheckingRequiredLocalAnchors()) {
        LOG(INFO) << "Skipping test as verifier doesn't support "
                     "VERIFY_REV_CHECKING_REQUIRED_LOCAL_ANCHORS";
    ...
    leaf->SetCrlDistributionPointUrl(RegisterSimpleTestServerHandler(MakeRandomPath(".crl"), HTTP_NOT_FOUND, "text/plain", "Not Found"));
...
// should fail.
TEST_P(CertVerifyProcInternalWithNetFetchingTest, RevocationHardFailIntermediateCrlDpHttpError)
{
    if (!SupportsRevCheckingRequiredLocalAnchors()) {
        LOG(INFO) << "Skipping test as verifier doesn't support "
                     "VERIFY_REV_CHECKING_REQUIRED_LOCAL_ANCHORS";
    ...
    intermediate->SetCrlDistributionPointUrl(RegisterSimpleTestServerHandler(MakeRandomPath(".crl"), HTTP_NOT_FOUND, "text/plain", "Not Found"));
...
TEST_P(CertVerifyProcInternalWithNetFetchingTest, RevocationSoftFailNoCrls)
{
    ...
    EXPECT_FALSE(verify_result.cert_status & CERT_STATUS_UNABLE_TO_CHECK_REVOCATION);
    EXPECT_TRUE(verify_result.cert_status & CERT_STATUS_REV_CHECKING_ENABLED);
}
...
// CRLs which have empty (non-present) revokedCertificates list. Verification
// should succeed.
TEST_P(CertVerifyProcInternalWithNetFetchingTest, RevocationSoftFailCrlGoodNoRevokedCertificates)
{
    if (!SupportsSoftFailRevChecking()) {
        LOG(INFO) << "Skipping test as verifier doesn't support "
                     "VERIFY_REV_CHECKING_ENABLED";
...
// CRLs which have revokedCertificates lists that revoke other irrelevant
// serial numbers. Verification should succeed.
TEST_P(CertVerifyProcInternalWithNetFetchingTest, RevocationSoftFailCrlGoodIrrelevantSerialsRevoked)
{
    if (!SupportsSoftFailRevChecking()) {
        LOG(INFO) << "Skipping test as verifier doesn't support "
                     "VERIFY_REV_CHECKING_ENABLED";
...
TEST_P(CertVerifyProcInternalWithNetFetchingTest, RevocationSoftFailLeafRevokedByCrl)
{
    if (!SupportsSoftFailRevChecking()) {
        LOG(INFO) << "Skipping test as verifier doesn't support "
                     "VERIFY_REV_CHECKING_ENABLED";
...
TEST_P(CertVerifyProcInternalWithNetFetchingTest, RevocationSoftFailLeafRevokedByCrlDisableNetworkFetches)
{
    if (!SupportsSoftFailRevChecking()) {
        LOG(INFO) << "Skipping test as verifier doesn't support "
                     "VERIFY_REV_CHECKING_ENABLED";
    ...
    // network fetches.
    const int flags = CertVerifyProc::VERIFY_REV_CHECKING_ENABLED | CertVerifyProc::VERIFY_DISABLE_NETWORK_FETCHES;
    CertVerifyResult verify_result;
    int error = Verify(chain.get(), kHostname, flags, &verify_result);
...
TEST_P(CertVerifyProcInternalWithNetFetchingTest, RevocationSoftFailLeafRevokedBySha1Crl)
{
    if (!SupportsSoftFailRevChecking()) {
        LOG(INFO) << "Skipping test as verifier doesn't support "
                     "VERIFY_REV_CHECKING_ENABLED";
    ...
    // ecdsaWithSha256.
    leaf->SetCrlDistributionPointUrl(CreateAndServeCrl(intermediate.get(), { leaf->GetSerialNumber() }, bssl::SignatureAlgorithm::kEcdsaSha1));
...
TEST_P(CertVerifyProcInternalWithNetFetchingTest, RevocationSoftFailLeafRevokedByMd5Crl)
{
    if (!SupportsSoftFailRevChecking()) {
        LOG(INFO) << "Skipping test as verifier doesn't support "
                     "VERIFY_REV_CHECKING_ENABLED";
    ...
    ASSERT_TRUE(intermediate->UseKeyFromFile(GetTestCertsDirectory().AppendASCII("rsa-2048-1.key")));
    leaf->SetSignatureAlgorithm(bssl::SignatureAlgorithm::kRsaPkcs1Sha256);

    // Leaf is revoked by intermediate issued CRL which is signed with
    // md5WithRSAEncryption.
    leaf->SetCrlDistributionPointUrl(
        CreateAndServeCrlWithAlgorithmTlvAndDigest(intermediate.get(), { leaf->GetSerialNumber() }, Md5WithRSAEncryption(), EVP_md5()));
...
// the leaf's distribution point returns an http error. Verification should
// succeed.
TEST_P(CertVerifyProcInternalWithNetFetchingTest, RevocationSoftFailLeafCrlDpHttpError)
{
    if (!SupportsSoftFailRevChecking()) {
        LOG(INFO) << "Skipping test as verifier doesn't support "
                     "VERIFY_REV_CHECKING_ENABLED";
...
// the intermediate's distribution point returns an http error. Verification
// should succeed.
TEST_P(CertVerifyProcInternalWithNetFetchingTest, RevocationSoftFailIntermediateCrlDpHttpError)
{
    if (!SupportsSoftFailRevChecking()) {
        LOG(INFO) << "Skipping test as verifier doesn't support "
                     "VERIFY_REV_CHECKING_ENABLED";
...
TEST_P(CertVerifyProcInternalWithNetFetchingTest, EVOnlineOCSPRevocationCheckingGood)
{
    if (!SupportsEV()) {
        LOG(INFO) << "Skipping test as EV verification is not yet supported";
        return;
    ...
    // Consider the root of the test chain a valid EV root for the test policy.
    ScopedTestEVPolicy scoped_test_ev_policy(EVRootCAMetadata::GetInstance(), X509Certificate::CalculateFingerprint256(root->cert_buffer()), kEVTestCertPolicy);

    CertVerifyResult verify_result;
    int flags = 0;
    int error = Verify(chain.get(), ocsp_test_server.host_port_pair().host(), flags, &verify_result);
    EXPECT_THAT(error, IsOk());
    EXPECT_TRUE(verify_result.cert_status & CERT_STATUS_IS_EV);
    EXPECT_FALSE(verify_result.cert_status & CERT_STATUS_REV_CHECKING_ENABLED);
}
...
TEST_P(CertVerifyProcInternalWithNetFetchingTest, EVOnlineOCSPRevocationCheckingSoftFail)
{
    if (!SupportsEV()) {
        LOG(INFO) << "Skipping test as EV verification is not yet supported";
        return;
    ...
    cert_config.ocsp_config = EmbeddedTestServer::OCSPConfig(EmbeddedTestServer::OCSPConfig::ResponseType::kInternalError);
...
TEST_P(CertVerifyProcInternalWithNetFetchingTest, EVOnlineOCSPRevocationCheckingRevoked)
{
    if (!SupportsEV()) {
        LOG(INFO) << "Skipping test as EV verification is not yet supported";
        return;
...
protected:
    ...
    int VerifyWithTrust(bssl::CertificateTrust trust)
    {
        ...
        CertVerifyResult verify_result;
        int flags = 0;
        return CertVerifyProcInternalTest::Verify(chain_.front()->GetX509CertificateChain().get(), "www.example.com", flags, &verify_result);
    }
    ...
    {
        return VerifyWithTrust(bssl::CertificateTrust::ForTrustAnchor().WithEnforceAnchorExpiry().WithEnforceAnchorConstraints());
    }

    int VerifyWithExpiryAndFullConstraints()
    {
        return VerifyWithTrust(
            bssl::CertificateTrust::ForTrustAnchor().WithEnforceAnchorExpiry().WithEnforceAnchorConstraints().WithRequireAnchorBasicConstraints());
    }
...
INSTANTIATE_TEST_SUITE_P(All, CertVerifyProcConstraintsTest, testing::ValuesIn(kAllCertVerifiers), VerifyProcTypeToName);
...
TEST_P(CertVerifyProcConstraintsTest, BaseCase)
{
    ...
    if (VerifyProcTypeIsBuiltin()) {
        ...
        EXPECT_THAT(VerifyWithTrust(bssl::CertificateTrust::ForTrustAnchorOrLeaf()), IsOk());
        EXPECT_THAT(VerifyWithTrust(bssl::CertificateTrust::ForTrustedLeaf()), IsError(ERR_CERT_AUTHORITY_INVALID));
    }
}
...
TEST_P(CertVerifyProcConstraintsTest, BasicConstraintsNotCaRoot)
{
    ...
    if (VerifyProcTypeIsBuiltin()) {
        ...
        EXPECT_THAT(VerifyWithExpiryAndFullConstraints(), IsError(ERR_CERT_INVALID));
...
TEST_P(CertVerifyProcConstraintsTest, BasicConstraintsIsCaLeaf)
{
    for (bool has_key_usage_cert_sign : { false, true }) {
        ...
        if (has_key_usage_cert_sign) {
            chain_[0]->SetKeyUsages({ bssl::KEY_USAGE_BIT_KEY_CERT_SIGN, bssl::KEY_USAGE_BIT_DIGITAL_SIGNATURE });
        } else {
...
TEST_P(CertVerifyProcConstraintsTest, BasicConstraintsPathlen0Root)
{
    ...
    if (VerifyProcTypeIsBuiltin()) {
        EXPECT_THAT(Verify(), IsOk());
        EXPECT_THAT(VerifyWithExpiryAndConstraints(), IsError(ERR_CERT_INVALID));
    } else if (VerifyProcTypeIsIOSAtMostOS14() || verify_proc_type() == CERT_VERIFY_PROC_ANDROID) {
        EXPECT_THAT(Verify(), IsOk());
    } else {
        EXPECT_THAT(Verify(), IsError(ERR_CERT_INVALID));
    }
...
TEST_P(CertVerifyProcConstraintsTest, BasicConstraintsPathlen0IntermediateParent)
{
...
TEST_P(CertVerifyProcConstraintsTest, BasicConstraintsPathlen1IntermediateParent)
{
...
TEST_P(CertVerifyProcConstraintsTest, BasicConstraintsPathlen0IntermediateChild)
{
...
{
    chain_[3]->SetValidity(base::Time::Now() - base::Days(14), base::Time::Now() - base::Days(7));

    if (VerifyProcTypeIsBuiltin()) {
        EXPECT_THAT(Verify(), IsOk());
        EXPECT_THAT(VerifyWithExpiryAndConstraints(), IsError(ERR_CERT_DATE_INVALID));
        EXPECT_THAT(VerifyWithExpiryAndFullConstraints(), IsError(ERR_CERT_DATE_INVALID));
    } else if (verify_proc_type() == CERT_VERIFY_PROC_ANDROID) {
        EXPECT_THAT(Verify(), IsOk());
    } else {
...
{
    chain_[3]->SetValidity(base::Time::Now() + base::Days(7), base::Time::Now() + base::Days(14));
...
{
    chain_[2]->SetValidity(base::Time::Now() - base::Days(14), base::Time::Now() - base::Days(7));
...
{
    chain_[2]->SetValidity(base::Time::Now() + base::Days(7), base::Time::Now() + base::Days(14));
    ...
    for (bool leaf_has_policy : { false, true }) {
        ...
        if (leaf_has_policy) {
            ...
            if (VerifyProcTypeIsBuiltin()) {
                EXPECT_THAT(Verify(), IsOk());
                EXPECT_THAT(VerifyWithExpiryAndConstraints(), IsError(ERR_CERT_INVALID));
            } else if (verify_proc_type() == CERT_VERIFY_PROC_IOS || verify_proc_type() == CERT_VERIFY_PROC_ANDROID) {
                EXPECT_THAT(Verify(), IsOk());
            } else {
                EXPECT_THAT(Verify(), IsError(ERR_CERT_INVALID));
            }
...
TEST_P(CertVerifyProcConstraintsTest, ExtendedKeyUsageNoServerAuthRoot)
{
    ...
    if (VerifyProcTypeIsBuiltin()) {
        ...
        EXPECT_THAT(VerifyWithExpiryAndConstraints(), IsError(ERR_CERT_INVALID));
        EXPECT_THAT(VerifyWithExpiryAndFullConstraints(), IsError(ERR_CERT_INVALID));
    } else if (verify_proc_type() == CERT_VERIFY_PROC_ANDROID || verify_proc_type() == CERT_VERIFY_PROC_IOS) {
        EXPECT_THAT(Verify(), IsOk());
    } else {
        EXPECT_THAT(Verify(), IsError(ERR_CERT_INVALID));
    }
...
TEST_P(CertVerifyProcConstraintsTest, ExtendedKeyUsageNoServerAuthIntermediate)
{
    chain_[2]->SetExtendedKeyUsages({ bssl::der::Input(bssl::kCodeSigning) });

    if (verify_proc_type() == CERT_VERIFY_PROC_ANDROID || VerifyProcTypeIsIOSAtMostOS15()) {
        EXPECT_THAT(Verify(), IsOk());
    } else {
        EXPECT_THAT(Verify(), IsError(ERR_CERT_INVALID));
    }
...
class CertVerifyProcConstraintsTrustedLeafTest : public CertVerifyProcInternalTest {
protected:
    void SetUp() override
    {
        CertVerifyProcInternalTest::SetUp();
    ...
    int VerifyWithTrust(bssl::CertificateTrust trust)
    {
        ...
        CertVerifyResult verify_result;
        int flags = 0;
        return CertVerifyProcInternalTest::Verify(chain_.front()->GetX509Certificate().get(), "www.example.com", flags, &verify_result);
    }
...
INSTANTIATE_TEST_SUITE_P(All, CertVerifyProcConstraintsTrustedLeafTest, testing::ValuesIn(kAllCertVerifiers), VerifyProcTypeToName);
...
TEST_P(CertVerifyProcConstraintsTrustedLeafTest, BaseCase)
{
    ...
    if (VerifyProcTypeIsBuiltin()) {
        ...
        EXPECT_THAT(VerifyWithTrust(bssl::CertificateTrust::ForTrustAnchorOrLeaf()), IsOk());
        EXPECT_THAT(VerifyWithTrust(bssl::CertificateTrust::ForTrustedLeaf().WithRequireLeafSelfSigned()), IsError(ERR_CERT_AUTHORITY_INVALID));
        EXPECT_THAT(VerifyWithTrust(bssl::CertificateTrust::ForTrustAnchorOrLeaf().WithRequireLeafSelfSigned()), IsError(ERR_CERT_AUTHORITY_INVALID));
    } else {
        EXPECT_THAT(Verify(), IsOk());
    }
}
    ...
    if (VerifyProcTypeIsBuiltin()) {
        {
            ...
            // all.
            EXPECT_THAT(VerifyWithTrust(bssl::CertificateTrust::ForUnspecified()), IsOk());
        }
        {
            ScopedTestRoot test_root1(chain_[1]->GetX509Certificate());
            // If the leaf is explicitly distrusted, verification should fail even if
            // the root is trusted.
            EXPECT_THAT(VerifyWithTrust(bssl::CertificateTrust::ForDistrusted()), IsError(ERR_CERT_AUTHORITY_INVALID));
        }
        {
            ScopedTestRoot test_root(chain_[1]->GetX509Certificate());
            EXPECT_THAT(VerifyAsTrustedLeaf(), IsOk());
        ...
        {
            ScopedTestRoot test_root(chain_[1]->GetX509Certificate());
            EXPECT_THAT(VerifyWithTrust(bssl::CertificateTrust::ForTrustAnchorOrLeaf().WithRequireLeafSelfSigned()), IsOk());
        }
    }
}
...
{
    chain_[0]->SetValidity(base::Time::Now() - base::Days(14), base::Time::Now() - base::Days(7));
...
class CertVerifyProcConstraintsTrustedSelfSignedTest : public CertVerifyProcInternalTest {
protected:
    void SetUp() override
    {
        CertVerifyProcInternalTest::SetUp();
    ...
    int VerifyWithTrust(bssl::CertificateTrust trust)
    {
        ...
        CertVerifyResult verify_result;
        int flags = 0;
        return CertVerifyProcInternalTest::Verify(cert_->GetX509Certificate().get(), "www.example.com", flags, &verify_result);
    }
    ...
    {
        return VerifyWithTrust(bssl::CertificateTrust::ForTrustedLeaf().WithRequireLeafSelfSigned());
    }

    std::unique_ptr<CertBuilder> cert_;
};

INSTANTIATE_TEST_SUITE_P(All, CertVerifyProcConstraintsTrustedSelfSignedTest, testing::ValuesIn(kAllCertVerifiers), VerifyProcTypeToName);
...
TEST_P(CertVerifyProcConstraintsTrustedSelfSignedTest, BaseCase)
{
    ...
    if (VerifyProcTypeIsBuiltin()) {
        ...
        EXPECT_THAT(VerifyWithTrust(bssl::CertificateTrust::ForTrustedLeaf()), IsOk());
        EXPECT_THAT(VerifyWithTrust(bssl::CertificateTrust::ForTrustAnchorOrLeaf()), IsOk());
        ...
        // has CA=false.
        EXPECT_THAT(VerifyWithTrust(bssl::CertificateTrust::ForTrustAnchor().WithEnforceAnchorConstraints()), IsError(ERR_CERT_INVALID));

        // Should be allowed since it will be evaluated as a trusted leaf, so
        // anchor constraints being enabled doesn't matter.
        EXPECT_THAT(VerifyWithTrust(bssl::CertificateTrust::ForTrustAnchorOrLeaf().WithEnforceAnchorConstraints()), IsOk());
    } else {
        EXPECT_THAT(Verify(), IsOk());
    }
}
...
TEST_P(CertVerifyProcConstraintsTrustedSelfSignedTest, BasicConstraintsIsCa)
{
    for (bool has_key_usage_cert_sign : { false, true }) {
        ...
        if (has_key_usage_cert_sign) {
            cert_->SetKeyUsages({ bssl::KEY_USAGE_BIT_KEY_CERT_SIGN, bssl::KEY_USAGE_BIT_DIGITAL_SIGNATURE });
        } else {
...
TEST_P(CertVerifyProcConstraintsTrustedSelfSignedTest, BasicConstraintsNotCaPathlen)
{
...
TEST_P(CertVerifyProcConstraintsTrustedSelfSignedTest, BasicConstraintsIsCaPathlen)
{
...
TEST_P(CertVerifyProcConstraintsTrustedSelfSignedTest, BasicConstraintsMissing)
{
...
TEST_P(CertVerifyProcConstraintsTrustedSelfSignedTest, NameConstraintsNotMatching)
{
...
{
    cert_->SetValidity(base::Time::Now() - base::Days(14), base::Time::Now() - base::Days(7));

    EXPECT_THAT(Verify(), IsError(ERR_CERT_DATE_INVALID));
    if (VerifyProcTypeIsBuiltin()) {
        EXPECT_THAT(VerifyAsTrustedSelfSignedLeaf(), IsError(ERR_CERT_DATE_INVALID));
    }
}
...
TEST_P(CertVerifyProcConstraintsTrustedSelfSignedTest, KeyUsageNoDigitalSignature)
{
    // This test is mostly uninteresting since keyUsage on the end-entity is only
    // checked at the TLS layer, not during cert verification.
...
TEST_P(CertVerifyProcConstraintsTrustedSelfSignedTest, KeyUsageCertSignLeaf)
{
    ...
    if (VerifyProcTypeIsBuiltin()) {
        EXPECT_THAT(VerifyWithTrust(bssl::CertificateTrust::ForTrustAnchor().WithEnforceAnchorConstraints()), IsError(ERR_CERT_INVALID));
        EXPECT_THAT(VerifyAsTrustedSelfSignedLeaf(), IsOk());
        EXPECT_THAT(VerifyWithTrust(bssl::CertificateTrust::ForTrustAnchorOrLeaf().WithEnforceAnchorConstraints().WithRequireLeafSelfSigned()), IsOk());
    }
}

TEST_P(CertVerifyProcConstraintsTrustedSelfSignedTest, ExtendedKeyUsageNoServerAuth)
{
...
TEST_P(CertVerifyProcConstraintsTrustedSelfSignedTest, UnknownSignatureAlgorithm)
{
    ...
    if (VerifyProcTypeIsBuiltin()) {
        ...
        EXPECT_THAT(VerifyAsTrustedSelfSignedLeaf(), IsError(ERR_CERT_AUTHORITY_INVALID));
        ...
        EXPECT_THAT(VerifyWithTrust(bssl::CertificateTrust::ForTrustAnchorOrLeaf().WithRequireLeafSelfSigned()), IsError(ERR_CERT_INVALID));
    } else {
        EXPECT_THAT(Verify(), IsOk());
    }
}
...
TEST(CertVerifyProcTest, RejectsPublicSHA1)
{
    ...
    int flags = 0;
    CertVerifyResult verify_result;
    int error = verify_proc->Verify(cert.get(), "127.0.0.1", /*ocsp_response=*/std::string(),
        /*sct_list=*/std::string(), flags, &verify_result, NetLogWithSource());
    ...
    verify_result.Reset();
    error = verify_proc->Verify(cert.get(), "127.0.0.1", /*ocsp_response=*/std::string(),
        /*sct_list=*/std::string(), flags, &verify_result, NetLogWithSource());
    EXPECT_THAT(error, IsError(ERR_CERT_WEAK_SIGNATURE_ALGORITHM));
    EXPECT_TRUE(verify_result.cert_status & CERT_STATUS_WEAK_SIGNATURE_ALGORITHM);
}
...
{
    *os << "root: " << StringOrDefault(data.root_cert_filename, "none") << "; intermediate: " << StringOrDefault(data.intermediate_cert_filename, "none")
        << "; end-entity: " << data.ee_cert_filename;
}

class CertVerifyProcWeakDigestTest : public testing::TestWithParam<WeakDigestTestData> {
public:
...
TEST_P(CertVerifyProcWeakDigestTest, VerifyDetectsAlgorithm)
{
    ...
    scoped_refptr<X509Certificate> ee_chain = X509Certificate::CreateFromBuffer(bssl::UpRef(ee_cert->cert_buffer()), std::move(intermediates));
    ...
    EXPECT_EQ(!!(data.expected_algorithms & EXPECT_STATUS_INVALID), !!(verify_result.cert_status & CERT_STATUS_INVALID));
    EXPECT_EQ(!!(data.expected_algorithms & EXPECT_STATUS_INVALID), error == ERR_CERT_INVALID);
}
...
INSTANTIATE_TEST_SUITE_P(VerifyRoot, CertVerifyProcWeakDigestTest, testing::ValuesIn(kVerifyRootCATestData));

// The signature algorithm of intermediates should be properly detected.
const WeakDigestTestData kVerifyIntermediateCATestData[] = {
    { "weak_digest_sha1_root.pem", "weak_digest_md5_intermediate.pem", "weak_digest_sha1_ee.pem", EXPECT_STATUS_INVALID | EXPECT_SHA1 },
    { "weak_digest_sha1_root.pem", "weak_digest_md4_intermediate.pem", "weak_digest_sha1_ee.pem", EXPECT_STATUS_INVALID | EXPECT_SHA1 },
    { "weak_digest_sha1_root.pem", "weak_digest_md2_intermediate.pem", "weak_digest_sha1_ee.pem", EXPECT_STATUS_INVALID | EXPECT_SHA1 },
};

INSTANTIATE_TEST_SUITE_P(VerifyIntermediate, CertVerifyProcWeakDigestTest, testing::ValuesIn(kVerifyIntermediateCATestData));

// The signature algorithm of end-entity should be properly detected.
const WeakDigestTestData kVerifyEndEntityTestData[] = {
    { "weak_digest_sha1_root.pem", "weak_digest_sha1_intermediate.pem", "weak_digest_md5_ee.pem", EXPECT_STATUS_INVALID },
    { "weak_digest_sha1_root.pem", "weak_digest_sha1_intermediate.pem", "weak_digest_md4_ee.pem", EXPECT_STATUS_INVALID },
    { "weak_digest_sha1_root.pem", "weak_digest_sha1_intermediate.pem", "weak_digest_md2_ee.pem", EXPECT_STATUS_INVALID },
};

INSTANTIATE_TEST_SUITE_P(VerifyEndEntity, CertVerifyProcWeakDigestTest, testing::ValuesIn(kVerifyEndEntityTestData));
...
const WeakDigestTestData kVerifyIncompleteIntermediateTestData[] = {
    { nullptr, "weak_digest_md5_intermediate.pem", "weak_digest_sha1_ee.pem", EXPECT_SHA1 },
    { nullptr, "weak_digest_md4_intermediate.pem", "weak_digest_sha1_ee.pem", EXPECT_SHA1 },
    { nullptr, "weak_digest_md2_intermediate.pem", "weak_digest_sha1_ee.pem", EXPECT_SHA1 },
};

INSTANTIATE_TEST_SUITE_P(MAYBE_VerifyIncompleteIntermediate, CertVerifyProcWeakDigestTest, testing::ValuesIn(kVerifyIncompleteIntermediateTestData));
...
const WeakDigestTestData kVerifyIncompleteEETestData[] = {
    { nullptr, "weak_digest_sha1_intermediate.pem", "weak_digest_md5_ee.pem", EXPECT_STATUS_INVALID },
    { nullptr, "weak_digest_sha1_intermediate.pem", "weak_digest_md4_ee.pem", EXPECT_STATUS_INVALID },
    { nullptr, "weak_digest_sha1_intermediate.pem", "weak_digest_md2_ee.pem", EXPECT_STATUS_INVALID },
};

INSTANTIATE_TEST_SUITE_P(VerifyIncompleteEndEntity, CertVerifyProcWeakDigestTest, testing::ValuesIn(kVerifyIncompleteEETestData));

// Md2, Md4, and Md5 are all considered invalid.
const WeakDigestTestData kVerifyMixedTestData[] = {
    { "weak_digest_sha1_root.pem", "weak_digest_md5_intermediate.pem", "weak_digest_md2_ee.pem", EXPECT_STATUS_INVALID },
    { "weak_digest_sha1_root.pem", "weak_digest_md2_intermediate.pem", "weak_digest_md5_ee.pem", EXPECT_STATUS_INVALID },
    { "weak_digest_sha1_root.pem", "weak_digest_md4_intermediate.pem", "weak_digest_md2_ee.pem", EXPECT_STATUS_INVALID },
};

INSTANTIATE_TEST_SUITE_P(VerifyMixed, CertVerifyProcWeakDigestTest, testing::ValuesIn(kVerifyMixedTestData));
...
INSTANTIATE_TEST_SUITE_P(VerifyTrustedEE, CertVerifyProcWeakDigestTest, testing::ValuesIn(kVerifyTrustedEETestData));
...
protected:
    ...
    {
        scoped_refptr<X509Certificate> cert(ImportCertFromFile(GetTestCertsDirectory(), "subjectAltName_sanity_check.pem"));
        ASSERT_TRUE(cert);
        CertVerifyResult result;
        result.is_issued_by_known_root = false;
        auto verify_proc = base::MakeRefCounted<MockCertVerifyProc>(result);
        ...
        CertVerifyResult verify_result;
        int error = verify_proc->Verify(cert.get(), hostname, /*ocsp_response=*/std::string(),
            /*sct_list=*/std::string(), 0, &verify_result, NetLogWithSource());
        if (valid) {
            EXPECT_THAT(error, IsOk());
...
TEST(CertVerifyProcTest, HasTrustAnchorVerifyUMA)
{
    ...
    SHA256HashValue root_hash = { { 0x98, 0x47, 0xe5, 0x65, 0x3e, 0x5e, 0x9e, 0x84, 0x75, 0x16, 0xe5, 0xcb, 0x81, 0x86, 0x06, 0xaa, 0x75, 0x44, 0xa1, 0x9b,
        0xe6, 0x7f, 0xd7, 0x36, 0x6d, 0x50, 0x69, 0x88, 0xe8, 0xd8, 0x43, 0x47 } };
    ...
    int error = verify_proc->Verify(cert.get(), "127.0.0.1", /*ocsp_response=*/std::string(),
        /*sct_list=*/std::string(), flags, &verify_result, NetLogWithSource());
    EXPECT_EQ(OK, error);
    histograms.ExpectUniqueSample(kTrustAnchorVerifyHistogram, kGTSRootR4HistogramID, 1);
}
...
TEST(CertVerifyProcTest, LogsOnlyMostSpecificTrustAnchorUMA)
{
    ...
    SHA256HashValue leaf_hash = { { 0 } };
    SHA256HashValue intermediate_hash = { { 1 } };
    SHA256HashValue gts_root_r3_hash = { { 0x41, 0x79, 0xed, 0xd9, 0x81, 0xef, 0x74, 0x74, 0x77, 0xb4, 0x96, 0x26, 0x40, 0x8a, 0xf4, 0x3d, 0xaa, 0x2c, 0xa7,
        0xab, 0x7f, 0x9e, 0x08, 0x2c, 0x10, 0x60, 0xf8, 0x40, 0x96, 0x77, 0x43, 0x48 } };
    SHA256HashValue gts_root_r4_hash = { { 0x98, 0x47, 0xe5, 0x65, 0x3e, 0x5e, 0x9e, 0x84, 0x75, 0x16, 0xe5, 0xcb, 0x81, 0x86, 0x06, 0xaa, 0x75, 0x44, 0xa1,
        0x9b, 0xe6, 0x7f, 0xd7, 0x36, 0x6d, 0x50, 0x69, 0x88, 0xe8, 0xd8, 0x43, 0x47 } };
    result.public_key_hashes.push_back(HashValue(leaf_hash));
    result.public_key_hashes.push_back(HashValue(intermediate_hash));
    ...
    histograms.ExpectUniqueSample(kTrustAnchorVerifyHistogram, kGTSRootR3HistogramID, 1);
}
...
TEST(CertVerifyProcTest, HasTrustAnchorVerifyOutOfDateUMA)
{
    ...
    histograms.ExpectUniqueSample(kTrustAnchorVerifyHistogram, kUnknownRootHistogramID, 1);
...
TEST(CertVerifyProcTest, DoesNotRecalculateStapledOCSPResult)
{
    ...
    EXPECT_EQ(bssl::OCSPVerifyResult::PROVIDED, verify_result.ocsp_result.response_status);
    EXPECT_EQ(bssl::OCSPRevocationStatus::GOOD, verify_result.ocsp_result.revocation_status);
}
...
TEST(CertVerifyProcTest, CalculateStapledOCSPResultIfNotAlreadyDone)
{
    ...
    EXPECT_EQ(bssl::OCSPVerifyResult::NOT_CHECKED, result.ocsp_result.response_status);
    EXPECT_EQ(bssl::OCSPRevocationStatus::UNKNOWN, result.ocsp_result.revocation_status);

    auto verify_proc = base::MakeRefCounted<MockCertVerifyProc>(result);

    int flags = 0;
    CertVerifyResult verify_result;
    int error = verify_proc->Verify(cert.get(), "127.0.0.1", /*ocsp_response=*/"invalid OCSP data",
        /*sct_list=*/std::string(), flags, &verify_result, NetLogWithSource());
    EXPECT_EQ(OK, error);

    EXPECT_EQ(bssl::OCSPVerifyResult::PARSE_RESPONSE_ERROR, verify_result.ocsp_result.response_status);
    EXPECT_EQ(bssl::OCSPRevocationStatus::UNKNOWN, verify_result.ocsp_result.revocation_status);
}
