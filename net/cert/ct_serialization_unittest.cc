...
{
    std::string_view digitally_signed(test_digitally_signed_);
    std::string_view partial_digitally_signed(digitally_signed.substr(0, test_digitally_signed_.size() - 5));
    ct::DigitallySigned parsed;
...
TEST_F(CtSerializationTest, EncodesSignedEntryForPrecert)
{
    ...
    EXPECT_THAT(encoded.substr(2, 32), ElementsAreArray(entry.issuer_key_hash.data));
...
{
    base::Time timestamp = base::Time::UnixEpoch() + base::Milliseconds(1348589665525);
    std::string dummy_entry("abc");
    std::string empty_extensions;
    // For now, no known failure cases.
    std::string encoded;
    ASSERT_TRUE(ct::EncodeV1SCTSignedData(timestamp, dummy_entry, empty_extensions, &encoded));
    ...
    std::string expected_buffer("\x0\x0\x0\x0\x1\x39\xFE\x35\x3C\xF5\x61\x62\x63\x0\x0", 15);
...
TEST_F(CtSerializationTest, DecodesSignedCertificateTimestamp)
{
    ...
    base::Time expected_time = base::Time::UnixEpoch() + base::Milliseconds(1365181456089);
...
TEST_F(CtSerializationTest, FailsDecodingInvalidSignedCertificateTimestamp)
{
    ...
    ASSERT_FALSE(ct::DecodeSignedCertificateTimestamp(&invalid_version_sct, &sct));
...
TEST_F(CtSerializationTest, EncodesMerkleTreeLeafForX509Cert)
{
    ...
    EXPECT_EQ(std::string("\x00\x00\x01\x45\x3c\x5f\xb8\x35", 8), encoded.substr(2, 8)) << "Timestamp encoded incorrectly";
    ...
    EXPECT_EQ(tree_leaf.signed_entry.leaf_certificate, encoded.substr(15, 718)) << "Certificate encoded incorrectly";
...
TEST_F(CtSerializationTest, EncodesMerkleTreeLeafForPrecert)
{
    ...
    EXPECT_THAT(encoded.substr(12, 32), ElementsAreArray(tree_leaf.signed_entry.issuer_key_hash.data)) << "Issuer key hash encoded incorrectly";
    EXPECT_EQ(std::string("\x00\x02\x37", 3), encoded.substr(44, 3)) << "TBS certificate length encoded incorrectly";
    EXPECT_EQ(tree_leaf.signed_entry.tbs_certificate, encoded.substr(47, 567)) << "TBS certificate encoded incorrectly";
...
TEST_F(CtSerializationTest, EncodesValidSignedTreeHead)
{
    ...
    std::string expected_buffer("\x0\x1\x0\x0\x1\x45\x3c\x5f\xb8\x35\x0\x0\x0\x0\x0\x0\x0\x15", 18);
