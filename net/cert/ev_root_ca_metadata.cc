...
static base::LazyInstance<EVRootCAMetadata>::Leaky g_ev_root_ca_metadata = LAZY_INSTANCE_INITIALIZER;
...
std::string OIDStringToDER(std::string_view policy)
{
    ...
    size_t len;
    bssl::ScopedCBB cbb;
    if (!CBB_init(cbb.get(), 32) || !CBB_add_asn1_oid_from_text(cbb.get(), policy.data(), policy.size()) || !CBB_finish(cbb.get(), &der, &len)) {
        return std::string();
    }
...
bool EVRootCAMetadata::HasEVPolicyOID(const SHA256HashValue& fingerprint, bssl::der::Input policy_oid) const
{
...
bool EVRootCAMetadata::AddEVCA(const SHA256HashValue& fingerprint, const char* policy)
{
