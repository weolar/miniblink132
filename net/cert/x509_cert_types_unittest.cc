...
TEST(X509TypesTest, ParseDNVerisign)
{
    ...
    EXPECT_EQ("Class 1 Public Primary Certification Authority", verisign.organization_unit_names[0]);
}
...
TEST(X509TypesTest, ParseDNStartcom)
{
    ...
    EXPECT_EQ("Secure Digital Certificate Signing", startcom.organization_unit_names[0]);
}
...
TEST(X509TypesTest, ParseDNUserTrust)
{
    ...
    EXPECT_EQ("UTN-USERFirst-Client Authentication and Email", usertrust.common_name);
...
TEST(X509TypesTest, ParseDNTurkTrust)
{
    ...
    EXPECT_EQ("TÜRKTRUST Elektronik Sertifika Hizmet Sağlayıcısı", turktrust.common_name);
    ...
    EXPECT_EQ("TÜRKTRUST Bilgi İletişim ve Bilişim Güvenliği Hizmetleri A.Ş. (c) Kasım 2005", turktrust.organization_names[0]);
}
...
TEST(X509TypesTest, ParseDNATrust)
{
    ...
    EXPECT_EQ("A-Trust Ges. für Sicherheitssysteme im elektr. Datenverkehr GmbH", atrust.organization_names[0]);
...
TEST(X509TypesTest, ParseDNEntrust)
{
    ...
    EXPECT_EQ("www.entrust.net/CPS_2048 incorp. by ref. (limits liab.)", entrust.organization_unit_names[0]);
