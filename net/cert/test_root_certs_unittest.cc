...
public:
    scoped_refptr<CertVerifyProc> CreateCertVerifyProc()
    {
        ...
        if (use_chrome_cert_validator()) {
            return CertVerifyProc::CreateBuiltinWithChromeRootStore(
                /*cert_net_fetcher=*/nullptr, CRLSet::BuiltinCRLSet().get(), std::make_unique<DoNothingCTVerifier>(),
...
#else
        return CertVerifyProc::CreateSystemVerifyProc(/*cert_net_fetcher=*/nullptr, CRLSet::BuiltinCRLSet().get());
#endif
    }
    ...
    bool use_chrome_cert_validator()
    {
        return GetParam();
    }
};
...
TEST_P(TestRootCertsTest, OverrideTrust)
{
    ...
    int bad_status = verify_proc->Verify(test_cert.get(), "127.0.0.1",
        ...
        /*sct_list=*/std::string(), flags, &bad_verify_result, NetLogWithSource());
    ...
    int good_status = verify_proc->Verify(test_cert.get(), "127.0.0.1", /*ocsp_response=*/std::string(),
        /*sct_list=*/std::string(), flags, &good_verify_result, NetLogWithSource());
    ...
    int restored_status = verify_proc->Verify(test_cert.get(), "127.0.0.1", /*ocsp_response=*/std::string(),
        /*sct_list=*/std::string(), flags, &restored_verify_result, NetLogWithSource());
...
TEST_P(TestRootCertsTest, OverrideKnownRoot)
{
    ...
    scoped_refptr<CertVerifyProc> verify_proc(CreateCertVerifyProc());
    int flags = 0;
    int good_status = verify_proc->Verify(leaf->GetX509Certificate().get(), "www.example.com",
        /*ocsp_response=*/std::string(),
        /*sct_list=*/std::string(), flags, &good_verify_result, NetLogWithSource());
    EXPECT_THAT(good_status, IsOk());
    EXPECT_EQ(0u, good_verify_result.cert_status);
    ...
    CertVerifyResult restored_verify_result;
    int restored_status = verify_proc->Verify(leaf->GetX509Certificate().get(), "www.example.com",
        /*ocsp_response=*/std::string(),
        /*sct_list=*/std::string(), flags, &restored_verify_result, NetLogWithSource());
