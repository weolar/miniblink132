...
BOOL WINAPI InterceptedOpenStoreW(LPCSTR store_provider, DWORD encoding, HCRYPTPROV crypt_provider, DWORD flags, const void* extra, HCERTSTORE memory_store,
...
private:
        ...
        , original_handle(nullptr)
    {
        ...
        BOOL ok
            = CryptGetOIDFunctionAddress(registered_functions, 0, CERT_STORE_PROV_SYSTEM_W, 0, reinterpret_cast<void**>(&original_function), &original_handle);
        DCHECK(ok);
        ...
        // installed.
        const CRYPT_OID_FUNC_ENTRY kFunctionToIntercept = { CERT_STORE_PROV_SYSTEM_W, reinterpret_cast<void*>(&InterceptedOpenStoreW) };
        ...
        // registered function.
        ok = CryptInstallOIDFunctionAddress(nullptr, 0, CRYPT_OID_OPEN_STORE_PROV_FUNC, 1, &kFunctionToIntercept, CRYPT_INSTALL_OID_FUNC_BEFORE_FLAG);
        DCHECK(ok);
    }
...
base::LazyInstance<CryptoAPIInjector>::Leaky g_capi_injector = LAZY_INSTANCE_INITIALIZER;

BOOL WINAPI InterceptedOpenStoreW(LPCSTR store_provider, DWORD encoding, HCRYPTPROV crypt_provider, DWORD flags, const void* store_name,
    HCERTSTORE memory_store, PCERT_STORE_PROV_INFO store_info)
{
    ...
    BOOL ok = g_capi_injector.Get().original_function(store_provider, encoding, crypt_provider, flags, store_name, memory_store, store_info);
    ...
    return CertAddStoreToCollection(memory_store, TestRootCerts::GetInstance()->temporary_roots(), 0, 0);
}
...
bool TestRootCerts::AddImpl(X509Certificate* certificate)
{
    ...
    BOOL ok
        = CertAddEncodedCertificateToStore(temporary_roots_, X509_ASN_ENCODING, reinterpret_cast<const BYTE*>(CRYPTO_BUFFER_data(certificate->cert_buffer())),
            base::checked_cast<DWORD>(CRYPTO_BUFFER_len(certificate->cert_buffer())), CERT_STORE_ADD_NEW, nullptr);
    if (!ok) {
...
{
    for (PCCERT_CONTEXT prev_cert = CertEnumCertificatesInStore(temporary_roots_, nullptr); prev_cert;
...
crypto::ScopedHCERTCHAINENGINE TestRootCerts::GetChainEngine() const
{
    ...
    static const DWORD kSizeofCertChainEngineConfig = SIZEOF_STRUCT_WITH_SPECIFIED_LAST_MEMBER(CERT_CHAIN_ENGINE_CONFIG, hExclusiveTrustedPeople);
    ...
    BOOL ok = CertCreateCertificateChainEngine(&engine_config, crypto::ScopedHCERTCHAINENGINE::Receiver(chain_engine).get());
    DCHECK(ok);
    return chain_engine;
}
...
{
    temporary_roots_ = CertOpenStore(CERT_STORE_PROV_MEMORY, 0, NULL, CERT_STORE_DEFER_CLOSE_UNTIL_LAST_FREE_FLAG, nullptr);
