...
class NET_EXPORT CertNetFetcher : public base::RefCountedThreadSafe<CertNetFetcher> {
public:
    class Request {
    public:
    ...
    [[nodiscard]] virtual std::unique_ptr<Request> FetchCaIssuers(const GURL& url, int timeout_milliseconds, int max_response_bytes) = 0;

    [[nodiscard]] virtual std::unique_ptr<Request> FetchCrl(const GURL& url, int timeout_milliseconds, int max_response_bytes) = 0;

    [[nodiscard]] virtual std::unique_ptr<Request> FetchOcsp(const GURL& url, int timeout_milliseconds, int max_response_bytes) = 0;
