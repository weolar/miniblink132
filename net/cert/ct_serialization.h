...
NET_EXPORT_PRIVATE bool EncodeDigitallySigned(const DigitallySigned& input, std::string* output);
...
NET_EXPORT_PRIVATE bool DecodeDigitallySigned(std::string_view* input, DigitallySigned* output);
...
NET_EXPORT_PRIVATE bool EncodeSignedEntry(const SignedEntryData& input, std::string* output);
...
NET_EXPORT_PRIVATE bool EncodeV1SCTSignedData(
    const base::Time& timestamp, const std::string& serialized_log_entry, const std::string& extensions, std::string* output);
...
// otherwise.
NET_EXPORT_PRIVATE bool EncodeTreeHeadSignature(const SignedTreeHead& signed_tree_head, std::string* output);
...
// separately).
NET_EXPORT_PRIVATE bool DecodeSCTList(std::string_view input, std::vector<std::string_view>* output);

// Decodes a single SCT from |input| to |output|.
// Returns true if all fields in the SCT could be read and decoded, false
// otherwise.
NET_EXPORT_PRIVATE bool DecodeSignedCertificateTimestamp(std::string_view* input, scoped_refptr<ct::SignedCertificateTimestamp>* output);

// Serializes a Signed Certificate Timestamp (SCT) into |output|.
// Returns true if the SCT could be encoded successfully, false
// otherwise.
NET_EXPORT bool EncodeSignedCertificateTimestamp(const scoped_refptr<ct::SignedCertificateTimestamp>& input, std::string* output);

// Writes an SCTList into |output|, containing |scts|.
NET_EXPORT_PRIVATE bool EncodeSCTListForTesting(const std::vector<std::string>& scts, std::string* output);
} // namespace net::ct
