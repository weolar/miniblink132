...
DEFINE_CERT_ERROR_ID(kFailedConvertingAttributeValue, "Failed converting AttributeValue to string");
...
[[nodiscard]] bool NormalizeDirectoryString(CharsetEnforcement charset_enforcement, std::string* output)
{
...
// NOTE: |output| will be modified regardless of the return.
[[nodiscard]] bool NormalizeValue(X509NameAttribute attribute, std::string* output, CertErrors* errors)
{
    DCHECK(errors);

    if (!attribute.ValueAsStringUnsafe(output)) {
        errors->AddError(kFailedConvertingAttributeValue, CreateCertErrorParams1SizeT("tag", attribute.value_tag));
        return false;
    }
    ...
    if (!success) {
        errors->AddError(kFailedNormalizingString, CreateCertErrorParams1SizeT("tag", attribute.value_tag));
    }
...
bool VerifyValueMatch(X509NameAttribute a, X509NameAttribute b)
{
    if (IsNormalizableDirectoryString(a.value_tag) && IsNormalizableDirectoryString(b.value_tag)) {
        ...
        if (!NormalizeValue(a, &a_normalized, &unused_errors) || !NormalizeValue(b, &b_normalized, &unused_errors))
            return false;
...
bool VerifyNameMatchInternal(const der::Input& a, const der::Input& b, NameMatchType match_type)
{
...
bool NormalizeName(const der::Input& name_rdn_sequence, std::string* normalized_rdn_sequence, CertErrors* errors)
{
    DCHECK(errors);
    ...
    while (rdn_sequence_parser.HasMore()) {
        ...
        for (const auto& type_and_value : type_and_values) {
            ...
            if (!CBB_add_asn1(&rdn_cbb, &attribute_type_and_value_cbb, CBS_ASN1_SEQUENCE)) {
                return false;
            }

            // AttributeType ::= OBJECT IDENTIFIER
            if (!CBB_add_asn1(&attribute_type_and_value_cbb, &type_cbb, CBS_ASN1_OBJECT)
                || !CBB_add_bytes(&type_cbb, type_and_value.type.UnsafeData(), type_and_value.type.Length())) {
                return false;
            }
            ...
            if (IsNormalizableDirectoryString(type_and_value.value_tag)) {
                ...
                if (!NormalizeValue(type_and_value, &normalized_value, errors))
                    return false;
                if (!CBB_add_asn1(&attribute_type_and_value_cbb, &value_cbb, CBS_ASN1_UTF8STRING)
                    || !CBB_add_bytes(&value_cbb, reinterpret_cast<const uint8_t*>(normalized_value.data()), normalized_value.size()))
                    return false;
            } else {
                if (!CBB_add_asn1(&attribute_type_and_value_cbb, &value_cbb, type_and_value.value_tag)
                    || !CBB_add_bytes(&value_cbb, type_and_value.value.UnsafeData(), type_and_value.value.Length()))
                    return false;
            }
    ...
    normalized_rdn_sequence->assign(CBB_data(cbb.get()), CBB_data(cbb.get()) + CBB_len(cbb.get()));
    return true;
}

bool VerifyNameMatch(const der::Input& a_rdn_sequence, const der::Input& b_rdn_sequence)
{
...
{
    return VerifyNameMatchInternal(name_rdn_sequence, parent_rdn_sequence, SUBTREE_MATCH);
}

bool NameContainsEmailAddress(const der::Input& name_rdn_sequence, bool* contained_email_address)
{
    der::Parser rdn_sequence_parser(name_rdn_sequence);
