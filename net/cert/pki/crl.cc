...
[[nodiscard]] bool NormalizeNameTLV(const der::Input& name_tlv, std::string* out_normalized_name)
{
    der::Parser parser(name_tlv);
    der::Input name_rdn;
    net::CertErrors unused_errors;
    return parser.ReadTag(der::kSequence, &name_rdn) && NormalizeName(name_rdn, out_normalized_name, &unused_errors) && !parser.HasMore();
}

bool ContainsExactMatchingName(std::vector<std::string_view> a, std::vector<std::string_view> b)
{
...
bool ParseCrlCertificateList(
    const der::Input& crl_tlv, der::Input* out_tbs_cert_list_tlv, der::Input* out_signature_algorithm_tlv, der::BitString* out_signature_value)
{
    ...
    absl::optional<der::BitString> signature_value = certificate_list_parser.ReadBitString();
    if (!signature_value)
        return false;
...
bool ParseCrlTbsCertList(const der::Input& tbs_tlv, ParsedCrlTbsCertList* out)
{
    ...
    if (tbs_parser.PeekTagAndValue(&maybe_next_update_tag, &unused_next_update_input)
        && (maybe_next_update_tag == der::kUtcTime || maybe_next_update_tag == der::kGeneralizedTime)) {
    ...
    if (tbs_parser.PeekTagAndValue(&maybe_revoked_certifigates_tag, &unused_revoked_certificates) && maybe_revoked_certifigates_tag == der::kSequence) {
    ...
    if (!tbs_parser.ReadOptionalTag(der::ContextSpecificConstructed(0), &out->crl_extensions_tlv)) {
        return false;
    }
...
bool ParseIssuingDistributionPoint(
    const der::Input& extension_value, std::unique_ptr<GeneralNames>* out_distribution_point_names, ContainedCertsType* out_only_contains_cert_type)
{
    ...
    if (!idp_parser.ReadOptionalTag(der::kTagContextSpecific | der::kTagConstructed | 0, &distribution_point)) {
        return false;
    }
    ...
    if (distribution_point.has_value()) {
        ...
        if (!dp_name_parser.ReadOptionalTag(der::kTagContextSpecific | der::kTagConstructed | 0, &der_full_name)) {
            return false;
        }
        if (!der_full_name) {
        ...
        CertErrors errors;
        *out_distribution_point_names = GeneralNames::CreateFromValue(*der_full_name, &errors);
    ...
    *out_only_contains_cert_type = ContainedCertsType::ANY_CERTS;
    ...
    if (!idp_parser.ReadOptionalTag(der::kTagContextSpecific | 1, &only_contains_user_certs)) {
        return false;
    }
    ...
    if (!idp_parser.ReadOptionalTag(der::kTagContextSpecific | 2, &only_contains_ca_certs)) {
        return false;
    }
...
CRLRevocationStatus GetCRLStatusForCert(const der::Input& cert_serial, CrlVersion crl_version, const absl::optional<der::Input>& revoked_certificates_tlv)
{
...
CRLRevocationStatus CheckCRL(std::string_view raw_crl, const ParsedCertificateList& valid_chain, size_t target_cert_index,
    const ParsedDistributionPoint& cert_dp, const base::Time& verify_time, const base::TimeDelta& max_age)
{
    ...
    if (!ParseCrlCertificateList(der::Input(raw_crl), &tbs_cert_list_tlv, &signature_algorithm_tlv, &signature_value)) {
        return CRLRevocationStatus::UNKNOWN;
    }
    ...
    // policy.
    absl::optional<SignatureAlgorithm> signature_algorithm = ParseSignatureAlgorithm(signature_algorithm_tlv,
        /*errors=*/nullptr);
    ...
    absl::optional<SignatureAlgorithm> tbs_alg = ParseSignatureAlgorithm(tbs_cert_list.signature_algorithm_tlv,
        /*errors=*/nullptr);
    ...
    if (!CheckRevocationDateValid(tbs_cert_list.this_update, base::OptionalToPtr(tbs_cert_list.next_update), verify_time, max_age)) {
        return CRLRevocationStatus::UNKNOWN;
    }
    ...
    if (tbs_cert_list.crl_extensions_tlv.has_value()) {
        ...
        if (ConsumeExtension(der::Input(kIssuingDistributionPointOid), &extensions, &idp_extension)) {
            std::unique_ptr<GeneralNames> distribution_point_names;
            ContainedCertsType only_contains_cert_type;
            if (!ParseIssuingDistributionPoint(idp_extension.value, &distribution_point_names, &only_contains_cert_type)) {
                return CRLRevocationStatus::UNKNOWN;
            }
            ...
            case ContainedCertsType::USER_CERTS:
                ...
                if ((target_cert->has_basic_constraints() && target_cert->basic_constraints().is_ca) || target_cert->tbs().version == CertificateVersion::V1
            ...
            case ContainedCertsType::CA_CERTS:
                ...
                // certificate.
                if (!target_cert->has_basic_constraints() || !target_cert->basic_constraints().is_ca) {
                    return CRLRevocationStatus::UNKNOWN;
                }
                break;
    ...
    for (size_t i = target_cert_index + 1; i < valid_chain.size(); ++i) {
        ...
        if (issuer_cert->has_key_usage() && !issuer_cert->key_usage().AssertsBit(KEY_USAGE_BIT_CRL_SIGN)) {
            continue;
        }

        // 6.3.3 (g) Validate the signature on the complete CRL using the public
        //           key validated in step (f).
        if (!VerifySignedData(*signature_algorithm, tbs_cert_list_tlv, signature_value, issuer_cert->tbs().spki_tlv)) {
            continue;
        }
        ...
        return GetCRLStatusForCert(target_cert->tbs().serial_number, tbs_cert_list.version, tbs_cert_list.revoked_certificates_tlv);
