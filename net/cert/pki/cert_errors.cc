...
void AppendLinesWithIndentation(const std::string& text, const std::string& indentation, std::string* out)
{
...
CertError::CertError(Severity severity, CertErrorId id, std::unique_ptr<CertErrorParams> params)
    : severity(severity)
    , id(id)
...
void CertErrors::Add(CertError::Severity severity, CertErrorId id, std::unique_ptr<CertErrorParams> params)
{
    nodes_.emplace_back(severity, id, std::move(params));
}

void CertErrors::AddError(CertErrorId id, std::unique_ptr<CertErrorParams> params)
{
...
void CertErrors::AddWarning(CertErrorId id, std::unique_ptr<CertErrorParams> params)
{
...
bool CertErrors::ContainsAnyErrorWithSeverity(CertError::Severity severity) const
{
    for (const CertError& node : nodes_) {
...
bool CertPathErrors::ContainsAnyErrorWithSeverity(CertError::Severity severity) const
{
    for (const CertErrors& errors : cert_errors_) {
...
std::string CertPathErrors::ToDebugString(const ParsedCertificateList& certs) const
{
    ...
    for (size_t i = 0; i < cert_errors_.size(); ++i) {
        ...
        if (i < certs.size() && certs[i]) {
            RDNSequence subject;
            if (ParseName(certs[i]->tbs().subject_tlv, &subject) && ConvertToRFC2253(subject, &cert_name_debug_str)) {
