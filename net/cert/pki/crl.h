...
enum class CRLRevocationStatus { GOOD = 0, REVOKED = 1, UNKNOWN = 2, MAX_VALUE = UNKNOWN };
...
//                                       -- if present, version MUST be v2
//                                   }
[[nodiscard]] NET_EXPORT_PRIVATE bool ParseCrlTbsCertList(const der::Input& tbs_tlv, ParsedCrlTbsCertList* out);
...
NET_EXPORT_PRIVATE CRLRevocationStatus GetCRLStatusForCert(
    const der::Input& cert_serial, CrlVersion crl_version, const absl::optional<der::Input>& revoked_certificates_tlv);
...
[[nodiscard]] NET_EXPORT CRLRevocationStatus CheckCRL(std::string_view raw_crl, const ParsedCertificateList& valid_chain, size_t target_cert_index,
