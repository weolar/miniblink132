...
inline constexpr uint8_t kTypeDomainComponentOid[] = { 0x09, 0x92, 0x26, 0x89, 0x93, 0xF2, 0x2C, 0x64, 0x01, 0x19 };
...
inline constexpr uint8_t kTypeEmailAddressOid[] = { 0x2A, 0x86, 0x48, 0x86, 0xF7, 0x0D, 0x01, 0x09, 0x01 };
...
struct NET_EXPORT X509NameAttribute {
    X509NameAttribute(der::Input in_type, der::Tag in_value_tag, der::Input in_value)
        : type(in_type)
    ...
    [[nodiscard]] bool ValueAsStringWithUnsafeOptions(PrintableStringHandling printable_string_handling, std::string* out) const;
...
[[nodiscard]] NET_EXPORT bool ReadRdn(der::Parser* parser, RelativeDistinguishedName* out);

// Parses a DER-encoded "Name" as specified by 5280. Returns true on success
// and sets the results in |out|.
[[nodiscard]] NET_EXPORT bool ParseName(const der::Input& name_tlv, RDNSequence* out);
...
[[nodiscard]] NET_EXPORT bool ParseNameValue(const der::Input& name_value, RDNSequence* out);
...
// successful.
[[nodiscard]] NET_EXPORT bool ConvertToRFC2253(const RDNSequence& rdn_sequence, std::string* out);
} // namespace net
