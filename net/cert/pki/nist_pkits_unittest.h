...
struct PkitsTestInfo {
    ...
    InitialAnyPolicyInhibit initial_inhibit_any_policy = InitialAnyPolicyInhibit::kFalse;
...
template <typename PkitsTestDelegate> class PkitsTest : public ::testing::Test {
public:
    template <size_t num_certs, size_t num_crls>
    void RunTest(const char* const (&cert_names)[num_certs], const char* const (&crl_names)[num_crls], const PkitsTestInfo& info)
    {
        std::vector<std::string> cert_ders;
        for (const std::string& s : cert_names)
            cert_ders.push_back(net::ReadTestFileToString("net/third_party/nist-pkits/certs/" + s + ".crt"));
        std::vector<std::string> crl_ders;
        for (const std::string& s : crl_names)
            crl_ders.push_back(net::ReadTestFileToString("net/third_party/nist-pkits/crls/" + s + ".crl"));
        ...
        if (test_number == "4.1.2" || test_number == "4.1.3" || test_number == "4.1.6") {
            ...
            PkitsTestDelegate::RunTest(cert_ders, crl_ders, modified_info);
        } else if (test_number == "4.13.21" || test_number == "4.13.23" || test_number == "4.13.25" || test_number == "4.13.27" || test_number == "4.13.34"
            || test_number == "4.13.36") {
            PkitsTestInfo modified_info = info;
            modified_info.should_validate = false;
            PkitsTestDelegate::RunTest(cert_ders, crl_ders, modified_info);
