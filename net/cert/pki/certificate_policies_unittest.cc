...
::testing::AssertionResult LoadTestData(const std::string& name, std::string* result)
{
...
class ParseCertificatePoliciesExtensionOidsTest : public testing::TestWithParam<bool> {
protected:
    bool fail_parsing_unknown_qualifier_oids() const
    {
        return GetParam();
    }
};
...
INSTANTIATE_TEST_SUITE_P(All, ParseCertificatePoliciesExtensionOidsTest, testing::Bool());
...
TEST_P(ParseCertificatePoliciesExtensionOidsTest, InvalidEmpty)
{
    ...
    CertErrors errors;
    EXPECT_FALSE(ParseCertificatePoliciesExtensionOids(der::Input(&der), fail_parsing_unknown_qualifier_oids(), &policies, &errors));
}
...
TEST_P(ParseCertificatePoliciesExtensionOidsTest, AnyPolicy)
{
    ...
    std::vector<der::Input> policies;
    CertErrors errors;
    EXPECT_TRUE(ParseCertificatePoliciesExtensionOids(der::Input(&der), fail_parsing_unknown_qualifier_oids(), &policies, &errors));
...
TEST_P(ParseCertificatePoliciesExtensionOidsTest, InvalidAnyPolicyWithCustomQualifier)
{
    std::string der;
    ASSERT_TRUE(LoadTestData("invalid-anypolicy_with_custom_qualifier.pem", &der));
    std::vector<der::Input> policies;
    CertErrors errors;
    EXPECT_FALSE(ParseCertificatePoliciesExtensionOids(der::Input(&der), fail_parsing_unknown_qualifier_oids(), &policies, &errors));
}
...
TEST_P(ParseCertificatePoliciesExtensionOidsTest, OnePolicyWithCustomQualifier)
{
    std::string der;
    ASSERT_TRUE(LoadTestData("policy_1_2_3_with_custom_qualifier.pem", &der));
    std::vector<der::Input> policies;
    CertErrors errors;
    bool result = ParseCertificatePoliciesExtensionOids(der::Input(&der), fail_parsing_unknown_qualifier_oids(), &policies, &errors);
...
TEST_P(ParseCertificatePoliciesExtensionOidsTest, InvalidPolicyWithDuplicatePolicyOid)
{
    std::string der;
...
TEST_P(ParseCertificatePoliciesExtensionOidsTest, InvalidPolicyWithEmptyQualifiersSequence)
{
    std::string der;
    ASSERT_TRUE(LoadTestData("invalid-policy_1_2_3_with_empty_qualifiers_sequence.pem", &der));
    std::vector<der::Input> policies;
    CertErrors errors;
    EXPECT_FALSE(ParseCertificatePoliciesExtensionOids(der::Input(&der), fail_parsing_unknown_qualifier_oids(), &policies, &errors));
}

TEST_P(ParseCertificatePoliciesExtensionOidsTest, InvalidPolicyInformationHasUnconsumedData)
{
    std::string der;
    ASSERT_TRUE(LoadTestData("invalid-policy_1_2_3_policyinformation_unconsumed_data.pem", &der));
    std::vector<der::Input> policies;
    CertErrors errors;
    EXPECT_FALSE(ParseCertificatePoliciesExtensionOids(der::Input(&der), fail_parsing_unknown_qualifier_oids(), &policies, &errors));
}

TEST_P(ParseCertificatePoliciesExtensionOidsTest, InvalidPolicyQualifierInfoHasUnconsumedData)
{
    std::string der;
    ASSERT_TRUE(LoadTestData("invalid-policy_1_2_3_policyqualifierinfo_unconsumed_data.pem", &der));
    std::vector<der::Input> policies;
    CertErrors errors;
    EXPECT_FALSE(ParseCertificatePoliciesExtensionOids(der::Input(&der), fail_parsing_unknown_qualifier_oids(), &policies, &errors));
}
...
TEST(ParseCertificatePoliciesExtensionTest, InvalidEmpty)
{
    ...
    CertErrors errors;
    EXPECT_FALSE(ParseCertificatePoliciesExtension(der::Input(&der), &policies, &errors));
}

TEST(ParseCertificatePoliciesExtensionTest, InvalidPolicyWithDuplicatePolicyOid)
{
    std::string der;
    ASSERT_TRUE(LoadTestData("invalid-policy_1_2_3_dupe.pem", &der));
    std::vector<PolicyInformation> policies;
...
TEST(ParseCertificatePoliciesExtensionTest, OnePolicyWithCustomQualifier)
{
    ...
    std::vector<PolicyInformation> policies;
    CertErrors errors;
    EXPECT_TRUE(ParseCertificatePoliciesExtension(der::Input(&der), &policies, &errors));
    ASSERT_EQ(1U, policies.size());
...
TEST(ParseCertificatePoliciesExtensionTest, TwoPoliciesWithQualifiers)
{
    ...
    ASSERT_EQ(2U, policies.size());
    {
        ...
        const uint8_t kExpectedQualifier[] = { 0x16, 0x19, 0x68, 0x74, 0x74, 0x70, 0x73, 0x3a, 0x2f, 0x2f, 0x65, 0x78, 0x61, 0x6d, 0x70, 0x6c, 0x65, 0x2e, 0x63,
            0x6f, 0x6d, 0x2f, 0x31, 0x5f, 0x32, 0x5f, 0x33 };
        EXPECT_EQ(der::Input(kExpectedQualifier), qualifier.qualifier);
    }
    {
        PolicyInformation& policy = policies[1];
        ...
        const uint8_t kExpectedQualifier[] = { 0x16, 0x18, 0x68, 0x74, 0x74, 0x70, 0x3a, 0x2f, 0x2f, 0x65, 0x78, 0x61, 0x6d, 0x70, 0x6c, 0x65, 0x2e, 0x63, 0x6f,
            0x6d, 0x2f, 0x31, 0x5f, 0x32, 0x5f, 0x34 };
        EXPECT_EQ(der::Input(kExpectedQualifier), qualifier.qualifier);
    }
}
