...
public:
    ...
    void SyncGetIssuersOf(const ParsedCertificate* cert, ParsedCertificateList* issuers) override
    {
    }
    void AsyncGetIssuersOf(const ParsedCertificate* cert, std::unique_ptr<Request>* out_req) override
    {
        num_async_gets_++;
    ...
    int num_async_gets() const
    {
        return num_async_gets_;
    }
...
::testing::AssertionResult ReadTestPem(const std::string& file_name, const std::string& block_name, std::string* result)
{
...
::testing::AssertionResult ReadTestCert(const std::string& file_name, scoped_refptr<ParsedCertificate>* result)
{
    std::string der;
    ::testing::AssertionResult r = ReadTestPem("net/data/ssl/certificates/" + file_name, "CERTIFICATE", &der);
    if (!r)
        return r;
    CertErrors errors;
    *result = ParsedCertificate::Create(
        bssl::UniquePtr<CRYPTO_BUFFER>(CRYPTO_BUFFER_new(reinterpret_cast<const uint8_t*>(der.data()), der.size(), nullptr)), {}, &errors);
    if (!*result) {
...
public:
    ...
    CertificateTrust GetTrust(const ParsedCertificate* cert, base::SupportsUserData* debug_data) const override
    {
...
TEST(PathBuilderResultUserDataTest, ModifyUserDataInConstructor)
{
    ...
    SimplePathBuilderDelegate delegate(1024, SimplePathBuilderDelegate::DigestPolicy::kWeakAllowSha1);
    ...
    CertPathBuilder path_builder(a_by_b, &trust_store, &delegate, verify_time, KeyPurpose::ANY_EKU, InitialExplicitPolicy::kFalse,
        { der::Input(kAnyPolicyOid) }, InitialPolicyMappingInhibit::kFalse, InitialAnyPolicyInhibit::kFalse);
    CertPathBuilder::Result result = path_builder.Run();
    auto* data = static_cast<TrustStoreThatStoresUserData::Data*>(result.GetUserData(kKey));
    ASSERT_TRUE(data);
...
public:
    PathBuilderMultiRootTest()
        : delegate_(1024, SimplePathBuilderDelegate::DigestPolicy::kWeakAllowSha1)
    {
    }
...
protected:
    scoped_refptr<ParsedCertificate> a_by_b_, b_by_c_, b_by_f_, c_by_d_, c_by_e_, d_by_d_, e_by_e_, f_by_e_;

    SimplePathBuilderDelegate delegate_;
    der::GeneralizedTime time_ = { 2017, 3, 1, 0, 0, 0 };

    const InitialExplicitPolicy initial_explicit_policy_ = InitialExplicitPolicy::kFalse;
    const std::set<der::Input> user_initial_policy_set_ = { der::Input(kAnyPolicyOid) };
    const InitialPolicyMappingInhibit initial_policy_mapping_inhibit_ = InitialPolicyMappingInhibit::kFalse;
    const InitialAnyPolicyInhibit initial_any_policy_inhibit_ = InitialAnyPolicyInhibit::kFalse;
};
...
TEST_F(PathBuilderMultiRootTest, TargetHasNameAndSpkiOfTrustAnchor)
{
    ...
    CertPathBuilder path_builder(a_by_b_, &trust_store, &delegate_, time_, KeyPurpose::ANY_EKU, initial_explicit_policy_, user_initial_policy_set_,
...
TEST_F(PathBuilderMultiRootTest, SelfSignedTrustAnchorSupplementalCert)
{
    ...
    CertPathBuilder path_builder(b_by_c_, &trust_store, &delegate_, expired_time, KeyPurpose::ANY_EKU, initial_explicit_policy_, user_initial_policy_set_,
        initial_policy_mapping_inhibit_, initial_any_policy_inhibit_);
...
TEST_F(PathBuilderMultiRootTest, TargetIsSelfSignedTrustAnchor)
{
    ...
    CertPathBuilder path_builder(e_by_e_, &trust_store, &delegate_, time_, KeyPurpose::ANY_EKU, initial_explicit_policy_, user_initial_policy_set_,
        initial_policy_mapping_inhibit_, initial_any_policy_inhibit_);
...
TEST_F(PathBuilderMultiRootTest, TestCertIssuerOrdering)
{
    ...
    for (bool reverse_order : { false, true }) {
        ...
        std::vector<scoped_refptr<ParsedCertificate>> certs = { b_by_c_, b_by_f_, f_by_e_, c_by_d_, c_by_e_ };
        CertIssuerSourceStatic sync_certs;
        if (reverse_order) {
...
TEST_F(PathBuilderMultiRootTest, TestIterationLimit)
{
    ...
    for (const bool insufficient_limit : { true, false }) {
        ...
        if (insufficient_limit) {
            ...
            EXPECT_THAT(histogram_tester.GetAllSamples("Net.CertVerifier.PathBuilderIterationCount"), ElementsAre(base::Bucket(/*sample=*/2, /*count=*/1)));
        } else {
            EXPECT_EQ(3U, result.iteration_count);
            EXPECT_THAT(histogram_tester.GetAllSamples("Net.CertVerifier.PathBuilderIterationCount"), ElementsAre(base::Bucket(/*sample=*/3, /*count=*/1)));
        }
    }
}
...
TEST_F(PathBuilderMultiRootTest, TestTrivialDeadline)
{
    ...
    for (const bool insufficient_limit : { true, false }) {
        ...
        if (insufficient_limit) {
            ...
            EXPECT_EQ(a_by_b_, result.paths[0]->certs[0]);
            EXPECT_TRUE(result.paths[0]->errors.ContainsError(cert_errors::kDeadlineExceeded));
        } else {
            ASSERT_EQ(1U, result.paths.size());
...
TEST_F(PathBuilderMultiRootTest, TestDeadline)
{
    ...
    AsyncCertIssuerSourceStatic async_certs;
    async_certs.AddCert(c_by_d_);
    async_certs.SetAsyncGetCallback(base::BindLambdaForTesting([&] { task_environment.FastForwardBy(base::Seconds(2)); }));
...
TEST_F(PathBuilderMultiRootTest, TestDepthLimit)
{
    ...
    for (const bool insufficient_limit : { true, false }) {
        ...
        EXPECT_EQ(!insufficient_limit, result.HasValidPath());
        EXPECT_EQ(insufficient_limit, result.AnyPathContainsError(cert_errors::kDepthLimitExceeded));
        if (insufficient_limit) {
...
TEST_F(PathBuilderMultiRootTest, TestDepthLimitMultiplePaths)
{
    ...
    EXPECT_TRUE(truncated_path->errors.ContainsError(cert_errors::kDepthLimitExceeded));
    ...
    EXPECT_FALSE(valid_path->errors.ContainsError(cert_errors::kDepthLimitExceeded));
...
void AddToStoreWithEKURestriction(HCERTSTORE store, const scoped_refptr<ParsedCertificate>& cert, LPCSTR usage_identifier)
{
    crypto::ScopedPCCERT_CONTEXT os_cert(CertCreateCertificateContext(X509_ASN_ENCODING, cert->der_cert().UnsafeData(), cert->der_cert().Length()));
    ...
    CertAddCertificateContextToStore(store, os_cert.get(), CERT_STORE_ADD_ALWAYS, nullptr);
}

bool AreCertsEq(const scoped_refptr<ParsedCertificate> cert_1, const scoped_refptr<ParsedCertificate> cert_2)
{
...
{
    crypto::ScopedHCERTSTORE root_store(CertOpenStore(CERT_STORE_PROV_MEMORY, X509_ASN_ENCODING, NULL, 0, nullptr));
    crypto::ScopedHCERTSTORE intermediate_store(CertOpenStore(CERT_STORE_PROV_MEMORY, X509_ASN_ENCODING, NULL, 0, nullptr));
    crypto::ScopedHCERTSTORE disallowed_store(CertOpenStore(CERT_STORE_PROV_MEMORY, X509_ASN_ENCODING, NULL, 0, nullptr));

    AddToStoreWithEKURestriction(root_store.get(), d_by_d_, szOID_PKIX_KP_SERVER_AUTH);
    AddToStoreWithEKURestriction(root_store.get(), e_by_e_, szOID_PKIX_KP_SERVER_AUTH);
    AddToStoreWithEKURestriction(intermediate_store.get(), c_by_e_, szOID_PKIX_KP_SERVER_AUTH);
    AddToStoreWithEKURestriction(disallowed_store.get(), c_by_d_, nullptr);

    std::unique_ptr<TrustStoreWin> trust_store
        = TrustStoreWin::CreateForTesting(std::move(root_store), std::move(intermediate_store), std::move(disallowed_store));

    CertPathBuilder path_builder(b_by_c_, trust_store.get(), &delegate_, time_, KeyPurpose::ANY_EKU, initial_explicit_policy_, user_initial_policy_set_,
        initial_policy_mapping_inhibit_, initial_any_policy_inhibit_);
    ...
    int valid_paths = base::ranges::count_if(result.paths, &CertPathBuilderResultPath::IsValid);
...
public:
    ...
    void SetUp() override
    {
        ...
        ASSERT_TRUE(ReadVerifyCertChainTestFromFile("net/data/verify_certificate_chain_unittest/key-rollover/oldchain.test", &test));
        path = test.chain;
        ...
        ASSERT_TRUE(ReadVerifyCertChainTestFromFile("net/data/verify_certificate_chain_unittest/"
...
TEST_F(PathBuilderKeyRolloverTest, TestRolloverOnlyOldRootTrusted)
{
    ...
    CertPathBuilder path_builder(target_, &trust_store, &delegate_, time_, KeyPurpose::ANY_EKU, initial_explicit_policy_, user_initial_policy_set_,
        initial_policy_mapping_inhibit_, initial_any_policy_inhibit_);
    path_builder.AddCertIssuerSource(&sync_certs);
...
TEST_F(PathBuilderKeyRolloverTest, TestMultipleRootMatchesOnlyOneWorks)
{
    ...
    CertPathBuilder path_builder(target_, &trust_store_collection, &delegate_, time_, KeyPurpose::ANY_EKU, initial_explicit_policy_, user_initial_policy_set_,
        initial_policy_mapping_inhibit_, initial_any_policy_inhibit_);
    path_builder.AddCertIssuerSource(&sync_certs);
...
TEST_F(PathBuilderKeyRolloverTest, ExploreAllPathsWithIterationLimit)
{
    ...
    for (const auto& expectation : kExpectations) {
        ...
        if (expectation.partial_path.empty()) {
            ...
            EXPECT_TRUE(path.errors.ContainsError(cert_errors::kIterationLimitExceeded));
        }
...
TEST_F(PathBuilderKeyRolloverTest, TestEndEntityIsTrustRoot)
{
    ...
    CertPathBuilder path_builder(newintermediate_, &trust_store, &delegate_, time_, KeyPurpose::ANY_EKU, initial_explicit_policy_, user_initial_policy_set_,
        initial_policy_mapping_inhibit_, initial_any_policy_inhibit_);
...
TEST_F(PathBuilderKeyRolloverTest, TestEndEntityHasSameNameAndSpkiAsIntermediate)
{
    // Trust oldroot.
    TrustStoreInMemory trust_store;
    trust_store.AddTrustAnchor(oldroot_);
    ...
    CertPathBuilder path_builder(newroot_, &trust_store, &delegate_, time_, KeyPurpose::ANY_EKU, initial_explicit_policy_, user_initial_policy_set_,
        initial_policy_mapping_inhibit_, initial_any_policy_inhibit_);
    path_builder.AddCertIssuerSource(&sync_certs);
...
TEST_F(PathBuilderKeyRolloverTest, TestEndEntityHasSameNameAndSpkiAsTrustAnchor)
{
...
TEST_F(PathBuilderKeyRolloverTest, TestDuplicateIntermediates)
{
    ...
    scoped_refptr<ParsedCertificate> oldintermediate_dupe(ParsedCertificate::Create(
        bssl::UniquePtr<CRYPTO_BUFFER>(CRYPTO_BUFFER_new(oldintermediate_->der_cert().UnsafeData(), oldintermediate_->der_cert().Length(), nullptr)), {},
        nullptr));
...
TEST_F(PathBuilderKeyRolloverTest, TestDuplicateIntermediateAndRoot)
{
    ...
    scoped_refptr<ParsedCertificate> newroot_dupe(ParsedCertificate::Create(
        bssl::UniquePtr<CRYPTO_BUFFER>(CRYPTO_BUFFER_new(newroot_->der_cert().UnsafeData(), newroot_->der_cert().Length(), nullptr)), {}, nullptr));
...
public:
    MOCK_METHOD2(SyncGetIssuersOf, void(const ParsedCertificate*, ParsedCertificateList*));
    MOCK_METHOD2(AsyncGetIssuersOf, void(const ParsedCertificate*, std::unique_ptr<Request>*));
};
...
public:
    explicit CertIssuerSourceRequestMover(std::unique_ptr<CertIssuerSource::Request> req)
        : request_(std::move(req))
    {
    }
    void MoveIt(const ParsedCertificate* cert, std::unique_ptr<CertIssuerSource::Request>* out_req)
    {
...
TEST_F(PathBuilderKeyRolloverTest, TestMultipleAsyncIssuersFromSingleSource)
{
    ...
    auto target_issuers_req_owner = std::make_unique<StrictMock<MockCertIssuerSourceRequest>>();
    // Keep a raw pointer to the Request...
    StrictMock<MockCertIssuerSourceRequest>* target_issuers_req = target_issuers_req_owner.get();
    ...
    CertIssuerSourceRequestMover req_mover(std::move(target_issuers_req_owner));
    {
        ...
        EXPECT_CALL(cert_issuer_source, AsyncGetIssuersOf(target_.get(), _)).WillOnce(Invoke(&req_mover, &CertIssuerSourceRequestMover::MoveIt));
    }
        ...
        .WillOnce(Invoke(AppendCertToList(newintermediate_)));
    {
        ...
        EXPECT_CALL(cert_issuer_source, SyncGetIssuersOf(oldintermediate_.get(), _));
        EXPECT_CALL(cert_issuer_source, AsyncGetIssuersOf(oldintermediate_.get(), _));
    }
...
protected:
    void SetUp() override
    {
        ...
        ASSERT_TRUE(ReadVerifyCertChainTestFromFile("net/data/verify_certificate_chain_unittest/target-and-intermediate/"
                                                    "main.test",
            &test_));
    ...
    CertPathBuilder::Result RunPathBuilder(const scoped_refptr<ParsedCertificate>& distrusted_cert, CertPathBuilderDelegate* optional_delegate)
    {
        ...
        SimplePathBuilderDelegate default_delegate(1024, SimplePathBuilderDelegate::DigestPolicy::kWeakAllowSha1);
        CertPathBuilderDelegate* delegate = optional_delegate ? optional_delegate : &default_delegate;

        const InitialExplicitPolicy initial_explicit_policy = InitialExplicitPolicy::kFalse;
        const std::set<der::Input> user_initial_policy_set = { der::Input(kAnyPolicyOid) };
        const InitialPolicyMappingInhibit initial_policy_mapping_inhibit = InitialPolicyMappingInhibit::kFalse;
        const InitialAnyPolicyInhibit initial_any_policy_inhibit = InitialAnyPolicyInhibit::kFalse;
...
protected:
    // Runs the path builder for the target certificate while |distrusted_cert| is
    // blocked.
    CertPathBuilder::Result RunPathBuilderWithDistrustedCert(const scoped_refptr<ParsedCertificate>& distrusted_cert)
    {
...
class PathBuilderCheckPathAfterVerificationTest : public PathBuilderSimpleChainTest { };

class CertPathBuilderDelegateBase : public SimplePathBuilderDelegate {
public:
    CertPathBuilderDelegateBase()
        : SimplePathBuilderDelegate(1024, SimplePathBuilderDelegate::DigestPolicy::kWeakAllowSha1)
    {
    }
    void CheckPathAfterVerification(const CertPathBuilder& path_builder, CertPathBuilderResultPath* path) override
    {
...
public:
    MOCK_METHOD2(CheckPathAfterVerification, void(const CertPathBuilder& path_builder, CertPathBuilderResultPath* path));
};
...
TEST_F(PathBuilderCheckPathAfterVerificationTest, AddsWarningToValidPath)
{
    ...
    const CertErrors* cert1_errors = result.GetBestValidPath()->errors.GetErrorsForCert(1);
...
TEST_F(PathBuilderCheckPathAfterVerificationTest, AddsErrorToValidPath)
{
    ...
    ASSERT_LT(result.best_result_index, result.paths.size());
    const CertPathBuilderResultPath* failed_path = result.paths[result.best_result_index].get();
...
TEST_F(PathBuilderCheckPathAfterVerificationTest, SetsDelegateData)
{
    ...
    DelegateData* data = reinterpret_cast<DelegateData*>(result.GetBestValidPath()->delegate_data.get());
...
{
    std::string test_dir = "net/data/path_builder_unittest/validity_date_prioritization/";
    scoped_refptr<ParsedCertificate> root = ReadCertFromFile(test_dir + "root.pem");
    ASSERT_TRUE(root);
    scoped_refptr<ParsedCertificate> int_ac = ReadCertFromFile(test_dir + "int_ac.pem");
    ASSERT_TRUE(int_ac);
    scoped_refptr<ParsedCertificate> int_ad = ReadCertFromFile(test_dir + "int_ad.pem");
    ASSERT_TRUE(int_ad);
    scoped_refptr<ParsedCertificate> int_bc = ReadCertFromFile(test_dir + "int_bc.pem");
    ASSERT_TRUE(int_bc);
    scoped_refptr<ParsedCertificate> int_bd = ReadCertFromFile(test_dir + "int_bd.pem");
    ASSERT_TRUE(int_bd);
    scoped_refptr<ParsedCertificate> target = ReadCertFromFile(test_dir + "target.pem");
    ASSERT_TRUE(target);
    ...
    for (bool reverse_input_order : { false, true }) {
        ...
        CertPathBuilder path_builder(target, &trust_store, &delegate, verify_time, KeyPurpose::ANY_EKU, InitialExplicitPolicy::kFalse,
            { der::Input(kAnyPolicyOid) }, InitialPolicyMappingInhibit::kFalse, InitialAnyPolicyInhibit::kFalse);
        path_builder.AddCertIssuerSource(&intermediates);
...
{
    std::string test_dir = "net/data/path_builder_unittest/key_id_prioritization/";
    scoped_refptr<ParsedCertificate> root = ReadCertFromFile(test_dir + "root.pem");
    ASSERT_TRUE(root);
    scoped_refptr<ParsedCertificate> int_matching_ski_a = ReadCertFromFile(test_dir + "int_matching_ski_a.pem");
    ASSERT_TRUE(int_matching_ski_a);
    scoped_refptr<ParsedCertificate> int_matching_ski_b = ReadCertFromFile(test_dir + "int_matching_ski_b.pem");
    ASSERT_TRUE(int_matching_ski_b);
    scoped_refptr<ParsedCertificate> int_no_ski_a = ReadCertFromFile(test_dir + "int_no_ski_a.pem");
    ASSERT_TRUE(int_no_ski_a);
    scoped_refptr<ParsedCertificate> int_no_ski_b = ReadCertFromFile(test_dir + "int_no_ski_b.pem");
    ASSERT_TRUE(int_no_ski_b);
    scoped_refptr<ParsedCertificate> int_different_ski_a = ReadCertFromFile(test_dir + "int_different_ski_a.pem");
    ASSERT_TRUE(int_different_ski_a);
    scoped_refptr<ParsedCertificate> int_different_ski_b = ReadCertFromFile(test_dir + "int_different_ski_b.pem");
...
TEST(PathBuilderPrioritizationTest, TrustAndKeyIdPrioritization)
{
    ...
    scoped_refptr<ParsedCertificate> root = ReadCertFromFile(test_dir + "root.pem");
    ASSERT_TRUE(root);
    scoped_refptr<ParsedCertificate> trusted_and_matching = ReadCertFromFile(test_dir + "int_matching_ski_a.pem");
    ASSERT_TRUE(trusted_and_matching);
    scoped_refptr<ParsedCertificate> matching = ReadCertFromFile(test_dir + "int_matching_ski_b.pem");
    ASSERT_TRUE(matching);
    scoped_refptr<ParsedCertificate> distrusted_and_matching = ReadCertFromFile(test_dir + "int_matching_ski_c.pem");
    ASSERT_TRUE(distrusted_and_matching);
    scoped_refptr<ParsedCertificate> trusted_and_no_match_data = ReadCertFromFile(test_dir + "int_no_ski_a.pem");
    ASSERT_TRUE(trusted_and_no_match_data);
    scoped_refptr<ParsedCertificate> no_match_data = ReadCertFromFile(test_dir + "int_no_ski_b.pem");
    ASSERT_TRUE(no_match_data);
    scoped_refptr<ParsedCertificate> distrusted_and_no_match_data = ReadCertFromFile(test_dir + "int_no_ski_c.pem");
    ASSERT_TRUE(distrusted_and_no_match_data);
    scoped_refptr<ParsedCertificate> trusted_and_mismatch = ReadCertFromFile(test_dir + "int_different_ski_a.pem");
    ASSERT_TRUE(trusted_and_mismatch);
    scoped_refptr<ParsedCertificate> mismatch = ReadCertFromFile(test_dir + "int_different_ski_b.pem");
    ASSERT_TRUE(mismatch);
    scoped_refptr<ParsedCertificate> distrusted_and_mismatch = ReadCertFromFile(test_dir + "int_different_ski_c.pem");
...
{
    std::string test_dir = "net/data/path_builder_unittest/key_id_name_and_serial_prioritization/";
    scoped_refptr<ParsedCertificate> root = ReadCertFromFile(test_dir + "root.pem");
    ASSERT_TRUE(root);
    scoped_refptr<ParsedCertificate> root2 = ReadCertFromFile(test_dir + "root2.pem");
    ASSERT_TRUE(root2);
    scoped_refptr<ParsedCertificate> int_matching = ReadCertFromFile(test_dir + "int_matching.pem");
    ASSERT_TRUE(int_matching);
    scoped_refptr<ParsedCertificate> int_match_name_only = ReadCertFromFile(test_dir + "int_match_name_only.pem");
    ASSERT_TRUE(int_match_name_only);
    scoped_refptr<ParsedCertificate> int_mismatch = ReadCertFromFile(test_dir + "int_mismatch.pem");
...
{
    std::string test_dir = "net/data/path_builder_unittest/self_issued_prioritization/";
    scoped_refptr<ParsedCertificate> root1 = ReadCertFromFile(test_dir + "root1.pem");
    ASSERT_TRUE(root1);
    scoped_refptr<ParsedCertificate> root1_cross = ReadCertFromFile(test_dir + "root1_cross.pem");
