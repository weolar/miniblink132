...
[[nodiscard]] NET_EXPORT bool VerifySerialNumber(const der::Input& value, bool warnings_only, CertErrors* errors);
...
[[nodiscard]] NET_EXPORT bool ReadUTCOrGeneralizedTime(der::Parser* parser, der::GeneralizedTime* out);
...
[[nodiscard]] NET_EXPORT bool ParseValidity(const der::Input& validity_tlv, der::GeneralizedTime* not_before, der::GeneralizedTime* not_after);
...
[[nodiscard]] NET_EXPORT bool ParseCertificate(const der::Input& certificate_tlv, der::Input* out_tbs_certificate_tlv, der::Input* out_signature_algorithm_tlv,
...
[[nodiscard]] NET_EXPORT bool ParseTbsCertificate(
    const der::Input& tbs_tlv, const ParseCertificateOptions& options, ParsedTbsCertificate* out, CertErrors* errors);
...
// On failure |out| has an undefined state. Some of its fields may have been
// updated during parsing, whereas others may not have been changed.
[[nodiscard]] NET_EXPORT bool ParseExtension(const der::Input& extension_tlv, ParsedExtension* out);
...
inline constexpr uint8_t kAuthorityInfoAccessOid[] = { 0x2B, 0x06, 0x01, 0x05, 0x05, 0x07, 0x01, 0x01 };
...
inline constexpr uint8_t kAdCaIssuersOid[] = { 0x2B, 0x06, 0x01, 0x05, 0x05, 0x07, 0x30, 0x02 };
...
inline constexpr uint8_t kAdOcspOid[] = { 0x2B, 0x06, 0x01, 0x05, 0x05, 0x07, 0x30, 0x01 };
...
[[nodiscard]] NET_EXPORT bool ParseExtensions(const der::Input& extensions_tlv, std::map<der::Input, ParsedExtension>* extensions);
...
[[nodiscard]] NET_EXPORT bool ConsumeExtension(const der::Input& oid, std::map<der::Input, ParsedExtension>* unconsumed_extensions, ParsedExtension* extension);
...
// into a uint8_t.
[[nodiscard]] NET_EXPORT bool ParseBasicConstraints(const der::Input& basic_constraints_tlv, ParsedBasicConstraints* out);
...
[[nodiscard]] NET_EXPORT bool ParseKeyUsage(const der::Input& key_usage_tlv, der::BitString* key_usage);
...
[[nodiscard]] NET_EXPORT bool ParseAuthorityInfoAccess(
    const der::Input& authority_info_access_tlv, std::vector<AuthorityInfoAccessDescription>* out_access_descriptions);
...
[[nodiscard]] NET_EXPORT bool ParseCrlDistributionPoints(const der::Input& distribution_points_tlv, std::vector<ParsedDistributionPoint>* distribution_points);
...
// not guaranteed.
[[nodiscard]] NET_EXPORT bool ParseAuthorityKeyIdentifier(const der::Input& extension_value, ParsedAuthorityKeyIdentifier* authority_key_identifier);
...
[[nodiscard]] NET_EXPORT bool ParseSubjectKeyIdentifier(const der::Input& extension_value, der::Input* subject_key_identifier);
