...
bool IsHandledCriticalExtension(const ParsedExtension& extension)
{
    ...
    if (extension.oid == der::Input(kCertificatePoliciesOid)) {
        ...
        return ParseCertificatePoliciesExtensionOids(extension.value, true /*fail_parsing_unknown_qualifier_oids*/, &unused_policies, &unused_errors);
...
// extensions.
void VerifyNoUnconsumedCriticalExtensions(const ParsedCertificate& cert, CertErrors* errors)
{
    for (const auto& it : cert.extensions()) {
        ...
        if (extension.critical && !IsHandledCriticalExtension(extension)) {
            errors->AddError(cert_errors::kUnconsumedCriticalExtension, CreateCertErrorParams2Der("oid", extension.oid, "value", extension.value));
        }
    }
}
...
void VerifyTimeValidity(const ParsedCertificate& cert, const der::GeneralizedTime& time, CertErrors* errors)
{
...
// compatibility sake.
bool VerifySignatureAlgorithmsMatch(const ParsedCertificate& cert, CertErrors* errors)
{
    ...
    absl::optional<SignatureAlgorithm> alg1 = ParseSignatureAlgorithm(alg1_tlv, errors);
    if (!alg1) {
        errors->AddError(cert_errors::kUnacceptableSignatureAlgorithm);
        return false;
    }
    absl::optional<SignatureAlgorithm> alg2 = ParseSignatureAlgorithm(alg2_tlv, errors);
    if (!alg2) {
        errors->AddError(cert_errors::kUnacceptableSignatureAlgorithm);
        return false;
    }
    ...
    if (*alg1 == *alg2) {
        errors->AddWarning(cert_errors::kSignatureAlgorithmsDifferentEncoding,
            CreateCertErrorParams2Der("Certificate.algorithm", alg1_tlv, "TBSCertificate.signature", alg2_tlv));
        return true;
    }

    errors->AddError(
        cert_errors::kSignatureAlgorithmMismatch, CreateCertErrorParams2Der("Certificate.algorithm", alg1_tlv, "TBSCertificate.signature", alg2_tlv));
    return false;
}

// Verify that |cert| can be used for |required_key_purpose|.
void VerifyExtendedKeyUsage(const ParsedCertificate& cert, KeyPurpose required_key_purpose, CertErrors* errors)
{
    ...
    case KeyPurpose::SERVER_AUTH: {
        ...
        if (has_nsgc) {
            ...
            // important.
            if ((cert.has_basic_constraints() && cert.basic_constraints().is_ca) && cert.signature_algorithm() == SignatureAlgorithm::kRsaPkcs1Sha1) {
                return;
            }
        }
...
bool SetContains(const std::set<der::Input>& policies, const der::Input& search_oid)
{
...
public:
    ...
    void GetValidRootPolicySet(std::set<der::Input>* policy_set)
    {
        ...
        if (policy_set->size() > 1 && SetContains(*policy_set, der::Input(kAnyPolicyOid))) {
    ...
    void AddNodeWithExpectedPolicySet(const Node& parent, const der::Input& policy_oid, const std::set<der::Input>& expected_policy_set)
    {
        Node new_node;
        ...
        new_node.root_policy = (parent.root_policy == der::Input(kAnyPolicyOid)) ? policy_oid : parent.root_policy;
    ...
    static void DeleteNodesMatchingValidPolicy(const der::Input& valid_policy, Level* level)
    {
...
public:
    ...
    void Run(const ParsedCertificateList& certs, const CertificateTrust& last_cert_trust, VerifyCertificateChainDelegate* delegate,
        const der::GeneralizedTime& time, KeyPurpose required_key_purpose, InitialExplicitPolicy initial_explicit_policy,
        const std::set<der::Input>& user_initial_policy_set, InitialPolicyMappingInhibit initial_policy_mapping_inhibit,
        InitialAnyPolicyInhibit initial_any_policy_inhibit, std::set<der::Input>* user_constrained_policy_set, CertPathErrors* errors);

private:
    // Verifies and updates the valid policies. This corresponds with RFC 5280
    // section 6.1.3 steps d-f.
    void VerifyPolicies(const ParsedCertificate& cert, bool is_target_cert, CertErrors* errors);
    ...
    void BasicCertificateProcessing(const ParsedCertificate& cert, bool is_target_cert, const der::GeneralizedTime& time, KeyPurpose required_key_purpose,
    ...
    void PrepareForNextCertificate(const ParsedCertificate& cert, CertErrors* errors);
    ...
    // itself.
    void ApplyTrustAnchorConstraints(const ParsedCertificate& cert, KeyPurpose required_key_purpose, CertErrors* errors);

    // Initializes the path validation algorithm given anchor constraints. This
    // follows the description in RFC 5937
    void ProcessRootCertificate(const ParsedCertificate& cert, const CertificateTrust& trust, const der::GeneralizedTime& time, KeyPurpose required_key_purpose,
        CertErrors* errors, bool* shortcircuit_chain_validation);
    ...
    bssl::UniquePtr<EVP_PKEY> ParseAndCheckPublicKey(const der::Input& spki, CertErrors* errors);
...
void PathVerifier::VerifyPolicies(const ParsedCertificate& cert, bool is_target_cert, CertErrors* errors)
{
    ...
    if (cert.has_policy_oids() && !valid_policy_tree_.IsNull()) {
        ...
        const ValidPolicyTree::Node* any_policy_node_prev_level = ValidPolicyTree::FindAnyPolicyNode(previous_level);
        ...
        //          this node.
        if (cert_has_any_policy && ((inhibit_any_policy_ > 0) || (!is_target_cert && IsSelfIssued(cert)))) {
            // Keep track of the existing policies at depth i.
            std::set<der::Input> child_node_policies;
            for (const ValidPolicyTree::Node& node : valid_policy_tree_.current_level())
                child_node_policies.insert(node.valid_policy);

            for (const ValidPolicyTree::Node& prev_node : previous_level) {
                for (const der::Input& expected_policy : prev_node.expected_policy_set) {
...
void PathVerifier::VerifyPolicyMappings(const ParsedCertificate& cert, CertErrors* errors)
{
    ...
    for (const ParsedPolicyMapping& mapping : cert.policy_mappings()) {
        if (mapping.issuer_domain_policy == der::Input(kAnyPolicyOid) || mapping.subject_domain_policy == der::Input(kAnyPolicyOid)) {
    ...
    if (policy_mapping_ > 0) {
        const ValidPolicyTree::Node* any_policy_node = ValidPolicyTree::FindAnyPolicyNode(valid_policy_tree_.current_level());

        // Group mappings by issuer domain policy.
        std::map<der::Input, std::set<der::Input>> mappings;
        for (const ParsedPolicyMapping& mapping : cert.policy_mappings()) {
            mappings[mapping.issuer_domain_policy].insert(mapping.subject_domain_policy);
        }
        ...
        for (const auto& it : mappings) {
            ...
            if (!found_node && any_policy_node) {
                valid_policy_tree_.AddNodeWithExpectedPolicySet(*any_policy_node, issuer_domain_policy, subject_domain_policies);
            }
        }
    }
    ...
    if (policy_mapping_ == 0) {
        for (const ParsedPolicyMapping& mapping : cert.policy_mappings()) {
            ValidPolicyTree::DeleteNodesMatchingValidPolicy(mapping.issuer_domain_policy, &valid_policy_tree_.current_level());
        }
    }
}

void PathVerifier::BasicCertificateProcessing(const ParsedCertificate& cert, bool is_target_cert, const der::GeneralizedTime& time,
    KeyPurpose required_key_purpose, CertErrors* errors, bool* shortcircuit_chain_validation)
{
    *shortcircuit_chain_validation = false;
    ...
    if (!cert.signature_algorithm().has_value() || !delegate_->IsSignatureAlgorithmAcceptable(*cert.signature_algorithm(), errors)) {
        *shortcircuit_chain_validation = true;
        errors->AddError(cert_errors::kUnacceptableSignatureAlgorithm);
        return;
    }
    ...
    if (working_public_key_) {
        ...
        if (!VerifySignedData(*cert.signature_algorithm(), cert.tbs_certificate_tlv(), cert.signature_value(), working_public_key_.get())) {
            *shortcircuit_chain_validation = true;
    ...
    if (!name_constraints_list_.empty() && (!IsSelfIssued(cert) || is_target_cert)) {
        for (const NameConstraints* nc : name_constraints_list_) {
            nc->IsPermittedCert(cert.normalized_subject(), cert.subject_alt_names(), errors);
        }
    }
...
void PathVerifier::PrepareForNextCertificate(const ParsedCertificate& cert, CertErrors* errors)
{
    ...
    if (cert.has_policy_constraints()) {
        ...
        if (cert.policy_constraints().require_explicit_policy && cert.policy_constraints().require_explicit_policy.value() < explicit_policy_) {
            explicit_policy_ = cert.policy_constraints().require_explicit_policy.value();
        }

        //         (2)  If inhibitPolicyMapping is present and is less than
        //              policy_mapping, set policy_mapping to the value of
        //              inhibitPolicyMapping.
        if (cert.policy_constraints().inhibit_policy_mapping && cert.policy_constraints().inhibit_policy_mapping.value() < policy_mapping_) {
            policy_mapping_ = cert.policy_constraints().inhibit_policy_mapping.value();
        }
    }
    ...
    if (cert.has_inhibit_any_policy() && cert.inhibit_any_policy() < inhibit_any_policy_) {
    ...
    if (cert.has_basic_constraints() && cert.basic_constraints().has_path_len && cert.basic_constraints().path_len < max_path_length_) {
...
// constraint.
void VerifyTargetCertHasConsistentCaBits(const ParsedCertificate& cert, CertErrors* errors)
{
    // Check if the certificate contains any property specific to CAs.
    bool has_ca_property = (cert.has_basic_constraints() && (cert.basic_constraints().is_ca || cert.basic_constraints().has_path_len))
        || (cert.has_key_usage() && cert.key_usage().AssertsBit(KEY_USAGE_BIT_KEY_CERT_SIGN));

    // If it "looks" like a CA because it has a CA-only property, then check that
    // it sets ALL the properties expected of a CA.
    if (has_ca_property) {
        bool success = cert.has_basic_constraints() && cert.basic_constraints().is_ca
            && (!cert.has_key_usage() || cert.key_usage().AssertsBit(KEY_USAGE_BIT_KEY_CERT_SIGN));
        if (!success) {
...
void PathVerifier::WrapUp(const ParsedCertificate& cert, CertErrors* errors)
{
    ...
    if (cert.has_policy_constraints() && cert.policy_constraints().require_explicit_policy.has_value()
...
void PathVerifier::ApplyTrustAnchorConstraints(const ParsedCertificate& cert, KeyPurpose required_key_purpose, CertErrors* errors)
{
...
void PathVerifier::ProcessRootCertificate(const ParsedCertificate& cert, const CertificateTrust& trust, const der::GeneralizedTime& time,
    KeyPurpose required_key_purpose, CertErrors* errors, bool* shortcircuit_chain_validation)
{
    *shortcircuit_chain_validation = false;
    switch (trust.type) {
...
bssl::UniquePtr<EVP_PKEY> PathVerifier::ParseAndCheckPublicKey(const der::Input& spki, CertErrors* errors)
{
...
void PathVerifier::Run(const ParsedCertificateList& certs, const CertificateTrust& last_cert_trust, VerifyCertificateChainDelegate* delegate,
    const der::GeneralizedTime& time, KeyPurpose required_key_purpose, InitialExplicitPolicy initial_explicit_policy,
    const std::set<der::Input>& user_initial_policy_set, InitialPolicyMappingInhibit initial_policy_mapping_inhibit,
    InitialAnyPolicyInhibit initial_any_policy_inhibit, std::set<der::Input>* user_constrained_policy_set, CertPathErrors* errors)
{
    ...
    explicit_policy_ = initial_explicit_policy == InitialExplicitPolicy::kTrue ? 0 : n + 1;

    // RFC 5280 section section 6.1.2:
    //
    // If initial-any-policy-inhibit is set, then the initial value
    // [of inhibit_anyPolicy] is 0, otherwise the initial value is n+1.
    inhibit_any_policy_ = initial_any_policy_inhibit == InitialAnyPolicyInhibit::kTrue ? 0 : n + 1;

    // RFC 5280 section section 6.1.2:
    //
    // If initial-policy-mapping-inhibit is set, then the initial value
    // [of policy_mapping] is 0, otherwise the initial value is n+1.
    policy_mapping_ = initial_policy_mapping_inhibit == InitialPolicyMappingInhibit::kTrue ? 0 : n + 1;
    ...
    for (size_t i = 0; i < certs.size(); ++i) {
        ...
        if (is_root_cert) {
            ...
            ProcessRootCertificate(cert, last_cert_trust, time, required_key_purpose, cert_errors, &shortcircuit_chain_validation);
            if (shortcircuit_chain_validation) {
                ...
                CHECK(cert_errors->ContainsAnyErrorWithSeverity(CertError::SEVERITY_HIGH));
                return;
            }
        ...
        BasicCertificateProcessing(cert, is_target_cert, time, required_key_purpose, cert_errors, &shortcircuit_chain_validation);
        if (shortcircuit_chain_validation) {
...
void VerifyCertificateChain(const ParsedCertificateList& certs, const CertificateTrust& last_cert_trust, VerifyCertificateChainDelegate* delegate,
    const der::GeneralizedTime& time, KeyPurpose required_key_purpose, InitialExplicitPolicy initial_explicit_policy,
    const std::set<der::Input>& user_initial_policy_set, InitialPolicyMappingInhibit initial_policy_mapping_inhibit,
    InitialAnyPolicyInhibit initial_any_policy_inhibit, std::set<der::Input>* user_constrained_policy_set, CertPathErrors* errors)
{
    PathVerifier verifier;
    verifier.Run(certs, last_cert_trust, delegate, time, required_key_purpose, initial_explicit_policy, user_initial_policy_set, initial_policy_mapping_inhibit,
