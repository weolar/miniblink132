...
//     ... -- Extensible
//     }
bool ParsePublicKey(const der::Input& public_key_spki, bssl::UniquePtr<EVP_PKEY>* public_key)
{
...
bool VerifySignedData(SignatureAlgorithm algorithm, const der::Input& signed_data, const der::BitString& signature_value, EVP_PKEY* public_key)
{
    ...
    if (!EVP_DigestVerifyUpdate(ctx.get(), signed_data.UnsafeData(), signed_data.Length())) {
        return false;
    }
...
bool VerifySignedData(SignatureAlgorithm algorithm, const der::Input& signed_data, const der::BitString& signature_value, const der::Input& public_key_spki)
{
    bssl::UniquePtr<EVP_PKEY> public_key;
    if (!ParsePublicKey(public_key_spki, &public_key))
        return false;
    return VerifySignedData(algorithm, signed_data, signature_value, public_key.get());
}
