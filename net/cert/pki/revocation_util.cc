...
bool CheckRevocationDateValid(
    const der::GeneralizedTime& this_update, const der::GeneralizedTime* next_update, const base::Time& verify_time, const base::TimeDelta& max_age)
{
    ...
    if (!der::EncodeTimeAsGeneralizedTime(verify_time - max_age, &earliest_this_update)) {
        return false;
    }
