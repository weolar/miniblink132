...
scoped_refptr<ParsedCertificate> ParseCertificateFromFile(const std::string& file_name, const ParseCertificateOptions& options)
{
    std::string data;
    ...
    scoped_refptr<ParsedCertificate> cert = ParsedCertificate::Create(
        bssl::UniquePtr<CRYPTO_BUFFER>(CRYPTO_BUFFER_new(reinterpret_cast<const uint8_t*>(data.data()), data.size(), nullptr)), options, &errors);
...
der::Input DavidBenOid()
{
    ...
    static const uint8_t kOid[] = { 0x2a, 0x86, 0x48, 0x86, 0xf7, 0x12, 0x04, 0x01, 0x84, 0xb7, 0x09, 0x00 };
...
{
    scoped_refptr<ParsedCertificate> cert = ParseCertificateFromFile("extension_critical.pem", {});
    ASSERT_TRUE(cert);
...
{
    scoped_refptr<ParsedCertificate> cert = ParseCertificateFromFile("extension_not_critical.pem", {});
    ASSERT_TRUE(cert);
...
{
    scoped_refptr<ParsedCertificate> cert = ParseCertificateFromFile("bad_signature_algorithm_oid.pem", {});
    ASSERT_TRUE(cert);
...
{
    scoped_refptr<ParsedCertificate> cert = ParseCertificateFromFile("failed_signature_algorithm.pem", {});
    ASSERT_TRUE(cert);
    ASSERT_FALSE(cert->signature_algorithm());
}
...
{
    ASSERT_FALSE(ParseCertificateFromFile("policy_qualifiers_empty_sequence.pem", {}));
}

TEST(ParsedCertificateTest, SubjectBlankSubjectAltNameNotCritical)
{
    ASSERT_FALSE(ParseCertificateFromFile("subject_blank_subjectaltname_not_critical.pem", {}));
}
...
{
    ASSERT_FALSE(ParseCertificateFromFile("subject_not_printable_string.pem", {}));
}
...
{
    ASSERT_FALSE(ParseCertificateFromFile("subjectaltname_dns_not_ascii.pem", {}));
}

TEST(ParsedCertificateTest, SubjectAltNameGeneralNamesEmptySequence)
{
    ASSERT_FALSE(ParseCertificateFromFile("subjectaltname_general_names_empty_sequence.pem", {}));
}

TEST(ParsedCertificateTest, SubjectAltNameTrailingData)
{
    ASSERT_FALSE(ParseCertificateFromFile("subjectaltname_trailing_data.pem", {}));
}
...
{
    scoped_refptr<ParsedCertificate> cert = ParseCertificateFromFile("extended_key_usage.pem", {});
    ASSERT_TRUE(cert);
...
{
    scoped_refptr<ParsedCertificate> cert = ParseCertificateFromFile("key_usage.pem", {});
    ASSERT_TRUE(cert);
...
{
    scoped_refptr<ParsedCertificate> cert = ParseCertificateFromFile("policies.pem", {});
    ASSERT_TRUE(cert);

    ASSERT_EQ(4u, cert->extensions().size());

    ParsedExtension extension;
    ASSERT_TRUE(cert->GetExtension(der::Input(kCertificatePoliciesOid), &extension));
...
{
    scoped_refptr<ParsedCertificate> cert = ParseCertificateFromFile("subject_alt_name.pem", {});
    ASSERT_TRUE(cert);
...
{
    scoped_refptr<ParsedCertificate> cert = ParseCertificateFromFile("extensions_real.pem", {});
    ASSERT_TRUE(cert);
    ...
    EXPECT_EQ(der::Input(expected_authority_key_identifier), cert->authority_key_identifier()->key_identifier);

    ASSERT_TRUE(cert->subject_key_identifier());
    const uint8_t expected_subject_key_identifier[]
        = { 0x4a, 0xdd, 0x06, 0x16, 0x1b, 0xbc, 0xf6, 0x68, 0xb5, 0x76, 0xf5, 0x81, 0xb6, 0xbb, 0x62, 0x1a, 0xba, 0x5a, 0x81, 0x2f };
    EXPECT_EQ(der::Input(expected_subject_key_identifier), cert->subject_key_identifier());
...
{
    scoped_refptr<ParsedCertificate> cert = ParseCertificateFromFile("basic_constraints_not_ca.pem", {});
    ASSERT_TRUE(cert);
...
{
    scoped_refptr<ParsedCertificate> cert = ParseCertificateFromFile("basic_constraints_ca_no_path.pem", {});
    ASSERT_TRUE(cert);
...
{
    scoped_refptr<ParsedCertificate> cert = ParseCertificateFromFile("basic_constraints_ca_path_9.pem", {});
    ASSERT_TRUE(cert);
...
{
    scoped_refptr<ParsedCertificate> cert = ParseCertificateFromFile("basic_constraints_pathlen_255.pem", {});
    ASSERT_TRUE(cert);
...
{
    ASSERT_FALSE(ParseCertificateFromFile("basic_constraints_path_too_large.pem", {}));
}
...
{
    scoped_refptr<ParsedCertificate> cert = ParseCertificateFromFile("basic_constraints_ca_false.pem", {});
    ASSERT_TRUE(cert);
...
{
    ASSERT_FALSE(ParseCertificateFromFile("basic_constraints_unconsumed_data.pem", {}));
}
...
{
    scoped_refptr<ParsedCertificate> cert = ParseCertificateFromFile("basic_constraints_pathlen_not_ca.pem", {});
    ASSERT_TRUE(cert);
...
{
    scoped_refptr<ParsedCertificate> cert = ParseCertificateFromFile("policy_constraints_require.pem", {});
    ASSERT_TRUE(cert);
...
{
    scoped_refptr<ParsedCertificate> cert = ParseCertificateFromFile("policy_constraints_inhibit.pem", {});
    ASSERT_TRUE(cert);
...
{
    scoped_refptr<ParsedCertificate> cert = ParseCertificateFromFile("policy_constraints_inhibit_require.pem", {});
    ASSERT_TRUE(cert);
...
{
    scoped_refptr<ParsedCertificate> cert = ParseCertificateFromFile("policy_constraints_empty.pem", {});
    ASSERT_FALSE(cert);
}
...
{
    scoped_refptr<ParsedCertificate> cert = ParseCertificateFromFile("serial_zero_padded.pem", {});
    ASSERT_TRUE(cert);
...
{
    scoped_refptr<ParsedCertificate> cert = ParseCertificateFromFile("serial_zero_padded_21_bytes.pem", {});
    ASSERT_FALSE(cert);
...
{
    scoped_refptr<ParsedCertificate> cert = ParseCertificateFromFile("serial_negative.pem", {});
    ASSERT_TRUE(cert);
...
{
    scoped_refptr<ParsedCertificate> cert = ParseCertificateFromFile("serial_37_bytes.pem", {});
    ASSERT_FALSE(cert);
...
{
    scoped_refptr<ParsedCertificate> cert = ParseCertificateFromFile("serial_zero.pem", {});
    ASSERT_TRUE(cert);
...
{
    scoped_refptr<ParsedCertificate> cert = ParseCertificateFromFile("serial_not_number.pem", {});
    ASSERT_FALSE(cert);
}
...
{
    scoped_refptr<ParsedCertificate> cert = ParseCertificateFromFile("serial_not_minimal.pem", {});
    ASSERT_FALSE(cert);
}
...
{
    scoped_refptr<ParsedCertificate> cert = ParseCertificateFromFile("inhibit_any_policy.pem", {});
    ASSERT_TRUE(cert);
...
{
    scoped_refptr<ParsedCertificate> cert = ParseCertificateFromFile("subject_key_identifier_not_octet_string.pem", {});
    ASSERT_FALSE(cert);
}
...
{
    scoped_refptr<ParsedCertificate> cert = ParseCertificateFromFile("authority_key_identifier_not_sequence.pem", {});
    ASSERT_FALSE(cert);
}
