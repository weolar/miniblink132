...
bool GetValue(std::string_view prefix, std::string_view line, std::string* value, bool* has_value)
{
...
::testing::AssertionResult ReadTestDataFromPemFile(const std::string& file_path_ascii, const PemBlockMapping* mappings, size_t mappings_length)
{
    ...
    if (!base::ReadFileToString(filepath, &file_data)) {
        return ::testing::AssertionFailure() << "Couldn't read file: " << filepath.value();
    }
    ...
    std::vector<PemBlockMapping> mappings_copy(mappings, mappings + mappings_length);
    ...
    while (pem_tokenizer.GetNext()) {
        for (auto& mapping : mappings_copy) {
            ...
            if (pem_tokenizer.block_type() == mapping.block_name) {
                if (!mapping.value) {
                    return ::testing::AssertionFailure() << "PEM block defined multiple times: " << mapping.block_name;
                }
    ...
    for (const auto& mapping : mappings_copy) {
        if (mapping.value && !mapping.optional) {
            return ::testing::AssertionFailure() << "PEM block missing: " << mapping.block_name;
        }
    }
...
bool ReadCertChainFromFile(const std::string& file_path_ascii, ParsedCertificateList* chain)
{
    ...
    *chain = ParsedCertificateList();
    ...
    while (pem_tokenizer.GetNext()) {
        ...
        if (!ParsedCertificate::CreateAndAddToVector(
                bssl::UniquePtr<CRYPTO_BUFFER>(CRYPTO_BUFFER_new(reinterpret_cast<const uint8_t*>(block_data.data()), block_data.size(), nullptr)), {}, chain,
                &errors)) {
...
scoped_refptr<ParsedCertificate> ReadCertFromFile(const std::string& file_path_ascii)
{
...
bool ReadVerifyCertChainTestFromFile(const std::string& file_path_ascii, VerifyCertChainTest* test)
{
    // Reset all the out parameters to their defaults.
    *test = {};
    ...
    for (std::string line; std::getline(stream, line, '\n');) {
        ...
        if (GetValue("chain: ", line_piece, &value, &has_chain)) {
        ...
        } else if (GetValue("key_purpose: ", line_piece, &value, &has_key_purpose)) {
            if (value == "ANY_EKU") {
            ...
            if (value == "TRUSTED_ANCHOR") {
                ...
                test->last_cert_trust = CertificateTrust::ForTrustAnchorEnforcingExpiration();
            } else if (value == "TRUSTED_ANCHOR_WITH_CONSTRAINTS") {
                test->last_cert_trust = CertificateTrust::ForTrustAnchorEnforcingConstraints();
        ...
        } else if (GetValue("digest_policy: ", line_piece, &value, &has_digest_policy)) {
            if (value == "STRONG") {
                test->digest_policy = SimplePathBuilderDelegate::DigestPolicy::kStrong;
            } else if (value == "ALLOW_SHA_1") {
                test->digest_policy = SimplePathBuilderDelegate::DigestPolicy::kWeakAllowSha1;
            } else {
...
void VerifyCertPathErrors(
    const std::string& expected_errors_str, const CertPathErrors& actual_errors, const ParsedCertificateList& chain, const std::string& errors_file_path)
{
    std::string actual_errors_str = actual_errors.ToDebugString(chain);

    if (expected_errors_str != actual_errors_str) {
        ADD_FAILURE() << "Cert path errors don't match expectations (" << errors_file_path << ")\n\n"
                      << "EXPECTED:\n\n"
                      << expected_errors_str << "\n"
                      << "ACTUAL:\n\n"
                      << actual_errors_str << "\n"
...
void VerifyCertErrors(const std::string& expected_errors_str, const CertErrors& actual_errors, const std::string& errors_file_path)
{
    std::string actual_errors_str = actual_errors.ToDebugString();

    if (expected_errors_str != actual_errors_str) {
        ADD_FAILURE() << "Cert errors don't match expectations (" << errors_file_path << ")\n\n"
                      << "EXPECTED:\n\n"
                      << expected_errors_str << "\n"
                      << "ACTUAL:\n\n"
                      << actual_errors_str << "\n"
