...
DEFINE_CERT_ERROR_ID(kFailedReadingGeneralNames, "Failed reading GeneralNames SEQUENCE");
DEFINE_CERT_ERROR_ID(kGeneralNamesTrailingData, "GeneralNames contains trailing data after the sequence");
DEFINE_CERT_ERROR_ID(kGeneralNamesEmpty, "GeneralNames is a sequence of 0 elements");
DEFINE_CERT_ERROR_ID(kFailedReadingGeneralName, "Failed reading GeneralName TLV");
...
// static
std::unique_ptr<GeneralNames> GeneralNames::Create(const der::Input& general_names_tlv, CertErrors* errors)
{
    DCHECK(errors);
...
// static
std::unique_ptr<GeneralNames> GeneralNames::CreateFromValue(const der::Input& general_names_value, CertErrors* errors)
{
    DCHECK(errors);
    ...
    while (sequence_parser.HasMore()) {
        ...
        if (!ParseGeneralName(raw_general_name, IP_ADDRESS_ONLY, general_names.get(), errors)) {
...
[[nodiscard]] bool ParseGeneralName(
    const der::Input& input, GeneralNames::ParseGeneralNameIPAddressType ip_address_type, GeneralNames* subtrees, CertErrors* errors)
{
    DCHECK(errors);
    ...
    if (tag == der::ContextSpecificConstructed(0)) {
        ...
        if (ip_address_type == GeneralNames::IP_ADDRESS_ONLY) {
            ...
            const IPAddress mask(value.UnsafeData() + value.Length() / 2, value.Length() / 2);
            const unsigned mask_prefix_length = MaskPrefixLength(mask);
            if (!IsSuffixZero(mask.bytes(), mask_prefix_length)) {
                errors->AddError(kFailedParsingIp);
                return false;
            }
            subtrees->ip_address_ranges.emplace_back(IPAddress(value.UnsafeData(), value.Length() / 2), mask_prefix_length);
        }
        ...
        errors->AddError(kUnknownGeneralNameType, CreateCertErrorParams1SizeT("tag", tag));
        return false;
    }
