...
public:
    static void Verify(const VerifyCertChainTest& test, const std::string& test_file_path)
    {
        ...
        CertPathBuilder path_builder(test.chain.front(), &trust_store, &path_builder_delegate, test.time, test.key_purpose, test.initial_explicit_policy,
            test.user_initial_policy_set, test.initial_policy_mapping_inhibit, test.initial_any_policy_inhibit);
...
INSTANTIATE_TYPED_TEST_SUITE_P(PathBuilder, VerifyCertificateChainSingleRootTest, PathBuilderTestDelegate);
