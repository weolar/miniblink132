...
bool DNSNameMatches(std::string_view name, std::string_view dns_constraint, WildcardMatchType wildcard_matching)
{
    ...
    if (wildcard_matching == WILDCARD_PARTIAL_MATCH && name.size() > 2 && name[0] == '*' && name[1] == '.') {
        ...
        if (dns_constraint_dot_pos != std::string::npos) {
            std::string_view dns_constraint_domain = dns_constraint.substr(dns_constraint_dot_pos + 1);
            std::string_view wildcard_domain = name.substr(2);
            if (net::string_util::IsEqualNoCase(wildcard_domain, dns_constraint_domain)) {
                return true;
            }
        }
    }
...
// return value.
[[nodiscard]] bool ParseGeneralSubtrees(const der::Input& value, GeneralNames* subtrees, CertErrors* errors)
{
    DCHECK(errors);
    ...
    while (sequence_parser.HasMore()) {
        ...
        if (!ParseGeneralName(raw_general_name, GeneralNames::IP_ADDRESS_AND_NETMASK, subtrees, errors)) {
...
// static
std::unique_ptr<NameConstraints> NameConstraints::Create(const der::Input& extension_value, bool is_critical, CertErrors* errors)
{
    DCHECK(errors);
...
bool NameConstraints::Parse(const der::Input& extension_value, bool is_critical, CertErrors* errors)
{
    DCHECK(errors);
    ...
    if (!sequence_parser.ReadOptionalTag(der::ContextSpecificConstructed(0), &permitted_subtrees_value)) {
        return false;
    }
    if (permitted_subtrees_value && !ParseGeneralSubtrees(permitted_subtrees_value.value(), &permitted_subtrees_, errors)) {
        return false;
    }
    constrained_name_types_ |= permitted_subtrees_.present_name_types & (is_critical ? GENERAL_NAME_ALL_TYPES : kSupportedNameTypes);

    absl::optional<der::Input> excluded_subtrees_value;
    if (!sequence_parser.ReadOptionalTag(der::ContextSpecificConstructed(1), &excluded_subtrees_value)) {
        return false;
    }
    if (excluded_subtrees_value && !ParseGeneralSubtrees(excluded_subtrees_value.value(), &excluded_subtrees_, errors)) {
        return false;
    }
    constrained_name_types_ |= excluded_subtrees_.present_name_types & (is_critical ? GENERAL_NAME_ALL_TYPES : kSupportedNameTypes);
...
void NameConstraints::IsPermittedCert(const der::Input& subject_rdn_sequence, const GeneralNames* subject_alt_names, CertErrors* errors) const
{
    ...
    if (subject_alt_names) {
        check_count
            += base::ClampMul(subject_alt_names->dns_names.size(), base::ClampAdd(excluded_subtrees_.dns_names.size(), permitted_subtrees_.dns_names.size()));
        check_count += base::ClampMul(
            subject_alt_names->directory_names.size(), base::ClampAdd(excluded_subtrees_.directory_names.size(), permitted_subtrees_.directory_names.size()));
        check_count += base::ClampMul(
            subject_alt_names->ip_addresses.size(), base::ClampAdd(excluded_subtrees_.ip_address_ranges.size(), permitted_subtrees_.ip_address_ranges.size()));
    }

    if (!(subject_alt_names && subject_rdn_sequence.Length() == 0)) {
        check_count += base::ClampAdd(excluded_subtrees_.directory_names.size(), permitted_subtrees_.directory_names.size());
    }
    ...
    if (subject_alt_names) {
        ...
        // extension of a subsequent certificate, then the application MUST
        // either process the constraint or reject the certificate.
        if (constrained_name_types() & subject_alt_names->present_name_types & ~kSupportedNameTypes) {
    ...
    if (!subject_alt_names && (constrained_name_types() & GENERAL_NAME_RFC822_NAME)) {
        bool contained_email_address = false;
        if (!NameContainsEmailAddress(subject_rdn_sequence, &contained_email_address)) {
            errors->AddError(cert_errors::kNotPermittedByNameConstraints);
            return;
        }
...
bool NameConstraints::IsPermittedDirectoryName(const der::Input& name_rdn_sequence) const
{
