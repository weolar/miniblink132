...
template <typename TestDelegate> class VerifyCertificateChainTest : public ::testing::Test {
public:
    void RunTest(const char* file_name)
    {
        VerifyCertChainTest test;

        std::string path = std::string("net/data/verify_certificate_chain_unittest/") + file_name;
...
template <typename TestDelegate> class VerifyCertificateChainSingleRootTest : public VerifyCertificateChainTest<TestDelegate> { };
...
TYPED_TEST_P(VerifyCertificateChainSingleRootTest, WrongSignature)
{
    ...
    this->RunTest("intermediate-wrong-signature-no-authority-key-identifier/main.test");
}
...
TYPED_TEST_P(VerifyCertificateChainSingleRootTest, KeyUsage)
{
    ...
    this->RunTest("target-serverauth-various-keyusages/rsa-digitalSignature.test");
...
TYPED_TEST_P(VerifyCertificateChainSingleRootTest, ExtendedKeyUsage)
{
    ...
    this->RunTest("intermediate-eku-server-gated-crypto/sha1-eku-clientAuth.test");
    this->RunTest("intermediate-eku-server-gated-crypto/sha1-eku-serverAuth.test");
    this->RunTest("intermediate-eku-server-gated-crypto/sha256-eku-any.test");
    this->RunTest("intermediate-eku-server-gated-crypto/sha256-eku-clientAuth.test");
    this->RunTest("intermediate-eku-server-gated-crypto/sha256-eku-serverAuth.test");
}

TYPED_TEST_P(VerifyCertificateChainSingleRootTest, IssuerAndSubjectNotByteForByteEqual)
{
...
REGISTER_TYPED_TEST_SUITE_P(VerifyCertificateChainSingleRootTest, Simple, BasicConstraintsCa, BasicConstraintsPathlen, UnknownExtension, WeakSignature,
    WrongSignature, LastCertificateNotTrusted, WeakPublicKey, TargetSignedUsingEcdsa, Expired, TargetNotEndEntity, KeyUsage, ExtendedKeyUsage,
    IssuerAndSubjectNotByteForByteEqual, TrustAnchorNotSelfSigned, KeyRollover, Policies, ManyNames);
