...
const uint8_t kOidRsaSsaPss[] = { 0x2a, 0x86, 0x48, 0x86, 0xf7, 0x0d, 0x01, 0x01, 0x0a };
...
const uint8_t kOidMgf1[] = { 0x2a, 0x86, 0x48, 0x86, 0xf7, 0x0d, 0x01, 0x01, 0x08 };
...
[[nodiscard]] bool ParseMaskGenAlgorithm(const der::Input input, DigestAlgorithm* mgf1_hash)
{
    der::Input oid;
    der::Input params;
...
absl::optional<SignatureAlgorithm> ParseRsaPss(const der::Input& params)
{
    ...
    if (!params_parser.ReadTag(der::ContextSpecificConstructed(0), &field) || !ParseHashAlgorithm(field, &hash)
        || !params_parser.ReadTag(der::ContextSpecificConstructed(1), &field) || !ParseMaskGenAlgorithm(field, &mgf1_hash)
        || !params_parser.ReadConstructed(der::ContextSpecificConstructed(2), &salt_length_parser) || !salt_length_parser.ReadUint64(&salt_length)
...
[[nodiscard]] bool ParseAlgorithmIdentifier(const der::Input& input, der::Input* algorithm, der::Input* parameters)
{
    der::Parser parser(input);
...
[[nodiscard]] bool ParseHashAlgorithm(const der::Input& input, DigestAlgorithm* out)
{
    CBS cbs;
...
absl::optional<SignatureAlgorithm> ParseSignatureAlgorithm(const der::Input& algorithm_identifier, CertErrors* errors)
{
    der::Input oid;
    der::Input params;
    ...
    if (errors) {
        errors->AddError(kUnknownSignatureAlgorithm, CreateCertErrorParams2Der("oid", oid, "params", params));
    }
    return absl::nullopt;
}

absl::optional<DigestAlgorithm> GetTlsServerEndpointDigestAlgorithm(SignatureAlgorithm alg)
{
