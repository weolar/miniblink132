...
bool ParseRevokedInfo(const der::Input& raw_tlv, OCSPCertStatus* out)
{
    ...
    if (!parser.ReadOptionalTag(der::ContextSpecificConstructed(0), &reason_input, &(out->has_reason))) {
        return false;
    }
    if (out->has_reason) {
...
bool AppendHashAsOctetString(const EVP_MD* hash_type, CBB* cbb, const der::Input& value)
{
    CBB octet_string;
    unsigned hash_len;
    uint8_t hash_buffer[EVP_MAX_MD_SIZE];

    return CBB_add_asn1(cbb, &octet_string, CBS_ASN1_OCTETSTRING) && EVP_Digest(value.UnsafeData(), value.Length(), hash_buffer, &hash_len, hash_type, nullptr)
...
// }
bool ParseOCSPSingleResponse(const der::Input& raw_tlv, OCSPSingleResponse* out)
{
    der::Parser outer_parser(raw_tlv);
    der::Parser parser;
    if (!outer_parser.ReadSequence(&parser))
    ...
    if (!parser.ReadOptionalTag(der::ContextSpecificConstructed(0), &next_update_input, &(out->has_next_update))) {
        return false;
    }
    ...
    if (!parser.ReadOptionalTag(der::ContextSpecificConstructed(1), &(out->extensions), &(out->has_extensions))) {
        return false;
    }
...
//      byKey                [2] KeyHash
// }
bool ParseResponderID(const der::Input& raw_tlv, OCSPResponseData::ResponderID* out)
{
    der::Parser parser(raw_tlv);
    der::Tag id_tag;
    der::Input id_input;
...
bool ParseOCSPResponseData(const der::Input& raw_tlv, OCSPResponseData* out)
{
    ...
    if (!parser.ReadOptionalTag(der::ContextSpecificConstructed(0), &version_input, &version_present)) {
        return false;
    }
...
bool ParseBasicOCSPResponse(const der::Input& raw_tlv, OCSPResponse* out)
{
    ...
    absl::optional<SignatureAlgorithm> sigalg = ParseSignatureAlgorithm(sigalg_tlv, /*errors=*/nullptr);
    if (!sigalg)
        return false;
    ...
    if (!parser.ReadOptionalTag(der::ContextSpecificConstructed(0), &certs_input, &(out->has_certs))) {
        return false;
    }
...
bool ParseOCSPResponse(const der::Input& raw_tlv, OCSPResponse* out)
{
    ...
    if (out->status == OCSPResponse::ResponseStatus::SUCCESSFUL) {
        ...
        if (!parser.ReadConstructed(der::ContextSpecificConstructed(0), &outer_bytes_parser)) {
            return false;
        }
...
bool VerifyHash(const EVP_MD* type, const der::Input& hash, const der::Input& value)
{
    unsigned value_hash_len;
    uint8_t value_hash[EVP_MAX_MD_SIZE];
    if (!EVP_Digest(value.UnsafeData(), value.Length(), value_hash, &value_hash_len, type, nullptr)) {
        return false;
    }
...
bool GetSubjectPublicKeyBytes(const der::Input& spki_tlv, der::Input* spk_tlv)
{
    ...
    if (!asn1::ExtractSubjectPublicKeyFromSPKI(spki_tlv.AsStringPiece(), &spk_strpiece)) {
        return false;
    }
    ...
    // result.
    if (!net::string_util::StartsWith(std::string_view(spk_strpiece.data(), spk_strpiece.size()), "\0"))
        return false;
...
bool CheckCertIDMatchesCertificate(const OCSPCertID& id, const ParsedCertificate* certificate, const ParsedCertificate* issuer_certificate)
{
...
scoped_refptr<ParsedCertificate> OCSPParseCertificate(std::string_view der)
{
    ...
    // parsing model.
    CertErrors errors;
    return ParsedCertificate::Create(x509_util::CreateCryptoBuffer(base::StringPiece(der.data(), der.size())), {}, &errors);
}
...
[[nodiscard]] bool CheckResponderIDMatchesCertificate(const OCSPResponseData::ResponderID& id, const ParsedCertificate* cert)
{
    switch (id.type) {
    case OCSPResponseData::ResponderType::NAME: {
        der::Input name_rdn;
        der::Input cert_rdn;
        if (!der::Parser(id.name).ReadTag(der::kSequence, &name_rdn) || !der::Parser(cert->tbs().subject_tlv).ReadTag(der::kSequence, &cert_rdn))
            return false;
...
[[nodiscard]] bool VerifyAuthorizedResponderCert(const ParsedCertificate* responder_certificate, const ParsedCertificate* issuer_certificate)
{
    ...
    if (!responder_certificate->signature_algorithm().has_value()
        || !VerifySignedData(*responder_certificate->signature_algorithm(), responder_certificate->tbs_certificate_tlv(),
            responder_certificate->signature_value(), issuer_certificate->tbs().spki_tlv)) {
        return false;
    }
    ...
    return base::Contains(responder_certificate->extended_key_usage(), der::Input(kOCSPSigning));
}

[[nodiscard]] bool VerifyOCSPResponseSignatureGivenCert(const OCSPResponse& response, const ParsedCertificate* cert)
{
    // TODO(eroman): Must check the signature algorithm against policy.
    return VerifySignedData(response.signature_algorithm, response.data, response.signature, cert->tbs().spki_tlv);
}
...
[[nodiscard]] bool VerifyOCSPResponseSignature(const OCSPResponse& response, const OCSPResponseData& response_data, const ParsedCertificate* issuer_certificate)
{
    ...
    if (CheckResponderIDMatchesCertificate(response_data.responder_id, issuer_certificate)
    ...
    for (const auto& responder_cert_tlv : response.certs) {
        scoped_refptr<ParsedCertificate> cur_responder_certificate = OCSPParseCertificate(responder_cert_tlv.AsStringView());
        ...
        if (!CheckResponderIDMatchesCertificate(response_data.responder_id, cur_responder_certificate.get())) {
            continue;
        }

        // If the certificate isn't a valid Authorized Responder certificate, keep
        // looking.
        if (!VerifyAuthorizedResponderCert(cur_responder_certificate.get(), issuer_certificate)) {
            continue;
        }

        // If the certificate signed this OCSP response, have found a match.
        // Otherwise keep looking.
        if (VerifyOCSPResponseSignatureGivenCert(response, cur_responder_certificate.get())) {
            return true;
        }
    }
...
bool ParseOCSPResponseDataExtensions(const der::Input& response_extensions, OCSPVerifyResult::ResponseStatus* response_details)
{
...
// the flag setting.
bool ParseOCSPSingleResponseExtensions(const der::Input& single_extensions, OCSPVerifyResult::ResponseStatus* response_details)
{
    std::map<der::Input, ParsedExtension> extensions;
    ...
    const uint8_t ct_ocsp_ext_oid[] = { 0x2B, 0x06, 0x01, 0x04, 0x01, 0xD6, 0x79, 0x02, 0x04, 0x05 };
...
OCSPRevocationStatus GetRevocationStatusForCert(const OCSPResponseData& response_data, const ParsedCertificate* cert,
    const ParsedCertificate* issuer_certificate, const base::Time& verify_time, const base::TimeDelta& max_age,
    OCSPVerifyResult::ResponseStatus* response_details)
{
    ...
    *response_details = OCSPVerifyResult::NO_MATCHING_RESPONSE;
    ...
    for (const auto& single_response_der : response_data.responses) {
        ...
        if (single_response.has_extensions && !ParseOCSPSingleResponseExtensions(single_response.extensions, response_details)) {
            return OCSPRevocationStatus::UNKNOWN;
        }
        ...
        if (!CheckRevocationDateValid(
                single_response.this_update, single_response.has_next_update ? &single_response.next_update : nullptr, verify_time, max_age)) {
        ...
        if (*response_details != OCSPVerifyResult::PROVIDED || result == OCSPRevocationStatus::GOOD
...
OCSPRevocationStatus CheckOCSP(std::string_view raw_response, std::string_view certificate_der, const ParsedCertificate* certificate,
    std::string_view issuer_certificate_der, const ParsedCertificate* issuer_certificate, const base::Time& verify_time, const base::TimeDelta& max_age,
    OCSPVerifyResult::ResponseStatus* response_details)
{
    *response_details = OCSPVerifyResult::NOT_CHECKED;
    ...
    if (response_data.has_extensions && !ParseOCSPResponseDataExtensions(response_data.extensions, response_details)) {
        return OCSPRevocationStatus::UNKNOWN;
    }
    ...
    // response.
    if (response_data.produced_at < certificate->tbs().validity_not_before || response_data.produced_at > certificate->tbs().validity_not_after) {
    ...
    // and time).
    OCSPRevocationStatus status = GetRevocationStatusForCert(response_data, certificate, issuer_certificate, verify_time, max_age, response_details);
...
OCSPRevocationStatus CheckOCSP(std::string_view raw_response, std::string_view certificate_der, std::string_view issuer_certificate_der,
    const base::Time& verify_time, const base::TimeDelta& max_age, OCSPVerifyResult::ResponseStatus* response_details)
{
    return CheckOCSP(raw_response, certificate_der, nullptr, issuer_certificate_der, nullptr, verify_time, max_age, response_details);
}

OCSPRevocationStatus CheckOCSP(std::string_view raw_response, const ParsedCertificate* certificate, const ParsedCertificate* issuer_certificate,
    const base::Time& verify_time, const base::TimeDelta& max_age, OCSPVerifyResult::ResponseStatus* response_details)
{
    return CheckOCSP(raw_response, std::string_view(), certificate, std::string_view(), issuer_certificate, verify_time, max_age, response_details);
}

bool CreateOCSPRequest(const ParsedCertificate* cert, const ParsedCertificate* issuer, std::vector<uint8_t>* request_der)
{
    request_der->clear();
    ...
    if (!CBB_add_asn1(&req_cert, &serial_number, CBS_ASN1_INTEGER))
        return false;
    if (!CBB_add_bytes(&serial_number, cert->tbs().serial_number.UnsafeData(), cert->tbs().serial_number.Length())) {
        return false;
    }
...
//    the OCSPRequest}
GURL CreateOCSPGetURL(const ParsedCertificate* cert, const ParsedCertificate* issuer, std::string_view ocsp_responder_url)
{
    ...
    len = EVP_EncodeBlock(encoded.data(), ocsp_request_der.data(), ocsp_request_der.size());
