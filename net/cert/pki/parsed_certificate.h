...
class NET_EXPORT ParsedCertificate : public base::RefCountedThreadSafe<ParsedCertificate> {
public:
    ...
    static scoped_refptr<ParsedCertificate> Create(bssl::UniquePtr<CRYPTO_BUFFER> cert_data, const ParseCertificateOptions& options, CertErrors* errors);
    ...
    // On either success or failure, if |errors| is non-null it may have error
    // information added to it.
    static bool CreateAndAddToVector(bssl::UniquePtr<CRYPTO_BUFFER> cert_data, const ParseCertificateOptions& options,
    ...
    const der::Input& der_cert() const
    {
        return cert_;
    }
    ...
    {
        return cert_data_.get();
    }

    // Accessors for raw fields of the Certificate.
    const der::Input& tbs_certificate_tlv() const
    {
        return tbs_certificate_tlv_;
    }
    ...
    der::Input subject_tlv() const
    {
        return tbs_.subject_tlv;
    }
    ...
    // tag). This is guaranteed to be valid DER, though the contents of unhandled
    // string types are treated as raw bytes.
    der::Input issuer_tlv() const
    {
        return tbs_.issuer_tlv;
    }
    ...
    bool has_basic_constraints() const
    {
        return has_basic_constraints_;
    }
    ...
    bool has_extended_key_usage() const
    {
        return has_extended_key_usage_;
    }
    ...
    bool has_subject_alt_names() const
    {
        return subject_alt_names_ != nullptr;
    }
    ...
    bool has_name_constraints() const
    {
        return name_constraints_ != nullptr;
    }
    ...
    bool has_authority_info_access() const
    {
        return has_authority_info_access_;
    }
    ...
    const std::vector<std::string_view>& ocsp_uris() const
    {
        return ocsp_uris_;
    }

    // Returns true if the certificate has a Policies extension.
    bool has_policy_oids() const
    {
        return has_policy_oids_;
    }
    ...
    bool has_policy_constraints() const
    {
        return has_policy_constraints_;
    }
    ...
    bool has_policy_mappings() const
    {
        return has_policy_mappings_;
    }
    ...
    bool has_inhibit_any_policy() const
    {
        return has_inhibit_any_policy_;
    }
    ...
    // one.
    const absl::optional<ParsedAuthorityKeyIdentifier>& authority_key_identifier() const
    {
    ...
    const ExtensionsMap& extensions() const
    {
        return extensions_;
    }
    ...
    bool GetExtension(const der::Input& extension_oid, ParsedExtension* parsed_extension) const;
