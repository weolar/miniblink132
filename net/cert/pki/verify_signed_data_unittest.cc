...
void RunTestCase(VerifyResult expected_result, const char* file_name)
{
    ...
    absl::optional<SignatureAlgorithm> signature_algorithm = ParseSignatureAlgorithm(der::Input(&algorithm), &algorithm_errors);
    ...
    absl::optional<der::BitString> signature_value_bit_string = signature_value_parser.ReadBitString();
    ASSERT_TRUE(signature_value_bit_string.has_value()) << "The signature value is not a valid BIT STRING";

    bool expected_result_bool = expected_result == SUCCESS;

    bool result = VerifySignedData(*signature_algorithm, der::Input(&signed_data), signature_value_bit_string.value(), der::Input(&public_key));
