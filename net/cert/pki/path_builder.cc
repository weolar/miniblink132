...
std::string FingerPrintParsedCertificate(const net::ParsedCertificate* cert)
{
    ...
    SHA256(cert->der_cert().AsSpan().data(), cert->der_cert().AsSpan().size(), digest);
...
std::string CertDebugString(const ParsedCertificate* cert)
{
    ...
    if (!ParseName(cert->tbs().subject_tlv, &subject) || !ConvertToRFC2253(subject, &subject_str))
        subject_str = "???";
...
{
    base::UmaHistogramCounts10000("Net.CertVerifier.PathBuilderIterationCount", iteration_count);
}
...
KeyIdentifierMatch CalculateKeyIdentifierMatch(const ParsedCertificate* target, const ParsedCertificate* issuer)
{
    ...
    if (target->authority_key_identifier()->key_identifier && issuer->subject_key_identifier()) {
        if (target->authority_key_identifier()->key_identifier != issuer->subject_key_identifier().value()) {
            return kMismatch;
        }
        return kMatch;
    }
...
int TrustAndKeyIdentifierMatchToOrder(const ParsedCertificate* target, const ParsedCertificate* issuer, const CertificateTrust& issuer_trust)
{
    enum {
...
public:
    ...
    CertIssuersIter(
        scoped_refptr<ParsedCertificate> cert, CertIssuerSources* cert_issuer_sources, const TrustStore* trust_store, base::SupportsUserData* debug_data);
    ...
    void increment_skipped_issuer_count()
    {
        skipped_issuer_count_++;
    }
    ...
    scoped_refptr<ParsedCertificate> reference_cert() const
    {
        return cert_;
    }
...
private:
    ...
    bool HasCurrentIssuer() const
    {
        return cur_issuer_ < issuers_.size();
    }
    ...
    std::vector<std::unique_ptr<CertIssuerSource::Request>> pending_async_requests_;

    raw_ptr<base::SupportsUserData> debug_data_;
};

CertIssuersIter::CertIssuersIter(
    scoped_refptr<ParsedCertificate> in_cert, CertIssuerSources* cert_issuer_sources, const TrustStore* trust_store, base::SupportsUserData* debug_data)
    : cert_(in_cert)
...
void CertIssuersIter::GetNextIssuer(IssuerEntry* out)
{
    ...
    if (!HasCurrentIssuer()) {
        ...
        while (!HasCurrentIssuer() && cur_async_request_ < pending_async_requests_.size()) {
            ParsedCertificateList new_issuers;
    ...
    if (HasCurrentIssuer()) {
        ...
        DVLOG(2) << "CertIssuersIter returning issuer " << cur_issuer_ << " of " << issuers_.size() << " for " << CertDebugString(cert());
    ...
    DVLOG(2) << "CertIssuersIter reached the end of all available issuers for " << CertDebugString(cert());
...
void CertIssuersIter::AddIssuers(ParsedCertificateList new_issuers)
{
    for (scoped_refptr<ParsedCertificate>& issuer : new_issuers) {
        if (present_issuers_.find(issuer->der_cert().AsStringView()) != present_issuers_.end())
            continue;
        ...
        entry.trust_and_key_id_match_ordering = TrustAndKeyIdentifierMatchToOrder(cert(), entry.cert.get(), entry.trust);
...
void CertIssuersIter::SortRemainingIssuers()
{
    ...
    std::stable_sort(issuers_.begin() + cur_issuer_, issuers_.end(), [](const IssuerEntry& issuer1, const IssuerEntry& issuer2) {
        // TODO(crbug.com/635205): Add other prioritization hints. (See big list
        // of possible sorting hints in RFC 4158.)
        const bool issuer1_self_issued = issuer1.cert->normalized_subject() == issuer1.cert->normalized_issuer();
        const bool issuer2_self_issued = issuer2.cert->normalized_subject() == issuer2.cert->normalized_issuer();
        return std::tie(issuer1.trust_and_key_id_match_ordering, issuer2_self_issued,
                   // Newer(larger) notBefore & notAfter dates are
                   // preferred, hence |issuer2| is on the LHS of
                   // the comparison and |issuer1| on the RHS.
                   issuer2.cert->tbs().validity_not_before, issuer2.cert->tbs().validity_not_after)
            < std::tie(
                issuer2.trust_and_key_id_match_ordering, issuer1_self_issued, issuer1.cert->tbs().validity_not_before, issuer1.cert->tbs().validity_not_after);
    });
...
public:
    ...
    {
        bool added = present_certs_.insert(GetKey(cert_issuers_iter->cert())).second;
        DCHECK(added);
    ...
    CertIssuersIter* back()
    {
        return cur_path_.back().get();
    }

    // Returns the length of the path.
    size_t Length() const
    {
        return cur_path_.size();
    }
...
private:
    ...
    static Key GetKey(const ParsedCertificate* cert)
    {
        ...
        return Key(cert->normalized_subject().AsStringView(), cert->subject_alt_names_extension().value.AsStringView(), cert->tbs().spki_tlv.AsStringView());
    }
...
class CertPathIter {
public:
    CertPathIter(scoped_refptr<ParsedCertificate> cert, const TrustStore* trust_store, base::SupportsUserData* debug_data);
    ...
    bool GetNextPath(ParsedCertificateList* out_certs, CertificateTrust* out_last_cert_trust, CertPathErrors* out_errors, const base::TimeTicks deadline,
        uint32_t* iteration_count, const uint32_t max_iteration_count, const uint32_t max_path_building_depth);
...
CertPathIter::CertPathIter(scoped_refptr<ParsedCertificate> cert, const TrustStore* trust_store, base::SupportsUserData* debug_data)
    : trust_store_(trust_store)
    , debug_data_(debug_data)
{
    // Initialize |next_issuer_| to the target certificate.
    next_issuer_.cert = std::move(cert);
    next_issuer_.trust = trust_store_->GetTrust(next_issuer_.cert.get(), debug_data_);
}
...
bool CertPathIter::GetNextPath(ParsedCertificateList* out_certs, CertificateTrust* out_last_cert_trust, CertPathErrors* out_errors,
    const base::TimeTicks deadline, uint32_t* iteration_count, const uint32_t max_iteration_count, const uint32_t max_path_building_depth)
{
    out_certs->clear();
    *out_last_cert_trust = CertificateTrust::ForUnspecified();
    ...
    while (true) {
        ...
        if (max_path_building_depth > 0 && cur_path_.Length() >= max_path_building_depth) {
            cur_path_.CopyPath(out_certs);
        ...
        if (!next_issuer_.cert) {
            ...
            if (max_iteration_count > 0 && *iteration_count > max_iteration_count) {
                cur_path_.CopyPath(out_certs);
                out_errors->GetOtherErrors()->AddError(cert_errors::kIterationLimitExceeded);
                return false;
            }
            ...
            if (!next_issuer_.cert) {
                if (!cur_path_.back()->had_non_skipped_issuers()) {
                    ...
                    cur_path_.CopyPath(out_certs);
                    out_errors->GetErrorsForCert(out_certs->size() - 1)->AddError(cert_errors::kNoIssuersFound);
        ...
        case CertificateTrustType::TRUSTED_ANCHOR_WITH_CONSTRAINTS: {
            ...
            DVLOG(2) << "CertPathIter got anchor: " << CertDebugString(next_issuer_.cert.get());
            cur_path_.CopyPath(out_certs);
        ...
        case CertificateTrustType::UNSPECIFIED: {
            ...
            if (cur_path_.IsPresent(next_issuer_.cert.get())) {
                ...
                DVLOG(1) << "CertPathIter skipping dupe cert: " << CertDebugString(next_issuer_.cert.get());
                next_issuer_ = IssuerEntry();
                continue;
            }

            cur_path_.Append(std::make_unique<CertIssuersIter>(std::move(next_issuer_.cert), &cert_issuer_sources_, trust_store_, debug_data_));
            next_issuer_ = IssuerEntry();
...
const CertPathBuilderResultPath* CertPathBuilder::Result::GetBestValidPath() const
{
...
const CertPathBuilderResultPath* CertPathBuilder::Result::GetBestPathPossiblyInvalid() const
{
    DCHECK((paths.empty() && best_result_index == 0) || best_result_index < paths.size());
...
CertPathBuilder::CertPathBuilder(scoped_refptr<ParsedCertificate> cert, TrustStore* trust_store, CertPathBuilderDelegate* delegate,
    const der::GeneralizedTime& time, KeyPurpose key_purpose, InitialExplicitPolicy initial_explicit_policy,
    const std::set<der::Input>& user_initial_policy_set, InitialPolicyMappingInhibit initial_policy_mapping_inhibit,
    InitialAnyPolicyInhibit initial_any_policy_inhibit)
    : cert_path_iter_(std::make_unique<CertPathIter>(std::move(cert), trust_store,
...
void CertPathBuilder::AddCertIssuerSource(CertIssuerSource* cert_issuer_source)
{
...
CertPathBuilder::Result CertPathBuilder::Run()
{
    ...
    while (true) {
        std::unique_ptr<CertPathBuilderResultPath> result_path = std::make_unique<CertPathBuilderResultPath>();

        if (!cert_path_iter_->GetNextPath(&result_path->certs, &result_path->last_cert_trust, &result_path->errors, deadline_, &iteration_count,
                max_iteration_count_, max_path_building_depth_)) {
            // There are no more paths to check or limits were exceeded.
            if (result_path->errors.ContainsError(cert_errors::kIterationLimitExceeded)) {
            ...
            if (!result_path->certs.empty()) {
                ...
                if (!result_path->errors.ContainsHighSeverityErrors()) {
                    result_path->errors.GetOtherErrors()->AddError(cert_errors::kInternalError);
                }
        ...
        if (result_path->last_cert_trust.HasUnspecifiedTrust()) {
            ...
            VerifyCertificateChain(result_path->certs, result_path->last_cert_trust, delegate_, time_, key_purpose_, initial_explicit_policy_,
                user_initial_policy_set_, initial_policy_mapping_inhibit_, initial_any_policy_inhibit_, &result_path->user_constrained_policy_set,
                &result_path->errors);
        }
...
void CertPathBuilder::AddResultPath(std::unique_ptr<CertPathBuilderResultPath> result_path)
{
    ...
    if (!out_result_.HasValidPath()) {
        const CertPathBuilderResultPath* old_best_path = out_result_.GetBestPathPossiblyInvalid();
        ...
        if (result_path->IsValid()
            || (!result_path->last_cert_trust.HasUnspecifiedTrust() && old_best_path && old_best_path->last_cert_trust.HasUnspecifiedTrust())) {
