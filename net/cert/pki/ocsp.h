...
inline constexpr uint8_t kBasicOCSPResponseOid[] = { 0x2b, 0x06, 0x01, 0x05, 0x05, 0x07, 0x30, 0x01, 0x01 };
...
NET_EXPORT_PRIVATE bool ParseOCSPCertID(const der::Input& raw_tlv, OCSPCertID* out);
...
// On failure |out| has an undefined state. Some of its fields may have been
// updated during parsing, whereas others may not have been changed.
NET_EXPORT_PRIVATE bool ParseOCSPSingleResponse(const der::Input& raw_tlv, OCSPSingleResponse* out);
...
// On failure |out| has an undefined state. Some of its fields may have been
// updated during parsing, whereas others may not have been changed.
NET_EXPORT_PRIVATE bool ParseOCSPResponseData(const der::Input& raw_tlv, OCSPResponseData* out);
...
// On failure |out| has an undefined state. Some of its fields may have been
// updated during parsing, whereas others may not have been changed.
NET_EXPORT_PRIVATE bool ParseOCSPResponse(const der::Input& raw_tlv, OCSPResponse* out);
...
[[nodiscard]] NET_EXPORT OCSPRevocationStatus CheckOCSP(std::string_view raw_response, std::string_view certificate_der,
    std::string_view issuer_certificate_der, const base::Time& verify_time, const base::TimeDelta& max_age, OCSPVerifyResult::ResponseStatus* response_details);
...
[[nodiscard]] NET_EXPORT OCSPRevocationStatus CheckOCSP(std::string_view raw_response, const ParsedCertificate* certificate,
...
NET_EXPORT bool CreateOCSPRequest(const ParsedCertificate* cert, const ParsedCertificate* issuer, std::vector<uint8_t>* request_der);

// Creates a URL to issue a GET request for OCSP information for |cert|.
NET_EXPORT GURL CreateOCSPGetURL(const ParsedCertificate* cert, const ParsedCertificate* issuer, std::string_view ocsp_responder_url);
