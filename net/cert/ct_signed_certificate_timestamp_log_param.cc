...
// description |key|.
void SetBinaryData(const char* key, std::string_view value, base::Value::Dict& dict)
{
...
base::Value SCTToDictionary(const ct::SignedCertificateTimestamp& sct, ct::SCTVerifyStatus status)
{
    ...
    dict.Set("timestamp", base::NumberToString(time_since_unix_epoch.InMilliseconds()));
    SetBinaryData("extensions", sct.extensions, dict);

    dict.Set("hash_algorithm", HashAlgorithmToString(sct.signature.hash_algorithm));
    dict.Set("signature_algorithm", SignatureAlgorithmToString(sct.signature.signature_algorithm));
...
base::Value::List SCTListToPrintableValues(const SignedCertificateTimestampAndStatusList& sct_and_status_list)
{
...
base::Value::Dict NetLogSignedCertificateTimestampParams(const SignedCertificateTimestampAndStatusList* scts)
{
    base::Value::Dict dict;
...
base::Value::Dict NetLogRawSignedCertificateTimestampParams(
    std::string_view embedded_scts, std::string_view sct_list_from_ocsp, std::string_view sct_list_from_tls_extension)
{
    base::Value::Dict dict;
