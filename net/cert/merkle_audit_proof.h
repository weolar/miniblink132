...
NET_EXPORT uint64_t CalculateAuditPathLength(uint64_t leaf_index, uint64_t tree_size);
...
struct NET_EXPORT MerkleAuditProof {
    ...
    MerkleAuditProof(uint64_t leaf_index, uint64_t tree_size, const std::vector<std::string>& audit_path);
    ~MerkleAuditProof();
