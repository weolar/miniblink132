...
public:
    ...
    // certificates.
    static scoped_refptr<X509Certificate> CreateFromDERCertChain(const std::vector<std::string_view>& der_certs);

    // Create an X509Certificate from a chain of DER encoded certificates with
    // non-standard parsing options.
    // Do not use without consulting //net owners.
    static scoped_refptr<X509Certificate> CreateFromDERCertChainUnsafeOptions(const std::vector<std::string_view>& der_certs, UnsafeCreateOptions options);

    // Create an X509Certificate from the DER-encoded representation.
    // Returns NULL on failure.
    static scoped_refptr<X509Certificate> CreateFromBytes(base::span<const uint8_t> data);

    // Create an X509Certificate with non-standard parsing options.
    // Do not use without consulting //net owners.
    static scoped_refptr<X509Certificate> CreateFromBytesUnsafeOptions(base::span<const uint8_t> data, UnsafeCreateOptions options);

    // Create an X509Certificate from the representation stored in the given
    // pickle.  The data for this object is found relative to the given
    // pickle_iter, which should be passed to the pickle's various Read* methods.
    // Returns NULL on failure.
    static scoped_refptr<X509Certificate> CreateFromPickle(base::PickleIterator* pickle_iter);

    // Create an X509Certificate from the representation stored in the given
    // pickle with non-standard parsing options.
    // Do not use without consulting //net owners.
    static scoped_refptr<X509Certificate> CreateFromPickleUnsafeOptions(base::PickleIterator* pickle_iter, UnsafeCreateOptions options);
    ...
    static CertificateList CreateCertificateListFromBytes(base::span<const uint8_t> data, int format);
    ...
    scoped_refptr<X509Certificate> CloneWithDifferentIntermediates(std::vector<bssl::UniquePtr<CRYPTO_BUFFER>> intermediates);
    ...
    const CertPrincipal& subject() const
    {
        return parsed_.subject_;
    }

    // The issuer of the certificate.
    const CertPrincipal& issuer() const
    {
        return parsed_.issuer_;
    }
    ...
    const base::Time& valid_start() const
    {
        return parsed_.valid_start_;
    }
    const base::Time& valid_expiry() const
    {
        return parsed_.valid_expiry_;
    }
    ...
    bool GetSubjectAltName(std::vector<std::string>* dns_names, std::vector<std::string>* ip_addrs) const;
    ...
    // |pem_encoded|.
    static bool GetPEMEncodedFromDER(std::string_view der_encoded, std::string* pem_encoded);
    ...
    static bool GetPEMEncoded(const CRYPTO_BUFFER* cert_buffer, std::string* pem_encoded);
    ...
    static void GetPublicKeyInfo(const CRYPTO_BUFFER* cert_buffer, size_t* size_bits, PublicKeyType* type);
    ...
    CRYPTO_BUFFER* cert_buffer() const
    {
        return cert_buffer_.get();
    }
    ...
    const std::vector<bssl::UniquePtr<CRYPTO_BUFFER>>& intermediate_buffers() const
    {
    ...
    static std::vector<bssl::UniquePtr<CRYPTO_BUFFER>> CreateCertBuffersFromBytes(base::span<const uint8_t> data, Format format);

    // Calculates the SHA-256 fingerprint of the certificate.  Returns an empty
    // (all zero) fingerprint on failure.
    static SHA256HashValue CalculateFingerprint256(const CRYPTO_BUFFER* cert_buffer);
...
private:
    ...
    public:
        ...
        bool Initialize(const CRYPTO_BUFFER* cert_buffer, UnsafeCreateOptions options);
    ...
    X509Certificate(ParsedFields parsed, bssl::UniquePtr<CRYPTO_BUFFER> cert_buffer, std::vector<bssl::UniquePtr<CRYPTO_BUFFER>> intermediates);

    // Copy |other|, except with a different set of intermediates.
    X509Certificate(const X509Certificate& other, std::vector<bssl::UniquePtr<CRYPTO_BUFFER>> intermediates);
