...
public:
    ...
    void AddResultForCert(scoped_refptr<X509Certificate> cert, const CertVerifyResult& verify_result, int rv);

    // Same as AddResultForCert(), but further restricts it to only return for
    // hostnames that match |host_pattern|.
    void AddResultForCertAndHost(scoped_refptr<X509Certificate> cert, const std::string& host_pattern, const CertVerifyResult& verify_result, int rv);
...
public:
    ...
    const std::vector<RequestParams>& GetVerifyParams() const
    {
        return params_;
    }
