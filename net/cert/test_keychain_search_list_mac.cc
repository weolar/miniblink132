...
TestKeychainSearchList::TestKeychainSearchList()
{
    ...
    scoped_keychain_search_list.reset(CFArrayCreateMutable(kCFAllocatorDefault, 0, &kCFTypeArrayCallBacks));
}
...
OSStatus TestKeychainSearchList::CopySearchList(CFArrayRef* keychain_search_list) const
{
    *keychain_search_list = CFArrayCreateCopy(kCFAllocatorDefault, scoped_keychain_search_list.get());
