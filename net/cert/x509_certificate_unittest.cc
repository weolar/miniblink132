...
// Google's cert.
SHA256HashValue google_fingerprint = { { 0x21, 0xaf, 0x58, 0x74, 0xea, 0x6b, 0xad, 0xbd, 0xe4, 0xb3, 0xb1, 0xaa, 0x53, 0x32, 0x80, 0x8f, 0xbf, 0x8a, 0x24, 0x7d,
    0x98, 0xec, 0x7f, 0x77, 0x49, 0x38, 0x42, 0x81, 0x26, 0x7f, 0xed, 0x38 } };
...
SHA256HashValue google_parse_fingerprint = { { 0xf6, 0x41, 0xc3, 0x6c, 0xfe, 0xf4, 0x9b, 0xc0, 0x71, 0x35, 0x9e, 0xcf, 0x88, 0xee, 0xd9, 0x31, 0x7b, 0x73, 0x8b,
    0x59, 0x89, 0x41, 0x6a, 0xd4, 0x01, 0x72, 0x0c, 0x0a, 0x4e, 0x2e, 0x63, 0x52 } };

// The fingerprint for the Thawte SGC certificate
SHA256HashValue thawte_parse_fingerprint = { { 0x10, 0x85, 0xa6, 0xf4, 0x54, 0xd0, 0xc9, 0x11, 0x98, 0xfd, 0xda, 0xb1, 0x1a, 0x31, 0xc7, 0x16, 0xd5, 0xdc, 0xd6,
    0x8d, 0xf9, 0x1c, 0x03, 0x9c, 0xe1, 0x8d, 0xca, 0x9b, 0xeb, 0x3c, 0xde, 0x3d } };
...
void CheckGoogleCert(const scoped_refptr<X509Certificate>& google_cert, const SHA256HashValue& expected_fingerprint, double valid_from, double valid_to)
{
    ...
    EXPECT_EQ(expected_fingerprint, X509Certificate::CalculateFingerprint256(google_cert->cert_buffer()));
}

void ExpectX509CertificateMembersEqual(const scoped_refptr<X509Certificate>& a, const scoped_refptr<X509Certificate>& b)
{
...
{
    scoped_refptr<X509Certificate> google_cert(X509Certificate::CreateFromBytes(google_der));
...
{
    scoped_refptr<X509Certificate> webkit_cert(X509Certificate::CreateFromBytes(webkit_der));
...
{
    scoped_refptr<X509Certificate> thawte_cert(X509Certificate::CreateFromBytes(thawte_der));
...
TEST(X509CertificateTest, InvalidPrintableStringIsUtf8)
{
    ...
    ASSERT_TRUE(base::ReadFileToString(certs_dir.AppendASCII("subject_printable_string_containing_utf8_client_cert.pem"), &file_data));
    ...
    bssl::UniquePtr<CRYPTO_BUFFER> cert_handle = x509_util::CreateCryptoBuffer(cert_der);
    ASSERT_TRUE(cert_handle);

    EXPECT_FALSE(X509Certificate::CreateFromBuffer(bssl::UpRef(cert_handle.get()), {}));

    X509Certificate::UnsafeCreateOptions options;
    options.printable_string_is_utf8 = true;
    scoped_refptr<X509Certificate> cert = X509Certificate::CreateFromBufferUnsafeOptions(bssl::UpRef(cert_handle.get()), {}, options);
...
TEST(X509CertificateTest, TeletexStringIsLatin1)
{
    ...
    scoped_refptr<X509Certificate> cert = ImportCertFromFile(certs_dir, "subject_t61string.pem");
    ASSERT_TRUE(cert);
...
TEST(X509CertificateTest, TeletexStringControlChars)
{
    ...
    scoped_refptr<X509Certificate> cert = ImportCertFromFile(certs_dir, "subject_t61string_1-32.pem");
    ASSERT_TRUE(cert);

    const CertPrincipal& subject = cert->subject();
    EXPECT_EQ("\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0a\x0b\x0c\x0d\x0e\x0f\x10\x11\x12"
...
TEST(X509CertificateTest, TeletexStringIsLatin1NotCp1252)
{
    ...
    scoped_refptr<X509Certificate> cert = ImportCertFromFile(certs_dir, "subject_t61string_126-160.pem");
    ASSERT_TRUE(cert);

    const CertPrincipal& subject = cert->subject();
    // TeletexString is decoded as latin1, so 127-160 get decoded to equivalent
    // unicode control chars.
    EXPECT_EQ("~\x7F\xC2\x80\xC2\x81\xC2\x82\xC2\x83\xC2\x84\xC2\x85\xC2\x86\xC2\x87"
...
TEST(X509CertificateTest, TeletexStringIsNotARealT61String)
{
    ...
    scoped_refptr<X509Certificate> cert = ImportCertFromFile(certs_dir, "subject_t61string_actual.pem");
    ASSERT_TRUE(cert);
...
{
    base::FilePath certs_dir = GetTestNetDataDirectory().AppendASCII("parse_certificate_unittest");
    scoped_refptr<X509Certificate> cert = ImportCertFromFile(certs_dir, "serial_zero_padded.pem");
    ASSERT_TRUE(cert);
...
{
    base::FilePath certs_dir = GetTestNetDataDirectory().AppendASCII("parse_certificate_unittest");
    scoped_refptr<X509Certificate> cert = ImportCertFromFile(certs_dir, "serial_zero_padded_21_bytes.pem");
    ASSERT_TRUE(cert);
    ...
    static const uint8_t expected_serial[21]
        = { 0x00, 0x80, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10, 0x11, 0x12, 0x13 };
    EXPECT_EQ(cert->serial_number(), base::as_string_view(expected_serial));
}
...
{
    base::FilePath certs_dir = GetTestNetDataDirectory().AppendASCII("parse_certificate_unittest");
    scoped_refptr<X509Certificate> cert = ImportCertFromFile(certs_dir, "serial_negative.pem");
    ASSERT_TRUE(cert);
...
{
    base::FilePath certs_dir = GetTestNetDataDirectory().AppendASCII("parse_certificate_unittest");
    scoped_refptr<X509Certificate> cert = ImportCertFromFile(certs_dir, "serial_37_bytes.pem");
    ASSERT_TRUE(cert);

    // Check a serial number which is very long. This should be an error, but
    // serial number parsing is currently permissive.
    static const uint8_t expected_serial[37] = { 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10, 0x11, 0x12,
...
TEST(X509CertificateTest, SHA256FingerprintsCorrectly)
{
    ...
    const SHA256HashValue google_sha256_fingerprint = { { 0x21, 0xaf, 0x58, 0x74, 0xea, 0x6b, 0xad, 0xbd, 0xe4, 0xb3, 0xb1, 0xaa, 0x53, 0x32, 0x80, 0x8f, 0xbf,
        0x8a, 0x24, 0x7d, 0x98, 0xec, 0x7f, 0x77, 0x49, 0x38, 0x42, 0x81, 0x26, 0x7f, 0xed, 0x38 } };

    EXPECT_EQ(google_sha256_fingerprint, X509Certificate::CalculateFingerprint256(google_cert->cert_buffer()));
}
...
TEST(X509CertificateTest, CAFingerprints)
{
    ...
    scoped_refptr<X509Certificate> cert_chain1 = X509Certificate::CreateFromBuffer(bssl::UpRef(server_cert->cert_buffer()), std::move(intermediates));
    ASSERT_TRUE(cert_chain1);

    intermediates.clear();
    intermediates.push_back(bssl::UpRef(intermediate_cert2->cert_buffer()));
    scoped_refptr<X509Certificate> cert_chain2 = X509Certificate::CreateFromBuffer(bssl::UpRef(server_cert->cert_buffer()), std::move(intermediates));
    ASSERT_TRUE(cert_chain2);

    // No intermediate CA certicates.
    intermediates.clear();
    scoped_refptr<X509Certificate> cert_chain3 = X509Certificate::CreateFromBuffer(bssl::UpRef(server_cert->cert_buffer()), std::move(intermediates));
    ASSERT_TRUE(cert_chain3);

    SHA256HashValue cert_chain1_chain_fingerprint_256 = { { 0xac, 0xff, 0xcc, 0x63, 0x0d, 0xd0, 0xa7, 0x19, 0x78, 0xb5, 0x8a, 0x47, 0x8b, 0x67, 0x97, 0xcb,
        0x8d, 0xe1, 0x6a, 0x8a, 0x57, 0x70, 0xda, 0x9a, 0x53, 0x72, 0xe2, 0xa0, 0x08, 0xab, 0xcc, 0x8f } };
    SHA256HashValue cert_chain2_chain_fingerprint_256 = { { 0x67, 0x3a, 0x11, 0x20, 0xd6, 0x94, 0x14, 0xe4, 0x16, 0x9f, 0x58, 0xe2, 0x8b, 0xf7, 0x27, 0xed,
        0xbb, 0xe8, 0xa7, 0xff, 0x1c, 0x8c, 0x0f, 0x21, 0x38, 0x16, 0x7c, 0xad, 0x1f, 0x22, 0x6f, 0x9b } };
    SHA256HashValue cert_chain3_chain_fingerprint_256 = { { 0x16, 0x7a, 0xbd, 0xb4, 0x57, 0x04, 0x65, 0x3c, 0x3b, 0xef, 0x6e, 0x6a, 0xa6, 0x02, 0x73, 0x30,
        0x3e, 0x34, 0x1b, 0x43, 0xc2, 0x7c, 0x98, 0x52, 0x9f, 0x34, 0x7f, 0x55, 0x97, 0xe9, 0x1a, 0x10 } };
    EXPECT_EQ(cert_chain1_chain_fingerprint_256, cert_chain1->CalculateChainFingerprint256());
    EXPECT_EQ(cert_chain2_chain_fingerprint_256, cert_chain2->CalculateChainFingerprint256());
    EXPECT_EQ(cert_chain3_chain_fingerprint_256, cert_chain3->CalculateChainFingerprint256());
}
...
TEST(X509CertificateTest, ParseSubjectAltNames)
{
    ...
    static const uint8_t kIPv4Address[] = { 0x7F, 0x00, 0x00, 0x02 };
    EXPECT_EQ(ip_addresses[0], base::as_string_view(kIPv4Address));

    static const uint8_t kIPv6Address[] = { 0xFE, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01 };
    ...
    scoped_refptr<X509Certificate> no_san_cert = ImportCertFromFile(certs_dir, "salesforce_com_test.pem");
...
TEST(X509CertificateTest, ExtractSPKIFromDERCert)
{
    ...
    EXPECT_TRUE(asn1::ExtractSPKIFromDERCert(base::as_string_view(cert->cert_span()), &spkiBytes));
...
{
    base::FilePath certs_dir = GetTestCertsDirectory();
    scoped_refptr<X509Certificate> cert = ImportCertFromFile(certs_dir, "can_sign_http_exchanges_draft_extension.pem");
    ASSERT_NE(static_cast<X509Certificate*>(nullptr), cert.get());

    EXPECT_TRUE(asn1::HasCanSignHttpExchangesDraftExtension(x509_util::CryptoBufferAsStringPiece(cert->cert_buffer())));
}

TEST(X509CertificateTest, HasCanSignHttpExchangesDraftExtensionInvalid)
{
    base::FilePath certs_dir = GetTestCertsDirectory();
    scoped_refptr<X509Certificate> cert = ImportCertFromFile(certs_dir, "can_sign_http_exchanges_draft_extension_invalid.pem");
    ASSERT_NE(static_cast<X509Certificate*>(nullptr), cert.get());

    EXPECT_FALSE(asn1::HasCanSignHttpExchangesDraftExtension(x509_util::CryptoBufferAsStringPiece(cert->cert_buffer())));
}

TEST(X509CertificateTest, DoesNotHaveCanSignHttpExchangesDraftExtension)
{
    base::FilePath certs_dir = GetTestCertsDirectory();
    scoped_refptr<X509Certificate> cert = ImportCertFromFile(certs_dir, "ok_cert.pem");
    ASSERT_NE(static_cast<X509Certificate*>(nullptr), cert.get());
...
TEST(X509CertificateTest, ExtractExtension)
{
    ...
    ASSERT_TRUE(asn1::ExtractExtensionFromDERCert(x509_util::CryptoBufferAsStringPiece(cert->cert_buffer()),
        bssl::der::Input(bssl::kBasicConstraintsOid).AsStringView(), &present, &critical, &contents));
    EXPECT_TRUE(present);
    ...
    ASSERT_TRUE(asn1::ExtractExtensionFromDERCert(
        x509_util::CryptoBufferAsStringPiece(cert->cert_buffer()), base::as_string_view(kNonsenseOID), &present, &critical, &contents));
    ASSERT_FALSE(present);

    scoped_refptr<X509Certificate> uid_cert = ImportCertFromFile(certs_dir, "ct-test-embedded-with-uids.pem");
    ASSERT_TRUE(uid_cert);
    ASSERT_TRUE(asn1::ExtractExtensionFromDERCert(x509_util::CryptoBufferAsStringPiece(uid_cert->cert_buffer()),
        bssl::der::Input(bssl::kBasicConstraintsOid).AsStringView(), &present, &critical, &contents));
    EXPECT_TRUE(present);
...
TEST(X509CertificateTest, Cache)
{
    ...
    scoped_refptr<X509Certificate> cert1(X509Certificate::CreateFromBuffer(std::move(google_cert_handle), {}));
    ASSERT_TRUE(cert1);

    // Add the same certificate, but as a new handle.
    google_cert_handle = x509_util::CreateCryptoBuffer(google_der);
    ASSERT_TRUE(google_cert_handle);
    scoped_refptr<X509Certificate> cert2(X509Certificate::CreateFromBuffer(std::move(google_cert_handle), {}));
    ASSERT_TRUE(cert2);
    ...
    scoped_refptr<X509Certificate> cert3(X509Certificate::CreateFromBuffer(std::move(google_cert_handle), std::move(intermediates)));
    ASSERT_TRUE(cert3);
    ...
    EXPECT_NE(cert1->intermediate_buffers().size(), cert3->intermediate_buffers().size());
}

TEST(X509CertificateTest, CloneWithDifferentIntermediates)
{
    CertificateList certs = CreateCertificateListFromFile(GetTestCertsDirectory(), "multi-root-chain1.pem", X509Certificate::FORMAT_PEM_CERT_SEQUENCE);
    ...
    }
    {
        ...
        auto cloned = leaf_with_no_intermediates->CloneWithDifferentIntermediates(std::move(intermediates));
        ASSERT_TRUE(cloned);
        ...
        EXPECT_TRUE(x509_util::CryptoBufferEqual(certs[1]->cert_buffer(), cloned->intermediate_buffers()[0].get()));
        EXPECT_TRUE(x509_util::CryptoBufferEqual(certs[2]->cert_buffer(), cloned->intermediate_buffers()[1].get()));
    }
    ...
    auto leaf_with_intermediates = X509Certificate::CreateFromBuffer(bssl::UpRef(certs[0]->cert_buffer()), std::move(leaf_intermediates));
    ...
    }
    {
        ...
        intermediates.push_back(bssl::UpRef(certs[1]->cert_buffer()));
        intermediates.push_back(bssl::UpRef(certs[2]->cert_buffer()));
        auto cloned = leaf_with_intermediates->CloneWithDifferentIntermediates(std::move(intermediates));
        // Intermediates are equal, so should return a reference to the same object.
    ...
    }
    {
        ...
        ExpectX509CertificateMembersEqual(leaf_with_intermediates, cloned);
        ASSERT_EQ(2u, cloned->intermediate_buffers().size());
        EXPECT_TRUE(x509_util::CryptoBufferEqual(certs[2]->cert_buffer(), cloned->intermediate_buffers()[0].get()));
        EXPECT_TRUE(x509_util::CryptoBufferEqual(certs[1]->cert_buffer(), cloned->intermediate_buffers()[1].get()));
    }
}

TEST(X509CertificateTest, Pickle)
{
    bssl::UniquePtr<CRYPTO_BUFFER> google_cert_handle = x509_util::CreateCryptoBuffer(google_der);
    ASSERT_TRUE(google_cert_handle);
    bssl::UniquePtr<CRYPTO_BUFFER> thawte_cert_handle = x509_util::CreateCryptoBuffer(thawte_der);
    ASSERT_TRUE(thawte_cert_handle);

    std::vector<bssl::UniquePtr<CRYPTO_BUFFER>> intermediates;
    intermediates.push_back(std::move(thawte_cert_handle));
    scoped_refptr<X509Certificate> cert = X509Certificate::CreateFromBuffer(std::move(google_cert_handle), std::move(intermediates));
    ASSERT_TRUE(cert);
    ...
    scoped_refptr<X509Certificate> cert_from_pickle = X509Certificate::CreateFromPickle(&iter);
    ASSERT_TRUE(cert_from_pickle);
    EXPECT_TRUE(x509_util::CryptoBufferEqual(cert->cert_buffer(), cert_from_pickle->cert_buffer()));
    ...
    for (size_t i = 0; i < cert_intermediates.size(); ++i) {
        EXPECT_TRUE(x509_util::CryptoBufferEqual(cert_intermediates[i].get(), pickle_intermediates[i].get()));
    }
}
...
TEST(X509CertificateTest, IntermediateCertificates)
{
    ...
    cert1 = X509Certificate::CreateFromBuffer(bssl::UpRef(google_handle.get()), {});
    ASSERT_TRUE(cert1);
    EXPECT_EQ(0u, cert1->intermediate_buffers().size());
    ...
    scoped_refptr<X509Certificate> cert2 = X509Certificate::CreateFromBuffer(std::move(google_handle), std::move(intermediates2));
    ASSERT_TRUE(cert2);
    ...
    EXPECT_TRUE(x509_util::CryptoBufferEqual(cert2_intermediates[0].get(), webkit_cert->cert_buffer()));
    EXPECT_TRUE(x509_util::CryptoBufferEqual(cert2_intermediates[1].get(), thawte_cert->cert_buffer()));
}
...
TEST(X509CertificateTest, Equals)
{
    ...
    scoped_refptr<X509Certificate> cert0_with_intermediate = X509Certificate::CreateFromBuffer(bssl::UpRef(certs[0]->cert_buffer()), std::move(intermediates1));
    ...
    scoped_refptr<X509Certificate> cert0_with_intermediate2
        = X509Certificate::CreateFromBuffer(bssl::UpRef(certs[0]->cert_buffer()), std::move(intermediates2));
    ...
    EXPECT_TRUE(cert0_with_intermediate2->EqualsExcludingChain(cert0_with_intermediate.get()));
    EXPECT_TRUE(cert0_with_intermediate->EqualsExcludingChain(cert0_with_intermediate2.get()));
    EXPECT_FALSE(cert0_with_intermediate2->EqualsIncludingChain(cert0_with_intermediate.get()));
    EXPECT_FALSE(cert0_with_intermediate->EqualsIncludingChain(cert0_with_intermediate2.get()));
    ...
    scoped_refptr<X509Certificate> cert0_with_intermediates12
        = X509Certificate::CreateFromBuffer(bssl::UpRef(certs[0]->cert_buffer()), std::move(intermediates12));
    ...
    scoped_refptr<X509Certificate> cert0_with_intermediates21
        = X509Certificate::CreateFromBuffer(bssl::UpRef(certs[0]->cert_buffer()), std::move(intermediates21));
    ...
    EXPECT_TRUE(cert0_with_intermediates21->EqualsExcludingChain(cert0_with_intermediates12.get()));
    EXPECT_TRUE(cert0_with_intermediates12->EqualsExcludingChain(cert0_with_intermediates21.get()));
    EXPECT_FALSE(cert0_with_intermediates21->EqualsIncludingChain(cert0_with_intermediates12.get()));
    EXPECT_FALSE(cert0_with_intermediates12->EqualsIncludingChain(cert0_with_intermediates21.get()));
    ...
    scoped_refptr<X509Certificate> cert0_with_intermediates12b
        = X509Certificate::CreateFromBuffer(bssl::UpRef(certs[0]->cert_buffer()), std::move(intermediates12b));
    ASSERT_TRUE(cert0_with_intermediates12b);

    // Comparing X509Certificate with two intermediates to X509Certificate with
    // same two intermediates in same order.
    EXPECT_TRUE(cert0_with_intermediates12->EqualsExcludingChain(cert0_with_intermediates12b.get()));
    EXPECT_TRUE(cert0_with_intermediates12b->EqualsExcludingChain(cert0_with_intermediates12.get()));
    EXPECT_TRUE(cert0_with_intermediates12->EqualsIncludingChain(cert0_with_intermediates12b.get()));
    EXPECT_TRUE(cert0_with_intermediates12b->EqualsIncludingChain(cert0_with_intermediates12.get()));
}
...
TEST(X509CertificateTest, IsIssuedByEncoded)
{
    ...
    scoped_refptr<X509Certificate> mit_davidben_cert(ImportCertFromFile(certs_dir, "mit.davidben.der"));
    ...
    scoped_refptr<X509Certificate> google_cert(ImportCertFromFile(certs_dir, "google.single.der"));
    ASSERT_NE(static_cast<X509Certificate*>(nullptr), google_cert.get());
...
TEST(X509CertificateTest, IsSelfSigned)
{
    ...
    scoped_refptr<X509Certificate> cert(ImportCertFromFile(certs_dir, "mit.davidben.der"));
    ASSERT_NE(static_cast<X509Certificate*>(nullptr), cert.get());
    EXPECT_FALSE(X509Certificate::IsSelfSigned(cert->cert_buffer()));

    scoped_refptr<X509Certificate> self_signed(ImportCertFromFile(certs_dir, "root_ca_cert.pem"));
    ...
    scoped_refptr<X509Certificate> bad_name(ImportCertFromFile(certs_dir, "self-signed-invalid-name.pem"));
    ASSERT_NE(static_cast<X509Certificate*>(nullptr), bad_name.get());
    EXPECT_FALSE(X509Certificate::IsSelfSigned(bad_name->cert_buffer()));

    scoped_refptr<X509Certificate> bad_sig(ImportCertFromFile(certs_dir, "self-signed-invalid-sig.pem"));
    ...
    bssl::UniquePtr<CRYPTO_BUFFER> invalid_cert_handle = x509_util::CreateCryptoBuffer(std::string_view(invalid_cert_data));
...
class X509CertificateParseTest : public testing::TestWithParam<CertificateFormatTestData> {
public:
...
{
    base::FilePath certs_dir = GetTestCertsDirectory();
    CertificateList certs = CreateCertificateListFromFile(certs_dir, test_data_.file_name, test_data_.format);
    ASSERT_FALSE(certs.empty());
    ASSERT_LE(certs.size(), std::size(test_data_.chain_fingerprints));
    CheckGoogleCert(certs.front(), google_parse_fingerprint, kGoogleParseValidFrom, kGoogleParseValidTo);
    ...
    for (size_t i = 0; i < std::size(test_data_.chain_fingerprints); ++i) {
        ...
        EXPECT_EQ(*test_data_.chain_fingerprints[i], X509Certificate::CalculateFingerprint256(certs[i]->cert_buffer()));
    }
}

INSTANTIATE_TEST_SUITE_P(All, X509CertificateParseTest, testing::ValuesIn(kFormatTestData));
...
void PrintTo(const CertificateNameVerifyTestData& data, std::ostream* os)
{
    ...
    *os << " expected: " << data.expected << "; hostname: " << data.hostname << "; dns_names: " << (data.dns_names ? data.dns_names : "")
...
const CertificateNameVerifyTestData kNameVerifyTestData[] = {
    ...
    { false, "121.2.3.512", "1*1.2.3.512,*1.2.3.512,1*.2.3.512,*.2.3.512", "121.2.3.0" },
...
class X509CertificateNameVerifyTest : public testing::TestWithParam<CertificateNameVerifyTestData> { };
...
TEST_P(X509CertificateNameVerifyTest, VerifyHostname)
{
    ...
    if (test_data.dns_names) {
        ...
        dns_names = base::SplitString(dns_name_line, ",", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL);
    }

    if (test_data.ip_addrs) {
        // Build up the certificate IP address list.
        std::string ip_addrs_line(test_data.ip_addrs);
        std::vector<std::string> ip_addressses_ascii = base::SplitString(ip_addrs_line, ",", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL);
        for (size_t i = 0; i < ip_addressses_ascii.size(); ++i) {
            ...
            if (addr_ascii[0] == 'x') { // Hex encoded address
                ...
                std::string bytes;
                EXPECT_TRUE(base::HexStringToString(addr_ascii, &bytes)) << "Could not parse hex address " << addr_ascii << " i = " << i;
                ip_addressses.push_back(std::move(bytes));
                ASSERT_EQ(16U, ip_addressses.back().size()) << i;
            } else { // Decimal groups
                std::vector<std::string> decimals_ascii_list = base::SplitString(addr_ascii, ".", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL);
    ...
    EXPECT_EQ(test_data.expected, X509Certificate::VerifyHostname(test_data.hostname, dns_names, ip_addressses));
}

INSTANTIATE_TEST_SUITE_P(All, X509CertificateNameVerifyTest, testing::ValuesIn(kNameVerifyTestData));
...
class X509CertificatePublicKeyInfoTest : public testing::TestWithParam<PublicKeyInfoTestData> { };
...
TEST_P(X509CertificatePublicKeyInfoTest, GetPublicKeyInfo)
{
    ...
    ASSERT_TRUE(leaf->UseKeyFromFile(GetTestCertsDirectory().AppendASCII(base::StrCat({ data.file_name, "-1.key" }))));

    size_t actual_bits = 0;
    X509Certificate::PublicKeyType actual_type = X509Certificate::kPublicKeyTypeUnknown;

    X509Certificate::GetPublicKeyInfo(leaf->GetCertBuffer(), &actual_bits, &actual_type);

    EXPECT_EQ(data.expected_bits, actual_bits);
    EXPECT_EQ(data.expected_type, actual_type);
}

INSTANTIATE_TEST_SUITE_P(All, X509CertificatePublicKeyInfoTest, testing::ValuesIn(kPublicKeyInfoTestData));
