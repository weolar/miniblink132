...
struct NET_EXPORT DigitallySigned {
    ...
    enum SignatureAlgorithm { SIG_ALGO_ANONYMOUS = 0, SIG_ALGO_RSA = 1, SIG_ALGO_DSA = 2, SIG_ALGO_ECDSA = 3 };
    ...
    bool SignatureParametersMatch(HashAlgorithm other_hash_algorithm, SignatureAlgorithm other_signature_algorithm) const;
...
struct NET_EXPORT SignedCertificateTimestamp : public base::RefCountedThreadSafe<SignedCertificateTimestamp> {
    // Predicate functor used in maps when SignedCertificateTimestamp is used as
    // the key.
    struct NET_EXPORT LessThan {
        bool operator()(const scoped_refptr<SignedCertificateTimestamp>& lhs, const scoped_refptr<SignedCertificateTimestamp>& rhs) const;
    };
    ...
    static scoped_refptr<SignedCertificateTimestamp> CreateFromPickle(base::PickleIterator* iter);
