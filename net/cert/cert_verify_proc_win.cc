...
int MapCertChainErrorStatusToCertStatus(DWORD error_status)
{
    ...
    const DWORD kCertInvalidErrors = CERT_TRUST_IS_CYCLIC | CERT_TRUST_INVALID_EXTENSION | CERT_TRUST_INVALID_POLICY_CONSTRAINTS
        | CERT_TRUST_INVALID_BASIC_CONSTRAINTS | CERT_TRUST_INVALID_NAME_CONSTRAINTS | CERT_TRUST_CTL_IS_NOT_SIGNATURE_VALID
        | CERT_TRUST_HAS_NOT_SUPPORTED_NAME_CONSTRAINT | CERT_TRUST_HAS_NOT_DEFINED_NAME_CONSTRAINT | CERT_TRUST_HAS_NOT_PERMITTED_NAME_CONSTRAINT
...
bool CertSubjectCommonNameHasNull(PCCERT_CONTEXT cert)
{
    ...
    BOOL rv;
    rv = CryptDecodeObjectEx(X509_ASN_ENCODING | PKCS_7_ASN_ENCODING, WINCRYPT_X509_NAME, cert->pCertInfo->Subject.pbData, cert->pCertInfo->Subject.cbData,
        CRYPT_DECODE_ALLOC_FLAG | CRYPT_DECODE_NOCOPY_FLAG, &decode_para, &name_info, &name_info_size);
    if (rv) {
        std::unique_ptr<CERT_NAME_INFO, base::FreeDeleter> scoped_name_info(name_info);
...
void GetCertChainInfo(PCCERT_CHAIN_CONTEXT chain_context, CertVerifyResult* verify_result)
{
    ...
    if (base::win::GetVersion() >= base::win::Version::WIN10) {
        ...
        for (DWORD i = 0; i < num_elements - 1; ++i) {
            ...
            if (strcmp(issuer->pCertInfo->SubjectPublicKeyInfo.Algorithm.pszObjId, szOID_ECC_PUBLIC_KEY)) {
                continue;
            }

            PCCERT_CONTEXT cert = element[i]->pCertContext;
            if (!CryptVerifyCertificateSignatureEx(NULL, X509_ASN_ENCODING, CRYPT_VERIFY_CERT_SIGN_SUBJECT_CERT, const_cast<PCERT_CONTEXT>(cert),
                    CRYPT_VERIFY_CERT_SIGN_ISSUER_CERT, const_cast<PCERT_CONTEXT>(issuer), 0, nullptr)) {
    ...
    if (verified_cert) {
        ...
        scoped_refptr<X509Certificate> verified_cert_with_chain = x509_util::CreateX509CertificateFromCertContexts(verified_cert, verified_chain);
...
void GetCertPoliciesInfo(PCCERT_CONTEXT cert, std::unique_ptr<CERT_POLICIES_INFO, base::FreeDeleter>* output)
{
    PCERT_EXTENSION extension = CertFindExtension(szOID_CERT_POLICIES, cert->pCertInfo->cExtension, cert->pCertInfo->rgExtension);
    if (!extension)
        return;
    ...
    BOOL rv;
    rv = CryptDecodeObjectEx(X509_ASN_ENCODING | PKCS_7_ASN_ENCODING, szOID_CERT_POLICIES, extension->Value.pbData, extension->Value.cbData,
        CRYPT_DECODE_ALLOC_FLAG | CRYPT_DECODE_NOCOPY_FLAG, &decode_para, &policies_info, &policies_info_size);
    if (rv)
...
{
    base::StringPiece der_bytes(reinterpret_cast<const char*>(cert->pbCertEncoded), cert->cbCertEncoded);
...
CRLSetResult CheckRevocationWithCRLSet(CRLSet* crl_set, PCCERT_CONTEXT subject_cert, PCCERT_CONTEXT issuer_cert, std::string* previous_hash)
{
    DCHECK(crl_set);
    DCHECK(subject_cert);
    ...
    if (!HashSPKI(subject_cert, &subject_hash) || !GetSubject(subject_cert, &subject_name)) {
        NOTREACHED(); // Indicates Windows accepted something irrecoverably bad.
        previous_hash->clear();
        return kCRLSetError;
    }

    if (crl_set->CheckSPKI(subject_hash) == CRLSet::REVOKED || crl_set->CheckSubject(subject_name, subject_hash) == CRLSet::REVOKED) {
    ...
    base::StringPiece serial(reinterpret_cast<const char*>(serial_bytes.get()), serial_blob->cbData);
...
//       no reason.
//   kCRLSetOk: otherwise.
CRLSetResult CheckChainRevocationWithCRLSet(PCCERT_CHAIN_CONTEXT chain, CRLSet* crl_set)
{
...
void AppendPublicKeyHashesAndUpdateKnownRoot(PCCERT_CHAIN_CONTEXT chain, HashValueVector* hashes, bool* known_root)
{
    if (chain->cChain == 0)
        return;
    ...
    for (DWORD i = num_elements; i > 0; i--) {
        ...
        if (!*known_root) {
            *known_root = GetNetTrustAnchorHistogramIdForSPKI(sha256) != 0 || IsKnownRoot(cert);
        }
    }
...
bool CheckEV(PCCERT_CHAIN_CONTEXT chain_context, bool rev_checking_enabled, const char* policy_oid)
{
...
BOOL WINAPI CertDllVerifyRevocationWithCRLSet(DWORD encoding_type, DWORD revocation_type, DWORD num_contexts, void* rgpvContext[], DWORD flags,
    PCERT_REVOCATION_PARA revocation_params, PCERT_REVOCATION_STATUS revocation_status);
...
public:
    CRLSet* GetCRLSet()
    {
        return thread_local_crlset.Get();
    }

    void SetCRLSet(CRLSet* crl_set)
    {
        thread_local_crlset.Set(crl_set);
    }
...
private:
    ...
    RevocationInjector()
    {
        const CRYPT_OID_FUNC_ENTRY kInterceptFunction[] = {
            { CRYPT_DEFAULT_OID, reinterpret_cast<void*>(&CertDllVerifyRevocationWithCRLSet) },
        };
        BOOL ok = CryptInstallOIDFunctionAddress(nullptr, X509_ASN_ENCODING, CRYPT_OID_VERIFY_REVOCATION_FUNC, std::size(kInterceptFunction),
            kInterceptFunction, CRYPT_INSTALL_OID_FUNC_BEFORE_FLAG);
        DCHECK(ok);
    }
    ...
    PCERT_REVOCATION_PARA revocation_params, PCERT_REVOCATION_STATUS revocation_status)
{
    ...
    if (revocation_params) {
        DWORD bytes_to_copy = std::min(revocation_params->cbSize, static_cast<DWORD>(sizeof(local_params)));
    ...
    if ((flags & CERT_VERIFY_REV_CHAIN_FLAG) && num_contexts > 1) {
        ...
        for (DWORD i = num_contexts; i > 0; --i) {
            ...
            CRLSetResult result = CheckRevocationWithCRLSet(crl_set, subject_cert, issuer_cert, &issuer_hash);
            if (result == kCRLSetRevoked) {
    ...
    if (local_params.pIssuerCert
        && CryptVerifyCertificateSignatureEx(NULL, subject_cert->dwCertEncodingType, CRYPT_VERIFY_CERT_SIGN_SUBJECT_CERT, subject_cert,
            CRYPT_VERIFY_CERT_SIGN_ISSUER_CERT, const_cast<PCERT_CONTEXT>(local_params.pIssuerCert), 0, nullptr)) {
        // Caller has already supplied the issuer cert via the revocation params;
        // just use that.
        issuer_cert.reset(CertDuplicateCertificateContext(local_params.pIssuerCert));
    } else if (CertCompareCertificateName(subject_cert->dwCertEncodingType, &subject_cert->pCertInfo->Subject, &subject_cert->pCertInfo->Issuer)
        && CryptVerifyCertificateSignatureEx(NULL, subject_cert->dwCertEncodingType, CRYPT_VERIFY_CERT_SIGN_SUBJECT_CERT, subject_cert,
            CRYPT_VERIFY_CERT_SIGN_ISSUER_CERT, subject_cert, 0, nullptr)) {
        ...
        for (DWORD i = 0; i < local_params.cCertStore && !issuer_cert; ++i) {
            ...
            for (;;) {
                ...
                previous_cert = CertGetIssuerCertificateFromStore(local_params.rgCertStore[i], subject_cert, previous_cert, &store_search_flags);
                if (!previous_cert)
                    break;
    ...
    std::string unused;
    CRLSetResult result = CheckRevocationWithCRLSet(crl_set, subject_cert, issuer_cert.get(), &unused);
    if (result == kCRLSetRevoked) {
        revocation_status->dwError = static_cast<DWORD>(CRYPT_E_REVOKED);
        revocation_status->dwReason = CRL_REASON_UNSPECIFIED;
        SetLastError(revocation_status->dwError);
...
public:
    ...
    ~ScopedThreadLocalCRLSet()
    {
        g_revocation_injector.Get().SetCRLSet(nullptr);
    }
};
    ...
    : event_(CreateEvent(nullptr, FALSE, TRUE, nullptr))
{
    ...
    constexpr wchar_t kAuthRootPath[] = L"SOFTWARE\\Microsoft\\SystemCertificates\\AuthRoot\\AutoUpdate";
...
AuthRootVersionChecker::AuthRootVersion AuthRootVersionChecker::GetAuthRootVersion()
{
...
bool AuthRootVersionChecker::ShouldUpdate()
{
    ...
    LSTATUS result
        = RegQueryInfoKeyW(key_.Handle(), nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, &current_timestamp);
...
void AuthRootVersionChecker::UpdateAuthRootVersion()
{
    ...
    crypto::ScopedPCCTL_CONTEXT ctl_context(CertCreateCTLContext(PKCS_7_ASN_ENCODING, value.data(), value.size()));
    if (!ctl_context || ctl_context->pCtlInfo->SequenceNumber.cbData == 0)
        return;

    auth_root_version_.sequence_number.assign(
        ctl_context->pCtlInfo->SequenceNumber.pbData, ctl_context->pCtlInfo->SequenceNumber.pbData + ctl_context->pCtlInfo->SequenceNumber.cbData);
    // Convert from Windows' little-endian representation to the expected (as
    // encoded) big-endian form.
    std::reverse(std::begin(auth_root_version_.sequence_number), std::end(auth_root_version_.sequence_number));
    auth_root_version_.this_update = base::Time::FromFileTime(ctl_context->pCtlInfo->ThisUpdate);
}

} // namespace

CertVerifyProcWin::ResultDebugData::ResultDebugData(base::Time authroot_this_update, std::vector<uint8_t> authroot_sequence_number)
    : authroot_this_update_(authroot_this_update)
    , authroot_sequence_number_(std::move(authroot_sequence_number))
{
}

CertVerifyProcWin::ResultDebugData::ResultDebugData(const ResultDebugData& other) = default;

CertVerifyProcWin::ResultDebugData::~ResultDebugData() = default;

// static
const CertVerifyProcWin::ResultDebugData* CertVerifyProcWin::ResultDebugData::Get(const base::SupportsUserData* debug_data)
{
    return static_cast<ResultDebugData*>(debug_data->GetUserData(kResultDebugDataKey));
}

// static
void CertVerifyProcWin::ResultDebugData::Create(
    base::Time authroot_this_update, std::vector<uint8_t> authroot_sequence_number, base::SupportsUserData* debug_data)
{
    debug_data->SetUserData(kResultDebugDataKey, std::make_unique<ResultDebugData>(authroot_this_update, std::move(authroot_sequence_number)));
}

std::unique_ptr<base::SupportsUserData::Data> CertVerifyProcWin::ResultDebugData::Clone()
{
...
bool CheckNeedVerify()
{
    ...
    //     }
    //
    //     need_verify = false; //os.wServicePackMajor >= 3;
    //   } while (false);
    HMODULE kernel32 = GetModuleHandleW(L"Kernel32.dll");
    need_verify = !!(GetProcAddress(kernel32, "CancelIoEx"));
...
int CertVerifyProcWin::VerifyInternal(X509Certificate* cert, const std::string& hostname, const std::string& ocsp_response, const std::string& sct_list,
    int flags, CRLSet* crl_set, const CertificateList& additional_trust_anchors, CertVerifyResult* verify_result, const NetLogWithSource& net_log)
{
    ...
    crypto::ScopedPCCERT_CONTEXT cert_list = x509_util::CreateCertContextWithChain(cert, x509_util::InvalidIntermediateBehavior::kIgnore);
    if (!cert_list) {
        verify_result->cert_status |= CERT_STATUS_INVALID;
        return ERR_CERT_INVALID;
    }
    ...
    // two usages.
    static const LPCSTR usage[] = { szOID_PKIX_KP_SERVER_AUTH, szOID_SERVER_GATED_CRYPTO, szOID_SGC_NETSCAPE };
    ...
    if (!ocsp_response.empty()) {
        ...
        CertSetCertificateContextProperty(cert_list.get(), CERT_OCSP_RESPONSE_PROP_ID, CERT_SET_PROPERTY_IGNORE_PERSIST_ERROR_FLAG, &ocsp_response_blob);
    }
    ...
    BOOL chain_result = CertGetCertificateChain(chain_engine.get(), cert_list.get(),
        nullptr, // current system time
        cert_list->hCertStore, &chain_para, chain_flags,
        nullptr, // reserved
        &chain_context);
    if (chain_result && chain_context && (chain_context->TrustStatus.dwErrorStatus & (CERT_TRUST_HAS_WEAK_SIGNATURE | CERT_TRUST_IS_NOT_SIGNATURE_VALID))) {
        ...
        chain_para.dwStrongSignFlags = 0;
        chain_result = CertGetCertificateChain(chain_engine.get(), cert_list.get(),
            nullptr, // current system time
            cert_list->hCertStore, &chain_para, chain_flags,
            nullptr, // reserved
            &chain_context);
    ...
    AuthRootVersionChecker::AuthRootVersion authroot_version = authroot_version_checker->GetAuthRootVersion();
    ResultDebugData::Create(std::move(authroot_version.this_update), std::move(authroot_version.sequence_number), verify_result);
    ...
    CRLSetResult crl_set_result = CheckChainRevocationWithCRLSet(chain_context, crl_set);
    ...
    if (!verify_result->is_issued_by_known_root && (flags & VERIFY_REV_CHECKING_REQUIRED_LOCAL_ANCHORS)) {
    ...
    verify_result->cert_status |= MapCertChainErrorStatusToCertStatus(chain_context->TrustStatus.dwErrorStatus);
    ...
    if (policy_status.dwError) {
        verify_result->cert_status |= MapNetErrorToCertStatus(MapSecurityError(policy_status.dwError));
    }
    ...
    if (!(!verify_result->is_issued_by_known_root && (flags & VERIFY_REV_CHECKING_REQUIRED_LOCAL_ANCHORS))) {
        verify_result->cert_status &= ~(CERT_STATUS_NO_REVOCATION_MECHANISM | CERT_STATUS_UNABLE_TO_CHECK_REVOCATION);
    }

    AppendPublicKeyHashesAndUpdateKnownRoot(chain_context, &verify_result->public_key_hashes, &verify_result->is_issued_by_known_root);
    ...
    LogNameNormalizationMetrics(".Win", verify_result->verified_cert.get(), verify_result->is_issued_by_known_root);
