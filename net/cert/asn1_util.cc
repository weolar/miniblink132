...
bool SeekToSubject(bssl::der::Input in, bssl::der::Parser* tbs_certificate)
{
    ...
    bool unused;
    if (!tbs_certificate->SkipOptionalTag(CBS_ASN1_CONSTRUCTED | CBS_ASN1_CONTEXT_SPECIFIC | 0, &unused)) {
        return false;
    }
...
// undefined state.
bool SeekToExtensions(bssl::der::Input in, bool* extensions_present, bssl::der::Parser* extensions_parser)
{
    bool present;
    ...
    // issuerUniqueID
    if (!tbs_cert_parser.SkipOptionalTag(CBS_ASN1_CONTEXT_SPECIFIC | 1, &present)) {
        return false;
    }
    // subjectUniqueID
    if (!tbs_cert_parser.SkipOptionalTag(CBS_ASN1_CONTEXT_SPECIFIC | 2, &present)) {
        return false;
    }

    std::optional<bssl::der::Input> extensions;
    if (!tbs_cert_parser.ReadOptionalTag(CBS_ASN1_CONSTRUCTED | CBS_ASN1_CONTEXT_SPECIFIC | 3, &extensions)) {
        return false;
    }
...
bool ExtractExtensionWithOID(std::string_view cert, bssl::der::Input extension_oid, bool* out_extension_present, bssl::ParsedExtension* out_extension)
{
    bssl::der::Parser extensions;
    bool extensions_present;
    if (!SeekToExtensions(bssl::der::Input(cert), &extensions_present, &extensions)) {
        return false;
    }
    ...
    while (extensions.HasMore()) {
        ...
        if (!extensions.ReadRawTLV(&extension_tlv) || !ParseExtension(extension_tlv, out_extension)) {
            return false;
        }
...
bool ExtractSubjectFromDERCert(std::string_view cert, std::string_view* subject_out)
{
...
bool ExtractSubjectPublicKeyFromSPKI(std::string_view spki, std::string_view* spk_out)
{
    // From RFC 5280, Section 4.1
...
bool HasCanSignHttpExchangesDraftExtension(std::string_view cert)
{
    ...
    static const uint8_t kCanSignHttpExchangesDraftOid[] = { 0x2B, 0x06, 0x01, 0x04, 0x01, 0xd6, 0x79, 0x02, 0x01, 0x16 };

    bool extension_present;
    bssl::ParsedExtension extension;
    if (!ExtractExtensionWithOID(cert, bssl::der::Input(kCanSignHttpExchangesDraftOid), &extension_present, &extension) || !extension_present) {
        return false;
    }
...
bool ExtractSignatureAlgorithmsFromDERCert(
    std::string_view cert, std::string_view* cert_signature_algorithm_sequence, std::string_view* tbs_signature_algorithm_sequence)
{
    // From RFC 5280, section 4.1
    //    Certificate  ::=  SEQUENCE  {
    //      tbsCertificate       TBSCertificate,
...
bool ExtractExtensionFromDERCert(
    std::string_view cert, std::string_view extension_oid, bool* out_extension_present, bool* out_extension_critical, std::string_view* out_contents)
{
    *out_extension_present = false;
    ...
    *out_contents = std::string_view();
    ...
    bssl::ParsedExtension extension;
    if (!ExtractExtensionWithOID(cert, bssl::der::Input(extension_oid), out_extension_present, &extension)) {
        return false;
    }
