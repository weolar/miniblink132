...
public:
    void SetUp() override
    {
        scoped_refptr<const CTLogVerifier> log(CTLogVerifier::Create(ct::GetTestPublicKey(), kLogDescription));
        ASSERT_TRUE(log);
        log_verifiers_.push_back(log);

        verifier_ = std::make_unique<MultiLogCTVerifier>(log_verifiers_);
        std::string der_test_cert(ct::GetDerEncodedX509Cert());
        chain_ = X509Certificate::CreateFromBytes(base::as_byte_span(der_test_cert));
        ASSERT_TRUE(chain_.get());

        embedded_sct_chain_ = CreateCertificateChainFromFile(GetTestCertsDirectory(), "ct-test-embedded-cert.pem", X509Certificate::FORMAT_AUTO);
        ASSERT_TRUE(embedded_sct_chain_.get());
    }

    bool CheckForEmbeddedSCTInNetLog(const RecordingNetLogObserver& net_log_observer)
    {
        auto entries = net_log_observer.GetEntries();
        if (entries.size() != 2)
            return false;

        auto embedded_scts = GetOptionalStringValueFromParams(entries[0], "embedded_scts");
        ...
        const std::string* verification_status = the_sct.GetDict().FindString("verification_status");
    ...
    bool VerifySinglePrecertificateChain(scoped_refptr<X509Certificate> chain)
    {
        ...
        verifier_->Verify(chain.get(), std::string_view(), std::string_view(), base::Time::Now(), &scts, NetLogWithSource());
        return !scts.empty();
    }
    ...
    bool CheckPrecertificateVerification(scoped_refptr<X509Certificate> chain)
    {
        ...
        NetLogWithSource net_log = NetLogWithSource::Make(NetLog::Get(), NetLogSourceType::SSL_CONNECT_JOB);
        verifier_->Verify(chain.get(), std::string_view(), std::string_view(), base::Time::Now(), &scts, net_log);
        return ct::CheckForSingleVerifiedSCTInResult(scts, kLogDescription) && ct::CheckForSCTOrigin(scts, ct::SignedCertificateTimestamp::SCT_EMBEDDED)
            && CheckForEmbeddedSCTInNetLog(net_log_observer);
    }

    // Histogram-related helper methods
    int GetValueFromHistogram(const std::string& histogram_name, int sample_index)
    {
        base::Histogram* histogram = static_cast<base::Histogram*>(base::StatisticsRecorder::FindHistogram(histogram_name));
    ...
    {
        return GetValueFromHistogram("Net.CertificateTransparency.SCTOrigin", ct::SignedCertificateTimestamp::SCT_EMBEDDED);
    }

    int NumValidSCTsInStatusHistogram()
    {
        return GetValueFromHistogram("Net.CertificateTransparency.SCTStatus", ct::SCT_STATUS_OK);
    }
...
TEST_F(MultiLogCTVerifierTest, VerifiesEmbeddedSCTWithPreCA)
{
    scoped_refptr<X509Certificate> chain(
        CreateCertificateChainFromFile(GetTestCertsDirectory(), "ct-test-embedded-with-preca-chain.pem", X509Certificate::FORMAT_AUTO));
...
{
    scoped_refptr<X509Certificate> chain(
        CreateCertificateChainFromFile(GetTestCertsDirectory(), "ct-test-embedded-with-intermediate-chain.pem", X509Certificate::FORMAT_AUTO));
    ASSERT_TRUE(chain.get());
    ASSERT_TRUE(CheckPrecertificateVerification(chain));
}
...
{
    scoped_refptr<X509Certificate> chain(
        CreateCertificateChainFromFile(GetTestCertsDirectory(), "ct-test-embedded-with-intermediate-preca-chain.pem", X509Certificate::FORMAT_AUTO));
    ASSERT_TRUE(chain.get());
    ASSERT_TRUE(CheckPrecertificateVerification(chain));
}
...
TEST_F(MultiLogCTVerifierTest, VerifiesSCTOverX509Cert)
{
    ...
    SignedCertificateTimestampAndStatusList scts;
    verifier_->Verify(chain_.get(), std::string_view(), sct_list, base::Time::Now(), &scts, NetLogWithSource());
    ASSERT_TRUE(ct::CheckForSingleVerifiedSCTInResult(scts, kLogDescription));
    ASSERT_TRUE(ct::CheckForSCTOrigin(scts, ct::SignedCertificateTimestamp::SCT_FROM_TLS_EXTENSION));
}
...
TEST_F(MultiLogCTVerifierTest, CountsInvalidSCTsInStatusHistogram)
{
    ...
    int num_valid_scts = NumValidSCTsInStatusHistogram();
    int num_invalid_scts = GetValueFromHistogram("Net.CertificateTransparency.SCTStatus", ct::SCT_STATUS_LOG_UNKNOWN);

    verifier_->Verify(chain_.get(), std::string_view(), sct_list, base::Time::Now(), &scts, NetLogWithSource());

    ASSERT_EQ(num_valid_scts, NumValidSCTsInStatusHistogram());
    ASSERT_EQ(num_invalid_scts + 1, GetValueFromHistogram("Net.CertificateTransparency.SCTStatus", ct::SCT_STATUS_LOG_UNKNOWN));
}
