...
MATCHER_P(CertChainMatches, expected_cert, "")
{
    CertificateList actual_certs
        = X509Certificate::CreateCertificateListFromBytes(base::as_bytes(base::make_span(arg)), X509Certificate::FORMAT_PEM_CERT_SEQUENCE);
    ...
    for (const auto& cert : actual_certs) {
        actual_der_certs.emplace_back(x509_util::CryptoBufferAsStringPiece(cert->cert_buffer()));
    }

    std::vector<std::string> expected_der_certs;
    expected_der_certs.emplace_back(x509_util::CryptoBufferAsStringPiece(expected_cert->cert_buffer()));
    for (const auto& buffer : expected_cert->intermediate_buffers()) {
        expected_der_certs.emplace_back(x509_util::CryptoBufferAsStringPiece(buffer.get()));
    }
...
public:
    RepeatedTestClosure()
        : closure_(base::BindRepeating(&RepeatedTestClosure::DidSetResult, base::Unretained(this)))
    {
    }

    const base::RepeatingClosure& closure() const
    {
        return closure_;
    }
...
public:
    ...
    int num_verifications() const
    {
        return num_verifications_;
    }
...
private:
    int VerifyInternal(X509Certificate* cert, const std::string& hostname, const std::string& ocsp_response, const std::string& sct_list, int flags,
        CRLSet* crl_set, const CertificateList& additional_trust_anchors, CertVerifyResult* verify_result, const NetLogWithSource& net_log) override;
...
int FakeCertVerifyProc::VerifyInternal(X509Certificate* cert, const std::string& hostname, const std::string& ocsp_response, const std::string& sct_list,
    int flags, CRLSet* crl_set, const CertificateList& additional_trust_anchors, CertVerifyResult* verify_result, const NetLogWithSource& net_log)
{
    *verify_result = result_;
    main_task_runner_->PostTask(FROM_HERE, base::BindOnce(&FakeCertVerifyProc::VerifyCalled, this));
...
int NotCalledCertVerifyProc::VerifyInternal(X509Certificate* cert, const std::string& hostname, const std::string& ocsp_response, const std::string& sct_list,
    int flags, CRLSet* crl_set, const CertificateList& additional_trust_anchors, CertVerifyResult* verify_result, const NetLogWithSource& net_log)
{
...
public:
    ...
    MOCK_METHOD9(VerifyInternal,
        int(X509Certificate* cert, const std::string& hostname, const std::string& ocsp_response, const std::string& sct_list, int flags, CRLSet* crl_set,
            const CertificateList& additional_trust_anchors, CertVerifyResult* verify_result, const NetLogWithSource& net_log));
...
public:
    ...
    scoped_refptr<net::CertVerifyProc> CreateCertVerifyProc(scoped_refptr<CertNetFetcher> cert_net_fetcher, const ChromeRootStoreData* root_store_data) override
    {
        return verify_proc_;
    }
...
struct TrialReportInfo {
    TrialReportInfo(const std::string& hostname, const scoped_refptr<X509Certificate>& unverified_cert, bool enable_rev_checking,
        bool require_rev_checking_local_anchors, bool enable_sha1_local_anchors, bool disable_symantec_enforcement, const std::string& stapled_ocsp,
        const std::string& sct_list, const CertVerifyResult& primary_result, const CertVerifyResult& trial_result)
        : hostname(hostname)
...
void RecordTrialReport(std::vector<TrialReportInfo>* reports, const std::string& hostname, const scoped_refptr<X509Certificate>& unverified_cert,
    bool enable_rev_checking, bool require_rev_checking_local_anchors, bool enable_sha1_local_anchors, bool disable_symantec_enforcement,
    const std::string& stapled_ocsp, const std::string& sct_list, const CertVerifyResult& primary_result, const CertVerifyResult& trial_result)
{
    TrialReportInfo report(hostname, unverified_cert, enable_rev_checking, require_rev_checking_local_anchors, enable_sha1_local_anchors,
        disable_symantec_enforcement, stapled_ocsp, sct_list, primary_result, trial_result);
...
class TrialComparisonCertVerifierTest : public TestWithTaskEnvironment {
    void SetUp() override
    {
        cert_chain_1_ = CreateCertificateChainFromFile(GetTestCertsDirectory(), "multi-root-chain1.pem", X509Certificate::FORMAT_AUTO);
        ASSERT_TRUE(cert_chain_1_);
        leaf_cert_1_ = X509Certificate::CreateFromBuffer(bssl::UpRef(cert_chain_1_->cert_buffer()), {});
        ASSERT_TRUE(leaf_cert_1_);
        cert_chain_2_ = CreateCertificateChainFromFile(GetTestCertsDirectory(), "multi-root-chain2.pem", X509Certificate::FORMAT_AUTO);
        ASSERT_TRUE(cert_chain_2_);

        lets_encrypt_dst_x3_ = CreateCertificateChainFromFile(GetTestCertsDirectory(), "lets-encrypt-dst-x3-root.pem", X509Certificate::FORMAT_AUTO);
        ASSERT_TRUE(lets_encrypt_dst_x3_);
        lets_encrypt_isrg_x1_ = CreateCertificateChainFromFile(GetTestCertsDirectory(), "lets-encrypt-isrg-x1-root.pem", X509Certificate::FORMAT_AUTO);
...
TEST_F(TrialComparisonCertVerifierTest, InitiallyDisallowed)
{
    ...
    TrialComparisonCertVerifier verifier(
        verify_proc, nullptr, base::MakeRefCounted<NotCalledCertVerifyProc>(), nullptr, base::BindRepeating(&RecordTrialReport, &reports));
    ...
    int error = verifier.Verify(params, &result, callback.callback(), &request, NetLogWithSource());
    ...
    histograms_.ExpectTotalCount("Net.CertVerifier_Job_Latency_TrialSecondary", 0);
...
TEST_F(TrialComparisonCertVerifierTest, InitiallyDisallowedThenAllowed)
{
    ...
    base::FilePath certs_dir = GetTestNetDataDirectory().AppendASCII("verify_certificate_chain_unittest").AppendASCII("many-names");
    scoped_refptr<X509Certificate> cert_chain = CreateCertificateChainFromFile(certs_dir, "ok-all-types.pem", X509Certificate::FORMAT_AUTO);
    ASSERT_TRUE(cert_chain);
    ASSERT_EQ(2U, cert_chain->intermediate_buffers().size());

    scoped_refptr<X509Certificate> leaf = X509Certificate::CreateFromBuffer(bssl::UpRef(cert_chain->cert_buffer()), {});
    ASSERT_TRUE(leaf);

    CertVerifyResult primary_result;
    primary_result.verified_cert = cert_chain;
    scoped_refptr<FakeCertVerifyProc> verify_proc1 = base::MakeRefCounted<FakeCertVerifyProc>(OK, primary_result);
    ...
    scoped_refptr<FakeCertVerifyProc> verify_proc2 = base::MakeRefCounted<FakeCertVerifyProc>(ERR_CERT_DATE_INVALID, secondary_result);

    std::vector<TrialReportInfo> reports;
    TrialComparisonCertVerifier verifier(verify_proc1, nullptr, verify_proc2, nullptr, base::BindRepeating(&RecordTrialReport, &reports));
    ...
    error = verifier.Verify(params2, &result2, callback2.callback(), &request2, NetLogWithSource());
    ASSERT_THAT(error, IsError(ERR_IO_PENDING));
    ...
    histograms_.ExpectTotalCount("Net.CertVerifier_Job_Latency_TrialSecondary", 1);
    histograms_.ExpectUniqueSample("Net.CertVerifier_TrialComparisonResult", TrialComparisonResult::kPrimaryValidSecondaryError, 1);
...
TEST_F(TrialComparisonCertVerifierTest, ConfigChangedDuringPrimaryVerification)
{
    CertVerifyResult primary_result;
    ...
    std::vector<TrialReportInfo> reports;
    TrialComparisonCertVerifier verifier(
        verify_proc1, nullptr, base::MakeRefCounted<NotCalledCertVerifyProc>(), nullptr, base::BindRepeating(&RecordTrialReport, &reports));
    verifier.set_trial_allowed(true);
...
TEST_F(TrialComparisonCertVerifierTest, CertVerifyProcChangedDuringPrimaryVerification)
{
    CertVerifyResult primary_result;
    primary_result.verified_cert = cert_chain_1_;
    scoped_refptr<FakeCertVerifyProc> verify_proc1 = base::MakeRefCounted<FakeCertVerifyProc>(OK, primary_result);

    scoped_refptr<SwapWithNewProcFactory> swap_proc = base::MakeRefCounted<SwapWithNewProcFactory>(base::MakeRefCounted<NotCalledCertVerifyProc>());
    std::vector<TrialReportInfo> reports;
    TrialComparisonCertVerifier verifier(
        verify_proc1, swap_proc, base::MakeRefCounted<NotCalledCertVerifyProc>(), nullptr, base::BindRepeating(&RecordTrialReport, &reports));
    verifier.set_trial_allowed(true);
...
TEST_F(TrialComparisonCertVerifierTest, CertVerifyProcChangedDuringTrialVerification)
{
    CertVerifyResult primary_result;
    primary_result.verified_cert = cert_chain_1_;
    scoped_refptr<FakeCertVerifyProc> verify_proc1 = base::MakeRefCounted<FakeCertVerifyProc>(OK, primary_result);
    ...
    scoped_refptr<SwapWithNewProcFactory> swap_proc = base::MakeRefCounted<SwapWithNewProcFactory>(base::MakeRefCounted<NotCalledCertVerifyProc>());
    std::vector<TrialReportInfo> reports;
    TrialComparisonCertVerifier verifier(verify_proc1, nullptr, verify_proc2, swap_proc, base::BindRepeating(&RecordTrialReport, &reports));
    verifier.set_trial_allowed(true);
...
TEST_F(TrialComparisonCertVerifierTest, SameResult)
{
    CertVerifyResult dummy_result;
    dummy_result.verified_cert = cert_chain_1_;
    scoped_refptr<FakeCertVerifyProc> verify_proc1 = base::MakeRefCounted<FakeCertVerifyProc>(OK, dummy_result);
    scoped_refptr<FakeCertVerifyProc> verify_proc2 = base::MakeRefCounted<FakeCertVerifyProc>(OK, dummy_result);
    ...
    histograms_.ExpectTotalCount("Net.CertVerifier_Job_Latency_TrialPrimary", 1);
    histograms_.ExpectTotalCount("Net.CertVerifier_Job_Latency_TrialSecondary", 1);
    histograms_.ExpectUniqueSample("Net.CertVerifier_TrialComparisonResult", TrialComparisonResult::kEqual, 1);
}
...
TEST_F(TrialComparisonCertVerifierTest, PrimaryVerifierErrorSecondaryOk)
{
    ...
    scoped_refptr<FakeCertVerifyProc> verify_proc1 = base::MakeRefCounted<FakeCertVerifyProc>(ERR_CERT_DATE_INVALID, primary_result);

    CertVerifyResult secondary_result;
    secondary_result.verified_cert = cert_chain_1_;
    scoped_refptr<FakeCertVerifyProc> verify_proc2 = base::MakeRefCounted<FakeCertVerifyProc>(OK, secondary_result);
    ...
    EXPECT_TRUE(report.primary_result.verified_cert->EqualsIncludingChain(cert_chain_1_.get()));
    EXPECT_TRUE(report.trial_result.verified_cert->EqualsIncludingChain(cert_chain_1_.get()));
    ...
    histograms_.ExpectTotalCount("Net.CertVerifier_Job_Latency_TrialPrimary", 1);
    histograms_.ExpectTotalCount("Net.CertVerifier_Job_Latency_TrialSecondary", 1);
    histograms_.ExpectUniqueSample("Net.CertVerifier_TrialComparisonResult", TrialComparisonResult::kPrimaryErrorSecondaryValid, 1);
}
...
TEST_F(TrialComparisonCertVerifierTest, PrimaryVerifierOkSecondaryError)
{
    ...
    CertVerifier::RequestParams params(leaf_cert_1_, "127.0.0.1", /*flags=*/0, "ocsp", "sct");
    CertVerifyResult result;
    TestCompletionCallback callback;
    std::unique_ptr<CertVerifier::Request> request;
    int error = verifier.Verify(params, &result, callback.callback(), &request, NetLogWithSource());
...
TEST_F(TrialComparisonCertVerifierTest, BothVerifiersDifferentErrors)
{
    ...
    primary_result.verified_cert = cert_chain_1_;
    scoped_refptr<FakeCertVerifyProc> verify_proc1 = base::MakeRefCounted<FakeCertVerifyProc>(ERR_CERT_VALIDITY_TOO_LONG, primary_result);
    ...
    histograms_.ExpectTotalCount("Net.CertVerifier_Job_Latency_TrialPrimary", 1);
    histograms_.ExpectTotalCount("Net.CertVerifier_Job_Latency_TrialSecondary", 1);
    histograms_.ExpectUniqueSample("Net.CertVerifier_TrialComparisonResult", TrialComparisonResult::kBothErrorDifferentDetails, 1);
}

TEST_F(TrialComparisonCertVerifierTest, BothVerifiersOkDifferentVerifiedChains)
{
    ...
    EXPECT_TRUE(report.primary_result.verified_cert->EqualsIncludingChain(cert_chain_1_.get()));
    EXPECT_TRUE(report.trial_result.verified_cert->EqualsIncludingChain(cert_chain_2_.get()));
    EXPECT_TRUE(report.unverified_cert->EqualsIncludingChain(leaf_cert_1_.get()));
    ...
    histograms_.ExpectTotalCount("Net.CertVerifier_Job_Latency_TrialPrimary", 1);
    histograms_.ExpectTotalCount("Net.CertVerifier_Job_Latency_TrialSecondary", 1);
    histograms_.ExpectUniqueSample("Net.CertVerifier_TrialComparisonResult", TrialComparisonResult::kBothValidDifferentDetails, 1);
}

TEST_F(TrialComparisonCertVerifierTest, DifferentVerifiedChainsAndConfigHasChanged)
{
    // Primary verifier returns chain1 regardless of arguments.
    CertVerifyResult primary_result;
    primary_result.verified_cert = cert_chain_1_;
    scoped_refptr<FakeCertVerifyProc> verify_proc1 = base::MakeRefCounted<FakeCertVerifyProc>(ERR_CERT_REVOKED, primary_result);
    ...
    histograms_.ExpectTotalCount("Net.CertVerifier_Job_Latency_TrialPrimary", 1);
    histograms_.ExpectTotalCount("Net.CertVerifier_Job_Latency_TrialSecondary", 1);
    histograms_.ExpectUniqueSample("Net.CertVerifier_TrialComparisonResult", TrialComparisonResult::kIgnoredConfigurationChanged, 1);
}

TEST_F(TrialComparisonCertVerifierTest, BothVerifiersOkDifferentVerifiedChainsEqualAfterReverification)
{
    ...
    scoped_refptr<MockCertVerifyProc> verify_proc1 = base::MakeRefCounted<MockCertVerifyProc>();
    // Primary verifier returns ok status and chain1 if verifying the leaf alone.
    EXPECT_CALL(*verify_proc1, VerifyInternal(leaf_cert_1_.get(), _, _, _, _, _, _, _, _)).WillOnce(DoAll(SetArgPointee<7>(chain1_result), Return(OK)));
    // Primary verifier returns ok status and chain2 if verifying chain2.
    EXPECT_CALL(*verify_proc1, VerifyInternal(cert_chain_2_.get(), _, _, _, _, _, _, _, _)).WillOnce(DoAll(SetArgPointee<7>(chain2_result), Return(OK)));

    // Trial verifier returns ok status and chain2.
    scoped_refptr<FakeCertVerifyProc> verify_proc2 = base::MakeRefCounted<FakeCertVerifyProc>(OK, chain2_result);
    ...
    histograms_.ExpectTotalCount("Net.CertVerifier_Job_Latency_TrialPrimary", 1);
    histograms_.ExpectTotalCount("Net.CertVerifier_Job_Latency_TrialSecondary", 1);
    histograms_.ExpectUniqueSample("Net.CertVerifier_TrialComparisonResult", TrialComparisonResult::kIgnoredDifferentPathReVerifiesEquivalent, 1);
}

TEST_F(TrialComparisonCertVerifierTest, DifferentVerifiedChainsIgnorableDifferenceAfterReverification)
{
    base::FilePath certs_dir = GetTestNetDataDirectory().AppendASCII("trial_comparison_cert_verifier_unittest").AppendASCII("target-multiple-policies");
    scoped_refptr<X509Certificate> cert_chain = CreateCertificateChainFromFile(certs_dir, "chain.pem", X509Certificate::FORMAT_AUTO);
    ASSERT_TRUE(cert_chain);
    ASSERT_EQ(2U, cert_chain->intermediate_buffers().size());
    ...
    intermediates.push_back(bssl::UpRef(cert_chain_1_->intermediate_buffers().back().get()));
    scoped_refptr<X509Certificate> different_chain = X509Certificate::CreateFromBuffer(bssl::UpRef(cert_chain->cert_buffer()), std::move(intermediates));
    ...
    ev_chain_result.cert_status = CERT_STATUS_IS_EV | CERT_STATUS_REV_CHECKING_ENABLED;

    SHA256HashValue root_fingerprint;
    crypto::SHA256HashString(
        x509_util::CryptoBufferAsStringPiece(cert_chain->intermediate_buffers().back().get()), root_fingerprint.data, sizeof(root_fingerprint.data));
    // Both policies in the target are EV policies, but only 1.2.6.7 is valid for
    // the root in cert_chain.
    ScopedTestEVPolicy scoped_ev_policy_1(EVRootCAMetadata::GetInstance(), root_fingerprint, "1.2.6.7");
    ScopedTestEVPolicy scoped_ev_policy_2(EVRootCAMetadata::GetInstance(), SHA256HashValue(), "1.2.3.4");

    scoped_refptr<MockCertVerifyProc> verify_proc1 = base::MakeRefCounted<MockCertVerifyProc>();
    // Primary verifier returns ok status and different_chain if verifying leaf
    // alone.
    EXPECT_CALL(*verify_proc1, VerifyInternal(leaf.get(), _, _, _, _, _, _, _, _)).WillOnce(DoAll(SetArgPointee<7>(different_chain_result), Return(OK)));
    // Primary verifier returns ok status and nonev_chain_result if verifying
    // cert_chain.
    EXPECT_CALL(*verify_proc1, VerifyInternal(cert_chain.get(), _, _, _, _, _, _, _, _)).WillOnce(DoAll(SetArgPointee<7>(nonev_chain_result), Return(OK)));

    // Trial verifier returns ok status and ev_chain_result.
    scoped_refptr<FakeCertVerifyProc> verify_proc2 = base::MakeRefCounted<FakeCertVerifyProc>(OK, ev_chain_result);
...
TEST_F(TrialComparisonCertVerifierTest, BothVerifiersOkDifferentCertStatus)
{
    ...
    EXPECT_EQ(CERT_STATUS_IS_EV | CERT_STATUS_REV_CHECKING_ENABLED, report.primary_result.cert_status);
...
TEST_F(TrialComparisonCertVerifierTest, CancelledDuringPrimaryVerification)
{
    ...
    CertVerifyResult result;
    std::unique_ptr<CertVerifier::Request> request;
    int error = verifier.Verify(params, &result, base::BindOnce(&NotCalledCallback), &request, NetLogWithSource());
    ASSERT_THAT(error, IsError(ERR_IO_PENDING));
    EXPECT_TRUE(request);
...
TEST_F(TrialComparisonCertVerifierTest, DeletedDuringVerificationResult)
{
    ...
    TestCompletionCallback callback;
    std::unique_ptr<CertVerifier::Request> request;
    int error = verifier->Verify(params, &result, base::BindLambdaForTesting([&callback, &verifier](int result) {
...
TEST_F(TrialComparisonCertVerifierTest, DeletedDuringTrialReport)
{
    ...
    verifier = std::make_unique<TrialComparisonCertVerifier>(verify_proc1, nullptr, verify_proc2, nullptr,
        base::BindLambdaForTesting([&verifier, &was_report_callback_called](const std::string& hostname, const scoped_refptr<X509Certificate>& unverified_cert,
                                       bool enable_rev_checking, bool require_rev_checking_local_anchors, bool enable_sha1_local_anchors,
                                       bool disable_symantec_enforcement, const std::string& stapled_ocsp, const std::string& sct_list,
                                       const net::CertVerifyResult& primary_result, const net::CertVerifyResult& trial_result) {
...
TEST_F(TrialComparisonCertVerifierTest, DeletedAfterTrialVerificationStarted)
{
    ...
    std::vector<TrialReportInfo> reports;
    auto verifier
        = std::make_unique<TrialComparisonCertVerifier>(verify_proc1, nullptr, verify_proc2, nullptr, base::BindRepeating(&RecordTrialReport, &reports));
    verifier->set_trial_allowed(true);
...
TEST_F(TrialComparisonCertVerifierTest, MacUndesiredRevocationChecking)
{
    ...
    // Primary verifier returns an error status.
    scoped_refptr<FakeCertVerifyProc> verify_proc1 = base::MakeRefCounted<FakeCertVerifyProc>(ERR_CERT_REVOKED, revoked_result);

    scoped_refptr<MockCertVerifyProc> verify_proc2 = base::MakeRefCounted<MockCertVerifyProc>();
    // Secondary verifier returns ok status...
    EXPECT_CALL(*verify_proc2, VerifyInternal(_, _, _, _, _, _, _, _, _)).WillOnce(DoAll(SetArgPointee<7>(ok_result), Return(OK)));
    ...
    EXPECT_CALL(*verify_proc2, VerifyInternal(_, _, _, _, CertVerifyProc::VERIFY_REV_CHECKING_ENABLED, _, _, _, _))
        .WillOnce(DoAll(SetArgPointee<7>(revoked_result), Return(ERR_CERT_REVOKED)));
#endif
    ...
    histograms_.ExpectUniqueSample("Net.CertVerifier_TrialComparisonResult", TrialComparisonResult::kIgnoredMacUndesiredRevocationChecking, 1);
#else
    // Expect a report.
...
TEST_F(TrialComparisonCertVerifierTest, MultipleEVPolicies)
{
    ...
    CertVerifyResult secondary_result;
    secondary_result.verified_cert = cert_chain;
    secondary_result.cert_status = CERT_STATUS_IS_EV | CERT_STATUS_REV_CHECKING_ENABLED;
    scoped_refptr<FakeCertVerifyProc> verify_proc2 = base::MakeRefCounted<FakeCertVerifyProc>(OK, secondary_result);
    ...
    histograms_.ExpectTotalCount("Net.CertVerifier_Job_Latency_TrialPrimary", 1);
    histograms_.ExpectTotalCount("Net.CertVerifier_Job_Latency_TrialSecondary", 1);
    histograms_.ExpectUniqueSample("Net.CertVerifier_TrialComparisonResult", TrialComparisonResult::kIgnoredMultipleEVPoliciesAndOneMatchesRoot, 1);
}
...
TEST_F(TrialComparisonCertVerifierTest, MultipleEVPoliciesNoneValidForRoot)
{
    ...
    // root in this chain.
    ScopedTestEVPolicy scoped_ev_policy_1(EVRootCAMetadata::GetInstance(), { 1 }, "1.2.6.7");
    ScopedTestEVPolicy scoped_ev_policy_2(EVRootCAMetadata::GetInstance(), { 2 }, "1.2.3.4");
...
TEST_F(TrialComparisonCertVerifierTest, LocallyTrustedLeaf)
{
    ...
    scoped_refptr<FakeCertVerifyProc> verify_proc2 = base::MakeRefCounted<FakeCertVerifyProc>(ERR_CERT_AUTHORITY_INVALID, secondary_result);
    ...
    histograms_.ExpectTotalCount("Net.CertVerifier_Job_Latency_TrialPrimary", 1);
    histograms_.ExpectTotalCount("Net.CertVerifier_Job_Latency_TrialSecondary", 1);
    histograms_.ExpectUniqueSample("Net.CertVerifier_TrialComparisonResult", TrialComparisonResult::kIgnoredLocallyTrustedLeaf, 1);
}
...
TEST_F(TrialComparisonCertVerifierTest, SHA1Ignored)
{
    CertVerifyResult primary_result;
    primary_result.verified_cert = cert_chain_1_;
    primary_result.cert_status = CERT_STATUS_SHA1_SIGNATURE_PRESENT | CERT_STATUS_REV_CHECKING_ENABLED;
    scoped_refptr<FakeCertVerifyProc> verify_proc1 = base::MakeRefCounted<FakeCertVerifyProc>(ERR_CERT_WEAK_SIGNATURE_ALGORITHM, primary_result);

    CertVerifyResult secondary_result;
    secondary_result.verified_cert = cert_chain_1_;
    secondary_result.cert_status = CERT_STATUS_SHA1_SIGNATURE_PRESENT;
    scoped_refptr<FakeCertVerifyProc> verify_proc2 = base::MakeRefCounted<FakeCertVerifyProc>(ERR_CERT_WEAK_SIGNATURE_ALGORITHM, secondary_result);
    ...
    histograms_.ExpectTotalCount("Net.CertVerifier_Job_Latency_TrialPrimary", 1);
    histograms_.ExpectTotalCount("Net.CertVerifier_Job_Latency_TrialSecondary", 1);
    histograms_.ExpectUniqueSample("Net.CertVerifier_TrialComparisonResult", TrialComparisonResult::kIgnoredSHA1SignaturePresent, 1);
}
...
TEST_F(TrialComparisonCertVerifierTest, BothAuthorityInvalidIgnored)
{
    CertVerifyResult primary_result;
    primary_result.verified_cert = cert_chain_1_;
    primary_result.cert_status = CERT_STATUS_SHA1_SIGNATURE_PRESENT | CERT_STATUS_AUTHORITY_INVALID;
    scoped_refptr<FakeCertVerifyProc> verify_proc1 = base::MakeRefCounted<FakeCertVerifyProc>(ERR_CERT_AUTHORITY_INVALID, primary_result);
    ...
    histograms_.ExpectTotalCount("Net.CertVerifier_Job_Latency_TrialPrimary", 1);
    histograms_.ExpectTotalCount("Net.CertVerifier_Job_Latency_TrialSecondary", 1);
    histograms_.ExpectUniqueSample("Net.CertVerifier_TrialComparisonResult", TrialComparisonResult::kIgnoredBothAuthorityInvalid, 1);
}
...
TEST_F(TrialComparisonCertVerifierTest, BothKnownRootsIgnored)
{
    ...
    histograms_.ExpectTotalCount("Net.CertVerifier_Job_Latency_TrialPrimary", 1);
    histograms_.ExpectTotalCount("Net.CertVerifier_Job_Latency_TrialSecondary", 1);
    histograms_.ExpectUniqueSample("Net.CertVerifier_TrialComparisonResult", TrialComparisonResult::kIgnoredBothKnownRoot, 1);
}
...
TEST_F(TrialComparisonCertVerifierTest, RevCheckingIgnoredWin)
{
    CertVerifyResult primary_result;
    primary_result.verified_cert = cert_chain_1_;
    primary_result.cert_status = CERT_STATUS_DATE_INVALID | CERT_STATUS_REV_CHECKING_ENABLED;
    scoped_refptr<FakeCertVerifyProc> verify_proc1 = base::MakeRefCounted<FakeCertVerifyProc>(ERR_CERT_WEAK_SIGNATURE_ALGORITHM, primary_result);

    CertVerifyResult secondary_result;
    secondary_result.verified_cert = cert_chain_1_;
    secondary_result.cert_status = CERT_STATUS_DATE_INVALID;
    scoped_refptr<FakeCertVerifyProc> verify_proc2 = base::MakeRefCounted<FakeCertVerifyProc>(ERR_CERTIFICATE_TRANSPARENCY_REQUIRED, secondary_result);
    ...
    histograms_.ExpectTotalCount("Net.CertVerifier_Job_Latency_TrialPrimary", 1);
    histograms_.ExpectTotalCount("Net.CertVerifier_Job_Latency_TrialSecondary", 1);
    histograms_.ExpectUniqueSample("Net.CertVerifier_TrialComparisonResult", TrialComparisonResult::kIgnoredWindowsRevCheckingEnabled, 1);
}
#endif
...
TEST_F(TrialComparisonCertVerifierTest, IgnoreAuthInvalidBuiltinWhenPrimaryReportSymantec)
{
    CertVerifyResult primary_result;
    primary_result.verified_cert = cert_chain_1_;
    primary_result.cert_status = CERT_STATUS_DATE_INVALID | CERT_STATUS_SYMANTEC_LEGACY;
    primary_result.is_issued_by_known_root = true;
    scoped_refptr<FakeCertVerifyProc> verify_proc1 = base::MakeRefCounted<FakeCertVerifyProc>(ERR_CERT_COMMON_NAME_INVALID, primary_result);
    ...
    histograms_.ExpectTotalCount("Net.CertVerifier_Job_Latency_TrialPrimary", 1);
    histograms_.ExpectTotalCount("Net.CertVerifier_Job_Latency_TrialSecondary", 1);
    histograms_.ExpectUniqueSample("Net.CertVerifier_TrialComparisonResult", TrialComparisonResult::kIgnoredBuiltinAuthorityInvalidPlatformSymantec, 1);
}
...
TEST_F(TrialComparisonCertVerifierTest, LetsEncryptSpecialCase)
{
    ...
    histograms_.ExpectTotalCount("Net.CertVerifier_Job_Latency_TrialPrimary", 1);
    histograms_.ExpectTotalCount("Net.CertVerifier_Job_Latency_TrialSecondary", 1);
    histograms_.ExpectUniqueSample("Net.CertVerifier_TrialComparisonResult", TrialComparisonResult::kIgnoredLetsEncryptExpiredRoot, 1);
}
