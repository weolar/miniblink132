...
public:
    CachingCertVerifierTest()
        : verifier_(std::make_unique<MockCertVerifier>())
    {
    }
...
TEST_F(CachingCertVerifierTest, CacheHit)
{
    ...
    error = callback.GetResult(verifier_.Verify(CertVerifier::RequestParams(test_cert, "www.example.com", 0,
...
TEST_F(CachingCertVerifierTest, CacheHitCTResultsCached)
{
    ...
    result = callback.GetResult(cache_verifier.Verify(CertVerifier::RequestParams(test_cert, "www.example.com", 0,
                                                          /*ocsp_response=*/std::string(),
                                                          /*sct_list=*/std::string()),
        &verify_result, callback.callback(), &request, NetLogWithSource()));
    ASSERT_EQ(OK, result);
    ...
    result = cache_verifier.Verify(CertVerifier::RequestParams(test_cert, "www.example.com", 0,
                                       /*ocsp_response=*/std::string(),
                                       /*sct_list=*/std::string()),
        &verify_result, callback.callback(), &request, NetLogWithSource());
    // Synchronous completion.
...
TEST_F(CachingCertVerifierTest, DifferentCACerts)
{
    ...
    error = callback.GetResult(verifier_.Verify(CertVerifier::RequestParams(cert_chain1, "www.example.com", 0,
                                                    /*ocsp_response=*/std::string(),
                                                    /*sct_list=*/std::string()),
        &verify_result, callback.callback(), &request, NetLogWithSource()));
    ASSERT_TRUE(IsCertificateError(error));
    ...
    error = callback.GetResult(verifier_.Verify(CertVerifier::RequestParams(cert_chain2, "www.example.com", 0,
                                                    /*ocsp_response=*/std::string(),
                                                    /*sct_list=*/std::string()),
        &verify_result, callback.callback(), &request, NetLogWithSource()));
    ASSERT_TRUE(IsCertificateError(error));
...
class CachingCertVerifierCacheClearingTest : public testing::TestWithParam<ChangeType> {
public:
    CachingCertVerifierCacheClearingTest()
    {
        auto mock_cert_verifier = std::make_unique<MockCertVerifier>();
        mock_verifier_ = mock_cert_verifier.get();
        verifier_ = std::make_unique<CachingCertVerifier>(std::move(mock_cert_verifier));
    }

    ChangeType change_type() const
    {
        return GetParam();
    }
