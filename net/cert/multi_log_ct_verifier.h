...
public:
    explicit MultiLogCTVerifier(const std::vector<scoped_refptr<const CTLogVerifier>>& log_verifiers);
    ...
    void Verify(X509Certificate* cert, std::string_view stapled_ocsp_response, std::string_view sct_list_from_tls_extension, base::Time current_time,
        SignedCertificateTimestampAndStatusList* output_scts, const NetLogWithSource& net_log) const override;
...
private:
    ...
    void VerifySCTs(std::string_view encoded_sct_list, const ct::SignedEntryData& expected_entry, ct::SignedCertificateTimestamp::Origin origin,
    ...
    bool VerifySingleSCT(scoped_refptr<ct::SignedCertificateTimestamp> sct, const ct::SignedEntryData& expected_entry, base::Time current_time,
