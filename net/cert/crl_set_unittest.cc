...
{
    std::string_view s(reinterpret_cast<const char*>(kGIACRLSet), sizeof(kGIACRLSet));
    ...
    EXPECT_EQ(std::string("\x10\x0D\x7F\x30\x00\x03\x00\x00\x23\xB0", 10), serials[0]);
    EXPECT_EQ(std::string("\x64\x63\x49\xD2\x00\x03\x00\x00\x1D\x77", 10), serials[kExpectedNumSerials - 1]);

    const std::string gia_spki_hash(reinterpret_cast<const char*>(kGIASPKISHA256), sizeof(kGIASPKISHA256));
    EXPECT_EQ(CRLSet::REVOKED, set->CheckSerial(std::string("\x16\x7D\x75\x9D\x00\x03\x00\x00\x14\x55", 10), gia_spki_hash));
    EXPECT_EQ(CRLSet::GOOD, set->CheckSerial(std::string("\x47\x54\x3E\x79\x00\x03\x00\x00\x14\xF5", 10), gia_spki_hash));
...
{
    std::string_view s(reinterpret_cast<const char*>(kBlockedSPKICRLSet), sizeof(kBlockedSPKICRLSet));
    scoped_refptr<CRLSet> set;
    EXPECT_TRUE(CRLSet::Parse(s, &set));
    ASSERT_TRUE(set.get() != nullptr);
    ...
    EXPECT_EQ(CRLSet::REVOKED, set->CheckSPKI(reinterpret_cast<const char*>(spki_hash)));
}
...
TEST(CertVerifyProcTest, CRLSetIncorporatesStaticBlocklist)
{
    ...
    for (size_t i = 0; kDigiNotarFilenames[i]; i++) {
        ...
        ASSERT_TRUE(asn1::ExtractSPKIFromDERCert(x509_util::CryptoBufferAsStringPiece(diginotar_cert->cert_buffer()), &spki));

        std::string spki_sha256 = crypto::SHA256HashString(spki);

        EXPECT_EQ(CRLSet::REVOKED, set1->CheckSPKI(spki_sha256)) << "Public key not blocked for " << kDigiNotarFilenames[i];
        EXPECT_EQ(CRLSet::REVOKED, set2->CheckSPKI(spki_sha256)) << "Public key not blocked for " << kDigiNotarFilenames[i];
    }
}
...
TEST(CRLSetTest, BlockedSubjects)
{
    ...
    EXPECT_TRUE(base::ReadFileToString(GetTestCertsDirectory().AppendASCII("crlset_by_root_subject.raw"), &crl_set_bytes));
    scoped_refptr<CRLSet> set;
    EXPECT_TRUE(CRLSet::Parse(crl_set_bytes, &set));
    ASSERT_TRUE(set.get() != nullptr);

    scoped_refptr<X509Certificate> root = CreateCertificateChainFromFile(GetTestCertsDirectory(), "root_ca_cert.pem", X509Certificate::FORMAT_AUTO);
    std::string_view root_der = net::x509_util::CryptoBufferAsStringPiece(root->cert_buffer());
    ...
    // hash.
    EXPECT_EQ(CRLSet::REVOKED, set->CheckSubject(subject, std::string_view(reinterpret_cast<const char*>(kGIASPKISHA256), sizeof(kGIASPKISHA256))));

    // When used with the correct hash, that subject should be accepted.
    EXPECT_EQ(CRLSet::GOOD, set->CheckSubject(subject, std::string_view(reinterpret_cast<const char*>(spki_sha256.data), sizeof(spki_sha256.data))));
}
...
TEST(CRLSetTest, Expired)
{
    ...
    // 1970.
    std::string_view s(reinterpret_cast<const char*>(kExpiredCRLSet), sizeof(kExpiredCRLSet));
    scoped_refptr<CRLSet> set;
    EXPECT_TRUE(CRLSet::Parse(s, &set));
    ASSERT_TRUE(set.get() != nullptr);
