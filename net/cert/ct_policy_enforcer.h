...
class NET_EXPORT CTPolicyEnforcer : public base::RefCountedThreadSafe<CTPolicyEnforcer> {
public:
    ...
    virtual ct::CTPolicyCompliance CheckCompliance(
        X509Certificate* cert, const ct::SCTList& verified_scts, base::Time current_time, const NetLogWithSource& net_log) const
        = 0;
    ...
    virtual std::optional<base::Time> GetLogDisqualificationTime(std::string_view log_id) const = 0;
...
public:
    ...
    ct::CTPolicyCompliance CheckCompliance(
        X509Certificate* cert, const ct::SCTList& verified_scts, base::Time current_time, const NetLogWithSource& net_log) const override;

    std::optional<base::Time> GetLogDisqualificationTime(std::string_view log_id) const override;
