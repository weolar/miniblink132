...
TEST_F(CoalescingCertVerifierTest, SyncCompletion)
{
    ...
    std::unique_ptr<MockCertVerifier> mock_verifier_owner = std::make_unique<MockCertVerifier>();
    ...
    int error = verifier.Verify(request_params, &result1, callback1.callback(), &request1, NetLogWithSource());
...
TEST_F(CoalescingCertVerifierTest, InflightJoin)
{
    ...
    error = verifier.Verify(request_params, &result2, callback2.callback(), &request2, NetLogWithSource());
    ASSERT_THAT(error, IsError(ERR_IO_PENDING));
...
TEST_F(CoalescingCertVerifierTest, DeleteSecondRequestDuringFirstCompletion)
{
    ...
    // called.
    int error = verifier.Verify(request_params, &result1, base::BindLambdaForTesting([&callback1, &request2](int result) {
        request2.reset();
...
TEST_F(CoalescingCertVerifierTest, DeleteVerifierDuringCompletion)
{
    ...
    auto verifier = std::make_unique<CoalescingCertVerifier>(std::move(mock_verifier_owner));
    ...
    // it will delete (reset) |request2|, which should prevent it from being
    // called.
    int error = verifier->Verify(request_params, &result1, base::BindLambdaForTesting([&callback1, &verifier](int result) {
        verifier.reset();
        callback1.callback().Run(result);
    }),
        &request1, NetLogWithSource());
...
// still notified.
TEST_F(CoalescingCertVerifierTest, DeleteFirstRequestBeforeCompletionStillCompletesSecondRequest)
{
    scoped_refptr<X509Certificate> test_cert(ImportCertFromFile(GetTestCertsDirectory(), "ok_cert.pem"));
    ASSERT_TRUE(test_cert);
...
TEST_F(CoalescingCertVerifierTest, DeleteRequestDuringCompletion)
{
    ...
    // Start an (asynchronous) initial request.
    int error = verifier.Verify(request_params, &result1, base::BindLambdaForTesting([&callback1, &request1](int result) {
