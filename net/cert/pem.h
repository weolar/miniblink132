...
public:
    ...
    PEMTokenizer(const base::StringPiece& str, const std::vector<std::string>& allowed_block_types);
...
private:
    void Init(const base::StringPiece& str, const std::vector<std::string>& allowed_block_types);
...
NET_EXPORT_PRIVATE std::string PEMEncode(base::StringPiece data, const std::string& type);
