...
{
    base::apple::ScopedCFTypeRef<SecCertificateRef> os_cert(x509_util::CreateSecCertificateFromX509Certificate(certificate));
    if (!os_cert) {
        return false;
    }

    if (CFArrayContainsValue(temporary_roots_.get(), CFRangeMake(0, CFArrayGetCount(temporary_roots_.get())), os_cert.get())) {
        return true;
    }
...
OSStatus TestRootCerts::FixupSecTrustRef(SecTrustRef trust_ref) const
{
    ...
    OSStatus status = SecTrustSetAnchorCertificates(trust_ref, temporary_roots_.get());
    if (status) {
        return status;
    }
...
{
    temporary_roots_.reset(CFArrayCreateMutable(kCFAllocatorDefault, 0, &kCFTypeArrayCallBacks));
}
