...
TEST(CertVerifyProcMacTest, DISABLED_MacCRLIntermediate)
{
    ...
    intermediates.push_back(bssl::UpRef(path_2_certs[1]->cert_buffer())); // B-by-C
    intermediates.push_back(bssl::UpRef(path_2_certs[2]->cert_buffer())); // C-by-E
    scoped_refptr<X509Certificate> cert = X509Certificate::CreateFromBuffer(bssl::UpRef(path_3_certs[0]->cert_buffer()), std::move(intermediates));
    ASSERT_TRUE(cert);
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
    ...
    base::FilePath keychain_path(GetTestCertsDirectory().AppendASCII("multi-root-BFE.keychain"));
    ...
    OSStatus status = SecKeychainOpen(keychain_path.MaybeAsASCII().c_str(), &keychain);
...
#pragma clang diagnostic pop
    ...
    EXPECT_TRUE(base::ReadFileToString(GetTestCertsDirectory().AppendASCII("multi-root-crlset-C.raw"), &crl_set_bytes));
    ASSERT_TRUE(CRLSet::Parse(crl_set_bytes, &crl_set));

    int flags = 0;
    CertVerifyResult verify_result;

    scoped_refptr<CertVerifyProc> verify_proc = base::MakeRefCounted<CertVerifyProcMac>();
    int error = verify_proc->Verify(cert.get(), "127.0.0.1", /*ocsp_response=*/std::string(),
        /*sct_list=*/std::string(), flags, crl_set.get(), CertificateList(), &verify_result, NetLogWithSource());
    ...
    EXPECT_TRUE(expected_intermediate->EqualsExcludingChain(intermediate.get()))
        << "Expected: " << expected_intermediate->subject().common_name << " issued by " << expected_intermediate->issuer().common_name
...
TEST(CertVerifyProcMacTest, DISABLED_MacKeychainReordering)
{
    ...
    scoped_refptr<X509Certificate> cert
        = CreateCertificateChainFromFile(GetTestCertsDirectory(), "gms.hongleong.com.my-verisign-chain.pem", X509Certificate::FORMAT_AUTO);
    ASSERT_TRUE(cert);
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
    ...
    base::FilePath keychain_path(GetTestCertsDirectory().AppendASCII("verisign_class3_g5_crosssigned-trusted.keychain"));
    // SecKeychainOpen does not fail if the file doesn't exist, so assert it here
    // for easier debugging.
    ASSERT_TRUE(base::PathExists(keychain_path));
    SecKeychainRef keychain;
...
#pragma clang diagnostic pop
    ...
    CertVerifyResult verify_result;
    scoped_refptr<CertVerifyProc> verify_proc = base::MakeRefCounted<CertVerifyProcMac>();
    int error = verify_proc->Verify(cert.get(), "gms.hongleong.com.my", /*ocsp_response=*/std::string(),
        /*sct_list=*/std::string(), flags, CRLSet::BuiltinCRLSet().get(), CertificateList(), &verify_result, NetLogWithSource());
...
{
    const char* root_keychain_path = "/System/Library/Keychains/SystemRootCertificates.keychain";
...
TEST(CertVerifyProcMacTest, LargeKey)
{
    ...
    ScopedTestRoot test_root(ImportCertFromFile(GetTestCertsDirectory(), "root_ca_cert.pem").get());

    scoped_refptr<X509Certificate> cert(ImportCertFromFile(GetTestCertsDirectory(), "large_key.pem"));
...
TEST(CertVerifyProcMacTest, CertValidityTooLong)
{
    ...
    scoped_refptr<X509Certificate> cert(ImportCertFromFile(GetTestCertsDirectory(), "900_days_after_2019_07_01.pem"));
    ...
    if (base::mac::IsAtLeastOS10_15()) {
        ...
        EXPECT_EQ((verify_result.cert_status & CERT_STATUS_ALL_ERRORS), CERT_STATUS_VALIDITY_TOO_LONG);
    } else {
