...
unsigned TrustTypeToNSSTrust(bssl::CertificateTrustType trust)
{
    ...
    case bssl::CertificateTrustType::TRUSTED_ANCHOR_OR_LEAF:
        return CERTDB_TRUSTED_CA | CERTDB_VALID_CA | CERTDB_TRUSTED | CERTDB_TERMINAL_RECORD;
    }
}
...
std::shared_ptr<const bssl::ParsedCertificate> GetASSLTrustedBuiltinRoot()
{
    ...
    scoped_refptr<X509Certificate> ssl_trusted_root = x509_util::CreateX509CertificateFromCERTCertificate(nss_cert.get());
    if (!ssl_trusted_root) {
        return nullptr;
    }
    return bssl::ParsedCertificate::Create(bssl::UpRef(ssl_trusted_root->cert_buffer()), x509_util::DefaultParseCertificateOptions(), &parsing_errors);
}

std::optional<unsigned> GetNSSTrustForCert(const bssl::ParsedCertificate* cert)
{
    SECItem der_cert;
    der_cert.data = const_cast<uint8_t*>(cert->der_cert().data());
    der_cert.len = base::checked_cast<unsigned>(cert->der_cert().size());
    der_cert.type = siDERCertBuffer;
    ScopedCERTCertificate nss_cert(CERT_FindCertByDERCert(CERT_GetDefaultCertDB(), &der_cert));
    if (!nss_cert) {
        return std::nullopt;
    }
...
public:
    ...
    {
        return bssl::CertificateTrust::ForTrustAnchor().WithEnforceAnchorConstraints().WithEnforceAnchorExpiry();
    }

    bssl::CertificateTrust ExpectedTrustForAnchorOrLeaf() const
    {
        return bssl::CertificateTrust::ForTrustAnchorOrLeaf().WithEnforceAnchorConstraints().WithEnforceAnchorExpiry();
    }
    ...
    void SetUp() override
    {
        ...
        ReadCertChainFromFile("net/data/verify_certificate_chain_unittest/key-rollover/oldchain.pem", &chain);
    ...
    {
        return "trust_store_nss_unittest" + base::NumberToString(nickname_counter_++);
    }

    void AddCertToNSSSlot(const bssl::ParsedCertificate* cert, PK11SlotInfo* slot)
    {
        ScopedCERTCertificate nss_cert(x509_util::CreateCERTCertificateFromBytes(cert->der_cert()));
        ASSERT_TRUE(nss_cert);
        SECStatus srv = PK11_ImportCert(slot, nss_cert.get(), CK_INVALID_HANDLE, GetUniqueNickname().c_str(), PR_FALSE /* includeTrust (unused) */);
    ...
    // for `cert`.)
    void AddCertToNSSSlotWithTrust(const bssl::ParsedCertificate* cert, PK11SlotInfo* slot, bssl::CertificateTrustType trust)
    {
    ...
    void AddCertsToNSS()
    {
        ...
        EXPECT_TRUE(TrustStoreContains(target_, { newintermediate_, oldintermediate_ }));

        EXPECT_TRUE(TrustStoreContains(newintermediate_, { newroot_, newrootrollover_, oldroot_ }));
        EXPECT_TRUE(TrustStoreContains(oldintermediate_, { newroot_, newrootrollover_, oldroot_ }));
        EXPECT_TRUE(TrustStoreContains(newrootrollover_, { newroot_, newrootrollover_, oldroot_ }));
        EXPECT_TRUE(TrustStoreContains(oldroot_, { newroot_, newrootrollover_, oldroot_ }));
        EXPECT_TRUE(TrustStoreContains(newroot_, { newroot_, newrootrollover_, oldroot_ }));
    }
    ...
    {
        ChangeCertTrust(cert, CERTDB_TERMINAL_RECORD | CERTDB_TRUSTED | CERTDB_TRUSTED_CA | CERTDB_VALID_CA);
    }
    ...
    void ChangeCertTrust(const bssl::ParsedCertificate* cert, int flags)
    {
        ...
        SECStatus srv = CERT_ChangeCertTrust(CERT_GetDefaultCertDB(), nss_cert.get(), &trust);
        ASSERT_EQ(SECSuccess, srv);
    }
    ...
    void ChangeCertTrustInSlot(const bssl::ParsedCertificate* cert, PK11SlotInfo* slot, bssl::CertificateTrustType trust)
    {
        crypto::ScopedCERTCertList cert_list(PK11_ListCertsInSlot(slot));
        ASSERT_TRUE(cert_list);

        for (CERTCertListNode* node = CERT_LIST_HEAD(cert_list); !CERT_LIST_END(node, cert_list); node = CERT_LIST_NEXT(node)) {
            if (x509_util::IsSameCertificate(node->cert, cert->cert_buffer())) {
                ...
                if (CERT_ChangeCertTrust(CERT_GetDefaultCertDB(), node->cert, &nss_trust) != SECSuccess) {
...
protected:
    bool TrustStoreContains(std::shared_ptr<const bssl::ParsedCertificate> cert, bssl::ParsedCertificateList expected_matches)
    {
    ...
    // their symbolic name).
    std::string GetCertString(const std::shared_ptr<const bssl::ParsedCertificate>& cert) const
    {
    ...
    bool HasTrust(const bssl::ParsedCertificateList& certs, bssl::CertificateTrust expected_trust)
    {
        bool success = true;
...
enum class SlotFilterType { kDontFilter, kAllowSpecifiedUserSlot };
...
class TrustStoreNSSTestWithSlotFilterType : public TrustStoreNSSTestBase, public testing::WithParamInterface<SlotFilterType> {
public:
    TrustStoreNSSTestWithSlotFilterType() = default;
    ~TrustStoreNSSTestWithSlotFilterType() override = default;

    SlotFilterType slot_filter_type() const
    {
        return GetParam();
    }
    ...
    std::unique_ptr<TrustStoreNSS> CreateTrustStoreNSS() override
    {
        switch (slot_filter_type()) {
        case SlotFilterType::kDontFilter:
            return std::make_unique<TrustStoreNSS>(TrustStoreNSS::UseTrustFromAllUserSlots());
        case SlotFilterType::kAllowSpecifiedUserSlot:
            return std::make_unique<TrustStoreNSS>(crypto::ScopedPK11Slot(PK11_ReferenceSlot(test_nssdb_.slot())));
        }
    }
};
...
TEST_P(TrustStoreNSSTestWithSlotFilterType, CertsNotPresent)
{
    ...
    EXPECT_TRUE(TrustStoreContains(newintermediate_, bssl::ParsedCertificateList()));
    ...
    EXPECT_TRUE(HasTrust({ newintermediate_ }, bssl::CertificateTrust::ForUnspecified()));
...
{
    ScopedCERTCertificate temp_nss_cert(x509_util::CreateCERTCertificateFromBytes(newintermediate_->der_cert()));
...
TEST_P(TrustStoreNSSTestWithSlotFilterType, TrustAllowedForBuiltinRootCerts)
{
    ...
    EXPECT_TRUE(HasTrust({ builtin_root_cert }, bssl::CertificateTrust::ForUnspecified()));
}
...
TEST_P(TrustStoreNSSTestWithSlotFilterType, ListCertsIgnoresBuiltinRoots)
{
    ...
    for (const auto& result : trust_store_nss_->TrustStoreNSS::ListCertsIgnoringNSSRoots()) {
        EXPECT_FALSE(x509_util::IsSameCertificate(result.cert.get(), root_cert.get()));
    }

    for (const auto& cert_with_trust : trust_store_nss_->GetAllUserAddedCerts()) {
        EXPECT_FALSE(x509_util::IsSameCertificate(x509_util::CreateCryptoBuffer(cert_with_trust.cert_bytes).get(), root_cert.get()));
    }
}
...
{
    scoped_refptr<X509Certificate> client_cert = ImportClientCertAndKeyFromFile(GetTestCertsDirectory(), "client_1.pem", "client_1.pk8", test_nssdb_.slot());
    ...
    for (const auto& cert_with_trust : trust_store_nss_->GetAllUserAddedCerts()) {
        EXPECT_FALSE(x509_util::CryptoBufferEqual(x509_util::CreateCryptoBuffer(cert_with_trust.cert_bytes).get(), client_cert->cert_buffer()));
    }
}

// Check that GetAllUserAddedCerts will return a client cert that has had trust
// bits added for server auth.
TEST_P(TrustStoreNSSTestWithSlotFilterType, GetAllUserAddedCertsManualTrustClientCert)
{
    scoped_refptr<X509Certificate> client_cert = ImportClientCertAndKeyFromFile(GetTestCertsDirectory(), "client_1.pem", "client_1.pk8", test_nssdb_.slot());
    ASSERT_TRUE(client_cert);
    std::shared_ptr<const bssl::ParsedCertificate> parsed_client_cert
        = bssl::ParsedCertificate::Create(bssl::UpRef(client_cert->cert_buffer()), x509_util::DefaultParseCertificateOptions(), nullptr);
...

    {
        bool found = false;
        for (const auto& cert_with_trust : trust_store_nss_->GetAllUserAddedCerts()) {
            found |= x509_util::CryptoBufferEqual(x509_util::CreateCryptoBuffer(cert_with_trust.cert_bytes).get(), client_cert->cert_buffer());
        }
        EXPECT_TRUE(found);
    }
}

INSTANTIATE_TEST_SUITE_P(All, TrustStoreNSSTestWithSlotFilterType, ::testing::Values(SlotFilterType::kDontFilter, SlotFilterType::kAllowSpecifiedUserSlot),
    [](const testing::TestParamInfo<TrustStoreNSSTestWithSlotFilterType::ParamType>& info) { return SlotFilterTypeToString(info.param); });
...
{
    ScopedCERTCertificate nss_cert(x509_util::CreateCERTCertificateFromBytes(newroot_->der_cert()));
    EXPECT_TRUE(HasTrust({ newroot_ }, bssl::CertificateTrust::ForUnspecified()));
}
...
{
    std::shared_ptr<const bssl::ParsedCertificate> system_root = GetASSLTrustedBuiltinRoot();
    ASSERT_TRUE(system_root);
    EXPECT_TRUE(HasTrust({ system_root }, bssl::CertificateTrust::ForUnspecified()));
}
...
TEST_F(TrustStoreNSSTestIgnoreSystemCerts, SystemRootCertIgnoredWhenPresentInUserDb)
{
    std::shared_ptr<const bssl::ParsedCertificate> system_root = GetASSLTrustedBuiltinRoot();
    ASSERT_TRUE(system_root);
...
TEST_F(TrustStoreNSSTestIgnoreSystemCerts, UserDbTrustForSystemRootHonored)
{
    ...
    AddCertToNSSSlotWithTrust(system_root.get(), test_nssdb_.slot(), bssl::CertificateTrustType::TRUSTED_ANCHOR);
    // NSS should see the cert as trusted.
    EXPECT_EQ(CERTDB_TRUSTED_CA | CERTDB_VALID_CA, GetNSSTrustForCert(system_root.get()));
...
TEST_F(TrustStoreNSSTestIgnoreSystemCerts, UserDbLeafTrustForSystemRootHonored)
{
    std::shared_ptr<const bssl::ParsedCertificate> system_root = GetASSLTrustedBuiltinRoot();
    ASSERT_TRUE(system_root);

    // Add unrelated trust record to test that we find the correct one.
    AddCertToNSSSlotWithTrust(newroot_.get(), test_nssdb_.slot(), bssl::CertificateTrustType::TRUSTED_ANCHOR);

    // Trust the system cert as a leaf.
    AddCertToNSSSlotWithTrust(system_root.get(), test_nssdb_.slot(), bssl::CertificateTrustType::TRUSTED_LEAF);

    // Add unrelated trust record to test that we find the correct one.
    AddCertToNSSSlotWithTrust(newintermediate_.get(), test_nssdb_.slot(), bssl::CertificateTrustType::DISTRUSTED);

    // NSS should see the cert as a trusted leaf.
    EXPECT_EQ(CERTDB_TRUSTED | CERTDB_TERMINAL_RECORD, GetNSSTrustForCert(system_root.get()));
...
TEST_F(TrustStoreNSSTestIgnoreSystemCerts, UserDbAnchorAndLeafTrustForSystemRootHonored)
{
    std::shared_ptr<const bssl::ParsedCertificate> system_root = GetASSLTrustedBuiltinRoot();
    ASSERT_TRUE(system_root);

    AddCertToNSSSlotWithTrust(system_root.get(), test_nssdb_.slot(), bssl::CertificateTrustType::TRUSTED_ANCHOR_OR_LEAF);

    // NSS should see the cert as both trusted leaf and CA.
    EXPECT_EQ(CERTDB_TRUSTED_CA | CERTDB_VALID_CA | CERTDB_TRUSTED | CERTDB_TERMINAL_RECORD, GetNSSTrustForCert(system_root.get()));
...
TEST_F(TrustStoreNSSTestIgnoreSystemCerts, UserDbDistrustForSystemRootHonored)
{
    ...
    AddCertToNSSSlotWithTrust(system_root.get(), test_nssdb_.slot(), bssl::CertificateTrustType::DISTRUSTED);
...
// unspecified trust.
TEST_F(TrustStoreNSSTestIgnoreSystemCerts, UserDbUnspecifiedTrustForSystemRootHonored)
{
    std::shared_ptr<const bssl::ParsedCertificate> system_root = GetASSLTrustedBuiltinRoot();
    ASSERT_TRUE(system_root);

    AddCertToNSSSlotWithTrust(system_root.get(), test_nssdb_.slot(), bssl::CertificateTrustType::UNSPECIFIED);
...
TEST_F(TrustStoreNSSTestWithoutSlotFilter, CertsPresentButNotTrusted)
{
    ...
    EXPECT_TRUE(HasTrust({ oldroot_, newroot_, target_, oldintermediate_, newintermediate_, newrootrollover_ }, bssl::CertificateTrust::ForUnspecified()));
}
...
TEST_F(TrustStoreNSSTestWithoutSlotFilter, TrustedCA)
{
    ...
    EXPECT_TRUE(HasTrust({ oldroot_, target_, oldintermediate_, newintermediate_, newrootrollover_ }, bssl::CertificateTrust::ForUnspecified()));
...
TEST_F(TrustStoreNSSTestWithoutSlotFilter, TrustedIntermediate)
{
    ...
    EXPECT_TRUE(HasTrust({ oldroot_, newroot_, target_, oldintermediate_, newrootrollover_ }, bssl::CertificateTrust::ForUnspecified()));
...
TEST_F(TrustStoreNSSTestWithoutSlotFilter, DistrustedIntermediate)
{
    ...
    EXPECT_TRUE(HasTrust({ oldroot_, newroot_, target_, oldintermediate_, newrootrollover_ }, bssl::CertificateTrust::ForUnspecified()));
    EXPECT_TRUE(HasTrust({ newintermediate_ }, bssl::CertificateTrust::ForDistrusted()));
}
...
TEST_F(TrustStoreNSSTestWithoutSlotFilter, TrustedServer)
{
    ...
    EXPECT_TRUE(HasTrust({ oldroot_, newroot_, oldintermediate_, newintermediate_, newrootrollover_ }, bssl::CertificateTrust::ForUnspecified()));
    EXPECT_TRUE(HasTrust({ target_ }, ExpectedTrustForLeaf()));
}
...
TEST_F(TrustStoreNSSTestWithoutSlotFilter, MultipleTrustedCAWithSameSubject)
{
    ...
    EXPECT_TRUE(HasTrust({ target_, oldintermediate_, newintermediate_, newrootrollover_ }, bssl::CertificateTrust::ForUnspecified()));
...
{
    AddCertToNSSSlotWithTrust(oldroot_.get(), first_test_nssdb_.slot(), bssl::CertificateTrustType::DISTRUSTED);
    AddCertToNSSSlotWithTrust(newroot_.get(), test_nssdb_.slot(), bssl::CertificateTrustType::TRUSTED_LEAF);

    {
        bool found_newroot = false;
        bool found_oldroot = false;
        for (const auto& result : trust_store_nss_->TrustStoreNSS::ListCertsIgnoringNSSRoots()) {
            found_oldroot |= x509_util::IsSameCertificate(result.cert.get(), oldroot_->cert_buffer());
            found_newroot |= x509_util::IsSameCertificate(result.cert.get(), newroot_->cert_buffer());
        }
...

    {
        ...
        bool found_oldroot = false;
        for (const auto& cert_with_trust : trust_store_nss_->GetAllUserAddedCerts()) {
            found_oldroot |= x509_util::CryptoBufferEqual(x509_util::CreateCryptoBuffer(cert_with_trust.cert_bytes).get(), oldroot_->cert_buffer());
            found_newroot |= x509_util::CryptoBufferEqual(x509_util::CreateCryptoBuffer(cert_with_trust.cert_bytes).get(), newroot_->cert_buffer());
        }
        EXPECT_TRUE(found_newroot);
        EXPECT_TRUE(found_oldroot);
    }
...
{
    AddCertToNSSSlotWithTrust(newroot_.get(), other_test_nssdb_.slot(), bssl::CertificateTrustType::TRUSTED_ANCHOR);
    EXPECT_TRUE(HasTrust({ newroot_ }, bssl::CertificateTrust::ForUnspecified()));
}
...
TEST_F(TrustStoreNSSTestAllowSpecifiedUserSlot, CertOnMultipleSlots)
{
    ...
    AddCertToNSSSlotWithTrust(newroot_.get(), first_test_nssdb_.slot(), bssl::CertificateTrustType::DISTRUSTED);
    AddCertToNSSSlotWithTrust(newroot_.get(), test_nssdb_.slot(), bssl::CertificateTrustType::TRUSTED_ANCHOR);
    AddCertToNSSSlotWithTrust(newroot_.get(), other_test_nssdb_.slot(), bssl::CertificateTrustType::TRUSTED_LEAF);

    // Add unrelated trust record to test that we find the correct one.
    AddCertToNSSSlotWithTrust(target_.get(), test_nssdb_.slot(), bssl::CertificateTrustType::DISTRUSTED);
...
TEST_F(TrustStoreNSSTestAllowSpecifiedUserSlot, SystemRootCertOnMultipleSlots)
{
    ...
    AddCertToNSSSlotWithTrust(system_root.get(), first_test_nssdb_.slot(), bssl::CertificateTrustType::DISTRUSTED);
    AddCertToNSSSlotWithTrust(system_root.get(), test_nssdb_.slot(), bssl::CertificateTrustType::TRUSTED_LEAF);
    AddCertToNSSSlotWithTrust(system_root.get(), other_test_nssdb_.slot(), bssl::CertificateTrustType::UNSPECIFIED);
...
TEST_F(TrustStoreNSSTestAllowSpecifiedUserSlot, ListCertsFiltersBySlot)
{
...

    {
        bool found_newroot = false;
        for (const auto& result : trust_store_nss_->TrustStoreNSS::ListCertsIgnoringNSSRoots()) {
            EXPECT_FALSE(x509_util::IsSameCertificate(result.cert.get(), oldroot_->cert_buffer()));
            found_newroot |= x509_util::IsSameCertificate(result.cert.get(), newroot_->cert_buffer());
        }
        EXPECT_TRUE(found_newroot);
    }

    {
        bool found_newroot = false;
        for (const auto& cert_with_trust : trust_store_nss_->GetAllUserAddedCerts()) {
            EXPECT_FALSE(x509_util::CryptoBufferEqual(x509_util::CreateCryptoBuffer(cert_with_trust.cert_bytes).get(), oldroot_->cert_buffer()));
            found_newroot |= x509_util::CryptoBufferEqual(x509_util::CreateCryptoBuffer(cert_with_trust.cert_bytes).get(), newroot_->cert_buffer());
        }
        EXPECT_TRUE(found_newroot);
    }
...
public:
    ...
    void AddCert(std::shared_ptr<const bssl::ParsedCertificate> cert)
    {
        ...
        ASSERT_TRUE(nss_cert);
        SECStatus srv
            = PK11_ImportCert(test_nssdb_.slot(), nss_cert.get(), CK_INVALID_HANDLE, GetUniqueNickname().c_str(), PR_FALSE /* includeTrust (unused) */);
        ASSERT_EQ(SECSuccess, srv);
    }

    bssl::CertIssuerSource& source()
    {
        return trust_store_nss_;
    }

protected:
    std::string GetUniqueNickname()
    {
        return "cert_issuer_source_nss_unittest" + base::NumberToString(nickname_counter_++);
    }
...
INSTANTIATE_TYPED_TEST_SUITE_P(TrustStoreNSSTest2, CertIssuerSourceSyncTest, TrustStoreNSSTestDelegate);
// NSS doesn't normalize UTF8String values, so use the not-normalized version
// of those tests.
INSTANTIATE_TYPED_TEST_SUITE_P(TrustStoreNSSNotNormalizedTest, CertIssuerSourceSyncNotNormalizedTest, TrustStoreNSSTestDelegate);
} // namespace
