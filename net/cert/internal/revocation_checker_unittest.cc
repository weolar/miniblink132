...
bool AddCertsToList(std::vector<CertBuilder*> builders, bssl::ParsedCertificateList* out_certs)
{
    for (auto* builder : builders) {
        if (!bssl::ParsedCertificate::CreateAndAddToVector(builder->DupCertBuffer(), {}, out_certs, /*errors=*/nullptr)) {
            return false;
        }
    }
    return true;
...
TEST(RevocationChecker, NoRevocationMechanism)
{
...

    {
        ...
        CheckValidatedChainRevocation(chain, policy, /*deadline=*/base::TimeTicks(),
            /*stapled_leaf_ocsp_response=*/std::string_view(), base::Time::Now(), mock_fetcher.get(), &errors, /*stapled_ocsp_verify_result=*/nullptr);
...

    {
        ...
        bssl::CertPathErrors errors;
        CheckValidatedChainRevocation(chain, policy, /*deadline=*/base::TimeTicks(),
            /*stapled_leaf_ocsp_response=*/std::string_view(), base::Time::Now(), mock_fetcher.get(), &errors,
...
TEST(RevocationChecker, ValidCRL)
{
    ...
    std::string crl_data_as_string_for_some_reason = BuildCrl(root->GetSubject(), root->GetKey(),
        /*revoked_serials=*/ {});
    std::vector<uint8_t> crl_data(crl_data_as_string_for_some_reason.begin(), crl_data_as_string_for_some_reason.end());

    {
        ...
        auto mock_fetcher = base::MakeRefCounted<StrictMock<MockCertNetFetcher>>();
        EXPECT_CALL(*mock_fetcher, FetchCrl(kTestCrlUrl, _, _)).WillOnce(Return(ByMove(MockCertNetFetcherRequest::Create(crl_data))));
...

    {
        ...
        EXPECT_TRUE(errors.ContainsHighSeverityErrors());
        EXPECT_TRUE(errors.ContainsError(bssl::cert_errors::kUnableToCheckRevocation));
    }
...
TEST(RevocationChecker, CRLRequestFails)
{
...

    {
        ...
        auto mock_fetcher = base::MakeRefCounted<StrictMock<MockCertNetFetcher>>();
        EXPECT_CALL(*mock_fetcher, FetchCrl(kTestCrlUrl, _, _)).WillOnce(Return(ByMove(MockCertNetFetcherRequest::Create(ERR_CONNECTION_FAILED))));
...
TEST(RevocationChecker, SkipEntireInvalidCRLDistributionPoints)
{
    ...
    //   }
    // }
    const uint8_t crldp[] = { 0x30, 0x31, 0x30, 0x09, 0xa0, 0x07, 0xa0, 0x05, 0x89, 0x03, 0x66, 0x6f, 0x6f, 0x30, 0x24, 0xa0, 0x22, 0xa0, 0x20, 0x86, 0x1e,
        0x68, 0x74, 0x74, 0x70, 0x3a, 0x2f, 0x2f, 0x77, 0x77, 0x77, 0x2e, 0x65, 0x78, 0x61, 0x6d, 0x70, 0x6c, 0x65, 0x2e, 0x63, 0x6f, 0x6d, 0x2f, 0x62, 0x61,
        0x72, 0x2e, 0x63, 0x72, 0x6c };
    leaf->SetExtension(bssl::der::Input(bssl::kCrlDistributionPointsOid), std::string(reinterpret_cast<const char*>(crldp), std::size(crldp)));
...
TEST(RevocationChecker, SkipUnsupportedCRLDistPointWithNonUriFullname)
{
    ...
    //   }
    // }
    const uint8_t crldp[] = { 0x30, 0x4b, 0x30, 0x23, 0xa0, 0x21, 0xa0, 0x1f, 0xa4, 0x1d, 0x30, 0x1b, 0x31, 0x0b, 0x30, 0x09, 0x06, 0x03, 0x55, 0x04, 0x06,
        0x13, 0x02, 0x55, 0x53, 0x31, 0x0c, 0x30, 0x0a, 0x06, 0x03, 0x55, 0x04, 0x03, 0x13, 0x03, 0x66, 0x6f, 0x6f, 0x30, 0x24, 0xa0, 0x22, 0xa0, 0x20, 0x86,
        0x1e, 0x68, 0x74, 0x74, 0x70, 0x3a, 0x2f, 0x2f, 0x77, 0x77, 0x77, 0x2e, 0x65, 0x78, 0x61, 0x6d, 0x70, 0x6c, 0x65, 0x2e, 0x63, 0x6f, 0x6d, 0x2f, 0x62,
    ...
    auto mock_fetcher = base::MakeRefCounted<StrictMock<MockCertNetFetcher>>();
    EXPECT_CALL(*mock_fetcher, FetchCrl(kSecondCrlUrl, _, _)).WillOnce(Return(ByMove(MockCertNetFetcherRequest::Create(crl_data))));
...
TEST(RevocationChecker, SkipUnsupportedCRLDistPointWithReasons)
{
    ...
    //   }
    // }
    const uint8_t crldp[] = { 0x30, 0x50, 0x30, 0x28, 0xa0, 0x22, 0xa0, 0x20, 0x86, 0x1e, 0x68, 0x74, 0x74, 0x70, 0x3a, 0x2f, 0x2f, 0x77, 0x77, 0x77, 0x2e,
        0x65, 0x78, 0x61, 0x6d, 0x70, 0x6c, 0x65, 0x2e, 0x63, 0x6f, 0x6d, 0x2f, 0x66, 0x6f, 0x6f, 0x2e, 0x63, 0x72, 0x6c, 0x81, 0x02, 0x05, 0x60, 0x30, 0x24,
        0xa0, 0x22, 0xa0, 0x20, 0x86, 0x1e, 0x68, 0x74, 0x74, 0x70, 0x3a, 0x2f, 0x2f, 0x77, 0x77, 0x77, 0x2e, 0x65, 0x78, 0x61, 0x6d, 0x70, 0x6c, 0x65, 0x2e,
...
TEST(RevocationChecker, SkipUnsupportedCRLDistPointWithCrlIssuer)
{
    ...
    //   }
    // }
    const uint8_t crldp[] = {  };
    leaf->SetExtension(bssl::der::Input(bssl::kCrlDistributionPointsOid), std::string(reinterpret_cast<const char*>(crldp), std::size(crldp)));
