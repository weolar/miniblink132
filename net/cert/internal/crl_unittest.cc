...
struct PrintTestName {
    std::string operator()(const testing::TestParamInfo<const char*>& info) const
    {
...
INSTANTIATE_TEST_SUITE_P(All, CheckCRLTest, ::testing::ValuesIn(kTestParams), PrintTestName());
...
TEST_P(CheckCRLTest, FromFile)
{
    ...
    if (!ca_data_2.empty()) {
        scoped_refptr<ParsedCertificate> issuer_cert_2 = ParseCertificate(ca_data_2);
    ...
    if (cert->GetExtension(der::Input(kCrlDistributionPointsOid), &crl_dp_extension)) {
    ...
    CRLRevocationStatus revocation_status = CheckCRL(crl_data, certs, /*target_cert_index=*/0, *cert_dp, kVerifyTime, kAgeOneWeek);
    ...
    ASSERT_TRUE(ReadCertChainFromFile("net/data/ssl/certificates/ok_cert.pem", &other_certs));
    ASSERT_FALSE(other_certs.empty());
    certs.insert(certs.begin(), other_certs[0]);
    revocation_status = CheckCRL(crl_data, certs, /*target_cert_index=*/1, *cert_dp, kVerifyTime, kAgeOneWeek);
    EXPECT_EQ(expected_revocation_status, revocation_status);
}
