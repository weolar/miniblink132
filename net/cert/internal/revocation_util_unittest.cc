...
TEST(CheckRevocationDateTest, Valid)
{
    ...
    ASSERT_TRUE(der::EncodeTimeAsGeneralizedTime(this_update, &encoded_this_update));
    EXPECT_TRUE(CheckRevocationDateValid(encoded_this_update, nullptr, now, kOneWeek));

    base::Time next_update = this_update + base::Days(7);
    der::GeneralizedTime encoded_next_update;
    ASSERT_TRUE(der::EncodeTimeAsGeneralizedTime(next_update, &encoded_next_update));
    EXPECT_TRUE(CheckRevocationDateValid(encoded_this_update, &encoded_next_update, now, kOneWeek));
}
...
TEST(CheckRevocationDateTest, ThisUpdateInTheFuture)
{
    ...
    der::GeneralizedTime encoded_this_update;
    ASSERT_TRUE(der::EncodeTimeAsGeneralizedTime(this_update, &encoded_this_update));
    EXPECT_FALSE(CheckRevocationDateValid(encoded_this_update, nullptr, now, kOneWeek));

    base::Time next_update = this_update + base::Days(7);
    der::GeneralizedTime encoded_next_update;
    ASSERT_TRUE(der::EncodeTimeAsGeneralizedTime(next_update, &encoded_next_update));
    EXPECT_FALSE(CheckRevocationDateValid(encoded_this_update, &encoded_next_update, now, kOneWeek));
}
...
TEST(CheckRevocationDateTest, ThisUpdateOlderThanMaxAge)
{
    ...
    ASSERT_TRUE(der::EncodeTimeAsGeneralizedTime(this_update - base::Seconds(1), &encoded_this_update));
    EXPECT_FALSE(CheckRevocationDateValid(encoded_this_update, nullptr, now, kOneWeek));
    EXPECT_FALSE(CheckRevocationDateValid(encoded_this_update, &encoded_next_update, now, kOneWeek));
}
...
TEST(CheckRevocationDateTest, VerifyTimeFromBeforeWindowsEpoch)
{
    ...
    der::GeneralizedTime encoded_this_update;
    ASSERT_TRUE(der::EncodeTimeAsGeneralizedTime(this_update, &encoded_this_update));
    EXPECT_FALSE(CheckRevocationDateValid(encoded_this_update, nullptr, verify_time, kOneWeek));

    base::Time next_update = this_update + kOneWeek;
    der::GeneralizedTime encoded_next_update;
    ASSERT_TRUE(der::EncodeTimeAsGeneralizedTime(next_update, &encoded_next_update));
    EXPECT_FALSE(CheckRevocationDateValid(encoded_this_update, &encoded_next_update, verify_time, kOneWeek));
}
...
TEST(CheckRevocationDateTest, VerifyTimeMinusAgeFromBeforeWindowsEpoch)
{
    ...
    EXPECT_EQ(can_encode_before_windows_epoch, CheckRevocationDateValid(encoded_this_update, nullptr, verify_time, kOneWeek));
}
