...
{
    std::string der;
    ::testing::AssertionResult r = ReadTestPem("net/data/cert_issuer_source_static_unittest/" + file_name, "CERTIFICATE", &der);
    if (!r) {
        return r;
    }
    ...
    *result = bssl::ParsedCertificate::Create(
    ...
    if (!*result) {
        return ::testing::AssertionFailure() << "ParsedCertificate::Create() failed:\n" << errors.ToDebugString();
    }
...
template <typename TestDelegate> class CertIssuerSourceSyncTest : public ::testing::Test {
public:
    void SetUp() override
    {
    ...
    {
        return delegate_.source();
    }

protected:
    bool IssuersMatch(std::shared_ptr<const bssl::ParsedCertificate> cert, bssl::ParsedCertificateList expected_matches)
    {
...
// normalization.
REGISTER_TYPED_TEST_SUITE_P(CertIssuerSourceSyncTest, NoMatch, OneMatch, MultipleMatches, SelfIssued, IsNotAsync);

template <typename TestDelegate> class CertIssuerSourceSyncNormalizationTest : public CertIssuerSourceSyncTest<TestDelegate> { };
TYPED_TEST_SUITE_P(CertIssuerSourceSyncNormalizationTest);

TYPED_TEST_P(CertIssuerSourceSyncNormalizationTest, MultipleMatchesAfterNormalization)
{
    this->AddAllCerts();
...
REGISTER_TYPED_TEST_SUITE_P(CertIssuerSourceSyncNormalizationTest, MultipleMatchesAfterNormalization);

template <typename TestDelegate> class CertIssuerSourceSyncNotNormalizedTest : public CertIssuerSourceSyncTest<TestDelegate> { };
TYPED_TEST_SUITE_P(CertIssuerSourceSyncNotNormalizedTest);

TYPED_TEST_P(CertIssuerSourceSyncNotNormalizedTest, OneMatchWithoutNormalization)
{
    this->AddAllCerts();
...
REGISTER_TYPED_TEST_SUITE_P(CertIssuerSourceSyncNotNormalizedTest, OneMatchWithoutNormalization);
