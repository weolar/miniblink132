...
// initialized with.
class NET_EXPORT CTLogVerifier : public base::RefCountedThreadSafe<CTLogVerifier> {
public:
    ...
    static scoped_refptr<const CTLogVerifier> Create(std::string_view public_key, std::string description);
    ...
    bool Verify(const ct::SignedEntryData& entry, const ct::SignedCertificateTimestamp& sct) const;
    ...
    // |new_tree_hash|.
    bool VerifyConsistencyProof(const ct::MerkleConsistencyProof& proof, const std::string& old_tree_hash, const std::string& new_tree_hash) const;
    ...
    bool VerifyAuditProof(const ct::MerkleAuditProof& proof, const std::string& root_hash, const std::string& leaf_hash) const;
...
private:
    ...
    bool VerifySignature(std::string_view data_to_sign, std::string_view signature) const;
    ...
    ct::DigitallySigned::HashAlgorithm hash_algorithm_ = ct::DigitallySigned::HASH_ALGO_NONE;
    ct::DigitallySigned::SignatureAlgorithm signature_algorithm_ = ct::DigitallySigned::SIG_ALGO_ANONYMOUS;
