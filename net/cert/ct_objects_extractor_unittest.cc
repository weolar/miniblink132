...
public:
    void SetUp() override
    {
        precert_chain_ = CreateCertificateListFromFile(GetTestCertsDirectory(), "ct-test-embedded-cert.pem", X509Certificate::FORMAT_AUTO);
        ASSERT_EQ(2u, precert_chain_.size());

        std::string der_test_cert(ct::GetDerEncodedX509Cert());
        test_cert_ = X509Certificate::CreateFromBytes(base::as_byte_span(der_test_cert));
    ...
    void ExtractEmbeddedSCT(scoped_refptr<X509Certificate> cert, scoped_refptr<SignedCertificateTimestamp>* sct)
    {
        std::string sct_list;
...
TEST_F(CTObjectsExtractorTest, ExtractEmbeddedSCT)
{
    ...
    base::Time expected_timestamp = base::Time::UnixEpoch() + base::Milliseconds(1365181456275);
...
{
    CertificateList certs = CreateCertificateListFromFile(GetTestCertsDirectory(), "ct-test-embedded-with-uids.pem", X509Certificate::FORMAT_PEM_CERT_SEQUENCE);
...
TEST_F(CTObjectsExtractorTest, ExtractPrecert)
{
    ...
    ASSERT_TRUE(GetPrecertSignedEntry(precert_chain_[0]->cert_buffer(), precert_chain_[1]->cert_buffer(), &entry));
...
TEST_F(CTObjectsExtractorTest, ExtractSCTListFromOCSPResponse)
{
    ...
    scoped_refptr<X509Certificate> subject_cert = X509Certificate::CreateFromBytes(base::as_byte_span(der_subject_cert));
    ASSERT_TRUE(subject_cert);
    std::string der_issuer_cert(ct::GetDerEncodedFakeOCSPResponseIssuerCert());
    scoped_refptr<X509Certificate> issuer_cert = X509Certificate::CreateFromBytes(base::as_byte_span(der_issuer_cert));
    ...
    EXPECT_TRUE(ct::ExtractSCTListFromOCSPResponse(issuer_cert->cert_buffer(), subject_cert->serial_number(), ocsp_response, &extracted_sct_list));
...
TEST_F(CTObjectsExtractorTest, ExtractSCTListFromOCSPResponseMatchesSerial)
{
    ...
    std::string extracted_sct_list;
    EXPECT_FALSE(ct::ExtractSCTListFromOCSPResponse(issuer_cert->cert_buffer(), test_cert_->serial_number(), ocsp_response, &extracted_sct_list));
}
...
TEST_F(CTObjectsExtractorTest, ExtractSCTListFromOCSPResponseMatchesIssuer)
{
    ...
    EXPECT_FALSE(ct::ExtractSCTListFromOCSPResponse(test_cert_->cert_buffer(), subject_cert->serial_number(), ocsp_response, &extracted_sct_list));
}
