...
const NSSCertDatabase::CertInfo* FindCertInfoForCert(const NSSCertDatabase::CertInfoList& cert_info_list, CERTCertificate* target_cert)
{
...
public:
    ...
    int client_cert_store_changes() const
    {
        // Also check that the NSSCertDatabase notifications were mirrored to the
        // CertDatabase observers.
        EXPECT_EQ(global_db_observer_.client_cert_store_changes_, client_cert_store_changes_);
...
public:
    void SetUp() override
    {
        ...
        cert_db_ = std::make_unique<NSSCertDatabase>(crypto::ScopedPK11Slot(PK11_ReferenceSlot(test_nssdb_.slot())) /* public slot */,
            crypto::ScopedPK11Slot(PK11_ReferenceSlot(test_nssdb_.slot())) /* private slot */);
...
protected:
    PK11SlotInfo* GetPublicSlot()
    {
        return public_slot_.get();
    }
    ...
    static bool ReadCertIntoList(const std::string& name, ScopedCERTCertificateList* certs)
    {
        ScopedCERTCertificate cert = ImportCERTCertificateFromFile(GetTestCertsDirectory(), name);
        if (!cert)
            return false;
    ...
    ScopedCERTCertificateList ListCerts()
    {
        ...
        crypto::ScopedCERTCertList cert_list(PK11_ListCertsInSlot(test_nssdb_.slot()));
        if (!cert_list)
            return result;
        ...
        std::sort(result.begin(), result.end(), [](const ScopedCERTCertificate& lhs, const ScopedCERTCertificate& rhs) {
            return x509_util::CalculateFingerprint256(lhs.get()) < x509_util::CalculateFingerprint256(rhs.get());
        });
        return result;
    }
...
TEST_F(CertDatabaseNSSTest, ListCertsInfo)
{
    ...
    ScopedCERTCertificateList test_root_certs = CreateCERTCertificateListFromFile(GetTestCertsDirectory(), "root_ca_cert.pem", X509Certificate::FORMAT_AUTO);
    ...
    NSSCertDatabase::CertInfoList certs_excluding_nss;
    {
        ...
        cert_db_->ListCertsInfo(future.GetCallback(), NSSCertDatabase::NSSRootsHandling::kInclude);
        certs_including_nss = future.Take();
    }
    {
        base::test::TestFuture<NSSCertDatabase::CertInfoList> future;
        cert_db_->ListCertsInfo(future.GetCallback(), NSSCertDatabase::NSSRootsHandling::kExclude);
    ...
    {
        const NSSCertDatabase::CertInfo* nss_root_info = FindCertInfoForCert(certs_including_nss, nss_root.get());
    ...
    EXPECT_FALSE(FindCertInfoForCert(certs_including_nss, test_root_certs[0].get()));
    EXPECT_FALSE(FindCertInfoForCert(certs_excluding_nss, test_root_certs[0].get()));

    // Import the NSS root into the test DB.
    SECStatus srv = PK11_ImportCert(test_nssdb_.slot(), nss_root.get(), CK_INVALID_HANDLE,
        net::x509_util::GetDefaultUniqueNickname(nss_root.get(), net::CA_CERT, test_nssdb_.slot()).c_str(), PR_FALSE /* includeTrust (unused) */);
    ASSERT_EQ(SECSuccess, srv);

    // Import test certificate to the test DB.
    NSSCertDatabase::ImportCertFailureList failed;
    EXPECT_TRUE(cert_db_->ImportCACerts(test_root_certs, NSSCertDatabase::TRUSTED_SSL, &failed));
    ...
    EXPECT_EQ(certs_including_nss_with_local.size(), 1 + certs_including_nss.size());
    EXPECT_EQ(certs_excluding_nss_with_local.size(), 2 + certs_excluding_nss.size());
    ...
    // thing.)
    EXPECT_LT(certs_excluding_nss_with_local.size(), certs_including_nss_with_local.size());

    // The NSS root that was imported to the test db should be in both lists now.
    {
        const NSSCertDatabase::CertInfo* nss_root_info = FindCertInfoForCert(certs_including_nss_with_local, nss_root.get());
        ASSERT_TRUE(nss_root_info);
        EXPECT_TRUE(nss_root_info->web_trust_anchor);
        EXPECT_FALSE(nss_root_info->untrusted);
        EXPECT_FALSE(nss_root_info->device_wide);
    ...
    {
        const NSSCertDatabase::CertInfo* nss_root_info = FindCertInfoForCert(certs_excluding_nss_with_local, nss_root.get());
        ASSERT_TRUE(nss_root_info);
    ...
    {
        const NSSCertDatabase::CertInfo* test_cert_info = FindCertInfoForCert(certs_including_nss_with_local, test_root_certs[0].get());
    ...
    {
        const NSSCertDatabase::CertInfo* test_cert_info = FindCertInfoForCert(certs_excluding_nss_with_local, test_root_certs[0].get());
        ASSERT_TRUE(test_cert_info);
        EXPECT_TRUE(test_cert_info->web_trust_anchor);
        EXPECT_FALSE(test_cert_info->untrusted);
        EXPECT_FALSE(test_cert_info->device_wide);
...
TEST_F(CertDatabaseNSSTest, ImportFromPKCS12AsExtractableAndExportAgain)
{
    ...
    EXPECT_EQ(1, cert_db_->ExportToPKCS12(cert_list, u"exportpw", &exported_data));
...
TEST_F(CertDatabaseNSSTest, ImportFromPKCS12AsUnextractableAndExportAgain)
{
    ...
    std::string exported_data;
    EXPECT_EQ(0, cert_db_->ExportToPKCS12(cert_list, u"exportpw", &exported_data));
}
...
{
    ScopedCERTCertificateList certs = CreateCERTCertificateListFromFile(GetTestCertsDirectory(), "root_ca_cert.pem", X509Certificate::FORMAT_AUTO);
    ASSERT_EQ(1U, certs.size());
    EXPECT_FALSE(GetCertIsPerm(certs[0].get()));

    // Import it.
    NSSCertDatabase::ImportCertFailureList failed;
    EXPECT_TRUE(cert_db_->ImportCACerts(certs, NSSCertDatabase::TRUSTED_SSL, &failed));
    ...
    EXPECT_EQ(NSSCertDatabase::TRUSTED_SSL, cert_db_->GetCertTrust(cert, CA_CERT));

    EXPECT_EQ(unsigned(CERTDB_VALID_CA | CERTDB_TRUSTED_CA | CERTDB_TRUSTED_CLIENT_CA), cert->trust->sslFlags);
...
TEST_F(CertDatabaseNSSTest, ImportCACert_EmailTrust)
{
    ...
    // Import it.
    NSSCertDatabase::ImportCertFailureList failed;
    EXPECT_TRUE(cert_db_->ImportCACerts(certs, NSSCertDatabase::TRUSTED_EMAIL, &failed));
    ...
    EXPECT_EQ(NSSCertDatabase::TRUSTED_EMAIL, cert_db_->GetCertTrust(cert, CA_CERT));

    EXPECT_EQ(unsigned(CERTDB_VALID_CA), cert->trust->sslFlags);
    EXPECT_EQ(unsigned(CERTDB_VALID_CA | CERTDB_TRUSTED_CA | CERTDB_TRUSTED_CLIENT_CA), cert->trust->emailFlags);
    EXPECT_EQ(unsigned(CERTDB_VALID_CA), cert->trust->objectSigningFlags);
...
TEST_F(CertDatabaseNSSTest, ImportCACert_ObjSignTrust)
{
    ...
    // Import it.
    NSSCertDatabase::ImportCertFailureList failed;
    EXPECT_TRUE(cert_db_->ImportCACerts(certs, NSSCertDatabase::TRUSTED_OBJ_SIGN, &failed));
    ...
    EXPECT_EQ(NSSCertDatabase::TRUSTED_OBJ_SIGN, cert_db_->GetCertTrust(cert, CA_CERT));

    EXPECT_EQ(unsigned(CERTDB_VALID_CA), cert->trust->sslFlags);
    EXPECT_EQ(unsigned(CERTDB_VALID_CA), cert->trust->emailFlags);
    EXPECT_EQ(unsigned(CERTDB_VALID_CA | CERTDB_TRUSTED_CA | CERTDB_TRUSTED_CLIENT_CA), cert->trust->objectSigningFlags);
...
{
    ScopedCERTCertificateList certs = CreateCERTCertificateListFromFile(GetTestCertsDirectory(), "ok_cert.pem", X509Certificate::FORMAT_AUTO);
    ASSERT_EQ(1U, certs.size());
    EXPECT_FALSE(GetCertIsPerm(certs[0].get()));
...
TEST_F(CertDatabaseNSSTest, ImportCACertHierarchy)
{
    ...
    EXPECT_TRUE(cert_db_->ImportCACerts(certs, NSSCertDatabase::TRUSTED_SSL | NSSCertDatabase::TRUSTED_EMAIL, &failed));
...
TEST_F(CertDatabaseNSSTest, ImportCACertHierarchyDupeRoot)
{
    ...
    EXPECT_EQ("D Root CA - Multi-root", GetSubjectCN(failed[0].certificate.get()));
...
TEST_F(CertDatabaseNSSTest, ImportCACertHierarchyUntrusted)
{
    ...
    // Import it.
    NSSCertDatabase::ImportCertFailureList failed;
    EXPECT_TRUE(cert_db_->ImportCACerts(certs, NSSCertDatabase::TRUST_DEFAULT, &failed));
...
TEST_F(CertDatabaseNSSTest, ImportServerCert)
{
    ...
    ASSERT_TRUE(ReadCertIntoList("ok_cert_by_intermediate.pem", &certs_to_import));
    ...
    NSSCertDatabase::ImportCertFailureList failed;
    EXPECT_TRUE(cert_db_->ImportServerCert(certs_to_import, NSSCertDatabase::TRUST_DEFAULT, &failed));
    EXPECT_EQ(0U, failed.size());
    ...
    EXPECT_EQ(NSSCertDatabase::TRUST_DEFAULT, cert_db_->GetCertTrust(found_server_cert, SERVER_CERT));
    EXPECT_EQ(0U, found_server_cert->trust->sslFlags);
    EXPECT_EQ(NSSCertDatabase::TRUST_DEFAULT, cert_db_->GetCertTrust(found_intermediate_cert, CA_CERT));
    EXPECT_EQ(0U, found_intermediate_cert->trust->sslFlags);
    EXPECT_EQ(NSSCertDatabase::TRUST_DEFAULT, cert_db_->GetCertTrust(found_root_cert, CA_CERT));
    ...
    scoped_refptr<CertVerifyProc> verify_proc(CertVerifyProc::CreateBuiltinWithChromeRootStore(
        /*cert_net_fetcher=*/nullptr, crl_set_, std::make_unique<DoNothingCTVerifier>(), base::MakeRefCounted<DefaultCTPolicyEnforcer>(),
...
TEST_F(CertDatabaseNSSTest, ImportServerCert_SelfSigned)
{
    ...
    NSSCertDatabase::ImportCertFailureList failed;
    EXPECT_TRUE(cert_db_->ImportServerCert(certs, NSSCertDatabase::TRUST_DEFAULT, &failed));
    ...
    EXPECT_EQ(NSSCertDatabase::TRUST_DEFAULT, cert_db_->GetCertTrust(puny_cert, SERVER_CERT));
    EXPECT_EQ(0U, puny_cert->trust->sslFlags);

    scoped_refptr<X509Certificate> x509_puny_cert = x509_util::CreateX509CertificateFromCERTCertificate(puny_cert);
...
TEST_F(CertDatabaseNSSTest, ImportServerCert_SelfSigned_Trusted)
{
    ...
    NSSCertDatabase::ImportCertFailureList failed;
    EXPECT_TRUE(cert_db_->ImportServerCert(certs, NSSCertDatabase::TRUSTED_SSL, &failed));
    ...
    EXPECT_EQ(NSSCertDatabase::TRUSTED_SSL, cert_db_->GetCertTrust(puny_cert, SERVER_CERT));
    EXPECT_EQ(unsigned(CERTDB_TRUSTED | CERTDB_TERMINAL_RECORD), puny_cert->trust->sslFlags);
...
{
    ScopedCERTCertificateList ca_certs = CreateCERTCertificateListFromFile(GetTestCertsDirectory(), "root_ca_cert.pem", X509Certificate::FORMAT_AUTO);
    ASSERT_EQ(1U, ca_certs.size());

    // Import CA cert and trust it.
    NSSCertDatabase::ImportCertFailureList failed;
    EXPECT_TRUE(cert_db_->ImportCACerts(ca_certs, NSSCertDatabase::TRUSTED_SSL, &failed));
    EXPECT_EQ(0U, failed.size());
    ...
    scoped_refptr<X509Certificate> x509_server_cert = x509_util::CreateX509CertificateFromCERTCertificate(certs[0].get());
...
TEST_F(CertDatabaseNSSTest, ImportCaAndServerCert_DistrustServer)
{
    ...
    // distrust).
    EXPECT_TRUE(cert_db_->ImportServerCert(certs, NSSCertDatabase::DISTRUSTED_SSL, &failed));
    EXPECT_EQ(0U, failed.size());
    EXPECT_EQ(NSSCertDatabase::DISTRUSTED_SSL, cert_db_->GetCertTrust(certs[0].get(), SERVER_CERT));
...
TEST_F(CertDatabaseNSSTest, TrustIntermediateCa)
{
    ...
    ScopedCERTCertificateList ca_certs = x509_util::CreateCERTCertificateListFromX509Certificate(root->GetX509Certificate().get());
    ASSERT_EQ(1U, ca_certs.size());

    // Import Root CA cert and distrust it.
    NSSCertDatabase::ImportCertFailureList failed;
    EXPECT_TRUE(cert_db_->ImportCACerts(ca_certs, NSSCertDatabase::DISTRUSTED_SSL, &failed));
    EXPECT_EQ(0U, failed.size());
    ...
    ScopedCERTCertificateList intermediate_certs = x509_util::CreateCERTCertificateListFromX509Certificate(intermediate->GetX509Certificate().get());
    ASSERT_EQ(1U, intermediate_certs.size());

    // Import Intermediate CA cert and trust it.
    EXPECT_TRUE(cert_db_->ImportCACerts(intermediate_certs, NSSCertDatabase::TRUSTED_SSL, &failed));
    EXPECT_EQ(0U, failed.size());
    ...
    ScopedCERTCertificateList certs = x509_util::CreateCERTCertificateListFromX509Certificate(x509_server_cert.get());
    ASSERT_EQ(1U, certs.size());

    // Import server cert with default trust.
    EXPECT_TRUE(cert_db_->ImportServerCert(certs, NSSCertDatabase::TRUST_DEFAULT, &failed));
    EXPECT_EQ(0U, failed.size());
    EXPECT_EQ(NSSCertDatabase::TRUST_DEFAULT, cert_db_->GetCertTrust(certs[0].get(), SERVER_CERT));
    ...
    EXPECT_TRUE(cert_db_->SetCertTrust(ca_certs[0].get(), CA_CERT, NSSCertDatabase::TRUSTED_SSL));
    EXPECT_TRUE(cert_db_->SetCertTrust(intermediate_certs[0].get(), CA_CERT, NSSCertDatabase::DISTRUSTED_SSL));
    EXPECT_EQ(unsigned(CERTDB_VALID_CA | CERTDB_TRUSTED_CA | CERTDB_TRUSTED_CLIENT_CA), ca_certs[0]->trust->sslFlags);
    ...
    EXPECT_EQ(unsigned(CERTDB_TERMINAL_RECORD), intermediate_certs[0]->trust->sslFlags);
    EXPECT_EQ(unsigned(CERTDB_VALID_CA), intermediate_certs[0]->trust->emailFlags);
    EXPECT_EQ(unsigned(CERTDB_VALID_CA), intermediate_certs[0]->trust->objectSigningFlags);

    // Server cert should fail to verify.
    CertVerifyResult verify_result2;
    error = verify_proc->Verify(x509_server_cert.get(), "www.example.com",
        /*ocsp_response=*/std::string(),
        /*sct_list=*/std::string(), flags, &verify_result2, NetLogWithSource());
    EXPECT_THAT(error, IsError(ERR_CERT_AUTHORITY_INVALID));
...
TEST_F(CertDatabaseNSSTest, TrustIntermediateCa2)
{
    ...
    EXPECT_TRUE(cert_db_->SetCertTrust(intermediate_certs[0].get(), CA_CERT, NSSCertDatabase::TRUST_DEFAULT));
...
TEST_F(CertDatabaseNSSTest, TrustIntermediateCa3)
{
    ...
    EXPECT_TRUE(cert_db_->ImportCACerts(ca_certs, NSSCertDatabase::TRUST_DEFAULT, &failed));
    EXPECT_EQ(0U, failed.size());
...
TEST_F(CertDatabaseNSSTest, TrustIntermediateCa4)
{
    ...
    EXPECT_TRUE(cert_db_->ImportCACerts(intermediate_certs, NSSCertDatabase::DISTRUSTED_SSL, &failed));
    EXPECT_EQ(0U, failed.size());
...
{
    ScopedCERTCertificateList certs = CreateCERTCertificateListFromFile(GetTestCertsDirectory(), "duplicate_cn_1.pem", X509Certificate::FORMAT_AUTO);
    ASSERT_EQ(1U, certs.size());
    ...
    ScopedCERTCertificateList certs2 = CreateCERTCertificateListFromFile(GetTestCertsDirectory(), "duplicate_cn_2.pem", X509Certificate::FORMAT_AUTO);
    ASSERT_EQ(1U, certs2.size());

    // Import server cert with default trust.
    EXPECT_TRUE(cert_db_->ImportServerCert(certs2, NSSCertDatabase::TRUST_DEFAULT, &failed));
    EXPECT_EQ(0U, failed.size());
    EXPECT_EQ(NSSCertDatabase::TRUST_DEFAULT, cert_db_->GetCertTrust(certs2[0].get(), SERVER_CERT));
