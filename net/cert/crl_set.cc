...
std::optional<base::Value> ReadHeader(std::string_view* data)
{
    ...
    std::optional<base::Value> header = base::JSONReader::Read(header_bytes, base::JSON_ALLOW_TRAILING_COMMAS);
...
bool ReadCRL(std::string_view* data, std::string* out_parent_spki_hash, std::vector<std::string>* out_serials)
{
...
// |header_dict|.
bool CopyHashListFromHeader(const base::Value::Dict& header_dict, const char* key, std::vector<std::string>* out)
{
...
bool CopyHashToHashesMapFromHeader(const base::Value::Dict& header_dict, const char* key, std::unordered_map<std::string, std::vector<std::string>>* out)
{
    out->clear();
    ...
    for (auto i : *dict) {
        ...
        for (const auto& j : i.second.GetList()) {
            ...
            if (!j.is_string() || !base::Base64Decode(j.GetString(), &allowed_spkis.back())) {
                return false;
            }
        }
...
#error assumes little endian
    ...
    if (!CopyHashListFromHeader(header_dict, "BlockedSPKIs", &crl_set->blocked_spkis_)
        || !CopyHashToHashesMapFromHeader(header_dict, "LimitedSubjects", &crl_set->limited_subjects_)
        || !CopyHashListFromHeader(header_dict, "KnownInterceptionSPKIs", &crl_set->known_interception_spkis_)
        || !CopyHashListFromHeader(header_dict, "BlockedInterceptionSPKIs", &blocked_interception_spkis)) {
        return false;
    }
    ...
    crl_set->blocked_spkis_.insert(crl_set->blocked_spkis_.end(), blocked_interception_spkis.begin(), blocked_interception_spkis.end());
    crl_set->known_interception_spkis_.insert(crl_set->known_interception_spkis_.end(), blocked_interception_spkis.begin(), blocked_interception_spkis.end());
...
#include "net/cert/cert_verify_proc_blocklist.inc"
    for (const auto& hash : kSPKIBlockList) {
        crl_set->blocked_spkis_.emplace_back(reinterpret_cast<const char*>(hash), crypto::kSHA256Length);
    }

    for (const auto& hash : kKnownInterceptionList) {
        crl_set->known_interception_spkis_.emplace_back(reinterpret_cast<const char*>(hash), crypto::kSHA256Length);
    }

    // Sort, as these will be std::binary_search()'d.
    std::sort(crl_set->blocked_spkis_.begin(), crl_set->blocked_spkis_.end());
    std::sort(crl_set->known_interception_spkis_.begin(), crl_set->known_interception_spkis_.end());
...
{
    if (std::binary_search(blocked_spkis_.begin(), blocked_spkis_.end(), spki_hash))
        return REVOKED;
    return GOOD;
}

CRLSet::Result CRLSet::CheckSubject(std::string_view encoded_subject, std::string_view spki_hash) const
{
...
CRLSet::Result CRLSet::CheckSerial(std::string_view serial_number, std::string_view issuer_spki_hash) const
{
...
{
    return std::binary_search(known_interception_spkis_.begin(), known_interception_spkis_.end(), spki_hash);
}
...
{
    constexpr char kCRLSet[] = "\x31\x00{\"ContentType\":\"CRLSet\",\"Sequence\":0,\"Version\":0}";
...
// static
scoped_refptr<CRLSet> CRLSet::ForTesting(bool is_expired, const SHA256HashValue* issuer_spki, std::string_view serial_number, std::string_view utf8_common_name,
    const std::vector<std::string>& acceptable_spki_hashes_for_cn)
{
    ...
    if (!utf8_common_name.empty()) {
        ...
        if (!CBB_init(&cbb, 32) || !CBB_add_asn1(&cbb, &top_level, CBS_ASN1_SEQUENCE) || !CBB_add_asn1(&top_level, &set, CBS_ASN1_SET)
            || !CBB_add_asn1(&set, &inner_seq, CBS_ASN1_SEQUENCE) || !CBB_add_asn1(&inner_seq, &oid, CBS_ASN1_OBJECT)
            || !CBB_add_bytes(&oid, kCommonNameOID, sizeof(kCommonNameOID)) || !CBB_add_asn1(&inner_seq, &cn, CBS_ASN1_UTF8STRING)
            || !CBB_add_bytes(&cn, reinterpret_cast<const uint8_t*>(utf8_common_name.data()), utf8_common_name.size())
            || !CBB_finish(&cbb, &x501_data, &x501_len)) {
            CBB_cleanup(&cbb);
            return nullptr;
        }

        subject_hash.assign(crypto::SHA256HashString(std::string_view(reinterpret_cast<char*>(x501_data), x501_len)));
    ...
    if (issuer_spki) {
        std::string spki(reinterpret_cast<const char*>(issuer_spki->data), sizeof(issuer_spki->data));
