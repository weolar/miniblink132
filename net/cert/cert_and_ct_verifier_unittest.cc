...
public:
    ...
    void Verify(base::StringPiece hostname, X509Certificate* cert, base::StringPiece stapled_ocsp_response, base::StringPiece sct_list_from_tls_extension,
        SignedCertificateTimestampAndStatusList* output_scts, const NetLogWithSource& net_log) override
    {
        *output_scts = scts_;
    }
...
TEST_F(CertAndCTVerifierTest, CertAndCTDetailsFilled_Sync)
{
    ...
    CertAndCTVerifier cert_and_ct_verifier(std::move(cert_verifier), std::move(ct_verifier));

    CertVerifyResult verify_result;
    TestCompletionCallback callback;
    std::unique_ptr<CertVerifier::Request> request;

    int result = callback.GetResult(cert_and_ct_verifier.Verify(CertVerifier::RequestParams(test_cert, "www.example.com", 0,
...
TEST_F(CertAndCTVerifierTest, CancelRequest)
{
    ...
    int result = cert_and_ct_verifier.Verify(CertVerifier::RequestParams(test_cert, "www.example.com", 0,
                                                 /*ocsp_response=*/std::string(),
                                                 /*sct_list=*/std::string()),
        &verify_result, base::BindOnce(&FailTest, std::move(helper)), &request, NetLogWithSource());
...
TEST_F(CertAndCTVerifierTest, DeleteVerifier)
{
    ...
    auto cert_and_ct_verifier = std::make_unique<CertAndCTVerifier>(std::move(cert_verifier), std::move(ct_verifier));
