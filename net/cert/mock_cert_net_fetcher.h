...
public:
    ...
    MOCK_METHOD3(FetchCaIssuers, std::unique_ptr<Request>(const GURL& url, int timeout_milliseconds, int max_response_bytes));
    MOCK_METHOD3(FetchCrl, std::unique_ptr<Request>(const GURL& url, int timeout_milliseconds, int max_response_bytes));

    MOCK_METHOD3(FetchOcsp, std::unique_ptr<Request>(const GURL& url, int timeout_milliseconds, int max_response_bytes));
...
public:
    ...
    // the specified bytes.
    static std::unique_ptr<CertNetFetcher::Request> Create(std::vector<uint8_t> bytes);

    // Creates a CertNetFetcher::Request that completes with OK error code and
    // the specified CRYPTO_BUFFER data.
    static std::unique_ptr<CertNetFetcher::Request> Create(const CRYPTO_BUFFER* buffer);
