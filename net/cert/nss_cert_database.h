...
public:
    ...
    using ListCertsInfoCallback = base::OnceCallback<void(CertInfoList certs_info)>;

    using ListCertsCallback = base::OnceCallback<void(ScopedCERTCertificateList certs)>;
    ...
    // be identical.
    NSSCertDatabase(crypto::ScopedPK11Slot public_slot, crypto::ScopedPK11Slot private_slot);
    ...
    virtual void ListCertsInfo(ListCertsInfoCallback callback, NSSRootsHandling nss_roots_handling);
    ...
    virtual void ListModules(std::vector<crypto::ScopedPK11Slot>* modules, bool need_rw) const;

    // Set trust values for certificate.
    // Returns true on success or false on failure.
    virtual bool SetCertTrust(CERTCertificate* cert, CertType type, TrustBits trust_bits);
    ...
    static int ExportToPKCS12(const ScopedCERTCertificateList& certs, const std::u16string& password, std::string* output);
    ...
    CERTCertificate* FindRootInList(const ScopedCERTCertificateList& certificates) const;
    ...
    // imported.
    bool ImportCACerts(const ScopedCERTCertificateList& certificates, TrustBits trust_bits, ImportCertFailureList* not_imported);
    ...
    // |not_imported| should be checked for any certificates that were not
    // imported.
    bool ImportServerCert(const ScopedCERTCertificateList& certificates, TrustBits trust_bits, ImportCertFailureList* not_imported);
    ...
    void DeleteCertAndKeyAsync(ScopedCERTCertificate cert, DeleteCertCallback callback);
...
protected:
    ...
    static CertInfoList ListCertsInfoImpl(crypto::ScopedPK11Slot slot, bool add_certs_info, NSSRootsHandling nss_roots_handling);
...
private:
    ...
    // |success| as argument.
    void NotifyCertRemovalAndCallBack(DeleteCertCallback callback, DeleteCertAndKeyResult result);
    ...
    static DeleteCertAndKeyResult DeleteCertAndKeyImplScoped(ScopedCERTCertificate cert);
