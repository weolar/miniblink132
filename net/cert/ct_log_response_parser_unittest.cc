...
{
    std::optional<base::Value> sample_sth_json = base::JSONReader::Read(GetSampleSTHAsJson());
    ...
    ASSERT_EQ(tree_head.signature.hash_algorithm, expected_signature.hash_algorithm);
    ASSERT_EQ(tree_head.signature.signature_algorithm, expected_signature.signature_algorithm);
    ASSERT_EQ(tree_head.signature.signature_data, expected_signature.signature_data);
}

TEST(CTLogResponseParserTest, FailsToParseMissingFields)
{
    std::optional<base::Value> missing_signature_sth
        = base::JSONReader::Read(CreateSignedTreeHeadJsonString(1 /* tree_size */, 123456u /* timestamp */, GetSampleSTHSHA256RootHash(), ""));

    SignedTreeHead tree_head;
    ASSERT_FALSE(FillSignedTreeHead(*missing_signature_sth, &tree_head));

    std::optional<base::Value> missing_root_hash_sth
        = base::JSONReader::Read(CreateSignedTreeHeadJsonString(1 /* tree_size */, 123456u /* timestamp */, "", GetSampleSTHTreeHeadSignature()));
...
TEST(CTLogResponseParserTest, FailsToParseIncorrectLengthRootHash)
{
    ...
    base::Base64Decode(std::string_view("/WHFMgXtI/umKKuACJIN0Bb73TcILm9WkeU6qszvoArK\n"), &too_long_hash);
    std::optional<base::Value> too_long_hash_json
        = base::JSONReader::Read(CreateSignedTreeHeadJsonString(1 /* tree_size */, 123456u /* timestamp */, GetSampleSTHSHA256RootHash(), too_long_hash));
    ASSERT_FALSE(FillSignedTreeHead(*too_long_hash_json, &tree_head));

    std::string too_short_hash;
    base::Base64Decode(std::string_view("/WHFMgXtI/umKKuACJIN0Bb73TcILm9WkeU6qszvoA==\n"), &too_short_hash);
    std::optional<base::Value> too_short_hash_json
        = base::JSONReader::Read(CreateSignedTreeHeadJsonString(1 /* tree_size */, 123456u /* timestamp */, GetSampleSTHSHA256RootHash(), too_short_hash));
...
TEST(CTLogResponseParserTest, ParsesJsonSTHWithLargeTimestamp)
{
    ...
    std::optional<base::Value> large_timestamp_json
        = base::JSONReader::Read(CreateSignedTreeHeadJsonString(100, INT64_C(1) << 34, GetSampleSTHSHA256RootHash(), GetSampleSTHTreeHeadSignature()));

    ASSERT_TRUE(FillSignedTreeHead(*large_timestamp_json, &tree_head));

    base::Time expected_time = base::Time::UnixEpoch() + base::Milliseconds(INT64_C(1) << 34);
...
TEST(CTLogResponseParserTest, ParsesConsistencyProofSuccessfully)
{
    ...
    std::optional<base::Value> sample_consistency_proof = base::JSONReader::Read(CreateConsistencyProofJsonString(raw_nodes));
...
TEST(CTLogResponseParserTest, FailsOnInvalidProofJson)
{
    ...
    std::optional<base::Value> badly_encoded = base::JSONReader::Read(std::string("{\"consistency\": [\"notbase64\"]}"));
    EXPECT_FALSE(FillConsistencyProof(*badly_encoded, &output));

    std::optional<base::Value> not_a_string = base::JSONReader::Read(std::string("{\"consistency\": [42, 16]}"));
    EXPECT_FALSE(FillConsistencyProof(*badly_encoded, &output));

    std::optional<base::Value> missing_consistency = base::JSONReader::Read(std::string("{}"));
    EXPECT_FALSE(FillConsistencyProof(*missing_consistency, &output));

    std::optional<base::Value> not_a_dict = base::JSONReader::Read(std::string("[]"));
...
TEST(CTLogResponseParserTest, ParsesProofJsonWithExtraFields)
{
    ...
    std::optional<base::Value> badly_encoded = base::JSONReader::Read(std::string("{\"consistency\": [], \"somethingelse\": 3}"));
