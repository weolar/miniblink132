...
typedef OSStatus (*SecTrustCopyExtendedResultFuncPtr)(SecTrustRef, CFDictionaryRef*);
...
CertStatus CertStatusFromOSStatus(OSStatus status)
{
    ...
    default: {
        ...
        OSSTATUS_LOG(WARNING, status) << "Unknown error mapped to CERT_STATUS_INVALID";
        return CERT_STATUS_INVALID;
    }
    }
}
...
{
    ScopedCFTypeRef<CFMutableArrayRef> local_policies(CFArrayCreateMutable(kCFAllocatorDefault, 0, &kCFTypeArrayCallBacks));
    if (!local_policies)
        return memFullErr;

    ScopedCFTypeRef<SecPolicyRef> ssl_policy(SecPolicyCreateSSL(/*server=*/true, /*hostname=*/nullptr));
    if (!ssl_policy)
    ...
    if (flags & CertVerifyProc::VERIFY_REV_CHECKING_ENABLED) {
        ...
        ScopedCFTypeRef<SecPolicyRef> revocation_policy(SecPolicyCreateRevocation(kSecRevocationUseAnyAvailableMethod | kSecRevocationRequirePositiveResponse));
...
void CopyCertChainToVerifyResult(CFArrayRef cert_chain, CertVerifyResult* verify_result)
{
    ...
    for (CFIndex i = 0, count = CFArrayGetCount(cert_chain); i < count; ++i) {
        SecCertificateRef chain_cert = reinterpret_cast<SecCertificateRef>(const_cast<void*>(CFArrayGetValueAtIndex(cert_chain, i)));
        if (i == 0) {
    ...
    scoped_refptr<X509Certificate> verified_cert_with_chain = x509_util::CreateX509CertificateFromSecCertificate(verified_cert, verified_chain);
...
bool CertUsesWeakHash(SecCertificateRef cert_handle)
{
    ...
    status = cached_cert.GetField(&CSSMOID_X509V1SignatureAlgorithm, &signature_field);
    ...
    return (x509_util::CSSMOIDEqual(alg_oid, &CSSMOID_MD2WithRSA) || x509_util::CSSMOIDEqual(alg_oid, &CSSMOID_MD4WithRSA)
        || x509_util::CSSMOIDEqual(alg_oid, &CSSMOID_MD5WithRSA) || x509_util::CSSMOIDEqual(alg_oid, &CSSMOID_SHA1WithRSA)
        || x509_util::CSSMOIDEqual(alg_oid, &CSSMOID_SHA1WithRSA_OIW) || x509_util::CSSMOIDEqual(alg_oid, &CSSMOID_SHA1WithDSA)
        || x509_util::CSSMOIDEqual(alg_oid, &CSSMOID_SHA1WithDSA_CMS) || x509_util::CSSMOIDEqual(alg_oid, &CSSMOID_SHA1WithDSA_JDK)
...
bool IsWeakChainBasedOnHashingAlgorithms(CFArrayRef cert_chain, const std::vector<CertEvidenceInfo>& chain_info)
{
    DCHECK_LT(0, CFArrayGetCount(cert_chain));
    DCHECK_EQ(chain_info.size(), static_cast<size_t>(CFArrayGetCount(cert_chain)));
    ...
    for (CFIndex i = 0, count = CFArrayGetCount(cert_chain); i < count; ++i) {
        ...
        if ((chain_info[i].status_bits & CSSM_CERT_STATUS_IS_IN_ANCHORS) || (chain_info[i].status_bits & CSSM_CERT_STATUS_IS_ROOT)) {
...
bool HasPolicyOrAnyPolicy(const ParsedCertificate* cert, const der::Input& ev_policy_oid)
{
...
void GetCandidateEVPolicy(const X509Certificate* cert_input, std::string* ev_policy_oid)
{
    ev_policy_oid->clear();

    scoped_refptr<ParsedCertificate> cert(ParsedCertificate::Create(bssl::UpRef(cert_input->cert_buffer()), {}, nullptr));
    if (!cert)
        return;
...
bool CheckCertChainEV(const X509Certificate* cert, const std::string& ev_policy_oid_string)
{
    der::Input ev_policy_oid(&ev_policy_oid_string);
    const std::vector<bssl::UniquePtr<CRYPTO_BUFFER>>& cert_chain = cert->intermediate_buffers();

    // Root should have matching policy in EVRootCAMetadata.
    if (cert_chain.empty())
        return false;
    SHA256HashValue fingerprint = X509Certificate::CalculateFingerprint256(cert_chain.back().get());
    EVRootCAMetadata* metadata = EVRootCAMetadata::GetInstance();
    ...
    for (size_t i = 0; i < cert_chain.size() - 1; ++i) {
        scoped_refptr<ParsedCertificate> intermediate_cert(ParsedCertificate::Create(bssl::UpRef(cert_chain[i].get()), {}, nullptr));
...
void AppendPublicKeyHashesAndUpdateKnownRoot(CFArrayRef chain, HashValueVector* hashes, bool* known_root)
{
    // Walk the chain in reverse, to optimize for IsKnownRoot checks.
    for (CFIndex i = CFArrayGetCount(chain); i > 0; i--) {
        SecCertificateRef cert = reinterpret_cast<SecCertificateRef>(const_cast<void*>(CFArrayGetValueAtIndex(chain, i - 1)));

        CSSM_DATA cert_data;
        OSStatus err = SecCertificateGetData(cert, &cert_data);
        DCHECK_EQ(err, noErr);
        base::StringPiece der_bytes(reinterpret_cast<const char*>(cert_data.Data), cert_data.Length);
...
CRLSetResult CheckRevocationWithCRLSet(CFArrayRef chain, CRLSet* crl_set)
{
    ...
    for (CFIndex i = CFArrayGetCount(chain); i > 0; i--) {
        ...
        if (!asn1::ExtractSPKIFromDERCert(der_bytes, &spki) || !asn1::ExtractSubjectFromDERCert(der_bytes, &subject)) {
            NOTREACHED();
            error = true;
            continue;
        }
        ...
        base::StringPiece serial(reinterpret_cast<const char*>(serial_number.field()->Data), serial_number.field()->Length);
...
// held.
int BuildAndEvaluateSecTrustRef(CFArrayRef cert_array, CFArrayRef trust_policies, CFDataRef ocsp_response_ref, CFArrayRef sct_array_ref, int flags,
    CFArrayRef keychain_search_list, ScopedCFTypeRef<SecTrustRef>* trust_ref, SecTrustResultType* trust_result, ScopedCFTypeRef<CFArrayRef>* verified_chain,
    std::vector<CertEvidenceInfo>* chain_info)
{
    ...
    ScopedCFTypeRef<CFDataRef> action_data_ref(CFDataCreate(kCFAllocatorDefault, reinterpret_cast<UInt8*>(&tp_action_data), sizeof(tp_action_data)));
    if (!action_data_ref)
        return ERR_OUT_OF_MEMORY;
    status = SecTrustSetParameters(tmp_trust, CSSM_TP_ACTION_DEFAULT, action_data_ref.get());
    if (status)
        return NetErrorFromOSStatus(status);
    ...
    status = SecTrustGetResult(tmp_trust, &tmp_trust_result, &tmp_verified_chain, &tmp_chain_info);
    if (status)
        return NetErrorFromOSStatus(status);
    ...
    chain_info->clear();
    for (CFIndex i = 0, chain_length = CFArrayGetCount(tmp_verified_chain); i < chain_length; ++i) {
        CertEvidenceInfo info;
        info.status_bits = tmp_chain_info[i].StatusBits;
        info.status_codes.assign(tmp_chain_info[i].StatusCodes, tmp_chain_info[i].StatusCodes + tmp_chain_info[i].NumStatusCodes);
...
int VerifyWithGivenFlags(X509Certificate* cert, const std::string& hostname, const std::string& ocsp_response, const std::string& sct_list, const int flags,
    bool rev_checking_soft_fail, CRLSet* crl_set, CertVerifyResult* verify_result, CRLSetResult* completed_chain_crl_result)
{
    ...
    *completed_chain_crl_result = kCRLSetUnknown;
    ...
    if (!ocsp_response.empty()) {
        ocsp_response_ref.reset(
            CFDataCreate(kCFAllocatorDefault, reinterpret_cast<const UInt8*>(ocsp_response.data()), base::checked_cast<CFIndex>(ocsp_response.size())));
        if (!ocsp_response_ref)
            return ERR_OUT_OF_MEMORY;
    }
    ...
    if (!sct_list.empty()) {
        if (__builtin_available(macOS 10.14.2, *)) {
            ...
            if (ct::DecodeSCTList(sct_list, &decoded_sct_list)) {
                sct_array_ref.reset(CFArrayCreateMutable(kCFAllocatorDefault, decoded_sct_list.size(), &kCFTypeArrayCallBacks));
                if (!sct_array_ref)
                    return ERR_OUT_OF_MEMORY;
                for (const auto& sct : decoded_sct_list) {
                    ScopedCFTypeRef<CFDataRef> sct_ref(
                        CFDataCreate(kCFAllocatorDefault, reinterpret_cast<const UInt8*>(sct.data()), base::checked_cast<CFIndex>(sct.size())));
                    if (!sct_ref)
                        return ERR_OUT_OF_MEMORY;
    ...
    for (bool try_reordered_keychain : { false, true }) {
        ...
        if (TestKeychainSearchList::HasInstance()) {
            ...
            status = TestKeychainSearchList::GetInstance()->CopySearchList(&keychain_search_list);
            if (status)
                return NetErrorFromOSStatus(status);
        ...
        if (try_reordered_keychain) {
            ...
            CFMutableArrayRef mutable_keychain_search_list = CFArrayCreateMutableCopy(
                kCFAllocatorDefault, CFArrayGetCount(scoped_alternate_keychain_search_list.get()) + 1, scoped_alternate_keychain_search_list.get());
        ...
        ScopedCFTypeRef<CFMutableArrayRef> cert_array(
            x509_util::CreateSecCertificateArrayForX509Certificate(cert, x509_util::InvalidIntermediateBehavior::kIgnore));
        if (!cert_array) {
            verify_result->cert_status |= CERT_STATUS_INVALID;
        ...
        while (CFArrayGetCount(cert_array) > 0) {
            ...
            int rv = BuildAndEvaluateSecTrustRef(cert_array, trust_policies, ocsp_response_ref.get(), sct_array_ref.get(), flags,
                scoped_alternate_keychain_search_list.get(), &temp_ref, &temp_trust_result, &temp_chain, &temp_chain_info);
            if (rv != OK)
                return rv;
            ...
            bool untrusted = (temp_trust_result != kSecTrustResultUnspecified && temp_trust_result != kSecTrustResultProceed) || crl_result == kCRLSetRevoked;
            ...
            if (CFArrayGetCount(temp_chain) == 0) {
                ...
                weak_chain = IsWeakChainBasedOnHashingAlgorithms(temp_chain, temp_chain_info);
            }
            ...
            if (CFArrayGetCount(cert_array) > kMaxTrimmedChainLength) {
                CFArrayReplaceValues(cert_array, CFRangeMake(kMaxTrimmedChainLength, CFArrayGetCount(cert_array) - kMaxTrimmedChainLength),
    ...
    case kSecTrustResultRecoverableTrustFailure:
        ...
        for (CFIndex index = 0, chain_count = CFArrayGetCount(completed_chain); index < chain_count; ++index) {
            if (chain_info[index].status_bits & CSSM_CERT_STATUS_EXPIRED || chain_info[index].status_bits & CSSM_CERT_STATUS_NOT_VALID_YET)
                verify_result->cert_status |= CERT_STATUS_DATE_INVALID;
            if (!IsCertStatusError(verify_result->cert_status) && chain_info[index].status_codes.empty()) {
                LOG(WARNING) << "chain_info[" << index << "].status_codes is empty, chain_info[" << index << "].status_bits is "
                             << chain_info[index].status_bits;
            }
    ...
    AppendPublicKeyHashesAndUpdateKnownRoot(completed_chain, &verify_result->public_key_hashes, &verify_result->is_issued_by_known_root);

    CertVerifyProcMac::ResultDebugData::Create(trust_result, cssm_result, std::move(chain_info), verify_result);
...
CertVerifyProcMac::ResultDebugData::CertEvidenceInfo::CertEvidenceInfo() = default;
CertVerifyProcMac::ResultDebugData::CertEvidenceInfo::~CertEvidenceInfo() = default;
CertVerifyProcMac::ResultDebugData::CertEvidenceInfo::CertEvidenceInfo(const CertEvidenceInfo&) = default;
CertVerifyProcMac::ResultDebugData::CertEvidenceInfo::CertEvidenceInfo(CertEvidenceInfo&&) = default;

CertVerifyProcMac::ResultDebugData::ResultDebugData(uint32_t trust_result, int32_t result_code, std::vector<CertEvidenceInfo> status_chain)
...
CertVerifyProcMac::ResultDebugData::ResultDebugData(const ResultDebugData&) = default;

// static
const CertVerifyProcMac::ResultDebugData* CertVerifyProcMac::ResultDebugData::Get(const base::SupportsUserData* debug_data)
{
...
void CertVerifyProcMac::ResultDebugData::Create(
    uint32_t trust_result, int32_t result_code, std::vector<CertEvidenceInfo> status_chain, base::SupportsUserData* debug_data)
{
    debug_data->SetUserData(kResultDebugDataKey, std::make_unique<ResultDebugData>(trust_result, result_code, status_chain));
}

std::unique_ptr<base::SupportsUserData::Data> CertVerifyProcMac::ResultDebugData::Clone()
{
...
int CertVerifyProcMac::VerifyInternal(X509Certificate* cert, const std::string& hostname, const std::string& ocsp_response, const std::string& sct_list,
    int flags, CRLSet* crl_set, const CertificateList& additional_trust_anchors, CertVerifyResult* verify_result, const NetLogWithSource& net_log)
{
    ...
    int rv = VerifyWithGivenFlags(cert, hostname, ocsp_response, sct_list, flags,
        /*rev_checking_soft_fail=*/true, crl_set, verify_result, &completed_chain_crl_result);
    if (rv != OK)
        return rv;

    if (!candidate_ev_policy_oid.empty() && CheckCertChainEV(verify_result->verified_cert.get(), candidate_ev_policy_oid)) {
    ...
    LogNameNormalizationMetrics(".Mac", verify_result->verified_cert.get(), verify_result->is_issued_by_known_root);
