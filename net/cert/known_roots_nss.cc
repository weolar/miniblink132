...
bool IsKnownRoot(CERTCertificate* root)
{
    ...
    static PK11HasAttributeSetFunction pk11_has_attribute_set = reinterpret_cast<PK11HasAttributeSetFunction>(dlsym(RTLD_DEFAULT, "PK11_HasAttributeSet"));
        ...
        for (const SECMODModuleList* item = SECMOD_GetDefaultModuleList(); item != nullptr; item = item->next) {
            for (int i = 0; i < item->module->slotCount; ++i) {
                ...
                if (PK11_IsPresent(slot) && PK11_HasRootCerts(slot)) {
                    ...
                    if (handle != CK_INVALID_HANDLE && pk11_has_attribute_set(slot, handle, CKA_NSS_MOZILLA_CA_POLICY, PR_FALSE) == CK_TRUE) {
                        return true;
                    }
                }
            }
