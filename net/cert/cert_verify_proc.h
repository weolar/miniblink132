...
class NET_EXPORT CertVerifyProc : public base::RefCountedThreadSafe<CertVerifyProc> {
public:
    enum VerifyFlags {
    ...
    struct NET_EXPORT CertificateWithConstraints {
        ...
        CertificateWithConstraints& operator=(const CertificateWithConstraints& other);
    ...
    struct NET_EXPORT InstanceParams {
        ...
        bssl::ParsedCertificateList additional_trust_anchors_with_enforced_constraints;
        ...
        std::vector<CertificateWithConstraints> additional_trust_anchors_with_constraints;
    ...
    enum class NameNormalizationResult { kError = 0, kByteEqual = 1, kNormalized = 2, kChainLengthOne = 3, kMaxValue = kChainLengthOne };
    ...
    static scoped_refptr<CertVerifyProc> CreateSystemVerifyProc(scoped_refptr<CertNetFetcher> cert_net_fetcher, scoped_refptr<CRLSet> crl_set);
#endif

#if BUILDFLAG(IS_FUCHSIA)
    // Creates and returns a CertVerifyProcBuiltin using the SSL SystemTrustStore.
    static scoped_refptr<CertVerifyProc> CreateBuiltinVerifyProc(scoped_refptr<CertNetFetcher> cert_net_fetcher, scoped_refptr<CRLSet> crl_set,
    ...
    // use the default.
    static scoped_refptr<CertVerifyProc> CreateBuiltinWithChromeRootStore(scoped_refptr<CertNetFetcher> cert_net_fetcher, scoped_refptr<CRLSet> crl_set,
    ...
    int Verify(X509Certificate* cert, const std::string& hostname, const std::string& ocsp_response, const std::string& sct_list, int flags,
...
protected:
    ...
    static void LogNameNormalizationResult(const std::string& histogram_suffix, NameNormalizationResult result);

    // Record a histogram of whether Name normalization was used in verifying the
    // chain. This should only be called for successfully validated chains.
    static void LogNameNormalizationMetrics(const std::string& histogram_suffix, X509Certificate* verified_cert, bool is_issued_by_known_root);
...
private:
    ...
    virtual int VerifyInternal(X509Certificate* cert, const std::string& hostname, const std::string& ocsp_response, const std::string& sct_list, int flags,
        CertVerifyResult* verify_result, const NetLogWithSource& net_log)
        = 0;
    ...
    static bool HasNameConstraintsViolation(const HashValueVector& public_key_hashes, const std::string& common_name, const std::vector<std::string>& dns_names,
...
class NET_EXPORT CertVerifyProcFactory : public base::RefCountedThreadSafe<CertVerifyProcFactory> {
public:
    // Create a new CertVerifyProc that uses the passed in CRLSet and
    // ChromeRootStoreData.
    virtual scoped_refptr<CertVerifyProc> CreateCertVerifyProc(
        scoped_refptr<CertNetFetcher> cert_net_fetcher, const CertVerifyProc::ImplParams& impl_params, const CertVerifyProc::InstanceParams& instance_params)
        = 0;
