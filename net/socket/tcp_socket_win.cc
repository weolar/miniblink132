...
bool SetTCPKeepAlive(SOCKET socket, BOOL enable, int delay_secs)
{
    ...
    int rv = WSAIoctl(socket, SIO_KEEPALIVE_VALS, &keepalive_vals, sizeof(keepalive_vals), nullptr, 0, &bytes_returned, nullptr, nullptr);
    int os_error = WSAGetLastError();
    DCHECK(!rv) << "Could not enable TCP Keep-Alive for socket: " << socket << " [error: " << os_error << "].";
...
public:
    TCPSocketDefaultWin(std::unique_ptr<SocketPerformanceWatcher> socket_performance_watcher, NetLog* net_log, const NetLogSource& source);

    TCPSocketDefaultWin(std::unique_ptr<SocketPerformanceWatcher> socket_performance_watcher, NetLogWithSource net_log_source);
...
private:
    ...
    public:
        explicit ReadDelegate(CoreImpl* core)
            : core_(core)
        {
        }
    ...
    public:
        explicit WriteDelegate(CoreImpl* core)
            : core_(core)
        {
        }
...
void TCPSocketDefaultWin::CoreImpl::ReadDelegate::OnObjectSignaled(HANDLE object)
{
...
void TCPSocketDefaultWin::CoreImpl::WriteDelegate::OnObjectSignaled(HANDLE object)
{
...
std::unique_ptr<TCPSocketWin> TCPSocketWin::Create(
    std::unique_ptr<SocketPerformanceWatcher> socket_performance_watcher, NetLog* net_log, const NetLogSource& source)
{
    if (base::FeatureList::IsEnabled(features::kTcpSocketIoCompletionPortWin)) {
        return std::make_unique<TcpSocketIoCompletionPortWin>(std::move(socket_performance_watcher), net_log, source);
    }
    return std::make_unique<TCPSocketDefaultWin>(std::move(socket_performance_watcher), net_log, source);
}

// static
std::unique_ptr<TCPSocketWin> TCPSocketWin::Create(std::unique_ptr<SocketPerformanceWatcher> socket_performance_watcher, NetLogWithSource net_log_source)
{
    if (base::FeatureList::IsEnabled(features::kTcpSocketIoCompletionPortWin)) {
        return std::make_unique<TcpSocketIoCompletionPortWin>(std::move(socket_performance_watcher), net_log_source);
    }
    return std::make_unique<TCPSocketDefaultWin>(std::move(socket_performance_watcher), std::move(net_log_source));
}

TCPSocketWin::TCPSocketWin(std::unique_ptr<SocketPerformanceWatcher> socket_performance_watcher, net::NetLog* net_log, const net::NetLogSource& source)
...
TCPSocketWin::TCPSocketWin(std::unique_ptr<SocketPerformanceWatcher> socket_performance_watcher, NetLogWithSource net_log_source)
    : socket_(INVALID_SOCKET)
    , socket_performance_watcher_(std::move(socket_performance_watcher))
    , accept_event_(WSA_INVALID_EVENT)
...
int TCPSocketWin::Open(AddressFamily family)
{
    ...
    socket_ = CreatePlatformSocket(ConvertAddressFamily(family), SOCK_STREAM, IPPROTO_TCP);
    int os_error = WSAGetLastError();
...
int TCPSocketWin::AdoptConnectedSocket(SocketDescriptor socket, const IPEndPoint& peer_address)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    DCHECK_EQ(socket_, INVALID_SOCKET);
    DCHECK(!core_.get());
...
int TCPSocketWin::Accept(std::unique_ptr<TCPSocketWin>* socket, IPEndPoint* address, CompletionOnceCallback callback)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    DCHECK(socket);
    DCHECK(address);
...
int TCPSocketDefaultWin::Read(IOBuffer* buf, int buf_len, CompletionOnceCallback callback)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    // is gone.
    int rv = ReadIfReady(buf, buf_len, base::BindOnce(&TCPSocketDefaultWin::RetryRead, base::Unretained(this)));
    if (rv != ERR_IO_PENDING)
        return rv;
...
int TCPSocketDefaultWin::ReadIfReady(IOBuffer* buf, int buf_len, CompletionOnceCallback callback)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    DCHECK_NE(socket_, INVALID_SOCKET);
    ...
    if (rv == SOCKET_ERROR) {
        if (os_error != WSAEWOULDBLOCK) {
            int net_error = MapSystemError(os_error);
            NetLogSocketError(net_log_, NetLogEventType::SOCKET_READ_ERROR, net_error, os_error);
            return net_error;
        }
    } else {
        net_log_.AddByteTransferEvent(NetLogEventType::SOCKET_BYTES_RECEIVED, rv, buf->data());
...
int TCPSocketDefaultWin::Write(IOBuffer* buf, int buf_len, CompletionOnceCallback callback, const NetworkTrafficAnnotationTag& /* traffic_annotation */)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    DCHECK_NE(socket_, INVALID_SOCKET);
    ...
    DWORD num;
    int rv = WSASend(socket_, &write_buffer, 1, &num, 0, &core.write_overlapped_, nullptr);
    int os_error = WSAGetLastError();
    if (rv == 0) {
        if (ResetEventIfSignaled(core.write_overlapped_.hEvent)) {
            ...
            if (rv > buf_len || rv < 0) {
                ...
                LOG(ERROR) << "Detected broken LSP: Asked to write " << buf_len << " bytes, but " << rv << " bytes reported.";
                return ERR_WINSOCK_UNEXPECTED_WRITTEN_BYTES;
            }
            net_log_.AddByteTransferEvent(NetLogEventType::SOCKET_BYTES_SENT, rv, buf->data());
            return rv;
        }
    } else {
        if (os_error != WSA_IO_PENDING) {
            int net_error = MapSystemError(os_error);
            NetLogSocketError(net_log_, NetLogEventType::SOCKET_WRITE_ERROR, net_error, os_error);
            return net_error;
        }
    }
...
int TCPSocketWin::SetExclusiveAddrUse()
{
    ...
    BOOL true_value = 1;
    int rv = setsockopt(socket_, SOL_SOCKET, SO_EXCLUSIVEADDRUSE, reinterpret_cast<const char*>(&true_value), sizeof(true_value));
    if (rv < 0)
...
void TCPSocketWin::StartLoggingMultipleConnectAttempts(const AddressList& addresses)
{
    if (!logging_multiple_connect_attempts_) {
...
int TCPSocketWin::AcceptInternal(std::unique_ptr<TCPSocketWin>* socket, IPEndPoint* address)
{
    SockaddrStorage storage;
    ...
    auto tcp_socket = TCPSocketWin::Create(nullptr, net_log_.net_log(), net_log_.source());
    int adopt_result = tcp_socket->AdoptConnectedSocket(new_socket, ip_end_point);
    if (adopt_result != OK) {
        net_log_.EndEventWithNetErrorCode(NetLogEventType::TCP_ACCEPT, adopt_result);
        return adopt_result;
    }
    *socket = std::move(tcp_socket);
    *address = ip_end_point;
    net_log_.EndEvent(NetLogEventType::TCP_ACCEPT, [&] { return CreateNetLogIPEndPointParams(&ip_end_point); });
    return OK;
}
...
int TCPSocketWin::DoConnect()
{
    ...
    net_log_.BeginEvent(NetLogEventType::TCP_CONNECT_ATTEMPT, [&] { return CreateNetLogIPEndPointParams(peer_address_.get()); });
    ...
    if (base::win::GetVersion() >= base::win::Version::WIN10_20H1 && base::FeatureList::IsEnabled(features::kEnableTcpPortRandomization)) {
        BOOL randomize_port = TRUE;
        setsockopt(socket_, SOL_SOCKET, SO_RANDOMIZE_PORT, reinterpret_cast<const char*>(&randomize_port), sizeof(randomize_port));
    }
...
void TCPSocketWin::DoConnectComplete(int result)
{
    ...
    connect_os_error_ = 0;
    if (result != OK) {
        net_log_.EndEventWithIntParams(NetLogEventType::TCP_CONNECT_ATTEMPT, "os_error", os_error);
    } else {
...
{
    net_log_.BeginEvent(NetLogEventType::TCP_CONNECT, [&] { return addresses.NetLogParams(); });
}
...
void TCPSocketDefaultWin::RetryRead(int rv)
{
    ...
    if (rv == OK) {
        ...
        // |this| is gone.
        rv = ReadIfReady(core.read_iobuffer_.get(), core.read_buffer_length_, base::BindOnce(&TCPSocketDefaultWin::RetryRead, base::Unretained(this)));
        if (rv == ERR_IO_PENDING)
            return;
    }
...
void TCPSocketWin::DidCompleteConnect()
{
    ...
    int os_error = WSAGetLastError();
    if (rv == SOCKET_ERROR) {
        DLOG(FATAL) << "WSAEnumNetworkEvents() failed with SOCKET_ERROR, os_error = " << os_error;
...
void TCPSocketDefaultWin::DidCompleteWrite()
{
    ...
    BOOL ok = WSAGetOverlappedResult(socket_, &core.write_overlapped_, &num_bytes, FALSE, &flags);
    int os_error = WSAGetLastError();
    WSAResetEvent(core.write_overlapped_.hEvent);
    int rv;
    if (!ok) {
        rv = MapSystemError(os_error);
        NetLogSocketError(net_log_, NetLogEventType::SOCKET_WRITE_ERROR, rv, os_error);
    } else {
        rv = static_cast<int>(num_bytes);
        if (rv > core.write_buffer_length_ || rv < 0) {
            // It seems that some winsock interceptors report that more was written
            // than was available. Treat this as an error.  http://crbug.com/27870
            LOG(ERROR) << "Detected broken LSP: Asked to write " << core.write_buffer_length_ << " bytes, but " << rv << " bytes reported.";
            rv = ERR_WINSOCK_UNEXPECTED_WRITTEN_BYTES;
        } else {
            net_log_.AddByteTransferEvent(NetLogEventType::SOCKET_BYTES_SENT, num_bytes, core.write_iobuffer_->data());
        }
    }
...
TCPSocketDefaultWin::TCPSocketDefaultWin(std::unique_ptr<SocketPerformanceWatcher> socket_performance_watcher, NetLog* net_log, const NetLogSource& source)
    : TCPSocketWin(std::move(socket_performance_watcher), net_log, source)
{
}

TCPSocketDefaultWin::TCPSocketDefaultWin(std::unique_ptr<SocketPerformanceWatcher> socket_performance_watcher, NetLogWithSource net_log_source)
    : TCPSocketWin(std::move(socket_performance_watcher), std::move(net_log_source))
{
}
