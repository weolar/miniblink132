...
public:
    ...
    struct Rule {
        explicit Rule(Type type, std::optional<std::vector<IPEndPoint>> expected_addresses = std::nullopt, Error connect_error = ERR_CONNECTION_FAILED);
        ~Rule();
        Rule(const Rule&);
    ...
    MockTransportClientSocketFactory(const MockTransportClientSocketFactory&) = delete;
    MockTransportClientSocketFactory& operator=(const MockTransportClientSocketFactory&) = delete;
    ...
    void set_default_client_socket_type(Type type)
    {
