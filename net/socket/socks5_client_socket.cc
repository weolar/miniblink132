...
SOCKS5ClientSocket::SOCKS5ClientSocket(
    std::unique_ptr<StreamSocket> transport_socket, const HostPortPair& destination, const NetworkTrafficAnnotationTag& traffic_annotation)
    : io_callback_(base::BindRepeating(&SOCKS5ClientSocket::OnIOComplete, base::Unretained(this)))
...
int SOCKS5ClientSocket::Read(IOBuffer* buf, int buf_len, CompletionOnceCallback callback)
{
    ...
    int rv = transport_socket_->Read(buf, buf_len, base::BindOnce(&SOCKS5ClientSocket::OnReadWriteComplete, base::Unretained(this), std::move(callback)));
    if (rv > 0)
        was_ever_used_ = true;
    return rv;
}
...
int SOCKS5ClientSocket::Write(IOBuffer* buf, int buf_len, CompletionOnceCallback callback, const NetworkTrafficAnnotationTag& traffic_annotation)
{
    DCHECK(completed_handshake_);
    DCHECK_EQ(STATE_NONE, next_state_);
    DCHECK(user_callback_.is_null());
    ...
    int rv = transport_socket_->Write(
        buf, buf_len, base::BindOnce(&SOCKS5ClientSocket::OnReadWriteComplete, base::Unretained(this), std::move(callback)), traffic_annotation);
    if (rv > 0)
        was_ever_used_ = true;
    return rv;
}
...
void SOCKS5ClientSocket::OnReadWriteComplete(CompletionOnceCallback callback, int result)
{
    DCHECK_NE(ERR_IO_PENDING, result);
    DCHECK(!callback.is_null());
...
int SOCKS5ClientSocket::DoLoop(int last_io_result)
{
    ...
    do {
        ...
        case STATE_GREET_WRITE_COMPLETE:
            ...
            net_log_.EndEventWithNetErrorCode(NetLogEventType::SOCKS5_GREET_WRITE, rv);
            break;
        ...
        case STATE_GREET_READ_COMPLETE:
            ...
            net_log_.EndEventWithNetErrorCode(NetLogEventType::SOCKS5_GREET_READ, rv);
            break;
        ...
        case STATE_HANDSHAKE_WRITE_COMPLETE:
            ...
            net_log_.EndEventWithNetErrorCode(NetLogEventType::SOCKS5_HANDSHAKE_WRITE, rv);
            break;
        ...
        case STATE_HANDSHAKE_READ_COMPLETE:
            ...
            net_log_.EndEventWithNetErrorCode(NetLogEventType::SOCKS5_HANDSHAKE_READ, rv);
            break;
        default:
...
int SOCKS5ClientSocket::DoGreetWrite()
{
    ...
    if (buffer_.empty()) {
        buffer_ = std::string(kSOCKS5GreetWriteData, std::size(kSOCKS5GreetWriteData));
        bytes_sent_ = 0;
    }
    ...
    memcpy(handshake_buf_->data(), &buffer_.data()[bytes_sent_], handshake_buf_len);
    return transport_socket_->Write(handshake_buf_.get(), handshake_buf_len, io_callback_, traffic_annotation_);
}
...
int SOCKS5ClientSocket::DoGreetRead()
{
    ...
    handshake_buf_ = base::MakeRefCounted<IOBufferWithSize>(handshake_buf_len);
    return transport_socket_->Read(handshake_buf_.get(), handshake_buf_len, io_callback_);
}
...
int SOCKS5ClientSocket::DoGreetReadComplete(int result)
{
    ...
    if (result == 0) {
        net_log_.AddEvent(NetLogEventType::SOCKS_UNEXPECTEDLY_CLOSED_DURING_GREETING);
        return ERR_SOCKS_CONNECTION_FAILED;
    }
    ...
    if (buffer_[0] != kSOCKS5Version) {
        net_log_.AddEventWithIntParams(NetLogEventType::SOCKS_UNEXPECTED_VERSION, "version", buffer_[0]);
        return ERR_SOCKS_CONNECTION_FAILED;
    }
    if (buffer_[1] != 0x00) {
        net_log_.AddEventWithIntParams(NetLogEventType::SOCKS_UNEXPECTED_AUTH, "method", buffer_[1]);
        return ERR_SOCKS_CONNECTION_FAILED;
    }
...
int SOCKS5ClientSocket::BuildHandshakeWriteBuffer(std::string* handshake) const
{
...
int SOCKS5ClientSocket::DoHandshakeReadComplete(int result)
{
    ...
    if (result == 0) {
        net_log_.AddEvent(NetLogEventType::SOCKS_UNEXPECTEDLY_CLOSED_DURING_HANDSHAKE);
        return ERR_SOCKS_CONNECTION_FAILED;
    }
    ...
    if (bytes_received_ == kReadHeaderSize) {
        ...
        if (buffer_[1] != 0x00) {
            net_log_.AddEventWithIntParams(NetLogEventType::SOCKS_SERVER_ERROR, "error_code", buffer_[1]);
            return ERR_SOCKS_CONNECTION_FAILED;
        }
        ...
        if (address_type == kEndPointDomain) {
            ...
            net_log_.AddEventWithIntParams(NetLogEventType::SOCKS_UNKNOWN_ADDRESS_TYPE, "address_type", buffer_[3]);
            return ERR_SOCKS_CONNECTION_FAILED;
        }
