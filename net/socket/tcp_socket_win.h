...
public:
    ...
    static std::unique_ptr<TCPSocketWin> Create(std::unique_ptr<SocketPerformanceWatcher> socket_performance_watcher, NetLogWithSource net_log_source);
    ...
    int Accept(std::unique_ptr<TCPSocketWin>* socket, IPEndPoint* address, CompletionOnceCallback callback);
    ...
    virtual int ReadIfReady(IOBuffer* buf, int buf_len, CompletionOnceCallback callback) = 0;
...
protected:
    ...
    TCPSocketWin(std::unique_ptr<SocketPerformanceWatcher> socket_performance_watcher, NetLog* net_log, const NetLogSource& source);

    TCPSocketWin(std::unique_ptr<SocketPerformanceWatcher> socket_performance_watcher, NetLogWithSource net_log_source);
    ...
    int AcceptInternal(std::unique_ptr<TCPSocketWin>* socket, IPEndPoint* address);
