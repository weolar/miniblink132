...
public:
    ...
    virtual int Listen(const IPEndPoint& address, int backlog, std::optional<bool> ipv6_only) = 0;
    ...
    virtual int ListenWithAddressAndPort(const std::string& address_string, uint16_t port, int backlog);
    ...
    // accepted.
    virtual int Accept(std::unique_ptr<StreamSocket>* socket, CompletionOnceCallback callback) = 0;
    ...
    // implementation.
    virtual int Accept(std::unique_ptr<StreamSocket>* socket, CompletionOnceCallback callback, IPEndPoint* peer_address);
};
