...
struct NET_EXPORT_PRIVATE StreamAttemptParams {
    static StreamAttemptParams FromHttpNetworkSession(HttpNetworkSession* session);

    StreamAttemptParams(ClientSocketFactory* client_socket_factory, SSLClientContext* ssl_client_context,
...
public:
    ...
    StreamAttempt(const StreamAttemptParams* params, IPEndPoint ip_endpoint, NetLogSourceType net_log_source_type, NetLogEventType net_log_attempt_event_type,
    ...
    StreamSocket* stream_socket() const
    {
        return stream_socket_.get();
    }
