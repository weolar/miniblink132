...
protected:
    ...
    void SetUpIPv4()
    {
        ...
        ASSERT_THAT(socket_.Listen(address, kListenBacklog, /*ipv6_only=*/std::nullopt), IsOk());
    ...
    void SetUpIPv6(bool* success)
    {
        *success = false;
        ...
        if (socket_.Listen(address, kListenBacklog, /*ipv6_only=*/std::nullopt) != 0) {
...
TEST_F(TCPServerSocketTest, Accept)
{
    ...
    int result = socket_.Accept(&accepted_socket, accept_callback.callback(), &peer_address);
    ...
    EXPECT_EQ(GetPeerAddress(accepted_socket.get()).address(), local_address_.address());
...
TEST_F(TCPServerSocketTest, AcceptAsync)
{
    ...
    ASSERT_THAT(socket_.Accept(&accepted_socket, accept_callback.callback(), &peer_address), IsError(ERR_IO_PENDING));
...
TEST_F(TCPServerSocketTest, AcceptClientDisconnectAfterConnect)
{
    ...
    int accept_result = socket_.Accept(&accepted_socket, accept_callback.callback(), &peer_address);
...
TEST_F(TCPServerSocketTest, Accept2Connections)
{
    ...
    ASSERT_EQ(ERR_IO_PENDING, socket_.Accept(&accepted_socket, accept_callback.callback(), &peer_address));
    ...
    int result = socket_.Accept(&accepted_socket2, accept_callback2.callback(), &peer_address2);
    ...
    EXPECT_EQ(GetPeerAddress(accepted_socket2.get()).address(), local_address_.address());
}
...
class TCPServerSocketTestWithIPv6Only : public TCPServerSocketTest, public testing::WithParamInterface<bool> {
public:
    void AttemptToConnect(const IPAddress& dest_addr, bool should_succeed)
    {
        TCPClientSocket connecting_socket(AddressList(IPEndPoint(dest_addr, local_address_.port())), nullptr, nullptr, nullptr, NetLogSource());
        ...
        int accept_result = socket_.Accept(&accepted_socket, accept_cb.callback(), &peer_address);
...
TEST_F(TCPServerSocketTest, AcceptIO)
{
    ...
    while (bytes_written < message.size()) {
        scoped_refptr<IOBufferWithSize> write_buffer = base::MakeRefCounted<IOBufferWithSize>(message.size() - bytes_written);
    ...
    while (bytes_read < message.size()) {
        scoped_refptr<IOBufferWithSize> read_buffer = base::MakeRefCounted<IOBufferWithSize>(message.size() - bytes_read);
