...
public:
    ...
    public:
        ...
        // result in an error.
        Request(ClientSocketHandle* handle, CompletionOnceCallback callback, const ProxyAuthCallback& proxy_auth_callback, RequestPriority priority,
            const SocketTag& socket_tag, RespectLimits respect_limits, Flags flags, scoped_refptr<SocketParams> socket_params,
        ...
        CompletionOnceCallback release_callback()
        {
        ...
        RespectLimits respect_limits() const
        {
        ...
        SocketParams* socket_params() const
        {
            return socket_params_.get();
        }
        const std::optional<NetworkTrafficAnnotationTag>& proxy_annotation_tag() const
        {
    ...
    TransportClientSocketPool(int max_sockets, int max_sockets_per_group, base::TimeDelta unused_idle_socket_timeout, const ProxyChain& proxy_chain,
        bool is_for_websockets, const CommonConnectJobParams* common_connect_job_params, bool cleanup_on_ip_address_change = true);

    TransportClientSocketPool(const TransportClientSocketPool&) = delete;
    TransportClientSocketPool& operator=(const TransportClientSocketPool&) = delete;
    ...
    static std::unique_ptr<TransportClientSocketPool> CreateForTesting(int max_sockets, int max_sockets_per_group, base::TimeDelta unused_idle_socket_timeout,
        base::TimeDelta used_idle_socket_timeout, const ProxyChain& proxy_chain_, bool is_for_websockets,
    ...
    int RequestSocket(const GroupId& group_id, scoped_refptr<SocketParams> params, const std::optional<NetworkTrafficAnnotationTag>& proxy_annotation_tag,
        RequestPriority priority, const SocketTag& socket_tag, RespectLimits respect_limits, ClientSocketHandle* handle, CompletionOnceCallback callback,
        const ProxyAuthCallback& proxy_auth_callback, const NetLogWithSource& net_log) override;
    int RequestSockets(const GroupId& group_id, scoped_refptr<SocketParams> params, const std::optional<NetworkTrafficAnnotationTag>& proxy_annotation_tag,
        int num_sockets, CompletionOnceCallback callback, const NetLogWithSource& net_log) override;
    ...
    void ReleaseSocket(const GroupId& group_id, std::unique_ptr<StreamSocket> socket, int64_t group_generation) override;
    ...
    void CloseIdleSocketsInGroup(const GroupId& group_id, const char* net_log_reason_utf8) override;
    int IdleSocketCount() const override;
    size_t IdleSocketCountInGroup(const GroupId& group_id) const override;
    LoadState GetLoadState(const GroupId& group_id, const ClientSocketHandle* handle) const override;
    base::Value GetInfoAsValue(const std::string& name, const std::string& type) const override;
    bool HasActiveSocket(const GroupId& group_id) const override;

    bool RequestInGroupWithHandleHasJobForTesting(const GroupId& group_id, const ClientSocketHandle* handle) const
    {
        return group_map_.find(group_id)->second->RequestWithHandleHasJobForTesting(handle);
    }

    size_t NumNeverAssignedConnectJobsInGroupForTesting(const GroupId& group_id) const
    {
        return NumNeverAssignedConnectJobsInGroup(group_id);
    }

    size_t NumUnassignedConnectJobsInGroupForTesting(const GroupId& group_id) const
    {
    ...
    void OnSSLConfigChanged(SSLClientContext::SSLConfigChangeType change_type) override;
    void OnSSLConfigForServersChanged(const base::flat_set<HostPortPair>& servers) override;
...
private:
    ...
    public:
        ...
        struct BoundRequest {
            BoundRequest();
            BoundRequest(std::unique_ptr<ConnectJob> connect_job, std::unique_ptr<Request> request, int64_t generation);
        ...
        Group(const GroupId& group_id, TransportClientSocketPool* client_socket_pool);
        ~Group() override;

        // ConnectJob::Delegate methods:
        void OnConnectJobComplete(int result, ConnectJob* job) override;
        void OnNeedsProxyAuth(
            const HttpResponseInfo& response, HttpAuthController* auth_controller, base::OnceClosure restart_with_auth_callback, ConnectJob* job) override;

        bool IsEmpty() const
        {
            return active_socket_count_ == 0 && idle_sockets_.empty() && jobs_.empty() && unbound_requests_.empty() && bound_requests_.empty();
        }
        ...
        {
            return active_socket_count_ + static_cast<int>(jobs_.size()) + static_cast<int>(idle_sockets_.size()) + static_cast<int>(bound_requests_.size());
        }
        ...
        {
            return HasAvailableSocketSlot(max_sockets_per_group) && unbound_requests_.size() > jobs_.size();
        }
        ...
        bool has_unbound_requests() const
        {
            return !unbound_requests_.empty();
        }

        size_t unbound_request_count() const
        {
            return unbound_requests_.size();
        }
        ...
        std::unique_ptr<Request> FindAndRemoveUnboundRequest(ClientSocketHandle* handle);
        ...
        std::optional<BoundRequest> FindAndRemoveBoundRequestForConnectJob(ConnectJob* connect_job);
        ...
        // was one.
        std::unique_ptr<Request> FindAndRemoveBoundRequest(ClientSocketHandle* client_socket_handle);
        ...
        void IncrementActiveSocketCount()
        {
            active_socket_count_++;
        }
        void DecrementActiveSocketCount()
        {
            active_socket_count_--;
        }
        ...
        bool RequestWithHandleHasJobForTesting(const ClientSocketHandle* handle) const;

        const GroupId& group_id()
        {
            return group_id_;
        }
        size_t unassigned_job_count() const
        {
            return unassigned_jobs_.size();
        }
        const JobList& jobs() const
        {
            return jobs_;
        }
        const std::list<IdleSocket>& idle_sockets() const
        {
            return idle_sockets_;
        }
        int active_socket_count() const
        {
            return active_socket_count_;
        }
        std::list<IdleSocket>* mutable_idle_sockets()
        {
            return &idle_sockets_;
        }
    ...
    private:
        ...
        std::unique_ptr<Request> RemoveUnboundRequest(const RequestQueue::Pointer& pointer);
        ...
        // when called.
        RequestQueue::Pointer FindUnboundRequestWithJob(const ConnectJob* job) const;
    ...
    using PendingCallbackMap = std::map<const ClientSocketHandle*, CallbackResultPair>;

    TransportClientSocketPool(int max_sockets, int max_sockets_per_group, base::TimeDelta unused_idle_socket_timeout, base::TimeDelta used_idle_socket_timeout,
    ...
    // reused.
    void CleanupIdleSocketsInGroup(bool force, Group* group, const base::TimeTicks& now, const char* net_log_reason_utf8);
    ...
    void HandOutSocket(std::unique_ptr<StreamSocket> socket, ClientSocketHandle::SocketReuseType reuse_type,
        const LoadTimingInfo::ConnectTiming& connect_timing, ClientSocketHandle* handle, base::TimeDelta time_idle, Group* group,
    ...
    int RequestSocketInternal(const GroupId& group_id, const Request& request, base::OnceClosure preconnect_done_closure);
    ...
    static void LogBoundConnectJobToRequest(const NetLogSource& connect_job_source, const Request& request);
    ...
    // created.
    void InvokeUserCallbackLater(ClientSocketHandle* handle, CompletionOnceCallback callback, int rv, const SocketTag& socket_tag);
    ...
    // refreshed.
    GroupMap::iterator RefreshGroup(GroupMap::iterator it, const base::TimeTicks& now, const char* net_log_reason_utf8);
