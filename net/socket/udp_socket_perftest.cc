...
perf_test::PerfResultReporter SetUpUDPSocketReporter(const std::string& story)
{
    ...
    reporter.RegisterImportantMetric(kMetricWriteSpeedBytesPerSecond, "bytesPerSecond_biggerIsBetter");
    return reporter;
}
...
public:
    ...
    void DoneWritePacketsToSocket(UDPClientSocket* socket, int num_of_packets, base::OnceClosure* done_callback, int error)
    {
        WritePacketsToSocket(socket, num_of_packets, done_callback);
    }

    // Send |num_of_packets| to |socket|. Invoke |done_callback| when done.
    void WritePacketsToSocket(UDPClientSocket* socket, int num_of_packets, base::OnceClosure* done_callback);
...
void CreateUDPAddress(const std::string& ip_str, uint16_t port, IPEndPoint* address)
{
...
void UDPSocketPerfTest::WritePacketsToSocket(UDPClientSocket* socket, int num_of_packets, base::OnceClosure* done_callback)
{
    scoped_refptr<IOBufferWithSize> io_buffer = base::MakeRefCounted<IOBufferWithSize>(kPacketSize);
    memset(io_buffer->data(), 'G', kPacketSize);

    while (num_of_packets) {
        int rv = socket->Write(io_buffer.get(), io_buffer->size(),
            base::BindOnce(&UDPSocketPerfTest::DoneWritePacketsToSocket, weak_factory_.GetWeakPtr(), socket, num_of_packets - 1, done_callback),
...
void UDPSocketPerfTest::WriteBenchmark(bool use_nonblocking_io)
{
    ...
    auto client = std::make_unique<UDPClientSocket>(DatagramSocket::DEFAULT_BIND, nullptr, NetLogSource());
    if (use_nonblocking_io)
    ...
    auto reporter = SetUpUDPSocketReporter(use_nonblocking_io ? "nonblocking" : "blocking");
    reporter.AddResult(kMetricElapsedTimeMs, total_elapsed);
    reporter.AddResult(kMetricWriteSpeedBytesPerSecond, packets * 1024 / write_elapsed);
}
