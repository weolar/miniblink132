...
BOOL QwaveApi::AddSocketToFlow(HANDLE handle, SOCKET socket, PSOCKADDR addr, QOS_TRAFFIC_TYPE traffic_type, DWORD flags, PQOS_FLOWID flow_id)
{
    return add_socket_to_flow_func_(handle, socket, addr, traffic_type, flags, flow_id);
}

BOOL QwaveApi::RemoveSocketFromFlow(HANDLE handle, SOCKET socket, QOS_FLOWID flow_id, DWORD reserved)
{
    return remove_socket_from_flow_func_(handle, socket, flow_id, reserved);
}

BOOL QwaveApi::SetFlow(HANDLE handle, QOS_FLOWID flow_id, QOS_SET_FLOW op, ULONG size, PVOID data, DWORD reserved, LPOVERLAPPED overlapped)
{
...
UDPSocketWin::UDPSocketWin(DatagramSocket::BindType bind_type, net::NetLog* net_log, const net::NetLogSource& source)
...
UDPSocketWin::UDPSocketWin(DatagramSocket::BindType bind_type, NetLogWithSource source_net_log)
    : socket_(INVALID_SOCKET)
    , socket_options_(SOCKET_OPTION_MULTICAST_LOOP)
    , net_log_(source_net_log)
{
    EnsureWinsockInit();
    net_log_.BeginEventReferencingSource(NetLogEventType::SOCKET_ALIVE, net_log_.source());
}
...
int UDPSocketWin::AdoptOpenedSocket(AddressFamily address_family, SOCKET socket)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    auto owned_socket_count = TryAcquireGlobalUDPSocketCount();
    if (owned_socket_count.empty()) {
...
void UDPSocketWin::Close()
{
    ...
    closesocket(socket_);
    UMA_HISTOGRAM_TIMES("Net.UDPSocketWinClose", base::TimeTicks::Now() - start_time);
    socket_ = INVALID_SOCKET;
    addr_family_ = 0;
    is_connected_ = false;
...
int UDPSocketWin::GetLocalAddress(IPEndPoint* address) const
{
    ...
    if (!local_address_.get()) {
        ...
        net_log_.AddEvent(NetLogEventType::UDP_LOCAL_ADDRESS, [&] { return CreateNetLogUDPConnectParams(*local_address_, handles::kInvalidNetworkHandle); });
    }
...
int UDPSocketWin::Read(IOBuffer* buf, int buf_len, CompletionOnceCallback callback)
{
    return RecvFrom(buf, buf_len, nullptr, std::move(callback));
}

int UDPSocketWin::RecvFrom(IOBuffer* buf, int buf_len, IPEndPoint* address, CompletionOnceCallback callback)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    int nread = core_ ? InternalRecvFromOverlapped(buf, buf_len, address) : InternalRecvFromNonBlocking(buf, buf_len, address);
...
int UDPSocketWin::Write(IOBuffer* buf, int buf_len, CompletionOnceCallback callback, const NetworkTrafficAnnotationTag& /* traffic_annotation */)
{
    return SendToOrWrite(buf, buf_len, remote_address_.get(), std::move(callback));
}

int UDPSocketWin::SendTo(IOBuffer* buf, int buf_len, const IPEndPoint& address, CompletionOnceCallback callback)
{
    if (dscp_manager_) {
...
int UDPSocketWin::SendToOrWrite(IOBuffer* buf, int buf_len, const IPEndPoint* address, CompletionOnceCallback callback)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    DCHECK_NE(INVALID_SOCKET, socket_);
    ...
    int nwrite = core_ ? InternalSendToOverlapped(buf, buf_len, address) : InternalSendToNonBlocking(buf, buf_len, address);
...
int UDPSocketWin::Connect(const IPEndPoint& address)
{
    ...
    net_log_.BeginEvent(NetLogEventType::UDP_CONNECT, [&] { return CreateNetLogUDPConnectParams(address, handles::kInvalidNetworkHandle); });
...
int UDPSocketWin::InternalConnect(const IPEndPoint& address)
{
    ...
    setsockopt(socket_, SOL_SOCKET, SO_RANDOMIZE_PORT, reinterpret_cast<const char*>(&randomize_port_value), sizeof(randomize_port_value));
...
int UDPSocketWin::SetReceiveBufferSize(int32_t size)
{
    ...
    rv = getsockopt(socket_, SOL_SOCKET, SO_RCVBUF, reinterpret_cast<char*>(&actual_size), &option_size);
    if (rv != 0)
        return MapSystemError(WSAGetLastError());
    if (actual_size >= size)
        return OK;
    UMA_HISTOGRAM_CUSTOM_COUNTS("Net.SocketUnchangeableReceiveBuffer", actual_size, 1000, 1000000, 50);
...
int UDPSocketWin::SetSendBufferSize(int32_t size)
{
    ...
    int32_t actual_size = 0;
    int option_size = sizeof(actual_size);
    rv = getsockopt(socket_, SOL_SOCKET, SO_SNDBUF, reinterpret_cast<char*>(&actual_size), &option_size);
    if (rv != 0)
        return MapSystemError(WSAGetLastError());
    if (actual_size >= size)
        return OK;
    UMA_HISTOGRAM_CUSTOM_COUNTS("Net.SocketUnchangeableSendBuffer", actual_size, 1000, 1000000, 50);
...
int UDPSocketWin::SetDoNotFragment()
{
    ...
    DWORD val = 1;
    int rv = setsockopt(socket_, IPPROTO_IP, IP_DONTFRAGMENT, reinterpret_cast<const char*>(&val), sizeof(val));
...
LPFN_WSARECVMSG UDPSocketWin::GetRecvMsgPointer()
{
    ...
    DWORD size;
    if (WSAIoctl(socket_, SIO_GET_EXTENSION_FUNCTION_POINTER, &message_code, sizeof(message_code), &rv, sizeof(rv), &size, NULL, NULL) == SOCKET_ERROR) {
        return nullptr;
    }
    return rv;
}
...
int UDPSocketWin::SetRecvTos()
{
    ...
    if (addr_family_ == AF_INET6 && !address.address().IsIPv4MappedIPv6()) {
        rv = setsockopt(socket_, IPPROTO_IPV6, IPV6_RECVECN, reinterpret_cast<const char*>(&ecn), sizeof(ecn));
        if (rv != 0) {
        ...
        if (!address.address().IsZero()) {
            ...
            rv = getsockopt(socket_, IPPROTO_IPV6, IPV6_V6ONLY, reinterpret_cast<char*>(&v6_only), &option_size);
            if (rv != 0) {
                return LogAndReturnError();
            }
        }
    ...
    if (v6_only == 0) {
        rv = setsockopt(socket_, IPPROTO_IP, IP_RECVECN, reinterpret_cast<const char*>(&ecn), sizeof(ecn));
        if (rv != 0) {
            return LogAndReturnError();
        }
    }
...
int UDPSocketWin::AllowAddressReuse()
{
    ...
    int rv = setsockopt(socket_, SOL_SOCKET, SO_REUSEADDR, reinterpret_cast<const char*>(&true_value), sizeof(true_value));
    return rv == 0 ? OK : MapSystemError(WSAGetLastError());
}
...
int UDPSocketWin::SetBroadcast(bool broadcast)
{
    ...
    int rv = setsockopt(socket_, SOL_SOCKET, SO_BROADCAST, reinterpret_cast<const char*>(&value), sizeof(value));
    return rv == 0 ? OK : MapSystemError(WSAGetLastError());
}
...
void UDPSocketWin::DidCompleteRead()
{
    ...
    BOOL ok = WSAGetOverlappedResult(socket_, &core_->read_overlapped_, &num_bytes, FALSE, &flags);
    ...
    if (result >= 0) {
        if (address.FromSockAddr(core_->recv_addr_storage_.addr, core_->recv_addr_storage_.addr_len)) {
...
{
    DWORD num_bytes, flags;
    BOOL ok = WSAGetOverlappedResult(socket_, &core_->write_overlapped_, &num_bytes, FALSE, &flags);
...
{
    int rv = InternalRecvFromNonBlocking(read_iobuffer_.get(), read_iobuffer_len_, recv_from_address_);
...
{
    int rv = InternalSendToNonBlocking(write_iobuffer_.get(), write_iobuffer_len_, send_to_address_.get());
    if (rv == ERR_IO_PENDING) {
        return;
    }
    write_iobuffer_ = nullptr;
...
void UDPSocketWin::LogRead(int result, const char* bytes, const IPEndPoint* address) const
{
    if (result < 0) {
        net_log_.AddEventWithNetErrorCode(NetLogEventType::UDP_RECEIVE_ERROR, result);
        return;
    }

    if (net_log_.IsCapturing()) {
        NetLogUDPDataTransfer(net_log_, NetLogEventType::UDP_BYTES_RECEIVED, result, bytes, address);
    }

    activity_monitor::IncrementBytesReceived(result);
}

void UDPSocketWin::LogWrite(int result, const char* bytes, const IPEndPoint* address) const
{
    if (result < 0) {
    ...
    if (net_log_.IsCapturing()) {
        NetLogUDPDataTransfer(net_log_, NetLogEventType::UDP_BYTES_SENT, result, bytes, address);
    }
}

void UDPSocketWin::PopulateWSAMSG(WSAMSG& message, SockaddrStorage& storage, WSABUF* data_buffer, WSABUF& control_buffer, bool send)
{
    bool is_ipv6;
...
void UDPSocketWin::SetLastTosFromWSAMSG(WSAMSG& message)
{
    ...
    for (WSACMSGHDR* cmsg = WSA_CMSG_FIRSTHDR(&message); cmsg != NULL; cmsg = WSA_CMSG_NXTHDR(&message, cmsg)) {
        if ((cmsg->cmsg_level == IPPROTO_IPV6 && cmsg->cmsg_type == IPV6_ECN) || (cmsg->cmsg_level == IPPROTO_IP && cmsg->cmsg_type == IP_ECN)) {
...
int UDPSocketWin::InternalRecvFromOverlapped(IOBuffer* buf, int buf_len, IPEndPoint* address)
{
    ...
    if (report_ecn_) {
        ...
        rv = wsa_recv_msg_(socket_, message.get(), &num, &core_->read_overlapped_, nullptr);
        if (rv == 0) {
            SetLastTosFromWSAMSG(*message);
        }
    } else {
        rv = WSARecvFrom(socket_, &read_buffer, 1, &num, &flags, storage.addr, &storage.addr_len, &core_->read_overlapped_, nullptr);
    }
    if (rv == 0) {
        if (ResetEventIfSignaled(core_->read_overlapped_.hEvent)) {
            ...
            IPEndPoint* address_to_log = nullptr;
            if (result >= 0) {
                if (address_storage.FromSockAddr(core_->recv_addr_storage_.addr, core_->recv_addr_storage_.addr_len)) {
                    if (address) {
...
int UDPSocketWin::InternalSendToOverlapped(IOBuffer* buf, int buf_len, const IPEndPoint* address)
{
    ...
    if (send_ecn_ != ECN_NOT_ECT) {
        ...
        rv = wsa_send_msg_(socket_, &message, flags, &num, &core_->write_overlapped_, nullptr);
    } else {
        rv = WSASendTo(socket_, &write_buffer, 1, &num, flags, addr, storage.addr_len, &core_->write_overlapped_, nullptr);
    }
    if (rv == 0) {
...
int UDPSocketWin::InternalRecvFromNonBlocking(IOBuffer* buf, int buf_len, IPEndPoint* address)
{
    ...
    if (report_ecn_) {
        ...
        rv = recvfrom(socket_, buf->data(), buf_len, 0, storage.addr, &storage.addr_len);
    }
    if (rv == SOCKET_ERROR) {
        int os_error = WSAGetLastError();
...
int UDPSocketWin::InternalSendToNonBlocking(IOBuffer* buf, int buf_len, const IPEndPoint* address)
{
...
int UDPSocketWin::SetMulticastOptions()
{
    if (!(socket_options_ & SOCKET_OPTION_MULTICAST_LOOP)) {
        ...
        int rv = setsockopt(socket_, protocol_level, option, reinterpret_cast<const char*>(&loop), sizeof(loop));
        if (rv < 0) {
            return MapSystemError(WSAGetLastError());
        }
    }
    if (multicast_time_to_live_ != 1) {
        ...
        int rv = setsockopt(socket_, protocol_level, option, reinterpret_cast<const char*>(&hops), sizeof(hops));
        if (rv < 0) {
            return MapSystemError(WSAGetLastError());
        }
    }
    if (multicast_interface_ != 0) {
        ...
        case AF_INET: {
            ...
            int rv = setsockopt(socket_, IPPROTO_IP, IP_MULTICAST_IF, reinterpret_cast<const char*>(&address), sizeof(address));
            if (rv) {
                return MapSystemError(WSAGetLastError());
            }
            break;
        ...
        case AF_INET6: {
            ...
            int rv = setsockopt(socket_, IPPROTO_IPV6, IPV6_MULTICAST_IF, reinterpret_cast<const char*>(&interface_index), sizeof(interface_index));
            if (rv) {
                return MapSystemError(WSAGetLastError());
            }
            break;
...
int UDPSocketWin::JoinGroup(const IPAddress& group_address) const
{
    ...
    case IPAddress::kIPv4AddressSize: {
        ...
        memcpy(&mreq.imr_multiaddr, group_address.bytes().data(), IPAddress::kIPv4AddressSize);
        int rv = setsockopt(socket_, IPPROTO_IP, IP_ADD_MEMBERSHIP, reinterpret_cast<const char*>(&mreq), sizeof(mreq));
        if (rv) {
            return MapSystemError(WSAGetLastError());
        }
        return OK;
    ...
    case IPAddress::kIPv6AddressSize: {
        ...
        memcpy(&mreq.ipv6mr_multiaddr, group_address.bytes().data(), IPAddress::kIPv6AddressSize);
        int rv = setsockopt(socket_, IPPROTO_IPV6, IPV6_ADD_MEMBERSHIP, reinterpret_cast<const char*>(&mreq), sizeof(mreq));
        if (rv) {
            return MapSystemError(WSAGetLastError());
        }
        return OK;
...
int UDPSocketWin::LeaveGroup(const IPAddress& group_address) const
{
    ...
    case IPAddress::kIPv4AddressSize: {
        ...
        mreq.imr_interface.s_addr = htonl(multicast_interface_);
        memcpy(&mreq.imr_multiaddr, group_address.bytes().data(), IPAddress::kIPv4AddressSize);
        int rv = setsockopt(socket_, IPPROTO_IP, IP_DROP_MEMBERSHIP, reinterpret_cast<const char*>(&mreq), sizeof(mreq));
        if (rv) {
            return MapSystemError(WSAGetLastError());
        }
        return OK;
    ...
    case IPAddress::kIPv6AddressSize: {
        ...
        mreq.ipv6mr_interface = multicast_interface_;
        memcpy(&mreq.ipv6mr_multiaddr, group_address.bytes().data(), IPAddress::kIPv6AddressSize);
        int rv = setsockopt(socket_, IPPROTO_IPV6, IP_DROP_MEMBERSHIP, reinterpret_cast<const char*>(&mreq), sizeof(mreq));
        if (rv) {
            return MapSystemError(WSAGetLastError());
        }
        return OK;
...
int DscpManager::PrepareForSend(const IPEndPoint& remote_address)
{
    ...
    if (!api_->AddSocketToFlow(qos_handle_, socket_, storage.addr, traffic_type, QOS_NON_ADAPTIVE_FLOW, &flow_id_)) {
    ...
    if (new_flow) {
        ...
        api_->SetFlow(qos_handle_, flow_id_, QOSSetOutgoingDSCPValue, sizeof(buf), &buf, 0, nullptr);
    }
...
void DscpManager::RequestHandle()
{
    ...
    base::ThreadPool::PostTaskAndReplyWithResult(FROM_HERE, { base::MayBlock() }, base::BindOnce(&DscpManager::DoCreateHandle, api_),
        base::BindOnce(&DscpManager::OnHandleCreated, api_, weak_ptr_factory_.GetWeakPtr()));
}
...
void DscpManager::OnHandleCreated(QwaveApi* api, base::WeakPtr<DscpManager> dscp_manager, HANDLE handle)
{
    if (!handle) {
        api->OnFatalError();
    }
