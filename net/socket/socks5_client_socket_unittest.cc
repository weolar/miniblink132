...
public:
    ...
    std::unique_ptr<SOCKS5ClientSocket> BuildMockSocket(
        base::span<const MockRead> reads, base::span<const MockWrite> writes, const std::string& hostname, int port, NetLog* net_log);
...
void SOCKS5ClientSocketTest::SetUp()
{
    ...
    address_list_ = AddressList::CreateFromIPAddress(IPAddress::IPv4Localhost(), 1080);
}
    ...
    base::span<const MockRead> reads, base::span<const MockWrite> writes, const std::string& hostname, int port, NetLog* net_log)
{
    ...
    auto tcp_sock = std::make_unique<MockTCPClientSocket>(address_list_, net_log, data_.get());
    ...
    return std::make_unique<SOCKS5ClientSocket>(std::move(tcp_sock), HostPortPair(hostname, port), TRAFFIC_ANNOTATION_FOR_TESTS);
}
...
TEST_F(SOCKS5ClientSocketTest, CompleteHandshake)
{
    ...
    const char kOkRequest[] = {
        ...
        // Domain string:
        'l', 'o', 'c', 'a', 'l', 'h', 'o', 's', 't', 0x00, 0x50, // 16-bit port (80)
    };

    MockWrite data_writes[] = { MockWrite(ASYNC, kSOCKS5GreetRequest, kSOCKS5GreetRequestLength), MockWrite(ASYNC, kOkRequest, std::size(kOkRequest)),
        MockWrite(ASYNC, payload_write.data(), payload_write.size()) };
    MockRead data_reads[] = { MockRead(ASYNC, kSOCKS5GreetResponse, kSOCKS5GreetResponseLength), MockRead(ASYNC, kSOCKS5OkResponse, kSOCKS5OkResponseLength),
        MockRead(ASYNC, payload_read.data(), payload_read.size()) };

    user_sock_ = BuildMockSocket(data_reads, data_writes, "localhost", 80, NetLog::Get());
    ...
    EXPECT_TRUE(LogContainsBeginEvent(net_log_entries, 0, NetLogEventType::SOCKS5_CONNECT));
    ...
    EXPECT_TRUE(LogContainsEndEvent(net_log_entries, -1, NetLogEventType::SOCKS5_CONNECT));
...
TEST_F(SOCKS5ClientSocketTest, ConnectAndDisconnectTwice)
{
    ...
    for (int i = 0; i < 2; ++i) {
        MockWrite data_writes[]
            = { MockWrite(SYNCHRONOUS, kSOCKS5GreetRequest, kSOCKS5GreetRequestLength), MockWrite(SYNCHRONOUS, request.data(), request.size()) };
        MockRead data_reads[]
            = { MockRead(SYNCHRONOUS, kSOCKS5GreetResponse, kSOCKS5GreetResponseLength), MockRead(SYNCHRONOUS, kSOCKS5OkResponse, kSOCKS5OkResponseLength) };

        user_sock_ = BuildMockSocket(data_reads, data_writes, hostname, 80, nullptr);
...
TEST_F(SOCKS5ClientSocketTest, LargeHostNameFails)
{
    ...
    user_sock_ = BuildMockSocket(data_reads, data_writes, large_host_name, 80, nullptr);
...
TEST_F(SOCKS5ClientSocketTest, PartialReadWrites)
{
    ...
    const char kOkRequest[] = {
        ...
        // Domain string:
        'w', 'w', 'w', '.', 'g', 'o', 'o', 'g', 'l', 'e', '.', 'c', 'o', 'm', 0x00, 0x50, // 16-bit port (80)
    };
    ...
    // Test for partial greet request write
    {
        ...
        MockWrite data_writes[] = { MockWrite(ASYNC, partial1, std::size(partial1)), MockWrite(ASYNC, partial2, std::size(partial2)),
            MockWrite(ASYNC, kOkRequest, std::size(kOkRequest)) };
        MockRead data_reads[]
            = { MockRead(ASYNC, kSOCKS5GreetResponse, kSOCKS5GreetResponseLength), MockRead(ASYNC, kSOCKS5OkResponse, kSOCKS5OkResponseLength) };
        user_sock_ = BuildMockSocket(data_reads, data_writes, hostname, 80, NetLog::Get());
        int rv = user_sock_->Connect(callback_.callback());
        EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
    ...
    // Test for partial greet response read
    {
        ...
        const char partial2[] = { 0x00 };
        MockWrite data_writes[] = { MockWrite(ASYNC, kSOCKS5GreetRequest, kSOCKS5GreetRequestLength), MockWrite(ASYNC, kOkRequest, std::size(kOkRequest)) };
        MockRead data_reads[] = { MockRead(ASYNC, partial1, std::size(partial1)), MockRead(ASYNC, partial2, std::size(partial2)),
    ...
    // Test for partial handshake request write.
    {
        ...
        MockWrite data_writes[] = { MockWrite(ASYNC, kSOCKS5GreetRequest, kSOCKS5GreetRequestLength), MockWrite(ASYNC, kOkRequest, kSplitPoint),
            MockWrite(ASYNC, kOkRequest + kSplitPoint, std::size(kOkRequest) - kSplitPoint) };
        MockRead data_reads[]
            = { MockRead(ASYNC, kSOCKS5GreetResponse, kSOCKS5GreetResponseLength), MockRead(ASYNC, kSOCKS5OkResponse, kSOCKS5OkResponseLength) };
        user_sock_ = BuildMockSocket(data_reads, data_writes, hostname, 80, NetLog::Get());
        int rv = user_sock_->Connect(callback_.callback());
    ...
    // Test for partial handshake response read
    {
        ...
        MockWrite data_writes[] = { MockWrite(ASYNC, kSOCKS5GreetRequest, kSOCKS5GreetRequestLength), MockWrite(ASYNC, kOkRequest, std::size(kOkRequest)) };
        MockRead data_reads[] = { MockRead(ASYNC, kSOCKS5GreetResponse, kSOCKS5GreetResponseLength), MockRead(ASYNC, kSOCKS5OkResponse, kSplitPoint),
            MockRead(ASYNC, kSOCKS5OkResponse + kSplitPoint, kSOCKS5OkResponseLength - kSplitPoint) };
...
TEST_F(SOCKS5ClientSocketTest, Tag)
{
    ...
    // to it.
    SOCKS5ClientSocket socket(std::move(tagging_sock), HostPortPair("localhost", 80), TRAFFIC_ANNOTATION_FOR_TESTS);
