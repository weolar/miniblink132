...
static_assert(UNSET_UID == SocketTag::UNSET_UID, "TrafficStatsUid does not match SocketTag::UNSET_UID");
// Expose UNSET_TAG to Java.
// GENERATED_JAVA_ENUM_PACKAGE: org.chromium.net
enum TrafficStatsTag {
    UNSET_TAG = -1,
};
static_assert(UNSET_TAG == SocketTag::UNSET_TAG, "TrafficStatsTag does not match SocketTag::UNSET_TAG");
#endif // BUILDFLAG(IS_ANDROID)

bool SocketTag::operator<(const SocketTag& other) const
{
#if BUILDFLAG(IS_ANDROID)
    return std::tie(uid_, traffic_stats_tag_) < std::tie(other.uid_, other.traffic_stats_tag_);
#else
    return false;
#endif // BUILDFLAG(IS_ANDROID)
}
