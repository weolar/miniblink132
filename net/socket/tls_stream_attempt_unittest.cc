...
void ValidateConnectTiming(const LoadTimingInfo::ConnectTiming& connect_timing)
{
...
public:
    ...
    // available.
    explicit TlsStreamAttemptHelper(const StreamAttemptParams* params, std::optional<SSLConfig> ssl_config = SSLConfig())
        : attempt_(std::make_unique<TlsStreamAttempt>(params, IPEndPoint(IPAddress(192, 0, 2, 1), 443), HostPortPair("a.test", 443), this))
    ...
    int Start()
    {
        return attempt_->Start(base::BindOnce(&TlsStreamAttemptHelper::OnComplete, base::Unretained(this)));
    }
...
public:
    TlsStreamAttemptTest()
        ...
        , params_(StreamAttemptParams::FromHttpNetworkSession(http_network_session_.get()))
    {
    }
...
protected:
    ...
    const StreamAttemptParams* params() const
    {
        return &params_;
    }
...
TEST_F(TlsStreamAttemptTest, SuccessSync)
{
    ...
    std::unique_ptr<StreamSocket> stream_socket = helper.attempt()->ReleaseStreamSocket();
...
TEST_F(TlsStreamAttemptTest, ConnectAndConfirmDelayed)
{
    ...
    socket_factory().AddSocketDataProvider(&data);
    SSLSocketDataProvider ssl(ASYNC, OK);
    ssl.connect_callback = base::BindLambdaForTesting([&] { FastForwardBy(kDelay); });
    ssl.confirm = MockConfirm(SYNCHRONOUS, OK);
    ssl.confirm_callback = base::BindLambdaForTesting([&] { FastForwardBy(kDelay); });
    socket_factory().AddSSLSocketDataProvider(&ssl);
...
TEST_F(TlsStreamAttemptTest, ClientAuthCertNeeded)
{
    ...
    std::unique_ptr<StreamSocket> stream_socket = helper.attempt()->ReleaseStreamSocket();
    ASSERT_FALSE(stream_socket);
    scoped_refptr<SSLCertRequestInfo> cert_request_info = helper.attempt()->GetCertRequestInfo();
...
TEST_F(TlsStreamAttemptTest, EchOk)
{
    ...
    ASSERT_TRUE(MakeTestEchKeys("public.example", /*max_name_len=*/128, &ech_config_list));
...
TEST_F(TlsStreamAttemptTest, EchRetryOk)
{
    ...
    std::vector<uint8_t> ech_config_list;
    ASSERT_TRUE(MakeTestEchKeys("public1.example", /*max_name_len=*/128, &ech_config_list));

    std::vector<uint8_t> ech_retry_config_list;
    ASSERT_TRUE(MakeTestEchKeys("public2.example", /*max_name_len=*/128, &ech_config_list));
