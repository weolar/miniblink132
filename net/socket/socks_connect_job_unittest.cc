...
public:
    ...
    static scoped_refptr<SOCKSSocketParams> CreateSOCKSParams(SOCKSVersion socks_version, SecureDnsPolicy secure_dns_policy = SecureDnsPolicy::kAllow)
    {
        return base::MakeRefCounted<SOCKSSocketParams>(ConnectJobParams(base::MakeRefCounted<TransportSocketParams>(HostPortPair(kProxyHostName, kProxyPort),
                                                           NetworkAnonymizationKey(), secure_dns_policy, OnHostResolutionCallback(),
                                                           /*supported_alpns=*/base::flat_set<std::string>())),
            socks_version == SOCKSVersion::V5,
            socks_version == SOCKSVersion::V4 ? HostPortPair(kSOCKS4TestHost, kSOCKS4TestPort) : HostPortPair(kSOCKS5TestHost, kSOCKS5TestPort),
...
TEST_F(SOCKSConnectJobTest, HostResolutionFailure)
{
    ...
    for (bool failure_synchronous : { false, true }) {
        ...
        SOCKSConnectJob socks_connect_job(
            DEFAULT_PRIORITY, SocketTag(), &common_connect_job_params_, CreateSOCKSParams(SOCKSVersion::V5), &test_delegate, nullptr /* net_log */);
        test_delegate.StartJobExpectingResult(&socks_connect_job, ERR_PROXY_CONNECTION_FAILED, failure_synchronous);
        EXPECT_THAT(socks_connect_job.GetResolveErrorInfo().error, test::IsError(ERR_DNS_TIMED_OUT));
    }
}
...
TEST_F(SOCKSConnectJobTest, HostResolutionFailureSOCKS4Endpoint)
{
    ...
    for (bool failure_synchronous : { false, true }) {
        ...
        SequencedSocketData sequenced_socket_data { base::span<MockRead>(), base::span<MockWrite>() };
        ...
        scoped_refptr<SOCKSSocketParams> socket_params
            = base::MakeRefCounted<SOCKSSocketParams>(ConnectJobParams(base::MakeRefCounted<TransportSocketParams>(HostPortPair(kProxyHostName, kProxyPort),
                                                          NetworkAnonymizationKey(), SecureDnsPolicy::kAllow, OnHostResolutionCallback(),
                                                          /*supported_alpns=*/base::flat_set<std::string>())),
                false /* socks_v5 */, HostPortPair(hostname, kSOCKS4TestPort), NetworkAnonymizationKey(), TRAFFIC_ANNOTATION_FOR_TESTS);

        TestConnectJobDelegate test_delegate;
        SOCKSConnectJob socks_connect_job(DEFAULT_PRIORITY, SocketTag(), &common_connect_job_params_, socket_params, &test_delegate, nullptr /* net_log */);
        test_delegate.StartJobExpectingResult(&socks_connect_job, ERR_NAME_NOT_RESOLVED, failure_synchronous);
        EXPECT_THAT(socks_connect_job.GetResolveErrorInfo().error, test::IsError(ERR_DNS_TIMED_OUT));
    }
}
...
TEST_F(SOCKSConnectJobTest, HandshakeError)
{
    for (bool host_resolution_synchronous : { false, true }) {
        for (bool write_failure_synchronous : { false, true }) {
            ...
            MockWrite writes[] = {
                MockWrite(write_failure_synchronous ? SYNCHRONOUS : ASYNC, ERR_UNEXPECTED, 0),
            };
            ...
            SOCKSConnectJob socks_connect_job(
                DEFAULT_PRIORITY, SocketTag(), &common_connect_job_params_, CreateSOCKSParams(SOCKSVersion::V5), &test_delegate, nullptr /* net_log */);
            test_delegate.StartJobExpectingResult(&socks_connect_job, ERR_UNEXPECTED, host_resolution_synchronous && write_failure_synchronous);
        }
    }
}
...
TEST_F(SOCKSConnectJobTest, SOCKS4)
{
    for (bool host_resolution_synchronous : { false, true }) {
        for (bool read_and_writes_synchronous : { true }) {
            ...
            MockWrite writes[] = {
                MockWrite(SYNCHRONOUS, kSOCKS4OkRequestLocalHostPort80, kSOCKS4OkRequestLocalHostPort80Length, 0),
            };
            ...
            TestConnectJobDelegate test_delegate;
            SOCKSConnectJob socks_connect_job(
                DEFAULT_PRIORITY, SocketTag(), &common_connect_job_params_, CreateSOCKSParams(SOCKSVersion::V4), &test_delegate, nullptr /* net_log */);
            test_delegate.StartJobExpectingResult(&socks_connect_job, OK, host_resolution_synchronous && read_and_writes_synchronous);
...
TEST_F(SOCKSConnectJobTest, SOCKS5)
{
    for (bool host_resolution_synchronous : { false, true }) {
        for (bool read_and_writes_synchronous : { true }) {
            ...
            MockRead reads[] = {
                MockRead(SYNCHRONOUS, kSOCKS5GreetResponse, kSOCKS5GreetResponseLength, 1),
...
TEST_F(SOCKSConnectJobTest, HasEstablishedConnection)
{
    ...
    MockWrite writes[] = {
        MockWrite(ASYNC, kSOCKS4OkRequestLocalHostPort80, kSOCKS4OkRequestLocalHostPort80Length, 0),
    };
...
TEST_F(SOCKSConnectJobTest, TimeoutDuringDnsResolution)
{
    ...
    EXPECT_TRUE(test_delegate.has_result());
    EXPECT_THAT(test_delegate.WaitForResult(), test::IsError(ERR_PROXY_CONNECTION_FAILED));
}
...
TEST_F(SOCKSConnectJobTest, Priority)
{
    ...
    for (int initial_priority = MINIMUM_PRIORITY; initial_priority <= MAXIMUM_PRIORITY; ++initial_priority) {
        for (int new_priority = MINIMUM_PRIORITY; new_priority <= MAXIMUM_PRIORITY; ++new_priority) {
            ...
            TestConnectJobDelegate test_delegate;
            SOCKSConnectJob socks_connect_job(static_cast<RequestPriority>(initial_priority), SocketTag(), &common_connect_job_params_,
                CreateSOCKSParams(SOCKSVersion::V4), &test_delegate, nullptr /* net_log */);
            ...
            // Change priority.
            socks_connect_job.ChangePriority(static_cast<RequestPriority>(new_priority));
            EXPECT_EQ(new_priority, host_resolver_.request_priority(request_id));

            // Restore initial priority.
            socks_connect_job.ChangePriority(static_cast<RequestPriority>(initial_priority));
            EXPECT_EQ(initial_priority, host_resolver_.request_priority(request_id));
        }
    }
}
...
TEST_F(SOCKSConnectJobTest, SecureDnsPolicy)
{
    for (auto secure_dns_policy : { SecureDnsPolicy::kAllow, SecureDnsPolicy::kDisable }) {
        TestConnectJobDelegate test_delegate;
        SOCKSConnectJob socks_connect_job(DEFAULT_PRIORITY, SocketTag(), &common_connect_job_params_, CreateSOCKSParams(SOCKSVersion::V4, secure_dns_policy),
...
TEST_F(SOCKSConnectJobTest, ConnectTiming)
{
    ...
    MockRead reads[] = {
        MockRead(SYNCHRONOUS, kSOCKS5GreetResponse, kSOCKS5GreetResponseLength, 2),
    ...
    EXPECT_EQ(base::TimeTicks(), socks_connect_job.connect_timing().domain_lookup_start);
    EXPECT_EQ(base::TimeTicks(), socks_connect_job.connect_timing().domain_lookup_end);
    ...
    EXPECT_EQ(start + 2 * kTinyTime, socks_connect_job.connect_timing().connect_end);
...
TEST_F(SOCKSConnectJobTest, CancelDuringDnsResolution)
{
    ...
    TestConnectJobDelegate test_delegate;
    std::unique_ptr<SOCKSConnectJob> socks_connect_job = std::make_unique<SOCKSConnectJob>(
        DEFAULT_PRIORITY, SocketTag(), &common_connect_job_params_, CreateSOCKSParams(SOCKSVersion::V5), &test_delegate, nullptr /* net_log */);
    socks_connect_job->Connect();
