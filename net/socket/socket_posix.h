...
class NET_EXPORT_PRIVATE SocketPosix : public base::MessagePumpForIO::FdWatcher {
public:
    SocketPosix();
    ...
    // given peer address.
    int AdoptConnectedSocket(SocketDescriptor socket, const SockaddrStorage& peer_address);
    ...
    int Accept(std::unique_ptr<SocketPosix>* socket, CompletionOnceCallback callback);
