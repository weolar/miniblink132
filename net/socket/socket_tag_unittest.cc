...
TEST(SocketTagTest, Apply)
{
    ...
    ASSERT_EQ(send(s, kRequest1, strlen(kRequest1), 0), static_cast<int>(strlen(kRequest1)));
    ...
    ASSERT_EQ(send(s, kRequest2, strlen(kRequest2), 0), static_cast<int>(strlen(kRequest2)));
    EXPECT_GT(GetTaggedBytes(tag_val1), old_traffic);
