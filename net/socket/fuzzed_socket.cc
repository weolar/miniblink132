...
const Error kConnectErrors[]
    = { ERR_CONNECTION_RESET, ERR_CONNECTION_CLOSED, ERR_FAILED, ERR_CONNECTION_TIMED_OUT, ERR_ACCESS_DENIED, ERR_CONNECTION_REFUSED, ERR_ADDRESS_UNREACHABLE };
...
const Error kReadWriteErrors[] = { ERR_CONNECTION_CLOSED, ERR_FAILED, ERR_TIMED_OUT, ERR_CONNECTION_RESET };

} // namespace

FuzzedSocket::FuzzedSocket(FuzzedDataProvider* data_provider, net::NetLog* net_log)
...
int FuzzedSocket::Read(IOBuffer* buf, int buf_len, CompletionOnceCallback callback)
{
    ...
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
        FROM_HERE, base::BindOnce(&FuzzedSocket::OnReadComplete, weak_factory_.GetWeakPtr(), std::move(callback), result));
    return ERR_IO_PENDING;
}

int FuzzedSocket::Write(IOBuffer* buf, int buf_len, CompletionOnceCallback callback, const NetworkTrafficAnnotationTag& /* traffic_annotation */)
{
    DCHECK(!connect_pending_);
    ...
    write_pending_ = true;
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
        FROM_HERE, base::BindOnce(&FuzzedSocket::OnWriteComplete, weak_factory_.GetWeakPtr(), std::move(callback), result));
    return ERR_IO_PENDING;
}
...
int FuzzedSocket::Connect(CompletionOnceCallback callback)
{
    ...
    if (result != OK)
        error_pending_ = true;
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
        FROM_HERE, base::BindOnce(&FuzzedSocket::OnConnectComplete, weak_factory_.GetWeakPtr(), std::move(callback), result));
    return ERR_IO_PENDING;
}
...
void FuzzedSocket::OnWriteComplete(CompletionOnceCallback callback, int result)
{
...
void FuzzedSocket::OnConnectComplete(CompletionOnceCallback callback, int result)
{
