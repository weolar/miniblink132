...
class NET_EXPORT_PRIVATE ClientSocketPoolManagerImpl : public ClientSocketPoolManager {
public:
    ...
    ClientSocketPoolManagerImpl(const CommonConnectJobParams& common_connect_job_params, const CommonConnectJobParams& websocket_common_connect_job_params,
        HttpNetworkSession::SocketPoolType pool_type, bool cleanup_on_ip_address_change = true);

    ClientSocketPoolManagerImpl(const ClientSocketPoolManagerImpl&) = delete;
    ClientSocketPoolManagerImpl& operator=(const ClientSocketPoolManagerImpl&) = delete;

    ~ClientSocketPoolManagerImpl() override;

    void FlushSocketPoolsWithError(int net_error, const char* net_log_reason_utf8) override;
