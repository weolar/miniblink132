...
ClientSocketPool::GroupId TestGroupId(std::string_view host, int port = 80, std::string_view scheme = url::kHttpScheme,
    PrivacyMode privacy_mode = PrivacyMode::PRIVACY_MODE_DISABLED, NetworkAnonymizationKey network_anonymization_key = NetworkAnonymizationKey())
{
    return ClientSocketPool::GroupId(url::SchemeHostPort(scheme, host, port), privacy_mode, network_anonymization_key, SecureDnsPolicy::kAllow,
...
void TestLoadTimingInfoConnectedNotReused(const ClientSocketHandle& handle)
{
    ...
    ExpectConnectTimingHasTimes(load_timing_info.connect_timing, CONNECT_TIMING_HAS_CONNECT_TIMES_ONLY);
    ExpectLoadTimingHasOnlyConnectionTimes(load_timing_info);
...
public:
    ...
    int Read(IOBuffer* /* buf */, int len, CompletionOnceCallback /* callback */) override
    {
    ...
    int Write(IOBuffer* /* buf */, int len, CompletionOnceCallback /* callback */, const NetworkTrafficAnnotationTag& /*traffic_annotation*/) override
    {
        was_used_to_convey_data_ = true;
        return len;
    }
...
public:
    ...
    std::unique_ptr<TransportClientSocket> CreateTransportClientSocket(const AddressList& addresses,
        std::unique_ptr<SocketPerformanceWatcher> /* socket_performance_watcher */, NetworkQualityEstimator* /* network_quality_estimator */,
        NetLog* /* net_log */, const NetLogSource& /*source*/) override
    {
        allocation_count_++;
        return nullptr;
    }
    ...
    std::unique_ptr<SSLClientSocket> CreateSSLClientSocket(
        SSLClientContext* context, std::unique_ptr<StreamSocket> stream_socket, const HostPortPair& host_and_port, const SSLConfig& ssl_config) override
    {
        NOTIMPLEMENTED();
        return nullptr;
    }

    void WaitForSignal(TestConnectJob* job)
    {
        waiting_jobs_.push_back(job);
    }
...
public:
    ...
    TestConnectJob(JobType job_type, RequestPriority request_priority, SocketTag socket_tag, base::TimeDelta timeout_duration,
        const CommonConnectJobParams* common_connect_job_params, ConnectJob::Delegate* delegate, MockClientSocketFactory* client_socket_factory)
        : ConnectJob(request_priority, socket_tag, timeout_duration, common_connect_job_params, delegate, nullptr /* net_log */,
            NetLogSourceType::TRANSPORT_CONNECT_JOB, NetLogEventType::TRANSPORT_CONNECT_JOB_CONNECT)
        , job_type_(job_type)
    ...
    void set_load_state(LoadState load_state)
    {
    ...
    {
        return store_additional_error_state_;
    }
...
private:
    ...
    {
        AddressList ignored;
        client_socket_factory_->CreateTransportClientSocket(ignored, nullptr, nullptr, nullptr, NetLogSource());
        switch (job_type_) {
        case kMockJob:
            return DoConnect(true /* successful */, false /* sync */, false /* cert_error */);
        case kMockFailingJob:
            return DoConnect(false /* error */, false /* sync */, false /* cert_error */);
        case kMockPendingJob:
            ...
            base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE,
                base::BindOnce(base::IgnoreResult(&TestConnectJob::DoConnect), weak_factory_.GetWeakPtr(), true /* successful */, true /* async */,
                    false /* cert_error */),
        ...
        case kMockPendingFailingJob:
            ...
            base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE,
                base::BindOnce(
                    base::IgnoreResult(&TestConnectJob::DoConnect), weak_factory_.GetWeakPtr(), false /* error */, true /* async */, false /* cert_error */),
                base::Milliseconds(2));
            return ERR_IO_PENDING;
        ...
        case kMockCertErrorJob:
            return DoConnect(false /* error */, false /* sync */, true /* cert_error */);
        case kMockPendingCertErrorJob:
            set_load_state(LOAD_STATE_CONNECTING);
            base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE,
                base::BindOnce(
                    base::IgnoreResult(&TestConnectJob::DoConnect), weak_factory_.GetWeakPtr(), false /* error */, true /* async */, true /* cert_error */),
                base::Milliseconds(2));
            return ERR_IO_PENDING;
        ...
        case kMockUnreadDataJob: {
            int ret = DoConnect(true /* successful */, false /* sync */, false /* cert_error */);
    ...
    int DoConnect(bool succeed, bool was_async, bool cert_error)
    {
        ...
        has_established_connection_ = true;
        if (succeed) {
            SetSocket(std::make_unique<MockClientSocket>(net_log().net_log()), std::nullopt);
    ...
    void DoAdvanceAuthChallenge(int remaining_challenges, bool succeed_after_last_challenge)
    {
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE,
            base::BindOnce(&TestConnectJob::InvokeNextProxyAuthCallback, weak_factory_.GetWeakPtr(), remaining_challenges, succeed_after_last_challenge));
    }

    void InvokeNextProxyAuthCallback(int remaining_challenges, bool succeed_after_last_challenge)
    {
        set_load_state(LOAD_STATE_ESTABLISHING_PROXY_TUNNEL);
        if (remaining_challenges == 0) {
            DoConnect(succeed_after_last_challenge, true /* was_async */, false /* cert_error */);
            return;
        }
        ...
        NotifyDelegateOfProxyAuth(info, nullptr /* http_auth_controller */,
            base::BindOnce(&TestConnectJob::DoAdvanceAuthChallenge, weak_factory_.GetWeakPtr(), remaining_challenges - 1, succeed_after_last_challenge));
    }
...
public:
    ...
    void set_job_type(TestConnectJob::JobType job_type)
    {
        job_type_ = job_type;
    }
    ...
    std::unique_ptr<ConnectJob> CreateConnectJob(Endpoint endpoint, const ProxyChain& proxy_chain,
        const std::optional<NetworkTrafficAnnotationTag>& proxy_annotation_tag, const std::vector<SSLConfig::CertAndStatus>& allowed_bad_certs,
        ConnectJobFactory::AlpnMode alpn_mode, bool force_tunnel, PrivacyMode privacy_mode, const OnHostResolutionCallback& resolution_callback,
        RequestPriority request_priority, SocketTag socket_tag, const NetworkAnonymizationKey& network_anonymization_key, SecureDnsPolicy secure_dns_policy,
        bool disable_cert_network_fetches, const CommonConnectJobParams* common_connect_job_params, ConnectJob::Delegate* delegate) const override
    {
        ...
        return std::make_unique<TestConnectJob>(
            job_type, request_priority, socket_tag, timeout_duration_, common_connect_job_params, delegate, client_socket_factory_);
    }
...
void MockClientSocketFactory::SetJobLoadState(size_t job, LoadState load_state)
{
    ASSERT_LT(job, waiting_jobs_.size());
...
protected:
    ...
    {
        connect_backup_jobs_enabled_ = TransportClientSocketPool::connect_backup_jobs_enabled();
        TransportClientSocketPool::set_connect_backup_jobs_enabled(true);
    }

    ~ClientSocketPoolBaseTest() override
    {
        TransportClientSocketPool::set_connect_backup_jobs_enabled(connect_backup_jobs_enabled_);
    }

    void CreatePool(int max_sockets, int max_sockets_per_group, bool enable_backup_connect_jobs = false)
    {
        CreatePoolWithIdleTimeouts(
            max_sockets, max_sockets_per_group, kUnusedIdleSocketTimeout, ClientSocketPool::used_idle_socket_timeout(), enable_backup_connect_jobs);
    }

    void CreatePoolWithIdleTimeouts(int max_sockets, int max_sockets_per_group, base::TimeDelta unused_idle_socket_timeout,
        base::TimeDelta used_idle_socket_timeout, bool enable_backup_connect_jobs = false, ProxyChain proxy_chain = ProxyChain::Direct())
    {
        DCHECK(!pool_.get());
        std::unique_ptr<TestConnectJobFactory> connect_job_factory = std::make_unique<TestConnectJobFactory>(&client_socket_factory_);
        connect_job_factory_ = connect_job_factory.get();
        pool_ = TransportClientSocketPool::CreateForTesting(max_sockets, max_sockets_per_group, unused_idle_socket_timeout, used_idle_socket_timeout,
            proxy_chain, /*is_for_websockets=*/false, &common_connect_job_params_, std::move(connect_job_factory), nullptr /* ssl_config_service */,
            enable_backup_connect_jobs);
    }

    int StartRequestWithIgnoreLimits(const ClientSocketPool::GroupId& group_id, RequestPriority priority, ClientSocketPool::RespectLimits respect_limits)
    {
        return test_base_.StartRequestUsingPool(pool_.get(), group_id, priority, respect_limits, params_);
    }

    int StartRequest(const ClientSocketPool::GroupId& group_id, RequestPriority priority)
    {
        return StartRequestWithIgnoreLimits(group_id, priority, ClientSocketPool::RespectLimits::ENABLED);
    }
    ...
    void ExpectSocketClosedWithReason(NetLogSource expected_source, const char* expected_reason)
    {
        auto entries = net_log_observer_.GetEntriesForSourceWithType(expected_source, NetLogEventType::SOCKET_POOL_CLOSING_SOCKET, NetLogEventPhase::NONE);
    ...
    TestSocketRequest* request(int i)
    {
        return test_base_.request(i);
    }
    size_t requests_size() const
    {
        return test_base_.requests_size();
    }
    ...
    size_t completion_count() const
    {
        return test_base_.completion_count();
    }

    const StaticHttpUserAgentSettings http_user_agent_settings_ = { "*", "test-ua" };
    const CommonConnectJobParams common_connect_job_params_ { /*client_socket_factory=*/nullptr,
        ...
        /*proxy_delegate=*/nullptr, &http_user_agent_settings_,
        /*ssl_client_context=*/nullptr,
        /*socket_performance_watcher_factory=*/nullptr,
        /*network_quality_estimator=*/nullptr, NetLog::Get(),
...
TEST_F(ClientSocketPoolBaseTest, BasicSynchronous)
{
    ...
    NetLogWithSource net_log_with_source = NetLogWithSource::Make(NetLogSourceType::NONE);

    TestLoadTimingInfoNotConnected(handle);

    EXPECT_EQ(OK,
        handle.Init(TestGroupId("a"), params_, std::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback.callback(),
    ...
    auto entries = net_log_observer_.GetEntriesForSource(net_log_with_source.source());

    EXPECT_EQ(5u, entries.size());
    EXPECT_TRUE(LogContainsEvent(entries, 0, NetLogEventType::TCP_CLIENT_SOCKET_POOL_REQUESTED_SOCKET, NetLogEventPhase::NONE));
    EXPECT_TRUE(LogContainsBeginEvent(entries, 1, NetLogEventType::SOCKET_POOL));
    EXPECT_TRUE(LogContainsEvent(entries, 2, NetLogEventType::SOCKET_POOL_BOUND_TO_CONNECT_JOB, NetLogEventPhase::NONE));
    EXPECT_TRUE(LogContainsEvent(entries, 3, NetLogEventType::SOCKET_POOL_BOUND_TO_SOCKET, NetLogEventPhase::NONE));
...
TEST_F(ClientSocketPoolBaseTest, ReleaseAndCloseConnection)
{
    ...
    ExpectSocketClosedWithReason(source, TransportClientSocketPool::kClosedConnectionReturnedToPool);
}
...
TEST_F(ClientSocketPoolBaseTest, SocketWithUnreadDataReturnedToPool)
{
    ...
    ExpectSocketClosedWithReason(source, TransportClientSocketPool::kDataReceivedUnexpectedly);
}
...
TEST_F(ClientSocketPoolBaseTest, GroupSeparation)
{
    ...
    const PrivacyMode kPrivacyModes[] = { PrivacyMode::PRIVACY_MODE_DISABLED, PrivacyMode::PRIVACY_MODE_ENABLED };
    ...
    const SecureDnsPolicy kSecureDnsPolicys[] = { SecureDnsPolicy::kAllow, SecureDnsPolicy::kDisable };
    ...
    for (const auto& host_port_pair : kHostPortPairs) {
        ...
        for (const char* scheme : kSchemes) {
            ...
            for (const auto& privacy_mode : kPrivacyModes) {
                ...
                for (const auto& network_anonymization_key : kNetworkAnonymizationKeys) {
                    ...
                    for (const auto& secure_dns_policy : kSecureDnsPolicys) {
                        ...
                        ClientSocketPool::GroupId group_id(url::SchemeHostPort(scheme, host_port_pair.host(), host_port_pair.port()), privacy_mode,
                        ...
                        // synchronously.
                        EXPECT_THAT(handle.Init(group_id, params_, std::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED,
                                        callback.callback(), ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()),
...
TEST_F(ClientSocketPoolBaseTest, TotalLimit)
{
    ...
    EXPECT_EQ(static_cast<int>(requests_size()), client_socket_factory_.allocation_count());
...
TEST_F(ClientSocketPoolBaseTest, CorrectlyCountStalledGroups)
{
    ...
    EXPECT_THAT(StartRequest(TestGroupId("b"), DEFAULT_PRIORITY), IsError(ERR_IO_PENDING));
    EXPECT_THAT(StartRequest(TestGroupId("c"), DEFAULT_PRIORITY), IsError(ERR_IO_PENDING));
...
TEST_F(ClientSocketPoolBaseTest, StallAndThenCancelAndTriggerAvailableSocket)
{
    ...
    EXPECT_EQ(ERR_IO_PENDING,
        stalled_handle.Init(TestGroupId("a"), params_, std::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED,
            callback.callback(), ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));

    ClientSocketHandle handles[4];
    for (auto& handle : handles) {
        EXPECT_EQ(ERR_IO_PENDING,
            handle.Init(TestGroupId("b"), params_, std::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback.callback(),
...
TEST_F(ClientSocketPoolBaseTest, CancelStalledSocketAtSocketLimit)
{
...

    {
        ...
        for (int i = 0; i < kDefaultMaxSockets; ++i) {
            EXPECT_EQ(OK,
                handles[i].Init(TestGroupId("a" + base::NumberToString(i)), params_, std::nullopt, DEFAULT_PRIORITY, SocketTag(),
                    ClientSocketPool::RespectLimits::ENABLED, callbacks[i].callback(), ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
        }
        ...
        TestCompletionCallback callback;
        EXPECT_EQ(ERR_IO_PENDING,
            stalled_handle.Init(TestGroupId("foo"), params_, std::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED,
                callback.callback(), ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
...
TEST_F(ClientSocketPoolBaseTest, CancelPendingSocketAtSocketLimit)
{
...

    {
        ...
        for (int i = 0; i < kDefaultMaxSockets; ++i) {
            ...
            EXPECT_EQ(ERR_IO_PENDING,
                handles[i].Init(TestGroupId("a" + base::NumberToString(i)), params_, std::nullopt, DEFAULT_PRIORITY, SocketTag(),
                    ClientSocketPool::RespectLimits::ENABLED, callback.callback(), ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
        }
        ...
        EXPECT_EQ(0u, pool_->NumNeverAssignedConnectJobsInGroupForTesting(TestGroupId("foo")));
        EXPECT_EQ(0u, pool_->NumUnassignedConnectJobsInGroupForTesting(TestGroupId("foo")));
...
TEST_F(ClientSocketPoolBaseTest, WaitForStalledSocketAtSocketLimit)
{
    ...
    TestCompletionCallback callback;
    {
        ...
        for (int i = 0; i < kDefaultMaxSockets; ++i) {
            EXPECT_EQ(OK,
                handles[i].Init(TestGroupId(base::StringPrintf("take-2-%d", i)), params_, std::nullopt, DEFAULT_PRIORITY, SocketTag(),
                    ClientSocketPool::RespectLimits::ENABLED, callback.callback(), ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
        }
...
{
    CreatePool(kDefaultMaxSockets, kDefaultMaxSocketsPerGroup, true /* enable_backup_connect_jobs */);
    connect_job_factory_->set_job_type(TestConnectJob::kMockJob);
    ...
    for (int i = 0; i < kDefaultMaxSockets; ++i) {
        ...
        TestCompletionCallback callback;
        EXPECT_EQ(OK,
            handle.Init(TestGroupId("a" + base::NumberToString(i)), params_, std::nullopt, DEFAULT_PRIORITY, SocketTag(),
                ClientSocketPool::RespectLimits::ENABLED, callback.callback(), ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
    }
    ...
    EXPECT_EQ(OK,
        handle.Init(TestGroupId("a0"), params_, std::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback.callback(),
            ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
...
TEST_F(ClientSocketPoolBaseTest, PendingRequests)
{
    ...
    EXPECT_EQ(kDefaultMaxSocketsPerGroup, client_socket_factory_.allocation_count());
...
TEST_F(ClientSocketPoolBaseTest, CancelRequestAndCloseSocketWhenMoreRequestsThanConnectJobs)
{
    CreatePool(kDefaultMaxSockets, kDefaultMaxSocketsPerGroup);
    ...
    for (bool cancel_when_callback_pending : { false, true }) {
        ...
        for (int i = 0; i < kDefaultMaxSocketsPerGroup + 1; ++i) {
            std::unique_ptr<ClientSocketHandle> handle = std::make_unique<ClientSocketHandle>();
            EXPECT_EQ(ERR_IO_PENDING,
                handle->Init(TestGroupId("a"), params_, std::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED,
                    callback.callback(), ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
            handles.push_back(std::move(handle));
            ASSERT_TRUE(pool_->HasGroupForTesting(TestGroupId("a")));
            EXPECT_EQ(static_cast<size_t>(std::min(i + 1, kDefaultMaxSocketsPerGroup)), pool_->NumConnectJobsInGroupForTesting(TestGroupId("a")));
        }

        if (cancel_when_callback_pending) {
            client_socket_factory_.SignalJobs();
            ASSERT_TRUE(pool_->HasGroupForTesting(TestGroupId("a")));
            EXPECT_EQ(kDefaultMaxSocketsPerGroup, pool_->NumActiveSocketsInGroupForTesting(TestGroupId("a")));
        }
        ...
        if (cancel_when_callback_pending) {
            ...
            EXPECT_EQ(static_cast<size_t>(kDefaultMaxSocketsPerGroup), pool_->NumConnectJobsInGroupForTesting(TestGroupId("a")));
        }
        ...
        for (int i = kDefaultMaxSocketsPerGroup - 1; i >= 0; --i) {
            ...
            if (i > 0) {
                ASSERT_TRUE(pool_->HasGroupForTesting(TestGroupId("a")));
                if (cancel_when_callback_pending) {
                    EXPECT_EQ(i, pool_->NumActiveSocketsInGroupForTesting(TestGroupId("a")));
                } else {
                    EXPECT_EQ(static_cast<size_t>(i), pool_->NumConnectJobsInGroupForTesting(TestGroupId("a")));
                }
            } else {
                EXPECT_FALSE(pool_->HasGroupForTesting(TestGroupId("a")));
            }
...
TEST_F(ClientSocketPoolBaseTest, ConnectCancelConnect)
{
    ...
    EXPECT_EQ(ERR_IO_PENDING,
        handle.Init(TestGroupId("a"), params_, std::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback2.callback(),
            ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
    ...
    EXPECT_EQ(1u, pool_->NumConnectJobsInGroupForTesting(TestGroupId("a")) + pool_->IdleSocketCountInGroup(TestGroupId("a")));

    handle.Reset();
    ASSERT_TRUE(pool_->HasGroupForTesting(TestGroupId("a")));
    EXPECT_EQ(2u, pool_->NumConnectJobsInGroupForTesting(TestGroupId("a")) + pool_->IdleSocketCountInGroup(TestGroupId("a")));
...
TEST_F(ClientSocketPoolBaseTest, CancelRequest)
{
    ...
    EXPECT_EQ(kDefaultMaxSocketsPerGroup, client_socket_factory_.allocation_count());
    EXPECT_EQ(requests_size() - kDefaultMaxSocketsPerGroup - 1, completion_count());
...
void RequestSocketOnComplete(ClientSocketHandle* handle, TransportClientSocketPool* pool, TestConnectJobFactory* test_connect_job_factory,
    TestConnectJob::JobType next_job_type, TestCompletionCallback* nested_callback, int first_request_result)
{
    ...
    TestCompletionCallback callback;
    int rv = handle->Init(TestGroupId("a"), ClientSocketPool::SocketParams::CreateForHttpForTesting(), std::nullopt, LOWEST, SocketTag(),
        ClientSocketPool::RespectLimits::ENABLED, nested_callback->callback(), ClientSocketPool::ProxyAuthCallback(), pool, NetLogWithSource());
...
TEST_F(ClientSocketPoolBaseTest, RequestPendingJobTwice)
{
    ...
    int rv = handle.Init(TestGroupId("a"), params_, std::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED,
        base::BindOnce(&RequestSocketOnComplete, &handle, pool_.get(), connect_job_factory_, TestConnectJob::kMockPendingJob, &second_result_callback),
        ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource());
...
TEST_F(ClientSocketPoolBaseTest, CancelActiveRequestThenRequestSocket)
{
    ...
    ClientSocketHandle handle;
    TestCompletionCallback callback;
    int rv = handle.Init(TestGroupId("a"), params_, std::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback.callback(),
        ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource());
    ...
    rv = handle.Init(TestGroupId("a"), params_, std::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback.callback(),
        ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource());
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
    EXPECT_THAT(callback.WaitForResult(), IsOk());
...
TEST_F(ClientSocketPoolBaseTest, CloseIdleSocketsForced)
{
    ...
    ClientSocketHandle handle;
    TestCompletionCallback callback;
    int rv = handle.Init(TestGroupId("a"), params_, std::nullopt, LOWEST, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback.callback(),
        ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource::Make(NetLogSourceType::NONE));
    EXPECT_THAT(rv, IsOk());
...
TEST_F(ClientSocketPoolBaseTest, CloseIdleSocketsInGroupForced)
{
    ...
    int rv = handle1.Init(TestGroupId("a"), params_, std::nullopt, LOWEST, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback.callback(),
        ClientSocketPool::ProxyAuthCallback(), pool_.get(), net_log_with_source);
    EXPECT_THAT(rv, IsOk());
    ClientSocketHandle handle2;
    rv = handle2.Init(TestGroupId("a"), params_, std::nullopt, LOWEST, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback.callback(),
        ClientSocketPool::ProxyAuthCallback(), pool_.get(), net_log_with_source);
    ClientSocketHandle handle3;
    rv = handle3.Init(TestGroupId("b"), params_, std::nullopt, LOWEST, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback.callback(),
        ClientSocketPool::ProxyAuthCallback(), pool_.get(), net_log_with_source);
    EXPECT_THAT(rv, IsOk());
    handle1.Reset();
    handle2.Reset();
...
TEST_F(ClientSocketPoolBaseTest, CleanUpUnusableIdleSockets)
{
    ...
    ExpectSocketClosedWithReason(source, TransportClientSocketPool::kRemoteSideClosedConnection);
}
...
TEST_F(ClientSocketPoolBaseTest, BasicAsynchronous)
{
    ...
    int rv = handle.Init(TestGroupId("a"), params_, std::nullopt, LOWEST, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback.callback(),
        ClientSocketPool::ProxyAuthCallback(), pool_.get(), net_log_with_source);
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
    EXPECT_EQ(LOAD_STATE_CONNECTING, pool_->GetLoadState(TestGroupId("a"), &handle));
    TestLoadTimingInfoNotConnected(handle);
...
TEST_F(ClientSocketPoolBaseTest, TwoRequestsCancelOne)
{
    ...
    EXPECT_EQ(ERR_IO_PENDING,
        handle2.Init(TestGroupId("a"), params_, std::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback2.callback(),
            ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
...
TEST_F(ClientSocketPoolBaseTest, CancelRequestLimitsJobs)
{
    ...
    EXPECT_EQ(kDefaultMaxSocketsPerGroup, static_cast<int>(pool_->NumConnectJobsInGroupForTesting(TestGroupId("a"))));
...
TEST_F(ClientSocketPoolBaseTest, ReleaseSockets)
{
    ...
    int rv = req1.handle()->Init(TestGroupId("a"), params_, std::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED,
        req1.callback(), ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource());
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
    ...
    rv = req2.handle()->Init(TestGroupId("a"), params_, std::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, req2.callback(),
        ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource());
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
    TestSocketRequest req3(&request_order, &completion_count);
    rv = req3.handle()->Init(TestGroupId("a"), params_, std::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, req3.callback(),
        ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource());
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
...
TEST_F(ClientSocketPoolBaseTest, LoadStateTwoRequests)
{
    ...
    ClientSocketHandle handle2;
    TestCompletionCallback callback2;
    rv = handle2.Init(TestGroupId("a"), params_, std::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback2.callback(),
        ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource());
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
...
TEST_F(ClientSocketPoolBaseTest, LoadStateGroupLimit)
{
    ...
    ClientSocketHandle handle;
    TestCompletionCallback callback;
    int rv = handle.Init(TestGroupId("a"), params_, std::nullopt, MEDIUM, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback.callback(),
        ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource());
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
    EXPECT_EQ(LOAD_STATE_CONNECTING, handle.GetLoadState());
    ...
    ClientSocketHandle handle2;
    TestCompletionCallback callback2;
    rv = handle2.Init(TestGroupId("a"), params_, std::nullopt, HIGHEST, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback2.callback(),
        ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource());
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
...
TEST_F(ClientSocketPoolBaseTest, LoadStatePoolLimit)
{
    ...
    ClientSocketHandle handle2;
    TestCompletionCallback callback2;
    rv = handle2.Init(TestGroupId("b"), params_, std::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback2.callback(),
        ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource());
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
    ...
    rv = handle3.Init(TestGroupId("a"), params_, std::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback2.callback(),
        ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource());
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
...
{
    CreatePool(kDefaultMaxSockets, kDefaultMaxSocketsPerGroup);
    connect_job_factory_->set_job_type(TestConnectJob::kMockAdditionalErrorStateJob);
...
TEST_F(ClientSocketPoolBaseTest, AdditionalErrorStateAsynchronous)
{
    ...
    connect_job_factory_->set_job_type(TestConnectJob::kMockPendingAdditionalErrorStateJob);
    ClientSocketHandle handle;
    TestCompletionCallback callback;
    EXPECT_EQ(ERR_IO_PENDING,
        handle.Init(TestGroupId("a"), params_, std::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback.callback(),
...
TEST_F(ClientSocketPoolBaseTest, CleanupTimedOutIdleSocketsReuse)
{
    ...
    EXPECT_EQ(1, handle.socket()->Write(nullptr, 1, CompletionOnceCallback(), TRAFFIC_ANNOTATION_FOR_TESTS));
    TestLoadTimingInfoConnectedNotReused(handle);
    handle.Reset();
    ...
    // synchronously.
    NetLogWithSource net_log_with_source = NetLogWithSource::Make(NetLogSourceType::NONE);
    rv = handle.Init(TestGroupId("a"), params_, std::nullopt, LOWEST, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, CompletionOnceCallback(),
        ClientSocketPool::ProxyAuthCallback(), pool_.get(), net_log_with_source);
    ASSERT_THAT(rv, IsOk());
    ...
    EXPECT_TRUE(LogContainsEvent(entries, 0, NetLogEventType::TCP_CLIENT_SOCKET_POOL_REQUESTED_SOCKET, NetLogEventPhase::NONE));
    EXPECT_TRUE(LogContainsBeginEvent(entries, 1, NetLogEventType::SOCKET_POOL));
    EXPECT_TRUE(LogContainsEntryWithType(entries, 2, NetLogEventType::SOCKET_POOL_REUSED_AN_EXISTING_SOCKET));
}
...
TEST_F(ClientSocketPoolBaseTest, CleanupTimedOutIdleSocketsNoReuse)
{
    ...
    ClientSocketHandle handle2;
    TestCompletionCallback callback2;
    rv = handle2.Init(TestGroupId("a"), params_, std::nullopt, LOWEST, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback2.callback(),
        ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource());
    ASSERT_THAT(rv, IsError(ERR_IO_PENDING));
    EXPECT_EQ(LOAD_STATE_CONNECTING, pool_->GetLoadState(TestGroupId("a"), &handle2));
    ...
    // Use the socket.
    EXPECT_EQ(1, handle2.socket()->Write(nullptr, 1, CompletionOnceCallback(), TRAFFIC_ANNOTATION_FOR_TESTS));
    handle2.Reset();
    ...
    NetLogWithSource net_log_with_source = NetLogWithSource::Make(NetLogSourceType::NONE);
    TestCompletionCallback callback3;
    rv = handle.Init(TestGroupId("a"), params_, std::nullopt, LOWEST, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback3.callback(),
        ClientSocketPool::ProxyAuthCallback(), pool_.get(), net_log_with_source);
    ASSERT_THAT(rv, IsError(ERR_IO_PENDING));
    ...
    auto entries = net_log_observer_.GetEntriesForSource(net_log_with_source.source());
    EXPECT_FALSE(LogContainsEntryWithType(entries, 1, NetLogEventType::SOCKET_POOL_REUSED_AN_EXISTING_SOCKET));
    ExpectSocketClosedWithReason(net_log_source2, TransportClientSocketPool::kIdleTimeLimitExpired);
}
...
TEST_F(ClientSocketPoolBaseTest, MultipleReleasingDisconnectedSockets)
{
    ...
    ClientSocketHandle handle3;
    TestCompletionCallback callback3;
    rv = handle3.Init(TestGroupId("a"), params_, std::nullopt, LOWEST, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback3.callback(),
        ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource());
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));

    ClientSocketHandle handle4;
    TestCompletionCallback callback4;
    rv = handle4.Init(TestGroupId("a"), params_, std::nullopt, LOWEST, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback4.callback(),
        ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource());
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
...
{
    CreatePoolWithIdleTimeouts(4 /* socket limit */, 4 /* socket limit per group */,
        base::TimeDelta(), // Time out unused sockets immediately.
        base::Days(1)); // Don't time out used sockets.
    ...
    for (int i = 0; i < 2; ++i) {
        EXPECT_EQ(OK,
            handle_a[i].Init(TestGroupId("a"), params_, std::nullopt, LOWEST, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback_a[i].callback(),
                ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
        EXPECT_EQ(OK,
            handle_b[i].Init(TestGroupId("b"), params_, std::nullopt, LOWEST, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback_b[i].callback(),
                ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
    }
...
TEST_F(ClientSocketPoolBaseTest, ReleasingDisconnectedSocketsMaintainsPriorityOrder)
{
    CreatePool(kDefaultMaxSockets, kDefaultMaxSocketsPerGroup);
...
public:
    TestReleasingSocketRequest(TransportClientSocketPool* pool, int expected_result, bool reset_releasing_handle)
        : pool_(pool)
    ...
    {
        return base::BindOnce(&TestReleasingSocketRequest::OnComplete, base::Unretained(this));
    }
...
private:
    void OnComplete(int result)
    {
        ...
        EXPECT_EQ(expected_result_,
            handle2_.Init(TestGroupId("a"), ClientSocketPool::SocketParams::CreateForHttpForTesting(), std::nullopt, DEFAULT_PRIORITY, SocketTag(),
                ClientSocketPool::RespectLimits::ENABLED, CompletionOnceCallback(), ClientSocketPool::ProxyAuthCallback(), pool_, NetLogWithSource()));
    }
...
TEST_F(ClientSocketPoolBaseTest, AdditionalErrorSocketsDontUseSlot)
{
    ...
    EXPECT_EQ(ERR_IO_PENDING,
        req.handle()->Init(TestGroupId("a"), params_, std::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, req.callback(),
            ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
...
TEST_F(ClientSocketPoolBaseTest, DoNotReuseSocketAfterFlush)
{
    ...
    ExpectSocketClosedWithReason(source, TransportClientSocketPool::kSocketGenerationOutOfDate);
}

class ConnectWithinCallback : public TestCompletionCallbackBase {
public:
    ConnectWithinCallback(
        const ClientSocketPool::GroupId& group_id, const scoped_refptr<ClientSocketPool::SocketParams>& params, TransportClientSocketPool* pool)
        : group_id_(group_id)
        , params_(params)
        , pool_(pool)
    {
    ...
    CompletionOnceCallback callback()
    {
        return base::BindOnce(&ConnectWithinCallback::OnComplete, base::Unretained(this));
    }
...
private:
    void OnComplete(int result)
    {
        SetResult(result);
        EXPECT_EQ(ERR_IO_PENDING,
            handle_.Init(group_id_, params_, std::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, nested_callback_.callback(),
...
{
    CreatePool(kDefaultMaxSockets, kDefaultMaxSockets, true /* enable_backup_connect_jobs */);

    connect_job_factory_->set_job_type(TestConnectJob::kMockWaitingJob);
    ClientSocketHandle handle;
    TestCompletionCallback callback;
    EXPECT_EQ(ERR_IO_PENDING,
        handle.Init(TestGroupId("bar"), params_, std::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback.callback(),
            ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
    ...
    client_socket_factory_.SetJobLoadState(0, LOAD_STATE_CONNECTING);
    FastForwardBy(base::Milliseconds(ClientSocketPool::kMaxConnectRetryIntervalMs));
    EXPECT_EQ(2, client_socket_factory_.allocation_count());
}
...
TEST_F(ClientSocketPoolBaseTest, BackupSocketCancelAtMaxSockets)
{
    ...
    for (int i = 1; i < kDefaultMaxSockets; ++i) {
        EXPECT_EQ(OK,
            handles[i].Init(TestGroupId("bar"), params_, std::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED,
                callback.callback(), ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
    }
    ...
    FastForwardBy(base::Milliseconds(ClientSocketPool::kMaxConnectRetryIntervalMs / 2 * 3));
...
TEST_F(ClientSocketPoolBaseTest, CancelBackupSocketAfterCancelingAllRequests)
{
    ...
    EXPECT_EQ(0u, pool_->NumNeverAssignedConnectJobsInGroupForTesting(TestGroupId("bar")));
    EXPECT_EQ(0u, pool_->NumUnassignedConnectJobsInGroupForTesting(TestGroupId("bar")));
...
TEST_F(ClientSocketPoolBaseTest, CancelBackupSocketAfterFinishingAllRequests)
{
    ...
    TestCompletionCallback callback2;
    EXPECT_EQ(ERR_IO_PENDING,
        handle2.Init(TestGroupId("bar"), params_, std::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback2.callback(),
            ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
    ASSERT_TRUE(pool_->HasGroupForTesting(TestGroupId("bar")));
...
TEST_F(ClientSocketPoolBaseTest, DelayedSocketBindingWaitingForConnect)
{
    ...
    TestCompletionCallback callback;
    EXPECT_EQ(ERR_IO_PENDING,
        handle1.Init(TestGroupId("a"), params_, std::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback.callback(),
            ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
    EXPECT_THAT(callback.WaitForResult(), IsOk());
    ...
    ClientSocketHandle handle2;
    EXPECT_EQ(ERR_IO_PENDING,
        handle2.Init(TestGroupId("a"), params_, std::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback.callback(),
            ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
...
TEST_F(ClientSocketPoolBaseTest, SynchronouslyProcessOnePendingRequest)
{
    ...
    EXPECT_EQ(ERR_IO_PENDING,
        handle1.Init(TestGroupId("a"), params_, std::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback1.callback(),
            ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
    EXPECT_EQ(1u, pool_->NumConnectJobsInGroupForTesting(TestGroupId("a")));
...
TEST_F(ClientSocketPoolBaseTest, PreferUsedSocketToUnusedSocket)
{
    ...
    TestCompletionCallback callback3;
    EXPECT_EQ(ERR_IO_PENDING,
        handle3.Init(TestGroupId("a"), params_, std::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback3.callback(),
            ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
    ...
    // Use the socket.
    EXPECT_EQ(1, handle1.socket()->Write(nullptr, 1, CompletionOnceCallback(), TRAFFIC_ANNOTATION_FOR_TESTS));
    EXPECT_EQ(1, handle3.socket()->Write(nullptr, 1, CompletionOnceCallback(), TRAFFIC_ANNOTATION_FOR_TESTS));
...
TEST_F(ClientSocketPoolBaseTest, RequestSockets)
{
    ...
    ASSERT_TRUE(pool_->HasGroupForTesting(TestGroupId("a")));
    EXPECT_EQ(2u, pool_->NumConnectJobsInGroupForTesting(TestGroupId("a")));
    EXPECT_EQ(2u, pool_->NumNeverAssignedConnectJobsInGroupForTesting(TestGroupId("a")));
    EXPECT_EQ(2u, pool_->NumUnassignedConnectJobsInGroupForTesting(TestGroupId("a")));
    EXPECT_EQ(0u, pool_->IdleSocketCountInGroup(TestGroupId("a")));
    ...
    EXPECT_EQ(2u, pool_->NumConnectJobsInGroupForTesting(TestGroupId("a")));
    EXPECT_EQ(0u, pool_->NumNeverAssignedConnectJobsInGroupForTesting(TestGroupId("a")));
    EXPECT_EQ(0u, pool_->NumUnassignedConnectJobsInGroupForTesting(TestGroupId("a")));
    EXPECT_EQ(0u, pool_->IdleSocketCountInGroup(TestGroupId("a")));
...
TEST_F(ClientSocketPoolBaseTest, RequestSocketsWhenAlreadyHaveAConnectJob)
{
    ...
    EXPECT_EQ(2u, pool_->NumConnectJobsInGroupForTesting(TestGroupId("a")));
    EXPECT_EQ(1u, pool_->NumNeverAssignedConnectJobsInGroupForTesting(TestGroupId("a")));
    EXPECT_EQ(1u, pool_->NumUnassignedConnectJobsInGroupForTesting(TestGroupId("a")));
    EXPECT_EQ(0u, pool_->IdleSocketCountInGroup(TestGroupId("a")));
...
TEST_F(ClientSocketPoolBaseTest, RequestSocketsWhenAlreadyHaveMultipleConnectJob)
{
    CreatePool(4, 4);
    connect_job_factory_->set_job_type(TestConnectJob::kMockPendingJob);
    ...
    EXPECT_EQ(OK, pool_->RequestSockets(TestGroupId("a"), params_, std::nullopt, 2, CompletionOnceCallback(), NetLogWithSource()));
...
TEST_F(ClientSocketPoolBaseTest, RequestSocketsAtMaxSocketLimit)
{
    ...
    TestCompletionCallback preconnect_callback;
    EXPECT_EQ(
        ERR_IO_PENDING, pool_->RequestSockets(TestGroupId("a"), params_, std::nullopt, kDefaultMaxSockets, preconnect_callback.callback(), NetLogWithSource()));

    ASSERT_TRUE(pool_->HasGroupForTesting(TestGroupId("a")));
    EXPECT_EQ(kDefaultMaxSockets, static_cast<int>(pool_->NumConnectJobsInGroupForTesting(TestGroupId("a"))));
    EXPECT_EQ(kDefaultMaxSockets, static_cast<int>(pool_->NumNeverAssignedConnectJobsInGroupForTesting(TestGroupId("a"))));
    EXPECT_EQ(kDefaultMaxSockets, static_cast<int>(pool_->NumUnassignedConnectJobsInGroupForTesting(TestGroupId("a"))));

    ASSERT_FALSE(pool_->HasGroupForTesting(TestGroupId("b")));

    EXPECT_EQ(OK, pool_->RequestSockets(TestGroupId("b"), params_, std::nullopt, kDefaultMaxSockets, CompletionOnceCallback(), NetLogWithSource()));
...
TEST_F(ClientSocketPoolBaseTest, RequestSocketsHitMaxSocketLimit)
{
    ...
    EXPECT_EQ(ERR_IO_PENDING,
        pool_->RequestSockets(TestGroupId("a"), params_, std::nullopt, kDefaultMaxSockets - 1, preconnect_callback1.callback(), NetLogWithSource()));

    ASSERT_TRUE(pool_->HasGroupForTesting(TestGroupId("a")));
    EXPECT_EQ(kDefaultMaxSockets - 1, static_cast<int>(pool_->NumConnectJobsInGroupForTesting(TestGroupId("a"))));
    EXPECT_EQ(kDefaultMaxSockets - 1, static_cast<int>(pool_->NumNeverAssignedConnectJobsInGroupForTesting(TestGroupId("a"))));
    EXPECT_EQ(kDefaultMaxSockets - 1, static_cast<int>(pool_->NumUnassignedConnectJobsInGroupForTesting(TestGroupId("a"))));
    EXPECT_FALSE(pool_->IsStalled());
    ...
    EXPECT_EQ(ERR_IO_PENDING,
        pool_->RequestSockets(TestGroupId("b"), params_, std::nullopt, kDefaultMaxSockets, preconnect_callback2.callback(), NetLogWithSource()));
...
TEST_F(ClientSocketPoolBaseTest, RequestSocketsSynchronous)
{
    ...
    EXPECT_EQ(OK, pool_->RequestSockets(TestGroupId("a"), params_, std::nullopt, kDefaultMaxSocketsPerGroup, CompletionOnceCallback(), NetLogWithSource()));

    ASSERT_TRUE(pool_->HasGroupForTesting(TestGroupId("a")));
    EXPECT_EQ(0u, pool_->NumConnectJobsInGroupForTesting(TestGroupId("a")));
    EXPECT_EQ(0u, pool_->NumNeverAssignedConnectJobsInGroupForTesting(TestGroupId("a")));
    EXPECT_EQ(0u, pool_->NumUnassignedConnectJobsInGroupForTesting(TestGroupId("a")));
    EXPECT_EQ(kDefaultMaxSocketsPerGroup, static_cast<int>(pool_->IdleSocketCountInGroup(TestGroupId("a"))));

    EXPECT_EQ(OK, pool_->RequestSockets(TestGroupId("b"), params_, std::nullopt, kDefaultMaxSocketsPerGroup, CompletionOnceCallback(), NetLogWithSource()));

    EXPECT_EQ(0u, pool_->NumConnectJobsInGroupForTesting(TestGroupId("b")));
    EXPECT_EQ(0u, pool_->NumNeverAssignedConnectJobsInGroupForTesting(TestGroupId("b")));
    EXPECT_EQ(0u, pool_->NumUnassignedConnectJobsInGroupForTesting(TestGroupId("b")));
    EXPECT_EQ(kDefaultMaxSocketsPerGroup, static_cast<int>(pool_->IdleSocketCountInGroup(TestGroupId("b"))));
}
...
TEST_F(ClientSocketPoolBaseTest, RequestSocketsDifferentNumSockets)
{
    ...
    TestCompletionCallback preconnect_callback1;
    EXPECT_EQ(ERR_IO_PENDING, pool_->RequestSockets(TestGroupId("a"), params_, std::nullopt, 1, preconnect_callback1.callback(), NetLogWithSource()));
    ...
    TestCompletionCallback preconnect_callback2;
    EXPECT_EQ(ERR_IO_PENDING, pool_->RequestSockets(TestGroupId("a"), params_, std::nullopt, 2, preconnect_callback2.callback(), NetLogWithSource()));
    EXPECT_EQ(2u, pool_->NumConnectJobsInGroupForTesting(TestGroupId("a")));
    EXPECT_EQ(2u, pool_->NumNeverAssignedConnectJobsInGroupForTesting(TestGroupId("a")));
    EXPECT_EQ(2u, pool_->NumUnassignedConnectJobsInGroupForTesting(TestGroupId("a")));
    EXPECT_EQ(0u, pool_->IdleSocketCountInGroup(TestGroupId("a")));
    ...
    EXPECT_EQ(ERR_IO_PENDING, pool_->RequestSockets(TestGroupId("a"), params_, std::nullopt, 3, preconnect_callback3.callback(), NetLogWithSource()));
    EXPECT_EQ(3u, pool_->NumConnectJobsInGroupForTesting(TestGroupId("a")));
    EXPECT_EQ(3u, pool_->NumNeverAssignedConnectJobsInGroupForTesting(TestGroupId("a")));
    EXPECT_EQ(3u, pool_->NumUnassignedConnectJobsInGroupForTesting(TestGroupId("a")));
    EXPECT_EQ(0u, pool_->IdleSocketCountInGroup(TestGroupId("a")));

    EXPECT_EQ(OK, pool_->RequestSockets(TestGroupId("a"), params_, std::nullopt, 1, CompletionOnceCallback(), NetLogWithSource()));
    EXPECT_EQ(3u, pool_->NumConnectJobsInGroupForTesting(TestGroupId("a")));
    EXPECT_EQ(3u, pool_->NumNeverAssignedConnectJobsInGroupForTesting(TestGroupId("a")));
    EXPECT_EQ(3u, pool_->NumUnassignedConnectJobsInGroupForTesting(TestGroupId("a")));
    EXPECT_EQ(0u, pool_->IdleSocketCountInGroup(TestGroupId("a")));
...
TEST_F(ClientSocketPoolBaseTest, PreconnectClosesIdleSocketRemovesGroup)
{
    ...
    TestCompletionCallback callback2;
    EXPECT_EQ(ERR_IO_PENDING,
        handle1.Init(TestGroupId("b"), params_, std::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback1.callback(),
            ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
    EXPECT_EQ(ERR_IO_PENDING,
        handle2.Init(TestGroupId("b"), params_, std::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback2.callback(),
            ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
...
TEST_F(ClientSocketPoolBaseTest, PreconnectWithoutBackupJob)
{
    ...
    base::RunLoop loop;
    connect_job_factory_->set_job_type(TestConnectJob::kMockPendingJob);
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, loop.QuitClosure(), base::Seconds(1));
    loop.Run();
    EXPECT_FALSE(pool_->HasGroupForTesting(TestGroupId("a")));
}
...
TEST_F(ClientSocketPoolBaseTest, RequestGetsAssignedJob)
{
    ...
    EXPECT_TRUE(pool_->RequestInGroupWithHandleHasJobForTesting(TestGroupId("a"), &handle1));
}
...
TEST_F(ClientSocketPoolBaseTest, MultipleRequestsGetAssignedJobs)
{
    ...
    EXPECT_TRUE(pool_->RequestInGroupWithHandleHasJobForTesting(TestGroupId("a"), &handle1));
    EXPECT_TRUE(pool_->RequestInGroupWithHandleHasJobForTesting(TestGroupId("a"), &handle2));
...
TEST_F(ClientSocketPoolBaseTest, HigherPriorityRequestStealsJob)
{
    ...
    TestCompletionCallback callback2;
    EXPECT_EQ(ERR_IO_PENDING,
        handle2.Init(TestGroupId("a"), params_, std::nullopt, HIGHEST, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback2.callback(),
            ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
    ...
    // request.
    EXPECT_TRUE(pool_->RequestInGroupWithHandleHasJobForTesting(TestGroupId("a"), &handle2));
    EXPECT_FALSE(pool_->RequestInGroupWithHandleHasJobForTesting(TestGroupId("a"), &handle1));
}
...
TEST_F(ClientSocketPoolBaseTest, RequestStealsJobFromLowestRequestWithJob)
{
    ...
    EXPECT_EQ(ERR_IO_PENDING,
        handle_lowest.Init(TestGroupId("a"), params_, std::nullopt, LOWEST, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback_lowest.callback(),
            ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
    ...
    EXPECT_EQ(ERR_IO_PENDING,
        handle_highest.Init(TestGroupId("a"), params_, std::nullopt, HIGHEST, SocketTag(), ClientSocketPool::RespectLimits::ENABLED,
            callback_highest.callback(), ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
    ...
    EXPECT_EQ(ERR_IO_PENDING,
        handle_low.Init(TestGroupId("a"), params_, std::nullopt, LOW, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback_low.callback(),
            ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
    ...
    EXPECT_EQ(ERR_IO_PENDING,
        handle_lowest2.Init(TestGroupId("a"), params_, std::nullopt, LOWEST, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback_lowest2.callback(),
            ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
    ...
    EXPECT_TRUE(pool_->RequestInGroupWithHandleHasJobForTesting(TestGroupId("a"), &handle_highest));
    EXPECT_TRUE(pool_->RequestInGroupWithHandleHasJobForTesting(TestGroupId("a"), &handle_low));
    EXPECT_TRUE(pool_->RequestInGroupWithHandleHasJobForTesting(TestGroupId("a"), &handle_lowest));
    EXPECT_FALSE(pool_->RequestInGroupWithHandleHasJobForTesting(TestGroupId("a"), &handle_lowest2));
    ...
    EXPECT_EQ(ERR_IO_PENDING,
        handle_medium.Init(TestGroupId("a"), params_, std::nullopt, MEDIUM, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback_medium.callback(),
            ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
    ...
    EXPECT_EQ(0u, pool_->IdleSocketCountInGroup(TestGroupId("a")));
    EXPECT_TRUE(pool_->RequestInGroupWithHandleHasJobForTesting(TestGroupId("a"), &handle_highest));
    EXPECT_TRUE(pool_->RequestInGroupWithHandleHasJobForTesting(TestGroupId("a"), &handle_medium));
    EXPECT_TRUE(pool_->RequestInGroupWithHandleHasJobForTesting(TestGroupId("a"), &handle_low));
    EXPECT_FALSE(pool_->RequestInGroupWithHandleHasJobForTesting(TestGroupId("a"), &handle_lowest));
    EXPECT_FALSE(pool_->RequestInGroupWithHandleHasJobForTesting(TestGroupId("a"), &handle_lowest2));
}
...
TEST_F(ClientSocketPoolBaseTest, ReprioritizeRequestStealsJob)
{
    ...
    EXPECT_TRUE(pool_->RequestInGroupWithHandleHasJobForTesting(TestGroupId("a"), &handle1));
    EXPECT_FALSE(pool_->RequestInGroupWithHandleHasJobForTesting(TestGroupId("a"), &handle2));
...
TEST_F(ClientSocketPoolBaseTest, JobCompletionReassignsJob)
{
    ...
    TestCompletionCallback callback1;
    EXPECT_EQ(ERR_IO_PENDING,
        handle1.Init(TestGroupId("a"), params_, std::nullopt, HIGHEST, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback1.callback(),
            ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
...
public:
    MockLayeredPool(TransportClientSocketPool* pool, const ClientSocketPool::GroupId& group_id)
        : pool_(pool)
        , group_id_(group_id)
    {
    ...
    {
        return handle_.Init(group_id_, ClientSocketPool::SocketParams::CreateForHttpForTesting(), std::nullopt, DEFAULT_PRIORITY, SocketTag(),
            ClientSocketPool::RespectLimits::ENABLED, callback_.callback(), ClientSocketPool::ProxyAuthCallback(), pool, NetLogWithSource());
    }

    int RequestSocketWithoutLimits(TransportClientSocketPool* pool)
    {
        return handle_.Init(group_id_, ClientSocketPool::SocketParams::CreateForHttpForTesting(), std::nullopt, MAXIMUM_PRIORITY, SocketTag(),
            ClientSocketPool::RespectLimits::DISABLED, callback_.callback(), ClientSocketPool::ProxyAuthCallback(), pool, NetLogWithSource());
    }
...
TEST_F(ClientSocketPoolBaseTest, CloseIdleSocketsHeldByLayeredPoolWhenNeeded)
{
    ...
    EXPECT_CALL(mock_layered_pool, CloseOneIdleConnection()).WillOnce(Invoke(&mock_layered_pool, &MockLayeredPool::ReleaseOneConnection));
    ClientSocketHandle handle;
    TestCompletionCallback callback;
    EXPECT_EQ(ERR_IO_PENDING,
        handle.Init(TestGroupId("a"), params_, std::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback.callback(),
...
// fails.
TEST_F(ClientSocketPoolBaseTest, CloseIdleSocketsHeldByLayeredPoolWhenNeededFails)
{
    CreatePool(1, 1);
    connect_job_factory_->set_job_type(TestConnectJob::kMockJob);
...
TEST_F(ClientSocketPoolBaseTest, CloseIdleSocketsHeldByLayeredPoolWhenNeededSameGroup)
{
    CreatePool(2, 2);
    connect_job_factory_->set_job_type(TestConnectJob::kMockJob);
    ...
    TestCompletionCallback callback1;
    EXPECT_EQ(OK,
        handle1.Init(TestGroupId("group1"), params_, std::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED,
            callback1.callback(), ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
    ...
    TestCompletionCallback callback2;
    EXPECT_EQ(ERR_IO_PENDING,
        handle.Init(TestGroupId("group2"), params_, std::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback2.callback(),
            ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
    EXPECT_THAT(callback2.WaitForResult(), IsOk());
}
...
TEST_F(ClientSocketPoolBaseTest, CloseIdleSocketsHeldByLayeredPoolInSameGroupWhenNeeded)
{
    CreatePool(2, 2);
    ...
    MockLayeredPool mock_layered_pool(pool_.get(), TestGroupId("group2"));
    EXPECT_THAT(mock_layered_pool.RequestSocket(pool_.get()), IsOk());
    EXPECT_CALL(mock_layered_pool, CloseOneIdleConnection()).WillRepeatedly(Invoke(&mock_layered_pool, &MockLayeredPool::ReleaseOneConnection));
    mock_layered_pool.set_can_release_connection(false);
    ...
    TestCompletionCallback callback3;
    EXPECT_EQ(ERR_IO_PENDING,
        handle3.Init(TestGroupId("group3"), params_, std::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED,
            callback3.callback(), ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
    ...
    TestCompletionCallback callback4;
    EXPECT_EQ(ERR_IO_PENDING,
        handle4.Init(TestGroupId("group3"), params_, std::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED,
            callback4.callback(), ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
    EXPECT_THAT(callback3.WaitForResult(), IsOk());
...
TEST_F(ClientSocketPoolBaseTest, CloseIdleSocketsHeldByLayeredPoolInSameGroupWhenNeeded2)
{
    CreatePool(2, 2);
    std::list<TestConnectJob::JobType> job_types;
    job_types.push_back(TestConnectJob::kMockJob);
    ...
    TestCompletionCallback callback3;
    EXPECT_EQ(ERR_IO_PENDING,
        handle3.Init(TestGroupId("group3"), params_, std::nullopt, MEDIUM, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback3.callback(),
            ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
    ...
    TestCompletionCallback callback4;
    EXPECT_EQ(ERR_IO_PENDING,
        handle4.Init(TestGroupId("group3"), params_, std::nullopt, HIGHEST, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback4.callback(),
            ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()));
    EXPECT_THAT(callback4.WaitForResult(), IsOk());
    EXPECT_FALSE(callback3.have_result());
...
TEST_F(ClientSocketPoolBaseTest, CloseMultipleIdleSocketsHeldByLayeredPoolWhenNeeded)
{
    CreatePool(1, 1);
    connect_job_factory_->set_job_type(TestConnectJob::kMockJob);
    ...
    EXPECT_CALL(mock_layered_pool1, CloseOneIdleConnection()).WillRepeatedly(Invoke(&mock_layered_pool1, &MockLayeredPool::ReleaseOneConnection));
    MockLayeredPool mock_layered_pool2(pool_.get(), TestGroupId("bar"));
    EXPECT_THAT(mock_layered_pool2.RequestSocketWithoutLimits(pool_.get()), IsOk());
    EXPECT_CALL(mock_layered_pool2, CloseOneIdleConnection()).WillRepeatedly(Invoke(&mock_layered_pool2, &MockLayeredPool::ReleaseOneConnection));
    ClientSocketHandle handle;
    TestCompletionCallback callback;
    EXPECT_EQ(ERR_IO_PENDING,
        handle.Init(TestGroupId("a"), params_, std::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback.callback(),
...
TEST_F(ClientSocketPoolBaseTest, IgnoreLimits)
{
    ...
    EXPECT_EQ(OK, StartRequestWithIgnoreLimits(TestGroupId("a"), MAXIMUM_PRIORITY, ClientSocketPool::RespectLimits::ENABLED));
    EXPECT_EQ(0u, pool_->NumConnectJobsInGroupForTesting(TestGroupId("a")));

    connect_job_factory_->set_job_type(TestConnectJob::kMockPendingJob);

    EXPECT_EQ(ERR_IO_PENDING, StartRequestWithIgnoreLimits(TestGroupId("a"), MAXIMUM_PRIORITY, ClientSocketPool::RespectLimits::ENABLED));
    EXPECT_EQ(0u, pool_->NumConnectJobsInGroupForTesting(TestGroupId("a")));

    // Issue a request that ignores the limits, so a new ConnectJob is
    // created.
    EXPECT_EQ(ERR_IO_PENDING, StartRequestWithIgnoreLimits(TestGroupId("a"), MAXIMUM_PRIORITY, ClientSocketPool::RespectLimits::DISABLED));
...
public:
    ...
    void InitHandle(scoped_refptr<ClientSocketPool::SocketParams> params, TransportClientSocketPool* pool, RequestPriority priority = DEFAULT_PRIORITY,
    ...
    {
        EXPECT_EQ(ERR_IO_PENDING,
            handle_.Init(group_id_in, params, std::nullopt, priority, SocketTag(), respect_limits, callback_.callback(),
                base::BindRepeating(&TestAuthHelper::AuthCallback, base::Unretained(this)), pool, NetLogWithSource()));
    }
    ...
    int auth_count() const
    {
        return auth_count_;
    }
    int have_result() const
    {
        return callback_.have_result();
    }

private:
    void AuthCallback(const HttpResponseInfo& response, HttpAuthController* auth_controller, base::OnceClosure restart_with_auth_callback)
    {
        EXPECT_FALSE(restart_with_auth_callback_);
...
TEST_F(ClientSocketPoolBaseTest, ProxyAuthOnce)
{
    ...
    EXPECT_EQ(1u, pool_->NumConnectJobsInGroupForTesting(TestGroupId("a")));
    EXPECT_EQ(LOAD_STATE_CONNECTING, pool_->GetLoadState(TestGroupId("a"), auth_helper.handle()));

    auth_helper.WaitForAuth();
    EXPECT_EQ(1u, pool_->NumConnectJobsInGroupForTesting(TestGroupId("a")));
    EXPECT_EQ(LOAD_STATE_ESTABLISHING_PROXY_TUNNEL, pool_->GetLoadState(TestGroupId("a"), auth_helper.handle()));
...
{
    CreatePool(1, 1);
    connect_job_factory_->set_job_type(TestConnectJob::kMockAuthChallengeOnceFailingJob);
...
{
    CreatePool(1, 1);
    connect_job_factory_->set_job_type(TestConnectJob::kMockAuthChallengeTwiceJob);
...
{
    CreatePool(1, 1);
    connect_job_factory_->set_job_type(TestConnectJob::kMockAuthChallengeTwiceFailingJob);
...
// created, if needed.
TEST_F(ClientSocketPoolBaseTest, ProxyAuthCreateNewConnectJobOnDestroyBoundRequest)
{
...
TEST_F(ClientSocketPoolBaseTest, ProxyAuthCreateNewConnectJobOnDestroyBoundRequestDifferentGroups)
{
    CreatePool(1 /* max_sockets */, 1 /* max_sockets_per_group */);
    connect_job_factory_->set_job_type(TestConnectJob::kMockAuthChallengeOnceFailingJob);
    ...
    // has been reached.
    TestAuthHelper auth_helper2;
    auth_helper2.InitHandle(params_, pool_.get(), DEFAULT_PRIORITY, ClientSocketPool::RespectLimits::ENABLED, TestGroupId("b"));
    EXPECT_EQ(1u, pool_->NumConnectJobsInGroupForTesting(TestGroupId("a")));
    EXPECT_EQ(0u, pool_->NumConnectJobsInGroupForTesting(TestGroupId("b")));
...
TEST_F(ClientSocketPoolBaseTest, ProxyAuthStaysBound)
{
    ...
    auth_helper4.InitHandle(params_, pool_.get(), HIGHEST, ClientSocketPool::RespectLimits::DISABLED);
    EXPECT_EQ(2u, pool_->NumConnectJobsInGroupForTesting(TestGroupId("a")));
...
class ClientSocketPoolBaseRefreshTest : public ClientSocketPoolBaseTest, public testing::WithParamInterface<RefreshType> {
public:
    void CreatePoolForRefresh(int max_sockets, int max_sockets_per_group, bool enable_backup_connect_jobs = false)
    {
        switch (GetParam()) {
        case RefreshType::kServer:
            CreatePool(max_sockets, max_sockets_per_group, enable_backup_connect_jobs);
            break;
        case RefreshType::kProxy:
            CreatePoolWithIdleTimeouts(max_sockets, max_sockets_per_group, kUnusedIdleSocketTimeout, ClientSocketPool::used_idle_socket_timeout(),
                enable_backup_connect_jobs, PacResultElementToProxyChain("HTTPS myproxy:70"));
            break;
        }
    }
    ...
    static ClientSocketPool::GroupId GetGroupIdInPartition()
    {
        ...
        return TestGroupId("a", 443, url::kHttpsScheme, PrivacyMode::PRIVACY_MODE_DISABLED, kNetworkAnonymizationKey);
    }
...
INSTANTIATE_TEST_SUITE_P(RefreshType, ClientSocketPoolBaseRefreshTest, ::testing::Values(RefreshType::kServer, RefreshType::kProxy));
...
TEST_P(ClientSocketPoolBaseRefreshTest, RefreshGroupCreatesNewConnectJobs)
{
    ...
    ClientSocketHandle handle;
    TestCompletionCallback callback;
    EXPECT_THAT(handle.Init(kGroupId, params_, std::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback.callback(),
                    ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()),
        IsError(ERR_IO_PENDING));
...
TEST_P(ClientSocketPoolBaseRefreshTest, RefreshGroupClosesIdleConnectJobs)
{
    ...
    EXPECT_EQ(OK, pool_->RequestSockets(kGroupId, params_, std::nullopt, 2, CompletionOnceCallback(), NetLogWithSource()));

    EXPECT_EQ(OK, pool_->RequestSockets(kGroupIdInPartition, params_, std::nullopt, 2, CompletionOnceCallback(), NetLogWithSource()));
    ASSERT_TRUE(pool_->HasGroupForTesting(kGroupId));
...
TEST_F(ClientSocketPoolBaseTest, RefreshGroupDoesNotCloseIdleConnectJobsInOtherGroup)
{
    CreatePool(kDefaultMaxSockets, kDefaultMaxSocketsPerGroup);
    const ClientSocketPool::GroupId kGroupId = TestGroupId("a", 443, url::kHttpsScheme);
    const ClientSocketPool::GroupId kOtherGroupId = TestGroupId("b", 443, url::kHttpsScheme);

    EXPECT_EQ(OK, pool_->RequestSockets(kOtherGroupId, params_, std::nullopt, 2, CompletionOnceCallback(), NetLogWithSource()));
...
TEST_F(ClientSocketPoolBaseTest, RefreshGroupDoesNotPreventSocketReuseInOtherGroup)
{
    CreatePool(kDefaultMaxSockets, kDefaultMaxSocketsPerGroup);
    const ClientSocketPool::GroupId kGroupId = TestGroupId("a", 443, url::kHttpsScheme);
    const ClientSocketPool::GroupId kOtherGroupId = TestGroupId("b", 443, url::kHttpsScheme);
    ...
    ClientSocketHandle handle;
    TestCompletionCallback callback;
    EXPECT_THAT(handle.Init(kOtherGroupId, params_, std::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback.callback(),
                    ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()),
        IsOk());
    ASSERT_TRUE(pool_->HasGroupForTesting(kOtherGroupId));
...
TEST_P(ClientSocketPoolBaseRefreshTest, RefreshGroupReplacesBoundConnectJobOnConnect)
{
    ...
    TestAuthHelper auth_helper;
    auth_helper.InitHandle(params_, pool_.get(), DEFAULT_PRIORITY, ClientSocketPool::RespectLimits::ENABLED, kGroupId);
...
TEST_F(ClientSocketPoolBaseTest, RefreshProxyRefreshesAllGroups)
{
    ...
    CreatePoolWithIdleTimeouts(kDefaultMaxSockets, kDefaultMaxSocketsPerGroup, kUnusedIdleSocketTimeout, ClientSocketPool::used_idle_socket_timeout(),
        false /* no backup connect jobs */, proxy_chain);

    const ClientSocketPool::GroupId kGroupId1 = TestGroupId("a", 443, url::kHttpsScheme);
    const ClientSocketPool::GroupId kGroupId2 = TestGroupId("b", 443, url::kHttpsScheme);
    const ClientSocketPool::GroupId kGroupId3 = TestGroupId("c", 443, url::kHttpsScheme);

    // Make three sockets in three different groups. The third socket is released
    // to the pool as idle.
    ClientSocketHandle handle1, handle2, handle3;
    TestCompletionCallback callback;
    EXPECT_THAT(handle1.Init(kGroupId1, params_, std::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback.callback(),
                    ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()),
        IsOk());
    EXPECT_THAT(handle2.Init(kGroupId2, params_, std::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback.callback(),
                    ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()),
        IsOk());
    EXPECT_THAT(handle3.Init(kGroupId3, params_, std::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback.callback(),
                    ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()),
        IsOk());
    handle3.Reset();
...
TEST_F(ClientSocketPoolBaseTest, RefreshBothPrivacyAndNormalSockets)
{
    ...
    const ClientSocketPool::GroupId kGroupIdPrivacy = TestGroupId("a", 443, url::kHttpsScheme, PrivacyMode::PRIVACY_MODE_ENABLED);
    const ClientSocketPool::GroupId kOtherGroupId = TestGroupId("b", 443, url::kHttpsScheme);

    // Make a socket in each groups.
    ClientSocketHandle handle1, handle2, handle3;
    TestCompletionCallback callback;
    EXPECT_THAT(handle1.Init(kGroupId, params_, std::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback.callback(),
                    ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()),
        IsOk());
    EXPECT_THAT(handle2.Init(kGroupIdPrivacy, params_, std::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED,
                    callback.callback(), ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()),
        IsOk());
    EXPECT_THAT(handle3.Init(kOtherGroupId, params_, std::nullopt, DEFAULT_PRIORITY, SocketTag(), ClientSocketPool::RespectLimits::ENABLED, callback.callback(),
                    ClientSocketPool::ProxyAuthCallback(), pool_.get(), NetLogWithSource()),
        IsOk());
    ASSERT_TRUE(pool_->HasGroupForTesting(kGroupId));
    EXPECT_EQ(1, pool_->NumActiveSocketsInGroupForTesting(kGroupId));
