...
class TransportClientSocketTest : public ::testing::Test, public WithTaskEnvironment {
public:
...
void TransportClientSocketTest::SetUp()
{
    ...
    ASSERT_THAT(listen_sock_->Listen(local_address, 1, /*ipv6_only=*/std::nullopt), IsOk());
    ...
    listen_sock_->Accept(&connected_sock_, base::BindOnce(&TransportClientSocketTest::AcceptCallback, base::Unretained(this)));

    AddressList addr = AddressList::CreateFromIPAddress(IPAddress::IPv4Localhost(), listen_port_);
    sock_ = socket_factory_->CreateTransportClientSocket(addr, nullptr, nullptr, NetLog::Get(), NetLogSource());
}

void TransportClientSocketTest::EstablishConnection(TestCompletionCallback* callback)
{
...
TEST_F(TransportClientSocketTest, Connect)
{
    ...
    EXPECT_TRUE(LogContainsBeginEvent(net_log_entries, 0, NetLogEventType::SOCKET_ALIVE));
    EXPECT_TRUE(LogContainsBeginEvent(net_log_entries, 1, NetLogEventType::TCP_CONNECT));
    // Now wait for the client socket to accept the connection.
    if (rv != OK) {
    ...
    EXPECT_TRUE(LogContainsEndEvent(net_log_entries, -1, NetLogEventType::TCP_CONNECT));
...
TEST_F(TransportClientSocketTest, IsConnected)
{
    ...
    bytes_read = DrainStreamSocket(sock_.get(), buf.get(), 4096, strlen(kServerReply) - 1, &callback);
...
TEST_F(TransportClientSocketTest, Read)
{
    ...
    auto buf = base::MakeRefCounted<IOBufferWithSize>(4096);
    uint32_t bytes_read = DrainStreamSocket(sock_.get(), buf.get(), 4096, strlen(kServerReply), &callback);
...
TEST_F(TransportClientSocketTest, FullDuplex_ReadFirst)
{
    ...
    int bytes_written = 0;
    while (true) {
        rv = sock_->Write(request_buffer.get(), kWriteBufLen, write_callback.callback(), TRAFFIC_ANNOTATION_FOR_TESTS);
...
TEST_F(TransportClientSocketTest, FullDuplex_WriteFirst)
{
    ...
    int bytes_written = 0;
    while (true) {
        int rv = sock_->Write(request_buffer.get(), kWriteBufLen, write_callback.callback(), TRAFFIC_ANNOTATION_FOR_TESTS);
        ASSERT_TRUE(rv >= 0 || rv == ERR_IO_PENDING);
