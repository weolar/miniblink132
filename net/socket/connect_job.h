...
struct NET_EXPORT_PRIVATE CommonConnectJobParams {
    ...
    // instead.
    CommonConnectJobParams(ClientSocketFactory* client_socket_factory, HostResolver* host_resolver, HttpAuthCache* http_auth_cache,
...
using OnHostResolutionCallback = base::RepeatingCallback<OnHostResolutionCallbackResult(
...
public:
    ...
    ConnectJob(RequestPriority priority, const SocketTag& socket_tag, base::TimeDelta timeout_duration, const CommonConnectJobParams* common_connect_job_params,
    ...
    virtual std::optional<HostResolverEndpointResult> GetHostResolverEndpointResult() const;
...
protected:
    ...
    void SetSocket(std::unique_ptr<StreamSocket> socket, std::optional<std::set<std::string>> dns_aliases);
    void NotifyDelegateOfCompletion(int rv);
    void NotifyDelegateOfProxyAuth(const HttpResponseInfo& response, HttpAuthController* auth_controller, base::OnceClosure restart_with_auth_callback);
