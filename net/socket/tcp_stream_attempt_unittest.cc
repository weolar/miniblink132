...
public:
    ...
    const std::vector<NetLogEntry>& entries() const
    {
        return entries_;
    }
...
public:
    ...
    std::unique_ptr<SocketPerformanceWatcher> CreateSocketPerformanceWatcher(const Protocol protocol, const IPAddress& ip_address) override
    {
...
public:
    ...
    int Start()
    {
        return attempt_->Start(base::BindOnce(&StreamAttemptHelper::OnComplete, base::Unretained(this)));
    }
...
protected:
    ...
    {
        params_.socket_performance_watcher_factory = &socket_performance_watcher_factory_;
    }

    MockTransportClientSocketFactory& socket_factory()
    {
        return socket_factory_;
    }

    StreamAttemptParams* params()
    {
        return &params_;
    }
...
{
    socket_factory().set_default_client_socket_type(MockTransportClientSocketFactory::Type::kSynchronous);
...
{
    socket_factory().set_default_client_socket_type(MockTransportClientSocketFactory::Type::kPending);
    StreamAttemptHelper helper(params(), MakeIPEndPoint("192.0.2.1"));
    int rv = helper.Start();
...
{
    socket_factory().set_default_client_socket_type(MockTransportClientSocketFactory::Type::kFailing);
    StreamAttemptHelper helper(params(), MakeIPEndPoint("192.0.2.1"));
    int rv = helper.Start();
...
{
    socket_factory().set_default_client_socket_type(MockTransportClientSocketFactory::Type::kPendingFailing);
    StreamAttemptHelper helper(params(), MakeIPEndPoint("192.0.2.1"));
    int rv = helper.Start();
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
...
{
    socket_factory().set_default_client_socket_type(MockTransportClientSocketFactory::Type::kStalled);
    StreamAttemptHelper helper(params(), MakeIPEndPoint("192.0.2.1"));
    int rv = helper.Start();
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
...
{
    socket_factory().set_default_client_socket_type(MockTransportClientSocketFactory::Type::kPending);
    auto helper = std::make_unique<StreamAttemptHelper>(params(), MakeIPEndPoint("192.0.2.1"));
    int rv = helper->Start();
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
    ...
    std::optional<int> error = observer.entries().front().params.FindInt("net_error");
