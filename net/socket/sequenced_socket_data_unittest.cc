...
public:
    explicit ReentrantHelper(StreamSocket* socket)
        : socket_(socket)
    {
    }
    ...
    void SetInvokeRead(scoped_refptr<IOBuffer> read_buf, int read_len, int second_rv, CompletionOnceCallback callback)
    {
        second_read_ = true;
    ...
    void SetInvokeWrite(const char* write_data, int write_len, int second_rv, CompletionOnceCallback callback)
    {
        second_read_ = false;
        second_rv_ = second_rv;
    ...
    {
        return base::BindOnce(&ReentrantHelper::OnIOComplete, base::Unretained(this));
    }
...
private:
    void OnIOComplete(int rv)
    {
        ...
        if (verify_read_) {
            ASSERT_EQ(std::string(first_read_data_, first_len_), std::string(first_read_buf_->data(), rv));
        }

        if (second_read_) {
            ASSERT_EQ(second_rv_, socket_->Read(second_read_buf_.get(), second_len_, std::move(second_callback_)));
        } else {
            auto write_buf = base::MakeRefCounted<IOBufferWithSize>(second_len_);
            memcpy(write_buf->data(), second_write_data_, second_len_);
            ASSERT_EQ(second_rv_, socket_->Write(write_buf.get(), second_len_, std::move(second_callback_), TRAFFIC_ANNOTATION_FOR_TESTS));
        }
    }
...
public:
    ...
    // value.
    void ReentrantReadCallback(const char* data, int len1, int len2, int expected_rv2, int rv);
    ...
    // correct value.
    void ReentrantWriteCallback(int expected_rv1, const char* data, int len, int expected_rv2, int rv);

    // This method is used at the completion callback for an async operation.
    // When executed, verifies that |rv| equals |expected_rv| and then
    // attempts an aync write of |data| with |callback|
    void ReentrantAsyncWriteCallback(const char* data, int len, CompletionOnceCallback callback, int expected_rv, int rv);
...
protected:
    ...
    void set_expect_eof(bool expect_eof)
    {
    ...
    {
        return base::BindOnce(&SequencedSocketDataTest::FailingCompletionCallback, base::Unretained(this));
    }
...
void SequencedSocketDataTest::Initialize(base::span<const MockRead> reads, base::span<const MockWrite> writes)
{
    data_ = std::make_unique<SequencedSocketData>(MockConnect(SYNCHRONOUS, OK), reads, writes);
    socket_factory_.AddSocketDataProvider(data_.get());
    sock_ = socket_factory_.CreateTransportClientSocket(AddressList(IPEndPoint(IPAddress::IPv4Localhost(), 443)), nullptr /* socket_performance_watcher */,
        nullptr /* network_quality_estimator */, nullptr /* net_log */, NetLogSource());
...
void SequencedSocketDataTest::AssertReadBufferEquals(const char* data, int len)
{
...
void SequencedSocketDataTest::AssertAsyncWriteEquals(const char* data, int len)
{
...
void SequencedSocketDataTest::AssertWriteReturns(const char* data, int len, int rv)
{
    auto buf = base::MakeRefCounted<IOBufferWithSize>(len);
    memcpy(buf->data(), data, len);

    if (rv == ERR_IO_PENDING) {
        ASSERT_EQ(rv, sock_->Write(buf.get(), len, write_callback_.callback(), TRAFFIC_ANNOTATION_FOR_TESTS));
        ASSERT_FALSE(write_callback_.have_result());
    } else {
        ASSERT_EQ(rv, sock_->Write(buf.get(), len, failing_callback(), TRAFFIC_ANNOTATION_FOR_TESTS));
    }
}

void SequencedSocketDataTest::ReentrantReadCallback(const char* data, int len1, int len2, int expected_rv2, int rv)
{
    ASSERT_EQ(len1, rv);
...
void SequencedSocketDataTest::ReentrantAsyncReadCallback(int expected_rv, int len, int rv)
{
...
void SequencedSocketDataTest::ReentrantWriteCallback(int expected_rv1, const char* data, int len, int expected_rv2, int rv)
{
...
void SequencedSocketDataTest::ReentrantAsyncWriteCallback(const char* data, int len, CompletionOnceCallback callback, int expected_rv, int rv)
{
    EXPECT_EQ(expected_rv, rv);
    auto write_buf = base::MakeRefCounted<IOBufferWithSize>(len);
    memcpy(write_buf->data(), data, len);
    EXPECT_THAT(sock_->Write(write_buf.get(), len, std::move(callback), TRAFFIC_ANNOTATION_FOR_TESTS), IsError(ERR_IO_PENDING));
}
...
TEST_F(SequencedSocketDataTest, SyncReadFromCompletionCallback)
{
    ...
    ASSERT_EQ(ERR_IO_PENDING,
        sock_->Read(
            read_buf_.get(), kLen1, base::BindOnce(&SequencedSocketDataTest::ReentrantReadCallback, base::Unretained(this), kMsg1, kLen1, kLen2, kLen2)));
...
TEST_F(SequencedSocketDataTest, ManyReentrantReads)
{
    ...
    helper3.SetInvokeRead(read_buf_, kLen4, ERR_IO_PENDING, read_callback_.callback());

    ReentrantHelper helper2(sock_.get());
    helper2.SetExpectedRead(kMsg2, kLen2);
    helper2.SetInvokeRead(helper3.read_buf(), kLen3, ERR_IO_PENDING, helper3.callback());

    ReentrantHelper helper(sock_.get());
    helper.SetExpectedRead(kMsg1, kLen1);
    helper.SetInvokeRead(helper2.read_buf(), kLen2, ERR_IO_PENDING, helper2.callback());
...
TEST_F(SequencedSocketDataTest, AsyncReadFromCompletionCallback)
{
    ...
    ASSERT_EQ(ERR_IO_PENDING,
        sock_->Read(read_buf_.get(), kLen1,
            base::BindOnce(&SequencedSocketDataTest::ReentrantReadCallback, base::Unretained(this), kMsg1, kLen1, kLen2, ERR_IO_PENDING)));
...
TEST_F(SequencedSocketDataTest, SingleSyncReadTooEarly)
{
    ...
    EXPECT_NONFATAL_FAILURE(AssertReadReturns(kLen1, ERR_UNEXPECTED), "Unable to perform synchronous IO while stopped");
...
TEST_F(SequencedSocketDataTest, SingleAsyncReadLargeBuffer)
{
    ...
    auto read_buf = base::MakeRefCounted<IOBufferWithSize>(2 * kLen1);
    ASSERT_EQ(ERR_IO_PENDING, sock_->Read(read_buf.get(), 2 * kLen1, read_callback_.callback()));
...
TEST_F(SequencedSocketDataTest, HangingRead)
{
    ...
    ASSERT_EQ(ERR_IO_PENDING, sock_->Read(read_buf.get(), 1, read_callback_.callback()));
    ASSERT_FALSE(read_callback_.have_result());
...
TEST_F(SequencedSocketDataTest, SingleSyncWriteTooEarly)
{
    ...
    EXPECT_NONFATAL_FAILURE(AssertWriteReturns(kMsg1, kLen1, ERR_UNEXPECTED), "Unable to perform synchronous IO while stopped");
...
TEST_F(SequencedSocketDataTest, SingleSyncWriteTooSmall)
{
    ...
    ASSERT_EQ(std::size(kExpectedFailures), static_cast<size_t>(gtest_failures.size()));
...
TEST_F(SequencedSocketDataTest, SyncWriteFromCompletionCallback)
{
    ...
    ASSERT_EQ(ERR_IO_PENDING,
        sock_->Write(write_buf.get(), kLen1,
            base::BindOnce(&SequencedSocketDataTest::ReentrantWriteCallback, base::Unretained(this), kLen1, kMsg2, kLen2, kLen2),
...
TEST_F(SequencedSocketDataTest, AsyncWriteFromCompletionCallback)
{
    ...
    ASSERT_EQ(ERR_IO_PENDING,
        sock_->Write(write_buf.get(), kLen1,
            base::BindOnce(&SequencedSocketDataTest::ReentrantWriteCallback, base::Unretained(this), kLen1, kMsg2, kLen2, ERR_IO_PENDING),
            TRAFFIC_ANNOTATION_FOR_TESTS));
...
TEST_F(SequencedSocketDataTest, ManyReentrantWrites)
{
    ...
    helper3.SetInvokeWrite(kMsg4, kLen4, ERR_IO_PENDING, write_callback_.callback());
    ...
    auto write_buf = base::MakeRefCounted<IOBufferWithSize>(kLen1);
    memcpy(write_buf->data(), kMsg1, kLen1);
    sock_->Write(write_buf.get(), kLen1, helper.callback(), TRAFFIC_ANNOTATION_FOR_TESTS);
...
TEST_F(SequencedSocketDataTest, AsyncReadFromWriteCompletionCallback)
{
    ...
    memcpy(write_buf->data(), kMsg1, kLen1);
    ASSERT_EQ(ERR_IO_PENDING,
        sock_->Write(write_buf.get(), kLen1, base::BindOnce(&SequencedSocketDataTest::ReentrantAsyncReadCallback, base::Unretained(this), kLen1, kLen2),
            TRAFFIC_ANNOTATION_FOR_TESTS));
...
TEST_F(SequencedSocketDataTest, AsyncWriteFromReadCompletionCallback)
{
    ...
    ASSERT_EQ(ERR_IO_PENDING,
        sock_->Read(read_buf.get(), kLen1,
            base::BindOnce(&SequencedSocketDataTest::ReentrantAsyncWriteCallback, base::Unretained(this), kMsg2, kLen2, write_callback_.callback(), kLen1)));
...
TEST_F(SequencedSocketDataTest, MixedReentrantOperationsThenSynchronousRead)
{
    ...
    auto write_buf = base::MakeRefCounted<IOBufferWithSize>(kLen1);
    memcpy(write_buf->data(), kMsg1, kLen1);
    ASSERT_EQ(ERR_IO_PENDING, sock_->Write(write_buf.get(), kLen1, helper.callback(), TRAFFIC_ANNOTATION_FOR_TESTS));
...
TEST_F(SequencedSocketDataTest, MixedReentrantOperationsThenSynchronousWrite)
{
    ...
    ASSERT_EQ(ERR_IO_PENDING, sock_->Read(helper.read_buf().get(), kLen1, helper.callback()));
