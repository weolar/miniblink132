...
public:
    ...
    {
#if BUILDFLAG(IS_WIN)
        scoped_feature_list_.InitWithFeatureState(features::kTcpSocketIoCompletionPortWin, GetParam());
#else
        CHECK(!GetParam());
    ...
    void CreateConnectedSockets(std::unique_ptr<StreamSocket>* accepted_socket, std::unique_ptr<TCPClientSocket>* client_socket,
        std::unique_ptr<ServerSocket>* server_socket_opt = nullptr)
    {
        IPAddress local_address = IPAddress::IPv4Localhost();

        std::unique_ptr<TCPServerSocket> server_socket = std::make_unique<TCPServerSocket>(nullptr, NetLogSource());
        ...
        *client_socket = std::make_unique<TCPClientSocket>(AddressList(server_address), nullptr, nullptr, nullptr, NetLogSource());

        EXPECT_THAT((*client_socket)->Bind(IPEndPoint(local_address, 0)), IsOk());

        IPEndPoint local_address_result;
        EXPECT_THAT((*client_socket)->GetLocalAddress(&local_address_result), IsOk());
        ...
        int result = server_socket->Accept(accepted_socket, accept_callback.callback());
...
TEST_P(TCPClientSocketTest, BindLoopbackToLoopback)
{
    ...
    TCPClientSocket socket(AddressList(server_address), nullptr, nullptr, nullptr, NetLogSource());
    ...
    EXPECT_EQ(ERR_SOCKET_NOT_CONNECTED, socket.GetLocalAddress(&local_address_result));
}
...
TEST_P(TCPClientSocketTest, BindLoopbackToExternal)
{
    ...
    TCPClientSocket socket(AddressList::CreateFromIPAddress(external_ip, 80), nullptr, nullptr, nullptr, NetLogSource());
...
TEST_P(TCPClientSocketTest, WasEverUsed)
{
    IPAddress lo_address = IPAddress::IPv4Localhost();
    TCPServerSocket server(nullptr, NetLogSource());
    ASSERT_THAT(server.Listen(IPEndPoint(lo_address, 0), 1, /*ipv6_only=*/std::nullopt), IsOk());
    IPEndPoint server_address;
    ASSERT_THAT(server.GetLocalAddress(&server_address), IsOk());
    ...
    result = socket.Write(write_buffer.get(), write_buffer->size(), write_callback.callback(), TRAFFIC_ANNOTATION_FOR_TESTS);
...
TEST_P(TCPClientSocketTest, DnsAliasesPersistForReuse)
{
    ...
    EXPECT_THAT(socket.GetDnsAliases(), testing::UnorderedElementsAre("alias1", "alias2", "host"));
    ...
    connect_result = socket.Connect(connect_callback2.callback());
    EXPECT_FALSE(socket.WasEverUsed());
    EXPECT_THAT(socket.GetDnsAliases(), testing::ElementsAre("alias1", "alias2", "host"));
}
...
TEST_P(TCPClientSocketTest, MAYBE_TestSocketPerformanceWatcher)
{
    ...
    TCPClientSocket socket(AddressList::CreateFromIPAddressList(ip_list, std::move(aliases)), std::move(watcher), nullptr, nullptr, NetLogSource());
...
TEST_P(TCPClientSocketTest, Tag)
{
    ...
    EXPECT_EQ(s.Write(write_buffer1.get(), strlen(kRequest1), write_callback1.callback(), TRAFFIC_ANNOTATION_FOR_TESTS), static_cast<int>(strlen(kRequest1)));
    ...
    scoped_refptr<IOBufferWithSize> write_buffer2 = base::MakeRefCounted<IOBufferWithSize>(strlen(kRequest2));
    memmove(write_buffer2->data(), kRequest2, strlen(kRequest2));
    TestCompletionCallback write_callback2;
    EXPECT_EQ(s.Write(write_buffer2.get(), strlen(kRequest2), write_callback2.callback(), TRAFFIC_ANNOTATION_FOR_TESTS), static_cast<int>(strlen(kRequest2)));
    EXPECT_GT(GetTaggedBytes(tag_val1), old_traffic);
...
public:
    NeverConnectingTCPClientSocket(const AddressList& addresses, std::unique_ptr<SocketPerformanceWatcher> socket_performance_watcher,
        NetworkQualityEstimator* network_quality_estimator, net::NetLog* net_log, const net::NetLogSource& source)
        : TCPClientSocket(addresses, std::move(socket_performance_watcher), network_quality_estimator, net_log, source)
    {
    }

    // Returns the number of times that ConnectInternal() was called.
    int connect_internal_counter() const
    {
        return connect_internal_counter_;
    }
...
TEST_P(TCPClientSocketTest, SuspendBeforeConnect)
{
    ...
    TestCompletionCallback accept_callback;
    std::unique_ptr<StreamSocket> accepted_socket;
    ASSERT_THAT(server.Accept(&accepted_socket, accept_callback.callback()), IsError(ERR_IO_PENDING));
...
TEST_P(TCPClientSocketTest, SuspendDuringConnect)
{
    ...
    NeverConnectingTCPClientSocket socket(AddressList(server_address), nullptr, nullptr, nullptr, NetLogSource());
    ...
    Suspend();
    EXPECT_THAT(connect_callback.WaitForResult(), IsError(ERR_NETWORK_IO_SUSPENDED));
}
...
TEST_P(TCPClientSocketTest, SuspendDuringConnectMultipleAddresses)
{
    ...
    address_list.push_back(IPEndPoint(IPAddress(127, 0, 0, 1), server_address.port()));
    address_list.push_back(IPEndPoint(IPAddress(127, 0, 0, 2), server_address.port()));
    NeverConnectingTCPClientSocket socket(address_list, nullptr, nullptr, nullptr, NetLogSource());
...
TEST_P(TCPClientSocketTest, SuspendWhileIdle)
{
    ...
    EXPECT_THAT(client_socket->Read(buffer.get(), 1, callback.callback()), IsError(ERR_NETWORK_IO_SUSPENDED));
    EXPECT_THAT(client_socket->Write(buffer.get(), 1, callback.callback(), TRAFFIC_ANNOTATION_FOR_TESTS), IsError(ERR_NETWORK_IO_SUSPENDED));

    // Check that the accepted socket is disconnected, and actions fail with
    // ERR_NETWORK_IO_SUSPENDED.
    EXPECT_FALSE(accepted_socket->IsConnected());
    EXPECT_THAT(accepted_socket->Read(buffer.get(), 1, callback.callback()), IsError(ERR_NETWORK_IO_SUSPENDED));
    EXPECT_THAT(accepted_socket->Write(buffer.get(), 1, callback.callback(), TRAFFIC_ANNOTATION_FOR_TESTS), IsError(ERR_NETWORK_IO_SUSPENDED));
    ...
    TestCompletionCallback accept_callback;
    int accept_result = server_socket->Accept(&accepted_socket, accept_callback.callback());
...
TEST_P(TCPClientSocketTest, SuspendDuringRead)
{
    ...
    TestCompletionCallback callback;
    ASSERT_THAT(client_socket->Read(read_buffer.get(), 1, callback.callback()), IsError(ERR_IO_PENDING));
    ...
    EXPECT_FALSE(client_socket->IsConnected());
    EXPECT_THAT(client_socket->Read(read_buffer.get(), 1, callback.callback()), IsError(ERR_NETWORK_IO_SUSPENDED));
    EXPECT_THAT(client_socket->Write(read_buffer.get(), 1, callback.callback(), TRAFFIC_ANNOTATION_FOR_TESTS), IsError(ERR_NETWORK_IO_SUSPENDED));
}
...
TEST_P(TCPClientSocketTest, SuspendDuringWrite)
{
    ...
    TestCompletionCallback callback;
    while (true) {
        int rv = client_socket->Write(write_buffer.get(), kBufferSize, callback.callback(), TRAFFIC_ANNOTATION_FOR_TESTS);
    ...
    // Check that the client socket really is disconnected.
    EXPECT_FALSE(client_socket->IsConnected());
    EXPECT_THAT(client_socket->Read(write_buffer.get(), 1, callback.callback()), IsError(ERR_NETWORK_IO_SUSPENDED));
    EXPECT_THAT(client_socket->Write(write_buffer.get(), 1, callback.callback(), TRAFFIC_ANNOTATION_FOR_TESTS), IsError(ERR_NETWORK_IO_SUSPENDED));
}
...
TEST_P(TCPClientSocketTest, SuspendDuringReadAndWrite)
{
    ...
    for (ReadCallbackAction read_callback_action : {
        ...
        CompletionOnceCallback read_completion_once_callback = base::BindLambdaForTesting([&](int result) {
            EXPECT_FALSE(client_socket->IsConnected());
            ...
            case ReadCallbackAction::kReconnectSocket: {
                TestCompletionCallback connect_callback;
                nested_connect_result = client_socket->Connect(nested_connect_callback.callback());
                break;
            }
            }
            read_callback.callback().Run(result);
        });
        ASSERT_THAT(client_socket->Read(read_buffer.get(), 1, std::move(read_completion_once_callback)), IsError(ERR_IO_PENDING));
        ...
        TestCompletionCallback write_callback;
        while (true) {
            int rv = client_socket->Write(write_buffer.get(), kBufferSize, write_callback.callback(), TRAFFIC_ANNOTATION_FOR_TESTS);
            if (rv == ERR_IO_PENDING)
                break;
            ASSERT_GT(rv, 0);
        }
        ...
        client_socket->OnSuspend();
        EXPECT_THAT(read_callback.WaitForResult(), IsError(ERR_NETWORK_IO_SUSPENDED));
        if (read_callback_action == ReadCallbackAction::kNone) {
            EXPECT_THAT(write_callback.WaitForResult(), IsError(ERR_NETWORK_IO_SUSPENDED));

            // Check that the client socket really is disconnected.
            EXPECT_FALSE(client_socket->IsConnected());
            EXPECT_THAT(client_socket->Read(read_buffer.get(), 1, read_callback.callback()), IsError(ERR_NETWORK_IO_SUSPENDED));
            EXPECT_THAT(
                client_socket->Write(write_buffer.get(), 1, write_callback.callback(), TRAFFIC_ANNOTATION_FOR_TESTS), IsError(ERR_NETWORK_IO_SUSPENDED));
        } else {
        ...
        if (read_callback_action == ReadCallbackAction::kReconnectSocket) {
            ...
            int accept_result = server_socket->Accept(&accepted_socket, accept_callback.callback());
            ASSERT_THAT(accept_callback.GetResult(accept_result), IsOk());
            EXPECT_THAT(nested_connect_callback.GetResult(nested_connect_result), IsOk());
        }
    }
}
...
INSTANTIATE_TEST_SUITE_P(Any, TCPClientSocketTest,
...
public:
    OverrideTcpConnectAttemptTimeout(double rtt_multipilier, base::TimeDelta min_timeout, base::TimeDelta max_timeout)
    {
        base::FieldTrialParams params;
        params[features::kTimeoutTcpConnectAttemptRTTMultiplier.name] = base::NumberToString(rtt_multipilier);
        params[features::kTimeoutTcpConnectAttemptMin.name] = base::NumberToString(min_timeout.InMilliseconds()) + "ms";
        params[features::kTimeoutTcpConnectAttemptMax.name] = base::NumberToString(max_timeout.InMilliseconds()) + "ms";

        scoped_feature_list_.InitAndEnableFeatureWithParameters(features::kTimeoutTcpConnectAttempt, params);
    }
...
{
    OverrideTcpConnectAttemptTimeout override_timeout(1, base::Seconds(4), base::Seconds(10));
...
{
    OverrideTcpConnectAttemptTimeout override_timeout(5, base::Seconds(4), base::Seconds(10));
    ...
    NeverConnectingTCPClientSocket socket(AddressList(server_address), nullptr, &network_quality_estimator, nullptr, NetLogSource());
...
TEST_F(TCPClientSocketMockTimeTest, ConnectAttemptTimeoutIndependent)
{
    ...
    NeverConnectingTCPClientSocket socket(addresses, nullptr, nullptr, nullptr, NetLogSource());
