...
TEST_F(UnixDomainServerSocketTest, ListenWithInvalidPath)
{
    ...
    EXPECT_EQ(ERR_FILE_NOT_FOUND, server_socket.BindAndListen(kInvalidSocketPath, /*backlog=*/1));
}
...
TEST_F(UnixDomainServerSocketTest, ListenWithInvalidPathWithAbstractNamespace)
{
    ...
    EXPECT_THAT(server_socket.BindAndListen(kInvalidSocketPath, /*backlog=*/1), IsOk());
#else
    EXPECT_EQ(ERR_ADDRESS_INVALID, server_socket.BindAndListen(kInvalidSocketPath, /*backlog=*/1));
#endif
}
...
TEST_F(UnixDomainServerSocketTest, AcceptWithForbiddenUser)
{
    ...
    UnixDomainServerSocket server_socket(CreateAuthCallback(false), kUseAbstractNamespace);
    EXPECT_THAT(server_socket.BindAndListen(socket_path_, /*backlog=*/1), IsOk());
    ...
    int rv = connect_callback.GetResult(client_socket.Connect(connect_callback.callback()));
    ASSERT_THAT(rv, IsOk());
    ...
    rv = read_callback.GetResult(client_socket.Read(read_buffer.get(), read_buffer_size, read_callback.callback()));
...
TEST_F(UnixDomainServerSocketTest, UnimplementedMethodsFail)
{
    ...
    IPEndPoint ep;
    EXPECT_THAT(server_socket.Listen(ep, 0, /*ipv6_only=*/std::nullopt), IsError(ERR_NOT_IMPLEMENTED));
    EXPECT_EQ(ERR_NOT_IMPLEMENTED,
        server_socket.ListenWithAddressAndPort(kInvalidSocketPath, 0,
            /*backlog=*/1));
