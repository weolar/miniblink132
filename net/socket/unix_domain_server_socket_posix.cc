...
UnixDomainServerSocket::UnixDomainServerSocket(const AuthCallback& auth_callback, bool use_abstract_namespace)
...
// static
bool UnixDomainServerSocket::GetPeerCredentials(SocketDescriptor socket, Credentials* credentials)
{
...
int UnixDomainServerSocket::Listen(const IPEndPoint& address, int backlog, std::optional<bool> ipv6_only)
{
    NOTIMPLEMENTED();
    return ERR_NOT_IMPLEMENTED;
}

int UnixDomainServerSocket::ListenWithAddressAndPort(const std::string& address_string, uint16_t port, int backlog)
{
    NOTIMPLEMENTED();
    return ERR_NOT_IMPLEMENTED;
}

int UnixDomainServerSocket::BindAndListen(const std::string& socket_path, int backlog)
{
    ...
    DCHECK_NE(ERR_IO_PENDING, rv);
    if (rv != OK) {
        PLOG(ERROR) << "Could not bind unix domain socket to " << socket_path << (use_abstract_namespace_ ? " (with abstract namespace)" : "");
        return rv;
    }
...
int UnixDomainServerSocket::Accept(std::unique_ptr<StreamSocket>* socket, CompletionOnceCallback callback)
{
    DCHECK(socket);
    DCHECK(callback);
...
int UnixDomainServerSocket::AcceptSocketDescriptor(SocketDescriptor* socket, CompletionOnceCallback callback)
{
    DCHECK(socket);
    DCHECK(callback);
    DCHECK(!callback_ && !out_socket_.stream && !out_socket_.descriptor);
...
int UnixDomainServerSocket::DoAccept()
{
    ...
    while (true) {
        int rv = listen_socket_->Accept(&accept_socket_, base::BindOnce(&UnixDomainServerSocket::AcceptCompleted, base::Unretained(this)));
        if (rv != OK)
            return rv;
...
void UnixDomainServerSocket::SetSocketResult(std::unique_ptr<SocketPosix> accepted_socket)
{
    ...
    *out_socket_.stream = std::make_unique<UnixDomainClientSocket>(std::move(accepted_socket));
}
