...
base::Value::Dict NetLogPrivateKeyOperationParams(uint16_t algorithm, SSLPrivateKey* key)
{
    return base::Value::Dict().Set("algorithm", SSL_get_signature_algorithm_name(algorithm, 0 /* exclude curve */)).Set("provider", key->GetProviderName());
}
...
base::Value::Dict NetLogSSLInfoParams(SSLClientSocketImpl* socket)
{
    ...
    SSLVersionToString(&version_str, SSLConnectionStatusToVersion(ssl_info.connection_status));
    return base::Value::Dict()
        .Set("version", version_str)
        .Set("is_resumed", ssl_info.handshake_type == SSLInfo::HANDSHAKE_RESUME)
        .Set("cipher_suite", SSLConnectionStatusToCipherSuite(ssl_info.connection_status))
...
base::Value::Dict NetLogSSLMessageParams(bool is_write, const void* bytes, size_t len, NetLogCaptureMode capture_mode)
{
    if (len == 0) {
        NOTREACHED();
    }
    ...
    if (!is_write || type != SSL3_MT_CERTIFICATE || NetLogCaptureIncludesSocketBytes(capture_mode)) {
...
public:
    ...
    {
        return base::Singleton<SSLContext, base::LeakySingletonTraits<SSLContext>>::get();
    }
    SSL_CTX* ssl_ctx()
    {
        return ssl_ctx_.get();
    ...
    {
        DCHECK(ssl);
        SSLClientSocketImpl* socket = static_cast<SSLClientSocketImpl*>(SSL_get_ex_data(ssl, ssl_socket_data_index_));
        DCHECK(socket);
        return socket;
    }
    ...
    void SetSSLKeyLogger(std::unique_ptr<SSLKeyLogger> logger)
    {
        ...
        SSL_CTX_set_keylog_callback(ssl_ctx_.get(), SSLKeyLoggerManager::KeyLogCallback);
    }
...
private:
    ...
    SSLContext()
    {
        ...
        SSL_CTX_set_session_cache_mode(ssl_ctx_.get(), SSL_SESS_CACHE_CLIENT | SSL_SESS_CACHE_NO_INTERNAL);
    ...
    static ssl_private_key_result_t PrivateKeySignCallback(
        SSL* ssl, uint8_t* out, size_t* out_len, size_t max_out, uint16_t algorithm, const uint8_t* in, size_t in_len)
    {
        SSLClientSocketImpl* socket = GetInstance()->GetClientSocketFromSSL(ssl);
        return socket->PrivateKeySignCallback(out, out_len, max_out, algorithm, in, in_len);
    }

    static ssl_private_key_result_t PrivateKeyCompleteCallback(SSL* ssl, uint8_t* out, size_t* out_len, size_t max_out)
    {
        SSLClientSocketImpl* socket = GetInstance()->GetClientSocketFromSSL(ssl);
...
const SSL_PRIVATE_KEY_METHOD SSLClientSocketImpl::SSLContext::kPrivateKeyMethod = {
...
void SSLClientSocketImpl::SetSSLKeyLogger(std::unique_ptr<SSLKeyLogger> logger)
{
...
int SSLClientSocketImpl::ExportKeyingMaterial(std::string_view label, bool has_context, std::string_view context, unsigned char* out, unsigned int outlen)
{
    if (!IsConnected())
    ...
    if (!SSL_export_keying_material(ssl_.get(), out, outlen, label.data(), label.size(), reinterpret_cast<const unsigned char*>(context.data()),
...
std::optional<std::string_view> SSLClientSocketImpl::GetPeerApplicationSettings() const
{
...
bool SSLClientSocketImpl::GetSSLInfo(SSLInfo* ssl_info)
{
    ...
    ssl_info->peer_signature_algorithm = SSL_get_peer_signature_algorithm(ssl_.get());

    SSLConnectionStatusSetCipherSuite(SSL_CIPHER_get_protocol_id(cipher), &ssl_info->connection_status);
    SSLConnectionStatusSetVersion(GetNetSSLVersion(ssl_.get()), &ssl_info->connection_status);

    ssl_info->handshake_type = SSL_session_reused(ssl_.get()) ? SSLInfo::HANDSHAKE_RESUME : SSLInfo::HANDSHAKE_FULL;
...
void SSLClientSocketImpl::GetSSLCertRequestInfo(SSLCertRequestInfo* cert_request_info) const
{
    if (!ssl_) {
        NOTREACHED();
    }
    ...
    const STACK_OF(CRYPTO_BUFFER)* authorities = SSL_get0_server_requested_CAs(ssl_.get());
    for (const CRYPTO_BUFFER* ca_name : authorities) {
        cert_request_info->cert_authorities.emplace_back(reinterpret_cast<const char*>(CRYPTO_BUFFER_data(ca_name)), CRYPTO_BUFFER_len(ca_name));
    }

    const uint16_t* algorithms;
    size_t num_algorithms = SSL_get0_peer_verify_algorithms(ssl_.get(), &algorithms);
    cert_request_info->signature_algorithms.assign(algorithms, algorithms + num_algorithms);
}
...
int SSLClientSocketImpl::Read(IOBuffer* buf, int buf_len, CompletionOnceCallback callback)
{
...
int SSLClientSocketImpl::ReadIfReady(IOBuffer* buf, int buf_len, CompletionOnceCallback callback)
{
...
int SSLClientSocketImpl::Write(IOBuffer* buf, int buf_len, CompletionOnceCallback callback, const NetworkTrafficAnnotationTag& traffic_annotation)
{
    user_write_buf_ = buf;
...
int SSLClientSocketImpl::Init()
{
    ...
    const bool host_is_ip_address = HostIsIPAddressNoBrackets(host_and_port_.host());
    ...
    if (!host_is_ip_address && !SSL_set_tlsext_host_name(ssl_.get(), host_and_port_.host().c_str())) {
        return ERR_UNEXPECTED;
    }

    if (context_->config().PostQuantumKeyAgreementEnabled()) {
        const uint16_t postquantum_group = base::FeatureList::IsEnabled(features::kUseMLKEM) ? SSL_GROUP_X25519_MLKEM768 : SSL_GROUP_X25519_KYBER768_DRAFT00;
    ...
    if (IsCachingEnabled()) {
        bssl::UniquePtr<SSL_SESSION> session = context_->ssl_client_session_cache()->Lookup(GetSessionCacheKey(/*dest_ip_addr=*/std::nullopt));
        if (!session) {
            ...
            if (stream_socket_->GetPeerAddress(&peer_address) == OK) {
                session = context_->ssl_client_session_cache()->Lookup(GetSessionCacheKey(peer_address.address()));
            }
        }
        if (session)
    ...
    transport_adapter_ = std::make_unique<SocketBIOAdapter>(stream_socket_.get(), kDefaultOpenSSLBufferSize, kDefaultOpenSSLBufferSize, this);
    ...
    uint16_t version_min = ssl_config_.version_min_override.value_or(context_->config().version_min);
    uint16_t version_max = ssl_config_.version_max_override.value_or(context_->config().version_max);
    if (version_min < TLS1_2_VERSION || version_max < TLS1_2_VERSION) {
        // TLS versions before TLS 1.2 are no longer supported.
        return ERR_UNEXPECTED;
    }

    if (!SSL_set_min_proto_version(ssl_.get(), version_min) || !SSL_set_max_proto_version(ssl_.get(), version_max)) {
        return ERR_UNEXPECTED;
    }
    ...
    SSL_set_alps_use_new_codepoint(ssl_.get(), base::FeatureList::IsEnabled(features::kUseNewAlpsCodepointHttp2));
        ...
        for (NextProto proto : ssl_config_.alpn_protos) {
            ...
            if (iter != ssl_config_.application_settings.end()) {
                ...
                if (!SSL_add_application_settings(
                        ssl_.get(), reinterpret_cast<const uint8_t*>(proto_string), strlen(proto_string), iter->second.data(), iter->second.size())) {
                    return ERR_UNEXPECTED;
                }
            }
        }
    ...
    if (ssl_config_.privacy_mode == PRIVACY_MODE_ENABLED_WITHOUT_CLIENT_CERTS) {
        ...
        send_client_cert_ = context_->GetClientCertificate(host_and_port_, &client_cert_, &client_private_key_);
    }
    ...
    if (!ssl_config_.ech_config_list.empty()) {
        ...
        net_log_.AddEvent(
            NetLogEventType::SSL_ECH_CONFIG_LIST, [&] { return base::Value::Dict().Set("bytes", NetLogBinaryValue(ssl_config_.ech_config_list)); });
        if (!SSL_set1_ech_config_list(ssl_.get(), ssl_config_.ech_config_list.data(), ssl_config_.ech_config_list.size())) {
...
int SSLClientSocketImpl::DoHandshake()
{
    ...
    if (rv <= 0) {
        ...
        LOG(ERROR) << "handshake failed; returned " << rv << ", SSL error code " << ssl_error << ", net_error " << net_error;
        NetLogOpenSSLError(net_log_, NetLogEventType::SSL_HANDSHAKE_ERROR, net_error, ssl_error, error_info);
    }
...
int SSLClientSocketImpl::DoHandshakeComplete(int result)
{
    ...
    if (SSL_version(ssl_.get()) < TLS1_3_VERSION) {
        ...
        if (SSL_in_early_data(ssl_.get())) {
            ...
            details = used_hello_retry_request ? SSLHandshakeDetails::kTLS13ResumeWithHelloRetryRequest : SSLHandshakeDetails::kTLS13Resume;
        } else {
            details = used_hello_retry_request ? SSLHandshakeDetails::kTLS13FullWithHelloRetryRequest : SSLHandshakeDetails::kTLS13Full;
        }
    }
    ...
    base::UmaHistogramBoolean("Net.SSLRenegotiationInfoSupported", SSL_get_secure_renegotiation_support(ssl_.get()));
    base::UmaHistogramBoolean("Net.SSLExtendedMainSecretSupported", SSL_get_extms_support(ssl_.get()));
    ...
    base::SequencedTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(&SSLClientSocketImpl::DoPeek, weak_factory_.GetWeakPtr()));

    return OK;
}

ssl_verify_result_t SSLClientSocketImpl::VerifyCertCallback(SSL* ssl, uint8_t* out_alert)
{
    SSLClientSocketImpl* socket = SSLContext::GetInstance()->GetClientSocketFromSSL(ssl);
    DCHECK(socket);
...
ssl_verify_result_t SSLClientSocketImpl::VerifyCert()
{
    ...
    CHECK(!server_cert_);
    server_cert_ = x509_util::CreateX509CertificateFromBuffers(SSL_get0_peer_certificates(ssl_.get()));
    ...
    net_log_.AddEvent(
        NetLogEventType::SSL_CERTIFICATES_RECEIVED, [&] { return base::Value::Dict().Set("certificates", NetLogX509CertificateList(server_cert_.get())); });
    ...
    size_t ocsp_response_len;
    SSL_get0_ocsp_response(ssl_.get(), &ocsp_response_raw, &ocsp_response_len);
    std::string_view ocsp_response(reinterpret_cast<const char*>(ocsp_response_raw), ocsp_response_len);

    const uint8_t* sct_list_raw;
    size_t sct_list_len;
    SSL_get0_signed_cert_timestamp_list(ssl_.get(), &sct_list_raw, &sct_list_len);
    std::string_view sct_list(reinterpret_cast<const char*>(sct_list_raw), sct_list_len);

    cert_verification_result_
        = context_->cert_verifier()->Verify(CertVerifier::RequestParams(server_cert_, ech_name_override.empty() ? host_and_port_.host() : ech_name_override,
                                                ssl_config_.GetCertVerifyFlags(), std::string(ocsp_response), std::string(sct_list)),
            &server_cert_verify_result_, base::BindOnce(&SSLClientSocketImpl::OnVerifyComplete, base::Unretained(this)), &cert_verifier_request_, net_log_);
...
ssl_verify_result_t SSLClientSocketImpl::HandleVerifyResult()
{
    ...
    if (result == OK) {
        ...
        TransportSecurityState::PKPStatus pin_validity = context_->transport_security_state()->CheckPublicKeyPins(
            host_and_port_, server_cert_verify_result_.is_issued_by_known_root, server_cert_verify_result_.public_key_hashes);
        switch (pin_validity) {
    ...
    is_fatal_cert_error_ = IsCertStatusError(server_cert_verify_result_.cert_status) && result != ERR_CERT_KNOWN_INTERCEPTION_BLOCKED
        && context_->transport_security_state()->ShouldSSLErrorsBeFatal(host_and_port_.host());
...
int SSLClientSocketImpl::CheckCTRequirements()
{
    TransportSecurityState::CTRequirementsStatus ct_requirement_status
        = context_->transport_security_state()->CheckCTRequirements(host_and_port_, server_cert_verify_result_.is_issued_by_known_root,
            server_cert_verify_result_.public_key_hashes, server_cert_verify_result_.verified_cert.get(), server_cert_verify_result_.policy_compliance);

    if (context_->sct_auditing_delegate()) {
        context_->sct_auditing_delegate()->MaybeEnqueueReport(host_and_port_, server_cert_verify_result_.verified_cert.get(), server_cert_verify_result_.scts);
    }
...
int SSLClientSocketImpl::DoPayloadRead(IOBuffer* buf, int buf_len)
{
    ...
    if (pending_read_error_ != kSSLClientSocketNoPendingResult) {
        ...
        pending_read_error_ = kSSLClientSocketNoPendingResult;
        if (rv == 0) {
            net_log_.AddByteTransferEvent(NetLogEventType::SSL_SOCKET_BYTES_RECEIVED, rv, buf->data());
        } else {
            NetLogOpenSSLError(net_log_, NetLogEventType::SSL_READ_ERROR, rv, pending_read_ssl_error_, pending_read_error_info_);
        }
    ...
    do {
        ...
        // synchronously.
    } while (ssl_err == SSL_ERROR_WANT_RENEGOTIATE || (total_bytes_read < buf_len && ssl_ret > 0 && transport_adapter_->HasPendingReadData()));
    ...
    if (ssl_ret <= 0) {
        ...
        if (pending_read_ssl_error_ == SSL_ERROR_ZERO_RETURN) {
            ...
            pending_read_error_ = MapLastOpenSSLError(pending_read_ssl_error_, err_tracer, &pending_read_error_info_);
        }
...
int SSLClientSocketImpl::DoPayloadWrite()
{
    ...
    if (rv >= 0) {
        ...
        net_log_.AddByteTransferEvent(NetLogEventType::SSL_SOCKET_BYTES_SENT, rv, user_write_buf_->data());
        return rv;
    }
    ...
    if (net_error != ERR_IO_PENDING) {
        NetLogOpenSSLError(net_log_, NetLogEventType::SSL_WRITE_ERROR, net_error, ssl_error, error_info);
    }
    return net_error;
}
...
void SSLClientSocketImpl::DoPeek()
{
    ...
    if (ssl_config_.early_data_enabled && !handled_early_data_result_) {
        ...
        ssl_early_data_reason_t early_data_reason = SSL_get_early_data_reason(ssl_.get());
        UMA_HISTOGRAM_ENUMERATION("Net.SSLHandshakeEarlyDataReason", early_data_reason, ssl_early_data_reason_max_value + 1);
        if (IsGoogleHost(host_and_port_.host())) {
            ...
            UMA_HISTOGRAM_ENUMERATION("Net.SSLHandshakeEarlyDataReason.Google", early_data_reason, ssl_early_data_reason_max_value + 1);
        }
        ...
        if (err == ERR_EARLY_DATA_REJECTED || err == ERR_WRONG_VERSION_ON_EARLY_DATA) {
            context_->ssl_client_session_cache()->ClearEarlyData(GetSessionCacheKey(std::nullopt));
        }
...
int SSLClientSocketImpl::ClientCertRequestCallback(SSL* ssl)
{
    ...
    if (client_cert_.get()) {
        ...
        if (!SetSSLChainAndKey(ssl_.get(), client_cert_.get(), nullptr, &SSLContext::kPrivateKeyMethod)) {
            OpenSSLPutNetError(FROM_HERE, ERR_SSL_CLIENT_AUTH_CERT_BAD_FORMAT);
            return -1;
        }

        std::vector<uint16_t> preferences = client_private_key_->GetAlgorithmPreferences();
        ...
        if (base::FeatureList::IsEnabled(features::kLegacyPKCS1ForTLS13) && base::Contains(preferences, SSL_SIGN_RSA_PKCS1_SHA256)) {
            preferences.push_back(SSL_SIGN_RSA_PKCS1_SHA256_LEGACY);
        }
        SSL_set_signing_algorithm_prefs(ssl_.get(), preferences.data(), preferences.size());

        net_log_.AddEventWithIntParams(
            NetLogEventType::SSL_CLIENT_CERT_PROVIDED, "cert_count", base::checked_cast<int>(1 + client_cert_->intermediate_buffers().size()));
        return 1;
    }
#endif // !BUILDFLAG(ENABLE_CLIENT_CERTIFICATES)

    // Send no client certificate.
    net_log_.AddEventWithIntParams(NetLogEventType::SSL_CLIENT_CERT_PROVIDED, "cert_count", 0);
    return 1;
}
...
int SSLClientSocketImpl::NewSessionCallback(SSL_SESSION* session)
{
    ...
    context_->ssl_client_session_cache()->Insert(GetSessionCacheKey(ip_addr), bssl::UniquePtr<SSL_SESSION>(session));
    return 1;
}

SSLClientSessionCache::Key SSLClientSocketImpl::GetSessionCacheKey(std::optional<IPAddress> dest_ip_addr) const
{
...
ssl_private_key_result_t SSLClientSocketImpl::PrivateKeySignCallback(
    uint8_t* out, size_t* out_len, size_t max_out, uint16_t algorithm, const uint8_t* in, size_t in_len)
{
    ...
    net_log_.BeginEvent(NetLogEventType::SSL_PRIVATE_KEY_OP, [&] {
        return NetLogPrivateKeyOperationParams(algorithm,
    ...
    if (base::FeatureList::IsEnabled(features::kLegacyPKCS1ForTLS13) && algorithm == SSL_SIGN_RSA_PKCS1_SHA256_LEGACY) {
        algorithm = SSL_SIGN_RSA_PKCS1_SHA256;
    }

    signature_result_ = ERR_IO_PENDING;
    client_private_key_->Sign(algorithm, base::make_span(in, in_len), base::BindOnce(&SSLClientSocketImpl::OnPrivateKeyComplete, weak_factory_.GetWeakPtr()));
    return ssl_private_key_retry;
}

ssl_private_key_result_t SSLClientSocketImpl::PrivateKeyCompleteCallback(uint8_t* out, size_t* out_len, size_t max_out)
{
    DCHECK_NE(kSSLClientSocketNoPendingResult, signature_result_);
    DCHECK(client_private_key_);
...
void SSLClientSocketImpl::OnPrivateKeyComplete(Error error, const std::vector<uint8_t>& signature)
{
...
void SSLClientSocketImpl::MessageCallback(int is_write, int content_type, const void* buf, size_t len)
{
    switch (content_type) {
    case SSL3_RT_ALERT:
        net_log_.AddEvent(is_write ? NetLogEventType::SSL_ALERT_SENT : NetLogEventType::SSL_ALERT_RECEIVED, [&] { return NetLogSSLAlertParams(buf, len); });
        break;
    case SSL3_RT_HANDSHAKE:
        net_log_.AddEvent(is_write ? NetLogEventType::SSL_HANDSHAKE_MESSAGE_SENT : NetLogEventType::SSL_HANDSHAKE_MESSAGE_RECEIVED,
            [&](NetLogCaptureMode capture_mode) { return NetLogSSLMessageParams(!!is_write, buf, len, capture_mode); });
        break;
...
void SSLClientSocketImpl::LogConnectEndEvent(int rv)
{
    ...
    net_log_.EndEvent(NetLogEventType::SSL_CONNECT, [&] { return NetLogSSLInfoParams(this); });
}

void SSLClientSocketImpl::RecordNegotiatedProtocol() const
{
    UMA_HISTOGRAM_ENUMERATION("Net.SSLNegotiatedAlpnProtocol", negotiated_protocol_, kProtoLast + 1);
}

int SSLClientSocketImpl::MapLastOpenSSLError(int ssl_error, const crypto::OpenSSLErrStackTracer& tracer, OpenSSLErrorInfo* info)
{
    int net_error = MapOpenSSLErrorWithDetails(ssl_error, tracer, info);

    if (ssl_error == SSL_ERROR_SSL && ERR_GET_LIB(info->error_code) == ERR_LIB_SSL) {
        ...
        if (ERR_GET_REASON(info->error_code) == SSL_R_SSLV3_ALERT_HANDSHAKE_FAILURE && certificate_requested_ && send_client_cert_ && !client_cert_) {
        ...
        if (ERR_GET_REASON(info->error_code) == SSL_R_TLSV1_ALERT_ACCESS_DENIED && !certificate_requested_) {
            net_error = ERR_SSL_PROTOCOL_ERROR;
        }

        // This error is specific to the client, so map it here.
        if (ERR_GET_REASON(info->error_code) == SSL_R_NO_COMMON_SIGNATURE_ALGORITHMS) {
...
bool SSLClientSocketImpl::IsAllowedBadCert(X509Certificate* cert, CertStatus* cert_status) const
{
    if (!GetECHNameOverride().empty()) {
        // Certificate exceptions are only applicable for the origin name. For
        // simplicity, we do not allow certificate exceptions for the public name.
