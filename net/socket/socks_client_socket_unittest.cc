...
public:
    ...
    std::unique_ptr<SOCKSClientSocket> BuildMockSocket(base::span<const MockRead> reads, base::span<const MockWrite> writes, HostResolver* host_resolver,
        const std::string& hostname, int port, NetLog* net_log);
    void SetUp() override;
...
std::unique_ptr<SOCKSClientSocket> SOCKSClientSocketTest::BuildMockSocket(
    base::span<const MockRead> reads, base::span<const MockWrite> writes, HostResolver* host_resolver, const std::string& hostname, int port, NetLog* net_log)
{
    TestCompletionCallback callback;
    data_ = std::make_unique<StaticSocketDataProvider>(reads, writes);
    auto socket = std::make_unique<MockTCPClientSocket>(address_list_, net_log, data_.get());
    ...
    return std::make_unique<SOCKSClientSocket>(std::move(socket), HostPortPair(hostname, port), NetworkAnonymizationKey(), DEFAULT_PRIORITY, host_resolver,
...
TEST_F(SOCKSClientSocketTest, CompleteHandshake)
{
    ...
    for (bool use_read_if_ready : { true, false }) {
        ...
        MockRead data_reads[] = { MockRead(ASYNC, kSOCKS4OkReply, kSOCKS4OkReplyLength), MockRead(ASYNC, payload_read.data(), payload_read.size()) };
        RecordingNetLogObserver log_observer;

        user_sock_ = BuildMockSocket(data_reads, data_writes, host_resolver_.get(), "localhost", 80, NetLog::Get());
        ...
        EXPECT_TRUE(LogContainsBeginEvent(entries, 0, NetLogEventType::SOCKS_CONNECT));
        EXPECT_FALSE(user_sock_->IsConnected());
        ...
        rv = user_sock_->Write(buffer.get(), payload_write.size(), callback_.callback(), TRAFFIC_ANNOTATION_FOR_TESTS);
        EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
        rv = callback_.WaitForResult();
        ...
        if (use_read_if_ready) {
            rv = user_sock_->ReadIfReady(buffer.get(), payload_read.size(), callback_.callback());
            EXPECT_THAT(rv, IsError(ERR_IO_PENDING));
            rv = callback_.WaitForResult();
            EXPECT_EQ(net::OK, rv);
            rv = user_sock_->ReadIfReady(buffer.get(), payload_read.size(), callback_.callback());
...
TEST_F(SOCKSClientSocketTest, CancelPendingReadIfReady)
{
    ...
    MockWrite data_writes[] = { MockWrite(ASYNC, kSOCKS4OkRequestLocalHostPort80, kSOCKS4OkRequestLocalHostPort80Length) };
    MockRead data_reads[] = { MockRead(ASYNC, kSOCKS4OkReply, kSOCKS4OkReplyLength), MockRead(ASYNC, payload_read.data(), payload_read.size()) };
    user_sock_ = BuildMockSocket(data_reads, data_writes, host_resolver_.get(), "localhost", 80, nullptr);
...
TEST_F(SOCKSClientSocketTest, HandshakeFailures)
{
    ...
    for (const auto& test : tests) {
        MockWrite data_writes[] = { MockWrite(SYNCHRONOUS, kSOCKS4OkRequestLocalHostPort80, kSOCKS4OkRequestLocalHostPort80Length) };
        MockRead data_reads[] = { MockRead(SYNCHRONOUS, test.fail_reply, std::size(test.fail_reply)) };
        RecordingNetLogObserver log_observer;

        user_sock_ = BuildMockSocket(data_reads, data_writes, host_resolver_.get(), "socks.test", 80, NetLog::Get());
...
TEST_F(SOCKSClientSocketTest, PartialServerReads)
{
    ...
    MockWrite data_writes[] = { MockWrite(ASYNC, kSOCKS4OkRequestLocalHostPort80, kSOCKS4OkRequestLocalHostPort80Length) };
    MockRead data_reads[]
        = { MockRead(ASYNC, kSOCKSPartialReply1, std::size(kSOCKSPartialReply1)), MockRead(ASYNC, kSOCKSPartialReply2, std::size(kSOCKSPartialReply2)) };
    RecordingNetLogObserver log_observer;
...
TEST_F(SOCKSClientSocketTest, PartialClientWrites)
{
    ...
    MockRead data_reads[] = { MockRead(ASYNC, kSOCKS4OkReply, kSOCKS4OkReplyLength) };
    RecordingNetLogObserver log_observer;
...
{
    MockWrite data_writes[] = { MockWrite(ASYNC, kSOCKS4OkRequestLocalHostPort80, kSOCKS4OkRequestLocalHostPort80Length) };
    MockRead data_reads[] = { MockRead(ASYNC, kSOCKS4OkReply, kSOCKS4OkReplyLength - 2),
...
TEST_F(SOCKSClientSocketTest, FailedDNS)
{
    ...
    user_sock_ = BuildMockSocket(base::span<MockRead>(), base::span<MockWrite>(), host_resolver_.get(), hostname, 80, NetLog::Get());
    ...
    EXPECT_THAT(user_sock_->GetResolveErrorInfo().error, IsError(ERR_DNS_TIMED_OUT));
    EXPECT_FALSE(user_sock_->IsConnected());
    entries = log_observer.GetEntries();
    EXPECT_TRUE(LogContainsEndEvent(entries, -1, NetLogEventType::SOCKS_CONNECT));
}
...
TEST_F(SOCKSClientSocketTest, DisconnectWhileHostResolveInProgress)
{
    ...
    user_sock_ = BuildMockSocket(data_reads, data_writes, hanging_resolver.get(), "foo", 80, nullptr);
...
TEST_F(SOCKSClientSocketTest, NoIPv6)
{
    ...
    user_sock_ = BuildMockSocket(base::span<MockRead>(), base::span<MockWrite>(), host_resolver_.get(), kHostName, 80, nullptr);

    EXPECT_EQ(ERR_NAME_NOT_RESOLVED, callback_.GetResult(user_sock_->Connect(callback_.callback())));
}
...
TEST_F(SOCKSClientSocketTest, NoIPv6RealResolver)
{
    ...
    std::unique_ptr<HostResolver> host_resolver(HostResolver::CreateStandaloneResolver(nullptr));

    user_sock_ = BuildMockSocket(base::span<MockRead>(), base::span<MockWrite>(), host_resolver.get(), kHostName, 80, nullptr);
...
TEST_F(SOCKSClientSocketTest, Tag)
{
    ...
    auto tagging_sock = std::make_unique<MockTaggingStreamSocket>(std::make_unique<MockTCPClientSocket>(address_list_, NetLog::Get(), &data));
    ...
    SOCKSClientSocket socket(std::move(tagging_sock), HostPortPair("localhost", 80), NetworkAnonymizationKey(), DEFAULT_PRIORITY, &host_resolver,
        SecureDnsPolicy::kAllow, TRAFFIC_ANNOTATION_FOR_TESTS);
...
TEST_F(SOCKSClientSocketTest, SetSecureDnsPolicy)
{
    for (auto secure_dns_policy : { SecureDnsPolicy::kAllow, SecureDnsPolicy::kDisable }) {
        ...
        SOCKSClientSocket socket(std::make_unique<MockTCPClientSocket>(address_list_, NetLog::Get(), &data), HostPortPair("doh.test", 80),
            NetworkAnonymizationKey(), DEFAULT_PRIORITY, &host_resolver, secure_dns_policy, TRAFFIC_ANNOTATION_FOR_TESTS);
