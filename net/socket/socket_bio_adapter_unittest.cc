...
class SocketBIOAdapterTest : public testing::TestWithParam<ReadIfReadySupport>, public SocketBIOAdapter::Delegate, public WithTaskEnvironment {
protected:
    void SetUp() override
    {
    ...
    std::unique_ptr<StreamSocket> MakeTestSocket(SocketDataProvider* data)
    {
        ...
        std::unique_ptr<StreamSocket> socket = factory_.CreateTransportClientSocket(AddressList(), nullptr, nullptr, nullptr, NetLogSource());
        CHECK_EQ(OK, socket->Connect(CompletionOnceCallback()));
        return socket;
    }

    void set_reset_on_write_ready(std::unique_ptr<SocketBIOAdapter>* reset_on_write_ready)
    {
        reset_on_write_ready_ = reset_on_write_ready;
    }

    void ExpectReadError(BIO* bio, int error, const crypto::OpenSSLErrStackTracer& tracer)
    {
    ...
    void ExpectWriteError(BIO* bio, int error, const crypto::OpenSSLErrStackTracer& tracer)
    {
...
INSTANTIATE_TEST_SUITE_P(All, SocketBIOAdapterTest, testing::Values(READ_IF_READY_SUPPORTED, READ_IF_READY_NOT_SUPPORTED));
...
TEST_P(SocketBIOAdapterTest, ReadSync)
{
    ...
    MockRead reads[] = {
        MockRead(SYNCHRONOUS, 0, "hello"),
        MockRead(SYNCHRONOUS, 1, "world"),
    ...
    std::unique_ptr<SocketBIOAdapter> adapter = std::make_unique<SocketBIOAdapter>(socket.get(), 100, 100, this);
    BIO* bio = adapter->bio();
...
TEST_P(SocketBIOAdapterTest, ReadAsync)
{
    ...
    MockRead reads[] = {
        MockRead(ASYNC, 0, "hello"),
        MockRead(ASYNC, 1, "world"),
...
TEST_P(SocketBIOAdapterTest, WriteSync)
{
    ...
    std::unique_ptr<StreamSocket> socket = MakeTestSocket(&data);
    std::unique_ptr<SocketBIOAdapter> adapter = std::make_unique<SocketBIOAdapter>(socket.get(), 10, 10, this);
    BIO* bio = adapter->bio();
...
TEST_P(SocketBIOAdapterTest, AsyncWriteInterruptsBoth)
{
    ...
    SequencedSocketData data(reads, writes);
    std::unique_ptr<StreamSocket> socket = MakeTestSocket(&data);
    std::unique_ptr<SocketBIOAdapter> adapter = std::make_unique<SocketBIOAdapter>(socket.get(), 5, 5, this);
    BIO* bio = adapter->bio();
