...
UDPSocketPosix::UDPSocketPosix(DatagramSocket::BindType bind_type, net::NetLog* net_log, const net::NetLogSource& source)
...
UDPSocketPosix::UDPSocketPosix(DatagramSocket::BindType bind_type, NetLogWithSource source_net_log)
    : socket_(kInvalidSocket)
    , bind_type_(bind_type)
    , read_socket_watcher_(FROM_HERE)
    , write_socket_watcher_(FROM_HERE)
...
int UDPSocketPosix::AdoptOpenedSocket(AddressFamily address_family, int socket)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    DCHECK_EQ(socket_, kInvalidSocket);
    auto owned_socket_count = TryAcquireGlobalUDPSocketCount();
...
int UDPSocketPosix::ConfigureOpenedSocket()
{
    ...
    PCHECK(change_fdguard_np(socket_, nullptr, 0, &guardid, GUARD_CLOSE | GUARD_DUP, nullptr) == 0);
...
int UDPSocketPosix::GetLocalAddress(IPEndPoint* address) const
{
    ...
    if (!local_address_.get()) {
        ...
        net_log_.AddEvent(NetLogEventType::UDP_LOCAL_ADDRESS, [&] { return CreateNetLogUDPConnectParams(*local_address_, bound_network_); });
    }
...
int UDPSocketPosix::Read(IOBuffer* buf, int buf_len, CompletionOnceCallback callback)
{
    return RecvFrom(buf, buf_len, nullptr, std::move(callback));
}

int UDPSocketPosix::RecvFrom(IOBuffer* buf, int buf_len, IPEndPoint* address, CompletionOnceCallback callback)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    if (!base::CurrentIOThread::Get()->WatchFileDescriptor(socket_, true, base::MessagePumpForIO::WATCH_READ, &read_socket_watcher_, &read_watcher_)) {
...
int UDPSocketPosix::Write(IOBuffer* buf, int buf_len, CompletionOnceCallback callback, const NetworkTrafficAnnotationTag& traffic_annotation)
{
    return SendToOrWrite(buf, buf_len, nullptr, std::move(callback));
}

int UDPSocketPosix::SendTo(IOBuffer* buf, int buf_len, const IPEndPoint& address, CompletionOnceCallback callback)
{
    return SendToOrWrite(buf, buf_len, &address, std::move(callback));
}

int UDPSocketPosix::SendToOrWrite(IOBuffer* buf, int buf_len, const IPEndPoint* address, CompletionOnceCallback callback)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    DCHECK_NE(kInvalidSocket, socket_);
    ...
    if (int result = InternalSendTo(buf, buf_len, address); result != ERR_IO_PENDING) {
        return result;
    }

    if (!base::CurrentIOThread::Get()->WatchFileDescriptor(socket_, true, base::MessagePumpForIO::WATCH_WRITE, &write_socket_watcher_, &write_watcher_)) {
...
int UDPSocketPosix::Connect(const IPEndPoint& address)
{
    ...
    net_log_.BeginEvent(NetLogEventType::UDP_CONNECT, [&] { return CreateNetLogUDPConnectParams(address, bound_network_); });
...
int UDPSocketPosix::InternalConnect(const IPEndPoint& address)
{
    ...
    if (bind_type_ == DatagramSocket::RANDOM_BIND) {
        ...
        size_t addr_size = address.GetSockAddrFamily() == AF_INET ? IPAddress::kIPv4AddressSize : IPAddress::kIPv6AddressSize;
...
int UDPSocketPosix::SetDoNotFragment()
{
    ...
    if (addr_family_ == AF_INET6) {
        int rv = setsockopt(socket_, IPPROTO_IPV6, IPV6_DONTFRAG, &val, sizeof(val));
    ...
    if (addr_family_ == AF_INET6) {
        ...
        if (setsockopt(socket_, IPPROTO_IPV6, IPV6_MTU_DISCOVER, &val, sizeof(val)) != 0) {
            return MapSystemError(errno);
        }

        int v6_only = false;
        socklen_t v6_only_len = sizeof(v6_only);
        if (getsockopt(socket_, IPPROTO_IPV6, IPV6_V6ONLY, &v6_only, &v6_only_len) != 0) {
            return MapSystemError(errno);
        }
...
int UDPSocketPosix::SetRecvTos()
{
    ...
    unsigned int ecn = 1;
    if (addr_family_ == AF_INET6) {
        if (setsockopt(socket_, IPPROTO_IPV6, IPV6_RECVTCLASS, &ecn, sizeof(ecn)) != 0) {
            return MapSystemError(errno);
        }
#if BUILDFLAG(IS_APPLE)
...
{
    TRACE_EVENT(NetTracingCategory(), "UDPSocketPosix::ReadWatcher::OnFileCanReadWithoutBlocking");
...
void UDPSocketPosix::LogRead(int result, const char* bytes, socklen_t addr_len, const sockaddr* addr)
{
    if (result < 0) {
    ...
    if (net_log_.IsCapturing()) {
        ...
        NetLogUDPDataTransfer(net_log_, NetLogEventType::UDP_BYTES_RECEIVED, result, bytes, is_address_valid ? &address : nullptr);
    }
...
void UDPSocketPosix::LogWrite(int result, const char* bytes, const IPEndPoint* address)
{
    if (result < 0) {
...
// course.
int UDPSocketPosix::InternalRecvFrom(IOBuffer* buf, int buf_len, IPEndPoint* address)
{
    ...
    if (experimental_recv_optimization_enabled_ && is_connected_ && remote_address_) {
        return InternalRecvFromConnectedSocket(buf, buf_len, address);
    }
    return InternalRecvFromNonConnectedSocket(buf, buf_len, address);
}

int UDPSocketPosix::InternalRecvFromConnectedSocket(IOBuffer* buf, int buf_len, IPEndPoint* address)
{
    ...
    bool success = remote_address_->ToSockAddr(sock_addr.addr, &sock_addr.addr_len);
    DCHECK(success);
    LogRead(result, buf->data(), sock_addr.addr_len, sock_addr.addr);
    return result;
}

int UDPSocketPosix::InternalRecvFromNonConnectedSocket(IOBuffer* buf, int buf_len, IPEndPoint* address)
{
    SockaddrStorage storage;
    struct iovec iov = {
        .iov_base = buf->data(),
...
#if BUILDFLAG(IS_APPLE)
                if ((cmsg->cmsg_level == IPPROTO_IP && cmsg->cmsg_type == IP_RECVTOS) || (cmsg->cmsg_level == IPPROTO_IPV6 && cmsg->cmsg_type == IPV6_TCLASS)) {
#else
...
int UDPSocketPosix::InternalSendTo(IOBuffer* buf, int buf_len, const IPEndPoint* address)
{
    SockaddrStorage storage;
    ...
    int result = HANDLE_EINTR(sendto(socket_, buf->data(), buf_len, sendto_flags_, addr, storage.addr_len));
    if (result < 0)
        result = MapSystemError(errno);
    if (result != ERR_IO_PENDING)
...
int UDPSocketPosix::SetMulticastOptions()
{
    if (!(socket_options_ & SOCKET_OPTION_MULTICAST_LOOP)) {
        ...
        if (addr_family_ == AF_INET) {
            u_char loop = 0;
            rv = setsockopt(socket_, IPPROTO_IP, IP_MULTICAST_LOOP, &loop, sizeof(loop));
        } else {
            u_int loop = 0;
            rv = setsockopt(socket_, IPPROTO_IPV6, IPV6_MULTICAST_LOOP, &loop, sizeof(loop));
        }
        if (rv < 0)
            return MapSystemError(errno);
    }
    if (multicast_time_to_live_ != IP_DEFAULT_MULTICAST_TTL) {
        ...
        if (addr_family_ == AF_INET) {
            ...
            rv = setsockopt(socket_, IPPROTO_IPV6, IPV6_MULTICAST_HOPS, &ttl, sizeof(ttl));
        }
        if (rv < 0)
            return MapSystemError(errno);
    }
    if (multicast_interface_ != 0) {
        ...
        case AF_INET: {
            ...
            int rv = setsockopt(socket_, IPPROTO_IP, IP_MULTICAST_IF, reinterpret_cast<const char*>(&mreq), sizeof(mreq));
            if (rv)
                return MapSystemError(errno);
            break;
        }
...
int UDPSocketPosix::SetTos(DiffServCodePoint dscp, EcnCodePoint ecn)
{
    ...
    int rv = setsockopt(socket_, IPPROTO_IP, IP_TOS, &dscp_and_ecn, sizeof(dscp_and_ecn));
    if (addr_family_ == AF_INET6) {
        ...
        rv = setsockopt(socket_, IPPROTO_IPV6, IPV6_TCLASS, &dscp_and_ecn, sizeof(dscp_and_ecn));
    }
    if (rv < 0)
        return MapSystemError(errno);
    return OK;
...
#if BUILDFLAG(IS_IOS)
    if (setsockopt(socket_, SOL_SOCKET, SO_NET_SERVICE_TYPE, &ios_network_service_type, sizeof(ios_network_service_type))) {
        return MapSystemError(errno);
    }
