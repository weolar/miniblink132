...
bool CreateUDPAddress(const std::string& ip_str, uint16_t port, IPEndPoint* address)
{
...
public:
    UDPSocketTest()
        : buffer_(base::MakeRefCounted<IOBufferWithSize>(kMaxRead))
    {
    }
    ...
    std::string RecvFromSocket(UDPServerSocket* socket, DiffServCodePoint dscp, EcnCodePoint ecn)
    {
        TestCompletionCallback callback;

        int rv = socket->RecvFrom(buffer_.get(), kMaxRead, &recv_from_address_, callback.callback());
    ...
    int SendToSocket(UDPServerSocket* socket, std::string msg, const IPEndPoint& address)
    {
        scoped_refptr<StringIOBuffer> io_buffer = base::MakeRefCounted<StringIOBuffer>(msg);
        TestCompletionCallback callback;
        int rv = socket->SendTo(io_buffer.get(), io_buffer->size(), address, callback.callback());
    ...
    std::string ReadSocket(UDPClientSocket* socket, DiffServCodePoint dscp, EcnCodePoint ecn)
    {
        TestCompletionCallback callback;
    ...
    int WriteSocket(UDPClientSocket* socket, const std::string& msg)
    {
        scoped_refptr<StringIOBuffer> io_buffer = base::MakeRefCounted<StringIOBuffer>(msg);
        TestCompletionCallback callback;
        int rv = socket->Write(io_buffer.get(), io_buffer->size(), callback.callback(), TRAFFIC_ANNOTATION_FOR_TESTS);
        return callback.GetResult(rv);
    }

    void WriteSocketIgnoreResult(UDPClientSocket* socket, const std::string& msg)
    {
        WriteSocket(socket, msg);
    }

    // And again for a bare socket
    int SendToSocket(UDPSocket* socket, std::string msg, const IPEndPoint& address)
    {
...
void ReadCompleteCallback(int* result_out, base::OnceClosure callback, int result)
{
    *result_out = result;
...
void UDPSocketTest::ConnectTest(bool use_nonblocking_io, bool use_async)
{
    ...
    auto server = std::make_unique<UDPServerSocket>(NetLog::Get(), NetLogSource());
    ...
    // Setup the client.
    auto client = std::make_unique<UDPClientSocket>(DatagramSocket::DEFAULT_BIND, NetLog::Get(), NetLogSource());
    if (use_nonblocking_io)
    ...
    EXPECT_EQ(simple_message.length(), static_cast<size_t>(WriteSocket(client.get(), simple_message)));
    ...
    // Server echoes reply.
    EXPECT_EQ(simple_message.length(), static_cast<size_t>(SendToSocket(server.get(), simple_message)));
    ...
    int read_result = 0;
    int rv = server->RecvFrom(buffer_.get(), kMaxRead, &recv_from_address_, base::BindOnce(&ReadCompleteCallback, &read_result, run_loop.QuitClosure()));
    EXPECT_THAT(rv, IsError(ERR_IO_PENDING));

    // Client sends to the server.
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
        FROM_HERE, base::BindOnce(&UDPSocketTest::WriteSocketIgnoreResult, base::Unretained(this), client.get(), simple_message));
    run_loop.Run();
    ...
    auto server_entries = net_log_observer.GetEntriesForSource(server_net_log_source);
    ASSERT_EQ(6u, server_entries.size());
    EXPECT_TRUE(LogContainsBeginEvent(server_entries, 0, NetLogEventType::SOCKET_ALIVE));
    EXPECT_TRUE(LogContainsEvent(server_entries, 1, NetLogEventType::UDP_LOCAL_ADDRESS, NetLogEventPhase::NONE));
    EXPECT_TRUE(LogContainsEvent(server_entries, 2, NetLogEventType::UDP_BYTES_RECEIVED, NetLogEventPhase::NONE));
    EXPECT_TRUE(LogContainsEvent(server_entries, 3, NetLogEventType::UDP_BYTES_SENT, NetLogEventPhase::NONE));
    EXPECT_TRUE(LogContainsEvent(server_entries, 4, NetLogEventType::UDP_BYTES_RECEIVED, NetLogEventPhase::NONE));
    EXPECT_TRUE(LogContainsEndEvent(server_entries, 5, NetLogEventType::SOCKET_ALIVE));

    // Check the client's log.
    auto client_entries = net_log_observer.GetEntriesForSource(client_net_log_source);
    EXPECT_EQ(7u, client_entries.size());
    EXPECT_TRUE(LogContainsBeginEvent(client_entries, 0, NetLogEventType::SOCKET_ALIVE));
    EXPECT_TRUE(LogContainsBeginEvent(client_entries, 1, NetLogEventType::UDP_CONNECT));
    EXPECT_TRUE(LogContainsEndEvent(client_entries, 2, NetLogEventType::UDP_CONNECT));
    EXPECT_TRUE(LogContainsEvent(client_entries, 3, NetLogEventType::UDP_BYTES_SENT, NetLogEventPhase::NONE));
    EXPECT_TRUE(LogContainsEvent(client_entries, 4, NetLogEventType::UDP_BYTES_RECEIVED, NetLogEventPhase::NONE));
    EXPECT_TRUE(LogContainsEvent(client_entries, 5, NetLogEventType::UDP_BYTES_SENT, NetLogEventPhase::NONE));
    EXPECT_TRUE(LogContainsEndEvent(client_entries, 6, NetLogEventType::SOCKET_ALIVE));
}
...
TEST_F(UDPSocketTest, PartialRecv)
{
    ...
    ASSERT_THAT(server_socket.Listen(IPEndPoint(IPAddress::IPv4Localhost(), 0)), IsOk());
    IPEndPoint server_address;
    ASSERT_THAT(server_socket.GetLocalAddress(&server_address), IsOk());

    UDPClientSocket client_socket(DatagramSocket::DEFAULT_BIND, nullptr, NetLogSource());
    ASSERT_THAT(client_socket.Connect(server_address), IsOk());

    std::string test_packet("hello world!");
    ASSERT_EQ(static_cast<int>(test_packet.size()), WriteSocket(&client_socket, test_packet));
    ...
    int rv = server_socket.RecvFrom(buffer.get(), kPartialReadSize, &recv_from_address_, recv_callback.callback());
    ...
    ASSERT_EQ(static_cast<int>(second_packet.size()), WriteSocket(&client_socket, second_packet));
...
TEST_F(UDPSocketTest, MAYBE_LocalBroadcast)
{
    ...
    auto server1 = std::make_unique<UDPServerSocket>(NetLog::Get(), NetLogSource());
    auto server2 = std::make_unique<UDPServerSocket>(NetLog::Get(), NetLogSource());
    ...
    ASSERT_TRUE(CreateUDPAddress("127.255.255.255", listen_address.port(), &broadcast_address));
    ASSERT_EQ(static_cast<int>(first_message.size()), SendToSocket(server1.get(), first_message, broadcast_address));
    std::string str = RecvFromSocket(server1.get());
    ASSERT_EQ(first_message, str);
    str = RecvFromSocket(server2.get());
    ASSERT_EQ(first_message, str);

    ASSERT_EQ(static_cast<int>(second_message.size()), SendToSocket(server2.get(), second_message, broadcast_address));
...
TEST_F(UDPSocketTest, ConnectRandomBind)
{
    ...
    for (int i = 0; i < kIterations; ++i) {
        ...
        EXPECT_THAT(socket.Connect(IPEndPoint(IPAddress::IPv4Localhost(), 53)), IsOk());
...
TEST_F(UDPSocketTest, ConnectFail)
{
    ...
    // IPv4.
    EXPECT_THAT(socket.Connect(net::IPEndPoint(IPAddress::IPv6Localhost(), 53)), Not(IsOk()));
...
{
    auto socketfd = CreatePlatformSocket(ConvertAddressFamily(ADDRESS_FAMILY_IPV4), SOCK_DGRAM, AF_UNIX ? 0 : IPPROTO_UDP);
    UDPSocket socket(DatagramSocket::DEFAULT_BIND, nullptr, NetLogSource());
...
TEST_F(UDPSocketTest, VerifyConnectBindsAddr)
{
    ...
    EXPECT_EQ(simple_message.length(), static_cast<size_t>(WriteSocket(&client, simple_message)));
    ...
    // Server2 sends reply.
    EXPECT_EQ(foreign_message.length(), static_cast<size_t>(SendToSocket(&server2, foreign_message, client_address)));

    // Server1 sends reply.
    EXPECT_EQ(simple_message.length(), static_cast<size_t>(SendToSocket(&server1, simple_message, client_address)));
...
TEST_F(UDPSocketTest, ClientGetLocalPeerAddresses)
{
    ...
    for (const auto& test : tests) {
        SCOPED_TRACE(std::string("Connecting from ") + test.local_address + std::string(" to ") + test.remote_address);
...
TEST_F(UDPSocketTest, ServerSetDoNotFragment)
{
    for (std::string ip : { "127.0.0.1", "::1" }) {
        ...
        if (bind_address.address().IsIPv6() && (rv == ERR_ADDRESS_INVALID || rv == ERR_ADDRESS_UNREACHABLE))
            return;
        EXPECT_THAT(rv, IsOk());
...
// OS_FUCHSIA.
#if BUILDFLAG(IS_WIN) || BUILDFLAG(IS_FUCHSIA)
    IPEndPoint receive_address(IPAddress::AllZeros(group_ip.size()), 0 /* port */);
#else
    ...
    interfaces.erase(std::remove_if(interfaces.begin(), interfaces.end(), [](const auto& iface) { return iface.friendly_name.rfind("vEthernet", 0) == 0; }),
        interfaces.end());
    ...
    ASSERT_THAT(socket1.SetMulticastInterface(interfaces[0].interface_index), IsOk());
    ...
    ASSERT_THAT(socket2.SetMulticastInterface(interfaces[0].interface_index), IsOk());
...
TEST_F(UDPSocketTest, VerifyDscpAndEcnExchangeV4)
{
    ...
    EXPECT_EQ(WriteSocket(&client, client_message), static_cast<int>(client_message.length()));
    EXPECT_EQ(RecvFromSocket(&server, DSCP_AF41, ECN_ECT1), client_message.data());

    // Server messages
    EXPECT_EQ(server.SetTos(DSCP_AF41, ECN_ECT1), 0);
    std::string first_message = "foobar";
    EXPECT_EQ(SendToSocket(&server, first_message), static_cast<int>(first_message.length()));
    ...
    EXPECT_EQ(SendToSocket(&server, second_message), static_cast<int>(second_message.length()));
...
TEST_F(UDPSocketTest, VerifyDscpAndEcnExchangeDualStack)
{
    ...
    // to localhost.
    IPEndPoint server_v4_address(IPAddress::IPv4Localhost(), server_v6_address.port());
    UDPClientSocket client(DatagramSocket::DEFAULT_BIND, nullptr, NetLogSource());
    ...
    std::string first_message = "foobar";
    EXPECT_EQ(WriteSocket(&client, first_message), static_cast<int>(first_message.length()));
...
#endif
    EXPECT_EQ(WriteSocket(&client, second_message), static_cast<int>(second_message.length()));
    ...
    EXPECT_EQ(WriteSocket(&client, second_message), static_cast<int>(second_message.length()));
    EXPECT_EQ(RecvFromSocket(&server, DSCP_CS2, final_ecn), second_message.data());

#if !BUILDFLAG(IS_WIN)
    EXPECT_EQ(client.SetTos(DSCP_AF41, ECN_NO_CHANGE), 0);
#endif
    EXPECT_EQ(WriteSocket(&client, second_message), static_cast<int>(second_message.length()));
    EXPECT_EQ(RecvFromSocket(&server, DSCP_AF41, final_ecn), second_message.data());
...
TEST_F(UDPSocketTest, VerifyDscpAndEcnExchangeDualStackV4Mapped)
{
    ...
    IPEndPoint server_v6_address(*IPAddress::FromIPLiteral("::ffff:7f00:0001"), 0);
    UDPServerSocket server(nullptr, NetLogSource());
    server.AllowAddressReuse();
    ASSERT_THAT(server.Listen(server_v6_address), IsOk());
    // Get bound port.
...
TEST_F(UDPSocketTest, ConnectUsingNetwork)
{
...

    {
        ...
        UDPClientSocket socket(DatagramSocket::RANDOM_BIND, nullptr, NetLogSource());
        int rv = socket.ConnectUsingNetwork(wrong_network_handle, fake_server_address);
...

    {
        ...
        UDPClientSocket socket(DatagramSocket::RANDOM_BIND, nullptr, NetLogSource());
        const handles::NetworkHandle network_handle = NetworkChangeNotifier::GetDefaultNetwork();
        if (network_handle != handles::kInvalidNetworkHandle) {
            EXPECT_EQ(OK, socket.ConnectUsingNetwork(network_handle, fake_server_address));
            EXPECT_EQ(network_handle, socket.GetBoundNetwork());
        }
    }
#else
    UDPClientSocket socket(DatagramSocket::RANDOM_BIND, nullptr, NetLogSource());
    EXPECT_EQ(ERR_NOT_IMPLEMENTED, socket.ConnectUsingNetwork(wrong_network_handle, fake_server_address));
#endif // BUILDFLAG(IS_ANDROID)
}
...
TEST_F(UDPSocketTest, ConnectUsingNetworkAsync)
{
...

    {
        ...
        UDPClientSocket socket(DatagramSocket::RANDOM_BIND, nullptr, NetLogSource());
        TestCompletionCallback callback;
        int rv = socket.ConnectUsingNetworkAsync(wrong_network_handle, fake_server_address, callback.callback());
...

    {
        ...
        const handles::NetworkHandle network_handle = NetworkChangeNotifier::GetDefaultNetwork();
        if (network_handle != handles::kInvalidNetworkHandle) {
            int rv = socket.ConnectUsingNetworkAsync(network_handle, fake_server_address, callback.callback());
            if (rv == ERR_IO_PENDING) {
                rv = callback.WaitForResult();
            }
            EXPECT_EQ(OK, rv);
    ...
    UDPClientSocket socket(DatagramSocket::RANDOM_BIND, nullptr, NetLogSource());
    TestCompletionCallback callback;
    EXPECT_EQ(ERR_NOT_IMPLEMENTED, socket.ConnectUsingNetworkAsync(wrong_network_handle, fake_server_address, callback.callback()));
#endif // BUILDFLAG(IS_ANDROID)
}
...
public:
    TestUDPSocketWin(QwaveApi* qos, DatagramSocket::BindType bind_type, net::NetLog* net_log, const net::NetLogSource& source)
        : UDPSocketWin(bind_type, net_log, source)
        , qos_(qos)
    {
    }
    ...
    QwaveApi* GetQwaveApi() const override
    {
        return qos_;
    }
...
public:
    ...
    MOCK_METHOD6(AddSocketToFlow, BOOL(HANDLE handle, SOCKET socket, PSOCKADDR addr, QOS_TRAFFIC_TYPE traffic_type, DWORD flags, PQOS_FLOWID flow_id));

    MOCK_METHOD4(RemoveSocketFromFlow, BOOL(HANDLE handle, SOCKET socket, QOS_FLOWID flow_id, DWORD reserved));
    MOCK_METHOD7(SetFlow, BOOL(HANDLE handle, QOS_FLOWID flow_id, QOS_SET_FLOW op, ULONG size, PVOID data, DWORD reserved, LPOVERLAPPED overlapped));
};

std::unique_ptr<UDPSocket> OpenedDscpTestClient(QwaveApi* api, IPEndPoint bind_address)
{
    auto client = std::make_unique<TestUDPSocketWin>(api, DatagramSocket::DEFAULT_BIND, nullptr, NetLogSource());
    int rv = client->Open(bind_address.GetFamily());
    EXPECT_THAT(rv, IsOk());
...
TEST_F(UDPSocketTest, ConnectedSocketDelayedInitAndUpdate)
{
    ...
    std::unique_ptr<UDPSocket> client = ConnectedDscpTestClient(&api);
    EXPECT_CALL(api, qwave_supported()).WillRepeatedly(Return(true));
    EXPECT_CALL(api, CreateHandle(_, _)).WillOnce(DoAll(SetArgPointee<1>(kFakeHandle1), Return(true)));

    EXPECT_CALL(api, AddSocketToFlow(_, _, _, _, _, _)).WillOnce(DoAll(SetArgPointee<5>(kFakeFlowId1), Return(true)));
    ...
    EXPECT_CALL(api, AddSocketToFlow(_, _, _, QOSTrafficTypeBestEffort, _, _)).WillOnce(DoAll(SetArgPointee<5>(kFakeFlowId2), Return(true)));
    EXPECT_CALL(api, SetFlow(_, _, QOSSetOutgoingDSCPValue, _, DscpPointee(DSCP_DEFAULT), _, _));
...
TEST_F(UDPSocketTest, SendToCallsApisAfterDeferredInit)
{
    ...
    EXPECT_CALL(api, AddSocketToFlow(_, _, _, QOSTrafficTypeControl, _, _)).WillOnce(DoAll(SetArgPointee<5>(kFakeFlowId1), Return(true)));
...
protected:
    DscpManagerTest()
    {
        ...
        EXPECT_CALL(api_, CreateHandle(_, _)).WillOnce(DoAll(SetArgPointee<1>(kFakeHandle1), Return(true)));
        dscp_manager_ = std::make_unique<DscpManager>(&api_, INVALID_SOCKET);

        CreateUDPAddress("1.2.3.4", 9001, &address1_);
        CreateUDPAddress("1234:5678:90ab:cdef:1234:5678:90ab:cdef", 9002, &address2_);
    }
...
TEST_F(DscpManagerTest, PrepareForSendCallsQwaveApisAfterSet)
{
    ...
    EXPECT_CALL(api_, AddSocketToFlow(_, _, _, _, _, _)).WillOnce(DoAll(SetArgPointee<5>(kFakeFlowId1), Return(true)));
...
TEST_F(DscpManagerTest, SetDestroysExistingFlow)
{
    ...
    EXPECT_CALL(api_, AddSocketToFlow(_, _, _, _, _, _)).WillOnce(DoAll(SetArgPointee<5>(kFakeFlowId2), Return(true)));
...
TEST_F(DscpManagerTest, SocketReAddedOnRecreateHandle)
{
    ...
    EXPECT_CALL(api_, AddSocketToFlow(_, _, _, _, _, _)).WillOnce(DoAll(SetArgPointee<5>(kFakeFlowId1), Return(true)));
    EXPECT_CALL(api_, SetFlow(_, kFakeFlowId1, _, _, _, _, _)).WillOnce(Return(true));
    EXPECT_THAT(dscp_manager_->PrepareForSend(address1_), IsOk());

    // Make Second flow operation fail (requires resetting the codepoint).
    EXPECT_CALL(api_, RemoveSocketFromFlow(_, _, kFakeFlowId1, _)).WillOnce(Return(true));
    ...
    EXPECT_CALL(api_, SetFlow(_, _, _, _, _, _, _)).Times(0);
    EXPECT_CALL(api_, CloseHandle(kFakeHandle1));
    EXPECT_CALL(api_, CreateHandle(_, _)).WillOnce(DoAll(SetArgPointee<1>(kFakeHandle2), Return(true)));
    ...
    EXPECT_CALL(api_, AddSocketToFlow(_, _, _, QOSTrafficTypeControl, _, _)).WillOnce(DoAll(SetArgPointee<5>(kFakeFlowId2), Return(true)));
    EXPECT_CALL(api_, SetFlow(_, kFakeFlowId2, _, _, _, _, _)).WillOnce(Return(true));
    EXPECT_THAT(dscp_manager_->PrepareForSend(address1_), IsOk());
...
TEST_F(UDPSocketTest, ReadWithSocketOptimization)
{
    ...
    EXPECT_EQ(simple_message.length(), static_cast<size_t>(SendToSocket(&server, simple_message, client_address)));
...
TEST_F(UDPSocketTest, ReadWithSocketOptimizationTruncation)
{
    ...
    EXPECT_EQ(too_long_message.length(), static_cast<size_t>(SendToSocket(&server, too_long_message, client_address)));
    EXPECT_EQ(right_length_message.length(), static_cast<size_t>(SendToSocket(&server, right_length_message, client_address)));
    EXPECT_EQ(exact_length_message.length(), static_cast<size_t>(SendToSocket(&server, exact_length_message, client_address)));
...
TEST_F(UDPSocketTest, BindToNetwork)
{
    ...
    const handles::NetworkHandle wrong_network_handle = 65536;
    UDPClientSocket wrong_socket(DatagramSocket::RANDOM_BIND, nullptr, NetLogSource(), wrong_network_handle);
    ...
    const handles::NetworkHandle network_handle = NetworkChangeNotifier::GetDefaultNetwork();
    if (network_handle != handles::kInvalidNetworkHandle) {
        UDPClientSocket correct_socket(DatagramSocket::RANDOM_BIND, nullptr, NetLogSource(), network_handle);
...
public:
    explicit OverrideUDPSocketLimit(int new_limit)
    {
        ...
        params[features::kLimitOpenUDPSocketsMax.name] = base::NumberToString(new_limit);

        scoped_feature_list_.InitAndEnableFeatureWithParameters(features::kLimitOpenUDPSockets, params);
    }
...
TEST_F(UDPSocketTest, LimitClientSocket)
{
    ...
    auto socket1 = std::make_unique<UDPClientSocket>(DatagramSocket::DEFAULT_BIND, nullptr, NetLogSource());
    auto socket2 = std::make_unique<UDPClientSocket>(DatagramSocket::DEFAULT_BIND, nullptr, NetLogSource());
    ...
    auto socket3 = std::make_unique<UDPClientSocket>(DatagramSocket::DEFAULT_BIND, nullptr, NetLogSource());
    EXPECT_THAT(socket3->Connect(server_address), IsError(ERR_INSUFFICIENT_RESOURCES));
    EXPECT_EQ(2, GetGlobalUDPSocketCountForTesting());
    ...
    // it will work.
    auto socket4 = std::make_unique<UDPClientSocket>(DatagramSocket::DEFAULT_BIND, nullptr, NetLogSource());
...
TEST_F(UDPSocketTest, LimitConnectMultithreaded)
{
    ...
    for (const auto& thread : threads) {
        thread->task_runner()->PostTask(FROM_HERE, base::BindOnce([] {
            ...
            UDPClientSocket socket(DatagramSocket::DEFAULT_BIND, nullptr, NetLogSource());
