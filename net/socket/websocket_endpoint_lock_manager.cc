...
WebSocketEndpointLockManager::LockReleaser::LockReleaser(WebSocketEndpointLockManager* websocket_endpoint_lock_manager, IPEndPoint endpoint)
...
int WebSocketEndpointLockManager::LockEndpoint(const IPEndPoint& endpoint, Waiter* waiter)
{
...
base::TimeDelta WebSocketEndpointLockManager::SetUnlockDelayForTesting(base::TimeDelta new_delay)
{
...
void WebSocketEndpointLockManager::RegisterLockReleaser(LockReleaser* lock_releaser, IPEndPoint endpoint)
{
    DCHECK(lock_releaser);
    auto lock_info_it = lock_info_map_.find(endpoint);
    ...
    DVLOG(3) << "Registered (LockReleaser*)" << lock_releaser << " for " << endpoint.ToString();
}

void WebSocketEndpointLockManager::UnlockEndpointAfterDelay(const IPEndPoint& endpoint)
{
    DVLOG(3) << "Delaying " << unlock_delay_.InMilliseconds() << "ms before unlocking endpoint " << endpoint.ToString();
    ++pending_unlock_count_;
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(
        FROM_HERE, base::BindOnce(&WebSocketEndpointLockManager::DelayedUnlockEndpoint, weak_factory_.GetWeakPtr(), endpoint), unlock_delay_);
}

void WebSocketEndpointLockManager::DelayedUnlockEndpoint(const IPEndPoint& endpoint)
{
    auto lock_info_it = lock_info_map_.find(endpoint);
    ...
    DVLOG(3) << "Unlocking endpoint " << lock_info_it->first.ToString() << " and activating next waiter";
