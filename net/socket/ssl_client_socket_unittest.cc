...
public:
    ...
    SynchronousErrorStreamSocket& operator=(const SynchronousErrorStreamSocket&) = delete;
    ...
    int ReadIfReady(IOBuffer* buf, int buf_len, CompletionOnceCallback callback) override;
    int Write(IOBuffer* buf, int buf_len, CompletionOnceCallback callback, const NetworkTrafficAnnotationTag& traffic_annotation) override;
...
int SynchronousErrorStreamSocket::Read(IOBuffer* buf, int buf_len, CompletionOnceCallback callback)
{
    if (have_read_error_)
...
int SynchronousErrorStreamSocket::ReadIfReady(IOBuffer* buf, int buf_len, CompletionOnceCallback callback)
{
    if (have_read_error_)
        return pending_read_error_;
...
int SynchronousErrorStreamSocket::Write(IOBuffer* buf, int buf_len, CompletionOnceCallback callback, const NetworkTrafficAnnotationTag& traffic_annotation)
{
    if (have_write_error_)
        return pending_write_error_;
    return transport_->Write(buf, buf_len, std::move(callback), traffic_annotation);
}
...
public:
    ...
    int pending_read_result() const
    {
        return pending_read_result_;
    }
    IOBuffer* pending_read_buf() const
    {
        return pending_read_buf_.get();
    }
...
int FakeBlockingStreamSocket::Read(IOBuffer* buf, int len, CompletionOnceCallback callback)
{
    ...
    int rv = transport_->Read(buf, len, base::BindOnce(&FakeBlockingStreamSocket::OnReadCompleted, base::Unretained(this)));
...
int FakeBlockingStreamSocket::ReadIfReady(IOBuffer* buf, int len, CompletionOnceCallback callback)
{
    ...
    int rv = Read(buf_copy.get(), len, base::BindOnce(&FakeBlockingStreamSocket::CompleteReadIfReady, base::Unretained(this), buf_copy));
    if (rv > 0)
...
int FakeBlockingStreamSocket::Write(IOBuffer* buf, int len, CompletionOnceCallback callback, const NetworkTrafficAnnotationTag& traffic_annotation)
{
    DCHECK(buf);
    DCHECK_LE(0, len);
...
void FakeBlockingStreamSocket::UnblockWrite()
{
    ...
    int rv = transport_->Write(pending_write_buf_.get(), pending_write_len_,
        base::BindOnce(&FakeBlockingStreamSocket::CallPendingWriteCallback, base::Unretained(this)), TRAFFIC_ANNOTATION_FOR_TESTS);
...
void FakeBlockingStreamSocket::CompleteReadIfReady(scoped_refptr<IOBuffer> buf, int rv)
{
...
public:
    ...
    {
        return base::BindOnce(&DeleteSocketCallback::OnComplete, base::Unretained(this));
    }
...
public:
    ...
    MOCK_METHOD(void, MaybeEnqueueReport, (const net::HostPortPair&, const net::X509Certificate*, const net::SignedCertificateTimestampAndStatusList&));
};

class ManySmallRecordsHttpResponse : public test_server::HttpResponse {
public:
    static std::unique_ptr<test_server::HttpResponse> HandleRequest(const test_server::HttpRequest& request)
    {
    ...
    void SendResponse(base::WeakPtr<test_server::HttpResponseDelegate> delegate) override
    {
        base::StringPairs headers
            = { { "Connection", "close" }, { "Content-Length", base::NumberToString(chunk_size_ * chunk_count_) }, { "Content-Type", "text/plain" } };
        delegate->SendResponseHeaders(HTTP_OK, "OK", headers);
        SendChunks(chunk_size_, chunk_count_, delegate);
    }

private:
    static void SendChunks(size_t chunk_size, size_t chunk_count, base::WeakPtr<test_server::HttpResponseDelegate> delegate)
    {
        if (!delegate)
            return;
        ...
        delegate->SendContents(chunk, base::BindOnce(&SendChunks, chunk_size, chunk_count - 1, delegate));
    }
...
public:
    SSLClientSocketTest()
        ...
        , ssl_config_service_(std::make_unique<TestSSLConfigService>(SSLContextConfig()))
        ...
        , context_(std::make_unique<SSLClientContext>(
              ssl_config_service_.get(), cert_verifier_.get(), transport_security_state_.get(), ssl_client_session_cache_.get(), nullptr))
    {
...
protected:
    ...
    // on success.
    bool StartEmbeddedTestServer(EmbeddedTestServer::ServerCertificate cert, const SSLServerConfig& server_config)
    {
        embedded_test_server_ = std::make_unique<EmbeddedTestServer>(EmbeddedTestServer::TYPE_HTTPS);
    ...
    // Starts the embedded test server with the specified parameters. Returns true
    // on success.
    bool StartEmbeddedTestServer(const EmbeddedTestServer::ServerCertificateConfig& cert_config, const SSLServerConfig& server_config)
    {
        embedded_test_server_ = std::make_unique<EmbeddedTestServer>(EmbeddedTestServer::TYPE_HTTPS);
    ...
    virtual void RegisterEmbeddedTestServerHandlers(EmbeddedTestServer* server)
    {
        ...
        server->RegisterRequestHandler(base::BindRepeating(&ManySmallRecordsHttpResponse::HandleRequest));
        server->RegisterRequestHandler(base::BindRepeating(&HandleSSLInfoRequest, base::Unretained(this)));
    }

    std::unique_ptr<SSLClientSocket> CreateSSLClientSocket(
        std::unique_ptr<StreamSocket> transport_socket, const HostPortPair& host_and_port, const SSLConfig& ssl_config)
    {
        return socket_factory_->CreateSSLClientSocket(context_.get(), std::move(transport_socket), host_and_port, ssl_config);
    }
    ...
    bool CreateAndConnectSSLClientSocketWithHost(const SSLConfig& ssl_config, const HostPortPair& host_port_pair, int* result)
    {
        auto transport = std::make_unique<TCPClientSocket>(addr_, nullptr, nullptr, NetLog::Get(), NetLogSource());
        ...
        sock_ = CreateSSLClientSocket(std::move(transport), host_port_pair, ssl_config);
    ...
    bool CreateAndConnectSSLClientSocket(const SSLConfig& ssl_config, int* result)
    {
        return CreateAndConnectSSLClientSocketWithHost(ssl_config, host_port_pair(), result);
    }
...
private:
    static std::unique_ptr<test_server::HttpResponse> HandleSSLInfoRequest(SSLClientSocketTest* test, const test_server::HttpRequest& request)
    {
...
public:
    explicit StreamSocketWithoutReadIfReady(std::unique_ptr<StreamSocket> transport)
        : WrappedStreamSocket(std::move(transport))
    {
    }
...
public:
    ...
    std::unique_ptr<TransportClientSocket> CreateTransportClientSocket(const AddressList& addresses,
        std::unique_ptr<SocketPerformanceWatcher> socket_performance_watcher, NetworkQualityEstimator* network_quality_estimator, NetLog* net_log,
        const NetLogSource& source) override
    {
        return factory_->CreateTransportClientSocket(addresses, std::move(socket_performance_watcher), network_quality_estimator, net_log, source);
    }
    ...
    {
        stream_socket = std::make_unique<StreamSocketWithoutReadIfReady>(std::move(stream_socket));
        return factory_->CreateSSLClientSocket(context, std::move(stream_socket), host_and_port, ssl_config);
    }
...
class SSLClientSocketVersionTest : public SSLClientSocketTest, public ::testing::WithParamInterface<uint16_t> {
protected:
...
class SSLClientSocketReadTest : public SSLClientSocketTest, public ::testing::WithParamInterface<std::tuple<ReadIfReadyTransport, ReadIfReadySSL, uint16_t>> {
protected:
    SSLClientSocketReadTest()
        : SSLClientSocketTest()
    {
        if (!read_if_ready_supported()) {
            wrapped_socket_factory_ = std::make_unique<ClientSocketFactoryWithoutReadIfReady>(socket_factory_);
    ...
    int Read(StreamSocket* socket, IOBuffer* buf, int buf_len, CompletionOnceCallback callback)
    {
    ...
    int WaitForReadCompletion(StreamSocket* socket, IOBuffer* buf, int buf_len, TestCompletionCallback* callback, int rv)
    {
        if (!test_ssl_read_if_ready())
    ...
    int ReadAndWaitForCompletion(StreamSocket* socket, IOBuffer* buf, int buf_len)
    {
...
INSTANTIATE_TEST_SUITE_P(All, SSLClientSocketReadTest,
    Combine(Values(READ_IF_READY_SUPPORTED, READ_IF_READY_NOT_SUPPORTED), Values(TEST_SSL_READ_IF_READY, TEST_SSL_READ), ValuesIn(GetTLSVersions())));
...
protected:
    ...
    void CreateAndConnectUntilServerFinishedReceived(const SSLConfig& client_config, TestCompletionCallback* callback,
        FakeBlockingStreamSocket** out_raw_transport, std::unique_ptr<SSLClientSocket>* out_sock)
    {
        CHECK(embedded_test_server());

        auto real_transport = std::make_unique<TCPClientSocket>(addr(), nullptr, nullptr, nullptr, NetLogSource());
        auto transport = std::make_unique<FakeBlockingStreamSocket>(std::move(real_transport));
        int rv = callback->GetResult(transport->Connect(callback->callback()));
        EXPECT_THAT(rv, IsOk());

        FakeBlockingStreamSocket* raw_transport = transport.get();
        std::unique_ptr<SSLClientSocket> sock = CreateSSLClientSocket(std::move(transport), host_port_pair(), client_config);
    ...
    void TestFalseStart(const SSLServerConfig& server_config, const SSLConfig& client_config, bool expect_false_start)
    {
        ASSERT_TRUE(StartEmbeddedTestServer(EmbeddedTestServer::CERT_OK, server_config));

        TestCompletionCallback callback;
        FakeBlockingStreamSocket* raw_transport = nullptr;
        std::unique_ptr<SSLClientSocket> sock;
        ASSERT_NO_FATAL_FAILURE(CreateAndConnectUntilServerFinishedReceived(client_config, &callback, &raw_transport, &sock));
        ...
        if (expect_false_start) {
            ...
            static const int kRequestTextSize = static_cast<int>(std::size(request_text) - 1);
            auto request_buffer = base::MakeRefCounted<IOBufferWithSize>(kRequestTextSize);
            memcpy(request_buffer->data(), request_text, kRequestTextSize);

            // Write the request.
            rv = callback.GetResult(sock->Write(request_buffer.get(), kRequestTextSize, callback.callback(), TRAFFIC_ANNOTATION_FOR_TESTS));
...
int MakeHTTPRequest(StreamSocket* socket, const char* path = "/")
{
    ...
    TestCompletionCallback callback;
    while (!request.empty()) {
        auto request_buffer = base::MakeRefCounted<StringIOBuffer>(std::string(request));
        int rv = callback.GetResult(socket->Write(request_buffer.get(), request_buffer->size(), callback.callback(), TRAFFIC_ANNOTATION_FOR_TESTS));
        if (rv < 0) {
            return rv;
        }
    ...
    int rv = callback.GetResult(socket->Read(response_buffer.get(), 1024, callback.callback()));
    if (rv < 0) {
        return rv;
    }
    return OK;
...
public:
    explicit ZeroRTTResponse(bool zero_rtt)
        : zero_rtt_(zero_rtt)
    {
    }
...
std::unique_ptr<test_server::HttpResponse> HandleZeroRTTRequest(const test_server::HttpRequest& request)
{
    ...
    return std::make_unique<ZeroRTTResponse>(request.ssl_info->early_data_received);
}

class SSLClientSocketZeroRTTTest : public SSLClientSocketTest {
protected:
    SSLClientSocketZeroRTTTest()
        : SSLClientSocketTest()
    {
    ...
    {
        embedded_test_server()->ResetSSLConfig(net::EmbeddedTestServer::CERT_OK, server_config);
    }
    ...
    FakeBlockingStreamSocket* MakeClient(bool early_data_enabled)
    {
        ...
        real_transport_ = std::make_unique<TCPClientSocket>(addr(), nullptr, nullptr, nullptr, NetLogSource());
        auto transport = std::make_unique<FakeBlockingStreamSocket>(std::move(real_transport_));
        FakeBlockingStreamSocket* raw_transport = transport.get();

        int rv = callback_.GetResult(transport->Connect(callback_.callback()));
        EXPECT_THAT(rv, IsOk());

        ssl_socket_ = CreateSSLClientSocket(std::move(transport), host_port_pair(), ssl_config);
    ...
    {
        auto request_buffer = base::MakeRefCounted<IOBufferWithSize>(request.size());
        memcpy(request_buffer->data(), request.data(), request.size());
        return callback_.GetResult(ssl_socket_->Write(request_buffer.get(), request.size(), callback_.callback(), TRAFFIC_ANNOTATION_FOR_TESTS));
    }

    int ReadAndWait(IOBuffer* buf, size_t len)
    {
        return callback_.GetResult(ssl_socket_->Read(buf, len, callback_.callback()));
    }
...
public:
    ...
    int Verify(const RequestParams& params, CertVerifyResult* verify_result, CompletionOnceCallback callback, std::unique_ptr<Request>* out_req,
...
public:
    ...
    MOCK_METHOD1(OnSSLConfigForServersChanged, void(const base::flat_set<HostPortPair>&));
};

} // namespace

INSTANTIATE_TEST_SUITE_P(TLSVersion, SSLClientSocketVersionTest, ValuesIn(GetTLSVersions()));

TEST_P(SSLClientSocketVersionTest, Connect)
{
    ASSERT_TRUE(StartEmbeddedTestServer(EmbeddedTestServer::CERT_OK, GetServerConfig()));

    TestCompletionCallback callback;
    auto transport = std::make_unique<TCPClientSocket>(addr(), nullptr, nullptr, NetLog::Get(), NetLogSource());
    int rv = callback.GetResult(transport->Connect(callback.callback()));
    EXPECT_THAT(rv, IsOk());

    std::unique_ptr<SSLClientSocket> sock(CreateSSLClientSocket(std::move(transport), host_port_pair(), SSLConfig()));
...
{
    ASSERT_TRUE(StartEmbeddedTestServer(EmbeddedTestServer::CERT_EXPIRED, GetServerConfig()));
...
TEST_P(SSLClientSocketVersionTest, SocketDestroyedDuringVerify)
{
    ...
    context_
        = std::make_unique<SSLClientContext>(ssl_config_service_.get(), &verifier, transport_security_state_.get(), ssl_client_session_cache_.get(), nullptr);
...
{
    ASSERT_TRUE(StartEmbeddedTestServer(EmbeddedTestServer::CERT_MISMATCHED_NAME, GetServerConfig()));
...
{
    ASSERT_TRUE(StartEmbeddedTestServer(EmbeddedTestServer::CERT_BAD_VALIDITY, GetServerConfig()));
    cert_verifier_->set_default_result(ERR_CERT_DATE_INVALID);
...
TEST_P(SSLClientSocketReadTest, Read)
{
    ...
    TestCompletionCallback callback;
    auto transport = std::make_unique<TCPClientSocket>(addr(), nullptr, nullptr, nullptr, NetLogSource());
    ...
    const char request_text[] = "GET / HTTP/1.0\r\n\r\n";
    auto request_buffer = base::MakeRefCounted<IOBufferWithSize>(std::size(request_text) - 1);
    memcpy(request_buffer->data(), request_text, std::size(request_text) - 1);

    rv = callback.GetResult(sock->Write(request_buffer.get(), std::size(request_text) - 1, callback.callback(), TRAFFIC_ANNOTATION_FOR_TESTS));
    ...
    EXPECT_GE(sock->GetTotalReceivedBytes() - network_bytes_read_during_handshake, unencrypted_bytes_read);
...
{
    ASSERT_TRUE(StartEmbeddedTestServer(EmbeddedTestServer::CERT_OK, SSLServerConfig()));

    TestCompletionCallback callback;
    auto real_transport = std::make_unique<TCPClientSocket>(addr(), nullptr, nullptr, nullptr, NetLogSource());
    auto transport = std::make_unique<SynchronousErrorStreamSocket>(std::move(real_transport));
    int rv = callback.GetResult(transport->Connect(callback.callback()));
    EXPECT_THAT(rv, IsOk());
...
TEST_P(SSLClientSocketReadTest, Read_WithSynchronousError)
{
    ...
    SynchronousErrorStreamSocket* raw_transport = transport.get();
    std::unique_ptr<SSLClientSocket> sock(CreateSSLClientSocket(std::move(transport), host_port_pair(), config));
...
TEST_P(SSLClientSocketVersionTest, Write_WithSynchronousError)
{
    ...
    auto error_socket = std::make_unique<SynchronousErrorStreamSocket>(std::move(real_transport));
    SynchronousErrorStreamSocket* raw_error_socket = error_socket.get();
    auto transport = std::make_unique<FakeBlockingStreamSocket>(std::move(error_socket));
    FakeBlockingStreamSocket* raw_transport = transport.get();
    int rv = callback.GetResult(transport->Connect(callback.callback()));
    EXPECT_THAT(rv, IsOk());
...
TEST_P(SSLClientSocketVersionTest, Write_WithSynchronousErrorNoRead)
{
    ...
    auto error_socket = std::make_unique<SynchronousErrorStreamSocket>(std::move(real_transport));
    SynchronousErrorStreamSocket* raw_error_socket = error_socket.get();
    auto counting_socket = std::make_unique<CountingStreamSocket>(std::move(error_socket));
    ...
    std::unique_ptr<SSLClientSocket> sock(CreateSSLClientSocket(std::move(counting_socket), host_port_pair(), SSLConfig()));
    ...
    base::RunLoop loop;
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, loop.QuitClosure(), base::Milliseconds(100));
    loop.Run();
...
TEST_P(SSLClientSocketReadTest, Read_FullDuplex)
{
    ...
    rv = callback2.GetResult(sock_->Write(request_buffer.get(), request_text.size(), callback2.callback(), TRAFFIC_ANNOTATION_FOR_TESTS));
    EXPECT_EQ(static_cast<int>(request_text.size()), rv);

    // Now get the Read result.
    read_rv = WaitForReadCompletion(sock_.get(), buf.get(), 4096, &callback, read_rv);
    EXPECT_GT(read_rv, 0);
}
...
TEST_P(SSLClientSocketReadTest, Read_DeleteWhilePendingFullDuplex)
{
    ...
    scoped_refptr<DrainableIOBuffer> request_buffer
        = base::MakeRefCounted<DrainableIOBuffer>(base::MakeRefCounted<StringIOBuffer>(request_text), request_text.size());
    ...
    rv = raw_sock->Write(request_buffer.get(), request_buffer->BytesRemaining(), callback.callback(), TRAFFIC_ANNOTATION_FOR_TESTS);
    ASSERT_THAT(rv, IsError(ERR_IO_PENDING));
...
TEST_P(SSLClientSocketReadTest, Read_WithWriteError)
{
    ...
    scoped_refptr<DrainableIOBuffer> long_request_buffer
        = base::MakeRefCounted<DrainableIOBuffer>(base::MakeRefCounted<StringIOBuffer>(long_request_text), long_request_text.size());
    ...
    do {
        rv = callback.GetResult(
            sock->Write(long_request_buffer.get(), long_request_buffer->BytesRemaining(), callback.callback(), TRAFFIC_ANNOTATION_FOR_TESTS));
        if (rv > 0) {
...
TEST_P(SSLClientSocketReadTest, Read_WithFatalAlert)
{
    ...
    TestCompletionCallback callback;
    auto buf = base::MakeRefCounted<IOBufferWithSize>(4096);
    EXPECT_EQ(ERR_SSL_PROTOCOL_ERROR, ReadAndWaitForCompletion(sock_.get(), buf.get(), 4096));
}
...
TEST_P(SSLClientSocketReadTest, Read_SmallChunks)
{
    ...
    TestCompletionCallback callback;
    rv = callback.GetResult(sock_->Write(request_buffer.get(), std::size(request_text) - 1, callback.callback(), TRAFFIC_ANNOTATION_FOR_TESTS));
    EXPECT_EQ(static_cast<int>(std::size(request_text) - 1), rv);
...
TEST_P(SSLClientSocketReadTest, Read_ManySmallRecords)
{
    ...
    auto real_transport = std::make_unique<TCPClientSocket>(addr(), nullptr, nullptr, nullptr, NetLogSource());
    auto transport = std::make_unique<ReadBufferingStreamSocket>(std::move(real_transport));
...
TEST_P(SSLClientSocketReadTest, Read_FullLogging)
{
    ...
    auto entries = log_observer_.GetEntries();
    size_t last_index = ExpectLogContainsSomewhereAfter(entries, 5, NetLogEventType::SSL_SOCKET_BYTES_SENT, NetLogEventPhase::NONE);
    ...
    for (;;) {
        ...
        entries = log_observer_.GetEntries();
        last_index = ExpectLogContainsSomewhereAfter(entries, last_index + 1, NetLogEventType::SSL_SOCKET_BYTES_RECEIVED, NetLogEventPhase::NONE);
    }
}
...
TEST_F(SSLClientSocketTest, PrematureApplicationData)
{
    ...
    static const unsigned char application_data[] = { 0x17, 0x03, 0x01, 0x00, 0x4a, 0x02, 0x00, 0x00, 0x46, 0x03, 0x01, 0x4b, 0xc2, 0xf8, 0xb2, 0xc1, 0x56,
        0x42, 0xb9, 0x57, 0x7f, 0xde, 0x87, 0x46, 0xf7, 0xa3, 0x52, 0x42, 0x21, 0xf0, 0x13, 0x1c, 0x9c, 0x83, 0x88, 0xd6, 0x93, 0x0c, 0xf6, 0x36, 0x30, 0x05,
        0x7e, 0x20, 0xb5, 0xb5, 0x73, 0x36, 0x53, 0x83, 0x0a, 0xfc, 0x17, 0x63, 0xbf, 0xa0, 0xe4, 0x42, 0x90, 0x0d, 0x2f, 0x18, 0x6d, 0x20, 0xd8, 0x36, 0x3f,
        0xfc, 0xe6, 0x01, 0xfa, 0x0f, 0xa5, 0x75, 0x7f, 0x09, 0x00, 0x04, 0x00, 0x16, 0x03, 0x01, 0x11, 0x57, 0x0b, 0x00, 0x11, 0x53, 0x00, 0x11, 0x50, 0x00,
        0x06, 0x22, 0x30, 0x82, 0x06, 0x1e, 0x30, 0x82, 0x05, 0x06, 0xa0, 0x03, 0x02, 0x01, 0x02, 0x02, 0x0a };

    // All reads and writes complete synchronously (async=false).
    MockRead data_reads[] = {
        MockRead(SYNCHRONOUS, reinterpret_cast<const char*>(application_data), std::size(application_data)),
    ...
    TestCompletionCallback callback;
    std::unique_ptr<StreamSocket> transport(std::make_unique<MockTCPClientSocket>(addr(), nullptr, &data));
    int rv = callback.GetResult(transport->Connect(callback.callback()));
    EXPECT_THAT(rv, IsOk());
...
TEST_F(SSLClientSocketTest, ClientSocketHandleNotFromPool)
{
    ...
    std::unique_ptr<SSLClientSocket> sock(socket_factory_->CreateSSLClientSocket(context_.get(), std::move(transport), host_port_pair(), SSLConfig()));
...
TEST_P(SSLClientSocketVersionTest, ExportKeyingMaterial)
{
    ...
    rv = sock_->ExportKeyingMaterial(kKeyingLabel1, false, kKeyingContext1, client_out1, sizeof(client_out1));
    EXPECT_EQ(rv, OK);
    ...
    rv = sock_->ExportKeyingMaterial(kKeyingLabel2, false, kKeyingContext1, client_out2, sizeof(client_out2));
    EXPECT_EQ(rv, OK);
    EXPECT_NE(memcmp(client_out1, client_out2, kKeyingMaterialSize), 0);

    const char kKeyingContext2[] = "context";
    rv = sock_->ExportKeyingMaterial(kKeyingLabel1, true, kKeyingContext2, client_out2, sizeof(client_out2));
    EXPECT_EQ(rv, OK);
    EXPECT_NE(memcmp(client_out1, client_out2, kKeyingMaterialSize), 0);
    ...
    memset(client_out2, 0, sizeof(client_out2));
    rv = sock_->ExportKeyingMaterial(kKeyingLabel1, true, kKeyingContext1, client_out2, sizeof(client_out2));
    EXPECT_EQ(rv, OK);
...
TEST_P(SSLClientSocketVersionTest, VerifyServerChainProperlyOrdered)
{
    ...
    ASSERT_TRUE(StartEmbeddedTestServer(EmbeddedTestServer::CERT_CHAIN_WRONG_ROOT, GetServerConfig()));
    ...
    CertificateList server_certs = CreateCertificateListFromFile(GetTestCertsDirectory(), "redundant-server-chain.pem", X509Certificate::FORMAT_AUTO);
    ...
    EXPECT_TRUE(x509_util::CryptoBufferEqual(server_certificate->cert_buffer(), server_certs[0]->cert_buffer()));

    ASSERT_EQ(3U, server_intermediates.size());

    EXPECT_TRUE(x509_util::CryptoBufferEqual(server_intermediates[0].get(), server_certs[1]->cert_buffer()));
    EXPECT_TRUE(x509_util::CryptoBufferEqual(server_intermediates[1].get(), server_certs[2]->cert_buffer()));
    EXPECT_TRUE(x509_util::CryptoBufferEqual(server_intermediates[2].get(), server_certs[3]->cert_buffer()));
...
TEST_P(SSLClientSocketVersionTest, VerifyReturnChainProperlyOrdered)
{
    ...
    CertificateList unverified_certs = CreateCertificateListFromFile(GetTestCertsDirectory(), "redundant-server-chain.pem", X509Certificate::FORMAT_AUTO);
    ASSERT_EQ(4u, unverified_certs.size());

    // We will expect SSLInfo to ultimately contain this chain.
    CertificateList certs = CreateCertificateListFromFile(GetTestCertsDirectory(), "redundant-validated-chain.pem", X509Certificate::FORMAT_AUTO);
    ...
    verify_result.verified_cert = X509Certificate::CreateFromBuffer(bssl::UpRef(certs[0]->cert_buffer()), std::move(temp_intermediates));
    ...
    scoped_refptr<X509Certificate> root_cert = ImportCertFromFile(GetTestCertsDirectory(), "redundant-validated-chain-root.pem");
    ...
    EXPECT_TRUE(x509_util::CryptoBufferEqual(ssl_info.cert->cert_buffer(), certs[0]->cert_buffer()));
    EXPECT_TRUE(x509_util::CryptoBufferEqual(intermediates[0].get(), certs[1]->cert_buffer()));
    EXPECT_TRUE(x509_util::CryptoBufferEqual(intermediates[1].get(), certs[2]->cert_buffer()));

    // Verify that SSLInfo also contains the chain as received from the server.
    ASSERT_TRUE(ssl_info.unverified_cert);
    const auto& served_intermediates = ssl_info.unverified_cert->intermediate_buffers();
    ASSERT_EQ(3U, served_intermediates.size());
    EXPECT_TRUE(x509_util::CryptoBufferEqual(ssl_info.cert->cert_buffer(), unverified_certs[0]->cert_buffer()));
    EXPECT_TRUE(x509_util::CryptoBufferEqual(served_intermediates[0].get(), unverified_certs[1]->cert_buffer()));
    EXPECT_TRUE(x509_util::CryptoBufferEqual(served_intermediates[1].get(), unverified_certs[2]->cert_buffer()));
    EXPECT_TRUE(x509_util::CryptoBufferEqual(served_intermediates[2].get(), unverified_certs[3]->cert_buffer()));
...
// Client certificates are disabled on iOS.
#if BUILDFLAG(ENABLE_CLIENT_CERTIFICATES)
INSTANTIATE_TEST_SUITE_P(TLSVersion, SSLClientSocketCertRequestInfoTest, ValuesIn(GetTLSVersions()));

TEST_P(SSLClientSocketCertRequestInfoTest, DontRequestClientCertsIfServerCertInvalid)
{
    SSLServerConfig config = GetServerConfig();
    config.client_cert_type = SSLServerConfig::OPTIONAL_CLIENT_CERT;
    ASSERT_TRUE(StartEmbeddedTestServer(EmbeddedTestServer::CERT_EXPIRED, config));
...
{
    const unsigned char kThawteDN[] = { 0x30, 0x4c, 0x31, 0x0b, 0x30, 0x09, 0x06, 0x03, 0x55, 0x04, 0x06, 0x13, 0x02, 0x5a, 0x41, 0x31, 0x25, 0x30, 0x23, 0x06,
        0x03, 0x55, 0x04, 0x0a, 0x13, 0x1c, 0x54, 0x68, 0x61, 0x77, 0x74, 0x65, 0x20, 0x43, 0x6f, 0x6e, 0x73, 0x75, 0x6c, 0x74, 0x69, 0x6e, 0x67, 0x20, 0x28,
        0x50, 0x74, 0x79, 0x29, 0x20, 0x4c, 0x74, 0x64, 0x2e, 0x31, 0x16, 0x30, 0x14, 0x06, 0x03, 0x55, 0x04, 0x03, 0x13, 0x0d, 0x54, 0x68, 0x61, 0x77, 0x74,
        0x65, 0x20, 0x53, 0x47, 0x43, 0x20, 0x43, 0x41 };

    const unsigned char kDiginotarDN[] = { 0x30, 0x5f, 0x31, 0x0b, 0x30, 0x09, 0x06, 0x03, 0x55, 0x04, 0x06, 0x13, 0x02, 0x4e, 0x4c, 0x31, 0x12, 0x30, 0x10,
        0x06, 0x03, 0x55, 0x04, 0x0a, 0x13, 0x09, 0x44, 0x69, 0x67, 0x69, 0x4e, 0x6f, 0x74, 0x61, 0x72, 0x31, 0x1a, 0x30, 0x18, 0x06, 0x03, 0x55, 0x04, 0x03,
        0x13, 0x11, 0x44, 0x69, 0x67, 0x69, 0x4e, 0x6f, 0x74, 0x61, 0x72, 0x20, 0x52, 0x6f, 0x6f, 0x74, 0x20, 0x43, 0x41, 0x31, 0x20, 0x30, 0x1e, 0x06, 0x09,
        0x2a, 0x86, 0x48, 0x86, 0xf7, 0x0d, 0x01, 0x09, 0x01, 0x16, 0x11, 0x69, 0x6e, 0x66, 0x6f, 0x40, 0x64, 0x69, 0x67, 0x69, 0x6e, 0x6f, 0x74, 0x61, 0x72,
        0x2e, 0x6e, 0x6c };

    SSLServerConfig config = GetServerConfig();
    config.client_cert_type = SSLServerConfig::OPTIONAL_CLIENT_CERT;
    config.cert_authorities.emplace_back(std::begin(kThawteDN), std::end(kThawteDN));
    config.cert_authorities.emplace_back(std::begin(kDiginotarDN), std::end(kDiginotarDN));
    ASSERT_TRUE(StartEmbeddedTestServer(EmbeddedTestServer::CERT_OK, config));
    scoped_refptr<SSLCertRequestInfo> request_info = GetCertRequest();
    ASSERT_TRUE(request_info.get());
...
TEST_P(SSLClientSocketCertRequestInfoTest, CertKeyTypes)
{
    ...
    EXPECT_THAT(request_info->signature_algorithms, testing::Contains(SSL_SIGN_ECDSA_SECP256R1_SHA256));
    EXPECT_THAT(request_info->signature_algorithms, testing::Contains(SSL_SIGN_RSA_PSS_RSAE_SHA256));
}
#endif // BUILDFLAG(ENABLE_CLIENT_CERTIFICATES)
...
TEST_P(SSLClientSocketVersionTest, ConnectSignedCertTimestampsTLSExtension)
{
    ...
    SSLServerConfig server_config = GetServerConfig();
    server_config.signed_cert_timestamp_list = std::vector<uint8_t>(sct_ext.begin(), sct_ext.end());
    ASSERT_TRUE(StartEmbeddedTestServer(EmbeddedTestServer::CERT_OK, server_config));
    ...
    EXPECT_TRUE(params.certificate()->EqualsIncludingChain(embedded_test_server()->GetCertificate().get()));
...
TEST_P(SSLClientSocketVersionTest, ReuseStates)
{
    ...
    TestCompletionCallback callback;
    rv = callback.GetResult(sock_->Write(request_buffer.get(), kRequestLen, callback.callback(), TRAFFIC_ANNOTATION_FOR_TESTS));
...
TEST_P(SSLClientSocketVersionTest, ReusableAfterWrite)
{
    ...
    auto transport = std::make_unique<FakeBlockingStreamSocket>(std::move(real_transport));
    FakeBlockingStreamSocket* raw_transport = transport.get();
    ASSERT_THAT(callback.GetResult(transport->Connect(callback.callback())), IsOk());
    ...
    EXPECT_EQ(
        static_cast<int>(kRequestLen), callback.GetResult(sock->Write(request_buffer.get(), kRequestLen, callback.callback(), TRAFFIC_ANNOTATION_FOR_TESTS)));
...
TEST_P(SSLClientSocketVersionTest, SessionResumption)
{
    ...
    TestCompletionCallback callback;
    ASSERT_THAT(callback.GetResult(transport->Connect(callback.callback())), IsOk());
    std::unique_ptr<SSLClientSocket> sock = CreateSSLClientSocket(std::move(transport), HostPortPair("example.com", 443), ssl_config);
    ASSERT_THAT(callback.GetResult(sock->Connect(callback.callback())), IsOk());
...
public:
    FakePeerAddressSocket(std::unique_ptr<StreamSocket> socket, const IPEndPoint& address)
        : WrappedStreamSocket(std::move(socket))
        , address_(address)
    {
    }
...
TEST_F(SSLClientSocketTest, SessionResumption_RSA)
{
    for (bool use_rsa : { false, true }) {
        ...
        SSLServerConfig server_config;
        server_config.version_max = SSL_PROTOCOL_VERSION_TLS1_2;
        server_config.cipher_suite_for_testing = use_rsa ? kRSACipher : kModernTLS12Cipher;
        ASSERT_TRUE(StartEmbeddedTestServer(EmbeddedTestServer::CERT_OK, server_config));
        SSLConfig ssl_config;
        ssl_client_session_cache_->Flush();
        ...
        for (int i = 0; i < 3; i++) {
            ...
            auto socket = std::make_unique<FakePeerAddressSocket>(std::move(transport), fake_peer_address);
            std::unique_ptr<SSLClientSocket> sock = CreateSSLClientSocket(std::move(socket), HostPortPair("example.com", 443), ssl_config);
            ASSERT_THAT(callback.GetResult(sock->Connect(callback.callback())), IsOk());
            SSLInfo ssl_info;
            ASSERT_TRUE(sock->GetSSLInfo(&ssl_info));
            sock.reset();
            ...
            case 2:
                ...
                EXPECT_EQ(use_rsa ? SSLInfo::HANDSHAKE_FULL : SSLInfo::HANDSHAKE_RESUME, ssl_info.handshake_type);
                break;
            default:
                NOTREACHED();
            }
...
// feature is disabled.
TEST_P(SSLClientSocketVersionTest, SessionResumptionNetworkIsolationKeyDisabled)
{
    ...
    ssl_config.network_anonymization_key = NetworkAnonymizationKey::CreateSameSite(kSiteA);
    ASSERT_TRUE(CreateAndConnectSSLClientSocket(ssl_config, &rv));
    ASSERT_THAT(rv, IsOk());
    ASSERT_TRUE(sock_->GetSSLInfo(&ssl_info));
    EXPECT_EQ(SSLInfo::HANDSHAKE_RESUME, ssl_info.handshake_type);
    ...
    ssl_config.network_anonymization_key = NetworkAnonymizationKey::CreateSameSite(kSiteB);
    ASSERT_TRUE(CreateAndConnectSSLClientSocket(ssl_config, &rv));
    ASSERT_THAT(rv, IsOk());
    ASSERT_TRUE(sock_->GetSSLInfo(&ssl_info));
    EXPECT_EQ(SSLInfo::HANDSHAKE_RESUME, ssl_info.handshake_type);
...
// feature is enabled.
TEST_P(SSLClientSocketVersionTest, SessionResumptionNetworkIsolationKeyEnabled)
{
    base::test::ScopedFeatureList feature_list;
    ...
    const auto kNetworkAnonymizationKeyA = NetworkAnonymizationKey::CreateSameSite(kSiteA);
    const auto kNetworkAnonymizationKeyB = NetworkAnonymizationKey::CreateSameSite(kSiteB);
...
TEST_F(SSLClientSocketFalseStartTest, NoSessionResumptionBeforeFinished)
{
    ...
    ASSERT_NO_FATAL_FAILURE(CreateAndConnectUntilServerFinishedReceived(client_config, &callback, &raw_transport1, &sock1));
...
TEST_P(SSLClientSocketVersionTest, SendGoodCert)
{
    ...
    scoped_refptr<X509Certificate> client_cert = ImportCertFromFile(certs_dir, "client_1.pem");
    ...
    context_->SetClientCertificate(host_port_pair(), client_cert, key_util::LoadPrivateKeyOpenSSL(certs_dir.AppendASCII("client_1.key")));
...
TEST_F(SSLClientSocketTest, ClearSessionCacheOnClientCertChange)
{
    ...
    context_->SetClientCertificate(
        host_port_pair(), ImportCertFromFile(certs_dir, "client_1.pem"), key_util::LoadPrivateKeyOpenSSL(certs_dir.AppendASCII("client_1.key")));
...
TEST_F(SSLClientSocketTest, ClearSessionCacheOnClientCertDatabaseChange)
{
    ...
    EXPECT_CALL(observer, OnSSLConfigForServersChanged(base::flat_set<HostPortPair>({ host_port_pair() })));
    EXPECT_CALL(observer, OnSSLConfigForServersChanged(base::flat_set<HostPortPair>({ host_port_pair2 })));
    EXPECT_CALL(observer, OnSSLConfigForServersChanged(base::flat_set<HostPortPair>({ host_port_pair(), host_port_pair2 })));
    ...
    context_->SetClientCertificate(
        host_port_pair2, ImportCertFromFile(certs_dir, "client_2.pem"), key_util::LoadPrivateKeyOpenSSL(certs_dir.AppendASCII("client_2.key")));
...
TEST_F(SSLClientSocketTest, DontClearEmptyClientCertCache)
{
    ...
    base::FilePath certs_dir = GetTestCertsDirectory();
    scoped_refptr<net::X509Certificate> certificate1 = ImportCertFromFile(certs_dir, "client_1.pem");
    ...
    auto entries = log_observer_.GetEntriesWithType(NetLogEventType::CLEAR_CACHED_CLIENT_CERT);
    ASSERT_EQ(1u, entries.size());
    EXPECT_EQ(GetStringValueFromParams(entries[0], "host"), host_port_pair().ToString());
...
TEST_F(SSLClientSocketTest, DontClearMatchingClientCertificates)
{
    ...
    base::FilePath certs_dir = GetTestCertsDirectory();
    scoped_refptr<net::X509Certificate> certificate1 = ImportCertFromFile(certs_dir, "client_1.pem");
    scoped_refptr<net::SSLPrivateKey> private_key1 = key_util::LoadPrivateKeyOpenSSL(certs_dir.AppendASCII("client_1.key"));
    ...
    EXPECT_EQ(context_->GetClientCertificateCachedServersForTesting().size(), 1U);
    EXPECT_TRUE(context_->GetClientCertificateCachedServersForTesting().contains(host_port_pair()));
    EXPECT_EQ(context_->ssl_client_session_cache()->size(), 1U);
...
TEST_F(SSLClientSocketTest, ClearMismatchingClientCertificates)
{
    ...
    testing::StrictMock<MockSSLClientContextObserver> observer;
    EXPECT_CALL(observer, OnSSLConfigForServersChanged(base::flat_set<HostPortPair>({ host_port_pair() }))).Times(2);
    context_->AddObserver(&observer);
    ...
    scoped_refptr<net::X509Certificate> certificate2 = ImportCertFromFile(certs_dir, "client_2.pem");
...
TEST_F(SSLClientSocketTest, ClearMismatchingClientCertificatesWithNullParameter)
{
    SSLServerConfig server_config;
    // TLS 1.3 reports client certificate errors after the handshake, so test at
    // TLS 1.2 for simplicity.
...
TEST_F(SSLClientSocketTest, ClearMismatchingClientCertificatesWithNullCachedCert)
{
    SSLServerConfig server_config;
    // TLS 1.3 reports client certificate errors after the handshake, so test at
    // TLS 1.2 for simplicity.
...
TEST_F(SSLClientSocketTest, ClearMatchingCertDontClearEmptyClientCertCache)
{
    ...
    auto entries = log_observer_.GetEntriesWithType(NetLogEventType::CLEAR_MATCHING_CACHED_CLIENT_CERT);
    ASSERT_EQ(1u, entries.size());

    const auto& log_entry = entries[0];
    ASSERT_FALSE(log_entry.params.empty());

    const base::Value::List* hosts_values = log_entry.params.FindListByDottedPath("hosts");
    ASSERT_TRUE(hosts_values);
    ASSERT_TRUE(hosts_values->empty());

    const base::Value::List* certificates_values = log_entry.params.FindListByDottedPath("certificates");
...
TEST_F(SSLClientSocketTest, ClearMatchingCertSingleMatching)
{
    ...
    HostPortPair host_port_pair2("example.com", 42);
    scoped_refptr<net::X509Certificate> certificate2 = ImportCertFromFile(certs_dir, "client_2.pem");
    scoped_refptr<net::SSLPrivateKey> private_key2 = key_util::LoadPrivateKeyOpenSSL(certs_dir.AppendASCII("client_2.key"));
    ...
    context_->RemoveObserver(&observer);
    auto cached_servers_with_decision = context_->GetClientCertificateCachedServersForTesting();
...
TEST_F(SSLClientSocketTest, DontClearSessionCacheOnServerCertDatabaseChange)
{
    ...
    EXPECT_CALL(observer, OnSSLConfigForServersChanged(base::flat_set<HostPortPair>({ host_port_pair() })));
    EXPECT_CALL(observer, OnSSLConfigForServersChanged(base::flat_set<HostPortPair>({ host_port_pair2 })));
    EXPECT_CALL(observer, OnSSLConfigChanged(SSLClientContext::SSLConfigChangeType::kCertDatabaseChanged));
...
{
    base::FilePath certs_dir = GetTestCertsDirectory();
    scoped_refptr<net::X509Certificate> client_cert = ImportCertFromFile(certs_dir, "client_1.pem");
    scoped_refptr<net::SSLPrivateKey> client_key = key_util::LoadPrivateKeyOpenSSL(certs_dir.AppendASCII("client_1.key"));
    ...
    const struct {
    ...
    } kTests[] = {
        {
            ...
            .server_prefs = { SSL_SIGN_RSA_PSS_RSAE_SHA384, SSL_SIGN_RSA_PSS_RSAE_SHA256 },
            .client_prefs = { SSL_SIGN_RSA_PSS_RSAE_SHA256, SSL_SIGN_RSA_PSS_RSAE_SHA384 },
        ...
        },
        {
            ...
            .version = SSL_PROTOCOL_VERSION_TLS1_3,
            .server_prefs = { SSL_SIGN_RSA_PKCS1_SHA256_LEGACY, SSL_SIGN_RSA_PSS_RSAE_SHA256 },
            .client_prefs = { SSL_SIGN_RSA_PKCS1_SHA256, SSL_SIGN_RSA_PSS_RSAE_SHA256 },
    ...
    for (const auto& test : kTests) {
        ...
        scoped_feature_list.InitWithFeatureState(net::features::kLegacyPKCS1ForTLS13, test.legacy_pkcs1_enabled);
        ...
        context_->SetClientCertificate(host_port_pair(), client_cert, WrapSSLPrivateKeyWithPreferences(client_key, test.client_prefs));
        int rv;
        ASSERT_TRUE(CreateAndConnectSSLClientSocket(SSLConfig(), &rv));
        if (test.error != OK) {
        ...
        EXPECT_EQ(server_ssl_info->peer_signature_algorithm, test.expected_signature_algorithm);
    }
}
#endif // BUILDFLAG(ENABLE_CLIENT_CERTIFICATES)
...
TEST_P(SSLClientSocketVersionTest, PKPBypassedSet)
{
    ...
    ASSERT_TRUE(StartEmbeddedTestServer(EmbeddedTestServer::CERT_OK, GetServerConfig()));
    scoped_refptr<X509Certificate> server_cert = embedded_test_server()->GetCertificate();
    ...
    verify_result.public_key_hashes = MakeHashValueVector(kBadHashValueVectorInput);
    ...
    ASSERT_TRUE(CreateAndConnectSSLClientSocketWithHost(ssl_config, new_host_port_pair, &rv));
    SSLInfo ssl_info;
    ASSERT_TRUE(sock_->GetSSLInfo(&ssl_info));
...
class SSLClientSocketKeyUsageTest : public SSLClientSocketTest, public ::testing::WithParamInterface<std::tuple<KeyUsageTest, bool /*known_root*/>> { };

const KeyUsageTest kKeyUsageTests[] = {
    // keyUsage matches cipher suite.
    { EmbeddedTestServer::CERT_KEY_USAGE_RSA_DIGITAL_SIGNATURE, kSigningCipher, true },
    { EmbeddedTestServer::CERT_KEY_USAGE_RSA_ENCIPHERMENT, kEncryptingCipher, true },
    // keyUsage does not match cipher suite.
    { EmbeddedTestServer::CERT_KEY_USAGE_RSA_ENCIPHERMENT, kSigningCipher, false },
    { EmbeddedTestServer::CERT_KEY_USAGE_RSA_DIGITAL_SIGNATURE, kEncryptingCipher, false },
};
...
TEST_P(SSLClientSocketKeyUsageTest, RSAKeyUsage)
{
    ...
    verify_result.verified_cert = server_cert;
    verify_result.public_key_hashes = MakeHashValueVector(kGoodHashValueVectorInput);
    cert_verifier_->AddResultForCert(server_cert.get(), verify_result, OK);
...
INSTANTIATE_TEST_SUITE_P(RSAKeyUsageInstantiation, SSLClientSocketKeyUsageTest, Combine(ValuesIn(kKeyUsageTests), Bool()));
...
TEST_P(SSLClientSocketVersionTest, CTIsRequired)
{
    ...
    verify_result.verified_cert = server_cert;
    verify_result.public_key_hashes = MakeHashValueVector(kGoodHashValueVectorInput);
    verify_result.policy_compliance = ct::CTPolicyCompliance::CT_POLICY_NOT_ENOUGH_SCTS;
    cert_verifier_->AddResultForCert(server_cert.get(), verify_result, OK);
    ...
    EXPECT_CALL(require_ct_delegate, IsCTRequiredForHost(_, _, _))
        .WillRepeatedly(Return(TransportSecurityState::RequireCTDelegate::CTRequirementLevel::NOT_REQUIRED));
    EXPECT_CALL(require_ct_delegate, IsCTRequiredForHost(host_port_pair().host(), _, _))
...
TEST_P(SSLClientSocketVersionTest, PKPMoreImportantThanCT)
{
    ...
    ASSERT_TRUE(CreateAndConnectSSLClientSocketWithHost(ssl_config, ct_host_port_pair, &rv));
    SSLInfo ssl_info;
    ASSERT_TRUE(sock_->GetSSLInfo(&ssl_info));
...
TEST_P(SSLClientSocketVersionTest, SCTAuditingReportCollected)
{
    ...
    verify_result.verified_cert = server_cert;
    verify_result.public_key_hashes = MakeHashValueVector(kGoodHashValueVectorInput);
    verify_result.policy_compliance = ct::CTPolicyCompliance::CT_POLICY_COMPLIES_VIA_SCTS;
    cert_verifier_->AddResultForCert(server_cert.get(), verify_result, OK);
    ...
    context_ = std::make_unique<SSLClientContext>(
        ssl_config_service_.get(), cert_verifier_.get(), transport_security_state_.get(), ssl_client_session_cache_.get(), &sct_auditing_delegate);

    EXPECT_CALL(sct_auditing_delegate, IsSCTAuditingEnabled()).WillRepeatedly(Return(true));
    EXPECT_CALL(sct_auditing_delegate, MaybeEnqueueReport(host_port_pair(), server_cert.get(), _)).Times(1);
...
TEST_F(SSLClientSocketTest, HandshakeFailureServerHello)
{
    ...
    raw_transport->ReplaceReadResult(FormatTLS12Alert(40 /* AlertDescription.handshake_failure */));
    raw_transport->UnblockReadResult();
...
TEST_F(SSLClientSocketTest, AccessDeniedNoClientCerts)
{
    ...
    // Replace it with an alert.
    raw_transport->ReplaceReadResult(FormatTLS12Alert(49 /* AlertDescription.access_denied */));
    raw_transport->UnblockReadResult();
...
TEST_F(SSLClientSocketZeroRTTTest, ZeroRTTEarlyDataLimit)
{
    ...
    int write_rv = ssl_socket()->Write(write_buf.get(), request.size(), write_callback.callback(), TRAFFIC_ANNOTATION_FOR_TESTS);
    ...
    TestCompletionCallback read_callback;
    auto read_buf = base::MakeRefCounted<IOBufferWithSize>(4096);
    int read_rv = ssl_socket()->Read(read_buf.get(), 4096, read_callback.callback());
    ASSERT_THAT(read_rv, IsError(ERR_IO_PENDING));
    ...
    socket->UnblockReadResult();
    EXPECT_EQ(static_cast<int>(request.size()), write_callback.GetResult(write_rv));
...
TEST_F(SSLClientSocketZeroRTTTest, ZeroRTTEarlyDataLimitCancelReadIfReady)
{
    ...
    TestCompletionCallback read_callback;
    auto read_buf = base::MakeRefCounted<IOBufferWithSize>(4096);
    int read_rv = ssl_socket()->ReadIfReady(read_buf.get(), 4096, read_callback.callback());
    ASSERT_THAT(read_rv, IsError(ERR_IO_PENDING));
    ...
    read_rv = read_callback2.GetResult(ssl_socket()->Read(read_buf.get(), 4096, read_callback2.callback()));
    ASSERT_GT(read_rv, 0);
}
...
TEST_F(SSLClientSocketZeroRTTTest, ZeroRTTConfirmHandshake)
{
    ...
    TestCompletionCallback callback;
    ASSERT_EQ(ERR_IO_PENDING, ssl_socket()->ConfirmHandshake(callback.callback()));
    base::RunLoop().RunUntilIdle();
    EXPECT_FALSE(callback.have_result());
...
TEST_F(SSLClientSocketZeroRTTTest, ZeroRTTReadBeforeWrite)
{
    ...
    auto buf = base::MakeRefCounted<IOBufferWithSize>(4096);
    TestCompletionCallback read_callback;
    ASSERT_EQ(ERR_IO_PENDING, ssl_socket()->Read(buf.get(), 4096, read_callback.callback()));
...
TEST_F(SSLClientSocketZeroRTTTest, ZeroRTTDoubleConfirmHandshake)
{
    ...
    ASSERT_THAT(Connect(), IsOk());
    TestCompletionCallback callback;
    ASSERT_THAT(callback.GetResult(ssl_socket()->ConfirmHandshake(callback.callback())), IsOk());
    // After the handshake is confirmed, ConfirmHandshake should return
    // synchronously.
    ASSERT_THAT(ssl_socket()->ConfirmHandshake(callback.callback()), IsOk());
    constexpr std::string_view kRequest = "GET /zerortt HTTP/1.0\r\n\r\n";
...
TEST_P(SSLClientSocketReadTest, IdleAfterRead)
{
    ...
    ASSERT_THAT(server_listener.Listen(IPEndPoint(IPAddress::IPv4Localhost(), 0), 1, /*ipv6_only=*/std::nullopt), IsOk());
    ...
    auto client_transport = std::make_unique<TCPClientSocket>(AddressList(server_address), nullptr, nullptr, nullptr, NetLogSource());
    ...
    ASSERT_TRUE(cert);
    bssl::UniquePtr<EVP_PKEY> pkey = key_util::LoadEVP_PKEYFromPEM(certs_dir.AppendASCII("ok_cert.pem"));
    ASSERT_TRUE(pkey);
    std::unique_ptr<crypto::RSAPrivateKey> key = crypto::RSAPrivateKey::CreateFromKey(pkey.get());
    ASSERT_TRUE(key);
    std::unique_ptr<SSLServerContext> server_context = CreateSSLServerContext(cert.get(), *key.get(), GetServerConfig());

    // Complete the SSL handshake on both sides.
    std::unique_ptr<SSLClientSocket> client(CreateSSLClientSocket(std::move(client_transport), HostPortPair::FromIPEndPoint(server_address), SSLConfig()));
    std::unique_ptr<SSLServerSocket> server(server_context->CreateSSLServerSocket(std::move(server_transport)));
    ...
    server_rv = server->Write(write_buf.get(), 1, server_callback.callback(), TRAFFIC_ANNOTATION_FOR_TESTS);

    // Read that record on the server, but with a much larger buffer than
    // necessary.
    auto read_buf = base::MakeRefCounted<IOBufferWithSize>(1024);
    client_rv = Read(client.get(), read_buf.get(), 1024, client_callback.callback());

    EXPECT_EQ(1, server_callback.GetResult(server_rv));
    EXPECT_EQ(1, WaitForReadCompletion(client.get(), read_buf.get(), 1024, &client_callback, client_rv));
...
TEST_F(SSLClientSocketTest, SSLOverSSLBadCertificate)
{
    ...
    base::FilePath certs_dir = GetTestCertsDirectory();
    scoped_refptr<net::X509Certificate> ok_cert = ImportCertFromFile(certs_dir, "ok_cert.pem");
    ASSERT_TRUE(ok_cert);
    bssl::UniquePtr<EVP_PKEY> ok_pkey = key_util::LoadEVP_PKEYFromPEM(certs_dir.AppendASCII("ok_cert.pem"));
    ASSERT_TRUE(ok_pkey);

    scoped_refptr<net::X509Certificate> expired_cert = ImportCertFromFile(certs_dir, "expired_cert.pem");
    ASSERT_TRUE(expired_cert);
    bssl::UniquePtr<EVP_PKEY> expired_pkey = key_util::LoadEVP_PKEYFromPEM(certs_dir.AppendASCII("expired_cert.pem"));
    ...
    cert_verifier_->AddResultForCert(expired_cert, expired_result, ERR_CERT_DATE_INVALID);
    ...
    std::unique_ptr<crypto::RSAPrivateKey> ok_key = crypto::RSAPrivateKey::CreateFromKey(ok_pkey.get());
    ASSERT_TRUE(ok_key);
    std::unique_ptr<SSLServerContext> ok_server_context = CreateSSLServerContext(ok_cert.get(), *ok_key.get(), SSLServerConfig());

    std::unique_ptr<crypto::RSAPrivateKey> expired_key = crypto::RSAPrivateKey::CreateFromKey(expired_pkey.get());
    ASSERT_TRUE(expired_key);
    std::unique_ptr<SSLServerContext> expired_server_context = CreateSSLServerContext(expired_cert.get(), *expired_key.get(), SSLServerConfig());

    // Complete the proxy SSL handshake with ok_cert.pem. This should succeed.
    std::unique_ptr<SSLClientSocket> client = CreateSSLClientSocket(std::move(client_transport), HostPortPair("proxy.test", 443), SSLConfig());
    std::unique_ptr<SSLServerSocket> server = ok_server_context->CreateSSLServerSocket(std::move(server_transport));
    ...
    client = CreateSSLClientSocket(std::move(client), HostPortPair("server.test", 443), SSLConfig());
    ...
    EXPECT_THAT(client_callback.GetResult(client_rv), IsError(ERR_CERT_DATE_INVALID));
    SSLInfo ssl_info;
    ...
    EXPECT_THAT(server_callback.GetResult(server_rv), IsError(ERR_BAD_SSL_CLIENT_AUTH_CERT));
}
...
TEST_F(SSLClientSocketTest, Tag)
{
    ...
    auto tagging_sock = std::make_unique<MockTaggingStreamSocket>(std::move(transport));
    auto* tagging_sock_ptr = tagging_sock.get();

    // |sock| takes ownership of |tagging_sock|, but keep a
    // non-owning pointer to it.
    std::unique_ptr<SSLClientSocket> sock(CreateSSLClientSocket(std::move(tagging_sock), host_port_pair(), SSLConfig()));
...
TEST_F(SSLClientSocketTest, ECH)
{
    SSLServerConfig server_config;
    SSLConfig client_config;
    server_config.ech_keys = MakeTestEchKeys("public.example", /*max_name_len=*/64, &client_config.ech_config_list);
...
TEST_F(SSLClientSocketTest, ECHWrongKeys)
{
    ...
    bssl::UniquePtr<SSL_ECH_KEYS> keys1 = MakeTestEchKeys(kPublicName, /*max_name_len=*/64, &ech_config_list1);
    ASSERT_TRUE(keys1);
    bssl::UniquePtr<SSL_ECH_KEYS> keys2 = MakeTestEchKeys(kPublicName, /*max_name_len=*/64, &ech_config_list2);
    ASSERT_TRUE(keys2);
    ...
    CertVerifyResult verify_result;
    verify_result.verified_cert = server_cert;
    cert_verifier_->AddResultForCertAndHost(server_cert, kPublicName, verify_result, OK);
...
TEST_F(SSLClientSocketTest, ECHSecurelyDisabled)
{
    ...
    bssl::UniquePtr<SSL_ECH_KEYS> keys = MakeTestEchKeys(kPublicName, /*max_name_len=*/64, &ech_config_list);
    ASSERT_TRUE(keys);
...
TEST_F(SSLClientSocketTest, ECHGreaseEnabled)
{
    ...
    SSLServerConfig server_config;
    server_config.client_hello_callback_for_testing = base::BindLambdaForTesting([&](const SSL_CLIENT_HELLO* client_hello) {
        const uint8_t* data;
        size_t len;
        EXPECT_TRUE(SSL_early_callback_ctx_extension_get(client_hello, TLSEXT_TYPE_encrypted_client_hello, &data, &len));
        ran_callback = true;
        return true;
    });
    ASSERT_TRUE(StartEmbeddedTestServer(EmbeddedTestServer::CERT_OK, server_config));
...
TEST_F(SSLClientSocketTest, ECHGreaseDisabled)
{
    ...
    server_config.client_hello_callback_for_testing = base::BindLambdaForTesting([&](const SSL_CLIENT_HELLO* client_hello) {
        const uint8_t* data;
        size_t len;
        EXPECT_FALSE(SSL_early_callback_ctx_extension_get(client_hello, TLSEXT_TYPE_encrypted_client_hello, &data, &len));
        ran_callback = true;
        return true;
    });
    ASSERT_TRUE(StartEmbeddedTestServer(EmbeddedTestServer::CERT_OK, server_config));
...
const SSLHandshakeDetailsParams kSSLHandshakeDetailsParams[] = {
    ...
    { false /* no ALPN */, false /* no early data */, SSL_PROTOCOL_VERSION_TLS1_2, SSLHandshakeDetails::kTLS12Full, SSLHandshakeDetails::kTLS12Resume },
    { true /* ALPN */, false /* no early data */, SSL_PROTOCOL_VERSION_TLS1_2, SSLHandshakeDetails::kTLS12FalseStart, SSLHandshakeDetails::kTLS12Resume },

    // TLS 1.3 supports full handshakes, resumption, and 0-RTT.
    { false /* no ALPN */, false /* no early data */, SSL_PROTOCOL_VERSION_TLS1_3, SSLHandshakeDetails::kTLS13Full, SSLHandshakeDetails::kTLS13Resume },
    { false /* no ALPN */, true /* early data */, SSL_PROTOCOL_VERSION_TLS1_3, SSLHandshakeDetails::kTLS13Full, SSLHandshakeDetails::kTLS13Early },
};

class SSLHandshakeDetailsTest : public SSLClientSocketTest, public ::testing::WithParamInterface<SSLHandshakeDetailsParams> { };

INSTANTIATE_TEST_SUITE_P(All, SSLHandshakeDetailsTest, ValuesIn(kSSLHandshakeDetailsParams));
...
TEST_P(SSLHandshakeDetailsTest, Metrics)
{
    ...
    // Make the initial connection.
    {
        ...
        histograms.ExpectUniqueSample("Net.SSLHandshakeDetails", GetParam().expected_initial, 1);
    ...
    // Make a resumption connection.
    {
        ...
        histograms.ExpectUniqueSample("Net.SSLHandshakeDetails", GetParam().expected_resume, 1);
    }
}
...
TEST_F(SSLClientSocketZeroRTTTest, EarlyDataReasonNewSession)
{
    ...
    base::HistogramTester histograms;
    ASSERT_TRUE(RunInitialConnection());
    histograms.ExpectUniqueSample(kReasonHistogram, ssl_early_data_no_session_offered, 1);
}
...
TEST_F(SSLClientSocketZeroRTTTest, EarlyDataReasonNoResume)
{
    ...
    base::RunLoop().RunUntilIdle();
    histograms.ExpectUniqueSample(kReasonHistogram, ssl_early_data_session_not_resumed, 1);
}
...
TEST_F(SSLClientSocketTest, VersionMaxOverride)
{
    ...
    SSLInfo info;
    ASSERT_TRUE(sock_->GetSSLInfo(&info));
    EXPECT_EQ(SSL_CONNECTION_VERSION_TLS1_3, SSLConnectionStatusToVersion(info.connection_status));
    ...
    EXPECT_THAT(rv, IsOk());
    ASSERT_TRUE(sock_->GetSSLInfo(&info));
    EXPECT_EQ(SSL_CONNECTION_VERSION_TLS1_2, SSLConnectionStatusToVersion(info.connection_status));
}
...
TEST_F(SSLClientSocketTest, CancelReadIfReady)
{
    ...
    SSLConfig config;
    config.disable_post_handshake_peek_for_testing = true;
    auto sock = CreateSSLClientSocket(std::move(transport), host_port_pair(), config);
    ASSERT_THAT(callback.GetResult(sock->Connect(callback.callback())), IsOk());

    // Block the socket and wait for some data to arrive from the server.
    raw_transport->BlockReadResult();
    auto write_buf = base::MakeRefCounted<StringIOBuffer>("GET / HTTP/1.0\r\n\r\n");
    ASSERT_EQ(callback.GetResult(sock->Write(write_buf.get(), write_buf->size(), callback.callback(), TRAFFIC_ANNOTATION_FOR_TESTS)), write_buf->size());
    ...
    int rv = sock->ReadIfReady(read_buf.get(), 100, base::BindLambdaForTesting([&](int rv) { callback_called = true; }));
    ASSERT_THAT(rv, IsError(ERR_IO_PENDING));
    ...
    EXPECT_GT(callback.GetResult(sock->Read(read_buf.get(), 100, callback.callback())), 0);
}
...
TEST_F(SSLClientSocketTest, ServerName)
{
    ...
    int rv;
    ASSERT_TRUE(CreateAndConnectSSLClientSocketWithHost(SSLConfig(), HostPortPair("example.com", port), &rv));
    ASSERT_THAT(rv, IsOk());
    EXPECT_TRUE(ran_callback);
    ...
    ASSERT_TRUE(CreateAndConnectSSLClientSocketWithHost(SSLConfig(), HostPortPair("1.2.3.4", port), &rv));
    ASSERT_THAT(rv, IsOk());
    EXPECT_TRUE(ran_callback);
    EXPECT_EQ(got_server_name, std::nullopt);

    reset_callback_state();
    ASSERT_TRUE(CreateAndConnectSSLClientSocketWithHost(SSLConfig(), HostPortPair("::1", port), &rv));
    ASSERT_THAT(rv, IsOk());
    EXPECT_TRUE(ran_callback);
    EXPECT_EQ(got_server_name, std::nullopt);

    reset_callback_state();
    ASSERT_TRUE(CreateAndConnectSSLClientSocketWithHost(SSLConfig(), HostPortPair("2001:db8::42", port), &rv));
    ASSERT_THAT(rv, IsOk());
    EXPECT_TRUE(ran_callback);
    EXPECT_EQ(got_server_name, std::nullopt);
}
...
TEST_F(SSLClientSocketTest, PostQuantumKeyExchange)
{
    for (bool server_mlkem : { false, true }) {
        ...
        SSLServerConfig server_config;
        server_config.curves_for_testing.push_back(server_mlkem ? NID_X25519MLKEM768 : NID_X25519Kyber768Draft00);
        ASSERT_TRUE(StartEmbeddedTestServer(EmbeddedTestServer::CERT_OK, server_config));
...
class SSLClientSocketAlpsTest : public SSLClientSocketTest, public ::testing::WithParamInterface<std::tuple<bool, bool, bool>> {
public:
    ...
    bool client_alps_enabled() const
    {
        return std::get<0>(GetParam());
    }
    bool server_alps_enabled() const
    {
        return std::get<1>(GetParam());
    }
    bool client_use_new_alps() const
    {
        return std::get<2>(GetParam());
    }
...
INSTANTIATE_TEST_SUITE_P(All, SSLClientSocketAlpsTest, Combine(Bool(), Bool(), Bool()));
...
TEST_P(SSLClientSocketAlpsTest, Alps)
{
    ...
    if (server_alps_enabled()) {
        server_config.application_settings[kProtoHTTP2] = std::vector<uint8_t>(server_data.begin(), server_data.end());
    }
    // Configure the server to support whichever ALPS codepoint the client sent.
    server_config.client_hello_callback_for_testing = base::BindRepeating([](const SSL_CLIENT_HELLO* client_hello) {
        const uint8_t* unused_extension_bytes;
        size_t unused_extension_len;
        int use_alps_new_codepoint
            = SSL_early_callback_ctx_extension_get(client_hello, TLSEXT_TYPE_application_settings, &unused_extension_bytes, &unused_extension_len);
        SSL_set_alps_use_new_codepoint(client_hello->ssl, use_alps_new_codepoint);
        return true;
    });
    ...
    if (client_alps_enabled()) {
        client_config.application_settings[kProtoHTTP2] = std::vector<uint8_t>(client_data.begin(), client_data.end());
    }
...
TEST_P(SSLClientSocketAlpsTest, UnusedProtocols)
{
    ...
    server_config.client_hello_callback_for_testing = base::BindLambdaForTesting([&](const SSL_CLIENT_HELLO* client_hello) {
        ...
        if (!SSL_early_callback_ctx_extension_get(
                client_hello, client_use_new_alps() ? TLSEXT_TYPE_application_settings : TLSEXT_TYPE_application_settings_old, &data, &len)) {
            return false;
        }
        ...
        EXPECT_EQ(std::vector<uint8_t>(data, data + len), std::vector<uint8_t>({ 0x00, 0x03, 0x02, 'h', '2' }));
        return true;
    });
    ASSERT_TRUE(StartEmbeddedTestServer(EmbeddedTestServer::CERT_OK, server_config));
