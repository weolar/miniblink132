...
public:
    ...
    TestConnectJob(JobType job_type, base::TimeDelta timeout_duration, const CommonConnectJobParams* common_connect_job_params, ConnectJob::Delegate* delegate)
        : ConnectJob(DEFAULT_PRIORITY, SocketTag(), timeout_duration, common_connect_job_params, delegate, nullptr /* net_log */,
        ...
        , job_type_(job_type)
    {
        ...
        case JobType::kAsyncSuccess:
            ...
            return;
        case JobType::kHung:
            socket_data_provider_.set_connect_data(MockConnect(SYNCHRONOUS, ERR_IO_PENDING));
            return;
        }
    }
    ...
    {
        SetSocket(std::make_unique<MockTCPClientSocket>(AddressList(), net_log().net_log(), &socket_data_provider_), std::nullopt /* dns_aliases */);
        return socket()->Connect(base::BindOnce(&TestConnectJob::NotifyDelegateOfCompletion, base::Unretained(this)));
    }
    ...
    RequestPriority last_seen_priority() const
    {
        return last_seen_priority_;
    }
...
{
    TestConnectJob job(TestConnectJob::JobType::kSyncSuccess, base::Microseconds(1), &common_connect_job_params_, &delegate_);
...
{
    TestConnectJob job(TestConnectJob::JobType::kAsyncSuccess, base::Minutes(1), &common_connect_job_params_, &delegate_);
...
{
    TestConnectJob job(TestConnectJob::JobType::kHung, base::TimeDelta(), &common_connect_job_params_, &delegate_);
    ASSERT_THAT(job.Connect(), test::IsError(ERR_IO_PENDING));
...
{
    TestConnectJob job(TestConnectJob::JobType::kAsyncSuccess, base::Microseconds(1), &common_connect_job_params_, &delegate_);
    ASSERT_THAT(job.Connect(), test::IsError(ERR_IO_PENDING));
...
TEST_F(ConnectJobTest, TimedOut)
{
    ...
    std::unique_ptr<TestConnectJob> job = std::make_unique<TestConnectJob>(TestConnectJob::JobType::kHung, kTimeout, &common_connect_job_params_, &delegate_);
    ASSERT_THAT(job->Connect(), test::IsError(ERR_IO_PENDING));
    ...
    EXPECT_TRUE(LogContainsBeginEvent(entries, 1, NetLogEventType::TRANSPORT_CONNECT_JOB_CONNECT));
    EXPECT_TRUE(LogContainsEvent(entries, 2, NetLogEventType::CONNECT_JOB_SET_SOCKET, NetLogEventPhase::NONE));
    EXPECT_TRUE(LogContainsEvent(entries, 3, NetLogEventType::CONNECT_JOB_TIMED_OUT, NetLogEventPhase::NONE));
    EXPECT_TRUE(LogContainsEndEvent(entries, 4, NetLogEventType::TRANSPORT_CONNECT_JOB_CONNECT));
...
TEST_F(ConnectJobTest, TimedOutWithRestartedTimer)
{
    ...
    TestConnectJob job(TestConnectJob::JobType::kHung, kTimeout, &common_connect_job_params_, &delegate_);
    ASSERT_THAT(job.Connect(), test::IsError(ERR_IO_PENDING));
