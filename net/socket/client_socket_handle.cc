...
int ClientSocketHandle::Init(const ClientSocketPool::GroupId& group_id, scoped_refptr<ClientSocketPool::SocketParams> socket_params,
    const std::optional<NetworkTrafficAnnotationTag>& proxy_annotation_tag, RequestPriority priority, const SocketTag& socket_tag,
    ClientSocketPool::RespectLimits respect_limits, CompletionOnceCallback callback, const ClientSocketPool::ProxyAuthCallback& proxy_auth_callback,
    ClientSocketPool* pool, const NetLogWithSource& net_log)
{
    ...
    pool_ = pool;
    group_id_ = group_id;
    CompletionOnceCallback io_complete_callback = base::BindOnce(&ClientSocketHandle::OnIOComplete, base::Unretained(this));
    int rv = pool_->RequestSocket(group_id, std::move(socket_params), proxy_annotation_tag, priority, socket_tag, respect_limits, this,
...
void ClientSocketHandle::RemoveHigherLayeredPool(HigherLayeredPool* higher_pool)
{
    CHECK(higher_pool_);
...
void ClientSocketHandle::CloseIdleSocketsInGroup(const char* net_log_reason_utf8)
{
    if (pool_)
...
void ClientSocketHandle::HandleInitCompletion(int result)
{
    ...
    if (result != OK) {
        if (!socket()) {
            ResetInternal(false /* cancel */, false /* cancel_connect_job */); // Nothing to cancel since
                // the request failed.
        } else {
    ...
    set_is_initialized(true);
    CHECK_NE(-1, group_generation_) << "Pool should have set |group_generation_| to a valid value.";
    ...
    DCHECK(socket());
    socket()->NetLog().BeginEventReferencingSource(NetLogEventType::SOCKET_IN_USE, requesting_source_);
}
