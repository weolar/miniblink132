...
public:
    ...
    UnixDomainServerSocket(const AuthCallback& auth_callack, bool use_abstract_namespace);
    ...
    static bool GetPeerCredentials(SocketDescriptor socket_fd, Credentials* credentials);

    // ServerSocket implementation.
    int Listen(const IPEndPoint& address, int backlog, std::optional<bool> ipv6_only) override;
    int ListenWithAddressAndPort(const std::string& address_string, uint16_t port, int backlog) override;
    ...
    int AcceptSocketDescriptor(SocketDescriptor* socket_descriptor, CompletionOnceCallback callback);
