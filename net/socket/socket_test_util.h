...
template <MockReadWriteType type> struct MockReadWrite {
    ...
    MockReadWrite(IoMode io_mode, const char* data, int data_len, int seq, uint8_t tos_byte)
        : mode(io_mode)
        , result(0)
        , data(data)
        , data_len(data_len)
...
public:
    ...
    KeepAliveState keep_alive_state() const
    {
        return keep_alive_state_;
    }
    // Last set TCP keepalive delay.
    int keep_alive_delay() const
    {
        return keep_alive_delay_;
    }
    void set_keep_alive(bool enable, int delay)
    {
        keep_alive_state_ = enable ? KeepAliveState::kEnabled : KeepAliveState::kDisabled;
    ...
    bool set_no_delay_result() const
    {
        return set_no_delay_result_;
    }
    ...
    bool set_keep_alive_result() const
    {
        return set_keep_alive_result_;
    }
    ...
    MockConnect connect_data() const
    {
        return connect_;
    }
    void set_connect_data(const MockConnect& connect)
    {
        connect_ = connect;
    }
...
public:
    StaticSocketDataHelper(base::span<const MockRead> reads, base::span<const MockWrite> writes);
    ...
    {
        return read_index() >= read_count();
    }
    bool AllWriteDataConsumed() const
    {
        return write_index() >= write_count();
    }
...
public:
    ...
    StaticSocketDataProvider(base::span<const MockRead> reads, base::span<const MockWrite> writes);
    ...
    size_t read_index() const
    {
        return helper_.read_index();
    }
    size_t write_index() const
    {
        return helper_.write_index();
    }
    size_t read_count() const
    {
        return helper_.read_count();
    }
    size_t write_count() const
    {
        return helper_.write_count();
    }

    void set_printer(SocketDataPrinter* printer)
    {
        printer_ = printer;
    }
...
struct SSLSocketDataProvider {
    ...
    bool ConnectDataConsumed() const
    {
        return is_connect_data_consumed;
    }

    // Returns whether MockConfirm data has been consumed.
    bool ConfirmDataConsumed() const
    {
        return is_confirm_data_consumed;
    }
...
public:
    ...
    SequencedSocketData(base::span<const MockRead> reads, base::span<const MockWrite> writes);

    // |connect| is the result for the connect phase.
    // |reads| is the list of MockRead completions.
    // |writes| is the list of MockWrite completions.
    SequencedSocketData(const MockConnect& connect, base::span<const MockRead> reads, base::span<const MockWrite> writes);
...
// array.
template <typename T> class SocketDataProviderArray {
public:
...
public:
    ...
    int Read(IOBuffer* buf, int buf_len, CompletionOnceCallback callback) override = 0;
    int Write(IOBuffer* buf, int buf_len, CompletionOnceCallback callback, const NetworkTrafficAnnotationTag& traffic_annotation) override = 0;
...
public:
    MockTCPClientSocket(const AddressList& addresses, net::NetLog* net_log, SocketDataProvider* socket);
    ...
    // StreamSocket implementation.
    void SetBeforeConnectCallback(const BeforeConnectCallback& before_connect_callback) override;
...
private:
    ...
    int ReadIfReadyImpl(IOBuffer* buf, int buf_len, CompletionOnceCallback callback);
...
public:
    MockSSLClientSocket(
        std::unique_ptr<StreamSocket> stream_socket, const HostPortPair& host_and_port, const SSLConfig& ssl_config, SSLSocketDataProvider* socket);
...
private:
    static void ConnectCallback(MockSSLClientSocket* ssl_client_socket, CompletionOnceCallback callback, int rv);
...
public:
    explicit MockUDPClientSocket(SocketDataProvider* data = nullptr, net::NetLog* net_log = nullptr);
    ...
    int ConnectUsingNetwork(handles::NetworkHandle network, const IPEndPoint& address) override;
    int ConnectUsingDefaultNetwork(const IPEndPoint& address) override;
    int ConnectAsync(const IPEndPoint& address, CompletionOnceCallback callback) override;
    int ConnectUsingNetworkAsync(handles::NetworkHandle network, const IPEndPoint& address, CompletionOnceCallback callback) override;
    int ConnectUsingDefaultNetworkAsync(const IPEndPoint& address, CompletionOnceCallback callback) override;
    ...
    void set_source_host(IPAddress addr)
    {
        source_host_ = addr;
    }
    IPAddress source_host() const
    {
        return source_host_;
    }
...
public:
    TestSocketRequest(std::vector<raw_ptr<TestSocketRequest, VectorExperimental>>* request_order, size_t* completion_count);
    ...
    {
        return base::BindOnce(&TestSocketRequest::OnComplete, base::Unretained(this));
    }
...
private:
    ...
    raw_ptr<std::vector<raw_ptr<TestSocketRequest, VectorExperimental>>> request_order_;
...
public:
    ...
    int StartRequestUsingPool(PoolType* socket_pool, const ClientSocketPool::GroupId& group_id, RequestPriority priority,
        ClientSocketPool::RespectLimits respect_limits, const scoped_refptr<typename PoolType::SocketParams>& socket_params)
    {
        DCHECK(socket_pool);
        TestSocketRequest* request(new TestSocketRequest(&request_order_, &completion_count_));
        requests_.push_back(base::WrapUnique(request));
        int rv = request->handle()->Init(group_id, socket_params, std::nullopt /* proxy_annotation_tag */, priority, SocketTag(), respect_limits,
            request->callback(), ClientSocketPool::ProxyAuthCallback(), socket_pool, NetLogWithSource());
...
class MockTransportSocketParams : public base::RefCounted<MockTransportSocketParams> {
public:
    MockTransportSocketParams(const MockTransportSocketParams&) = delete;
    MockTransportSocketParams& operator=(const MockTransportSocketParams&) = delete;
...
public:
    ...
    public:
        MockConnectJob(std::unique_ptr<StreamSocket> socket, ClientSocketHandle* handle, const SocketTag& socket_tag, CompletionOnceCallback callback,
        ...
        ClientSocketHandle* handle() const
        {
            return handle_;
        }
    ...
    MockTransportClientSocketPool(int max_sockets, int max_sockets_per_group, const CommonConnectJobParams* common_connect_job_params);

    MockTransportClientSocketPool(const MockTransportClientSocketPool&) = delete;
    MockTransportClientSocketPool& operator=(const MockTransportClientSocketPool&) = delete;
    ...
    int RequestSocket(const GroupId& group_id, scoped_refptr<ClientSocketPool::SocketParams> socket_params,
        const std::optional<NetworkTrafficAnnotationTag>& proxy_annotation_tag, RequestPriority priority, const SocketTag& socket_tag,
        RespectLimits respect_limits, ClientSocketHandle* handle, CompletionOnceCallback callback, const ProxyAuthCallback& on_auth_callback,
        const NetLogWithSource& net_log) override;
    void SetPriority(const GroupId& group_id, ClientSocketHandle* handle, RequestPriority priority) override;
    void CancelRequest(const GroupId& group_id, ClientSocketHandle* handle, bool cancel_connect_job) override;
    void ReleaseSocket(const GroupId& group_id, std::unique_ptr<StreamSocket> socket, int64_t generation) override;
...
public:
    ...
    bool tagged_before_connected() const
    {
        return tagged_before_connected_;
    }
...
public:
    ...
    MockTaggingClientSocketFactory(const MockTaggingClientSocketFactory&) = delete;
    MockTaggingClientSocketFactory& operator=(const MockTaggingClientSocketFactory&) = delete;
    ...
    MockUDPClientSocket* GetLastProducedUDPSocket() const
    {
        return udp_socket_;
    }

private:
    raw_ptr<MockTaggingStreamSocket, AcrossTasksDanglingUntriaged> tcp_socket_ = nullptr;
    raw_ptr<MockUDPClientSocket, AcrossTasksDanglingUntriaged> udp_socket_ = nullptr;
};
