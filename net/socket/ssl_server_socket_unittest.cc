...
public:
    ...
    int Write(IOBuffer* buf, int buf_len, CompletionOnceCallback callback, const NetworkTrafficAnnotationTag& traffic_annotation)
    {
        ...
        if (closed_) {
            ...
            base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
                FROM_HERE, base::BindOnce(&FakeDataChannel::DoWriteCallback, weak_factory_.GetWeakPtr()));
            return ERR_IO_PENDING;
        }
        // This function returns synchronously, so make a copy of the buffer.
        data_.push(base::MakeRefCounted<DrainableIOBuffer>(base::MakeRefCounted<StringIOBuffer>(std::string(buf->data(), buf_len)), buf_len));
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(&FakeDataChannel::DoReadCallback, weak_factory_.GetWeakPtr()));
        return buf_len;
    }
    ...
    void Close()
    {
        ...
        if (!read_callback_.is_null()) {
            base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
                FROM_HERE, base::BindOnce(&FakeDataChannel::DoReadCallback, weak_factory_.GetWeakPtr()));
        }
    }
...
public:
    FakeSocket(FakeDataChannel* incoming_channel, FakeDataChannel* outgoing_channel)
    ...
    int Write(IOBuffer* buf, int buf_len, CompletionOnceCallback callback, const NetworkTrafficAnnotationTag& traffic_annotation) override
    {
        ...
        buf_len = rand() % buf_len + 1;
        return outgoing_->Write(buf, buf_len, std::move(callback), TRAFFIC_ANNOTATION_FOR_TESTS);
    }
...
TEST(FakeSocketTest, DataTransfer)
{
    ...
    // Write then read.
    int written = server.Write(write_buf.get(), kTestDataSize, CompletionOnceCallback(), TRAFFIC_ANNOTATION_FOR_TESTS);
    EXPECT_GT(written, 0);
    EXPECT_LE(written, kTestDataSize);

    int read = client.Read(read_buf.get(), kReadBufSize, CompletionOnceCallback());
    EXPECT_GT(read, 0);
    ...
    EXPECT_EQ(ERR_IO_PENDING, server.Read(read_buf.get(), kReadBufSize, callback.callback()));

    written = client.Write(write_buf.get(), kTestDataSize, CompletionOnceCallback(), TRAFFIC_ANNOTATION_FOR_TESTS);
    EXPECT_GT(written, 0);
    EXPECT_LE(written, kTestDataSize);
...
public:
    ...
    void SetUp() override
    {
        ...
        std::unique_ptr<crypto::RSAPrivateKey> key = ReadTestKey("unittest.key.bin");
        ASSERT_TRUE(key);
        server_ssl_private_key_ = WrapOpenSSLPrivateKey(bssl::UpRef(key->key()));

        // Certificate provided by the host doesn't need authority.
        client_ssl_config_.allowed_bad_certs.emplace_back(server_cert_, CERT_STATUS_AUTHORITY_INVALID);
...
protected:
    void CreateContext()
    {
        ...
        channel_1_.reset();
        channel_2_.reset();
        server_context_ = CreateSSLServerContext(server_cert_.get(), *server_private_key_, server_ssl_config_);
    }
    ...
    void CreateContextSSLPrivateKey()
    {
        ...
        server_context_ = CreateSSLServerContext(server_cert_.get(), server_ssl_private_key_, server_ssl_config_);
    }

    static HostPortPair GetHostAndPort()
    {
        return HostPortPair("unittest", 0);
    }
    ...
    void CreateSockets()
    {
        ...
        std::unique_ptr<StreamSocket> client_connection = std::make_unique<FakeSocket>(channel_1_.get(), channel_2_.get());
        std::unique_ptr<StreamSocket> server_socket = std::make_unique<FakeSocket>(channel_2_.get(), channel_1_.get());

        client_socket_ = client_context_->CreateSSLClientSocket(std::move(client_connection), GetHostAndPort(), client_ssl_config_);
...
// Client certificates are disabled on iOS.
    ...
    {
        scoped_refptr<X509Certificate> client_cert = ImportCertFromFile(GetTestCertsDirectory(), cert_file_name);
        ASSERT_TRUE(client_cert);

        std::unique_ptr<crypto::RSAPrivateKey> key = ReadTestKey(private_key_file_name);
        ASSERT_TRUE(key);

        client_context_->SetClientCertificate(GetHostAndPort(), std::move(client_cert), WrapOpenSSLPrivateKey(bssl::UpRef(key->key())));
    }
    ...
    void ConfigureClientCertsForServer()
    {
        ...
        static const uint8_t kClientCertCAName[] = { 0x30, 0x0f, 0x31, 0x0d, 0x30, 0x0b, 0x06, 0x03, 0x55, 0x04, 0x03, 0x0c, 0x04, 0x42, 0x20, 0x43, 0x41 };
        server_ssl_config_.cert_authorities.emplace_back(std::begin(kClientCertCAName), std::end(kClientCertCAName));

        scoped_refptr<X509Certificate> expected_client_cert(ImportCertFromFile(GetTestCertsDirectory(), kClientCertFileName));
    ...
    std::unique_ptr<crypto::RSAPrivateKey> ReadTestKey(std::string_view name)
    {
        ...
        std::vector<uint8_t> key_vector(
            reinterpret_cast<const uint8_t*>(key_string.data()), reinterpret_cast<const uint8_t*>(key_string.data() + key_string.length()));
        std::unique_ptr<crypto::RSAPrivateKey> key(crypto::RSAPrivateKey::CreateFromPrivateKeyInfo(key_vector));
        return key;
    }
    ...
    {
        const int kReadBufSize = 1024;
        scoped_refptr<StringIOBuffer> write_buf = base::MakeRefCounted<StringIOBuffer>("testing123");
        scoped_refptr<DrainableIOBuffer> read_buf = base::MakeRefCounted<DrainableIOBuffer>(base::MakeRefCounted<IOBufferWithSize>(kReadBufSize), kReadBufSize);
        TestCompletionCallback write_callback;
        TestCompletionCallback read_callback;
        int server_ret = server_socket_->Write(write_buf.get(), write_buf->size(), write_callback.callback(), TRAFFIC_ANNOTATION_FOR_TESTS);
        EXPECT_TRUE(server_ret > 0 || server_ret == ERR_IO_PENDING);
        int client_ret = client_socket_->Read(read_buf.get(), read_buf->BytesRemaining(), read_callback.callback());
...
class SSLServerSocketReadTest : public SSLServerSocketTest, public ::testing::WithParamInterface<bool> {
protected:
    SSLServerSocketReadTest()
        : read_if_ready_enabled_(GetParam())
    {
    }
    ...
    bool read_if_ready_enabled() const
    {
        return read_if_ready_enabled_;
    }
...
INSTANTIATE_TEST_SUITE_P(/* no prefix */, SSLServerSocketReadTest, ::testing::Bool());
...
TEST_F(SSLServerSocketTest, Handshake)
{
    ...
    bool is_aead;
    bool is_tls13;
    SSLCipherSuiteToStrings(&key_exchange, &cipher, &mac, &is_aead, &is_tls13, cipher_suite);
    EXPECT_TRUE(is_aead);
}
...
TEST_F(SSLServerSocketTest, HandshakeWithClientCert)
{
    ...
    ASSERT_NO_FATAL_FAILURE(ConfigureClientCertsForClient(kClientCertFileName, kClientPrivateKeyFileName));
...
TEST_F(SSLServerSocketTest, HandshakeWithClientCertRequiredNotSupplied)
{
    ...
    TestCompletionCallback connect_callback;
    EXPECT_EQ(ERR_SSL_CLIENT_AUTH_CERT_NEEDED, connect_callback.GetResult(client_socket_->Connect(connect_callback.callback())));
    ...
    EXPECT_THAT(handshake_callback.GetResult(server_ret), IsError(ERR_CONNECTION_CLOSED));
}
...
TEST_F(SSLServerSocketTest, HandshakeWithClientCertRequiredNotSuppliedCached)
{
    ...
    TestCompletionCallback connect_callback2;
    EXPECT_EQ(ERR_SSL_CLIENT_AUTH_CERT_NEEDED, connect_callback2.GetResult(client_socket_->Connect(connect_callback2.callback())));
    ...
    EXPECT_THAT(handshake_callback2.GetResult(server_ret2), IsError(ERR_CONNECTION_CLOSED));
}
...
TEST_F(SSLServerSocketTest, HandshakeWithWrongClientCertSupplied)
{
    ...
    ASSERT_NO_FATAL_FAILURE(ConfigureClientCertsForClient(kWrongClientCertFileName, kWrongClientPrivateKeyFileName));
    ASSERT_NO_FATAL_FAILURE(ConfigureClientCertsForServer());
    ASSERT_NO_FATAL_FAILURE(CreateContext());
    ASSERT_NO_FATAL_FAILURE(CreateSockets());
    ...
    EXPECT_EQ(ERR_BAD_SSL_CLIENT_AUTH_CERT, handshake_callback.GetResult(server_ret));

    // Pump client read to get client cert error.
    const int kReadBufSize = 1024;
    scoped_refptr<DrainableIOBuffer> read_buf = base::MakeRefCounted<DrainableIOBuffer>(base::MakeRefCounted<IOBufferWithSize>(kReadBufSize), kReadBufSize);
    TestCompletionCallback read_callback;
    client_ret = client_socket_->Read(read_buf.get(), read_buf->BytesRemaining(), read_callback.callback());
    client_ret = read_callback.GetResult(client_ret);
...
TEST_F(SSLServerSocketTest, HandshakeWithWrongClientCertSuppliedTLS12)
{
    ...
    EXPECT_EQ(ERR_BAD_SSL_CLIENT_AUTH_CERT, connect_callback.GetResult(client_ret));
    EXPECT_EQ(ERR_BAD_SSL_CLIENT_AUTH_CERT, handshake_callback.GetResult(server_ret));
}
...
TEST_F(SSLServerSocketTest, HandshakeWithWrongClientCertSuppliedCached)
{
    ...
    EXPECT_EQ(ERR_BAD_SSL_CLIENT_AUTH_CERT, handshake_callback2.GetResult(server_ret2));
...
TEST_P(SSLServerSocketReadTest, DataTransfer)
{
    ...
    TestCompletionCallback write_callback;
    TestCompletionCallback read_callback;
    server_ret = server_socket_->Write(write_buf.get(), write_buf->size(), write_callback.callback(), TRAFFIC_ANNOTATION_FOR_TESTS);
    EXPECT_TRUE(server_ret > 0 || server_ret == ERR_IO_PENDING);
    client_ret = client_socket_->Read(read_buf.get(), read_buf->BytesRemaining(), read_callback.callback());
    EXPECT_TRUE(client_ret > 0 || client_ret == ERR_IO_PENDING);
    ...
    server_ret = Read(server_socket_.get(), read_buf.get(), read_buf->BytesRemaining(), read_callback.callback());
    EXPECT_EQ(server_ret, ERR_IO_PENDING);
    client_ret = client_socket_->Write(write_buf.get(), write_buf->size(), write_callback.callback(), TRAFFIC_ANNOTATION_FOR_TESTS);
    EXPECT_TRUE(client_ret > 0 || client_ret == ERR_IO_PENDING);
...
TEST_F(SSLServerSocketTest, ClientWriteAfterServerClose)
{
    ...
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, run_loop.QuitClosure(), base::Milliseconds(10));
    run_loop.Run();
}
...
TEST_F(SSLServerSocketTest, ExportKeyingMaterial)
{
    ...
    int rv = server_socket_->ExportKeyingMaterial(kKeyingLabel, false, kKeyingContext, server_out, sizeof(server_out));
    ASSERT_THAT(rv, IsOk());

    unsigned char client_out[kKeyingMaterialSize];
    rv = client_socket_->ExportKeyingMaterial(kKeyingLabel, false, kKeyingContext, client_out, sizeof(client_out));
    ASSERT_THAT(rv, IsOk());
    ...
    rv = client_socket_->ExportKeyingMaterial(kKeyingLabelBad, false, kKeyingContext, client_bad, sizeof(client_bad));
    ASSERT_EQ(rv, OK);
...
TEST_F(SSLServerSocketTest, RequireEcdheFlag)
{
    ...
    config.disabled_cipher_suites.assign(kEcdheCiphers, kEcdheCiphers + std::size(kEcdheCiphers));
...
public:
    SSLPrivateKeyHook(scoped_refptr<SSLPrivateKey> private_key, base::RepeatingClosure on_sign)
        : private_key_(std::move(private_key))
        , on_sign_(std::move(on_sign))
    {
    }
...
TEST_F(SSLServerSocketTest, HandshakeServerSSLPrivateKeyDisconnectDuringSigning_ReturnsError)
{
    auto on_sign = base::BindLambdaForTesting([&]() {
        client_socket_->Disconnect();
        ASSERT_FALSE(client_socket_->IsConnected());
    });
    server_ssl_private_key_ = base::MakeRefCounted<SSLPrivateKeyHook>(std::move(server_ssl_private_key_), on_sign);
    ASSERT_NO_FATAL_FAILURE(CreateContextSSLPrivateKey());
    ASSERT_NO_FATAL_FAILURE(CreateSockets());
...
class SSLServerSocketAlpsTest : public SSLServerSocketTest, public ::testing::WithParamInterface<std::tuple<bool, bool>> {
public:
...
INSTANTIATE_TEST_SUITE_P(All, SSLServerSocketAlpsTest, ::testing::Combine(::testing::Bool(), ::testing::Bool()));
...
TEST_P(SSLServerSocketAlpsTest, Alps)
{
    ...
    if (server_alps_enabled_) {
        server_ssl_config_.application_settings[kProtoHTTP2] = std::vector<uint8_t>(server_data.begin(), server_data.end());
    }

    client_ssl_config_.alpn_protos = { kProtoHTTP2 };
    if (client_alps_enabled_) {
        client_ssl_config_.application_settings[kProtoHTTP2] = std::vector<uint8_t>(client_data.begin(), client_data.end());
    }
    ...
    const auto alps_data_received_by_client = client_socket_->GetPeerApplicationSettings();
    const auto alps_data_received_by_server = server_socket_->GetPeerApplicationSettings();
...
TEST_F(SSLServerSocketTest, CancelReadIfReady)
{
    ...
    int read_ret = server_socket_->ReadIfReady(read_buf.get(), 1, read_callback.callback());
    ...
    TestCompletionCallback write_callback;
    ASSERT_EQ(write_callback.GetResult(client_socket_->Write(write_buf.get(), write_buf->size(), write_callback.callback(), TRAFFIC_ANNOTATION_FOR_TESTS)),
        write_buf->size());
    ...
    while (true) {
        ...
        read_ret = server_socket_->ReadIfReady(read_buf.get(), 1, read_callback2.callback());
