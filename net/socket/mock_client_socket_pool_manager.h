...
public:
    ...
    MockClientSocketPoolManager& operator=(const MockClientSocketPoolManager&) = delete;

    ~MockClientSocketPoolManager() override;

    // Sets socket pool that gets used for the specified ProxyChain.
    void SetSocketPool(const ProxyChain& proxy_chain, std::unique_ptr<ClientSocketPool> pool);

    // ClientSocketPoolManager methods:
    void FlushSocketPoolsWithError(int error, const char* net_log_reason_utf8) override;
...
private:
    using ClientSocketPoolMap = std::map<ProxyChain, std::unique_ptr<ClientSocketPool>>;
