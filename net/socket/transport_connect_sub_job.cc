...
public:
    WebSocketStreamSocket(
        std::unique_ptr<StreamSocket> wrapped_socket, WebSocketEndpointLockManager* websocket_endpoint_lock_manager, const IPEndPoint& address)
    ...
    {
        return wrapped_socket_->Write(buf, buf_len, std::move(callback), traffic_annotation);
    }
...
TransportConnectSubJob::TransportConnectSubJob(std::vector<IPEndPoint> addresses, TransportConnectJob* parent_job, SubJobType type)
...
int TransportConnectSubJob::DoEndpointLock()
{
    ...
    return parent_job_->websocket_endpoint_lock_manager()->LockEndpoint(CurrentAddress(), this);
}
...
int TransportConnectSubJob::DoEndpointLockComplete()
{
    ...
    if (auto* factory = parent_job_->socket_performance_watcher_factory(); factory != nullptr) {
        socket_performance_watcher = factory->CreateSocketPerformanceWatcher(SocketPerformanceWatcherFactory::PROTOCOL_TCP, CurrentAddress().address());
    }

    const NetLogWithSource& net_log = parent_job_->net_log();
    transport_socket_ = parent_job_->client_socket_factory()->CreateTransportClientSocket(
        one_address, std::move(socket_performance_watcher), parent_job_->network_quality_estimator(), net_log.net_log(), net_log.source());
    ...
    if (parent_job_->websocket_endpoint_lock_manager()) {
        transport_socket_
            = std::make_unique<WebSocketStreamSocket>(std::move(transport_socket_), parent_job_->websocket_endpoint_lock_manager(), CurrentAddress());
    }
    ...
    return transport_socket_->Connect(base::BindOnce(&TransportConnectSubJob::OnIOComplete, base::Unretained(this)));
}
...
int TransportConnectSubJob::DoTransportConnectComplete(int result)
{
    ...
    if (result != OK) {
        ...
        parent_job_->connection_attempts_.push_back(ConnectionAttempt(CurrentAddress(), result));

        // Don't try the next address if entering suspend mode.
        if (result != ERR_NETWORK_IO_SUSPENDED && current_address_index_ + 1 < addresses_.size()) {
