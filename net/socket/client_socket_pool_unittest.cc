...
TEST(ClientSocketPool, GroupIdOperators)
{
    ...
    const SecureDnsPolicy kDisableSecureDnsValues[] = { SecureDnsPolicy::kAllow, SecureDnsPolicy::kDisable };
                ...
                for (const auto& privacy_mode : kPrivacyModes) {
                    ...
                    for (const auto& network_anonymization_key : kNetworkAnonymizationKeys) {
                        ...
                        for (const auto& secure_dns_policy : kDisableSecureDnsValues) {
                            ClientSocketPool::GroupId group_id(url::SchemeHostPort(scheme, host, port), privacy_mode, network_anonymization_key,
                                secure_dns_policy,
...
TEST(ClientSocketPool, GroupIdToString)
{
    ...
    EXPECT_EQ("http://foo <null>",
        ClientSocketPool::GroupId(url::SchemeHostPort(url::kHttpScheme, "foo", 80), PrivacyMode::PRIVACY_MODE_DISABLED, NetworkAnonymizationKey(),
            SecureDnsPolicy::kAllow, /*disable_cert_network_fetches=*/false)
            .ToString());
    EXPECT_EQ("http://bar:443 <null>",
        ClientSocketPool::GroupId(url::SchemeHostPort(url::kHttpScheme, "bar", 443), PrivacyMode::PRIVACY_MODE_DISABLED, NetworkAnonymizationKey(),
            SecureDnsPolicy::kAllow, /*disable_cert_network_fetches=*/false)
            .ToString());
    EXPECT_EQ("pm/http://bar <null>",
        ClientSocketPool::GroupId(url::SchemeHostPort(url::kHttpScheme, "bar", 80), PrivacyMode::PRIVACY_MODE_ENABLED, NetworkAnonymizationKey(),
            SecureDnsPolicy::kAllow, /*disable_cert_network_fetches=*/false)
            .ToString());

    EXPECT_EQ("https://foo:80 <null>",
        ClientSocketPool::GroupId(url::SchemeHostPort(url::kHttpsScheme, "foo", 80), PrivacyMode::PRIVACY_MODE_DISABLED, NetworkAnonymizationKey(),
            SecureDnsPolicy::kAllow, /*disable_cert_network_fetches=*/false)
            .ToString());
    EXPECT_EQ("https://bar <null>",
        ClientSocketPool::GroupId(url::SchemeHostPort(url::kHttpsScheme, "bar", 443), PrivacyMode::PRIVACY_MODE_DISABLED, NetworkAnonymizationKey(),
            SecureDnsPolicy::kAllow, /*disable_cert_network_fetches=*/false)
            .ToString());
    EXPECT_EQ("pm/https://bar:80 <null>",
        ClientSocketPool::GroupId(url::SchemeHostPort(url::kHttpsScheme, "bar", 80), PrivacyMode::PRIVACY_MODE_ENABLED, NetworkAnonymizationKey(),
            SecureDnsPolicy::kAllow, /*disable_cert_network_fetches=*/false)
            .ToString());

    EXPECT_EQ("https://foo <https://foo.test cross_site>",
        ClientSocketPool::GroupId(url::SchemeHostPort(url::kHttpsScheme, "foo", 443), PrivacyMode::PRIVACY_MODE_DISABLED,
            NetworkAnonymizationKey::CreateCrossSite(SchemefulSite(GURL("https://foo.test"))), SecureDnsPolicy::kAllow, /*disable_cert_network_fetches=*/false)
            .ToString());
...
TEST(ClientSocketPool, SplitHostCacheByNetworkIsolationKeyDisabled)
{
    ...
    ClientSocketPool::GroupId group_id1(url::SchemeHostPort(url::kHttpsScheme, "foo", 443), PrivacyMode::PRIVACY_MODE_DISABLED,
        NetworkAnonymizationKey::CreateSameSite(kSiteFoo), SecureDnsPolicy::kAllow, /*disable_cert_network_fetches=*/false);

    ClientSocketPool::GroupId group_id2(url::SchemeHostPort(url::kHttpsScheme, "foo", 443), PrivacyMode::PRIVACY_MODE_DISABLED,
        NetworkAnonymizationKey::CreateSameSite(kSiteBar), SecureDnsPolicy::kAllow, /*disable_cert_network_fetches=*/false);
    ...
    EXPECT_EQ(group_id1.network_anonymization_key(), group_id2.network_anonymization_key());
