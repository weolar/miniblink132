...
const net::NetworkTrafficAnnotationTag kTrafficAnnotation = net::DefineNetworkTrafficAnnotation("socket_bio_adapter", R"(
      semantics {
...
SocketBIOAdapter::SocketBIOAdapter(StreamSocket* socket, int read_buffer_capacity, int write_buffer_capacity, Delegate* delegate)
    : socket_(socket)
    ...
    , delegate_(delegate)
{
    ...
    read_callback_ = base::BindRepeating(&SocketBIOAdapter::OnSocketReadComplete, weak_factory_.GetWeakPtr());
    write_callback_ = base::BindRepeating(&SocketBIOAdapter::OnSocketWriteComplete, weak_factory_.GetWeakPtr());
}
...
int SocketBIOAdapter::BIORead(base::span<uint8_t> out)
{
    ...
    if (write_error_ != OK && write_error_ != ERR_IO_PENDING && (read_result_ == 0 || read_result_ == ERR_IO_PENDING)) {
    ...
    if (read_result_ == 0) {
        ...
        read_buffer_ = base::MakeRefCounted<IOBufferWithSize>(read_buffer_capacity_);
        read_result_ = ERR_IO_PENDING;
        int result = socket_->ReadIfReady(
            read_buffer_.get(), read_buffer_capacity_, base::BindOnce(&SocketBIOAdapter::OnSocketReadIfReadyComplete, weak_factory_.GetWeakPtr()));
    ...
    base::span<const uint8_t> read_data
        = read_buffer_->span().subspan(read_offset_, std::min(out.size(), base::checked_cast<size_t>(read_result_ - read_offset_)));
...
int SocketBIOAdapter::BIOWrite(base::span<const uint8_t> in)
{
    ...
    if (write_buffer_used_ < write_buffer_->RemainingCapacity()) {
        base::span<const uint8_t> chunk = in.first(std::min(base::checked_cast<size_t>(write_buffer_->RemainingCapacity() - write_buffer_used_), in.size()));
    ...
    if (in.size() > 0 && write_buffer_used_ < write_buffer_->capacity()) {
        ...
        base::span<const uint8_t> chunk = in.first(std::min(in.size(), base::checked_cast<size_t>(write_buffer_->capacity() - write_buffer_used_)));
    ...
    if (write_error_ != OK && write_error_ != ERR_IO_PENDING && read_result_ == ERR_IO_PENDING) {
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(&SocketBIOAdapter::CallOnReadReady, weak_factory_.GetWeakPtr()));
    }
...
void SocketBIOAdapter::SocketWrite()
{
    ...
    while (write_error_ == OK && write_buffer_used_ > 0) {
        ...
        int write_size = std::min(write_buffer_used_, write_buffer_->RemainingCapacity());

        // TODO(crbug.com/40064248): Remove this once the crash is resolved.
        char debug[128];
        snprintf(debug, sizeof(debug), "offset=%d;remaining=%d;used=%d;write_size=%d", write_buffer_->offset(), write_buffer_->RemainingCapacity(),
            write_buffer_used_, write_size);
        base::debug::Alias(debug);

        write_error_ = ERR_IO_PENDING;
        int result = socket_->Write(write_buffer_.get(), write_size, write_callback_, kTrafficAnnotation);
...
{
    SocketBIOAdapter* adapter = reinterpret_cast<SocketBIOAdapter*>(BIO_get_data(bio));
    if (adapter) {
...
long SocketBIOAdapter::BIOCtrlWrapper(BIO* bio, int cmd, long larg, void* parg)
{
    switch (cmd) {
    case BIO_CTRL_FLUSH:
