...
public:
    ...
    UnexportableKeyServiceFactory& operator=(const UnexportableKeyServiceFactory&) = delete;

    void SetUnexportableKeyFactoryForTesting(unexportable_keys::UnexportableKeyService* (*func)());

    static UnexportableKeyServiceFactory* GetInstanceForTesting();

private:
    friend class base::NoDestructor<UnexportableKeyServiceFactory>;
    std::unique_ptr<unexportable_keys::UnexportableKeyService> unexportable_key_service_;
