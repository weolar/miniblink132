...
public:
    ...
    //    allow this.
    static std::optional<CookieCraving> Create(const GURL& url, const std::string& name, const std::string& attributes, base::Time creation_time,
    ...
    static CookieCraving CreateUnsafeForTesting(std::string name, std::string domain, std::string path, base::Time creation, bool secure, bool httponly,
    ...
    static std::optional<CookieCraving> CreateFromProto(const proto::CookieCraving& proto);

private:
    CookieCraving();

    // Prefer Create() over this constructor. This may return non-valid instances.
    CookieCraving(std::string name, std::string domain, std::string path, base::Time creation, bool secure, bool httponly, CookieSameSite same_site,
