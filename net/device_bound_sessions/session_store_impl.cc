...
constexpr base::TaskTraits kDBTaskTraits = { base::MayBlock(), base::TaskPriority::USER_VISIBLE, base::TaskShutdownBehavior::BLOCK_SHUTDOWN };
...
SessionStoreImpl::DBStatus InitializeOnDbSequence(sql::Database* db, base::FilePath db_storage_path, sqlite_proto::ProtoTableManager* table_manager,
    sqlite_proto::KeyValueData<proto::SiteSessions>* session_data)
{
    ...
    table_manager->InitializeOnDbSequence(db, std::vector<std::string> { kSessionTableName }, kCurrentSchemaVersion);
...
SessionStoreImpl::SessionStoreImpl(base::FilePath db_storage_path, UnexportableKeyService& key_service)
    : key_service_(key_service)
    , db_task_runner_(base::ThreadPool::CreateSequencedTaskRunner(kDBTaskTraits))
    , db_storage_path_(std::move(db_storage_path))
    , db_(std::make_unique<sql::Database>(sql::DatabaseOptions { .page_size = 4096, .cache_size = 500 }))
    , table_manager_(base::MakeRefCounted<sqlite_proto::ProtoTableManager>(db_task_runner_))
    , session_table_(std::make_unique<sqlite_proto::KeyValueTable<proto::SiteSessions>>(kSessionTableName))
    , session_data_(std::make_unique<sqlite_proto::KeyValueData<proto::SiteSessions>>(table_manager_, session_table_.get(),
          /*max_num_entries=*/std::nullopt, kFlushDelay))
{
...
SessionStoreImpl::~SessionStoreImpl()
{
    ...
    db_task_runner_->PostTaskAndReply(FROM_HERE,
        base::BindOnce([](scoped_refptr<sqlite_proto::ProtoTableManager> table_manager, std::unique_ptr<sql::Database> db,
...
void SessionStoreImpl::LoadSessions(LoadSessionsCallback callback)
{
    ...
    db_task_runner_->PostTaskAndReplyWithResult(FROM_HERE,
        base::BindOnce(&InitializeOnDbSequence, base::Unretained(db_.get()), db_storage_path_, base::Unretained(table_manager_.get()),
            base::Unretained(session_data_.get())),
        base::BindOnce(&SessionStoreImpl::OnDatabaseLoaded, weak_ptr_factory_.GetWeakPtr(), std::move(callback)));
}

void SessionStoreImpl::OnDatabaseLoaded(LoadSessionsCallback callback, DBStatus db_status)
{
    db_status_ = db_status;
    SessionsMap sessions;
    if (db_status == DBStatus::kSuccess) {
        std::vector<std::string> keys_to_delete;
        sessions = CreateSessionsFromLoadedData(session_data_->GetAllCached(), keys_to_delete);
...
SessionStore::SessionsMap SessionStoreImpl::CreateSessionsFromLoadedData(
    const std::map<std::string, proto::SiteSessions>& loaded_data, std::vector<std::string>& keys_to_delete)
{
    ...
    for (const auto& [site_str, site_proto] : loaded_data) {
        ...
        for (const auto& [session_id, session_proto] : site_proto.sessions()) {
            if (!session_proto.has_wrapped_key() || session_proto.wrapped_key().empty()) {
...
void SessionStoreImpl::SetShutdownCallbackForTesting(base::OnceClosure shutdown_callback)
{
    shutdown_callback_ = std::move(shutdown_callback);
}

void SessionStoreImpl::SaveSession(const SchemefulSite& site, const Session& session)
{
    if (db_status_ != DBStatus::kSuccess) {
        return;
    }
    ...
    ServiceErrorOr<std::vector<uint8_t>> wrapped_key = key_service_->GetWrappedKey(*session.unexportable_key_id());
    ...
    session_proto.set_wrapped_key(std::string(wrapped_key->begin(), wrapped_key->end()));
    proto::SiteSessions site_proto;
    std::string site_str = site.Serialize();
    session_data_->TryGetData(site_str, &site_proto);
    (*site_proto.mutable_sessions())[session_proto.id()] = std::move(session_proto);

    session_data_->UpdateData(site_str, site_proto);
}

void SessionStoreImpl::DeleteSession(const SchemefulSite& site, const Session::Id& session_id)
{
    if (db_status_ != DBStatus::kSuccess) {
        return;
    }
...
SessionStore::SessionsMap SessionStoreImpl::GetAllSessions() const
{
    ...
    std::vector<std::string> keys_to_delete;
    SessionsMap all_sessions = CreateSessionsFromLoadedData(session_data_->GetAllCached(), keys_to_delete);
...
void SessionStoreImpl::RestoreSessionBindingKey(const SchemefulSite& site, const Session::Id& session_id, RestoreSessionBindingKeyCallback callback)
{
    ...
    if (session_data_->TryGetData(site.Serialize(), &site_proto)) {
        ...
        if (it != site_proto.sessions().end()) {
            ...
            std::vector<uint8_t> wrapped_key(it->second.wrapped_key().begin(), it->second.wrapped_key().end());
            key_service_->FromWrappedSigningKeySlowlyAsync(wrapped_key, BackgroundTaskPriority::kUserVisible, std::move(callback));
            return;
        }
    }
