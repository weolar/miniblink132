...
public:
    ...
    static std::vector<SessionChallengeParam> CreateIfValid(const GURL& request_url, const HttpResponseHeaders* headers);
...
private:
    SessionChallengeParam(std::optional<std::string> session_id, std::string challenge);

    static std::optional<SessionChallengeParam> ParseItem(const structured_headers::ParameterizedMember& session_challenge);
