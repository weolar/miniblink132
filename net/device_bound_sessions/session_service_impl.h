...
public:
    SessionServiceImpl(unexportable_keys::UnexportableKeyService& key_service, const URLRequestContext* request_context, SessionStore* store);
    ...
    void RegisterBoundSession(OnAccessCallback on_access_callback, RegistrationFetcherParam registration_params, const IsolationInfo& isolation_info) override;

    std::optional<Session::Id> GetAnySessionRequiringDeferral(URLRequest* request) override;

    void DeferRequestForRefresh(
        URLRequest* request, Session::Id session_id, RefreshCompleteCallback restart_callback, RefreshCompleteCallback continue_callback) override;

    void SetChallengeForBoundSession(OnAccessCallback on_access_callback, const GURL& request_url, const SessionChallengeParam& param) override;

    Session* GetSessionForTesting(const SchemefulSite& site, const std::string& session_id) const;
...
private:
    ...
    void OnRegistrationComplete(OnAccessCallback on_access_callback, std::optional<RegistrationFetcher::RegistrationCompleteParams> params);

    void StartSessionRefresh(const Session& session, const IsolationInfo& isolation_info,
    ...
    std::pair<SessionsMap::iterator, SessionsMap::iterator> GetSessionsForSite(const SchemefulSite& site);
    ...
    [[nodiscard]] SessionsMap::iterator ClearSessionInternal(const SchemefulSite& site, SessionsMap::iterator it);
