...
constexpr net::NetworkTrafficAnnotationTag kDummyAnnotation = net::DefineNetworkTrafficAnnotation("dbsc_registration", "");

class SessionServiceImplTest : public TestWithTaskEnvironment {
protected:
    SessionServiceImplTest()
        : context_(CreateTestURLRequestContextBuilder()->Build())
        , service_(*UnexportableKeyServiceFactory::GetInstance()->GetShared(), context_.get(),
              /*store=*/nullptr)
    {
    }
...
std::optional<RegistrationFetcher::RegistrationCompleteParams> TestFetcher()
{
    ...
    cookie_credentials.push_back(SessionParams::Credential { "test_cookie", "secure" });
    SessionParams::Scope scope;
    scope.include_site = true;
    SessionParams session_params(g_session_id, kUrlString, std::move(scope), std::move(cookie_credentials));
    unexportable_keys::UnexportableKeyId key_id;
    return std::make_optional<RegistrationFetcher::RegistrationCompleteParams>(std::move(session_params), std::move(key_id), kTestUrl, std::nullopt);
}
...
TEST_F(SessionServiceImplTest, TestDefer)
{
    ...
    std::unique_ptr<URLRequest> request = context_->CreateRequest(kTestUrl, IDLE, &delegate, kDummyAnnotation);
    service().DeferRequestForRefresh(request.get(), Session::Id("test"), std::move(cb1), std::move(cb2));
}
...
TEST_F(SessionServiceImplTest, RegisterSuccess)
{
    ...
    auto fetch_param = RegistrationFetcherParam::CreateInstanceForTesting(
        kTestUrl, { crypto::SignatureVerifier::SignatureAlgorithm::ECDSA_SHA256 }, "challenge", /*authorization=*/std::nullopt);
    service().RegisterBoundSession(base::DoNothing(), std::move(fetch_param), IsolationInfo::CreateTransient());
    net::TestDelegate delegate;
    std::unique_ptr<URLRequest> request = context_->CreateRequest(kTestUrl, IDLE, &delegate, kDummyAnnotation);
    ...
    std::optional<Session::Id> maybe_id = service().GetAnySessionRequiringDeferral(request.get());
...
TEST_F(SessionServiceImplTest, SetChallengeForBoundSession)
{
    ...
    scoped_refptr<net::HttpResponseHeaders> headers = HttpResponseHeaders::Builder({ 1, 1 }, "200 OK").Build();
    ...
    std::vector<SessionChallengeParam> params = SessionChallengeParam::CreateIfValid(kTestUrl, headers.get());
    ...
    const Session* session = service().GetSessionForTesting(SchemefulSite(kTestUrl), g_session_id);
    ASSERT_TRUE(session);
    EXPECT_EQ(session->cached_challenge(), "challenge");

    session = service().GetSessionForTesting(SchemefulSite(kTestUrl), "NonExisted");
...
TEST_F(SessionServiceImplTest, ExpiryExtendedOnUser)
{
    ...
    Session* session = service().GetSessionForTesting(SchemefulSite(kTestUrl), g_session_id);
    ASSERT_TRUE(session);
...
TEST_F(SessionServiceImplTest, NullAccessObserver)
{
    ...
    auto fetch_param = RegistrationFetcherParam::CreateInstanceForTesting(
        kTestUrl, { crypto::SignatureVerifier::SignatureAlgorithm::ECDSA_SHA256 }, "challenge", /*authorization=*/std::nullopt);
    service().RegisterBoundSession(SessionService::OnAccessCallback(), std::move(fetch_param), IsolationInfo::CreateTransient());
...
TEST_F(SessionServiceImplTest, AccessObserverCalledOnRegistration)
{
    ...
    service().RegisterBoundSession(future.GetRepeatingCallback<const SessionKey&>(), std::move(fetch_param), IsolationInfo::CreateTransient());
...
TEST_F(SessionServiceImplTest, AccessObserverCalledOnDeferral)
{
    ...
    base::test::TestFuture<SessionKey> future;
    request->SetDeviceBoundSessionAccessCallback(future.GetRepeatingCallback<const SessionKey&>());
    service().GetAnySessionRequiringDeferral(request.get());
...
TEST_F(SessionServiceImplTest, AccessObserverCalledOnSetChallenge)
{
    ...
    base::test::TestFuture<SessionKey> future;
    service().SetChallengeForBoundSession(future.GetRepeatingCallback<const SessionKey&>(), kTestUrl, params[0]);
...
public:
    SessionServiceImplWithStoreTest()
        ...
        , service_(*UnexportableKeyServiceFactory::GetInstance()->GetShared(), context_.get(), store_.get())
    {
    }
    ...
    StrictMock<SessionStoreMock>& store()
    {
        return *store_;
    }
...
TEST_F(SessionServiceImplWithStoreTest, UsesSessionStore)
{
    {
        InSequence seq;
        EXPECT_CALL(store(), LoadSessions).Times(1).WillOnce(Invoke(this, &SessionServiceImplWithStoreTest::OnSessionsLoaded));
