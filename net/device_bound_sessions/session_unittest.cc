...
SessionParams CreateValidParams()
{
    ...
    std::vector<SessionParams::Credential> cookie_credentials({ SessionParams::Credential { "test_cookie", "Secure; Domain=example.test" } });
    SessionParams params { kSessionId, kUrlString, std::move(scope), std::move(cookie_credentials) };
    return params;
}
...
{
    std::unique_ptr<Session> session = Session::CreateIfValid(CreateValidParams(), kTestUrl);
    ASSERT_TRUE(session);
    ...
    EXPECT_EQ(sproto.should_defer_when_expired(), session->should_defer_when_expired());
...
TEST_F(SessionTest, NotDeferredSubdomain)
{
    ...
    ASSERT_TRUE(session);
    net::TestDelegate delegate;
    std::unique_ptr<URLRequest> request = context_->CreateRequest(url_subdomain, IDLE, &delegate, kDummyAnnotation);
...
TEST_F(SessionTest, NotDeferredWithCookieSession)
{
    ...
    auto cookie
        = CanonicalCookie::Create(kTestUrl, "test_cookie=v;Secure; Domain=example.test", base::Time::Now(), std::nullopt, std::nullopt, source, &status);
    ASSERT_TRUE(cookie);
...
TEST_F(SessionTest, NotDeferredInsecure)
{
    ...
    ASSERT_TRUE(session);
    net::TestDelegate delegate;
    std::unique_ptr<URLRequest> request = context_->CreateRequest(test_insecure_url, IDLE, &delegate, kDummyAnnotation);
    request->set_site_for_cookies(SiteForCookies::FromUrl(kTestUrl));
...
public:
    ...
    CookieAccessSemantics GetAccessSemantics(const CanonicalCookie& cookie) const override
    {
    ...
    bool ShouldIgnoreSameSiteRestrictions(const GURL& url, const SiteForCookies& site_for_cookies) const override
    {
        return true;
    }
    [[nodiscard]] std::optional<std::pair<FirstPartySetMetadata, FirstPartySetsCacheFilter::MatchInfo>> ComputeFirstPartySetMetadataMaybeAsync(
        const net::SchemefulSite& site, const net::SchemefulSite* top_frame_site,
        base::OnceCallback<void(FirstPartySetMetadata, FirstPartySetsCacheFilter::MatchInfo)> callback) const override
    {
        return std::nullopt;
    }
    [[nodiscard]] std::optional<base::flat_map<net::SchemefulSite, net::FirstPartySetEntry>> FindFirstPartySetEntries(
        const base::flat_set<net::SchemefulSite>& sites,
        base::OnceCallback<void(base::flat_map<net::SchemefulSite, net::FirstPartySetEntry>)> callback) const override
    {
        return std::nullopt;
    }
};
...
{
    context_->cookie_store()->SetCookieAccessDelegate(std::make_unique<InsecureDelegate>());
    auto params = CreateValidParams();
    auto session = Session::CreateIfValid(params, kTestUrl);
    ASSERT_TRUE(session);
    net::TestDelegate delegate;
...
TEST_F(SessionTest, NotDeferredIncludedSubdomainHostCraving)
{
    ...
    spec.path = "/index.html";
    params.scope.specifications.push_back(spec);
    std::vector<SessionParams::Credential> cookie_credentials({ SessionParams::Credential { "test_cookie", "Secure;" } });
