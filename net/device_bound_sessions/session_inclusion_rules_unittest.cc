...
#define ASSERT_DOMAIN_AND_REGISTRY(origin, expected_domain_and_registry)                                                                                       \
    {                                                                                                                                                          \
        ASSERT_EQ(                                                                                                                                             \
            registry_controlled_domains::GetDomainAndRegistry(origin, registry_controlled_domains::INCLUDE_PRIVATE_REGISTRIES), expected_domain_and_registry)  \
...
void CheckEvaluateUrlTestCases(const SessionInclusionRules& inclusion_rules, std::initializer_list<EvaluateUrlTestCase> test_cases)
{
    for (const auto& test_case : test_cases) {
        SCOPED_TRACE(test_case.url);
        EXPECT_EQ(inclusion_rules.EvaluateRequestUrl(GURL(test_case.url)), test_case.expected_result);
    }
}
...
void CheckAddUrlRuleTestCases(SessionInclusionRules& inclusion_rules, std::initializer_list<AddUrlRuleTestCase> test_cases)
{
    for (const auto& test_case : test_cases) {
        SCOPED_TRACE(base::JoinString({ test_case.host_pattern, test_case.path_prefix }, ", "));
        bool is_added = inclusion_rules.AddUrlRuleIfValid(test_case.rule_type, test_case.host_pattern, test_case.path_prefix);
...
TEST(SessionInclusionRulesTest, DefaultConstructorMatchesNothing)
{
    ...
    EXPECT_EQ(Result::kExclude, inclusion_rules.EvaluateRequestUrl(GURL("https://origin.test")));
...
{
    url::Origin subdomain_origin = url::Origin::Create(GURL("https://some.site.test"));
...
TEST(SessionInclusionRulesTest, IncludeSiteAttemptedButNotAllowed)
{
    ...
    CheckEvaluateUrlTestCases(inclusion_rules, { { "https://some.site.test", Result::kInclude }, { "https://other.site.test", Result::kExclude } });
...
TEST(SessionInclusionRulesTest, IncludeSite)
{
    ...
    CheckEvaluateUrlTestCases(inclusion_rules,
        { // URL not valid.
            ...
            { "https://some.site.test", Result::kInclude }, { "https://some.other.site.test", Result::kInclude },
            // Not same host (superdomain).
            { "https://test", Result::kExclude },
            // Unrelated site.
            { "https://unrelated.test", Result::kExclude },
...
TEST(SessionInclusionRulesTest, AddUrlRuleToOriginOnly)
{
    ...
    CheckAddUrlRuleTestCases(inclusion_rules,
        { // Host pattern equals origin's host. Path is valid.
            ...
            { Result::kExclude, "*.site.test", "/", false }, { Result::kExclude, "unrelated.test", "/", false }, { Result::kExclude, "site.test", "/", false },
            { Result::kExclude, "other.site.test", "/", false }, { Result::kExclude, "https://some.site.test", "/", false },
    ...
    CheckEvaluateUrlTestCases(inclusion_rules,
        { // Matches the rule.
            ...
            // origin).
            { "https://some.site.test/staticcccccccc", Result::kInclude }, { "https://other.site.test/static", Result::kExclude },
    ...
    EXPECT_TRUE(inclusion_rules.AddUrlRuleIfValid(Result::kExclude, "some.site.test", "/"));
    EXPECT_EQ(Result::kExclude, inclusion_rules.EvaluateRequestUrl(GURL("https://some.site.test/static/included")));
}
...
TEST(SessionInclusionRulesTest, AddUrlRuleToOriginThatMayIncludeSite)
{
    ...
    CheckEvaluateUrlTestCases(inclusion_rules, { { "https://site.test/static", Result::kInclude }, { "https://other.site.test", Result::kExclude } });
    ...
    CheckAddUrlRuleTestCases(inclusion_rules,
        { { Result::kExclude, "excluded.site.test", "/", true }, { Result::kInclude, "included.site.test", "/", true },
    ...
    CheckEvaluateUrlTestCases(inclusion_rules,
        { // Path is excluded by rule.
            ...
            { "https://other.site.test", Result::kExclude }, { "https://site.test/stuff", Result::kInclude } });
}
...
TEST(SessionInclusionRulesTest, AddUrlRuleToRulesIncludingSite)
{
    ...
    CheckEvaluateUrlTestCases(inclusion_rules, { { "https://site.test/static", Result::kInclude }, { "https://other.site.test", Result::kInclude } });
    ...
    CheckEvaluateUrlTestCases(inclusion_rules,
        { // Path is excluded by rule.
            ...
            { "https://other.site.test", Result::kInclude }, { "https://site.test/stuff", Result::kInclude } });
...
TEST(SessionInclusionRulesTest, UrlRuleParsing)
{
    ...
    CheckAddUrlRuleTestCases(inclusion_rules,
        { // Empty host pattern not permitted.
            ...
            { Result::kExclude, "https://site.test", "/", false }, { Result::kExclude, "site.test:8888", "/", false },
            { Result::kExclude, "site.test,other.test", "/", false },
            // Non-IPv6-allowable characters within the brackets.
            { Result::kExclude, "[*.:abcd::3:4:ff]", "/", false }, { Result::kExclude, "[1:ab+cd::3:4:ff]", "/", false },
            ...
            { Result::kExclude, "4.31.198.44", "/", false }, { Result::kExclude, "[1:abcd::3:4:ff]", "/", false }, { Result::kExclude, "co.uk", "/", false },
            { Result::kExclude, "com", "/", false } });
}
...
TEST(SessionInclusionRulesTest, UrlRuleParsingIPv4Address)
{
    ...
    CheckAddUrlRuleTestCases(inclusion_rules,
        { // Exact host is allowed.
            ...
            // Wildcards are not permitted.
            { Result::kExclude, "*.31.198.44", "/", false }, { Result::kExclude, "*.4.31.198.44", "/", false },
            // Other hosts with no registrable domain are not allowed.
            { Result::kExclude, "[1:abcd::3:4:ff]", "/", false }, { Result::kExclude, "co.uk", "/", false }, { Result::kExclude, "com", "/", false } });
}

TEST(SessionInclusionRulesTest, UrlRuleParsingIPv6Address)
{
    url::Origin ipv6_origin = url::Origin::Create(GURL("https://[1:abcd::3:4:ff]"));
    ...
    CheckAddUrlRuleTestCases(inclusion_rules,
        { // Exact host is allowed.
            ...
            { Result::kExclude, "*.[1:abcd::3:4:ff]", "/", false },
            // Brackets mismatched.
            { Result::kExclude, "[1:abcd::3:4:ff", "/", false }, { Result::kExclude, "1:abcd::3:4:ff]", "/", false },
            // Non-IPv6-allowable characters within the brackets.
            { Result::kExclude, "[*.:abcd::3:4:ff]", "/", false }, { Result::kExclude, "[1:ab+cd::3:4:ff]", "/", false },
            { Result::kExclude, "[[1:abcd::3:4:ff]]", "/", false },
            // Other hosts with no registrable domain are not allowed.
            { Result::kExclude, "4.31.198.44", "/", false }, { Result::kExclude, "co.uk", "/", false }, { Result::kExclude, "com", "/", false } });
}
...
{
    url::Origin nonstandard_port_origin = url::Origin::Create(GURL("https://site.test:8888"));
    ...
    CheckEvaluateUrlTestCases(inclusion_rules,
        { { "https://site.test", Result::kExclude }, { "https://site.test:8888", Result::kInclude }, { "https://other.site.test", Result::kExclude } });

    // If we include_site, then same-site URLs regardless of port number are
    // included.
    inclusion_rules.SetIncludeSite(true);
    CheckEvaluateUrlTestCases(inclusion_rules,
        { { "https://site.test", Result::kInclude }, { "https://site.test:8888", Result::kInclude }, { "https://site.test:1234", Result::kInclude },
    ...
    CheckEvaluateUrlTestCases(inclusion_rules,
        { // This is same-origin but gets caught in the "site.test" rule because
            ...
            { "https://site.test", Result::kExclude }, { "https://site.test:443", Result::kExclude } });
}
...
TEST(SessionInclusionRulesTest, ToFromProto)
{
    ...
    EXPECT_TRUE(inclusion_rules.may_include_site_for_testing());
    inclusion_rules.SetIncludeSite(true);
    EXPECT_TRUE(inclusion_rules.AddUrlRuleIfValid(Result::kExclude, "excluded.site.test", "/"));
    EXPECT_TRUE(inclusion_rules.AddUrlRuleIfValid(Result::kInclude, "included.site.test", "/"));
    ...
    std::unique_ptr<SessionInclusionRules> restored_inclusion_rules = SessionInclusionRules::CreateFromProto(proto);
