...
std::string CreateHeaderStringForTest(std::optional<base::cstring_view> session_id, base::cstring_view challenge)
{
    if (session_id.has_value()) {
        return base::StringPrintf(R"("%s";%s="%s")", challenge.c_str(), kSessionIdKey, session_id->c_str());
    }
...
TEST(SessionChallengeParamTest, ValidBareChallenge)
{
    ...
    scoped_refptr<net::HttpResponseHeaders> headers = net::HttpResponseHeaders::Builder({ 1, 1 }, "200 OK")
                                                          .AddHeader(kSessionChallengeHeaderForTest, CreateHeaderStringForTest(std::nullopt, kSampleChallenge))
                                                          .Build();
    const std::vector<SessionChallengeParam> params = SessionChallengeParam::CreateIfValid(url, headers.get());
...
TEST(SessionChallengeParamTest, ValidSessionAndChallenge)
{
    ...
    scoped_refptr<net::HttpResponseHeaders> headers
        = net::HttpResponseHeaders::Builder({ 1, 1 }, "200 OK")
              .AddHeader(kSessionChallengeHeaderForTest, CreateHeaderStringForTest(kSampleSessionId, kSampleChallenge))
              .Build();
    const std::vector<SessionChallengeParam> params = SessionChallengeParam::CreateIfValid(url, headers.get());
    ASSERT_EQ(params.size(), 1U);
...
{
    const GURL url(kTestUrl);
    scoped_refptr<net::HttpResponseHeaders> headers = net::HttpResponseHeaders::Builder({ 1, 1 }, "200 OK").Build();
    const std::vector<SessionChallengeParam> params = SessionChallengeParam::CreateIfValid(url, headers.get());
    ASSERT_TRUE(params.empty());
}
...
TEST(SessionChallengeParamTest, EmptyHeader)
{
    const GURL url(kTestUrl);
    scoped_refptr<net::HttpResponseHeaders> headers
        = net::HttpResponseHeaders::Builder({ 1, 1 }, "200 OK").AddHeader(kSessionChallengeHeaderForTest, "").Build();
    const std::vector<SessionChallengeParam> params = SessionChallengeParam::CreateIfValid(url, headers.get());
    ASSERT_TRUE(params.empty());
}
...
TEST(SessionChallengeParamTest, EmptySessionId)
{
    ...
    scoped_refptr<net::HttpResponseHeaders> headers
        = net::HttpResponseHeaders::Builder({ 1, 1 }, "200 OK")
              .AddHeader(kSessionChallengeHeaderForTest, CreateHeaderStringForTest(empty_session_id, kSampleChallenge))
              .Build();
    const std::vector<SessionChallengeParam> params = SessionChallengeParam::CreateIfValid(url, headers.get());
    ASSERT_EQ(params.size(), 1U);
    EXPECT_FALSE(params[0].session_id());
...
TEST(SessionChallengeParamTest, EmptyChallenge)
{
    const GURL url(kTestUrl);
    scoped_refptr<net::HttpResponseHeaders> headers = net::HttpResponseHeaders::Builder({ 1, 1 }, "200 OK")
                                                          .AddHeader(kSessionChallengeHeaderForTest, CreateHeaderStringForTest(kSampleSessionId, ""))
                                                          .Build();
    const std::vector<SessionChallengeParam> params = SessionChallengeParam::CreateIfValid(url, headers.get());
    ASSERT_TRUE(params.empty());
}
...
TEST(SessionChallengeParamTest, NoQuotes)
{
    ...
    scoped_refptr<net::HttpResponseHeaders> headers
        = net::HttpResponseHeaders::Builder({ 1, 1 }, "200 OK")
              .AddHeader(kSessionChallengeHeaderForTest, base::StringPrintf(R"(%s;%s="%s")", kSampleChallenge, kSessionIdKey, kSampleSessionId))
              .Build();
    const std::vector<SessionChallengeParam> params = SessionChallengeParam::CreateIfValid(url, headers.get());
    ASSERT_TRUE(params.empty());
}
...
TEST(SessionChallengeParamTest, InvalidNonsenseCharacters)
{
    ...
    scoped_refptr<net::HttpResponseHeaders> headers
        = net::HttpResponseHeaders::Builder({ 1, 1 }, "200 OK")
              .AddHeader(kSessionChallengeHeaderForTest, base::StringPrintf(R"("%s"; %s="%s";;=;OTHER)", kSampleChallenge, kSessionIdKey, kSampleSessionId))
              .Build();
    const std::vector<SessionChallengeParam> params = SessionChallengeParam::CreateIfValid(url, headers.get());
    ASSERT_TRUE(params.empty());
}
...
TEST(SessionChallengeParamTest, ExtraSymbol)
{
    ...
    scoped_refptr<net::HttpResponseHeaders> headers
        = net::HttpResponseHeaders::Builder({ 1, 1 }, "200 OK")
              .AddHeader(kSessionChallengeHeaderForTest, base::StringPrintf(R"("%s"; %s="%s";cache)", kSampleChallenge, kSessionIdKey, kSampleSessionId))
              .Build();
    const std::vector<SessionChallengeParam> params = SessionChallengeParam::CreateIfValid(url, headers.get());
    ASSERT_EQ(params.size(), 1U);
    EXPECT_EQ(params[0].session_id(), kSampleSessionId);
...
TEST(SessionChallengeParamTest, ExtraParameters)
{
    ...
    scoped_refptr<net::HttpResponseHeaders> headers
        = net::HttpResponseHeaders::Builder({ 1, 1 }, "200 OK")
              .AddHeader(
                  kSessionChallengeHeaderForTest, base::StringPrintf(R"("%s"; %s="%s";cache;key=value;k=v)", kSampleChallenge, kSessionIdKey, kSampleSessionId))
              .Build();
    const std::vector<SessionChallengeParam> params = SessionChallengeParam::CreateIfValid(url, headers.get());
    ASSERT_EQ(params.size(), 1U);
    EXPECT_EQ(params[0].session_id(), kSampleSessionId);
...
TEST(SessionChallengeParamTest, InnerListParameter)
{
    const GURL url(kTestUrl);
    scoped_refptr<net::HttpResponseHeaders> headers = net::HttpResponseHeaders::Builder({ 1, 1 }, "200 OK")
                                                          .AddHeader(kSessionChallengeHeaderForTest, R"(("challenge";id="id"), ("challenge1" "id1"))")
                                                          .Build();
    const std::vector<SessionChallengeParam> params = SessionChallengeParam::CreateIfValid(url, headers.get());
    ASSERT_TRUE(params.empty());
}
...
TEST(SessionChallengeParamTest, SessionChallengeAsByteSequence)
{
    ...
    scoped_refptr<net::HttpResponseHeaders> headers
        = net::HttpResponseHeaders::Builder({ 1, 1 }, "200 OK")
              .AddHeader(kSessionChallengeHeaderForTest, base::StringPrintf(R"("%s"; %s=%s)", kSampleChallenge, kSessionIdKey, ":Y29kZWQ=:"))
              .Build();
    const std::vector<SessionChallengeParam> params = SessionChallengeParam::CreateIfValid(url, headers.get());
    ASSERT_TRUE(params.empty());
}
...
TEST(SessionChallengeParamTest, BareChallengeAsByteSequence)
{
    const GURL url(kTestUrl);
    scoped_refptr<net::HttpResponseHeaders> headers
        = net::HttpResponseHeaders::Builder({ 1, 1 }, "200 OK").AddHeader(kSessionChallengeHeaderForTest, ":Y29kZWQ=:").Build();
    const std::vector<SessionChallengeParam> params = SessionChallengeParam::CreateIfValid(url, headers.get());
    ASSERT_TRUE(params.empty());
}
...
TEST(SessionChallengeParamTest, ValidTwoSessionChallenges)
{
    ...
    scoped_refptr<net::HttpResponseHeaders> headers
        = net::HttpResponseHeaders::Builder({ 1, 1 }, "200 OK")
              .AddHeader(kSessionChallengeHeaderForTest, CreateHeaderStringForTest(kSampleSessionId, kSampleChallenge))
              .AddHeader(kSessionChallengeHeaderForTest, CreateHeaderStringForTest(session_id2, challenge2))
              .Build();
    const std::vector<SessionChallengeParam> params = SessionChallengeParam::CreateIfValid(url, headers.get());
...
TEST(SessionChallengeParamTest, ValidTwoBareChallenges)
{
    ...
    scoped_refptr<net::HttpResponseHeaders> headers = net::HttpResponseHeaders::Builder({ 1, 1 }, "200 OK")
                                                          .AddHeader(kSessionChallengeHeaderForTest, CreateHeaderStringForTest(std::nullopt, kSampleChallenge))
                                                          .AddHeader(kSessionChallengeHeaderForTest, CreateHeaderStringForTest(std::nullopt, challenge2))
                                                          .Build();
    const std::vector<SessionChallengeParam> params = SessionChallengeParam::CreateIfValid(url, headers.get());
...
TEST(SessionChallengeParamTest, ValidMixedChallenges)
{
    ...
    scoped_refptr<net::HttpResponseHeaders> headers
        = net::HttpResponseHeaders::Builder({ 1, 1 }, "200 OK")
              .AddHeader(kSessionChallengeHeaderForTest, CreateHeaderStringForTest(std::nullopt, challenge))
              .AddHeader(kSessionChallengeHeaderForTest, CreateHeaderStringForTest(kSampleSessionId, kSampleChallenge))
              .Build();
    const std::vector<SessionChallengeParam> params = SessionChallengeParam::CreateIfValid(url, headers.get());
    ...
    scoped_refptr<net::HttpResponseHeaders> headers
        = net::HttpResponseHeaders::Builder({ 1, 1 }, "200 OK")
              .AddHeader(kSessionChallengeHeaderForTest,
                  base::StrCat({ CreateHeaderStringForTest(kSampleSessionId, kSampleChallenge), ",", CreateHeaderStringForTest(session_id2, challenge2) }))
              .Build();
    const std::vector<SessionChallengeParam> params = SessionChallengeParam::CreateIfValid(url, headers.get());
...
TEST(SessionChallengeParamTest, ThreeChallengesInTwoHeaders)
{
    ...
    scoped_refptr<net::HttpResponseHeaders> headers
        = net::HttpResponseHeaders::Builder({ 1, 1 }, "200 OK")
              .AddHeader(kSessionChallengeHeaderForTest,
                  base::StrCat({ CreateHeaderStringForTest(kSampleSessionId, kSampleChallenge), ", ", CreateHeaderStringForTest(session_id2, challenge2) }))
              .AddHeader(kSessionChallengeHeaderForTest, CreateHeaderStringForTest(session_id3, challenge3))
              .Build();
    const std::vector<SessionChallengeParam> params = SessionChallengeParam::CreateIfValid(url, headers.get());
