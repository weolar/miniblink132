...
const base::FilePath::CharType dbsc_filename[] = FILE_PATH_LITERAL("DBSC_Sessions");
...
public:
    DBSCSessionStorePerfTest()
        : key_service_(task_manager_)
    {
    }

    void CreateStore()
    {
        store_ = std::make_unique<SessionStoreImpl>(temp_dir_.GetPath().Append(dbsc_filename), key_service_);
    }
    ...
    unexportable_keys::UnexportableKeyId GenerateNewKey()
    {
        ...
        key_service_.GenerateSigningKeySlowlyAsync(kAcceptableAlgorithms, kTaskPriority, generate_future.GetCallback());
    ...
    void AddSession(int site_idx, int session_idx)
    {
        ...
        std::string url_str = base::StringPrintf("https://%d.example%d.test", session_idx, site_idx);
        std::string refresh_url = base::StringPrintf("https://example%d.test/refresh.html", site_idx);
        std::string cookie_name = base::StringPrintf("cookie_%d_%d", site_idx, session_idx);
        std::string cookie_attr = base::StringPrintf("Secure; Domain=example%d.test", site_idx);

        SessionParams::Scope scope;
        std::vector<SessionParams::Credential> cookie_credentials({ SessionParams::Credential { cookie_name, cookie_attr } });
        SessionParams params { session_str, refresh_url, std::move(scope), std::move(cookie_credentials) };
        std::unique_ptr<Session> session = Session::CreateIfValid(params, GURL(url_str));
        ASSERT_TRUE(session);
    ...
    unsigned int NumSessionsInStore()
    {
        return store_->GetAllSessions().size();
    }
