...
SessionParams::Scope ParseScope(const base::Value::Dict& scope_dict)
{
    ...
    const base::Value::List* specifications_list = scope_dict.FindList("scope_specification");
    ...
    for (const auto& specification : *specifications_list) {
        ...
        if (type && !type->empty() && domain && !domain->empty() && path && !path->empty()) {
            if (*type == "include") {
                scope.specifications.push_back(SessionParams::Scope::Specification { SessionParams::Scope::Specification::Type::kInclude, *domain, *path });
            } else if (*type == "exclude") {
                scope.specifications.push_back(SessionParams::Scope::Specification { SessionParams::Scope::Specification::Type::kExclude, *domain, *path });
            }
        }
    }
...
std::vector<SessionParams::Credential> ParseCredentials(const base::Value::List& credentials_list)
{
...
std::optional<SessionParams> ParseSessionInstructionJson(std::string_view response_json)
{
    ...
    std::optional<base::Value::Dict> maybe_root = base::JSONReader::ReadDict(response_json, base::JSON_PARSE_RFC, /*max_depth=*/5u);
    if (!maybe_root) {
        return std::nullopt;
    }
    ...
    return SessionParams(*session_id, refresh_url ? *refresh_url : "", scope_dict ? ParseScope(*scope_dict) : SessionParams::Scope {}, std::move(credentials));
}
