...
public:
    static std::unique_ptr<SessionStore> Create(const base::FilePath& db_storage_path);
    ...
    virtual void SaveSession(const SchemefulSite& site, const Session& session) = 0;

    virtual void DeleteSession(const SchemefulSite& site, const Session::Id& session_id) = 0;
    ...
    using RestoreSessionBindingKeyCallback = base::OnceCallback<void(unexportable_keys::ServiceErrorOr<unexportable_keys::UnexportableKeyId>)>;
    virtual void RestoreSessionBindingKey(const SchemefulSite& site, const Session::Id& session_id, RestoreSessionBindingKeyCallback callback) = 0;
