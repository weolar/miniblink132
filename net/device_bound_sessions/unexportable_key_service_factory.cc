...
#if BUILDFLAG(IS_MAC)
constexpr char kKeychainAccessGroup[] = MAC_TEAM_IDENTIFIER_STRING "." MAC_BUNDLE_IDENTIFIER_STRING ".unexportable-keys";
constexpr char kKeychainAccessGroup[] = ".org.chromium.Chromium.unexportable-keys";
#endif // BUILDFLAG(IS_MAC)

// Returns a newly created task manager instance, or nullptr if unexportable
// keys are not available.
std::unique_ptr<unexportable_keys::UnexportableKeyTaskManager> CreateTaskManagerInstance()
{
    ...
    if (!unexportable_keys::UnexportableKeyServiceImpl::IsUnexportableKeyProviderSupported(config)) {
        return nullptr;
    }
    return std::make_unique<unexportable_keys::UnexportableKeyTaskManager>(std::move(config));
}
...
{
    static base::NoDestructor<std::unique_ptr<unexportable_keys::UnexportableKeyTaskManager>> instance(CreateTaskManagerInstance());
    return instance->get();
}
...
void UnexportableKeyServiceFactory::SetUnexportableKeyFactoryForTesting(unexportable_keys::UnexportableKeyService* (*func)())
{
    if (g_mock_factory) {
        CHECK(!func);
...
unexportable_keys::UnexportableKeyService* UnexportableKeyServiceFactory::GetShared()
{
    if (g_mock_factory) {
    ...
    if (!has_created_service_) {
        ...
        unexportable_keys::UnexportableKeyTaskManager* task_manager = GetSharedTaskManagerInstance();
        if (task_manager) {
            unexportable_key_service_ = std::make_unique<unexportable_keys::UnexportableKeyServiceImpl>(*task_manager);
        }
    }
...
UnexportableKeyServiceFactory* UnexportableKeyServiceFactory::GetInstanceForTesting()
{
