...
TEST(HttpAuthNegotiateAndroidTest, GenerateAuthToken)
{
    ...
    authenticator.ExpectSecurityContext("Negotiate", GSS_S_COMPLETE, 0, mockContext, "", "DummyToken");

    MockAllowHttpAuthPreferences prefs;
    prefs.set_auth_android_negotiate_account_type("org.chromium.test.DummySpnegoAuthenticator");
    HttpAuthNegotiateAndroid auth(&prefs);
    EXPECT_TRUE(auth.Init(NetLogWithSource()));

    TestCompletionCallback callback;
    EXPECT_EQ(OK, callback.GetResult(auth.GenerateAuthToken(nullptr, "Dummy", std::string(), &auth_token, NetLogWithSource(), callback.callback())));
...
TEST(HttpAuthNegotiateAndroidTest, ParseChallenge_FirstRound)
{
    ...
    EXPECT_EQ(HttpAuth::AUTHORIZATION_RESULT_ACCEPT, auth.ParseChallenge(&challenge));
}
...
TEST(HttpAuthNegotiateAndroidTest, ParseChallenge_UnexpectedTokenFirstRound)
{
    ...
    EXPECT_EQ(HttpAuth::AUTHORIZATION_RESULT_INVALID, auth.ParseChallenge(&challenge));
}
...
TEST(HttpAuthNegotiateAndroidTest, ParseChallenge_TwoRounds)
{
    ...
    EXPECT_EQ(HttpAuth::AUTHORIZATION_RESULT_ACCEPT, auth.ParseChallenge(&first_challenge));

    HttpAuthChallengeTokenizer second_challenge("Negotiate Zm9vYmFy");
    EXPECT_EQ(HttpAuth::AUTHORIZATION_RESULT_ACCEPT, auth.ParseChallenge(&second_challenge));
}
...
TEST(HttpAuthNegotiateAndroidTest, ParseChallenge_MissingTokenSecondRound)
{
    ...
    EXPECT_EQ(HttpAuth::AUTHORIZATION_RESULT_REJECT, auth.ParseChallenge(&second_challenge));
}
