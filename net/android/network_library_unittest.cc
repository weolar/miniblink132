...
TEST(NetworkLibraryTest, GetDnsSearchDomains)
{
    ...
    if (!GetCurrentDnsServers(&dns_servers, &dns_over_tls_active, &dns_over_tls_hostname, &search_suffixes)) {
        return;
    }
...
TEST(NetworkLibraryTest, GetDnsSearchDomainsForNetwork)
{
    ...
    if (!GetDnsServersForNetwork(&dns_servers, &dns_over_tls_active, &dns_over_tls_hostname, &search_suffixes, default_network_handle)) {
        return;
    }
...
TEST(NetworkLibraryTest, BindToNetwork)
{
    ...
    NetworkChangeNotifier::DisableForTest ncn_disable_for_test;
    std::unique_ptr<NetworkChangeNotifier> ncn(ncn_factory.CreateInstance());
    std::unique_ptr<TCPSocket> socket_tcp_ipv4 = TCPSocket::Create(nullptr, nullptr, NetLogSource());
    ASSERT_EQ(OK, socket_tcp_ipv4->Open(ADDRESS_FAMILY_IPV4));
    std::unique_ptr<TCPSocket> socket_tcp_ipv6 = TCPSocket::Create(nullptr, nullptr, NetLogSource());
    ASSERT_EQ(OK, socket_tcp_ipv6->Open(ADDRESS_FAMILY_IPV6));
    UDPSocket socket_udp_ipv4(DatagramSocket::DEFAULT_BIND, nullptr, NetLogSource());
    ASSERT_EQ(OK, socket_udp_ipv4.Open(ADDRESS_FAMILY_IPV4));
    UDPSocket socket_udp_ipv6(DatagramSocket::DEFAULT_BIND, nullptr, NetLogSource());
    ASSERT_EQ(OK, socket_udp_ipv6.Open(ADDRESS_FAMILY_IPV6));
    std::array sockets { socket_tcp_ipv4->SocketDescriptorForTesting(), socket_tcp_ipv6->SocketDescriptorForTesting(),
        socket_udp_ipv4.SocketDescriptorForTesting(), socket_udp_ipv6.SocketDescriptorForTesting() };
    ...
    for (SocketDescriptor socket : sockets) {
        if (base::android::BuildInfo::GetInstance()->sdk_int() >= base::android::SDK_VERSION_LOLLIPOP) {
            ...
            handles::NetworkHandle existing_network_handle = NetworkChangeNotifier::GetDefaultNetwork();
            ...
            EXPECT_EQ(ERR_INVALID_ARGUMENT, BindToNetwork(socket, handles::kInvalidNetworkHandle));
        }
        ...
        } else if (base::android::BuildInfo::GetInstance()->sdk_int() >= base::android::SDK_VERSION_LOLLIPOP
        ...
        } else if (base::android::BuildInfo::GetInstance()->sdk_int() >= base::android::SDK_VERSION_MARSHMALLOW) {
