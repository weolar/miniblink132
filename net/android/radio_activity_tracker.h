...
public:
    ...
    void OverrideRadioActivityForTesting(absl::optional<base::android::RadioDataActivity> radio_activity)
    {
        radio_activity_override_for_testing_ = radio_activity;
    }
    void OverrideRadioTypeForTesting(absl::optional<base::android::RadioConnectionType> radio_type)
    {
...
private:
    ...
    base::android::RadioDataActivity last_radio_data_activity_ = base::android::RadioDataActivity::kNone;
    base::TimeTicks last_check_time_;

    // Radio state overrides for testing.
    absl::optional<base::android::RadioDataActivity> radio_activity_override_for_testing_;
    absl::optional<base::android::RadioConnectionType> radio_type_override_for_testing_;
};

constexpr char kUmaNamePossibleWakeupTriggerTCPWriteAnnotationId[] = "Net.Radio.PossibleWakeupTrigger.TCPWriteAnnotationId";
constexpr char kUmaNamePossibleWakeupTriggerUDPWriteAnnotationId[] = "Net.Radio.PossibleWakeupTrigger.UDPWriteAnnotationId";

// Records a histogram when writing data to a TCP socket likely wake-ups radio.
NET_EXPORT void MaybeRecordTCPWriteForWakeupTrigger(const NetworkTrafficAnnotationTag& traffic_annotation);

// Records a histogram when writing data to a UDP socket likely wake-ups radio.
NET_EXPORT void MaybeRecordUDPWriteForWakeupTrigger(const NetworkTrafficAnnotationTag& traffic_annotation);
