...
using DnsServerGetter = base::RepeatingCallback<bool(
    std::vector<IPEndPoint>* dns_servers, bool* dns_over_tls_active, std::string* dns_over_tls_hostname, std::vector<std::string>* search_suffixes)>;
...
// Returns false when a valid server config could not be read.
NET_EXPORT_PRIVATE bool GetDnsServersForNetwork(std::vector<IPEndPoint>* dns_servers, bool* dns_over_tls_active, std::string* dns_over_tls_hostname,
...
NET_EXPORT_PRIVATE void TagSocket(SocketDescriptor socket, uid_t uid, int32_t tag);
...
NET_EXPORT_PRIVATE int BindToNetwork(SocketDescriptor socket, handles::NetworkHandle network);
