...
public:
    ...
    {
        return base::BindOnce(&ResultSavingCookieCallback<T>::Run, base::Unretained(this));
    }
...
public:
    ...
    void Run(const CookieAccessResultList& cookies, const CookieAccessResultList& excluded_cookies);

    // Makes a callback that will invoke Run. Assumes that |this| will be kept
    // alive till the time the callback is used.
    base::OnceCallback<void(const CookieAccessResultList&, const CookieAccessResultList&)> MakeCallback()
    {
...
public:
    ...
    explicit GetAllCookiesWithAccessSemanticsCallback(base::Thread* run_in_thread);

    ~GetAllCookiesWithAccessSemanticsCallback();

    void Run(const CookieList& cookies, const std::vector<CookieAccessSemantics>& access_semantics_list);

    // Makes a callback that will invoke Run. Assumes that |this| will be kept
    // alive till the time the callback is used.
    base::OnceCallback<void(const CookieList&, const std::vector<CookieAccessSemantics>&)> MakeCallback()
    {
        return base::BindOnce(&GetAllCookiesWithAccessSemanticsCallback::Run, base::Unretained(this));
    }
