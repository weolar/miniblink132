...
public:
    ...
    explicit CookiePartitionKeyCollection(base::flat_set<CookiePartitionKey> keys);

    CookiePartitionKeyCollection& operator=(const CookiePartitionKeyCollection& other);
    ...
    static CookiePartitionKeyCollection MatchesSite(const net::SchemefulSite& top_level_site);

    static CookiePartitionKeyCollection FromOptional(const std::optional<CookiePartitionKey>& opt_key)
    {
        return opt_key ? CookiePartitionKeyCollection(opt_key.value()) : CookiePartitionKeyCollection();
    }
    ...
    bool IsEmpty() const
    {
        return !contains_all_keys_ && keys_.empty();
    }

    // Returns if the key collection contains every partition key.
    bool ContainsAllKeys() const
    {
        return contains_all_keys_;
    }
...
NET_EXPORT bool operator==(const CookiePartitionKeyCollection& lhs, const CookiePartitionKeyCollection& rhs);

NET_EXPORT std::ostream& operator<<(std::ostream& os, const CookiePartitionKeyCollection& keys);
