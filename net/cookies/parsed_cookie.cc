...
inline bool SeekToCharacter(std::string_view::iterator* it, const std::string_view::iterator& end, const char character)
{
...
inline bool SeekTo(std::string_view::iterator* it, const std::string_view::iterator& end, const char* chars)
{
...
// Returns true if it hit the end, false otherwise.
inline bool SeekPast(std::string_view::iterator* it, const std::string_view::iterator& end, const char* chars)
{
    for (; *it != end && CharIsA(**it, chars); ++(*it)) {
    }
    return *it == end;
}
inline bool SeekBackPast(std::string_view::iterator* it, const std::string_view::iterator& end, const char* chars)
{
    for (; *it != end && CharIsA(**it, chars); --(*it)) {
    }
    return *it == end;
...
std::string_view ValidStringPieceForValue(std::string_view value)
{
    ...
    std::string_view::iterator end = net::ParsedCookie::FindFirstTerminator(value);
...
ParsedCookie::ParsedCookie(std::string_view cookie_line, CookieInclusionStatus* status_out)
{
...
CookieSameSite ParsedCookie::SameSite(CookieSameSiteString* samesite_string) const
{
    CookieSameSite samesite = CookieSameSite::UNSPECIFIED;
    if (same_site_index_ != 0) {
        samesite = StringToCookieSameSite(pairs_[same_site_index_].second, samesite_string);
...
std::string ParsedCookie::ToCookieLine() const
{
    ...
    for (auto it = pairs_.begin(); it != pairs_.end(); ++it) {
        ...
        if (it == pairs_.begin() || (it->first != kSecureTokenName && it->first != kHttpOnlyTokenName && it->first != kPartitionedTokenName)) {
            out.append("=");
...
// static
std::string_view::iterator ParsedCookie::FindFirstTerminator(std::string_view s)
{
    std::string_view::iterator end = s.end();
    size_t term_pos = s.find_first_of(std::string_view(kTerminator, kTerminatorLen));
...
// static
bool ParsedCookie::ParseToken(
    std::string_view::iterator* it, const std::string_view::iterator& end, std::string_view::iterator* token_start, std::string_view::iterator* token_end)
{
...
// static
void ParsedCookie::ParseValue(
    std::string_view::iterator* it, const std::string_view::iterator& end, std::string_view::iterator* value_start, std::string_view::iterator* value_end)
{
...
// static
bool ParsedCookie::CookieAttributeValueHasValidCharSet(const std::string& value)
{
...
// static
bool ParsedCookie::IsValidCookieNameValuePair(const std::string& name, const std::string& value, CookieInclusionStatus* status_out)
{
    // Ignore cookies with neither name nor value.
    if (name.empty() && value.empty()) {
        if (status_out != nullptr) {
            status_out->AddExclusionReason(CookieInclusionStatus::EXCLUDE_NO_COOKIE_CONTENT);
        }
    ...
    if (!name_value_pair_size.IsValid() || (name_value_pair_size.ValueOrDie() > kMaxCookieNamePlusValueSize)) {
        if (status_out != nullptr) {
            status_out->AddExclusionReason(CookieInclusionStatus::EXCLUDE_NAME_VALUE_PAIR_EXCEEDS_MAX_SIZE);
        }
        return false;
    }
    ...
    if (!IsValidCookieName(name) || !IsValidCookieValue(value)) {
        if (status_out != nullptr) {
            status_out->AddExclusionReason(CookieInclusionStatus::EXCLUDE_DISALLOWED_CHARACTER);
        }
        return false;
    }
    return true;
...
void ParsedCookie::ParseTokenValuePairs(std::string_view cookie_line, CookieInclusionStatus& status_out)
{
    pairs_.clear();
    ...
    for (int pair_num = 0; it != end; ++pair_num) {
        ...
        if (pair_num == 0 && (pair.first.find_first_of("\t") != std::string::npos || pair.second.find_first_of("\t") != std::string::npos)) {
            internal_htab_ = true;
        }
        ...
        if (pair_num == 0) {
            ...
            if (!CookieAttributeValueHasValidSize(pair.second)) {
                ...
                ignore_pair = true;
                status_out.AddWarningReason(CookieInclusionStatus::WARN_ATTRIBUTE_VALUE_EXCEEDS_MAX_SIZE);
            }
        }
...
bool ParsedCookie::SetString(size_t* index, const std::string& key, const std::string& untrusted_value)
{
...
bool ParsedCookie::SetAttributePair(size_t* index, const std::string& key, const std::string& value)
{
...
void ParsedCookie::ClearAttributePair(size_t index)
{
    ...
    size_t* indexes[] = { &path_index_, &domain_index_, &expires_index_, &maxage_index_, &secure_index_, &httponly_index_, &same_site_index_, &priority_index_,
        &partitioned_index_ };
