...
struct NET_EXPORT CookieChangeInfo {
    CookieChangeInfo();
    CookieChangeInfo(const CanonicalCookie& cookie, CookieAccessResult access_result, CookieChangeCause cause);
    ~CookieChangeInfo();
...
using CookieChangeCallback = base::RepeatingCallback<void(const CookieChangeInfo&)>;
...
public:
    ...
    // parameter.
    [[nodiscard]] virtual std::unique_ptr<CookieChangeSubscription> AddCallbackForCookie(
    ...
    // Unpartitioned cookies are not affected by the `cookie_partition_key`
    // parameter.
    [[nodiscard]] virtual std::unique_ptr<CookieChangeSubscription> AddCallbackForUrl(
    ...
    [[nodiscard]] virtual std::unique_ptr<CookieChangeSubscription> AddCallbackForAllChanges(CookieChangeCallback callback) = 0;
};
