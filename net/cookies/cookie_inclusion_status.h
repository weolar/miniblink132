...
public:
    ...
    using ExclusionReasonBitset = std::bitset<ExclusionReason::NUM_EXCLUSION_REASONS>;
    ...
    ExemptionReason exemption_reason() const
    {
        return exemption_reason_;
    }
    ...
    bool HasSchemefulDowngradeWarning(CookieInclusionStatus::WarningReason* reason = nullptr) const;
    ...
    ExclusionReasonBitset exclusion_reasons() const
    {
    ...
    WarningReasonBitset warning_reasons() const
    {
        return warning_reasons_;
    }
    ...
    ContextDowngradeMetricValues GetBreakingDowngradeMetricsEnumValue(const GURL& url) const;
    ...
    bool HasExactlyExclusionReasonsForTesting(std::vector<ExclusionReason> reasons) const;

    // Checks whether the warning reasons are exactly the set of warning
    // reasons in the vector. (Ignores exclusions.)
    bool HasExactlyWarningReasonsForTesting(std::vector<WarningReason> reasons) const;
    ...
    static bool ValidateExclusionAndWarningFromWire(uint32_t exclusion_reasons, uint32_t warning_reasons);
    ...
    static CookieInclusionStatus MakeFromReasonsForTesting(std::vector<ExclusionReason> exclusions,
        std::vector<WarningReason> warnings = std::vector<WarningReason>(), ExemptionReason exemption = ExemptionReason::kNone, bool use_literal = false);
...
private:
    ...
    // and exemption.
    CookieInclusionStatus(std::vector<ExclusionReason> exclusions, std::vector<WarningReason> warnings, ExemptionReason exemption);

    // Returns the `exclusion_reasons_` with the given `reasons` unset.
    ExclusionReasonBitset ExclusionReasonsWithout(const std::vector<ExclusionReason>& reasons) const;
...
NET_EXPORT inline std::ostream& operator<<(std::ostream& os, const CookieInclusionStatus status)
{
