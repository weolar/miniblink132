...
const std::string GetArbitraryNameValueString(FuzzedDataProvider* data_provider)
{
    ...
    return data_provider->ConsumeRandomLengthString(net::ParsedCookie::kMaxCookieNamePlusValueSize * 2);
}

const std::string GetArbitraryAttributeValueString(FuzzedDataProvider* data_provider)
{
    // Adding a fudge factor to kMaxCookieAttributeValueSize so that both branches
    // of the bounds detection code will be tested.
    return data_provider->ConsumeRandomLengthString(net::ParsedCookie::kMaxCookieAttributeValueSize + 10);
}
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    if (parsed_cookie.IsValid()) {
        ...
        case 3:
            if (parsed_cookie.IsValid())
                parsed_cookie.SetPath(GetArbitraryAttributeValueString(&data_provider));
            break;
        case 4:
            parsed_cookie.SetDomain(GetArbitraryAttributeValueString(&data_provider));
            break;
        case 5:
            parsed_cookie.SetExpires(GetArbitraryAttributeValueString(&data_provider));
            break;
        case 6:
            parsed_cookie.SetMaxAge(GetArbitraryAttributeValueString(&data_provider));
            break;
        case 7:
        ...
        case 8:
            ...
            break;
        case 9:
            parsed_cookie.SetSameSite(GetArbitraryAttributeValueString(&data_provider));
            break;
        case 10:
            parsed_cookie.SetPriority(GetArbitraryAttributeValueString(&data_provider));
            break;
        case 11:
