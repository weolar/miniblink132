...
public:
    ...
    static bool FromWire(const SchemefulSite& site, bool schemefully_same, SiteForCookies* out);
    ...
    // registered domain.
    bool IsFirstPartyWithSchemefulMode(const GURL& url, bool compute_schemefully) const;
    ...
    {
        return site_.site_as_origin_.scheme();
    }
    ...
    // or std::map).
    NET_EXPORT friend bool operator<(const SiteForCookies& lhs, const SiteForCookies& rhs);
