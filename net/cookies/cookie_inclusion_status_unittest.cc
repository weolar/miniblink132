...
{
    int num_exclusion_reasons = static_cast<int>(CookieInclusionStatus::NUM_EXCLUSION_REASONS);
    int num_warning_reasons = static_cast<int>(CookieInclusionStatus::NUM_WARNING_REASONS);
    // Zero-argument constructor
    CookieInclusionStatus status;
    EXPECT_TRUE(status.IsInclude());
    for (int i = 0; i < num_exclusion_reasons; ++i) {
        EXPECT_FALSE(status.HasExclusionReason(static_cast<CookieInclusionStatus::ExclusionReason>(i)));
    }
    for (int i = 0; i < num_warning_reasons; ++i) {
        EXPECT_FALSE(status.HasWarningReason(static_cast<CookieInclusionStatus::WarningReason>(i)));
    }
    EXPECT_FALSE(status.HasExclusionReason(CookieInclusionStatus::EXCLUDE_UNKNOWN_ERROR));
}
...
TEST(CookieInclusionStatusTest, ExcludeStatus)
{
    ...
    for (int i = 0; i < num_exclusion_reasons; ++i) {
        ...
        if (reason1 != CookieInclusionStatus::EXCLUDE_THIRD_PARTY_PHASEOUT
            && reason1 != CookieInclusionStatus::EXCLUDE_THIRD_PARTY_BLOCKED_WITHIN_FIRST_PARTY_SET) {
            continue;
        }
        ...
        for (int j = 0; j < num_exclusion_reasons; ++j) {
            ...
            if (reason2 != CookieInclusionStatus::EXCLUDE_THIRD_PARTY_PHASEOUT
                && reason2 != CookieInclusionStatus::EXCLUDE_THIRD_PARTY_BLOCKED_WITHIN_FIRST_PARTY_SET) {
                continue;
            }
            ...
            if (reason1 != CookieInclusionStatus::EXCLUDE_THIRD_PARTY_PHASEOUT && reason2 != CookieInclusionStatus::EXCLUDE_THIRD_PARTY_PHASEOUT) {
...
TEST(CookieInclusionStatusTest, ExcludeStatus_MaybeClearThirdPartyPhaseoutReason)
{
    int num_exclusion_reasons = static_cast<int>(CookieInclusionStatus::NUM_EXCLUSION_REASONS);
    CookieInclusionStatus::ExclusionReason reason1 = CookieInclusionStatus::EXCLUDE_THIRD_PARTY_PHASEOUT;
    ...
    for (int j = 0; j < num_exclusion_reasons; ++j) {
        ...
        if (reason2 == CookieInclusionStatus::EXCLUDE_THIRD_PARTY_BLOCKED_WITHIN_FIRST_PARTY_SET) {
            EXPECT_TRUE(status_two_reasons.HasExclusionReason(reason1));
            EXPECT_TRUE(status_two_reasons.HasExclusionReason(reason2));
        } else {
...
TEST(CookieInclusionStatusTest, AddExclusionReason_MaybeClearThirdPartyPhaseoutReason)
{
    CookieInclusionStatus status;
    status.AddWarningReason(CookieInclusionStatus::WARN_THIRD_PARTY_PHASEOUT);
    ASSERT_TRUE(status.ShouldWarn());
    ASSERT_TRUE(status.HasExactlyWarningReasonsForTesting({ CookieInclusionStatus::WARN_THIRD_PARTY_PHASEOUT }));
    // Adding an exclusion reason should clear 3PCD warning reason.
    status.AddExclusionReason(CookieInclusionStatus::EXCLUDE_THIRD_PARTY_PHASEOUT);
    EXPECT_TRUE(status.HasExactlyExclusionReasonsForTesting({ CookieInclusionStatus::EXCLUDE_THIRD_PARTY_PHASEOUT }));
    EXPECT_FALSE(status.ShouldWarn());

    status.AddExclusionReason(CookieInclusionStatus::EXCLUDE_THIRD_PARTY_BLOCKED_WITHIN_FIRST_PARTY_SET);
    ...
    EXPECT_TRUE(status.HasExactlyExclusionReasonsForTesting({ CookieInclusionStatus::EXCLUDE_SAMESITE_NONE_INSECURE }));
...
TEST(CookieInclusionStatusTest, AddExclusionReason)
{
    ...
    EXPECT_TRUE(status.HasExactlyExclusionReasonsForTesting({ CookieInclusionStatus::EXCLUDE_UNKNOWN_ERROR }));
    ...
    EXPECT_TRUE(status.HasExactlyExclusionReasonsForTesting({ CookieInclusionStatus::EXCLUDE_SAMESITE_UNSPECIFIED_TREATED_AS_LAX }));
    EXPECT_TRUE(status.HasExactlyWarningReasonsForTesting({ CookieInclusionStatus::WARN_SAMESITE_UNSPECIFIED_CROSS_SITE_CONTEXT }));
}

TEST(CookieInclusionStatusTest, ExemptionReason)
{
    CookieInclusionStatus status;
    status.MaybeSetExemptionReason(CookieInclusionStatus::ExemptionReason::k3PCDMetadata);
    ASSERT_EQ(status.exemption_reason(), CookieInclusionStatus::ExemptionReason::k3PCDMetadata);
    ASSERT_TRUE(status.IsInclude());
    ASSERT_EQ(status.GetDebugString(), "INCLUDE, DO_NOT_WARN, Exemption3PCDMetadata");

    // Updating exemption reason would be no-op.
    status.MaybeSetExemptionReason(CookieInclusionStatus::ExemptionReason::kEnterprisePolicy);
    EXPECT_EQ(status.exemption_reason(), CookieInclusionStatus::ExemptionReason::k3PCDMetadata);

    // Adding an exclusion reason resets the exemption reason.
    status.AddExclusionReason(CookieInclusionStatus::EXCLUDE_UNKNOWN_ERROR);
    EXPECT_EQ(status.exemption_reason(), CookieInclusionStatus::ExemptionReason::kNone);
...
TEST(CookieInclusionStatusTest, CheckEachWarningReason)
{
    ...
    for (int i = 0; i < num_warning_reasons; ++i) {
        ...
        for (int j = 0; j < num_warning_reasons; ++j) {
            if (i == j)
                continue;
            EXPECT_FALSE(status.HasWarningReason(static_cast<CookieInclusionStatus::WarningReason>(j)));
        }
...
TEST(CookieInclusionStatusTest, RemoveExclusionReason)
{
    ...
    ASSERT_TRUE(status.HasExclusionReason(CookieInclusionStatus::EXCLUDE_UNKNOWN_ERROR));
    ...
    ASSERT_FALSE(status.HasExclusionReason(CookieInclusionStatus::NUM_EXCLUSION_REASONS));
    status.RemoveExclusionReason(CookieInclusionStatus::NUM_EXCLUSION_REASONS);
    EXPECT_FALSE(status.HasExclusionReason(CookieInclusionStatus::NUM_EXCLUSION_REASONS));
}

TEST(CookieInclusionStatusTest, RemoveWarningReason)
{
    CookieInclusionStatus status(CookieInclusionStatus::EXCLUDE_UNKNOWN_ERROR, CookieInclusionStatus::WARN_SAMESITE_NONE_INSECURE);
    EXPECT_TRUE(status.ShouldWarn());
    ASSERT_TRUE(status.HasWarningReason(CookieInclusionStatus::WARN_SAMESITE_NONE_INSECURE));

    status.RemoveWarningReason(CookieInclusionStatus::WARN_SAMESITE_NONE_INSECURE);
    EXPECT_FALSE(status.ShouldWarn());
    EXPECT_FALSE(status.HasWarningReason(CookieInclusionStatus::WARN_SAMESITE_NONE_INSECURE));

    // Removing a nonexistent warning reason doesn't do anything.
    ASSERT_FALSE(status.HasWarningReason(CookieInclusionStatus::WARN_SAMESITE_UNSPECIFIED_CROSS_SITE_CONTEXT));
    status.RemoveWarningReason(CookieInclusionStatus::WARN_SAMESITE_UNSPECIFIED_CROSS_SITE_CONTEXT);
    EXPECT_FALSE(status.ShouldWarn());
    EXPECT_FALSE(status.HasWarningReason(CookieInclusionStatus::WARN_SAMESITE_UNSPECIFIED_CROSS_SITE_CONTEXT));
}
...
TEST(CookieInclusionStatusTest, HasSchemefulDowngradeWarning)
{
    ...
    not_downgrade.AddWarningReason(CookieInclusionStatus::WARN_SAMESITE_UNSPECIFIED_CROSS_SITE_CONTEXT);
...
{
    EXPECT_TRUE(CookieInclusionStatus::MakeFromReasonsForTesting({}).ShouldRecordDowngradeMetrics());
...
{
    CookieInclusionStatus status = CookieInclusionStatus::MakeFromReasonsForTesting({
    ...
    status.RemoveExclusionReasons(
        { CookieInclusionStatus::EXCLUDE_UNKNOWN_ERROR, CookieInclusionStatus::EXCLUDE_UNKNOWN_ERROR, CookieInclusionStatus::EXCLUDE_SAMESITE_STRICT });
    EXPECT_TRUE(status.HasExactlyExclusionReasonsForTesting({
        CookieInclusionStatus::EXCLUDE_USER_PREFERENCES,
    }));
...
TEST(CookieInclusionStatusTest, ValidateExclusionAndWarningFromWire)
{
    ...
    EXPECT_TRUE(CookieInclusionStatus::ValidateExclusionAndWarningFromWire(exclusion_reasons, warning_reasons));

    exclusion_reasons = static_cast<uint32_t>(~0ul);
    warning_reasons = static_cast<uint32_t>(~0ul);
    EXPECT_FALSE(CookieInclusionStatus::ValidateExclusionAndWarningFromWire(exclusion_reasons, warning_reasons));
    EXPECT_FALSE(CookieInclusionStatus::ValidateExclusionAndWarningFromWire(exclusion_reasons, 0u));
    EXPECT_FALSE(CookieInclusionStatus::ValidateExclusionAndWarningFromWire(0u, warning_reasons));
    ...
    exclusion_reasons = (1u << (CookieInclusionStatus::NUM_EXCLUSION_REASONS - 1));
...
{
    CookieInclusionStatus status = CookieInclusionStatus::MakeFromReasonsForTesting({ CookieInclusionStatus::ExclusionReason::EXCLUDE_USER_PREFERENCES });
    ...
    status = CookieInclusionStatus::MakeFromReasonsForTesting({
        CookieInclusionStatus::ExclusionReason::EXCLUDE_THIRD_PARTY_PHASEOUT,
        CookieInclusionStatus::ExclusionReason::EXCLUDE_THIRD_PARTY_BLOCKED_WITHIN_FIRST_PARTY_SET,
    });
    EXPECT_TRUE(status.ExcludedByUserPreferencesOrTPCD());
