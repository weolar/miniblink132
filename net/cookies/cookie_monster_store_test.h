...
struct CookieStoreCommand {
    ...
    CookieStoreCommand(Type type, CookieMonster::PersistentCookieStore::LoadedCallback loaded_callback, const std::string& key);
...
public:
    ...
    MockPersistentCookieStore& operator=(const MockPersistentCookieStore&) = delete;
    ...
    void SetLoadExpectation(bool return_value, std::vector<std::unique_ptr<CanonicalCookie>> result);
    ...
    CookieMonster::PersistentCookieStore::LoadedCallback TakeCallbackAt(size_t i)
    {
...
class MockSimplePersistentCookieStore : public CookieMonster::PersistentCookieStore {
public:
...
private:
    typedef std::map<CanonicalCookie::UniqueCookieKey, CanonicalCookie> CanonicalCookieMap;
