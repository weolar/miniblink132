...
// true and false.
bool CookieChangeLessThan(const CookieChangeInfo& lhs, const CookieChangeInfo& rhs)
{
...
template <class CookieStoreTestTraits> class CookieStoreChangeTestBase : public CookieStoreTest<CookieStoreTestTraits> {
protected:
    ...
    bool FindAndDeleteCookie(CookieStore* cs, const std::string& domain, const std::string& name, const std::string& path)
    {
    ...
    ::testing::AssertionResult MatchesCause(CookieChangeCause expected_cause, CookieChangeCause actual_cause)
    {
        if (!CookieChangeCauseIsDeletion(expected_cause) || CookieStoreTestTraits::has_exact_change_cause) {
            if (expected_cause == actual_cause)
                return ::testing::AssertionSuccess();
            return ::testing::AssertionFailure() << "expected " << expected_cause << " got " << actual_cause;
        }
        if (CookieChangeCauseIsDeletion(actual_cause))
            return ::testing::AssertionSuccess();
        return ::testing::AssertionFailure() << "expected a deletion cause, got " << actual_cause;
    }

    bool IsExpectedAccessSemantics(net::CookieAccessSemantics expected_semantics, net::CookieAccessSemantics actual_semantics)
    {
    ...
    static void OnCookieChange(std::vector<CookieChangeInfo>* changes, const CookieChangeInfo& notification)
    {
        if (CookieStoreTestTraits::has_exact_change_ordering) {
            ...
            // will remain sorted.
            changes->insert(std::upper_bound(changes->begin(), changes->end(), notification, CookieChangeLessThan), notification);
        }
    }
};

template <class CookieStoreTestTraits> class CookieStoreChangeGlobalTest : public CookieStoreChangeTestBase<CookieStoreTestTraits> { };
TYPED_TEST_SUITE_P(CookieStoreChangeGlobalTest);

template <class CookieStoreTestTraits> class CookieStoreChangeUrlTest : public CookieStoreChangeTestBase<CookieStoreTestTraits> { };
TYPED_TEST_SUITE_P(CookieStoreChangeUrlTest);

template <class CookieStoreTestTraits> class CookieStoreChangeNamedTest : public CookieStoreChangeTestBase<CookieStoreTestTraits> { };
...
TYPED_TEST_P(CookieStoreChangeGlobalTest, NoCookie)
{
    ...
    std::unique_ptr<CookieChangeSubscription> subscription = cs->GetChangeDispatcher().AddCallbackForAllChanges(
        base::BindRepeating(&CookieStoreChangeTestBase<TypeParam>::OnCookieChange, base::Unretained(&cookie_changes)));
...
TYPED_TEST_P(CookieStoreChangeGlobalTest, InsertOne)
{
    ...
    EXPECT_TRUE(this->MatchesCause(CookieChangeCause::INSERTED, cookie_changes[0].cause));
    EXPECT_EQ(this->http_www_foo_.url().host(), cookie_changes[0].cookie.Domain());
...
TYPED_TEST_P(CookieStoreChangeGlobalTest, InsertMany)
{
    ...
    EXPECT_EQ(this->http_www_foo_.url().host(), cookie_changes[1].cookie.Domain());
    EXPECT_TRUE(this->MatchesCause(CookieChangeCause::INSERTED, cookie_changes[1].cause));
    EXPECT_EQ("C", cookie_changes[1].cookie.Name());
    EXPECT_EQ("D", cookie_changes[1].cookie.Value());

    ASSERT_LE(3u, cookie_changes.size());
    EXPECT_EQ(this->http_www_foo_.url().host(), cookie_changes[2].cookie.Domain());
    EXPECT_TRUE(this->MatchesCause(CookieChangeCause::INSERTED, cookie_changes[2].cause));
    EXPECT_EQ("E", cookie_changes[2].cookie.Name());
    EXPECT_EQ("F", cookie_changes[2].cookie.Value());

    ASSERT_LE(4u, cookie_changes.size());
    EXPECT_EQ(this->http_bar_com_.url().host(), cookie_changes[3].cookie.Domain());
    EXPECT_TRUE(this->MatchesCause(CookieChangeCause::INSERTED, cookie_changes[3].cause));
...
TYPED_TEST_P(CookieStoreChangeGlobalTest, DeleteOne)
{
    ...
    EXPECT_TRUE(this->FindAndDeleteCookie(cs, this->http_www_foo_.url().host(), "A"));
    this->DeliverChangeNotifications();

    ASSERT_EQ(1u, cookie_changes.size());
    EXPECT_EQ(this->http_www_foo_.url().host(), cookie_changes[0].cookie.Domain());
    EXPECT_TRUE(this->MatchesCause(CookieChangeCause::EXPLICIT, cookie_changes[0].cause));
    EXPECT_EQ("A", cookie_changes[0].cookie.Name());
    EXPECT_EQ("B", cookie_changes[0].cookie.Value());
}
...
TYPED_TEST_P(CookieStoreChangeGlobalTest, DeleteTwo)
{
    ...
    EXPECT_TRUE(this->FindAndDeleteCookie(cs, this->http_www_foo_.url().host(), "C"));
    EXPECT_TRUE(this->FindAndDeleteCookie(cs, this->http_bar_com_.url().host(), "G"));
    this->DeliverChangeNotifications();
    ...
    EXPECT_EQ(this->http_bar_com_.url().host(), cookie_changes[1].cookie.Domain());
    EXPECT_TRUE(this->MatchesCause(CookieChangeCause::EXPLICIT, cookie_changes[1].cause));
...
TYPED_TEST_P(CookieStoreChangeGlobalTest, Overwrite)
{
    ...
    ASSERT_LE(1u, cookie_changes.size());
    EXPECT_EQ(this->http_www_foo_.url().host(), cookie_changes[0].cookie.Domain());
    EXPECT_TRUE(this->MatchesCause(CookieChangeCause::OVERWRITE, cookie_changes[0].cause));
    EXPECT_EQ("A", cookie_changes[0].cookie.Name());
    EXPECT_EQ("B", cookie_changes[0].cookie.Value());
...
TYPED_TEST_P(CookieStoreChangeGlobalTest, OverwriteWithHttpOnly)
{
    ...
    EXPECT_TRUE(this->SetCookie(cs, this->http_www_foo_.url(), "A=B; path=/path1"));
    this->DeliverChangeNotifications();
    ASSERT_EQ(1u, cookie_changes.size());
    EXPECT_TRUE(this->MatchesCause(CookieChangeCause::INSERTED, cookie_changes[0].cause));
    EXPECT_EQ(this->http_www_foo_.url().host(), cookie_changes[0].cookie.Domain());
    ...
    EXPECT_TRUE(this->CreateAndSetCookie(cs, this->http_www_foo_.url(), "A=C; path=/path1; httponly", allow_httponly));
    this->DeliverChangeNotifications();
...
{
    if (!TypeParam::supports_global_cookie_tracking || !TypeParam::supports_multiple_tracking_callbacks)
        return;
    ...
    std::unique_ptr<CookieChangeSubscription> subscription1 = cs->GetChangeDispatcher().AddCallbackForAllChanges(
        base::BindRepeating(&CookieStoreChangeTestBase<TypeParam>::OnCookieChange, base::Unretained(&cookie_changes_1)));

    std::vector<CookieChangeInfo> cookie_changes_2;
    std::unique_ptr<CookieChangeSubscription> subscription2 = cs->GetChangeDispatcher().AddCallbackForAllChanges(
        base::BindRepeating(&CookieStoreChangeTestBase<TypeParam>::OnCookieChange, base::Unretained(&cookie_changes_2)));
    this->DeliverChangeNotifications();
...
TYPED_TEST_P(CookieStoreChangeGlobalTest, ChangeIncludesCookieAccessSemantics)
{
    ...
    access_delegate->SetExpectationForCookieDomain("domain1.test", CookieAccessSemantics::LEGACY);
    access_delegate->SetExpectationForCookieDomain("domain2.test", CookieAccessSemantics::NONLEGACY);
    access_delegate->SetExpectationForCookieDomain("domain3.test", CookieAccessSemantics::UNKNOWN);
    ...
    EXPECT_TRUE(this->IsExpectedAccessSemantics(CookieAccessSemantics::LEGACY, cookie_changes[0].access_result.access_semantics));
    EXPECT_EQ("domain2.test", cookie_changes[1].cookie.Domain());
    EXPECT_TRUE(this->IsExpectedAccessSemantics(CookieAccessSemantics::NONLEGACY, cookie_changes[1].access_result.access_semantics));
    EXPECT_EQ("domain3.test", cookie_changes[2].cookie.Domain());
    EXPECT_TRUE(this->IsExpectedAccessSemantics(CookieAccessSemantics::UNKNOWN, cookie_changes[2].access_result.access_semantics));
    EXPECT_EQ("domain4.test", cookie_changes[3].cookie.Domain());
    EXPECT_TRUE(this->IsExpectedAccessSemantics(CookieAccessSemantics::UNKNOWN, cookie_changes[3].access_result.access_semantics));
}

TYPED_TEST_P(CookieStoreChangeGlobalTest, PartitionedCookies)
{
    if (!TypeParam::supports_named_cookie_tracking || !TypeParam::supports_partitioned_cookies) {
        return;
    }
    ...
    std::unique_ptr<CookieChangeSubscription> global_subscription = cs->GetChangeDispatcher().AddCallbackForAllChanges(
        base::BindRepeating(&CookieStoreChangeTestBase<TypeParam>::OnCookieChange, base::Unretained(&all_cookie_changes)));
    // Set two cookies in two separate partitions, one with nonce.
    this->CreateAndSetCookie(cs, GURL("https://www.example2.com"), "__Host-a=1; Secure; Path=/; Partitioned", CookieOptions::MakeAllInclusive(),
        std::nullopt /* server_time */, std::nullopt /* system_time */, CookiePartitionKey::FromURLForTesting(GURL("https://www.foo.com")));
    this->CreateAndSetCookie(cs, GURL("https://www.example2.com"), "__Host-a=2; Secure; Path=/; Partitioned; Max-Age=7200", CookieOptions::MakeAllInclusive(),
        std::nullopt /* server_time */, std::nullopt /* system_time */,
        CookiePartitionKey::FromURLForTesting(GURL("https://www.bar.com"), CookiePartitionKey::AncestorChainBit::kCrossSite, base::UnguessableToken::Create()));
    this->DeliverChangeNotifications();
...
TYPED_TEST_P(CookieStoreChangeUrlTest, NoCookie)
{
    ...
    CookieStore* cs = this->GetCookieStore();
    std::vector<CookieChangeInfo> cookie_changes;
    std::unique_ptr<CookieChangeSubscription> subscription = cs->GetChangeDispatcher().AddCallbackForUrl(this->http_www_foo_.url(),
        std::nullopt /* cookie_partition_key */, base::BindRepeating(&CookieStoreChangeTestBase<TypeParam>::OnCookieChange, base::Unretained(&cookie_changes)));
    this->DeliverChangeNotifications();
    EXPECT_EQ(0u, cookie_changes.size());
}
...
TYPED_TEST_P(CookieStoreChangeUrlTest, InsertFiltering)
{
    ...
    CookieStore* cs = this->GetCookieStore();
    std::vector<CookieChangeInfo> cookie_changes;
    std::unique_ptr<CookieChangeSubscription> subscription = cs->GetChangeDispatcher().AddCallbackForUrl(this->www_foo_foo_.url(),
        std::nullopt /* cookie_partition_key */, base::BindRepeating(&CookieStoreChangeTestBase<TypeParam>::OnCookieChange, base::Unretained(&cookie_changes)));
    this->DeliverChangeNotifications();
    ASSERT_EQ(0u, cookie_changes.size());
    ...
    EXPECT_TRUE(this->SetCookie(cs, this->http_www_foo_.url(), "G=H; path=/foo/bar"));
    EXPECT_TRUE(this->SetCookie(cs, this->http_www_foo_.url(), "I=J; path=/foo"));
    EXPECT_TRUE(this->SetCookie(cs, this->http_www_foo_.url(), "K=L; domain=foo.com"));
    this->DeliverChangeNotifications();
...
TYPED_TEST_P(CookieStoreChangeUrlTest, DeleteOne)
{
    ...
    EXPECT_EQ("B", cookie_changes[0].cookie.Value());
    EXPECT_EQ(this->http_www_foo_.url().host(), cookie_changes[0].cookie.Domain());
    ASSERT_TRUE(this->MatchesCause(CookieChangeCause::EXPLICIT, cookie_changes[0].cause));
}
...
TYPED_TEST_P(CookieStoreChangeUrlTest, DeleteTwo)
{
    ...
    EXPECT_TRUE(this->FindAndDeleteCookie(cs, this->http_www_foo_.url().host(), "C"));
    EXPECT_TRUE(this->FindAndDeleteCookie(cs, this->http_www_foo_.url().host(), "G"));
    this->DeliverChangeNotifications();
...
TYPED_TEST_P(CookieStoreChangeUrlTest, DeleteFiltering)
{
    ...
    EXPECT_TRUE(this->FindAndDeleteCookie(cs, this->http_www_foo_.url().host(), "A"));
    EXPECT_TRUE(this->FindAndDeleteCookie(cs, this->http_bar_com_.url().host(), "C"));
    EXPECT_TRUE(this->FindAndDeleteCookie(cs, this->http_www_foo_.url().host(), "E"));
    EXPECT_TRUE(this->FindAndDeleteCookie(cs, this->http_www_foo_.url().host(), "G"));
    EXPECT_TRUE(this->FindAndDeleteCookie(cs, this->http_www_foo_.url().host(), "I"));
    ...
    EXPECT_EQ("/", cookie_changes[2].cookie.Path());
    EXPECT_EQ(".foo.com", cookie_changes[2].cookie.Domain());
    EXPECT_TRUE(this->MatchesCause(CookieChangeCause::EXPLICIT, cookie_changes[2].cause));
...
TYPED_TEST_P(CookieStoreChangeUrlTest, OverwriteFiltering)
{
    ...
    EXPECT_EQ(this->http_www_foo_.url().host(), cookie_changes[2].cookie.Domain());
    EXPECT_TRUE(this->MatchesCause(CookieChangeCause::OVERWRITE, cookie_changes[2].cause));
    ...
    EXPECT_EQ(this->http_www_foo_.url().host(), cookie_changes[3].cookie.Domain());
    EXPECT_TRUE(this->MatchesCause(CookieChangeCause::INSERTED, cookie_changes[3].cause));
    ...
    EXPECT_TRUE(this->MatchesCause(CookieChangeCause::OVERWRITE, cookie_changes[4].cause));
    ...
    EXPECT_TRUE(this->MatchesCause(CookieChangeCause::INSERTED, cookie_changes[5].cause));
...
TYPED_TEST_P(CookieStoreChangeUrlTest, OverwriteWithHttpOnly)
{
    ...
    EXPECT_TRUE(this->CreateAndSetCookie(cs, this->http_www_foo_.url(), "A=C; path=/foo; httponly", allow_httponly));
    this->DeliverChangeNotifications();
...
{
    if (!TypeParam::supports_url_cookie_tracking || !TypeParam::supports_multiple_tracking_callbacks)
        return;
    ...
    std::unique_ptr<CookieChangeSubscription> subscription1
        = cs->GetChangeDispatcher().AddCallbackForUrl(this->http_www_foo_.url(), std::nullopt /* cookie_partition_key */,
            base::BindRepeating(&CookieStoreChangeTestBase<TypeParam>::OnCookieChange, base::Unretained(&cookie_changes_1)));
...
TYPED_TEST_P(CookieStoreChangeUrlTest, DifferentSubscriptionsDisjoint)
{
    ...
    std::unique_ptr<CookieChangeSubscription> subscription1
        = cs->GetChangeDispatcher().AddCallbackForUrl(this->http_www_foo_.url(), std::nullopt /* cookie_partition_key */,
            base::BindRepeating(&CookieStoreChangeTestBase<TypeParam>::OnCookieChange, base::Unretained(&cookie_changes_1)));
    std::unique_ptr<CookieChangeSubscription> subscription2
        = cs->GetChangeDispatcher().AddCallbackForUrl(this->http_bar_com_.url(), std::nullopt /* cookie_partition_key */,
            base::BindRepeating(&CookieStoreChangeTestBase<TypeParam>::OnCookieChange, base::Unretained(&cookie_changes_2)));
    this->DeliverChangeNotifications();
    ASSERT_EQ(0u, cookie_changes_1.size());
    ASSERT_EQ(0u, cookie_changes_2.size());
    ...
    EXPECT_EQ("A", cookie_changes_1[0].cookie.Name());
    EXPECT_EQ("B", cookie_changes_1[0].cookie.Value());
    EXPECT_EQ(this->http_www_foo_.url().host(), cookie_changes_1[0].cookie.Domain());

    ASSERT_EQ(1u, cookie_changes_2.size());
    EXPECT_EQ("C", cookie_changes_2[0].cookie.Name());
    EXPECT_EQ("D", cookie_changes_2[0].cookie.Value());
    EXPECT_EQ(this->http_bar_com_.url().host(), cookie_changes_2[0].cookie.Domain());
}
...
TYPED_TEST_P(CookieStoreChangeUrlTest, DifferentSubscriptionsPaths)
{
    ...
    std::unique_ptr<CookieChangeSubscription> subscription1
        = cs->GetChangeDispatcher().AddCallbackForUrl(this->http_www_foo_.url(), std::nullopt /* cookie_partition_key */,
            base::BindRepeating(&CookieStoreChangeTestBase<TypeParam>::OnCookieChange, base::Unretained(&cookie_changes_1)));
    std::unique_ptr<CookieChangeSubscription> subscription2
        = cs->GetChangeDispatcher().AddCallbackForUrl(this->www_foo_foo_.url(), std::nullopt /* cookie_partition_key */,
            base::BindRepeating(&CookieStoreChangeTestBase<TypeParam>::OnCookieChange, base::Unretained(&cookie_changes_2)));
    this->DeliverChangeNotifications();
    ASSERT_EQ(0u, cookie_changes_1.size());
    ASSERT_EQ(0u, cookie_changes_2.size());
    ...
    EXPECT_EQ(this->http_www_foo_.url().host(), cookie_changes_2[0].cookie.Domain());
    ...
    EXPECT_EQ(this->http_www_foo_.url().host(), cookie_changes_2[1].cookie.Domain());
...
TYPED_TEST_P(CookieStoreChangeUrlTest, DifferentSubscriptionsFiltering)
{
    ...
    std::unique_ptr<CookieChangeSubscription> subscription3
        = cs->GetChangeDispatcher().AddCallbackForUrl(this->www_foo_foo_.url(), std::nullopt /* cookie_partition_key */,
            base::BindRepeating(&CookieStoreChangeTestBase<TypeParam>::OnCookieChange, base::Unretained(&cookie_changes_3)));
    this->DeliverChangeNotifications();
    ASSERT_EQ(0u, cookie_changes_1.size());
    ASSERT_EQ(0u, cookie_changes_2.size());
    ...
    EXPECT_EQ(this->http_www_foo_.url().host(), cookie_changes_3[0].cookie.Domain());
    ...
    EXPECT_EQ(this->http_www_foo_.url().host(), cookie_changes_3[1].cookie.Domain());
...
TYPED_TEST_P(CookieStoreChangeUrlTest, ChangeIncludesCookieAccessSemantics)
{
    ...
    std::vector<CookieChangeInfo> cookie_changes;
    std::unique_ptr<CookieChangeSubscription> subscription = cs->GetChangeDispatcher().AddCallbackForUrl(GURL("http://domain1.test"),
        std::nullopt /* cookie_partition_key */, base::BindRepeating(&CookieStoreChangeTestBase<TypeParam>::OnCookieChange, base::Unretained(&cookie_changes)));
...
{
    if (!TypeParam::supports_url_cookie_tracking || !TypeParam::supports_partitioned_cookies)
        return;

    CookieStore* cs = this->GetCookieStore();
    std::vector<CookieChangeInfo> cookie_changes;
    std::unique_ptr<CookieChangeSubscription> subscription
        = cs->GetChangeDispatcher().AddCallbackForUrl(GURL("https://www.example.com/"), CookiePartitionKey::FromURLForTesting(GURL("https://www.foo.com")),
            base::BindRepeating(&CookieStoreChangeTestBase<TypeParam>::OnCookieChange, base::Unretained(&cookie_changes)));

    // Unpartitioned cookie
    this->CreateAndSetCookie(cs, GURL("https://www.example.com/"), "__Host-a=1; Secure; Path=/", CookieOptions::MakeAllInclusive());
    // Partitioned cookie with the same partition key
    this->CreateAndSetCookie(cs, GURL("https://www.example.com/"), "__Host-b=2; Secure; Path=/; Partitioned", CookieOptions::MakeAllInclusive(),
        std::nullopt /* server_time */, std::nullopt /* system_time */, CookiePartitionKey::FromURLForTesting(GURL("https://sub.foo.com")));
    // Partitioned cookie with a different partition key
    this->CreateAndSetCookie(cs, GURL("https://www.example.com"), "__Host-c=3; Secure; Path=/; Partitioned", CookieOptions::MakeAllInclusive(),
        std::nullopt /* server_time */, std::nullopt /* system_time */, CookiePartitionKey::FromURLForTesting(GURL("https://www.bar.com")));
    this->DeliverChangeNotifications();
    ...
    EXPECT_EQ(CookiePartitionKey::FromURLForTesting(GURL("https://www.foo.com")), cookie_changes[1].cookie.PartitionKey().value());
    ...
    std::unique_ptr<CookieChangeSubscription> other_subscription
        = cs->GetChangeDispatcher().AddCallbackForUrl(GURL("https://www.example.com/"), std::nullopt /* cookie_partition_key */,
            base::BindRepeating(&CookieStoreChangeTestBase<TypeParam>::OnCookieChange, base::Unretained(&other_cookie_changes)));
    // Update Max-Age: None -> 7200
    this->CreateAndSetCookie(cs, GURL("https://www.example.com"), "__Host-b=2; Secure; Path=/; Partitioned; Max-Age=7200", CookieOptions::MakeAllInclusive(),
        std::nullopt /* server_time */, std::nullopt /* system_time */, CookiePartitionKey::FromURLForTesting(GURL("https://www.foo.com")));
    this->DeliverChangeNotifications();
...
TYPED_TEST_P(CookieStoreChangeUrlTest, PartitionedCookies_WithNonce)
{
    ...
    std::vector<CookieChangeInfo> cookie_changes;
    std::unique_ptr<CookieChangeSubscription> subscription = cs->GetChangeDispatcher().AddCallbackForUrl(GURL("https://www.example.com"),
        CookiePartitionKey::FromURLForTesting(GURL("https://www.foo.com"), CookiePartitionKey::AncestorChainBit::kCrossSite, nonce),
        base::BindRepeating(&CookieStoreChangeTestBase<TypeParam>::OnCookieChange, base::Unretained(&cookie_changes)));
    ...
    this->CreateAndSetCookie(cs, GURL("https://www.example.com"), "__Host-a=2; Secure; Path=/; Partitioned", CookieOptions::MakeAllInclusive(),
        std::nullopt /* server_time */, std::nullopt /* system_time */, CookiePartitionKey::FromURLForTesting(GURL("https://www.foo.com")));
    this->DeliverChangeNotifications();
    ASSERT_EQ(0u, cookie_changes.size());

    // Set partitioned cookie with nonce.
    this->CreateAndSetCookie(cs, GURL("https://www.example.com"), "__Host-a=3; Secure; Path=/; Partitioned", CookieOptions::MakeAllInclusive(),
        std::nullopt /* server_time */, std::nullopt /* system_time */,
        CookiePartitionKey::FromURLForTesting(GURL("https://www.foo.com"), CookiePartitionKey::AncestorChainBit::kCrossSite, nonce));
    this->DeliverChangeNotifications();
    ASSERT_EQ(1u, cookie_changes.size());
...
TYPED_TEST_P(CookieStoreChangeNamedTest, NoCookie)
{
    ...
    CookieStore* cs = this->GetCookieStore();
    std::vector<CookieChangeInfo> cookie_changes;
    std::unique_ptr<CookieChangeSubscription> subscription = cs->GetChangeDispatcher().AddCallbackForCookie(this->http_www_foo_.url(), "abc",
        std::nullopt /* cookie_partition_key */, base::BindRepeating(&CookieStoreChangeTestBase<TypeParam>::OnCookieChange, base::Unretained(&cookie_changes)));
    this->DeliverChangeNotifications();
    EXPECT_EQ(0u, cookie_changes.size());
}
...
TYPED_TEST_P(CookieStoreChangeNamedTest, InsertTwo)
{
    ...
    CookieStore* cs = this->GetCookieStore();
    std::vector<CookieChangeInfo> cookie_changes;
    std::unique_ptr<CookieChangeSubscription> subscription = cs->GetChangeDispatcher().AddCallbackForCookie(this->www_foo_foo_.url(), "abc",
        std::nullopt /* cookie_partition_key */, base::BindRepeating(&CookieStoreChangeTestBase<TypeParam>::OnCookieChange, base::Unretained(&cookie_changes)));
    this->DeliverChangeNotifications();
    ASSERT_EQ(0u, cookie_changes.size());

    EXPECT_TRUE(this->SetCookie(cs, this->http_www_foo_.url(), "abc=def"));
    EXPECT_TRUE(this->SetCookie(cs, this->http_www_foo_.url(), "abc=hij; path=/foo"));
    this->DeliverChangeNotifications();
...
TYPED_TEST_P(CookieStoreChangeNamedTest, InsertFiltering)
{
    ...
    EXPECT_TRUE(this->SetCookie(cs, this->http_www_foo_.url(), "abc=def; path=/"));
    EXPECT_TRUE(this->SetCookie(cs, this->http_bar_com_.url(), "abc=ghi; path=/"));
    EXPECT_TRUE(this->SetCookie(cs, this->http_www_foo_.url(), "abc=jkl; path=/bar"));
    EXPECT_TRUE(this->SetCookie(cs, this->http_www_foo_.url(), "abc=mno; path=/foo/bar"));
    EXPECT_TRUE(this->SetCookie(cs, this->http_www_foo_.url(), "xyz=zyx"));
    EXPECT_TRUE(this->SetCookie(cs, this->http_www_foo_.url(), "abc=pqr; path=/foo"));
    EXPECT_TRUE(this->SetCookie(cs, this->http_www_foo_.url(), "abc=stu; domain=foo.com"));
    this->DeliverChangeNotifications();
...
TYPED_TEST_P(CookieStoreChangeNamedTest, DeleteOne)
{
    ...
    EXPECT_TRUE(this->FindAndDeleteCookie(cs, this->http_www_foo_.url().host(), "abc"));
    this->DeliverChangeNotifications();
...
TYPED_TEST_P(CookieStoreChangeNamedTest, DeleteTwo)
{
    ...
    EXPECT_TRUE(this->FindAndDeleteCookie(cs, this->http_www_foo_.url().host(), "abc", "/"));
    EXPECT_TRUE(this->FindAndDeleteCookie(cs, this->http_www_foo_.url().host(), "abc", "/foo"));
    this->DeliverChangeNotifications();
...
TYPED_TEST_P(CookieStoreChangeNamedTest, DeleteFiltering)
{
    ...
    std::unique_ptr<CookieChangeSubscription> subscription = cs->GetChangeDispatcher().AddCallbackForCookie(this->www_foo_foo_.url(), "abc",
        std::nullopt /* cookie_partition_key */, base::BindRepeating(&CookieStoreChangeTestBase<TypeParam>::OnCookieChange, base::Unretained(&cookie_changes)));
    EXPECT_TRUE(this->SetCookie(cs, this->http_www_foo_.url(), "xyz=zyx; path=/"));
    EXPECT_TRUE(this->SetCookie(cs, this->http_bar_com_.url(), "abc=def; path=/"));
    EXPECT_TRUE(this->SetCookie(cs, this->http_www_foo_.url(), "abc=hij; path=/foo/bar"));
    EXPECT_TRUE(this->SetCookie(cs, this->http_www_foo_.url(), "abc=mno; path=/foo"));
    EXPECT_TRUE(this->SetCookie(cs, this->http_www_foo_.url(), "abc=pqr; path=/"));
    EXPECT_TRUE(this->SetCookie(cs, this->http_www_foo_.url(), "abc=stu; domain=foo.com"));
    this->DeliverChangeNotifications();
    EXPECT_EQ(3u, cookie_changes.size());
    cookie_changes.clear();

    EXPECT_TRUE(this->FindAndDeleteCookie(cs, this->http_www_foo_.url().host(), "xyz"));
    EXPECT_TRUE(this->FindAndDeleteCookie(cs, this->http_bar_com_.url().host(), "abc"));
    EXPECT_TRUE(this->FindAndDeleteCookie(cs, this->http_www_foo_.url().host(), "abc", "/foo/bar"));
    EXPECT_TRUE(this->FindAndDeleteCookie(cs, this->http_www_foo_.url().host(), "abc", "/foo"));
    EXPECT_TRUE(this->FindAndDeleteCookie(cs, this->http_www_foo_.url().host(), "abc", "/"));
...
TYPED_TEST_P(CookieStoreChangeNamedTest, OverwriteFiltering)
{
    ...
    EXPECT_TRUE(this->SetCookie(cs, this->http_www_foo_.url(), "xyz=zyx1; path=/"));
    EXPECT_TRUE(this->SetCookie(cs, this->http_bar_com_.url(), "abc=def1; path=/"));
    EXPECT_TRUE(this->SetCookie(cs, this->http_www_foo_.url(), "abc=hij1; path=/foo/bar"));
    EXPECT_TRUE(this->SetCookie(cs, this->http_www_foo_.url(), "abc=mno1; path=/foo"));
    EXPECT_TRUE(this->SetCookie(cs, this->http_www_foo_.url(), "abc=pqr1; path=/"));
    EXPECT_TRUE(this->SetCookie(cs, this->http_www_foo_.url(), "abc=stu1; domain=foo.com"));
    this->DeliverChangeNotifications();
    EXPECT_EQ(3u, cookie_changes.size());
    cookie_changes.clear();
    ...
    // Replacing an existing cookie is actually a two-phase delete + set
    // operation, so we get two notifications per overwrite.
    EXPECT_TRUE(this->SetCookie(cs, this->http_www_foo_.url(), "xyz=zyx2; path=/"));
    EXPECT_TRUE(this->SetCookie(cs, this->http_bar_com_.url(), "abc=def2; path=/"));
    EXPECT_TRUE(this->SetCookie(cs, this->http_www_foo_.url(), "abc=hij2; path=/foo/bar"));
    EXPECT_TRUE(this->SetCookie(cs, this->http_www_foo_.url(), "abc=mno2; path=/foo"));
    EXPECT_TRUE(this->SetCookie(cs, this->http_www_foo_.url(), "abc=pqr2; path=/"));
    EXPECT_TRUE(this->SetCookie(cs, this->http_www_foo_.url(), "abc=stu2; domain=foo.com"));
    this->DeliverChangeNotifications();
...
TYPED_TEST_P(CookieStoreChangeNamedTest, OverwriteWithHttpOnly)
{
    ...
    EXPECT_TRUE(this->SetCookie(cs, this->http_www_foo_.url(), "abc=def; path=/foo"));
    this->DeliverChangeNotifications();
    ASSERT_EQ(1u, cookie_changes.size());
    EXPECT_TRUE(this->MatchesCause(CookieChangeCause::INSERTED, cookie_changes[0].cause));
    EXPECT_EQ(this->http_www_foo_.url().host(), cookie_changes[0].cookie.Domain());
    ...
    EXPECT_TRUE(this->CreateAndSetCookie(cs, this->http_www_foo_.url(), "abc=hij; path=/foo; httponly", allow_httponly));
    this->DeliverChangeNotifications();
...
TYPED_TEST_P(CookieStoreChangeNamedTest, Deregister)
{
    ...
    // Insert a second cookie and make sure it's not visible.
    EXPECT_TRUE(this->SetCookie(cs, this->http_www_foo_.url(), "abc=hij; path=/"));
    this->DeliverChangeNotifications();
...
{
    if (!TypeParam::supports_named_cookie_tracking || !TypeParam::supports_multiple_tracking_callbacks)
        return;
    ...
    std::vector<CookieChangeInfo> cookie_changes_1, cookie_changes_2;
    std::unique_ptr<CookieChangeSubscription> subscription1
        = cs->GetChangeDispatcher().AddCallbackForCookie(this->www_foo_foo_.url(), "abc", std::nullopt /* cookie_partition_key */,
            base::BindRepeating(&CookieStoreChangeTestBase<TypeParam>::OnCookieChange, base::Unretained(&cookie_changes_1)));
    std::unique_ptr<CookieChangeSubscription> subscription2
        = cs->GetChangeDispatcher().AddCallbackForCookie(this->www_foo_foo_.url(), "abc", std::nullopt /* cookie_partition_key */,
            base::BindRepeating(&CookieStoreChangeTestBase<TypeParam>::OnCookieChange, base::Unretained(&cookie_changes_2)));
...
TYPED_TEST_P(CookieStoreChangeNamedTest, DifferentSubscriptionsDisjoint)
{
    ...
    std::vector<CookieChangeInfo> cookie_changes_1, cookie_changes_2;
    std::unique_ptr<CookieChangeSubscription> subscription1
        = cs->GetChangeDispatcher().AddCallbackForCookie(this->http_www_foo_.url(), "abc", std::nullopt /* cookie_partition_key */,
            base::BindRepeating(&CookieStoreChangeTestBase<TypeParam>::OnCookieChange, base::Unretained(&cookie_changes_1)));
    std::unique_ptr<CookieChangeSubscription> subscription2
        = cs->GetChangeDispatcher().AddCallbackForCookie(this->http_bar_com_.url(), "ghi", std::nullopt /* cookie_partition_key */,
            base::BindRepeating(&CookieStoreChangeTestBase<TypeParam>::OnCookieChange, base::Unretained(&cookie_changes_2)));
    this->DeliverChangeNotifications();
    ASSERT_EQ(0u, cookie_changes_1.size());
    ASSERT_EQ(0u, cookie_changes_2.size());
...
TYPED_TEST_P(CookieStoreChangeNamedTest, DifferentSubscriptionsDomains)
{
    ...
    std::unique_ptr<CookieChangeSubscription> subscription1
        = cs->GetChangeDispatcher().AddCallbackForCookie(this->http_www_foo_.url(), "abc", std::nullopt /* cookie_partition_key */,
            base::BindRepeating(&CookieStoreChangeTestBase<TypeParam>::OnCookieChange, base::Unretained(&cookie_changes_1)));
    std::unique_ptr<CookieChangeSubscription> subscription2
        = cs->GetChangeDispatcher().AddCallbackForCookie(this->http_bar_com_.url(), "abc", std::nullopt /* cookie_partition_key */,
            base::BindRepeating(&CookieStoreChangeTestBase<TypeParam>::OnCookieChange, base::Unretained(&cookie_changes_2)));
    this->DeliverChangeNotifications();
    ASSERT_EQ(0u, cookie_changes_1.size());
    ASSERT_EQ(0u, cookie_changes_2.size());
...
TYPED_TEST_P(CookieStoreChangeNamedTest, DifferentSubscriptionsNames)
{
    ...
    std::unique_ptr<CookieChangeSubscription> subscription1
        = cs->GetChangeDispatcher().AddCallbackForCookie(this->http_www_foo_.url(), "abc", std::nullopt /* cookie_partition_key */,
            base::BindRepeating(&CookieStoreChangeTestBase<TypeParam>::OnCookieChange, base::Unretained(&cookie_changes_1)));
    std::unique_ptr<CookieChangeSubscription> subscription2
        = cs->GetChangeDispatcher().AddCallbackForCookie(this->http_www_foo_.url(), "ghi", std::nullopt /* cookie_partition_key */,
            base::BindRepeating(&CookieStoreChangeTestBase<TypeParam>::OnCookieChange, base::Unretained(&cookie_changes_2)));
    this->DeliverChangeNotifications();
    ASSERT_EQ(0u, cookie_changes_1.size());
    ASSERT_EQ(0u, cookie_changes_2.size());
...
TYPED_TEST_P(CookieStoreChangeNamedTest, DifferentSubscriptionsPaths)
{
    ...
    EXPECT_TRUE(this->SetCookie(cs, this->http_www_foo_.url(), "abc=ghi; path=/foo"));
    this->DeliverChangeNotifications();
...
TYPED_TEST_P(CookieStoreChangeNamedTest, DifferentSubscriptionsFiltering)
{
    ...
    std::unique_ptr<CookieChangeSubscription> subscription1
        = cs->GetChangeDispatcher().AddCallbackForCookie(this->http_www_foo_.url(), "abc", std::nullopt /* cookie_partition_key */,
            base::BindRepeating(&CookieStoreChangeTestBase<TypeParam>::OnCookieChange, base::Unretained(&cookie_changes_1)));
    std::unique_ptr<CookieChangeSubscription> subscription2
        = cs->GetChangeDispatcher().AddCallbackForCookie(this->http_www_foo_.url(), "hij", std::nullopt /* cookie_partition_key */,
            base::BindRepeating(&CookieStoreChangeTestBase<TypeParam>::OnCookieChange, base::Unretained(&cookie_changes_2)));
    std::unique_ptr<CookieChangeSubscription> subscription3
        = cs->GetChangeDispatcher().AddCallbackForCookie(this->http_bar_com_.url(), "abc", std::nullopt /* cookie_partition_key */,
            base::BindRepeating(&CookieStoreChangeTestBase<TypeParam>::OnCookieChange, base::Unretained(&cookie_changes_3)));
    std::unique_ptr<CookieChangeSubscription> subscription4
        = cs->GetChangeDispatcher().AddCallbackForCookie(this->www_foo_foo_.url(), "abc", std::nullopt /* cookie_partition_key */,
            base::BindRepeating(&CookieStoreChangeTestBase<TypeParam>::OnCookieChange, base::Unretained(&cookie_changes_4)));
    this->DeliverChangeNotifications();
    ASSERT_EQ(0u, cookie_changes_1.size());
    ASSERT_EQ(0u, cookie_changes_2.size());
    EXPECT_EQ(0u, cookie_changes_3.size());
    ...
    EXPECT_TRUE(this->SetCookie(cs, this->http_www_foo_.url(), "abc=vwx; path=/foo"));
    this->DeliverChangeNotifications();
    ...
    EXPECT_EQ(this->http_bar_com_.url().host(), cookie_changes_3[0].cookie.Domain());
    EXPECT_EQ(1u, cookie_changes_3.size());
    ...
    EXPECT_EQ(this->http_www_foo_.url().host(), cookie_changes_4[0].cookie.Domain());
    ...
    EXPECT_EQ(this->http_www_foo_.url().host(), cookie_changes_4[1].cookie.Domain());
...
TYPED_TEST_P(CookieStoreChangeNamedTest, SubscriptionOutlivesStore)
{
    ...
    std::vector<CookieChangeInfo> cookie_changes;
    std::unique_ptr<CookieChangeSubscription> subscription
        = this->GetCookieStore()->GetChangeDispatcher().AddCallbackForCookie(this->http_www_foo_.url(), "abc", std::nullopt /* cookie_partition_key */,
            base::BindRepeating(&CookieStoreChangeTestBase<TypeParam>::OnCookieChange, base::Unretained(&cookie_changes)));
...
TYPED_TEST_P(CookieStoreChangeNamedTest, ChangeIncludesCookieAccessSemantics)
{
    ...
    std::vector<CookieChangeInfo> cookie_changes;
    std::unique_ptr<CookieChangeSubscription> subscription = cs->GetChangeDispatcher().AddCallbackForCookie(GURL("http://domain1.test"), "cookie",
        std::nullopt /* cookie_partition_key */, base::BindRepeating(&CookieStoreChangeTestBase<TypeParam>::OnCookieChange, base::Unretained(&cookie_changes)));
...
TYPED_TEST_P(CookieStoreChangeNamedTest, PartitionedCookies)
{
    ...
    CookieStore* cs = this->GetCookieStore();
    std::vector<CookieChangeInfo> cookie_changes;
    std::unique_ptr<CookieChangeSubscription> subscription = cs->GetChangeDispatcher().AddCallbackForCookie(GURL("https://www.example.com"), "__Host-a",
    ...
    std::vector<CookieChangeInfo> other_cookie_changes;
    std::unique_ptr<CookieChangeSubscription> other_subscription
        = cs->GetChangeDispatcher().AddCallbackForCookie(GURL("https://www.example.com"), "__Host-a", std::nullopt /* cookie_partition_key */,
            base::BindRepeating(&CookieStoreChangeTestBase<TypeParam>::OnCookieChange, base::Unretained(&other_cookie_changes)));
    // Update Max-Age: None -> 7200
    this->CreateAndSetCookie(cs, GURL("https://www.example.com"), "__Host-a=2; Secure; Path=/; Partitioned; Max-Age=7200", CookieOptions::MakeAllInclusive(),
        std::nullopt /* server_time */, std::nullopt /* system_time */, CookiePartitionKey::FromURLForTesting(GURL("https://www.foo.com")));
    this->DeliverChangeNotifications();
    ASSERT_EQ(0u, other_cookie_changes.size());
    // Check that the other listener was invoked.
...
REGISTER_TYPED_TEST_SUITE_P(CookieStoreChangeGlobalTest, NoCookie, InitialCookie, InsertOne, InsertMany, DeleteOne, DeleteTwo, Overwrite, OverwriteWithHttpOnly,
    Deregister, DeregisterMultiple, DispatchRace, DeregisterRace, DeregisterRaceMultiple, MultipleSubscriptions, ChangeIncludesCookieAccessSemantics,
    PartitionedCookies);

REGISTER_TYPED_TEST_SUITE_P(CookieStoreChangeUrlTest, NoCookie, InitialCookie, InsertOne, InsertMany, InsertFiltering, DeleteOne, DeleteTwo, DeleteFiltering,
    Overwrite, OverwriteFiltering, OverwriteWithHttpOnly, Deregister, DeregisterMultiple, DispatchRace, DeregisterRace, DeregisterRaceMultiple,
    DifferentSubscriptionsDisjoint, DifferentSubscriptionsDomains, DifferentSubscriptionsPaths, DifferentSubscriptionsFiltering, MultipleSubscriptions,
    ChangeIncludesCookieAccessSemantics, PartitionedCookies, PartitionedCookies_WithNonce);

REGISTER_TYPED_TEST_SUITE_P(CookieStoreChangeNamedTest, NoCookie, InitialCookie, InsertOne, InsertTwo, InsertFiltering, DeleteOne, DeleteTwo, DeleteFiltering,
    Overwrite, OverwriteFiltering, OverwriteWithHttpOnly, Deregister, DeregisterMultiple, DispatchRace, DeregisterRace, DeregisterRaceMultiple,
    DifferentSubscriptionsDisjoint, DifferentSubscriptionsDomains, DifferentSubscriptionsNames, DifferentSubscriptionsPaths, DifferentSubscriptionsFiltering,
    MultipleSubscriptions, SubscriptionOutlivesStore, ChangeIncludesCookieAccessSemantics, PartitionedCookies, PartitionedCookies_WithNonce);
