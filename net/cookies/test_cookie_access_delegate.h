...
public:
    ...
    std::optional<std::pair<FirstPartySetMetadata, FirstPartySetsCacheFilter::MatchInfo>> ComputeFirstPartySetMetadataMaybeAsync(const SchemefulSite& site,
        const SchemefulSite* top_frame_site, base::OnceCallback<void(FirstPartySetMetadata, FirstPartySetsCacheFilter::MatchInfo)> callback) const override;
    std::optional<base::flat_map<SchemefulSite, FirstPartySetEntry>> FindFirstPartySetEntries(
        const base::flat_set<SchemefulSite>& sites, base::OnceCallback<void(base::flat_map<SchemefulSite, FirstPartySetEntry>)> callback) const override;
    ...
    void SetExpectationForCookieDomain(const std::string& cookie_domain, CookieAccessSemantics access_semantics);
    ...
    void SetIgnoreSameSiteRestrictionsScheme(const std::string& site_for_cookies_scheme, bool require_secure_origin);
    ...
    void SetFirstPartySets(const base::flat_map<SchemefulSite, FirstPartySetEntry>& sets);
...
private:
    ...
    std::optional<FirstPartySetEntry> FindFirstPartySetEntry(const SchemefulSite& site) const;
    ...
    template <class T> std::optional<T> RunMaybeAsync(T result, base::OnceCallback<void(T)> callback) const;
    ...
    SchemefulSite trustworthy_site_ = SchemefulSite(GURL("http://trustworthysitefortestdelegate.example"));
};
