...
SiteForCookies& SiteForCookies::operator=(const SiteForCookies& other) = default;
SiteForCookies& SiteForCookies::operator=(SiteForCookies&& site_for_cookies) = default;

// static
bool SiteForCookies::FromWire(const SchemefulSite& site, bool schemefully_same, SiteForCookies* out)
{
...
std::string SiteForCookies::ToDebugString() const
{
    ...
    return base::StrCat({ "SiteForCookies: {site=", site_.Serialize(), "; schemefully_same=", same_scheme_string, "}" });
}

bool SiteForCookies::IsFirstParty(const GURL& url) const
{
    return IsFirstPartyWithSchemefulMode(url, cookie_util::IsSchemefulSameSiteEnabled());
}

bool SiteForCookies::IsFirstPartyWithSchemefulMode(const GURL& url, bool compute_schemefully) const
{
...
bool SiteForCookies::IsEquivalent(const SiteForCookies& other) const
{
    ...
    if (cookie_util::IsSchemefulSameSiteEnabled() || !site_.has_registrable_domain_or_host()) {
...
bool SiteForCookies::CompareWithFrameTreeSiteAndRevise(const SchemefulSite& other)
{
    ...
    bool nullify = site_.has_registrable_domain_or_host() ? !site_.SchemelesslyEqual(other) : site_ != other;
...
bool SiteForCookies::CompareWithFrameTreeOriginAndRevise(const url::Origin& other)
{
...
void SiteForCookies::MarkIfCrossScheme(const SchemefulSite& other)
{
    ...
    if (scheme() == other.site_as_origin_.scheme() || (scheme() == url::kHttpsScheme && other.site_as_origin_.scheme() == url::kWssScheme)
        || (scheme() == url::kHttpScheme && other.site_as_origin_.scheme() == url::kWsScheme)) {
        return;
    }
