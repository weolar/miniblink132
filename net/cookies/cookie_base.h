...
public:
    ...
    using StrictlyUniqueCookieKey = std::tuple<std::optional<CookiePartitionKey>,
        /*name=*/std::string,
        /*domain=*/std::string,
        /*path=*/std::string, CookieSourceScheme,
        /*source_port=*/int>;
    ...
    using UniqueCookieKey = std::tuple<std::optional<CookiePartitionKey>,
        /*name=*/std::string,
        /*domain=*/std::string,
        /*path=*/std::string, std::optional<CookieSourceScheme>,
    ...
    // not!
    CookieAccessResult IncludeForRequestURL(const GURL& url, const CookieOptions& options, const CookieAccessParams& params) const;
    ...
    CookieAccessResult IsSetPermittedInContext(const GURL& source_url, const CookieOptions& options, const CookieAccessParams& params,
        const std::vector<std::string>& cookieable_schemes, const std::optional<CookieAccessResult>& cookie_access_result = std::nullopt) const;
    ...
    const base::Time& CreationDate() const
    {
        return creation_date_;
    }
    bool SecureAttribute() const
    {
        return secure_;
    }
    ...
    {
        return partition_key_.has_value();
    }
    ...
    CookieSourceScheme SourceScheme() const
    {
    ...
    bool IsHostCookie() const
    {
        return !IsDomainCookie();
    }
    ...
    {
        return std::make_tuple(partition_key_, name_, domain_, path_, source_scheme_, source_port_);
    }
    ...
    void SetCreationDate(const base::Time& date)
    {
        creation_date_ = date;
    }
...
protected:
    ...
    CookieBase(std::string name, std::string domain, std::string path, base::Time creation, bool secure, bool httponly, CookieSameSite same_site,
        std::optional<CookiePartitionKey> partition_key, CookieSourceScheme source_scheme = CookieSourceScheme::kUnset,
    ...
    CookieEffectiveSameSite GetEffectiveSameSite(CookieAccessSemantics access_semantics) const;
...
private:
    ...
    virtual void PostIncludeForRequestURL(const CookieAccessResult& access_result, const CookieOptions& options_used,
        CookieOptions::SameSiteCookieContext::ContextType cookie_inclusion_context_used) const
    {
    }
    virtual void PostIsSetPermittedInContext(const CookieAccessResult& access_result, const CookieOptions& options_used) const
    {
    }
