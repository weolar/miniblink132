...
enum CookieType { COOKIE_TYPE_SAME_SITE = 0, COOKIE_TYPE_HTTPONLY, COOKIE_TYPE_SECURE, COOKIE_TYPE_PERSISTENT, COOKIE_TYPE_LAST_ENTRY };

void MaybeRunDeleteCallback(base::WeakPtr<net::CookieMonster> cookie_monster, base::OnceClosure callback)
{
    if (cookie_monster && callback)
        std::move(callback).Run();
}

template <typename CB, typename... R> void MaybeRunCookieCallback(base::OnceCallback<CB> callback, R&&... result)
{
    if (callback) {
...
// GetCookie...().
bool IncludeUnpartitionedCookies(const net::CookiePartitionKeyCollection& list)
{
...
size_t NumBytesInCookieMapForKey(const net::CookieMonster::CookieMap& cookie_map, const std::string& key)
{
    size_t result = 0;
...
size_t NumBytesInCookieItVector(const net::CookieMonster::CookieItVector& cookie_its)
{
    size_t result = 0;
...
void LogStoredCookieToUMA(const net::CanonicalCookie& cc, const net::CookieAccessResult& access_result)
{
    // Cookie.Type2 collects a bitvector of important cookie attributes.
    int32_t type_sample = !cc.IsEffectivelySameSiteNone(access_result.access_semantics) ? 1 << COOKIE_TYPE_SAME_SITE : 0;
    ...
    UMA_HISTOGRAM_EXACT_LINEAR("Cookie.Type2", type_sample, (1 << COOKIE_TYPE_LAST_ENTRY));
...
struct OrderByCreationTimeDesc {
    bool operator()(const CookieMonster::CookieMap::iterator& a, const CookieMonster::CookieMap::iterator& b) const
    {
        return a->second->CreationDate() > b->second->CreationDate();
    }
};

bool LRACookieSorter(const CookieMonster::CookieMap::iterator& it1, const CookieMonster::CookieMap::iterator& it2)
{
...
void SortLeastRecentlyAccessed(CookieMonster::CookieItVector::iterator it_begin, CookieMonster::CookieItVector::iterator it_end, size_t num_sort)
{
...
void SplitCookieVectorIntoSecureAndNonSecure(
    const CookieMonster::CookieItVector& cookie_its, CookieMonster::CookieItVector* secure_cookie_its, CookieMonster::CookieItVector* non_secure_cookie_its)
{
...
bool LowerBoundAccessDateComparator(const CookieMonster::CookieMap::iterator it, const Time& access_date)
{
...
CookieMonster::CookieItVector::iterator LowerBoundAccessDate(
    const CookieMonster::CookieItVector::iterator its_begin, const CookieMonster::CookieItVector::iterator its_end, const Time& access_date)
{
    return std::lower_bound(its_begin, its_end, access_date, LowerBoundAccessDateComparator);
}
...
bool IsCookieEligibleForEviction(CookiePriority current_priority_level, bool protect_secure_cookies, const CanonicalCookie* cookie)
{
...
size_t CountCookiesForPossibleDeletion(CookiePriority priority, const CookieMonster::CookieItVector* cookies, bool protect_secure_cookies)
{
...
size_t CountCookiesAndGenerateListsForPossibleDeletion(
    CookiePriority priority, DeletionCookieLists& could_be_deleted, const CookieMonster::CookieItList* cookies, bool generate_for_secure)
{
...
// persistent).
void HistogramExpirationDuration(const CanonicalCookie& cookie, base::Time creation_time)
{
    if (!cookie.IsPersistent())
        return;

    int expiration_duration_minutes = (cookie.ExpiryDate() - creation_time).InMinutes();
    if (cookie.SecureAttribute()) {
        UMA_HISTOGRAM_CUSTOM_COUNTS("Cookie.ExpirationDurationMinutesSecure", expiration_duration_minutes, 1, kMinutesInTenYears, 50);
    } else {
        UMA_HISTOGRAM_CUSTOM_COUNTS("Cookie.ExpirationDurationMinutesNonSecure", expiration_duration_minutes, 1, kMinutesInTenYears, 50);
    }
    ...
    if (expiration_duration_days > 400) {
        UMA_HISTOGRAM_CUSTOM_COUNTS("Cookie.ExpirationDuration400DaysGT", expiration_duration_days, 401, kDaysInTenYears, 100);
    } else {
        UMA_HISTOGRAM_CUSTOM_COUNTS("Cookie.ExpirationDuration400DaysLTE", expiration_duration_days, 1, 400, 50);
    }
}

} // namespace

CookieMonster::CookieMonster(scoped_refptr<PersistentCookieStore> store, NetLog* net_log)
    : CookieMonster(std::move(store), base::Seconds(kDefaultAccessUpdateThresholdSeconds), net_log)
{
}

CookieMonster::CookieMonster(scoped_refptr<PersistentCookieStore> store, base::TimeDelta last_access_threshold, NetLog* net_log)
...
{
    cookieable_schemes_.insert(cookieable_schemes_.begin(), kDefaultCookieableSchemes, kDefaultCookieableSchemes + kDefaultCookieableSchemesCount);
    net_log_.BeginEvent(NetLogEventType::COOKIE_STORE_ALIVE, [&] { return NetLogCookieMonsterConstructorParams(store_ != nullptr); });
}
...
void CookieMonster::FlushStore(base::OnceClosure callback)
{
    ...
    if (initialized_ && store_.get()) {
        ...
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, std::move(callback));
    }
}
...
void CookieMonster::SetAllCookiesAsync(const CookieList& list, SetCookiesCallback callback)
{
    DoCookieCallback(base::BindOnce(
        ...
        // the object.
        &CookieMonster::SetAllCookies, base::Unretained(this), list, std::move(callback)));
}

void CookieMonster::SetCanonicalCookieAsync(std::unique_ptr<CanonicalCookie> cookie, const GURL& source_url, const CookieOptions& options,
    SetCookiesCallback callback, std::optional<CookieAccessResult> cookie_access_result)
{
    DCHECK(cookie->IsCanonical());

    std::string domain = cookie->Domain();
    DoCookieCallbackForHostOrDomain(base::BindOnce(
                                        // base::Unretained is safe as DoCookieCallbackForHostOrDomain stores
                                        // the callback on |*this|, so the callback will not outlive
                                        // the object.
                                        &CookieMonster::SetCanonicalCookie, base::Unretained(this), std::move(cookie), source_url, options, std::move(callback),
                                        std::move(cookie_access_result)),
        domain);
}

void CookieMonster::GetCookieListWithOptionsAsync(
    const GURL& url, const CookieOptions& options, const CookiePartitionKeyCollection& cookie_partition_key_collection, GetCookieListCallback callback)
{
    DoCookieCallbackForURL(
        base::BindOnce(
            ...
            // the callback on |*this|, so the callback will not outlive
            // the object.
            &CookieMonster::GetCookieListWithOptions, base::Unretained(this), url, options, cookie_partition_key_collection, std::move(callback)),
        url);
}
...
void CookieMonster::GetAllCookiesAsync(GetAllCookiesCallback callback)
{
    DoCookieCallback(base::BindOnce(
        ...
        // the callback on |*this|, so the callback will not outlive
        // the object.
        &CookieMonster::GetAllCookies, base::Unretained(this), std::move(callback)));
}

void CookieMonster::GetAllCookiesWithAccessSemanticsAsync(GetAllCookiesWithAccessSemanticsCallback callback)
{
    DoCookieCallback(base::BindOnce(
        // base::Unretained is safe as DoCookieCallback stores
        // the callback on |*this|, so the callback will not outlive
        ...
        base::BindOnce(&CookieMonster::AttachAccessSemanticsListForCookieList, base::Unretained(this), std::move(callback))));
}

void CookieMonster::DeleteCanonicalCookieAsync(const CanonicalCookie& cookie, DeleteCallback callback)
{
    DoCookieCallback(base::BindOnce(
        // base::Unretained is safe as DoCookieCallback stores
        // the callback on |*this|, so the callback will not outlive
        // the object.
        &CookieMonster::DeleteCanonicalCookie, base::Unretained(this), cookie, std::move(callback)));
}

void CookieMonster::DeleteAllCreatedInTimeRangeAsync(const TimeRange& creation_range, DeleteCallback callback)
{
    DoCookieCallback(base::BindOnce(
        // base::Unretained is safe as DoCookieCallback stores
        // the callback on |*this|, so the callback will not outlive
        // the object.
        &CookieMonster::DeleteAllCreatedInTimeRange, base::Unretained(this), creation_range, std::move(callback)));
}

void CookieMonster::DeleteAllMatchingInfoAsync(CookieDeletionInfo delete_info, DeleteCallback callback)
{
    auto cookie_matcher = base::BindRepeating(&CookieMonster::MatchCookieDeletionInfo, base::Unretained(this), std::move(delete_info));

    DoCookieCallback(base::BindOnce(
        // base::Unretained is safe as DoCookieCallback stores
        // the callback on |*this|, so the callback will not outlive
        // the object.
        &CookieMonster::DeleteMatchingCookies, base::Unretained(this), std::move(cookie_matcher), DELETE_COOKIE_EXPLICIT, std::move(callback)));
}

void CookieMonster::DeleteSessionCookiesAsync(CookieStore::DeleteCallback callback)
{
    auto session_cookie_matcher = base::BindRepeating([](const net::CanonicalCookie& cookie) { return !cookie.IsPersistent(); });
    DoCookieCallback(base::BindOnce(
        // base::Unretained is safe as DoCookieCallback stores
        // the callback on |*this|, so the callback will not outlive
        // the object.
        &CookieMonster::DeleteMatchingCookies, base::Unretained(this), std::move(session_cookie_matcher), DELETE_COOKIE_EXPIRED, std::move(callback)));
}

void CookieMonster::DeleteMatchingCookiesAsync(CookieStore::DeletePredicate predicate, CookieStore::DeleteCallback callback)
{
    DoCookieCallback(base::BindOnce(
        // base::Unretained is safe as DoCookieCallback stores
        // the callback on |*this|, so the callback will not outlive
        // the object.
        &CookieMonster::DeleteMatchingCookies, base::Unretained(this), std::move(predicate), DELETE_COOKIE_EXPLICIT, std::move(callback)));
}

void CookieMonster::SetCookieableSchemes(const std::vector<std::string>& schemes, SetCookieableSchemesCallback callback)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
void CookieMonster::SetPersistSessionCookies(bool persist_session_cookies)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    DCHECK(!initialized_);
    net_log_.AddEntryWithBoolParams(NetLogEventType::COOKIE_STORE_SESSION_PERSISTENCE, NetLogEventPhase::NONE, "persistence", persist_session_cookies);
    persist_session_cookies_ = persist_session_cookies;
}

const char* const CookieMonster::kDefaultCookieableSchemes[] = { "http", "https", "ws", "wss" };
const int CookieMonster::kDefaultCookieableSchemesCount = std::size(kDefaultCookieableSchemes);
...
// static
bool CookieMonster::CookieSorter(const CanonicalCookie* cc1, const CanonicalCookie* cc2)
{
...
void CookieMonster::GetAllCookies(GetAllCookiesCallback callback)
{
    ...
    GarbageCollectExpired(Time::Now(), CookieMapItPair(cookies_.begin(), cookies_.end()), nullptr);
...
void CookieMonster::AttachAccessSemanticsListForCookieList(GetAllCookiesWithAccessSemanticsCallback callback, const CookieList& cookie_list)
{
    ...
    MaybeRunCookieCallback(std::move(callback), cookie_list, access_semantics_list);
}
    ...
    if (HasCookieableScheme(url)) {
        ...
        if (!cookie_partition_key_collection.IsEmpty()) {
            if (cookie_partition_key_collection.ContainsAllKeys()) {
                for (PartitionedCookieMap::iterator partition_it = partitioned_cookies_.begin(); partition_it != partitioned_cookies_.end();) {
                    ...
                    std::vector<CanonicalCookie*> partitioned_cookie_ptrs = FindPartitionedCookiesForRegistryControlledHost(cur_partition_it->first, url);
                    cookie_ptrs.insert(cookie_ptrs.end(), partitioned_cookie_ptrs.begin(), partitioned_cookie_ptrs.end());
                }
            } else {
                for (const CookiePartitionKey& key : cookie_partition_key_collection.PartitionKeys()) {
                    std::vector<CanonicalCookie*> partitioned_cookie_ptrs = FindPartitionedCookiesForRegistryControlledHost(key, url);
                    cookie_ptrs.insert(cookie_ptrs.end(), partitioned_cookie_ptrs.begin(), partitioned_cookie_ptrs.end());
                }
            }
        }
        ...
        FilterCookiesWithOptions(url, options, &cookie_ptrs, &included_cookies, &excluded_cookies);
    }

    MaybeRunCookieCallback(std::move(callback), included_cookies, excluded_cookies);
}

void CookieMonster::DeleteAllCreatedInTimeRange(const TimeRange& creation_range, DeleteCallback callback)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    for (PartitionedCookieMap::iterator partition_it = partitioned_cookies_.begin(); partition_it != partitioned_cookies_.end();) {
        ...
        while (cookie_it != cookie_end) {
            ...
            if (creation_range.Contains(cc->CreationDate())) {
                InternalDeletePartitionedCookie(cur_partition_it, cur_cookie_it, true /*sync_to_store*/, DELETE_COOKIE_EXPLICIT);
                ++num_deleted;
            }
        }
    }
    ...
    FlushStore(base::BindOnce(
        &MaybeRunDeleteCallback, weak_ptr_factory_.GetWeakPtr(), callback ? base::BindOnce(std::move(callback), num_deleted) : base::OnceClosure()));
}

bool CookieMonster::MatchCookieDeletionInfo(const CookieDeletionInfo& delete_info, const net::CanonicalCookie& cookie)
{
    bool delegate_treats_url_as_trustworthy = false; // irrelevant if no URL.
    if (delete_info.url.has_value()) {
        delegate_treats_url_as_trustworthy = cookie_access_delegate() && cookie_access_delegate()->ShouldTreatUrlAsTrustworthy(delete_info.url.value());
    }

    return delete_info.Matches(cookie, CookieAccessParams { GetAccessSemanticsForCookie(cookie), delegate_treats_url_as_trustworthy });
}

void CookieMonster::DeleteCanonicalCookie(const CanonicalCookie& cookie, DeleteCallback callback)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    uint32_t result = 0u;
    ...
    if (cookie.IsPartitioned()) {
        cookie_partition_it = partitioned_cookies_.find(cookie.PartitionKey().value());
    ...
    if (cookie_map) {
        for (CookieMapItPair its = cookie_map->equal_range(GetKey(cookie.Domain())); its.first != its.second; ++its.first) {
            ...
            if (candidate->IsEquivalent(cookie) && candidate->Value() == cookie.Value()) {
                if (cookie.IsPartitioned()) {
                    InternalDeletePartitionedCookie(cookie_partition_it, its.first, true, DELETE_COOKIE_EXPLICIT);
                } else {
    ...
    FlushStore(
        base::BindOnce(&MaybeRunDeleteCallback, weak_ptr_factory_.GetWeakPtr(), callback ? base::BindOnce(std::move(callback), result) : base::OnceClosure()));
}

void CookieMonster::DeleteMatchingCookies(DeletePredicate predicate, DeletionCause cause, DeleteCallback callback)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    DCHECK(predicate);
    ...
    for (auto partition_it = partitioned_cookies_.begin(); partition_it != partitioned_cookies_.end();) {
        ...
        while (cookie_it != cookie_end) {
            ...
            if (predicate.Run(*cc)) {
                InternalDeletePartitionedCookie(cur_partition_it, cur_cookie_it, true, cause);
                ++num_deleted;
            }
        }
    }
...
void CookieMonster::FetchAllCookies()
{
    ...
    DCHECK(!finished_fetching_all_cookies_) << "All cookies have already been fetched.";

    // We bind in the current time so that we can report the wall-clock time for
    // loading cookies.
    store_->Load(base::BindOnce(&CookieMonster::OnLoaded, weak_ptr_factory_.GetWeakPtr(), TimeTicks::Now()), net_log_);
}

void CookieMonster::OnLoaded(TimeTicks beginning_time, std::vector<std::unique_ptr<CanonicalCookie>> cookies)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    StoreLoadedCookies(std::move(cookies));
    base::TimeTicks now = base::TimeTicks::Now();
    base::UmaHistogramCustomTimes("Cookie.TimeBlockedOnLoad", now - beginning_time, base::Milliseconds(1), base::Minutes(1), 50);
    ...
    base::UmaHistogramCustomTimes("Cookie.TimeOpsBlockedDueToGlobalOp", blocked_due_to_global_op, base::Milliseconds(1), base::Minutes(1), 50);

    // Invoke the task queue of cookie request.
    InvokeQueue();
}

void CookieMonster::OnKeyLoaded(const std::string& key, std::vector<std::unique_ptr<CanonicalCookie>> cookies)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
void CookieMonster::StoreLoadedCookies(std::vector<std::unique_ptr<CanonicalCookie>> cookies)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    std::vector<PartitionedCookieMapIterators> partitioned_cookies_with_control_chars;
    ...
    for (auto& cookie : cookies) {
        ...
        if (cookie_ptr->IsPartitioned()) {
            auto inserted = InternalInsertPartitionedCookie(
                GetKey(cookie_ptr->Domain()), std::move(cookie), false /* sync_to_store */, access_result, false /* dispatch_change */);
            if (ContainsControlCharacter(cookie_ptr->Name()) || ContainsControlCharacter(cookie_ptr->Value())) {
                partitioned_cookies_with_control_chars.push_back(inserted);
            }
        } else {
            auto inserted
                = InternalInsertCookie(GetKey(cookie_ptr->Domain()), std::move(cookie), false /* sync_to_store */, access_result, false /* dispatch_change */);
    ...
    for (auto it = cookies_with_control_chars.begin(); it != cookies_with_control_chars.end();) {
        auto curit = it;
        ++it;
        InternalDeleteCookie(*curit, true, DELETE_COOKIE_CONTROL_CHAR);
    }
    for (auto it = partitioned_cookies_with_control_chars.begin(); it != partitioned_cookies_with_control_chars.end();) {
        ...
        auto curit = it;
        ++it;
        InternalDeletePartitionedCookie(curit->first, curit->second, true, DELETE_COOKIE_CONTROL_CHAR);
    }
...
void CookieMonster::InvokeQueue()
{
    ...
    for (auto& tasks_for_key : tasks_pending_for_key_) {
        tasks_pending_.insert(
            tasks_pending_.begin(), std::make_move_iterator(tasks_for_key.second.begin()), std::make_move_iterator(tasks_for_key.second.end()));
    }
...
void CookieMonster::EnsureCookiesMapIsValid()
{
    ...
    for (auto next = cookies_.begin(); next != cookies_.end();) {
        ...
        TrimDuplicateCookiesForKey(key, cur_range_begin, cur_range_end, std::nullopt);
    }

    for (auto cookie_partition_it = partitioned_cookies_.begin(); cookie_partition_it != partitioned_cookies_.end();) {
        ...
        while (prev_range_end != cookie_partition->end()) {
            ...
            TrimDuplicateCookiesForKey(key, cur_range_begin, cur_range_end, std::make_optional(cur_cookie_partition_it));
        }
    }
}
...
void CookieMonster::TrimDuplicateCookiesForKey(
    const std::string& key, CookieMap::iterator begin, CookieMap::iterator end, std::optional<PartitionedCookieMap::iterator> cookie_partition_it)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    typedef std::map<CanonicalCookie::UniqueDomainCookieKey, CookieSet> DomainEquivalenceMap;
    ...
    for (auto it = begin; it != end; ++it) {
        ...
        // Is a domain cookie.
            ...
            CanonicalCookie::UniqueDomainCookieKey signature(cookie->UniqueDomainKey());
    ...
    for (std::pair<const CanonicalCookie::UniqueCookieKey, CookieSet>& equivalent_cookie : equivalent_cookies) {
        ...
        LOG(ERROR) << base::StringPrintf("Found %d duplicate cookies for key='%s', "
                                         "with {name='%s', domain='%s', path='%s'}",
            static_cast<int>(dupes.size()), key.c_str(), std::get<1>(signature).c_str(), std::get<2>(signature).c_str(), std::get<3>(signature).c_str());
        ...
        for (const CookieMap::iterator& dupe : dupes) {
            if (cookie_partition_it) {
                InternalDeletePartitionedCookie(cookie_partition_it.value(), dupe, true, DELETE_COOKIE_DUPLICATE_IN_BACKING_STORE);
            } else {
                InternalDeleteCookie(dupe, true, DELETE_COOKIE_DUPLICATE_IN_BACKING_STORE);
            }
        }
    }
    ...
    for (std::pair<const CanonicalCookie::UniqueDomainCookieKey, CookieSet>& equivalent_domain_cookie : equivalent_domain_cookies) {
        const CanonicalCookie::UniqueDomainCookieKey& signature = equivalent_domain_cookie.first;
        ...
        // statement as well if needed.
        // TODO(crbug.com/40165805): Include source scheme and source port.
        LOG(ERROR) << base::StringPrintf("Found %d duplicate domain cookies for key='%s', "
                                         "with {name='%s', domain='%s', path='%s'}",
            static_cast<int>(dupes.size()), key.c_str(), std::get<1>(signature).c_str(), std::get<2>(signature).c_str(), std::get<3>(signature).c_str());
...
std::vector<CanonicalCookie*> CookieMonster::FindCookiesForRegistryControlledHost(
    const GURL& url, CookieMap* cookie_map, CookieMonster::PartitionedCookieMap::iterator* partition_it)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    for (CookieMapItPair its = cookie_map->equal_range(key); its.first != its.second;) {
        ...
        if (cc->IsExpired(current_time)) {
            if (cc->IsPartitioned()) {
                ...
                InternalDeletePartitionedCookie(*partition_it, curit, true, DELETE_COOKIE_EXPIRED);
            } else {
...
std::vector<CanonicalCookie*> CookieMonster::FindPartitionedCookiesForRegistryControlledHost(const CookiePartitionKey& cookie_partition_key, const GURL& url)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);

    PartitionedCookieMap::iterator it = partitioned_cookies_.find(cookie_partition_key);
...
void CookieMonster::FilterCookiesWithOptions(const GURL& url, const CookieOptions options, std::vector<CanonicalCookie*>* cookie_ptrs,
    CookieAccessResultList* included_cookies, CookieAccessResultList* excluded_cookies)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    bool delegate_treats_url_as_trustworthy = cookie_access_delegate() && cookie_access_delegate()->ShouldTreatUrlAsTrustworthy(url);

    std::vector<std::pair<CanonicalCookie*, CookieAccessResult>> cookies_and_access_results;
    ...
    for (CanonicalCookie* cookie_ptr : *cookie_ptrs) {
        ...
        CookieAccessResult access_result = cookie_ptr->IncludeForRequestURL(
            url, options, CookieAccessParams { GetAccessSemanticsForCookie(*cookie_ptr), delegate_treats_url_as_trustworthy });
        ...
        status_copy.RemoveExclusionReason(CookieInclusionStatus::EXCLUDE_NOT_ON_PATH);

        bool exclusion_or_warning = !status_copy.IsInclude() || status_copy.HasWarningReason(CookieInclusionStatus::WARN_SCHEME_MISMATCH)
    ...
    for (auto& cookie_result : cookies_and_access_results) {
        ...
        if (access_result.status.IsInclude()) {
            ...
            if (IsLocalhost(url)) {
                UMA_HISTOGRAM_ENUMERATION("Cookie.Port.Read.Localhost", ReducePortRangeForCookieHistogram(destination_port));
                UMA_HISTOGRAM_ENUMERATION(
                    "Cookie.Port.ReadDiffersFromSet.Localhost", IsCookieSentToSamePortThatSetIt(url, cookie_ptr->SourcePort(), cookie_ptr->SourceScheme()));
            } else {
                UMA_HISTOGRAM_ENUMERATION("Cookie.Port.Read.RemoteHost", ReducePortRangeForCookieHistogram(destination_port));
                UMA_HISTOGRAM_ENUMERATION(
                    "Cookie.Port.ReadDiffersFromSet.RemoteHost", IsCookieSentToSamePortThatSetIt(url, cookie_ptr->SourcePort(), cookie_ptr->SourceScheme()));
            }

            if (cookie_ptr->IsDomainCookie()) {
                UMA_HISTOGRAM_ENUMERATION(
                    "Cookie.Port.ReadDiffersFromSet.DomainSet", IsCookieSentToSamePortThatSetIt(url, cookie_ptr->SourcePort(), cookie_ptr->SourceScheme()));
            }
        }
        ...
        // reasons).
        bool scheme_mismatch = access_result.status.HasExclusionReason(CookieInclusionStatus::EXCLUDE_SCHEME_MISMATCH)
            || access_result.status.HasWarningReason(CookieInclusionStatus::WARN_SCHEME_MISMATCH);

        if (cookie_ptr->IsDomainCookie() && !scheme_mismatch && origin_cookie_names.count(cookie_ptr->Name())) {
            if (cookie_util::IsSchemeBoundCookiesEnabled()) {
                access_result.status.AddExclusionReason(CookieInclusionStatus::EXCLUDE_SHADOWING_DOMAIN);
            } else {
                access_result.status.AddWarningReason(CookieInclusionStatus::WARN_SHADOWING_DOMAIN);
            }
        }
...
void CookieMonster::MaybeDeleteEquivalentCookieAndUpdateStatus(const std::string& key, const CanonicalCookie& cookie_being_set,
    bool allowed_to_set_secure_cookie, bool skip_httponly, bool already_expired, base::Time* creation_date_to_inherit, CookieInclusionStatus* status,
    std::optional<PartitionedCookieMap::iterator> cookie_partition_it)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    DCHECK(!status->HasExclusionReason(CookieInclusionStatus::EXCLUDE_OVERWRITE_SECURE));
    DCHECK(!status->HasExclusionReason(CookieInclusionStatus::EXCLUDE_OVERWRITE_HTTP_ONLY));
    ...
    for (auto cur_it = range_its.first; cur_it != range_its.second; ++cur_it) {
        ...
        if (cur_existing_cookie->SecureAttribute() && !allowed_to_set_secure_cookie
            && cookie_being_set.IsEquivalentForSecureCookieMatching(*cur_existing_cookie)) {
            ...
            net_log_.AddEvent(NetLogEventType::COOKIE_STORE_COOKIE_REJECTED_SECURE, [&](NetLogCaptureMode capture_mode) {
                return NetLogCookieMonsterCookieRejectedSecure(skipped_secure_cookie, &cookie_being_set, capture_mode);
            });
            status->AddExclusionReason(CookieInclusionStatus::EXCLUDE_OVERWRITE_SECURE);
        }
        ...
        if (cookie_being_set.IsEquivalent(*cur_existing_cookie)) {
            ...
            CHECK(!found_equivalent_cookie) << "Duplicate equivalent cookies found, cookie store is corrupted.";
            ...
            if (skip_httponly && cur_existing_cookie->IsHttpOnly()) {
                net_log_.AddEvent(NetLogEventType::COOKIE_STORE_COOKIE_REJECTED_HTTPONLY, [&](NetLogCaptureMode capture_mode) {
                    return NetLogCookieMonsterCookieRejectedHttponly(cur_existing_cookie, &cookie_being_set, capture_mode);
                });
                status->AddExclusionReason(CookieInclusionStatus::EXCLUDE_OVERWRITE_HTTP_ONLY);
            } else {
    ...
    if (deletion_candidate_it != cookie_map->end()) {
        ...
        if (status->IsInclude()) {
            if (cookie_being_set.IsPartitioned()) {
                InternalDeletePartitionedCookie(cookie_partition_it.value(), deletion_candidate_it, true /* sync_to_store */,
                    already_expired ? DELETE_COOKIE_EXPIRED_OVERWRITE : DELETE_COOKIE_OVERWRITE);
            } else {
                InternalDeleteCookie(
                    deletion_candidate_it, true /* sync_to_store */, already_expired ? DELETE_COOKIE_EXPIRED_OVERWRITE : DELETE_COOKIE_OVERWRITE);
            }
        } else if (status->HasExclusionReason(CookieInclusionStatus::EXCLUDE_OVERWRITE_SECURE)) {
            ...
            net_log_.AddEvent(NetLogEventType::COOKIE_STORE_COOKIE_PRESERVED_SKIPPED_SECURE, [&](NetLogCaptureMode capture_mode) {
                return NetLogCookieMonsterCookiePreservedSkippedSecure(skipped_secure_cookie, deletion_candidate, &cookie_being_set, capture_mode);
            });
        }
    }
}
...
CookieMonster::CookieMap::iterator CookieMonster::InternalInsertCookie(
    const std::string& key, std::unique_ptr<CanonicalCookie> cc, bool sync_to_store, const CookieAccessResult& access_result, bool dispatch_change)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    CanonicalCookie* cc_ptr = cc.get();

    net_log_.AddEvent(NetLogEventType::COOKIE_STORE_COOKIE_ADDED,
        [&](NetLogCaptureMode capture_mode) { return NetLogCookieMonsterCookieAdded(cc.get(), sync_to_store, capture_mode); });
    ...
    if (dispatch_change) {
        change_dispatcher_.DispatchChange(CookieChangeInfo(*cc_ptr, access_result, CookieChangeCause::INSERTED), true);
    }

    // If this is the first cookie in |cookies_| with this key, increment the
    // |num_keys_| counter.
    bool different_prev = inserted == cookies_.begin() || std::prev(inserted)->first != key;
    ...
    DCHECK(std::next(inserted) == cookies_.end() || std::next(inserted)->first != key);
    if (different_prev)
        ++num_keys_;
...
CookieMonster::PartitionedCookieMapIterators CookieMonster::InternalInsertPartitionedCookie(
    std::string key, std::unique_ptr<CanonicalCookie> cc, bool sync_to_store, const CookieAccessResult& access_result, bool dispatch_change)
{
    ...
    PartitionedCookieMap::iterator partition_it = partitioned_cookies_.find(partition_key);
    if (partition_it == partitioned_cookies_.end()) {
        partition_it = partitioned_cookies_.insert(PartitionedCookieMap::value_type(std::move(partition_key), std::make_unique<CookieMap>())).first;
    }

    CookieMap::iterator cookie_it = partition_it->second->insert(CookieMap::value_type(std::move(key), std::move(cc)));
...
void CookieMonster::SetCanonicalCookie(std::unique_ptr<CanonicalCookie> cc, const GURL& source_url, const CookieOptions& options, SetCookiesCallback callback,
...
// crashing network service process.
    ...
    bool delegate_treats_url_as_trustworthy = cookie_access_delegate() && cookie_access_delegate()->ShouldTreatUrlAsTrustworthy(source_url);

    CookieAccessResult access_result = cc->IsSetPermittedInContext(source_url, options,
        CookieAccessParams(GetAccessSemanticsForCookie(*cc), delegate_treats_url_as_trustworthy), cookieable_schemes_, cookie_access_result);
    ...
    if (should_try_to_delete_duplicates) {
        MaybeDeleteEquivalentCookieAndUpdateStatus(key, *cc, access_result.is_allowed_to_access_secure_cookies, options.exclude_httponly(), already_expired,
            &creation_date_to_inherit, &access_result.status, cookie_partition_it);
    }

    if (access_result.status.HasExclusionReason(CookieInclusionStatus::EXCLUDE_OVERWRITE_SECURE)
        || access_result.status.HasExclusionReason(CookieInclusionStatus::EXCLUDE_OVERWRITE_HTTP_ONLY)) {
        DVLOG(net::cookie_util::kVlogSetCookies) << "SetCookie() not clobbering httponly cookie or secure cookie for "
                                                    "insecure scheme";
    }

    if (access_result.status.IsInclude()) {
        DVLOG(net::cookie_util::kVlogSetCookies) << "SetCookie() key: " << key << " cc: " << cc->DebugString();
        ...
        if (cc->IsEffectivelySameSiteNone()) {
            ...
            if (cc->IsPartitioned()) {
                UMA_HISTOGRAM_COUNTS_10000("Cookie.SameSiteNoneSizeBytes.Partitioned", cookie_size);
            } else {
                UMA_HISTOGRAM_COUNTS_10000("Cookie.SameSiteNoneSizeBytes.Unpartitioned", cookie_size);
            }
        }
        ...
        if (!already_expired) {
            ...
            DVLOG(net::cookie_util::kVlogSetCookies) << "SetCookie() not storing already expired cookie.";
        }
        ...
        if (cookie_partition_key.has_value()) {
            GarbageCollectPartitionedCookies(creation_date, cookie_partition_key.value(), key);
        } else {
            GarbageCollect(creation_date, key);
        }

        if (IsLocalhost(source_url)) {
            UMA_HISTOGRAM_ENUMERATION("Cookie.Port.Set.Localhost", ReducePortRangeForCookieHistogram(source_url.EffectiveIntPort()));
        } else {
            UMA_HISTOGRAM_ENUMERATION("Cookie.Port.Set.RemoteHost", ReducePortRangeForCookieHistogram(source_url.EffectiveIntPort()));
        }

        UMA_HISTOGRAM_ENUMERATION("Cookie.CookieSourceSchemeName", GetSchemeNameEnum(source_url));
    } else {
        // If the cookie would be excluded, don't bother warning about the 3p cookie
        // phaseout.
        access_result.status.RemoveWarningReason(net::CookieInclusionStatus::WARN_THIRD_PARTY_PHASEOUT);
    }
...
void CookieMonster::SetAllCookies(CookieList list, SetCookiesCallback callback)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    for (const auto& cookie : list) {
        ...
        if (cookie.IsPartitioned()) {
            InternalInsertPartitionedCookie(key, std::make_unique<CanonicalCookie>(cookie), true, access_result);
            GarbageCollectPartitionedCookies(creation_time, cookie.PartitionKey().value(), key);
        } else {
            InternalInsertCookie(key, std::make_unique<CanonicalCookie>(cookie), true, access_result);
...
void CookieMonster::InternalUpdateCookieAccessTime(CanonicalCookie* cc, const Time& current)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
void CookieMonster::InternalDeleteCookie(CookieMap::iterator it, bool sync_to_store, DeletionCause deletion_cause)
{
    ...
    // this check here.
    static_assert(std::size(kChangeCauseMapping) == DELETE_COOKIE_LAST_ENTRY + 1, "kChangeCauseMapping size should match DeletionCause size");

    CanonicalCookie* cc = it->second.get();
    DVLOG(net::cookie_util::kVlogSetCookies) << "InternalDeleteCookie()"
    ...
    if (deletion_cause != DELETE_COOKIE_DONT_RECORD) {
        net_log_.AddEvent(NetLogEventType::COOKIE_STORE_COOKIE_DELETED,
            [&](NetLogCaptureMode capture_mode) { return NetLogCookieMonsterCookieDeleted(cc, mapping.cause, sync_to_store, capture_mode); });
    }

    if (ShouldUpdatePersistentStore(cc) && sync_to_store)
        store_->DeleteCookie(*cc);

    change_dispatcher_.DispatchChange(CookieChangeInfo(*cc,
                                          CookieAccessResult(CookieEffectiveSameSite::UNDEFINED, CookieInclusionStatus(), GetAccessSemanticsForCookie(*cc),
    ...
    // |num_keys_| counter.
    bool different_prev = it == cookies_.begin() || std::prev(it)->first != it->first;
    bool different_next = std::next(it) == cookies_.end() || std::next(it)->first != it->first;
    if (different_prev && different_next)
        --num_keys_;

    DCHECK(cookies_.find(it->first) != cookies_.end()) << "Called erase with an iterator not in the cookie map";
    cookies_.erase(it);
}

void CookieMonster::InternalDeletePartitionedCookie(
    PartitionedCookieMap::iterator partition_it, CookieMap::iterator cookie_it, bool sync_to_store, DeletionCause deletion_cause)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    DCHECK(cc->IsPartitioned());
    DVLOG(net::cookie_util::kVlogSetCookies) << "InternalDeletePartitionedCookie()"
                                             << ", cause:" << deletion_cause << ", cc: " << cc->DebugString();
    ...
    DCHECK(partition_it->second->find(cookie_it->first) != partition_it->second->end()) << "Called erase with an iterator not in this partitioned cookie map";
...
size_t CookieMonster::GarbageCollect(const Time& current, const std::string& key)
{
    ...
    const bool obc_behavior_enabled = cookie_util::IsOriginBoundCookiesPartiallyEnabled();

    // Collect garbage for this key, minding cookie priorities.
    if (cookies_.count(key) > kDomainMaxCookies) {
        DVLOG(net::cookie_util::kVlogGarbageCollection) << "GarbageCollect() key: " << key;
        ...
        if (cookie_its->size() > kDomainMaxCookies) {
            DVLOG(net::cookie_util::kVlogGarbageCollection) << "Deep Garbage Collect domain.";
            ...
            for (const auto& purge_round : kPurgeRounds) {
                ...
                if (purge_goal > 0) {
                    if (obc_behavior_enabled) {
                        just_deleted
                            = PurgeLeastRecentMatchesForOBC(&cookie_it_list, purge_round.priority, quota, purge_goal, !purge_round.protect_secure_cookies);
                    } else {
                        just_deleted = PurgeLeastRecentMatches(cookie_its, purge_round.priority, quota, purge_goal, purge_round.protect_secure_cookies);
                    }
    ...
    if (cookies_.size() > kMaxCookies && earliest_access_time_ < safe_date) {
        DVLOG(net::cookie_util::kVlogGarbageCollection) << "GarbageCollect() everything";
        CookieItVector cookie_its;

        num_deleted += GarbageCollectExpired(current, CookieMapItPair(cookies_.begin(), cookies_.end()), &cookie_its);

        if (cookie_its.size() > kMaxCookies) {
            DVLOG(net::cookie_util::kVlogGarbageCollection) << "Deep Garbage Collect everything.";
            ...
            SplitCookieVectorIntoSecureAndNonSecure(cookie_its, &secure_cookie_its, &non_secure_cookie_its);
            size_t non_secure_purge_goal = std::min<size_t>(purge_goal, non_secure_cookie_its.size());

            base::Time earliest_non_secure_access_time;
            size_t just_deleted = GarbageCollectLeastRecentlyAccessed(
                current, safe_date, non_secure_purge_goal, std::move(non_secure_cookie_its), &earliest_non_secure_access_time);
            num_deleted += just_deleted;
            ...
            if (secure_cookie_its.size() == 0) {
                ...
                size_t secure_purge_goal = std::min<size_t>(purge_goal - just_deleted, secure_cookie_its.size());
                base::Time earliest_secure_access_time;
                num_deleted
                    += GarbageCollectLeastRecentlyAccessed(current, safe_date, secure_purge_goal, std::move(secure_cookie_its), &earliest_secure_access_time);

                if (!earliest_non_secure_access_time.is_null() && earliest_non_secure_access_time < earliest_secure_access_time) {
                    earliest_access_time_ = earliest_non_secure_access_time;
                } else {
...
size_t CookieMonster::GarbageCollectPartitionedCookies(const base::Time& current, const CookiePartitionKey& cookie_partition_key, const std::string& key)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);

    size_t num_deleted = 0;
    PartitionedCookieMap::iterator cookie_partition_it = partitioned_cookies_.find(cookie_partition_key);

    if (cookie_partition_it == partitioned_cookies_.end())
        return num_deleted;

    if (NumBytesInCookieMapForKey(*cookie_partition_it->second.get(), key) > kPerPartitionDomainMaxCookieBytes
        ...
        CookieItVector non_expired_cookie_its;
        num_deleted
            += GarbageCollectExpiredPartitionedCookies(current, cookie_partition_it, cookie_partition_it->second->equal_range(key), &non_expired_cookie_its);

        size_t bytes_used = NumBytesInCookieItVector(non_expired_cookie_its);

        if (bytes_used > kPerPartitionDomainMaxCookieBytes || non_expired_cookie_its.size() > kPerPartitionDomainMaxCookies) {
            // TODO(crbug.com/40188414): Log deep garbage collection for partitioned
            // cookies.
            std::sort(non_expired_cookie_its.begin(), non_expired_cookie_its.end(), LRACookieSorter);

            for (size_t i = 0; bytes_used > kPerPartitionDomainMaxCookieBytes || non_expired_cookie_its.size() - i > kPerPartitionDomainMaxCookies; ++i) {
                bytes_used -= NameValueSizeBytes(*non_expired_cookie_its[i]->second);
                InternalDeletePartitionedCookie(cookie_partition_it, non_expired_cookie_its[i], true, DELETE_COOKIE_EVICTED_PER_PARTITION_DOMAIN);
                ++num_deleted;
            }
        }
    }
...
size_t CookieMonster::PurgeLeastRecentMatches(
    CookieItVector* cookies, CookiePriority priority, size_t to_protect, size_t purge_goal, bool protect_secure_cookies)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);

    // 1. Count number of the cookies at |priority|
    size_t cookies_count_possibly_to_be_deleted = CountCookiesForPossibleDeletion(priority, cookies, false /* count all cookies */);
    ...
    if (protect_secure_cookies) {
        secure_cookies = CountCookiesForPossibleDeletion(priority, cookies, protect_secure_cookies /* count secure cookies */);
    ...
    while ((removed < purge_goal && current < cookies->size()) && cookies_count_possibly_to_be_deleted > 0) {
        ...
        // the current level.
        if (IsCookieEligibleForEviction(priority, protect_secure_cookies, current_cookie)) {
            InternalDeleteCookie(cookies->at(current), true, DELETE_COOKIE_EVICTED_DOMAIN);
...
size_t CookieMonster::PurgeLeastRecentMatchesForOBC(
    CookieItList* cookies, CookiePriority priority, size_t to_protect, size_t purge_goal, bool delete_secure_cookies)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    size_t total_could_be_deleted_for_priority = CountCookiesAndGenerateListsForPossibleDeletion(priority, could_be_deleted, cookies, delete_secure_cookies);
    ...
    size_t total_deletable = could_be_deleted.host_cookies.size() + could_be_deleted.domain_cookies.size();
    size_t max_cookies_to_delete_this_round = std::min(total_deletable, total_could_be_deleted_for_priority - to_protect);
    ...
    for (auto domain_list_it = could_be_deleted.domain_cookies.begin();
         domain_list_it != could_be_deleted.domain_cookies.end() && removed < purge_goal && max_cookies_to_delete_this_round > 0;) {
        auto cookies_list_it = *domain_list_it;
        auto cookie_map_it = *cookies_list_it;
        // Delete from the cookie store.
        InternalDeleteCookie(cookie_map_it, /*sync_to_store=*/true, DELETE_COOKIE_EVICTED_DOMAIN);
        // Delete from `cookies`.
    ...
    for (auto host_list_it = could_be_deleted.host_cookies.begin();
         host_list_it != could_be_deleted.host_cookies.end() && removed < purge_goal && max_cookies_to_delete_this_round > 0;) {
...
size_t CookieMonster::GarbageCollectExpired(const Time& current, const CookieMapItPair& itpair, CookieItVector* cookie_its)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
size_t CookieMonster::GarbageCollectExpiredPartitionedCookies(
    const Time& current, const PartitionedCookieMap::iterator& cookie_partition_it, const CookieMapItPair& itpair, CookieItVector* cookie_its)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    for (CookieMap::iterator it = itpair.first, end = itpair.second; it != end;) {
        ...
        if (curit->second->IsExpired(current)) {
            InternalDeletePartitionedCookie(cookie_partition_it, curit, true, DELETE_COOKIE_EXPIRED);
            ++num_deleted;
        } else if (cookie_its) {
            cookie_its->push_back(curit);
        }
...
void CookieMonster::GarbageCollectAllExpiredPartitionedCookies(const Time& current)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    for (auto it = partitioned_cookies_.begin(); it != partitioned_cookies_.end();) {
        ...
        ++it;
        GarbageCollectExpiredPartitionedCookies(current, cur_cookie_partition_it,
            CookieMapItPair(cur_cookie_partition_it->second->begin(), cur_cookie_partition_it->second->end()), nullptr /*cookie_its*/);
    }
}

size_t CookieMonster::GarbageCollectDeleteRange(const Time& current, DeletionCause cause, CookieItVector::iterator it_begin, CookieItVector::iterator it_end)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
size_t CookieMonster::GarbageCollectLeastRecentlyAccessed(
    const base::Time& current, const base::Time& safe_date, size_t purge_goal, CookieItVector cookie_its, base::Time* earliest_time)
{
    ...
    SortLeastRecentlyAccessed(cookie_its.begin(), cookie_its.end(), cookie_its.size() < purge_goal ? purge_goal + 1 : purge_goal);
    // Find boundary to cookies older than safe_date.
    auto global_purge_it = LowerBoundAccessDate(cookie_its.begin(), cookie_its.begin() + purge_goal, safe_date);
    // Only delete the old cookies and delete non-secure ones first.
    size_t num_deleted = GarbageCollectDeleteRange(current, DELETE_COOKIE_EVICTED_GLOBAL, cookie_its.begin(), global_purge_it);
...
{
    std::string effective_domain(registry_controlled_domains::GetDomainAndRegistry(domain, registry_controlled_domains::INCLUDE_PRIVATE_REGISTRIES));
...
bool CookieMonster::HasCookieableScheme(const GURL& url)
{
    ...
    bool is_cookieable
        = base::ranges::any_of(cookieable_schemes_, [&url](const std::string& cookieable_scheme) { return url.SchemeIs(cookieable_scheme.c_str()); });

    if (!is_cookieable) {
        // The scheme didn't match any in our allowed list.
        DVLOG(net::cookie_util::kVlogPerCookieMonster) << "WARNING: Unsupported cookie scheme: " << url.scheme();
    }
    return is_cookieable;
}

CookieAccessSemantics CookieMonster::GetAccessSemanticsForCookie(const CanonicalCookie& cookie) const
{
...
bool CookieMonster::DoRecordPeriodicStats()
{
    ...
    if (cookie_access_delegate()) {
        ...
        for (const auto& entry : cookies_) {
            sites.emplace_back(GURL(base::StrCat({ url::kHttpsScheme, "://", entry.first })));
        }
        for (const auto& [partition_key, cookie_map] : partitioned_cookies_) {
            for (const auto& [domain, unused_cookie] : *cookie_map) {
                sites.emplace_back(GURL(base::StrCat({ url::kHttpsScheme, "://", domain })));
            }
        }
        std::optional<base::flat_map<SchemefulSite, FirstPartySetEntry>> maybe_sets = cookie_access_delegate()->FindFirstPartySetEntries(
            sites, base::BindOnce(&CookieMonster::RecordPeriodicFirstPartySetsStats, weak_ptr_factory_.GetWeakPtr()));
    ...
    base::UmaHistogramCounts1000("Cookie.MaxSameSiteNoneCookiesPerKey", max_n_cookies);
    base::UmaHistogramCounts100000("Cookie.CookieJarSize", n_bytes >> 10);
    base::UmaHistogramCounts100000("Cookie.AvgCookieJarSizePerKey2", n_bytes / std::max(num_keys_, static_cast<size_t>(1)));
    base::UmaHistogramCounts100000("Cookie.MaxCookieJarSizePerKey", max_n_bytes >> 10);

    // Collect stats for partitioned cookies.
    base::UmaHistogramCounts1000("Cookie.PartitionCount", partitioned_cookies_.size());
    base::UmaHistogramCounts100000("Cookie.PartitionedCookieCount", num_partitioned_cookies_);
    base::UmaHistogramCounts100000("Cookie.PartitionedCookieCount.Nonced", num_nonced_partitioned_cookies_);
    base::UmaHistogramCounts100000("Cookie.PartitionedCookieCount.Unnonced", num_partitioned_cookies_ - num_nonced_partitioned_cookies_);
    base::UmaHistogramCounts100000("Cookie.PartitionedCookieJarSizeKibibytes", num_partitioned_cookies_bytes_ >> 10);
    base::UmaHistogramCounts100000("Cookie.PartitionedCookieJarSizeKibibytes.Nonced", num_nonced_partitioned_cookie_bytes_ >> 10);
    base::UmaHistogramCounts100000(
        "Cookie.PartitionedCookieJarSizeKibibytes.Unnonced", (num_partitioned_cookies_bytes_ - num_nonced_partitioned_cookie_bytes_) >> 10);

    for (const auto& it : bytes_per_cookie_partition_) {
        base::UmaHistogramCounts100000("Cookie.CookiePartitionSizeKibibytes", it.second >> 10);
    }

    return true;
}

void CookieMonster::RecordPeriodicFirstPartySetsStats(base::flat_map<SchemefulSite, FirstPartySetEntry> sets) const
{
    ...
    for (const auto& set : grouped_by_owner) {
        int sample = std::accumulate(set.second.begin(), set.second.end(), 0, [this](int acc, const net::SchemefulSite& site) -> int {
            DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
            if (!site.has_registrable_domain_or_host())
                return acc;
            return acc + cookies_.count(GetKey(site.GetURL().host()));
        });
        base::UmaHistogramCustomCounts("Cookie.PerFirstPartySetCount", sample, 0, 4000, 50);
    }
}
...
void CookieMonster::DoCookieCallbackForURL(base::OnceClosure callback, const GURL& url)
{
    DoCookieCallbackForHostOrDomain(std::move(callback), url.host_piece());
}

void CookieMonster::DoCookieCallbackForHostOrDomain(base::OnceClosure callback, std::string_view host_or_domain)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    MarkCookieStoreAsInitialized();
    FetchAllCookiesIfNecessary();
    ...
    if (!finished_fetching_all_cookies_ && store_.get()) {
        ...
        if (keys_loaded_.find(key) == keys_loaded_.end()) {
            ...
            if (it == tasks_pending_for_key_.end()) {
                store_->LoadCookiesForKey(key, base::BindOnce(&CookieMonster::OnKeyLoaded, weak_ptr_factory_.GetWeakPtr(), key));
                it = tasks_pending_for_key_.emplace(key, base::circular_deque<base::OnceClosure>()).first;
            }
...
CookieMonster::CookieSentToSamePort CookieMonster::IsCookieSentToSamePortThatSetIt(const GURL& destination, int source_port, CookieSourceScheme source_scheme)
{
    ...
    bool destination_port_is_default = url::DefaultPortForScheme(destination_scheme) == destination_port;
    ...
    std::string source_scheme_string
        = source_scheme == CookieSourceScheme::kSecure ? url::kHttpsScheme : url::kHttpScheme; // wss/ws have the same default port values as
        // https/http, so it's ok that we use these.

    bool source_port_is_default = url::DefaultPortForScheme(source_scheme_string) == source_port;
...
std::optional<bool> CookieMonster::SiteHasCookieInOtherPartition(const net::SchemefulSite& site, const std::optional<CookiePartitionKey>& partition_key) const
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    if (store_ && !finished_fetching_all_cookies_ && !keys_loaded_.count(domain)) {
        return std::nullopt;
    }
