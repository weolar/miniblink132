...
public:
    using CookieChangeCallbackList = base::RepeatingCallbackList<void(const CookieChangeInfo&)>;
    ...
    CookieMonsterChangeDispatcher& operator=(const CookieMonsterChangeDispatcher&) = delete;
...
private:
    class Subscription : public base::LinkNode<Subscription>, public CookieChangeSubscription {
    public:
        Subscription(base::WeakPtr<CookieMonsterChangeDispatcher> change_dispatcher, std::string domain_key, std::string name_key, GURL url,
            CookiePartitionKeyCollection cookie_partition_key_collection, net::CookieChangeCallback callback);
        ...
        void DispatchChange(const CookieChangeInfo& change, const CookieAccessDelegate* cookie_access_delegate);
    ...
    void DispatchChangeToDomainKey(const CookieChangeInfo& change, const std::string& domain_key);

    void DispatchChangeToNameKey(const CookieChangeInfo& change, CookieNameMap& name_map, const std::string& name_key);
