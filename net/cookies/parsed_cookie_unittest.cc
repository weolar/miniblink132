...
TEST(ParsedCookieTest, TestBasic)
{
    ...
    ParsedCookie pc2("c=d; secure; httponly; path=/foo; domain=bar.test; "
...
TEST(ParsedCookieTest, TestSetEmptyNameValue)
{
    ...
    EXPECT_TRUE(status.HasExclusionReason(CookieInclusionStatus::ExclusionReason::EXCLUDE_NO_COOKIE_CONTENT));
...
TEST(ParsedCookieTest, ParseValueStrings)
{
    std::string valid_values[]
        = { "httpONLY", "1%7C1624663551161", "<K0<r<C_<G_<S0", "lastRequest=1624663552846&activeDays=%5B0%2C0", "si=8da88dce-5fee-4835" };
    ...
    for (const auto& value : leading_whitespace_values) {
        EXPECT_TRUE(ParsedCookie::ParseValueString(value).length() == value.length() - 1);
        EXPECT_FALSE(ParsedCookie::ValueMatchesParsedValue(value));
    }
    ...
    const size_t valid_value_length = valid_values_with_trailing_chars[0].length() - 1;
    for (const auto& value : valid_values_with_trailing_chars) {
        EXPECT_TRUE(ParsedCookie::ParseValueString(value).length() == valid_value_length);
        EXPECT_FALSE(ParsedCookie::ValueMatchesParsedValue(value));
    }

    // A valid value (truncated after the ';') but parses out to a substring.
    std::string value_with_separator_in_middle("lastRequest=1624663552846; activeDays=%5B0%2C0");
    EXPECT_TRUE(ParsedCookie::ParseValueString(value_with_separator_in_middle).length() == value_with_separator_in_middle.find(';'));
    EXPECT_FALSE(ParsedCookie::ValueMatchesParsedValue(value_with_separator_in_middle));
}
...
TEST(ParsedCookieTest, TestQuoted)
{
    ...
    const struct {
        const char* input;
        const char* expected;
    } kTests[] = { // Trailing whitespace after a quoted value.  The whitespace after
    ...
    for (const auto& test : kTests) {
        ParsedCookie pc(std::string("aBc=") + test.input + " ; path=\"/\"  ; httponly ");
...
{
    ParsedCookie pc("BLAH; Path=/; sECuRe; httpONLY; sAmESitE=LaX; pRIoRitY=hIgH; "
                    "pARTitIoNeD;");
    EXPECT_TRUE(pc.IsValid());
    EXPECT_TRUE(pc.IsSecure());
    EXPECT_TRUE(pc.IsHttpOnly());
...
TEST(ParsedCookieTest, EnforceSizeConstraints)
{
    ...
    EXPECT_TRUE(status.HasOnlyExclusionReason(CookieInclusionStatus::ExclusionReason::EXCLUDE_NAME_VALUE_PAIR_EXCEEDS_MAX_SIZE));
    ...
    EXPECT_TRUE(status.HasWarningReason(CookieInclusionStatus::WARN_ATTRIBUTE_VALUE_EXCEEDS_MAX_SIZE));
    ...
    std::string padded_test_domain = lots_of_spaces + test_domain + lots_of_spaces;
...
TEST(ParsedCookieTest, EmbeddedTerminator)
{
    ...
    EXPECT_FALSE(pc3.IsValid());
    EXPECT_TRUE(status1.HasOnlyExclusionReason(CookieInclusionStatus::ExclusionReason::EXCLUDE_DISALLOWED_CHARACTER));
    EXPECT_TRUE(status2.HasOnlyExclusionReason(CookieInclusionStatus::ExclusionReason::EXCLUDE_DISALLOWED_CHARACTER));
    EXPECT_TRUE(status3.HasOnlyExclusionReason(CookieInclusionStatus::ExclusionReason::EXCLUDE_DISALLOWED_CHARACTER));
}
...
TEST(ParsedCookieTest, ParseTokensAndValues)
{
    ...
    EXPECT_EQ("hello world\tgood", ParsedCookie::ParseTokenString("hello world\tgood\rbye"));
    ...
    EXPECT_EQ("hello world\tgood", ParsedCookie::ParseValueString("hello world\tgood\rbye"));
...
TEST(ParsedCookieTest, ToCookieLineSpecialTokens)
{
    ...
    {
        ParsedCookie pc("partitioned; partitioned; secure; httponly; httponly; secure");
        EXPECT_EQ("", pc.Name());
        EXPECT_EQ("partitioned", pc.Value());
        EXPECT_TRUE(pc.IsPartitioned());
    }
...
TEST(ParsedCookieTest, SameSiteValues)
{
    struct TestCase {
        ...
        bool valid;
        CookieSameSite mode;
    } cases[] { { "n=v; samesite=strict", true, CookieSameSite::STRICT_MODE }, { "n=v; samesite=lax", true, CookieSameSite::LAX_MODE },
        { "n=v; samesite=none", true, CookieSameSite::NO_RESTRICTION }, { "n=v; samesite=boo", true, CookieSameSite::UNSPECIFIED },
        { "n=v; samesite", true, CookieSameSite::UNSPECIFIED }, { "n=v", true, CookieSameSite::UNSPECIFIED } };
...
TEST(ParsedCookieTest, InvalidNonAlphanumericChars)
{
    ...
    for (size_t i = 0; i < std::size(cases); i++) {
        SCOPED_TRACE(testing::Message() << "Test case #" << base::NumberToString(i + 1));
        CookieInclusionStatus status;
        ParsedCookie pc(cases[i], &status);
        EXPECT_FALSE(pc.IsValid());
        EXPECT_TRUE(status.HasOnlyExclusionReason(CookieInclusionStatus::ExclusionReason::EXCLUDE_DISALLOWED_CHARACTER));
    }
}
...
TEST(ParsedCookieTest, PreviouslyTruncatingCharInCookieLine)
{
    ...
    } kTests[] = { { '\x0', true }, { '\xD', true }, { '\xA', true }, { '\x9', false }, { '\x1B', false } };

    for (const auto& test : kTests) {
        SCOPED_TRACE(testing::Message() << "Using test.ctlChar == " << base::NumberToString(test.ctlChar));
        std::string ctl_string(1, test.ctlChar);
        std::string ctl_at_start_cookie_string = base::StrCat({ ctl_string, "foo=bar" });
        ...
        std::string ctl_at_middle_cookie_string = base::StrCat({ "foo=bar;", ctl_string, "secure" });
        ...
        std::string ctl_at_end_cookie_string = base::StrCat({ "foo=bar;", "secure;", ctl_string });
