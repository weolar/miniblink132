...
public:
    ...
    public:
        ...
        // to this method.
        SerializedCookiePartitionKey(base::PassKey<CookiePartitionKey> key, const std::string& site, bool has_cross_site_ancestor);
    ...
    [[nodiscard]] static base::expected<SerializedCookiePartitionKey, std::string> Serialize(const std::optional<CookiePartitionKey>& in);

    static CookiePartitionKey FromURLForTesting(
        const GURL& url, AncestorChainBit ancestor_chain_bit = AncestorChainBit::kCrossSite, std::optional<base::UnguessableToken> nonce = std::nullopt)
    {
    ...
    static std::optional<CookiePartitionKey> FromNetworkIsolationKey(const NetworkIsolationKey& network_isolation_key, const SiteForCookies& site_for_cookies,
        const SchemefulSite& request_site, bool main_frame_navigation);
    ...
    static CookiePartitionKey FromWire(
        const SchemefulSite& site, AncestorChainBit ancestor_chain_bit, std::optional<base::UnguessableToken> nonce = std::nullopt)
    {
    ...
    [[nodiscard]] static std::optional<CookiePartitionKey> FromStorageKeyComponents(
        const SchemefulSite& top_level_site, AncestorChainBit ancestor_chain_bit, const std::optional<base::UnguessableToken>& nonce);
    ...
    // storage.
    [[nodiscard]] static base::expected<std::optional<CookiePartitionKey>, std::string> FromStorage(
        const std::string& top_level_site, bool has_cross_site_ancestor);
    ...
    [[nodiscard]] static base::expected<CookiePartitionKey, std::string> FromUntrustedInput(const std::string& top_level_site, bool has_cross_site_ancestor);
...
private:
    ...
    explicit CookiePartitionKey(const SchemefulSite& site, std::optional<base::UnguessableToken> nonce, AncestorChainBit ancestor_chain_bit);
    ...
    [[nodiscard]] static base::expected<CookiePartitionKey, std::string> DeserializeInternal(
    ...
    bool ancestor_chain_enabled_ = base::FeatureList::IsEnabled(features::kAncestorChainBitEnabledInPartitionedCookies);
...
NET_EXPORT std::ostream& operator<<(std::ostream& os, const CookiePartitionKey& cpk);
