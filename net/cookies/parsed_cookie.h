...
public:
    ...
    // is valid.
    explicit ParsedCookie(std::string_view cookie_line, CookieInclusionStatus* status_out = nullptr);
    ...
    {
        return pairs_[0].second;
    }

    bool HasPath() const
    {
        return path_index_ != 0;
    }
    ...
    bool IsSecure() const
    {
        return secure_index_ != 0;
    }
    bool IsHttpOnly() const
    {
        return httponly_index_ != 0;
    }
    // Also spits out an enum value representing the string given as the SameSite
    // attribute value, if |samesite_string| is non-null.
    CookieSameSite SameSite(CookieSameSiteString* samesite_string = nullptr) const;
    CookiePriority Priority() const;
    bool IsPartitioned() const
    {
        return partitioned_index_ != 0;
    }
    bool HasInternalHtab() const
    {
        return internal_htab_;
    }
    ...
    static bool IsValidCookieNameValuePair(const std::string& name, const std::string& value, CookieInclusionStatus* status_out = nullptr);

private:
    void ParseTokenValuePairs(std::string_view cookie_line, CookieInclusionStatus& status_out);
    ...
    bool SetString(size_t* index, const std::string& key, const std::string& value);
    ...
    bool SetAttributePair(size_t* index, const std::string& key, const std::string& value);
