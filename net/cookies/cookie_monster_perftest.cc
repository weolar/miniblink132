...
perf_test::PerfResultReporter SetUpCookieMonsterReporter(const std::string& story)
{
...
public:
    void SetCookie(CookieMonster* cm, const GURL& gurl, const std::string& cookie_line)
    {
        auto cookie = CanonicalCookie::CreateForTesting(gurl, cookie_line, base::Time::Now());
        cm->SetCanonicalCookieAsync(std::move(cookie), gurl, options_, base::BindOnce(&SetCookieCallback::Run, base::Unretained(this)));
        WaitForCallback();
    }
...
private:
    ...
    {
        EXPECT_TRUE(result.status.IsInclude()) << "result.status: " << result.status.GetDebugString();
...
public:
    ...
    {
        cm->GetCookieListWithOptionsAsync(gurl, options_, CookiePartitionKeyCollection(), base::BindOnce(&GetCookieListCallback::Run, base::Unretained(this)));
        WaitForCallback();
        return cookie_list_;
    }

private:
    void Run(const CookieAccessResultList& cookie_list, const CookieAccessResultList& excluded_cookies)
    {
...
public:
    ...
    {
        cm->GetAllCookiesAsync(base::BindOnce(&GetAllCookiesCallback::Run, base::Unretained(this)));
        WaitForCallback();
        return cookies_;
    }
...
TEST_F(CookieMonsterTest, TestAddCookiesOnSingleHost)
{
    ...
    reporter.AddResult(kMetricQueryTimeMs, query_timer.Elapsed().InMillisecondsF());

    base::ElapsedTimer delete_all_timer;
    cm->DeleteAllAsync(CookieMonster::DeleteCallback());
    base::RunLoop().RunUntilIdle();
    reporter.AddResult(kMetricDeleteAllTimeMs, delete_all_timer.Elapsed().InMillisecondsF());
}
...
TEST_F(CookieMonsterTest, TestAddCookieOnManyHosts)
{
    ...
    base::ElapsedTimer add_timer;
    for (std::vector<GURL>::const_iterator it = gurls.begin(); it != gurls.end(); ++it) {
...
TEST_F(CookieMonsterTest, TestDomainTree)
{
    ...
    GetCookieListCallback getCookieListCallback;
    SetCookieCallback setCookieCallback;
    const char domain_cookie_format_tree[] = "a=b; domain=%s; samesite=none; secure";
    ...
    for (std::vector<std::string>::const_iterator it = domain_list.begin(); it != domain_list.end(); it++) {
        GURL gurl("https://" + *it + "/");
    ...
    const CookieList& cookie_list = getCookieListCallback.GetCookieList(cm.get(), probe_gurl);
    EXPECT_EQ(5u, cookie_list.size()) << CanonicalCookie::BuildCookieLine(cookie_list);
    ...
    reporter.AddResult(kMetricQueryDomainTimeMs, query_domain_timer.Elapsed().InMillisecondsF());
}
...
TEST_F(CookieMonsterTest, TestDomainLine)
{
    ...
    const char domain_cookie_format_line[] = "a%03d=b; domain=%s; samesite=none; secure";
    for (int i = 0; i < 8; i++) {
        for (std::vector<std::string>::const_iterator it = domain_list.begin(); it != domain_list.end(); it++) {
            GURL gurl("https://" + *it + "/");
...
TEST_F(CookieMonsterTest, TestImport)
{
    ...
    for (int domain_num = 0; domain_num < 300; domain_num++) {
        ...
        for (int cookie_num = 0; cookie_num < 50; cookie_num++) {
            std::string cookie_line(base::StringPrintf("Cookie_%d=1; Path=/", cookie_num));
            AddCookieToList(gurl, cookie_line, base::Time::FromInternalValue(time_tick++), &initial_cookies);
        }
    }
    ...
    reporter.AddResult(kMetricImportTimeMs, import_timer.Elapsed().InMillisecondsF());
...
TEST_F(CookieMonsterTest, TestGetKey)
{
    ...
    reporter.AddResult(kMetricGetKeyTimeMs, get_key_timer.Elapsed().InMillisecondsF());
}
...
TEST_F(CookieMonsterTest, TestGCTimes)
{
    ...
    for (const auto& test_case : test_cases) {
        std::unique_ptr<CookieMonster> cm
            = CreateMonsterFromStoreForGC(test_case.num_cookies, test_case.num_old_cookies, 0, 0, CookieMonster::kSafeFromGlobalPurgeDays * 2);
