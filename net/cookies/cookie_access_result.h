...
struct NET_EXPORT CookieAccessResult {
    ...
    // indicates inclusion.
    CookieAccessResult();
    CookieAccessResult(CookieEffectiveSameSite effective_same_site, CookieInclusionStatus status, CookieAccessSemantics access_semantics,
    ...
    {
        return status == other.status && effective_same_site == other.effective_same_site && access_semantics == other.access_semantics
            && is_allowed_to_access_secure_cookies == other.is_allowed_to_access_secure_cookies;
    }

    CookieInclusionStatus status;
    CookieEffectiveSameSite effective_same_site = CookieEffectiveSameSite::UNDEFINED;
...
inline void PrintTo(const CookieAccessResult& car, std::ostream* os)
{
    ...
    *os << " }, effective_same_site=" << static_cast<int>(car.effective_same_site) << ", access_semantics=" << static_cast<int>(car.access_semantics)
        << ", is_allowed_to_access_secure_cookies=" << car.is_allowed_to_access_secure_cookies << " }";
}
