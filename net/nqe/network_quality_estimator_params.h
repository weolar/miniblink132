...
NET_EXPORT extern const base::TimeDelta kHttpRttEffectiveConnectionTypeThresholds[EFFECTIVE_CONNECTION_TYPE_LAST];
...
public:
    ...
    explicit NetworkQualityEstimatorParams(const std::map<std::string, std::string>& params);

    NetworkQualityEstimatorParams(const NetworkQualityEstimatorParams&) = delete;
    NetworkQualityEstimatorParams& operator=(const NetworkQualityEstimatorParams&) = delete;
    ...
    const nqe::internal::NetworkQuality& DefaultObservation(NetworkChangeNotifier::ConnectionType type) const;

    // Returns the typical network quality for connection |type|.
    const nqe::internal::NetworkQuality& TypicalNetworkQuality(EffectiveConnectionType type) const;

    // Returns the threshold for effective connection type |type|.
    const nqe::internal::NetworkQuality& ConnectionThreshold(EffectiveConnectionType type) const;
    ...
    std::optional<EffectiveConnectionType> GetForcedEffectiveConnectionType(NetworkChangeNotifier::ConnectionType connection_type);

    void SetForcedEffectiveConnectionType(EffectiveConnectionType forced_effective_connection_type)
    {
    ...
    void set_persistent_cache_reading_enabled(bool persistent_cache_reading_enabled)
    {
        DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    static base::TimeDelta GetDefaultTypicalHttpRtt(EffectiveConnectionType effective_connection_type);

    // Returns the typical downslink throughput (in kbps) that maps to the given
    // |effective_connection_type|. May return invalid value if
    // |effective_connection_type| is less than Slow2G or faster than 4G,
    static int32_t GetDefaultTypicalDownlinkKbps(EffectiveConnectionType effective_connection_type);
    ...
    // ObservationBuffer.
    size_t observation_buffer_size() const
    {
        return observation_buffer_size_;
    }
    ...
    // quality estimate.
    bool use_end_to_end_rtt() const
    {
        return use_end_to_end_rtt_;
    }
...
private:
    ...
    const base::TimeDelta hanging_request_upper_bound_min_http_rtt_ = base::Milliseconds(500);
    ...
    const base::TimeDelta hanging_request_min_duration_ = base::Milliseconds(3000);
    ...
    nqe::internal::NetworkQuality default_observations_[NetworkChangeNotifier::CONNECTION_LAST + 1];

    // Typical network quality for different effective connection types obtained
    // from |params_|.
    nqe::internal::NetworkQuality typical_network_quality_[EffectiveConnectionType::EFFECTIVE_CONNECTION_TYPE_LAST];
    ...
    // in general.
    nqe::internal::NetworkQuality connection_thresholds_[EffectiveConnectionType::EFFECTIVE_CONNECTION_TYPE_LAST];
