...
TEST(EffectiveConnectionTypeTest, NameConnectionTypeConversion)
{
    ...
    EXPECT_FALSE(GetEffectiveConnectionTypeForName("InvalidEffectiveConnectionTypeName"));
    EXPECT_FALSE(GetEffectiveConnectionTypeForName(std::string()));

    for (size_t i = 0; i < EFFECTIVE_CONNECTION_TYPE_LAST; ++i) {
        const EffectiveConnectionType effective_connection_type = static_cast<EffectiveConnectionType>(i);
        std::string connection_type_name = std::string(GetNameForEffectiveConnectionType(effective_connection_type));
        ...
        if (effective_connection_type != EFFECTIVE_CONNECTION_TYPE_SLOW_2G) {
            ...
            EXPECT_EQ(connection_type_name, DeprecatedGetNameForEffectiveConnectionType(effective_connection_type));
        }

        EXPECT_EQ(effective_connection_type, GetEffectiveConnectionTypeForName(connection_type_name));
    }
}
...
TEST(EffectiveConnectionTypeTest, Slow2GTypeConversion)
{
    ...
    // representation.
    std::optional<EffectiveConnectionType> type = GetEffectiveConnectionTypeForName("Slow2G");
    EXPECT_EQ(EFFECTIVE_CONNECTION_TYPE_SLOW_2G, type.value());

    type = GetEffectiveConnectionTypeForName("Slow-2G");
    EXPECT_EQ(EFFECTIVE_CONNECTION_TYPE_SLOW_2G, type.value());

    EXPECT_EQ("Slow-2G", std::string(GetNameForEffectiveConnectionType(EFFECTIVE_CONNECTION_TYPE_SLOW_2G)));
    EXPECT_EQ("Slow2G", std::string(DeprecatedGetNameForEffectiveConnectionType(EFFECTIVE_CONNECTION_TYPE_SLOW_2G)));
}
