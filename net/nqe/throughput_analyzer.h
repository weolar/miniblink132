...
public:
    ...
    ThroughputAnalyzer(const NetworkQualityEstimator* network_quality_estimator, const NetworkQualityEstimatorParams* params,
        scoped_refptr<base::SingleThreadTaskRunner> task_runner, ThroughputObservationCallback throughput_observation_callback,
        const base::TickClock* tick_clock, const NetLogWithSource& net_log);
    ...
    void NotifyExpectedResponseContentSize(const URLRequest& request, int64_t expected_content_size);
...
private:
    ...
    // computation.
    typedef std::unordered_set<raw_ptr<const URLRequest, CtnExperimental>> AccuracyDegradingRequests;
    ...
    void UpdateResponseContentSize(const URLRequest* request, int64_t response_size);
