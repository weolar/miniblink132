...
Observation::Observation(int32_t value, base::TimeTicks timestamp, int32_t signal_strength, NetworkQualityObservationSource source)
    : Observation(value, timestamp, signal_strength, source, std::nullopt)
{
}

Observation::Observation(
    int32_t value, base::TimeTicks timestamp, int32_t signal_strength, NetworkQualityObservationSource source, const std::optional<IPHash>& host)
    : value_(value)
    ...
    , host_(host)
{
    ...
    DCHECK(signal_strength_ == INT32_MIN || (signal_strength_ >= 0 && signal_strength_ <= 4));
}
...
std::vector<ObservationCategory> Observation::GetObservationCategories() const
{
    ...
    case DEPRECATED_NETWORK_QUALITY_OBSERVATION_SOURCE_HTTP_EXTERNAL_ESTIMATE:
        observation_categories.push_back(ObservationCategory::OBSERVATION_CATEGORY_HTTP);
    ...
    case NETWORK_QUALITY_OBSERVATION_SOURCE_TCP:
        observation_categories.push_back(ObservationCategory::OBSERVATION_CATEGORY_TRANSPORT);
        return observation_categories;
    case NETWORK_QUALITY_OBSERVATION_SOURCE_QUIC:
    case NETWORK_QUALITY_OBSERVATION_SOURCE_H2_PINGS:
        observation_categories.push_back(ObservationCategory::OBSERVATION_CATEGORY_TRANSPORT);
        observation_categories.push_back(ObservationCategory::OBSERVATION_CATEGORY_END_TO_END);
        return observation_categories;
