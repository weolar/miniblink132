...
{
    static_assert(kMaximumNetworkQualityCacheSize > 0, "Size of the network quality cache must be > 0");
    ...
    static_assert(kMaximumNetworkQualityCacheSize <= 20, "Size of the network quality cache must <= 20");
}
...
void NetworkQualityStore::Add(const nqe::internal::NetworkID& network_id, const nqe::internal::CachedNetworkQuality& cached_network_quality)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK_LE(cached_network_qualities_.size(), static_cast<size_t>(kMaximumNetworkQualityCacheSize));

    if (cached_network_quality.effective_connection_type() == EFFECTIVE_CONNECTION_TYPE_UNKNOWN) {
        return;
    }
    ...
    if (cached_network_qualities_.size() == kMaximumNetworkQualityCacheSize) {
        ...
        for (auto it = cached_network_qualities_.begin(); it != cached_network_qualities_.end(); ++it) {
...
bool NetworkQualityStore::GetById(const nqe::internal::NetworkID& network_id, nqe::internal::CachedNetworkQuality* cached_network_quality) const
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);

    // First check if an exact match can be found.
    for (const auto& cached_quality : cached_network_qualities_) {
        if (network_id.type != cached_quality.first.type || network_id.id != cached_quality.first.id) {
    ...
    if (network_id.signal_strength == INT32_MIN) {
        ...
        for (auto it = cached_network_qualities_.begin(); it != cached_network_qualities_.end(); ++it) {
            ...
            if (matching_it == cached_network_qualities_.end() || it->first.signal_strength > matching_it->first.signal_strength) {
                matching_it = it;
            }
        }
    ...
    for (auto it = cached_network_qualities_.begin(); it != cached_network_qualities_.end(); ++it) {
        ...
        if (it->first.signal_strength == INT32_MIN) {
            ...
            diff_signal_strength = std::abs(network_id.signal_strength - it->first.signal_strength);
        }

        if (matching_it == cached_network_qualities_.end() || diff_signal_strength < matching_it_diff_signal_strength) {
            matching_it = it;
...
void NetworkQualityStore::AddNetworkQualitiesCacheObserver(NetworkQualitiesCacheObserver* observer)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
        FROM_HERE, base::BindOnce(&NetworkQualityStore::NotifyCacheObserverIfPresent, weak_ptr_factory_.GetWeakPtr(), base::UnsafeDangling(observer)));
}

void NetworkQualityStore::RemoveNetworkQualitiesCacheObserver(NetworkQualitiesCacheObserver* observer)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    network_qualities_cache_observer_list_.RemoveObserver(observer);
}

void NetworkQualityStore::NotifyCacheObserverIfPresent(MayBeDangling<NetworkQualitiesCacheObserver> observer) const
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
