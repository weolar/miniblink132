...
typedef std::map<nqe::internal::NetworkID, nqe::internal::CachedNetworkQuality> ParsedPrefs;
...
class NET_EXPORT NetworkQualitiesPrefsManager : public nqe::internal::NetworkQualityStore::NetworkQualitiesCacheObserver {
public:
    ...
    explicit NetworkQualitiesPrefsManager(std::unique_ptr<PrefDelegate> pref_delegate);

    NetworkQualitiesPrefsManager(const NetworkQualitiesPrefsManager&) = delete;
    NetworkQualitiesPrefsManager& operator=(const NetworkQualitiesPrefsManager&) = delete;
    ...
    void InitializeOnNetworkThread(NetworkQualityEstimator* network_quality_estimator);
