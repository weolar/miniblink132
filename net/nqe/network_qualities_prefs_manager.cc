...
ParsedPrefs ConvertDictionaryValueToMap(const base::Value::Dict& value)
{
    ...
    for (auto it : value) {
        nqe::internal::NetworkID network_id = nqe::internal::NetworkID::FromString(it.first);

        if (!it.second.is_string())
            continue;
        std::optional<EffectiveConnectionType> effective_connection_type = GetEffectiveConnectionTypeForName(it.second.GetString());
        DCHECK(effective_connection_type.has_value());

        nqe::internal::CachedNetworkQuality cached_network_quality(effective_connection_type.value_or(EFFECTIVE_CONNECTION_TYPE_UNKNOWN));
...
NetworkQualitiesPrefsManager::NetworkQualitiesPrefsManager(std::unique_ptr<PrefDelegate> pref_delegate)
...
void NetworkQualitiesPrefsManager::InitializeOnNetworkThread(NetworkQualityEstimator* network_quality_estimator)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    const nqe::internal::NetworkID& network_id, const nqe::internal::CachedNetworkQuality& cached_network_quality)
{
    ...
    prefs_.Set(network_id_string, GetNameForEffectiveConnectionType(cached_network_quality.effective_connection_type()));
