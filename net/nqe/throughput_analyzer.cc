...
ThroughputAnalyzer::ThroughputAnalyzer(const NetworkQualityEstimator* network_quality_estimator, const NetworkQualityEstimatorParams* params,
    scoped_refptr<base::SingleThreadTaskRunner> task_runner, ThroughputObservationCallback throughput_observation_callback, const base::TickClock* tick_clock,
...
void ThroughputAnalyzer::MaybeStartThroughputObservationWindow()
{
    ...
    if (accuracy_degrading_requests_.size() > 0 || IsCurrentlyTrackingThroughput() || requests_.size() < params_->throughput_min_requests_in_flight()) {
        return;
    }
...
void ThroughputAnalyzer::SetTickClockForTesting(const base::TickClock* tick_clock)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    tick_clock_ = tick_clock;
    DCHECK(tick_clock_);
...
void ThroughputAnalyzer::UpdateResponseContentSize(const URLRequest* request, int64_t response_size)
{
    ...
    if (response_content_sizes_.find(request) != response_content_sizes_.end()) {
        total_response_content_size_ += response_size - response_content_sizes_[request];
    } else {
...
void ThroughputAnalyzer::NotifyRequestCompleted(const URLRequest& request)
{
    ...
    if (MaybeGetThroughputObservation(&downstream_kbps)) {
        ...
        task_runner_->PostTask(FROM_HERE, base::BindOnce(throughput_observation_callback_, downstream_kbps));
    }
...
void ThroughputAnalyzer::NotifyExpectedResponseContentSize(const URLRequest& request, int64_t expected_content_size)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
bool ThroughputAnalyzer::IsHangingWindow(int64_t bits_received, base::TimeDelta duration) const
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    size_t bits_received_over_one_http_rtt = bits_received * (network_quality_estimator_->GetHttpRTT().value_or(base::Seconds(10)) / duration);
    ...
    bool is_hanging = bits_received_over_one_http_rtt < (kCwndSizeBits * params_->throughput_hanging_requests_cwnd_size_multiplier());

    return is_hanging;
}

bool ThroughputAnalyzer::MaybeGetThroughputObservation(int32_t* downstream_kbps)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    if (!params_->use_small_responses() && bits_received < params_->GetThroughputMinTransferSizeBits()) {
        return false;
    }
...
void ThroughputAnalyzer::SetUseLocalHostRequestsForTesting(bool use_localhost_requests)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
bool ThroughputAnalyzer::DegradesAccuracy(const URLRequest& request) const
{
    ...
    return !(use_localhost_requests_for_tests_ || !private_network_request) || request.creation_time() < last_connection_change_;
}
...
void ThroughputAnalyzer::EraseHangingRequests(const URLRequest& request)
{
    ...
    const base::TimeDelta http_rtt = network_quality_estimator_->GetHttpRTT().value_or(base::Seconds(60));
    ...
    if (request_it != requests_.end()) {
        ...
        if (time_since_last_received >= params_->hanging_request_duration_http_rtt_multiplier() * http_rtt
