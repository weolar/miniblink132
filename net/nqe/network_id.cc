...
NetworkID NetworkID::FromString(const std::string& network_id)
{
    ...
    if (!base::Base64Decode(network_id, &base64_decoded)) {
        return NetworkID(NetworkChangeNotifier::CONNECTION_UNKNOWN, std::string(), INT32_MIN);
    }
    ...
    return NetworkID(
        static_cast<NetworkChangeNotifier::ConnectionType>(network_id_proto.connection_type()), network_id_proto.id(), network_id_proto.signal_strength());
}

NetworkID::NetworkID(NetworkChangeNotifier::ConnectionType type, const std::string& id, int32_t signal_strength)
    : type(type)
    , id(id)
    , signal_strength(signal_strength)
{
    ...
    // inclusive).
    DCHECK((0 <= signal_strength && 4 >= signal_strength) || (INT32_MIN == signal_strength));
}
...
{
    return type == other.type && id == other.id && signal_strength == other.signal_strength;
}
...
bool NetworkID::operator<(const NetworkID& other) const
{
    return std::tie(type, id, signal_strength) < std::tie(other.type, other.id, other.signal_strength);
}
