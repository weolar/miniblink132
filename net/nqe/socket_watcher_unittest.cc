...
public:
    NetworkQualitySocketWatcherTest(const NetworkQualitySocketWatcherTest&) = delete;
    NetworkQualitySocketWatcherTest& operator=(const NetworkQualitySocketWatcherTest&) = delete;

protected:
    NetworkQualitySocketWatcherTest()
    {
        ResetExpectedCallbackParams();
    }
    ~NetworkQualitySocketWatcherTest() override
    {
        ResetExpectedCallbackParams();
    }

    static void OnUpdatedRTTAvailableStoreParams(
        SocketPerformanceWatcherFactory::Protocol protocol, const base::TimeDelta& rtt, const std::optional<IPHash>& host)
    {
    ...
    static void OnUpdatedRTTAvailable(SocketPerformanceWatcherFactory::Protocol protocol, const base::TimeDelta& rtt, const std::optional<IPHash>& host)
    {
        // Need to verify before another callback is executed, or explicitly call
        // |ResetCallbackParams()|.
        ASSERT_FALSE(callback_executed_);
    ...
    static void VerifyCallbackParams(const base::TimeDelta& rtt, const std::optional<IPHash>& host)
    {
    ...
    static base::TimeDelta callback_rtt()
    {
        return callback_rtt_;
    }
...
base::TimeDelta NetworkQualitySocketWatcherTest::callback_rtt_ = base::Milliseconds(0);

std::optional<IPHash> NetworkQualitySocketWatcherTest::callback_host_ = std::nullopt;
...
TEST_F(NetworkQualitySocketWatcherTest, NotificationsThrottled)
{
    ...
    SocketWatcher socket_watcher(SocketPerformanceWatcherFactory::PROTOCOL_TCP, ip_address, base::Milliseconds(2000), false,
        base::SingleThreadTaskRunner::GetCurrentDefault(), base::BindRepeating(OnUpdatedRTTAvailable), base::BindRepeating(ShouldNotifyRTTCallback),
        &tick_clock);
...
TEST_F(NetworkQualitySocketWatcherTest, QuicFirstNotificationDropped)
{
    ...
    SocketWatcher socket_watcher(SocketPerformanceWatcherFactory::PROTOCOL_QUIC, ip_address, base::Milliseconds(2000), false,
        base::SingleThreadTaskRunner::GetCurrentDefault(), base::BindRepeating(OnUpdatedRTTAvailableStoreParams), base::BindRepeating(ShouldNotifyRTTCallback),
        &tick_clock);
...
TEST_F(NetworkQualitySocketWatcherTest, MAYBE_PrivateAddressRTTNotNotified)
{
    ...
    for (const auto& test : tests) {
        ...
        EXPECT_EQ(test.expect_should_notify_rtt, socket_watcher.ShouldNotifyUpdatedRTT());
        socket_watcher.OnUpdatedRTTAvailable(base::Seconds(10));
        base::RunLoop().RunUntilIdle();
        ResetExpectedCallbackParams();
...
TEST_F(NetworkQualitySocketWatcherTest, RemoteHostIPHashComputedCorrectly)
{
    ...
    const struct {
        ...
        { "112.112.112.250", 0x00000000707070faUL }, { "2001:0db8:85a3:0000:0000:8a2e:0370:7334", 0x20010db885a30000UL }, // IPv6.
    ...
    for (const auto& test : tests) {
        ...
        SocketWatcher socket_watcher(SocketPerformanceWatcherFactory::PROTOCOL_TCP, ip_address, base::Milliseconds(2000), false,
            base::SingleThreadTaskRunner::GetCurrentDefault(), base::BindRepeating(OnUpdatedRTTAvailableStoreParams),
