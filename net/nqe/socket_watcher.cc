...
SocketWatcher::SocketWatcher(SocketPerformanceWatcherFactory::Protocol protocol, const IPAddress& address, base::TimeDelta min_notification_interval,
    bool allow_rtt_private_address, scoped_refptr<base::SingleThreadTaskRunner> task_runner, OnUpdatedRTTAvailableCallback updated_rtt_observation_callback,
    ShouldNotifyRTTCallback should_notify_rtt_callback, const base::TickClock* tick_clock)
    : protocol_(protocol)
    ...
    , run_rtt_callback_(allow_rtt_private_address || address.IsPubliclyRoutable())
...
void SocketWatcher::OnUpdatedRTTAvailable(const base::TimeDelta& rtt)
{
    ...
    if (!first_quic_rtt_notification_received_ && protocol_ == SocketPerformanceWatcherFactory::PROTOCOL_QUIC) {
    ...
    task_runner_->PostTask(FROM_HERE, base::BindOnce(updated_rtt_observation_callback_, protocol_, rtt, host_));
}
