...
NetworkQuality::NetworkQuality(const base::TimeDelta& http_rtt, const base::TimeDelta& transport_rtt, int32_t downstream_throughput_kbps)
    : http_rtt_(http_rtt)
...
NetworkQuality::NetworkQuality(const NetworkQuality& other)
    : NetworkQuality(other.http_rtt_, other.transport_rtt_, other.downstream_throughput_kbps_)
{
    VerifyValueCorrectness();
    DETACH_FROM_SEQUENCE(sequence_checker_);
}
...
bool NetworkQuality::operator==(const NetworkQuality& other) const
{
    ...
    return http_rtt_ == other.http_rtt_ && transport_rtt_ == other.transport_rtt_ && downstream_throughput_kbps_ == other.downstream_throughput_kbps_;
}

bool NetworkQuality::IsFaster(const NetworkQuality& other) const
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    return (http_rtt() == InvalidRTT() || other.http_rtt() == InvalidRTT() || http_rtt() <= other.http_rtt())
        && (transport_rtt() == InvalidRTT() || other.transport_rtt() == InvalidRTT() || transport_rtt() <= other.transport_rtt())
        && (downstream_throughput_kbps() == INVALID_RTT_THROUGHPUT || other.downstream_throughput_kbps() == INVALID_RTT_THROUGHPUT
