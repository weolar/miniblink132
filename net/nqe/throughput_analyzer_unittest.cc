...
std::unique_ptr<HostResolver> CreateMockHostResolver()
{
    ...
    host_resolver->LoadIntoCache(url::SchemeHostPort("http", "local.com", 80), NetworkAnonymizationKey(), std::nullopt);
...
public:
    TestThroughputAnalyzer(NetworkQualityEstimator* network_quality_estimator, NetworkQualityEstimatorParams* params, const base::TickClock* tick_clock)
        : internal::ThroughputAnalyzer(network_quality_estimator, params, base::SingleThreadTaskRunner::GetCurrentDefault(),
            base::BindRepeating(&TestThroughputAnalyzer::OnNewThroughputObservationAvailable, base::Unretained(this)), tick_clock,
    ...
    int64_t GetBitsReceived() const override
    {
        return bits_received_;
    }
    ...
    using internal::ThroughputAnalyzer::disable_throughput_measurements_for_testing;
...
TEST_F(ThroughputAnalyzerTest, PrivateHost)
{
    ...
    EXPECT_FALSE(nqe::internal::IsPrivateHostForTesting(host_resolver.get(), url::SchemeHostPort("http", "example.com", 80), NetworkAnonymizationKey()));
    EXPECT_TRUE(nqe::internal::IsPrivateHostForTesting(host_resolver.get(), url::SchemeHostPort("http", "local.com", 80), NetworkAnonymizationKey()));
}
...
TEST_F(ThroughputAnalyzerTest, MAYBE_MaximumRequests)
{
    ...
    for (const auto& test_case : kTestCases) {
        ...
        TestThroughputAnalyzer throughput_analyzer(&network_quality_estimator, &params, tick_clock);
        ...
        ASSERT_FALSE(throughput_analyzer.disable_throughput_measurements_for_testing());
        ...
        EXPECT_EQ(test_case.is_local,
            nqe::internal::IsPrivateHostForTesting(context->host_resolver(), url::SchemeHostPort(test_case.url), NetworkAnonymizationKey()));
        for (size_t i = 0; i < 1000; ++i) {
            std::unique_ptr<URLRequest> request(context->CreateRequest(test_case.url, DEFAULT_PRIORITY, &test_delegate, TRAFFIC_ANNOTATION_FOR_TESTS));
        ...
        EXPECT_NE(test_case.is_local, throughput_analyzer.IsCurrentlyTrackingThroughput());
    }
}

#if BUILDFLAG(IS_IOS)
// Flaky on iOS: crbug.com/672917.
#define MAYBE_MaximumRequestsWithNetworkAnonymizationKey DISABLED_MaximumRequestsWithNetworkAnonymizationKey
#else
#define MAYBE_MaximumRequestsWithNetworkAnonymizationKey MaximumRequestsWithNetworkAnonymizationKey
#endif
// Make sure that the NetworkAnonymizationKey is respected when resolving a host
// from the cache.
TEST_F(ThroughputAnalyzerTest, MAYBE_MaximumRequestsWithNetworkAnonymizationKey)
{
    ...
    for (bool use_network_isolation_key : { false, true }) {
        ...
        mock_host_resolver->LoadIntoCache(url::SchemeHostPort(kUrl), NetworkAnonymizationKey(), std::nullopt);
        ...
        mock_host_resolver->LoadIntoCache(url::SchemeHostPort(kUrl), kNetworkAnonymizationKey, std::nullopt);
        ...
        EXPECT_EQ(use_network_isolation_key,
            nqe::internal::IsPrivateHostForTesting(
                context->host_resolver(), url::SchemeHostPort(kUrl), use_network_isolation_key ? kNetworkAnonymizationKey : NetworkAnonymizationKey()));
        for (size_t i = 0; i < 1000; ++i) {
            std::unique_ptr<URLRequest> request(context->CreateRequest(kUrl, DEFAULT_PRIORITY, &test_delegate, TRAFFIC_ANNOTATION_FOR_TESTS));
            if (use_network_isolation_key) {
                request->set_isolation_info(
                    net::IsolationInfo::Create(net::IsolationInfo::RequestType::kOther, kSiteOrigin, kSiteOrigin, net::SiteForCookies()));
            }
            throughput_analyzer.NotifyStartTransaction(*(request.get()));
            requests.push_back(std::move(request));
        }
        // Too many local requests should cause the |throughput_analyzer| to disable
        // throughput measurements.
        EXPECT_NE(use_network_isolation_key, throughput_analyzer.IsCurrentlyTrackingThroughput());
    }
}
...
TEST_F(ThroughputAnalyzerTest, TestMinRequestsForThroughputSample)
{
    ...
    for (size_t num_requests = 1; num_requests <= params.throughput_min_requests_in_flight() + 1; ++num_requests) {
        TestThroughputAnalyzer throughput_analyzer(&network_quality_estimator, &params, tick_clock);
        auto context_builder = CreateTestURLRequestContextBuilder();
        context_builder->set_host_resolver(CreateMockHostResolver());
        auto context = context_builder->Build();
        ...
        for (auto& delegate : not_local_test_delegates) {
            ...
            std::unique_ptr<URLRequest> request_not_local(
                context->CreateRequest(GURL("http://example.com/echo.html"), DEFAULT_PRIORITY, &delegate, TRAFFIC_ANNOTATION_FOR_TESTS));
        ...
        int expected_throughput_observations = num_requests >= params.throughput_min_requests_in_flight() ? 1 : 0;
        EXPECT_EQ(expected_throughput_observations, throughput_analyzer.throughput_observations_received());
    }
}
...
TEST_F(ThroughputAnalyzerTest, TestHangingRequests)
{
    ...
    for (const auto& test : tests) {
        ...
        variation_params["hanging_request_http_rtt_upper_bound_transport_rtt_multiplier"] = "10000";
        variation_params["throughput_hanging_requests_cwnd_size_multiplier"] = "-1";
        variation_params["hanging_request_duration_http_rtt_multiplier"] = base::NumberToString(test.hanging_request_duration_http_rtt_multiplier);
        ...
        for (size_t i = 0; i < num_requests; ++i) {
            ...
            std::unique_ptr<URLRequest> request_not_local(
                context->CreateRequest(GURL("http://example.com/echo.html"), DEFAULT_PRIORITY, &not_local_test_delegates[i], TRAFFIC_ANNOTATION_FOR_TESTS));
            request_not_local->Start();
            requests_not_local.push_back(std::move(request_not_local));
        }
        ...
        for (size_t i = 0; i < num_requests; ++i) {
            ...
            if (!test.expect_throughput_observation) {
                ...
                EXPECT_EQ(requests_not_local.size() - i - 1, throughput_analyzer.CountActiveInFlightRequests());
            }
        }

        base::RunLoop().RunUntilIdle();

        EXPECT_EQ(test.expect_throughput_observation, throughput_analyzer.throughput_observations_received() > 0);
    }
}
...
TEST_F(ThroughputAnalyzerTest, TestHangingRequestsCheckedOnlyPeriodically)
{
    ...
    for (size_t i = 0; i < 2; ++i) {
        std::unique_ptr<URLRequest> request_not_local(
            context->CreateRequest(GURL("http://example.com/echo.html"), DEFAULT_PRIORITY, &test_delegate, TRAFFIC_ANNOTATION_FOR_TESTS));
        request_not_local->Start();
        requests_not_local.push_back(std::move(request_not_local));
    }

    std::unique_ptr<URLRequest> some_other_request(
        context->CreateRequest(GURL("http://example.com/echo.html"), DEFAULT_PRIORITY, &test_delegate, TRAFFIC_ANNOTATION_FOR_TESTS));
...
#if BUILDFLAG(IS_IOS)
// Flaky on iOS: crbug.com/672917.
#define MAYBE_TestThroughputWithMultipleRequestsOverlap DISABLED_TestThroughputWithMultipleRequestsOverlap
#else
#define MAYBE_TestThroughputWithMultipleRequestsOverlap TestThroughputWithMultipleRequestsOverlap
#endif
// Tests if the throughput observation is taken correctly when local and network
// requests overlap.
TEST_F(ThroughputAnalyzerTest, MAYBE_TestThroughputWithMultipleRequestsOverlap)
{
    static const struct {
    ...
    for (const auto& test : tests) {
        ...
        // TestDelegates must be before URLRequests that point to them.
        std::vector<TestDelegate> not_local_test_delegates(params.throughput_min_requests_in_flight());
        std::vector<std::unique_ptr<URLRequest>> requests_not_local;
        ...
        if (test.start_local_request) {
            request_local = context->CreateRequest(GURL("http://127.0.0.1/echo.html"), DEFAULT_PRIORITY, &local_delegate, TRAFFIC_ANNOTATION_FOR_TESTS);
            request_local->Start();
        }
...
TEST_F(ThroughputAnalyzerTest, TestThroughputWithNetworkRequestsOverlap)
{
    ...
    for (const auto& test : tests) {
        ...
        // Localhost requests are not allowed for estimation purposes.
        std::map<std::string, std::string> variation_params;
        variation_params["throughput_min_requests_in_flight"] = base::NumberToString(test.throughput_min_requests_in_flight);
        variation_params["throughput_hanging_requests_cwnd_size_multiplier"] = "-1";
        NetworkQualityEstimatorParams params(variation_params);
        // Set HTTP RTT to a large value so that the throughput observation window
        // is not detected as hanging. In practice, this would be provided by
        ...
        // TestDelegates must be before URLRequests that point to them.
        std::vector<TestDelegate> in_flight_test_delegates(test.number_requests_in_flight);
        ...
        for (size_t i = 0; i < test.number_requests_in_flight; ++i) {
            ...
            std::unique_ptr<URLRequest> request_network_1
                = context->CreateRequest(GURL("http://example.com/echo.html"), DEFAULT_PRIORITY, &in_flight_test_delegates[i], TRAFFIC_ANNOTATION_FOR_TESTS);
...
TEST_F(ThroughputAnalyzerTest, TestThroughputWithMultipleNetworkRequests)
{
    ...
    std::unique_ptr<URLRequest> request_1
        = context->CreateRequest(GURL("http://example.com/echo.html"), DEFAULT_PRIORITY, &test_delegate, TRAFFIC_ANNOTATION_FOR_TESTS);
    std::unique_ptr<URLRequest> request_2
        = context->CreateRequest(GURL("http://example.com/echo.html"), DEFAULT_PRIORITY, &test_delegate, TRAFFIC_ANNOTATION_FOR_TESTS);
    std::unique_ptr<URLRequest> request_3
        = context->CreateRequest(GURL("http://example.com/echo.html"), DEFAULT_PRIORITY, &test_delegate, TRAFFIC_ANNOTATION_FOR_TESTS);
    std::unique_ptr<URLRequest> request_4
        = context->CreateRequest(GURL("http://example.com/echo.html"), DEFAULT_PRIORITY, &test_delegate, TRAFFIC_ANNOTATION_FOR_TESTS);
...
TEST_F(ThroughputAnalyzerTest, TestHangingWindow)
{
    ...
    network_quality_estimator.SetStartTimeNullHttpRtt(base::Milliseconds(http_rtt_msec));
    std::map<std::string, std::string> variation_params;
    variation_params["throughput_hanging_requests_cwnd_size_multiplier"] = "1";
    NetworkQualityEstimatorParams params(variation_params);
    ...
    for (const auto& test : tests) {
        base::HistogramTester histogram_tester;
        EXPECT_EQ(test.expected_hanging, throughput_analyzer.IsHangingWindow(test.bits_received, test.window_duration));
    }
}
