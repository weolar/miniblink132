...
template <typename Id, typename Priority> void ScheduleIds(BTreeScheduler<Id, Priority>& scheduler, absl::Span<const Id> ids)
{
    for (Id id : ids) {
...
template <typename Id, typename Priority> std::vector<Id> PopAll(BTreeScheduler<Id, Priority>& scheduler)
{
...
TEST(BTreeSchedulerTest, NumEntriesInRange)
{
    ...
    EXPECT_EQ(scheduler.NumScheduled(), 0u);
    EXPECT_EQ(scheduler.NumScheduledInPriorityRange(std::nullopt, std::nullopt), 0u);
    ...
    EXPECT_EQ(scheduler.NumScheduledInPriorityRange(std::nullopt, std::nullopt), 9u);
...
TEST(BTreeSchedulerTest, Registration)
{
    ...
    EXPECT_THAT(scheduler.Register(2, 0), StatusIs(absl::StatusCode::kAlreadyExists));
    QUICHE_EXPECT_OK(scheduler.Unregister(2));
    EXPECT_EQ(scheduler.NumScheduled(), 1u);
    EXPECT_FALSE(scheduler.IsScheduled(2));

    EXPECT_THAT(scheduler.UpdatePriority(2, 1234), StatusIs(absl::StatusCode::kNotFound));
...
struct CustomId {
    ...
    template <typename H> friend H AbslHashValue(H h, const CustomId& c)
    {
...
TEST(BTreeSchedulerTest, CustomIds)
{
    ...
    EXPECT_THAT(scheduler.Register(CustomId { 1, "foo" }, 10), StatusIs(absl::StatusCode::kAlreadyExists));

    ScheduleIds(scheduler, { CustomId { 1, "foo" }, CustomId { 1, "bar" }, CustomId { 2, "foo" } });
    ...
    EXPECT_THAT(PopAll(scheduler), ElementsAre(CustomId { 1, "bar" }, CustomId { 2, "foo" }, CustomId { 1, "foo" }));
}
