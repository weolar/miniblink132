...
public:
    ...
    QUICHE_EXPORT friend bool operator==(QuicheIpAddress lhs, QuicheIpAddress rhs);
    QUICHE_EXPORT friend bool operator!=(QuicheIpAddress lhs, QuicheIpAddress rhs);
...
inline std::ostream& operator<<(std::ostream& os, const QuicheIpAddress address)
{
    os << address.ToString();
    return os;
}
...
public:
    ...
    explicit QuicheIpPrefix(const QuicheIpAddress& address, uint8_t prefix_length);

    QuicheIpAddress address() const
    {
        return address_;
    }
    ...
    QUICHE_EXPORT friend bool operator==(const QuicheIpPrefix& lhs, const QuicheIpPrefix& rhs);
    QUICHE_EXPORT friend bool operator!=(const QuicheIpPrefix& lhs, const QuicheIpPrefix& rhs);
