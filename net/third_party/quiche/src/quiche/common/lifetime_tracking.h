...
struct QUICHE_EXPORT LifetimeInfo {
    bool IsDead() const
    {
        return destructor_stack.has_value();
    }
...
public:
    ...
    bool IsTrackedObjectDead() const
    {
        return info_->IsDead();
    }

    template <typename Sink> friend void AbslStringify(Sink& sink, const LifetimeTracker& tracker)
    {
        if (tracker.info_->IsDead()) {
            absl::Format(&sink, "Tracked object has died with %v%s%s", SymbolizeStackTrace(*tracker.info_->destructor_stack),
                tracker.info_->annotations.empty() ? "" : "\nannotations:\n",
                tracker.info_->annotations.empty() ? "" : absl::StrJoin(tracker.info_->annotations, "\n"));
        } else {
...
private:
    ...
    explicit LifetimeTracker(std::shared_ptr<const LifetimeInfo> info)
        : info_(std::move(info))
    {
        QUICHE_CHECK(info_ != nullptr) << "Passed a null info pointer into the lifetime tracker";
    }
    void CopyFrom(const LifetimeTracker& other)
    {
        info_ = other.info_;
    }
...
public:
    ...
    LifetimeTrackable(const LifetimeTrackable&)
        : LifetimeTrackable()
    {
    }
    LifetimeTrackable& operator=(const LifetimeTrackable&)
    {
        return *this;
    }
    LifetimeTrackable(LifetimeTrackable&&)
        : LifetimeTrackable()
    {
    }
    LifetimeTrackable& operator=(LifetimeTrackable&&)
    {
        return *this;
    }
