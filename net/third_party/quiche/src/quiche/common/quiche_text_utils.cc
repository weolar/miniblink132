...
// static
void QuicheTextUtils::Base64Encode(const uint8_t* data, size_t data_len, std::string* output)
{
...
// static
std::optional<std::string> QuicheTextUtils::Base64Decode(absl::string_view input)
{
    std::string output;
...
std::string QuicheTextUtils::HexDump(absl::string_view binary_data)
{
    ...
    while (bytes_remaining > 0) {
        ...
        for (int i = 0; i < kBytesPerLine; ++i) {
            if (i < line_bytes) {
                absl::StrAppendFormat(&output, "%02x", static_cast<unsigned char>(p[i]));
            } else {
