...
TEST(QuicheTestUtilsTest, StringPieceCaseHash)
{
    ...
    EXPECT_EQ(hasher("This is a string that is too long for inlining and requires a "
...
TEST(QuicheTextUtilsTest, HexDump)
{
    ...
    EXPECT_EQ(quiche::QuicheTextUtils::HexDump(packet),
    ...
    ASSERT_TRUE(absl::HexStringToBytes("20217e7f", &printable_and_unprintable_chars));
...
TEST(QuicheTextUtilsTest, Base64Encode)
{
    ...
    quiche::QuicheTextUtils::Base64Encode(reinterpret_cast<const uint8_t*>(input.data()), input.length(), &output);
    EXPECT_EQ("SGVsbG8", output);

    input = "Hello, QUIC! This string should be long enough to span"
            "multiple lines of output\n";
    quiche::QuicheTextUtils::Base64Encode(reinterpret_cast<const uint8_t*>(input.data()), input.length(), &output);
    EXPECT_EQ("SGVsbG8sIFFVSUMhIFRoaXMgc3RyaW5nIHNob3VsZCBiZSBsb25n"
