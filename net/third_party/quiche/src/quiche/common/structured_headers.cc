...
public:
    ...
    std::optional<Item> ReadBareItem()
    {
        ...
        default:
            if (input_.front() == '-' || absl::ascii_isdigit(input_.front()))
                return ReadNumber();
            if (absl::ascii_isalpha(input_.front()))
                return ReadToken();
            return std::nullopt;
        }
    }
    ...
    std::optional<ParameterisedList> ReadParameterisedList()
    {
        ...
        while (true) {
            std::optional<ParameterisedIdentifier> item = ReadParameterisedIdentifier();
            if (!item)
                return std::nullopt;
...
private:
    ...
    std::optional<ParameterisedIdentifier> ReadParameterisedIdentifier()
    {
        ...
        while (ConsumeChar(';')) {
            ...
            if (!parameters.emplace(*name, std::move(value)).second) {
                QUICHE_DVLOG(1) << "ReadParameterisedIdentifier: duplicated parameter: " << *name;
                return std::nullopt;
            }
            SkipWhitespaces();
        }
        return ParameterisedIdentifier(std::move(*primary_identifier), std::move(parameters));
    }
    ...
    std::optional<ParameterizedMember> ReadItemOrInnerList()
    {
        ...
        if (member_is_inner_list) {
            ...
            if (!item)
                return std::nullopt;
            return ParameterizedMember(std::move(item->item), std::move(item->params));
        }
    }
    ...
    std::optional<ParameterizedMember> ReadInnerList()
    {
        ...
        while (true) {
            ...
            if (ConsumeChar(')')) {
                ...
                if (!parameters)
                    return std::nullopt;
                return ParameterizedMember(std::move(inner_list), true, std::move(*parameters));
            }
            auto item = ReadItem();
            if (!item)
                return std::nullopt;
    ...
    std::optional<std::string> ReadKey()
    {
        if (version_ == kDraft09) {
            ...
            if (input_.empty() || (!absl::ascii_islower(input_.front()) && input_.front() != '*')) {
                LogParseError("ReadKey", "lcalpha | *");
                return std::nullopt;
            }
        }
        const char* allowed_chars = (version_ == kDraft09 ? kKeyChars09 : kKeyChars);
    ...
    {
        if (input_.empty() || !(absl::ascii_isalpha(input_.front()) || input_.front() == '*')) {
            LogParseError("ReadToken", "ALPHA");
            return std::nullopt;
        }
        size_t len = input_.find_first_not_of(version_ == kDraft09 ? kTokenChars09 : kTokenChars);
        if (len == absl::string_view::npos)
            len = input_.size();
    ...
    std::optional<Item> ReadNumber()
    {
        ...
        for (; i < input_.size(); ++i) {
            ...
            if (!absl::ascii_isdigit(input_[i]))
                break;
        }
        if (i == 0) {
        ...
        if (is_decimal) {
            ...
            // successful.
            double f;
            if (!absl::SimpleAtod(output_number_string, &f))
                return std::nullopt;
            ...
            // successful.
            int64_t n;
            if (!absl::SimpleAtoi(output_number_string, &n))
                return std::nullopt;
            QUICHE_CHECK(version_ != kFinal || (n <= kMaxInteger && n >= kMinInteger));
    ...
    std::optional<Item> ReadByteSequence()
    {
        ...
        if (!absl::Base64Unescape(base64, &binary)) {
            QUICHE_DVLOG(1) << "ReadByteSequence: failed to decode base64: " << base64;
            return std::nullopt;
        }
        input_.remove_prefix(len);
    ...
    {
        QUICHE_DVLOG(1) << func << ": " << expected << " expected, got " << (input_.empty() ? "EOS" : "'" + std::string(input_.substr(0, 1)) + "'");
    }
...
public:
    ...
    bool WriteDictionary(const Dictionary& value)
    {
        ...
        for (const auto& [dict_key, dict_value] : value) {
            if (!first)
                output_ << ", ";
            if (!WriteKey(dict_key))
                return false;
            first = false;
            if (!dict_value.member_is_inner_list && !dict_value.member.empty() && dict_value.member.front().item.is_boolean()
                && dict_value.member.front().item.GetBoolean()) {
                if (!WriteParameters(dict_value.params))
                    return false;
            } else {
                output_ << "=";
                if (!WriteParameterizedMember(dict_value))
                    return false;
            }
        }
        return true;
...
bool IsValidToken(absl::string_view str)
{
    ...
    if (str.empty() || !(absl::ascii_isalpha(str.front()) || str.front() == '*')) {
        return false;
    }
...
{
    return absl::c_find_if(members_, [key](const auto& member) { return member.first == key; });
}
