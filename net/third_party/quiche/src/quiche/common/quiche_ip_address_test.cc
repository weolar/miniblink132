...
TEST(QuicheIpAddressTest, IPv4)
{
    ...
    const uint8_t* const v4_address_ptr = reinterpret_cast<const uint8_t*>(&v4_address);
...
TEST(QuicheIpAddressTest, IPv6)
{
    ...
    const uint16_t* const v6_address_ptr = reinterpret_cast<const uint16_t*>(&v6_address);
...
TEST(QuicheIpAddressTest, Subnets)
{
    ...
    for (const auto& test_case : test_cases) {
        ...
        EXPECT_EQ(test_case.same_subnet, address1.InSameSubnet(address2, test_case.subnet_size))
            << "Addresses: " << test_case.address1 << ", " << test_case.address2 << "; subnet: /" << test_case.subnet_size;
    }
}
