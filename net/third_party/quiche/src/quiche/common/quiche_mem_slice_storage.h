...
public:
    QuicheMemSliceStorage(const struct iovec* iov, int iov_count, QuicheBufferAllocator* allocator, const quic::QuicByteCount max_slice_len);
    ...
    absl::Span<QuicheMemSlice> ToSpan()
    {
        return absl::MakeSpan(storage_);
    }
