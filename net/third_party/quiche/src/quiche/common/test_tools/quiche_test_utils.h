...
void CompareCharArraysWithHexError(const std::string& description, const char* actual, const int actual_len, const char* expected, const int expected_len);
...
template <typename T> const absl::Status& ExtractStatus(const absl::StatusOr<T>& status_or)
{
...
MATCHER_P(IsOkAndHolds, matcher, "Matcher against the inner value of absl::StatusOr")
{
    if (!arg.ok()) {
...
MATCHER_P(StatusIs, code, "Matcher against only a specific status code")
{
    if (ExtractStatus(arg).code() != code) {
        *result_listener << "Expected status " << absl::StatusCodeToString(code) << ", got " << ExtractStatus(arg);
        return false;
    }
    return true;
}
...
MATCHER_P2(StatusIs, code, matcher, "Matcher against a specific status code")
{
    ...
    return ::testing::ExplainMatchResult(matcher, ExtractStatus(arg).message(), result_listener);
}
