...
struct QUICHE_EXPORT StringPieceCaseHash {
    size_t operator()(absl::string_view data) const
    {
        ...
        constexpr size_t kInlineStorage = ((kLongestExpectedHeaderName + kCacheLineSize - 1) / kCacheLineSize) * kCacheLineSize;
        ...
        constexpr auto ascii_tolower = [](char c) { return (c >= 'A' && c <= 'Z') ? c | 32 : c; };
        ABSL_CACHELINE_ALIGNED absl::FixedArray<char, kInlineStorage> lower(data.size());
