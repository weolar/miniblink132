...
template <typename Id, typename Priority> class QUICHE_NO_EXPORT BTreeScheduler {
public:
    ...
    size_t NumScheduledInPriorityRange(std::optional<Priority> min, std::optional<Priority> max) const;
...
private:
    ...
    struct StreamEntry {
        ...
        {
            return current_sequence_number.has_value();
        }
    };
    ...
    struct ScheduleKey {
        ...
        {
            return std::make_tuple(priority, sequence_number) > std::make_tuple(other.priority, other.sequence_number);
        }
    ...
    using ScheduleIterator = typename absl::btree_map<ScheduleKey, FullStreamEntry*>::const_iterator;
...
size_t BTreeScheduler<Id, Priority>::NumScheduledInPriorityRange(std::optional<Priority> min, std::optional<Priority> max) const
{
    ...
    // order.
    ScheduleIterator begin = max.has_value() ? schedule_.lower_bound(ScheduleKey::MinForPriority(*max)) : schedule_.begin();
    ScheduleIterator end = min.has_value() ? schedule_.upper_bound(ScheduleKey::MaxForPriority(*min)) : schedule_.end();
    return end - begin;
}

template <typename Id, typename Priority> absl::Status BTreeScheduler<Id, Priority>::Register(Id stream_id, const Priority& priority)
{
...
template <typename Id, typename Priority> auto BTreeScheduler<Id, Priority>::DescheduleStream(const StreamEntry& entry) -> absl::StatusOr<FullScheduleEntry>
{
    QUICHE_DCHECK(entry.scheduled());
    auto it = schedule_.find(ScheduleKey { entry.priority, *entry.current_sequence_number });
    if (it == schedule_.end()) {
        return absl::InternalError("Calling DescheduleStream() on an entry that is not in the schedule at "
                                   "the expected key.");
    }
...
template <typename Id, typename Priority> absl::Status BTreeScheduler<Id, Priority>::Unregister(Id stream_id)
{
    ...
    if (stream.scheduled()) {
        if (!DescheduleStream(stream).ok()) {
            QUICHE_BUG(BTreeSchedule_Unregister_NotInSchedule)
                << "UnregisterStream() called on a stream ID " << stream_id << ", which is marked ready, but is not in the schedule";
        }
    }
...
template <typename Id, typename Priority> absl::Status BTreeScheduler<Id, Priority>::UpdatePriority(Id stream_id, const Priority& new_priority)
{
    auto it = streams_.find(stream_id);
    if (it == streams_.end()) {
    ...
    if (stream.scheduled()) {
        ...
        if (old_entry.ok()) {
            ...
            QUICHE_BUG(BTreeScheduler_Update_Not_In_Schedule)
                << "UpdatePriority() called on a stream ID " << stream_id << ", which is marked ready, but is not in the schedule";
        }
    }
...
template <typename Id, typename Priority> absl::StatusOr<bool> BTreeScheduler<Id, Priority>::ShouldYield(Id stream_id) const
{
...
template <typename Id, typename Priority> absl::StatusOr<Id> BTreeScheduler<Id, Priority>::PopFront()
{
    if (schedule_.empty()) {
...
template <typename Id, typename Priority> absl::Status BTreeScheduler<Id, Priority>::Schedule(Id stream_id)
{
    const auto stream_it = streams_.find(stream_id);
    if (stream_it == streams_.end()) {
        return absl::NotFoundError("ID not registered");
    ...
    auto [schedule_it, success] = schedule_.insert({ ScheduleKey { stream_it->second.priority, --current_write_sequence_number_ }, &*stream_it });
    QUICHE_BUG_IF(WebTransportWriteBlockedList_AddStream_conflict, !success) << "Conflicting key in scheduler for stream " << stream_id;
    stream_it->second.current_sequence_number = schedule_it->first.sequence_number;
    return absl::OkStatus();
}

template <typename Id, typename Priority> bool BTreeScheduler<Id, Priority>::IsScheduled(Id stream_id) const
{
    const auto stream_it = streams_.find(stream_id);
    if (stream_it == streams_.end()) {
        return false;
