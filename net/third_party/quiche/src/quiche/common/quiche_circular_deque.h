...
template <typename T, size_t MinCapacityIncrement = 3, typename Allocator = std::allocator<T>> class QUICHE_NO_EXPORT QuicheCircularDeque {
    using AllocatorTraits = std::allocator_traits<Allocator>;

    // Pointee is either T or const T.
    template <typename Pointee> class QUICHE_NO_EXPORT basic_iterator {
    ...
    public:
        ...
        basic_iterator(const basic_iterator<value_type>& it) // NOLINT(runtime/explicit)
            : deque_(it.deque_)
            , index_(it.index_)
        {
        }
        ...
        {
            return *deque_->index_to_address(index_);
        }
        ...
        friend basic_iterator operator+(const basic_iterator& it, difference_type delta)
        {
    ...
    private:
        ...
        void IncrementBy(difference_type delta)
        {
            if (delta >= 0) {
                ...
                QUICHE_DCHECK_LE(static_cast<size_type>(ExternalPosition() + delta), deque_->size());
            } else {
...
public:
    ...
    QuicheCircularDeque()
        : QuicheCircularDeque(allocator_type())
    {
    }
    ...
    QuicheCircularDeque(size_type count, const T& value, const Allocator& alloc = allocator_type())
        : allocator_and_data_(alloc)
    {
        resize(count, value);
    }

    explicit QuicheCircularDeque(size_type count, const Allocator& alloc = allocator_type())
        : allocator_and_data_(alloc)
    {
        resize(count);
    }
    ...
    template <class InputIt,
        typename = std::enable_if_t<std::is_base_of<std::input_iterator_tag, typename std::iterator_traits<InputIt>::iterator_category>::value>>
    QuicheCircularDeque(InputIt first, InputIt last, const Allocator& alloc = allocator_type())
        : allocator_and_data_(alloc)
    {
    ...
    QuicheCircularDeque(const QuicheCircularDeque& other)
        : QuicheCircularDeque(other, AllocatorTraits::select_on_container_copy_construction(other.allocator_and_data_.allocator()))
    {
    }

    QuicheCircularDeque(const QuicheCircularDeque& other, const allocator_type& alloc)
        : allocator_and_data_(alloc)
    {
    ...
    QuicheCircularDeque(std::initializer_list<T> init, const allocator_type& alloc = allocator_type())
    ...
    QuicheCircularDeque& operator=(const QuicheCircularDeque& other)
    {
        ...
        if (AllocatorTraits::propagate_on_container_copy_assignment::value && (allocator_and_data_.allocator() != other.allocator_and_data_.allocator())) {
            ...
            begin_ = end_ = 0;
            allocator_and_data_ = AllocatorAndData(other.allocator_and_data_.allocator());
        }
        assign(other.begin(), other.end());
        return *this;
    }
    ...
    void swap(QuicheCircularDeque& other)
    {
        ...
        if (AllocatorTraits::propagate_on_container_swap::value) {
            ...
            // unequal allocators.
            QUICHE_DCHECK(get_allocator() == other.get_allocator()) << "Undefined swap behavior";
            swap(allocator_and_data_.data, other.allocator_and_data_.data);
            swap(allocator_and_data_.data_capacity, other.allocator_and_data_.data_capacity);
        }
    }
    ...
    friend bool operator==(const QuicheCircularDeque& lhs, const QuicheCircularDeque& rhs)
    {
    ...
    friend QUICHE_NO_EXPORT std::ostream& operator<<(std::ostream& os, const QuicheCircularDeque& dq)
    {
        os << "{";
...
private:
    void MoveRetainAllocator(QuicheCircularDeque&& other)
    {
        if (get_allocator() == other.get_allocator()) {
            ...
            allocator_and_data_.data_capacity = other.allocator_and_data_.data_capacity;
    ...
    {
        ClearRetainCapacity();
        if (std::is_base_of<std::random_access_iterator_tag, typename std::iterator_traits<InputIt>::iterator_category>::value) {
    ...
    void DestroyAndDeallocateAll()
    {
        ...
        if (data_capacity() > 0) {
            ...
            AllocatorTraits::deallocate(allocator_and_data_.allocator(), allocator_and_data_.data, data_capacity());
        }
    }
    ...
    void MaybeExpandCapacity(size_t num_additional_elements)
    {
        ...
        size_t new_capacity = std::max(new_size, capacity() + min_additional_capacity);
    ...
    void Relocate(size_t new_capacity)
    {
        ...
        QUICHE_DCHECK_GT(new_capacity, num_elements) << "new_capacity:" << new_capacity << ", num_elements:" << num_elements;

        size_t new_data_capacity = new_capacity + 1;
        pointer new_data = AllocatorTraits::allocate(allocator_and_data_.allocator(), new_data_capacity);
    ...
    typename std::enable_if<std::is_trivially_copyable<T_>::value, void>::type RelocateUnwrappedRange(size_type begin, size_type end, pointer dest) const
    {
    ...
    template <typename T_ = T>
    typename std::enable_if<!std::is_trivially_copyable<T_>::value && std::is_move_constructible<T_>::value, void>::type RelocateUnwrappedRange(
        size_type begin, size_type end, pointer dest) const
    {
        QUICHE_DCHECK_LE(begin, end) << "begin:" << begin << ", end:" << end;
        pointer src = index_to_address(begin);
    ...
    template <class... U> void ResizeInternal(size_type count, U&&... u)
    {
        if (count > size()) {
            // Expanding.
    ...
    {
        return allocator_and_data_.data_capacity;
    }
