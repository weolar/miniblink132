...
private:
    ...
    public:
        HeaderValue(HttpHeaderStorage* storage, absl::string_view key, absl::string_view initial_value);
    ...
    typedef quiche::QuicheLinkedHashMap<absl::string_view, HeaderValue, quiche::StringPieceCaseHash, quiche::StringPieceCaseEqual> MapType;
...
public:
...
#if SPDY_HEADER_DEBUG
        void forbid_dereference()
        {
            dereference_forbidden_ = true;
        }
#endif // SPDY_HEADER_DEBUG
    ...
    iterator begin()
    {
        return wrap_iterator(map_.begin());
    }
    iterator end()
    {
        return wrap_iterator(map_.end());
    }
    const_iterator begin() const
    {
        return wrap_const_iterator(map_.begin());
    }
    const_iterator end() const
    {
        return wrap_const_iterator(map_.end());
    }
    bool empty() const
    {
        return map_.empty();
    ...
    void AppendValueOrAddHeader(const absl::string_view key, const absl::string_view value);
    ...
    private:
        ...
        ValueProxy(HttpHeaderBlock* block, HttpHeaderBlock::MapType::iterator lookup_result, const absl::string_view key, size_t* spdy_header_block_value_size);
...
private:
    ...
    inline const_iterator wrap_const_iterator(MapType::const_iterator inner_iterator) const
    {
#if SPDY_HEADER_DEBUG
...
inline bool operator==(absl::string_view lhs, const HttpHeaderBlock::ValueProxy& rhs)
{
    return rhs == lhs;
}
