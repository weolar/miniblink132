...
HttpHeaderStorage::HttpHeaderStorage()
...
absl::string_view HttpHeaderStorage::WriteFragments(const Fragments& fragments, absl::string_view separator)
{
    if (fragments.empty()) {
...
size_t Join(char* dst, const Fragments& fragments, absl::string_view separator)
{
    if (fragments.empty()) {
        return 0;
    }
