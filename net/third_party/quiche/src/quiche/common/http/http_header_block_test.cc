...
std::pair<absl::string_view, absl::string_view> Pair(absl::string_view k, absl::string_view v)
{
...
TEST(HttpHeaderBlockTest, MovedFromIsValid)
{
    ...
    EXPECT_THAT(block5, ElementsAre(Pair("foo", std::string("bar\0bar2", 8)), Pair("foo2", "baz")));
}
...
TEST(HttpHeaderBlockTest, UpperCaseNames)
{
    ...
    EXPECT_THAT(block, ElementsAre(Pair("Foo", std::string("foo\0bar\0baz", 11))));
}
...
TEST(HttpHeaderBlockTest, OrderPreserved)
{
    ...
    EXPECT_THAT(block, ElementsAre(Pair(":method", "GET"), Pair("foo", "bar"), Pair(":path", "/")));
}

TEST(HttpHeaderBlockTest, InsertReturnValue)
{
    HttpHeaderBlock block;
    EXPECT_EQ(HttpHeaderBlock::InsertResult::kInserted, block.insert({ "foo", "bar" }));
    EXPECT_EQ(HttpHeaderBlock::InsertResult::kReplaced, block.insert({ "foo", "baz" }));
}
