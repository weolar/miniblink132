...
protected:
    ...
    const std::shared_ptr<LifetimeInfo>& GetLifetimeInfoFromTrackable(LifetimeTrackable& trackable)
    {
...
TEST_P(LifetimeTrackingTest, SingleTrackerQueryLiveness)
{
    ...
    EXPECT_THAT(absl::StrCat(tracker), testing::HasSubstr("Tracked object is alive"));
    FreeTrackable();
    EXPECT_TRUE(tracker.IsTrackedObjectDead());
    EXPECT_THAT(absl::StrCat(tracker), testing::HasSubstr("Tracked object has died"));
}
...
TEST_P(LifetimeTrackingTest, MultiTrackersQueryLiveness)
{
    ...
    EXPECT_FALSE(tracker2.IsTrackedObjectDead()); // NOLINT(bugprone-use-after-move)
    EXPECT_FALSE(tracker3.IsTrackedObjectDead()); // NOLINT(bugprone-use-after-move)
    EXPECT_FALSE(tracker4.IsTrackedObjectDead()); // NOLINT(bugprone-use-after-move)
    ...
    EXPECT_TRUE(tracker2.IsTrackedObjectDead()); // NOLINT(bugprone-use-after-move)
    EXPECT_TRUE(tracker3.IsTrackedObjectDead()); // NOLINT(bugprone-use-after-move)
    EXPECT_TRUE(tracker4.IsTrackedObjectDead()); // NOLINT(bugprone-use-after-move)
...
INSTANTIATE_TEST_SUITE_P(Tests, LifetimeTrackingTest, testing::Values(TrackableType::kComposed, TrackableType::kInherited), testing::PrintToStringParamName());
