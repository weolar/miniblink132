...
TEST(QuicheBuffer, CopyFromIovecZeroBytes)
{
    ...
    SimpleBufferAllocator allocator;
    QuicheBuffer buffer = QuicheBuffer::CopyFromIovec(&allocator, nullptr,
...
TEST(QuicheBuffer, CopyFromIovecSimple)
{
    ...
    SimpleBufferAllocator allocator;
    QuicheBuffer buffer = QuicheBuffer::CopyFromIovec(&allocator, &iov,
...
TEST(QuicheBuffer, CopyFromIovecMultiple)
{
    ...
    SimpleBufferAllocator allocator;
    QuicheBuffer buffer = QuicheBuffer::CopyFromIovec(&allocator, &iov[0],
        /* iov_count = */ 2, /* iov_offset = */ 0,
        /* buffer_length = */ 6);
    EXPECT_EQ("foobar", buffer.AsStringView());

    buffer = QuicheBuffer::CopyFromIovec(&allocator, &iov[0],
        /* iov_count = */ 2, /* iov_offset = */ 0,
        /* buffer_length = */ 3);
    EXPECT_EQ("foo", buffer.AsStringView());
...
TEST(QuicheBuffer, CopyFromIovecOffsetTooLarge)
{
    ...
    SimpleBufferAllocator allocator;
    EXPECT_QUICHE_BUG(QuicheBuffer::CopyFromIovec(&allocator, &iov[0],
