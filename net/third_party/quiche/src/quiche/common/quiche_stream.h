...
public:
    ...
    struct PeekResult {
        ...
        bool has_data() const
        {
            return !peeked_data.empty() || fin_next;
        }
    };
...
inline bool ProcessAllReadableRegions(ReadStream& stream, UnretainedCallback<void(absl::string_view)> callback)
{
    for (;;) {
...
public:
    ...
    void set_send_fin(bool send_fin)
    {
        send_fin_ = send_fin;
    }
    ...
    bool buffer_unconditionally() const
    {
        return buffer_unconditionally_;
    }
...
inline constexpr StreamWriteOptions kDefaultStreamWriteOptions = StreamWriteOptions();
...
public:
    ...
    virtual absl::Status Writev(absl::Span<const absl::string_view> data, const StreamWriteOptions& options) = 0;
...
inline absl::Status WriteIntoStream(WriteStream& stream, absl::string_view data, const StreamWriteOptions& options = kDefaultStreamWriteOptions)
{
...
inline size_t TotalStringViewSpanSize(absl::Span<const absl::string_view> span)
{
    size_t total = 0;
