...
inline bool operator==(const ParameterizedMember& lhs, const ParameterizedMember& rhs)
{
    return std::tie(lhs.member, lhs.member_is_inner_list, lhs.params) == std::tie(rhs.member, rhs.member_is_inner_list, rhs.params);
}
...
QUICHE_EXPORT std::optional<ParameterisedList> ParseParameterisedList(absl::string_view str);
...
// Structured-Headers Draft 09 only.
QUICHE_EXPORT std::optional<ListOfLists> ParseListOfLists(absl::string_view str);
...
QUICHE_EXPORT std::optional<std::string> SerializeItem(const ParameterizedItem& value);
QUICHE_EXPORT std::optional<std::string> SerializeList(const List& value);
QUICHE_EXPORT std::optional<std::string> SerializeDictionary(const Dictionary& value);
