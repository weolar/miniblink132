...
QuicheBuffer QuicheBuffer::CopyFromIovec(QuicheBufferAllocator* allocator, const struct iovec* iov, int iov_count, size_t iov_offset, size_t buffer_length)
{
    if (buffer_length == 0) {
        return {};
    }
    ...
    if (iovnum >= iov_count) {
        QUICHE_BUG(quiche_bug_10839_1) << "iov_offset larger than iovec total size.";
        return {};
    }
    ...
    QUICHE_BUG_IF(quiche_bug_10839_2, buffer_length > 0) << "iov_offset + buffer_length larger than iovec total size.";
