...
template <typename T, template <typename> class BaseAllocator = std::allocator> class CountingAllocator : public BaseAllocator<T> {
...
public:
    ...
    {
        return shared_counts_->allocate_count;
    }

    size_t deallocate_count() const
    {
        return shared_counts_->deallocate_count;
    }

    friend bool operator==(const CountingAllocator& lhs, const CountingAllocator& rhs)
    {
...
template <typename T, typename propagate_on_copy_assignment, typename propagate_on_move_assignment, typename propagate_on_swap, bool equality_result,
...
struct ConfigurableAllocator : public BaseAllocator<T> {
    ...
    friend bool operator==(const ConfigurableAllocator& /*lhs*/, const ConfigurableAllocator& /*rhs*/)
    {
...
// [1, 2, 3, 4] ==> [4, 1, 2, 3]
template <typename Deque> void ShiftRight(Deque* dq, bool emplace)
{
    auto back = *(&dq->back());
    dq->pop_back();
    if (emplace) {
...
// [1, 2, 3, 4] ==> [2, 3, 4, 1]
template <typename Deque> void ShiftLeft(Deque* dq, bool emplace)
{
    auto front = *(&dq->front());
    dq->pop_front();
    if (emplace) {
...
TEST_F(QuicheCircularDequeTest, Constructor)
{
    ...
    QuicheCircularDeque<int, 3, CountingAllocator<int>> dq8_temp = { 5, 6, 7, 8, 9 };
    QuicheCircularDeque<int, 3, CountingAllocator<int>> dq8_unequal_allocator(std::move(*&dq8_temp), CountingAllocator<int>());
...
TEST_F(QuicheCircularDequeTest, Assign)
{
    ...
    ASSERT_TRUE(decltype(dq3.get_allocator())::propagate_on_container_move_assignment::value);
    decltype(dq3) dq8;
    dq8 = std::move(*&dq3);
...
TEST_F(QuicheCircularDequeTest, Iterate)
{
    ...
    for (QuicheCircularDeque<int>::iterator it = dq.begin(); it != dq.end(); ++it) {
        EXPECT_EQ(expected_value++, *it);
    }

    expected_value = 1;
    for (QuicheCircularDeque<int>::const_iterator it = dq.cbegin(); it != dq.cend(); ++it) {
        EXPECT_EQ(expected_value++, *it);
    }

    // Reverse iterate.
    expected_value = 3;
    for (QuicheCircularDeque<int>::reverse_iterator it = dq.rbegin(); it != dq.rend(); ++it) {
        EXPECT_EQ(expected_value--, *it);
    }

    expected_value = 3;
    for (QuicheCircularDeque<int>::const_reverse_iterator it = dq.crbegin(); it != dq.crend(); ++it) {
        EXPECT_EQ(expected_value--, *it);
    }
}
...
TEST_F(QuicheCircularDequeTest, Iterator)
{
    ...
    EXPECT_EQ(QuicheCircularDeque<int>::iterator(), QuicheCircularDeque<int>::iterator());
    EXPECT_EQ(QuicheCircularDeque<int>::const_iterator(), QuicheCircularDeque<int>::const_iterator());
    EXPECT_EQ(QuicheCircularDeque<int>::reverse_iterator(), QuicheCircularDeque<int>::reverse_iterator());
    EXPECT_EQ(QuicheCircularDeque<int>::const_reverse_iterator(), QuicheCircularDeque<int>::const_reverse_iterator());

    QuicheCircularDeque<QuicheCircularDeque<int>, 3> dqdq = { { 1, 2 }, { 10, 20, 30 }, { 100, 200, 300, 400 } };
...
TEST_F(QuicheCircularDequeTest, Resize)
{
    ...
    for (size_t new_size = dq.size(); new_size != 0; --new_size) {
        dq.resize(new_size);
        EXPECT_TRUE(std::equal(dq.begin(), dq.end(), dq2.begin(), dq2.begin() + new_size));
    }
...
TEST_F(QuicheCircularDequeTest, RelocateNonTriviallyCopyable)
{
...

    {
        ...
        QuicheCircularDeque<MoveConstructible, 3, CountingAllocator<MoveConstructible>> dq1;
        dq1.resize(3);
...

    {
        ...
        QuicheCircularDeque<NonMoveConstructible, 3, CountingAllocator<NonMoveConstructible>> dq2;
        dq2.resize(3);
...
TEST_F(QuicheCircularDequeTest, PushPop)
{
    ...
    { // Pop n elements from back.
        ...
        EXPECT_THAT(dq3, ElementsAre(Foo(1), Foo(2), Foo(3), Foo(4), Foo(5), Foo(6)));

        ShiftRight(&dq3, true);
        ShiftRight(&dq3, true);
        ShiftRight(&dq3, true);
        EXPECT_THAT(dq3, ElementsAre(Foo(4), Foo(5), Foo(6), Foo(1), Foo(2), Foo(3)));
...
TEST_F(QuicheCircularDequeTest, Allocation)
{
...

    {
        ...
        for (int i = 1; i <= 18; ++i) {
            SCOPED_TRACE(testing::Message() << "i=" << i << ", capacity_b4_push=" << dq.capacity());
            dq.push_back(i);
...
template <typename T>
using SwappableAllocator = quiche::test::ConfigurableAllocator<T,
    /*propagate_on_copy_assignment=*/std::true_type,
    /*propagate_on_move_assignment=*/std::true_type,
    /*propagate_on_swap=*/std::true_type,
    /*equality_result=*/true>;
...
template <typename T>
using UnswappableEqualAllocator = quiche::test::ConfigurableAllocator<T,
    /*propagate_on_copy_assignment=*/std::true_type,
    /*propagate_on_move_assignment=*/std::true_type,
...
template <typename T>
using UnswappableUnequalAllocator = quiche::test::ConfigurableAllocator<T,
    /*propagate_on_copy_assignment=*/std::true_type,
    /*propagate_on_move_assignment=*/std::true_type,
    /*propagate_on_swap=*/std::false_type,
    /*equality_result=*/false>;
...
TEST_F(QuicheCircularDequeTest, Swap)
{
    ...
    quiche::QuicheCircularDeque<char, 3, UnswappableEqualAllocator<char>> dq3, dq4;
    dq3 = { 1, 2, 3, 4, 5 };
    dq4 = { 6, 7, 8, 9, 0 };
    swap(dq3, dq4);
    ...
    quiche::QuicheCircularDeque<int, 3, UnswappableUnequalAllocator<int>> dq5, dq6;
    dq6.push_front(4);
