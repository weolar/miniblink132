...
std::string SymbolizeStackTraceImpl(absl::Span<void* const> stacktrace)
{
    ...
    for (void* function : stacktrace) {
        ...
        absl::StrAppendFormat(&formatted_trace, "    %p    %s\n", function, success ? absl::string_view(symbol_name) : kUnknownSymbol);
    }
...
bool QuicheShouldRunStackTraceTestImpl()
{
    ...
    int stack_traces_found = absl::GetStackTrace(unused, ABSL_ARRAYSIZE(unused), /*skip_count=*/0);
