...
std::vector<std::string> QuicheParseCommandLineFlagsImpl(const char* usage, int argc, const char* const* argv, bool /*parse_only*/)
{
    SetUsage(usage);
    std::vector<char*> parsed = absl::ParseCommandLine(argc, const_cast<char**>(argv));
    absl::InitializeLog();
