    ...
    absl::flat_hash_set<std::string>* vars_found)
{
    ...
    for (const auto& pair : parameters) {
        ...
        int num_replaced = absl::StrReplaceAll({ { name_input, encoded_value } }, &result);
        if (num_replaced > 0) {
            found.insert(name);
        }
    }
...
std::optional<std::string> AsciiUrlDecodeImpl(absl::string_view input)
{
    ...
    url::DecodeURLEscapeSequences(input_encoded.c_str(), input_encoded.length(), url::DecodeURLMode::kUTF8, &canon_output);
    std::string output;
