...
inline bool GetGooglePacketHeadersFromControlMessageImpl(struct ::cmsghdr* /*cmsg*/, char** /*packet_headers*/, size_t* /*packet_headers_len*/)
{
    return false;
}
...
inline int GetEcnCmsgArgsPreserveDscpImpl(const int fd, const int address_family, uint8_t ecn_codepoint, int& type, void* value, socklen_t& value_len)
{
    if ((address_family != AF_INET && address_family != AF_INET6) || (ecn_codepoint & kQuichePlatformImplEcnMask) != ecn_codepoint) {
        return -EINVAL;
    }
    ...
    if (getsockopt(fd, (address_family == AF_INET) ? IPPROTO_IP : IPPROTO_IPV6, (address_family == AF_INET) ? IP_TOS : IPV6_TCLASS, arg, &value_len) != 0) {
        return -1 * errno;
    }
