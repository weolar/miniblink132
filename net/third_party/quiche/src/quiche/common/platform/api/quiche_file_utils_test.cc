...
{
    std::string path = absl::StrCat(QuicheGetCommonSourcePath(), "/platform/api/testdir/testfile");
...
{
    std::string path = absl::StrCat(QuicheGetCommonSourcePath(), "/platform/api/testdir/file-that-does-not-exist");
    std::optional<std::string> contents = ReadFileContents(path);
...
{
    std::string path = absl::StrCat(QuicheGetCommonSourcePath(), "/platform/api/testdir");
...
{
    std::string path = absl::StrCat(QuicheGetCommonSourcePath(), "/platform/api/testdir/no-such-directory");
    std::vector<std::string> dirs;
    std::vector<std::string> files;
    bool success = EnumerateDirectory(path, dirs, files);
...
{
    std::vector<std::string> expected_paths = { "a/b/c/d/e", "a/subdir/testfile", "a/z", "testfile", "README.md" };

    std::string root_path = absl::StrCat(QuicheGetCommonSourcePath(), "/platform/api/testdir");
