...
public:
    explicit QuicheReaderMutexLock(QuicheMutex* lock) QUICHE_SHARED_LOCK_FUNCTION(lock);
...
public:
    explicit QuicheWriterMutexLock(QuicheMutex* lock) QUICHE_EXCLUSIVE_LOCK_FUNCTION(lock);
