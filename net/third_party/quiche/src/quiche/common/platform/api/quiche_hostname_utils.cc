...
std::string CanonicalizeHost(absl::string_view host, url::CanonHostInfo* host_info)
{
    ...
    if (host_info->out_host.is_nonempty() && host_info->family != url::CanonHostInfo::BROKEN) {
        ...
        QUICHE_DCHECK_EQ(host_info->out_host.len, static_cast<int>(canon_host.length()));
    } else {
...
bool QuicheHostnameUtils::IsValidSNI(absl::string_view sni)
{
    ...
    return !host_info.IsIPAddress() && IsCanonicalizedHostCompliant(canonicalized_host);
}
