...
void Apply(const std::vector<int>& container, UnretainedCallback<void(int)> function)
{
    for (int n : container) {
        function(n);
    }
...
TEST(QuicheCallbacksTest, SingleUseCallback)
{
    ...
    EXPECT_EQ(called, 1);
    EXPECT_QUICHE_DEBUG_DEATH(std::move(new_callback)(), // NOLINT(bugprone-use-after-move)
        "AnyInvocable");
}

class SetFlagOnDestruction {
public:
    SetFlagOnDestruction(bool* flag)
        : flag_(flag)
    {
    }
