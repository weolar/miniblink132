...
QuicheDataWriter::QuicheDataWriter(size_t size, char* buffer, quiche::Endianness endianness)
    : buffer_(buffer)
    , capacity_(size)
    , length_(0)
...
bool QuicheDataWriter::WriteBytes(const void* data, size_t data_len)
{
    ...
    std::copy(static_cast<const char*>(data), static_cast<const char*>(data) + data_len, dest);
...
bool QuicheDataWriter::WriteStringPieceVarInt62(const absl::string_view& string_piece)
{
...
// static
QuicheVariableLengthIntegerLength QuicheDataWriter::GetVarInt62Len(uint64_t value)
{
    if ((value & kVarInt62ErrorMask) != 0) {
        QUICHE_BUG(invalid_varint) << "Attempted to encode a value, " << value << ", that is too big for VarInt62";
...
bool QuicheDataWriter::WriteVarInt62WithForcedLength(uint64_t value, QuicheVariableLengthIntegerLength write_length)
{
    ...
    if (write_length < min_length) {
        QUICHE_BUG(invalid_varint_forced) << "Cannot write value " << value << " with write_length " << write_length;
        return false;
    }
    if (write_length == min_length) {
    ...
    if (write_length == VARIABLE_LENGTH_INTEGER_LENGTH_8) {
        return WriteUInt8(0b11000000) && WriteUInt8(0) && WriteUInt16(0) && WriteUInt32(value);
    }

    QUICHE_BUG(invalid_write_length) << "Invalid write_length " << static_cast<int>(write_length);
    return false;
}
