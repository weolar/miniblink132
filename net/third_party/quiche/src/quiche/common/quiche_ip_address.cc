...
bool operator==(QuicheIpAddress lhs, QuicheIpAddress rhs)
{
    ...
    case IpAddressFamily::IP_V4:
        return std::equal(lhs.address_.bytes, lhs.address_.bytes + QuicheIpAddress::kIPv4AddressSize, rhs.address_.bytes);
    case IpAddressFamily::IP_V6:
        return std::equal(lhs.address_.bytes, lhs.address_.bytes + QuicheIpAddress::kIPv6AddressSize, rhs.address_.bytes);
    case IpAddressFamily::IP_UNSPEC:
        return true;
    }
    QUICHE_BUG(quiche_bug_10126_2) << "Invalid IpAddressFamily " << static_cast<int32_t>(lhs.family_);
    return false;
}
...
std::string QuicheIpAddress::ToPackedString() const
{
    ...
    QUICHE_BUG(quiche_bug_10126_3) << "Invalid IpAddressFamily " << static_cast<int32_t>(family_);
    return "";
}
...
std::string QuicheIpAddress::ToString() const
{
    ...
    const char* result = inet_ntop(AddressFamilyToInt(), address_.bytes, buffer, sizeof(buffer));
    QUICHE_BUG_IF(quiche_bug_10126_4, result == nullptr) << "Failed to convert an IP address to string";
    return buffer;
}
...
QuicheIpAddress QuicheIpAddress::Normalized() const
{
    ...
    if (!std::equal(std::begin(kMappedAddressPrefix), std::end(kMappedAddressPrefix), address_.bytes)) {
        return *this;
    }
...
QuicheIpAddress QuicheIpAddress::DualStacked() const
{
    ...
    QuicheIpAddress result;
    result.family_ = IpAddressFamily::IP_V6;
    memcpy(result.address_.bytes, kMappedAddressPrefix, sizeof(kMappedAddressPrefix));
...
{
    for (IpAddressFamily family : { IpAddressFamily::IP_V6, IpAddressFamily::IP_V4 }) {
        int result = inet_pton(ToPlatformAddressFamily(family), str.c_str(), address_.bytes);
        if (result > 0) {
            family_ = family;
            return true;
        }
...
bool QuicheIpAddress::InSameSubnet(const QuicheIpAddress& other, int subnet_length)
{
    if (!IsInitialized()) {
        QUICHE_BUG(quiche_bug_10126_5) << "Attempting to do subnet matching on undefined address";
        return false;
    }
...
QuicheIpPrefix::QuicheIpPrefix(const QuicheIpAddress& address, uint8_t prefix_length)
    : address_(address)
    , prefix_length_(prefix_length)
{
    QUICHE_DCHECK(prefix_length <= QuicheIpPrefix(address).prefix_length()) << "prefix_length cannot be longer than the size of the IP address";
}
...
{
    return lhs.address_ == rhs.address_ && lhs.prefix_length_ == rhs.prefix_length_;
}
