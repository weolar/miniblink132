...
// status correctly.
inline void CopyStatusPayloads(const absl::Status& original, absl::Status& target)
{
    original.ForEachPayload([&](absl::string_view key, const absl::Cord& value) { target.SetPayload(key, value); });
}

// Appends additional into to a status message if the status message is
// an error.
template <typename... T> absl::Status AppendToStatus(absl::Status input, T&&... args)
{
    if (ABSL_PREDICT_TRUE(input.ok())) {
        return input;
    }
    absl::Status result = absl::Status(input.code(), absl::StrCat(input.message(), std::forward<T>(args)...));
