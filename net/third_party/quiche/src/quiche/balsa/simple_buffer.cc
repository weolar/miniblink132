...
{
    if (size <= 0) {
        QUICHE_BUG_IF(simple_buffer_write_negative_size, size < 0) << "size must not be negative: " << size;
        return 0;
    }
...
{
    if (size < 0) {
        QUICHE_BUG(simple_buffer_read_negative_size) << "size must not be negative: " << size;
        return 0;
    }
...
{
    if (size < 0) {
        QUICHE_BUG(simple_buffer_reserve_negative_size) << "size must not be negative: " << size;
        return;
    }
...
void SimpleBuffer::AdvanceReadablePtr(int amount_to_advance)
{
    if (amount_to_advance < 0) {
        QUICHE_BUG(simple_buffer_advance_read_negative_arg) << "amount_to_advance must not be negative: " << amount_to_advance;
        return;
    }

    read_idx_ += amount_to_advance;
    if (read_idx_ > write_idx_) {
        QUICHE_BUG(simple_buffer_read_ptr_too_far) << "error: readable pointer advanced beyond writable one";
...
{
    if (amount_to_advance < 0) {
        QUICHE_BUG(simple_buffer_advance_write_negative_arg) << "amount_to_advance must not be negative: " << amount_to_advance;
        return;
    }

    write_idx_ += amount_to_advance;
    if (write_idx_ > storage_size_) {
        QUICHE_BUG(simple_buffer_write_ptr_too_far) << "error: writable pointer advanced beyond end of storage";
...
SimpleBuffer::ReleasedBuffer SimpleBuffer::Release()
{
    ...
    ReleasedBuffer buffer { std::unique_ptr<char[]>(storage_), static_cast<size_t>(write_idx_) };
    Clear();
    storage_ = nullptr;
    storage_size_ = 0;
    return buffer;
