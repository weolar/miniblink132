...
public:
    ...
    {
        size_t buffer_size = 0;
        for (Blocks::const_iterator iter = blocks_.begin(); iter != blocks_.end(); ++iter) {
    ...
    {
        QUICHE_DCHECK_LT(block_idx, blocks_.size()) << block_idx << ", " << blocks_.size();
        return block_idx >= blocks_.size() ? nullptr : blocks_[block_idx].buffer.get();
    }
    ...
    void WriteToContiguousBuffer(absl::string_view sp)
    {
        ...
        if (blocks_[0].buffer == nullptr && sp.size() <= blocksize_) {
            ...
            const size_t new_storage_size = old_storage_size_used + (old_storage_size_used < sp.size() ? sp.size() : old_storage_size_used);
    ...
    {
        QUICHE_BUG_IF(bug_if_1182_1, blocks_.empty()) << "First block not allocated yet!";
        return blocks_.empty() ? nullptr : blocks_[0].buffer.get();
    }

    char* EndOfFirstBlock() const
    {
        QUICHE_BUG_IF(bug_if_1182_2, blocks_.empty()) << "First block not allocated yet!";
    ...
    Blocks::size_type num_blocks() const
    {
        return blocks_.size();
    }
    size_t buffer_size(size_t idx) const
    {
        return blocks_[idx].buffer_size;
    }
    size_t bytes_used(size_t idx) const
    {
        return blocks_[idx].bytes_used();
    }
...
private:
    ...
    {
        return BufferBlock { std::make_unique<char[]>(blocksize), blocksize, blocksize };
    }
...
public:
    ...
    struct QUICHE_EXPORT HeaderLineDescription {
        HeaderLineDescription(
            size_t first_character_index, size_t key_end_index, size_t value_begin_index, size_t last_character_index, size_t buffer_base_index)
    ...
    template <typename IteratorType> class QUICHE_EXPORT iterator_range {
    public:
    ...
    using MultivaluedHeadersValuesMap = absl::flat_hash_map<absl::string_view, std::vector<absl::string_view>, StringPieceCaseHash, StringPieceCaseEqual>;
    ...
    iterator_range<const_header_lines_key_iterator> lines(absl::string_view key) const;
    ...
    const_header_lines_key_iterator GetIteratorForKey(absl::string_view key) const;
    ...
    void ReplaceOrAppendHeader(absl::string_view key, absl::string_view value) override;
    ...
    // function on a key which exists several times in the headers will produce
    // unpredictable results.
    void AppendToHeaderWithCommaAndSpace(absl::string_view key, absl::string_view value) override;
    ...
    void GetAllOfHeader(absl::string_view key, std::vector<absl::string_view>* out) const override;
    ...
    void GetAllOfHeaderIncludeRemoved(absl::string_view key, std::vector<absl::string_view>* out) const;
    ...
    // for the value.
    bool HeaderHasValue(absl::string_view key, absl::string_view value) const override
    {
        return HeaderHasValueHelper(key, value, true);
    }
    // Same as above, but also case-insensitive for the value.
    bool HeaderHasValueIgnoreCase(absl::string_view key, absl::string_view value) const override
    {
    ...
    void GetAllOfHeaderWithPrefix(absl::string_view prefix, std::vector<std::pair<absl::string_view, absl::string_view>>* out) const override;

    void GetAllHeadersWithLimit(std::vector<std::pair<absl::string_view, absl::string_view>>* out, int limit) const override;
    ...
    template <typename Buffer> void WriteHeaderAndEndingToBuffer(Buffer* buffer, CaseOption case_option, CoalesceOption coalesce_option) const
    {
        WriteToBuffer(buffer, case_option, coalesce_option);
        WriteHeaderEndingToBuffer(buffer);
    }

    template <typename Buffer> void WriteHeaderAndEndingToBuffer(Buffer* buffer) const
    {
        WriteHeaderAndEndingToBuffer(buffer, CaseOption::kNoModification, CoalesceOption::kNoCoalesce);
    }
    ...
    // to the buffer.
    template <typename Buffer> static void WriteHeaderEndingToBuffer(Buffer* buffer)
    {
        buffer->WriteString("\r\n");
    }
    ...
    template <typename Buffer> void WriteToBuffer(Buffer* buffer, CaseOption case_option, CoalesceOption coalesce_option) const;

    template <typename Buffer> void WriteToBuffer(Buffer* buffer) const
    {
        WriteToBuffer(buffer, CaseOption::kNoModification, CoalesceOption::kNoCoalesce);
    }
    ...
    void WriteToBufferCoalescingMultivaluedHeaders(Buffer* buffer, const MultivaluedHeadersSet& multivalued_headers, CaseOption case_option) const;

    // Populates |multivalues| with values of |header_lines_| with keys present
    // in |multivalued_headers| set.
    void GetValuesOfMultivaluedHeaders(const MultivaluedHeadersSet& multivalued_headers, MultivaluedHeadersValuesMap* multivalues) const;
    ...
    template <typename Buffer> void WriteHeaderKeyToBuffer(Buffer* buffer, absl::string_view key, CaseOption case_option) const
    {
    ...
    template <typename Buffer> void WriteHeaderLineToBuffer(Buffer* buffer, absl::string_view key, absl::string_view value, CaseOption case_option) const
    {
    ...
    template <typename Buffer>
    void WriteHeaderLineValuesToBuffer(Buffer* buffer, absl::string_view key, const std::vector<absl::string_view>& values, CaseOption case_option) const
    {
        // If the key is empty, we don't want to write the rest because it
        // will not be a well-formed header line.
        if (!key.empty()) {
    ...
    bool ForEachHeader(quiche::UnretainedCallback<bool(const absl::string_view key, const absl::string_view value)> fn) const override;
    ...
    absl::string_view first_line() const
    {
        ...
        return whitespace_4_idx_ == non_whitespace_1_idx_
            ? ""
            : absl::string_view(BeginningOfFirstLine() + non_whitespace_1_idx_, whitespace_4_idx_ - non_whitespace_1_idx_);
    }
    ...
    size_t parsed_response_code() const override
    {
        return parsed_response_code_;
    }
    ...
    absl::string_view request_method() const override
    {
        ...
        return whitespace_2_idx_ == non_whitespace_1_idx_
            ? ""
            : absl::string_view(BeginningOfFirstLine() + non_whitespace_1_idx_, whitespace_2_idx_ - non_whitespace_1_idx_);
    }
    ...
    absl::string_view request_uri() const override
    {
        ...
        return whitespace_3_idx_ == non_whitespace_2_idx_
            ? ""
            : absl::string_view(BeginningOfFirstLine() + non_whitespace_2_idx_, whitespace_3_idx_ - non_whitespace_2_idx_);
    }
    ...
    absl::string_view request_version() const override
    {
        ...
        return whitespace_4_idx_ == non_whitespace_3_idx_
            ? ""
            : absl::string_view(BeginningOfFirstLine() + non_whitespace_3_idx_, whitespace_4_idx_ - non_whitespace_3_idx_);
    }
    ...
    void SetRequestFirstlineFromStringPieces(absl::string_view method, absl::string_view uri, absl::string_view version)
    {
        SetFirstlineFromStringPieces(method, uri, version);
    }

    void SetResponseFirstline(absl::string_view version, size_t parsed_response_code, absl::string_view reason_phrase);
    ...
    void SetParsedResponseCodeAndUpdateFirstline(size_t parsed_response_code) override;
    ...
    bool is_framed_by_connection_close() const
    {
        ...
        // will return false.
        return (content_length_status_ == BalsaHeadersEnums::NO_CONTENT_LENGTH) && !transfer_encoding_is_chunked_
    ...
    void ApplyToCookie(quiche::UnretainedCallback<void(absl::string_view cookie)> f) const override;
    ...
    static void ParseTokenList(absl::string_view header_value, HeaderTokenList* tokens);
...
private:
    ...
    friend bool ParseHTTPFirstLine(char* begin, char* end, bool is_request, BalsaHeaders* headers, BalsaFrameEnums::ErrorCode* error_code,
    ...
    absl::string_view GetValueFromHeaderLineDescription(const HeaderLineDescription& line) const;

    void AddAndMakeDescription(absl::string_view key, absl::string_view value, HeaderLineDescription* d);

    void AppendAndMakeDescription(absl::string_view key, absl::string_view value, HeaderLineDescription* d);

    // Removes all header lines with the given key starting at start.
    void RemoveAllOfHeaderStartingAt(absl::string_view key, HeaderLines::iterator start);

    HeaderLines::const_iterator GetConstHeaderLinesIterator(absl::string_view key) const;

    HeaderLines::iterator GetHeaderLinesIterator(absl::string_view key, HeaderLines::iterator start);

    HeaderLines::iterator GetHeaderLinesIteratorForLastMultivaluedHeader(absl::string_view key);

    template <typename IteratorType> const IteratorType HeaderLinesBeginHelper() const;

    template <typename IteratorType> const IteratorType HeaderLinesEndHelper() const;

    // Helper function for HeaderHasValue and HeaderHasValueIgnoreCase that
    // does most of the work.
    bool HeaderHasValueHelper(absl::string_view key, absl::string_view value, bool case_sensitive) const;
    ...
    void SetFirstlineFromStringPieces(absl::string_view firstline_a, absl::string_view firstline_b, absl::string_view firstline_c);
...
class QUICHE_EXPORT BalsaHeaders::iterator_base : public std::iterator<std::forward_iterator_tag, std::pair<absl::string_view, absl::string_view>> {
public:
    iterator_base()
        : headers_(nullptr)
        , idx_(0)
    ...
    // well.
    friend QUICHE_EXPORT std::ostream& operator<<(std::ostream& os, const iterator_base& it)
    {
...
private:
    ...
    std::pair<absl::string_view, absl::string_view>& Lookup(HeaderLines::size_type index) const
    {
        ...
        if (!value_.has_value()) {
            ...
            value_ = std::make_pair(absl::string_view(stream_begin + line.first_char_idx, line.KeyLength()),
                absl::string_view(stream_begin + line.value_begin_idx, line.ValuesLength()));
        }
        return *value_;
    }
...
class QUICHE_EXPORT BalsaHeaders::const_header_lines_iterator : public BalsaHeaders::iterator_base {
public:
...
private:
    ...
    const_header_lines_iterator(const BalsaHeaders* headers, HeaderLines::size_type index)
...
class QUICHE_EXPORT BalsaHeaders::const_header_lines_key_iterator : public BalsaHeaders::iterator_base {
public:
...
private:
    ...
    const_header_lines_key_iterator(const BalsaHeaders* headers, HeaderLines::size_type index, absl::string_view key)
        : iterator_base(headers, index)
        , key_(key)
    {
    }
    ...
    const_header_lines_key_iterator(const BalsaHeaders* headers, HeaderLines::size_type index)
        : iterator_base(headers, index)
    {
    }
    ...
    bool AtEnd() const
    {
        return *this >= headers_->lines().end();
    }

    absl::string_view key_;
};

inline BalsaHeaders::iterator_range<BalsaHeaders::const_header_lines_iterator> BalsaHeaders::lines() const
{
    return { HeaderLinesBeginHelper<const_header_lines_iterator>(), HeaderLinesEndHelper<const_header_lines_iterator>() };
}

inline BalsaHeaders::iterator_range<BalsaHeaders::const_header_lines_key_iterator> BalsaHeaders::lines(absl::string_view key) const
{
    return { GetIteratorForKey(key), header_lines_key_end() };
}

inline BalsaHeaders::const_header_lines_key_iterator BalsaHeaders::header_lines_key_end() const
{
...
template <typename Buffer> void BalsaHeaders::WriteToBuffer(Buffer* buffer, CaseOption case_option, CoalesceOption coalesce_option) const
{
    ...
    if (coalesce_option != CoalesceOption::kCoalesce) {
        ...
        for (HeaderLines::size_type i = 0; i < end; ++i) {
            ...
            WriteHeaderLineToBuffer(buffer, absl::string_view(line_ptr + line.first_char_idx, line.KeyLength()),
                absl::string_view(line_ptr + line.value_begin_idx, line.ValuesLength()), case_option);
        }
    } else {
        WriteToBufferCoalescingMultivaluedHeaders(buffer, multivalued_envoy_headers(), case_option);
    }
}

inline void BalsaHeaders::GetValuesOfMultivaluedHeaders(const MultivaluedHeadersSet& multivalued_headers, MultivaluedHeadersValuesMap* multivalues) const
{
    ...
    for (const auto& line : header_lines_) {
        ...
        const char* line_ptr = GetPtr(line.buffer_base_idx);
        absl::string_view header_key = absl::string_view(line_ptr + line.first_char_idx, line.KeyLength());
        // If this is multivalued header, it may need to be coalesced.
        if (multivalued_headers.contains(header_key)) {
            absl::string_view header_value = absl::string_view(line_ptr + line.value_begin_idx, line.ValuesLength());
...
template <typename Buffer>
void BalsaHeaders::WriteToBufferCoalescingMultivaluedHeaders(Buffer* buffer, const MultivaluedHeadersSet& multivalued_headers, CaseOption case_option) const
{
    ...
    for (const auto& line : header_lines_) {
        ...
        if (header_multivalue == multivalues.end() || header_multivalue->second.size() == 1) {
            WriteHeaderLineToBuffer(buffer, header_key, absl::string_view(line_ptr + line.value_begin_idx, line.ValuesLength()), case_option);
        } else {
            ...
            if (!header_multivalue->second.empty()) {
                WriteHeaderLineValuesToBuffer(buffer, header_key, header_multivalue->second, case_option);
...
template <typename IteratorType> const IteratorType BalsaHeaders::HeaderLinesBeginHelper() const
{
...
template <typename IteratorType> const IteratorType BalsaHeaders::HeaderLinesEndHelper() const
{
    if (header_lines_.empty()) {
        return IteratorType(this, 0);
    }
