...
public:
    ...
    BalsaFrameEnums::ParseState ParseState() const
    {
        return parse_state_;
    }
    ...
    BalsaFrameEnums::ErrorCode ErrorCode() const
    {
        return last_error_;
    }

    const BalsaHeaders* headers() const
    {
        return headers_;
    }
    ...
    bool is_valid_target_uri() const
    {
        return is_valid_target_uri_;
    }

protected:
    inline BalsaHeadersEnums::ContentLengthStatus ProcessContentLengthLine(size_t line_idx, size_t* length);
    ...
    void ProcessHeaderLines(const Lines& lines, bool is_trailer, BalsaHeaders* headers);

    // Returns true if there are invalid characters, false otherwise.
    bool CheckHeaderLinesForInvalidChars(const Lines& lines, const BalsaHeaders* headers);

    inline size_t ProcessHeaders(const char* message_start, size_t message_length);
...
private:
    ...
    bool FindColonsAndParseIntoKeyValue(const Lines& lines, bool is_trailer, BalsaHeaders* headers);
