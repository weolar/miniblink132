...
absl::string_view::difference_type FindIgnoreCase(absl::string_view haystack, absl::string_view needle)
{
...
absl::string_view::difference_type RemoveLeadingWhitespace(absl::string_view* text)
{
    size_t count = 0;
...
absl::string_view::difference_type RemoveTrailingWhitespace(absl::string_view* text)
{
    size_t count = 0;
...
absl::string_view::difference_type RemoveWhitespaceContext(absl::string_view* text)
{
...
const BalsaHeaders::MultivaluedHeadersSet& BalsaHeaders::multivalued_envoy_headers()
{
    static const MultivaluedHeadersSet* multivalued_envoy_headers = new MultivaluedHeadersSet({ ALL_ENVOY_HEADERS(MULTIVALUE_ENVOY_HEADER) });
    return *multivalued_envoy_headers;
}

void BalsaHeaders::ParseTokenList(absl::string_view header_value, HeaderTokenList* tokens)
{
...
void BalsaHeaders::AddAndMakeDescription(absl::string_view key, absl::string_view value, HeaderLineDescription* d)
{
    ...
    *d = HeaderLineDescription(base_idx, base_idx + key.size(), base_idx + key.size() + 2, base_idx + key.size() + 2 + value.size(), block_buffer_idx);
}

void BalsaHeaders::AppendAndMakeDescription(absl::string_view key, absl::string_view value, HeaderLineDescription* d)
{
    ...
    absl::string_view old_value(GetPtr(d->buffer_base_idx) + d->value_begin_idx, old_value_size);
    ...
    *d = HeaderLineDescription(base_idx, base_idx + key.size(), base_idx + key.size() + 2, base_idx + new_size, block_buffer_idx);
}
...
void BalsaHeaders::RemoveAllOfHeaderStartingAt(absl::string_view key, HeaderLines::iterator start)
{
...
void BalsaHeaders::ReplaceOrAppendHeader(absl::string_view key, absl::string_view value)
{
...
void BalsaHeaders::AppendHeader(absl::string_view key, absl::string_view value)
{
...
void BalsaHeaders::AppendToHeader(absl::string_view key, absl::string_view value)
{
...
void BalsaHeaders::AppendToHeaderWithCommaAndSpace(absl::string_view key, absl::string_view value)
{
...
absl::string_view BalsaHeaders::GetValueFromHeaderLineDescription(const HeaderLineDescription& line) const
{
    QUICHE_DCHECK_GE(line.last_char_idx, line.value_begin_idx);
    return absl::string_view(GetPtr(line.buffer_base_idx) + line.value_begin_idx, line.last_char_idx - line.value_begin_idx);
}

absl::string_view BalsaHeaders::GetHeader(absl::string_view key) const
{
    QUICHE_DCHECK(!header_properties::IsMultivaluedHeader(key)) << "Header '" << key << "' may consist of multiple lines. Do not "
...
BalsaHeaders::const_header_lines_iterator BalsaHeaders::GetHeaderPosition(absl::string_view key) const
{
    const HeaderLines::const_iterator end = header_lines_.end();
    HeaderLines::const_iterator i = GetConstHeaderLinesIterator(key);
    if (i == end) {
...
BalsaHeaders::const_header_lines_key_iterator BalsaHeaders::GetIteratorForKey(absl::string_view key) const
{
    HeaderLines::const_iterator i = GetConstHeaderLinesIterator(key);
    if (i == header_lines_.end()) {
    ...
    return const_header_lines_key_iterator(this, (i - header_lines_.begin()), key);
}

BalsaHeaders::HeaderLines::const_iterator BalsaHeaders::GetConstHeaderLinesIterator(absl::string_view key) const
{
    const HeaderLines::const_iterator end = header_lines_.end();
    for (HeaderLines::const_iterator i = header_lines_.begin(); i != end; ++i) {
        ...
        const absl::string_view current_key(GetPtr(line.buffer_base_idx) + line.first_char_idx, line.key_end_idx - line.first_char_idx);
...
BalsaHeaders::HeaderLines::iterator BalsaHeaders::GetHeaderLinesIterator(absl::string_view key, BalsaHeaders::HeaderLines::iterator start)
{
    const HeaderLines::iterator end = header_lines_.end();
...
BalsaHeaders::HeaderLines::iterator BalsaHeaders::GetHeaderLinesIteratorForLastMultivaluedHeader(absl::string_view key)
{
    const HeaderLines::iterator end = header_lines_.end();
...
void BalsaHeaders::GetAllOfHeader(absl::string_view key, std::vector<absl::string_view>* out) const
{
    for (const_header_lines_key_iterator it = GetIteratorForKey(key); it != lines().end(); ++it) {
        out->push_back(it->second);
    }
}

void BalsaHeaders::GetAllOfHeaderIncludeRemoved(absl::string_view key, std::vector<absl::string_view>* out) const
{
...
bool SurroundedOnlyBySpacesAndCommas(absl::string_view::difference_type idx, absl::string_view::difference_type end_idx, absl::string_view line)
{
...
bool BalsaHeaders::HeaderHasValueHelper(absl::string_view key, absl::string_view value, bool case_sensitive) const
{
    for (const_header_lines_key_iterator it = GetIteratorForKey(key); it != lines().end(); ++it) {
        absl::string_view line = it->second;
        absl::string_view::size_type idx = case_sensitive ? line.find(value, 0) : FindIgnoreCase(line, value);
...
{
    for (const_header_lines_key_iterator it = GetIteratorForKey(key); it != header_lines_key_end(); ++it) {
        if (!it->second.empty()) {
            return true;
        }
    }
...
std::string BalsaHeaders::GetAllOfHeaderAsString(absl::string_view key) const
{
    ...
    auto formatter = [](std::string* out, std::pair<absl::string_view, absl::string_view> header) { return absl::AlphaNumFormatter()(out, header.second); };
    return absl::StrJoin(GetIteratorForKey(key), header_lines_key_end(), ",", formatter);
}
...
void BalsaHeaders::RemoveAllOfHeaderInList(const HeaderTokenList& keys)
{
    ...
    for (HeaderLineDescription& line : header_lines_) {
        ...
        absl::string_view key(GetPtr(line.buffer_base_idx) + line.first_char_idx, key_len);
...
void BalsaHeaders::RemoveAllHeadersWithPrefix(absl::string_view prefix)
{
    for (HeaderLines::size_type i = 0; i < header_lines_.size(); ++i) {
        ...
        const absl::string_view current_key_prefix(GetPtr(line.buffer_base_idx) + line.first_char_idx, prefix.size());
        if (absl::EqualsIgnoreCase(current_key_prefix, prefix)) {
            const absl::string_view current_key(GetPtr(line.buffer_base_idx) + line.first_char_idx, key_len);
...
void BalsaHeaders::GetAllOfHeaderWithPrefix(absl::string_view prefix, std::vector<std::pair<absl::string_view, absl::string_view>>* out) const
{
    for (HeaderLines::size_type i = 0; i < header_lines_.size(); ++i) {
        if (header_lines_[i].skip) {
            continue;
        ...
        const HeaderLineDescription& line = header_lines_[i];
        absl::string_view key(GetPtr(line.buffer_base_idx) + line.first_char_idx, line.key_end_idx - line.first_char_idx);
        if (absl::StartsWithIgnoreCase(key, prefix)) {
            out->push_back(
                std::make_pair(key, absl::string_view(GetPtr(line.buffer_base_idx) + line.value_begin_idx, line.last_char_idx - line.value_begin_idx)));
        }
    }
}

void BalsaHeaders::GetAllHeadersWithLimit(std::vector<std::pair<absl::string_view, absl::string_view>>* out, int limit) const
{
    for (HeaderLines::size_type i = 0; i < header_lines_.size(); ++i) {
        ...
        const HeaderLineDescription& line = header_lines_[i];
        absl::string_view key(GetPtr(line.buffer_base_idx) + line.first_char_idx, line.key_end_idx - line.first_char_idx);
        out->push_back(std::make_pair(key, absl::string_view(GetPtr(line.buffer_base_idx) + line.value_begin_idx, line.last_char_idx - line.value_begin_idx)));
    }
}

size_t BalsaHeaders::RemoveValue(absl::string_view key, absl::string_view search_value)
{
    // Remove whitespace around search value.
    absl::string_view needle = search_value;
    RemoveWhitespaceContext(&needle);
    QUICHE_BUG_IF(bug_22783_2, needle != search_value) << "Search value should not be surrounded by spaces.";
    ...
    for (HeaderLines::iterator it = GetHeaderLinesIterator(key, header_lines_.begin()); it != header_lines_.end(); it = GetHeaderLinesIterator(key, ++it)) {
        ...
        while (values.size() >= needle.size()) {
            ...
            const size_t next_comma = values.find(',', /* pos = */ (found ? needle.size() : 0));
...
bool BalsaHeaders::ForEachHeader(quiche::UnretainedCallback<bool(const absl::string_view key, const absl::string_view value)> fn) const
{
    ...
    for (int i = 0; i < s; ++i) {
        ...
        if (!desc.skip && desc.KeyLength() > 0) {
            ...
            if (!fn(absl::string_view(stream_begin + desc.first_char_idx, desc.KeyLength()),
                    absl::string_view(stream_begin + desc.value_begin_idx, desc.ValuesLength()))) {
                return false;
            }
        }
    }
...
void BalsaHeaders::DumpToPrefixedString(const char* spaces, std::string* str) const
{
    ...
    if (!FramerIsDoneWriting()) {
        absl::StrAppendFormat(str, "\n%s<incomplete header len: %d>\n%s%.*s\n", spaces, buffer_length, spaces, buffer_length, OriginalHeaderStreamBegin());
        return;
    }
...
void BalsaHeaders::ClearContentLength()
{
    RemoveAllOfHeader(kContentLength);
}
...
absl::string_view BalsaHeaders::Authority() const
{
    return GetHeader(kHost);
}
...
void BalsaHeaders::RemoveAuthority()
{
    RemoveAllOfHeader(kHost);
}

void BalsaHeaders::ApplyToCookie(quiche::UnretainedCallback<void(absl::string_view cookie)> f) const
{
    f(GetHeader(kCookie));
}

void BalsaHeaders::SetResponseFirstline(absl::string_view version, size_t parsed_response_code, absl::string_view reason_phrase)
{
    SetFirstlineFromStringPieces(version, absl::StrCat(parsed_response_code), reason_phrase);
    parsed_response_code_ = parsed_response_code;
}

void BalsaHeaders::SetFirstlineFromStringPieces(absl::string_view firstline_a, absl::string_view firstline_b, absl::string_view firstline_c)
{
...
void BalsaHeaders::SetRequestMethod(absl::string_view method)
{
    ...
    if (method.size() <= (whitespace_2_idx_ - non_whitespace_1_idx_)) {
        ...
        SetRequestFirstlineFromStringPieces(method, request_uri(), request_version());
    }
}
...
void BalsaHeaders::SetParsedResponseCodeAndUpdateFirstline(size_t parsed_response_code)
{
    parsed_response_code_ = parsed_response_code;
...
void BalsaHeaders::SetRequestVersion(absl::string_view version)
{
    ...
    // component.
    bool fits_in_space_allowed = version.size() + 1 <= whitespace_4_idx_ - whitespace_3_idx_;

    if (!fits_in_space_allowed) {
        // If the new version is too large, then reformat the firstline.
        SetRequestFirstlineFromStringPieces(request_method(), request_uri(), version);
        return;
    }
...
{
    BalsaHeaders::HeaderLines::iterator it = GetHeaderLinesIterator(key, header_lines_.begin());
    if (it == header_lines_.end()) {
        QUICHE_DLOG(WARNING) << "Attempting to remove last token from a non-existent "
                             << "header \"" << key << "\"";
        return;
    }
    ...
    absl::string_view value(GetPtr(header_line->buffer_base_idx) + header_line->value_begin_idx, header_line->last_char_idx - header_line->value_begin_idx);
    // Tokenize.
    ParseTokenList(value, &tokens);

    if (tokens.empty()) {
        QUICHE_DLOG(WARNING) << "Attempting to remove a token from an empty header value "
                             << "for header \"" << key << "\"";
