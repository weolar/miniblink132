...
public:
    ...
    static char* GetWritableOutput(SpdyFrameBuilder* builder, size_t desired_length, size_t* actual_length)
    {
...
TEST(SpdyFrameBuilderTest, GetWritableBuffer)
{
    ...
    char* writable_buffer = SpdyFrameBuilderPeer::GetWritableBuffer(&builder, kBuilderSize);
    ...
    EXPECT_EQ(absl::string_view(expected, kBuilderSize), absl::string_view(frame.data(), kBuilderSize));
}
...
TEST(SpdyFrameBuilderTest, GetWritableOutput)
{
    ...
    char* writable_buffer = SpdyFrameBuilderPeer::GetWritableOutput(&builder, kBuilderSize, &actual_size);
    memset(writable_buffer, ~1, kBuilderSize);
    EXPECT_TRUE(builder.Seek(kBuilderSize));
