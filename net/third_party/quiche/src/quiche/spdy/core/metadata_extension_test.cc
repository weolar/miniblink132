...
protected:
    MetadataExtensionTest()
        : test_buffer_(kBuffer, kBufferSize)
    {
    }
    ...
    void SetUp() override
    {
        extension_ = absl::make_unique<MetadataVisitor>(
            bind_front(&MetadataExtensionTest::OnCompletePayload, this), bind_front(&MetadataExtensionTest::OnMetadataSupport, this));
    }

    void OnCompletePayload(spdy::SpdyStreamId stream_id, MetadataVisitor::MetadataPayload payload)
    {
        ++received_count_;
    ...
    absl::flat_hash_map<spdy::SpdyStreamId, Http2HeaderBlock> received_payload_map_;
...
TEST_F(MetadataExtensionTest, MetadataDeliveredToUnknownFrameCallbacks)
{
    ...
    EXPECT_CALL(visitor, OnCommonHeader(3, _, MetadataVisitor::kMetadataFrameType, _));
    // The Return(true) should not be necessary. http://b/36023792
    EXPECT_CALL(visitor, OnUnknownFrame(3, MetadataVisitor::kMetadataFrameType)).WillOnce(::testing::Return(true));
    EXPECT_CALL(visitor, OnUnknownFrameStart(3, _, MetadataVisitor::kMetadataFrameType, MetadataVisitor::kEndMetadataFlag));
...
TEST_F(MetadataExtensionTest, MetadataPayloadEndToEnd)
{
    ...
    block2["bar"] = "The color taupe truly represents a triumph of the human spirit over "
                    "adversity.";
    block2["baz"] = "Or perhaps it represents abject surrender to the implacable and "
    ...
    for (const Http2HeaderBlock& payload_block : { std::move(block1), std::move(block2) }) {
        extension_->OnSetting(MetadataVisitor::kMetadataExtensionId, 1);
        ASSERT_TRUE(extension_->PeerSupportsMetadata());
...
TEST_F(MetadataExtensionTest, EmptyBlock)
{
    ...
    EXPECT_EQ(MetadataVisitor::kEndMetadataFlag, metadata_frame->flags() & MetadataVisitor::kEndMetadataFlag);
...
TEST_F(MetadataExtensionTest, LargeBlock)
{
    ...
    while (sequence.HasNext()) {
        ...
        if (sequence.HasNext()) {
            EXPECT_EQ(0u, metadata_frame->flags() & MetadataVisitor::kEndMetadataFlag);
        } else {
            EXPECT_EQ(MetadataVisitor::kEndMetadataFlag, metadata_frame->flags() & MetadataVisitor::kEndMetadataFlag);
        }
    }
