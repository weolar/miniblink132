...
SpdyPrefixedBufferReader::SpdyPrefixedBufferReader(const char* prefix, size_t prefix_length, const char* suffix, size_t suffix_length)
    : prefix_(prefix)
    , suffix_(suffix)
...
bool SpdyPrefixedBufferReader::ReadN(size_t count, SpdyPinnableBufferPiece* out)
{
    if (Available() < count) {
        return false;
    }
