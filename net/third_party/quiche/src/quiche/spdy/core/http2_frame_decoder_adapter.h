...
class QUICHE_EXPORT_PRIVATE Http2DecoderAdapter : public http2::Http2FrameDecoderListener {
public:
    // HTTP2 states.
    enum SpdyState {
        SPDY_ERROR,
    ...
    spdy::SpdyFramerVisitorInterface* visitor() const
    {
        return visitor_;
    }
...
private:
    ...
    void OnPriorityFrame(const Http2FrameHeader& header, const Http2PriorityFields& priority) override;
    ...
    void OnRstStream(const Http2FrameHeader& header, Http2ErrorCode http2_error_code) override;
    ...
    void OnPushPromiseStart(const Http2FrameHeader& header, const Http2PushPromiseFields& promise, size_t total_padding_length) override;
    void OnPushPromiseEnd() override;
    void OnPing(const Http2FrameHeader& header, const Http2PingFields& ping) override;
    void OnPingAck(const Http2FrameHeader& header, const Http2PingFields& ping) override;
    void OnGoAwayStart(const Http2FrameHeader& header, const Http2GoAwayFields& goaway) override;
    void OnGoAwayOpaqueData(const char* data, size_t len) override;
    void OnGoAwayEnd() override;
    void OnWindowUpdate(const Http2FrameHeader& header, uint32_t increment) override;
    void OnAltSvcStart(const Http2FrameHeader& header, size_t origin_length, size_t value_length) override;
    void OnAltSvcOriginData(const char* data, size_t len) override;
    void OnAltSvcValueData(const char* data, size_t len) override;
    void OnAltSvcEnd() override;
    void OnPriorityUpdateStart(const Http2FrameHeader& header, const Http2PriorityUpdateFields& priority_update) override;
    ...
    void OnPaddingTooLong(const Http2FrameHeader& header, size_t missing_length) override;
...
public:
    ...
    virtual void OnError(http2::Http2DecoderAdapter::SpdyFramerError error, std::string detailed_error) = 0;

    // Called when the common header for a frame is received. Validating the
    // common header occurs in later processing.
    virtual void OnCommonHeader(SpdyStreamId /*stream_id*/, size_t /*length*/, uint8_t /*type*/, uint8_t /*flags*/)
    {
    }
    ...
    virtual void OnDataFrameHeader(SpdyStreamId stream_id, size_t length, bool fin) = 0;

    // Called when data is received.
    // |stream_id| The stream receiving data.
    // |data| A buffer containing the data received.
    // |len| The length of the data buffer.
    virtual void OnStreamFrameData(SpdyStreamId stream_id, const char* data, size_t len) = 0;
    ...
    virtual SpdyHeadersHandlerInterface* OnHeaderFrameStart(SpdyStreamId stream_id) = 0;
    ...
    virtual void OnRstStream(SpdyStreamId stream_id, SpdyErrorCode error_code) = 0;
    ...
    virtual void OnGoAway(SpdyStreamId last_accepted_stream_id, SpdyErrorCode error_code) = 0;
    ...
    virtual void OnWindowUpdate(SpdyStreamId stream_id, int delta_window_size) = 0;
    ...
    // Note that header block data is not included. See OnHeaderFrameStart().
    virtual void OnPushPromise(SpdyStreamId stream_id, SpdyStreamId promised_stream_id, bool end) = 0;
    ...
    //     CONTINUATION frame.
    virtual void OnContinuation(SpdyStreamId stream_id, size_t payload_length, bool end) = 0;

    // Called when an ALTSVC frame has been parsed.
    virtual void OnAltSvc(SpdyStreamId /*stream_id*/, absl::string_view /*origin*/, const SpdyAltSvcWireFormat::AlternativeServiceVector& /*altsvc_vector*/)
    {
    }
    ...
    //     |parent_stream_id|.
    virtual void OnPriority(SpdyStreamId stream_id, SpdyStreamId parent_stream_id, int weight, bool exclusive) = 0;
    ...
    virtual void OnPriorityUpdate(SpdyStreamId prioritized_stream_id, absl::string_view priority_field_value) = 0;
    ...
    virtual void OnUnknownFrameStart(SpdyStreamId stream_id, size_t length, uint8_t type, uint8_t flags) = 0;
    ...
    virtual void OnUnknownFramePayload(SpdyStreamId stream_id, absl::string_view payload) = 0;
};
...
public:
    ...
    virtual bool OnFrameHeader(SpdyStreamId stream_id, size_t length, uint8_t type, uint8_t flags) = 0;
