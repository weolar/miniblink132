...
public:
    ...
    using HeaderListener = std::function<void(absl::string_view, absl::string_view)>;
    ...
    using IndexingPolicy = std::function<bool(absl::string_view, absl::string_view)>;
    ...
    std::unique_ptr<ProgressiveEncoder> EncodeHeaderSet(const Http2HeaderBlock& header_set);
