...
class HpackDecoderAdapterTest : public quiche::test::QuicheTestWithParam<std::tuple<StartChoice, bool>> {
protected:
    ...
    Http2HeaderBlock MakeHeaderBlock(const std::vector<std::pair<std::string, std::string>>& headers)
    {
...
INSTANTIATE_TEST_SUITE_P(NoHandler, HpackDecoderAdapterTest, ::testing::Combine(::testing::Values(START_WITHOUT_HANDLER, NO_START), ::testing::Bool()));

INSTANTIATE_TEST_SUITE_P(WithHandler, HpackDecoderAdapterTest, ::testing::Combine(::testing::Values(START_WITH_HANDLER), ::testing::Bool()));
...
TEST_P(HpackDecoderAdapterTest, IndexedHeaderDynamic)
{
    ...
    const Http2HeaderBlock& header_set1 = DecodeBlockExpectingSuccess("\x40\x03"
                                                                      "foo"
                                                                      "\x03"
                                                                      "bar");
    Http2HeaderBlock expected_header_set1;
    ...
    const Http2HeaderBlock& header_set2 = DecodeBlockExpectingSuccess("\xbe\x40\x04"
                                                                      "spam"
                                                                      "\x04"
                                                                      "eggs");
    Http2HeaderBlock expected_header_set2;
...
TEST_P(HpackDecoderAdapterTest, LiteralHeaderNoIndexing)
{
    ...
    const Http2HeaderBlock& header_set = DecodeBlockExpectingSuccess(absl::string_view(input, ABSL_ARRAYSIZE(input) - 1));
...
TEST_P(HpackDecoderAdapterTest, SectionC4RequestHuffmanExamples)
{
    ...
    //                                         | www.example.com
    //                                         | -> :authority: www.example.com
    std::string first = absl::HexStringToBytes("828684418cf1e3c2e5f23a6ba0ab90f4ff");
    ...
    const Http2HeaderBlock& second_header_set = DecodeBlockExpectingSuccess(second);

    EXPECT_THAT(second_header_set,
        ElementsAre(
            // clang-format on
            Pair(":method", "GET"), Pair(":scheme", "http"), Pair(":path", "/"), Pair(":authority", "www.example.com"), Pair("cache-control", "no-cache")));
    // clang-format on
    ...
    std::string third = absl::HexStringToBytes("828785bf408825a849e95ba97d7f8925a849e95bb8e8b4bf");
...
TEST_P(HpackDecoderAdapterTest, SectionC6ResponseHuffmanExamples)
{
    ...
    std::string first = absl::HexStringToBytes("488264025885aec3771a4b6196d07abe"
    ...
    std::string third = absl::HexStringToBytes("88c16196d07abe941054d444a8200595"
