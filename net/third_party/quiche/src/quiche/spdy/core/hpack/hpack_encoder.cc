...
public:
    ...
    RepresentationIterator(const Representations& pseudo_headers, const Representations& regular_headers)
...
std::string HpackEncoder::EncodeHeaderBlock(const Http2HeaderBlock& header_set)
{
...
std::string HpackEncoder::EncodeRepresentations(RepresentationIterator* iter)
{
    ...
    while (iter->HasNext()) {
        ...
        if (enable_compression_) {
            size_t index = header_table_.GetByNameAndValue(header.first, header.second);
...
{
    QUICHE_DVLOG(2) << "Emitting indexed literal: (" << representation.first << ", " << representation.second << ")";
...
void HpackEncoder::EmitNonIndexedLiteral(const Representation& representation, bool enable_compression)
{
    QUICHE_DVLOG(2) << "Emitting nonindexed literal: (" << representation.first << ", " << representation.second << ")";
...
{
    size_t encoded_size = enable_compression_ ? http2::HuffmanSize(str) : str.size();
    if (encoded_size < str.size()) {
        QUICHE_DVLOG(2) << "Emitted Huffman-encoded string of length " << encoded_size;
...
void HpackEncoder::MaybeEmitTableSize()
{
    ...
    QUICHE_DVLOG(1) << "MaybeEmitTableSize min_table_size_setting_received_=" << min_table_size_setting_received_;
...
// static
void HpackEncoder::CookieToCrumbs(const Representation& cookie, Representations* out)
{
    ...
    for (size_t pos = 0;;) {
        ...
        out->push_back(std::make_pair(cookie.first, cookie_value.substr(pos, end - pos)));
...
// static
void HpackEncoder::DecomposeRepresentation(const Representation& header_field, Representations* out)
{
    size_t pos = 0;
    size_t end = 0;
    while (end != absl::string_view::npos) {
        end = header_field.second.find('\0', pos);
        out->push_back(std::make_pair(header_field.first, header_field.second.substr(pos, end == absl::string_view::npos ? end : end - pos)));
        pos = end + 1;
    }
}
...
HpackEncoder::Encoderator::Encoderator(const Http2HeaderBlock& header_set, HpackEncoder* encoder)
    : encoder_(encoder)
    , has_next_(true)
{
    // Separate header set into pseudo-headers and regular headers.
    ...
    header_it_ = std::make_unique<RepresentationIterator>(pseudo_headers_, regular_headers_);

    encoder_->MaybeEmitTableSize();
}

HpackEncoder::Encoderator::Encoderator(const Representations& representations, HpackEncoder* encoder)
    : encoder_(encoder)
    , has_next_(true)
{
...
{
    QUICHE_BUG_IF(spdy_bug_61_1, !has_next_) << "Encoderator::Next called with nothing left to encode.";
    const bool enable_compression = encoder_->enable_compression_;

    // Encode up to max_encoded_bytes of headers.
    while (header_it_->HasNext() && encoder_->output_stream_.size() <= max_encoded_bytes) {
        const Representation header = header_it_->Next();
        encoder_->listener_(header.first, header.second);
        if (enable_compression) {
            size_t index = encoder_->header_table_.GetByNameAndValue(header.first, header.second);
            if (index != kHpackEntryNotFound) {
...
std::unique_ptr<HpackEncoder::ProgressiveEncoder> HpackEncoder::EncodeHeaderSet(const Http2HeaderBlock& header_set)
{
    return std::make_unique<Encoderator>(header_set, this);
}

std::unique_ptr<HpackEncoder::ProgressiveEncoder> HpackEncoder::EncodeRepresentations(const Representations& representations)
{
