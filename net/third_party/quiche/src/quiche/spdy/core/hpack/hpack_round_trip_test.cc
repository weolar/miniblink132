...
class HpackRoundTripTest : public quiche::test::QuicheTestWithParam<InputSizeParam> {
protected:
    void SetUp() override
    {
    ...
    bool RoundTrip(const Http2HeaderBlock& header_set)
    {
        ...
        if (GetParam() == ALL_INPUT) {
            ...
            success = decoder_.HandleControlFrameHeadersData(encoded.data(), encoded.size());
            ...
            const char* data = encoded.data();
            for (size_t ndx = 0; ndx < encoded.size() && success; ++ndx) {
                success = (decoder_.HandleControlFrameHeadersData(data + ndx, 0) && decoder_.HandleControlFrameHeadersData(data + ndx, 1));
            }
        } else {
    ...
    {
        return std::min<size_t>(-std::log(random_.RandDouble()) * mean, sanity_bound);
    }
...
INSTANTIATE_TEST_SUITE_P(Tests, HpackRoundTripTest, ::testing::Values(ALL_INPUT, ONE_BYTE, ZERO_THEN_ONE_BYTE));
    ...
    for (size_t i = 0; i != 2000; ++i) {
        ...
        for (size_t j = 0; j != header_count; ++j) {
            ...
            if (value_index >= values.size()) {
                std::string newvalue = random_.RandString(1 + SampleExponential(15, 75));
