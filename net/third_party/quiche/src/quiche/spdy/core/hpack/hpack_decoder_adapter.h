...
public:
    ...
    bool HandleControlFrameHeadersData(const char* headers_data, size_t headers_data_length);
    ...
    http2::HpackDecodingError error() const
    {
        return error_;
    }
...
private:
    class QUICHE_EXPORT_PRIVATE ListenerAdapter : public http2::HpackDecoderListener {
    public:
        ListenerAdapter();
        ...
        void AddToTotalHpackBytes(size_t delta)
        {
            total_hpack_bytes_ += delta;
        }
        size_t total_hpack_bytes() const
        {
            return total_hpack_bytes_;
        }
