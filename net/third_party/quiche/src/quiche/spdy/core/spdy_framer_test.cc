...
public:
    MOCK_METHOD4(OnSendCompressedFrame, void(SpdyStreamId stream_id, SpdyFrameType type, size_t payload_len, size_t frame_len));

    MOCK_METHOD3(OnReceiveCompressedFrame, void(SpdyStreamId stream_id, SpdyFrameType type, size_t frame_len));
};
...
public:
    ...
    static SpdySerializedFrame SerializeHeaders(SpdyFramer* framer, const SpdyHeadersIR& headers)
    {
        ...
        while (it.HasNextFrame()) {
            ...
            frame_list.emplace_back(SpdySerializedFrame(frame_list_buffer.Begin() + size_before, frame_list_buffer.Size() - size_before, false));
        }
    ...
    static SpdySerializedFrame SerializeHeaders(SpdyFramer* framer, const SpdyHeadersIR& headers, ArrayOutputBuffer* output)
    {
        ...
        SpdySerializedFrame serialized_headers_old_version(output->Begin(), output->Size(), false);
        framer->hpack_encoder_.reset(nullptr);
        auto* saved_debug_visitor = framer->debug_visitor_;
        framer->debug_visitor_ = nullptr;
...
TEST_P(SpdyFramerTest, AcceptMaxFrameSizeSetting)
{
    ...
    SpdySerializedFrame frame(reinterpret_cast<char*>(kH2FrameData), sizeof(kH2FrameData), false);
...
TEST_P(SpdyFramerTest, PushPromiseFramesWithIterator)
{
    ...
    EXPECT_TRUE(frame_it.HasNextFrame());
    EXPECT_GT(frame_it.NextFrame(&output_), 0u);
    SpdySerializedFrame push_promise_frame(output_.Begin(), output_.Size(), false);
...
TEST_P(SpdyFramerTest, ContinuationFrameFlags)
{
    ...
    do {
        ...
        if (use_output_) {
            ...
            ASSERT_TRUE(framer.SerializeContinuation(continuation, &output_));
            frame1 = SpdySerializedFrame(begin, output_.Size() - frame0.size(), false);
        } else {
...
namespace {
void CheckFrameAndIRSize(SpdyFrameIR* ir, SpdyFramer* framer, ArrayOutputBuffer* output_buffer)
{
    output_buffer->Reset();
