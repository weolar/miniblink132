...
class QUICHE_EXPORT_PRIVATE MetadataVisitor : public spdy::ExtensionVisitorInterface {
public:
    using MetadataPayload = spdy::Http2HeaderBlock;

    static_assert(!std::is_copy_constructible<MetadataPayload>::value, "MetadataPayload should be a move-only type!");

    using OnMetadataSupport = std::function<void(bool)>;
    using OnCompletePayload = std::function<void(spdy::SpdyStreamId, MetadataPayload)>;
    ...
    bool OnFrameHeader(spdy::SpdyStreamId stream_id, size_t length, uint8_t type, uint8_t flags) override;
...
private:
    ...
    using StreamMetadataMap = absl::flat_hash_map<spdy::SpdyStreamId, std::unique_ptr<MetadataPayloadState>>;
