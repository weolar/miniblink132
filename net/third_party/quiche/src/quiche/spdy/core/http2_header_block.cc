...
Http2HeaderBlock::HeaderValue::HeaderValue(Http2HeaderStorage* storage, absl::string_view key, absl::string_view initial_value)
    : storage_(storage)
...
Http2HeaderBlock::HeaderValue& Http2HeaderBlock::HeaderValue::operator=(HeaderValue&& other)
{
    storage_ = other.storage_;
    fragments_ = std::move(other.fragments_);
    pair_ = std::move(other.pair_);
...
absl::string_view Http2HeaderBlock::HeaderValue::ConsolidatedValue() const
{
    ...
    if (fragments_.size() > 1) {
        fragments_ = { storage_->WriteFragments(fragments_, SeparatorForKey(pair_.first)) };
    }
    return fragments_[0];
}
...
const std::pair<absl::string_view, absl::string_view>& Http2HeaderBlock::HeaderValue::as_pair() const
{
    pair_.second = ConsolidatedValue();
    return pair_;
}

Http2HeaderBlock::iterator::iterator(MapType::const_iterator it)
    : it_(it)
{
}
...
Http2HeaderBlock::ValueProxy::ValueProxy(
    Http2HeaderBlock* block, Http2HeaderBlock::MapType::iterator lookup_result, const absl::string_view key, size_t* spdy_header_block_value_size)
    : block_(block)
...
Http2HeaderBlock::ValueProxy& Http2HeaderBlock::ValueProxy::operator=(Http2HeaderBlock::ValueProxy&& other)
{
    block_ = other.block_;
    lookup_result_ = other.lookup_result_;
    key_ = other.key_;
...
Http2HeaderBlock::ValueProxy& Http2HeaderBlock::ValueProxy::operator=(absl::string_view value)
{
    *spdy_header_block_value_size_ += value.size();
    ...
    if (lookup_result_ == block_->map_.end()) {
        ...
        lookup_result_ = block_->map_.emplace(std::make_pair(key_, HeaderValue(storage, key_, storage->Write(value)))).first;
    } else {
...
Http2HeaderBlock::Http2HeaderBlock()
...
void Http2HeaderBlock::insert(const Http2HeaderBlock::value_type& value)
{
    ...
    if (iter == map_.end()) {
        ...
        QUICHE_DVLOG(1) << "Updating key: " << iter->first << " with value: " << value.second;
        value_size_ -= iter->second.SizeEstimate();
        iter->second = HeaderValue(&storage_, iter->first, storage_.Write(value.second));
    }
}

Http2HeaderBlock::ValueProxy Http2HeaderBlock::operator[](const absl::string_view key)
{
    ...
    if (iter == map_.end()) {
        ...
        QUICHE_DVLOG(2) << "Key written as: " << std::hex << static_cast<const void*>(key.data()) << ", " << std::dec << key.size();
    } else {
        out_key = iter->first;
    }
...
void Http2HeaderBlock::AppendValueOrAddHeader(const absl::string_view key, const absl::string_view value)
{
    ...
    QUICHE_DVLOG(1) << "Updating key: " << iter->first << "; appending value: " << value;
    value_size_ += SeparatorForKey(key).size();
    iter->second.Append(storage_.Write(value));
}

void Http2HeaderBlock::AppendHeader(const absl::string_view key, const absl::string_view value)
{
    auto backed_key = WriteKey(key);
    map_.emplace(std::make_pair(backed_key, HeaderValue(&storage_, backed_key, storage_.Write(value))));
}
