...
MetadataFrameSequence::MetadataFrameSequence(SpdyStreamId stream_id, spdy::Http2HeaderBlock payload)
...
std::unique_ptr<spdy::SpdyFrameIR> MetadataFrameSequence::Next()
{
    ...
    std::string payload = progressive_encoder_->Next(spdy::kHttp2DefaultFramePayloadLimit);
    const bool end_metadata = !HasNext();
    const uint8_t flags = end_metadata ? MetadataVisitor::kEndMetadataFlag : 0;
    return absl::make_unique<spdy::SpdyUnknownIR>(stream_id_, MetadataVisitor::kMetadataFrameType, flags, std::move(payload));
}
...
MetadataVisitor::MetadataVisitor(OnCompletePayload on_payload, OnMetadataSupport on_support)
...
void MetadataVisitor::OnSetting(SpdySettingsId id, uint32_t value)
{
    ...
    if (id == kMetadataExtensionId) {
        if (value == 0) {
            ...
            if (previous_state == MetadataSupportState::UNSPECIFIED || previous_state == MetadataSupportState::SUPPORTED) {
                on_support_(false);
            }
        } else if (value == 1) {
            const MetadataSupportState previous_state = peer_supports_metadata_;
            peer_supports_metadata_ = MetadataSupportState::SUPPORTED;
            if (previous_state == MetadataSupportState::UNSPECIFIED || previous_state == MetadataSupportState::NOT_SUPPORTED) {
                on_support_(true);
            }
        } else {
            QUICHE_LOG_EVERY_N_SEC(WARNING, 1) << "Unrecognized value for setting " << id << ": " << value;
        }
    }
}

bool MetadataVisitor::OnFrameHeader(SpdyStreamId stream_id, size_t length, uint8_t type, uint8_t flags)
{
    QUICHE_VLOG(1) << "OnFrameHeader(stream_id=" << stream_id << ", length=" << length << ", type=" << static_cast<int>(type)
    ...
    if (it == metadata_map_.end()) {
        auto state = absl::make_unique<MetadataPayloadState>(length, flags & kEndMetadataFlag);
        auto result = metadata_map_.insert(std::make_pair(stream_id, std::move(state)));
        QUICHE_BUG_IF(bug_if_2781_1, !result.second) << "Map insertion failed.";
        it = result.first;
    } else {
        QUICHE_BUG_IF(bug_22051_1, it->second->end_metadata) << "Inconsistent metadata payload state!";
        QUICHE_BUG_IF(bug_if_2781_2, it->second->bytes_remaining > 0) << "Incomplete metadata block!";
    }
...
{
    QUICHE_VLOG(1) << "OnFramePayload(stream_id=" << current_stream_ << ", len=" << len << ")";
    ...
    if (it == metadata_map_.end() || it->second == nullptr) {
        ...
        if (len < state->bytes_remaining) {
            ...
            QUICHE_BUG_IF(bug_22051_2, len > state->bytes_remaining)
                << "Metadata payload overflow! len: " << len << " bytes_remaining: " << state->bytes_remaining;
            ...
            if (state->end_metadata) {
                ...
                success = success && decoder.EndDecodingBlock() && !listener.hpack_error();
                if (success) {
