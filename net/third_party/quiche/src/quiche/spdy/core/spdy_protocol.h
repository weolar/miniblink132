...
// The initial value for the maximum frame payload size as per the spec. This is
// the maximum control frame size we accept.
const uint32_t kHttp2DefaultFrameSizeLimit = kHttp2DefaultFramePayloadLimit + kFrameHeaderSize;
...
QUICHE_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& out, SpdyKnownSettingsId id);
...
QUICHE_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& out, SpdyFrameType frame_type);
...
QUICHE_EXPORT_PRIVATE bool IsValidHTTP2FrameStreamId(SpdyStreamId current_frame_stream_id, SpdyFrameType frame_type_field);
...
QUICHE_EXPORT_PRIVATE bool ParseSettingsId(SpdySettingsId wire_setting_id, SpdyKnownSettingsId* setting_id);
...
QUICHE_EXPORT_PRIVATE const char* WriteSchedulerTypeToString(WriteSchedulerType type);
...
const size_t kSettingsOneSettingSize = sizeof(uint32_t) + sizeof(SpdySettingsId);
...
template <typename StreamIdType> class QUICHE_EXPORT_PRIVATE StreamPrecedence {
public:
    ...
    StreamPrecedence(StreamIdType parent_id, int weight, bool is_exclusive)
        ...
        , http2_stream_dependency_ { parent_id, ClampHttp2Weight(weight), is_exclusive }
    {
    }
    ...
    {
        return is_spdy3_priority_ ? spdy3_priority_ : Http2WeightToSpdy3Priority(http2_stream_dependency_.weight);
    }
    ...
    {
        return is_spdy3_priority_ ? kHttp2RootStreamId : http2_stream_dependency_.parent_id;
    }
    ...
    int weight() const
    {
        return is_spdy3_priority_ ? Spdy3PriorityToHttp2Weight(spdy3_priority_) : http2_stream_dependency_.weight;
    }
...
class QUICHE_EXPORT_PRIVATE SpdyFrameWithHeaderBlockIR : public SpdyFrameWithFinIR {
public:
    ~SpdyFrameWithHeaderBlockIR() override;

    const Http2HeaderBlock& header_block() const
    {
...
protected:
    SpdyFrameWithHeaderBlockIR(SpdyStreamId stream_id, Http2HeaderBlock header_block);
...
class QUICHE_EXPORT_PRIVATE SpdyPushPromiseIR : public SpdyFrameWithHeaderBlockIR {
public:
    SpdyPushPromiseIR(SpdyStreamId stream_id, SpdyStreamId promised_stream_id)
        : SpdyPushPromiseIR(stream_id, promised_stream_id, Http2HeaderBlock())
    {
    }
    SpdyPushPromiseIR(SpdyStreamId stream_id, SpdyStreamId promised_stream_id, Http2HeaderBlock header_block)
        : SpdyFrameWithHeaderBlockIR(stream_id, std::move(header_block))
...
public:
    ...
    {
        return std::string { frame_, size_ };
    }
