...
char* SpdyFrameBuilder::GetWritableOutput(size_t length, size_t* actual_length)
{
    char* dest = nullptr;
    int size = 0;
...
bool SpdyFrameBuilder::BeginNewFrame(SpdyFrameType type, uint8_t flags, SpdyStreamId stream_id)
{
    ...
    bool success = true;
    if (length_ > 0) {
        QUICHE_BUG(spdy_bug_73_1) << "SpdyFrameBuilder doesn't have a clean state when BeginNewFrame"
...
bool SpdyFrameBuilder::BeginNewFrame(SpdyFrameType type, uint8_t flags, SpdyStreamId stream_id, size_t length)
{
    uint8_t raw_frame_type = SerializeFrameType(type);
    QUICHE_DCHECK(IsDefinedFrameType(raw_frame_type));
    QUICHE_DCHECK_EQ(0u, stream_id & ~kStreamIdMask);
    QUICHE_BUG_IF(spdy_bug_73_2, length > kHttp2DefaultFramePayloadLimit) << "Frame length  " << length_ << " is longer than frame size limit.";
    return BeginNewFrameInternal(raw_frame_type, flags, stream_id, length);
}

bool SpdyFrameBuilder::BeginNewUncheckedFrame(uint8_t raw_frame_type, uint8_t flags, SpdyStreamId stream_id, size_t length)
{
    return BeginNewFrameInternal(raw_frame_type, flags, stream_id, length);
}

bool SpdyFrameBuilder::BeginNewFrameInternal(uint8_t raw_frame_type, uint8_t flags, SpdyStreamId stream_id, size_t length)
{
...
bool SpdyFrameBuilder::CanWrite(size_t length) const
{
    ...
    if (output_ == nullptr) {
        if (offset_ + length_ + length > capacity_) {
            QUICHE_DLOG(FATAL) << "Requested: " << length << " capacity: " << capacity_ << " used: " << offset_ + length_;
            return false;
        }
    } else {
