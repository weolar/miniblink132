...
Http2HeaderStorage::Http2HeaderStorage()
...
absl::string_view Http2HeaderStorage::WriteFragments(const std::vector<absl::string_view>& fragments, absl::string_view separator)
{
    if (fragments.empty()) {
...
size_t Join(char* dst, const std::vector<absl::string_view>& fragments, absl::string_view separator)
{
    if (fragments.empty()) {
        return 0;
    }
