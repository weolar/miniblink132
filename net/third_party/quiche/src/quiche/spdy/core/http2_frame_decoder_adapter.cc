...
{
    return type == Http2FrameType::DATA || type == Http2FrameType::HEADERS || type == Http2FrameType::PUSH_PROMISE;
}
...
Http2DecoderAdapter::SpdyFramerError HpackDecodingErrorToSpdyFramerError(HpackDecodingError error)
{
    switch (error) {
    ...
    case HpackDecodingError::kIndexVarintError:
        return Http2DecoderAdapter::SpdyFramerError::SPDY_HPACK_INDEX_VARINT_ERROR;
    case HpackDecodingError::kNameLengthVarintError:
        return Http2DecoderAdapter::SpdyFramerError::SPDY_HPACK_NAME_LENGTH_VARINT_ERROR;
    case HpackDecodingError::kValueLengthVarintError:
        return Http2DecoderAdapter::SpdyFramerError::SPDY_HPACK_VALUE_LENGTH_VARINT_ERROR;
    ...
    case HpackDecodingError::kNameHuffmanError:
        return Http2DecoderAdapter::SpdyFramerError::SPDY_HPACK_NAME_HUFFMAN_ERROR;
    case HpackDecodingError::kValueHuffmanError:
        return Http2DecoderAdapter::SpdyFramerError::SPDY_HPACK_VALUE_HUFFMAN_ERROR;
    case HpackDecodingError::kMissingDynamicTableSizeUpdate:
        return Http2DecoderAdapter::SpdyFramerError::SPDY_HPACK_MISSING_DYNAMIC_TABLE_SIZE_UPDATE;
    ...
    case HpackDecodingError::kInvalidNameIndex:
        return Http2DecoderAdapter::SpdyFramerError::SPDY_HPACK_INVALID_NAME_INDEX;
    case HpackDecodingError::kDynamicTableSizeUpdateNotAllowed:
        return Http2DecoderAdapter::SpdyFramerError::SPDY_HPACK_DYNAMIC_TABLE_SIZE_UPDATE_NOT_ALLOWED;
    case HpackDecodingError::kInitialDynamicTableSizeUpdateIsAboveLowWaterMark:
        return Http2DecoderAdapter::SpdyFramerError::SPDY_HPACK_INITIAL_DYNAMIC_TABLE_SIZE_UPDATE_IS_ABOVE_LOW_WATER_MARK;
    case HpackDecodingError::kDynamicTableSizeUpdateIsAboveAcknowledgedSetting:
        return Http2DecoderAdapter::SpdyFramerError::SPDY_HPACK_DYNAMIC_TABLE_SIZE_UPDATE_IS_ABOVE_ACKNOWLEDGED_SETTING;
    ...
    case HpackDecodingError::kCompressedHeaderSizeExceedsLimit:
        return Http2DecoderAdapter::SpdyFramerError::SPDY_HPACK_COMPRESSED_HEADER_SIZE_EXCEEDS_LIMIT;
    }
...
const char* Http2DecoderAdapter::SpdyFramerErrorToString(SpdyFramerError spdy_framer_error)
{
...
void Http2DecoderAdapter::set_debug_visitor(SpdyFramerDebugVisitorInterface* debug_visitor)
{
    debug_visitor_ = debug_visitor;
}

void Http2DecoderAdapter::set_extension_visitor(ExtensionVisitorInterface* visitor)
{
    extension_ = visitor;
}
...
size_t Http2DecoderAdapter::ProcessInput(const char* data, size_t len)
{
    ...
    while (len > 0 && spdy_state_ != SPDY_ERROR) {
        ...
        // in an error state.
        QUICHE_DCHECK(processed > 0) << "processed=" << processed << "   spdy_state_=" << spdy_state_ << "   spdy_framer_error_=" << spdy_framer_error_;
...
Http2DecoderAdapter::SpdyFramerError Http2DecoderAdapter::spdy_framer_error() const
{
...
{
    SetSpdyErrorAndNotify(SpdyFramerError::SPDY_STOP_PROCESSING, "Ignoring further events on this connection.");
}
...
bool Http2DecoderAdapter::OnFrameHeader(const Http2FrameHeader& header)
{
    ...
    visitor()->OnCommonHeader(header.stream_id, header.payload_length, raw_frame_type, header.flags);
    if (has_expected_frame_type_ && header.type != expected_frame_type_) {
        ...
        // unknown frame.
        QUICHE_VLOG(1) << "The framer was expecting to receive a " << expected_frame_type_ << " frame, but instead received an unknown frame of type "
                       << header.type;
    ...
    if (!IsSupportedHttp2FrameType(header.type)) {
        ...
        bool valid_stream = visitor()->OnUnknownFrame(header.stream_id, raw_frame_type);
        if (!valid_stream) {
            // Report an invalid frame error if the stream_id is not valid.
            QUICHE_VLOG(1) << "Unknown control frame type " << header.type << " received on invalid stream " << header.stream_id;
    ...
    if (!IsValidHTTP2FrameStreamId(header.stream_id, frame_type)) {
        QUICHE_VLOG(1) << "The framer received an invalid streamID of " << header.stream_id << " for a frame of type " << header.type;
        SetSpdyErrorAndNotify(SpdyFramerError::SPDY_INVALID_STREAM_ID, "");
        return false;
    }

    if (has_expected_frame_type_ && header.type != expected_frame_type_) {
        QUICHE_VLOG(1) << "Expected frame type " << expected_frame_type_ << ", not " << header.type;
        SetSpdyErrorAndNotify(SpdyFramerError::SPDY_UNEXPECTED_FRAME, "");
        return false;
    }

    if (!has_expected_frame_type_ && header.type == Http2FrameType::CONTINUATION) {
...
void Http2DecoderAdapter::OnDataStart(const Http2FrameHeader& header)
{
    ...
    if (IsOkToStartFrame(header) && HasRequiredStreamId(header)) {
        ...
        visitor()->OnDataFrameHeader(header.stream_id, header.payload_length, header.IsEndStream());
    }
}
...
void Http2DecoderAdapter::OnHeadersStart(const Http2FrameHeader& header)
{
    ...
    if (IsOkToStartFrame(header) && HasRequiredStreamId(header)) {
        ...
        visitor()->OnHeaders(header.stream_id, header.payload_length, kNotHasPriorityFields,
            0, // priority
            0, // parent_stream_id
            false, // exclusive
...
void Http2DecoderAdapter::OnHeadersPriority(const Http2PriorityFields& priority)
{
    ...
    if (!visitor()) {
        QUICHE_BUG(spdy_bug_1_1) << "Visitor is nullptr, handling priority in headers failed."
                                 << " priority:" << priority << " frame_header:" << frame_header_;
        return;
    }
    visitor()->OnHeaders(frame_header_.stream_id, frame_header_.payload_length, kHasPriorityFields, priority.weight, priority.stream_dependency,
        priority.is_exclusive, frame_header_.IsEndStream(), frame_header_.IsEndHeaders());
    CommonStartHpackBlock();
}
...
void Http2DecoderAdapter::OnHpackFragment(const char* data, size_t len)
{
    ...
    if (!decoder->HandleControlFrameHeadersData(data, len)) {
        SetSpdyErrorAndNotify(HpackDecodingErrorToSpdyFramerError(decoder->error()), decoder->detailed_error());
        return;
    }
}
...
void Http2DecoderAdapter::OnPriorityFrame(const Http2FrameHeader& header, const Http2PriorityFields& priority)
{
    QUICHE_DVLOG(1) << "OnPriorityFrame: " << header << "; priority: " << priority;
    if (IsOkToStartFrame(header) && HasRequiredStreamId(header)) {
        visitor()->OnPriority(header.stream_id, priority.stream_dependency, priority.weight, priority.is_exclusive);
    }
}
...
void Http2DecoderAdapter::OnContinuationStart(const Http2FrameHeader& header)
{
    ...
    if (IsOkToStartFrame(header) && HasRequiredStreamId(header)) {
        ...
        has_frame_header_ = true;
        ReportReceiveCompressedFrame(header);
        visitor()->OnContinuation(header.stream_id, header.payload_length, header.IsEndHeaders());
    }
}
...
void Http2DecoderAdapter::OnPadding(const char* /*padding*/, size_t skipped_length)
{
...
void Http2DecoderAdapter::OnRstStream(const Http2FrameHeader& header, Http2ErrorCode http2_error_code)
{
    QUICHE_DVLOG(1) << "OnRstStream: " << header << "; code=" << http2_error_code;
    if (IsOkToStartFrame(header) && HasRequiredStreamId(header)) {
        SpdyErrorCode error_code = ParseErrorCode(static_cast<uint32_t>(http2_error_code));
...
void Http2DecoderAdapter::OnPushPromiseStart(const Http2FrameHeader& header, const Http2PushPromiseFields& promise, size_t total_padding_length)
{
    QUICHE_DVLOG(1) << "OnPushPromiseStart: " << header << "; promise: " << promise << "; total_padding_length: " << total_padding_length;
    if (IsOkToStartFrame(header) && HasRequiredStreamId(header)) {
        ...
        has_frame_header_ = true;
        ReportReceiveCompressedFrame(header);
        visitor()->OnPushPromise(header.stream_id, promise.promised_stream_id, header.IsEndHeaders());
        CommonStartHpackBlock();
    }
}
...
void Http2DecoderAdapter::OnPing(const Http2FrameHeader& header, const Http2PingFields& ping)
{
...
void Http2DecoderAdapter::OnPingAck(const Http2FrameHeader& header, const Http2PingFields& ping)
{
...
void Http2DecoderAdapter::OnGoAwayStart(const Http2FrameHeader& header, const Http2GoAwayFields& goaway)
{
    QUICHE_DVLOG(1) << "OnGoAwayStart: " << header << "; goaway: " << goaway;
    if (IsOkToStartFrame(header) && HasRequiredStreamIdZero(header)) {
        frame_header_ = header;
        has_frame_header_ = true;
        SpdyErrorCode error_code = ParseErrorCode(static_cast<uint32_t>(goaway.error_code));
...
void Http2DecoderAdapter::OnWindowUpdate(const Http2FrameHeader& header, uint32_t increment)
{
    QUICHE_DVLOG(1) << "OnWindowUpdate: " << header << "; increment=" << increment;
...
void Http2DecoderAdapter::OnAltSvcStart(const Http2FrameHeader& header, size_t origin_length, size_t value_length)
{
    QUICHE_DVLOG(1) << "OnAltSvcStart: " << header << "; origin_length: " << origin_length << "; value_length: " << value_length;
    if (!IsOkToStartFrame(header)) {
        return;
    }
    frame_header_ = header;
...
{
    QUICHE_DVLOG(1) << "OnAltSvcEnd: origin.size(): " << alt_svc_origin_.size() << "; value.size(): " << alt_svc_value_.size();
...
{
    QUICHE_DVLOG(1) << "OnPriorityUpdateStart: " << header << "; prioritized_stream_id: " << priority_update.prioritized_stream_id;
    if (IsOkToStartFrame(header) && HasRequiredStreamIdZero(header) && HasRequiredStreamId(priority_update.prioritized_stream_id)) {
        frame_header_ = header;
        has_frame_header_ = true;
...
void Http2DecoderAdapter::OnPriorityUpdatePayload(const char* data, size_t len)
{
...
{
    QUICHE_DVLOG(1) << "OnPriorityUpdateEnd: priority_field_value.size(): " << priority_field_value_.size();
...
void Http2DecoderAdapter::OnUnknownStart(const Http2FrameHeader& header)
{
    ...
    if (IsOkToStartFrame(header)) {
        ...
        if (extension_ != nullptr) {
            handling_extension_payload_ = extension_->OnFrameHeader(header.stream_id, header.payload_length, type, flags);
        }
        visitor()->OnUnknownFrameStart(header.stream_id, header.payload_length, type, flags);
    }
}
...
void Http2DecoderAdapter::OnUnknownPayload(const char* data, size_t len)
{
    ...
    visitor()->OnUnknownFramePayload(frame_header_.stream_id, absl::string_view(data, len));
}
...
void Http2DecoderAdapter::OnPaddingTooLong(const Http2FrameHeader& header, size_t missing_length)
{
    QUICHE_DVLOG(1) << "OnPaddingTooLong: " << header << "; missing_length: " << missing_length;
    if (header.type == Http2FrameType::DATA) {
...
size_t Http2DecoderAdapter::ProcessInputFrame(const char* data, size_t len)
{
    ...
    if (spdy_state_ != SpdyState::SPDY_ERROR) {
        ...
        QUICHE_VLOG(1) << "ProcessInputFrame spdy_framer_error_=" << SpdyFramerErrorToString(spdy_framer_error_);
        if (spdy_framer_error_ == SpdyFramerError::SPDY_INVALID_PADDING && has_frame_header_ && frame_type() != Http2FrameType::DATA) {
            ...
            if (total <= frame_header().payload_length) {
                ...
                QUICHE_VLOG(1) << "Skipping past " << avail << " bytes, of " << total << " total remaining in the frame's payload.";
                db.AdvanceCursor(avail);
            } else {
                QUICHE_BUG(spdy_bug_1_2) << "Total remaining (" << total << ") should not be greater than the payload length; " << frame_header();
            }
        }
    }
    return db.Offset();
        ...
        if (IsDiscardingPayload()) {
            if (remaining_total_payload() == 0) {
                ...
                if (decode_status != DecodeStatus::kDecodeDone) {
                    QUICHE_BUG(spdy_bug_1_3) << "Expected to be done decoding the frame, not " << decode_status;
                    SetSpdyErrorAndNotify(SPDY_INTERNAL_FRAMER_ERROR, "");
                } else if (spdy_framer_error_ != SPDY_NO_ERROR) {
                    QUICHE_BUG(spdy_bug_1_4) << "Expected to have no error, not " << SpdyFramerErrorToString(spdy_framer_error_);
                } else {
                    ResetBetweenFrames();
                }
            } else {
...
void Http2DecoderAdapter::SetSpdyErrorAndNotify(SpdyFramerError error, std::string detailed_error)
{
    if (HasError()) {
...
{
    bool result = frame_header_.IsPadded() && opt_pad_length_ && frame_decoder_.remaining_payload() == 0 && frame_decoder_.remaining_padding() > 0;
...
bool Http2DecoderAdapter::HasRequiredStreamIdZero(const Http2FrameHeader& header)
{
    return HasRequiredStreamIdZero(header.stream_id);
}

void Http2DecoderAdapter::ReportReceiveCompressedFrame(const Http2FrameHeader& header)
{
    if (debug_visitor() != nullptr) {
        size_t total = header.payload_length + Http2FrameHeader::EncodedSize();
        debug_visitor()->OnReceiveCompressedFrame(header.stream_id, ToSpdyFrameType(header.type), total);
    }
}
...
void Http2DecoderAdapter::CommonStartHpackBlock()
{
    ...
    SpdyHeadersHandlerInterface* handler = visitor()->OnHeaderFrameStart(stream_id());
    if (handler == nullptr) {
...
void Http2DecoderAdapter::CommonHpackFragmentEnd()
{
    ...
    MaybeAnnounceEmptyFirstHpackFragment();
    if (frame_header_.IsEndHeaders()) {
        QUICHE_DCHECK_EQ(has_hpack_first_frame_header_, frame_type() == Http2FrameType::CONTINUATION) << frame_header();
        ...
        const Http2FrameHeader& first = frame_type() == Http2FrameType::CONTINUATION ? hpack_first_frame_header_ : frame_header_;
...
bool SpdyFramerVisitorInterface::OnGoAwayFrameData(const char* /*goaway_data*/, size_t /*len*/)
{
    return true;
}
