...
{
    return a.payload_length == b.payload_length && a.stream_id == b.stream_id && a.type == b.type && a.flags == b.flags;
}
...
{
    std::stringstream ss;
    ss << "E=" << (is_exclusive ? "true" : "false") << ", stream=" << stream_dependency << ", weight=" << static_cast<uint32_t>(weight);
    return ss.str();
}
...
bool operator==(const Http2PushPromiseFields& a, const Http2PushPromiseFields& b)
{
...
{
    static_assert((sizeof a.opaque_bytes) == Http2PingFields::EncodedSize(), "Why not the same size?");
    return 0 == std::memcmp(a.opaque_bytes, b.opaque_bytes, sizeof a.opaque_bytes);
}

std::ostream& operator<<(std::ostream& out, const Http2PingFields& v)
{
    return out << "opaque_bytes=0x" << absl::BytesToHexString(absl::string_view(reinterpret_cast<const char*>(v.opaque_bytes), sizeof v.opaque_bytes));
}
...
{
    return out << "last_stream_id=" << v.last_stream_id << ", error_code=" << v.error_code;
}

// Http2WindowUpdateFields:

bool operator==(const Http2WindowUpdateFields& a, const Http2WindowUpdateFields& b)
{
...
bool operator==(const Http2PriorityUpdateFields& a, const Http2PriorityUpdateFields& b)
{
...
std::ostream& operator<<(std::ostream& out, const Http2PriorityUpdateFields& v)
{
    return out << v.ToString();
}
