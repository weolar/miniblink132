...
{
    return v <= static_cast<uint32_t>(Http2FrameType::ALTSVC) || v == static_cast<uint32_t>(Http2FrameType::PRIORITY_UPDATE);
}
...
QUICHE_EXPORT inline std::ostream& operator<<(std::ostream& out, Http2FrameType v)
{
...
QUICHE_EXPORT std::string Http2FrameFlagsToString(Http2FrameType type, uint8_t flags);
...
QUICHE_EXPORT inline std::ostream& operator<<(std::ostream& out, Http2ErrorCode v)
{
...
{
    return 0 < v && v <= static_cast<uint32_t>(Http2SettingsParameter::MAX_HEADER_LIST_SIZE);
}
...
QUICHE_EXPORT std::string Http2SettingsParameterToString(Http2SettingsParameter v);
...
public:
    ...
    static constexpr uint32_t DefaultHeaderTableSize()
    {
        return 4096;
    }

    // Default value for ENABLE_PUSH.
    static constexpr bool DefaultEnablePush()
    {
        return true;
    }

    // Default value for INITIAL_WINDOW_SIZE.
    static constexpr uint32_t DefaultInitialWindowSize()
    {
        return 65535;
    }
    ...
    static constexpr uint32_t MaximumWindowSize()
    {
        return UInt31Mask();
    }

    // Default value for MAX_FRAME_SIZE.
    static constexpr uint32_t DefaultMaxFrameSize()
    {
        return 16384;
    }

    // Minimum value for MAX_FRAME_SIZE.
    static constexpr uint32_t MinimumMaxFrameSize()
    {
        return 16384;
    }

    // Maximum value for MAX_FRAME_SIZE.
    static constexpr uint32_t MaximumMaxFrameSize()
    {
        return (1 << 24) - 1;
    }
};
...
using InvalidHeaderSet = absl::flat_hash_set<absl::string_view, quiche::StringPieceCaseHash, quiche::StringPieceCaseEqual>;
