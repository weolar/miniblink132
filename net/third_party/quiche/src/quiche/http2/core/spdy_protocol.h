...
inline constexpr size_t kHttp2MaxControlFrameSendSize = kHttp2DefaultFramePayloadLimit - 1;
...
// The initial value for the maximum frame payload size as per the spec. This is
// the maximum control frame size we accept.
inline constexpr uint32_t kHttp2DefaultFrameSizeLimit = kHttp2DefaultFramePayloadLimit + kFrameHeaderSize;
...
inline constexpr int32_t kSpdyMaximumWindowSize = 0x7FFFFFFF; // Max signed 32bit int
...
QUICHE_EXPORT std::ostream& operator<<(std::ostream& out, SpdyKnownSettingsId id);
...
QUICHE_EXPORT std::ostream& operator<<(std::ostream& out, SpdyFrameType frame_type);
...
QUICHE_EXPORT bool IsValidHTTP2FrameStreamId(SpdyStreamId current_frame_stream_id, SpdyFrameType frame_type_field);
...
QUICHE_EXPORT bool ParseSettingsId(SpdySettingsId wire_setting_id, SpdyKnownSettingsId* setting_id);
...
inline constexpr size_t kSettingsOneSettingSize = sizeof(uint32_t) + sizeof(SpdySettingsId);
...
inline constexpr size_t kMaxFrameSizeLimit = kSpdyMaxFrameSizeLimit + kFrameHeaderSize;
...
template <typename StreamIdType> class QUICHE_EXPORT StreamPrecedence {
public:
    ...
    StreamPrecedence(StreamIdType parent_id, int weight, bool is_exclusive)
        : precedence_(Http2StreamDependency { parent_id, ClampHttp2Weight(weight), is_exclusive })
    {
    }
    ...
    {
        return is_spdy3_priority() ? absl::get<SpdyPriority>(precedence_) : Http2WeightToSpdy3Priority(absl::get<Http2StreamDependency>(precedence_).weight);
    }
    ...
    {
        return is_spdy3_priority() ? kHttp2RootStreamId : absl::get<Http2StreamDependency>(precedence_).parent_id;
    }
    ...
    int weight() const
    {
        return is_spdy3_priority() ? Spdy3PriorityToHttp2Weight(absl::get<SpdyPriority>(precedence_)) : absl::get<Http2StreamDependency>(precedence_).weight;
    }
    ...
    {
        return absl::holds_alternative<Http2StreamDependency>(precedence_) && absl::get<Http2StreamDependency>(precedence_).is_exclusive;
    }
...
private:
    struct QUICHE_EXPORT Http2StreamDependency {
        ...
        {
            return parent_id == other.parent_id && weight == other.weight && is_exclusive == other.is_exclusive;
        }
    };
...
protected:
    ...
    explicit SpdyFrameIR(SpdyStreamId stream_id)
...
public:
    ...
    const quiche::HttpHeaderBlock& header_block() const
    {
...
protected:
    SpdyFrameWithHeaderBlockIR(SpdyStreamId stream_id, quiche::HttpHeaderBlock header_block);
    SpdyFrameWithHeaderBlockIR(const SpdyFrameWithHeaderBlockIR&) = delete;
    SpdyFrameWithHeaderBlockIR& operator=(const SpdyFrameWithHeaderBlockIR&) = delete;
...
public:
    ...
    int padding_payload_len() const
    {
        return padding_payload_len_;
    }
...
public:
    ...
    void AddSetting(SpdySettingsId id, int32_t value)
    {
        values_[id] = value;
    }
...
public:
    ...
    // this SpdyGoAwayIR.
    SpdyGoAwayIR(SpdyStreamId last_good_stream_id, SpdyErrorCode error_code, absl::string_view description);

    // References description, doesn't copy it, so description must outlast
    // this SpdyGoAwayIR.
    SpdyGoAwayIR(SpdyStreamId last_good_stream_id, SpdyErrorCode error_code, const char* description);
    ...
    SpdyGoAwayIR(SpdyStreamId last_good_stream_id, SpdyErrorCode error_code, std::string description);
...
public:
    ...
    void set_has_priority(bool has_priority)
    {
        has_priority_ = has_priority;
    }
    int weight() const
    {
        return weight_;
    ...
    SpdyStreamId parent_stream_id() const
    {
        return parent_stream_id_;
    }
    void set_parent_stream_id(SpdyStreamId id)
    {
...
public:
    SpdyPushPromiseIR(SpdyStreamId stream_id, SpdyStreamId promised_stream_id)
        : SpdyPushPromiseIR(stream_id, promised_stream_id, quiche::HttpHeaderBlock())
    {
    }
    SpdyPushPromiseIR(SpdyStreamId stream_id, SpdyStreamId promised_stream_id, quiche::HttpHeaderBlock header_block)
        : SpdyFrameWithHeaderBlockIR(stream_id, std::move(header_block))
    ...
    SpdyStreamId promised_stream_id() const
    {
...
public:
    ...
    void set_end_headers(bool end_headers)
    {
    ...
    void take_encoding(std::string encoding)
    {
...
public:
    SpdyPriorityIR(SpdyStreamId stream_id, SpdyStreamId parent_stream_id, int weight, bool exclusive)
        : SpdyFrameIR(stream_id)
...
public:
    SpdyPriorityUpdateIR(SpdyStreamId stream_id, SpdyStreamId prioritized_stream_id, std::string priority_field_value)
        : SpdyFrameIR(stream_id)
    ...
    SpdyStreamId prioritized_stream_id() const
    {
        return prioritized_stream_id_;
    }
...
public:
    SpdyUnknownIR(SpdyStreamId stream_id, uint8_t type, uint8_t flags, std::string payload)
        : SpdyFrameIR(stream_id)
        , type_(type)
        , flags_(flags)
...
public:
    ...
    virtual void VisitPriorityUpdate(const SpdyPriorityUpdateIR& priority_update) = 0;
...
public:
    ...
    virtual void OnSendCompressedFrame(SpdyStreamId /*stream_id*/, SpdyFrameType /*type*/, size_t /*payload_len*/, size_t /*frame_len*/)
    {
    }
    ...
    // |frame_len| is the compressed frame size.
    virtual void OnReceiveCompressedFrame(SpdyStreamId /*stream_id*/, SpdyFrameType /*type*/, size_t /*frame_len*/)
    {
    }
};
...
size_t GetPushPromiseFrameSizeSansBlock(const SpdyPushPromiseIR& push_promise_ir);
