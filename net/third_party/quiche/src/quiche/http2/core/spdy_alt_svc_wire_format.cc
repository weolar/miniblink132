...
template <class T> bool ParsePositiveIntegerImpl(absl::string_view::const_iterator c, absl::string_view::const_iterator end, T* value)
{
    *value = 0;
...
SpdyAltSvcWireFormat::AlternativeService::AlternativeService(
    const std::string& protocol_id, const std::string& host, uint16_t port, uint32_t max_age_seconds, VersionVector version)
...
SpdyAltSvcWireFormat::AlternativeService::AlternativeService(const AlternativeService& other) = default;

// static
bool SpdyAltSvcWireFormat::ParseHeaderFieldValue(absl::string_view value, AlternativeServiceVector* altsvc_vector)
{
    ...
    while (c != value.end()) {
        // Parse protocol-id.
        absl::string_view::const_iterator percent_encoded_protocol_id_end = std::find(c, value.end(), '=');
        ...
        absl::string_view::const_iterator parameters_end = std::find(c, value.end(), ',');
        while (c != parameters_end) {
            ...
            if (parameter_name == "ma") {
                if (!ParsePositiveInteger32(parameter_value_begin, c, &max_age_seconds)) {
                    return false;
                }
                ...
                if (!HexDecodeToUInt32(absl::string_view(&*parameter_value_begin, c - parameter_value_begin), &quic_version) || quic_version == 0) {
                    return false;
                }
                version.push_back(quic_version);
            }
        }
        altsvc_vector->emplace_back(protocol_id, host, port, max_age_seconds, version);
...
// static
std::string SpdyAltSvcWireFormat::SerializeHeaderFieldValue(const AlternativeServiceVector& altsvc_vector)
{
    ...
    for (const AlternativeService& altsvc : altsvc_vector) {
        ...
        if (!altsvc.version.empty()) {
            if (is_ietf_format_quic) {
                ...
                value.append("; v=\"");
                for (auto it = altsvc.version.begin(); it != altsvc.version.end(); ++it) {
...
// static
void SpdyAltSvcWireFormat::SkipWhiteSpace(absl::string_view::const_iterator* c, absl::string_view::const_iterator end)
{
    for (; *c != end && (**c == ' ' || **c == '\t'); ++*c) {
    }
}
...
// static
bool SpdyAltSvcWireFormat::PercentDecode(absl::string_view::const_iterator c, absl::string_view::const_iterator end, std::string* output)
{
    output->clear();
    for (; c != end; ++c) {
        if (*c != '%') {
...
// static
bool SpdyAltSvcWireFormat::ParseAltAuthority(absl::string_view::const_iterator c, absl::string_view::const_iterator end, std::string* host, uint16_t* port)
{
    host->clear();
    if (c == end) {
        return false;
...
// static
bool SpdyAltSvcWireFormat::ParsePositiveInteger16(absl::string_view::const_iterator c, absl::string_view::const_iterator end, uint16_t* value)
{
    return ParsePositiveIntegerImpl<uint16_t>(c, end, value);
}

// static
bool SpdyAltSvcWireFormat::ParsePositiveInteger32(absl::string_view::const_iterator c, absl::string_view::const_iterator end, uint32_t* value)
{
...
// static
bool SpdyAltSvcWireFormat::HexDecodeToUInt32(absl::string_view data, uint32_t* value)
{
