...
#define HTTP2_TRACE_LOG(perspective, is_enabled) QUICHE_LOG_IF(INFO, is_enabled()) << "[HTTP2_TRACE " << perspective << "] "
...
public:
    ...
    Http2TraceLogger(SpdyFramerVisitorInterface* parent, absl::string_view perspective, quiche::MultiUseCallback<bool()> is_enabled, const void* connection_id);
    ...
    void OnCommonHeader(SpdyStreamId stream_id, size_t length, uint8_t type, uint8_t flags) override;
    spdy::SpdyHeadersHandlerInterface* OnHeaderFrameStart(SpdyStreamId stream_id) override;
    ...
    void OnContinuation(SpdyStreamId stream_id, size_t payload_length, bool end) override;
...
public:
    ...
    Http2FrameLogger(absl::string_view perspective, quiche::MultiUseCallback<bool()> is_enabled, const void* connection_id)
