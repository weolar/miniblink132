...
public:
    explicit PriorityWriteSchedulerPeer(PriorityWriteScheduler<StreamIdType>* scheduler)
...
public:
    static constexpr int kLowestPriority = PriorityWriteScheduler<SpdyStreamId>::kLowestPriority;
...
TEST_F(PriorityWriteSchedulerTest, RegisterUnregisterStreams)
{
    ...
    EXPECT_QUICHE_BUG(scheduler_.RegisterStream(1, 1), "Stream 1 already registered");
    EXPECT_EQ(1u, scheduler_.NumRegisteredStreams());

    EXPECT_QUICHE_BUG(scheduler_.RegisterStream(1, 2), "Stream 1 already registered");
    EXPECT_EQ(1u, scheduler_.NumRegisteredStreams());
...
TEST_F(PriorityWriteSchedulerTest, GetStreamPriority)
{
    ...
    EXPECT_QUICHE_BUG(scheduler_.RegisterStream(1, 4), "Stream 1 already registered");
    EXPECT_EQ(3, scheduler_.GetStreamPriority(1));
...
{
    EXPECT_FALSE(scheduler_.HasReadyStreams());
    EXPECT_QUICHE_BUG(scheduler_.MarkStreamReady(1, false), "Stream 1 not registered");
    EXPECT_FALSE(scheduler_.HasReadyStreams());
    EXPECT_QUICHE_BUG(EXPECT_EQ(0u, scheduler_.PopNextReadyStream()), "No ready streams available");
...
{
    EXPECT_FALSE(scheduler_.HasReadyStreams());
    EXPECT_QUICHE_BUG(scheduler_.MarkStreamReady(1, true), "Stream 1 not registered");
    EXPECT_FALSE(scheduler_.HasReadyStreams());
    EXPECT_QUICHE_BUG(EXPECT_EQ(0u, scheduler_.PopNextReadyStream()), "No ready streams available");
...
TEST_F(PriorityWriteSchedulerTest, MarkStreamNotReady)
{
    ...
    EXPECT_QUICHE_BUG(scheduler_.MarkStreamNotReady(3), "Stream 3 not registered");
}
...
{
    EXPECT_QUICHE_BUG(scheduler_.RecordStreamEventTime(3, absl::FromUnixMicros(5)), "Stream 3 not registered");
    EXPECT_QUICHE_BUG(EXPECT_FALSE(scheduler_.GetLatestEventWithPriority(4).has_value()), "Stream 4 not registered");
    ...
    for (int i = 2; i < 5; ++i) {
        EXPECT_THAT(scheduler_.GetLatestEventWithPriority(i), Optional(Eq(absl::FromUnixMicros((i - 1) * 100))));
    }
}
