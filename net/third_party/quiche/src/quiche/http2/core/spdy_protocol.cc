...
const char* const kHttp2ConnectionHeaderPrefix = "PRI * HTTP/2.0\r\n\r\nSM\r\n\r\n";
...
SpdyPriority ClampSpdy3Priority(SpdyPriority priority)
{
    ...
    if (priority > kV3LowestPriority) {
        QUICHE_BUG(spdy_bug_22_1) << "Invalid priority: " << static_cast<int>(priority);
...
{
    QUICHE_BUG_IF(spdy_bug_22_4, !IsDefinedFrameType(frame_type_field)) << "Frame type not defined: " << static_cast<int>(frame_type_field);
...
bool IsValidHTTP2FrameStreamId(SpdyStreamId current_frame_stream_id, SpdyFrameType frame_type_field)
{
...
bool ParseSettingsId(SpdySettingsId wire_setting_id, SpdyKnownSettingsId* setting_id)
{
    if (wire_setting_id != SETTINGS_EXPERIMENT_SCHEDULER && (wire_setting_id < SETTINGS_MIN || wire_setting_id > SETTINGS_MAX)) {
        return false;
    }
...
SpdyFrameWithHeaderBlockIR::SpdyFrameWithHeaderBlockIR(SpdyStreamId stream_id, quiche::HttpHeaderBlock header_block)
...
{
    return kFrameHeaderSize + (padded() ? 1 + padding_payload_len() + data_len() : data_len());
}

SpdyRstStreamIR::SpdyRstStreamIR(SpdyStreamId stream_id, SpdyErrorCode error_code)
...
SpdyGoAwayIR::SpdyGoAwayIR(SpdyStreamId last_good_stream_id, SpdyErrorCode error_code, absl::string_view description)
...
SpdyGoAwayIR::SpdyGoAwayIR(SpdyStreamId last_good_stream_id, SpdyErrorCode error_code, const char* description)
    : SpdyGoAwayIR(last_good_stream_id, error_code, absl::string_view(description))
{
}

SpdyGoAwayIR::SpdyGoAwayIR(SpdyStreamId last_good_stream_id, SpdyErrorCode error_code, std::string description)
...
size_t SpdyHeadersIR::size() const
{
    ...
    size += header_block().TotalBytesUsed() + header_block().size() * kPerHeaderHpackOverhead;
    if (size > kHttp2MaxControlFrameSendSize) {
        size += GetNumberRequiredContinuationFrames(size) * kContinuationFrameMinimumSize;
    }
    return size;
}
...
size_t SpdyAltSvcIR::size() const
{
    ...
    std::string str = SpdyAltSvcWireFormat::SerializeHeaderFieldValue(altsvc_vector_);
    size += str.size();
    return size;
}
...
size_t SpdyAcceptChIR::size() const
{
    ...
    for (const AcceptChOriginValuePair& entry : entries_) {
        total_size += entry.origin.size() + entry.value.size() + kAcceptChFramePerEntryOverhead;
    }
    return total_size;
}
