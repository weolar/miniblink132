...
namespace test {
template <typename StreamIdType> class PriorityWriteSchedulerPeer;
}
...
//
template <typename StreamIdType, typename PriorityType = spdy::SpdyPriority, typename PriorityTypeToInt = SpdyPriorityToSpdyPriority,
...
public:
    ...
    {
        auto stream_info = std::make_unique<StreamInfo>(StreamInfo { std::move(priority), stream_id, false });
        bool inserted = stream_infos_.insert(std::make_pair(stream_id, std::move(stream_info))).second;
        QUICHE_BUG_IF(spdy_bug_19_2, !inserted) << "Stream " << stream_id << " already registered";
    }
    ...
    void UnregisterStream(StreamIdType stream_id)
    {
        ...
        if (stream_info->ready) {
            bool erased = Erase(&priority_infos_[PriorityTypeToInt()(stream_info->priority)].ready_list, stream_info);
    ...
    void UpdateStreamPriority(StreamIdType stream_id, PriorityType priority)
    {
        ...
        // value changes.
        if (PriorityTypeToInt()(stream_info->priority) != PriorityTypeToInt()(priority) && stream_info->ready) {
            bool erased = Erase(&priority_infos_[PriorityTypeToInt()(stream_info->priority)].ready_list, stream_info);
            QUICHE_DCHECK(erased);
            priority_infos_[PriorityTypeToInt()(priority)].ready_list.push_back(stream_info);
            ++num_ready_streams_;
        }
    ...
    void RecordStreamEventTime(StreamIdType stream_id, absl::Time now)
    {
        ...
        PriorityInfo& priority_info = priority_infos_[PriorityTypeToInt()(it->second->priority)];
        priority_info.last_event_time = std::max(priority_info.last_event_time, absl::make_optional(now));
    }
    ...
    //
    // Preconditions: `stream_id` should be registered.
    std::optional<absl::Time> GetLatestEventWithPriority(StreamIdType stream_id) const
    {
        auto it = stream_infos_.find(stream_id);
        if (it == stream_infos_.end()) {
        ...
        const StreamInfo* const stream_info = it->second.get();
        for (int p = kHighestPriority; p < PriorityTypeToInt()(stream_info->priority); ++p) {
            last_event_time = std::max(last_event_time, priority_infos_[p].last_event_time);
        }
    ...
    std::tuple<StreamIdType, PriorityType> PopNextReadyStreamAndPriority()
    {
        for (int p = kHighestPriority; p <= kLowestPriority; ++p) {
            ...
            if (!ready_list.empty()) {
                ...
                QUICHE_DCHECK(stream_infos_.find(info->stream_id) != stream_infos_.end());
                info->ready = false;
    ...
    bool ShouldYield(StreamIdType stream_id) const
    {
        ...
        // no need to yield.
        const auto& ready_list = priority_infos_[PriorityTypeToInt()(it->second->priority)].ready_list;
    ...
    void MarkStreamReady(StreamIdType stream_id, bool add_to_front)
    {
        ...
        ReadyList& ready_list = priority_infos_[PriorityTypeToInt()(stream_info->priority)].ready_list;
        if (add_to_front) {
    ...
    {
        return absl::StrCat("PriorityWriteScheduler {num_streams=", stream_infos_.size(), " num_ready_streams=", NumReadyStreams(), "}");
    }
...
private:
    ...
    using StreamInfoMap = absl::flat_hash_map<StreamIdType, std::unique_ptr<StreamInfo>>;
