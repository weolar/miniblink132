...
template <typename StreamIdType> class QUICHE_EXPORT_PRIVATE WriteScheduler {
public:
    ...
    virtual void RegisterStream(StreamIdType stream_id, const StreamPrecedenceType& precedence) = 0;
    ...
    //
    // Preconditions: |stream_id| should be registered.
    virtual StreamPrecedenceType GetStreamPrecedence(StreamIdType stream_id) const = 0;
    ...
    // Preconditions: |stream_id| should be unregistered, and
    // |precedence.parent_id()| should be registered or |kHttp2RootStreamId|.
    virtual void UpdateStreamPrecedence(StreamIdType stream_id, const StreamPrecedenceType& precedence) = 0;
    ...
    //
    // Preconditions: |stream_id| should be registered.
    virtual std::vector<StreamIdType> GetStreamChildren(StreamIdType stream_id) const = 0;

    // Records time (in microseconds) of a read/write event for the given
    // stream.
    //
    // Preconditions: |stream_id| should be registered.
    virtual void RecordStreamEventTime(StreamIdType stream_id, int64_t now_in_usec) = 0;
    ...
    //
    // Preconditions: |stream_id| should be registered.
    virtual int64_t GetLatestEventWithPrecedence(StreamIdType stream_id) const = 0;
    ...
    //
    // Preconditions: |HasReadyStreams() == true|
    virtual std::tuple<StreamIdType, StreamPrecedenceType> PopNextReadyStreamAndPrecedence() = 0;
