...
public:
    ...
    using HeaderListener = quiche::MultiUseCallback<void(absl::string_view, absl::string_view)>;
    ...
    using IndexingPolicy = quiche::MultiUseCallback<bool(absl::string_view, absl::string_view)>;
    ...
    std::unique_ptr<ProgressiveEncoder> EncodeHeaderSet(const quiche::HttpHeaderBlock& header_set);
    ...
    std::unique_ptr<ProgressiveEncoder> EncodeRepresentations(const Representations& representations);
...
private:
    ...
    void EmitNonIndexedLiteral(const Representation& representation, bool enable_compression);
    ...
    static void CookieToCrumbs(const Representation& cookie, Representations* crumbs_out);

    // Crumbles other header field values at \0 delimiters.
    static void DecomposeRepresentation(const Representation& header_field, Representations* out);
