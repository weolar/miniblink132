...
uint64_t HiValueOfExtensionBytes(uint32_t extension_bytes, uint32_t prefix_length)
{
    return (1 << prefix_length) - 2 + (extension_bytes == 0 ? 0 : (1LLU << (extension_bytes * 7)));
}
...
protected:
    ...
    void DecodeSeveralWays(uint64_t expected_value, uint32_t expected_offset)
    {
        ...
        Validator validator = [expected_value, this](const DecodeBuffer& /*db*/, DecodeStatus /*status*/) -> AssertionResult {
            if (decoder_.value() != expected_value) {
                return AssertionFailure() << "Value doesn't match expected: " << decoder_.value() << " != " << expected_value;
            }
        ...
        EXPECT_TRUE(DecodeAndValidateSeveralWays(&b, return_non_zero_on_first, validator));
    ...
    void EncodeNoRandom(uint64_t value, uint8_t prefix_length)
    {
        ...
        ASSERT_EQ(static_cast<uint8_t>(buffer_[0]), static_cast<uint8_t>(buffer_[0]) & prefix_mask);
    }
    ...
    // range desired.
    void ValidateEncoding(uint64_t value, uint64_t minimum, uint64_t maximum, size_t expected_bytes)
    {
    ...
    void EncodeAndDecodeValues(const std::set<uint64_t>& values, uint8_t prefix_length, size_t expected_bytes)
    {
        ...
        for (const uint64_t value : values) {
            ...
            std::string msg = absl::StrCat("value=", value, " (0x", absl::Hex(value), "), prefix_length=", prefix_length, ", expected_bytes=", expected_bytes,
            ...
            if (1 < expected_bytes) {
                ...
                for (uint8_t total_bytes = expected_bytes + 1; total_bytes <= 6; ++total_bytes) {
    ...
    void EncodeAndDecodeValuesInRange(uint64_t start, uint64_t range, uint8_t prefix_length, size_t expected_bytes)
    {
        ...
        QUICHE_LOG(INFO) << "prefix_mask=" << std::hex << static_cast<int>(prefix_mask);
        ...
        if (expected_bytes < 11) {
            ...
            EXPECT_EQ(expected_bytes + 1, buffer_.size()) << quiche::QuicheTextUtils::HexDump(buffer_);
        }
...
TEST_F(HpackVarintRoundTripTest, Encode)
{
    for (int prefix_length = 3; prefix_length <= 8; ++prefix_length) {
        ...
        QUICHE_LOG(INFO) << "############################################################";
        QUICHE_LOG(INFO) << "prefix_length=" << prefix_length << "   a=" << a << "   b=" << b << "   c=" << c << "   d=" << d << "   e=" << e << "   f=" << f
                         << "   g=" << g << "   h=" << h << "   i=" << i << "   j=" << j;
        ...
        for (uint64_t value : values) {
            ...
            QUICHE_LOG(INFO) << absl::StrFormat("%10llu %0#18x ", value, value) << quiche::QuicheTextUtils::HexDump(buffer_).substr(7);
        }
    }
}
