...
#ifndef NDEBUG
const uint8_t kHpackVarintDecoderOffsetDone = std::numeric_limits<uint8_t>::max();
#endif
...
public:
    ...
    DecodeStatus Start(uint8_t prefix_value, uint8_t prefix_length, DecodeBuffer* db);
    ...
    DecodeStatus StartForTest(uint8_t prefix_value, uint8_t prefix_length, DecodeBuffer* db);
