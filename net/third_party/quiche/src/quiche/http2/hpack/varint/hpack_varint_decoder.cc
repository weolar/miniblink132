...
DecodeStatus HpackVarintDecoder::Start(uint8_t prefix_value, uint8_t prefix_length, DecodeBuffer* db)
{
...
DecodeStatus HpackVarintDecoder::StartExtended(uint8_t prefix_length, DecodeBuffer* db)
{
    QUICHE_DCHECK_LE(3u, prefix_length);
    QUICHE_DCHECK_LE(prefix_length, 8u);
...
DecodeStatus HpackVarintDecoder::Resume(DecodeBuffer* db)
{
    ...
    QUICHE_DLOG(WARNING) << "Variable length int encoding is too large or too long. " << DebugString();
    MarkDone();
...
DecodeStatus HpackVarintDecoder::StartForTest(uint8_t prefix_value, uint8_t prefix_length, DecodeBuffer* db)
{
    return Start(prefix_value, prefix_length, db);
}

DecodeStatus HpackVarintDecoder::StartExtendedForTest(uint8_t prefix_length, DecodeBuffer* db)
{
