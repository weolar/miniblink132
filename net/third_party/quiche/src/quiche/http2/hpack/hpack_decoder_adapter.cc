...
void HpackDecoderAdapter::HandleControlFrameHeadersStart(SpdyHeadersHandlerInterface* handler)
{
...
bool HpackDecoderAdapter::HandleControlFrameHeadersData(const char* headers_data, size_t headers_data_length)
{
    QUICHE_DVLOG(2) << "HpackDecoderAdapter::HandleControlFrameHeadersData: len=" << headers_data_length;
    ...
    if (headers_data_length > 0) {
        ...
        if (headers_data_length > max_decode_buffer_size_bytes_) {
            QUICHE_DVLOG(1) << "max_decode_buffer_size_bytes_ < headers_data_length: " << max_decode_buffer_size_bytes_ << " < " << headers_data_length;
            error_ = http2::HpackDecodingError::kFragmentTooLong;
            return false;
        }
        listener_adapter_.AddToTotalHpackBytes(headers_data_length);
        if (max_header_block_bytes_ != 0 && listener_adapter_.total_hpack_bytes() > max_header_block_bytes_) {
...
void HpackDecoderAdapter::set_max_decode_buffer_size_bytes(size_t max_decode_buffer_size_bytes)
{
...
void HpackDecoderAdapter::set_max_header_block_bytes(size_t max_header_block_bytes)
{
    max_header_block_bytes_ = max_header_block_bytes;
}

HpackDecoderAdapter::ListenerAdapter::ListenerAdapter()
    : no_op_handler_(nullptr)
    , handler_(&no_op_handler_)
{
}
HpackDecoderAdapter::ListenerAdapter::~ListenerAdapter() = default;

void HpackDecoderAdapter::ListenerAdapter::set_handler(SpdyHeadersHandlerInterface* handler)
{
...
void HpackDecoderAdapter::ListenerAdapter::OnHeader(absl::string_view name, absl::string_view value)
{
...
void HpackDecoderAdapter::ListenerAdapter::OnHeaderErrorDetected(absl::string_view error_message)
{
