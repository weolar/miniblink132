...
public:
    ...
    {
        return encoder_->enable_compression_;
    }
    ...
    static void CookieToCrumbs(absl::string_view cookie, std::vector<absl::string_view>* out)
    {
        Representations tmp;
    ...
    static void DecomposeRepresentation(absl::string_view value, std::vector<absl::string_view>* out)
    {
        Representations tmp;
        HpackEncoder::DecomposeRepresentation(std::make_pair("foobar", value), &tmp);
    ...
    static std::string EncodeHeaderBlock(HpackEncoder* encoder, const quiche::HttpHeaderBlock& header_set)
    {
        return encoder->EncodeHeaderBlock(header_set);
    }

    static bool EncodeIncremental(HpackEncoder* encoder, const quiche::HttpHeaderBlock& header_set, std::string* output)
    {
        std::unique_ptr<HpackEncoder::ProgressiveEncoder> encoderator = encoder->EncodeHeaderSet(header_set);
        ...
        while (encoderator->HasNext()) {
            std::string second_buffer = encoderator->Next(random.UniformInRange(0, 16));
    ...
    static bool EncodeRepresentations(HpackEncoder* encoder, const Representations& representations, std::string* output)
    {
        std::unique_ptr<HpackEncoder::ProgressiveEncoder> encoderator = encoder->EncodeRepresentations(representations);
        http2::test::Http2Random random;
        std::string output_buffer = encoderator->Next(random.UniformInRange(0, 16));
        while (encoderator->HasNext()) {
            std::string second_buffer = encoderator->Next(random.UniformInRange(0, 16));
...
class HpackEncoderTest : public quiche::test::QuicheTestWithParam<EncodeStrategy> {
protected:
    ...
    {
        absl::string_view n(headers_storage_.Memdup(name.data(), name.size()), name.size());
        absl::string_view v(headers_storage_.Memdup(value.data(), value.size()), value.size());
    ...
    void ExpectNonIndexedLiteral(absl::string_view name, absl::string_view value)
    {
    ...
    void ExpectNonIndexedLiteralWithNameIndex(size_t key_index, absl::string_view value)
    {
        expected_.AppendPrefix(kLiteralNoIndexOpcode);
        expected_.AppendUint32(key_index);
        ExpectString(&expected_, value);
    ...
    {
        size_t encoded_size = peer_.compression_enabled() ? http2::HuffmanSize(str) : str.size();
    ...
    Representations MakeRepresentations(const quiche::HttpHeaderBlock& header_set)
    {
        Representations r;
    ...
    void CompareWithExpectedEncoding(const quiche::HttpHeaderBlock& header_set)
    {
        ...
        switch (strategy_) {
        case kDefault:
            actual_out = test::HpackEncoderPeer::EncodeHeaderBlock(&encoder_, header_set);
            break;
        case kIncremental:
            EXPECT_TRUE(test::HpackEncoderPeer::EncodeIncremental(&encoder_, header_set, &actual_out));
            break;
        case kRepresentations:
            EXPECT_TRUE(test::HpackEncoderPeer::EncodeRepresentations(&encoder_, MakeRepresentations(header_set), &actual_out));
            break;
        }
    ...
    void CompareWithExpectedEncoding(const Representations& representations)
    {
        std::string actual_out;
        std::string expected_out = expected_.TakeString();
        EXPECT_TRUE(test::HpackEncoderPeer::EncodeRepresentations(&encoder_, representations, &actual_out));
        EXPECT_EQ(expected_out, actual_out);
    }
    ...
    std::vector<std::pair<absl::string_view, absl::string_view>> headers_observed_;
...
INSTANTIATE_TEST_SUITE_P(HpackEncoderTests, HpackEncoderTestWithDefaultStrategy, ::testing::Values(kDefault));
...
TEST_P(HpackEncoderTestWithDefaultStrategy, EncodeRepresentations)
{
    ...
    encoder_.SetHeaderListener([this](absl::string_view name, absl::string_view value) { this->SaveHeaders(name, value); });
    const std::vector<std::pair<absl::string_view, absl::string_view>> header_list = { { "cookie", "val1; val2;val3" }, { ":path", "/home" },
        { "accept", "text/html, text/plain,application/xml" }, { "cookie", "val4" }, { "withnul", absl::string_view("one\0two", 7) } };
    ExpectNonIndexedLiteralWithNameIndex(peer_.table()->GetByName(":path"), "/home");
    ...
    ExpectIndexedLiteral(peer_.table()->GetByName("accept"), "text/html, text/plain,application/xml");
    ...
    EXPECT_THAT(headers_observed_,
        ElementsAre(Pair(":path", "/home"), Pair("cookie", "val1"), Pair("cookie", "val2"), Pair("cookie", "val3"),
            Pair("accept", "text/html, text/plain,application/xml"), Pair("cookie", "val4"), Pair("withnul", absl::string_view("one\0two", 7))));
...
TEST_P(HpackEncoderTestWithDefaultStrategy, WithoutCookieCrumbling)
{
    ...
    CompareWithExpectedEncoding(header_list);
    EXPECT_THAT(headers_observed_,
        ElementsAre(Pair(":path", "/home"), Pair("cookie", "val1; val2;val3"), Pair("accept", "text/html, text/plain,application/xml"), Pair("cookie", "val4"),
...
TEST_P(HpackEncoderTestWithDefaultStrategy, DynamicTableGrows)
{
    ...
    const std::vector<std::pair<absl::string_view, absl::string_view>> header_list = { { "cookie", "val1; val2;val3" }, { ":path", "/home" },
        { "accept", "text/html, text/plain,application/xml" }, { "cookie", "val4" }, { "withnul", absl::string_view("one\0two", 7) } };
    std::string out;
    EXPECT_TRUE(test::HpackEncoderPeer::EncodeRepresentations(&encoder_, header_list, &out));
...
INSTANTIATE_TEST_SUITE_P(HpackEncoderTests, HpackEncoderTest, ::testing::Values(kDefault, kIncremental, kRepresentations));
...
TEST_P(HpackEncoderTest, SingleDynamicIndex)
{
    ...
    EXPECT_THAT(headers_observed_, ElementsAre(Pair(key_2_->name(), key_2_->value())));
}
...
TEST_P(HpackEncoderTest, EncodingWithoutCompression)
{
    ...
    if (strategy_ == kRepresentations) {
        EXPECT_THAT(headers_observed_,
            ElementsAre(Pair(":path", "/index.html"), Pair("cookie", "foo=bar"), Pair("cookie", "baz=bing"),
                Pair("hello", absl::string_view("goodbye\0aloha", 13)), Pair("multivalue", "value1, value2")));
    } else {
        EXPECT_THAT(headers_observed_,
            ElementsAre(Pair(":path", "/index.html"), Pair("cookie", "foo=bar"), Pair("cookie", "baz=bing"), Pair("hello", "goodbye"), Pair("hello", "aloha"),
...
TEST_P(HpackEncoderTest, MultipleEncodingPasses)
{
    ...
    // clang-format on
    EXPECT_THAT(headers_observed_,
        ElementsAre(Pair("key1", "value1"), Pair("cookie", "a=bb"), Pair("key2", "value2"), Pair("cookie", "c=dd"), Pair("cookie", "e=ff"),
            Pair("key2", "value2"), Pair("cookie", "a=bb"), Pair("cookie", "b=cc"), Pair("cookie", "c=dd")));
    // clang-format on
}
...
TEST_P(HpackEncoderTest, PseudoHeadersFirst)
{
    ...
    ExpectNonIndexedLiteralWithNameIndex(peer_.table()->GetByName(":path"), "/spam/eggs.html");
    ExpectIndexedLiteral(peer_.table()->GetByName(":authority"), "www.example.com");
