...
protected:
    ...
    AssertionResult TranscodeAndValidateSeveralWays(absl::string_view plain, absl::string_view expected_huffman)
    {
        ...
        return DecodeAndValidateSeveralWays(&db, return_non_zero_on_first, ValidateDoneAndEmpty(validator));
    }
...
TEST_F(HpackHuffmanTranscoderTest, RoundTripRandomAsciiNonControlString)
{
    for (size_t length = 0; length != 20; length++) {
        ...
        ASSERT_TRUE(TranscodeAndValidateSeveralWays(s)) << "Unable to decode:\n\n"
...
class HpackHuffmanTranscoderAdjacentCharTest : public HpackHuffmanTranscoderTest, public testing::WithParamInterface<int> {
protected:
...
INSTANTIATE_TEST_SUITE_P(HpackHuffmanTranscoderAdjacentCharTest, HpackHuffmanTranscoderAdjacentCharTest, Range(0, 256));
...
class HpackHuffmanTranscoderRepeatedCharTest : public HpackHuffmanTranscoderTest, public testing::WithParamInterface<std::tuple<int, int>> {
protected:
...
INSTANTIATE_TEST_SUITE_P(HpackHuffmanTranscoderRepeatedCharTest, HpackHuffmanTranscoderRepeatedCharTest, Combine(Range(0, 256), Values(1, 2, 3, 4, 8, 16, 32)));
