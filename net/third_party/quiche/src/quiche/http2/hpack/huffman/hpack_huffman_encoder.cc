...
void HuffmanEncode(absl::string_view input, size_t encoded_size, std::string* output)
{
    ...
    for (uint8_t c : input) {
        ...
        uint64_t code = static_cast<uint64_t>(HuffmanSpecTables::kLeftCodes[c]) << (8 - (bit_counter % 8));
