...
public:
    ...
    std::vector<HpackEntry*> EvictionSet(absl::string_view name, absl::string_view value)
    {
    ...
    size_t EvictionCountForEntry(absl::string_view name, absl::string_view value)
    {
...
TEST_F(HpackHeaderTableTest, StaticTableInitialization)
{
    ...
    const HpackHeaderTable::StaticEntryTable& static_entries = peer_.static_entries();
...
TEST_F(HpackHeaderTableTest, EntryIndexing)
{
    ...
    EXPECT_EQ(1u, table_.GetByNameAndValue(first_static_entry->name(), first_static_entry->value()));
    ...
    EXPECT_EQ(1u, table_.GetByNameAndValue(first_static_entry->name(), first_static_entry->value()));
    EXPECT_EQ(67u, table_.GetByNameAndValue(first_static_entry->name(), "Value Four"));
    ...
    EXPECT_EQ(1u, table_.GetByNameAndValue(first_static_entry->name(), first_static_entry->value()));
    EXPECT_EQ(67u, table_.GetByNameAndValue(first_static_entry->name(), "Value Four"));
    EXPECT_EQ(kHpackEntryNotFound, table_.GetByNameAndValue("key-1", "Not Present"));
    EXPECT_EQ(kHpackEntryNotFound, table_.GetByNameAndValue("not-present", "Value One"));
    ...
    peer_.Evict(1);
    EXPECT_EQ(kHpackEntryNotFound, table_.GetByNameAndValue(first_static_entry->name(), "Value Four"));
...
TEST_F(HpackHeaderTableTest, TryAddEntryEviction)
{
    ...
    EXPECT_EQ(peer_.dynamic_entries().size() - 1, peer_.EvictionSet(long_entry.name(), long_entry.value()).size());

    table_.TryAddEntry(long_entry.name(), long_entry.value());
    EXPECT_EQ(2u, peer_.dynamic_entries().size());
    EXPECT_EQ(63u, table_.GetByNameAndValue(survivor_entry->name(), survivor_entry->value()));
    EXPECT_EQ(62u, table_.GetByNameAndValue(long_entry.name(), long_entry.value()));
}
...
TEST_F(HpackHeaderTableTest, TryAddTooLargeEntry)
{
    ...
    EXPECT_EQ(peer_.dynamic_entries().size(), peer_.EvictionSet(long_entry.name(), long_entry.value()).size());
