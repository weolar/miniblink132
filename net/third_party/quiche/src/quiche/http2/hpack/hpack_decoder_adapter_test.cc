...
public:
    ...
    void HandleHeaderRepresentation(const std::string& name, const std::string& value)
    {
...
public:
    static void CookieToCrumbs(const HpackEncoder::Representation& cookie, HpackEncoder::Representations* crumbs_out)
    {
...
protected:
    ...
    void SetUp() override
    {
        randomly_split_input_buffer_ = GetParam();
    }
    ...
    bool DecodeHeaderBlock(absl::string_view str, bool check_decoded_size = true)
    {
        ...
        if (check_decoded_size) {
            EXPECT_EQ(handler_.uncompressed_header_bytes(), SizeOfHeaders(decoded_block()));
        }
        return true;
    }
    ...
    const quiche::HttpHeaderBlock& DecodeBlockExpectingSuccess(absl::string_view str)
    {
        EXPECT_TRUE(DecodeHeaderBlock(str));
        return decoded_block();
    }

    void expectEntry(size_t index, size_t size, const std::string& name, const std::string& value)
    {
    ...
    quiche::HttpHeaderBlock MakeHeaderBlock(const std::vector<std::pair<std::string, std::string>>& headers)
    {
...
INSTANTIATE_TEST_SUITE_P(WithHandler, HpackDecoderAdapterTest, ::testing::Bool());
...
TEST_P(HpackDecoderAdapterTest, AddHeaderDataWithHandleControlFrameHeadersData)
{
    ...
    HpackBlockBuilder hbb;
    hbb.AppendLiteralNameAndValue(HpackEntryType::kNeverIndexedLiteralHeader, false, "a", false, a_value);
...
TEST_P(HpackDecoderAdapterTest, NameTooLong)
{
    ...
    HpackBlockBuilder hbb;
    hbb.AppendLiteralNameAndValue(HpackEntryType::kNeverIndexedLiteralHeader, false, name, false, value);
...
TEST_P(HpackDecoderAdapterTest, HeaderBlockTooLong)
{
    ...
    HpackBlockBuilder hbb;
    hbb.AppendLiteralNameAndValue(HpackEntryType::kIndexedLiteralHeader, false, name, false, value);
    while (hbb.size() < kMaxBufferSizeBytes) {
        hbb.AppendLiteralNameAndValue(HpackEntryType::kIndexedLiteralHeader, false, "", false, "");
    }
...
TEST_P(HpackDecoderAdapterTest, DecodeWithIncompleteData)
{
    ...
    std::vector<std::pair<std::string, std::string>> expected_headers = { { ":method", "GET" }, { ":path", "/index.html" }, { ":method", "GET" } };
    ...
    EXPECT_TRUE(HandleControlFrameHeadersData("\x40\x03goo"
...
TEST_P(HpackDecoderAdapterTest, HandleHeaderRepresentation)
{
    ...
    decoder_peer_.HandleHeaderRepresentation("passed-through", std::string("foo\0baz", 7));
    ...
    EXPECT_THAT(decoded_block(),
        ElementsAre(Pair("cookie", " part 1; part 2 ; part3;  fin!"), Pair("passed-through", absl::string_view("foo\0baz", 7)),
            Pair("joined", absl::string_view("joined\0value 1\0value 2", 22)), Pair("empty", ""), Pair("empty-joined", absl::string_view("\0foo\0\0", 6))));
}
...
TEST_P(HpackDecoderAdapterTest, IndexedHeaderStatic)
{
    ...
    const quiche::HttpHeaderBlock& header_set1 = DecodeBlockExpectingSuccess("\x82\x85");
    ...
    const quiche::HttpHeaderBlock& header_set2 = DecodeBlockExpectingSuccess("\x82");
...
TEST_P(HpackDecoderAdapterTest, IndexedHeaderDynamic)
{
    ...
    const quiche::HttpHeaderBlock& header_set1 = DecodeBlockExpectingSuccess("\x40\x03"
                                                                             "foo"
                                                                             "\x03"
                                                                             "bar");
    quiche::HttpHeaderBlock expected_header_set1;
    ...
    const quiche::HttpHeaderBlock& header_set2 = DecodeBlockExpectingSuccess("\xbe\x40\x04"
                                                                             "spam"
                                                                             "\x04"
                                                                             "eggs");
    quiche::HttpHeaderBlock expected_header_set2;
    ...
    const quiche::HttpHeaderBlock& header_set3 = DecodeBlockExpectingSuccess("\xbe");
...
{
    EXPECT_EQ(kDefaultHeaderTableSizeSetting, decoder_peer_.header_table_size_limit());
    std::string input;
    {
...
TEST_P(HpackDecoderAdapterTest, LiteralHeaderNoIndexing)
{
    ...
    const quiche::HttpHeaderBlock& header_set = DecodeBlockExpectingSuccess(absl::string_view(input, ABSL_ARRAYSIZE(input) - 1));
...
TEST_P(HpackDecoderAdapterTest, BasicC31)
{
    ...
    std::string encoded_header_set = encoder.EncodeHeaderBlock(expected_header_set);
...
TEST_P(HpackDecoderAdapterTest, SectionC4RequestHuffmanExamples)
{
    ...
    //                                         | -> :authority: www.example.com
    std::string first;
    ASSERT_TRUE(absl::HexStringToBytes("828684418cf1e3c2e5f23a6ba0ab90f4ff", &first));
    const quiche::HttpHeaderBlock& first_header_set = DecodeBlockExpectingSuccess(first);

    EXPECT_THAT(first_header_set,
        ElementsAre(
            // clang-format on
            Pair(":method", "GET"), Pair(":scheme", "http"), Pair(":path", "/"), Pair(":authority", "www.example.com")));
    // clang-format on
    ...
    const quiche::HttpHeaderBlock& second_header_set = DecodeBlockExpectingSuccess(second);
    ...
    std::string third;
    ASSERT_TRUE(absl::HexStringToBytes("828785bf408825a849e95ba97d7f8925a849e95bb8e8b4bf", &third));
    const quiche::HttpHeaderBlock& third_header_set = DecodeBlockExpectingSuccess(third);
...
TEST_P(HpackDecoderAdapterTest, SectionC6ResponseHuffmanExamples)
{
    ...
    std::string first;
    ASSERT_TRUE(absl::HexStringToBytes("488264025885aec3771a4b6196d07abe941054d444a8200595040b8166e082a62d1bff6e"
    ...
    std::string third;
    ASSERT_TRUE(absl::HexStringToBytes("88c16196d07abe941054d444a8200595040b8166e084a62d1bffc05a839bd9ab77ad94e7"
...
TEST_P(HpackDecoderAdapterTest, ReuseNameOfEvictedEntry)
{
    ...
    hbb.AppendLiteralNameAndValue(HpackEntryType::kIndexedLiteralHeader, false, name, false, value1);
    ...
    // first.
    hbb.AppendNameIndexAndLiteralValue(HpackEntryType::kIndexedLiteralHeader, 62, false, value2);
    ...
    // second.
    hbb.AppendNameIndexAndLiteralValue(HpackEntryType::kIndexedLiteralHeader, 62, false, value3);
    ...
    EXPECT_EQ(joined_values.size(), 2 * value1.size() + 2 * value2.size() + 2 * value3.size() + 5);

    EXPECT_EQ(expected_header_set, decoded_block());

    EXPECT_EQ(handler_.uncompressed_header_bytes(), 6 * name.size() + 2 * value1.size() + 2 * value2.size() + 2 * value3.size());
}
