...
std::string HpackEncoder::EncodeHeaderBlock(const quiche::HttpHeaderBlock& header_set)
{
...
{
    std::vector<absl::string_view> pieces = absl::StrSplit(header_field.second, '\0');
...
HpackEncoder::Encoderator::Encoderator(const quiche::HttpHeaderBlock& header_set, HpackEncoder* encoder)
    : encoder_(encoder)
    , has_next_(true)
{
    // Separate header set into pseudo-headers and regular headers.
...
std::unique_ptr<HpackEncoder::ProgressiveEncoder> HpackEncoder::EncodeHeaderSet(const quiche::HttpHeaderBlock& header_set)
{
