...
{
    if (!before_entry_) {
        QUICHE_DVLOG(2) << "HpackBlockDecoder::Decode resume entry, db->Remaining=" << db->Remaining();
    ...
    while (db->HasData()) {
        QUICHE_DVLOG(2) << "HpackBlockDecoder::Decode start entry, db->Remaining=" << db->Remaining();
...
{
    return absl::StrCat("HpackBlockDecoder(", entry_decoder_.DebugString(), ", listener@", absl::Hex(reinterpret_cast<intptr_t>(listener_)),
