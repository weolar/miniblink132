...
HpackWholeEntryBuffer::HpackWholeEntryBuffer(HpackWholeEntryListener* listener, size_t max_string_size_bytes)
...
void HpackWholeEntryBuffer::set_max_string_size_bytes(size_t max_string_size_bytes)
{
    max_string_size_bytes_ = max_string_size_bytes;
}
...
void HpackWholeEntryBuffer::OnStartLiteralHeader(HpackEntryType entry_type, size_t maybe_name_index)
{
    QUICHE_DVLOG(2) << "HpackWholeEntryBuffer::OnStartLiteralHeader: entry_type=" << entry_type << ",  maybe_name_index=" << maybe_name_index;
...
{
    QUICHE_DVLOG(2) << "HpackWholeEntryBuffer::OnNameStart: huffman_encoded=" << (huffman_encoded ? "true" : "false") << ",  len=" << len;
    QUICHE_DCHECK_EQ(maybe_name_index_, 0u);
    if (!error_detected_) {
        if (len > max_string_size_bytes_) {
            QUICHE_DVLOG(1) << "Name length (" << len << ") is longer than permitted (" << max_string_size_bytes_ << ")";
...
{
    QUICHE_DVLOG(2) << "HpackWholeEntryBuffer::OnNameData: len=" << len << " data:\n" << quiche::QuicheTextUtils::HexDump(absl::string_view(data, len));
    QUICHE_DCHECK_EQ(maybe_name_index_, 0u);
...
{
    QUICHE_DVLOG(2) << "HpackWholeEntryBuffer::OnValueStart: huffman_encoded=" << (huffman_encoded ? "true" : "false") << ",  len=" << len;
    if (!error_detected_) {
        if (len > max_string_size_bytes_) {
            QUICHE_DVLOG(1) << "Value length (" << len << ") of [" << name_.GetStringIfComplete() << "] is longer than permitted (" << max_string_size_bytes_
                            << ")";
...
{
    QUICHE_DVLOG(2) << "HpackWholeEntryBuffer::OnValueData: len=" << len << " data:\n" << quiche::QuicheTextUtils::HexDump(absl::string_view(data, len));
...
void HpackWholeEntryBuffer::OnValueEnd()
{
    ...
    if (maybe_name_index_ == 0) {
        ...
        listener_->OnNameIndexAndLiteralValue(entry_type_, maybe_name_index_, &value_);
    }
    value_.Reset();
}
...
{
    QUICHE_DVLOG(2) << "HpackWholeEntryBuffer::OnDynamicTableSizeUpdate: size=" << size;
...
{
    if (!error_detected_) {
        QUICHE_DVLOG(1) << "HpackWholeEntryBuffer::ReportError: " << HpackDecodingErrorToString(error);
