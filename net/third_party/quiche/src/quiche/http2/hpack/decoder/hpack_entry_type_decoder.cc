...
{
    return absl::StrCat("HpackEntryTypeDecoder(varint_decoder=", varint_decoder_.DebugString(), ", entry_type=", entry_type_, ")");
}
...
DecodeStatus HpackEntryTypeDecoder::Start(DecodeBuffer* db)
{
    ...
    QUICHE_BUG(http2_bug_66_1) << "Unreachable, byte=" << std::hex << static_cast<uint32_t>(byte);
