...
DecodeStatus HpackEntryDecoder::Start(DecodeBuffer* db, HpackEntryDecoderListener* listener)
{
...
DecodeStatus HpackEntryDecoder::Resume(DecodeBuffer* db, HpackEntryDecoderListener* listener)
{
    QUICHE_DCHECK(db != nullptr);
    QUICHE_DCHECK(listener != nullptr);
    ...
    do {
        ...
        case EntryDecoderState::kResumeDecodingType:
            ...
            QUICHE_DVLOG(1) << "kResumeDecodingType: db->Remaining=" << db->Remaining();
        ...
        case EntryDecoderState::kStartDecodingName:
            QUICHE_DVLOG(1) << "kStartDecodingName: db->Remaining=" << db->Remaining();
            {
        ...
        case EntryDecoderState::kStartDecodingValue:
            QUICHE_DVLOG(1) << "kStartDecodingValue: db->Remaining=" << db->Remaining();
            {
        ...
        case EntryDecoderState::kResumeDecodingName:
            ...
            QUICHE_DVLOG(1) << "kResumeDecodingName: db->Remaining=" << db->Remaining();
            {
                NameDecoderListener ncb(listener);
        ...
        case EntryDecoderState::kResumeDecodingValue:
            ...
            QUICHE_DVLOG(1) << "kResumeDecodingValue: db->Remaining=" << db->Remaining();
            {
                ValueDecoderListener vcb(listener);
...
{
    out << "HpackEntryDecoder(state=" << state_ << ", " << entry_type_decoder_ << ", " << string_decoder_ << ")";
}
...
std::ostream& operator<<(std::ostream& out, HpackEntryDecoder::EntryDecoderState state)
{
