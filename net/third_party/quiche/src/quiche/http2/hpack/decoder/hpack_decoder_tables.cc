...
{
    static const std::vector<HpackStringPair>* const g_static_table = MakeStaticTable();
...
HpackDecoderStaticTable::HpackDecoderStaticTable(const std::vector<HpackStringPair>* table)
    : table_(table)
{
}

HpackDecoderStaticTable::HpackDecoderStaticTable()
    : table_(GetStaticTable())
{
}
...
{
    QUICHE_DVLOG(3) << "HpackDecoderDynamicTable::DynamicTableSizeUpdate " << size_limit;
...
void HpackDecoderDynamicTable::Insert(std::string name, std::string value)
{
    ...
    if (entry_size > size_limit_) {
        QUICHE_DVLOG(2) << "InsertEntry: entry larger than table, removing " << table_.size() << " entries, of total size " << current_size_ << " bytes.";
        table_.clear();
        current_size_ = 0;
        return;
    }
...
{
    QUICHE_DVLOG(2) << "EnsureSizeNoMoreThan limit=" << limit << ", current_size_=" << current_size_;
...
void HpackDecoderDynamicTable::RemoveLastEntry()
{
    ...
    if (!table_.empty()) {
        QUICHE_DVLOG(2) << "RemoveLastEntry current_size_=" << current_size_ << ", last entry size=" << table_.back().size();
