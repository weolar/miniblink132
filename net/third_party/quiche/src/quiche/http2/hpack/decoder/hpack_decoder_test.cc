...
public:
    ...
    static void set_listener(HpackDecoderState* state, HpackDecoderListener* listener)
    {
...
class HpackDecoderTest : public quiche::test::QuicheTestWithParam<bool>, public HpackDecoderListener {
protected:
    ...
    void OnHeaderErrorDetected(absl::string_view error_message) override
    {
        ...
        HpackDecoderStatePeer::set_listener(HpackDecoderPeer::GetDecoderState(&decoder_), &mock_listener_);
    }
    ...
    AssertionResult VerifyEntry(size_t dynamic_index, const char* name, const char* value)
    {
    ...
    AssertionResult VerifyDynamicTableContents(const std::vector<std::pair<const char*, const char*>>& entries)
    {
        size_t index = 1;
...
TEST_P(HpackDecoderTest, C5_ResponseExamples)
{
    ...
    ASSERT_THAT(header_entries_,
        ElementsAreArray({
            ...
            HpackHeaderEntry { "set-cookie", "foo=ASDJKHQKBZXOQWEOPIUAXQWEOIU; max-age=3600; version=1" },
        }));
...
TEST_P(HpackDecoderTest, ProcessesOptionalTableSizeUpdates)
{
    ...
    // size is 1000, not 500.
    {
        ...
        EXPECT_EQ(HpackDecodingError::kDynamicTableSizeUpdateNotAllowed, decoder_.error());
        EXPECT_EQ(1u, error_messages_.size());
        EXPECT_THAT(error_messages_[0], Eq("Dynamic table size update not allowed"));
...
TEST_P(HpackDecoderTest, ProcessesRequiredTableSizeUpdate)
{
    ...
    EXPECT_EQ(2048u, decoder_.GetCurrentHeaderTableSizeSetting());
    {
        ...
        EXPECT_TRUE(DecodeBlock(hbb.buffer()));
        EXPECT_THAT(header_entries_, ElementsAreArray({ HpackHeaderEntry { ":path", "/" } }));
    ...
    EXPECT_EQ(1500u, decoder_.GetCurrentHeaderTableSizeSetting());
    {
        ...
        EXPECT_TRUE(DecodeBlock(hbb.buffer()));
        EXPECT_THAT(header_entries_, ElementsAreArray({ HpackHeaderEntry { ":path", "/index.html" } }));
...
TEST_P(HpackDecoderTest, InvalidRequiredSizeUpdate)
{
    ...
    EXPECT_FALSE(decoder_.DecodeFragment(&db));
    EXPECT_FALSE(saw_end_);
    EXPECT_EQ(HpackDecodingError::kInitialDynamicTableSizeUpdateIsAboveLowWaterMark, decoder_.error());
    EXPECT_EQ(1u, error_messages_.size());
    EXPECT_THAT(error_messages_[0], Eq("Initial dynamic table size update is above low water mark"));
    EXPECT_EQ(Http2SettingsInfo::DefaultHeaderTableSize(), header_table_size_limit());
}
...
TEST_P(HpackDecoderTest, RequiredTableSizeChangeBeforeEnd)
{
    ...
    EXPECT_EQ(HpackDecodingError::kMissingDynamicTableSizeUpdate, decoder_.error());
    EXPECT_EQ(1u, error_messages_.size());
...
TEST_P(HpackDecoderTest, RequiredTableSizeChangeBeforeIndexedHeaderName)
{
    decoder_.ApplyHeaderTableSizeSetting(1024);
    HpackBlockBuilder hbb;
    hbb.AppendNameIndexAndLiteralValue(HpackEntryType::kIndexedLiteralHeader, 2, false, "PUT");
    EXPECT_FALSE(DecodeBlock(hbb.buffer()));
    EXPECT_EQ(HpackDecodingError::kMissingDynamicTableSizeUpdate, decoder_.error());
    EXPECT_EQ(1u, error_messages_.size());
    EXPECT_THAT(error_messages_[0], Eq("Missing dynamic table size update"));
...
TEST_P(HpackDecoderTest, RequiredTableSizeChangeBeforeLiteralName)
{
    decoder_.ApplyHeaderTableSizeSetting(1024);
    HpackBlockBuilder hbb;
    hbb.AppendLiteralNameAndValue(HpackEntryType::kNeverIndexedLiteralHeader, false, "name", false, "some data.");
    EXPECT_FALSE(DecodeBlock(hbb.buffer()));
    EXPECT_EQ(HpackDecodingError::kMissingDynamicTableSizeUpdate, decoder_.error());
    EXPECT_EQ(1u, error_messages_.size());
    EXPECT_THAT(error_messages_[0], Eq("Missing dynamic table size update"));
...
TEST_P(HpackDecoderTest, InvalidIndexedHeaderVarint)
{
    ...
    EXPECT_EQ(1u, error_messages_.size());
    EXPECT_THAT(error_messages_[0], Eq("Index varint beyond implementation limit"));
    EXPECT_TRUE(header_entries_.empty());
...
TEST_P(HpackDecoderTest, InvalidIndex)
{
    ...
    EXPECT_EQ(1u, error_messages_.size());
    EXPECT_THAT(error_messages_[0], Eq("Invalid index in indexed header field representation"));
    EXPECT_TRUE(header_entries_.empty());
    // Now that an error has been detected, EndDecodingBlock should not succeed.
    EXPECT_FALSE(decoder_.EndDecodingBlock());
}
...
TEST_P(HpackDecoderTest, TruncatedBlock)
{
    ...
    EXPECT_EQ(1u, error_messages_.size());
    EXPECT_THAT(error_messages_[0], Eq("Block ends in the middle of an instruction"));
...
TEST_P(HpackDecoderTest, OversizeStringDetected)
{
    HpackBlockBuilder hbb;
    hbb.AppendLiteralNameAndValue(HpackEntryType::kNeverIndexedLiteralHeader, false, "name", false, "some data.");
    hbb.AppendLiteralNameAndValue(HpackEntryType::kUnindexedLiteralHeader, false, "name2", false, "longer data");

    // Normally able to decode this block.
    EXPECT_TRUE(DecodeBlock(hbb.buffer()));
    EXPECT_THAT(header_entries_, ElementsAreArray({ HpackHeaderEntry { "name", "some data." }, HpackHeaderEntry { "name2", "longer data" } }));

    // But not if the maximum size of strings is less than the longest string.
    decoder_.set_max_string_size_bytes(10);
    EXPECT_FALSE(DecodeBlock(hbb.buffer()));
    EXPECT_THAT(header_entries_, ElementsAreArray({ HpackHeaderEntry { "name", "some data." } }));
    EXPECT_FALSE(saw_end_);
