...
TEST_F(HpackEntryDecoderTest, IndexedHeader_Various)
{
    ...
    for (const uint32_t ndx : { 1, 2, 61, 62, 63, 126, 127, 254, 255, 256 }) {
        ...
        auto do_check = [this, ndx]() { return collector_.ValidateIndexedHeader(ndx); };
        EXPECT_TRUE(DecodeAndValidateSeveralWays(hbb, ValidateDoneAndEmpty(do_check)));
        EXPECT_TRUE(do_check());
    }
}
...
{
    const char input[] = "\x7f" // == Literal indexed, name index 0x40 ==
                         "\x01" // 2nd byte of name index (0x01 + 0x3f == 0x40)
                         "\x0d" // Value length (13)
                         "custom-header"; // Value
    DecodeBuffer b(input, sizeof input - 1);
    auto do_check = [this]() { return collector_.ValidateLiteralValueHeader(HpackEntryType::kIndexedLiteralHeader, 0x40, false, "custom-header"); };
    EXPECT_TRUE(DecodeAndValidateSeveralWays(&b, ValidateDoneAndEmpty(do_check)));
    EXPECT_TRUE(do_check());
}
...
{
    const char input[] = "\x40" // == Literal indexed ==
                         "\x0a" // Name length (10)
                         "custom-key" // Name
                         "\x0d" // Value length (13)
                         "custom-header"; // Value
    ...
    DecodeBuffer b(input, sizeof input - 1);
    auto do_check
        = [this]() { return collector_.ValidateLiteralNameValueHeader(HpackEntryType::kIndexedLiteralHeader, false, "custom-key", false, "custom-header"); };
    EXPECT_TRUE(DecodeAndValidateSeveralWays(&b, ValidateDoneAndEmpty(do_check)));
    EXPECT_TRUE(do_check());
}
...
TEST_F(HpackEntryDecoderTest, DynamicTableSizeUpdate_Literal)
{
    ...
    auto do_check = [this]() { return collector_.ValidateDynamicTableSizeUpdate(31); };
    EXPECT_TRUE(DecodeAndValidateSeveralWays(&b, ValidateDoneAndEmpty(do_check)));
    EXPECT_TRUE(do_check());
}

class HpackLiteralEntryDecoderTest : public HpackEntryDecoderTest, public ::testing::WithParamInterface<HpackEntryType> {
protected:
    HpackLiteralEntryDecoderTest()
        : entry_type_(GetParam())
    {
    }
...
INSTANTIATE_TEST_SUITE_P(AllLiteralTypes, HpackLiteralEntryDecoderTest,
    testing::Values(HpackEntryType::kIndexedLiteralHeader, HpackEntryType::kUnindexedLiteralHeader, HpackEntryType::kNeverIndexedLiteralHeader));
...
TEST_P(HpackLiteralEntryDecoderTest, RandNameIndexAndLiteralValue)
{
    for (int n = 0; n < 10; n++) {
        ...
        HpackBlockBuilder hbb;
        hbb.AppendNameIndexAndLiteralValue(entry_type_, ndx, value_is_huffman_encoded, value);
        auto do_check = [this, ndx, value_is_huffman_encoded, value]() -> AssertionResult {
            return collector_.ValidateLiteralValueHeader(entry_type_, ndx, value_is_huffman_encoded, value);
        };
        EXPECT_TRUE(DecodeAndValidateSeveralWays(hbb, ValidateDoneAndEmpty(do_check)));
        EXPECT_TRUE(do_check());
    }
...
TEST_P(HpackLiteralEntryDecoderTest, RandLiteralNameAndValue)
{
    for (int n = 0; n < 10; n++) {
        ...
        HpackBlockBuilder hbb;
        hbb.AppendLiteralNameAndValue(entry_type_, name_is_huffman_encoded, name, value_is_huffman_encoded, value);
        auto do_check = [this, name_is_huffman_encoded, name, value_is_huffman_encoded, value]() -> AssertionResult {
            return collector_.ValidateLiteralNameValueHeader(entry_type_, name_is_huffman_encoded, name, value_is_huffman_encoded, value);
        };
        EXPECT_TRUE(DecodeAndValidateSeveralWays(hbb, ValidateDoneAndEmpty(do_check)));
        EXPECT_TRUE(do_check());
    }
