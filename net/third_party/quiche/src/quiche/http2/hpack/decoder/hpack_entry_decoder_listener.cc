...
void HpackEntryDecoderVLoggingListener::OnStartLiteralHeader(HpackEntryType entry_type, size_t maybe_name_index)
{
    QUICHE_VLOG(1) << "OnStartLiteralHeader: entry_type=" << entry_type << ", maybe_name_index=" << maybe_name_index;
    if (wrapped_) {
        wrapped_->OnStartLiteralHeader(entry_type, maybe_name_index);
    }
}

void HpackEntryDecoderVLoggingListener::OnNameStart(bool huffman_encoded, size_t len)
{
...
void HpackEntryDecoderVLoggingListener::OnNameData(const char* data, size_t len)
{
...
void HpackEntryDecoderVLoggingListener::OnValueStart(bool huffman_encoded, size_t len)
{
...
void HpackEntryDecoderVLoggingListener::OnValueData(const char* data, size_t len)
{
