...
protected:
    HpackBlockDecoderTest()
        : listener_(&collector_)
        , decoder_(&listener_)
    {
    ...
    AssertionResult DecodeAndValidateSeveralWays(DecodeBuffer* db, const Validator& validator)
    {
        bool return_non_zero_on_first = false;
        return RandomDecoderTest::DecodeAndValidateSeveralWays(db, return_non_zero_on_first, validator);
    }

    AssertionResult DecodeAndValidateSeveralWays(const HpackBlockBuilder& hbb, const Validator& validator)
    {
        DecodeBuffer db(hbb.buffer());
        return DecodeAndValidateSeveralWays(&db, validator);
    }

    AssertionResult DecodeHpackExampleAndValidateSeveralWays(absl::string_view hpack_example, Validator validator)
    {
    ...
    std::string Rand8String()
    {
        return Random().RandString(Rand8());
    }
...
{
    auto do_check = [this]() {
        return collector_.ValidateSoleLiteralNameValueHeader(HpackEntryType::kIndexedLiteralHeader, false, "custom-key", false, "custom-header");
    };
    ...
    EXPECT_TRUE(DecodeHpackExampleAndValidateSeveralWays(hpack_example, ValidateDoneAndEmpty(do_check)));
...
{
    auto do_check = [this]() { return collector_.ValidateSoleLiteralValueHeader(HpackEntryType::kUnindexedLiteralHeader, 4, false, "/sample/path"); };
    const char hpack_example[] = R"(
...
{
    auto do_check
        = [this]() { return collector_.ValidateSoleLiteralNameValueHeader(HpackEntryType::kNeverIndexedLiteralHeader, false, "password", false, "secret"); };
    const char hpack_example[] = R"(
...
TEST_F(HpackBlockDecoderTest, SpecExample_C_3_1)
{
    ...
    expected.ExpectNameIndexAndLiteralValue(HpackEntryType::kIndexedLiteralHeader, 1, false, "www.example.com");
    EXPECT_TRUE(DecodeHpackExampleAndValidateSeveralWays(example, ValidateDoneAndEmpty([&] { return collector_.VerifyEq(expected); })));
...
TEST_F(HpackBlockDecoderTest, SpecExample_C_5_1)
{
    ...
    HpackBlockCollector expected;
    expected.ExpectNameIndexAndLiteralValue(HpackEntryType::kIndexedLiteralHeader, 8, false, "302");
    expected.ExpectNameIndexAndLiteralValue(HpackEntryType::kIndexedLiteralHeader, 24, false, "private");
    expected.ExpectNameIndexAndLiteralValue(HpackEntryType::kIndexedLiteralHeader, 33, false, "Mon, 21 Oct 2013 20:13:21 GMT");
    expected.ExpectNameIndexAndLiteralValue(HpackEntryType::kIndexedLiteralHeader, 46, false, "https://www.example.com");
    EXPECT_TRUE(DecodeHpackExampleAndValidateSeveralWays(example, ValidateDoneAndEmpty([&] { return collector_.VerifyEq(expected); })));
    EXPECT_TRUE(collector_.VerifyEq(expected));
}
...
TEST_F(HpackBlockDecoderTest, Computed)
{
    ...
    for (auto type : { HpackEntryType::kIndexedLiteralHeader, HpackEntryType::kUnindexedLiteralHeader, HpackEntryType::kNeverIndexedLiteralHeader }) {
        for (bool value_huffman : { false, true }) {
            ...
            expected.ExpectNameIndexAndLiteralValue(type, Rand8() + 1, value_huffman, Rand8String());
            // And two entries with literal names, one plain, one huffman encoded.
            expected.ExpectLiteralNameAndValue(type, false, Rand8String(), value_huffman, Rand8String());
            expected.ExpectLiteralNameAndValue(type, true, Rand8String(), value_huffman, Rand8String());
        }
    }
    ...
    EXPECT_TRUE(DecodeAndValidateSeveralWays(hbb, ValidateDoneAndEmpty([&] { return collector_.VerifyEq(expected); })));
    EXPECT_TRUE(collector_.VerifyEq(expected));
}
