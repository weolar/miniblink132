...
std::ostream& operator<<(std::ostream& out, const HpackDecoderStringBuffer::State v)
{
    switch (v) {
    ...
    QUICHE_BUG(http2_bug_50_1) << "Invalid HpackDecoderStringBuffer::State: " << unknown;
    return out << "HpackDecoderStringBuffer::State(" << unknown << ")";
}

std::ostream& operator<<(std::ostream& out, const HpackDecoderStringBuffer::Backing v)
{
    switch (v) {
    ...
    QUICHE_BUG(http2_bug_50_2) << "Invalid HpackDecoderStringBuffer::Backing: " << v2;
...
{
    QUICHE_DVLOG(2) << "HpackDecoderStringBuffer::OnData state=" << state_ << ", backing=" << backing_;
...
{
    QUICHE_DVLOG(3) << "HpackDecoderStringBuffer::BufferStringIfUnbuffered state=" << state_ << ", backing=" << backing_;
    if (state_ != State::RESET && backing_ == Backing::UNBUFFERED) {
        QUICHE_DVLOG(2) << "HpackDecoderStringBuffer buffering std::string of length " << value_.size();
