...
public:
    ...
    MOCK_METHOD(void, OnNameIndexAndLiteralValue, (HpackEntryType entry_type, size_t name_index, HpackDecoderStringBuffer* value_buffer), (override));
    MOCK_METHOD(
        void, OnLiteralNameAndValue, (HpackEntryType entry_type, HpackDecoderStringBuffer* name_buffer, HpackDecoderStringBuffer* value_buffer), (override));
...
protected:
    HpackWholeEntryBufferTest()
        : entry_buffer_(&listener_, kMaxStringSize)
    {
    }
...
{
    entry_buffer_.OnStartLiteralHeader(HpackEntryType::kNeverIndexedLiteralHeader, 123);
    ...
    EXPECT_CALL(listener_,
        OnNameIndexAndLiteralValue(HpackEntryType::kNeverIndexedLiteralHeader, 123,
            AllOf(Property(&HpackDecoderStringBuffer::str, "some data."), Property(&HpackDecoderStringBuffer::BufferedLength, 10))));
...
TEST_F(HpackWholeEntryBufferTest, OnLiteralNameAndValue)
{
    ...
    EXPECT_CALL(listener_,
        OnLiteralNameAndValue(HpackEntryType::kIndexedLiteralHeader,
            AllOf(Property(&HpackDecoderStringBuffer::str, "some-name"), Property(&HpackDecoderStringBuffer::BufferedLength, 9)),
            AllOf(Property(&HpackDecoderStringBuffer::str, "Header Value"), Property(&HpackDecoderStringBuffer::BufferedLength, 0))));
...
TEST_F(HpackWholeEntryBufferTest, NameHuffmanError)
{
    ...
    entry_buffer_.OnStartLiteralHeader(HpackEntryType::kUnindexedLiteralHeader, 0);
    entry_buffer_.OnNameStart(true, 4);
    entry_buffer_.OnNameData(data, 3);

    EXPECT_CALL(listener_, OnHpackDecodeError(HpackDecodingError::kNameHuffmanError));
...
TEST_F(HpackWholeEntryBufferTest, ValueHuffmanError)
{
    ...
    entry_buffer_.OnStartLiteralHeader(HpackEntryType::kNeverIndexedLiteralHeader, 61);
    entry_buffer_.OnValueStart(true, 3);
    entry_buffer_.OnValueData(data, 3);

    EXPECT_CALL(listener_, OnHpackDecodeError(HpackDecodingError::kValueHuffmanError));
