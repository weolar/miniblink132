...
template <class C> void ShuffleCollection(C* collection, Http2Random* r)
{
...
protected:
    ...
    size_t FakeTrim(size_t limit)
    {
        ...
        for (size_t ndx = 0; ndx < fake_dynamic_table_.size(); ++ndx) {
            ...
            if (total_size > limit) {
                ...
                fake_dynamic_table_.erase(fake_dynamic_table_.begin() + ndx, fake_dynamic_table_.end());
    ...
    AssertionResult VerifyDynamicTableContents()
    {
        ...
        HTTP2_VERIFY_EQ(nullptr, Lookup(fake_dynamic_table_.size() + kFirstDynamicTableIndex));
        return AssertionSuccess();
    }
...
TEST_F(HpackDecoderTablesTest, RandomDynamicTable)
{
    ...
    for (size_t limit : table_sizes) {
        ...
        for (int insert_count = 0; insert_count < 100; ++insert_count) {
            std::string name = GenerateHttp2HeaderName(random_.UniformInRange(2, 40), RandomPtr());
            std::string value = GenerateWebSafeString(random_.UniformInRange(2, 600), RandomPtr());
