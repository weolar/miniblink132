...
public:
    ...
    template <class Listener> DecodeStatus Start(DecodeBuffer* db, Listener* cb)
    {
    ...
    template <class Listener> DecodeStatus Resume(DecodeBuffer* db, Listener* cb)
    {
        DecodeStatus status;
        while (true) {
            switch (state_) {
            case kStartDecodingLength:
                QUICHE_DVLOG(2) << "kStartDecodingLength: db->Remaining=" << db->Remaining();
            ...
            case kDecodingString:
                QUICHE_DVLOG(2) << "kDecodingString: db->Remaining=" << db->Remaining() << "    remaining_=" << remaining_;
                return DecodeString(db, cb);

            case kResumeDecodingLength:
                QUICHE_DVLOG(2) << "kResumeDecodingLength: db->Remaining=" << db->Remaining();
...
private:
    ...
    template <class Listener> bool StartDecodingLength(DecodeBuffer* db, Listener* cb, DecodeStatus* status)
    {
        if (db->Empty()) {
    ...
    // If the length is not fully decoded, case state_ is set appropriately
    // for the next call to Resume.
    template <class Listener> bool ResumeDecodingLength(DecodeBuffer* db, Listener* cb, DecodeStatus* status)
    {
    ...
    template <class Listener> void OnStringStart(Listener* cb, DecodeStatus* /*status*/)
    {
    ...
    template <class Listener> DecodeStatus DecodeString(DecodeBuffer* db, Listener* cb)
    {
...
QUICHE_EXPORT std::ostream& operator<<(std::ostream& out, const HpackStringDecoder& v);
