...
public:
    ...
    using DynamicEntryTable = quiche::QuicheCircularDeque<std::unique_ptr<HpackEntry>>;
    ...
    size_t settings_size_bound() const
    {
        return settings_size_bound_;
    }
    ...
    const HpackEntry* TryAddEntry(absl::string_view name, absl::string_view value);

private:
    // Returns number of evictions required to enter |name| & |value|.
    size_t EvictionCountForEntry(absl::string_view name, absl::string_view value) const;
