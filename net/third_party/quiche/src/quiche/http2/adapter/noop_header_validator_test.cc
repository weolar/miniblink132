...
TEST(NoopHeaderValidatorTest, HeaderNameEmpty)
{
    ...
    NoopHeaderValidator::HeaderStatus status = v.ValidateSingleHeader("", "value");
...
TEST(NoopHeaderValidatorTest, ExceedsMaxSize)
{
    ...
    NoopHeaderValidator::HeaderStatus status = v.ValidateSingleHeader("name", "value");
    EXPECT_EQ(NoopHeaderValidator::HEADER_OK, status);
...
TEST(NoopHeaderValidatorTest, AnyNameCharIsValid)
{
    ...
    for (int i = std::numeric_limits<char>::min(); i < std::numeric_limits<char>::max(); ++i) {
        ...
        EXPECT_EQ(NoopHeaderValidator::HEADER_OK, v.ValidateSingleHeader(sv, "value"));
...
TEST(NoopHeaderValidatorTest, AnyValueCharIsValid)
{
    ...
    for (int i = std::numeric_limits<char>::min(); i < std::numeric_limits<char>::max(); ++i) {
        ...
        EXPECT_EQ(NoopHeaderValidator::HEADER_OK, v.ValidateSingleHeader("name", sv));
    }
}
...
TEST(NoopHeaderValidatorTest, AnyStatusIsValid)
{
    ...
    for (HeaderType type : { HeaderType::RESPONSE, HeaderType::RESPONSE_100 }) {
        v.StartHeaderBlock();
        EXPECT_EQ(NoopHeaderValidator::HEADER_OK, v.ValidateSingleHeader(":status", "bar"));
        EXPECT_TRUE(v.FinishHeaderBlock(type));

        v.StartHeaderBlock();
        EXPECT_EQ(NoopHeaderValidator::HEADER_OK, v.ValidateSingleHeader(":status", "10"));
        EXPECT_TRUE(v.FinishHeaderBlock(type));

        v.StartHeaderBlock();
        EXPECT_EQ(NoopHeaderValidator::HEADER_OK, v.ValidateSingleHeader(":status", "9000"));
        EXPECT_TRUE(v.FinishHeaderBlock(type));

        v.StartHeaderBlock();
        EXPECT_EQ(NoopHeaderValidator::HEADER_OK, v.ValidateSingleHeader(":status", "400"));
        EXPECT_TRUE(v.FinishHeaderBlock(type));
    }
}
...
TEST(NoopHeaderValidatorTest, AnyAuthorityCharIsValid)
{
    ...
    for (int i = std::numeric_limits<char>::min(); i < std::numeric_limits<char>::max(); ++i) {
        ...
        for (absl::string_view key : { ":authority", "host" }) {
            NoopHeaderValidator v;
            v.StartHeaderBlock();
            EXPECT_EQ(NoopHeaderValidator::HEADER_OK, v.ValidateSingleHeader(key, sv));
        }
    }
}
...
TEST(NoopHeaderValidatorTest, RequestHostAndAuthority)
{
    ...
    for (Header to_add : kSampleRequestPseudoheaders) {
        EXPECT_EQ(NoopHeaderValidator::HEADER_OK, v.ValidateSingleHeader(to_add.first, to_add.second));
    }
    // If both "host" and ":authority" have the same value, validation succeeds.
    EXPECT_EQ(NoopHeaderValidator::HEADER_OK, v.ValidateSingleHeader("host", "www.foo.com"));
    ...
    // succeeds.
    EXPECT_EQ(NoopHeaderValidator::HEADER_OK, v.ValidateSingleHeader("host", "www.bar.com"));
}
...
TEST(NoopHeaderValidatorTest, RequestPseudoHeaders)
{
    ...
    EXPECT_EQ(NoopHeaderValidator::HEADER_OK, v.ValidateSingleHeader(":extra", "blah"));
    EXPECT_TRUE(v.FinishHeaderBlock(HeaderType::REQUEST));
...
TEST(NoopHeaderValidatorTest, WebsocketPseudoHeaders)
{
    ...
    EXPECT_EQ(NoopHeaderValidator::HEADER_OK, v.ValidateSingleHeader(":protocol", "websocket"));
    ...
    for (Header to_add : kSampleRequestPseudoheaders) {
        if (to_add.first == ":method") {
            EXPECT_EQ(NoopHeaderValidator::HEADER_OK, v.ValidateSingleHeader(to_add.first, "CONNECT"));
        } else {
            EXPECT_EQ(NoopHeaderValidator::HEADER_OK, v.ValidateSingleHeader(to_add.first, to_add.second));
        }
    }
...
TEST(NoopHeaderValidatorTest, AsteriskPathPseudoHeader)
{
    ...
    for (Header to_add : kSampleRequestPseudoheaders) {
        if (to_add.first == ":path") {
            EXPECT_EQ(NoopHeaderValidator::HEADER_OK, v.ValidateSingleHeader(to_add.first, "*"));
        } else {
            EXPECT_EQ(NoopHeaderValidator::HEADER_OK, v.ValidateSingleHeader(to_add.first, to_add.second));
        }
    }
    ...
    for (Header to_add : kSampleRequestPseudoheaders) {
        if (to_add.first == ":path") {
            ...
            EXPECT_EQ(NoopHeaderValidator::HEADER_OK, v.ValidateSingleHeader(to_add.first, "OPTIONS"));
        } else {
            EXPECT_EQ(NoopHeaderValidator::HEADER_OK, v.ValidateSingleHeader(to_add.first, to_add.second));
        }
    }
...
TEST(NoopHeaderValidatorTest, InvalidPathPseudoHeader)
{
    ...
    for (Header to_add : kSampleRequestPseudoheaders) {
        if (to_add.first == ":path") {
            EXPECT_EQ(NoopHeaderValidator::HEADER_OK, v.ValidateSingleHeader(to_add.first, "shawarma"));
        } else {
            EXPECT_EQ(NoopHeaderValidator::HEADER_OK, v.ValidateSingleHeader(to_add.first, to_add.second));
        }
    }
...
TEST(NoopHeaderValidatorTest, ResponsePseudoHeaders)
{
    ...
    for (HeaderType type : { HeaderType::RESPONSE, HeaderType::RESPONSE_100 }) {
        ...
        v.StartHeaderBlock();
        EXPECT_EQ(NoopHeaderValidator::HEADER_OK, v.ValidateSingleHeader("foo", "bar"));
        EXPECT_TRUE(v.FinishHeaderBlock(type));

        // When all pseudo-headers are present, final validation succeeds.
        v.StartHeaderBlock();
        EXPECT_EQ(NoopHeaderValidator::HEADER_OK, v.ValidateSingleHeader(":status", "199"));
        EXPECT_TRUE(v.FinishHeaderBlock(type));
        ...
        v.StartHeaderBlock();
        EXPECT_EQ(NoopHeaderValidator::HEADER_OK, v.ValidateSingleHeader(":status", "199"));
        EXPECT_EQ(NoopHeaderValidator::HEADER_OK, v.ValidateSingleHeader(":status", "299"));
        EXPECT_TRUE(v.FinishHeaderBlock(type));

        // When an extra pseudo-header is present, final validation succeeds.
        v.StartHeaderBlock();
        EXPECT_EQ(NoopHeaderValidator::HEADER_OK, v.ValidateSingleHeader(":status", "199"));
        EXPECT_EQ(NoopHeaderValidator::HEADER_OK, v.ValidateSingleHeader(":extra", "blorp"));
        EXPECT_TRUE(v.FinishHeaderBlock(type));
    }
}
...
TEST(NoopHeaderValidatorTest, ResponseWithHost)
{
    ...
    v.StartHeaderBlock();
    EXPECT_EQ(NoopHeaderValidator::HEADER_OK, v.ValidateSingleHeader(":status", "200"));
    EXPECT_EQ(NoopHeaderValidator::HEADER_OK, v.ValidateSingleHeader("host", "myserver.com"));
...
TEST(NoopHeaderValidatorTest, Response204)
{
    ...
    v.StartHeaderBlock();
    EXPECT_EQ(NoopHeaderValidator::HEADER_OK, v.ValidateSingleHeader(":status", "204"));
    EXPECT_EQ(NoopHeaderValidator::HEADER_OK, v.ValidateSingleHeader("x-content", "is not present"));
    EXPECT_TRUE(v.FinishHeaderBlock(HeaderType::RESPONSE));
}
...
TEST(NoopHeaderValidatorTest, ResponseWithMultipleIdenticalContentLength)
{
    ...
    v.StartHeaderBlock();
    EXPECT_EQ(NoopHeaderValidator::HEADER_OK, v.ValidateSingleHeader(":status", "200"));
    EXPECT_EQ(NoopHeaderValidator::HEADER_OK, v.ValidateSingleHeader("content-length", "13"));
    EXPECT_EQ(NoopHeaderValidator::HEADER_OK, v.ValidateSingleHeader("content-length", "13"));
}
...
TEST(NoopHeaderValidatorTest, ResponseWithMultipleDifferingContentLength)
{
    ...
    EXPECT_EQ(NoopHeaderValidator::HEADER_OK, v.ValidateSingleHeader(":status", "200"));
    EXPECT_EQ(NoopHeaderValidator::HEADER_OK, v.ValidateSingleHeader("content-length", "13"));
    EXPECT_EQ(NoopHeaderValidator::HEADER_OK, v.ValidateSingleHeader("content-length", "17"));
}
...
TEST(NoopHeaderValidatorTest, Response204WithContentLengthZero)
{
    ...
    EXPECT_EQ(NoopHeaderValidator::HEADER_OK, v.ValidateSingleHeader(":status", "204"));
    EXPECT_EQ(NoopHeaderValidator::HEADER_OK, v.ValidateSingleHeader("x-content", "is not present"));
    EXPECT_EQ(NoopHeaderValidator::HEADER_OK, v.ValidateSingleHeader("content-length", "0"));
    EXPECT_TRUE(v.FinishHeaderBlock(HeaderType::RESPONSE));
}
...
TEST(NoopHeaderValidatorTest, Response204WithContentLength)
{
    ...
    EXPECT_EQ(NoopHeaderValidator::HEADER_OK, v.ValidateSingleHeader(":status", "204"));
    EXPECT_EQ(NoopHeaderValidator::HEADER_OK, v.ValidateSingleHeader("x-content", "is not present"));
    EXPECT_EQ(NoopHeaderValidator::HEADER_OK, v.ValidateSingleHeader("content-length", "1"));
}
...
TEST(NoopHeaderValidatorTest, Response100)
{
    ...
    v.StartHeaderBlock();
    EXPECT_EQ(NoopHeaderValidator::HEADER_OK, v.ValidateSingleHeader(":status", "100"));
    EXPECT_EQ(NoopHeaderValidator::HEADER_OK, v.ValidateSingleHeader("x-content", "is not present"));
    EXPECT_TRUE(v.FinishHeaderBlock(HeaderType::RESPONSE));
}
...
TEST(NoopHeaderValidatorTest, ValidContentLength)
{
    ...
    EXPECT_EQ(NoopHeaderValidator::HEADER_OK, v.ValidateSingleHeader("content-length", "41"));
    EXPECT_EQ(v.content_length(), std::nullopt);

    v.StartHeaderBlock();
    EXPECT_EQ(v.content_length(), std::nullopt);
    EXPECT_EQ(NoopHeaderValidator::HEADER_OK, v.ValidateSingleHeader("content-length", "42"));
    EXPECT_EQ(v.content_length(), std::nullopt);
}
...
TEST(NoopHeaderValidatorTest, InvalidContentLength)
{
    ...
    v.StartHeaderBlock();
    EXPECT_EQ(v.content_length(), std::nullopt);
    EXPECT_EQ(NoopHeaderValidator::HEADER_OK, v.ValidateSingleHeader("content-length", ""));
    EXPECT_EQ(v.content_length(), std::nullopt);
    EXPECT_EQ(NoopHeaderValidator::HEADER_OK, v.ValidateSingleHeader("content-length", "nan"));
    EXPECT_EQ(v.content_length(), std::nullopt);
    EXPECT_EQ(NoopHeaderValidator::HEADER_OK, v.ValidateSingleHeader("content-length", "-42"));
    EXPECT_EQ(v.content_length(), std::nullopt);
    EXPECT_EQ(NoopHeaderValidator::HEADER_OK, v.ValidateSingleHeader("content-length", "42"));
...
TEST(NoopHeaderValidatorTest, TeHeader)
{
    ...
    v.StartHeaderBlock();
    EXPECT_EQ(NoopHeaderValidator::HEADER_OK, v.ValidateSingleHeader("te", "trailers"));

    v.StartHeaderBlock();
    EXPECT_EQ(NoopHeaderValidator::HEADER_OK, v.ValidateSingleHeader("te", "trailers, deflate"));
}
...
TEST(NoopHeaderValidatorTest, ConnectionSpecificHeaders)
{
    ...
    for (const auto& [connection_key, connection_value] : connection_headers) {
        ...
        for (const auto& [sample_key, sample_value] : kSampleRequestPseudoheaders) {
            EXPECT_EQ(NoopHeaderValidator::HEADER_OK, v.ValidateSingleHeader(sample_key, sample_value));
        }
        EXPECT_EQ(NoopHeaderValidator::HEADER_OK, v.ValidateSingleHeader(connection_key, connection_value));
    }
}
