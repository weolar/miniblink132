...
constexpr Header kSampleRequestPseudoheaders[] = { { ":authority", "www.foo.com" }, { ":method", "GET" }, { ":path", "/foo" }, { ":scheme", "https" } };
...
TEST(HeaderValidatorTest, ExceedsMaxSize)
{
    ...
    HeaderValidator::HeaderStatus status = v.ValidateSingleHeader("name", "value");
    EXPECT_EQ(HeaderValidator::HEADER_OK, status);
    status = v.ValidateSingleHeader("name2", "Antidisestablishmentariansism is supercalifragilisticexpialodocious.");
...
TEST(HeaderValidatorTest, NameHasInvalidChar)
{
    ...
    for (const bool is_pseudo_header : { true, false }) {
        ...
        for (const char* c : { "!", "3", "a", "_", "|", "~" }) {
            const std::string name = is_pseudo_header ? absl::StrCat(":met", c, "hod") : absl::StrCat("na", c, "me");
            HeaderValidator::HeaderStatus status = v.ValidateSingleHeader(name, "value");
            EXPECT_EQ(HeaderValidator::HEADER_OK, status);
        }
        ...
        for (const char* c : { "\\", "<", ";", "[", "=", " ", "\r", "\n", ",", "\"", "\x1F", "\x91" }) {
            const std::string name = is_pseudo_header ? absl::StrCat(":met", c, "hod") : absl::StrCat("na", c, "me");
            HeaderValidator::HeaderStatus status = v.ValidateSingleHeader(name, "value");
            EXPECT_EQ(HeaderValidator::HEADER_FIELD_INVALID, status) << "with name [" << name << "]";
        }
        // Test nul separately.
        {
            const absl::string_view name = is_pseudo_header ? absl::string_view(":met\0hod", 8) : absl::string_view("na\0me", 5);
            HeaderValidator::HeaderStatus status = v.ValidateSingleHeader(name, "value");
            EXPECT_EQ(HeaderValidator::HEADER_FIELD_INVALID, status);
        }
        // Uppercase characters in header names should not be allowed.
        const std::string uc_name = is_pseudo_header ? ":Method" : "Name";
        HeaderValidator::HeaderStatus status = v.ValidateSingleHeader(uc_name, "value");
        EXPECT_EQ(HeaderValidator::HEADER_FIELD_INVALID, status);
    }
}
...
TEST(HeaderValidatorTest, ValueHasInvalidChar)
{
    HeaderValidator v;
    // These characters should be allowed. (Not exhaustive.)
    for (const char* c : { "!", "3", "a", "_", "|", "~", "\\", "<", ";", "[", "=", "A", "\t" }) {
        const std::string value = absl::StrCat("val", c, "ue");
        EXPECT_TRUE(HeaderValidator::IsValidHeaderValue(value, ObsTextOption::kDisallow));
        HeaderValidator::HeaderStatus status = v.ValidateSingleHeader("name", value);
        EXPECT_EQ(HeaderValidator::HEADER_OK, status);
    }
    // These should not.
    for (const char* c : { "\r", "\n" }) {
        const std::string value = absl::StrCat("val", c, "ue");
        EXPECT_FALSE(HeaderValidator::IsValidHeaderValue(value, ObsTextOption::kDisallow));
        HeaderValidator::HeaderStatus status = v.ValidateSingleHeader("name", value);
        EXPECT_EQ(HeaderValidator::HEADER_FIELD_INVALID, status);
    }
    // Test nul separately.
    ...
    }
    {
        ...
        EXPECT_EQ(HeaderValidator::HEADER_FIELD_INVALID, v.ValidateSingleHeader("name", obs_text_value));
        // Test that obs-text is disallowed when configured.
        v.SetObsTextOption(ObsTextOption::kDisallow);
        EXPECT_FALSE(HeaderValidator::IsValidHeaderValue(obs_text_value, ObsTextOption::kDisallow));
        EXPECT_EQ(HeaderValidator::HEADER_FIELD_INVALID, v.ValidateSingleHeader("name", obs_text_value));
        // Test that obs-text is allowed when configured.
        v.SetObsTextOption(ObsTextOption::kAllow);
        EXPECT_TRUE(HeaderValidator::IsValidHeaderValue(obs_text_value, ObsTextOption::kAllow));
        EXPECT_EQ(HeaderValidator::HEADER_OK, v.ValidateSingleHeader("name", obs_text_value));
    }
}
...
TEST(HeaderValidatorTest, StatusHasInvalidChar)
{
    ...
    for (HeaderType type : { HeaderType::RESPONSE, HeaderType::RESPONSE_100 }) {
        ...
        v.StartHeaderBlock();
        EXPECT_EQ(HeaderValidator::HEADER_FIELD_INVALID, v.ValidateSingleHeader(":status", "bar"));
        EXPECT_FALSE(v.FinishHeaderBlock(type));

        // When `:status` is too short, validation will fail.
        v.StartHeaderBlock();
        EXPECT_EQ(HeaderValidator::HEADER_FIELD_INVALID, v.ValidateSingleHeader(":status", "10"));
        EXPECT_FALSE(v.FinishHeaderBlock(type));

        // When `:status` is too long, validation will fail.
        v.StartHeaderBlock();
        EXPECT_EQ(HeaderValidator::HEADER_FIELD_INVALID, v.ValidateSingleHeader(":status", "9000"));
        EXPECT_FALSE(v.FinishHeaderBlock(type));

        // When `:status` is just right, validation will succeed.
        v.StartHeaderBlock();
        EXPECT_EQ(HeaderValidator::HEADER_OK, v.ValidateSingleHeader(":status", "400"));
...
TEST(HeaderValidatorTest, AuthorityHasInvalidChar)
{
    for (absl::string_view key : { ":authority", "host" }) {
        ...
        for (const absl::string_view c : { "1", "-", "!", ":", "+", "=", "," }) {
            ...
            EXPECT_EQ(HeaderValidator::HEADER_OK, status) << " with name [" << key << "] and value [" << value << "]";
        }
        // These should not.
...

        {
            ...
            HeaderValidator v;
            v.StartHeaderBlock();
            HeaderValidator::HeaderStatus status = v.ValidateSingleHeader(key, value1);
            EXPECT_EQ(HeaderValidator::HEADER_OK, status);
...
TEST(HeaderValidatorTest, RequestHostAndAuthority)
{
    ...
    for (Header to_add : kSampleRequestPseudoheaders) {
        EXPECT_EQ(HeaderValidator::HEADER_OK, v.ValidateSingleHeader(to_add.first, to_add.second));
    }
    // If both "host" and ":authority" have the same value, validation succeeds.
    EXPECT_EQ(HeaderValidator::HEADER_OK, v.ValidateSingleHeader("host", "www.foo.com"));
    ...
    EXPECT_EQ(HeaderValidator::HEADER_FIELD_INVALID, v.ValidateSingleHeader("host", "www.bar.com"));
}
...
TEST(HeaderValidatorTest, RequestHostAndAuthorityLax)
{
    ...
    EXPECT_EQ(HeaderValidator::HEADER_OK, v.ValidateSingleHeader("host", "www.bar.com"));
}
...
TEST(HeaderValidatorTest, MethodHasInvalidChar)
{
    ...
    for (absl::string_view value : bad_methods) {
        v.StartHeaderBlock();
        EXPECT_EQ(HeaderValidator::HEADER_FIELD_INVALID, v.ValidateSingleHeader(":method", value));
    }

    for (absl::string_view value : good_methods) {
        v.StartHeaderBlock();
        EXPECT_EQ(HeaderValidator::HEADER_OK, v.ValidateSingleHeader(":method", value));
        for (Header to_add : kSampleRequestPseudoheaders) {
...
TEST(HeaderValidatorTest, RequestPseudoHeaders)
{
    ...
    EXPECT_EQ(HeaderValidator::HEADER_OK, v.ValidateSingleHeader(":extra", "blah"));
    EXPECT_FALSE(v.FinishHeaderBlock(HeaderType::REQUEST));
...
TEST(HeaderValidatorTest, ConnectHeaders)
{
    ...
    HeaderValidator v;
    v.StartHeaderBlock();
    EXPECT_EQ(HeaderValidator::HEADER_OK, v.ValidateSingleHeader(":authority", "athena.dialup.mit.edu:23"));
    EXPECT_FALSE(v.FinishHeaderBlock(HeaderType::REQUEST));

    v.StartHeaderBlock();
    EXPECT_EQ(HeaderValidator::HEADER_OK, v.ValidateSingleHeader(":method", "CONNECT"));
    EXPECT_FALSE(v.FinishHeaderBlock(HeaderType::REQUEST));
    ...
    // Empty :authority
    v.StartHeaderBlock();
    EXPECT_EQ(HeaderValidator::HEADER_OK, v.ValidateSingleHeader(":authority", ""));
    EXPECT_EQ(HeaderValidator::HEADER_OK, v.ValidateSingleHeader(":method", "CONNECT"));
    EXPECT_FALSE(v.FinishHeaderBlock(HeaderType::REQUEST));
...
TEST(HeaderValidatorTest, WebsocketPseudoHeaders)
{
    ...
    EXPECT_EQ(HeaderValidator::HEADER_OK, v.ValidateSingleHeader(":protocol", "websocket"));
    ...
    for (Header to_add : kSampleRequestPseudoheaders) {
        if (to_add.first == ":method") {
            EXPECT_EQ(HeaderValidator::HEADER_OK, v.ValidateSingleHeader(to_add.first, "CONNECT"));
        } else {
            EXPECT_EQ(HeaderValidator::HEADER_OK, v.ValidateSingleHeader(to_add.first, to_add.second));
        }
    }
...
TEST(HeaderValidatorTest, AsteriskPathPseudoHeader)
{
    ...
    for (Header to_add : kSampleRequestPseudoheaders) {
        if (to_add.first == ":path") {
            EXPECT_EQ(HeaderValidator::HEADER_OK, v.ValidateSingleHeader(to_add.first, "*"));
        } else {
            EXPECT_EQ(HeaderValidator::HEADER_OK, v.ValidateSingleHeader(to_add.first, to_add.second));
        }
    }
    ...
    for (Header to_add : kSampleRequestPseudoheaders) {
        if (to_add.first == ":path") {
            ...
            EXPECT_EQ(HeaderValidator::HEADER_OK, v.ValidateSingleHeader(to_add.first, "OPTIONS"));
        } else {
            EXPECT_EQ(HeaderValidator::HEADER_OK, v.ValidateSingleHeader(to_add.first, to_add.second));
        }
    }
...
TEST(HeaderValidatorTest, InvalidPathPseudoHeader)
{
    ...
    for (Header to_add : kSampleRequestPseudoheaders) {
        if (to_add.first == ":path") {
            EXPECT_EQ(HeaderValidator::HEADER_FIELD_INVALID, v.ValidateSingleHeader(to_add.first, ""));
        } else {
            EXPECT_EQ(HeaderValidator::HEADER_OK, v.ValidateSingleHeader(to_add.first, to_add.second));
        }
    }
    ...
    for (Header to_add : kSampleRequestPseudoheaders) {
        if (to_add.first == ":path") {
            EXPECT_EQ(HeaderValidator::HEADER_OK, v.ValidateSingleHeader(to_add.first, "shawarma"));
        } else {
            EXPECT_EQ(HeaderValidator::HEADER_OK, v.ValidateSingleHeader(to_add.first, to_add.second));
        }
    }
    ...
    for (const absl::string_view c : { "/", "?", "_", "'", "9", "&", "(", "@", ":" }) {
        ...
        for (Header to_add : kSampleRequestPseudoheaders) {
            if (to_add.first == ":path") {
                EXPECT_EQ(HeaderValidator::HEADER_OK, validator.ValidateSingleHeader(to_add.first, value)) << "Problematic char: [" << c << "]";
            } else {
                EXPECT_EQ(HeaderValidator::HEADER_OK, validator.ValidateSingleHeader(to_add.first, to_add.second));
            }
        }
    ...
    for (const absl::string_view c : { "[", "<", "}", "`", "\\", " ", "\t", "#" }) {
        ...
        for (Header to_add : kSampleRequestPseudoheaders) {
            if (to_add.first == ":path") {
                EXPECT_EQ(HeaderValidator::HEADER_FIELD_INVALID, validator.ValidateSingleHeader(to_add.first, value));
            } else {
                EXPECT_EQ(HeaderValidator::HEADER_OK, validator.ValidateSingleHeader(to_add.first, to_add.second));
            }
        }
    ...
    // The fragment initial character can be explicitly allowed.
    {
        ...
        for (Header to_add : kSampleRequestPseudoheaders) {
            if (to_add.first == ":path") {
                EXPECT_EQ(HeaderValidator::HEADER_OK, validator.ValidateSingleHeader(to_add.first, "/shawa#rma"));
            } else {
                EXPECT_EQ(HeaderValidator::HEADER_OK, validator.ValidateSingleHeader(to_add.first, to_add.second));
            }
        }
...
TEST(HeaderValidatorTest, ResponsePseudoHeaders)
{
    ...
    for (HeaderType type : { HeaderType::RESPONSE, HeaderType::RESPONSE_100 }) {
        ...
        // When all pseudo-headers are present, final validation will succeed.
        v.StartHeaderBlock();
        EXPECT_EQ(HeaderValidator::HEADER_OK, v.ValidateSingleHeader(":status", "199"));
        EXPECT_TRUE(v.FinishHeaderBlock(type));
        ...
        v.StartHeaderBlock();
        EXPECT_EQ(HeaderValidator::HEADER_OK, v.ValidateSingleHeader(":status", "199"));
        EXPECT_EQ(HeaderValidator::HEADER_OK, v.ValidateSingleHeader(":status", "299"));
        EXPECT_FALSE(v.FinishHeaderBlock(type));

        // When an extra pseudo-header is present, final validation will fail.
        v.StartHeaderBlock();
        EXPECT_EQ(HeaderValidator::HEADER_OK, v.ValidateSingleHeader(":status", "199"));
        EXPECT_EQ(HeaderValidator::HEADER_OK, v.ValidateSingleHeader(":extra", "blorp"));
        EXPECT_FALSE(v.FinishHeaderBlock(type));
    }
}
...
TEST(HeaderValidatorTest, ResponseWithHost)
{
    ...
    v.StartHeaderBlock();
    EXPECT_EQ(HeaderValidator::HEADER_OK, v.ValidateSingleHeader(":status", "200"));
    EXPECT_EQ(HeaderValidator::HEADER_OK, v.ValidateSingleHeader("host", "myserver.com"));
...
TEST(HeaderValidatorTest, Response204)
{
    ...
    v.StartHeaderBlock();
    EXPECT_EQ(HeaderValidator::HEADER_OK, v.ValidateSingleHeader(":status", "204"));
    EXPECT_EQ(HeaderValidator::HEADER_OK, v.ValidateSingleHeader("x-content", "is not present"));
    EXPECT_TRUE(v.FinishHeaderBlock(HeaderType::RESPONSE));
}
...
TEST(HeaderValidatorTest, ResponseWithMultipleIdenticalContentLength)
{
    ...
    v.StartHeaderBlock();
    EXPECT_EQ(HeaderValidator::HEADER_OK, v.ValidateSingleHeader(":status", "200"));
    EXPECT_EQ(HeaderValidator::HEADER_OK, v.ValidateSingleHeader("content-length", "13"));
    EXPECT_EQ(HeaderValidator::HEADER_SKIP, v.ValidateSingleHeader("content-length", "13"));
}
...
TEST(HeaderValidatorTest, ResponseWithMultipleDifferingContentLength)
{
    ...
    EXPECT_EQ(HeaderValidator::HEADER_OK, v.ValidateSingleHeader(":status", "200"));
    EXPECT_EQ(HeaderValidator::HEADER_OK, v.ValidateSingleHeader("content-length", "13"));
    EXPECT_EQ(HeaderValidator::HEADER_FIELD_INVALID, v.ValidateSingleHeader("content-length", "17"));
}
...
TEST(HeaderValidatorTest, Response204WithContentLengthZero)
{
    ...
    EXPECT_EQ(HeaderValidator::HEADER_OK, v.ValidateSingleHeader(":status", "204"));
    EXPECT_EQ(HeaderValidator::HEADER_OK, v.ValidateSingleHeader("x-content", "is not present"));
    EXPECT_EQ(HeaderValidator::HEADER_OK, v.ValidateSingleHeader("content-length", "0"));
    EXPECT_TRUE(v.FinishHeaderBlock(HeaderType::RESPONSE));
}
...
TEST(HeaderValidatorTest, Response204WithContentLength)
{
    ...
    EXPECT_EQ(HeaderValidator::HEADER_OK, v.ValidateSingleHeader(":status", "204"));
    EXPECT_EQ(HeaderValidator::HEADER_OK, v.ValidateSingleHeader("x-content", "is not present"));
    EXPECT_EQ(HeaderValidator::HEADER_FIELD_INVALID, v.ValidateSingleHeader("content-length", "1"));
}
...
TEST(HeaderValidatorTest, Response100)
{
    ...
    v.StartHeaderBlock();
    EXPECT_EQ(HeaderValidator::HEADER_OK, v.ValidateSingleHeader(":status", "100"));
    EXPECT_EQ(HeaderValidator::HEADER_OK, v.ValidateSingleHeader("x-content", "is not present"));
    EXPECT_TRUE(v.FinishHeaderBlock(HeaderType::RESPONSE));
}
...
TEST(HeaderValidatorTest, ValidContentLength)
{
    ...
    EXPECT_EQ(HeaderValidator::HEADER_OK, v.ValidateSingleHeader("content-length", "41"));
    EXPECT_THAT(v.content_length(), Optional(41));

    v.StartHeaderBlock();
    EXPECT_EQ(v.content_length(), std::nullopt);
    EXPECT_EQ(HeaderValidator::HEADER_OK, v.ValidateSingleHeader("content-length", "42"));
...
TEST(HeaderValidatorTest, InvalidContentLength)
{
    ...
    v.StartHeaderBlock();
    EXPECT_EQ(v.content_length(), std::nullopt);
    EXPECT_EQ(HeaderValidator::HEADER_FIELD_INVALID, v.ValidateSingleHeader("content-length", ""));
    EXPECT_EQ(v.content_length(), std::nullopt);
    EXPECT_EQ(HeaderValidator::HEADER_FIELD_INVALID, v.ValidateSingleHeader("content-length", "nan"));
    EXPECT_EQ(v.content_length(), std::nullopt);
    EXPECT_EQ(HeaderValidator::HEADER_FIELD_INVALID, v.ValidateSingleHeader("content-length", "-42"));
    EXPECT_EQ(v.content_length(), std::nullopt);
...
TEST(HeaderValidatorTest, TeHeader)
{
    ...
    v.StartHeaderBlock();
    EXPECT_EQ(HeaderValidator::HEADER_OK, v.ValidateSingleHeader("te", "trailers"));

    v.StartHeaderBlock();
    EXPECT_EQ(HeaderValidator::HEADER_FIELD_INVALID, v.ValidateSingleHeader("te", "trailers, deflate"));
}
...
TEST(HeaderValidatorTest, ConnectionSpecificHeaders)
{
    ...
    for (const auto& [connection_key, connection_value] : connection_headers) {
        ...
        for (const auto& [sample_key, sample_value] : kSampleRequestPseudoheaders) {
            EXPECT_EQ(HeaderValidator::HEADER_OK, v.ValidateSingleHeader(sample_key, sample_value));
        }
        EXPECT_EQ(HeaderValidator::HEADER_FIELD_INVALID, v.ValidateSingleHeader(connection_key, connection_value));
    }
}
...
TEST(HeaderValidatorTest, MixedCaseHeaderName)
{
    ...
    EXPECT_EQ(HeaderValidator::HEADER_OK, v.ValidateSingleHeader("MixedCaseName", "value"));
}
...
TEST(HeaderValidatorTest, MixedCasePseudoHeader)
{
    HeaderValidator v;
    v.SetAllowUppercaseInHeaderNames();
    EXPECT_EQ(HeaderValidator::HEADER_FIELD_INVALID, v.ValidateSingleHeader(":PATH", "/"));
}
