...
public:
    ...
    void InitializeSession()
    {
        ...
        if (GetPerspective() == Perspective::kClient) {
            nghttp2_session_client_new2(&ptr, nghttp2_callbacks.get(), &mock_callbacks_, options);
        } else {
            nghttp2_session_server_new2(&ptr, nghttp2_callbacks.get(), &mock_callbacks_, options);
        }
        nghttp2_option_del(options);

        // Sets up the Send() callback to append to |serialized_|.
        EXPECT_CALL(mock_callbacks_, Send(_, _, _)).WillRepeatedly([this](const uint8_t* data, size_t length, int /*flags*/) {
        ...
        EXPECT_CALL(mock_callbacks_, SendData(_, _, _, _))
            .WillRepeatedly([this](nghttp2_frame* /*frame*/, const uint8_t* framehd, size_t length, nghttp2_data_source* source) {
                QUICHE_LOG(INFO) << "Appending frame header and " << length << " bytes of data";
                auto* s = static_cast<TestDataSource*>(source->ptr);
                absl::StrAppend(&serialized_, ToStringView(framehd, 9), s->ReadNext(length));
                return 0;
            });
...
public:
    Perspective GetPerspective() override
    {
        return Perspective::kClient;
    }
};
...
TEST_F(Nghttp2ClientTest, ClientReceivesUnexpectedHeaders)
{
    ...
    EXPECT_CALL(mock_callbacks_, OnBeginFrame(HasFrameHeader(0, WINDOW_UPDATE, 0)));
    EXPECT_CALL(mock_callbacks_, OnFrameRecv(IsWindowUpdate(1000)));

    ssize_t result = nghttp2_session_mem_recv(session_.get(), ToUint8Ptr(initial_frames.data()), initial_frames.size());
    ...
    nghttp2_session_mem_recv(session_.get(), ToUint8Ptr(unexpected_stream_frames.data()), unexpected_stream_frames.size());
}
...
TEST_F(Nghttp2ClientTest, ClientSendsRequest)
{
    ...
    ssize_t recv_result = nghttp2_session_mem_recv(session_.get(), ToUint8Ptr(initial_frames.data()), initial_frames.size());
    ...
    std::vector<std::pair<absl::string_view, absl::string_view>> headers
        = { { ":method", "POST" }, { ":scheme", "http" }, { ":authority", "example.com" }, { ":path", "/this/is/request/one" } };
    ...
    int stream_id = nghttp2_submit_request(session_.get(), nullptr /* pri_spec */, nvs.data(), nvs.size(), &provider, nullptr /* stream_data */);
    ...
    EXPECT_THAT(serialized_, EqualsFrames({ spdy::SpdyFrameType::HEADERS, spdy::SpdyFrameType::DATA }));
...
TEST_F(Nghttp2ServerTest, MismatchedContentLength)
{
    ...
    // HEADERS on stream 1
    EXPECT_CALL(mock_callbacks_, OnBeginFrame(HasFrameHeader(1, HEADERS, NGHTTP2_FLAG_END_HEADERS)));

    EXPECT_CALL(mock_callbacks_, OnBeginHeaders(IsHeaders(1, NGHTTP2_FLAG_END_HEADERS, NGHTTP2_HCAT_REQUEST)));
    ...
    EXPECT_CALL(mock_callbacks_, OnFrameRecv(IsHeaders(1, NGHTTP2_FLAG_END_HEADERS, NGHTTP2_HCAT_REQUEST)));

    // DATA on stream 1
    EXPECT_CALL(mock_callbacks_, OnBeginFrame(HasFrameHeader(1, DATA, NGHTTP2_FLAG_END_STREAM)));

    EXPECT_CALL(mock_callbacks_, OnDataChunkRecv(NGHTTP2_FLAG_END_STREAM, 1, "Less than 50 bytes."));
