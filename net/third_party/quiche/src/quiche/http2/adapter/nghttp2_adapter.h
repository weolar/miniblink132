...
public:
    ...
    // |options|.
    static std::unique_ptr<NgHttp2Adapter> CreateClientAdapter(Http2VisitorInterface& visitor, const nghttp2_option* options = nullptr);

    // Creates an adapter that functions as a server. Does not take ownership of
    // |options|.
    static std::unique_ptr<NgHttp2Adapter> CreateServerAdapter(Http2VisitorInterface& visitor, const nghttp2_option* options = nullptr);
    ...
    void SubmitPriorityForStream(Http2StreamId stream_id, Http2StreamId parent_stream_id, int weight, bool exclusive) override;
    ...
    void SubmitGoAway(Http2StreamId last_accepted_stream_id, Http2ErrorCode error_code, absl::string_view opaque_data) override;

    void SubmitWindowUpdate(Http2StreamId stream_id, int window_increment) override;

    void SubmitRst(Http2StreamId stream_id, Http2ErrorCode error_code) override;

    void SubmitMetadata(Http2StreamId stream_id, size_t max_frame_size, std::unique_ptr<MetadataSource> source) override;
    ...
    void MarkDataConsumedForStream(Http2StreamId stream_id, size_t num_bytes) override;

    int32_t SubmitRequest(absl::Span<const Header> headers, std::unique_ptr<DataFrameSource> data_source, bool end_stream, void* user_data) override;

    int SubmitResponse(Http2StreamId stream_id, absl::Span<const Header> headers, std::unique_ptr<DataFrameSource> data_source, bool end_stream) override;

    int SubmitTrailer(Http2StreamId stream_id, absl::Span<const Header> trailers) override;
    ...
    size_t sources_size() const
    {
        return sources_.size();
    }
    size_t stream_metadata_size() const
    {
        return stream_metadata_.size();
    }
    size_t pending_metadata_count(Http2StreamId stream_id) const
    {
        if (auto it = stream_metadata_.find(stream_id); it != stream_metadata_.end()) {
    ...
    // DataFrameSource.
    ssize_t DelegateReadCallback(int32_t stream_id, size_t max_length, uint32_t* data_flags);

    // Delegates a DATA frame send callback to either the visitor or a registered
    // DataFrameSource.
    int DelegateSendCallback(int32_t stream_id, const uint8_t* framehd, size_t length);

private:
    class NotifyingMetadataSource;
    class NotifyingVisitorMetadataSource;

    NgHttp2Adapter(Http2VisitorInterface& visitor, Perspective perspective, const nghttp2_option* options);
    ...
    using MetadataSourceVec = absl::InlinedVector<std::unique_ptr<MetadataSource>, 2>;
