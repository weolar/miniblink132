...
TEST(ClientCallbackVisitorUnitTest, ConnectionFrames)
{
    ...
    CallbackVisitor visitor(Perspective::kClient, *MockNghttp2Callbacks::GetCallbacks(), &callbacks);
    ...
    // PING ack
    EXPECT_CALL(callbacks, OnBeginFrame(HasFrameHeader(0, PING, NGHTTP2_FLAG_ACK)));
    ...
    EXPECT_CALL(callbacks, OnFrameRecv(IsGoAway(5, NGHTTP2_ENHANCE_YOUR_CALM, "calm down!!")));
...
TEST(ClientCallbackVisitorUnitTest, StreamFrames)
{
    ...
    visitor.set_stream_close_listener([&stream_close_counts](Http2StreamId stream_id) { ++stream_close_counts[stream_id]; });
    ...
    EXPECT_CALL(callbacks, OnHeader(_, "date", "Tue, 6 Apr 2021 12:54:01 GMT", _));
    ...
    EXPECT_CALL(callbacks, OnBeginFrame(HasFrameHeader(1, DATA, NGHTTP2_FLAG_END_STREAM)));
    ...
    EXPECT_EQ(visitor.stream_map_size(), 0);
    EXPECT_THAT(stream_close_counts, UnorderedElementsAre(Pair(3, 1), Pair(1, 1)));
    ...
    EXPECT_EQ(visitor.stream_map_size(), 0);
    EXPECT_THAT(stream_close_counts, UnorderedElementsAre(Pair(3, 1), Pair(1, 1), Pair(5, 1)));
}
...
TEST(ClientCallbackVisitorUnitTest, HeadersWithContinuation)
{
    ...
    EXPECT_CALL(callbacks, OnBeginFrame(HasFrameHeader(1, CONTINUATION, END_HEADERS_FLAG)));
...
TEST(ClientCallbackVisitorUnitTest, ResetAndGoaway)
{
    ...
    EXPECT_CALL(callbacks, OnFrameRecv(IsGoAway(3, NGHTTP2_ENHANCE_YOUR_CALM, "calma te")));
    EXPECT_TRUE(visitor.OnGoAway(3, Http2ErrorCode::ENHANCE_YOUR_CALM, "calma te"));

    EXPECT_CALL(callbacks, OnStreamClose(5, NGHTTP2_STREAM_CLOSED)).WillOnce(testing::Return(NGHTTP2_ERR_CALLBACK_FAILURE));
...
{
    testing::StrictMock<MockNghttp2Callbacks> callbacks;
    CallbackVisitor visitor(Perspective::kServer, *MockNghttp2Callbacks::GetCallbacks(), &callbacks);
...
TEST(ServerCallbackVisitorUnitTest, StreamFrames)
{
    ...
    // HEADERS on stream 1
    EXPECT_CALL(callbacks, OnBeginFrame(HasFrameHeader(1, HEADERS, NGHTTP2_FLAG_END_HEADERS)));
    visitor.OnFrameHeader(1, 23, HEADERS, 4);

    EXPECT_CALL(callbacks, OnBeginHeaders(IsHeaders(1, NGHTTP2_FLAG_END_HEADERS, NGHTTP2_HCAT_REQUEST)));
    visitor.OnBeginHeadersForStream(1);
    ...
    EXPECT_CALL(callbacks, OnFrameRecv(IsHeaders(1, NGHTTP2_FLAG_END_HEADERS, NGHTTP2_HCAT_REQUEST)));
    visitor.OnEndHeadersForStream(1);
    ...
    EXPECT_CALL(callbacks, OnDataChunkRecv(NGHTTP2_FLAG_END_STREAM, 1, "This is the request body."));
    EXPECT_CALL(callbacks, OnFrameRecv(IsData(1, _, NGHTTP2_FLAG_END_STREAM)));
...
TEST(ServerCallbackVisitorUnitTest, DataWithPadding)
{
    ...
    EXPECT_CALL(callbacks, OnDataChunkRecv(kFlags, 1, "This is the request body."));
    ...
    // Data before padding.
    EXPECT_CALL(callbacks, OnDataChunkRecv(kFlags, 3, "This is the request body."));
    ...
    EXPECT_CALL(callbacks, OnDataChunkRecv(kFlags, 5, "This is the request body."));
    EXPECT_TRUE(visitor.OnDataForStream(5, "This is the request body."));

    EXPECT_CALL(callbacks, OnFrameRecv(IsData(5, _, kFlags, kPaddingLength))).WillOnce(testing::Return(NGHTTP2_ERR_CALLBACK_FAILURE));
...
TEST(ServerCallbackVisitorUnitTest, MismatchedContentLengthCallbacks)
{
    ...
    const std::string frames
        = TestFrameSequence()
              .ClientPreface()
              .Headers(1, { { ":method", "POST" }, { ":scheme", "https" }, { ":authority", "example.com" }, { ":path", "/" }, { "content-length", "50" } },
                  /*fin=*/false)
    ...
    EXPECT_CALL(callbacks, OnDataChunkRecv(NGHTTP2_FLAG_END_STREAM, 1, "Less than 50 bytes."));
...
TEST(ServerCallbackVisitorUnitTest, HeadersAfterFin)
{
    ...
    // HEADERS on stream 1
    EXPECT_CALL(callbacks, OnBeginFrame(HasFrameHeader(1, HEADERS, NGHTTP2_FLAG_END_HEADERS | NGHTTP2_FLAG_END_STREAM)));
    visitor.OnFrameHeader(1, 23, HEADERS, 5);

    EXPECT_CALL(callbacks, OnBeginHeaders(IsHeaders(1, NGHTTP2_FLAG_END_HEADERS | NGHTTP2_FLAG_END_STREAM, NGHTTP2_HCAT_REQUEST)));
    ...
    EXPECT_CALL(callbacks, OnFrameRecv(IsHeaders(1, NGHTTP2_FLAG_END_HEADERS | NGHTTP2_FLAG_END_STREAM, NGHTTP2_HCAT_REQUEST)));
    visitor.OnEndHeadersForStream(1);
    ...
    EXPECT_CALL(callbacks, OnBeginHeaders(IsHeaders(1, NGHTTP2_FLAG_END_HEADERS, NGHTTP2_HCAT_HEADERS)));
    EXPECT_TRUE(visitor.OnBeginHeadersForStream(1));
