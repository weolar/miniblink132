...
{
    return chunks_.empty() || (chunks_.size() == 1 && chunks_.front().live.empty());
}
...
void ChunkedBuffer::Chunk::AppendSuffix(absl::string_view to_append)
{
    ...
    if (live.empty()) {
        ...
        std::copy(to_append.begin(), to_append.end(), const_cast<char*>(live.data()) + live.size());
...
void ChunkedBuffer::TrimFirstChunk()
{
    ...
    if (chunks_.empty() || (chunks_.size() == 1 && chunks_.front().size == kDefaultChunkSize)) {
        return;
    }
    chunks_.pop_front();
}
