...
constexpr absl::string_view kHttpTokenChars = "!#$%&'*+-.^_`|~0123456789"
                                              "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ";

constexpr absl::string_view kHttp2HeaderNameAllowedChars = "!#$%&'*+-.0123456789"
...
constexpr absl::string_view kValidAuthorityChars = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789-._~%!$&'()["
                                                   "]*+,;=:";

constexpr absl::string_view kValidPathChars = "/abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789-._~%!$&'()"
                                              "*+,;=:@?";

constexpr absl::string_view kValidPathCharsWithFragment = "/abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789-._~%!$&'()"
                                                          "*+,;=:@?#";
...
{
    static constexpr CharMap valid_chars = BuildValidCharMap(kHttp2StatusValueAllowedChars);
...
HeaderValidator::HeaderStatus HeaderValidator::ValidateSingleHeader(absl::string_view key, absl::string_view value)
{
    if (key.empty()) {
        return HEADER_FIELD_INVALID;
    }
    if (max_field_size_.has_value() && key.size() + value.size() > *max_field_size_) {
        QUICHE_VLOG(2) << "Header field size is " << key.size() + value.size() << ", exceeds max size of " << *max_field_size_;
    ...
    if (key[0] == ':') {
        ...
        if (key == "status") {
            ...
            if (value == "*") {
                ...
                return HEADER_FIELD_INVALID;
            } else if (validate_path_ && !IsValidPath(value, allow_fragment_in_path_)) {
                return HEADER_FIELD_INVALID;
            }
            if (value[0] == '/') {
...
bool HeaderValidator::FinishHeaderBlock(HeaderType type)
{
    ...
    case HeaderType::REQUEST:
        return ValidateRequestHeaders(pseudo_headers_, pseudo_header_state_, allow_extended_connect_);
...
{
    static constexpr CharMap valid_chars = BuildValidCharMap(kHttp2HeaderNameAllowedChars);
    return AllCharsInMap(name, valid_chars);
}

bool HeaderValidator::IsValidHeaderValue(absl::string_view value, ObsTextOption option)
{
    static constexpr CharMap valid_chars = BuildValidCharMap(kHttp2HeaderValueAllowedChars);
    static constexpr CharMap valid_chars_with_obs_text = AllowObsText(BuildValidCharMap(kHttp2HeaderValueAllowedChars));
    return AllCharsInMap(value, option == ObsTextOption::kAllow ? valid_chars_with_obs_text : valid_chars);
}

bool HeaderValidator::IsValidAuthority(absl::string_view authority)
{
    static constexpr CharMap valid_chars = BuildValidCharMap(kValidAuthorityChars);
...
bool HeaderValidator::IsValidPath(absl::string_view path, bool allow_fragment)
{
    ...
    static constexpr CharMap valid_chars_with_fragment = BuildValidCharMap(kValidPathCharsWithFragment);
    if (allow_fragment) {
...
HeaderValidator::ContentLengthStatus HeaderValidator::HandleContentLength(absl::string_view value)
{
    if (value.empty()) {
    ...
    if (content_length_.has_value()) {
        return content_length == *content_length_ ? CONTENT_LENGTH_SKIP : CONTENT_LENGTH_ERROR;
    }
...
bool HeaderValidator::ValidateAndSetAuthority(absl::string_view authority)
{
    ...
    if (!allow_different_host_and_authority_ && pseudo_headers_[TAG_AUTHORITY] && authority != authority_) {
        return false;
    }
    if (!authority.empty()) {
...
bool HeaderValidator::ValidateRequestHeaders(
    const PseudoHeaderTagSet& pseudo_headers, const PseudoHeaderStateSet& pseudo_header_state, bool allow_extended_connect)
{
    QUICHE_VLOG(2) << "Request pseudo-headers: [" << pseudo_headers << "], pseudo_header_state: [" << pseudo_header_state
    ...
    if (pseudo_header_state[STATE_METHOD_IS_CONNECT]) {
        if (allow_extended_connect) {
            ...
            static const auto* kExtendedConnectHeaders = new PseudoHeaderTagSet(0b0011111);
        ...
        return pseudo_header_state[STATE_AUTHORITY_IS_NONEMPTY] && pseudo_headers == *kConnectHeaders;
    }
...
bool HeaderValidator::ValidateRequestTrailers(const PseudoHeaderTagSet& pseudo_headers)
{
    return pseudo_headers.none();
}

bool HeaderValidator::ValidateResponseHeaders(const PseudoHeaderTagSet& pseudo_headers)
{
...
bool HeaderValidator::ValidateResponseTrailers(const PseudoHeaderTagSet& pseudo_headers)
{
    return pseudo_headers.none();
}
