...
int TestSendCallback(nghttp2_session*, nghttp2_frame* /*frame*/, const uint8_t* framehd, size_t length, nghttp2_data_source* source, void* user_data)
{
...
TEST(NgHttp2AdapterTest, ClientHandlesFrames)
{
    ...
    EXPECT_EQ(0, result);
    EXPECT_THAT(visitor.data(), testing::StrEq(spdy::kHttp2ConnectionHeaderPrefix));
    visitor.Clear();

    EXPECT_EQ(0, adapter->GetHighestReceivedStreamId());

    const std::string initial_frames = TestFrameSequence().ServerPreface().Ping(42).WindowUpdate(0, 1000).Serialize();
    ...
    const int32_t stream_id3 = adapter->SubmitRequest(headers3, nullptr, true, const_cast<char*>(kSentinel3));
    ...
    result = adapter->Send();
    EXPECT_EQ(0, result);
    EXPECT_THAT(visitor.data(), EqualsFrames({ SpdyFrameType::HEADERS, SpdyFrameType::HEADERS, SpdyFrameType::HEADERS }));
    visitor.Clear();

    // All streams are active and have not yet received any data, so the receive
    // window should be at the initial value.
    EXPECT_EQ(kInitialFlowControlWindowSize, adapter->GetStreamReceiveWindowSize(stream_id1));
    EXPECT_EQ(kInitialFlowControlWindowSize, adapter->GetStreamReceiveWindowSize(stream_id2));
    EXPECT_EQ(kInitialFlowControlWindowSize, adapter->GetStreamReceiveWindowSize(stream_id3));

    // Upper bound on the flow control receive window should be the initial value.
    EXPECT_EQ(kInitialFlowControlWindowSize, adapter->GetStreamReceiveWindowLimit(stream_id1));
    ...
    EXPECT_CALL(visitor, OnCloseStream(3, Http2ErrorCode::INTERNAL_ERROR)).WillOnce([&adapter](Http2StreamId stream_id, Http2ErrorCode /*error_code*/) {
        adapter->RemoveStream(stream_id);
        return true;
    });
    EXPECT_CALL(visitor, OnFrameHeader(0, 19, GOAWAY, 0));
    EXPECT_CALL(visitor, OnGoAway(5, Http2ErrorCode::ENHANCE_YOUR_CALM, "calm down!!"));
    const int64_t stream_result = adapter->ProcessBytes(stream_frames);
    EXPECT_EQ(stream_frames.size(), stream_result);

    // First stream has received some data.
    EXPECT_GT(kInitialFlowControlWindowSize, adapter->GetStreamReceiveWindowSize(stream_id1));
    ...
    EXPECT_EQ(adapter->GetReceiveWindowSize(), adapter->GetStreamReceiveWindowSize(stream_id1));
    ...
    EXPECT_CALL(visitor, OnCloseStream(1, Http2ErrorCode::HTTP2_NO_ERROR)).WillOnce([&adapter](Http2StreamId stream_id, Http2ErrorCode /*error_code*/) {
        adapter->RemoveStream(stream_id);
        return true;
    });
    EXPECT_CALL(visitor, OnFrameHeader(5, 4, RST_STREAM, 0));
    EXPECT_CALL(visitor, OnRstStream(5, Http2ErrorCode::REFUSED_STREAM));
    EXPECT_CALL(visitor, OnCloseStream(5, Http2ErrorCode::REFUSED_STREAM)).WillOnce([&adapter](Http2StreamId stream_id, Http2ErrorCode /*error_code*/) {
        adapter->RemoveStream(stream_id);
        return true;
    });
    adapter->ProcessBytes(TestFrameSequence().Data(1, "", true).RstStream(5, Http2ErrorCode::REFUSED_STREAM).Serialize());
...
TEST(NgHttp2AdapterTest, ClientRejectsFinFollowing100Headers)
{
    ...
    EXPECT_CALL(visitor, OnBeforeFrameSent(RST_STREAM, 1, _, 0x0));
    EXPECT_CALL(visitor, OnFrameSent(RST_STREAM, 1, _, 0x0, 1));
    EXPECT_CALL(visitor, OnCloseStream(stream_id1, Http2ErrorCode::PROTOCOL_ERROR));
...
TEST_P(NgHttp2AdapterDataTest, ClientSendsTrailers)
{
    ...
    EXPECT_THAT(data, testing::StartsWith(spdy::kHttp2ConnectionHeaderPrefix));
    data.remove_prefix(strlen(spdy::kHttp2ConnectionHeaderPrefix));
    EXPECT_THAT(data, EqualsFrames({ SpdyFrameType::HEADERS, SpdyFrameType::DATA }));
    visitor.Clear();
...
TEST(NgHttp2AdapterTest, ClientHandlesMetadataWithEmptyPayload)
{
    ...
    const int32_t stream_id = adapter->SubmitRequest(headers1, nullptr, true, nullptr);
    ASSERT_GT(stream_id, 0);
...
TEST(NgHttp2AdapterTest, ClientHandlesMetadataWithError)
{
    ...
    EXPECT_CALL(visitor, OnFrameHeader(1, _, kMetadataFrameType, 4));
    EXPECT_CALL(visitor, OnBeginMetadataForStream(1, _));
    EXPECT_CALL(visitor, OnMetadataForStream(1, _)).WillOnce(testing::Return(false));
...
TEST(NgHttp2AdapterTest, ClientReceivesGoAway)
{
    ...
    EXPECT_THAT(data, testing::StartsWith(spdy::kHttp2ConnectionHeaderPrefix));
    data.remove_prefix(strlen(spdy::kHttp2ConnectionHeaderPrefix));
    EXPECT_THAT(data, EqualsFrames({ SpdyFrameType::HEADERS, SpdyFrameType::HEADERS }));
    visitor.Clear();
    ...
    EXPECT_CALL(visitor, OnGoAway(1, Http2ErrorCode::INTERNAL_ERROR, "indigestion"));
...
TEST(NgHttp2AdapterTest, ClientReceivesMultipleGoAways)
{
    ...
    EXPECT_CALL(visitor, OnSettingsEnd());
    EXPECT_CALL(visitor, OnFrameHeader(0, _, GOAWAY, 0));
    EXPECT_CALL(visitor, OnGoAway(kMaxStreamId, Http2ErrorCode::INTERNAL_ERROR, "indigestion"));
    ...
    EXPECT_CALL(visitor, OnFrameHeader(0, _, GOAWAY, 0));
    EXPECT_CALL(visitor, OnGoAway(0, Http2ErrorCode::INTERNAL_ERROR, "indigestion"));
    EXPECT_CALL(visitor, OnCloseStream(1, Http2ErrorCode::REFUSED_STREAM));
...
TEST(NgHttp2AdapterTest, ClientFailsOnGoAway)
{
    ...
    EXPECT_CALL(visitor, OnEndHeadersForStream(1));
    EXPECT_CALL(visitor, OnFrameHeader(0, _, GOAWAY, 0));
    EXPECT_CALL(visitor, OnGoAway(1, Http2ErrorCode::INTERNAL_ERROR, "indigestion")).WillOnce(testing::Return(false));
    EXPECT_CALL(visitor, OnConnectionError(ConnectionError::kParseError));
...
TEST_P(NgHttp2AdapterDataTest, ClientSubmitRequest)
{
    ...
    int stream_id = adapter->SubmitRequest(
        ToHeaders({ { ":method", "POST" }, { ":scheme", "http" }, { ":authority", "example.com" }, { ":path", "/this/is/request/one" } }),
        GetParam() ? nullptr : std::move(body1), false, const_cast<char*>(kSentinel));
    ...
    EXPECT_EQ(kInitialFlowControlWindowSize, adapter->GetStreamReceiveWindowSize(stream_id));
    EXPECT_EQ(kInitialFlowControlWindowSize, adapter->GetReceiveWindowSize());
    EXPECT_EQ(kInitialFlowControlWindowSize, adapter->GetStreamReceiveWindowLimit(stream_id));
    ...
    // the default.
    EXPECT_LT(adapter->GetStreamSendWindowSize(stream_id), kInitialFlowControlWindowSize);
    ...
    stream_id = adapter->SubmitRequest(
    ...
    EXPECT_EQ(adapter->GetStreamSendWindowSize(stream_id), kInitialFlowControlWindowSize);
}
...
TEST(NgHttp2AdapterTest, ClientSubmitRequestWithDataProvider)
{
    ...
    std::unique_ptr<DataFrameSource> frame_source = MakeZeroCopyDataFrameSource(provider, &visitor, std::move(send_callback));
    int stream_id = adapter->SubmitRequest(
        ToHeaders({ { ":method", "POST" }, { ":scheme", "http" }, { ":authority", "example.com" }, { ":path", "/this/is/request/one" } }),
...
TEST(NgHttp2AdapterTest, ClientSubmitRequestWithDataProviderAndWriteBlock)
{
    ...
    absl::string_view serialized = visitor.data();
    EXPECT_THAT(serialized, EqualsFrames({ SpdyFrameType::HEADERS, SpdyFrameType::DATA }));
    EXPECT_FALSE(adapter->want_write());
}
...
INSTANTIATE_TEST_SUITE_P(WithAndWithoutNewApi, MetadataApiTest, testing::Bool());
...
TEST_P(MetadataApiTest, SubmitMetadata)
{
    ...
    const quiche::HttpHeaderBlock block = ToHeaderBlock(ToHeaders({ { "query-cost", "is too darn high" }, { "secret-sauce", "hollandaise" } }));
    if (GetParam()) {
    ...
    EXPECT_THAT(serialized, testing::StartsWith(spdy::kHttp2ConnectionHeaderPrefix));
    serialized.remove_prefix(strlen(spdy::kHttp2ConnectionHeaderPrefix));
    EXPECT_THAT(serialized, EqualsFrames({ static_cast<SpdyFrameType>(kMetadataFrameType) }));
    EXPECT_FALSE(adapter->want_write());
}
...
TEST_P(MetadataApiTest, SubmitMetadataMultipleFrames)
{
    ...
    const quiche::HttpHeaderBlock block = ToHeaderBlock(ToHeaders({ { "large-value", kLargeValue } }));
    if (GetParam()) {
        visitor.AppendMetadataForStream(1, block);
    ...
    EXPECT_THAT(serialized,
        EqualsFrames({ static_cast<SpdyFrameType>(kMetadataFrameType), static_cast<SpdyFrameType>(kMetadataFrameType),
            static_cast<SpdyFrameType>(kMetadataFrameType), static_cast<SpdyFrameType>(kMetadataFrameType) }));
    EXPECT_FALSE(adapter->want_write());
}
...
TEST_P(MetadataApiTest, ClientSubmitMetadataWithGoaway)
{
    ...
    const std::string initial_frames = TestFrameSequence().ServerPreface().GoAway(3, Http2ErrorCode::HTTP2_NO_ERROR, "server shutting down").Serialize();
    testing::InSequence s;
    ...
    EXPECT_CALL(visitor, OnCloseStream(stream_id, Http2ErrorCode::REFUSED_STREAM));
    ...
    EXPECT_THAT(serialized, testing::StartsWith(spdy::kHttp2ConnectionHeaderPrefix));
    serialized.remove_prefix(strlen(spdy::kHttp2ConnectionHeaderPrefix));
    EXPECT_THAT(serialized, EqualsFrames({ SpdyFrameType::SETTINGS, SpdyFrameType::SETTINGS, static_cast<SpdyFrameType>(kMetadataFrameType) }));
    EXPECT_FALSE(adapter->want_write());
}
...
TEST_P(MetadataApiTest, ClientSubmitMetadataWithFailureBefore)
{
    ...
    EXPECT_CALL(visitor, OnBeforeFrameSent(SETTINGS, 0, _, 0x1));
    EXPECT_CALL(visitor, OnFrameSent(SETTINGS, 0, _, 0x1, 0));
    EXPECT_CALL(visitor, OnBeforeFrameSent(kMetadataFrameType, stream_id, _, 0x4)).WillOnce(testing::Return(NGHTTP2_ERR_TEMPORAL_CALLBACK_FAILURE));
    EXPECT_CALL(visitor, OnConnectionError(Http2VisitorInterface::ConnectionError::kSendError));
...
TEST_P(MetadataApiTest, ClientSubmitMetadataWithFailureDuring)
{
    ...
    const quiche::HttpHeaderBlock block = ToHeaderBlock(ToHeaders({ { "more-than-one-frame", std::string(20000, 'a') } }));
    if (GetParam()) {
        visitor.AppendMetadataForStream(stream_id, block);
    ...
    EXPECT_CALL(visitor, OnBeforeFrameSent(SETTINGS, 0, _, 0x1));
    EXPECT_CALL(visitor, OnFrameSent(SETTINGS, 0, _, 0x1, 0));
    EXPECT_CALL(visitor, OnBeforeFrameSent(kMetadataFrameType, stream_id, _, 0x0));
    EXPECT_CALL(visitor, OnFrameSent(kMetadataFrameType, stream_id, _, 0x0, 0)).WillOnce(testing::Return(NGHTTP2_ERR_TEMPORAL_CALLBACK_FAILURE));
    EXPECT_CALL(visitor, OnConnectionError(Http2VisitorInterface::ConnectionError::kSendError));
...
TEST_P(MetadataApiTest, ClientSubmitMetadataWithFailureSending)
{
    ...
    if (GetParam()) {
        ...
        auto source = std::make_unique<TestMetadataSource>(ToHeaderBlock(ToHeaders({ { "more-than-one-frame", std::string(20000, 'a') } })));
...
TEST_P(NgHttp2AdapterDataTest, ClientReceivesInitialWindowSettingAfterStreamStart)
{
    TestVisitor visitor;
    auto adapter = NgHttp2Adapter::CreateClientAdapter(visitor);
...
TEST(NgHttp2AdapterTest, InitialWindowSettingCausesOverflow)
{
    ...
    EXPECT_CALL(visitor, OnCloseStream(stream_id, Http2ErrorCode::FLOW_CONTROL_ERROR));
...
TEST(NgHttp2AdapterTest, ClientForbidsPushPromise)
{
    ...
    EXPECT_CALL(visitor, OnBeforeFrameSent(GOAWAY, 0, _, 0x0));
    EXPECT_CALL(visitor, OnFrameSent(GOAWAY, 0, _, 0x0, static_cast<int32_t>(Http2ErrorCode::PROTOCOL_ERROR)));
...
TEST(NgHttp2AdapterTest, ConnectionErrorOnControlFrameSent)
{
    ...
    // SETTINGS ack
    EXPECT_CALL(visitor, OnBeforeFrameSent(SETTINGS, 0, 0, 0x1));
    EXPECT_CALL(visitor, OnFrameSent(SETTINGS, 0, 0, 0x1, 0)).WillOnce(testing::Return(-902));
    EXPECT_CALL(visitor, OnConnectionError(ConnectionError::kSendError));
...
TEST(NgHttp2AdapterTest, ServerHandlesFrames)
{
    ...
    EXPECT_GT(kInitialFlowControlWindowSize, adapter->GetStreamReceiveWindowSize(1));
    EXPECT_EQ(adapter->GetStreamReceiveWindowSize(1), adapter->GetReceiveWindowSize());
    // Upper bound should still be the original value.
    EXPECT_EQ(kInitialFlowControlWindowSize, adapter->GetStreamReceiveWindowLimit(1));
    ...
    EXPECT_THAT(visitor.data(), EqualsFrames({ SpdyFrameType::SETTINGS, SpdyFrameType::PING, SpdyFrameType::PING }));
}
...
TEST_P(NgHttp2AdapterDataTest, ServerSubmitsTrailersWithDataEndStream)
{
    ...
    EXPECT_CALL(visitor, OnBeforeFrameSent(HEADERS, 1, _, END_HEADERS_FLAG | END_STREAM_FLAG));
    EXPECT_CALL(visitor, OnFrameSent(HEADERS, 1, _, END_HEADERS_FLAG | END_STREAM_FLAG, 0));

    const int send_result = adapter->Send();
    EXPECT_EQ(send_result, 0);
    EXPECT_THAT(visitor.data(), EqualsFrames({ SpdyFrameType::SETTINGS, SpdyFrameType::HEADERS, SpdyFrameType::HEADERS }));
}

TEST_P(NgHttp2AdapterDataTest, ServerSubmitsTrailersWithDataEndStreamAndDeferral)
{
    TestVisitor visitor;
    auto adapter = NgHttp2Adapter::CreateServerAdapter(visitor);
...
TEST(NgHttp2AdapterTest, ClientDisobeysConnectionFlowControlAcrossReads)
{
    ...
    EXPECT_CALL(visitor, OnDataForStream(1, _)).WillRepeatedly([&adapter](Http2StreamId stream_id, absl::string_view data) {
    ...
    send_result = adapter->Send();
    EXPECT_EQ(0, send_result);
    EXPECT_THAT(visitor.data(), EqualsFrames({ SpdyFrameType::WINDOW_UPDATE, SpdyFrameType::WINDOW_UPDATE }));
}
...
TEST(NgHttp2AdapterTest, ServerReceivesTooLargeHeader)
{
    ...
    EXPECT_CALL(visitor, OnFrameSent(GOAWAY, 0, 8, 0x0, static_cast<int>(Http2ErrorCode::COMPRESSION_ERROR)));
...
TEST_P(NgHttp2AdapterDataTest, ServerSubmitResponseWithResetFromClient)
{
    ...
    EXPECT_CALL(visitor, OnFrameHeader(1, 4, RST_STREAM, 0));
    EXPECT_CALL(visitor, OnRstStream(1, Http2ErrorCode::CANCEL));
    EXPECT_CALL(visitor, OnCloseStream(1, Http2ErrorCode::CANCEL)).WillOnce([&adapter](Http2StreamId stream_id, Http2ErrorCode /*error_code*/) {
        adapter->RemoveStream(stream_id);
        return true;
    });
...
TEST_P(NgHttp2AdapterDataTest, ServerSendsTrailers)
{
    ...
    int trailer_result = adapter->SubmitTrailer(1, ToHeaders({ { "final-status", "a-ok" }, { "x-comment", "trailers sure are cool" } }));
    ASSERT_EQ(trailer_result, 0);
    EXPECT_TRUE(adapter->want_write());
...
TEST(NgHttp2AdapterTest, ClientSendsMetadataWithContinuation)
{
    ...
    const std::string frames
        = TestFrameSequence()
              ...
              .Headers(1, { { ":method", "GET" }, { ":scheme", "https" }, { ":authority", "example.com" }, { ":path", "/this/is/request/one" } },
                  /*fin=*/false,
                  /*add_continuation=*/true)
              .Metadata(1, "Some stream metadata that's also sent in multiple frames", true)
              .Serialize();
    testing::InSequence s;
    ...
    const int64_t result = adapter->ProcessBytes(frames);
    EXPECT_EQ(frames.size(), result);
    EXPECT_EQ("Example connection metadata in multiple frames", absl::StrJoin(visitor.GetMetadata(0), ""));
    EXPECT_EQ("Some stream metadata that's also sent in multiple frames", absl::StrJoin(visitor.GetMetadata(1), ""));
}
...
TEST_P(NgHttp2AdapterDataTest, ServerSubmitsResponseWithDataSourceError)
{
    ...
    int send_result = adapter->Send();
    EXPECT_EQ(0, send_result);
    EXPECT_THAT(visitor.data(), EqualsFrames({ SpdyFrameType::SETTINGS, SpdyFrameType::HEADERS, SpdyFrameType::RST_STREAM }));
    visitor.Clear();
    EXPECT_FALSE(adapter->want_write());
...
TEST(NgHttp2AdapterInteractionTest, ClientServerInteractionRepeatedHeaderNames)
{
...
TEST_P(NgHttp2AdapterDataTest, SkipsSendingFramesForRejectedStream)
{
    ...
    int send_result = adapter->Send();
    EXPECT_EQ(0, send_result);
    EXPECT_THAT(visitor.data(), EqualsFrames({ SpdyFrameType::SETTINGS, static_cast<SpdyFrameType>(kMetadataFrameType), SpdyFrameType::RST_STREAM }));
}
...
TEST_P(NgHttp2AdapterDataTest, ServerQueuesMetadataWithStreamReset)
{
    ...
    const std::string reset_frame = TestFrameSequence().RstStream(1, Http2ErrorCode::CANCEL).Serialize();
    ...
    source = std::make_unique<TestMetadataSource>(ToHeaderBlock(ToHeaders({ { "really-important", "information!" } })));
    adapter->SubmitMetadata(1, 16384u, std::move(source));
    ...
    EXPECT_EQ(0, send_result);
    EXPECT_THAT(visitor.data(),
        EqualsFrames({ SpdyFrameType::SETTINGS, static_cast<SpdyFrameType>(kMetadataFrameType), static_cast<SpdyFrameType>(kMetadataFrameType) }));
...
TEST_P(NgHttp2AdapterDataTest, ServerDoesNotSendFramesAfterImmediateGoAway)
{
    ...
    EXPECT_THAT(visitor.data(), EqualsFrames({ SpdyFrameType::SETTINGS, SpdyFrameType::SETTINGS, SpdyFrameType::GOAWAY }));
    visitor.Clear();
...
TEST(NgHttp2AdapterTest, ServerHandlesContentLength)
{
    ...
    const std::string stream_frames = TestFrameSequence()
                                          ...
                                          .Headers(3,
                                              { { ":method", "GET" }, { ":scheme", "https" }, { ":authority", "example.com" },
                                                  { ":path", "/this/is/request/two" }, { "content-length", "nan" } },
                                              /*fin=*/true)
                                          .Serialize();
...
TEST(NgHttp2AdapterTest, ServerHandlesContentLengthMismatch)
{
    ...
    EXPECT_EQ(0, result);
    EXPECT_THAT(visitor.data(),
        EqualsFrames({ SpdyFrameType::SETTINGS, SpdyFrameType::RST_STREAM, SpdyFrameType::RST_STREAM, SpdyFrameType::RST_STREAM, SpdyFrameType::RST_STREAM }));
}
...
TEST(NgHttp2AdapterTest, ServerHandlesInvalidPath)
{
    ...
    int result = adapter->Send();
    EXPECT_EQ(0, result);
    EXPECT_THAT(visitor.data(), EqualsFrames({ SpdyFrameType::SETTINGS, SpdyFrameType::RST_STREAM, SpdyFrameType::RST_STREAM, SpdyFrameType::RST_STREAM }));
}
...
TEST(OgHttp2AdapterTest, ServerConsumesDataWithPadding)
{
    ...
    const int send_result = adapter->Send();
    EXPECT_EQ(0, send_result);
    EXPECT_THAT(visitor.data(), EqualsFrames({ SpdyFrameType::SETTINGS, SpdyFrameType::WINDOW_UPDATE, SpdyFrameType::WINDOW_UPDATE }));
}
