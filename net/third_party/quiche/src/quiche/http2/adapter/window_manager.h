...
public:
    ...
    using ShouldWindowUpdateFn = bool (*)(int64_t limit, int64_t size, int64_t delta);

    WindowManager(
        int64_t window_size_limit, WindowUpdateListener listener, ShouldWindowUpdateFn should_window_update_fn = {}, bool update_window_on_notify = true);

    int64_t CurrentWindowSize() const
    {
        return window_;
    }
    int64_t WindowSizeLimit() const
    {
        return limit_;
    }
    ...
    void IncreaseWindow(int64_t delta)
    {
        window_ += delta;
    }
