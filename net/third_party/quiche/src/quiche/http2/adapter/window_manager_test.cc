...
public:
    explicit WindowManagerPeer(const WindowManager& wm)
        : wm_(wm)
    {
    }
...
TEST_F(WindowManagerTest, DataBufferedAndFlushed)
{
    ...
    while (call_sequence_.empty()) {
        int64_t buffered = std::min<int64_t>(kDefaultLimit - total_buffered, random_.Uniform(1024));
        wm_.MarkDataBuffered(buffered);
        total_buffered += buffered;
        EXPECT_TRUE(call_sequence_.empty());
        int64_t flushed = (total_buffered - total_flushed) > 0 ? random_.Uniform(total_buffered - total_flushed) : 0;
...
TEST(WindowManagerNoUpdateTest, NoWindowUpdateOnListener)
{
    ...
    WindowManager wm1(
        kDefaultLimit, [&call_sequence1](int64_t delta) { call_sequence1.push_back(delta); },
        /*should_notify_listener=*/ {},
        /*update_window_on_notify=*/true); // default
    std::list<int64_t> call_sequence2;
    WindowManager wm2(
        kDefaultLimit, [&call_sequence2](int64_t delta) { call_sequence2.push_back(delta); },
        /*should_notify_listener=*/ {},
...
TEST(WindowManagerShouldUpdateTest, CustomShouldWindowUpdateFn)
{
    ...
    WindowManager wm1(
        kDefaultLimit, [&call_sequence1](int64_t delta) { call_sequence1.push_back(delta); },
        [](int64_t /*limit*/, int64_t /*window*/, int64_t /*delta*/) { return true; });
    // This window manager should never notify.
    std::list<int64_t> call_sequence2;
    WindowManager wm2(
        kDefaultLimit, [&call_sequence2](int64_t delta) { call_sequence2.push_back(delta); },
        [](int64_t /*limit*/, int64_t /*window*/, int64_t /*delta*/) { return false; });
    // This window manager should notify as long as no data is buffered.
    std::list<int64_t> call_sequence3;
    WindowManager wm3(
        kDefaultLimit, [&call_sequence3](int64_t delta) { call_sequence3.push_back(delta); },
        [](int64_t limit, int64_t window, int64_t delta) { return delta == limit - window; });
    ...
    EXPECT_THAT(call_sequence1, testing::ElementsAre(consumed, buffered / 3, 2 * buffered / 3));
