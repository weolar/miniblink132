...
NgHttp2Session::NgHttp2Session(Perspective perspective, nghttp2_session_callbacks_unique_ptr callbacks, const nghttp2_option* options, void* userdata)
...
NgHttp2Session::~NgHttp2Session()
{
    ...
    if (pending_reads || pending_writes) {
        QUICHE_VLOG(1) << "Shutting down connection with pending reads: " << pending_reads << " or pending writes: " << pending_writes;
    }
}

int64_t NgHttp2Session::ProcessBytes(absl::string_view bytes)
{
    return nghttp2_session_mem_recv(session_.get(), reinterpret_cast<const uint8_t*>(bytes.data()), bytes.size());
}
