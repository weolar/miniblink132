...
public:
    ...
    virtual void SubmitPriorityForStream(Http2StreamId stream_id, Http2StreamId parent_stream_id, int weight, bool exclusive) = 0;
    ...
    virtual void SubmitGoAway(Http2StreamId last_accepted_stream_id, Http2ErrorCode error_code, absl::string_view opaque_data) = 0;

    // Submits a WINDOW_UPDATE for the given stream (a |stream_id| of 0 indicates
    // a connection-level WINDOW_UPDATE).
    virtual void SubmitWindowUpdate(Http2StreamId stream_id, int window_increment) = 0;

    // Submits a RST_STREAM for the given |stream_id| and |error_code|.
    virtual void SubmitRst(Http2StreamId stream_id, Http2ErrorCode error_code) = 0;

    // Submits a sequence of METADATA frames for the given stream. A |stream_id|
    // of 0 indicates connection-level METADATA.
    virtual void SubmitMetadata(Http2StreamId stream_id, size_t max_frame_size, std::unique_ptr<MetadataSource> source) = 0;
    ...
    virtual void MarkDataConsumedForStream(Http2StreamId stream_id, size_t num_bytes) = 0;
    ...
    int32_t SubmitRequest(absl::Span<const Header> headers, std::unique_ptr<DataFrameSource> data_source, void* user_data)
    {
        const bool end_headers = (data_source == nullptr);
        return SubmitRequest(headers, std::move(data_source), end_headers, user_data);
    }
    virtual int32_t SubmitRequest(absl::Span<const Header> headers, std::unique_ptr<DataFrameSource> data_source, bool end_stream, void* user_data) = 0;
    ...
    // DEPRECATED in favor of the version with end_headers.
    int SubmitResponse(Http2StreamId stream_id, absl::Span<const Header> headers, std::unique_ptr<DataFrameSource> data_source)
    {
        const bool end_headers = (data_source == nullptr);
        return SubmitResponse(stream_id, headers, std::move(data_source), end_headers);
    }
    virtual int SubmitResponse(Http2StreamId stream_id, absl::Span<const Header> headers, std::unique_ptr<DataFrameSource> data_source, bool end_stream) = 0;

    // Queues trailers to be sent after any outstanding data on the stream with ID
    // |stream_id|. Returns 0 on success.
    virtual int SubmitTrailer(Http2StreamId stream_id, absl::Span<const Header> trailers) = 0;
    ...
    virtual void FrameNotSent(Http2StreamId /*stream_id*/, uint8_t /*frame_type*/)
    {
    }
...
protected:
    ...
    explicit Http2Adapter(Http2VisitorInterface& visitor)
        : visitor_(visitor)
    {
    }
