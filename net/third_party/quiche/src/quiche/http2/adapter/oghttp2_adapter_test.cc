...
TEST(OgHttp2AdapterTest, ProcessBytes)
{
    ...
    adapter->ProcessBytes(TestFrameSequence().ClientPreface().Ping(17).Serialize());
}
...
TEST(OgHttp2AdapterTest, HeaderValuesWithObsTextAllowedByDefault)
{
    ...
    const std::string frames
        = TestFrameSequence()
              .ClientPreface()
              .Headers(1, { { ":method", "GET" }, { ":scheme", "https" }, { ":authority", "example.com" }, { ":path", "/" }, { "name", "val\xa1ue" } },
                  /*fin=*/true)
              .Serialize();
...
TEST(OgHttp2AdapterTest, HeaderValuesWithObsTextDisallowed)
{
    ...
    EXPECT_CALL(visitor, OnHeaderForStream(1, ":authority", "example.com"));
    EXPECT_CALL(visitor, OnHeaderForStream(1, ":path", "/"));
    EXPECT_CALL(visitor, OnInvalidFrame(1, Http2VisitorInterface::InvalidFrameError::kHttpHeader));
...
TEST(OgHttp2AdapterTest, RequestPathWithSpaceOrTab)
{
    ...
    const std::string frames = TestFrameSequence()
                                   .ClientPreface()
                                   .Headers(1, { { ":method", "GET" }, { ":scheme", "https" }, { ":authority", "example.com" }, { ":path", "/ fragment" } },
                                       /*fin=*/true)
                                   .Headers(3, { { ":method", "GET" }, { ":scheme", "https" }, { ":authority", "example.com" }, { ":path", "/\tfragment2" } },
                                       /*fin=*/true)
                                   .Serialize();
    testing::InSequence s;
    ...
    EXPECT_CALL(visitor, OnInvalidFrame(3, Http2VisitorInterface::InvalidFrameError::kHttpHeader));
...
TEST(OgHttp2AdapterTest, InitialSettingsNoExtendedConnect)
{
    ...
    EXPECT_CALL(server_visitor, OnSetting(Http2Setting { Http2KnownSettingsId::ENABLE_PUSH, 0u }));
    EXPECT_CALL(server_visitor, OnSetting(Http2Setting { Http2KnownSettingsId::MAX_HEADER_LIST_SIZE, 42u }));
...
TEST(OgHttp2AdapterTest, InitialSettings)
{
    ...
    EXPECT_CALL(client_visitor, OnSettingsStart());
    EXPECT_CALL(client_visitor, OnSetting(Http2Setting { Http2KnownSettingsId::ENABLE_CONNECT_PROTOCOL, 1u }));
    EXPECT_CALL(client_visitor, OnSettingsEnd());
    {
        const int64_t result = client_adapter->ProcessBytes(server_visitor.data());
        EXPECT_EQ(server_visitor.data().size(), static_cast<size_t>(result));
...
TEST(OgHttp2AdapterTest, AutomaticSettingsAndPingAcks)
{
    ...
    const std::string frames = TestFrameSequence().ClientPreface().Ping(42).Serialize();
    testing::InSequence s;
    ...
    EXPECT_THAT(visitor.data(), EqualsFrames({ SpdyFrameType::SETTINGS, SpdyFrameType::SETTINGS, SpdyFrameType::PING }));
}
...
TEST(OgHttp2AdapterTest, AutomaticPingAcksDisabled)
{
    ...
    int send_result = adapter->Send();
    EXPECT_EQ(0, send_result);
    EXPECT_THAT(visitor.data(), EqualsFrames({ SpdyFrameType::SETTINGS, SpdyFrameType::SETTINGS }));
}
...
TEST(OgHttp2AdapterTest, InvalidMaxFrameSizeSetting)
{
    ...
    const std::string frames = TestFrameSequence().ClientPreface({ { MAX_FRAME_SIZE, 3u } }).Serialize();
    testing::InSequence s;

    // Client preface
    EXPECT_CALL(visitor, OnFrameHeader(0, 6, SETTINGS, 0));
    EXPECT_CALL(visitor, OnSettingsStart());
    EXPECT_CALL(visitor, OnInvalidFrame(0, Http2VisitorInterface::InvalidFrameError::kProtocol));
    ...
    EXPECT_CALL(visitor, OnFrameSent(GOAWAY, 0, _, 0x0, static_cast<int>(Http2ErrorCode::PROTOCOL_ERROR)));

    int send_result = adapter->Send();
    EXPECT_EQ(0, send_result);
    EXPECT_THAT(visitor.data(), EqualsFrames({ SpdyFrameType::SETTINGS, SpdyFrameType::GOAWAY }));
}
...
TEST(OgHttp2AdapterTest, InvalidPushSetting)
{
    ...
    const std::string frames = TestFrameSequence().ClientPreface({ { ENABLE_PUSH, 3u } }).Serialize();
    testing::InSequence s;
...
TEST(OgHttp2AdapterTest, InvalidConnectProtocolSetting)
{
    ...
    const std::string frames = TestFrameSequence().ClientPreface({ { ENABLE_CONNECT_PROTOCOL, 3u } }).Serialize();
    testing::InSequence s;
    ...
    const std::string frames2 = TestFrameSequence().ClientPreface({ { ENABLE_CONNECT_PROTOCOL, 1 } }).Settings({ { ENABLE_CONNECT_PROTOCOL, 0 } }).Serialize();
...
TEST(OgHttp2AdapterTest, ClientSetsRemoteMaxStreamOption)
{
    ...
    const std::vector<Header> headers = ToHeaders({ { ":method", "GET" }, { ":scheme", "http" }, { ":authority", "example.com" }, { ":path", "/" } });

    const int32_t stream_id1 = adapter->SubmitRequest(headers, nullptr, true, nullptr);
    const int32_t stream_id2 = adapter->SubmitRequest(headers, nullptr, true, nullptr);
    const int32_t stream_id3 = adapter->SubmitRequest(headers, nullptr, true, nullptr);
    const int32_t stream_id4 = adapter->SubmitRequest(headers, nullptr, true, nullptr);

    EXPECT_CALL(visitor, OnBeforeFrameSent(SETTINGS, 0, _, 0x0));
    EXPECT_CALL(visitor, OnFrameSent(SETTINGS, 0, _, 0x0, 0));
    EXPECT_CALL(visitor, OnBeforeFrameSent(HEADERS, stream_id1, _, END_STREAM_FLAG | END_HEADERS_FLAG));
    EXPECT_CALL(visitor, OnFrameSent(HEADERS, stream_id1, _, END_STREAM_FLAG | END_HEADERS_FLAG, 0));
    EXPECT_CALL(visitor, OnBeforeFrameSent(HEADERS, stream_id2, _, END_STREAM_FLAG | END_HEADERS_FLAG));
    EXPECT_CALL(visitor, OnFrameSent(HEADERS, stream_id2, _, END_STREAM_FLAG | END_HEADERS_FLAG, 0));
    EXPECT_CALL(visitor, OnBeforeFrameSent(HEADERS, stream_id3, _, END_STREAM_FLAG | END_HEADERS_FLAG));
    EXPECT_CALL(visitor, OnFrameSent(HEADERS, stream_id3, _, END_STREAM_FLAG | END_HEADERS_FLAG, 0));
    ...
    const std::string stream_frames
        = TestFrameSequence()
              .ServerPreface()
              .Headers(stream_id1, { { ":status", "200" }, { "server", "my-fake-server" }, { "date", "Tue, 6 Apr 2021 12:54:01 GMT" } },
                  /*fin=*/true)
              .Serialize();
    ...
    EXPECT_CALL(visitor, OnHeaderForStream(stream_id1, "server", "my-fake-server"));
    EXPECT_CALL(visitor, OnHeaderForStream(stream_id1, "date", "Tue, 6 Apr 2021 12:54:01 GMT"));
    EXPECT_CALL(visitor, OnEndHeadersForStream(stream_id1));
    EXPECT_CALL(visitor, OnEndStream(stream_id1));
    EXPECT_CALL(visitor, OnCloseStream(stream_id1, Http2ErrorCode::HTTP2_NO_ERROR));
    ...
    EXPECT_CALL(visitor, OnBeforeFrameSent(HEADERS, stream_id4, _, END_STREAM_FLAG | END_HEADERS_FLAG));
    EXPECT_CALL(visitor, OnFrameSent(HEADERS, stream_id4, _, END_STREAM_FLAG | END_HEADERS_FLAG, 0));
    result = adapter->Send();
    EXPECT_EQ(0, result);
}
...
TEST(OgHttp2AdapterTest, ClientHandles100Headers)
{
    ...
    const std::vector<Header> headers1
        = ToHeaders({ { ":method", "GET" }, { ":scheme", "http" }, { ":authority", "example.com" }, { ":path", "/this/is/request/one" } });

    const int32_t stream_id1 = adapter->SubmitRequest(headers1, nullptr, true, nullptr);
    ...
    const std::string stream_frames = TestFrameSequence()
                                          ...
                                          .Headers(1, { { ":status", "100" } },
                                              /*fin=*/false)
                                          .Ping(101)
                                          .Headers(1, { { ":status", "200" }, { "server", "my-fake-server" }, { "date", "Tue, 6 Apr 2021 12:54:01 GMT" } },
                                              /*fin=*/true)
                                          .Serialize();
    ...
    EXPECT_CALL(visitor, OnHeaderForStream(1, "date", "Tue, 6 Apr 2021 12:54:01 GMT"));
    EXPECT_CALL(visitor, OnEndHeadersForStream(1));
    EXPECT_CALL(visitor, OnEndStream(1));
    ...
    result = adapter->Send();
    EXPECT_EQ(0, result);
    EXPECT_THAT(visitor.data(), EqualsFrames({ SpdyFrameType::SETTINGS, SpdyFrameType::PING }));
}
...
TEST(OgHttp2AdapterTest, QueuingWindowUpdateAffectsWindow)
{
    ...
    EXPECT_EQ(adapter->GetReceiveWindowSize(), kInitialFlowControlWindowSize + 10000);

    const std::vector<Header> headers
        = ToHeaders({ { ":method", "GET" }, { ":scheme", "http" }, { ":authority", "example.com" }, { ":path", "/this/is/request/one" } });
    const int32_t stream_id = adapter->SubmitRequest(headers, nullptr, true, nullptr);

    EXPECT_CALL(visitor, OnBeforeFrameSent(SETTINGS, 0, _, 0x0));
    EXPECT_CALL(visitor, OnFrameSent(SETTINGS, 0, _, 0x0, 0));
    EXPECT_CALL(visitor, OnBeforeFrameSent(WINDOW_UPDATE, 0, 4, 0x0));
    EXPECT_CALL(visitor, OnFrameSent(WINDOW_UPDATE, 0, 4, 0x0, 0));
    EXPECT_CALL(visitor, OnBeforeFrameSent(HEADERS, stream_id, _, END_STREAM_FLAG | END_HEADERS_FLAG));
    EXPECT_CALL(visitor, OnFrameSent(HEADERS, stream_id, _, END_STREAM_FLAG | END_HEADERS_FLAG, 0));

    int result = adapter->Send();
    EXPECT_EQ(0, result);

    EXPECT_EQ(adapter->GetStreamReceiveWindowSize(stream_id), kInitialFlowControlWindowSize);
    adapter->SubmitWindowUpdate(1, 20000);
    EXPECT_EQ(adapter->GetStreamReceiveWindowSize(stream_id), kInitialFlowControlWindowSize + 20000);
}
...
TEST(OgHttp2AdapterTest, AckOfSettingInitialWindowSizeAffectsWindow)
{
    ...
    EXPECT_EQ(adapter->GetStreamReceiveWindowSize(stream_id1), kInitialFlowControlWindowSize);
    ...
    const std::string settings_ack = TestFrameSequence().SettingsAck().Serialize();
    ...
    const std::vector<Header> headers2
        = ToHeaders({ { ":method", "GET" }, { ":scheme", "http" }, { ":authority", "example.com" }, { ":path", "/this/is/request/two" } });
    const int32_t stream_id2 = adapter->SubmitRequest(headers, nullptr, true, nullptr);
...
TEST(OgHttp2AdapterTest, ClientRejects100HeadersWithFin)
{
    ...
    EXPECT_CALL(visitor, OnBeginHeadersForStream(1));
    EXPECT_CALL(visitor, OnHeaderForStream(1, ":status", "100"));
    EXPECT_CALL(visitor, OnInvalidFrame(1, Http2VisitorInterface::InvalidFrameError::kHttpMessaging));
    ...
    result = adapter->Send();
    EXPECT_EQ(0, result);
    EXPECT_THAT(visitor.data(), EqualsFrames({ SpdyFrameType::SETTINGS, SpdyFrameType::RST_STREAM }));
}
...
TEST(OgHttp2AdapterTest, ClientHandlesFinFollowing100Headers)
{
    ...
    const std::string stream_frames
        = TestFrameSequence().ServerPreface().Headers(stream_id1, { { ":status", "100" } }, /*fin=*/false).Data(stream_id1, "", /*fin=*/true).Serialize();
...
TEST(OgHttp2AdapterTest, ClientRejects100HeadersWithContent)
{
    ...
    EXPECT_CALL(visitor, OnFrameSent(SETTINGS, 0, _, ACK_FLAG, 0));
    EXPECT_CALL(visitor, OnBeforeFrameSent(RST_STREAM, 1, _, 0x0));
    EXPECT_CALL(visitor, OnFrameSent(RST_STREAM, 1, _, 0x0, static_cast<int>(Http2ErrorCode::PROTOCOL_ERROR)));
    EXPECT_CALL(visitor, OnCloseStream(1, Http2ErrorCode::HTTP2_NO_ERROR));
...
TEST(OgHttp2AdapterTest, ClientHandlesResponseWithContentLengthAndPadding)
{
    ...
    const int32_t stream_id2 = adapter->SubmitRequest(headers2, nullptr, true, nullptr);
...
class ResponseCompleteBeforeRequestTest : public quiche::test::QuicheTestWithParam<std::tuple<bool, bool>> {
public:
    bool HasTrailers() const
    {
        return std::get<0>(GetParam());
    }
    bool HasRstStream() const
    {
        return std::get<1>(GetParam());
    }
};

INSTANTIATE_TEST_SUITE_P(TrailersAndRstStreamAllCombinations, ResponseCompleteBeforeRequestTest, testing::Combine(testing::Bool(), testing::Bool()));

TEST_P(ResponseCompleteBeforeRequestTest, ClientHandlesResponseBeforeRequestComplete)
{
    TestVisitor visitor;
    OgHttp2Adapter::Options options;
    options.perspective = Perspective::kClient;
    ...
    const std::vector<Header> headers1
        = ToHeaders({ { ":method", "POST" }, { ":scheme", "http" }, { ":authority", "example.com" }, { ":path", "/this/is/request/one" } });

    auto body1 = std::make_unique<VisitorDataSource>(visitor, 1);
    const int32_t stream_id1 = adapter->SubmitRequest(headers1, std::move(body1), false, nullptr);
    ASSERT_GT(stream_id1, 0);

    EXPECT_CALL(visitor, OnBeforeFrameSent(SETTINGS, 0, _, 0x0));
    EXPECT_CALL(visitor, OnFrameSent(SETTINGS, 0, _, 0x0, 0));
    EXPECT_CALL(visitor, OnBeforeFrameSent(HEADERS, stream_id1, _, END_HEADERS_FLAG));
    EXPECT_CALL(visitor, OnFrameSent(HEADERS, stream_id1, _, END_HEADERS_FLAG, 0));
    ...
    EXPECT_CALL(visitor, OnEndHeadersForStream(1));
    // DATA frame with padding for stream 1
    EXPECT_CALL(visitor, OnFrameHeader(1, 2 + 10, DATA, HasTrailers() ? 0x8 : 0x9));
    EXPECT_CALL(visitor, OnBeginDataForStream(1, 2 + 10));
    EXPECT_CALL(visitor, OnDataPaddingLength(1, 10));
    EXPECT_CALL(visitor, OnDataForStream(1, "hi"));
    if (HasTrailers()) {
        ...
        EXPECT_CALL(visitor, OnFrameHeader(1, _, HEADERS, 5));
        EXPECT_CALL(visitor, OnBeginHeadersForStream(1));
        EXPECT_CALL(visitor, OnHeaderForStream(1, "my-weird-trailer", "has a value"));
        EXPECT_CALL(visitor, OnEndHeadersForStream(1));
    }
    // END_STREAM for stream 1
    EXPECT_CALL(visitor, OnEndStream(1));
...
TEST(OgHttp2AdapterTest, ClientHandles204WithContent)
{
    ...
    EXPECT_CALL(visitor, OnFrameSent(RST_STREAM, 3, _, 0x0, static_cast<int>(Http2ErrorCode::PROTOCOL_ERROR)));
    EXPECT_CALL(visitor, OnCloseStream(3, Http2ErrorCode::HTTP2_NO_ERROR));

    EXPECT_TRUE(adapter->want_write());
    result = adapter->Send();
    EXPECT_EQ(0, result);
    EXPECT_THAT(visitor.data(), EqualsFrames({ SpdyFrameType::SETTINGS, SpdyFrameType::RST_STREAM, SpdyFrameType::RST_STREAM }));
}
...
TEST(OgHttp2AdapterTest, ClientHandlesTrailers)
{
    ...
    EXPECT_THAT(data, testing::StartsWith(spdy::kHttp2ConnectionHeaderPrefix));
    data.remove_prefix(strlen(spdy::kHttp2ConnectionHeaderPrefix));
    EXPECT_THAT(data, EqualsFrames({ SpdyFrameType::SETTINGS, SpdyFrameType::HEADERS }));
    visitor.Clear();
...
TEST_P(OgHttp2AdapterDataTest, ClientSendsTrailers)
{
    ...
    const int32_t stream_id1 = adapter->SubmitRequest(headers1, GetParam() ? nullptr : std::move(body1), false, nullptr);
    ...
    EXPECT_THAT(data, testing::StartsWith(spdy::kHttp2ConnectionHeaderPrefix));
    data.remove_prefix(strlen(spdy::kHttp2ConnectionHeaderPrefix));
    EXPECT_THAT(data, EqualsFrames({ SpdyFrameType::SETTINGS, SpdyFrameType::HEADERS, SpdyFrameType::DATA }));
    visitor.Clear();

    const std::vector<Header> trailers1 = ToHeaders({ { "extra-info", "Trailers are weird but good?" } });
...
TEST(OgHttp2AdapterTest, ClientRstStreamWhileHandlingHeaders)
{
    ...
    EXPECT_CALL(visitor, OnHeaderForStream(1, "server", "my-fake-server"));
    EXPECT_CALL(visitor, OnHeaderForStream(1, "date", "Tue, 6 Apr 2021 12:54:01 GMT"))
        .WillOnce(testing::DoAll(testing::InvokeWithoutArgs([&adapter]() { adapter->SubmitRst(1, Http2ErrorCode::REFUSED_STREAM); }),
    ...
    EXPECT_CALL(visitor, OnFrameSent(RST_STREAM, stream_id1, 4, 0x0, static_cast<int>(Http2ErrorCode::REFUSED_STREAM)));
    EXPECT_CALL(visitor, OnCloseStream(1, Http2ErrorCode::HTTP2_NO_ERROR));
...
TEST(OgHttp2AdapterTest, ClientConnectionErrorWhileHandlingHeaders)
{
    ...
    EXPECT_CALL(visitor, OnHeaderForStream(1, "server", "my-fake-server"));
    EXPECT_CALL(visitor, OnHeaderForStream(1, "date", "Tue, 6 Apr 2021 12:54:01 GMT"))
        .WillOnce(testing::Return(Http2VisitorInterface::HEADER_CONNECTION_ERROR));
    ...
    EXPECT_CALL(visitor, OnBeforeFrameSent(GOAWAY, 0, _, 0x0));
    EXPECT_CALL(visitor, OnFrameSent(GOAWAY, 0, _, 0x0, static_cast<int>(Http2ErrorCode::INTERNAL_ERROR)));
...
TEST(OgHttp2AdapterTest, ClientRejectsHeaders)
{
    ...
    EXPECT_CALL(visitor, OnFrameHeader(1, _, HEADERS, 4));
    EXPECT_CALL(visitor, OnBeginHeadersForStream(1)).WillOnce(testing::Return(false));
...
TEST(OgHttp2AdapterTest, ClientHandlesSmallerHpackHeaderTableSetting)
{
    ...
    const std::string stream_frames = TestFrameSequence().Settings({ { HEADER_TABLE_SIZE, 100u } }).Serialize();
    // Server preface (SETTINGS)
    EXPECT_CALL(visitor, OnFrameHeader(0, 6, SETTINGS, 0));
    EXPECT_CALL(visitor, OnSettingsStart());
...
TEST(OgHttp2AdapterTest, ClientHandlesLargerHpackHeaderTableSetting)
{
    ...
    const std::string stream_frames = TestFrameSequence().Settings({ { HEADER_TABLE_SIZE, 40960u } }).Serialize();
    // Server preface (SETTINGS)
    EXPECT_CALL(visitor, OnFrameHeader(0, 6, SETTINGS, 0));
    EXPECT_CALL(visitor, OnSettingsStart());
...
TEST(OgHttp2AdapterTest, ClientSendsHpackHeaderTableSetting)
{
    ...
    const std::string stream_frames
        = TestFrameSequence()
              ...
              .SettingsAck()
              .Headers(1,
                  { { ":status", "200" }, { "server", "my-fake-server" }, { "date", "Tue, 6 Apr 2021 12:54:01 GMT" },
                      { "x-i-do-not-like", "green eggs and ham" }, { "x-i-will-not-eat-them", "here or there, in a box, with a fox" },
                      { "x-like-them-in-a-house", "no" }, { "x-like-them-with-a-mouse", "no" } },
                  /*fin=*/true)
              .Serialize();
    // Server preface (empty SETTINGS)
    EXPECT_CALL(visitor, OnFrameHeader(0, 0, SETTINGS, 0));
    ...
    const std::string response_frames
        = TestFrameSequence()
              .Headers(stream_id2, { { ":status", "200" }, { "server", "my-fake-server" }, { "date", "Tue, 6 Apr 2021 12:54:01 GMT" } },
                  /*fin=*/true)
              .Serialize();
    ...
    EXPECT_CALL(visitor, OnCloseStream(stream_id2, Http2ErrorCode::HTTP2_NO_ERROR));
...
TEST(OgHttp2AdapterTest, ClientStartsShutdown)
{
    ...
    EXPECT_THAT(serialized, testing::StartsWith(spdy::kHttp2ConnectionHeaderPrefix));
...
TEST(OgHttp2AdapterTest, ClientReceivesGoAway)
{
    ...
    EXPECT_THAT(data, testing::StartsWith(spdy::kHttp2ConnectionHeaderPrefix));
    data.remove_prefix(strlen(spdy::kHttp2ConnectionHeaderPrefix));
    EXPECT_THAT(data, EqualsFrames({ SpdyFrameType::SETTINGS, SpdyFrameType::HEADERS, SpdyFrameType::HEADERS }));
    visitor.Clear();
...
TEST(OgHttp2AdapterTest, ClientReceivesMultipleGoAways)
{
    ...
    const std::string initial_frames = TestFrameSequence().ServerPreface().GoAway(kMaxStreamId, Http2ErrorCode::INTERNAL_ERROR, "indigestion").Serialize();
    ...
    result = adapter->Send();
    EXPECT_EQ(0, result);
    EXPECT_THAT(visitor.data(), EqualsFrames({ SpdyFrameType::SETTINGS, SpdyFrameType::WINDOW_UPDATE }));
    visitor.Clear();

    const std::string final_frames = TestFrameSequence().GoAway(0, Http2ErrorCode::INTERNAL_ERROR, "indigestion").Serialize();
...
TEST(OgHttp2AdapterTest, ClientReceivesMultipleGoAwaysWithIncreasingStreamId)
{
    ...
    EXPECT_CALL(visitor, OnConnectionError(ConnectionError::kInvalidGoAwayLastStreamId));
...
TEST(OgHttp2AdapterTest, ClientReceivesGoAwayWithPendingStreams)
{
    ...
    const std::string initial_frames = TestFrameSequence().ServerPreface({ { MAX_CONCURRENT_STREAMS, 1 } }).Serialize();
    ...
    result = adapter->Send();
    EXPECT_EQ(0, result);
    EXPECT_THAT(visitor.data(), EqualsFrames({ SpdyFrameType::SETTINGS, SpdyFrameType::HEADERS }));
    visitor.Clear();
    ...
    const std::string stream_frames
        = TestFrameSequence().GoAway(kMaxStreamId, Http2ErrorCode::INTERNAL_ERROR, "indigestion").Settings({ { MAX_CONCURRENT_STREAMS, 42u } }).Serialize();
    ...
    const std::vector<Header> headers3
        = ToHeaders({ { ":method", "GET" }, { ":scheme", "http" }, { ":authority", "example.com" }, { ":path", "/this/is/request/three" } });

    const int32_t stream_id3 = adapter->SubmitRequest(headers3, nullptr, true, nullptr);
...
TEST(OgHttp2AdapterTest, ClientFailsOnGoAway)
{
    ...
    EXPECT_CALL(visitor, OnGoAway(1, Http2ErrorCode::INTERNAL_ERROR, "")).WillOnce(testing::Return(false));
...
TEST(OgHttp2AdapterTest, ClientRejects101Response)
{
    ...
    const std::vector<Header> headers1 = ToHeaders(
        { { ":method", "GET" }, { ":scheme", "http" }, { ":authority", "example.com" }, { ":path", "/this/is/request/one" }, { "upgrade", "new-protocol" } });
    ...
    const std::string stream_frames = TestFrameSequence()
                                          .ServerPreface()
                                          .Headers(1, { { ":status", "101" }, { "server", "my-fake-server" }, { "date", "Tue, 6 Apr 2021 12:54:01 GMT" } },
                                              /*fin=*/false)
                                          .Serialize();
    ...
    EXPECT_CALL(visitor, OnFrameSent(RST_STREAM, 1, 4, 0x0, static_cast<uint32_t>(Http2ErrorCode::PROTOCOL_ERROR)));
    EXPECT_CALL(visitor, OnCloseStream(1, Http2ErrorCode::HTTP2_NO_ERROR));
...
TEST_P(OgHttp2AdapterDataTest, ClientObeysMaxConcurrentStreams)
{
    ...
    EXPECT_CALL(visitor, OnBeforeFrameSent(HEADERS, stream_id, _, END_HEADERS_FLAG));
    ...
    const int next_stream_id = adapter->SubmitRequest(
        ToHeaders({ { ":method", "POST" }, { ":scheme", "http" }, { ":authority", "example.com" }, { ":path", "/this/is/request/two" } }), nullptr, true,
        nullptr);
    ...
    const std::string stream_frames
        = TestFrameSequence()
              .Headers(stream_id, { { ":status", "200" }, { "server", "my-fake-server" }, { "date", "Tue, 6 Apr 2021 12:54:01 GMT" } },
                  /*fin=*/false)
    ...
    EXPECT_CALL(visitor, OnHeaderForStream(stream_id, "server", "my-fake-server"));
    EXPECT_CALL(visitor, OnHeaderForStream(stream_id, "date", "Tue, 6 Apr 2021 12:54:01 GMT"));
    ...
    EXPECT_CALL(visitor, OnDataForStream(stream_id, "This is the response body."));
    ...
    EXPECT_CALL(visitor, OnBeforeFrameSent(HEADERS, next_stream_id, _, END_STREAM_FLAG | END_HEADERS_FLAG));
    EXPECT_CALL(visitor, OnFrameSent(HEADERS, next_stream_id, _, END_STREAM_FLAG | END_HEADERS_FLAG, 0));
...
TEST_P(OgHttp2AdapterDataTest, ClientReceivesInitialWindowSetting)
{
    ...
    const std::string initial_frames = TestFrameSequence().Settings({ { INITIAL_WINDOW_SIZE, 80000u } }).WindowUpdate(0, 65536).Serialize();
    ...
    EXPECT_THAT(serialized, testing::StartsWith(spdy::kHttp2ConnectionHeaderPrefix));
    serialized.remove_prefix(strlen(spdy::kHttp2ConnectionHeaderPrefix));
    EXPECT_THAT(serialized, EqualsFrames({ SpdyFrameType::SETTINGS, SpdyFrameType::SETTINGS }));
    visitor.Clear();
    ...
    EXPECT_EQ(0, result);
    EXPECT_THAT(visitor.data(),
        EqualsFrames({ SpdyFrameType::HEADERS, SpdyFrameType::DATA, SpdyFrameType::DATA, SpdyFrameType::DATA, SpdyFrameType::DATA, SpdyFrameType::DATA }));
}

TEST_P(OgHttp2AdapterDataTest, ClientReceivesInitialWindowSettingAfterStreamStart)
{
    TestVisitor visitor;
    OgHttp2Adapter::Options options;
    options.perspective = Perspective::kClient;
    ...
    const std::string initial_frames = TestFrameSequence().ServerPreface().WindowUpdate(0, 65536).Serialize();
    // Server preface (empty SETTINGS)
    EXPECT_CALL(visitor, OnFrameHeader(0, 0, SETTINGS, 0));
    EXPECT_CALL(visitor, OnSettingsStart());
    EXPECT_CALL(visitor, OnSettingsEnd());
    ...
    result = adapter->Send();
    EXPECT_EQ(0, result);
    EXPECT_THAT(visitor.data(), EqualsFrames({ SpdyFrameType::HEADERS, SpdyFrameType::DATA, SpdyFrameType::DATA, SpdyFrameType::DATA, SpdyFrameType::DATA }));
    visitor.Clear();

    // Can't write any more due to flow control.
    EXPECT_FALSE(adapter->want_write());

    const std::string settings_frame = TestFrameSequence().Settings({ { INITIAL_WINDOW_SIZE, 80000u } }).Serialize();
    ...
    result = adapter->Send();
    EXPECT_EQ(0, result);
    EXPECT_THAT(visitor.data(), EqualsFrames({ SpdyFrameType::SETTINGS, SpdyFrameType::DATA }));
}
...
TEST(OgHttp2AdapterTest, InvalidInitialWindowSetting)
{
    ...
    const std::string initial_frames = TestFrameSequence().Settings({ { INITIAL_WINDOW_SIZE, kTooLargeInitialWindow } }).Serialize();
    // Server preface (SETTINGS with INITIAL_STREAM_WINDOW)
    EXPECT_CALL(visitor, OnFrameHeader(0, 6, SETTINGS, 0));
    EXPECT_CALL(visitor, OnSettingsStart());
    EXPECT_CALL(visitor, OnInvalidFrame(0, Http2VisitorInterface::InvalidFrameError::kFlowControl));
    ...
    EXPECT_CALL(visitor, OnFrameSent(SETTINGS, 0, _, 0x0, 0));
    EXPECT_CALL(visitor, OnBeforeFrameSent(GOAWAY, 0, _, 0x0));
    EXPECT_CALL(visitor, OnFrameSent(GOAWAY, 0, _, 0x0, static_cast<int>(Http2ErrorCode::FLOW_CONTROL_ERROR)));
    ...
    EXPECT_THAT(serialized, testing::StartsWith(spdy::kHttp2ConnectionHeaderPrefix));
    serialized.remove_prefix(strlen(spdy::kHttp2ConnectionHeaderPrefix));
    EXPECT_THAT(serialized, EqualsFrames({ SpdyFrameType::SETTINGS, SpdyFrameType::GOAWAY }));
    visitor.Clear();
}
...
TEST(OggHttp2AdapterClientTest, InitialWindowSettingCausesOverflow)
{
    ...
    EXPECT_CALL(visitor, OnFrameHeader(0, 6, SETTINGS, 0));
    EXPECT_CALL(visitor, OnSettingsStart());
    EXPECT_CALL(visitor, OnSetting(Http2Setting { INITIAL_WINDOW_SIZE, kLargeInitialWindow }));
    EXPECT_CALL(visitor, OnSettingsEnd());
    ...
    EXPECT_CALL(visitor, OnFrameSent(RST_STREAM, stream_id, 4, 0x0, static_cast<int>(Http2ErrorCode::FLOW_CONTROL_ERROR)));
    EXPECT_CALL(visitor, OnCloseStream(stream_id, Http2ErrorCode::HTTP2_NO_ERROR));

    int result = adapter->Send();
    EXPECT_EQ(0, result);
    EXPECT_THAT(visitor.data(), EqualsFrames({ SpdyFrameType::SETTINGS, SpdyFrameType::SETTINGS, SpdyFrameType::RST_STREAM }));
}
...
TEST(OgHttp2AdapterTest, MaxFrameSizeSettingNotAppliedBeforeAck)
{
    ...
    const int32_t stream_id = adapter->SubmitRequest(
        ToHeaders({ { ":method", "GET" }, { ":scheme", "https" }, { ":authority", "example.com" }, { ":path", "/this/is/request/one" } }),
    ...
    const std::string server_frames
        = TestFrameSequence().ServerPreface().Headers(1, { { ":status", "200" } }, /*fin=*/false).Data(1, std::string(large_frame_size, 'a')).Serialize();
    ...
    EXPECT_CALL(visitor, OnBeforeFrameSent(GOAWAY, 0, _, 0x0));
    EXPECT_CALL(visitor, OnFrameSent(GOAWAY, 0, _, 0x0, static_cast<int>(Http2ErrorCode::FRAME_SIZE_ERROR)));
...
TEST(OgHttp2AdapterTest, MaxFrameSizeSettingAppliedAfterAck)
{
    ...
    const std::string server_frames = TestFrameSequence()
                                          .ServerPreface()
                                          .SettingsAck()
...
TEST(OgHttp2AdapterTest, ClientForbidsPushPromise)
{
    ...
    const std::vector<Header> push_headers
        = ToHeaders({ { ":method", "GET" }, { ":scheme", "http" }, { ":authority", "example.com" }, { ":path", "/this/is/request/push" } });
    const std::string frames = TestFrameSequence().ServerPreface().SettingsAck().PushPromise(stream_id, 2, push_headers).Serialize();
...
TEST(OgHttp2AdapterTest, ClientForbidsPushStream)
{
    ...
    const std::string frames = TestFrameSequence()
                                   .ServerPreface()
                                   .SettingsAck()
                                   .Headers(2, { { ":status", "200" }, { "server", "my-fake-server" }, { "date", "Tue, 6 Apr 2021 12:54:01 GMT" } },
                                       /*fin=*/true)
                                   .Serialize();
...
TEST(OgHttp2AdapterTest, ClientSubmitRequestWithDataProviderAndWriteBlock)
{
    ...
    std::unique_ptr<DataFrameSource> frame_source = std::make_unique<VisitorDataSource>(visitor, 1);
    visitor.AppendPayloadForStream(1, kBody);
    visitor.SetEndData(1, true);
...
TEST(OgHttp2AdapterTest, ClientReceivesDataOnClosedStream)
{
    ...
    const std::string initial_frames = TestFrameSequence().ServerPreface().Serialize();
    testing::InSequence s;
    ...
    int stream_id = adapter->SubmitRequest(
        ToHeaders({ { ":method", "GET" }, { ":scheme", "http" }, { ":authority", "example.com" }, { ":path", "/this/is/request/one" } }), nullptr, true,
        nullptr);
    EXPECT_GT(stream_id, 0);
    ...
    EXPECT_CALL(visitor, OnFrameSent(RST_STREAM, stream_id, _, 0x0, static_cast<int>(Http2ErrorCode::CANCEL)));
    EXPECT_CALL(visitor, OnCloseStream(stream_id, Http2ErrorCode::HTTP2_NO_ERROR));
...
TEST_P(OgHttp2AdapterDataTest, ClientEncountersFlowControlBlock)
{
    ...
    const int32_t stream_id2 = adapter->SubmitRequest(headers2, GetParam() ? nullptr : std::move(body2), false, nullptr);
    ...
    const std::string stream_frames = TestFrameSequence().ServerPreface().WindowUpdate(0, 80000).WindowUpdate(stream_id1, 20000).Serialize();
    ...
    EXPECT_CALL(visitor, OnFrameSent(DATA, stream_id2, _, 0x0, 0)).Times(testing::AtLeast(1));
    EXPECT_CALL(visitor, OnFrameSent(DATA, stream_id1, _, 0x0, 0)).Times(testing::AtLeast(1));
...
TEST(OgHttp2AdapterTest, ClientQueuesRequests)
{
    ...
    const std::string initial_frames = TestFrameSequence().ServerPreface({ { MAX_CONCURRENT_STREAMS, 2 } }).SettingsAck().Serialize();

    EXPECT_CALL(visitor, OnFrameHeader(0, 6, SETTINGS, 0x0));
    EXPECT_CALL(visitor, OnSettingsStart());
    EXPECT_CALL(visitor, OnSetting(Http2Setting { Http2KnownSettingsId::MAX_CONCURRENT_STREAMS, 2u }));
    EXPECT_CALL(visitor, OnSettingsEnd());
    EXPECT_CALL(visitor, OnFrameHeader(0, 0, SETTINGS, ACK_FLAG));
    EXPECT_CALL(visitor, OnSettingsAck());
    ...
    const std::vector<Header> headers
        = ToHeaders({ { ":method", "GET" }, { ":scheme", "http" }, { ":authority", "example.com" }, { ":path", "/example/request" } });
    ...
    EXPECT_CALL(visitor, OnBeforeFrameSent(SETTINGS, 0, _, ACK_FLAG));
    EXPECT_CALL(visitor, OnFrameSent(SETTINGS, 0, _, ACK_FLAG, 0));
    EXPECT_CALL(visitor, OnBeforeFrameSent(HEADERS, stream_ids[0], _, END_STREAM_FLAG | END_HEADERS_FLAG));
    EXPECT_CALL(visitor, OnFrameSent(HEADERS, stream_ids[0], _, END_STREAM_FLAG | END_HEADERS_FLAG, 0));
    EXPECT_CALL(visitor, OnBeforeFrameSent(HEADERS, stream_ids[1], _, END_STREAM_FLAG | END_HEADERS_FLAG));
    EXPECT_CALL(visitor, OnFrameSent(HEADERS, stream_ids[1], _, END_STREAM_FLAG | END_HEADERS_FLAG, 0));

    adapter->Send();

    const std::string update_streams = TestFrameSequence().Settings({ { MAX_CONCURRENT_STREAMS, 5 } }).Serialize();

    EXPECT_CALL(visitor, OnFrameHeader(0, 6, SETTINGS, 0x0));
    EXPECT_CALL(visitor, OnSettingsStart());
    EXPECT_CALL(visitor, OnSetting(Http2Setting { Http2KnownSettingsId::MAX_CONCURRENT_STREAMS, 5u }));
    EXPECT_CALL(visitor, OnSettingsEnd());
    ...
    EXPECT_CALL(visitor, OnBeforeFrameSent(SETTINGS, 0, _, ACK_FLAG));
    EXPECT_CALL(visitor, OnFrameSent(SETTINGS, 0, _, ACK_FLAG, 0));
    EXPECT_CALL(visitor, OnBeforeFrameSent(HEADERS, stream_ids[2], _, END_STREAM_FLAG | END_HEADERS_FLAG));
    EXPECT_CALL(visitor, OnFrameSent(HEADERS, stream_ids[2], _, END_STREAM_FLAG | END_HEADERS_FLAG, 0));
    EXPECT_CALL(visitor, OnBeforeFrameSent(HEADERS, stream_ids[3], _, END_STREAM_FLAG | END_HEADERS_FLAG));
    EXPECT_CALL(visitor, OnFrameSent(HEADERS, stream_ids[3], _, END_STREAM_FLAG | END_HEADERS_FLAG, 0));
    EXPECT_CALL(visitor, OnBeforeFrameSent(HEADERS, stream_ids[4], _, END_STREAM_FLAG | END_HEADERS_FLAG));
    EXPECT_CALL(visitor, OnFrameSent(HEADERS, stream_ids[4], _, END_STREAM_FLAG | END_HEADERS_FLAG, 0));
...
TEST(OgHttp2AdapterTest, ClientAcceptsHeadResponseWithContentLength)
{
    ...
    const std::vector<Header> headers = ToHeaders({ { ":method", "HEAD" }, { ":scheme", "http" }, { ":authority", "example.com" }, { ":path", "/" } });
    const int32_t stream_id = adapter->SubmitRequest(headers, nullptr, true, nullptr);
...
TEST(OgHttp2AdapterTest, WindowUpdateZeroDelta)
{
    ...
    const std::string request = TestFrameSequence()
                                    .ClientPreface()
                                    .Headers(1, { { ":method", "GET" }, { ":scheme", "https" }, { ":authority", "example.com" }, { ":path", "/" } },
                                        /*fin=*/false)
                                    .WindowUpdate(1, 0)
    ...
    const std::string window_update = TestFrameSequence().WindowUpdate(0, 0).Serialize();
    EXPECT_CALL(visitor, OnFrameHeader(0, 4, WINDOW_UPDATE, 0));
    EXPECT_CALL(visitor, OnConnectionError(ConnectionError::kFlowControlError));
...
TEST(OgHttp2AdapterTest, WindowUpdateCausesWindowOverflow)
{
    ...
    EXPECT_CALL(visitor, OnBeforeFrameSent(RST_STREAM, 1, _, 0x0));
    EXPECT_CALL(visitor, OnFrameSent(RST_STREAM, 1, _, 0x0, static_cast<int>(Http2ErrorCode::FLOW_CONTROL_ERROR)));
    EXPECT_CALL(visitor, OnCloseStream(1, _));

    adapter->Send();

    const std::string window_update = TestFrameSequence().WindowUpdate(0, std::numeric_limits<int>::max()).Serialize();
...
TEST(OgHttp2AdapterTest, WindowUpdateRaisesFlowControlWindowLimit)
{
    ...
    EXPECT_EQ(kInitialFlowControlWindowSize + 2 * kDefaultFramePayloadSizeLimit, adapter->GetReceiveWindowSize());
    EXPECT_EQ(kInitialFlowControlWindowSize + 2 * kDefaultFramePayloadSizeLimit, adapter->GetStreamReceiveWindowSize(1));

    const std::string request_body
        = TestFrameSequence().Data(1, data_chunk).Data(1, data_chunk).Data(1, data_chunk).Data(1, data_chunk).Data(1, data_chunk).Serialize();
    ...
    EXPECT_EQ(kInitialFlowControlWindowSize - 3 * kDefaultFramePayloadSizeLimit, adapter->GetReceiveWindowSize());
    EXPECT_EQ(kInitialFlowControlWindowSize - 3 * kDefaultFramePayloadSizeLimit, adapter->GetStreamReceiveWindowSize(1));
    ...
    EXPECT_GT(adapter->GetStreamReceiveWindowSize(1), kInitialFlowControlWindowSize);
}
...
TEST(OgHttp2AdapterTest, MarkDataConsumedForNonexistentStream)
{
    ...
    const std::string frames
        = TestFrameSequence()
              .ClientPreface()
              .Headers(1, { { ":method", "GET" }, { ":scheme", "https" }, { ":authority", "example.com" }, { ":path", "/this/is/request/one" } },
                  /*fin=*/false)
...
TEST(OgHttp2AdapterTest, TestSerialize)
{
    ...
    adapter->SubmitSettings({ { HEADER_TABLE_SIZE, 128 }, { MAX_FRAME_SIZE, 128 << 10 } });
    EXPECT_TRUE(adapter->want_write());
    ...
    EXPECT_CALL(visitor, OnBeforeFrameSent(WINDOW_UPDATE, accepted_stream, _, 0x0));
    EXPECT_CALL(visitor, OnFrameSent(WINDOW_UPDATE, accepted_stream, _, 0x0, 0));

    int result = adapter->Send();
    EXPECT_EQ(0, result);
    EXPECT_THAT(visitor.data(),
        EqualsFrames({ SpdyFrameType::SETTINGS, SpdyFrameType::PRIORITY, SpdyFrameType::RST_STREAM, SpdyFrameType::PING, SpdyFrameType::GOAWAY,
...
TEST(OgHttp2AdapterTest, TestPartialSerialize)
{
    ...
    adapter->SubmitSettings({ { HEADER_TABLE_SIZE, 128 }, { MAX_FRAME_SIZE, 128 << 10 } });
    adapter->SubmitGoAway(13, Http2ErrorCode::HTTP2_NO_ERROR, "And don't come back!");
    adapter->SubmitPing(42);
    EXPECT_TRUE(adapter->want_write());
    ...
    EXPECT_EQ(0, result);
    EXPECT_FALSE(adapter->want_write());
    EXPECT_THAT(visitor.data(), EqualsFrames({ SpdyFrameType::SETTINGS, SpdyFrameType::GOAWAY, SpdyFrameType::PING }));
}
...
TEST(OgHttp2AdapterTest, ConnectionErrorOnControlFrameSent)
{
    ...
    // SETTINGS ack
    EXPECT_CALL(visitor, OnBeforeFrameSent(SETTINGS, 0, 0, ACK_FLAG));
    EXPECT_CALL(visitor, OnFrameSent(SETTINGS, 0, 0, ACK_FLAG, 0)).WillOnce(testing::Return(-902));
    EXPECT_CALL(visitor, OnConnectionError(ConnectionError::kSendError));
...
TEST_P(OgHttp2AdapterDataTest, ConnectionErrorOnDataFrameSent)
{
    ...
    EXPECT_CALL(visitor, OnSettingsEnd());
    // Stream 1
    EXPECT_CALL(visitor, OnFrameHeader(1, _, HEADERS, END_STREAM_FLAG | END_HEADERS_FLAG));
    EXPECT_CALL(visitor, OnBeginHeadersForStream(1));
    EXPECT_CALL(visitor, OnHeaderForStream(1, _, _)).Times(4);
    EXPECT_CALL(visitor, OnEndHeadersForStream(1));
    EXPECT_CALL(visitor, OnEndStream(1));
    ...
    visitor.AppendPayloadForStream(1, "Here is some data, which will lead to a fatal error");
    auto body = std::make_unique<VisitorDataSource>(visitor, 1);
    int submit_result = adapter->SubmitResponse(1, ToHeaders({ { ":status", "200" } }), GetParam() ? nullptr : std::move(body), false);
    ...
    EXPECT_CALL(visitor, OnFrameSent(DATA, 1, _, 0x0, 0)).WillOnce(testing::Return(-902));
    EXPECT_CALL(visitor, OnConnectionError(ConnectionError::kSendError));

    int send_result = adapter->Send();
    EXPECT_LT(send_result, 0);

    visitor.AppendPayloadForStream(1, "After the fatal error, data will be sent no more");
...
TEST_P(OgHttp2AdapterDataTest, RepeatedHeaderNames)
{
    ...
    const std::string frames = TestFrameSequence()
                                   .ClientPreface()
                                   .Headers(1,
                                       { { ":method", "GET" }, { ":scheme", "https" }, { ":authority", "example.com" }, { ":path", "/this/is/request/one" },
    ...
    const std::vector<Header> headers1 = ToHeaders({ { ":status", "200" }, { "content-length", "10" }, { "content-length", "10" } });
    visitor.AppendPayloadForStream(1, "perfection");
    visitor.SetEndData(1, true);
    auto body1 = std::make_unique<VisitorDataSource>(visitor, 1);

    int submit_result = adapter->SubmitResponse(1, headers1, GetParam() ? nullptr : std::move(body1), false);
    ASSERT_EQ(0, submit_result);
...
TEST_P(OgHttp2AdapterDataTest, ServerRespondsToRequestWithTrailers)
{
    ...
    int send_result = adapter->Send();
    EXPECT_EQ(0, send_result);
    EXPECT_THAT(visitor.data(), EqualsFrames({ SpdyFrameType::SETTINGS, SpdyFrameType::SETTINGS, SpdyFrameType::HEADERS }));
    visitor.Clear();
    ...
    EXPECT_CALL(visitor, OnFrameHeader(1, _, HEADERS, 5));
    EXPECT_CALL(visitor, OnBeginHeadersForStream(1));
    EXPECT_CALL(visitor, OnHeaderForStream(1, "extra-info", "Trailers are weird but good?"));
    EXPECT_CALL(visitor, OnEndHeadersForStream(1));
    EXPECT_CALL(visitor, OnEndStream(1));
...
TEST(OgHttp2AdapterTest, ServerReceivesMoreHeaderBytesThanConfigured)
{
    ...
    EXPECT_CALL(visitor, OnFrameSent(SETTINGS, 0, _, 0x0, 0));
    EXPECT_CALL(visitor, OnBeforeFrameSent(GOAWAY, 0, _, 0x0));
    EXPECT_CALL(visitor, OnFrameSent(GOAWAY, 0, _, 0x0, static_cast<int>(Http2ErrorCode::COMPRESSION_ERROR)));
...
TEST(OgHttp2AdapterTest, ServerVisitorRejectsHeaders)
{
    ...
    const std::string frames = TestFrameSequence()
                                   ...
                                   .Headers(1,
                                       { { ":method", "GET" }, { ":scheme", "https" }, { ":authority", "example.com" }, { ":path", "/this/is/request/one" },
                                           { "header1", "ok" }, { "header2", "rejected" }, { "header3", "not processed" }, // CONTINUATION starts here
                                           { "header4", "not processed" }, { "header5", "not processed" }, { "header6", "not processed" },
                                           { "header7", "not processed" }, { "header8", "not processed" } },
    ...
    EXPECT_CALL(visitor, OnHeaderForStream(1, "header2", _)).WillOnce(testing::Return(Http2VisitorInterface::HEADER_RST_STREAM));
    ...
    EXPECT_CALL(visitor, OnFrameSent(SETTINGS, 0, _, 0x1, 0));
    EXPECT_CALL(visitor, OnBeforeFrameSent(RST_STREAM, 1, _, 0x0));
    EXPECT_CALL(visitor, OnFrameSent(RST_STREAM, 1, _, 0x0, static_cast<int>(Http2ErrorCode::INTERNAL_ERROR)));
    EXPECT_CALL(visitor, OnCloseStream(1, Http2ErrorCode::HTTP2_NO_ERROR));
...
TEST_P(OgHttp2AdapterDataTest, ServerSubmitsResponseWithDataSourceError)
{
    ...
    int submit_result = adapter->SubmitResponse(
        1, ToHeaders({ { ":status", "200" }, { "x-comment", "Sure, sounds good." } }), GetParam() ? nullptr : std::move(body1), false);
    ...
    // the stream.
    int trailer_result = adapter->SubmitTrailer(1, ToHeaders({ { ":final-status", "a-ok" } }));
...
TEST(OgHttp2AdapterTest, CompleteRequestWithServerResponse)
{
    ...
    int submit_result = adapter->SubmitResponse(1, ToHeaders({ { ":status", "200" } }), nullptr, true);
    EXPECT_EQ(submit_result, 0);
    EXPECT_TRUE(adapter->want_write());
    ...
    EXPECT_CALL(visitor, OnBeforeFrameSent(SETTINGS, 0, _, ACK_FLAG));
    EXPECT_CALL(visitor, OnFrameSent(SETTINGS, 0, _, ACK_FLAG, 0));
    EXPECT_CALL(visitor, OnBeforeFrameSent(HEADERS, 1, _, END_STREAM_FLAG | END_HEADERS_FLAG));
    EXPECT_CALL(visitor, OnFrameSent(HEADERS, 1, _, END_STREAM_FLAG | END_HEADERS_FLAG, 0));
    EXPECT_CALL(visitor, OnCloseStream(1, Http2ErrorCode::HTTP2_NO_ERROR));
...
TEST(OgHttp2AdapterTest, IncompleteRequestWithServerResponseRstStreamEnabled)
{
    ...
    int send_result = adapter->Send();
    EXPECT_EQ(0, send_result);
    EXPECT_THAT(visitor.data(), EqualsFrames({ SpdyFrameType::SETTINGS, SpdyFrameType::SETTINGS, SpdyFrameType::HEADERS, SpdyFrameType::RST_STREAM }));
    EXPECT_FALSE(adapter->want_write());
}
...
TEST(OgHttp2AdapterTest, ServerHandlesMultipleContentLength)
{
    ...
    const std::string frames = TestFrameSequence()
                                   .ClientPreface()
                                   .Headers(1,
                                       { { ":method", "POST" }, { ":scheme", "https" }, { ":authority", "example.com" }, { ":path", "/1" },
                                           { "content-length", "7" }, { "content-length", "7" } },
                                       /*fin=*/false)
                                   .Headers(3,
                                       { { ":method", "POST" }, { ":scheme", "https" }, { ":authority", "example.com" }, { ":path", "/3" },
                                           { "content-length", "11" }, { "content-length", "13" } },
                                       /*fin=*/false)
                                   .Serialize();
    testing::InSequence s;
...
TEST(OgHttp2AdapterTest, ServerHandlesDataWithPadding)
{
    ...
    const std::string frames
        = TestFrameSequence()
              .ClientPreface()
              .Headers(1, { { ":method", "POST" }, { ":scheme", "https" }, { ":authority", "example.com" }, { ":path", "/this/is/request/one" } },
                  /*fin=*/false)
              .Data(1, "This is the request body.",
                  /*fin=*/true, /*padding_length=*/39)
              .Headers(3, { { ":method", "GET" }, { ":scheme", "http" }, { ":authority", "example.com" }, { ":path", "/this/is/request/two" } },
                  /*fin=*/true)
              .Serialize();
    testing::InSequence s;
...
TEST(OgHttp2AdapterTest, ServerHandlesHostHeader)
{
    ...
    const std::string frames
        = TestFrameSequence()
              .ClientPreface()
              .Headers(1, { { ":method", "POST" }, { ":scheme", "https" }, { ":path", "/this/is/request/one" }, { "host", "example.com" } },
                  /*fin=*/true)
              .Headers(3,
                  { { ":method", "POST" }, { ":scheme", "https" }, { ":authority", "example.com" }, { ":path", "/this/is/request/one" },
                      { "host", "example.com" } },
                  /*fin=*/true)
              .Headers(5,
                  { { ":method", "POST" }, { ":scheme", "https" }, { ":authority", "foo.com" }, { ":path", "/this/is/request/one" }, { "host", "bar.com" } },
                  /*fin=*/true)
              .Serialize();
    ...
    EXPECT_CALL(visitor, OnInvalidFrame(5, Http2VisitorInterface::InvalidFrameError::kHttpHeader));
    ...
    EXPECT_CALL(visitor, OnFrameSent(RST_STREAM, 5, 4, 0x0, static_cast<int>(Http2ErrorCode::PROTOCOL_ERROR)));
...
TEST_P(OgHttp2AdapterDataTest, ServerSubmitsTrailersWithFlowControlBlockage)
{
    ...
    send_result = adapter->Send();
    EXPECT_EQ(0, send_result);
    EXPECT_THAT(visitor.data(), EqualsFrames({ SpdyFrameType::DATA, SpdyFrameType::HEADERS }));
    EXPECT_FALSE(adapter->want_write());
}
...
TEST_P(OgHttp2AdapterDataTest, ServerSubmitsTrailersWithDataEndStream)
{
    ...
    const std::vector<Header> trailers = ToHeaders({ { "extra-info", "Trailers are weird but good?" } });
...
TEST_P(OgHttp2AdapterDataTest, ServerSubmitsTrailersWithDataEndStreamAndDeferral)
{
    TestVisitor visitor;
    OgHttp2Adapter::Options options;
    options.perspective = Perspective::kServer;
...
TEST(OgHttp2AdapterTest, ClientDisobeysConnectionFlowControlWithOneDataFrame)
{
    ...
    const std::string overflow_frames = TestFrameSequence().SettingsAck().Data(1, std::string(window_overflow_bytes, 'a')).Serialize();
...
TEST(OgHttp2AdapterTest, ClientDisobeysConnectionFlowControlAcrossReads)
{
    ...
    process_result = adapter->ProcessBytes(overflow_frames.substr(0, chunk_length));
...
TEST(OgHttp2AdapterTest, ClientDisobeysStreamFlowControl)
{
    ...
    int send_result = adapter->Send();
    EXPECT_EQ(0, send_result);
    EXPECT_THAT(visitor.data(), EqualsFrames({ SpdyFrameType::SETTINGS, SpdyFrameType::SETTINGS, SpdyFrameType::WINDOW_UPDATE }));
    visitor.Clear();
    ...
    EXPECT_TRUE(adapter->want_write());
    EXPECT_CALL(visitor, OnBeforeFrameSent(RST_STREAM, 1, 4, 0x0));
    EXPECT_CALL(visitor, OnFrameSent(RST_STREAM, 1, 4, 0x0, static_cast<int>(Http2ErrorCode::FLOW_CONTROL_ERROR)));
    EXPECT_CALL(visitor, OnCloseStream(1, Http2ErrorCode::HTTP2_NO_ERROR));
...
TEST(OgHttp2AdapterTest, ServerErrorWhileHandlingHeaders)
{
    ...
    EXPECT_CALL(visitor, OnHeaderForStream(1, ":authority", "example.com"));
    EXPECT_CALL(visitor, OnHeaderForStream(1, ":path", "/this/is/request/one"));
    EXPECT_CALL(visitor, OnHeaderForStream(1, "accept", "some bogus value!")).WillOnce(testing::Return(Http2VisitorInterface::HEADER_RST_STREAM));
    ...
    EXPECT_CALL(visitor, OnFrameSent(SETTINGS, 0, 0, ACK_FLAG, 0));
    EXPECT_CALL(visitor, OnBeforeFrameSent(RST_STREAM, 1, 4, 0x0));
    EXPECT_CALL(visitor, OnFrameSent(RST_STREAM, 1, 4, 0x0, static_cast<int>(Http2ErrorCode::INTERNAL_ERROR)));
    EXPECT_CALL(visitor, OnCloseStream(1, Http2ErrorCode::HTTP2_NO_ERROR));
...
TEST(OgHttp2AdapterTest, ServerErrorWhileHandlingHeadersDropsFrames)
{
    ...
    const std::string frames
        = TestFrameSequence()
              ...
              .WindowUpdate(0, 2000)
              .Headers(3, { { ":method", "GET" }, { ":scheme", "https" }, { ":authority", "example.com" }, { ":path", "/this/is/request/two" } },
                  /*fin=*/false)
    ...
    EXPECT_CALL(visitor, OnMetadataForStream(3, "This is the re"))
        .WillOnce(testing::DoAll(testing::InvokeWithoutArgs([&adapter]() { adapter->SubmitRst(3, Http2ErrorCode::REFUSED_STREAM); }), testing::Return(true)));
    ...
    EXPECT_CALL(visitor, OnFrameSent(RST_STREAM, 3, 4, 0x0, static_cast<int>(Http2ErrorCode::REFUSED_STREAM)));
    EXPECT_CALL(visitor, OnCloseStream(3, Http2ErrorCode::HTTP2_NO_ERROR));
    ...
    EXPECT_EQ(0, send_result);
    // SETTINGS ack
    EXPECT_THAT(visitor.data(), EqualsFrames({ SpdyFrameType::SETTINGS, SpdyFrameType::SETTINGS, SpdyFrameType::RST_STREAM, SpdyFrameType::RST_STREAM }));
}
...
TEST(OgHttp2AdapterTest, ServerConnectionErrorWhileHandlingHeaders)
{
    ...
    EXPECT_CALL(visitor, OnHeaderForStream(1, ":authority", "example.com"));
    EXPECT_CALL(visitor, OnHeaderForStream(1, ":path", "/this/is/request/one"));
    EXPECT_CALL(visitor, OnInvalidFrame(1, Http2VisitorInterface::InvalidFrameError::kHttpHeader)).WillOnce(testing::Return(false));
    EXPECT_CALL(visitor, OnConnectionError(ConnectionError::kHeaderError));
    ...
    EXPECT_CALL(visitor, OnFrameSent(SETTINGS, 0, _, 0x0, 0));
    EXPECT_CALL(visitor, OnBeforeFrameSent(RST_STREAM, 1, 4, 0x0));
    EXPECT_CALL(visitor, OnFrameSent(RST_STREAM, 1, 4, 0x0, static_cast<int>(Http2ErrorCode::PROTOCOL_ERROR)));
    EXPECT_CALL(visitor, OnCloseStream(1, Http2ErrorCode::HTTP2_NO_ERROR));
    EXPECT_CALL(visitor, OnBeforeFrameSent(GOAWAY, 0, _, 0x0));
    EXPECT_CALL(visitor, OnFrameSent(GOAWAY, 0, _, 0x0, static_cast<int>(Http2ErrorCode::PROTOCOL_ERROR)));
    ...
    // Some bytes should have been serialized.
    EXPECT_EQ(0, send_result);
    EXPECT_THAT(visitor.data(), EqualsFrames({ SpdyFrameType::SETTINGS, SpdyFrameType::RST_STREAM, SpdyFrameType::GOAWAY }));
}
...
TEST(OgHttp2AdapterTest, ServerErrorAfterHandlingHeaders)
{
    ...
    EXPECT_CALL(visitor, OnHeaderForStream(1, ":authority", "example.com"));
    EXPECT_CALL(visitor, OnHeaderForStream(1, ":path", "/this/is/request/one"));
    EXPECT_CALL(visitor, OnEndHeadersForStream(1)).WillOnce(testing::Return(false));
    EXPECT_CALL(visitor, OnConnectionError(ConnectionError::kParseError));
...
TEST(OgHttp2AdapterTest, ServerRejectsFrameHeader)
{
    ...
    EXPECT_CALL(visitor, OnFrameHeader(0, 8, PING, 0)).WillOnce(testing::Return(false));
    EXPECT_CALL(visitor, OnConnectionError(ConnectionError::kParseError));
...
TEST(OgHttp2AdapterTest, ServerRejectsBeginningOfData)
{
    ...
    EXPECT_CALL(visitor, OnBeginDataForStream(1, 25)).WillOnce(testing::Return(false));
    EXPECT_CALL(visitor, OnConnectionError(ConnectionError::kParseError));
...
TEST(OgHttp2AdapterTest, ServerReceivesTooLargeHeader)
{
    ...
    EXPECT_CALL(visitor, OnFrameHeader(3, _, HEADERS, END_STREAM_FLAG | END_HEADERS_FLAG));
    EXPECT_CALL(visitor, OnBeginHeadersForStream(3));
    EXPECT_CALL(visitor, OnHeaderForStream(3, _, _)).Times(4);
    EXPECT_CALL(visitor, OnEndHeadersForStream(3));
    EXPECT_CALL(visitor, OnEndStream(3));
...
TEST(OgHttp2AdapterTest, ServerReceivesInvalidAuthority)
{
    ...
    const std::string frames
        = TestFrameSequence()
              .ClientPreface()
              .Headers(1, { { ":method", "POST" }, { ":scheme", "https" }, { ":authority", "ex|ample.com" }, { ":path", "/this/is/request/one" } },
                  /*fin=*/false)
              .Serialize();
    testing::InSequence s;
...
TEST(OgHttpAdapterTest, ServerReceivesGoAway)
{
    ...
    const int submit_result = adapter->SubmitResponse(1, ToHeaders({ { ":status", "200" } }),
...
TEST_P(OgHttp2AdapterDataTest, ServerSubmitResponse)
{
    ...
    EXPECT_CALL(visitor, OnHeaderForStream(1, ":authority", "example.com"));
    EXPECT_CALL(visitor, OnHeaderForStream(1, ":path", "/this/is/request/one"));
    EXPECT_CALL(visitor, OnEndHeadersForStream(1)).WillOnce(testing::InvokeWithoutArgs([&adapter, kSentinel1]() {
    ...
    auto body1 = std::make_unique<VisitorDataSource>(visitor, 1);
    int submit_result = adapter->SubmitResponse(
        1, ToHeaders({ { ":status", "404" }, { "x-comment", "I have no idea what you're talking about." } }), GetParam() ? nullptr : std::move(body1), false);
    EXPECT_EQ(submit_result, 0);
    EXPECT_TRUE(adapter->want_write());
...
TEST_P(OgHttp2AdapterDataTest, ServerSubmitResponseWithResetFromClient)
{
    ...
    const std::string reset = TestFrameSequence().RstStream(1, Http2ErrorCode::CANCEL).Serialize();
    EXPECT_CALL(visitor, OnFrameHeader(1, 4, RST_STREAM, 0));
...
INSTANTIATE_TEST_SUITE_P(BothValues, OgHttp2AdapterInteractionDataTest, testing::Bool());
...
TEST_P(OgHttp2AdapterInteractionDataTest, ClientServerInteractionTest)
{
    ...
    EXPECT_CALL(client_visitor, OnReadyToSend(_)).WillRepeatedly(testing::Invoke(server_adapter.get(), &OgHttp2Adapter::ProcessBytes));
    // Feeds bytes sent from the server into the client's ProcessBytes.
    EXPECT_CALL(server_visitor, OnReadyToSend(_)).WillRepeatedly(testing::Invoke(client_adapter.get(), &OgHttp2Adapter::ProcessBytes));
    // Sets up the server to respond automatically to a request from a client.
    EXPECT_CALL(server_visitor, OnEndHeadersForStream(_)).WillRepeatedly([&server_adapter](Http2StreamId stream_id) {
        server_adapter->SubmitResponse(stream_id, ToHeaders({ { ":status", "200" } }), nullptr, true);
        server_adapter->Send();
        return true;
    });
    // Sets up the client to create a new stream automatically when receiving a
    // response.
    EXPECT_CALL(client_visitor, OnEndHeadersForStream(_)).WillRepeatedly([&client_adapter, &client_visitor](Http2StreamId stream_id) {
        if (stream_id < 10) {
            const Http2StreamId new_stream_id = stream_id + 2;
            client_visitor.AppendPayloadForStream(new_stream_id, "This is an example request body.");
            client_visitor.SetEndData(new_stream_id, true);
            auto body = std::make_unique<VisitorDataSource>(client_visitor, new_stream_id);
            const int created_stream_id
                = client_adapter->SubmitRequest(ToHeaders({ { ":method", "GET" }, { ":scheme", "http" }, { ":authority", "example.com" },
                                                    { ":path", absl::StrCat("/this/is/request/", new_stream_id) } }),
    ...
    int stream_id = client_adapter->SubmitRequest(
        ToHeaders({ { ":method", "POST" }, { ":scheme", "http" }, { ":authority", "example.com" }, { ":path", "/this/is/request/one" } }), nullptr, true,
        nullptr);
...
TEST(OgHttp2AdapterInteractionTest, ClientServerInteractionRepeatedHeaderNames)
{
    TestVisitor client_visitor;
    OgHttp2Adapter::Options client_options;
    client_options.perspective = Perspective::kClient;
    ...
    const std::vector<Header> headers1 = ToHeaders({ { ":method", "GET" }, { ":scheme", "http" }, { ":authority", "example.com" },
        { ":path", "/this/is/request/one" }, { "accept", "text/plain" }, { "accept", "text/html" } });

    const int32_t stream_id1 = client_adapter->SubmitRequest(headers1, nullptr, true, nullptr);
    ASSERT_GT(stream_id1, 0);

    EXPECT_CALL(client_visitor, OnBeforeFrameSent(SETTINGS, 0, _, 0x0));
    EXPECT_CALL(client_visitor, OnFrameSent(SETTINGS, 0, _, 0x0, 0));
    EXPECT_CALL(client_visitor, OnBeforeFrameSent(HEADERS, stream_id1, _, END_STREAM_FLAG | END_HEADERS_FLAG));
    EXPECT_CALL(client_visitor, OnFrameSent(HEADERS, stream_id1, _, END_STREAM_FLAG | END_HEADERS_FLAG, 0));
    ...
    EXPECT_CALL(server_visitor, OnHeaderForStream(1, ":authority", "example.com"));
    EXPECT_CALL(server_visitor, OnHeaderForStream(1, ":path", "/this/is/request/one"));
...
TEST(OgHttp2AdapterInteractionTest, ClientServerInteractionWithCookies)
{
    ...
    // serialization.
    const std::vector<Header> headers1 = ToHeaders({ { ":method", "GET" }, { ":scheme", "http" }, { ":authority", "example.com" },
        { ":path", "/this/is/request/one" }, { "cookie", "a; b=2; c" }, { "cookie", "d=e, f, g; h" } });
    ...
    EXPECT_CALL(server_visitor, OnHeaderForStream(1, ":authority", "example.com"));
    EXPECT_CALL(server_visitor, OnHeaderForStream(1, ":path", "/this/is/request/one"));
    EXPECT_CALL(server_visitor, OnHeaderForStream(1, "cookie", "a; b=2; c; d=e, f, g; h"));
    EXPECT_CALL(server_visitor, OnEndHeadersForStream(1));
    EXPECT_CALL(server_visitor, OnEndStream(1));
...
TEST(OgHttp2AdapterTest, ServerForbidsNewStreamBelowWatermark)
{
    ...
    const std::string frames
        = TestFrameSequence()
              .ClientPreface()
              .Headers(3, { { ":method", "POST" }, { ":scheme", "https" }, { ":authority", "example.com" }, { ":path", "/this/is/request/one" } },
                  /*fin=*/false)
              .Data(3, "This is the request body.")
              .Headers(1, { { ":method", "GET" }, { ":scheme", "http" }, { ":authority", "example.com" }, { ":path", "/this/is/request/two" } },
                  /*fin=*/true)
              .Serialize();
    testing::InSequence s;
...
TEST(OgHttp2AdapterTest, ServerForbidsWindowUpdateOnIdleStream)
{
    ...
    const std::string frames = TestFrameSequence().ClientPreface().WindowUpdate(1, 42).Serialize();
...
TEST(OgHttp2AdapterTest, ServerForbidsDataOnIdleStream)
{
    ...
    const std::string frames = TestFrameSequence().ClientPreface().Data(1, "Sorry, out of order").Serialize();
...
TEST(OgHttp2AdapterTest, ServerForbidsRstStreamOnIdleStream)
{
    ...
    const std::string frames = TestFrameSequence().ClientPreface().RstStream(1, Http2ErrorCode::ENHANCE_YOUR_CALM).Serialize();
...
TEST(OgHttp2AdapterTest, ServerForbidsNewStreamAboveStreamLimit)
{
    ...
    const std::string initial_frames = TestFrameSequence().ClientPreface().Serialize();
    ...
    EXPECT_CALL(visitor, OnEndStream(1));
    EXPECT_CALL(visitor, OnFrameHeader(3, _, HEADERS, END_STREAM_FLAG | END_HEADERS_FLAG));
    EXPECT_CALL(visitor, OnInvalidFrame(3, Http2VisitorInterface::InvalidFrameError::kProtocol));
    // The oghttp2 stack also signals the error via OnConnectionError().
    EXPECT_CALL(visitor, OnConnectionError(ConnectionError::kExceededMaxConcurrentStreams));
...
TEST(OgHttp2AdapterTest, ServerRstStreamsNewStreamAboveStreamLimitBeforeAck)
{
    ...
    EXPECT_CALL(visitor, OnEndStream(1));
    EXPECT_CALL(visitor, OnFrameHeader(3, _, HEADERS, END_STREAM_FLAG | END_HEADERS_FLAG));
    EXPECT_CALL(visitor, OnInvalidFrame(3, Http2VisitorInterface::InvalidFrameError::kRefusedStream));
    ...
    EXPECT_TRUE(adapter->want_write());
    EXPECT_CALL(visitor, OnBeforeFrameSent(RST_STREAM, 3, _, 0x0));
    EXPECT_CALL(visitor, OnFrameSent(RST_STREAM, 3, _, 0x0, static_cast<int>(Http2ErrorCode::REFUSED_STREAM)));
...
TEST(OgHttp2AdapterTest, ServerForbidsProtocolPseudoheaderBeforeAck)
{
    ...
    const std::string stream1_frames = TestFrameSequence()
                                           .Headers(1,
                                               { { ":method", "CONNECT" }, { ":scheme", "https" }, { ":authority", "example.com" },
                                                   { ":path", "/this/is/request/one" }, { ":protocol", "websocket" } },
                                               /*fin=*/true)
                                           .Serialize();
    ...
    int send_result = adapter->Send();
    EXPECT_EQ(0, send_result);
    EXPECT_THAT(visitor.data(), EqualsFrames({ SpdyFrameType::SETTINGS, SpdyFrameType::SETTINGS, SpdyFrameType::RST_STREAM, SpdyFrameType::SETTINGS }));
    visitor.Clear();

    // The client attempts to send a CONNECT request with the `:protocol`
    // pseudoheader before acking the server's SETTINGS frame.
    const std::string stream3_frames = TestFrameSequence()
                                           .Headers(3,
                                               { { ":method", "CONNECT" }, { ":scheme", "https" }, { ":authority", "example.com" },
                                                   { ":path", "/this/is/request/two" }, { ":protocol", "websocket" } },
                                               /*fin=*/true)
                                           .Serialize();
...
TEST_P(OgHttp2AdapterDataTest, SkipsSendingFramesForRejectedStream)
{
    ...
    const std::string initial_frames
        = TestFrameSequence()
              .ClientPreface()
              .Headers(1, { { ":method", "GET" }, { ":scheme", "http" }, { ":authority", "example.com" }, { ":path", "/this/is/request/one" } },
                  /*fin=*/true)
              .Serialize();
    ...
    visitor.AppendPayloadForStream(1, "Here is some data, which will be completely ignored!");
    auto body = std::make_unique<VisitorDataSource>(visitor, 1);

    int submit_result = adapter->SubmitResponse(1, ToHeaders({ { ":status", "200" } }), GetParam() ? nullptr : std::move(body), false);
    ASSERT_EQ(0, submit_result);

    auto source = std::make_unique<TestMetadataSource>(ToHeaderBlock(ToHeaders({ { "query-cost", "is too darn high" }, { "secret-sauce", "hollandaise" } })));
...
TEST(OgHttp2AdapterTest, ServerStartsShutdownAfterGoaway)
{
    ...
    adapter->SubmitGoAway(1, Http2ErrorCode::HTTP2_NO_ERROR, "and don't come back!");
    EXPECT_TRUE(adapter->want_write());
...
TEST_P(OgHttp2AdapterDataTest, ServerDoesNotSendFramesAfterImmediateGoAway)
{
    ...
    const std::string connection_error_frames = TestFrameSequence().WindowUpdate(3, 42).Serialize();
    ...
    const std::string stream_frames = TestFrameSequence()
                                          ...
                                          .Headers(1,
                                              { { ":method", "GET" }, { ":scheme", "https" }, { ":authority", "example.com" },
                                                  { ":path", "/this/is/request/one" }, { "content-length", "2" } })
                                          .Data(1, "hi", /*fin=*/true)
                                          .Headers(3,
                                              { { ":method", "GET" }, { ":scheme", "https" }, { ":authority", "example.com" },
                                                  { ":path", "/this/is/request/three" }, { "content-length", "nan" } },
                                              /*fin=*/true)
                                          .Serialize();
...
TEST(OgHttp2AdapterTest, ServerHandlesContentLengthMismatch)
{
    ...
    const std::string stream_frames = TestFrameSequence()
                                          ...
                                          .Headers(1,
                                              { { ":method", "GET" }, { ":scheme", "https" }, { ":authority", "example.com" },
                                                  { ":path", "/this/is/request/two" }, { "content-length", "2" } })
                                          .Data(1, "h", /*fin=*/true)
                                          .Headers(3,
                                              { { ":method", "GET" }, { ":scheme", "https" }, { ":authority", "example.com" },
                                                  { ":path", "/this/is/request/three" }, { "content-length", "2" } })
                                          .Data(3, "howdy", /*fin=*/true)
                                          .Headers(5,
                                              { { ":method", "GET" }, { ":scheme", "https" }, { ":authority", "example.com" },
                                                  { ":path", "/this/is/request/four" }, { "content-length", "2" } },
                                              /*fin=*/true)
                                          .Headers(7,
                                              { { ":method", "GET" }, { ":scheme", "https" }, { ":authority", "example.com" },
                                                  { ":path", "/this/is/request/four" }, { "content-length", "2" } },
    ...
    EXPECT_CALL(visitor, OnHeaderForStream(5, _, _)).Times(5);
    EXPECT_CALL(visitor, OnEndHeadersForStream(5));
    EXPECT_CALL(visitor, OnInvalidFrame(5, Http2VisitorInterface::InvalidFrameError::kHttpMessaging));
    ...
    EXPECT_CALL(visitor, OnEndHeadersForStream(7));
    EXPECT_CALL(visitor, OnInvalidFrame(7, Http2VisitorInterface::InvalidFrameError::kHttpMessaging));
    ...
    EXPECT_CALL(visitor, OnFrameSent(RST_STREAM, 5, _, 0x0, static_cast<int>(Http2ErrorCode::PROTOCOL_ERROR)));
    EXPECT_CALL(visitor, OnCloseStream(5, Http2ErrorCode::HTTP2_NO_ERROR));
    EXPECT_CALL(visitor, OnBeforeFrameSent(RST_STREAM, 7, _, 0x0));
    EXPECT_CALL(visitor, OnFrameSent(RST_STREAM, 7, _, 0x0, static_cast<int>(Http2ErrorCode::PROTOCOL_ERROR)));
    ...
    EXPECT_EQ(0, result);
    EXPECT_THAT(visitor.data(),
        EqualsFrames({ SpdyFrameType::SETTINGS, SpdyFrameType::SETTINGS, SpdyFrameType::RST_STREAM, SpdyFrameType::RST_STREAM, SpdyFrameType::RST_STREAM,
...
TEST(OgHttp2AdapterTest, ServerHandlesAsteriskPathForOptions)
{
    ...
    const std::string stream_frames = TestFrameSequence()
                                          .ClientPreface()
                                          .Headers(1, { { ":scheme", "https" }, { ":authority", "example.com" }, { ":path", "*" }, { ":method", "OPTIONS" } },
                                              /*fin=*/true)
                                          .Serialize();
...
TEST(OgHttp2AdapterTest, ServerHandlesInvalidPath)
{
    ...
    const std::string stream_frames
        = TestFrameSequence()
              .ClientPreface()
              .Headers(1, { { ":scheme", "https" }, { ":authority", "example.com" }, { ":path", "*" }, { ":method", "GET" } },
                  /*fin=*/true)
              .Headers(3, { { ":scheme", "https" }, { ":authority", "example.com" }, { ":path", "other/non/slash/starter" }, { ":method", "GET" } },
                  /*fin=*/true)
              .Headers(5, { { ":scheme", "https" }, { ":authority", "example.com" }, { ":path", "" }, { ":method", "GET" } },
                  /*fin=*/true)
              .Serialize();
    ...
    EXPECT_CALL(visitor, OnBeginHeadersForStream(3));
    EXPECT_CALL(visitor, OnHeaderForStream(3, _, _)).Times(4);
    EXPECT_CALL(visitor, OnInvalidFrame(3, Http2VisitorInterface::InvalidFrameError::kHttpMessaging));
...
TEST(OgHttp2AdapterTest, ServerHandlesTeHeader)
{
    ...
    const std::string stream_frames
        = TestFrameSequence()
              .ClientPreface()
              .Headers(1, { { ":scheme", "https" }, { ":authority", "example.com" }, { ":path", "/" }, { ":method", "GET" }, { "te", "trailers" } },
                  /*fin=*/true)
              .Headers(3, { { ":scheme", "https" }, { ":authority", "example.com" }, { ":path", "/" }, { ":method", "GET" }, { "te", "trailers, deflate" } },
                  /*fin=*/true)
              .Serialize();
...
TEST(OgHttp2AdapterTest, ServerHandlesConnectionSpecificHeaders)
{
    ...
    const std::string stream_frames
        = TestFrameSequence()
              .ClientPreface()
              .Headers(1, { { ":scheme", "https" }, { ":authority", "example.com" }, { ":path", "/" }, { ":method", "GET" }, { "connection", "keep-alive" } },
                  /*fin=*/true)
              .Headers(3,
                  { { ":scheme", "https" }, { ":authority", "example.com" }, { ":path", "/" }, { ":method", "GET" }, { "proxy-connection", "keep-alive" } },
                  /*fin=*/true)
              .Headers(5, { { ":scheme", "https" }, { ":authority", "example.com" }, { ":path", "/" }, { ":method", "GET" }, { "keep-alive", "timeout=42" } },
                  /*fin=*/true)
              .Headers(7,
                  { { ":scheme", "https" }, { ":authority", "example.com" }, { ":path", "/" }, { ":method", "GET" }, { "transfer-encoding", "chunked" } },
                  /*fin=*/true)
              .Headers(9, { { ":scheme", "https" }, { ":authority", "example.com" }, { ":path", "/" }, { ":method", "GET" }, { "upgrade", "h2c" } },
                  /*fin=*/true)
              .Serialize();
    ...
    EXPECT_CALL(visitor, OnInvalidFrame(7, Http2VisitorInterface::InvalidFrameError::kHttpHeader));
    ...
    EXPECT_CALL(visitor, OnInvalidFrame(9, Http2VisitorInterface::InvalidFrameError::kHttpHeader));
    ...
    EXPECT_CALL(visitor, OnFrameSent(RST_STREAM, 9, _, 0x0, static_cast<int>(Http2ErrorCode::PROTOCOL_ERROR)));
...
TEST(OgHttp2AdapterTest, ServerUsesCustomWindowUpdateStrategy)
{
    ...
    TestVisitor visitor;
    OgHttp2Adapter::Options options;
    options.should_window_update_fn = [](int64_t /*limit*/, int64_t /*size*/, int64_t /*delta*/) { return true; };
    options.perspective = Perspective::kServer;
    auto adapter = OgHttp2Adapter::Create(visitor, options);
    ...
    int send_result = adapter->Send();
    EXPECT_EQ(0, send_result);
    EXPECT_THAT(visitor.data(), EqualsFrames({ SpdyFrameType::SETTINGS, SpdyFrameType::SETTINGS, SpdyFrameType::WINDOW_UPDATE, SpdyFrameType::WINDOW_UPDATE }));
}
...
TEST(OgHttp2AdapterTest, ServerConsumesDataWithPadding)
{
    ...
    TestFrameSequence seq = std::move(TestFrameSequence().ClientPreface().Headers(1,
        { { ":method", "POST" }, { ":scheme", "https" }, { ":authority", "example.com" }, { ":path", "/this/is/request/one" } },
        /*fin=*/false));
    ...
    EXPECT_CALL(visitor, OnHeaderForStream(1, _, _)).Times(4);
    EXPECT_CALL(visitor, OnEndHeadersForStream(1));
    EXPECT_CALL(visitor, OnFrameHeader(1, _, DATA, 0x8)).Times(testing::AtLeast(1));
...
TEST(OgHttp2AdapterTest, NoopHeaderValidatorTest)
{
    ...
    const std::string frames
        = TestFrameSequence()
              .ClientPreface()
              .Headers(1,
                  { { ":method", "POST" }, { ":scheme", "https" }, { ":authority", "example.com" }, { ":path", "/1" }, { "content-length", "7" },
                      { "content-length", "7" } },
                  /*fin=*/false)
              .Headers(3,
                  { { ":method", "POST" }, { ":scheme", "https" }, { ":authority", "example.com" }, { ":path", "/3" }, { "content-length", "11" },
                      { "content-length", "13" } },
                  /*fin=*/false)
              .Headers(5, { { ":method", "POST" }, { ":scheme", "https" }, { ":authority", "foo.com" }, { ":path", "/" }, { "host", "bar.com" } },
                  /*fin=*/true)
              .Headers(7,
                  { { ":method", "POST" }, { ":scheme", "https" }, { ":authority", "example.com" }, { ":path", "/" }, { "Accept", "uppercase, oh boy!" } },
                  /*fin=*/false)
              .Headers(9, { { ":method", "POST" }, { ":scheme", "https" }, { ":authority", "ex|ample.com" }, { ":path", "/" } },
                  /*fin=*/false)
              .Headers(11, { { ":method", "GET" }, { ":scheme", "https" }, { ":authority", "example.com" }, { ":path", "/" }, { "content-length", "nan" } },
                  /*fin=*/true)
              .Serialize();
    testing::InSequence s;
...
TEST_P(OgHttp2AdapterDataTest, NegativeFlowControlStreamResumption)
{
    ...
    EXPECT_CALL(visitor, OnFrameHeader(0, 6, SETTINGS, 0));
    EXPECT_CALL(visitor, OnSettingsStart());
    EXPECT_CALL(visitor, OnSetting(Http2Setting { Http2KnownSettingsId::INITIAL_WINDOW_SIZE, 128u * 1024u }));
    EXPECT_CALL(visitor, OnSettingsEnd());
    ...
    EXPECT_CALL(visitor, OnFrameHeader(0, 6, SETTINGS, 0));
    EXPECT_CALL(visitor, OnSettingsStart());
    EXPECT_CALL(visitor, OnSetting(Http2Setting { Http2KnownSettingsId::INITIAL_WINDOW_SIZE, 64u * 1024u }));
    EXPECT_CALL(visitor, OnSettingsEnd());

    // Processing these SETTINGS will cause stream 1's send window to become
    // negative.
    adapter->ProcessBytes(TestFrameSequence().Settings({ { INITIAL_WINDOW_SIZE, 64u * 1024u } }).Serialize());
    EXPECT_TRUE(adapter->want_write());
...
TEST(OgHttp2AdapterTest, SetCookieRoundtrip)
{
    ...
    const int32_t stream_id1 = client_adapter->SubmitRequest(request_headers, nullptr, true, nullptr);
    ASSERT_GT(stream_id1, 0);
    ...
    ASSERT_EQ(client_visitor.data().size(), server_adapter->ProcessBytes(client_visitor.data()));

    // Response headers contain two individual Set-Cookie fields.
    const std::vector<Header> response_headers
        = ToHeaders({ { ":status", "200" }, { "set-cookie", "chocolate_chip=yummy" }, { "set-cookie", "macadamia_nut=okay" } });

    EXPECT_EQ(0, server_adapter->SubmitResponse(stream_id1, response_headers, nullptr, true));
    ...
    // Stream 1 response.
    EXPECT_CALL(server_visitor, OnBeforeFrameSent(HEADERS, stream_id1, _, END_STREAM_FLAG | END_HEADERS_FLAG));
    EXPECT_CALL(server_visitor, OnFrameSent(HEADERS, stream_id1, _, END_STREAM_FLAG | END_HEADERS_FLAG, 0));
    // Stream 1 is complete.
    EXPECT_CALL(server_visitor, OnCloseStream(stream_id1, Http2ErrorCode::HTTP2_NO_ERROR));
    ...
    EXPECT_CALL(client_visitor, OnHeaderForStream(stream_id1, "set-cookie", "chocolate_chip=yummy"));
    EXPECT_CALL(client_visitor, OnHeaderForStream(stream_id1, "set-cookie", "macadamia_nut=okay"));
    EXPECT_CALL(client_visitor, OnEndHeadersForStream(stream_id1));
    EXPECT_CALL(client_visitor, OnEndStream(stream_id1));
    EXPECT_CALL(client_visitor, OnCloseStream(stream_id1, Http2ErrorCode::HTTP2_NO_ERROR));

    // The client adapter processes the server's output.
    ASSERT_EQ(server_visitor.data().size(), client_adapter->ProcessBytes(server_visitor.data()));
}
