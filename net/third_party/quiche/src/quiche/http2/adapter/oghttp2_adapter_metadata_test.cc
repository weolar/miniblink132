...
TEST(OgHttp2AdapterTest, ClientHandlesMetadataWithPayloadError)
{
    ...
    EXPECT_CALL(visitor, OnMetadataForStream(stream_id, _)).WillOnce(testing::Return(false));
...
TEST(OgHttp2AdapterTest, ClientHandlesMetadataWithCompletionError)
{
    ...
    EXPECT_CALL(visitor, OnMetadataEndForStream(stream_id)).WillOnce(testing::Return(false));
    // Remaining frames are not processed due to the error.
    EXPECT_CALL(visitor, OnConnectionError(ConnectionError::kParseError));
...
TEST_P(MetadataApiTest, SubmitMetadata)
{
    ...
    int result = adapter->Send();
    EXPECT_EQ(0, result);
    EXPECT_THAT(visitor.data(), EqualsFrames({ SpdyFrameType::SETTINGS, static_cast<SpdyFrameType>(kMetadataFrameType) }));
    EXPECT_FALSE(adapter->want_write());
}
...
TEST_P(MetadataApiTest, ServerQueuesMetadataThenTrailers)
{
    ...
    visitor.SetEndData(1, false);
    auto body1 = std::make_unique<VisitorDataSource>(visitor, 1);
    int submit_result = adapter->SubmitResponse(1, ToHeaders({ { ":status", "200" }, { "x-comment", "Sure, sounds good." } }), std::move(body1), false);
    ...
    const quiche::HttpHeaderBlock block = ToHeaderBlock(ToHeaders({ { "key", "wild value!" } }));
    if (GetParam()) {
        visitor.AppendMetadataForStream(1, block);
        adapter->SubmitMetadata(1, 1);
    } else {
        adapter->SubmitMetadata(1, 16384u, std::make_unique<TestMetadataSource>(std::move(block)));
    }
    ...
    EXPECT_EQ(0, send_result);
    EXPECT_THAT(visitor.data(), EqualsFrames({ static_cast<SpdyFrameType>(kMetadataFrameType), SpdyFrameType::HEADERS }));
}
