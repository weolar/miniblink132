...
class QUICHE_NO_EXPORT TestVisitor : public testing::StrictMock<MockHttp2Visitor> {
public:
    ...
    {
        const bool ret = testing::StrictMock<MockHttp2Visitor>::OnMetadataForStream(stream_id, metadata);
        if (ret) {
            auto result = metadata_map_.try_emplace(stream_id, std::vector<std::string>());
    ...
    void AppendPayloadForStream(Http2StreamId stream_id, absl::string_view payload);
    ...
    void AppendMetadataForStream(Http2StreamId stream_id, const quiche::HttpHeaderBlock& payload);
    ...
    void set_send_limit(size_t limit)
    {
        send_limit_ = limit;
    }
    ...
    void set_is_write_blocked(bool value)
    {
        is_write_blocked_ = value;
    }

    void set_has_write_error()
    {
        has_write_error_ = true;
    }
