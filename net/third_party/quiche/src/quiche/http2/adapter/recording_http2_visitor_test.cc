...
TEST(RecordingHttp2VisitorTest, EmptySequence)
{
    ...
    EXPECT_EQ(chocolate_visitor.GetEventSequence(), vanilla_visitor.GetEventSequence());

    chocolate_visitor.OnSettingsStart();

    EXPECT_THAT(chocolate_visitor.GetEventSequence(), testing::Not(IsEmpty()));
    EXPECT_THAT(vanilla_visitor.GetEventSequence(), IsEmpty());
    EXPECT_NE(chocolate_visitor.GetEventSequence(), vanilla_visitor.GetEventSequence());
...
TEST(RecordingHttp2VisitorTest, SameEventsProduceSameSequence)
{
    ...
    const Http2ErrorCode error_code = static_cast<Http2ErrorCode>(random.Uniform(static_cast<int>(Http2ErrorCode::MAX_ERROR_CODE)));
    ...
    const std::string some_string = random.RandStringWithAlphabet(random.Rand8(), alphabet);
    const std::string another_string = random.RandStringWithAlphabet(random.Rand8(), alphabet);
    ...
    std::list<RecordingHttp2Visitor*> visitors = { &chocolate_visitor, &vanilla_visitor };
    for (RecordingHttp2Visitor* visitor : visitors) {
        visitor->OnConnectionError(Http2VisitorInterface::ConnectionError::kSendError);
        ...
        visitor->OnPriorityForStream(stream_id, another_stream_id, some_int, some_bool);
