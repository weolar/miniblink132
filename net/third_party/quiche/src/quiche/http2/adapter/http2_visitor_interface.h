...
public:
    ...
    // unrecoverable error.
    virtual DataFrameHeaderInfo OnReadyToSendDataForStream(Http2StreamId /*stream_id*/, size_t /*max_length*/)
    {
    ...
    virtual bool SendDataFrame(Http2StreamId /*stream_id*/, absl::string_view /*frame_header*/, size_t /*payload_bytes*/)
    {
        QUICHE_LOG(FATAL) << "Not implemented.";
        return false;
    }
    ...
    // error has occurred.
    virtual bool OnFrameHeader(Http2StreamId /*stream_id*/, size_t /*length*/, uint8_t /*type*/, uint8_t /*flags*/)
    {
        return true;
    }
    ...
    virtual OnHeaderResult OnHeaderForStream(Http2StreamId stream_id, absl::string_view key, absl::string_view value) = 0;
    ...
    virtual bool OnBeginDataForStream(Http2StreamId stream_id, size_t payload_length) = 0;
    ...
    // error has occurred.
    virtual bool OnDataPaddingLength(Http2StreamId stream_id, size_t padding_length) = 0;
    ...
    virtual bool OnDataForStream(Http2StreamId stream_id, absl::string_view data) = 0;
    ...
    virtual void OnRstStream(Http2StreamId stream_id, Http2ErrorCode error_code) = 0;

    // Called when a stream is closed. Returns false if a fatal error has
    // occurred.
    virtual bool OnCloseStream(Http2StreamId stream_id, Http2ErrorCode error_code) = 0;

    // Called when the connection receives a PRIORITY frame.
    virtual void OnPriorityForStream(Http2StreamId stream_id, Http2StreamId parent_stream_id, int weight, bool exclusive) = 0;
    ...
    virtual void OnPushPromiseForStream(Http2StreamId stream_id, Http2StreamId promised_stream_id) = 0;

    // Called when the connection receives a GOAWAY frame. Returns false if a
    // fatal error has occurred.
    virtual bool OnGoAway(Http2StreamId last_accepted_stream_id, Http2ErrorCode error_code, absl::string_view opaque_data) = 0;

    // Called when the connection receives a WINDOW_UPDATE frame. For
    // connection-level window updates, the |stream_id| will be 0.
    virtual void OnWindowUpdate(Http2StreamId stream_id, int window_increment) = 0;

    // Called immediately before a frame of the given type is sent. Should return
    // 0 on success.
    virtual int OnBeforeFrameSent(uint8_t frame_type, Http2StreamId stream_id, size_t length, uint8_t flags) = 0;
    ...
    // types.
    virtual int OnFrameSent(uint8_t frame_type, Http2StreamId stream_id, size_t length, uint8_t flags, uint32_t error_code) = 0;
    ...
    virtual bool OnInvalidFrame(Http2StreamId stream_id, InvalidFrameError error) = 0;
    ...
    virtual void OnBeginMetadataForStream(Http2StreamId stream_id, size_t payload_length) = 0;

    // Called when the connection receives |metadata| as part of a METADATA frame
    // payload for a stream. Returns false if a fatal error has occurred.
    virtual bool OnMetadataForStream(Http2StreamId stream_id, absl::string_view metadata) = 0;
    ...
    virtual std::pair<int64_t, bool> PackMetadataForStream(Http2StreamId /*stream_id*/, uint8_t* /*dest*/, size_t /*dest_len*/)
    {
