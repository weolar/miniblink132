...
constexpr spdy::SpdyErrorCode error_code = spdy::SpdyErrorCode::ERROR_CODE_ENHANCE_YOUR_CALM;
...
TEST(EventForwarderTest, ForwardsEventsWithTruePredicate)
{
    ...
    EXPECT_CALL(receiver, OnError(Http2DecoderAdapter::SpdyFramerError::SPDY_STOP_PROCESSING, std::string(some_data)));
    event_forwarder.OnError(Http2DecoderAdapter::SpdyFramerError::SPDY_STOP_PROCESSING, std::string(some_data));

    EXPECT_CALL(receiver, OnCommonHeader(stream_id, length, /*type=*/0x0, END_STREAM_FLAG));
    event_forwarder.OnCommonHeader(stream_id, length, /*type=*/0x0, END_STREAM_FLAG);
    ...
    EXPECT_CALL(receiver, OnStreamFrameData(stream_id, some_data.data(), some_data.size()));
    event_forwarder.OnStreamFrameData(stream_id, some_data.data(), some_data.size());
    ...
    spdy::SpdyHeadersHandlerInterface* handler = event_forwarder.OnHeaderFrameStart(stream_id);
    ...
    EXPECT_CALL(receiver, OnSetting(spdy::SpdyKnownSettingsId::SETTINGS_MAX_CONCURRENT_STREAMS, 100));
    event_forwarder.OnSetting(spdy::SpdyKnownSettingsId::SETTINGS_MAX_CONCURRENT_STREAMS, 100);
    ...
    event_forwarder.OnHeaders(stream_id, /*payload_length=*/1234,
        /*has_priority=*/false, /*weight=*/42, stream_id + 2, /*exclusive=*/false, /*fin=*/true,
        /*end=*/true);
    ...
    EXPECT_CALL(receiver, OnContinuation(stream_id, /*payload_length=*/42, /*end=*/true));
    ...
    EXPECT_CALL(receiver,
        OnPriority(stream_id, stream_id + 2, /*weight=*/42,
            /*exclusive=*/false));
    ...
    EXPECT_CALL(receiver,
        OnUnknownFrameStart(stream_id, /*length=*/42,
            /*type=*/0x4D, /*flags=*/0x0));
