...
int FakeSendCallback(nghttp2_session*, nghttp2_frame* /*frame*/, const uint8_t* framehd, size_t length, nghttp2_data_source* source, void* user_data)
{
...
TEST(MakeZeroCopyDataFrameSource, EmptyPayload)
{
    ...
    std::unique_ptr<DataFrameSource> frame_source = MakeZeroCopyDataFrameSource(provider, &result, FakeSendCallback);
...
TEST(MakeZeroCopyDataFrameSource, ShortPayload)
{
    ...
    const absl::string_view kShortBody = "<html><head><title>Example Page!</title></head>"
