...
public:
    ...
    MOCK_METHOD(int, SendData, (nghttp2_frame * frame, const uint8_t* framehd, size_t length, nghttp2_data_source* source), ());

    MOCK_METHOD(int, OnBeginHeaders, (const nghttp2_frame* frame), ());

    MOCK_METHOD(int, OnHeader, (const nghttp2_frame* frame, absl::string_view name, absl::string_view value, uint8_t flags), ());

    MOCK_METHOD(int, OnDataChunkRecv, (uint8_t flags, int32_t stream_id, absl::string_view data), ());
    ...
    MOCK_METHOD(int, OnFrameNotSend, (const nghttp2_frame* frame, int lib_error_code), ());

    MOCK_METHOD(int, OnInvalidFrameRecv, (const nghttp2_frame* frame, int error_code), ());

    MOCK_METHOD(int, OnErrorCallback2, (int lib_error_code, const char* msg, size_t len), ());

    MOCK_METHOD(ssize_t, OnPackExtension, (uint8_t * buf, size_t len, const nghttp2_frame* frame), ());
};
