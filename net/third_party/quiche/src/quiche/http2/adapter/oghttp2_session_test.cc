...
TEST(OgHttp2SessionTest, ClientHandlesFrames)
{
    ...
    EXPECT_EQ(session.GetRemoteWindowSize(), kInitialFlowControlWindowSize + 1000);
    EXPECT_EQ(0, session.GetHighestReceivedStreamId());
    ...
    int stream_id = session.SubmitRequest(
        ToHeaders({ { ":method", "POST" }, { ":scheme", "http" }, { ":authority", "example.com" }, { ":path", "/this/is/request/one" } }), std::move(body1),
        false, const_cast<char*>(kSentinel1));
    ASSERT_EQ(stream_id, 1);

    // Submit another request to ensure the next stream is created.
    int stream_id2 = session.SubmitRequest(
        ToHeaders({ { ":method", "GET" }, { ":scheme", "http" }, { ":authority", "example.com" }, { ":path", "/this/is/request/two" } }), nullptr, true,
        nullptr);
    ...
    EXPECT_CALL(visitor, OnCloseStream(stream_id2, Http2ErrorCode::INTERNAL_ERROR));
    ...
    EXPECT_GT(kInitialFlowControlWindowSize, session.GetStreamReceiveWindowSize(stream_id));
    // Connection receive window is equivalent to the first stream's.
    EXPECT_EQ(session.GetReceiveWindowSize(), session.GetStreamReceiveWindowSize(stream_id));
    // Receive window upper bound is still the initial value.
    EXPECT_EQ(kInitialFlowControlWindowSize, session.GetStreamReceiveWindowLimit(stream_id));
...
TEST(OgHttp2SessionTest, ClientEnqueuesSettingsBeforeOtherFrame)
{
    ...
    EXPECT_THAT(serialized, testing::StartsWith(spdy::kHttp2ConnectionHeaderPrefix));
    serialized.remove_prefix(strlen(spdy::kHttp2ConnectionHeaderPrefix));
    EXPECT_THAT(serialized, EqualsFrames({ SpdyFrameType::SETTINGS, SpdyFrameType::PING }));
}
...
TEST(OgHttp2SessionTest, ClientSubmitRequest)
{
    ...
    // the default.
    EXPECT_LT(session.GetStreamSendWindowSize(stream_id), kInitialFlowControlWindowSize);
    ...
    stream_id = session.SubmitRequest(
    ...
    EXPECT_EQ(session.GetStreamSendWindowSize(stream_id), kInitialFlowControlWindowSize);
}
...
TEST(OgHttp2SessionTest, ClientSubmitRequestWithLargePayload)
{
    ...
    const std::string initial_frames = TestFrameSequence().ServerPreface({ Http2Setting { Http2KnownSettingsId::MAX_FRAME_SIZE, 32768u } }).Serialize();
    testing::InSequence s;

    // Server preface (empty SETTINGS)
    EXPECT_CALL(visitor, OnFrameHeader(0, 6, SETTINGS, 0));
    EXPECT_CALL(visitor, OnSettingsStart());
    EXPECT_CALL(visitor, OnSetting(Http2Setting { Http2KnownSettingsId::MAX_FRAME_SIZE, 32768u }));
    EXPECT_CALL(visitor, OnSettingsEnd());
...
TEST(OgHttp2SessionTest, ClientSubmitRequestWithReadBlock)
{
    ...
    EXPECT_THAT(serialized, testing::StartsWith(spdy::kHttp2ConnectionHeaderPrefix));
    serialized.remove_prefix(strlen(spdy::kHttp2ConnectionHeaderPrefix));
    EXPECT_THAT(serialized, EqualsFrames({ SpdyFrameType::SETTINGS, SpdyFrameType::HEADERS }));
...
TEST(OgHttp2SessionTest, ClientSubmitRequestWithWriteBlock)
{
    ...
    EXPECT_THAT(serialized, testing::StartsWith(spdy::kHttp2ConnectionHeaderPrefix));
    serialized.remove_prefix(strlen(spdy::kHttp2ConnectionHeaderPrefix));
    EXPECT_THAT(serialized, EqualsFrames({ SpdyFrameType::SETTINGS, SpdyFrameType::HEADERS, SpdyFrameType::DATA }));
    EXPECT_FALSE(session.want_write());
}
...
TEST(OgHttp2SessionTest, ServerHandlesFrames)
{
    ...
    EXPECT_CALL(visitor, OnEndHeadersForStream(1)).WillOnce(testing::InvokeWithoutArgs([&session, kSentinel1]() {
    ...
    // The first stream is active and has received some data.
    EXPECT_GT(kInitialFlowControlWindowSize, session.GetStreamReceiveWindowSize(1));
    // Connection receive window is equivalent to the first stream's.
    EXPECT_EQ(session.GetReceiveWindowSize(), session.GetStreamReceiveWindowSize(1));
    // Receive window upper bound is still the initial value.
    EXPECT_EQ(kInitialFlowControlWindowSize, session.GetStreamReceiveWindowLimit(1));
    ...
    EXPECT_THAT(
        visitor.data(), EqualsFrames({ spdy::SpdyFrameType::SETTINGS, spdy::SpdyFrameType::SETTINGS, spdy::SpdyFrameType::PING, spdy::SpdyFrameType::PING }));
}
...
TEST(OgHttp2SessionTest, ServerSubmitResponse)
{
    ...
    auto body1 = std::make_unique<VisitorDataSource>(visitor, 1);
    int submit_result
        = session.SubmitResponse(1, ToHeaders({ { ":status", "404" }, { "x-comment", "I have no idea what you're talking about." } }), std::move(body1), false);
...
TEST(OgHttp2SessionTest, ServerSendsTrailers)
{
    ...
    auto body1 = std::make_unique<VisitorDataSource>(visitor, 1);
    int submit_result = session.SubmitResponse(1, ToHeaders({ { ":status", "200" }, { "x-comment", "Sure, sounds good." } }), std::move(body1), false);
    EXPECT_EQ(submit_result, 0);
    EXPECT_TRUE(session.want_write());
    ...
    int trailer_result = session.SubmitTrailer(1, ToHeaders({ { "final-status", "a-ok" }, { "x-comment", "trailers sure are cool" } }));
...
TEST(OgHttp2SessionTest, ServerQueuesTrailersWithResponse)
{
    ...
    send_result = session.Send();
    EXPECT_EQ(0, send_result);
    EXPECT_THAT(visitor.data(), EqualsFrames({ SpdyFrameType::HEADERS, SpdyFrameType::DATA, SpdyFrameType::HEADERS }));
}
...
TEST(OgHttp2SessionTest, ServerSeesErrorOnEndStream)
{
    ...
    const std::string frames = TestFrameSequence()
                                   .ClientPreface()
                                   .Headers(1, { { ":method", "POST" }, { ":scheme", "https" }, { ":authority", "example.com" }, { ":path", "/" } },
                                       /*fin=*/false)
    ...
    EXPECT_CALL(visitor, OnConnectionError(Http2VisitorInterface::ConnectionError::kParseError));
    ...
    EXPECT_CALL(visitor, OnFrameSent(GOAWAY, 0, _, 0x0, static_cast<int>(Http2VisitorInterface::ConnectionError::kParseError)));
