...
Http2FrameBuilder::Http2FrameBuilder(Http2FrameType type, uint8_t flags, uint32_t stream_id)
{
...
void Http2FrameBuilder::AppendUInt31(uint32_t value)
{
    ...
    EXPECT_EQ(value, value & StreamIdMask()) << "High-bit of uint32_t should be clear.";
    value = htonl(tmp);
    AppendBytes(&value, 4);
}
...
void Http2FrameBuilder::WriteBytesAt(const void* data, uint32_t num_bytes, size_t offset)
{
