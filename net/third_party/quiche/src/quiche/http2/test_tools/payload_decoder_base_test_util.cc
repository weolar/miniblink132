...
DecodeStatus PayloadDecoderBaseTest::StartDecoding(DecodeBuffer* db)
{
    ...
    if (db->Remaining() > frame_header_.payload_length) {
        ADD_FAILURE() << "DecodeBuffer has too much data: " << db->Remaining() << " > " << frame_header_.payload_length;
        return DecodeStatus::kDecodeError;
    }
    ...
    // method is called.
    FrameDecoderStatePeer::set_frame_header(frame_header_, frame_decoder_state_.get());
...
::testing::AssertionResult PayloadDecoderBaseTest::DecodePayloadAndValidateSeveralWays(absl::string_view payload, Validator validator)
{
