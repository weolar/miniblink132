...
public:
    ...
    HpackEntryCollector(HpackEntryType type, size_t index, bool value_huffman, const std::string& value);
    HpackEntryCollector(HpackEntryType type, bool name_huffman, const std::string& name, bool value_huffman, const std::string& value);

    ~HpackEntryCollector() override;

    // Methods defined by HpackEntryDecoderListener.
    void OnIndexedHeader(size_t index) override;
    void OnStartLiteralHeader(HpackEntryType header_type, size_t maybe_name_index) override;
    ...
    // value.
    ::testing::AssertionResult ValidateLiteralNameValueHeader(HpackEntryType expected_type, bool expected_name_huffman, absl::string_view expected_name,
    ...
    ::testing::AssertionResult ValidateDynamicTableSizeUpdate(size_t expected_size) const;

    void set_header_type(HpackEntryType v)
    {
        header_type_ = v;
    }
    HpackEntryType header_type() const
    {
        return header_type_;
    }
    ...
    void set_name(const HpackStringCollector& v)
    {
        name_ = v;
    }
    const HpackStringCollector& name() const
    {
        return name_;
    }

    void set_value(const HpackStringCollector& v)
    {
        value_ = v;
    }
    const HpackStringCollector& value() const
    {
        return value_;
    }
...
QUICHE_NO_EXPORT bool operator==(const HpackEntryCollector& a, const HpackEntryCollector& b);
QUICHE_NO_EXPORT bool operator!=(const HpackEntryCollector& a, const HpackEntryCollector& b);
QUICHE_NO_EXPORT std::ostream& operator<<(std::ostream& out, const HpackEntryCollector& v);
