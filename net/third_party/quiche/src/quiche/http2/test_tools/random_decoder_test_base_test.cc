...
TEST_F(RandomDecoderTestTest, InProgressWhenEmpty)
{
    ...
    EXPECT_EQ(DecodeStatus::kDecodeInProgress, DecodeSegments(&data_db_, SelectRandom(kMayReturnZeroOnFirst)));
...
TEST_F(RandomDecoderTestTest, DecodeSeveralWaysToEnd)
{
    ...
    EXPECT_TRUE(DecodeAndValidateSeveralWays(&data_db_, kMayReturnZeroOnFirst, validator));
...
TEST_F(RandomDecoderTestTest, DecodeTwoWaysAndStopEarly)
{
    ...
    Validator validator = [this](const DecodeBuffer& /*input*/, DecodeStatus status) -> AssertionResult {
        if (start_decoding_calls_ <= 2 && status != DecodeStatus::kDecodeDone) {
            return ::testing::AssertionFailure() << "Expected DecodeStatus::kDecodeDone, not " << status;
        }
        if (start_decoding_calls_ > 2) {
            return ::testing::AssertionFailure() << "How did we get to pass " << start_decoding_calls_;
        }
        return ::testing::AssertionSuccess();
    };

    EXPECT_FALSE(DecodeAndValidateSeveralWays(&data_db_, kMayReturnZeroOnFirst, validator));
...
TEST_F(RandomDecoderTestTest, DecodeThreeWaysAndError)
{
    ...
    Validator validator = ValidateDoneAndEmpty();
    EXPECT_FALSE(DecodeAndValidateSeveralWays(&data_db_, kReturnNonZeroOnFirst, validator));
