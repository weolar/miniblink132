...
public:
    ...
    // For use in tests where the expected frame has a variable size payload
    // and may be padded.
    FrameParts(const Http2FrameHeader& header, absl::string_view payload, size_t total_pad_length);
    ...
    const Http2FrameHeader& GetFrameHeader() const
    {
    ...
    std::optional<Http2PingFields> GetOptPing() const
    {
        return opt_ping_;
    }
    std::optional<Http2GoAwayFields> GetOptGoaway() const
    {
        return opt_goaway_;
    }
    std::optional<size_t> GetOptPadLength() const
    {
    ...
    bool GetHasFrameSizeError() const
    {
        return has_frame_size_error_;
    }
    ...
    void SetOptRstStreamErrorCode(std::optional<Http2ErrorCode> opt_rst_stream_error_code)
    {
        opt_rst_stream_error_code_ = opt_rst_stream_error_code;
    }
    void SetOptPushPromise(std::optional<Http2PushPromiseFields> opt_push_promise)
    {
    ...
    void SetOptAltsvcOriginLength(std::optional<size_t> opt_altsvc_origin_length)
    {
    ...
    void SetOptWindowUpdateIncrement(std::optional<size_t> opt_window_update_increment)
    {
        opt_window_update_increment_ = opt_window_update_increment;
    }
    void SetOptPriorityUpdate(std::optional<Http2PriorityUpdateFields> priority_update)
    {
...
private:
    ...
    ::testing::AssertionResult StartFrameOfType(const Http2FrameHeader& header, Http2FrameType expected_frame_type);
    ...
    ::testing::AssertionResult AppendString(absl::string_view source, std::string* target, std::optional<size_t>* opt_length);
...
QUICHE_NO_EXPORT std::ostream& operator<<(std::ostream& out, const FrameParts& v);
