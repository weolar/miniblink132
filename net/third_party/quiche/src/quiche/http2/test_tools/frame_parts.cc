...
static_assert(std::is_base_of<Http2FrameDecoderListener, FrameParts>::value && !std::is_abstract<FrameParts>::value,
...
template <class T> AssertionResult VerifyOptionalEq(const T& opt_a, const T& opt_b)
{
    if (opt_a) {
        if (opt_b) {
            HTTP2_VERIFY_EQ(opt_a.value(), opt_b.value());
        } else {
            return AssertionFailure() << "opt_b is not set; opt_a.value()=" << opt_a.value();
        }
    } else if (opt_b) {
        return AssertionFailure() << "opt_a is not set; opt_b.value()=" << opt_b.value();
    }
...
FrameParts::FrameParts(const Http2FrameHeader& header, absl::string_view payload)
    : FrameParts(header)
{
...
FrameParts::FrameParts(const Http2FrameHeader& header, absl::string_view payload, size_t total_pad_length)
...
#define HTTP2_VERIFY_OPTIONAL_FIELD(field_name) HTTP2_VERIFY_SUCCESS(VerifyOptionalEq(field_name, that.field_name))
...
void FrameParts::SetTotalPadLength(size_t total_pad_length)
{
    ...
    if (opt_pad_length_) {
        QUICHE_VLOG(1) << "SetTotalPadLength: pad_length=" << opt_pad_length_.value();
    } else {
        QUICHE_VLOG(1) << "SetTotalPadLength: has no pad length";
    }
}

void FrameParts::SetAltSvcExpected(absl::string_view origin, absl::string_view value)
{
...
{
    QUICHE_VLOG(1) << "OnDataPayload: len=" << len << "; frame_header_: " << frame_header_;
    ASSERT_TRUE(InFrameOfType(Http2FrameType::DATA)) << *this;
    ASSERT_TRUE(AppendString(absl::string_view(data, len), &payload_, &opt_payload_length_));
}
...
{
    QUICHE_VLOG(1) << "OnHeadersPriority: priority: " << priority << "; frame_header_: " << frame_header_;
    ...
    opt_payload_length_ = opt_payload_length_.value() - Http2PriorityFields::EncodedSize();
}

void FrameParts::OnHpackFragment(const char* data, size_t len)
{
    QUICHE_VLOG(1) << "OnHpackFragment: len=" << len << "; frame_header_: " << frame_header_;
...
void FrameParts::OnPriorityFrame(const Http2FrameHeader& header, const Http2PriorityFields& priority)
{
...
void FrameParts::OnPadding(const char* pad, size_t skipped_length)
{
    ...
    ASSERT_TRUE(AppendString(absl::string_view(pad, skipped_length), &padding_, &opt_pad_length_));
}

void FrameParts::OnRstStream(const Http2FrameHeader& header, Http2ErrorCode error_code)
{
...
void FrameParts::OnPushPromiseStart(const Http2FrameHeader& header, const Http2PushPromiseFields& promise, size_t total_padding_length)
{
    QUICHE_VLOG(1) << "OnPushPromiseStart header: " << header << "; promise: " << promise << "; total_padding_length: " << total_padding_length;
    ...
    opt_payload_length_ = header.payload_length - Http2PushPromiseFields::EncodedSize();
...
void FrameParts::OnPing(const Http2FrameHeader& header, const Http2PingFields& ping)
{
...
void FrameParts::OnPingAck(const Http2FrameHeader& header, const Http2PingFields& ping)
{
...
void FrameParts::OnGoAwayStart(const Http2FrameHeader& header, const Http2GoAwayFields& goaway)
{
    QUICHE_VLOG(1) << "OnGoAwayStart: " << goaway;
    ASSERT_TRUE(StartFrameOfType(header, Http2FrameType::GOAWAY)) << *this;
    ASSERT_FALSE(opt_goaway_);
    opt_goaway_ = goaway;
    opt_payload_length_ = header.payload_length - Http2GoAwayFields::EncodedSize();
}
...
void FrameParts::OnWindowUpdate(const Http2FrameHeader& header, uint32_t increment)
{
    QUICHE_VLOG(1) << "OnWindowUpdate header: " << header << "     increment=" << increment;
...
void FrameParts::OnAltSvcStart(const Http2FrameHeader& header, size_t origin_length, size_t value_length)
{
...
void FrameParts::OnAltSvcOriginData(const char* data, size_t len)
{
    ...
    ASSERT_TRUE(AppendString(absl::string_view(data, len), &altsvc_origin_, &opt_altsvc_origin_length_));
}
...
void FrameParts::OnAltSvcValueData(const char* data, size_t len)
{
    ...
    ASSERT_TRUE(InFrameOfType(Http2FrameType::ALTSVC)) << *this;
    ASSERT_TRUE(AppendString(absl::string_view(data, len), &altsvc_value_, &opt_altsvc_value_length_));
}
...
void FrameParts::OnPriorityUpdateStart(const Http2FrameHeader& header, const Http2PriorityUpdateFields& priority_update)
{
    QUICHE_VLOG(1) << "OnPriorityUpdateStart: " << header << "    prioritized_stream_id: " << priority_update.prioritized_stream_id;
    ASSERT_TRUE(StartFrameOfType(header, Http2FrameType::PRIORITY_UPDATE)) << *this;
    ASSERT_FALSE(opt_priority_update_);
    opt_priority_update_ = priority_update;
    opt_payload_length_ = header.payload_length - Http2PriorityUpdateFields::EncodedSize();
}
...
void FrameParts::OnPaddingTooLong(const Http2FrameHeader& header, size_t missing_length)
{
...
AssertionResult FrameParts::StartFrameOfType(const Http2FrameHeader& header, Http2FrameType expected_frame_type)
{
...
AssertionResult FrameParts::AppendString(absl::string_view source, std::string* target, std::optional<size_t>* opt_length)
{
    target->append(source.data(), source.size());
    if (opt_length != nullptr) {
        HTTP2_VERIFY_TRUE(*opt_length) << "Length is not set yet\n" << *this;
        HTTP2_VERIFY_LE(target->size(), opt_length->value()) << "String too large; source.size() = " << source.size() << "\n" << *this;
    }
