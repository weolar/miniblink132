...
protected:
    ...
    FrameDecoderState* mutable_state()
    {
        return frame_decoder_state_.get();
    }
    ...
    ::testing::AssertionResult DecodePayloadAndValidateSeveralWays(absl::string_view payload, Validator validator);
...
template <class Decoder, class DecoderPeer, class Listener, bool SupportedFrameType = true>
class QUICHE_NO_EXPORT AbstractPayloadDecoderTest : public PayloadDecoderBaseTest {
protected:
    ...
    uint8_t RandFlags()
    {
        return Random().Rand8() & KnownFlagsMaskForFrameType(DecoderPeer::FrameType()) & ~DecoderPeer::FlagsAffectingPayloadDecoding();
    }

    // Start decoding the payload.
    DecodeStatus StartDecodingPayload(DecodeBuffer* db) override
    {
        QUICHE_DVLOG(2) << "StartDecodingPayload, db->Remaining=" << db->Remaining();
    ...
    {
        QUICHE_DVLOG(2) << "ResumeDecodingPayload, db->Remaining=" << db->Remaining();
    ...
    AssertionResult DecodePayloadAndValidateSeveralWays(absl::string_view payload, const FrameParts& expected)
    {
        ...
        return PayloadDecoderBaseTest::DecodePayloadAndValidateSeveralWays(payload, ValidateDoneAndEmpty(validator));
    }
    ...
    ::testing::AssertionResult VerifyDetectsFrameSizeError(absl::string_view payload, const Http2FrameHeader& header, WrappedValidator wrapped_validator)
    {
        ...
        Validator validator
            = [header, validator = ToValidator(wrapped_validator), this](const DecodeBuffer& input, DecodeStatus status) -> ::testing::AssertionResult {
            QUICHE_DVLOG(2) << "VerifyDetectsFrameSizeError validator; status=" << status << "; input.Remaining=" << input.Remaining();
        ...
        return PayloadDecoderBaseTest::DecodePayloadAndValidateSeveralWays(payload, std::move(validator));
    }
    ...
    ::testing::AssertionResult VerifyDetectsMultipleFrameSizeErrors(
        uint8_t required_flags, absl::string_view unpadded_payload, ApproveSize approve_size, int total_pad_length)
    {
        ...
        HTTP2_VERIFY_EQ(required_flags, required_flags & KnownFlagsMaskForFrameType(frame_type));
        HTTP2_VERIFY_EQ(required_flags, required_flags & DecoderPeer::FlagsAffectingPayloadDecoding());

        if (0 != (Http2FrameFlag::PADDED & KnownFlagsMaskForFrameType(frame_type))) {
        ...
        for (size_t real_payload_size = 0; real_payload_size <= unpadded_payload.size(); ++real_payload_size) {
            ...
            HTTP2_VERIFY_SUCCESS(VerifyDetectsFrameSizeError(fb.buffer(), header, nullptr));
            validated = true;
        }
    ...
    ::testing::AssertionResult VerifyDetectsFrameSizeError(uint8_t required_flags, absl::string_view unpadded_payload, ApproveSize approve_size)
    {
        Http2FrameType frame_type = DecoderPeer::FrameType();
        ...
        return VerifyDetectsMultipleFrameSizeErrors(required_flags, unpadded_payload, std::move(approve_size), 0);
    }
...
class QUICHE_NO_EXPORT AbstractPaddablePayloadDecoderTest : public AbstractPayloadDecoderTest<Decoder, DecoderPeer, Listener>,
...
protected:
    ...
    AbstractPaddablePayloadDecoderTest()
    ...
    ::testing::AssertionResult VerifyDetectsPaddingTooLong(absl::string_view payload, const Http2FrameHeader& header, size_t expected_missing_length)
    {
        set_frame_header(header);
        auto& listener = listener_;
        Validator validator = [header, expected_missing_length, &listener](const DecodeBuffer&, DecodeStatus status) -> ::testing::AssertionResult {
            HTTP2_VERIFY_EQ(DecodeStatus::kDecodeError, status);
            ...
            HTTP2_VERIFY_EQ(expected_missing_length, frame->GetOptMissingLength().value());
    ...
    ::testing::AssertionResult VerifyDetectsPaddingTooLong()
    {
        ...
        const Http2FrameHeader header(payload_length, DecoderPeer::FrameType(), flags, RandStreamId());
