...
template <typename T, size_t N> absl::string_view ToStringPiece(T (&data)[N])
{
...
template <typename T, typename E = typename std::enable_if<std::is_enum<T>::value>::type> void CorruptEnum(T* out, Http2Random* rng)
{
    ...
    using unsigned_underlying_type_T = typename std::make_unsigned<underlying_type_T>::type;
...
public:
    ...
    // parameter.
    using SelectSize = quiche::MultiUseCallback<size_t(bool first, size_t offset, size_t remaining)>;
    ...
    using Validator = quiche::MultiUseCallback<AssertionResult(const DecodeBuffer& input, DecodeStatus status)>;
...
protected:
    ...
    DecodeStatus DecodeSegments(DecodeBuffer* original, const SelectSize& select_size);
    ...
    ::testing::AssertionResult DecodeSegmentsAndValidate(DecodeBuffer* original, const SelectSize& select_size, const Validator& validator)
    {
    ...
    {
        return [](bool /*first*/, size_t /*offset*/, size_t remaining) -> size_t { return remaining; };
    }

    // Returns a SelectSize function for decoding a single byte at a time.
    static SelectSize SelectOne()
    {
        return [](bool /*first*/, size_t /*offset*/, size_t /*remaining*/) -> size_t { return 1; };
    }
    ...
    // success.
    AssertionResult DecodeAndValidateSeveralWays(DecodeBuffer* original, bool return_non_zero_on_first, const Validator& validator);

    static Validator ToValidator(std::nullptr_t)
    {
        return [](const DecodeBuffer& /*input*/, DecodeStatus /*status*/) { return ::testing::AssertionSuccess(); };
    }
    ...
    static Validator ToValidator(NoArgValidator validator)
    {
        ...
        return [validator = std::move(validator)](const DecodeBuffer& /*input*/, DecodeStatus /*status*/) { return validator(); };
    }
    ...
    {
        return [wrapped = std::move(wrapped)](const DecodeBuffer& input, DecodeStatus status) -> AssertionResult {
    ...
    {
        return [wrapped = std::move(wrapped), offset](const DecodeBuffer& input, DecodeStatus status) -> AssertionResult {
            HTTP2_VERIFY_EQ(status, DecodeStatus::kDecodeDone);
    ...
    static Validator ValidateDoneAndOffset(uint32_t offset, NoArgValidator wrapped)
    {
        return [wrapped = std::move(wrapped), offset](const DecodeBuffer& input, DecodeStatus status) -> AssertionResult {
            HTTP2_VERIFY_EQ(status, DecodeStatus::kDecodeDone);
            HTTP2_VERIFY_EQ(offset, input.Offset()) << "\nRemaining=" << input.Remaining();
