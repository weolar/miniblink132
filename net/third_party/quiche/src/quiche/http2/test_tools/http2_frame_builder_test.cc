...
TEST(Http2FrameBuilderTest, Constructors)
{
    ...
    }
    {
        ...
        ASSERT_TRUE(absl::HexStringToBytes("000000" // Payload length: 0 (unset)
                                           "00" // Frame type: DATA
                                           "00" // Flags: none
    ...
    }
    {
        ...
        std::string expected_data;
        ASSERT_TRUE(absl::HexStringToBytes("ffffff" // Payload length: 2^24 - 1 (max uint24)
...
TEST(Http2FrameBuilderTest, SetPayloadLength)
{
    ...
    std::string expected_data;
    ASSERT_TRUE(absl::HexStringToBytes("00003d" // Payload length: 61
                                       "00" // Frame type: DATA
                                       "08" // Flags: PADDED
...
TEST(Http2FrameBuilderTest, Settings)
{
    ...
    std::string expected_data;
    ASSERT_TRUE(absl::HexStringToBytes("000024" // Payload length: 36
...
TEST(Http2FrameBuilderTest, PushPromise)
{
    ...
    }
    {
        ...
        EXPECT_NONFATAL_FAILURE(fb.Append(Http2PushPromiseFields { 0xffffffff }), kHighBitSetMsg);
        EXPECT_EQ(expected_data, fb.buffer());
    }
}
...
{
    std::string expected_data;
    ASSERT_TRUE(absl::HexStringToBytes("12345678" // Last Stream Id
                                       "00000001", // Error code
        &expected_data));
...
TEST(Http2FrameBuilderTest, WindowUpdate)
{
    ...
    EXPECT_NONFATAL_FAILURE(fb.Append(Http2WindowUpdateFields { 0x80000001 }), kHighBitSetMsg);
    ...
    std::string expected_data;
    ASSERT_TRUE(absl::HexStringToBytes("0001e240" // Valid Window Size Increment
...
TEST(Http2FrameBuilderTest, AltSvc)
{
    ...
    std::string expected_data;
    ASSERT_TRUE(absl::HexStringToBytes("0063" // Has origin.
