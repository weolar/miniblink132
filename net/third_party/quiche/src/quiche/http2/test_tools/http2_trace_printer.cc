...
Http2TracePrinter::Http2TracePrinter(absl::string_view perspective, const void* connection_id, bool consume_connection_preface)
    : logger_(&visitor_, perspective, IsLoggingEnabled, connection_id)
    , perspective_(perspective)
{
    decoder_.set_visitor(&logger_);
    if (consume_connection_preface) {
        remaining_preface_ = absl::string_view(spdy::kHttp2ConnectionHeaderPrefix, spdy::kHttp2ConnectionHeaderPrefixSize);
    }
}
...
{
    if (preface_error_) {
        HTTP2_TRACE_LOG(perspective_, IsLoggingEnabled) << "Earlier connection preface error, ignoring " << bytes.size() << " bytes";
        return;
    }
    if (!remaining_preface_.empty()) {
        ...
        HTTP2_TRACE_LOG(perspective_, IsLoggingEnabled) << "Received connection preface: " << absl::CEscape(preface);

        if (!absl::StartsWith(remaining_preface_, preface)) {
            HTTP2_TRACE_LOG(perspective_, IsLoggingEnabled)
                << "Received preface does not match expected remaining preface: " << absl::CEscape(remaining_preface_);
            preface_error_ = true;
            return;
        }
