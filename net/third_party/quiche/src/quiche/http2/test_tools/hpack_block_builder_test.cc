...
TEST(HpackBlockBuilderTest, ExamplesFromSpecC2)
{
    {
        HpackBlockBuilder b;
        b.AppendLiteralNameAndValue(HpackEntryType::kIndexedLiteralHeader, kUncompressed, "custom-key", kUncompressed, "custom-header");
        EXPECT_EQ(26u, b.size());

        const char kExpected[] = "\x40" // == Literal indexed ==
                                 "\x0a" // Name length (10)
                                 "custom-key" // Name
                                 "\x0d" // Value length (13)
                                 "custom-header"; // Value
    ...
    {
        HpackBlockBuilder b;
        b.AppendNameIndexAndLiteralValue(HpackEntryType::kUnindexedLiteralHeader, 4, kUncompressed, "/sample/path");
        EXPECT_EQ(14u, b.size());

        const char kExpected[] = "\x04" // == Literal unindexed, name index 0x04 ==
                                 "\x0c" // Value length (12)
                                 "/sample/path"; // Value
        EXPECT_EQ(kExpected, b.buffer());
    }
    {
        HpackBlockBuilder b;
        b.AppendLiteralNameAndValue(HpackEntryType::kNeverIndexedLiteralHeader, kUncompressed, "password", kUncompressed, "secret");
        EXPECT_EQ(17u, b.size());

        const char kExpected[] = "\x10" // == Literal never indexed ==
                                 "\x08" // Name length (8)
                                 "password" // Name
                                 "\x06" // Value length (6)
                                 "secret"; // Value
...
{
    {
        ...
        b.AppendNameIndexAndLiteralValue(HpackEntryType::kIndexedLiteralHeader, 1, kUncompressed, "www.example.com");
        ...
        std::string expected;
        ASSERT_TRUE(absl::HexStringToBytes("828684410f7777772e6578616d706c652e636f6d", &expected));
...
{
    {
        ...
        const char kHuffmanWwwExampleCom[] = { '\xf1', '\xe3', '\xc2', '\xe5', '\xf2', '\x3a', '\x6b', '\xa0', '\xab', '\x90', '\xf4', '\xff' };
        b.AppendNameIndexAndLiteralValue(
            HpackEntryType::kIndexedLiteralHeader, 1, kCompressed, absl::string_view(kHuffmanWwwExampleCom, sizeof kHuffmanWwwExampleCom));
        EXPECT_EQ(17u, b.size());
        ...
        std::string expected;
        ASSERT_TRUE(absl::HexStringToBytes("828684418cf1e3c2e5f23a6ba0ab90f4ff", &expected));
        EXPECT_EQ(expected, b.buffer());
    }
}
...
TEST(HpackBlockBuilderTest, DynamicTableSizeUpdate)
{
    ...
    }
    {
        ...
        const char kData[] = { '\x3f', '\xe1', '\x9f', '\x94', '\xa5', '\x8d', '\x1d' };
        absl::string_view expected(kData, sizeof kData);
        EXPECT_EQ(expected, b.buffer());
    }
}
