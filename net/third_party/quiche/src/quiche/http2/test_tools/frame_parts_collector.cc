...
void FramePartsCollector::ExpectFrameHeader(const Http2FrameHeader& header)
{
    ...
    EXPECT_FALSE(expected_header_set_) << "expected_header_: " << expected_header_;
...
Http2FrameDecoderListener* FramePartsCollector::StartFrame(const Http2FrameHeader& header)
{
...
Http2FrameDecoderListener* FramePartsCollector::StartAndEndFrame(const Http2FrameHeader& header)
{
    TestExpectedHeader(header);
    EXPECT_FALSE(IsInProgress());
    if (current_frame_ == nullptr) {
...
Http2FrameDecoderListener* FramePartsCollector::FrameError(const Http2FrameHeader& header)
{
    TestExpectedHeader(header);
    if (current_frame_ == nullptr) {
