...
{
    return absl::BytesToHexString(absl::string_view(reinterpret_cast<const char*>(key_), sizeof(key_)));
}
...
void Http2Random::FillRandom(void* buffer, size_t buffer_size)
{
    ...
    CRYPTO_chacha_20(buffer_u8, buffer_u8, buffer_size, key_, kZeroNonce, counter_++);
}
...
std::string Http2Random::RandStringWithAlphabet(int length, absl::string_view alphabet)
{
    std::string result;
    result.resize(length);
