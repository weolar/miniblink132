...
inline bool operator==(absl::string_view x, const quiche::HttpHeaderBlock::ValueProxy& y)
{
    return y.operator==(x);
}
...
std::string HexDumpWithMarks(const unsigned char* data, int length, const bool* marks, int mark_length);

void CompareCharArraysWithHexError(
    const std::string& description, const unsigned char* actual, const int actual_len, const unsigned char* expected, const int expected_len);
