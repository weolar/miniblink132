...
public:
    ...
    //
    template <class S> bool Start(S* out, DecodeBuffer* db)
    {
        static_assert(S::EncodedSize() <= sizeof buffer_, "buffer_ is too small");
        QUICHE_DVLOG(2) << __func__ << "@" << this << ": db->Remaining=" << db->Remaining() << "; EncodedSize=" << S::EncodedSize();
    ...
    template <class S> bool Resume(S* out, DecodeBuffer* db)
    {
        QUICHE_DVLOG(2) << __func__ << "@" << this << ": offset_=" << offset_ << "; db->Remaining=" << db->Remaining();
        if (ResumeFillingBuffer(db, S::EncodedSize())) {
            // We have the whole thing now.
            QUICHE_DVLOG(2) << __func__ << "@" << this << "    offset_=" << offset_ << "    Ready to decode from buffer_.";
    ...
    template <class S> DecodeStatus Start(S* out, DecodeBuffer* db, uint32_t* remaining_payload)
    {
        static_assert(S::EncodedSize() <= sizeof buffer_, "buffer_ is too small");
        QUICHE_DVLOG(2) << __func__ << "@" << this << ": *remaining_payload=" << *remaining_payload << "; db->Remaining=" << db->Remaining()
    ...
    template <class S> bool Resume(S* out, DecodeBuffer* db, uint32_t* remaining_payload)
    {
        QUICHE_DVLOG(3) << __func__ << "@" << this << ": offset_=" << offset_ << "; *remaining_payload=" << *remaining_payload
                        << "; db->Remaining=" << db->Remaining() << "; EncodedSize=" << S::EncodedSize();
...
private:
    ...
    DecodeStatus IncompleteStart(DecodeBuffer* db, uint32_t* remaining_payload, uint32_t target_size);

    bool ResumeFillingBuffer(DecodeBuffer* db, uint32_t target_size);
    bool ResumeFillingBuffer(DecodeBuffer* db, uint32_t* remaining_payload, uint32_t target_size);
