...
template <class S> std::string SerializeStructure(const S& s)
{
...
template <class S> class StructureDecoderTest : public quiche::test::QuicheTest {
protected:
    typedef S Structure;

    StructureDecoderTest()
        : random_()
        , random_decode_count_(100)
    {
    }
    ...
    void Randomize(S* p)
    {
        ::http2::test::Randomize(p, &random_);
    }
    ...
    template <size_t N> void DecodeLeadingStructure(const char (&data)[N])
    {
...
class PriorityFieldsDecoderTest : public StructureDecoderTest<Http2PriorityFields> { };
...
TEST_F(PriorityFieldsDecoderTest, DecodesLiteral)
{
    ...
    {
        const char kData[] = {
            '\x7f', '\xff', '\xff', '\xff', // Exclusive (no) and Dependency (0x7fffffff)
...
class RstStreamFieldsDecoderTest : public StructureDecoderTest<Http2RstStreamFields> { };
...
class SettingFieldsDecoderTest : public StructureDecoderTest<Http2SettingFields> { };
...
class PushPromiseFieldsDecoderTest : public StructureDecoderTest<Http2PushPromiseFields> { };
...
{
    {
        ...
        DecodeLeadingStructure(kData);
        if (!HasFailure()) {
            EXPECT_EQ(absl::string_view(kData, 8), ToStringPiece(structure_.opaque_bytes));
        }
    }
    {
...
TEST_F(GoAwayFieldsDecoderTest, DecodesLiteral)
{
    ...
    {
        const char kData[] = {
            '\xff', '\xff', '\xff', '\xff', // Last Stream ID: max uint31 and R-bit
            '\xff', '\xff', '\xff', '\xff', // Error: max uint32 (Unknown error code)
        };
        DecodeLeadingStructure(kData);
        if (!HasFailure()) {
...
class WindowUpdateFieldsDecoderTest : public StructureDecoderTest<Http2WindowUpdateFields> { };
