...
protected:
    ...
    void ConfirmDiscardsRemainingPayload()
    {
        ...
        size_t remaining = Http2FrameDecoderPeer::remaining_total_payload(decoder_.get());
    ...
    AssertionResult DecodePayloadAndValidateSeveralWays(absl::string_view payload, Validator validator)
    {
        DecodeBuffer db(payload);
        bool start_decoding_requires_non_empty = false;
        return DecodeAndValidateSeveralWays(&db, start_decoding_requires_non_empty, validator);
    }
    ...
    AssertionResult DecodePayloadAndValidateSeveralWays(absl::string_view payload, const FrameParts& expected)
    {
        auto validator = [&expected, this](const DecodeBuffer& /*input*/, DecodeStatus status) -> AssertionResult {
            HTTP2_VERIFY_EQ(status, DecodeStatus::kDecodeDone);
            return VerifyCollected(expected);
        };
        ResetDecodeSpeedCounters();
        HTTP2_VERIFY_SUCCESS(DecodePayloadAndValidateSeveralWays(payload, ValidateDoneAndEmpty(validator)));
        ...
        ResetDecodeSpeedCounters();
        HTTP2_VERIFY_SUCCESS(DecodePayloadAndValidateSeveralWays(payload, ValidateDoneAndOffset(payload.size(), validator)));
        HTTP2_VERIFY_GT(fast_decode_count_, 0u);
        HTTP2_VERIFY_GT(slow_decode_count_, 0u);
    ...
    template <size_t N> AssertionResult DecodePayloadAndValidateSeveralWays(const char (&buf)[N], const FrameParts& expected)
    {
        return DecodePayloadAndValidateSeveralWays(absl::string_view(buf, N), expected);
    }

    template <size_t N> AssertionResult DecodePayloadAndValidateSeveralWays(const char (&buf)[N], const Http2FrameHeader& header)
    {
        return DecodePayloadAndValidateSeveralWays(absl::string_view(buf, N), FrameParts(header));
    }

    template <size_t N> AssertionResult DecodePayloadExpectingError(const char (&buf)[N], const FrameParts& expected)
    {
        auto validator = [&expected, this](const DecodeBuffer& /*input*/, DecodeStatus status) -> AssertionResult {
        ...
        ResetDecodeSpeedCounters();
        EXPECT_TRUE(DecodePayloadAndValidateSeveralWays(ToStringPiece(buf), validator));
        EXPECT_GT(fast_decode_count_, 0u);
        EXPECT_GT(slow_decode_count_, 0u);
        return AssertionSuccess();
    }

    template <size_t N> AssertionResult DecodePayloadExpectingFrameSizeError(const char (&buf)[N], FrameParts expected)
    {
        expected.SetHasFrameSizeError(true);
        return DecodePayloadExpectingError(buf, expected);
    }

    template <size_t N> AssertionResult DecodePayloadExpectingFrameSizeError(const char (&buf)[N], const Http2FrameHeader& header)
    {
...
TEST_F(Http2FrameDecoderTest, PushPromiseMinimal)
{
    ...
    Http2FrameHeader header(4, Http2FrameType::PUSH_PROMISE, Http2FrameFlag::END_HEADERS, 2);
    FrameParts expected(header, "");
...
TEST_F(Http2FrameDecoderTest, Ping)
{
    ...
    FrameParts expected(header);
    expected.SetOptPing(Http2PingFields { { 's', 'o', 'm', 'e', 'd', 'a', 't', 'a' } });
    EXPECT_TRUE(DecodePayloadAndValidateSeveralWays(kFrameData, expected));
}
...
TEST_F(Http2FrameDecoderTest, GoAwayMinimal)
{
    ...
    FrameParts expected(header);
    expected.SetOptGoaway(Http2GoAwayFields(255, Http2ErrorCode::COMPRESSION_ERROR));
    EXPECT_TRUE(DecodePayloadAndValidateSeveralWays(kFrameData, expected));
}
...
TEST_F(Http2FrameDecoderTest, HeadersPayload)
{
    ...
    Http2FrameHeader header(3, Http2FrameType::HEADERS, Http2FrameFlag::END_STREAM | Http2FrameFlag::END_HEADERS, 2);
    FrameParts expected(header, "abc");
    EXPECT_TRUE(DecodePayloadAndValidateSeveralWays(kFrameData, expected));
}
...
TEST_F(Http2FrameDecoderTest, HeadersPriority)
{
    ...
    Http2FrameHeader header(5, Http2FrameType::HEADERS, Http2FrameFlag::PRIORITY, 2);
    FrameParts expected(header);
...
TEST_F(Http2FrameDecoderTest, Settings)
{
    ...
    FrameParts expected(header);
    expected.AppendSetting(Http2SettingFields(Http2SettingsParameter::INITIAL_WINDOW_SIZE, 168496141));
    expected.AppendSetting(Http2SettingFields(Http2SettingsParameter::ENABLE_PUSH, 3));
    EXPECT_TRUE(DecodePayloadAndValidateSeveralWays(kFrameData, expected));
}
...
TEST_F(Http2FrameDecoderTest, PushPromisePayload)
{
    ...
    Http2FrameHeader header(7, Http2FrameType::PUSH_PROMISE, Http2FrameFlag::END_HEADERS, 255);
    FrameParts expected(header, "abc");
...
TEST_F(Http2FrameDecoderTest, GoAwayOpaqueData)
{
    ...
    expected.SetOptGoaway(Http2GoAwayFields(256, Http2ErrorCode::FLOW_CONTROL_ERROR));
    EXPECT_TRUE(DecodePayloadAndValidateSeveralWays(kFrameData, expected));
}
...
TEST_F(Http2FrameDecoderTest, ContinuationPayload)
{
    ...
    Http2FrameHeader header(3, Http2FrameType::CONTINUATION, Http2FrameFlag::END_HEADERS, 2);
    FrameParts expected(header, "abc");
    EXPECT_TRUE(DecodePayloadAndValidateSeveralWays(kFrameData, expected));
}
...
TEST_F(Http2FrameDecoderTest, DataPayloadAndPadding)
{
    ...
    Http2FrameHeader header(7, Http2FrameType::DATA, Http2FrameFlag::END_STREAM | Http2FrameFlag::PADDED, 2);
...
TEST_F(Http2FrameDecoderTest, HeadersPayloadAndPadding)
{
    ...
    Http2FrameHeader header(7, Http2FrameType::HEADERS, Http2FrameFlag::PADDED, 2);
    size_t total_pad_length = 4; // Including the Pad Length field.
    FrameParts expected(header, "abc", total_pad_length);
    EXPECT_TRUE(DecodePayloadAndValidateSeveralWays(kFrameData, expected));
}
...
TEST_F(Http2FrameDecoderTest, HeadersPayloadPriorityAndPadding)
{
    ...
    Http2FrameHeader header(
        12, Http2FrameType::HEADERS, Http2FrameFlag::END_STREAM | Http2FrameFlag::END_HEADERS | Http2FrameFlag::PADDED | Http2FrameFlag::PRIORITY, 2);
    size_t total_pad_length = 4; // Including the Pad Length field.
    FrameParts expected(header, "abc", total_pad_length);
    expected.SetOptPriority(Http2PriorityFields(1, 17, true));
    EXPECT_TRUE(DecodePayloadAndValidateSeveralWays(kFrameData, expected));
...
TEST_F(Http2FrameDecoderTest, PushPromisePayloadAndPadding)
{
    ...
    Http2FrameHeader header(11, Http2FrameType::PUSH_PROMISE, Http2FrameFlag::END_HEADERS | Http2FrameFlag::PADDED, 1);
    size_t total_pad_length = 4; // Including the Pad Length field.
    FrameParts expected(header, "abc", total_pad_length);
...
TEST_F(Http2FrameDecoderTest, HeaderPaddingTooLong)
{
    ...
    Http2FrameHeader header(2, Http2FrameType::HEADERS, Http2FrameFlag::PADDED, 65536);
    FrameParts expected(header);
...
TEST_F(Http2FrameDecoderTest, HeaderMissingPriority)
{
    ...
    Http2FrameHeader header(4, Http2FrameType::HEADERS, Http2FrameFlag::PRIORITY, 65536);
...
TEST_F(Http2FrameDecoderTest, PushPromisePaddedTruncatedPromise)
{
    ...
    Http2FrameHeader header(4, Http2FrameType::PUSH_PROMISE, Http2FrameFlag::PADDED, 1);
    EXPECT_TRUE(DecodePayloadExpectingFrameSizeError(kFrameData, header));
}
...
TEST_F(Http2FrameDecoderTest, BeyondMaximum)
{
    ...
    ResetDecodeSpeedCounters();
    EXPECT_TRUE(DecodePayloadAndValidateSeveralWays(ToStringPiece(kFrameData), validator));
    EXPECT_GT(fast_decode_count_, 0u);
    EXPECT_GT(slow_decode_count_, 0u);
}
