...
public:
    ...
    virtual void OnHeadersPriority(const Http2PriorityFields& priority_fields) = 0;
    ...
    virtual void OnPriorityFrame(const Http2FrameHeader& header, const Http2PriorityFields& priority_fields) = 0;
    ...
    virtual void OnRstStream(const Http2FrameHeader& header, Http2ErrorCode error_code) = 0;
    ...
    virtual void OnPushPromiseStart(const Http2FrameHeader& header, const Http2PushPromiseFields& promise, size_t total_padding_length) = 0;
    ...
    virtual void OnPing(const Http2FrameHeader& header, const Http2PingFields& ping) = 0;

    // Called when an entire PING frame, with the ACK flag, has been decoded.
    virtual void OnPingAck(const Http2FrameHeader& header, const Http2PingFields& ping) = 0;
    ...
    virtual void OnGoAwayStart(const Http2FrameHeader& header, const Http2GoAwayFields& goaway) = 0;
    ...
    // increased.
    virtual void OnWindowUpdate(const Http2FrameHeader& header, uint32_t window_size_increment) = 0;
    ...
    virtual void OnAltSvcStart(const Http2FrameHeader& header, size_t origin_length, size_t value_length) = 0;
    ...
    virtual void OnPriorityUpdateStart(const Http2FrameHeader& header, const Http2PriorityUpdateFields& priority_update) = 0;
    ...
    virtual void OnPaddingTooLong(const Http2FrameHeader& header, size_t missing_length) = 0;
...
class QUICHE_EXPORT Http2FrameDecoderNoOpListener : public Http2FrameDecoderListener {
public:
    ...
    void OnRstStream(const Http2FrameHeader& /*header*/, Http2ErrorCode /*error_code*/) override
    {
    }
...
static_assert(!std::is_abstract<Http2FrameDecoderNoOpListener>(), "Http2FrameDecoderNoOpListener ought to be concrete.");
