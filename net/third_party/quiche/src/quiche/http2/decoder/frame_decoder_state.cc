...
DecodeStatus FrameDecoderState::ReadPadLength(DecodeBuffer* db, bool report_pad_length)
{
    QUICHE_DVLOG(2) << "ReadPadLength db->Remaining=" << db->Remaining() << "; payload_length=" << frame_header().payload_length;
...
{
    QUICHE_DVLOG(2) << "SkipPadding remaining_padding_=" << remaining_padding_ << ", db->Remaining=" << db->Remaining() << ", header: " << frame_header();
    QUICHE_DCHECK_EQ(remaining_payload_, 0u);
    QUICHE_DCHECK(IsPaddable()) << "header: " << frame_header();
    QUICHE_DCHECK(remaining_padding_ == 0 || frame_header().IsPadded()) << "remaining_padding_=" << remaining_padding_ << ", header: " << frame_header();
...
DecodeStatus FrameDecoderState::ReportFrameSizeError()
{
    QUICHE_DVLOG(2) << "FrameDecoderState::ReportFrameSizeError: "
                    << " remaining_payload_=" << remaining_payload_ << "; remaining_padding_=" << remaining_padding_ << ", header: " << frame_header();
