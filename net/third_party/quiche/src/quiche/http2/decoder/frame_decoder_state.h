...
public:
    ...
    // remaining payload.
    template <class S> DecodeStatus StartDecodingStructureInPayload(S* out, DecodeBuffer* db)
    {
        QUICHE_DVLOG(2) << __func__ << "\n\tdb->Remaining=" << db->Remaining() << "\n\tremaining_payload_=" << remaining_payload_
                        << "\n\tneed=" << S::EncodedSize();
        DecodeStatus status = structure_decoder_.Start(out, db, &remaining_payload_);
        if (status != DecodeStatus::kDecodeError) {
            return status;
        }
        QUICHE_DVLOG(2) << "StartDecodingStructureInPayload: detected frame size error";
    ...
    template <class S> DecodeStatus ResumeDecodingStructureInPayload(S* out, DecodeBuffer* db)
    {
        QUICHE_DVLOG(2) << __func__ << "\n\tdb->Remaining=" << db->Remaining() << "\n\tremaining_payload_=" << remaining_payload_;
        if (structure_decoder_.Resume(out, db, &remaining_payload_)) {
            ...
            QUICHE_DVLOG(2) << "ResumeDecodingStructureInPayload: detected frame size error";
            return ReportFrameSizeError();
        }
    }
...
private:
    ...
    void ClearFlags()
    {
        frame_header_.flags = Http2FrameFlag();
    }

    // Returns true if the type of frame being decoded can have padding.
    bool IsPaddable() const
    {
        return frame_header().type == Http2FrameType::DATA || frame_header().type == Http2FrameType::HEADERS
