...
class PriorityPayloadDecoderTest : public AbstractPayloadDecoderTest<PriorityPayloadDecoder, PriorityPayloadDecoderPeer, Listener> {
protected:
...
TEST_F(PriorityPayloadDecoderTest, VariousPayloads)
{
    for (int n = 0; n < 100; ++n) {
        ...
        Http2FrameBuilder fb;
        fb.Append(fields);
        Http2FrameHeader header(fb.size(), Http2FrameType::PRIORITY, RandFlags(), RandStreamId());
