...
class RstStreamPayloadDecoderTest : public AbstractPayloadDecoderTest<RstStreamPayloadDecoder, RstStreamPayloadDecoderPeer, Listener> {
protected:
...
{
    auto approve_size = [](size_t size) { return size != Http2RstStreamFields::EncodedSize(); };
...
TEST_F(RstStreamPayloadDecoderTest, AllErrors)
{
    for (auto error_code : AllHttp2ErrorCodes()) {
        ...
        Http2FrameBuilder fb;
        fb.Append(fields);
        Http2FrameHeader header(fb.size(), Http2FrameType::RST_STREAM, RandFlags(), RandStreamId());
