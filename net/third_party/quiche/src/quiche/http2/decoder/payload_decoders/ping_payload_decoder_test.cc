...
class PingPayloadDecoderTest : public AbstractPayloadDecoderTest<PingPayloadDecoder, PingPayloadDecoderPeer, Listener> {
protected:
...
{
    auto approve_size = [](size_t size) { return size != Http2PingFields::EncodedSize(); };
...
TEST_F(PingPayloadDecoderTest, Ping)
{
    for (int n = 0; n < 100; ++n) {
        ...
        Http2FrameBuilder fb;
        fb.Append(fields);
        Http2FrameHeader header(fb.size(), Http2FrameType::PING, RandFlags() & ~Http2FrameFlag::ACK, RandStreamId());
