...
class PriorityUpdatePayloadDecoderTest : public AbstractPayloadDecoderTest<PriorityUpdatePayloadDecoder, PriorityUpdatePayloadDecoderPeer, Listener> { };
...
{
    auto approve_size = [](size_t size) { return size != Http2PriorityUpdateFields::EncodedSize(); };
    Http2FrameBuilder fb;
    fb.Append(Http2PriorityUpdateFields(123));
    EXPECT_TRUE(VerifyDetectsFrameSizeError(0, fb.buffer(), approve_size));
}

class PriorityUpdatePayloadLengthTests : public AbstractPayloadDecoderTest<PriorityUpdatePayloadDecoder, PriorityUpdatePayloadDecoderPeer, Listener>,
...
INSTANTIATE_TEST_SUITE_P(VariousLengths, PriorityUpdatePayloadLengthTests, ::testing::Values(0, 1, 2, 3, 4, 5, 6));
...
TEST_P(PriorityUpdatePayloadLengthTests, ValidLength)
{
    ...
    Http2FrameHeader header(fb.size(), Http2FrameType::PRIORITY_UPDATE, RandFlags(), RandStreamId());
