...
class WindowUpdatePayloadDecoderTest : public AbstractPayloadDecoderTest<WindowUpdatePayloadDecoder, WindowUpdatePayloadDecoderPeer, Listener> {
protected:
...
{
    auto approve_size = [](size_t size) { return size != Http2WindowUpdateFields::EncodedSize(); };
...
TEST_F(WindowUpdatePayloadDecoderTest, VariousPayloads)
{
    for (int n = 0; n < 100; ++n) {
        ...
        Http2FrameBuilder fb;
        fb.Append(fields);
        Http2FrameHeader header(fb.size(), Http2FrameType::WINDOW_UPDATE, RandFlags(), stream_id);
