...
class HeadersPayloadDecoderTest : public AbstractPaddablePayloadDecoderTest<HeadersPayloadDecoder, HeadersPayloadDecoderPeer, Listener> { };

INSTANTIATE_TEST_SUITE_P(VariousPadLengths, HeadersPayloadDecoderTest, ::testing::Values(0, 1, 2, 3, 4, 254, 255, 256));
...
TEST_P(HeadersPayloadDecoderTest, VariousHpackPayloadSizes)
{
    for (size_t hpack_size : { 0, 1, 2, 3, 255, 256, 1024 }) {
        ...
        Http2PriorityFields priority(RandStreamId(), 1 + Random().Rand8(), Random().OneIn(2));
        ...
        for (bool has_priority : { false, true }) {
            ...
            Http2FrameHeader frame_header(frame_builder_.size(), Http2FrameType::HEADERS, flags, RandStreamId());
...
{
    auto approve_size = [](size_t size) { return size != Http2PriorityFields::EncodedSize(); };
    Http2FrameBuilder fb;
    fb.Append(Http2PriorityFields(RandStreamId(), 1 + Random().Rand8(), Random().OneIn(2)));
    EXPECT_TRUE(VerifyDetectsMultipleFrameSizeErrors(Http2FrameFlag::PRIORITY, fb.buffer(), approve_size, total_pad_length_));
}
