...
class SettingsPayloadDecoderTest : public AbstractPayloadDecoderTest<SettingsPayloadDecoder, SettingsPayloadDecoderPeer, Listener> {
protected:
...
TEST_F(SettingsPayloadDecoderTest, SettingsAkcWrongSize)
{
    ...
    fb.Append(RandSettingsFields());
    fb.Append(RandSettingsFields());
    EXPECT_TRUE(VerifyDetectsFrameSizeError(Http2FrameFlag::ACK, fb.buffer(), approve_size));
}
...
TEST_F(SettingsPayloadDecoderTest, SettingsAck)
{
    for (int stream_id = 0; stream_id < 3; ++stream_id) {
        Http2FrameHeader header(0, Http2FrameType::SETTINGS, RandFlags() | Http2FrameFlag::ACK, stream_id);
...
TEST_F(SettingsPayloadDecoderTest, OneRealSetting)
{
    ...
    for (auto param : AllHttp2SettingsParameters()) {
        for (uint32_t value : values) {
            ...
            Http2FrameBuilder fb;
            fb.Append(fields);
            Http2FrameHeader header(fb.size(), Http2FrameType::SETTINGS, RandFlags(), RandStreamId());
            set_frame_header(header);
            FrameParts expected(header);
...
TEST_F(SettingsPayloadDecoderTest, ManySettings)
{
    ...
    for (size_t n = 0; n < num_settings; ++n) {
        Http2SettingFields fields(static_cast<Http2SettingsParameter>(n), Random().Rand32());
        fb.Append(fields);
        expected.AppendSetting(fields);
    }
