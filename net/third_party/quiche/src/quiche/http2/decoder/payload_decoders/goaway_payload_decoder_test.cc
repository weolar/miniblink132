...
class GoAwayPayloadDecoderTest : public AbstractPayloadDecoderTest<GoAwayPayloadDecoder, GoAwayPayloadDecoderPeer, Listener> { };

// Confirm we get an error if the payload is not long enough to hold
// Http2GoAwayFields.
TEST_F(GoAwayPayloadDecoderTest, Truncated)
{
    auto approve_size = [](size_t size) { return size != Http2GoAwayFields::EncodedSize(); };
    Http2FrameBuilder fb;
    fb.Append(Http2GoAwayFields(123, Http2ErrorCode::ENHANCE_YOUR_CALM));
    EXPECT_TRUE(VerifyDetectsFrameSizeError(0, fb.buffer(), approve_size));
}

class GoAwayOpaqueDataLengthTests : public GoAwayPayloadDecoderTest, public ::testing::WithParamInterface<uint32_t> {
protected:
...
INSTANTIATE_TEST_SUITE_P(VariousLengths, GoAwayOpaqueDataLengthTests, ::testing::Values(0, 1, 2, 3, 4, 5, 6));
...
TEST_P(GoAwayOpaqueDataLengthTests, ValidLength)
{
    ...
    Http2FrameHeader header(fb.size(), Http2FrameType::GOAWAY, RandFlags(), RandStreamId());
