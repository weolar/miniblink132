...
public:
    ...
    template <size_t N>
    explicit DecodeBuffer(const char (&buf)[N])
        : DecodeBuffer(buf, N)
    {
    }
    ...
    void AdvanceCursor(size_t amount)
    {
        ...
        QUICHE_DCHECK_EQ(subset_, nullptr) << "Access via subset only when present.";
        cursor_ += amount;
    }
...
protected:
    ...
    void clear_subset_of_base(DecodeBuffer* base, const DecodeBufferSubset* subset);
#endif
