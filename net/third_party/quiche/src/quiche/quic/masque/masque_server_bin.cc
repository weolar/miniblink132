...
DEFINE_QUICHE_COMMAND_LINE_FLAG(int32_t, port, 9661, "The port the MASQUE server will listen on.");
...
DEFINE_QUICHE_COMMAND_LINE_FLAG(std::string, server_authority, "",
...
DEFINE_QUICHE_COMMAND_LINE_FLAG(bool, concealed_auth_on_all_requests, false,
...
int main(int argc, char* argv[])
{
    ...
    std::vector<std::string> non_option_args = quiche::QuicheParseCommandLineFlags(usage, argc, argv);
    ...
    auto backend = std::make_unique<quic::MasqueServerBackend>(
        masque_mode, quiche::GetQuicheCommandLineFlag(FLAGS_server_authority), quiche::GetQuicheCommandLineFlag(FLAGS_cache_dir));

    backend->SetConcealedAuth(quiche::GetQuicheCommandLineFlag(FLAGS_concealed_auth));
    backend->SetConcealedAuthOnAllRequests(quiche::GetQuicheCommandLineFlag(FLAGS_concealed_auth_on_all_requests));

    auto server = std::make_unique<quic::MasqueServer>(masque_mode, backend.get());
