...
class QUIC_NO_EXPORT MasqueClient : public QuicDefaultClient, public MasqueClientSession::Owner {
public:
    ...
    MasqueMode masque_mode() const
    {
        return masque_mode_;
    }
    std::string uri_template() const
    {
...
protected:
    ...
    MasqueClient(QuicSocketAddress server_address, const QuicServerId& server_id, QuicEventLoop* event_loop, const QuicConfig& config,
    ...
    MasqueClient(QuicSocketAddress server_address, const QuicServerId& server_id, MasqueMode masque_mode, QuicEventLoop* event_loop, const QuicConfig& config,
        std::unique_ptr<QuicClientDefaultNetworkHelper> network_helper, std::unique_ptr<ProofVerifier> proof_verifier, const std::string& uri_template);

private:
    // Constructor for when this is only an underlying client.
    // This constructor is private, use Create() instead.
    MasqueClient(QuicSocketAddress server_address, const QuicServerId& server_id, MasqueMode masque_mode, QuicEventLoop* event_loop,
