...
DEFINE_QUICHE_COMMAND_LINE_FLAG(std::string, tap_bridge_interface, "",
...
int CreateTunInterface(const QuicIpAddress& client_address, bool server)
{
    ...
    close(ip_fd);
    QUIC_DLOG(INFO) << "Successfully created TUN interface " << ifr.ifr_name << " with fd " << tun_fd;
...
int CreateTapInterface()
{
    ...
    QUIC_DLOG(INFO) << "Successfully created TAP interface " << ifr.ifr_name << " with fd " << tap_fd;
    ...
    const std::string tap_bridge_interface = quiche::GetQuicheCommandLineFlag(FLAGS_tap_bridge_interface);

    if (!tap_bridge_interface.empty()) {
        if (tap_bridge_interface.size() >= IFNAMSIZ) {
            QUIC_LOG(ERROR) << "tap bridge interface size too long: " << tap_bridge_interface.size();
            return -1;
        }
...
std::string ComputeConcealedAuthContext(uint16_t signature_scheme, absl::string_view key_id, absl::string_view public_key, absl::string_view scheme,
    absl::string_view host, uint16_t port, absl::string_view realm)
{
    QUIC_DVLOG(2) << "ComputeConcealedAuthContext: key_id=\"" << key_id << "\" public_key=" << absl::WebSafeBase64Escape(public_key) << " scheme=\"" << scheme
                  << "\" host=\"" << host << "\" port=" << port << " realm=\"" << realm << "\"";
    std::string key_exporter_context;
    key_exporter_context.resize(sizeof(signature_scheme) + QuicDataWriter::GetVarInt62Len(key_id.size()) + key_id.size()
        + QuicDataWriter::GetVarInt62Len(public_key.size()) + public_key.size() + QuicDataWriter::GetVarInt62Len(scheme.size()) + scheme.size()
        + QuicDataWriter::GetVarInt62Len(host.size()) + host.size() + sizeof(port) + QuicDataWriter::GetVarInt62Len(realm.size()) + realm.size());
    QuicDataWriter writer(key_exporter_context.size(), key_exporter_context.data());
    if (!writer.WriteUInt16(signature_scheme) || !writer.WriteStringPieceVarInt62(key_id) || !writer.WriteStringPieceVarInt62(public_key)
        || !writer.WriteStringPieceVarInt62(scheme) || !writer.WriteStringPieceVarInt62(host) || !writer.WriteUInt16(port)
...
std::string ConcealedAuthDataCoveredBySignature(absl::string_view signature_input)
{
    return absl::StrCat(std::string(64, 0x20), "HTTP Concealed Authentication", std::string(1, 0x00), signature_input);
}
