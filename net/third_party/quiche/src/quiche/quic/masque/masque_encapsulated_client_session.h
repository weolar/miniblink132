...
class QUIC_NO_EXPORT MasqueEncapsulatedClientSession : public MasqueClientSession,
...
public:
    ...
    MasqueEncapsulatedClientSession(const QuicConfig& config, const ParsedQuicVersionVector& supported_versions, QuicConnection* connection,
        const QuicServerId& server_id, QuicCryptoClientConfig* crypto_config, MasqueClientSession* masque_client_session, MasqueClientSession::Owner* owner);

    // Constructor for when this session is both encapsulated and underlying.
    MasqueEncapsulatedClientSession(MasqueMode masque_mode, const std::string& uri_template, const QuicConfig& config,
    ...
    MasqueEncapsulatedClientSession(const MasqueEncapsulatedClientSession&) = delete;
    MasqueEncapsulatedClientSession& operator=(const MasqueEncapsulatedClientSession&) = delete;

    // From MasqueClientSession::EncapsulatedClientSession.
    void ProcessPacket(absl::string_view packet, QuicSocketAddress server_address) override;
    void CloseConnection(QuicErrorCode error, const std::string& details, ConnectionCloseBehavior connection_close_behavior) override;
    ...
    // For CONNECT-IP.
    QuicIpAddress local_v4_address() const
    {
        return local_v4_address_;
    }
    QuicIpAddress local_v6_address() const
    {
        return local_v6_address_;
    }
