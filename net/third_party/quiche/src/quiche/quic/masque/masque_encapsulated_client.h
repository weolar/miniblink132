...
public:
    ...
    MasqueEncapsulatedClient(QuicSocketAddress server_address, const QuicServerId& server_id, QuicEventLoop* event_loop,
    ...
    static std::unique_ptr<MasqueEncapsulatedClient> Create(QuicSocketAddress server_address, const QuicServerId& server_id, const std::string& uri_template,
        MasqueMode masque_mode, QuicEventLoop* event_loop, std::unique_ptr<ProofVerifier> proof_verifier, MasqueClient* underlying_masque_client);
...
private:
    ...
    MasqueEncapsulatedClient(QuicSocketAddress server_address, const QuicServerId& server_id, MasqueMode masque_mode, QuicEventLoop* event_loop,
        std::unique_ptr<ProofVerifier> proof_verifier, MasqueClient* masque_client, const std::string& uri_template);

    MasqueClient* masque_client_; // Unowned.
};

QUIC_NO_EXPORT QuicByteCount MaxPacketSizeForEncapsulatedConnections(MasqueClient* underlying_masque_client);

// Default QuicConfig for use with MASQUE. Sets a custom max_packet_size.
QUIC_NO_EXPORT QuicConfig MasqueEncapsulatedConfig(MasqueClient* underlying_masque_client);
