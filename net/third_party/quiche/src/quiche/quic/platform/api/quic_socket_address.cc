...
QuicSocketAddress::QuicSocketAddress(const struct sockaddr_storage& saddr)
{
    ...
    default:
        QUIC_BUG(quic_bug_10075_1) << "Unknown address family passed: " << saddr.ss_family;
        break;
    }
}
...
QuicSocketAddress::QuicSocketAddress(const sockaddr* saddr, socklen_t len)
{
    ...
    static_assert(std::numeric_limits<socklen_t>::max() >= sizeof(storage), "Cannot cast sizeof(storage) to socklen_t as it does not fit");
    if (len < static_cast<socklen_t>(sizeof(sockaddr)) || (saddr->sa_family == AF_INET && len < static_cast<socklen_t>(sizeof(sockaddr_in)))
        || (saddr->sa_family == AF_INET6 && len < static_cast<socklen_t>(sizeof(sockaddr_in6))) || len > static_cast<socklen_t>(sizeof(storage))) {
...
int QuicSocketAddress::FromSocket(int fd)
{
    ...
    bool success = result == 0 && addr_len > 0 && static_cast<size_t>(addr_len) <= sizeof(addr);
    if (success) {
