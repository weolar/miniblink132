...
inline QuicIpAddress TestLoopback4()
{
    return quiche::TestLoopback4();
}

// Returns the only IPv6 loopback address.
inline QuicIpAddress TestLoopback6()
{
    return quiche::TestLoopback6();
}

// Returns an appropriate IPv4/Ipv6 loopback address based upon whether the
// test's environment.
inline QuicIpAddress TestLoopback()
{
    return quiche::TestLoopback();
}
