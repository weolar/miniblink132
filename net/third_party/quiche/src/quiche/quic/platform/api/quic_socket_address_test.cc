...
TEST(QuicSocketAddress, FromSockaddrIPv4)
{
    ...
    EXPECT_EQ("127.0.0.1:443", QuicSocketAddress(&address.addr, sizeof(address.v4)).ToString());
...
TEST(QuicSocketAddress, FromSockaddrIPv6)
{
    ...
    EXPECT_EQ("[::1]:443", QuicSocketAddress(&address.addr, sizeof(address.v6)).ToString());
...
TEST(QuicSocketAddres, ToSockaddrIPv4)
{
    ...
    address.storage = QuicSocketAddress(QuicIpAddress::Loopback4(), 443).generic_address();
...
TEST(QuicSocketAddress, FromSocket)
{
    ...
    for (int port = 50000; port < 50400; port++) {
        ...
        int bind_result = bind(fd, reinterpret_cast<const sockaddr*>(&raw_address), sizeof(sockaddr_in6));
