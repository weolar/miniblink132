...
constexpr uint8_t kLoadBalancerUnroutablePrefix = kLoadBalancerUnroutableConfigId << kConnectionIdLengthBits;
...
public:
    ...
    virtual void OnConfigChanged(uint8_t old_config_id, uint8_t new_config_id) = 0;
...
// QUIC Connection ID.
class QUIC_EXPORT_PRIVATE LoadBalancerEncoder : public ConnectionIdGeneratorInterface {
public:
    LoadBalancerEncoder(QuicRandom& random, LoadBalancerEncoderVisitorInterface* const visitor, const bool len_self_encoded)
        : LoadBalancerEncoder(random, visitor, len_self_encoded, kLoadBalancerUnroutableLen)
    {
    }
    ...
    static std::optional<LoadBalancerEncoder> Create(QuicRandom& random, LoadBalancerEncoderVisitorInterface* visitor, bool len_self_encoded,
    ...
    bool UpdateConfig(const LoadBalancerConfig& config, LoadBalancerServerId server_id);
    ...
    absl::uint128 num_nonces_left() const
    {
        return num_nonces_left_;
    }
    ...
    virtual bool len_self_encoded() const
    {
        return len_self_encoded_;
    }
    ...
    std::optional<QuicConnectionId> GenerateNextConnectionId(const QuicConnectionId& original) override;
    std::optional<QuicConnectionId> MaybeReplaceConnectionId(const QuicConnectionId& original, const ParsedQuicVersion& version) override;
...
protected:
    LoadBalancerEncoder(
        ...
        , visitor_(visitor)
    {
        std::fill_n(connection_id_lengths_, kNumLoadBalancerConfigs + 1, unroutable_connection_id_len);
    }
