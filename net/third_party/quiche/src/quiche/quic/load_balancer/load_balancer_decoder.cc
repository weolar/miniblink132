...
void LoadBalancerDecoder::DeleteConfig(uint8_t config_id)
{
    if (config_id >= kNumLoadBalancerConfigs) {
        QUIC_BUG(quic_bug_438896865_01) << "Decoder deleting config with invalid config_id " << static_cast<int>(config_id);
        return;
    }
...
bool LoadBalancerDecoder::GetServerId(const QuicConnectionId& connection_id, LoadBalancerServerId& server_id) const
{
    ...
    const uint8_t* data = reinterpret_cast<const uint8_t*>(connection_id.data()) + 1;
    ...
    return config->FourPassDecrypt(absl::MakeConstSpan(data, connection_id.length() - 1), server_id);
}

std::optional<uint8_t> LoadBalancerDecoder::GetConfigId(const QuicConnectionId& connection_id)
{
...
std::optional<uint8_t> LoadBalancerDecoder::GetConfigId(const uint8_t connection_id_first_byte)
{
