...
constexpr uint8_t kRawServerId[] = { 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f };

TEST_F(LoadBalancerServerIdTest, CreateReturnsNullIfTooLong)
{
    EXPECT_QUIC_BUG(
        EXPECT_FALSE(LoadBalancerServerId(absl::Span<const uint8_t>(kRawServerId, 16)).IsValid()), "Attempted to create LoadBalancerServerId with length 16");
    EXPECT_QUIC_BUG(EXPECT_FALSE(LoadBalancerServerId(absl::Span<const uint8_t>()).IsValid()), "Attempted to create LoadBalancerServerId with length 0");
}
...
TEST_F(LoadBalancerServerIdTest, CompareIdenticalExceptLength)
{
    ...
    LoadBalancerServerId shorter_server_id(absl::Span<const uint8_t>(kRawServerId, 5));
...
TEST_F(LoadBalancerServerIdTest, CompareDifferentServerIds)
{
    ...
    LoadBalancerServerId long_server_id(absl::Span<const uint8_t>(kRawServerId, 15));
...
TEST_F(LoadBalancerServerIdTest, SetLengthInvalid)
{
    ...
    EXPECT_QUIC_BUG(server_id.set_length(16), "Attempted to set LoadBalancerServerId length to 16");
    EXPECT_QUIC_BUG(server_id.set_length(0), "Attempted to set LoadBalancerServerId length to 0");
