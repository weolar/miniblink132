...
inline LoadBalancerServerId MakeServerId(const uint8_t array[], const uint8_t length)
{
    ...
    const struct LoadBalancerDecoderTestCase test_vectors[2] = { {
                                                                 ...
                                                                 },
        {
            *LoadBalancerConfig::CreateUnencrypted(1, 8, 5),
            QuicConnectionId({ 0x2d, 0xed, 0x79, 0x3a, 0x51, 0xd4, 0x9b, 0x8f, 0x5f, 0xee, 0x15, 0xda, 0x27, 0xc4 }),
...
TEST_F(LoadBalancerDecoderTest, InvalidConfigId)
{
    ...
    LoadBalancerDecoder decoder;
    EXPECT_TRUE(decoder.AddConfig(*LoadBalancerConfig::CreateUnencrypted(1, 3, 4)));
    QuicConnectionId wrong_config_id({ 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07 });
    LoadBalancerServerId answer;
    EXPECT_FALSE(decoder.GetServerId(QuicConnectionId({ 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07 }), answer));
}
...
TEST_F(LoadBalancerDecoderTest, UnroutableCodepoint)
{
    ...
    EXPECT_TRUE(decoder.AddConfig(*LoadBalancerConfig::CreateUnencrypted(1, 3, 4)));
    LoadBalancerServerId answer;
    EXPECT_FALSE(decoder.GetServerId(QuicConnectionId({ 0xe0, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07 }), answer));
}
...
TEST_F(LoadBalancerDecoderTest, ConnectionIdTooShort)
{
    ...
    EXPECT_TRUE(server_id.IsValid());
    LoadBalancerDecoder decoder;
    EXPECT_TRUE(decoder.AddConfig(*LoadBalancerConfig::CreateUnencrypted(0, 3, 4)));
    LoadBalancerServerId answer;
    EXPECT_FALSE(decoder.GetServerId(QuicConnectionId({ 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06 }), answer));
}
...
TEST_F(LoadBalancerDecoderTest, ConnectionIdTooLongIsOK)
{
    ...
    EXPECT_TRUE(decoder.AddConfig(*LoadBalancerConfig::CreateUnencrypted(0, 3, 4)));
    LoadBalancerServerId answer;
    EXPECT_TRUE(decoder.GetServerId(QuicConnectionId({ 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08 }), answer));
...
TEST_F(LoadBalancerDecoderTest, DeleteConfigBadId)
{
    ...
    EXPECT_QUIC_BUG(decoder.DeleteConfig(7), "Decoder deleting config with invalid config_id 7");
    LoadBalancerServerId answer;
    EXPECT_TRUE(decoder.GetServerId(QuicConnectionId({ 0x40, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07 }), answer));
}
...
TEST_F(LoadBalancerDecoderTest, DeleteConfigGoodId)
{
    ...
    LoadBalancerServerId answer;
    EXPECT_FALSE(decoder.GetServerId(QuicConnectionId({ 0x40, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07 }), answer));
}
...
TEST_F(LoadBalancerDecoderTest, TwoServerIds)
{
    ...
    EXPECT_TRUE(decoder.AddConfig(*LoadBalancerConfig::CreateUnencrypted(0, 3, 4)));
    LoadBalancerServerId answer;
    EXPECT_TRUE(decoder.GetServerId(QuicConnectionId({ 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07 }), answer));
    EXPECT_EQ(answer, server_id1);
    EXPECT_TRUE(decoder.GetServerId(QuicConnectionId({ 0x00, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a }), answer));
    EXPECT_EQ(answer, server_id2);
}

TEST_F(LoadBalancerDecoderTest, GetConfigId)
{
    EXPECT_FALSE(LoadBalancerDecoder::GetConfigId(QuicConnectionId()).has_value());
    for (uint8_t i = 0; i < kNumLoadBalancerConfigs; i++) {
        const QuicConnectionId connection_id({ static_cast<unsigned char>(i << kConnectionIdLengthBits) });
        auto config_id = LoadBalancerDecoder::GetConfigId(connection_id);
        EXPECT_EQ(config_id, LoadBalancerDecoder::GetConfigId(connection_id.data()[0]));
        EXPECT_TRUE(config_id.has_value());
        EXPECT_EQ(*config_id, i);
    }
    EXPECT_FALSE(LoadBalancerDecoder::GetConfigId(QuicConnectionId({ 0xe0 })).has_value());
}
...
{
    uint8_t ptext[] = { 0x00, 0xed, 0x79, 0x3a, 0x51, 0xd4, 0x9b, 0x8f, 0x5f, 0xee, 0x08, 0x0d, 0xbf, 0x48, 0xc0, 0xd1, 0xe5, 0xda, 0x41 };
    uint8_t ctext[] = { 0x00, 0x4d, 0xd2, 0xd0, 0x5a, 0x7b, 0x0d, 0xe9, 0xb2, 0xb9, 0x90, 0x7a, 0xfb, 0x5e, 0xcf, 0x8c, 0xc3, 0xda, 0x41 };
    LoadBalancerDecoder decoder;
    decoder.AddConfig(*LoadBalancerConfig::Create(0, 8, 8, absl::string_view(kRawKey, 16)));
