...
public:
    ...
    {
        if (config_id >= kNumLoadBalancerConfigs || !config_[config_id].has_value()) {
            return nullptr;
        }
    ...
    ABSL_MUST_USE_RESULT bool GetServerId(const QuicConnectionId& connection_id, LoadBalancerServerId& server_id) const;
    ...
    static std::optional<uint8_t> GetConfigId(const QuicConnectionId& connection_id);
