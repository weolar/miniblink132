...
public:
    static void SetNumNoncesLeft(LoadBalancerEncoder& encoder, uint64_t nonces_remaining)
    {
...
class TestLoadBalancerEncoderVisitor : public LoadBalancerEncoderVisitorInterface {
public:
    ...
    void OnConfigChanged(const uint8_t old_config_id, const uint8_t new_config_id) override
    {
        num_adds_++;
        num_deletes_++;
    ...
    uint32_t num_adds() const
    {
        return num_adds_;
    }
    uint32_t num_deletes() const
    {
        return num_deletes_;
    }
...
public:
    ...
    void RandBytes(void* data, size_t len) override
    {
        ...
        while (written < len) {
            ...
            size_t to_write = (len - written > sizeof(uint64_t)) ? sizeof(uint64_t) : (len - written);
...
constexpr char kRawKey[] = { 0x8f, 0x95, 0xf0, 0x92, 0x45, 0x76, 0x5f, 0x80, 0x25, 0x69, 0x34, 0xe5, 0x0c, 0x66, 0x20, 0x7f };
...
constexpr uint8_t kServerId[] = { 0xed, 0x79, 0x3a, 0x51, 0xd4, 0x9b, 0x8f, 0x5f, 0xab, 0x65, 0xba, 0x04, 0xc3, 0x33, 0x0a };

TEST_F(LoadBalancerEncoderTest, BadUnroutableLength)
{
    EXPECT_QUIC_BUG(EXPECT_FALSE(LoadBalancerEncoder::Create(random_, nullptr, false, 0).has_value()), "Invalid unroutable_connection_id_len = 0");
    EXPECT_QUIC_BUG(EXPECT_FALSE(LoadBalancerEncoder::Create(random_, nullptr, false, 21).has_value()), "Invalid unroutable_connection_id_len = 21");
}
...
TEST_F(LoadBalancerEncoderTest, BadServerIdLength)
{
    ...
    EXPECT_QUIC_BUG(EXPECT_FALSE(encoder->UpdateConfig(*config, MakeServerId(kServerId, 4))), "Server ID length 4 does not match configured value of 3");
...
TEST_F(LoadBalancerEncoderTest, FailToUpdateConfigWithSameId)
{
    ...
    EXPECT_QUIC_BUG(EXPECT_FALSE(encoder->UpdateConfig(*config, MakeServerId(kServerId, 3))), "Attempting to change config with same ID");
    EXPECT_EQ(visitor.num_adds(), 1u);
}
    ...
    const struct LoadBalancerEncoderTestCase test_vectors[2] = {
        ...
        },
        {
            *LoadBalancerConfig::CreateUnencrypted(1, 8, 5),
            QuicConnectionId({ 0x2d, 0xed, 0x79, 0x3a, 0x51, 0xd4, 0x9b, 0x8f, 0x5f, 0x8e, 0x98, 0x53, 0xfe, 0x93 }),
...
TEST_F(LoadBalancerEncoderTest, FollowSpecExample)
{
    ...
    auto encoder = LoadBalancerEncoder::Create(random_, nullptr, true, 8);
    ASSERT_TRUE(encoder.has_value());
    auto config = LoadBalancerConfig::Create(config_id, server_id_len, nonce_len, absl::string_view(raw_key));
    ASSERT_TRUE(config.has_value());
    EXPECT_TRUE(encoder->UpdateConfig(*config, LoadBalancerServerId(raw_server_id)));
    EXPECT_TRUE(encoder->IsEncoding());
    const char raw_connection_id[] = { 0x07, 0x67, 0x94, 0x7d, 0x29, 0xbe, 0x05, 0x4a };
    auto expected = QuicConnectionId(raw_connection_id, 1 + server_id_len + nonce_len);
    ...
    const LoadBalancerEncoderTestCase test_vectors[4] = {
        ...
        },
        {
            *LoadBalancerConfig::Create(1, 10, 5, kKey),
            QuicConnectionId({ 0x2f, 0xcc, 0x38, 0x1b, 0xc7, 0x4c, 0xb4, 0xfb, 0xad, 0x28, 0x23, 0xa3, 0xd1, 0xf8, 0xfe, 0xd2 }),
            MakeServerId(kServerId, 10),
        },
        {
            *LoadBalancerConfig::Create(2, 8, 8, kKey),
            QuicConnectionId({ 0x50, 0x4d, 0xd2, 0xd0, 0x5a, 0x7b, 0x0d, 0xe9, 0xb2, 0xb9, 0x90, 0x7a, 0xfb, 0x5e, 0xcf, 0x8c, 0xc3 }),
            MakeServerId(kServerId, 8),
        },
        {
            *LoadBalancerConfig::Create(0, 9, 9, kKey),
            QuicConnectionId({ 0x12, 0x57, 0x79, 0xc9, 0xcc, 0x86, 0xbe, 0xb3, 0xa3, 0xa4, 0xa3, 0xca, 0x96, 0xfc, 0xe4, 0xbf, 0xe0, 0xcd, 0xbc }),
...
TEST_F(LoadBalancerEncoderTest, RunOutOfNonces)
{
    ...
    ASSERT_TRUE(config.has_value());
    EXPECT_TRUE(encoder->UpdateConfig(*config, MakeServerId(kServerId, server_id_len)));
    EXPECT_EQ(visitor.num_adds(), 1u);
    LoadBalancerEncoderPeer::SetNumNoncesLeft(*encoder, 2);
    EXPECT_EQ(encoder->num_nonces_left(), 2);
    EXPECT_EQ(encoder->GenerateConnectionId(), QuicConnectionId({ 0x07, 0x29, 0xd8, 0xc2, 0x17, 0xce, 0x2d, 0x92 }));
...
TEST_F(LoadBalancerEncoderTest, MaybeReplaceConnectionIdReturnsNoChange)
{
    auto encoder = LoadBalancerEncoder::Create(random_, nullptr, false);
    ASSERT_TRUE(encoder.has_value());
    EXPECT_EQ(encoder->MaybeReplaceConnectionId(TestConnectionId(1), ParsedQuicVersion::Q046()), std::nullopt);
}
...
TEST_F(LoadBalancerEncoderTest, MaybeReplaceConnectionIdReturnsChange)
{
    ...
    // The remaining bytes are random, and therefore match kNonceHigh.
    QuicConnectionId expected({ 0xe3, 0x5d, 0x52, 0xde, 0x4d, 0xe3, 0xe7, 0x21 });
    EXPECT_EQ(*encoder->MaybeReplaceConnectionId(TestConnectionId(1), ParsedQuicVersion::RFCv1()), expected);
}
...
TEST_F(LoadBalancerEncoderTest, GenerateNextConnectionIdReturnsNoChange)
{
    ...
    auto encoder = LoadBalancerEncoder::Create(random_, nullptr, true);
    EXPECT_TRUE(encoder->UpdateConfig(*config, MakeServerId(kServerId, 3)));
    EXPECT_EQ(encoder->GenerateNextConnectionId(TestConnectionId(1)), std::nullopt);
}
...
TEST_F(LoadBalancerEncoderTest, ConnectionIdLengthsEncoded)
{
    ...
    auto config0 = LoadBalancerConfig::CreateUnencrypted(config_id, server_id_len, nonce_len);
    ASSERT_TRUE(config0.has_value());
    EXPECT_TRUE(encoder->UpdateConfig(*config0, MakeServerId(kServerId, server_id_len)));
    EXPECT_EQ(encoder->ConnectionIdLength(0xe8), kQuicDefaultConnectionIdLength);
    EXPECT_EQ(encoder->ConnectionIdLength(0x4a), kQuicDefaultConnectionIdLength);
    ...
    auto config1 = LoadBalancerConfig::CreateUnencrypted(config_id, server_id_len, nonce_len);
    ASSERT_TRUE(config1.has_value());
    // Old config length still there after replacement
    EXPECT_TRUE(encoder->UpdateConfig(*config1, MakeServerId(kServerId, server_id_len)));
    EXPECT_EQ(encoder->ConnectionIdLength(0xe8), kQuicDefaultConnectionIdLength);
