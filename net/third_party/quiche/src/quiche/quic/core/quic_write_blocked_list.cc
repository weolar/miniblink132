...
QuicWriteBlockedList::QuicWriteBlockedList()
    ...
    , respect_incremental_(GetQuicReloadableFlag(quic_priority_respect_incremental))
...
QuicStreamId QuicWriteBlockedList::PopFront()
{
    ...
    const auto [id, priority] = priority_write_scheduler_.PopNextReadyStreamAndPriority();
...
void QuicWriteBlockedList::RegisterStream(QuicStreamId stream_id, bool is_static_stream, const QuicStreamPriority& priority)
{
    QUICHE_DCHECK(!priority_write_scheduler_.StreamRegistered(stream_id)) << "stream " << stream_id << " already registered";
    if (is_static_stream) {
...
void QuicWriteBlockedList::UpdateStreamPriority(QuicStreamId stream_id, const QuicStreamPriority& new_priority)
{
    QUICHE_DCHECK(!static_stream_collection_.IsRegistered(stream_id));
    priority_write_scheduler_.UpdateStreamPriority(stream_id, new_priority.http());
}

void QuicWriteBlockedList::UpdateBytesForStream(QuicStreamId stream_id, size_t bytes)
{
    if (disable_batch_write_) {
    ...
    if (batch_write_stream_id_[last_priority_popped_] == stream_id) {
        ...
        bytes_left_for_batch_write_[last_priority_popped_] -= std::min(bytes_left_for_batch_write_[last_priority_popped_], bytes);
    }
}
...
void QuicWriteBlockedList::AddStream(QuicStreamId stream_id)
{
    ...
    if (respect_incremental_) {
        ...
        if (!priority_write_scheduler_.GetStreamPriority(stream_id).incremental) {
            const bool push_front = stream_id == batch_write_stream_id_[last_priority_popped_];
    ...
    const bool push_front = stream_id == batch_write_stream_id_[last_priority_popped_] && bytes_left_for_batch_write_[last_priority_popped_] > 0;
...
bool QuicWriteBlockedList::StaticStreamCollection::IsRegistered(QuicStreamId id) const
{
...
bool QuicWriteBlockedList::StaticStreamCollection::UnblockFirstBlocked(QuicStreamId* id)
{
    for (auto& stream : streams_) {
        if (stream.is_blocked) {
            --num_blocked_;
