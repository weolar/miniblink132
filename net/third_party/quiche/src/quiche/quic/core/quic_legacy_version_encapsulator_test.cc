...
class QuicLegacyVersionEncapsulatorTest : public QuicTestWithParam<ParsedQuicVersion> {
protected:
    ...
    {
        encapsulated_length_
            = QuicLegacyVersionEncapsulator::Encapsulate(sni_, inner_packet, server_connection_id_, QuicTime::Zero(), outer_max_packet_length_, outer_buffer_);
    }
    ...
    void CheckEncapsulation()
    {
        ...
        const QuicErrorCode error = QuicFramer::ParsePublicHeaderDispatcher(QuicEncryptedPacket(outer_buffer_, encapsulated_length_),
            kQuicDefaultConnectionIdLength, &format, &long_packet_type, &version_present, &has_length_prefix, &version_label, &parsed_version,
...
INSTANTIATE_TEST_SUITE_P(
    QuicLegacyVersionEncapsulatorTests, QuicLegacyVersionEncapsulatorTest, ::testing::ValuesIn(AllSupportedVersions()), ::testing::PrintToStringParamName());
