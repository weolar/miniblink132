...
public:
    ...
    public:
        ...
        virtual bool IsCryptoFrameExpectedForEncryptionLevel(EncryptionLevel level) const = 0;

        // Returns the encryption level to send CRYPTO_FRAME for `space`.
        virtual EncryptionLevel GetEncryptionLevelToSendCryptoDataOfSpace(PacketNumberSpace space) const = 0;
        ...
        virtual const QuicCryptoNegotiatedParameters& crypto_negotiated_params() const = 0;
        ...
        virtual void OnConnectionClosed(QuicErrorCode error, ConnectionCloseSource source) = 0;
        ...
        virtual void SetServerApplicationStateForResumption(std::unique_ptr<ApplicationState> application_state) = 0;

        // Called to obtain keying material export of length |result_len| with the
        // given |label| and |context|. Returns false on failure.
        virtual bool ExportKeyingMaterial(absl::string_view label, absl::string_view context, size_t result_len, std::string* result) = 0;
    };
    ...
    public:
        ...
        // completes.
        virtual void OnProofValid(const QuicCryptoClientConfig::CachedState& cached) = 0;
        ...
        virtual void OnProofVerifyDetailsAvailable(const ProofVerifyDetails& verify_details) = 0;
    };

    QuicCryptoClientStream(const QuicServerId& server_id, QuicSession* session, std::unique_ptr<ProofVerifyContext> verify_context,
        QuicCryptoClientConfig* crypto_config, ProofHandler* proof_handler, bool has_application_state);
