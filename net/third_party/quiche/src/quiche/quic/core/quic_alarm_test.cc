...
public:
    ...
    void set_alarm(QuicAlarm* alarm)
    {
        alarm_ = alarm;
    }
...
TEST_F(QuicAlarmTest, PermanentCancel)
{
    ...
    EXPECT_QUIC_BUG(alarm_.Set(deadline), "Set called after alarm is permanently cancelled");
    EXPECT_TRUE(alarm_.IsPermanentlyCancelled());
    EXPECT_FALSE(alarm_.IsSet());
    EXPECT_FALSE(alarm_.scheduled());
    EXPECT_EQ(QuicTime::Zero(), alarm_.deadline());

    EXPECT_QUIC_BUG(alarm_.Update(deadline, QuicTime::Delta::Zero()), "Update called after alarm is permanently cancelled");
    EXPECT_TRUE(alarm_.IsPermanentlyCancelled());
    EXPECT_FALSE(alarm_.IsSet());
    EXPECT_FALSE(alarm_.scheduled());
    EXPECT_EQ(QuicTime::Zero(), alarm_.deadline());
...
TEST_F(QuicAlarmTest, FireAndResetViaSet)
{
    ...
    EXPECT_CALL(*delegate_, OnAlarm()).WillOnce(Invoke(this, &QuicAlarmTest::ResetAlarm));
    alarm_.FireAlarm();
    EXPECT_TRUE(alarm_.IsSet());
    EXPECT_TRUE(alarm_.scheduled());
...
TEST_F(QuicAlarmTest, NullAlarmContext)
{
    ...
    EXPECT_CALL(*delegate_, OnAlarm()).WillOnce(Invoke([] { QUIC_TRACELITERAL("Alarm fired."); }));
    alarm_.FireAlarm();
}
