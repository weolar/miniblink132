...
public:
    ...
    void OnChlo(QuicTransportVersion version, QuicConnectionId connection_id, const CryptoHandshakeMessage& chlo) override
    {
        version_ = version;
...
public:
    ...
    void MakePacket(absl::string_view data, bool munge_offset, bool munge_stream_id)
    {
        ...
        QuicFramer framer(SupportedVersions(version_), QuicTime::Zero(), Perspective::IS_CLIENT, kQuicDefaultConnectionIdLength);
        framer.SetInitialObfuscators(TestConnectionId());
        if (!version_.UsesCryptoFrames() || munge_stream_id) {
            QuicStreamId stream_id = QuicUtils::GetCryptoStreamId(version_.transport_version);
            if (munge_stream_id) {
                stream_id++;
            }
            frames.push_back(QuicFrame(QuicStreamFrame(stream_id, false, offset, data)));
        } else {
            frames.push_back(QuicFrame(new QuicCryptoFrame(ENCRYPTION_INITIAL, offset, data)));
        }
        std::unique_ptr<QuicPacket> packet(BuildUnsizedDataPacket(&framer, header, frames));
        EXPECT_TRUE(packet != nullptr);
        size_t encrypted_length = framer.EncryptPayload(ENCRYPTION_INITIAL, header.packet_number, *packet, buffer_, ABSL_ARRAYSIZE(buffer_));
...
INSTANTIATE_TEST_SUITE_P(ChloExtractorTests, ChloExtractorTest, ::testing::ValuesIn(AllSupportedVersionsWithQuicCrypto()), ::testing::PrintToStringParamName());
...
TEST_P(ChloExtractorTest, FindsValidChlo)
{
    ...
    EXPECT_TRUE(ChloExtractor::Extract(*packet_, version_, {}, &delegate_, kQuicDefaultConnectionIdLength));
...
TEST_P(ChloExtractorTest, DoesNotFindValidChloOnWrongStream)
{
    ...
    EXPECT_FALSE(ChloExtractor::Extract(*packet_, version_, {}, &delegate_, kQuicDefaultConnectionIdLength));
}
...
{
    std::vector<std::unique_ptr<QuicReceivedPacket>> packets = GetFirstFlightOfPackets(version_);
    ASSERT_EQ(packets.size(), 1u);
    EXPECT_TRUE(ChloExtractor::Extract(*packets[0], version_, {}, &delegate_, kQuicDefaultConnectionIdLength));
    EXPECT_EQ(version_.transport_version, delegate_.transport_version());
    EXPECT_EQ(TestConnectionId(), delegate_.connection_id());
