...
public:
    ...
    QuicConnectionAlarmDelegate& operator=(const QuicConnectionAlarmDelegate&) = delete;
...
class ProcessUndecryptablePacketsAlarmDelegate : public QuicConnectionAlarmDelegate {
public:
    using QuicConnectionAlarmDelegate::QuicConnectionAlarmDelegate;
...
class DiscardPreviousOneRttKeysAlarmDelegate : public QuicConnectionAlarmDelegate {
public:
    using QuicConnectionAlarmDelegate::QuicConnectionAlarmDelegate;
...
class DiscardZeroRttDecryptionKeysAlarmDelegate : public QuicConnectionAlarmDelegate {
public:
    using QuicConnectionAlarmDelegate::QuicConnectionAlarmDelegate;
...
public:
    ...
    IdleDetectorAlarmDelegate& operator=(const IdleDetectorAlarmDelegate&) = delete;
...
class NetworkBlackholeDetectorAlarmDelegate : public QuicConnectionAlarmDelegate {
public:
    using QuicConnectionAlarmDelegate::QuicConnectionAlarmDelegate;

    NetworkBlackholeDetectorAlarmDelegate(const NetworkBlackholeDetectorAlarmDelegate&) = delete;
    NetworkBlackholeDetectorAlarmDelegate& operator=(const NetworkBlackholeDetectorAlarmDelegate&) = delete;
...
QuicAlarmMultiplexer::QuicAlarmMultiplexer(absl::Nonnull<QuicConnectionAlarmsDelegate*> connection, QuicConnectionArena& arena, QuicAlarmFactory& alarm_factory)
    : deadlines_({ QuicTime::Zero(), QuicTime::Zero(), QuicTime::Zero(), QuicTime::Zero(), QuicTime::Zero(), QuicTime::Zero(), QuicTime::Zero(),
        QuicTime::Zero(), QuicTime::Zero(), QuicTime::Zero(), QuicTime::Zero() })
    , now_alarm_(alarm_factory.CreateAlarm(arena.New<MultiplexerAlarmDelegate>(this), &arena))
    , later_alarm_(alarm_factory.CreateAlarm(arena.New<MultiplexerAlarmDelegate>(this), &arena))
    , connection_(connection)
    , underlying_alarm_granularity_(QuicTimeDelta::FromMicroseconds(GetQuicFlag(quic_multiplexer_alarm_granularity_us)))
{
}
...
void QuicAlarmMultiplexer::Set(QuicAlarmSlot slot, QuicTime new_deadline)
{
    ...
    if (permanently_cancelled_) {
        QUICHE_BUG(quic_alarm_multiplexer_illegal_set) << "Set called after alarms are permanently cancelled. new_deadline:" << new_deadline;
        return;
    }
...
void QuicAlarmMultiplexer::Update(QuicAlarmSlot slot, QuicTime new_deadline, QuicTimeDelta granularity)
{
    if (permanently_cancelled_) {
        QUICHE_BUG(quic_alarm_multiplexer_illegal_update)
            << "Update called after alarm is permanently cancelled. new_deadline:" << new_deadline << ", granularity:" << granularity;
        return;
    }
    ...
    if (std::abs((new_deadline - GetDeadline(slot)).ToMicroseconds()) < granularity.ToMicroseconds()) {
        return;
    }
    SetDeadlineFor(slot, new_deadline);
    MaybeRescheduleUnderlyingAlarms();
...
{
    if (permanently_cancelled_) {
        QUICHE_BUG(multiplexer_fire_alarms_permanently_cancelled) << "FireAlarms() called when all alarms have been permanently "
                                                                     "cancelled.";
        return;
    }
    ...
    for (size_t slot_number = 0; slot_number < deadlines_.size(); ++slot_number) {
        if (deadlines_[slot_number].IsInitialized() && deadlines_[slot_number] <= now) {
            scheduled.push_back(static_cast<QuicAlarmSlot>(slot_number));
        }
    }

    // Execute them in order of scheduled deadlines.
    absl::c_sort(scheduled, [this](QuicAlarmSlot a, QuicAlarmSlot b) { return GetDeadline(a) < GetDeadline(b); });
...
void QuicAlarmMultiplexer::RescheduleUnderlyingAlarms()
{
    ...
    QUICHE_DVLOG(1) << "Rescheduled alarms; now = " << (schedule_now ? "true" : "false") << "; later = " << later_alarm_deadline;
...
std::string QuicAlarmMultiplexer::DebugString()
{
    ...
    for (const auto& [deadline, slot] : scheduled) {
        ...
        absl::StrAppendFormat(&result, "        %.1fms --- %s\n", relative.ToMicroseconds() / 1000.f, QuicAlarmSlotName(slot));
    }
    return result;
}
...
QuicConnectionAlarmHolder::QuicConnectionAlarmHolder(QuicConnectionAlarmsDelegate* delegate, QuicAlarmFactory& alarm_factory, QuicConnectionArena& arena)
    : ack_alarm_(alarm_factory.CreateAlarm(arena.New<AckAlarmDelegate>(delegate), &arena))
    , retransmission_alarm_(alarm_factory.CreateAlarm(arena.New<RetransmissionAlarmDelegate>(delegate), &arena))
    , send_alarm_(alarm_factory.CreateAlarm(arena.New<SendAlarmDelegate>(delegate), &arena))
    , mtu_discovery_alarm_(alarm_factory.CreateAlarm(arena.New<MtuDiscoveryAlarmDelegate>(delegate), &arena))
    , process_undecryptable_packets_alarm_(alarm_factory.CreateAlarm(arena.New<ProcessUndecryptablePacketsAlarmDelegate>(delegate), &arena))
    , discard_previous_one_rtt_keys_alarm_(alarm_factory.CreateAlarm(arena.New<DiscardPreviousOneRttKeysAlarmDelegate>(delegate), &arena))
    , discard_zero_rtt_decryption_keys_alarm_(alarm_factory.CreateAlarm(arena.New<DiscardZeroRttDecryptionKeysAlarmDelegate>(delegate), &arena))
    , multi_port_probing_alarm_(alarm_factory.CreateAlarm(arena.New<MultiPortProbingAlarmDelegate>(delegate), &arena))
    , idle_network_detector_alarm_(alarm_factory.CreateAlarm(arena.New<IdleDetectorAlarmDelegate>(delegate), &arena))
    , network_blackhole_detector_alarm_(alarm_factory.CreateAlarm(arena.New<NetworkBlackholeDetectorAlarmDelegate>(delegate), &arena))
    , ping_alarm_(alarm_factory.CreateAlarm(arena.New<PingAlarmDelegate>(delegate), &arena))
{
}

QuicConnectionAlarms::QuicConnectionAlarms(QuicConnectionAlarmsDelegate* delegate, QuicAlarmFactory& alarm_factory, QuicConnectionArena& arena)
