...
void QuicAlarm::Set(QuicTime new_deadline)
{
    ...
    if (IsPermanentlyCancelled()) {
        QUIC_BUG(quic_alarm_illegal_set) << "Set called after alarm is permanently cancelled. new_deadline:" << new_deadline;
        return;
    }
...
{
    if (IsPermanentlyCancelled()) {
        QUIC_BUG(quic_alarm_illegal_update) << "Update called after alarm is permanently cancelled. new_deadline:" << new_deadline
    ...
    if (std::abs((new_deadline - deadline_).ToMicroseconds()) < granularity.ToMicroseconds()) {
        return;
    }
...
void QuicAlarm::Fire()
{
    ...
    deadline_ = QuicTime::Zero();
    if (!IsPermanentlyCancelled()) {
        QuicConnectionContextSwitcher context_switcher(delegate_->GetConnectionContext());
        delegate_->OnAlarm();
    }
}
