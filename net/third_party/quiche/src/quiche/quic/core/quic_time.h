...
public:
    ...
    explicit QuicTimeDelta(absl::Duration duration)
        : time_offset_((duration == absl::InfiniteDuration()) ? kInfiniteTimeUs : absl::ToInt64Microseconds(duration))
    {
    }
    ...
    template <typename Sink> friend void AbslStringify(Sink& sink, QuicTimeDelta delta)
    {
...
private:
    ...
    friend inline constexpr QuicTimeDelta operator+(QuicTimeDelta lhs, QuicTimeDelta rhs);
    friend inline constexpr QuicTimeDelta operator-(QuicTimeDelta lhs, QuicTimeDelta rhs);
    ...
    static constexpr int64_t kInfiniteTimeUs = std::numeric_limits<int64_t>::max();
...
private:
    ...
    explicit constexpr QuicTime(int64_t time)
        : time_(time)
    {
    }
...
{
    return QuicTimeDelta(static_cast<int64_t>(std::llround(static_cast<double>(lhs.time_offset_) * rhs)));
}
...
inline std::ostream& operator<<(std::ostream& output, const QuicTimeDelta delta)
{
