...
QUICHE_EXPORT std::string QuicVersionToString(QuicTransportVersion transport_version);
...
QUICHE_EXPORT std::string HandshakeProtocolToString(HandshakeProtocol handshake_protocol);

// Returns whether |transport_version| uses CRYPTO frames for the handshake
// instead of stream 1.
QUICHE_EXPORT constexpr bool QuicVersionUsesCryptoFrames(QuicTransportVersion transport_version)
{
...
QUICHE_EXPORT constexpr bool ParsedQuicVersionIsValid(HandshakeProtocol handshake_protocol, QuicTransportVersion transport_version)
{
    ...
    case PROTOCOL_QUIC_CRYPTO:
        return transport_version != QUIC_VERSION_UNSUPPORTED && transport_version != QUIC_VERSION_RESERVED_FOR_NEGOTIATION
            && transport_version != QUIC_VERSION_IETF_DRAFT_29 && transport_version != QUIC_VERSION_IETF_RFC_V1
            && transport_version != QUIC_VERSION_IETF_RFC_V2;
    case PROTOCOL_TLS1_3:
        return transport_version != QUIC_VERSION_UNSUPPORTED && QuicVersionUsesCryptoFrames(transport_version);
    }
    return false;
}
...
struct QUICHE_EXPORT ParsedQuicVersion {
    ...
    constexpr ParsedQuicVersion(HandshakeProtocol handshake_protocol, QuicTransportVersion transport_version)
        : handshake_protocol(handshake_protocol)
        , transport_version(transport_version)
    {
        QUICHE_DCHECK(ParsedQuicVersionIsValid(handshake_protocol, transport_version))
            << QuicVersionToString(transport_version) << " " << HandshakeProtocolToString(handshake_protocol);
    }
    ...
    {
        QUICHE_DCHECK(ParsedQuicVersionIsValid(other.handshake_protocol, other.transport_version))
            << QuicVersionToString(other.transport_version) << " " << HandshakeProtocolToString(other.handshake_protocol);
        if (this != &other) {
    ...
    {
        return handshake_protocol == other.handshake_protocol && transport_version == other.transport_version;
    }
    ...
    {
        return ParsedQuicVersion(PROTOCOL_TLS1_3, QUIC_VERSION_RESERVED_FOR_NEGOTIATION);
    }
...
QUICHE_EXPORT std::ostream& operator<<(std::ostream& os, const ParsedQuicVersion& version);

using ParsedQuicVersionVector = std::vector<ParsedQuicVersion>;

QUICHE_EXPORT std::ostream& operator<<(std::ostream& os, const ParsedQuicVersionVector& versions);
...
QUICHE_EXPORT QuicVersionLabel MakeVersionLabel(uint8_t a, uint8_t b, uint8_t c, uint8_t d);

QUICHE_EXPORT std::ostream& operator<<(std::ostream& os, const QuicVersionLabelVector& version_labels);
...
QUICHE_EXPORT std::ostream& operator<<(std::ostream& os, const QuicTransportVersionVector& transport_versions);
...
QUICHE_EXPORT ParsedQuicVersionVector FilterSupportedVersions(ParsedQuicVersionVector versions);
...
QUICHE_EXPORT ParsedQuicVersionVector ParsedVersionOfIndex(const ParsedQuicVersionVector& versions, int index);
...
// ParsedQuicVersion.
QUICHE_EXPORT ParsedQuicVersion ParseQuicVersionLabel(QuicVersionLabel version_label);

// Helper function that translates from a QuicVersionLabelVector to a
// ParsedQuicVersionVector.
QUICHE_EXPORT ParsedQuicVersionVector ParseQuicVersionLabelVector(const QuicVersionLabelVector& version_labels);
...
QUICHE_EXPORT ParsedQuicVersion ParseQuicVersionString(absl::string_view version_string);
...
QUICHE_EXPORT ParsedQuicVersionVector ParseQuicVersionVectorString(absl::string_view versions_string);
...
QUICHE_EXPORT QuicVersionLabel CreateQuicVersionLabel(ParsedQuicVersion parsed_version);

// Constructs a QuicVersionLabelVector from the provided
// ParsedQuicVersionVector.
QUICHE_EXPORT QuicVersionLabelVector CreateQuicVersionLabelVector(const ParsedQuicVersionVector& versions);

// Helper function which translates from a QuicVersionLabel to a string.
QUICHE_EXPORT std::string QuicVersionLabelToString(QuicVersionLabel version_label);
...
// otherwise.
QUICHE_EXPORT ParsedQuicVersion ParseQuicVersionLabelString(absl::string_view version_label_string);
...
QUICHE_EXPORT std::string QuicVersionLabelVectorToString(
    const QuicVersionLabelVector& version_labels, const std::string& separator, size_t skip_after_nth_version);

// Returns comma separated list of string representations of QuicVersionLabel
// values in the supplied |version_labels| vector.
QUICHE_EXPORT inline std::string QuicVersionLabelVectorToString(const QuicVersionLabelVector& version_labels)
{
    return QuicVersionLabelVectorToString(version_labels, ",", std::numeric_limits<size_t>::max());
}
...
QUICHE_EXPORT std::string QuicTransportVersionVectorToString(const QuicTransportVersionVector& versions);

// Returns comma separated list of string representations of ParsedQuicVersion
// values in the supplied |versions| vector.
QUICHE_EXPORT std::string ParsedQuicVersionVectorToString(const ParsedQuicVersionVector& versions);
...
QUICHE_EXPORT std::string ParsedQuicVersionVectorToString(const ParsedQuicVersionVector& versions, const std::string& separator, size_t skip_after_nth_version);

// Returns comma separated list of string representations of ParsedQuicVersion
// values in the supplied |versions| vector.
QUICHE_EXPORT inline std::string ParsedQuicVersionVectorToString(const ParsedQuicVersionVector& versions)
{
    return ParsedQuicVersionVectorToString(versions, ",", std::numeric_limits<size_t>::max());
}
...
// * DATA frame has frame headers.
// * GOAWAY is moved to HTTP layer.
QUICHE_EXPORT constexpr bool VersionUsesHttp3(QuicTransportVersion transport_version)
{
...
// Returns whether the transport_version supports the variable length integer
// length field as defined by IETF QUIC draft-13 and later.
QUICHE_EXPORT constexpr bool QuicVersionHasLongHeaderLengths(QuicTransportVersion transport_version)
{
...
// Returns whether |transport_version| makes use of IETF QUIC
// frames or not.
QUICHE_EXPORT constexpr bool VersionHasIetfQuicFrames(QuicTransportVersion transport_version)
{
...
// connection ID lengths as described in draft-ietf-quic-invariants-06 and
// draft-ietf-quic-transport-22.
QUICHE_EXPORT bool VersionHasLengthPrefixedConnectionIds(QuicTransportVersion transport_version);

// Returns true if this version supports the old Google-style Alt-Svc
// advertisement format.
QUICHE_EXPORT bool VersionSupportsGoogleAltSvcFormat(QuicTransportVersion transport_version);

// Returns whether this version allows server connection ID lengths that are
// not 64 bits.
QUICHE_EXPORT bool VersionAllowsVariableLengthConnectionIds(QuicTransportVersion transport_version);
...
QUICHE_EXPORT bool QuicVersionLabelUses4BitConnectionIdLength(QuicVersionLabel version_label);
