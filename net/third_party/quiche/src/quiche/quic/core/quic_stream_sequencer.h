...
public:
    ...
    public:
        ...
        // being closed.
        virtual void OnUnrecoverableError(QuicErrorCode error, const std::string& details) = 0;
        // Called when an error has occurred which should result in the connection
        // being closed, specifying the wire error code |ietf_error| explicitly.
        virtual void OnUnrecoverableError(QuicErrorCode error, QuicIetfTransportErrorCodes ietf_error, const std::string& details) = 0;
    ...
    QuicStreamOffset close_offset() const
    {
    ...
    bool ignore_read_data() const
    {
        return ignore_read_data_;
    }
    ...
    {
        return level_triggered_;
    }

    void set_stream(StreamInterface* stream)
    {
        stream_ = stream;
    }
...
private:
    ...
    void OnFrameData(QuicStreamOffset byte_offset, size_t data_len, const char* data_buffer);
