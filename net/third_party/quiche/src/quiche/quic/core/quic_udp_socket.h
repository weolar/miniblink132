...
static_assert(
    static_cast<size_t>(QuicUdpPacketInfoBit::NUM_BITS) <= QuicUdpPacketInfoBitMask::NumBits(), "QuicUdpPacketInfoBitMask not wide enough to hold all bits.");
...
public:
    QuicUdpPacketInfoBitMask bitmask() const
    {
        return bitmask_;
    }
    ...
    bool HasValue(QuicUdpPacketInfoBit bit) const
    {
        return bitmask_.IsSet(bit);
    }
...
public:
    ...
    // failed.
    QuicUdpSocketFd Create(int address_family, int receive_buffer_size, int send_buffer_size, bool ipv6_only = false);
    ...
    void ReadPacket(QuicUdpSocketFd fd, QuicUdpPacketInfoBitMask packet_info_interested, ReadPacketResult* result);
    ...
    size_t ReadMultiplePackets(QuicUdpSocketFd fd, QuicUdpPacketInfoBitMask packet_info_interested, ReadPacketResults* results);
    ...
    WriteResult WritePacket(QuicUdpSocketFd fd, const char* packet_buffer, size_t packet_buffer_len, const QuicUdpPacketInfo& packet_info);

protected:
    bool SetupSocket(QuicUdpSocketFd fd, int address_family, int receive_buffer_size, int send_buffer_size, bool ipv6_only);
