...
public:
    ...
    static QuicByteCount CryptoMessageFramingOverhead(QuicTransportVersion version, QuicConnectionId connection_id);
    ...
    bool OnCryptoFrameAcked(const QuicCryptoFrame& frame, QuicTime::Delta ack_delay_time);
    ...
    virtual std::string GetAddressToken(const CachedNetworkParameters* cached_network_params) const = 0;
    ...
    virtual const CachedNetworkParameters* PreviousCachedNetworkParams() const = 0;
    ...
    virtual void SetPreviousCachedNetworkParams(CachedNetworkParameters cached_network_params) = 0;
    ...
    virtual void SetServerApplicationStateForResumption(std::unique_ptr<ApplicationState> state) = 0;
    ...
    bool RetransmitStreamData(QuicStreamOffset offset, QuicByteCount data_length, bool fin, TransmissionType type) override;
    ...
    // |offset|.
    bool WriteCryptoFrame(EncryptionLevel level, QuicStreamOffset offset, QuicByteCount data_length, QuicDataWriter* writer);
    ...
    bool IsFrameOutstanding(EncryptionLevel level, size_t offset, size_t length) const;
    ...
    virtual void OnDataAvailableInSequencer(QuicStreamSequencer* sequencer, EncryptionLevel level);

    QuicStreamSequencer* GetStreamSequencerForPacketNumberSpace(PacketNumberSpace packet_number_space)
    {
