...
public:
    virtual void OnMultiPortPathContextAvailable(std::unique_ptr<QuicPathValidationContext>) = 0;
...
public:
    ...
    virtual void OnConnectionClosed(const QuicConnectionCloseFrame& frame, ConnectionCloseSource source) = 0;
    ...
    virtual void OnSuccessfulVersionNegotiation(const ParsedQuicVersion& version) = 0;
    ...
    // probe.
    virtual void OnPacketReceived(const QuicSocketAddress& self_address, const QuicSocketAddress& peer_address, bool is_connectivity_probe) = 0;
    ...
    virtual bool MaybeReserveConnectionId(const QuicConnectionId& server_connection_id) = 0;

    // Called when server stops to use a server issued connection ID.
    virtual void OnServerConnectionIdRetired(const QuicConnectionId& server_connection_id) = 0;
    ...
    // is not thread safe.
    virtual void CreateContextForMultiPortPath(std::unique_ptr<MultiPortPathContextObserver> context_observer) = 0;

    // Migrate to the multi-port path which is identified by |context|.
    virtual void MigrateToMultiPortPath(std::unique_ptr<QuicPathValidationContext> context) = 0;

    // Called when the client receives a preferred address from its peer.
    virtual void OnServerPreferredAddressAvailable(const QuicSocketAddress& server_preferred_address) = 0;
...
class QUICHE_EXPORT QuicConnectionDebugVisitor : public QuicSentPacketManager::DebugDelegate {
public:
    ...
    virtual void OnPacketSent(QuicPacketNumber /*packet_number*/, QuicPacketLength /*packet_length*/, bool /*has_crypto_handshake*/,
        TransmissionType /*transmission_type*/, EncryptionLevel /*encryption_level*/, const QuicFrames& /*retransmittable_frames*/,
        const QuicFrames& /*nonretransmittable_frames*/, QuicTime /*sent_time*/, uint32_t /*batch_id*/)
    {
    }

    // Called when a coalesced packet is successfully serialized.
    virtual void OnCoalescedPacketSent(const QuicCoalescedPacket& /*coalesced_packet*/, size_t /*length*/)
    {
    }
    ...
    // validated or parsed.
    virtual void OnPacketReceived(const QuicSocketAddress& /*self_address*/, const QuicSocketAddress& /*peer_address*/, const QuicEncryptedPacket& /*packet*/)
    {
    }
    ...
    virtual void OnUndecryptablePacket(EncryptionLevel /*decryption_level*/, bool /*dropped*/)
    {
    }

    // Called when attempting to process a previously undecryptable packet.
    virtual void OnAttemptingToProcessUndecryptablePacket(EncryptionLevel /*decryption_level*/)
    {
    }
    ...
    virtual void OnPacketHeader(const QuicPacketHeader& /*header*/, QuicTime /*receive_time*/, EncryptionLevel /*level*/)
    {
    }
    ...
    virtual void OnPingFrame(const QuicPingFrame& /*frame*/, QuicTime::Delta /*ping_received_delay*/)
    {
    }
    ...
    virtual void OnWindowUpdateFrame(const QuicWindowUpdateFrame& /*frame*/, const QuicTime& /*receive_time*/)
    {
    }
    ...
    virtual void OnNewConnectionIdFrame(const QuicNewConnectionIdFrame& /*frame*/)
    {
    }

    // Called when a RetireConnectionIdFrame has been parsed.
    virtual void OnRetireConnectionIdFrame(const QuicRetireConnectionIdFrame& /*frame*/)
    {
    }
    ...
    virtual void OnVersionNegotiationPacket(const QuicVersionNegotiationPacket& /*packet*/)
    {
    }

    // Called when the connection is closed.
    virtual void OnConnectionClosed(const QuicConnectionCloseFrame& /*frame*/, ConnectionCloseSource /*source*/)
    {
    }
    ...
    virtual void OnSendConnectionState(const CachedNetworkParameters& /*cached_network_params*/)
    {
    }

    // Called when a CachedNetworkParameters are received from the client.
    virtual void OnReceiveConnectionState(const CachedNetworkParameters& /*cached_network_params*/)
    {
    }
    ...
    virtual void OnNPacketNumbersSkipped(QuicPacketCount /*count*/, QuicTime /*now*/)
    {
    }
    ...
    virtual void OnTransportParametersSent(const TransportParameters& /*transport_parameters*/)
    {
    }

    // Called for QUIC+TLS versions when we receive transport parameters.
    virtual void OnTransportParametersReceived(const TransportParameters& /*transport_parameters*/)
    {
    }

    // Called for QUIC+TLS versions when we resume cached transport parameters for
    // 0-RTT.
    virtual void OnTransportParametersResumed(const TransportParameters& /*transport_parameters*/)
    {
    }
    ...
    virtual void OnPeerAddressChange(AddressChangeType /*type*/, QuicTime::Delta /*connection_time*/)
    {
    }
    ...
    virtual void OnEncryptedClientHelloReceived(absl::string_view /*client_hello*/)
    {
    }
...
class QUICHE_EXPORT QuicConnection : public QuicFramerVisitorInterface,
...
public:
    ...
    QuicConnection(QuicConnectionId server_connection_id, QuicSocketAddress initial_self_address, QuicSocketAddress initial_peer_address,
    ...
    void AddDispatcherSentPackets(absl::Span<const DispatcherSentPacket> dispatcher_sent_packets);
    ...
    virtual void ResumeConnectionState(const CachedNetworkParameters& cached_network_params, bool max_bandwidth_resumption);
    ...
    virtual void SetApplicationDrivenPacingRate(QuicBandwidth application_driven_pacing_rate);

    // Allows the client to adjust network parameters based on external
    // information.
    void AdjustNetworkParameters(const SendAlgorithmInterface::NetworkParams& params);
    void AdjustNetworkParameters(QuicBandwidth bandwidth, QuicTime::Delta rtt, bool allow_cwnd_to_decrease);

    // Install a loss detection tuner. Must be called before OnConfigNegotiated.
    void SetLossDetectionTuner(std::unique_ptr<LossDetectionTunerInterface> tuner);
    ...
    virtual QuicConsumedData SendStreamData(QuicStreamId id, size_t write_length, QuicStreamOffset offset, StreamSendingState state);
    ...
    virtual void CloseConnection(QuicErrorCode error, const std::string& details, ConnectionCloseBehavior connection_close_behavior);
    ...
    QuicConnectionStats& mutable_stats()
    {
        return stats_;
    }
    ...
    virtual void ProcessUdpPacket(const QuicSocketAddress& self_address, const QuicSocketAddress& peer_address, const QuicReceivedPacket& packet);
    ...
    bool ShouldGeneratePacket(HasRetransmittableData retransmittable, IsHandshake handshake) override;
    void MaybeBundleOpportunistically(TransmissionType transmission_type) override;
    ...
    void OnUnrecoverableError(QuicErrorCode error, const std::string& error_details) override;
    ...
    void OnSelfIssuedConnectionIdRetired(const QuicConnectionId& connection_id) override;
    ...
    void OnMultiPortPathProbingSuccess(std::unique_ptr<QuicPathValidationContext> context, QuicTime start_time);
    ...
    void set_initial_retransmittable_on_wire_timeout(QuicTime::Delta retransmittable_on_wire_timeout);
    // Used in Chromium, but not internally.
    ...
    const QuicClock* clock() const override
    {
        return clock_;
    }
    QuicRandom* random_generator() const
    {
    ...
    size_t mtu_probe_count() const
    {
        return mtu_probe_count_;
    }
    ...
    {
        return !buffered_packets_.empty();
    }
    ...
    void SetNetworkTimeouts(QuicTime::Delta handshake_timeout, QuicTime::Delta idle_timeout);
    ...
    public:
        ScopedEncryptionLevelContext(QuicConnection* connection, EncryptionLevel level);
    ...
    const QuicPacketWriter* writer() const
    {
        return writer_;
    }
    ...
    virtual bool SendConnectivityProbingPacket(QuicPacketWriter* probing_writer, const QuicSocketAddress& peer_address);
    ...
    virtual MessageStatus SendMessage(QuicMessageId message_id, absl::Span<quiche::QuicheMemSlice> message, bool flush);
    ...
    {
        return termination_info_ != nullptr && !termination_info_->termination_packets.empty();
    }

    // Returns the termination packets and clears them from the connection.
    // Postcondition: HasTerminationPackets() == false
    std::vector<std::unique_ptr<QuicEncryptedPacket>> ConsumeTerminationPackets()
    {
    ...
    const QuicConnectionHelperInterface* helper() const
    {
        return helper_;
    }
    ...
    const QuicFramer& framer() const
    {
        return framer_;
    }

    const QuicPacketCreator& packet_creator() const
    {
    ...
    bool IsPathDegrading() const
    {
        return is_path_degrading_;
    }
    ...
    void SetOriginalDestinationConnectionId(const QuicConnectionId& original_destination_connection_id);
    ...
    // Called for QUIC+TLS versions when we send transport parameters.
    void OnTransportParametersSent(const TransportParameters& transport_parameters) const;

    // Called for QUIC+TLS versions when we receive transport parameters.
    void OnTransportParametersReceived(const TransportParameters& transport_parameters) const;

    // Called for QUIC+TLS versions when we resume cached transport parameters for
    // 0-RTT.
    void OnTransportParametersResumed(const TransportParameters& transport_parameters) const;
    ...
    bool SendPathChallenge(const QuicPathFrameBuffer& data_buffer, const QuicSocketAddress& self_address, const QuicSocketAddress& peer_address,
        const QuicSocketAddress& effective_peer_address, QuicPacketWriter* writer) override;
    ...
    // kInitialRtt.
    QuicTime GetRetryTimeout(const QuicSocketAddress& peer_address_to_use, QuicPacketWriter* writer_to_use) const override;
    ...
    {
        return framer_.is_processing_packet();
    }
    ...
    bool MigratePath(const QuicSocketAddress& self_address, const QuicSocketAddress& peer_address, QuicPacketWriter* writer, bool owns_writer);

    // Called to clear the alternative_path_ when path validation failed on the
    // client side.
    void OnPathValidationFailureAtClient(bool is_multi_port, const QuicPathValidationContext& context);
    ...
    QuicConnectionContext* context() override
    {
        return &context_;
    }
    const QuicConnectionContext* context() const
    {
        return &context_;
    }

    void set_context_listener(std::unique_ptr<QuicConnectionContextListener> listener)
    {
    ...
    bool in_probe_time_out() const
    {
        return in_probe_time_out_;
    }
    ...
    static QuicTime::Delta CalculateNetworkBlackholeDelay(QuicTime::Delta blackhole_delay, QuicTime::Delta path_degrading_delay, QuicTime::Delta pto_delay);

    void DisableLivenessTesting()
    {
        liveness_testing_disabled_ = true;
    }

    void AddKnownServerAddress(const QuicSocketAddress& address);

    std::optional<QuicNewConnectionIdFrame> MaybeIssueNewConnectionIdForPreferredAddress();
    ...
    // also passed in.
    void OnServerPreferredAddressValidated(QuicPathValidationContext& context, bool owns_writer);

    void set_expected_server_preferred_address(const QuicSocketAddress& expected_server_preferred_address)
    {
    ...
    bool ignore_gquic_probing() const
    {
        return ignore_gquic_probing_;
    }
    ...
    uint32_t outgoing_flow_label() const
    {
        return outgoing_flow_label_;
    }
    ...
    std::unique_ptr<SerializedPacket> SerializeLargePacketNumberConnectionClosePacket(QuicErrorCode error, const std::string& error_details);
...
protected:
    ...
    PerPacketOptions* per_packet_options()
    {
        return per_packet_options_;
    }
    ...
    virtual void SendConnectionClosePacket(QuicErrorCode error, QuicIetfTransportErrorCodes ietf_error, const std::string& details);
...
private:
    ...
    struct QUICHE_EXPORT PathState {
        ...
        PathState(const QuicSocketAddress& alternative_self_address, const QuicSocketAddress& alternative_peer_address,
            const QuicConnectionId& client_connection_id, const QuicConnectionId& server_connection_id,
    ...
    struct QUICHE_EXPORT BufferedPacket {
        BufferedPacket(const SerializedPacket& packet, const QuicSocketAddress& self_address, const QuicSocketAddress& peer_address,
            QuicEcnCodepoint ecn_codepoint, uint32_t flow_label);
        BufferedPacket(const char* encrypted_buffer, QuicPacketLength encrypted_length, const QuicSocketAddress& self_address,
            const QuicSocketAddress& peer_address, QuicEcnCodepoint ecn_codepoint, uint32_t flow_label);
        // Please note, this buffered packet contains random bytes (and is not
        // *actually* a QUIC packet).
        BufferedPacket(QuicRandom& random, QuicPacketLength encrypted_length, const QuicSocketAddress& self_address, const QuicSocketAddress& peer_address);
    ...
    struct QUICHE_EXPORT ReceivedPacketInfo {
        ...
        ReceivedPacketInfo(const QuicSocketAddress& destination_address, const QuicSocketAddress& source_address, QuicTime receipt_time, QuicByteCount length,
            QuicEcnCodepoint ecn_codepoint, uint32_t flow_label);
    ...
    QUICHE_EXPORT friend std::ostream& operator<<(std::ostream& os, const QuicConnection::ReceivedPacketInfo& info);

    // UndecrytablePacket comprises a undecryptable packet and related
    // information.
    struct QUICHE_EXPORT UndecryptablePacket {
        UndecryptablePacket(const QuicEncryptedPacket& packet, EncryptionLevel encryption_level, const ReceivedPacketInfo& packet_info)
            : packet(packet.Clone())
    ...
    class ReversePathValidationResultDelegate : public QuicPathValidator::ResultDelegate {
    public:
        ReversePathValidationResultDelegate(QuicConnection* connection, const QuicSocketAddress& direct_peer_address);
    ...
    public:
        ...
        void OnMultiPortPathContextAvailable(std::unique_ptr<QuicPathValidationContext> path_context) override;
    ...
    // validation success.
    class MultiPortPathValidationResultDelegate : public QuicPathValidator::ResultDelegate {
    public:
    ...
    // |error| will be used.
    void TearDownLocalConnectionState(QuicErrorCode error, QuicIetfTransportErrorCodes ietf_error, const std::string& details, ConnectionCloseSource source);
    void TearDownLocalConnectionState(const QuicConnectionCloseFrame& frame, ConnectionCloseSource source);
    ...
    void ReplaceInitialServerConnectionId(const QuicConnectionId& new_server_connection_id);
    ...
    void FindMatchingOrNewClientConnectionIdOrToken(const PathState& default_path, const PathState& alternative_path,
    ...
    // otherwise.
    bool FindOnPathConnectionIds(const QuicSocketAddress& self_address, const QuicSocketAddress& peer_address, QuicConnectionId* client_connection_id,
    ...
    bool UpdateConnectionIdsOnMigration(const QuicSocketAddress& self_address, const QuicSocketAddress& peer_address);
    ...
    void QueueUndecryptablePacket(const QuicEncryptedPacket& packet, EncryptionLevel decryption_level);
    ...
    bool IsTerminationPacket(const SerializedPacket& packet, QuicErrorCode* error_code);
    ...
    QuicByteCount GetLimitedMaxPacketSize(QuicByteCount suggested_max_packet_size);
    ...
    bool SendGenericPathProbePacket(QuicPacketWriter* probing_writer, const QuicSocketAddress& peer_address);
    ...
    bool ShouldEnqueueUnDecryptablePacket(EncryptionLevel decryption_level, bool has_decryption_key) const;
    ...
    // |writer|.
    bool WritePacketUsingWriter(std::unique_ptr<SerializedPacket> packet, QuicPacketWriter* writer, const QuicSocketAddress& self_address,
    ...
    void MaybeUpdateBytesSentToAlternativeAddress(const QuicSocketAddress& peer_address, QuicByteCount sent_packet_size);
    void MaybeUpdateBytesReceivedFromAlternativeAddress(QuicByteCount received_packet_size);
    ...
    // of the default path.
    bool IsDefaultPath(const QuicSocketAddress& self_address, const QuicSocketAddress& peer_address) const;

    // Return true if the |self_address| and |peer_address| is the same as the
    // self address and peer address of the alternative path.
    bool IsAlternativePath(const QuicSocketAddress& self_address, const QuicSocketAddress& peer_address) const;
    ...
    void RestoreToLastValidatedPath(QuicSocketAddress original_direct_peer_address);
    ...
    std::unique_ptr<QuicSelfIssuedConnectionIdManager> MakeSelfIssuedConnectionIdManager();
    ...
    NewConnectionIdResult OnNewConnectionIdFrameInner(const QuicNewConnectionIdFrame& frame);
    ...
    EncryptionLevel GetEncryptionLevelToSendPingForSpace(PacketNumberSpace space) const;
    ...
    QuicEcnCodepoint GetEcnCodepointToSend(const QuicSocketAddress& destination_address) const;

    // Writes the packet to |writer| with the ECN mark specified in
    // |ecn_codepoint|. Will also set last_ecn_sent_ appropriately.
    WriteResult SendPacketToWriter(const char* buffer, size_t buf_len, const QuicIpAddress& self_address, const QuicSocketAddress& destination_address,
        QuicPacketWriter* writer, const QuicEcnCodepoint ecn_codepoint, uint32_t flow_label);
    ...
    QuicConstAlarmProxy ack_alarm() const
    {
        return alarms_.ack_alarm();
    }
    ...
    QuicConstAlarmProxy send_alarm() const
    {
        return alarms_.send_alarm();
    }
    ...
    // the current packet.
    quiche::QuicheCircularDeque<std::unique_ptr<QuicEncryptedPacket>> received_coalesced_packets_;
    ...
    quiche::QuicheCircularDeque<QuicPathFrameBuffer> received_path_challenge_payloads_;
    ...
    const bool default_enable_5rto_blackhole_detection_ = GetQuicReloadableFlag(quic_default_enable_5rto_blackhole_detection2);

    // True if next packet is intended to consume remaining space in the
    // coalescer.
    bool fill_coalesced_packet_ = false;

    size_t anti_amplification_factor_ = GetQuicFlag(quic_anti_amplification_factor);
    ...
    const bool ignore_gquic_probing_ = GetQuicReloadableFlag(quic_ignore_gquic_probing);
    ...
    // limit.
    const bool enforce_strict_amplification_factor_ = GetQuicFlag(quic_enforce_strict_amplification_factor);
    ...
    // original address.
    QuicLRUCache<QuicSocketAddress, bool, QuicSocketAddressHash> received_client_addresses_cache_;
    ...
    const bool quic_limit_new_streams_per_loop_2_ = GetQuicReloadableFlag(quic_limit_new_streams_per_loop_2);

    const bool quic_test_peer_addr_change_after_normalize_ = GetQuicReloadableFlag(quic_test_peer_addr_change_after_normalize);
