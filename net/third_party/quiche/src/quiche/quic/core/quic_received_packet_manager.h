...
public:
    ...
    QuicReceivedPacketManager& operator=(const QuicReceivedPacketManager&) = delete;
    ...
    virtual void RecordPacketReceived(const QuicPacketHeader& header, QuicTime receipt_time, QuicEcnCodepoint ecn);
    ...
    void MaybeUpdateAckTimeout(bool should_last_packet_instigate_acks, QuicPacketNumber last_received_packet_number, QuicTime last_packet_receipt_time,
        QuicTime now, const RttStats* rtt_stats);
    ...
    void set_connection_stats(QuicConnectionStats* stats)
    {
        stats_ = stats;
    }
...
private:
    ...
    QuicTime::Delta GetMaxAckDelay(QuicPacketNumber last_received_packet_number, const RttStats& rtt_stats) const;
