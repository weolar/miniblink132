...
public:
    ...
    QuicIdleNetworkDetector(Delegate* delegate, QuicTime now, QuicAlarmProxy alarm);

    void OnAlarm();

    // Called to set handshake_timeout_ and idle_network_timeout_.
    void SetTimeouts(QuicTime::Delta handshake_timeout, QuicTime::Delta idle_network_timeout);
    ...
    QuicTime::Delta handshake_timeout() const
    {
    ...
    {
        return std::max(time_of_last_received_packet_, time_of_first_packet_sent_after_receiving_);
    }

    QuicTime::Delta idle_network_timeout() const
    {
