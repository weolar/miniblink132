...
struct QUICHE_EXPORT TimeWaitConnectionInfo {
    ...
    TimeWaitConnectionInfo(bool ietf_quic, std::vector<std::unique_ptr<QuicEncryptedPacket>>* termination_packets,
...
// connection_id.
class QUICHE_EXPORT QuicTimeWaitListManager : public QuicBlockedWriterInterface {
public:
    ...
    public:
        ...
        virtual void OnConnectionAddedToTimeWaitList(QuicConnectionId connection_id) = 0;
    ...
    QuicTimeWaitListManager(QuicPacketWriter* writer, Visitor* visitor, const QuicClock* clock, QuicAlarmFactory* alarm_factory);
    ...
    virtual void AddConnectionIdToTimeWait(TimeWaitAction action, TimeWaitConnectionInfo info);
    ...
    // ReceivedPacketInfo.
    virtual void ProcessPacket(const QuicSocketAddress& self_address, const QuicSocketAddress& peer_address, QuicConnectionId connection_id,
    ...
    {
        return connection_id_map_.size();
    }
    ...
    virtual void SendVersionNegotiationPacket(QuicConnectionId server_connection_id, QuicConnectionId client_connection_id, bool ietf_quic,
    ...
    virtual void SendPublicReset(const QuicSocketAddress& self_address, const QuicSocketAddress& peer_address, QuicConnectionId connection_id, bool ietf_quic,
        size_t received_packet_length, std::unique_ptr<QuicPerPacketContext> packet_context);

    // Called to send |packet|.
    virtual void SendPacket(const QuicSocketAddress& self_address, const QuicSocketAddress& peer_address, const QuicEncryptedPacket& packet);
...
protected:
    virtual std::unique_ptr<QuicEncryptedPacket> BuildPublicReset(const QuicPublicResetPacket& packet);
    ...
    // packet.
    virtual StatelessResetToken GetStatelessResetToken(QuicConnectionId connection_id) const;

    // Internal structure to store pending termination packets.
    class QUICHE_EXPORT QueuedPacket {
    public:
        QueuedPacket(const QuicSocketAddress& self_address, const QuicSocketAddress& peer_address, std::unique_ptr<QuicEncryptedPacket> packet)
    ...
    virtual bool SendOrQueuePacket(std::unique_ptr<QueuedPacket> packet, const QuicPerPacketContext* packet_context);

    const quiche::QuicheCircularDeque<std::unique_ptr<QueuedPacket>>& pending_packets_queue() const
    {
...
private:
    ...
    virtual void OnPacketReceivedForKnownConnection(int /*num_packets*/, QuicTime::Delta /*delta*/, QuicTime::Delta /*srtt*/) const
    {
    }

    std::unique_ptr<QuicEncryptedPacket> BuildIetfStatelessResetPacket(QuicConnectionId connection_id, size_t received_packet_length);

    // A map from a recently closed connection_id to the number of packets
    // received after the termination of the connection bound to the
    // connection_id.
    struct QUICHE_EXPORT ConnectionIdData {
        ConnectionIdData(int num_packets, QuicTime time_added, TimeWaitAction action, TimeWaitConnectionInfo info);
    ...
    using ConnectionIdMap = quiche::QuicheLinkedHashMap<QuicConnectionId, ConnectionIdData, QuicConnectionIdHash>;
    ...
    absl::flat_hash_map<QuicConnectionId, QuicConnectionId, QuicConnectionIdHash> indirect_connection_id_map_;

    // Find an iterator for the given connection_id. Returns
    // connection_id_map_.end() if none found.
    ConnectionIdMap::iterator FindConnectionIdDataInMap(const QuicConnectionId& connection_id);
    // Inserts a ConnectionIdData entry to connection_id_map_.
    void AddConnectionIdDataToMap(const QuicConnectionId& canonical_connection_id, int num_packets, TimeWaitAction action, TimeWaitConnectionInfo info);
