...
inline constexpr uint64_t kNumMicrosPerSecond = kNumMicrosPerMilli * kNumMillisPerSecond;
...
inline constexpr QuicByteCount kMinimumFlowControlSendWindow = 16 * 1024; // 16 KB
// Default size of initial flow control window, for both stream and session.
inline constexpr QuicByteCount kDefaultFlowControlSendWindow = 16 * 1024; // 16 KB

// Maximum flow control receive window limits for connection and stream.
inline constexpr QuicByteCount kStreamReceiveWindowLimit = 16 * 1024 * 1024; // 16 MB
inline constexpr QuicByteCount kSessionReceiveWindowLimit = 24 * 1024 * 1024; // 24 MB

// Minimum size of the CWND, in packets, when doing bandwidth resumption.
inline constexpr QuicPacketCount kMinCongestionWindowForBandwidthResumption = 10;
...
inline constexpr uint32_t kMinUntrustedInitialRoundTripTimeUs = 10 * kNumMicrosPerMilli;

// The lower bound of a trusted initial rtt value.
inline constexpr uint32_t kMinTrustedInitialRoundTripTimeUs = 5 * kNumMicrosPerMilli;
...
inline constexpr int kUFloat16MaxExponent = (1 << kUFloat16ExponentBits) - 2; // 30
inline constexpr int kUFloat16MantissaBits = 16 - kUFloat16ExponentBits; // 11
inline constexpr int kUFloat16MantissaEffectiveBits = kUFloat16MantissaBits + 1; // 12
inline constexpr uint64_t kUFloat16MaxValue = // 0x3FFC0000000
    ((UINT64_C(1) << kUFloat16MantissaEffectiveBits) - 1) << kUFloat16MaxExponent;
...
inline constexpr QuicTime::Delta kAlarmGranularity = QuicTime::Delta::FromMilliseconds(1);
...
inline constexpr QuicTime::Delta kDefaultMultiPortProbingInterval = QuicTime::Delta::FromSeconds(3);
