...
public:
    static void SetOneImmediateAck(QuicReceivedPacketManager* manager, bool one_immediate_ack)
    {
        manager->one_immediate_ack_ = one_immediate_ack;
    }

    static void SetAckDecimationDelay(QuicReceivedPacketManager* manager, float ack_decimation_delay)
    {
...
protected:
    ...
    void RecordPacketReceipt(uint64_t packet_number, QuicTime receipt_time, QuicEcnCodepoint ecn_codepoint)
    {
    ...
    {
        received_manager_.MaybeUpdateAckTimeout(should_last_packet_instigate_acks, QuicPacketNumber(last_received_packet_number),
...
TEST_F(QuicReceivedPacketManagerTest, LimitAckRanges)
{
    ...
    for (int i = 0; i < 100; ++i) {
        ...
        EXPECT_EQ(QuicPacketNumber(1u + 2 * i), received_manager_.ack_frame().packets.Max());
        for (int j = 0; j < std::min(10, i + 1); ++j) {
            ASSERT_GE(i, j);
            EXPECT_TRUE(received_manager_.ack_frame().packets.Contains(QuicPacketNumber(1 + (i - j) * 2)));
            if (i > j) {
                EXPECT_FALSE(received_manager_.ack_frame().packets.Contains(QuicPacketNumber((i - j) * 2)));
            }
        }
    }
}
...
TEST_F(QuicReceivedPacketManagerTest, TrimAckRangesEarly)
{
    ...
    for (size_t i = 0; i < kMaxAckRanges + 10; ++i) {
        ...
        if (i < kMaxAckRanges) {
            ...
            EXPECT_EQ(kMaxAckRanges, received_manager_.ack_frame().packets.NumIntervals());
        }
    }
}
...
TEST_F(QuicReceivedPacketManagerTest, IgnoreOutOfOrderPackets)
{
    ...
    EXPECT_TRUE(received_manager_.ack_frame_updated());
    EXPECT_EQ(1u, received_manager_.ack_frame().received_packet_times.size());
    RecordPacketReceipt(4, QuicTime::Zero() + QuicTime::Delta::FromMilliseconds(1));
    EXPECT_EQ(2u, received_manager_.ack_frame().received_packet_times.size());

    RecordPacketReceipt(3, QuicTime::Zero() + QuicTime::Delta::FromMilliseconds(3));
    EXPECT_EQ(2u, received_manager_.ack_frame().received_packet_times.size());
}

TEST_F(QuicReceivedPacketManagerTest, HasMissingPackets)
{
    EXPECT_QUIC_BUG(received_manager_.PeerFirstSendingPacketNumber(), "No packets have been received yet");
    RecordPacketReceipt(4, QuicTime::Zero());
    EXPECT_EQ(QuicPacketNumber(4), received_manager_.PeerFirstSendingPacketNumber());
    EXPECT_FALSE(received_manager_.HasMissingPackets());
    RecordPacketReceipt(3, QuicTime::Zero());
    EXPECT_FALSE(received_manager_.HasMissingPackets());
    EXPECT_EQ(QuicPacketNumber(3), received_manager_.PeerFirstSendingPacketNumber());
    RecordPacketReceipt(1, QuicTime::Zero());
    EXPECT_EQ(QuicPacketNumber(1), received_manager_.PeerFirstSendingPacketNumber());
...
TEST_F(QuicReceivedPacketManagerTest, SendDelayedAckDecimationMin1ms)
{
    ...
    rtt_stats_.UpdateRtt(kAlarmGranularity, QuicTime::Delta::Zero(), clock_.ApproximateNow());
...
TEST_F(QuicReceivedPacketManagerTest, SendDelayedAckDecimationUnlimitedAggregation)
{
    EXPECT_FALSE(HasPendingAck());
    QuicConfig config;
...
{
    EXPECT_FALSE(HasPendingAck());
    QuicReceivedPacketManagerPeer::SetAckDecimationDelay(&received_manager_, 0.125);
...
TEST_F(QuicReceivedPacketManagerTest, UpdateMaxAckDelayAndAckFrequencyFromAckFrequencyFrame)
{
    EXPECT_FALSE(HasPendingAck());
...
TEST_F(QuicReceivedPacketManagerTest, DisableOutOfOrderAckByIgnoreOrderFromAckFrequencyFrame)
{
    EXPECT_FALSE(HasPendingAck());
...
TEST_F(QuicReceivedPacketManagerTest, DisableMissingPaketsAckByIgnoreOrderFromAckFrequencyFrame)
{
    EXPECT_FALSE(HasPendingAck());
    QuicConfig config;
...
TEST_F(QuicReceivedPacketManagerTest, AckDecimationDisabledWhenAckFrequencyFrameIsReceived)
{
    EXPECT_FALSE(HasPendingAck());
...
TEST_F(QuicReceivedPacketManagerTest, UpdateAckTimeoutOnPacketReceiptTime)
{
    ...
    received_manager_.MaybeUpdateAckTimeout(kInstigateAck, QuicPacketNumber(3),
        /*last_packet_receipt_time=*/packet_receipt_time3, clock_.ApproximateNow(), &rtt_stats_);
...
TEST_F(QuicReceivedPacketManagerTest, UpdateAckTimeoutOnPacketReceiptTimeLongerQueuingTime)
{
    EXPECT_FALSE(HasPendingAck());
