...
struct QUICHE_EXPORT QuicConnectionIdData {
    QuicConnectionIdData(const QuicConnectionId& connection_id, uint64_t sequence_number, const StatelessResetToken& stateless_reset_token);
...
public:
    ...
    virtual bool MaybeReserveConnectionId(const QuicConnectionId& connection_id) = 0;
    virtual void OnSelfIssuedConnectionIdRetired(const QuicConnectionId& connection_id) = 0;
};
...
public:
    ...
    QuicPeerIssuedConnectionIdManager(size_t active_connection_id_limit, const QuicConnectionId& initial_peer_issued_connection_id, const QuicClock* clock,
        QuicAlarmFactory* alarm_factory, QuicConnectionIdManagerVisitorInterface* visitor, QuicConnectionContext* context);

    ~QuicPeerIssuedConnectionIdManager();

    QuicErrorCode OnNewConnectionIdFrame(const QuicNewConnectionIdFrame& frame, std::string* error_detail, bool* is_duplicate_frame);
    ...
    void MaybeRetireUnusedConnectionIds(const std::vector<QuicConnectionId>& active_connection_ids_on_path);
    ...
    void ReplaceConnectionId(const QuicConnectionId& old_connection_id, const QuicConnectionId& new_connection_id);
...
private:
    ...
    void PrepareToRetireConnectionIdPriorTo(uint64_t retire_prior_to, std::vector<QuicConnectionIdData>* cid_data_vector);
...
public:
    QuicSelfIssuedConnectionIdManager(size_t active_connection_id_limit, const QuicConnectionId& initial_connection_id, const QuicClock* clock,
        QuicAlarmFactory* alarm_factory, QuicConnectionIdManagerVisitorInterface* visitor, QuicConnectionContext* context,
    ...
    QuicErrorCode OnRetireConnectionIdFrame(const QuicRetireConnectionIdFrame& frame, QuicTime::Delta pto_delay, std::string* error_detail);
...
private:
    ...
    // retired.
    std::vector<std::pair<QuicConnectionId, QuicTime>> to_be_retired_connection_ids_;
