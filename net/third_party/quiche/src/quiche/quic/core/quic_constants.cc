...
{
    static const QuicPacketNumber kMaxRandomInitialPacketNumber = QuicPacketNumber(0x7fffffff);
...
int64_t GetDefaultDelayedAckTimeMs()
{
    ...
    return std::min<int64_t>(GetQuicFlag(quic_default_delayed_ack_time_ms), kMinRetransmissionTimeMs / 2);
}
