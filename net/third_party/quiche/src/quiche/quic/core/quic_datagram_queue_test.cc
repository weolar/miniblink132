...
public:
    ...
    QuicDatagramQueueObserver()
        : context_(new Context())
    {
    }
    QuicDatagramQueueObserver(const QuicDatagramQueueObserver&) = delete;
    QuicDatagramQueueObserver& operator=(const QuicDatagramQueueObserver&) = delete;
...
protected:
    ...
    {
        return quiche::QuicheMemSlice(quiche::QuicheBuffer::Copy(helper_.GetStreamSendBufferAllocator(), data));
    }
...
{
    EXPECT_CALL(*connection_, SendMessage(_, _, _)).WillOnce(Return(MESSAGE_STATUS_SUCCESS));
...
{
    EXPECT_CALL(*connection_, SendMessage(_, _, _)).WillOnce(Return(MESSAGE_STATUS_BLOCKED));
    MessageStatus initial_status = queue_.SendOrQueueDatagram(CreateMemSlice("test"));
...
TEST_F(QuicDatagramQueueTest, MultipleDatagrams)
{
    ...
    EXPECT_CALL(*connection_, SendMessage(_, _, _)).Times(5).WillRepeatedly(Return(MESSAGE_STATUS_SUCCESS));
    size_t num_messages = queue_.SendDatagrams();
...
{
    EXPECT_EQ(QuicTime::Delta::Zero(), connection_->sent_packet_manager().GetRttStats()->min_rtt());
    EXPECT_EQ(QuicTime::Delta::FromMilliseconds(4), queue_.GetMaxTimeInQueue());

    RttStats* stats = const_cast<RttStats*>(connection_->sent_packet_manager().GetRttStats());
    stats->UpdateRtt(QuicTime::Delta::FromMilliseconds(100), QuicTime::Delta::Zero(), helper_.GetClock()->Now());
...
TEST_F(QuicDatagramQueueTest, Expiry)
{
    ...
    EXPECT_CALL(*connection_, SendMessage(_, _, _))
        .WillRepeatedly([&messages](QuicMessageId /*id*/, absl::Span<quiche::QuicheMemSlice> message, bool /*flush*/) {
...
protected:
    ...
    quiche::QuicheReferenceCountedPointer<QuicDatagramQueueObserver::Context> context_;
    QuicDatagramQueue queue_;
};
...
TEST_F(QuicDatagramQueueWithObserverTest, ObserveSuccessImmediately)
{
    ...
    EXPECT_EQ(MESSAGE_STATUS_SUCCESS, queue_.SendOrQueueDatagram(CreateMemSlice("a")));
...
TEST_F(QuicDatagramQueueWithObserverTest, ObserveFailureImmediately)
{
    ...
    EXPECT_CALL(*connection_, SendMessage(_, _, _)).WillOnce(Return(MESSAGE_STATUS_TOO_LARGE));

    EXPECT_EQ(MESSAGE_STATUS_TOO_LARGE, queue_.SendOrQueueDatagram(CreateMemSlice("a")));
...
TEST_F(QuicDatagramQueueWithObserverTest, BlockingShouldNotBeObserved)
{
    ...
    EXPECT_CALL(*connection_, SendMessage(_, _, _)).WillRepeatedly(Return(MESSAGE_STATUS_BLOCKED));

    EXPECT_EQ(MESSAGE_STATUS_BLOCKED, queue_.SendOrQueueDatagram(CreateMemSlice("a")));
    EXPECT_EQ(0u, queue_.SendDatagrams());
