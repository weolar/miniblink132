...
public:
    ...
    void EstablishZeroRttEncryption()
    {
        ...
        session()->connection()->SetEncrypter(ENCRYPTION_ZERO_RTT, std::make_unique<NullEncrypter>(session()->perspective()));
    }
    ...
    void OnHandshakeMessage(const CryptoHandshakeMessage& /*message*/) override
    {
        ...
        session()->OnNewEncryptionKeyAvailable(ENCRYPTION_FORWARD_SECURE, std::make_unique<NullEncrypter>(session()->perspective()));
    ...
    std::string GetAddressToken(const CachedNetworkParameters* /*cached_network_parameters*/) const override
    {
        return "";
    }
    ...
    MOCK_METHOD(std::unique_ptr<QuicDecrypter>, AdvanceKeysAndCreateCurrentOneRttDecrypter, (), (override));
    MOCK_METHOD(std::unique_ptr<QuicEncrypter>, CreateCurrentOneRttEncrypter, (), (override));
    ...
    bool ExportKeyingMaterial(absl::string_view /*label*/, absl::string_view /*context*/, size_t /*result_len*/, std::string* /*result*/) override
    {
        return false;
    }
...
public:
    ...
    TestStream(QuicStreamId id, QuicSession* session, bool is_static, StreamType type)
...
public:
    explicit TestSession(QuicConnection* connection, MockQuicSessionVisitor* session_visitor)
        : QuicSession(connection, session_visitor, DefaultQuicConfig(), CurrentSupportedVersions(),
        ...
        , num_incoming_streams_created_(0)
    {
        ...
        Initialize();
        this->connection()->SetEncrypter(ENCRYPTION_FORWARD_SECURE, std::make_unique<NullEncrypter>(connection->perspective()));
    ...
    TestStream* CreateOutgoingBidirectionalStream()
    {
        ...
        if (id == QuicUtils::GetInvalidStreamId(connection()->transport_version())) {
            return nullptr;
        }
    ...
    QuicStream* ProcessBidirectionalPendingStream(PendingStream* pending) override
    {
        return CreateIncomingStream(pending);
    }
    QuicStream* ProcessReadUnidirectionalPendingStream(PendingStream* pending) override
    {
        struct iovec iov;
    ...
    MOCK_METHOD(void, OnCanCreateNewOutgoingStream, (bool unidirectional), (override));
    ...
    {
        if (!QuicUtils::IsCryptoStreamId(connection()->transport_version(), stream->id())
    ...
    bool UsesPendingStreamForFrame(QuicFrameType type, QuicStreamId stream_id) const override
    {
        ...
        StreamType stream_type = QuicUtils::GetStreamType(stream_id, perspective(), is_incoming_stream, version());
        switch (type) {
        case STREAM_FRAME:
...
protected:
        ...
        , configure_session_(configure_session)
    {
        ...
        if (configure_session) {
            ...
            QuicConfigPeer::SetReceivedMaxBidirectionalStreams(session_.config(), kDefaultMaxStreamsPerConnection);
            QuicConfigPeer::SetReceivedMaxUnidirectionalStreams(session_.config(), kDefaultMaxStreamsPerConnection);
    ...
    {
        QuicStreamId first_stream_id = QuicUtils::GetFirstBidirectionalStreamId(connection_->transport_version(), Perspective::IS_CLIENT);
        if (!QuicVersionUsesCryptoFrames(connection_->transport_version())) {
            first_stream_id = QuicUtils::GetCryptoStreamId(connection_->transport_version());
        }
    ...
    {
        if (VersionHasIetfQuicFrames(transport_version())) {
            if (QuicUtils::GetStreamType(id, session_.perspective(), session_.IsIncomingStream(id), connection_->version()) == READ_UNIDIRECTIONAL) {
                // Verify STOP_SENDING but no RST_STREAM is sent for
                // READ_UNIDIRECTIONAL streams.
                EXPECT_CALL(*connection_, SendControlFrame(IsFrame(STOP_SENDING_FRAME))).Times(1).WillOnce(Invoke(&ClearControlFrame));
                EXPECT_CALL(*connection_, OnStreamReset(id, _)).Times(1);
            } else if (QuicUtils::GetStreamType(id, session_.perspective(), session_.IsIncomingStream(id), connection_->version()) == WRITE_UNIDIRECTIONAL) {
                // Verify RST_STREAM but not STOP_SENDING is sent for write-only
                // stream.
                EXPECT_CALL(*connection_, SendControlFrame(IsFrame(RST_STREAM_FRAME))).Times(1).WillOnce(Invoke(&ClearControlFrame));
                EXPECT_CALL(*connection_, OnStreamReset(id, _));
            } else {
                // Verify RST_STREAM and STOP_SENDING are sent for BIDIRECTIONAL
                // streams.
                EXPECT_CALL(*connection_, SendControlFrame(IsFrame(RST_STREAM_FRAME))).WillRepeatedly(Invoke(&ClearControlFrame));
                EXPECT_CALL(*connection_, SendControlFrame(IsFrame(STOP_SENDING_FRAME))).WillRepeatedly(Invoke(&ClearControlFrame));
                EXPECT_CALL(*connection_, OnStreamReset(id, _));
            }
        } else {
    ...
    {
        return QuicUtils::GetFirstBidirectionalStreamId(connection_->transport_version(), Perspective::IS_CLIENT)
            + QuicUtils::StreamIdDelta(connection_->transport_version()) * n;
    }

    QuicStreamId GetNthClientInitiatedUnidirectionalId(int n)
    {
        return QuicUtils::GetFirstUnidirectionalStreamId(connection_->transport_version(), Perspective::IS_CLIENT)
            + QuicUtils::StreamIdDelta(connection_->transport_version()) * n;
    }

    QuicStreamId GetNthServerInitiatedBidirectionalId(int n)
    {
        return QuicUtils::GetFirstBidirectionalStreamId(connection_->transport_version(), Perspective::IS_SERVER)
            + QuicUtils::StreamIdDelta(connection_->transport_version()) * n;
    }

    QuicStreamId GetNthServerInitiatedUnidirectionalId(int n)
    {
        return QuicUtils::GetFirstUnidirectionalStreamId(connection_->transport_version(), Perspective::IS_SERVER)
            + QuicUtils::StreamIdDelta(connection_->transport_version()) * n;
    }
...
public:
    ...
    WriteResult CheckMultiPathResponse(
        const char* buffer, size_t buf_len, const QuicIpAddress& /*self_address*/, const QuicSocketAddress& /*peer_address*/, PerPacketOptions* /*options*/)
    {
        QuicEncryptedPacket packet(buffer, buf_len);
        {
            ...
            EXPECT_CALL(framer_visitor_, OnPathResponseFrame(_)).WillOnce(WithArg<0>(Invoke([this](const QuicPathResponseFrame& frame) {
...
protected:
    QuicSessionTestServer()
        ...
        , client_framer_(SupportedVersions(GetParam()), QuicTime::Zero(), Perspective::IS_CLIENT, kQuicDefaultConnectionIdLength)
    {
        client_framer_.set_visitor(&framer_visitor_);
        client_framer_.SetInitialObfuscators(TestConnectionId());
        if (client_framer_.version().KnowsWhichDecrypterToUse()) {
            client_framer_.InstallDecrypter(ENCRYPTION_FORWARD_SECURE, std::make_unique<NullDecrypter>(Perspective::IS_CLIENT));
        }
    }
...
INSTANTIATE_TEST_SUITE_P(Tests, QuicSessionTestServer, ::testing::ValuesIn(AllSupportedVersions()), ::testing::PrintToStringParamName());
...
{
    EXPECT_CALL(*connection_, CloseConnection(_, _, _)).WillOnce(Invoke(connection_, &MockQuicConnection::ReallyCloseConnection));
    connection_->CloseConnection(QUIC_NO_ERROR, "Everything is fine.", ConnectionCloseBehavior::SILENT_CLOSE);
...
{
    ASSERT_TRUE(session_.GetOrCreateStream(GetNthClientInitiatedBidirectionalId(3)) != nullptr);
    ...
    EXPECT_TRUE(QuicSessionPeer::IsStreamAvailable(&session_, GetNthClientInitiatedBidirectionalId(2)));
...
{
    ASSERT_TRUE(session_.GetOrCreateStream(GetNthClientInitiatedUnidirectionalId(3)) != nullptr);
    // Smaller unidirectional streams should be available.
    EXPECT_TRUE(QuicSessionPeer::IsStreamAvailable(&session_, GetNthClientInitiatedUnidirectionalId(1)));
    EXPECT_TRUE(QuicSessionPeer::IsStreamAvailable(&session_, GetNthClientInitiatedUnidirectionalId(2)));
    ASSERT_TRUE(session_.GetOrCreateStream(GetNthClientInitiatedUnidirectionalId(2)) != nullptr);
    ASSERT_TRUE(session_.GetOrCreateStream(GetNthClientInitiatedUnidirectionalId(1)) != nullptr);
}

TEST_P(QuicSessionTestServer, MaxAvailableBidirectionalStreams)
{
    if (VersionHasIetfQuicFrames(transport_version())) {
        EXPECT_EQ(session_.max_open_incoming_bidirectional_streams(), session_.MaxAvailableBidirectionalStreams());
    } else {
        ...
        EXPECT_EQ(session_.max_open_incoming_bidirectional_streams() * kMaxAvailableStreamsMultiplier, session_.MaxAvailableBidirectionalStreams());
    }
}
...
{
    if (VersionHasIetfQuicFrames(transport_version())) {
        EXPECT_EQ(session_.max_open_incoming_unidirectional_streams(), session_.MaxAvailableUnidirectionalStreams());
    } else {
        // The protocol specification requires that there can be at least 10 times
        // as many available streams as the connection's maximum open streams.
...
TEST_P(QuicSessionTestServer, IsClosedBidirectionalStreamPeerCreated)
{
    ...
    QuicStream* stream3 = session_.GetOrCreateStream(stream_id2 + 2 * QuicUtils::StreamIdDelta(connection_->transport_version()));
    CheckClosedStreams();
...
TEST_P(QuicSessionTestServer, MaximumAvailableOpenedBidirectionalStreams)
{
    ...
    EXPECT_NE(nullptr, session_.GetOrCreateStream(GetNthClientInitiatedBidirectionalId(session_.max_open_incoming_bidirectional_streams() - 1)));
}
...
TEST_P(QuicSessionTestServer, MaximumAvailableOpenedUnidirectionalStreams)
{
    ...
    session_.GetOrCreateStream(stream_id);
    EXPECT_CALL(*connection_, CloseConnection(_, _, _)).Times(0);
    EXPECT_NE(nullptr, session_.GetOrCreateStream(GetNthClientInitiatedUnidirectionalId(session_.max_open_incoming_unidirectional_streams() - 1)));
}
...
TEST_P(QuicSessionTestServer, TooManyAvailableBidirectionalStreams)
{
    ...
    stream_id2 = GetNthClientInitiatedBidirectionalId(session_.MaxAvailableBidirectionalStreams() + 2);
    if (VersionHasIetfQuicFrames(transport_version())) {
...
TEST_P(QuicSessionTestServer, TooManyAvailableUnidirectionalStreams)
{
    ...
    EXPECT_NE(nullptr, session_.GetOrCreateStream(stream_id1));
    // A stream ID which is too large to create.
    stream_id2 = GetNthClientInitiatedUnidirectionalId(session_.MaxAvailableUnidirectionalStreams() + 2);
    if (VersionHasIetfQuicFrames(transport_version())) {
        // IETF QUIC terminates the connection with invalid stream id
        EXPECT_CALL(*connection_, CloseConnection(QUIC_INVALID_STREAM_ID, _, _));
    } else {
...
TEST_P(QuicSessionTestServer, ManyAvailableBidirectionalStreams)
{
    ...
    EXPECT_CALL(*connection_, CloseConnection(_, _, _)).Times(0);
    EXPECT_NE(nullptr, session_.GetOrCreateStream(GetNthClientInitiatedBidirectionalId(199)));
    ...
    if (VersionHasIetfQuicFrames(transport_version())) {
        ...
        EXPECT_NE(nullptr, session_.GetOrCreateStream(GetNthClientInitiatedUnidirectionalId(49)));
        // and this should fail because it exceeds the unidirectional limit
        // (but not the bi-)
        EXPECT_CALL(*connection_,
            CloseConnection(QUIC_INVALID_STREAM_ID, "Stream id 798 would exceed stream count limit 50", ConnectionCloseBehavior::SEND_CONNECTION_CLOSE_PACKET))
            .Times(1);
        EXPECT_EQ(nullptr, session_.GetOrCreateStream(GetNthClientInitiatedUnidirectionalId(199)));
    }
}
...
TEST_P(QuicSessionTestServer, ManyAvailableUnidirectionalStreams)
{
    ...
    // GetNth... starts at 0, so for 200 streams, get the 199th.
    EXPECT_CALL(*connection_, CloseConnection(_, _, _)).Times(0);
    EXPECT_NE(nullptr, session_.GetOrCreateStream(GetNthClientInitiatedUnidirectionalId(199)));
    if (VersionHasIetfQuicFrames(transport_version())) {
        ...
        EXPECT_NE(nullptr, session_.GetOrCreateStream(GetNthClientInitiatedBidirectionalId(49)));
        ...
        EXPECT_CALL(*connection_, CloseConnection(QUIC_INVALID_STREAM_ID, error_detail, ConnectionCloseBehavior::SEND_CONNECTION_CLOSE_PACKET)).Times(1);
        EXPECT_EQ(nullptr, session_.GetOrCreateStream(GetNthClientInitiatedBidirectionalId(199)));
    }
}
...
TEST_P(QuicSessionTestServer, OnCanWrite)
{
    ...
    if (!GetQuicReloadableFlag(quic_disable_batch_write) || GetQuicReloadableFlag(quic_priority_respect_incremental)) {
...
TEST_P(QuicSessionTestServer, TestBatchedWrites)
{
    ...
    const QuicStreamPriority priority(HttpStreamPriority { HttpStreamPriority::kDefaultUrgency,
        /* incremental = */ true });
    ...
    stream6->SetPriority(QuicStreamPriority(HttpStreamPriority { kV3HighestPriority, HttpStreamPriority::kDefaultIncremental }));
    if (GetQuicReloadableFlag(quic_disable_batch_write)) {
        EXPECT_CALL(*stream2, OnCanWrite()).WillOnce(Invoke([this, stream2, stream6]() {
            session_.SendLargeFakeData(stream2, 6000);
            session_.MarkConnectionLevelWriteBlocked(stream2->id());
            session_.MarkConnectionLevelWriteBlocked(stream6->id());
        }));
    } else {
        EXPECT_CALL(*stream4, OnCanWrite()).WillOnce(Invoke([this, stream4, stream6]() {
            session_.SendLargeFakeData(stream4, 6000);
            session_.MarkConnectionLevelWriteBlocked(stream4->id());
            session_.MarkConnectionLevelWriteBlocked(stream6->id());
        }));
    ...
    EXPECT_CALL(*stream6, OnCanWrite()).WillOnce(Invoke([this, stream4, stream6]() {
...
TEST_P(QuicSessionTestServer, OnCanWriteBundlesStreams)
{
    ...
    CompleteHandshake();
    MockPacketWriter* writer = static_cast<MockPacketWriter*>(QuicConnectionPeer::GetWriter(session_.connection()));
    ...
    EXPECT_CALL(*writer, WritePacket(_, _, _, _, _, _)).WillOnce(Return(WriteResult(WRITE_STATUS_OK, 0)));
...
TEST_P(QuicSessionTestServer, SendStreamsBlocked)
{
    ...
    EXPECT_CALL(*connection_, SendControlFrame(_)).WillOnce(Invoke([](const QuicFrame& frame) {
        EXPECT_FALSE(frame.streams_blocked_frame.unidirectional);
        EXPECT_EQ(kDefaultMaxStreamsPerConnection, frame.streams_blocked_frame.stream_count);
...
TEST_P(QuicSessionTestServer, LimitMaxStreams)
{
    ...
    EXPECT_EQ(kMaxStreams, QuicSessionPeer::ietf_streamid_manager(&session_)->advertised_max_incoming_bidirectional_streams());
    ...
    EXPECT_CALL(*connection_, SendControlFrame(IsFrame(MAX_STREAMS_FRAME))).Times(2).WillRepeatedly(Invoke([&max_stream_frames](const QuicFrame& frame) {
    ...
    EXPECT_EQ(2 * kMaxStreams, QuicSessionPeer::ietf_streamid_manager(&session_)->advertised_max_incoming_bidirectional_streams());
    ...
    for (size_t i = 0; i < kMaxStreams; ++i) {
        QuicStreamId stream_id = GetNthClientInitiatedBidirectionalId(i + kMaxStreams);
        QuicStreamFrame data1(stream_id, true, 0, absl::string_view("HT"));
        session_.OnStreamFrame(data1);
    ...
    EXPECT_CALL(*connection_, SendControlFrame(IsFrame(MAX_STREAMS_FRAME))).WillOnce(Invoke(&ClearControlFrame));
    session_.OnFrameAcked(QuicFrame(max_stream_frames[0]), QuicTime::Delta::Zero(), QuicTime::Zero());
    EXPECT_EQ(3 * kMaxStreams, QuicSessionPeer::ietf_streamid_manager(&session_)->advertised_max_incoming_bidirectional_streams());
    ...
    for (size_t i = 0; i < kMaxStreams; ++i) {
        QuicStreamId stream_id = GetNthClientInitiatedBidirectionalId(i + 2 * kMaxStreams);
        QuicStreamFrame data1(stream_id, true, 0, absl::string_view("HT"));
        session_.OnStreamFrame(data1);
    }
    ...
    session_.OnFrameAcked(QuicFrame(max_stream_frames[1]), QuicTime::Delta::Zero(), QuicTime::Zero());
}
...
TEST_P(QuicSessionTestServer, BufferedHandshake)
{
    ...
    session_.MarkConnectionLevelWriteBlocked(QuicUtils::GetCryptoStreamId(connection_->transport_version()));
...
TEST_P(QuicSessionTestServer, ServerReplyToConnectivityProbe)
{
    ...
    MockPacketWriter* writer = static_cast<MockPacketWriter*>(QuicConnectionPeer::GetWriter(session_.connection()));
    EXPECT_CALL(*writer, WritePacket(_, _, _, new_peer_address, _, _)).WillOnce(Return(WriteResult(WRITE_STATUS_OK, 0)));

    EXPECT_CALL(*connection_, SendConnectivityProbingPacket(_, _)).WillOnce(Invoke(connection_, &MockQuicConnection::ReallySendConnectivityProbingPacket));
...
TEST_P(QuicSessionTestServer, OnStreamFrameFinStaticStreamId)
{
    ...
    QuicStreamId headers_stream_id = QuicUtils::GetHeadersStreamId(connection_->transport_version());
    std::unique_ptr<TestStream> fake_headers_stream = std::make_unique<TestStream>(headers_stream_id, &session_,
...
{
    // Send two bytes of payload.
    QuicStreamFrame data1(QuicUtils::GetInvalidStreamId(connection_->transport_version()), true, 0, absl::string_view("HT"));
    EXPECT_CALL(
...
{
    // Send two bytes of payload.
    QuicRstStreamFrame rst1(kInvalidControlFrameId, QuicUtils::GetInvalidStreamId(connection_->transport_version()), QUIC_ERROR_PROCESSING_STREAM, 0);
    EXPECT_CALL(
        *connection_, CloseConnection(QUIC_INVALID_STREAM_ID, "Received data for an invalid stream", ConnectionCloseBehavior::SEND_CONNECTION_CLOSE_PACKET));
...
TEST_P(QuicSessionTestServer, OnResetStreamAtInvalidStreamId)
{
    ...
    // Send two bytes of payload.
    QuicResetStreamAtFrame rst1(kInvalidControlFrameId, QuicUtils::GetInvalidStreamId(connection_->transport_version()), QUIC_ERROR_PROCESSING_STREAM, 10, 0);
    EXPECT_CALL(
        *connection_, CloseConnection(QUIC_INVALID_STREAM_ID, "Received data for an invalid stream", ConnectionCloseBehavior::SEND_CONNECTION_CLOSE_PACKET));
...
TEST_P(QuicSessionTestServer, ConnectionFlowControlAccountingRstOutOfOrder)
{
    ...
    if (VersionHasIetfQuicFrames(transport_version())) {
        ...
        QuicStopSendingFrame frame(kInvalidControlFrameId, stream->id(), QUIC_STREAM_CANCELLED);
        EXPECT_CALL(*connection_, CloseConnection(_, _, _)).Times(0);
...
TEST_P(QuicSessionTestServer, ConnectionFlowControlAccountingFinAndLocalReset)
{
    ...
    EXPECT_EQ(kByteOffset + frame.data_length, stream->highest_received_byte_offset());

    // Reset stream locally.
    EXPECT_CALL(*connection_, SendControlFrame(_));
    EXPECT_CALL(*connection_, OnStreamReset(stream->id(), _));
    stream->Reset(QUIC_STREAM_CANCELLED);
    EXPECT_EQ(kByteOffset + frame.data_length, session_.flow_controller()->bytes_consumed());
}
...
TEST_P(QuicSessionTestServer, ConnectionFlowControlAccountingFinAfterRst)
{
    ...
    EXPECT_LT(kInitialConnectionBytesConsumed, kInitialConnectionHighestReceivedOffset);
    session_.flow_controller()->UpdateHighestReceivedOffset(kInitialConnectionHighestReceivedOffset);
    ...
    QuicStreamFrame frame(stream->id(), true, kByteOffset, absl::string_view(body));
    session_.OnStreamFrame(frame);

    QuicStreamOffset total_stream_bytes_sent_by_peer = kByteOffset + body.length();
    EXPECT_EQ(kInitialConnectionBytesConsumed + total_stream_bytes_sent_by_peer, session_.flow_controller()->bytes_consumed());
    EXPECT_EQ(kInitialConnectionHighestReceivedOffset + total_stream_bytes_sent_by_peer, session_.flow_controller()->highest_received_byte_offset());
}
...
TEST_P(QuicSessionTestServer, ConnectionFlowControlAccountingRstAfterRst)
{
    ...
    EXPECT_EQ(kInitialConnectionBytesConsumed + kByteOffset, session_.flow_controller()->bytes_consumed());
    EXPECT_EQ(kInitialConnectionHighestReceivedOffset + kByteOffset, session_.flow_controller()->highest_received_byte_offset());
}
...
TEST_P(QuicSessionTestServer, FlowControlWithInvalidFinalOffset)
{
    ...
    EXPECT_CALL(*connection_, CloseConnection(QUIC_FLOW_CONTROL_RECEIVED_TOO_MUCH_DATA, _, _)).Times(2);
    ...
    QuicRstStreamFrame rst_frame(kInvalidControlFrameId, stream->id(), QUIC_STREAM_CANCELLED, kLargeOffset);
    session_.OnRstStream(rst_frame);
}
...
TEST_P(QuicSessionTestServer, TooManyUnfinishedStreamsCauseServerRejectStream)
{
    ...
    for (QuicStreamId i = kFirstStreamId; i < kFinalStreamId; i += QuicUtils::StreamIdDelta(connection_->transport_version())) {
    ...
    if (VersionHasIetfQuicFrames(transport_version())) {
        EXPECT_CALL(*connection_, CloseConnection(QUIC_INVALID_STREAM_ID, "Stream id 20 would exceed stream count limit 5", _));
    } else {
...
TEST_P(QuicSessionTestServer, BufferAllIncomingStreams)
{
    ...
    QuicStreamId bidirectional_stream_id = QuicUtils::GetFirstBidirectionalStreamId(transport_version(), Perspective::IS_CLIENT);
    QuicStreamFrame data3(bidirectional_stream_id, false, 0, absl::string_view("HT"));
    session_.OnStreamFrame(data3);
    EXPECT_TRUE(QuicSessionPeer::GetPendingStream(&session_, bidirectional_stream_id));
    EXPECT_EQ(0, session_.num_incoming_streams_created());
    ...
    EXPECT_FALSE(QuicSessionPeer::GetPendingStream(&session_, stream_id));
    EXPECT_FALSE(QuicSessionPeer::GetPendingStream(&session_, bidirectional_stream_id));
    EXPECT_EQ(2, session_.num_incoming_streams_created());
    EXPECT_EQ(1, QuicSessionPeer::GetStream(&session_, stream_id)->pending_duration().ToMilliseconds());
    EXPECT_EQ(1, QuicSessionPeer::GetStream(&session_, bidirectional_stream_id)->pending_duration().ToMilliseconds());
...
TEST_P(QuicSessionTestServer, RstPendingStreams)
{
    ...
    QuicRstStreamFrame rst1(kInvalidControlFrameId, stream_id, QUIC_ERROR_PROCESSING_STREAM, 12);
    session_.OnRstStream(rst1);
    EXPECT_FALSE(QuicSessionPeer::GetPendingStream(&session_, stream_id));
    EXPECT_EQ(0, session_.num_incoming_streams_created());
    EXPECT_EQ(0u, QuicSessionPeer::GetNumOpenDynamicStreams(&session_));
    ...
    QuicRstStreamFrame rst2(kInvalidControlFrameId, bidirectional_stream_id, QUIC_ERROR_PROCESSING_STREAM, 12);
...
TEST_P(QuicSessionTestServer, OnFinPendingStreamsBidirectional)
{
    ...
    // Bidirectional stream is buffered.
    QuicStreamId bidirectional_stream_id = QuicUtils::GetFirstBidirectionalStreamId(transport_version(), Perspective::IS_CLIENT);
    QuicStreamFrame data2(bidirectional_stream_id, true, 0, absl::string_view("HT"));
    session_.OnStreamFrame(data2);
    EXPECT_TRUE(QuicSessionPeer::GetPendingStream(&session_, bidirectional_stream_id));
    EXPECT_EQ(0, session_.num_incoming_streams_created());
    ...
    EXPECT_FALSE(QuicSessionPeer::GetPendingStream(&session_, bidirectional_stream_id));
    EXPECT_EQ(1, session_.num_incoming_streams_created());
    QuicStream* bidirectional_stream = QuicSessionPeer::GetStream(&session_, bidirectional_stream_id);
...
TEST_P(QuicSessionTestServer, UnidirectionalPendingStreamOnWindowUpdate)
{
    ...
    EXPECT_TRUE(QuicSessionPeer::GetPendingStream(&session_, stream_id));
    EXPECT_EQ(0, session_.num_incoming_streams_created());
    QuicWindowUpdateFrame window_update_frame(kInvalidControlFrameId, stream_id, 0);
    EXPECT_CALL(*connection_,
        CloseConnection(QUIC_WINDOW_UPDATE_RECEIVED_ON_READ_UNIDIRECTIONAL_STREAM, "WindowUpdateFrame received on READ_UNIDIRECTIONAL stream.", _));
...
TEST_P(QuicSessionTestServer, BidirectionalPendingStreamOnWindowUpdate)
{
    ...
    session_.set_uses_pending_streams(true);
    QuicStreamId stream_id = QuicUtils::GetFirstBidirectionalStreamId(transport_version(), Perspective::IS_CLIENT);
    QuicStreamFrame data(stream_id, true, 10, absl::string_view("HT"));
    session_.OnStreamFrame(data);
    QuicWindowUpdateFrame window_update_frame(kInvalidControlFrameId, stream_id, kDefaultFlowControlSendWindow * 2);
    session_.OnWindowUpdateFrame(window_update_frame);
    EXPECT_TRUE(QuicSessionPeer::GetPendingStream(&session_, stream_id));
    EXPECT_EQ(0, session_.num_incoming_streams_created());
    ...
    EXPECT_FALSE(QuicSessionPeer::GetPendingStream(&session_, stream_id));
    EXPECT_EQ(1, session_.num_incoming_streams_created());
    QuicStream* bidirectional_stream = QuicSessionPeer::GetStream(&session_, stream_id);
    QuicByteCount send_window = QuicStreamPeer::SendWindowSize(bidirectional_stream);
...
TEST_P(QuicSessionTestServer, UnidirectionalPendingStreamOnStopSending)
{
    ...
    EXPECT_TRUE(QuicSessionPeer::GetPendingStream(&session_, stream_id));
    EXPECT_EQ(0, session_.num_incoming_streams_created());
    QuicStopSendingFrame stop_sending_frame(kInvalidControlFrameId, stream_id, QUIC_STREAM_CANCELLED);
    EXPECT_CALL(*connection_, CloseConnection(QUIC_INVALID_STREAM_ID, "Received STOP_SENDING for a read-only stream", _));
...
TEST_P(QuicSessionTestServer, DrainingStreamsDoNotCountAsOpened)
{
    ...
    const QuicStreamId kFirstStreamId = GetNthClientInitiatedBidirectionalId(0);
    const QuicStreamId kFinalStreamId = GetNthClientInitiatedBidirectionalId(2 * kMaxStreams + 1);
    for (QuicStreamId i = kFirstStreamId; i < kFinalStreamId; i += QuicUtils::StreamIdDelta(connection_->transport_version())) {
...
INSTANTIATE_TEST_SUITE_P(Tests, QuicSessionTestClient, ::testing::ValuesIn(AllSupportedVersions()), ::testing::PrintToStringParamName());
...
TEST_P(QuicSessionTestClient, AvailableBidirectionalStreamsClient)
{
    ...
    EXPECT_FALSE(QuicSessionPeer::IsStreamAvailable(&session_, GetNthClientInitiatedBidirectionalId(1)));
}
...
TEST_P(QuicSessionTestClient, DonotSendRetireCIDFrameWhenConnectionClosed)
{
    ...
    connection_->ReallyCloseConnection(QUIC_NO_ERROR, "closing", ConnectionCloseBehavior::SILENT_CLOSE);
    EXPECT_FALSE(connection_->connected());
    if (!GetQuicReloadableFlag(quic_no_write_control_frame_upon_connection_close2)) {
        EXPECT_QUIC_BUG(session_.SendRetireConnectionId(20), "Try to write control frame");
    } else {
...
TEST_P(QuicSessionTestClient, InvalidSessionFlowControlWindowInHandshake)
{
    ...
    // torn down.
    const uint32_t kInvalidWindow = kMinimumFlowControlSendWindow - 1;
    QuicConfigPeer::SetReceivedInitialSessionFlowControlWindow(session_.config(), kInvalidWindow);
    EXPECT_CALL(*connection_,
        CloseConnection(connection_->version().AllowsLowFlowControlLimits() ? QUIC_ZERO_RTT_RESUMPTION_LIMIT_REDUCED : QUIC_FLOW_CONTROL_INVALID_WINDOW, _, _));
    connection_->SetDefaultEncryptionLevel(ENCRYPTION_FORWARD_SECURE);
    session_.OnConfigNegotiated();
}
...
TEST_P(QuicSessionTestClient, InvalidBidiStreamLimitInHandshake)
{
    ...
    QuicConfigPeer::SetReceivedMaxBidirectionalStreams(session_.config(), kDefaultMaxStreamsPerConnection - 1);
    EXPECT_CALL(*connection_, CloseConnection(QUIC_ZERO_RTT_RESUMPTION_LIMIT_REDUCED, _, _));
    connection_->SetDefaultEncryptionLevel(ENCRYPTION_FORWARD_SECURE);
    session_.OnConfigNegotiated();
}
...
TEST_P(QuicSessionTestClient, InvalidUniStreamLimitInHandshake)
{
    ...
    QuicConfigPeer::SetReceivedMaxUnidirectionalStreams(session_.config(), kDefaultMaxStreamsPerConnection - 1);
    EXPECT_CALL(*connection_, CloseConnection(QUIC_ZERO_RTT_RESUMPTION_LIMIT_REDUCED, _, _));
    connection_->SetDefaultEncryptionLevel(ENCRYPTION_FORWARD_SECURE);
    session_.OnConfigNegotiated();
}
...
TEST_P(QuicSessionTestClient, InvalidStreamFlowControlWindowInHandshake)
{
    ...
    session_.CreateOutgoingBidirectionalStream();
    session_.CreateOutgoingBidirectionalStream();
    QuicConfigPeer::SetReceivedInitialMaxStreamDataBytesOutgoingBidirectional(session_.config(), kMinimumFlowControlSendWindow - 1);
...
{
    ASSERT_TRUE(session_.GetOrCreateStream(GetNthServerInitiatedUnidirectionalId(2)) != nullptr);
    // Smaller unidirectional streams should be available.
    EXPECT_TRUE(QuicSessionPeer::IsStreamAvailable(&session_, GetNthServerInitiatedUnidirectionalId(0)));
    EXPECT_TRUE(QuicSessionPeer::IsStreamAvailable(&session_, GetNthServerInitiatedUnidirectionalId(1)));
    ASSERT_TRUE(session_.GetOrCreateStream(GetNthServerInitiatedUnidirectionalId(0)) != nullptr);
    ASSERT_TRUE(session_.GetOrCreateStream(GetNthServerInitiatedUnidirectionalId(1)) != nullptr);
    // And 5 should be not available.
    EXPECT_FALSE(QuicSessionPeer::IsStreamAvailable(&session_, GetNthClientInitiatedUnidirectionalId(1)));
}
...
{
    const QuicErrorCode expected_error = VersionHasIetfQuicFrames(transport_version()) ? QUIC_HTTP_STREAM_WRONG_DIRECTION : QUIC_INVALID_STREAM_ID;
    EXPECT_CALL(*connection_, CloseConnection(expected_error, "Data for nonexistent stream", ConnectionCloseBehavior::SEND_CONNECTION_CLOSE_PACKET));

    QuicStreamFrame frame(GetNthClientInitiatedBidirectionalId(1),
        /* fin = */ false, /* offset = */ 0, absl::string_view("foo"));
    session_.OnStreamFrame(frame);
}
...
TEST_P(QuicSessionTestClient, MinAckDelaySetOnTheClientQuicConfig)
{
    ...
    session_.Initialize();
    ASSERT_EQ(session_.config()->GetMinAckDelayToSendMs(), kDefaultMinAckDelayTimeMs);
...
TEST_P(QuicSessionTestClient, FailedToCreateStreamIfTooCloseToIdleTimeout)
{
    ...
    QuicConnectionPeer::GetIdleNetworkDetector(connection_).OnPacketReceived(helper_.GetClock()->ApproximateNow());
...
TEST_P(QuicSessionTestServer, RstStreamReceivedAfterRstStreamSent)
{
    ...
    QuicRstStreamFrame rst1(kInvalidControlFrameId, stream2->id(), QUIC_ERROR_PROCESSING_STREAM, 0);
    if (!VersionHasIetfQuicFrames(transport_version())) {
        EXPECT_CALL(session_, OnCanCreateNewOutgoingStream(false)).Times(1);
    }
    session_.OnRstStream(rst1);
...
TEST_P(QuicSessionTestServer, TestZombieStreams)
{
    ...
    QuicRstStreamFrame rst_frame(kInvalidControlFrameId, stream2->id(), QUIC_STREAM_CANCELLED, 1234);
    // Just for the RST_STREAM
    EXPECT_CALL(*connection_, SendControlFrame(_)).WillOnce(Invoke(&ClearControlFrame));
    if (VersionHasIetfQuicFrames(transport_version())) {
        EXPECT_CALL(*connection_, OnStreamReset(stream2->id(), QUIC_STREAM_CANCELLED));
    } else {
        EXPECT_CALL(*connection_, OnStreamReset(stream2->id(), QUIC_RST_ACKNOWLEDGEMENT));
    }
    ...
    if (VersionHasIetfQuicFrames(transport_version())) {
        // The test requires the stream to be fully closed in both directions. For
        // IETF QUIC, the RST_STREAM only closes one side.
        QuicStopSendingFrame frame(kInvalidControlFrameId, stream2->id(), QUIC_STREAM_CANCELLED);
        EXPECT_CALL(*connection_, CloseConnection(_, _, _)).Times(0);
        session_.OnStopSendingFrame(frame);
    }
    ASSERT_EQ(1u, session_.closed_streams()->size());
    ...
    EXPECT_CALL(*connection_, OnStreamReset(stream4->id(), QUIC_STREAM_CANCELLED));
...
TEST_P(QuicSessionTestServer, OnStreamFrameLost)
{
    ...
    if (!QuicVersionUsesCryptoFrames(connection_->transport_version())) {
        frame1 = QuicStreamFrame(QuicUtils::GetCryptoStreamId(connection_->transport_version()), false, 0, 1300);
    }
...
TEST_P(QuicSessionTestServer, RetransmitLostDataCausesConnectionClose)
{
    ...
    EXPECT_CALL(*stream, HasPendingRetransmission()).Times(2).WillOnce(Return(true)).WillOnce(Return(false));
    ...
    EXPECT_CALL(*stream, OnCanWrite()).WillOnce(Invoke([this, stream]() { session_.ResetStream(stream->id(), QUIC_STREAM_CANCELLED); }));
    if (VersionHasIetfQuicFrames(transport_version())) {
        // Once for the RST_STREAM, once for the STOP_SENDING
        EXPECT_CALL(*connection_, SendControlFrame(_)).Times(2).WillRepeatedly(Invoke(&session_, &TestSession::SaveFrame));
    } else {
        // Just for the RST_STREAM
        EXPECT_CALL(*connection_, SendControlFrame(_)).WillOnce(Invoke(&session_, &TestSession::SaveFrame));
    }
    EXPECT_CALL(*connection_, OnStreamReset(stream->id(), _));
    session_.OnCanWrite();
}
...
TEST_P(QuicSessionTestServer, SendMessage)
{
    ...
    EXPECT_FALSE(session_.OneRttKeysAvailable());
    EXPECT_EQ(MessageResult(MESSAGE_STATUS_ENCRYPTION_NOT_ESTABLISHED, 0), session_.SendMessage(MemSliceFromString("")));

    CompleteHandshake();
    EXPECT_TRUE(session_.OneRttKeysAvailable());

    EXPECT_CALL(*connection_, SendMessage(1, _, false)).WillOnce(Return(MESSAGE_STATUS_SUCCESS));
    EXPECT_EQ(MessageResult(MESSAGE_STATUS_SUCCESS, 1), session_.SendMessage(MemSliceFromString("")));
    // Verify message_id increases.
    EXPECT_CALL(*connection_, SendMessage(2, _, false)).WillOnce(Return(MESSAGE_STATUS_TOO_LARGE));
    EXPECT_EQ(MessageResult(MESSAGE_STATUS_TOO_LARGE, 0), session_.SendMessage(MemSliceFromString("")));
    // Verify unsent message does not consume a message_id.
    EXPECT_CALL(*connection_, SendMessage(2, _, false)).WillOnce(Return(MESSAGE_STATUS_SUCCESS));
    EXPECT_EQ(MessageResult(MESSAGE_STATUS_SUCCESS, 2), session_.SendMessage(MemSliceFromString("")));
...
TEST_P(QuicSessionTestServer, LocallyResetZombieStreams)
{
    ...
    EXPECT_CALL(*stream2, HasPendingRetransmission()).WillRepeatedly(Return(true));
    session_.OnFrameLost(QuicFrame(frame));
...
{
    CompleteHandshake();
    EXPECT_FALSE(QuicSessionPeer::GetCleanUpClosedStreamsAlarm(&session_)->IsSet());
    ...
    CloseStream(stream2->id());
    EXPECT_EQ(1u, session_.closed_streams()->size());
    EXPECT_TRUE(QuicSessionPeer::GetCleanUpClosedStreamsAlarm(&session_)->IsSet());

    alarm_factory_.FireAlarm(QuicSessionPeer::GetCleanUpClosedStreamsAlarm(&session_));
...
{
    session_.set_writev_consumes_all_data(true);
    TestStream* stream4 = new TestStream(GetNthServerInitiatedUnidirectionalId(1), &session_, WRITE_UNIDIRECTIONAL);
...
TEST_P(QuicSessionTestServer, ReceivedDataOnWriteUnidirectionalStream)
{
    ...
    EXPECT_CALL(*connection_, CloseConnection(QUIC_DATA_RECEIVED_ON_WRITE_UNIDIRECTIONAL_STREAM, _, _)).Times(1);
    QuicStreamFrame stream_frame(GetNthServerInitiatedUnidirectionalId(1), false, 0, 2);
...
TEST_P(QuicSessionTestServer, ReadUnidirectionalStream)
{
    ...
    std::string data(100, '.');
    QuicStreamFrame stream_frame(GetNthClientInitiatedUnidirectionalId(1), false, 0, data);
    stream4->OnStreamFrame(stream_frame);
    EXPECT_TRUE(session_.closed_streams()->empty());

    QuicStreamFrame stream_frame2(GetNthClientInitiatedUnidirectionalId(1), true, 100, data);
...
TEST_P(QuicSessionTestServer, WriteOrBufferDataOnReadUnidirectionalStream)
{
    ...
    EXPECT_CALL(*connection_, CloseConnection(QUIC_TRY_TO_WRITE_DATA_ON_READ_UNIDIRECTIONAL_STREAM, _, _)).Times(1);
    std::string body(100, '.');
    stream4->WriteOrBufferData(body, false, nullptr);
}
...
TEST_P(QuicSessionTestServer, WritevDataOnReadUnidirectionalStream)
{
    ...
    quiche::QuicheMemSliceStorage storage(&iov, 1, session_.connection()->helper()->GetStreamSendBufferAllocator(), 1024);
...
TEST_P(QuicSessionTestServer, NewStreamIdBelowLimit)
{
    ...
    QuicStreamId bidirectional_stream_id
        = StreamCountToId(QuicSessionPeer::ietf_streamid_manager(&session_)->advertised_max_incoming_bidirectional_streams() - 1, Perspective::IS_CLIENT,
            /*bidirectional=*/true);

    QuicStreamFrame bidirectional_stream_frame(bidirectional_stream_id, false, 0, "Random String");
    EXPECT_CALL(*connection_, CloseConnection(_, _, _)).Times(0);
    session_.OnStreamFrame(bidirectional_stream_frame);

    QuicStreamId unidirectional_stream_id
        = StreamCountToId(QuicSessionPeer::ietf_streamid_manager(&session_)->advertised_max_incoming_unidirectional_streams() - 1, Perspective::IS_CLIENT,
            /*bidirectional=*/false);
    QuicStreamFrame unidirectional_stream_frame(unidirectional_stream_id, false, 0, "Random String");
    EXPECT_CALL(*connection_, CloseConnection(_, _, _)).Times(0);
...
TEST_P(QuicSessionTestServer, NewStreamIdAtLimit)
{
    ...
    QuicStreamId bidirectional_stream_id = StreamCountToId(
        QuicSessionPeer::ietf_streamid_manager(&session_)->advertised_max_incoming_bidirectional_streams(), Perspective::IS_CLIENT, /*bidirectional=*/true);
    QuicStreamFrame bidirectional_stream_frame(bidirectional_stream_id, false, 0, "Random String");
    EXPECT_CALL(*connection_, CloseConnection(_, _, _)).Times(0);
    session_.OnStreamFrame(bidirectional_stream_frame);

    QuicStreamId unidirectional_stream_id = StreamCountToId(
        QuicSessionPeer::ietf_streamid_manager(&session_)->advertised_max_incoming_unidirectional_streams(), Perspective::IS_CLIENT, /*bidirectional=*/false);
    QuicStreamFrame unidirectional_stream_frame(unidirectional_stream_id, false, 0, "Random String");
    EXPECT_CALL(*connection_, CloseConnection(_, _, _)).Times(0);
    session_.OnStreamFrame(unidirectional_stream_frame);
}
...
TEST_P(QuicSessionTestServer, NewStreamIdAboveLimit)
{
    ...
    QuicStreamId bidirectional_stream_id = StreamCountToId(
        QuicSessionPeer::ietf_streamid_manager(&session_)->advertised_max_incoming_bidirectional_streams() + 1, Perspective::IS_CLIENT, /*bidirectional=*/true);
    QuicStreamFrame bidirectional_stream_frame(bidirectional_stream_id, false, 0, "Random String");
    EXPECT_CALL(*connection_, CloseConnection(QUIC_INVALID_STREAM_ID, "Stream id 400 would exceed stream count limit 100", _));
    session_.OnStreamFrame(bidirectional_stream_frame);
    ...
    QuicStreamId unidirectional_stream_id
        = StreamCountToId(QuicSessionPeer::ietf_streamid_manager(&session_)->advertised_max_incoming_unidirectional_streams() + 1, Perspective::IS_CLIENT,
            /*bidirectional=*/false);
    QuicStreamFrame unidirectional_stream_frame(unidirectional_stream_id, false, 0, "Random String");
    EXPECT_CALL(*connection_, CloseConnection(QUIC_INVALID_STREAM_ID, "Stream id 402 would exceed stream count limit 100", _));
    session_.OnStreamFrame(unidirectional_stream_frame);
}
...
TEST_P(QuicSessionTestServer, OnStopSendingInvalidStreamId)
{
    ...
    EXPECT_CALL(*connection_, CloseConnection(QUIC_INVALID_STREAM_ID, "Received STOP_SENDING for an invalid stream", _));
    session_.OnStopSendingFrame(frame);
}
...
TEST_P(QuicSessionTestServer, OnStopSendingReadUnidirectional)
{
    ...
    QuicStopSendingFrame frame(1, GetNthClientInitiatedUnidirectionalId(1), QUIC_STREAM_CANCELLED);
    EXPECT_CALL(*connection_, CloseConnection(QUIC_INVALID_STREAM_ID, "Received STOP_SENDING for a read-only stream", _));
    session_.OnStopSendingFrame(frame);
}
...
TEST_P(QuicSessionTestServer, OnStopSendingStaticStreams)
{
    ...
    std::unique_ptr<TestStream> fake_static_stream = std::make_unique<TestStream>(stream_id, &session_, /*is_static*/ true, BIDIRECTIONAL);
    ...
    EXPECT_CALL(*connection_, CloseConnection(QUIC_INVALID_STREAM_ID, "Received STOP_SENDING for a static stream", _));
    session_.OnStopSendingFrame(frame);
}
...
TEST_P(QuicSessionTestServer, OnStopSendingInputNonExistentLocalStream)
{
    ...
    QuicStopSendingFrame frame(1, GetNthServerInitiatedBidirectionalId(123456), QUIC_STREAM_CANCELLED);
    EXPECT_CALL(*connection_, CloseConnection(QUIC_HTTP_STREAM_WRONG_DIRECTION, "Data for nonexistent stream", _)).Times(1);
    session_.OnStopSendingFrame(frame);
}
...
TEST_P(QuicSessionTestServer, OnStopSendingNewStream)
{
    ...
    QuicStopSendingFrame frame(1, GetNthClientInitiatedBidirectionalId(1), QUIC_STREAM_CANCELLED);
    ...
    QuicStream* stream = session_.GetOrCreateStream(GetNthClientInitiatedBidirectionalId(1));
    EXPECT_TRUE(stream);
    EXPECT_TRUE(stream->write_side_closed());
}
...
TEST_P(QuicSessionTestServer, WriteBufferedCryptoFrames)
{
    ...
    EXPECT_CALL(*connection_, SendCryptoData(ENCRYPTION_INITIAL, 1350, 0)).WillOnce(Return(1000));
    crypto_stream->WriteCryptoData(ENCRYPTION_INITIAL, data);
    EXPECT_TRUE(session_.HasPendingHandshake());
    EXPECT_TRUE(session_.WillingAndAbleToWrite());

    EXPECT_CALL(*connection_, SendCryptoData(_, _, _)).Times(0);
    connection_->SetEncrypter(ENCRYPTION_ZERO_RTT, std::make_unique<NullEncrypter>(connection_->perspective()));
    crypto_stream->WriteCryptoData(ENCRYPTION_ZERO_RTT, data);

    EXPECT_CALL(*connection_, SendCryptoData(ENCRYPTION_INITIAL, 350, 1000)).WillOnce(Return(350));
    EXPECT_CALL(
        *connection_, SendCryptoData(crypto_stream->GetEncryptionLevelToSendCryptoDataOfSpace(QuicUtils::GetPacketNumberSpace(ENCRYPTION_ZERO_RTT)), 1350, 0))
...
TEST_P(QuicSessionTestServer, StreamFrameReceivedAfterFin)
{
    ...
    EXPECT_CALL(*connection_, CloseConnection(QUIC_STREAM_DATA_BEYOND_CLOSE_OFFSET, _, _));
...
TEST_P(QuicSessionTestServer, ResetForIETFStreamTypes)
{
    ...
    EXPECT_CALL(*connection_, SendControlFrame(_)).Times(1).WillOnce(Invoke(&ClearControlFrame));
...
TEST_P(QuicSessionTestServer, DecryptionKeyAvailableBeforeEncryptionKey)
{
    ...
    ASSERT_FALSE(connection_->framer().HasEncrypterOfEncryptionLevel(ENCRYPTION_HANDSHAKE));
    EXPECT_FALSE(session_.OnNewDecryptionKeyAvailable(ENCRYPTION_HANDSHAKE, /*decrypter=*/nullptr,
...
TEST_P(QuicSessionTestServer, BlockedFrameCausesWriteError)
{
    ...
    QuicFlowControllerPeer::SetSendWindowOffset(session_.flow_controller(), kWindow);
    auto stream = session_.GetOrCreateStream(GetNthClientInitiatedBidirectionalId(0));
    // Try to send more data than the flow control limit allows.
    const uint64_t kOverflow = 15;
    std::string body(kWindow + kOverflow, 'a');
    EXPECT_CALL(*connection_, SendControlFrame(_)).WillOnce(testing::InvokeWithoutArgs([this]() {
        connection_->ReallyCloseConnection(QUIC_PACKET_WRITE_ERROR, "write error", ConnectionCloseBehavior::SILENT_CLOSE);
        return false;
    }));
...
TEST_P(QuicSessionTestServer, BufferedCryptoFrameCausesWriteError)
{
    ...
    TestCryptoStream* crypto_stream = session_.GetMutableCryptoStream();
    // Only consumed 1000 bytes.
    EXPECT_CALL(*connection_, SendCryptoData(ENCRYPTION_FORWARD_SECURE, 1350, 0)).WillOnce(Return(1000));
    crypto_stream->WriteCryptoData(ENCRYPTION_FORWARD_SECURE, data);
    EXPECT_TRUE(session_.HasPendingHandshake());
    EXPECT_TRUE(session_.WillingAndAbleToWrite());

    EXPECT_CALL(*connection_, SendCryptoData(ENCRYPTION_FORWARD_SECURE, 350, 1000)).WillOnce(Return(0));
    ...
    // Flush both frames.
    EXPECT_CALL(*connection_, SendCryptoData(ENCRYPTION_FORWARD_SECURE, 350, 1000)).WillOnce(testing::InvokeWithoutArgs([this]() {
        connection_->ReallyCloseConnection(QUIC_PACKET_WRITE_ERROR, "write error", ConnectionCloseBehavior::SILENT_CLOSE);
        return 350;
    }));
    if (!GetQuicReloadableFlag(quic_no_write_control_frame_upon_connection_close)) {
        EXPECT_CALL(*connection_, SendControlFrame(_)).WillOnce(Return(false));
...
TEST_P(QuicSessionTestServer, DonotPtoStreamDataBeforeHandshakeConfirmed)
{
    ...
    EXPECT_CALL(*connection_, SendCryptoData(ENCRYPTION_INITIAL, _, _)).WillOnce(Return(350));
...
TEST_P(QuicSessionTestServer, SetServerPreferredAddressAccordingToAddressFamily)
{
    ...
    EXPECT_EQ(quiche::IpAddressFamily::IP_V4, connection_->peer_address().host().address_family());
    QuicConnectionPeer::SetEffectivePeerAddress(connection_, connection_->peer_address());
    QuicTagVector copt;
    ...
    session_.config()->SetIPv6AlternateServerAddressToSend(QuicSocketAddress(QuicIpAddress::Loopback6(), 12345));

    connection_->SetDefaultEncryptionLevel(ENCRYPTION_FORWARD_SECURE);
    session_.OnConfigNegotiated();
    EXPECT_EQ(QuicSocketAddress(QuicIpAddress::Loopback4(), 12345), session_.config()->GetPreferredAddressToSend(quiche::IpAddressFamily::IP_V4).value());
    EXPECT_FALSE(session_.config()->GetPreferredAddressToSend(quiche::IpAddressFamily::IP_V6).has_value());
    EXPECT_EQ(preferred_address, connection_->expected_server_preferred_address());
}
...
TEST_P(QuicSessionTestServer, SetDNatServerPreferredAddressAccordingToAddressFamily)
{
    ...
    QuicSocketAddress expected_preferred_address(QuicIpAddress::Loopback4(), 12346);
    session_.config()->SetIPv4AlternateServerAddressForDNat(sent_preferred_address, expected_preferred_address);
    session_.config()->SetIPv6AlternateServerAddressForDNat(
        QuicSocketAddress(QuicIpAddress::Loopback6(), 12345), QuicSocketAddress(QuicIpAddress::Loopback6(), 12346));

    connection_->SetDefaultEncryptionLevel(ENCRYPTION_FORWARD_SECURE);
    session_.OnConfigNegotiated();
    EXPECT_EQ(QuicSocketAddress(QuicIpAddress::Loopback4(), 12345), session_.config()->GetPreferredAddressToSend(quiche::IpAddressFamily::IP_V4).value());
    EXPECT_FALSE(session_.config()->GetPreferredAddressToSend(quiche::IpAddressFamily::IP_V6).has_value());
    EXPECT_EQ(expected_preferred_address, connection_->expected_server_preferred_address());
}
...
TEST_P(QuicSessionTestServer, NoServerPreferredAddressIfAddressFamilyMismatch)
{
    ...
    connection_->SetDefaultEncryptionLevel(ENCRYPTION_FORWARD_SECURE);
    session_.OnConfigNegotiated();
    EXPECT_FALSE(session_.config()->GetPreferredAddressToSend(quiche::IpAddressFamily::IP_V4).has_value());
    EXPECT_FALSE(session_.config()->GetPreferredAddressToSend(quiche::IpAddressFamily::IP_V6).has_value());
    EXPECT_FALSE(connection_->expected_server_preferred_address().IsInitialized());
}
...
TEST_P(QuicSessionTestServer, OpenStreamLimitPerEventLoop)
{
    ...
    QuicStreamId unidirectional_stream_id = QuicUtils::GetFirstUnidirectionalStreamId(transport_version(), Perspective::IS_CLIENT);
    QuicStreamFrame data1(unidirectional_stream_id, false, 10, absl::string_view("HT"));
    session_.OnStreamFrame(data1);
    EXPECT_TRUE(QuicSessionPeer::GetPendingStream(&session_, unidirectional_stream_id));
    EXPECT_EQ(0, session_.num_incoming_streams_created());
    ...
    EXPECT_EQ(GetNthClientInitiatedBidirectionalId(i - 1), QuicSessionPeer::GetLargestPeerCreatedStreamId(&session_, false));
    EXPECT_TRUE(session_.GetActiveStream(GetNthClientInitiatedBidirectionalId(4))->pending_duration().IsZero());
    // Receive 1st byte on the read uni stream. The stream should still be pending
    // due to the stream limit.
    QuicStreamFrame data2(unidirectional_stream_id, false, 0, absl::string_view("HT"));
    session_.OnStreamFrame(data2);
    EXPECT_TRUE(QuicSessionPeer::GetPendingStream(&session_, unidirectional_stream_id));
    ...
    EXPECT_EQ(100, session_.GetActiveStream(unidirectional_stream_id)->pending_duration().ToMicroseconds());
    EXPECT_EQ(100, session_.GetActiveStream(GetNthClientInitiatedBidirectionalId(i - 2))->pending_duration().ToMicroseconds());
    // The 10th bidi stream should remain pending.
    EXPECT_EQ(nullptr, session_.GetActiveStream(GetNthClientInitiatedBidirectionalId(i - 1)));
}
...
INSTANTIATE_TEST_SUITE_P(Tests, QuicSessionTestClientUnconfigured, ::testing::ValuesIn(AllSupportedVersions()), ::testing::PrintToStringParamName());
