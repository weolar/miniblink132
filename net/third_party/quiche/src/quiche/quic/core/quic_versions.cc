...
void SetVersionFlag(const ParsedQuicVersion& version, bool should_enable)
{
    ...
    if (version == ParsedQuicVersion::RFCv2()) {
        ...
        QUIC_BUG(quic_bug_10589_1) << "Cannot " << (enable ? "en" : "dis") << "able version " << version;
    }
}
...
bool VersionHasLengthPrefixedConnectionIds(QuicTransportVersion transport_version)
{
...
std::ostream& operator<<(std::ostream& os, const ParsedQuicVersionVector& versions)
{
...
std::ostream& operator<<(std::ostream& os, const QuicVersionLabelVector& version_labels)
{
    os << QuicVersionLabelVectorToString(version_labels);
    return os;
}

std::ostream& operator<<(std::ostream& os, const QuicTransportVersionVector& transport_versions)
{
...
QuicVersionLabel CreateQuicVersionLabel(ParsedQuicVersion parsed_version)
{
    ...
    QUIC_BUG(quic_bug_10589_2) << "Unsupported version " << QuicVersionToString(parsed_version.transport_version) << " "
                               << HandshakeProtocolToString(parsed_version.handshake_protocol);
    return 0;
}

QuicVersionLabelVector CreateQuicVersionLabelVector(const ParsedQuicVersionVector& versions)
{
...
ParsedQuicVersionVector AllSupportedVersionsWithQuicCrypto()
{
    ...
    QUIC_BUG_IF(quic_bug_10589_3, versions.empty()) << "No version with QUIC crypto found.";
    return versions;
}
...
ParsedQuicVersionVector CurrentSupportedVersionsWithQuicCrypto()
{
    ...
    QUIC_BUG_IF(quic_bug_10589_4, versions.empty()) << "No version with QUIC crypto found.";
    return versions;
}
...
ParsedQuicVersionVector AllSupportedVersionsWithTls()
{
    ...
    QUIC_BUG_IF(quic_bug_10589_5, versions.empty()) << "No version with TLS handshake found.";
    return versions;
}
...
ParsedQuicVersionVector CurrentSupportedVersionsWithTls()
{
    ...
    QUIC_BUG_IF(quic_bug_10589_6, versions.empty()) << "No version with TLS handshake found.";
    return versions;
}

ParsedQuicVersionVector ObsoleteSupportedVersions()
{
    return ParsedQuicVersionVector { quic::ParsedQuicVersion::Q046(), quic::ParsedQuicVersion::Draft29() };
}

bool IsObsoleteSupportedVersion(ParsedQuicVersion version)
{
    static const ParsedQuicVersionVector obsolete_versions = ObsoleteSupportedVersions();
...
ParsedQuicVersionVector CurrentSupportedVersionsForClients()
{
    ...
    QUIC_BUG_IF(quic_bug_10589_8, versions.empty()) << "No supported client versions found.";
    return versions;
}
...
ParsedQuicVersionVector CurrentSupportedHttp3Versions()
{
    ...
    QUIC_BUG_IF(no_version_uses_http3, versions.empty()) << "No version speaking Http3 found.";
    return versions;
}
...
ParsedQuicVersion ParseQuicVersionLabel(QuicVersionLabel version_label)
{
    ...
    QUIC_DLOG(INFO) << "Unsupported QuicVersionLabel version: " << QuicVersionLabelToString(version_label);
    return UnsupportedQuicVersion();
}

ParsedQuicVersionVector ParseQuicVersionLabelVector(const QuicVersionLabelVector& version_labels)
{
...
ParsedQuicVersion ParseQuicVersionString(absl::string_view version_string)
{
    ...
    for (const ParsedQuicVersion& version : supported_versions) {
        if (version_string == ParsedQuicVersionToString(version) || (version_string == AlpnForVersion(version) && !version.AlpnDeferToRFCv1())
            || (version.handshake_protocol == PROTOCOL_QUIC_CRYPTO && version_string == QuicVersionToString(version.transport_version))) {
            return version;
        }
    }
    for (const ParsedQuicVersion& version : supported_versions) {
        if (version.UsesHttp3() && version_string == QuicVersionLabelToString(CreateQuicVersionLabel(version))) {
            return version;
        }
    }
    int quic_version_number = 0;
    if (absl::SimpleAtoi(version_string, &quic_version_number) && quic_version_number > 0) {
        QuicTransportVersion transport_version = static_cast<QuicTransportVersion>(quic_version_number);
...
ParsedQuicVersionVector ParseQuicVersionVectorString(absl::string_view versions_string)
{
    ParsedQuicVersionVector versions;
    std::vector<absl::string_view> version_strings = absl::StrSplit(versions_string, ',');
    for (absl::string_view version_string : version_strings) {
        quiche::QuicheTextUtils::RemoveLeadingAndTrailingWhitespace(&version_string);
        ParsedQuicVersion version = ParseQuicVersionString(version_string);
        if (!version.IsKnown() || std::find(versions.begin(), versions.end(), version) != versions.end()) {
            continue;
        }
        versions.push_back(version);
    }
...
QuicTransportVersionVector AllSupportedTransportVersions()
{
    ...
    for (const ParsedQuicVersion& version : AllSupportedVersions()) {
        if (std::find(transport_versions.begin(), transport_versions.end(), version.transport_version) == transport_versions.end()) {
...
ParsedQuicVersionVector AllSupportedVersions()
{
    ...
    return ParsedQuicVersionVector(supported_versions.begin(), supported_versions.end());
}
...
ParsedQuicVersionVector FilterSupportedVersions(ParsedQuicVersionVector versions)
{
    static_assert(SupportedVersions().size() == 4u, "Supported versions out of sync");
    ...
    for (const ParsedQuicVersion& version : versions) {
        if (version == ParsedQuicVersion::RFCv2()) {
            ...
            QUIC_BUG(quic_bug_10589_7) << "QUIC version " << version << " has no flag protection";
            filtered_versions.push_back(version);
        }
    }
...
ParsedQuicVersionVector ParsedVersionOfIndex(const ParsedQuicVersionVector& versions, int index)
{
...
ParsedQuicVersion ParseQuicVersionLabelString(absl::string_view version_label_string)
{
    const ParsedQuicVersionVector supported_versions = AllSupportedVersions();
    for (const ParsedQuicVersion& version : supported_versions) {
        if (version_label_string == QuicVersionLabelToString(CreateQuicVersionLabel(version))) {
            return version;
        }
    }
    return UnsupportedQuicVersion();
...
std::string QuicVersionLabelVectorToString(const QuicVersionLabelVector& version_labels, const std::string& separator, size_t skip_after_nth_version)
{
    std::string result;
...
std::string QuicVersionToString(QuicTransportVersion transport_version)
{
    ...
    return absl::StrCat("QUIC_VERSION_UNKNOWN(", static_cast<int>(transport_version), ")");
}
...
std::string ParsedQuicVersionVectorToString(const ParsedQuicVersionVector& versions, const std::string& separator, size_t skip_after_nth_version)
{
    std::string result;
    for (size_t i = 0; i < versions.size(); ++i) {
        if (i != 0) {
...
bool VersionAllowsVariableLengthConnectionIds(QuicTransportVersion transport_version)
{
    QUICHE_DCHECK_NE(transport_version, QUIC_VERSION_UNSUPPORTED);
    return transport_version > QUIC_VERSION_46;
}

bool QuicVersionLabelUses4BitConnectionIdLength(QuicVersionLabel version_label)
{
