...
class QUICHE_EXPORT QpackDecoder : public QpackEncoderStreamReceiver::Delegate,
...
public:
    ...
    public:
        ...
        virtual void OnEncoderStreamError(QuicErrorCode error_code, absl::string_view error_message) = 0;
    };

    QpackDecoder(uint64_t maximum_dynamic_table_capacity, uint64_t maximum_blocked_streams, EncoderStreamErrorDelegate* encoder_stream_error_delegate);
    ...
    void OnDecodingCompleted(QuicStreamId stream_id, uint64_t required_insert_count) override;
    ...
    std::unique_ptr<QpackProgressiveDecoder> CreateProgressiveDecoder(QuicStreamId stream_id, QpackProgressiveDecoder::HeadersHandlerInterface* handler);

    // QpackEncoderStreamReceiver::Delegate implementation
    void OnInsertWithNameReference(bool is_static, uint64_t name_index, absl::string_view value) override;
    void OnInsertWithoutNameReference(absl::string_view name, absl::string_view value) override;
...
class QUICHE_EXPORT NoopEncoderStreamErrorDelegate : public QpackDecoder::EncoderStreamErrorDelegate {
public:
