...
template <typename T> class QpackHeaderTableTest : public QuicTest {
protected:
    ~QpackHeaderTableTest() override = default;

    void SetUp() override
    {
        ASSERT_TRUE(table_.SetMaximumDynamicTableCapacity(kMaximumDynamicTableCapacityForTesting));
        ASSERT_TRUE(table_.SetDynamicTableCapacity(kMaximumDynamicTableCapacityForTesting));
    }

    bool EntryFitsDynamicTableCapacity(absl::string_view name, absl::string_view value) const
    {
    ...
    {
        return table_.max_entries();
    }
    uint64_t inserted_entry_count() const
    {
        return table_.inserted_entry_count();
    }
    uint64_t dropped_entry_count() const
    {
        return table_.dropped_entry_count();
    }

    T table_;
};

using MyTypes = ::testing::Types<QpackEncoderHeaderTable, QpackDecoderHeaderTable>;
...
TYPED_TEST(QpackHeaderTableTest, SetDynamicTableCapacity)
{
    ...
    EXPECT_FALSE(this->SetDynamicTableCapacity(2 * kMaximumDynamicTableCapacityForTesting));
}
...
class QpackEncoderHeaderTableTest : public QpackHeaderTableTest<QpackEncoderHeaderTable> {
protected:
    ...
    std::tuple<MatchType, bool, uint64_t> FindHeaderField(absl::string_view name, absl::string_view value) const
    {
        QpackEncoderHeaderTable::MatchResult match_result = table_.FindHeaderField(name, value);

        return { static_cast<MatchType>(match_result.match_type), match_result.is_static, match_result.index };
    }

    std::tuple<MatchType, bool, uint64_t> FindHeaderName(absl::string_view name) const
    {
        QpackEncoderHeaderTable::MatchResult match_result = table_.FindHeaderName(name);
...
TEST_F(QpackEncoderHeaderTableTest, FindStaticHeaderField)
{
    ...
    EXPECT_THAT(FindHeaderField(":method", "GET"), FieldsAre(kNameAndValue, kStaticEntry, 17u));

    EXPECT_THAT(FindHeaderField(":method", "POST"), FieldsAre(kNameAndValue, kStaticEntry, 20u));

    // ":method: TRACE" does not exist in the static table.
    // Both following calls return the lowest index with key ":method".
    EXPECT_THAT(FindHeaderField(":method", "TRACE"), FieldsAre(kName, kStaticEntry, 15u));
    ...
    EXPECT_THAT(FindHeaderField("accept-encoding", "gzip, deflate, br"), FieldsAre(kNameAndValue, kStaticEntry, 31u));

    EXPECT_THAT(FindHeaderField("accept-encoding", "compress"), FieldsAre(kName, kStaticEntry, 31u));

    EXPECT_THAT(FindHeaderField("accept-encoding", ""), FieldsAre(kName, kStaticEntry, 31u));

    EXPECT_THAT(FindHeaderName("accept-encoding"), FieldsAre(kName, kStaticEntry, 31u));

    // A header name that has a single entry with empty value.
    EXPECT_THAT(FindHeaderField("location", ""), FieldsAre(kNameAndValue, kStaticEntry, 12u));

    EXPECT_THAT(FindHeaderField("location", "foo"), FieldsAre(kName, kStaticEntry, 12u));
...
TEST_F(QpackEncoderHeaderTableTest, FindDynamicHeaderField)
{
    ...
    EXPECT_THAT(FindHeaderField("foo", "bar"), FieldsAre(kNameAndValue, kDynamicEntry, 0u));

    // Match name only.
    EXPECT_THAT(FindHeaderField("foo", "baz"), FieldsAre(kName, kDynamicEntry, 0u));
    ...
    // Match name and value.
    EXPECT_THAT(FindHeaderField("foo", "bar"), FieldsAre(kNameAndValue, kDynamicEntry, 1u));

    // Match name only.
    EXPECT_THAT(FindHeaderField("foo", "baz"), FieldsAre(kName, kDynamicEntry, 1u));
...
TEST_F(QpackEncoderHeaderTableTest, FindHeaderFieldPrefersStaticTable)
{
    ...
    EXPECT_THAT(FindHeaderField(":method", "TRACE"), FieldsAre(kNameAndValue, kDynamicEntry, 1u));
}
...
TEST_F(QpackEncoderHeaderTableTest, EvictByInsertion)
{
    ...
    EXPECT_THAT(FindHeaderField("foo", "bar"), FieldsAre(kNoMatch, _, _));
    EXPECT_THAT(FindHeaderField("baz", "qux"), FieldsAre(kNameAndValue, kDynamicEntry, 1u));
}
...
TEST_F(QpackEncoderHeaderTableTest, EvictOldestOfIdentical)
{
    ...
    EXPECT_THAT(FindHeaderField("foo", "bar"), FieldsAre(kNameAndValue, kDynamicEntry, 1u));
    EXPECT_THAT(FindHeaderField("baz", "qux"), FieldsAre(kNameAndValue, kDynamicEntry, 2u));
}
...
TEST_F(QpackEncoderHeaderTableTest, EvictOldestOfSameName)
{
    ...
    EXPECT_THAT(FindHeaderField("foo", "foo"), FieldsAre(kName, kDynamicEntry, 1u));
...
TEST_F(QpackEncoderHeaderTableTest, MaxInsertSizeWithoutEvictingGivenEntry)
{
    ...
    InsertEntry("foo", "bar");
    EXPECT_EQ(dynamic_table_capacity - entry_size1, MaxInsertSizeWithoutEvictingGivenEntry(0));
    ...
    EXPECT_EQ(dynamic_table_capacity - entry_size2, MaxInsertSizeWithoutEvictingGivenEntry(1));
    // First and second entry must stay.
    EXPECT_EQ(dynamic_table_capacity - entry_size2 - entry_size1, MaxInsertSizeWithoutEvictingGivenEntry(0));
    ...
    EXPECT_EQ(dynamic_table_capacity - entry_size3, MaxInsertSizeWithoutEvictingGivenEntry(2));
    // Second and third entry must stay.
    EXPECT_EQ(dynamic_table_capacity - entry_size3 - entry_size2, MaxInsertSizeWithoutEvictingGivenEntry(1));
}
...
class QpackDecoderHeaderTableTest : public QpackHeaderTableTest<QpackDecoderHeaderTable> {
protected:
    ~QpackDecoderHeaderTableTest() override = default;

    void ExpectEntryAtIndex(bool is_static, uint64_t index, absl::string_view expected_name, absl::string_view expected_value) const
    {
    ...
    void RegisterObserver(uint64_t required_insert_count, QpackDecoderHeaderTable::Observer* observer)
    {
        table_.RegisterObserver(required_insert_count, observer);
    }

    void UnregisterObserver(uint64_t required_insert_count, QpackDecoderHeaderTable::Observer* observer)
    {
