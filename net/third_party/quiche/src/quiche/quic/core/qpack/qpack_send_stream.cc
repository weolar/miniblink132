...
namespace quic {
QpackSendStream::QpackSendStream(QuicStreamId id, QuicSession* session, uint64_t http3_stream_type)
...
{
    QUIC_BUG(quic_bug_10805_1) << "OnStreamReset() called for write unidirectional stream.";
}

bool QpackSendStream::OnStopSending(QuicResetStreamError /* code */)
{
    stream_delegate()->OnStreamError(QUIC_HTTP_CLOSED_CRITICAL_STREAM, "STOP_SENDING received for QPACK send stream");
    return false;
}
