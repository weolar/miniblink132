...
class QUICHE_EXPORT QpackSendStream : public QuicStream, public QpackStreamSenderDelegate {
public:
    // |session| can't be nullptr, and the ownership is not passed. |session| owns
    // this stream.
    QpackSendStream(QuicStreamId id, QuicSession* session, uint64_t http3_stream_type);
