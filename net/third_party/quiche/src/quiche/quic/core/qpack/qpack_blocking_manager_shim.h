...
public:
    struct IndexSet : absl::variant<QpackBlockingManager::IndexSet, NewQpackBlockingManager::IndexSet> {
        IndexSet()
        {
    ...
    void OnHeaderBlockSent(QuicStreamId stream_id, IndexSet indices, uint64_t required_insert_count)
    {
        if (use_new_qpack_blocking_manager()) {
            new_manager().OnHeaderBlockSent(stream_id, std::move(indices.new_variant()), required_insert_count);
        } else {
            old_manager().OnHeaderBlockSent(stream_id, std::move(indices.old_variant()), required_insert_count);
        }
    }

    bool blocking_allowed_on_stream(QuicStreamId stream_id, uint64_t maximum_blocked_streams) const
    {
        if (use_new_qpack_blocking_manager()) {
            return new_manager().blocking_allowed_on_stream(stream_id, maximum_blocked_streams);
        }
        return old_manager().blocking_allowed_on_stream(stream_id, maximum_blocked_streams);
    }
    ...
    {
        if (use_new_qpack_blocking_manager()) {
            return NewQpackBlockingManager::RequiredInsertCount(indices.new_variant());
        }
