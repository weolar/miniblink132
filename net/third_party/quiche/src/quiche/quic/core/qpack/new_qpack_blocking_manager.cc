...
NewQpackBlockingManager::IndexSet::IndexSet(std::initializer_list<uint64_t> indices)
{
...
{
    if (empty()) {
        QUIC_BUG(qpack_blocking_manager_required_insert_count_on_empty_set) << "RequiredInsertCount called on an empty IndexSet.";
        return 0;
    }
    return max_index_ + 1;
}
...
bool NewQpackBlockingManager::OnHeaderAcknowledgement(QuicStreamId stream_id)
{
    ...
    if (it->second->header_blocks.empty()) {
        QUIC_BUG(qpack_blocking_manager_no_unacked_header_blocks_in_stream) << "OnHeaderAcknowledgement is called on a stream with no "
    ...
    if (it->second->header_blocks.empty()) {
        if (blocked_streams_.is_linked(it->second.get())) {
            ...
            QUIC_BUG(qpack_blocking_manager_stream_blocked_unexpectedly) << "Stream is blocked unexpectedly. stream_id:" << stream_id;
            ok = false;
...
void NewQpackBlockingManager::IncreaseKnownReceivedCount(uint64_t new_known_received_count)
{
    if (new_known_received_count <= known_received_count_) {
        QUIC_BUG(qpack_blocking_manager_known_received_count_not_increased)
            << "new_known_received_count:" << new_known_received_count << ", known_received_count_:" << known_received_count_;
        return;
    }
...
{
    if (increment > std::numeric_limits<uint64_t>::max() - known_received_count_) {
        return false;
    }
...
void NewQpackBlockingManager::OnHeaderBlockSent(QuicStreamId stream_id, IndexSet indices, uint64_t required_insert_count)
{
    if (indices.empty()) {
        QUIC_BUG(qpack_blocking_manager_empty_indices) << "OnHeaderBlockSent must not be called with empty indices. stream_id:" << stream_id;
        return;
    }
    ...
    auto it = stream_map_.find(stream_id);
    if (it == stream_map_.end()) {
        it = stream_map_.insert({ stream_id, std::make_unique<StreamRecord>() }).first;
    }
...
void NewQpackBlockingManager::UpdateBlockedListForStream(StreamRecord& stream_record)
{
...
{
    auto it = stream_map_.find(stream_id);
    return it != stream_map_.end() && blocked_streams_.is_linked(it->second.get());
}

bool NewQpackBlockingManager::blocking_allowed_on_stream(QuicStreamId stream_id, uint64_t maximum_blocked_streams) const
{
...
{
    return min_index_reference_counts_.empty() ? std::numeric_limits<uint64_t>::max() : min_index_reference_counts_.begin()->first;
}
...
void NewQpackBlockingManager::DecMinIndexReferenceCounts(uint64_t min_index)
{
    ...
    if (it == min_index_reference_counts_.end()) {
        QUIC_BUG(qpack_blocking_manager_removing_non_existent_min_index)
            << "Removing min index:" << min_index << " which do not exist in min_index_reference_counts_.";
        return;
    }
    if (it->second == 1) {
