...
public:
    ...
    MOCK_METHOD(void, OnInsertWithNameReference, (bool is_static, uint64_t name_index, absl::string_view value), (override));
    MOCK_METHOD(void, OnInsertWithoutNameReference, (absl::string_view name, absl::string_view value), (override));
    ...
    MOCK_METHOD(void, OnErrorDetected, (QuicErrorCode error_code, absl::string_view error_message), (override));
};

class QpackEncoderStreamReceiverTest : public QuicTest {
protected:
    QpackEncoderStreamReceiverTest()
        : stream_(&delegate_)
    {
    }
...
TEST_F(QpackEncoderStreamReceiverTest, InsertWithNameReference)
{
    ...
    EXPECT_CALL(*delegate(), OnInsertWithNameReference(false, 42, Eq(std::string(127, 'Z'))));

    std::string encoded_data;
    ASSERT_TRUE(absl::HexStringToBytes("c500"
                                       "c28294e7"
                                       "bf4a03626172"
...
{
    EXPECT_CALL(*delegate(), OnErrorDetected(QUIC_QPACK_ENCODER_STREAM_INTEGER_TOO_LARGE, Eq("Encoded integer too large.")));

    std::string encoded_data;
    ASSERT_TRUE(absl::HexStringToBytes("bfffffffffffffffffffffff", &encoded_data));
    Decode(encoded_data);
}
...
TEST_F(QpackEncoderStreamReceiverTest, InsertWithNameReferenceValueTooLong)
{
    ...
    std::string encoded_data;
    ASSERT_TRUE(absl::HexStringToBytes("c57fffffffffffffffffffff", &encoded_data));
    Decode(encoded_data);
}
...
TEST_F(QpackEncoderStreamReceiverTest, InsertWithoutNameReference)
{
    ...
    // 'Z' would be Huffman encoded to 8 bits, so no Huffman encoding is used.
    EXPECT_CALL(*delegate(), OnInsertWithoutNameReference(Eq(std::string(31, 'Z')), Eq(std::string(127, 'Z'))));

    std::string encoded_data;
    ASSERT_TRUE(absl::HexStringToBytes("4000"
                                       "4362617203626172"
                                       "6294e78294e7"
...
{
    EXPECT_CALL(*delegate(), OnErrorDetected(QUIC_QPACK_ENCODER_STREAM_STRING_LITERAL_TOO_LONG, Eq("String literal too long.")));
...
TEST_F(QpackEncoderStreamReceiverTest, InsertWithoutNameReferenceValueTooLongForVarintDecoder)
{
    ...
    std::string encoded_data;
    ASSERT_TRUE(absl::HexStringToBytes("436261727fffffffffffffffffffff", &encoded_data));
    Decode(encoded_data);
}

// Value Length value can be decoded by varint decoder but exceeds 1 MB limit.
TEST_F(QpackEncoderStreamReceiverTest, InsertWithoutNameReferenceValueExceedsLimit)
{
    EXPECT_CALL(*delegate(), OnErrorDetected(QUIC_QPACK_ENCODER_STREAM_STRING_LITERAL_TOO_LONG, Eq("String literal too long.")));
...
{
    EXPECT_CALL(*delegate(), OnErrorDetected(QUIC_QPACK_ENCODER_STREAM_HUFFMAN_ENCODING_ERROR, Eq("Error in Huffman-encoded string.")));
