...
public:
    static bool stream_is_blocked(const QpackBlockingManager* manager, QuicStreamId stream_id)
    {
        for (const auto& header_blocks_for_stream : manager->header_blocks_) {
            ...
            for (const auto& header_block : header_blocks_for_stream.second) {
                QUICHE_DCHECK_EQ(header_block.required_insert_count, QpackBlockingManager::RequiredInsertCount(header_block.indices));
                if (header_block.required_insert_count > manager->known_received_count_) {
...
TEST_F(QpackBlockingManagerTest, Empty)
{
    ...
    EXPECT_EQ(std::numeric_limits<uint64_t>::max(), manager_.smallest_blocking_index());
...
TEST_F(QpackBlockingManagerTest, InsertCountIncrementOverflow)
{
    ...
    EXPECT_FALSE(manager_.OnInsertCountIncrement(std::numeric_limits<uint64_t>::max() - 5));
}
