...
public:
    ...
    MOCK_METHOD(void, OnHeaderAcknowledgement, (QuicStreamId stream_id), (override));
...
protected:
    QpackDecoderStreamReceiverTest()
        : stream_(&delegate_)
    {
    }
...
TEST_F(QpackDecoderStreamReceiverTest, InsertCountIncrement)
{
    ...
    EXPECT_CALL(delegate_, OnErrorDetected(QUIC_QPACK_DECODER_STREAM_INTEGER_TOO_LARGE, Eq("Encoded integer too large.")));
