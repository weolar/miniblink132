...
public:
    ...
    void SendInsertWithNameReference(bool is_static, uint64_t name_index, absl::string_view value);
    // 5.2.2. Insert Without Name Reference
    void SendInsertWithoutNameReference(absl::string_view name, absl::string_view value);
    // 5.2.3. Duplicate
    ...
    QuicByteCount BufferedByteCount() const
    {
        return buffer_.size();
    }
