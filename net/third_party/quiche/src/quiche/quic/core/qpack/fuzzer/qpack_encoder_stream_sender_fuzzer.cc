...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    QpackEncoderStreamSender sender(provider.ConsumeBool() ? HuffmanEncoding::kEnabled : HuffmanEncoding::kDisabled);
    ...
    while (provider.remaining_bytes() != 0) {
        ...
        case 0: {
            ...
            uint16_t value_length = provider.ConsumeIntegralInRange<uint16_t>(0, kMaxStringLength);
        ...
        case 1: {
            uint16_t name_length = provider.ConsumeIntegralInRange<uint16_t>(0, kMaxStringLength);
