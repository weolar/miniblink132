...
public:
    ErrorDelegate(bool* error_detected)
...
public:
    HeadersHandler(QuicStreamId stream_id, DecoderAndHandlerMap* processing_decoders, bool* error_detected)
    ...
    void OnHeaderDecoded(absl::string_view /*name*/, absl::string_view /*value*/) override
    {
    }
    ...
    void OnDecodingErrorDetected(QuicErrorCode /*error_code*/, absl::string_view /*error_message*/) override
    {
        *error_detected_ = true;
    }
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    QpackDecoder decoder(maximum_dynamic_table_capacity, maximum_blocked_streams, &encoder_stream_error_delegate);
    ...
    while (fragment_count > 0 && !error_detected && provider.remaining_bytes() > 0) {
        --fragment_count;
        ...
        case 0: {
            ...
            std::string encoded_data = provider.ConsumeRandomLengthString(fragment_size);
        ...
        case 1: {
            ...
            if (reading_decoders.find(stream_id) != reading_decoders.end() || processing_decoders.find(stream_id) != processing_decoders.end()) {
                continue;
            }

            DecoderAndHandler decoder_and_handler;
            decoder_and_handler.handler = std::make_unique<HeadersHandler>(stream_id, &processing_decoders, &error_detected);
            decoder_and_handler.decoder = decoder.CreateProgressiveDecoder(stream_id, decoder_and_handler.handler.get());
        ...
        case 2: {
            ...
            auto distance = provider.ConsumeIntegralInRange<uint8_t>(0, reading_decoders.size() - 1);
