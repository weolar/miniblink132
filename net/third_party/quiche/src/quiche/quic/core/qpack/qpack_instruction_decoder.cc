...
QpackInstructionDecoder::QpackInstructionDecoder(const QpackLanguage* language, Delegate* delegate)
    : language_(language)
    , delegate_(delegate)
    , s_bit_(false)
    , varint_(0)
...
bool QpackInstructionDecoder::Decode(absl::string_view data)
{
    ...
    while (true) {
        ...
        data = absl::string_view(data.data() + bytes_consumed, data.size() - bytes_consumed);

        // Stop processing if no more data but next state would require it.
        if (data.empty() && (state_ != State::kStartField) && (state_ != State::kVarintDone) && (state_ != State::kReadStringDone)) {
            return true;
        }
    }
}
...
bool QpackInstructionDecoder::DoVarintStart(absl::string_view data, size_t* bytes_consumed)
{
    QUICHE_DCHECK(!data.empty());
    ...
    http2::DecodeStatus status = varint_decoder_.Start(data[0], field_->param, &buffer);
...
bool QpackInstructionDecoder::DoVarintResume(absl::string_view data, size_t* bytes_consumed)
{
    QUICHE_DCHECK(!data.empty());
    QUICHE_DCHECK(field_->type == QpackInstructionFieldType::kVarint || field_->type == QpackInstructionFieldType::kVarint2
        || field_->type == QpackInstructionFieldType::kName || field_->type == QpackInstructionFieldType::kValue);
...
bool QpackInstructionDecoder::DoVarintDone()
{
    ...
    std::string* const string = (field_->type == QpackInstructionFieldType::kName) ? &name_ : &value_;
    string->clear();
...
bool QpackInstructionDecoder::DoReadString(absl::string_view data, size_t* bytes_consumed)
{
    QUICHE_DCHECK(!data.empty());
...
bool QpackInstructionDecoder::DoReadStringDone()
{
    ...
    if (is_huffman_encoded_) {
        ...
        if (!huffman_decoder_.InputProperlyTerminated()) {
            OnError(ErrorCode::HUFFMAN_ENCODING_ERROR, "Error in Huffman-encoded string.");
            return false;
        }
...
const QpackInstruction* QpackInstructionDecoder::LookupOpcode(uint8_t byte) const
{
...
void QpackInstructionDecoder::OnError(ErrorCode error_code, absl::string_view error_message)
{
    QUICHE_DCHECK(!error_detected_);
