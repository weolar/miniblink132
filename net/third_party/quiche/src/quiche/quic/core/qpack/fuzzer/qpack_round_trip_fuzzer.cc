...
public:
    EncodingEndpoint(
        uint64_t maximum_dynamic_table_capacity, uint64_t maximum_blocked_streams, HuffmanEncoding huffman_encoding, CookieCrumbling cookie_crumbling)
        : encoder_(&decoder_stream_error_delegate, huffman_encoding, cookie_crumbling)
    {
    ...
    ~EncodingEndpoint()
    {
        ...
        QUICHE_CHECK_EQ(std::numeric_limits<uint64_t>::max(), QpackEncoderPeer::smallest_blocking_index(&encoder_));
    }
    ...
    std::string EncodeHeaderList(QuicStreamId stream_id, const quiche::HttpHeaderBlock& header_list)
    {
...
private:
    ...
    class CrashingDecoderStreamErrorDelegate : public QpackEncoder::DecoderStreamErrorDelegate {
    public:
        ...
        {
            QUICHE_CHECK(false) << QuicErrorCodeToString(error_code) << " " << error_message;
        }
    };
...
public:
    ...
    public:
        ...
        virtual void OnHeaderBlockFragment(QuicStreamId stream_id, absl::string_view data) = 0;
    ...
    ~DelayedHeaderBlockTransmitter()
    {
        QUICHE_CHECK(header_blocks_.empty());
    }

    // Enqueues |encoded_header_block| for delayed transmission.
    void SendEncodedHeaderBlock(QuicStreamId stream_id, std::string encoded_header_block)
    {
    ...
    void MaybeTransmitSomeData()
    {
        ...
        auto index = provider_->ConsumeIntegralInRange<size_t>(0, header_blocks_.size() - 1);
        ...
        size_t length = provider_->ConsumeIntegralInRange<size_t>(1, header_block.RemainingLength());
    ...
    void Flush()
    {
        while (!header_blocks_.empty()) {
            ...
            visitor_->OnHeaderBlockFragment(stream_id, header_block.ConsumeRemaining());
...
private:
    ...
    public:
        ...
        absl::string_view ConsumeRemaining()
        {
...
public:
    ...
    VerifyingDecoder(QuicStreamId stream_id, Visitor* visitor, QpackDecoder* qpack_decoder, QuicHeaderList expected_header_list)
        : stream_id_(stream_id)
        , visitor_(visitor)
        , accumulator_(stream_id, qpack_decoder, this,
    ...
    void OnHeadersDecoded(QuicHeaderList headers, bool header_list_size_limit_exceeded) override
    {
        // Verify headers.
    ...
    void OnHeaderDecodingError(QuicErrorCode error_code, absl::string_view error_message) override
    {
        QUICHE_CHECK(false) << QuicErrorCodeToString(error_code) << " " << error_message;
    }
...
// complete.
class DecodingEndpoint : public DelayedHeaderBlockTransmitter::Visitor, public VerifyingDecoder::Visitor {
public:
    DecodingEndpoint(uint64_t maximum_dynamic_table_capacity, uint64_t maximum_blocked_streams, FuzzedDataProvider* provider)
        : decoder_(maximum_dynamic_table_capacity, maximum_blocked_streams, &encoder_stream_error_delegate_)
        , provider_(provider)
    {
    }
    ...
    void AddExpectedHeaderList(QuicStreamId stream_id, QuicHeaderList expected_header_list)
    {
    ...
    void OnHeaderBlockStart(QuicStreamId stream_id) override
    {
        ...
        auto verifying_decoder = std::make_unique<VerifyingDecoder>(stream_id, this, &decoder_, std::move(expected_header_list));
        auto result = verifying_decoders_.insert({ stream_id, std::move(verifying_decoder) });
        QUICHE_CHECK(result.second);
    }

    void OnHeaderBlockFragment(QuicStreamId stream_id, absl::string_view data) override
    {
...
private:
    ...
    class CrashingEncoderStreamErrorDelegate : public QpackDecoder::EncoderStreamErrorDelegate {
    public:
...
public:
    DelayedStreamDataTransmitter(QpackStreamReceiver* receiver, FuzzedDataProvider* provider)
        : receiver_(receiver)
        , provider_(provider)
    {
    }

    ~DelayedStreamDataTransmitter()
    {
        QUICHE_CHECK(stream_data.empty());
    }
...
QuicHeaderList SplitHeaderList(const quiche::HttpHeaderBlock& header_list, CookieCrumbling cookie_crumbling)
{
    QuicHeaderList split_header_list;

    size_t total_size = 0;
    ValueSplittingHeaderList splitting_header_list(&header_list, cookie_crumbling);
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    const uint64_t maximum_dynamic_table_capacity = provider.ConsumeIntegral<uint8_t>();
    // Maximum 256 blocked streams.
    const uint64_t maximum_blocked_streams = provider.ConsumeIntegral<uint8_t>();

    // Set up encoder.
    const CookieCrumbling cookie_crumbling = provider.ConsumeBool() ? CookieCrumbling::kEnabled : CookieCrumbling::kDisabled;
    EncodingEndpoint encoder(maximum_dynamic_table_capacity, maximum_blocked_streams,
        provider.ConsumeBool() ? HuffmanEncoding::kEnabled : HuffmanEncoding::kDisabled, cookie_crumbling);

    // Set up decoder.
    DecodingEndpoint decoder(maximum_dynamic_table_capacity, maximum_blocked_streams, &provider);

    // Transmit encoder stream data from encoder to decoder.
    DelayedStreamDataTransmitter encoder_stream_transmitter(decoder.encoder_stream_receiver(), &provider);
    ...
    DelayedStreamDataTransmitter decoder_stream_transmitter(encoder.decoder_stream_receiver(), &provider);
    ...
    while (header_list_count > 0 && provider.remaining_bytes() > 0) {
        ...
        // Encode header list.
        std::string encoded_header_block = encoder.EncodeHeaderList(stream_id, header_list);
        ...
        QuicHeaderList expected_header_list = SplitHeaderList(header_list, cookie_crumbling);
        decoder.AddExpectedHeaderList(stream_id, std::move(expected_header_list));

        header_block_transmitter.SendEncodedHeaderBlock(stream_id, std::move(encoded_header_block));

        // Transmit some encoder stream data, decoder stream data, or header blocks
        // on the request stream, repeating a few times.
        for (auto transmit_data_count = provider.ConsumeIntegralInRange(1, 5); transmit_data_count > 0; --transmit_data_count) {
