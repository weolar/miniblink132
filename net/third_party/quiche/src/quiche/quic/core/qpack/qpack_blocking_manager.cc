...
void QpackBlockingManager::OnHeaderBlockSent(QuicStreamId stream_id, IndexSet indices, uint64_t required_insert_count)
{
    QUICHE_DCHECK(!indices.empty());

    IncreaseReferenceCounts(indices);
    header_blocks_[stream_id].push_back({ std::move(indices), required_insert_count });
    if (optimize_qpack_blocking_manager_ && required_insert_count > known_received_count_) {
...
bool QpackBlockingManager::blocking_allowed_on_stream(QuicStreamId stream_id, uint64_t maximum_blocked_streams) const
{
    if (optimize_qpack_blocking_manager_) {
        ...
        return blocked_streams_.contains(stream_id) || blocked_streams_.size() < maximum_blocked_streams;
    }
...
{
    return entry_reference_counts_.empty() ? std::numeric_limits<uint64_t>::max() : entry_reference_counts_.begin()->first;
}
...
void QpackBlockingManager::OnKnownReceivedCountIncreased()
{
    ...
    for (auto blocked_it = blocked_streams_.begin(); blocked_it != blocked_streams_.end();) {
