...
public:
    ...
    MOCK_METHOD(void, OnHeadersDecoded, (QuicHeaderList headers, bool header_list_size_limit_exceeded), (override));
    MOCK_METHOD(void, OnHeaderDecodingError, (QuicErrorCode error_code, absl::string_view error_message), (override));
};
...
protected:
    QpackDecodedHeadersAccumulatorTest()
        : qpack_decoder_(kMaxDynamicTableCapacity, kMaximumBlockedStreams, &encoder_stream_error_delegate_)
        , accumulator_(kTestStreamId, &qpack_decoder_, &visitor_, kMaxHeaderListSize)
    {
...
{
    EXPECT_CALL(visitor_, OnHeaderDecodingError(QUIC_QPACK_DECOMPRESSION_FAILED, Eq("Incomplete header data prefix.")));
...
TEST_F(QpackDecodedHeadersAccumulatorTest, EmptyHeaderList)
{
    ...
    EXPECT_CALL(visitor_, OnHeadersDecoded(_, false)).WillOnce(SaveArg<0>(&header_list));
    accumulator_.EndHeaderBlock();
...
TEST_F(QpackDecodedHeadersAccumulatorTest, TruncatedPayload)
{
    ...
    EXPECT_CALL(visitor_, OnHeaderDecodingError(QUIC_QPACK_DECOMPRESSION_FAILED, Eq("Incomplete header block.")));
    accumulator_.EndHeaderBlock();
}
...
{
    EXPECT_CALL(visitor_, OnHeaderDecodingError(QUIC_QPACK_DECOMPRESSION_FAILED, Eq("Static table entry not found.")));
    std::string encoded_data;
...
TEST_F(QpackDecodedHeadersAccumulatorTest, Success)
{
    ...
    EXPECT_EQ(strlen("foo") + strlen("bar"), header_list.uncompressed_header_bytes());
    EXPECT_EQ(encoded_data.size(), header_list.compressed_header_bytes());
}
...
TEST_F(QpackDecodedHeadersAccumulatorTest, ExceedLimitThenSplitInstruction)
{
    ...
    ASSERT_TRUE(absl::HexStringToBytes("0000" // header block prefix
        ...
        &encoded_data));
    accumulator_.Decode(encoded_data);
    ASSERT_TRUE(absl::HexStringToBytes("0f", // second byte of a two-byte long Indexed Header Field instruction
        &encoded_data));
    accumulator_.Decode(encoded_data);
...
TEST_F(QpackDecodedHeadersAccumulatorTest, ExceedLimitBlocked)
{
    std::string encoded_data;
    // Total length of header list exceeds kMaxHeaderListSize.
    ASSERT_TRUE(absl::HexStringToBytes("0200" // header block prefix
...
TEST_F(QpackDecodedHeadersAccumulatorTest, BlockedDecodingUnblockedBeforeEndOfHeaderBlock)
{
    std::string encoded_data;
    // Reference to dynamic table entry not yet received.
    ASSERT_TRUE(absl::HexStringToBytes("020080", &encoded_data));
...
TEST_F(QpackDecodedHeadersAccumulatorTest, BlockedDecodingUnblockedAndErrorBeforeEndOfHeaderBlock)
{
    std::string encoded_data;
    ...
    EXPECT_CALL(visitor_, OnHeaderDecodingError(QUIC_QPACK_DECOMPRESSION_FAILED, Eq("Invalid relative index.")));
    qpack_decoder_.OnInsertWithoutNameReference("foo", "bar");
}
