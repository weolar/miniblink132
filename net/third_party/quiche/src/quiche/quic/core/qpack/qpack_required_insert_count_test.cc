...
TEST(QpackRequiredInsertCountTest, QpackDecodeRequiredInsertCount)
{
    for (size_t i = 0; i < ABSL_ARRAYSIZE(kTestData); ++i) {
        ...
        const uint64_t total_number_of_inserts = kTestData[i].total_number_of_inserts;
        ...
        if (required_insert_count != 0) {
            ...
            ASSERT_LT(total_number_of_inserts, required_insert_count + max_entries) << i;
            ...
            ASSERT_LE(required_insert_count, total_number_of_inserts + max_entries) << i;
        }

        uint64_t encoded_required_insert_count = QpackEncodeRequiredInsertCount(required_insert_count, max_entries);
        ...
        EXPECT_TRUE(QpackDecodeRequiredInsertCount(encoded_required_insert_count, max_entries, total_number_of_inserts, &decoded_required_insert_count)) << i;
...
TEST(QpackRequiredInsertCountTest, DecodeRequiredInsertCountError)
{
    for (size_t i = 0; i < ABSL_ARRAYSIZE(kInvalidTestData); ++i) {
        ...
        EXPECT_FALSE(QpackDecodeRequiredInsertCount(kInvalidTestData[i].encoded_required_insert_count, kInvalidTestData[i].max_entries,
            kInvalidTestData[i].total_number_of_inserts, &decoded_required_insert_count))
            << i;
    }
}
