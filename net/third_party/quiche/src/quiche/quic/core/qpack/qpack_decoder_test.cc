...
protected:
    QpackDecoderTest()
        : qpack_decoder_(kMaximumDynamicTableCapacity, kMaximumBlockedStreams, &encoder_stream_error_delegate_)
        , fragment_mode_(GetParam())
    {
        qpack_decoder_.set_qpack_stream_sender_delegate(&decoder_stream_sender_delegate_);
    }
    ...
    void SetUp() override
    {
        ...
        ON_CALL(handler_, OnDecodingErrorDetected(_, _)).WillByDefault(Invoke([this](QuicErrorCode /* error_code */, absl::string_view /* error_message */) {
    ...
    std::unique_ptr<QpackProgressiveDecoder> CreateProgressiveDecoder(QuicStreamId stream_id)
    {
    ...
    {
        auto fragment_size_generator = FragmentModeToFragmentSizeGenerator(fragment_mode_);
...
INSTANTIATE_TEST_SUITE_P(All, QpackDecoderTest, Values(FragmentMode::kSingleChunk, FragmentMode::kOctetByOctet));

TEST_P(QpackDecoderTest, NoPrefix)
{
    EXPECT_CALL(handler_, OnDecodingErrorDetected(QUIC_QPACK_DECOMPRESSION_FAILED, Eq("Incomplete header data prefix.")));
...
TEST_P(QpackDecoderTest, InvalidPrefix)
{
    ...
    EXPECT_CALL(handler_, OnDecodingErrorDetected(QUIC_QPACK_DECOMPRESSION_FAILED, Eq("Encoded integer too large.")));
...
TEST_P(QpackDecoderTest, MultipleLiteralEntries)
{
    ...
    std::string input;
    ASSERT_TRUE(absl::HexStringToBytes("0000" // prefix
...
{
    EXPECT_CALL(handler_, OnDecodingErrorDetected(QUIC_QPACK_DECOMPRESSION_FAILED, Eq("String literal too long.")));
...
TEST_P(QpackDecoderTest, ValueLenTooLargeForVarintDecoder)
{
    ...
    std::string input;
    ASSERT_TRUE(absl::HexStringToBytes("000023666f6f7fffffffffffffffffffff", &input));
    DecodeHeaderBlock(input);
}
...
{
    EXPECT_CALL(handler_, OnDecodingErrorDetected(QUIC_QPACK_DECOMPRESSION_FAILED, Eq("Incomplete header block.")));
...
TEST_P(QpackDecoderTest, HuffmanSimple)
{
    ...
    std::string input;
    ASSERT_TRUE(absl::HexStringToBytes("00002f0125a849e95ba97d7f8925a849e95bb8e8b4bf", &input));
    DecodeHeaderBlock(input);
}

TEST_P(QpackDecoderTest, AlternatingHuffmanNonHuffman)
{
    EXPECT_CALL(handler_, OnHeaderDecoded(Eq("custom-key"), Eq("custom-value"))).Times(4);
    EXPECT_CALL(handler_, OnDecodingCompleted());
...
{
    EXPECT_CALL(handler_, OnDecodingErrorDetected(QUIC_QPACK_DECOMPRESSION_FAILED, Eq("Error in Huffman-encoded string.")));

    // 'y' ends in 0b0 on the most significant bit of the last byte.
    // The remaining 7 bits must be a prefix of EOS, which is all 1s.
    std::string input;
    ASSERT_TRUE(absl::HexStringToBytes("00002f0125a849e95ba97d7e8925a849e95bb8e8b4bf", &input));
    DecodeHeaderBlock(input);
}
...
TEST_P(QpackDecoderTest, HuffmanValueDoesNotHaveEOSPrefix)
{
    ...
    std::string input;
    ASSERT_TRUE(absl::HexStringToBytes("00002f0125a849e95ba97d7f8925a849e95bb8e8b4be", &input));
    DecodeHeaderBlock(input);
}
...
TEST_P(QpackDecoderTest, HuffmanNameEOSPrefixTooLong)
{
    ...
    // prefix of EOS.
    std::string input;
    ASSERT_TRUE(absl::HexStringToBytes("00002f0225a849e95ba97d7fff8925a849e95bb8e8b4bf", &input));
    DecodeHeaderBlock(input);
}
...
TEST_P(QpackDecoderTest, HuffmanValueEOSPrefixTooLong)
{
    ...
    // prefix of EOS.
    std::string input;
    ASSERT_TRUE(absl::HexStringToBytes("00002f0125a849e95ba97d7f8a25a849e95bb8e8b4bfff", &input));
    DecodeHeaderBlock(input);
}
...
TEST_P(QpackDecoderTest, StaticTable)
{
    ...
    EXPECT_CALL(handler_, OnHeaderDecoded(Eq("accept-encoding"), Eq("gzip, deflate, br")));
    ...
    std::string input;
    ASSERT_TRUE(absl::HexStringToBytes("0000d1dfccd45f108621e9aec2a11f5c8294e75f000554524143455f1000", &input));
    DecodeHeaderBlock(input);
}
...
TEST_P(QpackDecoderTest, TooHighStaticTableIndex)
{
    ...
    EXPECT_CALL(handler_, OnHeaderDecoded(Eq("x-frame-options"), Eq("sameorigin")));

    // Addressing entry 99 should trigger an error.
    EXPECT_CALL(handler_, OnDecodingErrorDetected(QUIC_QPACK_DECOMPRESSION_FAILED, Eq("Static table entry not found.")));
...
{
    std::string input;
    ASSERT_TRUE(absl::HexStringToBytes("3fe107" // Set dynamic table capacity to 1024.
    ...
    EXPECT_CALL(handler_, OnHeaderDecoded(Eq(":method"), Eq("foo"))).InSequence(s);
    EXPECT_CALL(handler_, OnHeaderDecoded(Eq("foo"), Eq("ZZZ"))).InSequence(s);
    EXPECT_CALL(handler_, OnHeaderDecoded(Eq(":method"), Eq("ZZ"))).InSequence(s);
    EXPECT_CALL(handler_, OnDecodingCompleted()).InSequence(s);
    EXPECT_CALL(decoder_stream_sender_delegate_, WriteStreamData(Eq(kHeaderAcknowledgement))).InSequence(s);

    ASSERT_TRUE(absl::HexStringToBytes("0500" // Required Insert Count 4 and Delta Base 0.
                                       // Base is 4 + 0 = 4.
    ...
    ASSERT_TRUE(absl::HexStringToBytes("0502" // Required Insert Count 4 and Delta Base 2.
                                       // Base is 4 + 2 = 6.
    ...
    ASSERT_TRUE(absl::HexStringToBytes("0582" // Required Insert Count 4 and Delta Base 2 with sign bit set.
                                       // Base is 4 - 2 - 1 = 1.
...
TEST_P(QpackDecoderTest, DecreasingDynamicTableCapacityEvictsEntries)
{
    ...
    EXPECT_CALL(handler_, OnHeaderDecoded(Eq("foo"), Eq("bar")));
    EXPECT_CALL(handler_, OnDecodingCompleted());
    EXPECT_CALL(decoder_stream_sender_delegate_, WriteStreamData(Eq(kHeaderAcknowledgement)));

    ASSERT_TRUE(absl::HexStringToBytes("0200" // Required Insert Count 1 and Delta Base 0.
                                       // Base is 1 + 0 = 1.
                                       "80", // Dynamic table entry with relative index 0, absolute index 0.
        &input));
    DecodeHeaderBlock(input);
    ...
    EXPECT_CALL(handler_, OnDecodingErrorDetected(QUIC_QPACK_DECOMPRESSION_FAILED, Eq("Dynamic table entry already evicted.")));
...
{
    std::string input;
    EXPECT_CALL(encoder_stream_error_delegate_, OnEncoderStreamError(QUIC_QPACK_ENCODER_STREAM_ERROR_INSERTING_LITERAL, Eq("Error inserting literal entry.")));
...
{
    EXPECT_CALL(encoder_stream_error_delegate_, OnEncoderStreamError(QUIC_QPACK_ENCODER_STREAM_INVALID_STATIC_ENTRY, Eq("Invalid static table entry.")));
...
{
    EXPECT_CALL(
        encoder_stream_error_delegate_, OnEncoderStreamError(QUIC_QPACK_ENCODER_STREAM_INSERTION_INVALID_RELATIVE_INDEX, Eq("Invalid relative index.")));
...
{
    EXPECT_CALL(
        encoder_stream_error_delegate_, OnEncoderStreamError(QUIC_QPACK_ENCODER_STREAM_DUPLICATE_INVALID_RELATIVE_INDEX, Eq("Invalid relative index.")));
...
{
    EXPECT_CALL(encoder_stream_error_delegate_, OnEncoderStreamError(QUIC_QPACK_ENCODER_STREAM_INTEGER_TOO_LARGE, Eq("Encoded integer too large.")));
...
{
    EXPECT_CALL(handler_, OnDecodingErrorDetected(QUIC_QPACK_DECOMPRESSION_FAILED, Eq("Invalid relative index.")));
    ...
    ASSERT_TRUE(absl::HexStringToBytes("0280" // Required Insert Count is 1.  Base 1 - 1 - 0 = 0 is explicitly
...
{
    EXPECT_CALL(handler_, OnDecodingErrorDetected(QUIC_QPACK_DECOMPRESSION_FAILED, Eq("Error calculating Base.")));
...
TEST_P(QpackDecoderTest, EvictedDynamicTableEntry)
{
    ...
    ASSERT_TRUE(absl::HexStringToBytes("0380" // Required Insert Count 2 and Delta Base 0 with sign bit set.
                                       // Base is 2 - 0 - 1 = 1
...
TEST_P(QpackDecoderTest, TableCapacityMustNotExceedMaximum)
{
    EXPECT_CALL(encoder_stream_error_delegate_,
        OnEncoderStreamError(QUIC_QPACK_ENCODER_STREAM_SET_DYNAMIC_TABLE_CAPACITY, Eq("Error updating dynamic table capacity.")));
...
TEST_P(QpackDecoderTest, InvalidEncodedRequiredInsertCount)
{
    ...
    EXPECT_CALL(handler_, OnDecodingErrorDetected(QUIC_QPACK_DECOMPRESSION_FAILED, Eq("Error decoding Required Insert Count.")));
    std::string input;
...
TEST_P(QpackDecoderTest, WrappedRequiredInsertCount)
{
    ...
    ASSERT_TRUE(absl::HexStringToBytes("6294e7" // Name "foo".
    ...
    ASSERT_TRUE(absl::HexStringToBytes("0a00" // Encoded Required Insert Count 10, Required Insert Count 201,
...
TEST_P(QpackDecoderTest, NonZeroRequiredInsertCountButNoDynamicEntries)
{
    ...
    EXPECT_CALL(handler_, OnHeaderDecoded(Eq(":method"), Eq("GET")));
    EXPECT_CALL(handler_, OnDecodingErrorDetected(QUIC_QPACK_DECOMPRESSION_FAILED, Eq("Required Insert Count too large.")));

    ASSERT_TRUE(absl::HexStringToBytes("0200" // Required Insert Count is 1.
...
TEST_P(QpackDecoderTest, AddressEntryNotAllowedByRequiredInsertCount)
{
    ...
    EXPECT_CALL(handler_, OnDecodingErrorDetected(QUIC_QPACK_DECOMPRESSION_FAILED, Eq("Absolute Index must be smaller than Required Insert Count.")));

    ASSERT_TRUE(absl::HexStringToBytes("0201" // Required Insert Count 1 and Delta Base 1.
                                       // Base is 1 + 1 = 2.
...
TEST_P(QpackDecoderTest, PromisedRequiredInsertCountLargerThanActual)
{
    ...
    ASSERT_TRUE(absl::HexStringToBytes("0300" // Required Insert Count 2 and Delta Base 0.
                                       // Base is 2 + 0 = 2.
    ...
    ASSERT_TRUE(absl::HexStringToBytes("0481" // Required Insert Count 3 and Delta Base 1 with sign bit set.
                                       // Base is 3 - 1 - 1 = 1.
                                       "10", // Indexed Header Field with Post-Base Index instruction
        // addressing dynamic table entry with post-base index 0,
...
TEST_P(QpackDecoderTest, BlockedDecodingUnblockedBeforeEndOfHeaderBlock)
{
    ...
    ASSERT_TRUE(absl::HexStringToBytes("80" // Indexed Header Field instruction addressing dynamic table
                                       // entry with relative index 0, absolute index 0.
...
TEST_P(QpackDecoderTest, BlockedDecodingUnblockedAndErrorBeforeEndOfHeaderBlock)
{
    std::string input;
    StartDecoding();
    ASSERT_TRUE(absl::HexStringToBytes("0200" // Required Insert Count 1 and Delta Base 0.
...
TEST_P(QpackDecoderTest, BlockedDecodingAndEvictedEntries)
{
    ...
    ASSERT_TRUE(absl::HexStringToBytes("0700" // Required Insert Count 6 and Delta Base 0.
                                       // Base is 6 + 0 = 6.
                                       "80", // Indexed Header Field instruction addressing dynamic table
...
TEST_P(QpackDecoderTest, TooManyBlockedStreams)
{
    ...
    EXPECT_CALL(handler_, OnDecodingErrorDetected(QUIC_QPACK_DECOMPRESSION_FAILED, Eq("Limit on number of blocked streams exceeded.")));
...
TEST_P(QpackDecoderTest, InsertCountIncrement)
{
    ...
    ASSERT_TRUE(absl::HexStringToBytes("81" // Header Acknowledgement on stream 1
                                       "01", // Insert Count Increment with increment of one
        &expected_data));
    EXPECT_CALL(decoder_stream_sender_delegate_, WriteStreamData(Eq(expected_data)));
