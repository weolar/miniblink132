...
ValueSplittingHeaderList::const_iterator::const_iterator(
    const quiche::HttpHeaderBlock* header_list, quiche::HttpHeaderBlock::const_iterator header_list_iterator, CookieCrumbling cookie_crumbling)
...
bool ValueSplittingHeaderList::const_iterator::operator==(const const_iterator& other) const
{
    return header_list_iterator_ == other.header_list_iterator_ && value_start_ == other.value_start_;
}
...
const ValueSplittingHeaderList::const_iterator& ValueSplittingHeaderList::const_iterator::operator++()
{
...
const ValueSplittingHeaderList::value_type& ValueSplittingHeaderList::const_iterator::operator*() const
{
...
void ValueSplittingHeaderList::const_iterator::UpdateHeaderField()
{
    ...
    const absl::string_view value = original_value.substr(value_start_, value_end_ - value_start_);
    header_field_ = std::make_pair(name, value);

    // Skip character after ';' separator if it is a space.
    if (name == kCookieKey && value_end_ != absl::string_view::npos && value_end_ + 1 < original_value.size()
        && original_value[value_end_ + 1] == kOptionalSpaceAfterCookieSeparator) {
        ++value_end_;
    }
}

ValueSplittingHeaderList::ValueSplittingHeaderList(const quiche::HttpHeaderBlock* header_list, CookieCrumbling cookie_crumbling)
    : header_list_(header_list)
    , cookie_crumbling_(cookie_crumbling)
{
...
ValueSplittingHeaderList::const_iterator ValueSplittingHeaderList::begin() const
{
