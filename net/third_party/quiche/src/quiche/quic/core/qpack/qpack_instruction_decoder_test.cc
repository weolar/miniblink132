...
{
    static const QpackInstruction* const instruction = new QpackInstruction { QpackInstructionOpcode { 0x00, 0x80 },
        { { QpackInstructionFieldType::kSbit, 0x40 }, { QpackInstructionFieldType::kVarint, 6 }, { QpackInstructionFieldType::kVarint2, 8 } } };
    return instruction;
}
...
const QpackInstruction* TestInstruction2()
{
    static const QpackInstruction* const instruction
        = new QpackInstruction { QpackInstructionOpcode { 0x80, 0x80 }, { { QpackInstructionFieldType::kName, 6 }, { QpackInstructionFieldType::kValue, 7 } } };
    return instruction;
}

const QpackLanguage* TestLanguage()
{
    static const QpackLanguage* const language = new QpackLanguage { TestInstruction1(), TestInstruction2() };
    return language;
}
...
public:
    ...
    MOCK_METHOD(bool, OnInstructionDecoded, (const QpackInstruction*), (override));
    MOCK_METHOD(void, OnInstructionDecodingError, (QpackInstructionDecoder::ErrorCode error_code, absl::string_view error_message), (override));
};

class QpackInstructionDecoderTest : public QuicTestWithParam<FragmentMode> {
protected:
    QpackInstructionDecoderTest()
        : decoder_(std::make_unique<QpackInstructionDecoder>(TestLanguage(), &delegate_))
    ...
    void SetUp() override
    {
        ...
        ON_CALL(delegate_, OnInstructionDecodingError(_, _)).WillByDefault(InvokeWithoutArgs([this]() { decoder_.reset(); }));
    }
    ...
    void DecodeInstruction(absl::string_view data)
    {
        ...
        FragmentSizeGenerator fragment_size_generator = FragmentModeToFragmentSizeGenerator(fragment_mode_);
...
INSTANTIATE_TEST_SUITE_P(All, QpackInstructionDecoderTest, Values(FragmentMode::kSingleChunk, FragmentMode::kOctetByOctet));
...
{
    std::string encoded_data;
    EXPECT_CALL(delegate_, OnInstructionDecodingError(QpackInstructionDecoder::ErrorCode::HUFFMAN_ENCODING_ERROR, Eq("Error in Huffman-encoded string.")));
...
{
    std::string encoded_data;
    EXPECT_CALL(delegate_, OnInstructionDecodingError(QpackInstructionDecoder::ErrorCode::INTEGER_TOO_LARGE, Eq("Encoded integer too large.")));
...
{
    std::string encoded_data;
    EXPECT_CALL(delegate_, OnInstructionDecodingError(QpackInstructionDecoder::ErrorCode::STRING_LITERAL_TOO_LONG, Eq("String literal too long.")));
...
TEST_P(QpackInstructionDecoderTest, DelegateSignalsError)
{
    ...
    Expectation first_call = EXPECT_CALL(delegate_, OnInstructionDecoded(TestInstruction1())).WillOnce(InvokeWithoutArgs([this]() -> bool {
        EXPECT_EQ(1u, decoder_->varint());
        return true;
    }));

    // Second instruction is invalid.  Decoding must halt.
    EXPECT_CALL(delegate_, OnInstructionDecoded(TestInstruction1())).After(first_call).WillOnce(InvokeWithoutArgs([this]() -> bool {
...
{
    EXPECT_CALL(delegate_, OnInstructionDecoded(TestInstruction1())).WillOnce(InvokeWithoutArgs([this]() -> bool {
        EXPECT_EQ(1u, decoder_->varint());
        decoder_.reset();
        return false;
    }));
