...
class MockDecoderStreamErrorDelegate : public QpackEncoder::DecoderStreamErrorDelegate {
public:
    ~MockDecoderStreamErrorDelegate() override = default;

    MOCK_METHOD(void, OnDecoderStreamError, (QuicErrorCode error_code, absl::string_view error_message), (override));
};
...
protected:
    QpackEncoderTest()
        ...
        , encoder_(&decoder_stream_error_delegate_, huffman_encoding_, CookieCrumbling::kEnabled)
    ...
    {
        return encoder_.EncodeHeaderList(/* stream_id = */ 1, header_list, &encoder_stream_sent_byte_count_);
    }
...
INSTANTIATE_TEST_SUITE_P(HuffmanEncoding, QpackEncoderTest, ::testing::ValuesIn({ HuffmanEncoding::kEnabled, HuffmanEncoding::kDisabled }), PrintToString);

TEST_P(QpackEncoderTest, Empty)
{
    EXPECT_CALL(encoder_stream_sender_delegate_, NumBytesBuffered()).WillRepeatedly(Return(0));
...
TEST_P(QpackEncoderTest, Simple)
{
    ...
    if (HuffmanEnabled()) {
        ...
        ASSERT_TRUE(absl::HexStringToBytes("000023666f6f03626172", &expected_output));
    }
    EXPECT_EQ(expected_output, output);
}
...
TEST_P(QpackEncoderTest, Multiple)
{
    ...
    if (HuffmanEnabled()) {
        expected_output_hex = "0000" // prefix
                              "2a94e703626172"; // foo: bar
    } else {
        expected_output_hex = "0000" // prefix
                              "23666f6f03626172"; // foo: bar
    }
    expected_output_hex += "27005a5a5a5a5a5a5a" // 7 octet long header name, the smallest number
...
TEST_P(QpackEncoderTest, StaticTable)
{
    ...
    }
    {
        ...
        std::string expected_output;
        if (HuffmanEnabled()) {
            ASSERT_TRUE(absl::HexStringToBytes("0000d45f108621e9aec2a11f5c8294e7", &expected_output));
        } else {
            ASSERT_TRUE(absl::HexStringToBytes("0000d45f1008636f6d70726573735c03666f6f", &expected_output));
        }
        EXPECT_EQ(expected_output, output);
    }
    {
        ...
        std::string output = Encode(header_list);
        std::string expected_output;
        ASSERT_TRUE(absl::HexStringToBytes("00005f000554524143455f1000", &expected_output));
        EXPECT_EQ(expected_output, output);
    }
}
...
{
    EXPECT_CALL(decoder_stream_error_delegate_, OnDecoderStreamError(QUIC_QPACK_DECODER_STREAM_INTEGER_TOO_LARGE, Eq("Encoded integer too large.")));

    QpackEncoder encoder(&decoder_stream_error_delegate_, huffman_encoding_, CookieCrumbling::kEnabled);
...
TEST_P(QpackEncoderTest, ZeroInsertCountIncrement)
{
    ...
    EXPECT_CALL(decoder_stream_error_delegate_, OnDecoderStreamError(QUIC_QPACK_DECODER_STREAM_INVALID_ZERO_INCREMENT, Eq("Invalid increment value 0.")));
...
{
    QpackEncoderHeaderTable* header_table = QpackEncoderPeer::header_table(&encoder_);
    ...
    EXPECT_CALL(decoder_stream_error_delegate_,
        OnDecoderStreamError(QUIC_QPACK_DECODER_STREAM_INCREMENT_OVERFLOW, Eq("Insert Count Increment instruction causes overflow.")));
...
TEST_P(QpackEncoderTest, DynamicTable)
{
    ...
    if (HuffmanEnabled()) {
        insert_entries_hex = "62" // insert without name reference
                             "94e7"; // Huffman-encoded literal name "foo"
    } else {
        insert_entries_hex = "43" // insert without name reference
                             "666f6f"; // literal name "foo"
    }
    insert_entries_hex += "03626172" // value "bar"
    ...
    EXPECT_CALL(encoder_stream_sender_delegate_, WriteStreamData(Eq(absl::StrCat(set_dyanamic_table_capacity, insert_entries))));

    std::string expected_output;
    ASSERT_TRUE(absl::HexStringToBytes("0400" // prefix
...
TEST_P(QpackEncoderTest, SmallDynamicTable)
{
    ...
    if (HuffmanEnabled()) {
        ASSERT_TRUE(absl::HexStringToBytes("62" // insert without name reference
                                           "94e7" // Huffman-encoded literal name "foo"
                                           "03626172", // value "bar"
            &insert_entry));
    } else {
        ASSERT_TRUE(absl::HexStringToBytes("43" // insert without name reference
                                           "666f6f" // literal name "foo"
                                           "03626172", // value "bar"
            &insert_entry));
    }
    EXPECT_CALL(encoder_stream_sender_delegate_, WriteStreamData(Eq(absl::StrCat(set_dyanamic_table_capacity, insert_entry))));

    std::string expected_output;
    ASSERT_TRUE(absl::HexStringToBytes("0200" // prefix
                                       "80" // dynamic entry 0
...
TEST_P(QpackEncoderTest, BlockedStream)
{
    ...
    EXPECT_CALL(encoder_stream_sender_delegate_, WriteStreamData(Eq(absl::StrCat(set_dyanamic_table_capacity, insert_entry1))));

    std::string expected_output;
    ASSERT_TRUE(absl::HexStringToBytes("0200" // prefix
                                       "80", // dynamic entry 0
        &expected_output));
    EXPECT_EQ(expected_output, encoder_.EncodeHeaderList(/* stream_id = */ 1, header_list1, &encoder_stream_sent_byte_count_));
    ...
    EXPECT_EQ(entries, encoder_.EncodeHeaderList(/* stream_id = */ 2, header_list2, &encoder_stream_sent_byte_count_));
    ...
    // Insert three entries into the dynamic table.
    std::string insert_entries;
    ASSERT_TRUE(absl::HexStringToBytes("80" // insert with name reference, dynamic index 0
                                       "0362617a" // value "baz"
                                       "c5" // insert with name reference, static index 5
                                       "0362617a" // value "baz"
                                       ...
                                       "0362617a", // value "baz"
        &insert_entries));
    EXPECT_CALL(encoder_stream_sender_delegate_, WriteStreamData(Eq(insert_entries)));

    ASSERT_TRUE(absl::HexStringToBytes("0500" // prefix
                                       "83828180", // dynamic entries
        &expected_output));
    EXPECT_EQ(expected_output, encoder_.EncodeHeaderList(/* stream_id = */ 3, header_list2, &encoder_stream_sent_byte_count_));
    EXPECT_EQ(insert_entries.size(), encoder_stream_sent_byte_count_);
    ...
    EXPECT_EQ(expected2, encoder_.EncodeHeaderList(/* stream_id = */ 4, header_list2, &encoder_stream_sent_byte_count_));
    EXPECT_EQ(0u, encoder_stream_sent_byte_count_);
    ...
    ASSERT_TRUE(absl::HexStringToBytes("0400" // prefix
                                       ...
                                       "0362617a", // with literal value "baz"
        &expected3));
    EXPECT_EQ(expected3, encoder_.EncodeHeaderList(/* stream_id = */ 5, header_list2, &encoder_stream_sent_byte_count_));
    EXPECT_EQ(0u, encoder_stream_sent_byte_count_);
    ...
    ASSERT_TRUE(absl::HexStringToBytes("0500" // prefix
                                       "83828180", // dynamic entries
        &expected4));
    EXPECT_EQ(expected4, encoder_.EncodeHeaderList(/* stream_id = */ 6, header_list2, &encoder_stream_sent_byte_count_));
    EXPECT_EQ(0u, encoder_stream_sent_byte_count_);
}
...
TEST_P(QpackEncoderTest, Draining)
{
    ...
    for (const auto& header_field : header_list1) {
        maximum_dynamic_table_capacity += QpackEntry::Size(header_field.first, header_field.second);
    }
    ...
    std::string expected_output;
    ASSERT_TRUE(absl::HexStringToBytes("0b00" // prefix
    ...
    EXPECT_CALL(encoder_stream_sender_delegate_, WriteStreamData(Eq(expected_output)));

    ASSERT_TRUE(absl::HexStringToBytes("0c00" // prefix
    ...
    std::string entries = "0000" // prefix
    ...
    entries += "2374776f" // literal name "two"
...
{
    EXPECT_CALL(encoder_stream_sender_delegate_, NumBytesBuffered()).WillRepeatedly(Return(kTooManyBytesBuffered));
    encoder_.SetMaximumBlockedStreams(1);
    encoder_.SetMaximumDynamicTableCapacity(4096);
    encoder_.SetDynamicTableCapacity(4096);
...
TEST_P(QpackEncoderTest, UnackedEntryCannotBeEvicted)
{
    ...
    EXPECT_CALL(encoder_stream_sender_delegate_, WriteStreamData(Eq(absl::StrCat(set_dyanamic_table_capacity, insert_entries1))));
    ...
    ASSERT_TRUE(absl::HexStringToBytes("0000" // prefix
        ...
        &expected_output));
    EXPECT_EQ(expected_output, encoder_.EncodeHeaderList(/* stream_id = */ 2, header_list2, &encoder_stream_sent_byte_count_));
...
TEST_P(QpackEncoderTest, UseStaticTableNameOnlyMatch)
{
    ...
    // Insert one entry into the dynamic table.
    std::string insert_entry1;
    ASSERT_TRUE(absl::HexStringToBytes("cf" // insert with name of static table entry 15
                                       "03626172", // literal value "bar"
        &insert_entry1));
    EXPECT_CALL(encoder_stream_sender_delegate_, WriteStreamData(Eq(absl::StrCat(set_dyanamic_table_capacity, insert_entry1))));
    ...
    ASSERT_TRUE(absl::HexStringToBytes("0200" // prefix
                                       "80", // dynamic entry 0
        &expected_output));
    EXPECT_EQ(expected_output, encoder_.EncodeHeaderList(/* stream_id = */ 1, header_list, &encoder_stream_sent_byte_count_));
    EXPECT_EQ(insert_entry1.size(), encoder_stream_sent_byte_count_);

    // Stream 2 uses the same dynamic entry.
    EXPECT_EQ(expected_output, encoder_.EncodeHeaderList(/* stream_id = */ 2, header_list, &encoder_stream_sent_byte_count_));
    EXPECT_EQ(0u, encoder_stream_sent_byte_count_);
...
TEST_P(QpackEncoderTest, UseDynamicTableNameOnlyMatch)
{
    ...
    ASSERT_TRUE(absl::HexStringToBytes("89" // insert entry with same name as dynamic table entry 9
...
TEST_P(QpackEncoderTest, CookieCrumblingEnabledDynamicTable)
{
    ...
    std::string insert_entries;
    if (HuffmanEnabled()) {
        ASSERT_TRUE(absl::HexStringToBytes("c5" // insert with name reference, static index 5
    ...
    std::string expected_output;
    ASSERT_TRUE(absl::HexStringToBytes("0300" // prefix
...
{
    QpackEncoder encoder(&decoder_stream_error_delegate_, huffman_encoding_, CookieCrumbling::kDisabled);
    ...
    EXPECT_EQ(expected_output, encoder.EncodeHeaderList(/* stream_id = */ 1, header_list, &encoder_stream_sent_byte_count_));
