...
const char* QuicRstStreamErrorCodeToString(QuicRstStreamErrorCode error)
{
    switch (error) {
        ...
        RETURN_STRING_LITERAL(QUIC_STREAM_WEBTRANSPORT_BUFFERED_STREAMS_LIMIT_EXCEEDED);
...
const char* QuicErrorCodeToString(QuicErrorCode error)
{
    switch (error) {
        ...
        RETURN_STRING_LITERAL(QUIC_WINDOW_UPDATE_RECEIVED_ON_READ_UNIDIRECTIONAL_STREAM);
        ...
        RETURN_STRING_LITERAL(QUIC_QPACK_ENCODER_STREAM_INSERTION_INVALID_RELATIVE_INDEX);
        RETURN_STRING_LITERAL(QUIC_QPACK_ENCODER_STREAM_INSERTION_DYNAMIC_ENTRY_NOT_FOUND);
        ...
        RETURN_STRING_LITERAL(QUIC_QPACK_ENCODER_STREAM_DUPLICATE_INVALID_RELATIVE_INDEX);
...
QuicErrorCodeToIetfMapping QuicErrorCodeToTransportErrorCode(QuicErrorCode error)
{
    switch (error) {
    case QUIC_NO_ERROR:
    ...
    case QUIC_QPACK_ENCODER_STREAM_ERROR:
        return { false, static_cast<uint64_t>(QuicHttpQpackErrorCode::ENCODER_STREAM_ERROR) };
    ...
    case QUIC_HTTP_CLOSED_CRITICAL_STREAM:
        return { false, static_cast<uint64_t>(QuicHttp3ErrorCode::CLOSED_CRITICAL_STREAM) };
    case QUIC_HTTP_MISSING_SETTINGS_FRAME:
        return { false, static_cast<uint64_t>(QuicHttp3ErrorCode::MISSING_SETTINGS) };
    ...
    case QUIC_HTTP_STREAM_LIMIT_TOO_LOW:
        return { false, static_cast<uint64_t>(QuicHttp3ErrorCode::GENERAL_PROTOCOL_ERROR) };
    ...
    case QUIC_TLS_BAD_CERTIFICATE:
        return { true, static_cast<uint64_t>(CRYPTO_ERROR_FIRST + SSL_AD_BAD_CERTIFICATE) };
    case QUIC_TLS_UNSUPPORTED_CERTIFICATE:
        return { true, static_cast<uint64_t>(CRYPTO_ERROR_FIRST + SSL_AD_UNSUPPORTED_CERTIFICATE) };
    case QUIC_TLS_CERTIFICATE_REVOKED:
        return { true, static_cast<uint64_t>(CRYPTO_ERROR_FIRST + SSL_AD_CERTIFICATE_REVOKED) };
    case QUIC_TLS_CERTIFICATE_EXPIRED:
        return { true, static_cast<uint64_t>(CRYPTO_ERROR_FIRST + SSL_AD_CERTIFICATE_EXPIRED) };
    case QUIC_TLS_CERTIFICATE_UNKNOWN:
        return { true, static_cast<uint64_t>(CRYPTO_ERROR_FIRST + SSL_AD_CERTIFICATE_UNKNOWN) };
    case QUIC_TLS_INTERNAL_ERROR:
        return { true, static_cast<uint64_t>(CRYPTO_ERROR_FIRST + SSL_AD_INTERNAL_ERROR) };
    case QUIC_TLS_UNRECOGNIZED_NAME:
        return { true, static_cast<uint64_t>(CRYPTO_ERROR_FIRST + SSL_AD_UNRECOGNIZED_NAME) };
    case QUIC_TLS_CERTIFICATE_REQUIRED:
        return { true, static_cast<uint64_t>(CRYPTO_ERROR_FIRST + SSL_AD_CERTIFICATE_REQUIRED) };
...
uint64_t RstStreamErrorCodeToIetfResetStreamErrorCode(QuicRstStreamErrorCode rst_stream_error_code)
{
    ...
    case QUIC_STREAM_DECOMPRESSION_FAILED:
        return static_cast<uint64_t>(QuicHttpQpackErrorCode::DECOMPRESSION_FAILED);
    case QUIC_STREAM_ENCODER_STREAM_ERROR:
        return static_cast<uint64_t>(QuicHttpQpackErrorCode::ENCODER_STREAM_ERROR);
    case QUIC_STREAM_DECODER_STREAM_ERROR:
        return static_cast<uint64_t>(QuicHttpQpackErrorCode::DECODER_STREAM_ERROR);
...
QuicRstStreamErrorCode IetfResetStreamErrorCodeToRstStreamErrorCode(uint64_t ietf_error_code)
{
...
// static
QuicResetStreamError QuicResetStreamError::FromInternal(QuicRstStreamErrorCode code)
{
    return QuicResetStreamError(code, RstStreamErrorCodeToIetfResetStreamErrorCode(code));
}

// static
QuicResetStreamError QuicResetStreamError::FromIetf(uint64_t code)
{
    return QuicResetStreamError(IetfResetStreamErrorCodeToRstStreamErrorCode(code), code);
}
...
// static
QuicResetStreamError QuicResetStreamError::FromIetf(QuicHttpQpackErrorCode code)
{
    return FromIetf(static_cast<uint64_t>(code));
}
