...
class QUICHE_EXPORT TlsHandshaker : public TlsConnection::Delegate, public CryptoMessageParser {
public:
    ...
    absl::string_view extra_error_details() const
    {
        return extra_error_details_;
    }
    ...
    bool ExportKeyingMaterialForLabel(absl::string_view label, absl::string_view context, size_t result_len, std::string* result);
...
protected:
    ...
    // explicitly.
    void CloseConnection(QuicErrorCode error, QuicIetfTransportErrorCodes ietf_error, const std::string& reason_phrase);

    void OnConnectionClosed(QuicErrorCode error, ConnectionCloseSource source);

    bool is_connection_closed() const
    {
        return is_connection_closed_;
    }
    ...
    int expected_ssl_error() const
    {
        return expected_ssl_error_;
    }
    ...
    virtual QuicAsyncStatus VerifyCertChain(const std::vector<std::string>& certs, std::string* error_details, std::unique_ptr<ProofVerifyDetails>* details,
    ...
    bool SetReadSecret(EncryptionLevel level, const SSL_CIPHER* cipher, absl::Span<const uint8_t> read_secret) override;
    ...
    // and |data|.
    void MessageCallback(bool is_write, int version, int content_type, absl::string_view data) override;
