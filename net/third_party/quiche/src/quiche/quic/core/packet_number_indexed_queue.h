...
template <typename T> class QUICHE_NO_EXPORT PacketNumberIndexedQueue {
public:
    PacketNumberIndexedQueue()
        : number_of_present_entries_(0)
    {
    }
    ...
    template <typename... Args> bool Emplace(QuicPacketNumber packet_number, Args&&... args);
    ...
    // before removing it.
    template <typename Function> bool Remove(QuicPacketNumber packet_number, Function f);
    ...
    bool IsEmpty() const
    {
        return number_of_present_entries_ == 0;
    }
    ...
    size_t entry_slots_used() const
    {
        return entries_.size();
    }

    // Packet number of the first entry in the queue.
    QuicPacketNumber first_packet() const
    {
...
template <typename T> T* PacketNumberIndexedQueue<T>::GetEntry(QuicPacketNumber packet_number)
{
...
template <typename T> const T* PacketNumberIndexedQueue<T>::GetEntry(QuicPacketNumber packet_number) const
{
...
template <typename T> template <typename... Args> bool PacketNumberIndexedQueue<T>::Emplace(QuicPacketNumber packet_number, Args&&... args)
{
    if (!packet_number.IsInitialized()) {
        QUIC_BUG(quic_bug_10359_1) << "Try to insert an uninitialized packet number";
        return false;
    }
...
template <typename T> bool PacketNumberIndexedQueue<T>::Remove(QuicPacketNumber packet_number)
{
    return Remove(packet_number, [](const T&) {});
}

template <typename T> template <typename Function> bool PacketNumberIndexedQueue<T>::Remove(QuicPacketNumber packet_number, Function f)
{
    EntryWrapper* entry = GetEntryWrapper(packet_number);
    if (entry == nullptr) {
        return false;
...
template <typename T> void PacketNumberIndexedQueue<T>::RemoveUpTo(QuicPacketNumber packet_number)
{
    while (!entries_.empty() && first_packet_.IsInitialized() && first_packet_ < packet_number) {
...
template <typename T> void PacketNumberIndexedQueue<T>::Cleanup()
{
...
template <typename T> auto PacketNumberIndexedQueue<T>::GetEntryWrapper(QuicPacketNumber packet_number) const -> const EntryWrapper*
{
    if (!packet_number.IsInitialized() || IsEmpty() || packet_number < first_packet_) {
        return nullptr;
    }
