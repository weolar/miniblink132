...
static_assert(kMtuDiscoveryAttempts + 8 < 8 * sizeof(QuicPacketNumber), "The number of MTU discovery attempts is too high");
static_assert(kPacketsBetweenMtuProbesBase < (1 << 8), "The initial number of packets between MTU probes is too high");
...
static_assert(kMtuDiscoveryTargetPacketSizeHigh <= kMaxOutgoingPacketSize, "MTU discovery target is too large");
...
public:
    ...
    QuicConnectionMtuDiscoverer(QuicPacketCount packets_between_probes_base, QuicPacketNumber next_probe_at);
    ...
    void Enable(QuicByteCount max_packet_length, QuicByteCount target_max_packet_length);
    ...
    // of a probing packet.
    void OnMaxPacketLengthUpdated(QuicByteCount old_value, QuicByteCount new_value);
    ...
    QuicPacketNumber next_probe_at() const
    {
        return next_probe_at_;
    }

    QUICHE_EXPORT friend std::ostream& operator<<(std::ostream& os, const QuicConnectionMtuDiscoverer& d);
...
private:
    ...
    // sent.
    QuicPacketNumber next_probe_at_ = QuicPacketNumber(kPacketsBetweenMtuProbesBase);
};
