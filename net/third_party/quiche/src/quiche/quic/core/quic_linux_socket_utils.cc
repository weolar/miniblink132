...
void QuicMsgHdr::SetPeerAddress(const QuicSocketAddress& peer_address)
{
    ...
    hdr_.msg_namelen = raw_peer_address_.ss_family == AF_INET ? sizeof(sockaddr_in) : sizeof(sockaddr_in6);
}
...
void QuicMsgHdr::SetIpInNextCmsg(const QuicIpAddress& self_address)
{
    ...
    if (self_address.IsIPv4()) {
        QuicLinuxSocketUtils::SetIpInfoInCmsgData(self_address, GetNextCmsgData<in_pktinfo>(IPPROTO_IP, IP_PKTINFO));
    } else {
        QuicLinuxSocketUtils::SetIpInfoInCmsgData(self_address, GetNextCmsgData<in6_pktinfo>(IPPROTO_IPV6, IPV6_PKTINFO));
    }
}

void* QuicMsgHdr::GetNextCmsgDataInternal(int cmsg_level, int cmsg_type, size_t data_size)
{
...
void QuicMMsgHdr::InitOneHeader(int i, const BufferedWrite& buffered_write)
{
    ...
    *peer_address_storage = buffered_write.peer_address.generic_address();
    ...
    hdr->msg_namelen = peer_address_storage->ss_family == AF_INET ? sizeof(sockaddr_in) : sizeof(sockaddr_in6);
}
...
void QuicMMsgHdr::SetIpInNextCmsg(int i, const QuicIpAddress& self_address)
{
    ...
    if (self_address.IsIPv4()) {
        QuicLinuxSocketUtils::SetIpInfoInCmsgData(self_address, GetNextCmsgData<in_pktinfo>(i, IPPROTO_IP, IP_PKTINFO));
    } else {
        QuicLinuxSocketUtils::SetIpInfoInCmsgData(self_address, GetNextCmsgData<in6_pktinfo>(i, IPPROTO_IPV6, IPV6_PKTINFO));
    }
}

void* QuicMMsgHdr::GetNextCmsgDataInternal(int i, int cmsg_level, int cmsg_type, size_t data_size)
{
    mmsghdr* mhdr = GetMMsgHdr(i);
    msghdr* hdr = &mhdr->msg_hdr;
...
int QuicLinuxSocketUtils::GetUDPSegmentSize(int fd)
{
    ...
    if (rc < 0) {
        QUIC_LOG_EVERY_N_SEC(INFO, 10) << "getsockopt(UDP_SEGMENT) failed: " << strerror(errno);
        return -1;
    }
    QUIC_LOG_EVERY_N_SEC(INFO, 10) << "getsockopt(UDP_SEGMENT) returned segment size: " << optval;
    return optval;
}
...
bool QuicLinuxSocketUtils::EnableReleaseTime(int fd, clockid_t clockid)
{
    ...
    if (setsockopt(fd, SOL_SOCKET, SO_TXTIME, &so_txtime_val, sizeof(so_txtime_val)) != 0) {
        QUIC_LOG_EVERY_N_SEC(INFO, 10) << "setsockopt(SOL_SOCKET,SO_TXTIME) failed: " << strerror(errno);
        return false;
    }
...
// static
void QuicLinuxSocketUtils::SetIpInfoInCmsgData(const QuicIpAddress& self_address, void* cmsg_data)
{
...
// static
size_t QuicLinuxSocketUtils::SetIpInfoInCmsg(const QuicIpAddress& self_address, cmsghdr* cmsg)
{
...
// static
WriteResult QuicLinuxSocketUtils::WriteMultiplePackets(int fd, QuicMMsgHdr* mhdr, int* num_packets_sent)
{
    *num_packets_sent = 0;
    ...
    int rc;
    do {
        rc = GetGlobalSyscallWrapper()->Sendmmsg(fd, mhdr->mhdr(), mhdr->num_msgs(), 0);
    } while (rc < 0 && errno == EINTR);
    ...
    if (rc > 0) {
        *num_packets_sent = rc;
        ...
        QUIC_BUG(quic_bug_10598_3) << "sendmmsg returned 0, returning WRITE_STATUS_ERROR. errno: " << errno;
        errno = EIO;
    }
