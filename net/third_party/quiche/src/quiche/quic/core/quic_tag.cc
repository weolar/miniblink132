...
bool FindMutualQuicTag(const QuicTagVector& our_tags, const QuicTagVector& their_tags, QuicTag* out_result, size_t* out_index)
{
...
std::string QuicTagToString(QuicTag tag)
{
    ...
    for (size_t i = 0; i < ABSL_ARRAYSIZE(chars); i++) {
        ...
        if ((chars[i] == 0 || chars[i] == '\xff') && i == ABSL_ARRAYSIZE(chars) - 1) {
            chars[i] = ' ';
        }
    ...
    return absl::BytesToHexString(absl::string_view(reinterpret_cast<const char*>(&orig_tag), sizeof(orig_tag)));
}
...
QuicTagVector ParseQuicTagVector(absl::string_view tags_string)
{
    ...
    if (!tags_string.empty()) {
        std::vector<absl::string_view> tag_strings = absl::StrSplit(tags_string, ',');
