...
public:
    explicit QuicBatchWriterBase(std::unique_ptr<QuicBatchWriterBuffer> batch_buffer);
    ...
    bool IsWriteBlocked() const final
    {
        return write_blocked_;
    }

    void SetWritable() final
    {
    ...
    QuicByteCount GetMaxPacketSize(const QuicSocketAddress& /*peer_address*/) const final
    {
    ...
    QuicPacketBuffer GetNextWriteLocation(const QuicIpAddress& /*self_address*/, const QuicSocketAddress& /*peer_address*/) final
    {
...
protected:
    const QuicBatchWriterBuffer& batch_buffer() const
    {
        return *batch_buffer_;
    }
    QuicBatchWriterBuffer& batch_buffer()
    {
        return *batch_buffer_;
    }
    ...
    virtual ReleaseTime GetReleaseTime(const QuicPacketWriterParams& /*params*/) const
    {
        QUICHE_DCHECK(false) << "Should not be called since release time is unsupported.";
    ...
    virtual CanBatchResult CanBatch(const char* buffer, size_t buf_len, const QuicIpAddress& self_address, const QuicSocketAddress& peer_address,
        const PerPacketOptions* options, const QuicPacketWriterParams& params, uint64_t release_time) const
        = 0;
...
private:
    WriteResult InternalWritePacket(const char* buffer, size_t buf_len, const QuicIpAddress& self_address, const QuicSocketAddress& peer_address,
...
public:
    QuicUdpBatchWriter(std::unique_ptr<QuicBatchWriterBuffer> batch_buffer, int fd)
        : QuicBatchWriterBase(std::move(batch_buffer))
        , fd_(fd)
    {
    }
