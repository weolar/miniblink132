...
std::string QuicBatchWriterBuffer::DebugString() const
{
    ...
    os << "{ buffer: " << static_cast<const void*>(buffer_) << " buffer_end: " << static_cast<const void*>(buffer_end())
       << " buffered_writes_.size(): " << buffered_writes_.size() << " next_write_loc: " << static_cast<const void*>(GetNextWriteLocation())
       << " SizeInUse: " << SizeInUse() << " }";
    return os.str();
}
...
bool QuicBatchWriterBuffer::Invariants() const
{
    ...
    for (auto iter = buffered_writes_.begin(); iter != buffered_writes_.end(); ++iter) {
        if ((iter->buffer != next_buffer) || (iter->buffer + iter->buf_len > buffer_end())) {
            return false;
        }
...
{
    const char* next_loc = buffered_writes_.empty() ? buffer_ : buffered_writes_.back().buffer + buffered_writes_.back().buf_len;
...
QuicBatchWriterBuffer::PushResult QuicBatchWriterBuffer::PushBufferedWrite(const char* buffer, size_t buf_len, const QuicIpAddress& self_address,
    const QuicSocketAddress& peer_address, const PerPacketOptions* options, const QuicPacketWriterParams& params, uint64_t release_time)
{
    ...
    if (buffer != next_write_location) {
        if (IsExternalBuffer(buffer, buf_len)) {
            ...
            QUIC_BUG(quic_bug_10831_1) << "Buffer[" << static_cast<const void*>(buffer) << ", " << static_cast<const void*>(buffer + buf_len)
                                       << ") overlaps with internal buffer[" << static_cast<const void*>(buffer_) << ", "
    ...
    buffered_writes_.emplace_back(
        next_write_location, buf_len, self_address, peer_address, options ? options->Clone() : std::unique_ptr<PerPacketOptions>(), params, release_time);
...
QuicBatchWriterBuffer::PopResult QuicBatchWriterBuffer::PopBufferedWrite(int32_t num_buffered_writes)
{
    QUICHE_DCHECK(Invariants());
    QUICHE_DCHECK_GE(num_buffered_writes, 0);
    QUICHE_DCHECK_LE(static_cast<size_t>(num_buffered_writes), buffered_writes_.size());
    ...
    result.num_buffers_popped = std::min<int32_t>(result.num_buffers_popped, buffered_writes_.size());
    ...
    if (!buffered_writes_.empty()) {
        ...
        size_t buffer_len_to_move = buffered_writes_.back().buffer + buffered_writes_.back().buf_len - buffer_before_move;
...
size_t QuicBatchWriterBuffer::SizeInUse() const
{
    ...
    return buffered_writes_.back().buffer + buffered_writes_.back().buf_len - buffer_;
}
