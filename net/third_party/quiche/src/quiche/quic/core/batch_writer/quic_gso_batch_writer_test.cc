...
public:
    ...
    static std::unique_ptr<TestQuicGsoBatchWriter> NewInstanceWithReleaseTimeSupport()
    {
        return std::unique_ptr<TestQuicGsoBatchWriter>(new TestQuicGsoBatchWriter(std::make_unique<QuicBatchWriterBuffer>(),
...
struct QUICHE_EXPORT TestBufferedWrite : public BufferedWrite {
    ...
    TestBufferedWrite(const TestBufferedWrite& other)
        : BufferedWrite(other.buffer, other.buf_len, other.self_address, other.peer_address,
            other.options ? other.options->Clone() : std::unique_ptr<PerPacketOptions>(), QuicPacketWriterParams(), other.release_time)
    {
    }
};
...
struct QUICHE_EXPORT BatchCriteriaTestData {
    BatchCriteriaTestData(
        size_t buf_len, const QuicIpAddress& self_address, const QuicSocketAddress& peer_address, uint64_t release_time, bool can_batch, bool must_flush)
        : buffered_write(unused_packet_buffer, buf_len, self_address, peer_address, std::unique_ptr<PerPacketOptions>(), QuicPacketWriterParams(), release_time)
        , can_batch(can_batch)
...
std::vector<BatchCriteriaTestData> BatchCriteriaTestData_SizeDecrease()
{
    ...
    std::vector<BatchCriteriaTestData> test_data_table = {
        ...
        { 1350, self_addr, peer_addr, 0, true, false }, { 1350, self_addr, peer_addr, 0, true, false }, { 1350, self_addr, peer_addr, 0, true, false },
        { 39, self_addr, peer_addr, 0, true, true }, { 39, self_addr, peer_addr, 0, false, true }, { 1350, self_addr, peer_addr, 0, false, true },
        // clang-format on
    };
...
std::vector<BatchCriteriaTestData> BatchCriteriaTestData_MaxSegments(size_t gso_size)
{
    const QuicIpAddress self_addr;
    const QuicSocketAddress peer_addr;
    std::vector<BatchCriteriaTestData> test_data_table;
    ...
    test_data_table.push_back({ gso_size, self_addr, peer_addr, /*release_time=*/0, false, true });
    return test_data_table;
}
...
protected:
    ...
    {
        return writer->WritePacket(&packet_buffer_[0], packet_size, self_address_, peer_address_, nullptr, QuicPacketWriterParams());
    }

    WriteResult WritePacketWithParams(QuicGsoBatchWriter* writer, QuicPacketWriterParams& params)
    {
        return writer->WritePacket(&packet_buffer_[0], 1350, self_address_, peer_address_, nullptr, params);
    }
...
TEST_F(QuicGsoBatchWriterTest, BatchCriteria)
{
    ...
    for (size_t i = 0; i < test_data_tables.size(); ++i) {
        ...
        for (size_t j = 0; j < test_data_table.size(); ++j) {
            ...
            params.release_time_delay = QuicTime::Delta::FromMicroseconds(test_data.buffered_write.release_time);
            TestQuicGsoBatchWriter::CanBatchResult result = writer->CanBatch(test_data.buffered_write.buffer, test_data.buffered_write.buf_len,
                test_data.buffered_write.self_address, test_data.buffered_write.peer_address, nullptr, params, test_data.buffered_write.release_time);
            ...
            if (result.can_batch) {
                ASSERT_TRUE(writer->batch_buffer()
                                .PushBufferedWrite(test_data.buffered_write.buffer, test_data.buffered_write.buf_len, test_data.buffered_write.self_address,
                                    test_data.buffered_write.peer_address, nullptr, params, test_data.buffered_write.release_time)
                                .succeeded);
            }
        }
    }
...
TEST_F(QuicGsoBatchWriterTest, WriteSuccess)
{
    ...
    EXPECT_CALL(mock_syscalls_, Sendmsg(_, _, _)).WillOnce(Invoke([](int /*sockfd*/, const msghdr* msg, int /*flags*/) {
...
TEST_F(QuicGsoBatchWriterTest, WriteBlockDataNotBuffered)
{
    ...
    ASSERT_EQ(WriteResult(WRITE_STATUS_BLOCKED, EWOULDBLOCK), WritePacket(&writer, 150));
...
TEST_F(QuicGsoBatchWriterTest, WriteBlockDataBuffered)
{
    ...
    ASSERT_EQ(WriteResult(WRITE_STATUS_BLOCKED_DATA_BUFFERED, EWOULDBLOCK), WritePacket(&writer, 50));
...
TEST_F(QuicGsoBatchWriterTest, ReleaseTime)
{
    ...
    EXPECT_EQ(MillisToNanos(1), writer->GetReleaseTime(params).actual_release_time);
    ...
    params.allow_burst = true;
    EXPECT_EQ(MillisToNanos(6), writer->GetReleaseTime(params).actual_release_time);
    ASSERT_EQ(WriteResult(WRITE_STATUS_OK, 3000), writer->WritePacket(&packet_buffer_[0], 300, self_address_, peer_address_, nullptr, params));
...
TEST_F(QuicGsoBatchWriterTest, EcnCodepoint)
{
    ...
    EXPECT_CALL(mock_syscalls_, Sendmsg(_, _, _)).WillOnce(Invoke([](int /*sockfd*/, const msghdr* msg, int /*flags*/) {
        ...
        for (struct cmsghdr* cmsg = CMSG_FIRSTHDR(&mutable_msg); cmsg != NULL; cmsg = CMSG_NXTHDR(&mutable_msg, cmsg)) {
...
TEST_F(QuicGsoBatchWriterTest, FlowLabelIPv6)
{
    ...
    for (uint32_t i = 1; i < 5; ++i) {
        ...
        EXPECT_CALL(mock_syscalls_, Sendmsg(_, _, _)).WillOnce(Invoke([&params](int /*sockfd*/, const msghdr* msg, int /*flags*/) {
            ...
            for (struct cmsghdr* cmsg = CMSG_FIRSTHDR(&mutable_msg); cmsg != NULL; cmsg = CMSG_NXTHDR(&mutable_msg, cmsg)) {
                if (cmsg->cmsg_level == IPPROTO_IPV6 && cmsg->cmsg_type == IPV6_FLOWINFO) {
                    ...
                    uint32_t cmsg_flow_label = ntohl(*reinterpret_cast<uint32_t*> CMSG_DATA(cmsg));
