...
static bool IsAddressFamilySupported(int address_family)
{
    static auto check_function = [](int address_family) {
        ...
        if (fd < 0) {
            QUIC_LOG(ERROR) << "address_family not supported: " << address_family << ", error: " << strerror(errno);
...
static bool CreateSocket(int family, QuicSocketAddress* address, int* fd)
{
    ...
    *fd = socket_api.Create(family,
    ...
    if (address->FromSocket(*fd) != 0) {
        QUIC_LOG(ERROR) << "Unable to get self address.  Error: " << strerror(errno);
        return false;
    }
    return true;
}
...
struct QUICHE_EXPORT QuicUdpBatchWriterIOTestParams {
    ...
    QUICHE_EXPORT friend std::ostream& operator<<(std::ostream& os, const QuicUdpBatchWriterIOTestParams& p)
    {
        os << "{ address_family: " << p.address_family << " data_size: " << p.data_size << " packet_size: " << p.packet_size << " }";
        return os;
    }
};

template <class QuicUdpBatchWriterIOTestDelegateT> static std::vector<QuicUdpBatchWriterIOTestParams> MakeQuicBatchWriterTestParams()
{
    static_assert(std::is_base_of<QuicUdpBatchWriterIOTestDelegate, QuicUdpBatchWriterIOTestDelegateT>::value,
        ...
        for (int data_size : { 1, 150, 1500, 15000, 64000, 512 * 1024 }) {
            for (int packet_size : { 1, 50, 1350, 1452 }) {
                if (packet_size <= data_size && (data_size / packet_size < 2000)) {
                    params.push_back({ std::make_unique<QuicUdpBatchWriterIOTestDelegateT>(), address_family, data_size, packet_size });
                }
            }
        }
    }
...
class QUICHE_EXPORT QuicUdpBatchWriterIOTest : public QuicTestWithParam<QuicUdpBatchWriterIOTestParams> {
protected:
    ...
    bool ShouldSkip()
    {
        if (!IsAddressFamilySupported(address_family_)) {
            QUIC_LOG(WARNING) << "Test skipped since address_family is not supported.";
            return true;
        }
    ...
    void Initialize()
    {
        ...
        QUIC_DLOG(INFO) << "Self address: " << self_address_.ToString() << ", fd " << self_socket_;
        QUIC_DLOG(INFO) << "Peer address: " << peer_address_.ToString() << ", fd " << peer_socket_;
        GetParam().delegate->ResetWriter(self_socket_);
    }

    QuicUdpBatchWriter* GetWriter()
    {
        return GetParam().delegate->GetWriter();
    }
    ...
    void ValidateWrite()
    {
        ...
        for (size_t bytes_sent = 0; bytes_sent < data_size_; bytes_sent += this_packet_size, ++this_packet_content) {
            this_packet_size = std::min(packet_size_, data_size_ - bytes_sent);
            memset(&packet_buffer_[0], this_packet_content, this_packet_size);

            result = GetWriter()->WritePacket(&packet_buffer_[0], this_packet_size, self_address_.host(), peer_address_, nullptr, QuicPacketWriterParams());

            ASSERT_EQ(WRITE_STATUS_OK, result.status) << strerror(result.error_code);
            bytes_flushed += result.bytes_written;
            ++num_writes;

            QUIC_DVLOG(1) << "[write #" << num_writes << "] this_packet_size: " << this_packet_size << ", total_bytes_sent: " << bytes_sent + this_packet_size
                          << ", bytes_flushed: " << bytes_flushed << ", pkt content:" << std::hex << int(this_packet_content);
        }
        ...
        QUIC_LOG(INFO) << "Sent " << data_size_ << " bytes in " << num_writes << " writes.";
    }
    ...
    void ValidateRead()
    {
        ...
        for (size_t bytes_received = 0; bytes_received < data_size_; bytes_received += this_packet_size, ++this_packet_content) {
            this_packet_size = std::min(packet_size_, data_size_ - bytes_received);
            SCOPED_TRACE(testing::Message() << "Before ReadPacket: bytes_received=" << bytes_received << ", this_packet_size=" << this_packet_size);
            ...
            ASSERT_TRUE(QuicUdpSocketApi().WaitUntilReadable(peer_socket_, QuicTime::Delta::FromSeconds(1)));
            QuicUdpSocketApi().ReadPacket(peer_socket_,
                quic::QuicUdpPacketInfoBitMask({ QuicUdpPacketInfoBit::V4_SELF_IP, QuicUdpPacketInfoBit::V6_SELF_IP, QuicUdpPacketInfoBit::PEER_ADDRESS }),
                &result);
            ASSERT_TRUE(result.ok);
            ASSERT_TRUE(result.packet_info.HasValue(QuicUdpPacketInfoBit::PEER_ADDRESS));
            QuicSocketAddress read_peer_address = result.packet_info.peer_address();
            QuicIpAddress read_self_address = read_peer_address.host().IsIPv6() ? result.packet_info.self_v6_ip() : result.packet_info.self_v4_ip();
        ...
        QUIC_LOG(INFO) << "Received " << data_size_ << " bytes in " << packets_received << " packets.";
    }

    QuicSocketAddress self_address_;
    QuicSocketAddress peer_address_;
    ABSL_CACHELINE_ALIGNED char packet_buffer_[1500];
    ABSL_CACHELINE_ALIGNED char control_buffer_[kDefaultUdpPacketControlBufferSize];
    int address_family_;
