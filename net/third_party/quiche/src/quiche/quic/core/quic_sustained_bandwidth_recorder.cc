...
void QuicSustainedBandwidthRecorder::RecordEstimate(
    bool in_recovery, bool in_slow_start, QuicBandwidth bandwidth, QuicTime estimate_time, QuicWallTime wall_time, QuicTime::Delta srtt)
{
    if (in_recovery) {
        is_recording_ = false;
        QUIC_DVLOG(1) << "Stopped recording at: " << estimate_time.ToDebuggingValue();
        return;
    }
    ...
    if (estimate_time - start_time_ >= 3 * srtt) {
        ...
        QUIC_DVLOG(1) << "New sustained bandwidth estimate (KBytes/s): " << bandwidth_estimate_.ToKBytesPerSecond();
    }
    ...
    if (bandwidth > max_bandwidth_estimate_) {
        ...
        QUIC_DVLOG(1) << "New max bandwidth estimate (KBytes/s): " << max_bandwidth_estimate_.ToKBytesPerSecond();
    }
}
