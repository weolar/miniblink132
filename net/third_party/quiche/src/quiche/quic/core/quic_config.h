...
public:
    ...
    virtual QuicErrorCode ProcessPeerHello(const CryptoHandshakeMessage& peer_hello, HelloType hello_type, std::string* error_details) = 0;
...
public:
    ...
    QuicErrorCode ProcessPeerHello(const CryptoHandshakeMessage& peer_hello, HelloType hello_type, std::string* error_details) override;
...
public:
    ...
    int32_t GetDiscardLengthReceived() const
    {
        return discard_length_received_;
    }
    ...
    bool HasClientSentConnectionOption(QuicTag tag, Perspective perspective) const;

    void SetClientConnectionOptions(const QuicTagVector& client_connection_options);
    ...
    bool HasClientRequestedIndependentOption(QuicTag tag, Perspective perspective) const;

    const QuicTagVector& ClientRequestedIndependentOptions(Perspective perspective) const;
    ...
    void set_max_time_before_crypto_handshake(QuicTime::Delta max_time_before_crypto_handshake)
    {
    ...
    void set_max_idle_time_before_crypto_handshake(QuicTime::Delta max_idle_time_before_crypto_handshake)
    {
    ...
    void SetInitialMaxStreamDataBytesIncomingBidirectionalToSend(uint64_t window_bytes);
    ...
    // returns the value passed to SetInitialStreamFlowControlWindowToSend.
    void SetInitialMaxStreamDataBytesOutgoingBidirectionalToSend(uint64_t window_bytes);
    ...
    void SetIPv6AlternateServerAddressToSend(const QuicSocketAddress& alternate_server_address_ipv6);
    ...
    void SetIPv4AlternateServerAddressToSend(const QuicSocketAddress& alternate_server_address_ipv4);
    ...
    void SetPreferredAddressConnectionIdAndTokenToSend(const QuicConnectionId& connection_id, const StatelessResetToken& stateless_reset_token);

    // Preferred Address Connection ID and Token.
    bool HasReceivedPreferredAddressConnectionIdAndToken() const;
    const std::pair<QuicConnectionId, StatelessResetToken>& ReceivedPreferredAddressConnectionIdAndToken() const;
    std::optional<QuicSocketAddress> GetPreferredAddressToSend(quiche::IpAddressFamily address_family) const;
    void ClearAlternateServerAddressToSend(quiche::IpAddressFamily address_family);
    ...
    std::optional<QuicSocketAddress> GetMappedAlternativeServerAddress(quiche::IpAddressFamily address_family) const;
    ...
    void SetOriginalConnectionIdToSend(const QuicConnectionId& original_destination_connection_id);
    bool HasReceivedOriginalConnectionId() const;
    QuicConnectionId ReceivedOriginalConnectionId() const;

    // Stateless reset token.
    void SetStatelessResetTokenToSend(const StatelessResetToken& stateless_reset_token);
    ...
    void SetInitialSourceConnectionIdToSend(const QuicConnectionId& initial_source_connection_id);
    ...
    void SetRetrySourceConnectionIdToSend(const QuicConnectionId& retry_source_connection_id);
    ...
    void ToHandshakeMessage(CryptoHandshakeMessage* out, QuicTransportVersion transport_version) const;
    ...
    QuicErrorCode ProcessPeerHello(const CryptoHandshakeMessage& peer_hello, HelloType hello_type, std::string* error_details);
    ...
    // |*error_details|.
    QuicErrorCode ProcessTransportParameters(const TransportParameters& params, bool is_resumption, std::string* error_details);

    TransportParameters::ParameterMap& custom_transport_parameters_to_send()
    {
        return custom_transport_parameters_to_send_;
    }
    const TransportParameters::ParameterMap& received_custom_transport_parameters() const
    {
...
private:
    ...
    std::optional<std::pair<QuicConnectionId, StatelessResetToken>> preferred_address_connection_id_and_token_;
