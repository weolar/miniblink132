...
public:
    ...
    MOCK_METHOD(void, OnUnrecoverableError, (QuicErrorCode error, const std::string& details), (override));
    MOCK_METHOD(void, OnUnrecoverableError, (QuicErrorCode error, QuicIetfTransportErrorCodes ietf_error, const std::string& details), (override));
...
protected:
    ...
    bool VerifyIovecs(iovec* iovecs, size_t num_iovecs, const std::vector<std::string>& expected)
    {
        return VerifyIovecs(*sequencer_, iovecs, num_iovecs, expected);
    }

    bool VerifyReadableRegion(const QuicStreamSequencer& sequencer, const std::vector<std::string>& expected)
    {
        iovec iovecs[1];
        if (sequencer.GetReadableRegions(iovecs, 1)) {
            return (VerifyIovecs(sequencer, iovecs, 1, std::vector<std::string> { expected[0] }));
        }
        return false;
    }
    ...
    // Verify that the data in each of currently readable regions match with each
    // item given in |expected|.
    bool VerifyReadableRegions(const QuicStreamSequencer& sequencer, const std::vector<std::string>& expected)
    {
        iovec iovecs[5];
        size_t num_iovecs = sequencer.GetReadableRegions(iovecs, ABSL_ARRAYSIZE(iovecs));
        return VerifyReadableRegion(sequencer, expected) && VerifyIovecs(sequencer, iovecs, num_iovecs, expected);
    }

    bool VerifyIovecs(const QuicStreamSequencer& /*sequencer*/, iovec* iovecs, size_t num_iovecs, const std::vector<std::string>& expected)
    {
        int start_position = 0;
        for (size_t i = 0; i < num_iovecs; ++i) {
            if (!VerifyIovec(iovecs[i], expected[0].substr(start_position, iovecs[i].iov_len))) {
                return false;
            }
    ...
    bool VerifyIovec(const iovec& iovec, absl::string_view expected)
    {
        if (iovec.iov_len != expected.length()) {
            QUIC_LOG(ERROR) << "Invalid length: " << iovec.iov_len << " vs " << expected.length();
            return false;
        }
        if (memcmp(iovec.iov_base, expected.data(), expected.length()) != 0) {
            QUIC_LOG(ERROR) << "Invalid data: " << static_cast<char*>(iovec.iov_base) << " vs " << expected;
            return false;
        }
        return true;
    }
...
TEST_F(QuicStreamSequencerTest, RejectOldFrame)
{
    ...
    EXPECT_CALL(stream_, OnDataAvailable()).WillOnce(testing::Invoke([this]() { ConsumeData(3); }));
...
TEST_F(QuicStreamSequencerTest, PartialFrameConsumed)
{
    ...
    EXPECT_CALL(stream_, OnDataAvailable()).WillOnce(testing::Invoke([this]() { ConsumeData(2); }));
...
TEST_F(QuicStreamSequencerTest, OutOfOrderFrameProcessed)
{
    ...
    EXPECT_CALL(stream_, OnDataAvailable()).WillOnce(testing::Invoke([this]() { ConsumeData(9); }));
...
TEST_F(QuicStreamSequencerTest, BasicHalfCloseUnorderedWithFlush)
{
    ...
    EXPECT_CALL(stream_, OnDataAvailable()).WillOnce(testing::Invoke([this]() { ConsumeData(6); }));
    EXPECT_FALSE(sequencer_->IsClosed());
    OnFrame(0, "abc");
    EXPECT_TRUE(sequencer_->IsClosed());
}
...
TEST_F(QuicStreamSequencerTest, MultipleOffsets)
{
    ...
    EXPECT_CALL(stream_,
        OnUnrecoverableError(QUIC_STREAM_SEQUENCER_INVALID_STATE,
...
public:
    ...
    void CreateFrames()
    {
        ...
        while (remaining_payload != 0) {
            ...
            list_.push_back(std::make_pair(index, std::string(kPayload + index, size)));
...
{
    EXPECT_CALL(stream_, OnDataAvailable()).Times(AnyNumber()).WillRepeatedly(Invoke(this, &QuicSequencerRandomTest::ReadAvailableData));
    QuicByteCount total_bytes_consumed = 0;
    EXPECT_CALL(stream_, AddBytesConsumed(_)).Times(AnyNumber()).WillRepeatedly(testing::Invoke([&total_bytes_consumed](QuicByteCount bytes) {
    ...
    while (output_.size() != ABSL_ARRAYSIZE(kPayload) - 1) {
        if (!list_.empty() && OneToN(2) == 1) { // Send data
            ...
            for (int i = 0; i < iovs_peeked; ++i) {
                int bytes_to_peek = std::min<int>(peek_iov[i].iov_len, total_bytes_to_peek);
...
TEST_F(QuicStreamSequencerTest, MarkConsumedError)
{
    ...
    EXPECT_QUIC_BUG(
        {
            EXPECT_CALL(stream_, ResetWithError(QuicResetStreamError::FromInternal(QUIC_ERROR_PROCESSING_STREAM)));
...
TEST_F(QuicStreamSequencerTest, OverlappingFramesReceived)
{
    ...
    EXPECT_CALL(stream_, OnUnrecoverableError(QUIC_OVERLAPPING_STREAM_DATA, _)).Times(0);
