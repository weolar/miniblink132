...
const std::optional<ParsedClientHello> kDefaultParsedChlo = absl::make_optional<ParsedClientHello>();
...
EnqueuePacketResult EnqueuePacketToStore(QuicBufferedPacketStore& store, QuicConnectionId connection_id, PacketHeaderFormat form,
    QuicLongHeaderType long_packet_type, const QuicReceivedPacket& packet, QuicSocketAddress self_address, QuicSocketAddress peer_address,
    const ParsedQuicVersion& version, std::optional<ParsedClientHello> parsed_chlo, ConnectionIdGeneratorInterface& connection_id_generator)
{
    ...
    return store.EnqueuePacket(packet_info, std::move(parsed_chlo), connection_id_generator);
}

class QuicBufferedPacketStoreVisitor : public QuicBufferedPacketStore::VisitorInterface {
public:
    ...
    HandleCidCollisionResult HandleConnectionIdCollision(const QuicConnectionId& /*original_connection_id*/, const QuicConnectionId& /*replaced_connection_id*/,
        const QuicSocketAddress& /*self_address*/, const QuicSocketAddress& /*peer_address*/, ParsedQuicVersion /*version*/,
...
TEST_F(QuicBufferedPacketStoreTest, SimpleEnqueueAndDeliverPacket)
{
    ...
    EnqueuePacketToStore(store_, connection_id, GOOGLE_QUIC_PACKET, INVALID_PACKET_TYPE, packet_, self_address_, peer_address_, invalid_version_, kNoParsedChlo,
...
TEST_F(QuicBufferedPacketStoreTest, SimpleEnqueueAckSent)
{
    ...
    QuicFramer client_framer(ParsedQuicVersionVector { ParsedQuicVersion::RFCv1() }, QuicTime::Zero(), Perspective::IS_CLIENT, 8);
    ...
    size_t length = client_framer.BuildDataPacket(header, frames, buffer, 1500, level);

    ASSERT_GT(length, 0);

    // Re-construct the data packet with data ownership.
    auto data = std::make_unique<QuicPacket>(buffer, length, /* owns_buffer */ true, GetIncludedDestinationConnectionIdLength(header),
    ...
    size_t final_size = client_framer.EncryptPayload(ENCRYPTION_INITIAL, header.packet_number, *data, (char*)raw, 1500);
    QuicReceivedPacket packet((char*)raw, final_size, QuicTime::Zero(), false, 0, true, nullptr, 0, false, ECN_ECT1);
    ...
    EXPECT_CALL(writer, WritePacket(_, _, _, _, _, _))
        .WillOnce(Invoke([&](const char* buffer, size_t buf_len, const QuicIpAddress& /*self_address*/, const QuicSocketAddress& /*peer_address*/,
                             PerPacketOptions* /*options*/, const QuicPacketWriterParams& /*params*/) {
            auto tmp_packet = std::make_unique<QuicEncryptedPacket>(buffer, buf_len);
            ack_packet = tmp_packet->Clone();
            return WriteResult(WRITE_STATUS_OK, 1);
        }));
    EXPECT_CALL(writer, Flush());
    EnqueuePacketToStore(store_, connection_id, IETF_QUIC_LONG_HEADER_PACKET, INITIAL, packet, self_address_, peer_address_, ParsedQuicVersion::RFCv1(),
    ...
    EXPECT_EQ(buffered_list->dispatcher_sent_packets[0].largest_acked, QuicPacketNumber(1));
    ...
    EXPECT_CALL(mock_framer_visitor, OnAckFrameStart(_, _)).WillOnce(Return(true));
    EXPECT_CALL(mock_framer_visitor, OnAckRange(_, _)).WillOnce(Return(true));
    std::optional<QuicEcnCounts> counts = QuicEcnCounts(0, 1, 0);
    EXPECT_CALL(mock_framer_visitor, OnAckFrameEnd(_, counts)).WillOnce(Return(true));
...
TEST_F(QuicBufferedPacketStoreTest, DifferentPacketAddressOnOneConnection)
{
    ...
    EnqueuePacketToStore(store_, connection_id, GOOGLE_QUIC_PACKET, INVALID_PACKET_TYPE, packet_, self_address_, peer_address_, invalid_version_, kNoParsedChlo,
        connection_id_generator_);
    EnqueuePacketToStore(store_, connection_id, GOOGLE_QUIC_PACKET, INVALID_PACKET_TYPE, packet_, self_address_, addr_with_new_port, invalid_version_,
        kNoParsedChlo, connection_id_generator_);
    std::list<BufferedPacket> queue = store_.DeliverPackets(connection_id).buffered_packets;
...
TEST_F(QuicBufferedPacketStoreTest, EnqueueAndDeliverMultiplePacketsOnMultipleConnections)
{
    ...
    for (uint64_t conn_id = 1; conn_id <= num_connections; ++conn_id) {
        ...
        EnqueuePacketToStore(store_, connection_id, GOOGLE_QUIC_PACKET, INVALID_PACKET_TYPE, packet_, self_address_, peer_address_, invalid_version_,
            kNoParsedChlo, connection_id_generator_);
        EnqueuePacketToStore(store_, connection_id, GOOGLE_QUIC_PACKET, INVALID_PACKET_TYPE, packet_, self_address_, peer_address_, invalid_version_,
            kNoParsedChlo, connection_id_generator_);
    }
...
// buffered.
TEST_F(QuicBufferedPacketStoreTest, FailToBufferTooManyPacketsOnExistingConnection)
{
    ...
    EXPECT_EQ(QuicBufferedPacketStore::SUCCESS,
        EnqueuePacketToStore(store_, connection_id, IETF_QUIC_LONG_HEADER_PACKET, INITIAL, packet_, self_address_, peer_address_, valid_version_,
    ...
    for (size_t i = 1; i <= kMaxPacketsPerConnection; ++i) {
        ...
        EnqueuePacketResult result = EnqueuePacketToStore(store_, connection_id, GOOGLE_QUIC_PACKET, INVALID_PACKET_TYPE, packet_, self_address_, peer_address_,
    ...
    EXPECT_EQ(store_.DeliverPackets(connection_id).buffered_packets.size(), kMaxPacketsPerConnection);
}
...
TEST_F(QuicBufferedPacketStoreTest, FullStoreFailToBufferDataPacketOnNewConnection)
{
    // Send enough CHLOs so that store gets full before number of connections
    // without CHLO reaches its upper limit.
    size_t num_chlos = kDefaultMaxConnectionsInStore - kMaxConnectionsWithoutCHLO + 1;
    for (uint64_t conn_id = 1; conn_id <= num_chlos; ++conn_id) {
        EXPECT_EQ(EnqueuePacketResult::SUCCESS,
            EnqueuePacketToStore(store_, TestConnectionId(conn_id), GOOGLE_QUIC_PACKET, INVALID_PACKET_TYPE, packet_, self_address_, peer_address_,
    ...
    for (uint64_t conn_id = num_chlos + 1; conn_id <= (kDefaultMaxConnectionsInStore + 1); ++conn_id) {
        QuicConnectionId connection_id = TestConnectionId(conn_id);
        EnqueuePacketResult result = EnqueuePacketToStore(store_, connection_id, GOOGLE_QUIC_PACKET, INVALID_PACKET_TYPE, packet_, self_address_, peer_address_,
            valid_version_, kDefaultParsedChlo, connection_id_generator_);
...
{
    EXPECT_EQ(EnqueuePacketResult::SUCCESS,
        EnqueuePacketToStore(store_, TestConnectionId(1), GOOGLE_QUIC_PACKET, INVALID_PACKET_TYPE, packet_, self_address_, peer_address_, valid_version_,
            kDefaultParsedChlo, connection_id_generator_));
    QuicConnectionId delivered_conn_id;
    BufferedPacketList packet_list = store_.DeliverPacketsForNextConnection(&delivered_conn_id);
...
TEST_F(QuicBufferedPacketStoreTest, EnqueueChloOnTooManyDifferentConnections)
{
    ...
    for (size_t i = kMaxConnectionsWithoutCHLO + 1; i <= kDefaultMaxConnectionsInStore + 1; ++i) {
        QuicConnectionId connection_id = TestConnectionId(i);
        EnqueuePacketResult rs = EnqueuePacketToStore(store_, connection_id, GOOGLE_QUIC_PACKET, INVALID_PACKET_TYPE, packet_, self_address_, peer_address_,
            valid_version_, kDefaultParsedChlo, connection_id_generator_);
    ...
    QuicConnectionId delivered_conn_id;
    for (size_t i = 0; i < kDefaultMaxConnectionsInStore - kMaxConnectionsWithoutCHLO + 1; ++i) {
        BufferedPacketList packet_list = store_.DeliverPacketsForNextConnection(&delivered_conn_id);
        if (i < kDefaultMaxConnectionsInStore - kMaxConnectionsWithoutCHLO) {
            // Only CHLO is buffered.
            EXPECT_EQ(1u, packet_list.buffered_packets.size());
            EXPECT_EQ(TestConnectionId(i + kMaxConnectionsWithoutCHLO + 1), delivered_conn_id);
        } else {
            EXPECT_EQ(2u, packet_list.buffered_packets.size());
...
TEST_F(QuicBufferedPacketStoreTest, PacketQueueExpiredBeforeDelivery)
{
    ...
    EXPECT_EQ(EnqueuePacketResult::SUCCESS,
        EnqueuePacketToStore(store_, connection_id2, GOOGLE_QUIC_PACKET, INVALID_PACKET_TYPE, packet_, self_address_, peer_address_, invalid_version_,
            kNoParsedChlo, connection_id_generator_));
    ...
    EnqueuePacketToStore(store_, connection_id3, GOOGLE_QUIC_PACKET, INVALID_PACKET_TYPE, packet_, self_address_, another_client_address, valid_version_,
        kDefaultParsedChlo, connection_id_generator_);

    // Advance clock to the time when connection 1 and 2 expires.
    clock_.AdvanceTime(QuicBufferedPacketStorePeer::expiration_alarm(&store_)->deadline() - clock_.ApproximateNow());
    ASSERT_GE(clock_.ApproximateNow(), QuicBufferedPacketStorePeer::expiration_alarm(&store_)->deadline());
    // Fire alarm to remove long-staying connection 1 and 2 packets.
    alarm_factory_.FireAlarm(QuicBufferedPacketStorePeer::expiration_alarm(&store_));
    ...
    EXPECT_EQ(another_client_address, packet_list.buffered_packets.front().peer_address);
    ...
    EnqueuePacketToStore(store_, connection_id4, GOOGLE_QUIC_PACKET, INVALID_PACKET_TYPE, packet_, self_address_, peer_address_, invalid_version_,
        kNoParsedChlo, connection_id_generator_);
    EnqueuePacketToStore(store_, connection_id4, GOOGLE_QUIC_PACKET, INVALID_PACKET_TYPE, packet_, self_address_, peer_address_, invalid_version_,
        kNoParsedChlo, connection_id_generator_);
    clock_.AdvanceTime(QuicBufferedPacketStorePeer::expiration_alarm(&store_)->deadline() - clock_.ApproximateNow());
...
TEST_F(QuicBufferedPacketStoreTest, MultipleDiscardPackets)
{
    ...
    EnqueuePacketToStore(store_, connection_id_1, GOOGLE_QUIC_PACKET, INVALID_PACKET_TYPE, packet_, self_address_, peer_address_, invalid_version_,
        kNoParsedChlo, connection_id_generator_);
    EnqueuePacketToStore(store_, connection_id_1, GOOGLE_QUIC_PACKET, INVALID_PACKET_TYPE, packet_, self_address_, peer_address_, invalid_version_,
        kNoParsedChlo, connection_id_generator_);
    ...
    EnqueuePacketToStore(store_, connection_id_2, IETF_QUIC_LONG_HEADER_PACKET, INITIAL, packet_, self_address_, peer_address_, valid_version_, parsed_chlo,
        connection_id_generator_);
...
TEST_F(QuicBufferedPacketStoreTest, IngestPacketForTlsChloExtraction)
{
    ...
    EXPECT_FALSE(store_.HasBufferedPackets(connection_id));
    EnqueuePacketToStore(store_, connection_id, GOOGLE_QUIC_PACKET, INVALID_PACKET_TYPE, packet_, self_address_, peer_address_, valid_version_, kNoParsedChlo,
        connection_id_generator_);
    EXPECT_TRUE(store_.HasBufferedPackets(connection_id));

    // The packet in 'packet_' is not a TLS CHLO packet.
    EXPECT_FALSE(store_.IngestPacketForTlsChloExtraction(connection_id, valid_version_, packet_, &supported_groups, &cert_compression_algos, &alpns, &sni,
    ...
    config.custom_transport_parameters_to_send()[kCustomParameterId] = kCustomParameterValue;
    auto packets = GetFirstFlightOfPackets(valid_version_, config);
    ASSERT_EQ(packets.size(), 2u);

    EnqueuePacketToStore(store_, connection_id, GOOGLE_QUIC_PACKET, INVALID_PACKET_TYPE, *packets[0], self_address_, peer_address_, valid_version_,
        kNoParsedChlo, connection_id_generator_);
    EnqueuePacketToStore(store_, connection_id, GOOGLE_QUIC_PACKET, INVALID_PACKET_TYPE, *packets[1], self_address_, peer_address_, valid_version_,
        kNoParsedChlo, connection_id_generator_);

    EXPECT_TRUE(store_.HasBufferedPackets(connection_id));
    EXPECT_FALSE(store_.IngestPacketForTlsChloExtraction(connection_id, valid_version_, *packets[0], &supported_groups, &cert_compression_algos, &alpns, &sni,
        &resumption_attempted, &early_data_attempted, &tls_alert));
    EXPECT_TRUE(store_.IngestPacketForTlsChloExtraction(connection_id, valid_version_, *packets[1], &supported_groups, &cert_compression_algos, &alpns, &sni,
        &resumption_attempted, &early_data_attempted, &tls_alert));
...
TEST_F(QuicBufferedPacketStoreTest, DeliverInitialPacketsFirst)
{
    ...
    config.custom_transport_parameters_to_send()[kCustomParameterId] = custom_parameter_value;
    ...
    EXPECT_THAT(initial_packets, Each(Truly([](const std::unique_ptr<QuicReceivedPacket>& packet) {
        ...
        QuicErrorCode error_code = QuicFramer::ParsePublicHeaderDispatcher(*packet, kQuicDefaultConnectionIdLength, &unused_format, &long_packet_type,
            &unused_version_flag, &unused_use_length_prefix, &unused_version_label, &unused_parsed_version, &unused_destination_connection_id,
            &unused_source_connection_id, &unused_retry_token, &unused_detailed_error);
    ...
    error_code = QuicFramer::ParsePublicHeaderDispatcher(packet_, kQuicDefaultConnectionIdLength, &packet_format, &long_packet_type, &unused_version_flag,
        &unused_use_length_prefix, &unused_version_label, &unused_parsed_version, &unused_destination_connection_id, &unused_source_connection_id,
    ...
    EnqueuePacketToStore(
        store_, connection_id, packet_format, long_packet_type, packet_, self_address_, peer_address_, valid_version_, kNoParsedChlo, connection_id_generator_);
    EnqueuePacketToStore(store_, connection_id, IETF_QUIC_LONG_HEADER_PACKET, INITIAL, *initial_packets[0], self_address_, peer_address_, valid_version_,
        kNoParsedChlo, connection_id_generator_);
    EnqueuePacketToStore(store_, connection_id, IETF_QUIC_LONG_HEADER_PACKET, INITIAL, *initial_packets[1], self_address_, peer_address_, valid_version_,
        kNoParsedChlo, connection_id_generator_);
    ...
    for (const auto& packet : delivered_packets.buffered_packets) {
        error_code = QuicFramer::ParsePublicHeaderDispatcher(*packet.packet, kQuicDefaultConnectionIdLength, &packet_format, &long_packet_type,
            &unused_version_flag, &unused_use_length_prefix, &unused_version_label, &unused_parsed_version, &unused_destination_connection_id,
            &unused_source_connection_id, &unused_retry_token, &unused_detailed_error);
        EXPECT_THAT(error_code, IsQuicNoError());

        // INITIAL packets should not follow a non-INITIAL packet.
        EXPECT_THAT(long_packet_type, Conditional(previous_packet_type == INITIAL, A<QuicLongHeaderType>(), Ne(INITIAL)));
...
{
    QuicConnectionId connection_id = TestConnectionId(1);
    QuicReceivedPacket ect1_packet(packet_content_.data(), packet_content_.size(), packet_time_, false, 0, true, nullptr, 0, false, ECN_ECT1);
    EnqueuePacketToStore(store_, connection_id, GOOGLE_QUIC_PACKET, INVALID_PACKET_TYPE, ect1_packet, self_address_, peer_address_, valid_version_,
        kNoParsedChlo, connection_id_generator_);
    BufferedPacketList delivered_packets = store_.DeliverPackets(connection_id);
