...
quic_trace::EncryptionLevel EncryptionLevelToProto(EncryptionLevel level)
{
    ...
    case NUM_ENCRYPTION_LEVELS:
        QUIC_BUG(EncryptionLevelToProto.Invalid) << "Invalid encryption level specified";
        return quic_trace::ENCRYPTION_UNKNOWN;
    }
    QUIC_BUG(EncryptionLevelToProto.Unknown) << "Unknown encryption level specified " << static_cast<int>(level);
    return quic_trace::ENCRYPTION_UNKNOWN;
}
...
{
    std::string binary_connection_id(connection->connection_id().data(), connection->connection_id().length());
...
void QuicTraceVisitor::OnPacketSent(QuicPacketNumber packet_number, QuicPacketLength packet_length, bool /*has_crypto_handshake*/,
    ...
    const QuicFrames& /*nonretransmittable_frames*/, QuicTime sent_time, uint32_t /*batch_id*/)
{
    ...
    for (const QuicFrame& frame : retransmittable_frames) {
        ...
        case ACK_FRAME:
            QUIC_BUG(quic_bug_12732_1) << "Frames of type are not retransmittable and are not supposed "
    ...
    if (connection_->sent_packet_manager().GetSendAlgorithm()->GetCongestionControlType() == kPCC) {
        PopulateTransportState(event->mutable_transport_state());
    }
}

void QuicTraceVisitor::PopulateFrameInfo(const QuicFrame& frame, quic_trace::Frame* frame_record)
{
    switch (frame.type) {
    case STREAM_FRAME: {
        frame_record->set_frame_type(quic_trace::STREAM);

        quic_trace::StreamFrameInfo* info = frame_record->mutable_stream_frame_info();
    ...
    case RST_STREAM_FRAME: {
        ...
        quic_trace::ResetStreamInfo* info = frame_record->mutable_reset_stream_info();
    ...
    case CONNECTION_CLOSE_FRAME: {
        ...
        info->set_close_type(static_cast<quic_trace::CloseType>(frame.connection_close_frame->close_type));
        info->set_transport_close_frame_type(frame.connection_close_frame->transport_close_frame_type);
        break;
    }
    ...
    case WINDOW_UPDATE_FRAME: {
        ...
        frame_record->set_frame_type(is_connection ? quic_trace::MAX_DATA : quic_trace::MAX_STREAM_DATA);

        quic_trace::FlowControlInfo* info = frame_record->mutable_flow_control_info();
    ...
    case BLOCKED_FRAME: {
        ...
        frame_record->set_frame_type(is_connection ? quic_trace::BLOCKED : quic_trace::STREAM_BLOCKED);
...
void QuicTraceVisitor::OnIncomingAck(QuicPacketNumber /*ack_packet_number*/, EncryptionLevel ack_decrypted_level, const QuicAckFrame& ack_frame,
...
void QuicTraceVisitor::OnPacketLoss(
...
void QuicTraceVisitor::OnWindowUpdateFrame(const QuicWindowUpdateFrame& frame, const QuicTime& receive_time)
{
    quic_trace::Event* event = trace_.add_events();
...
void QuicTraceVisitor::OnSuccessfulVersionNegotiation(const ParsedQuicVersion& version)
{
    uint32_t tag = quiche::QuicheEndian::HostToNet32(CreateQuicVersionLabel(version));
...
{
    quic_trace::Event* event = trace_.add_events();
    event->set_time_us(ConvertTimestampToRecordedFormat(connection_->clock()->ApproximateNow()));
    event->set_event_type(quic_trace::APPLICATION_LIMITED);
}

void QuicTraceVisitor::OnAdjustNetworkParameters(QuicBandwidth bandwidth, QuicTime::Delta rtt, QuicByteCount /*old_cwnd*/, QuicByteCount /*new_cwnd*/)
{
    quic_trace::Event* event = trace_.add_events();
    event->set_time_us(ConvertTimestampToRecordedFormat(connection_->clock()->ApproximateNow()));
    event->set_event_type(quic_trace::EXTERNAL_PARAMETERS);

    quic_trace::ExternalNetworkParameters* parameters = event->mutable_external_network_parameters();
    if (!bandwidth.IsZero()) {
...
uint64_t QuicTraceVisitor::ConvertTimestampToRecordedFormat(QuicTime timestamp)
{
    if (timestamp < start_time_) {
        QUIC_BUG(quic_bug_10284_4) << "Timestamp went back in time while recording a trace";
        return 0;
    }
...
void QuicTraceVisitor::PopulateTransportState(quic_trace::TransportState* state)
{
    ...
    state->set_cwnd_bytes(connection_->sent_packet_manager().GetCongestionWindowInBytes());
    QuicByteCount in_flight = connection_->sent_packet_manager().GetBytesInFlight();
    state->set_in_flight_bytes(in_flight);
    state->set_pacing_rate_bps(connection_->sent_packet_manager().GetSendAlgorithm()->PacingRate(in_flight).ToBitsPerSecond());

    if (connection_->sent_packet_manager().GetSendAlgorithm()->GetCongestionControlType() == kPCC) {
        state->set_congestion_control_state(connection_->sent_packet_manager().GetSendAlgorithm()->GetDebugState());
    }
}
