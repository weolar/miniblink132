...
public:
    ...
    virtual void RegisterStream(QuicStreamId stream_id, bool is_static_stream, const QuicStreamPriority& priority) = 0;
    ...
    // streams.
    virtual void UpdateStreamPriority(QuicStreamId stream_id, const QuicStreamPriority& new_priority) = 0;
...
class QUICHE_EXPORT QuicWriteBlockedList : public QuicWriteBlockedListInterface {
public:
    ...
    {
        return NumBlockedSpecialStreams() + priority_write_scheduler_.NumReadyStreams();
    }
    ...
    // Register a stream with given priority.
    // `priority` is ignored for static streams.
    void RegisterStream(QuicStreamId stream_id, bool is_static_stream, const QuicStreamPriority& priority) override;
...
private:
    ...
    http2::PriorityWriteScheduler<QuicStreamId, HttpStreamPriority, HttpStreamPriorityToInt, IntToHttpStreamPriority> priority_write_scheduler_;
    ...
    public:
        ...
        StreamsVector::const_iterator begin() const
        {
            return streams_.cbegin();
        }

        StreamsVector::const_iterator end() const
        {
            return streams_.cend();
        }

        size_t num_blocked() const
        {
            return num_blocked_;
        }
