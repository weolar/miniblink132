...
class QuicChaosProtectorTest : public QuicTestWithParam<ParsedQuicVersion>, public QuicStreamFrameDataProducer {
public:
    QuicChaosProtectorTest()
        : version_(GetParam())
        , framer_({ version_ }, QuicTime::Zero(), Perspective::IS_CLIENT, kQuicDefaultConnectionIdLength)
    ...
    {
        chaos_protector_ = std::make_unique<QuicChaosProtector>(crypto_frame_, num_padding_bytes_, packet_size_, SetupHeaderAndFramers(), &random_);
    }
...
protected:
    QuicFramer* SetupHeaderAndFramers()
    {
        ...
        validation_framer_.framer()->SetInitialObfuscators(header_.destination_connection_id);
        // Setup framer.
    ...
    {
        std::optional<size_t> length = chaos_protector_->BuildDataPacket(header_, packet_buffer_.get());
        ASSERT_TRUE(length.has_value());
        ASSERT_GT(length.value(), 0u);
        size_t encrypted_length = framer_.EncryptInPlace(
            level_, header_.packet_number, GetStartOfEncryptedData(framer_.transport_version(), header_), length.value(), packet_size_, packet_buffer_.get());
        ASSERT_GT(encrypted_length, 0u);
        ASSERT_TRUE(validation_framer_.ProcessPacket(QuicEncryptedPacket(absl::string_view(packet_buffer_.get(), encrypted_length))));
    }
...
INSTANTIATE_TEST_SUITE_P(QuicChaosProtectorTests, QuicChaosProtectorTest, ::testing::ValuesIn(TestVersions()), ::testing::PrintToStringParamName());
...
TEST_P(QuicChaosProtectorTest, RandomnessZero)
{
    ...
    EXPECT_EQ(validation_framer_.crypto_frames()[0]->data_length, crypto_data_length_);
...
TEST_P(QuicChaosProtectorTest, ZeroRemainingBytesAfterSplit)
{
    ...
    num_padding_bytes_ = QuicFramer::GetMinCryptoFrameSize(crypto_frame_.offset + new_length, new_length);
    ...
    EXPECT_EQ(validation_framer_.crypto_frames()[1]->data_length, crypto_data_length_ - 4);
    ASSERT_EQ(validation_framer_.ping_frames().size(), 0u);
}
