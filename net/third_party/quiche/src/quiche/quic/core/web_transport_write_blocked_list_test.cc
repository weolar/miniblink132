...
protected:
    ...
    void RegisterHttpStream(QuicStreamId id, int urgency = HttpStreamPriority::kDefaultUrgency)
    {
        HttpStreamPriority priority;
        priority.urgency = urgency;
        list_.RegisterStream(id, /*is_static_stream=*/false, QuicStreamPriority(priority));
    }
    void RegisterWebTransportDataStream(QuicStreamId id, WebTransportStreamPriority priority)
    {
        list_.RegisterStream(id, /*is_static_stream=*/false, QuicStreamPriority(priority));
    }
    ...
    std::vector<QuicStreamId> PopAll()
    {
        ...
        while (list_.NumBlockedStreams() > 0) {
            EXPECT_TRUE(list_.HasWriteBlockedDataStreams() || list_.HasWriteBlockedSpecialStream());
...
TEST_F(WebTransportWriteBlockedListTest, UpdatePriorityHttp)
{
    ...
    list_.UpdateStreamPriority(2, QuicStreamPriority(HttpStreamPriority { HttpStreamPriority::kMaximumUrgency, false }));
...
TEST_F(WebTransportWriteBlockedListTest, UpdatePriorityWebTransport)
{
    ...
    list_.UpdateStreamPriority(2, QuicStreamPriority(WebTransportStreamPriority { 0, 0, 1 }));
...
TEST_F(WebTransportWriteBlockedListTest, RandomizedTest)
{
    ...
    constexpr std::array<QuicStreamId, 10> order = { 3, 9, 8, 7, 10, 1, 4, 2, 5, 6 };
    ...
    for (int i = 0; i < 1000; ++i) {
        ...
        for (int j = pushed_streams.size() - 1; j > 0; --j) {
            std::swap(pushed_streams[j], pushed_streams[random.RandUint64() % (j + 1)]);
        }
        ...
        absl::c_copy_if(order, std::back_inserter(expected_streams), [&](QuicStreamId id) { return absl::c_find(pushed_streams, id) != pushed_streams.end(); });
