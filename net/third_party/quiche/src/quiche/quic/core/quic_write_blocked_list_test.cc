...
protected:
    ...
    {
        return write_blocked_list_->PopFront();
    }

    void RegisterStream(QuicStreamId stream_id, bool is_static_stream, const HttpStreamPriority& priority)
    {
        write_blocked_list_->RegisterStream(stream_id, is_static_stream, QuicStreamPriority(priority));
    }
    ...
    void UpdateStreamPriority(QuicStreamId stream_id, const HttpStreamPriority& new_priority)
    {
        write_blocked_list_->UpdateStreamPriority(stream_id, QuicStreamPriority(new_priority));
    }
...
class QuicWriteBlockedListParameterizedTest : public QuicWriteBlockedListTest, public ::testing::WithParamInterface<std::tuple<bool, bool>> {
protected:
    ...
    {
        SetQuicReloadableFlag(quic_priority_respect_incremental, priority_respect_incremental_);
...
INSTANTIATE_TEST_SUITE_P(BatchWrite, QuicWriteBlockedListParameterizedTest, ::testing::Combine(::testing::Bool(), ::testing::Bool()),
    [](const testing::TestParamInfo<QuicWriteBlockedListParameterizedTest::ParamType>& info) {
        return absl::StrCat(std::get<0>(info.param) ? "RespectIncrementalTrue" : "RespectIncrementalFalse",
            std::get<1>(info.param) ? "DisableBatchWriteTrue" : "DisableBatchWriteFalse");
    });
...
TEST_P(QuicWriteBlockedListParameterizedTest, NonIncrementalStreamsKeepWriting)
{
...
TEST_P(QuicWriteBlockedListParameterizedTest, IncrementalAndNonIncrementalStreams)
{
    if (!priority_respect_incremental_) {
        return;
    }
...
TEST_F(QuicWriteBlockedListTest, UpdateStaticStreamPriority)
{
    ...
    EXPECT_QUICHE_DEBUG_DEATH(UpdateStreamPriority(2, { kV3HighestPriority, kNotIncremental }), "IsRegistered");
}
