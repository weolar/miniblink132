...
class QUICHE_EXPORT PendingStream : public QuicStreamSequencer::StreamInterface {
public:
    ...
    void OnUnrecoverableError(QuicErrorCode error, const std::string& details) override;
    void OnUnrecoverableError(QuicErrorCode error, QuicIetfTransportErrorCodes ietf_error, const std::string& details) override;
...
public:
    ...
    QuicStream(QuicStreamId id, QuicSession* session, bool is_static, StreamType type);
    ...
    bool write_side_closed() const
    {
        return write_side_closed_;
    }
    bool read_side_closed() const
    {
        return read_side_closed_;
    }
    ...
    bool rst_received() const
    {
        return rst_received_;
    }
    bool rst_sent() const
    {
        return rst_sent_;
    ...
    bool fin_outstanding() const
    {
        return fin_outstanding_;
    }
    bool fin_lost() const
    {
        return fin_lost_;
    ...
    size_t busy_counter() const
    {
        return busy_counter_;
    }
    void set_busy_counter(size_t busy_counter)
    {
    ...
    // not valid.
    bool MaybeConfigSendWindowOffset(QuicStreamOffset new_offset, bool was_zero_rtt_rejected);
    ...
    // arrives.
    bool HasReceivedFinalOffset() const
    {
    ...
    // true.
    void WriteOrBufferData(absl::string_view data, bool fin, quiche::QuicheReferenceCountedPointer<QuicAckListenerInterface> ack_listener);
    ...
    bool WriteStreamData(QuicStreamOffset offset, QuicByteCount data_length, QuicDataWriter* writer);
    ...
    virtual bool OnStreamFrameAcked(QuicStreamOffset offset, QuicByteCount data_length, bool fin_acked, QuicTime::Delta ack_delay_time,
    ...
    virtual void OnStreamFrameRetransmitted(QuicStreamOffset offset, QuicByteCount data_length, bool fin_retransmitted);

    // Called when data [offset, offset + data_length) is considered as lost.
    // |fin_lost| indicates whether the fin is considered as lost.
    virtual void OnStreamFrameLost(QuicStreamOffset offset, QuicByteCount data_length, bool fin_lost);
    ...
    virtual bool RetransmitStreamData(QuicStreamOffset offset, QuicByteCount data_length, bool fin, TransmissionType type);
    ...
    QuicConsumedData WriteMemSlices(absl::Span<quiche::QuicheMemSlice> span, bool fin, bool buffer_uncondtionally = false);
    ...
    // otherwise.
    bool IsStreamFrameOutstanding(QuicStreamOffset offset, QuicByteCount data_length, bool fin) const;
    ...
    bool was_draining() const
    {
        return was_draining_;
    }
    ...
    bool fin_buffered() const
    {
        return fin_buffered_;
    }
    ...
    const QuicTime::Delta pending_duration() const
    {
        return pending_duration_;
    }
...
protected:
    ...
    virtual void OnDataBuffered(
        QuicStreamOffset /*offset*/, QuicByteCount /*data_length*/, const quiche::QuicheReferenceCountedPointer<QuicAckListenerInterface>&
        /*ack_listener*/)
    {
    }
    ...
    void set_rst_received(bool rst_received)
    {
        rst_received_ = rst_received;
    }
    void set_stream_error(QuicResetStreamError error)
    {
        stream_error_ = error;
    }

    StreamDelegateInterface* stream_delegate()
    {
        return stream_delegate_;
    }
    ...
    const QuicStreamSendBuffer& send_buffer() const
    {
        return send_buffer_;
    }
...
private:
    ...
    QuicStream(QuicStreamId id, QuicSession* session, QuicStreamSequencer sequencer, bool is_static, StreamType type, uint64_t stream_bytes_read,
