...
struct TestParams {
    ...
    TestParams()
        : TestParams(kQuicDefaultConnectionIdLength)
    {
    }
...
class DeterministicConnectionIdGeneratorTest : public QuicTestWithParam<TestParams> {
public:
...
INSTANTIATE_TEST_SUITE_P(DeterministicConnectionIdGeneratorTests, DeterministicConnectionIdGeneratorTest, ::testing::ValuesIn(GetTestParams()));

TEST_P(DeterministicConnectionIdGeneratorTest, NextConnectionIdIsDeterministic)
{
    ...
    EXPECT_EQ(*generator_.GenerateNextConnectionId(connection_id64a), *generator_.GenerateNextConnectionId(connection_id64b));
    ...
    EXPECT_EQ(*generator_.GenerateNextConnectionId(connection_id72a), *generator_.GenerateNextConnectionId(connection_id72b));
}

TEST_P(DeterministicConnectionIdGeneratorTest, NextConnectionIdLengthIsCorrect)
{
    ...
    for (uint8_t i = 0; i < sizeof(connection_id_bytes) - 1; ++i) {
        ...
        std::optional<QuicConnectionId> replacement_connection_id = generator_.GenerateNextConnectionId(connection_id);
...
TEST_P(DeterministicConnectionIdGeneratorTest, OnlyReplaceConnectionIdWithWrongLength)
{
    const char connection_id_input[]
    ...
    for (int i = 0; i < kQuicMaxConnectionIdWithLengthPrefixLength; i++) {
        ...
        std::optional<QuicConnectionId> output = generator_.MaybeReplaceConnectionId(input, version_);
