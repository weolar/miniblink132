...
template <typename T> class QUICHE_NO_EXPORT QuicIntervalSet {
public:
...
public:
    ...
    void AddOptimizedForAppend(const value_type& interval)
    {
        ...
        if (interval.min() < last_interval->min() || interval.min() > last_interval->max()) {
            Add(interval);
            return;
        }
    ...
    {
        return a.Size() == b.Size() && std::equal(a.begin(), a.end(), b.begin(), NonemptyIntervalEq());
    }
...
private:
    ...
    static bool FindNextIntersectingPairImpl(X* x, const QuicIntervalSet& y, const_iterator* mine, const_iterator* theirs, Func on_hole);

    // The variant of the above method that doesn't mutate this QuicIntervalSet.
    bool FindNextIntersectingPair(const QuicIntervalSet& other, const_iterator* mine, const_iterator* theirs) const
    {
        return FindNextIntersectingPairImpl(this, other, mine, theirs, [](const QuicIntervalSet*, const_iterator, const_iterator end) { return end; });
    }

    // The variant of the above method that mutates this QuicIntervalSet by
    // erasing holes.
    bool FindNextIntersectingPairAndEraseHoles(const QuicIntervalSet& other, const_iterator* mine, const_iterator* theirs)
    {
        return FindNextIntersectingPairImpl(
            this, other, mine, theirs, [](QuicIntervalSet* x, const_iterator from, const_iterator to) { return x->intervals_.erase(from, to); });
    }
...
template <typename T> auto operator<<(std::ostream& out, const QuicIntervalSet<T>& seq) -> decltype(out << *seq.begin())
{
    out << "{";
...
template <typename T> typename QuicIntervalSet<T>::value_type QuicIntervalSet<T>::SpanningInterval() const
{
    value_type result;
    if (!intervals_.empty()) {
...
template <typename T> void QuicIntervalSet<T>::Add(const value_type& interval)
{
    if (interval.Empty())
        return;
...
template <typename T> bool QuicIntervalSet<T>::Contains(const T& value) const
{
...
template <typename T> bool QuicIntervalSet<T>::Contains(const value_type& interval) const
{
    // Find the first interval with min() > value, then move back one step.
...
template <typename T> bool QuicIntervalSet<T>::Contains(const QuicIntervalSet<T>& other) const
{
...
template <typename T> typename QuicIntervalSet<T>::const_iterator QuicIntervalSet<T>::Find(const T& value) const
{
    const_iterator it = intervals_.upper_bound(value);
    if (it == intervals_.begin())
        return intervals_.end();
...
template <typename T> typename QuicIntervalSet<T>::const_iterator QuicIntervalSet<T>::Find(const value_type& probe) const
{
...
template <typename T> typename QuicIntervalSet<T>::const_iterator QuicIntervalSet<T>::LowerBound(const T& value) const
{
...
template <typename T> typename QuicIntervalSet<T>::const_iterator QuicIntervalSet<T>::UpperBound(const T& value) const
{
    return intervals_.upper_bound(value);
}

template <typename T> bool QuicIntervalSet<T>::IsDisjoint(const value_type& interval) const
{
    if (interval.Empty())
        return true;
...
template <typename T> void QuicIntervalSet<T>::Union(const QuicIntervalSet& other)
{
...
template <typename T> typename QuicIntervalSet<T>::const_iterator QuicIntervalSet<T>::FindIntersectionCandidate(const QuicIntervalSet& other) const
{
    return FindIntersectionCandidate(*other.intervals_.begin());
}

template <typename T> typename QuicIntervalSet<T>::const_iterator QuicIntervalSet<T>::FindIntersectionCandidate(const value_type& interval) const
{
...
template <typename T>
template <typename X, typename Func>
bool QuicIntervalSet<T>::FindNextIntersectingPairImpl(X* x, const QuicIntervalSet& y, const_iterator* mine, const_iterator* theirs, Func on_hole)
{
...
template <typename T> void QuicIntervalSet<T>::Intersection(const QuicIntervalSet& other)
{
...
template <typename T> bool QuicIntervalSet<T>::Intersects(const QuicIntervalSet& other) const
{
...
template <typename T> void QuicIntervalSet<T>::Difference(const value_type& interval)
{
...
template <typename T> void QuicIntervalSet<T>::Difference(const T& min, const T& max)
{
    Difference(value_type(min, max));
}

template <typename T> void QuicIntervalSet<T>::Difference(const QuicIntervalSet& other)
{
...
template <typename T> void QuicIntervalSet<T>::Complement(const T& min, const T& max)
{
...
template <typename T> std::string QuicIntervalSet<T>::ToString() const
{
...
template <typename T> bool QuicIntervalSet<T>::Valid() const
{
...
template <typename T> bool QuicIntervalSet<T>::IntervalLess::operator()(const value_type& a, const value_type& b) const
{
...
template <typename T> bool QuicIntervalSet<T>::IntervalLess::operator()(const value_type& a, const T& point) const
{
    // Compare an interval to a point.
    return a.min() < point;
}

template <typename T> bool QuicIntervalSet<T>::IntervalLess::operator()(const value_type& a, T&& point) const
{
    // Compare an interval to a point
    return a.min() < point;
}
...
template <typename T> bool QuicIntervalSet<T>::IntervalLess::operator()(const T& point, const value_type& a) const
{
    // Compare an interval to a point.
    return point < a.min();
}

template <typename T> bool QuicIntervalSet<T>::IntervalLess::operator()(T&& point, const value_type& a) const
{
    // Compare an interval to a point.
    return point < a.min();
}
