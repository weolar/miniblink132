...
template <typename ForwardIterator> void STLDeleteContainerPointers(ForwardIterator begin, ForwardIterator end)
{
    while (begin != end) {
        auto temp = begin;
        ++begin;
...
public:
    ...
    bool operator<(const ConstructorListener&)
    {
        return false;
    }
    bool operator>(const ConstructorListener&)
...
TEST(QuicIntervalConstructorTest, ImplicitConversion)
{
    struct WrappedInt {
        ...
        bool operator<(const WrappedInt& other)
        {
    ...
    static_assert(std::is_constructible<QuicInterval<WrappedInt>, int, int>::value, "");
...
protected:
    ...
    void TestIntersect(const QuicInterval<int64_t>& i1, const QuicInterval<int64_t>& i2, bool changes_i1, bool changes_i2, const QuicInterval<int64_t>& result)
    {
...
TEST_F(QuicIntervalTest, Length)
{
    ...
    const QuicInterval<QuicTime> empty4(QuicTime::Zero() + QuicTime::Delta::FromSeconds(1), QuicTime::Zero());
    const QuicInterval<int> d1(1, 2);
    const QuicInterval<int> d2(0, 50);
    const QuicInterval<QuicTime> d3(QuicTime::Zero(), QuicTime::Zero() + QuicTime::Delta::FromSeconds(1));
    const QuicInterval<QuicTime> d4(QuicTime::Zero() + QuicTime::Delta::FromSeconds(3600), QuicTime::Zero() + QuicTime::Delta::FromSeconds(5400));
