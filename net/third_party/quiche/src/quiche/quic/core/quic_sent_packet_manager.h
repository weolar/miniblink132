...
public:
    ...
    public:
        ...
        virtual void OnSpuriousPacketRetransmission(TransmissionType /*transmission_type*/, QuicByteCount /*byte_size*/)
        {
        }

        virtual void OnIncomingAck(QuicPacketNumber /*ack_packet_number*/, EncryptionLevel /*ack_decrypted_level*/, const QuicAckFrame& /*ack_frame*/,
            QuicTime /*ack_receive_time*/, QuicPacketNumber /*largest_observed*/, bool /*rtt_updated*/, QuicPacketNumber /*least_unacked_sent_packet*/)
        {
        }

        virtual void OnPacketLoss(
            QuicPacketNumber /*lost_packet_number*/, EncryptionLevel /*encryption_level*/, TransmissionType /*transmission_type*/, QuicTime /*detection_time*/)
        {
        }
        ...
        virtual void OnAdjustNetworkParameters(QuicBandwidth /*bandwidth*/, QuicTime::Delta /*rtt*/, QuicByteCount /*old_cwnd*/, QuicByteCount /*new_cwnd*/)
        {
        }
    ...
    void ResumeConnectionState(const CachedNetworkParameters& cached_network_params, bool max_bandwidth_resumption);
    ...
    // congestion.
    void SetApplicationDrivenPacingRate(QuicBandwidth application_driven_pacing_rate)
    {
        pacing_sender_.set_application_driven_pacing_rate(application_driven_pacing_rate);
    }
    ...
    bool OnPacketSent(SerializedPacket* mutable_packet, QuicTime sent_time, TransmissionType transmission_type, HasRetransmittableData has_retransmittable_data,
    ...
    const QuicTransmissionInfo& AddDispatcherSentPacket(const DispatcherSentPacket& packet);
    ...
    const QuicTime::Delta GetNetworkBlackholeDelay(int8_t num_rtos_for_blackhole_detection) const;

    // Returns the delay before reducing max packet size. This delay is guranteed
    // to be smaller than the network blackhole delay.
    QuicTime::Delta GetMtuReductionDelay(int8_t num_rtos_for_blackhole_detection) const;
    ...
    // sent.
    QuicPacketCount EstimateMaxPacketsInFlight(QuicByteCount max_packet_length) const
    {
    ...
    std::unique_ptr<SendAlgorithmInterface> OnConnectionMigration(bool reset_send_algorithm);

    // Called when an ack frame is initially parsed.
    void OnAckFrameStart(QuicPacketNumber largest_acked, QuicTime::Delta ack_delay_time, QuicTime ack_receive_time);
    ...
    QuicPacketNumber GetLargestAckedPacket(EncryptionLevel decrypted_packet_level) const;
    ...
    QuicPacketNumber GetLeastPacketAwaitedByPeer(EncryptionLevel encryption_level) const;

    QuicPacketNumber GetLargestPacketPeerKnowsIsAcked(EncryptionLevel decrypted_packet_level) const;
    ...
    size_t GetConsecutivePtoCount() const
    {
        return consecutive_pto_count_;
    }
    ...
    bool EnableECT0()
    {
        return send_algorithm_->EnableECT0();
    }
    bool EnableECT1()
    {
        return send_algorithm_->EnableECT1();
    }
    ...
    QuicTime::Delta peer_max_ack_delay() const
    {
    ...
    void set_peer_max_ack_delay(QuicTime::Delta peer_max_ack_delay)
    {
        ...
        QUICHE_DCHECK_LE(peer_max_ack_delay, (QuicTime::Delta::FromMilliseconds(kMinRetransmissionTimeMs) * 0.5));
    ...
    {
        return handshake_mode_disabled_;
    }

    bool zero_rtt_packet_acked() const
    {
        return zero_rtt_packet_acked_;
    }

    bool one_rtt_packet_acked() const
    {
        return one_rtt_packet_acked_;
    }

    void OnUserAgentIdKnown()
    {
    ...
    QuicTime GetEarliestPacketSentTimeForPto(PacketNumberSpace* packet_number_space) const;
...
private:
    ...
    bool MaybeUpdateRTT(QuicPacketNumber largest_acked, QuicTime::Delta ack_delay_time, QuicTime ack_receive_time);
    ...
    // triggered.
    void MaybeInvokeCongestionEvent(
    ...
    void PostProcessNewlyAckedPackets(QuicPacketNumber ack_packet_number, EncryptionLevel ack_decrypted_level, const QuicAckFrame& ack_frame,
    ...
    QuicTime::Delta GetNConsecutiveRetransmissionTimeoutDelay(int num_timeouts) const;
    ...
    void OnAckFrequencyFrameSent(const QuicAckFrequencyFrame& ack_frequency_frame);

    // Called when an AckFrequencyFrame is acked.
    void OnAckFrequencyFrameAcked(const QuicAckFrequencyFrame& ack_frequency_frame);
    ...
    void RecordEcnMarkingSent(QuicEcnCodepoint ecn_codepoint, EncryptionLevel level);
    ...
    QuicPacketNumber largest_packets_peer_knows_is_acked_[NUM_PACKET_NUMBER_SPACES];
    ...
    quiche::QuicheCircularDeque<std::pair<QuicTime::Delta, /*sequence_number=*/uint64_t>> in_use_sent_ack_delays_;
