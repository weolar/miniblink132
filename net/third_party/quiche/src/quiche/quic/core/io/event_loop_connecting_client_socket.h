...
// QuicEventLoop.
class EventLoopConnectingClientSocket : public ConnectingClientSocket, public QuicSocketEventListener {
public:
    ...
    EventLoopConnectingClientSocket(socket_api::SocketProtocol protocol, const quic::QuicSocketAddress& peer_address, QuicByteCount receive_buffer_size,
        QuicByteCount send_buffer_size, QuicEventLoop* event_loop, quiche::QuicheBufferAllocator* buffer_allocator, AsyncVisitor* async_visitor);
    ...
    absl::StatusOr<quiche::QuicheMemSlice> ReceiveBlocking(QuicByteCount max_size) override;
    ...
    void OnSocketEvent(QuicEventLoop* event_loop, SocketFd fd, QuicSocketEventMask events) override;
...
private:
    ...
    absl::variant<absl::monostate, std::string, quiche::QuicheMemSlice> send_data_;
