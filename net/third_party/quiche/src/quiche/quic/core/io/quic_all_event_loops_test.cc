...
constexpr QuicSocketEventMask kAllEvents = kSocketEventReadable | kSocketEventWritable | kSocketEventError;

class MockQuicSocketEventListener : public QuicSocketEventListener {
public:
    MOCK_METHOD(void, OnSocketEvent, (QuicEventLoop* /*event_loop*/, SocketFd /*fd*/, QuicSocketEventMask /*events*/), (override));
};
...
{
    QUICHE_CHECK(::fcntl(fd, F_SETFL, ::fcntl(fd, F_GETFL) | O_NONBLOCK) == 0) << "Failed to mark FD non-blocking, errno: " << errno;
}

class QuicEventLoopFactoryTest : public QuicTestWithParam<QuicEventLoopFactory*> {
public:
    void SetUp() override
    {
    ...
    template <typename Condition> void RunEventLoopUntil(Condition condition, QuicTime::Delta timeout)
    {
...
INSTANTIATE_TEST_SUITE_P(QuicEventLoopFactoryTests, QuicEventLoopFactoryTest, ::testing::ValuesIn(GetAllSupportedEventLoops()), GetTestParamName);
...
TEST_P(QuicEventLoopFactoryTest, RearmWriter)
{
    ...
    if (loop_->SupportsEdgeTriggered()) {
        EXPECT_CALL(listener, OnSocketEvent(_, write_fd_, kSocketEventWritable)).Times(1);
        loop_->RunEventLoopOnce(QuicTime::Delta::FromMilliseconds(1));
        loop_->RunEventLoopOnce(QuicTime::Delta::FromMilliseconds(1));
    } else {
        EXPECT_CALL(listener, OnSocketEvent(_, write_fd_, kSocketEventWritable)).Times(2);
        loop_->RunEventLoopOnce(QuicTime::Delta::FromMilliseconds(1));
...
TEST_P(QuicEventLoopFactoryTest, ArtificialNotifyFromCallback)
{
    ...
    EXPECT_CALL(listener, OnSocketEvent(_, read_fd_, kSocketEventReadable)).Times(loop_->SupportsEdgeTriggered() ? (kTimes + 1) : kTimes).WillRepeatedly([&]() {
        char buf[5];
        int read_result = read(read_fd_, buf, sizeof(buf));
        if (read_result > 0) {
            ASSERT_EQ(read_result, 5);
            if (loop_->SupportsEdgeTriggered()) {
                EXPECT_TRUE(loop_->ArtificiallyNotifyEvent(read_fd_, kSocketEventReadable));
            } else {
...
TEST_P(QuicEventLoopFactoryTest, ArtificialNotifyOncePerIteration)
{
    ...
    EXPECT_CALL(listener, OnSocketEvent(_, read_fd_, kSocketEventReadable)).WillRepeatedly([&]() {
        read_event_count_++;
        EXPECT_TRUE(loop_->ArtificiallyNotifyEvent(read_fd_, kSocketEventReadable));
    });
...
TEST_P(QuicEventLoopFactoryTest, UnregisterInsideEventHandler)
{
    ...
    int total_called = 0;
    EXPECT_CALL(listener, OnSocketEvent(_, read_fd_, kSocketEventReadable)).Times(AtMost(1)).WillOnce([&]() {
        ++total_called;
        ASSERT_TRUE(loop_->UnregisterSocket(write_fd_));
    });
    EXPECT_CALL(listener, OnSocketEvent(_, write_fd_, kSocketEventWritable)).Times(AtMost(1)).WillOnce([&]() {
        ++total_called;
...
TEST_P(QuicEventLoopFactoryTest, UnregisterSelfInsideEventHandler)
{
    ...
    EXPECT_CALL(listener, OnSocketEvent(_, write_fd_, kSocketEventWritable)).WillOnce([&]() { ASSERT_TRUE(loop_->UnregisterSocket(write_fd_)); });
    loop_->RunEventLoopOnce(QuicTime::Delta::FromMilliseconds(1));
}
...
TEST_P(QuicEventLoopFactoryTest, ReadWriteSocket)
{
    ...
    EXPECT_CALL(listener, OnSocketEvent(_, sockets[0], HasFlagSet(kSocketEventWritable)));
    loop_->RunEventLoopOnce(QuicTime::Delta::FromMilliseconds(4));
...
TEST_P(QuicEventLoopFactoryTest, AlarmInFuture)
{
    ...
    RunEventLoopUntil([&]() { return alarm_called; }, QuicTime::Delta::FromMilliseconds(100));
}
...
TEST_P(QuicEventLoopFactoryTest, ScheduleAlarmInPastFromInsideAlarm)
{
    ...
    alarm1_ptr->Set(clock_.Now() - kAlarmTimeout);
    EXPECT_CALL(*delegate1, OnAlarm()).WillOnce([&, alarm2_unowned = alarm2_ptr.get()]() { alarm2_unowned->Set(clock_.Now() - 2 * kAlarmTimeout); });
    bool fired = false;
    EXPECT_CALL(*delegate2, OnAlarm()).WillOnce([&]() { fired = true; });

    RunEventLoopUntil([&]() { return fired; }, QuicTime::Delta::FromMilliseconds(100));
}
