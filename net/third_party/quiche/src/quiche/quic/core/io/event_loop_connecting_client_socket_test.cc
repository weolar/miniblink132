...
public:
    using SocketBehavior = quiche::MultiUseCallback<void(SocketFd connected_socket, socket_api::SocketProtocol protocol)>;

    TestServerSocketRunner(SocketFd server_socket_descriptor, SocketBehavior behavior)
        : QuicheThread("TestServerSocketRunner")
        , server_socket_descriptor_(server_socket_descriptor)
        , behavior_(std::move(behavior))
    {
    }
    ~TestServerSocketRunner() override
    {
        WaitForCompletion();
    }
    ...
    {
        completion_notification_.WaitForNotification();
    }
...
protected:
    ...
    const SocketBehavior& behavior() const
    {
        return behavior_;
    }
...
public:
    ...
    TestTcpServerSocketRunner(SocketFd server_socket_descriptor, SocketBehavior behavior)
        : TestServerSocketRunner(server_socket_descriptor, std::move(behavior))
    {
        Start();
    }

    ~TestTcpServerSocketRunner() override
    {
        Join();
    }
...
private:
    void AcceptSocket()
    {
        absl::StatusOr<socket_api::AcceptResult> connection_socket = socket_api::Accept(server_socket_descriptor(), /*blocking=*/true);
...
public:
    ...
    TestUdpServerSocketRunner(SocketFd server_socket_descriptor, SocketBehavior behavior, QuicSocketAddress client_socket_address)
        : TestServerSocketRunner(server_socket_descriptor, std::move(behavior))
    ...
    ~TestUdpServerSocketRunner() override
    {
        Join();
    }
...
private:
    void ConnectSocket()
    {
        QUICHE_CHECK(socket_api::Connect(server_socket_descriptor(), client_socket_address_).ok());
    }
...
class EventLoopConnectingClientSocketTest : public quiche::test::QuicheTestWithParam<std::tuple<socket_api::SocketProtocol, QuicEventLoopFactory*>>,
...
public:
    void SetUp() override
    {
        ...
        socket_factory_ = std::make_unique<EventLoopSocketFactory>(event_loop_.get(), quiche::SimpleBufferAllocator::Get());
...
protected:
    std::unique_ptr<ConnectingClientSocket> CreateSocket(const quic::QuicSocketAddress& peer_address, ConnectingClientSocket::AsyncVisitor* async_visitor)
    {
        switch (protocol_) {
        case socket_api::SocketProtocol::kUdp:
            return socket_factory_->CreateConnectingUdpClientSocket(peer_address, /*receive_buffer_size=*/0, /*send_buffer_size=*/0, async_visitor);
        case socket_api::SocketProtocol::kTcp:
            return socket_factory_->CreateTcpClientSocket(peer_address, /*receive_buffer_size=*/0, /*send_buffer_size=*/0, async_visitor);
        default:
            // Unexpected protocol.
            QUICHE_NOTREACHED();
            return nullptr;
    ...
    std::unique_ptr<ConnectingClientSocket> CreateSocketToEncourageDelayedSend(
        const quic::QuicSocketAddress& peer_address, ConnectingClientSocket::AsyncVisitor* async_visitor)
    {
        switch (protocol_) {
        case socket_api::SocketProtocol::kUdp:
        ...
        case socket_api::SocketProtocol::kTcp:
            ...
            // delayed.
            return socket_factory_->CreateTcpClientSocket(peer_address, /*receive_buffer_size=*/0, /*send_buffer_size=*/4, async_visitor);
        default:
            // Unexpected protocol.
            QUICHE_NOTREACHED();
            return nullptr;
    ...
    {
        absl::StatusOr<SocketFd> socket = socket_api::CreateSocket(quiche::TestLoopback().address_family(), protocol_,
            /*blocking=*/true);
        ...
        if (protocol_ == socket_api::SocketProtocol::kTcp) {
            ...
            absl::Status result = socket_api::SetReceiveBufferSize(socket.value(), kReceiveBufferSize);
        ...
        absl::StatusOr<QuicSocketAddress> socket_address = socket_api::GetSocketAddress(socket.value());
    ...
    std::unique_ptr<TestServerSocketRunner> CreateServerSocketRunner(TestServerSocketRunner::SocketBehavior behavior, ConnectingClientSocket* client_socket)
    {
        std::unique_ptr<TestServerSocketRunner> runner;
        switch (protocol_) {
        case socket_api::SocketProtocol::kUdp: {
            absl::StatusOr<QuicSocketAddress> client_socket_address = client_socket->GetLocalAddress();
            QUICHE_CHECK(client_socket_address.ok());
            runner = std::make_unique<TestUdpServerSocketRunner>(server_socket_descriptor_, std::move(behavior), std::move(client_socket_address).value());
            break;
        }
        case socket_api::SocketProtocol::kTcp:
            runner = std::make_unique<TestTcpServerSocketRunner>(server_socket_descriptor_, std::move(behavior));
            break;
        default:
            // Unexpected protocol.
            QUICHE_NOTREACHED();
...
std::string GetTestParamName(::testing::TestParamInfo<std::tuple<socket_api::SocketProtocol, QuicEventLoopFactory*>> info)
{
    auto [protocol, event_loop_factory] = info.param;

    return EscapeTestParamName(absl::StrCat(socket_api::GetProtocolName(protocol), "_", event_loop_factory->GetName()));
}

INSTANTIATE_TEST_SUITE_P(EventLoopConnectingClientSocketTests, EventLoopConnectingClientSocketTest,
    Combine(Values(socket_api::SocketProtocol::kUdp, socket_api::SocketProtocol::kTcp), ValuesIn(GetAllSupportedEventLoops())), &GetTestParamName);

TEST_P(EventLoopConnectingClientSocketTest, ConnectBlocking)
{
    std::unique_ptr<ConnectingClientSocket> socket = CreateSocket(server_socket_address_,
...
void SendDataOnSocket(absl::string_view data, SocketFd connected_socket, socket_api::SocketProtocol protocol)
{
    ...
    do {
        absl::StatusOr<absl::string_view> remainder = socket_api::Send(connected_socket, data);
        if (!remainder.ok()) {
            return;
        }
...
TEST_P(EventLoopConnectingClientSocketTest, ReceiveBlocking)
{
    ...
    std::unique_ptr<TestServerSocketRunner> runner = CreateServerSocketRunner(absl::bind_front(&SendDataOnSocket, expected), socket.get());
    ...
    } while (protocol_ == socket_api::SocketProtocol::kTcp && !data.value().empty());
...
TEST_P(EventLoopConnectingClientSocketTest, ReceiveAsync)
{
    ...
    std::string received(receive_result_.value().value().data(), receive_result_.value().value().length());
...
void ReceiveDataFromSocket(std::string* out_received, SocketFd connected_socket, socket_api::SocketProtocol protocol)
{
    out_received->clear();
    ...
    do {
        ...
        out_received->insert(out_received->end(), received.value().begin(), received.value().end());
    } while (protocol == socket_api::SocketProtocol::kTcp && !received.value().empty());
...
TEST_P(EventLoopConnectingClientSocketTest, SendBlocking)
{
    ...
    std::string sent;
    std::unique_ptr<TestServerSocketRunner> runner = CreateServerSocketRunner(absl::bind_front(&ReceiveDataFromSocket, &sent), socket.get());
...
{
    std::unique_ptr<ConnectingClientSocket> socket = CreateSocketToEncourageDelayedSend(server_socket_address_,
        /*async_visitor=*/this);
    ASSERT_TRUE(socket->ConnectBlocking().ok());
    ...
    case socket_api::SocketProtocol::kTcp:
        ...
        runner = CreateServerSocketRunner(absl::bind_front(&ReceiveDataFromSocket, &sent), socket.get());
