...
absl::StatusOr<AcceptResult> AcceptInternal(SocketFd fd)
{
    ...
    SocketFd connection_socket = SyscallAccept(fd, reinterpret_cast<struct sockaddr*>(&peer_addr), &peer_addr_len);
...
absl::Status Connect(SocketFd fd, const QuicSocketAddress& peer_address)
{
    ...
    int connect_result = SyscallConnect(fd, reinterpret_cast<sockaddr*>(&addr), addrlen);

    if (connect_result >= 0) {
        return absl::OkStatus();
    } else {
        // For ::connect(), only `EINPROGRESS` indicates unavailable.
        absl::Status status = LastSocketOperationError("::connect()", /*unavailable_error_numbers=*/
            { EINPROGRESS });
...
absl::Status GetSocketError(SocketFd fd)
{
    ...
    int sockopt_result = SyscallGetsockopt(fd, SOL_SOCKET, SO_ERROR, &socket_error, &len);
...
absl::StatusOr<QuicSocketAddress> GetSocketAddress(SocketFd fd)
{
    ...
    int result = SyscallGetsockname(fd, reinterpret_cast<sockaddr*>(&addr), &addr_len);
...
absl::StatusOr<AcceptResult> Accept(SocketFd fd, bool blocking)
{
    ...
    // do it now.
    absl::Status set_non_blocking_result = SetSocketBlocking(accept_result->fd, /*blocking=*/false);
    if (!set_non_blocking_result.ok()) {
        ...
        if (!Close(accept_result->fd).ok()) {
            QUICHE_LOG_FIRST_N(ERROR, 100) << "Failed to close socket " << accept_result->fd << " after error setting non-blocking on acceptance.";
        }
...
absl::StatusOr<absl::string_view> Send(SocketFd fd, absl::string_view buffer)
{
    ...
    PlatformSsizeT num_sent = SyscallSend(fd, buffer.data(), buffer.size(), /*flags=*/0);
...
absl::StatusOr<absl::string_view> SendTo(SocketFd fd, const QuicSocketAddress& peer_address, absl::string_view buffer)
{
    QUICHE_DCHECK_NE(fd, kInvalidSocketFd);
    QUICHE_DCHECK(peer_address.IsInitialized());
    QUICHE_DCHECK(!buffer.empty());
    ...
    PlatformSsizeT num_sent = SyscallSendTo(fd, buffer.data(), buffer.size(),
        /*flags=*/0, reinterpret_cast<sockaddr*>(&addr), addrlen);

    if (num_sent > 0 && static_cast<size_t>(num_sent) > buffer.size()) {
        QUICHE_LOG_FIRST_N(WARNING, 100) << "Sent more bytes (" << num_sent << ") to socket " << fd << " to address: " << peer_address.ToString()
                                         << " than buffer size (" << buffer.size() << ").";
        return absl::OutOfRangeError("::sendto(): Sent more bytes than buffer size.");
    } else if (num_sent >= 0) {
        return buffer.substr(num_sent);
    } else {
        absl::Status status = LastSocketOperationError("::sendto()");
        QUICHE_DVLOG(1) << "Failed to send to socket: " << fd << " to address: " << peer_address.ToString() << " with error: " << status;
        return status;
    }
}
