...
SocketFd CreateTestSocket(socket_api::SocketProtocol protocol, bool blocking = true)
{
    absl::StatusOr<SocketFd> socket = socket_api::CreateSocket(quiche::TestLoopback().address_family(), protocol, blocking);
...
SocketFd CreateTestRawSocket(bool blocking = true, IpAddressFamily address_family = IpAddressFamily::IP_UNSPEC)
{
    absl::StatusOr<SocketFd> socket;
    switch (address_family) {
    case IpAddressFamily::IP_V4:
        socket = socket_api::CreateSocket(quiche::TestLoopback4().address_family(), socket_api::SocketProtocol::kRawIp, blocking);
        break;
    case IpAddressFamily::IP_V6:
        socket = socket_api::CreateSocket(quiche::TestLoopback6().address_family(), socket_api::SocketProtocol::kRawIp, blocking);
        break;
    case IpAddressFamily::IP_UNSPEC:
        socket = socket_api::CreateSocket(quiche::TestLoopback().address_family(), socket_api::SocketProtocol::kRawIp, blocking);
        break;
    }
    ...
    if (socket.ok()) {
        ...
        QUICHE_CHECK(absl::IsPermissionDenied(socket.status()) || absl::IsNotFound(socket.status()));
        return kInvalidSocketFd;
    }
}
...
TEST(SocketTest, CreateAndCloseSocket)
{
    ...
    absl::StatusOr<SocketFd> created_socket = socket_api::CreateSocket(localhost_address.address_family(), socket_api::SocketProtocol::kUdp);
...
{
    QuicIpAddress localhost_address = quiche::TestLoopback();
    absl::StatusOr<SocketFd> created_socket = socket_api::CreateSocket(localhost_address.address_family(), socket_api::SocketProtocol::kRawIp);

    // Raw IP socket creation will typically fail if not run with relevant admin
    // privileges.
    if (!created_socket.ok()) {
        EXPECT_THAT(created_socket.status(), StatusIs(absl::StatusCode::kPermissionDenied));
        return;
    }
...
{
    SocketFd socket = CreateTestRawSocket(/*blocking=*/true, IpAddressFamily::IP_V4);
    if (socket == kInvalidSocketFd) {
        GTEST_SKIP();
    }

    QUICHE_EXPECT_OK(socket_api::SetIpHeaderIncluded(socket, IpAddressFamily::IP_V4, /*ip_header_included=*/true));
...
{
    SocketFd socket = CreateTestRawSocket(/*blocking=*/true, IpAddressFamily::IP_V6);
    if (socket == kInvalidSocketFd) {
        GTEST_SKIP();
    }

    QUICHE_EXPECT_OK(socket_api::SetIpHeaderIncluded(socket, IpAddressFamily::IP_V6, /*ip_header_included=*/true));
...
TEST(SocketTest, Connect)
{
    ...
    QUICHE_EXPECT_OK(socket_api::Connect(socket, QuicSocketAddress(quiche::TestLoopback(), /*port=*/0)));
...
TEST(SocketTest, Bind)
{
    ...
    QUICHE_EXPECT_OK(socket_api::Bind(socket, QuicSocketAddress(quiche::TestLoopback(), /*port=*/0)));
...
{
    SocketFd socket = CreateTestSocket(socket_api::SocketProtocol::kUdp);
    QUICHE_ASSERT_OK(socket_api::Bind(socket, QuicSocketAddress(quiche::TestLoopback(), /*port=*/0)));

    absl::StatusOr<QuicSocketAddress> address = socket_api::GetSocketAddress(socket);
...
TEST(SocketTest, Accept)
{
    ...
    SocketFd socket = CreateTestSocket(socket_api::SocketProtocol::kTcp, /*blocking=*/false);
    QUICHE_ASSERT_OK(socket_api::Bind(socket, QuicSocketAddress(quiche::TestLoopback(), /*port=*/0)));
...
TEST(SocketTest, Receive)
{
    ...
    absl::StatusOr<absl::Span<char>> result = socket_api::Receive(socket, absl::MakeSpan(buffer));
    EXPECT_THAT(result, StatusIs(absl::StatusCode::kUnavailable));
...
TEST(SocketTest, Peek)
{
    ...
    std::string buffer(100, 0);
    absl::StatusOr<absl::Span<char>> result = socket_api::Receive(socket, absl::MakeSpan(buffer), /*peek=*/true);
    EXPECT_THAT(result, StatusIs(absl::StatusCode::kUnavailable));
...
TEST(SocketTest, Send)
{
    SocketFd socket = CreateTestSocket(socket_api::SocketProtocol::kUdp);
    // UDP, so "connecting" should succeed without any listening sockets.
    QUICHE_ASSERT_OK(socket_api::Connect(socket, QuicSocketAddress(quiche::TestLoopback(), /*port=*/0)));

    char buffer[] = { 12, 34, 56, 78 };
    // Expect at least some data to be sent successfully.
    absl::StatusOr<absl::string_view> result = socket_api::Send(socket, absl::string_view(buffer, sizeof(buffer)));
...
TEST(SocketTest, SendTo)
{
    ...
    absl::StatusOr<absl::string_view> result
        = socket_api::SendTo(socket, QuicSocketAddress(quiche::TestLoopback(), /*port=*/57290), absl::string_view(buffer, sizeof(buffer)));
...
TEST(SocketTest, SendToWithConnection)
{
    ...
    char buffer[] = { 12, 34, 56, 78 };
    absl::StatusOr<absl::string_view> result
        = socket_api::SendTo(socket, QuicSocketAddress(quiche::TestLoopback(), /*port=*/50495), absl::string_view(buffer, sizeof(buffer)));
    // Expect at least some data to be sent successfully.
    QUICHE_ASSERT_OK(result.status());
    EXPECT_THAT(result.value(), SizeIs(Lt(4)));
...
TEST(SocketTest, SendToForRaw)
{
    ...
    QuicIpAddress localhost_address = quiche::TestLoopback();
    QUICHE_EXPECT_OK(socket_api::SetIpHeaderIncluded(socket, localhost_address.address_family(),
    ...
    absl::StatusOr<absl::string_view> result = socket_api::SendTo(socket, QuicSocketAddress(localhost_address, /*port=*/56362), packet);
...
TEST(SocketTest, SendToForRawWithIpHeader)
{
    ...
    QuicIpAddress localhost_address = quiche::TestLoopback();
    QUICHE_EXPECT_OK(socket_api::SetIpHeaderIncluded(socket, localhost_address.address_family(), /*ip_header_included=*/true));

    // Arbitrarily-chosen ephemeral ports.
    QuicSocketAddress client_address(localhost_address, /*port=*/53368);
    QuicSocketAddress server_address(localhost_address, /*port=*/56362);
    std::string packet = CreateIpPacket(client_address.host(), server_address.host(), CreateUdpPacket(client_address, server_address, "foo"));
    absl::StatusOr<absl::string_view> result = socket_api::SendTo(socket, QuicSocketAddress(localhost_address, /*port=*/56362), packet);
