...
public:
    ...
    ABSL_MUST_USE_RESULT bool RegisterSocket(SocketFd fd, QuicSocketEventMask events, QuicSocketEventListener* listener) override;
    ABSL_MUST_USE_RESULT bool UnregisterSocket(SocketFd fd) override;
    ABSL_MUST_USE_RESULT bool RearmSocket(SocketFd fd, QuicSocketEventMask events) override;
    ABSL_MUST_USE_RESULT bool ArtificiallyNotifyEvent(SocketFd fd, QuicSocketEventMask events) override;
...
private:
    ...
    public:
        Alarm(QuicPollEventLoop* loop, QuicArenaScopedPtr<QuicAlarm::Delegate> delegate);
    ...
    public:
        AlarmFactory(QuicPollEventLoop* loop)
            : loop_(loop)
        {
        }
    ...
    using RegistrationMap = quiche::QuicheLinkedHashMap<SocketFd, std::shared_ptr<Registration>>;
    ...
    QuicTime::Delta ComputePollTimeout(QuicTime now, QuicTime::Delta default_timeout) const;
    ...
    // system call.
    int PollWithRetries(absl::Span<pollfd> fds, QuicTime start_time, QuicTime::Delta timeout);
