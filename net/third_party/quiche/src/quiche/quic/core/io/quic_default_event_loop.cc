...
{
    if (QuicEventLoopFactory* factory = quiche::GetOverrideForDefaultEventLoop()) {
        return factory;
    }
#ifdef QUICHE_ENABLE_LIBEVENT
...
std::vector<QuicEventLoopFactory*> GetAllSupportedEventLoops()
{
    ...
    if (QuicLibeventEventLoopFactory::Get()->GetName() != QuicLibeventEventLoopFactory::GetLevelTriggeredBackendForTests()->GetName()) {
        loops.push_back(QuicLibeventEventLoopFactory::GetLevelTriggeredBackendForTests());
    }
#endif
    std::vector<QuicEventLoopFactory*> extra = quiche::GetExtraEventLoopImplementations();
