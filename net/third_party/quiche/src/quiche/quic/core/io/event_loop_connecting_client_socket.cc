...
EventLoopConnectingClientSocket::EventLoopConnectingClientSocket(socket_api::SocketProtocol protocol, const quic::QuicSocketAddress& peer_address,
    QuicByteCount receive_buffer_size, QuicByteCount send_buffer_size, QuicEventLoop* event_loop, quiche::QuicheBufferAllocator* buffer_allocator,
...
EventLoopConnectingClientSocket::~EventLoopConnectingClientSocket()
{
    ...
    if (descriptor_ != kInvalidSocketFd) {
        QUICHE_BUG(quic_event_loop_connecting_socket_invalid_destruction) << "Must call Disconnect() on connected socket before destruction.";
        Close();
    }
...
absl::Status EventLoopConnectingClientSocket::ConnectBlocking()
{
    ...
    if (!status.ok()) {
        QUICHE_LOG_FIRST_N(WARNING, 100) << "Failed to set socket to address: " << peer_address_.ToString()
    ...
    if (absl::IsUnavailable(status)) {
        QUICHE_LOG_FIRST_N(ERROR, 100) << "Non-blocking connect to should-be blocking socket to address:" << peer_address_.ToString() << ".";
        Close();
    ...
    if (!status.ok()) {
        QUICHE_LOG_FIRST_N(WARNING, 100) << "Failed to return socket to address: " << peer_address_.ToString()
...
void EventLoopConnectingClientSocket::Disconnect()
{
    ...
    bool require_send_callback = !absl::holds_alternative<absl::monostate>(send_data_);
...
absl::StatusOr<QuicSocketAddress> EventLoopConnectingClientSocket::GetLocalAddress()
{
    QUICHE_DCHECK_NE(descriptor_, kInvalidSocketFd);
...
absl::StatusOr<quiche::QuicheMemSlice> EventLoopConnectingClientSocket::ReceiveBlocking(QuicByteCount max_size)
{
    ...
    absl::Status status = socket_api::SetSocketBlocking(descriptor_, /*blocking=*/true);
    if (!status.ok()) {
        QUICHE_LOG_FIRST_N(WARNING, 100) << "Failed to set socket to address: " << peer_address_.ToString()
    ...
    if (!buffer.ok() && absl::IsUnavailable(buffer.status())) {
        QUICHE_LOG_FIRST_N(ERROR, 100) << "Non-blocking receive from should-be blocking socket to address:" << peer_address_.ToString() << ".";
        receive_max_size_.reset();
    } else {
        QUICHE_DCHECK(!receive_max_size_.has_value());
    }

    absl::Status set_non_blocking_status = socket_api::SetSocketBlocking(descriptor_, /*blocking=*/false);
    if (!set_non_blocking_status.ok()) {
        QUICHE_LOG_FIRST_N(WARNING, 100) << "Failed to return socket to address: " << peer_address_.ToString()
                                         << " to non-blocking after receive with error: " << set_non_blocking_status;
...
absl::Status EventLoopConnectingClientSocket::SendBlocking(quiche::QuicheMemSlice data)
{
    QUICHE_DCHECK(!data.empty());
    QUICHE_DCHECK(absl::holds_alternative<absl::monostate>(send_data_));
...
void EventLoopConnectingClientSocket::SendAsync(quiche::QuicheMemSlice data)
{
    ...
    send_data_ = std::move(data);
    send_remaining_ = absl::get<quiche::QuicheMemSlice>(send_data_).AsStringView();

    FinishOrRearmAsyncSend(SendInternal());
}

void EventLoopConnectingClientSocket::OnSocketEvent(QuicEventLoop* event_loop, SocketFd fd, QuicSocketEventMask events)
{
    QUICHE_DCHECK_EQ(event_loop, event_loop_);
    QUICHE_DCHECK_EQ(fd, descriptor_);

    if (connect_status_ == ConnectStatus::kConnecting && (events & (kSocketEventWritable | kSocketEventError))) {
        FinishOrRearmAsyncConnect(GetConnectResult());
        return;
    }

    if (receive_max_size_.has_value() && (events & (kSocketEventReadable | kSocketEventError))) {
        FinishOrRearmAsyncReceive(ReceiveInternal());
    }
    if (!send_remaining_.empty() && (events & (kSocketEventWritable | kSocketEventError))) {
        FinishOrRearmAsyncSend(SendInternal());
    }
}
...
absl::Status EventLoopConnectingClientSocket::Open()
{
    ...
    absl::StatusOr<SocketFd> descriptor = socket_api::CreateSocket(peer_address_.host().address_family(), protocol_,
        /*blocking=*/false);
    if (!descriptor.ok()) {
        QUICHE_DVLOG(1) << "Failed to open socket for connection to address: " << peer_address_.ToString() << " with error: " << descriptor.status();
    ...
    if (async_visitor_) {
        ...
        if (event_loop_->SupportsEdgeTriggered()) {
            registered = event_loop_->RegisterSocket(descriptor_, kSocketEventReadable | kSocketEventWritable | kSocketEventError, this);
        } else {
    ...
    if (receive_buffer_size_ != 0) {
        absl::Status status = socket_api::SetReceiveBufferSize(descriptor_, receive_buffer_size_);
        if (!status.ok()) {
            QUICHE_LOG_FIRST_N(WARNING, 100) << "Failed to set receive buffer size to: " << receive_buffer_size_
                                             << " for socket to address: " << peer_address_.ToString() << " with error: " << status;
            Close();
            return status;
        }
    }
    ...
    if (send_buffer_size_ != 0) {
        absl::Status status = socket_api::SetSendBufferSize(descriptor_, send_buffer_size_);
        if (!status.ok()) {
            QUICHE_LOG_FIRST_N(WARNING, 100) << "Failed to set send buffer size to: " << send_buffer_size_
                                             << " for socket to address: " << peer_address_.ToString() << " with error: " << status;
            Close();
            return status;
        }
...
void EventLoopConnectingClientSocket::Close()
{
    ...
    if (!status.ok()) {
        QUICHE_LOG_FIRST_N(WARNING, 100) << "Could not close socket to address: " << peer_address_.ToString() << " with error: " << status;
    }
...
absl::Status EventLoopConnectingClientSocket::DoInitialConnect()
{
    ...
    if (connect_result.ok()) {
        ...
        QUICHE_DVLOG(1) << "Synchronously failed to connect socket to address: " << peer_address_.ToString() << " with error: " << connect_result;
        Close();
        connect_status_ = ConnectStatus::kNotConnected;
    }
...
absl::Status EventLoopConnectingClientSocket::GetConnectResult()
{
    ...
    if (!error.ok()) {
        QUICHE_DVLOG(1) << "Asynchronously failed to connect socket to address: " << peer_address_.ToString() << " with error: " << error;
        Close();
        connect_status_ = ConnectStatus::kNotConnected;
        return error;
    }
    ...
    if (peek_data.ok() || absl::IsUnavailable(peek_data.status())) {
        ...
        QUICHE_LOG_FIRST_N(WARNING, 100) << "Socket to address: " << peer_address_.ToString()
...
void EventLoopConnectingClientSocket::FinishOrRearmAsyncConnect(absl::Status status)
{
    if (absl::IsUnavailable(status)) {
        if (!event_loop_->SupportsEdgeTriggered()) {
            bool result = event_loop_->RearmSocket(descriptor_, kSocketEventWritable | kSocketEventError);
...
absl::StatusOr<quiche::QuicheMemSlice> EventLoopConnectingClientSocket::ReceiveInternal()
{
    QUICHE_DCHECK_NE(descriptor_, kInvalidSocketFd);
    QUICHE_DCHECK(connect_status_ == ConnectStatus::kConnected);
    ...
    absl::StatusOr<absl::Span<char>> received = socket_api::Receive(descriptor_, absl::MakeSpan(buffer.data(), buffer.size()));
    ...
    if (received.ok()) {
        ...
        receive_max_size_.reset();
        return quiche::QuicheMemSlice(quiche::QuicheBuffer(buffer.Release(), received->size()));
    } else {
        if (!absl::IsUnavailable(received.status())) {
            QUICHE_DVLOG(1) << "Failed to receive from socket to address: " << peer_address_.ToString() << " with error: " << received.status();
            receive_max_size_.reset();
        }
...
void EventLoopConnectingClientSocket::FinishOrRearmAsyncReceive(absl::StatusOr<quiche::QuicheMemSlice> buffer)
{
    QUICHE_DCHECK(async_visitor_);
    QUICHE_DCHECK(connect_status_ == ConnectStatus::kConnected);
    ...
    if (!buffer.ok() && absl::IsUnavailable(buffer.status())) {
        if (!event_loop_->SupportsEdgeTriggered()) {
            bool result = event_loop_->RearmSocket(descriptor_, kSocketEventReadable | kSocketEventError);
            QUICHE_DCHECK(result);
        }
        QUICHE_DCHECK(receive_max_size_.has_value());
    } else {
...
absl::StatusOr<bool> EventLoopConnectingClientSocket::OneBytePeek()
{
    ...
    char peek_buffer;
    absl::StatusOr<absl::Span<char>> peek_received = socket_api::Receive(descriptor_, absl::MakeSpan(&peek_buffer, /*size=*/1), /*peek=*/true);
    if (!peek_received.ok()) {
...
absl::Status EventLoopConnectingClientSocket::SendBlockingInternal()
{
    ...
    absl::Status status = socket_api::SetSocketBlocking(descriptor_, /*blocking=*/true);
    if (!status.ok()) {
        QUICHE_LOG_FIRST_N(WARNING, 100) << "Failed to set socket to address: " << peer_address_.ToString() << " as blocking for send with error: " << status;
        send_data_ = absl::monostate();
        return status;
    }
    ...
    status = SendInternal();
    if (absl::IsUnavailable(status)) {
        QUICHE_LOG_FIRST_N(ERROR, 100) << "Non-blocking send for should-be blocking socket to address:" << peer_address_.ToString();
        send_data_ = absl::monostate();
        send_remaining_ = "";
    } else {
        QUICHE_DCHECK(absl::holds_alternative<absl::monostate>(send_data_));
    ...
    if (!set_non_blocking_status.ok()) {
        QUICHE_LOG_FIRST_N(WARNING, 100) << "Failed to return socket to address: " << peer_address_.ToString()
                                         << " to non-blocking after send with error: " << set_non_blocking_status;
        return set_non_blocking_status;
    }
...
absl::Status EventLoopConnectingClientSocket::SendInternal()
{
    ...
    while (!send_remaining_.empty()) {
        absl::StatusOr<absl::string_view> remainder = socket_api::Send(descriptor_, send_remaining_);

        if (remainder.ok()) {
            QUICHE_DCHECK(
                remainder->empty() || (remainder->data() >= send_remaining_.data() && remainder->data() < send_remaining_.data() + send_remaining_.size()));
            QUICHE_DCHECK(remainder->empty() || (remainder->data() + remainder->size() == send_remaining_.data() + send_remaining_.size()));
            send_remaining_ = *remainder;
        } else {
            if (!absl::IsUnavailable(remainder.status())) {
                QUICHE_DVLOG(1) << "Failed to send to socket to address: " << peer_address_.ToString() << " with error: " << remainder.status();
                send_data_ = absl::monostate();
                send_remaining_ = "";
            }
...
void EventLoopConnectingClientSocket::FinishOrRearmAsyncSend(absl::Status status)
{
    QUICHE_DCHECK(async_visitor_);
    QUICHE_DCHECK(connect_status_ == ConnectStatus::kConnected);
