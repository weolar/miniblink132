...
public:
    ...
    virtual void OnSocketEvent(QuicEventLoop* event_loop, SocketFd fd, QuicSocketEventMask events) = 0;
};
...
public:
    ...
    virtual ABSL_MUST_USE_RESULT bool RegisterSocket(SocketFd fd, QuicSocketEventMask events, QuicSocketEventListener* listener) = 0;
    ...
    // false.
    virtual ABSL_MUST_USE_RESULT bool RearmSocket(SocketFd fd, QuicSocketEventMask events) = 0;
    // Causes the |fd| to be notified of |events| on the next event loop iteration
    // even if none of the specified events has happened.
    virtual ABSL_MUST_USE_RESULT bool ArtificiallyNotifyEvent(SocketFd fd, QuicSocketEventMask events) = 0;
