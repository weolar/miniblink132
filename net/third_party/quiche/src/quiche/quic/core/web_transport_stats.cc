...
webtransport::DatagramStats WebTransportDatagramStatsForQuicSession(const QuicSession& session)
{
...
webtransport::SessionStats WebTransportStatsForQuicSession(const QuicSession& session)
{
    const RttStats* rtt_stats = session.connection()->sent_packet_manager().GetRttStats();
    ...
    result.estimated_send_rate_bps = session.connection()->sent_packet_manager().BandwidthEstimate().ToBitsPerSecond();
