...
public:
    ...
    TlsServerHandshaker(QuicSession* session, const QuicCryptoServerConfig* crypto_config);
    ...
    void SendServerConfigUpdate(const CachedNetworkParameters* cached_network_params) override;
    ...
    void SetServerApplicationStateForResumption(std::unique_ptr<ApplicationState> state) override;
    size_t BufferSizeLimitForLevel(EncryptionLevel level) const override;
    std::unique_ptr<QuicDecrypter> AdvanceKeysAndCreateCurrentOneRttDecrypter() override;
    std::unique_ptr<QuicEncrypter> CreateCurrentOneRttEncrypter() override;
    void SetWriteSecret(EncryptionLevel level, const SSL_CIPHER* cipher, absl::Span<const uint8_t> write_secret) override;
    ...
    virtual std::string GetAcceptChValueForHostname(const std::string& hostname) const;
...
protected:
    ...
    virtual bool ProcessAdditionalTransportParameters(const TransportParameters& /*params*/)
    {
        return true;
    }
    ...
    int SelectAlpn(const uint8_t** out, uint8_t* out_len, const uint8_t* in, unsigned in_len) override;
    ssl_private_key_result_t PrivateKeySign(uint8_t* out, size_t* out_len, size_t max_out, uint16_t sig_alg, absl::string_view in) override;
    ssl_private_key_result_t PrivateKeyComplete(uint8_t* out, size_t* out_len, size_t max_out) override;
    size_t SessionTicketMaxOverhead() override;
    int SessionTicketSeal(uint8_t* out, size_t* out_len, size_t max_out_len, absl::string_view in) override;
    ssl_ticket_aead_result_t SessionTicketOpen(uint8_t* out, size_t* out_len, size_t max_out_len, absl::string_view in) override;
    // Called when ticket_decryption_callback_ is done to determine a final
    // decryption result.
    ssl_ticket_aead_result_t FinalizeSessionTicketOpen(uint8_t* out, size_t* out_len, size_t max_out_len);
    TlsConnection::Delegate* ConnectionDelegate() override
    {
        return this;
    }
    ...
    void OnSelectCertificateDone(bool ok, bool is_sync, SSLConfig ssl_config, absl::string_view ticket_encryption_key, bool cert_matched_sni) override;

    void OnComputeSignatureDone(bool ok, bool is_sync, std::string signature, std::unique_ptr<ProofSource::Details> details) override;
    ...
    void SetIgnoreTicketOpen(bool value)
    {
        ignore_ticket_open_ = value;
    }
...
private:
    ...
    public:
        ...
        QuicAsyncStatus SelectCertificate(const QuicSocketAddress& server_address, const QuicSocketAddress& client_address,
            ...
            const std::optional<std::vector<uint8_t>>& early_data_context, const QuicSSLConfig& ssl_config) override;

        // Delegates to proof_source_->ComputeTlsSignature.
        // Returns QUIC_SUCCESS, QUIC_FAILURE or QUIC_PENDING.
        QuicAsyncStatus ComputeSignature(const QuicSocketAddress& server_address, const QuicSocketAddress& client_address, const std::string& hostname,
            uint16_t signature_algorithm, absl::string_view in, size_t max_signature_size) override;
    ...
    private:
        class QUICHE_EXPORT DefaultSignatureCallback : public ProofSource::SignatureCallback {
        public:
            ...
            void Run(bool ok, std::string signature, std::unique_ptr<ProofSource::Details> details) override
            {
                ...
                if (handle->handshaker_ != nullptr) {
                    handle->handshaker_->OnComputeSignatureDone(ok, is_sync_, std::move(signature), std::move(details));
                }
            }
    ...
    bool ProcessTransportParameters(const SSL_CLIENT_HELLO* client_hello, std::string* error_details);
    // Compares |serialized_params| with |server_params_|.
    // Returns true if handshaker serialization is equivalent.
    bool TransportParametersMatch(absl::Span<const uint8_t> serialized_params) const;
    ...
    quiche::QuicheReferenceCountedPointer<QuicCryptoNegotiatedParameters> crypto_negotiated_params_;
    ...
    mutable std::unique_ptr<CachedNetworkParameters> last_received_cached_network_params_;
