...
void PopulatePacketInfoFromControlMessageBase(PlatformCmsghdr* cmsg, QuicUdpPacketInfo* packet_info, QuicUdpPacketInfoBitMask packet_info_interested)
{
...
QuicUdpSocketFd QuicUdpSocketApi::Create(int address_family, int receive_buffer_size, int send_buffer_size, bool ipv6_only)
{
    ...
    if (!SetupSocket(*socket, address_family, receive_buffer_size, send_buffer_size, ipv6_only)) {
        Destroy(*socket);
        return kQuicInvalidSocketFd;
    }
...
{
    int get_self_ip = 1;
    return 0 == setsockopt(fd, IPPROTO_IP, IP_PKTINFO, reinterpret_cast<char*>(&get_self_ip), sizeof(get_self_ip));
}

bool QuicUdpSocketApi::EnableReceiveSelfIpAddressForV6(QuicUdpSocketFd fd)
{
    int get_self_ip = 1;
    return 0 == setsockopt(fd, IPPROTO_IPV6, kIpv6RecvPacketInfo, reinterpret_cast<char*>(&get_self_ip), sizeof(get_self_ip));
}
