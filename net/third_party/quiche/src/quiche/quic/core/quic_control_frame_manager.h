...
public:
    ...
    public:
        ...
        virtual void OnControlFrameManagerError(QuicErrorCode error_code, std::string error_details) = 0;

        virtual bool WriteControlFrame(const QuicFrame& frame, TransmissionType type) = 0;
    };
    ...
    // immediately.
    void WriteOrBufferRstStream(QuicControlFrameId id, QuicResetStreamError error, QuicStreamOffset bytes_written);

    // Tries to send a RESET_STREAM_AT frame. Buffers the frame if it cannot be
    // sent immediately.
    void WriteOrBufferResetStreamAt(QuicStreamId id, QuicResetStreamError error, QuicStreamOffset bytes_written, QuicStreamOffset reliable_size);

    // Tries to send a GOAWAY_FRAME. Buffers the frame if it cannot be sent
    // immediately.
    void WriteOrBufferGoAway(QuicErrorCode error, QuicStreamId last_good_stream_id, const std::string& reason);
    ...
    void WriteOrBufferStopSending(QuicResetStreamError error, QuicStreamId stream_id);
...
private:
    ...
    quiche::QuicheLinkedHashMap<QuicControlFrameId, bool> pending_retransmissions_;
