...
QuicDatagramQueue::QuicDatagramQueue(QuicSession* session, std::unique_ptr<Observer> observer)
    : session_(session)
...
MessageStatus QuicDatagramQueue::SendOrQueueDatagram(quiche::QuicheMemSlice datagram)
{
    ...
    queue_.emplace_back(Datagram { std::move(datagram), clock_->ApproximateNow() + GetMaxTimeInQueue() });
...
std::optional<MessageStatus> QuicDatagramQueue::TrySendingNextDatagram()
{
    ...
    MessageResult result = session_->SendMessage(absl::MakeSpan(&queue_.front().datagram, 1));
    if (result.status != MESSAGE_STATUS_BLOCKED) {
        queue_.pop_front();
        if (observer_) {
            observer_->OnDatagramProcessed(result.status);
...
QuicTime::Delta QuicDatagramQueue::GetMaxTimeInQueue() const
{
    ...
    const QuicTime::Delta min_rtt = session_->connection()->sent_packet_manager().GetRttStats()->min_rtt();
    return std::max(kExpiryInMinRtts * min_rtt, kMinPacingWindows * kAlarmGranularity);
}
