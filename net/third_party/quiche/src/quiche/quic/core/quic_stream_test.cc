...
public:
    ...
    void Initialize(Perspective perspective = Perspective::IS_SERVER)
    {
        ...
        connection_ = new StrictMock<MockQuicConnection>(&helper_, &alarm_factory_, perspective, version_vector);
        connection_->AdvanceTime(QuicTime::Delta::FromSeconds(1));
        session_ = std::make_unique<StrictMock<MockQuicSession>>(connection_);
        session_->Initialize();
        connection_->SetEncrypter(ENCRYPTION_FORWARD_SECURE, std::make_unique<NullEncrypter>(connection_->perspective()));
        ...
        stream_ = new StrictMock<TestStream>(kTestStreamId, session_.get(), BIDIRECTIONAL);
        EXPECT_NE(nullptr, stream_);
        EXPECT_CALL(*session_, ShouldKeepConnectionAlive()).WillRepeatedly(Return(true));
        // session_ now owns stream_.
        session_->ActivateStream(absl::WrapUnique(stream_));
        // Ignore resetting when session_ is terminated.
        EXPECT_CALL(*session_, MaybeSendStopSendingFrame(kTestStreamId, _)).Times(AnyNumber());
        EXPECT_CALL(*session_, MaybeSendRstStreamFrame(kTestStreamId, _, _)).Times(AnyNumber());
    ...
    QuicConsumedData CloseStreamOnWriteError(QuicStreamId id, QuicByteCount /*write_length*/, QuicStreamOffset /*offset*/, StreamSendingState /*state*/,
...
protected:
    ...
    QuicStreamId kTestStreamId = GetNthClientInitiatedBidirectionalStreamId(GetParam().transport_version, 1);
    const QuicStreamId kTestPendingStreamId = GetNthClientInitiatedUnidirectionalStreamId(GetParam().transport_version, 1);
};

INSTANTIATE_TEST_SUITE_P(QuicStreamTests, QuicStreamTest, ::testing::ValuesIn(AllSupportedVersions()), ::testing::PrintToStringParamName());

using PendingStreamTest = QuicStreamTest;

INSTANTIATE_TEST_SUITE_P(PendingStreamTests, PendingStreamTest, ::testing::ValuesIn(CurrentSupportedHttp3Versions()), ::testing::PrintToStringParamName());
...
TEST_P(PendingStreamTest, PendingStreamTypeOnClient)
{
    ...
    QuicStreamId server_initiated_pending_stream_id = GetNthServerInitiatedUnidirectionalStreamId(session_->transport_version(), 1);
...
TEST_P(PendingStreamTest, PendingStreamTooMuchData)
{
    ...
    QuicStreamFrame frame(kTestPendingStreamId, false, kInitialSessionFlowControlWindowForTest + 1, ".");
...
TEST_P(PendingStreamTest, PendingStreamTooMuchDataInRstStream)
{
    ...
    // higher than the receive window offset.
    QuicRstStreamFrame frame1(kInvalidControlFrameId, kTestPendingStreamId, QUIC_STREAM_CANCELLED, kInitialSessionFlowControlWindowForTest + 1);
    ...
    QuicStreamId bidirection_stream_id = QuicUtils::GetFirstBidirectionalStreamId(session_->transport_version(), Perspective::IS_CLIENT);
    PendingStream pending2(bidirection_stream_id, session_.get());
    // Receive a rst stream frame that violates flow control: the byte offset is
    // higher than the receive window offset.
    QuicRstStreamFrame frame2(kInvalidControlFrameId, bidirection_stream_id, QUIC_STREAM_CANCELLED, kInitialSessionFlowControlWindowForTest + 1);
...
TEST_P(PendingStreamTest, PendingStreamRstStream)
{
    ...
    QuicRstStreamFrame frame(kInvalidControlFrameId, kTestPendingStreamId, QUIC_STREAM_CANCELLED, final_byte_offset);
...
TEST_P(PendingStreamTest, PendingStreamWindowUpdate)
{
    ...
    QuicWindowUpdateFrame frame(kInvalidControlFrameId, bidirection_stream_id, kDefaultFlowControlSendWindow * 2);
    pending.OnWindowUpdateFrame(frame);
    TestStream stream(&pending, session_.get(), false);

    EXPECT_EQ(QuicStreamPeer::SendWindowSize(&stream), kDefaultFlowControlSendWindow * 2);
}
...
TEST_P(PendingStreamTest, PendingStreamStopSending)
{
    ...
    QuicStreamId bidirection_stream_id = QuicUtils::GetFirstBidirectionalStreamId(session_->transport_version(), Perspective::IS_CLIENT);
    PendingStream pending(bidirection_stream_id, session_.get());
    QuicResetStreamError error = QuicResetStreamError::FromInternal(QUIC_STREAM_INTERNAL_ERROR);
...
TEST_P(PendingStreamTest, FromPendingStream)
{
    ...
    EXPECT_EQ(frame2.offset + 1, session_->flow_controller()->highest_received_byte_offset());
}
...
TEST_P(PendingStreamTest, ResetStreamAt)
{
    ...
    QuicResetStreamAtFrame rst(0, kTestPendingStreamId, QUIC_STREAM_CANCELLED, 100, 3);
    ...
    EXPECT_CALL(*stream, OnDataAvailable()).WillOnce([&]() { stream->ConsumeData(3); });
...
TEST_P(QuicStreamTest, WriteAllData)
{
    ...
    QuicByteCount length = 1
        + QuicPacketCreator::StreamFramePacketOverhead(connection_->transport_version(), kPacket8ByteConnectionId, kPacket0ByteConnectionId, !kIncludeVersion,
            !kIncludeDiversificationNonce, PACKET_4BYTE_PACKET_NUMBER, quiche::VARIABLE_LENGTH_INTEGER_LENGTH_0, quiche::VARIABLE_LENGTH_INTEGER_LENGTH_0, 0u);
    connection_->SetMaxPacketLength(length);

    EXPECT_CALL(*session_, WritevData(kTestStreamId, _, _, _, _, _)).WillOnce(Invoke(session_.get(), &MockQuicSession::ConsumeData));
...
TEST_P(QuicStreamTest, NoBlockingIfNoDataOrFin)
{
    ...
    // blocked.
    EXPECT_QUIC_BUG(stream_->WriteOrBufferData(absl::string_view(), false, nullptr), "");
    EXPECT_FALSE(HasWriteBlockedStreams());
}
...
TEST_P(QuicStreamTest, BlockIfOnlySomeDataConsumed)
{
    ...
    EXPECT_CALL(*session_, WritevData(kTestStreamId, _, _, _, _, _)).WillOnce(InvokeWithoutArgs([this]() {
        return session_->ConsumeData(stream_->id(), 1u, 0u, NO_FIN, NOT_RETRANSMISSION, std::nullopt);
    }));
...
TEST_P(QuicStreamTest, BlockIfFinNotConsumedWithData)
{
    ...
    // last data)
    EXPECT_CALL(*session_, WritevData(kTestStreamId, _, _, _, _, _)).WillOnce(InvokeWithoutArgs([this]() {
        return session_->ConsumeData(stream_->id(), 2u, 0u, NO_FIN, NOT_RETRANSMISSION, std::nullopt);
    }));
...
TEST_P(QuicStreamTest, BlockIfSoloFinNotConsumed)
{
    ...
    EXPECT_CALL(*session_, WritevData(kTestStreamId, _, _, _, _, _)).WillOnce(Return(QuicConsumedData(0, false)));
...
TEST_P(QuicStreamTest, CloseOnPartialWrite)
{
    ...
    EXPECT_CALL(*session_, WritevData(kTestStreamId, _, _, _, _, _)).WillOnce(Invoke(this, &QuicStreamTest::CloseStreamOnWriteError));
    stream_->WriteOrBufferData(absl::string_view(kData1, 2), false, nullptr);
...
TEST_P(QuicStreamTest, WriteOrBufferData)
{
    ...
    EXPECT_CALL(*session_, WritevData(_, _, _, _, _, _)).WillOnce(InvokeWithoutArgs([this]() {
        return session_->ConsumeData(stream_->id(), kDataLen - 1, 0u, NO_FIN, NOT_RETRANSMISSION, std::nullopt);
    }));
    stream_->WriteOrBufferData(kData1, false, nullptr);
    ...
    InSequence s;
    EXPECT_CALL(*session_, WritevData(_, _, _, _, _, _)).WillOnce(InvokeWithoutArgs([this]() {
        return session_->ConsumeData(stream_->id(), kDataLen - 1, kDataLen - 1, NO_FIN, NOT_RETRANSMISSION, std::nullopt);
    }));
    ...
    EXPECT_CALL(*session_, WritevData(_, _, _, _, _, _)).WillOnce(InvokeWithoutArgs([this]() {
        return session_->ConsumeData(stream_->id(), 2u, 2 * kDataLen - 2, NO_FIN, NOT_RETRANSMISSION, std::nullopt);
    }));
    EXPECT_CALL(*stream_, OnCanWriteNewData());
    stream_->OnCanWrite();
    EXPECT_TRUE(session_->HasUnackedStreamData());
...
TEST_P(QuicStreamTest, WriteOrBufferDataReachStreamLimit)
{
    ...
    QuicStreamPeer::SetStreamBytesWritten(kMaxStreamLength - data.length(), stream_);
    EXPECT_CALL(*session_, WritevData(_, _, _, _, _, _)).WillOnce(Invoke(session_.get(), &MockQuicSession::ConsumeData));
...
TEST_P(QuicStreamTest, ConnectionCloseAfterStreamClose)
{
    ...
    QuicRstStreamFrame rst_frame(kInvalidControlFrameId, stream_->id(), QUIC_STREAM_CANCELLED, 1234);
    ...
    if (VersionHasIetfQuicFrames(session_->transport_version())) {
        ...
        QuicStopSendingFrame stop_sending(kInvalidControlFrameId, stream_->id(), QUIC_STREAM_CANCELLED);
...
TEST_P(QuicStreamTest, RstNotSentIfFinSent)
{
    ...
    EXPECT_CALL(*session_, WritevData(kTestStreamId, _, _, _, _, _)).WillOnce(InvokeWithoutArgs([this]() {
        return session_->ConsumeData(stream_->id(), 1u, 0u, FIN, NOT_RETRANSMISSION, std::nullopt);
    }));
...
TEST_P(QuicStreamTest, StreamFlowControlMultipleWindowUpdates)
{
    ...
    EXPECT_EQ(kMinimumFlowControlSendWindow, QuicStreamPeer::SendWindowOffset(stream_));

    // Check a single WINDOW_UPDATE results in correct offset.
    QuicWindowUpdateFrame window_update_1(kInvalidControlFrameId, stream_->id(), kMinimumFlowControlSendWindow + 5);
    stream_->OnWindowUpdateFrame(window_update_1);
    EXPECT_EQ(window_update_1.max_data, QuicStreamPeer::SendWindowOffset(stream_));

    // Now send a few more WINDOW_UPDATES and make sure that only the largest is
    // remembered.
    QuicWindowUpdateFrame window_update_2(kInvalidControlFrameId, stream_->id(), 1);
    QuicWindowUpdateFrame window_update_3(kInvalidControlFrameId, stream_->id(), kMinimumFlowControlSendWindow + 10);
    QuicWindowUpdateFrame window_update_4(kInvalidControlFrameId, stream_->id(), 5678);
    ...
    EXPECT_EQ(window_update_3.max_data, QuicStreamPeer::SendWindowOffset(stream_));
}
...
TEST_P(QuicStreamTest, StreamSequencerNeverSeesPacketsViolatingFlowControl)
{
    ...
    // Receive a stream frame that violates flow control: the byte offset is
    // higher than the receive window offset.
    QuicStreamFrame frame(stream_->id(), false, kInitialSessionFlowControlWindowForTest + 1, ".");
...
TEST_P(QuicStreamTest, StopReadingSendsFlowControl)
{
    ...
    EXPECT_CALL(*connection_, CloseConnection(QUIC_FLOW_CONTROL_RECEIVED_TOO_MUCH_DATA, _, _)).Times(0);
    EXPECT_CALL(*session_, WriteControlFrame(_, _)).Times(AtLeast(1)).WillRepeatedly(Invoke(&ClearControlFrameWithTransmissionType));

    std::string data(1000, 'x');
    for (QuicStreamOffset offset = 0; offset < 2 * kInitialStreamFlowControlWindowForTest; offset += data.length()) {
        QuicStreamFrame frame(stream_->id(), false, offset, data);
        stream_->OnStreamFrame(frame);
    }
    EXPECT_LT(kInitialStreamFlowControlWindowForTest, QuicStreamPeer::ReceiveWindowOffset(stream_));
}
...
TEST_P(QuicStreamTest, InvalidFinalByteOffsetFromRst)
{
    ...
    EXPECT_FALSE(stream_->HasReceivedFinalOffset());
    QuicRstStreamFrame rst_frame(kInvalidControlFrameId, stream_->id(), QUIC_STREAM_CANCELLED, 0xFFFFFFFFFFFF);
    // Stream should not accept the frame, and the connection should be closed.
    EXPECT_CALL(*connection_, CloseConnection(QUIC_FLOW_CONTROL_RECEIVED_TOO_MUCH_DATA, _, _));
    stream_->OnStreamReset(rst_frame);
    EXPECT_TRUE(stream_->HasReceivedFinalOffset());
...
TEST_P(QuicStreamTest, FinalByteOffsetFromZeroLengthStreamFrame)
{
    ...
    const QuicStreamOffset current_stream_flow_control_offset = QuicStreamPeer::ReceiveWindowOffset(stream_);
    ...
    ASSERT_GT(kByteOffsetExceedingFlowControlWindow, current_stream_flow_control_offset);
    ASSERT_GT(kByteOffsetExceedingFlowControlWindow, current_connection_flow_control_offset);
    QuicStreamFrame zero_length_stream_frame_with_fin(stream_->id(), /*fin=*/true, kByteOffsetExceedingFlowControlWindow, absl::string_view());
    ...
    EXPECT_EQ(current_stream_flow_control_offset, QuicStreamPeer::ReceiveWindowOffset(stream_));
    EXPECT_EQ(current_connection_flow_control_offset, QuicFlowControllerPeer::ReceiveWindowOffset(session_->flow_controller()));
}

TEST_P(QuicStreamTest, OnStreamResetOffsetOverflow)
{
    Initialize();
    QuicRstStreamFrame rst_frame(kInvalidControlFrameId, stream_->id(), QUIC_STREAM_CANCELLED, kMaxStreamLength + 1);
    EXPECT_CALL(*connection_, CloseConnection(QUIC_STREAM_LENGTH_OVERFLOW, _, _));
    stream_->OnStreamReset(rst_frame);
}
...
TEST_P(QuicStreamTest, OnStreamFrameUpperLimit)
{
    ...
    QuicFlowControllerPeer::SetReceiveWindowOffset(session_->flow_controller(), kMaxStreamLength + 5u);
    QuicStreamSequencerPeer::SetFrameBufferTotalBytesRead(QuicStreamPeer::sequencer(stream_), kMaxStreamLength - 10u);

    EXPECT_CALL(*connection_, CloseConnection(QUIC_STREAM_LENGTH_OVERFLOW, _, _)).Times(0);
...
TEST_P(QuicStreamTest, StreamTooLong)
{
    ...
    EXPECT_QUIC_PEER_BUG(
        {
            EXPECT_CALL(*connection_, CloseConnection(QUIC_STREAM_LENGTH_OVERFLOW, _, _)).Times(1);
            stream_->OnStreamFrame(stream_frame);
        },
        absl::StrCat("Receive stream frame on stream ", stream_->id(), " reaches max stream length"));
}
...
TEST_P(QuicStreamTest, SetDrainingIncomingOutgoing)
{
    ...
    EXPECT_CALL(*session_, WritevData(kTestStreamId, _, _, _, _, _)).WillOnce(InvokeWithoutArgs([this]() {
        return session_->ConsumeData(stream_->id(), 2u, 0u, FIN, NOT_RETRANSMISSION, std::nullopt);
    }));
    stream_->WriteOrBufferData(absl::string_view(kData1, 2), true, nullptr);
...
TEST_P(QuicStreamTest, EarlyResponseFinHandling)
{
    ...
    Initialize();
    EXPECT_CALL(*connection_, CloseConnection(_, _, _)).Times(0);
    EXPECT_CALL(*session_, WritevData(_, _, _, _, _, _)).WillRepeatedly(Invoke(session_.get(), &MockQuicSession::ConsumeData));
...
TEST_P(QuicStreamTest, StreamDataGetAckedOutOfOrder)
{
    ...
    EXPECT_EQ(9u, newly_acked_length);
    EXPECT_EQ(3u, QuicStreamPeer::SendBuffer(stream_).size());
    EXPECT_TRUE(stream_->OnStreamFrameAcked(18, 9, false, QuicTime::Delta::Zero(), QuicTime::Zero(), &newly_acked_length));
    EXPECT_TRUE(session_->HasUnackedStreamData());
    EXPECT_EQ(9u, newly_acked_length);
    EXPECT_EQ(3u, QuicStreamPeer::SendBuffer(stream_).size());
    EXPECT_TRUE(stream_->OnStreamFrameAcked(0, 9, false, QuicTime::Delta::Zero(), QuicTime::Zero(), &newly_acked_length));
...
TEST_P(QuicStreamTest, CancelStream)
{
    ...
    EXPECT_TRUE(stream_->IsWaitingForAcks());
    EXPECT_TRUE(session_->HasUnackedStreamData());
    EXPECT_CALL(*connection_, OnStreamReset(stream_->id(), QUIC_STREAM_CANCELLED));
    EXPECT_CALL(*session_, WriteControlFrame(_, _)).Times(AtLeast(1)).WillRepeatedly(Invoke(&ClearControlFrameWithTransmissionType));

    EXPECT_CALL(*session_, MaybeSendRstStreamFrame(_, _, _)).WillOnce(InvokeWithoutArgs([this]() {
        session_->ReallyMaybeSendRstStreamFrame(stream_->id(), QUIC_STREAM_CANCELLED, stream_->stream_bytes_written());
    }));
...
TEST_P(QuicStreamTest, RstFrameReceivedStreamNotFinishSending)
{
    ...
    // RST_STREAM received.
    QuicRstStreamFrame rst_frame(kInvalidControlFrameId, stream_->id(), QUIC_STREAM_CANCELLED, 9);

    EXPECT_CALL(*session_, MaybeSendRstStreamFrame(stream_->id(), QuicResetStreamError::FromInternal(QUIC_RST_ACKNOWLEDGEMENT), 9));
    stream_->OnStreamReset(rst_frame);
    EXPECT_EQ(1u, QuicStreamPeer::SendBuffer(stream_).size());
...
TEST_P(QuicStreamTest, WriteBufferedData)
{
    ...
    EXPECT_CALL(*session_, WritevData(_, _, _, _, _, _)).WillOnce(InvokeWithoutArgs([this]() {
        return session_->ConsumeData(stream_->id(), 100u, 0u, NO_FIN, NOT_RETRANSMISSION, std::nullopt);
    }));
    stream_->WriteOrBufferData(data, false, nullptr);
    stream_->WriteOrBufferData(data, false, nullptr);
    stream_->WriteOrBufferData(data, false, nullptr);
    ...
    EXPECT_CALL(*session_, WritevData(_, _, _, _, _, _)).WillOnce(InvokeWithoutArgs([this]() {
        return session_->ConsumeData(stream_->id(), 100, 100u, NO_FIN, NOT_RETRANSMISSION, std::nullopt);
    }));
    ...
    QuicByteCount data_to_write = 3 * data.length() - 200 - GetQuicFlag(quic_buffered_data_threshold) + 1;
    EXPECT_CALL(*session_, WritevData(_, _, _, _, _, _)).WillOnce(InvokeWithoutArgs([this, data_to_write]() {
        return session_->ConsumeData(stream_->id(), data_to_write, 200u, NO_FIN, NOT_RETRANSMISSION, std::nullopt);
    }));
    // Buffered data size < threshold, ask upper layer for more data.
    EXPECT_CALL(*stream_, OnCanWriteNewData()).Times(1);
    stream_->OnCanWrite();
    EXPECT_EQ(static_cast<uint64_t>(GetQuicFlag(quic_buffered_data_threshold) - 1), stream_->BufferedDataBytes());
    EXPECT_TRUE(stream_->CanWriteNewData());
    ...
    // Testing Writev.
    EXPECT_CALL(*session_, WritevData(_, _, _, _, _, _)).WillOnce(Return(QuicConsumedData(0, false)));
    ...
    quiche::QuicheMemSliceStorage storage2(&iov, 1, session_->connection()->helper()->GetStreamSendBufferAllocator(), 1024);
    ...
    EXPECT_CALL(*session_, WritevData(_, _, _, _, _, _)).WillOnce(InvokeWithoutArgs([this, data_to_write]() {
        return session_->ConsumeData(stream_->id(), data_to_write, 0u, NO_FIN, NOT_RETRANSMISSION, std::nullopt);
    }));
    ...
    quiche::QuicheMemSliceStorage storage3(&iov, 1, session_->connection()->helper()->GetStreamSendBufferAllocator(), 1024);
    consumed = stream_->WriteMemSlices(storage3.ToSpan(), false);
    EXPECT_EQ(data.length(), consumed.bytes_consumed);
    EXPECT_FALSE(consumed.fin_consumed);
    EXPECT_EQ(data.length() + GetQuicFlag(quic_buffered_data_threshold) - 1, stream_->BufferedDataBytes());
    EXPECT_FALSE(stream_->CanWriteNewData());
}
...
TEST_P(QuicStreamTest, WritevDataReachStreamLimit)
{
    ...
    quiche::QuicheMemSliceStorage storage2(&iov2, 1, session_->connection()->helper()->GetStreamSendBufferAllocator(), 1024);
    EXPECT_QUIC_BUG(
        {
            EXPECT_CALL(*connection_, CloseConnection(QUIC_STREAM_LENGTH_OVERFLOW, _, _));
...
TEST_P(QuicStreamTest, WriteMemSlices)
{
    ...
    quiche::QuicheBufferAllocator* allocator = connection_->helper()->GetStreamSendBufferAllocator();
    std::vector<quiche::QuicheMemSlice> vector1;
    vector1.push_back(quiche::QuicheMemSlice(quiche::QuicheBuffer(allocator, kDataSize)));
    vector1.push_back(quiche::QuicheMemSlice(quiche::QuicheBuffer(allocator, kDataSize)));
    std::vector<quiche::QuicheMemSlice> vector2;
    vector2.push_back(quiche::QuicheMemSlice(quiche::QuicheBuffer(allocator, kDataSize)));
    vector2.push_back(quiche::QuicheMemSlice(quiche::QuicheBuffer(allocator, kDataSize)));
    ...
    QuicByteCount data_to_write = 2 * kDataSize - 100 - GetQuicFlag(quic_buffered_data_threshold) + 1;
    EXPECT_CALL(*session_, WritevData(_, _, _, _, _, _)).WillOnce(InvokeWithoutArgs([this, data_to_write]() {
        return session_->ConsumeData(stream_->id(), data_to_write, 100u, NO_FIN, NOT_RETRANSMISSION, std::nullopt);
    }));
    EXPECT_CALL(*stream_, OnCanWriteNewData()).Times(1);
    stream_->OnCanWrite();
    EXPECT_EQ(static_cast<uint64_t>(GetQuicFlag(quic_buffered_data_threshold) - 1), stream_->BufferedDataBytes());
    ...
    EXPECT_EQ(2 * kDataSize + GetQuicFlag(quic_buffered_data_threshold) - 1, stream_->BufferedDataBytes());
...
TEST_P(QuicStreamTest, WriteMemSlicesReachStreamLimit)
{
    ...
    EXPECT_CALL(*session_, WritevData(_, _, _, _, _, _)).WillOnce(InvokeWithoutArgs([this]() {
        return session_->ConsumeData(stream_->id(), 5u, 0u, NO_FIN, NOT_RETRANSMISSION, std::nullopt);
    }));
    // There is no buffered data before, all data should be consumed.
...
TEST_P(QuicStreamTest, OnStreamFrameLost)
{
    ...
    EXPECT_CALL(*session_, WritevData(_, _, _, _, _, _)).WillOnce(InvokeWithoutArgs([this]() {
        return session_->ConsumeData(stream_->id(), 9u, 18u, FIN, NOT_RETRANSMISSION, std::nullopt);
    }));
    stream_->OnCanWrite();
    EXPECT_FALSE(stream_->HasPendingRetransmission());
...
TEST_P(QuicStreamTest, CannotBundleLostFin)
{
    ...
    InSequence s;
    EXPECT_CALL(*session_, WritevData(_, _, _, _, _, _)).WillOnce(InvokeWithoutArgs([this]() {
        return session_->ConsumeData(stream_->id(), 9u, 0u, NO_FIN, NOT_RETRANSMISSION, std::nullopt);
    }));
    EXPECT_CALL(*session_, WritevData(_, _, _, _, _, _)).WillOnce(Return(QuicConsumedData(0, true)));
    stream_->OnCanWrite();
}
...
TEST_P(QuicStreamTest, MarkConnectionLevelWriteBlockedOnWindowUpdateFrame)
{
    ...
    QuicConfigPeer::SetReceivedInitialStreamFlowControlWindow(session_->config(), 100);
    QuicConfigPeer::SetReceivedInitialMaxStreamDataBytesIncomingBidirectional(session_->config(), 100);
    auto stream = new TestStream(GetNthClientInitiatedBidirectionalStreamId(GetParam().transport_version, 2), session_.get(), BIDIRECTIONAL);
    session_->ActivateStream(absl::WrapUnique(stream));
    ...
    QuicWindowUpdateFrame window_update(kInvalidControlFrameId, stream_->id(), 1234);
...
TEST_P(QuicStreamTest, MarkConnectionLevelWriteBlockedOnWindowUpdateFrameWithNoBufferedData)
{
    Initialize();
    ...
    QuicWindowUpdateFrame window_update(kInvalidControlFrameId, stream_->id(), 120);
    stream->OnWindowUpdateFrame(window_update);
...
TEST_P(QuicStreamTest, RetransmitStreamData)
{
    ...
    // Send [0, 18) with fin.
    EXPECT_CALL(*session_, WritevData(stream_->id(), _, _, _, _, _)).Times(2).WillRepeatedly(Invoke(session_.get(), &MockQuicSession::ConsumeData));
    stream_->WriteOrBufferData(kData1, false, nullptr);
    stream_->WriteOrBufferData(kData1, true, nullptr);
    // Ack [10, 13).
    QuicByteCount newly_acked_length = 0;
    stream_->OnStreamFrameAcked(10, 3, false, QuicTime::Delta::Zero(), QuicTime::Zero(), &newly_acked_length);
    EXPECT_EQ(3u, newly_acked_length);
    // Retransmit [0, 18) with fin, and only [0, 8) is consumed.
    EXPECT_CALL(*session_, WritevData(stream_->id(), 10, 0, NO_FIN, _, _)).WillOnce(InvokeWithoutArgs([this]() {
        return session_->ConsumeData(stream_->id(), 8, 0u, NO_FIN, NOT_RETRANSMISSION, std::nullopt);
    }));
    EXPECT_FALSE(stream_->RetransmitStreamData(0, 18, true, PTO_RETRANSMISSION));

    // Retransmit [0, 18) with fin, and all is consumed.
    EXPECT_CALL(*session_, WritevData(stream_->id(), 10, 0, NO_FIN, _, _)).WillOnce(Invoke(session_.get(), &MockQuicSession::ConsumeData));
    EXPECT_CALL(*session_, WritevData(stream_->id(), 5, 13, FIN, _, _)).WillOnce(Invoke(session_.get(), &MockQuicSession::ConsumeData));
    EXPECT_TRUE(stream_->RetransmitStreamData(0, 18, true, PTO_RETRANSMISSION));

    // Retransmit [0, 8) with fin, and all is consumed.
    EXPECT_CALL(*session_, WritevData(stream_->id(), 8, 0, NO_FIN, _, _)).WillOnce(Invoke(session_.get(), &MockQuicSession::ConsumeData));
    EXPECT_CALL(*session_, WritevData(stream_->id(), 0, 18, FIN, _, _)).WillOnce(Invoke(session_.get(), &MockQuicSession::ConsumeData));
...
TEST_P(QuicStreamTest, ResetStreamOnTtlExpiresRetransmitLostData)
{
    ...
    EXPECT_CALL(*session_, WritevData(stream_->id(), 200, 0, FIN, _, _)).WillOnce(Invoke(session_.get(), &MockQuicSession::ConsumeData));
    ...
    EXPECT_CALL(*session_, WritevData(stream_->id(), 100, 0, NO_FIN, _, _)).WillOnce(Invoke(session_.get(), &MockQuicSession::ConsumeData));
    ...
    if (session_->version().UsesHttp3()) {
        EXPECT_CALL(*session_, MaybeSendStopSendingFrame(_, QuicResetStreamError::FromInternal(QUIC_STREAM_TTL_EXPIRED))).Times(1);
    }
    EXPECT_CALL(*session_, MaybeSendRstStreamFrame(_, QuicResetStreamError::FromInternal(QUIC_STREAM_TTL_EXPIRED), _)).Times(1);
    stream_->OnCanWrite();
}
...
TEST_P(QuicStreamTest, WindowUpdateForReadOnlyStream)
{
    ...
    QuicStreamId stream_id = QuicUtils::GetFirstUnidirectionalStreamId(connection_->transport_version(), Perspective::IS_CLIENT);
...
TEST_P(QuicStreamTest, EmptyStreamFrameWithNoFin)
{
    ...
    if (stream_->version().HasIetfQuicFrames()) {
        EXPECT_CALL(*connection_, CloseConnection(QUIC_EMPTY_STREAM_FRAME_NO_FIN, _, _)).Times(0);
    } else {
        EXPECT_CALL(*connection_, CloseConnection(QUIC_EMPTY_STREAM_FRAME_NO_FIN, _, _));
    }
...
TEST_P(QuicStreamTest, SendRstWithCustomIetfCode)
{
    ...
    EXPECT_CALL(*session_, MaybeSendRstStreamFrame(kTestStreamId, error, _)).Times(1);
...
TEST_P(QuicStreamTest, ResetWhenOffsetReached)
{
    ...
    char data[100];
    EXPECT_CALL(*stream_, OnDataAvailable()).WillOnce([this]() { stream_->ConsumeData(99); });
    stream_->OnStreamFrame(QuicStreamFrame(stream_->id(), false, 0, absl::string_view(data, 99)));
    EXPECT_FALSE(stream_->rst_received());
    EXPECT_FALSE(stream_->read_side_closed());
    EXPECT_CALL(*stream_, OnDataAvailable()).WillOnce([this]() { stream_->ConsumeData(1); });
    stream_->OnStreamFrame(QuicStreamFrame(stream_->id(), false, 99, absl::string_view(data + 99, 1)));
...
TEST_P(QuicStreamTest, ResetWhenOffsetReachedOutOfOrder)
{
    ...
    EXPECT_FALSE(stream_->rst_received());
    EXPECT_FALSE(stream_->read_side_closed());
    EXPECT_CALL(*stream_, OnDataAvailable()).WillOnce([this]() { stream_->ConsumeData(100); });
    stream_->OnStreamFrame(QuicStreamFrame(stream_->id(), false, 0, absl::string_view(data, 99)));
    EXPECT_TRUE(stream_->rst_received());
    EXPECT_TRUE(stream_->read_side_closed());
}
...
TEST_P(QuicStreamTest, HigherReliableSizeIgnored)
{
    ...
    QuicResetStreamAtFrame rst(0, stream_->id(), QUIC_STREAM_CANCELLED, 400, 100);
    stream_->OnResetStreamAtFrame(rst); // Nothing happens.
    QuicResetStreamAtFrame rst2(0, stream_->id(), QUIC_STREAM_CANCELLED, 400, 200);
...
TEST_P(QuicStreamTest, InstantReset)
{
    ...
    char data[100];
    EXPECT_CALL(*stream_, OnDataAvailable()).WillOnce([this]() { stream_->ConsumeData(100); });
    stream_->OnStreamFrame(QuicStreamFrame(stream_->id(), false, 0, absl::string_view(data, 100)));
    QuicResetStreamAtFrame rst(0, stream_->id(), QUIC_STREAM_CANCELLED, 400, 100);
    EXPECT_FALSE(stream_->rst_received());
    EXPECT_FALSE(stream_->read_side_closed());
...
TEST_P(QuicStreamTest, ResetIgnoredDueToFin)
{
    ...
    char data[100];
    EXPECT_CALL(*stream_, OnDataAvailable()).WillOnce([this]() { stream_->ConsumeData(98); });
    stream_->OnStreamFrame(QuicStreamFrame(stream_->id(), false, 0, absl::string_view(data, 98)));
    ...
    stream_->OnStreamFrame(QuicStreamFrame(stream_->id(), true, 98, absl::string_view(data + 98, 2)));
    EXPECT_FALSE(stream_->rst_received());
    EXPECT_TRUE(stream_->read_side_closed());
}
...
TEST_P(QuicStreamTest, ReliableOffsetBeyondFin)
{
    ...
    char data[100];
    stream_->OnStreamFrame(QuicStreamFrame(stream_->id(), true, 98, absl::string_view(data + 98, 2)));
    EXPECT_CALL(*connection_, CloseConnection(QUIC_STREAM_MULTIPLE_OFFSET, _, _)).Times(1);
...
TEST_P(QuicStreamTest, FinBeforeReliableOffset)
{
    ...
    char data[100];
    EXPECT_CALL(*connection_, CloseConnection(QUIC_STREAM_MULTIPLE_OFFSET, _, _)).Times(1);
    stream_->OnStreamFrame(QuicStreamFrame(stream_->id(), true, 0, absl::string_view(data, 100)));
}
