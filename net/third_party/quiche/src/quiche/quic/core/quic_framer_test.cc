...
{
    uint8_t connection_id_bytes[9] = { 0xFE, 0xDC, 0xBA, 0x98, 0x76, 0x54, 0x32, 0x10, 0x42 };
    return QuicConnectionId(reinterpret_cast<char*>(connection_id_bytes), sizeof(connection_id_bytes));
}

const QuicPacketNumber kPacketNumber = QuicPacketNumber(UINT64_C(0x12345678));
const QuicPacketNumber kSmallLargestObserved = QuicPacketNumber(UINT16_C(0x1234));
...
// varint62 format.
const QuicPacketNumber kLargestIetfLargestObserved = QuicPacketNumber(UINT64_C(0x3fffffffffffffff));
...
public:
    ...
    bool EncryptPacket(uint64_t packet_number, absl::string_view associated_data, absl::string_view plaintext, char* output, size_t* output_length,
...
public:
    ...
    bool DecryptPacket(uint64_t packet_number, absl::string_view associated_data, absl::string_view ciphertext, char* output, size_t* output_length,
        size_t /*max_output_length*/) override
    {
        packet_number_ = QuicPacketNumber(packet_number);
        associated_data_ = std::string(associated_data);
    ...
    std::string GenerateHeaderProtectionMask(QuicDataReader* /*sample_reader*/) override
    {
        return std::string(5, 0);
    }
    size_t GetKeySize() const override
...
std::unique_ptr<QuicEncryptedPacket> EncryptPacketWithTagAndPhase(const QuicPacket& packet, uint8_t tag, bool phase)
{
...
public:
    ...
    {
        QUIC_DLOG(INFO) << "QuicFramer Error: " << QuicErrorCodeToString(f->error()) << " (" << f->error() << ")";
        ++error_count_;
    }
    ...
    void OnVersionNegotiationPacket(const QuicVersionNegotiationPacket& packet) override
    {
        version_negotiation_packet_ = std::make_unique<QuicVersionNegotiationPacket>((packet));
        EXPECT_EQ(0u, framer_->current_received_frame_type());
    }

    void OnRetryPacket(QuicConnectionId original_connection_id, QuicConnectionId new_connection_id, absl::string_view retry_token,
        absl::string_view retry_integrity_tag, absl::string_view retry_without_tag) override
    {
        on_retry_packet_called_ = true;
        retry_original_connection_id_ = std::make_unique<QuicConnectionId>(original_connection_id);
        retry_new_connection_id_ = std::make_unique<QuicConnectionId>(new_connection_id);
        retry_token_ = std::make_unique<std::string>(std::string(retry_token));
        retry_token_integrity_tag_ = std::make_unique<std::string>(std::string(retry_integrity_tag));
        retry_without_tag_ = std::make_unique<std::string>(std::string(retry_without_tag));
        EXPECT_EQ(0u, framer_->current_received_frame_type());
    }

    bool OnProtocolVersionMismatch(ParsedQuicVersion received_version) override
    {
        QUIC_DLOG(INFO) << "QuicFramer Version Mismatch, version: " << received_version;
        ++version_mismatch_;
        EXPECT_EQ(0u, framer_->current_received_frame_type());
        return false;
    }
    ...
    void OnUndecryptablePacket(const QuicEncryptedPacket& packet, EncryptionLevel decryption_level, bool has_decryption_key) override
    {
    ...
    bool OnCryptoFrame(const QuicCryptoFrame& frame) override
    {
        ...
        crypto_frames_.push_back(std::make_unique<QuicCryptoFrame>(frame.level, frame.offset, *string_data));
        if (VersionHasIetfQuicFrames(transport_version_)) {
    ...
    bool OnAckFrameStart(QuicPacketNumber largest_acked, QuicTime::Delta ack_delay_time) override
    {
        ++frame_count_;
        ...
        if (VersionHasIetfQuicFrames(transport_version_)) {
            EXPECT_TRUE(IETF_ACK == framer_->current_received_frame_type() || IETF_ACK_ECN == framer_->current_received_frame_type()
    ...
    bool OnAckTimestamp(QuicPacketNumber packet_number, QuicTime timestamp) override
    {
        ack_frames_[ack_frames_.size() - 1]->received_packet_times.push_back(std::make_pair(packet_number, timestamp));
        if (VersionHasIetfQuicFrames(transport_version_)) {
            EXPECT_TRUE(IETF_ACK == framer_->current_received_frame_type() || IETF_ACK_ECN == framer_->current_received_frame_type()
                || IETF_ACK_RECEIVE_TIMESTAMPS == framer_->current_received_frame_type());
        } else {
    ...
    bool OnAckFrameEnd(QuicPacketNumber /*start*/, const std::optional<QuicEcnCounts>& /*ecn_counts*/) override
    {
        return true;
    }
    ...
    {
        ++frame_count_;
        stop_waiting_frames_.push_back(std::make_unique<QuicStopWaitingFrame>(frame));
        EXPECT_EQ(0u, framer_->current_received_frame_type());
        return true;
    }
    ...
    {
        ++frame_count_;
        message_frames_.push_back(std::make_unique<QuicMessageFrame>(frame.data, frame.message_length));
        if (VersionHasIetfQuicFrames(transport_version_)) {
            EXPECT_TRUE(IETF_EXTENSION_MESSAGE_NO_LENGTH_V99 == framer_->current_received_frame_type()
    ...
    {
        ++frame_count_;
        handshake_done_frames_.push_back(std::make_unique<QuicHandshakeDoneFrame>(frame));
    ...
    {
        ++frame_count_;
        ack_frequency_frames_.emplace_back(std::make_unique<QuicAckFrequencyFrame>(frame));
        QUICHE_DCHECK(VersionHasIetfQuicFrames(transport_version_));
    ...
    {
        ++frame_count_;
        reset_stream_at_frames_.push_back(std::make_unique<QuicResetStreamAtFrame>(frame));
    ...
    bool OnConnectionCloseFrame(const QuicConnectionCloseFrame& frame) override
    {
        ...
        if (VersionHasIetfQuicFrames(transport_version_)) {
            ...
            if (frame.close_type == IETF_QUIC_TRANSPORT_CONNECTION_CLOSE) {
                EXPECT_EQ(IETF_CONNECTION_CLOSE, framer_->current_received_frame_type());
            } else {
                EXPECT_EQ(IETF_APPLICATION_CLOSE, framer_->current_received_frame_type());
            }
        } else {
            EXPECT_EQ(0u, framer_->current_received_frame_type());
        }
    ...
    bool OnMaxStreamsFrame(const QuicMaxStreamsFrame& frame) override
    {
        ...
        EXPECT_TRUE(VersionHasIetfQuicFrames(transport_version_));
        EXPECT_TRUE(IETF_MAX_STREAMS_UNIDIRECTIONAL == framer_->current_received_frame_type()
    ...
    bool OnStreamsBlockedFrame(const QuicStreamsBlockedFrame& frame) override
    {
        ...
        EXPECT_TRUE(VersionHasIetfQuicFrames(transport_version_));
        EXPECT_TRUE(IETF_STREAMS_BLOCKED_UNIDIRECTIONAL == framer_->current_received_frame_type()
            || IETF_STREAMS_BLOCKED_BIDIRECTIONAL == framer_->current_received_frame_type());
        return true;
    }
    ...
    bool OnWindowUpdateFrame(const QuicWindowUpdateFrame& frame) override
    {
        ...
        if (VersionHasIetfQuicFrames(transport_version_)) {
            EXPECT_TRUE(IETF_MAX_DATA == framer_->current_received_frame_type() || IETF_MAX_STREAM_DATA == framer_->current_received_frame_type());
        } else {
            EXPECT_EQ(0u, framer_->current_received_frame_type());
        }
        return true;
    ...
    bool OnBlockedFrame(const QuicBlockedFrame& frame) override
    {
        ...
        if (VersionHasIetfQuicFrames(transport_version_)) {
            EXPECT_TRUE(IETF_DATA_BLOCKED == framer_->current_received_frame_type() || IETF_STREAM_DATA_BLOCKED == framer_->current_received_frame_type());
        } else {
            EXPECT_EQ(0u, framer_->current_received_frame_type());
        }
        return true;
    ...
    bool OnRetireConnectionIdFrame(const QuicRetireConnectionIdFrame& frame) override
    {
        EXPECT_EQ(IETF_RETIRE_CONNECTION_ID, framer_->current_received_frame_type());
        EXPECT_TRUE(VersionHasIetfQuicFrames(transport_version_));
    ...
    void OnAuthenticatedIetfStatelessResetPacket(const QuicIetfStatelessResetPacket& packet) override
    {
        stateless_reset_packet_ = std::make_unique<QuicIetfStatelessResetPacket>(packet);
        EXPECT_EQ(0u, framer_->current_received_frame_type());
    }
    ...
    std::unique_ptr<QuicDecrypter> AdvanceKeysAndCreateCurrentOneRttDecrypter() override
    {
    ...
    size_t key_update_count() const
    {
        return key_update_reasons_.size();
    }
...
public:
    QuicFramerTest()
        ...
        , framer_(AllSupportedVersions(), start_, Perspective::IS_SERVER, kQuicDefaultConnectionIdLength)
    {
        framer_.set_version(version_);
        if (framer_.version().KnowsWhichDecrypterToUse()) {
            framer_.InstallDecrypter(ENCRYPTION_INITIAL, std::unique_ptr<QuicDecrypter>(decrypter_));
        } else {
            framer_.SetDecrypter(ENCRYPTION_INITIAL, std::unique_ptr<QuicDecrypter>(decrypter_));
        }
        framer_.SetEncrypter(ENCRYPTION_INITIAL, std::unique_ptr<QuicEncrypter>(encrypter_));
    ...
    bool CheckEncryption(QuicPacketNumber packet_number, QuicPacket* packet)
    {
        if (packet_number != encrypter_->packet_number_) {
            QUIC_LOG(ERROR) << "Encrypted incorrect packet number.  expected " << packet_number << " actual: " << encrypter_->packet_number_;
            return false;
        }
        if (packet->AssociatedData(framer_.transport_version()) != encrypter_->associated_data_) {
            QUIC_LOG(ERROR) << "Encrypted incorrect associated data.  expected " << packet->AssociatedData(framer_.transport_version())
                            << " actual: " << encrypter_->associated_data_;
            return false;
        }
        if (packet->Plaintext(framer_.transport_version()) != encrypter_->plaintext_) {
            QUIC_LOG(ERROR) << "Encrypted incorrect plaintext data.  expected " << packet->Plaintext(framer_.transport_version())
    ...
    bool CheckDecryption(const QuicEncryptedPacket& encrypted, bool includes_version, bool includes_diversification_nonce,
        uint8_t destination_connection_id_length, uint8_t source_connection_id_length)
    {
        return CheckDecryption(encrypted, includes_version, includes_diversification_nonce, destination_connection_id_length, source_connection_id_length,
            quiche::VARIABLE_LENGTH_INTEGER_LENGTH_0, 0, quiche::VARIABLE_LENGTH_INTEGER_LENGTH_0);
    }

    bool CheckDecryption(const QuicEncryptedPacket& encrypted, bool includes_version, bool includes_diversification_nonce,
        uint8_t destination_connection_id_length, uint8_t source_connection_id_length, quiche::QuicheVariableLengthIntegerLength retry_token_length_length,
        size_t retry_token_length, quiche::QuicheVariableLengthIntegerLength length_length)
    {
        if (visitor_.header_->packet_number != decrypter_->packet_number_) {
            QUIC_LOG(ERROR) << "Decrypted incorrect packet number.  expected " << visitor_.header_->packet_number << " actual: " << decrypter_->packet_number_;
            return false;
        }
        absl::string_view associated_data = QuicFramer::GetAssociatedDataFromEncryptedPacket(framer_.transport_version(), encrypted,
            destination_connection_id_length, source_connection_id_length, includes_version, includes_diversification_nonce, PACKET_4BYTE_PACKET_NUMBER,
            retry_token_length_length, retry_token_length, length_length);
        if (associated_data != decrypter_->associated_data_) {
            QUIC_LOG(ERROR) << "Decrypted incorrect associated data.  expected " << absl::BytesToHexString(associated_data)
                            << " actual: " << absl::BytesToHexString(decrypter_->associated_data_);
            return false;
        }
        absl::string_view ciphertext(
            encrypted.AsStringPiece().substr(GetStartOfEncryptedData(framer_.transport_version(), destination_connection_id_length, source_connection_id_length,
                includes_version, includes_diversification_nonce, PACKET_4BYTE_PACKET_NUMBER, retry_token_length_length, retry_token_length, length_length)));
        if (ciphertext != decrypter_->ciphertext_) {
            QUIC_LOG(ERROR) << "Decrypted incorrect ciphertext data.  expected " << absl::BytesToHexString(ciphertext)
                            << " actual: " << absl::BytesToHexString(decrypter_->ciphertext_)
                            << " associated data: " << absl::BytesToHexString(associated_data);
            return false;
        }
        return true;
    }
    ...
    std::unique_ptr<QuicEncryptedPacket> AssemblePacketFromFragments(const PacketFragments& fragments)
    {
    ...
    void CheckFramingBoundaries(const PacketFragments& fragments, QuicErrorCode error_code)
    {
        std::unique_ptr<QuicEncryptedPacket> packet(AssemblePacketFromFragments(fragments));
    ...
    void CheckProcessingFails(const QuicEncryptedPacket& packet, size_t len, std::string expected_error, QuicErrorCode error_code)
    {
    ...
    void CheckProcessingFails(unsigned char* packet, size_t len, std::string expected_error, QuicErrorCode error_code)
    {
    ...
    void CheckCalculatePacketNumber(uint64_t expected_packet_number, QuicPacketNumber last_packet_number)
    {
        uint64_t wire_packet_number = expected_packet_number & kMask;
        EXPECT_EQ(
            expected_packet_number, QuicFramerPeer::CalculatePacketNumberFromWire(&framer_, PACKET_4BYTE_PACKET_NUMBER, last_packet_number, wire_packet_number))
            << "last_packet_number: " << last_packet_number << " wire_packet_number: " << wire_packet_number;
    }

    std::unique_ptr<QuicPacket> BuildDataPacket(const QuicPacketHeader& header, const QuicFrames& frames)
    {
        return BuildUnsizedDataPacket(&framer_, header, frames);
    }

    std::unique_ptr<QuicPacket> BuildDataPacket(const QuicPacketHeader& header, const QuicFrames& frames, size_t packet_size)
    {
        return BuildUnsizedDataPacket(&framer_, header, frames, packet_size);
    }

    // N starts at 1.
    QuicStreamId GetNthStreamid(QuicTransportVersion transport_version, Perspective perspective, bool bidirectional, int n)
    {
        if (bidirectional) {
            return QuicUtils::GetFirstBidirectionalStreamId(transport_version, perspective) + ((n - 1) * QuicUtils::StreamIdDelta(transport_version));
        }
        // Unidirectional
        return QuicUtils::GetFirstUnidirectionalStreamId(transport_version, perspective) + ((n - 1) * QuicUtils::StreamIdDelta(transport_version));
    }

    QuicTime CreationTimePlus(uint64_t offset_us)
    {
        return framer_.creation_time() + QuicTime::Delta::FromMicroseconds(offset_us);
    }
...
#define QUIC_VERSION_BYTES GetQuicVersionByte(0), GetQuicVersionByte(1), GetQuicVersionByte(2), GetQuicVersionByte(3)

// Run all framer tests with all supported versions of QUIC.
INSTANTIATE_TEST_SUITE_P(QuicFramerTests, QuicFramerTest, ::testing::ValuesIn(AllSupportedVersions()), ::testing::PrintToStringParamName());
...
TEST_P(QuicFramerTest, CalculatePacketNumberFromWireNearEpochStart)
{
    ...
    for (QuicPacketNumber last = QuicPacketNumber(1); last < QuicPacketNumber(10); last++) {
...
TEST_P(QuicFramerTest, LargePacket)
{
    ...
    const size_t header_size = GetPacketHeaderSize(framer_.transport_version(), kPacket8ByteConnectionId, kPacket0ByteConnectionId, !kIncludeVersion,
        !kIncludeDiversificationNonce, PACKET_4BYTE_PACKET_NUMBER, quiche::VARIABLE_LENGTH_INTEGER_LENGTH_0, 0, quiche::VARIABLE_LENGTH_INTEGER_LENGTH_0);
    ...
    EXPECT_EQ(FramerTestConnectionId(), visitor_.header_->destination_connection_id);
...
TEST_P(QuicFramerTest, LongPacketHeader)
{
  ...
  PacketFragments packet = {
    ...
    std::unique_ptr<QuicEncryptedPacket> encrypted(AssemblePacketFromFragments(packet));
    ...
    const QuicErrorCode error_code = QuicFramer::ParsePublicHeaderDispatcher(*encrypted, kQuicDefaultConnectionIdLength, &format, &long_packet_type,
        &version_flag, &use_length_prefix, &version_label, &parsed_version, &destination_connection_id, &source_connection_id, &retry_token, &detailed_error);
...
TEST_P(QuicFramerTest, LongPacketHeaderWithBothConnectionIds)
{
  ...
  unsigned char packet49[] = {
    ...
    EXPECT_THAT(error_code, IsQuicNoError());
    EXPECT_FALSE(retry_token.has_value());
    EXPECT_EQ(framer_.version().HasLengthPrefixedConnectionIds(), use_length_prefix);
    EXPECT_EQ("", detailed_error);
    EXPECT_EQ(IETF_QUIC_LONG_HEADER_PACKET, format);
    EXPECT_TRUE(version_flag);
    EXPECT_EQ(FramerTestConnectionId(), destination_connection_id);
...
TEST_P(QuicFramerTest, ParsePublicHeader)
{
  ...
  unsigned char packet49[] = {
    ...
    QuicVersionLabel version_label = 0;
    ParsedQuicVersion parsed_version = UnsupportedQuicVersion();
    QuicConnectionId destination_connection_id = EmptyQuicConnectionId(), source_connection_id = EmptyQuicConnectionId();
    QuicLongHeaderType long_packet_type = INVALID_PACKET_TYPE;
    quiche::QuicheVariableLengthIntegerLength retry_token_length_length = quiche::VARIABLE_LENGTH_INTEGER_LENGTH_4;
    absl::string_view retry_token;
    std::string detailed_error = "foobar";

    QuicDataReader reader(AsChars(p), p_length);
    const QuicErrorCode parse_error = QuicFramer::ParsePublicHeader(&reader, kQuicDefaultConnectionIdLength, /*ietf_format=*/true, &first_byte, &format,
        &version_present, &has_length_prefix, &version_label, &parsed_version, &destination_connection_id, &source_connection_id, &long_packet_type,
    ...
    EXPECT_EQ(framer_.version().HasLengthPrefixedConnectionIds(), has_length_prefix);
    EXPECT_EQ(CreateQuicVersionLabel(framer_.version()), version_label);
    EXPECT_EQ(framer_.version(), parsed_version);
    EXPECT_EQ(FramerTestConnectionId(), destination_connection_id);
    EXPECT_EQ(EmptyQuicConnectionId(), source_connection_id);
    EXPECT_EQ(quiche::VARIABLE_LENGTH_INTEGER_LENGTH_0, retry_token_length_length);
...
TEST_P(QuicFramerTest, ParsePublicHeaderProxBadSourceConnectionIdLength)
{
  ...
  unsigned char packet[] = {
    ...
    QuicDataReader reader(AsChars(p), p_length);
    const QuicErrorCode parse_error = QuicFramer::ParsePublicHeader(&reader, kQuicDefaultConnectionIdLength,
        /*ietf_format=*/true, &first_byte, &format, &version_present, &has_length_prefix, &version_label, &parsed_version, &destination_connection_id,
        &source_connection_id, &long_packet_type, &retry_token_length_length, &retry_token, &detailed_error);
    EXPECT_THAT(parse_error, IsQuicNoError());
    EXPECT_EQ("", detailed_error);
    EXPECT_EQ(p[0], first_byte);
    EXPECT_TRUE(version_present);
...
TEST_P(QuicFramerTest, ClientConnectionIdFromShortHeaderToClient)
{
    ...
    SetDecrypterLevel(ENCRYPTION_FORWARD_SECURE);
    QuicFramerPeer::SetLastSerializedServerConnectionId(&framer_, TestConnectionId(0x33));
...
{
    SetDecrypterLevel(ENCRYPTION_FORWARD_SECURE);
    QuicFramerPeer::SetLastSerializedServerConnectionId(&framer_, FramerTestConnectionId());
    QuicFramerPeer::SetPerspective(&framer_, Perspective::IS_CLIENT);
  ...
  PacketFragments packet_hp = {
    ...
    PacketFragments& fragments = framer_.version().HasHeaderProtection() ? packet_hp : packet;
    std::unique_ptr<QuicEncryptedPacket> encrypted(AssemblePacketFromFragments(fragments));
    EXPECT_FALSE(framer_.ProcessPacket(*encrypted));
    EXPECT_THAT(framer_.error(), IsError(QUIC_MISSING_PAYLOAD));
    ASSERT_TRUE(visitor_.header_.get());
    EXPECT_FALSE(visitor_.header_->reset_flag);
...
TEST_P(QuicFramerTest, PacketHeaderWithVersionFlag)
{
  ...
  PacketFragments packet49 = {
    ...
    ReviseFirstByteByVersion(packet49);
    PacketFragments& fragments = framer_.version().HasLongHeaderLengths() ? packet49 : packet;
    std::unique_ptr<QuicEncryptedPacket> encrypted(AssemblePacketFromFragments(fragments));
    EXPECT_FALSE(framer_.ProcessPacket(*encrypted));
    EXPECT_THAT(framer_.error(), IsError(QUIC_MISSING_PAYLOAD));
    ASSERT_TRUE(visitor_.header_.get());
...
TEST_P(QuicFramerTest, PaddingFrame)
{
  ...
  unsigned char packet_ietf[] = {
    ...
    EXPECT_THAT(framer_.error(), IsQuicNoError());
    ASSERT_TRUE(visitor_.header_.get());
    EXPECT_TRUE(CheckDecryption(encrypted, !kIncludeVersion, !kIncludeDiversificationNonce, kPacket8ByteConnectionId, kPacket0ByteConnectionId));
...
TEST_P(QuicFramerTest, StreamFrame)
{
  ...
  PacketFragments packet_ietf = {
    ...
    PacketFragments& fragments = VersionHasIetfQuicFrames(framer_.transport_version()) ? packet_ietf : packet;
    std::unique_ptr<QuicEncryptedPacket> encrypted(AssemblePacketFromFragments(fragments));
    EXPECT_TRUE(framer_.ProcessPacket(*encrypted));
...
TEST_P(QuicFramerTest, MissingDiversificationNonce)
{
    ...
    decrypter_ = new test::TestDecrypter();
    if (framer_.version().KnowsWhichDecrypterToUse()) {
        framer_.InstallDecrypter(ENCRYPTION_INITIAL, std::make_unique<NullDecrypter>(Perspective::IS_CLIENT));
        framer_.InstallDecrypter(ENCRYPTION_ZERO_RTT, std::unique_ptr<QuicDecrypter>(decrypter_));
    } else {
        framer_.SetDecrypter(ENCRYPTION_INITIAL, std::make_unique<NullDecrypter>(Perspective::IS_CLIENT));
        framer_.SetAlternativeDecrypter(ENCRYPTION_ZERO_RTT, std::unique_ptr<QuicDecrypter>(decrypter_), false);
    }
  ...
  unsigned char packet49[] = {
    ...
    if (framer_.version().HasHeaderProtection()) {
        ...
        EXPECT_EQ("Unable to decrypt ENCRYPTION_ZERO_RTT header protection.", framer_.detailed_error());
    } else {
...
TEST_P(QuicFramerTest, StreamFrameWithVersion)
{
  ...
  PacketFragments packet_ietf = {
    ...
    quiche::QuicheVariableLengthIntegerLength length_length
        = QuicVersionHasLongHeaderLengths(framer_.transport_version()) ? quiche::VARIABLE_LENGTH_INTEGER_LENGTH_1 : quiche::VARIABLE_LENGTH_INTEGER_LENGTH_0;

    ReviseFirstByteByVersion(packet_ietf);
    PacketFragments& fragments
        = VersionHasIetfQuicFrames(framer_.transport_version()) ? packet_ietf : (framer_.version().HasLongHeaderLengths() ? packet49 : packet);
    std::unique_ptr<QuicEncryptedPacket> encrypted(AssemblePacketFromFragments(fragments));
    EXPECT_TRUE(framer_.ProcessPacket(*encrypted));
    ...
    CheckFramingBoundaries(fragments, framer_.version().HasLongHeaderLengths() ? QUIC_INVALID_PACKET_HEADER : QUIC_INVALID_STREAM_DATA);
}
...
TEST_P(QuicFramerTest, ThirdAckBlockUnderflowGap)
{
  ...
  PacketFragments packet_ietf = {
    ...
    std::unique_ptr<QuicEncryptedPacket> encrypted(AssemblePacketFromFragments(packet_ietf));
    EXPECT_FALSE(framer_.ProcessPacket(*encrypted));
    EXPECT_EQ(framer_.detailed_error(), "Underflow with gap block length 30 previous ack block start is 30.");
...
TEST_P(QuicFramerTest, ThirdAckBlockUnderflowAck)
{
  ...
  PacketFragments packet_ietf = {
    ...
    std::unique_ptr<QuicEncryptedPacket> encrypted(AssemblePacketFromFragments(packet_ietf));
    EXPECT_FALSE(framer_.ProcessPacket(*encrypted));
    EXPECT_EQ(framer_.detailed_error(), "Underflow with ack block length 31 latest ack block end is 25.");
    CheckFramingBoundaries(packet_ietf, QUIC_INVALID_ACK_DATA);
}
...
TEST_P(QuicFramerTest, AckBlockUnderflowGapWrap)
{
  ...
  PacketFragments packet_ietf = {
    ...
    std::unique_ptr<QuicEncryptedPacket> encrypted(AssemblePacketFromFragments(packet_ietf));
    EXPECT_FALSE(framer_.ProcessPacket(*encrypted));
    EXPECT_EQ(framer_.detailed_error(), "Underflow with gap block length 2 previous ack block start is 1.");
    CheckFramingBoundaries(packet_ietf, QUIC_INVALID_ACK_DATA);
}
...
TEST_P(QuicFramerTest, AckBlockUnderflowAckWrap)
{
  ...
  PacketFragments packet_ietf = {
    ...
    std::unique_ptr<QuicEncryptedPacket> encrypted(AssemblePacketFromFragments(packet_ietf));
    EXPECT_FALSE(framer_.ProcessPacket(*encrypted));
    EXPECT_EQ(framer_.detailed_error(), "Underflow with ack block length 10 latest ack block end is 1.");
    CheckFramingBoundaries(packet_ietf, QUIC_INVALID_ACK_DATA);
}
...
TEST_P(QuicFramerTest, AckBlockAcksEverything)
{
  ...
  PacketFragments packet_ietf = {
    ...
    EXPECT_EQ(kLargestIetfLargestObserved.ToUint64(), frame.packets.NumPacketsSlow());
}
...
TEST_P(QuicFramerTest, AckFrameReceiveTimestampGapTooHigh)
{
  ...
  PacketFragments packet_ietf = {
    ...
    framer_.set_process_timestamps(true);
    EXPECT_FALSE(framer_.ProcessPacket(*encrypted));
    EXPECT_TRUE(absl::StartsWith(framer_.detailed_error(), "Receive timestamp gap too high."));
}
...
TEST_P(QuicFramerTest, AckFrameReceiveTimestampCountTooHigh)
{
  ...
  PacketFragments packet_ietf = {
    ...
    framer_.set_process_timestamps(true);
    EXPECT_FALSE(framer_.ProcessPacket(*encrypted));
    EXPECT_TRUE(absl::StartsWith(framer_.detailed_error(), "Receive timestamp delta too high."));
}
...
TEST_P(QuicFramerTest, AckFrameReceiveTimestampDeltaTooHigh)
{
  ...
  PacketFragments packet_ietf = {
    ...
    framer_.set_process_timestamps(true);
    EXPECT_FALSE(framer_.ProcessPacket(*encrypted));
    EXPECT_TRUE(absl::StartsWith(framer_.detailed_error(), "Receive timestamp count too high."));
}
...
TEST_P(QuicFramerTest, AckFrameTimeStampDeltaTooHigh)
{
  ...
  unsigned char packet[] = {
    ...
    QuicEncryptedPacket encrypted(AsChars(packet), ABSL_ARRAYSIZE(packet), false);
    EXPECT_FALSE(framer_.ProcessPacket(encrypted));
    EXPECT_TRUE(absl::StartsWith(framer_.detailed_error(), "delta_from_largest_observed too high"));
}
...
TEST_P(QuicFramerTest, ConnectionCloseFrame)
{
  ...
  PacketFragments packet_ietf = {
    ...
    EXPECT_EQ(0u, visitor_.stream_frames_.size());
    EXPECT_EQ(0x11u, static_cast<unsigned>(visitor_.connection_close_frame_.wire_error_code));
    EXPECT_EQ("because I can", visitor_.connection_close_frame_.error_details);
    if (VersionHasIetfQuicFrames(framer_.transport_version())) {
        EXPECT_EQ(0x1234u, visitor_.connection_close_frame_.transport_close_frame_type);
        ...
        // same value.
        EXPECT_EQ(0x11u, static_cast<unsigned>(visitor_.connection_close_frame_.quic_error_code));
    }
...
TEST_P(QuicFramerTest, ConnectionCloseFrameWithExtractedInfoIgnoreGCuic)
{
  ...
  PacketFragments packet_ietf = {
    ...
    if (VersionHasIetfQuicFrames(framer_.transport_version())) {
        ...
        EXPECT_EQ("17767:because I can", visitor_.connection_close_frame_.error_details);
    }
...
TEST_P(QuicFramerTest, ApplicationCloseFrame)
{
  ...
  PacketFragments packet_ietf = {
    ...
    EXPECT_EQ(IETF_QUIC_APPLICATION_CONNECTION_CLOSE, visitor_.connection_close_frame_.close_type);
...
TEST_P(QuicFramerTest, MaxDataFrame)
{
  ...
  PacketFragments packet_ietf = {
    ...
    EXPECT_EQ(QuicUtils::GetInvalidStreamId(framer_.transport_version()), visitor_.window_update_frame_.stream_id);
    EXPECT_EQ(kStreamOffset, visitor_.window_update_frame_.max_data);
...
TEST_P(QuicFramerTest, PingFrame)
{
  ...
  unsigned char packet_ietf[] = {
    ...
    QuicEncryptedPacket encrypted(AsChars(VersionHasIetfQuicFrames(framer_.transport_version()) ? packet_ietf : packet),
        VersionHasIetfQuicFrames(framer_.transport_version()) ? ABSL_ARRAYSIZE(packet_ietf) : ABSL_ARRAYSIZE(packet), false);
    EXPECT_TRUE(framer_.ProcessPacket(encrypted));
...
TEST_P(QuicFramerTest, IetfStatelessResetPacket)
{
  ...
  unsigned char packet[] = {
    ...
    EXPECT_EQ(kTestStatelessResetToken, visitor_.stateless_reset_packet_->stateless_reset_token);
}
...
TEST_P(QuicFramerTest, VersionNegotiationPacketServer)
{
  ...
  unsigned char packet2[] = {
    ...
    QuicEncryptedPacket encrypted(AsChars(p), p_length, false);
    EXPECT_FALSE(framer_.ProcessPacket(encrypted));
    EXPECT_THAT(framer_.error(), IsError(QUIC_INVALID_VERSION_NEGOTIATION_PACKET));
    EXPECT_EQ("Server received version negotiation packet.", framer_.detailed_error());
...
TEST_P(QuicFramerTest, ParseIetfRetryPacket)
{
  ...
  unsigned char packet_with_tag[] = {
    ...
    if (framer_.version().UsesTls()) {
        ...
        quiche::test::CompareCharArraysWithHexError("retry integrity tag", visitor_.retry_token_integrity_tag_->data(),
            visitor_.retry_token_integrity_tag_->length(), reinterpret_cast<const char*>(expected_integrity_tag), ABSL_ARRAYSIZE(expected_integrity_tag));
        ASSERT_TRUE(visitor_.retry_without_tag_.get());
        quiche::test::CompareCharArraysWithHexError("retry without tag", visitor_.retry_without_tag_->data(), visitor_.retry_without_tag_->length(),
            reinterpret_cast<const char*>(packet_with_tag), 35);
    } else {
        ASSERT_TRUE(visitor_.retry_original_connection_id_.get());
        EXPECT_EQ(FramerTestConnectionId(), *visitor_.retry_original_connection_id_.get());
    }

    EXPECT_EQ(FramerTestConnectionIdPlusOne(), *visitor_.retry_new_connection_id_.get());
...
TEST_P(QuicFramerTest, BuildPaddingFramePacket)
{
  ...
  unsigned char packet_ietf[kMaxOutgoingPacketSize] = {
    ...
    uint64_t header_size = GetPacketHeaderSize(framer_.transport_version(), kPacket8ByteConnectionId, kPacket0ByteConnectionId, !kIncludeVersion,
        !kIncludeDiversificationNonce, PACKET_4BYTE_PACKET_NUMBER, quiche::VARIABLE_LENGTH_INTEGER_LENGTH_0, 0, quiche::VARIABLE_LENGTH_INTEGER_LENGTH_0);
    ...
    quiche::test::CompareCharArraysWithHexError("constructed packet", data->data(), data->length(), AsChars(p), ABSL_ARRAYSIZE(packet));
}
...
TEST_P(QuicFramerTest, BuildStreamFramePacketWithNewPaddingFrame)
{
    ...
    header.version_flag = false;
    header.packet_number = kPacketNumber;
    QuicStreamFrame stream_frame(kStreamId, true, kStreamOffset, absl::string_view("hello world!"));
    QuicPaddingFrame padding_frame(2);
    QuicFrames frames = { QuicFrame(padding_frame), QuicFrame(stream_frame), QuicFrame(padding_frame) };
  ...
  unsigned char packet_ietf[] = {
    ...
    quiche::test::CompareCharArraysWithHexError("constructed packet", data->data(), data->length(), AsChars(p), p_size);
}
...
TEST_P(QuicFramerTest, Build2ByteSequenceNumberPaddingFramePacket)
{
  ...
  unsigned char packet_ietf[kMaxOutgoingPacketSize] = {
    ...
    uint64_t header_size = GetPacketHeaderSize(framer_.transport_version(), kPacket8ByteConnectionId, kPacket0ByteConnectionId, !kIncludeVersion,
        !kIncludeDiversificationNonce, PACKET_2BYTE_PACKET_NUMBER, quiche::VARIABLE_LENGTH_INTEGER_LENGTH_0, 0, quiche::VARIABLE_LENGTH_INTEGER_LENGTH_0);
    memset(p + header_size + 1, 0x00, kMaxOutgoingPacketSize - header_size - 1);
...
TEST_P(QuicFramerTest, Build1ByteSequenceNumberPaddingFramePacket)
{
  ...
  unsigned char packet_ietf[kMaxOutgoingPacketSize] = {
    ...
    uint64_t header_size = GetPacketHeaderSize(framer_.transport_version(), kPacket8ByteConnectionId, kPacket0ByteConnectionId, !kIncludeVersion,
        !kIncludeDiversificationNonce, PACKET_1BYTE_PACKET_NUMBER, quiche::VARIABLE_LENGTH_INTEGER_LENGTH_0, 0, quiche::VARIABLE_LENGTH_INTEGER_LENGTH_0);
    memset(p + header_size + 1, 0x00, kMaxOutgoingPacketSize - header_size - 1);
...
TEST_P(QuicFramerTest, BuildCryptoFramePacket)
{
    ...
    QuicCryptoFrame crypto_frame(ENCRYPTION_INITIAL, kStreamOffset, crypto_frame_contents.length());
    data_producer.SaveCryptoData(ENCRYPTION_INITIAL, kStreamOffset, crypto_frame_contents);
  ...
  unsigned char packet_ietf[] = {
    ...
    std::unique_ptr<QuicPacket> data(BuildDataPacket(header, frames));
    ASSERT_TRUE(data != nullptr);
    quiche::test::CompareCharArraysWithHexError("constructed packet", data->data(), data->length(), AsChars(packet), packet_size);
}
...
TEST_P(QuicFramerTest, CryptoFrame)
{
  ...
  PacketFragments packet_ietf = {
    ...
    PacketFragments& fragments = framer_.version().HasIetfQuicFrames() ? packet_ietf : packet48;
    std::unique_ptr<QuicEncryptedPacket> encrypted(AssemblePacketFromFragments(fragments));
    EXPECT_TRUE(framer_.ProcessPacket(*encrypted));
    ...
    EXPECT_EQ("hello world!", std::string(frame->data_buffer, frame->data_length));
...
TEST_P(QuicFramerTest, BuildVersionNegotiationPacket) {
  ...
  unsigned char packet49[] = {
    ...
    QuicConnectionId connection_id = FramerTestConnectionId();
    std::unique_ptr<QuicEncryptedPacket> data(QuicFramer::BuildVersionNegotiationPacket(
        connection_id, EmptyQuicConnectionId(), /*ietf_quic=*/true, framer_.version().HasLengthPrefixedConnectionIds(), SupportedVersions(GetParam())));
    quiche::test::CompareCharArraysWithHexError("constructed packet", data->data(), data->length(), AsChars(p), p_size);
}
...
TEST_P(QuicFramerTest, BuildVersionNegotiationPacketWithClientConnectionId)
{
  ...
  unsigned char packet[] = {
    ...
    std::unique_ptr<QuicEncryptedPacket> data(
        QuicFramer::BuildVersionNegotiationPacket(server_connection_id, client_connection_id, true, true, SupportedVersions(GetParam())));
    quiche::test::CompareCharArraysWithHexError("constructed packet", data->data(), data->length(), AsChars(packet), ABSL_ARRAYSIZE(packet));
}
...
TEST_P(QuicFramerTest, BuildAckReceiveTimestampsFrameMultipleRanges)
{
    ...
    std::unique_ptr<QuicPacket> data(BuildDataPacket(header, frames));
    ASSERT_TRUE(data != nullptr);
    quiche::test::CompareCharArraysWithHexError("constructed packet", data->data(), data->length(), AsChars(packet_ietf), ABSL_ARRAYSIZE(packet_ietf));
}
...
TEST_P(QuicFramerTest, BuildAndProcessAckReceiveTimestampsWithMultipleRanges)
{
    ...
    framer_.InstallDecrypter(ENCRYPTION_FORWARD_SECURE, std::make_unique<StrictTaggingDecrypter>(/*key=*/0));
    ...
    std::unique_ptr<QuicPacket> data(BuildDataPacket(header, frames));
    ASSERT_TRUE(data != nullptr);
    std::unique_ptr<QuicEncryptedPacket> encrypted(EncryptPacketWithTagAndPhase(*data, 0, false));
...
TEST_P(QuicFramerTest, BuildAndProcessAckReceiveTimestampsExceedsMaxTimestamps)
{
    if (!VersionHasIetfQuicFrames(framer_.transport_version())) {
        return;
    }
...
TEST_P(QuicFramerTest, BuildAndProcessAckReceiveTimestampsWithExponentNoTruncation)
{
    if (!VersionHasIetfQuicFrames(framer_.transport_version())) {
        return;
    }
...
TEST_P(QuicFramerTest, BuildAndProcessAckReceiveTimestampsWithExponentTruncation)
{
    if (!VersionHasIetfQuicFrames(framer_.transport_version())) {
        return;
    }
...
TEST_P(QuicFramerTest, AckReceiveTimestampsPacketOutOfOrder)
{
    ...
    EXPECT_QUIC_BUG(BuildDataPacket(header, frames), "Packet number and/or receive time not in order.");
}
...
TEST_P(QuicFramerTest, IetfAckReceiveTimestampsTruncate)
{
    ...
    for (QuicPacketNumber i(1); i <= kSmallLargestObserved; i += 2) {
        ack_frame.received_packet_times.push_back({ i, CreationTimePlus((0x29ff << 3)) });
    }
...
TEST_P(QuicFramerTest, IetfAckReceiveTimestampsAckRangeTruncation)
{
    ...
    size_t encrypted_length = framer_.EncryptPayload(ENCRYPTION_INITIAL, header.packet_number, *raw_ack_packet, buffer, kMaxOutgoingPacketSize);
    ASSERT_NE(0u, encrypted_length);
    // Now make sure we can turn our ack packet back into an ack frame.
    QuicFramerPeer::SetPerspective(&framer_, Perspective::IS_SERVER);
    ASSERT_TRUE(framer_.ProcessPacket(QuicEncryptedPacket(buffer, encrypted_length, false)));
    ASSERT_EQ(1u, visitor_.ack_frames_.size());
    QuicAckFrame& processed_ack_frame = *visitor_.ack_frames_[0];
    EXPECT_EQ(QuicPacketNumber(kMaxIetfVarInt), LargestAcked(processed_ack_frame));
    // Verify ACK ranges in the frame gets truncated.
    ASSERT_LT(processed_ack_frame.packets.NumPacketsSlow(), ack_frame.packets.NumIntervals());
    EXPECT_EQ(158u, processed_ack_frame.packets.NumPacketsSlow());
    EXPECT_LT(processed_ack_frame.packets.NumIntervals(), ack_frame.packets.NumIntervals());
    EXPECT_EQ(QuicPacketNumber(kMaxIetfVarInt), processed_ack_frame.packets.Max());
...
TEST_P(QuicFramerTest, BuildAckFramePacketMultipleAckBlocks)
{
    ...
    // Use kSmallLargestObserved to make this test finished in a short time.
    QuicAckFrame ack_frame = InitAckFrame({ { QuicPacketNumber(1), QuicPacketNumber(5) }, { QuicPacketNumber(10), QuicPacketNumber(500) },
        { QuicPacketNumber(900), kSmallMissingPacket }, { kSmallMissingPacket + 1, kSmallLargestObserved + 1 } });
    ack_frame.ack_delay_time = QuicTime::Delta::Zero();
...
TEST_P(QuicFramerTest, BuildCloseFramePacket)
{
    ...
    QuicConnectionCloseFrame close_frame(framer_.transport_version(), QUIC_INTERNAL_ERROR, NO_IETF_QUIC_ERROR, "because I can", 0x05);
...
TEST_P(QuicFramerTest, BuildCloseFramePacketExtendedInfo)
{
    ...
    QuicConnectionCloseFrame close_frame(framer_.transport_version(),
        static_cast<QuicErrorCode>(VersionHasIetfQuicFrames(framer_.transport_version()) ? 0x01 : 0x05060708), NO_IETF_QUIC_ERROR, "because I can", 0x05);
...
TEST_P(QuicFramerTest, BuildTruncatedCloseFramePacket)
{
    ...
    QuicConnectionCloseFrame close_frame(framer_.transport_version(), QUIC_INTERNAL_ERROR, NO_IETF_QUIC_ERROR, std::string(2048, 'A'), 0x05);
    QuicFrames frames = { QuicFrame(&close_frame) };
...
TEST_P(QuicFramerTest, BuildMaxDataPacket)
{
    ...
    QuicWindowUpdateFrame window_update_frame;
    window_update_frame.stream_id = QuicUtils::GetInvalidStreamId(framer_.transport_version());
    window_update_frame.max_data = 0x1122334455667788;
...
TEST_P(QuicFramerTest, BuildBlockedPacket)
{
    ...
    if (VersionHasIetfQuicFrames(framer_.transport_version())) {
        ...
        blocked_frame.stream_id = QuicUtils::GetInvalidStreamId(framer_.transport_version());
    } else {
...
TEST_P(QuicFramerTest, BuildPublicResetPacket)
{
  ...
  unsigned char packet[] = {
    ...
    std::unique_ptr<QuicEncryptedPacket> data(framer_.BuildPublicResetPacket(reset_packet));
    ASSERT_TRUE(data != nullptr);
    quiche::test::CompareCharArraysWithHexError("constructed packet", data->data(), data->length(), AsChars(packet), ABSL_ARRAYSIZE(packet));
}
...
TEST_P(QuicFramerTest, BuildPublicResetPacketWithClientAddress)
{
    ...
    reset_packet.connection_id = FramerTestConnectionId();
    reset_packet.nonce_proof = kNonceProof;
    reset_packet.client_address = QuicSocketAddress(QuicIpAddress::Loopback4(), 0x1234);
...
TEST_P(QuicFramerTest, BuildPublicResetPacketWithEndpointId)
{
  ...
  unsigned char packet_variant2[] = {
    ...
    if ('d' == data->data()[data->length() - 1]) {
        quiche::test::CompareCharArraysWithHexError(
            "constructed packet", data->data(), data->length(), AsChars(packet_variant1), ABSL_ARRAYSIZE(packet_variant1));
    } else {
        quiche::test::CompareCharArraysWithHexError(
            "constructed packet", data->data(), data->length(), AsChars(packet_variant2), ABSL_ARRAYSIZE(packet_variant2));
    }
}
...
TEST_P(QuicFramerTest, BuildIetfStatelessResetPacket)
{
    ...
    std::unique_ptr<QuicEncryptedPacket> data(
        framer_.BuildIetfStatelessResetPacket(FramerTestConnectionId(), QuicFramer::GetMinStatelessResetPacketLength() + 1, kTestStatelessResetToken));
    ASSERT_TRUE(data);
    ...
    quiche::test::CompareCharArraysWithHexError("constructed packet", data->data() + data->length() - kStatelessResetTokenLength, kStatelessResetTokenLength,
        AsChars(packet) + ABSL_ARRAYSIZE(packet) - kStatelessResetTokenLength, kStatelessResetTokenLength);

    // Packets with length <= minimal stateless reset does not trigger stateless
    // reset.
    std::unique_ptr<QuicEncryptedPacket> data2(
        framer_.BuildIetfStatelessResetPacket(FramerTestConnectionId(), QuicFramer::GetMinStatelessResetPacketLength(), kTestStatelessResetToken));
    ASSERT_FALSE(data2);

    // Do not send stateless reset >= minimal stateless reset + 1 + max
    // connection ID length.
    std::unique_ptr<QuicEncryptedPacket> data3(framer_.BuildIetfStatelessResetPacket(FramerTestConnectionId(), 1000, kTestStatelessResetToken));
    ASSERT_TRUE(data3);
    EXPECT_EQ(QuicFramer::GetMinStatelessResetPacketLength() + 1 + kQuicMaxConnectionIdWithLengthPrefixLength, data3->length());
}
...
TEST_P(QuicFramerTest, BuildIetfStatelessResetPacketCallerProvidedRandomBytes)
{
    ...
    EXPECT_CALL(random, InsecureRandBytes(_, _)).WillOnce(testing::Invoke(generate_random_bytes));
    std::unique_ptr<QuicEncryptedPacket> data(
        framer_.BuildIetfStatelessResetPacket(FramerTestConnectionId(), QuicFramer::GetMinStatelessResetPacketLength() + 1, kTestStatelessResetToken, &random));
    ASSERT_TRUE(data);
    EXPECT_EQ(QuicFramer::GetMinStatelessResetPacketLength(), data->length());
    // Verify the first 2 bits are 01.
    EXPECT_FALSE(data->data()[0] & FLAGS_LONG_HEADER);
...
TEST_P(QuicFramerTest, EncryptPacket)
{
  ...
  unsigned char packet50[] = {
    ...
    std::unique_ptr<QuicPacket> raw(new QuicPacket(AsChars(p), p_size, false, kPacket8ByteConnectionId, kPacket0ByteConnectionId, !kIncludeVersion,
        !kIncludeDiversificationNonce, PACKET_4BYTE_PACKET_NUMBER, quiche::VARIABLE_LENGTH_INTEGER_LENGTH_0, 0, quiche::VARIABLE_LENGTH_INTEGER_LENGTH_0));
    char buffer[kMaxOutgoingPacketSize];
    size_t encrypted_length = framer_.EncryptPayload(ENCRYPTION_INITIAL, packet_number, *raw, buffer, kMaxOutgoingPacketSize);
...
{
    auto packet = std::make_unique<QuicPacket>(new char[100], 0, true, kPacket8ByteConnectionId, kPacket0ByteConnectionId,
        /*includes_version=*/true,
        /*includes_diversification_nonce=*/true, PACKET_1BYTE_PACKET_NUMBER, quiche::VARIABLE_LENGTH_INTEGER_LENGTH_0,
        /*retry_token_length=*/0, quiche::VARIABLE_LENGTH_INTEGER_LENGTH_0);
    char buffer[kMaxOutgoingPacketSize];
    size_t encrypted_length = 1;
    EXPECT_QUIC_BUG(
        {
            encrypted_length = framer_.EncryptPayload(ENCRYPTION_INITIAL, kPacketNumber, *packet, buffer, kMaxOutgoingPacketSize);
...
TEST_P(QuicFramerTest, AckTruncationSmallPacket)
{
    ...
    // Build an ack packet with truncation due to limit in number of nack ranges.
    QuicFramerPeer::SetPerspective(&framer_, Perspective::IS_CLIENT);
    std::unique_ptr<QuicPacket> raw_ack_packet(BuildDataPacket(header, frames, 500));
    ASSERT_TRUE(raw_ack_packet != nullptr);
    char buffer[kMaxOutgoingPacketSize];
    size_t encrypted_length = framer_.EncryptPayload(ENCRYPTION_INITIAL, header.packet_number, *raw_ack_packet, buffer, kMaxOutgoingPacketSize);
    ASSERT_NE(0u, encrypted_length);
...
{
    return (arg.stream_id == kTestQuicStreamId || QuicUtils::IsCryptoStreamId(version.transport_version, arg.stream_id)) && !arg.fin && arg.offset == 0
...
TEST_P(QuicFramerTest, ConstructEncryptedPacket)
{
    ...
    // crypto to be Null.
    if (framer_.version().KnowsWhichDecrypterToUse()) {
        framer_.InstallDecrypter(ENCRYPTION_FORWARD_SECURE, std::make_unique<StrictTaggingDecrypter>((uint8_t)ENCRYPTION_FORWARD_SECURE));
    } else {
        framer_.SetDecrypter(ENCRYPTION_FORWARD_SECURE, std::make_unique<StrictTaggingDecrypter>((uint8_t)ENCRYPTION_FORWARD_SECURE));
    }
    ParsedQuicVersionVector versions;
    versions.push_back(framer_.version());
    std::unique_ptr<QuicEncryptedPacket> packet(ConstructEncryptedPacket(TestConnectionId(), EmptyQuicConnectionId(), false, false, kTestQuicStreamId,
        kTestString, CONNECTION_ID_PRESENT, CONNECTION_ID_ABSENT, PACKET_4BYTE_PACKET_NUMBER, &versions));

    MockFramerVisitor visitor;
    framer_.set_visitor(&visitor);
    EXPECT_CALL(visitor, OnPacket()).Times(1);
    EXPECT_CALL(visitor, OnUnauthenticatedPublicHeader(_)).Times(1).WillOnce(Return(true));
    EXPECT_CALL(visitor, OnUnauthenticatedHeader(_)).Times(1).WillOnce(Return(true));
    ...
    if (!QuicVersionUsesCryptoFrames(framer_.version().transport_version)) {
        EXPECT_CALL(visitor, OnStreamFrame(ExpectedStreamFrame(framer_.version()))).Times(1);
    } else {
...
TEST_P(QuicFramerTest, ConstructMisFramedEncryptedPacket)
{
    ...
    // crypto to be Null.
    if (framer_.version().KnowsWhichDecrypterToUse()) {
        framer_.InstallDecrypter(ENCRYPTION_FORWARD_SECURE, std::make_unique<StrictTaggingDecrypter>(ENCRYPTION_FORWARD_SECURE));
    }
    std::unique_ptr<QuicEncryptedPacket> packet(ConstructMisFramedEncryptedPacket(TestConnectionId(), EmptyQuicConnectionId(), false, false, kTestQuicStreamId,
        kTestString, CONNECTION_ID_PRESENT, CONNECTION_ID_ABSENT, PACKET_4BYTE_PACKET_NUMBER, framer_.version(), Perspective::IS_CLIENT));
...
TEST_P(QuicFramerTest, UniDiMaxStreamsFrame)
{
  ...
  PacketFragments packet_ietf = {
    ...
    EXPECT_THAT(framer_.error(), IsQuicNoError());
    ASSERT_TRUE(visitor_.header_.get());
    EXPECT_TRUE(CheckDecryption(*encrypted, !kIncludeVersion, !kIncludeDiversificationNonce, kPacket0ByteConnectionId, kPacket0ByteConnectionId));
...
TEST_P(QuicFramerTest, BiDiMaxStreamsFrameTooBig)
{
  ...
  unsigned char packet_ietf[] = {
    ...
    QuicEncryptedPacket encrypted(AsChars(packet_ietf), ABSL_ARRAYSIZE(packet_ietf), false);
    EXPECT_TRUE(framer_.ProcessPacket(encrypted));
    EXPECT_THAT(framer_.error(), IsQuicNoError());
    ASSERT_TRUE(visitor_.header_.get());
    EXPECT_TRUE(CheckDecryption(encrypted, !kIncludeVersion, !kIncludeDiversificationNonce, kPacket8ByteConnectionId, kPacket0ByteConnectionId));
...
TEST_P(QuicFramerTest, StreamsBlockedFrameTooBig)
{
  ...
  unsigned char packet_ietf[] = {
    ...
    EXPECT_EQ(framer_.detailed_error(), "STREAMS_BLOCKED stream count exceeds implementation limit.");
}
...
TEST_P(QuicFramerTest, NewConnectionIdFrame)
{
  ...
  PacketFragments packet_ietf = {
    ...
    EXPECT_EQ(FramerTestConnectionIdPlusOne(), visitor_.new_connection_id_.connection_id);
    EXPECT_EQ(0x11u, visitor_.new_connection_id_.sequence_number);
    EXPECT_EQ(0x09u, visitor_.new_connection_id_.retire_prior_to);
    EXPECT_EQ(kTestStatelessResetToken, visitor_.new_connection_id_.stateless_reset_token);
...
TEST_P(QuicFramerTest, NewConnectionIdFrameVariableLength)
{
  ...
  PacketFragments packet_ietf = {
    ...
    EXPECT_EQ(FramerTestConnectionIdNineBytes(), visitor_.new_connection_id_.connection_id);
    EXPECT_EQ(0x11u, visitor_.new_connection_id_.sequence_number);
...
TEST_P(QuicFramerTest, InvalidLongNewConnectionIdFrame)
{
  ...
  PacketFragments packet_ietf = {
    ...
    EXPECT_EQ("Invalid new connection ID length for version.", framer_.detailed_error());
}
...
TEST_P(QuicFramerTest, NewTokenFrame)
{
  ...
  PacketFragments packet = {
    ...
    // clang-format on
    uint8_t expected_token_value[] = { 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07 };
    ...
    EXPECT_EQ(0, memcmp(expected_token_value, visitor_.new_token_.token.data(), sizeof(expected_token_value)));
...
TEST_P(QuicFramerTest, BuildNewTokenFramePacket)
{
    ...
    QuicNewTokenFrame frame(0, absl::string_view((const char*)(expected_token_value), sizeof(expected_token_value)));
...
TEST_P(QuicFramerTest, IetfStopSendingFrame)
{
  ...
  PacketFragments packet_ietf = {
    ...
    EXPECT_EQ(QUIC_STREAM_UNKNOWN_APPLICATION_ERROR_CODE, visitor_.stop_sending_frame_.error_code);
    EXPECT_EQ(static_cast<uint64_t>(0x7654), visitor_.stop_sending_frame_.ietf_error_code);
...
TEST_P(QuicFramerTest, BuildIetfStopSendingPacket)
{
    ...
    frame.ietf_error_code = static_cast<uint64_t>(QuicHttpQpackErrorCode::ENCODER_STREAM_ERROR);
    QuicFrames frames = { QuicFrame(frame) };
...
TEST_P(QuicFramerTest, IetfPathChallengeFrame)
{
  ...
  PacketFragments packet_ietf = {
    ...
    EXPECT_EQ(QuicPathFrameBuffer({ { 0, 1, 2, 3, 4, 5, 6, 7 } }), visitor_.path_challenge_frame_.data_buffer);
...
TEST_P(QuicFramerTest, IetfPathResponseFrame)
{
  ...
  PacketFragments packet_ietf = {
    ...
    EXPECT_EQ(QuicPathFrameBuffer({ { 0, 1, 2, 3, 4, 5, 6, 7 } }), visitor_.path_response_frame_.data_buffer);
...
TEST_P(QuicFramerTest, GetRetransmittableControlFrameSize)
{
    ...
    EXPECT_EQ(QuicFramer::GetRstStreamFrameSize(framer_.transport_version(), rst_stream),
        QuicFramer::GetRetransmittableControlFrameSize(framer_.transport_version(), QuicFrame(&rst_stream)));

    std::string error_detail(2048, 'e');
    QuicConnectionCloseFrame connection_close(framer_.transport_version(), QUIC_NETWORK_IDLE_TIMEOUT, NO_IETF_QUIC_ERROR, error_detail,
        /*transport_close_frame_type=*/0);

    EXPECT_EQ(QuicFramer::GetConnectionCloseFrameSize(framer_.transport_version(), connection_close),
        QuicFramer::GetRetransmittableControlFrameSize(framer_.transport_version(), QuicFrame(&connection_close)));

    QuicGoAwayFrame goaway(2, QUIC_PEER_GOING_AWAY, 3, error_detail);
    EXPECT_EQ(QuicFramer::GetMinGoAwayFrameSize() + 256, QuicFramer::GetRetransmittableControlFrameSize(framer_.transport_version(), QuicFrame(&goaway)));

    QuicWindowUpdateFrame window_update(3, 3, 1024);
    EXPECT_EQ(QuicFramer::GetWindowUpdateFrameSize(framer_.transport_version(), window_update),
        QuicFramer::GetRetransmittableControlFrameSize(framer_.transport_version(), QuicFrame(window_update)));

    QuicBlockedFrame blocked(4, 3, 1024);
    EXPECT_EQ(QuicFramer::GetBlockedFrameSize(framer_.transport_version(), blocked),
        QuicFramer::GetRetransmittableControlFrameSize(framer_.transport_version(), QuicFrame(blocked)));
    ...
    QuicNewConnectionIdFrame new_connection_id(5, TestConnectionId(), 1, kTestStatelessResetToken, 1);
    EXPECT_EQ(QuicFramer::GetNewConnectionIdFrameSize(new_connection_id),
        QuicFramer::GetRetransmittableControlFrameSize(framer_.transport_version(), QuicFrame(&new_connection_id)));

    QuicMaxStreamsFrame max_streams(6, 3, /*unidirectional=*/false);
    EXPECT_EQ(QuicFramer::GetMaxStreamsFrameSize(framer_.transport_version(), max_streams),
        QuicFramer::GetRetransmittableControlFrameSize(framer_.transport_version(), QuicFrame(max_streams)));

    QuicStreamsBlockedFrame streams_blocked(7, 3, /*unidirectional=*/false);
    EXPECT_EQ(QuicFramer::GetStreamsBlockedFrameSize(framer_.transport_version(), streams_blocked),
        QuicFramer::GetRetransmittableControlFrameSize(framer_.transport_version(), QuicFrame(streams_blocked)));

    QuicPathFrameBuffer buffer = { { 0x80, 0x91, 0xa2, 0xb3, 0xc4, 0xd5, 0xe5, 0xf7 } };
    QuicPathResponseFrame path_response_frame(8, buffer);
    EXPECT_EQ(QuicFramer::GetPathResponseFrameSize(path_response_frame),
        QuicFramer::GetRetransmittableControlFrameSize(framer_.transport_version(), QuicFrame(path_response_frame)));

    QuicPathChallengeFrame path_challenge_frame(9, buffer);
    EXPECT_EQ(QuicFramer::GetPathChallengeFrameSize(path_challenge_frame),
        QuicFramer::GetRetransmittableControlFrameSize(framer_.transport_version(), QuicFrame(path_challenge_frame)));
...
TEST_P(QuicFramerTest, FirstAckBlockJustUnderFlow)
{
  ...
  unsigned char packet_ietf[] = {
    ...
    QuicEncryptedPacket encrypted(AsChars(p), p_size, false);
    EXPECT_FALSE(framer_.ProcessPacket(encrypted));
    EXPECT_EQ(framer_.detailed_error(), "Underflow with first ack block length 3 largest acked is 2.");
}
...
TEST_P(QuicFramerTest, ThirdAckBlockJustUnderflow)
{
  ...
  unsigned char packet_ietf[] = {
    ...
    EXPECT_FALSE(framer_.ProcessPacket(encrypted));
    if (VersionHasIetfQuicFrames(framer_.transport_version())) {
        EXPECT_EQ(framer_.detailed_error(), "Underflow with ack block length 6 latest ack block end is 5.");
    } else {
        EXPECT_EQ(framer_.detailed_error(), "Underflow with ack block length 6, end of block is 6.");
    }
}
...
TEST_P(QuicFramerTest, UndecryptablePacketWithoutDecrypter)
{
    ...
    if (!framer_.version().KnowsWhichDecrypterToUse()) {
        ...
        CryptoUtils::CreateInitialObfuscators(Perspective::IS_CLIENT, framer_.version(), bogus_connection_id, &bogus_crypters);
        // This removes all other decrypters.
        framer_.SetDecrypter(ENCRYPTION_FORWARD_SECURE, std::move(bogus_crypters.decrypter));
    }
  ...
  unsigned char packet49[] = {
    ...
    EXPECT_FALSE(framer_.ProcessPacket(QuicEncryptedPacket(AsChars(p), p_length, false)));
    EXPECT_THAT(framer_.error(), IsError(QUIC_DECRYPTION_FAILURE));
    ...
    quiche::test::CompareCharArraysWithHexError(
        "undecryptable packet", visitor_.undecryptable_packets_[0]->data(), visitor_.undecryptable_packets_[0]->length(), AsChars(p), p_length);
    if (framer_.version().KnowsWhichDecrypterToUse()) {
...
TEST_P(QuicFramerTest, UndecryptablePacketWithDecrypter)
{
    ...
    CryptoUtils::CreateInitialObfuscators(Perspective::IS_CLIENT, framer_.version(), bogus_connection_id, &bad_handshake_crypters);
    if (framer_.version().KnowsWhichDecrypterToUse()) {
        framer_.InstallDecrypter(ENCRYPTION_HANDSHAKE, std::move(bad_handshake_crypters.decrypter));
    } else {
        framer_.SetDecrypter(ENCRYPTION_HANDSHAKE, std::move(bad_handshake_crypters.decrypter));
    }
  ...
  unsigned char packet49[] = {
    ...
    EXPECT_EQ(framer_.version().KnowsWhichDecrypterToUse(), visitor_.undecryptable_has_decryption_keys_[0]);
}
...
TEST_P(QuicFramerTest, UndecryptableCoalescedPacket)
{
  ...
  unsigned char packet_ietf[] = {
    ...
    quiche::test::CompareCharArraysWithHexError("undecryptable packet", visitor_.undecryptable_packets_[0]->data(),
        visitor_.undecryptable_packets_[0]->length(), AsChars(p), length_of_first_coalesced_packet);
    EXPECT_EQ(ENCRYPTION_HANDSHAKE, visitor_.undecryptable_decryption_levels_[0]);
...
{
    if (!QuicVersionHasLongHeaderLengths(framer_.transport_version()) || !framer_.version().UsesInitialObfuscators()) {
        return;
    }
    ASSERT_TRUE(framer_.version().KnowsWhichDecrypterToUse());
    QuicConnectionId connection_id = FramerTestConnectionId();
    ...
    CryptoUtils::CreateInitialObfuscators(Perspective::IS_CLIENT, framer_.version(), connection_id, &client_crypters);
    framer_.SetEncrypter(ENCRYPTION_INITIAL, std::move(client_crypters.encrypter));
    ...
    QuicFrames frames = { QuicFrame(QuicPingFrame()), QuicFrame(QuicPaddingFrame(3)) };
    ...
    unsigned char packet[kMaxOutgoingPacketSize] = {};
    size_t encrypted_length = framer_.EncryptPayload(ENCRYPTION_INITIAL, header.packet_number, *data, AsChars(packet), ABSL_ARRAYSIZE(packet));
    ASSERT_NE(0u, encrypted_length);

    QuicFramerPeer::SetPerspective(&framer_, Perspective::IS_SERVER);
    CrypterPair server_crypters;
    CryptoUtils::CreateInitialObfuscators(Perspective::IS_SERVER, framer_.version(), connection_id, &server_crypters);
    framer_.InstallDecrypter(ENCRYPTION_INITIAL, std::move(server_crypters.decrypter));
...
TEST_P(QuicFramerTest, PacketHeaderWithVariableLengthConnectionId)
{
    ...
    SetDecrypterLevel(ENCRYPTION_FORWARD_SECURE);
    uint8_t connection_id_bytes[9] = { 0xFE, 0xDC, 0xBA, 0x98, 0x76, 0x54, 0x32, 0x10, 0x42 };
    QuicConnectionId connection_id(reinterpret_cast<char*>(connection_id_bytes), sizeof(connection_id_bytes));
    QuicFramerPeer::SetLargestPacketNumber(&framer_, kPacketNumber - 2);
    QuicFramerPeer::SetExpectedServerConnectionIDLength(&framer_, connection_id.length());
  ...
  PacketFragments packet_with_padding = {
    ...
    PacketFragments& fragments = framer_.version().HasHeaderProtection() ? packet_with_padding : packet;
    std::unique_ptr<QuicEncryptedPacket> encrypted(AssemblePacketFromFragments(fragments));
    if (framer_.version().HasHeaderProtection()) {
        EXPECT_TRUE(framer_.ProcessPacket(*encrypted));
        EXPECT_THAT(framer_.error(), IsQuicNoError());
...
TEST_P(QuicFramerTest, MultiplePacketNumberSpaces)
{
  ...
  unsigned char long_header_packet_ietf[] = {
    ...
    if (framer_.version().KnowsWhichDecrypterToUse()) {
        framer_.InstallDecrypter(ENCRYPTION_ZERO_RTT, std::make_unique<TestDecrypter>());
        framer_.RemoveDecrypter(ENCRYPTION_INITIAL);
    } else {
        framer_.SetDecrypter(ENCRYPTION_ZERO_RTT, std::make_unique<TestDecrypter>());
    }
    if (!QuicVersionHasLongHeaderLengths(framer_.transport_version())) {
        EXPECT_TRUE(framer_.ProcessPacket(QuicEncryptedPacket(AsChars(long_header_packet), ABSL_ARRAYSIZE(long_header_packet), false)));
    } else {
        ReviseFirstByteByVersion(long_header_packet_ietf);
        EXPECT_TRUE(framer_.ProcessPacket(QuicEncryptedPacket(AsChars(long_header_packet_ietf), ABSL_ARRAYSIZE(long_header_packet_ietf), false)));
    }

    EXPECT_THAT(framer_.error(), IsQuicNoError());
    EXPECT_FALSE(QuicFramerPeer::GetLargestDecryptedPacketNumber(&framer_, INITIAL_DATA).IsInitialized());
    EXPECT_FALSE(QuicFramerPeer::GetLargestDecryptedPacketNumber(&framer_, HANDSHAKE_DATA).IsInitialized());
    EXPECT_EQ(kPacketNumber, QuicFramerPeer::GetLargestDecryptedPacketNumber(&framer_, APPLICATION_DATA));
  ...
  unsigned char short_header_packet[] = {
    ...
    QuicEncryptedPacket short_header_encrypted(AsChars(short_header_packet), ABSL_ARRAYSIZE(short_header_packet), false);
    if (framer_.version().KnowsWhichDecrypterToUse()) {
        framer_.InstallDecrypter(ENCRYPTION_FORWARD_SECURE, std::make_unique<TestDecrypter>());
        framer_.RemoveDecrypter(ENCRYPTION_ZERO_RTT);
    } else {
        framer_.SetDecrypter(ENCRYPTION_FORWARD_SECURE, std::make_unique<TestDecrypter>());
    }
    ...
    EXPECT_FALSE(QuicFramerPeer::GetLargestDecryptedPacketNumber(&framer_, INITIAL_DATA).IsInitialized());
    EXPECT_FALSE(QuicFramerPeer::GetLargestDecryptedPacketNumber(&framer_, HANDSHAKE_DATA).IsInitialized());
    EXPECT_EQ(kPacketNumber + 1, QuicFramerPeer::GetLargestDecryptedPacketNumber(&framer_, APPLICATION_DATA));
}

TEST_P(QuicFramerTest, IetfRetryPacketRejected)
{
    if (!framer_.version().KnowsWhichDecrypterToUse() || framer_.version().SupportsRetry()) {
        return;
    }
...
TEST_P(QuicFramerTest, WriteClientVersionNegotiationProbePacket)
{
  ...
  static const uint8_t expected_packet[1200] = {
    ...
    quiche::test::CompareCharArraysWithHexError(
        "constructed packet", packet, sizeof(packet), reinterpret_cast<const char*>(expected_packet), sizeof(expected_packet));
    ...
    EXPECT_TRUE(framer_.ProcessPacket(encrypted));
    ASSERT_TRUE(visitor_.header_.get());
    QuicConnectionId probe_payload_connection_id(reinterpret_cast<const char*>(destination_connection_id_bytes), sizeof(destination_connection_id_bytes));
    EXPECT_EQ(probe_payload_connection_id, visitor_.header_.get()->destination_connection_id);
}
...
TEST_P(QuicFramerTest, DispatcherParseOldClientVersionNegotiationProbePacket)
{
  ...
  static const uint8_t packet[1200] = {
    ...
    // clang-format on
    char expected_destination_connection_id_bytes[] = { 0x56, 0x4e, 0x20, 0x70, 0x6c, 0x7a, 0x20, 0x21 };
    QuicConnectionId expected_destination_connection_id(
        reinterpret_cast<const char*>(expected_destination_connection_id_bytes), sizeof(expected_destination_connection_id_bytes));

    QuicEncryptedPacket encrypted(reinterpret_cast<const char*>(packet), sizeof(packet));
    PacketHeaderFormat format = GOOGLE_QUIC_PACKET;
    QuicLongHeaderType long_packet_type = INVALID_PACKET_TYPE;
    ...
    std::optional<absl::string_view> retry_token;
    std::string detailed_error = "foobar";
    QuicErrorCode header_parse_result
        = QuicFramer::ParsePublicHeaderDispatcher(encrypted, kQuicDefaultConnectionIdLength, &format, &long_packet_type, &version_present, &has_length_prefix,
...
TEST_P(QuicFramerTest, DispatcherParseClientInitialPacketNumber)
{
  ...
  PacketFragments packet = {
    ...
    EXPECT_EQ(QUIC_NO_ERROR,
        QuicFramer::ParsePublicHeaderDispatcherShortHeaderLengthUnknown(*encrypted, &format, &long_packet_type, &version_flag, &use_length_prefix,
            &version_label, &parsed_version, &destination_connection_id, &source_connection_id, &retry_token, &detailed_error, generator));
    EXPECT_EQ(parsed_version, version_);
    if (parsed_version != ParsedQuicVersion::RFCv1() && parsed_version != ParsedQuicVersion::Draft29()) {
        return;
    }
    ...
    EXPECT_EQ(QUIC_NO_ERROR,
        QuicFramer::TryDecryptInitialPacketDispatcher(*encrypted, parsed_version, format, long_packet_type, destination_connection_id, source_connection_id,
            retry_token,
            /*largest_decrypted_inital_packet_number=*/QuicPacketNumber(), *decrypter_, &packet_number));
    EXPECT_THAT(packet_number, Optional(2));
}

TEST_P(QuicFramerTest, DispatcherParseClientInitialPacketNumberFromCoalescedPacket)
{
    if (!QuicVersionHasLongHeaderLengths(framer_.transport_version())) {
        return;
    }
...
TEST_P(QuicFramerTest, ParseServerVersionNegotiationProbeResponse)
{
  ...
  const uint8_t packet[] = {
    ...
    EXPECT_TRUE(QuicFramer::ParseServerVersionNegotiationProbeResponse(reinterpret_cast<const char*>(packet), sizeof(packet),
        reinterpret_cast<char*>(parsed_probe_payload_bytes), &parsed_probe_payload_length, &parse_detailed_error));
    EXPECT_EQ("", parse_detailed_error);
    quiche::test::CompareCharArraysWithHexError(
        "parsed probe", parsed_probe_payload_bytes, parsed_probe_payload_length, probe_payload_bytes, sizeof(probe_payload_bytes));
}

TEST_P(QuicFramerTest, ParseClientVersionNegotiationProbePacket)
{
    char packet[1200];
    char input_destination_connection_id_bytes[] = { 0x56, 0x4e, 0x20, 0x70, 0x6c, 0x7a, 0x20, 0x21 };
    ASSERT_TRUE(QuicFramer::WriteClientVersionNegotiationProbePacket(
        packet, sizeof(packet), input_destination_connection_id_bytes, sizeof(input_destination_connection_id_bytes)));
    char parsed_destination_connection_id_bytes[255] = { 0 };
    uint8_t parsed_destination_connection_id_length = sizeof(parsed_destination_connection_id_bytes);
    ASSERT_TRUE(
        ParseClientVersionNegotiationProbePacket(packet, sizeof(packet), parsed_destination_connection_id_bytes, &parsed_destination_connection_id_length));
    quiche::test::CompareCharArraysWithHexError("parsed destination connection ID", parsed_destination_connection_id_bytes,
        parsed_destination_connection_id_length, input_destination_connection_id_bytes, sizeof(input_destination_connection_id_bytes));
}
...
TEST_P(QuicFramerTest, WriteServerVersionNegotiationProbeResponse)
{
    ...
    char input_source_connection_id_bytes[] = { 0x56, 0x4e, 0x20, 0x70, 0x6c, 0x7a, 0x20, 0x21 };
    ASSERT_TRUE(WriteServerVersionNegotiationProbeResponse(packet, &packet_length, input_source_connection_id_bytes, sizeof(input_source_connection_id_bytes)));
    char parsed_source_connection_id_bytes[255] = { 0 };
    uint8_t parsed_source_connection_id_length = sizeof(parsed_source_connection_id_bytes);
    std::string detailed_error;
    ASSERT_TRUE(QuicFramer::ParseServerVersionNegotiationProbeResponse(
        packet, packet_length, parsed_source_connection_id_bytes, &parsed_source_connection_id_length, &detailed_error))
        << detailed_error;
    quiche::test::CompareCharArraysWithHexError("parsed destination connection ID", parsed_source_connection_id_bytes, parsed_source_connection_id_length,
        input_source_connection_id_bytes, sizeof(input_source_connection_id_bytes));
}
...
TEST_P(QuicFramerTest, ClientConnectionIdFromLongHeaderToClient)
{
  ...
  unsigned char packet49[] = {
    ...
    const bool parse_success = framer_.ProcessPacket(QuicEncryptedPacket(AsChars(p), p_length, false));
    if (!framer_.version().AllowsVariableLengthConnectionIds()) {
    ...
    EXPECT_EQ("", framer_.detailed_error());
    ASSERT_TRUE(visitor_.header_.get());
    EXPECT_EQ(FramerTestConnectionId(), visitor_.header_.get()->destination_connection_id);
}
...
TEST_P(QuicFramerTest, ClientConnectionIdFromLongHeaderToServer)
{
  ...
  unsigned char packet49[] = {
    ...
    if (!framer_.version().SupportsClientConnectionIds()) {
        EXPECT_FALSE(parse_success);
        EXPECT_THAT(framer_.error(), IsError(QUIC_INVALID_PACKET_HEADER));
        EXPECT_EQ("Client connection ID not supported in this version.", framer_.detailed_error());
        return;
    }
    EXPECT_TRUE(parse_success);
    EXPECT_THAT(framer_.error(), IsQuicNoError());
    EXPECT_EQ("", framer_.detailed_error());
    ASSERT_TRUE(visitor_.header_.get());
    EXPECT_EQ(FramerTestConnectionId(), visitor_.header_.get()->source_connection_id);
}
...
TEST_P(QuicFramerTest, ProcessAndValidateIetfConnectionIdLengthClient)
{
    ...
    EXPECT_TRUE(QuicFramerPeer::ProcessAndValidateIetfConnectionIdLength(&reader, framer_.version(), Perspective::IS_CLIENT,
        should_update_expected_server_connection_id_length, &expected_server_connection_id_length, &destination_connection_id_length,
    ...
    EXPECT_TRUE(QuicFramerPeer::ProcessAndValidateIetfConnectionIdLength(&reader2, framer_.version(), Perspective::IS_CLIENT,
        should_update_expected_server_connection_id_length, &expected_server_connection_id_length, &destination_connection_id_length,
        &source_connection_id_length, &detailed_error));
    EXPECT_EQ(8, expected_server_connection_id_length);
    EXPECT_EQ(0, destination_connection_id_length);
...
TEST_P(QuicFramerTest, ProcessAndValidateIetfConnectionIdLengthServer)
{
    ...
    EXPECT_TRUE(QuicFramerPeer::ProcessAndValidateIetfConnectionIdLength(&reader, framer_.version(), Perspective::IS_SERVER,
        should_update_expected_server_connection_id_length, &expected_server_connection_id_length, &destination_connection_id_length,
        &source_connection_id_length, &detailed_error));
    EXPECT_EQ(8, expected_server_connection_id_length);
    ...
    should_update_expected_server_connection_id_length = true;
    expected_server_connection_id_length = 33;
    EXPECT_TRUE(QuicFramerPeer::ProcessAndValidateIetfConnectionIdLength(&reader2, framer_.version(), Perspective::IS_SERVER,
        should_update_expected_server_connection_id_length, &expected_server_connection_id_length, &destination_connection_id_length,
        &source_connection_id_length, &detailed_error));
    EXPECT_EQ(8, expected_server_connection_id_length);
    EXPECT_EQ(8, destination_connection_id_length);
...
TEST_P(QuicFramerTest, TestExtendedErrorCodeParser)
{
    ...
    MaybeExtractQuicErrorCode(&frame);
    EXPECT_THAT(frame.quic_error_code, IsError(QUIC_IETF_GQUIC_ERROR_MISSING));
    EXPECT_EQ("1a234:this has a colon, but a malformed error number", frame.error_details);
    ...
    frame.error_details = "1234 :this is not good, space between last digit and colon";
    MaybeExtractQuicErrorCode(&frame);
    EXPECT_THAT(frame.quic_error_code, IsError(QUIC_IETF_GQUIC_ERROR_MISSING));
    EXPECT_EQ("1234 :this is not good, space between last digit and colon", frame.error_details);
    ...
    MaybeExtractQuicErrorCode(&frame);
    EXPECT_THAT(frame.quic_error_code, IsError(QUIC_IETF_GQUIC_ERROR_MISSING));
    EXPECT_EQ(" 1234:this is not good, space before first digit", frame.error_details);
...
TEST_P(QuicFramerTest, OverlyLargeAckDelay)
{
  ...
  unsigned char packet_ietf[] = {
    ...
    framer_.ProcessPacket(QuicEncryptedPacket(AsChars(packet_ietf), ABSL_ARRAYSIZE(packet_ietf), false));
    ASSERT_EQ(1u, visitor_.ack_frames_.size());
    // Verify ack_delay_time is set correctly.
    EXPECT_EQ(QuicTime::Delta::Infinite(), visitor_.ack_frames_[0]->ack_delay_time);
}
...
TEST_P(QuicFramerTest, KeyUpdateReceivedWhenNotEnabled)
{
    ...
    std::unique_ptr<QuicPacket> data(BuildDataPacket(header, frames));
    ASSERT_TRUE(data != nullptr);
    std::unique_ptr<QuicEncryptedPacket> encrypted(EncryptPacketWithTagAndPhase(*data, 1, true));
    ASSERT_TRUE(encrypted);
...
TEST_P(QuicFramerTest, KeyUpdateOnFirstReceivedPacket)
{
    ...
    std::unique_ptr<QuicPacket> data(BuildDataPacket(header, frames));
    ASSERT_TRUE(data != nullptr);
    std::unique_ptr<QuicEncryptedPacket> encrypted(EncryptPacketWithTagAndPhase(*data, /*tag=*/1, /*phase=*/true));
    ASSERT_TRUE(encrypted);
...
{
    if (!VersionHasIetfQuicFrames(framer_.transport_version()) || !QuicVersionHasLongHeaderLengths(framer_.transport_version())
  ...
  unsigned char packet[] = {
    ...
    EXPECT_THAT(framer_.error(), IsError(IETF_QUIC_PROTOCOL_VIOLATION));
    EXPECT_EQ("IETF frame type IETF_ACK is unexpected at encryption level "
              "ENCRYPTION_ZERO_RTT",
