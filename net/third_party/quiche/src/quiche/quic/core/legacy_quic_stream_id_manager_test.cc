...
{
    return absl::StrCat(ParsedQuicVersionToString(p.version), (p.perspective == Perspective::IS_CLIENT ? "Client" : "Server"));
}
...
public:
    LegacyQuicStreamIdManagerTest()
        : manager_(GetParam().perspective, GetParam().version.transport_version, kDefaultMaxStreamsPerConnection, kDefaultMaxStreamsPerConnection)
    {
    }
...
protected:
    QuicStreamId GetNthPeerInitiatedId(int n)
    {
        if (GetParam().perspective == Perspective::IS_SERVER) {
            return QuicUtils::GetFirstBidirectionalStreamId(GetParam().version.transport_version, Perspective::IS_CLIENT) + 2 * n;
        } else {
            return 2 + 2 * n;
        }
    }
...
INSTANTIATE_TEST_SUITE_P(Tests, LegacyQuicStreamIdManagerTest, ::testing::ValuesIn(GetTestParams()), ::testing::PrintToStringParamName());
...
{
    ASSERT_TRUE(manager_.MaybeIncreaseLargestPeerStreamId(GetNthPeerInitiatedId(3)));
    ...
    ASSERT_TRUE(manager_.MaybeIncreaseLargestPeerStreamId(GetNthPeerInitiatedId(2)));
    ASSERT_TRUE(manager_.MaybeIncreaseLargestPeerStreamId(GetNthPeerInitiatedId(1)));
}
...
TEST_P(LegacyQuicStreamIdManagerTest, MaxAvailableStreams)
{
    ...
    EXPECT_EQ(manager_.max_open_incoming_streams() * kMaxAvailableStreamsMultiplier, manager_.MaxAvailableStreams());
    ...
    EXPECT_TRUE(manager_.MaybeIncreaseLargestPeerStreamId(GetNthPeerInitiatedId(0)));

    // Establish available streams up to the server's limit.
    const int kLimitingStreamId = GetNthPeerInitiatedId(kAvailableStreamLimit + 1);
    ...
    // violates the quota.
    EXPECT_FALSE(manager_.MaybeIncreaseLargestPeerStreamId(kLimitingStreamId + 2 * 2));
}
...
TEST_P(LegacyQuicStreamIdManagerTest, MaximumAvailableOpenedStreams)
{
    ...
    EXPECT_TRUE(manager_.MaybeIncreaseLargestPeerStreamId(stream_id + 2 * (manager_.max_open_incoming_streams() - 1)));
}
...
TEST_P(LegacyQuicStreamIdManagerTest, TooManyAvailableStreams)
{
    ...
    QuicStreamId stream_id2 = GetNthPeerInitiatedId(2 * manager_.MaxAvailableStreams() + 4);
...
TEST_P(LegacyQuicStreamIdManagerTest, ManyAvailableStreams)
{
    ...
    EXPECT_TRUE(manager_.MaybeIncreaseLargestPeerStreamId(GetNthPeerInitiatedId(199)));
}

TEST_P(LegacyQuicStreamIdManagerTest, TestMaxIncomingAndOutgoingStreamsAllowed)
{
    EXPECT_EQ(manager_.max_open_incoming_streams(), kDefaultMaxStreamsPerConnection);
    EXPECT_EQ(manager_.max_open_outgoing_streams(), kDefaultMaxStreamsPerConnection);
}
