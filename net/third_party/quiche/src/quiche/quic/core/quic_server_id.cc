...
// static
std::optional<QuicServerId> QuicServerId::ParseFromHostPortString(absl::string_view host_port_string)
{
    ...
    url::ParseAuthority(
        host_port_string.data(), url::Component(0, host_port_string.size()), &username_component, &password_component, &host_component, &port_component);

    // Only support "host:port" and nothing more or less.
    if (username_component.is_valid() || password_component.is_valid() || !host_component.is_nonempty() || !port_component.is_nonempty()) {
    ...
    int parsed_port_number = url::ParsePort(host_port_string.data(), port_component);
    ...
    if (parsed_port_number <= 0) {
        QUICHE_DVLOG(1) << "Port could not be parsed while parsing QuicServerId from: " << host_port_string;
        return std::nullopt;
    }
    QUICHE_DCHECK_LE(parsed_port_number, std::numeric_limits<uint16_t>::max());

    return QuicServerId(std::move(hostname), static_cast<uint16_t>(parsed_port_number));
}
...
{
    if (!absl::StrContains(host_, ':') || host_.length() <= 2 || (host_.front() == '[' && host_.back() == ']')) {
        return host_;
    } else {
