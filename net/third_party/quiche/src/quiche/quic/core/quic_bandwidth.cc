...
std::string QuicBandwidth::ToDebuggingValue() const
{
    if (bits_per_second_ < 80000) {
        return absl::StrFormat("%d bits/s (%d bytes/s)", bits_per_second_, bits_per_second_ / 8);
    }
    ...
    return absl::StrFormat("%.2f %cbits/s (%.2f %cbytes/s)", bits_per_second_with_unit, unit, bytes_per_second_with_unit, unit);
}
