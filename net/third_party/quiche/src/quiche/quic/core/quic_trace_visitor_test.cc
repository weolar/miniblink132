...
public:
    QuicTraceVisitorTest()
    {
        ...
        simulator::QuicEndpoint client(&simulator, "Client", "Server", Perspective::IS_CLIENT, connection_id);
        simulator::QuicEndpoint server(&simulator, "Server", "Client", Perspective::IS_SERVER, connection_id);
        ...
        simulator::SymmetricLink client_link(&client, network_switch.port(1), 2 * kBandwidth, kDelay);
        simulator::SymmetricLink server_link(&server, network_switch.port(2), kBandwidth, kDelay);
        ...
        const QuicTime::Delta kDeadline = 3 * kBandwidth.TransferTime(kTransferSize);
        client.AddBytesToTransfer(kTransferSize);
        bool simulator_result = simulator.RunUntilOrTimeout([&]() { return server.bytes_received() >= kTransferSize; }, kDeadline);
    ...
    std::vector<quic_trace::Event> AllEventsWithType(quic_trace::EventType event_type)
    {
...
TEST_F(QuicTraceVisitorTest, ConnectionId)
{
    ...
    EXPECT_EQ(std::string(expected_cid, sizeof(expected_cid)), trace_.destination_connection_id());
}
...
TEST_F(QuicTraceVisitorTest, Version)
{
    ...
    EXPECT_TRUE(version[0] != 0 || version[1] != 0 || version[2] != 0 || version[3] != 0);
}
        ...
        if (packet.event_type() == quic_trace::PACKET_RECEIVED) {
            for (const quic_trace::Frame& frame : packet.frames()) {
                ...
                for (const auto& block : info.acked_packets()) {
                    packets.Add(QuicPacketNumber(block.first_packet()), QuicPacketNumber(block.last_packet()) + 1);
                }
            }
        }
        if (packet.event_type() == quic_trace::PACKET_LOST) {
            packets.Add(QuicPacketNumber(packet.packet_number()), QuicPacketNumber(packet.packet_number()) + 1);
        }
    }
...
TEST_F(QuicTraceVisitorTest, TransportState)
{
    ...
    EXPECT_LE((4 * kDelay).ToMicroseconds() * 1., acks.rbegin()->transport_state().min_rtt_us());
    EXPECT_GE((4 * kDelay).ToMicroseconds() * 1.25, acks.rbegin()->transport_state().min_rtt_us());
}
