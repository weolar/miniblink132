...
public:
    ...
    virtual bool ReadAndDispatchPackets(int fd, int port, const QuicClock& clock, ProcessPacketInterface* processor, QuicPacketCount* packets_dropped);
...
private:
    ...
    static QuicIpAddress GetSelfIpFromPacketInfo(const QuicUdpPacketInfo& packet_info, bool prefer_v6_ip);

    struct QUICHE_EXPORT ReadBuffer {
        ABSL_CACHELINE_ALIGNED char control_buffer[kDefaultUdpPacketControlBufferSize]; // For ancillary
            // data.
