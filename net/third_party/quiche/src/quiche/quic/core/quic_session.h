...
class QUICHE_EXPORT QuicSession : public QuicConnectionVisitorInterface,
...
public:
    ...
    public:
        ...
        virtual bool TryAddNewConnectionId(const QuicConnectionId& server_connection_id, const QuicConnectionId& new_connection_id) = 0;

        // Called when a ConnectionId has been retired.
        virtual void OnConnectionIdRetired(const QuicConnectionId& server_connection_id) = 0;
    ...
    QuicSession(QuicConnection* connection, Visitor* owner, const QuicConfig& config, const ParsedQuicVersionVector& supported_versions,
        QuicStreamCount num_expected_unidirectional_static_streams);
    QuicSession(QuicConnection* connection, Visitor* owner, const QuicConfig& config, const ParsedQuicVersionVector& supported_versions,
        QuicStreamCount num_expected_unidirectional_static_streams, std::unique_ptr<QuicDatagramQueue::Observer> datagram_observer,
    ...
    void OnSuccessfulVersionNegotiation(const ParsedQuicVersion& version) override;
    void OnPacketReceived(const QuicSocketAddress& self_address, const QuicSocketAddress& peer_address, bool is_connectivity_probe) override;
    ...
    // mapping.
    bool MaybeReserveConnectionId(const QuicConnectionId& server_connection_id) override;
    void OnServerConnectionIdRetired(const QuicConnectionId& server_connection_id) override;
    ...
    void CreateContextForMultiPortPath(std::unique_ptr<MultiPortPathContextObserver> /*context_observer*/) override
    {
    }
    void MigrateToMultiPortPath(std::unique_ptr<QuicPathValidationContext> /*context*/) override
    {
    }
    void OnServerPreferredAddressAvailable(const QuicSocketAddress& /*server_preferred_address*/) override;
    ...
    WriteStreamDataResult WriteStreamData(QuicStreamId id, QuicStreamOffset offset, QuicByteCount data_length, QuicDataWriter* writer) override;
    bool WriteCryptoData(EncryptionLevel level, QuicStreamOffset offset, QuicByteCount data_length, QuicDataWriter* writer) override;

    // SessionNotifierInterface methods:
    bool OnFrameAcked(const QuicFrame& frame, QuicTime::Delta ack_delay_time, QuicTime receive_timestamp) override;
    void OnStreamFrameRetransmitted(const QuicStreamFrame& frame) override;
    void OnFrameLost(const QuicFrame& frame) override;
    bool RetransmitFrames(const QuicFrames& frames, TransmissionType type) override;
    ...
    void SendMaxStreams(QuicStreamCount stream_count, bool unidirectional) override;
    ...
    MessageResult SendMessage(absl::Span<quiche::QuicheMemSlice> message, bool flush);
    ...
    virtual void OnMessageAcked(QuicMessageId message_id, QuicTime receive_timestamp);
    ...
    void OnControlFrameManagerError(QuicErrorCode error_code, std::string error_details) override;
    ...
    bool WriteControlFrame(const QuicFrame& frame, TransmissionType type) override;
    ...
    virtual std::optional<std::string> OnAlpsData(const uint8_t* alps_data, size_t alps_length);

    // From HandshakerDelegateInterface
    bool OnNewDecryptionKeyAvailable(
        EncryptionLevel level, std::unique_ptr<QuicDecrypter> decrypter, bool set_alternative_decrypter, bool latch_once_used) override;
    void OnNewEncryptionKeyAvailable(EncryptionLevel level, std::unique_ptr<QuicEncrypter> encrypter) override;
    ...
    QuicErrorCode ProcessTransportParameters(const TransportParameters& params, bool is_resumption, std::string* error_details) override;
    void OnHandshakeCallbackDone() override;
    bool PacketFlusherAttached() const override;
    ParsedQuicVersion parsed_version() const override
    {
        return version();
    }
    void OnEncryptedClientHelloSent(absl::string_view client_hello) const override;
    void OnEncryptedClientHelloReceived(absl::string_view client_hello) const override;

    // Implement StreamDelegateInterface.
    void OnStreamError(QuicErrorCode error_code, std::string error_details) override;
    void OnStreamError(QuicErrorCode error_code, QuicIetfTransportErrorCodes ietf_error, std::string error_details) override;
    // Sets priority in the write blocked list.
    void RegisterStreamPriority(QuicStreamId id, bool is_static, const QuicStreamPriority& priority) override;
    ...
    size_t SendCryptoData(EncryptionLevel level, size_t write_length, QuicStreamOffset offset, TransmissionType type) override;

    // Called by the QuicCryptoStream when a handshake message is sent.
    virtual void OnCryptoHandshakeMessageSent(const CryptoHandshakeMessage& message);

    // Called by the QuicCryptoStream when a handshake message is received.
    virtual void OnCryptoHandshakeMessageReceived(const CryptoHandshakeMessage& message);
    ...
    bool transport_goaway_sent() const
    {
        return transport_goaway_sent_;
    }

    bool transport_goaway_received() const
    {
        return transport_goaway_received_;
    }

    // Returns the Google QUIC error code
    QuicErrorCode error() const
    {
        return on_closed_frame_.quic_error_code;
    }
    // The error code on the wire.  For Google QUIC frames, this has the same
    // value as `error()`.
    uint64_t wire_error() const
    {
        return on_closed_frame_.wire_error_code;
    }
    ...
    static void RecordConnectionCloseAtServer(QuicErrorCode error, ConnectionCloseSource source);
    ...
    virtual void MaybeSendRstStreamFrame(QuicStreamId id, QuicResetStreamError error, QuicStreamOffset bytes_written);

    // Sends a STOP_SENDING frame if the stream type allows.
    virtual void MaybeSendStopSendingFrame(QuicStreamId id, QuicResetStreamError error);
    ...
    virtual QuicSSLConfig GetSSLConfig() const
    {
        return QuicSSLConfig();
    }
    ...
    void set_client_original_supported_versions(const ParsedQuicVersionVector& client_original_supported_versions)
    {
        QUICHE_DCHECK_EQ(perspective_, Perspective::IS_CLIENT);
    ...
    uint64_t total_datagrams_lost() const
    {
        return total_datagrams_lost_;
    }
    ...
    QuicPriorityType priority_type() const
    {
        return priority_type_;
    }
...
protected:
    using StreamMap = absl::flat_hash_map<QuicStreamId, std::unique_ptr<QuicStream>>;

    // Use a linked hash map for pending streams so that they will be processed in
    // a FIFO order to avoid starvation.
    using PendingStreamMap = quiche::QuicheLinkedHashMap<QuicStreamId, std::unique_ptr<PendingStream>>;

    using ClosedStreams = std::vector<std::unique_ptr<QuicStream>>;

    using ZombieStreamMap = absl::flat_hash_map<QuicStreamId, std::unique_ptr<QuicStream>>;
    ...
    // controller.
    virtual void OnFinalByteOffsetReceived(QuicStreamId id, QuicStreamOffset final_byte_offset);
    ...
    virtual bool UsesPendingStreamForFrame(QuicFrameType /*type*/, QuicStreamId /*stream_id*/) const
    {
        return false;
    }
    ...
    {
        return write_blocked_streams_->GetPriorityOfStream(stream_id).http().urgency;
    }

    size_t pending_streams_size() const
    {
        return pending_stream_map_.size();
    }

    ClosedStreams* closed_streams()
    {
        return &closed_streams_;
    }

    void set_largest_peer_created_stream_id(QuicStreamId largest_peer_created_stream_id);
    ...
    void InsertLocallyClosedStreamsHighestOffset(const QuicStreamId id, QuicStreamOffset offset);
    ...
    virtual void HandleRstOnValidNonexistentStream(const QuicRstStreamFrame& frame);
    ...
    QuicDatagramQueue* datagram_queue()
    {
        return &datagram_queue_;
    }

    size_t num_static_streams() const
    {
        return num_static_streams_;
    }

    size_t num_zombie_streams() const
    {
        return num_zombie_streams_;
    }

    bool was_zero_rtt_rejected() const
    {
        return was_zero_rtt_rejected_;
    }
    ...
    size_t num_draining_streams() const
    {
        return num_draining_streams_;
    }
    ...
    virtual QuicStream* ProcessReadUnidirectionalPendingStream(PendingStream* /*pending*/)
    {
        QUICHE_BUG(received unexpected pending read unidirectional stream);
        return nullptr;
    }
    virtual QuicStream* ProcessBidirectionalPendingStream(PendingStream* /*pending*/)
    {
    ...
    void PerformActionOnActiveStreams(quiche::UnretainedCallback<bool(QuicStream*)> action);
    void PerformActionOnActiveStreams(quiche::UnretainedCallback<bool(QuicStream*)> action) const;
    ...
    bool MaybeSetStreamPriority(QuicStreamId stream_id, const QuicStreamPriority& priority);
    ...
    virtual std::optional<CachedNetworkParameters> GenerateCachedNetworkParameters() const
    {
        return std::nullopt;
    }
    ...
    void set_max_streams_accepted_per_loop(QuicStreamCount max_streams_accepted_per_loop)
    {
...
private:
    ...
    void OnNewStreamOutgoingBidirectionalFlowControlWindow(QuicStreamOffset new_window);

    // Called in OnConfigNegotiated when we receive a new incoming bidirectional
    // stream flow control window in a negotiated config.
    void OnNewStreamIncomingBidirectionalFlowControlWindow(QuicStreamOffset new_window);
    ...
    bool CheckStreamNotBusyLooping(QuicStream* stream, uint64_t previous_bytes_written, bool previous_fin_sent);
    ...
    // stream.
    bool ShouldProcessFrameByPendingStream(QuicFrameType type, QuicStreamId id) const;
    ...
    absl::flat_hash_map<QuicStreamId, QuicStreamOffset> locally_closed_streams_highest_offset_;
    ...
    quiche::QuicheLinkedHashMap<QuicStreamId, bool> streams_with_pending_retransmission_;
    ...
    const bool enable_stop_sending_for_zombie_streams_ = GetQuicReloadableFlag(quic_deliver_stop_sending_to_zombie_streams);
};
