...
TEST_F(QuicIntervalSetTest, IsDisjoint)
{
    ...
    EXPECT_TRUE(is.IsDisjoint(QuicInterval<int>(2300, std::numeric_limits<int>::max())));
    ...
    EXPECT_FALSE(is.IsDisjoint(QuicInterval<int>(2199, std::numeric_limits<int>::max())));
    // Empty intervals.
...
static bool VA_Check(const QuicIntervalSet<int>& is, int count, va_list ap)
{
    ...
    if (count != static_cast<int>(intervals.size())) {
        QUIC_LOG(ERROR) << "Expected " << count << " intervals, got " << intervals.size() << ": " << is;
        return false;
    }
    if (count != static_cast<int>(is.Size())) {
        QUIC_LOG(ERROR) << "Expected " << count << " intervals, got Size " << is.Size() << ": " << is;
        return false;
    }
    bool result = true;
    for (int i = 0; i < count; i++) {
        ...
        if (min != intervals[i].min() || max != intervals[i].max()) {
            QUIC_LOG(ERROR) << "Expected: [" << min << ", " << max << ") got " << intervals[i] << " in " << is;
            result = false;
        }
    }
    return result;
...
static void TestContainsAndFind(const QuicIntervalSet<int>& is, int min, int max)
{
    EXPECT_TRUE(is.Contains(min, max)) << "Set does not contain interval with min " << min << "and max " << max;
    auto it = is.Find(min, max);
    EXPECT_NE(it, is.end()) << "No iterator to interval with min " << min << "and max " << max;
...
static void TestNotContainsAndFind(const QuicIntervalSet<int>& is, int value)
{
    ...
    auto it = is.Find(value);
    EXPECT_EQ(it, is.end()) << "There is iterator to interval containing " << value;
}

static void TestNotContainsAndFind(const QuicIntervalSet<int>& is, int min, int max)
{
    EXPECT_FALSE(is.Contains(min, max)) << "Set contains interval with min " << min << "and max " << max;
    auto it = is.Find(min, max);
    EXPECT_EQ(it, is.end()) << "There is iterator to interval with min " << min << "and max " << max;
}
...
TEST_F(QuicIntervalSetTest, QuicIntervalSetBasic)
{
    ...
    // to iterate over intervals.
    {
        ...
        // in reverse order.
        EXPECT_THAT(std::vector<QuicInterval<int>>(iset.rbegin(), iset.rend()), ElementsAreArray(expected.rbegin(), expected.rend()));
    }
...
{
    is.Union(other);
    EXPECT_TRUE(Check(is, 12, 50, 70, 100, 200, 300, 400, 470, 600, 650, 670, 700, 830, 870, 1000, 1100, 1230, 1270, 1830, 1900, 2000, 2100, 2200, 2250, 2270));
}
...
TEST_F(QuicIntervalSetTest, QuicIntervalSetIntersection)
{
    ...
    EXPECT_TRUE(Check(is, 7, 350, 360, 370, 380, 500, 530, 770, 800, 1300, 1400, 1500, 1600, 1700, 1800));
    EXPECT_TRUE(is.Intersects(other));
    EXPECT_TRUE(other.Intersects(is));
}
...
TEST_F(QuicIntervalSetTest, QuicIntervalSetIntersectionTheirsBeforeMineInt64Singletons)
{
...
TEST_F(QuicIntervalSetTest, QuicIntervalSetIntersectionMineBeforeTheirsIntSingletons)
{
...
TEST_F(QuicIntervalSetTest, QuicIntervalSetIntersectionAdjacentAlternatingNonIntersectingIntervals)
{
    ...
    const QuicIntervalSet<int16_t> y2({ { -50, -40 }, { -2, 0 }, { 10, 20 }, { 32, 40 }, { 90, 95 } });
...
TEST_F(QuicIntervalSetTest, QuicIntervalSetIntersectionAlternatingIntersectingIntervals)
{
    const QuicIntervalSet<int> x1({ { 0, 10 } });
    ...
    const QuicIntervalSet<int16_t> x2({ { 0, 10 }, { 20, 30 }, { 40, 90 } });
    const QuicIntervalSet<int16_t> y2({ { -50, -40 }, { -2, 2 }, { 9, 21 }, { 32, 41 }, { 85, 95 } });
    const QuicIntervalSet<int16_t> expected_result2({ { 0, 2 }, { 9, 10 }, { 20, 21 }, { 40, 41 }, { 85, 90 } });
...
{
    is.Difference(other);
    EXPECT_TRUE(Check(is, 10, 100, 200, 300, 350, 360, 370, 380, 400, 530, 600, 700, 770, 900, 1000, 1100, 1200, 1900, 2000, 2100, 2200));
    QuicIntervalSet<int> copy = is;
    is.Difference(copy);
    EXPECT_TRUE(is.Empty());
}
...
// completement case.
static bool CheckOneComplement(int add_min, int add_max, int comp_min, int comp_max, int count, ...)
{
    QuicIntervalSet<int> iset;
...
static bool CheckComplement(const QuicIntervalSet<int>& iset, int comp_min, int comp_max, int count, ...)
{
...
TEST_F(QuicIntervalSetTest, MultiIntervalComplement)
{
    ...
    //        |----- i -----|
    //    |---------- comp  ----------|
    EXPECT_TRUE(CheckComplement(iset, 0, 700, 4, 0, 100, 200, 300, 400, 500, 600, 700));
...
struct NotOstreamable {
    bool operator<(const NotOstreamable&) const
    {
        return false;
    }
    bool operator>(const NotOstreamable&) const
