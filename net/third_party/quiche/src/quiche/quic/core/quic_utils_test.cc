...
TEST_F(QuicUtilsTest, DetermineAddressChangeType)
{
    ...
    EXPECT_EQ(NO_CHANGE, QuicUtils::DetermineAddressChangeType(old_address, new_address));
    ...
    EXPECT_EQ(PORT_CHANGE, QuicUtils::DetermineAddressChangeType(old_address, new_address));
    ...
    ASSERT_TRUE(address.FromString(kIPv6String1));
    new_address = QuicSocketAddress(address, 1234);
    EXPECT_EQ(IPV4_TO_IPV6_CHANGE, QuicUtils::DetermineAddressChangeType(old_address, new_address));

    old_address = QuicSocketAddress(address, 1234);
    ASSERT_TRUE(address.FromString(kIPv4String1));
    new_address = QuicSocketAddress(address, 1234);
    EXPECT_EQ(IPV6_TO_IPV4_CHANGE, QuicUtils::DetermineAddressChangeType(old_address, new_address));

    ASSERT_TRUE(address.FromString(kIPv6String2));
    new_address = QuicSocketAddress(address, 1234);
    EXPECT_EQ(IPV6_TO_IPV6_CHANGE, QuicUtils::DetermineAddressChangeType(old_address, new_address));

    ASSERT_TRUE(address.FromString(kIPv4String1));
    old_address = QuicSocketAddress(address, 1234);
    ASSERT_TRUE(address.FromString(kIPv4String2));
    new_address = QuicSocketAddress(address, 1234);
    EXPECT_EQ(IPV4_SUBNET_CHANGE, QuicUtils::DetermineAddressChangeType(old_address, new_address));
    ASSERT_TRUE(address.FromString(kIPv4String3));
    new_address = QuicSocketAddress(address, 1234);
    EXPECT_EQ(IPV4_TO_IPV4_CHANGE, QuicUtils::DetermineAddressChangeType(old_address, new_address));
}
...
absl::uint128 IncrementalHashReference(const void* data, size_t len)
{
    ...
    absl::uint128 hash = absl::MakeUint128(UINT64_C(7809847782465536322), UINT64_C(7113472399480571277));
...
TEST_F(QuicUtilsTest, ReferenceTest)
{
    ...
    EXPECT_EQ(IncrementalHashReference(data.data(), data.size()),
        QuicUtils::FNV1a_128_Hash(absl::string_view(reinterpret_cast<const char*>(data.data()), data.size())));
}
...
TEST_F(QuicUtilsTest, RetransmissionTypeToPacketState)
{
    for (size_t i = FIRST_TRANSMISSION_TYPE; i <= LAST_TRANSMISSION_TYPE; ++i) {
        ...
        SentPacketState state = QuicUtils::RetransmissionTypeToPacketState(static_cast<TransmissionType>(i));
        if (i == HANDSHAKE_RETRANSMISSION) {
            ...
            QUICHE_DCHECK(false) << "No corresponding packet state according to transmission type: " << i;
        }
    }
}
...
TEST_F(QuicUtilsTest, RandomConnectionId)
{
    ...
    EXPECT_EQ(connection_id, QuicConnectionId(static_cast<char*>(connection_id_bytes), ABSL_ARRAYSIZE(connection_id_bytes)));
    ...
    EXPECT_EQ(QuicUtils::CreateRandomConnectionId().length(), kQuicDefaultConnectionIdLength);
}
...
TEST_F(QuicUtilsTest, RandomConnectionIdVariableLength)
{
    ...
    QuicConnectionId connection_id = QuicUtils::CreateRandomConnectionId(connection_id_length, &random);
    ...
    EXPECT_NE(connection_id, TestConnectionId(1));
    EXPECT_NE(connection_id, TestConnectionIdNineBytesLong(1));
    EXPECT_EQ(QuicUtils::CreateRandomConnectionId(connection_id_length).length(), connection_id_length);
}

TEST_F(QuicUtilsTest, VariableLengthConnectionId)
{
    EXPECT_FALSE(VersionAllowsVariableLengthConnectionIds(QUIC_VERSION_46));
    EXPECT_TRUE(QuicUtils::IsConnectionIdValidForVersion(QuicUtils::CreateZeroConnectionId(QUIC_VERSION_46), QUIC_VERSION_46));
    EXPECT_NE(QuicUtils::CreateZeroConnectionId(QUIC_VERSION_46), EmptyQuicConnectionId());
    EXPECT_FALSE(QuicUtils::IsConnectionIdValidForVersion(EmptyQuicConnectionId(), QUIC_VERSION_46));
}
...
TEST_F(QuicUtilsTest, StatelessResetToken)
{
    ...
    StatelessResetToken token1a = QuicUtils::GenerateStatelessResetToken(connection_id1a);
    StatelessResetToken token1b = QuicUtils::GenerateStatelessResetToken(connection_id1b);
    StatelessResetToken token2 = QuicUtils::GenerateStatelessResetToken(connection_id2);
...
{
    BitMask<TestEnumClassBit> mask({ TestEnumClassBit::BIT_ZERO, TestEnumClassBit::BIT_TWO });
