...
public:
    ...
    void PrintLiteral(const char* literal) override
    {
        trace_.push_back(literal);
    }
...
struct FakeConnection {
    FakeConnection()
    {
        context.tracer = std::make_unique<TraceCollector>();
    }
...
void NestedSwitch()
{
    ...
    EXPECT_THAT(outer.trace(), ElementsAre("outer literal 0", "outer string 0", "outer printf 0", "outer literal 1", "outer string 1", "outer printf 1"));

    EXPECT_THAT(inner.trace(), ElementsAre("inner literal", "inner string", "inner printf"));
}
...
void AlternatingSwitch()
{
    ...
    for (int i = 0; i < 15; ++i) {
        FakeConnection* connection = ((i % 3) == 0) ? &zero : (((i % 3) == 1) ? &one : &two);
...
template <ThreadFunction func> class TestThread : public QuicThread {
public:
    TestThread()
        : QuicThread("TestThread")
    {
    }
...
template <ThreadFunction func> void RunInThreads(size_t n_threads)
{
...
TEST_F(QuicConnectionContextTest, NullTracerOK)
{
    ...
    EXPECT_THAT(static_cast<TraceCollector*>(tracer.get())->trace(), ElementsAre("msg 1 recorded"));
}
