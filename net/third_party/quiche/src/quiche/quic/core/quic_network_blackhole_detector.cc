...
QuicNetworkBlackholeDetector::QuicNetworkBlackholeDetector(Delegate* delegate, QuicAlarmProxy alarm)
    : delegate_(delegate)
    , alarm_(alarm)
{
}
...
void QuicNetworkBlackholeDetector::OnAlarm()
{
    ...
    QUIC_DVLOG(1) << "BlackholeDetector alarm firing. next_deadline:" << next_deadline << ", path_degrading_deadline_:" << path_degrading_deadline_
                  << ", path_mtu_reduction_deadline_:" << path_mtu_reduction_deadline_ << ", blackhole_deadline_:" << blackhole_deadline_;
...
void QuicNetworkBlackholeDetector::RestartDetection(QuicTime path_degrading_deadline, QuicTime blackhole_deadline, QuicTime path_mtu_reduction_deadline)
{
    ...
    QUIC_BUG_IF(quic_bug_12708_1, blackhole_deadline_.IsInitialized() && blackhole_deadline_ != GetLastDeadline())
...
QuicTime QuicNetworkBlackholeDetector::GetEarliestDeadline() const
{
    ...
    for (QuicTime t : { path_degrading_deadline_, blackhole_deadline_, path_mtu_reduction_deadline_ }) {
        if (!t.IsInitialized()) {
            continue;
        }
...
{
    return std::max({ path_degrading_deadline_, blackhole_deadline_, path_mtu_reduction_deadline_ });
}
...
void QuicNetworkBlackholeDetector::UpdateAlarm()
{
    ...
    QUIC_DVLOG(1) << "Updating alarm. next_deadline:" << next_deadline << ", path_degrading_deadline_:" << path_degrading_deadline_
                  << ", path_mtu_reduction_deadline_:" << path_mtu_reduction_deadline_ << ", blackhole_deadline_:" << blackhole_deadline_;
