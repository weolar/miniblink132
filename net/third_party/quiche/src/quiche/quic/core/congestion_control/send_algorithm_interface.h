...
public:
    ...
    struct QUICHE_EXPORT NetworkParams {
        ...
        NetworkParams(const QuicBandwidth& bandwidth, const QuicTime::Delta& rtt, bool allow_cwnd_to_decrease)
            : bandwidth(bandwidth)
            , rtt(rtt)
        ...
        {
            return bandwidth == other.bandwidth && rtt == other.rtt && max_initial_congestion_window == other.max_initial_congestion_window
                && allow_cwnd_to_decrease == other.allow_cwnd_to_decrease && is_rtt_trusted == other.is_rtt_trusted;
        }
    ...
    static SendAlgorithmInterface* Create(const QuicClock* clock, const RttStats* rtt_stats, const QuicUnackedPacketMap* unacked_packets,
        CongestionControlType type, QuicRandom* random, QuicConnectionStats* stats, QuicPacketCount initial_congestion_window,
    ...
    virtual void SetFromConfig(const QuicConfig& config, Perspective perspective) = 0;

    virtual void ApplyConnectionOptions(const QuicTagVector& connection_options) = 0;
    ...
    virtual void SetApplicationDrivenPacingRate(QuicBandwidth application_bandwidth_target) = 0;
    ...
    // these fields.
    virtual void OnCongestionEvent(bool rtt_updated, QuicByteCount prior_in_flight, QuicTime event_time, const AckedPacketVector& acked_packets,
