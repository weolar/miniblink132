...
public:
    ...
    void set_application_driven_pacing_rate(QuicBandwidth application_driven_pacing_rate)
    {
    ...
    void set_remove_non_initial_burst()
    {
        remove_non_initial_burst_ = true;
    }

    QuicBandwidth max_pacing_rate() const
    {
        return max_pacing_rate_;
    }

    void OnCongestionEvent(bool rtt_updated, QuicByteCount bytes_in_flight, QuicTime event_time, const AckedPacketVector& acked_packets,
    ...
    QuicTime::Delta TimeUntilSend(QuicTime now, QuicByteCount bytes_in_flight) const;
    ...
    uint32_t initial_burst_size() const
    {
        return initial_burst_size_;
    }

protected:
    uint32_t lumpy_tokens() const
    {
        return lumpy_tokens_;
    }
...
private:
    ...
    bool remove_non_initial_burst_ = GetQuicReloadableFlag(quic_pacing_remove_non_initial_burst);
};
