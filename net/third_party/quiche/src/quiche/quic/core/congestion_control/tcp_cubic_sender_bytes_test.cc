...
public:
    TcpCubicSenderBytesPeer(const QuicClock* clock, bool reno)
        : TcpCubicSenderBytes(clock, &rtt_stats_, reno, kInitialCongestionWindowPackets, kMaxCongestionWindowPackets, &stats_)
    {
    }
...
protected:
    ...
    int SendAvailableSendWindow(QuicPacketLength /*packet_length*/)
    {
        ...
        while (can_send) {
            sender_->OnPacketSent(clock_.Now(), bytes_in_flight_, QuicPacketNumber(packet_number_++), kDefaultTCPMSS, HAS_RETRANSMITTABLE_DATA);
            ++packets_sent;
    ...
    {
        sender_->rtt_stats_.UpdateRtt(QuicTime::Delta::FromMilliseconds(60), QuicTime::Delta::Zero(), clock_.Now());
        AckedPacketVector acked_packets;
        LostPacketVector lost_packets;
        for (int i = 0; i < n; ++i) {
            ++acked_packet_number_;
            acked_packets.push_back(AckedPacket(QuicPacketNumber(acked_packet_number_), kDefaultTCPMSS, QuicTime::Zero()));
        }
        sender_->OnCongestionEvent(true, bytes_in_flight_, clock_.Now(), acked_packets, lost_packets, 0, 0);
    ...
    void LoseNPackets(int n, QuicPacketLength packet_length)
    {
        ...
        for (int i = 0; i < n; ++i) {
            ++acked_packet_number_;
            lost_packets.push_back(LostPacket(QuicPacketNumber(acked_packet_number_), packet_length));
        }
        sender_->OnCongestionEvent(false, bytes_in_flight_, clock_.Now(), acked_packets, lost_packets, 0, 0);
    ...
    void LosePacket(uint64_t packet_number)
    {
        AckedPacketVector acked_packets;
        LostPacketVector lost_packets;
        lost_packets.push_back(LostPacket(QuicPacketNumber(packet_number), kDefaultTCPMSS));
        sender_->OnCongestionEvent(false, bytes_in_flight_, clock_.Now(), acked_packets, lost_packets, 0, 0);
        bytes_in_flight_ -= kDefaultTCPMSS;
    }
...
TEST_F(TcpCubicSenderBytesTest, ExponentialSlowStart)
{
    ...
    EXPECT_EQ(QuicBandwidth::FromBytesAndTimeDelta(cwnd, sender_->rtt_stats_.smoothed_rtt()), sender_->BandwidthEstimate());
}
...
TEST_F(TcpCubicSenderBytesTest, BandwidthResumption)
{
    ...
    const QuicBandwidth kBandwidthEstimate = QuicBandwidth::FromBytesPerSecond(kNumberOfPackets * kDefaultTCPMSS);
    ...
    const QuicBandwidth kUnreasonableBandwidth = QuicBandwidth::FromBytesPerSecond((kMaxResumptionCongestionWindow + 1) * kDefaultTCPMSS);
    ...
    EXPECT_EQ(kMaxResumptionCongestionWindow * kDefaultTCPMSS, sender_->GetCongestionWindow());
}
...
TEST_F(TcpCubicSenderBytesTest, NoPRR)
{
    ...
    const QuicBandwidth expected_pacing_rate = QuicBandwidth::FromBytesAndTimeDelta(kRenoBeta * kDefaultWindowTCP, sender_->rtt_stats_.smoothed_rtt());
    EXPECT_EQ(expected_pacing_rate, sender_->PacingRate(0));
    EXPECT_EQ(window_in_packets, static_cast<uint64_t>(SendAvailableSendWindow()));
    EXPECT_EQ(expected_pacing_rate, sender_->PacingRate(kRenoBeta * kDefaultWindowTCP));
}
...
TEST_F(TcpCubicSenderBytesTest, ResetAfterConnectionMigration)
{
    ...
    EXPECT_EQ(kDefaultWindowTCP, sender_->GetCongestionWindow());
    EXPECT_EQ(kMaxCongestionWindowPackets * kDefaultTCPMSS, sender_->GetSlowStartThreshold());
    EXPECT_FALSE(sender_->hybrid_slow_start().started());
}
...
TEST_F(TcpCubicSenderBytesTest, DefaultMaxCwnd)
{
    ...
    std::unique_ptr<SendAlgorithmInterface> sender(SendAlgorithmInterface::Create(
        &clock_, &rtt_stats, /*unacked_packets=*/nullptr, kCubicBytes, QuicRandom::GetInstance(), &stats, kInitialCongestionWindow, nullptr));

    AckedPacketVector acked_packets;
    LostPacketVector missing_packets;
    QuicPacketCount max_congestion_window = GetQuicFlag(quic_max_congestion_window);
    for (uint64_t i = 1; i < max_congestion_window; ++i) {
        acked_packets.clear();
        acked_packets.push_back(AckedPacket(QuicPacketNumber(i), 1350, QuicTime::Zero()));
        sender->OnCongestionEvent(true, sender->GetCongestionWindow(), clock_.Now(), acked_packets, missing_packets, 0, 0);
    }
