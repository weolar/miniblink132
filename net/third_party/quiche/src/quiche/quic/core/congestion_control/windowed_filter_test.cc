...
public:
    ...
    WindowedFilterTest()
        : windowed_min_rtt_(QuicTime::Delta::FromMilliseconds(99), QuicTime::Delta::Zero(), QuicTime::Zero())
        , windowed_max_bw_(QuicTime::Delta::FromMilliseconds(99), QuicBandwidth::Zero(), QuicTime::Zero())
    {
    }
    ...
    void InitializeMinFilter()
    {
        ...
        for (int i = 0; i < 5; ++i) {
            ...
            QUIC_VLOG(1) << "i: " << i << " sample: " << rtt_sample.ToMilliseconds() << " mins: "
                         << " " << windowed_min_rtt_.GetBest().ToMilliseconds() << " " << windowed_min_rtt_.GetSecondBest().ToMilliseconds() << " "
        ...
        EXPECT_EQ(QuicTime::Delta::FromMilliseconds(40), windowed_min_rtt_.GetSecondBest());
        EXPECT_EQ(QuicTime::Delta::FromMilliseconds(50), windowed_min_rtt_.GetThirdBest());
    }
    ...
    void InitializeMaxFilter()
    {
        ...
        for (int i = 0; i < 5; ++i) {
            ...
            QUIC_VLOG(1) << "i: " << i << " sample: " << bw_sample.ToBitsPerSecond() << " maxs: "
                         << " " << windowed_max_bw_.GetBest().ToBitsPerSecond() << " " << windowed_max_bw_.GetSecondBest().ToBitsPerSecond() << " "
        ...
        EXPECT_EQ(QuicBandwidth::FromBitsPerSecond(700), windowed_max_bw_.GetSecondBest());
        EXPECT_EQ(QuicBandwidth::FromBitsPerSecond(600), windowed_max_bw_.GetThirdBest());
    }

protected:
    WindowedFilter<QuicTime::Delta, MinFilter<QuicTime::Delta>, QuicTime, QuicTime::Delta> windowed_min_rtt_;
    WindowedFilter<QuicBandwidth, MaxFilter<QuicBandwidth>, QuicTime, QuicTime::Delta> windowed_max_bw_;
};

namespace {
// Test helper function: updates the filter with a lot of small values in order
// to ensure that it is not susceptible to noise.
void UpdateWithIrrelevantSamples(WindowedFilter<uint64_t, MaxFilter<uint64_t>, uint64_t, uint64_t>* filter, uint64_t max_value, uint64_t time)
{
...
TEST_F(WindowedFilterTest, MonotonicallyIncreasingMin)
{
    ...
    for (int i = 0; i < 6; ++i) {
        ...
        if (i < 3) {
            ...
            EXPECT_EQ(QuicTime::Delta::FromMilliseconds(40), windowed_min_rtt_.GetBest());
        }
    }
}
...
TEST_F(WindowedFilterTest, MonotonicallyDecreasingMax)
{
    ...
    for (int i = 0; i < 6; ++i) {
        ...
        if (i < 3) {
            ...
            EXPECT_EQ(QuicBandwidth::FromBitsPerSecond(700), windowed_max_bw_.GetBest());
        }
    }
}
...
TEST_F(WindowedFilterTest, SampleChangesThirdBestMin)
{
    ...
    QuicTime::Delta rtt_sample = windowed_min_rtt_.GetThirdBest() - QuicTime::Delta::FromMilliseconds(5);
    // This assert is necessary to avoid triggering -Wstrict-overflow
    // See crbug/616957
    ASSERT_GT(windowed_min_rtt_.GetThirdBest(), QuicTime::Delta::FromMilliseconds(5));
...
TEST_F(WindowedFilterTest, SampleChangesThirdBestMax)
{
    ...
    QuicBandwidth bw_sample = windowed_max_bw_.GetThirdBest() + QuicBandwidth::FromBitsPerSecond(50);
    // Latest sample was recorded at 100ms.
    QuicTime now = QuicTime::Zero() + QuicTime::Delta::FromMilliseconds(101);
    windowed_max_bw_.Update(bw_sample, now);
...
TEST_F(WindowedFilterTest, SampleChangesSecondBestMin)
{
    ...
    QuicTime::Delta rtt_sample = windowed_min_rtt_.GetSecondBest() - QuicTime::Delta::FromMilliseconds(5);
    // This assert is necessary to avoid triggering -Wstrict-overflow
    // See crbug/616957
    ASSERT_GT(windowed_min_rtt_.GetSecondBest(), QuicTime::Delta::FromMilliseconds(5));
    // Latest sample was recorded at 100ms.
    QuicTime now = QuicTime::Zero() + QuicTime::Delta::FromMilliseconds(101);
    windowed_min_rtt_.Update(rtt_sample, now);
    EXPECT_EQ(rtt_sample, windowed_min_rtt_.GetThirdBest());
...
TEST_F(WindowedFilterTest, SampleChangesSecondBestMax)
{
    ...
    QuicBandwidth bw_sample = windowed_max_bw_.GetSecondBest() + QuicBandwidth::FromBitsPerSecond(50);
    // Latest sample was recorded at 100ms.
    QuicTime now = QuicTime::Zero() + QuicTime::Delta::FromMilliseconds(101);
    windowed_max_bw_.Update(bw_sample, now);
    EXPECT_EQ(bw_sample, windowed_max_bw_.GetThirdBest());
...
TEST_F(WindowedFilterTest, SampleChangesAllMins)
{
    ...
    QuicTime::Delta rtt_sample = windowed_min_rtt_.GetBest() - QuicTime::Delta::FromMilliseconds(5);
    // This assert is necessary to avoid triggering -Wstrict-overflow
    // See crbug/616957
...
TEST_F(WindowedFilterTest, SampleChangesAllMaxs)
{
    ...
    QuicBandwidth bw_sample = windowed_max_bw_.GetBest() + QuicBandwidth::FromBitsPerSecond(50);
    // Latest sample was recorded at 100ms.
    QuicTime now = QuicTime::Zero() + QuicTime::Delta::FromMilliseconds(101);
    windowed_max_bw_.Update(bw_sample, now);
    EXPECT_EQ(bw_sample, windowed_max_bw_.GetThirdBest());
...
TEST_F(WindowedFilterTest, ExpireBestMin)
{
    ...
    QuicTime::Delta rtt_sample = old_third_best + QuicTime::Delta::FromMilliseconds(5);
...
TEST_F(WindowedFilterTest, ExpireBestMax)
{
    ...
    QuicBandwidth bw_sample = old_third_best - QuicBandwidth::FromBitsPerSecond(50);
...
TEST_F(WindowedFilterTest, ExpireAllMins)
{
    ...
    // This assert is necessary to avoid triggering -Wstrict-overflow
    // See crbug/616957
    ASSERT_LT(windowed_min_rtt_.GetThirdBest(), QuicTime::Delta::Infinite() - QuicTime::Delta::FromMilliseconds(5));
...
TEST_F(WindowedFilterTest, ExpireCounterBasedMax)
{
    ...
    WindowedFilter<uint64_t, MaxFilter<uint64_t>, uint64_t, uint64_t> max_filter(2, 0, 0);
