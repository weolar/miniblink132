...
template <typename T> class QUICHE_EXPORT Limits {
public:
    Limits(T min, T max)
        : min_(min)
        , max_(max)
...
template <typename T> QUICHE_EXPORT inline Limits<T> MinMax(T min, T max)
{
    return Limits<T>(min, max);
}

template <typename T> QUICHE_EXPORT inline Limits<T> NoLessThan(T min)
{
    return Limits<T>(min, std::numeric_limits<T>::max());
}

template <typename T> QUICHE_EXPORT inline Limits<T> NoGreaterThan(T max)
{
    return Limits<T>(std::numeric_limits<T>::min(), max);
}

template <typename T> QUICHE_EXPORT inline Limits<T> Unlimited()
{
    return Limits<T>(std::numeric_limits<T>::min(), std::numeric_limits<T>::max());
}

template <typename T> QUICHE_EXPORT inline std::ostream& operator<<(std::ostream& os, const Limits<T>& limits)
{
...
struct QUICHE_EXPORT Bbr2Params {
   ...
   */
    ...
    QuicTime::Delta probe_bw_probe_base_duration = QuicTime::Delta::FromMilliseconds(GetQuicFlag(quic_bbr2_default_probe_bw_base_duration_ms));

    // The upper bound of the random amount of BBR-native probes.
    QuicTime::Delta probe_bw_probe_max_rand_duration = QuicTime::Delta::FromMilliseconds(GetQuicFlag(quic_bbr2_default_probe_bw_max_rand_duration_ms));

    // The minimum number of loss marking events to exit the PROBE_UP phase.
    int64_t probe_bw_full_loss_count = GetQuicFlag(quic_bbr2_default_probe_bw_full_loss_count);
   ...
   */
    float probe_rtt_inflight_target_bdp_fraction = GetQuicFlag(quic_bbr2_default_probe_rtt_inflight_target_bdp_fraction);
    QuicTime::Delta probe_rtt_period = QuicTime::Delta::FromMilliseconds(GetQuicFlag(quic_bbr2_default_probe_rtt_period_ms));
    QuicTime::Delta probe_rtt_duration = QuicTime::Delta::FromMilliseconds(GetQuicFlag(quic_bbr2_default_probe_rtt_duration_ms));
   ...
   */
    ...
    QuicRoundTripCount initial_max_ack_height_filter_window = GetQuicFlag(quic_bbr2_default_initial_ack_height_filter_window);

    // Fraction of unutilized headroom to try to leave in path upon high loss.
    float inflight_hi_headroom = GetQuicFlag(quic_bbr2_default_inflight_hi_headroom);
...
public:
    ...
    QuicRoundTripCount Count() const
    {
...
public:
    MinRttFilter(QuicTime::Delta initial_min_rtt, QuicTime initial_min_rtt_timestamp);

    void Update(QuicTime::Delta sample_rtt, QuicTime now);

    void ForceUpdate(QuicTime::Delta sample_rtt, QuicTime now);

    QuicTime::Delta Get() const
    {
        return min_rtt_;
    }

    QuicTime GetTimestamp() const
    {
        return min_rtt_timestamp_;
    }
...
private:
    QuicBandwidth max_bandwidth_[2] = { QuicBandwidth::Zero(), QuicBandwidth::Zero() };
};
...
public:
    Bbr2NetworkModel(const Bbr2Params* params, QuicTime::Delta initial_rtt, QuicTime initial_rtt_timestamp, float cwnd_gain, float pacing_gain,
    ...
    void OnCongestionEventFinish(QuicPacketNumber least_unacked_packet, const Bbr2CongestionEvent& congestion_event);
    ...
    {
        bandwidth_sampler_.OnAppLimited();
    }
    ...
    {
        return min_rtt_filter_.Get();
    }

    QuicTime MinRttTimestamp() const
    {
        return min_rtt_filter_.GetTimestamp();
    }
    ...
    QuicBandwidth MaxBandwidth() const
    {
    ...
    bool IsInflightTooHigh(const Bbr2CongestionEvent& congestion_event, int64_t max_loss_events) const;
    ...
    void CheckPersistentQueue(const Bbr2CongestionEvent& congestion_event, float target_gain);
    ...
    int64_t loss_events_in_round() const
    {
        return loss_events_in_round_;
    }
    ...
    QuicBandwidth bandwidth_latest() const
    {
        return bandwidth_latest_;
    }
    QuicBandwidth bandwidth_lo() const
    {
        return bandwidth_lo_;
    }
    static QuicBandwidth bandwidth_lo_default()
    {
        return QuicBandwidth::Infinite();
    }
    void clear_bandwidth_lo()
    {
        bandwidth_lo_ = bandwidth_lo_default();
    }

    QuicByteCount inflight_latest() const
    {
        return inflight_latest_;
    }
    QuicByteCount inflight_lo() const
    {
        return inflight_lo_;
    }
    ...
    void set_cwnd_gain(float cwnd_gain)
    {
    ...
    void set_pacing_gain(float pacing_gain)
    {
        pacing_gain_ = pacing_gain;
    }

    bool full_bandwidth_reached() const
    {
        return full_bandwidth_reached_;
    }
    void set_full_bandwidth_reached()
    {
...
private:
    ...
    QuicByteCount min_bytes_in_flight_in_round_ = std::numeric_limits<uint64_t>::max();
...
QUICHE_EXPORT inline std::ostream& operator<<(std::ostream& os, const Bbr2Mode& mode)
{
    switch (mode) {
...
public:
    ...
    virtual void Enter(QuicTime now, const Bbr2CongestionEvent* congestion_event) = 0;
    virtual void Leave(QuicTime now, const Bbr2CongestionEvent* congestion_event) = 0;

    virtual Bbr2Mode OnCongestionEvent(QuicByteCount prior_in_flight, QuicTime event_time, const AckedPacketVector& acked_packets,
    ...
    virtual Bbr2Mode OnExitQuiescence(QuicTime now, QuicTime quiescence_start_time) = 0;
...
QUICHE_EXPORT inline QuicByteCount BytesInFlight(const SendTimeState& send_state)
{
    QUICHE_DCHECK(send_state.is_valid);
    if (send_state.bytes_in_flight != 0) {
        return send_state.bytes_in_flight;
    }
    return send_state.total_bytes_sent - send_state.total_bytes_acked - send_state.total_bytes_lost;
}
