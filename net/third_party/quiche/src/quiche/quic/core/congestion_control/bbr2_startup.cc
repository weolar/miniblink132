...
Bbr2StartupMode::Bbr2StartupMode(const Bbr2Sender* sender, Bbr2NetworkModel* model, QuicTime now)
...
void Bbr2StartupMode::Enter(QuicTime /*now*/, const Bbr2CongestionEvent* /*congestion_event*/)
{
    QUIC_BUG(quic_bug_10463_1) << "Bbr2StartupMode::Enter should not be called";
}

void Bbr2StartupMode::Leave(QuicTime now, const Bbr2CongestionEvent* /*congestion_event*/)
{
...
Bbr2Mode Bbr2StartupMode::OnCongestionEvent(QuicByteCount /*prior_in_flight*/, QuicTime /*event_time*/, const AckedPacketVector& /*acked_packets*/,
    const LostPacketVector& /*lost_packets*/, const Bbr2CongestionEvent& congestion_event)
{
    ...
    if (Params().always_exit_startup_on_excess_loss || (!congestion_event.last_packet_send_state.is_app_limited && !has_bandwidth_growth)) {
        ...
        if (!congestion_event.last_packet_send_state.is_app_limited) {
            ...
            if (max_bw_at_round_beginning_ > QuicBandwidth::Zero()) {
                const float bandwidth_ratio
                    = std::max(1., model_->MaxBandwidth().ToBitsPerSecond() / static_cast<double>(max_bw_at_round_beginning_.ToBitsPerSecond()));
                // Even when bandwidth isn't increasing, use a gain large enough to
                // cause a full_bw_threshold increase.
                const float new_gain = ((bandwidth_ratio - 1) * (Params().startup_pacing_gain - Params().full_bw_threshold)) + Params().full_bw_threshold;
                // Allow the pacing gain to decrease.
                model_->set_pacing_gain(std::min(Params().startup_pacing_gain, new_gain));
                ...
                if (model_->bandwidth_lo() < model_->MaxBandwidth() * model_->pacing_gain()) {
                    model_->clear_bandwidth_lo();
                }
            }
...
void Bbr2StartupMode::CheckExcessiveLosses(const Bbr2CongestionEvent& congestion_event)
{
    ...
    if (model_->IsInflightTooHigh(congestion_event, Params().startup_full_loss_count)) {
        ...
        QUIC_DVLOG(3) << sender_ << " Exiting STARTUP due to loss at round " << model_->RoundTripCount() << ". inflight_hi:" << new_inflight_hi;
...
Bbr2StartupMode::DebugState Bbr2StartupMode::ExportDebugState() const
{
    ...
    s.round_trips_without_bandwidth_growth = model_->rounds_without_bandwidth_growth();
    return s;
}

std::ostream& operator<<(std::ostream& os, const Bbr2StartupMode::DebugState& state)
{
