...
DEFINE_QUICHE_COMMAND_LINE_FLAG(std::string, quic_bbr2_test_regression_mode, "",
...
const uint32_t kDefaultInitialCwndBytes = kDefaultInitialCwndPackets * kDefaultTCPMSS;
...
public:
    ...
    {
        return 2
            * (local_link.delay + test_link.delay + local_link.bandwidth.TransferTime(kMaxOutgoingPacketSize)
                + test_link.bandwidth.TransferTime(kMaxOutgoingPacketSize));
    }

    QuicByteCount BDP() const
    {
        return BottleneckBandwidth() * RTT();
    }
    ...
    std::string ToString() const
    {
        ...
        os << "{ BottleneckBandwidth: " << BottleneckBandwidth() << " RTT: " << RTT() << " BDP: " << BDP() << " BottleneckQueueSize: " << SwitchQueueCapacity()
           << "}";
        return os.str();
    }
};
...
protected:
    ...
    {
        const std::string regression_mode = quiche::GetQuicheCommandLineFlag(FLAGS_quic_bbr2_test_regression_mode);
        const QuicTime::Delta simulated_duration = SimulatedNow() - QuicTime::Zero();
        if (regression_mode == "record") {
            RecordSendAlgorithmTestResult(random_seed_, simulated_duration.ToMicroseconds());
        } else if (regression_mode == "regress") {
            CompareSendAlgorithmTestResult(simulated_duration.ToMicroseconds());
        }
    }

    QuicTime SimulatedNow() const
    {
        return simulator_.GetClock()->Now();
    }
...
protected:
    Bbr2DefaultTopologyTest()
        : sender_endpoint_(&simulator_, "Sender", "Receiver", Perspective::IS_CLIENT, TestConnectionId(42))
        , receiver_endpoint_(&simulator_, "Receiver", "Sender", Perspective::IS_SERVER, TestConnectionId(42))
    {
    ...
    ~Bbr2DefaultTopologyTest()
    {
        ...
        QUIC_LOG(INFO) << "Bbr2DefaultTopologyTest." << test_info->name() << " completed at simulated time: " << SimulatedNow().ToDebuggingValue() / 1e6
                       << " sec. packet loss:" << sender_loss_rate_in_packets() * 100 << "%, bw_hi:" << debug_state.bandwidth_hi;
    }

    QuicUnackedPacketMap* GetUnackedMap(QuicConnection* connection)
    {
        return QuicSentPacketManagerPeer::GetUnackedPacketMap(QuicConnectionPeer::GetSentPacketManager(connection));
    }

    Bbr2Sender* SetupBbr2Sender(simulator::QuicEndpoint* endpoint, BbrSender* old_sender)
    {
        // Ownership of the sender will be overtaken by the endpoint.
        Bbr2Sender* sender = new Bbr2Sender(endpoint->connection()->clock()->Now(), endpoint->connection()->sent_packet_manager().GetRttStats(),
            GetUnackedMap(endpoint->connection()), kDefaultInitialCwndPackets, GetQuicFlag(quic_max_congestion_window), &random_,
    ...
    void CreateNetwork(const DefaultTopologyParams& params)
    {
        ...
        switch_ = std::make_unique<simulator::Switch>(&simulator_, "Switch", params.switch_port_count, params.SwitchQueueCapacity());
        ...
        network_links_.push_back(
            std::make_unique<simulator::SymmetricLink>(&sender_endpoint_, switch_->port(1), params.local_link.bandwidth, params.local_link.delay));

        // Test link connects receiver and port 2.
        if (params.sender_policer_params.has_value()) {
            const TrafficPolicerParams& policer_params = params.sender_policer_params.value();
            sender_policer_ = std::make_unique<simulator::TrafficPolicer>(&simulator_, policer_params.name, policer_params.initial_burst_size,
                policer_params.max_bucket_size, policer_params.target_bandwidth, switch_->port(2));
            network_links_.push_back(
                std::make_unique<simulator::SymmetricLink>(&receiver_endpoint_, sender_policer_.get(), params.test_link.bandwidth, params.test_link.delay));
        } else {
            network_links_.push_back(
                std::make_unique<simulator::SymmetricLink>(&receiver_endpoint_, switch_->port(2), params.test_link.bandwidth, params.test_link.delay));
        }
    }

    simulator::SymmetricLink* TestLink()
    {
        return network_links_[1].get();
    }
    ...
    void DoSimpleTransfer(QuicByteCount transfer_size, QuicTime::Delta timeout)
    {
        ...
        bool simulator_result = simulator_.RunUntilOrTimeout([this]() { return sender_endpoint_.bytes_to_transfer() == 0; }, timeout);
        EXPECT_TRUE(simulator_result) << "Simple transfer failed.  Bytes remaining: " << sender_endpoint_.bytes_to_transfer();
    ...
    void DriveOutOfStartup(const DefaultTopologyParams& params)
    {
        ...
        EXPECT_APPROX_EQ(params.BottleneckBandwidth(), sender_->ExportDebugState().bandwidth_hi, 0.02f);
    }

    // Send |bytes|-sized bursts of data |number_of_bursts| times, waiting for
    // |wait_time| between each burst.
    void SendBursts(const DefaultTopologyParams& params, size_t number_of_bursts, QuicByteCount bytes, QuicTime::Delta wait_time)
    {
    ...
    template <class TerminationPredicate> bool SendUntilOrTimeout(TerminationPredicate termination_predicate, QuicTime::Delta timeout)
    {
        ...
        do {
            ...
            if (simulator_.RunUntilOrTimeout([this]() { return sender_endpoint_.bytes_to_transfer() == 0; }, deadline - SimulatedNow())
    ...
    void EnableAggregation(QuicByteCount aggregation_bytes, QuicTime::Delta aggregation_timeout)
    {
        switch_->port_queue(1)->EnableAggregation(aggregation_bytes, aggregation_timeout);
    }
    ...
    QuicConnection* sender_connection()
    {
        return sender_endpoint_.connection();
    }
    ...
    {
        return static_cast<float>(sender_connection_stats().packets_lost) / sender_connection_stats().packets_sent;
    }
...
TEST_F(Bbr2DefaultTopologyTest, NormalStartup)
{
    ...
    EXPECT_EQ(3u, sender_->ExportDebugState().startup.round_trips_without_bandwidth_growth);
    EXPECT_EQ(0u, sender_connection_stats().packets_lost);
    EXPECT_APPROX_EQ(params.BottleneckBandwidth(), sender_->ExportDebugState().bandwidth_hi, 0.01f);
...
TEST_F(Bbr2DefaultTopologyTest, NormalStartupB207)
{
    ...
    EXPECT_EQ(1u, sender_->ExportDebugState().startup.round_trips_without_bandwidth_growth);
    EXPECT_APPROX_EQ(params.BottleneckBandwidth(), sender_->ExportDebugState().bandwidth_hi, 0.01f);
    EXPECT_EQ(0u, sender_connection_stats().packets_lost);
}
...
TEST_F(Bbr2DefaultTopologyTest, NormalStartupB207andB205)
{
    ...
    EXPECT_EQ(Bbr2Mode::DRAIN, sender_->ExportDebugState().mode);
    EXPECT_EQ(1u, sender_->ExportDebugState().round_trip_count - max_bw_round);
    EXPECT_EQ(2u, sender_->ExportDebugState().startup.round_trips_without_bandwidth_growth);
    EXPECT_APPROX_EQ(params.BottleneckBandwidth(), sender_->ExportDebugState().bandwidth_hi, 0.01f);
    EXPECT_EQ(0u, sender_connection_stats().packets_lost);
}
...
TEST_F(Bbr2DefaultTopologyTest, SimpleTransfer)
{
    ...
    QuicBandwidth expected_pacing_rate = QuicBandwidth::FromBytesAndTimeDelta(2.885 * kDefaultInitialCwndBytes, rtt_stats()->initial_rtt());
    EXPECT_APPROX_EQ(expected_pacing_rate.ToBitsPerSecond(), sender_->PacingRate(0).ToBitsPerSecond(), 0.01f);
...
TEST_F(Bbr2DefaultTopologyTest, SimpleTransfer2RTTAggregationBytesB201)
{
    ...
    EXPECT_APPROX_EQ(params.BottleneckBandwidth(), sender_->ExportDebugState().bandwidth_hi, 0.5f);
...
TEST_F(Bbr2DefaultTopologyTest, QUIC_SLOW_TEST(BandwidthDecrease))
{
    ...
    EXPECT_APPROX_EQ(params.test_link.bandwidth, sender_->ExportDebugState().bandwidth_est, 0.1f);
    EXPECT_EQ(0u, sender_connection_stats().packets_lost);
    ...
    bool simulator_result = simulator_.RunUntilOrTimeout([this]() { return sender_endpoint_.bytes_to_transfer() == 0; }, QuicTime::Delta::FromSeconds(800));
...
TEST_F(Bbr2DefaultTopologyTest, QUIC_SLOW_TEST(BandwidthIncreaseB203))
{
    ...
    bool simulator_result = simulator_.RunUntilOrTimeout([this]() { return sender_endpoint_.bytes_to_transfer() == 0; }, QuicTime::Delta::FromSeconds(50));
    EXPECT_TRUE(simulator_result);
    // Ensure the full bandwidth is discovered.
    EXPECT_APPROX_EQ(params.test_link.bandwidth, sender_->ExportDebugState().bandwidth_hi, 0.02f);
}
...
TEST_F(Bbr2DefaultTopologyTest, QUIC_SLOW_TEST(BandwidthIncreaseBBQ0Aggregation))
{
    SetConnectionOption(kBBQ0);
    DefaultTopologyParams params;
    params.local_link.bandwidth = QuicBandwidth::FromKBitsPerSecond(15000);
    ...
    EXPECT_APPROX_EQ(params.test_link.bandwidth, sender_->ExportDebugState().bandwidth_est, 0.6f);
    ...
    EXPECT_APPROX_EQ(params.test_link.bandwidth, sender_->ExportDebugState().bandwidth_hi, 0.90f);
}
...
TEST_F(Bbr2DefaultTopologyTest, QUIC_SLOW_TEST(BandwidthIncreaseB202))
{
    ...
    EXPECT_TRUE(simulator_result);
    // Ensure the full bandwidth is discovered.
    EXPECT_APPROX_EQ(params.test_link.bandwidth, sender_->ExportDebugState().bandwidth_hi, 0.1f);
}

// Test Bbr2's reaction to a 100x bandwidth increase during a transfer with B202
// in the presence of ACK aggregation.
TEST_F(Bbr2DefaultTopologyTest, QUIC_SLOW_TEST(BandwidthIncreaseB202Aggregation))
{
    SetConnectionOption(kB202);
    DefaultTopologyParams params;
    params.local_link.bandwidth = QuicBandwidth::FromKBitsPerSecond(15000);
    ...
    EXPECT_TRUE(simulator_result);
    // Ensure at least 10% of full bandwidth is discovered.
    EXPECT_APPROX_EQ(params.test_link.bandwidth, sender_->ExportDebugState().bandwidth_hi, 0.92f);
}
...
TEST_F(Bbr2DefaultTopologyTest, QUIC_SLOW_TEST(BandwidthIncreaseAggregation))
{
    ...
    // This is much farther off when aggregation is present,
    // Ideally BSAO or another option would fix this.
    EXPECT_APPROX_EQ(params.test_link.bandwidth, sender_->ExportDebugState().bandwidth_est, 0.60f);
    EXPECT_LE(sender_loss_rate_in_packets(), 0.35);
    ...
    EXPECT_TRUE(simulator_result);
    // Ensure at least 10% of full bandwidth is discovered.
    EXPECT_APPROX_EQ(params.test_link.bandwidth, sender_->ExportDebugState().bandwidth_hi, 0.91f);
}
...
// Test Bbr2's reaction to a 100x bandwidth increase during a transfer with BBHI
// in the presence of ACK aggregation.
TEST_F(Bbr2DefaultTopologyTest, QUIC_SLOW_TEST(BandwidthIncreaseBBHIAggregation))
{
    SetQuicReloadableFlag(quic_bbr2_simplify_inflight_hi, true);
    SetConnectionOption(kBBHI);
    DefaultTopologyParams params;
...
TEST_F(Bbr2DefaultTopologyTest, QUIC_SLOW_TEST(BandwidthIncreaseBBHI_B202Aggregation))
{
    SetQuicReloadableFlag(quic_bbr2_simplify_inflight_hi, true);
    SetConnectionOption(kBBHI);
    SetConnectionOption(kB202);
    ...
    EXPECT_APPROX_EQ(params.test_link.bandwidth, sender_->ExportDebugState().bandwidth_hi, 0.85f);
}
...
// Test Bbr2's reaction to a 100x bandwidth increase during a transfer with B204
// in the presence of ACK aggregation.
TEST_F(Bbr2DefaultTopologyTest, QUIC_SLOW_TEST(BandwidthIncreaseB204Aggregation))
{
    SetConnectionOption(kB204);
    DefaultTopologyParams params;
    params.local_link.bandwidth = QuicBandwidth::FromKBitsPerSecond(15000);
    ...
    EXPECT_TRUE(simulator_result);
    // Ensure at least 10% of full bandwidth is discovered.
    EXPECT_APPROX_EQ(params.test_link.bandwidth, sender_->ExportDebugState().bandwidth_hi, 0.95f);
}
...
// Test Bbr2's reaction to a 100x bandwidth increase during a transfer with B205
// in the presence of ACK aggregation.
TEST_F(Bbr2DefaultTopologyTest, QUIC_SLOW_TEST(BandwidthIncreaseB205Aggregation))
{
    SetConnectionOption(kB205);
    DefaultTopologyParams params;
    params.local_link.bandwidth = QuicBandwidth::FromKBitsPerSecond(15000);
    ...
    // This is much farther off when aggregation is present,
    // Ideally BSAO or another option would fix this.
    EXPECT_APPROX_EQ(params.test_link.bandwidth, sender_->ExportDebugState().bandwidth_est, 0.45f);
    ...
    EXPECT_APPROX_EQ(params.test_link.bandwidth, sender_->ExportDebugState().bandwidth_hi, 0.9f);
}
...
// Test Bbr2's reaction to a 100x bandwidth increase during a transfer with BB2U
// in the presence of ACK aggregation.
TEST_F(Bbr2DefaultTopologyTest, QUIC_SLOW_TEST(BandwidthIncreaseBB2UAggregation))
{
    SetQuicReloadableFlag(quic_bbr2_probe_two_rounds, true);
    SetConnectionOption(kBB2U);
    DefaultTopologyParams params;
...
TEST_F(Bbr2DefaultTopologyTest, QUIC_SLOW_TEST(BandwidthIncreaseBB2UandBBHIAggregation))
{
    SetQuicReloadableFlag(quic_bbr2_probe_two_rounds, true);
    SetConnectionOption(kBB2U);
    SetQuicReloadableFlag(quic_bbr2_simplify_inflight_hi, true);
...
TEST_F(Bbr2DefaultTopologyTest, PacketLossBBQ6SmallBufferStartup)
{
    ...
    EXPECT_EQ(sender_->ExportDebugState().bandwidth_lo, QuicBandwidth::Infinite());
}
...
TEST_F(Bbr2DefaultTopologyTest, Drain)
{
    ...
    simulator_result = simulator_.RunUntilOrTimeout([this]() { return sender_->ExportDebugState().mode != Bbr2Mode::STARTUP; }, timeout);
    ASSERT_TRUE(simulator_result);
    ASSERT_EQ(Bbr2Mode::DRAIN, sender_->ExportDebugState().mode);
    EXPECT_APPROX_EQ(sender_->BandwidthEstimate() * (1 / 2.885f), sender_->PacingRate(0), 0.01f);
    ...
    const QuicTime::Delta queueing_delay = params.test_link.bandwidth.TransferTime(queue->bytes_queued());
    EXPECT_APPROX_EQ(params.RTT() + queueing_delay, rtt_stats()->latest_rtt(), 0.1f);

    // Transition to the drain phase and verify that it makes the queue
    // have at most a BDP worth of packets.
    simulator_result = simulator_.RunUntilOrTimeout([this]() { return sender_->ExportDebugState().mode != Bbr2Mode::DRAIN; }, timeout);
    ASSERT_TRUE(simulator_result);
    ...
    const QuicRoundTripCount start_round_trip = sender_->ExportDebugState().round_trip_count;
    simulator_result = simulator_.RunUntilOrTimeout(
        [this, start_round_trip]() {
            const auto& debug_state = sender_->ExportDebugState();
            QuicRoundTripCount rounds_passed = debug_state.round_trip_count - start_round_trip;
            return rounds_passed >= 4 && debug_state.mode == Bbr2Mode::PROBE_BW && debug_state.probe_bw.phase == CyclePhase::PROBE_REFILL;
        },
        timeout);
    ASSERT_TRUE(simulator_result);
...
TEST_F(Bbr2DefaultTopologyTest, InFlightAwareGainCycling)
{
    ...
    simulator_result = SendUntilOrTimeout([this]() { return sender_->ExportDebugState().probe_bw.phase == CyclePhase::PROBE_REFILL; }, timeout);
    ASSERT_TRUE(simulator_result);
    ...
    if (GetQuicReloadableFlag(quic_pacing_remove_non_initial_burst)) {
        QuicSentPacketManagerPeer::GetPacingSender(&sender_connection()->sent_packet_manager())->SetBurstTokens(10);
    }
    ...
    simulator_result = simulator_.RunUntilOrTimeout([this]() { return sender_->ExportDebugState().probe_bw.phase == CyclePhase::PROBE_DOWN; }, timeout);
    ASSERT_TRUE(simulator_result);
    simulator_.RunFor(0.75 * sender_->ExportDebugState().min_rtt);
    EXPECT_EQ(Bbr2Mode::PROBE_BW, sender_->ExportDebugState().mode);
    EXPECT_EQ(CyclePhase::PROBE_CRUISE, sender_->ExportDebugState().probe_bw.phase);
}
...
TEST_F(Bbr2DefaultTopologyTest, ExitStartupDueToLossB2NE)
{
    ...
    EXPECT_EQ(0u, sender_->ExportDebugState().startup.round_trips_without_bandwidth_growth);
    EXPECT_NE(0u, sender_connection_stats().packets_lost);
}
...
TEST_F(Bbr2DefaultTopologyTest, SenderPoliced)
{
    ...
    params.sender_policer_params->target_bandwidth = params.BottleneckBandwidth() * 0.25;
...
TEST_F(Bbr2DefaultTopologyTest, StartupStats)
{
    ...
    EXPECT_THAT(stats.slowstart_duration.GetTotalElapsedTime(), AllOf(Ge(QuicTime::Delta::FromMilliseconds(500)), Le(QuicTime::Delta::FromMilliseconds(1500))));
    EXPECT_EQ(stats.slowstart_duration.GetTotalElapsedTime(), QuicConnectionPeer::GetSentPacketManager(sender_connection())->GetSlowStartDuration());
}
...
TEST_F(Bbr2DefaultTopologyTest, ProbeUpAdaptInflightHiGradually)
{
    ...
    QuicPacketNumber acked_packet_number = sender_unacked_map()->GetLeastUnacked();
    for (auto& info : *sender_unacked_map()) {
        acked_packets.emplace_back(acked_packet_number++, info.bytes_sent, SimulatedNow());
    }
    ...
    sender_->OnCongestionEvent(
        /*rtt_updated=*/true, sender_unacked_map()->bytes_in_flight(), now, acked_packets, {}, 0, 0);
    ASSERT_EQ(CyclePhase::PROBE_REFILL, sender_->ExportDebugState().probe_bw.phase);

    // Send and Ack one packet to exit app limited and enter PROBE_UP.
    sender_->OnPacketSent(now, /*bytes_in_flight=*/0, next_packet_number++, kDefaultMaxPacketSize, HAS_RETRANSMITTABLE_DATA);
    now = now + params.RTT();
    sender_->OnCongestionEvent(
        /*rtt_updated=*/true, kDefaultMaxPacketSize, now, { AckedPacket(next_packet_number - 1, kDefaultMaxPacketSize, now) }, {}, 0, 0);
    ...
    for (uint64_t i = 0; i < 2; ++i) {
        sender_->OnPacketSent(now, /*bytes_in_flight=*/i * kDefaultMaxPacketSize, next_packet_number++, kDefaultMaxPacketSize, HAS_RETRANSMITTABLE_DATA);
    }
    now = now + params.RTT();
    sender_->OnCongestionEvent(
        /*rtt_updated=*/true, 2 * kDefaultMaxPacketSize, now, { AckedPacket(next_packet_number - 1, kDefaultMaxPacketSize, now) },
...
TEST_F(Bbr2DefaultTopologyTest, LossOnlyCongestionEvent)
{
    ...
    bool simulator_result = simulator_.RunUntilOrTimeout([&]() { return sender_unacked_map()->bytes_in_flight() > 0; }, QuicTime::Delta::FromSeconds(5));
    ASSERT_TRUE(simulator_result);

    const QuicBandwidth prior_bandwidth_estimate = sender_->BandwidthEstimate();
    EXPECT_APPROX_EQ(params.BottleneckBandwidth(), prior_bandwidth_estimate, 0.01f);

    // Lose the least unacked packet.
    LostPacketVector lost_packets;
    lost_packets.emplace_back(sender_connection()->sent_packet_manager().GetLeastUnacked(), kDefaultMaxPacketSize);

    QuicTime now = simulator_.GetClock()->Now() + params.RTT() * 0.25;
    sender_->OnCongestionEvent(false, sender_unacked_map()->bytes_in_flight(), now, {}, lost_packets, 0, 0);
...
TEST_F(Bbr2DefaultTopologyTest, SpuriousLossEvent)
{
    ...
    LostPacketVector lost_packets;
    for (QuicPacketNumber packet_number = sender_unacked_map()->GetLeastUnacked(); sender_unacked_map()->HasInFlightPackets(); packet_number++) {
    ...
    sender_->OnCongestionEvent(false, prior_inflight, now, {}, lost_packets, 0, 0);
    ...
    sender_->OnCongestionEvent(false, sender_unacked_map()->bytes_in_flight(), now, acked_packets, {}, 0, 0);

    EXPECT_EQ(sender_->GetNetworkModel().total_bytes_sent(), sender_->GetNetworkModel().total_bytes_acked() + sender_->GetNetworkModel().total_bytes_lost());
}
...
TEST_F(Bbr2DefaultTopologyTest, ProbeRttAfterQuiescenceImmediatelyExits)
{
    ...
    simulator_result = SendUntilOrTimeout([this]() { return sender_->ExportDebugState().mode == Bbr2Mode::PROBE_RTT; }, timeout);
    ASSERT_TRUE(simulator_result);

    // Wait for entering a quiescence of 5 seconds.
    ASSERT_TRUE(simulator_.RunUntilOrTimeout(
        [this]() { return sender_unacked_map()->bytes_in_flight() == 0 && sender_->ExportDebugState().mode == Bbr2Mode::PROBE_RTT; }, timeout));
    ...
    sender_->OnPacketSent(
        SimulatedNow(), /*bytes_in_flight=*/0, sender_unacked_map()->largest_sent_packet() + 1, kDefaultMaxPacketSize, HAS_RETRANSMITTABLE_DATA);
...
TEST_F(Bbr2DefaultTopologyTest, ProbeBwAfterQuiescencePostponeMinRttTimestamp)
{
    ...
    const QuicTime min_rtt_timestamp_before_idle = sender_->ExportDebugState().min_rtt_timestamp;

    // Wait for entering a quiescence of 15 seconds.
    ASSERT_TRUE(simulator_.RunUntilOrTimeout([this]() { return sender_unacked_map()->bytes_in_flight() == 0; }, params.RTT() + timeout));

    simulator_.RunFor(QuicTime::Delta::FromSeconds(15));

    // Send some data to exit quiescence.
    SendBursts(params, 1, kDefaultTCPMSS, QuicTime::Delta::Zero());
    const QuicTime min_rtt_timestamp_after_idle = sender_->ExportDebugState().min_rtt_timestamp;

    EXPECT_LT(min_rtt_timestamp_before_idle + QuicTime::Delta::FromSeconds(14), min_rtt_timestamp_after_idle);
}

TEST_F(Bbr2DefaultTopologyTest, SwitchToBbr2MidConnection)
{
    QuicTime now = QuicTime::Zero();
    BbrSender old_sender(sender_connection()->clock()->Now(), sender_connection()->sent_packet_manager().GetRttStats(), GetUnackedMap(sender_connection()),
        kDefaultInitialCwndPackets + 1, GetQuicFlag(quic_max_congestion_window), &random_, QuicConnectionPeer::GetStats(sender_connection()));
...
TEST_F(Bbr2DefaultTopologyTest, AdjustNetworkParameters)
{
    ...
    sender_connection()->AdjustNetworkParameters(SendAlgorithmInterface::NetworkParams(params.BottleneckBandwidth(), params.RTT(),
        /*allow_cwnd_to_decrease=*/false));

    EXPECT_EQ(params.BDP(), sender_->ExportDebugState().congestion_window);

    EXPECT_EQ(params.BottleneckBandwidth(), sender_->PacingRate(/*bytes_in_flight=*/0));
...
TEST_F(Bbr2DefaultTopologyTest, 200InitialCongestionWindowWithNetworkParameterAdjusted)
{
    DefaultTopologyParams params;
    CreateNetwork(params);
    ...
    const QuicTime::Delta timeout = QuicTime::Delta::FromSeconds(5);
    bool simulator_result = simulator_.RunUntilOrTimeout([this]() { return !sender_->ExportDebugState().min_rtt.IsZero(); }, timeout);
    ASSERT_TRUE(simulator_result);

    // Bootstrap cwnd by a overly large bandwidth sample.
    sender_connection()->AdjustNetworkParameters(SendAlgorithmInterface::NetworkParams(1024 * params.BottleneckBandwidth(), QuicTime::Delta::Zero(), false));

    // Verify cwnd is capped at 200.
    EXPECT_EQ(200 * kDefaultTCPMSS, sender_->ExportDebugState().congestion_window);
    EXPECT_GT(1024 * params.BottleneckBandwidth(), sender_->PacingRate(0));
}

TEST_F(Bbr2DefaultTopologyTest, 100InitialCongestionWindowFromNetworkParameter)
{
    DefaultTopologyParams params;
    CreateNetwork(params);
    ...
    // Bootstrap cwnd by a overly large bandwidth sample.
    SendAlgorithmInterface::NetworkParams network_params(1024 * params.BottleneckBandwidth(), QuicTime::Delta::Zero(), false);
    ...
    EXPECT_EQ(100 * kDefaultTCPMSS, sender_->ExportDebugState().congestion_window);
    EXPECT_GT(1024 * params.BottleneckBandwidth(), sender_->PacingRate(0));
}

TEST_F(Bbr2DefaultTopologyTest, 100InitialCongestionWindowWithNetworkParameterAdjusted)
{
...
public:
    ...
    std::array<LinkParams, kNumLocalLinks> local_links = {
        LinkParams(10000, 1987),
        LinkParams(10000, 1993),
        LinkParams(10000, 1997),
        LinkParams(10000, 1999),
        LinkParams(10000, 2003),
        LinkParams(10000, 2011),
        LinkParams(10000, 2017),
        LinkParams(10000, 2027),
    };
    ...
    {
        return 2
            * (local_links[n].delay + test_link.delay + local_links[n].bandwidth.TransferTime(kMaxOutgoingPacketSize)
                + test_link.bandwidth.TransferTime(kMaxOutgoingPacketSize));
    }

    QuicByteCount Bdp(size_t n) const
    {
        return BottleneckBandwidth() * Rtt(n);
    }
...
protected:
    Bbr2MultiSenderTest()
    {
        ...
        for (size_t i = 0; i < MultiSenderTopologyParams::kNumLocalLinks; ++i) {
            ...
            sender_endpoints_.push_back(std::make_unique<simulator::QuicEndpoint>(
                &simulator_, sender_name, receiver_name, Perspective::IS_CLIENT, TestConnectionId(first_connection_id + i)));
            receiver_endpoints_.push_back(std::make_unique<simulator::QuicEndpoint>(
                &simulator_, receiver_name, sender_name, Perspective::IS_SERVER, TestConnectionId(first_connection_id + i)));
            receiver_endpoint_pointers.push_back(receiver_endpoints_.back().get());
        }
        receiver_multiplexer_ = std::make_unique<simulator::QuicEndpointMultiplexer>("Receiver multiplexer", receiver_endpoint_pointers);
    ...
    {
        const auto* test_info = ::testing::UnitTest::GetInstance()->current_test_info();
        QUIC_LOG(INFO) << "Bbr2MultiSenderTest." << test_info->name() << " completed at simulated time: " << SimulatedNow().ToDebuggingValue() / 1e6
                       << " sec. Per sender stats:";
        for (size_t i = 0; i < sender_endpoints_.size(); ++i) {
            QUIC_LOG(INFO) << "sender[" << i << "]: " << sender_connection(i)->sent_packet_manager().GetSendAlgorithm()->GetCongestionControlType()
                           << ", packet_loss:" << 100.0 * sender_loss_rate_in_packets(i) << "%";
        }
    }
    ...
    Bbr2Sender* SetupBbr2Sender(simulator::QuicEndpoint* endpoint)
    {
        // Ownership of the sender will be overtaken by the endpoint.
        Bbr2Sender* sender = new Bbr2Sender(endpoint->connection()->clock()->Now(), endpoint->connection()->sent_packet_manager().GetRttStats(),
            QuicSentPacketManagerPeer::GetUnackedPacketMap(QuicConnectionPeer::GetSentPacketManager(endpoint->connection())), kDefaultInitialCwndPackets,
            GetQuicFlag(quic_max_congestion_window), &random_, QuicConnectionPeer::GetStats(endpoint->connection()), nullptr);
    ...
    {
        // Ownership of the sender will be overtaken by the endpoint.
        BbrSender* sender = new BbrSender(endpoint->connection()->clock()->Now(), endpoint->connection()->sent_packet_manager().GetRttStats(),
            QuicSentPacketManagerPeer::GetUnackedPacketMap(QuicConnectionPeer::GetSentPacketManager(endpoint->connection())), kDefaultInitialCwndPackets,
            GetQuicFlag(quic_max_congestion_window), &random_, QuicConnectionPeer::GetStats(endpoint->connection()));
        QuicConnectionPeer::SetSendAlgorithm(endpoint->connection(), sender);
        endpoint->RecordTrace();
        return sender;
    }
    ...
    TcpCubicSenderBytes* SetupTcpSender(simulator::QuicEndpoint* endpoint, bool reno)
    {
        // Ownership of the sender will be overtaken by the endpoint.
        TcpCubicSenderBytes* sender = new TcpCubicSenderBytes(endpoint->connection()->clock(), endpoint->connection()->sent_packet_manager().GetRttStats(),
            reno, kDefaultInitialCwndPackets, GetQuicFlag(quic_max_congestion_window), QuicConnectionPeer::GetStats(endpoint->connection()));
        QuicConnectionPeer::SetSendAlgorithm(endpoint->connection(), sender);
        endpoint->RecordTrace();
        return sender;
    }
    ...
    void CreateNetwork(const MultiSenderTopologyParams& params)
    {
        ...
        network_links_.push_back(
            std::make_unique<simulator::SymmetricLink>(receiver_multiplexer_.get(), switch_->port(1), params.test_link.bandwidth, params.test_link.delay));
        for (size_t i = 0; i < MultiSenderTopologyParams::kNumLocalLinks; ++i) {
            simulator::SwitchPortNumber port_number = i + 2;
            network_links_.push_back(std::make_unique<simulator::SymmetricLink>(
                sender_endpoints_[i].get(), switch_->port(port_number), params.local_links[i].bandwidth, params.local_links[i].delay));
        }
    }
    ...
    {
        return static_cast<float>(sender_connection_stats(which).packets_lost) / sender_connection_stats(which).packets_sent;
    }
...
TEST_F(Bbr2MultiSenderTest, Bbr2VsBbr2)
{
    ...
    const QuicTime::Delta transfer_time = params.BottleneckBandwidth().TransferTime(transfer_size);
    ...
    bool simulator_result = simulator_.RunUntilOrTimeout([this]() { return receiver_endpoints_[0]->bytes_received() >= 0.1 * transfer_size; }, transfer_time);
    ASSERT_TRUE(simulator_result);

    // Start the second transfer and wait until both finish.
    sender_endpoints_[1]->AddBytesToTransfer(transfer_size);
    simulator_result = simulator_.RunUntilOrTimeout(
        [this]() { return receiver_endpoints_[0]->bytes_received() == transfer_size && receiver_endpoints_[1]->bytes_received() == transfer_size; },
        3 * transfer_time);
    ASSERT_TRUE(simulator_result);
}
...
TEST_F(Bbr2MultiSenderTest, QUIC_SLOW_TEST(MultipleBbr2s))
{
    ...
    const QuicByteCount transfer_size = 10 * 1024 * 1024;
    const QuicTime::Delta transfer_time = params.BottleneckBandwidth().TransferTime(transfer_size);
    QUIC_LOG(INFO) << "Single flow transfer time: " << transfer_time << ". Now: " << SimulatedNow();

    // Start all transfers.
    for (int i = 0; i < kTotalNumSenders; ++i) {
        if (i != 0) {
            const QuicTime sender_start_time = SimulatedNow() + QuicTime::Delta::FromSeconds(2);
            bool simulator_result = simulator_.RunUntilOrTimeout([&]() { return SimulatedNow() >= sender_start_time; }, transfer_time);
            ASSERT_TRUE(simulator_result);
        }
    ...
    QuicTime::Delta expected_total_transfer_time_upper_bound = QuicTime::Delta::FromMicroseconds(kTotalNumSenders * transfer_time.ToMicroseconds() * 1.1);
    bool simulator_result = simulator_.RunUntilOrTimeout(
        [this]() {
            for (int i = 0; i < kTotalNumSenders; ++i) {
...
{
    auto time_in_seconds = [](int64_t seconds) { return QuicTime::Zero() + QuicTime::Delta::FromSeconds(seconds); };

    MinRttFilter filter(QuicTime::Delta::FromMilliseconds(10), time_in_seconds(100));
    ...
    filter.ForceUpdate(QuicTime::Delta::FromMilliseconds(-2), time_in_seconds(200));
