...
protected:
    ...
    void InitPacingRate(QuicPacketCount burst_size, QuicBandwidth bandwidth)
    {
        ...
        if (burst_size == 0) {
            ...
            pacing_sender_->OnCongestionEvent(true, 1234, clock_.Now(), empty, lost_packets, 0, 0);
        } else if (burst_size != kInitialBurstPackets) {
            QUIC_LOG(FATAL) << "Unsupported burst_size " << burst_size << " specificied, only 0 and " << kInitialBurstPackets << " are supported.";
        }
    }

    void CheckPacketIsSentImmediately(HasRetransmittableData retransmittable_data, QuicByteCount prior_in_flight, bool in_recovery, QuicPacketCount cwnd)
    {
        // In order for the packet to be sendable, the underlying sender must
        // permit it to be sent immediately.
        for (int i = 0; i < 2; ++i) {
            EXPECT_CALL(*mock_sender_, CanSend(prior_in_flight)).WillOnce(Return(true));
            // Verify that the packet can be sent immediately.
            EXPECT_EQ(zero_time_, pacing_sender_->TimeUntilSend(clock_.Now(), prior_in_flight)) << "Next packet to send is " << packet_number_;
        }

        // Actually send the packet.
        if (prior_in_flight == 0 && !GetQuicReloadableFlag(quic_pacing_remove_non_initial_burst)) {
            EXPECT_CALL(*mock_sender_, InRecovery()).WillOnce(Return(in_recovery));
        }
        EXPECT_CALL(*mock_sender_, OnPacketSent(clock_.Now(), prior_in_flight, packet_number_, kMaxOutgoingPacketSize, retransmittable_data));
        EXPECT_CALL(*mock_sender_, GetCongestionWindow()).WillRepeatedly(Return(cwnd * kDefaultTCPMSS));
        EXPECT_CALL(*mock_sender_, CanSend(prior_in_flight + kMaxOutgoingPacketSize))
            .Times(AtMost(1))
            .WillRepeatedly(Return((prior_in_flight + kMaxOutgoingPacketSize) < (cwnd * kDefaultTCPMSS)));
        pacing_sender_->OnPacketSent(clock_.Now(), prior_in_flight, packet_number_++, kMaxOutgoingPacketSize, retransmittable_data);
    }

    void CheckPacketIsSentImmediately()
    {
        CheckPacketIsSentImmediately(HAS_RETRANSMITTABLE_DATA, kBytesInFlight, false, 10);
    }
    ...
    void CheckPacketIsDelayed(QuicTime::Delta delay)
    {
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            EXPECT_EQ(delay.ToMicroseconds(), pacing_sender_->TimeUntilSend(clock_.Now(), kBytesInFlight).ToMicroseconds());
        }
    }

    void UpdateRtt()
    {
        EXPECT_CALL(*mock_sender_, OnCongestionEvent(true, kBytesInFlight, _, _, _, _, _));
        AckedPacketVector empty_acked;
        LostPacketVector empty_lost;
        pacing_sender_->OnCongestionEvent(true, kBytesInFlight, clock_.Now(), empty_acked, empty_lost, 0, 0);
    }
...
TEST_F(PacingSenderTest, NoSend)
{
    for (int i = 0; i < 2; ++i) {
        ...
        EXPECT_EQ(infinite_time_, pacing_sender_->TimeUntilSend(clock_.Now(), kBytesInFlight));
    }
}
...
TEST_F(PacingSenderTest, SendNow)
{
    for (int i = 0; i < 2; ++i) {
        EXPECT_CALL(*mock_sender_, CanSend(kBytesInFlight)).WillOnce(Return(true));
        EXPECT_EQ(zero_time_, pacing_sender_->TimeUntilSend(clock_.Now(), kBytesInFlight));
    }
}
...
TEST_F(PacingSenderTest, VariousSending)
{
    ...
    InitPacingRate(0, QuicBandwidth::FromBytesAndTimeDelta(kMaxOutgoingPacketSize, QuicTime::Delta::FromMilliseconds(1)));
...
TEST_F(PacingSenderTest, InitialBurst)
{
    ...
    InitPacingRate(10, QuicBandwidth::FromBytesAndTimeDelta(kMaxOutgoingPacketSize, QuicTime::Delta::FromMilliseconds(1)));
...
TEST_F(PacingSenderTest, FastSending)
{
    ...
    InitPacingRate(10, QuicBandwidth::FromBytesAndTimeDelta(2 * kMaxOutgoingPacketSize, QuicTime::Delta::FromMilliseconds(1)));
    // Update the RTT and verify that the first 10 packets aren't paced.
    UpdateRtt();
...
TEST_F(PacingSenderTest, NoBurstEnteringRecovery)
{
    ...
    lost_packets.push_back(LostPacket(QuicPacketNumber(1), kMaxOutgoingPacketSize));
    AckedPacketVector empty_acked;
    EXPECT_CALL(*mock_sender_, OnCongestionEvent(true, kMaxOutgoingPacketSize, _, testing::IsEmpty(), _, _, _));
    pacing_sender_->OnCongestionEvent(true, kMaxOutgoingPacketSize, clock_.Now(), empty_acked, lost_packets, 0, 0);
    // One packet is sent immediately, because of 1ms pacing granularity.
    CheckPacketIsSentImmediately();
    // Ensure packets are immediately paced.
    EXPECT_CALL(*mock_sender_, CanSend(kMaxOutgoingPacketSize)).WillOnce(Return(true));
    // Verify the next packet is paced and delayed 2ms due to granularity.
    EXPECT_EQ(QuicTime::Delta::FromMilliseconds(2), pacing_sender_->TimeUntilSend(clock_.Now(), kMaxOutgoingPacketSize));
    CheckPacketIsDelayed(QuicTime::Delta::FromMilliseconds(2));
}
...
TEST_F(PacingSenderTest, CwndLimited)
{
    ...
    CheckPacketIsSentImmediately(HAS_RETRANSMITTABLE_DATA, 2 * kMaxOutgoingPacketSize, false, 2);
...
TEST_F(PacingSenderTest, LumpyPacingWithInitialBurstToken)
{
    ...
    CheckPacketIsSentImmediately(HAS_RETRANSMITTABLE_DATA, 20 * kMaxOutgoingPacketSize, false, 20);

    clock_.AdvanceTime(QuicTime::Delta::FromMilliseconds(100));
    // Suppose cwnd size is 5, so that lumpy size becomes 2.
    CheckPacketIsSentImmediately(HAS_RETRANSMITTABLE_DATA, kBytesInFlight, false, 5);
    CheckPacketIsSentImmediately();
...
TEST_F(PacingSenderTest, NoLumpyPacingForLowBandwidthFlows)
{
    ...
    InitPacingRate(kInitialBurstPackets, QuicBandwidth::FromBytesAndTimeDelta(kMaxOutgoingPacketSize, inter_packet_delay));
    UpdateRtt();
...
TEST_F(PacingSenderTest, NoBurstsForLumpyPacingWithAckAggregation)
{
    ...
    CheckPacketIsSentImmediately(HAS_RETRANSMITTABLE_DATA, 10 * kMaxOutgoingPacketSize, false, 10);
...
TEST_F(PacingSenderTest, IdealNextPacketSendTimeWithLumpyPacing)
{
    ...
    CheckPacketIsSentImmediately();
    EXPECT_EQ(pacing_sender_->ideal_next_packet_send_time(), clock_.Now() + inter_packet_delay);
    EXPECT_EQ(pacing_sender_->lumpy_tokens(), 2u);

    CheckPacketIsSentImmediately();
    EXPECT_EQ(pacing_sender_->ideal_next_packet_send_time(), clock_.Now() + 2 * inter_packet_delay);
    EXPECT_EQ(pacing_sender_->lumpy_tokens(), 1u);

    CheckPacketIsSentImmediately();
    EXPECT_EQ(pacing_sender_->ideal_next_packet_send_time(), clock_.Now() + 3 * inter_packet_delay);
    ...
    CheckPacketIsSentImmediately();
    EXPECT_EQ(pacing_sender_->ideal_next_packet_send_time(), clock_.Now() - 0.5 * inter_packet_delay);
    EXPECT_EQ(pacing_sender_->lumpy_tokens(), 2u);
    ...
    CheckPacketIsSentImmediately();
    EXPECT_EQ(pacing_sender_->ideal_next_packet_send_time(), clock_.Now() + 1.5 * inter_packet_delay);
    EXPECT_EQ(pacing_sender_->lumpy_tokens(), 0u);
