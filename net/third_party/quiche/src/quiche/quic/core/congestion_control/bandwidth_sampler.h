...
struct QUICHE_EXPORT SendTimeState {
    ...
    SendTimeState(
        bool is_app_limited, QuicByteCount total_bytes_sent, QuicByteCount total_bytes_acked, QuicByteCount total_bytes_lost, QuicByteCount bytes_in_flight)
        : is_valid(true)
    ...
    friend QUICHE_EXPORT std::ostream& operator<<(std::ostream& os, const SendTimeState& s);
...
public:
    ...
    QuicByteCount Update(QuicBandwidth bandwidth_estimate, bool is_new_max_bandwidth, QuicRoundTripCount round_trip_count,
...
private:
    ...
    // bandwidth.
    using MaxAckHeightFilter = WindowedFilter<ExtraAckedEvent, MaxFilter<ExtraAckedEvent>, QuicRoundTripCount, QuicRoundTripCount>;
    ...
    double ack_aggregation_bandwidth_threshold_ = GetQuicFlag(quic_ack_aggregation_bandwidth_threshold);
...
public:
    ...
    virtual CongestionEventSample OnCongestionEvent(QuicTime ack_time, const AckedPacketVector& acked_packets, const LostPacketVector& lost_packets,
        QuicBandwidth max_bandwidth, QuicBandwidth est_bandwidth_upper_bound, QuicRoundTripCount round_trip_count)
        = 0;
...
public:
    BandwidthSampler(const QuicUnackedPacketMap* unacked_packet_map, QuicRoundTripCount max_height_tracker_window_length);
    ...
    void OnPacketSent(QuicTime sent_time, QuicPacketNumber packet_number, QuicByteCount bytes, QuicByteCount bytes_in_flight,
        HasRetransmittableData has_retransmittable_data) override;
    void OnPacketNeutered(QuicPacketNumber packet_number) override;

    CongestionEventSample OnCongestionEvent(QuicTime ack_time, const AckedPacketVector& acked_packets, const LostPacketVector& lost_packets,
        QuicBandwidth max_bandwidth, QuicBandwidth est_bandwidth_upper_bound, QuicRoundTripCount round_trip_count) override;
    QuicByteCount OnAckEventEnd(QuicBandwidth bandwidth_estimate, bool is_new_max_bandwidth, QuicRoundTripCount round_trip_count);
    ...
    QuicByteCount max_ack_height() const
    {
        return max_ack_height_tracker_.Get();
    }
    ...
    void ResetMaxAckHeightTracker(QuicByteCount new_height, QuicRoundTripCount new_time)
    {
    ...
    struct QUICHE_EXPORT AckPoint {
        ...
        friend QUICHE_EXPORT std::ostream& operator<<(std::ostream& os, const AckPoint& ack_point)
        {
    ...
    public:
        ...
        void Clear()
        {
            ack_points_[0] = ack_points_[1] = AckPoint();
        }

        const AckPoint& MostRecentPoint() const
        {
...
private:
    ...
    public:
        ...
        QuicTime sent_time() const
        {
            return sent_time_;
        }

        // Size of the packet.
        QuicByteCount size() const
        {
            return size_;
        }
        ...
        const SendTimeState& send_time_state() const
        {
            return send_time_state_;
        }
        ...
        ConnectionStateOnSentPacket(QuicTime sent_time, QuicByteCount size, QuicByteCount bytes_in_flight, const BandwidthSampler& sampler)
            : sent_time_(sent_time)
            , size_(size)
            , total_bytes_sent_at_last_acked_packet_(sampler.total_bytes_sent_at_last_acked_packet_)
            ...
            , send_time_state_(sampler.is_app_limited_, sampler.total_bytes_sent_, sampler.total_bytes_acked_, sampler.total_bytes_lost_, bytes_in_flight)
        {
        }
        ...
        friend QUICHE_EXPORT std::ostream& operator<<(std::ostream& os, const ConnectionStateOnSentPacket& p)
        {
            os << "{sent_time:" << p.sent_time() << ", size:" << p.size()
               << ", total_bytes_sent_at_last_acked_packet:" << p.total_bytes_sent_at_last_acked_packet()
               << ", last_acked_packet_sent_time:" << p.last_acked_packet_sent_time() << ", last_acked_packet_ack_time:" << p.last_acked_packet_ack_time()
    ...
    BandwidthSample OnPacketAcknowledged(QuicTime ack_time, QuicPacketNumber packet_number);

    SendTimeState OnPacketLost(QuicPacketNumber packet_number, QuicPacketLength bytes_lost);
    ...
    void SentPacketToSendTimeState(const ConnectionStateOnSentPacket& sent_packet, SendTimeState* send_time_state) const;
    ...
    BandwidthSample OnPacketAcknowledgedInner(QuicTime ack_time, QuicPacketNumber packet_number, const ConnectionStateOnSentPacket& sent_packet);
