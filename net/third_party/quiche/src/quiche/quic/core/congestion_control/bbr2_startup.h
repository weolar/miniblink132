...
public:
    Bbr2StartupMode(const Bbr2Sender* sender, Bbr2NetworkModel* model, QuicTime now);
    ...
    Limits<QuicByteCount> GetCwndLimits() const override
    {
        ...
        QUICHE_DCHECK_EQ(Bbr2NetworkModel::inflight_lo_default(), model_->inflight_lo());
...
QUICHE_EXPORT std::ostream& operator<<(std::ostream& os, const Bbr2StartupMode::DebugState& state);
