...
TEST_F(PrrSenderTest, SingleLossResultsInSendOnEveryOtherAck)
{
    ...
    EXPECT_TRUE(prr.CanSend(congestion_window, bytes_in_flight, ssthresh_after_loss * kMaxSegmentSize));
    // Send retransmission.
    prr.OnPacketSent(kMaxSegmentSize);
    // PRR shouldn't allow sending any more packets.
    EXPECT_FALSE(prr.CanSend(congestion_window, bytes_in_flight, ssthresh_after_loss * kMaxSegmentSize));
