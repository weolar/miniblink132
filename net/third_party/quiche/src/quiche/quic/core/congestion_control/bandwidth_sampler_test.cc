...
public:
    ...
    static QuicByteCount GetPacketSize(const BandwidthSampler& sampler, QuicPacketNumber packet_number)
    {
...
static_assert((kRegularPacketSize & 31) == 0, "kRegularPacketSize has to be five times divisible by 2");
...
{
    return p.overestimate_avoidance ? "enable_overestimate_avoidance" : "no_enable_overestimate_avoidance";
}
...
protected:
    ...
    void SendPacketInner(uint64_t packet_number, QuicByteCount bytes, HasRetransmittableData has_retransmittable_data)
    {
        sampler_.OnPacketSent(clock_.Now(), QuicPacketNumber(packet_number), bytes, bytes_in_flight_, has_retransmittable_data);
    ...
    {
        SendPacketInner(packet_number, kRegularPacketSize, HAS_RETRANSMITTABLE_DATA);
    }

    BandwidthSample AckPacketInner(uint64_t packet_number)
    {
        QuicByteCount size = BandwidthSamplerPeer::GetPacketSize(sampler_, QuicPacketNumber(packet_number));
    ...
    {
        return LostPacket(QuicPacketNumber(packet_number), BandwidthSamplerPeer::GetPacketSize(sampler_, QuicPacketNumber(packet_number)));
    }
    ...
    BandwidthSampler::CongestionEventSample OnCongestionEvent(std::set<uint64_t> acked_packet_numbers, std::set<uint64_t> lost_packet_numbers)
    {
        AckedPacketVector acked_packets;
        for (auto it = acked_packet_numbers.begin(); it != acked_packet_numbers.end(); ++it) {
            acked_packets.push_back(MakeAckedPacket(*it));
            bytes_in_flight_ -= acked_packets.back().bytes_acked;
        }

        LostPacketVector lost_packets;
        for (auto it = lost_packet_numbers.begin(); it != lost_packet_numbers.end(); ++it) {
            lost_packets.push_back(MakeLostPacket(*it));
            bytes_in_flight_ -= lost_packets.back().bytes_lost;
        }

        BandwidthSampler::CongestionEventSample sample
            = sampler_.OnCongestionEvent(clock_.Now(), acked_packets, lost_packets, max_bandwidth_, est_bandwidth_upper_bound_, round_trip_count_);
        max_bandwidth_ = std::max(max_bandwidth_, sample.sample_max_bandwidth);
        return sample;
    }
...
INSTANTIATE_TEST_SUITE_P(BandwidthSamplerTests, BandwidthSamplerTest,
    testing::Values(TestParameters { /*overestimate_avoidance=*/false }, TestParameters { /*overestimate_avoidance=*/true }),
...
TEST_P(BandwidthSamplerTest, SendAndWait)
{
    ...
    QuicBandwidth expected_bandwidth = QuicBandwidth::FromBytesPerSecond(kRegularPacketSize * 100);
...
TEST_P(BandwidthSamplerTest, SendTimeState)
{
    ...
    for (int i = 4; i <= 10; i++) {
        ...
        EXPECT_EQ(send_time_state.total_bytes_sent - send_time_state.total_bytes_acked - send_time_state.total_bytes_lost, send_time_state.bytes_in_flight);
...
{
    const QuicTime::Delta time_between_packets = QuicTime::Delta::FromMilliseconds(1);
    QuicBandwidth expected_bandwidth = QuicBandwidth::FromKBytesPerSecond(kRegularPacketSize);
...
{
    const QuicTime::Delta time_between_packets = QuicTime::Delta::FromMilliseconds(1);
    QuicBandwidth expected_bandwidth = QuicBandwidth::FromKBytesPerSecond(kRegularPacketSize) * 0.5;
...
TEST_P(BandwidthSamplerTest, NotCongestionControlled)
{
    ...
    // controlled.
    for (int i = 1; i <= 20; i++) {
        SendPacketInner(i, kRegularPacketSize, i % 2 == 0 ? HAS_RETRANSMITTABLE_DATA : NO_RETRANSMITTABLE_DATA);
        clock_.AdvanceTime(time_between_packets);
    }
    ...
    for (int i = 1; i <= 20; i++) {
        ...
        SendPacketInner(i + 20, kRegularPacketSize, i % 2 == 0 ? HAS_RETRANSMITTABLE_DATA : NO_RETRANSMITTABLE_DATA);
        clock_.AdvanceTime(time_between_packets);
    }
...
TEST_P(BandwidthSamplerTest, CompressedAck)
{
    ...
    QuicBandwidth last_bandwidth = QuicBandwidth::Zero();
    QuicTime::Delta ridiculously_small_time_delta = QuicTime::Delta::FromMicroseconds(20);
    for (int i = 21; i <= 40; i++) {
        last_bandwidth = AckPacket(i);
...
TEST_P(BandwidthSamplerTest, AppLimited)
{
    ...
    for (int i = 1; i <= 20; i++) {
        ...
        EXPECT_EQ(sample.state_at_send.is_app_limited, sampler_app_limited_at_start_);
        SendPacket(i + 20);
        clock_.AdvanceTime(time_between_packets);
    }
...
TEST_P(BandwidthSamplerTest, FirstRoundTrip)
{
    ...
    const QuicBandwidth real_bandwidth = QuicBandwidth::FromBytesAndTimeDelta(num_bytes, rtt);
...
TEST_P(BandwidthSamplerTest, NeuterPacket)
{
    ...
    clock_.AdvanceTime(QuicTime::Delta::FromMilliseconds(10));
    BandwidthSampler::CongestionEventSample sample = sampler_.OnCongestionEvent(clock_.Now(),
        { AckedPacket(QuicPacketNumber(1), kRegularPacketSize, clock_.Now()) }, {}, max_bandwidth_, est_bandwidth_upper_bound_, round_trip_count_);
    EXPECT_EQ(0u, sampler_.total_bytes_acked());
...
{
    QuicTime::Delta time_between_packets = QuicTime::Delta::FromMilliseconds(10);
    QuicBandwidth sending_rate = QuicBandwidth::FromBytesAndTimeDelta(kRegularPacketSize, time_between_packets);
    ...
    for (uint64_t i = 1; i < 21; i++) {
        ...
        BandwidthSampler::CongestionEventSample sample = OnCongestionEvent({ i - 1, i }, {});
        ...
        EXPECT_TRUE(sample.last_packet_send_state.is_valid);
        EXPECT_EQ(2 * kRegularPacketSize, sample.last_packet_send_state.bytes_in_flight);
        EXPECT_EQ(i * kRegularPacketSize, sample.last_packet_send_state.total_bytes_sent);
        EXPECT_EQ((i - 2) * kRegularPacketSize, sample.last_packet_send_state.total_bytes_acked);
...
TEST_P(BandwidthSamplerTest, LoseEveryOtherPacket)
{
    ...
    for (uint64_t i = 1; i < 21; i++) {
        ...
        BandwidthSampler::CongestionEventSample sample = OnCongestionEvent({ i }, { i - 1 });
        ...
        EXPECT_EQ(2 * kRegularPacketSize, sample.last_packet_send_state.bytes_in_flight);
        EXPECT_EQ(i * kRegularPacketSize, sample.last_packet_send_state.total_bytes_sent);
        EXPECT_EQ((i - 2) * kRegularPacketSize / 2, sample.last_packet_send_state.total_bytes_acked);
        EXPECT_EQ((i - 2) * kRegularPacketSize / 2, sample.last_packet_send_state.total_bytes_lost);
        sampler_.RemoveObsoletePackets(QuicPacketNumber(i - 2));
    }
}
...
{
    QuicTime::Delta time_between_packets = QuicTime::Delta::FromMilliseconds(10);
    QuicBandwidth first_packet_sending_rate = QuicBandwidth::FromBytesAndTimeDelta(kRegularPacketSize, time_between_packets);
...
protected:
    MaxAckHeightTrackerTest()
        : tracker_(/*initial_filter_window=*/10)
    {
    ...
    void AggregationEpisode(
        QuicBandwidth aggregation_bandwidth, QuicTime::Delta aggregation_duration, QuicByteCount bytes_per_ack, bool expect_new_aggregation_epoch)
    {
        ASSERT_GE(aggregation_bandwidth, bandwidth_);
        const QuicTime start_time = now_;

        const QuicByteCount aggregation_bytes = aggregation_bandwidth * aggregation_duration;

        const int num_acks = aggregation_bytes / bytes_per_ack;
        ASSERT_EQ(aggregation_bytes, num_acks * bytes_per_ack) << "aggregation_bytes: " << aggregation_bytes << " [" << aggregation_bandwidth << " in "
                                                               << aggregation_duration << "], bytes_per_ack: " << bytes_per_ack;

        const QuicTime::Delta time_between_acks = QuicTime::Delta::FromMicroseconds(aggregation_duration.ToMicroseconds() / num_acks);
        ASSERT_EQ(aggregation_duration, num_acks * time_between_acks)
            << "aggregation_bytes: " << aggregation_bytes << ", num_acks: " << num_acks << ", time_between_acks: " << time_between_acks;

        // The total duration of aggregation time and quiet period.
        const QuicTime::Delta total_duration = QuicTime::Delta::FromMicroseconds(aggregation_bytes * 8 * 1000000 / bandwidth_.ToBitsPerSecond());
        ASSERT_EQ(aggregation_bytes, total_duration * bandwidth_) << "total_duration: " << total_duration << ", bandwidth_: " << bandwidth_;

        QuicByteCount last_extra_acked = 0;
        for (QuicByteCount bytes = 0; bytes < aggregation_bytes; bytes += bytes_per_ack) {
            QuicByteCount extra_acked
                = tracker_.Update(bandwidth_, true, RoundTripCount(), last_sent_packet_number_, last_acked_packet_number_, now_, bytes_per_ack);
            QUIC_VLOG(1) << "T" << now_ << ": Update after " << bytes_per_ack << " bytes acked, " << extra_acked << " extra bytes acked";
        ...
        QUIC_VLOG(1) << "Advanced time from " << time_after_aggregation << " to " << now_ << ". Aggregation time[" << (time_after_aggregation - start_time)
                     << "], Quiet time[" << (now_ - time_after_aggregation) << "].";
    }
...
{
    AggregationEpisode(bandwidth_ * 20, QuicTime::Delta::FromMilliseconds(6), 1200, true);
    AggregationEpisode(bandwidth_ * 20, QuicTime::Delta::FromMilliseconds(6), 1200, true);
    ...
    if (tracker_.ack_aggregation_bandwidth_threshold() > 1.1) {
        ...
        AggregationEpisode(bandwidth_ * 20, QuicTime::Delta::FromMilliseconds(6), 1200, false);
...
{
    AggregationEpisode(bandwidth_ * 20, QuicTime::Delta::FromMilliseconds(6), 300, true);
    AggregationEpisode(bandwidth_ * 20, QuicTime::Delta::FromMilliseconds(6), 300, true);
    now_ = now_ - QuicTime::Delta::FromMilliseconds(1);
    ...
    if (tracker_.ack_aggregation_bandwidth_threshold() > 1.1) {
        ...
        AggregationEpisode(bandwidth_ * 20, QuicTime::Delta::FromMilliseconds(6), 300, false);
        EXPECT_EQ(2u, tracker_.num_ack_aggregation_epochs());
    }
}
...
{
    AggregationEpisode(bandwidth_ * 2, QuicTime::Delta::FromMilliseconds(50), 1000, true);
    AggregationEpisode(bandwidth_ * 2, QuicTime::Delta::FromMilliseconds(50), 1000, true);
    now_ = now_ - QuicTime::Delta::FromMilliseconds(1);
    ...
    if (tracker_.ack_aggregation_bandwidth_threshold() > 1.1) {
        ...
        AggregationEpisode(bandwidth_ * 2, QuicTime::Delta::FromMilliseconds(50), 1000, false);
        EXPECT_EQ(2u, tracker_.num_ack_aggregation_epochs());
    }
}
...
{
    AggregationEpisode(bandwidth_ * 2, QuicTime::Delta::FromMilliseconds(50), 100, true);
    AggregationEpisode(bandwidth_ * 2, QuicTime::Delta::FromMilliseconds(50), 100, true);
    now_ = now_ - QuicTime::Delta::FromMilliseconds(1);
    ...
    if (tracker_.ack_aggregation_bandwidth_threshold() > 1.1) {
        ...
        AggregationEpisode(bandwidth_ * 2, QuicTime::Delta::FromMilliseconds(50), 100, false);
        EXPECT_EQ(2u, tracker_.num_ack_aggregation_epochs());
    }
}
...
{
    AggregationEpisode(bandwidth_, QuicTime::Delta::FromMilliseconds(100), 100, true);
...
TEST_F(MaxAckHeightTrackerTest, StartNewEpochAfterAFullRound)
{
    ...
    tracker_.Update(bandwidth_ * 0.1, true, RoundTripCount(), last_sent_packet_number_, last_acked_packet_number_, now_, 100);
