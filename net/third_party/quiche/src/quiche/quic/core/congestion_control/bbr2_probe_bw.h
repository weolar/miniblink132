...
public:
    ...
    void Enter(QuicTime now, const Bbr2CongestionEvent* congestion_event) override;
    void Leave(QuicTime /*now*/, const Bbr2CongestionEvent* /*congestion_event*/) override
    {
    }

    Bbr2Mode OnCongestionEvent(QuicByteCount prior_in_flight, QuicTime event_time, const AckedPacketVector& acked_packets, const LostPacketVector& lost_packets,
    ...
    Bbr2Mode OnExitQuiescence(QuicTime now, QuicTime quiescence_start_time) override;
...
private:
    ...
    void UpdateProbeUp(QuicByteCount prior_in_flight, const Bbr2CongestionEvent& congestion_event);
    void UpdateProbeDown(QuicByteCount prior_in_flight, const Bbr2CongestionEvent& congestion_event);
    ...
    AdaptUpperBoundsResult MaybeAdaptUpperBounds(const Bbr2CongestionEvent& congestion_event);

    void EnterProbeDown(bool probed_too_high, bool stopped_risky_probe, QuicTime now);
    ...
    float PercentTimeElapsedToProbeBandwidth(const Bbr2CongestionEvent& congestion_event) const;

    bool IsTimeToProbeBandwidth(const Bbr2CongestionEvent& congestion_event) const;
    bool HasStayedLongEnoughInProbeDown(const Bbr2CongestionEvent& congestion_event) const;
    bool HasCycleLasted(QuicTime::Delta duration, const Bbr2CongestionEvent& congestion_event) const;
    bool HasPhaseLasted(QuicTime::Delta duration, const Bbr2CongestionEvent& congestion_event) const;
    bool IsTimeToProbeForRenoCoexistence(double probe_wait_fraction, const Bbr2CongestionEvent& congestion_event) const;
...
QUICHE_EXPORT std::ostream& operator<<(std::ostream& os, const Bbr2ProbeBwMode::DebugState& state);

QUICHE_EXPORT std::ostream& operator<<(std::ostream& os, const Bbr2ProbeBwMode::CyclePhase phase);
