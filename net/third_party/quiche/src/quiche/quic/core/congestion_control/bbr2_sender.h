...
public:
    Bbr2Sender(QuicTime now, const RttStats* rtt_stats, const QuicUnackedPacketMap* unacked_packets, QuicPacketCount initial_cwnd_in_packets,
    ...
    const Bbr2NetworkModel& GetNetworkModel() const
    {
        return model_;
    }
...
private:
    ...
    QuicByteCount max_cwnd_when_network_parameters_adjusted_ = kMaxInitialCongestionWindow * kDefaultTCPMSS;
...
QUICHE_EXPORT std::ostream& operator<<(std::ostream& os, const Bbr2Sender::DebugState& state);
