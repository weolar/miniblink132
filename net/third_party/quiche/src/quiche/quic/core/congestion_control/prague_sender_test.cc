...
public:
    explicit PragueSenderPeer(const QuicClock* clock)
        : PragueSender(clock, &rtt_stats_, kInitialCongestionWindowPackets, kMaxCongestionWindowPackets, &stats_)
    {
    }

    QuicTimeDelta rtt_virt() const
    {
        return rtt_virt_;
    }
    bool InReducedRttDependenceMode() const
    {
        return reduce_rtt_dependence_;
    }
    float alpha() const
    {
        return *prague_alpha_;
    }
...
protected:
    ...
    void AckNPackets(int n, int ce)
    {
        ...
        sender_.OnCongestionEvent(true, bytes_in_flight_, clock_.Now(), acked_packets, lost_packets, n - ce, ce);
    ...
    void MaybeUpdateAlpha(float& alpha, QuicTime& last_update, uint64_t& ect, uint64_t& ce)
    {
...
TEST_F(PragueSenderTest, EcnResponseInCongestionAvoidance)
{
    ...
    while (sender_.GetCongestionWindow() == original_cwnd) {
        AckNPackets(1, 0);
        expected_cwnd = cubic_.CongestionWindowAfterAck(kDefaultTCPMSS, expected_cwnd, kRtt, clock_.Now());
        EXPECT_EQ(expected_cwnd, sender_.GetCongestionWindow());
    ...
    expected_cwnd = cubic_.CongestionWindowAfterAck(kDefaultTCPMSS / 2, expected_cwnd, kRtt, clock_.Now());
    expected_cwnd = cubic_.CongestionWindowAfterAck(kDefaultTCPMSS / 2, expected_cwnd, kRtt, clock_.Now());
    EXPECT_EQ(expected_cwnd, sender_.GetCongestionWindow());
    ...
    // that.
    LoseNPackets(1);
    expected_cwnd = cubic_.CongestionWindowAfterPacketLoss(expected_cwnd + loss_reduction);
    EXPECT_EQ(expected_cwnd, sender_.GetCongestionWindow());
...
TEST_F(PragueSenderTest, ReducedRttDependence)
{
    ...
    while (!sender_.InReducedRttDependenceMode()) {
        ...
        for (int i = 0; (i < num_sent - 1); ++i) {
            if (last_alpha_update.has_value()) {
                ++num_ect;
                MaybeUpdateAlpha(expected_alpha, last_alpha_update.value(), num_ect, num_ce);
            }
            AckNPackets(1, 0);
        }
        ...
        if (!last_decrease.has_value() || (clock_.Now() - last_decrease.value() > sender_.rtt_virt())) {
    ...
    QuicByteCount expected_cwnd = sender_.GetCongestionWindow();
    QuicByteCount expected_increase = cubic_.CongestionWindowAfterAck(kDefaultTCPMSS, expected_cwnd, kRtt, clock_.Now()) - expected_cwnd;
