...
protected:
    ...
    {
        unacked_packets_ = std::make_unique<QuicUnackedPacketMap>(Perspective::IS_CLIENT);
    ...
    void SendPacket(uint64_t packet_number, EncryptionLevel encryption_level)
    {
        ...
        QuicTransportVersion version = CurrentSupportedVersions()[0].transport_version;
        frame.stream_id = QuicUtils::GetFirstBidirectionalStreamId(version, Perspective::IS_CLIENT);
    ...
    void AckPackets(const std::vector<uint64_t>& packets_acked)
    {
        ...
        for (uint64_t acked : packets_acked) {
            ...
            packets_acked_.push_back(AckedPacket(QuicPacketNumber(acked), kMaxOutgoingPacketSize, QuicTime::Zero()));
        }
    }
    ...
    {
        return VerifyLosses(largest_newly_acked, packets_acked, losses_expected, std::nullopt);
    }
...
TEST_F(UberLossAlgorithmTest, ScenarioA)
{
    ...
    AckPackets({ 1, 4 });
    unacked_packets_->MaybeUpdateLargestAckedOfPacketNumberSpace(HANDSHAKE_DATA, QuicPacketNumber(4));
...
TEST_F(UberLossAlgorithmTest, ScenarioB)
{
    ...
    AckPackets({ 4 });
    unacked_packets_->MaybeUpdateLargestAckedOfPacketNumberSpace(APPLICATION_DATA, QuicPacketNumber(4));
    ...
    AckPackets({ 6 });
    unacked_packets_->MaybeUpdateLargestAckedOfPacketNumberSpace(APPLICATION_DATA, QuicPacketNumber(6));
...
TEST_F(UberLossAlgorithmTest, ScenarioC)
{
    ...
    QuicUnackedPacketMapPeer::SetPerspective(unacked_packets_.get(), Perspective::IS_SERVER);
    ...
    AckPackets({ 4, 5 });
    unacked_packets_->MaybeUpdateLargestAckedOfPacketNumberSpace(APPLICATION_DATA, QuicPacketNumber(4));
    unacked_packets_->MaybeUpdateLargestAckedOfPacketNumberSpace(HANDSHAKE_DATA, QuicPacketNumber(5));
...
TEST_F(UberLossAlgorithmTest, PacketInLimbo)
{
    ...
    SendPacket(5, ENCRYPTION_FORWARD_SECURE);
    AckPackets({ 1, 3, 4 });
    unacked_packets_->MaybeUpdateLargestAckedOfPacketNumberSpace(APPLICATION_DATA, QuicPacketNumber(3));
    unacked_packets_->MaybeUpdateLargestAckedOfPacketNumberSpace(HANDSHAKE_DATA, QuicPacketNumber(4));
...
public:
    TestLossTuner(bool forced_start_result, LossDetectionParameters forced_parameters)
...
TEST_F(UberLossAlgorithmTest, LossDetectionTuning_SetFromConfigFirst)
{
    ...
    const QuicPacketCount old_reordering_threshold = loss_algorithm_.GetPacketReorderingThreshold();
    ...
    TestLossTuner* test_tuner = new TestLossTuner(
        ...
        LossDetectionParameters { /*reordering_shift=*/old_reordering_shift + 1,
            /*reordering_threshold=*/old_reordering_threshold * 2 });
    loss_algorithm_.SetLossDetectionTuner(std::unique_ptr<LossDetectionTunerInterface>(test_tuner));
    ...
    EXPECT_EQ(old_reordering_threshold, loss_algorithm_.GetPacketReorderingThreshold());
    ...
    EXPECT_NE(old_reordering_threshold, loss_algorithm_.GetPacketReorderingThreshold());
}
