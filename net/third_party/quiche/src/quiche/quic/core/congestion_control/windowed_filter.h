...
// to the second.
template <class T> struct QUICHE_EXPORT MinFilter {
...
// to the second.
template <class T> struct QUICHE_EXPORT MaxFilter {
    bool operator()(const T& lhs, const T& rhs) const
    {
        return lhs >= rhs;
    }
...
//    of type TimeT.
template <class T, class Compare, typename TimeT, typename TimeDeltaT> class QUICHE_EXPORT WindowedFilter {
public:
    ...
    WindowedFilter(TimeDeltaT window_length, T zero_value, TimeT zero_time)
        ...
        , estimates_ { Sample(zero_value_, zero_time), Sample(zero_value_, zero_time), Sample(zero_value_, zero_time) }
    {
    }
    ...
    void Update(T new_sample, TimeT new_time)
    {
        ...
        if (estimates_[0].sample == zero_value_ || Compare()(new_sample, estimates_[0].sample) || new_time - estimates_[2].time > window_length_) {
        ...
        if (estimates_[1].sample == estimates_[0].sample && new_time - estimates_[1].time > window_length_ >> 2) {
        ...
        if (estimates_[2].sample == estimates_[1].sample && new_time - estimates_[2].time > window_length_ >> 1) {
    ...
    void Clear()
    {
        Reset(zero_value_, zero_time_);
    }

    T GetBest() const
    {
        return estimates_[0].sample;
    }
    T GetSecondBest() const
    {
        return estimates_[1].sample;
    }
    T GetThirdBest() const
    {
        return estimates_[2].sample;
    }
