...
static_assert(static_cast<int>(QUIC_STREAM_LAST_ERROR) <= std::numeric_limits<uint8_t>::max(), "QuicRstStreamErrorCode exceeds single octet");
...
static_assert(static_cast<uint64_t>(QUIC_LAST_ERROR) <= static_cast<uint64_t>(std::numeric_limits<uint32_t>::max()), "QuicErrorCode exceeds four octets");
...
enum class QuicHttpQpackErrorCode { DECOMPRESSION_FAILED = 0x200, ENCODER_STREAM_ERROR = 0x201, DECODER_STREAM_ERROR = 0x202 };
...
public:
    ...
    QuicRstStreamErrorCode internal_code() const
    {
        return internal_code_;
    }
    uint64_t ietf_application_code() const
    {
        return ietf_application_code_;
    }

    bool operator==(const QuicResetStreamError& other) const
    {
        return internal_code() == other.internal_code() && ietf_application_code() == other.ietf_application_code();
    }

    // Returns true if the object holds no error.
    bool ok() const
    {
        return internal_code() == QUIC_STREAM_NO_ERROR;
    }
...
// is not recognized.
QUICHE_EXPORT std::optional<QuicErrorCode> TlsAlertToQuicErrorCode(uint8_t desc);

// Returns the name of the QuicRstStreamErrorCode as a char*
QUICHE_EXPORT const char* QuicRstStreamErrorCodeToString(QuicRstStreamErrorCode error);
...
QUICHE_EXPORT std::string QuicIetfTransportErrorCodeString(QuicIetfTransportErrorCodes c);

QUICHE_EXPORT std::ostream& operator<<(std::ostream& os, const QuicIetfTransportErrorCodes& c);
...
QUICHE_EXPORT QuicErrorCodeToIetfMapping QuicErrorCodeToTransportErrorCode(QuicErrorCode error);

// Convert a QuicRstStreamErrorCode to an application error code to be used in
// an IETF QUIC RESET_STREAM frame
QUICHE_EXPORT uint64_t RstStreamErrorCodeToIetfResetStreamErrorCode(QuicRstStreamErrorCode rst_stream_error_code);

// Convert the application error code of an IETF QUIC RESET_STREAM frame
// to QuicRstStreamErrorCode.
QUICHE_EXPORT QuicRstStreamErrorCode IetfResetStreamErrorCodeToRstStreamErrorCode(uint64_t ietf_error_code);
...
QUICHE_EXPORT inline std::string HistogramEnumDescription(QuicErrorCode /*dummy*/)
{
    return "cause";
}
