...
{
    if (stream_ == nullptr) {
        QUIC_BUG(quic_bug_10858_1) << "Double free'ing QuicStreamSequencer at " << this << ". " << QuicStackTrace();
    }
    stream_ = nullptr;
}
...
void QuicStreamSequencer::OnStreamFrame(const QuicStreamFrame& frame)
{
    ...
    if (frame.fin && (!CloseStreamAtOffset(frame.offset + data_len) || data_len == 0)) {
        return;
    }
...
void QuicStreamSequencer::OnFrameData(QuicStreamOffset byte_offset, size_t data_len, const char* data_buffer)
{
    ...
    QuicErrorCode result = buffered_frames_.OnStreamData(byte_offset, absl::string_view(data_buffer, data_len), &bytes_written, &error_details);
    if (result != QUIC_NO_ERROR) {
        std::string details = absl::StrCat("Stream ", stream_->id(), ": ", QuicErrorCodeToString(result), ": ", error_details);
    ...
    const bool stream_unblocked = previous_readable_bytes == 0 && buffered_frames_.ReadableBytes() > 0;
    if (stream_unblocked) {
        if (ignore_read_data_) {
            FlushBufferedFrames();
        } else {
...
bool QuicStreamSequencer::CloseStreamAtOffset(QuicStreamOffset offset)
{
    ...
    if (close_offset_ != kMaxOffset && offset != close_offset_) {
        stream_->OnUnrecoverableError(QUIC_STREAM_SEQUENCER_INVALID_STATE,
            absl::StrCat("Stream ", stream_->id(), " received new final offset: ", offset, ", which is different from close offset: ", close_offset_));
        return false;
    }
    ...
    if (offset < highest_offset_) {
        stream_->OnUnrecoverableError(QUIC_STREAM_SEQUENCER_INVALID_STATE,
            absl::StrCat("Stream ", stream_->id(), " received fin with offset: ", offset, ", which reduces current highest offset: ", highest_offset_));
        return false;
    }

    if (offset < reliable_offset_) {
        stream_->OnUnrecoverableError(QUIC_STREAM_MULTIPLE_OFFSET,
            absl::StrCat("Stream ", stream_->id(), " received fin with offset: ", offset, ", which reduces current reliable offset: ", reliable_offset_));
        return false;
    }
...
void QuicStreamSequencer::MaybeCloseStream()
{
    ...
    QUIC_DVLOG(1) << "Passing up termination, as we've processed " << buffered_frames_.BytesConsumed() << " of " << close_offset_ << " bytes.";
...
bool QuicStreamSequencer::PeekRegion(QuicStreamOffset offset, iovec* iov) const
{
    QUICHE_DCHECK(!blocked_);
...
size_t QuicStreamSequencer::Readv(const struct iovec* iov, size_t iov_len)
{
    ...
    std::string error_details;
    size_t bytes_read;
    QuicErrorCode read_error = buffered_frames_.Readv(iov, iov_len, &bytes_read, &error_details);
    if (read_error != QUIC_NO_ERROR) {
        std::string details = absl::StrCat("Stream ", stream_->id(), ": ", error_details);
...
void QuicStreamSequencer::MarkConsumed(size_t num_bytes_consumed)
{
    ...
    if (!result) {
        QUIC_BUG(quic_bug_10858_2) << "Invalid argument to MarkConsumed."
                                   << " expect to consume: " << num_bytes_consumed << ", but not enough bytes available. " << DebugString();
        stream_->ResetWithError(QuicResetStreamError::FromInternal(QUIC_ERROR_PROCESSING_STREAM));
        return;
    }
...
void QuicStreamSequencer::FlushBufferedFrames()
{
    ...
    QUIC_DVLOG(1) << "Flushing buffered data at offset " << buffered_frames_.BytesConsumed() << " length " << bytes_flushed << " for stream " << stream_->id();
...
{
    // clang-format on
    return absl::StrCat("QuicStreamSequencer:  bytes buffered: ", NumBytesBuffered(), "\n  bytes consumed: ", NumBytesConsumed(),
