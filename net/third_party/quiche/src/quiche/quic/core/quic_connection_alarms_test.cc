...
public:
    static MockAlarmFactory::TestAlarm* GetNowAlarm(QuicAlarmMultiplexer& multiplexer)
    {
        return static_cast<MockAlarmFactory::TestAlarm*>(multiplexer.now_alarm_.get());
    }
    static MockAlarmFactory::TestAlarm* GetLaterAlarm(QuicAlarmMultiplexer& multiplexer)
    {
        return static_cast<MockAlarmFactory::TestAlarm*>(multiplexer.later_alarm_.get());
    }
};
...
TEST_F(QuicAlarmMultiplexerTest, SetUpdateCancel)
{
    ...
    // Test granularity.
    multiplexer_.Update(QuicAlarmSlot::kSend, time2, QuicTimeDelta::FromSeconds(1000));
    EXPECT_TRUE(multiplexer_.IsSet(QuicAlarmSlot::kSend));
    EXPECT_EQ(multiplexer_.GetDeadline(QuicAlarmSlot::kSend), time1);

    // Test cancel-via-update.
    multiplexer_.Update(QuicAlarmSlot::kSend, QuicTime::Zero(), QuicTimeDelta::Zero());
    EXPECT_FALSE(multiplexer_.IsSet(QuicAlarmSlot::kSend));
}
...
TEST_F(QuicAlarmMultiplexerTest, PermanentlyCancel)
{
    ...
    EXPECT_QUICHE_BUG(multiplexer_.Set(QuicAlarmSlot::kSend, time), "permanently cancelled");
    EXPECT_QUICHE_BUG(multiplexer_.Update(QuicAlarmSlot::kSend, time, QuicTimeDelta::Zero()), "permanently cancelled");
}
...
{
    multiplexer_.Set(QuicAlarmSlot::kMtuDiscovery, clock_->Now() - QuicTimeDelta::FromMilliseconds(100));
    EXPECT_EQ(now_alarm_->deadline(), clock_->Now());
    EXPECT_FALSE(later_alarm_->IsSet());
}
...
TEST_F(QuicAlarmMultiplexerTest, SingleAlarmScheduledForFuture)
{
    multiplexer_.Set(QuicAlarmSlot::kMtuDiscovery, clock_->Now() + QuicTimeDelta::FromMilliseconds(100));
    EXPECT_FALSE(now_alarm_->IsSet());
    EXPECT_EQ(later_alarm_->deadline(), clock_->Now() + QuicTimeDelta::FromMilliseconds(100));
}

TEST_F(QuicAlarmMultiplexerTest, MultipleAlarmsNowAndFuture)
{
    multiplexer_.Set(QuicAlarmSlot::kMtuDiscovery, clock_->Now());
    multiplexer_.Set(QuicAlarmSlot::kAck, clock_->Now() + QuicTimeDelta::FromMilliseconds(100));
    EXPECT_TRUE(now_alarm_->IsSet());
    EXPECT_EQ(later_alarm_->deadline(), clock_->Now() + QuicTimeDelta::FromMilliseconds(100));
}
...
TEST_F(QuicAlarmMultiplexerTest, AlarmReschedulesItself)
{
    multiplexer_.Set(QuicAlarmSlot::kPing, clock_->Now());
    ASSERT_TRUE(now_alarm_->IsSet());
    EXPECT_CALL(delegate_, OnPingAlarm()).Times(1).WillRepeatedly([&] { multiplexer_.Set(QuicAlarmSlot::kPing, clock_->Now()); });
    now_alarm_->Fire();
    EXPECT_TRUE(multiplexer_.IsSet(QuicAlarmSlot::kPing));
}
...
{
    multiplexer_.Set(QuicAlarmSlot::kMtuDiscovery, clock_->Now());
    multiplexer_.Set(QuicAlarmSlot::kPing, clock_->Now() + QuicTimeDelta::FromMilliseconds(123));
