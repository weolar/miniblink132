...
struct QUICHE_EXPORT StreamPendingRetransmission {
    constexpr StreamPendingRetransmission(QuicStreamOffset offset, QuicByteCount length)
        : offset(offset)
        , length(length)
    {
    }
...
public:
    ...
    bool OnStreamDataAcked(QuicStreamOffset offset, QuicByteCount data_length, QuicByteCount* newly_acked_length);
    ...
    void OnStreamDataRetransmitted(QuicStreamOffset offset, QuicByteCount data_length);
    ...
    bool IsStreamDataOutstanding(QuicStreamOffset offset, QuicByteCount data_length) const;

    // Number of data slices in send buffer.
    size_t size() const;

    QuicStreamOffset stream_offset() const
    {
