...
{
    return absl::string_view(reinterpret_cast<const char*>(iov.iov_base), iov.iov_len);
}
...
char GetCharFromIOVecs(size_t offset, iovec iov[], size_t count)
{
    ...
    QUIC_LOG(ERROR) << "Could not locate char at offset " << offset << " in " << count << " iovecs";
    for (size_t i = 0; i < count; ++i) {
...
protected:
    void Initialize()
    {
        buffer_ = std::make_unique<QuicStreamSequencerBuffer>((max_capacity_bytes_));
...
{
    QuicErrorCode error = buffer_->OnStreamData(800, "", &written_, &error_details_);
...
TEST_F(QuicStreamSequencerBufferTest, OnStreamDataWithinBlock)
{
    ...
    EXPECT_THAT(buffer_->OnStreamData(800, source, &written_, &error_details_), IsQuicNoError());
...
TEST_F(QuicStreamSequencerBufferTest, DISABLED_OnStreamDataInvalidSource)
{
    ...
    EXPECT_THAT(buffer_->OnStreamData(800, source, &written_, &error_details_), IsError(QUIC_STREAM_SEQUENCER_INVALID_STATE));
    EXPECT_EQ(0u,
        error_details_.find(absl::StrCat("QuicStreamSequencerBuffer error: OnStreamData() "
                                         "dest == nullptr: ",
...
TEST_F(QuicStreamSequencerBufferTest, OnStreamDataWithOverlap)
{
    ...
    EXPECT_THAT(buffer_->OnStreamData(0, source, &written_, &error_details_), IsQuicNoError());
    EXPECT_THAT(buffer_->OnStreamData(1024, source, &written_, &error_details_), IsQuicNoError());
}

TEST_F(QuicStreamSequencerBufferTest, OnStreamDataOverlapAndDuplicateCornerCases)
{
    std::string source(1024, 'a');
    // Write something into [800, 1824)
    ...
    // Write [1, 801).
    EXPECT_THAT(buffer_->OnStreamData(1, source, &written_, &error_details_), IsQuicNoError());
    // Write [0, 800).
    EXPECT_THAT(buffer_->OnStreamData(0, source, &written_, &error_details_), IsQuicNoError());
    // Write [1823, 1824).
    EXPECT_THAT(buffer_->OnStreamData(1823, one_byte, &written_, &error_details_), IsQuicNoError());
    EXPECT_EQ(0u, written_);
    // write one byte to [1824, 1825)
    EXPECT_THAT(buffer_->OnStreamData(1824, one_byte, &written_, &error_details_), IsQuicNoError());
    EXPECT_TRUE(helper_->CheckBufferInvariants());
}
...
TEST_F(QuicStreamSequencerBufferTest, OnStreamDataWithoutOverlap)
{
    ...
    EXPECT_THAT(buffer_->OnStreamData(kBlockSizeBytes * 2 - 20, source, &written_, &error_details_), IsQuicNoError());
...
TEST_F(QuicStreamSequencerBufferTest, OnStreamDataInLongStreamWithOverlap)
{
    ...
    EXPECT_THAT(buffer_->OnStreamData(offset, source, &written_, &error_details_), IsQuicNoError());
    EXPECT_EQ(2, helper_->IntervalSize());
...
TEST_F(QuicStreamSequencerBufferTest, OnStreamDataTillEnd)
{
    ...
    std::string source(kBytesToWrite, 'a');
    EXPECT_THAT(buffer_->OnStreamData(max_capacity_bytes_ - kBytesToWrite, source, &written_, &error_details_), IsQuicNoError());
...
TEST_F(QuicStreamSequencerBufferTest, OnStreamDataBeyondCapacity)
{
    ...
    EXPECT_THAT(buffer_->OnStreamData(max_capacity_bytes_ - 50, source, &written_, &error_details_), IsError(QUIC_INTERNAL_ERROR));
    EXPECT_TRUE(helper_->CheckBufferInvariants());

    source = "b";
    EXPECT_THAT(buffer_->OnStreamData(max_capacity_bytes_, source, &written_, &error_details_), IsError(QUIC_INTERNAL_ERROR));
    EXPECT_TRUE(helper_->CheckBufferInvariants());

    EXPECT_THAT(buffer_->OnStreamData(max_capacity_bytes_ * 1000, source, &written_, &error_details_), IsError(QUIC_INTERNAL_ERROR));
    EXPECT_TRUE(helper_->CheckBufferInvariants());

    // Disallow current_gap != gaps_.end()
    EXPECT_THAT(buffer_->OnStreamData(static_cast<QuicStreamOffset>(-1), source, &written_, &error_details_), IsError(QUIC_INTERNAL_ERROR));
    EXPECT_TRUE(helper_->CheckBufferInvariants());

    // Disallow offset + size overflow
    source = "bbb";
    EXPECT_THAT(buffer_->OnStreamData(static_cast<QuicStreamOffset>(-2), source, &written_, &error_details_), IsError(QUIC_INTERNAL_ERROR));
    EXPECT_TRUE(helper_->CheckBufferInvariants());
...
TEST_F(QuicStreamSequencerBufferTest, Readv100Bytes)
{
    ...
    size_t read;
    EXPECT_THAT(buffer_->Readv(iovecs, 3, &read, &error_details_), IsQuicNoError());
...
TEST_F(QuicStreamSequencerBufferTest, ReadvAcrossBlocks)
{
    ...
    while (helper_->ReadableBytes()) {
        ...
        size_t read;
        EXPECT_THAT(buffer_->Readv(iovecs, 2, &read, &error_details_), IsQuicNoError());
    }
...
TEST_F(QuicStreamSequencerBufferTest, OnStreamDataAcrossLastBlockAndFillCapacity)
{
    std::string source(kBlockSizeBytes + 50, 'a');
    // Write 1st block to full with 'a'.
    buffer_->OnStreamData(0, source, &written_, &error_details_);
    ...
    EXPECT_THAT(buffer_->OnStreamData(2 * kBlockSizeBytes, source, &written_, &error_details_), IsQuicNoError());
    EXPECT_EQ(source.size(), written_);
    EXPECT_TRUE(helper_->CheckBufferInvariants());
}

TEST_F(QuicStreamSequencerBufferTest, OnStreamDataAcrossLastBlockAndExceedCapacity)
{
    std::string source(kBlockSizeBytes + 50, 'a');
    ...
    EXPECT_THAT(buffer_->OnStreamData(8 * kBlockSizeBytes, source, &written_, &error_details_), IsError(QUIC_INTERNAL_ERROR));
    EXPECT_TRUE(helper_->CheckBufferInvariants());
}
...
TEST_F(QuicStreamSequencerBufferTest, ReadvAcrossLastBlock)
{
    ...
    buffer_->OnStreamData(max_capacity_bytes_, source, &written_, &error_details_);
    EXPECT_TRUE(helper_->CheckBufferInvariants());
    ...
    EXPECT_THAT(buffer_->Readv(&iov1, 1, &read, &error_details_), IsQuicNoError());
...
TEST_F(QuicStreamSequencerBufferTest, GetReadableRegionsTillEndOfBlock)
{
    ...
    EXPECT_EQ(std::string(kBlockSizeBytes - 256, 'a'), IovecToStringPiece(iovs[0]));
}
...
TEST_F(QuicStreamSequencerBufferTest, GetReadableRegionsAcrossBlockWithLongIOV)
{
...
TEST_F(QuicStreamSequencerBufferTest, GetReadableRegionsWithMultipleIOVsAcrossEnd)
{
    ...
    buffer_->OnStreamData(8.5 * kBlockSizeBytes - 1024, source, &written_, &error_details_);
    // Use short iovec's.
    iovec iovs[2];
    int iov_count = buffer_->GetReadableRegions(iovs, 2);
...
TEST_F(QuicStreamSequencerBufferTest, GetReadableRegionTillGap)
{
    ...
    iovec iov;
    EXPECT_TRUE(buffer_->GetReadableRegion(&iov));
    EXPECT_EQ(std::string(kBlockSizeBytes - 1 - 256, 'a'), IovecToStringPiece(iov));
}
...
TEST_F(QuicStreamSequencerBufferTest, PeekTwoWritesInSingleBlock)
{
    ...
    EXPECT_EQ(absl::string_view(source1).substr(offset1), IovecToStringPiece(iov).substr(0, length1 - offset1));
    EXPECT_EQ(absl::string_view(source2), IovecToStringPiece(iov).substr(length1 - offset1));

    // Peek with an offset inside the second write.
    const QuicStreamOffset offset2 = 1500;
    EXPECT_TRUE(buffer_->PeekRegion(offset2, &iov));
    EXPECT_EQ(absl::string_view(source2).substr(offset2 - length1), IovecToStringPiece(iov));
...
TEST_F(QuicStreamSequencerBufferTest, PeekBufferWithMultipleBlocks)
{
    ...
    EXPECT_EQ(absl::string_view(source2).substr(0, kBlockSizeBytes - length1), IovecToStringPiece(iov).substr(length1));

    EXPECT_TRUE(buffer_->PeekRegion(length1, &iov));
    EXPECT_EQ(absl::string_view(source2).substr(0, kBlockSizeBytes - length1), IovecToStringPiece(iov));

    EXPECT_TRUE(buffer_->PeekRegion(kBlockSizeBytes, &iov));
    EXPECT_EQ(absl::string_view(source2).substr(kBlockSizeBytes - length1), IovecToStringPiece(iov));
...
TEST_F(QuicStreamSequencerBufferTest, TooManyGaps)
{
    ...
    for (QuicStreamOffset begin = 1; begin <= max_capacity_bytes_; begin += 2) {
        QuicErrorCode rs = buffer_->OnStreamData(begin, "a", &written_, &error_details_);

        QuicStreamOffset last_straw = 2 * kMaxNumGapsAllowed - 1;
        if (begin == last_straw) {
            EXPECT_THAT(rs, IsError(QUIC_TOO_MANY_STREAM_DATA_INTERVALS));
            EXPECT_EQ("Too many data intervals received for this stream.", error_details_);
            break;
        }
    }
}

class QuicStreamSequencerBufferRandomIOTest : public QuicStreamSequencerBufferTest {
public:
    ...
    void CreateSourceAndShuffle(size_t max_chunk_size_bytes)
    {
        ...
        std::unique_ptr<OffsetSizePair[]> chopped_stream(new OffsetSizePair[bytes_to_buffer_]);
        ...
        while (start_chopping_offset < bytes_to_buffer_) {
            size_t max_chunk = std::min<size_t>(max_chunk_size_bytes_, bytes_to_buffer_ - start_chopping_offset);
        ...
        for (int i = chunk_num - 1; i >= 0; --i) {
            ...
            QUIC_DVLOG(1) << "chunk offset " << chopped_stream[random_idx].first << " size " << chopped_stream[random_idx].second;
    ...
    void WriteNextChunkToBuffer()
    {
        ...
        auto result = buffer_->OnStreamData(offset, string_piece_w, &written_, &error_details_);
        ...
        QUIC_DVLOG(1) << " write at offset: " << offset << " len to write: " << num_to_write << " write result: " << result
...
TEST_F(QuicStreamSequencerBufferRandomIOTest, RandomWriteAndReadv)
{
    ...
    while ((!shuffled_buf_.empty() || total_bytes_read_ < bytes_to_buffer_) && iterations <= 2 * bytes_to_buffer_) {
        ...
        case 1: { // readv
            std::unique_ptr<char[][kMaxReadSize]> read_buf { new char[kNumReads][kMaxReadSize] };
            ...
            EXPECT_THAT(buffer_->Readv(dest_iov, kNumReads, &actually_read, &error_details_), IsQuicNoError());
            ASSERT_LE(actually_read, num_to_read);
            QUIC_DVLOG(1) << " read from offset: " << total_bytes_read_ << " size: " << num_to_read << " actual read: " << actually_read;
            for (size_t i = 0; i < actually_read; ++i) {
                char ch = (i + total_bytes_read_) % 256;
                ASSERT_EQ(ch, GetCharFromIOVecs(i, dest_iov, kNumReads)) << " at iteration " << iterations;
            }
    ...
    EXPECT_LE(bytes_to_buffer_, total_bytes_read_) << "iterations: " << iterations;
...
TEST_F(QuicStreamSequencerBufferRandomIOTest, RandomWriteAndConsumeInPlace)
{
    ...
    while ((!shuffled_buf_.empty() || total_bytes_read_ < bytes_to_buffer_) && iterations <= 2 * bytes_to_buffer_) {
        ...
        case 1: { // GetReadableRegions and then MarkConsumed
            ...
            for (size_t i = 0; i < actually_num_read; ++i) {
                size_t bytes_in_block = std::min<size_t>(bytes_to_process - bytes_processed, dest_iov[i].iov_len);
                if (bytes_in_block == 0) {
                    break;
                }
                for (size_t j = 0; j < bytes_in_block; ++j) {
                    ...
                    ASSERT_EQ(char_expected, reinterpret_cast<const char*>(dest_iov[i].iov_base)[j]) << " at iteration " << iterations;
                    ++bytes_processed;
                }
            }
            ...
            QUIC_DVLOG(1) << "iteration " << iterations << ": try to get " << num_read << " readable regions, actually get " << actually_num_read
...
TEST_F(QuicStreamSequencerBufferTest, GrowBlockSizeOnDemand)
{
    ...
    buffer_->OnStreamData(kBlockSizeBytes * 7, source_of_one_block, &written_, &error_details_);
    ASSERT_EQ(helper_->current_blocks_count(), 8u);
    ...
    // that's not enough.
    buffer_->OnStreamData(kBlockSizeBytes * 139, source_of_one_block, &written_, &error_details_);
    ASSERT_EQ(helper_->current_blocks_count(), 140u);

    // Number of blocks grows by a factor of 4 normally.
    buffer_->OnStreamData(kBlockSizeBytes * 140, source_of_one_block, &written_, &error_details_);
    ASSERT_EQ(helper_->current_blocks_count(), 560u);

    // max_capacity_bytes is reached and number of blocks is capped.
    buffer_->OnStreamData(kBlockSizeBytes * 560, source_of_one_block, &written_, &error_details_);
    ASSERT_EQ(helper_->current_blocks_count(), 1024u);

    // max_capacity_bytes is reached and number of blocks is capped.
    buffer_->OnStreamData(kBlockSizeBytes * 1025, source_of_one_block, &written_, &error_details_);
    ASSERT_EQ(helper_->current_blocks_count(), 1024u);
}
