...
class QUICHE_EXPORT WebTransportWriteBlockedList : public QuicWriteBlockedListInterface {
public:
    ...
    void RegisterStream(QuicStreamId stream_id, bool is_static_stream, const QuicStreamPriority& raw_priority) override;
    void UnregisterStream(QuicStreamId stream_id) override;
    void UpdateStreamPriority(QuicStreamId stream_id, const QuicStreamPriority& new_priority) override;
...
private:
    ...
    public:
        ...
        {
            return ScheduleKey(priority.web_transport().session_id, priority.web_transport().send_group_number);
        }
        ...
        template <typename H> friend H AbslHashValue(H h, const ScheduleKey& key)
        {
        ...
        {
            return group_ != kNoSendGroup;
        }
        quic::QuicStreamId stream() const
        {
            return stream_;
        }
        ...
        friend inline std::ostream& operator<<(std::ostream& os, const ScheduleKey& key)
        {
            os << key.DebugString();
            return os;
        }
    ...
    private:
        static constexpr webtransport::SendGroupId kNoSendGroup = std::numeric_limits<webtransport::SendGroupId>::max();

        explicit ScheduleKey(quic::QuicStreamId stream, webtransport::SendGroupId group)
            : stream_(stream)
            , group_(group)
        {
        }
    ...
    using Subscheduler = quiche::BTreeScheduler<QuicStreamId, webtransport::SendOrder>;
    ...
    absl::flat_hash_map<ScheduleKey, Subscheduler> web_transport_session_schedulers_;
};
