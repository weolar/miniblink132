...
public:
    ...
    public:
        ...
        virtual void OnUnrecoverableError(QuicErrorCode error, const std::string& error_details) = 0;

        // Consults delegate whether a packet should be generated.
        virtual bool ShouldGeneratePacket(HasRetransmittableData retransmittable, IsHandshake handshake) = 0;
        ...
        virtual void MaybeBundleOpportunistically(TransmissionType transmission_type) = 0;
        ...
        // serialized.
        virtual SerializedPacketFate GetSerializedPacketFate(bool is_mtu_discovery, EncryptionLevel encryption_level) = 0;
    };
    ...
    QuicPacketCreator(QuicConnectionId server_connection_id, QuicFramer* framer, DelegateInterface* delegate);
    QuicPacketCreator(QuicConnectionId server_connection_id, QuicFramer* framer, QuicRandom* random, DelegateInterface* delegate);
    ...
    // creator.
    void UpdatePacketNumberLength(QuicPacketNumber least_packet_awaited_by_peer, QuicPacketCount max_packets_in_flight);

    // Skip |count| packet numbers.
    void SkipNPacketNumbers(QuicPacketCount count, QuicPacketNumber least_packet_awaited_by_peer, QuicPacketCount max_packets_in_flight);

    // The overhead the framing will add for a packet with one frame.
    static size_t StreamFramePacketOverhead(QuicTransportVersion version, uint8_t destination_connection_id_length, uint8_t source_connection_id_length,
    ...
    bool ConsumeCryptoDataToFillCurrentPacket(
    ...
    bool HasRoomForStreamFrame(QuicStreamId id, QuicStreamOffset offset, size_t data_size);
    ...
    // QuicStreamFrame.
    void CreateAndSerializeStreamFrame(QuicStreamId id, size_t write_length, QuicStreamOffset iov_offset, QuicStreamOffset stream_offset, bool fin,
    ...
    static size_t ExpansionOnNewFrameWithLastFrame(const QuicFrame& last_frame, QuicTransportVersion version);
    ...
    // MTU bytes long.
    std::unique_ptr<SerializedPacket> SerializePathChallengeConnectivityProbingPacket(const QuicPathFrameBuffer& payload);
    ...
    // |payloads| is cleared.
    std::unique_ptr<SerializedPacket> SerializePathResponseConnectivityProbingPacket(
    ...
    std::unique_ptr<SerializedPacket> SerializeLargePacketNumberConnectionClosePacket(
    ...
    void SetServerConnectionIdIncluded(QuicConnectionIdIncluded server_connection_id_included);
    ...
    {
        return packet_.encryption_level;
    }
    ...
    bool has_ack() const
    {
        return packet_.has_ack;
    }

    bool has_stop_waiting() const
    {
        return packet_.has_stop_waiting;
    }
    ...
    QuicConsumedData ConsumeData(QuicStreamId id, size_t write_length, QuicStreamOffset offset, StreamSendingState state);
    ...
    QuicConsumedData ConsumeDataFastPath(QuicStreamId id, size_t write_length, QuicStreamOffset offset, bool fin, size_t total_bytes_consumed);
    ...
    // packets.
    size_t ConsumeCryptoData(EncryptionLevel level, size_t write_length, QuicStreamOffset offset);
    ...
    MessageStatus AddMessageFrame(QuicMessageId message_id, absl::Span<quiche::QuicheMemSlice> message);
    ...
    QuicByteCount pending_padding_bytes() const
    {
        return pending_padding_bytes_;
    }
    ...
    static size_t MinPlaintextPacketSize(const ParsedQuicVersion& version, QuicPacketNumberLength packet_number_length);
    ...
    size_t BuildPaddedPathChallengePacket(
    ...
    // successfully built.
    size_t BuildPathResponsePacket(const QuicPacketHeader& header, char* buffer, size_t packet_length,
    ...
    size_t BuildConnectivityProbingPacket(const QuicPacketHeader& header, char* buffer, size_t packet_length, EncryptionLevel level);

    // Serializes |coalesced| to provided |buffer|, returns coalesced packet
    // length if serialization succeeds. Otherwise, returns 0.
    size_t SerializeCoalescedPacket(const QuicCoalescedPacket& coalesced, char* buffer, size_t buffer_len);
...
private:
    ...
    std::optional<size_t> MaybeBuildDataPacketWithChaosProtection(const QuicPacketHeader& header, char* buffer);
    ...
    // the fin.
    void CreateStreamFrame(QuicStreamId id, size_t data_size, QuicStreamOffset offset, bool fin, QuicFrame* frame);
    ...
    bool CreateCryptoFrame(EncryptionLevel level, size_t write_length, QuicStreamOffset offset, QuicFrame* frame);
    ...
    ABSL_MUST_USE_RESULT bool SerializePacket(QuicOwnedPacketBuffer encrypted_buffer, size_t encrypted_buffer_len, bool allow_padding);
    ...
    // serialization fails.
    size_t ReserializeInitialPacketInCoalescedPacket(const SerializedPacket& packet, size_t padding_size, char* buffer, size_t buffer_len);
