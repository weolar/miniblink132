...
bool HasExtension(const SSL_CLIENT_HELLO* client_hello, uint16_t extension)
{
    ...
    return 1 == SSL_early_callback_ctx_extension_get(client_hello, extension, &unused_extension_bytes, &unused_extension_len);
}
...
std::vector<uint16_t> GetSupportedGroups(const SSL_CLIENT_HELLO* client_hello)
{
    ...
    int rv = SSL_early_callback_ctx_extension_get(client_hello, TLSEXT_TYPE_supported_groups, &extension_data, &extension_len);
    if (rv != 1) {
        return {};
    }
    ...
    QuicDataReader named_groups_reader(reinterpret_cast<const char*>(extension_data), extension_len);
    uint16_t named_groups_len;
    if (!named_groups_reader.ReadUInt16(&named_groups_len) || named_groups_len + sizeof(uint16_t) != extension_len) {
...
std::vector<uint16_t> GetCertCompressionAlgos(const SSL_CLIENT_HELLO* client_hello)
{
    const uint8_t* extension_data;
    size_t extension_len;
    int rv = SSL_early_callback_ctx_extension_get(client_hello, TLSEXT_TYPE_cert_compression, &extension_data, &extension_len);
    if (rv != 1) {
        return {};
    }
    // See https://datatracker.ietf.org/doc/html/rfc8879#section-3 for the format
    // of this extension.
    QuicDataReader cert_compression_algos_reader(reinterpret_cast<const char*>(extension_data), extension_len);
    uint8_t algos_len;
    if (!cert_compression_algos_reader.ReadUInt8(&algos_len) || algos_len == 0 || algos_len % sizeof(uint16_t) != 0
...
TlsChloExtractor& TlsChloExtractor::operator=(TlsChloExtractor&& other)
{
    ...
    parsed_crypto_frame_in_this_packet_ = other.parsed_crypto_frame_in_this_packet_;
...
void TlsChloExtractor::IngestPacket(const ParsedQuicVersion& version, const QuicReceivedPacket& packet)
{
    ...
    if (framer_) {
        ...
        if (!framer_->IsSupportedVersion(version)) {
            QUIC_DLOG(ERROR) << "Not ingesting packet with version mismatch, expected " << framer_->version() << ", got " << version;
            return;
        }
    } else {
        // This is the first packet we have ingested, setup parser.
        framer_ = std::make_unique<QuicFramer>(
            ParsedQuicVersionVector { version }, QuicTime::Zero(), Perspective::IS_SERVER, /*expected_server_connection_id_length=*/0);
...
bool TlsChloExtractor::OnUnauthenticatedPublicHeader(const QuicPacketHeader& header)
{
...
bool TlsChloExtractor::OnProtocolVersionMismatch(ParsedQuicVersion version)
{
    ...
    // IngestPacket.
    QUIC_BUG(quic_bug_10855_1) << "Unexpected version mismatch, expected " << framer_->version() << ", got " << version;
    return false;
}
...
void TlsChloExtractor::OnUnrecoverableError(QuicErrorCode error, const std::string& details)
{
    HandleUnrecoverableError(absl::StrCat("Crypto stream error ", QuicErrorCodeToString(error), ": ", details));
}

void TlsChloExtractor::OnUnrecoverableError(QuicErrorCode error, QuicIetfTransportErrorCodes ietf_error, const std::string& details)
{
    HandleUnrecoverableError(
        absl::StrCat("Crypto stream error ", QuicErrorCodeToString(error), "(", QuicIetfTransportErrorCodeString(ietf_error), "): ", details));
}
...
void TlsChloExtractor::OnDataAvailable()
{
    ...
    while (crypto_stream_sequencer_.GetReadableRegion(&iov)) {
        const int rv = SSL_provide_quic_data(ssl_.get(), ssl_encryption_initial, reinterpret_cast<const uint8_t*>(iov.iov_base), iov.iov_len);
        if (rv != 1) {
...
TlsChloExtractor* TlsChloExtractor::GetInstanceFromSSL(SSL* ssl)
{
    std::pair<SSL_CTX*, int> shared_handles = GetSharedSslHandles();
    int ex_data_index = shared_handles.second;
    return reinterpret_cast<TlsChloExtractor*>(SSL_get_ex_data(ssl, ex_data_index));
}

// static
int TlsChloExtractor::SetReadSecretCallback(
    SSL* ssl, enum ssl_encryption_level_t /*level*/, const SSL_CIPHER* /*cipher*/, const uint8_t* /*secret*/, size_t /*secret_length*/)
{
...
// static
int TlsChloExtractor::WriteMessageCallback(SSL* ssl, enum ssl_encryption_level_t /*level*/, const uint8_t* /*data*/, size_t /*len*/)
{
...
void TlsChloExtractor::HandleUnexpectedCallback(const std::string& callback_name)
{
    std::string error_details = absl::StrCat("Unexpected callback ", callback_name);
    QUIC_BUG(quic_bug_10855_3) << error_details;
    HandleUnrecoverableError(error_details);
}

// static
int TlsChloExtractor::SendAlertCallback(SSL* ssl, enum ssl_encryption_level_t /*level*/, uint8_t desc)
{
...
void TlsChloExtractor::SendAlert(uint8_t tls_alert_value)
{
    ...
    HandleUnrecoverableError(
        absl::StrCat("BoringSSL attempted to send alert ", static_cast<int>(tls_alert_value), " ", SSL_alert_desc_string_long(tls_alert_value)));
    if (state_ == State::kUnrecoverableFailure) {
...
// static
enum ssl_select_cert_result_t TlsChloExtractor::SelectCertCallback(const SSL_CLIENT_HELLO* client_hello)
{
...
void TlsChloExtractor::HandleParsedChlo(const SSL_CLIENT_HELLO* client_hello)
{
    ...
    resumption_attempted_ = HasExtension(client_hello, TLSEXT_TYPE_pre_shared_key);
    early_data_attempted_ = HasExtension(client_hello, TLSEXT_TYPE_early_data);

    QUICHE_DCHECK(client_hello_bytes_.empty());
    client_hello_bytes_.assign(client_hello->client_hello, client_hello->client_hello + client_hello->client_hello_len);

    const uint8_t* alpn_data;
    size_t alpn_len;
    int rv = SSL_early_callback_ctx_extension_get(client_hello, TLSEXT_TYPE_application_layer_protocol_negotiation, &alpn_data, &alpn_len);
    if (rv == 1) {
        QuicDataReader alpns_reader(reinterpret_cast<const char*>(alpn_data), alpn_len);
    ...
    if (GetQuicReloadableFlag(quic_parse_cert_compression_algos_from_chlo)) {
        ...
        if (cert_compression_algos_.empty()) {
            QUIC_RELOADABLE_FLAG_COUNT_N(quic_parse_cert_compression_algos_from_chlo, 1, 2);
        } else {
            QUIC_RELOADABLE_FLAG_COUNT_N(quic_parse_cert_compression_algos_from_chlo, 2, 2);
        }
    }
    ...
    if (state_ == State::kInitial) {
        ...
        QUIC_BUG(quic_bug_10855_4) << "Unexpected state on successful parse " << StateToString(state_);
    }
}
...
std::pair<SSL_CTX*, int> TlsChloExtractor::GetSharedSslHandles()
{
    ...
    static std::pair<SSL_CTX*, int>* shared_handles = []() {
        ...
        static const SSL_QUIC_METHOD kQuicCallbacks { TlsChloExtractor::SetReadSecretCallback, TlsChloExtractor::SetWriteSecretCallback,
            TlsChloExtractor::WriteMessageCallback, TlsChloExtractor::FlushFlightCallback, TlsChloExtractor::SendAlertCallback };
        SSL_CTX_set_quic_method(ssl_ctx, &kQuicCallbacks);
        SSL_CTX_set_select_certificate_cb(ssl_ctx, TlsChloExtractor::SelectCertCallback);
        int ex_data_index = SSL_get_ex_new_index(0, nullptr, nullptr, nullptr, nullptr);
...
void TlsChloExtractor::HandleUnrecoverableError(const std::string& error_details)
{
...
std::ostream& operator<<(std::ostream& os, const TlsChloExtractor::State& state)
{
