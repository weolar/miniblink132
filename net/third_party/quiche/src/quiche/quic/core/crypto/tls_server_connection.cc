...
TlsServerConnection::TlsServerConnection(SSL_CTX* ssl_ctx, Delegate* delegate, QuicSSLConfig ssl_config)
    : TlsConnection(ssl_ctx, delegate->ConnectionDelegate(), std::move(ssl_config))
    , delegate_(delegate)
{
...
// static
bssl::UniquePtr<SSL_CTX> TlsServerConnection::CreateSslCtx(ProofSource* proof_source)
{
    ...
    SSL_CTX_set_tlsext_servername_callback(ssl_ctx.get(), &TlsExtServernameCallback);
    ...
    if (proof_source->GetTicketCrypter()) {
        ...
        SSL_CTX_set_ticket_aead_method(ssl_ctx.get(), &TlsServerConnection::kSessionTicketMethod);
    } else {
    ...
    SSL_CTX_set_select_certificate_cb(ssl_ctx.get(), &TlsServerConnection::EarlySelectCertCallback);
...
absl::Status TlsServerConnection::ConfigureSSL(ProofSourceHandleCallback::ConfigureSSLFunc configure_ssl)
{
...
{
    SSL_set_chain_and_key(ssl(), cert_chain.data(), cert_chain.size(), nullptr, &TlsServerConnection::kPrivateKeyMethod);
}
...
{
    return ConnectionFromSsl(client_hello->ssl)->delegate_->EarlySelectCertCallback(client_hello);
}
...
// static
int TlsServerConnection::SelectAlpnCallback(SSL* ssl, const uint8_t** out, uint8_t* out_len, const uint8_t* in, unsigned in_len, void* /*arg*/)
{
    return ConnectionFromSsl(ssl)->delegate_->SelectAlpn(out, out_len, in, in_len);
}

// static
ssl_private_key_result_t TlsServerConnection::PrivateKeySign(
    SSL* ssl, uint8_t* out, size_t* out_len, size_t max_out, uint16_t sig_alg, const uint8_t* in, size_t in_len)
{
    return ConnectionFromSsl(ssl)->delegate_->PrivateKeySign(out, out_len, max_out, sig_alg, absl::string_view(reinterpret_cast<const char*>(in), in_len));
}

// static
ssl_private_key_result_t TlsServerConnection::PrivateKeyComplete(SSL* ssl, uint8_t* out, size_t* out_len, size_t max_out)
{
    return ConnectionFromSsl(ssl)->delegate_->PrivateKeyComplete(out, out_len, max_out);
}
...
// static
int TlsServerConnection::SessionTicketSeal(SSL* ssl, uint8_t* out, size_t* out_len, size_t max_out_len, const uint8_t* in, size_t in_len)
{
    return ConnectionFromSsl(ssl)->delegate_->SessionTicketSeal(out, out_len, max_out_len, absl::string_view(reinterpret_cast<const char*>(in), in_len));
}

// static
enum ssl_ticket_aead_result_t TlsServerConnection::SessionTicketOpen(
    SSL* ssl, uint8_t* out, size_t* out_len, size_t max_out_len, const uint8_t* in, size_t in_len)
{
    return ConnectionFromSsl(ssl)->delegate_->SessionTicketOpen(out, out_len, max_out_len, absl::string_view(reinterpret_cast<const char*>(in), in_len));
}
