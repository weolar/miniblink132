...
TEST(CertificateUtilTest, CreateSelfSignedCertificate)
{
    ...
    std::string der_cert = CreateSelfSignedCertificate(*cert_key.private_key(), options);
    ASSERT_FALSE(der_cert.empty());

    QuicSaveTestOutput("CertificateUtilTest_CreateSelfSignedCert.crt", der_cert);

    std::unique_ptr<CertificateView> cert_view = CertificateView::ParseSingleCertificate(der_cert);
    ...
    EXPECT_TRUE(cert_key.ValidForSignatureAlgorithm(SSL_SIGN_ECDSA_SECP256R1_SHA256));
