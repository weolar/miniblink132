...
public:
    ...
    std::unique_ptr<CryptoHandshakeMessage> release()
    {
...
// static
std::unique_ptr<CryptoHandshakeMessage> CryptoFramer::ParseMessage(absl::string_view in)
{
...
bool CryptoFramer::ProcessInput(absl::string_view input, EncryptionLevel /*level*/)
{
...
{
    QuicDataReader reader(buffer_.data(), buffer_.length(), quiche::HOST_BYTE_ORDER);
...
// static
std::unique_ptr<QuicData> CryptoFramer::ConstructHandshakeMessage(const CryptoHandshakeMessage& message)
{
    ...
    // Tags and offsets
    for (auto it = message.tag_value_map().begin(); it != message.tag_value_map().end(); ++it) {
        if (it->first == kPAD && need_pad_tag) {
            ...
            QUICHE_DCHECK(false) << "Message needed padding but already contained a PAD tag";
            return nullptr;
        }
...
QuicErrorCode CryptoFramer::Process(absl::string_view input)
{
    ...
    case STATE_READING_TAGS_AND_LENGTHS: {
        ...
        for (unsigned i = 0; i < num_entries_; ++i) {
            ...
            if (end_offset < last_end_offset) {
                error_detail_ = absl::StrCat("End offset: ", end_offset, " vs ", last_end_offset);
                return QUIC_CRYPTO_TAGS_OUT_OF_ORDER;
            }
            tags_and_lengths_.push_back(std::make_pair(tag, static_cast<size_t>(end_offset - last_end_offset)));
    ...
    case STATE_READING_VALUES:
        if (reader.BytesRemaining() < values_len_) {
            ...
            QUIC_LOG(ERROR) << "Trunacted message. Missing " << values_len_ - reader.BytesRemaining() << " bytes.";
        }
        for (const std::pair<QuicTag, size_t>& item : tags_and_lengths_) {
            absl::string_view value;
...
// static
bool CryptoFramer::WritePadTag(QuicDataWriter* writer, size_t pad_length, uint32_t* end_offset)
{
