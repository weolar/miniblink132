...
class QUICHE_EXPORT WebTransportFingerprintProofVerifier : public ProofVerifier {
public:
    ...
    public:
        explicit Details(Status status)
            : status_(status)
        {
        }
        Status status() const
    ...
    WebTransportFingerprintProofVerifier(const QuicClock* clock, int max_validity_days);
