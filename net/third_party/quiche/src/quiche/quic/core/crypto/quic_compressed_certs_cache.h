...
public:
    ...
    const std::string* GetCompressedCert(const quiche::QuicheReferenceCountedPointer<ProofSource::Chain>& chain, const std::string& client_cached_cert_hashes);
    ...
    void Insert(const quiche::QuicheReferenceCountedPointer<ProofSource::Chain>& chain, const std::string& client_cached_cert_hashes,
...
private:
    ...
    struct QUICHE_EXPORT UncompressedCerts {
        UncompressedCerts();
        UncompressedCerts(const quiche::QuicheReferenceCountedPointer<ProofSource::Chain>& chain, const std::string* client_cached_cert_hashes);
        ~UncompressedCerts();
    ...
    public:
        CachedCerts();
        CachedCerts(const UncompressedCerts& uncompressed_certs, const std::string& compressed_cert);
        ...
        bool MatchesUncompressedCerts(const UncompressedCerts& uncompressed_certs) const;
    ...
    uint64_t ComputeUncompressedCertsHash(const UncompressedCerts& uncompressed_certs);
