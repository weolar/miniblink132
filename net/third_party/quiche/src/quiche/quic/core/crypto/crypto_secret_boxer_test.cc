...
TEST_F(CryptoSecretBoxerTest, BoxAndUnbox)
{
    ...
    EXPECT_FALSE(boxer.Unbox(box.substr(1, std::string::npos), &storage, &result));
    EXPECT_FALSE(boxer.Unbox(std::string(), &storage, &result));
    EXPECT_FALSE(boxer.Unbox(std::string(1, box[0] ^ 0x80) + box.substr(1, std::string::npos), &storage, &result));
}

// Helper function to test whether one boxer can decode the output of another.
static bool CanDecode(const CryptoSecretBoxer& decoder, const CryptoSecretBoxer& encoder)
{
    absl::string_view message("hello world");
