...
public:
    ...
    static std::unique_ptr<CryptoHandshakeMessage> ParseMessage(absl::string_view in);
    ...
    static std::unique_ptr<QuicData> ConstructHandshakeMessage(const CryptoHandshakeMessage& message);
...
private:
    ...
    static bool WritePadTag(QuicDataWriter* writer, size_t pad_length, uint32_t* end_offset);

    // Represents the current state of the parsing state machine.
    enum CryptoFramerState { STATE_READING_TAG, STATE_READING_NUM_ENTRIES, STATE_READING_TAGS_AND_LENGTHS, STATE_READING_VALUES };
