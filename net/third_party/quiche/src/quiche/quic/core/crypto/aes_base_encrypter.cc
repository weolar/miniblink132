...
bool AesBaseEncrypter::SetHeaderProtectionKey(absl::string_view key)
{
    if (key.size() != GetKeySize()) {
        QUIC_BUG(quic_bug_10726_1) << "Invalid key size for header protection: " << key.size();
        return false;
    }
    if (AES_set_encrypt_key(reinterpret_cast<const uint8_t*>(key.data()), key.size() * 8, &pne_key_) != 0) {
...
std::string AesBaseEncrypter::GenerateHeaderProtectionMask(absl::string_view sample)
{
    if (sample.size() != AES_BLOCK_SIZE) {
        return std::string();
    }
    std::string out(AES_BLOCK_SIZE, 0);
    AES_encrypt(reinterpret_cast<const uint8_t*>(sample.data()), reinterpret_cast<uint8_t*>(const_cast<char*>(out.data())), &pne_key_);
    return out;
}
...
QuicPacketCount AesBaseEncrypter::GetConfidentialityLimit() const
{
    ...
    static_assert(kMaxOutgoingPacketSize <= 2048, "This key limit requires limits on encryption payload sizes");
    return 268435456U;
}
