...
TEST(CryptoFramerTest, ProcessInput)
{
    ...
    EXPECT_TRUE(framer.ProcessInput(absl::string_view(AsChars(input), ABSL_ARRAYSIZE(input))));
...
TEST(CryptoFramerTest, ProcessInputTagsOutOfOrder)
{
    ...
    EXPECT_FALSE(framer.ProcessInput(absl::string_view(AsChars(input), ABSL_ARRAYSIZE(input))));
