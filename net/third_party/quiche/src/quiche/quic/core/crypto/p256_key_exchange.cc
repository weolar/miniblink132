...
P256KeyExchange::P256KeyExchange(bssl::UniquePtr<EC_KEY> private_key, const uint8_t* public_key)
...
std::unique_ptr<P256KeyExchange> P256KeyExchange::New(absl::string_view key)
{
    ...
    bssl::UniquePtr<EC_KEY> private_key(d2i_ECPrivateKey(nullptr, &keyp, key.size()));
    ...
    if (EC_POINT_point2oct(EC_KEY_get0_group(private_key.get()), EC_KEY_get0_public_key(private_key.get()), POINT_CONVERSION_UNCOMPRESSED, public_key,
    ...
    return absl::WrapUnique(new P256KeyExchange(std::move(private_key), public_key));
}
...
bool P256KeyExchange::CalculateSharedKeySync(absl::string_view peer_public_value, std::string* shared_key) const
{
    ...
    bssl::UniquePtr<EC_POINT> point(EC_POINT_new(EC_KEY_get0_group(private_key_.get())));
    if (!point.get()
        || !EC_POINT_oct2point(/* also test if point is on curve */
            EC_KEY_get0_group(private_key_.get()), point.get(), reinterpret_cast<const uint8_t*>(peer_public_value.data()), peer_public_value.size(),
            nullptr)) {
    ...
    if (ECDH_compute_key(result, sizeof(result), point.get(), private_key_.get(), nullptr) != sizeof(result)) {
...
{
    return absl::string_view(reinterpret_cast<const char*>(public_key_), sizeof(public_key_));
}
