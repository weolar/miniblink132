...
public:
    ...
    public:
        TestCallback(TestCallbackResult* result)
            : result_(result)
        {
        }
...
TEST_F(Curve25519KeyExchangeTest, SharedKey)
{
    ...
    for (int i = 0; i < 5; i++) {
        ...
        std::unique_ptr<Curve25519KeyExchange> alice(Curve25519KeyExchange::New(alice_key));
        std::unique_ptr<Curve25519KeyExchange> bob(Curve25519KeyExchange::New(bob_key));
...
TEST_F(Curve25519KeyExchangeTest, SharedKeyAsync)
{
    ...
    for (int i = 0; i < 5; i++) {
        ...
        alice->CalculateSharedKeyAsync(bob_public, &alice_shared, std::make_unique<TestCallback>(&alice_result));
        ASSERT_TRUE(alice_result.ok());
        TestCallbackResult bob_result;
        ASSERT_FALSE(bob_result.ok());
        bob->CalculateSharedKeyAsync(alice_public, &bob_shared, std::make_unique<TestCallback>(&bob_result));
