...
{
    std::vector<std::string> certs = { "leaf cert", "intermediate cert", "root cert" };
    ...
    const std::string* cached_value = certs_cache_.GetCompressedCert(chain, cached_certs);
...
TEST_F(QuicCompressedCertsCacheTest, CacheMiss)
{
    ...
    EXPECT_EQ(nullptr, certs_cache_.GetCompressedCert(chain, "mismatched cached certs"));
...
TEST_F(QuicCompressedCertsCacheTest, CacheMissDueToEviction)
{
    ...
    // cached cert.
    for (unsigned int i = 0; i < QuicCompressedCertsCache::kQuicCompressedCertsCacheSize; i++) {
