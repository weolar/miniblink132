...
bool NullDecrypter::DecryptPacket(
...
{
    QuicDataReader reader(ciphertext.data(), ciphertext.length(), quiche::HOST_BYTE_ORDER);
    absl::uint128 hash;
    ...
    if (plaintext.length() > max_output_length) {
        QUIC_BUG(quic_bug_10652_3) << "Output buffer must be larger than the plaintext.";
        return false;
    }
...
std::string NullDecrypter::GenerateHeaderProtectionMask(QuicDataReader* /*sample_reader*/)
{
...
absl::uint128 NullDecrypter::ComputeHash(const absl::string_view data1, const absl::string_view data2) const
{
