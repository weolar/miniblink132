...
TlsClientConnection::TlsClientConnection(SSL_CTX* ssl_ctx, Delegate* delegate, QuicSSLConfig ssl_config)
...
// static
bssl::UniquePtr<SSL_CTX> TlsClientConnection::CreateSslCtx(bool enable_early_data)
{
...
void TlsClientConnection::SetCertChain(const std::vector<CRYPTO_BUFFER*>& cert_chain, EVP_PKEY* privkey)
{
...
{
    static_cast<TlsClientConnection*>(ConnectionFromSsl(ssl))->delegate_->InsertSession(bssl::UniquePtr<SSL_SESSION>(session));
    return 1;
}
