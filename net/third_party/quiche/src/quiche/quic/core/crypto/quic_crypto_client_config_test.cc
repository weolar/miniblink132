...
TEST_F(QuicCryptoClientConfigTest, InchoateChlo)
{
    ...
    config.set_alpn("hq");
    quiche::QuicheReferenceCountedPointer<QuicCryptoNegotiatedParameters> params(new QuicCryptoNegotiatedParameters);
...
TEST_F(QuicCryptoClientConfigTest, InchoateChloSecureWithSCIDNoEXPY)
{
    ...
    state.SetServerConfig(scfg.GetSerialized().AsStringPiece(), now, expiry, &details);
...
TEST_F(QuicCryptoClientConfigTest, InchoateChloSecureWithSCID)
{
    ...
    scfg.SetStringPiece(kSCID, "12345678");
    std::string details;
    state.SetServerConfig(scfg.GetSerialized().AsStringPiece(), QuicWallTime::FromUNIXSeconds(1), QuicWallTime::FromUNIXSeconds(0), &details);
    EXPECT_FALSE(state.IsEmpty());
...
TEST_F(QuicCryptoClientConfigTest, FillClientHello)
{
    ...
    QuicServerId server_id("www.google.com", 443);
    config.FillClientHello(server_id, kConnectionId, QuicVersionMax(), QuicVersionMax(), &state, QuicWallTime::Zero(), &rand, params, &chlo, &error_details);
...
TEST_F(QuicCryptoClientConfigTest, ProcessServerDowngradeAttack)
{
    ...
    quiche::QuicheReferenceCountedPointer<QuicCryptoNegotiatedParameters> out_params(new QuicCryptoNegotiatedParameters);
    std::string error;
    QuicCryptoClientConfig config(crypto_test_utils::ProofVerifierForTesting());
    EXPECT_THAT(config.ProcessServerHello(msg, EmptyQuicConnectionId(), supported_versions.front(), supported_versions, &cached, out_params, &error),
...
TEST_F(QuicCryptoClientConfigTest, ClearCachedStates)
{
    ...
    struct TestCase {
            ...
            , state(config->LookupOrCreate(server_id))
        {
            ...
            scfg.SetStringPiece(kSCID, "12345678");
            std::string details;
            state->SetServerConfig(scfg.GetSerialized().AsStringPiece(), QuicWallTime::FromUNIXSeconds(0), QuicWallTime::FromUNIXSeconds(future), &details);
        ...
        QuicCryptoClientConfig::CachedState* state;
    } test_cases[] = { TestCase("www.google.com", &config), TestCase("www.example.com", &config) };

    // Verify LookupOrCreate returns the same data.
    for (const TestCase& test_case : test_cases) {
        QuicCryptoClientConfig::CachedState* other = config.LookupOrCreate(test_case.server_id);
    ...
    QuicCryptoClientConfig::CachedState* cleared_cache = config.LookupOrCreate(test_cases[0].server_id);
    ...
    QuicCryptoClientConfig::CachedState* existing_cache = config.LookupOrCreate(test_cases[1].server_id);
...
TEST_F(QuicCryptoClientConfigTest, ProcessReject)
{
    ...
    std::string error;
    QuicCryptoClientConfig config(crypto_test_utils::ProofVerifierForTesting());
    EXPECT_THAT(config.ProcessRejection(
                    rej, QuicWallTime::FromUNIXSeconds(0), AllSupportedVersionsWithQuicCrypto().front().transport_version, "", &cached, out_params, &error),
        IsQuicNoError());
}
...
TEST_F(QuicCryptoClientConfigTest, ProcessRejectWithLongTTL)
{
    ...
    EXPECT_FALSE(cached.IsComplete(QuicWallTime::FromUNIXSeconds(one_week.ToSeconds())));
    EXPECT_TRUE(cached.IsComplete(QuicWallTime::FromUNIXSeconds(one_week.ToSeconds() - 1)));
}
...
TEST_F(QuicCryptoClientConfigTest, ServerNonceinSHLO)
{
    ...
    quiche::QuicheReferenceCountedPointer<QuicCryptoNegotiatedParameters> out_params(new QuicCryptoNegotiatedParameters);
    std::string error_details;
    EXPECT_THAT(config.ProcessServerHello(msg, EmptyQuicConnectionId(), version, supported_versions, &cached, out_params, &error_details),
...
TEST_F(QuicCryptoClientConfigTest, MultipleCanonicalEntries)
{
    ...
    QuicCryptoClientConfig::CachedState* state1 = config.LookupOrCreate(canonical_server_id1);
    ...
    QuicWallTime now = QuicWallTime::FromUNIXSeconds(1);
    QuicWallTime expiry = QuicWallTime::FromUNIXSeconds(2);
    state1->SetServerConfig(scfg.GetSerialized().AsStringPiece(), now, expiry, &details);
    ...
    QuicCryptoClientConfig::CachedState* state2 = config.LookupOrCreate(canonical_server_id2);
    ...
    QuicCryptoClientConfig::CachedState* state3 = config.LookupOrCreate(canonical_server_id3);
