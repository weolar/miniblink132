...
public:
    ...
    void Run(bool /*ok*/, const std::string& /*error_details*/, std::unique_ptr<ProofVerifyDetails>* /*details*/) override
    {
        QUICHE_DCHECK(false);
    }
};
...
struct TestParams {
    ...
    {
        os << "  versions: " << ParsedQuicVersionVectorToString(p.supported_versions) << " }";
        return os;
    }
...
std::vector<TestParams> GetTestParams()
{
    ...
    ParsedQuicVersionVector supported_versions = AllSupportedVersionsWithQuicCrypto();
...
public:
    CryptoServerTest()
        ...
        , config_(QuicCryptoServerConfig::TESTING, rand_, crypto_test_utils::ProofSourceForTesting(), KeyExchangeSource::Default())
        , peer_(&config_)
        ...
        , chlo_packet_size_(kDefaultMaxPacketSize)
    {
        ...
        client_version_string_ = std::string(reinterpret_cast<const char*>(&client_version_label_), sizeof(client_version_label_));
    }
    ...
    void SetUp() override
    {
        ...
        QuicServerConfigProtobuf primary_config = config_.GenerateConfig(rand_, &clock_, config_options_);
        primary_config.set_primary_time(clock_.WallNow().ToUNIXSeconds());
        std::unique_ptr<CryptoHandshakeMessage> msg(config_.AddConfig(primary_config, clock_.WallNow()));
        ...
        pub_hex_ = "#" + absl::BytesToHexString(absl::string_view(public_value, sizeof(public_value)));

        CryptoHandshakeMessage client_hello = crypto_test_utils::CreateCHLO({ { "PDMD", "X509" }, { "AEAD", "AESG" }, { "KEXS", "C255" }, { "PUBS", pub_hex_ },
                                                                                { "NONC", nonce_hex_ }, { "CSCT", "" }, { "VER\0", client_version_string_ } },
            kClientHelloMinimumSize);
        ShouldSucceed(client_hello);
        // The message should be rejected because the source-address token is
        // missing.
        CheckRejectTag();
        const HandshakeFailureReason kRejectReasons[] = { SERVER_CONFIG_INCHOATE_HELLO_FAILURE };
        ...
        signed_config_ = quiche::QuicheReferenceCountedPointer<QuicSignedServerConfig>(new QuicSignedServerConfig());
    ...
    public:
        ValidateCallback(CryptoServerTest* test, bool should_succeed, const char* error_substr, bool* called)
            : test_(test)
        ...
        void Run(quiche::QuicheReferenceCountedPointer<Result> result, std::unique_ptr<ProofSource::Details> /* details */) override
        {
            ASSERT_FALSE(*called_);
            test_->ProcessValidationResult(std::move(result), should_succeed_, error_substr_);
            *called_ = true;
        }
    ...
    void ShouldSucceed(const CryptoHandshakeMessage& message)
    {
        ...
        config_.ValidateClientHello(message, client_address_, server_address, supported_versions_.front().transport_version, &clock_, signed_config_,
            std::make_unique<ValidateCallback>(this, true, "", &called));
        EXPECT_TRUE(called);
    }

    void ShouldFailMentioning(const char* error_substr, const CryptoHandshakeMessage& message)
    {
        bool called = false;
    ...
    void ShouldFailMentioning(const char* error_substr, const CryptoHandshakeMessage& message, bool* called)
    {
        QuicSocketAddress server_address(QuicIpAddress::Any4(), 5);
        config_.ValidateClientHello(message, client_address_, server_address, supported_versions_.front().transport_version, &clock_, signed_config_,
    ...
    public:
        ProcessCallback(quiche::QuicheReferenceCountedPointer<ValidateCallback::Result> result, bool should_succeed, const char* error_substr, bool* called,
        ...
        {
            if (should_succeed_) {
                ASSERT_EQ(error, QUIC_NO_ERROR) << "Message failed with error " << error_details << ": " << result_->client_hello.DebugString();
            } else {
                ASSERT_NE(error, QUIC_NO_ERROR) << "Message didn't fail: " << result_->client_hello.DebugString();
                EXPECT_TRUE(absl::StrContains(error_details, error_substr_)) << error_substr_ << " not in " << error_details;
            }
            if (message != nullptr) {
                *out_ = *message;
            }
    ...
    private:
        const quiche::QuicheReferenceCountedPointer<ValidateCallback::Result> result_;
        const bool should_succeed_;
        const char* const error_substr_;
        bool* called_;
    ...
    void ProcessValidationResult(quiche::QuicheReferenceCountedPointer<ValidateCallback::Result> result, bool should_succeed, const char* error_substr)
    {
        QuicSocketAddress server_address(QuicIpAddress::Any4(), 5);
        bool called;
        config_.ProcessClientHello(result, /*reject_only=*/false,
            /*connection_id=*/TestConnectionId(1), server_address, client_address_, supported_versions_.front(), supported_versions_, &clock_, rand_,
            &compressed_certs_cache_, params_, signed_config_,
            /*total_framing_overhead=*/50, chlo_packet_size_, std::make_unique<ProcessCallback>(result, should_succeed, error_substr, &called, &out_));
        EXPECT_TRUE(called);
    }
    ...
    {
        std::string nonce;
        CryptoUtils::GenerateNonce(clock_.WallNow(), rand_, absl::string_view(reinterpret_cast<const char*>(orbit_), sizeof(orbit_)), &nonce);
        return nonce;
    }

    void CheckRejectReasons(const HandshakeFailureReason* expected_handshake_failures, size_t expected_count)
    {
        ...
        for (size_t i = 0; i < reject_reasons.size(); ++i) {
            EXPECT_EQ(static_cast<QuicTag>(expected_handshake_failures[i]), reject_reasons[i]);
        }
    }
    ...
    std::string XlctHexString()
    {
        ...
        return "#" + absl::BytesToHexString(absl::string_view(reinterpret_cast<char*>(&xlct), sizeof(xlct)));
    }
...
INSTANTIATE_TEST_SUITE_P(CryptoServerTests, CryptoServerTest, ::testing::ValuesIn(GetTestParams()), ::testing::PrintToStringParamName());
...
TEST_P(CryptoServerTest, BadSNI)
{
    ...
    for (const std::string& bad_sni : badSNIs) {
        CryptoHandshakeMessage msg
            = crypto_test_utils::CreateCHLO({ { "PDMD", "X509" }, { "SNI", bad_sni }, { "VER\0", client_version_string_ } }, kClientHelloMinimumSize);
    ...
    CryptoHandshakeMessage msg
        = crypto_test_utils::CreateCHLO({ { "PDMD", "X509" }, { "SNI", "foo" }, { "VER\0", client_version_string_ } }, kClientHelloMinimumSize);
    ShouldSucceed(msg);
}
...
TEST_P(CryptoServerTest, DefaultCert)
{
    ...
    CryptoHandshakeMessage msg = crypto_test_utils::CreateCHLO(
        { { "AEAD", "AESG" }, { "KEXS", "C255" }, { "PUBS", pub_hex_ }, { "NONC", nonce_hex_ }, { "PDMD", "X509" }, { "VER\0", client_version_string_ } },
        kClientHelloMinimumSize);
...
TEST_P(CryptoServerTest, RejectTooLarge)
{
    ...
    CryptoHandshakeMessage msg = crypto_test_utils::CreateCHLO({ { "PDMD", "X509" }, { "AEAD", "AESG" }, { "KEXS", "C255" }, { "PUBS", pub_hex_ },
                                                                   { "NONC", nonce_hex_ }, { "PDMD", "X509" }, { "VER\0", client_version_string_ } },
        kClientHelloMinimumSize);
...
TEST_P(CryptoServerTest, RejectTooLargeButValidSTK)
{
    ...
    // constructed with a PDMD but no SKT when the REJ would be too large.
    CryptoHandshakeMessage msg
        = crypto_test_utils::CreateCHLO({ { "PDMD", "X509" }, { "AEAD", "AESG" }, { "KEXS", "C255" }, { "PUBS", pub_hex_ }, { "NONC", nonce_hex_ },
                                            { "#004b5453", srct_hex_ }, { "PDMD", "X509" }, { "VER\0", client_version_string_ } },
            kClientHelloMinimumSize);
...
TEST_P(CryptoServerTest, BadSourceAddressToken)
{
  ...
  static const char* const kBadSourceAddressTokens[] = {
    ...
    for (size_t i = 0; i < ABSL_ARRAYSIZE(kBadSourceAddressTokens); i++) {
        CryptoHandshakeMessage msg = crypto_test_utils::CreateCHLO(
            { { "PDMD", "X509" }, { "STK", kBadSourceAddressTokens[i] }, { "VER\0", client_version_string_ } }, kClientHelloMinimumSize);
        ShouldSucceed(msg);
        const HandshakeFailureReason kRejectReasons[] = { SERVER_CONFIG_INCHOATE_HELLO_FAILURE };
        CheckRejectReasons(kRejectReasons, ABSL_ARRAYSIZE(kRejectReasons));
    }
...
TEST_P(CryptoServerTest, BadClientNonce)
{
  ...
  static const char* const kBadNonces[] = {
    ...
    for (size_t i = 0; i < ABSL_ARRAYSIZE(kBadNonces); i++) {
        ...
        CryptoHandshakeMessage msg
            = crypto_test_utils::CreateCHLO({ { "PDMD", "X509" }, { "NONC", kBadNonces[i] }, { "VER\0", client_version_string_ } }, kClientHelloMinimumSize);
        ...
        CryptoHandshakeMessage msg1 = crypto_test_utils::CreateCHLO(
            { { "PDMD", "X509" }, { "AEAD", "AESG" }, { "KEXS", "C255" }, { "SCID", scid_hex_ }, { "#004b5453", srct_hex_ }, { "PUBS", pub_hex_ },
                { "NONC", kBadNonces[i] }, { "NONP", kBadNonces[i] }, { "XLCT", XlctHexString() }, { "VER\0", client_version_string_ } },
            kClientHelloMinimumSize);

        ShouldSucceed(msg1);

        CheckRejectTag();
        const HandshakeFailureReason kRejectReasons1[] = { CLIENT_NONCE_INVALID_FAILURE, SERVER_NONCE_REQUIRED_FAILURE };
        CheckRejectReasons(kRejectReasons1, (GetQuicReloadableFlag(quic_require_handshake_confirmation) ? ABSL_ARRAYSIZE(kRejectReasons1) : 1));
    }
}
...
TEST_P(CryptoServerTest, NoClientNonce)
{
    ...
    CryptoHandshakeMessage msg = crypto_test_utils::CreateCHLO({ { "PDMD", "X509" }, { "VER\0", client_version_string_ } }, kClientHelloMinimumSize);
    ...
    CryptoHandshakeMessage msg1
        = crypto_test_utils::CreateCHLO({ { "PDMD", "X509" }, { "AEAD", "AESG" }, { "KEXS", "C255" }, { "SCID", scid_hex_ }, { "#004b5453", srct_hex_ },
                                            { "PUBS", pub_hex_ }, { "XLCT", XlctHexString() }, { "VER\0", client_version_string_ } },
            kClientHelloMinimumSize);

    ShouldSucceed(msg1);
    CheckRejectTag();
    const HandshakeFailureReason kRejectReasons1[] = { SERVER_CONFIG_INCHOATE_HELLO_FAILURE };
...
TEST_P(CryptoServerTest, DowngradeAttack)
{
    ...
    std::string bad_version = ParsedQuicVersionToString(supported_versions_.back());

    CryptoHandshakeMessage msg = crypto_test_utils::CreateCHLO({ { "PDMD", "X509" }, { "VER\0", bad_version } }, kClientHelloMinimumSize);
...
TEST_P(CryptoServerTest, CorruptServerConfig)
{
    ...
    CryptoHandshakeMessage msg
        = crypto_test_utils::CreateCHLO({ { "PDMD", "X509" }, { "AEAD", "AESG" }, { "KEXS", "C255" }, { "SCID", (std::string(1, 'X') + scid_hex_) },
                                            { "#004b5453", srct_hex_ }, { "PUBS", pub_hex_ }, { "NONC", nonce_hex_ }, { "VER\0", client_version_string_ } },
            kClientHelloMinimumSize);

    ShouldSucceed(msg);
    CheckRejectTag();
    const HandshakeFailureReason kRejectReasons[] = { SERVER_CONFIG_UNKNOWN_CONFIG_FAILURE };
    CheckRejectReasons(kRejectReasons, ABSL_ARRAYSIZE(kRejectReasons));
}
...
TEST_P(CryptoServerTest, CorruptSourceAddressToken)
{
    ...
    CryptoHandshakeMessage msg = crypto_test_utils::CreateCHLO(
        { { "PDMD", "X509" }, { "AEAD", "AESG" }, { "KEXS", "C255" }, { "SCID", scid_hex_ }, { "#004b5453", (std::string(1, 'X') + srct_hex_) },
            { "PUBS", pub_hex_ }, { "NONC", nonce_hex_ }, { "XLCT", XlctHexString() }, { "VER\0", client_version_string_ } },
        kClientHelloMinimumSize);

    ShouldSucceed(msg);
    CheckRejectTag();
    const HandshakeFailureReason kRejectReasons[] = { SOURCE_ADDRESS_TOKEN_DECRYPTION_FAILURE };
    const HandshakeFailureReason kRejectReasons1[] = { SOURCE_ADDRESS_TOKEN_DECRYPTION_FAILURE, SERVER_NONCE_REQUIRED_FAILURE };
    CheckRejectReasons((GetQuicReloadableFlag(quic_require_handshake_confirmation) ? kRejectReasons1 : kRejectReasons),
        (GetQuicReloadableFlag(quic_require_handshake_confirmation) ? ABSL_ARRAYSIZE(kRejectReasons1) : ABSL_ARRAYSIZE(kRejectReasons)));
}
...
TEST_P(CryptoServerTest, CorruptSourceAddressTokenIsStillAccepted)
{
    ...
    if (GetQuicReloadableFlag(quic_require_handshake_confirmation)) {
        CheckRejectTag();
        const HandshakeFailureReason kRejectReasons[] = { SERVER_NONCE_REQUIRED_FAILURE };
        CheckRejectReasons(kRejectReasons, ABSL_ARRAYSIZE(kRejectReasons));
...
TEST_P(CryptoServerTest, CorruptClientNonceAndSourceAddressToken)
{
    ...
    CryptoHandshakeMessage msg = crypto_test_utils::CreateCHLO(
        { { "PDMD", "X509" }, { "AEAD", "AESG" }, { "KEXS", "C255" }, { "SCID", scid_hex_ }, { "#004b5453", (std::string(1, 'X') + srct_hex_) },
            { "PUBS", pub_hex_ }, { "NONC", (std::string(1, 'X') + nonce_hex_) }, { "XLCT", XlctHexString() }, { "VER\0", client_version_string_ } },
        kClientHelloMinimumSize);

    ShouldSucceed(msg);
    CheckRejectTag();
    const HandshakeFailureReason kRejectReasons[] = { SOURCE_ADDRESS_TOKEN_DECRYPTION_FAILURE, CLIENT_NONCE_INVALID_FAILURE };
    const HandshakeFailureReason kRejectReasons1[] = { SOURCE_ADDRESS_TOKEN_DECRYPTION_FAILURE, CLIENT_NONCE_INVALID_FAILURE, SERVER_NONCE_REQUIRED_FAILURE };
    CheckRejectReasons((GetQuicReloadableFlag(quic_require_handshake_confirmation) ? kRejectReasons1 : kRejectReasons),
        (GetQuicReloadableFlag(quic_require_handshake_confirmation) ? ABSL_ARRAYSIZE(kRejectReasons1) : ABSL_ARRAYSIZE(kRejectReasons)));
}
...
TEST_P(CryptoServerTest, CorruptMultipleTags)
{
    ...
    CryptoHandshakeMessage msg = crypto_test_utils::CreateCHLO(
        { { "PDMD", "X509" }, { "AEAD", "AESG" }, { "KEXS", "C255" }, { "SCID", scid_hex_ }, { "#004b5453", (std::string(1, 'X') + srct_hex_) },
            { "PUBS", pub_hex_ }, { "NONC", (std::string(1, 'X') + nonce_hex_) }, { "NONP", (std::string(1, 'X') + nonce_hex_) },
            { "SNO\0", (std::string(1, 'X') + nonce_hex_) }, { "XLCT", XlctHexString() }, { "VER\0", client_version_string_ } },
        kClientHelloMinimumSize);
...
TEST_P(CryptoServerTest, NoServerNonce)
{
    ...
    CryptoHandshakeMessage msg = crypto_test_utils::CreateCHLO(
        { { "PDMD", "X509" }, { "AEAD", "AESG" }, { "KEXS", "C255" }, { "SCID", scid_hex_ }, { "#004b5453", srct_hex_ }, { "PUBS", pub_hex_ },
            { "NONC", nonce_hex_ }, { "NONP", nonce_hex_ }, { "XLCT", XlctHexString() }, { "VER\0", client_version_string_ } },
        kClientHelloMinimumSize);
...
TEST_P(CryptoServerTest, ProofForSuppliedServerConfig)
{
    ...
    CryptoHandshakeMessage msg = crypto_test_utils::CreateCHLO(
        { { "AEAD", "AESG" }, { "KEXS", "C255" }, { "PDMD", "X509" }, { "SCID", kOldConfigId }, { "#004b5453", srct_hex_ }, { "PUBS", pub_hex_ },
            { "NONC", nonce_hex_ }, { "NONP", "123456789012345678901234567890" }, { "VER\0", client_version_string_ }, { "XLCT", XlctHexString() } },
        kClientHelloMinimumSize);
    ...
    // longer valid.
    CheckRejectTag();
    const HandshakeFailureReason kRejectReasons[] = { SOURCE_ADDRESS_TOKEN_DIFFERENT_IP_ADDRESS_FAILURE };
    const HandshakeFailureReason kRejectReasons1[] = { SOURCE_ADDRESS_TOKEN_DIFFERENT_IP_ADDRESS_FAILURE, SERVER_NONCE_REQUIRED_FAILURE };
    CheckRejectReasons((GetQuicReloadableFlag(quic_require_handshake_confirmation) ? kRejectReasons1 : kRejectReasons),
        (GetQuicReloadableFlag(quic_require_handshake_confirmation) ? ABSL_ARRAYSIZE(kRejectReasons1) : ABSL_ARRAYSIZE(kRejectReasons)));
    ...
    std::unique_ptr<CryptoHandshakeMessage> scfg(CryptoFramer::ParseMessage(scfg_str));
    absl::string_view scid;
    ...
    std::unique_ptr<ProofVerifier> proof_verifier(crypto_test_utils::ProofVerifierForTesting());
    std::unique_ptr<ProofVerifyContext> verify_context(crypto_test_utils::ProofVerifyContextForTesting());
    std::unique_ptr<ProofVerifyDetails> details;
    std::string error_details;
    std::unique_ptr<ProofVerifierCallback> callback(new DummyProofVerifierCallback());
    const std::string chlo_hash = CryptoUtils::HashHandshakeMessage(msg, Perspective::IS_SERVER);
    EXPECT_EQ(QUIC_SUCCESS,
        proof_verifier->VerifyProof("test.example.com", 443, (std::string(scfg_str)), client_version_.transport_version, chlo_hash, certs, "",
            (std::string(proof)), verify_context.get(), &error_details, &details, std::move(callback)));
}
...
TEST_P(CryptoServerTest, RejectInvalidXlct)
{
    CryptoHandshakeMessage msg = crypto_test_utils::CreateCHLO(
        { { "PDMD", "X509" }, { "AEAD", "AESG" }, { "KEXS", "C255" }, { "SCID", scid_hex_ }, { "#004b5453", srct_hex_ }, { "PUBS", pub_hex_ },
            { "NONC", nonce_hex_ }, { "VER\0", client_version_string_ }, { "XLCT", "#0102030405060708" } },
        kClientHelloMinimumSize);
    ...
    const HandshakeFailureReason kRejectReasons1[] = { INVALID_EXPECTED_LEAF_CERTIFICATE };
...
TEST_P(CryptoServerTest, ValidXlct)
{
    CryptoHandshakeMessage msg
        = crypto_test_utils::CreateCHLO({ { "PDMD", "X509" }, { "AEAD", "AESG" }, { "KEXS", "C255" }, { "SCID", scid_hex_ }, { "#004b5453", srct_hex_ },
                                            { "PUBS", pub_hex_ }, { "NONC", nonce_hex_ }, { "VER\0", client_version_string_ }, { "XLCT", XlctHexString() } },
            kClientHelloMinimumSize);
...
TEST_P(CryptoServerTest, ProofSourceFailure)
{
    ...
    CryptoHandshakeMessage msg = crypto_test_utils::CreateCHLO({ { "AEAD", "AESG" }, { "KEXS", "C255" }, { "SCID", scid_hex_ }, { "PUBS", pub_hex_ },
                                                                   { "NONC", nonce_hex_ }, { "PDMD", "X509" }, { "VER\0", client_version_string_ } },
        kClientHelloMinimumSize);
...
TEST_F(CryptoServerConfigGenerationTest, Determinism)
{
    ...
    QuicCryptoServerConfig a(QuicCryptoServerConfig::TESTING, &rand_a, crypto_test_utils::ProofSourceForTesting(), KeyExchangeSource::Default());
    QuicCryptoServerConfig b(QuicCryptoServerConfig::TESTING, &rand_b, crypto_test_utils::ProofSourceForTesting(), KeyExchangeSource::Default());
    std::unique_ptr<CryptoHandshakeMessage> scfg_a(a.AddDefaultConfig(&rand_a, &clock, options));
    std::unique_ptr<CryptoHandshakeMessage> scfg_b(b.AddDefaultConfig(&rand_b, &clock, options));
...
TEST_F(CryptoServerConfigGenerationTest, SCIDIsHashOfServerConfig)
{
    ...
    QuicCryptoServerConfig a(QuicCryptoServerConfig::TESTING, &rand_a, crypto_test_utils::ProofSourceForTesting(), KeyExchangeSource::Default());
    std::unique_ptr<CryptoHandshakeMessage> scfg(a.AddDefaultConfig(&rand_a, &clock, options));
