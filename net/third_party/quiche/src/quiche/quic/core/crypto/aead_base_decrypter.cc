...
AeadBaseDecrypter::AeadBaseDecrypter(
    const EVP_AEAD* (*aead_getter)(), size_t key_size, size_t auth_tag_size, size_t nonce_size, bool use_ietf_nonce_construction)
...
bool AeadBaseDecrypter::SetNoncePrefix(absl::string_view nonce_prefix)
{
    if (use_ietf_nonce_construction_) {
        QUIC_BUG(quic_bug_10709_1) << "Attempted to set nonce prefix on IETF QUIC crypter";
        return false;
    }
...
bool AeadBaseDecrypter::SetDiversificationNonce(const DiversificationNonce& nonce)
{
    ...
    DiversifyPreliminaryKey(absl::string_view(reinterpret_cast<const char*>(key_), key_size_),
        absl::string_view(reinterpret_cast<const char*>(iv_), prefix_size), nonce, key_size_, prefix_size, &key, &nonce_prefix);

    if (!SetKey(key) || (!use_ietf_nonce_construction_ && !SetNoncePrefix(nonce_prefix)) || (use_ietf_nonce_construction_ && !SetIV(nonce_prefix))) {
        QUICHE_DCHECK(false);
        return false;
    }
...
bool AeadBaseDecrypter::DecryptPacket(
    uint64_t packet_number, absl::string_view associated_data, absl::string_view ciphertext, char* output, size_t* output_length, size_t max_output_length)
{
    ...
    if (have_preliminary_key_) {
        QUIC_BUG(quic_bug_10709_3) << "Unable to decrypt while key diversification is pending";
        return false;
    }
    ...
    if (use_ietf_nonce_construction_) {
        for (size_t i = 0; i < sizeof(packet_number); ++i) {
            nonce[prefix_len + i] ^= (packet_number >> ((sizeof(packet_number) - i - 1) * 8)) & 0xff;
        }
    } else {
        memcpy(nonce + prefix_len, &packet_number, sizeof(packet_number));
    }
    if (!EVP_AEAD_CTX_open(ctx_.get(), reinterpret_cast<uint8_t*>(output), output_length, max_output_length, reinterpret_cast<const uint8_t*>(nonce),
            nonce_size_, reinterpret_cast<const uint8_t*>(ciphertext.data()), ciphertext.size(), reinterpret_cast<const uint8_t*>(associated_data.data()),
...
{
    return absl::string_view(reinterpret_cast<const char*>(iv_), nonce_size_ - sizeof(QuicPacketNumber));
}
