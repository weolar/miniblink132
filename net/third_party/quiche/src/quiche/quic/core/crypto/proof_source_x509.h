...
public:
    ...
    static std::unique_ptr<ProofSourceX509> Create(quiche::QuicheReferenceCountedPointer<Chain> default_chain, CertificatePrivateKey default_key);
    ...
    ABSL_MUST_USE_RESULT bool AddCertificateChain(quiche::QuicheReferenceCountedPointer<Chain> chain, CertificatePrivateKey key);

protected:
    ProofSourceX509(quiche::QuicheReferenceCountedPointer<Chain> default_chain, CertificatePrivateKey default_key);
    bool valid() const
    {
        return default_certificate_ != nullptr;
    }

    // Gives an opportunity for the subclass proof source to provide SCTs for a
    // given hostname.
    virtual void MaybeAddSctsForHostname(absl::string_view /*hostname*/, std::string& /*leaf_cert_scts*/)
    {
    }
...
private:
    ...
    // found.
    Certificate* GetCertificate(const std::string& hostname, bool* cert_matched_sni) const;
