...
public:
    ...
    virtual void Insert(
        const QuicServerId& server_id, bssl::UniquePtr<SSL_SESSION> session, const TransportParameters& params, const ApplicationState* application_state)
        = 0;
    ...
    // are used only once.
    virtual std::unique_ptr<QuicResumptionState> Lookup(const QuicServerId& server_id, QuicWallTime now, const SSL_CTX* ctx) = 0;
    ...
    virtual void OnNewTokenReceived(const QuicServerId& server_id, absl::string_view token) = 0;
...
public:
    ...
    public:
        ...
        // has expired.
        ServerConfigState SetServerConfig(absl::string_view server_config, QuicWallTime now, QuicWallTime expiry_time, std::string* error_details);
        ...
        void SetProof(const std::vector<std::string>& certs, absl::string_view cert_sct, absl::string_view chlo_hash, absl::string_view signature);
        ...
        bool Initialize(absl::string_view server_config, absl::string_view source_address_token, const std::vector<std::string>& certs,
            const std::string& cert_sct, absl::string_view chlo_hash, absl::string_view signature, QuicWallTime now, QuicWallTime expiration_time);
    ...
    explicit QuicCryptoClientConfig(std::unique_ptr<ProofVerifier> proof_verifier);
    QuicCryptoClientConfig(std::unique_ptr<ProofVerifier> proof_verifier, std::shared_ptr<SessionCache> session_cache);
    ...
    void FillInchoateClientHello(const QuicServerId& server_id, const ParsedQuicVersion preferred_version, const CachedState* cached, QuicRandom* rand,
        bool demand_x509_proof, quiche::QuicheReferenceCountedPointer<QuicCryptoNegotiatedParameters> out_params, CryptoHandshakeMessage* out) const;
    ...
    QuicErrorCode FillClientHello(const QuicServerId& server_id, QuicConnectionId connection_id, const ParsedQuicVersion preferred_version,
    ...
    QuicErrorCode ProcessRejection(const CryptoHandshakeMessage& rej, QuicWallTime now, QuicTransportVersion version, absl::string_view chlo_hash,
        CachedState* cached, quiche::QuicheReferenceCountedPointer<QuicCryptoNegotiatedParameters> out_params, std::string* error_details);
    ...
    QuicErrorCode ProcessServerHello(const CryptoHandshakeMessage& server_hello, QuicConnectionId connection_id, ParsedQuicVersion version,
    ...
    QuicErrorCode ProcessServerConfigUpdate(const CryptoHandshakeMessage& server_config_update, QuicWallTime now, const QuicTransportVersion version,
    ...
    void InitializeFrom(const QuicServerId& server_id, const QuicServerId& canonical_server_id, QuicCryptoClientConfig* canonical_crypto_config);
    ...
    // handshake message.
    const std::string& user_agent_id() const
    {
    ...
    void set_alpn(const std::string& alpn)
    {
        alpn_ = alpn;
    }
    ...
    bool pad_inchoate_hello() const
    {
        return pad_inchoate_hello_;
    }
    ...
    bool pad_full_hello() const
    {
        return pad_full_hello_;
    }
    void set_pad_full_hello(bool new_value)
    {
        pad_full_hello_ = new_value;
    }

#if BORINGSSL_API_VERSION >= 27
    bool alps_use_new_codepoint() const
    {
        return alps_use_new_codepoint_;
    }
...
private:
    ...
    QuicErrorCode CacheNewServerConfig(const CryptoHandshakeMessage& message, QuicWallTime now, QuicTransportVersion version, absl::string_view chlo_hash,
        const std::vector<std::string>& cached_certs, CachedState* cached, std::string* error_details);
    ...
    bool PopulateFromCanonicalConfig(const QuicServerId& server_id, CachedState* cached);
