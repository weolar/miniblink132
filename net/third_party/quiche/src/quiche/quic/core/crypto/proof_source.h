...
public:
    ...
    public:
        ...
        virtual void Run(bool ok, const quiche::QuicheReferenceCountedPointer<Chain>& chain, const QuicCryptoProof& proof, std::unique_ptr<Details> details)
            = 0;
    ...
    public:
        ...
        virtual void Run(bool ok, std::string signature, std::unique_ptr<Details> details) = 0;
    ...
    virtual void GetProof(const QuicSocketAddress& server_address, const QuicSocketAddress& client_address, const std::string& hostname,
        const std::string& server_config, QuicTransportVersion transport_version, absl::string_view chlo_hash, std::unique_ptr<Callback> callback)
        = 0;
    ...
    //
    // Callers should expect that |callback| might be invoked synchronously.
    virtual void ComputeTlsSignature(const QuicSocketAddress& server_address, const QuicSocketAddress& client_address, const std::string& hostname,
        uint16_t signature_algorithm, absl::string_view in, std::unique_ptr<SignatureCallback> callback)
        = 0;
    ...
    virtual QuicSignatureAlgorithmVector SupportedTlsSignatureAlgorithms() const = 0;
    ...
    public:
        ...
        virtual std::vector<uint8_t> Encrypt(absl::string_view in, absl::string_view encryption_key) = 0;
        ...
        // vector.
        virtual void Decrypt(absl::string_view in, std::shared_ptr<DecryptCallback> callback) = 0;
    };
...
public:
    ...
    using ConfigureSSLFunc = quiche::SingleUseCallback<absl::Status(SSL& ssl, const SSL_PRIVATE_KEY_METHOD& key)>;

    // Functor to call to select ALPN and configure ALPS.
    using ALPNSelectFunc = quiche::SingleUseCallback<int(SSL& ssl, const uint8_t** out, uint8_t* out_len, const uint8_t* in, unsigned in_len)>;
    ...
    virtual void OnSelectCertificateDone(bool ok, bool is_sync, SSLConfig ssl_config, absl::string_view ticket_encryption_key, bool cert_matched_sni) = 0;

    // Called when a ProofSourceHandle::ComputeSignature operation completes.
    virtual void OnComputeSignatureDone(bool ok, bool is_sync, std::string signature, std::unique_ptr<ProofSource::Details> details) = 0;
...
public:
    ...
    //   invoked.
    virtual QuicAsyncStatus SelectCertificate(const QuicSocketAddress& server_address, const QuicSocketAddress& client_address,
    ...
    virtual QuicAsyncStatus ComputeSignature(const QuicSocketAddress& server_address, const QuicSocketAddress& client_address, const std::string& hostname,
...
// it matches with |key|.
QUICHE_EXPORT bool ValidateCertAndKey(const quiche::QuicheReferenceCountedPointer<ProofSource::Chain>& chain, const CertificatePrivateKey& key);
