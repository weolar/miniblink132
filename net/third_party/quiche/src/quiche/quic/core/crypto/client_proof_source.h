...
public:
    ...
    struct QUICHE_EXPORT CertAndKey {
        CertAndKey(quiche::QuicheReferenceCountedPointer<Chain> chain, CertificatePrivateKey private_key)
    ...
    // default cert.
    virtual std::shared_ptr<const CertAndKey> GetCertAndKey(absl::string_view server_hostname) const = 0;
};
...
public:
    ...
    bool AddCertAndKey(std::vector<std::string> server_hostnames, quiche::QuicheReferenceCountedPointer<Chain> chain, CertificatePrivateKey private_key);

    // ClientProofSource implementation
    std::shared_ptr<const CertAndKey> GetCertAndKey(absl::string_view hostname) const override;

private:
    std::shared_ptr<const CertAndKey> LookupExact(absl::string_view map_key) const;
