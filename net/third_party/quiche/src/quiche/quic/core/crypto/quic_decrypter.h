...
public:
    ...
    static std::unique_ptr<QuicDecrypter> Create(const ParsedQuicVersion& version, QuicTag algorithm);
    ...
    static std::unique_ptr<QuicDecrypter> CreateFromCipherSuite(uint32_t cipher_suite);
    ...
    virtual std::string GenerateHeaderProtectionMask(QuicDataReader* sample_reader) = 0;
    ...
    static void DiversifyPreliminaryKey(absl::string_view preliminary_key, absl::string_view nonce_prefix, const DiversificationNonce& nonce, size_t key_size,
        size_t nonce_prefix_size, std::string* out_key, std::string* out_nonce_prefix);
};
