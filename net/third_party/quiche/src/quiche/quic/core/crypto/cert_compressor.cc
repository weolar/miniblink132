...
std::vector<CertEntry> MatchCerts(const std::vector<std::string>& certs, absl::string_view client_cached_cert_hashes)
{
    ...
    for (auto i = certs.begin(); i != certs.end(); ++i) {
        ...
        if (cached_valid) {
            ...
            for (size_t j = 0; j < client_cached_cert_hashes.size(); j += sizeof(uint64_t)) {
                uint64_t cached_hash;
                memcpy(&cached_hash, client_cached_cert_hashes.data() + j, sizeof(uint64_t));
                if (hash != cached_hash) {
                    continue;
                }
...
std::string ZlibDictForEntries(const std::vector<CertEntry>& entries, const std::vector<std::string>& certs)
{
    ...
    zlib_dict += std::string(reinterpret_cast<const char*>(kCommonCertSubstrings), sizeof(kCommonCertSubstrings));
...
// the trailing data.
bool ParseEntries(
    absl::string_view* in_out, const std::vector<std::string>& cached_certs, std::vector<CertEntry>* out_entries, std::vector<std::string>* out_certs)
{
...
// static
std::string CertCompressor::CompressChain(const std::vector<std::string>& certs, absl::string_view client_cached_cert_hashes)
{
    const std::vector<CertEntry> entries = MatchCerts(certs, client_cached_cert_hashes);
    ...
    if (uncompressed_size > 0) {
        ...
        rv = deflateSetDictionary(&z, reinterpret_cast<const uint8_t*>(&zlib_dict[0]), zlib_dict.size());
        QUICHE_DCHECK_EQ(Z_OK, rv);
        if (rv != Z_OK) {
            return "";
        }
...
// static
bool CertCompressor::DecompressChain(absl::string_view in, const std::vector<std::string>& cached_certs, std::vector<std::string>* out_certs)
{
    std::vector<CertEntry> entries;
    ...
    if (!in.empty()) {
        ...
        uncompressed = absl::string_view(reinterpret_cast<char*>(uncompressed_data.get()), uncompressed_size);
    }
