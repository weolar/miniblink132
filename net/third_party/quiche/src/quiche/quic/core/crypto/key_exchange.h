...
public:
    ...
    // into |*shared_key|.
    virtual void CalculateSharedKeyAsync(absl::string_view peer_public_value, std::string* shared_key, std::unique_ptr<Callback> callback) const = 0;
...
public:
    ...
    void CalculateSharedKeyAsync(absl::string_view peer_public_value, std::string* shared_key, std::unique_ptr<Callback> callback) const final
    {
    ...
    virtual bool CalculateSharedKeySync(absl::string_view peer_public_value, std::string* shared_key) const = 0;
...
// invalid.
std::unique_ptr<SynchronousKeyExchange> CreateLocalSynchronousKeyExchange(QuicTag type, absl::string_view private_key);
...
std::unique_ptr<SynchronousKeyExchange> CreateLocalSynchronousKeyExchange(QuicTag type, QuicRandom* rand);
