...
QuicData* DecryptWithNonce(Aes128Gcm12Decrypter* decrypter, absl::string_view nonce, absl::string_view associated_data, absl::string_view ciphertext)
{
    uint64_t packet_number;
    absl::string_view nonce_prefix(nonce.data(), nonce.size() - sizeof(packet_number));
    decrypter->SetNoncePrefix(nonce_prefix);
    memcpy(&packet_number, nonce.data() + nonce_prefix.size(), sizeof(packet_number));
    std::unique_ptr<char[]> output(new char[ciphertext.length()]);
    size_t output_length = 0;
    const bool success = decrypter->DecryptPacket(packet_number, associated_data, ciphertext, output.get(), &output_length, ciphertext.length());
    if (!success) {
        return nullptr;
    }
...
TEST_F(Aes128Gcm12DecrypterTest, Decrypt)
{
    for (size_t i = 0; i < ABSL_ARRAYSIZE(test_group_array); i++) {
        ...
        for (size_t j = 0; test_vectors[j].key != nullptr; j++) {
            ...
            // the first 12.
            ASSERT_LE(static_cast<size_t>(Aes128Gcm12Decrypter::kAuthTagSize), tag.length());
