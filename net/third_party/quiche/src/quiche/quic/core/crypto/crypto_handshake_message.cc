...
CryptoHandshakeMessage::CryptoHandshakeMessage(const CryptoHandshakeMessage& other)
    : tag_(other.tag_)
...
CryptoHandshakeMessage::CryptoHandshakeMessage(CryptoHandshakeMessage&& other) = default;

CryptoHandshakeMessage::~CryptoHandshakeMessage()
{
}

CryptoHandshakeMessage& CryptoHandshakeMessage::operator=(const CryptoHandshakeMessage& other)
{
    tag_ = other.tag_;
    tag_value_map_ = other.tag_value_map_;
...
CryptoHandshakeMessage& CryptoHandshakeMessage::operator=(CryptoHandshakeMessage&& other) = default;

bool CryptoHandshakeMessage::operator==(const CryptoHandshakeMessage& rhs) const
{
    return tag_ == rhs.tag_ && tag_value_map_ == rhs.tag_value_map_ && minimum_size_ == rhs.minimum_size_;
}
...
void CryptoHandshakeMessage::SetVersionVector(QuicTag tag, ParsedQuicVersionVector versions)
{
    QuicVersionLabelVector version_labels;
    for (const ParsedQuicVersion& version : versions) {
        version_labels.push_back(quiche::QuicheEndian::HostToNet32(CreateQuicVersionLabel(version)));
    }
    SetVector(tag, version_labels);
}

void CryptoHandshakeMessage::SetVersion(QuicTag tag, ParsedQuicVersion version)
{
    SetValue(tag, quiche::QuicheEndian::HostToNet32(CreateQuicVersionLabel(version)));
}

void CryptoHandshakeMessage::SetStringPiece(QuicTag tag, absl::string_view value)
{
...
QuicErrorCode CryptoHandshakeMessage::GetTaglist(QuicTag tag, QuicTagVector* out_tags) const
{
...
QuicErrorCode CryptoHandshakeMessage::GetVersionLabelList(QuicTag tag, QuicVersionLabelVector* out) const
{
...
QuicErrorCode CryptoHandshakeMessage::GetVersionLabel(QuicTag tag, QuicVersionLabel* out) const
{
...
bool CryptoHandshakeMessage::GetStringPiece(QuicTag tag, absl::string_view* out) const
{
    auto it = tag_value_map_.find(tag);
    if (it == tag_value_map_.end()) {
        return false;
...
QuicErrorCode CryptoHandshakeMessage::GetNthValue24(QuicTag tag, unsigned index, absl::string_view* out) const
{
...
QuicErrorCode CryptoHandshakeMessage::GetUint32(QuicTag tag, uint32_t* out) const
{
    return GetPOD(tag, out, sizeof(uint32_t));
}

QuicErrorCode CryptoHandshakeMessage::GetUint64(QuicTag tag, uint64_t* out) const
{
    return GetPOD(tag, out, sizeof(uint64_t));
}

QuicErrorCode CryptoHandshakeMessage::GetStatelessResetToken(QuicTag tag, StatelessResetToken* out) const
{
...
QuicErrorCode CryptoHandshakeMessage::GetPOD(QuicTag tag, void* out, size_t len) const
{
    auto it = tag_value_map_.find(tag);
    QuicErrorCode ret = QUIC_NO_ERROR;
...
{
    std::string ret = std::string(2 * indent, ' ') + QuicTagToString(tag_) + "<\n";
    ++indent;
    for (auto it = tag_value_map_.begin(); it != tag_value_map_.end(); ++it) {
        ...
        case kRREJ:
            ...
            if (it->second.size() % sizeof(uint32_t) == 0) {
                for (size_t j = 0; j < it->second.size(); j += sizeof(uint32_t)) {
                    ...
                    ret += CryptoUtils::HandshakeFailureReasonToString(static_cast<HandshakeFailureReason>(value));
                }
                done = true;
            }
            break;
        ...
        case kSCFG:
            // nested messages.
            if (!it->second.empty()) {
                std::unique_ptr<CryptoHandshakeMessage> msg(CryptoFramer::ParseMessage(it->second));
                if (msg) {
                    ret += "\n";
