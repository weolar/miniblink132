...
public:
    ...
    struct QUICHE_EXPORT Result : public quiche::QuicheReferenceCounted {
        Result(const CryptoHandshakeMessage& in_client_hello, QuicIpAddress in_client_ip, QuicWallTime in_now);
    ...
    ValidateClientHelloResultCallback(const ValidateClientHelloResultCallback&) = delete;
    ValidateClientHelloResultCallback& operator=(const ValidateClientHelloResultCallback&) = delete;
    virtual ~ValidateClientHelloResultCallback();
    virtual void Run(quiche::QuicheReferenceCountedPointer<Result> result, std::unique_ptr<ProofSource::Details> details) = 0;
};
...
public:
    ...
    ProcessClientHelloResultCallback(const ProcessClientHelloResultCallback&) = delete;
    ProcessClientHelloResultCallback& operator=(const ProcessClientHelloResultCallback&) = delete;
    virtual ~ProcessClientHelloResultCallback();
    virtual void Run(QuicErrorCode error, const std::string& error_details, std::unique_ptr<CryptoHandshakeMessage> message,
        std::unique_ptr<DiversificationNonce> diversification_nonce, std::unique_ptr<ProofSource::Details> details)
        = 0;
};
...
public:
    ...
    BuildServerConfigUpdateMessageResultCallback(const BuildServerConfigUpdateMessageResultCallback&) = delete;
    BuildServerConfigUpdateMessageResultCallback& operator=(const BuildServerConfigUpdateMessageResultCallback&) = delete;
...
public:
    ...
    virtual void OnRejectionBuilt(const std::vector<uint32_t>& reasons, CryptoHandshakeMessage* out) const = 0;
};
...
public:
    ...
    virtual std::unique_ptr<AsynchronousKeyExchange> Create(std::string server_config_id, bool is_fallback, QuicTag type, absl::string_view private_key) = 0;
};
...
public:
    ...
    QuicCryptoServerConfig(absl::string_view source_address_token_secret, QuicRandom* server_nonce_entropy, std::unique_ptr<ProofSource> proof_source,
    ...
    static QuicServerConfigProtobuf GenerateConfig(QuicRandom* rand, const QuicClock* clock, const ConfigOptions& options);
    ...
    std::unique_ptr<CryptoHandshakeMessage> AddConfig(const QuicServerConfigProtobuf& protobuf, QuicWallTime now);
    ...
    // the arguments.
    std::unique_ptr<CryptoHandshakeMessage> AddDefaultConfig(QuicRandom* rand, const QuicClock* clock, const ConfigOptions& options);
    ...
    bool SetConfigs(const std::vector<QuicServerConfigProtobuf>& protobufs, const QuicServerConfigProtobuf* fallback_protobuf, QuicWallTime now);
    ...
    void ValidateClientHello(const CryptoHandshakeMessage& client_hello, const QuicSocketAddress& client_address, const QuicSocketAddress& server_address,
        QuicTransportVersion version, const QuicClock* clock, quiche::QuicheReferenceCountedPointer<QuicSignedServerConfig> signed_config,
    ...
    void ProcessClientHello(quiche::QuicheReferenceCountedPointer<ValidateClientHelloResultCallback::Result> validate_chlo_result, bool reject_only,
        QuicConnectionId connection_id, const QuicSocketAddress& server_address, const QuicSocketAddress& client_address, ParsedQuicVersion version,
        const ParsedQuicVersionVector& supported_versions, const QuicClock* clock, QuicRandom* rand, QuicCompressedCertsCache* compressed_certs_cache,
    ...
    void BuildServerConfigUpdateMessage(QuicTransportVersion version, absl::string_view chlo_hash, const SourceAddressTokens& previous_source_address_tokens,
        const QuicSocketAddress& server_address, const QuicSocketAddress& client_address, const QuicClock* clock, QuicRandom* rand,
        QuicCompressedCertsCache* compressed_certs_cache, const QuicCryptoNegotiatedParameters& params, const CachedNetworkParameters* cached_network_params,
    ...
    bool validate_chlo_size() const
    {
        return validate_chlo_size_;
    }
    ...
    void AcquirePrimaryConfigChangedCb(std::unique_ptr<PrimaryConfigChangedCallback> cb);
    ...
    // |cached_network_params| is optional, and can be nullptr.
    std::string NewSourceAddressToken(const CryptoSecretBoxer& crypto_secret_boxer, const SourceAddressTokens& previous_tokens, const QuicIpAddress& ip,
        QuicRandom* rand, QuicWallTime now, const CachedNetworkParameters* cached_network_params) const;
    ...
    // failure.
    HandshakeFailureReason ParseSourceAddressToken(const CryptoSecretBoxer& crypto_secret_boxer, absl::string_view token, SourceAddressTokens& tokens) const;
    ...
    HandshakeFailureReason ValidateSourceAddressTokens(
        const SourceAddressTokens& tokens, const QuicIpAddress& ip, QuicWallTime now, CachedNetworkParameters* cached_network_params) const;
    ...
    void set_pad_rej(bool new_value)
    {
        pad_rej_ = new_value;
    }
    ...
    void set_pad_shlo(bool new_value)
    {
        pad_shlo_ = new_value;
    }
    ...
    const QuicSSLConfig& ssl_config() const
    {
        return ssl_config_;
    }
...
private:
    ...
    class QUICHE_EXPORT Config : public QuicCryptoConfig, public quiche::QuicheReferenceCounted {
    public:
        Config();
    ...
    using ConfigMap = std::map<ServerConfigID, quiche::QuicheReferenceCountedPointer<Config>>;

    // Get a ref to the config with a given server config id.
    quiche::QuicheReferenceCountedPointer<Config> GetConfigWithScid(absl::string_view requested_scid) const QUICHE_SHARED_LOCKS_REQUIRED(configs_lock_);
    ...
    // b->primary_time.
    static bool ConfigPrimaryTimeLessThan(const quiche::QuicheReferenceCountedPointer<Config>& a, const quiche::QuicheReferenceCountedPointer<Config>& b);

    // SelectNewPrimaryConfig reevaluates the primary config based on the
    // "primary_time" deadlines contained in each.
    void SelectNewPrimaryConfig(QuicWallTime now) const QUICHE_EXCLUSIVE_LOCKS_REQUIRED(configs_lock_);
    ...
    void EvaluateClientHello(const QuicSocketAddress& server_address, const QuicSocketAddress& client_address, QuicTransportVersion version,
        const Configs& configs, quiche::QuicheReferenceCountedPointer<ValidateClientHelloResultCallback::Result> client_hello_state,
        std::unique_ptr<ValidateClientHelloResultCallback> done_cb) const;
    ...
    public:
        ProcessClientHelloContext(quiche::QuicheReferenceCountedPointer<ValidateClientHelloResultCallback::Result> validate_chlo_result, bool reject_only,
            QuicConnectionId connection_id, const QuicSocketAddress& server_address, const QuicSocketAddress& client_address, ParsedQuicVersion version,
            const ParsedQuicVersionVector& supported_versions, const QuicClock* clock, QuicRandom* rand, QuicCompressedCertsCache* compressed_certs_cache,
            quiche::QuicheReferenceCountedPointer<QuicCryptoNegotiatedParameters> params,
            quiche::QuicheReferenceCountedPointer<QuicSignedServerConfig> signed_config, QuicByteCount total_framing_overhead, QuicByteCount chlo_packet_size,
        ...
        void Succeed(std::unique_ptr<CryptoHandshakeMessage> message, std::unique_ptr<DiversificationNonce> diversification_nonce,
            std::unique_ptr<ProofSource::Details> proof_source_details);

        // Member accessors
        quiche::QuicheReferenceCountedPointer<ValidateClientHelloResultCallback::Result> validate_chlo_result() const
        {
            return validate_chlo_result_;
        }
        bool reject_only() const
        {
            return reject_only_;
        }
        ...
        QuicSocketAddress server_address() const
        {
            return server_address_;
        }
        QuicSocketAddress client_address() const
        {
        ...
        const QuicClock* clock() const
        {
            return clock_;
        }
        QuicRandom* rand() const
        {
            return rand_;
        } // NOLINT
        ...
        quiche::QuicheReferenceCountedPointer<QuicCryptoNegotiatedParameters> params() const
        {
            return params_;
        }
        quiche::QuicheReferenceCountedPointer<QuicSignedServerConfig> signed_config() const
        {
        ...
        QuicByteCount chlo_packet_size() const
        {
            return chlo_packet_size_;
        }
        ...
        const ClientHelloInfo& info() const
        {
            return validate_chlo_result()->info;
        }
    ...
    private:
        const quiche::QuicheReferenceCountedPointer<ValidateClientHelloResultCallback::Result> validate_chlo_result_;
        ...
        const quiche::QuicheReferenceCountedPointer<QuicCryptoNegotiatedParameters> params_;
        const quiche::QuicheReferenceCountedPointer<QuicSignedServerConfig> signed_config_;
    ...
    void ProcessClientHelloAfterGetProof(bool found_error, std::unique_ptr<ProofSource::Details> proof_source_details,
    ...
    void ProcessClientHelloAfterCalculateSharedKeys(bool found_error, std::unique_ptr<ProofSource::Details> proof_source_details, QuicTag key_exchange_type,
        std::unique_ptr<CryptoHandshakeMessage> out, absl::string_view public_value, std::unique_ptr<ProcessClientHelloContext> context,
    ...
    void SendRejectWithFallbackConfig(std::unique_ptr<ProcessClientHelloContext> context, quiche::QuicheReferenceCountedPointer<Config> fallback_config) const;
    ...
    void SendRejectWithFallbackConfigAfterGetProof(bool found_error, std::unique_ptr<ProofSource::Details> proof_source_details,
    ...
    void BuildRejectionAndRecordStats(
    ...
    static std::string CompressChain(QuicCompressedCertsCache* compressed_certs_cache, const quiche::QuicheReferenceCountedPointer<ProofSource::Chain>& chain,
    ...
    // nullptr.
    quiche::QuicheReferenceCountedPointer<Config> ParseConfigProtobuf(const QuicServerConfigProtobuf& protobuf, bool is_fallback) const;
    ...
    // for failure.
    HandshakeFailureReason ValidateSingleSourceAddressToken(const SourceAddressToken& token, const QuicIpAddress& ip, QuicWallTime now) const;
    ...
    // reason for failure.
    HandshakeFailureReason ValidateSourceAddressTokenTimestamp(const SourceAddressToken& token, QuicWallTime now) const;
    ...
    bool ValidateExpectedLeafCertificate(const CryptoHandshakeMessage& client_hello, const std::vector<std::string>& certs) const;
    ...
    class BuildServerConfigUpdateMessageProofSourceCallback : public ProofSource::Callback {
    public:
        BuildServerConfigUpdateMessageProofSourceCallback(const BuildServerConfigUpdateMessageProofSourceCallback&) = delete;
        ~BuildServerConfigUpdateMessageProofSourceCallback() override;
        void operator=(const BuildServerConfigUpdateMessageProofSourceCallback&) = delete;
        BuildServerConfigUpdateMessageProofSourceCallback(const QuicCryptoServerConfig* config, QuicCompressedCertsCache* compressed_certs_cache,
            const QuicCryptoNegotiatedParameters& params, CryptoHandshakeMessage message, std::unique_ptr<BuildServerConfigUpdateMessageResultCallback> cb);
    ...
    void FinishBuildServerConfigUpdateMessage(QuicCompressedCertsCache* compressed_certs_cache, const std::string& client_cached_cert_hashes,
        bool sct_supported_by_client, const std::string& sni, bool ok, const quiche::QuicheReferenceCountedPointer<ProofSource::Chain>& chain,
        const std::string& signature, const std::string& leaf_cert_sct, std::unique_ptr<ProofSource::Details> details, CryptoHandshakeMessage message,
        std::unique_ptr<BuildServerConfigUpdateMessageResultCallback> cb) const;

    // Returns true if the next config promotion should happen now.
    bool IsNextConfigReady(QuicWallTime now) const QUICHE_SHARED_LOCKS_REQUIRED(configs_lock_);
    ...
    mutable quiche::QuicheReferenceCountedPointer<Config> primary_config_ QUICHE_GUARDED_BY(configs_lock_);
    ...
    quiche::QuicheReferenceCountedPointer<Config> fallback_config_ QUICHE_GUARDED_BY(configs_lock_);

    // next_config_promotion_time_ contains the nearest, future time when an
    // active config will be promoted to primary.
    mutable QuicWallTime next_config_promotion_time_ QUICHE_GUARDED_BY(configs_lock_);

    // Callback to invoke when the primary config changes.
    std::unique_ptr<PrimaryConfigChangedCallback> primary_config_changed_cb_ QUICHE_GUARDED_BY(configs_lock_);
...
struct QUICHE_EXPORT QuicSignedServerConfig : public quiche::QuicheReferenceCounted {
