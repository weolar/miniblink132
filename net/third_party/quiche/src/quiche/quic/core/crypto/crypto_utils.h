...
public:
    ...
    public:
        ...
        static Diversification Never()
        {
            return Diversification(NEVER, nullptr);
        }
    ...
    static void InitializeCrypterSecrets(const EVP_MD* prf, const std::vector<uint8_t>& pp_secret, const ParsedQuicVersion& version, QuicCrypter* crypter);
    ...
    static void SetKeyAndIV(const EVP_MD* prf, absl::Span<const uint8_t> pp_secret, const ParsedQuicVersion& version, QuicCrypter* crypter);
    ...
    static std::vector<uint8_t> GenerateNextKeyPhaseSecret(const EVP_MD* prf, const ParsedQuicVersion& version, const std::vector<uint8_t>& current_secret);
    ...
    static void CreateInitialObfuscators(Perspective perspective, ParsedQuicVersion version, QuicConnectionId connection_id, CrypterPair* crypters);
    ...
    //   <20 bytes> random
    static void GenerateNonce(QuicWallTime now, QuicRandom* random_generator, absl::string_view orbit, std::string* nonce);
    ...
    // complete keying.
    static bool DeriveKeys(const ParsedQuicVersion& version, absl::string_view premaster_secret, QuicTag aead, absl::string_view client_nonce,
        absl::string_view server_nonce, absl::string_view pre_shared_key, const std::string& hkdf_input, Perspective perspective,
        Diversification diversification, CrypterPair* crypters, std::string* subkey_secret);
    ...
    // false and fills in |error_details|.
    static bool ValidateServerVersions(const QuicVersionLabelVector& version_information_other_versions, const ParsedQuicVersion& session_version,
    ...
    static const char* HandshakeFailureReasonToString(HandshakeFailureReason reason);
    ...
    static std::string HashHandshakeMessage(const CryptoHandshakeMessage& message, Perspective perspective);

    // Wraps SSL_serialize_capabilities. Return nullptr if failed.
    static bool GetSSLCapabilities(const SSL* ssl, bssl::UniquePtr<uint8_t>* capabilities, size_t* capabilities_len);

    // Computes the contents of a binary message that is signed inside QUIC Crypto
    // protocol using the certificate key.
    static std::optional<std::string> GenerateProofPayloadToBeSigned(absl::string_view chlo_hash, absl::string_view server_config);
