...
public:
    ...
    template <class T> void SetVector(QuicTag tag, const std::vector<T>& v)
    {
        if (v.empty()) {
            ...
            tag_value_map_[tag] = std::string(reinterpret_cast<const char*>(&v[0]), v.size() * sizeof(T));
        }
    }
    ...
    const QuicTagValueMap& tag_value_map() const
    {
        return tag_value_map_;
    }
    ...
    QuicErrorCode GetVersionLabelList(QuicTag tag, QuicVersionLabelVector* out) const;
    ...
    // index.
    QuicErrorCode GetNthValue24(QuicTag tag, unsigned index, absl::string_view* out) const;
    QuicErrorCode GetUint32(QuicTag tag, uint32_t* out) const;
    QuicErrorCode GetUint64(QuicTag tag, uint64_t* out) const;

    QuicErrorCode GetStatelessResetToken(QuicTag tag, StatelessResetToken* out) const;
