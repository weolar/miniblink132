...
// static
std::unique_ptr<Curve25519KeyExchange> Curve25519KeyExchange::New(QuicRandom* rand)
{
    std::unique_ptr<Curve25519KeyExchange> result = New(Curve25519KeyExchange::NewPrivateKey(rand));
    QUIC_BUG_IF(quic_bug_12891_1, result == nullptr);
    return result;
}

// static
std::unique_ptr<Curve25519KeyExchange> Curve25519KeyExchange::New(absl::string_view private_key)
{
    ...
    // header.
    static_assert(sizeof(Curve25519KeyExchange::private_key_) == X25519_PRIVATE_KEY_LEN, "header out of sync");
    static_assert(sizeof(Curve25519KeyExchange::public_key_) == X25519_PUBLIC_VALUE_LEN, "header out of sync");
...
bool Curve25519KeyExchange::CalculateSharedKeySync(absl::string_view peer_public_value, std::string* shared_key) const
{
    ...
    if (!X25519(result, private_key_, reinterpret_cast<const uint8_t*>(peer_public_value.data()))) {
        return false;
    }
