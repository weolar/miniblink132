...
TEST_F(NullDecrypterTest, DecryptClient)
{
    ...
    char buffer[256];
    size_t length = 0;
    ASSERT_TRUE(decrypter.DecryptPacket(0, "hello world!", absl::string_view(data, len), buffer, &length, 256));
    EXPECT_LT(0u, length);
...
TEST_F(NullDecrypterTest, BadHash)
{
    ...
    char buffer[256];
    size_t length = 0;
    ASSERT_FALSE(decrypter.DecryptPacket(0, "hello world!", absl::string_view(data, len), buffer, &length, 256));
}
