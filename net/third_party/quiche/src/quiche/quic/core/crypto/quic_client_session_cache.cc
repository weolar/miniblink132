...
bool IsValid(SSL_SESSION* session, uint64_t now)
{
    ...
    return !(now + 1 < SSL_SESSION_get_time(session) || now >= SSL_SESSION_get_time(session) + SSL_SESSION_get_timeout(session));
}

bool DoApplicationStatesMatch(const ApplicationState* state, ApplicationState* other)
{
    ...
    const QuicServerId& server_id, bssl::UniquePtr<SSL_SESSION> session, const TransportParameters& params, const ApplicationState* application_state)
{
    ...
    if (params == *iter->second->params && DoApplicationStatesMatch(application_state, iter->second->application_state.get())) {
...
std::unique_ptr<QuicResumptionState> QuicClientSessionCache::Lookup(const QuicServerId& server_id, QuicWallTime now, const SSL_CTX* /*ctx*/)
{
    auto iter = cache_.Lookup(server_id);
    if (iter == cache_.end())
        return nullptr;
    ...
    if (iter->second->params != nullptr) {
        state->transport_params = std::make_unique<TransportParameters>(*iter->second->params);
    }
    if (iter->second->application_state != nullptr) {
        state->application_state = std::make_unique<ApplicationState>(*iter->second->application_state);
    }
...
void QuicClientSessionCache::OnNewTokenReceived(const QuicServerId& server_id, absl::string_view token)
{
    if (token.empty()) {
        return;
    }
    ...
    const QuicServerId& server_id, bssl::UniquePtr<SSL_SESSION> session, const TransportParameters& params, const ApplicationState* application_state)
{
    ...
    if (application_state) {
        entry->application_state = std::make_unique<ApplicationState>(*application_state);
    }
...
void QuicClientSessionCache::Entry::PushSession(bssl::UniquePtr<SSL_SESSION> session)
{
