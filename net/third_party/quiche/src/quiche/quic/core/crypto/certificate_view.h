...
QUICHE_EXPORT PublicKeyType PublicKeyTypeFromSignatureAlgorithm(uint16_t signature_algorithm);
...
public:
    ...
    // error.
    static std::unique_ptr<CertificateView> ParseSingleCertificate(absl::string_view certificate);
    ...
    QuicWallTime validity_start() const
    {
        return validity_start_;
    }
    QuicWallTime validity_end() const
    {
        return validity_end_;
    }
    const EVP_PKEY* public_key() const
    {
    ...
    bool VerifySignature(absl::string_view data, absl::string_view signature, uint16_t signature_algorithm) const;
...
public:
    ...
    static std::unique_ptr<CertificatePrivateKey> LoadFromDer(absl::string_view private_key);
    ...
    static std::unique_ptr<CertificatePrivateKey> LoadPemFromStream(std::istream* input);
...
// testing.
QUICHE_EXPORT std::optional<quic::QuicWallTime> ParseDerTime(unsigned tag, absl::string_view payload);
