...
public:
    ...
    struct QUICHE_EXPORT BufferedPacket {
        BufferedPacket(std::unique_ptr<QuicReceivedPacket> packet, QuicSocketAddress self_address, QuicSocketAddress peer_address, bool is_ietf_initial_packet);
    ...
    struct QUICHE_EXPORT BufferedPacketList {
        ...
        {
            return replaced_connection_id.has_value() && !replaced_connection_id->IsEmpty();
        }
        ...
        void SetAttemptedToReplaceConnectionId(ConnectionIdGeneratorInterface* generator)
        {
    ...
    struct QUICHE_EXPORT BufferedPacketListNode : public quiche::QuicheIntrusiveLink<BufferedPacketListNode>,
                                                  public quiche::QuicheIntrusiveLink<BufferedPacketListNode, BufferedSessionsWithChloList>,
    ...
    public:
        ...
        virtual HandleCidCollisionResult HandleConnectionIdCollision(const QuicConnectionId& original_connection_id,
            const QuicConnectionId& replaced_connection_id, const QuicSocketAddress& self_address, const QuicSocketAddress& peer_address,
            ParsedQuicVersion version, const ParsedClientHello* parsed_chlo)
            = 0;
    };

    QuicBufferedPacketStore(VisitorInterface* visitor, const QuicClock* clock, QuicAlarmFactory* alarm_factory, QuicDispatcherStats& stats);
    ...
    bool IngestPacketForTlsChloExtraction(const QuicConnectionId& connection_id, const ParsedQuicVersion& version, const QuicReceivedPacket& packet,
        std::vector<uint16_t>* out_supported_groups, std::vector<uint16_t>* out_cert_compression_algos, std::vector<std::string>* out_alpns,
        std::string* out_sni, bool* out_resumption_attempted, bool* out_early_data_attempted, std::optional<uint8_t>* tls_alert);
    ...
    BufferedPacketList DeliverPacketsForNextConnection(QuicConnectionId* connection_id);
    ...
    const BufferedPacketList* GetPacketList(const QuicConnectionId& connection_id) const;
...
private:
    ...
    void MaybeAckInitialPacket(const ReceivedPacketInfo& packet_info, BufferedPacketList& packet_list);
    ...
    absl::flat_hash_map<QuicConnectionId, std::shared_ptr<BufferedPacketListNode>, QuicConnectionIdHash> buffered_session_map_;
    ...
    quiche::QuicheIntrusiveList<BufferedPacketListNode, BufferedSessionsWithChloList> buffered_sessions_with_chlo_;
...
class QUICHE_NO_EXPORT PacketCollector : public QuicPacketCreator::DelegateInterface, public QuicStreamFrameDataProducer {
public:
    ...
    void OnSerializedPacket(SerializedPacket serialized_packet) override
    {
        ...
        packets_.emplace_back(new QuicEncryptedPacket(CopyBuffer(serialized_packet), serialized_packet.encrypted_length, true));
    }
