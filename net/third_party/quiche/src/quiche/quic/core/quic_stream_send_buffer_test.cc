...
public:
        ...
        : send_buffer_(&allocator_)
    {
        ...
        std::string data1 = absl::StrCat(std::string(1536, 'a'), std::string(256, 'b'), std::string(256, 'c'));
...
TEST_F(QuicStreamSendBufferTest, CopyDataToBuffer)
{
    ...
    std::string copy2 = std::string(512, 'a') + std::string(256, 'b') + std::string(256, 'c');
    ...
    std::string copy5 = std::string(536, 'a') + std::string(256, 'b') + std::string(232, 'c');
    ...
    EXPECT_QUIC_BUG(send_buffer_.WriteStreamData(0, 4000, &writer3), "Writer fails to write.");
...
TEST_F(QuicStreamSendBufferTest, WriteStreamDataContainsBothRetransmissionAndNewData)
{
    std::string copy1(1024, 'a');
    std::string copy2 = std::string(512, 'a') + std::string(256, 'b') + std::string(256, 'c');
...
TEST_F(QuicStreamSendBufferTest, PendingRetransmission)
{
    ...
    EXPECT_EQ(StreamPendingRetransmission(0, 1200), send_buffer_.NextPendingRetransmission());
    // Retransmit data [0, 500).
    send_buffer_.OnStreamDataRetransmitted(0, 500);
    EXPECT_TRUE(send_buffer_.IsStreamDataOutstanding(0, 500));
    EXPECT_EQ(StreamPendingRetransmission(500, 700), send_buffer_.NextPendingRetransmission());
    // Ack data [500, 1200).
    ...
    EXPECT_TRUE(send_buffer_.HasPendingRetransmission());
    EXPECT_EQ(StreamPendingRetransmission(1500, 500), send_buffer_.NextPendingRetransmission());
    ...
    EXPECT_EQ(StreamPendingRetransmission(200, 300), send_buffer_.NextPendingRetransmission());
...
TEST_F(QuicStreamSendBufferTest, EndOffset)
{
    ...
    ASSERT_TRUE(send_buffer_.WriteStreamData(1024 + 512, 3840 - 1024 - 512, &writer));
