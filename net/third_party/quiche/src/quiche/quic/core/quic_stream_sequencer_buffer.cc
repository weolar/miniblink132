...
{
    return (max_capacity_bytes + QuicStreamSequencerBuffer::kBlockSizeBytes - 1) / QuicStreamSequencerBuffer::kBlockSizeBytes;
}
...
void QuicStreamSequencerBuffer::MaybeAddMoreBlocks(QuicStreamOffset next_expected_byte)
{
    ...
    if (last_byte < max_buffer_capacity_bytes_) {
        num_of_blocks_needed = std::max(GetBlockIndex(last_byte) + 1, kInitialBlockCount);
    } else {
    ...
    new_block_count = std::min(std::max(new_block_count, num_of_blocks_needed), max_blocks_count_);
    auto new_blocks = std::make_unique<BufferBlock*[]>(new_block_count);
    if (blocks_ != nullptr) {
        memcpy(new_blocks.get(), blocks_.get(), current_blocks_count_ * sizeof(BufferBlock*));
    }
...
QuicErrorCode QuicStreamSequencerBuffer::OnStreamData(
    QuicStreamOffset starting_offset, absl::string_view data, size_t* const bytes_buffered, std::string* error_details)
{
    *bytes_buffered = 0;
    ...
    if (starting_offset + size > total_bytes_read_ + max_buffer_capacity_bytes_ || starting_offset + size < starting_offset) {
        *error_details = "Received data beyond available range.";
        return QUIC_INTERNAL_ERROR;
    }

    if (bytes_received_.Empty() || starting_offset >= bytes_received_.rbegin()->max()
        || bytes_received_.IsDisjoint(QuicInterval<QuicStreamOffset>(starting_offset, starting_offset + size))) {
        // Optimization for the typical case, when all data is newly received.
        bytes_received_.AddOptimizedForAppend(starting_offset, starting_offset + size);
    ...
    QuicIntervalSet<QuicStreamOffset> newly_received(starting_offset, starting_offset + size);
    ...
    for (const auto& interval : newly_received) {
        ...
        size_t bytes_copy = 0;
        if (!CopyStreamData(copy_offset, data.substr(copy_offset - starting_offset, copy_length), &bytes_copy, error_details)) {
            return QUIC_STREAM_SEQUENCER_INVALID_STATE;
        }
        *bytes_buffered += bytes_copy;
    }
...
bool QuicStreamSequencerBuffer::CopyStreamData(QuicStreamOffset offset, absl::string_view data, size_t* bytes_copy, std::string* error_details)
{
    *bytes_copy = 0;
    ...
    while (source_remaining > 0) {
        ...
        if (write_block_num >= current_blocks_count) {
            *error_details = absl::StrCat("QuicStreamSequencerBuffer error: OnStreamData() exceed array bounds."
                                          "write offset = ",
                offset, " write_block_num = ", write_block_num, " current_blocks_count_ = ", current_blocks_count);
            return false;
        }
        if (blocks_ == nullptr) {
            *error_details = "QuicStreamSequencerBuffer error: OnStreamData() blocks_ is null";
            return false;
        }
        ...
        const size_t bytes_to_copy = std::min<size_t>(bytes_avail, source_remaining);
        char* dest = blocks_[write_block_num]->buffer + write_block_offset;
        QUIC_DVLOG(1) << "Write at offset: " << offset << " length: " << bytes_to_copy;

        if (dest == nullptr || source == nullptr) {
            *error_details = absl::StrCat("QuicStreamSequencerBuffer error: OnStreamData()"
                                          " dest == nullptr: ",
                (dest == nullptr), " source == nullptr: ", (source == nullptr), " Writing at offset ", offset,
                " Received frames: ", ReceivedFramesDebugString(), " total_bytes_read_ = ", total_bytes_read_);
            return false;
        }
...
QuicErrorCode QuicStreamSequencerBuffer::Readv(const iovec* dest_iov, size_t dest_count, size_t* bytes_read, std::string* error_details)
{
    *bytes_read = 0;
    for (size_t i = 0; i < dest_count && ReadableBytes() > 0; ++i) {
        ...
        while (dest_remaining > 0 && ReadableBytes() > 0) {
            ...
            size_t bytes_available_in_block = std::min<size_t>(ReadableBytes(), block_capacity - start_offset_in_block);
            size_t bytes_to_copy = std::min<size_t>(bytes_available_in_block, dest_remaining);
            QUICHE_DCHECK_GT(bytes_to_copy, 0u);
            if (blocks_[block_idx] == nullptr || dest == nullptr) {
                *error_details = absl::StrCat("QuicStreamSequencerBuffer error:"
                                              " Readv() dest == nullptr: ",
                    (dest == nullptr), " blocks_[", block_idx, "] == nullptr: ", (blocks_[block_idx] == nullptr),
                    " Received frames: ", ReceivedFramesDebugString(), " total_bytes_read_ = ", total_bytes_read_);
                return QUIC_STREAM_SEQUENCER_INVALID_STATE;
            }
            memcpy(dest, blocks_[block_idx]->buffer + start_offset_in_block, bytes_to_copy);
            dest += bytes_to_copy;
            ...
            *bytes_read += bytes_to_copy;
            ...
            if (bytes_to_copy == bytes_available_in_block) {
                ...
                if (!retire_successfully) {
                    *error_details = absl::StrCat("QuicStreamSequencerBuffer error: fail to retire block ", block_idx,
                        " as the block is already released, total_bytes_read_ = ", total_bytes_read_, " Received frames: ", ReceivedFramesDebugString());
                    return QUIC_STREAM_SEQUENCER_INVALID_STATE;
                }
            }
        }
...
int QuicStreamSequencerBuffer::GetReadableRegions(struct iovec* iov, int iov_len) const
{
    ...
    QUIC_DVLOG(1) << "Got first block " << start_block_idx << " with len " << iov[0].iov_len;
    QUICHE_DCHECK_GT(readable_offset_end + 1, total_bytes_read_ + iov[0].iov_len) << "there should be more available data";
...
bool QuicStreamSequencerBuffer::PeekRegion(QuicStreamOffset offset, iovec* iov) const
{
    QUICHE_DCHECK(iov);
    ...
    if (block_idx == end_block_idx && block_offset < GetInBlockOffset(FirstMissingByte())) {
    ...
    QUIC_BUG_IF(quic_invalid_peek_region, iov->iov_len > kBlockSizeBytes) << "PeekRegion() at " << offset << " gets bad iov with length " << iov->iov_len;
    return true;
}
...
bool QuicStreamSequencerBuffer::MarkConsumed(size_t bytes_consumed)
{
    ...
    while (bytes_to_consume > 0) {
        ...
        size_t bytes_available = std::min<size_t>(ReadableBytes(), GetBlockCapacity(block_idx) - offset_in_block);
...
size_t QuicStreamSequencerBuffer::GetInBlockOffset(QuicStreamOffset offset) const
{
    return (offset % max_buffer_capacity_bytes_) % kBlockSizeBytes;
}
...
{
    QUICHE_DCHECK(ReadableBytes() == 0 || GetInBlockOffset(total_bytes_read_) == 0) << "RetireBlockIfEmpty() should only be called when advancing to next "
...
{
    return bytes_received_.Empty() || (bytes_received_.Size() == 1 && total_bytes_read_ > 0 && bytes_received_.begin()->max() == total_bytes_read_);
}
