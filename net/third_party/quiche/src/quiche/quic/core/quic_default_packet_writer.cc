...
WriteResult QuicDefaultPacketWriter::WritePacket(const char* buffer, size_t buf_len, const QuicIpAddress& self_address, const QuicSocketAddress& peer_address,
    PerPacketOptions* /*options*/, const QuicPacketWriterParams& params)
{
    ...
    WriteResult result = QuicUdpSocketApi().WritePacket(fd_, buffer, buf_len, packet_info);
...
QuicByteCount QuicDefaultPacketWriter::GetMaxPacketSize(const QuicSocketAddress& /*peer_address*/) const
{
...
QuicPacketBuffer QuicDefaultPacketWriter::GetNextWriteLocation(const QuicIpAddress& /*self_address*/, const QuicSocketAddress& /*peer_address*/)
{
