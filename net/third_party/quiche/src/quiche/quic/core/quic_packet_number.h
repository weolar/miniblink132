...
public:
    ...
    {
        QUICHE_DCHECK_NE(UninitializedPacketNumber(), packet_number) << "Use default constructor for uninitialized packet number";
    }
    ...
    QUICHE_EXPORT friend std::ostream& operator<<(std::ostream& os, const QuicPacketNumber& p);
...
private:
    ...
    friend inline QuicPacketNumber operator+(QuicPacketNumber lhs, uint64_t delta);
...
{
    QUICHE_DCHECK(lhs.IsInitialized() && rhs.IsInitialized()) << lhs << " vs. " << rhs;
...
inline QuicPacketNumber operator+(QuicPacketNumber lhs, uint64_t delta)
{
    ...
    QUICHE_DCHECK_GT(std::numeric_limits<uint64_t>::max() - lhs.ToUint64(), delta);
#endif
...
{
    QUICHE_DCHECK(lhs.IsInitialized() && rhs.IsInitialized() && lhs >= rhs) << lhs << " vs. " << rhs;
    return lhs.packet_number_ - rhs.packet_number_;
}
