...
class QUICHE_EXPORT QuicDefaultConnectionHelper : public QuicConnectionHelperInterface {
public:
    static QuicDefaultConnectionHelper* Get()
    {
        static QuicDefaultConnectionHelper* helper = new QuicDefaultConnectionHelper();
        return helper;
    }

    // Creates a helper that uses the default allocator.
    QuicDefaultConnectionHelper()
        : QuicDefaultConnectionHelper(nullptr)
    {
    }
    // If |allocator| is nullptr, the default one is used.
    QuicDefaultConnectionHelper(std::unique_ptr<quiche::QuicheBufferAllocator> allocator)
        : allocator_(std::move(allocator))
    {
    }

    QuicDefaultConnectionHelper(const QuicDefaultConnectionHelper&) = delete;
    QuicDefaultConnectionHelper& operator=(const QuicDefaultConnectionHelper&) = delete;

    const QuicClock* GetClock() const override
    {
        return QuicDefaultClock::Get();
    }
