...
template <typename T> class QUICHE_NO_EXPORT QuicInterval {
private:
    ...
    template <typename U> class QUICHE_NO_EXPORT DiffTypeOrVoid {
    private:
        template <typename V> static auto f(const V* v) -> decltype(*v - *v);
        template <typename V> static void f(...);
...
public:
    ...
    // QuicInterval.
    QuicInterval(const T& min, const T& max)
        : min_(min)
        , max_(max)
    {
    }

    template <typename U1, typename U2, typename = typename std::enable_if<std::is_convertible<U1, T>::value && std::is_convertible<U2, T>::value>::type>
    QuicInterval(U1&& min, U2&& max)
        : min_(std::forward<U1>(min))
        , max_(std::forward<U2>(max))
    {
    }
    ...
    // be non-null.
    bool Difference(const QuicInterval& i, std::vector<QuicInterval*>* difference) const;
    ...
    // is non-empty.
    bool Difference(const QuicInterval& i, QuicInterval* lo, QuicInterval* hi) const;
...
template <typename T> QuicInterval<T> MakeQuicInterval(T&& lhs, T&& rhs)
{
...
template <typename T> auto operator<<(std::ostream& out, const QuicInterval<T>& i) -> decltype(out << i.min())
{
...
template <typename T> bool QuicInterval<T>::Intersects(const QuicInterval& i, QuicInterval* out) const
{
    if (!Intersects(i))
        return false;
    if (out != nullptr) {
...
template <typename T> bool QuicInterval<T>::IntersectWith(const QuicInterval& i)
{
    if (Empty())
        return false;
    bool modified = false;
...
template <typename T> bool QuicInterval<T>::SpanningUnion(const QuicInterval& i)
{
    if (i.Empty())
        return false;
    if (Empty()) {
...
template <typename T> bool QuicInterval<T>::Difference(const QuicInterval& i, std::vector<QuicInterval*>* difference) const
{
    if (Empty()) {
        // <empty> - <i> = <empty>
        return false;
...
template <typename T> bool QuicInterval<T>::Difference(const QuicInterval& i, QuicInterval* lo, QuicInterval* hi) const
{
