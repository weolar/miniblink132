...
public:
    ...
    template <typename... Args> void Printf(const absl::FormatSpec<Args...>& format, const Args&... args)
    {
...
public:
    ...
    virtual void OnQuicBug(const char* bug_id, const char* file, int line, absl::string_view bug_message) = 0;
};
...
// any).
template <typename... Args> void QUIC_TRACEPRINTF(const absl::FormatSpec<Args...>& format, const Args&... args)
{
    QuicConnectionContext* current = QuicConnectionContext::Current();
    if (current && current->tracer) {
