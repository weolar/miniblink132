...
public:
    QuicCryptoClientStreamTest()
        ...
        , crypto_config_(crypto_test_utils::ProofVerifierForTesting(), std::make_unique<test::SimpleSessionCache>())
    ...
    void CreateSession()
    {
        session_ = std::make_unique<TestQuicSpdyClientSession>(connection_, DefaultQuicConfig(), supported_versions_, server_id_, &crypto_config_);
    ...
    void CompleteCryptoHandshake()
    {
        ...
        if (stream()->handshake_protocol() != PROTOCOL_TLS1_3) {
            EXPECT_CALL(*session_, OnProofValid(testing::_)).Times(testing::AtLeast(1));
            proof_verify_details_calls = 0;
        }
        EXPECT_CALL(*session_, OnProofVerifyDetailsAvailable(testing::_)).Times(testing::AtLeast(proof_verify_details_calls));
...
TEST_F(QuicCryptoClientStreamTest, MessageAfterHandshake)
{
    ...
    EXPECT_CALL(*connection_, CloseConnection(QUIC_CRYPTO_MESSAGE_AFTER_HANDSHAKE_COMPLETE, _, _));
    message_.set_tag(kCHLO);
    crypto_test_utils::SendHandshakeMessageToStream(stream(), message_, Perspective::IS_CLIENT);
}
...
TEST_F(QuicCryptoClientStreamTest, NegotiatedParameters)
{
    ...
    const QuicCryptoNegotiatedParameters& crypto_params(stream()->crypto_negotiated_params());
...
TEST_F(QuicCryptoClientStreamTest, ExpiredServerConfig)
{
    ...
    // server config.
    connection_->AdvanceTime(QuicTime::Delta::FromSeconds(60 * 60 * 24 * 365 * 5));
...
TEST_F(QuicCryptoClientStreamTest, InvalidCachedServerConfig)
{
    ...
    EXPECT_CALL(*session_, OnProofVerifyDetailsAvailable(testing::_)).Times(testing::AnyNumber());
    stream()->CryptoConnect();
    // Check that a client hello was sent.
    ASSERT_EQ(1u, connection_->encrypted_packets_.size());
}
...
TEST_F(QuicCryptoClientStreamTest, ServerConfigUpdate)
{
    ...
    crypto_test_utils::SendHandshakeMessageToStream(stream(), server_config_update, Perspective::IS_SERVER);
    ...
    quiche::test::CompareCharArraysWithHexError("scfg", cached_scfg.data(), cached_scfg.length(), reinterpret_cast<char*>(scfg), ABSL_ARRAYSIZE(scfg));
...
TEST_F(QuicCryptoClientStreamTest, ServerConfigUpdateWithCert)
{
    ...
    crypto_test_utils::SetupCryptoServerConfigForTest(connection_->clock(), QuicRandom::GetInstance(), &crypto_config);
    ...
    bool ok = false;
    crypto_config.BuildServerConfigUpdateMessage(session_->transport_version(), stream()->chlo_hash(), tokens,
        QuicSocketAddress(QuicIpAddress::Loopback6(), 1234), QuicSocketAddress(QuicIpAddress::Loopback6(), 4321), connection_->clock(),
        QuicRandom::GetInstance(), &cache, stream()->crypto_negotiated_params(), &network_params,
        std::unique_ptr<BuildServerConfigUpdateMessageResultCallback>(new Callback(&ok, &server_config_update)));
    EXPECT_TRUE(ok);
...
{
    EXPECT_CALL(*connection_, CloseConnection(QUIC_CRYPTO_UPDATE_BEFORE_HANDSHAKE_COMPLETE, _, _));
    CryptoHandshakeMessage server_config_update;
    server_config_update.set_tag(kSCUP);
    crypto_test_utils::SendHandshakeMessageToStream(stream(), server_config_update, Perspective::IS_SERVER);
}
