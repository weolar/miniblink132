...
public:
    QuicPathValidationContext(const QuicSocketAddress& self_address, const QuicSocketAddress& peer_address)
    ...
    QuicPathValidationContext(const QuicSocketAddress& self_address, const QuicSocketAddress& peer_address, const QuicSocketAddress& effective_peer_address)
        : self_address_(self_address)
        , peer_address_(peer_address)
...
private:
    QUICHE_EXPORT friend std::ostream& operator<<(std::ostream& os, const QuicPathValidationContext& context);
...
public:
    ...
    public:
        ...
        // the validation.
        virtual bool SendPathChallenge(const QuicPathFrameBuffer& data_buffer, const QuicSocketAddress& self_address, const QuicSocketAddress& peer_address,
        ...
        // address and writer.
        virtual QuicTime GetRetryTimeout(const QuicSocketAddress& peer_address, QuicPacketWriter* writer) const = 0;
    };
    ...
    public:
        ...
        virtual void OnPathValidationSuccess(std::unique_ptr<QuicPathValidationContext> context, QuicTime start_time) = 0;

        virtual void OnPathValidationFailure(std::unique_ptr<QuicPathValidationContext> context) = 0;
    };

    QuicPathValidator(QuicAlarmFactory* alarm_factory, QuicConnectionArena* arena, SendDelegate* delegate, QuicRandom* random, const QuicClock* clock,
        QuicConnectionContext* context);

    // Send PATH_CHALLENGE and start the retry timer.
    void StartPathValidation(std::unique_ptr<QuicPathValidationContext> context, std::unique_ptr<ResultDelegate> result_delegate, PathValidationReason reason);
    ...
    void OnPathResponse(const QuicPathFrameBuffer& probing_data, QuicSocketAddress self_address);
    ...
    PathValidationReason GetPathValidationReason() const
    {
        return reason_;
    }
    ...
    // address is pending.
    void MaybeWritePacketToAddress(const char* buffer, size_t buf_len, const QuicSocketAddress& peer_address);
