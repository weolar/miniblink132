...
QuicFrame::QuicFrame(QuicNewTokenFrame* frame)
    : type(NEW_TOKEN_FRAME)
    , new_token_frame(frame)
{
}

QuicFrame::QuicFrame(QuicAckFrequencyFrame* frame)
    : type(ACK_FREQUENCY_FRAME)
    , ack_frequency_frame(frame)
{
}

QuicFrame::QuicFrame(QuicResetStreamAtFrame* frame)
    : type(RESET_STREAM_AT_FRAME)
    , reset_stream_at_frame(frame)
{
}
...
void DeleteFrame(QuicFrame* frame)
{
    ...
    if (frame->type != PADDING_FRAME && frame->type != MTU_DISCOVERY_FRAME && frame->type != PING_FRAME && frame->type != MAX_STREAMS_FRAME
        && frame->type != STOP_WAITING_FRAME && frame->type != STREAMS_BLOCKED_FRAME && frame->type != STREAM_FRAME && frame->type != HANDSHAKE_DONE_FRAME
        && frame->type != WINDOW_UPDATE_FRAME && frame->type != BLOCKED_FRAME && frame->type != STOP_SENDING_FRAME && frame->type != PATH_CHALLENGE_FRAME
...
void SetControlFrameId(QuicControlFrameId control_frame_id, QuicFrame* frame)
{
    ...
    case RESET_STREAM_AT_FRAME:
        ...
        return;
    default:
        QUIC_BUG(quic_bug_12594_1) << "Try to set control frame id of a frame without control frame id";
    }
}
...
QuicFrame CopyRetransmittableControlFrame(const QuicFrame& frame)
{
    ...
    case STOP_SENDING_FRAME:
        ...
        break;
    case NEW_CONNECTION_ID_FRAME:
        copy = QuicFrame(new QuicNewConnectionIdFrame(*frame.new_connection_id_frame));
        break;
    case RETIRE_CONNECTION_ID_FRAME:
        copy = QuicFrame(new QuicRetireConnectionIdFrame(*frame.retire_connection_id_frame));
        break;
    case STREAMS_BLOCKED_FRAME:
    ...
    case MAX_STREAMS_FRAME:
        ...
        break;
    case HANDSHAKE_DONE_FRAME:
        copy = QuicFrame(QuicHandshakeDoneFrame(frame.handshake_done_frame.control_frame_id));
        break;
    case ACK_FREQUENCY_FRAME:
    ...
    case NEW_TOKEN_FRAME:
        ...
        break;
    case RESET_STREAM_AT_FRAME:
        copy = QuicFrame(new QuicResetStreamAtFrame(*frame.reset_stream_at_frame));
        break;
    default:
        QUIC_BUG(quic_bug_10533_1) << "Try to copy a non-retransmittable control frame: " << frame;
...
QuicFrame CopyQuicFrame(quiche::QuicheBufferAllocator* allocator, const QuicFrame& frame)
{
    QuicFrame copy;
    switch (frame.type) {
    case PADDING_FRAME:
    ...
    case RST_STREAM_FRAME:
        ...
        break;
    case CONNECTION_CLOSE_FRAME:
        copy = QuicFrame(new QuicConnectionCloseFrame(*frame.connection_close_frame));
        break;
    case GOAWAY_FRAME:
        copy = QuicFrame(new QuicGoAwayFrame(*frame.goaway_frame));
        break;
    ...
    case MESSAGE_FRAME:
        ...
        for (const auto& slice : frame.message_frame->message_data) {
            quiche::QuicheBuffer buffer = quiche::QuicheBuffer::Copy(allocator, slice.AsStringView());
            copy.message_frame->message_data.push_back(quiche::QuicheMemSlice(std::move(buffer)));
        }
        break;
    case NEW_TOKEN_FRAME:
        copy = QuicFrame(new QuicNewTokenFrame(*frame.new_token_frame));
...
QuicFrames CopyQuicFrames(quiche::QuicheBufferAllocator* allocator, const QuicFrames& frames)
{
    QuicFrames copy;
...
std::ostream& operator<<(std::ostream& os, const QuicFrame& frame)
{
    ...
    case CONNECTION_CLOSE_FRAME: {
        os << "type { CONNECTION_CLOSE_FRAME } " << *(frame.connection_close_frame);
        break;
    }
    case GOAWAY_FRAME: {
    ...
    case NEW_CONNECTION_ID_FRAME:
        ...
        break;
    case RETIRE_CONNECTION_ID_FRAME:
        os << "type { RETIRE_CONNECTION_ID } " << *(frame.retire_connection_id_frame);
        break;
    case MAX_STREAMS_FRAME:
