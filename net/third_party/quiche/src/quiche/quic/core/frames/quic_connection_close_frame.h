...
struct QUICHE_EXPORT QuicConnectionCloseFrame {
    ...
    QuicConnectionCloseFrame(QuicTransportVersion transport_version, QuicErrorCode error_code, QuicIetfTransportErrorCodes ietf_error, std::string error_phrase,
        uint64_t transport_close_frame_type);

    friend QUICHE_EXPORT std::ostream& operator<<(std::ostream& os, const QuicConnectionCloseFrame& c);
