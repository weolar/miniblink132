...
struct QUICHE_EXPORT QuicPathResponseFrame : public QuicInlinedFrame<QuicPathResponseFrame> {
    QuicPathResponseFrame();
    QuicPathResponseFrame(QuicControlFrameId control_frame_id, const QuicPathFrameBuffer& data_buff);
    ~QuicPathResponseFrame() = default;

    friend QUICHE_EXPORT std::ostream& operator<<(std::ostream& os, const QuicPathResponseFrame& frame);
