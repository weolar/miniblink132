...
struct QUICHE_EXPORT QuicFrame {
    ...
    QUICHE_EXPORT friend std::ostream& operator<<(std::ostream& os, const QuicFrame& frame);
...
static_assert(std::is_standard_layout<QuicFrame>::value, "QuicFrame must have a standard layout");
static_assert(sizeof(QuicFrame) <= 24, "Frames larger than 24 bytes should be referenced by pointer.");
static_assert(offsetof(QuicStreamFrame, type) == offsetof(QuicFrame, type), "Offset of |type| must match in QuicFrame and QuicStreamFrame");
...
QUICHE_EXPORT void RemoveFramesForStream(QuicFrames* frames, QuicStreamId stream_id);
...
QUICHE_EXPORT void SetControlFrameId(QuicControlFrameId control_frame_id, QuicFrame* frame);

// Returns a copy of |frame|.
QUICHE_EXPORT QuicFrame CopyRetransmittableControlFrame(const QuicFrame& frame);

// Returns a copy of |frame|.
QUICHE_EXPORT QuicFrame CopyQuicFrame(quiche::QuicheBufferAllocator* allocator, const QuicFrame& frame);

// Returns a copy of |frames|.
QUICHE_EXPORT QuicFrames CopyQuicFrames(quiche::QuicheBufferAllocator* allocator, const QuicFrames& frames);
