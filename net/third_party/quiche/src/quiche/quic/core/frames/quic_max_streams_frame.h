...
struct QUICHE_EXPORT QuicMaxStreamsFrame : public QuicInlinedFrame<QuicMaxStreamsFrame> {
    QuicMaxStreamsFrame();
    QuicMaxStreamsFrame(QuicControlFrameId control_frame_id, QuicStreamCount stream_count, bool unidirectional);

    friend QUICHE_EXPORT std::ostream& operator<<(std::ostream& os, const QuicMaxStreamsFrame& frame);
