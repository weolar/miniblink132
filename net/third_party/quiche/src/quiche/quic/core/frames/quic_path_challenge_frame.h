...
struct QUICHE_EXPORT QuicPathChallengeFrame : public QuicInlinedFrame<QuicPathChallengeFrame> {
    QuicPathChallengeFrame();
    QuicPathChallengeFrame(QuicControlFrameId control_frame_id, const QuicPathFrameBuffer& data_buff);
    ~QuicPathChallengeFrame() = default;

    friend QUICHE_EXPORT std::ostream& operator<<(std::ostream& os, const QuicPathChallengeFrame& frame);
