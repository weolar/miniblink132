...
bool IsAwaitingPacket(const QuicAckFrame& ack_frame, QuicPacketNumber packet_number, QuicPacketNumber peer_least_packet_awaiting_ack)
{
    QUICHE_DCHECK(packet_number.IsInitialized());
    return (!peer_least_packet_awaiting_ack.IsInitialized() || packet_number >= peer_least_packet_awaiting_ack) && !ack_frame.packets.Contains(packet_number);
}
...
{
    os << "{ largest_acked: " << LargestAcked(ack_frame) << ", ack_delay_time: " << ack_frame.ack_delay_time.ToMicroseconds() << ", packets: [ "
       << ack_frame.packets << " ]"
       << ", received_packets: [ ";
    for (const std::pair<QuicPacketNumber, QuicTime>& p : ack_frame.received_packet_times) {
    ...
    if (ack_frame.ecn_counters.has_value()) {
        os << ", ect_0_count: " << ack_frame.ecn_counters->ect0 << ", ect_1_count: " << ack_frame.ecn_counters->ect1
...
PacketNumberQueue& PacketNumberQueue::operator=(const PacketNumberQueue& other) = default;
PacketNumberQueue& PacketNumberQueue::operator=(PacketNumberQueue&& other) = default;
...
void PacketNumberQueue::Add(QuicPacketNumber packet_number)
{
    ...
    packet_number_intervals_.AddOptimizedForAppend(packet_number, packet_number + 1);
}

void PacketNumberQueue::AddRange(QuicPacketNumber lower, QuicPacketNumber higher)
{
...
void PacketNumberQueue::RemoveSmallestInterval()
{
    ...
    QUIC_BUG_IF(quic_bug_12614_1, packet_number_intervals_.Size() < 2) << (Empty() ? "No intervals to remove." : "Can't remove the last interval.");
...
std::ostream& operator<<(std::ostream& os, const PacketNumberQueue& q)
{
    for (const QuicInterval<QuicPacketNumber>& interval : q) {
        QUIC_BUG_IF(quic_bug_12614_2, interval.min() >= interval.max())
            << "Ack Range minimum (" << interval.min() << "Not less than max (" << interval.max() << ")";
