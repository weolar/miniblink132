...
public:
    ...
    using const_reverse_iterator = QuicIntervalSet<QuicPacketNumber>::const_reverse_iterator;
    ...
    friend QUICHE_EXPORT std::ostream& operator<<(std::ostream& os, const PacketNumberQueue& q);
...
struct QUICHE_EXPORT QuicAckFrame {
    ...
    friend QUICHE_EXPORT std::ostream& operator<<(std::ostream& os, const QuicAckFrame& ack_frame);
...
{
    QUICHE_DCHECK(frame.packets.Empty() || frame.packets.Max() == frame.largest_acked);
...
QUICHE_EXPORT bool IsAwaitingPacket(const QuicAckFrame& ack_frame, QuicPacketNumber packet_number, QuicPacketNumber peer_least_packet_awaiting_ack);
