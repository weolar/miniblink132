...
struct QUICHE_EXPORT QuicStreamFrame : public QuicInlinedFrame<QuicStreamFrame> {
    QuicStreamFrame();
    QuicStreamFrame(QuicStreamId stream_id, bool fin, QuicStreamOffset offset, absl::string_view data);
    QuicStreamFrame(QuicStreamId stream_id, bool fin, QuicStreamOffset offset, QuicPacketLength data_length);

    friend QUICHE_EXPORT std::ostream& operator<<(std::ostream& os, const QuicStreamFrame& s);
    ...
    QuicStreamFrame(QuicStreamId stream_id, bool fin, QuicStreamOffset offset, const char* data_buffer, QuicPacketLength data_length);
};
