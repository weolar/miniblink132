...
QuicMessageFrame::QuicMessageFrame(QuicMessageId message_id, absl::Span<quiche::QuicheMemSlice> span)
    : message_id(message_id)
    , data(nullptr)
    , message_length(0)
{
...
QuicMessageFrame::QuicMessageFrame(QuicMessageId message_id, quiche::QuicheMemSlice slice)
...
{
    os << " message_id: " << s.message_id << ", message_length: " << s.message_length << " }\n";
    return os;
}
