...
public:
    QuicClientPushPromiseIndexTest()
        ...
        , promised_(&session_, GetNthServerInitiatedUnidirectionalStreamId(connection_->transport_version(), 0), url_)
    {
        request_[":path"] = "/bar";
...
TEST_F(QuicClientPushPromiseIndexTest, TryRequestSuccess)
{
    ...
    EXPECT_CALL(promised_, HandleClientRequest(_, _)).WillOnce(Return(QUIC_SUCCESS));
...
{
    (*index_.promised_by_url())[url_] = &promised_;
    EXPECT_CALL(promised_, HandleClientRequest(_, _)).WillOnce(Return(QUIC_PENDING));
...
{
    (*index_.promised_by_url())[url_] = &promised_;
    EXPECT_CALL(promised_, HandleClientRequest(_, _)).WillOnce(Return(QUIC_FAILURE));
