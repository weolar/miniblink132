...
// static
std::string SpdyServerPushUtils::GetPromisedHostNameFromHeaders(const Http2HeaderBlock& headers)
{
...
// static
std::string SpdyServerPushUtils::GetPushPromiseUrl(absl::string_view scheme, absl::string_view authority, absl::string_view path)
{
    ...
    if (!url::CanonicalizeScheme(scheme.data(), scheme_component, &canon_scheme_output, &canon_component) || !canon_component.is_nonempty()
    ...
    url::ParseAuthority(authority.data(), auth_component, &username_component, &password_component, &host_component, &port_component);
    ...
    if (!url::CanonicalizeHost(authority.data(), host_component, &canon_host_output, &canon_component) || !canon_component.is_nonempty()
        || canon_component.begin != 0) {
        return std::string();
    }
    ...
    if (!origin_url.is_valid() || !origin_url.SchemeIsHTTPOrHTTPS() ||
        ...
        origin_url.has_username() || origin_url.has_password() || (origin_url.has_path() && origin_url.path_piece() != "/") || origin_url.has_query()
        || origin_url.has_ref()) {
        return std::string();
    }
