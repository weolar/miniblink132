...
TEST(HttpFramesTest, PriorityUpdateFrame)
{
    ...
    EXPECT_EQ("Priority Frame : {prioritized_element_id: 4, priority_field_value: foo}", a.ToString());
    std::stringstream s;
    s << a;
    EXPECT_EQ("Priority Frame : {prioritized_element_id: 4, priority_field_value: foo}", s.str());
}
...
TEST(HttpFramesTest, AcceptChFrame)
{
    ...
    EXPECT_EQ("ACCEPT_CH frame with 1 entries: origin: foo; value: bar", a.ToString());
    std::stringstream s;
    s << a;
