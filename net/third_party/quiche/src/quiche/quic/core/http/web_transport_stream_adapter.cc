...
WebTransportStreamAdapter::WebTransportStreamAdapter(
    QuicSession* session, QuicStream* stream, QuicStreamSequencer* sequencer, std::optional<QuicStreamId> session_id)
    : session_(session)
    , stream_(stream)
...
WebTransportStream::ReadResult WebTransportStreamAdapter::Read(absl::Span<char> buffer)
{
    iovec iov;
...
WebTransportStream::ReadResult WebTransportStreamAdapter::Read(std::string* output)
{
    const size_t old_size = output->size();
    const size_t bytes_to_read = ReadableBytes();
    output->resize(old_size + bytes_to_read);
    ReadResult result = Read(absl::Span<char>(&(*output)[old_size], bytes_to_read));
    QUICHE_DCHECK_EQ(bytes_to_read, result.bytes_read);
    output->resize(old_size + result.bytes_read);
    return result;
}

absl::Status WebTransportStreamAdapter::Writev(absl::Span<const absl::string_view> data, const quiche::StreamWriteOptions& options)
{
    if (data.empty() && !options.send_fin()) {
        return absl::InvalidArgumentError("Writev() called without any data or a FIN");
    }
    const absl::Status initial_check_status = CheckBeforeStreamWrite();
    if (!initial_check_status.ok() && !(initial_check_status.code() == absl::StatusCode::kUnavailable && options.buffer_unconditionally())) {
    ...
    quiche::QuicheMemSliceStorage storage(
        iovecs.data(), iovecs.size(), session_->connection()->helper()->GetStreamSendBufferAllocator(), GetQuicFlag(quic_send_buffer_max_data_slice_size));
    QuicConsumedData consumed = stream_->WriteMemSlices(storage.ToSpan(), /*fin=*/options.send_fin(),
    ...
    constexpr absl::string_view kErrorMessage = "WriteMemSlices() unexpectedly partially consumed the input data";
    QUIC_BUG(WebTransportStreamAdapter partial write) << kErrorMessage << ", provided: " << total_size << ", written: " << consumed.bytes_consumed;
    stream_->OnUnrecoverableError(QUIC_INTERNAL_ERROR, std::string(kErrorMessage));
...
{
    QUIC_DLOG(WARNING) << (session_->perspective() == Perspective::IS_CLIENT ? "Client: " : "Server: ") << "Abruptly terminating stream " << stream_->id()
...
quiche::ReadStream::PeekResult WebTransportStreamAdapter::PeekNextReadableRegion() const
{
    iovec iov;
    PeekResult result;
    if (sequencer_->GetReadableRegion(&iov)) {
        result.peeked_data = absl::string_view(static_cast<const char*>(iov.iov_base), iov.iov_len);
    }
...
void WebTransportStreamAdapter::ResetWithUserCode(WebTransportStreamError error)
{
    stream_->ResetWriteSide(QuicResetStreamError(QUIC_STREAM_CANCELLED, WebTransportErrorToHttp3(error)));
}

void WebTransportStreamAdapter::SendStopSending(WebTransportStreamError error)
{
    stream_->SendStopSending(QuicResetStreamError(QUIC_STREAM_CANCELLED, WebTransportErrorToHttp3(error)));
}

void WebTransportStreamAdapter::SetPriority(const webtransport::StreamPriority& priority)
{
    ...
    QuicStreamId session_id = session_id_.value_or(std::numeric_limits<QuicStreamId>::max());
    stream_->SetPriority(QuicStreamPriority(WebTransportStreamPriority { session_id, priority.send_group_id, priority.send_order }));
}
...
void WebTransportStreamAdapter::SetSessionId(QuicStreamId id)
{
    ...
    case QuicPriorityType::kHttp:
        stream_->SetPriority(QuicStreamPriority(WebTransportStreamPriority { id, 0, 0 }));
        break;
    case QuicPriorityType::kWebTransport:
        stream_->SetPriority(
            QuicStreamPriority(WebTransportStreamPriority { id, old_priority.web_transport().send_group_number, old_priority.web_transport().send_order }));
        break;
    }
}
