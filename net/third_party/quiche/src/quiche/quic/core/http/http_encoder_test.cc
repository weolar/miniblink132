...
TEST(HttpEncoderTest, SerializeDataFrameHeader)
{
    ...
    quiche::test::CompareCharArraysWithHexError("DATA", buffer.data(), buffer.size(), output, ABSL_ARRAYSIZE(output));
}

TEST(HttpEncoderTest, SerializeHeadersFrameHeader)
{
    std::string header = HttpEncoder::SerializeHeadersFrameHeader(/* payload_length = */ 7);
    char output[] = { 0x01, // type (HEADERS)
        0x07 }; // length
    quiche::test::CompareCharArraysWithHexError("HEADERS", header.data(), header.length(), output, ABSL_ARRAYSIZE(output));
}
...
TEST(HttpEncoderTest, SerializeSettingsFrame)
{
    ...
    quiche::test::CompareCharArraysWithHexError("SETTINGS", frame.data(), frame.length(), output, ABSL_ARRAYSIZE(output));
}
...
TEST(HttpEncoderTest, SerializeGoAwayFrame)
{
    ...
    quiche::test::CompareCharArraysWithHexError("GOAWAY", frame.data(), frame.length(), output, ABSL_ARRAYSIZE(output));
}
...
TEST(HttpEncoderTest, SerializePriorityUpdateFrame)
{
    ...
    std::string frame1 = HttpEncoder::SerializePriorityUpdateFrame(priority_update1);
    quiche::test::CompareCharArraysWithHexError("PRIORITY_UPDATE", frame1.data(), frame1.length(), reinterpret_cast<char*>(output1), ABSL_ARRAYSIZE(output1));
    ...
    std::string frame2 = HttpEncoder::SerializePriorityUpdateFrame(priority_update2);
    quiche::test::CompareCharArraysWithHexError("PRIORITY_UPDATE", frame2.data(), frame2.length(), reinterpret_cast<char*>(output2), ABSL_ARRAYSIZE(output2));
}
...
TEST(HttpEncoderTest, SerializeEmptyOriginFrame)
{
    ...
    quiche::test::CompareCharArraysWithHexError("ORIGIN", output.data(), output.length(), reinterpret_cast<char*>(expected), ABSL_ARRAYSIZE(expected));
}
...
TEST(HttpEncoderTest, SerializeAcceptChFrame)
{
    ...
    quiche::test::CompareCharArraysWithHexError("ACCEPT_CH", frame1.data(), frame1.length(), reinterpret_cast<char*>(output1), ABSL_ARRAYSIZE(output1));
    ...
    quiche::test::CompareCharArraysWithHexError("ACCEPT_CH", frame2.data(), frame2.length(), reinterpret_cast<char*>(output2), ABSL_ARRAYSIZE(output2));
}
...
TEST(HttpEncoderTest, SerializeWebTransportStreamFrameHeader)
{
    ...
    std::string frame = HttpEncoder::SerializeWebTransportStreamFrameHeader(session_id);
    quiche::test::CompareCharArraysWithHexError("WEBTRANSPORT_STREAM", frame.data(), frame.length(), output, sizeof(output));
}
...
TEST(HttpEncoderTest, SerializeMetadataFrameHeader)
{
    ...
    char output[] = { 0x40, 0x4d, // type (METADATA, 0x4d, varint encoded)
        0x07 }; // length
    quiche::test::CompareCharArraysWithHexError("METADATA", frame.data(), frame.length(), output, ABSL_ARRAYSIZE(output));
}
