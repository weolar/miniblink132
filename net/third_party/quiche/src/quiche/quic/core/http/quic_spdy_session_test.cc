...
public:
    ...
    void EstablishZeroRttEncryption()
    {
        ...
        session()->connection()->SetEncrypter(ENCRYPTION_ZERO_RTT, std::make_unique<TaggingEncrypter>(ENCRYPTION_ZERO_RTT));
    }
    ...
    void OnHandshakeMessage(const CryptoHandshakeMessage& /*message*/) override
    {
        ...
        session()->config()->SetInitialStreamFlowControlWindowToSend(kInitialStreamFlowControlWindowForTest);
        session()->config()->SetInitialSessionFlowControlWindowToSend(kInitialSessionFlowControlWindowForTest);
        if (session()->version().UsesTls()) {
            if (session()->perspective() == Perspective::IS_CLIENT) {
                session()->config()->SetOriginalConnectionIdToSend(session()->connection()->connection_id());
                session()->config()->SetInitialSourceConnectionIdToSend(session()->connection()->connection_id());
            } else {
                session()->config()->SetInitialSourceConnectionIdToSend(session()->connection()->client_connection_id());
            }
            TransportParameters transport_parameters;
            EXPECT_TRUE(session()->config()->FillTransportParameters(&transport_parameters));
            error = session()->config()->ProcessTransportParameters(transport_parameters, /* is_resumption = */ false, &error_details);
        } else {
        ...
        session()->OnNewEncryptionKeyAvailable(ENCRYPTION_FORWARD_SECURE, std::make_unique<TaggingEncrypter>(ENCRYPTION_FORWARD_SECURE));
        session()->OnConfigNegotiated();
        if (session()->connection()->version().handshake_protocol == PROTOCOL_TLS1_3) {
    ...
    void SetServerApplicationStateForResumption(std::unique_ptr<ApplicationState> /*application_state*/) override
    {
    }
    ...
    const QuicCryptoNegotiatedParameters& crypto_negotiated_params() const override
    {
        return *params_;
    }
    ...
    std::string GetAddressToken(const CachedNetworkParameters* /*cached_network_params*/) const override
    {
        return "";
    }
    ...
    void SetPreviousCachedNetworkParams(CachedNetworkParameters /*cached_network_params*/) override
    {
    }

    MOCK_METHOD(void, OnCanWrite, (), (override));

    bool HasPendingCryptoRetransmission() const override
    {
        return false;
    }
    ...
    bool IsCryptoFrameExpectedForEncryptionLevel(EncryptionLevel level) const override
    {
        return level != ENCRYPTION_ZERO_RTT;
    }
    EncryptionLevel GetEncryptionLevelToSendCryptoDataOfSpace(PacketNumberSpace space) const override
    {
        switch (space) {
        case INITIAL_DATA:
    ...
    bool ExportKeyingMaterial(absl::string_view /*label*/, absl::string_view /*context*/, size_t /*result_len*/, std::string*
        /*result*/) override
    {
        return false;
    }
...
public:
    ...
    MOCK_METHOD(void, OnCanWrite, (), (override));
    MOCK_METHOD(bool, RetransmitStreamData, (QuicStreamOffset, QuicByteCount, bool, TransmissionType), (override));
...
public:
    explicit TestSession(QuicConnection* connection)
        : QuicSpdySession(connection, nullptr, DefaultQuicConfig(), CurrentSupportedVersions())
        , crypto_stream_(this)
        , writev_consumes_all_data_(false)
    {
        this->connection()->SetEncrypter(ENCRYPTION_FORWARD_SECURE, std::make_unique<TaggingEncrypter>(ENCRYPTION_FORWARD_SECURE));
    ...
    {
        TestStream* stream = new TestStream(GetNextOutgoingBidirectionalStreamId(), this, BIDIRECTIONAL);
    ...
    {
        TestStream* stream = new TestStream(GetNextOutgoingUnidirectionalStreamId(), this, WRITE_UNIDIRECTIONAL);
        ActivateStream(absl::WrapUnique(stream));
        return stream;
    }
    ...
    TestStream* CreateIncomingStream(QuicStreamId id) override
    {
        ...
        if (!VersionHasIetfQuicFrames(connection()->transport_version())
            && stream_id_manager().num_open_incoming_streams() + 1 > max_open_incoming_bidirectional_streams()) {
            connection()->CloseConnection(QUIC_TOO_MANY_OPEN_STREAMS, "Too many streams!", ConnectionCloseBehavior::SEND_CONNECTION_CLOSE_PACKET);
            return nullptr;
        } else {
    ...
    QuicConsumedData WritevData(
        QuicStreamId id, size_t write_length, QuicStreamOffset offset, StreamSendingState state, TransmissionType type, EncryptionLevel level) override
    {
        bool fin = state != NO_FIN;
        QuicConsumedData consumed(write_length, fin);
        if (!writev_consumes_all_data_) {
            consumed = QuicSession::WritevData(id, write_length, offset, state, type, level);
        }
        QuicSessionPeer::GetWriteBlockedStreams(this)->UpdateBytesForStream(id, consumed.bytes_consumed);
        return consumed;
    }
    ...
    {
        if (!QuicUtils::IsCryptoStreamId(connection()->transport_version(), stream->id()) && connection()->encryption_level() != ENCRYPTION_FORWARD_SECURE) {
            this->connection()->SetDefaultEncryptionLevel(ENCRYPTION_FORWARD_SECURE);
        }
        QuicStreamPeer::SendBuffer(stream).SaveStreamData("not empty");
        QuicConsumedData consumed = WritevData(stream->id(), 9, 0, FIN, NOT_RETRANSMISSION, GetEncryptionLevelToSendApplicationData());
        QuicStreamPeer::SendBuffer(stream).OnStreamDataConsumed(consumed.bytes_consumed);
        return consumed;
    }
    ...
    QuicConsumedData SendLargeFakeData(QuicStream* stream, int bytes)
    {
        ...
        return WritevData(stream->id(), bytes, 0, FIN, NOT_RETRANSMISSION, GetEncryptionLevelToSendApplicationData());
    }

    WebTransportHttp3VersionSet LocallySupportedWebTransportVersions() const override
    {
        return locally_supported_web_transport_versions_;
    }
    void set_supports_webtransport(bool value)
    {
        locally_supported_web_transport_versions_ = value ? kDefaultSupportedWebTransportVersions : WebTransportHttp3VersionSet();
    }
    void set_locally_supported_web_transport_versions(WebTransportHttp3VersionSet versions)
    {
...
protected:
    explicit QuicSpdySessionTestBase(Perspective perspective, bool allow_extended_connect)
        : connection_(new StrictMock<MockQuicConnection>(&helper_, &alarm_factory_, perspective, SupportedVersions(GetParam())))
    ...
    void Initialize()
    {
        ...
        if (qpack_maximum_dynamic_table_capacity_.has_value()) {
            session_->set_qpack_maximum_dynamic_table_capacity(*qpack_maximum_dynamic_table_capacity_);
        }
        if (connection_->perspective() == Perspective::IS_SERVER && VersionUsesHttp3(transport_version())) {
            session_->set_allow_extended_connect(allow_extended_connect_);
        }
        session_->Initialize();
        session_->config()->SetInitialStreamFlowControlWindowToSend(kInitialStreamFlowControlWindowForTest);
        session_->config()->SetInitialSessionFlowControlWindowToSend(kInitialSessionFlowControlWindowForTest);
        if (VersionUsesHttp3(transport_version())) {
            QuicConfigPeer::SetReceivedMaxUnidirectionalStreams(session_->config(), kHttp3StaticUnidirectionalStreamCount);
        }
        QuicConfigPeer::SetReceivedInitialSessionFlowControlWindow(session_->config(), kMinimumFlowControlSendWindow);
        QuicConfigPeer::SetReceivedInitialMaxStreamDataBytesUnidirectional(session_->config(), kMinimumFlowControlSendWindow);
        QuicConfigPeer::SetReceivedInitialMaxStreamDataBytesIncomingBidirectional(session_->config(), kMinimumFlowControlSendWindow);
        QuicConfigPeer::SetReceivedInitialMaxStreamDataBytesOutgoingBidirectional(session_->config(), kMinimumFlowControlSendWindow);
        session_->OnConfigNegotiated();
        connection_->AdvanceTime(QuicTime::Delta::FromSeconds(1));
        TestCryptoStream* crypto_stream = session_->GetMutableCryptoStream();
        EXPECT_CALL(*crypto_stream, HasPendingRetransmission()).Times(testing::AnyNumber());
        writer_ = static_cast<MockPacketWriter*>(QuicConnectionPeer::GetWriter(session_->connection()));
    }

    void CheckClosedStreams()
    {
        QuicStreamId first_stream_id = QuicUtils::GetFirstBidirectionalStreamId(transport_version(), Perspective::IS_CLIENT);
    ...
    void CloseStream(QuicStreamId id)
    {
        if (!VersionHasIetfQuicFrames(transport_version())) {
            EXPECT_CALL(*connection_, SendControlFrame(_)).WillOnce(Invoke(&ClearControlFrame));
        } else {
            // IETF QUIC has two frames, RST_STREAM and STOP_SENDING
            EXPECT_CALL(*connection_, SendControlFrame(_)).Times(2).WillRepeatedly(Invoke(&ClearControlFrame));
        }
    ...
    QuicStreamId StreamCountToId(QuicStreamCount stream_count, Perspective perspective, bool bidirectional)
    {
        ...
        QuicStreamId id = ((stream_count - 1) * QuicUtils::StreamIdDelta(transport_version()));
        if (!bidirectional) {
            id |= 0x2;
        }
    ...
    {
        if (VersionHasIetfQuicFrames(transport_version())) {
            EXPECT_CALL(*writer_, WritePacket(_, _, _, _, _, _)).WillOnce(Return(WriteResult(WRITE_STATUS_OK, 0)));
        }
        if (connection_->version().UsesTls() && connection_->perspective() == Perspective::IS_SERVER) {
    ...
    void ReceiveWebTransportSettings(WebTransportHttp3VersionSet versions = kDefaultSupportedWebTransportVersions)
    {
        ...
        std::string data = std::string(1, kControlStream) + HttpEncoder::SerializeSettingsFrame(settings);
        QuicStreamId control_stream_id = session_->perspective() == Perspective::IS_SERVER
    ...
    {
        QuicStreamFrame frame(session_id, /*fin=*/false, /*offset=*/0, absl::string_view());
        session_->OnStreamFrame(frame);
        QuicSpdyStream* stream = static_cast<QuicSpdyStream*>(session_->GetOrCreateStream(session_id));
        QuicHeaderList headers;
        headers.OnHeader(":method", "CONNECT");
        headers.OnHeader(":protocol", "webtransport");
        stream->OnStreamHeaderList(/*fin=*/true, 0, headers);
        WebTransportHttp3* web_transport = session_->GetWebTransportSession(session_id);
        ASSERT_TRUE(web_transport != nullptr);
        quiche::HttpHeaderBlock header_block;
        web_transport->HeadersReceived(header_block);
    }

    void ReceiveWebTransportUnidirectionalStream(WebTransportSessionId session_id, QuicStreamId stream_id)
    {
        char buffer[256];
    ...
    void TestHttpDatagramSetting(
        HttpDatagramSupport local_support, HttpDatagramSupport remote_support, HttpDatagramSupport expected_support, bool expected_datagram_supported);
...
INSTANTIATE_TEST_SUITE_P(Tests, QuicSpdySessionTestServer, ::testing::ValuesIn(AllSupportedVersions()), ::testing::PrintToStringParamName());
...
TEST_P(QuicSpdySessionTestServer, UsesPendingStreamsForFrame)
{
    ...
    EXPECT_TRUE(session_->UsesPendingStreamForFrame(STREAM_FRAME, QuicUtils::GetFirstUnidirectionalStreamId(transport_version(), Perspective::IS_CLIENT)));
    EXPECT_TRUE(session_->UsesPendingStreamForFrame(RST_STREAM_FRAME, QuicUtils::GetFirstUnidirectionalStreamId(transport_version(), Perspective::IS_CLIENT)));
    EXPECT_FALSE(session_->UsesPendingStreamForFrame(RST_STREAM_FRAME, QuicUtils::GetFirstUnidirectionalStreamId(transport_version(), Perspective::IS_SERVER)));
    EXPECT_FALSE(
        session_->UsesPendingStreamForFrame(STOP_SENDING_FRAME, QuicUtils::GetFirstUnidirectionalStreamId(transport_version(), Perspective::IS_CLIENT)));
    EXPECT_FALSE(session_->UsesPendingStreamForFrame(RST_STREAM_FRAME, QuicUtils::GetFirstBidirectionalStreamId(transport_version(), Perspective::IS_CLIENT)));
}

TEST_P(QuicSpdySessionTestServer, PeerAddress)
{
    Initialize();
    EXPECT_EQ(QuicSocketAddress(QuicIpAddress::Loopback4(), kTestPort), session_->peer_address());
}
...
{
    Initialize();
    ASSERT_TRUE(session_->GetOrCreateStream(GetNthClientInitiatedBidirectionalId(2)) != nullptr);
    // Both client initiated streams with smaller stream IDs are available.
    EXPECT_TRUE(QuicSessionPeer::IsStreamAvailable(&*session_, GetNthClientInitiatedBidirectionalId(0)));
    EXPECT_TRUE(QuicSessionPeer::IsStreamAvailable(&*session_, GetNthClientInitiatedBidirectionalId(1)));
    ASSERT_TRUE(session_->GetOrCreateStream(GetNthClientInitiatedBidirectionalId(1)) != nullptr);
    ASSERT_TRUE(session_->GetOrCreateStream(GetNthClientInitiatedBidirectionalId(0)) != nullptr);
}
...
TEST_P(QuicSpdySessionTestServer, MaximumAvailableOpenedStreams)
{
    ...
    if (VersionHasIetfQuicFrames(transport_version())) {
        ...
        QuicStreamId stream_id = StreamCountToId(QuicSessionPeer::ietf_streamid_manager(&*session_)->max_incoming_bidirectional_streams(),
            Perspective::IS_CLIENT, // Client initates stream, allocs stream id.
            /*bidirectional=*/true);
        EXPECT_NE(nullptr, session_->GetOrCreateStream(stream_id));
        stream_id = StreamCountToId(QuicSessionPeer::ietf_streamid_manager(&*session_)->max_incoming_unidirectional_streams(), Perspective::IS_CLIENT,
            /*bidirectional=*/false);
        EXPECT_NE(nullptr, session_->GetOrCreateStream(stream_id));
        EXPECT_CALL(*connection_, CloseConnection(_, _, _)).Times(2);
        // Get the (max allowed stream ID)++. These should all fail.
        stream_id = StreamCountToId(QuicSessionPeer::ietf_streamid_manager(&*session_)->max_incoming_bidirectional_streams() + 1, Perspective::IS_CLIENT,
            /*bidirectional=*/true);
        EXPECT_EQ(nullptr, session_->GetOrCreateStream(stream_id));

        stream_id = StreamCountToId(QuicSessionPeer::ietf_streamid_manager(&*session_)->max_incoming_unidirectional_streams() + 1, Perspective::IS_CLIENT,
            /*bidirectional=*/false);
        EXPECT_EQ(nullptr, session_->GetOrCreateStream(stream_id));
    } else {
        ...
        EXPECT_NE(nullptr, session_->GetOrCreateStream(stream_id + IdDelta() * (session_->max_open_incoming_bidirectional_streams() - 1)));
    }
}
...
TEST_P(QuicSpdySessionTestServer, TooManyAvailableStreams)
{
    ...
    stream_id2 = GetNthClientInitiatedBidirectionalId(2 * session_->MaxAvailableBidirectionalStreams() + 4);
    if (VersionHasIetfQuicFrames(transport_version())) {
        EXPECT_CALL(*connection_, CloseConnection(QUIC_INVALID_STREAM_ID, _, _));
    } else {
        EXPECT_CALL(*connection_, CloseConnection(QUIC_TOO_MANY_AVAILABLE_STREAMS, _, _));
    }
...
TEST_P(QuicSpdySessionTestServer, ManyAvailableStreams)
{
    ...
    EXPECT_NE(nullptr, session_->GetOrCreateStream(GetNthClientInitiatedBidirectionalId(198)));
}

TEST_P(QuicSpdySessionTestServer, DebugDFatalIfMarkingClosedStreamWriteBlocked)
{
    Initialize();
    CompleteHandshake();
    EXPECT_CALL(*writer_, WritePacket(_, _, _, _, _, _)).WillRepeatedly(Return(WriteResult(WRITE_STATUS_OK, 0)));
    ...
    std::string msg = absl::StrCat("Marking unknown stream ", closed_stream_id, " blocked.");
    EXPECT_QUIC_BUG(session_->MarkConnectionLevelWriteBlocked(closed_stream_id), msg);
}
...
TEST_P(QuicSpdySessionTestServer, TooLargeStreamBlocked)
{
    ...
    // the peer is blocked.
    QuicSessionPeer::SetMaxOpenIncomingBidirectionalStreams(static_cast<QuicSession*>(&*session_), QuicUtils::GetMaxStreamCount());
...
TEST_P(QuicSpdySessionTestServer, OnCanWriteBundlesStreams)
{
    ...
    EXPECT_CALL(*send_algorithm, GetCongestionWindow()).WillRepeatedly(Return(kMaxOutgoingPacketSize * 10));
    EXPECT_CALL(*send_algorithm, InRecovery()).WillRepeatedly(Return(false));
    EXPECT_CALL(*stream2, OnCanWrite()).WillOnce(Invoke([this, stream2]() { session_->SendStreamData(stream2); }));
    EXPECT_CALL(*stream4, OnCanWrite()).WillOnce(Invoke([this, stream4]() { session_->SendStreamData(stream4); }));
    EXPECT_CALL(*stream6, OnCanWrite()).WillOnce(Invoke([this, stream6]() { session_->SendStreamData(stream6); }));
...
TEST_P(QuicSpdySessionTestServer, BufferedHandshake)
{
    ...
    session_->MarkConnectionLevelWriteBlocked(QuicUtils::GetCryptoStreamId(transport_version()));
    ...
    EXPECT_CALL(*stream2, OnCanWrite()).WillOnce(Invoke([this, stream2]() { session_->SendStreamData(stream2); }));
    EXPECT_CALL(*stream3, OnCanWrite()).WillOnce(Invoke([this, stream3]() { session_->SendStreamData(stream3); }));
...
TEST_P(QuicSpdySessionTestServer, OnCanWriteWithClosedStream)
{
    ...
    InSequence s;
    EXPECT_CALL(*connection_, SendControlFrame(_)).WillRepeatedly(Invoke(&ClearControlFrame));
    EXPECT_CALL(*stream2, OnCanWrite()).WillOnce(Invoke([this, stream2]() { session_->SendStreamData(stream2); }));
    EXPECT_CALL(*stream4, OnCanWrite()).WillOnce(Invoke([this, stream4]() { session_->SendStreamData(stream4); }));
    session_->OnCanWrite();
    EXPECT_FALSE(session_->WillingAndAbleToWrite());
...
TEST_P(QuicSpdySessionTestServer, OnCanWriteLimitsNumWritesIfFlowControlBlocked)
{
    Initialize();
    CompleteHandshake();
    // Drive congestion control manually in order to ensure that
    ...
    if (!VersionUsesHttp3(transport_version())) {
        ...
        session_->MarkConnectionLevelWriteBlocked(QuicUtils::GetHeadersStreamId(transport_version()));
...
TEST_P(QuicSpdySessionTestServer, SendGoAway)
{
    ...
    EXPECT_CALL(*connection_, SendControlFrame(_)).WillOnce(Invoke(connection_, &MockQuicConnection::ReallySendControlFrame));
    session_->SendGoAway(QUIC_PEER_GOING_AWAY, "Going Away.");
    EXPECT_TRUE(session_->goaway_sent());

    const QuicStreamId kTestStreamId = 5u;
    EXPECT_CALL(*connection_, SendControlFrame(_)).Times(0);
    EXPECT_CALL(*connection_, OnStreamReset(kTestStreamId, QUIC_STREAM_PEER_GOING_AWAY)).Times(0);
...
TEST_P(QuicSpdySessionTestServer, SendGoAwayWithoutEncryption)
{
    ...
    EXPECT_CALL(*connection_, CloseConnection(QUIC_PEER_GOING_AWAY, "Going Away.", ConnectionCloseBehavior::SEND_CONNECTION_CLOSE_PACKET));
    EXPECT_CALL(*connection_, SendControlFrame(_)).Times(0);
    session_->SendGoAway(QUIC_PEER_GOING_AWAY, "Going Away.");
...
TEST_P(QuicSpdySessionTestServer, SendHttp3GoAway)
{
    ...
    const QuicStreamId kTestStreamId = GetNthClientInitiatedBidirectionalStreamId(transport_version(), 0);
...
TEST_P(QuicSpdySessionTestServer, SendHttp3GoAwayAndNoMoreMaxStreams)
{
    ...
    const QuicStreamCount max_streams = QuicSessionPeer::ietf_streamid_manager(&*session_)->max_incoming_bidirectional_streams();
    for (QuicStreamCount i = 0; i < max_streams; ++i) {
        QuicStreamId stream_id = StreamCountToId(i + 1,
            Perspective::IS_CLIENT, // Client initates stream, allocs stream id.
            /*bidirectional=*/true);
        EXPECT_NE(nullptr, session_->GetOrCreateStream(stream_id));

        CloseStream(stream_id);
        QuicRstStreamFrame rst_frame(kInvalidControlFrameId, stream_id, QUIC_STREAM_CANCELLED,
            /* bytes_written = */ 0);
        session_->OnRstStream(rst_frame);
    }
    EXPECT_EQ(max_streams, QuicSessionPeer::ietf_streamid_manager(&*session_)->max_incoming_bidirectional_streams());
}
...
TEST_P(QuicSpdySessionTestServer, SendHttp3GoAwayWithoutEncryption)
{
    ...
    EXPECT_CALL(*connection_, CloseConnection(QUIC_PEER_GOING_AWAY, "Goaway", ConnectionCloseBehavior::SEND_CONNECTION_CLOSE_PACKET));
    session_->SendHttp3GoAway(QUIC_PEER_GOING_AWAY, "Goaway");
    EXPECT_FALSE(session_->goaway_sent());
}
...
TEST_P(QuicSpdySessionTestServer, InvalidGoAway)
{
    ...
    QuicGoAwayFrame go_away(kInvalidControlFrameId, QUIC_PEER_GOING_AWAY, session_->next_outgoing_bidirectional_stream_id(), "");
...
TEST_P(QuicSpdySessionTestServer, Http3GoAwayLargerIdThanBefore)
{
    ...
    EXPECT_CALL(*connection_, CloseConnection(QUIC_HTTP_GOAWAY_ID_LARGER_THAN_PREVIOUS, "GOAWAY received with ID 1 greater than previously received ID 0", _));
...
{
    Initialize();
    if (VersionHasIetfQuicFrames(transport_version()) || GetQuicReloadableFlag(quic_ignore_gquic_probing)) {
        return;
    }
    connection_->SetDefaultEncryptionLevel(ENCRYPTION_FORWARD_SECURE);
    QuicSocketAddress old_peer_address = QuicSocketAddress(QuicIpAddress::Loopback4(), kTestPort);
    EXPECT_EQ(old_peer_address, session_->peer_address());

    QuicSocketAddress new_peer_address = QuicSocketAddress(QuicIpAddress::Loopback4(), kTestPort + 1);

    EXPECT_CALL(*connection_, SendConnectivityProbingPacket(nullptr, new_peer_address));
...
{
    Initialize();
    EXPECT_EQ(kInitialIdleTimeoutSecs + 3, QuicConnectionPeer::GetNetworkTimeout(connection_).ToSeconds());
    CompleteHandshake();
    EXPECT_EQ(kMaximumIdleTimeoutSecs + 3, QuicConnectionPeer::GetNetworkTimeout(connection_).ToSeconds());
}
...
TEST_P(QuicSpdySessionTestServer, RstStreamBeforeHeadersDecompressed)
{
    ...
    QuicStreamFrame data1(GetNthClientInitiatedBidirectionalId(0), false, 0, absl::string_view("HT"));
    ...
    if (!VersionHasIetfQuicFrames(transport_version())) {
        ...
        EXPECT_CALL(*connection_, OnStreamReset(GetNthClientInitiatedBidirectionalId(0), _));
    }

    EXPECT_CALL(*connection_, SendControlFrame(_));
    QuicRstStreamFrame rst1(kInvalidControlFrameId, GetNthClientInitiatedBidirectionalId(0), QUIC_ERROR_PROCESSING_STREAM, 0);
    ...
    if (VersionHasIetfQuicFrames(transport_version())) {
        ...
        QuicStopSendingFrame stop_sending(kInvalidControlFrameId, GetNthClientInitiatedBidirectionalId(0), QUIC_ERROR_PROCESSING_STREAM);
        // Expect the RESET_STREAM that is generated in response to receiving a
        // STOP_SENDING.
        EXPECT_CALL(*connection_, OnStreamReset(GetNthClientInitiatedBidirectionalId(0), QUIC_ERROR_PROCESSING_STREAM));
...
TEST_P(QuicSpdySessionTestServer, OnStreamFrameFinStaticStreamId)
{
    ...
    EXPECT_CALL(
        *connection_, CloseConnection(QUIC_INVALID_STREAM_ID, "Attempt to close a static stream", ConnectionCloseBehavior::SEND_CONNECTION_CLOSE_PACKET));
    session_->OnStreamFrame(data1);
}
...
TEST_P(QuicSpdySessionTestServer, OnRstStreamStaticStreamId)
{
    ...
    // Send two bytes of payload.
    QuicRstStreamFrame rst1(kInvalidControlFrameId, id, QUIC_ERROR_PROCESSING_STREAM, 0);
    EXPECT_CALL(*connection_, CloseConnection(expected_error, error_message, ConnectionCloseBehavior::SEND_CONNECTION_CLOSE_PACKET));
    session_->OnRstStream(rst1);
}
...
TEST_P(QuicSpdySessionTestServer, OnStreamFrameInvalidStreamId)
{
    Initialize();
    // Send two bytes of payload.
    QuicStreamFrame data1(QuicUtils::GetInvalidStreamId(transport_version()), true, 0, absl::string_view("HT"));
    EXPECT_CALL(
        *connection_, CloseConnection(QUIC_INVALID_STREAM_ID, "Received data for an invalid stream", ConnectionCloseBehavior::SEND_CONNECTION_CLOSE_PACKET));
    session_->OnStreamFrame(data1);
}
...
TEST_P(QuicSpdySessionTestServer, OnRstStreamInvalidStreamId)
{
    Initialize();
    // Send two bytes of payload.
    QuicRstStreamFrame rst1(kInvalidControlFrameId, QuicUtils::GetInvalidStreamId(transport_version()), QUIC_ERROR_PROCESSING_STREAM, 0);
    EXPECT_CALL(
        *connection_, CloseConnection(QUIC_INVALID_STREAM_ID, "Received data for an invalid stream", ConnectionCloseBehavior::SEND_CONNECTION_CLOSE_PACKET));
    session_->OnRstStream(rst1);
}
...
TEST_P(QuicSpdySessionTestServer, HandshakeUnblocksFlowControlBlockedHeadersStream)
{
    Initialize();
    ...
    while (!headers_stream->IsFlowControlBlocked() && stream_id < 2000) {
        EXPECT_FALSE(session_->IsConnectionFlowControlBlocked());
        EXPECT_FALSE(session_->IsStreamFlowControlBlocked());
        headers["header"] = absl::StrCat(random.RandUint64(), random.RandUint64(), random.RandUint64());
        session_->WriteHeadersOnHeadersStream(stream_id, headers.Clone(), true, spdy::SpdyStreamPrecedence(0), nullptr);
        stream_id += IdDelta();
    }
    // Write once more to ensure that the headers stream has buffered data. The
    // random headers may have exactly filled the flow control window.
    session_->WriteHeadersOnHeadersStream(stream_id, std::move(headers), true, spdy::SpdyStreamPrecedence(0), nullptr);
    ...
    EXPECT_FALSE(session_->IsStreamFlowControlBlocked());
    EXPECT_TRUE(headers_stream->HasBufferedData());
    EXPECT_TRUE(QuicSessionPeer::IsStreamWriteBlocked(&*session_, QuicUtils::GetHeadersStreamId(transport_version())));
}
#endif // !defined(OS_IOS)

TEST_P(QuicSpdySessionTestServer, ConnectionFlowControlAccountingRstOutOfOrder)
{
    Initialize();
    ...
    QuicRstStreamFrame rst_frame(kInvalidControlFrameId, stream->id(), QUIC_STREAM_CANCELLED, kByteOffset);
    session_->OnRstStream(rst_frame);
    // Create and inject a STOP_SENDING frame. In GOOGLE QUIC, receiving a
    // RST_STREAM frame causes a two-way close. For IETF QUIC, RST_STREAM causes a
    // one-way close.
    if (VersionHasIetfQuicFrames(transport_version())) {
        // Only needed for version 99/IETF QUIC.
        QuicStopSendingFrame stop_sending(kInvalidControlFrameId, stream->id(), QUIC_STREAM_CANCELLED);
        // Expect the RESET_STREAM that is generated in response to receiving a
        // STOP_SENDING.
        EXPECT_CALL(*connection_, OnStreamReset(stream->id(), QUIC_STREAM_CANCELLED));
        EXPECT_CALL(*connection_, SendControlFrame(_));
        session_->OnStopSendingFrame(stop_sending);
    }
...
TEST_P(QuicSpdySessionTestServer, InvalidStreamFlowControlWindowInHandshake)
{
    ...
    QuicConfigPeer::SetReceivedInitialStreamFlowControlWindow(session_->config(), kInvalidWindow);

    EXPECT_CALL(*connection_, CloseConnection(QUIC_FLOW_CONTROL_INVALID_WINDOW, _, _));
    session_->OnConfigNegotiated();
}
...
TEST_P(QuicSpdySessionTestServer, TooLowUnidirectionalStreamLimitHttp3)
{
    ...
    EXPECT_CALL(*connection_, CloseConnection(_, "new unidirectional limit 2 decreases the current limit: 3", _));
    session_->OnConfigNegotiated();
}
...
TEST_P(QuicSpdySessionTestServer, CustomFlowControlWindow)
{
    ...
    connection_->SetDefaultEncryptionLevel(ENCRYPTION_FORWARD_SECURE);
    session_->OnConfigNegotiated();
    EXPECT_EQ(192 * 1024u, QuicFlowControllerPeer::ReceiveWindowSize(session_->flow_controller()));
}
...
TEST_P(QuicSpdySessionTestServer, WindowUpdateUnblocksHeadersStream)
{
    ...
    QuicWindowUpdateFrame window_update_frame(kInvalidControlFrameId, headers_stream->id(), 2 * kMinimumFlowControlSendWindow);
...
TEST_P(QuicSpdySessionTestServer, TooManyUnfinishedStreamsCauseServerRejectStream)
{
    Initialize();
    ...
    if (VersionHasIetfQuicFrames(transport_version())) {
        QuicSessionPeer::SetMaxOpenIncomingBidirectionalStreams(&*session_, kMaxStreams);
    } else {
    ...
    const QuicStreamId kFinalStreamId = GetNthClientInitiatedBidirectionalId(kMaxStreams);
    ...
    if (!VersionHasIetfQuicFrames(transport_version())) {
        ...
        EXPECT_CALL(*connection_, OnStreamReset(kFinalStreamId, QUIC_REFUSED_STREAM)).Times(1);
    } else {
        // On version 99 opening such a stream results in a connection close.
        EXPECT_CALL(*connection_, CloseConnection(QUIC_INVALID_STREAM_ID, testing::MatchesRegex("Stream id \\d+ would exceed stream count limit 5"), _));
    }
...
TEST_P(QuicSpdySessionTestServer, DrainingStreamsDoNotCountAsOpened)
{
    ...
    const QuicStreamId kFirstStreamId = GetNthClientInitiatedBidirectionalId(0);
    const QuicStreamId kFinalStreamId = GetNthClientInitiatedBidirectionalId(kMaxStreams + 1);
...
INSTANTIATE_TEST_SUITE_P(Tests, QuicSpdySessionTestClient, ::testing::ValuesIn(AllSupportedVersions()), ::testing::PrintToStringParamName());
...
TEST_P(QuicSpdySessionTestClient, UsesPendingStreamsForFrame)
{
    ...
    EXPECT_TRUE(session_->UsesPendingStreamForFrame(STREAM_FRAME, QuicUtils::GetFirstUnidirectionalStreamId(transport_version(), Perspective::IS_SERVER)));
    EXPECT_TRUE(session_->UsesPendingStreamForFrame(RST_STREAM_FRAME, QuicUtils::GetFirstUnidirectionalStreamId(transport_version(), Perspective::IS_SERVER)));
    EXPECT_FALSE(session_->UsesPendingStreamForFrame(RST_STREAM_FRAME, QuicUtils::GetFirstUnidirectionalStreamId(transport_version(), Perspective::IS_CLIENT)));
    EXPECT_FALSE(
        session_->UsesPendingStreamForFrame(STOP_SENDING_FRAME, QuicUtils::GetFirstUnidirectionalStreamId(transport_version(), Perspective::IS_SERVER)));
    EXPECT_FALSE(session_->UsesPendingStreamForFrame(RST_STREAM_FRAME, QuicUtils::GetFirstBidirectionalStreamId(transport_version(), Perspective::IS_SERVER)));
}
...
TEST_P(QuicSpdySessionTestClient, BadStreamFramePendingStream)
{
    ...
    CompleteHandshake();
    EXPECT_EQ(0u, QuicSessionPeer::GetNumOpenDynamicStreams(&*session_));
    QuicStreamId stream_id1 = GetNthServerInitiatedUnidirectionalStreamId(transport_version(), 0);
...
TEST_P(QuicSpdySessionTestClient, PendingStreamKeepsConnectionAlive)
{
    ...
    CompleteHandshake();
    QuicStreamId stream_id = QuicUtils::GetFirstUnidirectionalStreamId(transport_version(), Perspective::IS_SERVER);
...
{
    Initialize();
    ASSERT_TRUE(session_->GetOrCreateStream(GetNthServerInitiatedBidirectionalId(2)) != nullptr);
    // Both server initiated streams with smaller stream IDs should be available.
    EXPECT_TRUE(QuicSessionPeer::IsStreamAvailable(&*session_, GetNthServerInitiatedBidirectionalId(0)));
    EXPECT_TRUE(QuicSessionPeer::IsStreamAvailable(&*session_, GetNthServerInitiatedBidirectionalId(1)));
    ASSERT_TRUE(session_->GetOrCreateStream(GetNthServerInitiatedBidirectionalId(0)) != nullptr);
    ASSERT_TRUE(session_->GetOrCreateStream(GetNthServerInitiatedBidirectionalId(1)) != nullptr);
    // And client initiated stream ID should be not available.
    EXPECT_FALSE(QuicSessionPeer::IsStreamAvailable(&*session_, GetNthClientInitiatedBidirectionalId(0)));
}
...
TEST_P(QuicSpdySessionTestClient, TooLargeHeadersMustNotCauseWriteAfterReset)
{
    ...
    QuicHeaderList headers;
    EXPECT_CALL(*connection_, SendControlFrame(_));
    EXPECT_CALL(*connection_, OnStreamReset(stream->id(), QUIC_HEADERS_TOO_LARGE));
...
TEST_P(QuicSpdySessionTestClient, RecordFinAfterReadSideClosed)
{
    ...
    EXPECT_EQ(0u, QuicSessionPeer::GetLocallyClosedStreamsHighestOffset(&*session_).size());
}
...
TEST_P(QuicSpdySessionTestClient, WritePriority)
{
    ...
    session_->WritePriority(id, parent_stream_id, Spdy3PriorityToHttp2Weight(priority), exclusive);

    QuicStreamSendBuffer& send_buffer = QuicStreamPeer::SendBuffer(headers_stream);
    ASSERT_EQ(1u, send_buffer.size());

    SpdyPriorityIR priority_frame(id, parent_stream_id, Spdy3PriorityToHttp2Weight(priority), exclusive);
    SpdyFramer spdy_framer(SpdyFramer::ENABLE_COMPRESSION);
    SpdySerializedFrame frame = spdy_framer.SerializeFrame(priority_frame);

    const quiche::QuicheMemSlice& slice = QuicStreamSendBufferPeer::CurrentWriteSlice(&send_buffer)->slice;
    EXPECT_EQ(absl::string_view(frame.data(), frame.size()), absl::string_view(slice.data(), slice.length()));
}
...
TEST_P(QuicSpdySessionTestClient, Http3ServerPush)
{
    ...
    QuicStreamId stream_id1 = GetNthServerInitiatedUnidirectionalStreamId(transport_version(), 0);
    EXPECT_CALL(*connection_, CloseConnection(QUIC_HTTP_RECEIVE_SERVER_PUSH, _, _)).Times(1);
...
TEST_P(QuicSpdySessionTestClient, Http3ServerPushOutofOrderFrame)
{
    ...
    QuicStreamId stream_id = GetNthServerInitiatedUnidirectionalStreamId(transport_version(), 0);

    QuicStreamFrame data1(stream_id,
        /* fin = */ false, /* offset = */ 0, frame_type);
    QuicStreamFrame data2(stream_id,
        /* fin = */ false, /* offset = */ frame_type.size(), push_id);
...
TEST_P(QuicSpdySessionTestClient, ServerDisableQpackDynamicTable)
{
    ...
    QuicStreamId stream_id = GetNthServerInitiatedUnidirectionalStreamId(transport_version(), 3);
    char type[] = { kControlStream };
    QuicStreamFrame data1(stream_id, false, 0, absl::string_view(type, 1));
    session_->OnStreamFrame(data1);
    EXPECT_EQ(stream_id, QuicSpdySessionPeer::GetReceiveControlStream(&*session_)->id());
    ...
    QpackEncoderHeaderTable* encoder_header_table = QpackEncoderPeer::header_table(qpack_encoder);
    ...
    EXPECT_EQ(kDefaultQpackMaxDynamicTableCapacity, outgoing_settings.values[SETTINGS_QPACK_MAX_TABLE_CAPACITY]);
}
...
TEST_P(QuicSpdySessionTestServer, OnStreamFrameLost)
{
    ...
    if (!QuicVersionUsesCryptoFrames(transport_version())) {
        EXPECT_CALL(*crypto_stream, HasPendingRetransmission()).WillOnce(Return(true));
    }
    EXPECT_CALL(*stream2, HasPendingRetransmission()).WillOnce(Return(true));
    session_->OnFrameLost(QuicFrame(frame3));
    if (!QuicVersionUsesCryptoFrames(transport_version())) {
        QuicStreamFrame frame1(QuicUtils::GetCryptoStreamId(transport_version()), false, 0, 1300);
    ...
    if (!QuicVersionUsesCryptoFrames(transport_version())) {
        EXPECT_CALL(*crypto_stream, OnCanWrite());
        EXPECT_CALL(*crypto_stream, HasPendingRetransmission()).WillOnce(Return(false));
    }
...
TEST_P(QuicSpdySessionTestServer, DonotRetransmitDataOfClosedStreams)
{
    ...
    if (VersionUsesHttp3(transport_version())) {
        session_->qpack_decoder()->set_qpack_stream_sender_delegate(&qpack_stream_sender_delegate);
    }
...
TEST_P(QuicSpdySessionTestServer, RetransmitFrames)
{
    ...
    EXPECT_CALL(*stream2, RetransmitStreamData(_, _, _, _)).WillOnce(Return(true));
    EXPECT_CALL(*connection_, SendControlFrame(_)).WillOnce(Invoke(&ClearControlFrame));
    EXPECT_CALL(*stream4, RetransmitStreamData(_, _, _, _)).WillOnce(Return(true));
    EXPECT_CALL(*stream6, RetransmitStreamData(_, _, _, _)).WillOnce(Return(true));
    EXPECT_CALL(*send_algorithm, OnApplicationLimited(_));
...
TEST_P(QuicSpdySessionTestServer, OnPriorityFrame)
{
    ...
    session_->OnPriorityFrame(stream_id, spdy::SpdyStreamPrecedence(kV3HighestPriority));

    EXPECT_EQ((QuicStreamPriority(HttpStreamPriority { kV3HighestPriority, HttpStreamPriority::kDefaultIncremental })), stream->priority());
}
...
TEST_P(QuicSpdySessionTestServer, OnPriorityUpdateFrame)
{
    ...
    QuicStreamId receive_control_stream_id = GetNthClientInitiatedUnidirectionalStreamId(transport_version(), 3);
    char type[] = { kControlStream };
    ...
    EXPECT_CALL(debug_visitor, OnPeerControlStreamCreated(receive_control_stream_id));
    session_->OnStreamFrame(data1);
    EXPECT_EQ(receive_control_stream_id, QuicSpdySessionPeer::GetReceiveControlStream(&*session_)->id());

    // Send SETTINGS frame.
    std::string serialized_settings = HttpEncoder::SerializeSettingsFrame({});
    QuicStreamFrame data2(receive_control_stream_id, false, offset, serialized_settings);
    ...
    std::string serialized_priority_update1 = HttpEncoder::SerializePriorityUpdateFrame(priority_update1);
    ...
    EXPECT_EQ(QuicStreamPriority(HttpStreamPriority { HttpStreamPriority::kDefaultUrgency, HttpStreamPriority::kDefaultIncremental }), stream1->priority());
    EXPECT_CALL(debug_visitor, OnPriorityUpdateFrameReceived(priority_update1));
    session_->OnStreamFrame(data3);
    EXPECT_EQ(QuicStreamPriority(HttpStreamPriority { 2u, HttpStreamPriority::kDefaultIncremental }), stream1->priority());
    ...
    std::string serialized_priority_update2 = HttpEncoder::SerializePriorityUpdateFrame(priority_update2);
    QuicStreamFrame stream_frame3(receive_control_stream_id,
        /* fin = */ false, offset, serialized_priority_update2);
    ...
    EXPECT_EQ(QuicStreamPriority(HttpStreamPriority { 5u, true }), stream2->priority());
}
...
TEST_P(QuicSpdySessionTestServer, OnInvalidPriorityUpdateFrame)
{
    ...
    EXPECT_CALL(*connection_, CloseConnection(QUIC_INVALID_PRIORITY_UPDATE, "Invalid PRIORITY_UPDATE frame payload.", _));

    std::string serialized_priority_update = HttpEncoder::SerializePriorityUpdateFrame(priority_update);
    QuicStreamFrame data3(receive_control_stream_id,
...
TEST_P(QuicSpdySessionTestServer, SimplePendingStreamType)
{
    ...
    QuicStreamId stream_id = QuicUtils::GetFirstUnidirectionalStreamId(transport_version(), Perspective::IS_CLIENT);
    ...
    for (bool fin : { true, false }) {
        ...
        EXPECT_CALL(*connection_, SendControlFrame(_)).WillOnce(Invoke([stream_id](const QuicFrame& frame) {
            EXPECT_EQ(STOP_SENDING_FRAME, frame.type);
            ...
            EXPECT_EQ(static_cast<uint64_t>(QuicHttp3ErrorCode::STREAM_CREATION_ERROR), stop_sending.ietf_error_code);

            return ClearControlFrame(frame);
        }));
        session_->OnStreamFrame(frame);

        PendingStream* pending = QuicSessionPeer::GetPendingStream(&*session_, stream_id);
        if (fin) {
...
TEST_P(QuicSpdySessionTestServer, SimplePendingStreamTypeOutOfOrderDelivery)
{
    ...
    for (bool fin : { true, false }) {
        QuicStreamFrame frame1(stream_id, /* fin = */ false, /* offset = */ 0, payload.substr(0, 1));
        QuicStreamFrame frame2(stream_id, fin, /* offset = */ 1, payload.substr(1));

        // Deliver frames out of order.
        session_->OnStreamFrame(frame2);
        // A STOP_SENDING frame is sent in response to the unknown stream type.
        EXPECT_CALL(*connection_, SendControlFrame(_)).WillOnce(Invoke(&VerifyAndClearStopSendingFrame));
...
TEST_P(QuicSpdySessionTestServer, MultipleBytesPendingStreamTypeOutOfOrderDelivery)
{
    Initialize();
    if (!VersionUsesHttp3(transport_version())) {
        return;
    ...
    for (bool fin : { true, false }) {
        QuicStreamFrame frame1(stream_id, /* fin = */ false, /* offset = */ 0, payload.substr(0, 1));
        QuicStreamFrame frame2(stream_id, /* fin = */ false, /* offset = */ 1, payload.substr(1, 1));
...
TEST_P(QuicSpdySessionTestServer, ReceiveControlStream)
{
    ...
    QuicStreamId stream_id = GetNthClientInitiatedUnidirectionalStreamId(transport_version(), 3);
    char type[] = { kControlStream };
...
TEST_P(QuicSpdySessionTestServer, StreamClosedWhileHeaderDecodingBlocked)
{
    ...
    std::string headers_frame_header = HttpEncoder::SerializeHeadersFrameHeader(headers_frame_payload.length());
    std::string headers_frame = absl::StrCat(headers_frame_header, headers_frame_payload);
...
TEST_P(QuicSpdySessionTestClient, ResetAfterInvalidIncomingStreamType)
{
    ...
    const QuicStreamId stream_id = GetNthServerInitiatedUnidirectionalStreamId(transport_version(), 0);
    ...
    QuicStreamFrame frame(stream_id, /* fin = */ false, /* offset = */ 0, payload);
...
TEST_P(QuicSpdySessionTestClient, DuplicateHttp3UnidirectionalStreams)
{
    ...
    QuicStreamId id1 = GetNthServerInitiatedUnidirectionalStreamId(transport_version(), 0);
    ...
    session_->OnStreamFrame(data1);
    QuicStreamId id2 = GetNthServerInitiatedUnidirectionalStreamId(transport_version(), 1);
    ...
    EXPECT_QUIC_PEER_BUG(
        {
            EXPECT_CALL(*connection_, CloseConnection(QUIC_HTTP_DUPLICATE_UNIDIRECTIONAL_STREAM, "Control stream is received twice.", _));
            session_->OnStreamFrame(data2);
        },
        "Received a duplicate Control stream: Closing connection.");

    QuicStreamId id3 = GetNthServerInitiatedUnidirectionalStreamId(transport_version(), 2);
    ...
    QuicStreamId id4 = GetNthServerInitiatedUnidirectionalStreamId(transport_version(), 3);
    ...
    EXPECT_QUIC_PEER_BUG(
        {
            EXPECT_CALL(*connection_, CloseConnection(QUIC_HTTP_DUPLICATE_UNIDIRECTIONAL_STREAM, "QPACK encoder stream is received twice.", _));
            session_->OnStreamFrame(data4);
        },
        "Received a duplicate QPACK encoder stream: Closing connection.");

    QuicStreamId id5 = GetNthServerInitiatedUnidirectionalStreamId(transport_version(), 4);
    ...
    QuicStreamId id6 = GetNthServerInitiatedUnidirectionalStreamId(transport_version(), 5);
    ...
    EXPECT_QUIC_PEER_BUG(
        {
            EXPECT_CALL(*connection_, CloseConnection(QUIC_HTTP_DUPLICATE_UNIDIRECTIONAL_STREAM, "QPACK decoder stream is received twice.", _));
...
TEST_P(QuicSpdySessionTestClient, EncoderStreamError)
{
    ...
    CompleteHandshake();
    std::string data;
    ASSERT_TRUE(absl::HexStringToBytes("02" // Encoder stream.
    ...
    EXPECT_CALL(*connection_, CloseConnection(QUIC_QPACK_ENCODER_STREAM_DUPLICATE_INVALID_RELATIVE_INDEX, "Encoder stream error: Invalid relative index.", _));
    session_->OnStreamFrame(frame);
}
...
TEST_P(QuicSpdySessionTestClient, DecoderStreamError)
{
    ...
    CompleteHandshake();
    std::string data;
    ASSERT_TRUE(absl::HexStringToBytes("03" // Decoder stream.
    ...
    EXPECT_CALL(*connection_, CloseConnection(QUIC_QPACK_DECODER_STREAM_INVALID_ZERO_INCREMENT, "Decoder stream error: Invalid increment value 0.", _));
    session_->OnStreamFrame(frame);
}
...
TEST_P(QuicSpdySessionTestClient, InvalidHttp3GoAway)
{
    ...
    EXPECT_CALL(*connection_, CloseConnection(QUIC_HTTP_GOAWAY_INVALID_STREAM_ID, "GOAWAY with invalid stream ID", _));
    QuicStreamId stream_id = GetNthServerInitiatedUnidirectionalStreamId(transport_version(), 0);
...
TEST_P(QuicSpdySessionTestClient, Http3GoAwayLargerIdThanBefore)
{
    ...
    EXPECT_FALSE(session_->goaway_received());
    QuicStreamId stream_id1 = GetNthClientInitiatedBidirectionalStreamId(transport_version(), 0);
    session_->OnHttp3GoAway(stream_id1);
    EXPECT_TRUE(session_->goaway_received());

    EXPECT_CALL(*connection_, CloseConnection(QUIC_HTTP_GOAWAY_ID_LARGER_THAN_PREVIOUS, "GOAWAY received with ID 4 greater than previously received ID 0", _));
    QuicStreamId stream_id2 = GetNthClientInitiatedBidirectionalStreamId(transport_version(), 1);
...
TEST_P(QuicSpdySessionTestClient, CloseConnectionOnCancelPush)
{
    ...
    // Create control stream.
    QuicStreamId receive_control_stream_id = GetNthServerInitiatedUnidirectionalStreamId(transport_version(), 3);
    char type[] = { kControlStream };
    absl::string_view stream_type(type, 1);
    QuicStreamOffset offset = 0;
    QuicStreamFrame data1(receive_control_stream_id, /* fin = */ false, offset, stream_type);
    offset += stream_type.length();
    EXPECT_CALL(debug_visitor, OnPeerControlStreamCreated(receive_control_stream_id));
    session_->OnStreamFrame(data1);
    EXPECT_EQ(receive_control_stream_id, QuicSpdySessionPeer::GetReceiveControlStream(&*session_)->id());
    ...
    std::string serialized_settings = HttpEncoder::SerializeSettingsFrame({});
    QuicStreamFrame data2(receive_control_stream_id, /* fin = */ false, offset, serialized_settings);
    offset += serialized_settings.length();
    EXPECT_CALL(debug_visitor, OnSettingsFrameReceived(_));
    session_->OnStreamFrame(data2);

    std::string cancel_push_frame;
    ASSERT_TRUE(absl::HexStringToBytes("03" // CANCEL_PUSH
                                       "01" // length
                                       "00", // push ID
        &cancel_push_frame));
    QuicStreamFrame data3(receive_control_stream_id, /* fin = */ false, offset, cancel_push_frame);
    EXPECT_CALL(*connection_, CloseConnection(QUIC_HTTP_FRAME_ERROR, "CANCEL_PUSH frame received.", _))
        .WillOnce(Invoke(connection_, &MockQuicConnection::ReallyCloseConnection));
    EXPECT_CALL(*connection_, SendConnectionClosePacket(QUIC_HTTP_FRAME_ERROR, _, "CANCEL_PUSH frame received."));
    session_->OnStreamFrame(data3);
}
...
TEST_P(QuicSpdySessionTestServer, OnSetting)
{
    ...
    CompleteHandshake();
    if (VersionUsesHttp3(transport_version())) {
        EXPECT_EQ(std::numeric_limits<size_t>::max(), session_->max_outbound_header_list_size());
    ...
    spdy::HpackEncoder* hpack_encoder = QuicSpdySessionPeer::GetSpdyFramer(&*session_)->GetHpackEncoder();
...
TEST_P(QuicSpdySessionTestServer, FineGrainedHpackErrorCodes)
{
    ...
    ASSERT_TRUE(absl::HexStringToBytes("000006" // length
                                       "01" // type
        ...
        &headers_frame));
    QuicStreamId headers_stream_id = QuicUtils::GetHeadersStreamId(transport_version());
    QuicStreamFrame data(headers_stream_id, false, 0, headers_frame);

    EXPECT_CALL(*connection_,
        CloseConnection(QUIC_HPACK_INVALID_INDEX, "SPDY framing error: HPACK_INVALID_INDEX", ConnectionCloseBehavior::SEND_CONNECTION_CLOSE_PACKET));
...
TEST_P(QuicSpdySessionTestServer, PeerClosesCriticalReceiveStream)
{
    ...
    for (size_t i = 0; i < ABSL_ARRAYSIZE(kTestData); ++i) {
        QuicStreamId stream_id = GetNthClientInitiatedUnidirectionalStreamId(transport_version(), i + 1);
        const QuicByteCount data_length = 1;
        QuicStreamFrame data(stream_id, false, 0, absl::string_view(&kTestData[i].type, data_length));
        session_->OnStreamFrame(data);

        EXPECT_CALL(*connection_, CloseConnection(QUIC_HTTP_CLOSED_CRITICAL_STREAM, kTestData[i].error_details, _));

        QuicRstStreamFrame rst(kInvalidControlFrameId, stream_id, QUIC_STREAM_CANCELLED, data_length);
        session_->OnRstStream(rst);
    }
}

TEST_P(QuicSpdySessionTestServer, H3ControlStreamsLimitedByConnectionFlowControl)
{
    Initialize();
    if (!VersionUsesHttp3(transport_version())) {
        return;
    ...
    QuicSendControlStream* send_control_stream = QuicSpdySessionPeer::GetSendControlStream(&*session_);
...
TEST_P(QuicSpdySessionTestServer, PeerClosesCriticalSendStream)
{
    ...
    QuicSendControlStream* control_stream = QuicSpdySessionPeer::GetSendControlStream(&*session_);
    ASSERT_TRUE(control_stream);

    QuicStopSendingFrame stop_sending_control_stream(kInvalidControlFrameId, control_stream->id(), QUIC_STREAM_CANCELLED);
    EXPECT_CALL(*connection_, CloseConnection(QUIC_HTTP_CLOSED_CRITICAL_STREAM, "STOP_SENDING received for send control stream", _));
    session_->OnStopSendingFrame(stop_sending_control_stream);

    QpackSendStream* decoder_stream = QuicSpdySessionPeer::GetQpackDecoderSendStream(&*session_);
    ASSERT_TRUE(decoder_stream);

    QuicStopSendingFrame stop_sending_decoder_stream(kInvalidControlFrameId, decoder_stream->id(), QUIC_STREAM_CANCELLED);
    EXPECT_CALL(*connection_, CloseConnection(QUIC_HTTP_CLOSED_CRITICAL_STREAM, "STOP_SENDING received for QPACK send stream", _));
    session_->OnStopSendingFrame(stop_sending_decoder_stream);

    QpackSendStream* encoder_stream = QuicSpdySessionPeer::GetQpackEncoderSendStream(&*session_);
    ASSERT_TRUE(encoder_stream);

    QuicStopSendingFrame stop_sending_encoder_stream(kInvalidControlFrameId, encoder_stream->id(), QUIC_STREAM_CANCELLED);
    EXPECT_CALL(*connection_, CloseConnection(QUIC_HTTP_CLOSED_CRITICAL_STREAM, "STOP_SENDING received for QPACK send stream", _));
...
TEST_P(QuicSpdySessionTestServer, Http3GoAwayWhenClosingConnection)
{
    ...
    std::string headers_frame_header = HttpEncoder::SerializeHeadersFrameHeader(headers_payload_length);
    EXPECT_CALL(debug_visitor, OnHeadersFrameReceived(stream_id, headers_payload_length));
    session_->OnStreamFrame(QuicStreamFrame(stream_id, false, 0, headers_frame_header));

    EXPECT_EQ(stream_id, QuicSessionPeer::GetLargestPeerCreatedStreamId(&*session_, /*unidirectional = */ false));
    ...
    EXPECT_CALL(debug_visitor, OnGoAwayFrameSent(stream_id + QuicUtils::StreamIdDelta(transport_version())));

    // Close connection.
    EXPECT_CALL(*writer_, WritePacket(_, _, _, _, _, _)).WillRepeatedly(Return(WriteResult(WRITE_STATUS_OK, 0)));
    EXPECT_CALL(*connection_, CloseConnection(QUIC_NO_ERROR, _, _)).WillOnce(Invoke(connection_, &MockQuicConnection::ReallyCloseConnection));
    EXPECT_CALL(*connection_, SendConnectionClosePacket(QUIC_NO_ERROR, _, _))
        .WillOnce(Invoke(connection_, &MockQuicConnection::ReallySendConnectionClosePacket));
    connection_->CloseConnection(QUIC_NO_ERROR, "closing connection", ConnectionCloseBehavior::SEND_CONNECTION_CLOSE_PACKET);
}
...
TEST_P(QuicSpdySessionTestClient, ReceiveSpdySettingInHttp3)
{
    ...
    EXPECT_CALL(*connection_, CloseConnection(QUIC_HTTP_RECEIVE_SPDY_SETTING, _, _));
...
TEST_P(QuicSpdySessionTestClient, ReceiveAcceptChFrame)
{
    ...
    QuicStreamFrame data3(receive_control_stream_id, /* fin = */ false, offset, accept_ch_frame);
...
TEST_P(QuicSpdySessionTestClient, AcceptChViaAlps)
{
    ...
    ASSERT_TRUE(absl::HexStringToBytes("4089" // type (ACCEPT_CH)
                                       "08" // length
                                       "03" // length of origin
                                       "666f6f" // origin "foo"
                                       "03" // length of value
    ...
    EXPECT_CALL(debug_visitor, OnAcceptChFrameReceivedViaAlps(expected_accept_ch_frame));

    auto error = session_->OnAlpsData(reinterpret_cast<const uint8_t*>(serialized_accept_ch_frame.data()), serialized_accept_ch_frame.size());
    EXPECT_FALSE(error);
}
...
TEST_P(QuicSpdySessionTestClient, AlpsForbiddenFrame)
{
    ...
    ASSERT_TRUE(absl::HexStringToBytes("00" // type (DATA)
                                       "03" // length
                                       "66666f", // "foo"
        &forbidden_frame));

    auto error = session_->OnAlpsData(reinterpret_cast<const uint8_t*>(forbidden_frame.data()), forbidden_frame.size());
    ASSERT_TRUE(error);
...
TEST_P(QuicSpdySessionTestClient, AlpsIncompleteFrame)
{
    ...
    ASSERT_TRUE(absl::HexStringToBytes("04" // type (SETTINGS)
                                       "03", // non-zero length but empty payload
        &incomplete_frame));

    auto error = session_->OnAlpsData(reinterpret_cast<const uint8_t*>(incomplete_frame.data()), incomplete_frame.size());
    ASSERT_TRUE(error);
...
TEST_P(QuicSpdySessionTestClient, SettingsViaAlpsThenOnControlStream)
{
    ...
    SettingsFrame expected_settings_frame1 { { { SETTINGS_QPACK_MAX_TABLE_CAPACITY, 1024 }, { SETTINGS_QPACK_BLOCKED_STREAMS, 32 } } };
    EXPECT_CALL(debug_visitor, OnSettingsFrameReceivedViaAlps(expected_settings_frame1));

    auto error = session_->OnAlpsData(reinterpret_cast<const uint8_t*>(serialized_settings_frame1.data()), serialized_settings_frame1.size());
    EXPECT_FALSE(error);

    EXPECT_EQ(1024u, qpack_encoder->MaximumDynamicTableCapacity());
    EXPECT_EQ(32u, qpack_encoder->maximum_blocked_streams());

    const QuicStreamId control_stream_id = GetNthServerInitiatedUnidirectionalStreamId(transport_version(), 0);
    ...
    // increasing is okay.
    SettingsFrame expected_settings_frame2 { { { SETTINGS_QPACK_MAX_TABLE_CAPACITY, 1024 }, { SETTINGS_QPACK_BLOCKED_STREAMS, 48 } } };
    ...
    session_->OnStreamFrame(QuicStreamFrame(control_stream_id, /* fin = */ false,
        /* offset = */ stream_type.length(), serialized_settings_frame2));
...
TEST_P(QuicSpdySessionTestClient, SettingsViaAlpsConflictsSettingsViaControlStream)
{
    Initialize();
    if (!VersionUsesHttp3(transport_version())) {
        return;
...
TEST_P(QuicSpdySessionTestClient, AlpsTwoSettingsFrame)
{
    ...
    auto error = session_->OnAlpsData(reinterpret_cast<const uint8_t*>(banned_frame.data()), banned_frame.size());
    ASSERT_TRUE(error);
...
TEST_P(QuicSpdySessionTestClient, HttpDatagramSettingLocal04And09Remote04And09)
{
    Initialize();
    TestHttpDatagramSetting(
        /*local_support=*/HttpDatagramSupport::kRfcAndDraft04,
...
TEST_P(QuicSpdySessionTestClient, WebTransportSettingDraft02OnlyBothSides)
{
    ...
    session_->set_locally_supported_web_transport_versions(WebTransportHttp3VersionSet({ WebTransportHttp3Version::kDraft02 }));

    EXPECT_FALSE(session_->SupportsWebTransport());
    CompleteHandshake();
    ReceiveWebTransportSettings(WebTransportHttp3VersionSet({ WebTransportHttp3Version::kDraft02 }));
    EXPECT_TRUE(session_->ShouldProcessIncomingRequests());
    EXPECT_TRUE(session_->SupportsWebTransport());
    EXPECT_EQ(session_->SupportedWebTransportVersion(), WebTransportHttp3Version::kDraft02);
}
...
TEST_P(QuicSpdySessionTestClient, WebTransportSettingDraft07OnlyBothSides)
{
    ...
    session_->set_local_http_datagram_support(HttpDatagramSupport::kRfcAndDraft04);
    session_->set_locally_supported_web_transport_versions(WebTransportHttp3VersionSet({ WebTransportHttp3Version::kDraft07 }));

    EXPECT_FALSE(session_->SupportsWebTransport());
    CompleteHandshake();
    ReceiveWebTransportSettings(WebTransportHttp3VersionSet({ WebTransportHttp3Version::kDraft07 }));
    EXPECT_TRUE(session_->ShouldProcessIncomingRequests());
    EXPECT_TRUE(session_->SupportsWebTransport());
    EXPECT_EQ(session_->SupportedWebTransportVersion(), WebTransportHttp3Version::kDraft07);
}
...
TEST_P(QuicSpdySessionTestClient, WebTransportSettingBothDraftsBothSides)
{
    ...
    session_->set_locally_supported_web_transport_versions(
        WebTransportHttp3VersionSet({ WebTransportHttp3Version::kDraft02, WebTransportHttp3Version::kDraft07 }));

    EXPECT_FALSE(session_->SupportsWebTransport());
    CompleteHandshake();
    ReceiveWebTransportSettings(WebTransportHttp3VersionSet({ WebTransportHttp3Version::kDraft02, WebTransportHttp3Version::kDraft07 }));
    EXPECT_TRUE(session_->ShouldProcessIncomingRequests());
    EXPECT_TRUE(session_->SupportsWebTransport());
    EXPECT_EQ(session_->SupportedWebTransportVersion(), WebTransportHttp3Version::kDraft07);
}
...
TEST_P(QuicSpdySessionTestClient, WebTransportSettingSetToZero)
{
    ...
    std::string data = std::string(1, kControlStream) + HttpEncoder::SerializeSettingsFrame(server_settings);
    QuicStreamId stream_id = GetNthServerInitiatedUnidirectionalStreamId(transport_version(), 3);
    QuicStreamFrame frame(stream_id, /*fin=*/false, /*offset=*/0, data);
    EXPECT_CALL(debug_visitor, OnPeerControlStreamCreated(stream_id));
...
TEST_P(QuicSpdySessionTestServer, BufferingIncomingStreams)
{
    ...
    CompleteHandshake();
    QuicStreamId session_id = GetNthClientInitiatedBidirectionalStreamId(transport_version(), 1);

    QuicStreamId data_stream_id = GetNthClientInitiatedUnidirectionalStreamId(transport_version(), 4);
    ...
    EXPECT_CALL(*connection_, OnStreamReset(data_stream_id, QUIC_STREAM_WEBTRANSPORT_SESSION_GONE));
...
TEST_P(QuicSpdySessionTestServer, BufferingIncomingStreamsLimit)
{
    ...
    EXPECT_CALL(*connection_, SendControlFrame(_)).WillRepeatedly(Invoke(&ClearControlFrame));
    EXPECT_CALL(*connection_, OnStreamReset(_, QUIC_STREAM_WEBTRANSPORT_BUFFERED_STREAMS_LIMIT_EXCEEDED)).Times(4);
    for (int i = 0; i < streams_to_send; i++) {
        QuicStreamId data_stream_id = GetNthClientInitiatedUnidirectionalStreamId(transport_version(), 4 + i);
        ReceiveWebTransportUnidirectionalStream(session_id, data_stream_id);
    }
    ...
    EXPECT_EQ(web_transport->NumberOfAssociatedStreams(), kMaxUnassociatedWebTransportStreams);

    EXPECT_CALL(*connection_, SendControlFrame(_)).WillRepeatedly(Invoke(&ClearControlFrame));
    EXPECT_CALL(*connection_, OnStreamReset(_, _)).Times(kMaxUnassociatedWebTransportStreams + 1);
    session_->ResetStream(session_id, QUIC_STREAM_INTERNAL_ERROR);
}
...
TEST_P(QuicSpdySessionTestServer, BufferingIncomingStreamsWithFin)
{
    ...
    const UberQuicStreamIdManager& stream_id_manager = *QuicSessionPeer::ietf_streamid_manager(&*session_);
    const QuicStreamId initial_advertized_max_streams = stream_id_manager.advertised_max_incoming_unidirectional_streams();
    const size_t num_streams_to_open = session_->max_open_incoming_unidirectional_streams();
    ...
    for (size_t i = 0; i < num_streams_to_open; i++) {
        const QuicStreamId stream_id = GetNthClientInitiatedUnidirectionalStreamId(transport_version(), 4 + i);
        QuicStreamFrame frame(stream_id, /*fin=*/true, /*offset=*/0, /*data=*/"");
        session_->OnStreamFrame(frame);
    }
    EXPECT_LT(initial_advertized_max_streams, stream_id_manager.advertised_max_incoming_unidirectional_streams());
...
TEST_P(QuicSpdySessionTestServer, ResetOutgoingWebTransportStreams)
{
    ...
    WebTransportStream* stream = web_transport->OpenOutgoingUnidirectionalStream();
    EXPECT_EQ(web_transport->NumberOfAssociatedStreams(), 1u);
    ...
    EXPECT_CALL(*connection_, SendControlFrame(_)).WillRepeatedly(Invoke(&ClearControlFrame));
    EXPECT_CALL(*connection_, OnStreamReset(session_id, _));
    EXPECT_CALL(*connection_, OnStreamReset(stream_id, QUIC_STREAM_WEBTRANSPORT_SESSION_GONE));
    session_->ResetStream(session_id, QUIC_STREAM_INTERNAL_ERROR);
    EXPECT_EQ(web_transport->NumberOfAssociatedStreams(), 0u);
}
...
TEST_P(QuicSpdySessionTestClient, WebTransportWithoutExtendedConnect)
{
    ...
    std::string data = std::string(1, kControlStream) + HttpEncoder::SerializeSettingsFrame(settings);
    QuicStreamId control_stream_id = session_->perspective() == Perspective::IS_SERVER ? GetNthClientInitiatedUnidirectionalStreamId(transport_version(), 3)
                                                                                       : GetNthServerInitiatedUnidirectionalStreamId(transport_version(), 3);
    QuicStreamFrame frame(control_stream_id, /*fin=*/false, /*offset=*/0, data);
    session_->OnStreamFrame(frame);
...
TEST_P(QuicSpdySessionTestClient, LimitEncoderDynamicTableSize)
{
    ...
    QuicSpdySessionPeer::SetHeadersStream(&*session_, nullptr);
    TestHeadersStream* headers_stream = new StrictMock<TestHeadersStream>(&*session_);
    QuicSpdySessionPeer::SetHeadersStream(&*session_, headers_stream);
    ...
    ASSERT_TRUE(absl::HexStringToBytes("000009" // frame length
    ...
    ASSERT_TRUE(absl::HexStringToBytes("00000000" // stream dependency
                                       "92", // stream weight
    ...
    ASSERT_TRUE(absl::HexStringToBytes("3fe17f" // Dynamic Table Size Update to 16384
                                       "82", // Indexed Header Field Representation with index 2
        &expected_stream_data_3));
    EXPECT_EQ(expected_stream_data_3, stream_data);
}

class QuicSpdySessionTestServerNoExtendedConnect : public QuicSpdySessionTestBase {
public:
...
INSTANTIATE_TEST_SUITE_P(Tests, QuicSpdySessionTestServerNoExtendedConnect, ::testing::ValuesIn(AllSupportedVersions()), ::testing::PrintToStringParamName());

// Tests that receiving SETTINGS_ENABLE_CONNECT_PROTOCOL = 1 doesn't enable
// server session to support extended CONNECT.
TEST_P(QuicSpdySessionTestServerNoExtendedConnect, WebTransportSettingNoEffect)
{
    Initialize();
    if (!version().UsesHttp3()) {
        return;
...
TEST_P(QuicSpdySessionTestServerNoExtendedConnect, BadExtendedConnectSetting)
{
    ...
    EXPECT_QUIC_PEER_BUG(
        {
            EXPECT_CALL(*connection_, CloseConnection(QUIC_HTTP_INVALID_SETTING_VALUE, _, _));
            session_->OnStreamFrame(frame);
        },
