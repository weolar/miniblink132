...
public:
    ...
    public:
        ...
        virtual bool CheckVary(
            const spdy::Http2HeaderBlock& client_request, const spdy::Http2HeaderBlock& promise_request, const spdy::Http2HeaderBlock& promise_response)
            = 0;
    ...
    QuicClientPushPromiseIndex& operator=(const QuicClientPushPromiseIndex&) = delete;
    ...
    QuicAsyncStatus Try(const spdy::Http2HeaderBlock& request, Delegate* delegate, TryHandle** handle);

    QuicPromisedByUrlMap* promised_by_url()
    {
        return &promised_by_url_;
    }
