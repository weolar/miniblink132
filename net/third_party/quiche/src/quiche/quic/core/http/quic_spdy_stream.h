...
class QUICHE_EXPORT QuicSpdyStream : public QuicStream, public quiche::CapsuleParser::Visitor, public QpackDecodedHeadersAccumulator::Visitor {
public:
    ...
    public:
        ...
        virtual void OnMetadataComplete(size_t frame_len, const QuicHeaderList& header_list) = 0;
    };
    ...
    public:
        ...
        // the stream ID.
        virtual void OnHttp3Datagram(QuicStreamId stream_id, absl::string_view payload) = 0;

        // Called when a Capsule with an unknown type is received.
        virtual void OnUnknownCapsule(QuicStreamId stream_id, const quiche::UnknownCapsule& capsule) = 0;
    };
    ...
    public:
        ...
        virtual bool OnAddressAssignCapsule(const quiche::AddressAssignCapsule& capsule) = 0;
        virtual bool OnAddressRequestCapsule(const quiche::AddressRequestCapsule& capsule) = 0;
        virtual bool OnRouteAdvertisementCapsule(const quiche::RouteAdvertisementCapsule& capsule) = 0;
        virtual void OnHeadersWritten() = 0;
    };

    QuicSpdyStream(QuicStreamId id, QuicSpdySession* spdy_session, StreamType type);
    ...
    virtual void OnStreamHeadersPriority(const spdy::SpdyStreamPrecedence& precedence);

    // Called by the session when decompressed headers have been completely
    // delivered to this stream.  If |fin| is true, then this stream
    // should be closed; no more data will be sent by the peer.
    virtual void OnStreamHeaderList(bool fin, size_t frame_len, const QuicHeaderList& header_list);
    ...
    // QPACK.
    virtual size_t WriteHeaders(quiche::HttpHeaderBlock header_block, bool fin, quiche::QuicheReferenceCountedPointer<QuicAckListenerInterface> ack_listener);
    ...
    virtual size_t WriteTrailers(quiche::HttpHeaderBlock trailer_block, quiche::QuicheReferenceCountedPointer<QuicAckListenerInterface> ack_listener);

    // Override to report newly acked bytes via ack_listener_.
    bool OnStreamFrameAcked(QuicStreamOffset offset, QuicByteCount data_length, bool fin_acked, QuicTime::Delta ack_delay_time, QuicTime receive_timestamp,
        QuicByteCount* newly_acked_length) override;

    // Override to report bytes retransmitted via ack_listener_.
    void OnStreamFrameRetransmitted(QuicStreamOffset offset, QuicByteCount data_length, bool fin_retransmitted) override;
    ...
    QuicConsumedData WriteBodySlices(absl::Span<quiche::QuicheMemSlice> slices, bool fin);
    ...
    // code to |status_code|.
    static bool ParseHeaderStatusCode(const quiche::HttpHeaderBlock& header, int* status_code);
    ...
    static bool ParseHeaderStatusCode(absl::string_view status_value, int* status_code);
    ...
    bool headers_decompressed() const
    {
        return headers_decompressed_;
    }
    ...
    bool trailers_decompressed() const
    {
        return trailers_decompressed_;
    }
    ...
    QuicSpdySession* spdy_session() const
    {
...
protected:
    ...
    virtual void OnInitialHeadersComplete(bool fin, size_t frame_len, const QuicHeaderList& header_list);
    virtual void OnTrailingHeadersComplete(bool fin, size_t frame_len, const QuicHeaderList& header_list);
    ...
    void set_headers_decompressed(bool val)
    {
        headers_decompressed_ = val;
    }

    virtual bool uses_capsules() const
    {
        return capsule_parser_ != nullptr;
    }

    void set_ack_listener(quiche::QuicheReferenceCountedPointer<QuicAckListenerInterface> ack_listener)
    {
    ...
    virtual bool AreHeaderFieldValuesValid(const QuicHeaderList& header_list) const;
    ...
    virtual bool OnDataFrameStart(QuicByteCount header_length, QuicByteCount payload_length);
...
private:
    ...
    struct QUICHE_EXPORT WebTransportDataStream {
        WebTransportDataStream(QuicSpdyStream* stream, WebTransportSessionId session_id);
    ...
    bool OnDataFrameEnd();
    bool OnHeadersFrameStart(QuicByteCount header_length, QuicByteCount payload_length);
    bool OnHeadersFramePayload(absl::string_view payload);
    bool OnHeadersFrameEnd();
    void OnWebTransportStreamFrameType(QuicByteCount header_length, WebTransportSessionId session_id);
    bool OnMetadataFrameStart(QuicByteCount header_length, QuicByteCount payload_length);
    bool OnMetadataFramePayload(absl::string_view payload);
    bool OnMetadataFrameEnd();
    bool OnUnknownFrameStart(uint64_t frame_type, QuicByteCount header_length, QuicByteCount payload_length);
    ...
    QuicByteCount GetNumFrameHeadersInInterval(QuicStreamOffset offset, QuicByteCount data_length) const;
    ...
    ABSL_MUST_USE_RESULT bool WriteDataFrameHeader(QuicByteCount data_length, bool force_write);
    ...
    virtual bool NextHeaderIsTrailer() const
    {
        return headers_decompressed_;
    }
    ...
    std::unique_ptr<QpackDecodedHeadersAccumulator> qpack_decoded_headers_accumulator_;
