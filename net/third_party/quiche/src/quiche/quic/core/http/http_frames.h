...
struct QUICHE_EXPORT SettingsFrame {
    ...
    std::string ToString() const
    {
        std::string s;
        for (auto it : values) {
            std::string setting = absl::StrCat(H3SettingsToString(static_cast<Http3AndQpackSettingsIdentifiers>(it.first)), " = ", it.second, "; ");
            absl::StrAppend(&s, setting);
        }
        return s;
    }
    friend QUICHE_EXPORT std::ostream& operator<<(std::ostream& os, const SettingsFrame& s)
    {
        os << s.ToString();
        return os;
    }
...
struct QUICHE_EXPORT OriginFrame {
    ...
    friend QUICHE_EXPORT std::ostream& operator<<(std::ostream& os, const OriginFrame& s)
    {
        os << s.ToString();
        return os;
    }
...
struct QUICHE_EXPORT PriorityUpdateFrame {
    ...
    {
        return std::tie(prioritized_element_id, priority_field_value) == std::tie(rhs.prioritized_element_id, rhs.priority_field_value);
    }
    std::string ToString() const
    {
        return absl::StrCat("Priority Frame : {prioritized_element_id: ", prioritized_element_id, ", priority_field_value: ", priority_field_value, "}");
    }

    friend QUICHE_EXPORT std::ostream& operator<<(std::ostream& os, const PriorityUpdateFrame& s)
    {
        os << s.ToString();
        return os;
    }
...
struct QUICHE_EXPORT AcceptChFrame {
    ...
    {
        return entries.size() == rhs.entries.size() && std::equal(entries.begin(), entries.end(), rhs.entries.begin());
    }
    ...
    friend QUICHE_EXPORT std::ostream& operator<<(std::ostream& os, const AcceptChFrame& frame)
    {
