...
// is complete.
class QUIC_EXPORT_PRIVATE QuicClientPromisedInfo : public QuicClientPushPromiseIndex::TryHandle {
public:
    // Interface to QuicSpdyClientStream
    QuicClientPromisedInfo(QuicSpdyClientSessionBase* session, QuicStreamId id, std::string url);
    ...
    // has a matching URL.
    virtual QuicAsyncStatus HandleClientRequest(const spdy::Http2HeaderBlock& headers, QuicClientPushPromiseIndex::Delegate* delegate);
    ...
    {
        return client_request_delegate_ != nullptr;
    }

private:
    friend class test::QuicClientPromisedInfoPeer;

    class QUIC_EXPORT_PRIVATE CleanupAlarm : public QuicAlarm::DelegateWithoutContext {
    public:
