...
// static
bool HttpDecoder::DecodeSettings(const char* data, QuicByteCount len, SettingsFrame* frame)
{
...
QuicByteCount HttpDecoder::ProcessInput(const char* data, QuicByteCount len)
{
    ...
    while (continue_processing && (reader.BytesRemaining() != 0 || state_ == STATE_BUFFER_OR_PARSE_PAYLOAD || state_ == STATE_FINISH_PARSING)) {
        ...
        case STATE_PARSING_NO_LONGER_POSSIBLE:
            ...
            QUIC_BUG(HttpDecoder PARSING_NO_LONGER_POSSIBLE) << "HttpDecoder called after an indefinite-length frame has been "
                                                                "received";
...
bool HttpDecoder::ReadFrameType(QuicDataReader& reader)
{
    ...
    if (current_frame_type_ == static_cast<uint64_t>(http2::Http2FrameType::PRIORITY)
        || current_frame_type_ == static_cast<uint64_t>(http2::Http2FrameType::PING)
        || current_frame_type_ == static_cast<uint64_t>(http2::Http2FrameType::WINDOW_UPDATE)
        || current_frame_type_ == static_cast<uint64_t>(http2::Http2FrameType::CONTINUATION)) {
        RaiseError(QUIC_HTTP_RECEIVE_SPDY_FRAME, absl::StrCat("HTTP/2 frame received in a HTTP/3 connection: ", current_frame_type_));
        return false;
    }

    if (current_frame_type_ == static_cast<uint64_t>(HttpFrameType::CANCEL_PUSH)) {
        RaiseError(QUIC_HTTP_FRAME_ERROR, "CANCEL_PUSH frame received.");
        return false;
    }
    if (current_frame_type_ == static_cast<uint64_t>(HttpFrameType::PUSH_PROMISE)) {
...
bool HttpDecoder::ReadFrameLength(QuicDataReader& reader)
{
    ...
    if (current_length_field_length_ == 0) {
        ...
        QuicDataReader length_reader(length_buffer_.data(), current_length_field_length_);
    ...
    if (allow_web_transport_stream_ && current_frame_type_ == static_cast<uint64_t>(HttpFrameType::WEBTRANSPORT_STREAM)) {
        visitor_->OnWebTransportStreamFrameType(current_length_field_length_ + current_type_field_length_, current_frame_length_);
        state_ = STATE_PARSING_NO_LONGER_POSSIBLE;
        return false;
    }

    if (IsFrameBuffered() && current_frame_length_ > MaxFrameLength(current_frame_type_)) {
    ...
    // frame payload.
    bool continue_processing = true;
    const QuicByteCount header_length = current_length_field_length_ + current_type_field_length_;

    switch (current_frame_type_) {
    case static_cast<uint64_t>(HttpFrameType::DATA):
        continue_processing = visitor_->OnDataFrameStart(header_length, current_frame_length_);
        break;
    case static_cast<uint64_t>(HttpFrameType::HEADERS):
        continue_processing = visitor_->OnHeadersFrameStart(header_length, current_frame_length_);
        break;
    ...
    case static_cast<uint64_t>(HttpFrameType::METADATA):
        continue_processing = visitor_->OnMetadataFrameStart(header_length, current_frame_length_);
        break;
    default:
        if (enable_origin_frame_ && current_frame_type_ == static_cast<uint64_t>(HttpFrameType::ORIGIN)) {
            QUIC_CODE_COUNT_N(enable_h3_origin_frame, 1, 2);
            continue_processing = visitor_->OnOriginFrameStart(header_length);
            break;
        }
        continue_processing = visitor_->OnUnknownFrameStart(current_frame_type_, header_length, current_frame_length_);
        break;
    }
    ...
    state_ = (remaining_frame_length_ == 0) ? STATE_FINISH_PARSING : STATE_READING_FRAME_PAYLOAD;
    return continue_processing;
}
...
bool HttpDecoder::ReadFramePayload(QuicDataReader& reader)
{
    ...
    switch (current_frame_type_) {
    case static_cast<uint64_t>(HttpFrameType::DATA): {
        QuicByteCount bytes_to_read = std::min<QuicByteCount>(remaining_frame_length_, reader.BytesRemaining());
...
{
    QUICHE_DCHECK(IsFrameBuffered());
    QUICHE_DCHECK_EQ(current_frame_length_, buffer_.size() + remaining_frame_length_);

    if (buffer_.empty() && reader.BytesRemaining() >= current_frame_length_) {
        // |*reader| contains entire payload, which might be empty.
        remaining_frame_length_ = 0;
        QuicDataReader current_payload_reader(reader.PeekRemainingPayload().data(), current_frame_length_);
    ...
    QuicByteCount bytes_to_read = std::min<QuicByteCount>(remaining_frame_length_, reader.BytesRemaining());
    absl::StrAppend(&buffer_,
        reader.PeekRemainingPayload().substr(
            /* pos = */ 0, bytes_to_read));
...
bool HttpDecoder::ParseEntirePayload(QuicDataReader& reader)
{
    ...
    case static_cast<uint64_t>(HttpFrameType::MAX_PUSH_ID): {
        ...
        if (!reader.ReadVarInt62(&unused)) {
            RaiseError(QUIC_HTTP_FRAME_ERROR, "Unable to read MAX_PUSH_ID push_id.");
            return false;
        }
        if (!reader.IsDoneReading()) {
            RaiseError(QUIC_HTTP_FRAME_ERROR, "Superfluous data in MAX_PUSH_ID frame.");
            return false;
        }
...
{
    QuicByteCount bytes_to_read = std::min<QuicByteCount>(remaining_length_field_length_, reader.BytesRemaining());
    bool success = reader.ReadBytes(length_buffer_.data() + current_length_field_length_ - remaining_length_field_length_, bytes_to_read);
    QUICHE_DCHECK(success);
...
{
    QuicByteCount bytes_to_read = std::min<QuicByteCount>(remaining_type_field_length_, reader.BytesRemaining());
    bool success = reader.ReadBytes(type_buffer_.data() + current_type_field_length_ - remaining_type_field_length_, bytes_to_read);
    QUICHE_DCHECK(success);
...
bool HttpDecoder::ParseSettingsFrame(QuicDataReader& reader, SettingsFrame& frame)
{
    while (!reader.IsDoneReading()) {
        ...
        auto result = frame.values.insert({ id, content });
        if (!result.second) {
            RaiseError(QUIC_HTTP_DUPLICATE_SETTING_IDENTIFIER, "Duplicate setting identifier.");
            return false;
        }
    }
    return true;
...
bool HttpDecoder::ParsePriorityUpdateFrame(QuicDataReader& reader, PriorityUpdateFrame& frame)
{
    ...
    frame.priority_field_value = std::string(priority_field_value.data(), priority_field_value.size());
...
bool HttpDecoder::ParseAcceptChFrame(QuicDataReader& reader, AcceptChFrame& frame)
{
    absl::string_view origin;
    ...
    while (!reader.IsDoneReading()) {
        ...
        // Copy data.
        frame.entries.push_back({ std::string(origin.data(), origin.size()), std::string(value.data(), value.size()) });
    }
    return true;
}
