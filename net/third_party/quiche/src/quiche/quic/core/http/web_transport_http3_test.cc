...
TEST(WebTransportHttp3Test, ErrorCodesToWebTransport)
{
    ...
    EXPECT_EQ(Http3ErrorToWebTransport(std::numeric_limits<uint64_t>::max()), std::nullopt);
}
...
TEST(WebTransportHttp3Test, ErrorCodeRoundTrip)
{
    for (int error = 0; error <= 65536; error++) {
        ...
        std::optional<WebTransportStreamError> mapped_back = quic::Http3ErrorToWebTransport(http_error);
        ASSERT_THAT(mapped_back, Optional(error));
    }
    for (int64_t error = 0; error < std::numeric_limits<uint32_t>::max(); error += 65537) {
        uint64_t http_error = WebTransportErrorToHttp3(error);
        std::optional<WebTransportStreamError> mapped_back = quic::Http3ErrorToWebTransport(http_error);
        ASSERT_THAT(mapped_back, Optional(error));
    }
