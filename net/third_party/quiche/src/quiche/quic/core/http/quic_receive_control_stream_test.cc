...
public:
    QuicReceiveControlStreamTest()
        : connection_(new StrictMock<MockQuicConnection>(&helper_, &alarm_factory_, perspective(), SupportedVersions(GetParam().version)))
        , session_(connection_)
    {
        EXPECT_CALL(session_, OnCongestionWindowChange(_)).Times(AnyNumber());
        session_.Initialize();
        EXPECT_CALL(static_cast<const MockQuicCryptoStream&>(*session_.GetCryptoStream()), encryption_established()).WillRepeatedly(testing::Return(true));
        QuicStreamId id = perspective() == Perspective::IS_SERVER ? GetNthClientInitiatedUnidirectionalStreamId(session_.transport_version(), 3)
                                                                  : GetNthServerInitiatedUnidirectionalStreamId(session_.transport_version(), 3);
        char type[] = { kControlStream };

        QuicStreamFrame data1(id, false, 0, absl::string_view(type, 1));
        session_.OnStreamFrame(data1);

        receive_control_stream_ = QuicSpdySessionPeer::GetReceiveControlStream(&session_);

        stream_ = new TestStream(GetNthClientInitiatedBidirectionalStreamId(GetParam().version.transport_version, 0), &session_);
    ...
    {
        return QuicStreamPeer::sequencer(receive_control_stream_)->NumBytesConsumed();
    }
...
INSTANTIATE_TEST_SUITE_P(Tests, QuicReceiveControlStreamTest, ::testing::ValuesIn(GetTestParams()), ::testing::PrintToStringParamName());

TEST_P(QuicReceiveControlStreamTest, ResetControlStream)
{
    EXPECT_TRUE(receive_control_stream_->is_static());
    QuicRstStreamFrame rst_frame(kInvalidControlFrameId, receive_control_stream_->id(), QUIC_STREAM_CANCELLED, 1234);
    EXPECT_CALL(*connection_, CloseConnection(QUIC_HTTP_CLOSED_CRITICAL_STREAM, _, _));
...
TEST_P(QuicReceiveControlStreamTest, ReceiveSettingsTwice)
{
    ...
    receive_control_stream_->OnStreamFrame(QuicStreamFrame(receive_control_stream_->id(), /* fin = */ false, offset, settings_frame));
    ...
    EXPECT_CALL(*connection_, CloseConnection(QUIC_HTTP_INVALID_FRAME_SEQUENCE_ON_CONTROL_STREAM, "SETTINGS frame can only be received once.", _))
...
TEST_P(QuicReceiveControlStreamTest, ReceiveWrongFrame)
{
    ...
    QuicStreamFrame frame(receive_control_stream_->id(), false, 1, data.AsStringView());
    EXPECT_CALL(*connection_, CloseConnection(QUIC_HTTP_FRAME_UNEXPECTED_ON_CONTROL_STREAM, _, _));
    receive_control_stream_->OnStreamFrame(frame);
}

TEST_P(QuicReceiveControlStreamTest, ReceivePriorityUpdateFrameBeforeSettingsFrame)
{
...
TEST_P(QuicReceiveControlStreamTest, ReceiveGoAwayFrame)
{
    ...
    QuicStreamFrame frame(receive_control_stream_->id(), false, offset, goaway_frame);
...
TEST_P(QuicReceiveControlStreamTest, PushPromiseOnControlStreamShouldClose)
{
    ...
    ASSERT_TRUE(absl::HexStringToBytes("05" // PUSH_PROMISE
                                       "01" // length
                                       "00", // push ID
        &push_promise_frame));
    QuicStreamFrame frame(receive_control_stream_->id(), false, 1, push_promise_frame);
    EXPECT_CALL(*connection_, CloseConnection(QUIC_HTTP_FRAME_ERROR, _, _)).WillOnce(Invoke(connection_, &MockQuicConnection::ReallyCloseConnection));
    EXPECT_CALL(*connection_, SendConnectionClosePacket(_, _, _));
    EXPECT_CALL(session_, OnConnectionClosed(_, _));
    receive_control_stream_->OnStreamFrame(frame);
}
...
TEST_P(QuicReceiveControlStreamTest, ConsumeUnknownFrame)
{
    ...
    ASSERT_TRUE(absl::HexStringToBytes("21" // reserved frame type
                                       "03" // payload length
                                       "666f6f", // payload "foo"
        &unknown_frame));

    receive_control_stream_->OnStreamFrame(QuicStreamFrame(receive_control_stream_->id(), /* fin = */ false, offset, unknown_frame));
...
TEST_P(QuicReceiveControlStreamTest, ReceiveUnknownFrame)
{
    ...
    std::string settings_frame = HttpEncoder::SerializeSettingsFrame(settings);
    EXPECT_CALL(debug_visitor, OnSettingsFrameReceived(settings));
    receive_control_stream_->OnStreamFrame(QuicStreamFrame(id, /* fin = */ false, offset, settings_frame));
    offset += settings_frame.length();
    ...
    EXPECT_CALL(debug_visitor,
        OnUnknownFrameReceived(id, /* frame_type = */ 0x21,
            /* payload_length = */ 3));
    receive_control_stream_->OnStreamFrame(QuicStreamFrame(id, /* fin = */ false, offset, unknown_frame));
}

TEST_P(QuicReceiveControlStreamTest, CancelPushFrameBeforeSettings)
{
    std::string cancel_push_frame;
    ASSERT_TRUE(absl::HexStringToBytes("03" // type CANCEL_PUSH
                                       "01" // payload length
                                       "01", // push ID
        &cancel_push_frame));
    ...
    receive_control_stream_->OnStreamFrame(QuicStreamFrame(receive_control_stream_->id(), /* fin = */ false,
...
TEST_P(QuicReceiveControlStreamTest, AcceptChFrameBeforeSettings)
{
    ...
    if (perspective() == Perspective::IS_SERVER) {
        EXPECT_CALL(*connection_, CloseConnection(QUIC_HTTP_FRAME_UNEXPECTED_ON_CONTROL_STREAM, "Invalid frame type 137 received on control stream.", _))
            .WillOnce(Invoke(connection_, &MockQuicConnection::ReallyCloseConnection));
    } else {
        EXPECT_CALL(*connection_,
            CloseConnection(QUIC_HTTP_MISSING_SETTINGS_FRAME,
...
TEST_P(QuicReceiveControlStreamTest, ReceiveAcceptChFrame)
{
    ...
    receive_control_stream_->OnStreamFrame(QuicStreamFrame(id, /* fin = */ false, offset, accept_ch_frame));
}
...
TEST_P(QuicReceiveControlStreamTest, ReceiveOriginFrame)
{
    ...
    ASSERT_TRUE(absl::HexStringToBytes("0C" // type (ORIGIN)
                                       "00", // length
        &origin_frame));
    ...
    if (GetQuicReloadableFlag(enable_h3_origin_frame)) {
        if (perspective() == Perspective::IS_CLIENT) {
            ...
            EXPECT_CALL(*connection_, CloseConnection(QUIC_HTTP_FRAME_UNEXPECTED_ON_CONTROL_STREAM, "Invalid frame type 12 received on control stream.", _))
                .WillOnce(Invoke(connection_, &MockQuicConnection::ReallyCloseConnection));
            EXPECT_CALL(*connection_, SendConnectionClosePacket(_, _, _));
            EXPECT_CALL(session_, OnConnectionClosed(_, _));
        }
    ...
    receive_control_stream_->OnStreamFrame(QuicStreamFrame(id, /* fin = */ false, offset, origin_frame));
}
