...
public:
    TestStream(QuicStreamId id, QuicSpdySession* session, bool should_process_data)
    ...
    size_t WriteHeadersImpl(quiche::HttpHeaderBlock header_block, bool fin, quiche::QuicheReferenceCountedPointer<QuicAckListenerInterface>
        /*ack_listener*/) override
    {
        ...
        if (VersionUsesHttp3(transport_version())) {
            ...
            return QuicSpdyStream::WriteHeadersImpl(saved_headers_.Clone(), fin, nullptr);
        }
        return 0;
    }
    ...
    void OnStreamHeaderList(bool fin, size_t frame_len, const QuicHeaderList& header_list) override
    {
        headers_payload_length_ = frame_len;
        QuicSpdyStream::OnStreamHeaderList(fin, frame_len, header_list);
    }

    size_t headers_payload_length() const
    {
        return headers_payload_length_;
    }
...
public:
    ...
    void EnableWebTransport(WebTransportHttp3VersionSet versions = kDefaultSupportedWebTransportVersions)
    {
...
public:
    ...
    void UpdateStreamPriority(QuicStreamId id, const QuicStreamPriority& new_priority) override
    {
        EXPECT_EQ(id, expected_stream_->id());
        EXPECT_EQ(expected_priority_, new_priority.http());
        EXPECT_EQ(QuicStreamPriority(expected_priority_), expected_stream_->priority());
    }

    void SetExpectedStream(QuicSpdyStream* stream)
    {
        expected_stream_ = stream;
    }
...
protected:
    ...
    std::string EncodeQpackHeaders(std::vector<std::pair<absl::string_view, absl::string_view>> headers)
    {
        HttpHeaderBlock header_block;
        for (const auto& header_field : headers) {
            header_block.AppendValueOrAddHeader(header_field.first, header_field.second);
        }
    ...
    std::string EncodeQpackHeaders(const HttpHeaderBlock& header)
    {
        ...
        auto qpack_encoder = std::make_unique<QpackEncoder>(session_.get(), HuffmanEncoding::kEnabled, CookieCrumbling::kEnabled);
        ...
        return qpack_encoder->EncodeHeaderList(/* stream_id = */ 0, header, nullptr);
    }

    void Initialize(bool stream_should_process_data)
    {
        InitializeWithPerspective(stream_should_process_data, Perspective::IS_SERVER);
    }

    void InitializeWithPerspective(bool stream_should_process_data, Perspective perspective)
    {
        ...
        ON_CALL(*session_, WritevData(_, _, _, _, _, _)).WillByDefault(Invoke(session_.get(), &MockQuicSpdySession::ConsumeData));

        stream_ = new StrictMock<TestStream>(GetNthClientInitiatedBidirectionalId(0), session_.get(), stream_should_process_data);
        session_->ActivateStream(absl::WrapUnique(stream_));
        stream2_ = new StrictMock<TestStream>(GetNthClientInitiatedBidirectionalId(1), session_.get(), stream_should_process_data);
        ...
        if (UsesHttp3()) {
            ...
            auto send_control_stream = QuicSpdySessionPeer::GetSendControlStream(session_.get());
            EXPECT_CALL(*session_, WritevData(send_control_stream->id(), _, _, _, _, _)).Times(num_control_stream_writes);
        }
        TestCryptoStream* crypto_stream = session_->GetMutableCryptoStream();
        EXPECT_CALL(*crypto_stream, HasPendingRetransmission()).Times(AnyNumber());

        if (connection_->version().UsesTls() && session_->perspective() == Perspective::IS_SERVER) {
            // HANDSHAKE_DONE frame.
    ...
    {
        return GetNthClientInitiatedBidirectionalStreamId(connection_->transport_version(), n);
    }
    ...
    // dynamic table.
    std::string HeadersFrame(std::vector<std::pair<absl::string_view, absl::string_view>> headers)
    {
    ...
    {
        std::string headers_frame_header = HttpEncoder::SerializeHeadersFrameHeader(payload.length());
        return absl::StrCat(headers_frame_header, payload);
    }

    std::string DataFrame(absl::string_view payload)
    {
        quiche::QuicheBuffer header = HttpEncoder::SerializeDataFrameHeader(payload.length(), quiche::SimpleBufferAllocator::Get());
    ...
    {
        std::string frame;
        const size_t length = QuicDataWriter::GetVarInt62Len(frame_type) + QuicDataWriter::GetVarInt62Len(payload.size()) + payload.size();
        frame.resize(length);
...
INSTANTIATE_TEST_SUITE_P(Tests, QuicSpdyStreamTest, ::testing::ValuesIn(AllSupportedVersions()), ::testing::PrintToStringParamName());

TEST_P(QuicSpdyStreamTest, ProcessHeaderList)
{
    Initialize(kShouldProcessData);

    stream_->OnStreamHeadersPriority(spdy::SpdyStreamPrecedence(kV3HighestPriority));
...
TEST_P(QuicSpdyStreamTest, ProcessTooLargeHeaderList)
{
    ...
    if (!UsesHttp3()) {
        ...
        EXPECT_CALL(*session_, MaybeSendRstStreamFrame(stream_->id(), QuicResetStreamError::FromInternal(QUIC_HEADERS_TOO_LARGE), 0));
    ...
    std::string headers = HeadersFrame({ std::make_pair("foo", "too long headers") });

    QuicStreamFrame frame(stream_->id(), false, 0, headers);

    EXPECT_CALL(*session_, MaybeSendStopSendingFrame(stream_->id(), QuicResetStreamError::FromInternal(QUIC_HEADERS_TOO_LARGE)));
    EXPECT_CALL(*session_, MaybeSendRstStreamFrame(stream_->id(), QuicResetStreamError::FromInternal(QUIC_HEADERS_TOO_LARGE), 0));
...
TEST_P(QuicSpdyStreamTest, ProcessWrongFramesOnSpdyStream)
{
    ...
    EXPECT_EQ(headers, stream_->header_list());
    stream_->ConsumeHeaderList();
    QuicStreamFrame frame(GetNthClientInitiatedBidirectionalId(0), false, 0, goaway_frame);

    EXPECT_CALL(*connection_, CloseConnection(QUIC_HTTP_FRAME_UNEXPECTED_ON_SPDY_STREAM, _, _))
        .WillOnce((Invoke([this](QuicErrorCode error, const std::string& error_details, ConnectionCloseBehavior connection_close_behavior) {
            connection_->ReallyCloseConnection(error, error_details, connection_close_behavior);
        })));
    EXPECT_CALL(*connection_, SendConnectionClosePacket(_, _, _));
    EXPECT_CALL(*session_, OnConnectionClosed(_, _)).WillOnce(Invoke([this](const QuicConnectionCloseFrame& frame, ConnectionCloseSource source) {
...
TEST_P(QuicSpdyStreamTest, ProcessHeadersAndBody)
{
    ...
    EXPECT_EQ(headers, stream_->header_list());
    stream_->ConsumeHeaderList();
    QuicStreamFrame frame(GetNthClientInitiatedBidirectionalId(0), false, 0, absl::string_view(data));
    stream_->OnStreamFrame(frame);
    EXPECT_EQ(QuicHeaderList(), stream_->header_list());
...
TEST_P(QuicSpdyStreamTest, ProcessHeadersAndBodyFragments)
{
    ...
    for (size_t fragment_size = 1; fragment_size < data.size(); ++fragment_size) {
        ...
        for (size_t offset = 0; offset < data.size(); offset += fragment_size) {
            ...
            absl::string_view fragment(data.data() + offset, std::min(fragment_size, remaining_data));
            QuicStreamFrame frame(GetNthClientInitiatedBidirectionalId(0), false, offset, absl::string_view(fragment));
            stream_->OnStreamFrame(frame);
        }
...
TEST_P(QuicSpdyStreamTest, ProcessHeadersAndBodyFragmentsSplit)
{
    ...
    for (size_t split_point = 1; split_point < data.size() - 1; ++split_point) {
        ...
        QuicStreamFrame frame1(GetNthClientInitiatedBidirectionalId(0), false, 0, absl::string_view(fragment1));
        stream_->OnStreamFrame(frame1);

        absl::string_view fragment2(data.data() + split_point, data.size() - split_point);
        QuicStreamFrame frame2(GetNthClientInitiatedBidirectionalId(0), false, split_point, absl::string_view(fragment2));
...
TEST_P(QuicSpdyStreamTest, ProcessHeadersAndConsumeMultipleBody)
{
    ...
    ProcessHeaders(false, headers_);
    QuicStreamFrame frame1(GetNthClientInitiatedBidirectionalId(0), false, 0, absl::string_view(data1));
    QuicStreamFrame frame2(GetNthClientInitiatedBidirectionalId(0), false, data1.length(), absl::string_view(data2));
    stream_->OnStreamFrame(frame1);
    stream_->OnStreamFrame(frame2);
    stream_->ConsumeHeaderList();

    stream_->MarkConsumed(body1.length() + body2.length());
    EXPECT_EQ(data1.length() + data2.length(), QuicStreamPeer::bytes_consumed(stream_));
}
...
TEST_P(QuicSpdyStreamTest, StreamFlowControlBlocked)
{
    ...
    EXPECT_CALL(*session_, WritevData(_, _, _, _, _, _)).WillOnce(Return(QuicConsumedData(kWindow - kHeaderLength, true)));
...
TEST_P(QuicSpdyStreamTest, StreamFlowControlNoWindowUpdateIfNotConsumed)
{
    ...
    if (UsesHttp3()) {
        quiche::QuicheBuffer header = HttpEncoder::SerializeDataFrameHeader(body.length(), quiche::SimpleBufferAllocator::Get());
    ...
    QuicStreamFrame frame1(GetNthClientInitiatedBidirectionalId(0), false, 0, absl::string_view(data));
    stream_->OnStreamFrame(frame1);
    EXPECT_EQ(kWindow - (kWindow / 3) - header_length, QuicStreamPeer::ReceiveWindowSize(stream_));

    // Now receive another frame which results in the receive window being over
    // half full. This should all be buffered, decreasing the receive window but
    // not sending WINDOW_UPDATE.
    QuicStreamFrame frame2(GetNthClientInitiatedBidirectionalId(0), false, kWindow / 3 + header_length, absl::string_view(data));
    stream_->OnStreamFrame(frame2);
    EXPECT_EQ(kWindow - (2 * kWindow / 3) - 2 * header_length, QuicStreamPeer::ReceiveWindowSize(stream_));
}
...
TEST_P(QuicSpdyStreamTest, ConnectionFlowControlWindowUpdate)
{
    ...
    QuicFlowControllerPeer::SetReceiveWindowOffset(session_->flow_controller(), kWindow);
    QuicFlowControllerPeer::SetMaxReceiveWindow(session_->flow_controller(), kWindow);

    // Supply headers to both streams so that they are happy to receive data.
    auto headers = AsHeaderList(headers_);
    stream_->OnStreamHeaderList(false, headers.uncompressed_header_bytes(), headers);
    stream_->ConsumeHeaderList();
    stream2_->OnStreamHeaderList(false, headers.uncompressed_header_bytes(), headers);
    ...
    if (UsesHttp3()) {
        ...
        data = absl::StrCat(header.AsStringView(), body);
        header_length = header.size();
        quiche::QuicheBuffer header2 = HttpEncoder::SerializeDataFrameHeader(body.length(), quiche::SimpleBufferAllocator::Get());
    ...
    QuicStreamFrame frame1(GetNthClientInitiatedBidirectionalId(0), false, 0, absl::string_view(data));
    stream_->OnStreamFrame(frame1);
    QuicStreamFrame frame2(GetNthClientInitiatedBidirectionalId(1), false, 0, absl::string_view(data));
    ...
    EXPECT_CALL(*session_, SendWindowUpdate(_, _));
    EXPECT_CALL(*connection_, SendControlFrame(_));
    QuicStreamFrame frame3(GetNthClientInitiatedBidirectionalId(0), false, body.length() + header_length, absl::string_view(data2));
...
TEST_P(QuicSpdyStreamTest, StreamFlowControlViolation)
{
    ...
    std::string data = UsesHttp3() ? DataFrame(body) : body;
    QuicStreamFrame frame(GetNthClientInitiatedBidirectionalId(0), false, 0, absl::string_view(data));
    EXPECT_CALL(*connection_, CloseConnection(QUIC_FLOW_CONTROL_RECEIVED_TOO_MUCH_DATA, _, _));
    stream_->OnStreamFrame(frame);
}
...
TEST_P(QuicSpdyStreamTest, TestHandlingQuicRstStreamNoError)
{
    ...
    stream_->OnStreamReset(QuicRstStreamFrame(kInvalidControlFrameId, stream_->id(), QUIC_STREAM_NO_ERROR, 0));
...
TEST_P(QuicSpdyStreamTest, ConnectionFlowControlViolation)
{
    ...
    QuicFlowControllerPeer::SetReceiveWindowOffset(session_->flow_controller(), kConnectionWindow);
...
TEST_P(QuicSpdyStreamTest, StreamFlowControlFinNotBlocked)
{
    ...
    EXPECT_CALL(*session_, SendBlocked(GetNthClientInitiatedBidirectionalId(0), _)).Times(0);
...
TEST_P(QuicSpdyStreamTest, ReceivingTrailersWithOffset)
{
    ...
    QuicStreamFrame frame(GetNthClientInitiatedBidirectionalId(0), /*fin=*/false, 0, data);
    stream_->OnStreamFrame(frame);
    EXPECT_EQ(body, stream_->data());
    EXPECT_TRUE(stream_->IsDoneReading());
}
...
TEST_P(QuicSpdyStreamTest, ReceivingTrailersWithoutOffset)
{
    ...
    EXPECT_CALL(*connection_, CloseConnection(QUIC_INVALID_HEADERS_STREAM_DATA, _, _)).Times(1);
    stream_->OnStreamHeaderList(/*fin=*/true, trailers.uncompressed_header_bytes(), trailers);
}
...
TEST_P(QuicSpdyStreamTest, ReceivingTrailersWithoutFin)
{
    ...
    // Receive initial headers.
    auto headers = AsHeaderList(headers_);
    stream_->OnStreamHeaderList(/*fin=*/false, headers.uncompressed_header_bytes(), headers);
    stream_->ConsumeHeaderList();
    ...
    EXPECT_CALL(*connection_, CloseConnection(QUIC_INVALID_HEADERS_STREAM_DATA, _, _)).Times(1);
    stream_->OnStreamHeaderList(/*fin=*/false, trailers.uncompressed_header_bytes(), trailers);
}
...
TEST_P(QuicSpdyStreamTest, ReceivingTrailersAfterBodyWithFin)
{
    ...
    QuicStreamFrame frame(GetNthClientInitiatedBidirectionalId(0), /*fin=*/true, 0, "body");
    stream_->OnStreamFrame(frame);
...
TEST_P(QuicSpdyStreamTest, ClosingStreamWithNoTrailers)
{
    ...
    QuicStreamFrame frame(GetNthClientInitiatedBidirectionalId(0), /*fin=*/true, 0, data);
    stream_->OnStreamFrame(frame);
...
TEST_P(QuicSpdyStreamTest, DoNotSendPriorityUpdateWithDefaultUrgency)
{
    ...
    auto send_control_stream = QuicSpdySessionPeer::GetSendControlStream(session_.get());
    EXPECT_CALL(*session_, WritevData(send_control_stream->id(), _, _, _, _, _)).Times(0);
...
TEST_P(QuicSpdyStreamTest, ChangePriority)
{
    ...
    const HttpStreamPriority priority1 { kV3HighestPriority, HttpStreamPriority::kDefaultIncremental };
...
TEST_P(QuicSpdyStreamTest, ChangePriorityBeforeWritingHeaders)
{
    ...
    stream_->SetPriority(QuicStreamPriority(HttpStreamPriority { kV3HighestPriority, HttpStreamPriority::kDefaultIncremental }));
...
TEST_P(QuicSpdyStreamTest, WritingTrailersFinalOffset)
{
    ...
    QuicByteCount header_length = 0;
    if (UsesHttp3()) {
        header_length = HttpEncoder::SerializeDataFrameHeader(body.length(), quiche::SimpleBufferAllocator::Get()).size();
    }
    ...
    // on the headers stream.
    if (!UsesHttp3()) {
        expected_trailers[kFinalOffsetHeaderKey] = absl::StrCat(body.length() + header_length);
    }
...
TEST_P(QuicSpdyStreamTest, WritingTrailersClosesWriteSide)
{
    ...
    EXPECT_CALL(*session_, WritevData(stream_->id(), _, _, _, _, _)).Times(AtLeast(1));
...
TEST_P(QuicSpdyStreamTest, WritingTrailersWithQueuedBytes)
{
    ...
    EXPECT_CALL(*session_, WritevData(_, kBodySize, _, NO_FIN, _, _)).WillOnce(Return(QuicConsumedData(kBodySize - 1, false)));
    stream_->WriteOrBufferBody(std::string(kBodySize, 'x'), false);
...
TEST_P(QuicSpdyStreamTest, WritingTrailersAfterFIN)
{
    ...
    EXPECT_QUIC_BUG(stream_->WriteTrailers(HttpHeaderBlock(), nullptr), "Trailers cannot be sent after a FIN");
}
...
TEST_P(QuicSpdyStreamTest, HeaderStreamNotiferCorrespondingSpdyStream)
{
    ...
    EXPECT_CALL(*session_, WritevData(_, _, _, _, _, _)).Times(AtLeast(1));
    testing::InSequence s;
    quiche::QuicheReferenceCountedPointer<MockAckListener> ack_listener1(new MockAckListener());
    quiche::QuicheReferenceCountedPointer<MockAckListener> ack_listener2(new MockAckListener());
    ...
    QuicStreamFrame frame1(QuicUtils::GetHeadersStreamId(connection_->transport_version()), false, 0, kHeader1);

    std::string data1 = UsesHttp3() ? DataFrame(kBody1) : kBody1;
    QuicStreamFrame frame2(stream_->id(), true, 0, data1);
    QuicStreamFrame frame3(QuicUtils::GetHeadersStreamId(connection_->transport_version()), false, 7, kHeader2);
    ...
    EXPECT_TRUE(session_->OnFrameAcked(QuicFrame(frame1), QuicTime::Delta::Zero(), QuicTime::Zero()));
    EXPECT_CALL(*ack_listener1, OnPacketAcked(5, _));
    EXPECT_TRUE(session_->OnFrameAcked(QuicFrame(frame2), QuicTime::Delta::Zero(), QuicTime::Zero()));
    EXPECT_CALL(*ack_listener2, OnPacketAcked(7, _));
    EXPECT_TRUE(session_->OnFrameAcked(QuicFrame(frame3), QuicTime::Delta::Zero(), QuicTime::Zero()));
    EXPECT_CALL(*ack_listener2, OnPacketAcked(5, _));
    EXPECT_TRUE(session_->OnFrameAcked(QuicFrame(frame4), QuicTime::Delta::Zero(), QuicTime::Zero()));
}
...
TEST_P(QuicSpdyStreamTest, OnPriorityFrame)
{
    ...
    EXPECT_EQ(QuicStreamPriority(HttpStreamPriority { kV3HighestPriority, HttpStreamPriority::kDefaultIncremental }), stream_->priority());
}
...
{
    MockQuicConnection* connection = new StrictMock<MockQuicConnection>(&helper_, &alarm_factory_, Perspective::IS_SERVER, SupportedVersions(GetParam()));
    std::unique_ptr<TestMockUpdateStreamSession> session(new StrictMock<TestMockUpdateStreamSession>(connection));
    auto stream = new StrictMock<TestStream>(GetNthClientInitiatedBidirectionalStreamId(session->transport_version(), 0), session.get(),
    ...
    stream->SetPriority(QuicStreamPriority(HttpStreamPriority { kV3HighestPriority }));

    session->SetExpectedPriority(HttpStreamPriority { kV3LowestPriority });
    stream->SetPriority(QuicStreamPriority(HttpStreamPriority { kV3LowestPriority }));
}

TEST_P(QuicSpdyStreamTest, StreamWaitsForAcks)
{
    Initialize(kShouldProcessData);
    quiche::QuicheReferenceCountedPointer<MockAckListener> mock_ack_listener(new StrictMock<MockAckListener>);
    ...
    EXPECT_TRUE(stream_->OnStreamFrameAcked(0, 9, false, QuicTime::Delta::Zero(), QuicTime::Zero(), &newly_acked_length));
    ...
    // kData2 is acked.
    EXPECT_CALL(*mock_ack_listener, OnPacketAcked(9, _));
    EXPECT_TRUE(stream_->OnStreamFrameAcked(9, 9, false, QuicTime::Delta::Zero(), QuicTime::Zero(), &newly_acked_length));
    ...
    EXPECT_TRUE(stream_->OnStreamFrameAcked(18, 0, true, QuicTime::Delta::Zero(), QuicTime::Zero(), &newly_acked_length));
    EXPECT_FALSE(stream_->IsWaitingForAcks());
    EXPECT_EQ(0u, QuicStreamPeer::SendBuffer(stream_).size());
}
...
TEST_P(QuicSpdyStreamTest, StreamDataGetAckedMultipleTimes)
{
    ...
    EXPECT_TRUE(stream_->OnStreamFrameAcked(5, 17, false, QuicTime::Delta::Zero(), QuicTime::Zero(), &newly_acked_length));
    EXPECT_EQ(1u, QuicStreamPeer::SendBuffer(stream_).size());
    // Verify [22, 26) 4 bytes are acked.
    EXPECT_CALL(*mock_ack_listener, OnPacketAcked(4, _));
    EXPECT_TRUE(stream_->OnStreamFrameAcked(18, 8, false, QuicTime::Delta::Zero(), QuicTime::Zero(), &newly_acked_length));
    EXPECT_EQ(1u, QuicStreamPeer::SendBuffer(stream_).size());
    EXPECT_TRUE(stream_->IsWaitingForAcks());
    ...
    EXPECT_TRUE(stream_->OnStreamFrameAcked(26, 1, false, QuicTime::Delta::Zero(), QuicTime::Zero(), &newly_acked_length));
    EXPECT_EQ(0u, QuicStreamPeer::SendBuffer(stream_).size());
    EXPECT_TRUE(stream_->IsWaitingForAcks());

    // Ack Fin. Verify OnPacketAcked is called.
    EXPECT_CALL(*mock_ack_listener, OnPacketAcked(0, _));
    EXPECT_TRUE(stream_->OnStreamFrameAcked(27, 0, true, QuicTime::Delta::Zero(), QuicTime::Zero(), &newly_acked_length));
    EXPECT_EQ(0u, QuicStreamPeer::SendBuffer(stream_).size());
    EXPECT_FALSE(stream_->IsWaitingForAcks());
    ...
    EXPECT_FALSE(stream_->OnStreamFrameAcked(10, 17, true, QuicTime::Delta::Zero(), QuicTime::Zero(), &newly_acked_length));
    EXPECT_EQ(0u, QuicStreamPeer::SendBuffer(stream_).size());
    EXPECT_FALSE(stream_->IsWaitingForAcks());
}
...
TEST_P(QuicSpdyStreamTest, HeadersAckNotReportedWriteOrBufferBody)
{
    ...
    quiche::QuicheBuffer header = HttpEncoder::SerializeDataFrameHeader(body.length(), quiche::SimpleBufferAllocator::Get());
    quiche::QuicheBuffer header2 = HttpEncoder::SerializeDataFrameHeader(body2.length(), quiche::SimpleBufferAllocator::Get());

    EXPECT_CALL(*mock_ack_listener, OnPacketAcked(body.length(), _));
    QuicStreamFrame frame(stream_->id(), false, 0, absl::StrCat(header.AsStringView(), body));
    EXPECT_TRUE(session_->OnFrameAcked(QuicFrame(frame), QuicTime::Delta::Zero(), QuicTime::Zero()));

    EXPECT_CALL(*mock_ack_listener, OnPacketAcked(0, _));
    QuicStreamFrame frame2(stream_->id(), false, header.size() + body.length(), header2.AsStringView());
    EXPECT_TRUE(session_->OnFrameAcked(QuicFrame(frame2), QuicTime::Delta::Zero(), QuicTime::Zero()));

    EXPECT_CALL(*mock_ack_listener, OnPacketAcked(body2.length(), _));
    QuicStreamFrame frame3(stream_->id(), true, header.size() + body.length() + header2.size(), body2);
    EXPECT_TRUE(session_->OnFrameAcked(QuicFrame(frame3), QuicTime::Delta::Zero(), QuicTime::Zero()));

    EXPECT_TRUE(QuicSpdyStreamPeer::unacked_frame_headers_offsets(stream_).Empty());
}
...
TEST_P(QuicSpdyStreamTest, HeadersAckNotReportedWriteBodySlices)
{
    ...
    quiche::QuicheMemSliceStorage storage(&body1_iov, 1, helper_.GetStreamSendBufferAllocator(), 1024);
    quiche::QuicheMemSliceStorage storage2(&body2_iov, 1, helper_.GetStreamSendBufferAllocator(), 1024);
    EXPECT_CALL(*session_, WritevData(_, _, _, _, _, _)).Times(AtLeast(1));
    ...
    EXPECT_CALL(*mock_ack_listener, OnPacketAcked(body1.length() + body2.length(), _));
...
TEST_P(QuicSpdyStreamTest, HeaderBytesNotReportedOnRetransmission)
{
    ...
    EXPECT_FALSE(QuicSpdyStreamPeer::unacked_frame_headers_offsets(stream_).Empty());
}
...
TEST_P(QuicSpdyStreamTest, HeadersFrameOnRequestStream)
{
    ...
    std::string trailers = HeadersFrame({ std::make_pair("custom-key", "custom-value") });
    ...
    EXPECT_THAT(stream_->received_trailers(), ElementsAre(Pair("custom-key", "custom-value")));
}
...
TEST_P(QuicSpdyStreamTest, ImmediateHeaderDecodingWithDynamicTableEntries)
{
    ...
    EXPECT_CALL(debug_visitor, OnHeadersFrameReceived(stream_->id(), encoded_headers.length()));
    ...
    EXPECT_THAT(stream_->header_decoding_delay(), Optional(QuicTime::Delta::Zero()));

    // DATA frame.
    std::string data = DataFrame(kDataFramePayload);
    EXPECT_CALL(debug_visitor, OnDataFrameReceived(stream_->id(), kDataFramePayload.length()));
    ...
    EXPECT_CALL(debug_visitor, OnHeadersFrameReceived(stream_->id(), encoded_trailers.length()));
    // Header acknowledgement.
    EXPECT_CALL(debug_visitor, OnHeadersDecoded(stream_->id(), _));
    stream_->OnStreamFrame(QuicStreamFrame(stream_->id(), true, /* offset = */
        headers.length() + data.length(), trailers));

    // Trailers can be decoded immediately.
    EXPECT_TRUE(stream_->trailers_decompressed());

    // Verify trailers.
    EXPECT_THAT(stream_->received_trailers(), ElementsAre(Pair("trailing", "foobar")));
    stream_->MarkTrailersConsumed();
}
...
TEST_P(QuicSpdyStreamTest, BlockedHeaderDecodingAndStopReading)
{
    ...
    if (GetQuicReloadableFlag(quic_stop_reading_also_stops_header_decompression)) {
    ...
    // Deliver dynamic table entry to decoder.
    session_->qpack_decoder()->OnInsertWithoutNameReference("foo", "bar");
    EXPECT_NE(GetQuicReloadableFlag(quic_stop_reading_also_stops_header_decompression), stream_->headers_decompressed());
}
...
TEST_P(QuicSpdyStreamTest, HeaderDecodingUnblockedAfterStreamClosed)
{
    ...
    EXPECT_CALL(*session_, MaybeSendStopSendingFrame(stream_->id(), QuicResetStreamError::FromInternal(QUIC_STREAM_CANCELLED)));
    EXPECT_CALL(*session_, MaybeSendRstStreamFrame(stream_->id(), QuicResetStreamError::FromInternal(QUIC_STREAM_CANCELLED), _));
...
TEST_P(QuicSpdyStreamTest, HeaderDecodingUnblockedAfterResetReceived)
{
    ...
    stream_->OnStreamReset(QuicRstStreamFrame(kInvalidControlFrameId, stream_->id(), QUIC_STREAM_CANCELLED, 0));
...
INSTANTIATE_TEST_SUITE_P(Tests, QuicSpdyStreamIncrementalConsumptionTest, ::testing::ValuesIn(AllSupportedVersions()), ::testing::PrintToStringParamName());
...
TEST_P(QuicSpdyStreamIncrementalConsumptionTest, OnlyKnownFrames)
{
    ...
    QuicByteCount data_frame_header_length = data_frame.size() - data_payload.size();
    ...
    EXPECT_EQ(data_payload.substr(0, data_payload.size() - 1), ReadFromStream(data_payload.size() - 1));
...
TEST_P(QuicSpdyStreamIncrementalConsumptionTest, ReceiveUnsupportedMetadataFrame)
{
    if (!UsesHttp3()) {
        return;
    }
    ...
    QpackEncoder qpack_encoder(&delegate, HuffmanEncoding::kDisabled, CookieCrumbling::kEnabled);
    std::string metadata_frame_payload = qpack_encoder.EncodeHeaderList(stream_->id(), headers,
        /* encoder_stream_sent_byte_count = */ nullptr);
    std::string metadata_frame_header = HttpEncoder::SerializeMetadataFrameHeader(metadata_frame_payload.size());
    std::string metadata_frame = metadata_frame_header + metadata_frame_payload;

    EXPECT_CALL(debug_visitor,
        OnUnknownFrameReceived(stream_->id(), /* frame_type = */ 0x4d,
...
public:
    ...
    MOCK_METHOD(void, OnMetadataComplete, (size_t frame_len, const QuicHeaderList& header_list), (override));
};
...
TEST_P(QuicSpdyStreamIncrementalConsumptionTest, ReceiveMetadataFrame)
{
    ...
    EXPECT_CALL(metadata_visitor, OnMetadataComplete(metadata_frame.size(), _))
        .WillOnce(testing::WithArgs<1>(Invoke([&headers](const QuicHeaderList& header_list) {
            quiche::HttpHeaderBlock actual_headers;
            for (const auto& header : header_list) {
                actual_headers.AppendValueOrAddHeader(header.first, header.second);
            }
...
TEST_P(QuicSpdyStreamIncrementalConsumptionTest, ResetDuringMultipleMetadataFrames)
{
    if (!UsesHttp3()) {
        return;
    }
    ...
    EXPECT_CALL(metadata_visitor, OnMetadataComplete(metadata_frame.size(), _))
        .WillOnce(testing::WithArgs<1>(Invoke([&headers, this](const QuicHeaderList& header_list) {
            quiche::HttpHeaderBlock actual_headers;
            for (const auto& header : header_list) {
                actual_headers.AppendValueOrAddHeader(header.first, header.second);
            }
...
TEST_P(QuicSpdyStreamIncrementalConsumptionTest, UnknownFramesInterleaved)
{
    ...
    // so far, even though last HEADERS frame has not been received completely.
    EXPECT_EQ(data_payload.substr(data_payload.size() - 1), ReadFromStream(1));
    EXPECT_EQ(1 + unknown_frame3.size() + trailers.size() - 1, NewlyConsumedBytes());
...
TEST_P(QuicSpdyStreamTest, DataBeforeHeaders)
{
    ...
    EXPECT_CALL(*connection_,
        CloseConnection(
            QUIC_HTTP_INVALID_FRAME_SEQUENCE_ON_SPDY_STREAM, "Unexpected DATA frame received.", ConnectionCloseBehavior::SEND_CONNECTION_CLOSE_PACKET))
...
TEST_P(QuicSpdyStreamTest, TrailersAfterTrailers)
{
    ...
    stream_->OnStreamFrame(QuicStreamFrame(stream_->id(), false, offset, headers));
    offset += headers.size();
    ...
    std::string trailers1 = HeadersFrame({ std::make_pair("custom-key", "custom-value") });
    stream_->OnStreamFrame(QuicStreamFrame(stream_->id(), false, offset, trailers1));
    ...
    // reading data at the stream level to prevent QuicSpdyStream from blowing up.
    EXPECT_CALL(*connection_,
        CloseConnection(QUIC_HTTP_INVALID_FRAME_SEQUENCE_ON_SPDY_STREAM, "HEADERS frame received after trailing HEADERS.",
            ConnectionCloseBehavior::SEND_CONNECTION_CLOSE_PACKET))
        .WillOnce(InvokeWithoutArgs([this]() { stream_->StopReading(); }));

    // Receive another HEADERS frame, with no header fields.
    std::string trailers2 = HeadersFrame(HttpHeaderBlock());
    stream_->OnStreamFrame(QuicStreamFrame(stream_->id(), false, offset, trailers2));
}
...
TEST_P(QuicSpdyStreamTest, DataAfterTrailers)
{
    ...
    std::string trailers = HeadersFrame({ std::make_pair("custom-key", "custom-value") });
    stream_->OnStreamFrame(QuicStreamFrame(stream_->id(), false, offset, trailers));
...
TEST_P(QuicSpdyStreamTest, WriteHeadersReturnValue)
{
    ...
    QpackSendStream* encoder_stream = QuicSpdySessionPeer::GetQpackEncoderSendStream(session_.get());
    EXPECT_CALL(*session_, WritevData(encoder_stream->id(), _, _, _, _, _)).Times(AnyNumber());

    size_t bytes_written = 0;
    EXPECT_CALL(*session_, WritevData(stream_->id(), _, /* offset = */ 0, _, _, _))
        .WillOnce(DoAll(SaveArg<1>(&bytes_written), Invoke(session_.get(), &MockQuicSpdySession::ConsumeData)));

    HttpHeaderBlock request_headers;
    request_headers["foo"] = "bar";
    size_t write_headers_return_value = stream_->WriteHeaders(std::move(request_headers), /*fin=*/true, nullptr);
    EXPECT_TRUE(stream_->fin_sent());
...
TEST_P(QuicSpdyStreamTest, TwoResetStreamFrames)
{
    ...
    QuicRstStreamFrame rst_frame1(kInvalidControlFrameId, stream_->id(), QUIC_STREAM_CANCELLED, /* bytes_written = */ 0);
    stream_->OnStreamReset(rst_frame1);
    EXPECT_TRUE(stream_->read_side_closed());
    EXPECT_FALSE(stream_->write_side_closed());

    QuicRstStreamFrame rst_frame2(kInvalidControlFrameId, stream_->id(), QUIC_STREAM_NO_ERROR, /* bytes_written = */ 0);
...
TEST_P(QuicSpdyStreamTest, ProcessOutgoingWebTransportHeaders)
{
    ...
    QuicSpdySessionPeer::SetHttpDatagramSupport(session_.get(), HttpDatagramSupport::kRfc);

    EXPECT_CALL(*stream_, WriteHeadersMock(false));
    EXPECT_CALL(*session_, WritevData(stream_->id(), _, _, _, _, _)).Times(AnyNumber());
...
TEST_P(QuicSpdyStreamTest, IncomingWebTransportStreamWhenUnsupported)
{
    ...
    ASSERT_TRUE(absl::HexStringToBytes("40410400000000", &webtransport_stream_frame));
    QuicStreamFrame stream_frame(stream_->id(), /* fin = */ false,
...
TEST_P(QuicSpdyStreamTest, IncomingWebTransportStreamWithPaddingDraft07)
{
    ...
    EXPECT_CALL(*connection_, CloseConnection(QUIC_HTTP_INVALID_FRAME_SEQUENCE_ON_SPDY_STREAM, HasSubstr("non-zero offset"), _));
    stream_->OnStreamFrame(stream_frame);
    EXPECT_TRUE(stream_->web_transport_stream() == nullptr);
}
...
TEST_P(QuicSpdyStreamTest, ReceiveHttpDatagram)
{
    ...
    session_->OnMessageReceived(absl::string_view(datagram.data(), datagram.size()));
    EXPECT_THAT(h3_datagram_visitor.received_h3_datagrams(),
        ElementsAre(SavingHttp3DatagramVisitor::SavedHttp3Datagram { stream_->id(), std::string(&datagram[1], datagram.size() - 1) }));
    // Test move.
...
TEST_P(QuicSpdyStreamTest, SendHttpDatagram)
{
    ...
    EXPECT_EQ(stream_->SendHttp3Datagram(http_datagram_payload), MESSAGE_STATUS_SUCCESS);
}
...
TEST_P(QuicSpdyStreamTest, SendHttpDatagramWithoutLocalSupport)
{
    ...
    std::string http_datagram_payload = { 1, 2, 3, 4, 5, 6 };
    EXPECT_QUIC_BUG(stream_->SendHttp3Datagram(http_datagram_payload), "Cannot send HTTP Datagram when disabled locally");
}
...
TEST_P(QuicSpdyStreamTest, SendHttpDatagramBeforeReceivingSettings)
{
    ...
    session_->set_local_http_datagram_support(HttpDatagramSupport::kRfc);
    std::string http_datagram_payload = { 1, 2, 3, 4, 5, 6 };
    EXPECT_EQ(stream_->SendHttp3Datagram(http_datagram_payload), MESSAGE_STATUS_SETTINGS_NOT_RECEIVED);
}
...
TEST_P(QuicSpdyStreamTest, SendHttpDatagramWithoutPeerSupport)
{
    ...
    std::string http_datagram_payload = { 1, 2, 3, 4, 5, 6 };
    EXPECT_EQ(stream_->SendHttp3Datagram(http_datagram_payload), MESSAGE_STATUS_UNSUPPORTED);
}
...
TEST_P(QuicSpdyStreamTest, Capsules)
{
    ...
    EXPECT_THAT(
        h3_datagram_visitor.received_h3_datagrams(), ElementsAre(SavingHttp3DatagramVisitor::SavedHttp3Datagram { stream_->id(), http_datagram_payload }));
    ...
    ip_prefix_with_id.ip_prefix = quiche::QuicheIpPrefix(ip_address, /*prefix_length=*/96);
    Capsule address_assign_capsule = Capsule::AddressAssign();
    address_assign_capsule.address_assign_capsule().assigned_addresses.push_back(ip_prefix_with_id);
    stream_->OnCapsule(address_assign_capsule);
    EXPECT_THAT(connect_ip_visitor.received_address_assign_capsules(), ElementsAre(address_assign_capsule.address_assign_capsule()));
    // Address request capsule.
    Capsule address_request_capsule = Capsule::AddressRequest();
    address_request_capsule.address_request_capsule().requested_addresses.push_back(ip_prefix_with_id);
    stream_->OnCapsule(address_request_capsule);
    EXPECT_THAT(connect_ip_visitor.received_address_request_capsules(), ElementsAre(address_request_capsule.address_request_capsule()));
    ...
    route_advertisement_capsule.route_advertisement_capsule().ip_address_ranges.push_back(ip_address_range);
    stream_->OnCapsule(route_advertisement_capsule);
    EXPECT_THAT(connect_ip_visitor.received_route_advertisement_capsules(), ElementsAre(route_advertisement_capsule.route_advertisement_capsule()));
    // Unknown capsule.
    ...
    EXPECT_THAT(h3_datagram_visitor.received_unknown_capsules(),
        ElementsAre(SavingHttp3DatagramVisitor::SavedUnknownCapsule { stream_->id(), capsule_type, capsule_payload }));
    // Cleanup.
    stream_->UnregisterHttp3DatagramVisitor();
    stream_->UnregisterConnectIpVisitor();
}

TEST_P(QuicSpdyStreamTest, QUIC_TEST_DISABLED_IN_CHROME(HeadersAccumulatorNullptr))
{
    if (!UsesHttp3()) {
        return;
    }
...
TEST_P(QuicSpdyStreamTest, ColonDisallowedInHeaderName)
{
    ...
    EXPECT_EQ("Invalid character in header name foo:bar", stream_->invalid_request_details());
}
