...
public:
    ...
    static std::string GetPromisedUrlFromHeaders(const spdy::Http2HeaderBlock& headers);

    // Returns hostname, or empty string if missing.
    static std::string GetPromisedHostNameFromHeaders(const spdy::Http2HeaderBlock& headers);
    ...
    static std::string GetPushPromiseUrl(absl::string_view scheme, absl::string_view authority, absl::string_view path);
};
