...
#define ENDPOINT (session_->perspective() == Perspective::IS_SERVER ? "Server: " : "Client: ")
...
WebTransportHttp3::WebTransportHttp3(QuicSpdySession* session, QuicSpdyStream* connect_stream, WebTransportSessionId id)
    : session_(session)
...
void WebTransportHttp3::AssociateStream(QuicStreamId stream_id)
{
    ...
    if (QuicUtils::IsOutgoingStreamId(version, stream_id, session_->perspective())) {
        return;
    }
...
void WebTransportHttp3::CloseSession(WebTransportSessionError error_code, absl::string_view error_message)
{
    if (close_sent_) {
        QUIC_BUG(WebTransportHttp3 close sent twice) << "Calling WebTransportHttp3::CloseSession() more than once is not "
                                                        "allowed.";
        return;
    }
    close_sent_ = true;
    ...
    QuicConnection::ScopedPacketFlusher flusher(connect_stream_->spdy_session()->connection());
    connect_stream_->WriteCapsule(quiche::Capsule::CloseWebTransportSession(error_code, error_message),
        /*fin=*/true);
}

void WebTransportHttp3::OnCloseReceived(WebTransportSessionError error_code, absl::string_view error_message)
{
    if (close_received_) {
        QUIC_BUG(WebTransportHttp3 notified of close received twice) << "WebTransportHttp3::OnCloseReceived() may be only called once.";
    }
...
void WebTransportHttp3::HeadersReceived(const quiche::HttpHeaderBlock& headers)
{
...
WebTransportStream* WebTransportHttp3::AcceptIncomingUnidirectionalStream()
{
    while (!incoming_unidirectional_streams_.empty()) {
        ...
        return static_cast<WebTransportHttp3UnidirectionalStream*>(stream)->interface();
    }
    return nullptr;
}
...
{
    QuicSpdyStream* stream = session_->CreateOutgoingBidirectionalWebTransportStream(this);
    if (stream == nullptr) {
...
{
    WebTransportHttp3UnidirectionalStream* stream = session_->CreateOutgoingUnidirectionalWebTransportStream(this);
    if (stream == nullptr) {
...
webtransport::Stream* WebTransportHttp3::GetStreamById(webtransport::StreamId id)
{
    if (!streams_.contains(id)) {
        return nullptr;
    }
    QuicStream* stream = session_->GetActiveStream(id);
    const bool bidi = QuicUtils::IsBidirectionalStreamId(id, ParsedQuicVersion::RFCv1()); // Assume IETF QUIC for WebTransport
    if (bidi) {
...
webtransport::DatagramStatus WebTransportHttp3::SendOrQueueDatagram(absl::string_view datagram)
{
    return MessageStatusToWebTransportStatus(connect_stream_->SendHttp3Datagram(datagram));
}
...
void WebTransportHttp3::SetDatagramMaxTimeInQueue(absl::Duration max_time_in_queue)
{
...
{
    if (!drain_sent_) {
        connect_stream_->WriteCapsule(quiche::Capsule(quiche::DrainWebTransportSessionCapsule()));
        drain_sent_ = true;
    }
}

void WebTransportHttp3::OnHttp3Datagram(QuicStreamId stream_id, absl::string_view payload)
{
...
WebTransportHttp3UnidirectionalStream::WebTransportHttp3UnidirectionalStream(PendingStream* pending, QuicSpdySession* session)
...
WebTransportHttp3UnidirectionalStream::WebTransportHttp3UnidirectionalStream(QuicStreamId id, QuicSpdySession* session, WebTransportSessionId session_id)
...
void WebTransportHttp3UnidirectionalStream::WritePreamble()
{
    if (!needs_to_send_preamble_ || !session_id_.has_value()) {
        QUIC_BUG(WebTransportHttp3UnidirectionalStream duplicate preamble) << ENDPOINT << "Sending preamble on stream ID " << id() << " at the wrong time.";
    ...
    QUIC_DVLOG(1) << ENDPOINT << "Sent stream type and session ID (" << *session_id_ << ") on WebTransport stream " << id();
    needs_to_send_preamble_ = false;
}
...
bool WebTransportHttp3UnidirectionalStream::ReadSessionId()
{
    ...
    if (!reader.ReadVarInt62(&session_id)) {
        ...
        if (sequencer()->IsAllDataAvailable()) {
            QUIC_DLOG(WARNING) << ENDPOINT << "Failed to associate WebTransport stream " << id() << " with a session because the stream ended prematurely.";
...
void WebTransportHttp3UnidirectionalStream::OnClose()
{
    ...
    if (session == nullptr) {
        QUIC_DLOG(WARNING) << ENDPOINT << "WebTransport stream " << id() << " attempted to notify parent session " << *session_id_
...
void WebTransportHttp3UnidirectionalStream::OnStreamReset(const QuicRstStreamFrame& frame)
{
    if (adapter_.visitor() != nullptr) {
        adapter_.visitor()->OnResetStreamReceived(Http3ErrorToWebTransportOrDefault(frame.ietf_error_code));
    }
    QuicStream::OnStreamReset(frame);
}
bool WebTransportHttp3UnidirectionalStream::OnStopSending(QuicResetStreamError error)
{
    if (adapter_.visitor() != nullptr) {
        adapter_.visitor()->OnStopSendingReceived(Http3ErrorToWebTransportOrDefault(error.ietf_application_code()));
    }
...
std::optional<WebTransportStreamError> Http3ErrorToWebTransport(uint64_t http3_error_code)
{
    // Ensure the code is within the valid range.
    if (http3_error_code < kWebTransportMappedErrorCodeFirst || http3_error_code > kWebTransportMappedErrorCodeLast) {
        return std::nullopt;
    }
    ...
    QUICHE_DCHECK_LE(result, std::numeric_limits<webtransport::StreamErrorCode>::max());
    return static_cast<WebTransportStreamError>(result);
}

WebTransportStreamError Http3ErrorToWebTransportOrDefault(uint64_t http3_error_code)
{
    std::optional<WebTransportStreamError> result = Http3ErrorToWebTransport(http3_error_code);
    return result.has_value() ? *result : kDefaultWebTransportError;
}

uint64_t WebTransportErrorToHttp3(WebTransportStreamError webtransport_error_code)
{
    return kWebTransportMappedErrorCodeFirst + webtransport_error_code + webtransport_error_code / 0x1e;
}
