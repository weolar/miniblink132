...
QuicSpdyClientSession::QuicSpdyClientSession(const QuicConfig& config, const ParsedQuicVersionVector& supported_versions, QuicConnection* connection,
    const QuicServerId& server_id, QuicCryptoClientConfig* crypto_config)
    : QuicSpdyClientSession(config, supported_versions, connection, nullptr, server_id, crypto_config)
{
}

QuicSpdyClientSession::QuicSpdyClientSession(const QuicConfig& config, const ParsedQuicVersionVector& supported_versions, QuicConnection* connection,
    QuicSession::Visitor* visitor, const QuicServerId& server_id, QuicCryptoClientConfig* crypto_config)
...
void QuicSpdyClientSession::OnProofValid(const QuicCryptoClientConfig::CachedState& /*cached*/)
{
}

void QuicSpdyClientSession::OnProofVerifyDetailsAvailable(const ProofVerifyDetails& /*verify_details*/)
{
}
...
bool QuicSpdyClientSession::ShouldCreateOutgoingBidirectionalStream()
{
    if (!crypto_stream_->encryption_established()) {
        ...
        QUIC_CODE_COUNT(quic_client_fails_to_create_stream_encryption_not_established);
        return false;
    }
...
QuicSpdyClientStream* QuicSpdyClientSession::CreateOutgoingBidirectionalStream()
{
...
QuicSpdyClientStream* QuicSpdyClientSession::CreateOutgoingUnidirectionalStream()
{
    QUIC_BUG(quic_bug_10396_2) << "Try to create outgoing unidirectional client data streams";
    return nullptr;
}

std::unique_ptr<QuicSpdyClientStream> QuicSpdyClientSession::CreateClientStream()
{
    return std::make_unique<QuicSpdyClientStream>(GetNextOutgoingBidirectionalStreamId(), this, BIDIRECTIONAL);
}
...
const QuicCryptoClientStreamBase* QuicSpdyClientSession::GetCryptoStream() const
{
    return crypto_stream_.get();
}
...
bool QuicSpdyClientSession::ShouldCreateIncomingStream(QuicStreamId id)
{
    ...
    if (QuicUtils::IsClientInitiatedStreamId(transport_version(), id)) {
        QUIC_BUG(quic_bug_10396_4) << "ShouldCreateIncomingStream called with client initiated "
                                      "stream ID.";
        return false;
    }
    ...
    if (QuicUtils::IsClientInitiatedStreamId(transport_version(), id)) {
        ...
        connection()->CloseConnection(
            QUIC_INVALID_STREAM_ID, "Server created non write unidirectional stream", ConnectionCloseBehavior::SEND_CONNECTION_CLOSE_PACKET);
        return false;
    }

    if (VersionHasIetfQuicFrames(transport_version()) && QuicUtils::IsBidirectionalStreamId(id, version()) && !WillNegotiateWebTransport()) {
        connection()->CloseConnection(
            QUIC_HTTP_SERVER_INITIATED_BIDIRECTIONAL_STREAM, "Server created bidirectional stream.", ConnectionCloseBehavior::SEND_CONNECTION_CLOSE_PACKET);
        return false;
    }
...
QuicSpdyStream* QuicSpdyClientSession::CreateIncomingStream(PendingStream* pending)
{
...
QuicSpdyStream* QuicSpdyClientSession::CreateIncomingStream(QuicStreamId id)
{
    ...
    if (version().UsesHttp3() && QuicUtils::IsBidirectionalStreamId(id, version())) {
        QUIC_BUG_IF(QuicServerInitiatedSpdyStream but no WebTransport support, !WillNegotiateWebTransport())
...
std::unique_ptr<QuicCryptoClientStreamBase> QuicSpdyClientSession::CreateQuicCryptoStream()
{
    return std::make_unique<QuicCryptoClientStream>(
        server_id_, this, crypto_config_->proof_verifier()->CreateDefaultContext(), crypto_config_, this, /*has_application_state = */ version().UsesHttp3());
}
