...
public:
    MetadataDecoder(QuicStreamId id, size_t max_header_list_size, size_t frame_header_len, size_t payload_length);
    ...
    {
        return decoder_.error_message();
    }
    size_t frame_len()
    {
        return frame_len_;
    }
    const QuicHeaderList& headers()
    {
        return decoder_.headers();
    }
