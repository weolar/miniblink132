...
public:
    ...
    virtual void OnPriorityUpdateFrameReceived(const PriorityUpdateFrame& /*frame*/) = 0;
    ...
    virtual void OnDataFrameReceived(QuicStreamId /*stream_id*/, QuicByteCount /*payload_length*/) = 0;
    virtual void OnHeadersFrameReceived(QuicStreamId /*stream_id*/, QuicByteCount /*compressed_headers_length*/) = 0;
    virtual void OnHeadersDecoded(QuicStreamId /*stream_id*/, QuicHeaderList /*headers*/) = 0;

    // Incoming HTTP/3 frames of unknown type on any stream.
    virtual void OnUnknownFrameReceived(QuicStreamId /*stream_id*/, uint64_t /*frame_type*/, QuicByteCount /*payload_length*/) = 0;
    ...
    virtual void OnPriorityUpdateFrameSent(const PriorityUpdateFrame& /*frame*/) = 0;

    // Outgoing HTTP/3 frames on request or push streams.
    virtual void OnDataFrameSent(QuicStreamId /*stream_id*/, QuicByteCount /*payload_length*/) = 0;
    virtual void OnHeadersFrameSent(QuicStreamId /*stream_id*/, const quiche::HttpHeaderBlock& /*header_block*/) = 0;
...
inline constexpr WebTransportHttp3VersionSet kDefaultSupportedWebTransportVersions
    = WebTransportHttp3VersionSet({ WebTransportHttp3Version::kDraft02, WebTransportHttp3Version::kDraft07 });

QUICHE_EXPORT std::string HttpDatagramSupportToString(HttpDatagramSupport http_datagram_support);
QUICHE_EXPORT std::ostream& operator<<(std::ostream& os, const HttpDatagramSupport& http_datagram_support);

// A QUIC session for HTTP.
class QUICHE_EXPORT QuicSpdySession : public QuicSession, public QpackEncoder::DecoderStreamErrorDelegate, public QpackDecoder::EncoderStreamErrorDelegate {
public:
    // Does not take ownership of |connection| or |visitor|.
    QuicSpdySession(QuicConnection* connection, QuicSession::Visitor* visitor, const QuicConfig& config, const ParsedQuicVersionVector& supported_versions);
    ...
    void OnDecoderStreamError(QuicErrorCode error_code, absl::string_view error_message) override;

    // QpackDecoder::EncoderStreamErrorDelegate implementation.
    void OnEncoderStreamError(QuicErrorCode error_code, absl::string_view error_message) override;

    // Called by |headers_stream_| when headers with a priority have been
    // received for a stream.  This method will only be called for server streams.
    virtual void OnStreamHeadersPriority(QuicStreamId stream_id, const spdy::SpdyStreamPrecedence& precedence);

    // Called by |headers_stream_| when headers have been completely received
    // for a stream.  |fin| will be true if the fin flag was set in the headers
    // frame.
    virtual void OnStreamHeaderList(QuicStreamId stream_id, bool fin, size_t frame_len, const QuicHeaderList& header_list);

    // Called by |headers_stream_| when a PRIORITY frame has been received for a
    // stream. This method will only be called for server streams.
    virtual void OnPriorityFrame(QuicStreamId stream_id, const spdy::SpdyStreamPrecedence& precedence);
    ...
    bool OnPriorityUpdateForRequestStream(QuicStreamId stream_id, HttpStreamPriority priority);
    ...
    virtual void OnUnknownFrameStart(QuicStreamId /*stream_id*/, uint64_t /*frame_type*/, QuicByteCount /*header_length*/, QuicByteCount /*payload_length*/)
    {
    }
    virtual void OnUnknownFramePayload(QuicStreamId /*stream_id*/, absl::string_view /*payload*/)
    {
    }
    ...
    virtual size_t WriteHeadersOnHeadersStream(QuicStreamId id, quiche::HttpHeaderBlock headers, bool fin, const spdy::SpdyStreamPrecedence& precedence,
    ...
    size_t WritePriority(QuicStreamId stream_id, QuicStreamId parent_stream_id, int weight, bool exclusive);

    // Writes an HTTP/3 PRIORITY_UPDATE frame to the peer.
    void WriteHttp3PriorityUpdate(QuicStreamId stream_id, HttpStreamPriority priority);
    ...
    void CloseConnectionWithDetails(QuicErrorCode error, const std::string& details);

    // Must not be called after Initialize().
    // TODO(bnc): Move to constructor argument.
    void set_qpack_maximum_dynamic_table_capacity(uint64_t qpack_maximum_dynamic_table_capacity)
    {
        qpack_maximum_dynamic_table_capacity_ = qpack_maximum_dynamic_table_capacity;
    }
    ...
    // Must not be called after Initialize().
    // TODO(bnc): Move to constructor argument.
    void set_qpack_maximum_blocked_streams(uint64_t qpack_maximum_blocked_streams)
    {
    ...
    bool allow_extended_connect() const
    {
        return allow_extended_connect_;
    }
    ...
    int32_t destruction_indicator() const
    {
        return destruction_indicator_;
    }
    ...
    Http3DebugVisitor* debug_visitor()
    {
    ...
    static void LogHeaderCompressionRatioHistogram(bool using_qpack, bool is_sent, QuicByteCount compressed, QuicByteCount uncompressed);
    ...
    {
        return (qpack_encoder_ && qpack_encoder_->dynamic_table_entry_referenced()) || (qpack_decoder_ && qpack_decoder_->dynamic_table_entry_referenced());
    }
    ...
    std::optional<std::string> OnAlpsData(const uint8_t* alps_data, size_t alps_length) override;
    ...
    MessageStatus SendHttp3Datagram(QuicStreamId stream_id, absl::string_view payload);
    // This must not be used except by QuicSpdyStream::SetMaxDatagramTimeInQueue.
    void SetMaxDatagramTimeInQueueForStreamId(QuicStreamId stream_id, QuicTime::Delta max_time_in_queue);
    ...
    bool ShouldBufferRequestsUntilSettings()
    {
        return version().UsesHttp3() && perspective() == Perspective::IS_SERVER
            && (ShouldNegotiateWebTransport() || LocalHttpDatagramSupport() == HttpDatagramSupport::kRfcAndDraft04 || force_buffer_requests_until_settings_);
    }
    ...
    void AssociateIncomingWebTransportStreamWithSession(WebTransportSessionId session_id, QuicStreamId stream_id);

    void ProcessBufferedWebTransportStreamsForSession(WebTransportHttp3* session);

    bool CanOpenOutgoingUnidirectionalWebTransportStream(WebTransportSessionId /*id*/)
    {
        return CanOpenNextOutgoingUnidirectionalStream();
    }
    bool CanOpenOutgoingBidirectionalWebTransportStream(WebTransportSessionId /*id*/)
    {
    ...
    WebTransportHttp3UnidirectionalStream* CreateOutgoingUnidirectionalWebTransportStream(WebTransportHttp3* session);

    // Creates an outgoing bidirectional WebTransport stream.  Returns nullptr if
    // the stream cannot be created due to flow control or some other reason.
    QuicSpdyStream* CreateOutgoingBidirectionalWebTransportStream(WebTransportHttp3* session);
...
protected:
    ...
    virtual WebTransportHttp3VersionSet LocallySupportedWebTransportVersions() const;
    ...
    size_t WriteHeadersOnHeadersStreamImpl(QuicStreamId id, quiche::HttpHeaderBlock headers, bool fin, QuicStreamId parent_stream_id, int weight,
    ...
    const SettingsFrame& settings() const
    {
        return settings_;
    }
...
private:
    ...
    public:
        explicit DatagramObserver(QuicSpdySession* session)
            : session_(session)
        {
        }
    ...
    void OnHeaders(spdy::SpdyStreamId stream_id, bool has_priority, const spdy::SpdyStreamPrecedence& precedence, bool fin);

    // Called when a PRIORITY frame has been received.
    void OnPriority(spdy::SpdyStreamId stream_id, const spdy::SpdyStreamPrecedence& precedence);

    void CloseConnectionOnDuplicateHttp3UnidirectionalStreams(absl::string_view type);
    ...
    std::optional<WebTransportHttp3Version> NegotiatedWebTransportVersion() const
    {
        return (LocallySupportedWebTransportVersions() & peer_web_transport_versions_).Max();
    }
    ...
    // open yet.
    absl::flat_hash_map<QuicStreamId, HttpStreamPriority> buffered_stream_priorities_;
    ...
    absl::flat_hash_map<WebTransportHttp3Version, QuicStreamCount> max_webtransport_sessions_;
