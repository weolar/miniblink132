...
public:
        ...
        : decoder_(&visitor_)
    {
        ...
        ON_CALL(visitor_, OnPriorityUpdateFrameStart(_)).WillByDefault(Return(true));
...
TEST_F(HttpDecoderTest, UnknownFrame)
{
    ...
    for (auto payload_length : payload_lengths) {
        ...
        for (auto frame_type : frame_types) {
            const QuicByteCount total_length = QuicDataWriter::GetVarInt62Len(frame_type) + QuicDataWriter::GetVarInt62Len(payload_length) + payload_length;
            ...
            EXPECT_CALL(visitor_, OnUnknownFrameStart(frame_type, header_length, payload_length));
            if (payload_length > 0) {
...
TEST_F(HttpDecoderTest, PushPromiseFrame)
{
    ...
    ASSERT_TRUE(absl::HexStringToBytes("05" // type (PUSH PROMISE)
                                       "08" // length
                                       "1f", // push id 31
        &push_promise_bytes));
...
TEST_F(HttpDecoderTest, MaxPushId)
{
    InSequence s;
    std::string input;
    ASSERT_TRUE(absl::HexStringToBytes("0D" // type (MAX_PUSH_ID)
                                       "01" // length
                                       "01", // Push Id
        &input));
...
TEST_F(HttpDecoderTest, SettingsFrame)
{
    ...
    QuicByteCount processed_bytes = ProcessInputWithGarbageAppended(remaining_input);
...
{
    const char* const kPayload = "\x42\x11" // two-byte id
    ...
    for (const auto& test_data : kTestData) {
        ...
        QuicByteCount processed_bytes = decoder.ProcessInput(input.data(), input.size());
...
TEST_F(HttpDecoderTest, DuplicateSettingsIdentifier)
{
    ...
    EXPECT_THAT(decoder_.error(), IsError(QUIC_HTTP_DUPLICATE_SETTING_IDENTIFIER));
...
TEST_F(HttpDecoderTest, DataFrame)
{
    ...
    EXPECT_CALL(visitor_, OnDataFramePayload(absl::string_view("Data!"))).WillOnce(Return(false));
    processed_bytes = ProcessInputWithGarbageAppended(remaining_input);
    EXPECT_EQ(remaining_input.size(), processed_bytes);
...
TEST_F(HttpDecoderTest, FrameHeaderPartialDelivery)
{
    ...
    quiche::QuicheBuffer header = HttpEncoder::SerializeDataFrameHeader(input.length(), quiche::SimpleBufferAllocator::Get());
    ...
    EXPECT_EQ(header.size() - 1, decoder_.ProcessInput(header.data() + 1, header.size() - 1));
    EXPECT_THAT(decoder_.error(), IsQuicNoError());
    EXPECT_EQ("", decoder_.error_detail());
...
TEST_F(HttpDecoderTest, PartialDeliveryOfLargeFrameType)
{
    ...
    const QuicByteCount header_length = QuicDataWriter::GetVarInt62Len(frame_type) + QuicDataWriter::GetVarInt62Len(payload_length);
...
TEST_F(HttpDecoderTest, GoAway)
{
    InSequence s;
    std::string input;
    ASSERT_TRUE(absl::HexStringToBytes("07" // type (GOAWAY)
                                       "01" // length
                                       "01", // ID
        &input));

    // Visitor pauses processing.
    EXPECT_CALL(visitor_, OnGoAwayFrame(GoAwayFrame({ 1 }))).WillOnce(Return(false));
    EXPECT_EQ(input.size(), ProcessInputWithGarbageAppended(input));
    EXPECT_THAT(decoder_.error(), IsQuicNoError());
    EXPECT_EQ("", decoder_.error_detail());
...
TEST_F(HttpDecoderTest, HeadersFrame)
{
    InSequence s;
    std::string type_and_length_bytes;
    ASSERT_TRUE(absl::HexStringToBytes("01" // type (HEADERS)
                                       "07", // length
        &type_and_length_bytes));
    std::string input = absl::StrCat(type_and_length_bytes,
        "Headers"); // headers
    ...
    EXPECT_CALL(visitor_, OnHeadersFramePayload(absl::string_view("Headers"))).WillOnce(Return(false));
    processed_bytes = ProcessInputWithGarbageAppended(remaining_input);
    EXPECT_EQ(remaining_input.size(), processed_bytes);
...
TEST_F(HttpDecoderTest, MetadataFrame)
{
    InSequence s;
    std::string type_and_length_bytes;
    ASSERT_TRUE(absl::HexStringToBytes("404d" // 2 byte type (METADATA)
                                       "08", // length
        &type_and_length_bytes));
    std::string input = absl::StrCat(type_and_length_bytes,
        "Metadata"); // headers
    ...
    EXPECT_CALL(visitor_, OnMetadataFramePayload(absl::string_view("Metadata"))).WillOnce(Return(false));
    processed_bytes = ProcessInputWithGarbageAppended(remaining_input);
    EXPECT_EQ(remaining_input.size(), processed_bytes);
...
TEST_F(HttpDecoderTest, FrameWithOverlyLargePayload)
{
    ...
    for (uint64_t frame_type = 0; frame_type < 1025; frame_type++) {
        ...
        ASSERT_TRUE(writer.WriteVarInt62(quiche::kVarInt62MaxValue)); // frame length.
...
{
    std::string input;
    ASSERT_TRUE(absl::HexStringToBytes("06" // PING in HTTP/2 but not supported in HTTP/3.
                                       "05" // length
                                       "15", // random payload
        &input));
    ...
    EXPECT_EQ("HTTP/2 frame received in a HTTP/3 connection: 6", decoder_.error_detail());
}
...
TEST_F(HttpDecoderTest, LargeStreamIdInGoAway)
{
    ...
    EXPECT_EQ(goaway.length(), decoder_.ProcessInput(goaway.data(), goaway.length()));
    EXPECT_THAT(decoder_.error(), IsQuicNoError());
    EXPECT_EQ("", decoder_.error_detail());
}
...
TEST_F(HttpDecoderTest, ObsoletePriorityUpdateFrame)
{
    ...
    InSequence s;
    std::string input;
    ASSERT_TRUE(absl::HexStringToBytes("0f" // type (obsolete PRIORITY_UPDATE)
                                       "03" // length
                                       "666f6f", // payload "foo"
        &input));

    // Process frame as a whole.
    EXPECT_CALL(visitor_, OnUnknownFrameStart(0x0f, header_length, payload_length));
    EXPECT_CALL(visitor_, OnUnknownFramePayload(Eq("foo")));
    EXPECT_CALL(visitor_, OnUnknownFrameEnd()).WillOnce(Return(false));

    EXPECT_EQ(header_length + payload_length, ProcessInputWithGarbageAppended(input));
    EXPECT_THAT(decoder_.error(), IsQuicNoError());
    EXPECT_EQ("", decoder_.error_detail());
...
TEST_F(HttpDecoderTest, PriorityUpdateFrame)
{
    ...
    EXPECT_CALL(visitor_, OnPriorityUpdateFrame(priority_update1)).WillOnce(Return(false));
    processed_bytes = ProcessInputWithGarbageAppended(remaining_input);
    EXPECT_EQ(remaining_input.size(), processed_bytes);
    EXPECT_THAT(decoder_.error(), IsQuicNoError());
    EXPECT_EQ("", decoder_.error_detail());
    ...
    EXPECT_CALL(visitor_, OnPriorityUpdateFrame(priority_update2)).WillOnce(Return(false));
    processed_bytes = ProcessInputWithGarbageAppended(remaining_input);
    EXPECT_EQ(remaining_input.size(), processed_bytes);
    EXPECT_THAT(decoder_.error(), IsQuicNoError());
    EXPECT_EQ("", decoder_.error_detail());
