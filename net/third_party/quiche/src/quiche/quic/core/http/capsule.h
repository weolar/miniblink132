...
enum class CapsuleType : uint64_t {
    ...
    DATAGRAM_WITHOUT_CONTEXT = 0xff37a5, // draft-ietf-masque-h3-datagram-05 to -08.
...
QUIC_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, const CapsuleType& capsule_type);
...
public:
    ...
    static Capsule DatagramWithoutContext(absl::string_view http_datagram_payload = absl::string_view());
    static Capsule CloseWebTransportSession(WebTransportSessionError error_code = 0, absl::string_view error_message = "");
    ...
    friend QUIC_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os, const Capsule& capsule);

    CapsuleType capsule_type() const
    {
        return capsule_type_;
    }
    ...
    const DatagramWithoutContextCapsule& datagram_without_context_capsule() const
    {
        QUICHE_DCHECK_EQ(capsule_type_, CapsuleType::DATAGRAM_WITHOUT_CONTEXT);
        return datagram_without_context_capsule_;
    }
    ...
    {
        QUICHE_DCHECK(capsule_type_ != CapsuleType::LEGACY_DATAGRAM && capsule_type_ != CapsuleType::DATAGRAM_WITHOUT_CONTEXT
            && capsule_type_ != CapsuleType::CLOSE_WEBTRANSPORT_SESSION && capsule_type_ != CapsuleType::ADDRESS_REQUEST
            && capsule_type_ != CapsuleType::ADDRESS_ASSIGN && capsule_type_ != CapsuleType::ROUTE_ADVERTISEMENT)
            << capsule_type_;
...
QUIC_EXPORT_PRIVATE quiche::QuicheBuffer SerializeCapsule(const Capsule& capsule, quiche::QuicheBufferAllocator* allocator);
