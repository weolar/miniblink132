...
public:
    QuicSpdyClientStream(QuicStreamId id, QuicSpdyClientSession* session, StreamType type);
    QuicSpdyClientStream(PendingStream* pending, QuicSpdyClientSession* spdy_session);
    ...
    void OnTrailingHeadersComplete(bool fin, size_t frame_len, const QuicHeaderList& header_list) override;
    ...
    size_t SendRequest(quiche::HttpHeaderBlock headers, absl::string_view body, bool fin);
    ...
    size_t header_bytes_read() const
    {
        return header_bytes_read_;
    }

    size_t header_bytes_written() const
    {
        return header_bytes_written_;
    }
...
protected:
    ...
    // on error.
    virtual bool CopyAndValidateHeaders(const QuicHeaderList& header_list, int64_t& content_length, quiche::HttpHeaderBlock& headers);
...
private:
    ...
    QuicTime::Delta time_to_response_headers_received_ = QuicTime::Delta::Infinite();
