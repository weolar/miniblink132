...
QuicSpdyClientStream::QuicSpdyClientStream(QuicStreamId id, QuicSpdyClientSession* session, StreamType type)
...
QuicSpdyClientStream::QuicSpdyClientStream(PendingStream* pending, QuicSpdyClientSession* session)
...
bool QuicSpdyClientStream::CopyAndValidateHeaders(const QuicHeaderList& header_list, int64_t& content_length, quiche::HttpHeaderBlock& headers)
{
    return SpdyUtils::CopyAndValidateHeaders(header_list, &content_length, &headers);
}

bool QuicSpdyClientStream::ParseAndValidateStatusCode()
{
    if (!ParseHeaderStatusCode(response_headers_, &response_code_)) {
        QUIC_DLOG(ERROR) << "Received invalid response code: " << response_headers_[":status"].as_string() << " on stream " << id();
    ...
    if (response_code_ >= 100 && response_code_ < 200) {
        ...
        QUIC_DLOG(INFO) << "Received informational response code: " << response_headers_[":status"].as_string() << " on stream " << id();
...
void QuicSpdyClientStream::OnInitialHeadersComplete(bool fin, size_t frame_len, const QuicHeaderList& header_list)
{
    QuicSpdyStream::OnInitialHeadersComplete(fin, frame_len, header_list);
    time_to_response_headers_received_ = session()->GetClock()->ApproximateNow() - creation_time();
    ...
    if (!CopyAndValidateHeaders(header_list, content_length_, response_headers_)) {
        QUIC_DLOG(ERROR) << "Failed to parse header list: " << header_list.DebugString() << " on stream " << id();
        Reset(QUIC_BAD_APPLICATION_PAYLOAD);
        return;
    }
...
void QuicSpdyClientStream::OnTrailingHeadersComplete(bool fin, size_t frame_len, const QuicHeaderList& header_list)
{
...
void QuicSpdyClientStream::OnBodyAvailable()
{
    while (HasBytesToRead()) {
        ...
        if (content_length_ >= 0 && data_.size() > static_cast<uint64_t>(content_length_)) {
            QUIC_DLOG(ERROR) << "Invalid content length (" << content_length_ << ") with data of size " << data_.size();
            Reset(QUIC_BAD_APPLICATION_PAYLOAD);
            return;
        }
...
size_t QuicSpdyClientStream::SendRequest(HttpHeaderBlock headers, absl::string_view body, bool fin)
{
...
bool QuicSpdyClientStream::ValidateReceivedHeaders(const QuicHeaderList& header_list)
{
    ...
    for (const std::pair<std::string, std::string>& pair : header_list) {
        if (pair.first == ":status") {
            ...
            set_invalid_request_details(absl::StrCat("Unexpected ':' in header ", pair.first, "."));
...
{
    time_to_response_complete_ = session()->GetClock()->ApproximateNow() - creation_time();
