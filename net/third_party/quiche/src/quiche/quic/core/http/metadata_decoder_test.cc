...
protected:
    std::string EncodeHeaders(quiche::HttpHeaderBlock& headers)
    {
        ...
        quic::QpackEncoder encoder(&delegate, quic::HuffmanEncoding::kDisabled, quic::CookieCrumbling::kDisabled);
...
TEST_F(MetadataDecoderTest, Initialize)
{
    ...
    MetadataDecoder decoder(id_, max_header_list_size, frame_header_len, payload_len);
...
TEST_F(MetadataDecoderTest, Decode)
{
    ...
    const size_t frame_header_len = 4;
    MetadataDecoder decoder(id_, max_header_list_size, frame_header_len, data.length());
...
TEST_F(MetadataDecoderTest, DecodeInvalidHeaders)
{
    ...
    EXPECT_EQ("Error decoding metadata: Error decoding Required Insert Count.", decoder.error_message());
}
