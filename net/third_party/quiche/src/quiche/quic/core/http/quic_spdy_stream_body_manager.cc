...
size_t QuicSpdyStreamBodyManager::ReadBody(const struct iovec* iov, size_t iov_len, size_t* total_bytes_read)
{
    *total_bytes_read = 0;
    QuicByteCount bytes_to_consume = 0;
    ...
    while (!fragments_.empty()) {
        ...
        const size_t bytes_to_copy = std::min<size_t>(body.length(), dest_remaining);
