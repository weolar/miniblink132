...
public:
    ...
    {
        ON_CALL(session_, WritevData(_, _, _, _, _, _)).WillByDefault(Invoke(&session_, &MockQuicSpdySession::ConsumeData));
    }
...
INSTANTIATE_TEST_SUITE_P(Tests, QuicSendControlStreamTest, ::testing::ValuesIn(GetTestParams()), ::testing::PrintToStringParamName());
...
TEST_P(QuicSendControlStreamTest, WriteSettings)
{
    ...
    ASSERT_TRUE(absl::HexStringToBytes("00" // stream type: control stream
                                       ...
                                       "61", //  payload "a"
        &expected_write_data));
    if (perspective() == Perspective::IS_CLIENT && QuicSpdySessionPeer::LocalHttpDatagramSupport(&session_) != HttpDatagramSupport::kNone) {
        ASSERT_TRUE(absl::HexStringToBytes("00" // stream type: control stream
                                           "04" // frame type: SETTINGS frame
                                           "0d" // frame length
                                           "01" // SETTINGS_QPACK_MAX_TABLE_CAPACITY
    ...
    if (perspective() == Perspective::IS_SERVER && QuicSpdySessionPeer::LocalHttpDatagramSupport(&session_) == HttpDatagramSupport::kNone) {
        ASSERT_TRUE(absl::HexStringToBytes("00" // stream type: control stream
                                           "04" // frame type: SETTINGS frame
                                           "0d" // frame length
                                           "01" // SETTINGS_QPACK_MAX_TABLE_CAPACITY
    ...
    // called.
    auto save_write_data = [&writer, this](QuicStreamId /*id*/, size_t write_length, QuicStreamOffset offset, StreamSendingState /*state*/,
    ...
    EXPECT_CALL(session_, WritevData(send_control_stream_->id(), _, _, _, _, _)).WillRepeatedly(Invoke(save_write_data));

    send_control_stream_->MaybeSendSettingsFrame();
    quiche::test::CompareCharArraysWithHexError("settings", writer.data(), writer.length(), expected_write_data.data(), expected_write_data.length());
}
...
TEST_P(QuicSendControlStreamTest, WriteSettingsOnlyOnce)
{
    ...
    EXPECT_CALL(session_, WritevData(send_control_stream_->id(), _, _, _, _, _)).Times(2);
    send_control_stream_->MaybeSendSettingsFrame();
...
TEST_P(QuicSendControlStreamTest, SendOriginFrameOnce)
{
    ...
    EXPECT_CALL(session_, WritevData(send_control_stream_->id(), _, _, _, _, _)).Times(1);
...
TEST_P(QuicSendControlStreamTest, WritePriorityBeforeSettings)
{
    ...
    EXPECT_CALL(session_, WritevData(send_control_stream_->id(), _, _, _, _, _)).Times(4);
    send_control_stream_->WritePriorityUpdate(
        /* stream_id = */ 0, HttpStreamPriority { /* urgency = */ 3, /* incremental = */ false });
...
TEST_P(QuicSendControlStreamTest, CloseControlStream)
{
    ...
    send_control_stream_->OnStopSending(QuicResetStreamError::FromInternal(QUIC_STREAM_CANCELLED));
}
...
TEST_P(QuicSendControlStreamTest, ReceiveDataOnSendControlStream)
{
    ...
    EXPECT_CALL(*connection_, CloseConnection(QUIC_DATA_RECEIVED_ON_WRITE_UNIDIRECTIONAL_STREAM, _, _));
...
TEST_P(QuicSendControlStreamTest, SendGoAway)
{
    ...
    EXPECT_CALL(session_, WritevData(send_control_stream_->id(), _, _, _, _, _)).Times(AnyNumber());
