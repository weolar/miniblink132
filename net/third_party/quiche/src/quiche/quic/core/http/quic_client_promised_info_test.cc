...
public:
    explicit MockQuicSpdyClientSession(
        const ParsedQuicVersionVector& supported_versions, QuicConnection* connection, QuicClientPushPromiseIndex* push_promise_index)
        : QuicSpdyClientSession(
            DefaultQuicConfig(), supported_versions, connection, QuicServerId("example.com", 443, false), &crypto_config_, push_promise_index)
...
public:
    ...
    QuicClientPromisedInfoTest()
        : connection_(new StrictMock<MockQuicConnection>(&helper_, &alarm_factory_, Perspective::IS_CLIENT))
        , session_(connection_->supported_versions(), connection_, &push_promise_index_)
        , body_("hello world")
        , promise_id_(QuicUtils::GetInvalidStreamId(connection_->transport_version()))
    {
        ...
        promise_url_ = SpdyServerPushUtils::GetPromisedUrlFromHeaders(push_promise_);

        client_request_ = push_promise_.Clone();
        promise_id_ = GetNthServerInitiatedUnidirectionalStreamId(connection_->transport_version(), 0);
    }
    ...
    void ReceivePromise(QuicStreamId id)
    {
        ...
        stream_->OnPromiseHeaderList(id, headers.uncompressed_header_bytes(), headers);
    }
...
TEST_F(QuicClientPromisedInfoTest, PushPromiseCleanupAlarm)
{
    ...
    EXPECT_CALL(*connection_, OnStreamReset(promise_id_, QUIC_PUSH_STREAM_TIMED_OUT));
...
TEST_F(QuicClientPromisedInfoTest, PushPromiseInvalidMethod)
{
    ...
    EXPECT_CALL(session_, WriteControlFrame(_, _));
    EXPECT_CALL(*connection_, OnStreamReset(promise_id_, QUIC_INVALID_PROMISE_METHOD));
    ReceivePromise(promise_id_);
...
TEST_F(QuicClientPromisedInfoTest, PushPromiseInvalidUrl)
{
    ...
    EXPECT_CALL(session_, WriteControlFrame(_, _));
    EXPECT_CALL(*connection_, OnStreamReset(promise_id_, QUIC_INVALID_PROMISE_URL));
    ReceivePromise(promise_id_);
...
TEST_F(QuicClientPromisedInfoTest, PushPromiseUnauthorizedUrl)
{
    ...
    EXPECT_CALL(session_, WriteControlFrame(_, _));
    EXPECT_CALL(*connection_, OnStreamReset(promise_id_, QUIC_UNAUTHORIZED_PROMISE_URL));
...
TEST_F(QuicClientPromisedInfoTest, PushPromiseMismatch)
{
    ...
    QuicSpdyClientStream* promise_stream = static_cast<QuicSpdyClientStream*>(session_.GetOrCreateStream(promise_id_));
    auto headers = AsHeaderList(headers_);
    promise_stream->OnStreamHeaderList(false, headers.uncompressed_header_bytes(), headers);

    TestPushPromiseDelegate delegate(/*match=*/false);
    EXPECT_CALL(session_, WriteControlFrame(_, _));
    EXPECT_CALL(*connection_, OnStreamReset(promise_id_, QUIC_PROMISE_VARY_MISMATCH));
...
TEST_F(QuicClientPromisedInfoTest, PushPromiseDataClosed)
{
    ...
    EXPECT_CALL(session_, WriteControlFrame(_, _));
    EXPECT_CALL(*connection_, OnStreamReset(promise_id_, QUIC_STREAM_PEER_GOING_AWAY));
    session_.ResetStream(promise_id_, QUIC_STREAM_PEER_GOING_AWAY);

    // Now initiate rendezvous.
    TestPushPromiseDelegate delegate(/*match=*/true);
    EXPECT_EQ(promised->HandleClientRequest(client_request_, &delegate), QUIC_FAILURE);
