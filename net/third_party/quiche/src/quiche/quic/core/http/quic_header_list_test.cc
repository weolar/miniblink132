...
TEST_F(QuicHeaderListTest, OnHeader)
{
    ...
    EXPECT_THAT(headers, ElementsAre(Pair("foo", "bar"), Pair("april", "fools"), Pair("beep", "")));
}
...
TEST_F(QuicHeaderListTest, IsCopyableAndAssignable)
{
    ...
    EXPECT_THAT(headers2, ElementsAre(Pair("foo", "bar"), Pair("april", "fools"), Pair("beep", "")));
    EXPECT_THAT(headers3, ElementsAre(Pair("foo", "bar"), Pair("april", "fools"), Pair("beep", "")));
}
