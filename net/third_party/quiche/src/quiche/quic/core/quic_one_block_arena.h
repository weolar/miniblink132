...
template <uint32_t ArenaSize> class QUICHE_EXPORT QuicOneBlockArena {
...
public:
    ...
    template <typename T, typename... Args> QuicArenaScopedPtr<T> New(Args&&... args)
    {
        QUICHE_DCHECK_LT(AlignedSize<T>(), ArenaSize) << "Object is too large for the arena.";
        static_assert(alignof(T) > 1, "Objects added to the arena must be at least 2B aligned.");
        if (ABSL_PREDICT_FALSE(offset_ > ArenaSize - AlignedSize<T>())) {
            QUIC_BUG(quic_bug_10593_1) << "Ran out of space in QuicOneBlockArena at " << this << ", max size was " << ArenaSize << ", failing request was "
        ...
        return QuicArenaScopedPtr<T>(buf, QuicArenaScopedPtr<T>::ConstructFrom::kArena);
    }

private:
    // Returns the size of |T| aligned up to |kMaxAlign|.
    template <typename T> static inline uint32_t AlignedSize()
    {
