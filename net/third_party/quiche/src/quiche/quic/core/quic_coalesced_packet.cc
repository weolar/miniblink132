...
bool QuicCoalescedPacket::MaybeCoalescePacket(const SerializedPacket& packet, const QuicSocketAddress& self_address, const QuicSocketAddress& peer_address,
    quiche::QuicheBufferAllocator* allocator, QuicPacketLength current_max_packet_length, QuicEcnCodepoint ecn_codepoint, uint32_t flow_label)
{
    ...
    if (length_ == 0) {
        ...
        if (self_address_ != self_address || peer_address_ != peer_address) {
            ...
            QUIC_DLOG(INFO) << "Cannot coalesce packet because self/peer address changed";
            return false;
        }
        if (max_packet_length_ != current_max_packet_length) {
            QUIC_BUG(quic_bug_10611_2) << "Max packet length changes in the middle of the write path";
            return false;
        }
    ...
    QUIC_DVLOG(1) << "Successfully coalesced packet: encryption_level: " << packet.encryption_level << ", encrypted_length: " << packet.encrypted_length
                  << ", current length: " << length_ << ", max_packet_length: " << max_packet_length_;
    if (length_ > 0) {
    ...
    if (packet.encryption_level == ENCRYPTION_INITIAL) {
        ...
        initial_packet_ = absl::WrapUnique<SerializedPacket>(CopySerializedPacket(packet, allocator, /*copy_buffer=*/false));
        return true;
    }
    // Copy encrypted buffer of packets with other encryption levels.
    encrypted_buffers_[packet.encryption_level] = std::string(packet.encrypted_buffer, packet.encrypted_length);
    return true;
}
...
void QuicCoalescedPacket::NeuterInitialPacket()
{
    ...
    if (length_ < initial_packet_->encrypted_length) {
        QUIC_BUG(quic_bug_10611_3) << "length_: " << length_ << ", is less than initial packet length: " << initial_packet_->encrypted_length;
        Clear();
        return;
    }
...
bool QuicCoalescedPacket::CopyEncryptedBuffers(char* buffer, size_t buffer_len, size_t* length_copied) const
{
    *length_copied = 0;
...
bool QuicCoalescedPacket::ContainsPacketOfEncryptionLevel(EncryptionLevel level) const
{
    return !encrypted_buffers_[level].empty() || (level == ENCRYPTION_INITIAL && initial_packet_ != nullptr);
}

TransmissionType QuicCoalescedPacket::TransmissionTypeOfPacket(EncryptionLevel level) const
{
    if (!ContainsPacketOfEncryptionLevel(level)) {
        QUIC_BUG(quic_bug_10611_4) << "Coalesced packet does not contain packet of encryption level: " << EncryptionLevelToString(level);
...
std::string QuicCoalescedPacket::ToString(size_t serialized_length) const
{
    ...
    std::string info = absl::StrCat("total_length: ", serialized_length, " padding_size: ", serialized_length - length_, " packets: {");
    // Packets' encryption levels.
    bool first_packet = true;
    for (int8_t i = ENCRYPTION_INITIAL; i < NUM_ENCRYPTION_LEVELS; ++i) {
        if (ContainsPacketOfEncryptionLevel(static_cast<EncryptionLevel>(i))) {
            absl::StrAppend(&info, first_packet ? "" : ", ", EncryptionLevelToString(static_cast<EncryptionLevel>(i)));
            first_packet = false;
        }
    }
    absl::StrAppend(&info, "}");
...
std::vector<size_t> QuicCoalescedPacket::packet_lengths() const
{
    ...
    for (const auto& packet : encrypted_buffers_) {
        if (lengths.empty()) {
            lengths.push_back(initial_packet_ == nullptr ? 0 : initial_packet_->encrypted_length);
        } else {
