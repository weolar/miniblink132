...
struct QUICHE_EXPORT HttpStreamPriority {
    ...
    {
        return std::tie(urgency, incremental) == std::tie(other.urgency, other.incremental);
    }
...
struct QUICHE_EXPORT WebTransportStreamPriority {
    ...
    {
        return session_id == other.session_id && send_group_number == other.send_group_number && send_order == other.send_order;
    }
    bool operator!=(const WebTransportStreamPriority& other) const
    {
        return !(*this == other);
...
public:
    QuicStreamPriority()
        : value_(HttpStreamPriority())
    {
    }
    explicit QuicStreamPriority(HttpStreamPriority priority)
        : value_(priority)
    {
    }
    ...
    QuicPriorityType type() const
    {
        return absl::visit(TypeExtractor(), value_);
    }
    ...
    HttpStreamPriority http() const
    {
        ...
        QUICHE_BUG(invalid_priority_type_http) << "Tried to access HTTP priority for a priority type" << type();
    ...
    WebTransportStreamPriority web_transport() const
    {
        ...
        QUICHE_BUG(invalid_priority_type_wt) << "Tried to access WebTransport priority for a priority type" << type();
...
QUICHE_EXPORT std::string SerializePriorityFieldValue(HttpStreamPriority priority);

// Parses the Priority Field Value field of a PRIORITY_UPDATE frame.
// Returns nullopt on failure.
QUICHE_EXPORT std::optional<HttpStreamPriority> ParsePriorityFieldValue(absl::string_view priority_field_value);
