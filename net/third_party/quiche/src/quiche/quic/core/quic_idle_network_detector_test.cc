...
public:
    QuicIdleNetworkDetectorTest()
        ...
        , detector_(&delegate_, clock_.Now() + QuicTimeDelta::FromSeconds(1), alarms_.idle_network_detector_alarm())
        , alarm_(alarms_.idle_network_detector_alarm())
    {
        clock_.AdvanceTime(QuicTime::Delta::FromSeconds(1));
        ON_CALL(connection_alarms_delegate_, OnIdleDetectorAlarm()).WillByDefault([&] { detector_.OnAlarm(); });
    }
...
TEST_F(QuicIdleNetworkDetectorTest, IdleNetworkDetectedBeforeHandshakeCompletes)
{
    ...
    EXPECT_EQ(clock_.Now() + QuicTime::Delta::FromSeconds(20), alarm_->deadline());
...
TEST_F(QuicIdleNetworkDetectorTest, HandshakeTimeout)
{
    ...
    EXPECT_EQ(clock_.Now() + QuicTime::Delta::FromSeconds(15), alarm_->deadline());
...
TEST_F(QuicIdleNetworkDetectorTest, IdleNetworkDetectedAfterHandshakeCompletes)
{
    EXPECT_FALSE(alarm_->IsSet());
    detector_.SetTimeouts(
        /*handshake_timeout=*/QuicTime::Delta::FromSeconds(30),
    ...
    EXPECT_EQ(clock_.Now() + QuicTime::Delta::FromSeconds(600), alarm_->deadline());
...
TEST_F(QuicIdleNetworkDetectorTest, DoNotExtendIdleDeadlineOnConsecutiveSentPackets)
{
    EXPECT_FALSE(alarm_->IsSet());
    detector_.SetTimeouts(
        /*handshake_timeout=*/QuicTime::Delta::FromSeconds(30),
    ...
    detector_.OnPacketReceived(clock_.Now());
    detector_.SetTimeouts(
        /*handshake_timeout=*/QuicTime::Delta::Infinite(), QuicTime::Delta::FromSeconds(600));
    EXPECT_EQ(clock_.Now() + QuicTime::Delta::FromSeconds(600), alarm_->deadline());
    ...
    EXPECT_EQ(packet_sent_time + QuicTime::Delta::FromSeconds(600), alarm_->deadline());
    ...
    // No network activity for 600s.
    clock_.AdvanceTime(QuicTime::Delta::FromSeconds(600) - QuicTime::Delta::FromMilliseconds(200));
    EXPECT_CALL(delegate_, OnIdleNetworkDetected());
    alarm_->Fire();
}
...
TEST_F(QuicIdleNetworkDetectorTest, ShorterIdleTimeoutOnSentPacket)
{
    ...
    EXPECT_EQ(clock_.Now() + QuicTime::Delta::FromSeconds(30), alarm_->deadline());
...
TEST_F(QuicIdleNetworkDetectorTest, NoAlarmAfterStopped)
{
    ...
    EXPECT_QUIC_BUG(detector_.SetTimeouts(
                        /*handshake_timeout=*/QuicTime::Delta::FromSeconds(30),
                        /*idle_network_timeout=*/QuicTime::Delta::FromSeconds(20)),
