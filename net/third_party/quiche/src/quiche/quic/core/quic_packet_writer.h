...
public:
    ...
    virtual WriteResult WritePacket(const char* buffer, size_t buf_len, const QuicIpAddress& self_address, const QuicSocketAddress& peer_address,
    ...
    virtual QuicByteCount GetMaxPacketSize(const QuicSocketAddress& peer_address) const = 0;
    ...
    virtual QuicPacketBuffer GetNextWriteLocation(const QuicIpAddress& self_address, const QuicSocketAddress& peer_address) = 0;
