...
public:
    ...
    explicit QuicChaosProtector(const QuicCryptoFrame& crypto_frame, int num_padding_bytes, size_t packet_size, QuicFramer* framer, QuicRandom* random);
    ...
    std::optional<size_t> BuildDataPacket(const QuicPacketHeader& header, char* buffer);
...
private:
    ...
    std::optional<size_t> BuildPacket(const QuicPacketHeader& header, char* buffer);
