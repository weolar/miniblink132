...
struct TestIntervalItem {
    ...
    TestIntervalItem(int32_t val, std::size_t interval_start, std::size_t interval_end)
        : val(val)
...
TEST_F(QuicIntervalDequeTest, InsertDeleteIterate)
{
    ...
    for (int32_t i = 0; limit < qid_.Size(); ++i, ++limit) {
        ...
        const int32_t index_after_pop = QuicIntervalDequePeer::GetCachedIndex(&qid_);
...
TEST_F(QuicIntervalDequeTest, InsertIterateInsert)
{
    ...
    for (int32_t i = 0; i < kSize; ++i, ++iterated_elements) {
        ...
        lookup++;
        const int32_t after_inc = (iterated_elements + 1) == (kSize * 2) ? -1 : (iterated_elements + 1);
    ...
    const int32_t invalid_index_again = QuicIntervalDequePeer::GetCachedIndex(&qid_);
...
TEST_F(QuicIntervalDequeTest, RescanData)
{
    ...
    for (int32_t i = 0; i < kSize - 1; ++i, ++it) {
        ...
        EXPECT_EQ(i, lookup->val);
        // Make sure the index has changed just from using |DataAt|
        const int32_t cached_index_before = QuicIntervalDequePeer::GetCachedIndex(&qid_);
        ...
        const int32_t cached_index_after = QuicIntervalDequePeer::GetCachedIndex(&qid_);
...
{
    QID qid;
    EXPECT_QUIC_BUG(qid.PushBack(TestIntervalItem(0, 0, 0)), "Trying to save empty interval to .");
}
