...
QuicDataReader::QuicDataReader(const char* data, const size_t len, quiche::Endianness endianness)
...
bool QuicDataReader::ReadUFloat16(uint64_t* result)
{
    ...
    *result <<= exponent;
    QUICHE_DCHECK_GE(*result, static_cast<uint64_t>(1 << kUFloat16MantissaEffectiveBits));
    QUICHE_DCHECK_LE(*result, kUFloat16MaxValue);
    return true;
}

bool QuicDataReader::ReadConnectionId(QuicConnectionId* connection_id, uint8_t length)
{
    if (length == 0) {
    ...
    const bool ok = ReadBytes(connection_id->mutable_data(), connection_id->length());
    QUICHE_DCHECK(ok);
    return ok;
}

bool QuicDataReader::ReadLengthPrefixedConnectionId(QuicConnectionId* connection_id)
{
