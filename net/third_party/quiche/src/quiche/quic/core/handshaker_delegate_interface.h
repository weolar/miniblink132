...
public:
    ...
    virtual void OnNewEncryptionKeyAvailable(EncryptionLevel level, std::unique_ptr<QuicEncrypter> encrypter) = 0;
    ...
    // |error_details|.
    virtual QuicErrorCode ProcessTransportParameters(const TransportParameters& params, bool is_resumption, std::string* error_details) = 0;
    ...
    virtual void OnEncryptedClientHelloSent(absl::string_view client_hello) const = 0;

    // Called after an ClientHelloInner is received and decrypted as a server.
    virtual void OnEncryptedClientHelloReceived(absl::string_view client_hello) const = 0;
};
