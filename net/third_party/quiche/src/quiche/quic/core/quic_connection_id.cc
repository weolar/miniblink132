...
public:
    ...
    {
        return static_cast<size_t>(SIPHASH_24(sip_hash_key_, reinterpret_cast<const uint8_t*>(input), input_len));
    }
...
QuicConnectionId::QuicConnectionId()
    : QuicConnectionId(nullptr, 0)
{
    static_assert(offsetof(QuicConnectionId, padding_) == offsetof(QuicConnectionId, length_), "bad offset");
...
QuicConnectionId::QuicConnectionId(const absl::Span<const uint8_t> data)
    : QuicConnectionId(reinterpret_cast<const char*>(data.data()), data.length())
{
}
...
void QuicConnectionId::set_length(uint8_t length)
{
    ...
    if (length > sizeof(data_short_)) {
        if (length_ <= sizeof(data_short_)) {
            ...
            // Resize data_long_.
            char* realloc_result = reinterpret_cast<char*>(realloc(data_long_, length));
...
static_assert(kQuicDefaultConnectionIdLength == sizeof(uint64_t), "kQuicDefaultConnectionIdLength changed");
static_assert(kQuicDefaultConnectionIdLength == 8, "kQuicDefaultConnectionIdLength changed");
