...
public:
    ...
    void SetReadError(const std::string& error)
    {
        read_error_ = error;
    }
...
private:
    ...
    std::unique_ptr<QuicData> ReadPacket(bool* blocked, std::string* error) override
    {
        *blocked = false;
    ...
    bool WritePacket(const char* packet, size_t size, bool* blocked, std::string* error) override
    {
        *blocked = false;
...
TEST(QbonePacketExchangerTest, ReadAndDeliverPacketDeliversPacketToQboneClient)
{
    ...
    exchanger.AddPacketToBeRead(std::make_unique<QuicData>(packet.data(), packet.length()));
    EXPECT_CALL(client, ProcessPacketFromNetwork(StrEq("data")));

    EXPECT_TRUE(exchanger.ReadAndDeliverPacket(&client));
}

TEST(QbonePacketExchangerTest, ReadAndDeliverPacketNotifiesVisitorOnReadFailure)
{
    MockVisitor visitor;
    FakeQbonePacketExchanger exchanger(&visitor, kMaxPendingPackets);
...
TEST(QbonePacketExchangerTest, ReadAndDeliverPacketDoesNotNotifyVisitorOnBlockedIO)
{
    MockVisitor visitor;
    FakeQbonePacketExchanger exchanger(&visitor, kMaxPendingPackets);
    MockQboneClient client;
...
TEST(QbonePacketExchangerTest, WritePacketToNetworkWritesDirectlyToNetworkWhenNotBlocked)
{
    MockVisitor visitor;
    FakeQbonePacketExchanger exchanger(&visitor, kMaxPendingPackets);
    MockQboneClient client;
...
TEST(QbonePacketExchangerTest, WritePacketToNetworkQueuesPacketsAndProcessThemLater)
{
    MockVisitor visitor;
    FakeQbonePacketExchanger exchanger(&visitor, kMaxPendingPackets);
    MockQboneClient client;
...
TEST(QbonePacketExchangerTest, SetWritableContinuesProcessingPacketIfPreviousCallBlocked)
{
    MockVisitor visitor;
    FakeQbonePacketExchanger exchanger(&visitor, kMaxPendingPackets);
    MockQboneClient client;
