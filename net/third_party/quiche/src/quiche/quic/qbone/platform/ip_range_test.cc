...
TEST(IpRangeTest, TruncateWorksIPv6)
{
    ...
    EXPECT_EQ("ffff:ffff:ffff:ffff::/64", IpRange(before_truncate, 64).ToString());
    EXPECT_EQ("ffff:ffff:ffff:ffff:8000::/65", IpRange(before_truncate, 65).ToString());
    EXPECT_EQ("ffff:ffff:ffff:ffff:f903::4/127", IpRange(before_truncate, 127).ToString());
}
