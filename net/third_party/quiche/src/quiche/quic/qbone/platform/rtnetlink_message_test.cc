...
TEST(RtnetlinkMessageTest, LinkMessageCanBeCreatedForGetOperation)
{
    ...
    uint32_t seq = 42;
    uint32_t pid = 7;
    auto message = LinkMessage::New(RtnetlinkMessage::Operation::GET, flags, seq, pid, nullptr);
...
{
    struct ifinfomsg interface_info_header = { AF_INET, /* pad */ 0, ARPHRD_TUNNEL, 3, 0, 0xffffffff };
    uint16_t flags = NLM_F_REQUEST | NLM_F_ROOT | NLM_F_MATCH;
    uint32_t seq = 42;
    uint32_t pid = 7;
    auto message = LinkMessage::New(RtnetlinkMessage::Operation::NEW, flags, seq, pid, &interface_info_header);
    ...
    // nlmsghdr is built properly.
    auto iov = message.BuildIoVec();
    EXPECT_EQ(NLMSG_ALIGN(NLMSG_LENGTH(sizeof(struct ifinfomsg))), iov[0].iov_len);
    auto* netlink_message = reinterpret_cast<struct nlmsghdr*>(iov[0].iov_base);
    EXPECT_EQ(NLMSG_ALIGN(NLMSG_LENGTH(sizeof(struct ifinfomsg))) + RTA_LENGTH(device_name.size()), netlink_message->nlmsg_len);
    ...
    auto* parsed_header = reinterpret_cast<struct ifinfomsg*>(NLMSG_DATA(netlink_message));
    ...
    EXPECT_THAT(device_name, StrEq(std::string(reinterpret_cast<char*>(RTA_DATA(rta)), RTA_PAYLOAD(rta))));
}
...
TEST(RtnetlinkMessageTest, AddressMessageCanBeCreatedForGetOperation)
{
    ...
    uint32_t seq = 42;
    uint32_t pid = 7;
    auto message = AddressMessage::New(RtnetlinkMessage::Operation::GET, flags, seq, pid, nullptr);
...
TEST(RtnetlinkMessageTest, AddressMessageCanBeCreatedForNewOperation)
{
    ...
    uint32_t seq = 42;
    uint32_t pid = 7;
    auto message = AddressMessage::New(RtnetlinkMessage::Operation::NEW, flags, seq, pid, &interface_address_header);

    QuicIpAddress ip;
    QUICHE_CHECK(ip.FromString("10.0.100.3"));
    message.AppendAttribute(IFA_ADDRESS, ip.ToPackedString().c_str(), ip.ToPackedString().size());
    ...
    // nlmsghdr is built properly.
    auto iov = message.BuildIoVec();
    EXPECT_EQ(NLMSG_ALIGN(NLMSG_LENGTH(sizeof(struct ifaddrmsg))), iov[0].iov_len);
    auto* netlink_message = reinterpret_cast<struct nlmsghdr*>(iov[0].iov_base);
    EXPECT_EQ(NLMSG_ALIGN(NLMSG_LENGTH(sizeof(struct ifaddrmsg))) + RTA_LENGTH(ip.ToPackedString().size()), netlink_message->nlmsg_len);
    ...
    auto* parsed_header = reinterpret_cast<struct ifaddrmsg*>(NLMSG_DATA(netlink_message));
    EXPECT_EQ(interface_address_header.ifa_family, parsed_header->ifa_family);
    EXPECT_EQ(interface_address_header.ifa_prefixlen, parsed_header->ifa_prefixlen);
    ...
    EXPECT_THAT(ip.ToPackedString(), StrEq(std::string(reinterpret_cast<char*>(RTA_DATA(rta)), RTA_PAYLOAD(rta))));
}
...
TEST(RtnetlinkMessageTest, RouteMessageCanBeCreatedFromNewOperation)
{
    ...
    uint32_t seq = 42;
    uint32_t pid = 7;
    auto message = RouteMessage::New(RtnetlinkMessage::Operation::NEW, flags, seq, pid, &route_message_header);

    QuicIpAddress preferred_source;
    QUICHE_CHECK(preferred_source.FromString("ff80::1"));
    message.AppendAttribute(RTA_PREFSRC, preferred_source.ToPackedString().c_str(), preferred_source.ToPackedString().size());
    ...
    auto* netlink_message = reinterpret_cast<struct nlmsghdr*>(iov[0].iov_base);
    EXPECT_EQ(NLMSG_ALIGN(NLMSG_LENGTH(sizeof(struct rtmsg))) + RTA_LENGTH(preferred_source.ToPackedString().size()), netlink_message->nlmsg_len);
    ...
    auto* parsed_header = reinterpret_cast<struct rtmsg*>(NLMSG_DATA(netlink_message));
    ...
    // rtattr is handled properly.
    EXPECT_EQ(RTA_SPACE(preferred_source.ToPackedString().size()), iov[1].iov_len);
    auto* rta = reinterpret_cast<struct rtattr*>(iov[1].iov_base);
    EXPECT_EQ(RTA_PREFSRC, rta->rta_type);
    EXPECT_EQ(RTA_LENGTH(preferred_source.ToPackedString().size()), rta->rta_len);
    EXPECT_THAT(preferred_source.ToPackedString(), StrEq(std::string(reinterpret_cast<char*>(RTA_DATA(rta)), RTA_PAYLOAD(rta))));
}
