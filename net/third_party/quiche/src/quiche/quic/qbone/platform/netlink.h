...
public:
    ...
    bool GetLinkInfo(const std::string& interface_name, LinkInfo* link_info) override;
    ...
    // kernel should answer immediately.
    bool GetAddresses(int interface_index, uint8_t unwanted_flags, std::vector<AddressInfo>* addresses, int* num_ipv6_nodad_dadfailed_addresses) override;
    ...
    bool ChangeLocalAddress(uint32_t interface_index, Verb verb, const QuicIpAddress& address, uint8_t prefix_length, uint8_t ifa_flags, uint8_t ifa_scope,
    ...
    bool ChangeRoute(
        Netlink::Verb verb, uint32_t table, const IpRange& destination_subnet, uint8_t scope, QuicIpAddress preferred_source, int32_t interface_index) override;
