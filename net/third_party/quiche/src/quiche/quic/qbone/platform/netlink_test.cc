...
protected:
    NetlinkTest()
    {
        ON_CALL(mock_kernel_, socket(AF_NETLINK, SOCK_RAW, NETLINK_ROUTE)).WillByDefault(Invoke([this](Unused, Unused, Unused) {
            EXPECT_CALL(mock_kernel_, close(kSocketFd)).WillOnce(Return(0));
            return kSocketFd;
        }));
    }

    void ExpectNetlinkPacket(uint16_t type, uint16_t flags, const std::function<ssize_t(void* buf, size_t len, int seq)>& recv_callback,
        const std::function<void(const void* buf, size_t len)>& send_callback = nullptr)
    {
        static int seq = -1;
        InSequence s;

        EXPECT_CALL(mock_kernel_, sendmsg(kSocketFd, _, _)).WillOnce(Invoke([type, flags, send_callback](Unused, const struct msghdr* msg, int) {
            EXPECT_EQ(sizeof(struct sockaddr_nl), msg->msg_namelen);
            auto* nl_addr = reinterpret_cast<const struct sockaddr_nl*>(msg->msg_name);
            ...
            for (int i = 0; i < msg->msg_iovlen; i++) {
                buf.append(std::string(reinterpret_cast<char*>(msg->msg_iov[i].iov_base), msg->msg_iov[i].iov_len));
            }

            auto* netlink_message = reinterpret_cast<const struct nlmsghdr*>(buf.c_str());
        ...
        EXPECT_CALL(mock_kernel_, recvfrom(kSocketFd, _, 0, MSG_PEEK | MSG_TRUNC, _, _))
            .WillOnce(Invoke([this, recv_callback](Unused, Unused, Unused, Unused, struct sockaddr* src_addr, socklen_t* addrlen) {
        ...
        EXPECT_CALL(mock_kernel_, recvfrom(kSocketFd, _, _, _, _, _))
            .WillOnce(Invoke([recv_callback](Unused, void* buf, size_t len, Unused, struct sockaddr* src_addr, socklen_t* addrlen) {
                auto* nl_addr = reinterpret_cast<struct sockaddr_nl*>(src_addr);
                nl_addr->nl_family = AF_NETLINK;
                nl_addr->nl_pid = 0; // from kernel
                nl_addr->nl_groups = 0; // no multicast
...
void AddRTA(struct nlmsghdr* netlink_message, uint16_t type, const void* data, size_t len)
{
    auto* next_header_ptr = reinterpret_cast<char*>(netlink_message) + NLMSG_ALIGN(netlink_message->nlmsg_len);
    ...
    netlink_message->nlmsg_len = NLMSG_ALIGN(netlink_message->nlmsg_len) + RTA_LENGTH(len);
}

void CreateIfinfomsg(struct nlmsghdr* netlink_message, const std::string& interface_name, uint16_t type, int index, unsigned int flags, unsigned int change,
    uint8_t address[], int address_len, uint8_t broadcast[], int broadcast_len)
{
    auto* interface_info = reinterpret_cast<struct ifinfomsg*>(NLMSG_DATA(netlink_message));
    ...
    // Add name
    AddRTA(netlink_message, IFLA_IFNAME, interface_name.c_str(), interface_name.size());
}

struct nlmsghdr* CreateNetlinkMessage(void* buf, // NOLINT
    struct nlmsghdr* previous_netlink_message, uint16_t type, int seq)
{
    auto* next_header_ptr = reinterpret_cast<char*>(buf);
    if (previous_netlink_message != nullptr) {
        next_header_ptr = reinterpret_cast<char*>(previous_netlink_message) + NLMSG_ALIGN(previous_netlink_message->nlmsg_len);
    }
...
void CreateIfaddrmsg(struct nlmsghdr* nlm, int interface_index, unsigned char prefixlen, unsigned char flags, unsigned char scope, QuicIpAddress ip)
{
    ...
    case IpAddressFamily::IP_V6:
        ...
        break;
    default:
        QUIC_BUG(quic_bug_11034_1) << absl::StrCat("unexpected address family: ", ip.address_family());
        family = AF_UNSPEC;
    }
    ...
    // Add local address
    AddRTA(nlm, IFA_LOCAL, ip.ToPackedString().c_str(), ip.ToPackedString().size());
}

void CreateRtmsg(struct nlmsghdr* nlm, unsigned char family, unsigned char destination_length, unsigned char source_length, unsigned char tos,
    unsigned char table, unsigned char protocol, unsigned char scope, unsigned char type, unsigned int flags, QuicIpAddress destination, int interface_index)
{
    ...
    // Add destination
    AddRTA(nlm, RTA_DST, destination.ToPackedString().c_str(), destination.ToPackedString().size());
...
TEST_F(NetlinkTest, GetLinkInfoWorks)
{
    ...
    ExpectNetlinkPacket(RTM_GETLINK, NLM_F_ROOT | NLM_F_MATCH | NLM_F_REQUEST, [&hwaddr, &bcaddr](void* buf, size_t len, int seq) {
        int ret = 0;

        struct nlmsghdr* netlink_message = CreateNetlinkMessage(buf, nullptr, RTM_NEWLINK, seq);
        ...
        netlink_message = CreateNetlinkMessage(buf, netlink_message, NLMSG_DONE, seq);
        ret += NLMSG_ALIGN(netlink_message->nlmsg_len);
...
TEST_F(NetlinkTest, GetAddressesWorks)
{
    ...
    absl::node_hash_set<std::string> addresses = { QuicIpAddress::Any4().ToString(), QuicIpAddress::Any6().ToString() };

    ExpectNetlinkPacket(RTM_GETADDR, NLM_F_ROOT | NLM_F_MATCH | NLM_F_REQUEST, [&addresses](void* buf, size_t len, int seq) {
        int ret = 0;
    ...
    EXPECT_TRUE(netlink->GetAddresses(7, IFA_F_TENTATIVE | IFA_F_OPTIMISTIC, &reported_addresses, &num_ipv6_nodad_dadfailed_addresses));
...
TEST_F(NetlinkTest, ChangeLocalAddressAdd)
{
    ...
    ExpectNetlinkPacket(
        ...
        [](void* buf, size_t len, int seq) {
            struct nlmsghdr* netlink_message = CreateNetlinkMessage(buf, nullptr, NLMSG_ERROR, seq);
            auto* err = reinterpret_cast<struct nlmsgerr*>(NLMSG_DATA(netlink_message));
            // Ack the request
            err->error = 0;
        ...
        [ip](const void* buf, size_t len) {
            ...
            auto* ifa = reinterpret_cast<const struct ifaddrmsg*>(NLMSG_DATA(netlink_message));
            ...
            int num_rta = 0;
            for (rta = IFA_RTA(ifa); RTA_OK(rta, payload_length); rta = RTA_NEXT(rta, payload_length)) {
                switch (rta->rta_type) {
                case IFA_LOCAL: {
                    EXPECT_EQ(ip.ToPackedString().size(), RTA_PAYLOAD(rta));
                    const auto* raw_address = reinterpret_cast<const char*>(RTA_DATA(rta));
                ...
                case IFA_CACHEINFO: {
                    ...
                    const auto* cache_info = reinterpret_cast<const struct ifa_cacheinfo*>(RTA_DATA(rta));
    ...
    EXPECT_TRUE(netlink->ChangeLocalAddress(7, Netlink::Verb::kAdd, ip, 19, IFA_F_PERMANENT, RT_SCOPE_UNIVERSE, { &additional_rta.rta }));
}
...
TEST_F(NetlinkTest, ChangeLocalAddressRemove)
{
    ...
    EXPECT_TRUE(netlink->ChangeLocalAddress(7, Netlink::Verb::kRemove, ip, 32, 0, RT_SCOPE_UNIVERSE, {}));
}
...
TEST_F(NetlinkTest, GetRouteInfoWorks)
{
    ...
    ExpectNetlinkPacket(RTM_GETROUTE, NLM_F_ROOT | NLM_F_MATCH | NLM_F_REQUEST, [destination](void* buf, size_t len, int seq) {
        int ret = 0;
        struct nlmsghdr* netlink_message = CreateNetlinkMessage(buf, nullptr, RTM_NEWROUTE, seq);
        CreateRtmsg(netlink_message, AF_INET6, 48, 0, 0, RT_TABLE_MAIN, RTPROT_STATIC, RT_SCOPE_LINK, RTN_UNICAST, 0, destination, 7);
        ret += NLMSG_ALIGN(netlink_message->nlmsg_len);
    ...
    EXPECT_EQ(IpRange(destination, 48).ToString(), routing_rules[0].destination_subnet.ToString());
...
TEST_F(NetlinkTest, ChangeRouteAdd)
{
    ...
    ExpectNetlinkPacket(
        ...
        [preferred_ip, subnet, egress_interface_index](const void* buf, size_t len) {
            auto* netlink_message = reinterpret_cast<const struct nlmsghdr*>(buf);
            auto* rtm = reinterpret_cast<const struct rtmsg*>(NLMSG_DATA(netlink_message));
            ...
            int num_rta = 0;
            for (rta = RTM_RTA(rtm); RTA_OK(rta, payload_length); rta = RTA_NEXT(rta, payload_length)) {
                switch (rta->rta_type) {
                case RTA_PREFSRC: {
                    const auto* raw_address = reinterpret_cast<const char*>(RTA_DATA(rta));
                ...
                case RTA_OIF: {
                    ...
                    const auto* interface_index = reinterpret_cast<const int*>(RTA_DATA(rta));
                ...
                case RTA_DST: {
                    ...
                    QuicIpAddress address;
                    address.FromPackedString(raw_address, RTA_PAYLOAD(rta));
                    EXPECT_EQ(subnet.ToString(), IpRange(address, rtm->rtm_dst_len).ToString());
                    break;
                }
                case RTA_TABLE: {
                    ASSERT_EQ(*reinterpret_cast<uint32_t*>(RTA_DATA(rta)), QboneConstants::kQboneRouteTableId);
                    break;
                }
                default:
    ...
    EXPECT_TRUE(netlink->ChangeRoute(Netlink::Verb::kAdd, QboneConstants::kQboneRouteTableId, subnet, RT_SCOPE_LINK, preferred_ip, egress_interface_index));
}
...
TEST_F(NetlinkTest, ChangeRouteRemove)
{
    ...
    EXPECT_TRUE(netlink->ChangeRoute(Netlink::Verb::kRemove, QboneConstants::kQboneRouteTableId, subnet, RT_SCOPE_LINK, preferred_ip, egress_interface_index));
}
...
TEST_F(NetlinkTest, ChangeRouteReplace)
{
    ...
    EXPECT_TRUE(netlink->ChangeRoute(Netlink::Verb::kReplace, QboneConstants::kQboneRouteTableId, subnet, RT_SCOPE_LINK, preferred_ip, egress_interface_index));
}
