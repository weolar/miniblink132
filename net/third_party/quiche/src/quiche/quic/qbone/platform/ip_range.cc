...
QuicIpAddress TruncateToLength(const QuicIpAddress& input, size_t* prefix_length)
{
    ...
    if (input.IsIPv4()) {
        ...
        uint32_t raw_address = *reinterpret_cast<const uint32_t*>(input.ToPackedString().data());
        ...
        output.FromPackedString(reinterpret_cast<const char*>(&raw_address), sizeof(raw_address));
        return output;
    }
    if (input.IsIPv6()) {
