...
public:
    ...
    // interface_name.
    virtual bool GetLinkInfo(const std::string& interface_name, LinkInfo* link_info) = 0;
    ...
    // interface_index.
    virtual bool GetAddresses(int interface_index, uint8_t unwanted_flags, std::vector<AddressInfo>* addresses, int* num_ipv6_nodad_dadfailed_addresses) = 0;
    ...
    virtual bool ChangeLocalAddress(uint32_t interface_index, Verb verb, const QuicIpAddress& address, uint8_t prefix_length, uint8_t ifa_flags,
        uint8_t ifa_scope, const std::vector<struct rtattr*>& additional_attributes)
        = 0;
    ...
    virtual bool ChangeRoute(
        Verb verb, uint32_t table, const IpRange& destination_subnet, uint8_t scope, QuicIpAddress preferred_source, int32_t interface_index)
        = 0;
