...
void CreateTcpResetPacket(absl::string_view original_packet, quiche::UnretainedCallback<void(absl::string_view)> cb)
{
    ...
    if (ABSL_PREDICT_FALSE(quiche::QuicheEndian::NetToHost16(ip6_header->ip6_plen) < sizeof(tcphdr))) {
        return;
    }
    ...
    tcp_packet.ip_header.ip6_plen = quiche::QuicheEndian::HostToNet16(payload_size);
    ...
    if (tcp_header->ack) {
        ...
        tcp_packet.tcp_header.ack_seq = quiche::QuicheEndian::HostToNet32(quiche::QuicheEndian::NetToHost32(tcp_header->seq) + 1);
    }
    ...
    checksum.Update(reinterpret_cast<char*>(&pseudo_header), sizeof(pseudo_header));
    // TCP header.
    checksum.Update(reinterpret_cast<const char*>(&tcp_packet.tcp_header), sizeof(tcp_packet.tcp_header));
    // There is no body.
