...
constexpr size_t kICMPv6BodyMaxSize = kIPv6MinPacketSize - kIPv6HeaderSize - kICMPv6HeaderSize;
...
void CreateIcmpPacket(in6_addr src, in6_addr dst, const icmp6_hdr& icmp_header, absl::string_view body, quiche::UnretainedCallback<void(absl::string_view)> cb)
{
    ...
    icmp_packet.ip_header.ip6_plen = quiche::QuicheEndian::HostToNet16(payload_size);
    ...
    // ICMP header.
    checksum.Update(reinterpret_cast<const char*>(&icmp_packet.icmp_header), sizeof(icmp_packet.icmp_header));
    // Body.
