...
DEFINE_QUICHE_COMMAND_LINE_FLAG(bool, qbone_close_ephemeral_frames, true, "If true, we'll call CloseStream even when we receive ephemeral frames.");
...
QboneSessionBase::QboneSessionBase(
    QuicConnection* connection, Visitor* owner, const QuicConfig& config, const ParsedQuicVersionVector& supported_versions, QbonePacketWriter* writer)
    : QuicSession(connection, owner, config, supported_versions,
        /*num_expected_unidirectional_static_streams = */ 0)
{
    set_writer(writer);
    const uint32_t max_streams = (std::numeric_limits<uint32_t>::max() / kMaxAvailableStreamsMultiplier) - 1;
...
{
    return ActivateDataStream(CreateDataStream(GetNextOutgoingUnidirectionalStreamId()));
}
...
void QboneSessionBase::OnStreamFrame(const QuicStreamFrame& frame)
{
    if (frame.offset == 0 && frame.fin && frame.data_length > 0) {
        ...
        ProcessPacketFromPeer(absl::string_view(frame.data_buffer, frame.data_length));
...
std::unique_ptr<QuicStream> QboneSessionBase::CreateDataStream(QuicStreamId id)
{
...
QuicStream* QboneSessionBase::ActivateDataStream(std::unique_ptr<QuicStream> stream)
{
...
void QboneSessionBase::SendPacketToPeer(absl::string_view packet)
{
    if (crypto_stream_ == nullptr) {
        QUIC_BUG(quic_bug_10987_1) << "Attempting to send packet before encryption established";
        return;
    }

    if (send_packets_as_messages_) {
        quiche::QuicheMemSlice slice(quiche::QuicheBuffer::Copy(connection()->helper()->GetStreamSendBufferAllocator(), packet));
        ...
        case MESSAGE_STATUS_TOO_LARGE: {
            if (packet.size() < sizeof(ip6_hdr)) {
                QUIC_BUG(quic_bug_10987_2) << "Dropped malformed packet: IPv6 header too short";
                break;
            }
            auto* header = reinterpret_cast<const ip6_hdr*>(packet.begin());
            icmp6_hdr icmp_header {};
            icmp_header.icmp6_type = ICMP6_PACKET_TOO_BIG;
            icmp_header.icmp6_mtu = connection()->GetGuaranteedLargestMessagePayload();

            CreateIcmpPacket(header->ip6_dst, header->ip6_src, icmp_header, packet,
                [this](absl::string_view icmp_packet) { writer_->WritePacketToNetwork(icmp_packet.data(), icmp_packet.size()); });
            break;
        }
        case MESSAGE_STATUS_ENCRYPTION_NOT_ESTABLISHED:
            QUIC_BUG(quic_bug_10987_3) << "MESSAGE_STATUS_ENCRYPTION_NOT_ESTABLISHED";
            break;
    ...
    QboneWriteOnlyStream* qbone_stream = static_cast<QboneWriteOnlyStream*>(stream);
