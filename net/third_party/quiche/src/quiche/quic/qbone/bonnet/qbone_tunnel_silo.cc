...
void QboneTunnelSilo::Run()
{
    ...
    QUIC_LOG(INFO) << "Tunnel has disconnected in state: " << tunnel_->StateToString(tunnel_->Disconnect());
}
...
{
    bool post_init_shutdown_ready = only_setup_tun_ && tunnel_->state() == quic::QboneTunnelInterface::STARTED;
