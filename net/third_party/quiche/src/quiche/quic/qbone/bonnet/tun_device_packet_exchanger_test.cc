...
protected:
    TunDevicePacketExchangerTest()
        : exchanger_(kMtu, &mock_kernel_, nullptr, &mock_visitor_, kMaxPendingPackets, false, &mock_stats_, absl::string_view())
    {
...
TEST_F(TunDevicePacketExchangerTest, WritePacketReturnsFalseOnError)
{
    ...
    EXPECT_CALL(mock_kernel_, write(kFd, _, packet.size())).WillOnce(Invoke([](int fd, const void* buf, size_t count) {
        errno = ECOMM;
        return -1;
    }));
...
TEST_F(TunDevicePacketExchangerTest, WritePacketReturnFalseAndBlockedOnBlockedTunnel)
{
    std::string packet = "fake packet";
    EXPECT_CALL(mock_kernel_, write(kFd, _, packet.size())).WillOnce(Invoke([](int fd, const void* buf, size_t count) {
        errno = EAGAIN;
...
{
    std::string packet = "fake packet";
    EXPECT_CALL(mock_kernel_, write(kFd, _, packet.size())).WillOnce(Invoke([packet](int fd, const void* buf, size_t count) {
...
{
    EXPECT_CALL(mock_kernel_, read(kFd, _, kMtu)).WillOnce(Invoke([](int fd, void* buf, size_t count) {
        errno = ECOMM;
        return -1;
    }));
...
TEST_F(TunDevicePacketExchangerTest, ReadPacketReturnsThePacketOnSuccessfulRead)
{
    std::string packet = "fake_packet";
    EXPECT_CALL(mock_kernel_, read(kFd, _, kMtu)).WillOnce(Invoke([packet](int fd, void* buf, size_t count) {
