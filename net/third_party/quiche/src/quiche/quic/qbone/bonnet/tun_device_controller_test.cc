...
MATCHER_P(IpRangeEq, range, absl::StrCat("expected IpRange to equal ", range.ToString()))
{
    return arg == range;
}
...
public:
    ...
    {
        controller_.RegisterAddressUpdateCallback([this](QuicIpAddress address) { notified_address_ = address; });
    }

protected:
    void ExpectLinkInfo(const std::string& interface_name, int ifindex)
    {
        EXPECT_CALL(netlink_, GetLinkInfo(interface_name, _)).WillOnce(Invoke([ifindex](absl::string_view ifname, NetlinkInterface::LinkInfo* link_info) {
...
TEST_F(TunDeviceControllerTest, AddressAppliedWhenNoneExisted)
{
    ...
    EXPECT_CALL(netlink_,
        ChangeLocalAddress(
            kIfindex, NetlinkInterface::Verb::kAdd, kIpRange.FirstAddressInRange(), kIpRange.prefix_length(), IFA_F_PERMANENT | IFA_F_NODAD, RT_SCOPE_LINK, _))
...
TEST_F(TunDeviceControllerTest, OldAddressesAreRemoved)
{
    ...
    EXPECT_CALL(netlink_, GetAddresses(kIfindex, _, _, _))
        .WillOnce(Invoke(
            [](int interface_index, uint8_t unwanted_flags, std::vector<NetlinkInterface::AddressInfo>* addresses, int* num_ipv6_nodad_dadfailed_addresses) {
    ...
    EXPECT_CALL(netlink_, ChangeLocalAddress(kIfindex, NetlinkInterface::Verb::kRemove, old_address, kOldPrefixLen, _, _, _)).WillOnce(Return(true));
...
TEST_F(TunDeviceControllerTest, UpdateRoutesRemovedOldRoutes)
{
    ...
    EXPECT_CALL(netlink_, GetRouteInfo(_)).WillOnce(Invoke([](std::vector<NetlinkInterface::RoutingRule>* routing_rules) {
    ...
    EXPECT_CALL(netlink_, ChangeRoute(NetlinkInterface::Verb::kRemove, QboneConstants::kQboneRouteTableId, _, _, _, kIfindex))
        .Times(num_matching_routes)
        .WillRepeatedly(Return(true));

    EXPECT_CALL(netlink_, GetRuleInfo(_)).WillOnce(Return(true));

    EXPECT_CALL(netlink_, ChangeRule(NetlinkInterface::Verb::kAdd, QboneConstants::kQboneRouteTableId, IpRangeEq(kIpRange))).WillOnce(Return(true));

    EXPECT_CALL(netlink_, ChangeRoute(NetlinkInterface::Verb::kReplace, QboneConstants::kQboneRouteTableId, IpRangeEq(link_local_range_), _, _, kIfindex))
        .WillOnce(Return(true));
...
TEST_F(TunDeviceControllerTest, UpdateRoutesAddsNewRoutes)
{
    ...
    EXPECT_CALL(netlink_, ChangeRoute(NetlinkInterface::Verb::kReplace, QboneConstants::kQboneRouteTableId, IpRangeEq(kIpRange), _, _, kIfindex))
        .Times(2)
        .WillRepeatedly(Return(true))
        .RetiresOnSaturation();
