...
public:
    ...
    bool HasWriteErrors()
    {
        return !write_errors_.empty();
    }

    int WriteErrorCount(int error)
    {
        return write_errors_[error];
    }

    bool HasReadErrors()
    {
        return !read_errors_.empty();
    }

    int ReadErrorCount(int error)
    {
        return read_errors_[error];
    }
...
public:
    ...
    void SetFdExpectations()
    {
        ...
        EXPECT_CALL(kernel_, close(read_fd_)).WillOnce(Invoke([](int fd) { return close(fd); }));
    }
...
{
    IcmpReachable reachable(source_, destination_, QuicTime::Delta::Zero(), &kernel_, event_loop_.get(), &stats_);

    SetFdExpectations();
    ASSERT_TRUE(reachable.Init());

    EXPECT_CALL(kernel_, sendto(kFakeWriteFd, _, _, _, _, _))
        .WillOnce(Invoke([](int sockfd, const void* buf, size_t len, int flags, const struct sockaddr* dest_addr, socklen_t addrlen) {
...
TEST_F(IcmpReachableTest, HandlesUnreachableEvents)
{
    ...
    EXPECT_CALL(kernel_, sendto(kFakeWriteFd, _, _, _, _, _))
        .Times(2)
        .WillRepeatedly(Invoke([](int sockfd, const void* buf, size_t len, int flags, const struct sockaddr* dest_addr, socklen_t addrlen) { return len; }));
...
TEST_F(IcmpReachableTest, HandlesReachableEvents)
{
    ...
    EXPECT_CALL(kernel_, sendto(kFakeWriteFd, _, _, _, _, _))
        .Times(2)
        .WillRepeatedly(Invoke([&last_request_hdr](int sockfd, const void* buf, size_t len, int flags, const struct sockaddr* dest_addr, socklen_t addrlen) {
    ...
    EXPECT_CALL(kernel_, recvfrom(read_fd_, _, _, _, _, _))
        .WillOnce(Invoke([&source_addr](int sockfd, void* buf, size_t len, int flags, struct sockaddr* src_addr, socklen_t* addrlen) {
    ...
    write(read_src_fd_, reinterpret_cast<const void*>(&response), sizeof(icmp6_hdr));
...
TEST_F(IcmpReachableTest, HandlesReadErrors)
{
    ...
    EXPECT_CALL(kernel_, sendto(kFakeWriteFd, _, _, _, _, _))
        .WillOnce(Invoke([](int sockfd, const void* buf, size_t len, int flags, const struct sockaddr* dest_addr, socklen_t addrlen) { return len; }));

    EXPECT_CALL(kernel_, recvfrom(read_fd_, _, _, _, _, _))
        .WillOnce(Invoke([](int sockfd, void* buf, size_t len, int flags, struct sockaddr* src_addr, socklen_t* addrlen) {
            errno = EIO;
            return -1;
        }));
