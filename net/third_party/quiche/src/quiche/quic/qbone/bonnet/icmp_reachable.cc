...
constexpr QuicSocketEventMask kEventMask = kSocketEventReadable | kSocketEventWritable;
...
IcmpReachable::IcmpReachable(
    QuicIpAddress source, QuicIpAddress destination, QuicTime::Delta timeout, KernelInterface* kernel, QuicEventLoop* event_loop, StatsInterface* stats)
    : timeout_(timeout)
...
bool IcmpReachable::Init()
{
    ...
    if (kernel_->bind(send_fd_, reinterpret_cast<struct sockaddr*>(&src_), sizeof(sockaddr_in6)) < 0) {
        QUIC_PLOG(ERROR) << "Unable to bind socket.";
        return false;
    }

    recv_fd_ = kernel_->socket(PF_INET6, SOCK_RAW | SOCK_NONBLOCK, IPPROTO_ICMPV6);
    if (recv_fd_ < 0) {
        QUIC_PLOG(ERROR) << "Unable to open socket.";
        return false;
    }

    if (kernel_->bind(recv_fd_, reinterpret_cast<struct sockaddr*>(&src_), sizeof(sockaddr_in6)) < 0) {
        QUIC_PLOG(ERROR) << "Unable to bind socket.";
        return false;
    }
    ...
    if (kernel_->setsockopt(recv_fd_, SOL_ICMPV6, ICMP6_FILTER, &filter, sizeof(filter)) < 0) {
    ...
    QuicRandom::GetInstance()->RandBytes(&icmp_header_.icmp6_id, sizeof(uint16_t));
...
bool IcmpReachable::OnEvent(int fd)
{
    ...
    ssize_t size = kernel_->recvfrom(fd, &buffer, kMtu, 0, reinterpret_cast<sockaddr*>(&source_addr), &source_addr_len);
    ...
    QUIC_VLOG(2) << quiche::QuicheTextUtils::HexDump(absl::string_view(buffer, size));

    auto* header = reinterpret_cast<const icmp6_hdr*>(&buffer);
    quiche::QuicheWriterMutexLock mu(&header_lock_);
    if (header->icmp6_data32[0] != icmp_header_.icmp6_data32[0]) {
        QUIC_VLOG(2) << "Unexpected response. id: " << header->icmp6_id << " seq: " << header->icmp6_seq << " Expected id: " << icmp_header_.icmp6_id
    ...
    if (!source_ip.FromPackedString(reinterpret_cast<char*>(&source_addr.sin6_addr), sizeof(in6_addr))) {
...
void IcmpReachable::OnAlarm()
{
    ...
    CreateIcmpPacket(src_.sin6_addr, dst_.sin6_addr, icmp_header_, "", [this](absl::string_view packet) {
        QUIC_VLOG(2) << quiche::QuicheTextUtils::HexDump(packet);

        ssize_t size = kernel_->sendto(send_fd_, packet.data(), packet.size(), 0, reinterpret_cast<struct sockaddr*>(&dst_), sizeof(sockaddr_in6));
...
void IcmpReachable::EpollCallback::OnSocketEvent(QuicEventLoop* event_loop, SocketFd fd, QuicSocketEventMask events)
{
    bool can_read_more = reachable_->OnEvent(fd);
    if (can_read_more) {
        bool success = event_loop->ArtificiallyNotifyEvent(fd, kSocketEventReadable);
        QUICHE_DCHECK(success);
    }
}
