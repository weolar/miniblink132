...
public:
    ...
    TunDeviceController(std::string ifname, bool setup_tun, NetlinkInterface* netlink)
    ...
    virtual bool UpdateRoutesWithRetries(const IpRange& desired_range, const std::vector<IpRange>& desired_routes, int retries);

    virtual void RegisterAddressUpdateCallback(quiche::MultiUseCallback<void(const QuicIpAddress&)> cb);
...
private:
    ...
    std::vector<quiche::MultiUseCallback<void(const QuicIpAddress&)>> address_update_cbs_;
};
