...
ABSL_FLAG(std::string, qbone_client_tun_device_path, "/dev/net/tun", "The path to the QBONE client's TUN device.");

namespace quic {

const int kInvalidFd = -1;

TunTapDevice::TunTapDevice(const std::string& interface_name, int mtu, bool persist, bool setup_tun, bool is_tap, KernelInterface* kernel)
...
bool TunTapDevice::Init()
{
    if (interface_name_.empty() || interface_name_.size() >= IFNAMSIZ) {
        QUIC_BUG(quic_bug_10995_1) << "interface_name must be nonempty and shorter than " << IFNAMSIZ;
        return false;
    }
...
int TunTapDevice::GetFileDescriptor() const
{
...
bool TunTapDevice::OpenDevice()
{
    ...
    const std::string tun_device_path = absl::GetFlag(FLAGS_qbone_client_tun_device_path);
    ...
    if (kernel_.ioctl(fd, TUNSETPERSIST, persist_ ? reinterpret_cast<void*>(&if_request) : nullptr) != 0) {
...
bool TunTapDevice::CheckFeatures(int tun_device_fd)
{
    ...
    if ((required_features & actual_features) != required_features) {
        QUIC_LOG(WARNING) << "Required feature does not exist. required_features: 0x" << std::hex << required_features << " vs actual_features: 0x" << std::hex
                          << actual_features;
        return false;
    }
    return true;
