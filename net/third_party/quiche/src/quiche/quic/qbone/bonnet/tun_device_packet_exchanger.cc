...
TunDevicePacketExchanger::TunDevicePacketExchanger(size_t mtu, KernelInterface* kernel, NetlinkInterface* netlink, QbonePacketExchanger::Visitor* visitor,
    size_t max_pending_packets, bool is_tap, StatsInterface* stats, absl::string_view ifname)
...
bool TunDevicePacketExchanger::WritePacket(const char* packet, size_t size, bool* blocked, std::string* error)
{
    *blocked = false;
    if (fd_ < 0) {
    ...
    if (result == -1) {
        if (errno == EWOULDBLOCK || errno == EAGAIN) {
            ...
            *error = absl::ErrnoToStatus(errno, "Write to the TUN device was blocked.").message();
            *blocked = true;
            stats_->OnWriteError(error);
        }
        return false;
...
std::unique_ptr<QuicData> TunDevicePacketExchanger::ReadPacket(bool* blocked, std::string* error)
{
    *blocked = false;
    if (fd_ < 0) {
        *error = absl::StrCat("Invalid file descriptor of the TUN device: ", fd_);
    ...
    if (result <= 0) {
        if (errno == EAGAIN || errno == EWOULDBLOCK) {
            *error = absl::ErrnoToStatus(errno, "Read from the TUN device was blocked.").message();
            *blocked = true;
            stats_->OnReadError(error);
        }
        return nullptr;
...
const TunDevicePacketExchanger::StatsInterface* TunDevicePacketExchanger::stats_interface() const
{
    return stats_;
}

std::unique_ptr<QuicData> TunDevicePacketExchanger::ApplyL2Headers(const QuicData& l3_packet)
{
    if (is_tap_ && !mac_initialized_) {
        ...
        if (netlink_->GetLinkInfo(ifname_, &link_info)) {
            ...
            QUIC_LOG_EVERY_N_SEC(ERROR, 30) << "Unable to get link info for: " << ifname_;
        }
    }
...
std::unique_ptr<QuicData> TunDevicePacketExchanger::ConsumeL2Headers(const QuicData& l2_packet)
{
    ...
    if (is_icmp) {
        ...
        is_neighbor_solicit = reinterpret_cast<const icmp6_hdr*>(l2_packet.data() + kIp6PrefixLen)->icmp6_type == ND_NEIGHBOR_SOLICIT;
    }
    ...
    if (is_neighbor_solicit) {
        ...
        QuicIpAddress target_address(*reinterpret_cast<const in6_addr*>(icmp6_payload));
        ...
        // neighbor.
        CreateIcmpPacket(ip_hdr->ip6_src, ip_hdr->ip6_src, response_hdr, absl::string_view(payload.get(), payload_size), [this](absl::string_view packet) {
            bool blocked;
            std::string error;
            WritePacket(packet.data(), packet.size(), &blocked, &error);
        });
    ...
    return std::make_unique<QuicData>(shift_buffer.release(), l3_packet_size, true);
}
