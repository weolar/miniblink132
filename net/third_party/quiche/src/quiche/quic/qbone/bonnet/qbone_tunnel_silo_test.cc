...
TEST(QboneTunnelSiloTest, SiloRunsEventLoop)
{
    ...
    EXPECT_CALL(mock_tunnel, WaitForEvents).WillRepeatedly(Invoke([&event_loop_run]() {
    ...
    EXPECT_CALL(mock_tunnel, Disconnect).WillOnce(Invoke([&client_disconnected]() {
...
TEST(QboneTunnelSiloTest, SiloCanShutDownAfterInit)
{
    ...
    EXPECT_CALL(mock_tunnel, WaitForEvents).WillRepeatedly(Invoke([&iteration_count]() {
        iteration_count++;
        return false;
    }));

    EXPECT_CALL(mock_tunnel, state).WillOnce(Return(QboneTunnelInterface::START_REQUESTED)).WillOnce(Return(QboneTunnelInterface::STARTED));
