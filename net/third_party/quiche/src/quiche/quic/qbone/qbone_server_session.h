...
class QboneCryptoServerStreamHelper : public QuicCryptoServerStreamBase::Helper {
public:
    // This will look for the QBONE alpn.
    bool CanAcceptClientHello(const CryptoHandshakeMessage& chlo, const QuicSocketAddress& client_address, const QuicSocketAddress& peer_address,
        const QuicSocketAddress& self_address, std::string* error_details) const override;
};

class QUIC_EXPORT_PRIVATE QboneServerSession : public QboneSessionBase,
                                               public QbonePacketProcessor::OutputInterface,
                                               public QbonePacketProcessor::StatsInterface {
public:
    QboneServerSession(const quic::ParsedQuicVersionVector& supported_versions, QuicConnection* connection, Visitor* owner, const QuicConfig& config,
        ...
        QuicIpAddress self_ip, QuicIpAddress client_ip, size_t client_ip_subnet_length, QboneServerControlStream::Handler* handler);
    ...
    void OnPacketForwarded(QbonePacketProcessor::Direction direction, uint8_t traffic_class) override
    {
    }
    void OnPacketDroppedSilently(QbonePacketProcessor::Direction direction, uint8_t traffic_class) override
    {
    }
    void OnPacketDroppedWithIcmp(QbonePacketProcessor::Direction direction, uint8_t traffic_class) override
    {
    }
    void OnPacketDroppedWithTcpReset(QbonePacketProcessor::Direction direction, uint8_t traffic_class) override
    {
    }
    void RecordThroughput(size_t bytes, QbonePacketProcessor::Direction direction, uint8_t traffic_class) override
    {
    }
