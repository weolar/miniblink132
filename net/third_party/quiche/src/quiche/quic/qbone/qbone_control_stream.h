...
template <class T> class QUIC_EXPORT_PRIVATE QboneControlHandler {
public:
...
template <class Incoming, class Outgoing> class QUIC_EXPORT_PRIVATE QboneControlStream : public QboneControlStreamBase {
public:
    using Handler = QboneControlHandler<Incoming>;

    QboneControlStream(QuicSession* session, Handler* handler)
        : QboneControlStreamBase(session)
        , handler_(handler)
    {
    }
    QboneControlStream(quic::PendingStream* pending, QuicSession* session, Handler* handler)
        : QboneControlStreamBase(pending, session)
        , handler_(handler)
    {
    }

    bool SendRequest(const Outgoing& request)
    {
...
using QboneServerControlStream = QboneControlStream<QboneServerRequest, QboneClientRequest>;
using QboneClientControlStream = QboneControlStream<QboneClientRequest, QboneServerRequest>;
