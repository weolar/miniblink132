...
public:
    MockQuicSession(QuicConnection* connection, const QuicConfig& config)
        : QboneSessionBase(connection, nullptr /*visitor*/, config, CurrentSupportedVersions(), nullptr /*writer*/)
    {
    }
    ...
    void RegisterReliableStream(QuicStreamId stream_id)
    {
        ...
        write_blocked_streams()->RegisterStream(stream_id,
            /* is_static_stream = */ false, QuicStreamPriority());
    }
...
class QboneReadOnlyStreamTest : public ::testing::Test, public QuicConnectionHelperInterface {
public:
    void CreateReliableQuicStream()
    {
        ...
        connection_.reset(new QuicConnection(test::TestConnectionId(0), QuicSocketAddress(TestLoopback(), 0), QuicSocketAddress(TestLoopback(), 0),
            this /*QuicConnectionHelperInterface*/, alarm_factory_.get(), new DummyPacketWriter(), owns_writer, perspective,
            ParsedVersionOfIndex(CurrentSupportedVersions(), 0), connection_id_generator_));
        ...
        session_->ActivateReliableStream(std::unique_ptr<QboneReadOnlyStream>(stream_));
    }
...
protected:
    ...
    MockClock clock_;
    const QuicStreamId kStreamId = QuicUtils::GetFirstUnidirectionalStreamId(CurrentSupportedVersions()[0].transport_version, Perspective::IS_CLIENT);
...
TEST_F(QboneReadOnlyStreamTest, ReadBufferedTooLarge)
{
    ...
    EXPECT_CALL(*session_, MaybeSendStopSendingFrame(kStreamId, QuicResetStreamError::FromInternal(QUIC_BAD_APPLICATION_PAYLOAD)));
    EXPECT_CALL(*session_, MaybeSendRstStreamFrame(kStreamId, QuicResetStreamError::FromInternal(QUIC_BAD_APPLICATION_PAYLOAD), _));
    for (int i = 0; i < iterations; ++i) {
        QuicStreamFrame frame(kStreamId, i == (iterations - 1), i * packet.size(), packet);
