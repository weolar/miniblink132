...
bool QbonePacketExchanger::ReadAndDeliverPacket(QboneClientInterface* qbone_client)
{
    bool blocked = false;
    std::string error;
...
void QbonePacketExchanger::WritePacketToNetwork(const char* packet, size_t size)
{
    if (visitor_) {
    ...
    packet_queue_.push_back(std::make_unique<QuicData>(data_copy, size, /* owns_buffer = */ true));
}
...
void QbonePacketExchanger::SetWritable()
{
    ...
    while (!packet_queue_.empty()) {
        bool blocked = false;
        std::string error;
        if (WritePacket(packet_queue_.front()->data(), packet_queue_.front()->length(), &blocked, &error)) {
