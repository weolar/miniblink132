...
public:
    ...
    static void SetHeadersStream(QuicSpdySession* session, QuicHeadersStream* headers_stream);
    static spdy::SpdyFramer* GetSpdyFramer(QuicSpdySession* session);
    // Must be called before Initialize().
    static void SetMaxInboundHeaderListSize(QuicSpdySession* session, size_t max_inbound_header_size);
    static size_t WriteHeadersOnHeadersStream(QuicSpdySession* session, QuicStreamId id, quiche::HttpHeaderBlock headers, bool fin,
    ...
    static QuicStreamId GetNextOutgoingUnidirectionalStreamId(QuicSpdySession* session);
    static QuicReceiveControlStream* GetReceiveControlStream(QuicSpdySession* session);
    ...
    static QpackReceiveStream* GetQpackDecoderReceiveStream(QuicSpdySession* session);
    static QpackReceiveStream* GetQpackEncoderReceiveStream(QuicSpdySession* session);
    static void SetHttpDatagramSupport(QuicSpdySession* session, HttpDatagramSupport http_datagram_support);
