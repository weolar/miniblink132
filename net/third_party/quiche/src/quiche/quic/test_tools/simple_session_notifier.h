...
public:
    ...
    QuicConsumedData WriteOrBufferData(QuicStreamId id, QuicByteCount data_length, StreamSendingState state);
    QuicConsumedData WriteOrBufferData(QuicStreamId id, QuicByteCount data_length, StreamSendingState state, TransmissionType transmission_type);

    // Tries to write RST_STREAM_FRAME.
    void WriteOrBufferRstStream(QuicStreamId id, QuicRstStreamErrorCode error, QuicStreamOffset bytes_written);

    // Tries to write RESET_STREAM_AT_FRAME.
    void WriteOrBufferResetStreamAt(QuicStreamId id, QuicRstStreamErrorCode error, QuicStreamOffset bytes_written, QuicStreamOffset reliable_size);
    ...
    void WriteOrBufferAckFrequency(const QuicAckFrequencyFrame& ack_frequency_frame);

    // Tries to write CRYPTO data and returns the number of bytes written.
    size_t WriteCryptoData(EncryptionLevel level, QuicByteCount data_length, QuicStreamOffset offset);
...
private:
    ...
    void OnStreamDataConsumed(QuicStreamId id, QuicStreamOffset offset, QuicByteCount data_length, bool fin);
    ...
    QuicIntervalSet<QuicStreamOffset> crypto_bytes_transferred_[NUM_ENCRYPTION_LEVELS];
