...
public:
    SessionCloseVisitor(WebTransportSession* session)
        : session_(session)
    {
    }
    ...
    void OnIncomingUnidirectionalStreamAvailable() override
    {
        ...
        stream->SetVisitor(std::make_unique<WebTransportUnidirectionalEchoReadVisitor>(stream, [this](const std::string& data) {
            ...
            std::pair<absl::string_view, absl::string_view> parsed = absl::StrSplit(data, absl::MaxSplits(' ', 1));
...
QuicSimpleServerBackend::WebTransportResponse QuicTestBackend::ProcessWebTransportRequest(
    ...
    if (absl::StartsWith(path, "/echo")) {
        ...
        for (const auto& param : params) {
            ...
            if (absl::ConsumePrefix(&param_view, "set-header=")) {
                const std::vector<absl::string_view> header_value = absl::StrSplit(param_view, ':');
                if (header_value.size() == 2 && !absl::StartsWith(header_value[0], ":")) {
