...
bool LoadSendAlgorithmTestResult(SendAlgorithmTestResult* result)
{
    ...
    if (!QuicLoadTestOutput(GetSendAlgorithmTestResultFilename(), &test_result_file_content)) {
        return false;
    }
    return result->ParseFromString(test_result_file_content);
}

void RecordSendAlgorithmTestResult(uint64_t random_seed, int64_t simulated_duration_micros)
{
    ...
    QuicSaveTestOutput(GetSendAlgorithmTestResultFilename(), result.SerializeAsString());
}
...
void CompareSendAlgorithmTestResult(int64_t actual_simulated_duration_micros)
{
    ...
    QUIC_LOG(INFO) << "Loaded expected test result: " << expected.ShortDebugString();

    EXPECT_GE(expected.simulated_duration_micros(), actual_simulated_duration_micros);
}

std::string GetFullSendAlgorithmTestName()
{
    const auto* test_info = ::testing::UnitTest::GetInstance()->current_test_info();
    const std::string type_param = test_info->type_param() ? test_info->type_param() : "";
    const std::string value_param = test_info->value_param() ? test_info->value_param() : "";
    return absl::StrCat(test_info->test_suite_name(), ".", test_info->name(), "_", type_param, "_", value_param);
}
