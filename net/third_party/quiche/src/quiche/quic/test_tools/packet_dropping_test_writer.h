...
public:
    ...
    void Initialize(QuicConnectionHelperInterface* helper, QuicAlarmFactory* alarm_factory, std::unique_ptr<Delegate> on_can_write);
    ...
    void set_passthrough_for_next_n_packets(uint32_t passthrough_for_next_n_packets)
    {
        quiche::QuicheWriterMutexLock lock(&config_mutex_);
    ...
    void set_fake_blocked_socket_percentage(int32_t fake_blocked_socket_percentage)
    {
    ...
    // dropped.
    void set_max_bandwidth_and_buffer_size(QuicBandwidth fake_bandwidth, QuicByteCount buffer_size)
    {
        QUICHE_DCHECK(clock_);
        quiche::QuicheWriterMutexLock lock(&config_mutex_);
...
private:
    ...
    struct DelayedWrite {
    public:
        DelayedWrite(const char* buffer, size_t buf_len, const QuicIpAddress& self_address, const QuicSocketAddress& peer_address,
            std::unique_ptr<PerPacketOptions> options, const QuicPacketWriterParams& params, QuicTime send_time);
