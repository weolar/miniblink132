...
WriteResult PacketReorderingWriter::WritePacket(const char* buffer, size_t buf_len, const QuicIpAddress& self_address, const QuicSocketAddress& peer_address,
    PerPacketOptions* options, const QuicPacketWriterParams& params)
{
    if (!delay_next_) {
        QUIC_VLOG(2) << "Writing a non-delayed packet";
        WriteResult wr = QuicPacketWriterWrapper::WritePacket(buffer, buf_len, self_address, peer_address, options, params);
        ...
        if (num_packets_to_wait_ == 0) {
            ...
            QuicPacketWriterWrapper::WritePacket(
                delayed_data_.data(), delayed_data_.length(), delayed_self_address_, delayed_peer_address_, delayed_options_.get(), delayed_params_);
        }
        return wr;
    }
    // Still have packet to wait.
    QUICHE_DCHECK_LT(0u, num_packets_to_wait_) << "Only allow one packet to be delayed";
