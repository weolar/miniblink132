...
TrafficPolicer::TrafficPolicer(
    Simulator* simulator, std::string name, QuicByteCount initial_bucket_size, QuicByteCount max_bucket_size, QuicBandwidth target_bandwidth, Endpoint* input)
...
bool TrafficPolicer::FilterPacket(const Packet& packet)
{
    ...
    if (token_buckets_.count(packet.destination) == 0) {
        token_buckets_.insert(std::make_pair(packet.destination, initial_bucket_size_));
    }
