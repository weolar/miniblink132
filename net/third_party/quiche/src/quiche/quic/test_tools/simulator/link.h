...
public:
    OneWayLink(Simulator* simulator, std::string name, UnconstrainedPortInterface* sink, QuicBandwidth bandwidth, QuicTime::Delta propagation_delay);
...
class SymmetricLink {
public:
    SymmetricLink(Simulator* simulator, std::string name, UnconstrainedPortInterface* sink_a, UnconstrainedPortInterface* sink_b, QuicBandwidth bandwidth,
        QuicTime::Delta propagation_delay);
    SymmetricLink(Endpoint* endpoint_a, Endpoint* endpoint_b, QuicBandwidth bandwidth, QuicTime::Delta propagation_delay);
    SymmetricLink(const SymmetricLink&) = delete;
    SymmetricLink& operator=(const SymmetricLink&) = delete;

    QuicBandwidth bandwidth()
    {
        return a_to_b_link_.bandwidth();
    }
    void set_bandwidth(QuicBandwidth new_bandwidth)
    {
