...
Switch::Switch(Simulator* simulator, std::string name, SwitchPortNumber port_count, QuicByteCount queue_capacity)
{
    for (size_t port_number = 1; port_number <= port_count; port_number++) {
        ports_.emplace_back(simulator, absl::StrCat(name, " (port ", port_number, ")"), this, port_number, queue_capacity);
    }
}
...
Switch::Port::Port(Simulator* simulator, std::string name, Switch* parent, SwitchPortNumber port_number, QuicByteCount queue_capacity)
