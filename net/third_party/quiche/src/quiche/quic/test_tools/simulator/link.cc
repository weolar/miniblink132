...
OneWayLink::OneWayLink(Simulator* simulator, std::string name, UnconstrainedPortInterface* sink, QuicBandwidth bandwidth, QuicTime::Delta propagation_delay)
...
OneWayLink::QueuedPacket::QueuedPacket(std::unique_ptr<Packet> packet, QuicTime dequeue_time)
...
void OneWayLink::AcceptPacket(std::unique_ptr<Packet> packet)
{
    ...
    packets_in_transit_.emplace_back(std::move(packet),
        // Ensure that packets are delivered in order.
        std::max(next_write_at_ + propagation_delay_ + GetRandomDelay(transfer_time),
            packets_in_transit_.empty() ? QuicTime::Zero() : packets_in_transit_.back().dequeue_time));
...
QuicTime::Delta OneWayLink::GetRandomDelay(QuicTime::Delta transfer_time)
{
    ...
    QuicTime::Delta delta = QuicTime::Delta::FromMicroseconds(simulator_->GetRandomGenerator()->RandUint64() % (kMaxRandomDelayUs + 1));
    // Have an upper bound on the delay to ensure packets do not go out of order.
    delta = std::min(delta, transfer_time * 0.5);
    return delta;
}

SymmetricLink::SymmetricLink(Simulator* simulator, std::string name, UnconstrainedPortInterface* sink_a, UnconstrainedPortInterface* sink_b,
    QuicBandwidth bandwidth, QuicTime::Delta propagation_delay)
    : a_to_b_link_(simulator, absl::StrCat(name, " (A-to-B)"), sink_b, bandwidth, propagation_delay)
    , b_to_a_link_(simulator, absl::StrCat(name, " (B-to-A)"), sink_a, bandwidth, propagation_delay)
{
}

SymmetricLink::SymmetricLink(Endpoint* endpoint_a, Endpoint* endpoint_b, QuicBandwidth bandwidth, QuicTime::Delta propagation_delay)
    : SymmetricLink(endpoint_a->simulator(), absl::StrFormat("Link [%s]<->[%s]", endpoint_a->name(), endpoint_b->name()), endpoint_a->GetRxPort(),
        endpoint_b->GetRxPort(), bandwidth, propagation_delay)
{
