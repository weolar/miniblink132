...
class Switch {
public:
    Switch(Simulator* simulator, std::string name, SwitchPortNumber port_count, QuicByteCount queue_capacity);
...
private:
    ...
    public:
        Port(Simulator* simulator, std::string name, Switch* parent, SwitchPortNumber port_number, QuicByteCount queue_capacity);
        Port(Port&&) = delete;
    ...
    void DispatchPacket(SwitchPortNumber port_number, std::unique_ptr<Packet> packet);
