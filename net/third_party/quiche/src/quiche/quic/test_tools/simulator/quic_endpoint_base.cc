...
QuicEndpointBase::QuicEndpointBase(Simulator* simulator, std::string name, std::string peer_name)
    : Endpoint(simulator, name)
    , peer_name_(peer_name)
    , writer_(this)
    , nic_tx_queue_(simulator, absl::StrCat(name, " (TX Queue)"), kMaxOutgoingPacketSize * kTxQueueSize)
    , connection_(nullptr)
...
QuicEndpointBase::~QuicEndpointBase()
{
    if (trace_visitor_ != nullptr) {
        const char* perspective_prefix = connection_->perspective() == Perspective::IS_CLIENT ? "C" : "S";

        std::string identifier = absl::StrCat(perspective_prefix, connection_->connection_id().ToString());
...
void QuicEndpointBase::AcceptPacket(std::unique_ptr<Packet> packet)
{
    ...
    QuicReceivedPacket received_packet(packet->contents.data(), packet->contents.size(), clock_->Now());
    connection_->ProcessUdpPacket(connection_->self_address(), connection_->peer_address(), received_packet);
}
...
{
    if (writer_.IsWriteBlocked() && (nic_tx_queue_.capacity() - nic_tx_queue_.bytes_queued()) >= kMaxOutgoingPacketSize) {
        writer_.SetWritable();
...
WriteResult QuicEndpointBase::Writer::WritePacket(const char* buffer, size_t buf_len, const QuicIpAddress& /*self_address*/,
...
QuicByteCount QuicEndpointBase::Writer::GetMaxPacketSize(const QuicSocketAddress& /*peer_address*/) const
{
...
QuicPacketBuffer QuicEndpointBase::Writer::GetNextWriteLocation(const QuicIpAddress& /*self_address*/, const QuicSocketAddress& /*peer_address*/)
{
...
QuicEndpointMultiplexer::QuicEndpointMultiplexer(std::string name, const std::vector<QuicEndpointBase*>& endpoints)
