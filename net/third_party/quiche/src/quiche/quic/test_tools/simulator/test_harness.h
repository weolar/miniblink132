...
public:
    QuicEndpointWithConnection(Simulator* simulator, const std::string& name, const std::string& peer_name, Perspective perspective,
...
public:
    ...
    static constexpr QuicBandwidth kClientBandwidth = QuicBandwidth::FromKBitsPerSecond(10000);
    static constexpr QuicTime::Delta kClientPropagationDelay = QuicTime::Delta::FromMilliseconds(2);
    static constexpr QuicBandwidth kServerBandwidth = QuicBandwidth::FromKBitsPerSecond(4000);
    static constexpr QuicTime::Delta kServerPropagationDelay = QuicTime::Delta::FromMilliseconds(50);
    static constexpr QuicTime::Delta kTransferTime
        = kClientBandwidth.TransferTime(kMaxOutgoingPacketSize) + kServerBandwidth.TransferTime(kMaxOutgoingPacketSize);
    static constexpr QuicTime::Delta kRtt = (kClientPropagationDelay + kServerPropagationDelay + kTransferTime) * 2;
    static constexpr QuicByteCount kBdp = kRtt * kServerBandwidth;

    static constexpr QuicTime::Delta kDefaultTimeout = QuicTime::Delta::FromSeconds(3);
    ...
    void set_client(Endpoint* client)
    {
        client_ = client;
    }
    void set_server(Endpoint* server)
    {
        server_ = server;
    }
    ...
    template <class TerminationPredicate> bool RunUntilWithDefaultTimeout(TerminationPredicate termination_predicate)
    {
        return simulator_.RunUntilOrTimeout(std::move(termination_predicate), kDefaultTimeout);
    }
