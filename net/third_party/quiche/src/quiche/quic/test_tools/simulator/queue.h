...
public:
    ...
    QuicByteCount capacity() const
    {
        return capacity_;
    }
    QuicByteCount bytes_queued() const
    {
        return bytes_queued_;
    }
    QuicPacketCount packets_queued() const
    {
        return queue_.size();
    }
    ...
    void EnableAggregation(QuicByteCount aggregation_threshold, QuicTime::Delta aggregation_timeout);
...
private:
    ...
    struct EnqueuedPacket {
        EnqueuedPacket(std::unique_ptr<Packet> packet, AggregationBundleNumber bundle);
