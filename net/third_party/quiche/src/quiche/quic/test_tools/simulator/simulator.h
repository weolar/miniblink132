...
public:
    ...
    bool enable_random_delays() const
    {
        return enable_random_delays_;
    }
    ...
    template <class TerminationPredicate> bool RunUntil(TerminationPredicate termination_predicate);

    // Same as RunUntil, except this function also accepts a |deadline|, and will
    // return false if the deadline is exceeded.
    template <class TerminationPredicate> bool RunUntilOrTimeout(TerminationPredicate termination_predicate, QuicTime::Delta deadline);
...
private:
    ...
    public:
        ...
        // timestamp.
        const QuicTime kStartTime = QuicTime::Zero() + QuicTime::Delta::FromMicroseconds(1);
...
template <class TerminationPredicate> bool Simulator::RunUntil(TerminationPredicate termination_predicate)
{
...
template <class TerminationPredicate> bool Simulator::RunUntilOrTimeout(TerminationPredicate termination_predicate, QuicTime::Delta timeout)
{
    QuicTime end_time = clock_.Now() + timeout;
    bool return_value = RunUntil([end_time, &termination_predicate, this]() { return termination_predicate() || clock_.Now() >= end_time; });
