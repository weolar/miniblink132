...
public:
    ...
    public:
        ...
        virtual std::unique_ptr<QuicServerSessionBase> CreateSession(const QuicConfig& config, QuicConnection* connection, QuicSession::Visitor* visitor,
            ...
            QuicSimpleServerBackend* quic_simple_server_backend, absl::string_view alpn)
            = 0;
    };
    ...
    public:
        ...
        virtual QuicSimpleServerStream* CreateStream(QuicStreamId id, QuicSpdySession* session, QuicSimpleServerBackend* quic_simple_server_backend) = 0;

        virtual QuicSimpleServerStream* CreateStream(PendingStream* pending, QuicSpdySession* session, QuicSimpleServerBackend* quic_simple_server_backend) = 0;
    };
    ...
    public:
        ...
        virtual std::unique_ptr<QuicCryptoServerStreamBase> CreateCryptoStream(const QuicCryptoServerConfig* crypto_config, QuicServerSessionBase* session) = 0;
    };

    QuicTestServer(std::unique_ptr<ProofSource> proof_source, QuicSimpleServerBackend* quic_simple_server_backend);
    QuicTestServer(std::unique_ptr<ProofSource> proof_source, const QuicConfig& config, const ParsedQuicVersionVector& supported_versions,
...
public:
    ImmediateGoAwaySession(const QuicConfig& config, QuicConnection* connection, QuicSession::Visitor* visitor, QuicCryptoServerStreamBase::Helper* helper,
