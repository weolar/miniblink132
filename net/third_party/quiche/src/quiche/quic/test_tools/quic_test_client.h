...
public:
    ...
    SocketFd CreateUDPSocket(QuicSocketAddress server_address, bool* overflow_supported) override;

    QuicPacketWriter* CreateQuicPacketWriter() override;

    void set_socket_fd_configurator(quiche::MultiUseCallback<void(SocketFd)> socket_fd_configurator);
...
public:
    ...
    MockableQuicClient(QuicSocketAddress server_address, const QuicServerId& server_id, const QuicConfig& config,
...
public:
    QuicTestClient(QuicSocketAddress server_address, const std::string& server_hostname, const ParsedQuicVersionVector& supported_versions);
    QuicTestClient(
    ...
    QuicTestClient(QuicSocketAddress server_address, const std::string& server_hostname, const QuicConfig& config,
    ...
    int64_t SendData(const std::string& data, bool last_data, quiche::QuicheReferenceCountedPointer<QuicAckListenerInterface> ack_listener);
    ...
    void SendRequestsAndWaitForResponses(const std::vector<std::string>& url_list);
    ...
    int64_t SendMessage(const quiche::HttpHeaderBlock& headers, absl::string_view body);
    ...
    int64_t SendMessage(const quiche::HttpHeaderBlock& headers, absl::string_view body, bool fin);
    // Sends a request containing |headers| and |body| with the fin bit set to
    // |fin| and returns the number of bytes sent (the size of the serialized
    // request headers and body). If |flush| is true, will wait for the message to
    // be flushed before returning.
    int64_t SendMessage(const quiche::HttpHeaderBlock& headers, absl::string_view body, bool fin, bool flush);
    // Sends a request containing |headers| and |body|, waits for the response,
    // and returns the response body.
    std::string SendCustomSynchronousRequest(const quiche::HttpHeaderBlock& headers, const std::string& body);
    ...
    bool WaitUntil(int timeout_ms, std::optional<quiche::UnretainedCallback<bool()>> trigger);
    ...
    {
        WaitUntil(timeout_ms, [this]() { return !HaveActiveStream() || !closed_stream_states_.empty(); });
        if (response_complete()) {
            QUIC_VLOG(1) << "Client received response:" << response_headers()->DebugString() << response_body();
        }
    }
    ...
    {
        WaitUntil(timeout_ms, [this]() { return !HaveActiveStream() || response_size() != 0; });
    }
    ...
    const MockableQuicClient* client() const
    {
        return client_.get();
    }
    ...
    void set_auto_reconnect(bool reconnect)
    {
        auto_reconnect_ = reconnect;
    }

    void set_priority(spdy::SpdyPriority priority)
    {
        priority_ = priority;
    }
    ...
    bool PopulateHeaderBlockFromUrl(const std::string& uri, quiche::HttpHeaderBlock* headers);
...
private:
    ...
    struct PerStreamState {
        ...
        PerStreamState(QuicRstStreamErrorCode stream_error, bool response_complete, bool response_headers_complete,
    ...
    quiche::QuicheLinkedHashMap<QuicStreamId, PerStreamState> closed_stream_states_;
