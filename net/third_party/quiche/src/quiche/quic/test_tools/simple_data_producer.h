...
public:
    ...
    void SaveCryptoData(EncryptionLevel level, QuicStreamOffset offset, absl::string_view data);
...
private:
    using SendBufferMap = absl::flat_hash_map<QuicStreamId, std::unique_ptr<QuicStreamSendBuffer>>;

    using CryptoBufferMap = absl::flat_hash_map<std::pair<EncryptionLevel, QuicStreamOffset>, std::string>;
