...
public:
    static void set_ack_listener(QuicSpdyStream* stream, quiche::QuicheReferenceCountedPointer<QuicAckListenerInterface> ack_listener);
    static const QuicIntervalSet<QuicStreamOffset>& unacked_frame_headers_offsets(QuicSpdyStream* stream);
    static bool OnHeadersFrameEnd(QuicSpdyStream* stream);
    static void set_header_decoding_delay(QuicSpdyStream* stream, QuicTime::Delta delay);
};
