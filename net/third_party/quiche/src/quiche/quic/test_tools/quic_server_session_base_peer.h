...
public:
    static QuicStream* GetOrCreateStream(QuicServerSessionBase* s, QuicStreamId id)
    {
        return s->GetOrCreateStream(id);
    }
    static void SetCryptoStream(QuicServerSessionBase* s, QuicCryptoServerStreamBase* crypto_stream)
    {
