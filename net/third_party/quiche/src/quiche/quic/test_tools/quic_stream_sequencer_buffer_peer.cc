...
static const size_t kBlockSizeBytes = quic::QuicStreamSequencerBuffer::kBlockSizeBytes;

namespace quic {
namespace test {

QuicStreamSequencerBufferPeer::QuicStreamSequencerBufferPeer(QuicStreamSequencerBuffer* buffer)
    : buffer_(buffer)
{
}
...
size_t QuicStreamSequencerBufferPeer::Read(char* dest_buffer, size_t size)
{
    ...
    EXPECT_THAT(buffer_->Readv(&dest, 1, &bytes_read, &error_details), IsQuicNoError());
    return bytes_read;
}
...
{
    QuicStreamOffset data_span = buffer_->NextExpectedByte() - buffer_->total_bytes_read_;
    ...
    if (!capacity_sane) {
        ...
        QUIC_LOG(ERROR) << "total read: " << buffer_->total_bytes_read_ << " last byte: " << buffer_->NextExpectedByte();
    }
    bool total_read_sane = buffer_->FirstMissingByte() >= buffer_->total_bytes_read_;
    if (!total_read_sane) {
    ...
    bool block_match_capacity = (buffer_->max_buffer_capacity_bytes_ <= buffer_->max_blocks_count_ * kBlockSizeBytes)
        && (buffer_->max_buffer_capacity_bytes_ > (buffer_->max_blocks_count_ - 1) * kBlockSizeBytes);
    if (!capacity_sane) {
...
size_t QuicStreamSequencerBufferPeer::GetInBlockOffset(QuicStreamOffset offset)
{
...
int QuicStreamSequencerBufferPeer::IntervalSize()
{
    ...
    if (buffer_->bytes_received_.rbegin()->max() == std::numeric_limits<uint64_t>::max()) {
        --gap_size;
    }
    return gap_size;
}
...
void QuicStreamSequencerBufferPeer::set_total_bytes_read(QuicStreamOffset total_bytes_read)
{
    buffer_->total_bytes_read_ = total_bytes_read;
}

void QuicStreamSequencerBufferPeer::AddBytesReceived(QuicStreamOffset offset, QuicByteCount length)
{
...
const QuicIntervalSet<QuicStreamOffset>& QuicStreamSequencerBufferPeer::bytes_received()
{
