...
public:
    ...
    std::vector<uint8_t> Encrypt(absl::string_view in, absl::string_view encryption_key) override;
    void Decrypt(absl::string_view in, std::shared_ptr<ProofSource::DecryptCallback> callback) override;
    ...
    void set_fail_decrypt(bool fail_decrypt)
    {
        fail_decrypt_ = fail_decrypt;
    }
    void set_fail_encrypt(bool fail_encrypt)
    {
        fail_encrypt_ = fail_encrypt;
    }
