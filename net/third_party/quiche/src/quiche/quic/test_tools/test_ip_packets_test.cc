...
TEST(TestIpPacketsTest, CreateIpv4Packet)
{
    ...
    std::string packet = CreateIpPacket(source_ip, destination_ip,
        CreateUdpPacket(source_address, destination_address,
            /*payload=*/"foo"),
        IpPacketPayloadType::kUdp);

    constexpr static char kExpected[] = "\x45" // Version: 4, Header length: 5 words
                                        "\x00" // DSCP: 0, ECN: 0
                                        ...
                                        "foo"; // Payload
    EXPECT_EQ(absl::string_view(packet), absl::string_view(kExpected, sizeof(kExpected) - 1));
}
...
TEST(TestIpPacketsTest, CreateIpv6Packet)
{
    ...
    constexpr static char kExpected[] = "\x60\x00\x00\x00" // Version: 6, Traffic class: 0, Flow label: 0
