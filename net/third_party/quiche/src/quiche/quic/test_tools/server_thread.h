...
public:
    ServerThread(std::unique_ptr<QuicServer> server, const QuicSocketAddress& address);
    ...
    // true.
    bool WaitUntil(quiche::UnretainedCallback<bool()> termination_predicate, QuicTime::Delta timeout);
...
private:
    ...
    quiche::QuicheNotification confirmed_; // Notified when the first handshake is confirmed.
    ...
    quiche::QuicheNotification resume_; // Notified when the server should resume.
    ...
    quiche::QuicheCircularDeque<quiche::SingleUseCallback<void()>> scheduled_actions_ QUICHE_GUARDED_BY(scheduled_actions_lock_);
};
