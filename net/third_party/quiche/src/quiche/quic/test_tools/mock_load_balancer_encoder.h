...
public:
    MockLoadBalancerEncoder()
        : LoadBalancerEncoder(*QuicRandom::GetInstance(), nullptr, false, kLoadBalancerUnroutableLen)
    {
    }
    ...
    MOCK_METHOD(std::optional<QuicConnectionId>, GenerateNextConnectionId, (const QuicConnectionId& original), (override));
    MOCK_METHOD(std::optional<QuicConnectionId>, MaybeReplaceConnectionId, (const QuicConnectionId& original, const ParsedQuicVersion& version), (override));
    MOCK_METHOD(uint8_t, ConnectionIdLength, (uint8_t first_byte), (const, override));
