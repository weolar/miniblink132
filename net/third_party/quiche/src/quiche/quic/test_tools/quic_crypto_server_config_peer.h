...
public:
    ...
    quiche::QuicheReferenceCountedPointer<QuicCryptoServerConfig::Config> GetPrimaryConfig();

    // Returns the config associated with |config_id|.
    quiche::QuicheReferenceCountedPointer<QuicCryptoServerConfig::Config> GetConfig(std::string config_id);
    ...
    std::string NewSourceAddressToken(std::string config_id, SourceAddressTokens previous_tokens, const QuicIpAddress& ip, QuicRandom* rand, QuicWallTime now,
    ...
    HandshakeFailureReason ValidateSingleSourceAddressToken(absl::string_view token, const QuicIpAddress& ip, QuicWallTime now);
    ...
    //      {"id2", true},
    //      {"id3", false}});
    void CheckConfigs(std::vector<std::pair<ServerConfigID, bool>> expected_ids_and_status);
    ...
    std::string ConfigsDebug() QUICHE_SHARED_LOCKS_REQUIRED(server_config_->configs_lock_);
