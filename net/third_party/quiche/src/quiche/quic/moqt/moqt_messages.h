...
public:
    ...
    explicit FullTrackName(std::initializer_list<const absl::string_view> elements)
        : FullTrackName(absl::Span<const absl::string_view>(std::data(elements), std::size(elements)))
    {
    }
    ...
    template <typename H> friend H AbslHashValue(H h, const FullTrackName& m)
    {
        return H::combine(std::move(h), m.tuple_);
    }

    template <typename Sink> friend void AbslStringify(Sink& sink, const FullTrackName& track_name)
    {
...
struct FullSequence {
    ...
    bool operator<(const FullSequence& other) const
    {
        return group < other.group || (group == other.group && object < other.object);
    }
    bool operator<=(const FullSequence& other) const
    {
        return (group < other.group || (group == other.group && object <= other.object));
    ...
    template <typename H> friend H AbslHashValue(H h, const FullSequence& m);

    template <typename Sink> friend void AbslStringify(Sink& sink, const FullSequence& sequence)
    {
...
struct SubgroupPriority {
    ...
    {
        return publisher_priority == other.publisher_priority && subgroup_id == other.subgroup_id;
    }
    bool operator<(const SubgroupPriority& other) const
    {
        return publisher_priority < other.publisher_priority || (publisher_priority == other.publisher_priority && subgroup_id < other.subgroup_id);
    }
    bool operator<=(const SubgroupPriority& other) const
    {
        return (publisher_priority < other.publisher_priority || (publisher_priority == other.publisher_priority && subgroup_id <= other.subgroup_id));
...
template <typename H> H AbslHashValue(H h, const FullSequence& m)
{
...
struct QUICHE_EXPORT MoqtSubscribeParameters {
    ...
    {
        return authorization_info == other.authorization_info && delivery_timeout == other.delivery_timeout && max_cache_duration == other.max_cache_duration
...
std::string MoqtForwardingPreferenceToString(MoqtForwardingPreference preference);

MoqtForwardingPreference GetForwardingPreference(MoqtDataStreamType type);

MoqtDataStreamType GetMessageTypeForForwardingPreference(MoqtForwardingPreference preference);
