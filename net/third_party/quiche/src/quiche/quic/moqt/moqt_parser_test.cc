...
using GeneralizedMessageType = absl::variant<MoqtMessageType, MoqtDataStreamType>;
} // namespace
    ...
    for (MoqtMessageType message_type : kMessageTypes) {
        if (message_type == MoqtMessageType::kClientSetup) {
            for (const bool uses_web_transport : { false, true }) {
                params.push_back(MoqtParserTestParams(message_type, uses_web_transport));
            }
        } else {
...
std::string ParamNameFormatter(const testing::TestParamInfo<MoqtParserTestParams>& info)
{
    return absl::visit([](auto x) { return TypeFormatter(x); }, info.param.message_type) + "_" + (info.param.uses_web_transport ? "WebTransport" : "QUIC");
}

class MoqtParserTestVisitor : public MoqtControlParserVisitor, public MoqtDataParserVisitor {
public:
    ...
    template <typename Message> void OnControlMessage(const Message& message)
    {
...
class MoqtParserTest : public quic::test::QuicTestWithParam<MoqtParserTestParams> {
public:
    MoqtParserTest()
    ...
    std::unique_ptr<TestMessageBase> MakeMessage()
    {
        if (IsDataStream()) {
            ...
            return CreateTestMessage(absl::get<MoqtMessageType>(message_type_), webtrans_);
        }
    }
...
INSTANTIATE_TEST_SUITE_P(MoqtParserTests, MoqtParserTest, testing::ValuesIn(GetMoqtParserTestParams()), ParamNameFormatter);
...
TEST_P(MoqtParserTest, TwoPartMessage)
{
    ...
    ProcessData(message->PacketSample().substr(first_data_size, message->total_message_size() - first_data_size), true);
    EXPECT_EQ(visitor_.messages_received_, 1);
    EXPECT_TRUE(message->EqualFieldValues(*visitor_.last_message_));
    EXPECT_TRUE(visitor_.end_of_message_);
    EXPECT_FALSE(visitor_.parsing_error_.has_value());
...
TEST_P(MoqtParserTest, EarlyFin)
{
    ...
    EXPECT_EQ(visitor_.messages_received_, 0);
    EXPECT_THAT(visitor_.parsing_error_, AnyOf("FIN after incomplete message", "FIN received at an unexpected point in the stream"));
}
...
TEST_P(MoqtParserTest, SeparateEarlyFin)
{
    ...
    EXPECT_EQ(visitor_.messages_received_, 0);
    EXPECT_THAT(visitor_.parsing_error_, AnyOf("End of stream before complete message", "FIN received at an unexpected point in the stream"));
...
TEST_P(MoqtParserTest, PayloadLengthTooLong)
{
    ...
    EXPECT_EQ(visitor_.messages_received_, 1);
    EXPECT_EQ(visitor_.parsing_error_, "Message length does not match payload length");
}
...
TEST_F(MoqtMessageSpecificTest, ThreePartObjectFirstIncomplete)
{
    ...
    parser.ProcessData(message->PacketSample().substr(4, message->total_message_size() - 4), false);
    EXPECT_EQ(visitor_.messages_received_, 0);
    EXPECT_TRUE(message->EqualFieldValues(*visitor_.last_message_));
    EXPECT_FALSE(visitor_.end_of_message_);
...
TEST_F(MoqtMessageSpecificTest, ClientSetupRoleIsMissing)
{
    ...
    EXPECT_EQ(visitor_.messages_received_, 0);
    EXPECT_TRUE(visitor_.parsing_error_.has_value());
    EXPECT_EQ(*visitor_.parsing_error_, "ROLE parameter missing from CLIENT_SETUP message");
    EXPECT_EQ(visitor_.parsing_error_code_, MoqtError::kProtocolViolation);
}
...
TEST_F(MoqtMessageSpecificTest, ClientSetupMaxSubscribeIdAppearsTwice)
{
    ...
    EXPECT_EQ(visitor_.messages_received_, 0);
    EXPECT_TRUE(visitor_.parsing_error_.has_value());
    EXPECT_EQ(*visitor_.parsing_error_, "MAX_SUBSCRIBE_ID parameter appears twice in SETUP");
    EXPECT_EQ(visitor_.parsing_error_code_, MoqtError::kProtocolViolation);
}
...
TEST_F(MoqtMessageSpecificTest, ServerSetupRoleIsMissing)
{
    ...
    EXPECT_EQ(visitor_.messages_received_, 0);
    EXPECT_TRUE(visitor_.parsing_error_.has_value());
    EXPECT_EQ(*visitor_.parsing_error_, "ROLE parameter missing from SERVER_SETUP message");
    EXPECT_EQ(visitor_.parsing_error_code_, MoqtError::kProtocolViolation);
}
...
TEST_F(MoqtMessageSpecificTest, SetupRoleVarintLengthIsWrong)
{
    ...
    EXPECT_EQ(visitor_.messages_received_, 0);
    EXPECT_TRUE(visitor_.parsing_error_.has_value());
    EXPECT_EQ(*visitor_.parsing_error_, "Parameter length does not match varint encoding");
...
TEST_F(MoqtMessageSpecificTest, SetupPathAppearsTwice)
{
    ...
    EXPECT_EQ(visitor_.messages_received_, 0);
    EXPECT_TRUE(visitor_.parsing_error_.has_value());
    EXPECT_EQ(*visitor_.parsing_error_, "PATH parameter appears twice in CLIENT_SETUP");
    EXPECT_EQ(visitor_.parsing_error_code_, MoqtError::kProtocolViolation);
}
...
TEST_F(MoqtMessageSpecificTest, SetupPathOverWebtrans)
{
    ...
    EXPECT_EQ(visitor_.messages_received_, 0);
    EXPECT_TRUE(visitor_.parsing_error_.has_value());
    EXPECT_EQ(*visitor_.parsing_error_, "WebTransport connection is using PATH parameter in SETUP");
    EXPECT_EQ(visitor_.parsing_error_code_, MoqtError::kProtocolViolation);
}
...
TEST_F(MoqtMessageSpecificTest, SetupPathMissing)
{
    ...
    EXPECT_EQ(visitor_.messages_received_, 0);
    EXPECT_TRUE(visitor_.parsing_error_.has_value());
    EXPECT_EQ(*visitor_.parsing_error_, "PATH SETUP parameter missing from Client message over QUIC");
    EXPECT_EQ(visitor_.parsing_error_code_, MoqtError::kProtocolViolation);
}
...
TEST_F(MoqtMessageSpecificTest, SubscribeAuthorizationInfoTwice)
{
    MoqtControlParser parser(kWebTrans, visitor_);
    char subscribe[] = {
        0x03, 0x1a, 0x01, 0x02, 0x01, 0x03, 0x66, 0x6f, 0x6f, // track_namespace = "foo"
    ...
    EXPECT_EQ(visitor_.messages_received_, 0);
    EXPECT_EQ(visitor_.parsing_error_, "AUTHORIZATION_INFO parameter appears twice");
    EXPECT_EQ(visitor_.parsing_error_code_, MoqtError::kProtocolViolation);
}
...
TEST_F(MoqtMessageSpecificTest, SubscribeDeliveryTimeoutTwice)
{
    MoqtControlParser parser(kRawQuic, visitor_);
    char subscribe[] = {
        0x03, 0x18, 0x01, 0x02, 0x01, 0x03, 0x66, 0x6f, 0x6f, // track_namespace = "foo"
        0x04, 0x61, 0x62, 0x63, 0x64, // track_name = "abcd"
        0x20, 0x02, // priority = 0x20 descending
        0x02, // filter_type = kLatestObject
        0x02, // two params
    ...
    parser.ProcessData(absl::string_view(subscribe, sizeof(subscribe)), false);
    EXPECT_EQ(visitor_.messages_received_, 0);
    EXPECT_EQ(visitor_.parsing_error_, "DELIVERY_TIMEOUT parameter appears twice");
    EXPECT_EQ(visitor_.parsing_error_code_, MoqtError::kProtocolViolation);
}
...
TEST_F(MoqtMessageSpecificTest, SubscribeDeliveryTimeoutMalformed)
{
    MoqtControlParser parser(kRawQuic, visitor_);
    char subscribe[] = {
        0x03, 0x14, 0x01, 0x02, 0x01, 0x03, 0x66, 0x6f, 0x6f, // track_namespace = "foo"
        0x04, 0x61, 0x62, 0x63, 0x64, // track_name = "abcd"
        0x20, 0x02, // priority = 0x20 descending
        0x02, // filter_type = kLatestObject
        0x01, // one param
...
TEST_F(MoqtMessageSpecificTest, SubscribeMaxCacheDurationTwice)
{
    ...
    parser.ProcessData(absl::string_view(subscribe, sizeof(subscribe)), false);
    EXPECT_EQ(visitor_.messages_received_, 0);
    EXPECT_EQ(visitor_.parsing_error_, "MAX_CACHE_DURATION parameter appears twice");
    EXPECT_EQ(visitor_.parsing_error_code_, MoqtError::kProtocolViolation);
}
...
TEST_F(MoqtMessageSpecificTest, SubscribeOkHasAuthorizationInfo)
{
    ...
    parser.ProcessData(absl::string_view(subscribe_ok, sizeof(subscribe_ok)), false);
    EXPECT_EQ(visitor_.messages_received_, 0);
...
TEST_F(MoqtMessageSpecificTest, SubscribeUpdateHasAuthorizationInfo)
{
    ...
    parser.ProcessData(absl::string_view(subscribe_update, sizeof(subscribe_update)), false);
    EXPECT_EQ(visitor_.messages_received_, 0);
...
TEST_F(MoqtMessageSpecificTest, FinMidPayload)
{
    MoqtDataParser parser(&visitor_);
    auto message = std::make_unique<StreamHeaderSubgroupMessage>();
    parser.ProcessData(message->PacketSample().substr(0, message->total_message_size() - 1), true);
    EXPECT_EQ(visitor_.messages_received_, 0);
    EXPECT_EQ(visitor_.parsing_error_, "FIN received at an unexpected point in the stream");
    EXPECT_EQ(visitor_.parsing_error_code_, MoqtError::kProtocolViolation);
}
...
TEST_F(MoqtMessageSpecificTest, PartialPayloadThenFin)
{
    ...
    parser.ProcessData(message->PacketSample().substr(0, message->total_message_size() - 1), false);
...
TEST_F(MoqtMessageSpecificTest, InvalidObjectStatus)
{
    ...
    parser.ProcessData(absl::string_view(stream_header_subgroup, sizeof(stream_header_subgroup)), false);
...
TEST_F(MoqtMessageSpecificTest, Setup2KB)
{
    ...
    parser.ProcessData(absl::string_view(big_message, writer.length() - 1), false);
    EXPECT_EQ(visitor_.messages_received_, 0);
    EXPECT_TRUE(visitor_.parsing_error_.has_value());
...
TEST_F(MoqtMessageSpecificTest, LatestGroup)
{
    ...
    MoqtSubscribe message = std::get<MoqtSubscribe>(visitor_.last_message_.value());
...
TEST_F(MoqtMessageSpecificTest, SubscribeUpdateNoEndGroup)
{
    ...
    EXPECT_EQ(visitor_.messages_received_, 0);
    EXPECT_TRUE(visitor_.parsing_error_.has_value());
    EXPECT_EQ(*visitor_.parsing_error_, "SUBSCRIBE_UPDATE has end_object but no end_group");
}
...
TEST_F(MoqtMessageSpecificTest, ObjectAckNegativeDelta)
{
    ...
    MoqtObjectAck message = std::get<MoqtObjectAck>(visitor_.last_message_.value());
    EXPECT_EQ(message.subscribe_id, 0x01);
    EXPECT_EQ(message.group_id, 0x10);
    EXPECT_EQ(message.object_id, 0x20);
    EXPECT_EQ(message.delta_from_deadline, quic::QuicTimeDelta::FromMicroseconds(-0x40));
}
...
TEST_F(MoqtMessageSpecificTest, AllMessagesTogether)
{
    ...
    for (MoqtMessageType type : kMessageTypes) {
        ...
        std::unique_ptr<TestMessageBase> message = CreateTestMessage(type, kRawQuic);
        memcpy(buffer + write, message->PacketSample().data(), message->total_message_size());
        size_t new_read = write + message->total_message_size() / 2;
        parser.ProcessData(absl::string_view(buffer + read, new_read - read), false);
...
TEST_F(MoqtMessageSpecificTest, DatagramSuccessful)
{
    ...
    TestMessageBase::MessageStructuredData object_metadata = TestMessageBase::MessageStructuredData(object);
...
TEST_F(MoqtMessageSpecificTest, VeryTruncatedDatagram)
{
    char message = 0x40;
    MoqtObject object;
    absl::string_view payload = ParseDatagram(absl::string_view(&message, sizeof(message)), object);
    EXPECT_TRUE(payload.empty());
}
...
TEST_F(MoqtMessageSpecificTest, SubscribeOkInvalidContentExists)
{
    ...
    EXPECT_EQ(visitor_.messages_received_, 0);
    EXPECT_TRUE(visitor_.parsing_error_.has_value());
    EXPECT_EQ(*visitor_.parsing_error_, "SUBSCRIBE_OK ContentExists has invalid value");
}
...
TEST_F(MoqtMessageSpecificTest, SubscribeOkInvalidDeliveryOrder)
{
    ...
    EXPECT_EQ(visitor_.messages_received_, 0);
    EXPECT_TRUE(visitor_.parsing_error_.has_value());
    EXPECT_EQ(*visitor_.parsing_error_, "Invalid group order value in SUBSCRIBE_OK");
}
...
TEST_F(MoqtMessageSpecificTest, SubscribeDoneInvalidContentExists)
{
    ...
    EXPECT_EQ(visitor_.messages_received_, 0);
    EXPECT_TRUE(visitor_.parsing_error_.has_value());
    EXPECT_EQ(*visitor_.parsing_error_, "SUBSCRIBE_DONE ContentExists has invalid value");
}
...
TEST_F(MoqtMessageSpecificTest, FetchInvalidRange)
{
    ...
    EXPECT_EQ(visitor_.messages_received_, 0);
    EXPECT_TRUE(visitor_.parsing_error_.has_value());
    EXPECT_EQ(*visitor_.parsing_error_, "End object comes before start object in FETCH");
}
...
TEST_F(MoqtMessageSpecificTest, FetchInvalidGroupOrder)
{
    ...
    EXPECT_EQ(visitor_.messages_received_, 0);
    EXPECT_TRUE(visitor_.parsing_error_.has_value());
    EXPECT_EQ(*visitor_.parsing_error_, "Invalid group order value in FETCH message");
}
...
TEST_F(MoqtMessageSpecificTest, PaddingStream)
{
    ...
    ASSERT_TRUE(writer.WriteVarInt62(static_cast<uint64_t>(MoqtDataStreamType::kPadding)));
    for (int i = 0; i < 100; ++i) {
