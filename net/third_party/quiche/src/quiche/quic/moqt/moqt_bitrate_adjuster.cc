...
constexpr QuicTimeDelta kMaxTimeBetweenAdjustments = QuicTimeDelta::FromSeconds(3);

} // namespace

void MoqtBitrateAdjuster::OnObjectAckReceived(uint64_t /*group_id*/, uint64_t /*object_id*/, QuicTimeDelta delta_from_deadline)
{
...
void MoqtBitrateAdjuster::AttemptAdjustingDown()
{
    ...
    QuicTimeDelta adjustment_delay = QuicTimeDelta(stats.smoothed_rtt * kMinTimeBetweenAdjustmentsInRtts);
    ...
    QuicBandwidth target_bandwidth = kTargetBitrateMultiplier * QuicBandwidth::FromBitsPerSecond(stats.estimated_send_rate_bps);
    QuicBandwidth current_bandwidth = adjustable_->GetCurrentBitrate();
    if (current_bandwidth <= target_bandwidth) {
        return;
    }

    QUICHE_DLOG(INFO) << "Adjusting the bitrate from " << current_bandwidth << " to " << target_bandwidth;
...
{
    QUICHE_DLOG_IF(WARNING, !supported) << "OBJECT_ACK not supported; bitrate adjustments will not work.";
}
