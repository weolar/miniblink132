...
public:
    MoqtClientEndpoint(quic::simulator::Simulator* simulator, const std::string& name, const std::string& peer_name, MoqtVersion version);

    MoqtSession* session()
    {
        return &session_;
    }
    quic::QuicGenericClientSession* quic_session()
    {
        return &quic_session_;
    }
...
public:
    MoqtServerEndpoint(quic::simulator::Simulator* simulator, const std::string& name, const std::string& peer_name, MoqtVersion version);

    MoqtSession* session()
    {
        return &session_;
    }
    quic::QuicGenericServerSession* quic_session()
    {
        return &quic_session_;
    }
