...
public:
    ...
    using MessageStructuredData = absl::variant<MoqtClientSetup, MoqtServerSetup, MoqtObject, MoqtSubscribe, MoqtSubscribeOk, MoqtSubscribeError,
        MoqtUnsubscribe, MoqtSubscribeDone, MoqtSubscribeUpdate, MoqtAnnounce, MoqtAnnounceOk, MoqtAnnounceError, MoqtAnnounceCancel, MoqtTrackStatusRequest,
        MoqtUnannounce, MoqtTrackStatus, MoqtGoAway, MoqtSubscribeAnnounces, MoqtSubscribeAnnouncesOk, MoqtSubscribeAnnouncesError, MoqtUnsubscribeAnnounces,
        MoqtMaxSubscribeId, MoqtFetch, MoqtFetchCancel, MoqtFetchOk, MoqtFetchError, MoqtObjectAck>;

    // The total actual size of the message.
    size_t total_message_size() const
    {
        return wire_image_size_;
    }
    ...
    {
        size_t length_offset = 0x1 << ((static_cast<uint8_t>(wire_image_[0]) & 0xc0) >> 6);
...
protected:
    ...
    void ExpandVarintsImpl(absl::string_view varints, bool is_control_message = true)
    {
        int next_varint_len = 2;
        char new_wire_image[kMaxMessageHeaderSize + 1];
        quic::QuicDataReader reader(absl::string_view(wire_image_, wire_image_size_));
        ...
        if (is_control_message) {
            ...
            if (!nonvarint_type) {
                ...
                writer.WriteVarInt62WithForcedLength(value, static_cast<quiche::QuicheVariableLengthIntegerLength>(next_varint_len));
                next_varint_len *= 2;
            ...
            writer.WriteVarInt62WithForcedLength(value, static_cast<quiche::QuicheVariableLengthIntegerLength>(2));
        }
        ...
        if (is_control_message) {
            wire_image_[length_field + 1] = static_cast<uint8_t>(writer.length() - length_field - 2);
        }
    }
...
public:
    ...
    void ExpandVarints() override
    {
        ExpandVarintsImpl("vv-vvv", false);
    }
...
public:
    ...
    void ExpandVarints() override
    {
        ExpandVarintsImpl("vvvvv-v---", false);
    }
...
public:
    ...
    bool EqualFieldValues(MessageStructuredData& values) const override
    {
        ...
        if (cast.supported_versions.size() != client_setup_.supported_versions.size()) {
...
private:
    ...
    MoqtClientSetup client_setup_ = {
        /*supported_versions=*/std::vector<MoqtVersion>({ static_cast<MoqtVersion>(1), static_cast<MoqtVersion>(2) }),
...
private:
    ...
    MoqtSubscribe subscribe_ = {
        ...
        /*parameters=*/
        MoqtSubscribeParameters { "bar", quic::QuicTimeDelta::FromMilliseconds(10000), quic::QuicTimeDelta::FromMilliseconds(10000), std::nullopt },
    };
};
...
public:
    ...
    void ExpandVarints() override
    {
        ExpandVarintsImpl("vvvv--vvvvv--vv--");
    }
...
private:
    ...
    MoqtSubscribeOk subscribe_ok_ = {
        ...
        /*parameters=*/
        MoqtSubscribeParameters { std::nullopt, quic::QuicTimeDelta::FromMilliseconds(10000), quic::QuicTimeDelta::FromMilliseconds(10000), std::nullopt },
    };
};
...
private:
    ...
    MoqtAnnounce announce_ = {
        ...
        /*parameters=*/
        MoqtSubscribeParameters { "bar", std::nullopt, quic::QuicTimeDelta::FromMilliseconds(10000), std::nullopt },
    };
};
...
static inline std::unique_ptr<TestMessageBase> CreateTestMessage(MoqtMessageType message_type, bool is_webtrans)
{
    switch (message_type) {
...
static inline std::unique_ptr<TestMessageBase> CreateTestDataStream(MoqtDataStreamType type)
{
    switch (type) {
