...
std::optional<ProbeId> MoqtProbeManager::StartProbe(quic::QuicByteCount probe_size, quic::QuicTimeDelta timeout, Callback callback)
{
    if (probe_.has_value()) {
        return std::nullopt;
    }
    ...
    probe_ = PendingProbe { id, clock_->ApproximateNow(), clock_->ApproximateNow() + timeout, probe_size, stream->GetStreamId(), std::move(callback) };
    auto visitor_owned = std::make_unique<ProbeStreamVisitor>(this, stream, id, probe_size);
    ProbeStreamVisitor* visitor = visitor_owned.get();
    stream->SetVisitor(std::move(visitor_owned));
    stream->SetPriority(webtransport::StreamPriority { /*send_group_id=*/0, /*send_order=*/kMoqtProbeStreamSendOrder });
    visitor->OnCanWrite();
    RescheduleAlarm();
    return id;
}
...
void MoqtProbeManager::ProbeStreamVisitor::OnCanWrite()
{
    ...
    if (!header_sent_) {
        absl::Status status = quiche::WriteIntoStream(*stream_, *quiche::SerializeIntoString(quiche::WireVarInt62(MoqtDataStreamType::kPadding)));
...
void MoqtProbeManager::ProbeStreamVisitor::OnStopSendingReceived(webtransport::StreamErrorCode error)
{
    if (!ValidateProbe()) {
        return;
    }
...
{
    quic::QuicTime deadline = probe_.has_value() ? probe_->deadline : quic::QuicTime::Zero();
...
void MoqtProbeManager::ClosePendingProbe(ProbeStatus status)
{
    ...
    std::move(probe->callback)(ProbeResult { probe->id, status, probe->probe_size, now - probe->start });
}
} // namespace moqt
