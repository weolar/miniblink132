...
moqt::PublishedObject CachedObjectToPublishedObject(const CachedObject& object)
{
    ...
    if (object.payload != nullptr && !object.payload->empty()) {
        result.payload = quiche::QuicheMemSlice(object.payload->data(), object.payload->length(), [retained_pointer = object.payload](const char*) {});
    }
    return result;
}
