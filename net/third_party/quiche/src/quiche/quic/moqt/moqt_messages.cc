...
{
    if (integer >= static_cast<uint64_t>(MoqtObjectStatus::kInvalidObjectStatus)) {
...
MoqtFilterType GetFilterType(const MoqtSubscribe& message)
{
    ...
    bool has_start = message.start_group.has_value() && message.start_object.has_value();
    if (message.end_group.has_value()) {
        if (has_start) {
            if (*message.end_group < *message.start_group) {
                return MoqtFilterType::kNone;
            } else if (*message.end_group == *message.start_group && *message.end_object <= *message.start_object) {
...
std::string MoqtForwardingPreferenceToString(MoqtForwardingPreference preference)
{
    ...
    QUIC_BUG(quic_bug_bad_moqt_message_type_01) << "Unknown preference " << std::to_string(static_cast<int>(preference));
...
MoqtForwardingPreference GetForwardingPreference(MoqtDataStreamType type)
{
    ...
    QUIC_BUG(quic_bug_bad_moqt_message_type_02) << "Message type does not indicate forwarding preference";
    return MoqtForwardingPreference::kSubgroup;
};
...
MoqtDataStreamType GetMessageTypeForForwardingPreference(MoqtForwardingPreference preference)
{
    ...
    QUIC_BUG(quic_bug_bad_moqt_message_type_03) << "Forwarding preference does not indicate message type";
    return MoqtDataStreamType::kStreamHeaderSubgroup;
}
