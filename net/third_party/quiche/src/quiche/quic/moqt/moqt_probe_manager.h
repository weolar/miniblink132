...
public:
    ...
    virtual std::optional<ProbeId> StartProbe(quic::QuicByteCount probe_size, quic::QuicTimeDelta timeout, Callback callback) = 0;
...
public:
    explicit MoqtProbeManager(webtransport::Session* session, const quic::QuicClock* clock, quic::QuicAlarmFactory& alarm_factory)
        : session_(session)
        , clock_(clock)
    ...
    std::optional<ProbeId> StartProbe(quic::QuicByteCount probe_size, quic::QuicTimeDelta timeout, Callback callback) override;
...
private:
    ...
    public:
        ProbeStreamVisitor(MoqtProbeManager* manager, webtransport::Stream* stream, ProbeId probe_id, quic::QuicByteCount probe_size)
            : manager_(manager)
            , stream_(stream)
            , probe_id_(probe_id)
    ...
    public:
        explicit AlarmDelegate(MoqtProbeManager* manager)
            : manager_(manager)
        {
        }
