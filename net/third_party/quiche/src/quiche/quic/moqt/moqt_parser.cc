...
bool ParseDeliveryOrder(uint8_t raw_value, std::optional<MoqtDeliveryOrder>& output)
{
    switch (raw_value) {
    case 0x00:
        output = std::nullopt;
...
{
    constexpr std::array kAllowedStreamTypes = { MoqtDataStreamType::kStreamHeaderSubgroup, MoqtDataStreamType::kStreamHeaderTrack,
...
size_t ParseObjectHeader(quic::QuicDataReader& reader, MoqtObject& object, MoqtDataStreamType type)
{
    if (!reader.ReadVarInt62(&object.track_alias)) {
        return 0;
    }
    if (type != MoqtDataStreamType::kStreamHeaderTrack && type != MoqtDataStreamType::kStreamHeaderFetch && !reader.ReadVarInt62(&object.group_id)) {
        return 0;
    }
    ...
    if (type == MoqtDataStreamType::kObjectDatagram && !reader.ReadVarInt62(&object.object_id)) {
        return 0;
    }
    if (type != MoqtDataStreamType::kStreamHeaderFetch && !reader.ReadUInt8(&object.publisher_priority)) {
        return 0;
    }
    uint64_t status = static_cast<uint64_t>(MoqtObjectStatus::kNormal);
    if (type == MoqtDataStreamType::kObjectDatagram
        && (!reader.ReadVarInt62(&object.payload_length) || (object.payload_length == 0 && !reader.ReadVarInt62(&status)))) {
        return 0;
    }
...
size_t ParseObjectSubheader(quic::QuicDataReader& reader, MoqtObject& object, MoqtDataStreamType type)
{
    switch (type) {
    ...
    while (!reader->IsDoneReading()) {
        ...
        if (message_len == 0) {
            if (reader->BytesRemaining() > kMaxMessageHeaderSize) {
                ParseError(MoqtError::kInternalError, "Cannot parse non-OBJECT messages > 2KB");
                return;
            }
            if (fin) {
    ...
    for (uint64_t i = 0; i < num_params; ++i) {
        ...
        case MoqtSetupParameter::kPath:
            if (uses_web_transport_) {
                ParseError("WebTransport connection is using PATH parameter in SETUP");
                return 0;
            }
...
size_t MoqtControlParser::ProcessSubscribe(quic::QuicDataReader& reader)
{
    ...
    if (!reader.ReadVarInt62(&subscribe_request.subscribe_id) || !reader.ReadVarInt62(&subscribe_request.track_alias)
        || !ReadTrackNamespace(reader, subscribe_request.full_track_name) || !reader.ReadStringPieceVarInt62(&track_name)
        || !reader.ReadUInt8(&subscribe_request.subscriber_priority) || !reader.ReadUInt8(&group_order) || !reader.ReadVarInt62(&filter)) {
        return 0;
    }
    ...
    case MoqtFilterType::kAbsoluteRange:
        ...
        if (object == 0) {
            ...
            if (subscribe_request.start_group == subscribe_request.end_group && subscribe_request.end_object < subscribe_request.start_object) {
...
size_t MoqtControlParser::ProcessSubscribeOk(quic::QuicDataReader& reader)
{
    ...
    if (!reader.ReadVarInt62(&subscribe_ok.subscribe_id) || !reader.ReadVarInt62(&milliseconds) || !reader.ReadUInt8(&group_order)
    ...
    if (content_exists) {
        ...
        if (!reader.ReadVarInt62(&subscribe_ok.largest_id->group) || !reader.ReadVarInt62(&subscribe_ok.largest_id->object)) {
            return 0;
        }
    }
...
size_t MoqtControlParser::ProcessSubscribeError(quic::QuicDataReader& reader)
{
    ...
    uint64_t error_code;
    if (!reader.ReadVarInt62(&subscribe_error.subscribe_id) || !reader.ReadVarInt62(&error_code) || !reader.ReadStringVarInt62(subscribe_error.reason_phrase)
...
size_t MoqtControlParser::ProcessSubscribeDone(quic::QuicDataReader& reader)
{
    ...
    uint8_t content_exists;
    uint64_t value;
    if (!reader.ReadVarInt62(&subscribe_done.subscribe_id) || !reader.ReadVarInt62(&value) || !reader.ReadStringVarInt62(subscribe_done.reason_phrase)
        || !reader.ReadUInt8(&content_exists)) {
        return 0;
    }
    ...
    if (content_exists == 1) {
        ...
        if (!reader.ReadVarInt62(&subscribe_done.final_id->group) || !reader.ReadVarInt62(&subscribe_done.final_id->object)) {
            return 0;
        }
    }
...
size_t MoqtControlParser::ProcessSubscribeUpdate(quic::QuicDataReader& reader)
{
    ...
    if (!reader.ReadVarInt62(&subscribe_update.subscribe_id) || !reader.ReadVarInt62(&subscribe_update.start_group)
        || !reader.ReadVarInt62(&subscribe_update.start_object) || !reader.ReadVarInt62(&end_group) || !reader.ReadVarInt62(&end_object)
    ...
    if (end_object > 0) {
        ...
        if (subscribe_update.end_object.has_value() && subscribe_update.start_group == *subscribe_update.end_group
...
size_t MoqtControlParser::ProcessAnnounceCancel(quic::QuicDataReader& reader)
{
    ...
    if (!reader.ReadVarInt62(&announce_cancel.error_code) || !reader.ReadStringVarInt62(announce_cancel.reason_phrase)) {
        return 0;
    }
...
size_t MoqtControlParser::ProcessTrackStatusRequest(quic::QuicDataReader& reader)
{
...
size_t MoqtControlParser::ProcessTrackStatus(quic::QuicDataReader& reader)
{
    ...
    uint64_t value;
    if (!reader.ReadVarInt62(&value) || !reader.ReadVarInt62(&track_status.last_group) || !reader.ReadVarInt62(&track_status.last_object)) {
        return 0;
    }
...
size_t MoqtControlParser::ProcessSubscribeAnnounces(quic::QuicDataReader& reader)
{
...
size_t MoqtControlParser::ProcessSubscribeAnnouncesOk(quic::QuicDataReader& reader)
{
...
size_t MoqtControlParser::ProcessSubscribeAnnouncesError(quic::QuicDataReader& reader)
{
    MoqtSubscribeAnnouncesError subscribe_namespace_error;
    uint64_t error_code;
    if (!ReadTrackNamespace(reader, subscribe_namespace_error.track_namespace) || !reader.ReadVarInt62(&error_code)
        || !reader.ReadStringVarInt62(subscribe_namespace_error.reason_phrase)) {
        return 0;
    }
    subscribe_namespace_error.error_code = static_cast<MoqtAnnounceErrorCode>(error_code);
    visitor_.OnSubscribeAnnouncesErrorMessage(subscribe_namespace_error);
    return reader.PreviouslyReadPayload().length();
}

size_t MoqtControlParser::ProcessUnsubscribeAnnounces(quic::QuicDataReader& reader)
{
...
size_t MoqtControlParser::ProcessFetch(quic::QuicDataReader& reader)
{
    ...
    uint8_t group_order;
    uint64_t end_object;
    if (!reader.ReadVarInt62(&fetch.subscribe_id) || !ReadTrackNamespace(reader, fetch.full_track_name) || !reader.ReadStringPieceVarInt62(&track_name)
        || !reader.ReadUInt8(&fetch.subscriber_priority) || !reader.ReadUInt8(&group_order) || !reader.ReadVarInt62(&fetch.start_object.group)
        || !reader.ReadVarInt62(&fetch.start_object.object) || !reader.ReadVarInt62(&fetch.end_group) || !reader.ReadVarInt62(&end_object)
    ...
    fetch.end_object = end_object == 0 ? std::optional<uint64_t>() : (end_object - 1);
    if (fetch.end_group < fetch.start_object.group
        || (fetch.end_group == fetch.start_object.group && fetch.end_object.has_value() && *fetch.end_object < fetch.start_object.object)) {
...
size_t MoqtControlParser::ProcessFetchOk(quic::QuicDataReader& reader)
{
    ...
    uint8_t group_order;
    if (!reader.ReadVarInt62(&fetch_ok.subscribe_id) || !reader.ReadUInt8(&group_order) || !reader.ReadVarInt62(&fetch_ok.largest_id.group)
        || !reader.ReadVarInt62(&fetch_ok.largest_id.object) || !ReadSubscribeParameters(reader, fetch_ok.parameters)) {
        return 0;
    }
    if (group_order != 0x01 && group_order != 0x02) {
...
size_t MoqtControlParser::ProcessFetchError(quic::QuicDataReader& reader)
{
    ...
    uint64_t error_code;
    if (!reader.ReadVarInt62(&fetch_error.subscribe_id) || !reader.ReadVarInt62(&error_code) || !reader.ReadStringVarInt62(fetch_error.reason_phrase)) {
        return 0;
    }
...
size_t MoqtControlParser::ProcessObjectAck(quic::QuicDataReader& reader)
{
    ...
    uint64_t raw_delta;
    if (!reader.ReadVarInt62(&object_ack.subscribe_id) || !reader.ReadVarInt62(&object_ack.group_id) || !reader.ReadVarInt62(&object_ack.object_id)
        || !reader.ReadVarInt62(&raw_delta)) {
        return 0;
    }
    object_ack.delta_from_deadline = quic::QuicTimeDelta::FromMicroseconds(SignedVarintUnserializedForm(raw_delta));
...
void MoqtControlParser::ParseError(MoqtError error_code, absl::string_view reason)
{
    if (parsing_error_) {
...
bool MoqtControlParser::ReadVarIntPieceVarInt62(quic::QuicDataReader& reader, uint64_t& result)
{
    uint64_t length;
...
bool MoqtControlParser::ReadParameter(quic::QuicDataReader& reader, uint64_t& type, absl::string_view& value)
{
...
bool MoqtControlParser::ReadSubscribeParameters(quic::QuicDataReader& reader, MoqtSubscribeParameters& params)
{
    uint64_t num_params;
    if (!reader.ReadVarInt62(&num_params)) {
        return false;
    ...
    for (uint64_t i = 0; i < num_params; ++i) {
        ...
        case moqt::MoqtTrackRequestParameter::kDeliveryTimeout:
            ...
            params.delivery_timeout = quic::QuicTimeDelta::FromMilliseconds(raw_value);
            break;
        case moqt::MoqtTrackRequestParameter::kMaxCacheDuration:
            ...
            params.max_cache_duration = quic::QuicTimeDelta::FromMilliseconds(raw_value);
            break;
        case MoqtTrackRequestParameter::kOackWindowSize: {
            ...
            params.object_ack_window = quic::QuicTimeDelta::FromMicroseconds(raw_value);
            break;
        }
        default:
            // Skip over the parameter.
...
bool MoqtControlParser::StringViewToVarInt(absl::string_view& sv, uint64_t& vi)
{
    quic::QuicDataReader reader(sv);
    if (static_cast<size_t>(reader.PeekVarInt62Length()) != sv.length()) {
        ParseError(MoqtError::kParameterLengthMismatch, "Parameter length does not match varint encoding");
        return false;
    }
...
bool MoqtControlParser::ReadTrackNamespace(quic::QuicDataReader& reader, FullTrackName& full_track_name)
{
...
absl::string_view ParseDatagram(absl::string_view data, MoqtObject& object_metadata)
{
    uint64_t value;
    quic::QuicDataReader reader(data);
    if (!reader.ReadVarInt62(&value)) {
    ...
    if (static_cast<MoqtDataStreamType>(value) != MoqtDataStreamType::kObjectDatagram) {
        return absl::string_view();
    }
    size_t processed_data = ParseObjectHeader(reader, object_metadata, MoqtDataStreamType::kObjectDatagram);
...
{
    if (processing_) {
        QUICHE_BUG(MoqtDataParser_reentry) << "Calling ProcessData() when ProcessData() is already in progress.";
        return;
    }
    processing_ = true;
    auto on_return = absl::MakeCleanup([&] { processing_ = false; });
    ...
    if (fin) {
        if (!buffered_message_.empty() || !metadata_.has_value() || payload_length_remaining_ > 0) {
...
absl::string_view MoqtDataParser::ProcessDataInner(absl::string_view data)
{
    ...
    while (!reader.IsDoneReading()) {
        ...
        case kData: {
            absl::string_view payload = reader.ReadAtMost(payload_length_remaining_);
