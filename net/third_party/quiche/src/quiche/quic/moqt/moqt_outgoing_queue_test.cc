...
class TestMoqtOutgoingQueue : public MoqtOutgoingQueue, public MoqtObjectListener {
public:
    TestMoqtOutgoingQueue()
        : MoqtOutgoingQueue(FullTrackName { "test", "track" }, MoqtForwardingPreference::kSubgroup)
    {
    ...
    void OnNewObjectAvailable(FullSequence sequence) override
    {
        ...
        ASSERT_THAT(object->status, AnyOf(MoqtObjectStatus::kNormal, MoqtObjectStatus::kEndOfGroup));
        if (object->status == MoqtObjectStatus::kNormal) {
            PublishObject(object->sequence.group, object->sequence.object, object->payload.AsStringView());
        } else {
    ...
    {
        std::vector<FullSequence> objects = GetCachedObjectsInRange(FullSequence(0, 0), GetLargestSequence());
    ...
    MOCK_METHOD(void, PublishObject, (uint64_t group_id, uint64_t object_id, absl::string_view payload), ());
    MOCK_METHOD(void, OnTrackPublisherGone, (), (override));
};

absl::StatusOr<std::vector<std::string>> FetchToVector(std::unique_ptr<MoqtFetchTask> fetch)
{
    ...
    for (;;) {
        ...
        case MoqtFetchTask::kPending:
            return absl::InternalError("Unexpected kPending from MoqtOutgoingQueue");
...
TEST(MoqtOutgoingQueue, FirstObjectNotKeyframe)
{
    ...
    EXPECT_QUICHE_BUG(queue.AddObject(MemSliceFromString("a"), false), "The first object");
}
...
{
    TestMoqtOutgoingQueue queue;
    EXPECT_THAT(FetchToVector(queue.Fetch(FullSequence { 0, 0 }, 2, 0, MoqtDeliveryOrder::kAscending)), StatusIs(absl::StatusCode::kNotFound));
    ...
    EXPECT_THAT(FetchToVector(queue.Fetch(FullSequence { 0, 0 }, 2, 0, MoqtDeliveryOrder::kAscending)), IsOkAndHolds(ElementsAre("a", "b", "c", "d", "e")));
    EXPECT_THAT(FetchToVector(queue.Fetch(FullSequence { 0, 100 }, 0, 1000, MoqtDeliveryOrder::kAscending)), IsOkAndHolds(IsEmpty()));
    EXPECT_THAT(FetchToVector(queue.Fetch(FullSequence { 0, 0 }, 2, 0, MoqtDeliveryOrder::kDescending)), IsOkAndHolds(ElementsAre("e", "c", "d", "a", "b")));
    EXPECT_THAT(FetchToVector(queue.Fetch(FullSequence { 0, 0 }, 1, 0, MoqtDeliveryOrder::kAscending)), IsOkAndHolds(ElementsAre("a", "b", "c")));
    EXPECT_THAT(FetchToVector(queue.Fetch(FullSequence { 0, 0 }, 1, 0, MoqtDeliveryOrder::kAscending)), IsOkAndHolds(ElementsAre("a", "b", "c")));
    EXPECT_THAT(FetchToVector(queue.Fetch(FullSequence { 1, 0 }, 5, std::nullopt, MoqtDeliveryOrder::kAscending)), IsOkAndHolds(ElementsAre("c", "d", "e")));
    EXPECT_THAT(FetchToVector(queue.Fetch(FullSequence { 3, 0 }, 5, std::nullopt, MoqtDeliveryOrder::kAscending)), StatusIs(absl::StatusCode::kNotFound));

    queue.AddObject(MemSliceFromString("f"), true);
    queue.AddObject(MemSliceFromString("g"), false);
    EXPECT_THAT(FetchToVector(queue.Fetch(FullSequence { 0, 0 }, 0, 1, MoqtDeliveryOrder::kAscending)), StatusIs(absl::StatusCode::kNotFound));
    EXPECT_THAT(FetchToVector(queue.Fetch(FullSequence { 0, 0 }, 2, 0, MoqtDeliveryOrder::kAscending)), IsOkAndHolds(ElementsAre("c", "d", "e")));
    EXPECT_THAT(
        FetchToVector(queue.Fetch(FullSequence { 1, 0 }, 5, std::nullopt, MoqtDeliveryOrder::kAscending)), IsOkAndHolds(ElementsAre("c", "d", "e", "f", "g")));
    EXPECT_THAT(FetchToVector(queue.Fetch(FullSequence { 3, 0 }, 5, std::nullopt, MoqtDeliveryOrder::kAscending)), IsOkAndHolds(ElementsAre("f", "g")));
}
...
TEST(MoqtOutgoingQueue, ObjectsGoneWhileFetching)
{
    ...
    EXPECT_THAT(FetchToVector(queue.Fetch(FullSequence { 0, 0 }, 5, 0, MoqtDeliveryOrder::kAscending)), IsOkAndHolds(ElementsAre("c", "d", "e")));
    std::unique_ptr<MoqtFetchTask> deferred_fetch = queue.Fetch(FullSequence { 0, 0 }, 5, 0, MoqtDeliveryOrder::kAscending);
    ...
    EXPECT_THAT(FetchToVector(std::move(deferred_fetch)), IsOkAndHolds(IsEmpty()));
}
