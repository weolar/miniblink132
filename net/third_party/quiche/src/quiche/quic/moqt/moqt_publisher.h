...
public:
    ...
    virtual std::optional<PublishedObject> GetCachedObject(FullSequence sequence) const = 0;
    ...
    virtual std::vector<FullSequence> GetCachedObjectsInRange(FullSequence start, FullSequence end) const = 0;
    ...
    virtual std::unique_ptr<MoqtFetchTask> Fetch(FullSequence start, uint64_t end_group, std::optional<uint64_t> end_object, MoqtDeliveryOrder order) = 0;
};
...
public:
    ...
    virtual absl::StatusOr<std::shared_ptr<MoqtTrackPublisher>> GetTrack(const FullTrackName& track_name) = 0;
};
