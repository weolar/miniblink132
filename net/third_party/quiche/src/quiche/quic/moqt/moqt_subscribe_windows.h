...
public:
    ...
    SubscribeWindow(uint64_t start_group, uint64_t start_object, uint64_t end_group, uint64_t end_object)
...
public:
    ...
    template <typename H> friend H AbslHashValue(H h, const ReducedSequenceIndex& m)
    {
...
public:
    ...
    std::optional<webtransport::StreamId> GetStreamForSequence(FullSequence sequence) const;
...
private:
    absl::flat_hash_map<ReducedSequenceIndex, webtransport::StreamId> send_streams_;
