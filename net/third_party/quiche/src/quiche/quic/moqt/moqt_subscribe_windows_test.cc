...
TEST_F(SubscribeWindowTest, AddQueryRemoveStreamIdTrack)
{
    ...
    EXPECT_QUIC_BUG(stream_map.AddStream(FullSequence { 5, 2 }, 6), "Stream already added");
...
TEST_F(SubscribeWindowTest, AddQueryRemoveStreamIdSubgroup)
{
    ...
    EXPECT_QUIC_BUG(stream_map.AddStream(FullSequence { 5, 3 }, 6), "Stream already added");
...
TEST_F(SubscribeWindowTest, AddQueryRemoveStreamIdDatagram)
{
    ...
    EXPECT_QUIC_BUG(stream_map.AddStream(FullSequence { 4, 0 }, 2), "Adding a stream for datagram");
}

TEST_F(SubscribeWindowTest, UpdateStartEnd)
{
    SubscribeWindow window(start_, end_);
    EXPECT_TRUE(window.UpdateStartEnd(start_.next(), FullSequence(end_.group, end_.object - 1)));
    ...
    EXPECT_FALSE(window.UpdateStartEnd(start_, FullSequence(end_.group, end_.object - 1)));
