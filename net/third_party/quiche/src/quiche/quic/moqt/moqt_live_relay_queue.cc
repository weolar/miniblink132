...
bool MoqtLiveRelayQueue::AddRawObject(FullSequence sequence, MoqtObjectStatus status, MoqtPriority priority, absl::string_view payload)
{
    if (queue_.size() == kMaxQueuedGroups) {
        if (queue_.begin()->first > sequence.group) {
            QUICHE_DLOG(INFO) << "Skipping object from group " << sequence.group << " because it is too old.";
            return true;
        }
    ...
    if (!group_it.second) { // Group already exists.
        ...
        if (status == MoqtObjectStatus::kEndOfGroup && sequence.object < group.next_object) {
            QUICHE_DLOG(INFO) << "Skipping EndOfGroup because it is not the last "
                              << "object in the group.";
            return false;
        }
    }
    auto subgroup_it = group.subgroups.try_emplace(SubgroupPriority { priority, sequence.subgroup });
    ...
    std::shared_ptr<quiche::QuicheMemSlice> slice
        = payload.empty() ? nullptr : std::make_shared<quiche::QuicheMemSlice>(quiche::QuicheBuffer::Copy(quiche::SimpleBufferAllocator::Get(), payload));
    object_queue.emplace(sequence.object, CachedObject { sequence, status, priority, slice });
...
std::optional<PublishedObject> MoqtLiveRelayQueue::GetCachedObject(FullSequence sequence) const
{
    ...
    auto subgroup_it = group.subgroups.find(SubgroupPriority { publisher_priority_, sequence.subgroup });
...
std::vector<FullSequence> MoqtLiveRelayQueue::GetCachedObjectsInRange(FullSequence start, FullSequence end) const
{
    ...
    for (auto& group_it : queue_) {
        ...
        for (auto& subgroup_it : group_it.second.subgroups) {
            for (auto& object_it : subgroup_it.second) {
                ...
                if (group_it.first == end.group && object_it.second.sequence.object >= end.object) {
                    break;
                }
            }
        }
