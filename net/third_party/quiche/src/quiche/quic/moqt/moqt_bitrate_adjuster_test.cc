...
public:
    ...
    QuicBandwidth GetCurrentBitrate() const override
    {
        return bitrate_;
    }
...
constexpr QuicBandwidth kDefaultBitrate = QuicBandwidth::FromBitsPerSecond(2000);
...
protected:
        ...
        , adjuster_(&clock_, &session_, &adjustable_)
    {
        ...
        ON_CALL(session_, GetSessionStats()).WillByDefault([this] { return stats_; });
    }
...
TEST_F(MoqtBitrateAdjusterTest, AdjustDownOnce)
{
    ...
    EXPECT_CALL(adjustable_, OnBitrateAdjusted(_)).WillOnce([](QuicBandwidth new_bitrate) { EXPECT_LT(new_bitrate, kDefaultBitrate); });
...
TEST_F(MoqtBitrateAdjusterTest, AdjustDownTwice)
{
    ...
    EXPECT_CALL(adjustable_, OnBitrateAdjusted(_)).WillRepeatedly([&] { ++adjusted_times; });
