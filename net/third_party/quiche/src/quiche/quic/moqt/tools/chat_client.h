...
constexpr quic::QuicTime::Delta kChatEventLoopDuration = quic::QuicTime::Delta::FromMilliseconds(500);
...
public:
    ...
    virtual void Initialize(quiche::MultiUseCallback<void(absl::string_view)> callback, quic::QuicEventLoop* event_loop) = 0;
    // Write a peer message to the user output.
    virtual void WriteToOutput(absl::string_view user, absl::string_view message) = 0;
...
public:
    ...
    // an event loop.
    ChatClient(
        const quic::QuicServerId& server_id, bool ignore_certificate, std::unique_ptr<ChatUserInterface> interface, quic::QuicEventLoop* event_loop = nullptr);
    ~ChatClient()
    {
        if (session_ != nullptr) {
            session_->Close();
    ...
    bool Connect(absl::string_view path, absl::string_view username, absl::string_view chat_id);
    ...
    public:
        RemoteTrackVisitor(ChatClient* client)
            : client_(client)
        {
        }

        void OnReply(const moqt::FullTrackName& full_track_name, std::optional<absl::string_view> reason_phrase) override;

        void OnCanAckObjects(MoqtObjectAckFunction) override
        {
        }

        void OnObjectFragment(const moqt::FullTrackName& full_track_name, FullSequence sequence, moqt::MoqtPriority publisher_priority,
            moqt::MoqtObjectStatus status, moqt::MoqtForwardingPreference forwarding_preference, absl::string_view object, bool end_of_message) override;
    ...
    {
        return !catalog_group_.has_value() || subscribes_to_make_ > 0 || (queue_ == nullptr || !queue_->HasSubscribers());
    }

private:
    void RunEventLoop()
    {
        event_loop_->RunEventLoopOnce(kChatEventLoopDuration);
    }

    // Objects from the same catalog group arrive on the same stream, and in
    // object sequence order.
    void ProcessCatalog(absl::string_view object, moqt::RemoteTrack::Visitor* visitor, uint64_t group_sequence, uint64_t object_sequence);
