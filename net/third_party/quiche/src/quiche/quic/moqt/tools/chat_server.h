...
class ChatServer {
public:
    ChatServer(std::unique_ptr<quic::ProofSource> proof_source, absl::string_view chat_id, absl::string_view output_file);
    ~ChatServer();
    ...
    public:
        ...
        void OnObjectFragment(const moqt::FullTrackName& full_track_name, FullSequence sequence, moqt::MoqtPriority /*publisher_priority*/,
            moqt::MoqtObjectStatus /*status*/, moqt::MoqtForwardingPreference /*forwarding_preference*/, absl::string_view object,
    ...
    public:
        ...
        void set_iterator(const std::list<ChatServerSessionHandler>::const_iterator it)
        {
            it_ = it;
        }
    ...
    MoqtServer& moqt_server()
    {
        return server_;
    }

    RemoteTrackVisitor* remote_track_visitor()
    {
        return &remote_track_visitor_;
    }

    MoqtOutgoingQueue* catalog()
    {
        return catalog_.get();
    }
    ...
    int num_users() const
    {
        return user_queues_.size();
    }

private:
    absl::StatusOr<MoqtConfigureSessionCallback> IncomingSessionHandler(absl::string_view path);

    MoqtIncomingSessionCallback incoming_session_callback_ = [&](absl::string_view path) { return IncomingSessionHandler(path); };
    ...
    // indexed by username
    absl::flat_hash_map<std::string, std::shared_ptr<MoqtLiveRelayQueue>> user_queues_;
