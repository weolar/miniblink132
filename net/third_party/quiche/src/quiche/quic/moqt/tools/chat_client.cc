...
ChatClient::ChatClient(
    const quic::QuicServerId& server_id, bool ignore_certificate, std::unique_ptr<ChatUserInterface> interface, quic::QuicEventLoop* event_loop)
    ...
    , interface_(std::move(interface))
{
    ...
    quic::QuicSocketAddress peer_address = quic::tools::LookupAddress(AF_UNSPEC, server_id);
    ...
    client_ = std::make_unique<MoqtClient>(peer_address, server_id, std::move(verifier), event_loop_);
    session_callbacks_.session_established_callback = [this]() {
        std::cout << "Session established\n";
        session_is_open_ = true;
    };
    session_callbacks_.session_terminated_callback = [this](absl::string_view error_message) {
        std::cerr << "Closed session, reason = " << error_message << "\n";
        session_is_open_ = false;
        connect_failed_ = true;
    };
    session_callbacks_.session_deleted_callback = [this]() { session_ = nullptr; };
    interface_->Initialize([this](absl::string_view input_message) { OnTerminalLineInput(input_message); }, event_loop_);
}

bool ChatClient::Connect(absl::string_view path, absl::string_view username, absl::string_view chat_id)
{
    username_ = username;
...
void ChatClient::OnTerminalLineInput(absl::string_view input_message)
{
    ...
    quiche::QuicheMemSlice message_slice(quiche::QuicheBuffer::Copy(quiche::SimpleBufferAllocator::Get(), input_message));
    queue_->AddObject(std::move(message_slice), /*key=*/true);
}

void ChatClient::RemoteTrackVisitor::OnReply(const FullTrackName& full_track_name, std::optional<absl::string_view> reason_phrase)
{
...
void ChatClient::RemoteTrackVisitor::OnObjectFragment(const FullTrackName& full_track_name, FullSequence sequence, MoqtPriority /*publisher_priority*/,
    MoqtObjectStatus /*status*/, MoqtForwardingPreference /*forwarding_preference*/, absl::string_view object, bool end_of_message)
{
    if (!end_of_message) {
    ...
    std::string username(client_->chat_strings_->GetUsernameFromFullTrackName(full_track_name));
...
bool ChatClient::AnnounceAndSubscribe()
{
    ...
    if (!username_.empty()) {
        ...
        FullTrackName my_track_name = chat_strings_->GetFullTrackNameFromUsername(username_);
        queue_ = std::make_shared<MoqtOutgoingQueue>(my_track_name, MoqtForwardingPreference::kSubgroup);
        publisher_.Add(queue_);
        session_->set_publisher(&publisher_);
        MoqtOutgoingAnnounceCallback announce_callback = [this](FullTrackName track_namespace, std::optional<MoqtAnnounceErrorReason> reason) {
            if (reason.has_value()) {
    ...
    if (!session_->SubscribeCurrentGroup(
            catalog_name, remote_track_visitor_.get(), MoqtSubscribeParameters { username_, std::nullopt, std::nullopt, std::nullopt })) {
...
void ChatClient::ProcessCatalog(absl::string_view object, RemoteTrack::Visitor* visitor, uint64_t group_sequence, uint64_t object_sequence)
{
    ...
    std::vector<absl::string_view> lines = absl::StrSplit(object, '\n', absl::SkipEmpty());
    for (absl::string_view line : lines) {
        if (!got_version) {
            if (line != "version=1") {
                session_->Error(MoqtError::kProtocolViolation, "Catalog does not begin with version");
                return;
            }
            got_version = true;
            continue;
        ...
        if (it == other_users_.end()) {
            FullTrackName to_subscribe = chat_strings_->GetFullTrackNameFromUsername(user);
            auto new_user = other_users_.emplace(std::make_pair(user, ChatUser(to_subscribe, group_sequence)));
            ...
            if (it->second.from_group == group_sequence) {
                session_->Error(MoqtError::kProtocolViolation, "User listed twice in Catalog");
                return;
            }
    ...
    if (object_sequence == 0) { // Eliminate users that are no longer present
        absl::erase_if(other_users_, [&](const auto& kv) { return kv.second.from_group != group_sequence; });
    }
