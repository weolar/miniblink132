...
public:
    explicit MoqChatStrings(absl::string_view chat_id)
        : chat_id_(chat_id)
    {
    }
    ...
    std::string GetUsernameFromFullTrackName(FullTrackName full_track_name) const
    {
        ...
        std::vector<absl::string_view> elements = absl::StrSplit(full_track_name.tuple()[0], '/');
        if (elements.size() != 4 || elements[0] != kBasePath || elements[1] != chat_id_ || elements[2] != kParticipantPath) {
            return "";
        }
    ...
    {
        return FullTrackName { absl::StrCat(kBasePath, "/", chat_id_, "/", kParticipantPath, "/", username), "" };
    }

    FullTrackName GetCatalogName() const
    {
        return FullTrackName { absl::StrCat(kBasePath, "/", chat_id_), absl::StrCat("/", kCatalogPath) };
    }
