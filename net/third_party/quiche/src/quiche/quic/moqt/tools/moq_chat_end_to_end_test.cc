...
public:
    void Initialize(quiche::MultiUseCallback<void(absl::string_view)> callback, quic::QuicEventLoop* event_loop) override
    {
    ...
    MOCK_METHOD(void, WriteToOutput, (absl::string_view user, absl::string_view message), (override));

    void SendMessage(absl::string_view message)
    {
        callback_(message);
    }
...
public:
    MoqChatEndToEndTest()
        : server_(quic::test::crypto_test_utils::ProofSourceForTesting(), "test_chat", "")
    {
        quiche::QuicheIpAddress bind_address;
        std::string hostname(kChatHostname);
        bind_address.FromString(hostname);
        EXPECT_TRUE(server_.moqt_server().quic_server().CreateUDPSocketAndListen(quic::QuicSocketAddress(bind_address, 0)));
        ...
        client1_ = std::make_unique<ChatClient>(quic::QuicServerId(hostname, port), true, std::move(if1ptr), server_.moqt_server().quic_server().event_loop());
        client2_ = std::make_unique<ChatClient>(quic::QuicServerId(hostname, port), true, std::move(if2ptr), server_.moqt_server().quic_server().event_loop());
    }

    void SendAndWaitForOutput(MockChatUserInterface* sender, MockChatUserInterface* receiver, absl::string_view sender_name, absl::string_view message)
    {
        bool message_to_output = false;
        EXPECT_CALL(*receiver, WriteToOutput(sender_name, message)).WillOnce([&] { message_to_output = true; });
...
TEST_F(MoqChatEndToEndTest, LeaveAndRejoin)
{
    ...
    client1_ = std::make_unique<ChatClient>(
        quic::QuicServerId(std::string(kChatHostname), port), true, std::move(if1bptr), server_.moqt_server().quic_server().event_loop());
    EXPECT_TRUE(client1_->Connect("/moq-chat", "client1", "test_chat"));
    EXPECT_TRUE(client1_->AnnounceAndSubscribe());
