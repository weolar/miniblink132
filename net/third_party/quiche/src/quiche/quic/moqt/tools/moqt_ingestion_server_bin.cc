...
DEFINE_QUICHE_COMMAND_LINE_FLAG(bool, allow_invalid_track_namespaces, false, "If true, invalid track namespaces will be escaped rather than rejected.");
DEFINE_QUICHE_COMMAND_LINE_FLAG(std::string, tracks, "video,audio", "List of track names to request from the peer.");
...
public:
    explicit MoqtIngestionHandler(MoqtSession* session, absl::string_view output_root)
        : session_(session)
        , output_root_(output_root)
    {
        session_->callbacks().incoming_announce_callback = absl::bind_front(&MoqtIngestionHandler::OnAnnounceReceived, this);
    }

    std::optional<MoqtAnnounceErrorReason> OnAnnounceReceived(FullTrackName track_namespace)
    {
        if (!IsValidTrackNamespace(track_namespace) && !quiche::GetQuicheCommandLineFlag(FLAGS_allow_invalid_track_namespaces)) {
            QUICHE_DLOG(WARNING) << "Rejected remote announce as it contained "
                                    "disallowed characters; namespace: "
                                 << track_namespace;
            return MoqtAnnounceErrorReason { MoqtAnnounceErrorCode::kInternalError, "Track namespace contains disallowed characters" };
        }

        std::string directory_name
            = absl::StrCat(CleanUpTrackNamespace(track_namespace), "_", absl::FormatTime("%Y%m%d_%H%M%S", absl::Now(), absl::UTCTimeZone()));
        std::string directory_path = quiche::JoinPath(output_root_, directory_name);
        auto [it, added] = subscribed_namespaces_.emplace(track_namespace, NamespaceHandler(directory_path));
        if (!added) {
        ...
        if (absl::Status status = MakeDirectory(directory_path); !status.ok()) {
            ...
            QUICHE_LOG(ERROR) << "Failed to create directory " << directory_path << "; " << status;
            return MoqtAnnounceErrorReason { MoqtAnnounceErrorCode::kInternalError, "Failed to create output directory" };
        }

        std::string track_list = quiche::GetQuicheCommandLineFlag(FLAGS_tracks);
        std::vector<absl::string_view> tracks_to_subscribe = absl::StrSplit(track_list, ',', absl::AllowEmpty());
    ...
    public:
        ...
        void OnReply(const FullTrackName& full_track_name, std::optional<absl::string_view> error_reason_phrase) override
        {
            if (error_reason_phrase.has_value()) {
                QUICHE_LOG(ERROR) << "Failed to subscribe to the peer track " << full_track_name << ": " << *error_reason_phrase;
            }
        }
        ...
        {
            std::string file_name = absl::StrCat(sequence.group, "-", sequence.object, ".", full_track_name.tuple().back());
...
absl::StatusOr<MoqtConfigureSessionCallback> IncomingSessionHandler(std::string output_root, absl::string_view path)
{
    if (path != "/ingest") {
    ...
    return [output_root](MoqtSession* session) {
        ...
        session->callbacks().session_deleted_callback = [handler = std::move(handler)] {};
    };
}
...
DEFINE_QUICHE_COMMAND_LINE_FLAG(std::string, bind_address, "127.0.0.1", "Local IP address to bind to");
DEFINE_QUICHE_COMMAND_LINE_FLAG(uint16_t, port, 8000, "Port for the server to listen on");
...
int main(int argc, char** argv)
{
    ...
    if (absl::Status stat_status = IsDirectory(output_directory); !stat_status.ok()) {
        if (absl::IsNotFound(stat_status)) {
            absl::Status mkdir_status = MakeDirectory(output_directory);
            if (!mkdir_status.ok()) {
                QUICHE_LOG(ERROR) << "Failed to create output directory: " << mkdir_status;
                return 1;
            }
        } else {
    ...
    moqt::MoqtServer server(quiche::CreateDefaultProofSource(), absl::bind_front(moqt::IncomingSessionHandler, output_directory));
    quiche::QuicheIpAddress bind_address;
    QUICHE_CHECK(bind_address.FromString(quiche::GetQuicheCommandLineFlag(FLAGS_bind_address)));
    server.quic_server().CreateUDPSocketAndListen(quic::QuicSocketAddress(bind_address, quiche::GetQuicheCommandLineFlag(FLAGS_port)));
