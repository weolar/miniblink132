...
DEFINE_QUICHE_COMMAND_LINE_FLAG(std::string, output_file, "", "chat messages will stream to a file instead of stdout");
...
public:
    ...
    void Initialize(quic::InteractiveCli::LineCallback callback, quic::QuicEventLoop* event_loop) override
    {
        callback_ = std::move(callback);
        event_loop_ = event_loop;
    }

    void WriteToOutput(absl::string_view user, absl::string_view message) override
    {
        if (message.empty()) {
            return;
        }
    ...
    void IoLoop() override
    {
        ...
        QUIC_BUG_IF(quic_bug_moq_chat_user_interface_unitialized, event_loop_ == nullptr) << "IoLoop called before Initialize";
...
public:
    void Initialize(quic::InteractiveCli::LineCallback callback, quic::QuicEventLoop* event_loop) override
    {
        cli_ = std::make_unique<quic::InteractiveCli>(event_loop, std::move(callback));
        event_loop_ = event_loop;
    ...
    void WriteToOutput(absl::string_view user, absl::string_view message) override
    {
        QUIC_BUG_IF(quic_bug_moq_chat_user_interface_unitialized, cli_ == nullptr) << "WriteToOutput called before Initialize";
...
int main(int argc, char* argv[])
{
    ...
    std::string output_filename = quiche::GetQuicheCommandLineFlag(FLAGS_output_file);
    ...
    moqt::ChatClient client(server_id, quiche::GetQuicheCommandLineFlag(FLAGS_disable_certificate_verification), std::move(interface));
