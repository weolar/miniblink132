...
public:
    MoqtEndToEndTest()
        : server_(quic::test::crypto_test_utils::ProofSourceForTesting(), absl::bind_front(&MoqtEndToEndTest::ServerBackend, this))
    {
        quic::QuicIpAddress host = quic::TestLoopback();
        bool success = server_.quic_server().CreateUDPSocketAndListen(quic::QuicSocketAddress(host, /*port=*/0));
        QUICHE_CHECK(success);
        server_address_ = quic::QuicSocketAddress(host, server_.quic_server().port());
        event_loop_ = server_.quic_server().event_loop();
    }

    absl::StatusOr<MoqtConfigureSessionCallback> ServerBackend(absl::string_view path)
    {
        ...
        return [](MoqtSession* session) {
            session->callbacks().session_established_callback = []() { QUICHE_LOG(INFO) << "Server: session established"; };
            session->callbacks().session_terminated_callback
                = [](absl::string_view reason) { QUICHE_LOG(INFO) << "Server: session terminated with reason: " << reason; };
        };
    }
    ...
    std::unique_ptr<MoqtClient> CreateClient()
    {
        return std::make_unique<MoqtClient>(
            server_address_, quic::QuicServerId("test.example.com", 443), quic::test::crypto_test_utils::ProofVerifierForTesting(), event_loop_);
    }
...
TEST_F(MoqtEndToEndTest, HandshakeFailed404)
{
    ...
    callbacks.session_terminated_callback = [&](absl::string_view error) { resolved = true; };
    std::unique_ptr<MoqtClient> client = CreateClient();
