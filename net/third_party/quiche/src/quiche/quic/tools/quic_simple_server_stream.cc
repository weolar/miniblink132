...
QuicSimpleServerStream::QuicSimpleServerStream(QuicStreamId id, QuicSpdySession* session, StreamType type, QuicSimpleServerBackend* quic_simple_server_backend)
...
QuicSimpleServerStream::QuicSimpleServerStream(PendingStream* pending, QuicSpdySession* session, QuicSimpleServerBackend* quic_simple_server_backend)
...
void QuicSimpleServerStream::OnInitialHeadersComplete(bool fin, size_t frame_len, const QuicHeaderList& header_list)
{
    QuicSpdyStream::OnInitialHeadersComplete(fin, frame_len, header_list);
    // QuicSpdyStream::OnInitialHeadersComplete() may have already sent error
    // response.
    if (!response_sent_ && !SpdyUtils::CopyAndValidateHeaders(header_list, &content_length_, &request_headers_)) {
    ...
    if (!fin && !response_sent_ && IsConnectRequest()) {
        ...
        if (web_transport() != nullptr) {
            QuicSimpleServerBackend::WebTransportResponse response = quic_simple_server_backend_->ProcessWebTransportRequest(request_headers_, web_transport());
...
void QuicSimpleServerStream::OnBodyAvailable()
{
    while (HasBytesToRead()) {
        ...
        QUIC_DVLOG(1) << "Stream " << id() << " processed " << iov.iov_len << " bytes.";
        body_.append(static_cast<char*>(iov.iov_base), iov.iov_len);

        if (content_length_ >= 0 && body_.size() > static_cast<uint64_t>(content_length_)) {
            QUIC_DVLOG(1) << "Body size (" << body_.size() << ") > content length (" << content_length_ << ").";
            SendErrorResponse();
            return;
        }
...
void QuicSimpleServerStream::HandleRequestConnectData(bool fin_received)
{
    ...
    if (quic_simple_server_backend_ == nullptr) {
        ...
        ResetWriteSide(QuicResetStreamError::FromInternal(QUIC_STREAM_CONNECT_ERROR));
        return;
    }
    ...
    quic_simple_server_backend_->HandleConnectData(data,
        /*data_complete=*/fin_received, this);
}
...
void QuicSimpleServerStream::SendResponse()
{
    ...
    if (content_length_ > 0 && static_cast<uint64_t>(content_length_) != body_.size()) {
        QUIC_DVLOG(1) << "Content length (" << content_length_ << ") != body size (" << body_.size() << ").";
        SendErrorResponse();
        return;
    }
    ...
    // response is ready
    quic_simple_server_backend_->FetchResponseFromBackend(request_headers_, body_, this);
}
...
public:
    DelayedResponseAlarm(QuicSimpleServerStream* stream, const QuicBackendResponse* response)
...
void QuicSimpleServerStream::OnResponseBackendComplete(const QuicBackendResponse* response)
{
    if (response == nullptr) {
    ...
    delayed_response_alarm_.reset(connection->alarm_factory()->CreateAlarm(new DelayedResponseAlarm(this, response)));
...
void QuicSimpleServerStream::Respond(const QuicBackendResponse* response)
{
    ...
    for (const auto& headers : response->early_hints()) {
        QUIC_DVLOG(1) << "Stream " << id() << " sending an Early Hints response: " << headers.DebugString();
    ...
    std::string request_url = request_headers_[":authority"].as_string() + request_headers_[":path"].as_string();
    int response_code;
    ...
    if (!ParseHeaderStatusCode(response_headers, &response_code)) {
        ...
        if (status == response_headers.end()) {
            QUIC_LOG(WARNING) << ":status not present in response from cache for request " << request_url;
        } else {
            QUIC_LOG(WARNING) << "Illegal (non-integer) response :status from cache: " << status->second << " for request " << request_url;
        }
        SendErrorResponse();
        return;
    }
    ...
    if (response->response_type() == QuicBackendResponse::INCOMPLETE_RESPONSE) {
        QUIC_DVLOG(1) << "Stream " << id() << " sending an incomplete response, i.e. no trailer, no fin.";
...
void QuicSimpleServerStream::SendStreamData(absl::string_view data, bool close_stream)
{
...
void QuicSimpleServerStream::TerminateStreamWithError(QuicResetStreamError error)
{
    QUIC_DVLOG(1) << "Stream " << id() << " abruptly terminating with error " << error.internal_code();
...
void QuicSimpleServerStream::SendIncompleteResponse(std::optional<HttpHeaderBlock> response_headers, absl::string_view body)
{
    ...
    if (response_headers.has_value()) {
        QUIC_DLOG(INFO) << "Stream " << id() << " writing headers (fin = false) : " << response_headers.value().DebugString();
        // Do not mark response sent for early 100 continue response.
        int response_code;
        if (!ParseHeaderStatusCode(*response_headers, &response_code) || response_code != 100) {
            response_sent_ = true;
        }
        WriteHeaders(std::move(response_headers).value(), /*fin=*/false, nullptr);
    }

    QUIC_DLOG(INFO) << "Stream " << id() << " writing body (fin = false) with size: " << body.size();
    if (!body.empty()) {
        WriteOrBufferBody(body, /*fin=*/false);
    }
}

void QuicSimpleServerStream::SendHeadersAndBody(HttpHeaderBlock response_headers, absl::string_view body)
{
    SendHeadersAndBodyAndTrailers(std::move(response_headers), body, HttpHeaderBlock());
}

void QuicSimpleServerStream::SendHeadersAndBodyAndTrailers(
    std::optional<HttpHeaderBlock> response_headers, absl::string_view body, HttpHeaderBlock response_trailers)
{
    // Headers should be sent iff not sent in a previous response.
    QUICHE_DCHECK_NE(response_headers.has_value(), response_sent_);
    ...
    if (response_headers.has_value()) {
        ...
        QUIC_DLOG(INFO) << "Stream " << id() << " writing headers (fin = " << send_fin << ") : " << response_headers.value().DebugString();
    ...
    QUIC_DLOG(INFO) << "Stream " << id() << " writing body (fin = " << send_fin << ") with size: " << body.size();
    ...
    QUIC_DLOG(INFO) << "Stream " << id() << " writing trailers (fin = true): " << response_trailers.DebugString();
