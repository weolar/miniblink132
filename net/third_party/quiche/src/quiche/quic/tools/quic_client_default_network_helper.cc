...
std::unique_ptr<QuicPacketWriter> CreateDefaultWriterForEventLoop(SocketFd fd, QuicEventLoop* event_loop)
{
...
QuicClientDefaultNetworkHelper::QuicClientDefaultNetworkHelper(QuicEventLoop* event_loop, QuicClientBase* client)
...
QuicClientDefaultNetworkHelper::~QuicClientDefaultNetworkHelper()
{
    if (client_->connected()) {
        client_->session()->connection()->CloseConnection(
            QUIC_PEER_GOING_AWAY, "Client being torn down", ConnectionCloseBehavior::SEND_CONNECTION_CLOSE_PACKET);
    }

    CleanUpAllUDPSockets();
}

bool QuicClientDefaultNetworkHelper::CreateUDPSocketAndBind(QuicSocketAddress server_address, QuicIpAddress bind_to_address, int bind_to_port)
{
    ...
    if (rc < 0) {
        QUIC_LOG(ERROR) << "Bind failed: " << strerror(errno) << " bind_to_address:" << bind_to_address << ", bind_to_port:" << bind_to_port
    ...
    if (event_loop_->RegisterSocket(fd, kSocketEventReadable | kSocketEventWritable, this)) {
...
void QuicClientDefaultNetworkHelper::OnSocketEvent(QuicEventLoop* /*event_loop*/, QuicUdpSocketFd fd, QuicSocketEventMask events)
{
    if (events & kSocketEventReadable) {
        ...
        while (client_->connected() && more_to_read && times_to_read > 0) {
            more_to_read = packet_reader_->ReadAndDispatchPackets(
                fd, GetLatestClientAddress().port(), *client_->helper()->GetClock(), this, overflow_supported_ ? &packets_dropped : nullptr);
            --times_to_read;
        }
        if (packets_dropped_ < packets_dropped) {
            QUIC_LOG(ERROR) << packets_dropped - packets_dropped_ << " more packets are dropped in the socket receive buffer.";
            packets_dropped_ = packets_dropped;
        }
        if (client_->connected() && more_to_read) {
            bool success = event_loop_->ArtificiallyNotifyEvent(fd, kSocketEventReadable);
...
{
    fd_address_map_.back().second = QuicSocketAddress(GetLatestClientAddress().host(), port);
}

QuicSocketAddress QuicClientDefaultNetworkHelper::GetLatestClientAddress() const
{
...
SocketFd QuicClientDefaultNetworkHelper::CreateUDPSocket(QuicSocketAddress server_address, bool* overflow_supported)
{
...
bool QuicClientDefaultNetworkHelper::BindInterfaceNameIfNeeded(SocketFd fd)
{
    ...
    if (!interface_name.empty()) {
        if (!api.BindInterface(fd, interface_name)) {
            QUIC_DLOG(WARNING) << "Failed to bind socket (" << fd << ") to interface (" << interface_name << ").";
            return false;
        }
    }
    return true;
