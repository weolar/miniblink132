...
// response.
class QuicSimpleServerStream : public QuicSpdyServerStreamBase, public QuicSimpleServerBackend::RequestHandler {
public:
    QuicSimpleServerStream(QuicStreamId id, QuicSpdySession* session, StreamType type, QuicSimpleServerBackend* quic_simple_server_backend);
    QuicSimpleServerStream(PendingStream* pending, QuicSpdySession* session, QuicSimpleServerBackend* quic_simple_server_backend);
    ...
    // QuicSpdyStream
    void OnInitialHeadersComplete(bool fin, size_t frame_len, const QuicHeaderList& header_list) override;
...
protected:
    ...
    // fin.
    void SendIncompleteResponse(std::optional<quiche::HttpHeaderBlock> response_headers, absl::string_view body);

    void SendHeadersAndBody(quiche::HttpHeaderBlock response_headers, absl::string_view body);
    void SendHeadersAndBodyAndTrailers(
        std::optional<quiche::HttpHeaderBlock> response_headers, absl::string_view body, quiche::HttpHeaderBlock response_trailers);
    ...
    void set_quic_simple_server_backend_for_test(QuicSimpleServerBackend* backend)
    {
