...
QuicSpdyClientBase::QuicSpdyClientBase(const QuicServerId& server_id, const ParsedQuicVersionVector& supported_versions, const QuicConfig& config,
    ...
    : QuicClientBase(
        server_id, supported_versions, config, helper, alarm_factory, std::move(network_helper), std::move(proof_verifier), std::move(session_cache))
...
void QuicSpdyClientBase::InitializeSession()
{
    if (max_inbound_header_list_size_ > 0) {
        client_session()->set_max_inbound_header_list_size(max_inbound_header_list_size_);
    }
...
void QuicSpdyClientBase::OnClose(QuicSpdyStream* stream)
{
    ...
    if (response_listener_ != nullptr) {
        response_listener_->OnCompleteResponse(stream->id(), response_headers, client_stream->data());
    }
    ...
    if (store_response_) {
        ...
        for (const HttpHeaderBlock& headers : client_stream->preliminary_headers()) {
...
std::unique_ptr<QuicSession> QuicSpdyClientBase::CreateQuicClientSession(const quic::ParsedQuicVersionVector& supported_versions, QuicConnection* connection)
{
    return std::make_unique<QuicSpdyClientSession>(*config(), supported_versions, connection, server_id(), crypto_config());
}

void QuicSpdyClientBase::SendRequest(const HttpHeaderBlock& headers, absl::string_view body, bool fin)
{
...
void QuicSpdyClientBase::SendRequestInternal(HttpHeaderBlock sanitized_headers, absl::string_view body, bool fin)
{
...
void QuicSpdyClientBase::SendRequestAndWaitForResponse(const HttpHeaderBlock& headers, absl::string_view body, bool fin)
{
...
void QuicSpdyClientBase::SendRequestsAndWaitForResponse(const std::vector<std::string>& url_list)
{
...
QuicSpdyClientStream* QuicSpdyClientBase::CreateClientStream()
{
    ...
    auto* stream = static_cast<QuicSpdyClientStream*>(client_session()->CreateOutgoingBidirectionalStream());
    if (stream) {
...
{
    return client_session() ? client_session()->last_received_http3_goaway_id() : std::nullopt;
}
...
const HttpHeaderBlock& QuicSpdyClientBase::latest_response_header_block() const
{
