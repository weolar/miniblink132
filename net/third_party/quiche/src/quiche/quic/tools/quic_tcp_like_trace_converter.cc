...
QuicTcpLikeTraceConverter::StreamOffsetSegment::StreamOffsetSegment(QuicStreamOffset stream_offset, uint64_t connection_offset, QuicByteCount data_length)
...
QuicIntervalSet<uint64_t> QuicTcpLikeTraceConverter::OnCryptoFrameSent(EncryptionLevel level, QuicStreamOffset offset, QuicByteCount data_length)
{
    if (level >= NUM_ENCRYPTION_LEVELS) {
        QUIC_BUG(quic_bug_10907_1) << "Invalid encryption level";
        return {};
    }
    return OnFrameSent(offset, data_length, /*fin=*/false, &crypto_frames_info_[level]);
}

QuicIntervalSet<uint64_t> QuicTcpLikeTraceConverter::OnStreamFrameSent(QuicStreamId stream_id, QuicStreamOffset offset, QuicByteCount data_length, bool fin)
{
    return OnFrameSent(offset, data_length, fin, &streams_info_.emplace(stream_id, StreamInfo()).first->second);
}

QuicIntervalSet<uint64_t> QuicTcpLikeTraceConverter::OnFrameSent(QuicStreamOffset offset, QuicByteCount data_length, bool fin, StreamInfo* info)
{
    ...
    for (const auto& segment : info->segments) {
        ...
        const uint64_t connection_offset = segment.connection_offset + retransmission.min() - segment.stream_data.min();
        connection_offsets.Add(connection_offset, connection_offset + retransmission.Length());
    }
    ...
    QuicStreamOffset least_unsent_offset = info->segments.empty() ? 0 : info->segments.back().stream_data.max();
    ...
    connection_offsets.Add(connection_offset_, connection_offset_ + new_data_length);
    if (!info->segments.empty() && new_data_offset == least_unsent_offset
        && connection_offset_ == info->segments.back().connection_offset + info->segments.back().stream_data.Length()) {
        ...
        info->segments.emplace_back(new_data_offset, connection_offset_, new_data_length);
    }
    info->fin = fin;
...
QuicInterval<uint64_t> QuicTcpLikeTraceConverter::OnControlFrameSent(QuicControlFrameId control_frame_id, QuicByteCount control_frame_length)
{
    if (control_frame_id > largest_observed_control_frame_id_) {
        // New control frame.
        QuicInterval<uint64_t> connection_offset = QuicInterval<uint64_t>(connection_offset_, connection_offset_ + control_frame_length);
