...
public:
    explicit QuicLevelTriggeredPacketWriter(SocketFd fd, QuicEventLoop* event_loop)
    ...
    {
        WriteResult result = QuicDefaultPacketWriter::WritePacket(buffer, buf_len, self_address, peer_address, options, params);
...
class QuicClientDefaultNetworkHelper : public QuicClientBase::NetworkHelper, public QuicSocketEventListener, public ProcessPacketInterface {
public:
    QuicClientDefaultNetworkHelper(QuicEventLoop* event_loop, QuicClientBase* client);
    QuicClientDefaultNetworkHelper(const QuicClientDefaultNetworkHelper&) = delete;
    QuicClientDefaultNetworkHelper& operator=(const QuicClientDefaultNetworkHelper&) = delete;
    ...
    bool CreateUDPSocketAndBind(QuicSocketAddress server_address, QuicIpAddress bind_to_address, int bind_to_port) override;
    ...
    const quiche::QuicheLinkedHashMap<SocketFd, QuicSocketAddress>& fd_address_map() const
    {
    ...
    virtual SocketFd CreateUDPSocket(QuicSocketAddress server_address, bool* overflow_supported);
