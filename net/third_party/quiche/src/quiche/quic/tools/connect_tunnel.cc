...
std::optional<QuicServerId> ValidateHeadersAndGetAuthority(const quiche::HttpHeaderBlock& request_headers)
{
    ...
    if (scheme_it != request_headers.end()) {
        QUICHE_DVLOG(1) << "CONNECT request contains unexpected scheme: " << scheme_it->second;
        return std::nullopt;
    }

    auto path_it = request_headers.find(":path");
    if (path_it != request_headers.end()) {
        QUICHE_DVLOG(1) << "CONNECT request contains unexpected path: " << path_it->second;
        return std::nullopt;
    }
    ...
    std::optional<QuicServerId> server_id = QuicServerId::ParseFromHostPortString(authority_it->second);
    if (!server_id.has_value()) {
        QUICHE_DVLOG(1) << "CONNECT request authority is malformed: " << authority_it->second;
        return std::nullopt;
    }
...
bool ValidateAuthority(const QuicServerId& authority, const absl::flat_hash_set<QuicServerId>& acceptable_destinations)
{
    if (acceptable_destinations.contains(authority)) {
        return true;
    }

    QUICHE_DVLOG(1) << "CONNECT request authority: " << authority.ToHostPortString() << " is not an acceptable allow-listed destiation ";
    return false;
}

} // namespace

ConnectTunnel::ConnectTunnel(QuicSimpleServerBackend::RequestHandler* client_stream_request_handler, SocketFactory* socket_factory,
...
void ConnectTunnel::OpenTunnel(const quiche::HttpHeaderBlock& request_headers)
{
    ...
    std::optional<QuicServerId> authority = ValidateHeadersAndGetAuthority(request_headers);
    ...
    if (!ValidateAuthority(authority.value(), acceptable_destinations_)) {
        TerminateClientStream("disallowed request authority", QuicResetStreamError::FromIetf(QuicHttp3ErrorCode::REQUEST_REJECTED));
        return;
    }

    QuicSocketAddress address = tools::LookupAddress(AF_UNSPEC, authority.value());
    if (!address.IsInitialized()) {
        TerminateClientStream("host resolution error");
        return;
    }

    destination_socket_ = socket_factory_->CreateTcpClientSocket(address,
    ...
    if (!connect_result.ok()) {
        TerminateClientStream("error connecting TCP socket to destination server: " + connect_result.ToString());
        return;
    }

    QUICHE_DVLOG(1) << "CONNECT tunnel opened from stream " << client_stream_request_handler_->stream_id() << " to " << authority.value().ToHostPortString();
...
void ConnectTunnel::SendDataToDestination(absl::string_view data)
{
    ...
    absl::Status send_result = destination_socket_->SendBlocking(std::string(data));
    if (!send_result.ok()) {
        TerminateClientStream("TCP error sending data to destination server: " + send_result.ToString());
    }
}
...
void ConnectTunnel::OnClientStreamClose()
{
    ...
    QUICHE_DVLOG(1) << "CONNECT stream " << client_stream_request_handler_->stream_id() << " closed";
...
void ConnectTunnel::ReceiveComplete(absl::StatusOr<quiche::QuicheMemSlice> data)
{
    QUICHE_DCHECK(IsConnectedToDestination());
    QUICHE_DCHECK(receive_started_);
...
void ConnectTunnel::OnDestinationConnectionClosed()
{
    ...
    QUICHE_DVLOG(1) << "CONNECT stream " << client_stream_request_handler_->stream_id() << " destination connection closed";
    destination_socket_->Disconnect();
...
void ConnectTunnel::TerminateClientStream(absl::string_view error_description, QuicResetStreamError error_code)
{
    QUICHE_DCHECK(client_stream_request_handler_);
