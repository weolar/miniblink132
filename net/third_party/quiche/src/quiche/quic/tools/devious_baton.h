...
inline constexpr webtransport::SessionErrorCode kDeviousBatonErrorDaYamn = 0x01; // Insufficient flow control credit
inline constexpr webtransport::SessionErrorCode kDeviousBatonErrorBruh = 0x02; // Parse error
inline constexpr webtransport::SessionErrorCode kDeviousBatonErrorSus = 0x03; // Unexpected message
inline constexpr webtransport::SessionErrorCode kDeviousBatonErrorBored = 0x04; // Timeout
...
public:
    DeviousBatonSessionVisitor(webtransport::Session* session, bool is_server, int initial_value, int count)
        : session_(session)
    ...
    void OnSessionClosed(webtransport::SessionErrorCode error_code, const std::string& error_message) override;
...
private:
    ...
    // `send_function`.
    quiche::SingleUseCallback<void(std::string)> CreateResponseCallback(SendFunction send_function);
