...
constexpr QuicTime::Delta kTicketKeyLifetime = QuicTime::Delta::FromSeconds(60 * 60 * 24 * 7);
...
std::vector<uint8_t> SimpleTicketCrypter::Encrypt(absl::string_view in, absl::string_view encryption_key)
{
    ...
    if (!EVP_AEAD_CTX_seal(ctx, out.data() + kMessageOffset, &out_len, out.size() - kMessageOffset, out.data() + kIVOffset, kIVSize,
            reinterpret_cast<const uint8_t*>(in.data()), in.size(), nullptr, 0)) {
...
std::vector<uint8_t> SimpleTicketCrypter::Decrypt(absl::string_view in)
{
    ...
    if (!EVP_AEAD_CTX_open(ctx, out.data(), &out_len, out.size(), input + kIVOffset, kIVSize, input + kMessageOffset, in.size() - kMessageOffset, nullptr, 0)) {
        return std::vector<uint8_t>();
    }
    out.resize(out_len);
    return out;
...
void SimpleTicketCrypter::Decrypt(absl::string_view in, std::shared_ptr<quic::ProofSource::DecryptCallback> callback)
{
...
std::unique_ptr<SimpleTicketCrypter::Key> SimpleTicketCrypter::NewKey()
{
    ...
    EVP_AEAD_CTX_init(key->aead_ctx.get(), EVP_aead_aes_128_gcm(), key->key, kKeySize, EVP_AEAD_DEFAULT_TAG_LENGTH, nullptr);
