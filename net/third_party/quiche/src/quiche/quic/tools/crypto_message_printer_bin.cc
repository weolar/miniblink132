...
int main(int argc, char* argv[])
{
    ...
    std::vector<std::string> messages = quiche::QuicheParseCommandLineFlags(usage, argc, argv);
    if (messages.size() != 1) {
    ...
    if (framer.InputBytesRemaining() != 0) {
        cerr << "Input partially consumed. " << framer.InputBytesRemaining() << " bytes remaining." << endl;
        return 2;
    }
    return 0;
}
