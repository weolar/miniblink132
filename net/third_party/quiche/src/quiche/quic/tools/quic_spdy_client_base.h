...
class QuicSpdyClientBase : public QuicClientBase, public QuicSpdyStream::Visitor {
public:
    ...
    public:
        ...
        virtual void OnCompleteResponse(QuicStreamId id, const quiche::HttpHeaderBlock& response_headers, absl::string_view response_body) = 0;
    };

    QuicSpdyClientBase(const QuicServerId& server_id, const ParsedQuicVersionVector& supported_versions, const QuicConfig& config,
    ...
    void SendRequest(const quiche::HttpHeaderBlock& headers, absl::string_view body, bool fin);

    // Sends an HTTP request and waits for response before returning.
    void SendRequestAndWaitForResponse(const quiche::HttpHeaderBlock& headers, absl::string_view body, bool fin);
    ...
    void set_store_response(bool val)
    {
        store_response_ = val;
    }
    ...
    QuicTime::Delta latest_ttlb() const
    {
        return latest_ttlb_;
    }
    QuicTime::Delta latest_ttfb() const
    {
        return latest_ttfb_;
    }
    ...
    bool drop_response_body() const
    {
        return drop_response_body_;
    }
    ...
    bool enable_web_transport() const
    {
    ...
    bool use_datagram_contexts() const
    {
...
protected:
    ...
    std::unique_ptr<QuicSession> CreateQuicClientSession(const quic::ParsedQuicVersionVector& supported_versions, QuicConnection* connection) override;
...
private:
    void SendRequestInternal(quiche::HttpHeaderBlock sanitized_headers, absl::string_view body, bool fin);
