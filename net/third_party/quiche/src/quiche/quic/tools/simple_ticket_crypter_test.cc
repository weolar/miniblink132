...
public:
    explicit DecryptCallback(std::vector<uint8_t>* out)
        : out_(out)
    {
    }
...
TEST_F(SimpleTicketCrypterTest, EncryptDecrypt)
{
    ...
    std::vector<uint8_t> ciphertext = ticket_crypter_.Encrypt(StringPiece(plaintext), {});
    EXPECT_NE(plaintext, ciphertext);

    std::vector<uint8_t> out_plaintext;
    ticket_crypter_.Decrypt(StringPiece(ciphertext), std::make_unique<DecryptCallback>(&out_plaintext));
...
{
    std::vector<uint8_t> plaintext = { 1, 2, 3, 4, 5 };
    std::vector<uint8_t> ciphertext1 = ticket_crypter_.Encrypt(StringPiece(plaintext), {});
    std::vector<uint8_t> ciphertext2 = ticket_crypter_.Encrypt(StringPiece(plaintext), {});
...
TEST_F(SimpleTicketCrypterTest, DecryptionFailureWithModifiedCiphertext)
{
    ...
    for (size_t i = 0; i < ciphertext.size(); i++) {
        ...
        std::vector<uint8_t> out_plaintext;
        ticket_crypter_.Decrypt(StringPiece(munged_ciphertext), std::make_unique<DecryptCallback>(&out_plaintext));
...
{
    std::vector<uint8_t> out_plaintext;
    ticket_crypter_.Decrypt(absl::string_view(), std::make_unique<DecryptCallback>(&out_plaintext));
    EXPECT_TRUE(out_plaintext.empty());
}
