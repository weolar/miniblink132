...
InteractiveCli::InteractiveCli(QuicEventLoop* event_loop, LineCallback line_callback)
    ...
    , line_callback_(std::move(line_callback))
{
    ...
    [[maybe_unused]] bool success = event_loop_->RegisterSocket(STDIN_FILENO, kSocketEventReadable, this);
    QUICHE_LOG_IF(FATAL, !success) << "Failed to register stdin with the event loop";
...
InteractiveCli::~InteractiveCli()
{
    if (old_termios_ != nullptr) {
        tcsetattr(STDIN_FILENO, TCSANOW, reinterpret_cast<termios*>(old_termios_.get()));
    }
...
void InteractiveCli::OnSocketEvent(QuicEventLoop* event_loop, SocketFd fd, QuicSocketEventMask events)
{
    ...
    // Handle backspace.
    while (current_input_line_.size() >= 2 && current_input_line_.back() == '\x7f') {
    ...
    current_input_line_.erase(
        std::remove_if(current_input_line_.begin(), current_input_line_.end(), [](char c) { return absl::ascii_iscntrl(c); }), current_input_line_.end());
