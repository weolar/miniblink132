...
std::string ReadLink(const std::string& path)
{
    ...
    QUICHE_CHECK(result_size > 0 && static_cast<size_t>(result_size) < result.size())
        << "result_size:" << result_size << ", errno:" << errno << ", path:" << path;
...
size_t NumOpenSocketFDs()
{
    ...
    dirent* file;
    std::unique_ptr<DIR, int (*)(DIR*)> fd_directory(opendir(kPathToFds), closedir);
...
public:
    ...
    std::unique_ptr<QuicDefaultClient> CreateAndInitializeQuicClient()
    {
        ...
        auto client = std::make_unique<QuicDefaultClient>(server_address, server_id, versions, event_loop_.get(), crypto_test_utils::ProofVerifierForTesting());
...
TEST_F(QuicDefaultClientTest, CreateAndCleanUpUDPSockets)
{
    ...
    EXPECT_TRUE(client->default_network_helper()->CreateUDPSocketAndBind(client->server_address(), client->bind_to_address(), client->local_port()));
