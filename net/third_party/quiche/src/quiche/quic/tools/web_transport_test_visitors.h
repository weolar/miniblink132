...
public:
    WebTransportDiscardVisitor(WebTransportStream* stream)
        : stream_(stream)
    {
    }
    ...
    void OnCanRead() override
    {
        ...
        QUIC_DVLOG(2) << "Read " << result.bytes_read << " bytes from WebTransport stream " << stream_->GetStreamId() << ", fin: " << result.fin;
    }
...
public:
    ...
    void OnCanRead() override
    {
        ...
        QUIC_DVLOG(1) << "Attempted reading on WebTransport bidirectional stream " << stream_->GetStreamId() << ", bytes read: " << result.bytes_read;
        if (result.fin) {
            send_fin_ = true;
        }
        OnCanWrite();
    ...
    void OnCanWrite() override
    {
        ...
        if (!buffer_.empty()) {
            ...
            QUIC_DVLOG(1) << "Attempted writing on WebTransport bidirectional stream " << stream_->GetStreamId() << ", success: " << status;
            if (!status.ok()) {
                return;
            }
...
using WebTransportUnidirectionalEchoReadVisitor = ::webtransport::CompleteBufferVisitor;
using WebTransportUnidirectionalEchoWriteVisitor = ::webtransport::CompleteBufferVisitor;
...
public:
    EchoWebTransportSessionVisitor(WebTransportSession* session, bool open_server_initiated_echo_stream = true)
        : session_(session)
    ...
    void OnIncomingBidirectionalStreamAvailable() override
    {
        while (true) {
            ...
            QUIC_DVLOG(1) << "EchoWebTransportSessionVisitor received a bidirectional stream " << stream->GetStreamId();
            stream->SetVisitor(std::make_unique<WebTransportBidirectionalEchoVisitor>(stream));
            stream->visitor()->OnCanRead();
        }
    }
    ...
    void OnIncomingUnidirectionalStreamAvailable() override
    {
        while (true) {
            ...
            QUIC_DVLOG(1) << "EchoWebTransportSessionVisitor received a unidirectional stream";
            stream->SetVisitor(std::make_unique<WebTransportUnidirectionalEchoReadVisitor>(stream, [this](const std::string& data) {
    ...
    {
        while (!streams_to_echo_back_.empty() && session_->CanOpenNextOutgoingUnidirectionalStream()) {
            QUIC_DVLOG(1) << "EchoWebTransportServer echoed a unidirectional stream back";
            WebTransportStream* stream = session_->OpenOutgoingUnidirectionalStream();
            stream->SetVisitor(std::make_unique<WebTransportUnidirectionalEchoWriteVisitor>(stream, streams_to_echo_back_.front()));
