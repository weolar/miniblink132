...
public:
    ...
    virtual void OnResponseBackendComplete(const QuicBackendResponse* response) override
    {
...
protected:
    void CreateRequest(std::string host, std::string path, quiche::HttpHeaderBlock* headers)
    {
        (*headers)[":method"] = "GET";
        (*headers)[":path"] = path;
...
{
    const Response* response = cache_.GetResponse("mail.google.com", "/index.html");
...
TEST_F(QuicMemoryCacheBackendTest, AddResponse)
{
    ...
    cache_.AddResponse(kRequestHost, "/", response_headers.Clone(), kResponseBody, response_trailers.Clone());
...
TEST_F(QuicMemoryCacheBackendTest, MAYBE_ReadsCacheDir)
{
    ...
    const Response* response = cache_.GetResponse("test.example.com", "/index.html");
    ASSERT_TRUE(response);
    ASSERT_TRUE(response->headers().contains(":status"));
    EXPECT_EQ("200", response->headers().find(":status")->second);
...
{
    cache_.InitializeBackend(CacheDirectory());
    const Response* response = cache_.GetResponse("test.example.com", "/site_map.html");
    ASSERT_TRUE(response);
    ASSERT_TRUE(response->headers().contains(":status"));
    EXPECT_EQ("200", response->headers().find(":status")->second);
    // Connection headers are not valid in HTTP/2.
