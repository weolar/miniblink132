...
using WebTransportRequestCallback
    = quiche::MultiUseCallback<absl::StatusOr<std::unique_ptr<webtransport::SessionVisitor>>(absl::string_view path, WebTransportSession* session)>;
...
public:
    ...
    void FetchResponseFromBackend(const quiche::HttpHeaderBlock&, const std::string&, RequestHandler* request_handler) override;
    void CloseBackendResponseStream(RequestHandler*) override
    {
    }
    bool SupportsWebTransport() override
    {
        return true;
    }
    WebTransportResponse ProcessWebTransportRequest(const quiche::HttpHeaderBlock& request_headers, WebTransportSession* session) override;
