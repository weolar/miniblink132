...
public:
    ...
    // Section 2.
    ConnectServerBackend(std::unique_ptr<QuicSimpleServerBackend> non_connect_backend, absl::flat_hash_set<QuicServerId> acceptable_connect_destinations,
        absl::flat_hash_set<QuicServerId> acceptable_connect_udp_targets, std::string server_label);
    ...
    void FetchResponseFromBackend(const quiche::HttpHeaderBlock& request_headers, const std::string& request_body, RequestHandler* request_handler) override;
    void HandleConnectHeaders(const quiche::HttpHeaderBlock& request_headers, RequestHandler* request_handler) override;
    void HandleConnectData(absl::string_view data, bool data_complete, RequestHandler* request_handler) override;
...
private:
    ...
    absl::flat_hash_map<std::pair<QuicConnectionId, QuicStreamId>, std::unique_ptr<ConnectTunnel>> connect_tunnels_;
    absl::flat_hash_map<std::pair<QuicConnectionId, QuicStreamId>, std::unique_ptr<ConnectUdpTunnel>> connect_udp_tunnels_;
};
