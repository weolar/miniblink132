...
class QUIC_EXPORT_PRIVATE PathMigrationContext : public QuicPathValidationContext {
public:
    PathMigrationContext(std::unique_ptr<QuicPacketWriter> writer, const QuicSocketAddress& self_address, const QuicSocketAddress& peer_address)
...
public:
    ...
    public:
        ...
        virtual bool CreateUDPSocketAndBind(QuicSocketAddress server_address, QuicIpAddress bind_to_address, int bind_to_port) = 0;
    ...
    QuicClientBase(const QuicServerId& server_id, const ParsedQuicVersionVector& supported_versions, const QuicConfig& config,
    ...
    {
        crypto_config_.set_tls_signature_algorithms(std::move(signature_algorithms));
    }
    ...
    void set_connected_or_attempting_connect(bool connected_or_attempting_connect)
    {
    ...
    const QuicSocketAddress& server_address() const
    {
    ...
    void set_connection_debug_visitor(QuicConnectionDebugVisitor* connection_debug_visitor)
    {
    ...
    void set_server_connection_id_override(const QuicConnectionId& connection_id)
    {
    ...
    const std::vector<std::unique_ptr<QuicPathValidationContext>>& validated_paths() const
    {
    ...
    // default.
    void EnablePortMigrationUponPathDegrading(std::optional<int> num_ptos_for_path_degrading)
    {
        allow_port_migration_ = true;
        if (num_ptos_for_path_degrading.has_value()) {
            session_->connection()->sent_packet_manager().set_num_ptos_for_path_degrading(num_ptos_for_path_degrading.value());
        }
    }

    virtual void OnSocketMigrationProbingSuccess(std::unique_ptr<QuicPathValidationContext> context);
...
protected:
    ...
    virtual std::unique_ptr<QuicSession> CreateQuicClientSession(const ParsedQuicVersionVector& supported_versions, QuicConnection* connection) = 0;
...
private:
    ...
    std::unique_ptr<QuicPacketWriter> CreateWriterForNewNetwork(const QuicIpAddress& new_host, int port);
    ...
    DeterministicConnectionIdGenerator connection_id_generator_ { kQuicDefaultConnectionIdLength };
