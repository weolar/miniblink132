...
DEFINE_QUICHE_COMMAND_LINE_FLAG(std::string, quic_version, "", "If set, specify the QUIC version to use.");
...
public:
    ...
    bool OnProtocolVersionMismatch(ParsedQuicVersion received_version) override
    {
        ...
        std::cerr << "OnProtocolVersionMismatch: " << ParsedQuicVersionToString(received_version) << "\n";
        return true;
    }
    ...
    bool OnStreamFrame(const QuicStreamFrame& frame) override
    {
        ...
        std::cerr << "         data: { " << absl::BytesToHexString(absl::string_view(frame.data_buffer, frame.data_length)) << " }\n";
        return true;
    }
    ...
    {
        std::cerr << "OnAckTimestamp: [" << packet_number << ", " << timestamp.ToDebuggingValue() << ")";
        return true;
    }
...
int main(int argc, char* argv[])
{
    ...
    quic::QuicFramer framer(versions, start, perspective, quic::kQuicDefaultConnectionIdLength);
    const quic::ParsedQuicVersion& version = quic::ParseQuicVersionString(quiche::GetQuicheCommandLineFlag(FLAGS_quic_version));
