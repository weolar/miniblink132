...
std::string Serialize(DeviousBatonValue value)
{
    ...
    QuicByteCount padding_size = QuicRandom::GetInstance()->InsecureRandUint64() % kMaxPaddingSize;
    absl::string_view padding(kPaddingData, padding_size);

    absl::StatusOr<std::string> result
        = quiche::SerializeIntoString(quiche::WireStringWithLengthPrefix<quiche::WireVarInt62>(padding), quiche::WireUint8(value));
...
public:
    IncomingBidiBatonVisitor(webtransport::Session& session, webtransport::Stream& stream)
        : CompleteBufferVisitor(&stream, absl::bind_front(&IncomingBidiBatonVisitor::OnAllDataReceived, this))
        , session_(&session)
    {
    }
...
private:
    void OnAllDataReceived(std::string data)
    {
        ...
        if (!value.ok()) {
            session_->CloseSession(kDeviousBatonErrorBruh, absl::StrCat("Failed to parse incoming baton: ", value.status().message()));
            return;
        }
...
void DeviousBatonSessionVisitor::OnSessionReady()
{
    ...
    for (int i = 0; i < count_; ++i) {
        ...
        if (stream == nullptr) {
            session_->CloseSession(kDeviousBatonErrorDaYamn, "Insufficient flow control when opening initial baton streams");
            return;
        }
        stream->SetVisitor(std::make_unique<webtransport::CompleteBufferVisitor>(stream, Serialize(initial_value_)));
        stream->visitor()->OnCanWrite();
    }
}

void DeviousBatonSessionVisitor::OnSessionClosed(webtransport::SessionErrorCode error_code, const std::string& error_message)
{
    QUICHE_LOG(INFO) << "Devious Baton session closed with error " << error_code << " (message: " << error_message << ")";
}

void DeviousBatonSessionVisitor::OnIncomingBidirectionalStreamAvailable()
{
    while (true) {
        webtransport::Stream* stream = session_->AcceptIncomingBidirectionalStream();
        if (stream == nullptr) {
            return;
        }
        stream->SetVisitor(std::make_unique<IncomingBidiBatonVisitor>(*session_, *stream));
...
{
    while (true) {
        webtransport::Stream* stream = session_->AcceptIncomingUnidirectionalStream();
        if (stream == nullptr) {
            return;
        }
        stream->SetVisitor(
            std::make_unique<webtransport::CompleteBufferVisitor>(stream, CreateResponseCallback(&DeviousBatonSessionVisitor::SendBidirectionalBaton)));
        stream->visitor()->OnCanRead();
    }
}

void DeviousBatonSessionVisitor::OnDatagramReceived(absl::string_view datagram)
{
...
void DeviousBatonSessionVisitor::OnCanCreateNewOutgoingBidirectionalStream()
{
    while (!outgoing_bidi_batons_.empty()) {
        ...
        stream->SetVisitor(std::make_unique<webtransport::CompleteBufferVisitor>(
            stream, Serialize(outgoing_bidi_batons_.front()), CreateResponseCallback(&DeviousBatonSessionVisitor::SendUnidirectionalBaton)));
...
void DeviousBatonSessionVisitor::OnCanCreateNewOutgoingUnidirectionalStream()
{
    while (!outgoing_unidi_batons_.empty()) {
        ...
        stream->SetVisitor(std::make_unique<webtransport::CompleteBufferVisitor>(stream, Serialize(outgoing_unidi_batons_.front())));
        outgoing_unidi_batons_.pop_front();
        stream->visitor()->OnCanWrite();
    }
}

quiche::SingleUseCallback<void(std::string)> DeviousBatonSessionVisitor::CreateResponseCallback(SendFunction send_function)
{
