...
public:
    ...
    size_t num_padding_bytes() const
    {
        return num_padding_bytes_;
    }
...
protected:
    ...
    absl::Status EncodeKnownLengthFieldsAndBody(quiche::QuicheDataWriter& writer) const;
...
void QUICHE_EXPORT PrintTo(const BinaryHttpMessage::Field& msg, std::ostream* os);
...
public:
    ...
    struct QUICHE_EXPORT ControlData {
        ...
        {
            return method == rhs.method && scheme == rhs.scheme && authority == rhs.authority && path == rhs.path;
        }
        bool operator!=(const BinaryHttpRequest::ControlData& rhs) const
        {
            return !(*this == rhs);
    ...
    {
        return control_data_ == rhs.control_data_ && BinaryHttpMessage::IsPayloadEqual(rhs);
    }

    bool operator==(const BinaryHttpRequest& rhs) const
    {
        return IsPayloadEqual(rhs) && num_padding_bytes() == rhs.num_padding_bytes();
    }
...
public:
    ...
    public:
        ...
        InformationalResponse(uint16_t status_code, const std::vector<BinaryHttpMessage::Field>& fields)
            : status_code_(status_code)
        {
        ...
        bool operator==(const BinaryHttpResponse::InformationalResponse& rhs) const
        {
    ...
    absl::Status AddInformationalResponse(uint16_t status_code, std::vector<Field> header_fields);
    ...
    {
        return informational_response_control_data_ == rhs.informational_response_control_data_ && status_code_ == rhs.status_code_
