...
class EncapsulatedWebTransportTest : public quiche::test::QuicheTest, public quiche::CapsuleParser::Visitor {
public:
    EncapsulatedWebTransportTest()
        : parser_(this)
        , reader_(&read_buffer_)
    {
        ON_CALL(fatal_error_callback_, Call(_)).WillByDefault([](absl::string_view error) { ADD_FAILURE() << "Fatal session error: " << error; });
        ON_CALL(writer_, Writev(_, _)).WillByDefault([&](absl::Span<const absl::string_view> data, const quiche::StreamWriteOptions& options) {
    ...
    std::unique_ptr<EncapsulatedSession> CreateTransport(Perspective perspective)
    {
        auto transport = std::make_unique<EncapsulatedSession>(perspective, fatal_error_callback_.AsStdFunction());
    ...
    {
        quiche::QuicheBuffer buffer = quiche::SerializeCapsule(capsule, quiche::SimpleBufferAllocator::Get());
        read_buffer_.append(buffer.data(), buffer.size());
        session_->OnCanRead();
    }

    template <typename CapsuleType> void ProcessIncomingCapsule(const CapsuleType& capsule)
    {
        quiche::QuicheBuffer buffer = quiche::SerializeCapsule(quiche::Capsule(capsule), quiche::SimpleBufferAllocator::Get());
        read_buffer_.append(buffer.data(), buffer.size());
        session_->OnCanRead();
    }
    ...
    void DefaultHandshakeForClient(EncapsulatedSession& session)
    {
        ...
        session.InitializeClient(CreateAndStoreVisitor(), outgoing_headers, &writer_, &reader_);
...
{
    std::unique_ptr<EncapsulatedSession> session = CreateTransport(Perspective::kClient);
    quiche::HttpHeaderBlock outgoing_headers, incoming_headers;
...
{
    std::unique_ptr<EncapsulatedSession> session = CreateTransport(Perspective::kServer);
    quiche::HttpHeaderBlock outgoing_headers, incoming_headers;
    EXPECT_EQ(session->state(), EncapsulatedSession::kUninitialized);
    std::unique_ptr<SessionVisitor> visitor = CreateAndStoreVisitor();
    EXPECT_CALL(*visitor_, OnSessionReady());
    session->InitializeServer(std::move(visitor), outgoing_headers, incoming_headers, &writer_, &reader_);
    EXPECT_EQ(session->state(), EncapsulatedSession::kSessionOpen);
}
...
TEST_F(EncapsulatedWebTransportTest, CloseSession)
{
    ...
    EXPECT_CALL(*this, OnCapsule(_)).WillOnce([](const Capsule& capsule) {
        ...
        EXPECT_EQ(capsule.close_web_transport_session_capsule().error_message, "test close");
        return true;
    });
    EXPECT_EQ(session->state(), EncapsulatedSession::kSessionOpen);
    ...
    EXPECT_CALL(fatal_error_callback_, Call(_)).WillOnce([](absl::string_view error) {
...
TEST_F(EncapsulatedWebTransportTest, ReceiveMalformedData)
{
    ...
    EXPECT_CALL(fatal_error_callback_, Call(HasSubstr("too much capsule data"))).WillOnce([] {});
...
TEST_F(EncapsulatedWebTransportTest, ReceiveDatagrams)
{
    std::unique_ptr<EncapsulatedSession> session = CreateTransport(Perspective::kClient);
    DefaultHandshakeForClient(*session);
    EXPECT_CALL(*visitor_, OnDatagramReceived(_)).WillOnce([](absl::string_view data) { EXPECT_EQ(data, "test"); });
...
TEST_F(EncapsulatedWebTransportTest, WriteErrorDatagram)
{
    std::unique_ptr<EncapsulatedSession> session = CreateTransport(Perspective::kClient);
    DefaultHandshakeForClient(*session);
    EXPECT_CALL(writer_, Writev(_, _)).WillOnce(Return(absl::InternalError("Test write error")));
    EXPECT_CALL(fatal_error_callback_, Call(_)).WillOnce([](absl::string_view error) { EXPECT_THAT(error, HasSubstr("Test write error")); });
    DatagramStatus status = session->SendOrQueueDatagram("test");
...
TEST_F(EncapsulatedWebTransportTest, SimpleRead)
{
    ...
    EXPECT_CALL(*visitor_, OnIncomingBidirectionalStreamAvailable()).WillOnce([&] { stream_received = true; });
...
TEST_F(EncapsulatedWebTransportTest, ImmediateRead)
{
    ...
    ProcessIncomingCapsule(quiche::WebTransportStreamDataCapsule { 1, "abcd", false });
    Stream* stream = session->AcceptIncomingBidirectionalStream();
    ASSERT_TRUE(stream != nullptr);
    EXPECT_EQ(stream->ReadableBytes(), 4u);
...
TEST_F(EncapsulatedWebTransportTest, FinRead)
{
    ...
    DefaultHandshakeForClient(*session);
    EXPECT_CALL(*visitor_, OnIncomingBidirectionalStreamAvailable());
    ProcessIncomingCapsule(quiche::WebTransportStreamDataCapsule { 1, "abcdef", true });
    Stream* stream = session->AcceptIncomingBidirectionalStream();
    ASSERT_TRUE(stream != nullptr);
...
TEST_F(EncapsulatedWebTransportTest, LargeRead)
{
    ...
    DefaultHandshakeForClient(*session);
    EXPECT_CALL(*visitor_, OnIncomingBidirectionalStreamAvailable());
    ProcessIncomingCapsule(quiche::WebTransportStreamDataCapsule { 1, std::string(64 * 1024, 'a'), true });
    Stream* stream = session->AcceptIncomingBidirectionalStream();
    ASSERT_TRUE(stream != nullptr);
...
TEST_F(EncapsulatedWebTransportTest, DoubleFinReceived)
{
    ...
    EXPECT_CALL(fatal_error_callback_, Call(_)).WillOnce([](absl::string_view error) { EXPECT_THAT(error, HasSubstr("has already received a FIN")); });
...
TEST_F(EncapsulatedWebTransportTest, WriteOnlyGarbageCollection)
{
    ...
    EXPECT_THAT(stream->Writev(absl::Span<const absl::string_view>(), options), StatusIs(absl::StatusCode::kOk));
    session->GarbageCollectStreams();
    EXPECT_TRUE(deleted);
}
...
TEST_F(EncapsulatedWebTransportTest, FinOnlyWrite)
{
    ...
    options.set_send_fin(true);
    EXPECT_TRUE(stream->CanWrite());
    absl::Status status = stream->Writev(absl::Span<const absl::string_view>(), options);
    EXPECT_THAT(status, StatusIs(absl::StatusCode::kOk));
    EXPECT_FALSE(stream->CanWrite());
}
