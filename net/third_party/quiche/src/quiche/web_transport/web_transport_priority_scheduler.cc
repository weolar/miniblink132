...
absl::Status PriorityScheduler::Register(StreamId stream_id, const StreamPriority& priority)
{
    ...
    auto cleanup_nullptr_map_entry = absl::MakeCleanup([&] { stream_to_group_map_.erase(stream_id); });

    auto [scheduler_it, scheduler_created] = per_group_schedulers_.try_emplace(priority.send_group_id);
...
absl::Status PriorityScheduler::UpdateSendOrder(StreamId stream_id, SendOrder new_send_order)
{
...
absl::Status PriorityScheduler::UpdateSendGroup(StreamId stream_id, SendGroupId new_send_group)
{
    PerGroupScheduler* scheduler = SchedulerForStream(stream_id);
    if (scheduler == nullptr) {
        return absl::NotFoundError("Stream ID not registered");
    ...
    if (!send_order.has_value()) {
        return absl::InternalError("Stream registered at the top level scheduler, but not at the "
                                   "per-group one");
    }
    QUICHE_RETURN_IF_ERROR(Unregister(stream_id));
    QUICHE_RETURN_IF_ERROR(Register(stream_id, StreamPriority { new_send_group, *send_order }));
    if (is_scheduled) {
...
std::optional<StreamPriority> PriorityScheduler::GetPriorityFor(StreamId stream_id) const
{
    auto it = stream_to_group_map_.find(stream_id);
    if (it == stream_to_group_map_.end()) {
        return std::nullopt;
    ...
    std::optional<SendOrder> send_order = group_scheduler.GetPriorityFor(stream_id);
    if (!send_order.has_value()) {
        return std::nullopt;
    }
...
absl::StatusOr<StreamId> PriorityScheduler::PopFront()
{
    ...
    if (it == per_group_schedulers_.end()) {
        return absl::InternalError("Scheduled a group with no per-group scheduler attached");
    }
