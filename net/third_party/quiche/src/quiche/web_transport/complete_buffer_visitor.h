...
public:
    ...
    CompleteBufferVisitor(webtransport::Stream* stream, AllDataReadCallback incoming_data_callback)
        : stream_(stream)
        , incoming_data_callback_(std::in_place, std::move(incoming_data_callback))
    {
    }
    CompleteBufferVisitor(webtransport::Stream* stream, std::string outgoing_data, AllDataReadCallback incoming_data_callback)
        : stream_(stream)
        , outgoing_data_(std::in_place, std::move(outgoing_data))
        , incoming_data_callback_(std::in_place, std::move(incoming_data_callback))
    {
