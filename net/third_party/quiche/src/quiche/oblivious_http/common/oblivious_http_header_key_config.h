...
public:
    ...
    static constexpr absl::string_view kOhttpRequestLabel = "message/bhttp request";
    static constexpr absl::string_view kOhttpResponseLabel = "message/bhttp response";
    // Length of the Oblivious HTTP header.
    static constexpr uint32_t kHeaderLength = sizeof(uint8_t) + (3 * sizeof(uint16_t));
    static constexpr absl::string_view kKeyHkdfInfo = "key";
    static constexpr absl::string_view kNonceHkdfInfo = "nonce";

    static absl::StatusOr<ObliviousHttpHeaderKeyConfig> Create(uint8_t key_id, uint16_t kem_id, uint16_t kdf_id, uint16_t aead_id);

    // Copyable to support stack allocated pass-by-value for trivial data members.
    ObliviousHttpHeaderKeyConfig(const ObliviousHttpHeaderKeyConfig& other) = default;
    ObliviousHttpHeaderKeyConfig& operator=(const ObliviousHttpHeaderKeyConfig& other) = default;

    // Movable.
    ObliviousHttpHeaderKeyConfig(ObliviousHttpHeaderKeyConfig&& other) = default;
    ObliviousHttpHeaderKeyConfig& operator=(ObliviousHttpHeaderKeyConfig&& other) = default;
    ...
    uint16_t GetHpkeKemId() const
    {
        return kem_id_;
    }
    uint16_t GetHpkeKdfId() const
    {
        return kdf_id_;
    }
    uint16_t GetHpkeAeadId() const
    {
        return aead_id_;
    }
    ...
    std::string SerializeRecipientContextInfo(absl::string_view request_label = ObliviousHttpHeaderKeyConfig::kOhttpRequestLabel) const;
    ...
    static absl::StatusOr<uint8_t> ParseKeyIdFromObliviousHttpRequestPayload(absl::string_view payload_bytes);
...
private:
    // Constructor
    explicit ObliviousHttpHeaderKeyConfig(uint8_t key_id, uint16_t kem_id, uint16_t kdf_id, uint16_t aead_id);
...
public:
    ...
    struct SymmetricAlgorithmsConfig {
        ...
        template <typename H> friend H AbslHashValue(H h, const SymmetricAlgorithmsConfig& sym_alg_cfg)
        {
    ...
    struct OhttpKeyConfig {
        ...
        {
            return key_id == other.key_id && kem_id == other.kem_id && public_key == other.public_key && symmetric_algorithms == other.symmetric_algorithms;
        }

        template <typename H> friend H AbslHashValue(H h, const OhttpKeyConfig& ohttp_key_cfg)
        {
            return H::combine(std::move(h), ohttp_key_cfg.key_id, ohttp_key_cfg.kem_id, ohttp_key_cfg.public_key, ohttp_key_cfg.symmetric_algorithms);
        }
    };
    ...
    static absl::StatusOr<ObliviousHttpKeyConfigs> ParseConcatenatedKeys(absl::string_view key_configs);
    ...
    static absl::StatusOr<ObliviousHttpKeyConfigs> Create(absl::flat_hash_set<OhttpKeyConfig> ohttp_key_configs);
    ...
    static absl::StatusOr<ObliviousHttpKeyConfigs> Create(const ObliviousHttpHeaderKeyConfig& single_key_config, absl::string_view public_key);
...
private:
    ...
    using ConfigMap = absl::btree_map<uint8_t, std::vector<ObliviousHttpHeaderKeyConfig>, std::greater<uint8_t>>;
    ...
    static absl::Status ReadSingleKeyConfig(QuicheDataReader& reader, ConfigMap& configs, PublicKeyMap& keys);
