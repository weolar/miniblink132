...
template <typename T> class VerifyThatHelper {
public:
    ...
    const std::string& printed_value() const
    {
        return printed_value_;
    }
...
std::string GetBoolAssertionFailureMessage(
    const ::testing::AssertionResult& assertion_result, const char* expression_text, const char* actual_predicate_value, const char* expected_predicate_value);
...
inline ::testing::PolymorphicMatcher<net::test::StringPieceHasSubstrMatcher> HasSubstr(const std::string& substring)
{
    return ::testing::MakePolymorphicMatcher(net::test::StringPieceHasSubstrMatcher(substring));
}
} // namespace
...
#define VERIFY_TEST_BOOLEAN_(condition, text, actual, expected)                                                                                                \
    if (const ::testing::AssertionResult __assertion_result = ::testing::AssertionResult((condition) ? expected : actual))                                     \
        ;                                                                                                                                                      \
    else                                                                                                                                                       \
        return ::testing::AssertionFailure() << VERIFY_FAILED_LOCATION_                                                                                        \
                                             << ::http2::test::GetBoolAssertionFailureMessage(__assertion_result, text, #actual, #expected)
// clang-format on

// Boolean assertions. condition can be either a Boolean expression or an
// expression convertible to a boolean (such as a ::gtl::labs::optional).
#define VERIFY_TRUE(condition) VERIFY_TEST_BOOLEAN_(condition, #condition, false, true)

#define VERIFY_FALSE(condition) VERIFY_TEST_BOOLEAN_(condition, #condition, true, false)
...
#define VERIFY_THAT(value, matcher)                                                                                                                            \
    if (const auto& _verify_that_helper = ::http2::test::VerifyThatHelper<decltype(value)>(value, matcher))                                                    \
        ;                                                                                                                                                      \
    else                                                                                                                                                       \
        return ::testing::AssertionFailure() << "Failed to verify that '" #value "' (" << _verify_that_helper.printed_value() << ") "                          \
                                             << _verify_that_helper.matcher_description() << " (on " __FILE__ ":" << __LINE__ << "). "
