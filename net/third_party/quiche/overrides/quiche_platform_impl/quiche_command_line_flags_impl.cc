...
void AppendFlagDescription(const std::string& name, std::string help, std::string* out)
{
    ...
    while (!help.empty()) {
        ...
        base::StrAppend(out, { help.substr(0, wrap_pos), "\n", std::string(kStartCol, ' ') });
...
[[maybe_unused]] std::vector<std::string> ToQuicheStringVector(const std::vector<std::string>& v)
{
    return v;
}
...
[[maybe_unused]] std::vector<std::string> ToQuicheStringVector(const std::vector<std::wstring>& v)
{
...
void QuicheFlagRegistry::RegisterFlag(const char* name, std::unique_ptr<QuicheFlagHelper> helper)
{
    flags_.emplace(std::string(name), std::move(helper));
}

bool QuicheFlagRegistry::SetFlags(const base::CommandLine& command_line, std::string* error_msg) const
{
    for (const auto& kv : flags_) {
        ...
        if (!helper->SetFlag(value)) {
            *error_msg = base::StrCat({ "Invalid value \"", value, "\" for flag --", name });
            return false;
        }
...
template <> bool TypedQuicheFlagHelper<bool>::SetFlag(const std::string& s) const
{
    static const base::NoDestructor<std::set<std::string>> kTrueValues(std::initializer_list<std::string>({ "", "1", "t", "true", "y", "yes" }));
    static const base::NoDestructor<std::set<std::string>> kFalseValues(std::initializer_list<std::string>({ "0", "f", "false", "n", "no" }));
...
template <> bool TypedQuicheFlagHelper<uint16_t>::SetFlag(const std::string& s) const
{
    int value;
    if (!base::StringToInt(s, &value) || value < std::numeric_limits<uint16_t>::min() || value > std::numeric_limits<uint16_t>::max()) {
        return false;
    }
...
template <> bool TypedQuicheFlagHelper<int32_t>::SetFlag(const std::string& s) const
{
    int32_t value;
...
template <> bool TypedQuicheFlagHelper<std::string>::SetFlag(const std::string& s) const
{
    *flag_ = s;
    return true;
}
...
std::vector<std::string> QuicheParseCommandLineFlagsImpl(const char* usage, int argc, const char* const* argv)
{
    base::CommandLine::Init(argc, argv);
    auto result = QuicheParseCommandLineFlagsHelper(usage, *base::CommandLine::ForCurrentProcess());
...
QuicheParseCommandLineFlagsResult QuicheParseCommandLineFlagsHelper(const char* usage, const base::CommandLine& command_line)
{
...
{
    std::cout << usage << std::endl << "Options:" << std::endl << QuicheFlagRegistry::GetInstance().GetHelp() << std::endl;
}

QuicheParseCommandLineFlagsResult::QuicheParseCommandLineFlagsResult() = default;
QuicheParseCommandLineFlagsResult::QuicheParseCommandLineFlagsResult(const QuicheParseCommandLineFlagsResult&) = default;
QuicheParseCommandLineFlagsResult::~QuicheParseCommandLineFlagsResult() = default;
