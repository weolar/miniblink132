...
bool ExpandURITemplateImpl(const std::string& uri_template, const absl::flat_hash_map<std::string, std::string>& parameters, std::string* target,
    absl::flat_hash_set<std::string>* vars_found)
{
    ...
    const bool result = uri_template::Expand(uri_template, std_parameters, target, vars_found != nullptr ? &std_vars_found : nullptr);
...
std::optional<std::string> AsciiUrlDecodeImpl(std::string_view input)
{
    ...
    url::DecodeURLEscapeSequences(input, url::DecodeURLMode::kUTF8, &canon_output);
    std::string output;
