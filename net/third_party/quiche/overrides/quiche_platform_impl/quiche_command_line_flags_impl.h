...
public:
    explicit QuicheFlagHelper(const char* help)
        : help_(help)
    {
    }
...
template <typename T> class TypedQuicheFlagHelper : public QuicheFlagHelper {
public:
    TypedQuicheFlagHelper(T* flag, const T& default_value, const char* help)
        : QuicheFlagHelper(help)
        , flag_(flag)
        , default_value_(default_value)
    {
    }
    bool SetFlag(const std::string& value) const override;
    void ResetFlag() const override
    {
...
template <> bool TypedQuicheFlagHelper<bool>::SetFlag(const std::string&) const;
template <> bool TypedQuicheFlagHelper<uint16_t>::SetFlag(const std::string&) const;
template <> bool TypedQuicheFlagHelper<int32_t>::SetFlag(const std::string&) const;
template <> bool TypedQuicheFlagHelper<std::string>::SetFlag(const std::string&) const;
...
public:
    ...
    bool SetFlags(const base::CommandLine& command_line, std::string* error_msg) const;
...
template <typename T> class QuicheFlagSetup {
public:
    QuicheFlagSetup(T* flag, const char* name, const T& default_value, const char* help)
    {
        QuicheFlagRegistry::GetInstance().RegisterFlag(name, std::make_unique<TypedQuicheFlagHelper<T>>(flag, default_value, help));
    }
...
#define DEFINE_QUICHE_COMMAND_LINE_FLAG_IMPL(type, name, default_value, help)                                                                                  \
    ...
    bool FLAGS_no##name = quiche::QuicheFlagSetup<type>(&FLAGS_##name, #name, default_value, help)
...
template <typename T> T GetQuicheCommandLineFlag(const T& flag)
{
    return flag;
}
