...
#define QUICHE_DLOG_IF_IMPL(severity, condition) QUICHE_CHROMIUM_DLOG_IF_##severity(condition)
#define QUICHE_LOG_IF_IMPL(severity, condition) QUICHE_CHROMIUM_LOG_IF_##severity(condition)
...
#define QUICHE_CHROMIUM_LOG_FATAL LAZY_STREAM(LOG_STREAM(FATAL), ::logging::ShouldCreateLogMessage(::logging::LOGGING_FATAL))
...
#define QUICHE_DVLOG_IF_IMPL(verbose_level, condition) DVLOG_IF(verbose_level, condition)
...
namespace quic {
template <typename T> QUICHE_EXPORT inline std::ostream& operator<<(std::ostream& out, const std::vector<T>& v)
{
    out << "[";
    const char* sep = "";
