...
template <class T> class QuicheTestWithParamImpl : public ::testing::TestWithParam<T> {
private:
    QuicFlagChecker checker_;
    QuicFlagSaverImpl saver_; // Save/restore all QUIC flag values.
};
...
#define EXPECT_QUICHE_DEBUG_DEATH_IMPL(condition, message) EXPECT_DEBUG_DEATH(condition, message)
#else
...
#define EXPECT_QUICHE_DEATH_IMPL(condition, message) EXPECT_CHECK_DEATH_WITH(condition, message)
...
using ScopedEnvironmentForThreadsImpl = quiche::test::ScopedEnvironmentForThreadsImpl;

std::string QuicheGetTestMemoryCachePathImpl();

using QuicheScopedDisableExitOnDFatalImpl = net::test::ScopedDisableExitOnDFatal;
