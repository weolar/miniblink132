...
DEFINE_QUICHE_COMMAND_LINE_FLAG(int32_t, bar, 123, "A frog jumps into the pond,");
...
protected:
    void SetUp() override
    {
        QuicheFlagRegistry::GetInstance().ResetFlags();
    }

    static QuicheParseCommandLineFlagsResult QuicheParseCommandLineFlagsForTest(const char* usage, int argc, const char* const* argv)
    {
...
private:
    ...
    static void FillCommandLineArgs(int argc, const char* const* argv, std::vector<std::string>* v)
    {
    ...
    // std::u16string.
    static void FillCommandLineArgs(int argc, const char* const* argv, std::vector<std::u16string>* v)
    {
        for (int i = 0; i < argc; ++i) {
...
TEST_F(QuicheCommandLineFlagTest, NotSpecified)
{
    ...
    auto parse_result = QuicheParseCommandLineFlagsForTest("usage message", std::size(argv), argv);
...
{
    for (const char* s : { "--foo", "--foo=1", "--foo=t", "--foo=True", "--foo=Y", "--foo=yes" }) {
    ...
    for (const char* s : { "--foo=0", "--foo=f", "--foo=False", "--foo=N", "--foo=no" }) {
    ...
    for (const char* s : { "--foo=7", "--foo=abc", "--foo=trueish" }) {
        ...
        EXPECT_THAT(captured_stderr, testing::ContainsRegex("Invalid value.*for flag --foo"));
        EXPECT_TRUE(parse_result.non_flag_args.empty());
        EXPECT_FALSE(GetQuicheFlag(foo));
    }
}
...
TEST_F(QuicheCommandLineFlagTest, Int32Flag)
{
    ...
    for (const char* s : { "--bar", "--bar=a", "--bar=9999999999999" }) {
        ...
        EXPECT_TRUE(parse_result.exit_status.has_value());
        EXPECT_EQ(1, *parse_result.exit_status);
        EXPECT_THAT(captured_stderr, testing::ContainsRegex("Invalid value.*for flag --bar"));
        EXPECT_TRUE(parse_result.non_flag_args.empty());
...
TEST_F(QuicheCommandLineFlagTest, PrintHelp)
{
    ...
    EXPECT_THAT(captured_stdout, testing::ContainsRegex("--help +Print this help message."));
    EXPECT_THAT(captured_stdout, testing::ContainsRegex("--foo +An old silent pond..."));
    EXPECT_THAT(captured_stdout, testing::ContainsRegex("--bar +A frog jumps into the pond,"));
