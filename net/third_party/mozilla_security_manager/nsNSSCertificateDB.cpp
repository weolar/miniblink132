...
bool ImportCACerts(PK11SlotInfo* slot, const net::ScopedCERTCertificateList& certificates, CERTCertificate* root, net::NSSCertDatabase::TrustBits trustBits,
    net::NSSCertDatabase::ImportCertFailureList* not_imported)
{
    ...
    if (!CERT_IsCACert(root, NULL)) {
        not_imported->push_back(net::NSSCertDatabase::ImportCertFailure(net::x509_util::DupCERTCertificate(root), net::ERR_IMPORT_CA_CERT_NOT_CA));
        ...
        // settings?
        not_imported->push_back(net::NSSCertDatabase::ImportCertFailure(net::x509_util::DupCERTCertificate(root), net::ERR_IMPORT_CERT_ALREADY_EXISTS));
    } else {
        ...
        SECStatus srv = PK11_ImportCert(
            slot, root, CK_INVALID_HANDLE, net::x509_util::GetDefaultUniqueNickname(root, net::CA_CERT, slot).c_str(), PR_FALSE /* includeTrust (unused) */);
        if (srv != SECSuccess) {
    ...
    for (size_t i = 0; i < certificates.size(); i++) {
        ...
        if (!CERT_IsCACert(cert, NULL)) {
            not_imported->push_back(net::NSSCertDatabase::ImportCertFailure(net::x509_util::DupCERTCertificate(cert), net::ERR_IMPORT_CA_CERT_NOT_CA));
        ...
        if (cert_is_perm) {
            not_imported->push_back(net::NSSCertDatabase::ImportCertFailure(net::x509_util::DupCERTCertificate(cert), net::ERR_IMPORT_CERT_ALREADY_EXISTS));
            VLOG(1) << "skipping cert (perm)";
            continue;
        }

        if (CERT_VerifyCert(CERT_GetDefaultCertDB(), cert, PR_TRUE, certUsageVerifyCA, now, NULL, NULL) != SECSuccess) {
            ...
            // public.)
            not_imported->push_back(net::NSSCertDatabase::ImportCertFailure(net::x509_util::DupCERTCertificate(cert), net::ERR_FAILED));
        ...
        SECStatus srv = PK11_ImportCert(
            slot, cert, CK_INVALID_HANDLE, net::x509_util::GetDefaultUniqueNickname(cert, net::CA_CERT, slot).c_str(), PR_FALSE /* includeTrust (unused) */);
        if (srv != SECSuccess) {
            LOG(ERROR) << "PK11_ImportCert failed with error " << PORT_GetError();
            // TODO(mattm): Should we bail or continue on error here?  Mozilla doesn't
            // check error code at all.
            not_imported->push_back(net::NSSCertDatabase::ImportCertFailure(net::x509_util::DupCERTCertificate(cert), net::ERR_IMPORT_CA_CERT_FAILED));
        }
    }
...
bool ImportServerCert(PK11SlotInfo* slot, const net::ScopedCERTCertificateList& certificates, net::NSSCertDatabase::TrustBits trustBits,
    net::NSSCertDatabase::ImportCertFailureList* not_imported)
{
    ...
    for (size_t i = 0; i < certificates.size(); ++i) {
        ...
        // Mozilla uses CERT_ImportCerts, which doesn't take a slot arg.  We use
        // PK11_ImportCert instead.
        SECStatus srv = PK11_ImportCert(slot, cert, CK_INVALID_HANDLE, net::x509_util::GetDefaultUniqueNickname(cert, net::SERVER_CERT, slot).c_str(),
            PR_FALSE /* includeTrust (unused) */);
        if (srv != SECSuccess) {
            LOG(ERROR) << "PK11_ImportCert failed with error " << PORT_GetError();
            not_imported->push_back(net::NSSCertDatabase::ImportCertFailure(net::x509_util::DupCERTCertificate(cert), net::ERR_IMPORT_SERVER_CERT_FAILED));
            continue;
        }
    }
...
int ImportUserCert(CERTCertificate* cert, crypto::ScopedPK11Slot preferred_slot)
{
    ...
    SECKEYPrivateKey* private_key = PK11_FindKeyByDERCert(preferred_slot.get(), cert, nullptr);
    if (private_key) {
    ...
    // PK11_ImportCert instead.
    SECStatus srv = PK11_ImportCert(
        slot.get(), cert, key, net::x509_util::GetDefaultUniqueNickname(cert, net::USER_CERT, slot.get()).c_str(), PR_FALSE /* includeTrust (unused) */);
...
bool SetCertTrust(CERTCertificate* nsscert, net::CertType type, net::NSSCertDatabase::TrustBits trustBits)
{
    ...
    if ((trustBits & kSSLTrustBits) == kSSLTrustBits || (trustBits & kEmailTrustBits) == kEmailTrustBits
        || (trustBits & kObjSignTrustBits) == kObjSignTrustBits) {
        NOTREACHED() << "SetCertTrust called with conflicting trust bits " << trustBits;
    }
