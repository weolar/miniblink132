...
SECItem* PR_CALLBACK nickname_collision(SECItem* old_nick, PRBool* cancel, void* wincx)
{
    char* nick = NULL;
...
PRBool pip_ucs2_ascii_conversion_fn(
    PRBool toUnicode, unsigned char* inBuf, unsigned int inBufLen, unsigned char* outBuf, unsigned int maxOutBufLen, unsigned int* outBufLen, PRBool swapBytes)
{
...
int nsPKCS12Blob_ImportHelper(const char* pkcs12_data, size_t pkcs12_len, const std::u16string& password, bool is_extractable, bool try_zero_length_secitem,
    PK11SlotInfo* slot, net::ScopedCERTCertificateList* imported_certs)
{
    DCHECK(pkcs12_data);
    DCHECK(slot);
    ...
    srv = SEC_PKCS12DecoderUpdate(dcx, (unsigned char*)pkcs12_data, pkcs12_len);
    if (srv)
        goto finish;
    // verify the blob
    ...
    while (SEC_PKCS12DecoderIterateNext(dcx, &decoder_item) == SECSuccess) {
        ...
        net::ScopedCERTCertificate cert(PK11_FindCertFromDERCertItem(slot, decoder_item->der,
            NULL)); // wincx
        if (!cert) {
        ...
        if (decoder_item->hasKey && !is_extractable) {
            ...
            if (privKey) {
                ...
                srv = PK11_WriteRawAttribute(PK11_TypePrivKey, privKey, CKA_EXTRACTABLE, &attribute_value);
...
// SECSuccess.
SECStatus isExtractable(SECKEYPrivateKey* privKey, PRBool* extractable)
{
    SECItem value;
    SECStatus rv;
...
int nsPKCS12Blob_Import(PK11SlotInfo* slot, const char* pkcs12_data, size_t pkcs12_len, const std::u16string& password, bool is_extractable,
    net::ScopedCERTCertificateList* imported_certs)
{
    int rv = nsPKCS12Blob_ImportHelper(pkcs12_data, pkcs12_len, password, is_extractable, false, slot, imported_certs);
    ...
    //   flavors.
    if ((rv == net::ERR_PKCS12_IMPORT_BAD_PASSWORD || rv == net::ERR_PKCS12_IMPORT_INVALID_MAC) && password.empty()) {
        rv = nsPKCS12Blob_ImportHelper(pkcs12_data, pkcs12_len, password, is_extractable, true, slot, imported_certs);
    }
    return rv;
}
...
int nsPKCS12Blob_Export(std::string* output, const net::ScopedCERTCertificateList& certs, const std::u16string& password)
{
    int return_count = 0;
    SECStatus srv = SECSuccess;
    ...
    for (size_t i = 0; i < certs.size(); i++) {
        ...
        // the export behavior.
        if (nssCert->slot) {
            SECKEYPrivateKey* privKey = PK11_FindKeyByDERCert(nssCert->slot, nssCert,
                NULL); // wincx
            if (privKey) {
        ...
        if (!SEC_PKCS12IsEncryptionAllowed() || PK11_IsFIPS()) {
            ...
            certSafe = SEC_PKCS12CreatePasswordPrivSafe(ecx, &unicodePw, SEC_OID_PKCS12_V2_PBE_WITH_SHA1_AND_40_BIT_RC2_CBC);
        }
        if (!certSafe || !keySafe) {
        ...
        srv = SEC_PKCS12AddCertAndKey(
            ecx, certSafe, NULL, nssCert, CERT_GetDefaultCertDB(), keySafe, NULL, PR_TRUE, &unicodePw, SEC_OID_PKCS12_V2_PBE_WITH_SHA1_AND_3KEY_TRIPLE_DES_CBC);
        if (srv)
            goto finish;
        ++numCertsExported;
    }
