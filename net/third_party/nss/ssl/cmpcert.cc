...
bool GetIssuerAndSubject(CERTCertificate* cert, bssl::der::Input* issuer, bssl::der::Input* subject)
{
...
bool GetIssuerAndSubject(X509Certificate* cert, bssl::der::Input* issuer, bssl::der::Input* subject)
{
    bssl::der::Input tbs_certificate_tlv;
    bssl::der::Input signature_algorithm_tlv;
    bssl::der::BitString signature_value;
    if (!bssl::ParseCertificate(bssl::der::Input(CRYPTO_BUFFER_data(cert->cert_buffer()), CRYPTO_BUFFER_len(cert->cert_buffer())), &tbs_certificate_tlv,
...
bool MatchClientCertificateIssuers(X509Certificate* cert, const std::vector<std::string>& cert_authorities, ScopedCERTCertificateList* intermediates)
{
    ...
    while (intermediates->size() < kMaxDepth) {
        ...
        ScopedCERTCertificate nextcert(CERT_FindCertByName(CERT_GetDefaultCertDB(), &issuer_item));
        if (!nextcert)
            return false;
