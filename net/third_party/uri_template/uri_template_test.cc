...
std::unordered_map<string, string> parameters_ = { { "var", "value" }, { "hello", "Hello World!" }, { "path", "/foo/bar" }, { "empty", "" }, { "x", "1024" },
    { "y", "768" }, { "percent", "%31" }, { "bad_percent", "%1" }, { "escaped", " !\"#$%&'()*+,-./:;<=>?@[\\]^_`{|}~\x80\xFF" } };

void CheckExpansion(
    const string& uri_template, const string& expected_expansion, bool expected_validity = true, const std::set<string>* expected_vars = nullptr)
{
    string result;
    std::set<string> vars_found;
    EXPECT_EQ(expected_validity, Expand(uri_template, parameters_, &result, &vars_found));
...
TEST_F(UriTemplateTest, TestLevel2Templates)
{
    ...
    CheckExpansion("{+escaped}", "%20!%22#$%25&'()*+,-./:;%3C=%3E?@[%5C]%5E_%60%7B%7C%7D~%80%FF");
...
TEST_F(UriTemplateTest, TestVariableSet)
{
    ...
    CheckExpansion("{+path}{/z}{?y}&k=24", "/foo/bar?y=768&k=24", true, &expected_vars);
