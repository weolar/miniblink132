...
public:
    UriTemplateConfig(
        const char* prefix, const char* joiner, bool requires_variable_assignment, bool allow_reserved_expansion, bool no_variable_assignment_if_empty = false)
        : prefix_(prefix)
        , joiner_(joiner)
    ...
    void AppendValue(const string& variable, const string& value, bool use_prefix, string* target) const
    {
        ...
        if (requires_variable_assignment_) {
            if (value.empty() && no_variable_assignment_if_empty_) {
                ...
                target->append(joiner + EscapedValue(variable) + "=" + EscapedValue(value));
            }
        } else {
...
void ProcessVariableSection(string* variable_section, const std::unordered_map<string, string>& parameters, string* target, std::set<string>* vars_found)
{
    ...
    std::vector<string> variables = base::SplitString(*variable_section, ",", base::KEEP_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
    bool first_var = true;
...
bool Expand(const string& path_uri, const std::unordered_map<string, string>& parameters, string* target, std::set<string>* vars_found)
{
    size_t cur = 0;
